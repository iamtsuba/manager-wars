import{s as y,e as Y,g as ke,K as Ie,D as A,S as Me,a as z,H as Re,R as Ne,b as Be,r as Pe,c as qe,E as Oe,d as Fe,N as De,M as He,B as Ue,F as ie,f as ce,h as Ge,i as Ve}from"./formation-links-DJC25wD7.js";async function Ke(e){const[{count:i},{count:t},{count:n},{count:s},{count:l}]=await Promise.all([y.from("players").select("*",{count:"exact",head:!0}),y.from("clubs").select("*",{count:"exact",head:!0}),y.from("users").select("*",{count:"exact",head:!0}),y.from("cards").select("*",{count:"exact",head:!0}),y.from("matches").select("*",{count:"exact",head:!0})]);e.innerHTML=`
  ...
  </div>`}const pe={normal:"Normal",pepite:"Pépite",papyte:"Papyte",legende:"Légende"},Ye={normal:"#aaa",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"};async function Je(e,i){await W(e,i)}async function W(e,i){const{toast:t,openModal:n,closeModal:s}=i,[{data:l,error:o},{data:r}]=await Promise.all([y.from("players").select("*, clubs(encoded_name,logo_url)").order("surname_encoded"),y.from("clubs").select("id,encoded_name").order("encoded_name")]);if(o){e.innerHTML=`<p style="color:red">${o.message}</p>`;return}const d={};(r||[]).forEach(p=>{d[p.id]=p.encoded_name}),e.innerHTML=`
  ...
  `,s(X),document.getElementById("search-clubs").addEventListener("input",l=>{const o=l.target.value.toLowerCase();s(X.filter(r=>r.encoded_name.toLowerCase().includes(o)||r.real_name.toLowerCase().includes(o)))}),document.getElementById("add-club-btn").addEventListener("click",()=>{me(null,{toast:i,openModal:t,closeModal:n,reload:()=>Z(e,{toast:i,openModal:t,closeModal:n})})});function s(l){document.getElementById("clubs-tbody").innerHTML=l.map(o=>{var c;const r=Le(o),d=ke(r).replace("<svg ",'<svg style="width:36px;height:42px" ');return`
      <tr>
        <td>
        ...
        <td title="${((c=Ie[o.kit_style||"uni"])==null?void 0:c.label)||"Uni"}">${d}</td>
     ...
     <button class="btn btn-primary" id="m-save">${l?"Enregistrer":"Créer"}</button>`);function d(a,f){const g=document.getElementById(a),E=document.getElementById(f);!g||!E||(g.addEventListener("input",()=>{E.value=g.value,m()}),E.addEventListener("input",()=>{/^#[0-9a-fA-F]{6}$/.test(E.value)&&(g.value=E.value,m())}))}d("m-kit-color1","m-kit-color1-txt"),d("m-kit-color2","m-kit-color2-txt"),d("m-kit-shorts","m-kit-shorts-txt"),d("m-kit-socks","m-kit-socks-txt"),(p=document.getElementById("m-kit-style"))==null||p.addEventListener("change",m);function c(){var a,f,g,E,_;return{style:((a=document.getElementById("m-kit-style"))==null?void 0:a.value)||"uni",color1:((f=document.getElementById("m-kit-color1"))==null?void 0:f.value)||"#1A6B3C",color2:((g=document.getElementById("m-kit-color2"))==null?void 0:g.value)||"#ffffff",shorts:((E=document.getElementById("m-kit-shorts"))==null?void 0:E.value)||"#111111",socks:((_=document.getElementById("m-kit-socks"))==null?void 0:_.value)||"#ffffff"}}function m(){const a=document.getElementById("kit-preview-wrap");a&&(a.innerHTML=ke(c()))}m(),document.getElementById("m-cancel").addEventListener("click",n),document.getElementById("auto-encode").addEventListener("click",()=>{const a=document.getElementById("m-real").value,f=a.split(" ")[0]||a;document.getElementById("m-encoded").value=f.toUpperCase()+" FC"}),document.getElementById("m-real").addEventListener("input",()=>{const a=document.getElementById("m-real").value;if(!document.getElementById("m-encoded").value){const f=a.split(" ")[0]||a;document.getElementById("m-encoded").value=f.toUpperCase()+" FC"}}),document.getElementById("m-save").addEventListener("click",async()=>{const a=c(),f={real_name:document.getElementById("m-real").value.trim(),encoded_name:document.getElementById("m-encoded").value.trim().toUpperCase(),country_code:document.getElementById("m-country").value.trim().toUpperCase(),logo_url:document.getElementById("m-logo").value.trim()||null,kit_style:a.style,kit_color1:a.color1,kit_color2:a.color2,kit_shorts:a.shorts,kit_socks:a.socks};if(!f.real_name||!f.encoded_name||!f.country_code){i("Remplissez tous les champs obligatoires","error");return}const{error:g}=l?await y.from("clubs").update(f).eq("id",e.id):await y.from("clubs").insert(f);if(g){i(g.message,"error");return}i(l?"Club modifié ✅":"Club créé ✅","success"),n(),s()})}const Ze=["normal","pepite","papyte","legende"],ge=["GK","DEF","MIL","ATT"],Qe=[["MA","Maroc"],["FR","France"],["AR","Argentine"],["PT","Portugal"],["BR","Brésil"],["ES","Espagne"],["DE","Allemagne"],["GB","Angleterre"],["IT","Italie"],["CM","Cameroun"],["SN","Sénégal"],["NG","Nigéria"],["DK","Danemark"],["NL","Pays-Bas"],["BE","Belgique"],["CI","Côte d'Ivoire"],["AL","Albanie"],["HR","Croatie"],["RS","Serbie"],["TR","Turquie"],["MW","Malawi"]];let q={...z},Q=[];async function et(e,{toast:i}){const{data:t}=await y.from("clubs").select("id,encoded_name,logo_url,kit_style,kit_color1,kit_color2,kit_shorts,kit_socks").order("encoded_name");Q=t||[],q={...z},e.innerHTML=ot(Q),it(e,Q,i),j()}function be(){var t;const e=((t=document.getElementById("cb-surname-real"))==null?void 0:t.value)||"",i=document.getElementById("cb-surname-enc");i&&(i.value=Y(e))}function tt(){var t;const e=(t=document.getElementById("cb-club"))==null?void 0:t.value,i=Q.find(n=>String(n.id)===String(e));return i?{style:i.kit_style||A.style,color1:i.kit_color1||A.color1,color2:i.kit_color2||A.color2,shorts:i.kit_shorts||A.shorts,socks:i.kit_socks||A.socks}:{...A}}function j(){var a,f,g,E;const e=_=>{var u;return((u=document.getElementById(_))==null?void 0:u.value)||""},i={firstname:e("cb-firstname"),surname_encoded:e("cb-surname-enc")||Y(e("cb-surname-real")),country_code:e("cb-country"),club_encoded_name:((f=(a=document.getElementById("cb-club"))==null?void 0:a.selectedOptions[0])==null?void 0:f.text)||"",job:e("cb-job"),job2:e("cb-job2")||null,note_g:parseInt(e("cb-note-g"))||0,note_d:parseInt(e("cb-note-d"))||0,note_m:parseInt(e("cb-note-m"))||0,note_a:parseInt(e("cb-note-a"))||0,rarity:e("cb-rarity")},t=tt(),n=document.getElementById("cb-club"),s=((E=(g=n==null?void 0:n.selectedOptions[0])==null?void 0:g.dataset)==null?void 0:E.logo)||null,l=Be(q,t,120),o=`data:image/svg+xml;charset=utf-8,${encodeURIComponent(l)}`,r=Pe(i,{portraitUrl:o,clubLogoUrl:s,showNotes:!0}),d=document.getElementById("card-preview");d&&(d.innerHTML=r);const c=document.getElementById("avatar-preview-wrap");c&&(c.innerHTML=l);const m=e("cb-surname-real"),p=document.getElementById("encode-summary");p&&m&&(p.textContent=`${e("cb-firstname")} ${m} → ${e("cb-firstname")} ${i.surname_encoded}`)}function ze(){const e=i=>{var t;return((t=document.getElementById(i))==null?void 0:t.value)||""};q={skin:e("av-skin"),hair:e("av-hair"),hairColor:e("av-hairColor"),eyes:e("av-eyes"),nose:e("av-nose"),mouth:e("av-mouth"),eyebrows:e("av-eyebrows"),beard:e("av-beard")}}function it(e,i,t){var s,l,o,r,d;e.querySelectorAll("input,select").forEach(c=>{c.addEventListener("input",j),c.addEventListener("change",j)});const n=["av-skin","av-hair","av-hairColor","av-eyes","av-nose","av-mouth","av-eyebrows","av-beard"];n.forEach(c=>{var m;(m=document.getElementById(c))==null||m.addEventListener("change",()=>{ze(),j()})}),(s=document.getElementById("cb-surname-real"))==null||s.addEventListener("input",()=>{be(),j()}),(l=document.getElementById("btn-encode"))==null||l.addEventListener("click",()=>{be(),j()}),(o=document.getElementById("cb-club"))==null||o.addEventListener("change",j),(r=document.getElementById("btn-save-player"))==null||r.addEventListener("click",()=>nt(t)),(d=document.getElementById("btn-reset"))==null||d.addEventListener("click",()=>{e.querySelectorAll("input").forEach(c=>c.value=""),e.querySelectorAll("select").forEach(c=>c.selectedIndex=0),q={...z},n.forEach(c=>{const m=c.replace("av-",""),p=document.getElementById(c);p&&(p.value=z[m]||"")}),j()})}async function nt(e){const i=o=>{var r;return((r=document.getElementById(o))==null?void 0:r.value)||""},t=i("cb-surname-real").trim(),n=i("cb-surname-enc").trim()||Y(t);if(!i("cb-firstname")||!t||!i("cb-country")||!i("cb-job")){e("Remplissez les champs obligatoires (prénom, nom, pays, poste)","error");return}ze();const s={firstname:i("cb-firstname").trim(),surname_real:t,surname_encoded:n,country_code:i("cb-country"),club_id:i("cb-club")||null,job:i("cb-job"),job2:i("cb-job2")||null,note_g:parseInt(i("cb-note-g"))||0,note_d:parseInt(i("cb-note-d"))||0,note_m:parseInt(i("cb-note-m"))||0,note_a:parseInt(i("cb-note-a"))||0,rarity:i("cb-rarity")||"normal",note_min:parseInt(i("cb-note-min"))||null,note_max:parseInt(i("cb-note-max"))||null,sell_price:parseInt(i("cb-price"))||0,avatar_config:q,skin:q.skin||"blanc",hair:q.hairColor||"marron",hair_length:"court"},{error:l}=await y.from("players").insert(s);if(l){e("Erreur: "+l.message,"error");return}e(`✅ Joueur "${s.firstname} ${s.surname_encoded}" enregistré !`,"success")}function ot(e){const i=Qe.map(([c,m])=>`<option value="${c}">${m} (${c})</option>`).join(""),t='<option value="">— Sélectionner un club —</option>'+e.map(c=>`<option value="${c.id}" data-logo="${c.logo_url||""}">${c.encoded_name}</option>`).join(""),n=ge.map(c=>`<option value="${c}">${c}</option>`).join(""),s='<option value="">Aucun</option>'+ge.map(c=>`<option value="${c}">${c}</option>`).join(""),l=Ze.map(c=>`<option value="${c}">${c.charAt(0).toUpperCase()+c.slice(1)}</option>`).join(""),o=(c,m,p)=>Object.entries(m).map(([a,f])=>`<option value="${a}" ${a===(p||"")?"selected":""}>${f.label}</option>`).join(""),r=Object.keys(Me).map(c=>`<option value="${c}" ${c===z.skin?"selected":""}>${c.charAt(0).toUpperCase()+c.slice(1)}</option>`).join(""),d=Object.keys(Re).map(c=>`<option value="${c}" ${c===z.hairColor?"selected":""}>${c.charAt(0).toUpperCase()+c.slice(1)}</option>`).join("");return`
      ...
      <div class="card-panel">
        ...
        <div style="margin-top:8px;display:flex;gap:8px">
          ${["normal","pepite","papyte","legende"].map(c=>`
            <div style="background:${Ne[c]};color:#fff;padding:3px 8px;border-radius:4px;font-size:11px;font-weight:700">
              ${c.toUpperCase()}
            </div>`).join("")}
        </div>
      </div>
  ...
  `;const s=t||[];l(s),document.getElementById("search-users").addEventListener("input",o=>{const r=o.target.value.toLowerCase();l(s.filter(d=>{var c;return d.pseudo.toLowerCase().includes(r)||((c=d.club_name)==null?void 0:c.toLowerCase().includes(r))}))});function l(o){document.getElementById("users-tbody").innerHTML=o.map(r=>`
    ...
    `).join(""),document.querySelectorAll("[data-toggle-admin]").forEach(r=>{r.addEventListener("click",async()=>{const d=r.dataset.isAdmin!=="true";if(!confirm(`Voulez-vous ${d?"rendre admin":"retirer le rôle admin"} de ce Manager ?`))return;const{error:m}=await y.from("users").update({is_admin:d}).eq("id",r.dataset.toggleAdmin);m?i(m.message,"error"):(i("Rôle mis à jour ✅","success"),Ae(e,{toast:i}))})})}}async function Se(e,{toast:i}){const{data:t,error:n}=await y.from("market_listings").select(`
  ...
  `,document.querySelectorAll("[data-cancel]").forEach(r=>{r.addEventListener("click",async()=>{if(!confirm("Annuler cette annonce ?"))return;const{error:d}=await y.from("market_listings").update({status:"cancelled"}).eq("id",r.dataset.cancel);d?i(d.message,"error"):(i("Annonce annulée","success"),Se(e,{toast:i}))})})}async function at(e,{toast:i}){e.innerHTML=`
...
`;t.forEach(l=>{s+=[l.real_name,l.encoded_name,l.country_code,l.logo_url||""].map(fe).join(",")+`
...
`).filter(o=>o.trim());if(i.length<2)return[];const t=i[0],n=t.split(";").length>t.split(",").length?";":",",s=ve(t,n).map(o=>o.trim().replace(/^\uFEFF/,"").toLowerCase()),l=[];for(let o=1;o<i.length;o++){if(!i[o].trim())continue;const r=ve(i[o],n),d={};s.forEach((c,m)=>{d[c]=(r[m]||"").trim()}),!Object.values(d).every(c=>!c)&&l.push(d)}return l}function ve(e,i=","){const t=[];let n="",s=!1;for(let l=0;l<e.length;l++){const o=e[l];s?o==='"'?e[l+1]==='"'?(n+='"',l++):s=!1:n+=o:o==='"'?s=!0:o===i?(t.push(n),n=""):n+=o}return t.push(n),t}const rt={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};async function lt(e,{toast:i}){const t=Object.keys(ie);let n=t[0];const{data:s}=await y.from("formation_links_overrides").select("formation, links"),l={};(s||[]).forEach(m=>{l[m.formation]=m.links});let o=new Set;function r(m,p){return[m,p].sort().join("-")}function d(m){const p=l[m]||ie[m]||[];o=new Set(p.map(([a,f])=>r(a,f)))}d(n),e.innerHTML=`
  ...
  `;function c(){const m=Ge[n]||{},p=ce(n),a=320,f=400,g=22;function E(b){const h=m[b];return h?{x:h.x*a,y:h.y*f}:null}let _=`<svg width="${a}" height="${f}" viewBox="0 0 ${a} ${f}" xmlns="http://www.w3.org/2000/svg" style="display:block">`;p.forEach(([b,h],v)=>{const x=E(b),w=E(h);if(!x||!w)return;const B=r(b,h),$=o.has(B),S=$?"#3b82f6":"#999",M=$?.95:.35,R=$?4:3;_+=`<line x1="${x.x}" y1="${x.y}" x2="${w.x}" y2="${w.y}"
    ...
    <div style="display:flex;height:calc(100vh - 56px);overflow:hidden">
      ...
      ${!l()||f?d():""}
        ...
        </div>`,b.querySelectorAll(".icon-pick-item").forEach($=>{$.addEventListener("click",()=>{const S=p("#f-image-url");S&&(S.value=$.dataset.name),b.style.display="none"})})}catch(v){b.innerHTML=`<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Erreur API GitHub : ${v.message}</div>`}}});function a(){e.querySelectorAll("[data-rate-idx]").forEach(b=>{var v,x,w,B,$;const h=Number(b.dataset.rateIdx);h>=0&&h<n.length&&(n[h].card_type=((v=b.querySelector(".rate-type"))==null?void 0:v.value)||"player",n[h].rarity=((x=b.querySelector(".rate-rarity"))==null?void 0:x.value)||null,n[h].note_min=Number((w=b.querySelector(".rate-note-min"))==null?void 0:w.value)||null,n[h].note_max=Number((B=b.querySelector(".rate-note-max"))==null?void 0:B.value)||null,n[h].percentage=Number(($=b.querySelector(".rate-pct"))==null?void 0:$.value)||0)})}(I=p("#btn-add-rate"))==null||I.addEventListener("click",()=>{a(),n.push({id:null,booster_id:t,card_type:"player",rarity:"normal",note_min:null,note_max:null,percentage:10,sort_order:n.length}),c(!0)}),e.querySelectorAll(".btn-del-rate").forEach(b=>{b.addEventListener("click",()=>{a(),n.splice(Number(b.dataset.idx),1),c(!0)})}),(k=p("#btn-save"))==null||k.addEventListener("click",async()=>{var B,$,S,M,R,D,H,U,G;const b=(i||[]).find(L=>L.id===t);if(!b)return;const h=[];e.querySelectorAll("[data-rate-idx]").forEach(L=>{var T,V,le,de,se;const O=Number(L.dataset.rateIdx);h.push({booster_id:t,card_type:((T=L.querySelector(".rate-type"))==null?void 0:T.value)||"player",rarity:((V=L.querySelector(".rate-rarity"))==null?void 0:V.value)||null,note_min:Number((le=L.querySelector(".rate-note-min"))==null?void 0:le.value)||null,note_max:Number((de=L.querySelector(".rate-note-max"))==null?void 0:de.value)||null,percentage:Number((se=L.querySelector(".rate-pct"))==null?void 0:se.value)||0,sort_order:O});const C=h[h.length-1];C.rarity||(C.rarity=null),C.note_min||(C.note_min=null),C.note_max||(C.note_max=null)});const v=h.reduce((L,O)=>L+O.percentage,0);if(h.length&&Math.abs(v-100)>.5){alert(`La somme doit faire 100% (actuellement ${v.toFixed(1)}%)`);return}const x={name:(($=(B=p("#f-name"))==null?void 0:B.value)==null?void 0:$.trim())||b.name,image_url:((M=(S=p("#f-image-url"))==null?void 0:S.value)==null?void 0:M.trim())||null,price_type:(R=p("#f-price-type"))==null?void 0:R.value,price_credits:Number((D=p("#f-price-credits"))==null?void 0:D.value)||0,card_count:Number((H=p("#f-card-count"))==null?void 0:H.value)||5,is_active:((U=p("#f-active"))==null?void 0:U.checked)??b.is_active,sort_order:Number((G=p("#f-sort"))==null?void 0:G.value)||0},{error:w}=await y.from("booster_configs").update(x).eq("id",t);if(w){alert(w.message);return}if(Object.assign(b,x),await y.from("booster_drop_rates").delete().eq("booster_id",t),h.length){const{error:L}=await y.from("booster_drop_rates").insert(h);if(L){alert(L.message);return}}n=h,s=t,alert("✅ Booster enregistré !"),c(!0)})}c()}const Ce=[{value:"normal",label:"Normal",color:"#888"},{value:"pepite",label:"Pépite",color:"#D4A017"},{value:"papyte",label:"Papyte",color:"#909090"},{value:"legende",label:"Légende",color:"#7a28b8"}];async function ut(e){await oe(e)}async function oe(e){const{data:i,error:t}=await y.from("sell_price_configs").select("*").order("rarity").order("note_min");if(t){e.innerHTML=`<div style="padding:24px;color:red">Erreur : ${t.message}</div>`;return}e.innerHTML=`
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px">
          <div>
            ...
            <select id="form-rarity" style="width:100%;padding:9px;border:1.5px solid #ddd;border-radius:8px;font-size:14px">
              ${Ce.map(n=>`<option value="${n.value}">${n.label}</option>`).join("")}
            </select>
          </div>
          <div>
    ...
    </div>`,document.getElementById("add-config").addEventListener("click",()=>ye(null)),document.getElementById("form-cancel").addEventListener("click",()=>{document.getElementById("config-form").style.display="none"}),document.getElementById("form-save").addEventListener("click",()=>gt(e)),e.querySelectorAll("[data-edit]").forEach(n=>{const s=(i||[]).find(l=>l.id===n.dataset.edit);s&&n.addEventListener("click",()=>ye(s))}),e.querySelectorAll("[data-delete]").forEach(n=>{n.addEventListener("click",async()=>{confirm("Supprimer cette règle ?")&&(await y.from("sell_price_configs").delete().eq("id",n.dataset.delete),await oe(e))})})}function mt(e){const i=Ce.find(t=>t.value===e.rarity);return`
    ...
    </div>`,document.getElementById("add-article").addEventListener("click",()=>he(null)),document.getElementById("form-cancel").addEventListener("click",()=>{document.getElementById("article-form").style.display="none"}),document.getElementById("form-save").addEventListener("click",()=>xt(e)),e.querySelectorAll("[data-edit]").forEach(t=>{const n=(i||[]).find(s=>s.id===t.dataset.edit);n&&t.addEventListener("click",()=>he(n))}),e.querySelectorAll("[data-delete]").forEach(t=>{t.addEventListener("click",async()=>{confirm("Supprimer cet article ?")&&(await y.from("patch_notes").delete().eq("id",t.dataset.delete),await ae(e))})})}function ft(e){const i=new Date(e.published_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"short",year:"numeric"});return`
    ...
    <div style="padding:20px;max-width:820px;margin:0 auto">
      ...
      <div id="ts-form" style="display:none;margin-top:24px;background:#fff;border-radius:12px;padding:20px;box-shadow:0 1px 8px rgba(0,0,0,0.1)">
        ...
        <div style="display:flex;gap:10px">
          ...
          </div>`).join("")+"</div>",d.querySelectorAll("[data-pick]").forEach($=>{$.addEventListener("click",()=>{l&&(l.value=$.dataset.pick),c(),d.style.display="none"})})}catch(v){d.innerHTML='<div style="color:#cc2222;font-size:12px">Erreur chargement : '+v.message+"</div>"}}),(b=document.getElementById("ts-color"))==null||b.addEventListener("input",v=>{const x=document.getElementById("ts-color-hex");x&&(x.value=v.target.value)}),(h=document.getElementById("ts-color-hex"))==null||h.addEventListener("input",v=>{const x=v.target.value;if(/^#[0-9a-fA-F]{6}$/.test(x)){const w=document.getElementById("ts-color");w&&(w.value=x)}}),e.querySelectorAll("[data-edit]").forEach(v=>{const x=(i||[]).find(w=>w.id===v.dataset.edit);x&&v.addEventListener("click",()=>Ee(x))}),e.querySelectorAll("[data-delete]").forEach(v=>{v.addEventListener("click",async()=>{confirm("Supprimer cette étape ?")&&(await y.from("tutorial_steps").delete().eq("id",v.dataset.delete),F(e))})}),e.querySelectorAll("[data-toggle]").forEach(v=>{v.addEventListener("click",async()=>{const x=(i||[]).find(w=>w.id===v.dataset.toggle);x&&(await y.from("tutorial_steps").update({is_active:!x.is_active}).eq("id",x.id),F(e))})}),e.querySelectorAll("[data-up]").forEach(v=>{v.addEventListener("click",async()=>{const x=i||[],w=x.findIndex(B=>B.id===v.dataset.up);w<=0||(await Promise.all([y.from("tutorial_steps").update({step_order:x[w-1].step_order}).eq("id",x[w].id),y.from("tutorial_steps").update({step_order:x[w].step_order}).eq("id",x[w-1].id)]),F(e))})}),e.querySelectorAll("[data-down]").forEach(v=>{v.addEventListener("click",async()=>{const x=i||[],w=x.findIndex(B=>B.id===v.dataset.down);w<0||w>=x.length-1||(await Promise.all([y.from("tutorial_steps").update({step_order:x[w+1].step_order}).eq("id",x[w].id),y.from("tutorial_steps").update({step_order:x[w].step_order}).eq("id",x[w+1].id)]),F(e))})})}function yt(e){return`
  <div style="background:#fff;border-radius:10px;padding:12px 16px;box-shadow:0 1px 4px rgba(0,0,0,0.08);display:flex;align-items:center;gap:12px">
    ...
    <div style="padding:20px;max-width:820px;margin:0 auto">
      ...
      <div style="display:flex;flex-direction:column;gap:10px" id="st-list">
        ${(i||[]).map(r=>kt(r)).join("")||'<div style="color:#aaa;padding:24px;text-align:center">Aucun stade créé.</div>'}
      </div>
          ...
          <div style="flex:1;min-width:220px;display:flex;flex-direction:column;gap:12px">
            ...
            <div>
              ...
              <select id="st-country" style="width:100%;box-sizing:border-box;padding:9px;border:1.5px solid #ddd;border-radius:8px;font-size:13px;background:#fff">
                ...
                ${_t.map(([r,d])=>`<option value="${r}">${d} (${r})</option>`).join("")}
              </select>
            </div>
            <div>
    ...
    </div>`,(n=document.getElementById("st-add-btn"))==null||n.addEventListener("click",()=>we(null)),(s=document.getElementById("st-cancel"))==null||s.addEventListener("click",()=>{document.getElementById("st-form").style.display="none"}),(l=document.getElementById("st-save"))==null||l.addEventListener("click",()=>Bt(e)),(o=document.getElementById("st-preview-btn"))==null||o.addEventListener("click",()=>ee()),["st-name","st-club","st-country","st-image"].forEach(r=>{var d,c;(d=document.getElementById(r))==null||d.addEventListener("input",ee),(c=document.getElementById(r))==null||c.addEventListener("change",ee)}),e.querySelectorAll("[data-edit]").forEach(r=>{const d=(i||[]).find(c=>c.id===r.dataset.edit);d&&r.addEventListener("click",()=>we(d))}),e.querySelectorAll("[data-delete]").forEach(r=>{r.addEventListener("click",async()=>{confirm("Supprimer ce stade ?")&&(await y.from("stadium_definitions").delete().eq("id",r.dataset.delete),re(e))})})}function kt(e){var t,n;const i=(t=e.club)!=null&&t.logo_url?`<img src="${e.club.logo_url}" style="width:32px;height:32px;object-fit:contain">`:'<span style="font-size:20px">🌍</span>';return`
    <div style="background:#fff;border-radius:10px;padding:12px 16px;box-shadow:0 1px 4px rgba(0,0,0,0.08);display:flex;align-items:center;gap:12px">
  ...
  </div>`}async function Bt(e){const i=document.getElementById("st-id").value,t=document.getElementById("st-name").value.trim(),n=document.getElementById("st-club").value||null,s=document.getElementById("st-country").value.trim().toUpperCase()||null,l=document.getElementById("st-image").value.trim()||null;if(!t){alert("Le nom est obligatoire");return}const o={name:t,club_id:n,country_code:s,image_url:l},{error:r}=i?await y.from("stadium_definitions").update(o).eq("id",i):await y.from("stadium_definitions").insert(o);if(r){alert("Erreur : "+r.message);return}document.getElementById("st-form").style.display="none",re(e)}const N=[{value:"game_changer",label:"⚡ Game Changer",border:"#7a28b8",bg:"linear-gradient(135deg,#4a0080,#7a28b8)"},{value:"ultra_game_changer",label:"💎 Ultra Game Changer",border:"#00bcd4",bg:"linear-gradient(135deg,#006080,#00bcd4)"}],P=[{value:"purple",label:"Violet",hex:"#7a28b8"},{value:"light_blue",label:"Bleu clair",hex:"#00bcd4"}];async function Lt(e){e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">Chargement...</div>';const{data:i}=await y.from("gc_definitions").select("*").order("sort_order").order("created_at");let t=null;const n=[{value:"BOOST_STAT",label:"⚡ Boost stats (+N)",hasValue:!0,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"DEBUFF_STAT",label:"💀 Debuff stats (-N)",hasValue:!0,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"GRAY_PLAYER",label:"❄️ Griser un joueur",hasValue:!1,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"REVIVE_PLAYER",label:"💫 Ressusciter un joueur",hasValue:!1,hasTarget:!1,hasCount:!0,hasRoles:!1},{value:"REMOVE_GOAL",label:"🚫 Retirer un but adverse",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1},{value:"ADD_GOAL_DRAW",label:"⚽ +1 but si duel nul",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1},{value:"ADD_SUB",label:"🔄 Ajouter un remplacement",hasValue:!1,hasTarget:!1,hasCount:!0,hasRoles:!1},{value:"CUSTOM",label:"🛠️ Effet custom (hardcodé)",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1}],s=["GK","DEF","MIL","ATT"],l=()=>window.innerWidth<700,o=a=>e.querySelector(a);function r(a,f=130){if(!a)return"";const g=N.find(k=>k.value===a.gc_type)||N[0],E=P.find(k=>k.value===a.color)||P[0],_=Math.round(f*.55),u=Math.round(f*.15),I=Math.round(f*.3);return`<div style="width:${f}px;height:${Math.round(f*1.4)}px;border-radius:10px;border:3px solid ${E.hex};background:${g.bg};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 16px ${E.hex}55;flex-shrink:0">
    ...
    <div id="gc-list" style="background:#fff;${l()?"":"border-right:1px solid #e0e0e0;width:280px;flex-shrink:0;"}overflow-y:auto;height:100%">
      ...
      ${(i||[]).map(a=>{const f=N.find(E=>E.value===a.gc_type)||N[0],g=P.find(E=>E.value===a.color)||P[0];return`
    ...
    </div>`;const f=N.find(g=>g.value===a.gc_type)||N[0];return P.find(g=>g.value===a.color)||P[0],`
    ...
    <div style="display:flex;height:calc(100vh - 56px);overflow:hidden">
      ...
      ${!l()||t?c():""}
        ...
        </div>`,u.querySelectorAll(".gc-icon-item").forEach(x=>{x.addEventListener("click",()=>{const w=o("#gc-image-url");w&&(w.value=x.dataset.name),u.style.display="none"})})}catch(k){u.innerHTML=`<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Erreur API GitHub : ${k.message}</div>`}}}),(_=o("#btn-save-gc"))==null||_.addEventListener("click",async()=>{var x,w,B,$,S,M,R,D,H,U,G,L,O,C;const u=i.find(T=>T.id===t);if(!u)return;const I=n.find(T=>{var V;return T.value===(((V=o("#gc-effect-type"))==null?void 0:V.value)||"BOOST_STAT")})||n[0],k=I.hasRoles?[...e.querySelectorAll(".gc-p-role:checked")].map(T=>T.value):null,b={...I.hasValue?{value:Number((x=o("#gc-p-value"))==null?void 0:x.value)||2}:{},...I.hasCount?{count:Number((w=o("#gc-p-count"))==null?void 0:w.value)||1}:{},...I.hasTarget?{target:((B=o("#gc-p-target"))==null?void 0:B.value)||"home"}:{},...I.hasRoles?{roles:k!=null&&k.length?k:null}:{}},h={name:((S=($=o("#gc-name"))==null?void 0:$.value)==null?void 0:S.trim())||u.name,effect:((R=(M=o("#gc-effect"))==null?void 0:M.value)==null?void 0:R.trim())||null,image_url:((H=(D=o("#gc-image-url"))==null?void 0:D.value)==null?void 0:H.trim())||null,gc_type:((U=o("#gc-type"))==null?void 0:U.value)||"game_changer",color:((G=o("#gc-color"))==null?void 0:G.value)||"purple",sort_order:Number((L=o("#gc-sort"))==null?void 0:L.value)||0,is_active:((O=o("#gc-active"))==null?void 0:O.checked)??u.is_active,effect_type:((C=o("#gc-effect-type"))==null?void 0:C.value)||"BOOST_STAT",effect_params:b},{error:v}=await y.from("gc_definitions").update(h).eq("id",t);if(v){alert(v.message);return}Object.assign(u,h),alert("✅ Carte enregistrée !"),m()})}m()}Ve(Be);function zt(e,i="info"){const t=document.getElementById("toast");t.textContent=e,t.className=`show ${i}`,clearTimeout(t._t),t._t=setTimeout(()=>{t.className=""},3e3)}function At(e,i,t=""){document.getElementById("modal-title").textContent=e,document.getElementById("modal-body").innerHTML=i,document.getElementById("modal-footer").innerHTML=t,document.getElementById("modal-overlay").classList.remove("hidden")}function ne(){document.getElementById("modal-overlay").classList.add("hidden")}const _e={dashboard:{title:"Dashboard",fn:Ke},players:{title:"Joueurs & Cartes",fn:Je},clubs:{title:"Clubs",fn:We},"card-builder":{title:"Card Builder",fn:et},users:{title:"Managers",fn:Ae},market:{title:"Marché des transferts",fn:Se},"import-export":{title:"Import / Export CSV",fn:at},formations:{title:"Formations & Liens",fn:lt},"boosters-config":{title:"Boosters",fn:pt},"sell-price":{title:"Prix vente directe",fn:ut},journal:{title:"Journal",fn:bt},tutorial:{title:"Tutoriel",fn:vt},stadiums:{title:"Stades",fn:$t},"gc-cards":{title:"Game Changers",fn:Lt}};async function te(e){document.querySelectorAll(".admin-sidebar nav a").forEach(s=>{s.classList.toggle("active",s.dataset.page===e)});const i=document.getElementById("mobile-page-select");i&&(i.value=e);const t=_e[e]||_e.dashboard;document.getElementById("page-title").textContent=t.title;const n=document.getElementById("page-content");n.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">Chargement…</div>';try{await t.fn(n,{toast:zt,openModal:At,closeModal:ne,navigate:te})}catch(s){n.innerHTML=`<div style="padding:40px;color:#c0392b">Erreur: ${s.message}</div>`,console.error(s)}}async function St(){var i;const{data:{session:e}}=await y.auth.getSession();e&&await $e(e.user),document.getElementById("auth-btn").addEventListener("click",async()=>{const t=document.getElementById("auth-email").value.trim(),n=document.getElementById("auth-password").value,s=document.getElementById("auth-error");if(s.textContent="",!t||!n){s.textContent="Remplissez tous les champs.";return}const{data:l,error:o}=await y.auth.signInWithPassword({email:t,password:n});if(o){s.textContent=o.message;return}await $e(l.user)}),document.getElementById("auth-password").addEventListener("keydown",t=>{t.key==="Enter"&&document.getElementById("auth-btn").click()}),document.getElementById("logout-btn").addEventListener("click",async()=>{await y.auth.signOut(),document.getElementById("auth-screen").style.display="flex",document.getElementById("admin-app").style.display="none"}),document.getElementById("modal-close").addEventListener("click",ne),document.getElementById("modal-overlay").addEventListener("click",t=>{t.target===t.currentTarget&&ne()}),document.querySelectorAll(".admin-sidebar nav a").forEach(t=>{t.addEventListener("click",n=>{n.preventDefault(),te(t.dataset.page)})}),(i=document.getElementById("mobile-page-select"))==null||i.addEventListener("change",t=>{te(t.target.value)})}async function $e(e){const{data:i,error:t}=await y.from("users").select("*").eq("id",e.id).single(),n=document.getElementById("auth-error");if(t||!i){n.textContent="Profil introuvable.";return}if(!i.is_admin){n.textContent="Accès refusé. Vous n'êtes pas administrateur.",await y.auth.signOut();return}document.getElementById("auth-screen").style.display="none",document.getElementById("admin-app").style.display="flex",document.getElementById("admin-user").textContent=`Admin: ${i.pseudo}`,te("dashboard")}St();
