import{s as _,r as Ee,a as _e,A as ht,l as Et,g as _t,K as Me,D as z,b as wt,S as It,c as R,H as $t,R as kt,e as ue,d as Qe,f as Lt,h as At,E as Bt,i as St,N as Ct,M as Tt,B as Mt,j as zt,F as ge,k as ze,m as Rt,n as Xe,o as ye,p as jt}from"./special-cards-yI9ueqAE.js";async function Dt(e){var E;const[{count:t},{count:n},{count:i},{count:d},{count:l},{data:g}]=await Promise.all([_.from("players").select("*",{count:"exact",head:!0}),_.from("clubs").select("*",{count:"exact",head:!0}),_.from("users").select("*",{count:"exact",head:!0}),_.from("stadium_definitions").select("*",{count:"exact",head:!0}),_.from("users").select("*",{count:"exact",head:!0}).gt("last_seen_at",new Date(Date.now()-18e4).toISOString()),_.rpc("get_signup_password")]),x=g||"";e.innerHTML=`
  ...
  `,window.adminNav=a=>{var c;(c=document.querySelector(`[data-page="${a}"]`))==null||c.click()},(E=document.getElementById("signup-pwd-save"))==null||E.addEventListener("click",async()=>{const a=document.getElementById("signup-pwd-input").value.trim(),c=document.getElementById("signup-pwd-status");if(!a){c.textContent="Le code ne peut pas être vide.",c.style.color="#ff6b6b";return}const o=document.getElementById("signup-pwd-save");o.disabled=!0,o.textContent="⏳...";const{error:r}=await _.rpc("set_signup_password",{new_password:a});if(o.disabled=!1,o.textContent="💾 Enregistrer",r){c.textContent=r.message,c.style.color="#ff6b6b";return}c.textContent="✅ Code mis à jour.",c.style.color="#2ecc71"})}const et={normal:"Normal",pepite:"Pépite",papyte:"Papyte",legende:"Légende"},Pt={GK:"#aaaaaa",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};async function Nt(){const e={};for(const t of ht)e[t]=await Et(t);return e}async function Ft(e,t){const{toast:n}=t;if(!confirm(`Réattribuer une nouvelle photo à tous les joueurs ayant encore l'ancien format de chemin (public/faces/...) ?

Cette action est irréversible.`))return;const i=document.getElementById("fix-old-faces-btn");i&&(i.disabled=!0,i.textContent="⏳ En cours...");try{const{data:d,error:l}=await _.from("players").select("id, country_code, face").like("face","public/faces/%");if(l){n(l.message,"error");return}if(!(d!=null&&d.length)){n("Aucun joueur avec l'ancien format — rien à faire ✅","success");return}const{data:g}=await _.from("players").select("face").not("face","is",null),x=new Set((g||[]).map(c=>c.face).filter(c=>c&&!c.startsWith("public/faces/")));let E=0,a=0;for(const c of d){const o=await _e(c.country_code,x);if(!o){a++;continue}const{error:r}=await _.from("players").update({face:o}).eq("id",c.id);if(r){a++;continue}x.add(o),E++}n(`${E} photo(s) réattribuée(s)${a?`, ${a} échec(s)`:""} ✅`,a?"error":"success")}finally{i&&(i.disabled=!1,i.textContent="🖼️ Réattribuer anciennes photos")}}async function Ot(e,t){await se(e,t)}async function se(e,t,n=null){var a,c,o,r,u;const{toast:i}=t;n||(n={search:((a=document.getElementById("search-players"))==null?void 0:a.value)||"",job:((c=document.getElementById("filter-job"))==null?void 0:c.value)||"",rarity:((o=document.getElementById("filter-rarity"))==null?void 0:o.value)||"",club:((r=document.getElementById("filter-club"))==null?void 0:r.value)||"",country:((u=document.getElementById("filter-country"))==null?void 0:u.value)||""});const[{data:d,error:l},{data:g}]=await Promise.all([_.from("players").select("*, clubs(id,encoded_name,logo_url)"),_.from("clubs").select("id,encoded_name").order("encoded_name")]);if(l){e.innerHTML=`<p style="color:red">${l.message}</p>`;return}const x={GK:0,DEF:1,MIL:2,ATT:3},E=(d||[]).sort((p,f)=>{const m=(x[p.job]??4)-(x[f.job]??4);return m!==0?m:(p.surname_real||"").localeCompare(f.surname_real||"")});Ut(e,E,g||[],t,n)}function Ut(e,t,n,i,d=null){var c,o;const{toast:l}=i;if(e.innerHTML=`
    <div style="display:flex;flex-direction:column;gap:10px">
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        ...
        <select id="filter-rarity" style="width:110px">
          ...
          ${Object.entries(et).map(([r,u])=>`<option value="${r}">${u}</option>`).join("")}
        </select>
    ...
    </div>`,d){const r=d,u=p=>document.getElementById(p);r.search&&u("search-players")&&(u("search-players").value=r.search),r.job&&u("filter-job")&&(u("filter-job").value=r.job),r.rarity&&u("filter-rarity")&&(u("filter-rarity").value=r.rarity),r.club&&u("filter-club")&&(u("filter-club").value=r.club),r.country&&u("filter-country")&&(u("filter-country").value=r.country)}if(d){const r=d,u=p=>document.getElementById(p);r.search&&u("search-players")&&(u("search-players").value=r.search),r.job&&u("filter-job")&&(u("filter-job").value=r.job),r.rarity&&u("filter-rarity")&&(u("filter-rarity").value=r.rarity),r.club&&u("filter-club")&&(u("filter-club").value=r.club),r.country&&u("filter-country")&&(u("filter-country").value=r.country)}function g(){const r=document.getElementById("search-players").value.toLowerCase(),u=document.getElementById("filter-job").value,p=document.getElementById("filter-rarity").value,f=document.getElementById("filter-club").value,m=document.getElementById("filter-country").value;return t.filter(b=>(!r||`${b.firstname} ${b.surname_real}`.toLowerCase().includes(r))&&(!u||b.job===u)&&(!p||b.rarity===p)&&(!f||b.club_id===f)&&(!m||b.country_code===m))}const x=new Set;function E(){const r=document.getElementById("bulk-bar"),u=document.getElementById("bulk-count");r&&(r.style.display=x.size>0?"flex":"none",u&&(u.textContent=`${x.size} joueur(s) sélectionné(s)`))}function a(){const r=g();document.getElementById("count-label").textContent=`${r.length} joueur(s)`;const u=document.getElementById("players-list");if(!r.length){u.innerHTML='<div style="color:var(--gray-600);padding:20px;text-align:center">Aucun joueur.</div>';return}u.innerHTML=r.map(p=>{const f={...p,clubs:p.clubs,face:p.face||null},m=Ee(f,{width:120});return`<div style="position:relative;cursor:pointer" data-edit="${p.id}">
        ${m}
      ...
      </div>`}).join(""),u.querySelectorAll("[data-edit]").forEach(p=>{p.addEventListener("click",()=>{const f=t.find(m=>m.id===p.dataset.edit);f&&Re(f,n,e,i)})}),u.querySelectorAll(".btn-del-player").forEach(p=>{p.addEventListener("click",async f=>{if(f.stopPropagation(),!confirm("Supprimer ce joueur ?"))return;const{error:m}=await _.from("players").delete().eq("id",p.dataset.del);m?l(m.message,"error"):(l("Joueur supprimé ✅","success"),se(e,i))})})}a(),document.getElementById("search-players").addEventListener("input",a),document.getElementById("filter-job").addEventListener("change",a),document.getElementById("filter-rarity").addEventListener("change",a),document.getElementById("filter-club").addEventListener("change",a),document.getElementById("filter-country").addEventListener("change",a),(c=document.getElementById("bulk-cancel-btn"))==null||c.addEventListener("click",()=>{x.clear(),E(),a()}),(o=document.getElementById("bulk-delete-btn"))==null||o.addEventListener("click",async()=>{var f,m,b,$,L;if(!x.size||!confirm(`Supprimer ${x.size} joueur(s) ?`))return;const r=[...x],{error:u}=await _.from("players").delete().in("id",r);if(u){l(u.message,"error");return}l(`${r.length} joueur(s) supprimé(s) ✅`,"success"),x.clear();const p={search:((f=document.getElementById("search-players"))==null?void 0:f.value)||"",job:((m=document.getElementById("filter-job"))==null?void 0:m.value)||"",rarity:((b=document.getElementById("filter-rarity"))==null?void 0:b.value)||"",club:(($=document.getElementById("filter-club"))==null?void 0:$.value)||"",country:((L=document.getElementById("filter-country"))==null?void 0:L.value)||""};se(e,i,p)}),document.getElementById("add-player-btn").addEventListener("click",()=>Re(null,n,e,i)),document.getElementById("fix-old-faces-btn").addEventListener("click",()=>Ft(e,i))}async function Re(e,t,n,i){const{toast:d,openModal:l,closeModal:g}=i,x=!!e,E=await Nt(),{data:a}=await _.from("players").select("face").not("face","is",null),c=new Set((a||[]).map(p=>p.face).filter(Boolean));e!=null&&e.face&&c.delete(e.face);const o='<option value="">— Club —</option>'+t.map(p=>`<option value="${p.id}" ${(e==null?void 0:e.club_id)===p.id?"selected":""}>${p.encoded_name}</option>`).join(""),r=e!=null&&e.face?e.face.split("/")[0]:"",u=Object.keys(E).map(p=>`<option value="${p}" ${r===p?"selected":""}>${p}</option>`).join("");l(x?`✏️ ${e.firstname} ${e.surname_real}`:"➕ Nouveau joueur",`<div style="display:flex;gap:20px;align-items:flex-start">
    ...
    <div style="display:flex;gap:16px;height:calc(100vh - 56px);overflow:hidden;padding:16px">
    ...
    </div>`,g(re),document.getElementById("search-clubs").addEventListener("input",x=>{const E=x.target.value.toLowerCase();g(re.filter(a=>a.real_name.toLowerCase().includes(E)||a.encoded_name.toLowerCase().includes(E)))}),document.getElementById("add-club-btn").addEventListener("click",()=>je(null,e,t));function g(x){const E=document.getElementById("clubs-list");if(!x.length){E.innerHTML='<p style="color:var(--tile-fg-dim);padding:20px;text-align:center">Aucun club.</p>';return}const a={};x.forEach(r=>{const u=r.country_code||"—";a[u]||(a[u]=[]),a[u].push(r)});const c=Object.keys(a).sort(),o=r=>{const u=dt(r),p=r.logo_url?`<img src="${r.logo_url}" style="width:36px;height:36px;object-fit:contain;border-radius:8px;flex-shrink:0">`:`<div style="width:36px;height:36px;background:linear-gradient(135deg,${u.color1},${u.color2});border-radius:8px;display:flex;align-items:center;justify-content:center;color:#fff;font-size:9px;font-weight:900;flex-shrink:0">${r.encoded_name.slice(0,3)}</div>`,f=n[r.id]||0;return`
      ...
      <div style="display:flex;align-items:center;gap:8px;padding:8px 4px 4px;margin-top:6px">
        ...
        <span style="font-size:11px;color:var(--tile-fg-dim);background:var(--tile-bg);border:1px solid var(--tile-border);border-radius:10px;padding:1px 8px">${a[r].length}</span>
        <div style="flex:1;height:1px;background:var(--tile-border)"></div>
      </div>
      <div style="display:flex;flex-direction:column;gap:6px;margin-bottom:4px">
        ${a[r].map(o).join("")}
      </div>
    `).join(""),E.querySelectorAll(".club-row").forEach(r=>{r.addEventListener("click",()=>{const u=re.find(p=>p.id===r.dataset.clubId);u&&je(u,e,t),E.querySelectorAll(".club-row").forEach(p=>p.style.background="var(--tile-bg)"),r.style.background="rgba(26,107,60,0.18)"})}),E.querySelectorAll(".btn-del-club").forEach(r=>{r.addEventListener("click",async()=>{if(!confirm("Supprimer ce club ?"))return;const{error:u}=await _.from("clubs").delete().eq("id",r.dataset.del);u?i(u.message,"error"):(i("Club supprimé","success"),me(e,t))})})}}async function je(e,t,n){var m,b,$,L,v,w;const{toast:i}=n,d=!!e,l=e?dt(e):{...z},g=document.getElementById("club-panel");if(!g)return;const x=Object.entries(Me).map(([y,h])=>`<option value="${y}" ${l.style===y?"selected":""}>${h.label}</option>`).join(""),E=we.map(y=>`<option value="${y}" ${((e==null?void 0:e.country_code)||"FR")===y?"selected":""}>${y}</option>`).join("");g.style.display="block",g.style.alignItems="",g.style.justifyContent="",g.innerHTML=`
      ...
      <div style="border-top:1px solid var(--tile-border);padding-top:12px">
        ...
        <div style="display:flex;gap:12px;align-items:flex-start;flex-wrap:wrap">
          <div style="flex:1;min-width:200px;display:flex;flex-direction:column;gap:8px">
            ...
            ${[["Couleur 1","m-kit-color1",l.color1,!1],["Couleur 2","m-kit-color2",l.color2,!1],["Couleur 3","m-kit-color3",l.color3,!0],["Short","m-kit-shorts",l.shorts,!1],["Chaussettes","m-kit-socks",l.socks,!1]].map(([y,h,s,I])=>`
              <div class="form-group" id="wrap-${h}" ${I?'style="display:none"':""}>
                <label>${y}</label>
                <div style="display:flex;gap:6px;align-items:center">
                  <input type="color" id="${h}" value="${s||"#000000"}" style="width:38px;height:32px;padding:2px;border:1px solid var(--tile-border);border-radius:6px;cursor:pointer;flex-shrink:0">
                  <input id="${h}-txt" value="${s||"#000000"}" maxlength="7" style="flex:1;font-family:monospace;font-size:12px">
                </div>
              </div>`).join("")}
          </div>
  ...
  `,(m=document.getElementById("m-logo-file"))==null||m.addEventListener("change",y=>{var I;const h=(I=y.target.files)==null?void 0:I[0];if(!h)return;const s=new FileReader;s.onload=()=>{document.getElementById("logo-preview").innerHTML=`<img src="${s.result}" style="width:100%;height:100%;object-fit:contain">`},s.readAsDataURL(h)}),ne();function a(){var I,k;const y=((I=document.getElementById("m-kit-style"))==null?void 0:I.value)||"uni",h=((k=Me[y])==null?void 0:k.colors)===3,s=document.getElementById("wrap-m-kit-color3");s&&(s.style.display=h?"":"none")}a(),["m-kit-color1","m-kit-color2","m-kit-color3","m-kit-shorts","m-kit-socks"].forEach(y=>{const h=document.getElementById(y),s=document.getElementById(y+"-txt");!h||!s||(h.addEventListener("input",()=>{s.value=h.value,ne()}),s.addEventListener("input",()=>{const I=s.value.trim();/^#[0-9a-fA-F]{6}$/.test(I)&&(h.value=I,ne())}),s.addEventListener("change",()=>{let I=s.value.trim();I.startsWith("#")||(I="#"+I),/^#[0-9a-fA-F]{6}$/.test(I)&&(h.value=I,s.value=I,ne())}))}),(b=document.getElementById("m-kit-style"))==null||b.addEventListener("change",()=>{a(),ne()});const c=document.getElementById("m-real"),o=document.getElementById("m-encoded");function r(){if(!c||!o||o.value)return;const y=c.value.trim().split(/\s+/),h=y.length===1?y[0].toUpperCase().slice(0,6):y.filter(s=>s.length>2).map(s=>s[0].toUpperCase()).join("")||y[0].toUpperCase().slice(0,4);o.value=h}c==null||c.addEventListener("input",r),($=document.getElementById("auto-encode-btn"))==null||$.addEventListener("click",()=>{o&&(o.value=""),r()}),(L=document.getElementById("m-save"))==null||L.addEventListener("click",()=>ln(e,d,t,n));const u=document.getElementById("m-gen-squad"),p=document.getElementById("m-gen-strong-label");function f(){if(p&&(p.style.display=u!=null&&u.checked?"flex":"none",!(u!=null&&u.checked))){const y=document.getElementById("m-gen-strong");y&&(y.checked=!1)}}u==null||u.addEventListener("change",f),f(),(v=document.getElementById("btn-gen-squad-panel"))==null||v.addEventListener("click",()=>{tn(e,n,t)}),(w=document.getElementById("btn-gen-one-player"))==null||w.addEventListener("click",()=>{an(e,n)}),d&&$e(e.id)}async function $e(e){const{data:t}=await _.from("players").select("*, clubs(encoded_name, logo_url, kit_style, kit_color1, kit_color2, kit_color3, kit_shorts, kit_socks)").eq("club_id",e).order("surname_real"),n=document.getElementById("club-players-grid"),i=document.getElementById("club-players-count");if(i&&(i.textContent=(t||[]).length),!!n){if(!(t!=null&&t.length)){n.innerHTML='<div style="color:var(--tile-fg-dim);padding:20px;font-size:13px">Aucun joueur dans ce club.</div>';return}n.innerHTML=t.map(d=>Ee(d,{width:100})).join("")}}function tn(e,t,n){const{openModal:i,closeModal:d,toast:l}=t;i(`⚽ Générer — ${e.real_name}`,`<div style="display:flex;flex-direction:column;gap:14px">
    ...
    </div>`),setTimeout(()=>{var g,x;(g=document.getElementById("qg-cancel"))==null||g.addEventListener("click",()=>d()),(x=document.getElementById("qg-ok"))==null||x.addEventListener("click",async()=>{var a;const E=((a=document.getElementById("qg-strong"))==null?void 0:a.checked)??!1;d(),l("Génération en cours…","info"),await st(e.id,e.country_code,l,E),l("Effectif généré ✅","success"),$e(e.id),me(n,t)})},50)}const ct=["GK","DEF","MIL","ATT"],nn=[{value:"normal",label:"Normal"},{value:"pepite",label:"Pépite"},{value:"papyte",label:"Papyte"},{value:"legende",label:"Légende"}];function an(e,t){const{openModal:n,closeModal:i,toast:d}=t,l=we.map(a=>`<option value="${a}">${a}</option>`).join(""),g=a=>ct.map(c=>`<option value="${c}" ${c===a?"selected":""}>${c}</option>`).join(""),x=`
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
          <div>
            ...
            <select id="gen-one-rarity">
              ${nn.map(a=>`<option value="${a.value}">${a.label}</option>`).join("")}
            </select>
          </div>
          <div>
            <label>PAYS</label>
  ...
  `),setTimeout(()=>{var a,c;document.querySelectorAll('input[name="gen-one-mode"]').forEach(o=>{o.addEventListener("change",()=>{document.getElementById("gen-one-custom-fields").style.display=o.value==="custom"&&o.checked?"flex":"none"})}),(a=document.getElementById("gen-one-cancel"))==null||a.addEventListener("click",()=>i()),(c=document.getElementById("gen-one-ok"))==null||c.addEventListener("click",async()=>{var p,f,m,b,$;const o=((p=document.querySelector('input[name="gen-one-mode"]:checked'))==null?void 0:p.value)||"random",r=document.getElementById("gen-one-ok");r.disabled=!0,r.textContent="⏳ Génération…";const u={};o==="custom"&&(u.job=((f=document.getElementById("gen-one-job1"))==null?void 0:f.value)||null,u.job2=((m=document.getElementById("gen-one-job2"))==null?void 0:m.value)||null,u.rarity=((b=document.getElementById("gen-one-rarity"))==null?void 0:b.value)||null,u.country=(($=document.getElementById("gen-one-country"))==null?void 0:$.value)||null);try{await on(e,u),d("Joueur généré ✅","success"),i(),$e(e.id)}catch(L){d("Erreur : "+L.message,"error"),r.disabled=!1,r.textContent="➕ Générer"}})},50)}async function on(e,t={}){const n=t.country||(Math.random()<.5?e.country_code:it(e.country_code)),i=t.job||K(ct),d=t.job2||null,l=t.rarity||(()=>{const A=Math.random()*100;return A<1.5?"legende":A<3?"pepite":A<4.5?"papyte":"normal"})(),g=l==="pepite"?V(10,15):l==="papyte"?V(15,20):at(),x=d?Math.max(0,g-2):0;let E=0,a=0,c=0,o=0;i==="GK"&&(E=g),i==="DEF"&&(a=g),i==="MIL"&&(c=g),i==="ATT"&&(o=g),d==="GK"&&(E=x),d==="DEF"&&(a=x),d==="MIL"&&(c=x),d==="ATT"&&(o=x);const r=Ie(n),u=nt(n),p=K(tt),{data:f}=await _.from("players").select("surname_real").not("surname_real","is",null),m=new Set((f||[]).map(A=>A.surname_real).filter(Boolean)),b=lt(n),$=b.filter(A=>!m.has(A)),L=K($.length?$:b),v={job:i,job2:d,firstname:rt(n),surname_real:L,country_code:n,club_id:e.id,note_g:E,note_d:a,note_m:c,note_a:o,skin:r,hair:u,hair_length:p,rarity:l,sell_price:0,ethnie:be(n)},{data:w}=await _.from("players").select("face").not("face","is",null),y=new Set((w||[]).map(A=>A.face).filter(Boolean)),h=await _e(n,y);h&&(v.face=h);const{data:s,error:I}=await _.from("players").insert(v).select("id").single();if(I)throw I;const{error:k}=await _.from("cards").insert({card_type:"player",player_id:s.id});k&&console.warn("[GenOnePlayer] Erreur carte:",k.message)}function pt(){var e,t,n,i,d,l;return{style:((e=document.getElementById("m-kit-style"))==null?void 0:e.value)||"uni",color1:((t=document.getElementById("m-kit-color1"))==null?void 0:t.value)||"#1A6B3C",color2:((n=document.getElementById("m-kit-color2"))==null?void 0:n.value)||"#ffffff",color3:((i=document.getElementById("m-kit-color3"))==null?void 0:i.value)||"#000000",shorts:((d=document.getElementById("m-kit-shorts"))==null?void 0:d.value)||"#111111",socks:((l=document.getElementById("m-kit-socks"))==null?void 0:l.value)||"#ffffff"}}function ne(){const e=document.getElementById("kit-preview-wrap");e&&(e.innerHTML=wt(pt(),"panel"))}async function rn(e,t){const n=(e.name.split(".").pop()||"png").toLowerCase(),i=`clubs/${t||"new"}-${Date.now()}.${n}`,{error:d}=await _.storage.from("assets").upload(i,e,{upsert:!0,cacheControl:"3600"});if(d)throw d;const{data:l}=_.storage.from("assets").getPublicUrl(i);return l.publicUrl}async function ln(e,t,n,i){var L,v,w,y,h,s,I,k,A;const{toast:d}=i,l=document.getElementById("m-error"),g=document.getElementById("m-save"),x=(L=document.getElementById("m-real"))==null?void 0:L.value.trim(),E=(v=document.getElementById("m-encoded"))==null?void 0:v.value.trim().toUpperCase(),a=(w=document.getElementById("m-country"))==null?void 0:w.value.trim().toUpperCase(),c=((h=(y=document.getElementById("m-logo-file"))==null?void 0:y.files)==null?void 0:h[0])||null,o=((s=document.getElementById("m-logo-url-current"))==null?void 0:s.value)||null,r=((I=document.getElementById("m-gen-stadium"))==null?void 0:I.checked)??!1,u=((k=document.getElementById("m-gen-squad"))==null?void 0:k.checked)??!1,p=((A=document.getElementById("m-gen-strong"))==null?void 0:A.checked)??!1,f=pt();if(!x){l.textContent="Le nom du club est requis.";return}if(!E){l.textContent="Le nom encodé est requis.";return}if(!a){l.textContent="Le pays est requis.";return}g.disabled=!0,g.textContent="Enregistrement…";let m=o;if(c)try{g.textContent="📤 Envoi du logo…",m=await rn(c,e==null?void 0:e.id)}catch(B){l.textContent="Erreur upload logo : "+B.message,g.disabled=!1,g.textContent=t?"💾 Enregistrer":"✅ Créer le club";return}const b={real_name:x,encoded_name:E,country_code:a,logo_url:m,kit_style:f.style,kit_color1:f.color1,kit_color2:f.color2,kit_color3:f.color3,kit_shorts:f.shorts,kit_socks:f.socks};let $=e==null?void 0:e.id;if(t){const{error:B}=await _.from("clubs").update(b).eq("id",$);if(B){l.textContent=B.message,g.disabled=!1,g.textContent="💾 Enregistrer";return}}else{const{data:B,error:S}=await _.from("clubs").insert(b).select().single();if(S){l.textContent=S.message,g.disabled=!1,g.textContent="✅ Créer le club";return}if($=B.id,r){g.textContent="🏟️ Création du stade…";const{data:C,error:T}=await _.from("stadium_definitions").insert({name:`Stade de ${x}`,club_id:$,country_code:null}).select().single();T?console.warn("[Stadium] Erreur def stade:",T.message):C&&await _.from("cards").insert({card_type:"stadium",stadium_id:C.id})}u&&(g.textContent="⚽ Génération des joueurs…",await st($,a,d,p))}d(t?"Club modifié ✅":"Club créé ✅","success"),me(n,i)}const sn=["normal","pepite","papyte","legende"],De=["GK","DEF","MIL","ATT"],dn=[["MA","Maroc"],["FR","France"],["AR","Argentine"],["PT","Portugal"],["BR","Brésil"],["ES","Espagne"],["DE","Allemagne"],["GB","Angleterre"],["IT","Italie"],["CM","Cameroun"],["SN","Sénégal"],["NG","Nigéria"],["DK","Danemark"],["NL","Pays-Bas"],["BE","Belgique"],["CI","Côte d'Ivoire"],["AL","Albanie"],["HR","Croatie"],["RS","Serbie"],["TR","Turquie"],["MW","Malawi"]];let J={...R},le=[];async function cn(e,{toast:t}){const{data:n}=await _.from("clubs").select("id,encoded_name,logo_url,kit_style,kit_color1,kit_color2,kit_shorts,kit_socks").order("encoded_name");le=n||[],J={...R},e.innerHTML=gn(le),un(e,le,t),F()}function Pe(){var n;const e=((n=document.getElementById("cb-surname-real"))==null?void 0:n.value)||"",t=document.getElementById("cb-surname-enc");t&&(t.value=ue(e))}function pn(){var n;const e=(n=document.getElementById("cb-club"))==null?void 0:n.value,t=le.find(i=>String(i.id)===String(e));return t?{style:t.kit_style||z.style,color1:t.kit_color1||z.color1,color2:t.kit_color2||z.color2,shorts:t.kit_shorts||z.shorts,socks:t.kit_socks||z.socks}:{...z}}function F(){var r,u,p,f;const e=m=>{var b;return((b=document.getElementById(m))==null?void 0:b.value)||""},t={firstname:e("cb-firstname"),surname_real:e("cb-surname-enc")||ue(e("cb-surname-real")),country_code:e("cb-country"),club_encoded_name:((u=(r=document.getElementById("cb-club"))==null?void 0:r.selectedOptions[0])==null?void 0:u.text)||"",job:e("cb-job"),job2:e("cb-job2")||null,note_g:parseInt(e("cb-note-g"))||0,note_d:parseInt(e("cb-note-d"))||0,note_m:parseInt(e("cb-note-m"))||0,note_a:parseInt(e("cb-note-a"))||0,rarity:e("cb-rarity")},n=pn(),i=document.getElementById("cb-club"),d=((f=(p=i==null?void 0:i.selectedOptions[0])==null?void 0:p.dataset)==null?void 0:f.logo)||null,l=Qe(J,n,120),g=`data:image/svg+xml;charset=utf-8,${encodeURIComponent(l)}`,x=Lt(t,{portraitUrl:g,clubLogoUrl:d,showNotes:!0}),E=document.getElementById("card-preview");E&&(E.innerHTML=x);const a=document.getElementById("avatar-preview-wrap");a&&(a.innerHTML=l);const c=e("cb-surname-real"),o=document.getElementById("encode-summary");o&&c&&(o.textContent=`${e("cb-firstname")} ${c} → ${e("cb-firstname")} ${t.surname_real}`)}function ut(){const e=t=>{var n;return((n=document.getElementById(t))==null?void 0:n.value)||""};J={skin:e("av-skin"),hair:e("av-hair"),hairColor:e("av-hairColor"),eyes:e("av-eyes"),nose:e("av-nose"),mouth:e("av-mouth"),eyebrows:e("av-eyebrows"),beard:e("av-beard")}}function un(e,t,n){var d,l,g,x,E;e.querySelectorAll("input,select").forEach(a=>{a.addEventListener("input",F),a.addEventListener("change",F)});const i=["av-skin","av-hair","av-hairColor","av-eyes","av-nose","av-mouth","av-eyebrows","av-beard"];i.forEach(a=>{var c;(c=document.getElementById(a))==null||c.addEventListener("change",()=>{ut(),F()})}),(d=document.getElementById("cb-surname-real"))==null||d.addEventListener("input",()=>{Pe(),F()}),(l=document.getElementById("btn-encode"))==null||l.addEventListener("click",()=>{Pe(),F()}),(g=document.getElementById("cb-club"))==null||g.addEventListener("change",F),(x=document.getElementById("btn-save-player"))==null||x.addEventListener("click",()=>mn(n)),(E=document.getElementById("btn-reset"))==null||E.addEventListener("click",()=>{e.querySelectorAll("input").forEach(a=>a.value=""),e.querySelectorAll("select").forEach(a=>a.selectedIndex=0),J={...R},i.forEach(a=>{const c=a.replace("av-",""),o=document.getElementById(a);o&&(o.value=R[c]||"")}),F()})}async function mn(e){const t=g=>{var x;return((x=document.getElementById(g))==null?void 0:x.value)||""},n=t("cb-surname-real").trim(),i=t("cb-surname-enc").trim()||ue(n);if(!t("cb-firstname")||!n||!t("cb-country")||!t("cb-job")){e("Remplissez les champs obligatoires (prénom, nom, pays, poste)","error");return}ut();const d={firstname:t("cb-firstname").trim(),surname_real:n,surname_real:i,country_code:t("cb-country"),club_id:t("cb-club")||null,job:t("cb-job"),job2:t("cb-job2")||null,note_g:parseInt(t("cb-note-g"))||0,note_d:parseInt(t("cb-note-d"))||0,note_m:parseInt(t("cb-note-m"))||0,note_a:parseInt(t("cb-note-a"))||0,rarity:t("cb-rarity")||"normal",note_min:parseInt(t("cb-note-min"))||null,note_max:parseInt(t("cb-note-max"))||null,sell_price:parseInt(t("cb-price"))||0,avatar_config:J,skin:J.skin||"blanc",hair:J.hairColor||"marron",hair_length:"court"},{error:l}=await _.from("players").insert(d);if(l){e("Erreur: "+l.message,"error");return}e(`✅ Joueur "${d.firstname} ${d.surname_real}" enregistré !`,"success")}function gn(e){const t=dn.map(([a,c])=>`<option value="${a}">${c} (${a})</option>`).join(""),n='<option value="">— Sélectionner un club —</option>'+e.map(a=>`<option value="${a.id}" data-logo="${a.logo_url||""}">${a.encoded_name}</option>`).join(""),i=De.map(a=>`<option value="${a}">${a}</option>`).join(""),d='<option value="">Aucun</option>'+De.map(a=>`<option value="${a}">${a}</option>`).join(""),l=sn.map(a=>`<option value="${a}">${a.charAt(0).toUpperCase()+a.slice(1)}</option>`).join(""),g=(a,c,o)=>Object.entries(c).map(([r,u])=>`<option value="${r}" ${r===(o||"")?"selected":""}>${u.label}</option>`).join(""),x=Object.keys(It).map(a=>`<option value="${a}" ${a===R.skin?"selected":""}>${a.charAt(0).toUpperCase()+a.slice(1)}</option>`).join(""),E=Object.keys($t).map(a=>`<option value="${a}" ${a===R.hairColor?"selected":""}>${a.charAt(0).toUpperCase()+a.slice(1)}</option>`).join("");return`
  ...
  `}async function mt(e,{toast:t}){const[{data:n,error:i},{data:d},{data:l}]=await Promise.all([_.from("users").select("id,pseudo,club_name,credits,level,wins,draws,losses,trophies_top1,trophies_top2,trophies_top3,is_admin,created_at,mmr,mmr_deviation,rank_tier,placement_matches,ranked_wins,ranked_losses,ranked_draws").order("created_at",{ascending:!1}),_.from("user_solo_progress").select("user_id, unlocked_level"),_.rpc("admin_get_user_emails")]);if(i){e.innerHTML=`<p style="color:red">${i.message}</p>`;return}const g={};(d||[]).forEach(c=>{g[c.user_id]=c.unlocked_level});const x={};(l||[]).forEach(c=>{x[c.id]=c.email});const E=n||[];e.innerHTML=`
  ...
  `,a(E),document.getElementById("search-users").addEventListener("input",c=>{const o=c.target.value.toLowerCase();a(E.filter(r=>{var u;return r.pseudo.toLowerCase().includes(o)||((u=r.club_name)==null?void 0:u.toLowerCase().includes(o))}))});function a(c){document.getElementById("users-tbody").innerHTML=c.map(o=>{const r=o.mmr??1e3,u=o.mmr_deviation??350,p=o.placement_matches??0,f=zt(r),m=o.ranked_wins??0,b=o.ranked_losses??0,$=o.ranked_draws??0,L=m+b+$,v=L>0?Math.round(m/L*100):null,w=p<10,y=u<80?"#1A6B3C":u<150?"#e67e22":"#bb2020";return`
        <tr>
          <td>
          ...
          <td style="font-size:12px">
            ...
            ${w?`<div style="font-size:10px;color:#e67e22">${p}/10 placement${p<10?` (${10-p} restants)`:""}</div>`:`<div style="font-size:10px;color:var(--gray-600)">${m}V ${$}N ${b}D${v!==null?` · ${v}% WR`:""}</div>`}
          </td>
          <td style="font-size:12px;font-family:monospace">
            <div style="font-weight:700;font-size:14px">${r}</div>
            <div style="font-size:10px;color:${y}">RD ±${Math.round(u)}</div>
          </td>
          <td>
        ...
        </tr>`}).join(""),document.querySelectorAll(".btn-edit-credits").forEach(o=>{o.addEventListener("click",async()=>{const r=o.dataset.user,u=Number(o.dataset.current)||0,p=prompt("Nouveau solde de crédits :",u);if(p===null)return;const f=parseInt(p.replace(/\s/g,""),10);if(isNaN(f)||f<0){t("Valeur invalide","error");return}const{data:m,error:b}=await _.rpc("admin_update_user_credits",{p_user_id:r,p_new_credits:f});if(b){t(b.message,"error");return}if(!(m!=null&&m.success)){t((m==null?void 0:m.error)||"Échec de la mise à jour","error");return}t("Crédits mis à jour ✅","success");const $=document.getElementById(`credits-display-${r}`);$&&($.textContent=`${f.toLocaleString("fr")} cr.`),o.dataset.current=f})}),document.querySelectorAll("[data-toggle-admin]").forEach(o=>{o.addEventListener("click",async()=>{const r=o.dataset.isAdmin!=="true";if(!confirm(`Voulez-vous ${r?"rendre admin":"retirer le rôle admin"} de ce Manager ?`))return;const{error:u}=await _.from("users").update({is_admin:r}).eq("id",o.dataset.toggleAdmin);u?t(u.message,"error"):(t("Rôle mis à jour ✅","success"),mt(e,{toast:t}))})})}}const fn=[{value:"normal",label:"Normal",color:"#ccc"},{value:"pepite",label:"Pépite",color:"#D4A017"},{value:"papyte",label:"Papyte",color:"#909090"},{value:"legende",label:"Légende",color:"#7a28b8"}];async function gt(e,t){var E;const{toast:n}=t,{data:i,error:d}=await _.from("market_listings").select(`
    ...
    `).order("listed_at",{ascending:!1}).limit(100);if(d){e.innerHTML=`<p style="color:var(--danger)">${d.message}</p>`;return}const l=i||[],g=l.filter(a=>a.status==="active").length,x=l.filter(a=>a.status==="sold").length;e.innerHTML=`
    ...
    <div class="card-panel">
      <div class="table-wrap">
        <table>
          ...
          <tbody>
            ${l.map(a=>{var u,p,f,m;const c=(u=a.card)==null?void 0:u.player,o=c?`${c.firstname} ${c.surname_real}`:((p=a.card)==null?void 0:p.card_type)||"—",r={active:"#1A6B3C",sold:"#D4A017",cancelled:"#888"};return`<tr>
  ...
  `),bn(),(u=document.getElementById("pg-close"))==null||u.addEventListener("click",()=>{fe(),n()});function c(f){const m=f.dataset.id,b=f.dataset.field,$=b==="rarity"?f.value:Number(f.value)||0;return o(m,{[b]:$},f)}async function o(f,m,b){const $=document.querySelector(`tr[data-row="${f}"]`),L=I=>$.querySelector(`[data-field="${I}"]`),v="note_min"in m?m.note_min:Number(L("note_min").value),w="note_max"in m?m.note_max:Number(L("note_max").value),y="price_min"in m?m.price_min:Number(L("price_min").value),h="price_max"in m?m.price_max:Number(L("price_max").value);if(v>w){i("Note min doit être ≤ note max","error");return}if(y>h){i("Prix min doit être ≤ prix max","error");return}const{error:s}=await _.from("sell_price_configs").update(m).eq("id",f);if(s){i(s.message,"error");return}b&&(b.style.transition="background .3s",b.style.background="#e8f8ee",setTimeout(()=>{b.style.background=""},500))}function r(){document.querySelectorAll("#pg-tbody [data-field]").forEach(f=>{f.addEventListener("change",()=>c(f))})}r(),document.querySelectorAll("[data-del-row]").forEach(f=>{f.addEventListener("click",async()=>{confirm("Supprimer cette ligne ?")&&(await _.from("sell_price_configs").delete().eq("id",f.dataset.delRow),i("Ligne supprimée","success"),fe(),ve(e))})}),(p=document.getElementById("pg-add-row"))==null||p.addEventListener("click",async()=>{var L;const f={rarity:"normal",note_min:0,note_max:20,price_min:2500,price_max:15e3},{data:m,error:b}=await _.from("sell_price_configs").insert(f).select().single();if(b){i(b.message,"error");return}const $=document.getElementById("pg-tbody");$.querySelector("td[colspan]")&&($.innerHTML=""),$.insertAdjacentHTML("beforeend",x(m)),r(),(L=document.querySelector(`[data-del-row="${m.id}"]`))==null||L.addEventListener("click",async function(){confirm("Supprimer cette ligne ?")&&(await _.from("sell_price_configs").delete().eq("id",m.id),i("Ligne supprimée","success"),fe(),ve(e))}),i("Ligne ajoutée — modifie les valeurs directement","success")})}async function vn(e,{toast:t}){e.innerHTML=`
...
`;n.forEach(l=>{d+=[l.real_name,l.encoded_name,l.country_code,l.logo_url||""].map(Ne).join(",")+`
...
`;n.forEach(l=>{var g;d+=[l.firstname,l.surname_real,l.surname_real,l.country_code,((g=l.clubs)==null?void 0:g.encoded_name)||"",l.job,l.job2||"",l.note_g,l.note_d,l.note_m,l.note_a,l.rarity,l.note_min??"",l.note_max??"",l.skin,l.hair,l.hair_length,l.sell_price].map(Ne).join(",")+`
`}),ae("players_export.csv",d),t(`${n.length} joueurs exportés`,"success")}),document.getElementById("import-clubs").addEventListener("change",async n=>{const i=n.target.files[0];if(!i)return;const d=document.getElementById("clubs-import-result");d.innerHTML='<span style="color:var(--gray-600)">Import en cours...</span>';try{const l=await i.text(),g=Fe(l);if(g.length===0){d.innerHTML='<span style="color:#c0392b">Fichier vide</span>';return}let x=0,E=0;const a=[];for(const c of g){if(!c.real_name||!c.encoded_name||!c.country_code){E++,a.push(`Ligne ignorée (champs manquants): ${c.real_name||"?"}`);continue}const o={real_name:c.real_name,encoded_name:c.encoded_name.toUpperCase(),country_code:c.country_code.toUpperCase().slice(0,2),logo_url:c.logo_url||null},{error:r}=await _.from("clubs").upsert(o,{onConflict:"encoded_name"});r?(E++,a.push(`${c.encoded_name}: ${r.message}`)):x++}d.innerHTML=`<div style="color:var(--green)">✅ ${x} clubs importés</div>
        ${E>0?`<div style="color:#c0392b">❌ ${E} erreurs</div><div style="font-size:11px;color:var(--gray-600);max-height:100px;overflow-y:auto">${a.slice(0,10).join("<br>")}</div>`:""}`,t(`${x} clubs importés`,"success")}catch(l){d.innerHTML=`<span style="color:#c0392b">Erreur : ${l.message}</span>`}n.target.value=""}),document.getElementById("import-players").addEventListener("change",async n=>{const i=n.target.files[0];if(!i)return;const d=document.getElementById("players-import-result");d.innerHTML='<span style="color:var(--gray-600)">Import en cours...</span>';try{const l=await i.text(),g=Fe(l);if(g.length===0){d.innerHTML='<span style="color:#c0392b">Fichier vide</span>';return}const{data:x}=await _.from("clubs").select("id,encoded_name"),E={};(x||[]).forEach(b=>{E[b.encoded_name.toUpperCase()]=b.id});let a=0,c=0;const o=[],r=["GK","DEF","MIL","ATT"],u=["normal","pepite","papyte","legende"],p=["blanc","metisse","typ","noir"],f=["blond","marron","noir","chauve"],m=["rase","court","milong","long"];for(const b of g){if(!b.firstname||!b.surname_real||!b.country_code||!b.job){c++,o.push(`Ligne ignorée (champs requis manquants): ${b.firstname||"?"}`);continue}if(!r.includes(b.job)){c++,o.push(`${b.firstname}: job invalide "${b.job}"`);continue}const $=b.club_encoded_name&&E[b.club_encoded_name.toUpperCase()]||null,L={firstname:b.firstname,surname_real:b.surname_real,surname_real:b.surname_real||ue(b.surname_real),country_code:b.country_code.toUpperCase().slice(0,2),club_id:$,job:b.job,job2:r.includes(b.job2)?b.job2:null,note_g:parseInt(b.note_g)||0,note_d:parseInt(b.note_d)||0,note_m:parseInt(b.note_m)||0,note_a:parseInt(b.note_a)||0,rarity:u.includes(b.rarity)?b.rarity:"normal",note_min:b.note_min!==""&&b.note_min!=null?parseInt(b.note_min):null,note_max:b.note_max!==""&&b.note_max!=null?parseInt(b.note_max):null,skin:p.includes(b.skin)?b.skin:"blanc",hair:f.includes(b.hair)?b.hair:"noir",hair_length:m.includes(b.hair_length)?b.hair_length:"court",sell_price:parseInt(b.sell_price)||0},{error:v}=await _.from("players").insert(L);v?(c++,o.push(`${b.firstname} ${b.surname_real}: ${v.message}`)):a++}d.innerHTML=`<div style="color:var(--green)">✅ ${a} joueurs importés</div>
...
`).replace(/\r/g,`
`);const t=e.split(`
`).filter(g=>g.trim());if(t.length<2)return[];const n=t[0],i=n.split(";").length>n.split(",").length?";":",",d=Oe(n,i).map(g=>g.trim().replace(/^\uFEFF/,"").toLowerCase()),l=[];for(let g=1;g<t.length;g++){if(!t[g].trim())continue;const x=Oe(t[g],i),E={};d.forEach((a,c)=>{E[a]=(x[c]||"").trim()}),!Object.values(E).every(a=>!a)&&l.push(E)}return l}function Oe(e,t=","){const n=[];let i="",d=!1;for(let l=0;l<e.length;l++){const g=e[l];d?g==='"'?e[l+1]==='"'?(i+='"',l++):d=!1:i+=g:g==='"'?d=!0:g===t?(n.push(i),i=""):i+=g}return n.push(i),n}const xn={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};async function hn(e,{toast:t}){const n=Object.keys(ge);let i=n[0];const{data:d}=await _.from("formation_links_overrides").select("formation, links"),l={};(d||[]).forEach(c=>{l[c.formation]=c.links});let g=new Set;function x(c,o){return[c,o].sort().join("-")}function E(c){const o=l[c]||ge[c]||[];g=new Set(o.map(([r,u])=>x(r,u)))}E(i),e.innerHTML=`
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;gap:12px;flex-wrap:wrap">
  ...
  `;function a(){const c=Rt[i]||{},o=ze(i),r=320,u=400,p=22;function f(v){const w=c[v];return w?{x:w.x*r,y:w.y*u}:null}let m=`<svg width="${r}" height="${u}" viewBox="0 0 ${r} ${u}" xmlns="http://www.w3.org/2000/svg" style="display:block">`;o.forEach(([v,w],y)=>{const h=f(v),s=f(w);if(!h||!s)return;const I=x(v,w),k=g.has(I),A=k?"#3b82f6":"#999",B=k?.95:.35,S=k?4:3;m+=`<line x1="${h.x}" y1="${h.y}" x2="${s.x}" y2="${s.y}"
        stroke="transparent" stroke-width="14" class="link-hit" data-key="${I}" style="cursor:pointer"/>`,m+=`<line x1="${h.x}" y1="${h.y}" x2="${s.x}" y2="${s.y}"
        stroke="${A}" stroke-width="${S}" stroke-dasharray="${k?"none":"4,3"}"
        opacity="${B}" pointer-events="none" data-line-key="${I}"/>`});for(const v of Object.keys(c)){const w=f(v);if(!w)continue;const y=v.replace(/\d+/,""),h=xn[y]||"#555";m+=`<circle cx="${w.x}" cy="${w.y}" r="${p}" fill="${h}" stroke="rgba(255,255,255,0.7)" stroke-width="2" pointer-events="none"/>`,m+=`<text x="${w.x}" y="${w.y+4}" text-anchor="middle" font-size="10" font-weight="900" fill="white" font-family="Arial Black,Arial" pointer-events="none">${v}</text>`}m+="</svg>",document.getElementById("field-wrap").innerHTML=m;const b=document.getElementById("links-list");b.innerHTML=o.map(([v,w])=>{const y=x(v,w),h=g.has(y);return`
        ...
        </button>`}).join("");const $=e.querySelector('div[style*="font-weight:700;font-size:13px;margin-bottom:8px"]');$&&($.textContent=`Liens (${g.size}/${o.length} actifs)`);function L(v){g.has(v)?g.delete(v):g.add(v),a()}e.querySelectorAll(".link-hit").forEach(v=>{v.addEventListener("click",()=>L(v.dataset.key))}),e.querySelectorAll(".link-toggle").forEach(v=>{v.addEventListener("click",()=>L(v.dataset.key))})}a(),document.getElementById("formation-select").addEventListener("change",c=>{i=c.target.value,E(i),a()}),document.getElementById("reset-btn").addEventListener("click",()=>{const c=ge[i]||[];g=new Set(c.map(([o,r])=>x(o,r))),a(),t("Liens réinitialisés aux valeurs par défaut (non sauvegardé)","info")}),document.getElementById("save-btn").addEventListener("click",async()=>{const o=ze(i).filter(([u,p])=>g.has(x(u,p))),{error:r}=await _.from("formation_links_overrides").upsert({formation:i,links:o,updated_at:new Date().toISOString()});if(r){t(r.message,"error");return}l[i]=o,t(`Liens enregistrés pour ${i} (${o.length} actifs)`,"success")})}const En=[{value:"player",label:"Joueur"},{value:"formation",label:"Formation"},{value:"game_changer",label:"Game Changer"},{value:"ultra_game_changer",label:"Ultra Game Changer"},{value:"game_helper",label:"Game Helper"},{value:"stadium",label:"🏟️ Stade"}],_n=[{value:"",label:"Toutes"},{value:"normal",label:"Normal"},{value:"pepite",label:"Pépite"},{value:"papyte",label:"Papyte"},{value:"legende",label:"Légende"}],wn=[{value:"credits",label:"Crédits"},{value:"pub",label:"Publicité"},{value:"free",label:"Gratuit"}];async function In(e){e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">Chargement...</div>';const{data:t}=await _.from("booster_configs").select("*").order("sort_order");let n=null,i=[],d=null;const l=()=>window.innerWidth<700;async function g(){if(!n){i=[];return}if(n!==d){const{data:o}=await _.from("booster_drop_rates").select("*").eq("booster_id",n).order("sort_order");i=o||[],d=n}}function x(){return`
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
          <div>
            ...
            <select id="f-price-type" style="width:100%;padding:9px;border:1px solid #ddd;border-radius:6px;font-size:13px">
              ${wn.map(p=>`<option value="${p.value}" ${o.price_type===p.value?"selected":""}>${p.label}</option>`).join("")}
            </select>
          </div>
        ...
        ${i.length===0?'<div style="text-align:center;padding:16px;color:#aaa;font-size:13px">Aucun taux. Cliquez + Ligne.</div>':i.map((p,f)=>`
          <div style="display:grid;grid-template-columns:2fr 1.5fr 50px 50px 60px 32px;gap:4px;align-items:center;margin-bottom:6px;padding:6px;background:#f9f9f9;border-radius:6px" data-rate-idx="${f}">
            <select class="rate-type" data-idx="${f}" style="padding:6px 4px;border:1px solid #ddd;border-radius:4px;font-size:12px;width:100%">
              ${En.map(m=>`<option value="${m.value}" ${p.card_type===m.value?"selected":""}>${m.label}</option>`).join("")}
            </select>
            <select class="rate-rarity" data-idx="${f}" style="padding:6px 4px;border:1px solid #ddd;border-radius:4px;font-size:12px;width:100%">
              ${_n.map(m=>`<option value="${m.value}" ${(p.rarity||"")===m.value?"selected":""}>${m.label}</option>`).join("")}
            </select>
    ...
    </div>`}async function a(o=!1){o||await g();const r=!n&&l(),u=n&&l();e.innerHTML=`
    <div style="display:flex;height:calc(100vh - 56px);overflow:hidden">
      ${!l()||r||!n?x():""}
      ${!l()||u?E():""}
    </div>`,c()}function c(){var u,p,f,m,b,$,L;const o=v=>e.querySelector(v);e.querySelectorAll(".booster-row").forEach(v=>{v.addEventListener("click",w=>{w.target.closest(".btn-delete")||(n=v.dataset.id,d=null,a())})}),(u=o("#btn-back"))==null||u.addEventListener("click",()=>{n=null,a()}),(p=o("#btn-new"))==null||p.addEventListener("click",async()=>{const v=prompt("Nom du nouveau booster :");if(!(v!=null&&v.trim()))return;const{data:w,error:y}=await _.from("booster_configs").insert({name:v.trim(),price_type:"credits",price_credits:1e3,card_count:5}).select().single();if(y){alert(y.message);return}t.push(w),n=w.id,d=null,a()}),e.querySelectorAll(".btn-delete").forEach(v=>{v.addEventListener("click",async w=>{if(w.stopPropagation(),!confirm("Supprimer ce booster ?"))return;await _.from("booster_configs").delete().eq("id",v.dataset.id);const y=t.findIndex(h=>h.id===v.dataset.id);y>-1&&t.splice(y,1),n===v.dataset.id&&(n=null,d=null),a()})}),(f=o("#btn-cancel"))==null||f.addEventListener("click",()=>{n=null,a()}),(m=o("#f-price-type"))==null||m.addEventListener("change",v=>{const w=o("#credits-field");w&&(w.style.opacity=v.target.value!=="credits"?"0.4":"1")}),(b=o("#btn-pick-icon"))==null||b.addEventListener("click",async()=>{var w;const v=o("#icon-picker-grid");if(v){if(v.style.display!=="none"){v.style.display="none";return}v.innerHTML='<div style="padding:10px;text-align:center;color:#888;font-size:12px">⏳ Chargement...</div>',v.style.display="block";try{const h=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),s=Array.isArray(h)?h.filter(k=>k.name.startsWith("booster")&&/\.(png|jpg|svg|webp)$/i.test(k.name)):[];if(!s.length){v.innerHTML='<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Aucune icône "booster*" trouvée.</div>';return}const I=((w=o("#f-image-url"))==null?void 0:w.value)||"";v.innerHTML=`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(72px,1fr));gap:8px">
          ${s.map(k=>`
          <div class="icon-pick-item" data-name="${k.name}"
            style="cursor:pointer;text-align:center;padding:6px;border-radius:6px;border:2px solid ${k.name===I?"#1A6B3C":"#ddd"};background:${k.name===I?"#f0f7f0":"#fff"}">
        ...
        </div>`,v.querySelectorAll(".icon-pick-item").forEach(k=>{k.addEventListener("click",()=>{const A=o("#f-image-url");A&&(A.value=k.dataset.name),v.style.display="none"})})}catch(y){v.innerHTML=`<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Erreur API GitHub : ${y.message}</div>`}}});function r(){e.querySelectorAll("[data-rate-idx]").forEach(v=>{var y,h,s,I,k;const w=Number(v.dataset.rateIdx);w>=0&&w<i.length&&(i[w].card_type=((y=v.querySelector(".rate-type"))==null?void 0:y.value)||"player",i[w].rarity=((h=v.querySelector(".rate-rarity"))==null?void 0:h.value)||null,i[w].note_min=Number((s=v.querySelector(".rate-note-min"))==null?void 0:s.value)||null,i[w].note_max=Number((I=v.querySelector(".rate-note-max"))==null?void 0:I.value)||null,i[w].percentage=Number((k=v.querySelector(".rate-pct"))==null?void 0:k.value)||0)})}($=o("#btn-add-rate"))==null||$.addEventListener("click",()=>{r(),i.push({id:null,booster_id:n,card_type:"player",rarity:"normal",note_min:null,note_max:null,percentage:10,sort_order:i.length}),a(!0)}),e.querySelectorAll(".btn-del-rate").forEach(v=>{v.addEventListener("click",()=>{r(),i.splice(Number(v.dataset.idx),1),a(!0)})}),(L=o("#btn-save"))==null||L.addEventListener("click",async()=>{var I,k,A,B,S,C,T,O,U,G,N,D,ie;const v=(t||[]).find(M=>M.id===n);if(!v)return;const w=[];e.querySelectorAll("[data-rate-idx]").forEach(M=>{var Z,Q,X,ee,te;const Y=Number(M.dataset.rateIdx);w.push({booster_id:n,card_type:((Z=M.querySelector(".rate-type"))==null?void 0:Z.value)||"player",rarity:((Q=M.querySelector(".rate-rarity"))==null?void 0:Q.value)||null,note_min:Number((X=M.querySelector(".rate-note-min"))==null?void 0:X.value)||null,note_max:Number((ee=M.querySelector(".rate-note-max"))==null?void 0:ee.value)||null,percentage:Number((te=M.querySelector(".rate-pct"))==null?void 0:te.value)||0,sort_order:Y});const P=w[w.length-1];P.rarity||(P.rarity=null),P.note_min||(P.note_min=null),P.note_max||(P.note_max=null)});const y=w.reduce((M,Y)=>M+Y.percentage,0);if(w.length&&Math.abs(y-100)>.5){alert(`La somme doit faire 100% (actuellement ${y.toFixed(1)}%)`);return}const h={name:((k=(I=o("#f-name"))==null?void 0:I.value)==null?void 0:k.trim())||v.name,image_url:((B=(A=o("#f-image-url"))==null?void 0:A.value)==null?void 0:B.trim())||null,price_type:(S=o("#f-price-type"))==null?void 0:S.value,price_credits:Number((C=o("#f-price-credits"))==null?void 0:C.value)||0,card_count:Number((T=o("#f-card-count"))==null?void 0:T.value)||5,is_active:((O=o("#f-active"))==null?void 0:O.checked)??v.is_active,allow_duplicates:((U=o("#f-allow-dup"))==null?void 0:U.checked)??!0,sort_order:Number((G=o("#f-sort"))==null?void 0:G.value)||0,max_per_user:(N=o("#f-max-per-user"))!=null&&N.value?Number(o("#f-max-per-user").value):null,available_from:((D=o("#f-available-from"))==null?void 0:D.value)||null,available_until:((ie=o("#f-available-until"))==null?void 0:ie.value)||null},{error:s}=await _.from("booster_configs").update(h).eq("id",n);if(s){alert(s.message);return}if(Object.assign(v,h),await _.from("booster_drop_rates").delete().eq("booster_id",n),w.length){const{error:M}=await _.from("booster_drop_rates").insert(w);if(M){alert(M.message);return}}i=w,d=n,alert("✅ Booster enregistré !"),a(!0)})}a()}const ft=[{value:"normal",label:"Normal",color:"#888"},{value:"pepite",label:"Pépite",color:"#D4A017"},{value:"papyte",label:"Papyte",color:"#909090"},{value:"legende",label:"Légende",color:"#7a28b8"}];async function $n(e){await ke(e)}async function ke(e){const{data:t,error:n}=await _.from("sell_price_configs").select("*").order("rarity").order("note_min");if(n){e.innerHTML=`<div style="padding:24px;color:red">Erreur : ${n.message}</div>`;return}e.innerHTML=`
    ...
    </div>`,document.getElementById("add-config").addEventListener("click",()=>Ue(null)),document.getElementById("form-cancel").addEventListener("click",()=>{document.getElementById("config-form").style.display="none"}),document.getElementById("form-save").addEventListener("click",()=>Ln(e)),e.querySelectorAll("[data-edit]").forEach(i=>{const d=(t||[]).find(l=>l.id===i.dataset.edit);d&&i.addEventListener("click",()=>Ue(d))}),e.querySelectorAll("[data-delete]").forEach(i=>{i.addEventListener("click",async()=>{confirm("Supprimer cette règle ?")&&(await _.from("sell_price_configs").delete().eq("id",i.dataset.delete),await ke(e))})})}function kn(e){const t=ft.find(n=>n.value===e.rarity);return`
    ...
    </tr>`}function Ue(e){document.getElementById("config-form").style.display="block",document.getElementById("form-title").textContent=e?"Modifier la règle":"Nouvelle règle",document.getElementById("form-id").value=(e==null?void 0:e.id)||"",document.getElementById("form-rarity").value=(e==null?void 0:e.rarity)||"normal",document.getElementById("form-price").value=(e==null?void 0:e.price)||1e3,document.getElementById("form-note-min").value=(e==null?void 0:e.note_min)??1,document.getElementById("form-note-max").value=(e==null?void 0:e.note_max)??10,document.getElementById("config-form").scrollIntoView({behavior:"smooth"})}async function Ln(e){const t=document.getElementById("form-id").value,n=document.getElementById("form-rarity").value,i=parseInt(document.getElementById("form-price").value)||0,d=parseInt(document.getElementById("form-note-min").value)||1,l=parseInt(document.getElementById("form-note-max").value)||10;if(d>l){alert("Note min doit être ≤ note max");return}const g={rarity:n,price:i,note_min:d,note_max:l,updated_at:new Date().toISOString()};let x;if(t?{error:x}=await _.from("sell_price_configs").update(g).eq("id",t):{error:x}=await _.from("sell_price_configs").insert(g),x){alert("Erreur : "+x.message);return}document.getElementById("config-form").style.display="none",await ke(e)}async function An(e){await Le(e)}async function Le(e){const{data:t}=await _.from("patch_notes").select("*").order("published_at",{ascending:!1});e.innerHTML=`
    ...
    </div>`,document.getElementById("add-article").addEventListener("click",()=>Ge(null)),document.getElementById("form-cancel").addEventListener("click",()=>{document.getElementById("article-form").style.display="none"}),document.getElementById("form-save").addEventListener("click",()=>Cn(e)),e.querySelectorAll("[data-edit-row]").forEach(n=>{const i=(t||[]).find(d=>d.id===n.dataset.editRow);i&&n.addEventListener("click",()=>Ge(i))}),e.querySelectorAll("[data-delete]").forEach(n=>{n.addEventListener("click",async()=>{confirm("Supprimer cet article ?")&&(await _.from("patch_notes").delete().eq("id",n.dataset.delete),await Le(e))})})}function Bn(e){const t=new Date(e.published_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"short",year:"numeric"});return`
    ...
    </div>`}function Ge(e){const t=document.getElementById("article-form");t.style.display="block",document.getElementById("form-title").textContent=e?"Modifier l'article":"Nouvel article",document.getElementById("form-id").value=(e==null?void 0:e.id)||"",document.getElementById("form-art-title").value=(e==null?void 0:e.title)||"",document.getElementById("form-art-desc").value=(e==null?void 0:e.description)||"",document.getElementById("form-art-img-current").value=(e==null?void 0:e.image_url)||"",document.getElementById("form-art-img-file").value="";const n=document.getElementById("form-art-img-preview");n.innerHTML=e!=null&&e.image_url?`<img src="${e.image_url}" style="width:100%;height:100%;object-fit:cover">`:'<span style="font-size:20px;opacity:.4">📰</span>',document.getElementById("form-art-img-file").onchange=l=>{var E;const g=(E=l.target.files)==null?void 0:E[0];if(!g)return;const x=new FileReader;x.onload=()=>{n.innerHTML=`<img src="${x.result}" style="width:100%;height:100%;object-fit:cover">`},x.readAsDataURL(g)},document.getElementById("form-art-pub").checked=e?e.is_published:!0;const i=new Date((e==null?void 0:e.published_at)||Date.now()),d=new Date(i.getTime()-i.getTimezoneOffset()*6e4).toISOString().slice(0,16);document.getElementById("form-art-date").value=d,t.scrollIntoView({behavior:"smooth"})}async function Sn(e){const t=(e.name.split(".").pop()||"png").toLowerCase(),n=`journal/${Date.now()}.${t}`,{error:i}=await _.storage.from("assets").upload(n,e,{upsert:!0,cacheControl:"3600"});if(i)throw i;const{data:d}=_.storage.from("assets").getPublicUrl(n);return d.publicUrl}async function Cn(e){var u;const t=document.getElementById("form-id").value,n=document.getElementById("form-art-title").value.trim(),i=document.getElementById("form-art-desc").value.trim(),d=((u=document.getElementById("form-art-img-file").files)==null?void 0:u[0])||null,l=document.getElementById("form-art-img-current").value||null,g=document.getElementById("form-art-pub").checked,x=document.getElementById("form-art-date").value,E=document.getElementById("form-save");if(!n){alert("Le titre est obligatoire.");return}if(!i){alert("La description est obligatoire.");return}let a=l;if(d)try{E&&(E.textContent="📤 Envoi de l'image…"),a=await Sn(d)}catch(p){alert("Erreur upload image : "+p.message),E&&(E.textContent="💾 Enregistrer");return}const c=x?new Date(x).toISOString():new Date().toISOString(),o={title:n,description:i,image_url:a,is_published:g,published_at:c};let r;if(t?{error:r}=await _.from("patch_notes").update(o).eq("id",t):{error:r}=await _.from("patch_notes").insert(o),r){alert("Erreur : "+r.message);return}document.getElementById("article-form").style.display="none",await Le(e)}async function Tn(e){await W(e)}async function W(e){var c,o,r,u,p,f,m,b,$,L,v,w;const{data:t}=await _.from("tutorial_steps").select("*").order("step_order");e.innerHTML=`
    <div style="display:flex;gap:16px;height:calc(100vh - 56px);overflow:hidden;padding:16px">
      ...
      <div style="width:420px;flex-shrink:0;overflow-y:auto">
        ...
        <div style="margin-bottom:16px">
          ...
          <div id="ts-toolbar" style="display:flex;flex-wrap:wrap;gap:4px;padding:6px 8px;background:#f5f5f5;border:1.5px solid #ddd;border-bottom:none;border-radius:8px 8px 0 0">
            ${[["B","bold","<strong>Gras</strong>"],["I","italic","<em>Italique</em>"],["U","underline","Souligné"],["¶","insertParagraph","Paragraphe"],["•","insertUnorderedList","Liste à puces"],["1.","insertOrderedList","Liste numérotée"],["H2","formatBlock:h2","Titre H2"],["H3","formatBlock:h3","Titre H3"],["—","insertHorizontalRule","Séparateur"]].map(([y,h,s])=>`<button type="button" data-cmd="${h}" title="${s}" style="padding:4px 8px;border:1px solid #ddd;border-radius:4px;background:#fff;font-size:12px;font-weight:700;cursor:pointer;line-height:1;min-width:28px">${y}</button>`).join("")}
      ...
      <div style="flex:1;overflow-y:auto">
        ...
        <div id="ts-list" style="display:flex;flex-direction:column;gap:10px">
    ...
    </div>`,(c=document.getElementById("ts-add"))==null||c.addEventListener("click",()=>He(null,(t==null?void 0:t.length)||0)),(o=document.getElementById("ts-reset"))==null||o.addEventListener("click",async()=>{if(!confirm("Remettre tutorial_done = false pour TOUS les joueurs ? Ils reverront le tutoriel à leur prochaine connexion."))return;const{error:y}=await _.from("users").update({tutorial_done:!1}).neq("id","00000000-0000-0000-0000-000000000000");alert(y?"Erreur : "+y.message:"✅ Tutorial_done réinitialisé pour tous les joueurs.")}),(r=document.getElementById("ts-cancel"))==null||r.addEventListener("click",yt),(u=document.getElementById("ts-save"))==null||u.addEventListener("click",()=>Rn(e)),(p=document.getElementById("ts-preview-btn"))==null||p.addEventListener("click",zn);const n=document.getElementById("ts-editor"),i=()=>{const y=document.getElementById("ts-content");y&&n&&(y.value=n.innerHTML)};n==null||n.addEventListener("input",i),n==null||n.addEventListener("blur",i);const d=()=>{document.activeElement!==n&&(n==null||n.focus())};(f=document.getElementById("ts-toolbar"))==null||f.querySelectorAll("[data-cmd]").forEach(y=>{y.addEventListener("mousedown",h=>{h.preventDefault(),d();const s=y.dataset.cmd;s.startsWith("formatBlock:")?document.execCommand("formatBlock",!1,s.split(":")[1]):document.execCommand(s,!1,null),i()})}),(m=document.getElementById("ts-insert-color"))==null||m.addEventListener("mousedown",y=>{y.preventDefault(),d();const h=prompt("Couleur hex (ex: #D4A017 pour or, #1A6B3C pour vert, #cc2222 pour rouge) :");h&&(document.execCommand("foreColor",!1,h),i())}),(b=document.getElementById("ts-clear-format"))==null||b.addEventListener("mousedown",y=>{y.preventDefault(),d(),document.execCommand("removeFormat",!1,null),i()}),n==null||n.addEventListener("focus",()=>{n&&(n.style.borderColor="#1A6B3C")}),n==null||n.addEventListener("blur",()=>{n&&(n.style.borderColor="#ddd")});const l=document.getElementById("ts-image"),g=document.getElementById("ts-img-preview"),x=document.getElementById("ts-img-preview-el"),E=document.getElementById("ts-img-picker-grid"),a=()=>{var h;const y=(h=l==null?void 0:l.value)==null?void 0:h.trim();if(y){const s="/";x.src=`${s}icons/${y}`,g.style.display="block"}else g.style.display="none"};l==null||l.addEventListener("input",a),($=document.getElementById("ts-img-clear"))==null||$.addEventListener("click",()=>{l&&(l.value=""),g.style.display="none",E.style.display="none"}),(L=document.getElementById("ts-img-pick"))==null||L.addEventListener("click",async()=>{if(E.style.display!=="none"){E.style.display="none";return}E.innerHTML='<div style="color:#aaa;font-size:12px">Chargement…</div>',E.style.display="block";try{const h=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),s=Array.isArray(h)?h.filter(k=>k.name.startsWith("tuto_")):[];if(!s.length){E.innerHTML='<div style="color:#aaa;font-size:12px">Aucun fichier tuto_* trouvé dans /icons/.<br>Uploade des images nommées tuto_xxx.png dans public/icons/</div>';return}const I="/";E.innerHTML='<div style="display:flex;flex-wrap:wrap;gap:8px">'+s.map(k=>`
          <div data-pick="${k.name}" style="cursor:pointer;border:2px solid transparent;border-radius:8px;padding:4px;text-align:center;transition:border-color .15s" onmouseover="this.style.borderColor='#1A6B3C'" onmouseout="this.style.borderColor='transparent'">
            <img src="${I}icons/${k.name}" style="width:70px;height:56px;object-fit:contain;display:block;border-radius:4px">
          ...
          </div>`).join("")+"</div>",E.querySelectorAll("[data-pick]").forEach(k=>{k.addEventListener("click",()=>{l&&(l.value=k.dataset.pick),a(),E.style.display="none"})})}catch(y){E.innerHTML='<div style="color:#cc2222;font-size:12px">Erreur chargement : '+y.message+"</div>"}}),(v=document.getElementById("ts-color"))==null||v.addEventListener("input",y=>{const h=document.getElementById("ts-color-hex");h&&(h.value=y.target.value)}),(w=document.getElementById("ts-color-hex"))==null||w.addEventListener("input",y=>{const h=y.target.value;if(/^#[0-9a-fA-F]{6}$/.test(h)){const s=document.getElementById("ts-color");s&&(s.value=h)}}),e.querySelectorAll("[data-edit-row]").forEach(y=>{const h=(t||[]).find(s=>s.id===y.dataset.editRow);h&&y.addEventListener("click",()=>He(h))}),e.querySelectorAll("[data-delete]").forEach(y=>{y.addEventListener("click",async()=>{confirm("Supprimer cette étape ?")&&(await _.from("tutorial_steps").delete().eq("id",y.dataset.delete),W(e))})}),e.querySelectorAll("[data-toggle]").forEach(y=>{y.addEventListener("click",async()=>{const h=(t||[]).find(s=>s.id===y.dataset.toggle);h&&(await _.from("tutorial_steps").update({is_active:!h.is_active}).eq("id",h.id),W(e))})}),e.querySelectorAll("[data-up]").forEach(y=>{y.addEventListener("click",async()=>{const h=t||[],s=h.findIndex(I=>I.id===y.dataset.up);s<=0||(await Promise.all([_.from("tutorial_steps").update({step_order:h[s-1].step_order}).eq("id",h[s].id),_.from("tutorial_steps").update({step_order:h[s].step_order}).eq("id",h[s-1].id)]),W(e))})}),e.querySelectorAll("[data-down]").forEach(y=>{y.addEventListener("click",async()=>{const h=t||[],s=h.findIndex(I=>I.id===y.dataset.down);s<0||s>=h.length-1||(await Promise.all([_.from("tutorial_steps").update({step_order:h[s+1].step_order}).eq("id",h[s].id),_.from("tutorial_steps").update({step_order:h[s].step_order}).eq("id",h[s+1].id)]),W(e))})})}function Mn(e){return`
  <div data-edit-row="${e.id}" style="background:#fff;border-radius:10px;padding:12px 16px;box-shadow:0 1px 4px rgba(0,0,0,0.08);display:flex;align-items:center;gap:12px;cursor:pointer">
  ...
  </div>`}function He(e,t=0){document.getElementById("ts-form-empty").style.display="none",document.getElementById("ts-form-content").style.display="block",document.getElementById("ts-form-title").textContent=e?"Modifier l'étape":"Nouvelle étape",document.getElementById("ts-id").value=(e==null?void 0:e.id)||"",document.getElementById("ts-emoji").value=(e==null?void 0:e.emoji)||"⚽",document.getElementById("ts-title").value=(e==null?void 0:e.title)||"",document.getElementById("ts-content").value=(e==null?void 0:e.content)||"";const n=document.getElementById("ts-editor");n&&(n.innerHTML=(e==null?void 0:e.content)||"<p>Texte de l'étape…</p>"),document.getElementById("ts-order").value=(e==null?void 0:e.step_order)??t,document.getElementById("ts-active").checked=e?e.is_active:!0;const i=(e==null?void 0:e.color)||"#1A6B3C";document.getElementById("ts-color").value=i,document.getElementById("ts-color-hex").value=i;const d=(e==null?void 0:e.image_url)||"",l=document.getElementById("ts-image");l&&(l.value=d);const g=document.getElementById("ts-img-preview"),x=document.getElementById("ts-img-preview-el");if(d&&g&&x){const E="/";x.src=`${E}icons/${d}`,g.style.display="block"}else g&&(g.style.display="none");document.getElementById("ts-img-picker-grid").style.display="none",document.getElementById("ts-preview-area").innerHTML="",document.getElementById("ts-form").scrollIntoView({behavior:"smooth"})}function yt(){document.getElementById("ts-form-content").style.display="none",document.getElementById("ts-form-empty").style.display="block",document.getElementById("ts-preview-area").innerHTML=""}function zn(){var a,c;const e=document.getElementById("ts-emoji").value||"⚽",t=document.getElementById("ts-title").value||"Titre",n=document.getElementById("ts-editor"),i=n?n.innerHTML:document.getElementById("ts-content").value||"",d=document.getElementById("ts-color-hex").value||"#1A6B3C",l=(c=(a=document.getElementById("ts-image"))==null?void 0:a.value)==null?void 0:c.trim(),x=l?`<div style="padding:0 20px 12px;text-align:center"><img src="/icons/${l}" style="max-height:140px;max-width:100%;border-radius:10px;object-fit:contain"></div>`:"",E=document.getElementById("ts-preview-area");E.innerHTML=`
    ...
    </div>`,E.querySelectorAll("ul,ol").forEach(o=>{o.style.paddingLeft="20px",o.style.margin="6px 0"}),E.querySelectorAll("li").forEach(o=>{o.style.marginBottom="4px"}),E.querySelectorAll("p").forEach(o=>{o.style.marginBottom="8px"})}async function Rn(e){var o,r;const t=document.getElementById("ts-id").value,n=document.getElementById("ts-emoji").value.trim()||"⚽",i=document.getElementById("ts-title").value.trim(),d=document.getElementById("ts-editor"),l=(d?d.innerHTML:document.getElementById("ts-content").value).trim(),g=document.getElementById("ts-color-hex").value.trim()||"#1A6B3C",x=parseInt(document.getElementById("ts-order").value)||0,E=document.getElementById("ts-active").checked;if(!i||!l){alert("Titre et contenu sont obligatoires.");return}const a={emoji:n,title:i,content:l,color:g,step_order:x,is_active:E,image_url:((r=(o=document.getElementById("ts-image"))==null?void 0:o.value)==null?void 0:r.trim())||null};let c;if(t?{error:c}=await _.from("tutorial_steps").update(a).eq("id",t):{error:c}=await _.from("tutorial_steps").insert(a),c){alert("Erreur : "+c.message);return}yt(),W(e)}const bt="/",vt=[["MA","Maroc"],["FR","France"],["AR","Argentine"],["PT","Portugal"],["BR","Brésil"],["ES","Espagne"],["DE","Allemagne"],["GB","Angleterre"],["IT","Italie"],["CM","Cameroun"],["SN","Sénégal"],["NG","Nigéria"],["DK","Danemark"],["NL","Pays-Bas"],["BE","Belgique"],["CI","Côte d'Ivoire"],["AL","Albanie"],["HR","Croatie"],["RS","Serbie"],["TR","Turquie"],["MW","Malawi"],["GH","Ghana"],["ZA","Afrique du Sud"],["EG","Égypte"],["US","États-Unis"],["MX","Mexique"],["CO","Colombie"],["UY","Uruguay"],["CL","Chili"],["JP","Japon"],["KR","Corée du Sud"],["CN","Chine"],["AU","Australie"],["SW","Suisse"],["SE","Suède"],["NO","Norvège"],["PL","Pologne"]];async function jn(e,t){await ce(e,t)}async function ce(e,t){var d,l;const[{data:n},{data:i}]=await Promise.all([_.from("stadium_definitions").select("*, club:clubs(encoded_name, logo_url)").order("name"),_.from("clubs").select("id, encoded_name, logo_url").order("encoded_name")]);e.innerHTML=`
    ...
    </div>`,(d=document.getElementById("st-add-btn"))==null||d.addEventListener("click",()=>qe(null,i,e,t)),(l=document.getElementById("st-gen-countries-btn"))==null||l.addEventListener("click",async()=>{const{toast:g}=t,x=new Set((n||[]).filter(r=>!r.club_id&&r.country_code).map(r=>r.country_code)),E=vt.filter(([r])=>!x.has(r));if(!E.length){g("Tous les pays ont déjà un stade.","info");return}if(!confirm(`Créer ${E.length} stade(s) "Pays" manquant(s) ?`))return;const a=E.map(([r,u])=>({name:`Stade ${u}`,club_id:null,country_code:r})),{data:c,error:o}=await _.from("stadium_definitions").insert(a).select();if(o){g("Erreur : "+o.message,"error");return}c!=null&&c.length&&await _.from("cards").insert(c.map(r=>({card_type:"stadium",stadium_id:r.id}))),g(`${(c==null?void 0:c.length)||0} stade(s) Pays créé(s) ✅`,"success"),ce(e,t)}),e.querySelectorAll("[data-edit-stadium]").forEach(g=>{g.addEventListener("click",()=>{const x=(n||[]).find(E=>E.id===g.dataset.editStadium);x&&qe(x,i,e,t)})})}function Dn(e){var d,l;let t=null;e.image_url?t=e.image_url.startsWith("http")?e.image_url:`${bt}icons/${e.image_url}`:(d=e.club)!=null&&d.logo_url?t=e.club.logo_url:e.country_code&&(t=`https://flagsapi.com/${e.country_code.slice(0,2).toUpperCase()}/flat/64.png`);const n=((l=e.club)==null?void 0:l.encoded_name)||e.country_code||"—",i=Xe(e.name,t,`${n}<br>+10 ⭐`,{width:140});return`<div data-edit-stadium="${e.id}" style="cursor:pointer;transition:transform .15s" onmouseover="this.style.transform='translateY(-3px)'" onmouseout="this.style.transform=''">
    ${i}
  </div>`}function qe(e,t,n,i){var c,o,r;const{openModal:d,closeModal:l,toast:g}=i,x=`
      ...
      <div style="flex:1;min-width:220px;display:flex;flex-direction:column;gap:12px">
        ...
        <div>
          ...
          <select id="st-country">
            ...
            ${vt.map(([u,p])=>`<option value="${u}" ${(e==null?void 0:e.country_code)===u?"selected":""}>${p} (${u})</option>`).join("")}
          </select>
        </div>
        <div>
  ...
  `);const a=()=>{var h,s,I,k,A,B,S;const u=((h=document.getElementById("st-name"))==null?void 0:h.value)||"NOM DU STADE",p=(I=(s=document.getElementById("st-image"))==null?void 0:s.value)==null?void 0:I.trim(),f=(A=(k=document.getElementById("st-country"))==null?void 0:k.value)==null?void 0:A.trim(),m=document.getElementById("st-club"),b=(m==null?void 0:m.selectedIndex)||0,$=m&&b>0?m.options[b].text:"",L=((S=(B=m==null?void 0:m.options[b])==null?void 0:B.getAttribute)==null?void 0:S.call(B,"data-logo"))||"";let v=null;p?v=p.startsWith("http")?p:`${bt}icons/${p}`:L?v=L:f&&(v=`https://flagsapi.com/${f.toLowerCase().slice(0,2).toUpperCase()}/flat/64.png`);const w=$||f||"—",y=document.getElementById("st-preview-card");y&&(y.innerHTML=Xe(u,v,`${w}<br>+10 ⭐`,{width:140}))};a(),["st-name","st-club","st-country","st-image"].forEach(u=>{var p,f;(p=document.getElementById(u))==null||p.addEventListener("input",a),(f=document.getElementById(u))==null||f.addEventListener("change",a)}),(c=document.getElementById("st-cancel"))==null||c.addEventListener("click",()=>l()),(o=document.getElementById("st-save"))==null||o.addEventListener("click",async()=>{const u=document.getElementById("st-name").value.trim(),p=document.getElementById("st-club").value||null,f=document.getElementById("st-country").value.trim().toUpperCase()||null,m=document.getElementById("st-image").value.trim()||null;if(!u){g("Le nom est obligatoire","error");return}const b={name:u,club_id:p,country_code:f,image_url:m},{error:$}=e?await _.from("stadium_definitions").update(b).eq("id",e.id):await _.from("stadium_definitions").insert(b);if($){g("Erreur : "+$.message,"error");return}g(e?"Stade modifié ✅":"Stade créé ✅","success"),l(),ce(n,i)}),(r=document.getElementById("st-delete"))==null||r.addEventListener("click",async()=>{confirm("Supprimer ce stade ?")&&(await _.from("stadium_definitions").delete().eq("id",e.id),g("Stade supprimé","success"),l(),ce(n,i))})}const q=[{value:"game_changer",label:"⚡ Game Changer",border:"#7a28b8",bg:"linear-gradient(135deg,#4a0080,#7a28b8)"},{value:"ultra_game_changer",label:"💎 Ultra Game Changer",border:"#00bcd4",bg:"linear-gradient(135deg,#006080,#00bcd4)"}],Pn=[{value:"purple",label:"Violet",hex:"#7a28b8"},{value:"light_blue",label:"Bleu clair",hex:"#00bcd4"}],H=[{value:"BOOST_STAT",label:"⚡ Boost stats (+N)",hasValue:!0,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"DEBUFF_STAT",label:"💀 Debuff stats (-N)",hasValue:!0,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"GRAY_PLAYER",label:"❄️ Griser un joueur",hasValue:!1,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"REVIVE_PLAYER",label:"💫 Ressusciter un joueur",hasValue:!1,hasTarget:!1,hasCount:!0,hasRoles:!1},{value:"REMOVE_GOAL",label:"🚫 Retirer un but adverse",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1},{value:"ADD_GOAL_DRAW",label:"⚽ +1 but si duel nul",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1},{value:"ADD_SUB",label:"🔄 Ajouter un remplacement",hasValue:!1,hasTarget:!1,hasCount:!0,hasRoles:!1},{value:"CUSTOM",label:"🛠️ Effet custom (hardcodé)",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1}],Ke=["GK","DEF","MIL","ATT"];async function Nn(e,t){await Ae(e,t)}async function Ae(e,t){var i;const{data:n}=await _.from("gc_definitions").select("*").order("sort_order").order("created_at");e.innerHTML=`
    <div style="padding:20px;max-width:1100px;margin:0 auto">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px">
    ...
    </div>`,(i=document.getElementById("gc-add-btn"))==null||i.addEventListener("click",()=>Je(null,e,t)),e.querySelectorAll("[data-edit-gc]").forEach(d=>{d.addEventListener("click",()=>{const l=(n||[]).find(g=>g.id===d.dataset.editGc);l&&Je(l,e,t)})}),e.querySelectorAll(".btn-del-gc").forEach(d=>{d.addEventListener("click",async l=>{l.stopPropagation(),confirm("Supprimer cette carte Game Changer ?")&&(await _.from("gc_definitions").delete().eq("id",d.dataset.del),Ae(e,t))})})}function Fn(e){const t=q.find(d=>d.value===e.gc_type)||q[0],n=e.image_url?`/icons/${e.image_url}`:null,i=ye(e.name||"?",n,t.label.split(" ")[0],e.effect||"",{width:130});return`<div data-edit-gc="${e.id}" style="position:relative;cursor:pointer">
    ${i}
    <div style="position:absolute;top:4px;left:4px;z-index:10;display:flex;gap:3px">
  ...
  </div>`}function Je(e,t,n){var r,u,p,f;const{openModal:i,closeModal:d,toast:l}=n,g=!e,x=e||{name:"",gc_type:"game_changer",color:"purple",effect:"",image_url:"",sort_order:0,is_active:!0,effect_type:"BOOST_STAT",effect_params:{}},E=(()=>{const m=x.effect_params||{},b=H.find($=>$.value===(x.effect_type||"BOOST_STAT"))||H[0];return`
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
      ${b.hasValue?`<div>
      ...
      ${b.hasRoles?`<div>
        ...
        <div style="display:flex;gap:4px;flex-wrap:wrap">
          ${Ke.map($=>`<label style="display:flex;align-items:center;gap:2px;font-size:11px;cursor:pointer;color:#555">
            <input type="checkbox" class="gc-p-role" value="${$}" ${!m.roles||m.roles.includes($)?"checked":""}> ${$}
          </label>`).join("")}
        </div>
      </div>`:""}
    </div>`})(),a=`
  ...
  `);const o=()=>{var y,h,s,I,k;const m=((y=document.getElementById("gc-name"))==null?void 0:y.value)||"?",b=((h=document.getElementById("gc-effect"))==null?void 0:h.value)||"",$=((s=document.getElementById("gc-type"))==null?void 0:s.value)||"game_changer",L=(k=(I=document.getElementById("gc-image-url"))==null?void 0:I.value)==null?void 0:k.trim(),v=q.find(A=>A.value===$)||q[0],w=document.getElementById("gc-modal-preview");w&&(w.innerHTML=ye(m,L?`/icons/${L}`:null,v.label.split(" ")[0],b,{width:150}))};["gc-name","gc-effect","gc-type","gc-image-url"].forEach(m=>{var b,$;(b=document.getElementById(m))==null||b.addEventListener("input",o),($=document.getElementById(m))==null||$.addEventListener("change",o)}),(r=document.getElementById("gc-effect-type"))==null||r.addEventListener("change",()=>{const m=document.getElementById("gc-effect-type").value,b=H.find(L=>L.value===m)||H[0],$=document.getElementById("gc-params-wrap");$.innerHTML=`
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
      ...
      </div>`,m.querySelectorAll(".gc-icon-item").forEach(y=>{y.addEventListener("click",()=>{const h=document.getElementById("gc-image-url");h&&(h.value=y.dataset.name),m.style.display="none",o()})})}catch($){m.innerHTML=`<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Erreur API GitHub : ${$.message}</div>`}}}),(p=document.getElementById("gc-cancel"))==null||p.addEventListener("click",()=>d()),(f=document.getElementById("gc-save"))==null||f.addEventListener("click",async()=>{var w,y,h,s,I,k,A,B,S,C,T,O,U,G;const m=H.find(N=>{var D;return N.value===(((D=document.getElementById("gc-effect-type"))==null?void 0:D.value)||"BOOST_STAT")})||H[0],b=m.hasRoles?[...document.querySelectorAll(".gc-p-role:checked")].map(N=>N.value):null,$={...m.hasValue?{value:Number((w=document.getElementById("gc-p-value"))==null?void 0:w.value)||2}:{},...m.hasCount?{count:Number((y=document.getElementById("gc-p-count"))==null?void 0:y.value)||1}:{},...m.hasTarget?{target:((h=document.getElementById("gc-p-target"))==null?void 0:h.value)||"home"}:{},...m.hasRoles?{roles:b!=null&&b.length?b:null}:{}},L={name:((I=(s=document.getElementById("gc-name"))==null?void 0:s.value)==null?void 0:I.trim())||"",effect:((A=(k=document.getElementById("gc-effect"))==null?void 0:k.value)==null?void 0:A.trim())||null,image_url:((S=(B=document.getElementById("gc-image-url"))==null?void 0:B.value)==null?void 0:S.trim())||null,gc_type:((C=document.getElementById("gc-type"))==null?void 0:C.value)||"game_changer",color:((T=document.getElementById("gc-color"))==null?void 0:T.value)||"purple",sort_order:Number((O=document.getElementById("gc-sort"))==null?void 0:O.value)||0,is_active:((U=document.getElementById("gc-active"))==null?void 0:U.checked)??!0,effect_type:((G=document.getElementById("gc-effect-type"))==null?void 0:G.value)||"BOOST_STAT",effect_params:$};if(!L.name){l("Le nom est obligatoire","error");return}const{error:v}=g?await _.from("gc_definitions").insert(L):await _.from("gc_definitions").update(L).eq("id",x.id);if(v){l(v.message,"error");return}l(g?"Carte créée ✅":"Carte modifiée ✅","success"),d(),Ae(t,n)})}async function On(e,{toast:t,openModal:n,closeModal:i}){await j(e,{toast:t,openModal:n,closeModal:i})}async function j(e,t){var y,h;const{toast:n,openModal:i,closeModal:d}=t,[{data:l,error:g},{data:x},{data:E},{data:a}]=await Promise.all([_.from("ranked_seasons").select("*").order("start_at",{ascending:!1}),_.from("users").select("mmr,rank_tier,placement_matches").gte("placement_matches",1),_.from("season_reward_tiers").select("*").order("rank_min"),_.from("booster_configs").select("id,name").order("sort_order")]);if(g){e.innerHTML=`<p style="color:red">${g.message}</p>`;return}const c=l||[],o=c.find(s=>s.is_active),r=(x||[]).filter(s=>(s.placement_matches||0)>=1),u=r.filter(s=>(s.placement_matches||0)>=10),p=["bronze","silver","gold","platinum","diamond","master"],f={bronze:"Bronze",silver:"Argent",gold:"Or",platinum:"Platine",diamond:"Diamant",master:"Maître"},m={bronze:"#cd7f32",silver:"#a0a0a0",gold:"#D4A017",platinum:"#00e5ff",diamond:"#5b9bd5",master:"#9b59b6"},b={bronze:"🥉",silver:"🥈",gold:"🥇",platinum:"💎",diamond:"🔷",master:"👑"},$={};p.forEach(s=>{$[s]=0}),u.forEach(s=>{const I=s.rank_tier||"bronze";$[I]!==void 0&&$[I]++});const L=u.length?Math.round(u.reduce((s,I)=>s+(I.mmr||1e3),0)/u.length):0;function v(s){return s?new Date(s).toLocaleDateString("fr",{day:"numeric",month:"short",year:"numeric"}):"—"}function w(s){const I=new Date;return s.is_active?'<span style="background:#1A6B3C;color:#fff;padding:2px 8px;border-radius:10px;font-size:11px;font-weight:700">● ACTIVE</span>':new Date(s.end_at)<I?'<span style="background:#555;color:#ccc;padding:2px 8px;border-radius:10px;font-size:11px">Terminée</span>':'<span style="background:#e67e22;color:#fff;padding:2px 8px;border-radius:10px;font-size:11px">À venir</span>'}e.innerHTML=`
    <div style="display:flex;flex-direction:column;gap:20px">
      ...
      <div class="card-panel" style="background:linear-gradient(135deg,#0a3d1e,#1a6b3c20);border:1.5px solid #1A6B3C40">
        ...
        <div style="display:flex;flex-direction:column;gap:4px">
          ${p.map(s=>{const I=$[s],k=u.length>0?Math.round(I/u.length*100):0;return`
              <div style="display:flex;align-items:center;gap:8px;font-size:12px">
                ...
                <span style="width:40px;text-align:right;color:var(--gray-600)">${I} (${k}%)</span>
              </div>`}).join("")}
        ...
        <div class="table-wrap">
          <table>
            ...
            <tbody id="seasons-tbody">
              ${c.length===0?'<tr><td colspan="6" style="text-align:center;color:var(--gray-600);padding:24px">Aucune saison créée.</td></tr>':c.map(s=>{const I=Math.round((new Date(s.end_at)-new Date(s.start_at))/864e5),k=new Date(s.end_at)<new Date&&!s.is_active;return`
                    <tr>
                      <td><b>${s.name}</b></td>
                      <td style="font-size:12px">${v(s.start_at)}</td>
                      <td style="font-size:12px">${v(s.end_at)}</td>
      ...
      <div class="card-panel">
        ...
        <div class="table-wrap">
          <table>
            ...
            <tbody>
              ${(E||[]).map(s=>{var A;const I=!s.activate_at||new Date(s.activate_at)<=new Date;return`<tr>
                  <td style="font-size:12px">${((A=c.find(B=>B.id===s.season_id))==null?void 0:A.name)||"—"}</td>
                  <td><b>${s.label}</b></td>
    ...
    </div>`,(y=document.getElementById("create-season-btn"))==null||y.addEventListener("click",()=>{Ye(null,{toast:n,openModal:i,closeModal:d,reload:()=>j(e,t)})}),e.querySelectorAll("[data-edit]").forEach(s=>{const I=c.find(k=>k.id==s.dataset.edit);s.addEventListener("click",()=>{Ye(I,{toast:n,openModal:i,closeModal:d,reload:()=>j(e,t)})})}),e.querySelectorAll("[data-activate]").forEach(s=>{s.addEventListener("click",async()=>{const I=parseInt(s.dataset.activate);if(!confirm("Activer cette saison ? La saison actuellement active sera désactivée."))return;const{error:k}=await _.from("ranked_seasons").update({is_active:!1}).neq("id",0);if(k){n(k.message,"error");return}const{error:A}=await _.from("ranked_seasons").update({is_active:!0}).eq("id",I);if(A){n(A.message,"error");return}await _.from("users").update({current_season_id:I}).gt("placement_matches",0),n("Saison activée ✅","success"),j(e,t)})}),e.querySelectorAll("[data-deactivate]").forEach(s=>{s.addEventListener("click",async()=>{if(!confirm("Désactiver la saison active ? Le mode Ranked sera suspendu."))return;const{error:I}=await _.from("ranked_seasons").update({is_active:!1}).eq("id",parseInt(s.dataset.deactivate));if(I){n(I.message,"error");return}n("Saison désactivée","success"),j(e,t)})}),e.querySelectorAll("[data-delete]").forEach(s=>{s.addEventListener("click",async()=>{if(!confirm("Supprimer cette saison archivée ?"))return;const{error:I}=await _.from("ranked_seasons").delete().eq("id",parseInt(s.dataset.delete));if(I){n(I.message,"error");return}n("Saison supprimée","success"),j(e,t)})}),(h=document.getElementById("add-reward-tier-btn"))==null||h.addEventListener("click",()=>{Ve(null,c,o,a||[],{toast:n,openModal:i,closeModal:d,reload:()=>j(e,t)})}),e.querySelectorAll("[data-edit-tier]").forEach(s=>{const I=(E||[]).find(k=>k.id===s.dataset.editTier);s.addEventListener("click",()=>{Ve(I,c,o,a||[],{toast:n,openModal:i,closeModal:d,reload:()=>j(e,t)})})}),e.querySelectorAll("[data-distribute-tier]").forEach(s=>{s.addEventListener("click",async()=>{const I=(E||[]).find(B=>B.id===s.dataset.distributeTier);if(!confirm(`Distribuer les récompenses du palier "${I==null?void 0:I.label}" à tous les joueurs concernés ?`))return;s.disabled=!0,s.textContent="⏳...";const{data:k,error:A}=await _.rpc("admin_distribute_season_reward",{p_tier_id:s.dataset.distributeTier});if(A){n(A.message,"error"),s.disabled=!1,s.textContent="🎁 Distribuer";return}if(!(k!=null&&k.success)){n((k==null?void 0:k.error)||"Échec de la distribution","error"),s.disabled=!1,s.textContent="🎁 Distribuer";return}n(`Récompenses distribuées à ${k.rewarded} joueur(s) ✅`,"success"),j(e,t)})}),e.querySelectorAll("[data-delete-tier]").forEach(s=>{s.addEventListener("click",async()=>{if(!confirm("Supprimer ce palier de récompense ?"))return;const{error:I}=await _.from("season_reward_tiers").delete().eq("id",s.dataset.deleteTier);if(I){n(I.message,"error");return}n("Palier supprimé","success"),j(e,t)})})}function Ve(e,t,n,i,{toast:d,openModal:l,closeModal:g,reload:x}){var f,m,b,$,L;const E=!!e,a=(e==null?void 0:e.season_id)??(n==null?void 0:n.id)??((f=t[0])==null?void 0:f.id)??null;let c=[];const o=v=>{if(!v)return"";const w=new Date(v);return new Date(w.getTime()-w.getTimezoneOffset()*6e4).toISOString().slice(0,16)},r=`
    <div style="display:flex;flex-direction:column;gap:12px">
      <div>
        <label>SAISON RANKED</label>
        <select id="rt-season">
          ${t.map(v=>`<option value="${v.id}" ${v.id===a?"selected":""}>${v.name}${v.is_active?" (active)":""}</option>`).join("")}
        </select>
      </div>
      <div>
        <label>LIBELLÉ</label>
  ...
  `);function p(){const v=document.getElementById("rt-player-chips");v&&(v.innerHTML=c.map(w=>`
      ...
      </div>`).join(""),y.querySelectorAll("[data-add-player]").forEach(h=>{h.addEventListener("click",()=>{const s=h.dataset.addPlayer;c.some(I=>I.id===s)||c.push({id:s,label:h.dataset.label}),p(),y.style.display="none",document.getElementById("rt-player-search").value=""})})}),($=document.getElementById("rt-cancel"))==null||$.addEventListener("click",()=>g()),(L=document.getElementById("rt-save"))==null||L.addEventListener("click",async()=>{const v=document.getElementById("rt-error"),w=parseInt(document.getElementById("rt-season").value),y=document.getElementById("rt-label").value.trim(),h=parseInt(document.getElementById("rt-rank-min").value)||1,s=parseInt(document.getElementById("rt-rank-max").value)||1,I=parseInt(document.getElementById("rt-credits").value)||0,k=document.getElementById("rt-activate-at").value,A=k?new Date(k).toISOString():null,B=[...document.querySelectorAll(".rt-booster-cb:checked")].map(T=>T.value);if(!y){v.textContent="Le libellé est obligatoire.";return}if(h>s){v.textContent="Le classement min doit être ≤ au max.";return}if(!w){v.textContent="Choisis une saison.";return}const S={season_id:w,label:y,rank_min:h,rank_max:s,credits:I,booster_config_ids:B,player_ids:c.map(T=>T.id),activate_at:A},{error:C}=E?await _.from("season_reward_tiers").update(S).eq("id",e.id):await _.from("season_reward_tiers").insert(S);if(C){v.textContent=C.message;return}d(E?"Palier modifié ✅":"Palier créé ✅","success"),g(),x()})}function Ye(e,{toast:t,openModal:n,closeModal:i,reload:d}){const l=!!e,g=new Date().toISOString().slice(0,16),x=new Date(Date.now()+90*864e5).toISOString().slice(0,16),E=e?new Date(e.start_at).toISOString().slice(0,16):g,a=e?new Date(e.end_at).toISOString().slice(0,16):x,c=(e==null?void 0:e.name)||"";n(l?"✏️ Modifier la saison":"➕ Nouvelle saison",`<div style="display:flex;flex-direction:column;gap:14px">
      <div class="form-group">
    ...
    </div>`);function o(){var m,b;const r=(m=document.getElementById("season-start"))==null?void 0:m.value,u=(b=document.getElementById("season-end"))==null?void 0:b.value,p=document.getElementById("season-duration");if(!r||!u||!p)return;const f=Math.round((new Date(u)-new Date(r))/864e5);p.textContent=f>0?`Durée : ${f} jour${f>1?"s":""}`:"⚠️ La fin doit être après le début",p.style.color=f>0?"var(--gray-600)":"#bb2020"}setTimeout(()=>{var r,u,p;(r=document.getElementById("season-start"))==null||r.addEventListener("input",o),(u=document.getElementById("season-end"))==null||u.addEventListener("input",o),o(),(p=document.getElementById("season-save-btn"))==null||p.addEventListener("click",async()=>{var y,h,s;const f=(y=document.getElementById("season-name"))==null?void 0:y.value.trim(),m=(h=document.getElementById("season-start"))==null?void 0:h.value,b=(s=document.getElementById("season-end"))==null?void 0:s.value,$=document.getElementById("season-error");if(!f){$.textContent="Le nom est requis.";return}if(!m){$.textContent="La date de début est requise.";return}if(!b){$.textContent="La date de fin est requise.";return}if(new Date(b)<=new Date(m)){$.textContent="La date de fin doit être après le début.";return}const L=document.getElementById("season-save-btn");L.disabled=!0,L.textContent="Enregistrement…";const v={name:f,start_at:new Date(m).toISOString(),end_at:new Date(b).toISOString()};let w;if(l?{error:w}=await _.from("ranked_seasons").update(v).eq("id",e.id):{error:w}=await _.from("ranked_seasons").insert({...v,is_active:!1}),w){$.textContent=w.message,L.disabled=!1,L.textContent=l?"💾 Enregistrer":"✅ Créer la saison";return}t(l?"Saison modifiée ✅":"Saison créée ✅","success"),i(),d()})},50)}async function xe(e,t){var E;const{toast:n}=t,{data:i,error:d}=await _.from("solo_levels").select("*").order("level_number");if(d){e.innerHTML=`<p style="color:var(--danger)">${d.message}</p>`;return}const l=a=>`
    <tr data-row="${a.id}">
