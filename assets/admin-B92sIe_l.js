import{s as _,r as Ee,a as _e,A as ht,l as Et,g as _t,K as Me,D as z,b as wt,S as It,c as R,H as $t,R as kt,e as ue,d as Qe,f as Lt,h as At,E as Bt,i as St,N as Ct,M as Tt,B as Mt,j as zt,F as ge,k as ze,m as Rt,n as Xe,o as ye,p as jt}from"./special-cards-yI9ueqAE.js";async function Dt(e){var E;const[{count:t},{count:n},{count:a},{count:c},{count:r},{data:g}]=await Promise.all([_.from("players").select("*",{count:"exact",head:!0}),_.from("clubs").select("*",{count:"exact",head:!0}),_.from("users").select("*",{count:"exact",head:!0}),_.from("stadium_definitions").select("*",{count:"exact",head:!0}),_.from("users").select("*",{count:"exact",head:!0}).gt("last_seen_at",new Date(Date.now()-18e4).toISOString()),_.rpc("get_signup_password")]),x=g||"";e.innerHTML=`
  ...
  `,window.adminNav=i=>{var p;(p=document.querySelector(`[data-page="${i}"]`))==null||p.click()},(E=document.getElementById("signup-pwd-save"))==null||E.addEventListener("click",async()=>{const i=document.getElementById("signup-pwd-input").value.trim(),p=document.getElementById("signup-pwd-status");if(!i){p.textContent="Le code ne peut pas être vide.",p.style.color="#ff6b6b";return}const o=document.getElementById("signup-pwd-save");o.disabled=!0,o.textContent="⏳...";const{error:s}=await _.rpc("set_signup_password",{new_password:i});if(o.disabled=!1,o.textContent="💾 Enregistrer",s){p.textContent=s.message,p.style.color="#ff6b6b";return}p.textContent="✅ Code mis à jour.",p.style.color="#2ecc71"})}const et={normal:"Normal",pepite:"Pépite",papyte:"Papyte",legende:"Légende"},Pt={GK:"#aaaaaa",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};async function Nt(){const e={};for(const t of ht)e[t]=await Et(t);return e}async function Ft(e,t){const{toast:n}=t;if(!confirm(`Réattribuer une nouvelle photo à tous les joueurs ayant encore l'ancien format de chemin (public/faces/...) ?

Cette action est irréversible.`))return;const a=document.getElementById("fix-old-faces-btn");a&&(a.disabled=!0,a.textContent="⏳ En cours...");try{const{data:c,error:r}=await _.from("players").select("id, country_code, face").like("face","public/faces/%");if(r){n(r.message,"error");return}if(!(c!=null&&c.length)){n("Aucun joueur avec l'ancien format — rien à faire ✅","success");return}const{data:g}=await _.from("players").select("face").not("face","is",null),x=new Set((g||[]).map(p=>p.face).filter(p=>p&&!p.startsWith("public/faces/")));let E=0,i=0;for(const p of c){const o=await _e(p.country_code,x);if(!o){i++;continue}const{error:s}=await _.from("players").update({face:o}).eq("id",p.id);if(s){i++;continue}x.add(o),E++}n(`${E} photo(s) réattribuée(s)${i?`, ${i} échec(s)`:""} ✅`,i?"error":"success")}finally{a&&(a.disabled=!1,a.textContent="🖼️ Réattribuer anciennes photos")}}async function Ot(e,t){await se(e,t)}async function se(e,t,n=null){var i,p,o,s,u;const{toast:a}=t;n||(n={search:((i=document.getElementById("search-players"))==null?void 0:i.value)||"",job:((p=document.getElementById("filter-job"))==null?void 0:p.value)||"",rarity:((o=document.getElementById("filter-rarity"))==null?void 0:o.value)||"",club:((s=document.getElementById("filter-club"))==null?void 0:s.value)||"",country:((u=document.getElementById("filter-country"))==null?void 0:u.value)||""});const[{data:c,error:r},{data:g}]=await Promise.all([_.from("players").select("*, clubs(id,encoded_name,logo_url)"),_.from("clubs").select("id,encoded_name").order("encoded_name")]);if(r){e.innerHTML=`<p style="color:red">${r.message}</p>`;return}const x={GK:0,DEF:1,MIL:2,ATT:3},E=(c||[]).sort((d,f)=>{const m=(x[d.job]??4)-(x[f.job]??4);return m!==0?m:(d.surname_real||"").localeCompare(f.surname_real||"")});Ut(e,E,g||[],t,n)}function Ut(e,t,n,a,c=null){var p,o;const{toast:r}=a;if(e.innerHTML=`
    <div style="display:flex;flex-direction:column;gap:10px">
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        ...
        <select id="filter-rarity" style="width:110px">
          ...
          ${Object.entries(et).map(([s,u])=>`<option value="${s}">${u}</option>`).join("")}
        </select>
    ...
    </div>`,c){const s=c,u=d=>document.getElementById(d);s.search&&u("search-players")&&(u("search-players").value=s.search),s.job&&u("filter-job")&&(u("filter-job").value=s.job),s.rarity&&u("filter-rarity")&&(u("filter-rarity").value=s.rarity),s.club&&u("filter-club")&&(u("filter-club").value=s.club),s.country&&u("filter-country")&&(u("filter-country").value=s.country)}if(c){const s=c,u=d=>document.getElementById(d);s.search&&u("search-players")&&(u("search-players").value=s.search),s.job&&u("filter-job")&&(u("filter-job").value=s.job),s.rarity&&u("filter-rarity")&&(u("filter-rarity").value=s.rarity),s.club&&u("filter-club")&&(u("filter-club").value=s.club),s.country&&u("filter-country")&&(u("filter-country").value=s.country)}function g(){const s=document.getElementById("search-players").value.toLowerCase(),u=document.getElementById("filter-job").value,d=document.getElementById("filter-rarity").value,f=document.getElementById("filter-club").value,m=document.getElementById("filter-country").value;return t.filter(b=>(!s||`${b.firstname} ${b.surname_real}`.toLowerCase().includes(s))&&(!u||b.job===u)&&(!d||b.rarity===d)&&(!f||b.club_id===f)&&(!m||b.country_code===m))}const x=new Set;function E(){const s=document.getElementById("bulk-bar"),u=document.getElementById("bulk-count");s&&(s.style.display=x.size>0?"flex":"none",u&&(u.textContent=`${x.size} joueur(s) sélectionné(s)`))}function i(){const s=g();document.getElementById("count-label").textContent=`${s.length} joueur(s)`;const u=document.getElementById("players-list");if(!s.length){u.innerHTML='<div style="color:var(--gray-600);padding:20px;text-align:center">Aucun joueur.</div>';return}u.innerHTML=s.map(d=>{const f={...d,clubs:d.clubs,face:d.face||null},m=Ee(f,{width:120});return`<div style="position:relative;cursor:pointer" data-edit="${d.id}">
        ${m}
      ...
      </div>`}).join(""),u.querySelectorAll("[data-edit]").forEach(d=>{d.addEventListener("click",()=>{const f=t.find(m=>m.id===d.dataset.edit);f&&Re(f,n,e,a)})}),u.querySelectorAll(".btn-del-player").forEach(d=>{d.addEventListener("click",async f=>{if(f.stopPropagation(),!confirm("Supprimer ce joueur ?"))return;const{error:m}=await _.from("players").delete().eq("id",d.dataset.del);m?r(m.message,"error"):(r("Joueur supprimé ✅","success"),se(e,a))})})}i(),document.getElementById("search-players").addEventListener("input",i),document.getElementById("filter-job").addEventListener("change",i),document.getElementById("filter-rarity").addEventListener("change",i),document.getElementById("filter-club").addEventListener("change",i),document.getElementById("filter-country").addEventListener("change",i),(p=document.getElementById("bulk-cancel-btn"))==null||p.addEventListener("click",()=>{x.clear(),E(),i()}),(o=document.getElementById("bulk-delete-btn"))==null||o.addEventListener("click",async()=>{var f,m,b,k,L;if(!x.size||!confirm(`Supprimer ${x.size} joueur(s) ?`))return;const s=[...x],{error:u}=await _.from("players").delete().in("id",s);if(u){r(u.message,"error");return}r(`${s.length} joueur(s) supprimé(s) ✅`,"success"),x.clear();const d={search:((f=document.getElementById("search-players"))==null?void 0:f.value)||"",job:((m=document.getElementById("filter-job"))==null?void 0:m.value)||"",rarity:((b=document.getElementById("filter-rarity"))==null?void 0:b.value)||"",club:((k=document.getElementById("filter-club"))==null?void 0:k.value)||"",country:((L=document.getElementById("filter-country"))==null?void 0:L.value)||""};se(e,a,d)}),document.getElementById("add-player-btn").addEventListener("click",()=>Re(null,n,e,a)),document.getElementById("fix-old-faces-btn").addEventListener("click",()=>Ft(e,a))}async function Re(e,t,n,a){const{toast:c,openModal:r,closeModal:g}=a,x=!!e,E=await Nt(),{data:i}=await _.from("players").select("face").not("face","is",null),p=new Set((i||[]).map(d=>d.face).filter(Boolean));e!=null&&e.face&&p.delete(e.face);const o='<option value="">— Club —</option>'+t.map(d=>`<option value="${d.id}" ${(e==null?void 0:e.club_id)===d.id?"selected":""}>${d.encoded_name}</option>`).join(""),s=e!=null&&e.face?e.face.split("/")[0]:"",u=Object.keys(E).map(d=>`<option value="${d}" ${s===d?"selected":""}>${d}</option>`).join("");r(x?`✏️ ${e.firstname} ${e.surname_real}`:"➕ Nouveau joueur",`<div style="display:flex;gap:20px;align-items:flex-start">
      ...
      <div style="flex:1;min-width:300px;display:flex;flex-direction:column;gap:12px">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:8px">
          ...
          <div class="form-group">
            <label>Rareté</label>
            <select id="pm-rarity">
              ${["normal","pepite","papyte","legende"].map(d=>`<option value="${d}" ${(e==null?void 0:e.rarity)===d?"selected":""}>${et[d]}</option>`).join("")}
            </select>
          </div>
          <div class="form-group">
            <label>Pays</label>
        ...
        <div style="border-top:1px solid var(--gray-200);padding-top:10px">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:8px">
            ${[["GK","pm-g","note_g"],["DEF","pm-d","note_d"],["MIL","pm-m","note_m"],["ATT","pm-a","note_a"]].map(([d,f,m])=>`
              <div class="form-group" style="text-align:center">
                <label style="color:${Pt[d]};font-weight:700">${d}</label>
    ...
    <div style="display:flex;gap:16px;height:calc(100vh - 56px);overflow:hidden;padding:16px">
    ...
    </div>`,g(re),document.getElementById("search-clubs").addEventListener("input",x=>{const E=x.target.value.toLowerCase();g(re.filter(i=>i.real_name.toLowerCase().includes(E)||i.encoded_name.toLowerCase().includes(E)))}),document.getElementById("add-club-btn").addEventListener("click",()=>je(null,e,t));function g(x){const E=document.getElementById("clubs-list");if(!x.length){E.innerHTML='<p style="color:var(--tile-fg-dim);padding:20px;text-align:center">Aucun club.</p>';return}const i={};x.forEach(s=>{const u=s.country_code||"—";i[u]||(i[u]=[]),i[u].push(s)});const p=Object.keys(i).sort(),o=s=>{const u=dt(s),d=s.logo_url?`<img src="${s.logo_url}" style="width:36px;height:36px;object-fit:contain;border-radius:8px;flex-shrink:0">`:`<div style="width:36px;height:36px;background:linear-gradient(135deg,${u.color1},${u.color2});border-radius:8px;display:flex;align-items:center;justify-content:center;color:#fff;font-size:9px;font-weight:900;flex-shrink:0">${s.encoded_name.slice(0,3)}</div>`,f=n[s.id]||0;return`
      ...
      <div style="display:flex;align-items:center;gap:8px;padding:8px 4px 4px;margin-top:6px">
        ...
        <span style="font-size:11px;color:var(--tile-fg-dim);background:var(--tile-bg);border:1px solid var(--tile-border);border-radius:10px;padding:1px 8px">${i[s].length}</span>
        <div style="flex:1;height:1px;background:var(--tile-border)"></div>
      </div>
      <div style="display:flex;flex-direction:column;gap:6px;margin-bottom:4px">
        ${i[s].map(o).join("")}
      </div>
    `).join(""),E.querySelectorAll(".club-row").forEach(s=>{s.addEventListener("click",()=>{const u=re.find(d=>d.id===s.dataset.clubId);u&&je(u,e,t),E.querySelectorAll(".club-row").forEach(d=>d.style.background="var(--tile-bg)"),s.style.background="rgba(26,107,60,0.18)"})}),E.querySelectorAll(".btn-del-club").forEach(s=>{s.addEventListener("click",async()=>{if(!confirm("Supprimer ce club ?"))return;const{error:u}=await _.from("clubs").delete().eq("id",s.dataset.del);u?a(u.message,"error"):(a("Club supprimé","success"),me(e,t))})})}}async function je(e,t,n){var m,b,k,L,v,w;const{toast:a}=n,c=!!e,r=e?dt(e):{...z},g=document.getElementById("club-panel");if(!g)return;const x=Object.entries(Me).map(([y,h])=>`<option value="${y}" ${r.style===y?"selected":""}>${h.label}</option>`).join(""),E=we.map(y=>`<option value="${y}" ${((e==null?void 0:e.country_code)||"FR")===y?"selected":""}>${y}</option>`).join("");g.style.display="block",g.style.alignItems="",g.style.justifyContent="",g.innerHTML=`
      ...
      <div style="border-top:1px solid var(--tile-border);padding-top:12px">
        ...
        <div style="display:flex;gap:12px;align-items:flex-start;flex-wrap:wrap">
          <div style="flex:1;min-width:200px;display:flex;flex-direction:column;gap:8px">
            ...
            ${[["Couleur 1","m-kit-color1",r.color1,!1],["Couleur 2","m-kit-color2",r.color2,!1],["Couleur 3","m-kit-color3",r.color3,!0],["Short","m-kit-shorts",r.shorts,!1],["Chaussettes","m-kit-socks",r.socks,!1]].map(([y,h,l,I])=>`
              <div class="form-group" id="wrap-${h}" ${I?'style="display:none"':""}>
                <label>${y}</label>
                <div style="display:flex;gap:6px;align-items:center">
                  <input type="color" id="${h}" value="${l||"#000000"}" style="width:38px;height:32px;padding:2px;border:1px solid var(--tile-border);border-radius:6px;cursor:pointer;flex-shrink:0">
                  <input id="${h}-txt" value="${l||"#000000"}" maxlength="7" style="flex:1;font-family:monospace;font-size:12px">
                </div>
              </div>`).join("")}
          </div>
  ...
  `,(m=document.getElementById("m-logo-file"))==null||m.addEventListener("change",y=>{var I;const h=(I=y.target.files)==null?void 0:I[0];if(!h)return;const l=new FileReader;l.onload=()=>{document.getElementById("logo-preview").innerHTML=`<img src="${l.result}" style="width:100%;height:100%;object-fit:contain">`},l.readAsDataURL(h)}),ne();function i(){var I,$;const y=((I=document.getElementById("m-kit-style"))==null?void 0:I.value)||"uni",h=(($=Me[y])==null?void 0:$.colors)===3,l=document.getElementById("wrap-m-kit-color3");l&&(l.style.display=h?"":"none")}i(),["m-kit-color1","m-kit-color2","m-kit-color3","m-kit-shorts","m-kit-socks"].forEach(y=>{const h=document.getElementById(y),l=document.getElementById(y+"-txt");!h||!l||(h.addEventListener("input",()=>{l.value=h.value,ne()}),l.addEventListener("input",()=>{const I=l.value.trim();/^#[0-9a-fA-F]{6}$/.test(I)&&(h.value=I,ne())}),l.addEventListener("change",()=>{let I=l.value.trim();I.startsWith("#")||(I="#"+I),/^#[0-9a-fA-F]{6}$/.test(I)&&(h.value=I,l.value=I,ne())}))}),(b=document.getElementById("m-kit-style"))==null||b.addEventListener("change",()=>{i(),ne()});const p=document.getElementById("m-real"),o=document.getElementById("m-encoded");function s(){if(!p||!o||o.value)return;const y=p.value.trim().split(/\s+/),h=y.length===1?y[0].toUpperCase().slice(0,6):y.filter(l=>l.length>2).map(l=>l[0].toUpperCase()).join("")||y[0].toUpperCase().slice(0,4);o.value=h}p==null||p.addEventListener("input",s),(k=document.getElementById("auto-encode-btn"))==null||k.addEventListener("click",()=>{o&&(o.value=""),s()}),(L=document.getElementById("m-save"))==null||L.addEventListener("click",()=>ln(e,c,t,n));const u=document.getElementById("m-gen-squad"),d=document.getElementById("m-gen-strong-label");function f(){if(d&&(d.style.display=u!=null&&u.checked?"flex":"none",!(u!=null&&u.checked))){const y=document.getElementById("m-gen-strong");y&&(y.checked=!1)}}u==null||u.addEventListener("change",f),f(),(v=document.getElementById("btn-gen-squad-panel"))==null||v.addEventListener("click",()=>{tn(e,n,t)}),(w=document.getElementById("btn-gen-one-player"))==null||w.addEventListener("click",()=>{an(e,n)}),c&&$e(e.id)}async function $e(e){const{data:t}=await _.from("players").select("*, clubs(encoded_name, logo_url, kit_style, kit_color1, kit_color2, kit_color3, kit_shorts, kit_socks)").eq("club_id",e).order("surname_real"),n=document.getElementById("club-players-grid"),a=document.getElementById("club-players-count");if(a&&(a.textContent=(t||[]).length),!!n){if(!(t!=null&&t.length)){n.innerHTML='<div style="color:var(--tile-fg-dim);padding:20px;font-size:13px">Aucun joueur dans ce club.</div>';return}n.innerHTML=t.map(c=>Ee(c,{width:100})).join("")}}function tn(e,t,n){const{openModal:a,closeModal:c,toast:r}=t;a(`⚽ Générer — ${e.real_name}`,`<div style="display:flex;flex-direction:column;gap:14px">
    ...
    </div>`),setTimeout(()=>{var g,x;(g=document.getElementById("qg-cancel"))==null||g.addEventListener("click",()=>c()),(x=document.getElementById("qg-ok"))==null||x.addEventListener("click",async()=>{var i;const E=((i=document.getElementById("qg-strong"))==null?void 0:i.checked)??!1;c(),r("Génération en cours…","info"),await st(e.id,e.country_code,r,E),r("Effectif généré ✅","success"),$e(e.id),me(n,t)})},50)}const ct=["GK","DEF","MIL","ATT"],nn=[{value:"normal",label:"Normal"},{value:"pepite",label:"Pépite"},{value:"papyte",label:"Papyte"},{value:"legende",label:"Légende"}];function an(e,t){const{openModal:n,closeModal:a,toast:c}=t,r=we.map(i=>`<option value="${i}">${i}</option>`).join(""),g=i=>ct.map(p=>`<option value="${p}" ${p===i?"selected":""}>${p}</option>`).join(""),x=`
  ...
  `),setTimeout(()=>{var i,p;document.querySelectorAll('input[name="gen-one-mode"]').forEach(o=>{o.addEventListener("change",()=>{document.getElementById("gen-one-custom-fields").style.display=o.value==="custom"&&o.checked?"flex":"none"})}),(i=document.getElementById("gen-one-cancel"))==null||i.addEventListener("click",()=>a()),(p=document.getElementById("gen-one-ok"))==null||p.addEventListener("click",async()=>{var d,f,m,b,k;const o=((d=document.querySelector('input[name="gen-one-mode"]:checked'))==null?void 0:d.value)||"random",s=document.getElementById("gen-one-ok");s.disabled=!0,s.textContent="⏳ Génération…";const u={};o==="custom"&&(u.job=((f=document.getElementById("gen-one-job1"))==null?void 0:f.value)||null,u.job2=((m=document.getElementById("gen-one-job2"))==null?void 0:m.value)||null,u.rarity=((b=document.getElementById("gen-one-rarity"))==null?void 0:b.value)||null,u.country=((k=document.getElementById("gen-one-country"))==null?void 0:k.value)||null);try{await on(e,u),c("Joueur généré ✅","success"),a(),$e(e.id)}catch(L){c("Erreur : "+L.message,"error"),s.disabled=!1,s.textContent="➕ Générer"}})},50)}async function on(e,t={}){const n=t.country||(Math.random()<.5?e.country_code:it(e.country_code)),a=t.job||K(ct),c=t.job2||null,r=t.rarity||(()=>{const A=Math.random()*100;return A<1.5?"legende":A<3?"pepite":A<4.5?"papyte":"normal"})(),g=r==="pepite"?V(10,15):r==="papyte"?V(15,20):at(),x=c?Math.max(0,g-2):0;let E=0,i=0,p=0,o=0;a==="GK"&&(E=g),a==="DEF"&&(i=g),a==="MIL"&&(p=g),a==="ATT"&&(o=g),c==="GK"&&(E=x),c==="DEF"&&(i=x),c==="MIL"&&(p=x),c==="ATT"&&(o=x);const s=Ie(n),u=nt(n),d=K(tt),{data:f}=await _.from("players").select("surname_real").not("surname_real","is",null),m=new Set((f||[]).map(A=>A.surname_real).filter(Boolean)),b=lt(n),k=b.filter(A=>!m.has(A)),L=K(k.length?k:b),v={job:a,job2:c,firstname:rt(n),surname_real:L,country_code:n,club_id:e.id,note_g:E,note_d:i,note_m:p,note_a:o,skin:s,hair:u,hair_length:d,rarity:r,sell_price:0,ethnie:be(n)},{data:w}=await _.from("players").select("face").not("face","is",null),y=new Set((w||[]).map(A=>A.face).filter(Boolean)),h=await _e(n,y);h&&(v.face=h);const{data:l,error:I}=await _.from("players").insert(v).select("id").single();if(I)throw I;const{error:$}=await _.from("cards").insert({card_type:"player",player_id:l.id});$&&console.warn("[GenOnePlayer] Erreur carte:",$.message)}function pt(){var e,t,n,a,c,r;return{style:((e=document.getElementById("m-kit-style"))==null?void 0:e.value)||"uni",color1:((t=document.getElementById("m-kit-color1"))==null?void 0:t.value)||"#1A6B3C",color2:((n=document.getElementById("m-kit-color2"))==null?void 0:n.value)||"#ffffff",color3:((a=document.getElementById("m-kit-color3"))==null?void 0:a.value)||"#000000",shorts:((c=document.getElementById("m-kit-shorts"))==null?void 0:c.value)||"#111111",socks:((r=document.getElementById("m-kit-socks"))==null?void 0:r.value)||"#ffffff"}}function ne(){const e=document.getElementById("kit-preview-wrap");e&&(e.innerHTML=wt(pt(),"panel"))}async function rn(e,t){const n=(e.name.split(".").pop()||"png").toLowerCase(),a=`clubs/${t||"new"}-${Date.now()}.${n}`,{error:c}=await _.storage.from("assets").upload(a,e,{upsert:!0,cacheControl:"3600"});if(c)throw c;const{data:r}=_.storage.from("assets").getPublicUrl(a);return r.publicUrl}async function ln(e,t,n,a){var L,v,w,y,h,l,I,$,A;const{toast:c}=a,r=document.getElementById("m-error"),g=document.getElementById("m-save"),x=(L=document.getElementById("m-real"))==null?void 0:L.value.trim(),E=(v=document.getElementById("m-encoded"))==null?void 0:v.value.trim().toUpperCase(),i=(w=document.getElementById("m-country"))==null?void 0:w.value.trim().toUpperCase(),p=((h=(y=document.getElementById("m-logo-file"))==null?void 0:y.files)==null?void 0:h[0])||null,o=((l=document.getElementById("m-logo-url-current"))==null?void 0:l.value)||null,s=((I=document.getElementById("m-gen-stadium"))==null?void 0:I.checked)??!1,u=(($=document.getElementById("m-gen-squad"))==null?void 0:$.checked)??!1,d=((A=document.getElementById("m-gen-strong"))==null?void 0:A.checked)??!1,f=pt();if(!x){r.textContent="Le nom du club est requis.";return}if(!E){r.textContent="Le nom encodé est requis.";return}if(!i){r.textContent="Le pays est requis.";return}g.disabled=!0,g.textContent="Enregistrement…";let m=o;if(p)try{g.textContent="📤 Envoi du logo…",m=await rn(p,e==null?void 0:e.id)}catch(B){r.textContent="Erreur upload logo : "+B.message,g.disabled=!1,g.textContent=t?"💾 Enregistrer":"✅ Créer le club";return}const b={real_name:x,encoded_name:E,country_code:i,logo_url:m,kit_style:f.style,kit_color1:f.color1,kit_color2:f.color2,kit_color3:f.color3,kit_shorts:f.shorts,kit_socks:f.socks};let k=e==null?void 0:e.id;if(t){const{error:B}=await _.from("clubs").update(b).eq("id",k);if(B){r.textContent=B.message,g.disabled=!1,g.textContent="💾 Enregistrer";return}}else{const{data:B,error:S}=await _.from("clubs").insert(b).select().single();if(S){r.textContent=S.message,g.disabled=!1,g.textContent="✅ Créer le club";return}if(k=B.id,s){g.textContent="🏟️ Création du stade…";const{data:C,error:T}=await _.from("stadium_definitions").insert({name:`Stade de ${x}`,club_id:k,country_code:null}).select().single();T?console.warn("[Stadium] Erreur def stade:",T.message):C&&await _.from("cards").insert({card_type:"stadium",stadium_id:C.id})}u&&(g.textContent="⚽ Génération des joueurs…",await st(k,i,c,d))}c(t?"Club modifié ✅":"Club créé ✅","success"),me(n,a)}const sn=["normal","pepite","papyte","legende"],De=["GK","DEF","MIL","ATT"],dn=[["MA","Maroc"],["FR","France"],["AR","Argentine"],["PT","Portugal"],["BR","Brésil"],["ES","Espagne"],["DE","Allemagne"],["GB","Angleterre"],["IT","Italie"],["CM","Cameroun"],["SN","Sénégal"],["NG","Nigéria"],["DK","Danemark"],["NL","Pays-Bas"],["BE","Belgique"],["CI","Côte d'Ivoire"],["AL","Albanie"],["HR","Croatie"],["RS","Serbie"],["TR","Turquie"],["MW","Malawi"]];let J={...R},le=[];async function cn(e,{toast:t}){const{data:n}=await _.from("clubs").select("id,encoded_name,logo_url,kit_style,kit_color1,kit_color2,kit_shorts,kit_socks").order("encoded_name");le=n||[],J={...R},e.innerHTML=gn(le),un(e,le,t),F()}function Pe(){var n;const e=((n=document.getElementById("cb-surname-real"))==null?void 0:n.value)||"",t=document.getElementById("cb-surname-enc");t&&(t.value=ue(e))}function pn(){var n;const e=(n=document.getElementById("cb-club"))==null?void 0:n.value,t=le.find(a=>String(a.id)===String(e));return t?{style:t.kit_style||z.style,color1:t.kit_color1||z.color1,color2:t.kit_color2||z.color2,shorts:t.kit_shorts||z.shorts,socks:t.kit_socks||z.socks}:{...z}}function F(){var s,u,d,f;const e=m=>{var b;return((b=document.getElementById(m))==null?void 0:b.value)||""},t={firstname:e("cb-firstname"),surname_real:e("cb-surname-enc")||ue(e("cb-surname-real")),country_code:e("cb-country"),club_encoded_name:((u=(s=document.getElementById("cb-club"))==null?void 0:s.selectedOptions[0])==null?void 0:u.text)||"",job:e("cb-job"),job2:e("cb-job2")||null,note_g:parseInt(e("cb-note-g"))||0,note_d:parseInt(e("cb-note-d"))||0,note_m:parseInt(e("cb-note-m"))||0,note_a:parseInt(e("cb-note-a"))||0,rarity:e("cb-rarity")},n=pn(),a=document.getElementById("cb-club"),c=((f=(d=a==null?void 0:a.selectedOptions[0])==null?void 0:d.dataset)==null?void 0:f.logo)||null,r=Qe(J,n,120),g=`data:image/svg+xml;charset=utf-8,${encodeURIComponent(r)}`,x=Lt(t,{portraitUrl:g,clubLogoUrl:c,showNotes:!0}),E=document.getElementById("card-preview");E&&(E.innerHTML=x);const i=document.getElementById("avatar-preview-wrap");i&&(i.innerHTML=r);const p=e("cb-surname-real"),o=document.getElementById("encode-summary");o&&p&&(o.textContent=`${e("cb-firstname")} ${p} → ${e("cb-firstname")} ${t.surname_real}`)}function ut(){const e=t=>{var n;return((n=document.getElementById(t))==null?void 0:n.value)||""};J={skin:e("av-skin"),hair:e("av-hair"),hairColor:e("av-hairColor"),eyes:e("av-eyes"),nose:e("av-nose"),mouth:e("av-mouth"),eyebrows:e("av-eyebrows"),beard:e("av-beard")}}function un(e,t,n){var c,r,g,x,E;e.querySelectorAll("input,select").forEach(i=>{i.addEventListener("input",F),i.addEventListener("change",F)});const a=["av-skin","av-hair","av-hairColor","av-eyes","av-nose","av-mouth","av-eyebrows","av-beard"];a.forEach(i=>{var p;(p=document.getElementById(i))==null||p.addEventListener("change",()=>{ut(),F()})}),(c=document.getElementById("cb-surname-real"))==null||c.addEventListener("input",()=>{Pe(),F()}),(r=document.getElementById("btn-encode"))==null||r.addEventListener("click",()=>{Pe(),F()}),(g=document.getElementById("cb-club"))==null||g.addEventListener("change",F),(x=document.getElementById("btn-save-player"))==null||x.addEventListener("click",()=>mn(n)),(E=document.getElementById("btn-reset"))==null||E.addEventListener("click",()=>{e.querySelectorAll("input").forEach(i=>i.value=""),e.querySelectorAll("select").forEach(i=>i.selectedIndex=0),J={...R},a.forEach(i=>{const p=i.replace("av-",""),o=document.getElementById(i);o&&(o.value=R[p]||"")}),F()})}async function mn(e){const t=g=>{var x;return((x=document.getElementById(g))==null?void 0:x.value)||""},n=t("cb-surname-real").trim(),a=t("cb-surname-enc").trim()||ue(n);if(!t("cb-firstname")||!n||!t("cb-country")||!t("cb-job")){e("Remplissez les champs obligatoires (prénom, nom, pays, poste)","error");return}ut();const c={firstname:t("cb-firstname").trim(),surname_real:n,surname_real:a,country_code:t("cb-country"),club_id:t("cb-club")||null,job:t("cb-job"),job2:t("cb-job2")||null,note_g:parseInt(t("cb-note-g"))||0,note_d:parseInt(t("cb-note-d"))||0,note_m:parseInt(t("cb-note-m"))||0,note_a:parseInt(t("cb-note-a"))||0,rarity:t("cb-rarity")||"normal",note_min:parseInt(t("cb-note-min"))||null,note_max:parseInt(t("cb-note-max"))||null,sell_price:parseInt(t("cb-price"))||0,avatar_config:J,skin:J.skin||"blanc",hair:J.hairColor||"marron",hair_length:"court"},{error:r}=await _.from("players").insert(c);if(r){e("Erreur: "+r.message,"error");return}e(`✅ Joueur "${c.firstname} ${c.surname_real}" enregistré !`,"success")}function gn(e){const t=dn.map(([i,p])=>`<option value="${i}">${p} (${i})</option>`).join(""),n='<option value="">— Sélectionner un club —</option>'+e.map(i=>`<option value="${i.id}" data-logo="${i.logo_url||""}">${i.encoded_name}</option>`).join(""),a=De.map(i=>`<option value="${i}">${i}</option>`).join(""),c='<option value="">Aucun</option>'+De.map(i=>`<option value="${i}">${i}</option>`).join(""),r=sn.map(i=>`<option value="${i}">${i.charAt(0).toUpperCase()+i.slice(1)}</option>`).join(""),g=(i,p,o)=>Object.entries(p).map(([s,u])=>`<option value="${s}" ${s===(o||"")?"selected":""}>${u.label}</option>`).join(""),x=Object.keys(It).map(i=>`<option value="${i}" ${i===R.skin?"selected":""}>${i.charAt(0).toUpperCase()+i.slice(1)}</option>`).join(""),E=Object.keys($t).map(i=>`<option value="${i}" ${i===R.hairColor?"selected":""}>${i.charAt(0).toUpperCase()+i.slice(1)}</option>`).join("");return`
      ...
      <div class="card-panel">
        ...
        <div style="display:grid;grid-template-columns:1fr 120px;gap:16px;align-items:start">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
            ...
            <div>
              <label>Bouche</label>
              <select id="av-mouth">${g("av-mouth",Tt,R.mouth)}</select>
            </div>
            <div>
              <label>Barbe</label>
  ...
  `}async function mt(e,{toast:t}){const[{data:n,error:a},{data:c}]=await Promise.all([_.from("users").select("id,pseudo,club_name,credits,level,wins,draws,losses,trophies_top1,trophies_top2,trophies_top3,is_admin,created_at,mmr,mmr_deviation,rank_tier,placement_matches,ranked_wins,ranked_losses,ranked_draws").order("created_at",{ascending:!1}),_.from("user_solo_progress").select("user_id, unlocked_level")]);if(a){e.innerHTML=`<p style="color:red">${a.message}</p>`;return}const r={};(c||[]).forEach(E=>{r[E.user_id]=E.unlocked_level});const g=n||[];e.innerHTML=`
  ...
  `,x(g),document.getElementById("search-users").addEventListener("input",E=>{const i=E.target.value.toLowerCase();x(g.filter(p=>{var o;return p.pseudo.toLowerCase().includes(i)||((o=p.club_name)==null?void 0:o.toLowerCase().includes(i))}))});function x(E){document.getElementById("users-tbody").innerHTML=E.map(i=>{const p=i.mmr??1e3,o=i.mmr_deviation??350,s=i.placement_matches??0,u=zt(p),d=i.ranked_wins??0,f=i.ranked_losses??0,m=i.ranked_draws??0,b=d+f+m,k=b>0?Math.round(d/b*100):null,L=s<10,v=o<80?"#1A6B3C":o<150?"#e67e22":"#bb2020";return`
        <tr>
          <td>
          ...
          <td style="font-size:12px">
            ...
            ${L?`<div style="font-size:10px;color:#e67e22">${s}/10 placement${s<10?` (${10-s} restants)`:""}</div>`:`<div style="font-size:10px;color:var(--gray-600)">${d}V ${m}N ${f}D${k!==null?` · ${k}% WR`:""}</div>`}
          </td>
          <td style="font-size:12px;font-family:monospace">
            <div style="font-weight:700;font-size:14px">${p}</div>
            <div style="font-size:10px;color:${v}">RD ±${Math.round(o)}</div>
          </td>
          <td>
        ...
        </tr>`}).join(""),document.querySelectorAll(".btn-edit-credits").forEach(i=>{i.addEventListener("click",async()=>{const p=i.dataset.user,o=Number(i.dataset.current)||0,s=prompt("Nouveau solde de crédits :",o);if(s===null)return;const u=parseInt(s.replace(/\s/g,""),10);if(isNaN(u)||u<0){t("Valeur invalide","error");return}const{data:d,error:f}=await _.rpc("admin_update_user_credits",{p_user_id:p,p_new_credits:u});if(f){t(f.message,"error");return}if(!(d!=null&&d.success)){t((d==null?void 0:d.error)||"Échec de la mise à jour","error");return}t("Crédits mis à jour ✅","success");const m=document.getElementById(`credits-display-${p}`);m&&(m.textContent=`${u.toLocaleString("fr")} cr.`),i.dataset.current=u})}),document.querySelectorAll("[data-toggle-admin]").forEach(i=>{i.addEventListener("click",async()=>{const p=i.dataset.isAdmin!=="true";if(!confirm(`Voulez-vous ${p?"rendre admin":"retirer le rôle admin"} de ce Manager ?`))return;const{error:o}=await _.from("users").update({is_admin:p}).eq("id",i.dataset.toggleAdmin);o?t(o.message,"error"):(t("Rôle mis à jour ✅","success"),mt(e,{toast:t}))})})}}const fn=[{value:"normal",label:"Normal",color:"#ccc"},{value:"pepite",label:"Pépite",color:"#D4A017"},{value:"papyte",label:"Papyte",color:"#909090"},{value:"legende",label:"Légende",color:"#7a28b8"}];async function gt(e,t){var E;const{toast:n}=t,{data:a,error:c}=await _.from("market_listings").select(`
    ...
    `).order("listed_at",{ascending:!1}).limit(100);if(c){e.innerHTML=`<p style="color:var(--danger)">${c.message}</p>`;return}const r=a||[],g=r.filter(i=>i.status==="active").length,x=r.filter(i=>i.status==="sold").length;e.innerHTML=`
    ...
    <div class="card-panel">
      <div class="table-wrap">
        <table>
          ...
          <tbody>
            ${r.map(i=>{var u,d,f,m;const p=(u=i.card)==null?void 0:u.player,o=p?`${p.firstname} ${p.surname_real}`:((d=i.card)==null?void 0:d.card_type)||"—",s={active:"#1A6B3C",sold:"#D4A017",cancelled:"#888"};return`<tr>
                <td><b>${o}</b>${p?`<div style="font-size:10px;color:var(--tile-fg-dim)">${p.rarity} · ${p.job}</div>`:""}</td>
                ...
                <td>
                  <span style="background:${s[i.status]};color:#fff;padding:2px 6px;border-radius:3px;font-size:10px;font-weight:700">
  ...
  `,document.querySelectorAll("[data-cancel]").forEach(i=>{i.addEventListener("click",async()=>{if(!confirm("Annuler cette annonce ?"))return;const{error:p}=await _.from("market_listings").update({status:"cancelled"}).eq("id",i.dataset.cancel);p?n(p.message,"error"):(n("Annonce annulée","success"),gt(e,t))})}),(E=document.getElementById("btn-price-grid"))==null||E.addEventListener("click",()=>yn(t))}async function yn(e){await ve(e)}function bn(){const e=document.getElementById("modal-overlay");if(!e||e._pgBlocker)return;const t=n=>{n.target===n.currentTarget&&n.stopImmediatePropagation()};e.addEventListener("click",t,!0),e._pgBlocker=t}function fe(){const e=document.getElementById("modal-overlay");e!=null&&e._pgBlocker&&(e.removeEventListener("click",e._pgBlocker,!0),delete e._pgBlocker)}async function ve(e){var u,d;const{openModal:t,closeModal:n,toast:a}=e,{data:c,error:r}=await _.from("sell_price_configs").select("*").order("rarity").order("note_min");if(r){a(r.message,"error");return}const g=f=>fn.map(m=>`<option value="${m.value}" ${m.value===f?"selected":""}>${m.label}</option>`).join(""),x=f=>`
    <tr data-row="${f.id}">
  ...
  `),bn(),(u=document.getElementById("pg-close"))==null||u.addEventListener("click",()=>{fe(),n()});function p(f){const m=f.dataset.id,b=f.dataset.field,k=b==="rarity"?f.value:Number(f.value)||0;return o(m,{[b]:k},f)}async function o(f,m,b){const k=document.querySelector(`tr[data-row="${f}"]`),L=I=>k.querySelector(`[data-field="${I}"]`),v="note_min"in m?m.note_min:Number(L("note_min").value),w="note_max"in m?m.note_max:Number(L("note_max").value),y="price_min"in m?m.price_min:Number(L("price_min").value),h="price_max"in m?m.price_max:Number(L("price_max").value);if(v>w){a("Note min doit être ≤ note max","error");return}if(y>h){a("Prix min doit être ≤ prix max","error");return}const{error:l}=await _.from("sell_price_configs").update(m).eq("id",f);if(l){a(l.message,"error");return}b&&(b.style.transition="background .3s",b.style.background="#e8f8ee",setTimeout(()=>{b.style.background=""},500))}function s(){document.querySelectorAll("#pg-tbody [data-field]").forEach(f=>{f.addEventListener("change",()=>p(f))})}s(),document.querySelectorAll("[data-del-row]").forEach(f=>{f.addEventListener("click",async()=>{confirm("Supprimer cette ligne ?")&&(await _.from("sell_price_configs").delete().eq("id",f.dataset.delRow),a("Ligne supprimée","success"),fe(),ve(e))})}),(d=document.getElementById("pg-add-row"))==null||d.addEventListener("click",async()=>{var L;const f={rarity:"normal",note_min:0,note_max:20,price_min:2500,price_max:15e3},{data:m,error:b}=await _.from("sell_price_configs").insert(f).select().single();if(b){a(b.message,"error");return}const k=document.getElementById("pg-tbody");k.querySelector("td[colspan]")&&(k.innerHTML=""),k.insertAdjacentHTML("beforeend",x(m)),s(),(L=document.querySelector(`[data-del-row="${m.id}"]`))==null||L.addEventListener("click",async function(){confirm("Supprimer cette ligne ?")&&(await _.from("sell_price_configs").delete().eq("id",m.id),a("Ligne supprimée","success"),fe(),ve(e))}),a("Ligne ajoutée — modifie les valeurs directement","success")})}async function vn(e,{toast:t}){e.innerHTML=`
...
`;n.forEach(r=>{c+=[r.real_name,r.encoded_name,r.country_code,r.logo_url||""].map(Ne).join(",")+`
...
`;n.forEach(r=>{var g;c+=[r.firstname,r.surname_real,r.surname_real,r.country_code,((g=r.clubs)==null?void 0:g.encoded_name)||"",r.job,r.job2||"",r.note_g,r.note_d,r.note_m,r.note_a,r.rarity,r.note_min??"",r.note_max??"",r.skin,r.hair,r.hair_length,r.sell_price].map(Ne).join(",")+`
`}),ae("players_export.csv",c),t(`${n.length} joueurs exportés`,"success")}),document.getElementById("import-clubs").addEventListener("change",async n=>{const a=n.target.files[0];if(!a)return;const c=document.getElementById("clubs-import-result");c.innerHTML='<span style="color:var(--gray-600)">Import en cours...</span>';try{const r=await a.text(),g=Fe(r);if(g.length===0){c.innerHTML='<span style="color:#c0392b">Fichier vide</span>';return}let x=0,E=0;const i=[];for(const p of g){if(!p.real_name||!p.encoded_name||!p.country_code){E++,i.push(`Ligne ignorée (champs manquants): ${p.real_name||"?"}`);continue}const o={real_name:p.real_name,encoded_name:p.encoded_name.toUpperCase(),country_code:p.country_code.toUpperCase().slice(0,2),logo_url:p.logo_url||null},{error:s}=await _.from("clubs").upsert(o,{onConflict:"encoded_name"});s?(E++,i.push(`${p.encoded_name}: ${s.message}`)):x++}c.innerHTML=`<div style="color:var(--green)">✅ ${x} clubs importés</div>
        ${E>0?`<div style="color:#c0392b">❌ ${E} erreurs</div><div style="font-size:11px;color:var(--gray-600);max-height:100px;overflow-y:auto">${i.slice(0,10).join("<br>")}</div>`:""}`,t(`${x} clubs importés`,"success")}catch(r){c.innerHTML=`<span style="color:#c0392b">Erreur : ${r.message}</span>`}n.target.value=""}),document.getElementById("import-players").addEventListener("change",async n=>{const a=n.target.files[0];if(!a)return;const c=document.getElementById("players-import-result");c.innerHTML='<span style="color:var(--gray-600)">Import en cours...</span>';try{const r=await a.text(),g=Fe(r);if(g.length===0){c.innerHTML='<span style="color:#c0392b">Fichier vide</span>';return}const{data:x}=await _.from("clubs").select("id,encoded_name"),E={};(x||[]).forEach(b=>{E[b.encoded_name.toUpperCase()]=b.id});let i=0,p=0;const o=[],s=["GK","DEF","MIL","ATT"],u=["normal","pepite","papyte","legende"],d=["blanc","metisse","typ","noir"],f=["blond","marron","noir","chauve"],m=["rase","court","milong","long"];for(const b of g){if(!b.firstname||!b.surname_real||!b.country_code||!b.job){p++,o.push(`Ligne ignorée (champs requis manquants): ${b.firstname||"?"}`);continue}if(!s.includes(b.job)){p++,o.push(`${b.firstname}: job invalide "${b.job}"`);continue}const k=b.club_encoded_name&&E[b.club_encoded_name.toUpperCase()]||null,L={firstname:b.firstname,surname_real:b.surname_real,surname_real:b.surname_real||ue(b.surname_real),country_code:b.country_code.toUpperCase().slice(0,2),club_id:k,job:b.job,job2:s.includes(b.job2)?b.job2:null,note_g:parseInt(b.note_g)||0,note_d:parseInt(b.note_d)||0,note_m:parseInt(b.note_m)||0,note_a:parseInt(b.note_a)||0,rarity:u.includes(b.rarity)?b.rarity:"normal",note_min:b.note_min!==""&&b.note_min!=null?parseInt(b.note_min):null,note_max:b.note_max!==""&&b.note_max!=null?parseInt(b.note_max):null,skin:d.includes(b.skin)?b.skin:"blanc",hair:f.includes(b.hair)?b.hair:"noir",hair_length:m.includes(b.hair_length)?b.hair_length:"court",sell_price:parseInt(b.sell_price)||0},{error:v}=await _.from("players").insert(L);v?(p++,o.push(`${b.firstname} ${b.surname_real}: ${v.message}`)):i++}c.innerHTML=`<div style="color:var(--green)">✅ ${i} joueurs importés</div>
        ${p>0?`<div style="color:#c0392b">❌ ${p} erreurs</div><div style="font-size:11px;color:var(--gray-600);max-height:100px;overflow-y:auto">${o.slice(0,10).join("<br>")}</div>`:""}`,t(`${i} joueurs importés`,"success")}catch(r){c.innerHTML=`<span style="color:#c0392b">Erreur : ${r.message}</span>`}n.target.value=""})}function Ne(e){const t=String(e??"");return t.includes(",")||t.includes('"')||t.includes(`
`)?'"'+t.replace(/"/g,'""')+'"':t}function ae(e,t){const n=new Blob(["\uFEFF"+t],{type:"text/csv;charset=utf-8;"}),a=URL.createObjectURL(n),c=document.createElement("a");c.href=a,c.download=e,c.click(),URL.revokeObjectURL(a)}function Fe(e){e=e.replace(/^\uFEFF/,"").replace(/\r\n/g,`
`).replace(/\r/g,`
`);const t=e.split(`
`).filter(g=>g.trim());if(t.length<2)return[];const n=t[0],a=n.split(";").length>n.split(",").length?";":",",c=Oe(n,a).map(g=>g.trim().replace(/^\uFEFF/,"").toLowerCase()),r=[];for(let g=1;g<t.length;g++){if(!t[g].trim())continue;const x=Oe(t[g],a),E={};c.forEach((i,p)=>{E[i]=(x[p]||"").trim()}),!Object.values(E).every(i=>!i)&&r.push(E)}return r}function Oe(e,t=","){const n=[];let a="",c=!1;for(let r=0;r<e.length;r++){const g=e[r];c?g==='"'?e[r+1]==='"'?(a+='"',r++):c=!1:a+=g:g==='"'?c=!0:g===t?(n.push(a),a=""):a+=g}return n.push(a),n}const xn={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};async function hn(e,{toast:t}){const n=Object.keys(ge);let a=n[0];const{data:c}=await _.from("formation_links_overrides").select("formation, links"),r={};(c||[]).forEach(p=>{r[p.formation]=p.links});let g=new Set;function x(p,o){return[p,o].sort().join("-")}function E(p){const o=r[p]||ge[p]||[];g=new Set(o.map(([s,u])=>x(s,u)))}E(a),e.innerHTML=`
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;gap:12px;flex-wrap:wrap">
      <div style="display:flex;align-items:center;gap:10px">
        ...
        <select id="formation-select" style="padding:6px 10px;font-size:14px;border-radius:8px;border:1.5px solid #ccc">
          ${n.map(p=>`<option value="${p}" ${p===a?"selected":""}>${p}</option>`).join("")}
        </select>
  ...
  `;function i(){const p=Rt[a]||{},o=ze(a),s=320,u=400,d=22;function f(v){const w=p[v];return w?{x:w.x*s,y:w.y*u}:null}let m=`<svg width="${s}" height="${u}" viewBox="0 0 ${s} ${u}" xmlns="http://www.w3.org/2000/svg" style="display:block">`;o.forEach(([v,w],y)=>{const h=f(v),l=f(w);if(!h||!l)return;const I=x(v,w),$=g.has(I),A=$?"#3b82f6":"#999",B=$?.95:.35,S=$?4:3;m+=`<line x1="${h.x}" y1="${h.y}" x2="${l.x}" y2="${l.y}"
        stroke="transparent" stroke-width="14" class="link-hit" data-key="${I}" style="cursor:pointer"/>`,m+=`<line x1="${h.x}" y1="${h.y}" x2="${l.x}" y2="${l.y}"
        stroke="${A}" stroke-width="${S}" stroke-dasharray="${$?"none":"4,3"}"
        opacity="${B}" pointer-events="none" data-line-key="${I}"/>`});for(const v of Object.keys(p)){const w=f(v);if(!w)continue;const y=v.replace(/\d+/,""),h=xn[y]||"#555";m+=`<circle cx="${w.x}" cy="${w.y}" r="${d}" fill="${h}" stroke="rgba(255,255,255,0.7)" stroke-width="2" pointer-events="none"/>`,m+=`<text x="${w.x}" y="${w.y+4}" text-anchor="middle" font-size="10" font-weight="900" fill="white" font-family="Arial Black,Arial" pointer-events="none">${v}</text>`}m+="</svg>",document.getElementById("field-wrap").innerHTML=m;const b=document.getElementById("links-list");b.innerHTML=o.map(([v,w])=>{const y=x(v,w),h=g.has(y);return`
        ...
        </button>`}).join("");const k=e.querySelector('div[style*="font-weight:700;font-size:13px;margin-bottom:8px"]');k&&(k.textContent=`Liens (${g.size}/${o.length} actifs)`);function L(v){g.has(v)?g.delete(v):g.add(v),i()}e.querySelectorAll(".link-hit").forEach(v=>{v.addEventListener("click",()=>L(v.dataset.key))}),e.querySelectorAll(".link-toggle").forEach(v=>{v.addEventListener("click",()=>L(v.dataset.key))})}i(),document.getElementById("formation-select").addEventListener("change",p=>{a=p.target.value,E(a),i()}),document.getElementById("reset-btn").addEventListener("click",()=>{const p=ge[a]||[];g=new Set(p.map(([o,s])=>x(o,s))),i(),t("Liens réinitialisés aux valeurs par défaut (non sauvegardé)","info")}),document.getElementById("save-btn").addEventListener("click",async()=>{const o=ze(a).filter(([u,d])=>g.has(x(u,d))),{error:s}=await _.from("formation_links_overrides").upsert({formation:a,links:o,updated_at:new Date().toISOString()});if(s){t(s.message,"error");return}r[a]=o,t(`Liens enregistrés pour ${a} (${o.length} actifs)`,"success")})}const En=[{value:"player",label:"Joueur"},{value:"formation",label:"Formation"},{value:"game_changer",label:"Game Changer"},{value:"ultra_game_changer",label:"Ultra Game Changer"},{value:"game_helper",label:"Game Helper"},{value:"stadium",label:"🏟️ Stade"}],_n=[{value:"",label:"Toutes"},{value:"normal",label:"Normal"},{value:"pepite",label:"Pépite"},{value:"papyte",label:"Papyte"},{value:"legende",label:"Légende"}],wn=[{value:"credits",label:"Crédits"},{value:"pub",label:"Publicité"},{value:"free",label:"Gratuit"}];async function In(e){e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">Chargement...</div>';const{data:t}=await _.from("booster_configs").select("*").order("sort_order");let n=null,a=[],c=null;const r=()=>window.innerWidth<700;async function g(){if(!n){a=[];return}if(n!==c){const{data:o}=await _.from("booster_drop_rates").select("*").eq("booster_id",n).order("sort_order");a=o||[],c=n}}function x(){return`
    ...
    </div>`}function E(){const o=n?(t||[]).find(d=>d.id===n):null;if(!o)return`<div style="display:flex;align-items:center;justify-content:center;flex:1;color:#bbb;font-size:14px;padding:40px;text-align:center">
      ← Sélectionnez un booster ou créez-en un nouveau
    </div>`;const s=a.reduce((d,f)=>d+Number(f.percentage||0),0),u=Math.abs(s-100)<.1;return`
    <div id="booster-editor" style="flex:1;overflow-y:auto;background:#f8f8f8;padding:14px">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
          <div>
            ...
            <select id="f-price-type" style="width:100%;padding:9px;border:1px solid #ddd;border-radius:6px;font-size:13px">
              ${wn.map(d=>`<option value="${d.value}" ${o.price_type===d.value?"selected":""}>${d.label}</option>`).join("")}
            </select>
          </div>
      ...
      <div style="background:#fff;border-radius:10px;padding:14px;margin-bottom:14px">
        ...
        ${a.length===0?'<div style="text-align:center;padding:16px;color:#aaa;font-size:13px">Aucun taux. Cliquez + Ligne.</div>':a.map((d,f)=>`
          <div style="display:grid;grid-template-columns:2fr 1.5fr 50px 50px 60px 32px;gap:4px;align-items:center;margin-bottom:6px;padding:6px;background:#f9f9f9;border-radius:6px" data-rate-idx="${f}">
            <select class="rate-type" data-idx="${f}" style="padding:6px 4px;border:1px solid #ddd;border-radius:4px;font-size:12px;width:100%">
              ${En.map(m=>`<option value="${m.value}" ${d.card_type===m.value?"selected":""}>${m.label}</option>`).join("")}
            </select>
            <select class="rate-rarity" data-idx="${f}" style="padding:6px 4px;border:1px solid #ddd;border-radius:4px;font-size:12px;width:100%">
              ${_n.map(m=>`<option value="${m.value}" ${(d.rarity||"")===m.value?"selected":""}>${m.label}</option>`).join("")}
            </select>
    ...
    </div>`}async function i(o=!1){o||await g();const s=!n&&r(),u=n&&r();e.innerHTML=`
    <div style="display:flex;height:calc(100vh - 56px);overflow:hidden">
      ${!r()||s||!n?x():""}
      ${!r()||u?E():""}
    </div>`,p()}function p(){var u,d,f,m,b,k,L;const o=v=>e.querySelector(v);e.querySelectorAll(".booster-row").forEach(v=>{v.addEventListener("click",w=>{w.target.closest(".btn-delete")||(n=v.dataset.id,c=null,i())})}),(u=o("#btn-back"))==null||u.addEventListener("click",()=>{n=null,i()}),(d=o("#btn-new"))==null||d.addEventListener("click",async()=>{const v=prompt("Nom du nouveau booster :");if(!(v!=null&&v.trim()))return;const{data:w,error:y}=await _.from("booster_configs").insert({name:v.trim(),price_type:"credits",price_credits:1e3,card_count:5}).select().single();if(y){alert(y.message);return}t.push(w),n=w.id,c=null,i()}),e.querySelectorAll(".btn-delete").forEach(v=>{v.addEventListener("click",async w=>{if(w.stopPropagation(),!confirm("Supprimer ce booster ?"))return;await _.from("booster_configs").delete().eq("id",v.dataset.id);const y=t.findIndex(h=>h.id===v.dataset.id);y>-1&&t.splice(y,1),n===v.dataset.id&&(n=null,c=null),i()})}),(f=o("#btn-cancel"))==null||f.addEventListener("click",()=>{n=null,i()}),(m=o("#f-price-type"))==null||m.addEventListener("change",v=>{const w=o("#credits-field");w&&(w.style.opacity=v.target.value!=="credits"?"0.4":"1")}),(b=o("#btn-pick-icon"))==null||b.addEventListener("click",async()=>{var w;const v=o("#icon-picker-grid");if(v){if(v.style.display!=="none"){v.style.display="none";return}v.innerHTML='<div style="padding:10px;text-align:center;color:#888;font-size:12px">⏳ Chargement...</div>',v.style.display="block";try{const h=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),l=Array.isArray(h)?h.filter($=>$.name.startsWith("booster")&&/\.(png|jpg|svg|webp)$/i.test($.name)):[];if(!l.length){v.innerHTML='<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Aucune icône "booster*" trouvée.</div>';return}const I=((w=o("#f-image-url"))==null?void 0:w.value)||"";v.innerHTML=`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(72px,1fr));gap:8px">
          ${l.map($=>`
          <div class="icon-pick-item" data-name="${$.name}"
            style="cursor:pointer;text-align:center;padding:6px;border-radius:6px;border:2px solid ${$.name===I?"#1A6B3C":"#ddd"};background:${$.name===I?"#f0f7f0":"#fff"}">
        ...
        </div>`,v.querySelectorAll(".icon-pick-item").forEach($=>{$.addEventListener("click",()=>{const A=o("#f-image-url");A&&(A.value=$.dataset.name),v.style.display="none"})})}catch(y){v.innerHTML=`<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Erreur API GitHub : ${y.message}</div>`}}});function s(){e.querySelectorAll("[data-rate-idx]").forEach(v=>{var y,h,l,I,$;const w=Number(v.dataset.rateIdx);w>=0&&w<a.length&&(a[w].card_type=((y=v.querySelector(".rate-type"))==null?void 0:y.value)||"player",a[w].rarity=((h=v.querySelector(".rate-rarity"))==null?void 0:h.value)||null,a[w].note_min=Number((l=v.querySelector(".rate-note-min"))==null?void 0:l.value)||null,a[w].note_max=Number((I=v.querySelector(".rate-note-max"))==null?void 0:I.value)||null,a[w].percentage=Number(($=v.querySelector(".rate-pct"))==null?void 0:$.value)||0)})}(k=o("#btn-add-rate"))==null||k.addEventListener("click",()=>{s(),a.push({id:null,booster_id:n,card_type:"player",rarity:"normal",note_min:null,note_max:null,percentage:10,sort_order:a.length}),i(!0)}),e.querySelectorAll(".btn-del-rate").forEach(v=>{v.addEventListener("click",()=>{s(),a.splice(Number(v.dataset.idx),1),i(!0)})}),(L=o("#btn-save"))==null||L.addEventListener("click",async()=>{var I,$,A,B,S,C,T,O,U,G,N,D,ie;const v=(t||[]).find(M=>M.id===n);if(!v)return;const w=[];e.querySelectorAll("[data-rate-idx]").forEach(M=>{var Z,Q,X,ee,te;const Y=Number(M.dataset.rateIdx);w.push({booster_id:n,card_type:((Z=M.querySelector(".rate-type"))==null?void 0:Z.value)||"player",rarity:((Q=M.querySelector(".rate-rarity"))==null?void 0:Q.value)||null,note_min:Number((X=M.querySelector(".rate-note-min"))==null?void 0:X.value)||null,note_max:Number((ee=M.querySelector(".rate-note-max"))==null?void 0:ee.value)||null,percentage:Number((te=M.querySelector(".rate-pct"))==null?void 0:te.value)||0,sort_order:Y});const P=w[w.length-1];P.rarity||(P.rarity=null),P.note_min||(P.note_min=null),P.note_max||(P.note_max=null)});const y=w.reduce((M,Y)=>M+Y.percentage,0);if(w.length&&Math.abs(y-100)>.5){alert(`La somme doit faire 100% (actuellement ${y.toFixed(1)}%)`);return}const h={name:(($=(I=o("#f-name"))==null?void 0:I.value)==null?void 0:$.trim())||v.name,image_url:((B=(A=o("#f-image-url"))==null?void 0:A.value)==null?void 0:B.trim())||null,price_type:(S=o("#f-price-type"))==null?void 0:S.value,price_credits:Number((C=o("#f-price-credits"))==null?void 0:C.value)||0,card_count:Number((T=o("#f-card-count"))==null?void 0:T.value)||5,is_active:((O=o("#f-active"))==null?void 0:O.checked)??v.is_active,allow_duplicates:((U=o("#f-allow-dup"))==null?void 0:U.checked)??!0,sort_order:Number((G=o("#f-sort"))==null?void 0:G.value)||0,max_per_user:(N=o("#f-max-per-user"))!=null&&N.value?Number(o("#f-max-per-user").value):null,available_from:((D=o("#f-available-from"))==null?void 0:D.value)||null,available_until:((ie=o("#f-available-until"))==null?void 0:ie.value)||null},{error:l}=await _.from("booster_configs").update(h).eq("id",n);if(l){alert(l.message);return}if(Object.assign(v,h),await _.from("booster_drop_rates").delete().eq("booster_id",n),w.length){const{error:M}=await _.from("booster_drop_rates").insert(w);if(M){alert(M.message);return}}a=w,c=n,alert("✅ Booster enregistré !"),i(!0)})}i()}const ft=[{value:"normal",label:"Normal",color:"#888"},{value:"pepite",label:"Pépite",color:"#D4A017"},{value:"papyte",label:"Papyte",color:"#909090"},{value:"legende",label:"Légende",color:"#7a28b8"}];async function $n(e){await ke(e)}async function ke(e){const{data:t,error:n}=await _.from("sell_price_configs").select("*").order("rarity").order("note_min");if(n){e.innerHTML=`<div style="padding:24px;color:red">Erreur : ${n.message}</div>`;return}e.innerHTML=`
    ...
    </div>`,document.getElementById("add-config").addEventListener("click",()=>Ue(null)),document.getElementById("form-cancel").addEventListener("click",()=>{document.getElementById("config-form").style.display="none"}),document.getElementById("form-save").addEventListener("click",()=>Ln(e)),e.querySelectorAll("[data-edit]").forEach(a=>{const c=(t||[]).find(r=>r.id===a.dataset.edit);c&&a.addEventListener("click",()=>Ue(c))}),e.querySelectorAll("[data-delete]").forEach(a=>{a.addEventListener("click",async()=>{confirm("Supprimer cette règle ?")&&(await _.from("sell_price_configs").delete().eq("id",a.dataset.delete),await ke(e))})})}function kn(e){const t=ft.find(n=>n.value===e.rarity);return`
    ...
    </tr>`}function Ue(e){document.getElementById("config-form").style.display="block",document.getElementById("form-title").textContent=e?"Modifier la règle":"Nouvelle règle",document.getElementById("form-id").value=(e==null?void 0:e.id)||"",document.getElementById("form-rarity").value=(e==null?void 0:e.rarity)||"normal",document.getElementById("form-price").value=(e==null?void 0:e.price)||1e3,document.getElementById("form-note-min").value=(e==null?void 0:e.note_min)??1,document.getElementById("form-note-max").value=(e==null?void 0:e.note_max)??10,document.getElementById("config-form").scrollIntoView({behavior:"smooth"})}async function Ln(e){const t=document.getElementById("form-id").value,n=document.getElementById("form-rarity").value,a=parseInt(document.getElementById("form-price").value)||0,c=parseInt(document.getElementById("form-note-min").value)||1,r=parseInt(document.getElementById("form-note-max").value)||10;if(c>r){alert("Note min doit être ≤ note max");return}const g={rarity:n,price:a,note_min:c,note_max:r,updated_at:new Date().toISOString()};let x;if(t?{error:x}=await _.from("sell_price_configs").update(g).eq("id",t):{error:x}=await _.from("sell_price_configs").insert(g),x){alert("Erreur : "+x.message);return}document.getElementById("config-form").style.display="none",await ke(e)}async function An(e){await Le(e)}async function Le(e){const{data:t}=await _.from("patch_notes").select("*").order("published_at",{ascending:!1});e.innerHTML=`
    ...
    </div>`,document.getElementById("add-article").addEventListener("click",()=>Ge(null)),document.getElementById("form-cancel").addEventListener("click",()=>{document.getElementById("article-form").style.display="none"}),document.getElementById("form-save").addEventListener("click",()=>Cn(e)),e.querySelectorAll("[data-edit-row]").forEach(n=>{const a=(t||[]).find(c=>c.id===n.dataset.editRow);a&&n.addEventListener("click",()=>Ge(a))}),e.querySelectorAll("[data-delete]").forEach(n=>{n.addEventListener("click",async()=>{confirm("Supprimer cet article ?")&&(await _.from("patch_notes").delete().eq("id",n.dataset.delete),await Le(e))})})}function Bn(e){const t=new Date(e.published_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"short",year:"numeric"});return`
    ...
    </div>`}function Ge(e){const t=document.getElementById("article-form");t.style.display="block",document.getElementById("form-title").textContent=e?"Modifier l'article":"Nouvel article",document.getElementById("form-id").value=(e==null?void 0:e.id)||"",document.getElementById("form-art-title").value=(e==null?void 0:e.title)||"",document.getElementById("form-art-desc").value=(e==null?void 0:e.description)||"",document.getElementById("form-art-img-current").value=(e==null?void 0:e.image_url)||"",document.getElementById("form-art-img-file").value="";const n=document.getElementById("form-art-img-preview");n.innerHTML=e!=null&&e.image_url?`<img src="${e.image_url}" style="width:100%;height:100%;object-fit:cover">`:'<span style="font-size:20px;opacity:.4">📰</span>',document.getElementById("form-art-img-file").onchange=r=>{var E;const g=(E=r.target.files)==null?void 0:E[0];if(!g)return;const x=new FileReader;x.onload=()=>{n.innerHTML=`<img src="${x.result}" style="width:100%;height:100%;object-fit:cover">`},x.readAsDataURL(g)},document.getElementById("form-art-pub").checked=e?e.is_published:!0;const a=new Date((e==null?void 0:e.published_at)||Date.now()),c=new Date(a.getTime()-a.getTimezoneOffset()*6e4).toISOString().slice(0,16);document.getElementById("form-art-date").value=c,t.scrollIntoView({behavior:"smooth"})}async function Sn(e){const t=(e.name.split(".").pop()||"png").toLowerCase(),n=`journal/${Date.now()}.${t}`,{error:a}=await _.storage.from("assets").upload(n,e,{upsert:!0,cacheControl:"3600"});if(a)throw a;const{data:c}=_.storage.from("assets").getPublicUrl(n);return c.publicUrl}async function Cn(e){var u;const t=document.getElementById("form-id").value,n=document.getElementById("form-art-title").value.trim(),a=document.getElementById("form-art-desc").value.trim(),c=((u=document.getElementById("form-art-img-file").files)==null?void 0:u[0])||null,r=document.getElementById("form-art-img-current").value||null,g=document.getElementById("form-art-pub").checked,x=document.getElementById("form-art-date").value,E=document.getElementById("form-save");if(!n){alert("Le titre est obligatoire.");return}if(!a){alert("La description est obligatoire.");return}let i=r;if(c)try{E&&(E.textContent="📤 Envoi de l'image…"),i=await Sn(c)}catch(d){alert("Erreur upload image : "+d.message),E&&(E.textContent="💾 Enregistrer");return}const p=x?new Date(x).toISOString():new Date().toISOString(),o={title:n,description:a,image_url:i,is_published:g,published_at:p};let s;if(t?{error:s}=await _.from("patch_notes").update(o).eq("id",t):{error:s}=await _.from("patch_notes").insert(o),s){alert("Erreur : "+s.message);return}document.getElementById("article-form").style.display="none",await Le(e)}async function Tn(e){await W(e)}async function W(e){var p,o,s,u,d,f,m,b,k,L,v,w;const{data:t}=await _.from("tutorial_steps").select("*").order("step_order");e.innerHTML=`
    <div style="display:flex;gap:16px;height:calc(100vh - 56px);overflow:hidden;padding:16px">
      ...
      <div style="width:420px;flex-shrink:0;overflow-y:auto">
        ...
        <div style="margin-bottom:16px">
          ...
          <div id="ts-toolbar" style="display:flex;flex-wrap:wrap;gap:4px;padding:6px 8px;background:#f5f5f5;border:1.5px solid #ddd;border-bottom:none;border-radius:8px 8px 0 0">
            ${[["B","bold","<strong>Gras</strong>"],["I","italic","<em>Italique</em>"],["U","underline","Souligné"],["¶","insertParagraph","Paragraphe"],["•","insertUnorderedList","Liste à puces"],["1.","insertOrderedList","Liste numérotée"],["H2","formatBlock:h2","Titre H2"],["H3","formatBlock:h3","Titre H3"],["—","insertHorizontalRule","Séparateur"]].map(([y,h,l])=>`<button type="button" data-cmd="${h}" title="${l}" style="padding:4px 8px;border:1px solid #ddd;border-radius:4px;background:#fff;font-size:12px;font-weight:700;cursor:pointer;line-height:1;min-width:28px">${y}</button>`).join("")}
      ...
      <div style="flex:1;overflow-y:auto">
        ...
        <div id="ts-list" style="display:flex;flex-direction:column;gap:10px">
          ${(t||[]).length===0?`
            ...
            </div>`:(t||[]).map(y=>Mn(y)).join("")}
        </div>
      </div><!-- /colonne droite -->
    </div>`,(p=document.getElementById("ts-add"))==null||p.addEventListener("click",()=>He(null,(t==null?void 0:t.length)||0)),(o=document.getElementById("ts-reset"))==null||o.addEventListener("click",async()=>{if(!confirm("Remettre tutorial_done = false pour TOUS les joueurs ? Ils reverront le tutoriel à leur prochaine connexion."))return;const{error:y}=await _.from("users").update({tutorial_done:!1}).neq("id","00000000-0000-0000-0000-000000000000");alert(y?"Erreur : "+y.message:"✅ Tutorial_done réinitialisé pour tous les joueurs.")}),(s=document.getElementById("ts-cancel"))==null||s.addEventListener("click",yt),(u=document.getElementById("ts-save"))==null||u.addEventListener("click",()=>Rn(e)),(d=document.getElementById("ts-preview-btn"))==null||d.addEventListener("click",zn);const n=document.getElementById("ts-editor"),a=()=>{const y=document.getElementById("ts-content");y&&n&&(y.value=n.innerHTML)};n==null||n.addEventListener("input",a),n==null||n.addEventListener("blur",a);const c=()=>{document.activeElement!==n&&(n==null||n.focus())};(f=document.getElementById("ts-toolbar"))==null||f.querySelectorAll("[data-cmd]").forEach(y=>{y.addEventListener("mousedown",h=>{h.preventDefault(),c();const l=y.dataset.cmd;l.startsWith("formatBlock:")?document.execCommand("formatBlock",!1,l.split(":")[1]):document.execCommand(l,!1,null),a()})}),(m=document.getElementById("ts-insert-color"))==null||m.addEventListener("mousedown",y=>{y.preventDefault(),c();const h=prompt("Couleur hex (ex: #D4A017 pour or, #1A6B3C pour vert, #cc2222 pour rouge) :");h&&(document.execCommand("foreColor",!1,h),a())}),(b=document.getElementById("ts-clear-format"))==null||b.addEventListener("mousedown",y=>{y.preventDefault(),c(),document.execCommand("removeFormat",!1,null),a()}),n==null||n.addEventListener("focus",()=>{n&&(n.style.borderColor="#1A6B3C")}),n==null||n.addEventListener("blur",()=>{n&&(n.style.borderColor="#ddd")});const r=document.getElementById("ts-image"),g=document.getElementById("ts-img-preview"),x=document.getElementById("ts-img-preview-el"),E=document.getElementById("ts-img-picker-grid"),i=()=>{var h;const y=(h=r==null?void 0:r.value)==null?void 0:h.trim();if(y){const l="/";x.src=`${l}icons/${y}`,g.style.display="block"}else g.style.display="none"};r==null||r.addEventListener("input",i),(k=document.getElementById("ts-img-clear"))==null||k.addEventListener("click",()=>{r&&(r.value=""),g.style.display="none",E.style.display="none"}),(L=document.getElementById("ts-img-pick"))==null||L.addEventListener("click",async()=>{if(E.style.display!=="none"){E.style.display="none";return}E.innerHTML='<div style="color:#aaa;font-size:12px">Chargement…</div>',E.style.display="block";try{const h=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),l=Array.isArray(h)?h.filter($=>$.name.startsWith("tuto_")):[];if(!l.length){E.innerHTML='<div style="color:#aaa;font-size:12px">Aucun fichier tuto_* trouvé dans /icons/.<br>Uploade des images nommées tuto_xxx.png dans public/icons/</div>';return}const I="/";E.innerHTML='<div style="display:flex;flex-wrap:wrap;gap:8px">'+l.map($=>`
          <div data-pick="${$.name}" style="cursor:pointer;border:2px solid transparent;border-radius:8px;padding:4px;text-align:center;transition:border-color .15s" onmouseover="this.style.borderColor='#1A6B3C'" onmouseout="this.style.borderColor='transparent'">
            <img src="${I}icons/${$.name}" style="width:70px;height:56px;object-fit:contain;display:block;border-radius:4px">
          ...
          </div>`).join("")+"</div>",E.querySelectorAll("[data-pick]").forEach($=>{$.addEventListener("click",()=>{r&&(r.value=$.dataset.pick),i(),E.style.display="none"})})}catch(y){E.innerHTML='<div style="color:#cc2222;font-size:12px">Erreur chargement : '+y.message+"</div>"}}),(v=document.getElementById("ts-color"))==null||v.addEventListener("input",y=>{const h=document.getElementById("ts-color-hex");h&&(h.value=y.target.value)}),(w=document.getElementById("ts-color-hex"))==null||w.addEventListener("input",y=>{const h=y.target.value;if(/^#[0-9a-fA-F]{6}$/.test(h)){const l=document.getElementById("ts-color");l&&(l.value=h)}}),e.querySelectorAll("[data-edit-row]").forEach(y=>{const h=(t||[]).find(l=>l.id===y.dataset.editRow);h&&y.addEventListener("click",()=>He(h))}),e.querySelectorAll("[data-delete]").forEach(y=>{y.addEventListener("click",async()=>{confirm("Supprimer cette étape ?")&&(await _.from("tutorial_steps").delete().eq("id",y.dataset.delete),W(e))})}),e.querySelectorAll("[data-toggle]").forEach(y=>{y.addEventListener("click",async()=>{const h=(t||[]).find(l=>l.id===y.dataset.toggle);h&&(await _.from("tutorial_steps").update({is_active:!h.is_active}).eq("id",h.id),W(e))})}),e.querySelectorAll("[data-up]").forEach(y=>{y.addEventListener("click",async()=>{const h=t||[],l=h.findIndex(I=>I.id===y.dataset.up);l<=0||(await Promise.all([_.from("tutorial_steps").update({step_order:h[l-1].step_order}).eq("id",h[l].id),_.from("tutorial_steps").update({step_order:h[l].step_order}).eq("id",h[l-1].id)]),W(e))})}),e.querySelectorAll("[data-down]").forEach(y=>{y.addEventListener("click",async()=>{const h=t||[],l=h.findIndex(I=>I.id===y.dataset.down);l<0||l>=h.length-1||(await Promise.all([_.from("tutorial_steps").update({step_order:h[l+1].step_order}).eq("id",h[l].id),_.from("tutorial_steps").update({step_order:h[l].step_order}).eq("id",h[l+1].id)]),W(e))})})}function Mn(e){return`
  <div data-edit-row="${e.id}" style="background:#fff;border-radius:10px;padding:12px 16px;box-shadow:0 1px 4px rgba(0,0,0,0.08);display:flex;align-items:center;gap:12px;cursor:pointer">
  ...
  </div>`}function He(e,t=0){document.getElementById("ts-form-empty").style.display="none",document.getElementById("ts-form-content").style.display="block",document.getElementById("ts-form-title").textContent=e?"Modifier l'étape":"Nouvelle étape",document.getElementById("ts-id").value=(e==null?void 0:e.id)||"",document.getElementById("ts-emoji").value=(e==null?void 0:e.emoji)||"⚽",document.getElementById("ts-title").value=(e==null?void 0:e.title)||"",document.getElementById("ts-content").value=(e==null?void 0:e.content)||"";const n=document.getElementById("ts-editor");n&&(n.innerHTML=(e==null?void 0:e.content)||"<p>Texte de l'étape…</p>"),document.getElementById("ts-order").value=(e==null?void 0:e.step_order)??t,document.getElementById("ts-active").checked=e?e.is_active:!0;const a=(e==null?void 0:e.color)||"#1A6B3C";document.getElementById("ts-color").value=a,document.getElementById("ts-color-hex").value=a;const c=(e==null?void 0:e.image_url)||"",r=document.getElementById("ts-image");r&&(r.value=c);const g=document.getElementById("ts-img-preview"),x=document.getElementById("ts-img-preview-el");if(c&&g&&x){const E="/";x.src=`${E}icons/${c}`,g.style.display="block"}else g&&(g.style.display="none");document.getElementById("ts-img-picker-grid").style.display="none",document.getElementById("ts-preview-area").innerHTML="",document.getElementById("ts-form").scrollIntoView({behavior:"smooth"})}function yt(){document.getElementById("ts-form-content").style.display="none",document.getElementById("ts-form-empty").style.display="block",document.getElementById("ts-preview-area").innerHTML=""}function zn(){var i,p;const e=document.getElementById("ts-emoji").value||"⚽",t=document.getElementById("ts-title").value||"Titre",n=document.getElementById("ts-editor"),a=n?n.innerHTML:document.getElementById("ts-content").value||"",c=document.getElementById("ts-color-hex").value||"#1A6B3C",r=(p=(i=document.getElementById("ts-image"))==null?void 0:i.value)==null?void 0:p.trim(),x=r?`<div style="padding:0 20px 12px;text-align:center"><img src="/icons/${r}" style="max-height:140px;max-width:100%;border-radius:10px;object-fit:contain"></div>`:"",E=document.getElementById("ts-preview-area");E.innerHTML=`
    ...
    </div>`,E.querySelectorAll("ul,ol").forEach(o=>{o.style.paddingLeft="20px",o.style.margin="6px 0"}),E.querySelectorAll("li").forEach(o=>{o.style.marginBottom="4px"}),E.querySelectorAll("p").forEach(o=>{o.style.marginBottom="8px"})}async function Rn(e){var o,s;const t=document.getElementById("ts-id").value,n=document.getElementById("ts-emoji").value.trim()||"⚽",a=document.getElementById("ts-title").value.trim(),c=document.getElementById("ts-editor"),r=(c?c.innerHTML:document.getElementById("ts-content").value).trim(),g=document.getElementById("ts-color-hex").value.trim()||"#1A6B3C",x=parseInt(document.getElementById("ts-order").value)||0,E=document.getElementById("ts-active").checked;if(!a||!r){alert("Titre et contenu sont obligatoires.");return}const i={emoji:n,title:a,content:r,color:g,step_order:x,is_active:E,image_url:((s=(o=document.getElementById("ts-image"))==null?void 0:o.value)==null?void 0:s.trim())||null};let p;if(t?{error:p}=await _.from("tutorial_steps").update(i).eq("id",t):{error:p}=await _.from("tutorial_steps").insert(i),p){alert("Erreur : "+p.message);return}yt(),W(e)}const bt="/",vt=[["MA","Maroc"],["FR","France"],["AR","Argentine"],["PT","Portugal"],["BR","Brésil"],["ES","Espagne"],["DE","Allemagne"],["GB","Angleterre"],["IT","Italie"],["CM","Cameroun"],["SN","Sénégal"],["NG","Nigéria"],["DK","Danemark"],["NL","Pays-Bas"],["BE","Belgique"],["CI","Côte d'Ivoire"],["AL","Albanie"],["HR","Croatie"],["RS","Serbie"],["TR","Turquie"],["MW","Malawi"],["GH","Ghana"],["ZA","Afrique du Sud"],["EG","Égypte"],["US","États-Unis"],["MX","Mexique"],["CO","Colombie"],["UY","Uruguay"],["CL","Chili"],["JP","Japon"],["KR","Corée du Sud"],["CN","Chine"],["AU","Australie"],["SW","Suisse"],["SE","Suède"],["NO","Norvège"],["PL","Pologne"]];async function jn(e,t){await ce(e,t)}async function ce(e,t){var c,r;const[{data:n},{data:a}]=await Promise.all([_.from("stadium_definitions").select("*, club:clubs(encoded_name, logo_url)").order("name"),_.from("clubs").select("id, encoded_name, logo_url").order("encoded_name")]);e.innerHTML=`
    ...
    </div>`,(c=document.getElementById("st-add-btn"))==null||c.addEventListener("click",()=>qe(null,a,e,t)),(r=document.getElementById("st-gen-countries-btn"))==null||r.addEventListener("click",async()=>{const{toast:g}=t,x=new Set((n||[]).filter(s=>!s.club_id&&s.country_code).map(s=>s.country_code)),E=vt.filter(([s])=>!x.has(s));if(!E.length){g("Tous les pays ont déjà un stade.","info");return}if(!confirm(`Créer ${E.length} stade(s) "Pays" manquant(s) ?`))return;const i=E.map(([s,u])=>({name:`Stade ${u}`,club_id:null,country_code:s})),{data:p,error:o}=await _.from("stadium_definitions").insert(i).select();if(o){g("Erreur : "+o.message,"error");return}p!=null&&p.length&&await _.from("cards").insert(p.map(s=>({card_type:"stadium",stadium_id:s.id}))),g(`${(p==null?void 0:p.length)||0} stade(s) Pays créé(s) ✅`,"success"),ce(e,t)}),e.querySelectorAll("[data-edit-stadium]").forEach(g=>{g.addEventListener("click",()=>{const x=(n||[]).find(E=>E.id===g.dataset.editStadium);x&&qe(x,a,e,t)})})}function Dn(e){var c,r;let t=null;e.image_url?t=e.image_url.startsWith("http")?e.image_url:`${bt}icons/${e.image_url}`:(c=e.club)!=null&&c.logo_url?t=e.club.logo_url:e.country_code&&(t=`https://flagsapi.com/${e.country_code.slice(0,2).toUpperCase()}/flat/64.png`);const n=((r=e.club)==null?void 0:r.encoded_name)||e.country_code||"—",a=Xe(e.name,t,`${n}<br>+10 ⭐`,{width:140});return`<div data-edit-stadium="${e.id}" style="cursor:pointer;transition:transform .15s" onmouseover="this.style.transform='translateY(-3px)'" onmouseout="this.style.transform=''">
    ${a}
  </div>`}function qe(e,t,n,a){var p,o,s;const{openModal:c,closeModal:r,toast:g}=a,x=`
      ...
      <div style="flex:1;min-width:220px;display:flex;flex-direction:column;gap:12px">
        ...
        <div>
          ...
          <select id="st-country">
            ...
            ${vt.map(([u,d])=>`<option value="${u}" ${(e==null?void 0:e.country_code)===u?"selected":""}>${d} (${u})</option>`).join("")}
          </select>
        </div>
        <div>
  ...
  `);const i=()=>{var h,l,I,$,A,B,S;const u=((h=document.getElementById("st-name"))==null?void 0:h.value)||"NOM DU STADE",d=(I=(l=document.getElementById("st-image"))==null?void 0:l.value)==null?void 0:I.trim(),f=(A=($=document.getElementById("st-country"))==null?void 0:$.value)==null?void 0:A.trim(),m=document.getElementById("st-club"),b=(m==null?void 0:m.selectedIndex)||0,k=m&&b>0?m.options[b].text:"",L=((S=(B=m==null?void 0:m.options[b])==null?void 0:B.getAttribute)==null?void 0:S.call(B,"data-logo"))||"";let v=null;d?v=d.startsWith("http")?d:`${bt}icons/${d}`:L?v=L:f&&(v=`https://flagsapi.com/${f.toLowerCase().slice(0,2).toUpperCase()}/flat/64.png`);const w=k||f||"—",y=document.getElementById("st-preview-card");y&&(y.innerHTML=Xe(u,v,`${w}<br>+10 ⭐`,{width:140}))};i(),["st-name","st-club","st-country","st-image"].forEach(u=>{var d,f;(d=document.getElementById(u))==null||d.addEventListener("input",i),(f=document.getElementById(u))==null||f.addEventListener("change",i)}),(p=document.getElementById("st-cancel"))==null||p.addEventListener("click",()=>r()),(o=document.getElementById("st-save"))==null||o.addEventListener("click",async()=>{const u=document.getElementById("st-name").value.trim(),d=document.getElementById("st-club").value||null,f=document.getElementById("st-country").value.trim().toUpperCase()||null,m=document.getElementById("st-image").value.trim()||null;if(!u){g("Le nom est obligatoire","error");return}const b={name:u,club_id:d,country_code:f,image_url:m},{error:k}=e?await _.from("stadium_definitions").update(b).eq("id",e.id):await _.from("stadium_definitions").insert(b);if(k){g("Erreur : "+k.message,"error");return}g(e?"Stade modifié ✅":"Stade créé ✅","success"),r(),ce(n,a)}),(s=document.getElementById("st-delete"))==null||s.addEventListener("click",async()=>{confirm("Supprimer ce stade ?")&&(await _.from("stadium_definitions").delete().eq("id",e.id),g("Stade supprimé","success"),r(),ce(n,a))})}const q=[{value:"game_changer",label:"⚡ Game Changer",border:"#7a28b8",bg:"linear-gradient(135deg,#4a0080,#7a28b8)"},{value:"ultra_game_changer",label:"💎 Ultra Game Changer",border:"#00bcd4",bg:"linear-gradient(135deg,#006080,#00bcd4)"}],Pn=[{value:"purple",label:"Violet",hex:"#7a28b8"},{value:"light_blue",label:"Bleu clair",hex:"#00bcd4"}],H=[{value:"BOOST_STAT",label:"⚡ Boost stats (+N)",hasValue:!0,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"DEBUFF_STAT",label:"💀 Debuff stats (-N)",hasValue:!0,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"GRAY_PLAYER",label:"❄️ Griser un joueur",hasValue:!1,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"REVIVE_PLAYER",label:"💫 Ressusciter un joueur",hasValue:!1,hasTarget:!1,hasCount:!0,hasRoles:!1},{value:"REMOVE_GOAL",label:"🚫 Retirer un but adverse",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1},{value:"ADD_GOAL_DRAW",label:"⚽ +1 but si duel nul",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1},{value:"ADD_SUB",label:"🔄 Ajouter un remplacement",hasValue:!1,hasTarget:!1,hasCount:!0,hasRoles:!1},{value:"CUSTOM",label:"🛠️ Effet custom (hardcodé)",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1}],Ke=["GK","DEF","MIL","ATT"];async function Nn(e,t){await Ae(e,t)}async function Ae(e,t){var a;const{data:n}=await _.from("gc_definitions").select("*").order("sort_order").order("created_at");e.innerHTML=`
    <div style="padding:20px;max-width:1100px;margin:0 auto">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px">
      ...
      <div style="display:flex;flex-wrap:wrap;gap:14px" id="gc-list">
        ${(n||[]).length?(n||[]).map(c=>Fn(c)).join(""):'<div style="color:var(--tile-fg-dim);padding:24px;text-align:center;width:100%">Aucune carte Game Changer.</div>'}
      </div>
    </div>`,(a=document.getElementById("gc-add-btn"))==null||a.addEventListener("click",()=>Je(null,e,t)),e.querySelectorAll("[data-edit-gc]").forEach(c=>{c.addEventListener("click",()=>{const r=(n||[]).find(g=>g.id===c.dataset.editGc);r&&Je(r,e,t)})}),e.querySelectorAll(".btn-del-gc").forEach(c=>{c.addEventListener("click",async r=>{r.stopPropagation(),confirm("Supprimer cette carte Game Changer ?")&&(await _.from("gc_definitions").delete().eq("id",c.dataset.del),Ae(e,t))})})}function Fn(e){const t=q.find(c=>c.value===e.gc_type)||q[0],n=e.image_url?`/icons/${e.image_url}`:null,a=ye(e.name||"?",n,t.label.split(" ")[0],e.effect||"",{width:130});return`<div data-edit-gc="${e.id}" style="position:relative;cursor:pointer">
    ${a}
    <div style="position:absolute;top:4px;left:4px;z-index:10;display:flex;gap:3px">
  ...
  </div>`}function Je(e,t,n){var s,u,d,f;const{openModal:a,closeModal:c,toast:r}=n,g=!e,x=e||{name:"",gc_type:"game_changer",color:"purple",effect:"",image_url:"",sort_order:0,is_active:!0,effect_type:"BOOST_STAT",effect_params:{}},E=(()=>{const m=x.effect_params||{},b=H.find(k=>k.value===(x.effect_type||"BOOST_STAT"))||H[0];return`
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
      ${b.hasValue?`<div>
      ...
      ${b.hasRoles?`<div>
        ...
        <div style="display:flex;gap:4px;flex-wrap:wrap">
          ${Ke.map(k=>`<label style="display:flex;align-items:center;gap:2px;font-size:11px;cursor:pointer;color:#555">
            <input type="checkbox" class="gc-p-role" value="${k}" ${!m.roles||m.roles.includes(k)?"checked":""}> ${k}
          </label>`).join("")}
        </div>
      </div>`:""}
    </div>`})(),i=`
    ...
    <div style="display:flex;flex-direction:column;gap:12px">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
        ...
        <div>
          <label>COULEUR</label>
          <select id="gc-color">
            ${Pn.map(m=>`<option value="${m.value}" ${x.color===m.value?"selected":""}>${m.label}</option>`).join("")}
          </select>
        </div>
      </div>
      <div>
      ...
      <div style="border-top:1px solid #eee;padding-top:12px;margin-top:4px">
        ...
        <div>
          ...
          <select id="gc-effect-type" style="margin-bottom:10px">
            ${H.map(m=>`<option value="${m.value}" ${(x.effect_type||"BOOST_STAT")===m.value?"selected":""}>${m.label}</option>`).join("")}
          </select>
        </div>
        <div id="gc-params-wrap">${E}</div>
      </div>
    </div>
  `;a(g?"Nouvelle carte Game Changer":`Modifier : ${x.name}`,i,`
    <button id="gc-cancel" class="btn btn-ghost">Annuler</button>
    <button id="gc-save" class="btn btn-primary">💾 Enregistrer</button>
  `);const o=()=>{var y,h,l,I,$;const m=((y=document.getElementById("gc-name"))==null?void 0:y.value)||"?",b=((h=document.getElementById("gc-effect"))==null?void 0:h.value)||"",k=((l=document.getElementById("gc-type"))==null?void 0:l.value)||"game_changer",L=($=(I=document.getElementById("gc-image-url"))==null?void 0:I.value)==null?void 0:$.trim(),v=q.find(A=>A.value===k)||q[0],w=document.getElementById("gc-modal-preview");w&&(w.innerHTML=ye(m,L?`/icons/${L}`:null,v.label.split(" ")[0],b,{width:150}))};["gc-name","gc-effect","gc-type","gc-image-url"].forEach(m=>{var b,k;(b=document.getElementById(m))==null||b.addEventListener("input",o),(k=document.getElementById(m))==null||k.addEventListener("change",o)}),(s=document.getElementById("gc-effect-type"))==null||s.addEventListener("change",()=>{const m=document.getElementById("gc-effect-type").value,b=H.find(L=>L.value===m)||H[0],k=document.getElementById("gc-params-wrap");k.innerHTML=`
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
      ...
      ${b.hasRoles?`<div><label>POSTES CIBLÉS</label><div style="display:flex;gap:4px;flex-wrap:wrap">${Ke.map(L=>`<label style="display:flex;align-items:center;gap:2px;font-size:11px;cursor:pointer;color:#555"><input type="checkbox" class="gc-p-role" value="${L}" checked> ${L}</label>`).join("")}</div></div>`:""}
    </div>`}),(u=document.getElementById("btn-pick-gc-icon"))==null||u.addEventListener("click",async()=>{var b;const m=document.getElementById("gc-icon-picker");if(m){if(m.style.display!=="none"){m.style.display="none";return}m.innerHTML='<div style="padding:10px;text-align:center;color:#888;font-size:12px">⏳ Chargement...</div>',m.style.display="block";try{const L=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),v=Array.isArray(L)?L.filter(y=>y.name.startsWith("gamechanger-")&&/\.(png|jpg|svg|webp)$/i.test(y.name)):[];if(!v.length){m.innerHTML='<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Aucun fichier "gamechanger-*" trouvé dans public/icons/</div>';return}const w=((b=document.getElementById("gc-image-url"))==null?void 0:b.value)||"";m.innerHTML=`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(72px,1fr));gap:8px">
        ${v.map(y=>`
        <div class="gc-icon-item" data-name="${y.name}"
          style="cursor:pointer;text-align:center;padding:6px;border-radius:6px;border:2px solid ${y.name===w?"#7a28b8":"#ddd"};background:${y.name===w?"#f5f0ff":"#fff"}">
      ...
      </div>`,m.querySelectorAll(".gc-icon-item").forEach(y=>{y.addEventListener("click",()=>{const h=document.getElementById("gc-image-url");h&&(h.value=y.dataset.name),m.style.display="none",o()})})}catch(k){m.innerHTML=`<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Erreur API GitHub : ${k.message}</div>`}}}),(d=document.getElementById("gc-cancel"))==null||d.addEventListener("click",()=>c()),(f=document.getElementById("gc-save"))==null||f.addEventListener("click",async()=>{var w,y,h,l,I,$,A,B,S,C,T,O,U,G;const m=H.find(N=>{var D;return N.value===(((D=document.getElementById("gc-effect-type"))==null?void 0:D.value)||"BOOST_STAT")})||H[0],b=m.hasRoles?[...document.querySelectorAll(".gc-p-role:checked")].map(N=>N.value):null,k={...m.hasValue?{value:Number((w=document.getElementById("gc-p-value"))==null?void 0:w.value)||2}:{},...m.hasCount?{count:Number((y=document.getElementById("gc-p-count"))==null?void 0:y.value)||1}:{},...m.hasTarget?{target:((h=document.getElementById("gc-p-target"))==null?void 0:h.value)||"home"}:{},...m.hasRoles?{roles:b!=null&&b.length?b:null}:{}},L={name:((I=(l=document.getElementById("gc-name"))==null?void 0:l.value)==null?void 0:I.trim())||"",effect:((A=($=document.getElementById("gc-effect"))==null?void 0:$.value)==null?void 0:A.trim())||null,image_url:((S=(B=document.getElementById("gc-image-url"))==null?void 0:B.value)==null?void 0:S.trim())||null,gc_type:((C=document.getElementById("gc-type"))==null?void 0:C.value)||"game_changer",color:((T=document.getElementById("gc-color"))==null?void 0:T.value)||"purple",sort_order:Number((O=document.getElementById("gc-sort"))==null?void 0:O.value)||0,is_active:((U=document.getElementById("gc-active"))==null?void 0:U.checked)??!0,effect_type:((G=document.getElementById("gc-effect-type"))==null?void 0:G.value)||"BOOST_STAT",effect_params:k};if(!L.name){r("Le nom est obligatoire","error");return}const{error:v}=g?await _.from("gc_definitions").insert(L):await _.from("gc_definitions").update(L).eq("id",x.id);if(v){r(v.message,"error");return}r(g?"Carte créée ✅":"Carte modifiée ✅","success"),c(),Ae(t,n)})}async function On(e,{toast:t,openModal:n,closeModal:a}){await j(e,{toast:t,openModal:n,closeModal:a})}async function j(e,t){var y,h;const{toast:n,openModal:a,closeModal:c}=t,[{data:r,error:g},{data:x},{data:E},{data:i}]=await Promise.all([_.from("ranked_seasons").select("*").order("start_at",{ascending:!1}),_.from("users").select("mmr,rank_tier,placement_matches").gte("placement_matches",1),_.from("season_reward_tiers").select("*").order("rank_min"),_.from("booster_configs").select("id,name").order("sort_order")]);if(g){e.innerHTML=`<p style="color:red">${g.message}</p>`;return}const p=r||[],o=p.find(l=>l.is_active),s=(x||[]).filter(l=>(l.placement_matches||0)>=1),u=s.filter(l=>(l.placement_matches||0)>=10),d=["bronze","silver","gold","platinum","diamond","master"],f={bronze:"Bronze",silver:"Argent",gold:"Or",platinum:"Platine",diamond:"Diamant",master:"Maître"},m={bronze:"#cd7f32",silver:"#a0a0a0",gold:"#D4A017",platinum:"#00e5ff",diamond:"#5b9bd5",master:"#9b59b6"},b={bronze:"🥉",silver:"🥈",gold:"🥇",platinum:"💎",diamond:"🔷",master:"👑"},k={};d.forEach(l=>{k[l]=0}),u.forEach(l=>{const I=l.rank_tier||"bronze";k[I]!==void 0&&k[I]++});const L=u.length?Math.round(u.reduce((l,I)=>l+(I.mmr||1e3),0)/u.length):0;function v(l){return l?new Date(l).toLocaleDateString("fr",{day:"numeric",month:"short",year:"numeric"}):"—"}function w(l){const I=new Date;return l.is_active?'<span style="background:#1A6B3C;color:#fff;padding:2px 8px;border-radius:10px;font-size:11px;font-weight:700">● ACTIVE</span>':new Date(l.end_at)<I?'<span style="background:#555;color:#ccc;padding:2px 8px;border-radius:10px;font-size:11px">Terminée</span>':'<span style="background:#e67e22;color:#fff;padding:2px 8px;border-radius:10px;font-size:11px">À venir</span>'}e.innerHTML=`
    <div style="display:flex;flex-direction:column;gap:20px">
      ...
      <div class="card-panel" style="background:linear-gradient(135deg,#0a3d1e,#1a6b3c20);border:1.5px solid #1A6B3C40">
        ...
        <div style="display:flex;flex-direction:column;gap:4px">
          ${d.map(l=>{const I=k[l],$=u.length>0?Math.round(I/u.length*100):0;return`
              <div style="display:flex;align-items:center;gap:8px;font-size:12px">
                <span style="width:80px">${b[l]} ${f[l]}</span>
                <div style="flex:1;background:rgba(255,255,255,0.08);border-radius:4px;height:14px;overflow:hidden">
                  <div style="height:100%;width:${$}%;background:${m[l]};border-radius:4px;transition:width 0.5s"></div>
                </div>
        ...
        <div class="table-wrap">
          <table>
            ...
            <tbody id="seasons-tbody">
              ${p.length===0?'<tr><td colspan="6" style="text-align:center;color:var(--gray-600);padding:24px">Aucune saison créée.</td></tr>':p.map(l=>{const I=Math.round((new Date(l.end_at)-new Date(l.start_at))/864e5),$=new Date(l.end_at)<new Date&&!l.is_active;return`
                    <tr>
                      <td><b>${l.name}</b></td>
                      <td style="font-size:12px">${v(l.start_at)}</td>
                      <td style="font-size:12px">${v(l.end_at)}</td>
                      <td style="font-size:12px">${I} jours</td>
                      <td>${w(l)}</td>
                      <td style="display:flex;gap:6px;flex-wrap:wrap">
                        <button class="btn btn-ghost btn-sm" data-edit="${l.id}">✏️ Modifier</button>
                        ${l.is_active?`<button class="btn btn-ghost btn-sm" data-deactivate="${l.id}" style="color:#e67e22">⏹ Désactiver</button>`:`<button class="btn btn-primary btn-sm" data-activate="${l.id}">▶ Activer</button>`}
                        ${$?`<button class="btn btn-danger btn-sm" data-delete="${l.id}">🗑</button>`:""}
                      </td>
                    </tr>`}).join("")}
            </tbody>
          </table>
      ...
      <div class="card-panel">
        ...
        <div class="table-wrap">
          <table>
            ...
            <tbody>
              ${(E||[]).map(l=>{var A;const I=!l.activate_at||new Date(l.activate_at)<=new Date;return`<tr>
                  <td style="font-size:12px">${((A=p.find(B=>B.id===l.season_id))==null?void 0:A.name)||"—"}</td>
                  <td><b>${l.label}</b></td>
                  <td>${l.rank_min===l.rank_max?`#${l.rank_min}`:`#${l.rank_min}–${l.rank_max}`}</td>
                  <td style="text-align:right">${(l.credits||0).toLocaleString("fr")}</td>
                  <td style="text-align:center">${(l.player_ids||[]).length}</td>
                  <td style="text-align:center">${(l.booster_config_ids||[]).length}</td>
                  <td style="font-size:12px">${l.activate_at?v(l.activate_at):"Immédiat"}</td>
                  <td>
                    ${l.distributed_at?`<span style="background:#555;color:#ccc;padding:2px 8px;border-radius:10px;font-size:11px">Distribué le ${v(l.distributed_at)}</span>`:I?'<span style="background:#1A6B3C;color:#fff;padding:2px 8px;border-radius:10px;font-size:11px">● Prêt</span>':'<span style="background:#e67e22;color:#fff;padding:2px 8px;border-radius:10px;font-size:11px">⏳ Programmé</span>'}
                  </td>
                  <td style="white-space:nowrap">
                    <button class="btn btn-ghost btn-sm" data-edit-tier="${l.id}">✏️</button>
                    <button class="btn btn-primary btn-sm" data-distribute-tier="${l.id}" ${I?"":"disabled"}>🎁 Distribuer</button>
                    <button class="btn btn-danger btn-sm" data-delete-tier="${l.id}">🗑️</button>
                  </td>
    ...
    </div>`,(y=document.getElementById("create-season-btn"))==null||y.addEventListener("click",()=>{Ye(null,{toast:n,openModal:a,closeModal:c,reload:()=>j(e,t)})}),e.querySelectorAll("[data-edit]").forEach(l=>{const I=p.find($=>$.id==l.dataset.edit);l.addEventListener("click",()=>{Ye(I,{toast:n,openModal:a,closeModal:c,reload:()=>j(e,t)})})}),e.querySelectorAll("[data-activate]").forEach(l=>{l.addEventListener("click",async()=>{const I=parseInt(l.dataset.activate);if(!confirm("Activer cette saison ? La saison actuellement active sera désactivée."))return;const{error:$}=await _.from("ranked_seasons").update({is_active:!1}).neq("id",0);if($){n($.message,"error");return}const{error:A}=await _.from("ranked_seasons").update({is_active:!0}).eq("id",I);if(A){n(A.message,"error");return}await _.from("users").update({current_season_id:I}).gt("placement_matches",0),n("Saison activée ✅","success"),j(e,t)})}),e.querySelectorAll("[data-deactivate]").forEach(l=>{l.addEventListener("click",async()=>{if(!confirm("Désactiver la saison active ? Le mode Ranked sera suspendu."))return;const{error:I}=await _.from("ranked_seasons").update({is_active:!1}).eq("id",parseInt(l.dataset.deactivate));if(I){n(I.message,"error");return}n("Saison désactivée","success"),j(e,t)})}),e.querySelectorAll("[data-delete]").forEach(l=>{l.addEventListener("click",async()=>{if(!confirm("Supprimer cette saison archivée ?"))return;const{error:I}=await _.from("ranked_seasons").delete().eq("id",parseInt(l.dataset.delete));if(I){n(I.message,"error");return}n("Saison supprimée","success"),j(e,t)})}),(h=document.getElementById("add-reward-tier-btn"))==null||h.addEventListener("click",()=>{Ve(null,p,o,i||[],{toast:n,openModal:a,closeModal:c,reload:()=>j(e,t)})}),e.querySelectorAll("[data-edit-tier]").forEach(l=>{const I=(E||[]).find($=>$.id===l.dataset.editTier);l.addEventListener("click",()=>{Ve(I,p,o,i||[],{toast:n,openModal:a,closeModal:c,reload:()=>j(e,t)})})}),e.querySelectorAll("[data-distribute-tier]").forEach(l=>{l.addEventListener("click",async()=>{const I=(E||[]).find(B=>B.id===l.dataset.distributeTier);if(!confirm(`Distribuer les récompenses du palier "${I==null?void 0:I.label}" à tous les joueurs concernés ?`))return;l.disabled=!0,l.textContent="⏳...";const{data:$,error:A}=await _.rpc("admin_distribute_season_reward",{p_tier_id:l.dataset.distributeTier});if(A){n(A.message,"error"),l.disabled=!1,l.textContent="🎁 Distribuer";return}if(!($!=null&&$.success)){n(($==null?void 0:$.error)||"Échec de la distribution","error"),l.disabled=!1,l.textContent="🎁 Distribuer";return}n(`Récompenses distribuées à ${$.rewarded} joueur(s) ✅`,"success"),j(e,t)})}),e.querySelectorAll("[data-delete-tier]").forEach(l=>{l.addEventListener("click",async()=>{if(!confirm("Supprimer ce palier de récompense ?"))return;const{error:I}=await _.from("season_reward_tiers").delete().eq("id",l.dataset.deleteTier);if(I){n(I.message,"error");return}n("Palier supprimé","success"),j(e,t)})})}function Ve(e,t,n,a,{toast:c,openModal:r,closeModal:g,reload:x}){var f,m,b,k,L;const E=!!e,i=(e==null?void 0:e.season_id)??(n==null?void 0:n.id)??((f=t[0])==null?void 0:f.id)??null;let p=[];const o=v=>{if(!v)return"";const w=new Date(v);return new Date(w.getTime()-w.getTimezoneOffset()*6e4).toISOString().slice(0,16)},s=`
    <div style="display:flex;flex-direction:column;gap:12px">
      <div>
        <label>SAISON RANKED</label>
        <select id="rt-season">
          ${t.map(v=>`<option value="${v.id}" ${v.id===i?"selected":""}>${v.name}${v.is_active?" (active)":""}</option>`).join("")}
        </select>
      </div>
      <div>
        <label>LIBELLÉ</label>
      ...
      <div>
        ...
        <input id="rt-activate-at" type="datetime-local" value="${o(e==null?void 0:e.activate_at)}">
      </div>
      <div>
        ...
        <div style="display:flex;flex-direction:column;gap:6px;max-height:140px;overflow-y:auto;background:#f7f7f7;border-radius:8px;padding:8px">
          ${a.length?a.map(v=>`
            <label style="display:flex;align-items:center;gap:8px;font-size:13px;cursor:pointer;color:#333">
              <input type="checkbox" class="rt-booster-cb" value="${v.id}" ${((e==null?void 0:e.booster_config_ids)||[]).includes(v.id)?"checked":""}>
              ${v.name}
  ...
  `;r(E?`Modifier : ${e.label}`:"Nouveau palier de récompense",s,`
    <button id="rt-cancel" class="btn btn-ghost">Annuler</button>
    <button id="rt-save" class="btn btn-primary">💾 Enregistrer</button>
  `);function d(){const v=document.getElementById("rt-player-chips");v&&(v.innerHTML=p.map(w=>`
      ...
      </span>`).join(""),v.querySelectorAll("[data-remove-player]").forEach(w=>{w.addEventListener("click",()=>{p=p.filter(y=>y.id!==w.dataset.removePlayer),d()})}))}(m=e==null?void 0:e.player_ids)!=null&&m.length&&_.from("players").select("id,firstname,surname_real").in("id",e.player_ids).then(({data:v})=>{p=(v||[]).map(w=>({id:w.id,label:`${w.firstname} ${w.surname_real}`})),d()}),(b=document.getElementById("rt-player-search-btn"))==null||b.addEventListener("click",async()=>{const v=document.getElementById("rt-player-search").value.trim();if(v.length<2)return;const{data:w}=await _.from("players").select("id,firstname,surname_real,rarity").or(`firstname.ilike.%${v}%,surname_real.ilike.%${v}%`).limit(10),y=document.getElementById("rt-player-results");if(!(w!=null&&w.length)){y.style.display="flex",y.innerHTML='<div style="color:#999;font-size:12px;padding:4px">Aucun résultat.</div>';return}y.style.display="flex",y.innerHTML=w.map(h=>`
      ...
      </div>`).join(""),y.querySelectorAll("[data-add-player]").forEach(h=>{h.addEventListener("click",()=>{const l=h.dataset.addPlayer;p.some(I=>I.id===l)||p.push({id:l,label:h.dataset.label}),d(),y.style.display="none",document.getElementById("rt-player-search").value=""})})}),(k=document.getElementById("rt-cancel"))==null||k.addEventListener("click",()=>g()),(L=document.getElementById("rt-save"))==null||L.addEventListener("click",async()=>{const v=document.getElementById("rt-error"),w=parseInt(document.getElementById("rt-season").value),y=document.getElementById("rt-label").value.trim(),h=parseInt(document.getElementById("rt-rank-min").value)||1,l=parseInt(document.getElementById("rt-rank-max").value)||1,I=parseInt(document.getElementById("rt-credits").value)||0,$=document.getElementById("rt-activate-at").value,A=$?new Date($).toISOString():null,B=[...document.querySelectorAll(".rt-booster-cb:checked")].map(T=>T.value);if(!y){v.textContent="Le libellé est obligatoire.";return}if(h>l){v.textContent="Le classement min doit être ≤ au max.";return}if(!w){v.textContent="Choisis une saison.";return}const S={season_id:w,label:y,rank_min:h,rank_max:l,credits:I,booster_config_ids:B,player_ids:p.map(T=>T.id),activate_at:A},{error:C}=E?await _.from("season_reward_tiers").update(S).eq("id",e.id):await _.from("season_reward_tiers").insert(S);if(C){v.textContent=C.message;return}c(E?"Palier modifié ✅":"Palier créé ✅","success"),g(),x()})}function Ye(e,{toast:t,openModal:n,closeModal:a,reload:c}){const r=!!e,g=new Date().toISOString().slice(0,16),x=new Date(Date.now()+90*864e5).toISOString().slice(0,16),E=e?new Date(e.start_at).toISOString().slice(0,16):g,i=e?new Date(e.end_at).toISOString().slice(0,16):x,p=(e==null?void 0:e.name)||"";n(r?"✏️ Modifier la saison":"➕ Nouvelle saison",`<div style="display:flex;flex-direction:column;gap:14px">
      <div class="form-group">
    ...
    </div>`);function o(){var m,b;const s=(m=document.getElementById("season-start"))==null?void 0:m.value,u=(b=document.getElementById("season-end"))==null?void 0:b.value,d=document.getElementById("season-duration");if(!s||!u||!d)return;const f=Math.round((new Date(u)-new Date(s))/864e5);d.textContent=f>0?`Durée : ${f} jour${f>1?"s":""}`:"⚠️ La fin doit être après le début",d.style.color=f>0?"var(--gray-600)":"#bb2020"}setTimeout(()=>{var s,u,d;(s=document.getElementById("season-start"))==null||s.addEventListener("input",o),(u=document.getElementById("season-end"))==null||u.addEventListener("input",o),o(),(d=document.getElementById("season-save-btn"))==null||d.addEventListener("click",async()=>{var y,h,l;const f=(y=document.getElementById("season-name"))==null?void 0:y.value.trim(),m=(h=document.getElementById("season-start"))==null?void 0:h.value,b=(l=document.getElementById("season-end"))==null?void 0:l.value,k=document.getElementById("season-error");if(!f){k.textContent="Le nom est requis.";return}if(!m){k.textContent="La date de début est requise.";return}if(!b){k.textContent="La date de fin est requise.";return}if(new Date(b)<=new Date(m)){k.textContent="La date de fin doit être après le début.";return}const L=document.getElementById("season-save-btn");L.disabled=!0,L.textContent="Enregistrement…";const v={name:f,start_at:new Date(m).toISOString(),end_at:new Date(b).toISOString()};let w;if(r?{error:w}=await _.from("ranked_seasons").update(v).eq("id",e.id):{error:w}=await _.from("ranked_seasons").insert({...v,is_active:!1}),w){k.textContent=w.message,L.disabled=!1,L.textContent=r?"💾 Enregistrer":"✅ Créer la saison";return}t(r?"Saison modifiée ✅":"Saison créée ✅","success"),a(),c()})},50)}async function xe(e,t){var E;const{toast:n}=t,{data:a,error:c}=await _.from("solo_levels").select("*").order("level_number");if(c){e.innerHTML=`<p style="color:var(--danger)">${c.message}</p>`;return}const r=i=>`
    <tr data-row="${i.id}">
