import{s as h,e as H,g as Me,K as Ke,D as A,S as Je,a as z,H as Ye,R as We,b as je,r as Ze,c as Xe,E as Qe,d as et,N as tt,M as nt,B as it,f as ot,F as de,h as ve,i as at,j as rt}from"./formation-links-DwOY6Nx_.js";async function lt(e){const[{count:n},{count:t},{count:i},{count:p},{count:c}]=await Promise.all([h.from("players").select("*",{count:"exact",head:!0}),h.from("clubs").select("*",{count:"exact",head:!0}),h.from("users").select("*",{count:"exact",head:!0}),h.from("cards").select("*",{count:"exact",head:!0}),h.from("matches").select("*",{count:"exact",head:!0})]);e.innerHTML=`
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;gap:12px;flex-wrap:wrap">
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        ...
        <select id="filter-rarity" style="width:130px">
          ...
          ${Object.entries(ye).map(([r,d])=>`<option value="${r}">${d}</option>`).join("")}
        </select>
  ...
  `,o(c);function o(r){document.getElementById("count-label").textContent=`${r.length} joueur(s)`,document.getElementById("players-tbody").innerHTML=r.map(d=>{var y;return`
      <tr>
        <td>
    ...
    `}).join(""),document.querySelectorAll("[data-edit]").forEach(d=>{d.addEventListener("click",()=>{const y=c.find(f=>f.id===d.dataset.edit);he(y,a||[],{toast:t,openModal:i,closeModal:p,reload:()=>te(e,n)})})}),document.querySelectorAll("[data-del]").forEach(d=>{d.addEventListener("click",async()=>{if(!confirm("Supprimer ce joueur ? Les cartes associées resteront en jeu."))return;const{error:y}=await h.from("players").delete().eq("id",d.dataset.del);y?t(y.message,"error"):(t("Joueur supprimé","success"),te(e,n))})})}function u(){const r=document.getElementById("search-players").value.toLowerCase(),d=document.getElementById("filter-rarity").value,y=document.getElementById("filter-job").value;o(c.filter(f=>(!r||`${f.firstname} ${f.surname_encoded} ${f.surname_real}`.toLowerCase().includes(r))&&(!d||f.rarity===d)&&(!y||f.job===y)))}document.getElementById("search-players").addEventListener("input",u),document.getElementById("filter-rarity").addEventListener("change",u),document.getElementById("filter-job").addEventListener("change",u),document.getElementById("add-player-btn").addEventListener("click",()=>{he(null,a||[],{toast:t,openModal:i,closeModal:p,reload:()=>te(e,n)})})}function he(e,n,{toast:t,openModal:i,closeModal:p,reload:c}){const s=!!e,a='<option value="">— Club —</option>'+n.map(l=>`<option value="${l.id}" ${(e==null?void 0:e.club_id)===l.id?"selected":""}>${l.encoded_name}</option>`).join("");i(s?`Modifier ${e.firstname} ${e.surname_encoded}`:"Nouveau joueur",`<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
    ...
    <div id="clubs-list" style="display:flex;flex-direction:column;gap:8px"></div>`,s(ne),document.getElementById("search-clubs").addEventListener("input",a=>{const l=a.target.value.toLowerCase();s(ne.filter(o=>o.real_name.toLowerCase().includes(l)||o.encoded_name.toLowerCase().includes(l)))}),document.getElementById("add-club-btn").addEventListener("click",()=>Ee(null,e,n));function s(a){const l=document.getElementById("clubs-list");if(!a.length){l.innerHTML='<p style="color:var(--gray-600);padding:20px">Aucun club.</p>';return}l.innerHTML=a.map(o=>{const u=Pe(o),r=Me(u,o.id).replace("<svg ",'<svg style="width:40px;height:48px" '),d=o.logo_url?`<img src="${o.logo_url}" style="width:40px;height:40px;object-fit:contain;border-radius:8px">`:`<div style="width:40px;height:40px;background:linear-gradient(135deg,${u.color1},${u.color2});border-radius:8px;display:flex;align-items:center;justify-content:center;color:#fff;font-size:10px;font-weight:900">${o.encoded_name.slice(0,3)}</div>`,y=t[o.id]||0;return`
        ...
        </div>`}).join(""),l.querySelectorAll("[data-gen]").forEach(o=>{o.addEventListener("click",async()=>{confirm(`Générer 20 joueurs pour ${o.dataset.name} ?`)&&(o.disabled=!0,o.textContent="⏳",await De(o.dataset.gen,o.dataset.cc,i),ae(e,n))})}),l.querySelectorAll("[data-edit]").forEach(o=>{const u=ne.find(r=>r.id===o.dataset.edit);o.addEventListener("click",()=>Ee(u,e,n))}),l.querySelectorAll("[data-del]").forEach(o=>{o.addEventListener("click",async()=>{if(!confirm("Supprimer ce club ?"))return;const{error:u}=await h.from("clubs").delete().eq("id",o.dataset.del);u?i(u.message,"error"):(i("Club supprimé","success"),ae(e,n))})})}}function Ee(e,n,t){const{toast:i,openModal:p,closeModal:c}=t,s=!!e,a=e?Pe(e):{...A},l=Object.entries(Ke).map(([u,r])=>`<option value="${u}" ${a.style===u?"selected":""}>${r.label}</option>`).join(""),o=Re.map(u=>`<option value="${u}" ${((e==null?void 0:e.country_code)||"FR")===u?"selected":""}>${u}</option>`).join("");p(s?`✏️ ${e.real_name}`:"➕ Nouveau club",`<div style="display:flex;flex-direction:column;gap:14px">
      ...
      <div style="border-top:1px solid var(--gray-200);padding-top:12px">
        ...
        <div style="display:flex;gap:12px;align-items:flex-start">
          ...
          <div style="flex:1;display:flex;flex-direction:column;gap:8px">
            <div class="form-group">
              <label>Style</label>
              <select id="m-kit-style" style="width:100%">${l}</select>
            </div>
            ${[["Couleur principale","m-kit-color1",a.color1],["Couleur secondaire","m-kit-color2",a.color2],["Short","m-kit-shorts",a.shorts],["Chaussettes","m-kit-socks",a.socks]].map(([u,r,d])=>`
              <div class="form-group">
                <label>${u}</label>
                <div style="display:flex;gap:6px;align-items:center">
                  <input type="color" id="${r}" value="${d}" style="width:38px;height:32px;padding:2px;border:1px solid var(--gray-200);border-radius:6px;cursor:pointer;flex-shrink:0">
                  <input id="${r}-txt" value="${d}" maxlength="7" style="flex:1;font-family:monospace;font-size:12px">
                </div>
              </div>`).join("")}
          </div>
          <!-- Aperçu -->
    ...
    </div>`),setTimeout(()=>{var y,f,w;X(),["m-kit-color1","m-kit-color2","m-kit-shorts","m-kit-socks"].forEach(E=>{const m=document.getElementById(E),_=document.getElementById(E+"-txt");!m||!_||(m.addEventListener("input",()=>{_.value=m.value,X()}),_.addEventListener("input",()=>{const $=_.value.trim();/^#[0-9a-fA-F]{6}$/.test($)&&(m.value=$,X())}),_.addEventListener("change",()=>{let $=_.value.trim();$.startsWith("#")||($="#"+$),/^#[0-9a-fA-F]{6}$/.test($)&&(m.value=$,_.value=$,X())}))}),(y=document.getElementById("m-kit-style"))==null||y.addEventListener("change",X);const u=document.getElementById("m-real"),r=document.getElementById("m-encoded");function d(){if(!u||!r||r.value)return;const E=u.value.trim().split(/\s+/),m=E.length===1?E[0].toUpperCase().slice(0,6):E.filter(_=>_.length>2).map(_=>_[0].toUpperCase()).join("")||E[0].toUpperCase().slice(0,4);r.value=m}u==null||u.addEventListener("input",d),(f=document.getElementById("auto-encode-btn"))==null||f.addEventListener("click",()=>{r&&(r.value=""),d()}),(w=document.getElementById("m-save"))==null||w.addEventListener("click",()=>yt(e,s,n,t))},50)}function qe(){var e,n,t,i,p;return{style:((e=document.getElementById("m-kit-style"))==null?void 0:e.value)||"uni",color1:((n=document.getElementById("m-kit-color1"))==null?void 0:n.value)||"#1A6B3C",color2:((t=document.getElementById("m-kit-color2"))==null?void 0:t.value)||"#ffffff",shorts:((i=document.getElementById("m-kit-shorts"))==null?void 0:i.value)||"#111111",socks:((p=document.getElementById("m-kit-socks"))==null?void 0:p.value)||"#ffffff"}}function X(){const e=document.getElementById("kit-preview-wrap");e&&(e.innerHTML=Me(qe(),"modal"))}async function yt(e,n,t,i){var m,_,$,v,g,b;const{toast:p,closeModal:c}=i,s=document.getElementById("m-error"),a=document.getElementById("m-save"),l=(m=document.getElementById("m-real"))==null?void 0:m.value.trim(),o=(_=document.getElementById("m-encoded"))==null?void 0:_.value.trim().toUpperCase(),u=($=document.getElementById("m-country"))==null?void 0:$.value.trim().toUpperCase(),r=((v=document.getElementById("m-logo"))==null?void 0:v.value.trim())||null,d=((g=document.getElementById("m-gen-stadium"))==null?void 0:g.checked)??!1,y=((b=document.getElementById("m-gen-squad"))==null?void 0:b.checked)??!1,f=qe();if(!l){s.textContent="Le nom du club est requis.";return}if(!o){s.textContent="Le nom encodé est requis.";return}if(!u){s.textContent="Le pays est requis.";return}a.disabled=!0,a.textContent="Enregistrement…";const w={real_name:l,encoded_name:o,country_code:u,logo_url:r,kit_style:f.style,kit_color1:f.color1,kit_color2:f.color2,kit_shorts:f.shorts,kit_socks:f.socks};let E=e==null?void 0:e.id;if(n){const{error:x}=await h.from("clubs").update(w).eq("id",E);if(x){s.textContent=x.message,a.disabled=!1,a.textContent="💾 Enregistrer";return}}else{const{data:x,error:k}=await h.from("clubs").insert(w).select().single();if(k){s.textContent=k.message,a.disabled=!1,a.textContent="✅ Créer le club";return}if(E=x.id,d){a.textContent="🏟️ Création du stade…";const{data:L,error:I}=await h.from("stadium_definitions").insert({name:`Stade de ${l}`,club_id:E,country_code:u}).select().single();I?console.warn("[Stadium] Erreur def stade:",I.message):L&&await h.from("cards").insert({card_type:"stadium",stadium_id:L.id})}y&&(a.textContent="⚽ Génération des joueurs…",await De(E,u,p))}p(n?"Club modifié ✅":"Club créé ✅","success"),c(),ae(t,i)}const ht=["normal","pepite","papyte","legende"],we=["GK","DEF","MIL","ATT"],Et=[["MA","Maroc"],["FR","France"],["AR","Argentine"],["PT","Portugal"],["BR","Brésil"],["ES","Espagne"],["DE","Allemagne"],["GB","Angleterre"],["IT","Italie"],["CM","Cameroun"],["SN","Sénégal"],["NG","Nigéria"],["DK","Danemark"],["NL","Pays-Bas"],["BE","Belgique"],["CI","Côte d'Ivoire"],["AL","Albanie"],["HR","Croatie"],["RS","Serbie"],["TR","Turquie"],["MW","Malawi"]];let D={...z},ie=[];async function wt(e,{toast:n}){const{data:t}=await h.from("clubs").select("id,encoded_name,logo_url,kit_style,kit_color1,kit_color2,kit_shorts,kit_socks").order("encoded_name");ie=t||[],D={...z},e.innerHTML=It(ie),$t(e,ie,n),T()}function _e(){var t;const e=((t=document.getElementById("cb-surname-real"))==null?void 0:t.value)||"",n=document.getElementById("cb-surname-enc");n&&(n.value=H(e))}function _t(){var t;const e=(t=document.getElementById("cb-club"))==null?void 0:t.value,n=ie.find(i=>String(i.id)===String(e));return n?{style:n.kit_style||A.style,color1:n.kit_color1||A.color1,color2:n.kit_color2||A.color2,shorts:n.kit_shorts||A.shorts,socks:n.kit_socks||A.socks}:{...A}}function T(){var d,y,f,w;const e=E=>{var m;return((m=document.getElementById(E))==null?void 0:m.value)||""},n={firstname:e("cb-firstname"),surname_encoded:e("cb-surname-enc")||H(e("cb-surname-real")),country_code:e("cb-country"),club_encoded_name:((y=(d=document.getElementById("cb-club"))==null?void 0:d.selectedOptions[0])==null?void 0:y.text)||"",job:e("cb-job"),job2:e("cb-job2")||null,note_g:parseInt(e("cb-note-g"))||0,note_d:parseInt(e("cb-note-d"))||0,note_m:parseInt(e("cb-note-m"))||0,note_a:parseInt(e("cb-note-a"))||0,rarity:e("cb-rarity")},t=_t(),i=document.getElementById("cb-club"),p=((w=(f=i==null?void 0:i.selectedOptions[0])==null?void 0:f.dataset)==null?void 0:w.logo)||null,c=je(D,t,120),s=`data:image/svg+xml;charset=utf-8,${encodeURIComponent(c)}`,a=Ze(n,{portraitUrl:s,clubLogoUrl:p,showNotes:!0}),l=document.getElementById("card-preview");l&&(l.innerHTML=a);const o=document.getElementById("avatar-preview-wrap");o&&(o.innerHTML=c);const u=e("cb-surname-real"),r=document.getElementById("encode-summary");r&&u&&(r.textContent=`${e("cb-firstname")} ${u} → ${e("cb-firstname")} ${n.surname_encoded}`)}function Oe(){const e=n=>{var t;return((t=document.getElementById(n))==null?void 0:t.value)||""};D={skin:e("av-skin"),hair:e("av-hair"),hairColor:e("av-hairColor"),eyes:e("av-eyes"),nose:e("av-nose"),mouth:e("av-mouth"),eyebrows:e("av-eyebrows"),beard:e("av-beard")}}function $t(e,n,t){var p,c,s,a,l;e.querySelectorAll("input,select").forEach(o=>{o.addEventListener("input",T),o.addEventListener("change",T)});const i=["av-skin","av-hair","av-hairColor","av-eyes","av-nose","av-mouth","av-eyebrows","av-beard"];i.forEach(o=>{var u;(u=document.getElementById(o))==null||u.addEventListener("change",()=>{Oe(),T()})}),(p=document.getElementById("cb-surname-real"))==null||p.addEventListener("input",()=>{_e(),T()}),(c=document.getElementById("btn-encode"))==null||c.addEventListener("click",()=>{_e(),T()}),(s=document.getElementById("cb-club"))==null||s.addEventListener("change",T),(a=document.getElementById("btn-save-player"))==null||a.addEventListener("click",()=>kt(t)),(l=document.getElementById("btn-reset"))==null||l.addEventListener("click",()=>{e.querySelectorAll("input").forEach(o=>o.value=""),e.querySelectorAll("select").forEach(o=>o.selectedIndex=0),D={...z},i.forEach(o=>{const u=o.replace("av-",""),r=document.getElementById(o);r&&(r.value=z[u]||"")}),T()})}async function kt(e){const n=s=>{var a;return((a=document.getElementById(s))==null?void 0:a.value)||""},t=n("cb-surname-real").trim(),i=n("cb-surname-enc").trim()||H(t);if(!n("cb-firstname")||!t||!n("cb-country")||!n("cb-job")){e("Remplissez les champs obligatoires (prénom, nom, pays, poste)","error");return}Oe();const p={firstname:n("cb-firstname").trim(),surname_real:t,surname_encoded:i,country_code:n("cb-country"),club_id:n("cb-club")||null,job:n("cb-job"),job2:n("cb-job2")||null,note_g:parseInt(n("cb-note-g"))||0,note_d:parseInt(n("cb-note-d"))||0,note_m:parseInt(n("cb-note-m"))||0,note_a:parseInt(n("cb-note-a"))||0,rarity:n("cb-rarity")||"normal",note_min:parseInt(n("cb-note-min"))||null,note_max:parseInt(n("cb-note-max"))||null,sell_price:parseInt(n("cb-price"))||0,avatar_config:D,skin:D.skin||"blanc",hair:D.hairColor||"marron",hair_length:"court"},{error:c}=await h.from("players").insert(p);if(c){e("Erreur: "+c.message,"error");return}e(`✅ Joueur "${p.firstname} ${p.surname_encoded}" enregistré !`,"success")}function It(e){const n=Et.map(([o,u])=>`<option value="${o}">${u} (${o})</option>`).join(""),t='<option value="">— Sélectionner un club —</option>'+e.map(o=>`<option value="${o.id}" data-logo="${o.logo_url||""}">${o.encoded_name}</option>`).join(""),i=we.map(o=>`<option value="${o}">${o}</option>`).join(""),p='<option value="">Aucun</option>'+we.map(o=>`<option value="${o}">${o}</option>`).join(""),c=ht.map(o=>`<option value="${o}">${o.charAt(0).toUpperCase()+o.slice(1)}</option>`).join(""),s=(o,u,r)=>Object.entries(u).map(([d,y])=>`<option value="${d}" ${d===(r||"")?"selected":""}>${y.label}</option>`).join(""),a=Object.keys(Je).map(o=>`<option value="${o}" ${o===z.skin?"selected":""}>${o.charAt(0).toUpperCase()+o.slice(1)}</option>`).join(""),l=Object.keys(Ye).map(o=>`<option value="${o}" ${o===z.hairColor?"selected":""}>${o.charAt(0).toUpperCase()+o.slice(1)}</option>`).join("");return`
  ...
  `,c(p),document.getElementById("search-users").addEventListener("input",s=>{const a=s.target.value.toLowerCase();c(p.filter(l=>{var o;return l.pseudo.toLowerCase().includes(a)||((o=l.club_name)==null?void 0:o.toLowerCase().includes(a))}))});function c(s){document.getElementById("users-tbody").innerHTML=s.map(a=>{const l=a.mmr??1e3,o=a.mmr_deviation??350,u=a.placement_matches??0,r=ot(l),d=a.ranked_wins??0,y=a.ranked_losses??0,f=a.ranked_draws??0,w=d+y+f,E=w>0?Math.round(d/w*100):null,m=u<10,_=o<80?"#1A6B3C":o<150?"#e67e22":"#bb2020";return`
        <tr>
          <td>
    ...
    <div class="card-panel">
      <div class="table-wrap">
        <table>
          ...
          <tbody>
            ${p.map(a=>{var r,d,y,f;const l=(r=a.card)==null?void 0:r.player,o=l?`${l.firstname} ${l.surname_encoded}`:((d=a.card)==null?void 0:d.card_type)||"—",u={active:"#1A6B3C",sold:"#D4A017",cancelled:"#aaa"};return`<tr>
                <td><b>${o}</b>${l?`<div style="font-size:10px;color:#999">${l.rarity} · ${l.job}</div>`:""}</td>
  ...
  `,document.querySelectorAll("[data-cancel]").forEach(a=>{a.addEventListener("click",async()=>{if(!confirm("Annuler cette annonce ?"))return;const{error:l}=await h.from("market_listings").update({status:"cancelled"}).eq("id",a.dataset.cancel);l?n(l.message,"error"):(n("Annonce annulée","success"),He(e,{toast:n}))})})}async function Lt(e,{toast:n}){e.innerHTML=`
...
`).replace(/\r/g,`
`);const n=e.split(`
`).filter(s=>s.trim());if(n.length<2)return[];const t=n[0],i=t.split(";").length>t.split(",").length?";":",",p=Ie(t,i).map(s=>s.trim().replace(/^\uFEFF/,"").toLowerCase()),c=[];for(let s=1;s<n.length;s++){if(!n[s].trim())continue;const a=Ie(n[s],i),l={};p.forEach((o,u)=>{l[o]=(a[u]||"").trim()}),!Object.values(l).every(o=>!o)&&c.push(l)}return c}function Ie(e,n=","){const t=[];let i="",p=!1;for(let c=0;c<e.length;c++){const s=e[c];p?s==='"'?e[c+1]==='"'?(i+='"',c++):p=!1:i+=s:s==='"'?p=!0:s===n?(t.push(i),i=""):i+=s}return t.push(i),t}const Bt={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};async function zt(e,{toast:n}){const t=Object.keys(de);let i=t[0];const{data:p}=await h.from("formation_links_overrides").select("formation, links"),c={};(p||[]).forEach(u=>{c[u.formation]=u.links});let s=new Set;function a(u,r){return[u,r].sort().join("-")}function l(u){const r=c[u]||de[u]||[];s=new Set(r.map(([d,y])=>a(d,y)))}l(i),e.innerHTML=`
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;gap:12px;flex-wrap:wrap">
  ...
  `;function o(){const u=at[i]||{},r=ve(i),d=320,y=400,f=22;function w(v){const g=u[v];return g?{x:g.x*d,y:g.y*y}:null}let E=`<svg width="${d}" height="${y}" viewBox="0 0 ${d} ${y}" xmlns="http://www.w3.org/2000/svg" style="display:block">`;r.forEach(([v,g],b)=>{const x=w(v),k=w(g);if(!x||!k)return;const L=a(v,g),I=s.has(L),S=I?"#3b82f6":"#999",M=I?.95:.35,j=I?4:3;E+=`<line x1="${x.x}" y1="${x.y}" x2="${k.x}" y2="${k.y}"
        ...
        opacity="${M}" pointer-events="none" data-line-key="${L}"/>`});for(const v of Object.keys(u)){const g=w(v);if(!g)continue;const b=v.replace(/\d+/,""),x=Bt[b]||"#555";E+=`<circle cx="${g.x}" cy="${g.y}" r="${f}" fill="${x}" stroke="rgba(255,255,255,0.7)" stroke-width="2" pointer-events="none"/>`,E+=`<text x="${g.x}" y="${g.y+4}" text-anchor="middle" font-size="10" font-weight="900" fill="white" font-family="Arial Black,Arial" pointer-events="none">${v}</text>`}E+="</svg>",document.getElementById("field-wrap").innerHTML=E;const m=document.getElementById("links-list");m.innerHTML=r.map(([v,g])=>{const b=a(v,g),x=s.has(b);return`
        ...
        </button>`}).join("");const _=e.querySelector('div[style*="font-weight:700;font-size:13px;margin-bottom:8px"]');_&&(_.textContent=`Liens (${s.size}/${r.length} actifs)`);function $(v){s.has(v)?s.delete(v):s.add(v),o()}e.querySelectorAll(".link-hit").forEach(v=>{v.addEventListener("click",()=>$(v.dataset.key))}),e.querySelectorAll(".link-toggle").forEach(v=>{v.addEventListener("click",()=>$(v.dataset.key))})}o(),document.getElementById("formation-select").addEventListener("change",u=>{i=u.target.value,l(i),o()}),document.getElementById("reset-btn").addEventListener("click",()=>{const u=de[i]||[];s=new Set(u.map(([r,d])=>a(r,d))),o(),n("Liens réinitialisés aux valeurs par défaut (non sauvegardé)","info")}),document.getElementById("save-btn").addEventListener("click",async()=>{const r=ve(i).filter(([y,f])=>s.has(a(y,f))),{error:d}=await h.from("formation_links_overrides").upsert({formation:i,links:r,updated_at:new Date().toISOString()});if(d){n(d.message,"error");return}c[i]=r,n(`Liens enregistrés pour ${i} (${r.length} actifs)`,"success")})}const At=[{value:"player",label:"Joueur"},{value:"formation",label:"Formation"},{value:"game_changer",label:"Game Changer"},{value:"ultra_game_changer",label:"Ultra Game Changer"},{value:"game_helper",label:"Game Helper"},{value:"stadium",label:"🏟️ Stade"}],St=[{value:"",label:"Toutes"},{value:"normal",label:"Normal"},{value:"pepite",label:"Pépite"},{value:"papyte",label:"Papyte"},{value:"legende",label:"Légende"}],Ct=[{value:"credits",label:"Crédits"},{value:"pub",label:"Publicité"},{value:"free",label:"Gratuit"}];async function Tt(e){e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">Chargement...</div>';const{data:n}=await h.from("booster_configs").select("*").order("sort_order");let t=null,i=[],p=null;const c=()=>window.innerWidth<700;async function s(){if(!t){i=[];return}if(t!==p){const{data:r}=await h.from("booster_drop_rates").select("*").eq("booster_id",t).order("sort_order");i=r||[],p=t}}function a(){return`
    ...
    </div>`}function l(){const r=t?(n||[]).find(f=>f.id===t):null;if(!r)return`<div style="display:flex;align-items:center;justify-content:center;flex:1;color:#bbb;font-size:14px;padding:40px;text-align:center">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
          <div>
            ...
            <select id="f-price-type" style="width:100%;padding:9px;border:1px solid #ddd;border-radius:6px;font-size:13px">
              ${Ct.map(f=>`<option value="${f.value}" ${r.price_type===f.value?"selected":""}>${f.label}</option>`).join("")}
            </select>
          </div>
    ...
    </div>`,document.getElementById("add-article").addEventListener("click",()=>Be(null)),document.getElementById("form-cancel").addEventListener("click",()=>{document.getElementById("article-form").style.display="none"}),document.getElementById("form-save").addEventListener("click",()=>Pt(e)),e.querySelectorAll("[data-edit]").forEach(t=>{const i=(n||[]).find(p=>p.id===t.dataset.edit);i&&t.addEventListener("click",()=>Be(i))}),e.querySelectorAll("[data-delete]").forEach(t=>{t.addEventListener("click",async()=>{confirm("Supprimer cet article ?")&&(await h.from("patch_notes").delete().eq("id",t.dataset.delete),await pe(e))})})}function Dt(e){const n=new Date(e.published_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"short",year:"numeric"});return`
    ...
    </div>`}function Be(e){const n=document.getElementById("article-form");n.style.display="block",document.getElementById("form-title").textContent=e?"Modifier l'article":"Nouvel article",document.getElementById("form-id").value=(e==null?void 0:e.id)||"",document.getElementById("form-art-title").value=(e==null?void 0:e.title)||"",document.getElementById("form-art-desc").value=(e==null?void 0:e.description)||"",document.getElementById("form-art-img").value=(e==null?void 0:e.image_url)||"",document.getElementById("form-art-pub").checked=e?e.is_published:!0;const t=new Date((e==null?void 0:e.published_at)||Date.now()),i=new Date(t.getTime()-t.getTimezoneOffset()*6e4).toISOString().slice(0,16);document.getElementById("form-art-date").value=i,n.scrollIntoView({behavior:"smooth"})}async function Pt(e){const n=document.getElementById("form-id").value,t=document.getElementById("form-art-title").value.trim(),i=document.getElementById("form-art-desc").value.trim(),p=document.getElementById("form-art-img").value.trim()||null,c=document.getElementById("form-art-pub").checked,s=document.getElementById("form-art-date").value;if(!t){alert("Le titre est obligatoire.");return}if(!i){alert("La description est obligatoire.");return}const a=s?new Date(s).toISOString():new Date().toISOString(),l={title:t,description:i,image_url:p,is_published:c,published_at:a};let o;if(n?{error:o}=await h.from("patch_notes").update(l).eq("id",n):{error:o}=await h.from("patch_notes").insert(l),o){alert("Erreur : "+o.message);return}document.getElementById("article-form").style.display="none",await pe(e)}async function qt(e){await F(e)}async function F(e){var u,r,d,y,f,w,E,m,_,$,v,g;const{data:n}=await h.from("tutorial_steps").select("*").order("step_order");e.innerHTML=`
    ...
    </div>`,(u=document.getElementById("ts-add"))==null||u.addEventListener("click",()=>ze(null,(n==null?void 0:n.length)||0)),(r=document.getElementById("ts-reset"))==null||r.addEventListener("click",async()=>{if(!confirm("Remettre tutorial_done = false pour TOUS les joueurs ? Ils reverront le tutoriel à leur prochaine connexion."))return;const{error:b}=await h.from("users").update({tutorial_done:!1}).neq("id","00000000-0000-0000-0000-000000000000");alert(b?"Erreur : "+b.message:"✅ Tutorial_done réinitialisé pour tous les joueurs.")}),(d=document.getElementById("ts-cancel"))==null||d.addEventListener("click",Ue),(y=document.getElementById("ts-save"))==null||y.addEventListener("click",()=>Ht(e)),(f=document.getElementById("ts-preview-btn"))==null||f.addEventListener("click",Ft);const t=document.getElementById("ts-editor"),i=()=>{const b=document.getElementById("ts-content");b&&t&&(b.value=t.innerHTML)};t==null||t.addEventListener("input",i),t==null||t.addEventListener("blur",i);const p=()=>{document.activeElement!==t&&(t==null||t.focus())};(w=document.getElementById("ts-toolbar"))==null||w.querySelectorAll("[data-cmd]").forEach(b=>{b.addEventListener("mousedown",x=>{x.preventDefault(),p();const k=b.dataset.cmd;k.startsWith("formatBlock:")?document.execCommand("formatBlock",!1,k.split(":")[1]):document.execCommand(k,!1,null),i()})}),(E=document.getElementById("ts-insert-color"))==null||E.addEventListener("mousedown",b=>{b.preventDefault(),p();const x=prompt("Couleur hex (ex: #D4A017 pour or, #1A6B3C pour vert, #cc2222 pour rouge) :");x&&(document.execCommand("foreColor",!1,x),i())}),(m=document.getElementById("ts-clear-format"))==null||m.addEventListener("mousedown",b=>{b.preventDefault(),p(),document.execCommand("removeFormat",!1,null),i()}),t==null||t.addEventListener("focus",()=>{t&&(t.style.borderColor="#1A6B3C")}),t==null||t.addEventListener("blur",()=>{t&&(t.style.borderColor="#ddd")});const c=document.getElementById("ts-image"),s=document.getElementById("ts-img-preview"),a=document.getElementById("ts-img-preview-el"),l=document.getElementById("ts-img-picker-grid"),o=()=>{var x;const b=(x=c==null?void 0:c.value)==null?void 0:x.trim();if(b){const k="/";a.src=`${k}icons/${b}`,s.style.display="block"}else s.style.display="none"};c==null||c.addEventListener("input",o),(_=document.getElementById("ts-img-clear"))==null||_.addEventListener("click",()=>{c&&(c.value=""),s.style.display="none",l.style.display="none"}),($=document.getElementById("ts-img-pick"))==null||$.addEventListener("click",async()=>{if(l.style.display!=="none"){l.style.display="none";return}l.innerHTML='<div style="color:#aaa;font-size:12px">Chargement…</div>',l.style.display="block";try{const x=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),k=Array.isArray(x)?x.filter(I=>I.name.startsWith("tuto_")):[];if(!k.length){l.innerHTML='<div style="color:#aaa;font-size:12px">Aucun fichier tuto_* trouvé dans /icons/.<br>Uploade des images nommées tuto_xxx.png dans public/icons/</div>';return}const L="/";l.innerHTML='<div style="display:flex;flex-wrap:wrap;gap:8px">'+k.map(I=>`
          ...
          <div style="flex:1;min-width:220px;display:flex;flex-direction:column;gap:12px">
            ...
            <div>
              ...
              <select id="st-country" style="width:100%;box-sizing:border-box;padding:9px;border:1.5px solid #ddd;border-radius:8px;font-size:13px;background:#fff">
                ...
                ${Ut.map(([a,l])=>`<option value="${a}">${l} (${a})</option>`).join("")}
              </select>
            </div>
            <div>
    ...
    <div style="display:flex;flex-direction:column;gap:20px">
      ...
      <div class="card-panel" style="background:linear-gradient(135deg,#0a3d1e,#1a6b3c20);border:1.5px solid #1A6B3C40">
        ...
        <div style="display:flex;flex-direction:column;gap:4px">
          ${d.map(g=>{const b=E[g],x=r.length>0?Math.round(b/r.length*100):0;return`
        ...
        <div class="table-wrap">
          <table>
            ...
            <tbody id="seasons-tbody">
              ${l.length===0?'<tr><td colspan="6" style="text-align:center;color:var(--gray-600);padding:24px">Aucune saison créée.</td></tr>':l.map(g=>{const b=Math.round((new Date(g.end_at)-new Date(g.start_at))/864e5),x=new Date(g.end_at)<new Date&&!g.is_active;return`
                    <tr>
                      <td><b>${g.name}</b></td>
      ...
      <div class="form-group">
        ...
        <input id="season-start" type="datetime-local" value="${l}" style="width:100%">
      </div>
      <div class="form-group">
