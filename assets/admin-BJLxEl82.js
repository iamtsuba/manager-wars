import{s as _,r as Ee,a as _e,A as ht,l as Et,g as _t,K as Me,D as z,b as wt,S as It,c as R,H as $t,R as kt,e as ue,d as Xe,f as Lt,h as Bt,E as At,i as St,N as Tt,M as Ct,B as Mt,j as zt,F as ge,k as ze,m as Rt,n as Qe,o as ye,p as jt}from"./special-cards-CZxqvl_5.js";async function Dt(e){var E;const[{count:t},{count:n},{count:i},{count:d},{count:r},{data:g}]=await Promise.all([_.from("players").select("*",{count:"exact",head:!0}),_.from("clubs").select("*",{count:"exact",head:!0}),_.from("users").select("*",{count:"exact",head:!0}),_.from("stadium_definitions").select("*",{count:"exact",head:!0}),_.from("users").select("*",{count:"exact",head:!0}).gt("last_seen_at",new Date(Date.now()-18e4).toISOString()),_.rpc("get_signup_password")]),x=g||"";e.innerHTML=`
  ...
  `,window.adminNav=a=>{var c;(c=document.querySelector(`[data-page="${a}"]`))==null||c.click()},(E=document.getElementById("signup-pwd-save"))==null||E.addEventListener("click",async()=>{const a=document.getElementById("signup-pwd-input").value.trim(),c=document.getElementById("signup-pwd-status");if(!a){c.textContent="Le code ne peut pas être vide.",c.style.color="#ff6b6b";return}const o=document.getElementById("signup-pwd-save");o.disabled=!0,o.textContent="⏳...";const{error:l}=await _.rpc("set_signup_password",{new_password:a});if(o.disabled=!1,o.textContent="💾 Enregistrer",l){c.textContent=l.message,c.style.color="#ff6b6b";return}c.textContent="✅ Code mis à jour.",c.style.color="#2ecc71"})}const et={normal:"Normal",pepite:"Pépite",papyte:"Papyte",legende:"Légende"},Pt={GK:"#aaaaaa",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};async function Nt(){const e={};for(const t of ht)e[t]=await Et(t);return e}async function Ft(e,t){const{toast:n}=t;if(!confirm(`Réattribuer une nouvelle photo à tous les joueurs ayant encore l'ancien format de chemin (public/faces/...) ?

Cette action est irréversible.`))return;const i=document.getElementById("fix-old-faces-btn");i&&(i.disabled=!0,i.textContent="⏳ En cours...");try{const{data:d,error:r}=await _.from("players").select("id, country_code, face").like("face","public/faces/%");if(r){n(r.message,"error");return}if(!(d!=null&&d.length)){n("Aucun joueur avec l'ancien format — rien à faire ✅","success");return}const{data:g}=await _.from("players").select("face").not("face","is",null),x=new Set((g||[]).map(c=>c.face).filter(c=>c&&!c.startsWith("public/faces/")));let E=0,a=0;for(const c of d){const o=await _e(c.country_code,x);if(!o){a++;continue}const{error:l}=await _.from("players").update({face:o}).eq("id",c.id);if(l){a++;continue}x.add(o),E++}n(`${E} photo(s) réattribuée(s)${a?`, ${a} échec(s)`:""} ✅`,a?"error":"success")}finally{i&&(i.disabled=!1,i.textContent="🖼️ Réattribuer anciennes photos")}}async function Ot(e,t){await se(e,t)}async function se(e,t,n=null){var a,c,o,l,m;const{toast:i}=t;n||(n={search:((a=document.getElementById("search-players"))==null?void 0:a.value)||"",job:((c=document.getElementById("filter-job"))==null?void 0:c.value)||"",rarity:((o=document.getElementById("filter-rarity"))==null?void 0:o.value)||"",club:((l=document.getElementById("filter-club"))==null?void 0:l.value)||"",country:((m=document.getElementById("filter-country"))==null?void 0:m.value)||""});const[{data:d,error:r},{data:g}]=await Promise.all([_.from("players").select("*, clubs(id,encoded_name,logo_url)"),_.from("clubs").select("id,encoded_name").order("encoded_name")]);if(r){e.innerHTML=`<p style="color:red">${r.message}</p>`;return}const x={GK:0,DEF:1,MIL:2,ATT:3},E=(d||[]).sort((p,f)=>{const u=(x[p.job]??4)-(x[f.job]??4);return u!==0?u:(p.surname_real||"").localeCompare(f.surname_real||"")});Ut(e,E,g||[],t,n)}function Ut(e,t,n,i,d=null){var c,o;const{toast:r}=i;if(e.innerHTML=`
    <div style="display:flex;flex-direction:column;gap:10px">
    ...
    </div>`,d){const l=d,m=p=>document.getElementById(p);l.search&&m("search-players")&&(m("search-players").value=l.search),l.job&&m("filter-job")&&(m("filter-job").value=l.job),l.rarity&&m("filter-rarity")&&(m("filter-rarity").value=l.rarity),l.club&&m("filter-club")&&(m("filter-club").value=l.club),l.country&&m("filter-country")&&(m("filter-country").value=l.country)}if(d){const l=d,m=p=>document.getElementById(p);l.search&&m("search-players")&&(m("search-players").value=l.search),l.job&&m("filter-job")&&(m("filter-job").value=l.job),l.rarity&&m("filter-rarity")&&(m("filter-rarity").value=l.rarity),l.club&&m("filter-club")&&(m("filter-club").value=l.club),l.country&&m("filter-country")&&(m("filter-country").value=l.country)}function g(){const l=document.getElementById("search-players").value.toLowerCase(),m=document.getElementById("filter-job").value,p=document.getElementById("filter-rarity").value,f=document.getElementById("filter-club").value,u=document.getElementById("filter-country").value;return t.filter(b=>(!l||`${b.firstname} ${b.surname_real}`.toLowerCase().includes(l))&&(!m||b.job===m)&&(!p||b.rarity===p)&&(!f||b.club_id===f)&&(!u||b.country_code===u))}const x=new Set;function E(){const l=document.getElementById("bulk-bar"),m=document.getElementById("bulk-count");l&&(l.style.display=x.size>0?"flex":"none",m&&(m.textContent=`${x.size} joueur(s) sélectionné(s)`))}function a(){const l=g();document.getElementById("count-label").textContent=`${l.length} joueur(s)`;const m=document.getElementById("players-list");if(!l.length){m.innerHTML='<div style="color:var(--gray-600);padding:20px;text-align:center">Aucun joueur.</div>';return}m.innerHTML=l.map(p=>{const f={...p,clubs:p.clubs,face:p.face||null},u=Ee(f,{width:120});return`<div style="position:relative;cursor:pointer" data-edit="${p.id}">
        ${u}
      ...
      </div>`}).join(""),m.querySelectorAll("[data-edit]").forEach(p=>{p.addEventListener("click",()=>{const f=t.find(u=>u.id===p.dataset.edit);f&&Re(f,n,e,i)})}),m.querySelectorAll(".btn-del-player").forEach(p=>{p.addEventListener("click",async f=>{if(f.stopPropagation(),!confirm("Supprimer ce joueur ?"))return;const{error:u}=await _.from("players").delete().eq("id",p.dataset.del);u?r(u.message,"error"):(r("Joueur supprimé ✅","success"),se(e,i))})})}a(),document.getElementById("search-players").addEventListener("input",a),document.getElementById("filter-job").addEventListener("change",a),document.getElementById("filter-rarity").addEventListener("change",a),document.getElementById("filter-club").addEventListener("change",a),document.getElementById("filter-country").addEventListener("change",a),(c=document.getElementById("bulk-cancel-btn"))==null||c.addEventListener("click",()=>{x.clear(),E(),a()}),(o=document.getElementById("bulk-delete-btn"))==null||o.addEventListener("click",async()=>{var f,u,b,L,k;if(!x.size||!confirm(`Supprimer ${x.size} joueur(s) ?`))return;const l=[...x],{error:m}=await _.from("players").delete().in("id",l);if(m){r(m.message,"error");return}r(`${l.length} joueur(s) supprimé(s) ✅`,"success"),x.clear();const p={search:((f=document.getElementById("search-players"))==null?void 0:f.value)||"",job:((u=document.getElementById("filter-job"))==null?void 0:u.value)||"",rarity:((b=document.getElementById("filter-rarity"))==null?void 0:b.value)||"",club:((L=document.getElementById("filter-club"))==null?void 0:L.value)||"",country:((k=document.getElementById("filter-country"))==null?void 0:k.value)||""};se(e,i,p)}),document.getElementById("add-player-btn").addEventListener("click",()=>Re(null,n,e,i)),document.getElementById("fix-old-faces-btn").addEventListener("click",()=>Ft(e,i))}async function Re(e,t,n,i){const{toast:d,openModal:r,closeModal:g}=i,x=!!e,E=await Nt(),{data:a}=await _.from("players").select("face").not("face","is",null),c=new Set((a||[]).map(p=>p.face).filter(Boolean));e!=null&&e.face&&c.delete(e.face);const o='<option value="">— Club —</option>'+t.map(p=>`<option value="${p.id}" ${(e==null?void 0:e.club_id)===p.id?"selected":""}>${p.encoded_name}</option>`).join(""),l=e!=null&&e.face?e.face.split("/")[0]:"",m=Object.keys(E).map(p=>`<option value="${p}" ${l===p?"selected":""}>${p}</option>`).join("");r(x?`✏️ ${e.firstname} ${e.surname_real}`:"➕ Nouveau joueur",`<div style="display:flex;gap:20px;align-items:flex-start">
      ...
      <div style="flex:1;min-width:300px;display:flex;flex-direction:column;gap:12px">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:8px">
          ...
          <div class="form-group">
            <label>Rareté</label>
            <select id="pm-rarity">
              ${["normal","pepite","papyte","legende"].map(p=>`<option value="${p}" ${(e==null?void 0:e.rarity)===p?"selected":""}>${et[p]}</option>`).join("")}
            </select>
          </div>
          <div class="form-group">
            <label>Pays</label>
        ...
        <div style="border-top:1px solid var(--gray-200);padding-top:10px">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:8px">
            ${[["GK","pm-g","note_g"],["DEF","pm-d","note_d"],["MIL","pm-m","note_m"],["ATT","pm-a","note_a"]].map(([p,f,u])=>`
              <div class="form-group" style="text-align:center">
                <label style="color:${Pt[p]};font-weight:700">${p}</label>
                <input id="${f}" type="number" min="0" max="20" value="${(e==null?void 0:e[u])??0}" style="text-align:center;font-size:18px;font-weight:900;padding:6px 4px">
              </div>`).join("")}
          </div>
    ...
    </div>`,g(re),document.getElementById("search-clubs").addEventListener("input",x=>{const E=x.target.value.toLowerCase();g(re.filter(a=>a.real_name.toLowerCase().includes(E)||a.encoded_name.toLowerCase().includes(E)))}),document.getElementById("add-club-btn").addEventListener("click",()=>je(null,e,t));function g(x){const E=document.getElementById("clubs-list");if(!x.length){E.innerHTML='<p style="color:var(--tile-fg-dim);padding:20px;text-align:center">Aucun club.</p>';return}const a={};x.forEach(l=>{const m=l.country_code||"—";a[m]||(a[m]=[]),a[m].push(l)});const c=Object.keys(a).sort(),o=l=>{const m=dt(l),p=l.logo_url?`<img src="${l.logo_url}" style="width:36px;height:36px;object-fit:contain;border-radius:8px;flex-shrink:0">`:`<div style="width:36px;height:36px;background:linear-gradient(135deg,${m.color1},${m.color2});border-radius:8px;display:flex;align-items:center;justify-content:center;color:#fff;font-size:9px;font-weight:900;flex-shrink:0">${l.encoded_name.slice(0,3)}</div>`,f=n[l.id]||0;return`
    ...
    `).join(""),E.querySelectorAll(".club-row").forEach(l=>{l.addEventListener("click",()=>{const m=re.find(p=>p.id===l.dataset.clubId);m&&je(m,e,t),E.querySelectorAll(".club-row").forEach(p=>p.style.background="var(--tile-bg)"),l.style.background="rgba(26,107,60,0.18)"})}),E.querySelectorAll(".btn-del-club").forEach(l=>{l.addEventListener("click",async()=>{if(!confirm("Supprimer ce club ?"))return;const{error:m}=await _.from("clubs").delete().eq("id",l.dataset.del);m?i(m.message,"error"):(i("Club supprimé","success"),me(e,t))})})}}function tn(e){var b,L;const{openModal:t,closeModal:n}=e,i=6,d=40,r=d*i,g=[];for(let k=0;k<i;k++)for(let y=0;y<i;y++)g.push((k+y)%2===0?"#00AEEF":"#ffffff");let x="OP",E="#D4A017",a=50,c=0,o=0;function l(){let k="";for(let v=0;v<i;v++)for(let s=0;s<i;s++){const I=v*i+s;k+=`<rect x="${s*d}" y="${v*d}" width="${d}" height="${d}" fill="${g[I]}"/>`}const y=r/2+c/100*r,w=r/2+o/100*r,h=a/100*r;return`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${r} ${r}">
      ${k}
    ...
    </svg>`}function m(k){return"data:image/svg+xml;base64,"+btoa(unescape(encodeURIComponent(k)))}const p=`
    <div style="display:flex;flex-direction:column;gap:14px;align-items:center;max-height:70vh;overflow-y:auto;padding-right:4px">
      ...
      <div style="width:260px">
        <label>TAILLE DU TEXTE (<span id="flag-size-val">${a}</span>%)</label>
        <input type="range" id="flag-text-size" min="10" max="120" value="${a}" style="width:100%">
      </div>
      <div style="width:260px">
        <label>POSITION HORIZONTALE (<span id="flag-x-val">${c}</span>)</label>
        <input type="range" id="flag-text-x" min="-50" max="50" value="${c}" style="width:100%">
      </div>
      <div style="width:260px">
        <label>POSITION VERTICALE (<span id="flag-y-val">${o}</span>)</label>
        <input type="range" id="flag-text-y" min="-50" max="50" value="${o}" style="width:100%">
      </div>
  ...
  `);function u(){document.getElementById("flag-preview").innerHTML=l()}u(),document.getElementById("flag-text").addEventListener("input",k=>{x=k.target.value.toUpperCase().slice(0,3),k.target.value=x,u()}),document.getElementById("flag-text-color").addEventListener("input",k=>{E=k.target.value,u()}),document.getElementById("flag-text-size").addEventListener("input",k=>{a=parseInt(k.target.value),document.getElementById("flag-size-val").textContent=a,u()}),document.getElementById("flag-text-x").addEventListener("input",k=>{c=parseInt(k.target.value),document.getElementById("flag-x-val").textContent=c,u()}),document.getElementById("flag-text-y").addEventListener("input",k=>{o=parseInt(k.target.value),document.getElementById("flag-y-val").textContent=o,u()}),document.querySelectorAll(".flag-square-color").forEach(k=>{k.addEventListener("input",y=>{g[parseInt(y.target.dataset.i)]=y.target.value,u()})}),(b=document.getElementById("flag-cancel"))==null||b.addEventListener("click",()=>n()),(L=document.getElementById("flag-use"))==null||L.addEventListener("click",()=>{const k=m(l()),y=document.getElementById("m-logo-url-current");y&&(y.value=k);const w=document.getElementById("logo-preview");w&&(w.innerHTML=`<img src="${k}" style="width:100%;height:100%;object-fit:contain">`),n()})}async function je(e,t,n){var u,b,L,k,y,w,h;const{toast:i}=n,d=!!e,r=e?dt(e):{...z},g=document.getElementById("club-panel");if(!g)return;const x=Object.entries(Me).map(([v,s])=>`<option value="${v}" ${r.style===v?"selected":""}>${s.label}</option>`).join(""),E=we.map(v=>`<option value="${v}" ${((e==null?void 0:e.country_code)||"FR")===v?"selected":""}>${v}</option>`).join("");g.style.display="block",g.style.alignItems="",g.style.justifyContent="",g.innerHTML=`
      ...
      <div style="border-top:1px solid var(--tile-border);padding-top:12px">
        ...
        <div style="display:flex;gap:12px;align-items:flex-start;flex-wrap:wrap">
          <div style="flex:1;min-width:200px;display:flex;flex-direction:column;gap:8px">
            ...
            ${[["Couleur 1","m-kit-color1",r.color1,!1],["Couleur 2","m-kit-color2",r.color2,!1],["Couleur 3","m-kit-color3",r.color3,!0],["Short","m-kit-shorts",r.shorts,!1],["Chaussettes","m-kit-socks",r.socks,!1]].map(([v,s,I,$])=>`
              <div class="form-group" id="wrap-${s}" ${$?'style="display:none"':""}>
                <label>${v}</label>
                <div style="display:flex;gap:6px;align-items:center">
                  <input type="color" id="${s}" value="${I||"#000000"}" style="width:38px;height:32px;padding:2px;border:1px solid var(--tile-border);border-radius:6px;cursor:pointer;flex-shrink:0">
                  <input id="${s}-txt" value="${I||"#000000"}" maxlength="7" style="flex:1;font-family:monospace;font-size:12px">
                </div>
              </div>`).join("")}
          </div>
  ...
  `,(u=document.getElementById("m-logo-file"))==null||u.addEventListener("change",v=>{var $;const s=($=v.target.files)==null?void 0:$[0];if(!s)return;const I=new FileReader;I.onload=()=>{document.getElementById("logo-preview").innerHTML=`<img src="${I.result}" style="width:100%;height:100%;object-fit:contain">`},I.readAsDataURL(s)}),(b=document.getElementById("m-open-flag-builder"))==null||b.addEventListener("click",()=>{tn(n)}),ne();function a(){var $,B;const v=(($=document.getElementById("m-kit-style"))==null?void 0:$.value)||"uni",s=((B=Me[v])==null?void 0:B.colors)===3,I=document.getElementById("wrap-m-kit-color3");I&&(I.style.display=s?"":"none")}a(),["m-kit-color1","m-kit-color2","m-kit-color3","m-kit-shorts","m-kit-socks"].forEach(v=>{const s=document.getElementById(v),I=document.getElementById(v+"-txt");!s||!I||(s.addEventListener("input",()=>{I.value=s.value,ne()}),I.addEventListener("input",()=>{const $=I.value.trim();/^#[0-9a-fA-F]{6}$/.test($)&&(s.value=$,ne())}),I.addEventListener("change",()=>{let $=I.value.trim();$.startsWith("#")||($="#"+$),/^#[0-9a-fA-F]{6}$/.test($)&&(s.value=$,I.value=$,ne())}))}),(L=document.getElementById("m-kit-style"))==null||L.addEventListener("change",()=>{a(),ne()});const c=document.getElementById("m-real"),o=document.getElementById("m-encoded");function l(){if(!c||!o||o.value)return;const v=c.value.trim().split(/\s+/),s=v.length===1?v[0].toUpperCase().slice(0,6):v.filter(I=>I.length>2).map(I=>I[0].toUpperCase()).join("")||v[0].toUpperCase().slice(0,4);o.value=s}c==null||c.addEventListener("input",l),(k=document.getElementById("auto-encode-btn"))==null||k.addEventListener("click",()=>{o&&(o.value=""),l()}),(y=document.getElementById("m-save"))==null||y.addEventListener("click",()=>sn(e,d,t,n));const m=document.getElementById("m-gen-squad"),p=document.getElementById("m-gen-strong-label");function f(){if(p&&(p.style.display=m!=null&&m.checked?"flex":"none",!(m!=null&&m.checked))){const v=document.getElementById("m-gen-strong");v&&(v.checked=!1)}}m==null||m.addEventListener("change",f),f(),(w=document.getElementById("btn-gen-squad-panel"))==null||w.addEventListener("click",()=>{nn(e,n,t)}),(h=document.getElementById("btn-gen-one-player"))==null||h.addEventListener("click",()=>{on(e,n)}),d&&$e(e.id)}async function $e(e){const{data:t}=await _.from("players").select("*, clubs(encoded_name, logo_url, kit_style, kit_color1, kit_color2, kit_color3, kit_shorts, kit_socks)").eq("club_id",e).order("surname_real"),n=document.getElementById("club-players-grid"),i=document.getElementById("club-players-count");if(i&&(i.textContent=(t||[]).length),!!n){if(!(t!=null&&t.length)){n.innerHTML='<div style="color:var(--tile-fg-dim);padding:20px;font-size:13px">Aucun joueur dans ce club.</div>';return}n.innerHTML=t.map(d=>Ee(d,{width:100})).join("")}}function nn(e,t,n){const{openModal:i,closeModal:d,toast:r}=t;i(`⚽ Générer — ${e.real_name}`,`<div style="display:flex;flex-direction:column;gap:14px">
    ...
    </div>`),setTimeout(()=>{var g,x;(g=document.getElementById("qg-cancel"))==null||g.addEventListener("click",()=>d()),(x=document.getElementById("qg-ok"))==null||x.addEventListener("click",async()=>{var a;const E=((a=document.getElementById("qg-strong"))==null?void 0:a.checked)??!1;d(),r("Génération en cours…","info"),await st(e.id,e.country_code,r,E),r("Effectif généré ✅","success"),$e(e.id),me(n,t)})},50)}const ct=["GK","DEF","MIL","ATT"],an=[{value:"normal",label:"Normal"},{value:"pepite",label:"Pépite"},{value:"papyte",label:"Papyte"},{value:"legende",label:"Légende"}];function on(e,t){const{openModal:n,closeModal:i,toast:d}=t,r=we.map(a=>`<option value="${a}">${a}</option>`).join(""),g=a=>ct.map(c=>`<option value="${c}" ${c===a?"selected":""}>${c}</option>`).join(""),x=`
  ...
  `),setTimeout(()=>{var a,c;document.querySelectorAll('input[name="gen-one-mode"]').forEach(o=>{o.addEventListener("change",()=>{document.getElementById("gen-one-custom-fields").style.display=o.value==="custom"&&o.checked?"flex":"none"})}),(a=document.getElementById("gen-one-cancel"))==null||a.addEventListener("click",()=>i()),(c=document.getElementById("gen-one-ok"))==null||c.addEventListener("click",async()=>{var p,f,u,b,L;const o=((p=document.querySelector('input[name="gen-one-mode"]:checked'))==null?void 0:p.value)||"random",l=document.getElementById("gen-one-ok");l.disabled=!0,l.textContent="⏳ Génération…";const m={};o==="custom"&&(m.job=((f=document.getElementById("gen-one-job1"))==null?void 0:f.value)||null,m.job2=((u=document.getElementById("gen-one-job2"))==null?void 0:u.value)||null,m.rarity=((b=document.getElementById("gen-one-rarity"))==null?void 0:b.value)||null,m.country=((L=document.getElementById("gen-one-country"))==null?void 0:L.value)||null);try{await rn(e,m),d("Joueur généré ✅","success"),i(),$e(e.id)}catch(k){d("Erreur : "+k.message,"error"),l.disabled=!1,l.textContent="➕ Générer"}})},50)}async function rn(e,t={}){const n=t.country||(Math.random()<.5?e.country_code:it(e.country_code)),i=t.job||K(ct),d=t.job2||null,r=t.rarity||(()=>{const B=Math.random()*100;return B<1.5?"legende":B<3?"pepite":B<4.5?"papyte":"normal"})(),g=r==="pepite"?V(10,15):r==="papyte"?V(15,20):at(),x=d?Math.max(0,g-2):0;let E=0,a=0,c=0,o=0;i==="GK"&&(E=g),i==="DEF"&&(a=g),i==="MIL"&&(c=g),i==="ATT"&&(o=g),d==="GK"&&(E=x),d==="DEF"&&(a=x),d==="MIL"&&(c=x),d==="ATT"&&(o=x);const l=Ie(n),m=nt(n),p=K(tt),{data:f}=await _.from("players").select("surname_real").not("surname_real","is",null),u=new Set((f||[]).map(B=>B.surname_real).filter(Boolean)),b=lt(n),L=b.filter(B=>!u.has(B)),k=K(L.length?L:b),y={job:i,job2:d,firstname:rt(n),surname_real:k,country_code:n,club_id:e.id,note_g:E,note_d:a,note_m:c,note_a:o,skin:l,hair:m,hair_length:p,rarity:r,sell_price:0,ethnie:be(n)},{data:w}=await _.from("players").select("face").not("face","is",null),h=new Set((w||[]).map(B=>B.face).filter(Boolean)),v=await _e(n,h);v&&(y.face=v);const{data:s,error:I}=await _.from("players").insert(y).select("id").single();if(I)throw I;const{error:$}=await _.from("cards").insert({card_type:"player",player_id:s.id});$&&console.warn("[GenOnePlayer] Erreur carte:",$.message)}function pt(){var e,t,n,i,d,r;return{style:((e=document.getElementById("m-kit-style"))==null?void 0:e.value)||"uni",color1:((t=document.getElementById("m-kit-color1"))==null?void 0:t.value)||"#1A6B3C",color2:((n=document.getElementById("m-kit-color2"))==null?void 0:n.value)||"#ffffff",color3:((i=document.getElementById("m-kit-color3"))==null?void 0:i.value)||"#000000",shorts:((d=document.getElementById("m-kit-shorts"))==null?void 0:d.value)||"#111111",socks:((r=document.getElementById("m-kit-socks"))==null?void 0:r.value)||"#ffffff"}}function ne(){const e=document.getElementById("kit-preview-wrap");e&&(e.innerHTML=wt(pt(),"panel"))}async function ln(e,t){const n=(e.name.split(".").pop()||"png").toLowerCase(),i=`clubs/${t||"new"}-${Date.now()}.${n}`,{error:d}=await _.storage.from("assets").upload(i,e,{upsert:!0,cacheControl:"3600"});if(d)throw d;const{data:r}=_.storage.from("assets").getPublicUrl(i);return r.publicUrl}async function sn(e,t,n,i){var k,y,w,h,v,s,I,$,B;const{toast:d}=i,r=document.getElementById("m-error"),g=document.getElementById("m-save"),x=(k=document.getElementById("m-real"))==null?void 0:k.value.trim(),E=(y=document.getElementById("m-encoded"))==null?void 0:y.value.trim().toUpperCase(),a=(w=document.getElementById("m-country"))==null?void 0:w.value.trim().toUpperCase(),c=((v=(h=document.getElementById("m-logo-file"))==null?void 0:h.files)==null?void 0:v[0])||null,o=((s=document.getElementById("m-logo-url-current"))==null?void 0:s.value)||null,l=((I=document.getElementById("m-gen-stadium"))==null?void 0:I.checked)??!1,m=(($=document.getElementById("m-gen-squad"))==null?void 0:$.checked)??!1,p=((B=document.getElementById("m-gen-strong"))==null?void 0:B.checked)??!1,f=pt();if(!x){r.textContent="Le nom du club est requis.";return}if(!E){r.textContent="Le nom encodé est requis.";return}if(!a){r.textContent="Le pays est requis.";return}g.disabled=!0,g.textContent="Enregistrement…";let u=o;if(c)try{g.textContent="📤 Envoi du logo…",u=await ln(c,e==null?void 0:e.id)}catch(A){r.textContent="Erreur upload logo : "+A.message,g.disabled=!1,g.textContent=t?"💾 Enregistrer":"✅ Créer le club";return}const b={real_name:x,encoded_name:E,country_code:a,logo_url:u,kit_style:f.style,kit_color1:f.color1,kit_color2:f.color2,kit_color3:f.color3,kit_shorts:f.shorts,kit_socks:f.socks};let L=e==null?void 0:e.id;if(t){const{error:A}=await _.from("clubs").update(b).eq("id",L);if(A){r.textContent=A.message,g.disabled=!1,g.textContent="💾 Enregistrer";return}}else{const{data:A,error:S}=await _.from("clubs").insert(b).select().single();if(S){r.textContent=S.message,g.disabled=!1,g.textContent="✅ Créer le club";return}if(L=A.id,l){g.textContent="🏟️ Création du stade…";const{data:T,error:C}=await _.from("stadium_definitions").insert({name:`Stade de ${x}`,club_id:L,country_code:null}).select().single();C?console.warn("[Stadium] Erreur def stade:",C.message):T&&await _.from("cards").insert({card_type:"stadium",stadium_id:T.id})}m&&(g.textContent="⚽ Génération des joueurs…",await st(L,a,d,p))}d(t?"Club modifié ✅":"Club créé ✅","success"),me(n,i)}const dn=["normal","pepite","papyte","legende"],De=["GK","DEF","MIL","ATT"],cn=[["MA","Maroc"],["FR","France"],["AR","Argentine"],["PT","Portugal"],["BR","Brésil"],["ES","Espagne"],["DE","Allemagne"],["GB","Angleterre"],["IT","Italie"],["CM","Cameroun"],["SN","Sénégal"],["NG","Nigéria"],["DK","Danemark"],["NL","Pays-Bas"],["BE","Belgique"],["CI","Côte d'Ivoire"],["AL","Albanie"],["HR","Croatie"],["RS","Serbie"],["TR","Turquie"],["MW","Malawi"]];let J={...R},le=[];async function pn(e,{toast:t}){const{data:n}=await _.from("clubs").select("id,encoded_name,logo_url,kit_style,kit_color1,kit_color2,kit_shorts,kit_socks").order("encoded_name");le=n||[],J={...R},e.innerHTML=fn(le),mn(e,le,t),F()}function Pe(){var n;const e=((n=document.getElementById("cb-surname-real"))==null?void 0:n.value)||"",t=document.getElementById("cb-surname-enc");t&&(t.value=ue(e))}function un(){var n;const e=(n=document.getElementById("cb-club"))==null?void 0:n.value,t=le.find(i=>String(i.id)===String(e));return t?{style:t.kit_style||z.style,color1:t.kit_color1||z.color1,color2:t.kit_color2||z.color2,shorts:t.kit_shorts||z.shorts,socks:t.kit_socks||z.socks}:{...z}}function F(){var l,m,p,f;const e=u=>{var b;return((b=document.getElementById(u))==null?void 0:b.value)||""},t={firstname:e("cb-firstname"),surname_real:e("cb-surname-enc")||ue(e("cb-surname-real")),country_code:e("cb-country"),club_encoded_name:((m=(l=document.getElementById("cb-club"))==null?void 0:l.selectedOptions[0])==null?void 0:m.text)||"",job:e("cb-job"),job2:e("cb-job2")||null,note_g:parseInt(e("cb-note-g"))||0,note_d:parseInt(e("cb-note-d"))||0,note_m:parseInt(e("cb-note-m"))||0,note_a:parseInt(e("cb-note-a"))||0,rarity:e("cb-rarity")},n=un(),i=document.getElementById("cb-club"),d=((f=(p=i==null?void 0:i.selectedOptions[0])==null?void 0:p.dataset)==null?void 0:f.logo)||null,r=Xe(J,n,120),g=`data:image/svg+xml;charset=utf-8,${encodeURIComponent(r)}`,x=Lt(t,{portraitUrl:g,clubLogoUrl:d,showNotes:!0}),E=document.getElementById("card-preview");E&&(E.innerHTML=x);const a=document.getElementById("avatar-preview-wrap");a&&(a.innerHTML=r);const c=e("cb-surname-real"),o=document.getElementById("encode-summary");o&&c&&(o.textContent=`${e("cb-firstname")} ${c} → ${e("cb-firstname")} ${t.surname_real}`)}function ut(){const e=t=>{var n;return((n=document.getElementById(t))==null?void 0:n.value)||""};J={skin:e("av-skin"),hair:e("av-hair"),hairColor:e("av-hairColor"),eyes:e("av-eyes"),nose:e("av-nose"),mouth:e("av-mouth"),eyebrows:e("av-eyebrows"),beard:e("av-beard")}}function mn(e,t,n){var d,r,g,x,E;e.querySelectorAll("input,select").forEach(a=>{a.addEventListener("input",F),a.addEventListener("change",F)});const i=["av-skin","av-hair","av-hairColor","av-eyes","av-nose","av-mouth","av-eyebrows","av-beard"];i.forEach(a=>{var c;(c=document.getElementById(a))==null||c.addEventListener("change",()=>{ut(),F()})}),(d=document.getElementById("cb-surname-real"))==null||d.addEventListener("input",()=>{Pe(),F()}),(r=document.getElementById("btn-encode"))==null||r.addEventListener("click",()=>{Pe(),F()}),(g=document.getElementById("cb-club"))==null||g.addEventListener("change",F),(x=document.getElementById("btn-save-player"))==null||x.addEventListener("click",()=>gn(n)),(E=document.getElementById("btn-reset"))==null||E.addEventListener("click",()=>{e.querySelectorAll("input").forEach(a=>a.value=""),e.querySelectorAll("select").forEach(a=>a.selectedIndex=0),J={...R},i.forEach(a=>{const c=a.replace("av-",""),o=document.getElementById(a);o&&(o.value=R[c]||"")}),F()})}async function gn(e){const t=g=>{var x;return((x=document.getElementById(g))==null?void 0:x.value)||""},n=t("cb-surname-real").trim(),i=t("cb-surname-enc").trim()||ue(n);if(!t("cb-firstname")||!n||!t("cb-country")||!t("cb-job")){e("Remplissez les champs obligatoires (prénom, nom, pays, poste)","error");return}ut();const d={firstname:t("cb-firstname").trim(),surname_real:n,surname_real:i,country_code:t("cb-country"),club_id:t("cb-club")||null,job:t("cb-job"),job2:t("cb-job2")||null,note_g:parseInt(t("cb-note-g"))||0,note_d:parseInt(t("cb-note-d"))||0,note_m:parseInt(t("cb-note-m"))||0,note_a:parseInt(t("cb-note-a"))||0,rarity:t("cb-rarity")||"normal",note_min:parseInt(t("cb-note-min"))||null,note_max:parseInt(t("cb-note-max"))||null,sell_price:parseInt(t("cb-price"))||0,avatar_config:J,skin:J.skin||"blanc",hair:J.hairColor||"marron",hair_length:"court"},{error:r}=await _.from("players").insert(d);if(r){e("Erreur: "+r.message,"error");return}e(`✅ Joueur "${d.firstname} ${d.surname_real}" enregistré !`,"success")}function fn(e){const t=cn.map(([a,c])=>`<option value="${a}">${c} (${a})</option>`).join(""),n='<option value="">— Sélectionner un club —</option>'+e.map(a=>`<option value="${a.id}" data-logo="${a.logo_url||""}">${a.encoded_name}</option>`).join(""),i=De.map(a=>`<option value="${a}">${a}</option>`).join(""),d='<option value="">Aucun</option>'+De.map(a=>`<option value="${a}">${a}</option>`).join(""),r=dn.map(a=>`<option value="${a}">${a.charAt(0).toUpperCase()+a.slice(1)}</option>`).join(""),g=(a,c,o)=>Object.entries(c).map(([l,m])=>`<option value="${l}" ${l===(o||"")?"selected":""}>${m.label}</option>`).join(""),x=Object.keys(It).map(a=>`<option value="${a}" ${a===R.skin?"selected":""}>${a.charAt(0).toUpperCase()+a.slice(1)}</option>`).join(""),E=Object.keys($t).map(a=>`<option value="${a}" ${a===R.hairColor?"selected":""}>${a.charAt(0).toUpperCase()+a.slice(1)}</option>`).join("");return`
        ...
        <div style="display:grid;grid-template-columns:1fr 120px;gap:16px;align-items:start">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
            ...
            <div>
              ...
              <select id="av-eyebrows">${g("av-eyebrows",At,R.eyebrows)}</select>
            </div>
            <div>
              <label>Yeux</label>
  ...
  `}async function mt(e,{toast:t}){const[{data:n,error:i},{data:d},{data:r}]=await Promise.all([_.from("users").select("id,pseudo,club_name,credits,level,wins,draws,losses,trophies_top1,trophies_top2,trophies_top3,is_admin,created_at,mmr,mmr_deviation,rank_tier,placement_matches,ranked_wins,ranked_losses,ranked_draws").order("created_at",{ascending:!1}),_.from("user_solo_progress").select("user_id, unlocked_level"),_.rpc("admin_get_user_emails")]);if(i){e.innerHTML=`<p style="color:red">${i.message}</p>`;return}const g={};(d||[]).forEach(c=>{g[c.user_id]=c.unlocked_level});const x={};(r||[]).forEach(c=>{x[c.id]=c.email});const E=n||[];e.innerHTML=`
  ...
  `,a(E),document.getElementById("search-users").addEventListener("input",c=>{const o=c.target.value.toLowerCase();a(E.filter(l=>{var m;return l.pseudo.toLowerCase().includes(o)||((m=l.club_name)==null?void 0:m.toLowerCase().includes(o))}))});function a(c){document.getElementById("users-tbody").innerHTML=c.map(o=>{const l=o.mmr??1e3,m=o.mmr_deviation??350,p=o.placement_matches??0,f=zt(l),u=o.ranked_wins??0,b=o.ranked_losses??0,L=o.ranked_draws??0,k=u+b+L,y=k>0?Math.round(u/k*100):null,w=p<10,h=m<80?"#1A6B3C":m<150?"#e67e22":"#bb2020";return`
        <tr>
          <td>
            <div style="font-weight:700">${o.pseudo}</div>
            <div style="font-size:11px;color:var(--gray-600)">${o.club_name||"—"}</div>
            <div style="font-size:10.5px;color:var(--gray-600)">${x[o.id]||"—"}</div>
          </td>
          <td style="font-size:12px">
            ...
            <div style="color:var(--gray-600)">Niv. Solo ${g[o.id]??1}</div>
          </td>
          ...
          <td style="font-size:12px">
            ...
            ${w?`<div style="font-size:10px;color:#e67e22">${p}/10 placement${p<10?` (${10-p} restants)`:""}</div>`:`<div style="font-size:10px;color:var(--gray-600)">${u}V ${L}N ${b}D${y!==null?` · ${y}% WR`:""}</div>`}
          </td>
        ...
        </tr>`}).join(""),document.querySelectorAll(".btn-edit-credits").forEach(o=>{o.addEventListener("click",async()=>{const l=o.dataset.user,m=Number(o.dataset.current)||0,p=prompt("Nouveau solde de crédits :",m);if(p===null)return;const f=parseInt(p.replace(/\s/g,""),10);if(isNaN(f)||f<0){t("Valeur invalide","error");return}const{data:u,error:b}=await _.rpc("admin_update_user_credits",{p_user_id:l,p_new_credits:f});if(b){t(b.message,"error");return}if(!(u!=null&&u.success)){t((u==null?void 0:u.error)||"Échec de la mise à jour","error");return}t("Crédits mis à jour ✅","success");const L=document.getElementById(`credits-display-${l}`);L&&(L.textContent=`${f.toLocaleString("fr")} cr.`),o.dataset.current=f})}),document.querySelectorAll("[data-toggle-admin]").forEach(o=>{o.addEventListener("click",async()=>{const l=o.dataset.isAdmin!=="true";if(!confirm(`Voulez-vous ${l?"rendre admin":"retirer le rôle admin"} de ce Manager ?`))return;const{error:m}=await _.from("users").update({is_admin:l}).eq("id",o.dataset.toggleAdmin);m?t(m.message,"error"):(t("Rôle mis à jour ✅","success"),mt(e,{toast:t}))})})}}const yn=[{value:"normal",label:"Normal",color:"#ccc"},{value:"pepite",label:"Pépite",color:"#D4A017"},{value:"papyte",label:"Papyte",color:"#909090"},{value:"legende",label:"Légende",color:"#7a28b8"}];async function gt(e,t){var E;const{toast:n}=t,{data:i,error:d}=await _.from("market_listings").select(`
    ...
    `).order("listed_at",{ascending:!1}).limit(100);if(d){e.innerHTML=`<p style="color:var(--danger)">${d.message}</p>`;return}const r=i||[],g=r.filter(a=>a.status==="active").length,x=r.filter(a=>a.status==="sold").length;e.innerHTML=`
    ...
    <div class="card-panel">
      <div class="table-wrap">
        <table>
          ...
          <tbody>
            ${r.map(a=>{var m,p,f,u;const c=(m=a.card)==null?void 0:m.player,o=c?`${c.firstname} ${c.surname_real}`:((p=a.card)==null?void 0:p.card_type)||"—",l={active:"#1A6B3C",sold:"#D4A017",cancelled:"#888"};return`<tr>
                <td><b>${o}</b>${c?`<div style="font-size:10px;color:var(--tile-fg-dim)">${c.rarity} · ${c.job}</div>`:""}</td>
                <td style="font-size:12px">${((f=a.seller)==null?void 0:f.pseudo)||"—"}</td>
  ...
  `,document.querySelectorAll("[data-cancel]").forEach(a=>{a.addEventListener("click",async()=>{if(!confirm("Annuler cette annonce ?"))return;const{error:c}=await _.from("market_listings").update({status:"cancelled"}).eq("id",a.dataset.cancel);c?n(c.message,"error"):(n("Annonce annulée","success"),gt(e,t))})}),(E=document.getElementById("btn-price-grid"))==null||E.addEventListener("click",()=>bn(t))}async function bn(e){await ve(e)}function vn(){const e=document.getElementById("modal-overlay");if(!e||e._pgBlocker)return;const t=n=>{n.target===n.currentTarget&&n.stopImmediatePropagation()};e.addEventListener("click",t,!0),e._pgBlocker=t}function fe(){const e=document.getElementById("modal-overlay");e!=null&&e._pgBlocker&&(e.removeEventListener("click",e._pgBlocker,!0),delete e._pgBlocker)}async function ve(e){var m,p;const{openModal:t,closeModal:n,toast:i}=e,{data:d,error:r}=await _.from("sell_price_configs").select("*").order("rarity").order("note_min");if(r){i(r.message,"error");return}const g=f=>yn.map(u=>`<option value="${u.value}" ${u.value===f?"selected":""}>${u.label}</option>`).join(""),x=f=>`
    <tr data-row="${f.id}">
...
`}),ae("players_export.csv",d),t(`${n.length} joueurs exportés`,"success")}),document.getElementById("import-clubs").addEventListener("change",async n=>{const i=n.target.files[0];if(!i)return;const d=document.getElementById("clubs-import-result");d.innerHTML='<span style="color:var(--gray-600)">Import en cours...</span>';try{const r=await i.text(),g=Fe(r);if(g.length===0){d.innerHTML='<span style="color:#c0392b">Fichier vide</span>';return}let x=0,E=0;const a=[];for(const c of g){if(!c.real_name||!c.encoded_name||!c.country_code){E++,a.push(`Ligne ignorée (champs manquants): ${c.real_name||"?"}`);continue}const o={real_name:c.real_name,encoded_name:c.encoded_name.toUpperCase(),country_code:c.country_code.toUpperCase().slice(0,2),logo_url:c.logo_url||null},{error:l}=await _.from("clubs").upsert(o,{onConflict:"encoded_name"});l?(E++,a.push(`${c.encoded_name}: ${l.message}`)):x++}d.innerHTML=`<div style="color:var(--green)">✅ ${x} clubs importés</div>
        ${E>0?`<div style="color:#c0392b">❌ ${E} erreurs</div><div style="font-size:11px;color:var(--gray-600);max-height:100px;overflow-y:auto">${a.slice(0,10).join("<br>")}</div>`:""}`,t(`${x} clubs importés`,"success")}catch(r){d.innerHTML=`<span style="color:#c0392b">Erreur : ${r.message}</span>`}n.target.value=""}),document.getElementById("import-players").addEventListener("change",async n=>{const i=n.target.files[0];if(!i)return;const d=document.getElementById("players-import-result");d.innerHTML='<span style="color:var(--gray-600)">Import en cours...</span>';try{const r=await i.text(),g=Fe(r);if(g.length===0){d.innerHTML='<span style="color:#c0392b">Fichier vide</span>';return}const{data:x}=await _.from("clubs").select("id,encoded_name"),E={};(x||[]).forEach(b=>{E[b.encoded_name.toUpperCase()]=b.id});let a=0,c=0;const o=[],l=["GK","DEF","MIL","ATT"],m=["normal","pepite","papyte","legende"],p=["blanc","metisse","typ","noir"],f=["blond","marron","noir","chauve"],u=["rase","court","milong","long"];for(const b of g){if(!b.firstname||!b.surname_real||!b.country_code||!b.job){c++,o.push(`Ligne ignorée (champs requis manquants): ${b.firstname||"?"}`);continue}if(!l.includes(b.job)){c++,o.push(`${b.firstname}: job invalide "${b.job}"`);continue}const L=b.club_encoded_name&&E[b.club_encoded_name.toUpperCase()]||null,k={firstname:b.firstname,surname_real:b.surname_real,surname_real:b.surname_real||ue(b.surname_real),country_code:b.country_code.toUpperCase().slice(0,2),club_id:L,job:b.job,job2:l.includes(b.job2)?b.job2:null,note_g:parseInt(b.note_g)||0,note_d:parseInt(b.note_d)||0,note_m:parseInt(b.note_m)||0,note_a:parseInt(b.note_a)||0,rarity:m.includes(b.rarity)?b.rarity:"normal",note_min:b.note_min!==""&&b.note_min!=null?parseInt(b.note_min):null,note_max:b.note_max!==""&&b.note_max!=null?parseInt(b.note_max):null,skin:p.includes(b.skin)?b.skin:"blanc",hair:f.includes(b.hair)?b.hair:"noir",hair_length:u.includes(b.hair_length)?b.hair_length:"court",sell_price:parseInt(b.sell_price)||0},{error:y}=await _.from("players").insert(k);y?(c++,o.push(`${b.firstname} ${b.surname_real}: ${y.message}`)):a++}d.innerHTML=`<div style="color:var(--green)">✅ ${a} joueurs importés</div>
        ${c>0?`<div style="color:#c0392b">❌ ${c} erreurs</div><div style="font-size:11px;color:var(--gray-600);max-height:100px;overflow-y:auto">${o.slice(0,10).join("<br>")}</div>`:""}`,t(`${a} joueurs importés`,"success")}catch(r){d.innerHTML=`<span style="color:#c0392b">Erreur : ${r.message}</span>`}n.target.value=""})}function Ne(e){const t=String(e??"");return t.includes(",")||t.includes('"')||t.includes(`
...
`).replace(/\r/g,`
`);const t=e.split(`
`).filter(g=>g.trim());if(t.length<2)return[];const n=t[0],i=n.split(";").length>n.split(",").length?";":",",d=Oe(n,i).map(g=>g.trim().replace(/^\uFEFF/,"").toLowerCase()),r=[];for(let g=1;g<t.length;g++){if(!t[g].trim())continue;const x=Oe(t[g],i),E={};d.forEach((a,c)=>{E[a]=(x[c]||"").trim()}),!Object.values(E).every(a=>!a)&&r.push(E)}return r}function Oe(e,t=","){const n=[];let i="",d=!1;for(let r=0;r<e.length;r++){const g=e[r];d?g==='"'?e[r+1]==='"'?(i+='"',r++):d=!1:i+=g:g==='"'?d=!0:g===t?(n.push(i),i=""):i+=g}return n.push(i),n}const hn={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};async function En(e,{toast:t}){const n=Object.keys(ge);let i=n[0];const{data:d}=await _.from("formation_links_overrides").select("formation, links"),r={};(d||[]).forEach(c=>{r[c.formation]=c.links});let g=new Set;function x(c,o){return[c,o].sort().join("-")}function E(c){const o=r[c]||ge[c]||[];g=new Set(o.map(([l,m])=>x(l,m)))}E(i),e.innerHTML=`
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;gap:12px;flex-wrap:wrap">
  ...
  `;function a(){const c=Rt[i]||{},o=ze(i),l=320,m=400,p=22;function f(y){const w=c[y];return w?{x:w.x*l,y:w.y*m}:null}let u=`<svg width="${l}" height="${m}" viewBox="0 0 ${l} ${m}" xmlns="http://www.w3.org/2000/svg" style="display:block">`;o.forEach(([y,w],h)=>{const v=f(y),s=f(w);if(!v||!s)return;const I=x(y,w),$=g.has(I),B=$?"#3b82f6":"#999",A=$?.95:.35,S=$?4:3;u+=`<line x1="${v.x}" y1="${v.y}" x2="${s.x}" y2="${s.y}"
        stroke="transparent" stroke-width="14" class="link-hit" data-key="${I}" style="cursor:pointer"/>`,u+=`<line x1="${v.x}" y1="${v.y}" x2="${s.x}" y2="${s.y}"
        stroke="${B}" stroke-width="${S}" stroke-dasharray="${$?"none":"4,3"}"
        opacity="${A}" pointer-events="none" data-line-key="${I}"/>`});for(const y of Object.keys(c)){const w=f(y);if(!w)continue;const h=y.replace(/\d+/,""),v=hn[h]||"#555";u+=`<circle cx="${w.x}" cy="${w.y}" r="${p}" fill="${v}" stroke="rgba(255,255,255,0.7)" stroke-width="2" pointer-events="none"/>`,u+=`<text x="${w.x}" y="${w.y+4}" text-anchor="middle" font-size="10" font-weight="900" fill="white" font-family="Arial Black,Arial" pointer-events="none">${y}</text>`}u+="</svg>",document.getElementById("field-wrap").innerHTML=u;const b=document.getElementById("links-list");b.innerHTML=o.map(([y,w])=>{const h=x(y,w),v=g.has(h);return`
        ...
        </button>`}).join("");const L=e.querySelector('div[style*="font-weight:700;font-size:13px;margin-bottom:8px"]');L&&(L.textContent=`Liens (${g.size}/${o.length} actifs)`);function k(y){g.has(y)?g.delete(y):g.add(y),a()}e.querySelectorAll(".link-hit").forEach(y=>{y.addEventListener("click",()=>k(y.dataset.key))}),e.querySelectorAll(".link-toggle").forEach(y=>{y.addEventListener("click",()=>k(y.dataset.key))})}a(),document.getElementById("formation-select").addEventListener("change",c=>{i=c.target.value,E(i),a()}),document.getElementById("reset-btn").addEventListener("click",()=>{const c=ge[i]||[];g=new Set(c.map(([o,l])=>x(o,l))),a(),t("Liens réinitialisés aux valeurs par défaut (non sauvegardé)","info")}),document.getElementById("save-btn").addEventListener("click",async()=>{const o=ze(i).filter(([m,p])=>g.has(x(m,p))),{error:l}=await _.from("formation_links_overrides").upsert({formation:i,links:o,updated_at:new Date().toISOString()});if(l){t(l.message,"error");return}r[i]=o,t(`Liens enregistrés pour ${i} (${o.length} actifs)`,"success")})}const _n=[{value:"player",label:"Joueur"},{value:"formation",label:"Formation"},{value:"game_changer",label:"Game Changer"},{value:"ultra_game_changer",label:"Ultra Game Changer"},{value:"game_helper",label:"Game Helper"},{value:"stadium",label:"🏟️ Stade"}],wn=[{value:"",label:"Toutes"},{value:"normal",label:"Normal"},{value:"pepite",label:"Pépite"},{value:"papyte",label:"Papyte"},{value:"legende",label:"Légende"}],In=[{value:"credits",label:"Crédits"},{value:"pub",label:"Publicité"},{value:"free",label:"Gratuit"}];async function $n(e){e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">Chargement...</div>';const{data:t}=await _.from("booster_configs").select("*").order("sort_order");let n=null,i=[],d=null;const r=()=>window.innerWidth<700;async function g(){if(!n){i=[];return}if(n!==d){const{data:o}=await _.from("booster_drop_rates").select("*").eq("booster_id",n).order("sort_order");i=o||[],d=n}}function x(){return`
    ...
    </div>`}function E(){const o=n?(t||[]).find(p=>p.id===n):null;if(!o)return`<div style="display:flex;align-items:center;justify-content:center;flex:1;color:#bbb;font-size:14px;padding:40px;text-align:center">
      ← Sélectionnez un booster ou créez-en un nouveau
    </div>`;const l=i.reduce((p,f)=>p+Number(f.percentage||0),0),m=Math.abs(l-100)<.1;return`
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
          <div>
            ...
            <select id="f-price-type" style="width:100%;padding:9px;border:1px solid #ddd;border-radius:6px;font-size:13px">
              ${In.map(p=>`<option value="${p.value}" ${o.price_type===p.value?"selected":""}>${p.label}</option>`).join("")}
            </select>
          </div>
        ...
        ${i.length===0?'<div style="text-align:center;padding:16px;color:#aaa;font-size:13px">Aucun taux. Cliquez + Ligne.</div>':i.map((p,f)=>`
          <div style="display:grid;grid-template-columns:2fr 1.5fr 50px 50px 60px 32px;gap:4px;align-items:center;margin-bottom:6px;padding:6px;background:#f9f9f9;border-radius:6px" data-rate-idx="${f}">
            <select class="rate-type" data-idx="${f}" style="padding:6px 4px;border:1px solid #ddd;border-radius:4px;font-size:12px;width:100%">
              ${_n.map(u=>`<option value="${u.value}" ${p.card_type===u.value?"selected":""}>${u.label}</option>`).join("")}
            </select>
            <select class="rate-rarity" data-idx="${f}" style="padding:6px 4px;border:1px solid #ddd;border-radius:4px;font-size:12px;width:100%">
              ${wn.map(u=>`<option value="${u.value}" ${(p.rarity||"")===u.value?"selected":""}>${u.label}</option>`).join("")}
            </select>
    ...
    </div>`}async function a(o=!1){o||await g();const l=!n&&r(),m=n&&r();e.innerHTML=`
    <div style="display:flex;height:calc(100vh - 56px);overflow:hidden">
      ${!r()||l||!n?x():""}
      ${!r()||m?E():""}
    </div>`,c()}function c(){var m,p,f,u,b,L,k;const o=y=>e.querySelector(y);e.querySelectorAll(".booster-row").forEach(y=>{y.addEventListener("click",w=>{w.target.closest(".btn-delete")||(n=y.dataset.id,d=null,a())})}),(m=o("#btn-back"))==null||m.addEventListener("click",()=>{n=null,a()}),(p=o("#btn-new"))==null||p.addEventListener("click",async()=>{const y=prompt("Nom du nouveau booster :");if(!(y!=null&&y.trim()))return;const{data:w,error:h}=await _.from("booster_configs").insert({name:y.trim(),price_type:"credits",price_credits:1e3,card_count:5}).select().single();if(h){alert(h.message);return}t.push(w),n=w.id,d=null,a()}),e.querySelectorAll(".btn-delete").forEach(y=>{y.addEventListener("click",async w=>{if(w.stopPropagation(),!confirm("Supprimer ce booster ?"))return;await _.from("booster_configs").delete().eq("id",y.dataset.id);const h=t.findIndex(v=>v.id===y.dataset.id);h>-1&&t.splice(h,1),n===y.dataset.id&&(n=null,d=null),a()})}),(f=o("#btn-cancel"))==null||f.addEventListener("click",()=>{n=null,a()}),(u=o("#f-price-type"))==null||u.addEventListener("change",y=>{const w=o("#credits-field");w&&(w.style.opacity=y.target.value!=="credits"?"0.4":"1")}),(b=o("#btn-pick-icon"))==null||b.addEventListener("click",async()=>{var w;const y=o("#icon-picker-grid");if(y){if(y.style.display!=="none"){y.style.display="none";return}y.innerHTML='<div style="padding:10px;text-align:center;color:#888;font-size:12px">⏳ Chargement...</div>',y.style.display="block";try{const v=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),s=Array.isArray(v)?v.filter($=>$.name.startsWith("booster")&&/\.(png|jpg|svg|webp)$/i.test($.name)):[];if(!s.length){y.innerHTML='<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Aucune icône "booster*" trouvée.</div>';return}const I=((w=o("#f-image-url"))==null?void 0:w.value)||"";y.innerHTML=`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(72px,1fr));gap:8px">
          ${s.map($=>`
        ...
        </div>`,y.querySelectorAll(".icon-pick-item").forEach($=>{$.addEventListener("click",()=>{const B=o("#f-image-url");B&&(B.value=$.dataset.name),y.style.display="none"})})}catch(h){y.innerHTML=`<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Erreur API GitHub : ${h.message}</div>`}}});function l(){e.querySelectorAll("[data-rate-idx]").forEach(y=>{var h,v,s,I,$;const w=Number(y.dataset.rateIdx);w>=0&&w<i.length&&(i[w].card_type=((h=y.querySelector(".rate-type"))==null?void 0:h.value)||"player",i[w].rarity=((v=y.querySelector(".rate-rarity"))==null?void 0:v.value)||null,i[w].note_min=Number((s=y.querySelector(".rate-note-min"))==null?void 0:s.value)||null,i[w].note_max=Number((I=y.querySelector(".rate-note-max"))==null?void 0:I.value)||null,i[w].percentage=Number(($=y.querySelector(".rate-pct"))==null?void 0:$.value)||0)})}(L=o("#btn-add-rate"))==null||L.addEventListener("click",()=>{l(),i.push({id:null,booster_id:n,card_type:"player",rarity:"normal",note_min:null,note_max:null,percentage:10,sort_order:i.length}),a(!0)}),e.querySelectorAll(".btn-del-rate").forEach(y=>{y.addEventListener("click",()=>{l(),i.splice(Number(y.dataset.idx),1),a(!0)})}),(k=o("#btn-save"))==null||k.addEventListener("click",async()=>{var I,$,B,A,S,T,C,O,U,H,N,D,ie;const y=(t||[]).find(M=>M.id===n);if(!y)return;const w=[];e.querySelectorAll("[data-rate-idx]").forEach(M=>{var Z,X,Q,ee,te;const Y=Number(M.dataset.rateIdx);w.push({booster_id:n,card_type:((Z=M.querySelector(".rate-type"))==null?void 0:Z.value)||"player",rarity:((X=M.querySelector(".rate-rarity"))==null?void 0:X.value)||null,note_min:Number((Q=M.querySelector(".rate-note-min"))==null?void 0:Q.value)||null,note_max:Number((ee=M.querySelector(".rate-note-max"))==null?void 0:ee.value)||null,percentage:Number((te=M.querySelector(".rate-pct"))==null?void 0:te.value)||0,sort_order:Y});const P=w[w.length-1];P.rarity||(P.rarity=null),P.note_min||(P.note_min=null),P.note_max||(P.note_max=null)});const h=w.reduce((M,Y)=>M+Y.percentage,0);if(w.length&&Math.abs(h-100)>.5){alert(`La somme doit faire 100% (actuellement ${h.toFixed(1)}%)`);return}const v={name:(($=(I=o("#f-name"))==null?void 0:I.value)==null?void 0:$.trim())||y.name,image_url:((A=(B=o("#f-image-url"))==null?void 0:B.value)==null?void 0:A.trim())||null,price_type:(S=o("#f-price-type"))==null?void 0:S.value,price_credits:Number((T=o("#f-price-credits"))==null?void 0:T.value)||0,card_count:Number((C=o("#f-card-count"))==null?void 0:C.value)||5,is_active:((O=o("#f-active"))==null?void 0:O.checked)??y.is_active,allow_duplicates:((U=o("#f-allow-dup"))==null?void 0:U.checked)??!0,sort_order:Number((H=o("#f-sort"))==null?void 0:H.value)||0,max_per_user:(N=o("#f-max-per-user"))!=null&&N.value?Number(o("#f-max-per-user").value):null,available_from:((D=o("#f-available-from"))==null?void 0:D.value)||null,available_until:((ie=o("#f-available-until"))==null?void 0:ie.value)||null},{error:s}=await _.from("booster_configs").update(v).eq("id",n);if(s){alert(s.message);return}if(Object.assign(y,v),await _.from("booster_drop_rates").delete().eq("booster_id",n),w.length){const{error:M}=await _.from("booster_drop_rates").insert(w);if(M){alert(M.message);return}}i=w,d=n,alert("✅ Booster enregistré !"),a(!0)})}a()}const ft=[{value:"normal",label:"Normal",color:"#888"},{value:"pepite",label:"Pépite",color:"#D4A017"},{value:"papyte",label:"Papyte",color:"#909090"},{value:"legende",label:"Légende",color:"#7a28b8"}];async function kn(e){await ke(e)}async function ke(e){const{data:t,error:n}=await _.from("sell_price_configs").select("*").order("rarity").order("note_min");if(n){e.innerHTML=`<div style="padding:24px;color:red">Erreur : ${n.message}</div>`;return}e.innerHTML=`
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px">
          <div>
            ...
            <select id="form-rarity" style="width:100%;padding:9px;border:1.5px solid #ddd;border-radius:8px;font-size:14px">
              ${ft.map(i=>`<option value="${i.value}">${i.label}</option>`).join("")}
            </select>
          </div>
          <div>
    ...
    </div>`,document.getElementById("add-config").addEventListener("click",()=>Ue(null)),document.getElementById("form-cancel").addEventListener("click",()=>{document.getElementById("config-form").style.display="none"}),document.getElementById("form-save").addEventListener("click",()=>Bn(e)),e.querySelectorAll("[data-edit]").forEach(i=>{const d=(t||[]).find(r=>r.id===i.dataset.edit);d&&i.addEventListener("click",()=>Ue(d))}),e.querySelectorAll("[data-delete]").forEach(i=>{i.addEventListener("click",async()=>{confirm("Supprimer cette règle ?")&&(await _.from("sell_price_configs").delete().eq("id",i.dataset.delete),await ke(e))})})}function Ln(e){const t=ft.find(n=>n.value===e.rarity);return`
    ...
    </tr>`}function Ue(e){document.getElementById("config-form").style.display="block",document.getElementById("form-title").textContent=e?"Modifier la règle":"Nouvelle règle",document.getElementById("form-id").value=(e==null?void 0:e.id)||"",document.getElementById("form-rarity").value=(e==null?void 0:e.rarity)||"normal",document.getElementById("form-price").value=(e==null?void 0:e.price)||1e3,document.getElementById("form-note-min").value=(e==null?void 0:e.note_min)??1,document.getElementById("form-note-max").value=(e==null?void 0:e.note_max)??10,document.getElementById("config-form").scrollIntoView({behavior:"smooth"})}async function Bn(e){const t=document.getElementById("form-id").value,n=document.getElementById("form-rarity").value,i=parseInt(document.getElementById("form-price").value)||0,d=parseInt(document.getElementById("form-note-min").value)||1,r=parseInt(document.getElementById("form-note-max").value)||10;if(d>r){alert("Note min doit être ≤ note max");return}const g={rarity:n,price:i,note_min:d,note_max:r,updated_at:new Date().toISOString()};let x;if(t?{error:x}=await _.from("sell_price_configs").update(g).eq("id",t):{error:x}=await _.from("sell_price_configs").insert(g),x){alert("Erreur : "+x.message);return}document.getElementById("config-form").style.display="none",await ke(e)}async function An(e){await Le(e)}async function Le(e){const{data:t}=await _.from("patch_notes").select("*").order("published_at",{ascending:!1});e.innerHTML=`
    ...
    </div>`,document.getElementById("add-article").addEventListener("click",()=>He(null)),document.getElementById("form-cancel").addEventListener("click",()=>{document.getElementById("article-form").style.display="none"}),document.getElementById("form-save").addEventListener("click",()=>Cn(e)),e.querySelectorAll("[data-edit-row]").forEach(n=>{const i=(t||[]).find(d=>d.id===n.dataset.editRow);i&&n.addEventListener("click",()=>He(i))}),e.querySelectorAll("[data-delete]").forEach(n=>{n.addEventListener("click",async()=>{confirm("Supprimer cet article ?")&&(await _.from("patch_notes").delete().eq("id",n.dataset.delete),await Le(e))})})}function Sn(e){const t=new Date(e.published_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"short",year:"numeric"});return`
    ...
    <div style="display:flex;gap:16px;height:calc(100vh - 56px);overflow:hidden;padding:16px">
      ...
      <div style="width:420px;flex-shrink:0;overflow-y:auto">
        ...
        <div style="margin-bottom:16px">
          ...
          <div id="ts-toolbar" style="display:flex;flex-wrap:wrap;gap:4px;padding:6px 8px;background:#f5f5f5;border:1.5px solid #ddd;border-bottom:none;border-radius:8px 8px 0 0">
            ${[["B","bold","<strong>Gras</strong>"],["I","italic","<em>Italique</em>"],["U","underline","Souligné"],["¶","insertParagraph","Paragraphe"],["•","insertUnorderedList","Liste à puces"],["1.","insertOrderedList","Liste numérotée"],["H2","formatBlock:h2","Titre H2"],["H3","formatBlock:h3","Titre H3"],["—","insertHorizontalRule","Séparateur"]].map(([h,v,s])=>`<button type="button" data-cmd="${v}" title="${s}" style="padding:4px 8px;border:1px solid #ddd;border-radius:4px;background:#fff;font-size:12px;font-weight:700;cursor:pointer;line-height:1;min-width:28px">${h}</button>`).join("")}
      ...
      <div style="flex:1;overflow-y:auto">
        ...
        <div id="ts-list" style="display:flex;flex-direction:column;gap:10px">
    ...
    </div>`,(c=document.getElementById("ts-add"))==null||c.addEventListener("click",()=>Ge(null,(t==null?void 0:t.length)||0)),(o=document.getElementById("ts-reset"))==null||o.addEventListener("click",async()=>{if(!confirm("Remettre tutorial_done = false pour TOUS les joueurs ? Ils reverront le tutoriel à leur prochaine connexion."))return;const{error:h}=await _.from("users").update({tutorial_done:!1}).neq("id","00000000-0000-0000-0000-000000000000");alert(h?"Erreur : "+h.message:"✅ Tutorial_done réinitialisé pour tous les joueurs.")}),(l=document.getElementById("ts-cancel"))==null||l.addEventListener("click",yt),(m=document.getElementById("ts-save"))==null||m.addEventListener("click",()=>jn(e)),(p=document.getElementById("ts-preview-btn"))==null||p.addEventListener("click",Rn);const n=document.getElementById("ts-editor"),i=()=>{const h=document.getElementById("ts-content");h&&n&&(h.value=n.innerHTML)};n==null||n.addEventListener("input",i),n==null||n.addEventListener("blur",i);const d=()=>{document.activeElement!==n&&(n==null||n.focus())};(f=document.getElementById("ts-toolbar"))==null||f.querySelectorAll("[data-cmd]").forEach(h=>{h.addEventListener("mousedown",v=>{v.preventDefault(),d();const s=h.dataset.cmd;s.startsWith("formatBlock:")?document.execCommand("formatBlock",!1,s.split(":")[1]):document.execCommand(s,!1,null),i()})}),(u=document.getElementById("ts-insert-color"))==null||u.addEventListener("mousedown",h=>{h.preventDefault(),d();const v=prompt("Couleur hex (ex: #D4A017 pour or, #1A6B3C pour vert, #cc2222 pour rouge) :");v&&(document.execCommand("foreColor",!1,v),i())}),(b=document.getElementById("ts-clear-format"))==null||b.addEventListener("mousedown",h=>{h.preventDefault(),d(),document.execCommand("removeFormat",!1,null),i()}),n==null||n.addEventListener("focus",()=>{n&&(n.style.borderColor="#1A6B3C")}),n==null||n.addEventListener("blur",()=>{n&&(n.style.borderColor="#ddd")});const r=document.getElementById("ts-image"),g=document.getElementById("ts-img-preview"),x=document.getElementById("ts-img-preview-el"),E=document.getElementById("ts-img-picker-grid"),a=()=>{var v;const h=(v=r==null?void 0:r.value)==null?void 0:v.trim();if(h){const s="/";x.src=`${s}icons/${h}`,g.style.display="block"}else g.style.display="none"};r==null||r.addEventListener("input",a),(L=document.getElementById("ts-img-clear"))==null||L.addEventListener("click",()=>{r&&(r.value=""),g.style.display="none",E.style.display="none"}),(k=document.getElementById("ts-img-pick"))==null||k.addEventListener("click",async()=>{if(E.style.display!=="none"){E.style.display="none";return}E.innerHTML='<div style="color:#aaa;font-size:12px">Chargement…</div>',E.style.display="block";try{const v=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),s=Array.isArray(v)?v.filter($=>$.name.startsWith("tuto_")):[];if(!s.length){E.innerHTML='<div style="color:#aaa;font-size:12px">Aucun fichier tuto_* trouvé dans /icons/.<br>Uploade des images nommées tuto_xxx.png dans public/icons/</div>';return}const I="/";E.innerHTML='<div style="display:flex;flex-wrap:wrap;gap:8px">'+s.map($=>`
          ...
          </div>`).join("")+"</div>",E.querySelectorAll("[data-pick]").forEach($=>{$.addEventListener("click",()=>{r&&(r.value=$.dataset.pick),a(),E.style.display="none"})})}catch(h){E.innerHTML='<div style="color:#cc2222;font-size:12px">Erreur chargement : '+h.message+"</div>"}}),(y=document.getElementById("ts-color"))==null||y.addEventListener("input",h=>{const v=document.getElementById("ts-color-hex");v&&(v.value=h.target.value)}),(w=document.getElementById("ts-color-hex"))==null||w.addEventListener("input",h=>{const v=h.target.value;if(/^#[0-9a-fA-F]{6}$/.test(v)){const s=document.getElementById("ts-color");s&&(s.value=v)}}),e.querySelectorAll("[data-edit-row]").forEach(h=>{const v=(t||[]).find(s=>s.id===h.dataset.editRow);v&&h.addEventListener("click",()=>Ge(v))}),e.querySelectorAll("[data-delete]").forEach(h=>{h.addEventListener("click",async()=>{confirm("Supprimer cette étape ?")&&(await _.from("tutorial_steps").delete().eq("id",h.dataset.delete),W(e))})}),e.querySelectorAll("[data-toggle]").forEach(h=>{h.addEventListener("click",async()=>{const v=(t||[]).find(s=>s.id===h.dataset.toggle);v&&(await _.from("tutorial_steps").update({is_active:!v.is_active}).eq("id",v.id),W(e))})}),e.querySelectorAll("[data-up]").forEach(h=>{h.addEventListener("click",async()=>{const v=t||[],s=v.findIndex(I=>I.id===h.dataset.up);s<=0||(await Promise.all([_.from("tutorial_steps").update({step_order:v[s-1].step_order}).eq("id",v[s].id),_.from("tutorial_steps").update({step_order:v[s].step_order}).eq("id",v[s-1].id)]),W(e))})}),e.querySelectorAll("[data-down]").forEach(h=>{h.addEventListener("click",async()=>{const v=t||[],s=v.findIndex(I=>I.id===h.dataset.down);s<0||s>=v.length-1||(await Promise.all([_.from("tutorial_steps").update({step_order:v[s+1].step_order}).eq("id",v[s].id),_.from("tutorial_steps").update({step_order:v[s].step_order}).eq("id",v[s+1].id)]),W(e))})})}function zn(e){return`
  <div data-edit-row="${e.id}" style="background:#fff;border-radius:10px;padding:12px 16px;box-shadow:0 1px 4px rgba(0,0,0,0.08);display:flex;align-items:center;gap:12px;cursor:pointer">
  ...
  </div>`}function Ge(e,t=0){document.getElementById("ts-form-empty").style.display="none",document.getElementById("ts-form-content").style.display="block",document.getElementById("ts-form-title").textContent=e?"Modifier l'étape":"Nouvelle étape",document.getElementById("ts-id").value=(e==null?void 0:e.id)||"",document.getElementById("ts-emoji").value=(e==null?void 0:e.emoji)||"⚽",document.getElementById("ts-title").value=(e==null?void 0:e.title)||"",document.getElementById("ts-content").value=(e==null?void 0:e.content)||"";const n=document.getElementById("ts-editor");n&&(n.innerHTML=(e==null?void 0:e.content)||"<p>Texte de l'étape…</p>"),document.getElementById("ts-order").value=(e==null?void 0:e.step_order)??t,document.getElementById("ts-active").checked=e?e.is_active:!0;const i=(e==null?void 0:e.color)||"#1A6B3C";document.getElementById("ts-color").value=i,document.getElementById("ts-color-hex").value=i;const d=(e==null?void 0:e.image_url)||"",r=document.getElementById("ts-image");r&&(r.value=d);const g=document.getElementById("ts-img-preview"),x=document.getElementById("ts-img-preview-el");if(d&&g&&x){const E="/";x.src=`${E}icons/${d}`,g.style.display="block"}else g&&(g.style.display="none");document.getElementById("ts-img-picker-grid").style.display="none",document.getElementById("ts-preview-area").innerHTML="",document.getElementById("ts-form").scrollIntoView({behavior:"smooth"})}function yt(){document.getElementById("ts-form-content").style.display="none",document.getElementById("ts-form-empty").style.display="block",document.getElementById("ts-preview-area").innerHTML=""}function Rn(){var a,c;const e=document.getElementById("ts-emoji").value||"⚽",t=document.getElementById("ts-title").value||"Titre",n=document.getElementById("ts-editor"),i=n?n.innerHTML:document.getElementById("ts-content").value||"",d=document.getElementById("ts-color-hex").value||"#1A6B3C",r=(c=(a=document.getElementById("ts-image"))==null?void 0:a.value)==null?void 0:c.trim(),x=r?`<div style="padding:0 20px 12px;text-align:center"><img src="/icons/${r}" style="max-height:140px;max-width:100%;border-radius:10px;object-fit:contain"></div>`:"",E=document.getElementById("ts-preview-area");E.innerHTML=`
    ...
    </div>`,E.querySelectorAll("ul,ol").forEach(o=>{o.style.paddingLeft="20px",o.style.margin="6px 0"}),E.querySelectorAll("li").forEach(o=>{o.style.marginBottom="4px"}),E.querySelectorAll("p").forEach(o=>{o.style.marginBottom="8px"})}async function jn(e){var o,l;const t=document.getElementById("ts-id").value,n=document.getElementById("ts-emoji").value.trim()||"⚽",i=document.getElementById("ts-title").value.trim(),d=document.getElementById("ts-editor"),r=(d?d.innerHTML:document.getElementById("ts-content").value).trim(),g=document.getElementById("ts-color-hex").value.trim()||"#1A6B3C",x=parseInt(document.getElementById("ts-order").value)||0,E=document.getElementById("ts-active").checked;if(!i||!r){alert("Titre et contenu sont obligatoires.");return}const a={emoji:n,title:i,content:r,color:g,step_order:x,is_active:E,image_url:((l=(o=document.getElementById("ts-image"))==null?void 0:o.value)==null?void 0:l.trim())||null};let c;if(t?{error:c}=await _.from("tutorial_steps").update(a).eq("id",t):{error:c}=await _.from("tutorial_steps").insert(a),c){alert("Erreur : "+c.message);return}yt(),W(e)}const bt="/",vt=[["MA","Maroc"],["FR","France"],["AR","Argentine"],["PT","Portugal"],["BR","Brésil"],["ES","Espagne"],["DE","Allemagne"],["GB","Angleterre"],["IT","Italie"],["CM","Cameroun"],["SN","Sénégal"],["NG","Nigéria"],["DK","Danemark"],["NL","Pays-Bas"],["BE","Belgique"],["CI","Côte d'Ivoire"],["AL","Albanie"],["HR","Croatie"],["RS","Serbie"],["TR","Turquie"],["MW","Malawi"],["GH","Ghana"],["ZA","Afrique du Sud"],["EG","Égypte"],["US","États-Unis"],["MX","Mexique"],["CO","Colombie"],["UY","Uruguay"],["CL","Chili"],["JP","Japon"],["KR","Corée du Sud"],["CN","Chine"],["AU","Australie"],["SW","Suisse"],["SE","Suède"],["NO","Norvège"],["PL","Pologne"]];async function Dn(e,t){await ce(e,t)}async function ce(e,t){var d,r;const[{data:n},{data:i}]=await Promise.all([_.from("stadium_definitions").select("*, club:clubs(encoded_name, logo_url)").order("name"),_.from("clubs").select("id, encoded_name, logo_url").order("encoded_name")]);e.innerHTML=`
    ...
    </div>`,(d=document.getElementById("st-add-btn"))==null||d.addEventListener("click",()=>qe(null,i,e,t)),(r=document.getElementById("st-gen-countries-btn"))==null||r.addEventListener("click",async()=>{const{toast:g}=t,x=new Set((n||[]).filter(l=>!l.club_id&&l.country_code).map(l=>l.country_code)),E=vt.filter(([l])=>!x.has(l));if(!E.length){g("Tous les pays ont déjà un stade.","info");return}if(!confirm(`Créer ${E.length} stade(s) "Pays" manquant(s) ?`))return;const a=E.map(([l,m])=>({name:`Stade ${m}`,club_id:null,country_code:l})),{data:c,error:o}=await _.from("stadium_definitions").insert(a).select();if(o){g("Erreur : "+o.message,"error");return}c!=null&&c.length&&await _.from("cards").insert(c.map(l=>({card_type:"stadium",stadium_id:l.id}))),g(`${(c==null?void 0:c.length)||0} stade(s) Pays créé(s) ✅`,"success"),ce(e,t)}),e.querySelectorAll("[data-edit-stadium]").forEach(g=>{g.addEventListener("click",()=>{const x=(n||[]).find(E=>E.id===g.dataset.editStadium);x&&qe(x,i,e,t)})})}function Pn(e){var d,r;let t=null;e.image_url?t=e.image_url.startsWith("http")?e.image_url:`${bt}icons/${e.image_url}`:(d=e.club)!=null&&d.logo_url?t=e.club.logo_url:e.country_code&&(t=`https://flagsapi.com/${e.country_code.slice(0,2).toUpperCase()}/flat/64.png`);const n=((r=e.club)==null?void 0:r.encoded_name)||e.country_code||"—",i=Qe(e.name,t,`${n}<br>+10 ⭐`,{width:140});return`<div data-edit-stadium="${e.id}" style="cursor:pointer;transition:transform .15s" onmouseover="this.style.transform='translateY(-3px)'" onmouseout="this.style.transform=''">
    ${i}
  </div>`}function qe(e,t,n,i){var c,o,l;const{openModal:d,closeModal:r,toast:g}=i,x=`
      ...
      <div style="flex:1;min-width:220px;display:flex;flex-direction:column;gap:12px">
        ...
        <div>
          ...
          <select id="st-country">
            ...
            ${vt.map(([m,p])=>`<option value="${m}" ${(e==null?void 0:e.country_code)===m?"selected":""}>${p} (${m})</option>`).join("")}
          </select>
        </div>
        <div>
  ...
  `);const a=()=>{var v,s,I,$,B,A,S;const m=((v=document.getElementById("st-name"))==null?void 0:v.value)||"NOM DU STADE",p=(I=(s=document.getElementById("st-image"))==null?void 0:s.value)==null?void 0:I.trim(),f=(B=($=document.getElementById("st-country"))==null?void 0:$.value)==null?void 0:B.trim(),u=document.getElementById("st-club"),b=(u==null?void 0:u.selectedIndex)||0,L=u&&b>0?u.options[b].text:"",k=((S=(A=u==null?void 0:u.options[b])==null?void 0:A.getAttribute)==null?void 0:S.call(A,"data-logo"))||"";let y=null;p?y=p.startsWith("http")?p:`${bt}icons/${p}`:k?y=k:f&&(y=`https://flagsapi.com/${f.toLowerCase().slice(0,2).toUpperCase()}/flat/64.png`);const w=L||f||"—",h=document.getElementById("st-preview-card");h&&(h.innerHTML=Qe(m,y,`${w}<br>+10 ⭐`,{width:140}))};a(),["st-name","st-club","st-country","st-image"].forEach(m=>{var p,f;(p=document.getElementById(m))==null||p.addEventListener("input",a),(f=document.getElementById(m))==null||f.addEventListener("change",a)}),(c=document.getElementById("st-cancel"))==null||c.addEventListener("click",()=>r()),(o=document.getElementById("st-save"))==null||o.addEventListener("click",async()=>{const m=document.getElementById("st-name").value.trim(),p=document.getElementById("st-club").value||null,f=document.getElementById("st-country").value.trim().toUpperCase()||null,u=document.getElementById("st-image").value.trim()||null;if(!m){g("Le nom est obligatoire","error");return}const b={name:m,club_id:p,country_code:f,image_url:u},{error:L}=e?await _.from("stadium_definitions").update(b).eq("id",e.id):await _.from("stadium_definitions").insert(b);if(L){g("Erreur : "+L.message,"error");return}g(e?"Stade modifié ✅":"Stade créé ✅","success"),r(),ce(n,i)}),(l=document.getElementById("st-delete"))==null||l.addEventListener("click",async()=>{confirm("Supprimer ce stade ?")&&(await _.from("stadium_definitions").delete().eq("id",e.id),g("Stade supprimé","success"),r(),ce(n,i))})}const q=[{value:"game_changer",label:"⚡ Game Changer",border:"#7a28b8",bg:"linear-gradient(135deg,#4a0080,#7a28b8)"},{value:"ultra_game_changer",label:"💎 Ultra Game Changer",border:"#00bcd4",bg:"linear-gradient(135deg,#006080,#00bcd4)"}],Nn=[{value:"purple",label:"Violet",hex:"#7a28b8"},{value:"light_blue",label:"Bleu clair",hex:"#00bcd4"}],G=[{value:"BOOST_STAT",label:"⚡ Boost stats (+N)",hasValue:!0,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"DEBUFF_STAT",label:"💀 Debuff stats (-N)",hasValue:!0,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"GRAY_PLAYER",label:"❄️ Griser un joueur",hasValue:!1,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"REVIVE_PLAYER",label:"💫 Ressusciter un joueur",hasValue:!1,hasTarget:!1,hasCount:!0,hasRoles:!1},{value:"REMOVE_GOAL",label:"🚫 Retirer un but adverse",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1},{value:"ADD_GOAL_DRAW",label:"⚽ +1 but si duel nul",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1},{value:"ADD_SUB",label:"🔄 Ajouter un remplacement",hasValue:!1,hasTarget:!1,hasCount:!0,hasRoles:!1},{value:"CUSTOM",label:"🛠️ Effet custom (hardcodé)",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1}],Ke=["GK","DEF","MIL","ATT"];async function Fn(e,t){await Be(e,t)}async function Be(e,t){var i;const{data:n}=await _.from("gc_definitions").select("*").order("sort_order").order("created_at");e.innerHTML=`
    <div style="padding:20px;max-width:1100px;margin:0 auto">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px">
    ...
    </div>`,(i=document.getElementById("gc-add-btn"))==null||i.addEventListener("click",()=>Je(null,e,t)),e.querySelectorAll("[data-edit-gc]").forEach(d=>{d.addEventListener("click",()=>{const r=(n||[]).find(g=>g.id===d.dataset.editGc);r&&Je(r,e,t)})}),e.querySelectorAll(".btn-del-gc").forEach(d=>{d.addEventListener("click",async r=>{r.stopPropagation(),confirm("Supprimer cette carte Game Changer ?")&&(await _.from("gc_definitions").delete().eq("id",d.dataset.del),Be(e,t))})})}function On(e){const t=q.find(d=>d.value===e.gc_type)||q[0],n=e.image_url?`/icons/${e.image_url}`:null,i=ye(e.name||"?",n,t.label.split(" ")[0],e.effect||"",{width:130});return`<div data-edit-gc="${e.id}" style="position:relative;cursor:pointer">
    ${i}
    <div style="position:absolute;top:4px;left:4px;z-index:10;display:flex;gap:3px">
  ...
  </div>`}function Je(e,t,n){var l,m,p,f;const{openModal:i,closeModal:d,toast:r}=n,g=!e,x=e||{name:"",gc_type:"game_changer",color:"purple",effect:"",image_url:"",sort_order:0,is_active:!0,effect_type:"BOOST_STAT",effect_params:{}},E=(()=>{const u=x.effect_params||{},b=G.find(L=>L.value===(x.effect_type||"BOOST_STAT"))||G[0];return`
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
      ${b.hasValue?`<div>
  ...
  `;i(g?"Nouvelle carte Game Changer":`Modifier : ${x.name}`,a,`
    <button id="gc-cancel" class="btn btn-ghost">Annuler</button>
    <button id="gc-save" class="btn btn-primary">💾 Enregistrer</button>
  `);const o=()=>{var h,v,s,I,$;const u=((h=document.getElementById("gc-name"))==null?void 0:h.value)||"?",b=((v=document.getElementById("gc-effect"))==null?void 0:v.value)||"",L=((s=document.getElementById("gc-type"))==null?void 0:s.value)||"game_changer",k=($=(I=document.getElementById("gc-image-url"))==null?void 0:I.value)==null?void 0:$.trim(),y=q.find(B=>B.value===L)||q[0],w=document.getElementById("gc-modal-preview");w&&(w.innerHTML=ye(u,k?`/icons/${k}`:null,y.label.split(" ")[0],b,{width:150}))};["gc-name","gc-effect","gc-type","gc-image-url"].forEach(u=>{var b,L;(b=document.getElementById(u))==null||b.addEventListener("input",o),(L=document.getElementById(u))==null||L.addEventListener("change",o)}),(l=document.getElementById("gc-effect-type"))==null||l.addEventListener("change",()=>{const u=document.getElementById("gc-effect-type").value,b=G.find(k=>k.value===u)||G[0],L=document.getElementById("gc-params-wrap");L.innerHTML=`
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
    ...
    </div>`}),(m=document.getElementById("btn-pick-gc-icon"))==null||m.addEventListener("click",async()=>{var b;const u=document.getElementById("gc-icon-picker");if(u){if(u.style.display!=="none"){u.style.display="none";return}u.innerHTML='<div style="padding:10px;text-align:center;color:#888;font-size:12px">⏳ Chargement...</div>',u.style.display="block";try{const k=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),y=Array.isArray(k)?k.filter(h=>h.name.startsWith("gamechanger-")&&/\.(png|jpg|svg|webp)$/i.test(h.name)):[];if(!y.length){u.innerHTML='<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Aucun fichier "gamechanger-*" trouvé dans public/icons/</div>';return}const w=((b=document.getElementById("gc-image-url"))==null?void 0:b.value)||"";u.innerHTML=`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(72px,1fr));gap:8px">
        ${y.map(h=>`
      ...
      </div>`,u.querySelectorAll(".gc-icon-item").forEach(h=>{h.addEventListener("click",()=>{const v=document.getElementById("gc-image-url");v&&(v.value=h.dataset.name),u.style.display="none",o()})})}catch(L){u.innerHTML=`<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Erreur API GitHub : ${L.message}</div>`}}}),(p=document.getElementById("gc-cancel"))==null||p.addEventListener("click",()=>d()),(f=document.getElementById("gc-save"))==null||f.addEventListener("click",async()=>{var w,h,v,s,I,$,B,A,S,T,C,O,U,H;const u=G.find(N=>{var D;return N.value===(((D=document.getElementById("gc-effect-type"))==null?void 0:D.value)||"BOOST_STAT")})||G[0],b=u.hasRoles?[...document.querySelectorAll(".gc-p-role:checked")].map(N=>N.value):null,L={...u.hasValue?{value:Number((w=document.getElementById("gc-p-value"))==null?void 0:w.value)||2}:{},...u.hasCount?{count:Number((h=document.getElementById("gc-p-count"))==null?void 0:h.value)||1}:{},...u.hasTarget?{target:((v=document.getElementById("gc-p-target"))==null?void 0:v.value)||"home"}:{},...u.hasRoles?{roles:b!=null&&b.length?b:null}:{}},k={name:((I=(s=document.getElementById("gc-name"))==null?void 0:s.value)==null?void 0:I.trim())||"",effect:((B=($=document.getElementById("gc-effect"))==null?void 0:$.value)==null?void 0:B.trim())||null,image_url:((S=(A=document.getElementById("gc-image-url"))==null?void 0:A.value)==null?void 0:S.trim())||null,gc_type:((T=document.getElementById("gc-type"))==null?void 0:T.value)||"game_changer",color:((C=document.getElementById("gc-color"))==null?void 0:C.value)||"purple",sort_order:Number((O=document.getElementById("gc-sort"))==null?void 0:O.value)||0,is_active:((U=document.getElementById("gc-active"))==null?void 0:U.checked)??!0,effect_type:((H=document.getElementById("gc-effect-type"))==null?void 0:H.value)||"BOOST_STAT",effect_params:L};if(!k.name){r("Le nom est obligatoire","error");return}const{error:y}=g?await _.from("gc_definitions").insert(k):await _.from("gc_definitions").update(k).eq("id",x.id);if(y){r(y.message,"error");return}r(g?"Carte créée ✅":"Carte modifiée ✅","success"),d(),Be(t,n)})}async function Un(e,{toast:t,openModal:n,closeModal:i}){await j(e,{toast:t,openModal:n,closeModal:i})}async function j(e,t){var h,v;const{toast:n,openModal:i,closeModal:d}=t,[{data:r,error:g},{data:x},{data:E},{data:a}]=await Promise.all([_.from("ranked_seasons").select("*").order("start_at",{ascending:!1}),_.from("users").select("mmr,rank_tier,placement_matches").gte("placement_matches",1),_.from("season_reward_tiers").select("*").order("rank_min"),_.from("booster_configs").select("id,name").order("sort_order")]);if(g){e.innerHTML=`<p style="color:red">${g.message}</p>`;return}const c=r||[],o=c.find(s=>s.is_active),l=(x||[]).filter(s=>(s.placement_matches||0)>=1),m=l.filter(s=>(s.placement_matches||0)>=10),p=["bronze","silver","gold","platinum","diamond","master"],f={bronze:"Bronze",silver:"Argent",gold:"Or",platinum:"Platine",diamond:"Diamant",master:"Maître"},u={bronze:"#cd7f32",silver:"#a0a0a0",gold:"#D4A017",platinum:"#00e5ff",diamond:"#5b9bd5",master:"#9b59b6"},b={bronze:"🥉",silver:"🥈",gold:"🥇",platinum:"💎",diamond:"🔷",master:"👑"},L={};p.forEach(s=>{L[s]=0}),m.forEach(s=>{const I=s.rank_tier||"bronze";L[I]!==void 0&&L[I]++});const k=m.length?Math.round(m.reduce((s,I)=>s+(I.mmr||1e3),0)/m.length):0;function y(s){return s?new Date(s).toLocaleDateString("fr",{day:"numeric",month:"short",year:"numeric"}):"—"}function w(s){const I=new Date;return s.is_active?'<span style="background:#1A6B3C;color:#fff;padding:2px 8px;border-radius:10px;font-size:11px;font-weight:700">● ACTIVE</span>':new Date(s.end_at)<I?'<span style="background:#555;color:#ccc;padding:2px 8px;border-radius:10px;font-size:11px">Terminée</span>':'<span style="background:#e67e22;color:#fff;padding:2px 8px;border-radius:10px;font-size:11px">À venir</span>'}e.innerHTML=`
    <div style="display:flex;flex-direction:column;gap:20px">
      ...
      <div class="card-panel" style="background:linear-gradient(135deg,#0a3d1e,#1a6b3c20);border:1.5px solid #1A6B3C40">
        ...
        <div style="display:flex;flex-direction:column;gap:4px">
          ${p.map(s=>{const I=L[s],$=m.length>0?Math.round(I/m.length*100):0;return`
              <div style="display:flex;align-items:center;gap:8px;font-size:12px">
                <span style="width:80px">${b[s]} ${f[s]}</span>
                <div style="flex:1;background:rgba(255,255,255,0.08);border-radius:4px;height:14px;overflow:hidden">
                  <div style="height:100%;width:${$}%;background:${u[s]};border-radius:4px;transition:width 0.5s"></div>
                </div>
        ...
        <div class="table-wrap">
          <table>
            ...
            <tbody id="seasons-tbody">
              ${c.length===0?'<tr><td colspan="6" style="text-align:center;color:var(--gray-600);padding:24px">Aucune saison créée.</td></tr>':c.map(s=>{const I=Math.round((new Date(s.end_at)-new Date(s.start_at))/864e5),$=new Date(s.end_at)<new Date&&!s.is_active;return`
                    <tr>
                      <td><b>${s.name}</b></td>
                      <td style="font-size:12px">${y(s.start_at)}</td>
                      <td style="font-size:12px">${y(s.end_at)}</td>
                      ...
                      <td style="display:flex;gap:6px;flex-wrap:wrap">
                        ...
                        ${$?`<button class="btn btn-danger btn-sm" data-delete="${s.id}">🗑</button>`:""}
                      </td>
                    </tr>`}).join("")}
            </tbody>
          </table>
      ...
      <div class="card-panel">
        ...
        <div class="table-wrap">
          <table>
            ...
            <tbody>
              ${(E||[]).map(s=>{var B;const I=!s.activate_at||new Date(s.activate_at)<=new Date;return`<tr>
                  <td style="font-size:12px">${((B=c.find(A=>A.id===s.season_id))==null?void 0:B.name)||"—"}</td>
                  <td><b>${s.label}</b></td>
    ...
    </div>`,(h=document.getElementById("create-season-btn"))==null||h.addEventListener("click",()=>{Ye(null,{toast:n,openModal:i,closeModal:d,reload:()=>j(e,t)})}),e.querySelectorAll("[data-edit]").forEach(s=>{const I=c.find($=>$.id==s.dataset.edit);s.addEventListener("click",()=>{Ye(I,{toast:n,openModal:i,closeModal:d,reload:()=>j(e,t)})})}),e.querySelectorAll("[data-activate]").forEach(s=>{s.addEventListener("click",async()=>{const I=parseInt(s.dataset.activate);if(!confirm("Activer cette saison ? La saison actuellement active sera désactivée."))return;const{error:$}=await _.from("ranked_seasons").update({is_active:!1}).neq("id",0);if($){n($.message,"error");return}const{error:B}=await _.from("ranked_seasons").update({is_active:!0}).eq("id",I);if(B){n(B.message,"error");return}await _.from("users").update({current_season_id:I}).gt("placement_matches",0),n("Saison activée ✅","success"),j(e,t)})}),e.querySelectorAll("[data-deactivate]").forEach(s=>{s.addEventListener("click",async()=>{if(!confirm("Désactiver la saison active ? Le mode Ranked sera suspendu."))return;const{error:I}=await _.from("ranked_seasons").update({is_active:!1}).eq("id",parseInt(s.dataset.deactivate));if(I){n(I.message,"error");return}n("Saison désactivée","success"),j(e,t)})}),e.querySelectorAll("[data-delete]").forEach(s=>{s.addEventListener("click",async()=>{if(!confirm("Supprimer cette saison archivée ?"))return;const{error:I}=await _.from("ranked_seasons").delete().eq("id",parseInt(s.dataset.delete));if(I){n(I.message,"error");return}n("Saison supprimée","success"),j(e,t)})}),(v=document.getElementById("add-reward-tier-btn"))==null||v.addEventListener("click",()=>{Ve(null,c,o,a||[],{toast:n,openModal:i,closeModal:d,reload:()=>j(e,t)})}),e.querySelectorAll("[data-edit-tier]").forEach(s=>{const I=(E||[]).find($=>$.id===s.dataset.editTier);s.addEventListener("click",()=>{Ve(I,c,o,a||[],{toast:n,openModal:i,closeModal:d,reload:()=>j(e,t)})})}),e.querySelectorAll("[data-distribute-tier]").forEach(s=>{s.addEventListener("click",async()=>{const I=(E||[]).find(A=>A.id===s.dataset.distributeTier);if(!confirm(`Distribuer les récompenses du palier "${I==null?void 0:I.label}" à tous les joueurs concernés ?`))return;s.disabled=!0,s.textContent="⏳...";const{data:$,error:B}=await _.rpc("admin_distribute_season_reward",{p_tier_id:s.dataset.distributeTier});if(B){n(B.message,"error"),s.disabled=!1,s.textContent="🎁 Distribuer";return}if(!($!=null&&$.success)){n(($==null?void 0:$.error)||"Échec de la distribution","error"),s.disabled=!1,s.textContent="🎁 Distribuer";return}n(`Récompenses distribuées à ${$.rewarded} joueur(s) ✅`,"success"),j(e,t)})}),e.querySelectorAll("[data-delete-tier]").forEach(s=>{s.addEventListener("click",async()=>{if(!confirm("Supprimer ce palier de récompense ?"))return;const{error:I}=await _.from("season_reward_tiers").delete().eq("id",s.dataset.deleteTier);if(I){n(I.message,"error");return}n("Palier supprimé","success"),j(e,t)})})}function Ve(e,t,n,i,{toast:d,openModal:r,closeModal:g,reload:x}){var f,u,b,L,k;const E=!!e,a=(e==null?void 0:e.season_id)??(n==null?void 0:n.id)??((f=t[0])==null?void 0:f.id)??null;let c=[];const o=y=>{if(!y)return"";const w=new Date(y);return new Date(w.getTime()-w.getTimezoneOffset()*6e4).toISOString().slice(0,16)},l=`
    <div style="display:flex;flex-direction:column;gap:12px">
      <div>
  ...
  `;r(E?`Modifier : ${e.label}`:"Nouveau palier de récompense",l,`
    <button id="rt-cancel" class="btn btn-ghost">Annuler</button>
    <button id="rt-save" class="btn btn-primary">💾 Enregistrer</button>
  `);function p(){const y=document.getElementById("rt-player-chips");y&&(y.innerHTML=c.map(w=>`
      ...
      </div>`).join(""),h.querySelectorAll("[data-add-player]").forEach(v=>{v.addEventListener("click",()=>{const s=v.dataset.addPlayer;c.some(I=>I.id===s)||c.push({id:s,label:v.dataset.label}),p(),h.style.display="none",document.getElementById("rt-player-search").value=""})})}),(L=document.getElementById("rt-cancel"))==null||L.addEventListener("click",()=>g()),(k=document.getElementById("rt-save"))==null||k.addEventListener("click",async()=>{const y=document.getElementById("rt-error"),w=parseInt(document.getElementById("rt-season").value),h=document.getElementById("rt-label").value.trim(),v=parseInt(document.getElementById("rt-rank-min").value)||1,s=parseInt(document.getElementById("rt-rank-max").value)||1,I=parseInt(document.getElementById("rt-credits").value)||0,$=document.getElementById("rt-activate-at").value,B=$?new Date($).toISOString():null,A=[...document.querySelectorAll(".rt-booster-cb:checked")].map(C=>C.value);if(!h){y.textContent="Le libellé est obligatoire.";return}if(v>s){y.textContent="Le classement min doit être ≤ au max.";return}if(!w){y.textContent="Choisis une saison.";return}const S={season_id:w,label:h,rank_min:v,rank_max:s,credits:I,booster_config_ids:A,player_ids:c.map(C=>C.id),activate_at:B},{error:T}=E?await _.from("season_reward_tiers").update(S).eq("id",e.id):await _.from("season_reward_tiers").insert(S);if(T){y.textContent=T.message;return}d(E?"Palier modifié ✅":"Palier créé ✅","success"),g(),x()})}function Ye(e,{toast:t,openModal:n,closeModal:i,reload:d}){const r=!!e,g=new Date().toISOString().slice(0,16),x=new Date(Date.now()+90*864e5).toISOString().slice(0,16),E=e?new Date(e.start_at).toISOString().slice(0,16):g,a=e?new Date(e.end_at).toISOString().slice(0,16):x,c=(e==null?void 0:e.name)||"";n(r?"✏️ Modifier la saison":"➕ Nouvelle saison",`<div style="display:flex;flex-direction:column;gap:14px">
      <div class="form-group">
    ...
    </div>`);function o(){var u,b;const l=(u=document.getElementById("season-start"))==null?void 0:u.value,m=(b=document.getElementById("season-end"))==null?void 0:b.value,p=document.getElementById("season-duration");if(!l||!m||!p)return;const f=Math.round((new Date(m)-new Date(l))/864e5);p.textContent=f>0?`Durée : ${f} jour${f>1?"s":""}`:"⚠️ La fin doit être après le début",p.style.color=f>0?"var(--gray-600)":"#bb2020"}setTimeout(()=>{var l,m,p;(l=document.getElementById("season-start"))==null||l.addEventListener("input",o),(m=document.getElementById("season-end"))==null||m.addEventListener("input",o),o(),(p=document.getElementById("season-save-btn"))==null||p.addEventListener("click",async()=>{var h,v,s;const f=(h=document.getElementById("season-name"))==null?void 0:h.value.trim(),u=(v=document.getElementById("season-start"))==null?void 0:v.value,b=(s=document.getElementById("season-end"))==null?void 0:s.value,L=document.getElementById("season-error");if(!f){L.textContent="Le nom est requis.";return}if(!u){L.textContent="La date de début est requise.";return}if(!b){L.textContent="La date de fin est requise.";return}if(new Date(b)<=new Date(u)){L.textContent="La date de fin doit être après le début.";return}const k=document.getElementById("season-save-btn");k.disabled=!0,k.textContent="Enregistrement…";const y={name:f,start_at:new Date(u).toISOString(),end_at:new Date(b).toISOString()};let w;if(r?{error:w}=await _.from("ranked_seasons").update(y).eq("id",e.id):{error:w}=await _.from("ranked_seasons").insert({...y,is_active:!1}),w){L.textContent=w.message,k.disabled=!1,k.textContent=r?"💾 Enregistrer":"✅ Créer la saison";return}t(r?"Saison modifiée ✅":"Saison créée ✅","success"),i(),d()})},50)}async function xe(e,t){var E;const{toast:n}=t,{data:i,error:d}=await _.from("solo_levels").select("*").order("level_number");if(d){e.innerHTML=`<p style="color:var(--danger)">${d.message}</p>`;return}const r=a=>`
    <tr data-row="${a.id}">
