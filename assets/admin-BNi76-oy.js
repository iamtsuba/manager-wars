import{s as _,r as Ne,a as Fe,A as nt,l as it,g as ot,b as Pe,K as Ee,D as R,S as at,c as D,H as rt,R as lt,e as ce,d as Oe,f as st,h as dt,E as ct,i as pt,N as ut,M as mt,B as gt,j as ft,F as pe,k as _e,m as bt,n as Ue,o as yt,p as vt}from"./special-cards-CrbcHJPn.js";async function xt(e){var x;const[{count:t},{count:n},{count:i},{count:m},{count:d},{data:p}]=await Promise.all([_.from("players").select("*",{count:"exact",head:!0}),_.from("clubs").select("*",{count:"exact",head:!0}),_.from("users").select("*",{count:"exact",head:!0}),_.from("stadium_definitions").select("*",{count:"exact",head:!0}),_.from("users").select("*",{count:"exact",head:!0}).gt("last_seen_at",new Date(Date.now()-18e4).toISOString()),_.rpc("get_signup_password")]),u=p||"";e.innerHTML=`
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:16px;margin-bottom:24px">
      ...
      <div class="card-panel">
        ...
        <div style="font-size:32px;font-weight:900;color:var(--tile-fg-on-page)">${m??"–"}</div>
  ...
  `,window.adminNav=o=>{var c;(c=document.querySelector(`[data-page="${o}"]`))==null||c.click()},(x=document.getElementById("signup-pwd-save"))==null||x.addEventListener("click",async()=>{const o=document.getElementById("signup-pwd-input").value.trim(),c=document.getElementById("signup-pwd-status");if(!o){c.textContent="Le code ne peut pas être vide.",c.style.color="#ff6b6b";return}const r=document.getElementById("signup-pwd-save");r.disabled=!0,r.textContent="⏳...";const{error:a}=await _.rpc("set_signup_password",{new_password:o});if(r.disabled=!1,r.textContent="💾 Enregistrer",a){c.textContent=a.message,c.style.color="#ff6b6b";return}c.textContent="✅ Code mis à jour.",c.style.color="#2ecc71"})}const Ge={normal:"Normal",pepite:"Pépite",papyte:"Papyte",legende:"Légende"},ht={GK:"#aaaaaa",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};async function Et(){const e={};for(const t of nt)e[t]=await it(t);return e}async function _t(e,t){const{toast:n}=t;if(!confirm(`Réattribuer une nouvelle photo à tous les joueurs ayant encore l'ancien format de chemin (public/faces/...) ?

Cette action est irréversible.`))return;const i=document.getElementById("fix-old-faces-btn");i&&(i.disabled=!0,i.textContent="⏳ En cours...");try{const{data:m,error:d}=await _.from("players").select("id, country_code, face").like("face","public/faces/%");if(d){n(d.message,"error");return}if(!(m!=null&&m.length)){n("Aucun joueur avec l'ancien format — rien à faire ✅","success");return}const{data:p}=await _.from("players").select("face").not("face","is",null),u=new Set((p||[]).map(c=>c.face).filter(c=>c&&!c.startsWith("public/faces/")));let x=0,o=0;for(const c of m){const r=await Fe(c.country_code,u);if(!r){o++;continue}const{error:a}=await _.from("players").update({face:r}).eq("id",c.id);if(a){o++;continue}u.add(r),x++}n(`${x} photo(s) réattribuée(s)${o?`, ${o} échec(s)`:""} ✅`,o?"error":"success")}finally{i&&(i.disabled=!1,i.textContent="🖼️ Réattribuer anciennes photos")}}async function wt(e,t){await le(e,t)}async function le(e,t,n=null){var o,c,r,a,s;const{toast:i}=t;n||(n={search:((o=document.getElementById("search-players"))==null?void 0:o.value)||"",job:((c=document.getElementById("filter-job"))==null?void 0:c.value)||"",rarity:((r=document.getElementById("filter-rarity"))==null?void 0:r.value)||"",club:((a=document.getElementById("filter-club"))==null?void 0:a.value)||"",country:((s=document.getElementById("filter-country"))==null?void 0:s.value)||""});const[{data:m,error:d},{data:p}]=await Promise.all([_.from("players").select("*, clubs(id,encoded_name,logo_url)"),_.from("clubs").select("id,encoded_name").order("encoded_name")]);if(d){e.innerHTML=`<p style="color:red">${d.message}</p>`;return}const u={GK:0,DEF:1,MIL:2,ATT:3},x=(m||[]).sort((l,h)=>{const E=(u[l.job]??4)-(u[h.job]??4);return E!==0?E:(l.surname_real||"").localeCompare(h.surname_real||"")});kt(e,x,p||[],t,n)}function kt(e,t,n,i,m=null){var c,r;const{toast:d}=i;if(e.innerHTML=`
    <div style="display:flex;flex-direction:column;gap:10px">
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        ...
        <select id="filter-rarity" style="width:110px">
          ...
          ${Object.entries(Ge).map(([a,s])=>`<option value="${a}">${s}</option>`).join("")}
        </select>
    ...
    </div>`,m){const a=m,s=l=>document.getElementById(l);a.search&&s("search-players")&&(s("search-players").value=a.search),a.job&&s("filter-job")&&(s("filter-job").value=a.job),a.rarity&&s("filter-rarity")&&(s("filter-rarity").value=a.rarity),a.club&&s("filter-club")&&(s("filter-club").value=a.club),a.country&&s("filter-country")&&(s("filter-country").value=a.country)}if(m){const a=m,s=l=>document.getElementById(l);a.search&&s("search-players")&&(s("search-players").value=a.search),a.job&&s("filter-job")&&(s("filter-job").value=a.job),a.rarity&&s("filter-rarity")&&(s("filter-rarity").value=a.rarity),a.club&&s("filter-club")&&(s("filter-club").value=a.club),a.country&&s("filter-country")&&(s("filter-country").value=a.country)}function p(){const a=document.getElementById("search-players").value.toLowerCase(),s=document.getElementById("filter-job").value,l=document.getElementById("filter-rarity").value,h=document.getElementById("filter-club").value,E=document.getElementById("filter-country").value;return t.filter(b=>(!a||`${b.firstname} ${b.surname_real}`.toLowerCase().includes(a))&&(!s||b.job===s)&&(!l||b.rarity===l)&&(!h||b.club_id===h)&&(!E||b.country_code===E))}const u=new Set;function x(){const a=document.getElementById("bulk-bar"),s=document.getElementById("bulk-count");a&&(a.style.display=u.size>0?"flex":"none",s&&(s.textContent=`${u.size} joueur(s) sélectionné(s)`))}function o(){const a=p();document.getElementById("count-label").textContent=`${a.length} joueur(s)`;const s=document.getElementById("players-list");if(!a.length){s.innerHTML='<div style="color:var(--gray-600);padding:20px;text-align:center">Aucun joueur.</div>';return}s.innerHTML=a.map(l=>{const h={...l,clubs:l.clubs,face:l.face||null},E=Ne(h,{width:120});return`<div style="position:relative;cursor:pointer" data-edit="${l.id}">
        ${E}
      ...
      </div>`}).join(""),s.querySelectorAll("[data-edit]").forEach(l=>{l.addEventListener("click",()=>{const h=t.find(E=>E.id===l.dataset.edit);h&&we(h,n,e,i)})}),s.querySelectorAll(".btn-del-player").forEach(l=>{l.addEventListener("click",async h=>{if(h.stopPropagation(),!confirm("Supprimer ce joueur ?"))return;const{error:E}=await _.from("players").delete().eq("id",l.dataset.del);E?d(E.message,"error"):(d("Joueur supprimé ✅","success"),le(e,i))})})}o(),document.getElementById("search-players").addEventListener("input",o),document.getElementById("filter-job").addEventListener("change",o),document.getElementById("filter-rarity").addEventListener("change",o),document.getElementById("filter-club").addEventListener("change",o),document.getElementById("filter-country").addEventListener("change",o),(c=document.getElementById("bulk-cancel-btn"))==null||c.addEventListener("click",()=>{u.clear(),x(),o()}),(r=document.getElementById("bulk-delete-btn"))==null||r.addEventListener("click",async()=>{var h,E,b,I,k;if(!u.size||!confirm(`Supprimer ${u.size} joueur(s) ?`))return;const a=[...u],{error:s}=await _.from("players").delete().in("id",a);if(s){d(s.message,"error");return}d(`${a.length} joueur(s) supprimé(s) ✅`,"success"),u.clear();const l={search:((h=document.getElementById("search-players"))==null?void 0:h.value)||"",job:((E=document.getElementById("filter-job"))==null?void 0:E.value)||"",rarity:((b=document.getElementById("filter-rarity"))==null?void 0:b.value)||"",club:((I=document.getElementById("filter-club"))==null?void 0:I.value)||"",country:((k=document.getElementById("filter-country"))==null?void 0:k.value)||""};le(e,i,l)}),document.getElementById("add-player-btn").addEventListener("click",()=>we(null,n,e,i)),document.getElementById("fix-old-faces-btn").addEventListener("click",()=>_t(e,i))}async function we(e,t,n,i){const{toast:m,openModal:d,closeModal:p}=i,u=!!e,x=await Et(),{data:o}=await _.from("players").select("face").not("face","is",null),c=new Set((o||[]).map(l=>l.face).filter(Boolean));e!=null&&e.face&&c.delete(e.face);const r='<option value="">— Club —</option>'+t.map(l=>`<option value="${l.id}" ${(e==null?void 0:e.club_id)===l.id?"selected":""}>${l.encoded_name}</option>`).join(""),a=e!=null&&e.face?e.face.split("/")[0]:"",s=Object.keys(x).map(l=>`<option value="${l}" ${a===l?"selected":""}>${l}</option>`).join("");d(u?`✏️ ${e.firstname} ${e.surname_real}`:"➕ Nouveau joueur",`<div style="display:flex;gap:20px;align-items:flex-start">
      ...
      <div style="flex:1;min-width:300px;display:flex;flex-direction:column;gap:12px">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:8px">
          ...
          <div class="form-group">
            <label>Rareté</label>
            <select id="pm-rarity">
              ${["normal","pepite","papyte","legende"].map(l=>`<option value="${l}" ${(e==null?void 0:e.rarity)===l?"selected":""}>${Ge[l]}</option>`).join("")}
            </select>
          </div>
          <div class="form-group">
            <label>Pays</label>
        ...
        <div style="border-top:1px solid var(--gray-200);padding-top:10px">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:8px">
            ${[["GK","pm-g","note_g"],["DEF","pm-d","note_d"],["MIL","pm-m","note_m"],["ATT","pm-a","note_a"]].map(([l,h,E])=>`
              <div class="form-group" style="text-align:center">
                <label style="color:${ht[l]};font-weight:700">${l}</label>
    ...
    <div id="clubs-list" style="display:flex;flex-direction:column;gap:8px"></div>`,p(ae),document.getElementById("search-clubs").addEventListener("input",u=>{const x=u.target.value.toLowerCase();p(ae.filter(o=>o.real_name.toLowerCase().includes(x)||o.encoded_name.toLowerCase().includes(x)))}),document.getElementById("add-club-btn").addEventListener("click",()=>Ie(null,e,t));function p(u){const x=document.getElementById("clubs-list");if(!u.length){x.innerHTML='<p style="color:var(--gray-600);padding:20px">Aucun club.</p>';return}x.innerHTML=u.map(o=>{const c=Je(o),r=Pe(c,o.id).replace("<svg ",'<svg style="width:40px;height:48px" '),a=o.logo_url?`<img src="${o.logo_url}" style="width:40px;height:40px;object-fit:contain;border-radius:8px">`:`<div style="width:40px;height:40px;background:linear-gradient(135deg,${c.color1},${c.color2});border-radius:8px;display:flex;align-items:center;justify-content:center;color:#fff;font-size:10px;font-weight:900">${o.encoded_name.slice(0,3)}</div>`,s=n[o.id]||0;return`
        <div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;background:#fff;box-shadow:0 1px 4px rgba(0,0,0,0.08)">
          ...
          <div style="display:flex;flex-direction:column;gap:4px;flex-shrink:0">
            ${s===0?`<button class="btn btn-primary btn-sm" data-gen="${o.id}" data-cc="${o.country_code}" data-name="${o.real_name}" title="Générer joueurs">⚽</button>`:""}
            <button class="btn btn-ghost btn-sm" data-edit="${o.id}">✏️</button>
            <button class="btn btn-danger btn-sm" data-del="${o.id}">🗑️</button>
          </div>
        </div>`}).join(""),x.querySelectorAll("[data-gen]").forEach(o=>{o.addEventListener("click",()=>{const c=o.dataset.gen,r=o.dataset.name,a=o.dataset.cc;m(`⚽ Générer — ${r}`,`<div style="display:flex;flex-direction:column;gap:14px">
          ...
          </div>`),setTimeout(()=>{var s,l;(s=document.getElementById("qg-cancel"))==null||s.addEventListener("click",()=>d()),(l=document.getElementById("qg-ok"))==null||l.addEventListener("click",async()=>{var E;const h=((E=document.getElementById("qg-strong"))==null?void 0:E.checked)??!1;d(),o.disabled=!0,o.textContent="⏳",await Ke(c,a,i,h),se(e,t)})},50)})}),x.querySelectorAll("[data-edit]").forEach(o=>{const c=ae.find(r=>r.id===o.dataset.edit);o.addEventListener("click",()=>Ie(c,e,t))}),x.querySelectorAll("[data-del]").forEach(o=>{o.addEventListener("click",async()=>{if(!confirm("Supprimer ce club ?"))return;const{error:c}=await _.from("clubs").delete().eq("id",o.dataset.del);c?i(c.message,"error"):(i("Club supprimé","success"),se(e,t))})})}}function Ie(e,t,n){const{toast:i,openModal:m,closeModal:d}=n,p=!!e,u=e?Je(e):{...R},x=Object.entries(Ee).map(([c,r])=>`<option value="${c}" ${u.style===c?"selected":""}>${r.label}</option>`).join(""),o=He.map(c=>`<option value="${c}" ${((e==null?void 0:e.country_code)||"FR")===c?"selected":""}>${c}</option>`).join("");m(p?`✏️ ${e.real_name}`:"➕ Nouveau club",`<div style="display:flex;flex-direction:column;gap:14px">
      ...
      <div style="border-top:1px solid var(--gray-200);padding-top:12px">
        ...
        <div style="display:flex;gap:12px;align-items:flex-start">
          ...
          <div style="flex:1;display:flex;flex-direction:column;gap:8px">
            ...
            ${[["Couleur 1","m-kit-color1",u.color1,!1],["Couleur 2","m-kit-color2",u.color2,!1],["Couleur 3","m-kit-color3",u.color3,!0],["Short","m-kit-shorts",u.shorts,!1],["Chaussettes","m-kit-socks",u.socks,!1]].map(([c,r,a,s])=>`
              <div class="form-group" id="wrap-${r}" ${s?'style="display:none"':""}>
                <label>${c}</label>
    ...
    </div>`),setTimeout(()=>{var b,I,k;ee();function c(){var v,w;const y=((v=document.getElementById("m-kit-style"))==null?void 0:v.value)||"uni",g=((w=Ee[y])==null?void 0:w.colors)===3,f=document.getElementById("wrap-m-kit-color3");f&&(f.style.display=g?"":"none")}c(),["m-kit-color1","m-kit-color2","m-kit-color3","m-kit-shorts","m-kit-socks"].forEach(y=>{const g=document.getElementById(y),f=document.getElementById(y+"-txt");!g||!f||(g.addEventListener("input",()=>{f.value=g.value,ee()}),f.addEventListener("input",()=>{const v=f.value.trim();/^#[0-9a-fA-F]{6}$/.test(v)&&(g.value=v,ee())}),f.addEventListener("change",()=>{let v=f.value.trim();v.startsWith("#")||(v="#"+v),/^#[0-9a-fA-F]{6}$/.test(v)&&(g.value=v,f.value=v,ee())}))}),(b=document.getElementById("m-kit-style"))==null||b.addEventListener("change",()=>{c(),ee()});const r=document.getElementById("m-real"),a=document.getElementById("m-encoded");function s(){if(!r||!a||a.value)return;const y=r.value.trim().split(/\s+/),g=y.length===1?y[0].toUpperCase().slice(0,6):y.filter(f=>f.length>2).map(f=>f[0].toUpperCase()).join("")||y[0].toUpperCase().slice(0,4);a.value=g}r==null||r.addEventListener("input",s),(I=document.getElementById("auto-encode-btn"))==null||I.addEventListener("click",()=>{a&&(a.value=""),s()}),(k=document.getElementById("m-save"))==null||k.addEventListener("click",()=>Pt(e,p,t,n));const l=document.getElementById("m-gen-squad"),h=document.getElementById("m-gen-strong-label");function E(){if(h&&(h.style.display=l!=null&&l.checked?"flex":"none",!(l!=null&&l.checked))){const y=document.getElementById("m-gen-strong");y&&(y.checked=!1)}}l==null||l.addEventListener("change",E),E()},50)}function Ve(){var e,t,n,i,m,d;return{style:((e=document.getElementById("m-kit-style"))==null?void 0:e.value)||"uni",color1:((t=document.getElementById("m-kit-color1"))==null?void 0:t.value)||"#1A6B3C",color2:((n=document.getElementById("m-kit-color2"))==null?void 0:n.value)||"#ffffff",color3:((i=document.getElementById("m-kit-color3"))==null?void 0:i.value)||"#000000",shorts:((m=document.getElementById("m-kit-shorts"))==null?void 0:m.value)||"#111111",socks:((d=document.getElementById("m-kit-socks"))==null?void 0:d.value)||"#ffffff"}}function ee(){const e=document.getElementById("kit-preview-wrap");e&&(e.innerHTML=Pe(Ve(),"modal"))}async function Pt(e,t,n,i){var I,k,y,g,f,v,w;const{toast:m,closeModal:d}=i,p=document.getElementById("m-error"),u=document.getElementById("m-save"),x=(I=document.getElementById("m-real"))==null?void 0:I.value.trim(),o=(k=document.getElementById("m-encoded"))==null?void 0:k.value.trim().toUpperCase(),c=(y=document.getElementById("m-country"))==null?void 0:y.value.trim().toUpperCase(),r=((g=document.getElementById("m-logo"))==null?void 0:g.value.trim())||null,a=((f=document.getElementById("m-gen-stadium"))==null?void 0:f.checked)??!1,s=((v=document.getElementById("m-gen-squad"))==null?void 0:v.checked)??!1,l=((w=document.getElementById("m-gen-strong"))==null?void 0:w.checked)??!1,h=Ve();if(!x){p.textContent="Le nom du club est requis.";return}if(!o){p.textContent="Le nom encodé est requis.";return}if(!c){p.textContent="Le pays est requis.";return}u.disabled=!0,u.textContent="Enregistrement…";const E={real_name:x,encoded_name:o,country_code:c,logo_url:r,kit_style:h.style,kit_color1:h.color1,kit_color2:h.color2,kit_color3:h.color3,kit_shorts:h.shorts,kit_socks:h.socks};let b=e==null?void 0:e.id;if(t){const{error:A}=await _.from("clubs").update(E).eq("id",b);if(A){p.textContent=A.message,u.disabled=!1,u.textContent="💾 Enregistrer";return}}else{const{data:A,error:$}=await _.from("clubs").insert(E).select().single();if($){p.textContent=$.message,u.disabled=!1,u.textContent="✅ Créer le club";return}if(b=A.id,a){u.textContent="🏟️ Création du stade…";const{data:L,error:S}=await _.from("stadium_definitions").insert({name:`Stade de ${x}`,club_id:b,country_code:c}).select().single();S?console.warn("[Stadium] Erreur def stade:",S.message):L&&await _.from("cards").insert({card_type:"stadium",stadium_id:L.id})}s&&(u.textContent="⚽ Génération des joueurs…",await Ke(b,c,m,l))}m(t?"Club modifié ✅":"Club créé ✅","success"),d(),se(n,i)}const Ot=["normal","pepite","papyte","legende"],$e=["GK","DEF","MIL","ATT"],Ut=[["MA","Maroc"],["FR","France"],["AR","Argentine"],["PT","Portugal"],["BR","Brésil"],["ES","Espagne"],["DE","Allemagne"],["GB","Angleterre"],["IT","Italie"],["CM","Cameroun"],["SN","Sénégal"],["NG","Nigéria"],["DK","Danemark"],["NL","Pays-Bas"],["BE","Belgique"],["CI","Côte d'Ivoire"],["AL","Albanie"],["HR","Croatie"],["RS","Serbie"],["TR","Turquie"],["MW","Malawi"]];let q={...D},re=[];async function Gt(e,{toast:t}){const{data:n}=await _.from("clubs").select("id,encoded_name,logo_url,kit_style,kit_color1,kit_color2,kit_shorts,kit_socks").order("encoded_name");re=n||[],q={...D},e.innerHTML=Jt(re),qt(e,re,t),G()}function Ae(){var n;const e=((n=document.getElementById("cb-surname-real"))==null?void 0:n.value)||"",t=document.getElementById("cb-surname-enc");t&&(t.value=ce(e))}function Ht(){var n;const e=(n=document.getElementById("cb-club"))==null?void 0:n.value,t=re.find(i=>String(i.id)===String(e));return t?{style:t.kit_style||R.style,color1:t.kit_color1||R.color1,color2:t.kit_color2||R.color2,shorts:t.kit_shorts||R.shorts,socks:t.kit_socks||R.socks}:{...R}}function G(){var a,s,l,h;const e=E=>{var b;return((b=document.getElementById(E))==null?void 0:b.value)||""},t={firstname:e("cb-firstname"),surname_real:e("cb-surname-enc")||ce(e("cb-surname-real")),country_code:e("cb-country"),club_encoded_name:((s=(a=document.getElementById("cb-club"))==null?void 0:a.selectedOptions[0])==null?void 0:s.text)||"",job:e("cb-job"),job2:e("cb-job2")||null,note_g:parseInt(e("cb-note-g"))||0,note_d:parseInt(e("cb-note-d"))||0,note_m:parseInt(e("cb-note-m"))||0,note_a:parseInt(e("cb-note-a"))||0,rarity:e("cb-rarity")},n=Ht(),i=document.getElementById("cb-club"),m=((h=(l=i==null?void 0:i.selectedOptions[0])==null?void 0:l.dataset)==null?void 0:h.logo)||null,d=Oe(q,n,120),p=`data:image/svg+xml;charset=utf-8,${encodeURIComponent(d)}`,u=st(t,{portraitUrl:p,clubLogoUrl:m,showNotes:!0}),x=document.getElementById("card-preview");x&&(x.innerHTML=u);const o=document.getElementById("avatar-preview-wrap");o&&(o.innerHTML=d);const c=e("cb-surname-real"),r=document.getElementById("encode-summary");r&&c&&(r.textContent=`${e("cb-firstname")} ${c} → ${e("cb-firstname")} ${t.surname_real}`)}function Ye(){const e=t=>{var n;return((n=document.getElementById(t))==null?void 0:n.value)||""};q={skin:e("av-skin"),hair:e("av-hair"),hairColor:e("av-hairColor"),eyes:e("av-eyes"),nose:e("av-nose"),mouth:e("av-mouth"),eyebrows:e("av-eyebrows"),beard:e("av-beard")}}function qt(e,t,n){var m,d,p,u,x;e.querySelectorAll("input,select").forEach(o=>{o.addEventListener("input",G),o.addEventListener("change",G)});const i=["av-skin","av-hair","av-hairColor","av-eyes","av-nose","av-mouth","av-eyebrows","av-beard"];i.forEach(o=>{var c;(c=document.getElementById(o))==null||c.addEventListener("change",()=>{Ye(),G()})}),(m=document.getElementById("cb-surname-real"))==null||m.addEventListener("input",()=>{Ae(),G()}),(d=document.getElementById("btn-encode"))==null||d.addEventListener("click",()=>{Ae(),G()}),(p=document.getElementById("cb-club"))==null||p.addEventListener("change",G),(u=document.getElementById("btn-save-player"))==null||u.addEventListener("click",()=>Kt(n)),(x=document.getElementById("btn-reset"))==null||x.addEventListener("click",()=>{e.querySelectorAll("input").forEach(o=>o.value=""),e.querySelectorAll("select").forEach(o=>o.selectedIndex=0),q={...D},i.forEach(o=>{const c=o.replace("av-",""),r=document.getElementById(o);r&&(r.value=D[c]||"")}),G()})}async function Kt(e){const t=p=>{var u;return((u=document.getElementById(p))==null?void 0:u.value)||""},n=t("cb-surname-real").trim(),i=t("cb-surname-enc").trim()||ce(n);if(!t("cb-firstname")||!n||!t("cb-country")||!t("cb-job")){e("Remplissez les champs obligatoires (prénom, nom, pays, poste)","error");return}Ye();const m={firstname:t("cb-firstname").trim(),surname_real:n,surname_real:i,country_code:t("cb-country"),club_id:t("cb-club")||null,job:t("cb-job"),job2:t("cb-job2")||null,note_g:parseInt(t("cb-note-g"))||0,note_d:parseInt(t("cb-note-d"))||0,note_m:parseInt(t("cb-note-m"))||0,note_a:parseInt(t("cb-note-a"))||0,rarity:t("cb-rarity")||"normal",note_min:parseInt(t("cb-note-min"))||null,note_max:parseInt(t("cb-note-max"))||null,sell_price:parseInt(t("cb-price"))||0,avatar_config:q,skin:q.skin||"blanc",hair:q.hairColor||"marron",hair_length:"court"},{error:d}=await _.from("players").insert(m);if(d){e("Erreur: "+d.message,"error");return}e(`✅ Joueur "${m.firstname} ${m.surname_real}" enregistré !`,"success")}function Jt(e){const t=Ut.map(([o,c])=>`<option value="${o}">${c} (${o})</option>`).join(""),n='<option value="">— Sélectionner un club —</option>'+e.map(o=>`<option value="${o.id}" data-logo="${o.logo_url||""}">${o.encoded_name}</option>`).join(""),i=$e.map(o=>`<option value="${o}">${o}</option>`).join(""),m='<option value="">Aucun</option>'+$e.map(o=>`<option value="${o}">${o}</option>`).join(""),d=Ot.map(o=>`<option value="${o}">${o.charAt(0).toUpperCase()+o.slice(1)}</option>`).join(""),p=(o,c,r)=>Object.entries(c).map(([a,s])=>`<option value="${a}" ${a===(r||"")?"selected":""}>${s.label}</option>`).join(""),u=Object.keys(at).map(o=>`<option value="${o}" ${o===D.skin?"selected":""}>${o.charAt(0).toUpperCase()+o.slice(1)}</option>`).join(""),x=Object.keys(rt).map(o=>`<option value="${o}" ${o===D.hairColor?"selected":""}>${o.charAt(0).toUpperCase()+o.slice(1)}</option>`).join("");return`
  ...
  `,d(m),document.getElementById("search-users").addEventListener("input",p=>{const u=p.target.value.toLowerCase();d(m.filter(x=>{var o;return x.pseudo.toLowerCase().includes(u)||((o=x.club_name)==null?void 0:o.toLowerCase().includes(u))}))});function d(p){document.getElementById("users-tbody").innerHTML=p.map(u=>{const x=u.mmr??1e3,o=u.mmr_deviation??350,c=u.placement_matches??0,r=ft(x),a=u.ranked_wins??0,s=u.ranked_losses??0,l=u.ranked_draws??0,h=a+s+l,E=h>0?Math.round(a/h*100):null,b=c<10,I=o<80?"#1A6B3C":o<150?"#e67e22":"#bb2020";return`
        <tr>
          <td>
          ...
          <td style="font-size:12px">
            ...
            ${b?`<div style="font-size:10px;color:#e67e22">${c}/10 placement${c<10?` (${10-c} restants)`:""}</div>`:`<div style="font-size:10px;color:var(--gray-600)">${a}V ${l}N ${s}D${E!==null?` · ${E}% WR`:""}</div>`}
          </td>
        ...
        </tr>`}).join(""),document.querySelectorAll("[data-toggle-admin]").forEach(u=>{u.addEventListener("click",async()=>{const x=u.dataset.isAdmin!=="true";if(!confirm(`Voulez-vous ${x?"rendre admin":"retirer le rôle admin"} de ce Manager ?`))return;const{error:o}=await _.from("users").update({is_admin:x}).eq("id",u.dataset.toggleAdmin);o?t(o.message,"error"):(t("Rôle mis à jour ✅","success"),We(e,{toast:t}))})})}}async function Ze(e,{toast:t}){const{data:n,error:i}=await _.from("market_listings").select(`
    ...
    `).order("listed_at",{ascending:!1}).limit(100);if(i){e.innerHTML=`<p style="color:red">${i.message}</p>`;return}const m=n||[],d=m.filter(u=>u.status==="active").length,p=m.filter(u=>u.status==="sold").length;e.innerHTML=`
    ...
    <div class="card-panel">
      <div class="table-wrap">
        <table>
          ...
          <tbody>
            ${m.map(u=>{var r,a,s,l;const x=(r=u.card)==null?void 0:r.player,o=x?`${x.firstname} ${x.surname_real}`:((a=u.card)==null?void 0:a.card_type)||"—",c={active:"#1A6B3C",sold:"#D4A017",cancelled:"#aaa"};return`<tr>
                <td><b>${o}</b>${x?`<div style="font-size:10px;color:#999">${x.rarity} · ${x.job}</div>`:""}</td>
                <td style="font-size:12px">${((s=u.seller)==null?void 0:s.pseudo)||"—"}</td>
                <td style="font-size:12px">${((l=u.buyer)==null?void 0:l.pseudo)||"—"}</td>
                <td style="font-size:13px;font-weight:600">${(u.price||0).toLocaleString("fr")} cr.</td>
                <td>
                  <span style="background:${c[u.status]};color:#fff;padding:2px 6px;border-radius:3px;font-size:10px;font-weight:700">
                    ${u.status.toUpperCase()}
                  </span>
                </td>
                <td style="font-size:11px;color:#aaa">${new Date(u.listed_at).toLocaleDateString("fr")}</td>
                <td>
                  ${u.status==="active"?`<button class="btn btn-danger btn-sm" data-cancel="${u.id}">Annuler</button>`:""}
                </td>
              </tr>`}).join("")}
          </tbody>
        </table>
  ...
  `,document.querySelectorAll("[data-cancel]").forEach(u=>{u.addEventListener("click",async()=>{if(!confirm("Annuler cette annonce ?"))return;const{error:x}=await _.from("market_listings").update({status:"cancelled"}).eq("id",u.dataset.cancel);x?t(x.message,"error"):(t("Annonce annulée","success"),Ze(e,{toast:t}))})})}async function Vt(e,{toast:t}){e.innerHTML=`
...
`;n.forEach(d=>{m+=[d.real_name,d.encoded_name,d.country_code,d.logo_url||""].map(Le).join(",")+`
`}),ne("clubs_export.csv",m),t(`${n.length} clubs exportés`,"success")}),document.getElementById("export-players").addEventListener("click",async()=>{const{data:n,error:i}=await _.from("players").select("firstname,surname_real,surname_real,country_code,job,job2,note_g,note_d,note_m,note_a,rarity,note_min,note_max,skin,hair,hair_length,sell_price,clubs(encoded_name)").order("surname_real");if(i){t(i.message,"error");return}if(!n||n.length===0){t("Aucun joueur à exporter","info");return}let m=`firstname,surname_real,surname_real,country_code,club_encoded_name,job,job2,note_g,note_d,note_m,note_a,rarity,note_min,note_max,skin,hair,hair_length,sell_price
`;n.forEach(d=>{var p;m+=[d.firstname,d.surname_real,d.surname_real,d.country_code,((p=d.clubs)==null?void 0:p.encoded_name)||"",d.job,d.job2||"",d.note_g,d.note_d,d.note_m,d.note_a,d.rarity,d.note_min??"",d.note_max??"",d.skin,d.hair,d.hair_length,d.sell_price].map(Le).join(",")+`
`}),ne("players_export.csv",m),t(`${n.length} joueurs exportés`,"success")}),document.getElementById("import-clubs").addEventListener("change",async n=>{const i=n.target.files[0];if(!i)return;const m=document.getElementById("clubs-import-result");m.innerHTML='<span style="color:var(--gray-600)">Import en cours...</span>';try{const d=await i.text(),p=Se(d);if(p.length===0){m.innerHTML='<span style="color:#c0392b">Fichier vide</span>';return}let u=0,x=0;const o=[];for(const c of p){if(!c.real_name||!c.encoded_name||!c.country_code){x++,o.push(`Ligne ignorée (champs manquants): ${c.real_name||"?"}`);continue}const r={real_name:c.real_name,encoded_name:c.encoded_name.toUpperCase(),country_code:c.country_code.toUpperCase().slice(0,2),logo_url:c.logo_url||null},{error:a}=await _.from("clubs").upsert(r,{onConflict:"encoded_name"});a?(x++,o.push(`${c.encoded_name}: ${a.message}`)):u++}m.innerHTML=`<div style="color:var(--green)">✅ ${u} clubs importés</div>
        ${x>0?`<div style="color:#c0392b">❌ ${x} erreurs</div><div style="font-size:11px;color:var(--gray-600);max-height:100px;overflow-y:auto">${o.slice(0,10).join("<br>")}</div>`:""}`,t(`${u} clubs importés`,"success")}catch(d){m.innerHTML=`<span style="color:#c0392b">Erreur : ${d.message}</span>`}n.target.value=""}),document.getElementById("import-players").addEventListener("change",async n=>{const i=n.target.files[0];if(!i)return;const m=document.getElementById("players-import-result");m.innerHTML='<span style="color:var(--gray-600)">Import en cours...</span>';try{const d=await i.text(),p=Se(d);if(p.length===0){m.innerHTML='<span style="color:#c0392b">Fichier vide</span>';return}const{data:u}=await _.from("clubs").select("id,encoded_name"),x={};(u||[]).forEach(b=>{x[b.encoded_name.toUpperCase()]=b.id});let o=0,c=0;const r=[],a=["GK","DEF","MIL","ATT"],s=["normal","pepite","papyte","legende"],l=["blanc","metisse","typ","noir"],h=["blond","marron","noir","chauve"],E=["rase","court","milong","long"];for(const b of p){if(!b.firstname||!b.surname_real||!b.country_code||!b.job){c++,r.push(`Ligne ignorée (champs requis manquants): ${b.firstname||"?"}`);continue}if(!a.includes(b.job)){c++,r.push(`${b.firstname}: job invalide "${b.job}"`);continue}const I=b.club_encoded_name&&x[b.club_encoded_name.toUpperCase()]||null,k={firstname:b.firstname,surname_real:b.surname_real,surname_real:b.surname_real||ce(b.surname_real),country_code:b.country_code.toUpperCase().slice(0,2),club_id:I,job:b.job,job2:a.includes(b.job2)?b.job2:null,note_g:parseInt(b.note_g)||0,note_d:parseInt(b.note_d)||0,note_m:parseInt(b.note_m)||0,note_a:parseInt(b.note_a)||0,rarity:s.includes(b.rarity)?b.rarity:"normal",note_min:b.note_min!==""&&b.note_min!=null?parseInt(b.note_min):null,note_max:b.note_max!==""&&b.note_max!=null?parseInt(b.note_max):null,skin:l.includes(b.skin)?b.skin:"blanc",hair:h.includes(b.hair)?b.hair:"noir",hair_length:E.includes(b.hair_length)?b.hair_length:"court",sell_price:parseInt(b.sell_price)||0},{error:y}=await _.from("players").insert(k);y?(c++,r.push(`${b.firstname} ${b.surname_real}: ${y.message}`)):o++}m.innerHTML=`<div style="color:var(--green)">✅ ${o} joueurs importés</div>
...
`)?'"'+t.replace(/"/g,'""')+'"':t}function ne(e,t){const n=new Blob(["\uFEFF"+t],{type:"text/csv;charset=utf-8;"}),i=URL.createObjectURL(n),m=document.createElement("a");m.href=i,m.download=e,m.click(),URL.revokeObjectURL(i)}function Se(e){e=e.replace(/^\uFEFF/,"").replace(/\r\n/g,`
`).replace(/\r/g,`
`);const t=e.split(`
`).filter(p=>p.trim());if(t.length<2)return[];const n=t[0],i=n.split(";").length>n.split(",").length?";":",",m=Be(n,i).map(p=>p.trim().replace(/^\uFEFF/,"").toLowerCase()),d=[];for(let p=1;p<t.length;p++){if(!t[p].trim())continue;const u=Be(t[p],i),x={};m.forEach((o,c)=>{x[o]=(u[c]||"").trim()}),!Object.values(x).every(o=>!o)&&d.push(x)}return d}function Be(e,t=","){const n=[];let i="",m=!1;for(let d=0;d<e.length;d++){const p=e[d];m?p==='"'?e[d+1]==='"'?(i+='"',d++):m=!1:i+=p:p==='"'?m=!0:p===t?(n.push(i),i=""):i+=p}return n.push(i),n}const Yt={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};async function Wt(e,{toast:t}){const n=Object.keys(pe);let i=n[0];const{data:m}=await _.from("formation_links_overrides").select("formation, links"),d={};(m||[]).forEach(c=>{d[c.formation]=c.links});let p=new Set;function u(c,r){return[c,r].sort().join("-")}function x(c){const r=d[c]||pe[c]||[];p=new Set(r.map(([a,s])=>u(a,s)))}x(i),e.innerHTML=`
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;gap:12px;flex-wrap:wrap">
  ...
  `;function o(){const c=bt[i]||{},r=_e(i),a=320,s=400,l=22;function h(y){const g=c[y];return g?{x:g.x*a,y:g.y*s}:null}let E=`<svg width="${a}" height="${s}" viewBox="0 0 ${a} ${s}" xmlns="http://www.w3.org/2000/svg" style="display:block">`;r.forEach(([y,g],f)=>{const v=h(y),w=h(g);if(!v||!w)return;const A=u(y,g),$=p.has(A),L=$?"#3b82f6":"#999",S=$?.95:.35,B=$?4:3;E+=`<line x1="${v.x}" y1="${v.y}" x2="${w.x}" y2="${w.y}"
        stroke="transparent" stroke-width="14" class="link-hit" data-key="${A}" style="cursor:pointer"/>`,E+=`<line x1="${v.x}" y1="${v.y}" x2="${w.x}" y2="${w.y}"
        stroke="${L}" stroke-width="${B}" stroke-dasharray="${$?"none":"4,3"}"
        opacity="${S}" pointer-events="none" data-line-key="${A}"/>`});for(const y of Object.keys(c)){const g=h(y);if(!g)continue;const f=y.replace(/\d+/,""),v=Yt[f]||"#555";E+=`<circle cx="${g.x}" cy="${g.y}" r="${l}" fill="${v}" stroke="rgba(255,255,255,0.7)" stroke-width="2" pointer-events="none"/>`,E+=`<text x="${g.x}" y="${g.y+4}" text-anchor="middle" font-size="10" font-weight="900" fill="white" font-family="Arial Black,Arial" pointer-events="none">${y}</text>`}E+="</svg>",document.getElementById("field-wrap").innerHTML=E;const b=document.getElementById("links-list");b.innerHTML=r.map(([y,g])=>{const f=u(y,g),v=p.has(f);return`
        ...
        </button>`}).join("");const I=e.querySelector('div[style*="font-weight:700;font-size:13px;margin-bottom:8px"]');I&&(I.textContent=`Liens (${p.size}/${r.length} actifs)`);function k(y){p.has(y)?p.delete(y):p.add(y),o()}e.querySelectorAll(".link-hit").forEach(y=>{y.addEventListener("click",()=>k(y.dataset.key))}),e.querySelectorAll(".link-toggle").forEach(y=>{y.addEventListener("click",()=>k(y.dataset.key))})}o(),document.getElementById("formation-select").addEventListener("change",c=>{i=c.target.value,x(i),o()}),document.getElementById("reset-btn").addEventListener("click",()=>{const c=pe[i]||[];p=new Set(c.map(([r,a])=>u(r,a))),o(),t("Liens réinitialisés aux valeurs par défaut (non sauvegardé)","info")}),document.getElementById("save-btn").addEventListener("click",async()=>{const r=_e(i).filter(([s,l])=>p.has(u(s,l))),{error:a}=await _.from("formation_links_overrides").upsert({formation:i,links:r,updated_at:new Date().toISOString()});if(a){t(a.message,"error");return}d[i]=r,t(`Liens enregistrés pour ${i} (${r.length} actifs)`,"success")})}const Zt=[{value:"player",label:"Joueur"},{value:"formation",label:"Formation"},{value:"game_changer",label:"Game Changer"},{value:"ultra_game_changer",label:"Ultra Game Changer"},{value:"game_helper",label:"Game Helper"},{value:"stadium",label:"🏟️ Stade"}],Qt=[{value:"",label:"Toutes"},{value:"normal",label:"Normal"},{value:"pepite",label:"Pépite"},{value:"papyte",label:"Papyte"},{value:"legende",label:"Légende"}],Xt=[{value:"credits",label:"Crédits"},{value:"pub",label:"Publicité"},{value:"free",label:"Gratuit"}];async function en(e){e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">Chargement...</div>';const{data:t}=await _.from("booster_configs").select("*").order("sort_order");let n=null,i=[],m=null;const d=()=>window.innerWidth<700;async function p(){if(!n){i=[];return}if(n!==m){const{data:r}=await _.from("booster_drop_rates").select("*").eq("booster_id",n).order("sort_order");i=r||[],m=n}}function u(){return`
    ...
    </div>`}function x(){const r=n?(t||[]).find(l=>l.id===n):null;if(!r)return`<div style="display:flex;align-items:center;justify-content:center;flex:1;color:#bbb;font-size:14px;padding:40px;text-align:center">
      ← Sélectionnez un booster ou créez-en un nouveau
    </div>`;const a=i.reduce((l,h)=>l+Number(h.percentage||0),0),s=Math.abs(a-100)<.1;return`
    <div id="booster-editor" style="flex:1;overflow-y:auto;background:#f8f8f8;padding:14px">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
          <div>
            ...
            <select id="f-price-type" style="width:100%;padding:9px;border:1px solid #ddd;border-radius:6px;font-size:13px">
              ${Xt.map(l=>`<option value="${l.value}" ${r.price_type===l.value?"selected":""}>${l.label}</option>`).join("")}
            </select>
          </div>
      ...
      <div style="background:#fff;border-radius:10px;padding:14px;margin-bottom:14px">
        ...
        ${i.length===0?'<div style="text-align:center;padding:16px;color:#aaa;font-size:13px">Aucun taux. Cliquez + Ligne.</div>':i.map((l,h)=>`
          <div style="display:grid;grid-template-columns:2fr 1.5fr 50px 50px 60px 32px;gap:4px;align-items:center;margin-bottom:6px;padding:6px;background:#f9f9f9;border-radius:6px" data-rate-idx="${h}">
            <select class="rate-type" data-idx="${h}" style="padding:6px 4px;border:1px solid #ddd;border-radius:4px;font-size:12px;width:100%">
              ${Zt.map(E=>`<option value="${E.value}" ${l.card_type===E.value?"selected":""}>${E.label}</option>`).join("")}
            </select>
            <select class="rate-rarity" data-idx="${h}" style="padding:6px 4px;border:1px solid #ddd;border-radius:4px;font-size:12px;width:100%">
              ${Qt.map(E=>`<option value="${E.value}" ${(l.rarity||"")===E.value?"selected":""}>${E.label}</option>`).join("")}
            </select>
            <input class="rate-note-min" data-idx="${h}" type="number" min="0" max="20" value="${l.note_min||""}" placeholder="min" style="padding:6px 4px;border:1px solid #ddd;border-radius:4px;text-align:center;font-size:12px;width:100%">
            <input class="rate-note-max" data-idx="${h}" type="number" min="0" max="20" value="${l.note_max||""}" placeholder="max" style="padding:6px 4px;border:1px solid #ddd;border-radius:4px;text-align:center;font-size:12px;width:100%">
            <input class="rate-pct" data-idx="${h}" type="number" min="0.1" max="100" step="0.1" value="${l.percentage}" style="padding:6px 4px;border:1px solid #ddd;border-radius:4px;text-align:center;font-size:12px;width:100%">
    ...
    </div>`}async function o(r=!1){r||await p();const a=!n&&d(),s=n&&d();e.innerHTML=`
    <div style="display:flex;height:calc(100vh - 56px);overflow:hidden">
      ${!d()||a||!n?u():""}
      ${!d()||s?x():""}
    </div>`,c()}function c(){var s,l,h,E,b,I,k;const r=y=>e.querySelector(y);e.querySelectorAll(".booster-row").forEach(y=>{y.addEventListener("click",g=>{g.target.closest(".btn-delete")||(n=y.dataset.id,m=null,o())})}),(s=r("#btn-back"))==null||s.addEventListener("click",()=>{n=null,o()}),(l=r("#btn-new"))==null||l.addEventListener("click",async()=>{const y=prompt("Nom du nouveau booster :");if(!(y!=null&&y.trim()))return;const{data:g,error:f}=await _.from("booster_configs").insert({name:y.trim(),price_type:"credits",price_credits:1e3,card_count:5}).select().single();if(f){alert(f.message);return}t.push(g),n=g.id,m=null,o()}),e.querySelectorAll(".btn-delete").forEach(y=>{y.addEventListener("click",async g=>{if(g.stopPropagation(),!confirm("Supprimer ce booster ?"))return;await _.from("booster_configs").delete().eq("id",y.dataset.id);const f=t.findIndex(v=>v.id===y.dataset.id);f>-1&&t.splice(f,1),n===y.dataset.id&&(n=null,m=null),o()})}),(h=r("#btn-cancel"))==null||h.addEventListener("click",()=>{n=null,o()}),(E=r("#f-price-type"))==null||E.addEventListener("change",y=>{const g=r("#credits-field");g&&(g.style.opacity=y.target.value!=="credits"?"0.4":"1")}),(b=r("#btn-pick-icon"))==null||b.addEventListener("click",async()=>{var g;const y=r("#icon-picker-grid");if(y){if(y.style.display!=="none"){y.style.display="none";return}y.innerHTML='<div style="padding:10px;text-align:center;color:#888;font-size:12px">⏳ Chargement...</div>',y.style.display="block";try{const v=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),w=Array.isArray(v)?v.filter($=>$.name.startsWith("booster")&&/\.(png|jpg|svg|webp)$/i.test($.name)):[];if(!w.length){y.innerHTML='<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Aucune icône "booster*" trouvée.</div>';return}const A=((g=r("#f-image-url"))==null?void 0:g.value)||"";y.innerHTML=`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(72px,1fr));gap:8px">
          ${w.map($=>`
        ...
        </div>`,y.querySelectorAll(".icon-pick-item").forEach($=>{$.addEventListener("click",()=>{const L=r("#f-image-url");L&&(L.value=$.dataset.name),y.style.display="none"})})}catch(f){y.innerHTML=`<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Erreur API GitHub : ${f.message}</div>`}}});function a(){e.querySelectorAll("[data-rate-idx]").forEach(y=>{var f,v,w,A,$;const g=Number(y.dataset.rateIdx);g>=0&&g<i.length&&(i[g].card_type=((f=y.querySelector(".rate-type"))==null?void 0:f.value)||"player",i[g].rarity=((v=y.querySelector(".rate-rarity"))==null?void 0:v.value)||null,i[g].note_min=Number((w=y.querySelector(".rate-note-min"))==null?void 0:w.value)||null,i[g].note_max=Number((A=y.querySelector(".rate-note-max"))==null?void 0:A.value)||null,i[g].percentage=Number(($=y.querySelector(".rate-pct"))==null?void 0:$.value)||0)})}(I=r("#btn-add-rate"))==null||I.addEventListener("click",()=>{a(),i.push({id:null,booster_id:n,card_type:"player",rarity:"normal",note_min:null,note_max:null,percentage:10,sort_order:i.length}),o(!0)}),e.querySelectorAll(".btn-del-rate").forEach(y=>{y.addEventListener("click",()=>{a(),i.splice(Number(y.dataset.idx),1),o(!0)})}),(k=r("#btn-save"))==null||k.addEventListener("click",async()=>{var A,$,L,S,B,T,z,M,N,F,U,P,j;const y=(t||[]).find(C=>C.id===n);if(!y)return;const g=[];e.querySelectorAll("[data-rate-idx]").forEach(C=>{var Y,W,Z,Q,X;const K=Number(C.dataset.rateIdx);g.push({booster_id:n,card_type:((Y=C.querySelector(".rate-type"))==null?void 0:Y.value)||"player",rarity:((W=C.querySelector(".rate-rarity"))==null?void 0:W.value)||null,note_min:Number((Z=C.querySelector(".rate-note-min"))==null?void 0:Z.value)||null,note_max:Number((Q=C.querySelector(".rate-note-max"))==null?void 0:Q.value)||null,percentage:Number((X=C.querySelector(".rate-pct"))==null?void 0:X.value)||0,sort_order:K});const O=g[g.length-1];O.rarity||(O.rarity=null),O.note_min||(O.note_min=null),O.note_max||(O.note_max=null)});const f=g.reduce((C,K)=>C+K.percentage,0);if(g.length&&Math.abs(f-100)>.5){alert(`La somme doit faire 100% (actuellement ${f.toFixed(1)}%)`);return}const v={name:(($=(A=r("#f-name"))==null?void 0:A.value)==null?void 0:$.trim())||y.name,image_url:((S=(L=r("#f-image-url"))==null?void 0:L.value)==null?void 0:S.trim())||null,price_type:(B=r("#f-price-type"))==null?void 0:B.value,price_credits:Number((T=r("#f-price-credits"))==null?void 0:T.value)||0,card_count:Number((z=r("#f-card-count"))==null?void 0:z.value)||5,is_active:((M=r("#f-active"))==null?void 0:M.checked)??y.is_active,allow_duplicates:((N=r("#f-allow-dup"))==null?void 0:N.checked)??!0,sort_order:Number((F=r("#f-sort"))==null?void 0:F.value)||0,max_per_user:(U=r("#f-max-per-user"))!=null&&U.value?Number(r("#f-max-per-user").value):null,available_from:((P=r("#f-available-from"))==null?void 0:P.value)||null,available_until:((j=r("#f-available-until"))==null?void 0:j.value)||null},{error:w}=await _.from("booster_configs").update(v).eq("id",n);if(w){alert(w.message);return}if(Object.assign(y,v),await _.from("booster_drop_rates").delete().eq("booster_id",n),g.length){const{error:C}=await _.from("booster_drop_rates").insert(g);if(C){alert(C.message);return}}i=g,m=n,alert("✅ Booster enregistré !"),o(!0)})}o()}const Qe=[{value:"normal",label:"Normal",color:"#888"},{value:"pepite",label:"Pépite",color:"#D4A017"},{value:"papyte",label:"Papyte",color:"#909090"},{value:"legende",label:"Légende",color:"#7a28b8"}];async function tn(e){await fe(e)}async function fe(e){const{data:t,error:n}=await _.from("sell_price_configs").select("*").order("rarity").order("note_min");if(n){e.innerHTML=`<div style="padding:24px;color:red">Erreur : ${n.message}</div>`;return}e.innerHTML=`
    ...
    </div>`,document.getElementById("add-config").addEventListener("click",()=>Ce(null)),document.getElementById("form-cancel").addEventListener("click",()=>{document.getElementById("config-form").style.display="none"}),document.getElementById("form-save").addEventListener("click",()=>on(e)),e.querySelectorAll("[data-edit]").forEach(i=>{const m=(t||[]).find(d=>d.id===i.dataset.edit);m&&i.addEventListener("click",()=>Ce(m))}),e.querySelectorAll("[data-delete]").forEach(i=>{i.addEventListener("click",async()=>{confirm("Supprimer cette règle ?")&&(await _.from("sell_price_configs").delete().eq("id",i.dataset.delete),await fe(e))})})}function nn(e){const t=Qe.find(n=>n.value===e.rarity);return`
    ...
    </tr>`}function Ce(e){document.getElementById("config-form").style.display="block",document.getElementById("form-title").textContent=e?"Modifier la règle":"Nouvelle règle",document.getElementById("form-id").value=(e==null?void 0:e.id)||"",document.getElementById("form-rarity").value=(e==null?void 0:e.rarity)||"normal",document.getElementById("form-price").value=(e==null?void 0:e.price)||1e3,document.getElementById("form-note-min").value=(e==null?void 0:e.note_min)??1,document.getElementById("form-note-max").value=(e==null?void 0:e.note_max)??10,document.getElementById("config-form").scrollIntoView({behavior:"smooth"})}async function on(e){const t=document.getElementById("form-id").value,n=document.getElementById("form-rarity").value,i=parseInt(document.getElementById("form-price").value)||0,m=parseInt(document.getElementById("form-note-min").value)||1,d=parseInt(document.getElementById("form-note-max").value)||10;if(m>d){alert("Note min doit être ≤ note max");return}const p={rarity:n,price:i,note_min:m,note_max:d,updated_at:new Date().toISOString()};let u;if(t?{error:u}=await _.from("sell_price_configs").update(p).eq("id",t):{error:u}=await _.from("sell_price_configs").insert(p),u){alert("Erreur : "+u.message);return}document.getElementById("config-form").style.display="none",await fe(e)}async function an(e){await be(e)}async function be(e){const{data:t}=await _.from("patch_notes").select("*").order("published_at",{ascending:!1});e.innerHTML=`
    ...
    </div>`,document.getElementById("add-article").addEventListener("click",()=>Te(null)),document.getElementById("form-cancel").addEventListener("click",()=>{document.getElementById("article-form").style.display="none"}),document.getElementById("form-save").addEventListener("click",()=>ln(e)),e.querySelectorAll("[data-edit]").forEach(n=>{const i=(t||[]).find(m=>m.id===n.dataset.edit);i&&n.addEventListener("click",()=>Te(i))}),e.querySelectorAll("[data-delete]").forEach(n=>{n.addEventListener("click",async()=>{confirm("Supprimer cet article ?")&&(await _.from("patch_notes").delete().eq("id",n.dataset.delete),await be(e))})})}function rn(e){const t=new Date(e.published_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"short",year:"numeric"});return`
    ...
    </div>`}function Te(e){const t=document.getElementById("article-form");t.style.display="block",document.getElementById("form-title").textContent=e?"Modifier l'article":"Nouvel article",document.getElementById("form-id").value=(e==null?void 0:e.id)||"",document.getElementById("form-art-title").value=(e==null?void 0:e.title)||"",document.getElementById("form-art-desc").value=(e==null?void 0:e.description)||"",document.getElementById("form-art-img").value=(e==null?void 0:e.image_url)||"",document.getElementById("form-art-pub").checked=e?e.is_published:!0;const n=new Date((e==null?void 0:e.published_at)||Date.now()),i=new Date(n.getTime()-n.getTimezoneOffset()*6e4).toISOString().slice(0,16);document.getElementById("form-art-date").value=i,t.scrollIntoView({behavior:"smooth"})}async function ln(e){const t=document.getElementById("form-id").value,n=document.getElementById("form-art-title").value.trim(),i=document.getElementById("form-art-desc").value.trim(),m=document.getElementById("form-art-img").value.trim()||null,d=document.getElementById("form-art-pub").checked,p=document.getElementById("form-art-date").value;if(!n){alert("Le titre est obligatoire.");return}if(!i){alert("La description est obligatoire.");return}const u=p?new Date(p).toISOString():new Date().toISOString(),x={title:n,description:i,image_url:m,is_published:d,published_at:u};let o;if(t?{error:o}=await _.from("patch_notes").update(x).eq("id",t):{error:o}=await _.from("patch_notes").insert(x),o){alert("Erreur : "+o.message);return}document.getElementById("article-form").style.display="none",await be(e)}async function sn(e){await V(e)}async function V(e){var c,r,a,s,l,h,E,b,I,k,y,g;const{data:t}=await _.from("tutorial_steps").select("*").order("step_order");e.innerHTML=`
    <div style="padding:20px;max-width:820px;margin:0 auto">
      ...
      <div id="ts-form" style="display:none;margin-top:24px;background:#fff;border-radius:12px;padding:20px;box-shadow:0 1px 8px rgba(0,0,0,0.1)">
        ...
        <div style="display:flex;gap:10px">
    ...
    </div>`,(c=document.getElementById("ts-add"))==null||c.addEventListener("click",()=>ze(null,(t==null?void 0:t.length)||0)),(r=document.getElementById("ts-reset"))==null||r.addEventListener("click",async()=>{if(!confirm("Remettre tutorial_done = false pour TOUS les joueurs ? Ils reverront le tutoriel à leur prochaine connexion."))return;const{error:f}=await _.from("users").update({tutorial_done:!1}).neq("id","00000000-0000-0000-0000-000000000000");alert(f?"Erreur : "+f.message:"✅ Tutorial_done réinitialisé pour tous les joueurs.")}),(a=document.getElementById("ts-cancel"))==null||a.addEventListener("click",Xe),(s=document.getElementById("ts-save"))==null||s.addEventListener("click",()=>pn(e)),(l=document.getElementById("ts-preview-btn"))==null||l.addEventListener("click",cn);const n=document.getElementById("ts-editor"),i=()=>{const f=document.getElementById("ts-content");f&&n&&(f.value=n.innerHTML)};n==null||n.addEventListener("input",i),n==null||n.addEventListener("blur",i);const m=()=>{document.activeElement!==n&&(n==null||n.focus())};(h=document.getElementById("ts-toolbar"))==null||h.querySelectorAll("[data-cmd]").forEach(f=>{f.addEventListener("mousedown",v=>{v.preventDefault(),m();const w=f.dataset.cmd;w.startsWith("formatBlock:")?document.execCommand("formatBlock",!1,w.split(":")[1]):document.execCommand(w,!1,null),i()})}),(E=document.getElementById("ts-insert-color"))==null||E.addEventListener("mousedown",f=>{f.preventDefault(),m();const v=prompt("Couleur hex (ex: #D4A017 pour or, #1A6B3C pour vert, #cc2222 pour rouge) :");v&&(document.execCommand("foreColor",!1,v),i())}),(b=document.getElementById("ts-clear-format"))==null||b.addEventListener("mousedown",f=>{f.preventDefault(),m(),document.execCommand("removeFormat",!1,null),i()}),n==null||n.addEventListener("focus",()=>{n&&(n.style.borderColor="#1A6B3C")}),n==null||n.addEventListener("blur",()=>{n&&(n.style.borderColor="#ddd")});const d=document.getElementById("ts-image"),p=document.getElementById("ts-img-preview"),u=document.getElementById("ts-img-preview-el"),x=document.getElementById("ts-img-picker-grid"),o=()=>{var v;const f=(v=d==null?void 0:d.value)==null?void 0:v.trim();if(f){const w="/";u.src=`${w}icons/${f}`,p.style.display="block"}else p.style.display="none"};d==null||d.addEventListener("input",o),(I=document.getElementById("ts-img-clear"))==null||I.addEventListener("click",()=>{d&&(d.value=""),p.style.display="none",x.style.display="none"}),(k=document.getElementById("ts-img-pick"))==null||k.addEventListener("click",async()=>{if(x.style.display!=="none"){x.style.display="none";return}x.innerHTML='<div style="color:#aaa;font-size:12px">Chargement…</div>',x.style.display="block";try{const v=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),w=Array.isArray(v)?v.filter($=>$.name.startsWith("tuto_")):[];if(!w.length){x.innerHTML='<div style="color:#aaa;font-size:12px">Aucun fichier tuto_* trouvé dans /icons/.<br>Uploade des images nommées tuto_xxx.png dans public/icons/</div>';return}const A="/";x.innerHTML='<div style="display:flex;flex-wrap:wrap;gap:8px">'+w.map($=>`
          ...
          </div>`).join("")+"</div>",x.querySelectorAll("[data-pick]").forEach($=>{$.addEventListener("click",()=>{d&&(d.value=$.dataset.pick),o(),x.style.display="none"})})}catch(f){x.innerHTML='<div style="color:#cc2222;font-size:12px">Erreur chargement : '+f.message+"</div>"}}),(y=document.getElementById("ts-color"))==null||y.addEventListener("input",f=>{const v=document.getElementById("ts-color-hex");v&&(v.value=f.target.value)}),(g=document.getElementById("ts-color-hex"))==null||g.addEventListener("input",f=>{const v=f.target.value;if(/^#[0-9a-fA-F]{6}$/.test(v)){const w=document.getElementById("ts-color");w&&(w.value=v)}}),e.querySelectorAll("[data-edit]").forEach(f=>{const v=(t||[]).find(w=>w.id===f.dataset.edit);v&&f.addEventListener("click",()=>ze(v))}),e.querySelectorAll("[data-delete]").forEach(f=>{f.addEventListener("click",async()=>{confirm("Supprimer cette étape ?")&&(await _.from("tutorial_steps").delete().eq("id",f.dataset.delete),V(e))})}),e.querySelectorAll("[data-toggle]").forEach(f=>{f.addEventListener("click",async()=>{const v=(t||[]).find(w=>w.id===f.dataset.toggle);v&&(await _.from("tutorial_steps").update({is_active:!v.is_active}).eq("id",v.id),V(e))})}),e.querySelectorAll("[data-up]").forEach(f=>{f.addEventListener("click",async()=>{const v=t||[],w=v.findIndex(A=>A.id===f.dataset.up);w<=0||(await Promise.all([_.from("tutorial_steps").update({step_order:v[w-1].step_order}).eq("id",v[w].id),_.from("tutorial_steps").update({step_order:v[w].step_order}).eq("id",v[w-1].id)]),V(e))})}),e.querySelectorAll("[data-down]").forEach(f=>{f.addEventListener("click",async()=>{const v=t||[],w=v.findIndex(A=>A.id===f.dataset.down);w<0||w>=v.length-1||(await Promise.all([_.from("tutorial_steps").update({step_order:v[w+1].step_order}).eq("id",v[w].id),_.from("tutorial_steps").update({step_order:v[w].step_order}).eq("id",v[w+1].id)]),V(e))})})}function dn(e){return`
  <div style="background:#fff;border-radius:10px;padding:12px 16px;box-shadow:0 1px 4px rgba(0,0,0,0.08);display:flex;align-items:center;gap:12px">
  ...
  </div>`}function ze(e,t=0){document.getElementById("ts-form").style.display="block",document.getElementById("ts-form-title").textContent=e?"Modifier l'étape":"Nouvelle étape",document.getElementById("ts-id").value=(e==null?void 0:e.id)||"",document.getElementById("ts-emoji").value=(e==null?void 0:e.emoji)||"⚽",document.getElementById("ts-title").value=(e==null?void 0:e.title)||"",document.getElementById("ts-content").value=(e==null?void 0:e.content)||"";const n=document.getElementById("ts-editor");n&&(n.innerHTML=(e==null?void 0:e.content)||"<p>Texte de l'étape…</p>"),document.getElementById("ts-order").value=(e==null?void 0:e.step_order)??t,document.getElementById("ts-active").checked=e?e.is_active:!0;const i=(e==null?void 0:e.color)||"#1A6B3C";document.getElementById("ts-color").value=i,document.getElementById("ts-color-hex").value=i;const m=(e==null?void 0:e.image_url)||"",d=document.getElementById("ts-image");d&&(d.value=m);const p=document.getElementById("ts-img-preview"),u=document.getElementById("ts-img-preview-el");if(m&&p&&u){const x="/";u.src=`${x}icons/${m}`,p.style.display="block"}else p&&(p.style.display="none");document.getElementById("ts-img-picker-grid").style.display="none",document.getElementById("ts-preview-area").innerHTML="",document.getElementById("ts-form").scrollIntoView({behavior:"smooth"})}function Xe(){document.getElementById("ts-form").style.display="none",document.getElementById("ts-preview-area").innerHTML=""}function cn(){var o,c;const e=document.getElementById("ts-emoji").value||"⚽",t=document.getElementById("ts-title").value||"Titre",n=document.getElementById("ts-editor"),i=n?n.innerHTML:document.getElementById("ts-content").value||"",m=document.getElementById("ts-color-hex").value||"#1A6B3C",d=(c=(o=document.getElementById("ts-image"))==null?void 0:o.value)==null?void 0:c.trim(),u=d?`<div style="padding:0 20px 12px;text-align:center"><img src="/icons/${d}" style="max-height:140px;max-width:100%;border-radius:10px;object-fit:contain"></div>`:"",x=document.getElementById("ts-preview-area");x.innerHTML=`
    ...
    </div>`,x.querySelectorAll("ul,ol").forEach(r=>{r.style.paddingLeft="20px",r.style.margin="6px 0"}),x.querySelectorAll("li").forEach(r=>{r.style.marginBottom="4px"}),x.querySelectorAll("p").forEach(r=>{r.style.marginBottom="8px"})}async function pn(e){var r,a;const t=document.getElementById("ts-id").value,n=document.getElementById("ts-emoji").value.trim()||"⚽",i=document.getElementById("ts-title").value.trim(),m=document.getElementById("ts-editor"),d=(m?m.innerHTML:document.getElementById("ts-content").value).trim(),p=document.getElementById("ts-color-hex").value.trim()||"#1A6B3C",u=parseInt(document.getElementById("ts-order").value)||0,x=document.getElementById("ts-active").checked;if(!i||!d){alert("Titre et contenu sont obligatoires.");return}const o={emoji:n,title:i,content:d,color:p,step_order:u,is_active:x,image_url:((a=(r=document.getElementById("ts-image"))==null?void 0:r.value)==null?void 0:a.trim())||null};let c;if(t?{error:c}=await _.from("tutorial_steps").update(o).eq("id",t):{error:c}=await _.from("tutorial_steps").insert(o),c){alert("Erreur : "+c.message);return}Xe(),V(e)}const et="/",un=[["MA","Maroc"],["FR","France"],["AR","Argentine"],["PT","Portugal"],["BR","Brésil"],["ES","Espagne"],["DE","Allemagne"],["GB","Angleterre"],["IT","Italie"],["CM","Cameroun"],["SN","Sénégal"],["NG","Nigéria"],["DK","Danemark"],["NL","Pays-Bas"],["BE","Belgique"],["CI","Côte d'Ivoire"],["AL","Albanie"],["HR","Croatie"],["RS","Serbie"],["TR","Turquie"],["MW","Malawi"],["GH","Ghana"],["ZA","Afrique du Sud"],["EG","Égypte"],["US","États-Unis"],["MX","Mexique"],["CO","Colombie"],["UY","Uruguay"],["CL","Chili"],["JP","Japon"],["KR","Corée du Sud"],["CN","Chine"],["AU","Australie"],["SW","Suisse"],["SE","Suède"],["NO","Norvège"],["PL","Pologne"]];async function mn(e,t){await me(e,t)}async function me(e,t){var m;const[{data:n},{data:i}]=await Promise.all([_.from("stadium_definitions").select("*, club:clubs(encoded_name, logo_url)").order("name"),_.from("clubs").select("id, encoded_name, logo_url").order("encoded_name")]);e.innerHTML=`
    <div style="padding:20px;max-width:1100px;margin:0 auto">
      ...
      <div style="display:flex;flex-wrap:wrap;gap:14px" id="st-list">
        ${(n||[]).length?(n||[]).map(d=>gn(d)).join(""):'<div style="color:var(--tile-fg-dim);padding:24px;text-align:center;width:100%">Aucun stade créé.</div>'}
      </div>
    </div>`,(m=document.getElementById("st-add-btn"))==null||m.addEventListener("click",()=>Me(null,i,e,t)),e.querySelectorAll("[data-edit-stadium]").forEach(d=>{d.addEventListener("click",()=>{const p=(n||[]).find(u=>u.id===d.dataset.editStadium);p&&Me(p,i,e,t)})})}function gn(e){var m,d;let t=null;e.image_url?t=e.image_url.startsWith("http")?e.image_url:`${et}icons/${e.image_url}`:(m=e.club)!=null&&m.logo_url?t=e.club.logo_url:e.country_code&&(t=`https://flagsapi.com/${e.country_code.slice(0,2).toUpperCase()}/flat/64.png`);const n=((d=e.club)==null?void 0:d.encoded_name)||e.country_code||"—",i=Ue(e.name,t,`${n}<br>+10 ⭐`,{width:140});return`<div data-edit-stadium="${e.id}" style="cursor:pointer;transition:transform .15s" onmouseover="this.style.transform='translateY(-3px)'" onmouseout="this.style.transform=''">
    ${i}
  </div>`}function Me(e,t,n,i){var c,r,a;const{openModal:m,closeModal:d,toast:p}=i,u=`
      ...
      <div style="flex:1;min-width:220px;display:flex;flex-direction:column;gap:12px">
        ...
        <div>
          ...
          <select id="st-club">
            ...
            ${(t||[]).map(s=>`<option value="${s.id}" data-logo="${s.logo_url||""}" ${(e==null?void 0:e.club_id)===s.id?"selected":""}>${s.encoded_name}</option>`).join("")}
          </select>
        </div>
        <div>
          ...
          <select id="st-country">
            ...
            ${un.map(([s,l])=>`<option value="${s}" ${(e==null?void 0:e.country_code)===s?"selected":""}>${l} (${s})</option>`).join("")}
          </select>
        </div>
        <div>
  ...
  `;m(e?`Modifier : ${e.name}`:"Nouveau stade",u,`
    <button id="st-cancel" class="btn btn-ghost">Annuler</button>
    <button id="st-save" class="btn btn-primary">Enregistrer</button>
  `);const o=()=>{var v,w,A,$,L,S,B;const s=((v=document.getElementById("st-name"))==null?void 0:v.value)||"NOM DU STADE",l=(A=(w=document.getElementById("st-image"))==null?void 0:w.value)==null?void 0:A.trim(),h=(L=($=document.getElementById("st-country"))==null?void 0:$.value)==null?void 0:L.trim(),E=document.getElementById("st-club"),b=(E==null?void 0:E.selectedIndex)||0,I=E&&b>0?E.options[b].text:"",k=((B=(S=E==null?void 0:E.options[b])==null?void 0:S.getAttribute)==null?void 0:B.call(S,"data-logo"))||"";let y=null;l?y=l.startsWith("http")?l:`${et}icons/${l}`:k?y=k:h&&(y=`https://flagsapi.com/${h.toLowerCase().slice(0,2).toUpperCase()}/flat/64.png`);const g=I||h||"—",f=document.getElementById("st-preview-card");f&&(f.innerHTML=Ue(s,y,`${g}<br>+10 ⭐`,{width:140}))};o(),["st-name","st-club","st-country","st-image"].forEach(s=>{var l,h;(l=document.getElementById(s))==null||l.addEventListener("input",o),(h=document.getElementById(s))==null||h.addEventListener("change",o)}),(c=document.getElementById("st-cancel"))==null||c.addEventListener("click",()=>d()),(r=document.getElementById("st-save"))==null||r.addEventListener("click",async()=>{const s=document.getElementById("st-name").value.trim(),l=document.getElementById("st-club").value||null,h=document.getElementById("st-country").value.trim().toUpperCase()||null,E=document.getElementById("st-image").value.trim()||null;if(!s){p("Le nom est obligatoire","error");return}const b={name:s,club_id:l,country_code:h,image_url:E},{error:I}=e?await _.from("stadium_definitions").update(b).eq("id",e.id):await _.from("stadium_definitions").insert(b);if(I){p("Erreur : "+I.message,"error");return}p(e?"Stade modifié ✅":"Stade créé ✅","success"),d(),me(n,i)}),(a=document.getElementById("st-delete"))==null||a.addEventListener("click",async()=>{confirm("Supprimer ce stade ?")&&(await _.from("stadium_definitions").delete().eq("id",e.id),p("Stade supprimé","success"),d(),me(n,i))})}const H=[{value:"game_changer",label:"⚡ Game Changer",border:"#7a28b8",bg:"linear-gradient(135deg,#4a0080,#7a28b8)"},{value:"ultra_game_changer",label:"💎 Ultra Game Changer",border:"#00bcd4",bg:"linear-gradient(135deg,#006080,#00bcd4)"}],te=[{value:"purple",label:"Violet",hex:"#7a28b8"},{value:"light_blue",label:"Bleu clair",hex:"#00bcd4"}];async function fn(e){e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">Chargement...</div>';const{data:t}=await _.from("gc_definitions").select("*").order("sort_order").order("created_at");let n=null;const i=[{value:"BOOST_STAT",label:"⚡ Boost stats (+N)",hasValue:!0,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"DEBUFF_STAT",label:"💀 Debuff stats (-N)",hasValue:!0,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"GRAY_PLAYER",label:"❄️ Griser un joueur",hasValue:!1,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"REVIVE_PLAYER",label:"💫 Ressusciter un joueur",hasValue:!1,hasTarget:!1,hasCount:!0,hasRoles:!1},{value:"REMOVE_GOAL",label:"🚫 Retirer un but adverse",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1},{value:"ADD_GOAL_DRAW",label:"⚽ +1 but si duel nul",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1},{value:"ADD_SUB",label:"🔄 Ajouter un remplacement",hasValue:!1,hasTarget:!1,hasCount:!0,hasRoles:!1},{value:"CUSTOM",label:"🛠️ Effet custom (hardcodé)",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1}],m=["GK","DEF","MIL","ATT"],d=()=>window.innerWidth<700,p=a=>e.querySelector(a);function u(a,s=130){if(!a)return"";const l=H.find(E=>E.value===a.gc_type)||H[0],h=a.image_url?`/icons/${a.image_url}`:null;return yt(a.name||"?",h,l.label.split(" ")[0],a.effect||"",{width:s})}function x(){return`
    <div id="gc-list" style="background:#fff;${d()?"":"border-right:1px solid #e0e0e0;width:280px;flex-shrink:0;"}overflow-y:auto;height:100%">
      ...
      ${(t||[]).map(a=>{const s=H.find(h=>h.value===a.gc_type)||H[0],l=te.find(h=>h.value===a.color)||te[0];return`
        <div class="gc-row" data-id="${a.id}"
          ...
          <div style="width:36px;height:50px;border-radius:5px;border:2px solid ${l.hex};background:${s.bg};display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:18px">
            ${a.image_url?`<img src="/icons/${a.image_url}" style="width:28px;height:28px;object-fit:contain">`:s.label.split(" ")[0]}
          </div>
          <div style="flex:1;min-width:0">
            <div style="font-size:13px;font-weight:700;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${a.name}</div>
            <div style="font-size:11px;color:#888">${s.label} · ${a.is_active?"✅ Actif":"⛔ Inactif"}</div>
          </div>
          <div style="display:flex;flex-direction:column;gap:4px;flex-shrink:0">
    ...
    </div>`}function o(){const a=n?(t||[]).find(s=>s.id===n):null;return a?(H.find(s=>s.value===a.gc_type)||H[0],te.find(s=>s.value===a.color)||te[0],`
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
          <div>
            ...
            <select id="gc-type" style="width:100%;padding:9px;border:1px solid #ddd;border-radius:6px;font-size:13px">
              ${H.map(s=>`<option value="${s.value}" ${a.gc_type===s.value?"selected":""}>${s.label}</option>`).join("")}
            </select>
          </div>
          <div>
            ...
            <select id="gc-color" style="width:100%;padding:9px;border:1px solid #ddd;border-radius:6px;font-size:13px">
              ${te.map(s=>`<option value="${s.value}" ${a.color===s.value?"selected":""}>${s.label}</option>`).join("")}
            </select>
          </div>
        </div>
        ...
        <div style="border-top:1px solid #eee;padding-top:12px;margin-top:4px">
          ...
          <div>
            ...
            <select id="gc-effect-type" style="width:100%;padding:9px;border:1px solid #ddd;border-radius:6px;font-size:13px;margin-bottom:10px">
              ${i.map(s=>`<option value="${s.value}" ${(a.effect_type||"BOOST_STAT")===s.value?"selected":""}>${s.label}</option>`).join("")}
            </select>
          </div>
          ${(()=>{const s=a.effect_params||{},l=i.find(h=>h.value===(a.effect_type||"BOOST_STAT"))||i[0];return`
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
              ${l.hasValue?`<div>
                <label style="font-size:10px;color:#666;font-weight:700;display:block;margin-bottom:3px">VALEUR (+N)</label>
                <input id="gc-p-value" type="number" min="1" max="5" value="${s.value||2}" style="width:100%;padding:7px;border:1px solid #ddd;border-radius:4px;font-size:13px;box-sizing:border-box">
              </div>`:""}
              ${l.hasCount?`<div>
                <label style="font-size:10px;color:#666;font-weight:700;display:block;margin-bottom:3px">NB JOUEURS</label>
                <input id="gc-p-count" type="number" min="1" max="11" value="${s.count||1}" style="width:100%;padding:7px;border:1px solid #ddd;border-radius:4px;font-size:13px;box-sizing:border-box">
              </div>`:""}
              ${l.hasTarget?`<div>
              ...
              ${l.hasRoles?`<div>
                ...
                <div style="display:flex;gap:4px;flex-wrap:wrap">
                  ${m.map(h=>`<label style="display:flex;align-items:center;gap:2px;font-size:11px;cursor:pointer">
                    <input type="checkbox" class="gc-p-role" value="${h}" ${!s.roles||s.roles.includes(h)?"checked":""}> ${h}
                  </label>`).join("")}
                </div>
              </div>`:""}
            </div>`})()}
    ...
    <div style="display:flex;height:calc(100vh - 56px);overflow:hidden">
      ${!d()||!n?x():""}
      ${!d()||n?o():""}
    </div>`,r()}function r(){var a,s,l,h,E;e.querySelectorAll(".gc-row").forEach(b=>{b.addEventListener("click",I=>{I.target.closest(".btn-toggle-gc,.btn-delete-gc")||(n=b.dataset.id,c())})}),(a=p("#btn-gc-back"))==null||a.addEventListener("click",()=>{n=null,c()}),(s=p("#btn-cancel-gc"))==null||s.addEventListener("click",()=>{n=null,c()}),(l=p("#btn-new-gc"))==null||l.addEventListener("click",async()=>{const b=prompt("Nom de la carte Game Changer :");if(!(b!=null&&b.trim()))return;const{data:I,error:k}=await _.from("gc_definitions").insert({name:b.trim(),gc_type:"game_changer",color:"purple"}).select().single();if(k){alert(k.message);return}t.push(I),n=I.id,c()}),e.querySelectorAll(".btn-toggle-gc").forEach(b=>{b.addEventListener("click",async I=>{I.stopPropagation();const k=b.dataset.active!=="true";await _.from("gc_definitions").update({is_active:k}).eq("id",b.dataset.id);const y=t.find(g=>g.id===b.dataset.id);y&&(y.is_active=k),c()})}),e.querySelectorAll(".btn-delete-gc").forEach(b=>{b.addEventListener("click",async I=>{if(I.stopPropagation(),!confirm("Supprimer cette carte Game Changer ?"))return;await _.from("gc_definitions").delete().eq("id",b.dataset.id);const k=t.findIndex(y=>y.id===b.dataset.id);k>-1&&t.splice(k,1),n===b.dataset.id&&(n=null),c()})}),(h=p("#btn-pick-gc-icon"))==null||h.addEventListener("click",async()=>{var I;const b=p("#gc-icon-picker");if(b){if(b.style.display!=="none"){b.style.display="none";return}b.innerHTML='<div style="padding:10px;text-align:center;color:#888;font-size:12px">⏳ Chargement...</div>',b.style.display="block";try{const y=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),g=Array.isArray(y)?y.filter(v=>v.name.startsWith("gamechanger-")&&/\.(png|jpg|svg|webp)$/i.test(v.name)):[];if(!g.length){b.innerHTML='<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Aucun fichier "gamechanger-*" trouvé dans public/icons/</div>';return}const f=((I=p("#gc-image-url"))==null?void 0:I.value)||"";b.innerHTML=`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(72px,1fr));gap:8px">
          ${g.map(v=>`
        ...
        </div>`,b.querySelectorAll(".gc-icon-item").forEach(v=>{v.addEventListener("click",()=>{const w=p("#gc-image-url");w&&(w.value=v.dataset.name),b.style.display="none"})})}catch(k){b.innerHTML=`<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Erreur API GitHub : ${k.message}</div>`}}}),(E=p("#btn-save-gc"))==null||E.addEventListener("click",async()=>{var v,w,A,$,L,S,B,T,z,M,N,F,U,P;const b=t.find(j=>j.id===n);if(!b)return;const I=i.find(j=>{var C;return j.value===(((C=p("#gc-effect-type"))==null?void 0:C.value)||"BOOST_STAT")})||i[0],k=I.hasRoles?[...e.querySelectorAll(".gc-p-role:checked")].map(j=>j.value):null,y={...I.hasValue?{value:Number((v=p("#gc-p-value"))==null?void 0:v.value)||2}:{},...I.hasCount?{count:Number((w=p("#gc-p-count"))==null?void 0:w.value)||1}:{},...I.hasTarget?{target:((A=p("#gc-p-target"))==null?void 0:A.value)||"home"}:{},...I.hasRoles?{roles:k!=null&&k.length?k:null}:{}},g={name:((L=($=p("#gc-name"))==null?void 0:$.value)==null?void 0:L.trim())||b.name,effect:((B=(S=p("#gc-effect"))==null?void 0:S.value)==null?void 0:B.trim())||null,image_url:((z=(T=p("#gc-image-url"))==null?void 0:T.value)==null?void 0:z.trim())||null,gc_type:((M=p("#gc-type"))==null?void 0:M.value)||"game_changer",color:((N=p("#gc-color"))==null?void 0:N.value)||"purple",sort_order:Number((F=p("#gc-sort"))==null?void 0:F.value)||0,is_active:((U=p("#gc-active"))==null?void 0:U.checked)??b.is_active,effect_type:((P=p("#gc-effect-type"))==null?void 0:P.value)||"BOOST_STAT",effect_params:y},{error:f}=await _.from("gc_definitions").update(g).eq("id",n);if(f){alert(f.message);return}Object.assign(b,g),alert("✅ Carte enregistrée !"),c()})}c()}async function bn(e,{toast:t,openModal:n,closeModal:i}){await J(e,{toast:t,openModal:n,closeModal:i})}async function J(e,t){var y;const{toast:n,openModal:i,closeModal:m}=t,[{data:d,error:p},{data:u}]=await Promise.all([_.from("ranked_seasons").select("*").order("start_at",{ascending:!1}),_.from("users").select("mmr,rank_tier,placement_matches").gte("placement_matches",1)]);if(p){e.innerHTML=`<p style="color:red">${p.message}</p>`;return}const x=d||[],o=x.find(g=>g.is_active),c=(u||[]).filter(g=>(g.placement_matches||0)>=1),r=c.filter(g=>(g.placement_matches||0)>=10),a=["bronze","silver","gold","platinum","diamond","master"],s={bronze:"Bronze",silver:"Argent",gold:"Or",platinum:"Platine",diamond:"Diamant",master:"Maître"},l={bronze:"#cd7f32",silver:"#a0a0a0",gold:"#D4A017",platinum:"#00e5ff",diamond:"#5b9bd5",master:"#9b59b6"},h={bronze:"🥉",silver:"🥈",gold:"🥇",platinum:"💎",diamond:"🔷",master:"👑"},E={};a.forEach(g=>{E[g]=0}),r.forEach(g=>{const f=g.rank_tier||"bronze";E[f]!==void 0&&E[f]++});const b=r.length?Math.round(r.reduce((g,f)=>g+(f.mmr||1e3),0)/r.length):0;function I(g){return g?new Date(g).toLocaleDateString("fr",{day:"numeric",month:"short",year:"numeric"}):"—"}function k(g){const f=new Date;return g.is_active?'<span style="background:#1A6B3C;color:#fff;padding:2px 8px;border-radius:10px;font-size:11px;font-weight:700">● ACTIVE</span>':new Date(g.end_at)<f?'<span style="background:#555;color:#ccc;padding:2px 8px;border-radius:10px;font-size:11px">Terminée</span>':'<span style="background:#e67e22;color:#fff;padding:2px 8px;border-radius:10px;font-size:11px">À venir</span>'}e.innerHTML=`
    <div style="display:flex;flex-direction:column;gap:20px">
      ...
      <div class="card-panel" style="background:linear-gradient(135deg,#0a3d1e,#1a6b3c20);border:1.5px solid #1A6B3C40">
        ...
        <div style="display:flex;flex-direction:column;gap:4px">
          ${a.map(g=>{const f=E[g],v=r.length>0?Math.round(f/r.length*100):0;return`
              <div style="display:flex;align-items:center;gap:8px;font-size:12px">
                <span style="width:80px">${h[g]} ${s[g]}</span>
                <div style="flex:1;background:rgba(255,255,255,0.08);border-radius:4px;height:14px;overflow:hidden">
                  <div style="height:100%;width:${v}%;background:${l[g]};border-radius:4px;transition:width 0.5s"></div>
                </div>
      ...
      <div style="display:flex;justify-content:space-between;align-items:center">
        <div style="font-weight:700;font-size:14px">Toutes les saisons (${x.length})</div>
        ...
        <div class="table-wrap">
          <table>
            ...
            <tbody id="seasons-tbody">
              ${x.length===0?'<tr><td colspan="6" style="text-align:center;color:var(--gray-600);padding:24px">Aucune saison créée.</td></tr>':x.map(g=>{const f=Math.round((new Date(g.end_at)-new Date(g.start_at))/864e5),v=new Date(g.end_at)<new Date&&!g.is_active;return`
                    <tr>
                      <td><b>${g.name}</b></td>
    ...
    </div>`,(y=document.getElementById("create-season-btn"))==null||y.addEventListener("click",()=>{Re(null,{toast:n,openModal:i,closeModal:m,reload:()=>J(e,t)})}),e.querySelectorAll("[data-edit]").forEach(g=>{const f=x.find(v=>v.id==g.dataset.edit);g.addEventListener("click",()=>{Re(f,{toast:n,openModal:i,closeModal:m,reload:()=>J(e,t)})})}),e.querySelectorAll("[data-activate]").forEach(g=>{g.addEventListener("click",async()=>{const f=parseInt(g.dataset.activate);if(!confirm("Activer cette saison ? La saison actuellement active sera désactivée."))return;const{error:v}=await _.from("ranked_seasons").update({is_active:!1}).neq("id",0);if(v){n(v.message,"error");return}const{error:w}=await _.from("ranked_seasons").update({is_active:!0}).eq("id",f);if(w){n(w.message,"error");return}await _.from("users").update({current_season_id:f}).gt("placement_matches",0),n("Saison activée ✅","success"),J(e,t)})}),e.querySelectorAll("[data-deactivate]").forEach(g=>{g.addEventListener("click",async()=>{if(!confirm("Désactiver la saison active ? Le mode Ranked sera suspendu."))return;const{error:f}=await _.from("ranked_seasons").update({is_active:!1}).eq("id",parseInt(g.dataset.deactivate));if(f){n(f.message,"error");return}n("Saison désactivée","success"),J(e,t)})}),e.querySelectorAll("[data-delete]").forEach(g=>{g.addEventListener("click",async()=>{if(!confirm("Supprimer cette saison archivée ?"))return;const{error:f}=await _.from("ranked_seasons").delete().eq("id",parseInt(g.dataset.delete));if(f){n(f.message,"error");return}n("Saison supprimée","success"),J(e,t)})})}function Re(e,{toast:t,openModal:n,closeModal:i,reload:m}){const d=!!e,p=new Date().toISOString().slice(0,16),u=new Date(Date.now()+90*864e5).toISOString().slice(0,16),x=e?new Date(e.start_at).toISOString().slice(0,16):p,o=e?new Date(e.end_at).toISOString().slice(0,16):u,c=(e==null?void 0:e.name)||"";n(d?"✏️ Modifier la saison":"➕ Nouvelle saison",`<div style="display:flex;flex-direction:column;gap:14px">
      <div class="form-group">
    ...
    </div>`);function r(){var E,b;const a=(E=document.getElementById("season-start"))==null?void 0:E.value,s=(b=document.getElementById("season-end"))==null?void 0:b.value,l=document.getElementById("season-duration");if(!a||!s||!l)return;const h=Math.round((new Date(s)-new Date(a))/864e5);l.textContent=h>0?`Durée : ${h} jour${h>1?"s":""}`:"⚠️ La fin doit être après le début",l.style.color=h>0?"var(--gray-600)":"#bb2020"}setTimeout(()=>{var a,s,l;(a=document.getElementById("season-start"))==null||a.addEventListener("input",r),(s=document.getElementById("season-end"))==null||s.addEventListener("input",r),r(),(l=document.getElementById("season-save-btn"))==null||l.addEventListener("click",async()=>{var f,v,w;const h=(f=document.getElementById("season-name"))==null?void 0:f.value.trim(),E=(v=document.getElementById("season-start"))==null?void 0:v.value,b=(w=document.getElementById("season-end"))==null?void 0:w.value,I=document.getElementById("season-error");if(!h){I.textContent="Le nom est requis.";return}if(!E){I.textContent="La date de début est requise.";return}if(!b){I.textContent="La date de fin est requise.";return}if(new Date(b)<=new Date(E)){I.textContent="La date de fin doit être après le début.";return}const k=document.getElementById("season-save-btn");k.disabled=!0,k.textContent="Enregistrement…";const y={name:h,start_at:new Date(E).toISOString(),end_at:new Date(b).toISOString()};let g;if(d?{error:g}=await _.from("ranked_seasons").update(y).eq("id",e.id):{error:g}=await _.from("ranked_seasons").insert({...y,is_active:!1}),g){I.textContent=g.message,k.disabled=!1,k.textContent=d?"💾 Enregistrer":"✅ Créer la saison";return}t(d?"Saison modifiée ✅":"Saison créée ✅","success"),i(),m()})},50)}vt(Oe);function yn(e,t="info"){const n=document.getElementById("toast");n.textContent=e,n.className=`show ${t}`,clearTimeout(n._t),n._t=setTimeout(()=>{n.className=""},3e3)}function vn(e,t,n=""){document.getElementById("modal-title").textContent=e,document.getElementById("modal-body").innerHTML=t,document.getElementById("modal-footer").innerHTML=n,document.getElementById("modal-overlay").classList.remove("hidden")}function ge(){document.getElementById("modal-overlay").classList.add("hidden")}const De={dashboard:{title:"Dashboard",fn:xt},players:{title:"Joueurs & Cartes",fn:wt},clubs:{title:"Clubs",fn:Nt},"card-builder":{title:"Card Builder",fn:Gt},users:{title:"Managers",fn:We},market:{title:"Marché des transferts",fn:Ze},"import-export":{title:"Import / Export CSV",fn:Vt},formations:{title:"Formations & Liens",fn:Wt},"boosters-config":{title:"Boosters",fn:en},"sell-price":{title:"Prix vente directe",fn:tn},journal:{title:"Journal",fn:an},tutorial:{title:"Tutoriel",fn:sn},stadiums:{title:"Stades",fn:mn},"gc-cards":{title:"Game Changers",fn},"ranked-seasons":{title:"Saisons Ranked",fn:bn}};async function de(e){document.querySelectorAll(".admin-sidebar nav a").forEach(m=>{m.classList.toggle("active",m.dataset.page===e)});const t=document.getElementById("mobile-page-select");t&&(t.value=e);const n=De[e]||De.dashboard;document.getElementById("page-title").textContent=n.title;const i=document.getElementById("page-content");i.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">Chargement…</div>';try{await n.fn(i,{toast:yn,openModal:vn,closeModal:ge,navigate:de})}catch(m){i.innerHTML=`<div style="padding:40px;color:#c0392b">Erreur: ${m.message}</div>`,console.error(m)}}async function xn(){var t;const{data:{session:e}}=await _.auth.getSession();e&&await je(e.user),document.getElementById("auth-btn").addEventListener("click",async()=>{const n=document.getElementById("auth-email").value.trim(),i=document.getElementById("auth-password").value,m=document.getElementById("auth-error");if(m.textContent="",!n||!i){m.textContent="Remplissez tous les champs.";return}const{data:d,error:p}=await _.auth.signInWithPassword({email:n,password:i});if(p){m.textContent=p.message;return}await je(d.user)}),document.getElementById("auth-password").addEventListener("keydown",n=>{n.key==="Enter"&&document.getElementById("auth-btn").click()}),document.getElementById("logout-btn").addEventListener("click",async()=>{await _.auth.signOut(),document.getElementById("auth-screen").style.display="flex",document.getElementById("admin-app").style.display="none"}),document.getElementById("modal-close").addEventListener("click",ge),document.getElementById("modal-overlay").addEventListener("click",n=>{n.target===n.currentTarget&&ge()}),document.querySelectorAll(".admin-sidebar nav a").forEach(n=>{n.addEventListener("click",i=>{i.preventDefault(),de(n.dataset.page)})}),(t=document.getElementById("mobile-page-select"))==null||t.addEventListener("change",n=>{de(n.target.value)})}async function je(e){const{data:t,error:n}=await _.from("users").select("*").eq("id",e.id).single(),i=document.getElementById("auth-error");if(n||!t){i.textContent="Profil introuvable.";return}if(!t.is_admin){i.textContent="Accès refusé. Vous n'êtes pas administrateur.",await _.auth.signOut();return}document.getElementById("auth-screen").style.display="none",document.getElementById("admin-app").style.display="flex",document.getElementById("admin-user").textContent=`Admin: ${t.pseudo}`,de("dashboard")}xn();
