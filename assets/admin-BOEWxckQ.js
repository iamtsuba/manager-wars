import{s as _,r as he,a as Ee,A as bt,l as vt,g as xt,K as Te,D as z,b as ht,S as Et,c as R,H as _t,R as wt,e as ce,d as We,f as It,h as kt,E as $t,i as Bt,N as Lt,M as At,B as St,j as Ct,F as ue,k as Me,m as Tt,n as Ze,o as ge,p as Mt}from"./special-cards-hvDhyFrj.js";async function zt(e){var h;const[{count:t},{count:n},{count:i},{count:s},{count:a},{data:m}]=await Promise.all([_.from("players").select("*",{count:"exact",head:!0}),_.from("clubs").select("*",{count:"exact",head:!0}),_.from("users").select("*",{count:"exact",head:!0}),_.from("stadium_definitions").select("*",{count:"exact",head:!0}),_.from("users").select("*",{count:"exact",head:!0}).gt("last_seen_at",new Date(Date.now()-18e4).toISOString()),_.rpc("get_signup_password")]),u=m||"";e.innerHTML=`
...
Cette action est irréversible.`))return;const i=document.getElementById("fix-old-faces-btn");i&&(i.disabled=!0,i.textContent="⏳ En cours...");try{const{data:s,error:a}=await _.from("players").select("id, country_code, face").like("face","public/faces/%");if(a){n(a.message,"error");return}if(!(s!=null&&s.length)){n("Aucun joueur avec l'ancien format — rien à faire ✅","success");return}const{data:m}=await _.from("players").select("face").not("face","is",null),u=new Set((m||[]).map(c=>c.face).filter(c=>c&&!c.startsWith("public/faces/")));let h=0,o=0;for(const c of s){const r=await Ee(c.country_code,u);if(!r){o++;continue}const{error:g}=await _.from("players").update({face:r}).eq("id",c.id);if(g){o++;continue}u.add(r),h++}n(`${h} photo(s) réattribuée(s)${o?`, ${o} échec(s)`:""} ✅`,o?"error":"success")}finally{i&&(i.disabled=!1,i.textContent="🖼️ Réattribuer anciennes photos")}}async function Nt(e,t){await le(e,t)}async function le(e,t,n=null){var o,c,r,g,p;const{toast:i}=t;n||(n={search:((o=document.getElementById("search-players"))==null?void 0:o.value)||"",job:((c=document.getElementById("filter-job"))==null?void 0:c.value)||"",rarity:((r=document.getElementById("filter-rarity"))==null?void 0:r.value)||"",club:((g=document.getElementById("filter-club"))==null?void 0:g.value)||"",country:((p=document.getElementById("filter-country"))==null?void 0:p.value)||""});const[{data:s,error:a},{data:m}]=await Promise.all([_.from("players").select("*, clubs(id,encoded_name,logo_url)"),_.from("clubs").select("id,encoded_name").order("encoded_name")]);if(a){e.innerHTML=`<p style="color:red">${a.message}</p>`;return}const u={GK:0,DEF:1,MIL:2,ATT:3},h=(s||[]).sort((l,x)=>{const f=(u[l.job]??4)-(u[x.job]??4);return f!==0?f:(l.surname_real||"").localeCompare(x.surname_real||"")});Pt(e,h,m||[],t,n)}function Pt(e,t,n,i,s=null){var c,r;const{toast:a}=i;if(e.innerHTML=`
    <div style="display:flex;flex-direction:column;gap:10px">
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        ...
        <select id="filter-rarity" style="width:110px">
          ...
          ${Object.entries(Xe).map(([g,p])=>`<option value="${g}">${p}</option>`).join("")}
        </select>
    ...
    </div>`,s){const g=s,p=l=>document.getElementById(l);g.search&&p("search-players")&&(p("search-players").value=g.search),g.job&&p("filter-job")&&(p("filter-job").value=g.job),g.rarity&&p("filter-rarity")&&(p("filter-rarity").value=g.rarity),g.club&&p("filter-club")&&(p("filter-club").value=g.club),g.country&&p("filter-country")&&(p("filter-country").value=g.country)}if(s){const g=s,p=l=>document.getElementById(l);g.search&&p("search-players")&&(p("search-players").value=g.search),g.job&&p("filter-job")&&(p("filter-job").value=g.job),g.rarity&&p("filter-rarity")&&(p("filter-rarity").value=g.rarity),g.club&&p("filter-club")&&(p("filter-club").value=g.club),g.country&&p("filter-country")&&(p("filter-country").value=g.country)}function m(){const g=document.getElementById("search-players").value.toLowerCase(),p=document.getElementById("filter-job").value,l=document.getElementById("filter-rarity").value,x=document.getElementById("filter-club").value,f=document.getElementById("filter-country").value;return t.filter(y=>(!g||`${y.firstname} ${y.surname_real}`.toLowerCase().includes(g))&&(!p||y.job===p)&&(!l||y.rarity===l)&&(!x||y.club_id===x)&&(!f||y.country_code===f))}const u=new Set;function h(){const g=document.getElementById("bulk-bar"),p=document.getElementById("bulk-count");g&&(g.style.display=u.size>0?"flex":"none",p&&(p.textContent=`${u.size} joueur(s) sélectionné(s)`))}function o(){const g=m();document.getElementById("count-label").textContent=`${g.length} joueur(s)`;const p=document.getElementById("players-list");if(!g.length){p.innerHTML='<div style="color:var(--gray-600);padding:20px;text-align:center">Aucun joueur.</div>';return}p.innerHTML=g.map(l=>{const x={...l,clubs:l.clubs,face:l.face||null},f=he(x,{width:120});return`<div style="position:relative;cursor:pointer" data-edit="${l.id}">
        ${f}
      ...
      </div>`}).join(""),p.querySelectorAll("[data-edit]").forEach(l=>{l.addEventListener("click",()=>{const x=t.find(f=>f.id===l.dataset.edit);x&&ze(x,n,e,i)})}),p.querySelectorAll(".btn-del-player").forEach(l=>{l.addEventListener("click",async x=>{if(x.stopPropagation(),!confirm("Supprimer ce joueur ?"))return;const{error:f}=await _.from("players").delete().eq("id",l.dataset.del);f?a(f.message,"error"):(a("Joueur supprimé ✅","success"),le(e,i))})})}o(),document.getElementById("search-players").addEventListener("input",o),document.getElementById("filter-job").addEventListener("change",o),document.getElementById("filter-rarity").addEventListener("change",o),document.getElementById("filter-club").addEventListener("change",o),document.getElementById("filter-country").addEventListener("change",o),(c=document.getElementById("bulk-cancel-btn"))==null||c.addEventListener("click",()=>{u.clear(),h(),o()}),(r=document.getElementById("bulk-delete-btn"))==null||r.addEventListener("click",async()=>{var x,f,y,I,B;if(!u.size||!confirm(`Supprimer ${u.size} joueur(s) ?`))return;const g=[...u],{error:p}=await _.from("players").delete().in("id",g);if(p){a(p.message,"error");return}a(`${g.length} joueur(s) supprimé(s) ✅`,"success"),u.clear();const l={search:((x=document.getElementById("search-players"))==null?void 0:x.value)||"",job:((f=document.getElementById("filter-job"))==null?void 0:f.value)||"",rarity:((y=document.getElementById("filter-rarity"))==null?void 0:y.value)||"",club:((I=document.getElementById("filter-club"))==null?void 0:I.value)||"",country:((B=document.getElementById("filter-country"))==null?void 0:B.value)||""};le(e,i,l)}),document.getElementById("add-player-btn").addEventListener("click",()=>ze(null,n,e,i)),document.getElementById("fix-old-faces-btn").addEventListener("click",()=>jt(e,i))}async function ze(e,t,n,i){const{toast:s,openModal:a,closeModal:m}=i,u=!!e,h=await Dt(),{data:o}=await _.from("players").select("face").not("face","is",null),c=new Set((o||[]).map(l=>l.face).filter(Boolean));e!=null&&e.face&&c.delete(e.face);const r='<option value="">— Club —</option>'+t.map(l=>`<option value="${l.id}" ${(e==null?void 0:e.club_id)===l.id?"selected":""}>${l.encoded_name}</option>`).join(""),g=e!=null&&e.face?e.face.split("/")[0]:"",p=Object.keys(h).map(l=>`<option value="${l}" ${g===l?"selected":""}>${l}</option>`).join("");a(u?`✏️ ${e.firstname} ${e.surname_real}`:"➕ Nouveau joueur",`<div style="display:flex;gap:20px;align-items:flex-start">
    ...
    </div>`,m(ae),document.getElementById("search-clubs").addEventListener("input",u=>{const h=u.target.value.toLowerCase();m(ae.filter(o=>o.real_name.toLowerCase().includes(h)||o.encoded_name.toLowerCase().includes(h)))}),document.getElementById("add-club-btn").addEventListener("click",()=>Re(null,e,t));function m(u){const h=document.getElementById("clubs-list");if(!u.length){h.innerHTML='<p style="color:var(--tile-fg-dim);padding:20px;text-align:center">Aucun club.</p>';return}h.innerHTML=u.map(o=>{const c=lt(o),r=o.logo_url?`<img src="${o.logo_url}" style="width:36px;height:36px;object-fit:contain;border-radius:8px;flex-shrink:0">`:`<div style="width:36px;height:36px;background:linear-gradient(135deg,${c.color1},${c.color2});border-radius:8px;display:flex;align-items:center;justify-content:center;color:#fff;font-size:9px;font-weight:900;flex-shrink:0">${o.encoded_name.slice(0,3)}</div>`,g=n[o.id]||0;return`
      ...
      <div style="border-top:1px solid var(--tile-border);padding-top:12px">
        ...
        <div style="display:flex;gap:12px;align-items:flex-start;flex-wrap:wrap">
          <div style="flex:1;min-width:200px;display:flex;flex-direction:column;gap:8px">
            ...
            ${[["Couleur 1","m-kit-color1",a.color1,!1],["Couleur 2","m-kit-color2",a.color2,!1],["Couleur 3","m-kit-color3",a.color3,!0],["Short","m-kit-shorts",a.shorts,!1],["Chaussettes","m-kit-socks",a.socks,!1]].map(([d,v,w,k])=>`
              <div class="form-group" id="wrap-${v}" ${k?'style="display:none"':""}>
                <label>${d}</label>
  ...
  `,(f=document.getElementById("m-logo-file"))==null||f.addEventListener("change",d=>{var k;const v=(k=d.target.files)==null?void 0:k[0];if(!v)return;const w=new FileReader;w.onload=()=>{document.getElementById("logo-preview").innerHTML=`<img src="${w.result}" style="width:100%;height:100%;object-fit:contain">`},w.readAsDataURL(v)}),te();function o(){var k,$;const d=((k=document.getElementById("m-kit-style"))==null?void 0:k.value)||"uni",v=(($=Te[d])==null?void 0:$.colors)===3,w=document.getElementById("wrap-m-kit-color3");w&&(w.style.display=v?"":"none")}o(),["m-kit-color1","m-kit-color2","m-kit-color3","m-kit-shorts","m-kit-socks"].forEach(d=>{const v=document.getElementById(d),w=document.getElementById(d+"-txt");!v||!w||(v.addEventListener("input",()=>{w.value=v.value,te()}),w.addEventListener("input",()=>{const k=w.value.trim();/^#[0-9a-fA-F]{6}$/.test(k)&&(v.value=k,te())}),w.addEventListener("change",()=>{let k=w.value.trim();k.startsWith("#")||(k="#"+k),/^#[0-9a-fA-F]{6}$/.test(k)&&(v.value=k,w.value=k,te())}))}),(y=document.getElementById("m-kit-style"))==null||y.addEventListener("change",()=>{o(),te()});const c=document.getElementById("m-real"),r=document.getElementById("m-encoded");function g(){if(!c||!r||r.value)return;const d=c.value.trim().split(/\s+/),v=d.length===1?d[0].toUpperCase().slice(0,6):d.filter(w=>w.length>2).map(w=>w[0].toUpperCase()).join("")||d[0].toUpperCase().slice(0,4);r.value=v}c==null||c.addEventListener("input",g),(I=document.getElementById("auto-encode-btn"))==null||I.addEventListener("click",()=>{r&&(r.value=""),g()}),(B=document.getElementById("m-save"))==null||B.addEventListener("click",()=>on(e,s,t,n));const p=document.getElementById("m-gen-squad"),l=document.getElementById("m-gen-strong-label");function x(){if(l&&(l.style.display=p!=null&&p.checked?"flex":"none",!(p!=null&&p.checked))){const d=document.getElementById("m-gen-strong");d&&(d.checked=!1)}}p==null||p.addEventListener("change",x),x(),(E=document.getElementById("btn-gen-squad-panel"))==null||E.addEventListener("click",()=>{Xt(e,n,t)}),(b=document.getElementById("btn-gen-one-player"))==null||b.addEventListener("click",()=>{en(e,n)}),s&&Ie(e.id)}async function Ie(e){const{data:t}=await _.from("players").select("*, clubs(encoded_name, logo_url, kit_style, kit_color1, kit_color2, kit_color3, kit_shorts, kit_socks)").eq("club_id",e).order("surname_real"),n=document.getElementById("club-players-grid"),i=document.getElementById("club-players-count");if(i&&(i.textContent=(t||[]).length),!!n){if(!(t!=null&&t.length)){n.innerHTML='<div style="color:var(--tile-fg-dim);padding:20px;font-size:13px">Aucun joueur dans ce club.</div>';return}n.innerHTML=t.map(s=>he(s,{width:100})).join("")}}function Xt(e,t,n){const{openModal:i,closeModal:s,toast:a}=t;i(`⚽ Générer — ${e.real_name}`,`<div style="display:flex;flex-direction:column;gap:14px">
    ...
    </div>`),setTimeout(()=>{var m,u;(m=document.getElementById("qg-cancel"))==null||m.addEventListener("click",()=>s()),(u=document.getElementById("qg-ok"))==null||u.addEventListener("click",async()=>{var o;const h=((o=document.getElementById("qg-strong"))==null?void 0:o.checked)??!1;s(),a("Génération en cours…","info"),await rt(e.id,e.country_code,a,h),a("Effectif généré ✅","success"),Ie(e.id),pe(n,t)})},50)}const st=["GK","DEF","MIL","ATT"],Qt=[{value:"normal",label:"Normal"},{value:"pepite",label:"Pépite"},{value:"papyte",label:"Papyte"},{value:"legende",label:"Légende"}];function en(e,t){const{openModal:n,closeModal:i,toast:s}=t,a=_e.map(o=>`<option value="${o}">${o}</option>`).join(""),m=o=>st.map(c=>`<option value="${c}" ${c===o?"selected":""}>${c}</option>`).join(""),u=`
  ...
  `),setTimeout(()=>{var o,c;document.querySelectorAll('input[name="gen-one-mode"]').forEach(r=>{r.addEventListener("change",()=>{document.getElementById("gen-one-custom-fields").style.display=r.value==="custom"&&r.checked?"flex":"none"})}),(o=document.getElementById("gen-one-cancel"))==null||o.addEventListener("click",()=>i()),(c=document.getElementById("gen-one-ok"))==null||c.addEventListener("click",async()=>{var l,x,f,y,I;const r=((l=document.querySelector('input[name="gen-one-mode"]:checked'))==null?void 0:l.value)||"random",g=document.getElementById("gen-one-ok");g.disabled=!0,g.textContent="⏳ Génération…";const p={};r==="custom"&&(p.job=((x=document.getElementById("gen-one-job1"))==null?void 0:x.value)||null,p.job2=((f=document.getElementById("gen-one-job2"))==null?void 0:f.value)||null,p.rarity=((y=document.getElementById("gen-one-rarity"))==null?void 0:y.value)||null,p.country=((I=document.getElementById("gen-one-country"))==null?void 0:I.value)||null);try{await tn(e,p),s("Joueur généré ✅","success"),i(),Ie(e.id)}catch(B){s("Erreur : "+B.message,"error"),g.disabled=!1,g.textContent="➕ Générer"}})},50)}async function tn(e,t={}){const n=t.country||(Math.random()<.5?e.country_code:tt(e.country_code)),i=t.job||q(st),s=t.job2||null,a=t.rarity||(()=>{const L=Math.random()*100;return L<1.5?"legende":L<3?"pepite":L<4.5?"papyte":"normal"})(),m=nt(),u=s?Math.max(0,m-2):0;let h=0,o=0,c=0,r=0;i==="GK"&&(h=m),i==="DEF"&&(o=m),i==="MIL"&&(c=m),i==="ATT"&&(r=m),s==="GK"&&(h=u),s==="DEF"&&(o=u),s==="MIL"&&(c=u),s==="ATT"&&(r=u);const g=we(n),p=et(n),l=q(Qe),{data:x}=await _.from("players").select("surname_real").not("surname_real","is",null),f=new Set((x||[]).map(L=>L.surname_real).filter(Boolean)),y=at(n),I=y.filter(L=>!f.has(L)),B=q(I.length?I:y),E={job:i,job2:s,firstname:ot(n),surname_real:B,country_code:n,club_id:e.id,note_g:h,note_d:o,note_m:c,note_a:r,skin:g,hair:p,hair_length:l,rarity:a,sell_price:0,ethnie:ye(n)},{data:b}=await _.from("players").select("face").not("face","is",null),d=new Set((b||[]).map(L=>L.face).filter(Boolean)),v=await Ee(n,d);v&&(E.face=v);const{data:w,error:k}=await _.from("players").insert(E).select("id").single();if(k)throw k;const{error:$}=await _.from("cards").insert({card_type:"player",player_id:w.id});$&&console.warn("[GenOnePlayer] Erreur carte:",$.message)}function dt(){var e,t,n,i,s,a;return{style:((e=document.getElementById("m-kit-style"))==null?void 0:e.value)||"uni",color1:((t=document.getElementById("m-kit-color1"))==null?void 0:t.value)||"#1A6B3C",color2:((n=document.getElementById("m-kit-color2"))==null?void 0:n.value)||"#ffffff",color3:((i=document.getElementById("m-kit-color3"))==null?void 0:i.value)||"#000000",shorts:((s=document.getElementById("m-kit-shorts"))==null?void 0:s.value)||"#111111",socks:((a=document.getElementById("m-kit-socks"))==null?void 0:a.value)||"#ffffff"}}function te(){const e=document.getElementById("kit-preview-wrap");e&&(e.innerHTML=ht(dt(),"panel"))}async function nn(e,t){const n=(e.name.split(".").pop()||"png").toLowerCase(),i=`clubs/${t||"new"}-${Date.now()}.${n}`,{error:s}=await _.storage.from("assets").upload(i,e,{upsert:!0,cacheControl:"3600"});if(s)throw s;const{data:a}=_.storage.from("assets").getPublicUrl(i);return a.publicUrl}async function on(e,t,n,i){var B,E,b,d,v,w,k,$,L;const{toast:s}=i,a=document.getElementById("m-error"),m=document.getElementById("m-save"),u=(B=document.getElementById("m-real"))==null?void 0:B.value.trim(),h=(E=document.getElementById("m-encoded"))==null?void 0:E.value.trim().toUpperCase(),o=(b=document.getElementById("m-country"))==null?void 0:b.value.trim().toUpperCase(),c=((v=(d=document.getElementById("m-logo-file"))==null?void 0:d.files)==null?void 0:v[0])||null,r=((w=document.getElementById("m-logo-url-current"))==null?void 0:w.value)||null,g=((k=document.getElementById("m-gen-stadium"))==null?void 0:k.checked)??!1,p=(($=document.getElementById("m-gen-squad"))==null?void 0:$.checked)??!1,l=((L=document.getElementById("m-gen-strong"))==null?void 0:L.checked)??!1,x=dt();if(!u){a.textContent="Le nom du club est requis.";return}if(!h){a.textContent="Le nom encodé est requis.";return}if(!o){a.textContent="Le pays est requis.";return}m.disabled=!0,m.textContent="Enregistrement…";let f=r;if(c)try{m.textContent="📤 Envoi du logo…",f=await nn(c,e==null?void 0:e.id)}catch(A){a.textContent="Erreur upload logo : "+A.message,m.disabled=!1,m.textContent=t?"💾 Enregistrer":"✅ Créer le club";return}const y={real_name:u,encoded_name:h,country_code:o,logo_url:f,kit_style:x.style,kit_color1:x.color1,kit_color2:x.color2,kit_color3:x.color3,kit_shorts:x.shorts,kit_socks:x.socks};let I=e==null?void 0:e.id;if(t){const{error:A}=await _.from("clubs").update(y).eq("id",I);if(A){a.textContent=A.message,m.disabled=!1,m.textContent="💾 Enregistrer";return}}else{const{data:A,error:S}=await _.from("clubs").insert(y).select().single();if(S){a.textContent=S.message,m.disabled=!1,m.textContent="✅ Créer le club";return}if(I=A.id,g){m.textContent="🏟️ Création du stade…";const{data:C,error:M}=await _.from("stadium_definitions").insert({name:`Stade de ${u}`,club_id:I,country_code:o}).select().single();M?console.warn("[Stadium] Erreur def stade:",M.message):C&&await _.from("cards").insert({card_type:"stadium",stadium_id:C.id})}p&&(m.textContent="⚽ Génération des joueurs…",await rt(I,o,s,l))}s(t?"Club modifié ✅":"Club créé ✅","success"),pe(n,i)}const an=["normal","pepite","papyte","legende"],De=["GK","DEF","MIL","ATT"],rn=[["MA","Maroc"],["FR","France"],["AR","Argentine"],["PT","Portugal"],["BR","Brésil"],["ES","Espagne"],["DE","Allemagne"],["GB","Angleterre"],["IT","Italie"],["CM","Cameroun"],["SN","Sénégal"],["NG","Nigéria"],["DK","Danemark"],["NL","Pays-Bas"],["BE","Belgique"],["CI","Côte d'Ivoire"],["AL","Albanie"],["HR","Croatie"],["RS","Serbie"],["TR","Turquie"],["MW","Malawi"]];let K={...R},re=[];async function ln(e,{toast:t}){const{data:n}=await _.from("clubs").select("id,encoded_name,logo_url,kit_style,kit_color1,kit_color2,kit_shorts,kit_socks").order("encoded_name");re=n||[],K={...R},e.innerHTML=pn(re),dn(e,re,t),P()}function je(){var n;const e=((n=document.getElementById("cb-surname-real"))==null?void 0:n.value)||"",t=document.getElementById("cb-surname-enc");t&&(t.value=ce(e))}function sn(){var n;const e=(n=document.getElementById("cb-club"))==null?void 0:n.value,t=re.find(i=>String(i.id)===String(e));return t?{style:t.kit_style||z.style,color1:t.kit_color1||z.color1,color2:t.kit_color2||z.color2,shorts:t.kit_shorts||z.shorts,socks:t.kit_socks||z.socks}:{...z}}function P(){var g,p,l,x;const e=f=>{var y;return((y=document.getElementById(f))==null?void 0:y.value)||""},t={firstname:e("cb-firstname"),surname_real:e("cb-surname-enc")||ce(e("cb-surname-real")),country_code:e("cb-country"),club_encoded_name:((p=(g=document.getElementById("cb-club"))==null?void 0:g.selectedOptions[0])==null?void 0:p.text)||"",job:e("cb-job"),job2:e("cb-job2")||null,note_g:parseInt(e("cb-note-g"))||0,note_d:parseInt(e("cb-note-d"))||0,note_m:parseInt(e("cb-note-m"))||0,note_a:parseInt(e("cb-note-a"))||0,rarity:e("cb-rarity")},n=sn(),i=document.getElementById("cb-club"),s=((x=(l=i==null?void 0:i.selectedOptions[0])==null?void 0:l.dataset)==null?void 0:x.logo)||null,a=We(K,n,120),m=`data:image/svg+xml;charset=utf-8,${encodeURIComponent(a)}`,u=It(t,{portraitUrl:m,clubLogoUrl:s,showNotes:!0}),h=document.getElementById("card-preview");h&&(h.innerHTML=u);const o=document.getElementById("avatar-preview-wrap");o&&(o.innerHTML=a);const c=e("cb-surname-real"),r=document.getElementById("encode-summary");r&&c&&(r.textContent=`${e("cb-firstname")} ${c} → ${e("cb-firstname")} ${t.surname_real}`)}function ct(){const e=t=>{var n;return((n=document.getElementById(t))==null?void 0:n.value)||""};K={skin:e("av-skin"),hair:e("av-hair"),hairColor:e("av-hairColor"),eyes:e("av-eyes"),nose:e("av-nose"),mouth:e("av-mouth"),eyebrows:e("av-eyebrows"),beard:e("av-beard")}}function dn(e,t,n){var s,a,m,u,h;e.querySelectorAll("input,select").forEach(o=>{o.addEventListener("input",P),o.addEventListener("change",P)});const i=["av-skin","av-hair","av-hairColor","av-eyes","av-nose","av-mouth","av-eyebrows","av-beard"];i.forEach(o=>{var c;(c=document.getElementById(o))==null||c.addEventListener("change",()=>{ct(),P()})}),(s=document.getElementById("cb-surname-real"))==null||s.addEventListener("input",()=>{je(),P()}),(a=document.getElementById("btn-encode"))==null||a.addEventListener("click",()=>{je(),P()}),(m=document.getElementById("cb-club"))==null||m.addEventListener("change",P),(u=document.getElementById("btn-save-player"))==null||u.addEventListener("click",()=>cn(n)),(h=document.getElementById("btn-reset"))==null||h.addEventListener("click",()=>{e.querySelectorAll("input").forEach(o=>o.value=""),e.querySelectorAll("select").forEach(o=>o.selectedIndex=0),K={...R},i.forEach(o=>{const c=o.replace("av-",""),r=document.getElementById(o);r&&(r.value=R[c]||"")}),P()})}async function cn(e){const t=m=>{var u;return((u=document.getElementById(m))==null?void 0:u.value)||""},n=t("cb-surname-real").trim(),i=t("cb-surname-enc").trim()||ce(n);if(!t("cb-firstname")||!n||!t("cb-country")||!t("cb-job")){e("Remplissez les champs obligatoires (prénom, nom, pays, poste)","error");return}ct();const s={firstname:t("cb-firstname").trim(),surname_real:n,surname_real:i,country_code:t("cb-country"),club_id:t("cb-club")||null,job:t("cb-job"),job2:t("cb-job2")||null,note_g:parseInt(t("cb-note-g"))||0,note_d:parseInt(t("cb-note-d"))||0,note_m:parseInt(t("cb-note-m"))||0,note_a:parseInt(t("cb-note-a"))||0,rarity:t("cb-rarity")||"normal",note_min:parseInt(t("cb-note-min"))||null,note_max:parseInt(t("cb-note-max"))||null,sell_price:parseInt(t("cb-price"))||0,avatar_config:K,skin:K.skin||"blanc",hair:K.hairColor||"marron",hair_length:"court"},{error:a}=await _.from("players").insert(s);if(a){e("Erreur: "+a.message,"error");return}e(`✅ Joueur "${s.firstname} ${s.surname_real}" enregistré !`,"success")}function pn(e){const t=rn.map(([o,c])=>`<option value="${o}">${c} (${o})</option>`).join(""),n='<option value="">— Sélectionner un club —</option>'+e.map(o=>`<option value="${o.id}" data-logo="${o.logo_url||""}">${o.encoded_name}</option>`).join(""),i=De.map(o=>`<option value="${o}">${o}</option>`).join(""),s='<option value="">Aucun</option>'+De.map(o=>`<option value="${o}">${o}</option>`).join(""),a=an.map(o=>`<option value="${o}">${o.charAt(0).toUpperCase()+o.slice(1)}</option>`).join(""),m=(o,c,r)=>Object.entries(c).map(([g,p])=>`<option value="${g}" ${g===(r||"")?"selected":""}>${p.label}</option>`).join(""),u=Object.keys(Et).map(o=>`<option value="${o}" ${o===R.skin?"selected":""}>${o.charAt(0).toUpperCase()+o.slice(1)}</option>`).join(""),h=Object.keys(_t).map(o=>`<option value="${o}" ${o===R.hairColor?"selected":""}>${o.charAt(0).toUpperCase()+o.slice(1)}</option>`).join("");return`
  ...
  `,a(s),document.getElementById("search-users").addEventListener("input",m=>{const u=m.target.value.toLowerCase();a(s.filter(h=>{var o;return h.pseudo.toLowerCase().includes(u)||((o=h.club_name)==null?void 0:o.toLowerCase().includes(u))}))});function a(m){document.getElementById("users-tbody").innerHTML=m.map(u=>{const h=u.mmr??1e3,o=u.mmr_deviation??350,c=u.placement_matches??0,r=Ct(h),g=u.ranked_wins??0,p=u.ranked_losses??0,l=u.ranked_draws??0,x=g+p+l,f=x>0?Math.round(g/x*100):null,y=c<10,I=o<80?"#1A6B3C":o<150?"#e67e22":"#bb2020";return`
        <tr>
          <td>
          ...
          <td style="font-size:12px">
            ...
            ${y?`<div style="font-size:10px;color:#e67e22">${c}/10 placement${c<10?` (${10-c} restants)`:""}</div>`:`<div style="font-size:10px;color:var(--gray-600)">${g}V ${l}N ${p}D${f!==null?` · ${f}% WR`:""}</div>`}
          </td>
        ...
        </tr>`}).join(""),document.querySelectorAll("[data-toggle-admin]").forEach(u=>{u.addEventListener("click",async()=>{const h=u.dataset.isAdmin!=="true";if(!confirm(`Voulez-vous ${h?"rendre admin":"retirer le rôle admin"} de ce Manager ?`))return;const{error:o}=await _.from("users").update({is_admin:h}).eq("id",u.dataset.toggleAdmin);o?t(o.message,"error"):(t("Rôle mis à jour ✅","success"),pt(e,{toast:t}))})})}}const me=[{value:"normal",label:"Normal",color:"#ccc"},{value:"pepite",label:"Pépite",color:"#D4A017"},{value:"papyte",label:"Papyte",color:"#909090"},{value:"legende",label:"Légende",color:"#7a28b8"}];async function ut(e,t){var h;const{toast:n}=t,{data:i,error:s}=await _.from("market_listings").select(`
    ...
    `).order("listed_at",{ascending:!1}).limit(100);if(s){e.innerHTML=`<p style="color:var(--danger)">${s.message}</p>`;return}const a=i||[],m=a.filter(o=>o.status==="active").length,u=a.filter(o=>o.status==="sold").length;e.innerHTML=`
    ...
    <div class="card-panel">
      <div class="table-wrap">
        <table>
          ...
          <tbody>
            ${a.map(o=>{var p,l,x,f;const c=(p=o.card)==null?void 0:p.player,r=c?`${c.firstname} ${c.surname_real}`:((l=o.card)==null?void 0:l.card_type)||"—",g={active:"#1A6B3C",sold:"#D4A017",cancelled:"#888"};return`<tr>
  ...
  `,document.querySelectorAll("[data-cancel]").forEach(o=>{o.addEventListener("click",async()=>{if(!confirm("Annuler cette annonce ?"))return;const{error:c}=await _.from("market_listings").update({status:"cancelled"}).eq("id",o.dataset.cancel);c?n(c.message,"error"):(n("Annonce annulée","success"),ut(e,t))})}),(h=document.getElementById("btn-price-grid"))==null||h.addEventListener("click",()=>un(t))}async function un(e){const{openModal:t,closeModal:n,toast:i}=e;await be(e)}async function be(e){var o,c,r,g;const{openModal:t,closeModal:n,toast:i}=e,{data:s,error:a}=await _.from("sell_price_configs").select("*").order("rarity").order("note_min");if(a){i(a.message,"error");return}const m=p=>{const l=me.find(x=>x.value===p.rarity)||me[0];return`<tr data-row="${p.id}">
    ...
    <div id="pg-form" style="display:none;background:#f7f7f7;border-radius:10px;padding:14px;margin-top:10px">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px">
        <div>
          <label>RARETÉ</label>
          <select id="pg-rarity">${me.map(p=>`<option value="${p.value}">${p.label}</option>`).join("")}</select>
        </div>
        <div></div>
        <div>
          <label>NOTE MIN</label>
  ...
  `),(o=document.getElementById("pg-close"))==null||o.addEventListener("click",()=>n()),(c=document.getElementById("pg-add-row"))==null||c.addEventListener("click",()=>{document.getElementById("pg-form").style.display="block",document.getElementById("pg-id").value="",document.getElementById("pg-rarity").value="normal",document.getElementById("pg-note-min").value=0,document.getElementById("pg-note-max").value=20,document.getElementById("pg-price-min").value=2500,document.getElementById("pg-price-max").value=15e3}),document.querySelectorAll("[data-edit-row]").forEach(p=>{p.addEventListener("click",()=>{const l=(s||[]).find(x=>x.id===p.dataset.editRow);l&&(document.getElementById("pg-form").style.display="block",document.getElementById("pg-id").value=l.id,document.getElementById("pg-rarity").value=l.rarity,document.getElementById("pg-note-min").value=l.note_min,document.getElementById("pg-note-max").value=l.note_max,document.getElementById("pg-price-min").value=l.price_min,document.getElementById("pg-price-max").value=l.price_max)})}),document.querySelectorAll("[data-del-row]").forEach(p=>{p.addEventListener("click",async()=>{confirm("Supprimer cette ligne ?")&&(await _.from("sell_price_configs").delete().eq("id",p.dataset.delRow),i("Ligne supprimée","success"),be(e))})}),(r=document.getElementById("pg-form-cancel"))==null||r.addEventListener("click",()=>{document.getElementById("pg-form").style.display="none"}),(g=document.getElementById("pg-form-save"))==null||g.addEventListener("click",async()=>{const p=document.getElementById("pg-id").value,l={rarity:document.getElementById("pg-rarity").value,note_min:Number(document.getElementById("pg-note-min").value)||0,note_max:Number(document.getElementById("pg-note-max").value)||0,price_min:Number(document.getElementById("pg-price-min").value)||0,price_max:Number(document.getElementById("pg-price-max").value)||0};if(l.price_min>l.price_max){i("Le prix min doit être ≤ prix max","error");return}if(l.note_min>l.note_max){i("La note min doit être ≤ note max","error");return}const{error:x}=p?await _.from("sell_price_configs").update(l).eq("id",p):await _.from("sell_price_configs").insert(l);if(x){i(x.message,"error");return}i("Règle enregistrée ✅","success"),be(e)})}async function mn(e,{toast:t}){e.innerHTML=`
...
`).replace(/\r/g,`
`);const t=e.split(`
`).filter(m=>m.trim());if(t.length<2)return[];const n=t[0],i=n.split(";").length>n.split(",").length?";":",",s=Fe(n,i).map(m=>m.trim().replace(/^\uFEFF/,"").toLowerCase()),a=[];for(let m=1;m<t.length;m++){if(!t[m].trim())continue;const u=Fe(t[m],i),h={};s.forEach((o,c)=>{h[o]=(u[c]||"").trim()}),!Object.values(h).every(o=>!o)&&a.push(h)}return a}function Fe(e,t=","){const n=[];let i="",s=!1;for(let a=0;a<e.length;a++){const m=e[a];s?m==='"'?e[a+1]==='"'?(i+='"',a++):s=!1:i+=m:m==='"'?s=!0:m===t?(n.push(i),i=""):i+=m}return n.push(i),n}const gn={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};async function fn(e,{toast:t}){const n=Object.keys(ue);let i=n[0];const{data:s}=await _.from("formation_links_overrides").select("formation, links"),a={};(s||[]).forEach(c=>{a[c.formation]=c.links});let m=new Set;function u(c,r){return[c,r].sort().join("-")}function h(c){const r=a[c]||ue[c]||[];m=new Set(r.map(([g,p])=>u(g,p)))}h(i),e.innerHTML=`
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;gap:12px;flex-wrap:wrap">
  ...
  `;function o(){const c=Tt[i]||{},r=Me(i),g=320,p=400,l=22;function x(E){const b=c[E];return b?{x:b.x*g,y:b.y*p}:null}let f=`<svg width="${g}" height="${p}" viewBox="0 0 ${g} ${p}" xmlns="http://www.w3.org/2000/svg" style="display:block">`;r.forEach(([E,b],d)=>{const v=x(E),w=x(b);if(!v||!w)return;const k=u(E,b),$=m.has(k),L=$?"#3b82f6":"#999",A=$?.95:.35,S=$?4:3;f+=`<line x1="${v.x}" y1="${v.y}" x2="${w.x}" y2="${w.y}"
        stroke="transparent" stroke-width="14" class="link-hit" data-key="${k}" style="cursor:pointer"/>`,f+=`<line x1="${v.x}" y1="${v.y}" x2="${w.x}" y2="${w.y}"
        stroke="${L}" stroke-width="${S}" stroke-dasharray="${$?"none":"4,3"}"
        opacity="${A}" pointer-events="none" data-line-key="${k}"/>`});for(const E of Object.keys(c)){const b=x(E);if(!b)continue;const d=E.replace(/\d+/,""),v=gn[d]||"#555";f+=`<circle cx="${b.x}" cy="${b.y}" r="${l}" fill="${v}" stroke="rgba(255,255,255,0.7)" stroke-width="2" pointer-events="none"/>`,f+=`<text x="${b.x}" y="${b.y+4}" text-anchor="middle" font-size="10" font-weight="900" fill="white" font-family="Arial Black,Arial" pointer-events="none">${E}</text>`}f+="</svg>",document.getElementById("field-wrap").innerHTML=f;const y=document.getElementById("links-list");y.innerHTML=r.map(([E,b])=>{const d=u(E,b),v=m.has(d);return`
    ...
    </div>`;const g=i.reduce((l,x)=>l+Number(x.percentage||0),0),p=Math.abs(g-100)<.1;return`
    ...
    </div>`}async function o(r=!1){r||await m();const g=!n&&a(),p=n&&a();e.innerHTML=`
    <div style="display:flex;height:calc(100vh - 56px);overflow:hidden">
      ${!a()||g||!n?u():""}
      ${!a()||p?h():""}
    </div>`,c()}function c(){var p,l,x,f,y,I,B;const r=E=>e.querySelector(E);e.querySelectorAll(".booster-row").forEach(E=>{E.addEventListener("click",b=>{b.target.closest(".btn-delete")||(n=E.dataset.id,s=null,o())})}),(p=r("#btn-back"))==null||p.addEventListener("click",()=>{n=null,o()}),(l=r("#btn-new"))==null||l.addEventListener("click",async()=>{const E=prompt("Nom du nouveau booster :");if(!(E!=null&&E.trim()))return;const{data:b,error:d}=await _.from("booster_configs").insert({name:E.trim(),price_type:"credits",price_credits:1e3,card_count:5}).select().single();if(d){alert(d.message);return}t.push(b),n=b.id,s=null,o()}),e.querySelectorAll(".btn-delete").forEach(E=>{E.addEventListener("click",async b=>{if(b.stopPropagation(),!confirm("Supprimer ce booster ?"))return;await _.from("booster_configs").delete().eq("id",E.dataset.id);const d=t.findIndex(v=>v.id===E.dataset.id);d>-1&&t.splice(d,1),n===E.dataset.id&&(n=null,s=null),o()})}),(x=r("#btn-cancel"))==null||x.addEventListener("click",()=>{n=null,o()}),(f=r("#f-price-type"))==null||f.addEventListener("change",E=>{const b=r("#credits-field");b&&(b.style.opacity=E.target.value!=="credits"?"0.4":"1")}),(y=r("#btn-pick-icon"))==null||y.addEventListener("click",async()=>{var b;const E=r("#icon-picker-grid");if(E){if(E.style.display!=="none"){E.style.display="none";return}E.innerHTML='<div style="padding:10px;text-align:center;color:#888;font-size:12px">⏳ Chargement...</div>',E.style.display="block";try{const v=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),w=Array.isArray(v)?v.filter($=>$.name.startsWith("booster")&&/\.(png|jpg|svg|webp)$/i.test($.name)):[];if(!w.length){E.innerHTML='<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Aucune icône "booster*" trouvée.</div>';return}const k=((b=r("#f-image-url"))==null?void 0:b.value)||"";E.innerHTML=`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(72px,1fr));gap:8px">
          ${w.map($=>`
          <div class="icon-pick-item" data-name="${$.name}"
            style="cursor:pointer;text-align:center;padding:6px;border-radius:6px;border:2px solid ${$.name===k?"#1A6B3C":"#ddd"};background:${$.name===k?"#f0f7f0":"#fff"}">
    ...
    </tr>`}function Oe(e){document.getElementById("config-form").style.display="block",document.getElementById("form-title").textContent=e?"Modifier la règle":"Nouvelle règle",document.getElementById("form-id").value=(e==null?void 0:e.id)||"",document.getElementById("form-rarity").value=(e==null?void 0:e.rarity)||"normal",document.getElementById("form-price").value=(e==null?void 0:e.price)||1e3,document.getElementById("form-note-min").value=(e==null?void 0:e.note_min)??1,document.getElementById("form-note-max").value=(e==null?void 0:e.note_max)??10,document.getElementById("config-form").scrollIntoView({behavior:"smooth"})}async function _n(e){const t=document.getElementById("form-id").value,n=document.getElementById("form-rarity").value,i=parseInt(document.getElementById("form-price").value)||0,s=parseInt(document.getElementById("form-note-min").value)||1,a=parseInt(document.getElementById("form-note-max").value)||10;if(s>a){alert("Note min doit être ≤ note max");return}const m={rarity:n,price:i,note_min:s,note_max:a,updated_at:new Date().toISOString()};let u;if(t?{error:u}=await _.from("sell_price_configs").update(m).eq("id",t):{error:u}=await _.from("sell_price_configs").insert(m),u){alert("Erreur : "+u.message);return}document.getElementById("config-form").style.display="none",await ke(e)}async function wn(e){await $e(e)}async function $e(e){const{data:t}=await _.from("patch_notes").select("*").order("published_at",{ascending:!1});e.innerHTML=`
    ...
    </div>`,document.getElementById("add-article").addEventListener("click",()=>Ue(null)),document.getElementById("form-cancel").addEventListener("click",()=>{document.getElementById("article-form").style.display="none"}),document.getElementById("form-save").addEventListener("click",()=>$n(e)),e.querySelectorAll("[data-edit-row]").forEach(n=>{const i=(t||[]).find(s=>s.id===n.dataset.editRow);i&&n.addEventListener("click",()=>Ue(i))}),e.querySelectorAll("[data-delete]").forEach(n=>{n.addEventListener("click",async()=>{confirm("Supprimer cet article ?")&&(await _.from("patch_notes").delete().eq("id",n.dataset.delete),await $e(e))})})}function In(e){const t=new Date(e.published_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"short",year:"numeric"});return`
    ...
    </div>`}function Ue(e){const t=document.getElementById("article-form");t.style.display="block",document.getElementById("form-title").textContent=e?"Modifier l'article":"Nouvel article",document.getElementById("form-id").value=(e==null?void 0:e.id)||"",document.getElementById("form-art-title").value=(e==null?void 0:e.title)||"",document.getElementById("form-art-desc").value=(e==null?void 0:e.description)||"",document.getElementById("form-art-img-current").value=(e==null?void 0:e.image_url)||"",document.getElementById("form-art-img-file").value="";const n=document.getElementById("form-art-img-preview");n.innerHTML=e!=null&&e.image_url?`<img src="${e.image_url}" style="width:100%;height:100%;object-fit:cover">`:'<span style="font-size:20px;opacity:.4">📰</span>',document.getElementById("form-art-img-file").onchange=a=>{var h;const m=(h=a.target.files)==null?void 0:h[0];if(!m)return;const u=new FileReader;u.onload=()=>{n.innerHTML=`<img src="${u.result}" style="width:100%;height:100%;object-fit:cover">`},u.readAsDataURL(m)},document.getElementById("form-art-pub").checked=e?e.is_published:!0;const i=new Date((e==null?void 0:e.published_at)||Date.now()),s=new Date(i.getTime()-i.getTimezoneOffset()*6e4).toISOString().slice(0,16);document.getElementById("form-art-date").value=s,t.scrollIntoView({behavior:"smooth"})}async function kn(e){const t=(e.name.split(".").pop()||"png").toLowerCase(),n=`journal/${Date.now()}.${t}`,{error:i}=await _.storage.from("assets").upload(n,e,{upsert:!0,cacheControl:"3600"});if(i)throw i;const{data:s}=_.storage.from("assets").getPublicUrl(n);return s.publicUrl}async function $n(e){var p;const t=document.getElementById("form-id").value,n=document.getElementById("form-art-title").value.trim(),i=document.getElementById("form-art-desc").value.trim(),s=((p=document.getElementById("form-art-img-file").files)==null?void 0:p[0])||null,a=document.getElementById("form-art-img-current").value||null,m=document.getElementById("form-art-pub").checked,u=document.getElementById("form-art-date").value,h=document.getElementById("form-save");if(!n){alert("Le titre est obligatoire.");return}if(!i){alert("La description est obligatoire.");return}let o=a;if(s)try{h&&(h.textContent="📤 Envoi de l'image…"),o=await kn(s)}catch(l){alert("Erreur upload image : "+l.message),h&&(h.textContent="💾 Enregistrer");return}const c=u?new Date(u).toISOString():new Date().toISOString(),r={title:n,description:i,image_url:o,is_published:m,published_at:c};let g;if(t?{error:g}=await _.from("patch_notes").update(r).eq("id",t):{error:g}=await _.from("patch_notes").insert(r),g){alert("Erreur : "+g.message);return}document.getElementById("article-form").style.display="none",await $e(e)}async function Bn(e){await Y(e)}async function Y(e){var c,r,g,p,l,x,f,y,I,B,E,b;const{data:t}=await _.from("tutorial_steps").select("*").order("step_order");e.innerHTML=`
    <div style="display:flex;gap:16px;height:calc(100vh - 56px);overflow:hidden;padding:16px">
      ...
      <div style="flex:1;overflow-y:auto">
        ...
        <div id="ts-list" style="display:flex;flex-direction:column;gap:10px">
          ...
          <div data-pick="${$.name}" style="cursor:pointer;border:2px solid transparent;border-radius:8px;padding:4px;text-align:center;transition:border-color .15s" onmouseover="this.style.borderColor='#1A6B3C'" onmouseout="this.style.borderColor='transparent'">
            <img src="${k}icons/${$.name}" style="width:70px;height:56px;object-fit:contain;display:block;border-radius:4px">
          ...
          </div>`).join("")+"</div>",h.querySelectorAll("[data-pick]").forEach($=>{$.addEventListener("click",()=>{a&&(a.value=$.dataset.pick),o(),h.style.display="none"})})}catch(d){h.innerHTML='<div style="color:#cc2222;font-size:12px">Erreur chargement : '+d.message+"</div>"}}),(E=document.getElementById("ts-color"))==null||E.addEventListener("input",d=>{const v=document.getElementById("ts-color-hex");v&&(v.value=d.target.value)}),(b=document.getElementById("ts-color-hex"))==null||b.addEventListener("input",d=>{const v=d.target.value;if(/^#[0-9a-fA-F]{6}$/.test(v)){const w=document.getElementById("ts-color");w&&(w.value=v)}}),e.querySelectorAll("[data-edit-row]").forEach(d=>{const v=(t||[]).find(w=>w.id===d.dataset.editRow);v&&d.addEventListener("click",()=>Ge(v))}),e.querySelectorAll("[data-delete]").forEach(d=>{d.addEventListener("click",async()=>{confirm("Supprimer cette étape ?")&&(await _.from("tutorial_steps").delete().eq("id",d.dataset.delete),Y(e))})}),e.querySelectorAll("[data-toggle]").forEach(d=>{d.addEventListener("click",async()=>{const v=(t||[]).find(w=>w.id===d.dataset.toggle);v&&(await _.from("tutorial_steps").update({is_active:!v.is_active}).eq("id",v.id),Y(e))})}),e.querySelectorAll("[data-up]").forEach(d=>{d.addEventListener("click",async()=>{const v=t||[],w=v.findIndex(k=>k.id===d.dataset.up);w<=0||(await Promise.all([_.from("tutorial_steps").update({step_order:v[w-1].step_order}).eq("id",v[w].id),_.from("tutorial_steps").update({step_order:v[w].step_order}).eq("id",v[w-1].id)]),Y(e))})}),e.querySelectorAll("[data-down]").forEach(d=>{d.addEventListener("click",async()=>{const v=t||[],w=v.findIndex(k=>k.id===d.dataset.down);w<0||w>=v.length-1||(await Promise.all([_.from("tutorial_steps").update({step_order:v[w+1].step_order}).eq("id",v[w].id),_.from("tutorial_steps").update({step_order:v[w].step_order}).eq("id",v[w+1].id)]),Y(e))})})}function Ln(e){return`
  ...
  </div>`}function Ge(e,t=0){document.getElementById("ts-form-empty").style.display="none",document.getElementById("ts-form-content").style.display="block",document.getElementById("ts-form-title").textContent=e?"Modifier l'étape":"Nouvelle étape",document.getElementById("ts-id").value=(e==null?void 0:e.id)||"",document.getElementById("ts-emoji").value=(e==null?void 0:e.emoji)||"⚽",document.getElementById("ts-title").value=(e==null?void 0:e.title)||"",document.getElementById("ts-content").value=(e==null?void 0:e.content)||"";const n=document.getElementById("ts-editor");n&&(n.innerHTML=(e==null?void 0:e.content)||"<p>Texte de l'étape…</p>"),document.getElementById("ts-order").value=(e==null?void 0:e.step_order)??t,document.getElementById("ts-active").checked=e?e.is_active:!0;const i=(e==null?void 0:e.color)||"#1A6B3C";document.getElementById("ts-color").value=i,document.getElementById("ts-color-hex").value=i;const s=(e==null?void 0:e.image_url)||"",a=document.getElementById("ts-image");a&&(a.value=s);const m=document.getElementById("ts-img-preview"),u=document.getElementById("ts-img-preview-el");if(s&&m&&u){const h="/";u.src=`${h}icons/${s}`,m.style.display="block"}else m&&(m.style.display="none");document.getElementById("ts-img-picker-grid").style.display="none",document.getElementById("ts-preview-area").innerHTML="",document.getElementById("ts-form").scrollIntoView({behavior:"smooth"})}function gt(){document.getElementById("ts-form-content").style.display="none",document.getElementById("ts-form-empty").style.display="block",document.getElementById("ts-preview-area").innerHTML=""}function An(){var o,c;const e=document.getElementById("ts-emoji").value||"⚽",t=document.getElementById("ts-title").value||"Titre",n=document.getElementById("ts-editor"),i=n?n.innerHTML:document.getElementById("ts-content").value||"",s=document.getElementById("ts-color-hex").value||"#1A6B3C",a=(c=(o=document.getElementById("ts-image"))==null?void 0:o.value)==null?void 0:c.trim(),u=a?`<div style="padding:0 20px 12px;text-align:center"><img src="/icons/${a}" style="max-height:140px;max-width:100%;border-radius:10px;object-fit:contain"></div>`:"",h=document.getElementById("ts-preview-area");h.innerHTML=`
      ...
      <div style="flex:1;min-width:220px;display:flex;flex-direction:column;gap:12px">
        ...
        <div>
          ...
          <select id="st-country">
            ...
            ${Cn.map(([p,l])=>`<option value="${p}" ${(e==null?void 0:e.country_code)===p?"selected":""}>${l} (${p})</option>`).join("")}
          </select>
        </div>
        <div>
  ...
  `);const o=()=>{var v,w,k,$,L,A,S;const p=((v=document.getElementById("st-name"))==null?void 0:v.value)||"NOM DU STADE",l=(k=(w=document.getElementById("st-image"))==null?void 0:w.value)==null?void 0:k.trim(),x=(L=($=document.getElementById("st-country"))==null?void 0:$.value)==null?void 0:L.trim(),f=document.getElementById("st-club"),y=(f==null?void 0:f.selectedIndex)||0,I=f&&y>0?f.options[y].text:"",B=((S=(A=f==null?void 0:f.options[y])==null?void 0:A.getAttribute)==null?void 0:S.call(A,"data-logo"))||"";let E=null;l?E=l.startsWith("http")?l:`${ft}icons/${l}`:B?E=B:x&&(E=`https://flagsapi.com/${x.toLowerCase().slice(0,2).toUpperCase()}/flat/64.png`);const b=I||x||"—",d=document.getElementById("st-preview-card");d&&(d.innerHTML=Ze(p,E,`${b}<br>+10 ⭐`,{width:140}))};o(),["st-name","st-club","st-country","st-image"].forEach(p=>{var l,x;(l=document.getElementById(p))==null||l.addEventListener("input",o),(x=document.getElementById(p))==null||x.addEventListener("change",o)}),(c=document.getElementById("st-cancel"))==null||c.addEventListener("click",()=>a()),(r=document.getElementById("st-save"))==null||r.addEventListener("click",async()=>{const p=document.getElementById("st-name").value.trim(),l=document.getElementById("st-club").value||null,x=document.getElementById("st-country").value.trim().toUpperCase()||null,f=document.getElementById("st-image").value.trim()||null;if(!p){m("Le nom est obligatoire","error");return}const y={name:p,club_id:l,country_code:x,image_url:f},{error:I}=e?await _.from("stadium_definitions").update(y).eq("id",e.id):await _.from("stadium_definitions").insert(y);if(I){m("Erreur : "+I.message,"error");return}m(e?"Stade modifié ✅":"Stade créé ✅","success"),a(),ve(n,i)}),(g=document.getElementById("st-delete"))==null||g.addEventListener("click",async()=>{confirm("Supprimer ce stade ?")&&(await _.from("stadium_definitions").delete().eq("id",e.id),m("Stade supprimé","success"),a(),ve(n,i))})}const H=[{value:"game_changer",label:"⚡ Game Changer",border:"#7a28b8",bg:"linear-gradient(135deg,#4a0080,#7a28b8)"},{value:"ultra_game_changer",label:"💎 Ultra Game Changer",border:"#00bcd4",bg:"linear-gradient(135deg,#006080,#00bcd4)"}],zn=[{value:"purple",label:"Violet",hex:"#7a28b8"},{value:"light_blue",label:"Bleu clair",hex:"#00bcd4"}],G=[{value:"BOOST_STAT",label:"⚡ Boost stats (+N)",hasValue:!0,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"DEBUFF_STAT",label:"💀 Debuff stats (-N)",hasValue:!0,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"GRAY_PLAYER",label:"❄️ Griser un joueur",hasValue:!1,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"REVIVE_PLAYER",label:"💫 Ressusciter un joueur",hasValue:!1,hasTarget:!1,hasCount:!0,hasRoles:!1},{value:"REMOVE_GOAL",label:"🚫 Retirer un but adverse",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1},{value:"ADD_GOAL_DRAW",label:"⚽ +1 but si duel nul",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1},{value:"ADD_SUB",label:"🔄 Ajouter un remplacement",hasValue:!1,hasTarget:!1,hasCount:!0,hasRoles:!1},{value:"CUSTOM",label:"🛠️ Effet custom (hardcodé)",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1}],qe=["GK","DEF","MIL","ATT"];async function Rn(e,t){await Be(e,t)}async function Be(e,t){var i;const{data:n}=await _.from("gc_definitions").select("*").order("sort_order").order("created_at");e.innerHTML=`
  ...
  </div>`}function Ke(e,t,n){var g,p,l,x;const{openModal:i,closeModal:s,toast:a}=n,m=!e,u=e||{name:"",gc_type:"game_changer",color:"purple",effect:"",image_url:"",sort_order:0,is_active:!0,effect_type:"BOOST_STAT",effect_params:{}},h=(()=>{const f=u.effect_params||{},y=G.find(I=>I.value===(u.effect_type||"BOOST_STAT"))||G[0];return`
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
      ${y.hasValue?`<div>
    ...
    <div style="display:flex;justify-content:center;margin-bottom:16px;padding:20px;background:#1a1a2e;border-radius:12px">
      <div id="gc-modal-preview">${ge(u.name||"?",u.image_url?`/icons/${u.image_url}`:null,(H.find(f=>f.value===u.gc_type)||H[0]).label.split(" ")[0],u.effect||"",{width:150})}</div>
    </div>
    ...
    <div style="display:flex;flex-direction:column;gap:12px">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
        ...
        <div>
          <label>COULEUR</label>
          <select id="gc-color">
            ${zn.map(f=>`<option value="${f.value}" ${u.color===f.value?"selected":""}>${f.label}</option>`).join("")}
          </select>
        </div>
      </div>
      <div>
        ...
        <div style="display:flex;flex-direction:column;gap:4px">
          ${g.map(b=>{const d=f[b],v=r.length>0?Math.round(d/r.length*100):0;return`
              <div style="display:flex;align-items:center;gap:8px;font-size:12px">
                <span style="width:80px">${x[b]} ${p[b]}</span>
