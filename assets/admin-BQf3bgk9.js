import{s as w,r as Pe,a as Ne,A as et,l as tt,g as nt,b as Fe,K as Ee,D as T,S as it,c as R,H as ot,R as at,e as pe,d as Ge,f as rt,h as lt,E as st,i as dt,N as ct,M as pt,B as ut,j as mt,F as ue,k as we,m as gt,n as ft,o as bt,p as yt}from"./special-cards-DZxbxWIu.js";async function vt(e){const[{count:t},{count:n},{count:i},{count:p},{count:s}]=await Promise.all([w.from("players").select("*",{count:"exact",head:!0}),w.from("clubs").select("*",{count:"exact",head:!0}),w.from("users").select("*",{count:"exact",head:!0}),w.from("cards").select("*",{count:"exact",head:!0}),w.from("matches").select("*",{count:"exact",head:!0})]);e.innerHTML=`
...
Cette action est irréversible.`))return;const i=document.getElementById("fix-old-faces-btn");i&&(i.disabled=!0,i.textContent="⏳ En cours...");try{const{data:p,error:s}=await w.from("players").select("id, country_code, face").like("face","public/faces/%");if(s){n(s.message,"error");return}if(!(p!=null&&p.length)){n("Aucun joueur avec l'ancien format — rien à faire ✅","success");return}const{data:c}=await w.from("players").select("face").not("face","is",null),r=new Set((c||[]).map(m=>m.face).filter(m=>m&&!m.startsWith("public/faces/")));let b=0,o=0;for(const m of p){const l=await Ne(m.country_code,r);if(!l){o++;continue}const{error:a}=await w.from("players").update({face:l}).eq("id",m.id);if(a){o++;continue}r.add(l),b++}n(`${b} photo(s) réattribuée(s)${o?`, ${o} échec(s)`:""} ✅`,o?"error":"success")}finally{i&&(i.disabled=!1,i.textContent="🖼️ Réattribuer anciennes photos")}}async function wt(e,t){await se(e,t)}async function se(e,t,n=null){var o,m,l,a,d;const{toast:i}=t;n||(n={search:((o=document.getElementById("search-players"))==null?void 0:o.value)||"",job:((m=document.getElementById("filter-job"))==null?void 0:m.value)||"",rarity:((l=document.getElementById("filter-rarity"))==null?void 0:l.value)||"",club:((a=document.getElementById("filter-club"))==null?void 0:a.value)||"",country:((d=document.getElementById("filter-country"))==null?void 0:d.value)||""});const[{data:p,error:s},{data:c}]=await Promise.all([w.from("players").select("*, clubs(id,encoded_name,logo_url)"),w.from("clubs").select("id,encoded_name").order("encoded_name")]);if(s){e.innerHTML=`<p style="color:red">${s.message}</p>`;return}const r={GK:0,DEF:1,MIL:2,ATT:3},b=(p||[]).sort((u,h)=>{const E=(r[u.job]??4)-(r[h.job]??4);return E!==0?E:(u.surname_real||"").localeCompare(h.surname_real||"")});_t(e,b,c||[],t,n)}function _t(e,t,n,i,p=null){var m,l;const{toast:s}=i;if(e.innerHTML=`
      ...
      <div style="flex:1;min-width:300px;display:flex;flex-direction:column;gap:12px">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:8px">
          ...
          <div class="form-group">
            <label>Rareté</label>
            <select id="pm-rarity">
              ${["normal","pepite","papyte","legende"].map(u=>`<option value="${u}" ${(e==null?void 0:e.rarity)===u?"selected":""}>${He[u]}</option>`).join("")}
            </select>
          </div>
          <div class="form-group">
            <label>Pays</label>
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:8px">
            ${[["GK","pm-g","note_g"],["DEF","pm-d","note_d"],["MIL","pm-m","note_m"],["ATT","pm-a","note_a"]].map(([u,h,E])=>`
              <div class="form-group" style="text-align:center">
                <label style="color:${xt[u]};font-weight:700">${u}</label>
    ...
    <div id="clubs-list" style="display:flex;flex-direction:column;gap:8px"></div>`,c(ae),document.getElementById("search-clubs").addEventListener("input",r=>{const b=r.target.value.toLowerCase();c(ae.filter(o=>o.real_name.toLowerCase().includes(b)||o.encoded_name.toLowerCase().includes(b)))}),document.getElementById("add-club-btn").addEventListener("click",()=>$e(null,e,t));function c(r){const b=document.getElementById("clubs-list");if(!r.length){b.innerHTML='<p style="color:var(--gray-600);padding:20px">Aucun club.</p>';return}b.innerHTML=r.map(o=>{const m=Ke(o),l=Fe(m,o.id).replace("<svg ",'<svg style="width:40px;height:48px" '),a=o.logo_url?`<img src="${o.logo_url}" style="width:40px;height:40px;object-fit:contain;border-radius:8px">`:`<div style="width:40px;height:40px;background:linear-gradient(135deg,${m.color1},${m.color2});border-radius:8px;display:flex;align-items:center;justify-content:center;color:#fff;font-size:10px;font-weight:900">${o.encoded_name.slice(0,3)}</div>`,d=n[o.id]||0;return`
        ...
        </div>`}).join(""),b.querySelectorAll("[data-gen]").forEach(o=>{o.addEventListener("click",async()=>{confirm(`Générer 20 joueurs pour ${o.dataset.name} ?`)&&(o.disabled=!0,o.textContent="⏳",await Ue(o.dataset.gen,o.dataset.cc,i),de(e,t))})}),b.querySelectorAll("[data-edit]").forEach(o=>{const m=ae.find(l=>l.id===o.dataset.edit);o.addEventListener("click",()=>$e(m,e,t))}),b.querySelectorAll("[data-del]").forEach(o=>{o.addEventListener("click",async()=>{if(!confirm("Supprimer ce club ?"))return;const{error:m}=await w.from("clubs").delete().eq("id",o.dataset.del);m?i(m.message,"error"):(i("Club supprimé","success"),de(e,t))})})}}function $e(e,t,n){const{toast:i,openModal:p,closeModal:s}=n,c=!!e,r=e?Ke(e):{...T},b=Object.entries(Ee).map(([m,l])=>`<option value="${m}" ${r.style===m?"selected":""}>${l.label}</option>`).join(""),o=qe.map(m=>`<option value="${m}" ${((e==null?void 0:e.country_code)||"FR")===m?"selected":""}>${m}</option>`).join("");p(c?`✏️ ${e.real_name}`:"➕ Nouveau club",`<div style="display:flex;flex-direction:column;gap:14px">
    ...
    </div>`),setTimeout(()=>{var u,h,E;te();function m(){var x,f;const g=((x=document.getElementById("m-kit-style"))==null?void 0:x.value)||"uni",$=((f=Ee[g])==null?void 0:f.colors)===3,k=document.getElementById("wrap-m-kit-color3");k&&(k.style.display=$?"":"none")}m(),["m-kit-color1","m-kit-color2","m-kit-color3","m-kit-shorts","m-kit-socks"].forEach(g=>{const $=document.getElementById(g),k=document.getElementById(g+"-txt");!$||!k||($.addEventListener("input",()=>{k.value=$.value,te()}),k.addEventListener("input",()=>{const x=k.value.trim();/^#[0-9a-fA-F]{6}$/.test(x)&&($.value=x,te())}),k.addEventListener("change",()=>{let x=k.value.trim();x.startsWith("#")||(x="#"+x),/^#[0-9a-fA-F]{6}$/.test(x)&&($.value=x,k.value=x,te())}))}),(u=document.getElementById("m-kit-style"))==null||u.addEventListener("change",()=>{m(),te()});const l=document.getElementById("m-real"),a=document.getElementById("m-encoded");function d(){if(!l||!a||a.value)return;const g=l.value.trim().split(/\s+/),$=g.length===1?g[0].toUpperCase().slice(0,6):g.filter(k=>k.length>2).map(k=>k[0].toUpperCase()).join("")||g[0].toUpperCase().slice(0,4);a.value=$}l==null||l.addEventListener("input",d),(h=document.getElementById("auto-encode-btn"))==null||h.addEventListener("click",()=>{a&&(a.value=""),d()}),(E=document.getElementById("m-save"))==null||E.addEventListener("click",()=>jt(e,c,t,n))},50)}function Ve(){var e,t,n,i,p,s;return{style:((e=document.getElementById("m-kit-style"))==null?void 0:e.value)||"uni",color1:((t=document.getElementById("m-kit-color1"))==null?void 0:t.value)||"#1A6B3C",color2:((n=document.getElementById("m-kit-color2"))==null?void 0:n.value)||"#ffffff",color3:((i=document.getElementById("m-kit-color3"))==null?void 0:i.value)||"#000000",shorts:((p=document.getElementById("m-kit-shorts"))==null?void 0:p.value)||"#111111",socks:((s=document.getElementById("m-kit-socks"))==null?void 0:s.value)||"#ffffff"}}function te(){const e=document.getElementById("kit-preview-wrap");e&&(e.innerHTML=Fe(Ve(),"modal"))}async function jt(e,t,n,i){var g,$,k,x,f,y;const{toast:p,closeModal:s}=i,c=document.getElementById("m-error"),r=document.getElementById("m-save"),b=(g=document.getElementById("m-real"))==null?void 0:g.value.trim(),o=($=document.getElementById("m-encoded"))==null?void 0:$.value.trim().toUpperCase(),m=(k=document.getElementById("m-country"))==null?void 0:k.value.trim().toUpperCase(),l=((x=document.getElementById("m-logo"))==null?void 0:x.value.trim())||null,a=((f=document.getElementById("m-gen-stadium"))==null?void 0:f.checked)??!1,d=((y=document.getElementById("m-gen-squad"))==null?void 0:y.checked)??!1,u=Ve();if(!b){c.textContent="Le nom du club est requis.";return}if(!o){c.textContent="Le nom encodé est requis.";return}if(!m){c.textContent="Le pays est requis.";return}r.disabled=!0,r.textContent="Enregistrement…";const h={real_name:b,encoded_name:o,country_code:m,logo_url:l,kit_style:u.style,kit_color1:u.color1,kit_color2:u.color2,kit_color3:u.color3,kit_shorts:u.shorts,kit_socks:u.socks};let E=e==null?void 0:e.id;if(t){const{error:v}=await w.from("clubs").update(h).eq("id",E);if(v){c.textContent=v.message,r.disabled=!1,r.textContent="💾 Enregistrer";return}}else{const{data:v,error:_}=await w.from("clubs").insert(h).select().single();if(_){c.textContent=_.message,r.disabled=!1,r.textContent="✅ Créer le club";return}if(E=v.id,a){r.textContent="🏟️ Création du stade…";const{data:B,error:I}=await w.from("stadium_definitions").insert({name:`Stade de ${b}`,club_id:E,country_code:m}).select().single();I?console.warn("[Stadium] Erreur def stade:",I.message):B&&await w.from("cards").insert({card_type:"stadium",stadium_id:B.id})}d&&(r.textContent="⚽ Génération des joueurs…",await Ue(E,m,p))}p(t?"Club modifié ✅":"Club créé ✅","success"),s(),de(n,i)}const Dt=["normal","pepite","papyte","legende"],Ie=["GK","DEF","MIL","ATT"],Pt=[["MA","Maroc"],["FR","France"],["AR","Argentine"],["PT","Portugal"],["BR","Brésil"],["ES","Espagne"],["DE","Allemagne"],["GB","Angleterre"],["IT","Italie"],["CM","Cameroun"],["SN","Sénégal"],["NG","Nigéria"],["DK","Danemark"],["NL","Pays-Bas"],["BE","Belgique"],["CI","Côte d'Ivoire"],["AL","Albanie"],["HR","Croatie"],["RS","Serbie"],["TR","Turquie"],["MW","Malawi"]];let U={...R},re=[];async function Nt(e,{toast:t}){const{data:n}=await w.from("clubs").select("id,encoded_name,logo_url,kit_style,kit_color1,kit_color2,kit_shorts,kit_socks").order("encoded_name");re=n||[],U={...R},e.innerHTML=qt(re),Gt(e,re,t),N()}function Be(){var n;const e=((n=document.getElementById("cb-surname-real"))==null?void 0:n.value)||"",t=document.getElementById("cb-surname-enc");t&&(t.value=pe(e))}function Ft(){var n;const e=(n=document.getElementById("cb-club"))==null?void 0:n.value,t=re.find(i=>String(i.id)===String(e));return t?{style:t.kit_style||T.style,color1:t.kit_color1||T.color1,color2:t.kit_color2||T.color2,shorts:t.kit_shorts||T.shorts,socks:t.kit_socks||T.socks}:{...T}}function N(){var a,d,u,h;const e=E=>{var g;return((g=document.getElementById(E))==null?void 0:g.value)||""},t={firstname:e("cb-firstname"),surname_real:e("cb-surname-enc")||pe(e("cb-surname-real")),country_code:e("cb-country"),club_encoded_name:((d=(a=document.getElementById("cb-club"))==null?void 0:a.selectedOptions[0])==null?void 0:d.text)||"",job:e("cb-job"),job2:e("cb-job2")||null,note_g:parseInt(e("cb-note-g"))||0,note_d:parseInt(e("cb-note-d"))||0,note_m:parseInt(e("cb-note-m"))||0,note_a:parseInt(e("cb-note-a"))||0,rarity:e("cb-rarity")},n=Ft(),i=document.getElementById("cb-club"),p=((h=(u=i==null?void 0:i.selectedOptions[0])==null?void 0:u.dataset)==null?void 0:h.logo)||null,s=Ge(U,n,120),c=`data:image/svg+xml;charset=utf-8,${encodeURIComponent(s)}`,r=rt(t,{portraitUrl:c,clubLogoUrl:p,showNotes:!0}),b=document.getElementById("card-preview");b&&(b.innerHTML=r);const o=document.getElementById("avatar-preview-wrap");o&&(o.innerHTML=s);const m=e("cb-surname-real"),l=document.getElementById("encode-summary");l&&m&&(l.textContent=`${e("cb-firstname")} ${m} → ${e("cb-firstname")} ${t.surname_real}`)}function Je(){const e=t=>{var n;return((n=document.getElementById(t))==null?void 0:n.value)||""};U={skin:e("av-skin"),hair:e("av-hair"),hairColor:e("av-hairColor"),eyes:e("av-eyes"),nose:e("av-nose"),mouth:e("av-mouth"),eyebrows:e("av-eyebrows"),beard:e("av-beard")}}function Gt(e,t,n){var p,s,c,r,b;e.querySelectorAll("input,select").forEach(o=>{o.addEventListener("input",N),o.addEventListener("change",N)});const i=["av-skin","av-hair","av-hairColor","av-eyes","av-nose","av-mouth","av-eyebrows","av-beard"];i.forEach(o=>{var m;(m=document.getElementById(o))==null||m.addEventListener("change",()=>{Je(),N()})}),(p=document.getElementById("cb-surname-real"))==null||p.addEventListener("input",()=>{Be(),N()}),(s=document.getElementById("btn-encode"))==null||s.addEventListener("click",()=>{Be(),N()}),(c=document.getElementById("cb-club"))==null||c.addEventListener("change",N),(r=document.getElementById("btn-save-player"))==null||r.addEventListener("click",()=>Ht(n)),(b=document.getElementById("btn-reset"))==null||b.addEventListener("click",()=>{e.querySelectorAll("input").forEach(o=>o.value=""),e.querySelectorAll("select").forEach(o=>o.selectedIndex=0),U={...R},i.forEach(o=>{const m=o.replace("av-",""),l=document.getElementById(o);l&&(l.value=R[m]||"")}),N()})}async function Ht(e){const t=c=>{var r;return((r=document.getElementById(c))==null?void 0:r.value)||""},n=t("cb-surname-real").trim(),i=t("cb-surname-enc").trim()||pe(n);if(!t("cb-firstname")||!n||!t("cb-country")||!t("cb-job")){e("Remplissez les champs obligatoires (prénom, nom, pays, poste)","error");return}Je();const p={firstname:t("cb-firstname").trim(),surname_real:n,surname_real:i,country_code:t("cb-country"),club_id:t("cb-club")||null,job:t("cb-job"),job2:t("cb-job2")||null,note_g:parseInt(t("cb-note-g"))||0,note_d:parseInt(t("cb-note-d"))||0,note_m:parseInt(t("cb-note-m"))||0,note_a:parseInt(t("cb-note-a"))||0,rarity:t("cb-rarity")||"normal",note_min:parseInt(t("cb-note-min"))||null,note_max:parseInt(t("cb-note-max"))||null,sell_price:parseInt(t("cb-price"))||0,avatar_config:U,skin:U.skin||"blanc",hair:U.hairColor||"marron",hair_length:"court"},{error:s}=await w.from("players").insert(p);if(s){e("Erreur: "+s.message,"error");return}e(`✅ Joueur "${p.firstname} ${p.surname_real}" enregistré !`,"success")}function qt(e){const t=Pt.map(([o,m])=>`<option value="${o}">${m} (${o})</option>`).join(""),n='<option value="">— Sélectionner un club —</option>'+e.map(o=>`<option value="${o.id}" data-logo="${o.logo_url||""}">${o.encoded_name}</option>`).join(""),i=Ie.map(o=>`<option value="${o}">${o}</option>`).join(""),p='<option value="">Aucun</option>'+Ie.map(o=>`<option value="${o}">${o}</option>`).join(""),s=Dt.map(o=>`<option value="${o}">${o.charAt(0).toUpperCase()+o.slice(1)}</option>`).join(""),c=(o,m,l)=>Object.entries(m).map(([a,d])=>`<option value="${a}" ${a===(l||"")?"selected":""}>${d.label}</option>`).join(""),r=Object.keys(it).map(o=>`<option value="${o}" ${o===R.skin?"selected":""}>${o.charAt(0).toUpperCase()+o.slice(1)}</option>`).join(""),b=Object.keys(ot).map(o=>`<option value="${o}" ${o===R.hairColor?"selected":""}>${o.charAt(0).toUpperCase()+o.slice(1)}</option>`).join("");return`
        ...
        <div style="display:grid;grid-template-columns:1fr 120px;gap:16px;align-items:start">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
            ...
            <div>
              ...
              <select id="av-hair">${c("av-hair",lt,R.hair)}</select>
            </div>
            <div>
            ...
            <div>
              ...
              <select id="av-eyebrows">${c("av-eyebrows",st,R.eyebrows)}</select>
            </div>
            <div>
              <label>Yeux</label>
              <select id="av-eyes">${c("av-eyes",dt,R.eyes)}</select>
            </div>
            <div>
              <label>Nez</label>
              <select id="av-nose">${c("av-nose",ct,R.nose)}</select>
            </div>
            <div>
              <label>Bouche</label>
              <select id="av-mouth">${c("av-mouth",pt,R.mouth)}</select>
            </div>
            <div>
              <label>Barbe</label>
              <select id="av-beard">${c("av-beard",ut,R.beard)}</select>
            </div>
          </div>
...
`}),ie("players_export.csv",p),t(`${n.length} joueurs exportés`,"success")}),document.getElementById("import-clubs").addEventListener("change",async n=>{const i=n.target.files[0];if(!i)return;const p=document.getElementById("clubs-import-result");p.innerHTML='<span style="color:var(--gray-600)">Import en cours...</span>';try{const s=await i.text(),c=Le(s);if(c.length===0){p.innerHTML='<span style="color:#c0392b">Fichier vide</span>';return}let r=0,b=0;const o=[];for(const m of c){if(!m.real_name||!m.encoded_name||!m.country_code){b++,o.push(`Ligne ignorée (champs manquants): ${m.real_name||"?"}`);continue}const l={real_name:m.real_name,encoded_name:m.encoded_name.toUpperCase(),country_code:m.country_code.toUpperCase().slice(0,2),logo_url:m.logo_url||null},{error:a}=await w.from("clubs").upsert(l,{onConflict:"encoded_name"});a?(b++,o.push(`${m.encoded_name}: ${a.message}`)):r++}p.innerHTML=`<div style="color:var(--green)">✅ ${r} clubs importés</div>
        ...
        ${m>0?`<div style="color:#c0392b">❌ ${m} erreurs</div><div style="font-size:11px;color:var(--gray-600);max-height:100px;overflow-y:auto">${l.slice(0,10).join("<br>")}</div>`:""}`,t(`${o} joueurs importés`,"success")}catch(s){p.innerHTML=`<span style="color:#c0392b">Erreur : ${s.message}</span>`}n.target.value=""})}function Se(e){const t=String(e??"");return t.includes(",")||t.includes('"')||t.includes(`
`)?'"'+t.replace(/"/g,'""')+'"':t}function ie(e,t){const n=new Blob(["\uFEFF"+t],{type:"text/csv;charset=utf-8;"}),i=URL.createObjectURL(n),p=document.createElement("a");p.href=i,p.download=e,p.click(),URL.revokeObjectURL(i)}function Le(e){e=e.replace(/^\uFEFF/,"").replace(/\r\n/g,`
`).replace(/\r/g,`
`);const t=e.split(`
`).filter(c=>c.trim());if(t.length<2)return[];const n=t[0],i=n.split(";").length>n.split(",").length?";":",",p=Ce(n,i).map(c=>c.trim().replace(/^\uFEFF/,"").toLowerCase()),s=[];for(let c=1;c<t.length;c++){if(!t[c].trim())continue;const r=Ce(t[c],i),b={};p.forEach((o,m)=>{b[o]=(r[m]||"").trim()}),!Object.values(b).every(o=>!o)&&s.push(b)}return s}function Ce(e,t=","){const n=[];let i="",p=!1;for(let s=0;s<e.length;s++){const c=e[s];p?c==='"'?e[s+1]==='"'?(i+='"',s++):p=!1:i+=c:c==='"'?p=!0:c===t?(n.push(i),i=""):i+=c}return n.push(i),n}const Ut={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};async function Kt(e,{toast:t}){const n=Object.keys(ue);let i=n[0];const{data:p}=await w.from("formation_links_overrides").select("formation, links"),s={};(p||[]).forEach(m=>{s[m.formation]=m.links});let c=new Set;function r(m,l){return[m,l].sort().join("-")}function b(m){const l=s[m]||ue[m]||[];c=new Set(l.map(([a,d])=>r(a,d)))}b(i),e.innerHTML=`
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;gap:12px;flex-wrap:wrap">
  ...
  `;function o(){const m=gt[i]||{},l=we(i),a=320,d=400,u=22;function h(x){const f=m[x];return f?{x:f.x*a,y:f.y*d}:null}let E=`<svg width="${a}" height="${d}" viewBox="0 0 ${a} ${d}" xmlns="http://www.w3.org/2000/svg" style="display:block">`;l.forEach(([x,f],y)=>{const v=h(x),_=h(f);if(!v||!_)return;const B=r(x,f),I=c.has(B),L=I?"#3b82f6":"#999",S=I?.95:.35,A=I?4:3;E+=`<line x1="${v.x}" y1="${v.y}" x2="${_.x}" y2="${_.y}"
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
          <div>
            ...
            <select id="f-price-type" style="width:100%;padding:9px;border:1px solid #ddd;border-radius:6px;font-size:13px">
              ${Wt.map(u=>`<option value="${u.value}" ${l.price_type===u.value?"selected":""}>${u.label}</option>`).join("")}
            </select>
          </div>
        ...
        ${i.length===0?'<div style="text-align:center;padding:16px;color:#aaa;font-size:13px">Aucun taux. Cliquez + Ligne.</div>':i.map((u,h)=>`
          <div style="display:grid;grid-template-columns:2fr 1.5fr 50px 50px 60px 32px;gap:4px;align-items:center;margin-bottom:6px;padding:6px;background:#f9f9f9;border-radius:6px" data-rate-idx="${h}">
            <select class="rate-type" data-idx="${h}" style="padding:6px 4px;border:1px solid #ddd;border-radius:4px;font-size:12px;width:100%">
              ${Vt.map(E=>`<option value="${E.value}" ${u.card_type===E.value?"selected":""}>${E.label}</option>`).join("")}
            </select>
            <select class="rate-rarity" data-idx="${h}" style="padding:6px 4px;border:1px solid #ddd;border-radius:4px;font-size:12px;width:100%">
              ${Jt.map(E=>`<option value="${E.value}" ${(u.rarity||"")===E.value?"selected":""}>${E.label}</option>`).join("")}
            </select>
    ...
    </div>`,document.getElementById("add-config").addEventListener("click",()=>Ae(null)),document.getElementById("form-cancel").addEventListener("click",()=>{document.getElementById("config-form").style.display="none"}),document.getElementById("form-save").addEventListener("click",()=>Qt(e)),e.querySelectorAll("[data-edit]").forEach(i=>{const p=(t||[]).find(s=>s.id===i.dataset.edit);p&&i.addEventListener("click",()=>Ae(p))}),e.querySelectorAll("[data-delete]").forEach(i=>{i.addEventListener("click",async()=>{confirm("Supprimer cette règle ?")&&(await w.from("sell_price_configs").delete().eq("id",i.dataset.delete),await ge(e))})})}function Xt(e){const t=Ze.find(n=>n.value===e.rarity);return`
    ...
    </tr>`}function Ae(e){document.getElementById("config-form").style.display="block",document.getElementById("form-title").textContent=e?"Modifier la règle":"Nouvelle règle",document.getElementById("form-id").value=(e==null?void 0:e.id)||"",document.getElementById("form-rarity").value=(e==null?void 0:e.rarity)||"normal",document.getElementById("form-price").value=(e==null?void 0:e.price)||1e3,document.getElementById("form-note-min").value=(e==null?void 0:e.note_min)??1,document.getElementById("form-note-max").value=(e==null?void 0:e.note_max)??10,document.getElementById("config-form").scrollIntoView({behavior:"smooth"})}async function Qt(e){const t=document.getElementById("form-id").value,n=document.getElementById("form-rarity").value,i=parseInt(document.getElementById("form-price").value)||0,p=parseInt(document.getElementById("form-note-min").value)||1,s=parseInt(document.getElementById("form-note-max").value)||10;if(p>s){alert("Note min doit être ≤ note max");return}const c={rarity:n,price:i,note_min:p,note_max:s,updated_at:new Date().toISOString()};let r;if(t?{error:r}=await w.from("sell_price_configs").update(c).eq("id",t):{error:r}=await w.from("sell_price_configs").insert(c),r){alert("Erreur : "+r.message);return}document.getElementById("config-form").style.display="none",await ge(e)}async function en(e){await fe(e)}async function fe(e){const{data:t}=await w.from("patch_notes").select("*").order("published_at",{ascending:!1});e.innerHTML=`
    ...
    </div>`,document.getElementById("add-article").addEventListener("click",()=>ze(null)),document.getElementById("form-cancel").addEventListener("click",()=>{document.getElementById("article-form").style.display="none"}),document.getElementById("form-save").addEventListener("click",()=>nn(e)),e.querySelectorAll("[data-edit]").forEach(n=>{const i=(t||[]).find(p=>p.id===n.dataset.edit);i&&n.addEventListener("click",()=>ze(i))}),e.querySelectorAll("[data-delete]").forEach(n=>{n.addEventListener("click",async()=>{confirm("Supprimer cet article ?")&&(await w.from("patch_notes").delete().eq("id",n.dataset.delete),await fe(e))})})}function tn(e){const t=new Date(e.published_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"short",year:"numeric"});return`
  ...
  <div style="background:#fff;border-radius:10px;padding:12px 16px;box-shadow:0 1px 4px rgba(0,0,0,0.08);display:flex;align-items:center;gap:12px">
    ...
    </div>`,(i=document.getElementById("st-add-btn"))==null||i.addEventListener("click",()=>Te(null)),(p=document.getElementById("st-cancel"))==null||p.addEventListener("click",()=>{document.getElementById("st-form").style.display="none"}),(s=document.getElementById("st-save"))==null||s.addEventListener("click",()=>un(e)),(c=document.getElementById("st-preview-btn"))==null||c.addEventListener("click",()=>le()),["st-name","st-club","st-country","st-image"].forEach(r=>{var b,o;(b=document.getElementById(r))==null||b.addEventListener("input",le),(o=document.getElementById(r))==null||o.addEventListener("change",le)}),e.querySelectorAll("[data-edit]").forEach(r=>{const b=(t||[]).find(o=>o.id===r.dataset.edit);b&&r.addEventListener("click",()=>Te(b))}),e.querySelectorAll("[data-delete]").forEach(r=>{r.addEventListener("click",async()=>{confirm("Supprimer ce stade ?")&&(await w.from("stadium_definitions").delete().eq("id",r.dataset.delete),be(e))})})}function pn(e){var n,i;let t;return(n=e.club)!=null&&n.logo_url?t=`<img src="${e.club.logo_url}" style="width:32px;height:32px;object-fit:contain">`:e.country_code?t=`<img src="https://flagsapi.com/${e.country_code.slice(0,2).toUpperCase()}/flat/32.png" style="width:32px;height:24px;object-fit:contain;border-radius:3px" onerror="this.style.display='none'">`:t='<span style="font-size:20px">🌍</span>',`
    <div style="background:#fff;border-radius:10px;padding:12px 16px;box-shadow:0 1px 4px rgba(0,0,0,0.08);display:flex;align-items:center;gap:12px">
      ...
      <button data-edit="${e.id}" class="btn btn-ghost btn-sm">✏️</button>
      <button data-delete="${e.id}" class="btn btn-ghost btn-sm" style="color:#cc2222">🗑️</button>
    </div>`}function Te(e,t){var i,p;document.getElementById("st-form").style.display="block",document.getElementById("st-form-title").textContent=e?"Modifier le stade":"Nouveau stade",document.getElementById("st-id").value=(e==null?void 0:e.id)||"",document.getElementById("st-name").value=(e==null?void 0:e.name)||"",document.getElementById("st-club").value=(e==null?void 0:e.club_id)||"";const n=document.getElementById("st-country");n&&(n.value=(e==null?void 0:e.country_code)||""),document.getElementById("st-image").value=(e==null?void 0:e.image_url)||"",document.getElementById("st-preview-card").innerHTML="",le((i=e==null?void 0:e.club)==null?void 0:i.logo_url,(p=e==null?void 0:e.club)==null?void 0:p.encoded_name),document.getElementById("st-form").scrollIntoView({behavior:"smooth"})}function le(){var o,m,l,a,d,u,h;const e=((o=document.getElementById("st-name"))==null?void 0:o.value)||"NOM DU STADE",t=(l=(m=document.getElementById("st-image"))==null?void 0:m.value)==null?void 0:l.trim(),n=(d=(a=document.getElementById("st-country"))==null?void 0:a.value)==null?void 0:d.trim(),i=document.getElementById("st-club"),p=(i==null?void 0:i.selectedIndex)||0,s=i&&p>0?i.options[p].text:"",c=((h=(u=i==null?void 0:i.options[p])==null?void 0:u.getAttribute)==null?void 0:h.call(u,"data-logo"))||"";let r=null;t?r=`${sn}icons/${t}`:c?r=c:n&&(r=`https://flagsapi.com/${n.toLowerCase().slice(0,2).toUpperCase()}/flat/64.png`);const b=s||n||"—";document.getElementById("st-preview-card").innerHTML=ft(e,r,`${b}<br>+10 ⭐`,{width:140})}async function un(e){const t=document.getElementById("st-id").value,n=document.getElementById("st-name").value.trim(),i=document.getElementById("st-club").value||null,p=document.getElementById("st-country").value.trim().toUpperCase()||null,s=document.getElementById("st-image").value.trim()||null;if(!n){alert("Le nom est obligatoire");return}const c={name:n,club_id:i,country_code:p,image_url:s},{error:r}=t?await w.from("stadium_definitions").update(c).eq("id",t):await w.from("stadium_definitions").insert(c);if(r){alert("Erreur : "+r.message);return}document.getElementById("st-form").style.display="none",be(e)}const O=[{value:"game_changer",label:"⚡ Game Changer",border:"#7a28b8",bg:"linear-gradient(135deg,#4a0080,#7a28b8)"},{value:"ultra_game_changer",label:"💎 Ultra Game Changer",border:"#00bcd4",bg:"linear-gradient(135deg,#006080,#00bcd4)"}],ne=[{value:"purple",label:"Violet",hex:"#7a28b8"},{value:"light_blue",label:"Bleu clair",hex:"#00bcd4"}];async function mn(e){e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">Chargement...</div>';const{data:t}=await w.from("gc_definitions").select("*").order("sort_order").order("created_at");let n=null;const i=[{value:"BOOST_STAT",label:"⚡ Boost stats (+N)",hasValue:!0,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"DEBUFF_STAT",label:"💀 Debuff stats (-N)",hasValue:!0,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"GRAY_PLAYER",label:"❄️ Griser un joueur",hasValue:!1,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"REVIVE_PLAYER",label:"💫 Ressusciter un joueur",hasValue:!1,hasTarget:!1,hasCount:!0,hasRoles:!1},{value:"REMOVE_GOAL",label:"🚫 Retirer un but adverse",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1},{value:"ADD_GOAL_DRAW",label:"⚽ +1 but si duel nul",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1},{value:"ADD_SUB",label:"🔄 Ajouter un remplacement",hasValue:!1,hasTarget:!1,hasCount:!0,hasRoles:!1},{value:"CUSTOM",label:"🛠️ Effet custom (hardcodé)",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1}],p=["GK","DEF","MIL","ATT"],s=()=>window.innerWidth<700,c=a=>e.querySelector(a);function r(a,d=130){if(!a)return"";const u=O.find(E=>E.value===a.gc_type)||O[0],h=a.image_url?`/icons/${a.image_url}`:null;return bt(a.name||"?",h,u.label.split(" ")[0],a.effect||"",{width:d})}function b(){return`
    <div id="gc-list" style="background:#fff;${s()?"":"border-right:1px solid #e0e0e0;width:280px;flex-shrink:0;"}overflow-y:auto;height:100%">
      ...
      ${(t||[]).map(a=>{const d=O.find(h=>h.value===a.gc_type)||O[0],u=ne.find(h=>h.value===a.color)||ne[0];return`
    ...
    </div>`,(x=document.getElementById("create-season-btn"))==null||x.addEventListener("click",()=>{Re(null,{toast:n,openModal:i,closeModal:p,reload:()=>V(e,t)})}),e.querySelectorAll("[data-edit]").forEach(f=>{const y=b.find(v=>v.id==f.dataset.edit);f.addEventListener("click",()=>{Re(y,{toast:n,openModal:i,closeModal:p,reload:()=>V(e,t)})})}),e.querySelectorAll("[data-activate]").forEach(f=>{f.addEventListener("click",async()=>{const y=parseInt(f.dataset.activate);if(!confirm("Activer cette saison ? La saison actuellement active sera désactivée."))return;const{error:v}=await w.from("ranked_seasons").update({is_active:!1}).neq("id",0);if(v){n(v.message,"error");return}const{error:_}=await w.from("ranked_seasons").update({is_active:!0}).eq("id",y);if(_){n(_.message,"error");return}await w.from("users").update({current_season_id:y}).gt("placement_matches",0),n("Saison activée ✅","success"),V(e,t)})}),e.querySelectorAll("[data-deactivate]").forEach(f=>{f.addEventListener("click",async()=>{if(!confirm("Désactiver la saison active ? Le mode Ranked sera suspendu."))return;const{error:y}=await w.from("ranked_seasons").update({is_active:!1}).eq("id",parseInt(f.dataset.deactivate));if(y){n(y.message,"error");return}n("Saison désactivée","success"),V(e,t)})}),e.querySelectorAll("[data-delete]").forEach(f=>{f.addEventListener("click",async()=>{if(!confirm("Supprimer cette saison archivée ?"))return;const{error:y}=await w.from("ranked_seasons").delete().eq("id",parseInt(f.dataset.delete));if(y){n(y.message,"error");return}n("Saison supprimée","success"),V(e,t)})})}function Re(e,{toast:t,openModal:n,closeModal:i,reload:p}){const s=!!e,c=new Date().toISOString().slice(0,16),r=new Date(Date.now()+90*864e5).toISOString().slice(0,16),b=e?new Date(e.start_at).toISOString().slice(0,16):c,o=e?new Date(e.end_at).toISOString().slice(0,16):r,m=(e==null?void 0:e.name)||"";n(s?"✏️ Modifier la saison":"➕ Nouvelle saison",`<div style="display:flex;flex-direction:column;gap:14px">
      <div class="form-group">
