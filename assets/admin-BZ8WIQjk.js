import{s as x,e as K,g as xe,K as ye,D as A,S as Ie,a as B,H as Le,R as ze,b as he,r as Be,c as Ae,E as Se,d as Ce,N as Te,M as je,B as Re,F as ee,f as re,h as Me,i as Ne}from"./formation-links-CEzN0rcz.js";async function Pe(e){const[{count:t},{count:n},{count:o},{count:u},{count:r}]=await Promise.all([x.from("players").select("*",{count:"exact",head:!0}),x.from("clubs").select("*",{count:"exact",head:!0}),x.from("users").select("*",{count:"exact",head:!0}),x.from("cards").select("*",{count:"exact",head:!0}),x.from("matches").select("*",{count:"exact",head:!0})]);e.innerHTML=`
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:12px;margin-bottom:24px">
      ${V("🃏","Joueurs (cartes)",t??"–","#1A6B3C")}
      ${V("🏟️","Clubs",n??"–","#D4A017")}
      ${V("👥","Managers",o??"–","#7a28b8")}
      ${V("📦","Cartes possédées",u??"–","#2a8f52")}
  ...
  `,d(r);function d(l){document.getElementById("count-label").textContent=`${l.length} joueur(s)`,document.getElementById("players-tbody").innerHTML=l.map(i=>{var f;return`
    ...
    `}).join(""),document.querySelectorAll("[data-edit]").forEach(i=>{i.addEventListener("click",()=>{const f=r.find(m=>m.id===i.dataset.edit);se(f,s||[],{toast:n,openModal:o,closeModal:u,reload:()=>J(e,t)})})}),document.querySelectorAll("[data-del]").forEach(i=>{i.addEventListener("click",async()=>{if(!confirm("Supprimer ce joueur ? Les cartes associées resteront en jeu."))return;const{error:f}=await x.from("players").delete().eq("id",i.dataset.del);f?n(f.message,"error"):(n("Joueur supprimé","success"),J(e,t))})})}function b(){const l=document.getElementById("search-players").value.toLowerCase(),i=document.getElementById("filter-rarity").value,f=document.getElementById("filter-job").value;d(r.filter(m=>(!l||`${m.firstname} ${m.surname_encoded} ${m.surname_real}`.toLowerCase().includes(l))&&(!i||m.rarity===i)&&(!f||m.job===f)))}document.getElementById("search-players").addEventListener("input",b),document.getElementById("filter-rarity").addEventListener("change",b),document.getElementById("filter-job").addEventListener("change",b),document.getElementById("add-player-btn").addEventListener("click",()=>{se(null,s||[],{toast:n,openModal:o,closeModal:u,reload:()=>J(e,t)})})}function se(e,t,{toast:n,openModal:o,closeModal:u,reload:r}){const a=!!e,s='<option value="">— Club —</option>'+t.map(c=>`<option value="${c.id}" ${(e==null?void 0:e.club_id)===c.id?"selected":""}>${c.encoded_name}</option>`).join("");o(a?`Modifier ${e.firstname} ${e.surname_encoded}`:"Nouveau joueur",`<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
  ...
  `,u(W),document.getElementById("search-clubs").addEventListener("input",r=>{const a=r.target.value.toLowerCase();u(W.filter(s=>s.encoded_name.toLowerCase().includes(a)||s.real_name.toLowerCase().includes(a)))}),document.getElementById("add-club-btn").addEventListener("click",()=>{de(null,{toast:t,openModal:n,closeModal:o,reload:()=>X(e,{toast:t,openModal:n,closeModal:o})})});function u(r){document.getElementById("clubs-tbody").innerHTML=r.map(a=>{var d;const s=_e(a),c=xe(s).replace("<svg ",'<svg style="width:36px;height:42px" ');return`
      <tr>
        <td>
  ...
  `}async function Ee(e,{toast:t}){const{data:n,error:o}=await x.from("users").select("id,pseudo,club_name,credits,level,wins,draws,losses,trophies_top1,trophies_top2,trophies_top3,is_admin,created_at").order("created_at",{ascending:!1});if(o){e.innerHTML=`<p style="color:red">${o.message}</p>`;return}e.innerHTML=`
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px">
  ...
  `;const u=n||[];r(u),document.getElementById("search-users").addEventListener("input",a=>{const s=a.target.value.toLowerCase();r(u.filter(c=>{var d;return c.pseudo.toLowerCase().includes(s)||((d=c.club_name)==null?void 0:d.toLowerCase().includes(s))}))});function r(a){document.getElementById("users-tbody").innerHTML=a.map(s=>`
      <tr>
        <td><b>${s.pseudo}</b></td>
    ...
    `).join(""),document.querySelectorAll("[data-toggle-admin]").forEach(s=>{s.addEventListener("click",async()=>{const c=s.dataset.isAdmin!=="true";if(!confirm(`Voulez-vous ${c?"rendre admin":"retirer le rôle admin"} de ce Manager ?`))return;const{error:b}=await x.from("users").update({is_admin:c}).eq("id",s.dataset.toggleAdmin);b?t(b.message,"error"):(t("Rôle mis à jour ✅","success"),Ee(e,{toast:t}))})})}}async function we(e,{toast:t}){const{data:n,error:o}=await x.from("market_listings").select(`
    ...
    `).order("listed_at",{ascending:!1}).limit(100);if(o){e.innerHTML=`<p style="color:red">${o.message}</p>`;return}const u=n||[],r=u.filter(s=>s.status==="active").length,a=u.filter(s=>s.status==="sold").length;e.innerHTML=`
    ...
    <div class="card-panel">
      <div class="table-wrap">
        <table>
          ...
          <tbody>
            ${u.map(s=>{var l,i,f,m;const c=(l=s.card)==null?void 0:l.player,d=c?`${c.firstname} ${c.surname_encoded}`:((i=s.card)==null?void 0:i.card_type)||"—",b={active:"#1A6B3C",sold:"#D4A017",cancelled:"#aaa"};return`<tr>
                ...
                <td style="font-size:12px">${((m=s.buyer)==null?void 0:m.pseudo)||"—"}</td>
  ...
  `,document.querySelectorAll("[data-cancel]").forEach(s=>{s.addEventListener("click",async()=>{if(!confirm("Annuler cette annonce ?"))return;const{error:c}=await x.from("market_listings").update({status:"cancelled"}).eq("id",s.dataset.cancel);c?t(c.message,"error"):(t("Annonce annulée","success"),we(e,{toast:t}))})})}async function We(e,{toast:t}){e.innerHTML=`
...
`)}),document.getElementById("export-clubs").addEventListener("click",async()=>{const{data:n,error:o}=await x.from("clubs").select("real_name,encoded_name,country_code,logo_url").order("encoded_name");if(o){t(o.message,"error");return}if(!n||n.length===0){t("Aucun club à exporter","info");return}let u=`real_name,encoded_name,country_code,logo_url
`;n.forEach(r=>{u+=[r.real_name,r.encoded_name,r.country_code,r.logo_url||""].map(ue).join(",")+`
`}),Y("clubs_export.csv",u),t(`${n.length} clubs exportés`,"success")}),document.getElementById("export-players").addEventListener("click",async()=>{const{data:n,error:o}=await x.from("players").select("firstname,surname_real,surname_encoded,country_code,job,job2,note_g,note_d,note_m,note_a,rarity,note_min,note_max,skin,hair,hair_length,sell_price,clubs(encoded_name)").order("surname_encoded");if(o){t(o.message,"error");return}if(!n||n.length===0){t("Aucun joueur à exporter","info");return}let u=`firstname,surname_real,surname_encoded,country_code,club_encoded_name,job,job2,note_g,note_d,note_m,note_a,rarity,note_min,note_max,skin,hair,hair_length,sell_price
`;n.forEach(r=>{var a;u+=[r.firstname,r.surname_real,r.surname_encoded,r.country_code,((a=r.clubs)==null?void 0:a.encoded_name)||"",r.job,r.job2||"",r.note_g,r.note_d,r.note_m,r.note_a,r.rarity,r.note_min??"",r.note_max??"",r.skin,r.hair,r.hair_length,r.sell_price].map(ue).join(",")+`
`}),Y("players_export.csv",u),t(`${n.length} joueurs exportés`,"success")}),document.getElementById("import-clubs").addEventListener("change",async n=>{const o=n.target.files[0];if(!o)return;const u=document.getElementById("clubs-import-result");u.innerHTML='<span style="color:var(--gray-600)">Import en cours...</span>';try{const r=await o.text(),a=me(r);if(a.length===0){u.innerHTML='<span style="color:#c0392b">Fichier vide</span>';return}let s=0,c=0;const d=[];for(const b of a){if(!b.real_name||!b.encoded_name||!b.country_code){c++,d.push(`Ligne ignorée (champs manquants): ${b.real_name||"?"}`);continue}const l={real_name:b.real_name,encoded_name:b.encoded_name.toUpperCase(),country_code:b.country_code.toUpperCase().slice(0,2),logo_url:b.logo_url||null},{error:i}=await x.from("clubs").upsert(l,{onConflict:"encoded_name"});i?(c++,d.push(`${b.encoded_name}: ${i.message}`)):s++}u.innerHTML=`<div style="color:var(--green)">✅ ${s} clubs importés</div>
        ${c>0?`<div style="color:#c0392b">❌ ${c} erreurs</div><div style="font-size:11px;color:var(--gray-600);max-height:100px;overflow-y:auto">${d.slice(0,10).join("<br>")}</div>`:""}`,t(`${s} clubs importés`,"success")}catch(r){u.innerHTML=`<span style="color:#c0392b">Erreur : ${r.message}</span>`}n.target.value=""}),document.getElementById("import-players").addEventListener("change",async n=>{const o=n.target.files[0];if(!o)return;const u=document.getElementById("players-import-result");u.innerHTML='<span style="color:var(--gray-600)">Import en cours...</span>';try{const r=await o.text(),a=me(r);if(a.length===0){u.innerHTML='<span style="color:#c0392b">Fichier vide</span>';return}const{data:s}=await x.from("clubs").select("id,encoded_name"),c={};(s||[]).forEach(p=>{c[p.encoded_name.toUpperCase()]=p.id});let d=0,b=0;const l=[],i=["GK","DEF","MIL","ATT"],f=["normal","pepite","papyte","legende"],m=["blanc","metisse","typ","noir"],y=["blond","marron","noir","chauve"],_=["rase","court","milong","long"];for(const p of a){if(!p.firstname||!p.surname_real||!p.country_code||!p.job){b++,l.push(`Ligne ignorée (champs requis manquants): ${p.firstname||"?"}`);continue}if(!i.includes(p.job)){b++,l.push(`${p.firstname}: job invalide "${p.job}"`);continue}const E=p.club_encoded_name&&c[p.club_encoded_name.toUpperCase()]||null,$={firstname:p.firstname,surname_real:p.surname_real,surname_encoded:p.surname_encoded||K(p.surname_real),country_code:p.country_code.toUpperCase().slice(0,2),club_id:E,job:p.job,job2:i.includes(p.job2)?p.job2:null,note_g:parseInt(p.note_g)||0,note_d:parseInt(p.note_d)||0,note_m:parseInt(p.note_m)||0,note_a:parseInt(p.note_a)||0,rarity:f.includes(p.rarity)?p.rarity:"normal",note_min:p.note_min!==""&&p.note_min!=null?parseInt(p.note_min):null,note_max:p.note_max!==""&&p.note_max!=null?parseInt(p.note_max):null,skin:m.includes(p.skin)?p.skin:"blanc",hair:y.includes(p.hair)?p.hair:"noir",hair_length:_.includes(p.hair_length)?p.hair_length:"court",sell_price:parseInt(p.sell_price)||0},{error:g}=await x.from("players").insert($);g?(b++,l.push(`${p.firstname} ${p.surname_real}: ${g.message}`)):d++}u.innerHTML=`<div style="color:var(--green)">✅ ${d} joueurs importés</div>
        ${b>0?`<div style="color:#c0392b">❌ ${b} erreurs</div><div style="font-size:11px;color:var(--gray-600);max-height:100px;overflow-y:auto">${l.slice(0,10).join("<br>")}</div>`:""}`,t(`${d} joueurs importés`,"success")}catch(r){u.innerHTML=`<span style="color:#c0392b">Erreur : ${r.message}</span>`}n.target.value=""})}function ue(e){const t=String(e??"");return t.includes(",")||t.includes('"')||t.includes(`
`)?'"'+t.replace(/"/g,'""')+'"':t}function Y(e,t){const n=new Blob(["\uFEFF"+t],{type:"text/csv;charset=utf-8;"}),o=URL.createObjectURL(n),u=document.createElement("a");u.href=o,u.download=e,u.click(),URL.revokeObjectURL(o)}function me(e){e=e.replace(/^\uFEFF/,"").replace(/\r\n/g,`
`).replace(/\r/g,`
`);const t=e.split(`
`).filter(a=>a.trim());if(t.length<2)return[];const n=t[0],o=n.split(";").length>n.split(",").length?";":",",u=ge(n,o).map(a=>a.trim().replace(/^\uFEFF/,"").toLowerCase()),r=[];for(let a=1;a<t.length;a++){if(!t[a].trim())continue;const s=ge(t[a],o),c={};u.forEach((d,b)=>{c[d]=(s[b]||"").trim()}),!Object.values(c).every(d=>!d)&&r.push(c)}return r}function ge(e,t=","){const n=[];let o="",u=!1;for(let r=0;r<e.length;r++){const a=e[r];u?a==='"'?e[r+1]==='"'?(o+='"',r++):u=!1:o+=a:a==='"'?u=!0:a===t?(n.push(o),o=""):o+=a}return n.push(o),n}const Xe={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};async function Qe(e,{toast:t}){const n=Object.keys(ee);let o=n[0];const{data:u}=await x.from("formation_links_overrides").select("formation, links"),r={};(u||[]).forEach(b=>{r[b.formation]=b.links});let a=new Set;function s(b,l){return[b,l].sort().join("-")}function c(b){const l=r[b]||ee[b]||[];a=new Set(l.map(([i,f])=>s(i,f)))}c(o),e.innerHTML=`
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;gap:12px;flex-wrap:wrap">
  ...
  `;function d(){const b=Me[o]||{},l=re(o),i=320,f=400,m=22;function y(g){const v=b[g];return v?{x:v.x*i,y:v.y*f}:null}let _=`<svg width="${i}" height="${f}" viewBox="0 0 ${i} ${f}" xmlns="http://www.w3.org/2000/svg" style="display:block">`;l.forEach(([g,v],w)=>{const h=y(g),I=y(v);if(!h||!I)return;const z=s(g,v),k=a.has(z),S=k?"#3b82f6":"#999",R=k?.95:.35,M=k?4:3;_+=`<line x1="${h.x}" y1="${h.y}" x2="${I.x}" y2="${I.y}"
        ...
        opacity="${R}" pointer-events="none" data-line-key="${z}"/>`});for(const g of Object.keys(b)){const v=y(g);if(!v)continue;const w=g.replace(/\d+/,""),h=Xe[w]||"#555";_+=`<circle cx="${v.x}" cy="${v.y}" r="${m}" fill="${h}" stroke="rgba(255,255,255,0.7)" stroke-width="2" pointer-events="none"/>`,_+=`<text x="${v.x}" y="${v.y+4}" text-anchor="middle" font-size="10" font-weight="900" fill="white" font-family="Arial Black,Arial" pointer-events="none">${g}</text>`}_+="</svg>",document.getElementById("field-wrap").innerHTML=_;const p=document.getElementById("links-list");p.innerHTML=l.map(([g,v])=>{const w=s(g,v),h=a.has(w);return`
        ...
        </button>`}).join("");const E=e.querySelector('div[style*="font-weight:700;font-size:13px;margin-bottom:8px"]');E&&(E.textContent=`Liens (${a.size}/${l.length} actifs)`);function $(g){a.has(g)?a.delete(g):a.add(g),d()}e.querySelectorAll(".link-hit").forEach(g=>{g.addEventListener("click",()=>$(g.dataset.key))}),e.querySelectorAll(".link-toggle").forEach(g=>{g.addEventListener("click",()=>$(g.dataset.key))})}d(),document.getElementById("formation-select").addEventListener("change",b=>{o=b.target.value,c(o),d()}),document.getElementById("reset-btn").addEventListener("click",()=>{const b=ee[o]||[];a=new Set(b.map(([l,i])=>s(l,i))),d(),t("Liens réinitialisés aux valeurs par défaut (non sauvegardé)","info")}),document.getElementById("save-btn").addEventListener("click",async()=>{const l=re(o).filter(([f,m])=>a.has(s(f,m))),{error:i}=await x.from("formation_links_overrides").upsert({formation:o,links:l,updated_at:new Date().toISOString()});if(i){t(i.message,"error");return}r[o]=l,t(`Liens enregistrés pour ${o} (${l.length} actifs)`,"success")})}const Ze=[{value:"player",label:"Joueur"},{value:"formation",label:"Formation"},{value:"game_changer",label:"Game Changer"},{value:"ultra_game_changer",label:"Ultra Game Changer"},{value:"game_helper",label:"Game Helper"}],et=[{value:"",label:"Toutes"},{value:"normal",label:"Normal"},{value:"pepite",label:"Pépite"},{value:"papyte",label:"Papyte"},{value:"legende",label:"Légende"}],tt=[{value:"credits",label:"Crédits"},{value:"pub",label:"Publicité"},{value:"free",label:"Gratuit"}];async function nt(e){e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">Chargement...</div>';const{data:t}=await x.from("booster_configs").select("*").order("sort_order");let n=null,o=[],u=null;const r=()=>window.innerWidth<700;async function a(){if(!n){o=[];return}if(n!==u){const{data:l}=await x.from("booster_drop_rates").select("*").eq("booster_id",n).order("sort_order");o=l||[],u=n}}function s(){return`
    ...
    </div>`}function c(){const l=n?(t||[]).find(m=>m.id===n):null;if(!l)return`<div style="display:flex;align-items:center;justify-content:center;flex:1;color:#bbb;font-size:14px;padding:40px;text-align:center">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
          <div>
            ...
            <select id="f-price-type" style="width:100%;padding:9px;border:1px solid #ddd;border-radius:6px;font-size:13px">
              ${tt.map(m=>`<option value="${m.value}" ${l.price_type===m.value?"selected":""}>${m.label}</option>`).join("")}
            </select>
          </div>
        ...
        ${o.length===0?'<div style="text-align:center;padding:16px;color:#aaa;font-size:13px">Aucun taux. Cliquez + Ligne.</div>':o.map((m,y)=>`
          <div style="display:grid;grid-template-columns:2fr 1.5fr 50px 50px 60px 32px;gap:4px;align-items:center;margin-bottom:6px;padding:6px;background:#f9f9f9;border-radius:6px" data-rate-idx="${y}">
            <select class="rate-type" data-idx="${y}" style="padding:6px 4px;border:1px solid #ddd;border-radius:4px;font-size:12px;width:100%">
              ${Ze.map(_=>`<option value="${_.value}" ${m.card_type===_.value?"selected":""}>${_.label}</option>`).join("")}
            </select>
            <select class="rate-rarity" data-idx="${y}" style="padding:6px 4px;border:1px solid #ddd;border-radius:4px;font-size:12px;width:100%">
              ${et.map(_=>`<option value="${_.value}" ${(m.rarity||"")===_.value?"selected":""}>${_.label}</option>`).join("")}
            </select>
    ...
    <div style="display:flex;height:calc(100vh - 56px);overflow:hidden">
      ${!r()||i||!n?s():""}
      ${!r()||f?c():""}
    </div>`,b()}function b(){var f,m,y,_,p,E,$;const l=g=>e.querySelector(g);e.querySelectorAll(".booster-row").forEach(g=>{g.addEventListener("click",v=>{v.target.closest(".btn-delete")||(n=g.dataset.id,u=null,d())})}),(f=l("#btn-back"))==null||f.addEventListener("click",()=>{n=null,d()}),(m=l("#btn-new"))==null||m.addEventListener("click",async()=>{const g=prompt("Nom du nouveau booster :");if(!(g!=null&&g.trim()))return;const{data:v,error:w}=await x.from("booster_configs").insert({name:g.trim(),price_type:"credits",price_credits:1e3,card_count:5}).select().single();if(w){alert(w.message);return}t.push(v),n=v.id,u=null,d()}),e.querySelectorAll(".btn-delete").forEach(g=>{g.addEventListener("click",async v=>{if(v.stopPropagation(),!confirm("Supprimer ce booster ?"))return;await x.from("booster_configs").delete().eq("id",g.dataset.id);const w=t.findIndex(h=>h.id===g.dataset.id);w>-1&&t.splice(w,1),n===g.dataset.id&&(n=null,u=null),d()})}),(y=l("#btn-cancel"))==null||y.addEventListener("click",()=>{n=null,d()}),(_=l("#f-price-type"))==null||_.addEventListener("change",g=>{const v=l("#credits-field");v&&(v.style.opacity=g.target.value!=="credits"?"0.4":"1")}),(p=l("#btn-pick-icon"))==null||p.addEventListener("click",async()=>{var v;const g=l("#icon-picker-grid");if(g){if(g.style.display!=="none"){g.style.display="none";return}g.innerHTML='<div style="padding:10px;text-align:center;color:#888;font-size:12px">⏳ Chargement...</div>',g.style.display="block";try{const h=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),I=Array.isArray(h)?h.filter(k=>k.name.startsWith("booster")&&/\.(png|jpg|svg|webp)$/i.test(k.name)):[];if(!I.length){g.innerHTML='<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Aucune icône "booster*" trouvée.</div>';return}const z=((v=l("#f-image-url"))==null?void 0:v.value)||"";g.innerHTML=`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(72px,1fr));gap:8px">
          ${I.map(k=>`
        ...
        </div>`,g.querySelectorAll(".icon-pick-item").forEach(k=>{k.addEventListener("click",()=>{const S=l("#f-image-url");S&&(S.value=k.dataset.name),g.style.display="none"})})}catch(w){g.innerHTML=`<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Erreur API GitHub : ${w.message}</div>`}}});function i(){e.querySelectorAll("[data-rate-idx]").forEach(g=>{var w,h,I,z,k;const v=Number(g.dataset.rateIdx);v>=0&&v<o.length&&(o[v].card_type=((w=g.querySelector(".rate-type"))==null?void 0:w.value)||"player",o[v].rarity=((h=g.querySelector(".rate-rarity"))==null?void 0:h.value)||null,o[v].note_min=Number((I=g.querySelector(".rate-note-min"))==null?void 0:I.value)||null,o[v].note_max=Number((z=g.querySelector(".rate-note-max"))==null?void 0:z.value)||null,o[v].percentage=Number((k=g.querySelector(".rate-pct"))==null?void 0:k.value)||0)})}(E=l("#btn-add-rate"))==null||E.addEventListener("click",()=>{i(),o.push({id:null,booster_id:n,card_type:"player",rarity:"normal",note_min:null,note_max:null,percentage:10,sort_order:o.length}),d(!0)}),e.querySelectorAll(".btn-del-rate").forEach(g=>{g.addEventListener("click",()=>{i(),o.splice(Number(g.dataset.idx),1),d(!0)})}),($=l("#btn-save"))==null||$.addEventListener("click",async()=>{var z,k,S,R,M,F,D,H,U;const g=(t||[]).find(L=>L.id===n);if(!g)return;const v=[];e.querySelectorAll("[data-rate-idx]").forEach(L=>{var T,G,ie,oe,ae;const q=Number(L.dataset.rateIdx);v.push({booster_id:n,card_type:((T=L.querySelector(".rate-type"))==null?void 0:T.value)||"player",rarity:((G=L.querySelector(".rate-rarity"))==null?void 0:G.value)||null,note_min:Number((ie=L.querySelector(".rate-note-min"))==null?void 0:ie.value)||null,note_max:Number((oe=L.querySelector(".rate-note-max"))==null?void 0:oe.value)||null,percentage:Number((ae=L.querySelector(".rate-pct"))==null?void 0:ae.value)||0,sort_order:q});const C=v[v.length-1];C.rarity||(C.rarity=null),C.note_min||(C.note_min=null),C.note_max||(C.note_max=null)});const w=v.reduce((L,q)=>L+q.percentage,0);if(v.length&&Math.abs(w-100)>.5){alert(`La somme doit faire 100% (actuellement ${w.toFixed(1)}%)`);return}const h={name:((k=(z=l("#f-name"))==null?void 0:z.value)==null?void 0:k.trim())||g.name,image_url:((R=(S=l("#f-image-url"))==null?void 0:S.value)==null?void 0:R.trim())||null,price_type:(M=l("#f-price-type"))==null?void 0:M.value,price_credits:Number((F=l("#f-price-credits"))==null?void 0:F.value)||0,card_count:Number((D=l("#f-card-count"))==null?void 0:D.value)||5,is_active:((H=l("#f-active"))==null?void 0:H.checked)??g.is_active,sort_order:Number((U=l("#f-sort"))==null?void 0:U.value)||0},{error:I}=await x.from("booster_configs").update(h).eq("id",n);if(I){alert(I.message);return}if(Object.assign(g,h),await x.from("booster_drop_rates").delete().eq("booster_id",n),v.length){const{error:L}=await x.from("booster_drop_rates").insert(v);if(L){alert(L.message);return}}o=v,u=n,alert("✅ Booster enregistré !"),d(!0)})}d()}const ke=[{value:"normal",label:"Normal",color:"#888"},{value:"pepite",label:"Pépite",color:"#D4A017"},{value:"papyte",label:"Papyte",color:"#909090"},{value:"legende",label:"Légende",color:"#7a28b8"}];async function it(e){await ne(e)}async function ne(e){const{data:t,error:n}=await x.from("sell_price_configs").select("*").order("rarity").order("note_min");if(n){e.innerHTML=`<div style="padding:24px;color:red">Erreur : ${n.message}</div>`;return}e.innerHTML=`
    ...
    </div>`,document.getElementById("add-config").addEventListener("click",()=>be(null)),document.getElementById("form-cancel").addEventListener("click",()=>{document.getElementById("config-form").style.display="none"}),document.getElementById("form-save").addEventListener("click",()=>at(e)),e.querySelectorAll("[data-edit]").forEach(o=>{const u=(t||[]).find(r=>r.id===o.dataset.edit);u&&o.addEventListener("click",()=>be(u))}),e.querySelectorAll("[data-delete]").forEach(o=>{o.addEventListener("click",async()=>{confirm("Supprimer cette règle ?")&&(await x.from("sell_price_configs").delete().eq("id",o.dataset.delete),await ne(e))})})}function ot(e){const t=ke.find(n=>n.value===e.rarity);return`
    <tr style="border-top:1px solid #f0f0f0">
      ...
      <div style="height:${E}px;background:rgba(0,0,0,0.35);display:flex;align-items:center;justify-content:center;padding:4px 6px">
    ...
    <div id="gc-list" style="background:#fff;${r()?"":"border-right:1px solid #e0e0e0;width:280px;flex-shrink:0;"}overflow-y:auto;height:100%">
      ...
      ${(t||[]).map(i=>{const f=N.find(y=>y.value===i.gc_type)||N[0],m=P.find(y=>y.value===i.color)||P[0];return`
    ...
    </div>`}function d(){const i=n?(t||[]).find(m=>m.id===n):null;if(!i)return`<div style="flex:1;display:flex;align-items:center;justify-content:center;color:#bbb;font-size:14px;padding:40px;text-align:center">
    ...
    <div style="display:flex;height:calc(100vh - 56px);overflow:hidden">
      ${!r()||!n?c():""}
      ${!r()||n?d():""}
    </div>`,l()}function l(){var i,f,m,y,_;e.querySelectorAll(".gc-row").forEach(p=>{p.addEventListener("click",E=>{E.target.closest(".btn-toggle-gc,.btn-delete-gc")||(n=p.dataset.id,b())})}),(i=a("#btn-gc-back"))==null||i.addEventListener("click",()=>{n=null,b()}),(f=a("#btn-cancel-gc"))==null||f.addEventListener("click",()=>{n=null,b()}),(m=a("#btn-new-gc"))==null||m.addEventListener("click",async()=>{const p=prompt("Nom de la carte Game Changer :");if(!(p!=null&&p.trim()))return;const{data:E,error:$}=await x.from("gc_definitions").insert({name:p.trim(),gc_type:"game_changer",color:"purple"}).select().single();if($){alert($.message);return}t.push(E),n=E.id,b()}),e.querySelectorAll(".btn-toggle-gc").forEach(p=>{p.addEventListener("click",async E=>{E.stopPropagation();const $=p.dataset.active!=="true";await x.from("gc_definitions").update({is_active:$}).eq("id",p.dataset.id);const g=t.find(v=>v.id===p.dataset.id);g&&(g.is_active=$),b()})}),e.querySelectorAll(".btn-delete-gc").forEach(p=>{p.addEventListener("click",async E=>{if(E.stopPropagation(),!confirm("Supprimer cette carte Game Changer ?"))return;await x.from("gc_definitions").delete().eq("id",p.dataset.id);const $=t.findIndex(g=>g.id===p.dataset.id);$>-1&&t.splice($,1),n===p.dataset.id&&(n=null),b()})}),(y=a("#btn-pick-gc-icon"))==null||y.addEventListener("click",async()=>{var E;const p=a("#gc-icon-picker");if(p){if(p.style.display!=="none"){p.style.display="none";return}p.innerHTML='<div style="padding:10px;text-align:center;color:#888;font-size:12px">⏳ Chargement...</div>',p.style.display="block";try{const g=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),v=Array.isArray(g)?g.filter(h=>h.name.startsWith("gamechanger-")&&/\.(png|jpg|svg|webp)$/i.test(h.name)):[];if(!v.length){p.innerHTML='<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Aucun fichier "gamechanger-*" trouvé dans public/icons/</div>';return}const w=((E=a("#gc-image-url"))==null?void 0:E.value)||"";p.innerHTML=`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(72px,1fr));gap:8px">
          ${v.map(h=>`
        ...
        </div>`,p.querySelectorAll(".gc-icon-item").forEach(h=>{h.addEventListener("click",()=>{const I=a("#gc-image-url");I&&(I.value=h.dataset.name),p.style.display="none"})})}catch($){p.innerHTML=`<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Erreur API GitHub : ${$.message}</div>`}}}),(_=a("#btn-save-gc"))==null||_.addEventListener("click",async()=>{var h,I,z,k,S,R,M,F,D,H,U,L,q,C;const p=t.find(T=>T.id===n);if(!p)return;const E=o.find(T=>{var G;return T.value===(((G=a("#gc-effect-type"))==null?void 0:G.value)||"BOOST_STAT")})||o[0],$=E.hasRoles?[...e.querySelectorAll(".gc-p-role:checked")].map(T=>T.value):null,g={...E.hasValue?{value:Number((h=a("#gc-p-value"))==null?void 0:h.value)||2}:{},...E.hasCount?{count:Number((I=a("#gc-p-count"))==null?void 0:I.value)||1}:{},...E.hasTarget?{target:((z=a("#gc-p-target"))==null?void 0:z.value)||"home"}:{},...E.hasRoles?{roles:$!=null&&$.length?$:null}:{}},v={name:((S=(k=a("#gc-name"))==null?void 0:k.value)==null?void 0:S.trim())||p.name,effect:((M=(R=a("#gc-effect"))==null?void 0:R.value)==null?void 0:M.trim())||null,image_url:((D=(F=a("#gc-image-url"))==null?void 0:F.value)==null?void 0:D.trim())||null,gc_type:((H=a("#gc-type"))==null?void 0:H.value)||"game_changer",color:((U=a("#gc-color"))==null?void 0:U.value)||"purple",sort_order:Number((L=a("#gc-sort"))==null?void 0:L.value)||0,is_active:((q=a("#gc-active"))==null?void 0:q.checked)??p.is_active,effect_type:((C=a("#gc-effect-type"))==null?void 0:C.value)||"BOOST_STAT",effect_params:g},{error:w}=await x.from("gc_definitions").update(v).eq("id",n);if(w){alert(w.message);return}Object.assign(p,v),alert("✅ Carte enregistrée !"),b()})}b()}Ne(he);function lt(e,t="info"){const n=document.getElementById("toast");n.textContent=e,n.className=`show ${t}`,clearTimeout(n._t),n._t=setTimeout(()=>{n.className=""},3e3)}function st(e,t,n=""){document.getElementById("modal-title").textContent=e,document.getElementById("modal-body").innerHTML=t,document.getElementById("modal-footer").innerHTML=n,document.getElementById("modal-overlay").classList.remove("hidden")}function te(){document.getElementById("modal-overlay").classList.add("hidden")}const fe={dashboard:{title:"Dashboard",fn:Pe},players:{title:"Joueurs & Cartes",fn:qe},clubs:{title:"Clubs",fn:Fe},"card-builder":{title:"Card Builder",fn:Ge},users:{title:"Managers",fn:Ee},market:{title:"Marché des transferts",fn:we},"import-export":{title:"Import / Export CSV",fn:We},formations:{title:"Formations & Liens",fn:Qe},"boosters-config":{title:"Boosters",fn:nt},"sell-price":{title:"Prix vente directe",fn:it},"gc-cards":{title:"Game Changers",fn:rt}};async function Z(e){document.querySelectorAll(".admin-sidebar nav a").forEach(u=>{u.classList.toggle("active",u.dataset.page===e)});const t=document.getElementById("mobile-page-select");t&&(t.value=e);const n=fe[e]||fe.dashboard;document.getElementById("page-title").textContent=n.title;const o=document.getElementById("page-content");o.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">Chargement…</div>';try{await n.fn(o,{toast:lt,openModal:st,closeModal:te,navigate:Z})}catch(u){o.innerHTML=`<div style="padding:40px;color:#c0392b">Erreur: ${u.message}</div>`,console.error(u)}}async function dt(){var t;const{data:{session:e}}=await x.auth.getSession();e&&await ve(e.user),document.getElementById("auth-btn").addEventListener("click",async()=>{const n=document.getElementById("auth-email").value.trim(),o=document.getElementById("auth-password").value,u=document.getElementById("auth-error");if(u.textContent="",!n||!o){u.textContent="Remplissez tous les champs.";return}const{data:r,error:a}=await x.auth.signInWithPassword({email:n,password:o});if(a){u.textContent=a.message;return}await ve(r.user)}),document.getElementById("auth-password").addEventListener("keydown",n=>{n.key==="Enter"&&document.getElementById("auth-btn").click()}),document.getElementById("logout-btn").addEventListener("click",async()=>{await x.auth.signOut(),document.getElementById("auth-screen").style.display="flex",document.getElementById("admin-app").style.display="none"}),document.getElementById("modal-close").addEventListener("click",te),document.getElementById("modal-overlay").addEventListener("click",n=>{n.target===n.currentTarget&&te()}),document.querySelectorAll(".admin-sidebar nav a").forEach(n=>{n.addEventListener("click",o=>{o.preventDefault(),Z(n.dataset.page)})}),(t=document.getElementById("mobile-page-select"))==null||t.addEventListener("change",n=>{Z(n.target.value)})}async function ve(e){const{data:t,error:n}=await x.from("users").select("*").eq("id",e.id).single(),o=document.getElementById("auth-error");if(n||!t){o.textContent="Profil introuvable.";return}if(!t.is_admin){o.textContent="Accès refusé. Vous n'êtes pas administrateur.",await x.auth.signOut();return}document.getElementById("auth-screen").style.display="none",document.getElementById("admin-app").style.display="flex",document.getElementById("admin-user").textContent=`Admin: ${t.pseudo}`,Z("dashboard")}dt();
