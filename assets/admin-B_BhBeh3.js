import{s as y,e as H,g as Me,K as Ve,D as S,S as Ke,a as A,H as Je,R as Ye,b as je,r as We,c as Ze,E as Xe,d as Qe,N as et,M as tt,B as nt,f as it,F as le,h as ve,i as ot,j as at}from"./formation-links-5A-YoKwp.js";async function rt(e){const[{count:n},{count:t},{count:i},{count:p},{count:s}]=await Promise.all([y.from("players").select("*",{count:"exact",head:!0}),y.from("clubs").select("*",{count:"exact",head:!0}),y.from("users").select("*",{count:"exact",head:!0}),y.from("cards").select("*",{count:"exact",head:!0}),y.from("matches").select("*",{count:"exact",head:!0})]);e.innerHTML=`
  ...
  `,c(s);function c(r){document.getElementById("count-label").textContent=`${r.length} joueur(s)`,document.getElementById("players-tbody").innerHTML=r.map(l=>{var h;return`
    ...
    <div id="clubs-list" style="display:flex;flex-direction:column;gap:8px"></div>`,s(ne),document.getElementById("search-clubs").addEventListener("input",d=>{const a=d.target.value.toLowerCase();s(ne.filter(o=>o.real_name.toLowerCase().includes(a)||o.encoded_name.toLowerCase().includes(a)))}),document.getElementById("add-club-btn").addEventListener("click",()=>Ee(null,e,n));function s(d){const a=document.getElementById("clubs-list");if(!d.length){a.innerHTML='<p style="color:var(--gray-600);padding:20px">Aucun club.</p>';return}a.innerHTML=d.map(o=>{const c=De(o),u=Me(c).replace("<svg ",'<svg style="width:40px;height:48px" ');return`
        ...
        <div style="display:grid;grid-template-columns:1fr 120px;gap:16px;align-items:start">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
            ...
            <div>
              ...
              <select id="av-hair">${d("av-hair",Ze,A.hair)}</select>
            </div>
            <div>
            ...
            <div>
              ...
              <select id="av-eyebrows">${d("av-eyebrows",Xe,A.eyebrows)}</select>
            </div>
            <div>
              <label>Yeux</label>
              <select id="av-eyes">${d("av-eyes",Qe,A.eyes)}</select>
            </div>
            <div>
              <label>Nez</label>
              <select id="av-nose">${d("av-nose",et,A.nose)}</select>
            </div>
            <div>
              <label>Bouche</label>
              <select id="av-mouth">${d("av-mouth",tt,A.mouth)}</select>
            </div>
            <div>
              <label>Barbe</label>
              <select id="av-beard">${d("av-beard",nt,A.beard)}</select>
            </div>
          </div>
