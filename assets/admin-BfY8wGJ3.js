import{s as E,r as ve,g as Fe,K as ke,D as M,S as rt,a as q,H as at,R as st,e as be,b as He,c as lt,d as dt,E as ct,f as pt,N as ut,M as mt,B as gt,h as ft,F as le,i as Ie,j as W,k as bt,l as xt,m as vt,n as yt,o as ht,p as wt}from"./svg-config-DC1JcKBi.js";async function Et(e){const[{count:t},{count:n},{count:i},{count:u},{count:d}]=await Promise.all([E.from("players").select("*",{count:"exact",head:!0}),E.from("clubs").select("*",{count:"exact",head:!0}),E.from("users").select("*",{count:"exact",head:!0}),E.from("cards").select("*",{count:"exact",head:!0}),E.from("matches").select("*",{count:"exact",head:!0})]);e.innerHTML=`
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:12px;margin-bottom:24px">
      ${oe("🃏","Joueurs (cartes)",t??"–","#1A6B3C")}
      ${oe("🏟️","Clubs",n??"–","#D4A017")}
      ${oe("👥","Managers",i??"–","#7a28b8")}
      ${oe("📦","Cartes possédées",u??"–","#2a8f52")}
      ${oe("⚽","Matchs joués",d??"–","#bb2020")}
    </div>
  ...
  `,window.adminNav=c=>{var s;(s=document.querySelector(`[data-page="${c}"]`))==null||s.click()}}function oe(e,t,n,i){return`<div class="card-panel" style="text-align:center">
    ...
    <div style="display:flex;flex-direction:column;gap:10px">
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        ...
        <select id="filter-rarity" style="width:110px">
          ...
          ${Object.entries(Ue).map(([r,o])=>`<option value="${r}">${o}</option>`).join("")}
        </select>
      ...
      <div style="flex:1;min-width:300px;display:flex;flex-direction:column;gap:12px">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:8px">
          ...
          <div class="form-group">
            <label>Rareté</label>
            <select id="pm-rarity">
              ${["normal","pepite","papyte","legende"].map(o=>`<option value="${o}" ${(e==null?void 0:e.rarity)===o?"selected":""}>${Ue[o]}</option>`).join("")}
            </select>
          </div>
          <div class="form-group">
            <label>Pays</label>
    ...
    <div id="clubs-list" style="display:flex;flex-direction:column;gap:8px"></div>`,c(ce),document.getElementById("search-clubs").addEventListener("input",s=>{const m=s.target.value.toLowerCase();c(ce.filter(a=>a.real_name.toLowerCase().includes(m)||a.encoded_name.toLowerCase().includes(m)))}),document.getElementById("add-club-btn").addEventListener("click",()=>Se(null,e,t));function c(s){const m=document.getElementById("clubs-list");if(!s.length){m.innerHTML='<p style="color:var(--gray-600);padding:20px">Aucun club.</p>';return}m.innerHTML=s.map(a=>{const b=Je(a),l=Fe(b,a.id).replace("<svg ",'<svg style="width:40px;height:48px" '),r=a.logo_url?`<img src="${a.logo_url}" style="width:40px;height:40px;object-fit:contain;border-radius:8px">`:`<div style="width:40px;height:40px;background:linear-gradient(135deg,${b.color1},${b.color2});border-radius:8px;display:flex;align-items:center;justify-content:center;color:#fff;font-size:10px;font-weight:900">${a.encoded_name.slice(0,3)}</div>`,o=n[a.id]||0;return`
        ...
        </div>`}).join(""),m.querySelectorAll("[data-gen]").forEach(a=>{a.addEventListener("click",async()=>{confirm(`Générer 20 joueurs pour ${a.dataset.name} ?`)&&(a.disabled=!0,a.textContent="⏳",await Ye(a.dataset.gen,a.dataset.cc,i),ge(e,t))})}),m.querySelectorAll("[data-edit]").forEach(a=>{const b=ce.find(l=>l.id===a.dataset.edit);a.addEventListener("click",()=>Se(b,e,t))}),m.querySelectorAll("[data-del]").forEach(a=>{a.addEventListener("click",async()=>{if(!confirm("Supprimer ce club ?"))return;const{error:b}=await E.from("clubs").delete().eq("id",a.dataset.del);b?i(b.message,"error"):(i("Club supprimé","success"),ge(e,t))})})}}function Se(e,t,n){const{toast:i,openModal:u,closeModal:d}=n,c=!!e,s=e?Je(e):{...M},m=Object.entries(ke).map(([b,l])=>`<option value="${b}" ${s.style===b?"selected":""}>${l.label}</option>`).join(""),a=Ve.map(b=>`<option value="${b}" ${((e==null?void 0:e.country_code)||"FR")===b?"selected":""}>${b}</option>`).join("");u(c?`✏️ ${e.real_name}`:"➕ Nouveau club",`<div style="display:flex;flex-direction:column;gap:14px">
    ...
    </div>`),setTimeout(()=>{var p,h,w;ae();function b(){var x,g;const f=((x=document.getElementById("m-kit-style"))==null?void 0:x.value)||"uni",_=((g=ke[f])==null?void 0:g.colors)===3,$=document.getElementById("wrap-m-kit-color3");$&&($.style.display=_?"":"none")}b(),["m-kit-color1","m-kit-color2","m-kit-color3","m-kit-shorts","m-kit-socks"].forEach(f=>{const _=document.getElementById(f),$=document.getElementById(f+"-txt");!_||!$||(_.addEventListener("input",()=>{$.value=_.value,ae()}),$.addEventListener("input",()=>{const x=$.value.trim();/^#[0-9a-fA-F]{6}$/.test(x)&&(_.value=x,ae())}),$.addEventListener("change",()=>{let x=$.value.trim();x.startsWith("#")||(x="#"+x),/^#[0-9a-fA-F]{6}$/.test(x)&&(_.value=x,$.value=x,ae())}))}),(p=document.getElementById("m-kit-style"))==null||p.addEventListener("change",()=>{b(),ae()});const l=document.getElementById("m-real"),r=document.getElementById("m-encoded");function o(){if(!l||!r||r.value)return;const f=l.value.trim().split(/\s+/),_=f.length===1?f[0].toUpperCase().slice(0,6):f.filter($=>$.length>2).map($=>$[0].toUpperCase()).join("")||f[0].toUpperCase().slice(0,4);r.value=_}l==null||l.addEventListener("input",o),(h=document.getElementById("auto-encode-btn"))==null||h.addEventListener("click",()=>{r&&(r.value=""),o()}),(w=document.getElementById("m-save"))==null||w.addEventListener("click",()=>qt(e,c,t,n))},50)}function We(){var e,t,n,i,u,d;return{style:((e=document.getElementById("m-kit-style"))==null?void 0:e.value)||"uni",color1:((t=document.getElementById("m-kit-color1"))==null?void 0:t.value)||"#1A6B3C",color2:((n=document.getElementById("m-kit-color2"))==null?void 0:n.value)||"#ffffff",color3:((i=document.getElementById("m-kit-color3"))==null?void 0:i.value)||"#000000",shorts:((u=document.getElementById("m-kit-shorts"))==null?void 0:u.value)||"#111111",socks:((d=document.getElementById("m-kit-socks"))==null?void 0:d.value)||"#ffffff"}}function ae(){const e=document.getElementById("kit-preview-wrap");e&&(e.innerHTML=Fe(We(),"modal"))}async function qt(e,t,n,i){var f,_,$,x,g,v;const{toast:u,closeModal:d}=i,c=document.getElementById("m-error"),s=document.getElementById("m-save"),m=(f=document.getElementById("m-real"))==null?void 0:f.value.trim(),a=(_=document.getElementById("m-encoded"))==null?void 0:_.value.trim().toUpperCase(),b=($=document.getElementById("m-country"))==null?void 0:$.value.trim().toUpperCase(),l=((x=document.getElementById("m-logo"))==null?void 0:x.value.trim())||null,r=((g=document.getElementById("m-gen-stadium"))==null?void 0:g.checked)??!1,o=((v=document.getElementById("m-gen-squad"))==null?void 0:v.checked)??!1,p=We();if(!m){c.textContent="Le nom du club est requis.";return}if(!a){c.textContent="Le nom encodé est requis.";return}if(!b){c.textContent="Le pays est requis.";return}s.disabled=!0,s.textContent="Enregistrement…";const h={real_name:m,encoded_name:a,country_code:b,logo_url:l,kit_style:p.style,kit_color1:p.color1,kit_color2:p.color2,kit_color3:p.color3,kit_shorts:p.shorts,kit_socks:p.socks};let w=e==null?void 0:e.id;if(t){const{error:y}=await E.from("clubs").update(h).eq("id",w);if(y){c.textContent=y.message,s.disabled=!1,s.textContent="💾 Enregistrer";return}}else{const{data:y,error:k}=await E.from("clubs").insert(h).select().single();if(k){c.textContent=k.message,s.disabled=!1,s.textContent="✅ Créer le club";return}if(w=y.id,r){s.textContent="🏟️ Création du stade…";const{data:B,error:I}=await E.from("stadium_definitions").insert({name:`Stade de ${m}`,club_id:w,country_code:b}).select().single();I?console.warn("[Stadium] Erreur def stade:",I.message):B&&await E.from("cards").insert({card_type:"stadium",stadium_id:B.id})}o&&(s.textContent="⚽ Génération des joueurs…",await Ye(w,b,u))}u(t?"Club modifié ✅":"Club créé ✅","success"),d(),ge(n,i)}const Pt=["normal","pepite","papyte","legende"],Ae=["GK","DEF","MIL","ATT"],Ot=[["MA","Maroc"],["FR","France"],["AR","Argentine"],["PT","Portugal"],["BR","Brésil"],["ES","Espagne"],["DE","Allemagne"],["GB","Angleterre"],["IT","Italie"],["CM","Cameroun"],["SN","Sénégal"],["NG","Nigéria"],["DK","Danemark"],["NL","Pays-Bas"],["BE","Belgique"],["CI","Côte d'Ivoire"],["AL","Albanie"],["HR","Croatie"],["RS","Serbie"],["TR","Turquie"],["MW","Malawi"]];let Z={...q},pe=[];async function Ft(e,{toast:t}){const{data:n}=await E.from("clubs").select("id,encoded_name,logo_url,kit_style,kit_color1,kit_color2,kit_shorts,kit_socks").order("encoded_name");pe=n||[],Z={...q},e.innerHTML=Vt(pe),Gt(e,pe,t),V()}function ze(){var n;const e=((n=document.getElementById("cb-surname-real"))==null?void 0:n.value)||"",t=document.getElementById("cb-surname-enc");t&&(t.value=be(e))}function Ht(){var n;const e=(n=document.getElementById("cb-club"))==null?void 0:n.value,t=pe.find(i=>String(i.id)===String(e));return t?{style:t.kit_style||M.style,color1:t.kit_color1||M.color1,color2:t.kit_color2||M.color2,shorts:t.kit_shorts||M.shorts,socks:t.kit_socks||M.socks}:{...M}}function V(){var r,o,p,h;const e=w=>{var f;return((f=document.getElementById(w))==null?void 0:f.value)||""},t={firstname:e("cb-firstname"),surname_real:e("cb-surname-enc")||be(e("cb-surname-real")),country_code:e("cb-country"),club_encoded_name:((o=(r=document.getElementById("cb-club"))==null?void 0:r.selectedOptions[0])==null?void 0:o.text)||"",job:e("cb-job"),job2:e("cb-job2")||null,note_g:parseInt(e("cb-note-g"))||0,note_d:parseInt(e("cb-note-d"))||0,note_m:parseInt(e("cb-note-m"))||0,note_a:parseInt(e("cb-note-a"))||0,rarity:e("cb-rarity")},n=Ht(),i=document.getElementById("cb-club"),u=((h=(p=i==null?void 0:i.selectedOptions[0])==null?void 0:p.dataset)==null?void 0:h.logo)||null,d=He(Z,n,120),c=`data:image/svg+xml;charset=utf-8,${encodeURIComponent(d)}`,s=lt(t,{portraitUrl:c,clubLogoUrl:u,showNotes:!0}),m=document.getElementById("card-preview");m&&(m.innerHTML=s);const a=document.getElementById("avatar-preview-wrap");a&&(a.innerHTML=d);const b=e("cb-surname-real"),l=document.getElementById("encode-summary");l&&b&&(l.textContent=`${e("cb-firstname")} ${b} → ${e("cb-firstname")} ${t.surname_real}`)}function Ze(){const e=t=>{var n;return((n=document.getElementById(t))==null?void 0:n.value)||""};Z={skin:e("av-skin"),hair:e("av-hair"),hairColor:e("av-hairColor"),eyes:e("av-eyes"),nose:e("av-nose"),mouth:e("av-mouth"),eyebrows:e("av-eyebrows"),beard:e("av-beard")}}function Gt(e,t,n){var u,d,c,s,m;e.querySelectorAll("input,select").forEach(a=>{a.addEventListener("input",V),a.addEventListener("change",V)});const i=["av-skin","av-hair","av-hairColor","av-eyes","av-nose","av-mouth","av-eyebrows","av-beard"];i.forEach(a=>{var b;(b=document.getElementById(a))==null||b.addEventListener("change",()=>{Ze(),V()})}),(u=document.getElementById("cb-surname-real"))==null||u.addEventListener("input",()=>{ze(),V()}),(d=document.getElementById("btn-encode"))==null||d.addEventListener("click",()=>{ze(),V()}),(c=document.getElementById("cb-club"))==null||c.addEventListener("change",V),(s=document.getElementById("btn-save-player"))==null||s.addEventListener("click",()=>Ut(n)),(m=document.getElementById("btn-reset"))==null||m.addEventListener("click",()=>{e.querySelectorAll("input").forEach(a=>a.value=""),e.querySelectorAll("select").forEach(a=>a.selectedIndex=0),Z={...q},i.forEach(a=>{const b=a.replace("av-",""),l=document.getElementById(a);l&&(l.value=q[b]||"")}),V()})}async function Ut(e){const t=c=>{var s;return((s=document.getElementById(c))==null?void 0:s.value)||""},n=t("cb-surname-real").trim(),i=t("cb-surname-enc").trim()||be(n);if(!t("cb-firstname")||!n||!t("cb-country")||!t("cb-job")){e("Remplissez les champs obligatoires (prénom, nom, pays, poste)","error");return}Ze();const u={firstname:t("cb-firstname").trim(),surname_real:n,surname_real:i,country_code:t("cb-country"),club_id:t("cb-club")||null,job:t("cb-job"),job2:t("cb-job2")||null,note_g:parseInt(t("cb-note-g"))||0,note_d:parseInt(t("cb-note-d"))||0,note_m:parseInt(t("cb-note-m"))||0,note_a:parseInt(t("cb-note-a"))||0,rarity:t("cb-rarity")||"normal",note_min:parseInt(t("cb-note-min"))||null,note_max:parseInt(t("cb-note-max"))||null,sell_price:parseInt(t("cb-price"))||0,avatar_config:Z,skin:Z.skin||"blanc",hair:Z.hairColor||"marron",hair_length:"court"},{error:d}=await E.from("players").insert(u);if(d){e("Erreur: "+d.message,"error");return}e(`✅ Joueur "${u.firstname} ${u.surname_real}" enregistré !`,"success")}function Vt(e){const t=Ot.map(([a,b])=>`<option value="${a}">${b} (${a})</option>`).join(""),n='<option value="">— Sélectionner un club —</option>'+e.map(a=>`<option value="${a.id}" data-logo="${a.logo_url||""}">${a.encoded_name}</option>`).join(""),i=Ae.map(a=>`<option value="${a}">${a}</option>`).join(""),u='<option value="">Aucun</option>'+Ae.map(a=>`<option value="${a}">${a}</option>`).join(""),d=Pt.map(a=>`<option value="${a}">${a.charAt(0).toUpperCase()+a.slice(1)}</option>`).join(""),c=(a,b,l)=>Object.entries(b).map(([r,o])=>`<option value="${r}" ${r===(l||"")?"selected":""}>${o.label}</option>`).join(""),s=Object.keys(rt).map(a=>`<option value="${a}" ${a===q.skin?"selected":""}>${a.charAt(0).toUpperCase()+a.slice(1)}</option>`).join(""),m=Object.keys(at).map(a=>`<option value="${a}" ${a===q.hairColor?"selected":""}>${a.charAt(0).toUpperCase()+a.slice(1)}</option>`).join("");return`
      ...
      <div class="card-panel">
        ...
        <div style="margin-top:8px;display:flex;gap:8px">
          ${["normal","pepite","papyte","legende"].map(a=>`
            <div style="background:${st[a]};color:#fff;padding:3px 8px;border-radius:4px;font-size:11px;font-weight:700">
              ${a.toUpperCase()}
            </div>`).join("")}
        </div>
      </div>
        ...
        <div style="display:grid;grid-template-columns:1fr 120px;gap:16px;align-items:start">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
            ...
            <div>
              ...
              <select id="av-hair">${c("av-hair",dt,q.hair)}</select>
            </div>
            <div>
            ...
            <div>
              ...
              <select id="av-eyebrows">${c("av-eyebrows",ct,q.eyebrows)}</select>
            </div>
            <div>
              <label>Yeux</label>
              <select id="av-eyes">${c("av-eyes",pt,q.eyes)}</select>
            </div>
            <div>
              <label>Nez</label>
              <select id="av-nose">${c("av-nose",ut,q.nose)}</select>
            </div>
            <div>
              <label>Bouche</label>
              <select id="av-mouth">${c("av-mouth",mt,q.mouth)}</select>
            </div>
            <div>
              <label>Barbe</label>
              <select id="av-beard">${c("av-beard",gt,q.beard)}</select>
            </div>
          </div>
  ...
  `,d(u),document.getElementById("search-users").addEventListener("input",c=>{const s=c.target.value.toLowerCase();d(u.filter(m=>{var a;return m.pseudo.toLowerCase().includes(s)||((a=m.club_name)==null?void 0:a.toLowerCase().includes(s))}))});function d(c){document.getElementById("users-tbody").innerHTML=c.map(s=>{const m=s.mmr??1e3,a=s.mmr_deviation??350,b=s.placement_matches??0,l=ft(m),r=s.ranked_wins??0,o=s.ranked_losses??0,p=s.ranked_draws??0,h=r+o+p,w=h>0?Math.round(r/h*100):null,f=b<10,_=a<80?"#1A6B3C":a<150?"#e67e22":"#bb2020";return`
        <tr>
          <td>
  ...
  `,document.querySelectorAll("[data-cancel]").forEach(s=>{s.addEventListener("click",async()=>{if(!confirm("Annuler cette annonce ?"))return;const{error:m}=await E.from("market_listings").update({status:"cancelled"}).eq("id",s.dataset.cancel);m?t(m.message,"error"):(t("Annonce annulée","success"),Qe(e,{toast:t}))})})}async function Kt(e,{toast:t}){e.innerHTML=`
...
`)?'"'+t.replace(/"/g,'""')+'"':t}function se(e,t){const n=new Blob(["\uFEFF"+t],{type:"text/csv;charset=utf-8;"}),i=URL.createObjectURL(n),u=document.createElement("a");u.href=i,u.download=e,u.click(),URL.revokeObjectURL(i)}function Te(e){e=e.replace(/^\uFEFF/,"").replace(/\r\n/g,`
`).replace(/\r/g,`
`);const t=e.split(`
`).filter(c=>c.trim());if(t.length<2)return[];const n=t[0],i=n.split(";").length>n.split(",").length?";":",",u=Me(n,i).map(c=>c.trim().replace(/^\uFEFF/,"").toLowerCase()),d=[];for(let c=1;c<t.length;c++){if(!t[c].trim())continue;const s=Me(t[c],i),m={};u.forEach((a,b)=>{m[a]=(s[b]||"").trim()}),!Object.values(m).every(a=>!a)&&d.push(m)}return d}function Me(e,t=","){const n=[];let i="",u=!1;for(let d=0;d<e.length;d++){const c=e[d];u?c==='"'?e[d+1]==='"'?(i+='"',d++):u=!1:i+=c:c==='"'?u=!0:c===t?(n.push(i),i=""):i+=c}return n.push(i),n}const Yt={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};async function Jt(e,{toast:t}){const n=Object.keys(le);let i=n[0];const{data:u}=await E.from("formation_links_overrides").select("formation, links"),d={};(u||[]).forEach(b=>{d[b.formation]=b.links});let c=new Set;function s(b,l){return[b,l].sort().join("-")}function m(b){const l=d[b]||le[b]||[];c=new Set(l.map(([r,o])=>s(r,o)))}m(i),e.innerHTML=`
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;gap:12px;flex-wrap:wrap">
  ...
  `;function a(){const b=W[i]||{},l=Ie(i),r=320,o=400,p=22;function h(x){const g=b[x];return g?{x:g.x*r,y:g.y*o}:null}let w=`<svg width="${r}" height="${o}" viewBox="0 0 ${r} ${o}" xmlns="http://www.w3.org/2000/svg" style="display:block">`;l.forEach(([x,g],v)=>{const y=h(x),k=h(g);if(!y||!k)return;const B=s(x,g),I=c.has(B),L=I?"#3b82f6":"#999",A=I?.95:.35,C=I?4:3;w+=`<line x1="${y.x}" y1="${y.y}" x2="${k.x}" y2="${k.y}"
    ...
    </div>`,document.getElementById("add-config").addEventListener("click",()=>je(null)),document.getElementById("form-cancel").addEventListener("click",()=>{document.getElementById("config-form").style.display="none"}),document.getElementById("form-save").addEventListener("click",()=>nn(e)),e.querySelectorAll("[data-edit]").forEach(i=>{const u=(t||[]).find(d=>d.id===i.dataset.edit);u&&i.addEventListener("click",()=>je(u))}),e.querySelectorAll("[data-delete]").forEach(i=>{i.addEventListener("click",async()=>{confirm("Supprimer cette règle ?")&&(await E.from("sell_price_configs").delete().eq("id",i.dataset.delete),await ye(e))})})}function tn(e){const t=et.find(n=>n.value===e.rarity);return`
    ...
    </div>`,document.getElementById("add-article").addEventListener("click",()=>Re(null)),document.getElementById("form-cancel").addEventListener("click",()=>{document.getElementById("article-form").style.display="none"}),document.getElementById("form-save").addEventListener("click",()=>an(e)),e.querySelectorAll("[data-edit]").forEach(n=>{const i=(t||[]).find(u=>u.id===n.dataset.edit);i&&n.addEventListener("click",()=>Re(i))}),e.querySelectorAll("[data-delete]").forEach(n=>{n.addEventListener("click",async()=>{confirm("Supprimer cet article ?")&&(await E.from("patch_notes").delete().eq("id",n.dataset.delete),await he(e))})})}function rn(e){const t=new Date(e.published_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"short",year:"numeric"});return`
    ...
    <div style="padding:20px;max-width:820px;margin:0 auto">
      ...
      <div id="ts-form" style="display:none;margin-top:24px;background:#fff;border-radius:12px;padding:20px;box-shadow:0 1px 8px rgba(0,0,0,0.1)">
        ...
        <div style="display:flex;gap:10px">
          ...
          </div>`).join("")+"</div>",m.querySelectorAll("[data-pick]").forEach(I=>{I.addEventListener("click",()=>{d&&(d.value=I.dataset.pick),a(),m.style.display="none"})})}catch(v){m.innerHTML='<div style="color:#cc2222;font-size:12px">Erreur chargement : '+v.message+"</div>"}}),(x=document.getElementById("ts-color"))==null||x.addEventListener("input",v=>{const y=document.getElementById("ts-color-hex");y&&(y.value=v.target.value)}),(g=document.getElementById("ts-color-hex"))==null||g.addEventListener("input",v=>{const y=v.target.value;if(/^#[0-9a-fA-F]{6}$/.test(y)){const k=document.getElementById("ts-color");k&&(k.value=y)}}),e.querySelectorAll("[data-edit]").forEach(v=>{const y=(t||[]).find(k=>k.id===v.dataset.edit);y&&v.addEventListener("click",()=>De(y))}),e.querySelectorAll("[data-delete]").forEach(v=>{v.addEventListener("click",async()=>{confirm("Supprimer cette étape ?")&&(await E.from("tutorial_steps").delete().eq("id",v.dataset.delete),Q(e))})}),e.querySelectorAll("[data-toggle]").forEach(v=>{v.addEventListener("click",async()=>{const y=(t||[]).find(k=>k.id===v.dataset.toggle);y&&(await E.from("tutorial_steps").update({is_active:!y.is_active}).eq("id",y.id),Q(e))})}),e.querySelectorAll("[data-up]").forEach(v=>{v.addEventListener("click",async()=>{const y=t||[],k=y.findIndex(B=>B.id===v.dataset.up);k<=0||(await Promise.all([E.from("tutorial_steps").update({step_order:y[k-1].step_order}).eq("id",y[k].id),E.from("tutorial_steps").update({step_order:y[k].step_order}).eq("id",y[k-1].id)]),Q(e))})}),e.querySelectorAll("[data-down]").forEach(v=>{v.addEventListener("click",async()=>{const y=t||[],k=y.findIndex(B=>B.id===v.dataset.down);k<0||k>=y.length-1||(await Promise.all([E.from("tutorial_steps").update({step_order:y[k+1].step_order}).eq("id",y[k].id),E.from("tutorial_steps").update({step_order:y[k].step_order}).eq("id",y[k+1].id)]),Q(e))})})}function ln(e){return`
  <div style="background:#fff;border-radius:10px;padding:12px 16px;box-shadow:0 1px 4px rgba(0,0,0,0.08);display:flex;align-items:center;gap:12px">
    ...
    </div>`,(i=document.getElementById("st-add-btn"))==null||i.addEventListener("click",()=>Ne(null)),(u=document.getElementById("st-cancel"))==null||u.addEventListener("click",()=>{document.getElementById("st-form").style.display="none"}),(d=document.getElementById("st-save"))==null||d.addEventListener("click",()=>bn(e)),(c=document.getElementById("st-preview-btn"))==null||c.addEventListener("click",()=>ue()),["st-name","st-club","st-country","st-image"].forEach(s=>{var m,a;(m=document.getElementById(s))==null||m.addEventListener("input",ue),(a=document.getElementById(s))==null||a.addEventListener("change",ue)}),e.querySelectorAll("[data-edit]").forEach(s=>{const m=(t||[]).find(a=>a.id===s.dataset.edit);m&&s.addEventListener("click",()=>Ne(m))}),e.querySelectorAll("[data-delete]").forEach(s=>{s.addEventListener("click",async()=>{confirm("Supprimer ce stade ?")&&(await E.from("stadium_definitions").delete().eq("id",s.dataset.delete),we(e))})})}function gn(e){var n,i;const t=(n=e.club)!=null&&n.logo_url?`<img src="${e.club.logo_url}" style="width:32px;height:32px;object-fit:contain">`:'<span style="font-size:20px">🌍</span>';return`
    <div style="background:#fff;border-radius:10px;padding:12px 16px;box-shadow:0 1px 4px rgba(0,0,0,0.08);display:flex;align-items:center;gap:12px">
      <div style="width:40px;height:40px;border-radius:8px;background:${nt};display:flex;align-items:center;justify-content:center;flex-shrink:0">
        ${t}
      </div>
      <div style="flex:1">
    ...
    </div>`,(x=document.getElementById("create-season-btn"))==null||x.addEventListener("click",()=>{qe(null,{toast:n,openModal:i,closeModal:u,reload:()=>X(e,t)})}),e.querySelectorAll("[data-edit]").forEach(g=>{const v=m.find(y=>y.id==g.dataset.edit);g.addEventListener("click",()=>{qe(v,{toast:n,openModal:i,closeModal:u,reload:()=>X(e,t)})})}),e.querySelectorAll("[data-activate]").forEach(g=>{g.addEventListener("click",async()=>{const v=parseInt(g.dataset.activate);if(!confirm("Activer cette saison ? La saison actuellement active sera désactivée."))return;const{error:y}=await E.from("ranked_seasons").update({is_active:!1}).neq("id",0);if(y){n(y.message,"error");return}const{error:k}=await E.from("ranked_seasons").update({is_active:!0}).eq("id",v);if(k){n(k.message,"error");return}await E.from("users").update({current_season_id:v}).gt("placement_matches",0),n("Saison activée ✅","success"),X(e,t)})}),e.querySelectorAll("[data-deactivate]").forEach(g=>{g.addEventListener("click",async()=>{if(!confirm("Désactiver la saison active ? Le mode Ranked sera suspendu."))return;const{error:v}=await E.from("ranked_seasons").update({is_active:!1}).eq("id",parseInt(g.dataset.deactivate));if(v){n(v.message,"error");return}n("Saison désactivée","success"),X(e,t)})}),e.querySelectorAll("[data-delete]").forEach(g=>{g.addEventListener("click",async()=>{if(!confirm("Supprimer cette saison archivée ?"))return;const{error:v}=await E.from("ranked_seasons").delete().eq("id",parseInt(g.dataset.delete));if(v){n(v.message,"error");return}n("Saison supprimée","success"),X(e,t)})})}function qe(e,{toast:t,openModal:n,closeModal:i,reload:u}){const d=!!e,c=new Date().toISOString().slice(0,16),s=new Date(Date.now()+90*864e5).toISOString().slice(0,16),m=e?new Date(e.start_at).toISOString().slice(0,16):c,a=e?new Date(e.end_at).toISOString().slice(0,16):s,b=(e==null?void 0:e.name)||"";n(d?"✏️ Modifier la saison":"➕ Nouvelle saison",`<div style="display:flex;flex-direction:column;gap:14px">
      <div class="form-group">
  ...
  <div id="sve" style="display:flex;flex-direction:column;height:100%;background:#060d07;color:#fff;overflow:hidden;font-family:system-ui,sans-serif">
    ...
    <div style="flex-shrink:0;background:#0d1f0e;border-bottom:1px solid rgba(255,255,255,0.1);padding:8px 12px;display:flex;align-items:center;gap:8px;flex-wrap:wrap">
      ...
      <select id="ed-form" style="padding:4px 8px;border-radius:6px;background:#1a3a1a;border:1px solid rgba(255,255,255,0.2);color:#fff;font-size:11px">
        ${Object.keys(W).map(i=>`<option value="${i}" ${i===n.formation?"selected":""}>${i}</option>`).join("")}
      </select>
  ...
  </div>`,it(e,n.cfg),G(e,n),kn(e,n)}function N(e,t,n,i,u){return`<div style="margin-bottom:11px">
  ...
  </svg>`}function _n(e,t,n,i,u){e.viewport;const d=e.page,c=`<div style="margin-top:${n}px;overflow:visible">${t}</div>`;return d==="deck-select"?`
