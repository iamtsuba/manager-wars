import{s as _,r as Ee,a as _e,A as vt,l as xt,g as ht,K as Me,D as z,b as Et,S as _t,c as R,H as wt,R as It,e as pe,d as Ze,f as $t,h as kt,E as Lt,i as Bt,N as At,M as St,B as Ct,j as Tt,F as me,k as ze,m as Mt,n as Qe,o as fe,p as zt}from"./special-cards--9JnHrqo.js";async function Rt(e){var h;const[{count:t},{count:n},{count:o},{count:l},{count:a},{data:m}]=await Promise.all([_.from("players").select("*",{count:"exact",head:!0}),_.from("clubs").select("*",{count:"exact",head:!0}),_.from("users").select("*",{count:"exact",head:!0}),_.from("stadium_definitions").select("*",{count:"exact",head:!0}),_.from("users").select("*",{count:"exact",head:!0}).gt("last_seen_at",new Date(Date.now()-18e4).toISOString()),_.rpc("get_signup_password")]),u=m||"";e.innerHTML=`
  ...
  `,window.adminNav=i=>{var f;(f=document.querySelector(`[data-page="${i}"]`))==null||f.click()},(h=document.getElementById("signup-pwd-save"))==null||h.addEventListener("click",async()=>{const i=document.getElementById("signup-pwd-input").value.trim(),f=document.getElementById("signup-pwd-status");if(!i){f.textContent="Le code ne peut pas être vide.",f.style.color="#ff6b6b";return}const r=document.getElementById("signup-pwd-save");r.disabled=!0,r.textContent="⏳...";const{error:s}=await _.rpc("set_signup_password",{new_password:i});if(r.disabled=!1,r.textContent="💾 Enregistrer",s){f.textContent=s.message,f.style.color="#ff6b6b";return}f.textContent="✅ Code mis à jour.",f.style.color="#2ecc71"})}const Xe={normal:"Normal",pepite:"Pépite",papyte:"Papyte",legende:"Légende"},jt={GK:"#aaaaaa",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};async function Dt(){const e={};for(const t of vt)e[t]=await xt(t);return e}async function Nt(e,t){const{toast:n}=t;if(!confirm(`Réattribuer une nouvelle photo à tous les joueurs ayant encore l'ancien format de chemin (public/faces/...) ?

Cette action est irréversible.`))return;const o=document.getElementById("fix-old-faces-btn");o&&(o.disabled=!0,o.textContent="⏳ En cours...");try{const{data:l,error:a}=await _.from("players").select("id, country_code, face").like("face","public/faces/%");if(a){n(a.message,"error");return}if(!(l!=null&&l.length)){n("Aucun joueur avec l'ancien format — rien à faire ✅","success");return}const{data:m}=await _.from("players").select("face").not("face","is",null),u=new Set((m||[]).map(f=>f.face).filter(f=>f&&!f.startsWith("public/faces/")));let h=0,i=0;for(const f of l){const r=await _e(f.country_code,u);if(!r){i++;continue}const{error:s}=await _.from("players").update({face:r}).eq("id",f.id);if(s){i++;continue}u.add(r),h++}n(`${h} photo(s) réattribuée(s)${i?`, ${i} échec(s)`:""} ✅`,i?"error":"success")}finally{o&&(o.disabled=!1,o.textContent="🖼️ Réattribuer anciennes photos")}}async function Pt(e,t){await se(e,t)}async function se(e,t,n=null){var i,f,r,s,g;const{toast:o}=t;n||(n={search:((i=document.getElementById("search-players"))==null?void 0:i.value)||"",job:((f=document.getElementById("filter-job"))==null?void 0:f.value)||"",rarity:((r=document.getElementById("filter-rarity"))==null?void 0:r.value)||"",club:((s=document.getElementById("filter-club"))==null?void 0:s.value)||"",country:((g=document.getElementById("filter-country"))==null?void 0:g.value)||""});const[{data:l,error:a},{data:m}]=await Promise.all([_.from("players").select("*, clubs(id,encoded_name,logo_url)"),_.from("clubs").select("id,encoded_name").order("encoded_name")]);if(a){e.innerHTML=`<p style="color:red">${a.message}</p>`;return}const u={GK:0,DEF:1,MIL:2,ATT:3},h=(l||[]).sort((d,y)=>{const c=(u[d.job]??4)-(u[y.job]??4);return c!==0?c:(d.surname_real||"").localeCompare(y.surname_real||"")});Ft(e,h,m||[],t,n)}function Ft(e,t,n,o,l=null){var f,r;const{toast:a}=o;if(e.innerHTML=`
    <div style="display:flex;flex-direction:column;gap:10px">
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        ...
        <select id="filter-rarity" style="width:110px">
          ...
          ${Object.entries(Xe).map(([s,g])=>`<option value="${s}">${g}</option>`).join("")}
        </select>
    ...
    </div>`,l){const s=l,g=d=>document.getElementById(d);s.search&&g("search-players")&&(g("search-players").value=s.search),s.job&&g("filter-job")&&(g("filter-job").value=s.job),s.rarity&&g("filter-rarity")&&(g("filter-rarity").value=s.rarity),s.club&&g("filter-club")&&(g("filter-club").value=s.club),s.country&&g("filter-country")&&(g("filter-country").value=s.country)}if(l){const s=l,g=d=>document.getElementById(d);s.search&&g("search-players")&&(g("search-players").value=s.search),s.job&&g("filter-job")&&(g("filter-job").value=s.job),s.rarity&&g("filter-rarity")&&(g("filter-rarity").value=s.rarity),s.club&&g("filter-club")&&(g("filter-club").value=s.club),s.country&&g("filter-country")&&(g("filter-country").value=s.country)}function m(){const s=document.getElementById("search-players").value.toLowerCase(),g=document.getElementById("filter-job").value,d=document.getElementById("filter-rarity").value,y=document.getElementById("filter-club").value,c=document.getElementById("filter-country").value;return t.filter(b=>(!s||`${b.firstname} ${b.surname_real}`.toLowerCase().includes(s))&&(!g||b.job===g)&&(!d||b.rarity===d)&&(!y||b.club_id===y)&&(!c||b.country_code===c))}const u=new Set;function h(){const s=document.getElementById("bulk-bar"),g=document.getElementById("bulk-count");s&&(s.style.display=u.size>0?"flex":"none",g&&(g.textContent=`${u.size} joueur(s) sélectionné(s)`))}function i(){const s=m();document.getElementById("count-label").textContent=`${s.length} joueur(s)`;const g=document.getElementById("players-list");if(!s.length){g.innerHTML='<div style="color:var(--gray-600);padding:20px;text-align:center">Aucun joueur.</div>';return}g.innerHTML=s.map(d=>{const y={...d,clubs:d.clubs,face:d.face||null},c=Ee(y,{width:120});return`<div style="position:relative;cursor:pointer" data-edit="${d.id}">
        ${c}
      ...
      </div>`}).join(""),g.querySelectorAll("[data-edit]").forEach(d=>{d.addEventListener("click",()=>{const y=t.find(c=>c.id===d.dataset.edit);y&&Re(y,n,e,o)})}),g.querySelectorAll(".btn-del-player").forEach(d=>{d.addEventListener("click",async y=>{if(y.stopPropagation(),!confirm("Supprimer ce joueur ?"))return;const{error:c}=await _.from("players").delete().eq("id",d.dataset.del);c?a(c.message,"error"):(a("Joueur supprimé ✅","success"),se(e,o))})})}i(),document.getElementById("search-players").addEventListener("input",i),document.getElementById("filter-job").addEventListener("change",i),document.getElementById("filter-rarity").addEventListener("change",i),document.getElementById("filter-club").addEventListener("change",i),document.getElementById("filter-country").addEventListener("change",i),(f=document.getElementById("bulk-cancel-btn"))==null||f.addEventListener("click",()=>{u.clear(),h(),i()}),(r=document.getElementById("bulk-delete-btn"))==null||r.addEventListener("click",async()=>{var y,c,b,I,$;if(!u.size||!confirm(`Supprimer ${u.size} joueur(s) ?`))return;const s=[...u],{error:g}=await _.from("players").delete().in("id",s);if(g){a(g.message,"error");return}a(`${s.length} joueur(s) supprimé(s) ✅`,"success"),u.clear();const d={search:((y=document.getElementById("search-players"))==null?void 0:y.value)||"",job:((c=document.getElementById("filter-job"))==null?void 0:c.value)||"",rarity:((b=document.getElementById("filter-rarity"))==null?void 0:b.value)||"",club:((I=document.getElementById("filter-club"))==null?void 0:I.value)||"",country:(($=document.getElementById("filter-country"))==null?void 0:$.value)||""};se(e,o,d)}),document.getElementById("add-player-btn").addEventListener("click",()=>Re(null,n,e,o)),document.getElementById("fix-old-faces-btn").addEventListener("click",()=>Nt(e,o))}async function Re(e,t,n,o){const{toast:l,openModal:a,closeModal:m}=o,u=!!e,h=await Dt(),{data:i}=await _.from("players").select("face").not("face","is",null),f=new Set((i||[]).map(d=>d.face).filter(Boolean));e!=null&&e.face&&f.delete(e.face);const r='<option value="">— Club —</option>'+t.map(d=>`<option value="${d.id}" ${(e==null?void 0:e.club_id)===d.id?"selected":""}>${d.encoded_name}</option>`).join(""),s=e!=null&&e.face?e.face.split("/")[0]:"",g=Object.keys(h).map(d=>`<option value="${d}" ${s===d?"selected":""}>${d}</option>`).join("");a(u?`✏️ ${e.firstname} ${e.surname_real}`:"➕ Nouveau joueur",`<div style="display:flex;gap:20px;align-items:flex-start">
      ...
      <div style="flex:1;min-width:300px;display:flex;flex-direction:column;gap:12px">
        ...
        <div style="border-top:1px solid var(--gray-200);padding-top:10px">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:8px">
            ${[["GK","pm-g","note_g"],["DEF","pm-d","note_d"],["MIL","pm-m","note_m"],["ATT","pm-a","note_a"]].map(([d,y,c])=>`
              <div class="form-group" style="text-align:center">
                <label style="color:${jt[d]};font-weight:700">${d}</label>
                <input id="${y}" type="number" min="0" max="20" value="${(e==null?void 0:e[c])??0}" style="text-align:center;font-size:18px;font-weight:900;padding:6px 4px">
              </div>`).join("")}
          </div>
    ...
    </div>`,m(re),document.getElementById("search-clubs").addEventListener("input",u=>{const h=u.target.value.toLowerCase();m(re.filter(i=>i.real_name.toLowerCase().includes(h)||i.encoded_name.toLowerCase().includes(h)))}),document.getElementById("add-club-btn").addEventListener("click",()=>je(null,e,t));function m(u){const h=document.getElementById("clubs-list");if(!u.length){h.innerHTML='<p style="color:var(--tile-fg-dim);padding:20px;text-align:center">Aucun club.</p>';return}const i={};u.forEach(s=>{const g=s.country_code||"—";i[g]||(i[g]=[]),i[g].push(s)});const f=Object.keys(i).sort(),r=s=>{const g=st(s),d=s.logo_url?`<img src="${s.logo_url}" style="width:36px;height:36px;object-fit:contain;border-radius:8px;flex-shrink:0">`:`<div style="width:36px;height:36px;background:linear-gradient(135deg,${g.color1},${g.color2});border-radius:8px;display:flex;align-items:center;justify-content:center;color:#fff;font-size:9px;font-weight:900;flex-shrink:0">${s.encoded_name.slice(0,3)}</div>`,y=n[s.id]||0;return`
    ...
    `).join(""),h.querySelectorAll(".club-row").forEach(s=>{s.addEventListener("click",()=>{const g=re.find(d=>d.id===s.dataset.clubId);g&&je(g,e,t),h.querySelectorAll(".club-row").forEach(d=>d.style.background="var(--tile-bg)"),s.style.background="rgba(26,107,60,0.18)"})}),h.querySelectorAll(".btn-del-club").forEach(s=>{s.addEventListener("click",async()=>{if(!confirm("Supprimer ce club ?"))return;const{error:g}=await _.from("clubs").delete().eq("id",s.dataset.del);g?o(g.message,"error"):(o("Club supprimé","success"),ue(e,t))})})}}async function je(e,t,n){var c,b,I,$,E,v;const{toast:o}=n,l=!!e,a=e?st(e):{...z},m=document.getElementById("club-panel");if(!m)return;const u=Object.entries(Me).map(([p,x])=>`<option value="${p}" ${a.style===p?"selected":""}>${x.label}</option>`).join(""),h=we.map(p=>`<option value="${p}" ${((e==null?void 0:e.country_code)||"FR")===p?"selected":""}>${p}</option>`).join("");m.style.display="block",m.style.alignItems="",m.style.justifyContent="",m.innerHTML=`
      ...
      <div style="border-top:1px solid var(--tile-border);padding-top:12px">
        ...
        <div style="display:flex;gap:12px;align-items:flex-start;flex-wrap:wrap">
          <div style="flex:1;min-width:200px;display:flex;flex-direction:column;gap:8px">
            <div class="form-group">
              <label>Style</label>
              <select id="m-kit-style" style="width:100%">${u}</select>
            </div>
            ${[["Couleur 1","m-kit-color1",a.color1,!1],["Couleur 2","m-kit-color2",a.color2,!1],["Couleur 3","m-kit-color3",a.color3,!0],["Short","m-kit-shorts",a.shorts,!1],["Chaussettes","m-kit-socks",a.socks,!1]].map(([p,x,w,k])=>`
  ...
  `,(c=document.getElementById("m-logo-file"))==null||c.addEventListener("change",p=>{var k;const x=(k=p.target.files)==null?void 0:k[0];if(!x)return;const w=new FileReader;w.onload=()=>{document.getElementById("logo-preview").innerHTML=`<img src="${w.result}" style="width:100%;height:100%;object-fit:contain">`},w.readAsDataURL(x)}),ne();function i(){var k,L;const p=((k=document.getElementById("m-kit-style"))==null?void 0:k.value)||"uni",x=((L=Me[p])==null?void 0:L.colors)===3,w=document.getElementById("wrap-m-kit-color3");w&&(w.style.display=x?"":"none")}i(),["m-kit-color1","m-kit-color2","m-kit-color3","m-kit-shorts","m-kit-socks"].forEach(p=>{const x=document.getElementById(p),w=document.getElementById(p+"-txt");!x||!w||(x.addEventListener("input",()=>{w.value=x.value,ne()}),w.addEventListener("input",()=>{const k=w.value.trim();/^#[0-9a-fA-F]{6}$/.test(k)&&(x.value=k,ne())}),w.addEventListener("change",()=>{let k=w.value.trim();k.startsWith("#")||(k="#"+k),/^#[0-9a-fA-F]{6}$/.test(k)&&(x.value=k,w.value=k,ne())}))}),(b=document.getElementById("m-kit-style"))==null||b.addEventListener("change",()=>{i(),ne()});const f=document.getElementById("m-real"),r=document.getElementById("m-encoded");function s(){if(!f||!r||r.value)return;const p=f.value.trim().split(/\s+/),x=p.length===1?p[0].toUpperCase().slice(0,6):p.filter(w=>w.length>2).map(w=>w[0].toUpperCase()).join("")||p[0].toUpperCase().slice(0,4);r.value=x}f==null||f.addEventListener("input",s),(I=document.getElementById("auto-encode-btn"))==null||I.addEventListener("click",()=>{r&&(r.value=""),s()}),($=document.getElementById("m-save"))==null||$.addEventListener("click",()=>an(e,l,t,n));const g=document.getElementById("m-gen-squad"),d=document.getElementById("m-gen-strong-label");function y(){if(d&&(d.style.display=g!=null&&g.checked?"flex":"none",!(g!=null&&g.checked))){const p=document.getElementById("m-gen-strong");p&&(p.checked=!1)}}g==null||g.addEventListener("change",y),y(),(E=document.getElementById("btn-gen-squad-panel"))==null||E.addEventListener("click",()=>{Xt(e,n,t)}),(v=document.getElementById("btn-gen-one-player"))==null||v.addEventListener("click",()=>{tn(e,n)}),l&&$e(e.id)}async function $e(e){const{data:t}=await _.from("players").select("*, clubs(encoded_name, logo_url, kit_style, kit_color1, kit_color2, kit_color3, kit_shorts, kit_socks)").eq("club_id",e).order("surname_real"),n=document.getElementById("club-players-grid"),o=document.getElementById("club-players-count");if(o&&(o.textContent=(t||[]).length),!!n){if(!(t!=null&&t.length)){n.innerHTML='<div style="color:var(--tile-fg-dim);padding:20px;font-size:13px">Aucun joueur dans ce club.</div>';return}n.innerHTML=t.map(l=>Ee(l,{width:100})).join("")}}function Xt(e,t,n){const{openModal:o,closeModal:l,toast:a}=t;o(`⚽ Générer — ${e.real_name}`,`<div style="display:flex;flex-direction:column;gap:14px">
    ...
    </div>`),setTimeout(()=>{var m,u;(m=document.getElementById("qg-cancel"))==null||m.addEventListener("click",()=>l()),(u=document.getElementById("qg-ok"))==null||u.addEventListener("click",async()=>{var i;const h=((i=document.getElementById("qg-strong"))==null?void 0:i.checked)??!1;l(),a("Génération en cours…","info"),await lt(e.id,e.country_code,a,h),a("Effectif généré ✅","success"),$e(e.id),ue(n,t)})},50)}const dt=["GK","DEF","MIL","ATT"],en=[{value:"normal",label:"Normal"},{value:"pepite",label:"Pépite"},{value:"papyte",label:"Papyte"},{value:"legende",label:"Légende"}];function tn(e,t){const{openModal:n,closeModal:o,toast:l}=t,a=we.map(i=>`<option value="${i}">${i}</option>`).join(""),m=i=>dt.map(f=>`<option value="${f}" ${f===i?"selected":""}>${f}</option>`).join(""),u=`
  ...
  `;n(`➕ Générer un joueur — ${e.real_name}`,u,`
    <button id="gen-one-cancel" class="btn btn-ghost">Annuler</button>
    <button id="gen-one-ok" class="btn btn-primary">➕ Générer</button>
  `),setTimeout(()=>{var i,f;document.querySelectorAll('input[name="gen-one-mode"]').forEach(r=>{r.addEventListener("change",()=>{document.getElementById("gen-one-custom-fields").style.display=r.value==="custom"&&r.checked?"flex":"none"})}),(i=document.getElementById("gen-one-cancel"))==null||i.addEventListener("click",()=>o()),(f=document.getElementById("gen-one-ok"))==null||f.addEventListener("click",async()=>{var d,y,c,b,I;const r=((d=document.querySelector('input[name="gen-one-mode"]:checked'))==null?void 0:d.value)||"random",s=document.getElementById("gen-one-ok");s.disabled=!0,s.textContent="⏳ Génération…";const g={};r==="custom"&&(g.job=((y=document.getElementById("gen-one-job1"))==null?void 0:y.value)||null,g.job2=((c=document.getElementById("gen-one-job2"))==null?void 0:c.value)||null,g.rarity=((b=document.getElementById("gen-one-rarity"))==null?void 0:b.value)||null,g.country=((I=document.getElementById("gen-one-country"))==null?void 0:I.value)||null);try{await nn(e,g),l("Joueur généré ✅","success"),o(),$e(e.id)}catch($){l("Erreur : "+$.message,"error"),s.disabled=!1,s.textContent="➕ Générer"}})},50)}async function nn(e,t={}){const n=t.country||(Math.random()<.5?e.country_code:nt(e.country_code)),o=t.job||q(dt),l=t.job2||null,a=t.rarity||(()=>{const B=Math.random()*100;return B<1.5?"legende":B<3?"pepite":B<4.5?"papyte":"normal"})(),m=a==="pepite"?J(10,15):a==="papyte"?J(15,20):it(),u=l?Math.max(0,m-2):0;let h=0,i=0,f=0,r=0;o==="GK"&&(h=m),o==="DEF"&&(i=m),o==="MIL"&&(f=m),o==="ATT"&&(r=m),l==="GK"&&(h=u),l==="DEF"&&(i=u),l==="MIL"&&(f=u),l==="ATT"&&(r=u);const s=Ie(n),g=tt(n),d=q(et),{data:y}=await _.from("players").select("surname_real").not("surname_real","is",null),c=new Set((y||[]).map(B=>B.surname_real).filter(Boolean)),b=rt(n),I=b.filter(B=>!c.has(B)),$=q(I.length?I:b),E={job:o,job2:l,firstname:at(n),surname_real:$,country_code:n,club_id:e.id,note_g:h,note_d:i,note_m:f,note_a:r,skin:s,hair:g,hair_length:d,rarity:a,sell_price:0,ethnie:ye(n)},{data:v}=await _.from("players").select("face").not("face","is",null),p=new Set((v||[]).map(B=>B.face).filter(Boolean)),x=await _e(n,p);x&&(E.face=x);const{data:w,error:k}=await _.from("players").insert(E).select("id").single();if(k)throw k;const{error:L}=await _.from("cards").insert({card_type:"player",player_id:w.id});L&&console.warn("[GenOnePlayer] Erreur carte:",L.message)}function ct(){var e,t,n,o,l,a;return{style:((e=document.getElementById("m-kit-style"))==null?void 0:e.value)||"uni",color1:((t=document.getElementById("m-kit-color1"))==null?void 0:t.value)||"#1A6B3C",color2:((n=document.getElementById("m-kit-color2"))==null?void 0:n.value)||"#ffffff",color3:((o=document.getElementById("m-kit-color3"))==null?void 0:o.value)||"#000000",shorts:((l=document.getElementById("m-kit-shorts"))==null?void 0:l.value)||"#111111",socks:((a=document.getElementById("m-kit-socks"))==null?void 0:a.value)||"#ffffff"}}function ne(){const e=document.getElementById("kit-preview-wrap");e&&(e.innerHTML=Et(ct(),"panel"))}async function on(e,t){const n=(e.name.split(".").pop()||"png").toLowerCase(),o=`clubs/${t||"new"}-${Date.now()}.${n}`,{error:l}=await _.storage.from("assets").upload(o,e,{upsert:!0,cacheControl:"3600"});if(l)throw l;const{data:a}=_.storage.from("assets").getPublicUrl(o);return a.publicUrl}async function an(e,t,n,o){var $,E,v,p,x,w,k,L,B;const{toast:l}=o,a=document.getElementById("m-error"),m=document.getElementById("m-save"),u=($=document.getElementById("m-real"))==null?void 0:$.value.trim(),h=(E=document.getElementById("m-encoded"))==null?void 0:E.value.trim().toUpperCase(),i=(v=document.getElementById("m-country"))==null?void 0:v.value.trim().toUpperCase(),f=((x=(p=document.getElementById("m-logo-file"))==null?void 0:p.files)==null?void 0:x[0])||null,r=((w=document.getElementById("m-logo-url-current"))==null?void 0:w.value)||null,s=((k=document.getElementById("m-gen-stadium"))==null?void 0:k.checked)??!1,g=((L=document.getElementById("m-gen-squad"))==null?void 0:L.checked)??!1,d=((B=document.getElementById("m-gen-strong"))==null?void 0:B.checked)??!1,y=ct();if(!u){a.textContent="Le nom du club est requis.";return}if(!h){a.textContent="Le nom encodé est requis.";return}if(!i){a.textContent="Le pays est requis.";return}m.disabled=!0,m.textContent="Enregistrement…";let c=r;if(f)try{m.textContent="📤 Envoi du logo…",c=await on(f,e==null?void 0:e.id)}catch(A){a.textContent="Erreur upload logo : "+A.message,m.disabled=!1,m.textContent=t?"💾 Enregistrer":"✅ Créer le club";return}const b={real_name:u,encoded_name:h,country_code:i,logo_url:c,kit_style:y.style,kit_color1:y.color1,kit_color2:y.color2,kit_color3:y.color3,kit_shorts:y.shorts,kit_socks:y.socks};let I=e==null?void 0:e.id;if(t){const{error:A}=await _.from("clubs").update(b).eq("id",I);if(A){a.textContent=A.message,m.disabled=!1,m.textContent="💾 Enregistrer";return}}else{const{data:A,error:S}=await _.from("clubs").insert(b).select().single();if(S){a.textContent=S.message,m.disabled=!1,m.textContent="✅ Créer le club";return}if(I=A.id,s){m.textContent="🏟️ Création du stade…";const{data:C,error:M}=await _.from("stadium_definitions").insert({name:`Stade de ${u}`,club_id:I,country_code:i}).select().single();M?console.warn("[Stadium] Erreur def stade:",M.message):C&&await _.from("cards").insert({card_type:"stadium",stadium_id:C.id})}g&&(m.textContent="⚽ Génération des joueurs…",await lt(I,i,l,d))}l(t?"Club modifié ✅":"Club créé ✅","success"),ue(n,o)}const rn=["normal","pepite","papyte","legende"],De=["GK","DEF","MIL","ATT"],ln=[["MA","Maroc"],["FR","France"],["AR","Argentine"],["PT","Portugal"],["BR","Brésil"],["ES","Espagne"],["DE","Allemagne"],["GB","Angleterre"],["IT","Italie"],["CM","Cameroun"],["SN","Sénégal"],["NG","Nigéria"],["DK","Danemark"],["NL","Pays-Bas"],["BE","Belgique"],["CI","Côte d'Ivoire"],["AL","Albanie"],["HR","Croatie"],["RS","Serbie"],["TR","Turquie"],["MW","Malawi"]];let K={...R},le=[];async function sn(e,{toast:t}){const{data:n}=await _.from("clubs").select("id,encoded_name,logo_url,kit_style,kit_color1,kit_color2,kit_shorts,kit_socks").order("encoded_name");le=n||[],K={...R},e.innerHTML=un(le),cn(e,le,t),P()}function Ne(){var n;const e=((n=document.getElementById("cb-surname-real"))==null?void 0:n.value)||"",t=document.getElementById("cb-surname-enc");t&&(t.value=pe(e))}function dn(){var n;const e=(n=document.getElementById("cb-club"))==null?void 0:n.value,t=le.find(o=>String(o.id)===String(e));return t?{style:t.kit_style||z.style,color1:t.kit_color1||z.color1,color2:t.kit_color2||z.color2,shorts:t.kit_shorts||z.shorts,socks:t.kit_socks||z.socks}:{...z}}function P(){var s,g,d,y;const e=c=>{var b;return((b=document.getElementById(c))==null?void 0:b.value)||""},t={firstname:e("cb-firstname"),surname_real:e("cb-surname-enc")||pe(e("cb-surname-real")),country_code:e("cb-country"),club_encoded_name:((g=(s=document.getElementById("cb-club"))==null?void 0:s.selectedOptions[0])==null?void 0:g.text)||"",job:e("cb-job"),job2:e("cb-job2")||null,note_g:parseInt(e("cb-note-g"))||0,note_d:parseInt(e("cb-note-d"))||0,note_m:parseInt(e("cb-note-m"))||0,note_a:parseInt(e("cb-note-a"))||0,rarity:e("cb-rarity")},n=dn(),o=document.getElementById("cb-club"),l=((y=(d=o==null?void 0:o.selectedOptions[0])==null?void 0:d.dataset)==null?void 0:y.logo)||null,a=Ze(K,n,120),m=`data:image/svg+xml;charset=utf-8,${encodeURIComponent(a)}`,u=$t(t,{portraitUrl:m,clubLogoUrl:l,showNotes:!0}),h=document.getElementById("card-preview");h&&(h.innerHTML=u);const i=document.getElementById("avatar-preview-wrap");i&&(i.innerHTML=a);const f=e("cb-surname-real"),r=document.getElementById("encode-summary");r&&f&&(r.textContent=`${e("cb-firstname")} ${f} → ${e("cb-firstname")} ${t.surname_real}`)}function pt(){const e=t=>{var n;return((n=document.getElementById(t))==null?void 0:n.value)||""};K={skin:e("av-skin"),hair:e("av-hair"),hairColor:e("av-hairColor"),eyes:e("av-eyes"),nose:e("av-nose"),mouth:e("av-mouth"),eyebrows:e("av-eyebrows"),beard:e("av-beard")}}function cn(e,t,n){var l,a,m,u,h;e.querySelectorAll("input,select").forEach(i=>{i.addEventListener("input",P),i.addEventListener("change",P)});const o=["av-skin","av-hair","av-hairColor","av-eyes","av-nose","av-mouth","av-eyebrows","av-beard"];o.forEach(i=>{var f;(f=document.getElementById(i))==null||f.addEventListener("change",()=>{pt(),P()})}),(l=document.getElementById("cb-surname-real"))==null||l.addEventListener("input",()=>{Ne(),P()}),(a=document.getElementById("btn-encode"))==null||a.addEventListener("click",()=>{Ne(),P()}),(m=document.getElementById("cb-club"))==null||m.addEventListener("change",P),(u=document.getElementById("btn-save-player"))==null||u.addEventListener("click",()=>pn(n)),(h=document.getElementById("btn-reset"))==null||h.addEventListener("click",()=>{e.querySelectorAll("input").forEach(i=>i.value=""),e.querySelectorAll("select").forEach(i=>i.selectedIndex=0),K={...R},o.forEach(i=>{const f=i.replace("av-",""),r=document.getElementById(i);r&&(r.value=R[f]||"")}),P()})}async function pn(e){const t=m=>{var u;return((u=document.getElementById(m))==null?void 0:u.value)||""},n=t("cb-surname-real").trim(),o=t("cb-surname-enc").trim()||pe(n);if(!t("cb-firstname")||!n||!t("cb-country")||!t("cb-job")){e("Remplissez les champs obligatoires (prénom, nom, pays, poste)","error");return}pt();const l={firstname:t("cb-firstname").trim(),surname_real:n,surname_real:o,country_code:t("cb-country"),club_id:t("cb-club")||null,job:t("cb-job"),job2:t("cb-job2")||null,note_g:parseInt(t("cb-note-g"))||0,note_d:parseInt(t("cb-note-d"))||0,note_m:parseInt(t("cb-note-m"))||0,note_a:parseInt(t("cb-note-a"))||0,rarity:t("cb-rarity")||"normal",note_min:parseInt(t("cb-note-min"))||null,note_max:parseInt(t("cb-note-max"))||null,sell_price:parseInt(t("cb-price"))||0,avatar_config:K,skin:K.skin||"blanc",hair:K.hairColor||"marron",hair_length:"court"},{error:a}=await _.from("players").insert(l);if(a){e("Erreur: "+a.message,"error");return}e(`✅ Joueur "${l.firstname} ${l.surname_real}" enregistré !`,"success")}function un(e){const t=ln.map(([i,f])=>`<option value="${i}">${f} (${i})</option>`).join(""),n='<option value="">— Sélectionner un club —</option>'+e.map(i=>`<option value="${i.id}" data-logo="${i.logo_url||""}">${i.encoded_name}</option>`).join(""),o=De.map(i=>`<option value="${i}">${i}</option>`).join(""),l='<option value="">Aucun</option>'+De.map(i=>`<option value="${i}">${i}</option>`).join(""),a=rn.map(i=>`<option value="${i}">${i.charAt(0).toUpperCase()+i.slice(1)}</option>`).join(""),m=(i,f,r)=>Object.entries(f).map(([s,g])=>`<option value="${s}" ${s===(r||"")?"selected":""}>${g.label}</option>`).join(""),u=Object.keys(_t).map(i=>`<option value="${i}" ${i===R.skin?"selected":""}>${i.charAt(0).toUpperCase()+i.slice(1)}</option>`).join(""),h=Object.keys(wt).map(i=>`<option value="${i}" ${i===R.hairColor?"selected":""}>${i.charAt(0).toUpperCase()+i.slice(1)}</option>`).join("");return`
  ...
  `,a(l),document.getElementById("search-users").addEventListener("input",m=>{const u=m.target.value.toLowerCase();a(l.filter(h=>{var i;return h.pseudo.toLowerCase().includes(u)||((i=h.club_name)==null?void 0:i.toLowerCase().includes(u))}))});function a(m){document.getElementById("users-tbody").innerHTML=m.map(u=>{const h=u.mmr??1e3,i=u.mmr_deviation??350,f=u.placement_matches??0,r=Tt(h),s=u.ranked_wins??0,g=u.ranked_losses??0,d=u.ranked_draws??0,y=s+g+d,c=y>0?Math.round(s/y*100):null,b=f<10,I=i<80?"#1A6B3C":i<150?"#e67e22":"#bb2020";return`
        <tr>
          <td>
          ...
          <td style="font-size:12px">
            ...
            ${b?`<div style="font-size:10px;color:#e67e22">${f}/10 placement${f<10?` (${10-f} restants)`:""}</div>`:`<div style="font-size:10px;color:var(--gray-600)">${s}V ${d}N ${g}D${c!==null?` · ${c}% WR`:""}</div>`}
          </td>
    ...
    `).order("listed_at",{ascending:!1}).limit(100);if(l){e.innerHTML=`<p style="color:var(--danger)">${l.message}</p>`;return}const a=o||[],m=a.filter(i=>i.status==="active").length,u=a.filter(i=>i.status==="sold").length;e.innerHTML=`
    ...
    <div class="card-panel">
      <div class="table-wrap">
        <table>
          ...
          <tbody>
            ${a.map(i=>{var g,d,y,c;const f=(g=i.card)==null?void 0:g.player,r=f?`${f.firstname} ${f.surname_real}`:((d=i.card)==null?void 0:d.card_type)||"—",s={active:"#1A6B3C",sold:"#D4A017",cancelled:"#888"};return`<tr>
                <td><b>${r}</b>${f?`<div style="font-size:10px;color:var(--tile-fg-dim)">${f.rarity} · ${f.job}</div>`:""}</td>
  ...
  `,document.querySelectorAll("[data-cancel]").forEach(i=>{i.addEventListener("click",async()=>{if(!confirm("Annuler cette annonce ?"))return;const{error:f}=await _.from("market_listings").update({status:"cancelled"}).eq("id",i.dataset.cancel);f?n(f.message,"error"):(n("Annonce annulée","success"),mt(e,t))})}),(h=document.getElementById("btn-price-grid"))==null||h.addEventListener("click",()=>gn(t))}async function gn(e){await be(e)}function fn(){const e=document.getElementById("modal-overlay");if(!e||e._pgBlocker)return;const t=n=>{n.target===n.currentTarget&&n.stopImmediatePropagation()};e.addEventListener("click",t,!0),e._pgBlocker=t}function ge(){const e=document.getElementById("modal-overlay");e!=null&&e._pgBlocker&&(e.removeEventListener("click",e._pgBlocker,!0),delete e._pgBlocker)}async function be(e){var g,d;const{openModal:t,closeModal:n,toast:o}=e,{data:l,error:a}=await _.from("sell_price_configs").select("*").order("rarity").order("note_min");if(a){o(a.message,"error");return}const m=y=>mn.map(c=>`<option value="${c.value}" ${c.value===y?"selected":""}>${c.label}</option>`).join(""),u=y=>`
    <tr data-row="${y.id}">
      <td style="padding:4px 8px">
        <select data-field="rarity" data-id="${y.id}" style="width:100%;padding:6px;font-size:13px">${m(y.rarity)}</select>
      </td>
      <td style="padding:4px 8px">
    ...
    <div style="max-height:400px;overflow-y:auto;margin-bottom:14px">
      <table>
        ...
        <tbody id="pg-tbody">
          ${(l||[]).map(u).join("")||'<tr><td colspan="6" style="text-align:center;color:#aaa;padding:20px">Aucune règle définie.</td></tr>'}
        </tbody>
      </table>
    </div>
  ...
  `),fn(),(g=document.getElementById("pg-close"))==null||g.addEventListener("click",()=>{ge(),n()});function f(y){const c=y.dataset.id,b=y.dataset.field,I=b==="rarity"?y.value:Number(y.value)||0;return r(c,{[b]:I},y)}async function r(y,c,b){const I=document.querySelector(`tr[data-row="${y}"]`),$=k=>I.querySelector(`[data-field="${k}"]`),E="note_min"in c?c.note_min:Number($("note_min").value),v="note_max"in c?c.note_max:Number($("note_max").value),p="price_min"in c?c.price_min:Number($("price_min").value),x="price_max"in c?c.price_max:Number($("price_max").value);if(E>v){o("Note min doit être ≤ note max","error");return}if(p>x){o("Prix min doit être ≤ prix max","error");return}const{error:w}=await _.from("sell_price_configs").update(c).eq("id",y);if(w){o(w.message,"error");return}b&&(b.style.transition="background .3s",b.style.background="#e8f8ee",setTimeout(()=>{b.style.background=""},500))}function s(){document.querySelectorAll("#pg-tbody [data-field]").forEach(y=>{y.addEventListener("change",()=>f(y))})}s(),document.querySelectorAll("[data-del-row]").forEach(y=>{y.addEventListener("click",async()=>{confirm("Supprimer cette ligne ?")&&(await _.from("sell_price_configs").delete().eq("id",y.dataset.delRow),o("Ligne supprimée","success"),ge(),be(e))})}),(d=document.getElementById("pg-add-row"))==null||d.addEventListener("click",async()=>{var $;const y={rarity:"normal",note_min:0,note_max:20,price_min:2500,price_max:15e3},{data:c,error:b}=await _.from("sell_price_configs").insert(y).select().single();if(b){o(b.message,"error");return}const I=document.getElementById("pg-tbody");I.querySelector("td[colspan]")&&(I.innerHTML=""),I.insertAdjacentHTML("beforeend",u(c)),s(),($=document.querySelector(`[data-del-row="${c.id}"]`))==null||$.addEventListener("click",async function(){confirm("Supprimer cette ligne ?")&&(await _.from("sell_price_configs").delete().eq("id",c.id),o("Ligne supprimée","success"),ge(),be(e))}),o("Ligne ajoutée — modifie les valeurs directement","success")})}async function yn(e,{toast:t}){e.innerHTML=`
...
`}),oe("players_export.csv",l),t(`${n.length} joueurs exportés`,"success")}),document.getElementById("import-clubs").addEventListener("change",async n=>{const o=n.target.files[0];if(!o)return;const l=document.getElementById("clubs-import-result");l.innerHTML='<span style="color:var(--gray-600)">Import en cours...</span>';try{const a=await o.text(),m=Fe(a);if(m.length===0){l.innerHTML='<span style="color:#c0392b">Fichier vide</span>';return}let u=0,h=0;const i=[];for(const f of m){if(!f.real_name||!f.encoded_name||!f.country_code){h++,i.push(`Ligne ignorée (champs manquants): ${f.real_name||"?"}`);continue}const r={real_name:f.real_name,encoded_name:f.encoded_name.toUpperCase(),country_code:f.country_code.toUpperCase().slice(0,2),logo_url:f.logo_url||null},{error:s}=await _.from("clubs").upsert(r,{onConflict:"encoded_name"});s?(h++,i.push(`${f.encoded_name}: ${s.message}`)):u++}l.innerHTML=`<div style="color:var(--green)">✅ ${u} clubs importés</div>
        ${h>0?`<div style="color:#c0392b">❌ ${h} erreurs</div><div style="font-size:11px;color:var(--gray-600);max-height:100px;overflow-y:auto">${i.slice(0,10).join("<br>")}</div>`:""}`,t(`${u} clubs importés`,"success")}catch(a){l.innerHTML=`<span style="color:#c0392b">Erreur : ${a.message}</span>`}n.target.value=""}),document.getElementById("import-players").addEventListener("change",async n=>{const o=n.target.files[0];if(!o)return;const l=document.getElementById("players-import-result");l.innerHTML='<span style="color:var(--gray-600)">Import en cours...</span>';try{const a=await o.text(),m=Fe(a);if(m.length===0){l.innerHTML='<span style="color:#c0392b">Fichier vide</span>';return}const{data:u}=await _.from("clubs").select("id,encoded_name"),h={};(u||[]).forEach(b=>{h[b.encoded_name.toUpperCase()]=b.id});let i=0,f=0;const r=[],s=["GK","DEF","MIL","ATT"],g=["normal","pepite","papyte","legende"],d=["blanc","metisse","typ","noir"],y=["blond","marron","noir","chauve"],c=["rase","court","milong","long"];for(const b of m){if(!b.firstname||!b.surname_real||!b.country_code||!b.job){f++,r.push(`Ligne ignorée (champs requis manquants): ${b.firstname||"?"}`);continue}if(!s.includes(b.job)){f++,r.push(`${b.firstname}: job invalide "${b.job}"`);continue}const I=b.club_encoded_name&&h[b.club_encoded_name.toUpperCase()]||null,$={firstname:b.firstname,surname_real:b.surname_real,surname_real:b.surname_real||pe(b.surname_real),country_code:b.country_code.toUpperCase().slice(0,2),club_id:I,job:b.job,job2:s.includes(b.job2)?b.job2:null,note_g:parseInt(b.note_g)||0,note_d:parseInt(b.note_d)||0,note_m:parseInt(b.note_m)||0,note_a:parseInt(b.note_a)||0,rarity:g.includes(b.rarity)?b.rarity:"normal",note_min:b.note_min!==""&&b.note_min!=null?parseInt(b.note_min):null,note_max:b.note_max!==""&&b.note_max!=null?parseInt(b.note_max):null,skin:d.includes(b.skin)?b.skin:"blanc",hair:y.includes(b.hair)?b.hair:"noir",hair_length:c.includes(b.hair_length)?b.hair_length:"court",sell_price:parseInt(b.sell_price)||0},{error:E}=await _.from("players").insert($);E?(f++,r.push(`${b.firstname} ${b.surname_real}: ${E.message}`)):i++}l.innerHTML=`<div style="color:var(--green)">✅ ${i} joueurs importés</div>
...
`).replace(/\r/g,`
`);const t=e.split(`
`).filter(m=>m.trim());if(t.length<2)return[];const n=t[0],o=n.split(";").length>n.split(",").length?";":",",l=Oe(n,o).map(m=>m.trim().replace(/^\uFEFF/,"").toLowerCase()),a=[];for(let m=1;m<t.length;m++){if(!t[m].trim())continue;const u=Oe(t[m],o),h={};l.forEach((i,f)=>{h[i]=(u[f]||"").trim()}),!Object.values(h).every(i=>!i)&&a.push(h)}return a}function Oe(e,t=","){const n=[];let o="",l=!1;for(let a=0;a<e.length;a++){const m=e[a];l?m==='"'?e[a+1]==='"'?(o+='"',a++):l=!1:o+=m:m==='"'?l=!0:m===t?(n.push(o),o=""):o+=m}return n.push(o),n}const bn={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};async function vn(e,{toast:t}){const n=Object.keys(me);let o=n[0];const{data:l}=await _.from("formation_links_overrides").select("formation, links"),a={};(l||[]).forEach(f=>{a[f.formation]=f.links});let m=new Set;function u(f,r){return[f,r].sort().join("-")}function h(f){const r=a[f]||me[f]||[];m=new Set(r.map(([s,g])=>u(s,g)))}h(o),e.innerHTML=`
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;gap:12px;flex-wrap:wrap">
      <div style="display:flex;align-items:center;gap:10px">
        ...
        <select id="formation-select" style="padding:6px 10px;font-size:14px;border-radius:8px;border:1.5px solid #ccc">
          ${n.map(f=>`<option value="${f}" ${f===o?"selected":""}>${f}</option>`).join("")}
        </select>
  ...
  `;function i(){const f=Mt[o]||{},r=ze(o),s=320,g=400,d=22;function y(E){const v=f[E];return v?{x:v.x*s,y:v.y*g}:null}let c=`<svg width="${s}" height="${g}" viewBox="0 0 ${s} ${g}" xmlns="http://www.w3.org/2000/svg" style="display:block">`;r.forEach(([E,v],p)=>{const x=y(E),w=y(v);if(!x||!w)return;const k=u(E,v),L=m.has(k),B=L?"#3b82f6":"#999",A=L?.95:.35,S=L?4:3;c+=`<line x1="${x.x}" y1="${x.y}" x2="${w.x}" y2="${w.y}"
        ...
        opacity="${A}" pointer-events="none" data-line-key="${k}"/>`});for(const E of Object.keys(f)){const v=y(E);if(!v)continue;const p=E.replace(/\d+/,""),x=bn[p]||"#555";c+=`<circle cx="${v.x}" cy="${v.y}" r="${d}" fill="${x}" stroke="rgba(255,255,255,0.7)" stroke-width="2" pointer-events="none"/>`,c+=`<text x="${v.x}" y="${v.y+4}" text-anchor="middle" font-size="10" font-weight="900" fill="white" font-family="Arial Black,Arial" pointer-events="none">${E}</text>`}c+="</svg>",document.getElementById("field-wrap").innerHTML=c;const b=document.getElementById("links-list");b.innerHTML=r.map(([E,v])=>{const p=u(E,v),x=m.has(p);return`
        <button class="link-toggle" data-key="${p}" style="
        ...
        </button>`}).join("");const I=e.querySelector('div[style*="font-weight:700;font-size:13px;margin-bottom:8px"]');I&&(I.textContent=`Liens (${m.size}/${r.length} actifs)`);function $(E){m.has(E)?m.delete(E):m.add(E),i()}e.querySelectorAll(".link-hit").forEach(E=>{E.addEventListener("click",()=>$(E.dataset.key))}),e.querySelectorAll(".link-toggle").forEach(E=>{E.addEventListener("click",()=>$(E.dataset.key))})}i(),document.getElementById("formation-select").addEventListener("change",f=>{o=f.target.value,h(o),i()}),document.getElementById("reset-btn").addEventListener("click",()=>{const f=me[o]||[];m=new Set(f.map(([r,s])=>u(r,s))),i(),t("Liens réinitialisés aux valeurs par défaut (non sauvegardé)","info")}),document.getElementById("save-btn").addEventListener("click",async()=>{const r=ze(o).filter(([g,d])=>m.has(u(g,d))),{error:s}=await _.from("formation_links_overrides").upsert({formation:o,links:r,updated_at:new Date().toISOString()});if(s){t(s.message,"error");return}a[o]=r,t(`Liens enregistrés pour ${o} (${r.length} actifs)`,"success")})}const xn=[{value:"player",label:"Joueur"},{value:"formation",label:"Formation"},{value:"game_changer",label:"Game Changer"},{value:"ultra_game_changer",label:"Ultra Game Changer"},{value:"game_helper",label:"Game Helper"},{value:"stadium",label:"🏟️ Stade"}],hn=[{value:"",label:"Toutes"},{value:"normal",label:"Normal"},{value:"pepite",label:"Pépite"},{value:"papyte",label:"Papyte"},{value:"legende",label:"Légende"}],En=[{value:"credits",label:"Crédits"},{value:"pub",label:"Publicité"},{value:"free",label:"Gratuit"}];async function _n(e){e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">Chargement...</div>';const{data:t}=await _.from("booster_configs").select("*").order("sort_order");let n=null,o=[],l=null;const a=()=>window.innerWidth<700;async function m(){if(!n){o=[];return}if(n!==l){const{data:r}=await _.from("booster_drop_rates").select("*").eq("booster_id",n).order("sort_order");o=r||[],l=n}}function u(){return`
    ...
    </div>`}function h(){const r=n?(t||[]).find(d=>d.id===n):null;if(!r)return`<div style="display:flex;align-items:center;justify-content:center;flex:1;color:#bbb;font-size:14px;padding:40px;text-align:center">
      ← Sélectionnez un booster ou créez-en un nouveau
    </div>`;const s=o.reduce((d,y)=>d+Number(y.percentage||0),0),g=Math.abs(s-100)<.1;return`
    <div id="booster-editor" style="flex:1;overflow-y:auto;background:#f8f8f8;padding:14px">
      ...
      <div style="background:#fff;border-radius:10px;padding:14px;margin-bottom:14px">
        ...
        ${o.length===0?'<div style="text-align:center;padding:16px;color:#aaa;font-size:13px">Aucun taux. Cliquez + Ligne.</div>':o.map((d,y)=>`
          <div style="display:grid;grid-template-columns:2fr 1.5fr 50px 50px 60px 32px;gap:4px;align-items:center;margin-bottom:6px;padding:6px;background:#f9f9f9;border-radius:6px" data-rate-idx="${y}">
            ...
            <input class="rate-note-min" data-idx="${y}" type="number" min="0" max="20" value="${d.note_min||""}" placeholder="min" style="padding:6px 4px;border:1px solid #ddd;border-radius:4px;text-align:center;font-size:12px;width:100%">
            <input class="rate-note-max" data-idx="${y}" type="number" min="0" max="20" value="${d.note_max||""}" placeholder="max" style="padding:6px 4px;border:1px solid #ddd;border-radius:4px;text-align:center;font-size:12px;width:100%">
            <input class="rate-pct" data-idx="${y}" type="number" min="0.1" max="100" step="0.1" value="${d.percentage}" style="padding:6px 4px;border:1px solid #ddd;border-radius:4px;text-align:center;font-size:12px;width:100%">
    ...
    </div>`}async function i(r=!1){r||await m();const s=!n&&a(),g=n&&a();e.innerHTML=`
    <div style="display:flex;height:calc(100vh - 56px);overflow:hidden">
      ${!a()||s||!n?u():""}
      ${!a()||g?h():""}
    </div>`,f()}function f(){var g,d,y,c,b,I,$;const r=E=>e.querySelector(E);e.querySelectorAll(".booster-row").forEach(E=>{E.addEventListener("click",v=>{v.target.closest(".btn-delete")||(n=E.dataset.id,l=null,i())})}),(g=r("#btn-back"))==null||g.addEventListener("click",()=>{n=null,i()}),(d=r("#btn-new"))==null||d.addEventListener("click",async()=>{const E=prompt("Nom du nouveau booster :");if(!(E!=null&&E.trim()))return;const{data:v,error:p}=await _.from("booster_configs").insert({name:E.trim(),price_type:"credits",price_credits:1e3,card_count:5}).select().single();if(p){alert(p.message);return}t.push(v),n=v.id,l=null,i()}),e.querySelectorAll(".btn-delete").forEach(E=>{E.addEventListener("click",async v=>{if(v.stopPropagation(),!confirm("Supprimer ce booster ?"))return;await _.from("booster_configs").delete().eq("id",E.dataset.id);const p=t.findIndex(x=>x.id===E.dataset.id);p>-1&&t.splice(p,1),n===E.dataset.id&&(n=null,l=null),i()})}),(y=r("#btn-cancel"))==null||y.addEventListener("click",()=>{n=null,i()}),(c=r("#f-price-type"))==null||c.addEventListener("change",E=>{const v=r("#credits-field");v&&(v.style.opacity=E.target.value!=="credits"?"0.4":"1")}),(b=r("#btn-pick-icon"))==null||b.addEventListener("click",async()=>{var v;const E=r("#icon-picker-grid");if(E){if(E.style.display!=="none"){E.style.display="none";return}E.innerHTML='<div style="padding:10px;text-align:center;color:#888;font-size:12px">⏳ Chargement...</div>',E.style.display="block";try{const x=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),w=Array.isArray(x)?x.filter(L=>L.name.startsWith("booster")&&/\.(png|jpg|svg|webp)$/i.test(L.name)):[];if(!w.length){E.innerHTML='<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Aucune icône "booster*" trouvée.</div>';return}const k=((v=r("#f-image-url"))==null?void 0:v.value)||"";E.innerHTML=`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(72px,1fr));gap:8px">
          ${w.map(L=>`
        ...
        </div>`,E.querySelectorAll(".icon-pick-item").forEach(L=>{L.addEventListener("click",()=>{const B=r("#f-image-url");B&&(B.value=L.dataset.name),E.style.display="none"})})}catch(p){E.innerHTML=`<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Erreur API GitHub : ${p.message}</div>`}}});function s(){e.querySelectorAll("[data-rate-idx]").forEach(E=>{var p,x,w,k,L;const v=Number(E.dataset.rateIdx);v>=0&&v<o.length&&(o[v].card_type=((p=E.querySelector(".rate-type"))==null?void 0:p.value)||"player",o[v].rarity=((x=E.querySelector(".rate-rarity"))==null?void 0:x.value)||null,o[v].note_min=Number((w=E.querySelector(".rate-note-min"))==null?void 0:w.value)||null,o[v].note_max=Number((k=E.querySelector(".rate-note-max"))==null?void 0:k.value)||null,o[v].percentage=Number((L=E.querySelector(".rate-pct"))==null?void 0:L.value)||0)})}(I=r("#btn-add-rate"))==null||I.addEventListener("click",()=>{s(),o.push({id:null,booster_id:n,card_type:"player",rarity:"normal",note_min:null,note_max:null,percentage:10,sort_order:o.length}),i(!0)}),e.querySelectorAll(".btn-del-rate").forEach(E=>{E.addEventListener("click",()=>{s(),o.splice(Number(E.dataset.idx),1),i(!0)})}),($=r("#btn-save"))==null||$.addEventListener("click",async()=>{var k,L,B,A,S,C,M,F,O,U,N,j,ie;const E=(t||[]).find(T=>T.id===n);if(!E)return;const v=[];e.querySelectorAll("[data-rate-idx]").forEach(T=>{var Z,Q,X,ee,te;const V=Number(T.dataset.rateIdx);v.push({booster_id:n,card_type:((Z=T.querySelector(".rate-type"))==null?void 0:Z.value)||"player",rarity:((Q=T.querySelector(".rate-rarity"))==null?void 0:Q.value)||null,note_min:Number((X=T.querySelector(".rate-note-min"))==null?void 0:X.value)||null,note_max:Number((ee=T.querySelector(".rate-note-max"))==null?void 0:ee.value)||null,percentage:Number((te=T.querySelector(".rate-pct"))==null?void 0:te.value)||0,sort_order:V});const D=v[v.length-1];D.rarity||(D.rarity=null),D.note_min||(D.note_min=null),D.note_max||(D.note_max=null)});const p=v.reduce((T,V)=>T+V.percentage,0);if(v.length&&Math.abs(p-100)>.5){alert(`La somme doit faire 100% (actuellement ${p.toFixed(1)}%)`);return}const x={name:((L=(k=r("#f-name"))==null?void 0:k.value)==null?void 0:L.trim())||E.name,image_url:((A=(B=r("#f-image-url"))==null?void 0:B.value)==null?void 0:A.trim())||null,price_type:(S=r("#f-price-type"))==null?void 0:S.value,price_credits:Number((C=r("#f-price-credits"))==null?void 0:C.value)||0,card_count:Number((M=r("#f-card-count"))==null?void 0:M.value)||5,is_active:((F=r("#f-active"))==null?void 0:F.checked)??E.is_active,allow_duplicates:((O=r("#f-allow-dup"))==null?void 0:O.checked)??!0,sort_order:Number((U=r("#f-sort"))==null?void 0:U.value)||0,max_per_user:(N=r("#f-max-per-user"))!=null&&N.value?Number(r("#f-max-per-user").value):null,available_from:((j=r("#f-available-from"))==null?void 0:j.value)||null,available_until:((ie=r("#f-available-until"))==null?void 0:ie.value)||null},{error:w}=await _.from("booster_configs").update(x).eq("id",n);if(w){alert(w.message);return}if(Object.assign(E,x),await _.from("booster_drop_rates").delete().eq("booster_id",n),v.length){const{error:T}=await _.from("booster_drop_rates").insert(v);if(T){alert(T.message);return}}o=v,l=n,alert("✅ Booster enregistré !"),i(!0)})}i()}const gt=[{value:"normal",label:"Normal",color:"#888"},{value:"pepite",label:"Pépite",color:"#D4A017"},{value:"papyte",label:"Papyte",color:"#909090"},{value:"legende",label:"Légende",color:"#7a28b8"}];async function wn(e){await ke(e)}async function ke(e){const{data:t,error:n}=await _.from("sell_price_configs").select("*").order("rarity").order("note_min");if(n){e.innerHTML=`<div style="padding:24px;color:red">Erreur : ${n.message}</div>`;return}e.innerHTML=`
    ...
    </tr>`}function Ue(e){document.getElementById("config-form").style.display="block",document.getElementById("form-title").textContent=e?"Modifier la règle":"Nouvelle règle",document.getElementById("form-id").value=(e==null?void 0:e.id)||"",document.getElementById("form-rarity").value=(e==null?void 0:e.rarity)||"normal",document.getElementById("form-price").value=(e==null?void 0:e.price)||1e3,document.getElementById("form-note-min").value=(e==null?void 0:e.note_min)??1,document.getElementById("form-note-max").value=(e==null?void 0:e.note_max)??10,document.getElementById("config-form").scrollIntoView({behavior:"smooth"})}async function $n(e){const t=document.getElementById("form-id").value,n=document.getElementById("form-rarity").value,o=parseInt(document.getElementById("form-price").value)||0,l=parseInt(document.getElementById("form-note-min").value)||1,a=parseInt(document.getElementById("form-note-max").value)||10;if(l>a){alert("Note min doit être ≤ note max");return}const m={rarity:n,price:o,note_min:l,note_max:a,updated_at:new Date().toISOString()};let u;if(t?{error:u}=await _.from("sell_price_configs").update(m).eq("id",t):{error:u}=await _.from("sell_price_configs").insert(m),u){alert("Erreur : "+u.message);return}document.getElementById("config-form").style.display="none",await ke(e)}async function kn(e){await Le(e)}async function Le(e){const{data:t}=await _.from("patch_notes").select("*").order("published_at",{ascending:!1});e.innerHTML=`
    ...
    </div>`}function Ge(e){const t=document.getElementById("article-form");t.style.display="block",document.getElementById("form-title").textContent=e?"Modifier l'article":"Nouvel article",document.getElementById("form-id").value=(e==null?void 0:e.id)||"",document.getElementById("form-art-title").value=(e==null?void 0:e.title)||"",document.getElementById("form-art-desc").value=(e==null?void 0:e.description)||"",document.getElementById("form-art-img-current").value=(e==null?void 0:e.image_url)||"",document.getElementById("form-art-img-file").value="";const n=document.getElementById("form-art-img-preview");n.innerHTML=e!=null&&e.image_url?`<img src="${e.image_url}" style="width:100%;height:100%;object-fit:cover">`:'<span style="font-size:20px;opacity:.4">📰</span>',document.getElementById("form-art-img-file").onchange=a=>{var h;const m=(h=a.target.files)==null?void 0:h[0];if(!m)return;const u=new FileReader;u.onload=()=>{n.innerHTML=`<img src="${u.result}" style="width:100%;height:100%;object-fit:cover">`},u.readAsDataURL(m)},document.getElementById("form-art-pub").checked=e?e.is_published:!0;const o=new Date((e==null?void 0:e.published_at)||Date.now()),l=new Date(o.getTime()-o.getTimezoneOffset()*6e4).toISOString().slice(0,16);document.getElementById("form-art-date").value=l,t.scrollIntoView({behavior:"smooth"})}async function Bn(e){const t=(e.name.split(".").pop()||"png").toLowerCase(),n=`journal/${Date.now()}.${t}`,{error:o}=await _.storage.from("assets").upload(n,e,{upsert:!0,cacheControl:"3600"});if(o)throw o;const{data:l}=_.storage.from("assets").getPublicUrl(n);return l.publicUrl}async function An(e){var g;const t=document.getElementById("form-id").value,n=document.getElementById("form-art-title").value.trim(),o=document.getElementById("form-art-desc").value.trim(),l=((g=document.getElementById("form-art-img-file").files)==null?void 0:g[0])||null,a=document.getElementById("form-art-img-current").value||null,m=document.getElementById("form-art-pub").checked,u=document.getElementById("form-art-date").value,h=document.getElementById("form-save");if(!n){alert("Le titre est obligatoire.");return}if(!o){alert("La description est obligatoire.");return}let i=a;if(l)try{h&&(h.textContent="📤 Envoi de l'image…"),i=await Bn(l)}catch(d){alert("Erreur upload image : "+d.message),h&&(h.textContent="💾 Enregistrer");return}const f=u?new Date(u).toISOString():new Date().toISOString(),r={title:n,description:o,image_url:i,is_published:m,published_at:f};let s;if(t?{error:s}=await _.from("patch_notes").update(r).eq("id",t):{error:s}=await _.from("patch_notes").insert(r),s){alert("Erreur : "+s.message);return}document.getElementById("article-form").style.display="none",await Le(e)}async function Sn(e){await W(e)}async function W(e){var f,r,s,g,d,y,c,b,I,$,E,v;const{data:t}=await _.from("tutorial_steps").select("*").order("step_order");e.innerHTML=`
    <div style="display:flex;gap:16px;height:calc(100vh - 56px);overflow:hidden;padding:16px">
      ...
      <div style="width:420px;flex-shrink:0;overflow-y:auto">
        ...
        <div style="margin-bottom:16px">
          ...
          <div id="ts-toolbar" style="display:flex;flex-wrap:wrap;gap:4px;padding:6px 8px;background:#f5f5f5;border:1.5px solid #ddd;border-bottom:none;border-radius:8px 8px 0 0">
            ${[["B","bold","<strong>Gras</strong>"],["I","italic","<em>Italique</em>"],["U","underline","Souligné"],["¶","insertParagraph","Paragraphe"],["•","insertUnorderedList","Liste à puces"],["1.","insertOrderedList","Liste numérotée"],["H2","formatBlock:h2","Titre H2"],["H3","formatBlock:h3","Titre H3"],["—","insertHorizontalRule","Séparateur"]].map(([p,x,w])=>`<button type="button" data-cmd="${x}" title="${w}" style="padding:4px 8px;border:1px solid #ddd;border-radius:4px;background:#fff;font-size:12px;font-weight:700;cursor:pointer;line-height:1;min-width:28px">${p}</button>`).join("")}
      ...
      <div style="flex:1;overflow-y:auto">
        ...
        <div id="ts-list" style="display:flex;flex-direction:column;gap:10px">
          ${(t||[]).length===0?`
            ...
            </div>`:(t||[]).map(p=>Cn(p)).join("")}
        </div>
      </div><!-- /colonne droite -->
    </div>`,(f=document.getElementById("ts-add"))==null||f.addEventListener("click",()=>He(null,(t==null?void 0:t.length)||0)),(r=document.getElementById("ts-reset"))==null||r.addEventListener("click",async()=>{if(!confirm("Remettre tutorial_done = false pour TOUS les joueurs ? Ils reverront le tutoriel à leur prochaine connexion."))return;const{error:p}=await _.from("users").update({tutorial_done:!1}).neq("id","00000000-0000-0000-0000-000000000000");alert(p?"Erreur : "+p.message:"✅ Tutorial_done réinitialisé pour tous les joueurs.")}),(s=document.getElementById("ts-cancel"))==null||s.addEventListener("click",ft),(g=document.getElementById("ts-save"))==null||g.addEventListener("click",()=>Mn(e)),(d=document.getElementById("ts-preview-btn"))==null||d.addEventListener("click",Tn);const n=document.getElementById("ts-editor"),o=()=>{const p=document.getElementById("ts-content");p&&n&&(p.value=n.innerHTML)};n==null||n.addEventListener("input",o),n==null||n.addEventListener("blur",o);const l=()=>{document.activeElement!==n&&(n==null||n.focus())};(y=document.getElementById("ts-toolbar"))==null||y.querySelectorAll("[data-cmd]").forEach(p=>{p.addEventListener("mousedown",x=>{x.preventDefault(),l();const w=p.dataset.cmd;w.startsWith("formatBlock:")?document.execCommand("formatBlock",!1,w.split(":")[1]):document.execCommand(w,!1,null),o()})}),(c=document.getElementById("ts-insert-color"))==null||c.addEventListener("mousedown",p=>{p.preventDefault(),l();const x=prompt("Couleur hex (ex: #D4A017 pour or, #1A6B3C pour vert, #cc2222 pour rouge) :");x&&(document.execCommand("foreColor",!1,x),o())}),(b=document.getElementById("ts-clear-format"))==null||b.addEventListener("mousedown",p=>{p.preventDefault(),l(),document.execCommand("removeFormat",!1,null),o()}),n==null||n.addEventListener("focus",()=>{n&&(n.style.borderColor="#1A6B3C")}),n==null||n.addEventListener("blur",()=>{n&&(n.style.borderColor="#ddd")});const a=document.getElementById("ts-image"),m=document.getElementById("ts-img-preview"),u=document.getElementById("ts-img-preview-el"),h=document.getElementById("ts-img-picker-grid"),i=()=>{var x;const p=(x=a==null?void 0:a.value)==null?void 0:x.trim();if(p){const w="/";u.src=`${w}icons/${p}`,m.style.display="block"}else m.style.display="none"};a==null||a.addEventListener("input",i),(I=document.getElementById("ts-img-clear"))==null||I.addEventListener("click",()=>{a&&(a.value=""),m.style.display="none",h.style.display="none"}),($=document.getElementById("ts-img-pick"))==null||$.addEventListener("click",async()=>{if(h.style.display!=="none"){h.style.display="none";return}h.innerHTML='<div style="color:#aaa;font-size:12px">Chargement…</div>',h.style.display="block";try{const x=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),w=Array.isArray(x)?x.filter(L=>L.name.startsWith("tuto_")):[];if(!w.length){h.innerHTML='<div style="color:#aaa;font-size:12px">Aucun fichier tuto_* trouvé dans /icons/.<br>Uploade des images nommées tuto_xxx.png dans public/icons/</div>';return}const k="/";h.innerHTML='<div style="display:flex;flex-wrap:wrap;gap:8px">'+w.map(L=>`
          ...
          </div>`).join("")+"</div>",h.querySelectorAll("[data-pick]").forEach(L=>{L.addEventListener("click",()=>{a&&(a.value=L.dataset.pick),i(),h.style.display="none"})})}catch(p){h.innerHTML='<div style="color:#cc2222;font-size:12px">Erreur chargement : '+p.message+"</div>"}}),(E=document.getElementById("ts-color"))==null||E.addEventListener("input",p=>{const x=document.getElementById("ts-color-hex");x&&(x.value=p.target.value)}),(v=document.getElementById("ts-color-hex"))==null||v.addEventListener("input",p=>{const x=p.target.value;if(/^#[0-9a-fA-F]{6}$/.test(x)){const w=document.getElementById("ts-color");w&&(w.value=x)}}),e.querySelectorAll("[data-edit-row]").forEach(p=>{const x=(t||[]).find(w=>w.id===p.dataset.editRow);x&&p.addEventListener("click",()=>He(x))}),e.querySelectorAll("[data-delete]").forEach(p=>{p.addEventListener("click",async()=>{confirm("Supprimer cette étape ?")&&(await _.from("tutorial_steps").delete().eq("id",p.dataset.delete),W(e))})}),e.querySelectorAll("[data-toggle]").forEach(p=>{p.addEventListener("click",async()=>{const x=(t||[]).find(w=>w.id===p.dataset.toggle);x&&(await _.from("tutorial_steps").update({is_active:!x.is_active}).eq("id",x.id),W(e))})}),e.querySelectorAll("[data-up]").forEach(p=>{p.addEventListener("click",async()=>{const x=t||[],w=x.findIndex(k=>k.id===p.dataset.up);w<=0||(await Promise.all([_.from("tutorial_steps").update({step_order:x[w-1].step_order}).eq("id",x[w].id),_.from("tutorial_steps").update({step_order:x[w].step_order}).eq("id",x[w-1].id)]),W(e))})}),e.querySelectorAll("[data-down]").forEach(p=>{p.addEventListener("click",async()=>{const x=t||[],w=x.findIndex(k=>k.id===p.dataset.down);w<0||w>=x.length-1||(await Promise.all([_.from("tutorial_steps").update({step_order:x[w+1].step_order}).eq("id",x[w].id),_.from("tutorial_steps").update({step_order:x[w].step_order}).eq("id",x[w+1].id)]),W(e))})})}function Cn(e){return`
  <div data-edit-row="${e.id}" style="background:#fff;border-radius:10px;padding:12px 16px;box-shadow:0 1px 4px rgba(0,0,0,0.08);display:flex;align-items:center;gap:12px;cursor:pointer">
  ...
  </div>`}function He(e,t=0){document.getElementById("ts-form-empty").style.display="none",document.getElementById("ts-form-content").style.display="block",document.getElementById("ts-form-title").textContent=e?"Modifier l'étape":"Nouvelle étape",document.getElementById("ts-id").value=(e==null?void 0:e.id)||"",document.getElementById("ts-emoji").value=(e==null?void 0:e.emoji)||"⚽",document.getElementById("ts-title").value=(e==null?void 0:e.title)||"",document.getElementById("ts-content").value=(e==null?void 0:e.content)||"";const n=document.getElementById("ts-editor");n&&(n.innerHTML=(e==null?void 0:e.content)||"<p>Texte de l'étape…</p>"),document.getElementById("ts-order").value=(e==null?void 0:e.step_order)??t,document.getElementById("ts-active").checked=e?e.is_active:!0;const o=(e==null?void 0:e.color)||"#1A6B3C";document.getElementById("ts-color").value=o,document.getElementById("ts-color-hex").value=o;const l=(e==null?void 0:e.image_url)||"",a=document.getElementById("ts-image");a&&(a.value=l);const m=document.getElementById("ts-img-preview"),u=document.getElementById("ts-img-preview-el");if(l&&m&&u){const h="/";u.src=`${h}icons/${l}`,m.style.display="block"}else m&&(m.style.display="none");document.getElementById("ts-img-picker-grid").style.display="none",document.getElementById("ts-preview-area").innerHTML="",document.getElementById("ts-form").scrollIntoView({behavior:"smooth"})}function ft(){document.getElementById("ts-form-content").style.display="none",document.getElementById("ts-form-empty").style.display="block",document.getElementById("ts-preview-area").innerHTML=""}function Tn(){var i,f;const e=document.getElementById("ts-emoji").value||"⚽",t=document.getElementById("ts-title").value||"Titre",n=document.getElementById("ts-editor"),o=n?n.innerHTML:document.getElementById("ts-content").value||"",l=document.getElementById("ts-color-hex").value||"#1A6B3C",a=(f=(i=document.getElementById("ts-image"))==null?void 0:i.value)==null?void 0:f.trim(),u=a?`<div style="padding:0 20px 12px;text-align:center"><img src="/icons/${a}" style="max-height:140px;max-width:100%;border-radius:10px;object-fit:contain"></div>`:"",h=document.getElementById("ts-preview-area");h.innerHTML=`
    ...
    </div>`,h.querySelectorAll("ul,ol").forEach(r=>{r.style.paddingLeft="20px",r.style.margin="6px 0"}),h.querySelectorAll("li").forEach(r=>{r.style.marginBottom="4px"}),h.querySelectorAll("p").forEach(r=>{r.style.marginBottom="8px"})}async function Mn(e){var r,s;const t=document.getElementById("ts-id").value,n=document.getElementById("ts-emoji").value.trim()||"⚽",o=document.getElementById("ts-title").value.trim(),l=document.getElementById("ts-editor"),a=(l?l.innerHTML:document.getElementById("ts-content").value).trim(),m=document.getElementById("ts-color-hex").value.trim()||"#1A6B3C",u=parseInt(document.getElementById("ts-order").value)||0,h=document.getElementById("ts-active").checked;if(!o||!a){alert("Titre et contenu sont obligatoires.");return}const i={emoji:n,title:o,content:a,color:m,step_order:u,is_active:h,image_url:((s=(r=document.getElementById("ts-image"))==null?void 0:r.value)==null?void 0:s.trim())||null};let f;if(t?{error:f}=await _.from("tutorial_steps").update(i).eq("id",t):{error:f}=await _.from("tutorial_steps").insert(i),f){alert("Erreur : "+f.message);return}ft(),W(e)}const yt="/",zn=[["MA","Maroc"],["FR","France"],["AR","Argentine"],["PT","Portugal"],["BR","Brésil"],["ES","Espagne"],["DE","Allemagne"],["GB","Angleterre"],["IT","Italie"],["CM","Cameroun"],["SN","Sénégal"],["NG","Nigéria"],["DK","Danemark"],["NL","Pays-Bas"],["BE","Belgique"],["CI","Côte d'Ivoire"],["AL","Albanie"],["HR","Croatie"],["RS","Serbie"],["TR","Turquie"],["MW","Malawi"],["GH","Ghana"],["ZA","Afrique du Sud"],["EG","Égypte"],["US","États-Unis"],["MX","Mexique"],["CO","Colombie"],["UY","Uruguay"],["CL","Chili"],["JP","Japon"],["KR","Corée du Sud"],["CN","Chine"],["AU","Australie"],["SW","Suisse"],["SE","Suède"],["NO","Norvège"],["PL","Pologne"]];async function Rn(e,t){await ve(e,t)}async function ve(e,t){var l;const[{data:n},{data:o}]=await Promise.all([_.from("stadium_definitions").select("*, club:clubs(encoded_name, logo_url)").order("name"),_.from("clubs").select("id, encoded_name, logo_url").order("encoded_name")]);e.innerHTML=`
      ...
      <div style="flex:1;min-width:220px;display:flex;flex-direction:column;gap:12px">
        ...
        <div>
          ...
          <select id="st-country">
            ...
            ${zn.map(([g,d])=>`<option value="${g}" ${(e==null?void 0:e.country_code)===g?"selected":""}>${d} (${g})</option>`).join("")}
          </select>
        </div>
        <div>
  ...
  `;l(e?`Modifier : ${e.name}`:"Nouveau stade",u,`
    <button id="st-cancel" class="btn btn-ghost">Annuler</button>
    <button id="st-save" class="btn btn-primary">Enregistrer</button>
  `);const i=()=>{var x,w,k,L,B,A,S;const g=((x=document.getElementById("st-name"))==null?void 0:x.value)||"NOM DU STADE",d=(k=(w=document.getElementById("st-image"))==null?void 0:w.value)==null?void 0:k.trim(),y=(B=(L=document.getElementById("st-country"))==null?void 0:L.value)==null?void 0:B.trim(),c=document.getElementById("st-club"),b=(c==null?void 0:c.selectedIndex)||0,I=c&&b>0?c.options[b].text:"",$=((S=(A=c==null?void 0:c.options[b])==null?void 0:A.getAttribute)==null?void 0:S.call(A,"data-logo"))||"";let E=null;d?E=d.startsWith("http")?d:`${yt}icons/${d}`:$?E=$:y&&(E=`https://flagsapi.com/${y.toLowerCase().slice(0,2).toUpperCase()}/flat/64.png`);const v=I||y||"—",p=document.getElementById("st-preview-card");p&&(p.innerHTML=Qe(g,E,`${v}<br>+10 ⭐`,{width:140}))};i(),["st-name","st-club","st-country","st-image"].forEach(g=>{var d,y;(d=document.getElementById(g))==null||d.addEventListener("input",i),(y=document.getElementById(g))==null||y.addEventListener("change",i)}),(f=document.getElementById("st-cancel"))==null||f.addEventListener("click",()=>a()),(r=document.getElementById("st-save"))==null||r.addEventListener("click",async()=>{const g=document.getElementById("st-name").value.trim(),d=document.getElementById("st-club").value||null,y=document.getElementById("st-country").value.trim().toUpperCase()||null,c=document.getElementById("st-image").value.trim()||null;if(!g){m("Le nom est obligatoire","error");return}const b={name:g,club_id:d,country_code:y,image_url:c},{error:I}=e?await _.from("stadium_definitions").update(b).eq("id",e.id):await _.from("stadium_definitions").insert(b);if(I){m("Erreur : "+I.message,"error");return}m(e?"Stade modifié ✅":"Stade créé ✅","success"),a(),ve(n,o)}),(s=document.getElementById("st-delete"))==null||s.addEventListener("click",async()=>{confirm("Supprimer ce stade ?")&&(await _.from("stadium_definitions").delete().eq("id",e.id),m("Stade supprimé","success"),a(),ve(n,o))})}const H=[{value:"game_changer",label:"⚡ Game Changer",border:"#7a28b8",bg:"linear-gradient(135deg,#4a0080,#7a28b8)"},{value:"ultra_game_changer",label:"💎 Ultra Game Changer",border:"#00bcd4",bg:"linear-gradient(135deg,#006080,#00bcd4)"}],Dn=[{value:"purple",label:"Violet",hex:"#7a28b8"},{value:"light_blue",label:"Bleu clair",hex:"#00bcd4"}],G=[{value:"BOOST_STAT",label:"⚡ Boost stats (+N)",hasValue:!0,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"DEBUFF_STAT",label:"💀 Debuff stats (-N)",hasValue:!0,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"GRAY_PLAYER",label:"❄️ Griser un joueur",hasValue:!1,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"REVIVE_PLAYER",label:"💫 Ressusciter un joueur",hasValue:!1,hasTarget:!1,hasCount:!0,hasRoles:!1},{value:"REMOVE_GOAL",label:"🚫 Retirer un but adverse",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1},{value:"ADD_GOAL_DRAW",label:"⚽ +1 but si duel nul",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1},{value:"ADD_SUB",label:"🔄 Ajouter un remplacement",hasValue:!1,hasTarget:!1,hasCount:!0,hasRoles:!1},{value:"CUSTOM",label:"🛠️ Effet custom (hardcodé)",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1}],Ke=["GK","DEF","MIL","ATT"];async function Nn(e,t){await Be(e,t)}async function Be(e,t){var o;const{data:n}=await _.from("gc_definitions").select("*").order("sort_order").order("created_at");e.innerHTML=`
    <div style="padding:20px;max-width:1100px;margin:0 auto">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px">
  ...
  </div>`}function Je(e,t,n){var s,g,d,y;const{openModal:o,closeModal:l,toast:a}=n,m=!e,u=e||{name:"",gc_type:"game_changer",color:"purple",effect:"",image_url:"",sort_order:0,is_active:!0,effect_type:"BOOST_STAT",effect_params:{}},h=(()=>{const c=u.effect_params||{},b=G.find(I=>I.value===(u.effect_type||"BOOST_STAT"))||G[0];return`
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
      ${b.hasValue?`<div>
    ...
    <div style="display:flex;justify-content:center;margin-bottom:16px;padding:20px;background:#1a1a2e;border-radius:12px">
      <div id="gc-modal-preview">${fe(u.name||"?",u.image_url?`/icons/${u.image_url}`:null,(H.find(c=>c.value===u.gc_type)||H[0]).label.split(" ")[0],u.effect||"",{width:150})}</div>
    </div>
    ...
    <div style="display:flex;flex-direction:column;gap:12px">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
        <div>
          <label>TYPE</label>
          <select id="gc-type">
            ${H.map(c=>`<option value="${c.value}" ${u.gc_type===c.value?"selected":""}>${c.label}</option>`).join("")}
          </select>
        </div>
        <div>
          <label>COULEUR</label>
          <select id="gc-color">
            ${Dn.map(c=>`<option value="${c.value}" ${u.color===c.value?"selected":""}>${c.label}</option>`).join("")}
          </select>
        </div>
      </div>
      <div>
  ...
  `);const r=()=>{var p,x,w,k,L;const c=((p=document.getElementById("gc-name"))==null?void 0:p.value)||"?",b=((x=document.getElementById("gc-effect"))==null?void 0:x.value)||"",I=((w=document.getElementById("gc-type"))==null?void 0:w.value)||"game_changer",$=(L=(k=document.getElementById("gc-image-url"))==null?void 0:k.value)==null?void 0:L.trim(),E=H.find(B=>B.value===I)||H[0],v=document.getElementById("gc-modal-preview");v&&(v.innerHTML=fe(c,$?`/icons/${$}`:null,E.label.split(" ")[0],b,{width:150}))};["gc-name","gc-effect","gc-type","gc-image-url"].forEach(c=>{var b,I;(b=document.getElementById(c))==null||b.addEventListener("input",r),(I=document.getElementById(c))==null||I.addEventListener("change",r)}),(s=document.getElementById("gc-effect-type"))==null||s.addEventListener("change",()=>{const c=document.getElementById("gc-effect-type").value,b=G.find($=>$.value===c)||G[0],I=document.getElementById("gc-params-wrap");I.innerHTML=`
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
      ...
      ${b.hasRoles?`<div><label>POSTES CIBLÉS</label><div style="display:flex;gap:4px;flex-wrap:wrap">${Ke.map($=>`<label style="display:flex;align-items:center;gap:2px;font-size:11px;cursor:pointer;color:#555"><input type="checkbox" class="gc-p-role" value="${$}" checked> ${$}</label>`).join("")}</div></div>`:""}
    </div>`}),(g=document.getElementById("btn-pick-gc-icon"))==null||g.addEventListener("click",async()=>{var b;const c=document.getElementById("gc-icon-picker");if(c){if(c.style.display!=="none"){c.style.display="none";return}c.innerHTML='<div style="padding:10px;text-align:center;color:#888;font-size:12px">⏳ Chargement...</div>',c.style.display="block";try{const $=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),E=Array.isArray($)?$.filter(p=>p.name.startsWith("gamechanger-")&&/\.(png|jpg|svg|webp)$/i.test(p.name)):[];if(!E.length){c.innerHTML='<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Aucun fichier "gamechanger-*" trouvé dans public/icons/</div>';return}const v=((b=document.getElementById("gc-image-url"))==null?void 0:b.value)||"";c.innerHTML=`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(72px,1fr));gap:8px">
        ${E.map(p=>`
        <div class="gc-icon-item" data-name="${p.name}"
          style="cursor:pointer;text-align:center;padding:6px;border-radius:6px;border:2px solid ${p.name===v?"#7a28b8":"#ddd"};background:${p.name===v?"#f5f0ff":"#fff"}">
          <img src="/icons/${p.name}" style="height:44px;width:100%;object-fit:contain">
          <div style="font-size:8px;color:#666;margin-top:3px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${p.name.replace(/\.(png|jpg|svg|webp)$/i,"").replace("gamechanger-","")}</div>
        </div>`).join("")}
      </div>`,c.querySelectorAll(".gc-icon-item").forEach(p=>{p.addEventListener("click",()=>{const x=document.getElementById("gc-image-url");x&&(x.value=p.dataset.name),c.style.display="none",r()})})}catch(I){c.innerHTML=`<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Erreur API GitHub : ${I.message}</div>`}}}),(d=document.getElementById("gc-cancel"))==null||d.addEventListener("click",()=>l()),(y=document.getElementById("gc-save"))==null||y.addEventListener("click",async()=>{var v,p,x,w,k,L,B,A,S,C,M,F,O,U;const c=G.find(N=>{var j;return N.value===(((j=document.getElementById("gc-effect-type"))==null?void 0:j.value)||"BOOST_STAT")})||G[0],b=c.hasRoles?[...document.querySelectorAll(".gc-p-role:checked")].map(N=>N.value):null,I={...c.hasValue?{value:Number((v=document.getElementById("gc-p-value"))==null?void 0:v.value)||2}:{},...c.hasCount?{count:Number((p=document.getElementById("gc-p-count"))==null?void 0:p.value)||1}:{},...c.hasTarget?{target:((x=document.getElementById("gc-p-target"))==null?void 0:x.value)||"home"}:{},...c.hasRoles?{roles:b!=null&&b.length?b:null}:{}},$={name:((k=(w=document.getElementById("gc-name"))==null?void 0:w.value)==null?void 0:k.trim())||"",effect:((B=(L=document.getElementById("gc-effect"))==null?void 0:L.value)==null?void 0:B.trim())||null,image_url:((S=(A=document.getElementById("gc-image-url"))==null?void 0:A.value)==null?void 0:S.trim())||null,gc_type:((C=document.getElementById("gc-type"))==null?void 0:C.value)||"game_changer",color:((M=document.getElementById("gc-color"))==null?void 0:M.value)||"purple",sort_order:Number((F=document.getElementById("gc-sort"))==null?void 0:F.value)||0,is_active:((O=document.getElementById("gc-active"))==null?void 0:O.checked)??!0,effect_type:((U=document.getElementById("gc-effect-type"))==null?void 0:U.value)||"BOOST_STAT",effect_params:I};if(!$.name){a("Le nom est obligatoire","error");return}const{error:E}=m?await _.from("gc_definitions").insert($):await _.from("gc_definitions").update($).eq("id",u.id);if(E){a(E.message,"error");return}a(m?"Carte créée ✅":"Carte modifiée ✅","success"),l(),Be(t,n)})}async function Fn(e,{toast:t,openModal:n,closeModal:o}){await Y(e,{toast:t,openModal:n,closeModal:o})}async function Y(e,t){var E;const{toast:n,openModal:o,closeModal:l}=t,[{data:a,error:m},{data:u}]=await Promise.all([_.from("ranked_seasons").select("*").order("start_at",{ascending:!1}),_.from("users").select("mmr,rank_tier,placement_matches").gte("placement_matches",1)]);if(m){e.innerHTML=`<p style="color:red">${m.message}</p>`;return}const h=a||[],i=h.find(v=>v.is_active),f=(u||[]).filter(v=>(v.placement_matches||0)>=1),r=f.filter(v=>(v.placement_matches||0)>=10),s=["bronze","silver","gold","platinum","diamond","master"],g={bronze:"Bronze",silver:"Argent",gold:"Or",platinum:"Platine",diamond:"Diamant",master:"Maître"},d={bronze:"#cd7f32",silver:"#a0a0a0",gold:"#D4A017",platinum:"#00e5ff",diamond:"#5b9bd5",master:"#9b59b6"},y={bronze:"🥉",silver:"🥈",gold:"🥇",platinum:"💎",diamond:"🔷",master:"👑"},c={};s.forEach(v=>{c[v]=0}),r.forEach(v=>{const p=v.rank_tier||"bronze";c[p]!==void 0&&c[p]++});const b=r.length?Math.round(r.reduce((v,p)=>v+(p.mmr||1e3),0)/r.length):0;function I(v){return v?new Date(v).toLocaleDateString("fr",{day:"numeric",month:"short",year:"numeric"}):"—"}function $(v){const p=new Date;return v.is_active?'<span style="background:#1A6B3C;color:#fff;padding:2px 8px;border-radius:10px;font-size:11px;font-weight:700">● ACTIVE</span>':new Date(v.end_at)<p?'<span style="background:#555;color:#ccc;padding:2px 8px;border-radius:10px;font-size:11px">Terminée</span>':'<span style="background:#e67e22;color:#fff;padding:2px 8px;border-radius:10px;font-size:11px">À venir</span>'}e.innerHTML=`
    <div style="display:flex;flex-direction:column;gap:20px">
      ...
      <div class="card-panel" style="background:linear-gradient(135deg,#0a3d1e,#1a6b3c20);border:1.5px solid #1A6B3C40">
        ...
        <div style="display:flex;flex-direction:column;gap:4px">
          ${s.map(v=>{const p=c[v],x=r.length>0?Math.round(p/r.length*100):0;return`
              <div style="display:flex;align-items:center;gap:8px;font-size:12px">
                <span style="width:80px">${y[v]} ${g[v]}</span>
                ...
                <span style="width:40px;text-align:right;color:var(--gray-600)">${p} (${x}%)</span>
              </div>`}).join("")}
        ...
        <div class="table-wrap">
          <table>
            ...
            <tbody id="seasons-tbody">
              ${h.length===0?'<tr><td colspan="6" style="text-align:center;color:var(--gray-600);padding:24px">Aucune saison créée.</td></tr>':h.map(v=>{const p=Math.round((new Date(v.end_at)-new Date(v.start_at))/864e5),x=new Date(v.end_at)<new Date&&!v.is_active;return`
                    <tr>
                      <td><b>${v.name}</b></td>
                      <td style="font-size:12px">${I(v.start_at)}</td>
                      <td style="font-size:12px">${I(v.end_at)}</td>
                      <td style="font-size:12px">${p} jours</td>
                      <td>${$(v)}</td>
    ...
    </div>`,(E=document.getElementById("create-season-btn"))==null||E.addEventListener("click",()=>{Ve(null,{toast:n,openModal:o,closeModal:l,reload:()=>Y(e,t)})}),e.querySelectorAll("[data-edit]").forEach(v=>{const p=h.find(x=>x.id==v.dataset.edit);v.addEventListener("click",()=>{Ve(p,{toast:n,openModal:o,closeModal:l,reload:()=>Y(e,t)})})}),e.querySelectorAll("[data-activate]").forEach(v=>{v.addEventListener("click",async()=>{const p=parseInt(v.dataset.activate);if(!confirm("Activer cette saison ? La saison actuellement active sera désactivée."))return;const{error:x}=await _.from("ranked_seasons").update({is_active:!1}).neq("id",0);if(x){n(x.message,"error");return}const{error:w}=await _.from("ranked_seasons").update({is_active:!0}).eq("id",p);if(w){n(w.message,"error");return}await _.from("users").update({current_season_id:p}).gt("placement_matches",0),n("Saison activée ✅","success"),Y(e,t)})}),e.querySelectorAll("[data-deactivate]").forEach(v=>{v.addEventListener("click",async()=>{if(!confirm("Désactiver la saison active ? Le mode Ranked sera suspendu."))return;const{error:p}=await _.from("ranked_seasons").update({is_active:!1}).eq("id",parseInt(v.dataset.deactivate));if(p){n(p.message,"error");return}n("Saison désactivée","success"),Y(e,t)})}),e.querySelectorAll("[data-delete]").forEach(v=>{v.addEventListener("click",async()=>{if(!confirm("Supprimer cette saison archivée ?"))return;const{error:p}=await _.from("ranked_seasons").delete().eq("id",parseInt(v.dataset.delete));if(p){n(p.message,"error");return}n("Saison supprimée","success"),Y(e,t)})})}function Ve(e,{toast:t,openModal:n,closeModal:o,reload:l}){const a=!!e,m=new Date().toISOString().slice(0,16),u=new Date(Date.now()+90*864e5).toISOString().slice(0,16),h=e?new Date(e.start_at).toISOString().slice(0,16):m,i=e?new Date(e.end_at).toISOString().slice(0,16):u,f=(e==null?void 0:e.name)||"";n(a?"✏️ Modifier la saison":"➕ Nouvelle saison",`<div style="display:flex;flex-direction:column;gap:14px">
      <div class="form-group">
    ...
    </div>`);function r(){var c,b;const s=(c=document.getElementById("season-start"))==null?void 0:c.value,g=(b=document.getElementById("season-end"))==null?void 0:b.value,d=document.getElementById("season-duration");if(!s||!g||!d)return;const y=Math.round((new Date(g)-new Date(s))/864e5);d.textContent=y>0?`Durée : ${y} jour${y>1?"s":""}`:"⚠️ La fin doit être après le début",d.style.color=y>0?"var(--gray-600)":"#bb2020"}setTimeout(()=>{var s,g,d;(s=document.getElementById("season-start"))==null||s.addEventListener("input",r),(g=document.getElementById("season-end"))==null||g.addEventListener("input",r),r(),(d=document.getElementById("season-save-btn"))==null||d.addEventListener("click",async()=>{var p,x,w;const y=(p=document.getElementById("season-name"))==null?void 0:p.value.trim(),c=(x=document.getElementById("season-start"))==null?void 0:x.value,b=(w=document.getElementById("season-end"))==null?void 0:w.value,I=document.getElementById("season-error");if(!y){I.textContent="Le nom est requis.";return}if(!c){I.textContent="La date de début est requise.";return}if(!b){I.textContent="La date de fin est requise.";return}if(new Date(b)<=new Date(c)){I.textContent="La date de fin doit être après le début.";return}const $=document.getElementById("season-save-btn");$.disabled=!0,$.textContent="Enregistrement…";const E={name:y,start_at:new Date(c).toISOString(),end_at:new Date(b).toISOString()};let v;if(a?{error:v}=await _.from("ranked_seasons").update(E).eq("id",e.id):{error:v}=await _.from("ranked_seasons").insert({...E,is_active:!1}),v){I.textContent=v.message,$.disabled=!1,$.textContent=a?"💾 Enregistrer":"✅ Créer la saison";return}t(a?"Saison modifiée ✅":"Saison créée ✅","success"),o(),l()})},50)}async function xe(e,t){var h;const{toast:n}=t,{data:o,error:l}=await _.from("solo_levels").select("*").order("level_number");if(l){e.innerHTML=`<p style="color:var(--danger)">${l.message}</p>`;return}const a=i=>`
    <tr data-row="${i.id}">
