import{s as w,r as Ne,A as et,l as tt,g as nt,a as Fe,K as we,D as T,b as it,S as ot,c as R,H as at,R as rt,e as ue,d as Ge,f as lt,h as st,E as dt,i as ct,N as pt,M as ut,B as mt,j as gt,F as me,k as _e,m as ft,n as bt,o as yt,p as vt}from"./special-cards-3oqhuBBr.js";async function xt(e){const[{count:t},{count:n},{count:i},{count:u},{count:d}]=await Promise.all([w.from("players").select("*",{count:"exact",head:!0}),w.from("clubs").select("*",{count:"exact",head:!0}),w.from("users").select("*",{count:"exact",head:!0}),w.from("cards").select("*",{count:"exact",head:!0}),w.from("matches").select("*",{count:"exact",head:!0})]);e.innerHTML=`
  ...
  `,window.adminNav=c=>{var r;(r=document.querySelector(`[data-page="${c}"]`))==null||r.click()}}function ee(e,t,n,i){return`<div class="card-panel" style="text-align:center">
  ...
  </div>`}const He={normal:"Normal",pepite:"Pépite",papyte:"Papyte",legende:"Légende"},ht={GK:"#aaaaaa",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};let te=null;async function Et(){if(te)return te;te={};for(const e of et)te[e]=await tt(e);return te}async function wt(e,t){await de(e,t)}async function de(e,t,n=null){var o,g,l,a,s;const{toast:i}=t;n||(n={search:((o=document.getElementById("search-players"))==null?void 0:o.value)||"",job:((g=document.getElementById("filter-job"))==null?void 0:g.value)||"",rarity:((l=document.getElementById("filter-rarity"))==null?void 0:l.value)||"",club:((a=document.getElementById("filter-club"))==null?void 0:a.value)||"",country:((s=document.getElementById("filter-country"))==null?void 0:s.value)||""});const[{data:u,error:d},{data:c}]=await Promise.all([w.from("players").select("*, clubs(id,encoded_name,logo_url)"),w.from("clubs").select("id,encoded_name").order("encoded_name")]);if(d){e.innerHTML=`<p style="color:red">${d.message}</p>`;return}const r={GK:0,DEF:1,MIL:2,ATT:3},b=(u||[]).sort((p,h)=>{const E=(r[p.job]??4)-(r[h.job]??4);return E!==0?E:(p.surname_real||"").localeCompare(h.surname_real||"")});_t(e,b,c||[],t,n)}function _t(e,t,n,i,u=null){var g,l;const{toast:d}=i;if(e.innerHTML=`
    <div style="display:flex;flex-direction:column;gap:10px">
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        ...
        <select id="filter-rarity" style="width:110px">
          ...
          ${Object.entries(He).map(([a,s])=>`<option value="${a}">${s}</option>`).join("")}
        </select>
    ...
    </div>`,u){const a=u,s=p=>document.getElementById(p);a.search&&s("search-players")&&(s("search-players").value=a.search),a.job&&s("filter-job")&&(s("filter-job").value=a.job),a.rarity&&s("filter-rarity")&&(s("filter-rarity").value=a.rarity),a.club&&s("filter-club")&&(s("filter-club").value=a.club),a.country&&s("filter-country")&&(s("filter-country").value=a.country)}if(u){const a=u,s=p=>document.getElementById(p);a.search&&s("search-players")&&(s("search-players").value=a.search),a.job&&s("filter-job")&&(s("filter-job").value=a.job),a.rarity&&s("filter-rarity")&&(s("filter-rarity").value=a.rarity),a.club&&s("filter-club")&&(s("filter-club").value=a.club),a.country&&s("filter-country")&&(s("filter-country").value=a.country)}function c(){const a=document.getElementById("search-players").value.toLowerCase(),s=document.getElementById("filter-job").value,p=document.getElementById("filter-rarity").value,h=document.getElementById("filter-club").value,E=document.getElementById("filter-country").value;return t.filter(m=>(!a||`${m.firstname} ${m.surname_real}`.toLowerCase().includes(a))&&(!s||m.job===s)&&(!p||m.rarity===p)&&(!h||m.club_id===h)&&(!E||m.country_code===E))}const r=new Set;function b(){const a=document.getElementById("bulk-bar"),s=document.getElementById("bulk-count");a&&(a.style.display=r.size>0?"flex":"none",s&&(s.textContent=`${r.size} joueur(s) sélectionné(s)`))}function o(){const a=c();document.getElementById("count-label").textContent=`${a.length} joueur(s)`;const s=document.getElementById("players-list");if(!a.length){s.innerHTML='<div style="color:var(--gray-600);padding:20px;text-align:center">Aucun joueur.</div>';return}s.innerHTML=a.map(p=>{const h={...p,clubs:p.clubs,face:p.face||null},E=Ne(h,{width:120});return`<div style="position:relative;cursor:pointer" data-edit="${p.id}">
        ${E}
      ...
      </div>`}).join(""),s.querySelectorAll("[data-edit]").forEach(p=>{p.addEventListener("click",()=>{const h=t.find(E=>E.id===p.dataset.edit);h&&ke(h,n,e,i)})}),s.querySelectorAll(".btn-del-player").forEach(p=>{p.addEventListener("click",async h=>{if(h.stopPropagation(),!confirm("Supprimer ce joueur ?"))return;const{error:E}=await w.from("players").delete().eq("id",p.dataset.del);E?d(E.message,"error"):(d("Joueur supprimé ✅","success"),de(e,i))})})}o(),document.getElementById("search-players").addEventListener("input",o),document.getElementById("filter-job").addEventListener("change",o),document.getElementById("filter-rarity").addEventListener("change",o),document.getElementById("filter-club").addEventListener("change",o),document.getElementById("filter-country").addEventListener("change",o),(g=document.getElementById("bulk-cancel-btn"))==null||g.addEventListener("click",()=>{r.clear(),b(),o()}),(l=document.getElementById("bulk-delete-btn"))==null||l.addEventListener("click",async()=>{var h,E,m,$,k;if(!r.size||!confirm(`Supprimer ${r.size} joueur(s) ?`))return;const a=[...r],{error:s}=await w.from("players").delete().in("id",a);if(s){d(s.message,"error");return}d(`${a.length} joueur(s) supprimé(s) ✅`,"success"),r.clear();const p={search:((h=document.getElementById("search-players"))==null?void 0:h.value)||"",job:((E=document.getElementById("filter-job"))==null?void 0:E.value)||"",rarity:((m=document.getElementById("filter-rarity"))==null?void 0:m.value)||"",club:(($=document.getElementById("filter-club"))==null?void 0:$.value)||"",country:((k=document.getElementById("filter-country"))==null?void 0:k.value)||""};de(e,i,p)}),document.getElementById("add-player-btn").addEventListener("click",()=>ke(null,n,e,i))}async function ke(e,t,n,i){const{toast:u,openModal:d,closeModal:c}=i,r=!!e,b=await Et(),{data:o}=await w.from("players").select("face").not("face","is",null),g=new Set((o||[]).map(p=>p.face).filter(Boolean));e!=null&&e.face&&g.delete(e.face);const l='<option value="">— Club —</option>'+t.map(p=>`<option value="${p.id}" ${(e==null?void 0:e.club_id)===p.id?"selected":""}>${p.encoded_name}</option>`).join(""),a=e!=null&&e.face?e.face.split("/")[0]:"",s=Object.keys(b).map(p=>`<option value="${p}" ${a===p?"selected":""}>${p}</option>`).join("");d(r?`✏️ ${e.firstname} ${e.surname_real}`:"➕ Nouveau joueur",`<div style="display:flex;gap:20px;align-items:flex-start">
      ...
      <div style="flex:1;min-width:300px;display:flex;flex-direction:column;gap:12px">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:8px">
          ...
          <div class="form-group">
            <label>Rareté</label>
            <select id="pm-rarity">
              ${["normal","pepite","papyte","legende"].map(p=>`<option value="${p}" ${(e==null?void 0:e.rarity)===p?"selected":""}>${He[p]}</option>`).join("")}
            </select>
          </div>
          <div class="form-group">
            <label>Pays</label>
        ...
        <div style="border-top:1px solid var(--gray-200);padding-top:10px">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:8px">
            ${[["GK","pm-g","note_g"],["DEF","pm-d","note_d"],["MIL","pm-m","note_m"],["ATT","pm-a","note_a"]].map(([p,h,E])=>`
    ...
    <div id="clubs-list" style="display:flex;flex-direction:column;gap:8px"></div>`,c(re),document.getElementById("search-clubs").addEventListener("input",r=>{const b=r.target.value.toLowerCase();c(re.filter(o=>o.real_name.toLowerCase().includes(b)||o.encoded_name.toLowerCase().includes(b)))}),document.getElementById("add-club-btn").addEventListener("click",()=>Ie(null,e,t));function c(r){const b=document.getElementById("clubs-list");if(!r.length){b.innerHTML='<p style="color:var(--gray-600);padding:20px">Aucun club.</p>';return}b.innerHTML=r.map(o=>{const g=Ke(o),l=Fe(g,o.id).replace("<svg ",'<svg style="width:40px;height:48px" '),a=o.logo_url?`<img src="${o.logo_url}" style="width:40px;height:40px;object-fit:contain;border-radius:8px">`:`<div style="width:40px;height:40px;background:linear-gradient(135deg,${g.color1},${g.color2});border-radius:8px;display:flex;align-items:center;justify-content:center;color:#fff;font-size:10px;font-weight:900">${o.encoded_name.slice(0,3)}</div>`,s=n[o.id]||0;return`
        ...
        </div>`}).join(""),b.querySelectorAll("[data-gen]").forEach(o=>{o.addEventListener("click",async()=>{confirm(`Générer 20 joueurs pour ${o.dataset.name} ?`)&&(o.disabled=!0,o.textContent="⏳",await Ue(o.dataset.gen,o.dataset.cc,i),ce(e,t))})}),b.querySelectorAll("[data-edit]").forEach(o=>{const g=re.find(l=>l.id===o.dataset.edit);o.addEventListener("click",()=>Ie(g,e,t))}),b.querySelectorAll("[data-del]").forEach(o=>{o.addEventListener("click",async()=>{if(!confirm("Supprimer ce club ?"))return;const{error:g}=await w.from("clubs").delete().eq("id",o.dataset.del);g?i(g.message,"error"):(i("Club supprimé","success"),ce(e,t))})})}}function Ie(e,t,n){const{toast:i,openModal:u,closeModal:d}=n,c=!!e,r=e?Ke(e):{...T},b=Object.entries(we).map(([g,l])=>`<option value="${g}" ${r.style===g?"selected":""}>${l.label}</option>`).join(""),o=qe.map(g=>`<option value="${g}" ${((e==null?void 0:e.country_code)||"FR")===g?"selected":""}>${g}</option>`).join("");u(c?`✏️ ${e.real_name}`:"➕ Nouveau club",`<div style="display:flex;flex-direction:column;gap:14px">
      ...
      <div style="border-top:1px solid var(--gray-200);padding-top:12px">
        ...
        <div style="display:flex;gap:12px;align-items:flex-start">
          ...
          <div style="flex:1;display:flex;flex-direction:column;gap:8px">
            ...
            ${[["Couleur 1","m-kit-color1",r.color1,!1],["Couleur 2","m-kit-color2",r.color2,!1],["Couleur 3","m-kit-color3",r.color3,!0],["Short","m-kit-shorts",r.shorts,!1],["Chaussettes","m-kit-socks",r.socks,!1]].map(([g,l,a,s])=>`
    ...
    </div>`),setTimeout(()=>{var p,h,E;ne();function g(){var x,f;const m=((x=document.getElementById("m-kit-style"))==null?void 0:x.value)||"uni",$=((f=we[m])==null?void 0:f.colors)===3,k=document.getElementById("wrap-m-kit-color3");k&&(k.style.display=$?"":"none")}g(),["m-kit-color1","m-kit-color2","m-kit-color3","m-kit-shorts","m-kit-socks"].forEach(m=>{const $=document.getElementById(m),k=document.getElementById(m+"-txt");!$||!k||($.addEventListener("input",()=>{k.value=$.value,ne()}),k.addEventListener("input",()=>{const x=k.value.trim();/^#[0-9a-fA-F]{6}$/.test(x)&&($.value=x,ne())}),k.addEventListener("change",()=>{let x=k.value.trim();x.startsWith("#")||(x="#"+x),/^#[0-9a-fA-F]{6}$/.test(x)&&($.value=x,k.value=x,ne())}))}),(p=document.getElementById("m-kit-style"))==null||p.addEventListener("change",()=>{g(),ne()});const l=document.getElementById("m-real"),a=document.getElementById("m-encoded");function s(){if(!l||!a||a.value)return;const m=l.value.trim().split(/\s+/),$=m.length===1?m[0].toUpperCase().slice(0,6):m.filter(k=>k.length>2).map(k=>k[0].toUpperCase()).join("")||m[0].toUpperCase().slice(0,4);a.value=$}l==null||l.addEventListener("input",s),(h=document.getElementById("auto-encode-btn"))==null||h.addEventListener("click",()=>{a&&(a.value=""),s()}),(E=document.getElementById("m-save"))==null||E.addEventListener("click",()=>jt(e,c,t,n))},50)}function Ve(){var e,t,n,i,u,d;return{style:((e=document.getElementById("m-kit-style"))==null?void 0:e.value)||"uni",color1:((t=document.getElementById("m-kit-color1"))==null?void 0:t.value)||"#1A6B3C",color2:((n=document.getElementById("m-kit-color2"))==null?void 0:n.value)||"#ffffff",color3:((i=document.getElementById("m-kit-color3"))==null?void 0:i.value)||"#000000",shorts:((u=document.getElementById("m-kit-shorts"))==null?void 0:u.value)||"#111111",socks:((d=document.getElementById("m-kit-socks"))==null?void 0:d.value)||"#ffffff"}}function ne(){const e=document.getElementById("kit-preview-wrap");e&&(e.innerHTML=Fe(Ve(),"modal"))}async function jt(e,t,n,i){var m,$,k,x,f,y;const{toast:u,closeModal:d}=i,c=document.getElementById("m-error"),r=document.getElementById("m-save"),b=(m=document.getElementById("m-real"))==null?void 0:m.value.trim(),o=($=document.getElementById("m-encoded"))==null?void 0:$.value.trim().toUpperCase(),g=(k=document.getElementById("m-country"))==null?void 0:k.value.trim().toUpperCase(),l=((x=document.getElementById("m-logo"))==null?void 0:x.value.trim())||null,a=((f=document.getElementById("m-gen-stadium"))==null?void 0:f.checked)??!1,s=((y=document.getElementById("m-gen-squad"))==null?void 0:y.checked)??!1,p=Ve();if(!b){c.textContent="Le nom du club est requis.";return}if(!o){c.textContent="Le nom encodé est requis.";return}if(!g){c.textContent="Le pays est requis.";return}r.disabled=!0,r.textContent="Enregistrement…";const h={real_name:b,encoded_name:o,country_code:g,logo_url:l,kit_style:p.style,kit_color1:p.color1,kit_color2:p.color2,kit_color3:p.color3,kit_shorts:p.shorts,kit_socks:p.socks};let E=e==null?void 0:e.id;if(t){const{error:v}=await w.from("clubs").update(h).eq("id",E);if(v){c.textContent=v.message,r.disabled=!1,r.textContent="💾 Enregistrer";return}}else{const{data:v,error:_}=await w.from("clubs").insert(h).select().single();if(_){c.textContent=_.message,r.disabled=!1,r.textContent="✅ Créer le club";return}if(E=v.id,a){r.textContent="🏟️ Création du stade…";const{data:B,error:I}=await w.from("stadium_definitions").insert({name:`Stade de ${b}`,club_id:E,country_code:g}).select().single();I?console.warn("[Stadium] Erreur def stade:",I.message):B&&await w.from("cards").insert({card_type:"stadium",stadium_id:B.id})}s&&(r.textContent="⚽ Génération des joueurs…",await Ue(E,g,u))}u(t?"Club modifié ✅":"Club créé ✅","success"),d(),ce(n,i)}const Dt=["normal","pepite","papyte","legende"],Be=["GK","DEF","MIL","ATT"],Pt=[["MA","Maroc"],["FR","France"],["AR","Argentine"],["PT","Portugal"],["BR","Brésil"],["ES","Espagne"],["DE","Allemagne"],["GB","Angleterre"],["IT","Italie"],["CM","Cameroun"],["SN","Sénégal"],["NG","Nigéria"],["DK","Danemark"],["NL","Pays-Bas"],["BE","Belgique"],["CI","Côte d'Ivoire"],["AL","Albanie"],["HR","Croatie"],["RS","Serbie"],["TR","Turquie"],["MW","Malawi"]];let U={...R},le=[];async function Nt(e,{toast:t}){const{data:n}=await w.from("clubs").select("id,encoded_name,logo_url,kit_style,kit_color1,kit_color2,kit_shorts,kit_socks").order("encoded_name");le=n||[],U={...R},e.innerHTML=qt(le),Gt(e,le,t),N()}function Se(){var n;const e=((n=document.getElementById("cb-surname-real"))==null?void 0:n.value)||"",t=document.getElementById("cb-surname-enc");t&&(t.value=ue(e))}function Ft(){var n;const e=(n=document.getElementById("cb-club"))==null?void 0:n.value,t=le.find(i=>String(i.id)===String(e));return t?{style:t.kit_style||T.style,color1:t.kit_color1||T.color1,color2:t.kit_color2||T.color2,shorts:t.kit_shorts||T.shorts,socks:t.kit_socks||T.socks}:{...T}}function N(){var a,s,p,h;const e=E=>{var m;return((m=document.getElementById(E))==null?void 0:m.value)||""},t={firstname:e("cb-firstname"),surname_real:e("cb-surname-enc")||ue(e("cb-surname-real")),country_code:e("cb-country"),club_encoded_name:((s=(a=document.getElementById("cb-club"))==null?void 0:a.selectedOptions[0])==null?void 0:s.text)||"",job:e("cb-job"),job2:e("cb-job2")||null,note_g:parseInt(e("cb-note-g"))||0,note_d:parseInt(e("cb-note-d"))||0,note_m:parseInt(e("cb-note-m"))||0,note_a:parseInt(e("cb-note-a"))||0,rarity:e("cb-rarity")},n=Ft(),i=document.getElementById("cb-club"),u=((h=(p=i==null?void 0:i.selectedOptions[0])==null?void 0:p.dataset)==null?void 0:h.logo)||null,d=Ge(U,n,120),c=`data:image/svg+xml;charset=utf-8,${encodeURIComponent(d)}`,r=lt(t,{portraitUrl:c,clubLogoUrl:u,showNotes:!0}),b=document.getElementById("card-preview");b&&(b.innerHTML=r);const o=document.getElementById("avatar-preview-wrap");o&&(o.innerHTML=d);const g=e("cb-surname-real"),l=document.getElementById("encode-summary");l&&g&&(l.textContent=`${e("cb-firstname")} ${g} → ${e("cb-firstname")} ${t.surname_real}`)}function Je(){const e=t=>{var n;return((n=document.getElementById(t))==null?void 0:n.value)||""};U={skin:e("av-skin"),hair:e("av-hair"),hairColor:e("av-hairColor"),eyes:e("av-eyes"),nose:e("av-nose"),mouth:e("av-mouth"),eyebrows:e("av-eyebrows"),beard:e("av-beard")}}function Gt(e,t,n){var u,d,c,r,b;e.querySelectorAll("input,select").forEach(o=>{o.addEventListener("input",N),o.addEventListener("change",N)});const i=["av-skin","av-hair","av-hairColor","av-eyes","av-nose","av-mouth","av-eyebrows","av-beard"];i.forEach(o=>{var g;(g=document.getElementById(o))==null||g.addEventListener("change",()=>{Je(),N()})}),(u=document.getElementById("cb-surname-real"))==null||u.addEventListener("input",()=>{Se(),N()}),(d=document.getElementById("btn-encode"))==null||d.addEventListener("click",()=>{Se(),N()}),(c=document.getElementById("cb-club"))==null||c.addEventListener("change",N),(r=document.getElementById("btn-save-player"))==null||r.addEventListener("click",()=>Ht(n)),(b=document.getElementById("btn-reset"))==null||b.addEventListener("click",()=>{e.querySelectorAll("input").forEach(o=>o.value=""),e.querySelectorAll("select").forEach(o=>o.selectedIndex=0),U={...R},i.forEach(o=>{const g=o.replace("av-",""),l=document.getElementById(o);l&&(l.value=R[g]||"")}),N()})}async function Ht(e){const t=c=>{var r;return((r=document.getElementById(c))==null?void 0:r.value)||""},n=t("cb-surname-real").trim(),i=t("cb-surname-enc").trim()||ue(n);if(!t("cb-firstname")||!n||!t("cb-country")||!t("cb-job")){e("Remplissez les champs obligatoires (prénom, nom, pays, poste)","error");return}Je();const u={firstname:t("cb-firstname").trim(),surname_real:n,surname_real:i,country_code:t("cb-country"),club_id:t("cb-club")||null,job:t("cb-job"),job2:t("cb-job2")||null,note_g:parseInt(t("cb-note-g"))||0,note_d:parseInt(t("cb-note-d"))||0,note_m:parseInt(t("cb-note-m"))||0,note_a:parseInt(t("cb-note-a"))||0,rarity:t("cb-rarity")||"normal",note_min:parseInt(t("cb-note-min"))||null,note_max:parseInt(t("cb-note-max"))||null,sell_price:parseInt(t("cb-price"))||0,avatar_config:U,skin:U.skin||"blanc",hair:U.hairColor||"marron",hair_length:"court"},{error:d}=await w.from("players").insert(u);if(d){e("Erreur: "+d.message,"error");return}e(`✅ Joueur "${u.firstname} ${u.surname_real}" enregistré !`,"success")}function qt(e){const t=Pt.map(([o,g])=>`<option value="${o}">${g} (${o})</option>`).join(""),n='<option value="">— Sélectionner un club —</option>'+e.map(o=>`<option value="${o.id}" data-logo="${o.logo_url||""}">${o.encoded_name}</option>`).join(""),i=Be.map(o=>`<option value="${o}">${o}</option>`).join(""),u='<option value="">Aucun</option>'+Be.map(o=>`<option value="${o}">${o}</option>`).join(""),d=Dt.map(o=>`<option value="${o}">${o.charAt(0).toUpperCase()+o.slice(1)}</option>`).join(""),c=(o,g,l)=>Object.entries(g).map(([a,s])=>`<option value="${a}" ${a===(l||"")?"selected":""}>${s.label}</option>`).join(""),r=Object.keys(ot).map(o=>`<option value="${o}" ${o===R.skin?"selected":""}>${o.charAt(0).toUpperCase()+o.slice(1)}</option>`).join(""),b=Object.keys(at).map(o=>`<option value="${o}" ${o===R.hairColor?"selected":""}>${o.charAt(0).toUpperCase()+o.slice(1)}</option>`).join("");return`
  ...
  `,d(u),document.getElementById("search-users").addEventListener("input",c=>{const r=c.target.value.toLowerCase();d(u.filter(b=>{var o;return b.pseudo.toLowerCase().includes(r)||((o=b.club_name)==null?void 0:o.toLowerCase().includes(r))}))});function d(c){document.getElementById("users-tbody").innerHTML=c.map(r=>{const b=r.mmr??1e3,o=r.mmr_deviation??350,g=r.placement_matches??0,l=gt(b),a=r.ranked_wins??0,s=r.ranked_losses??0,p=r.ranked_draws??0,h=a+s+p,E=h>0?Math.round(a/h*100):null,m=g<10,$=o<80?"#1A6B3C":o<150?"#e67e22":"#bb2020";return`
        <tr>
          <td>
          ...
          <td style="font-size:12px">
            ...
            ${m?`<div style="font-size:10px;color:#e67e22">${g}/10 placement${g<10?` (${10-g} restants)`:""}</div>`:`<div style="font-size:10px;color:var(--gray-600)">${a}V ${p}N ${s}D${E!==null?` · ${E}% WR`:""}</div>`}
          </td>
    ...
    `).order("listed_at",{ascending:!1}).limit(100);if(i){e.innerHTML=`<p style="color:red">${i.message}</p>`;return}const u=n||[],d=u.filter(r=>r.status==="active").length,c=u.filter(r=>r.status==="sold").length;e.innerHTML=`
    ...
    <div class="card-panel">
      <div class="table-wrap">
        <table>
          ...
          <tbody>
            ${u.map(r=>{var l,a,s,p;const b=(l=r.card)==null?void 0:l.player,o=b?`${b.firstname} ${b.surname_real}`:((a=r.card)==null?void 0:a.card_type)||"—",g={active:"#1A6B3C",sold:"#D4A017",cancelled:"#aaa"};return`<tr>
                ...
                <td style="font-size:12px">${((p=r.buyer)==null?void 0:p.pseudo)||"—"}</td>
  ...
  `,document.querySelectorAll("[data-cancel]").forEach(r=>{r.addEventListener("click",async()=>{if(!confirm("Annuler cette annonce ?"))return;const{error:b}=await w.from("market_listings").update({status:"cancelled"}).eq("id",r.dataset.cancel);b?t(b.message,"error"):(t("Annonce annulée","success"),Ye(e,{toast:t}))})})}async function Ot(e,{toast:t}){e.innerHTML=`
...
`;n.forEach(d=>{u+=[d.real_name,d.encoded_name,d.country_code,d.logo_url||""].map(Le).join(",")+`
`}),oe("clubs_export.csv",u),t(`${n.length} clubs exportés`,"success")}),document.getElementById("export-players").addEventListener("click",async()=>{const{data:n,error:i}=await w.from("players").select("firstname,surname_real,surname_real,country_code,job,job2,note_g,note_d,note_m,note_a,rarity,note_min,note_max,skin,hair,hair_length,sell_price,clubs(encoded_name)").order("surname_real");if(i){t(i.message,"error");return}if(!n||n.length===0){t("Aucun joueur à exporter","info");return}let u=`firstname,surname_real,surname_real,country_code,club_encoded_name,job,job2,note_g,note_d,note_m,note_a,rarity,note_min,note_max,skin,hair,hair_length,sell_price
`;n.forEach(d=>{var c;u+=[d.firstname,d.surname_real,d.surname_real,d.country_code,((c=d.clubs)==null?void 0:c.encoded_name)||"",d.job,d.job2||"",d.note_g,d.note_d,d.note_m,d.note_a,d.rarity,d.note_min??"",d.note_max??"",d.skin,d.hair,d.hair_length,d.sell_price].map(Le).join(",")+`
`}),oe("players_export.csv",u),t(`${n.length} joueurs exportés`,"success")}),document.getElementById("import-clubs").addEventListener("change",async n=>{const i=n.target.files[0];if(!i)return;const u=document.getElementById("clubs-import-result");u.innerHTML='<span style="color:var(--gray-600)">Import en cours...</span>';try{const d=await i.text(),c=Ce(d);if(c.length===0){u.innerHTML='<span style="color:#c0392b">Fichier vide</span>';return}let r=0,b=0;const o=[];for(const g of c){if(!g.real_name||!g.encoded_name||!g.country_code){b++,o.push(`Ligne ignorée (champs manquants): ${g.real_name||"?"}`);continue}const l={real_name:g.real_name,encoded_name:g.encoded_name.toUpperCase(),country_code:g.country_code.toUpperCase().slice(0,2),logo_url:g.logo_url||null},{error:a}=await w.from("clubs").upsert(l,{onConflict:"encoded_name"});a?(b++,o.push(`${g.encoded_name}: ${a.message}`)):r++}u.innerHTML=`<div style="color:var(--green)">✅ ${r} clubs importés</div>
        ${b>0?`<div style="color:#c0392b">❌ ${b} erreurs</div><div style="font-size:11px;color:var(--gray-600);max-height:100px;overflow-y:auto">${o.slice(0,10).join("<br>")}</div>`:""}`,t(`${r} clubs importés`,"success")}catch(d){u.innerHTML=`<span style="color:#c0392b">Erreur : ${d.message}</span>`}n.target.value=""}),document.getElementById("import-players").addEventListener("change",async n=>{const i=n.target.files[0];if(!i)return;const u=document.getElementById("players-import-result");u.innerHTML='<span style="color:var(--gray-600)">Import en cours...</span>';try{const d=await i.text(),c=Ce(d);if(c.length===0){u.innerHTML='<span style="color:#c0392b">Fichier vide</span>';return}const{data:r}=await w.from("clubs").select("id,encoded_name"),b={};(r||[]).forEach(m=>{b[m.encoded_name.toUpperCase()]=m.id});let o=0,g=0;const l=[],a=["GK","DEF","MIL","ATT"],s=["normal","pepite","papyte","legende"],p=["blanc","metisse","typ","noir"],h=["blond","marron","noir","chauve"],E=["rase","court","milong","long"];for(const m of c){if(!m.firstname||!m.surname_real||!m.country_code||!m.job){g++,l.push(`Ligne ignorée (champs requis manquants): ${m.firstname||"?"}`);continue}if(!a.includes(m.job)){g++,l.push(`${m.firstname}: job invalide "${m.job}"`);continue}const $=m.club_encoded_name&&b[m.club_encoded_name.toUpperCase()]||null,k={firstname:m.firstname,surname_real:m.surname_real,surname_real:m.surname_real||ue(m.surname_real),country_code:m.country_code.toUpperCase().slice(0,2),club_id:$,job:m.job,job2:a.includes(m.job2)?m.job2:null,note_g:parseInt(m.note_g)||0,note_d:parseInt(m.note_d)||0,note_m:parseInt(m.note_m)||0,note_a:parseInt(m.note_a)||0,rarity:s.includes(m.rarity)?m.rarity:"normal",note_min:m.note_min!==""&&m.note_min!=null?parseInt(m.note_min):null,note_max:m.note_max!==""&&m.note_max!=null?parseInt(m.note_max):null,skin:p.includes(m.skin)?m.skin:"blanc",hair:h.includes(m.hair)?m.hair:"noir",hair_length:E.includes(m.hair_length)?m.hair_length:"court",sell_price:parseInt(m.sell_price)||0},{error:x}=await w.from("players").insert(k);x?(g++,l.push(`${m.firstname} ${m.surname_real}: ${x.message}`)):o++}u.innerHTML=`<div style="color:var(--green)">✅ ${o} joueurs importés</div>
...
`)?'"'+t.replace(/"/g,'""')+'"':t}function oe(e,t){const n=new Blob(["\uFEFF"+t],{type:"text/csv;charset=utf-8;"}),i=URL.createObjectURL(n),u=document.createElement("a");u.href=i,u.download=e,u.click(),URL.revokeObjectURL(i)}function Ce(e){e=e.replace(/^\uFEFF/,"").replace(/\r\n/g,`
`).replace(/\r/g,`
`);const t=e.split(`
`).filter(c=>c.trim());if(t.length<2)return[];const n=t[0],i=n.split(";").length>n.split(",").length?";":",",u=Ae(n,i).map(c=>c.trim().replace(/^\uFEFF/,"").toLowerCase()),d=[];for(let c=1;c<t.length;c++){if(!t[c].trim())continue;const r=Ae(t[c],i),b={};u.forEach((o,g)=>{b[o]=(r[g]||"").trim()}),!Object.values(b).every(o=>!o)&&d.push(b)}return d}function Ae(e,t=","){const n=[];let i="",u=!1;for(let d=0;d<e.length;d++){const c=e[d];u?c==='"'?e[d+1]==='"'?(i+='"',d++):u=!1:i+=c:c==='"'?u=!0:c===t?(n.push(i),i=""):i+=c}return n.push(i),n}const Ut={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};async function Kt(e,{toast:t}){const n=Object.keys(me);let i=n[0];const{data:u}=await w.from("formation_links_overrides").select("formation, links"),d={};(u||[]).forEach(g=>{d[g.formation]=g.links});let c=new Set;function r(g,l){return[g,l].sort().join("-")}function b(g){const l=d[g]||me[g]||[];c=new Set(l.map(([a,s])=>r(a,s)))}b(i),e.innerHTML=`
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;gap:12px;flex-wrap:wrap">
  ...
  `;function o(){const g=ft[i]||{},l=_e(i),a=320,s=400,p=22;function h(x){const f=g[x];return f?{x:f.x*a,y:f.y*s}:null}let E=`<svg width="${a}" height="${s}" viewBox="0 0 ${a} ${s}" xmlns="http://www.w3.org/2000/svg" style="display:block">`;l.forEach(([x,f],y)=>{const v=h(x),_=h(f);if(!v||!_)return;const B=r(x,f),I=c.has(B),L=I?"#3b82f6":"#999",S=I?.95:.35,A=I?4:3;E+=`<line x1="${v.x}" y1="${v.y}" x2="${_.x}" y2="${_.y}"
        ...
        opacity="${S}" pointer-events="none" data-line-key="${B}"/>`});for(const x of Object.keys(g)){const f=h(x);if(!f)continue;const y=x.replace(/\d+/,""),v=Ut[y]||"#555";E+=`<circle cx="${f.x}" cy="${f.y}" r="${p}" fill="${v}" stroke="rgba(255,255,255,0.7)" stroke-width="2" pointer-events="none"/>`,E+=`<text x="${f.x}" y="${f.y+4}" text-anchor="middle" font-size="10" font-weight="900" fill="white" font-family="Arial Black,Arial" pointer-events="none">${x}</text>`}E+="</svg>",document.getElementById("field-wrap").innerHTML=E;const m=document.getElementById("links-list");m.innerHTML=l.map(([x,f])=>{const y=r(x,f),v=c.has(y);return`
        ...
        </button>`}).join("");const $=e.querySelector('div[style*="font-weight:700;font-size:13px;margin-bottom:8px"]');$&&($.textContent=`Liens (${c.size}/${l.length} actifs)`);function k(x){c.has(x)?c.delete(x):c.add(x),o()}e.querySelectorAll(".link-hit").forEach(x=>{x.addEventListener("click",()=>k(x.dataset.key))}),e.querySelectorAll(".link-toggle").forEach(x=>{x.addEventListener("click",()=>k(x.dataset.key))})}o(),document.getElementById("formation-select").addEventListener("change",g=>{i=g.target.value,b(i),o()}),document.getElementById("reset-btn").addEventListener("click",()=>{const g=me[i]||[];c=new Set(g.map(([l,a])=>r(l,a))),o(),t("Liens réinitialisés aux valeurs par défaut (non sauvegardé)","info")}),document.getElementById("save-btn").addEventListener("click",async()=>{const l=_e(i).filter(([s,p])=>c.has(r(s,p))),{error:a}=await w.from("formation_links_overrides").upsert({formation:i,links:l,updated_at:new Date().toISOString()});if(a){t(a.message,"error");return}d[i]=l,t(`Liens enregistrés pour ${i} (${l.length} actifs)`,"success")})}const Vt=[{value:"player",label:"Joueur"},{value:"formation",label:"Formation"},{value:"game_changer",label:"Game Changer"},{value:"ultra_game_changer",label:"Ultra Game Changer"},{value:"game_helper",label:"Game Helper"},{value:"stadium",label:"🏟️ Stade"}],Jt=[{value:"",label:"Toutes"},{value:"normal",label:"Normal"},{value:"pepite",label:"Pépite"},{value:"papyte",label:"Papyte"},{value:"legende",label:"Légende"}],Wt=[{value:"credits",label:"Crédits"},{value:"pub",label:"Publicité"},{value:"free",label:"Gratuit"}];async function Yt(e){e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">Chargement...</div>';const{data:t}=await w.from("booster_configs").select("*").order("sort_order");let n=null,i=[],u=null;const d=()=>window.innerWidth<700;async function c(){if(!n){i=[];return}if(n!==u){const{data:l}=await w.from("booster_drop_rates").select("*").eq("booster_id",n).order("sort_order");i=l||[],u=n}}function r(){return`
    ...
    </div>`}function b(){const l=n?(t||[]).find(p=>p.id===n):null;if(!l)return`<div style="display:flex;align-items:center;justify-content:center;flex:1;color:#bbb;font-size:14px;padding:40px;text-align:center">
      ← Sélectionnez un booster ou créez-en un nouveau
    </div>`;const a=i.reduce((p,h)=>p+Number(h.percentage||0),0),s=Math.abs(a-100)<.1;return`
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
          <div>
            ...
            <select id="f-price-type" style="width:100%;padding:9px;border:1px solid #ddd;border-radius:6px;font-size:13px">
              ${Wt.map(p=>`<option value="${p.value}" ${l.price_type===p.value?"selected":""}>${p.label}</option>`).join("")}
            </select>
          </div>
        ...
        ${i.length===0?'<div style="text-align:center;padding:16px;color:#aaa;font-size:13px">Aucun taux. Cliquez + Ligne.</div>':i.map((p,h)=>`
          <div style="display:grid;grid-template-columns:2fr 1.5fr 50px 50px 60px 32px;gap:4px;align-items:center;margin-bottom:6px;padding:6px;background:#f9f9f9;border-radius:6px" data-rate-idx="${h}">
            <select class="rate-type" data-idx="${h}" style="padding:6px 4px;border:1px solid #ddd;border-radius:4px;font-size:12px;width:100%">
              ${Vt.map(E=>`<option value="${E.value}" ${p.card_type===E.value?"selected":""}>${E.label}</option>`).join("")}
            </select>
            <select class="rate-rarity" data-idx="${h}" style="padding:6px 4px;border:1px solid #ddd;border-radius:4px;font-size:12px;width:100%">
              ${Jt.map(E=>`<option value="${E.value}" ${(p.rarity||"")===E.value?"selected":""}>${E.label}</option>`).join("")}
            </select>
    ...
    </div>`}async function o(l=!1){l||await c();const a=!n&&d(),s=n&&d();e.innerHTML=`
    <div style="display:flex;height:calc(100vh - 56px);overflow:hidden">
      ${!d()||a||!n?r():""}
      ${!d()||s?b():""}
    </div>`,g()}function g(){var s,p,h,E,m,$,k;const l=x=>e.querySelector(x);e.querySelectorAll(".booster-row").forEach(x=>{x.addEventListener("click",f=>{f.target.closest(".btn-delete")||(n=x.dataset.id,u=null,o())})}),(s=l("#btn-back"))==null||s.addEventListener("click",()=>{n=null,o()}),(p=l("#btn-new"))==null||p.addEventListener("click",async()=>{const x=prompt("Nom du nouveau booster :");if(!(x!=null&&x.trim()))return;const{data:f,error:y}=await w.from("booster_configs").insert({name:x.trim(),price_type:"credits",price_credits:1e3,card_count:5}).select().single();if(y){alert(y.message);return}t.push(f),n=f.id,u=null,o()}),e.querySelectorAll(".btn-delete").forEach(x=>{x.addEventListener("click",async f=>{if(f.stopPropagation(),!confirm("Supprimer ce booster ?"))return;await w.from("booster_configs").delete().eq("id",x.dataset.id);const y=t.findIndex(v=>v.id===x.dataset.id);y>-1&&t.splice(y,1),n===x.dataset.id&&(n=null,u=null),o()})}),(h=l("#btn-cancel"))==null||h.addEventListener("click",()=>{n=null,o()}),(E=l("#f-price-type"))==null||E.addEventListener("change",x=>{const f=l("#credits-field");f&&(f.style.opacity=x.target.value!=="credits"?"0.4":"1")}),(m=l("#btn-pick-icon"))==null||m.addEventListener("click",async()=>{var f;const x=l("#icon-picker-grid");if(x){if(x.style.display!=="none"){x.style.display="none";return}x.innerHTML='<div style="padding:10px;text-align:center;color:#888;font-size:12px">⏳ Chargement...</div>',x.style.display="block";try{const v=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),_=Array.isArray(v)?v.filter(I=>I.name.startsWith("booster")&&/\.(png|jpg|svg|webp)$/i.test(I.name)):[];if(!_.length){x.innerHTML='<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Aucune icône "booster*" trouvée.</div>';return}const B=((f=l("#f-image-url"))==null?void 0:f.value)||"";x.innerHTML=`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(72px,1fr));gap:8px">
          ${_.map(I=>`
    ...
    </div>`,document.getElementById("add-config").addEventListener("click",()=>ze(null)),document.getElementById("form-cancel").addEventListener("click",()=>{document.getElementById("config-form").style.display="none"}),document.getElementById("form-save").addEventListener("click",()=>Qt(e)),e.querySelectorAll("[data-edit]").forEach(i=>{const u=(t||[]).find(d=>d.id===i.dataset.edit);u&&i.addEventListener("click",()=>ze(u))}),e.querySelectorAll("[data-delete]").forEach(i=>{i.addEventListener("click",async()=>{confirm("Supprimer cette règle ?")&&(await w.from("sell_price_configs").delete().eq("id",i.dataset.delete),await fe(e))})})}function Xt(e){const t=Ze.find(n=>n.value===e.rarity);return`
    ...
    </tr>`}function ze(e){document.getElementById("config-form").style.display="block",document.getElementById("form-title").textContent=e?"Modifier la règle":"Nouvelle règle",document.getElementById("form-id").value=(e==null?void 0:e.id)||"",document.getElementById("form-rarity").value=(e==null?void 0:e.rarity)||"normal",document.getElementById("form-price").value=(e==null?void 0:e.price)||1e3,document.getElementById("form-note-min").value=(e==null?void 0:e.note_min)??1,document.getElementById("form-note-max").value=(e==null?void 0:e.note_max)??10,document.getElementById("config-form").scrollIntoView({behavior:"smooth"})}async function Qt(e){const t=document.getElementById("form-id").value,n=document.getElementById("form-rarity").value,i=parseInt(document.getElementById("form-price").value)||0,u=parseInt(document.getElementById("form-note-min").value)||1,d=parseInt(document.getElementById("form-note-max").value)||10;if(u>d){alert("Note min doit être ≤ note max");return}const c={rarity:n,price:i,note_min:u,note_max:d,updated_at:new Date().toISOString()};let r;if(t?{error:r}=await w.from("sell_price_configs").update(c).eq("id",t):{error:r}=await w.from("sell_price_configs").insert(c),r){alert("Erreur : "+r.message);return}document.getElementById("config-form").style.display="none",await fe(e)}async function en(e){await be(e)}async function be(e){const{data:t}=await w.from("patch_notes").select("*").order("published_at",{ascending:!1});e.innerHTML=`
    ...
    </div>`,document.getElementById("add-article").addEventListener("click",()=>Me(null)),document.getElementById("form-cancel").addEventListener("click",()=>{document.getElementById("article-form").style.display="none"}),document.getElementById("form-save").addEventListener("click",()=>nn(e)),e.querySelectorAll("[data-edit]").forEach(n=>{const i=(t||[]).find(u=>u.id===n.dataset.edit);i&&n.addEventListener("click",()=>Me(i))}),e.querySelectorAll("[data-delete]").forEach(n=>{n.addEventListener("click",async()=>{confirm("Supprimer cet article ?")&&(await w.from("patch_notes").delete().eq("id",n.dataset.delete),await be(e))})})}function tn(e){const t=new Date(e.published_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"short",year:"numeric"});return`
  ...
  <div style="background:#fff;border-radius:10px;padding:12px 16px;box-shadow:0 1px 4px rgba(0,0,0,0.08);display:flex;align-items:center;gap:12px">
  ...
  </div>`}function Te(e,t=0){document.getElementById("ts-form").style.display="block",document.getElementById("ts-form-title").textContent=e?"Modifier l'étape":"Nouvelle étape",document.getElementById("ts-id").value=(e==null?void 0:e.id)||"",document.getElementById("ts-emoji").value=(e==null?void 0:e.emoji)||"⚽",document.getElementById("ts-title").value=(e==null?void 0:e.title)||"",document.getElementById("ts-content").value=(e==null?void 0:e.content)||"";const n=document.getElementById("ts-editor");n&&(n.innerHTML=(e==null?void 0:e.content)||"<p>Texte de l'étape…</p>"),document.getElementById("ts-order").value=(e==null?void 0:e.step_order)??t,document.getElementById("ts-active").checked=e?e.is_active:!0;const i=(e==null?void 0:e.color)||"#1A6B3C";document.getElementById("ts-color").value=i,document.getElementById("ts-color-hex").value=i;const u=(e==null?void 0:e.image_url)||"",d=document.getElementById("ts-image");d&&(d.value=u);const c=document.getElementById("ts-img-preview"),r=document.getElementById("ts-img-preview-el");if(u&&c&&r){const b="/";r.src=`${b}icons/${u}`,c.style.display="block"}else c&&(c.style.display="none");document.getElementById("ts-img-picker-grid").style.display="none",document.getElementById("ts-preview-area").innerHTML="",document.getElementById("ts-form").scrollIntoView({behavior:"smooth"})}function Xe(){document.getElementById("ts-form").style.display="none",document.getElementById("ts-preview-area").innerHTML=""}function rn(){var o,g;const e=document.getElementById("ts-emoji").value||"⚽",t=document.getElementById("ts-title").value||"Titre",n=document.getElementById("ts-editor"),i=n?n.innerHTML:document.getElementById("ts-content").value||"",u=document.getElementById("ts-color-hex").value||"#1A6B3C",d=(g=(o=document.getElementById("ts-image"))==null?void 0:o.value)==null?void 0:g.trim(),r=d?`<div style="padding:0 20px 12px;text-align:center"><img src="/icons/${d}" style="max-height:140px;max-width:100%;border-radius:10px;object-fit:contain"></div>`:"",b=document.getElementById("ts-preview-area");b.innerHTML=`
    ...
    </div>`,b.querySelectorAll("ul,ol").forEach(l=>{l.style.paddingLeft="20px",l.style.margin="6px 0"}),b.querySelectorAll("li").forEach(l=>{l.style.marginBottom="4px"}),b.querySelectorAll("p").forEach(l=>{l.style.marginBottom="8px"})}async function ln(e){var l,a;const t=document.getElementById("ts-id").value,n=document.getElementById("ts-emoji").value.trim()||"⚽",i=document.getElementById("ts-title").value.trim(),u=document.getElementById("ts-editor"),d=(u?u.innerHTML:document.getElementById("ts-content").value).trim(),c=document.getElementById("ts-color-hex").value.trim()||"#1A6B3C",r=parseInt(document.getElementById("ts-order").value)||0,b=document.getElementById("ts-active").checked;if(!i||!d){alert("Titre et contenu sont obligatoires.");return}const o={emoji:n,title:i,content:d,color:c,step_order:r,is_active:b,image_url:((a=(l=document.getElementById("ts-image"))==null?void 0:l.value)==null?void 0:a.trim())||null};let g;if(t?{error:g}=await w.from("tutorial_steps").update(o).eq("id",t):{error:g}=await w.from("tutorial_steps").insert(o),g){alert("Erreur : "+g.message);return}Xe(),J(e)}const sn="/",dn=[["MA","Maroc"],["FR","France"],["AR","Argentine"],["PT","Portugal"],["BR","Brésil"],["ES","Espagne"],["DE","Allemagne"],["GB","Angleterre"],["IT","Italie"],["CM","Cameroun"],["SN","Sénégal"],["NG","Nigéria"],["DK","Danemark"],["NL","Pays-Bas"],["BE","Belgique"],["CI","Côte d'Ivoire"],["AL","Albanie"],["HR","Croatie"],["RS","Serbie"],["TR","Turquie"],["MW","Malawi"],["GH","Ghana"],["ZA","Afrique du Sud"],["EG","Égypte"],["US","États-Unis"],["MX","Mexique"],["CO","Colombie"],["UY","Uruguay"],["CL","Chili"],["JP","Japon"],["KR","Corée du Sud"],["CN","Chine"],["AU","Australie"],["SW","Suisse"],["SE","Suède"],["NO","Norvège"],["PL","Pologne"]];async function cn(e){await ye(e)}async function ye(e){var i,u,d,c;const[{data:t},{data:n}]=await Promise.all([w.from("stadium_definitions").select("*, club:clubs(encoded_name, logo_url)").order("name"),w.from("clubs").select("id, encoded_name, logo_url").order("encoded_name")]);e.innerHTML=`
    <div style="padding:20px;max-width:820px;margin:0 auto">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px">
          ...
          <div style="flex:1;min-width:220px;display:flex;flex-direction:column;gap:12px">
            ...
            <div>
              ...
              <select id="st-country" style="width:100%;box-sizing:border-box;padding:9px;border:1.5px solid #ddd;border-radius:8px;font-size:13px;background:#fff">
                ...
                ${dn.map(([r,b])=>`<option value="${r}">${b} (${r})</option>`).join("")}
              </select>
            </div>
            <div>
    ...
    </div>`,(i=document.getElementById("st-add-btn"))==null||i.addEventListener("click",()=>Re(null)),(u=document.getElementById("st-cancel"))==null||u.addEventListener("click",()=>{document.getElementById("st-form").style.display="none"}),(d=document.getElementById("st-save"))==null||d.addEventListener("click",()=>un(e)),(c=document.getElementById("st-preview-btn"))==null||c.addEventListener("click",()=>se()),["st-name","st-club","st-country","st-image"].forEach(r=>{var b,o;(b=document.getElementById(r))==null||b.addEventListener("input",se),(o=document.getElementById(r))==null||o.addEventListener("change",se)}),e.querySelectorAll("[data-edit]").forEach(r=>{const b=(t||[]).find(o=>o.id===r.dataset.edit);b&&r.addEventListener("click",()=>Re(b))}),e.querySelectorAll("[data-delete]").forEach(r=>{r.addEventListener("click",async()=>{confirm("Supprimer ce stade ?")&&(await w.from("stadium_definitions").delete().eq("id",r.dataset.delete),ye(e))})})}function pn(e){var n,i;let t;return(n=e.club)!=null&&n.logo_url?t=`<img src="${e.club.logo_url}" style="width:32px;height:32px;object-fit:contain">`:e.country_code?t=`<img src="https://flagsapi.com/${e.country_code.slice(0,2).toUpperCase()}/flat/32.png" style="width:32px;height:24px;object-fit:contain;border-radius:3px" onerror="this.style.display='none'">`:t='<span style="font-size:20px">🌍</span>',`
    <div style="background:#fff;border-radius:10px;padding:12px 16px;box-shadow:0 1px 4px rgba(0,0,0,0.08);display:flex;align-items:center;gap:12px">
      ...
      <button data-edit="${e.id}" class="btn btn-ghost btn-sm">✏️</button>
      <button data-delete="${e.id}" class="btn btn-ghost btn-sm" style="color:#cc2222">🗑️</button>
    </div>`}function Re(e,t){var i,u;document.getElementById("st-form").style.display="block",document.getElementById("st-form-title").textContent=e?"Modifier le stade":"Nouveau stade",document.getElementById("st-id").value=(e==null?void 0:e.id)||"",document.getElementById("st-name").value=(e==null?void 0:e.name)||"",document.getElementById("st-club").value=(e==null?void 0:e.club_id)||"";const n=document.getElementById("st-country");n&&(n.value=(e==null?void 0:e.country_code)||""),document.getElementById("st-image").value=(e==null?void 0:e.image_url)||"",document.getElementById("st-preview-card").innerHTML="",se((i=e==null?void 0:e.club)==null?void 0:i.logo_url,(u=e==null?void 0:e.club)==null?void 0:u.encoded_name),document.getElementById("st-form").scrollIntoView({behavior:"smooth"})}function se(){var o,g,l,a,s,p,h;const e=((o=document.getElementById("st-name"))==null?void 0:o.value)||"NOM DU STADE",t=(l=(g=document.getElementById("st-image"))==null?void 0:g.value)==null?void 0:l.trim(),n=(s=(a=document.getElementById("st-country"))==null?void 0:a.value)==null?void 0:s.trim(),i=document.getElementById("st-club"),u=(i==null?void 0:i.selectedIndex)||0,d=i&&u>0?i.options[u].text:"",c=((h=(p=i==null?void 0:i.options[u])==null?void 0:p.getAttribute)==null?void 0:h.call(p,"data-logo"))||"";let r=null;t?r=`${sn}icons/${t}`:c?r=c:n&&(r=`https://flagsapi.com/${n.toLowerCase().slice(0,2).toUpperCase()}/flat/64.png`);const b=d||n||"—";document.getElementById("st-preview-card").innerHTML=bt(e,r,`${b}<br>+10 ⭐`,{width:140})}async function un(e){const t=document.getElementById("st-id").value,n=document.getElementById("st-name").value.trim(),i=document.getElementById("st-club").value||null,u=document.getElementById("st-country").value.trim().toUpperCase()||null,d=document.getElementById("st-image").value.trim()||null;if(!n){alert("Le nom est obligatoire");return}const c={name:n,club_id:i,country_code:u,image_url:d},{error:r}=t?await w.from("stadium_definitions").update(c).eq("id",t):await w.from("stadium_definitions").insert(c);if(r){alert("Erreur : "+r.message);return}document.getElementById("st-form").style.display="none",ye(e)}const O=[{value:"game_changer",label:"⚡ Game Changer",border:"#7a28b8",bg:"linear-gradient(135deg,#4a0080,#7a28b8)"},{value:"ultra_game_changer",label:"💎 Ultra Game Changer",border:"#00bcd4",bg:"linear-gradient(135deg,#006080,#00bcd4)"}],ie=[{value:"purple",label:"Violet",hex:"#7a28b8"},{value:"light_blue",label:"Bleu clair",hex:"#00bcd4"}];async function mn(e){e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">Chargement...</div>';const{data:t}=await w.from("gc_definitions").select("*").order("sort_order").order("created_at");let n=null;const i=[{value:"BOOST_STAT",label:"⚡ Boost stats (+N)",hasValue:!0,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"DEBUFF_STAT",label:"💀 Debuff stats (-N)",hasValue:!0,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"GRAY_PLAYER",label:"❄️ Griser un joueur",hasValue:!1,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"REVIVE_PLAYER",label:"💫 Ressusciter un joueur",hasValue:!1,hasTarget:!1,hasCount:!0,hasRoles:!1},{value:"REMOVE_GOAL",label:"🚫 Retirer un but adverse",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1},{value:"ADD_GOAL_DRAW",label:"⚽ +1 but si duel nul",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1},{value:"ADD_SUB",label:"🔄 Ajouter un remplacement",hasValue:!1,hasTarget:!1,hasCount:!0,hasRoles:!1},{value:"CUSTOM",label:"🛠️ Effet custom (hardcodé)",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1}],u=["GK","DEF","MIL","ATT"],d=()=>window.innerWidth<700,c=a=>e.querySelector(a);function r(a,s=130){if(!a)return"";const p=O.find(E=>E.value===a.gc_type)||O[0],h=a.image_url?`/icons/${a.image_url}`:null;return yt(a.name||"?",h,p.label.split(" ")[0],a.effect||"",{width:s})}function b(){return`
    <div id="gc-list" style="background:#fff;${d()?"":"border-right:1px solid #e0e0e0;width:280px;flex-shrink:0;"}overflow-y:auto;height:100%">
      ...
      ${(t||[]).map(a=>{const s=O.find(h=>h.value===a.gc_type)||O[0],p=ie.find(h=>h.value===a.color)||ie[0];return`
        <div class="gc-row" data-id="${a.id}"
          ...
          <div style="width:36px;height:50px;border-radius:5px;border:2px solid ${p.hex};background:${s.bg};display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:18px">
    ...
    </div>`}function o(){const a=n?(t||[]).find(s=>s.id===n):null;return a?(O.find(s=>s.value===a.gc_type)||O[0],ie.find(s=>s.value===a.color)||ie[0],`
    <div style="flex:1;overflow-y:auto;background:#f8f8f8;padding:14px">
      ...
      <div style="background:#fff;border-radius:10px;padding:14px;margin-bottom:14px;display:flex;flex-direction:column;gap:12px">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
          <div>
            ...
            <select id="gc-type" style="width:100%;padding:9px;border:1px solid #ddd;border-radius:6px;font-size:13px">
              ${O.map(s=>`<option value="${s.value}" ${a.gc_type===s.value?"selected":""}>${s.label}</option>`).join("")}
            </select>
          </div>
          <div>
            ...
            <select id="gc-color" style="width:100%;padding:9px;border:1px solid #ddd;border-radius:6px;font-size:13px">
              ${ie.map(s=>`<option value="${s.value}" ${a.color===s.value?"selected":""}>${s.label}</option>`).join("")}
            </select>
          </div>
        </div>
        ...
        <div style="border-top:1px solid #eee;padding-top:12px;margin-top:4px">
          ...
          ${(()=>{const s=a.effect_params||{},p=i.find(h=>h.value===(a.effect_type||"BOOST_STAT"))||i[0];return`
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
              ${p.hasValue?`<div>
    ...
    <div style="display:flex;height:calc(100vh - 56px);overflow:hidden">
      ${!d()||!n?b():""}
      ${!d()||n?o():""}
    </div>`,l()}function l(){var a,s,p,h,E;e.querySelectorAll(".gc-row").forEach(m=>{m.addEventListener("click",$=>{$.target.closest(".btn-toggle-gc,.btn-delete-gc")||(n=m.dataset.id,g())})}),(a=c("#btn-gc-back"))==null||a.addEventListener("click",()=>{n=null,g()}),(s=c("#btn-cancel-gc"))==null||s.addEventListener("click",()=>{n=null,g()}),(p=c("#btn-new-gc"))==null||p.addEventListener("click",async()=>{const m=prompt("Nom de la carte Game Changer :");if(!(m!=null&&m.trim()))return;const{data:$,error:k}=await w.from("gc_definitions").insert({name:m.trim(),gc_type:"game_changer",color:"purple"}).select().single();if(k){alert(k.message);return}t.push($),n=$.id,g()}),e.querySelectorAll(".btn-toggle-gc").forEach(m=>{m.addEventListener("click",async $=>{$.stopPropagation();const k=m.dataset.active!=="true";await w.from("gc_definitions").update({is_active:k}).eq("id",m.dataset.id);const x=t.find(f=>f.id===m.dataset.id);x&&(x.is_active=k),g()})}),e.querySelectorAll(".btn-delete-gc").forEach(m=>{m.addEventListener("click",async $=>{if($.stopPropagation(),!confirm("Supprimer cette carte Game Changer ?"))return;await w.from("gc_definitions").delete().eq("id",m.dataset.id);const k=t.findIndex(x=>x.id===m.dataset.id);k>-1&&t.splice(k,1),n===m.dataset.id&&(n=null),g()})}),(h=c("#btn-pick-gc-icon"))==null||h.addEventListener("click",async()=>{var $;const m=c("#gc-icon-picker");if(m){if(m.style.display!=="none"){m.style.display="none";return}m.innerHTML='<div style="padding:10px;text-align:center;color:#888;font-size:12px">⏳ Chargement...</div>',m.style.display="block";try{const x=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),f=Array.isArray(x)?x.filter(v=>v.name.startsWith("gamechanger-")&&/\.(png|jpg|svg|webp)$/i.test(v.name)):[];if(!f.length){m.innerHTML='<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Aucun fichier "gamechanger-*" trouvé dans public/icons/</div>';return}const y=(($=c("#gc-image-url"))==null?void 0:$.value)||"";m.innerHTML=`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(72px,1fr));gap:8px">
          ${f.map(v=>`
        ...
        <div style="display:flex;flex-direction:column;gap:4px">
          ${a.map(f=>{const y=E[f],v=l.length>0?Math.round(y/l.length*100):0;return`
              <div style="display:flex;align-items:center;gap:8px;font-size:12px">
                <span style="width:80px">${h[f]} ${s[f]}</span>
                <div style="flex:1;background:rgba(255,255,255,0.08);border-radius:4px;height:14px;overflow:hidden">
                  <div style="height:100%;width:${v}%;background:${p[f]};border-radius:4px;transition:width 0.5s"></div>
                </div>
    ...
    </div>`,(x=document.getElementById("create-season-btn"))==null||x.addEventListener("click",()=>{je(null,{toast:n,openModal:i,closeModal:u,reload:()=>V(e,t)})}),e.querySelectorAll("[data-edit]").forEach(f=>{const y=b.find(v=>v.id==f.dataset.edit);f.addEventListener("click",()=>{je(y,{toast:n,openModal:i,closeModal:u,reload:()=>V(e,t)})})}),e.querySelectorAll("[data-activate]").forEach(f=>{f.addEventListener("click",async()=>{const y=parseInt(f.dataset.activate);if(!confirm("Activer cette saison ? La saison actuellement active sera désactivée."))return;const{error:v}=await w.from("ranked_seasons").update({is_active:!1}).neq("id",0);if(v){n(v.message,"error");return}const{error:_}=await w.from("ranked_seasons").update({is_active:!0}).eq("id",y);if(_){n(_.message,"error");return}await w.from("users").update({current_season_id:y}).gt("placement_matches",0),n("Saison activée ✅","success"),V(e,t)})}),e.querySelectorAll("[data-deactivate]").forEach(f=>{f.addEventListener("click",async()=>{if(!confirm("Désactiver la saison active ? Le mode Ranked sera suspendu."))return;const{error:y}=await w.from("ranked_seasons").update({is_active:!1}).eq("id",parseInt(f.dataset.deactivate));if(y){n(y.message,"error");return}n("Saison désactivée","success"),V(e,t)})}),e.querySelectorAll("[data-delete]").forEach(f=>{f.addEventListener("click",async()=>{if(!confirm("Supprimer cette saison archivée ?"))return;const{error:y}=await w.from("ranked_seasons").delete().eq("id",parseInt(f.dataset.delete));if(y){n(y.message,"error");return}n("Saison supprimée","success"),V(e,t)})})}function je(e,{toast:t,openModal:n,closeModal:i,reload:u}){const d=!!e,c=new Date().toISOString().slice(0,16),r=new Date(Date.now()+90*864e5).toISOString().slice(0,16),b=e?new Date(e.start_at).toISOString().slice(0,16):c,o=e?new Date(e.end_at).toISOString().slice(0,16):r,g=(e==null?void 0:e.name)||"";n(d?"✏️ Modifier la saison":"➕ Nouvelle saison",`<div style="display:flex;flex-direction:column;gap:14px">
      <div class="form-group">
    ...
    </div>`);function l(){var E,m;const a=(E=document.getElementById("season-start"))==null?void 0:E.value,s=(m=document.getElementById("season-end"))==null?void 0:m.value,p=document.getElementById("season-duration");if(!a||!s||!p)return;const h=Math.round((new Date(s)-new Date(a))/864e5);p.textContent=h>0?`Durée : ${h} jour${h>1?"s":""}`:"⚠️ La fin doit être après le début",p.style.color=h>0?"var(--gray-600)":"#bb2020"}setTimeout(()=>{var a,s,p;(a=document.getElementById("season-start"))==null||a.addEventListener("input",l),(s=document.getElementById("season-end"))==null||s.addEventListener("input",l),l(),(p=document.getElementById("season-save-btn"))==null||p.addEventListener("click",async()=>{var y,v,_;const h=(y=document.getElementById("season-name"))==null?void 0:y.value.trim(),E=(v=document.getElementById("season-start"))==null?void 0:v.value,m=(_=document.getElementById("season-end"))==null?void 0:_.value,$=document.getElementById("season-error");if(!h){$.textContent="Le nom est requis.";return}if(!E){$.textContent="La date de début est requise.";return}if(!m){$.textContent="La date de fin est requise.";return}if(new Date(m)<=new Date(E)){$.textContent="La date de fin doit être après le début.";return}const k=document.getElementById("season-save-btn");k.disabled=!0,k.textContent="Enregistrement…";const x={name:h,start_at:new Date(E).toISOString(),end_at:new Date(m).toISOString()};let f;if(d?{error:f}=await w.from("ranked_seasons").update(x).eq("id",e.id):{error:f}=await w.from("ranked_seasons").insert({...x,is_active:!1}),f){$.textContent=f.message,k.disabled=!1,k.textContent=d?"💾 Enregistrer":"✅ Créer la saison";return}t(d?"Saison modifiée ✅":"Saison créée ✅","success"),i(),u()})},50)}vt(Ge);function fn(e,t="info"){const n=document.getElementById("toast");n.textContent=e,n.className=`show ${t}`,clearTimeout(n._t),n._t=setTimeout(()=>{n.className=""},3e3)}function bn(e,t,n=""){document.getElementById("modal-title").textContent=e,document.getElementById("modal-body").innerHTML=t,document.getElementById("modal-footer").innerHTML=n,document.getElementById("modal-overlay").classList.remove("hidden")}function ge(){document.getElementById("modal-overlay").classList.add("hidden")}const De={dashboard:{title:"Dashboard",fn:xt},players:{title:"Joueurs & Cartes",fn:wt},clubs:{title:"Clubs",fn:Tt},"card-builder":{title:"Card Builder",fn:Nt},users:{title:"Managers",fn:We},market:{title:"Marché des transferts",fn:Ye},"import-export":{title:"Import / Export CSV",fn:Ot},formations:{title:"Formations & Liens",fn:Kt},"boosters-config":{title:"Boosters",fn:Yt},"sell-price":{title:"Prix vente directe",fn:Zt},journal:{title:"Journal",fn:en},tutorial:{title:"Tutoriel",fn:on},stadiums:{title:"Stades",fn:cn},"gc-cards":{title:"Game Changers",fn:mn},"ranked-seasons":{title:"Saisons Ranked",fn:gn}};async function pe(e){document.querySelectorAll(".admin-sidebar nav a").forEach(u=>{u.classList.toggle("active",u.dataset.page===e)});const t=document.getElementById("mobile-page-select");t&&(t.value=e);const n=De[e]||De.dashboard;document.getElementById("page-title").textContent=n.title;const i=document.getElementById("page-content");i.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">Chargement…</div>';try{await n.fn(i,{toast:fn,openModal:bn,closeModal:ge,navigate:pe})}catch(u){i.innerHTML=`<div style="padding:40px;color:#c0392b">Erreur: ${u.message}</div>`,console.error(u)}}async function yn(){var t;const{data:{session:e}}=await w.auth.getSession();e&&await Pe(e.user),document.getElementById("auth-btn").addEventListener("click",async()=>{const n=document.getElementById("auth-email").value.trim(),i=document.getElementById("auth-password").value,u=document.getElementById("auth-error");if(u.textContent="",!n||!i){u.textContent="Remplissez tous les champs.";return}const{data:d,error:c}=await w.auth.signInWithPassword({email:n,password:i});if(c){u.textContent=c.message;return}await Pe(d.user)}),document.getElementById("auth-password").addEventListener("keydown",n=>{n.key==="Enter"&&document.getElementById("auth-btn").click()}),document.getElementById("logout-btn").addEventListener("click",async()=>{await w.auth.signOut(),document.getElementById("auth-screen").style.display="flex",document.getElementById("admin-app").style.display="none"}),document.getElementById("modal-close").addEventListener("click",ge),document.getElementById("modal-overlay").addEventListener("click",n=>{n.target===n.currentTarget&&ge()}),document.querySelectorAll(".admin-sidebar nav a").forEach(n=>{n.addEventListener("click",i=>{i.preventDefault(),pe(n.dataset.page)})}),(t=document.getElementById("mobile-page-select"))==null||t.addEventListener("change",n=>{pe(n.target.value)})}async function Pe(e){const{data:t,error:n}=await w.from("users").select("*").eq("id",e.id).single(),i=document.getElementById("auth-error");if(n||!t){i.textContent="Profil introuvable.";return}if(!t.is_admin){i.textContent="Accès refusé. Vous n'êtes pas administrateur.",await w.auth.signOut();return}document.getElementById("auth-screen").style.display="none",document.getElementById("admin-app").style.display="flex",document.getElementById("admin-user").textContent=`Admin: ${t.pseudo}`,pe("dashboard")}yn();
