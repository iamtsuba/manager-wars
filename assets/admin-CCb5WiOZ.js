import{s as _,r as ve,a as qe,A as st,l as dt,g as ct,K as ke,D as R,b as ut,S as pt,c as D,H as mt,R as gt,e as de,d as Ke,f as ft,h as yt,E as bt,i as vt,N as xt,M as ht,B as Et,j as _t,F as ue,k as Be,m as wt,n as Je,o as me,p as It}from"./special-cards-hvDhyFrj.js";async function $t(e){var h;const[{count:t},{count:n},{count:i},{count:r},{count:a},{data:g}]=await Promise.all([_.from("players").select("*",{count:"exact",head:!0}),_.from("clubs").select("*",{count:"exact",head:!0}),_.from("users").select("*",{count:"exact",head:!0}),_.from("stadium_definitions").select("*",{count:"exact",head:!0}),_.from("users").select("*",{count:"exact",head:!0}).gt("last_seen_at",new Date(Date.now()-18e4).toISOString()),_.rpc("get_signup_password")]),m=g||"";e.innerHTML=`
...
Cette action est irréversible.`))return;const i=document.getElementById("fix-old-faces-btn");i&&(i.disabled=!0,i.textContent="⏳ En cours...");try{const{data:r,error:a}=await _.from("players").select("id, country_code, face").like("face","public/faces/%");if(a){n(a.message,"error");return}if(!(r!=null&&r.length)){n("Aucun joueur avec l'ancien format — rien à faire ✅","success");return}const{data:g}=await _.from("players").select("face").not("face","is",null),m=new Set((g||[]).map(u=>u.face).filter(u=>u&&!u.startsWith("public/faces/")));let h=0,o=0;for(const u of r){const l=await qe(u.country_code,m);if(!l){o++;continue}const{error:f}=await _.from("players").update({face:l}).eq("id",u.id);if(f){o++;continue}m.add(l),h++}n(`${h} photo(s) réattribuée(s)${o?`, ${o} échec(s)`:""} ✅`,o?"error":"success")}finally{i&&(i.disabled=!1,i.textContent="🖼️ Réattribuer anciennes photos")}}async function At(e,t){await le(e,t)}async function le(e,t,n=null){var o,u,l,f,p;const{toast:i}=t;n||(n={search:((o=document.getElementById("search-players"))==null?void 0:o.value)||"",job:((u=document.getElementById("filter-job"))==null?void 0:u.value)||"",rarity:((l=document.getElementById("filter-rarity"))==null?void 0:l.value)||"",club:((f=document.getElementById("filter-club"))==null?void 0:f.value)||"",country:((p=document.getElementById("filter-country"))==null?void 0:p.value)||""});const[{data:r,error:a},{data:g}]=await Promise.all([_.from("players").select("*, clubs(id,encoded_name,logo_url)"),_.from("clubs").select("id,encoded_name").order("encoded_name")]);if(a){e.innerHTML=`<p style="color:red">${a.message}</p>`;return}const m={GK:0,DEF:1,MIL:2,ATT:3},h=(r||[]).sort((s,E)=>{const y=(m[s.job]??4)-(m[E.job]??4);return y!==0?y:(s.surname_real||"").localeCompare(E.surname_real||"")});St(e,h,g||[],t,n)}function St(e,t,n,i,r=null){var u,l;const{toast:a}=i;if(e.innerHTML=`
    <div style="display:flex;flex-direction:column;gap:10px">
    ...
    </div>`,r){const f=r,p=s=>document.getElementById(s);f.search&&p("search-players")&&(p("search-players").value=f.search),f.job&&p("filter-job")&&(p("filter-job").value=f.job),f.rarity&&p("filter-rarity")&&(p("filter-rarity").value=f.rarity),f.club&&p("filter-club")&&(p("filter-club").value=f.club),f.country&&p("filter-country")&&(p("filter-country").value=f.country)}if(r){const f=r,p=s=>document.getElementById(s);f.search&&p("search-players")&&(p("search-players").value=f.search),f.job&&p("filter-job")&&(p("filter-job").value=f.job),f.rarity&&p("filter-rarity")&&(p("filter-rarity").value=f.rarity),f.club&&p("filter-club")&&(p("filter-club").value=f.club),f.country&&p("filter-country")&&(p("filter-country").value=f.country)}function g(){const f=document.getElementById("search-players").value.toLowerCase(),p=document.getElementById("filter-job").value,s=document.getElementById("filter-rarity").value,E=document.getElementById("filter-club").value,y=document.getElementById("filter-country").value;return t.filter(v=>(!f||`${v.firstname} ${v.surname_real}`.toLowerCase().includes(f))&&(!p||v.job===p)&&(!s||v.rarity===s)&&(!E||v.club_id===E)&&(!y||v.country_code===y))}const m=new Set;function h(){const f=document.getElementById("bulk-bar"),p=document.getElementById("bulk-count");f&&(f.style.display=m.size>0?"flex":"none",p&&(p.textContent=`${m.size} joueur(s) sélectionné(s)`))}function o(){const f=g();document.getElementById("count-label").textContent=`${f.length} joueur(s)`;const p=document.getElementById("players-list");if(!f.length){p.innerHTML='<div style="color:var(--gray-600);padding:20px;text-align:center">Aucun joueur.</div>';return}p.innerHTML=f.map(s=>{const E={...s,clubs:s.clubs,face:s.face||null},y=ve(E,{width:120});return`<div style="position:relative;cursor:pointer" data-edit="${s.id}">
        ${y}
      ...
      <div style="flex:1;min-width:300px;display:flex;flex-direction:column;gap:12px">
        ...
        <div style="border-top:1px solid var(--gray-200);padding-top:10px">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:8px">
            ${[["GK","pm-g","note_g"],["DEF","pm-d","note_d"],["MIL","pm-m","note_m"],["ATT","pm-a","note_a"]].map(([s,E,y])=>`
              <div class="form-group" style="text-align:center">
                <label style="color:${kt[s]};font-weight:700">${s}</label>
                <input id="${E}" type="number" min="0" max="20" value="${(e==null?void 0:e[y])??0}" style="text-align:center;font-size:18px;font-weight:900;padding:6px 4px">
              </div>`).join("")}
          </div>
      ...
      <div style="border-top:1px solid var(--tile-border);padding-top:12px">
        ...
        <div style="display:flex;gap:12px;align-items:flex-start;flex-wrap:wrap">
          <div style="flex:1;min-width:200px;display:flex;flex-direction:column;gap:8px">
            ...
            ${[["Couleur 1","m-kit-color1",a.color1,!1],["Couleur 2","m-kit-color2",a.color2,!1],["Couleur 3","m-kit-color3",a.color3,!0],["Short","m-kit-shorts",a.shorts,!1],["Chaussettes","m-kit-socks",a.socks,!1]].map(([d,c,b,w])=>`
              <div class="form-group" id="wrap-${c}" ${w?'style="display:none"':""}>
                <label>${d}</label>
                <div style="display:flex;gap:6px;align-items:center">
                  <input type="color" id="${c}" value="${b||"#000000"}" style="width:38px;height:32px;padding:2px;border:1px solid var(--tile-border);border-radius:6px;cursor:pointer;flex-shrink:0">
                  <input id="${c}-txt" value="${b||"#000000"}" maxlength="7" style="flex:1;font-family:monospace;font-size:12px">
                </div>
              </div>`).join("")}
          </div>
        ...
        <div style="display:grid;grid-template-columns:1fr 120px;gap:16px;align-items:start">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
            ...
            <div>
              ...
              <select id="av-hair">${g("av-hair",yt,D.hair)}</select>
            </div>
            <div>
            ...
            <div>
              ...
              <select id="av-eyebrows">${g("av-eyebrows",bt,D.eyebrows)}</select>
            </div>
            <div>
              <label>Yeux</label>
  ...
  `,a(r),document.getElementById("search-users").addEventListener("input",g=>{const m=g.target.value.toLowerCase();a(r.filter(h=>{var o;return h.pseudo.toLowerCase().includes(m)||((o=h.club_name)==null?void 0:o.toLowerCase().includes(m))}))});function a(g){document.getElementById("users-tbody").innerHTML=g.map(m=>{const h=m.mmr??1e3,o=m.mmr_deviation??350,u=m.placement_matches??0,l=_t(h),f=m.ranked_wins??0,p=m.ranked_losses??0,s=m.ranked_draws??0,E=f+p+s,y=E>0?Math.round(f/E*100):null,v=u<10,$=o<80?"#1A6B3C":o<150?"#e67e22":"#bb2020";return`
        <tr>
          <td>
          ...
          <td style="font-size:12px">
            ...
            ${v?`<div style="font-size:10px;color:#e67e22">${u}/10 placement${u<10?` (${10-u} restants)`:""}</div>`:`<div style="font-size:10px;color:var(--gray-600)">${f}V ${s}N ${p}D${y!==null?` · ${y}% WR`:""}</div>`}
          </td>
    ...
    <div class="card-panel">
      <div class="table-wrap">
        <table>
          ...
          <tbody>
            ${a.map(o=>{var p,s,E,y;const u=(p=o.card)==null?void 0:p.player,l=u?`${u.firstname} ${u.surname_real}`:((s=o.card)==null?void 0:s.card_type)||"—",f={active:"#1A6B3C",sold:"#D4A017",cancelled:"#888"};return`<tr>
                ...
                <td style="font-size:12px">${((y=o.buyer)==null?void 0:y.pseudo)||"—"}</td>
...
`).filter(g=>g.trim());if(t.length<2)return[];const n=t[0],i=n.split(";").length>n.split(",").length?";":",",r=Re(n,i).map(g=>g.trim().replace(/^\uFEFF/,"").toLowerCase()),a=[];for(let g=1;g<t.length;g++){if(!t[g].trim())continue;const m=Re(t[g],i),h={};r.forEach((o,u)=>{h[o]=(m[u]||"").trim()}),!Object.values(h).every(o=>!o)&&a.push(h)}return a}function Re(e,t=","){const n=[];let i="",r=!1;for(let a=0;a<e.length;a++){const g=e[a];r?g==='"'?e[a+1]==='"'?(i+='"',a++):r=!1:i+=g:g==='"'?r=!0:g===t?(n.push(i),i=""):i+=g}return n.push(i),n}const an={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};async function rn(e,{toast:t}){const n=Object.keys(ue);let i=n[0];const{data:r}=await _.from("formation_links_overrides").select("formation, links"),a={};(r||[]).forEach(u=>{a[u.formation]=u.links});let g=new Set;function m(u,l){return[u,l].sort().join("-")}function h(u){const l=a[u]||ue[u]||[];g=new Set(l.map(([f,p])=>m(f,p)))}h(i),e.innerHTML=`
  ...
  `;function o(){const u=wt[i]||{},l=Be(i),f=320,p=400,s=22;function E(x){const d=u[x];return d?{x:d.x*f,y:d.y*p}:null}let y=`<svg width="${f}" height="${p}" viewBox="0 0 ${f} ${p}" xmlns="http://www.w3.org/2000/svg" style="display:block">`;l.forEach(([x,d],c)=>{const b=E(x),w=E(d);if(!b||!w)return;const B=m(x,d),k=g.has(B),A=k?"#3b82f6":"#999",L=k?.95:.35,S=k?4:3;y+=`<line x1="${b.x}" y1="${b.y}" x2="${w.x}" y2="${w.y}"
        stroke="transparent" stroke-width="14" class="link-hit" data-key="${B}" style="cursor:pointer"/>`,y+=`<line x1="${b.x}" y1="${b.y}" x2="${w.x}" y2="${w.y}"
        stroke="${A}" stroke-width="${S}" stroke-dasharray="${k?"none":"4,3"}"
        opacity="${L}" pointer-events="none" data-line-key="${B}"/>`});for(const x of Object.keys(u)){const d=E(x);if(!d)continue;const c=x.replace(/\d+/,""),b=an[c]||"#555";y+=`<circle cx="${d.x}" cy="${d.y}" r="${s}" fill="${b}" stroke="rgba(255,255,255,0.7)" stroke-width="2" pointer-events="none"/>`,y+=`<text x="${d.x}" y="${d.y+4}" text-anchor="middle" font-size="10" font-weight="900" fill="white" font-family="Arial Black,Arial" pointer-events="none">${x}</text>`}y+="</svg>",document.getElementById("field-wrap").innerHTML=y;const v=document.getElementById("links-list");v.innerHTML=l.map(([x,d])=>{const c=m(x,d),b=g.has(c);return`
        ...
        ${i.length===0?'<div style="text-align:center;padding:16px;color:#aaa;font-size:13px">Aucun taux. Cliquez + Ligne.</div>':i.map((s,E)=>`
          <div style="display:grid;grid-template-columns:2fr 1.5fr 50px 50px 60px 32px;gap:4px;align-items:center;margin-bottom:6px;padding:6px;background:#f9f9f9;border-radius:6px" data-rate-idx="${E}">
            <select class="rate-type" data-idx="${E}" style="padding:6px 4px;border:1px solid #ddd;border-radius:4px;font-size:12px;width:100%">
              ${ln.map(y=>`<option value="${y.value}" ${s.card_type===y.value?"selected":""}>${y.label}</option>`).join("")}
            </select>
            <select class="rate-rarity" data-idx="${E}" style="padding:6px 4px;border:1px solid #ddd;border-radius:4px;font-size:12px;width:100%">
              ${sn.map(y=>`<option value="${y.value}" ${(s.rarity||"")===y.value?"selected":""}>${y.label}</option>`).join("")}
            </select>
    ...
    <div style="display:flex;height:calc(100vh - 56px);overflow:hidden">
      ${!a()||f||!n?m():""}
      ${!a()||p?h():""}
    </div>`,u()}function u(){var p,s,E,y,v,$,I;const l=x=>e.querySelector(x);e.querySelectorAll(".booster-row").forEach(x=>{x.addEventListener("click",d=>{d.target.closest(".btn-delete")||(n=x.dataset.id,r=null,o())})}),(p=l("#btn-back"))==null||p.addEventListener("click",()=>{n=null,o()}),(s=l("#btn-new"))==null||s.addEventListener("click",async()=>{const x=prompt("Nom du nouveau booster :");if(!(x!=null&&x.trim()))return;const{data:d,error:c}=await _.from("booster_configs").insert({name:x.trim(),price_type:"credits",price_credits:1e3,card_count:5}).select().single();if(c){alert(c.message);return}t.push(d),n=d.id,r=null,o()}),e.querySelectorAll(".btn-delete").forEach(x=>{x.addEventListener("click",async d=>{if(d.stopPropagation(),!confirm("Supprimer ce booster ?"))return;await _.from("booster_configs").delete().eq("id",x.dataset.id);const c=t.findIndex(b=>b.id===x.dataset.id);c>-1&&t.splice(c,1),n===x.dataset.id&&(n=null,r=null),o()})}),(E=l("#btn-cancel"))==null||E.addEventListener("click",()=>{n=null,o()}),(y=l("#f-price-type"))==null||y.addEventListener("change",x=>{const d=l("#credits-field");d&&(d.style.opacity=x.target.value!=="credits"?"0.4":"1")}),(v=l("#btn-pick-icon"))==null||v.addEventListener("click",async()=>{var d;const x=l("#icon-picker-grid");if(x){if(x.style.display!=="none"){x.style.display="none";return}x.innerHTML='<div style="padding:10px;text-align:center;color:#888;font-size:12px">⏳ Chargement...</div>',x.style.display="block";try{const b=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),w=Array.isArray(b)?b.filter(k=>k.name.startsWith("booster")&&/\.(png|jpg|svg|webp)$/i.test(k.name)):[];if(!w.length){x.innerHTML='<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Aucune icône "booster*" trouvée.</div>';return}const B=((d=l("#f-image-url"))==null?void 0:d.value)||"";x.innerHTML=`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(72px,1fr));gap:8px">
          ${w.map(k=>`
    ...
    <div style="display:flex;gap:16px;height:calc(100vh - 56px);overflow:hidden;padding:16px">
      ...
      <div style="width:420px;flex-shrink:0;overflow-y:auto">
        ...
        <div style="margin-bottom:16px">
          ...
          <div id="ts-toolbar" style="display:flex;flex-wrap:wrap;gap:4px;padding:6px 8px;background:#f5f5f5;border:1.5px solid #ddd;border-bottom:none;border-radius:8px 8px 0 0">
            ${[["B","bold","<strong>Gras</strong>"],["I","italic","<em>Italique</em>"],["U","underline","Souligné"],["¶","insertParagraph","Paragraphe"],["•","insertUnorderedList","Liste à puces"],["1.","insertOrderedList","Liste numérotée"],["H2","formatBlock:h2","Titre H2"],["H3","formatBlock:h3","Titre H3"],["—","insertHorizontalRule","Séparateur"]].map(([c,b,w])=>`<button type="button" data-cmd="${b}" title="${w}" style="padding:4px 8px;border:1px solid #ddd;border-radius:4px;background:#fff;font-size:12px;font-weight:700;cursor:pointer;line-height:1;min-width:28px">${c}</button>`).join("")}
      ...
      <div style="flex:1;overflow-y:auto">
        ...
        <div id="ts-list" style="display:flex;flex-direction:column;gap:10px">
          ...
          </div>`).join("")+"</div>",h.querySelectorAll("[data-pick]").forEach(k=>{k.addEventListener("click",()=>{a&&(a.value=k.dataset.pick),o(),h.style.display="none"})})}catch(c){h.innerHTML='<div style="color:#cc2222;font-size:12px">Erreur chargement : '+c.message+"</div>"}}),(x=document.getElementById("ts-color"))==null||x.addEventListener("input",c=>{const b=document.getElementById("ts-color-hex");b&&(b.value=c.target.value)}),(d=document.getElementById("ts-color-hex"))==null||d.addEventListener("input",c=>{const b=c.target.value;if(/^#[0-9a-fA-F]{6}$/.test(b)){const w=document.getElementById("ts-color");w&&(w.value=b)}}),e.querySelectorAll("[data-edit]").forEach(c=>{const b=(t||[]).find(w=>w.id===c.dataset.edit);b&&c.addEventListener("click",()=>Ne(b))}),e.querySelectorAll("[data-delete]").forEach(c=>{c.addEventListener("click",async()=>{confirm("Supprimer cette étape ?")&&(await _.from("tutorial_steps").delete().eq("id",c.dataset.delete),V(e))})}),e.querySelectorAll("[data-toggle]").forEach(c=>{c.addEventListener("click",async()=>{const b=(t||[]).find(w=>w.id===c.dataset.toggle);b&&(await _.from("tutorial_steps").update({is_active:!b.is_active}).eq("id",b.id),V(e))})}),e.querySelectorAll("[data-up]").forEach(c=>{c.addEventListener("click",async()=>{const b=t||[],w=b.findIndex(B=>B.id===c.dataset.up);w<=0||(await Promise.all([_.from("tutorial_steps").update({step_order:b[w-1].step_order}).eq("id",b[w].id),_.from("tutorial_steps").update({step_order:b[w].step_order}).eq("id",b[w-1].id)]),V(e))})}),e.querySelectorAll("[data-down]").forEach(c=>{c.addEventListener("click",async()=>{const b=t||[],w=b.findIndex(B=>B.id===c.dataset.down);w<0||w>=b.length-1||(await Promise.all([_.from("tutorial_steps").update({step_order:b[w+1].step_order}).eq("id",b[w].id),_.from("tutorial_steps").update({step_order:b[w].step_order}).eq("id",b[w+1].id)]),V(e))})})}function xn(e){return`
  ...
  </div>`}function Ue(e,t,n){var f,p,s,E;const{openModal:i,closeModal:r,toast:a}=n,g=!e,m=e||{name:"",gc_type:"game_changer",color:"purple",effect:"",image_url:"",sort_order:0,is_active:!0,effect_type:"BOOST_STAT",effect_params:{}},h=(()=>{const y=m.effect_params||{},v=G.find($=>$.value===(m.effect_type||"BOOST_STAT"))||G[0];return`
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
      ${v.hasValue?`<div>
        <label>VALEUR (+N)</label>
        <input id="gc-p-value" type="number" min="1" max="5" value="${y.value||2}">
      </div>`:""}
      ${v.hasCount?`<div>
        <label>NB JOUEURS</label>
        <input id="gc-p-count" type="number" min="1" max="11" value="${y.count||1}">
      </div>`:""}
      ${v.hasTarget?`<div>
        <label>CIBLE</label>
        <select id="gc-p-target">
          <option value="home" ${(y.target||"home")==="home"?"selected":""}>Mon équipe</option>
          <option value="opponent" ${y.target==="opponent"?"selected":""}>Équipe adverse</option>
        </select>
      </div>`:""}
      ${v.hasRoles?`<div>
        ...
        <div style="display:flex;gap:4px;flex-wrap:wrap">
          ${Fe.map($=>`<label style="display:flex;align-items:center;gap:2px;font-size:11px;cursor:pointer;color:#555">
            <input type="checkbox" class="gc-p-role" value="${$}" ${!y.roles||y.roles.includes($)?"checked":""}> ${$}
          </label>`).join("")}
        </div>
      </div>`:""}
    </div>`})(),o=`
    <div style="display:flex;justify-content:center;margin-bottom:16px;padding:20px;background:#1a1a2e;border-radius:12px">
      <div id="gc-modal-preview">${me(m.name||"?",m.image_url?`/icons/${m.image_url}`:null,(H.find(y=>y.value===m.gc_type)||H[0]).label.split(" ")[0],m.effect||"",{width:150})}</div>
    </div>
    ...
    <div style="display:flex;flex-direction:column;gap:12px">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
        <div>
          <label>TYPE</label>
          <select id="gc-type">
            ${H.map(y=>`<option value="${y.value}" ${m.gc_type===y.value?"selected":""}>${y.label}</option>`).join("")}
          </select>
        </div>
        <div>
          <label>COULEUR</label>
          <select id="gc-color">
            ${$n.map(y=>`<option value="${y.value}" ${m.color===y.value?"selected":""}>${y.label}</option>`).join("")}
          </select>
        </div>
      </div>
      <div>
      ...
      <div style="border-top:1px solid #eee;padding-top:12px;margin-top:4px">
        ...
        <div>
          ...
          <select id="gc-effect-type" style="margin-bottom:10px">
            ${G.map(y=>`<option value="${y.value}" ${(m.effect_type||"BOOST_STAT")===y.value?"selected":""}>${y.label}</option>`).join("")}
          </select>
        </div>
  ...
  `);const l=()=>{var c,b,w,B,k;const y=((c=document.getElementById("gc-name"))==null?void 0:c.value)||"?",v=((b=document.getElementById("gc-effect"))==null?void 0:b.value)||"",$=((w=document.getElementById("gc-type"))==null?void 0:w.value)||"game_changer",I=(k=(B=document.getElementById("gc-image-url"))==null?void 0:B.value)==null?void 0:k.trim(),x=H.find(A=>A.value===$)||H[0],d=document.getElementById("gc-modal-preview");d&&(d.innerHTML=me(y,I?`/icons/${I}`:null,x.label.split(" ")[0],v,{width:150}))};["gc-name","gc-effect","gc-type","gc-image-url"].forEach(y=>{var v,$;(v=document.getElementById(y))==null||v.addEventListener("input",l),($=document.getElementById(y))==null||$.addEventListener("change",l)}),(f=document.getElementById("gc-effect-type"))==null||f.addEventListener("change",()=>{const y=document.getElementById("gc-effect-type").value,v=G.find(I=>I.value===y)||G[0],$=document.getElementById("gc-params-wrap");$.innerHTML=`
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
    ...
    </div>`}),(p=document.getElementById("btn-pick-gc-icon"))==null||p.addEventListener("click",async()=>{var v;const y=document.getElementById("gc-icon-picker");if(y){if(y.style.display!=="none"){y.style.display="none";return}y.innerHTML='<div style="padding:10px;text-align:center;color:#888;font-size:12px">⏳ Chargement...</div>',y.style.display="block";try{const I=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),x=Array.isArray(I)?I.filter(c=>c.name.startsWith("gamechanger-")&&/\.(png|jpg|svg|webp)$/i.test(c.name)):[];if(!x.length){y.innerHTML='<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Aucun fichier "gamechanger-*" trouvé dans public/icons/</div>';return}const d=((v=document.getElementById("gc-image-url"))==null?void 0:v.value)||"";y.innerHTML=`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(72px,1fr));gap:8px">
        ${x.map(c=>`
    ...
    <div style="display:flex;flex-direction:column;gap:20px">
      ...
      <div class="card-panel" style="background:linear-gradient(135deg,#0a3d1e,#1a6b3c20);border:1.5px solid #1A6B3C40">
        ...
        <div style="display:flex;flex-direction:column;gap:4px">
          ${f.map(d=>{const c=y[d],b=l.length>0?Math.round(c/l.length*100):0;return`
              <div style="display:flex;align-items:center;gap:8px;font-size:12px">
                ...
                <div style="flex:1;background:rgba(255,255,255,0.08);border-radius:4px;height:14px;overflow:hidden">
                  <div style="height:100%;width:${b}%;background:${s[d]};border-radius:4px;transition:width 0.5s"></div>
                </div>
                <span style="width:40px;text-align:right;color:var(--gray-600)">${c} (${b}%)</span>
              </div>`}).join("")}
        ...
        <div class="table-wrap">
          <table>
            ...
            <tbody id="seasons-tbody">
              ${h.length===0?'<tr><td colspan="6" style="text-align:center;color:var(--gray-600);padding:24px">Aucune saison créée.</td></tr>':h.map(d=>{const c=Math.round((new Date(d.end_at)-new Date(d.start_at))/864e5),b=new Date(d.end_at)<new Date&&!d.is_active;return`
                    <tr>
                      <td><b>${d.name}</b></td>
                      ...
                      <td style="display:flex;gap:6px;flex-wrap:wrap">
                        ...
                        ${b?`<button class="btn btn-danger btn-sm" data-delete="${d.id}">🗑</button>`:""}
                      </td>
                    </tr>`}).join("")}
            </tbody>
          </table>
