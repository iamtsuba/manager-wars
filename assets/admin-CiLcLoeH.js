...
Cette action est irréversible.`))return;const i=document.getElementById("fix-old-faces-btn");i&&(i.disabled=!0,i.textContent="⏳ En cours...");try{const{data:d,error:s}=await _.from("players").select("id, country_code, face").like("face","public/faces/%");if(s){n(s.message,"error");return}if(!(d!=null&&d.length)){n("Aucun joueur avec l'ancien format — rien à faire ✅","success");return}const{data:m}=await _.from("players").select("face").not("face","is",null),x=new Set((m||[]).map(l=>l.face).filter(l=>l&&!l.startsWith("public/faces/")));let E=0,o=0;for(const l of d){const a=await we(l.country_code,x);if(!a){o++;continue}const{error:r}=await _.from("players").update({face:a}).eq("id",l.id);if(r){o++;continue}x.add(a),E++}n(`${E} photo(s) réattribuée(s)${o?`, ${o} échec(s)`:""} ✅`,o?"error":"success")}finally{i&&(i.disabled=!1,i.textContent="🖼️ Réattribuer anciennes photos")}}async function Ut(e,t){await se(e,t)}async function se(e,t,n=null){var o,l,a,r,u;const{toast:i}=t;n||(n={search:((o=document.getElementById("search-players"))==null?void 0:o.value)||"",job:((l=document.getElementById("filter-job"))==null?void 0:l.value)||"",rarity:((a=document.getElementById("filter-rarity"))==null?void 0:a.value)||"",club:((r=document.getElementById("filter-club"))==null?void 0:r.value)||"",country:((u=document.getElementById("filter-country"))==null?void 0:u.value)||""});const[{data:d,error:s},{data:m}]=await Promise.all([_.from("players").select("*, clubs(id,encoded_name,logo_url)"),_.from("clubs").select("id,encoded_name").order("encoded_name")]);if(s){e.innerHTML=`<p style="color:red">${s.message}</p>`;return}const x={GK:0,DEF:1,MIL:2,ATT:3},E=(d||[]).sort((c,f)=>{const p=(x[c.job]??4)-(x[f.job]??4);return p!==0?p:(c.surname_real||"").localeCompare(f.surname_real||"")});qt(e,E,m||[],t,n)}function qt(e,t,n,i,d=null){var l,a;const{toast:s}=i;if(e.innerHTML=`
    <div style="display:flex;flex-direction:column;gap:10px">
    ...
    </div>`,d){const r=d,u=c=>document.getElementById(c);r.search&&u("search-players")&&(u("search-players").value=r.search),r.job&&u("filter-job")&&(u("filter-job").value=r.job),r.rarity&&u("filter-rarity")&&(u("filter-rarity").value=r.rarity),r.club&&u("filter-club")&&(u("filter-club").value=r.club),r.country&&u("filter-country")&&(u("filter-country").value=r.country)}if(d){const r=d,u=c=>document.getElementById(c);r.search&&u("search-players")&&(u("search-players").value=r.search),r.job&&u("filter-job")&&(u("filter-job").value=r.job),r.rarity&&u("filter-rarity")&&(u("filter-rarity").value=r.rarity),r.club&&u("filter-club")&&(u("filter-club").value=r.club),r.country&&u("filter-country")&&(u("filter-country").value=r.country)}function m(){const r=document.getElementById("search-players").value.toLowerCase(),u=document.getElementById("filter-job").value,c=document.getElementById("filter-rarity").value,f=document.getElementById("filter-club").value,p=document.getElementById("filter-country").value;return t.filter(y=>(!r||`${y.firstname} ${y.surname_real}`.toLowerCase().includes(r))&&(!u||y.job===u)&&(!c||y.rarity===c)&&(!f||y.club_id===f)&&(!p||y.country_code===p))}const x=new Set;function E(){const r=document.getElementById("bulk-bar"),u=document.getElementById("bulk-count");r&&(r.style.display=x.size>0?"flex":"none",u&&(u.textContent=`${x.size} joueur(s) sélectionné(s)`))}function o(){const r=m();document.getElementById("count-label").textContent=`${r.length} joueur(s)`;const u=document.getElementById("players-list");if(!r.length){u.innerHTML='<div style="color:var(--gray-600);padding:20px;text-align:center">Aucun joueur.</div>';return}u.innerHTML=r.map(c=>{const f={...c,clubs:c.clubs,face:c.face||null},p=_e(f,{width:120});return`<div style="position:relative;cursor:pointer" data-edit="${c.id}">
        ${p}
    ...
    `).join(""),E.querySelectorAll(".club-row").forEach(r=>{r.addEventListener("click",()=>{const u=re.find(c=>c.id===r.dataset.clubId);u&&je(u,e,t),E.querySelectorAll(".club-row").forEach(c=>c.style.background="var(--tile-bg)"),r.style.background="rgba(26,107,60,0.18)"})}),E.querySelectorAll(".btn-del-club").forEach(r=>{r.addEventListener("click",async()=>{if(!confirm("Supprimer ce club ?"))return;const{error:u}=await _.from("clubs").delete().eq("id",r.dataset.del);u?i(u.message,"error"):(i("Club supprimé","success"),me(e,t))})})}}function nn(e){var C,z,D,q;const{openModal:t,closeModal:n}=e;let i=6;const d=36;function s(S){return new Array(S*S).fill("#ffffff")}let m=s(i),x="OP",E="#D4A017",o=50,l=0,a=0,r="",u="#ffffff",c=25,f=0,p=35,y=null,k=null;function A(){const S=d*i;let T="";for(let R=0;R<i;R++)for(let j=0;j<i;j++){const P=R*i+j;T+=`<rect x="${j*d}" y="${R*d}" width="${d}" height="${d}" fill="${m[P]}"/>`}const M=(R,j,P,H,U)=>{if(!R)return"";const K=S/2+H/100*S,V=S/2+U/100*S,J=P/100*S;return`<text x="${K}" y="${V}" text-anchor="middle" dominant-baseline="central"
        font-family="Arial Black, Arial, sans-serif" font-weight="900" font-size="${J}"
        fill="${j}">${R.toUpperCase().slice(0,3)}</text>`};return`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${S} ${S}">
      ${T}
      ${M(x,E,o,l,a)}
      ${M(r,u,c,f,p)}
    </svg>`}function v(S){return"data:image/svg+xml;base64,"+btoa(unescape(encodeURIComponent(S)))}const I=`
      ...
      <div style="flex:0 0 260px;display:flex;flex-direction:column;gap:14px;align-items:center">
        ...
        <div style="width:100%">
          ...
          <div style="display:flex;gap:6px">
            ${[6,9,12].map(S=>`<button type="button" class="btn ${S===i?"btn-primary":"btn-ghost"} btn-sm flag-grid-size" data-n="${S}" style="flex:1">${S}×${S}</button>`).join("")}
          </div>
        </div>
        ...
        <div style="display:flex;align-items:center;gap:8px;width:100%">
          <label style="margin:0;white-space:nowrap">Couleur</label>
          <input type="color" id="flag-text2-color" value="${u}" style="width:44px;height:32px;padding:2px;cursor:pointer">
        </div>
        <div style="width:100%">
        ...
        <div style="width:100%">
          <label>Position V (<span id="flag-y2-val">${p}</span>)</label>
          <input type="range" id="flag-text2-y" min="-50" max="50" value="${p}" style="width:100%">
        </div>
      </div>

      <div style="flex:1;min-width:280px">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
          <label style="margin:0">Couleurs des ${i*i} carrés — clique un carré, puis Ctrl+C / Ctrl+V pour copier-coller sa couleur</label>
        </div>
  ...
  `);function b(){const S=document.getElementById("flag-squares-grid");if(!S)return;const T=i>=12?26:i>=9?32:40;S.style.gridTemplateColumns=`repeat(${i}, ${T}px)`,S.innerHTML=m.map((M,R)=>`
      <div class="flag-square" data-i="${R}" style="width:${T}px;height:${T}px;border-radius:4px;background:${M};cursor:pointer;
        box-sizing:border-box;border:${y===R?"3px solid #1A6B3C":"1px solid rgba(0,0,0,0.15)"}"></div>
    `).join(""),S.querySelectorAll(".flag-square").forEach(M=>{M.addEventListener("click",()=>{y=parseInt(M.dataset.i),b(),$()}),M.addEventListener("dblclick",()=>{const R=parseInt(M.dataset.i),j=document.createElement("input");j.type="color",j.value=m[R],j.style.position="fixed",j.style.opacity="0",document.body.appendChild(j),j.addEventListener("input",()=>{m[R]=j.value,b(),B()}),j.addEventListener("change",()=>j.remove()),j.click()})})}function $(){const S=document.getElementById("flag-copy-btn"),T=document.getElementById("flag-paste-btn");S.disabled=y===null,T.disabled=y===null||k===null}function g(){if(y===null)return;k=m[y];const S=document.getElementById("flag-clipboard-preview"),T=document.getElementById("flag-clipboard-swatch");S&&(S.style.display="inline-flex"),T&&(T.style.background=k),$()}function w(){y===null||k===null||(m[y]=k,b(),B())}(C=document.getElementById("flag-copy-btn"))==null||C.addEventListener("click",g),(z=document.getElementById("flag-paste-btn"))==null||z.addEventListener("click",w);function L(S){if(!document.getElementById("flag-squares-grid")){document.removeEventListener("keydown",L);return}!(S.ctrlKey||S.metaKey)||y===null||(S.key==="c"||S.key==="C"?(S.preventDefault(),g()):(S.key==="v"||S.key==="V")&&(S.preventDefault(),w()))}document.addEventListener("keydown",L),document.querySelectorAll(".flag-grid-size").forEach(S=>{S.addEventListener("click",()=>{const T=parseInt(S.dataset.n);T!==i&&(i=T,m=s(i),y=null,k=null,document.getElementById("flag-clipboard-preview").style.display="none",b(),$(),B(),document.querySelectorAll(".flag-grid-size").forEach(M=>{M.classList.toggle("btn-primary",parseInt(M.dataset.n)===i),M.classList.toggle("btn-ghost",parseInt(M.dataset.n)!==i)}))})});function B(){const S=document.getElementById("flag-preview");S&&(S.innerHTML=A())}b(),$(),B(),document.getElementById("flag-text").addEventListener("input",S=>{x=S.target.value.toUpperCase().slice(0,3),S.target.value=x,B()}),document.getElementById("flag-text-color").addEventListener("input",S=>{E=S.target.value,B()}),document.getElementById("flag-text-size").addEventListener("input",S=>{o=parseInt(S.target.value),document.getElementById("flag-size-val").textContent=o,B()}),document.getElementById("flag-text-x").addEventListener("input",S=>{l=parseInt(S.target.value),document.getElementById("flag-x-val").textContent=l,B()}),document.getElementById("flag-text-y").addEventListener("input",S=>{a=parseInt(S.target.value),document.getElementById("flag-y-val").textContent=a,B()}),document.getElementById("flag-text2").addEventListener("input",S=>{r=S.target.value.toUpperCase().slice(0,3),S.target.value=r,B()}),document.getElementById("flag-text2-color").addEventListener("input",S=>{u=S.target.value,B()}),document.getElementById("flag-text2-size").addEventListener("input",S=>{c=parseInt(S.target.value),document.getElementById("flag-size2-val").textContent=c,B()}),document.getElementById("flag-text2-x").addEventListener("input",S=>{f=parseInt(S.target.value),document.getElementById("flag-x2-val").textContent=f,B()}),document.getElementById("flag-text2-y").addEventListener("input",S=>{p=parseInt(S.target.value),document.getElementById("flag-y2-val").textContent=p,B()}),(D=document.getElementById("flag-cancel"))==null||D.addEventListener("click",()=>n()),(q=document.getElementById("flag-use"))==null||q.addEventListener("click",()=>{const S=v(A()),T=document.getElementById("m-logo-url-current");T&&(T.value=S);const M=document.getElementById("logo-preview");M&&(M.innerHTML=`<img src="${S}" style="width:100%;height:100%;object-fit:contain">`),n()})}async function je(e,t,n){var p,y,k,A,v,I,h;const{toast:i}=n,d=!!e,s=e?ct(e):{...F},m=document.getElementById("club-panel");if(!m)return;const x=Object.entries(ze).map(([b,$])=>`<option value="${b}" ${s.style===b?"selected":""}>${$.label}</option>`).join(""),E=Ie.map(b=>`<option value="${b}" ${((e==null?void 0:e.country_code)||"FR")===b?"selected":""}>${b}</option>`).join("");m.style.display="block",m.style.alignItems="",m.style.justifyContent="",m.innerHTML=`
      ...
      <div style="border-top:1px solid var(--tile-border);padding-top:12px">
        ...
        <div style="display:flex;gap:12px;align-items:flex-start;flex-wrap:wrap">
          <div style="flex:1;min-width:200px;display:flex;flex-direction:column;gap:8px">
            ...
            ${[["Couleur 1","m-kit-color1",s.color1,!1],["Couleur 2","m-kit-color2",s.color2,!1],["Couleur 3","m-kit-color3",s.color3,!0],["Short","m-kit-shorts",s.shorts,!1],["Chaussettes","m-kit-socks",s.socks,!1]].map(([b,$,g,w])=>`
  ...
  `,(p=document.getElementById("m-logo-file"))==null||p.addEventListener("change",b=>{var w;const $=(w=b.target.files)==null?void 0:w[0];if(!$)return;const g=new FileReader;g.onload=()=>{document.getElementById("logo-preview").innerHTML=`<img src="${g.result}" style="width:100%;height:100%;object-fit:contain">`},g.readAsDataURL($)}),(y=document.getElementById("m-open-flag-builder"))==null||y.addEventListener("click",()=>{nn(n)}),ie();function o(){var w,L;const b=((w=document.getElementById("m-kit-style"))==null?void 0:w.value)||"uni",$=((L=ze[b])==null?void 0:L.colors)===3,g=document.getElementById("wrap-m-kit-color3");g&&(g.style.display=$?"":"none")}o(),["m-kit-color1","m-kit-color2","m-kit-color3","m-kit-shorts","m-kit-socks"].forEach(b=>{const $=document.getElementById(b),g=document.getElementById(b+"-txt");!$||!g||($.addEventListener("input",()=>{g.value=$.value,ie()}),g.addEventListener("input",()=>{const w=g.value.trim();/^#[0-9a-fA-F]{6}$/.test(w)&&($.value=w,ie())}),g.addEventListener("change",()=>{let w=g.value.trim();w.startsWith("#")||(w="#"+w),/^#[0-9a-fA-F]{6}$/.test(w)&&($.value=w,g.value=w,ie())}))}),(k=document.getElementById("m-kit-style"))==null||k.addEventListener("change",()=>{o(),ie()});const l=document.getElementById("m-real"),a=document.getElementById("m-encoded");function r(){if(!l||!a||a.value)return;const b=l.value.trim().split(/\s+/),$=b.length===1?b[0].toUpperCase().slice(0,6):b.filter(g=>g.length>2).map(g=>g[0].toUpperCase()).join("")||b[0].toUpperCase().slice(0,4);a.value=$}l==null||l.addEventListener("input",r),(A=document.getElementById("auto-encode-btn"))==null||A.addEventListener("click",()=>{a&&(a.value=""),r()}),(v=document.getElementById("m-save"))==null||v.addEventListener("click",()=>dn(e,d,t,n));const u=document.getElementById("m-gen-squad"),c=document.getElementById("m-gen-strong-label");function f(){if(c&&(c.style.display=u!=null&&u.checked?"flex":"none",!(u!=null&&u.checked))){const b=document.getElementById("m-gen-strong");b&&(b.checked=!1)}}u==null||u.addEventListener("change",f),f(),(I=document.getElementById("btn-gen-squad-panel"))==null||I.addEventListener("click",()=>{an(e,n,t)}),(h=document.getElementById("btn-gen-one-player"))==null||h.addEventListener("click",()=>{rn(e,n)}),d&&ke(e.id)}async function ke(e){const{data:t}=await _.from("players").select("*, clubs(encoded_name, logo_url, kit_style, kit_color1, kit_color2, kit_color3, kit_shorts, kit_socks)").eq("club_id",e).order("surname_real"),n=document.getElementById("club-players-grid"),i=document.getElementById("club-players-count");if(i&&(i.textContent=(t||[]).length),!!n){if(!(t!=null&&t.length)){n.innerHTML='<div style="color:var(--tile-fg-dim);padding:20px;font-size:13px">Aucun joueur dans ce club.</div>';return}n.innerHTML=t.map(d=>_e(d,{width:100})).join("")}}function an(e,t,n){const{openModal:i,closeModal:d,toast:s}=t;i(`⚽ Générer — ${e.real_name}`,`<div style="display:flex;flex-direction:column;gap:14px">
    ...
    </div>`),setTimeout(()=>{var m,x;(m=document.getElementById("qg-cancel"))==null||m.addEventListener("click",()=>d()),(x=document.getElementById("qg-ok"))==null||x.addEventListener("click",async()=>{var o;const E=((o=document.getElementById("qg-strong"))==null?void 0:o.checked)??!1;d(),s("Génération en cours…","info"),await dt(e.id,e.country_code,s,E),s("Effectif généré ✅","success"),ke(e.id),me(n,t)})},50)}const ut=["GK","DEF","MIL","ATT"],on=[{value:"normal",label:"Normal"},{value:"pepite",label:"Pépite"},{value:"papyte",label:"Papyte"},{value:"legende",label:"Légende"}];function rn(e,t){const{openModal:n,closeModal:i,toast:d}=t,s=Ie.map(o=>`<option value="${o}">${o}</option>`).join(""),m=o=>ut.map(l=>`<option value="${l}" ${l===o?"selected":""}>${l}</option>`).join(""),x=`
  ...
  `),setTimeout(()=>{var o,l;document.querySelectorAll('input[name="gen-one-mode"]').forEach(a=>{a.addEventListener("change",()=>{document.getElementById("gen-one-custom-fields").style.display=a.value==="custom"&&a.checked?"flex":"none"})}),(o=document.getElementById("gen-one-cancel"))==null||o.addEventListener("click",()=>i()),(l=document.getElementById("gen-one-ok"))==null||l.addEventListener("click",async()=>{var c,f,p,y,k;const a=((c=document.querySelector('input[name="gen-one-mode"]:checked'))==null?void 0:c.value)||"random",r=document.getElementById("gen-one-ok");r.disabled=!0,r.textContent="⏳ Génération…";const u={};a==="custom"&&(u.job=((f=document.getElementById("gen-one-job1"))==null?void 0:f.value)||null,u.job2=((p=document.getElementById("gen-one-job2"))==null?void 0:p.value)||null,u.rarity=((y=document.getElementById("gen-one-rarity"))==null?void 0:y.value)||null,u.country=((k=document.getElementById("gen-one-country"))==null?void 0:k.value)||null);try{await ln(e,u),d("Joueur généré ✅","success"),i(),ke(e.id)}catch(A){d("Erreur : "+A.message,"error"),r.disabled=!1,r.textContent="➕ Générer"}})},50)}async function ln(e,t={}){const n=t.country||(Math.random()<.5?e.country_code:at(e.country_code)),i=t.job||Z(ut),d=t.job2||null,s=t.rarity||(()=>{const L=Math.random()*100;return L<1.5?"legende":L<3?"pepite":L<4.5?"papyte":"normal"})(),m=s==="pepite"?Q(10,15):s==="papyte"?Q(15,20):ot(),x=d?Math.max(0,m-2):0;let E=0,o=0,l=0,a=0;i==="GK"&&(E=m),i==="DEF"&&(o=m),i==="MIL"&&(l=m),i==="ATT"&&(a=m),d==="GK"&&(E=x),d==="DEF"&&(o=x),d==="MIL"&&(l=x),d==="ATT"&&(a=x);const r=$e(n),u=it(n),c=Z(nt),{data:f}=await _.from("players").select("surname_real").not("surname_real","is",null),p=new Set((f||[]).map(L=>L.surname_real).filter(Boolean)),y=st(n),k=y.filter(L=>!p.has(L)),A=Z(k.length?k:y),v={job:i,job2:d,firstname:lt(n),surname_real:A,country_code:n,club_id:e.id,note_g:E,note_d:o,note_m:l,note_a:a,skin:r,hair:u,hair_length:c,rarity:s,sell_price:0,ethnie:ve(n)},{data:I}=await _.from("players").select("face").not("face","is",null),h=new Set((I||[]).map(L=>L.face).filter(Boolean)),b=await we(n,h);b&&(v.face=b);const{data:$,error:g}=await _.from("players").insert(v).select("id").single();if(g)throw g;const{error:w}=await _.from("cards").insert({card_type:"player",player_id:$.id});w&&console.warn("[GenOnePlayer] Erreur carte:",w.message)}function pt(){var e,t,n,i,d,s;return{style:((e=document.getElementById("m-kit-style"))==null?void 0:e.value)||"uni",color1:((t=document.getElementById("m-kit-color1"))==null?void 0:t.value)||"#1A6B3C",color2:((n=document.getElementById("m-kit-color2"))==null?void 0:n.value)||"#ffffff",color3:((i=document.getElementById("m-kit-color3"))==null?void 0:i.value)||"#000000",shorts:((d=document.getElementById("m-kit-shorts"))==null?void 0:d.value)||"#111111",socks:((s=document.getElementById("m-kit-socks"))==null?void 0:s.value)||"#ffffff"}}function ie(){const e=document.getElementById("kit-preview-wrap");e&&(e.innerHTML=$t(pt(),"panel"))}async function sn(e,t){const n=(e.name.split(".").pop()||"png").toLowerCase(),i=`clubs/${t||"new"}-${Date.now()}.${n}`,{error:d}=await _.storage.from("assets").upload(i,e,{upsert:!0,cacheControl:"3600"});if(d)throw d;const{data:s}=_.storage.from("assets").getPublicUrl(i);return s.publicUrl}async function dn(e,t,n,i){var A,v,I,h,b,$,g,w,L;const{toast:d}=i,s=document.getElementById("m-error"),m=document.getElementById("m-save"),x=(A=document.getElementById("m-real"))==null?void 0:A.value.trim(),E=(v=document.getElementById("m-encoded"))==null?void 0:v.value.trim().toUpperCase(),o=(I=document.getElementById("m-country"))==null?void 0:I.value.trim().toUpperCase(),l=((b=(h=document.getElementById("m-logo-file"))==null?void 0:h.files)==null?void 0:b[0])||null,a=(($=document.getElementById("m-logo-url-current"))==null?void 0:$.value)||null,r=((g=document.getElementById("m-gen-stadium"))==null?void 0:g.checked)??!1,u=((w=document.getElementById("m-gen-squad"))==null?void 0:w.checked)??!1,c=((L=document.getElementById("m-gen-strong"))==null?void 0:L.checked)??!1,f=pt();if(!x){s.textContent="Le nom du club est requis.";return}if(!E){s.textContent="Le nom encodé est requis.";return}if(!o){s.textContent="Le pays est requis.";return}m.disabled=!0,m.textContent="Enregistrement…";let p=a;if(l)try{m.textContent="📤 Envoi du logo…",p=await sn(l,e==null?void 0:e.id)}catch(B){s.textContent="Erreur upload logo : "+B.message,m.disabled=!1,m.textContent=t?"💾 Enregistrer":"✅ Créer le club";return}const y={real_name:x,encoded_name:E,country_code:o,logo_url:p,kit_style:f.style,kit_color1:f.color1,kit_color2:f.color2,kit_color3:f.color3,kit_shorts:f.shorts,kit_socks:f.socks};let k=e==null?void 0:e.id;if(t){const{error:B}=await _.from("clubs").update(y).eq("id",k);if(B){s.textContent=B.message,m.disabled=!1,m.textContent="💾 Enregistrer";return}}else{const{data:B,error:C}=await _.from("clubs").insert(y).select().single();if(C){s.textContent=C.message,m.disabled=!1,m.textContent="✅ Créer le club";return}if(k=B.id,r){m.textContent="🏟️ Création du stade…";const{data:z,error:D}=await _.from("stadium_definitions").insert({name:`Stade de ${x}`,club_id:k,country_code:null}).select().single();D?console.warn("[Stadium] Erreur def stade:",D.message):z&&await _.from("cards").insert({card_type:"stadium",stadium_id:z.id})}u&&(m.textContent="⚽ Génération des joueurs…",await dt(k,o,d,c))}d(t?"Club modifié ✅":"Club créé ✅","success"),me(n,i)}const cn=["normal","pepite","papyte","legende"],Pe=["GK","DEF","MIL","ATT"],un=[["MA","Maroc"],["FR","France"],["AR","Argentine"],["PT","Portugal"],["BR","Brésil"],["ES","Espagne"],["DE","Allemagne"],["GB","Angleterre"],["IT","Italie"],["CM","Cameroun"],["SN","Sénégal"],["NG","Nigéria"],["DK","Danemark"],["NL","Pays-Bas"],["BE","Belgique"],["CI","Côte d'Ivoire"],["AL","Albanie"],["HR","Croatie"],["RS","Serbie"],["TR","Turquie"],["MW","Malawi"]];let X={...O},le=[];async function pn(e,{toast:t}){const{data:n}=await _.from("clubs").select("id,encoded_name,logo_url,kit_style,kit_color1,kit_color2,kit_shorts,kit_socks").order("encoded_name");le=n||[],X={...O},e.innerHTML=yn(le),gn(e,le,t),G()}function Ne(){var n;const e=((n=document.getElementById("cb-surname-real"))==null?void 0:n.value)||"",t=document.getElementById("cb-surname-enc");t&&(t.value=pe(e))}function mn(){var n;const e=(n=document.getElementById("cb-club"))==null?void 0:n.value,t=le.find(i=>String(i.id)===String(e));return t?{style:t.kit_style||F.style,color1:t.kit_color1||F.color1,color2:t.kit_color2||F.color2,shorts:t.kit_shorts||F.shorts,socks:t.kit_socks||F.socks}:{...F}}function G(){var r,u,c,f;const e=p=>{var y;return((y=document.getElementById(p))==null?void 0:y.value)||""},t={firstname:e("cb-firstname"),surname_real:e("cb-surname-enc")||pe(e("cb-surname-real")),country_code:e("cb-country"),club_encoded_name:((u=(r=document.getElementById("cb-club"))==null?void 0:r.selectedOptions[0])==null?void 0:u.text)||"",job:e("cb-job"),job2:e("cb-job2")||null,note_g:parseInt(e("cb-note-g"))||0,note_d:parseInt(e("cb-note-d"))||0,note_m:parseInt(e("cb-note-m"))||0,note_a:parseInt(e("cb-note-a"))||0,rarity:e("cb-rarity")},n=mn(),i=document.getElementById("cb-club"),d=((f=(c=i==null?void 0:i.selectedOptions[0])==null?void 0:c.dataset)==null?void 0:f.logo)||null,s=Xe(X,n,120),m=`data:image/svg+xml;charset=utf-8,${encodeURIComponent(s)}`,x=At(t,{portraitUrl:m,clubLogoUrl:d,showNotes:!0}),E=document.getElementById("card-preview");E&&(E.innerHTML=x);const o=document.getElementById("avatar-preview-wrap");o&&(o.innerHTML=s);const l=e("cb-surname-real"),a=document.getElementById("encode-summary");a&&l&&(a.textContent=`${e("cb-firstname")} ${l} → ${e("cb-firstname")} ${t.surname_real}`)}function mt(){const e=t=>{var n;return((n=document.getElementById(t))==null?void 0:n.value)||""};X={skin:e("av-skin"),hair:e("av-hair"),hairColor:e("av-hairColor"),eyes:e("av-eyes"),nose:e("av-nose"),mouth:e("av-mouth"),eyebrows:e("av-eyebrows"),beard:e("av-beard")}}function gn(e,t,n){var d,s,m,x,E;e.querySelectorAll("input,select").forEach(o=>{o.addEventListener("input",G),o.addEventListener("change",G)});const i=["av-skin","av-hair","av-hairColor","av-eyes","av-nose","av-mouth","av-eyebrows","av-beard"];i.forEach(o=>{var l;(l=document.getElementById(o))==null||l.addEventListener("change",()=>{mt(),G()})}),(d=document.getElementById("cb-surname-real"))==null||d.addEventListener("input",()=>{Ne(),G()}),(s=document.getElementById("btn-encode"))==null||s.addEventListener("click",()=>{Ne(),G()}),(m=document.getElementById("cb-club"))==null||m.addEventListener("change",G),(x=document.getElementById("btn-save-player"))==null||x.addEventListener("click",()=>fn(n)),(E=document.getElementById("btn-reset"))==null||E.addEventListener("click",()=>{e.querySelectorAll("input").forEach(o=>o.value=""),e.querySelectorAll("select").forEach(o=>o.selectedIndex=0),X={...O},i.forEach(o=>{const l=o.replace("av-",""),a=document.getElementById(o);a&&(a.value=O[l]||"")}),G()})}async function fn(e){const t=m=>{var x;return((x=document.getElementById(m))==null?void 0:x.value)||""},n=t("cb-surname-real").trim(),i=t("cb-surname-enc").trim()||pe(n);if(!t("cb-firstname")||!n||!t("cb-country")||!t("cb-job")){e("Remplissez les champs obligatoires (prénom, nom, pays, poste)","error");return}mt();const d={firstname:t("cb-firstname").trim(),surname_real:n,surname_real:i,country_code:t("cb-country"),club_id:t("cb-club")||null,job:t("cb-job"),job2:t("cb-job2")||null,note_g:parseInt(t("cb-note-g"))||0,note_d:parseInt(t("cb-note-d"))||0,note_m:parseInt(t("cb-note-m"))||0,note_a:parseInt(t("cb-note-a"))||0,rarity:t("cb-rarity")||"normal",note_min:parseInt(t("cb-note-min"))||null,note_max:parseInt(t("cb-note-max"))||null,sell_price:parseInt(t("cb-price"))||0,avatar_config:X,skin:X.skin||"blanc",hair:X.hairColor||"marron",hair_length:"court"},{error:s}=await _.from("players").insert(d);if(s){e("Erreur: "+s.message,"error");return}e(`✅ Joueur "${d.firstname} ${d.surname_real}" enregistré !`,"success")}function yn(e){const t=un.map(([o,l])=>`<option value="${o}">${l} (${o})</option>`).join(""),n='<option value="">— Sélectionner un club —</option>'+e.map(o=>`<option value="${o.id}" data-logo="${o.logo_url||""}">${o.encoded_name}</option>`).join(""),i=Pe.map(o=>`<option value="${o}">${o}</option>`).join(""),d='<option value="">Aucun</option>'+Pe.map(o=>`<option value="${o}">${o}</option>`).join(""),s=cn.map(o=>`<option value="${o}">${o.charAt(0).toUpperCase()+o.slice(1)}</option>`).join(""),m=(o,l,a)=>Object.entries(l).map(([r,u])=>`<option value="${r}" ${r===(a||"")?"selected":""}>${u.label}</option>`).join(""),x=Object.keys(kt).map(o=>`<option value="${o}" ${o===O.skin?"selected":""}>${o.charAt(0).toUpperCase()+o.slice(1)}</option>`).join(""),E=Object.keys(Lt).map(o=>`<option value="${o}" ${o===O.hairColor?"selected":""}>${o.charAt(0).toUpperCase()+o.slice(1)}</option>`).join("");return`
        ...
        <div style="display:grid;grid-template-columns:1fr 120px;gap:16px;align-items:start">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
            ...
            <div>
              ...
              <select id="av-hair">${m("av-hair",St,O.hair)}</select>
            </div>
            <div>
            ...
            <div>
              ...
              <select id="av-eyebrows">${m("av-eyebrows",Ct,O.eyebrows)}</select>
            </div>
            <div>
              <label>Yeux</label>
              <select id="av-eyes">${m("av-eyes",Tt,O.eyes)}</select>
            </div>
            <div>
              <label>Nez</label>
              <select id="av-nose">${m("av-nose",Mt,O.nose)}</select>
            </div>
            <div>
              <label>Bouche</label>
              <select id="av-mouth">${m("av-mouth",zt,O.mouth)}</select>
            </div>
            <div>
              <label>Barbe</label>
              <select id="av-beard">${m("av-beard",Rt,O.beard)}</select>
            </div>
          </div>
  ...
  `}async function gt(e,{toast:t}){const[{data:n,error:i},{data:d},{data:s}]=await Promise.all([_.from("users").select("id,pseudo,club_name,credits,level,wins,draws,losses,trophies_top1,trophies_top2,trophies_top3,is_admin,created_at,mmr,mmr_deviation,rank_tier,placement_matches,ranked_wins,ranked_losses,ranked_draws").order("created_at",{ascending:!1}),_.from("user_solo_progress").select("user_id, unlocked_level"),_.rpc("admin_get_user_emails")]);if(i){e.innerHTML=`<p style="color:red">${i.message}</p>`;return}const m={};(d||[]).forEach(l=>{m[l.user_id]=l.unlocked_level});const x={};(s||[]).forEach(l=>{x[l.id]=l.email});const E=n||[];e.innerHTML=`
  ...
  `,o(E),document.getElementById("search-users").addEventListener("input",l=>{const a=l.target.value.toLowerCase();o(E.filter(r=>{var u;return r.pseudo.toLowerCase().includes(a)||((u=r.club_name)==null?void 0:u.toLowerCase().includes(a))}))});function o(l){document.getElementById("users-tbody").innerHTML=l.map(a=>{const r=a.mmr??1e3,u=a.mmr_deviation??350,c=a.placement_matches??0,f=Dt(r),p=a.ranked_wins??0,y=a.ranked_losses??0,k=a.ranked_draws??0,A=p+y+k,v=A>0?Math.round(p/A*100):null,I=c<10,h=u<80?"#1A6B3C":u<150?"#e67e22":"#bb2020";return`
        <tr>
          <td>
          ...
          <td style="font-size:12px">
            ...
            ${I?`<div style="font-size:10px;color:#e67e22">${c}/10 placement${c<10?` (${10-c} restants)`:""}</div>`:`<div style="font-size:10px;color:var(--gray-600)">${p}V ${k}N ${y}D${v!==null?` · ${v}% WR`:""}</div>`}
          </td>
    ...
    `).order("listed_at",{ascending:!1}).limit(100);if(d){e.innerHTML=`<p style="color:var(--danger)">${d.message}</p>`;return}const s=i||[],m=s.filter(o=>o.status==="active").length,x=s.filter(o=>o.status==="sold").length;e.innerHTML=`
  ...
  `),xn(),(u=document.getElementById("pg-close"))==null||u.addEventListener("click",()=>{fe(),n()});function l(f){const p=f.dataset.id,y=f.dataset.field,k=y==="rarity"?f.value:Number(f.value)||0;return a(p,{[y]:k},f)}async function a(f,p,y){const k=document.querySelector(`tr[data-row="${f}"]`),A=g=>k.querySelector(`[data-field="${g}"]`),v="note_min"in p?p.note_min:Number(A("note_min").value),I="note_max"in p?p.note_max:Number(A("note_max").value),h="price_min"in p?p.price_min:Number(A("price_min").value),b="price_max"in p?p.price_max:Number(A("price_max").value);if(v>I){i("Note min doit être ≤ note max","error");return}if(h>b){i("Prix min doit être ≤ prix max","error");return}const{error:$}=await _.from("sell_price_configs").update(p).eq("id",f);if($){i($.message,"error");return}y&&(y.style.transition="background .3s",y.style.background="#e8f8ee",setTimeout(()=>{y.style.background=""},500))}function r(){document.querySelectorAll("#pg-tbody [data-field]").forEach(f=>{f.addEventListener("change",()=>l(f))})}r(),document.querySelectorAll("[data-del-row]").forEach(f=>{f.addEventListener("click",async()=>{confirm("Supprimer cette ligne ?")&&(await _.from("sell_price_configs").delete().eq("id",f.dataset.delRow),i("Ligne supprimée","success"),fe(),xe(e))})}),(c=document.getElementById("pg-add-row"))==null||c.addEventListener("click",async()=>{var A;const f={rarity:"normal",note_min:0,note_max:20,price_min:2500,price_max:15e3},{data:p,error:y}=await _.from("sell_price_configs").insert(f).select().single();if(y){i(y.message,"error");return}const k=document.getElementById("pg-tbody");k.querySelector("td[colspan]")&&(k.innerHTML=""),k.insertAdjacentHTML("beforeend",x(p)),r(),(A=document.querySelector(`[data-del-row="${p.id}"]`))==null||A.addEventListener("click",async function(){confirm("Supprimer cette ligne ?")&&(await _.from("sell_price_configs").delete().eq("id",p.id),i("Ligne supprimée","success"),fe(),xe(e))}),i("Ligne ajoutée — modifie les valeurs directement","success")})}async function hn(e,{toast:t}){e.innerHTML=`
...
`).replace(/\r/g,`
`);const t=e.split(`
`).filter(m=>m.trim());if(t.length<2)return[];const n=t[0],i=n.split(";").length>n.split(",").length?";":",",d=Ue(n,i).map(m=>m.trim().replace(/^\uFEFF/,"").toLowerCase()),s=[];for(let m=1;m<t.length;m++){if(!t[m].trim())continue;const x=Ue(t[m],i),E={};d.forEach((o,l)=>{E[o]=(x[l]||"").trim()}),!Object.values(E).every(o=>!o)&&s.push(E)}return s}function Ue(e,t=","){const n=[];let i="",d=!1;for(let s=0;s<e.length;s++){const m=e[s];d?m==='"'?e[s+1]==='"'?(i+='"',s++):d=!1:i+=m:m==='"'?d=!0:m===t?(n.push(i),i=""):i+=m}return n.push(i),n}const En={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};async function _n(e,{toast:t}){const n=Object.keys(ge);let i=n[0];const{data:d}=await _.from("formation_links_overrides").select("formation, links"),s={};(d||[]).forEach(l=>{s[l.formation]=l.links});let m=new Set;function x(l,a){return[l,a].sort().join("-")}function E(l){const a=s[l]||ge[l]||[];m=new Set(a.map(([r,u])=>x(r,u)))}E(i),e.innerHTML=`
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;gap:12px;flex-wrap:wrap">
  ...
  `;function o(){const l=jt[i]||{},a=Re(i),r=320,u=400,c=22;function f(v){const I=l[v];return I?{x:I.x*r,y:I.y*u}:null}let p=`<svg width="${r}" height="${u}" viewBox="0 0 ${r} ${u}" xmlns="http://www.w3.org/2000/svg" style="display:block">`;a.forEach(([v,I],h)=>{const b=f(v),$=f(I);if(!b||!$)return;const g=x(v,I),w=m.has(g),L=w?"#3b82f6":"#999",B=w?.95:.35,C=w?4:3;p+=`<line x1="${b.x}" y1="${b.y}" x2="${$.x}" y2="${$.y}"
        stroke="transparent" stroke-width="14" class="link-hit" data-key="${g}" style="cursor:pointer"/>`,p+=`<line x1="${b.x}" y1="${b.y}" x2="${$.x}" y2="${$.y}"
        stroke="${L}" stroke-width="${C}" stroke-dasharray="${w?"none":"4,3"}"
        opacity="${B}" pointer-events="none" data-line-key="${g}"/>`});for(const v of Object.keys(l)){const I=f(v);if(!I)continue;const h=v.replace(/\d+/,""),b=En[h]||"#555";p+=`<circle cx="${I.x}" cy="${I.y}" r="${c}" fill="${b}" stroke="rgba(255,255,255,0.7)" stroke-width="2" pointer-events="none"/>`,p+=`<text x="${I.x}" y="${I.y+4}" text-anchor="middle" font-size="10" font-weight="900" fill="white" font-family="Arial Black,Arial" pointer-events="none">${v}</text>`}p+="</svg>",document.getElementById("field-wrap").innerHTML=p;const y=document.getElementById("links-list");y.innerHTML=a.map(([v,I])=>{const h=x(v,I),b=m.has(h);return`
        ...
        </button>`}).join("");const k=e.querySelector('div[style*="font-weight:700;font-size:13px;margin-bottom:8px"]');k&&(k.textContent=`Liens (${m.size}/${a.length} actifs)`);function A(v){m.has(v)?m.delete(v):m.add(v),o()}e.querySelectorAll(".link-hit").forEach(v=>{v.addEventListener("click",()=>A(v.dataset.key))}),e.querySelectorAll(".link-toggle").forEach(v=>{v.addEventListener("click",()=>A(v.dataset.key))})}o(),document.getElementById("formation-select").addEventListener("change",l=>{i=l.target.value,E(i),o()}),document.getElementById("reset-btn").addEventListener("click",()=>{const l=ge[i]||[];m=new Set(l.map(([a,r])=>x(a,r))),o(),t("Liens réinitialisés aux valeurs par défaut (non sauvegardé)","info")}),document.getElementById("save-btn").addEventListener("click",async()=>{const a=Re(i).filter(([u,c])=>m.has(x(u,c))),{error:r}=await _.from("formation_links_overrides").upsert({formation:i,links:a,updated_at:new Date().toISOString()});if(r){t(r.message,"error");return}s[i]=a,t(`Liens enregistrés pour ${i} (${a.length} actifs)`,"success")})}const wn=[{value:"player",label:"Joueur"},{value:"formation",label:"Formation"},{value:"game_changer",label:"Game Changer"},{value:"ultra_game_changer",label:"Ultra Game Changer"},{value:"game_helper",label:"Game Helper"},{value:"stadium",label:"🏟️ Stade"}],In=[{value:"",label:"Toutes"},{value:"normal",label:"Normal"},{value:"pepite",label:"Pépite"},{value:"papyte",label:"Papyte"},{value:"legende",label:"Légende"}],$n=[{value:"credits",label:"Crédits"},{value:"pub",label:"Publicité"},{value:"free",label:"Gratuit"}];async function kn(e){e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">Chargement...</div>';const{data:t}=await _.from("booster_configs").select("*").order("sort_order");let n=null,i=[],d=null;const s=()=>window.innerWidth<700;async function m(){if(!n){i=[];return}if(n!==d){const{data:a}=await _.from("booster_drop_rates").select("*").eq("booster_id",n).order("sort_order");i=a||[],d=n}}function x(){return`
    ...
    </div>`}async function o(a=!1){a||await m();const r=!n&&s(),u=n&&s();e.innerHTML=`
    <div style="display:flex;height:calc(100vh - 56px);overflow:hidden">
      ${!s()||r||!n?x():""}
      ${!s()||u?E():""}
    </div>`,l()}function l(){var u,c,f,p,y,k,A;const a=v=>e.querySelector(v);e.querySelectorAll(".booster-row").forEach(v=>{v.addEventListener("click",I=>{I.target.closest(".btn-delete")||(n=v.dataset.id,d=null,o())})}),(u=a("#btn-back"))==null||u.addEventListener("click",()=>{n=null,o()}),(c=a("#btn-new"))==null||c.addEventListener("click",async()=>{const v=prompt("Nom du nouveau booster :");if(!(v!=null&&v.trim()))return;const{data:I,error:h}=await _.from("booster_configs").insert({name:v.trim(),price_type:"credits",price_credits:1e3,card_count:5}).select().single();if(h){alert(h.message);return}t.push(I),n=I.id,d=null,o()}),e.querySelectorAll(".btn-delete").forEach(v=>{v.addEventListener("click",async I=>{if(I.stopPropagation(),!confirm("Supprimer ce booster ?"))return;await _.from("booster_configs").delete().eq("id",v.dataset.id);const h=t.findIndex(b=>b.id===v.dataset.id);h>-1&&t.splice(h,1),n===v.dataset.id&&(n=null,d=null),o()})}),(f=a("#btn-cancel"))==null||f.addEventListener("click",()=>{n=null,o()}),(p=a("#f-price-type"))==null||p.addEventListener("change",v=>{const I=a("#credits-field");I&&(I.style.opacity=v.target.value!=="credits"?"0.4":"1")}),(y=a("#btn-pick-icon"))==null||y.addEventListener("click",async()=>{var I;const v=a("#icon-picker-grid");if(v){if(v.style.display!=="none"){v.style.display="none";return}v.innerHTML='<div style="padding:10px;text-align:center;color:#888;font-size:12px">⏳ Chargement...</div>',v.style.display="block";try{const b=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),$=Array.isArray(b)?b.filter(w=>w.name.startsWith("booster")&&/\.(png|jpg|svg|webp)$/i.test(w.name)):[];if(!$.length){v.innerHTML='<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Aucune icône "booster*" trouvée.</div>';return}const g=((I=a("#f-image-url"))==null?void 0:I.value)||"";v.innerHTML=`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(72px,1fr));gap:8px">
          ${$.map(w=>`
          <div class="icon-pick-item" data-name="${w.name}"
            style="cursor:pointer;text-align:center;padding:6px;border-radius:6px;border:2px solid ${w.name===g?"#1A6B3C":"#ddd"};background:${w.name===g?"#f0f7f0":"#fff"}">
        ...
        </div>`,v.querySelectorAll(".icon-pick-item").forEach(w=>{w.addEventListener("click",()=>{const L=a("#f-image-url");L&&(L.value=w.dataset.name),v.style.display="none"})})}catch(h){v.innerHTML=`<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Erreur API GitHub : ${h.message}</div>`}}});function r(){e.querySelectorAll("[data-rate-idx]").forEach(v=>{var h,b,$,g,w;const I=Number(v.dataset.rateIdx);I>=0&&I<i.length&&(i[I].card_type=((h=v.querySelector(".rate-type"))==null?void 0:h.value)||"player",i[I].rarity=((b=v.querySelector(".rate-rarity"))==null?void 0:b.value)||null,i[I].note_min=Number(($=v.querySelector(".rate-note-min"))==null?void 0:$.value)||null,i[I].note_max=Number((g=v.querySelector(".rate-note-max"))==null?void 0:g.value)||null,i[I].percentage=Number((w=v.querySelector(".rate-pct"))==null?void 0:w.value)||0)})}(k=a("#btn-add-rate"))==null||k.addEventListener("click",()=>{r(),i.push({id:null,booster_id:n,card_type:"player",rarity:"normal",note_min:null,note_max:null,percentage:10,sort_order:i.length}),o(!0)}),e.querySelectorAll(".btn-del-rate").forEach(v=>{v.addEventListener("click",()=>{r(),i.splice(Number(v.dataset.idx),1),o(!0)})}),(A=a("#btn-save"))==null||A.addEventListener("click",async()=>{var g,w,L,B,C,z,D,q,S,T,M,R,j;const v=(t||[]).find(P=>P.id===n);if(!v)return;const I=[];e.querySelectorAll("[data-rate-idx]").forEach(P=>{var K,V,J,te,ne;const H=Number(P.dataset.rateIdx);I.push({booster_id:n,card_type:((K=P.querySelector(".rate-type"))==null?void 0:K.value)||"player",rarity:((V=P.querySelector(".rate-rarity"))==null?void 0:V.value)||null,note_min:Number((J=P.querySelector(".rate-note-min"))==null?void 0:J.value)||null,note_max:Number((te=P.querySelector(".rate-note-max"))==null?void 0:te.value)||null,percentage:Number((ne=P.querySelector(".rate-pct"))==null?void 0:ne.value)||0,sort_order:H});const U=I[I.length-1];U.rarity||(U.rarity=null),U.note_min||(U.note_min=null),U.note_max||(U.note_max=null)});const h=I.reduce((P,H)=>P+H.percentage,0);if(I.length&&Math.abs(h-100)>.5){alert(`La somme doit faire 100% (actuellement ${h.toFixed(1)}%)`);return}const b={name:((w=(g=a("#f-name"))==null?void 0:g.value)==null?void 0:w.trim())||v.name,image_url:((B=(L=a("#f-image-url"))==null?void 0:L.value)==null?void 0:B.trim())||null,price_type:(C=a("#f-price-type"))==null?void 0:C.value,price_credits:Number((z=a("#f-price-credits"))==null?void 0:z.value)||0,card_count:Number((D=a("#f-card-count"))==null?void 0:D.value)||5,is_active:((q=a("#f-active"))==null?void 0:q.checked)??v.is_active,allow_duplicates:((S=a("#f-allow-dup"))==null?void 0:S.checked)??!0,sort_order:Number((T=a("#f-sort"))==null?void 0:T.value)||0,max_per_user:(M=a("#f-max-per-user"))!=null&&M.value?Number(a("#f-max-per-user").value):null,available_from:((R=a("#f-available-from"))==null?void 0:R.value)||null,available_until:((j=a("#f-available-until"))==null?void 0:j.value)||null},{error:$}=await _.from("booster_configs").update(b).eq("id",n);if($){alert($.message);return}if(Object.assign(v,b),await _.from("booster_drop_rates").delete().eq("booster_id",n),I.length){const{error:P}=await _.from("booster_drop_rates").insert(I);if(P){alert(P.message);return}}i=I,d=n,alert("✅ Booster enregistré !"),o(!0)})}o()}const yt=[{value:"normal",label:"Normal",color:"#888"},{value:"pepite",label:"Pépite",color:"#D4A017"},{value:"papyte",label:"Papyte",color:"#909090"},{value:"legende",label:"Légende",color:"#7a28b8"}];async function Ln(e){await Le(e)}async function Le(e){const{data:t,error:n}=await _.from("sell_price_configs").select("*").order("rarity").order("note_min");if(n){e.innerHTML=`<div style="padding:24px;color:red">Erreur : ${n.message}</div>`;return}e.innerHTML=`
    ...
    </tr>`}function qe(e){document.getElementById("config-form").style.display="block",document.getElementById("form-title").textContent=e?"Modifier la règle":"Nouvelle règle",document.getElementById("form-id").value=(e==null?void 0:e.id)||"",document.getElementById("form-rarity").value=(e==null?void 0:e.rarity)||"normal",document.getElementById("form-price").value=(e==null?void 0:e.price)||1e3,document.getElementById("form-note-min").value=(e==null?void 0:e.note_min)??1,document.getElementById("form-note-max").value=(e==null?void 0:e.note_max)??10,document.getElementById("config-form").scrollIntoView({behavior:"smooth"})}async function An(e){const t=document.getElementById("form-id").value,n=document.getElementById("form-rarity").value,i=parseInt(document.getElementById("form-price").value)||0,d=parseInt(document.getElementById("form-note-min").value)||1,s=parseInt(document.getElementById("form-note-max").value)||10;if(d>s){alert("Note min doit être ≤ note max");return}const m={rarity:n,price:i,note_min:d,note_max:s,updated_at:new Date().toISOString()};let x;if(t?{error:x}=await _.from("sell_price_configs").update(m).eq("id",t):{error:x}=await _.from("sell_price_configs").insert(m),x){alert("Erreur : "+x.message);return}document.getElementById("config-form").style.display="none",await Le(e)}async function Sn(e){await Be(e)}async function Be(e){const{data:t}=await _.from("patch_notes").select("*").order("published_at",{ascending:!1});e.innerHTML=`
      ...
      <div style="flex:1;overflow-y:auto">
        ...
        <div id="ts-list" style="display:flex;flex-direction:column;gap:10px">
          ...
          <div data-pick="${w.name}" style="cursor:pointer;border:2px solid transparent;border-radius:8px;padding:4px;text-align:center;transition:border-color .15s" onmouseover="this.style.borderColor='#1A6B3C'" onmouseout="this.style.borderColor='transparent'">
            <img src="${g}icons/${w.name}" style="width:70px;height:56px;object-fit:contain;display:block;border-radius:4px">
          ...
          </div>`).join("")+"</div>",E.querySelectorAll("[data-pick]").forEach(w=>{w.addEventListener("click",()=>{s&&(s.value=w.dataset.pick),o(),E.style.display="none"})})}catch(h){E.innerHTML='<div style="color:#cc2222;font-size:12px">Erreur chargement : '+h.message+"</div>"}}),(v=document.getElementById("ts-color"))==null||v.addEventListener("input",h=>{const b=document.getElementById("ts-color-hex");b&&(b.value=h.target.value)}),(I=document.getElementById("ts-color-hex"))==null||I.addEventListener("input",h=>{const b=h.target.value;if(/^#[0-9a-fA-F]{6}$/.test(b)){const $=document.getElementById("ts-color");$&&($.value=b)}}),e.querySelectorAll("[data-edit-row]").forEach(h=>{const b=(t||[]).find($=>$.id===h.dataset.editRow);b&&h.addEventListener("click",()=>Ge(b))}),e.querySelectorAll("[data-delete]").forEach(h=>{h.addEventListener("click",async()=>{confirm("Supprimer cette étape ?")&&(await _.from("tutorial_steps").delete().eq("id",h.dataset.delete),ee(e))})}),e.querySelectorAll("[data-toggle]").forEach(h=>{h.addEventListener("click",async()=>{const b=(t||[]).find($=>$.id===h.dataset.toggle);b&&(await _.from("tutorial_steps").update({is_active:!b.is_active}).eq("id",b.id),ee(e))})}),e.querySelectorAll("[data-up]").forEach(h=>{h.addEventListener("click",async()=>{const b=t||[],$=b.findIndex(g=>g.id===h.dataset.up);$<=0||(await Promise.all([_.from("tutorial_steps").update({step_order:b[$-1].step_order}).eq("id",b[$].id),_.from("tutorial_steps").update({step_order:b[$].step_order}).eq("id",b[$-1].id)]),ee(e))})}),e.querySelectorAll("[data-down]").forEach(h=>{h.addEventListener("click",async()=>{const b=t||[],$=b.findIndex(g=>g.id===h.dataset.down);$<0||$>=b.length-1||(await Promise.all([_.from("tutorial_steps").update({step_order:b[$+1].step_order}).eq("id",b[$].id),_.from("tutorial_steps").update({step_order:b[$].step_order}).eq("id",b[$+1].id)]),ee(e))})})}function Rn(e){return`
  <div data-edit-row="${e.id}" style="background:#fff;border-radius:10px;padding:12px 16px;box-shadow:0 1px 4px rgba(0,0,0,0.08);display:flex;align-items:center;gap:12px;cursor:pointer">
    ...
    <div style="padding:20px;max-width:1100px;margin:0 auto">
      ...
      <div style="display:flex;flex-wrap:wrap;gap:14px" id="st-list">
        ${(n||[]).length?(n||[]).map(m=>Nn(m)).join(""):'<div style="color:var(--tile-fg-dim);padding:24px;text-align:center;width:100%">Aucun stade créé.</div>'}
      </div>
    </div>`,(d=document.getElementById("st-add-btn"))==null||d.addEventListener("click",()=>Ke(null,i,e,t)),(s=document.getElementById("st-gen-countries-btn"))==null||s.addEventListener("click",async()=>{const{toast:m}=t,x=new Set((n||[]).filter(r=>!r.club_id&&r.country_code).map(r=>r.country_code)),E=xt.filter(([r])=>!x.has(r));if(!E.length){m("Tous les pays ont déjà un stade.","info");return}if(!confirm(`Créer ${E.length} stade(s) "Pays" manquant(s) ?`))return;const o=E.map(([r,u])=>({name:`Stade ${u}`,club_id:null,country_code:r})),{data:l,error:a}=await _.from("stadium_definitions").insert(o).select();if(a){m("Erreur : "+a.message,"error");return}l!=null&&l.length&&await _.from("cards").insert(l.map(r=>({card_type:"stadium",stadium_id:r.id}))),m(`${(l==null?void 0:l.length)||0} stade(s) Pays créé(s) ✅`,"success"),ce(e,t)}),e.querySelectorAll("[data-edit-stadium]").forEach(m=>{m.addEventListener("click",()=>{const x=(n||[]).find(E=>E.id===m.dataset.editStadium);x&&Ke(x,i,e,t)})})}function Nn(e){var d,s;let t=null;e.image_url?t=e.image_url.startsWith("http")?e.image_url:`${vt}icons/${e.image_url}`:(d=e.club)!=null&&d.logo_url?t=e.club.logo_url:e.country_code&&(t=`https://flagsapi.com/${e.country_code.slice(0,2).toUpperCase()}/flat/64.png`);const n=((s=e.club)==null?void 0:s.encoded_name)||e.country_code||"—",i=Qe(e.name,t,`${n}<br>+10 ⭐`,{width:140});return`<div data-edit-stadium="${e.id}" style="cursor:pointer;transition:transform .15s" onmouseover="this.style.transform='translateY(-3px)'" onmouseout="this.style.transform=''">
    ${i}
  </div>`}function Ke(e,t,n,i){var l,a,r;const{openModal:d,closeModal:s,toast:m}=i,x=`
  ...
  `);const o=()=>{var b,$,g,w,L,B,C;const u=((b=document.getElementById("st-name"))==null?void 0:b.value)||"NOM DU STADE",c=(g=($=document.getElementById("st-image"))==null?void 0:$.value)==null?void 0:g.trim(),f=(L=(w=document.getElementById("st-country"))==null?void 0:w.value)==null?void 0:L.trim(),p=document.getElementById("st-club"),y=(p==null?void 0:p.selectedIndex)||0,k=p&&y>0?p.options[y].text:"",A=((C=(B=p==null?void 0:p.options[y])==null?void 0:B.getAttribute)==null?void 0:C.call(B,"data-logo"))||"";let v=null;c?v=c.startsWith("http")?c:`${vt}icons/${c}`:A?v=A:f&&(v=`https://flagsapi.com/${f.toLowerCase().slice(0,2).toUpperCase()}/flat/64.png`);const I=k||f||"—",h=document.getElementById("st-preview-card");h&&(h.innerHTML=Qe(u,v,`${I}<br>+10 ⭐`,{width:140}))};o(),["st-name","st-club","st-country","st-image"].forEach(u=>{var c,f;(c=document.getElementById(u))==null||c.addEventListener("input",o),(f=document.getElementById(u))==null||f.addEventListener("change",o)}),(l=document.getElementById("st-cancel"))==null||l.addEventListener("click",()=>s()),(a=document.getElementById("st-save"))==null||a.addEventListener("click",async()=>{const u=document.getElementById("st-name").value.trim(),c=document.getElementById("st-club").value||null,f=document.getElementById("st-country").value.trim().toUpperCase()||null,p=document.getElementById("st-image").value.trim()||null;if(!u){m("Le nom est obligatoire","error");return}const y={name:u,club_id:c,country_code:f,image_url:p},{error:k}=e?await _.from("stadium_definitions").update(y).eq("id",e.id):await _.from("stadium_definitions").insert(y);if(k){m("Erreur : "+k.message,"error");return}m(e?"Stade modifié ✅":"Stade créé ✅","success"),s(),ce(n,i)}),(r=document.getElementById("st-delete"))==null||r.addEventListener("click",async()=>{confirm("Supprimer ce stade ?")&&(await _.from("stadium_definitions").delete().eq("id",e.id),m("Stade supprimé","success"),s(),ce(n,i))})}const W=[{value:"game_changer",label:"⚡ Game Changer",border:"#7a28b8",bg:"linear-gradient(135deg,#4a0080,#7a28b8)"},{value:"ultra_game_changer",label:"💎 Ultra Game Changer",border:"#00bcd4",bg:"linear-gradient(135deg,#006080,#00bcd4)"}],Fn=[{value:"purple",label:"Violet",hex:"#7a28b8"},{value:"light_blue",label:"Bleu clair",hex:"#00bcd4"}],Y=[{value:"BOOST_STAT",label:"⚡ Boost stats (+N)",hasValue:!0,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"DEBUFF_STAT",label:"💀 Debuff stats (-N)",hasValue:!0,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"GRAY_PLAYER",label:"❄️ Griser un joueur",hasValue:!1,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"REVIVE_PLAYER",label:"💫 Ressusciter un joueur",hasValue:!1,hasTarget:!1,hasCount:!0,hasRoles:!1},{value:"REMOVE_GOAL",label:"🚫 Retirer un but adverse",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1},{value:"ADD_GOAL_DRAW",label:"⚽ +1 but si duel nul",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1},{value:"ADD_SUB",label:"🔄 Ajouter un remplacement",hasValue:!1,hasTarget:!1,hasCount:!0,hasRoles:!1},{value:"CUSTOM",label:"🛠️ Effet custom (hardcodé)",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1}],Ve=["GK","DEF","MIL","ATT"];async function On(e,t){await Ae(e,t)}async function Ae(e,t){var i;const{data:n}=await _.from("gc_definitions").select("*").order("sort_order").order("created_at");e.innerHTML=`
    <div style="padding:20px;max-width:1100px;margin:0 auto">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px">
    ...
    </div>`,(i=document.getElementById("gc-add-btn"))==null||i.addEventListener("click",()=>Je(null,e,t)),e.querySelectorAll("[data-edit-gc]").forEach(d=>{d.addEventListener("click",()=>{const s=(n||[]).find(m=>m.id===d.dataset.editGc);s&&Je(s,e,t)})}),e.querySelectorAll(".btn-del-gc").forEach(d=>{d.addEventListener("click",async s=>{s.stopPropagation(),confirm("Supprimer cette carte Game Changer ?")&&(await _.from("gc_definitions").delete().eq("id",d.dataset.del),Ae(e,t))})})}function Un(e){const t=W.find(d=>d.value===e.gc_type)||W[0],n=e.image_url?`/icons/${e.image_url}`:null,i=be(e.name||"?",n,t.label.split(" ")[0],e.effect||"",{width:130});return`<div data-edit-gc="${e.id}" style="position:relative;cursor:pointer">
    ${i}
    <div style="position:absolute;top:4px;left:4px;z-index:10;display:flex;gap:3px">
  ...
  </div>`}function Je(e,t,n){var r,u,c,f;const{openModal:i,closeModal:d,toast:s}=n,m=!e,x=e||{name:"",gc_type:"game_changer",color:"purple",effect:"",image_url:"",sort_order:0,is_active:!0,effect_type:"BOOST_STAT",effect_params:{}},E=(()=>{const p=x.effect_params||{},y=Y.find(k=>k.value===(x.effect_type||"BOOST_STAT"))||Y[0];return`
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
      ${y.hasValue?`<div>
      ...
      ${y.hasRoles?`<div>
        ...
        <div style="display:flex;gap:4px;flex-wrap:wrap">
          ${Ve.map(k=>`<label style="display:flex;align-items:center;gap:2px;font-size:11px;cursor:pointer;color:#555">
    ...
    <div style="display:flex;justify-content:center;margin-bottom:16px;padding:20px;background:#1a1a2e;border-radius:12px">
      <div id="gc-modal-preview">${be(x.name||"?",x.image_url?`/icons/${x.image_url}`:null,(W.find(p=>p.value===x.gc_type)||W[0]).label.split(" ")[0],x.effect||"",{width:150})}</div>
    </div>
    ...
    <div style="display:flex;flex-direction:column;gap:12px">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
        <div>
          <label>TYPE</label>
          <select id="gc-type">
            ${W.map(p=>`<option value="${p.value}" ${x.gc_type===p.value?"selected":""}>${p.label}</option>`).join("")}
          </select>
        </div>
        <div>
          <label>COULEUR</label>
      ...
      <div style="border-top:1px solid #eee;padding-top:12px;margin-top:4px">
        ...
        <div>
          ...
          <select id="gc-effect-type" style="margin-bottom:10px">
            ${Y.map(p=>`<option value="${p.value}" ${(x.effect_type||"BOOST_STAT")===p.value?"selected":""}>${p.label}</option>`).join("")}
          </select>
        </div>
  ...
  `);const a=()=>{var h,b,$,g,w;const p=((h=document.getElementById("gc-name"))==null?void 0:h.value)||"?",y=((b=document.getElementById("gc-effect"))==null?void 0:b.value)||"",k=(($=document.getElementById("gc-type"))==null?void 0:$.value)||"game_changer",A=(w=(g=document.getElementById("gc-image-url"))==null?void 0:g.value)==null?void 0:w.trim(),v=W.find(L=>L.value===k)||W[0],I=document.getElementById("gc-modal-preview");I&&(I.innerHTML=be(p,A?`/icons/${A}`:null,v.label.split(" ")[0],y,{width:150}))};["gc-name","gc-effect","gc-type","gc-image-url"].forEach(p=>{var y,k;(y=document.getElementById(p))==null||y.addEventListener("input",a),(k=document.getElementById(p))==null||k.addEventListener("change",a)}),(r=document.getElementById("gc-effect-type"))==null||r.addEventListener("change",()=>{const p=document.getElementById("gc-effect-type").value,y=Y.find(A=>A.value===p)||Y[0],k=document.getElementById("gc-params-wrap");k.innerHTML=`
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
      ...
      ${y.hasRoles?`<div><label>POSTES CIBLÉS</label><div style="display:flex;gap:4px;flex-wrap:wrap">${Ve.map(A=>`<label style="display:flex;align-items:center;gap:2px;font-size:11px;cursor:pointer;color:#555"><input type="checkbox" class="gc-p-role" value="${A}" checked> ${A}</label>`).join("")}</div></div>`:""}
    </div>`}),(u=document.getElementById("btn-pick-gc-icon"))==null||u.addEventListener("click",async()=>{var y;const p=document.getElementById("gc-icon-picker");if(p){if(p.style.display!=="none"){p.style.display="none";return}p.innerHTML='<div style="padding:10px;text-align:center;color:#888;font-size:12px">⏳ Chargement...</div>',p.style.display="block";try{const A=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),v=Array.isArray(A)?A.filter(h=>h.name.startsWith("gamechanger-")&&/\.(png|jpg|svg|webp)$/i.test(h.name)):[];if(!v.length){p.innerHTML='<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Aucun fichier "gamechanger-*" trouvé dans public/icons/</div>';return}const I=((y=document.getElementById("gc-image-url"))==null?void 0:y.value)||"";p.innerHTML=`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(72px,1fr));gap:8px">
        ${v.map(h=>`
      ...
      </div>`,p.querySelectorAll(".gc-icon-item").forEach(h=>{h.addEventListener("click",()=>{const b=document.getElementById("gc-image-url");b&&(b.value=h.dataset.name),p.style.display="none",a()})})}catch(k){p.innerHTML=`<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Erreur API GitHub : ${k.message}</div>`}}}),(c=document.getElementById("gc-cancel"))==null||c.addEventListener("click",()=>d()),(f=document.getElementById("gc-save"))==null||f.addEventListener("click",async()=>{var I,h,b,$,g,w,L,B,C,z,D,q,S,T;const p=Y.find(M=>{var R;return M.value===(((R=document.getElementById("gc-effect-type"))==null?void 0:R.value)||"BOOST_STAT")})||Y[0],y=p.hasRoles?[...document.querySelectorAll(".gc-p-role:checked")].map(M=>M.value):null,k={...p.hasValue?{value:Number((I=document.getElementById("gc-p-value"))==null?void 0:I.value)||2}:{},...p.hasCount?{count:Number((h=document.getElementById("gc-p-count"))==null?void 0:h.value)||1}:{},...p.hasTarget?{target:((b=document.getElementById("gc-p-target"))==null?void 0:b.value)||"home"}:{},...p.hasRoles?{roles:y!=null&&y.length?y:null}:{}},A={name:((g=($=document.getElementById("gc-name"))==null?void 0:$.value)==null?void 0:g.trim())||"",effect:((L=(w=document.getElementById("gc-effect"))==null?void 0:w.value)==null?void 0:L.trim())||null,image_url:((C=(B=document.getElementById("gc-image-url"))==null?void 0:B.value)==null?void 0:C.trim())||null,gc_type:((z=document.getElementById("gc-type"))==null?void 0:z.value)||"game_changer",color:((D=document.getElementById("gc-color"))==null?void 0:D.value)||"purple",sort_order:Number((q=document.getElementById("gc-sort"))==null?void 0:q.value)||0,is_active:((S=document.getElementById("gc-active"))==null?void 0:S.checked)??!0,effect_type:((T=document.getElementById("gc-effect-type"))==null?void 0:T.value)||"BOOST_STAT",effect_params:k};if(!A.name){s("Le nom est obligatoire","error");return}const{error:v}=m?await _.from("gc_definitions").insert(A):await _.from("gc_definitions").update(A).eq("id",x.id);if(v){s(v.message,"error");return}s(m?"Carte créée ✅":"Carte modifiée ✅","success"),d(),Ae(t,n)})}async function qn(e,{toast:t,openModal:n,closeModal:i}){await N(e,{toast:t,openModal:n,closeModal:i})}async function N(e,t){var h,b,$;const{toast:n,openModal:i,closeModal:d}=t,[{data:s,error:m},{data:x},{data:E},{data:o}]=await Promise.all([_.from("ranked_seasons").select("*").order("start_at",{ascending:!1}),_.from("users").select("mmr,rank_tier,placement_matches").gte("placement_matches",1),_.from("season_reward_tiers").select("*").order("rank_min"),_.from("booster_configs").select("id,name").order("sort_order")]);if(m){e.innerHTML=`<p style="color:red">${m.message}</p>`;return}const l=s||[],a=l.find(g=>g.is_active),r=(x||[]).filter(g=>(g.placement_matches||0)>=1),u=r.filter(g=>(g.placement_matches||0)>=10),c=["bronze","silver","gold","platinum","diamond","master"],f={bronze:"Bronze",silver:"Argent",gold:"Or",platinum:"Platine",diamond:"Diamant",master:"Maître"},p={bronze:"#cd7f32",silver:"#a0a0a0",gold:"#D4A017",platinum:"#00e5ff",diamond:"#5b9bd5",master:"#9b59b6"},y={bronze:"🥉",silver:"🥈",gold:"🥇",platinum:"💎",diamond:"🔷",master:"👑"},k={};c.forEach(g=>{k[g]=0}),u.forEach(g=>{const w=g.rank_tier||"bronze";k[w]!==void 0&&k[w]++});const A=u.length?Math.round(u.reduce((g,w)=>g+(w.mmr||1e3),0)/u.length):0;function v(g){return g?new Date(g).toLocaleDateString("fr",{day:"numeric",month:"short",year:"numeric"}):"—"}function I(g){const w=new Date;return g.is_active?'<span style="background:#1A6B3C;color:#fff;padding:2px 8px;border-radius:10px;font-size:11px;font-weight:700">● ACTIVE</span>':new Date(g.end_at)<w?'<span style="background:#555;color:#ccc;padding:2px 8px;border-radius:10px;font-size:11px">Terminée</span>':'<span style="background:#e67e22;color:#fff;padding:2px 8px;border-radius:10px;font-size:11px">À venir</span>'}e.innerHTML=`
    <div style="display:flex;flex-direction:column;gap:20px">
      ...
      <div class="card-panel" style="background:linear-gradient(135deg,#0a3d1e,#1a6b3c20);border:1.5px solid #1A6B3C40">
        ...
        <div style="display:flex;flex-direction:column;gap:4px">
          ${c.map(g=>{const w=k[g],L=u.length>0?Math.round(w/u.length*100):0;return`
              <div style="display:flex;align-items:center;gap:8px;font-size:12px">
                <span style="width:80px">${y[g]} ${f[g]}</span>
                <div style="flex:1;background:rgba(255,255,255,0.08);border-radius:4px;height:14px;overflow:hidden">
                  <div style="height:100%;width:${L}%;background:${p[g]};border-radius:4px;transition:width 0.5s"></div>
                </div>
        ...
        <div class="table-wrap">
          <table>
            ...
            <tbody id="seasons-tbody">
              ${l.length===0?'<tr><td colspan="6" style="text-align:center;color:var(--gray-600);padding:24px">Aucune saison créée.</td></tr>':l.map(g=>{const w=Math.round((new Date(g.end_at)-new Date(g.start_at))/864e5),L=new Date(g.end_at)<new Date&&!g.is_active;return`
                    <tr>
                      <td><b>${g.name}</b></td>
                      <td style="font-size:12px">${v(g.start_at)}</td>
                      <td style="font-size:12px">${v(g.end_at)}</td>
                      ...
                      <td style="display:flex;gap:6px;flex-wrap:wrap">
                        ...
                        ${L?`<button class="btn btn-danger btn-sm" data-delete="${g.id}">🗑</button>`:""}
                      </td>
                    </tr>`}).join("")}
            </tbody>
          </table>
      ...
      <div class="card-panel" id="rewards-section">
        ...
        <div class="table-wrap">
          <table>
            ...
            <tbody>
              ${(E||[]).map(g=>{var B;const w=!g.activate_at||new Date(g.activate_at)<=new Date;return`<tr>
                  <td style="font-size:12px">${((B=l.find(C=>C.id===g.season_id))==null?void 0:B.name)||"—"}</td>
                  <td><b>${g.label}</b></td>
    ...
    </div>`,(h=document.getElementById("create-season-btn"))==null||h.addEventListener("click",()=>{Ye(null,{toast:n,openModal:i,closeModal:d,reload:()=>N(e,t)})}),(b=document.getElementById("start-new-season-btn"))==null||b.addEventListener("click",()=>{Hn(a,{toast:n,openModal:i,closeModal:d,reload:()=>N(e,t)})}),e.querySelectorAll("[data-edit]").forEach(g=>{const w=l.find(L=>L.id==g.dataset.edit);g.addEventListener("click",()=>{Ye(w,{toast:n,openModal:i,closeModal:d,reload:()=>N(e,t)})})}),e.querySelectorAll("[data-activate]").forEach(g=>{g.addEventListener("click",async()=>{const w=parseInt(g.dataset.activate);if(!confirm("Activer cette saison ? La saison actuellement active sera désactivée."))return;const{error:L}=await _.from("ranked_seasons").update({is_active:!1}).neq("id",0);if(L){n(L.message,"error");return}const{error:B}=await _.from("ranked_seasons").update({is_active:!0}).eq("id",w);if(B){n(B.message,"error");return}await _.from("users").update({current_season_id:w}).gt("placement_matches",0),n("Saison activée ✅","success"),N(e,t)})}),e.querySelectorAll("[data-rewards-for]").forEach(g=>{g.addEventListener("click",()=>{var B;const w=parseInt(g.dataset.rewardsFor);(B=document.getElementById("rewards-section"))==null||B.scrollIntoView({behavior:"smooth",block:"start"});const L=l.find(C=>C.id===w);ye(null,l,L,o||[],{toast:n,openModal:i,closeModal:d,reload:()=>N(e,t)})})}),e.querySelectorAll("[data-launch]").forEach(g=>{g.addEventListener("click",async()=>{const w=parseInt(g.dataset.launch),L=l.find(z=>z.id===w);if(!confirm(`Lancer "${L==null?void 0:L.name}" ?
...
Action irréversible. Continuer ?`))return;g.disabled=!0,g.textContent="⏳ Lancement...";const{data:B,error:C}=await _.rpc("admin_launch_season",{p_season_id:w});if(g.disabled=!1,g.textContent="▶ Lancer la saison",C){n(C.message,"error");return}if(!(B!=null&&B.success)){n((B==null?void 0:B.error)||"Échec du lancement","error");return}n(`Saison lancée ✅ (${B.logged} classement(s) archivé(s), ${B.reset} joueur(s) recalculé(s))`,"success"),N(e,t)})}),e.querySelectorAll("[data-deactivate]").forEach(g=>{g.addEventListener("click",async()=>{if(!confirm("Désactiver la saison active ? Le mode Ranked sera suspendu."))return;const{error:w}=await _.from("ranked_seasons").update({is_active:!1}).eq("id",parseInt(g.dataset.deactivate));if(w){n(w.message,"error");return}n("Saison désactivée","success"),N(e,t)})}),e.querySelectorAll("[data-delete]").forEach(g=>{g.addEventListener("click",async()=>{if(!confirm("Supprimer cette saison archivée ?"))return;const{error:w}=await _.from("ranked_seasons").delete().eq("id",parseInt(g.dataset.delete));if(w){n(w.message,"error");return}n("Saison supprimée","success"),N(e,t)})}),($=document.getElementById("add-reward-tier-btn"))==null||$.addEventListener("click",()=>{ye(null,l,a,o||[],{toast:n,openModal:i,closeModal:d,reload:()=>N(e,t)})}),e.querySelectorAll("[data-edit-tier]").forEach(g=>{const w=(E||[]).find(L=>L.id===g.dataset.editTier);g.addEventListener("click",()=>{ye(w,l,a,o||[],{toast:n,openModal:i,closeModal:d,reload:()=>N(e,t)})})}),e.querySelectorAll("[data-distribute-tier]").forEach(g=>{g.addEventListener("click",async()=>{const w=(E||[]).find(C=>C.id===g.dataset.distributeTier);if(!confirm(`Distribuer les récompenses du palier "${w==null?void 0:w.label}" à tous les joueurs concernés ?`))return;g.disabled=!0,g.textContent="⏳...";const{data:L,error:B}=await _.rpc("admin_distribute_season_reward",{p_tier_id:g.dataset.distributeTier});if(B){n(B.message,"error"),g.disabled=!1,g.textContent="🎁 Distribuer";return}if(!(L!=null&&L.success)){n((L==null?void 0:L.error)||"Échec de la distribution","error"),g.disabled=!1,g.textContent="🎁 Distribuer";return}n(`Récompenses distribuées à ${L.rewarded} joueur(s) ✅`,"success"),N(e,t)})}),e.querySelectorAll("[data-delete-tier]").forEach(g=>{g.addEventListener("click",async()=>{if(!confirm("Supprimer ce palier de récompense ?"))return;const{error:w}=await _.from("season_reward_tiers").delete().eq("id",g.dataset.deleteTier);if(w){n(w.message,"error");return}n("Palier supprimé","success"),N(e,t)})})}function ye(e,t,n,i,{toast:d,openModal:s,closeModal:m,reload:x}){var f,p,y,k,A;const E=!!e,o=(e==null?void 0:e.season_id)??(n==null?void 0:n.id)??((f=t[0])==null?void 0:f.id)??null;let l=[];const a=v=>{if(!v)return"";const I=new Date(v);return new Date(I.getTime()-I.getTimezoneOffset()*6e4).toISOString().slice(0,16)},r=`
    <div style="display:flex;flex-direction:column;gap:12px">
      <div>
  ...
  `);function c(){const v=document.getElementById("rt-player-chips");v&&(v.innerHTML=l.map(I=>`
      ...
      </div>`).join(""),h.querySelectorAll("[data-add-player]").forEach(b=>{b.addEventListener("click",()=>{const $=b.dataset.addPlayer;l.some(g=>g.id===$)||l.push({id:$,label:b.dataset.label}),c(),h.style.display="none",document.getElementById("rt-player-search").value=""})})}),(k=document.getElementById("rt-cancel"))==null||k.addEventListener("click",()=>m()),(A=document.getElementById("rt-save"))==null||A.addEventListener("click",async()=>{const v=document.getElementById("rt-error"),I=parseInt(document.getElementById("rt-season").value),h=document.getElementById("rt-label").value.trim(),b=parseInt(document.getElementById("rt-rank-min").value)||1,$=parseInt(document.getElementById("rt-rank-max").value)||1,g=parseInt(document.getElementById("rt-credits").value)||0,w=document.getElementById("rt-activate-at").value,L=w?new Date(w).toISOString():null,B=[...document.querySelectorAll(".rt-booster-cb:checked")].map(D=>D.value);if(!h){v.textContent="Le libellé est obligatoire.";return}if(b>$){v.textContent="Le classement min doit être ≤ au max.";return}if(!I){v.textContent="Choisis une saison.";return}const C={season_id:I,label:h,rank_min:b,rank_max:$,credits:g,booster_config_ids:B,player_ids:l.map(D=>D.id),activate_at:L},{error:z}=E?await _.from("season_reward_tiers").update(C).eq("id",e.id):await _.from("season_reward_tiers").insert(C);if(z){v.textContent=z.message;return}d(E?"Palier modifié ✅":"Palier créé ✅","success"),m(),x()})}function Hn(e,{toast:t,openModal:n,closeModal:i,reload:d}){var l,a;const s=new Date,m=new Date(s.getTime()+30*864e5),x=r=>r.toISOString().slice(0,10),E=`
    <div style="display:flex;flex-direction:column;gap:12px">
  ...
  `),(l=document.getElementById("sns-cancel"))==null||l.addEventListener("click",()=>i()),(a=document.getElementById("sns-launch"))==null||a.addEventListener("click",async()=>{const r=document.getElementById("sns-error"),u=document.getElementById("sns-name").value.trim(),c=document.getElementById("sns-start").value,f=document.getElementById("sns-end").value,p=document.getElementById("sns-confirm").checked;if(!u){r.textContent="Le nom de la saison est requis.";return}if(!c||!f){r.textContent="Les deux dates sont requises.";return}if(new Date(c)>=new Date(f)){r.textContent="La date de fin doit être après la date de début.";return}if(!p){r.textContent="Coche la case de confirmation pour continuer.";return}const y=document.getElementById("sns-launch");y.disabled=!0,y.textContent="⏳ Recalcul en cours...";const{data:k,error:A}=await _.rpc("admin_start_new_season",{p_name:u,p_start_at:new Date(c).toISOString(),p_end_at:new Date(f).toISOString()});if(y.disabled=!1,y.textContent="🚀 Démarrer la saison",A){r.textContent=A.message;return}if(!(k!=null&&k.success)){r.textContent=(k==null?void 0:k.error)||"Échec de l'opération.";return}t(`Nouvelle saison démarrée ✅ (${k.users_reset} joueur(s) recalculé(s))`,"success"),i(),d()})}function Ye(e,{toast:t,openModal:n,closeModal:i,reload:d}){const s=!!e,m=new Date().toISOString().slice(0,16),x=new Date(Date.now()+90*864e5).toISOString().slice(0,16),E=e?new Date(e.start_at).toISOString().slice(0,16):m,o=e?new Date(e.end_at).toISOString().slice(0,16):x,l=(e==null?void 0:e.name)||"";n(s?"✏️ Modifier la saison":"➕ Nouvelle saison",`<div style="display:flex;flex-direction:column;gap:14px">
      <div class="form-group">
        <label>Nom de la saison *</label>
    ...
    </div>`);function a(){var p,y;const r=(p=document.getElementById("season-start"))==null?void 0:p.value,u=(y=document.getElementById("season-end"))==null?void 0:y.value,c=document.getElementById("season-duration");if(!r||!u||!c)return;const f=Math.round((new Date(u)-new Date(r))/864e5);c.textContent=f>0?`Durée : ${f} jour${f>1?"s":""}`:"⚠️ La fin doit être après le début",c.style.color=f>0?"var(--gray-600)":"#bb2020"}setTimeout(()=>{var r,u,c;(r=document.getElementById("season-start"))==null||r.addEventListener("input",a),(u=document.getElementById("season-end"))==null||u.addEventListener("input",a),a(),(c=document.getElementById("season-save-btn"))==null||c.addEventListener("click",async()=>{var h,b,$;const f=(h=document.getElementById("season-name"))==null?void 0:h.value.trim(),p=(b=document.getElementById("season-start"))==null?void 0:b.value,y=($=document.getElementById("season-end"))==null?void 0:$.value,k=document.getElementById("season-error");if(!f){k.textContent="Le nom est requis.";return}if(!p){k.textContent="La date de début est requise.";return}if(!y){k.textContent="La date de fin est requise.";return}if(new Date(y)<=new Date(p)){k.textContent="La date de fin doit être après le début.";return}const A=document.getElementById("season-save-btn");A.disabled=!0,A.textContent="Enregistrement…";const v={name:f,start_at:new Date(p).toISOString(),end_at:new Date(y).toISOString()};let I;if(s?{error:I}=await _.from("ranked_seasons").update(v).eq("id",e.id):{error:I}=await _.from("ranked_seasons").insert({...v,is_active:!1}),I){k.textContent=I.message,A.disabled=!1,A.textContent=s?"💾 Enregistrer":"✅ Créer la saison";return}t(s?"Saison modifiée ✅":"Saison créée ✅","success"),i(),d()})},50)}async function he(e,t){var E;const{toast:n}=t,{data:i,error:d}=await _.from("solo_levels").select("*").order("level_number");if(d){e.innerHTML=`<p style="color:var(--danger)">${d.message}</p>`;return}const s=o=>`
    <tr data-row="${o.id}">
