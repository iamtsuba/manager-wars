...
`).filter(o=>o.trim());if(i.length<2)return[];const t=i[0],n=t.split(";").length>t.split(",").length?";":",",s=ye(t,n).map(o=>o.trim().replace(/^\uFEFF/,"").toLowerCase()),l=[];for(let o=1;o<i.length;o++){if(!i[o].trim())continue;const r=ye(i[o],n),d={};s.forEach((c,m)=>{d[c]=(r[m]||"").trim()}),!Object.values(d).every(c=>!c)&&l.push(d)}return l}function ye(e,i=","){const t=[];let n="",s=!1;for(let l=0;l<e.length;l++){const o=e[l];s?o==='"'?e[l+1]==='"'?(n+='"',l++):s=!1:n+=o:o==='"'?s=!0:o===i?(t.push(n),n=""):n+=o}return t.push(n),t}const lt={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};async function dt(e,{toast:i}){const t=Object.keys(ie);let n=t[0];const{data:s}=await y.from("formation_links_overrides").select("formation, links"),l={};(s||[]).forEach(m=>{l[m.formation]=m.links});let o=new Set;function r(m,p){return[m,p].sort().join("-")}function d(m){const p=l[m]||ie[m]||[];o=new Set(p.map(([a,f])=>r(a,f)))}d(n),e.innerHTML=`
  ...
  <div style="background:#fff;border-radius:10px;padding:12px 16px;box-shadow:0 1px 4px rgba(0,0,0,0.08);display:flex;align-items:center;gap:12px">
  ...
  </div>`}function we(e,i=0){document.getElementById("ts-form").style.display="block",document.getElementById("ts-form-title").textContent=e?"Modifier l'étape":"Nouvelle étape",document.getElementById("ts-id").value=(e==null?void 0:e.id)||"",document.getElementById("ts-emoji").value=(e==null?void 0:e.emoji)||"⚽",document.getElementById("ts-title").value=(e==null?void 0:e.title)||"",document.getElementById("ts-content").value=(e==null?void 0:e.content)||"";const t=document.getElementById("ts-editor");t&&(t.innerHTML=(e==null?void 0:e.content)||"<p>Texte de l'étape…</p>"),document.getElementById("ts-order").value=(e==null?void 0:e.step_order)??i,document.getElementById("ts-active").checked=e?e.is_active:!0;const n=(e==null?void 0:e.color)||"#1A6B3C";document.getElementById("ts-color").value=n,document.getElementById("ts-color-hex").value=n;const s=(e==null?void 0:e.image_url)||"",l=document.getElementById("ts-image");l&&(l.value=s);const o=document.getElementById("ts-img-preview"),r=document.getElementById("ts-img-preview-el");if(s&&o&&r){const d="/";r.src=`${d}icons/${s}`,o.style.display="block"}else o&&(o.style.display="none");document.getElementById("ts-img-picker-grid").style.display="none",document.getElementById("ts-preview-area").innerHTML="",document.getElementById("ts-form").scrollIntoView({behavior:"smooth"})}function je(){document.getElementById("ts-form").style.display="none",document.getElementById("ts-preview-area").innerHTML=""}function Et(){var c,m;const e=document.getElementById("ts-emoji").value||"⚽",i=document.getElementById("ts-title").value||"Titre",t=document.getElementById("ts-editor"),n=t?t.innerHTML:document.getElementById("ts-content").value||"",s=document.getElementById("ts-color-hex").value||"#1A6B3C",l=(m=(c=document.getElementById("ts-image"))==null?void 0:c.value)==null?void 0:m.trim(),r=l?`<div style="padding:0 20px 12px;text-align:center"><img src="/icons/${l}" style="max-height:140px;max-width:100%;border-radius:10px;object-fit:contain"></div>`:"",d=document.getElementById("ts-preview-area");d.innerHTML=`
    ...
    <div style="background:rgba(0,0,0,0.3);border-radius:6px;padding:4px 6px;text-align:center;margin-top:4px">
      ...
      <div style="flex:1;display:flex;align-items:center;justify-content:center;padding:4px">
        ${a.image_url?`<img src="/icons/${a.image_url}" style="max-height:${_}px;max-width:90%;object-fit:contain">`:`<div style="font-size:${Math.round(f*.3)}px">${g.label.split(" ")[0]}</div>`}
      </div>
      ...
      ${(i||[]).map(a=>{const f=R.find(E=>E.value===a.gc_type)||R[0],g=N.find(E=>E.value===a.color)||N[0];return`
        <div class="gc-row" data-id="${a.id}"
          ...
          <div style="width:36px;height:50px;border-radius:5px;border:2px solid ${g.hex};background:${f.bg};display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:18px">
            ${a.image_url?`<img src="/icons/${a.image_url}" style="width:28px;height:28px;object-fit:contain">`:f.label.split(" ")[0]}
          </div>
          <div style="flex:1;min-width:0">
