import{s as _,r as Fe,g as He,K as Ie,D,S as at,a as O,H as lt,R as st,e as xe,b as Ue,c as dt,d as ct,E as pt,f as ut,N as mt,M as gt,B as ft,h as bt,F as ve,i as ke,j as ie,k as xt,l as vt}from"./match-shared-3YVU6Yuq.js";async function yt(e){const[{count:t},{count:n},{count:i},{count:p},{count:d}]=await Promise.all([_.from("players").select("*",{count:"exact",head:!0}),_.from("clubs").select("*",{count:"exact",head:!0}),_.from("users").select("*",{count:"exact",head:!0}),_.from("cards").select("*",{count:"exact",head:!0}),_.from("matches").select("*",{count:"exact",head:!0})]);e.innerHTML=`
  ...
  `,window.adminNav=c=>{var r;(r=document.querySelector(`[data-page="${c}"]`))==null||r.click()}}function ae(e,t,n,i){return`<div class="card-panel" style="text-align:center">
  ...
  </div>`}const Ve="/",Ke={normal:"Normal",pepite:"Pépite",papyte:"Papyte",legende:"Légende"},ht={GK:"#aaaaaa",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};let le=null;async function Et(){if(le)return le;try{le=await(await fetch(Ve+"faces-manifest.json")).json()}catch{le={}}return le}async function _t(e,t){await ge(e,t)}async function ge(e,t,n=null){var a,u,l,o,s;const{toast:i}=t;n||(n={search:((a=document.getElementById("search-players"))==null?void 0:a.value)||"",job:((u=document.getElementById("filter-job"))==null?void 0:u.value)||"",rarity:((l=document.getElementById("filter-rarity"))==null?void 0:l.value)||"",club:((o=document.getElementById("filter-club"))==null?void 0:o.value)||"",country:((s=document.getElementById("filter-country"))==null?void 0:s.value)||""});const[{data:p,error:d},{data:c}]=await Promise.all([_.from("players").select("*, clubs(id,encoded_name,logo_url)"),_.from("clubs").select("id,encoded_name").order("encoded_name")]);if(d){e.innerHTML=`<p style="color:red">${d.message}</p>`;return}const r={GK:0,DEF:1,MIL:2,ATT:3},m=(p||[]).sort((g,E)=>{const h=(r[g.job]??4)-(r[E.job]??4);return h!==0?h:(g.surname_real||"").localeCompare(E.surname_real||"")});wt(e,m,c||[],t,n)}function wt(e,t,n,i,p=null){var u,l;const{toast:d}=i;if(e.innerHTML=`
    <div style="display:flex;flex-direction:column;gap:10px">
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        ...
        <select id="filter-rarity" style="width:110px">
          ...
          ${Object.entries(Ke).map(([o,s])=>`<option value="${o}">${s}</option>`).join("")}
        </select>
      ...
      <div style="flex:1;min-width:300px;display:flex;flex-direction:column;gap:12px">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:8px">
          ...
          <div class="form-group">
            <label>Rareté</label>
            <select id="pm-rarity">
              ${["normal","pepite","papyte","legende"].map(s=>`<option value="${s}" ${(e==null?void 0:e.rarity)===s?"selected":""}>${Ke[s]}</option>`).join("")}
            </select>
          </div>
          <div class="form-group">
            <label>Pays</label>
    ...
    <div id="clubs-list" style="display:flex;flex-direction:column;gap:8px"></div>`,c(pe),document.getElementById("search-clubs").addEventListener("input",r=>{const m=r.target.value.toLowerCase();c(pe.filter(a=>a.real_name.toLowerCase().includes(m)||a.encoded_name.toLowerCase().includes(m)))}),document.getElementById("add-club-btn").addEventListener("click",()=>Le(null,e,t));function c(r){const m=document.getElementById("clubs-list");if(!r.length){m.innerHTML='<p style="color:var(--gray-600);padding:20px">Aucun club.</p>';return}m.innerHTML=r.map(a=>{const u=Ze(a),l=He(u,a.id).replace("<svg ",'<svg style="width:40px;height:48px" '),o=a.logo_url?`<img src="${a.logo_url}" style="width:40px;height:40px;object-fit:contain;border-radius:8px">`:`<div style="width:40px;height:40px;background:linear-gradient(135deg,${u.color1},${u.color2});border-radius:8px;display:flex;align-items:center;justify-content:center;color:#fff;font-size:10px;font-weight:900">${a.encoded_name.slice(0,3)}</div>`,s=n[a.id]||0;return`
    ...
    </div>`),setTimeout(()=>{var g,E,h;se();function u(){var x,b;const f=((x=document.getElementById("m-kit-style"))==null?void 0:x.value)||"uni",w=((b=Ie[f])==null?void 0:b.colors)===3,I=document.getElementById("wrap-m-kit-color3");I&&(I.style.display=w?"":"none")}u(),["m-kit-color1","m-kit-color2","m-kit-color3","m-kit-shorts","m-kit-socks"].forEach(f=>{const w=document.getElementById(f),I=document.getElementById(f+"-txt");!w||!I||(w.addEventListener("input",()=>{I.value=w.value,se()}),I.addEventListener("input",()=>{const x=I.value.trim();/^#[0-9a-fA-F]{6}$/.test(x)&&(w.value=x,se())}),I.addEventListener("change",()=>{let x=I.value.trim();x.startsWith("#")||(x="#"+x),/^#[0-9a-fA-F]{6}$/.test(x)&&(w.value=x,I.value=x,se())}))}),(g=document.getElementById("m-kit-style"))==null||g.addEventListener("change",()=>{u(),se()});const l=document.getElementById("m-real"),o=document.getElementById("m-encoded");function s(){if(!l||!o||o.value)return;const f=l.value.trim().split(/\s+/),w=f.length===1?f[0].toUpperCase().slice(0,6):f.filter(I=>I.length>2).map(I=>I[0].toUpperCase()).join("")||f[0].toUpperCase().slice(0,4);o.value=w}l==null||l.addEventListener("input",s),(E=document.getElementById("auto-encode-btn"))==null||E.addEventListener("click",()=>{o&&(o.value=""),s()}),(h=document.getElementById("m-save"))==null||h.addEventListener("click",()=>Rt(e,c,t,n))},50)}function Xe(){var e,t,n,i,p,d;return{style:((e=document.getElementById("m-kit-style"))==null?void 0:e.value)||"uni",color1:((t=document.getElementById("m-kit-color1"))==null?void 0:t.value)||"#1A6B3C",color2:((n=document.getElementById("m-kit-color2"))==null?void 0:n.value)||"#ffffff",color3:((i=document.getElementById("m-kit-color3"))==null?void 0:i.value)||"#000000",shorts:((p=document.getElementById("m-kit-shorts"))==null?void 0:p.value)||"#111111",socks:((d=document.getElementById("m-kit-socks"))==null?void 0:d.value)||"#ffffff"}}function se(){const e=document.getElementById("kit-preview-wrap");e&&(e.innerHTML=He(Xe(),"modal"))}async function Rt(e,t,n,i){var f,w,I,x,b,v;const{toast:p,closeModal:d}=i,c=document.getElementById("m-error"),r=document.getElementById("m-save"),m=(f=document.getElementById("m-real"))==null?void 0:f.value.trim(),a=(w=document.getElementById("m-encoded"))==null?void 0:w.value.trim().toUpperCase(),u=(I=document.getElementById("m-country"))==null?void 0:I.value.trim().toUpperCase(),l=((x=document.getElementById("m-logo"))==null?void 0:x.value.trim())||null,o=((b=document.getElementById("m-gen-stadium"))==null?void 0:b.checked)??!1,s=((v=document.getElementById("m-gen-squad"))==null?void 0:v.checked)??!1,g=Xe();if(!m){c.textContent="Le nom du club est requis.";return}if(!a){c.textContent="Le nom encodé est requis.";return}if(!u){c.textContent="Le pays est requis.";return}r.disabled=!0,r.textContent="Enregistrement…";const E={real_name:m,encoded_name:a,country_code:u,logo_url:l,kit_style:g.style,kit_color1:g.color1,kit_color2:g.color2,kit_color3:g.color3,kit_shorts:g.shorts,kit_socks:g.socks};let h=e==null?void 0:e.id;if(t){const{error:y}=await _.from("clubs").update(E).eq("id",h);if(y){c.textContent=y.message,r.disabled=!1,r.textContent="💾 Enregistrer";return}}else{const{data:y,error:$}=await _.from("clubs").insert(E).select().single();if($){c.textContent=$.message,r.disabled=!1,r.textContent="✅ Créer le club";return}if(h=y.id,o){r.textContent="🏟️ Création du stade…";const{data:A,error:k}=await _.from("stadium_definitions").insert({name:`Stade de ${m}`,club_id:h,country_code:u}).select().single();k?console.warn("[Stadium] Erreur def stade:",k.message):A&&await _.from("cards").insert({card_type:"stadium",stadium_id:A.id})}s&&(r.textContent="⚽ Génération des joueurs…",await Je(h,u,p))}p(t?"Club modifié ✅":"Club créé ✅","success"),d(),fe(n,i)}const Dt=["normal","pepite","papyte","legende"],Ce=["GK","DEF","MIL","ATT"],Pt=[["MA","Maroc"],["FR","France"],["AR","Argentine"],["PT","Portugal"],["BR","Brésil"],["ES","Espagne"],["DE","Allemagne"],["GB","Angleterre"],["IT","Italie"],["CM","Cameroun"],["SN","Sénégal"],["NG","Nigéria"],["DK","Danemark"],["NL","Pays-Bas"],["BE","Belgique"],["CI","Côte d'Ivoire"],["AL","Albanie"],["HR","Croatie"],["RS","Serbie"],["TR","Turquie"],["MW","Malawi"]];let ee={...O},ue=[];async function Ot(e,{toast:t}){const{data:n}=await _.from("clubs").select("id,encoded_name,logo_url,kit_style,kit_color1,kit_color2,kit_shorts,kit_socks").order("encoded_name");ue=n||[],ee={...O},e.innerHTML=Ft(ue),qt(e,ue,t),W()}function Se(){var n;const e=((n=document.getElementById("cb-surname-real"))==null?void 0:n.value)||"",t=document.getElementById("cb-surname-enc");t&&(t.value=xe(e))}function Nt(){var n;const e=(n=document.getElementById("cb-club"))==null?void 0:n.value,t=ue.find(i=>String(i.id)===String(e));return t?{style:t.kit_style||D.style,color1:t.kit_color1||D.color1,color2:t.kit_color2||D.color2,shorts:t.kit_shorts||D.shorts,socks:t.kit_socks||D.socks}:{...D}}function W(){var o,s,g,E;const e=h=>{var f;return((f=document.getElementById(h))==null?void 0:f.value)||""},t={firstname:e("cb-firstname"),surname_real:e("cb-surname-enc")||xe(e("cb-surname-real")),country_code:e("cb-country"),club_encoded_name:((s=(o=document.getElementById("cb-club"))==null?void 0:o.selectedOptions[0])==null?void 0:s.text)||"",job:e("cb-job"),job2:e("cb-job2")||null,note_g:parseInt(e("cb-note-g"))||0,note_d:parseInt(e("cb-note-d"))||0,note_m:parseInt(e("cb-note-m"))||0,note_a:parseInt(e("cb-note-a"))||0,rarity:e("cb-rarity")},n=Nt(),i=document.getElementById("cb-club"),p=((E=(g=i==null?void 0:i.selectedOptions[0])==null?void 0:g.dataset)==null?void 0:E.logo)||null,d=Ue(ee,n,120),c=`data:image/svg+xml;charset=utf-8,${encodeURIComponent(d)}`,r=dt(t,{portraitUrl:c,clubLogoUrl:p,showNotes:!0}),m=document.getElementById("card-preview");m&&(m.innerHTML=r);const a=document.getElementById("avatar-preview-wrap");a&&(a.innerHTML=d);const u=e("cb-surname-real"),l=document.getElementById("encode-summary");l&&u&&(l.textContent=`${e("cb-firstname")} ${u} → ${e("cb-firstname")} ${t.surname_real}`)}function Qe(){const e=t=>{var n;return((n=document.getElementById(t))==null?void 0:n.value)||""};ee={skin:e("av-skin"),hair:e("av-hair"),hairColor:e("av-hairColor"),eyes:e("av-eyes"),nose:e("av-nose"),mouth:e("av-mouth"),eyebrows:e("av-eyebrows"),beard:e("av-beard")}}function qt(e,t,n){var p,d,c,r,m;e.querySelectorAll("input,select").forEach(a=>{a.addEventListener("input",W),a.addEventListener("change",W)});const i=["av-skin","av-hair","av-hairColor","av-eyes","av-nose","av-mouth","av-eyebrows","av-beard"];i.forEach(a=>{var u;(u=document.getElementById(a))==null||u.addEventListener("change",()=>{Qe(),W()})}),(p=document.getElementById("cb-surname-real"))==null||p.addEventListener("input",()=>{Se(),W()}),(d=document.getElementById("btn-encode"))==null||d.addEventListener("click",()=>{Se(),W()}),(c=document.getElementById("cb-club"))==null||c.addEventListener("change",W),(r=document.getElementById("btn-save-player"))==null||r.addEventListener("click",()=>Gt(n)),(m=document.getElementById("btn-reset"))==null||m.addEventListener("click",()=>{e.querySelectorAll("input").forEach(a=>a.value=""),e.querySelectorAll("select").forEach(a=>a.selectedIndex=0),ee={...O},i.forEach(a=>{const u=a.replace("av-",""),l=document.getElementById(a);l&&(l.value=O[u]||"")}),W()})}async function Gt(e){const t=c=>{var r;return((r=document.getElementById(c))==null?void 0:r.value)||""},n=t("cb-surname-real").trim(),i=t("cb-surname-enc").trim()||xe(n);if(!t("cb-firstname")||!n||!t("cb-country")||!t("cb-job")){e("Remplissez les champs obligatoires (prénom, nom, pays, poste)","error");return}Qe();const p={firstname:t("cb-firstname").trim(),surname_real:n,surname_real:i,country_code:t("cb-country"),club_id:t("cb-club")||null,job:t("cb-job"),job2:t("cb-job2")||null,note_g:parseInt(t("cb-note-g"))||0,note_d:parseInt(t("cb-note-d"))||0,note_m:parseInt(t("cb-note-m"))||0,note_a:parseInt(t("cb-note-a"))||0,rarity:t("cb-rarity")||"normal",note_min:parseInt(t("cb-note-min"))||null,note_max:parseInt(t("cb-note-max"))||null,sell_price:parseInt(t("cb-price"))||0,avatar_config:ee,skin:ee.skin||"blanc",hair:ee.hairColor||"marron",hair_length:"court"},{error:d}=await _.from("players").insert(p);if(d){e("Erreur: "+d.message,"error");return}e(`✅ Joueur "${p.firstname} ${p.surname_real}" enregistré !`,"success")}function Ft(e){const t=Pt.map(([a,u])=>`<option value="${a}">${u} (${a})</option>`).join(""),n='<option value="">— Sélectionner un club —</option>'+e.map(a=>`<option value="${a.id}" data-logo="${a.logo_url||""}">${a.encoded_name}</option>`).join(""),i=Ce.map(a=>`<option value="${a}">${a}</option>`).join(""),p='<option value="">Aucun</option>'+Ce.map(a=>`<option value="${a}">${a}</option>`).join(""),d=Dt.map(a=>`<option value="${a}">${a.charAt(0).toUpperCase()+a.slice(1)}</option>`).join(""),c=(a,u,l)=>Object.entries(u).map(([o,s])=>`<option value="${o}" ${o===(l||"")?"selected":""}>${s.label}</option>`).join(""),r=Object.keys(at).map(a=>`<option value="${a}" ${a===O.skin?"selected":""}>${a.charAt(0).toUpperCase()+a.slice(1)}</option>`).join(""),m=Object.keys(lt).map(a=>`<option value="${a}" ${a===O.hairColor?"selected":""}>${a.charAt(0).toUpperCase()+a.slice(1)}</option>`).join("");return`
        ...
        <div style="display:grid;grid-template-columns:1fr 120px;gap:16px;align-items:start">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
            ...
            <div>
              ...
              <select id="av-hair">${c("av-hair",ct,O.hair)}</select>
            </div>
            <div>
            ...
            <div>
              ...
              <select id="av-eyebrows">${c("av-eyebrows",pt,O.eyebrows)}</select>
            </div>
            <div>
              <label>Yeux</label>
              <select id="av-eyes">${c("av-eyes",ut,O.eyes)}</select>
            </div>
            <div>
              <label>Nez</label>
              <select id="av-nose">${c("av-nose",mt,O.nose)}</select>
            </div>
            <div>
              <label>Bouche</label>
              <select id="av-mouth">${c("av-mouth",gt,O.mouth)}</select>
            </div>
            <div>
              <label>Barbe</label>
              <select id="av-beard">${c("av-beard",ft,O.beard)}</select>
            </div>
          </div>
  ...
  `,d(p),document.getElementById("search-users").addEventListener("input",c=>{const r=c.target.value.toLowerCase();d(p.filter(m=>{var a;return m.pseudo.toLowerCase().includes(r)||((a=m.club_name)==null?void 0:a.toLowerCase().includes(r))}))});function d(c){document.getElementById("users-tbody").innerHTML=c.map(r=>{const m=r.mmr??1e3,a=r.mmr_deviation??350,u=r.placement_matches??0,l=bt(m),o=r.ranked_wins??0,s=r.ranked_losses??0,g=r.ranked_draws??0,E=o+s+g,h=E>0?Math.round(o/E*100):null,f=u<10,w=a<80?"#1A6B3C":a<150?"#e67e22":"#bb2020";return`
        ...
        </tr>`}).join(""),document.querySelectorAll("[data-toggle-admin]").forEach(r=>{r.addEventListener("click",async()=>{const m=r.dataset.isAdmin!=="true";if(!confirm(`Voulez-vous ${m?"rendre admin":"retirer le rôle admin"} de ce Manager ?`))return;const{error:a}=await _.from("users").update({is_admin:m}).eq("id",r.dataset.toggleAdmin);a?t(a.message,"error"):(t("Rôle mis à jour ✅","success"),et(e,{toast:t}))})})}}async function tt(e,{toast:t}){const{data:n,error:i}=await _.from("market_listings").select(`
  ...
  `,document.querySelectorAll("[data-cancel]").forEach(r=>{r.addEventListener("click",async()=>{if(!confirm("Annuler cette annonce ?"))return;const{error:m}=await _.from("market_listings").update({status:"cancelled"}).eq("id",r.dataset.cancel);m?t(m.message,"error"):(t("Annonce annulée","success"),tt(e,{toast:t}))})})}async function Ht(e,{toast:t}){e.innerHTML=`
...
`).filter(c=>c.trim());if(t.length<2)return[];const n=t[0],i=n.split(";").length>n.split(",").length?";":",",p=Me(n,i).map(c=>c.trim().replace(/^\uFEFF/,"").toLowerCase()),d=[];for(let c=1;c<t.length;c++){if(!t[c].trim())continue;const r=Me(t[c],i),m={};p.forEach((a,u)=>{m[a]=(r[u]||"").trim()}),!Object.values(m).every(a=>!a)&&d.push(m)}return d}function Me(e,t=","){const n=[];let i="",p=!1;for(let d=0;d<e.length;d++){const c=e[d];p?c==='"'?e[d+1]==='"'?(i+='"',d++):p=!1:i+=c:c==='"'?p=!0:c===t?(n.push(i),i=""):i+=c}return n.push(i),n}const Ut={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};async function Vt(e,{toast:t}){const n=Object.keys(ve);let i=n[0];const{data:p}=await _.from("formation_links_overrides").select("formation, links"),d={};(p||[]).forEach(u=>{d[u.formation]=u.links});let c=new Set;function r(u,l){return[u,l].sort().join("-")}function m(u){const l=d[u]||ve[u]||[];c=new Set(l.map(([o,s])=>r(o,s)))}m(i),e.innerHTML=`
  ...
  `;function a(){const u=ie[i]||{},l=ke(i),o=320,s=400,g=22;function E(x){const b=u[x];return b?{x:b.x*o,y:b.y*s}:null}let h=`<svg width="${o}" height="${s}" viewBox="0 0 ${o} ${s}" xmlns="http://www.w3.org/2000/svg" style="display:block">`;l.forEach(([x,b],v)=>{const y=E(x),$=E(b);if(!y||!$)return;const A=r(x,b),k=c.has(A),B=k?"#3b82f6":"#999",S=k?.95:.35,z=k?4:3;h+=`<line x1="${y.x}" y1="${y.y}" x2="${$.x}" y2="${$.y}"
        ...
        </button>`}).join("");const w=e.querySelector('div[style*="font-weight:700;font-size:13px;margin-bottom:8px"]');w&&(w.textContent=`Liens (${c.size}/${l.length} actifs)`);function I(x){c.has(x)?c.delete(x):c.add(x),a()}e.querySelectorAll(".link-hit").forEach(x=>{x.addEventListener("click",()=>I(x.dataset.key))}),e.querySelectorAll(".link-toggle").forEach(x=>{x.addEventListener("click",()=>I(x.dataset.key))})}a(),document.getElementById("formation-select").addEventListener("change",u=>{i=u.target.value,m(i),a()}),document.getElementById("reset-btn").addEventListener("click",()=>{const u=ve[i]||[];c=new Set(u.map(([l,o])=>r(l,o))),a(),t("Liens réinitialisés aux valeurs par défaut (non sauvegardé)","info")}),document.getElementById("save-btn").addEventListener("click",async()=>{const l=ke(i).filter(([s,g])=>c.has(r(s,g))),{error:o}=await _.from("formation_links_overrides").upsert({formation:i,links:l,updated_at:new Date().toISOString()});if(o){t(o.message,"error");return}d[i]=l,t(`Liens enregistrés pour ${i} (${l.length} actifs)`,"success")})}const Kt=[{value:"player",label:"Joueur"},{value:"formation",label:"Formation"},{value:"game_changer",label:"Game Changer"},{value:"ultra_game_changer",label:"Ultra Game Changer"},{value:"game_helper",label:"Game Helper"},{value:"stadium",label:"🏟️ Stade"}],Wt=[{value:"",label:"Toutes"},{value:"normal",label:"Normal"},{value:"pepite",label:"Pépite"},{value:"papyte",label:"Papyte"},{value:"legende",label:"Légende"}],Yt=[{value:"credits",label:"Crédits"},{value:"pub",label:"Publicité"},{value:"free",label:"Gratuit"}];async function Jt(e){e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">Chargement...</div>';const{data:t}=await _.from("booster_configs").select("*").order("sort_order");let n=null,i=[],p=null;const d=()=>window.innerWidth<700;async function c(){if(!n){i=[];return}if(n!==p){const{data:l}=await _.from("booster_drop_rates").select("*").eq("booster_id",n).order("sort_order");i=l||[],p=n}}function r(){return`
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
          <div>
            ...
            <select id="f-price-type" style="width:100%;padding:9px;border:1px solid #ddd;border-radius:6px;font-size:13px">
              ${Yt.map(g=>`<option value="${g.value}" ${l.price_type===g.value?"selected":""}>${g.label}</option>`).join("")}
            </select>
          </div>
        ...
        ${i.length===0?'<div style="text-align:center;padding:16px;color:#aaa;font-size:13px">Aucun taux. Cliquez + Ligne.</div>':i.map((g,E)=>`
          <div style="display:grid;grid-template-columns:2fr 1.5fr 50px 50px 60px 32px;gap:4px;align-items:center;margin-bottom:6px;padding:6px;background:#f9f9f9;border-radius:6px" data-rate-idx="${E}">
            <select class="rate-type" data-idx="${E}" style="padding:6px 4px;border:1px solid #ddd;border-radius:4px;font-size:12px;width:100%">
              ${Kt.map(h=>`<option value="${h.value}" ${g.card_type===h.value?"selected":""}>${h.label}</option>`).join("")}
            </select>
            <select class="rate-rarity" data-idx="${E}" style="padding:6px 4px;border:1px solid #ddd;border-radius:4px;font-size:12px;width:100%">
              ${Wt.map(h=>`<option value="${h.value}" ${(g.rarity||"")===h.value?"selected":""}>${h.label}</option>`).join("")}
            </select>
    ...
    </div>`,document.getElementById("add-config").addEventListener("click",()=>je(null)),document.getElementById("form-cancel").addEventListener("click",()=>{document.getElementById("config-form").style.display="none"}),document.getElementById("form-save").addEventListener("click",()=>Qt(e)),e.querySelectorAll("[data-edit]").forEach(i=>{const p=(t||[]).find(d=>d.id===i.dataset.edit);p&&i.addEventListener("click",()=>je(p))}),e.querySelectorAll("[data-delete]").forEach(i=>{i.addEventListener("click",async()=>{confirm("Supprimer cette règle ?")&&(await _.from("sell_price_configs").delete().eq("id",i.dataset.delete),await he(e))})})}function Xt(e){const t=nt.find(n=>n.value===e.rarity);return`
    ...
    </tr>`}function je(e){document.getElementById("config-form").style.display="block",document.getElementById("form-title").textContent=e?"Modifier la règle":"Nouvelle règle",document.getElementById("form-id").value=(e==null?void 0:e.id)||"",document.getElementById("form-rarity").value=(e==null?void 0:e.rarity)||"normal",document.getElementById("form-price").value=(e==null?void 0:e.price)||1e3,document.getElementById("form-note-min").value=(e==null?void 0:e.note_min)??1,document.getElementById("form-note-max").value=(e==null?void 0:e.note_max)??10,document.getElementById("config-form").scrollIntoView({behavior:"smooth"})}async function Qt(e){const t=document.getElementById("form-id").value,n=document.getElementById("form-rarity").value,i=parseInt(document.getElementById("form-price").value)||0,p=parseInt(document.getElementById("form-note-min").value)||1,d=parseInt(document.getElementById("form-note-max").value)||10;if(p>d){alert("Note min doit être ≤ note max");return}const c={rarity:n,price:i,note_min:p,note_max:d,updated_at:new Date().toISOString()};let r;if(t?{error:r}=await _.from("sell_price_configs").update(c).eq("id",t):{error:r}=await _.from("sell_price_configs").insert(c),r){alert("Erreur : "+r.message);return}document.getElementById("config-form").style.display="none",await he(e)}async function en(e){await Ee(e)}async function Ee(e){const{data:t}=await _.from("patch_notes").select("*").order("published_at",{ascending:!1});e.innerHTML=`
    ...
    </div>`,document.getElementById("add-article").addEventListener("click",()=>Re(null)),document.getElementById("form-cancel").addEventListener("click",()=>{document.getElementById("article-form").style.display="none"}),document.getElementById("form-save").addEventListener("click",()=>nn(e)),e.querySelectorAll("[data-edit]").forEach(n=>{const i=(t||[]).find(p=>p.id===n.dataset.edit);i&&n.addEventListener("click",()=>Re(i))}),e.querySelectorAll("[data-delete]").forEach(n=>{n.addEventListener("click",async()=>{confirm("Supprimer cet article ?")&&(await _.from("patch_notes").delete().eq("id",n.dataset.delete),await Ee(e))})})}function tn(e){const t=new Date(e.published_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"short",year:"numeric"});return`
  ...
  <div style="background:#fff;border-radius:10px;padding:12px 16px;box-shadow:0 1px 4px rgba(0,0,0,0.08);display:flex;align-items:center;gap:12px">
    ...
    </div>`,m.querySelectorAll("ul,ol").forEach(l=>{l.style.paddingLeft="20px",l.style.margin="6px 0"}),m.querySelectorAll("li").forEach(l=>{l.style.marginBottom="4px"}),m.querySelectorAll("p").forEach(l=>{l.style.marginBottom="8px"})}async function ln(e){var l,o;const t=document.getElementById("ts-id").value,n=document.getElementById("ts-emoji").value.trim()||"⚽",i=document.getElementById("ts-title").value.trim(),p=document.getElementById("ts-editor"),d=(p?p.innerHTML:document.getElementById("ts-content").value).trim(),c=document.getElementById("ts-color-hex").value.trim()||"#1A6B3C",r=parseInt(document.getElementById("ts-order").value)||0,m=document.getElementById("ts-active").checked;if(!i||!d){alert("Titre et contenu sont obligatoires.");return}const a={emoji:n,title:i,content:d,color:c,step_order:r,is_active:m,image_url:((o=(l=document.getElementById("ts-image"))==null?void 0:l.value)==null?void 0:o.trim())||null};let u;if(t?{error:u}=await _.from("tutorial_steps").update(a).eq("id",t):{error:u}=await _.from("tutorial_steps").insert(a),u){alert("Erreur : "+u.message);return}it(),oe(e)}const sn="/",ot="#E87722",dn=[["MA","Maroc"],["FR","France"],["AR","Argentine"],["PT","Portugal"],["BR","Brésil"],["ES","Espagne"],["DE","Allemagne"],["GB","Angleterre"],["IT","Italie"],["CM","Cameroun"],["SN","Sénégal"],["NG","Nigéria"],["DK","Danemark"],["NL","Pays-Bas"],["BE","Belgique"],["CI","Côte d'Ivoire"],["AL","Albanie"],["HR","Croatie"],["RS","Serbie"],["TR","Turquie"],["MW","Malawi"],["GH","Ghana"],["ZA","Afrique du Sud"],["EG","Égypte"],["US","États-Unis"],["MX","Mexique"],["CO","Colombie"],["UY","Uruguay"],["CL","Chili"],["JP","Japon"],["KR","Corée du Sud"],["CN","Chine"],["AU","Australie"],["SW","Suisse"],["SE","Suède"],["NO","Norvège"],["PL","Pologne"]];async function cn(e){await _e(e)}async function _e(e){var i,p,d,c;const[{data:t},{data:n}]=await Promise.all([_.from("stadium_definitions").select("*, club:clubs(encoded_name, logo_url)").order("name"),_.from("clubs").select("id, encoded_name, logo_url").order("encoded_name")]);e.innerHTML=`
    <div style="padding:20px;max-width:820px;margin:0 auto">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px">
          ...
          <div style="flex:1;min-width:220px;display:flex;flex-direction:column;gap:12px">
            ...
            <div>
              ...
              <select id="st-country" style="width:100%;box-sizing:border-box;padding:9px;border:1.5px solid #ddd;border-radius:8px;font-size:13px;background:#fff">
                ...
                ${dn.map(([r,m])=>`<option value="${r}">${m} (${r})</option>`).join("")}
              </select>
            </div>
            <div>
    ...
    </div>`,(i=document.getElementById("st-add-btn"))==null||i.addEventListener("click",()=>Pe(null)),(p=document.getElementById("st-cancel"))==null||p.addEventListener("click",()=>{document.getElementById("st-form").style.display="none"}),(d=document.getElementById("st-save"))==null||d.addEventListener("click",()=>mn(e)),(c=document.getElementById("st-preview-btn"))==null||c.addEventListener("click",()=>me()),["st-name","st-club","st-country","st-image"].forEach(r=>{var m,a;(m=document.getElementById(r))==null||m.addEventListener("input",me),(a=document.getElementById(r))==null||a.addEventListener("change",me)}),e.querySelectorAll("[data-edit]").forEach(r=>{const m=(t||[]).find(a=>a.id===r.dataset.edit);m&&r.addEventListener("click",()=>Pe(m))}),e.querySelectorAll("[data-delete]").forEach(r=>{r.addEventListener("click",async()=>{confirm("Supprimer ce stade ?")&&(await _.from("stadium_definitions").delete().eq("id",r.dataset.delete),_e(e))})})}function pn(e){var n,i;const t=(n=e.club)!=null&&n.logo_url?`<img src="${e.club.logo_url}" style="width:32px;height:32px;object-fit:contain">`:'<span style="font-size:20px">🌍</span>';return`
    <div style="background:#fff;border-radius:10px;padding:12px 16px;box-shadow:0 1px 4px rgba(0,0,0,0.08);display:flex;align-items:center;gap:12px">
      <div style="width:40px;height:40px;border-radius:8px;background:${ot};display:flex;align-items:center;justify-content:center;flex-shrink:0">
        ${t}
      </div>
      <div style="flex:1">
