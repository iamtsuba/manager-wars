import{s as y,e as J,g as ze,K as Ae,D as A,S as qe,a as z,H as Oe,R as De,b as Se,r as Fe,c as He,E as Ue,d as Ge,N as Ve,M as Ke,B as Ye,F as oe,f as ge,h as Je,i as We}from"./formation-links-Dnttc-Cp.js";async function Xe(e){const[{count:i},{count:t},{count:n},{count:c},{count:l}]=await Promise.all([y.from("players").select("*",{count:"exact",head:!0}),y.from("clubs").select("*",{count:"exact",head:!0}),y.from("users").select("*",{count:"exact",head:!0}),y.from("cards").select("*",{count:"exact",head:!0}),y.from("matches").select("*",{count:"exact",head:!0})]);e.innerHTML=`
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:12px;margin-bottom:24px">
      ${Y("🃏","Joueurs (cartes)",i??"–","#1A6B3C")}
      ${Y("🏟️","Clubs",t??"–","#D4A017")}
      ${Y("👥","Managers",n??"–","#7a28b8")}
      ${Y("📦","Cartes possédées",c??"–","#2a8f52")}
  ...
  </div>`}const be={normal:"Normal",pepite:"Pépite",papyte:"Papyte",legende:"Légende"},Qe={normal:"#aaa",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"};async function Ze(e,i){await X(e,i)}async function X(e,i){const{toast:t,openModal:n,closeModal:c}=i,[{data:l,error:o},{data:r}]=await Promise.all([y.from("players").select("*, clubs(encoded_name,logo_url)").order("surname_encoded"),y.from("clubs").select("id,encoded_name").order("encoded_name")]);if(o){e.innerHTML=`<p style="color:red">${o.message}</p>`;return}const d={};(r||[]).forEach(s=>{d[s.id]=s.encoded_name}),e.innerHTML=`
  ...
  `,p(l);function p(s){document.getElementById("count-label").textContent=`${s.length} joueur(s)`,document.getElementById("players-tbody").innerHTML=s.map(a=>{var f;return`
      <tr>
        <td>
        ...
        <td>
          <span style="background:${Qe[a.rarity]};color:#fff;padding:2px 6px;border-radius:3px;font-size:10px;font-weight:700">
  ...
  `,c(Q),document.getElementById("search-clubs").addEventListener("input",l=>{const o=l.target.value.toLowerCase();c(Q.filter(r=>r.encoded_name.toLowerCase().includes(o)||r.real_name.toLowerCase().includes(o)))}),document.getElementById("add-club-btn").addEventListener("click",()=>{xe(null,{toast:i,openModal:t,closeModal:n,reload:()=>Z(e,{toast:i,openModal:t,closeModal:n})})});function c(l){document.getElementById("clubs-tbody").innerHTML=l.map(o=>{var p;const r=Ce(o),d=ze(r).replace("<svg ",'<svg style="width:36px;height:42px" ');return`
      <tr>
        <td>
        ...
        <td title="${((p=Ae[o.kit_style||"uni"])==null?void 0:p.label)||"Uni"}">${d}</td>
     ...
     <button class="btn btn-primary" id="m-save">${l?"Enregistrer":"Créer"}</button>`);function d(a,f){const g=document.getElementById(a),E=document.getElementById(f);!g||!E||(g.addEventListener("input",()=>{E.value=g.value,m()}),E.addEventListener("input",()=>{/^#[0-9a-fA-F]{6}$/.test(E.value)&&(g.value=E.value,m())}))}d("m-kit-color1","m-kit-color1-txt"),d("m-kit-color2","m-kit-color2-txt"),d("m-kit-shorts","m-kit-shorts-txt"),d("m-kit-socks","m-kit-socks-txt"),(s=document.getElementById("m-kit-style"))==null||s.addEventListener("change",m);function p(){var a,f,g,E,_;return{style:((a=document.getElementById("m-kit-style"))==null?void 0:a.value)||"uni",color1:((f=document.getElementById("m-kit-color1"))==null?void 0:f.value)||"#1A6B3C",color2:((g=document.getElementById("m-kit-color2"))==null?void 0:g.value)||"#ffffff",shorts:((E=document.getElementById("m-kit-shorts"))==null?void 0:E.value)||"#111111",socks:((_=document.getElementById("m-kit-socks"))==null?void 0:_.value)||"#ffffff"}}function m(){const a=document.getElementById("kit-preview-wrap");a&&(a.innerHTML=ze(p()))}m(),document.getElementById("m-cancel").addEventListener("click",n),document.getElementById("auto-encode").addEventListener("click",()=>{const a=document.getElementById("m-real").value,f=a.split(" ")[0]||a;document.getElementById("m-encoded").value=f.toUpperCase()+" FC"}),document.getElementById("m-real").addEventListener("input",()=>{const a=document.getElementById("m-real").value;if(!document.getElementById("m-encoded").value){const f=a.split(" ")[0]||a;document.getElementById("m-encoded").value=f.toUpperCase()+" FC"}}),document.getElementById("m-save").addEventListener("click",async()=>{const a=p(),f={real_name:document.getElementById("m-real").value.trim(),encoded_name:document.getElementById("m-encoded").value.trim().toUpperCase(),country_code:document.getElementById("m-country").value.trim().toUpperCase(),logo_url:document.getElementById("m-logo").value.trim()||null,kit_style:a.style,kit_color1:a.color1,kit_color2:a.color2,kit_shorts:a.shorts,kit_socks:a.socks};if(!f.real_name||!f.encoded_name||!f.country_code){i("Remplissez tous les champs obligatoires","error");return}const{error:g}=l?await y.from("clubs").update(f).eq("id",e.id):await y.from("clubs").insert(f);if(g){i(g.message,"error");return}i(l?"Club modifié ✅":"Club créé ✅","success"),n(),c()})}const it=["normal","pepite","papyte","legende"],ve=["GK","DEF","MIL","ATT"],nt=[["MA","Maroc"],["FR","France"],["AR","Argentine"],["PT","Portugal"],["BR","Brésil"],["ES","Espagne"],["DE","Allemagne"],["GB","Angleterre"],["IT","Italie"],["CM","Cameroun"],["SN","Sénégal"],["NG","Nigéria"],["DK","Danemark"],["NL","Pays-Bas"],["BE","Belgique"],["CI","Côte d'Ivoire"],["AL","Albanie"],["HR","Croatie"],["RS","Serbie"],["TR","Turquie"],["MW","Malawi"]];let P={...z},ee=[];async function ot(e,{toast:i}){const{data:t}=await y.from("clubs").select("id,encoded_name,logo_url,kit_style,kit_color1,kit_color2,kit_shorts,kit_socks").order("encoded_name");ee=t||[],P={...z},e.innerHTML=dt(ee),rt(e,ee,i),T()}function ye(){var t;const e=((t=document.getElementById("cb-surname-real"))==null?void 0:t.value)||"",i=document.getElementById("cb-surname-enc");i&&(i.value=J(e))}function at(){var t;const e=(t=document.getElementById("cb-club"))==null?void 0:t.value,i=ee.find(n=>String(n.id)===String(e));return i?{style:i.kit_style||A.style,color1:i.kit_color1||A.color1,color2:i.kit_color2||A.color2,shorts:i.kit_shorts||A.shorts,socks:i.kit_socks||A.socks}:{...A}}function T(){var a,f,g,E;const e=_=>{var u;return((u=document.getElementById(_))==null?void 0:u.value)||""},i={firstname:e("cb-firstname"),surname_encoded:e("cb-surname-enc")||J(e("cb-surname-real")),country_code:e("cb-country"),club_encoded_name:((f=(a=document.getElementById("cb-club"))==null?void 0:a.selectedOptions[0])==null?void 0:f.text)||"",job:e("cb-job"),job2:e("cb-job2")||null,note_g:parseInt(e("cb-note-g"))||0,note_d:parseInt(e("cb-note-d"))||0,note_m:parseInt(e("cb-note-m"))||0,note_a:parseInt(e("cb-note-a"))||0,rarity:e("cb-rarity")},t=at(),n=document.getElementById("cb-club"),c=((E=(g=n==null?void 0:n.selectedOptions[0])==null?void 0:g.dataset)==null?void 0:E.logo)||null,l=Se(P,t,120),o=`data:image/svg+xml;charset=utf-8,${encodeURIComponent(l)}`,r=Fe(i,{portraitUrl:o,clubLogoUrl:c,showNotes:!0}),d=document.getElementById("card-preview");d&&(d.innerHTML=r);const p=document.getElementById("avatar-preview-wrap");p&&(p.innerHTML=l);const m=e("cb-surname-real"),s=document.getElementById("encode-summary");s&&m&&(s.textContent=`${e("cb-firstname")} ${m} → ${e("cb-firstname")} ${i.surname_encoded}`)}function Te(){const e=i=>{var t;return((t=document.getElementById(i))==null?void 0:t.value)||""};P={skin:e("av-skin"),hair:e("av-hair"),hairColor:e("av-hairColor"),eyes:e("av-eyes"),nose:e("av-nose"),mouth:e("av-mouth"),eyebrows:e("av-eyebrows"),beard:e("av-beard")}}function rt(e,i,t){var c,l,o,r,d;e.querySelectorAll("input,select").forEach(p=>{p.addEventListener("input",T),p.addEventListener("change",T)});const n=["av-skin","av-hair","av-hairColor","av-eyes","av-nose","av-mouth","av-eyebrows","av-beard"];n.forEach(p=>{var m;(m=document.getElementById(p))==null||m.addEventListener("change",()=>{Te(),T()})}),(c=document.getElementById("cb-surname-real"))==null||c.addEventListener("input",()=>{ye(),T()}),(l=document.getElementById("btn-encode"))==null||l.addEventListener("click",()=>{ye(),T()}),(o=document.getElementById("cb-club"))==null||o.addEventListener("change",T),(r=document.getElementById("btn-save-player"))==null||r.addEventListener("click",()=>lt(t)),(d=document.getElementById("btn-reset"))==null||d.addEventListener("click",()=>{e.querySelectorAll("input").forEach(p=>p.value=""),e.querySelectorAll("select").forEach(p=>p.selectedIndex=0),P={...z},n.forEach(p=>{const m=p.replace("av-",""),s=document.getElementById(p);s&&(s.value=z[m]||"")}),T()})}async function lt(e){const i=o=>{var r;return((r=document.getElementById(o))==null?void 0:r.value)||""},t=i("cb-surname-real").trim(),n=i("cb-surname-enc").trim()||J(t);if(!i("cb-firstname")||!t||!i("cb-country")||!i("cb-job")){e("Remplissez les champs obligatoires (prénom, nom, pays, poste)","error");return}Te();const c={firstname:i("cb-firstname").trim(),surname_real:t,surname_encoded:n,country_code:i("cb-country"),club_id:i("cb-club")||null,job:i("cb-job"),job2:i("cb-job2")||null,note_g:parseInt(i("cb-note-g"))||0,note_d:parseInt(i("cb-note-d"))||0,note_m:parseInt(i("cb-note-m"))||0,note_a:parseInt(i("cb-note-a"))||0,rarity:i("cb-rarity")||"normal",note_min:parseInt(i("cb-note-min"))||null,note_max:parseInt(i("cb-note-max"))||null,sell_price:parseInt(i("cb-price"))||0,avatar_config:P,skin:P.skin||"blanc",hair:P.hairColor||"marron",hair_length:"court"},{error:l}=await y.from("players").insert(c);if(l){e("Erreur: "+l.message,"error");return}e(`✅ Joueur "${c.firstname} ${c.surname_encoded}" enregistré !`,"success")}function dt(e){const i=nt.map(([p,m])=>`<option value="${p}">${m} (${p})</option>`).join(""),t='<option value="">— Sélectionner un club —</option>'+e.map(p=>`<option value="${p.id}" data-logo="${p.logo_url||""}">${p.encoded_name}</option>`).join(""),n=ve.map(p=>`<option value="${p}">${p}</option>`).join(""),c='<option value="">Aucun</option>'+ve.map(p=>`<option value="${p}">${p}</option>`).join(""),l=it.map(p=>`<option value="${p}">${p.charAt(0).toUpperCase()+p.slice(1)}</option>`).join(""),o=(p,m,s)=>Object.entries(m).map(([a,f])=>`<option value="${a}" ${a===(s||"")?"selected":""}>${f.label}</option>`).join(""),r=Object.keys(qe).map(p=>`<option value="${p}" ${p===z.skin?"selected":""}>${p.charAt(0).toUpperCase()+p.slice(1)}</option>`).join(""),d=Object.keys(Oe).map(p=>`<option value="${p}" ${p===z.hairColor?"selected":""}>${p.charAt(0).toUpperCase()+p.slice(1)}</option>`).join("");return`
    ...
    <div style="display:flex;flex-direction:column;gap:14px">
      ...
      <div class="card-panel">
        ...
        <div style="margin-top:8px;display:flex;gap:8px">
          ${["normal","pepite","papyte","legende"].map(p=>`
            <div style="background:${De[p]};color:#fff;padding:3px 8px;border-radius:4px;font-size:11px;font-weight:700">
              ${p.toUpperCase()}
            </div>`).join("")}
        </div>
      </div>
  ...
  `}async function je(e,{toast:i}){const{data:t,error:n}=await y.from("users").select("id,pseudo,club_name,credits,level,wins,draws,losses,trophies_top1,trophies_top2,trophies_top3,is_admin,created_at").order("created_at",{ascending:!1});if(n){e.innerHTML=`<p style="color:red">${n.message}</p>`;return}e.innerHTML=`
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px">
  ...
  `;const c=t||[];l(c),document.getElementById("search-users").addEventListener("input",o=>{const r=o.target.value.toLowerCase();l(c.filter(d=>{var p;return d.pseudo.toLowerCase().includes(r)||((p=d.club_name)==null?void 0:p.toLowerCase().includes(r))}))});function l(o){document.getElementById("users-tbody").innerHTML=o.map(r=>`
      <tr>
        <td><b>${r.pseudo}</b></td>
    ...
    `).join(""),document.querySelectorAll("[data-toggle-admin]").forEach(r=>{r.addEventListener("click",async()=>{const d=r.dataset.isAdmin!=="true";if(!confirm(`Voulez-vous ${d?"rendre admin":"retirer le rôle admin"} de ce Manager ?`))return;const{error:m}=await y.from("users").update({is_admin:d}).eq("id",r.dataset.toggleAdmin);m?i(m.message,"error"):(i("Rôle mis à jour ✅","success"),je(e,{toast:i}))})})}}async function Me(e,{toast:i}){const{data:t,error:n}=await y.from("market_listings").select(`
    ...
    `).order("listed_at",{ascending:!1}).limit(100);if(n){e.innerHTML=`<p style="color:red">${n.message}</p>`;return}const c=t||[],l=c.filter(r=>r.status==="active").length,o=c.filter(r=>r.status==="sold").length;e.innerHTML=`
    ...
    <div class="card-panel">
      <div class="table-wrap">
        <table>
          ...
          <tbody>
            ${c.map(r=>{var s,a,f,g;const d=(s=r.card)==null?void 0:s.player,p=d?`${d.firstname} ${d.surname_encoded}`:((a=r.card)==null?void 0:a.card_type)||"—",m={active:"#1A6B3C",sold:"#D4A017",cancelled:"#aaa"};return`<tr>
                <td><b>${p}</b>${d?`<div style="font-size:10px;color:#999">${d.rarity} · ${d.job}</div>`:""}</td>
  ...
  `,document.querySelectorAll("[data-cancel]").forEach(r=>{r.addEventListener("click",async()=>{if(!confirm("Annuler cette annonce ?"))return;const{error:d}=await y.from("market_listings").update({status:"cancelled"}).eq("id",r.dataset.cancel);d?i(d.message,"error"):(i("Annonce annulée","success"),Me(e,{toast:i}))})})}async function st(e,{toast:i}){e.innerHTML=`
...
`;t.forEach(l=>{c+=[l.real_name,l.encoded_name,l.country_code,l.logo_url||""].map(he).join(",")+`
`}),W("clubs_export.csv",c),i(`${t.length} clubs exportés`,"success")}),document.getElementById("export-players").addEventListener("click",async()=>{const{data:t,error:n}=await y.from("players").select("firstname,surname_real,surname_encoded,country_code,job,job2,note_g,note_d,note_m,note_a,rarity,note_min,note_max,skin,hair,hair_length,sell_price,clubs(encoded_name)").order("surname_encoded");if(n){i(n.message,"error");return}if(!t||t.length===0){i("Aucun joueur à exporter","info");return}let c=`firstname,surname_real,surname_encoded,country_code,club_encoded_name,job,job2,note_g,note_d,note_m,note_a,rarity,note_min,note_max,skin,hair,hair_length,sell_price
`;t.forEach(l=>{var o;c+=[l.firstname,l.surname_real,l.surname_encoded,l.country_code,((o=l.clubs)==null?void 0:o.encoded_name)||"",l.job,l.job2||"",l.note_g,l.note_d,l.note_m,l.note_a,l.rarity,l.note_min??"",l.note_max??"",l.skin,l.hair,l.hair_length,l.sell_price].map(he).join(",")+`
`}),W("players_export.csv",c),i(`${t.length} joueurs exportés`,"success")}),document.getElementById("import-clubs").addEventListener("change",async t=>{const n=t.target.files[0];if(!n)return;const c=document.getElementById("clubs-import-result");c.innerHTML='<span style="color:var(--gray-600)">Import en cours...</span>';try{const l=await n.text(),o=Ee(l);if(o.length===0){c.innerHTML='<span style="color:#c0392b">Fichier vide</span>';return}let r=0,d=0;const p=[];for(const m of o){if(!m.real_name||!m.encoded_name||!m.country_code){d++,p.push(`Ligne ignorée (champs manquants): ${m.real_name||"?"}`);continue}const s={real_name:m.real_name,encoded_name:m.encoded_name.toUpperCase(),country_code:m.country_code.toUpperCase().slice(0,2),logo_url:m.logo_url||null},{error:a}=await y.from("clubs").upsert(s,{onConflict:"encoded_name"});a?(d++,p.push(`${m.encoded_name}: ${a.message}`)):r++}c.innerHTML=`<div style="color:var(--green)">✅ ${r} clubs importés</div>
        ${d>0?`<div style="color:#c0392b">❌ ${d} erreurs</div><div style="font-size:11px;color:var(--gray-600);max-height:100px;overflow-y:auto">${p.slice(0,10).join("<br>")}</div>`:""}`,i(`${r} clubs importés`,"success")}catch(l){c.innerHTML=`<span style="color:#c0392b">Erreur : ${l.message}</span>`}t.target.value=""}),document.getElementById("import-players").addEventListener("change",async t=>{const n=t.target.files[0];if(!n)return;const c=document.getElementById("players-import-result");c.innerHTML='<span style="color:var(--gray-600)">Import en cours...</span>';try{const l=await n.text(),o=Ee(l);if(o.length===0){c.innerHTML='<span style="color:#c0392b">Fichier vide</span>';return}const{data:r}=await y.from("clubs").select("id,encoded_name"),d={};(r||[]).forEach(u=>{d[u.encoded_name.toUpperCase()]=u.id});let p=0,m=0;const s=[],a=["GK","DEF","MIL","ATT"],f=["normal","pepite","papyte","legende"],g=["blanc","metisse","typ","noir"],E=["blond","marron","noir","chauve"],_=["rase","court","milong","long"];for(const u of o){if(!u.firstname||!u.surname_real||!u.country_code||!u.job){m++,s.push(`Ligne ignorée (champs requis manquants): ${u.firstname||"?"}`);continue}if(!a.includes(u.job)){m++,s.push(`${u.firstname}: job invalide "${u.job}"`);continue}const I=u.club_encoded_name&&d[u.club_encoded_name.toUpperCase()]||null,k={firstname:u.firstname,surname_real:u.surname_real,surname_encoded:u.surname_encoded||J(u.surname_real),country_code:u.country_code.toUpperCase().slice(0,2),club_id:I,job:u.job,job2:a.includes(u.job2)?u.job2:null,note_g:parseInt(u.note_g)||0,note_d:parseInt(u.note_d)||0,note_m:parseInt(u.note_m)||0,note_a:parseInt(u.note_a)||0,rarity:f.includes(u.rarity)?u.rarity:"normal",note_min:u.note_min!==""&&u.note_min!=null?parseInt(u.note_min):null,note_max:u.note_max!==""&&u.note_max!=null?parseInt(u.note_max):null,skin:g.includes(u.skin)?u.skin:"blanc",hair:E.includes(u.hair)?u.hair:"noir",hair_length:_.includes(u.hair_length)?u.hair_length:"court",sell_price:parseInt(u.sell_price)||0},{error:b}=await y.from("players").insert(k);b?(m++,s.push(`${u.firstname} ${u.surname_real}: ${b.message}`)):p++}c.innerHTML=`<div style="color:var(--green)">✅ ${p} joueurs importés</div>
        ${m>0?`<div style="color:#c0392b">❌ ${m} erreurs</div><div style="font-size:11px;color:var(--gray-600);max-height:100px;overflow-y:auto">${s.slice(0,10).join("<br>")}</div>`:""}`,i(`${p} joueurs importés`,"success")}catch(l){c.innerHTML=`<span style="color:#c0392b">Erreur : ${l.message}</span>`}t.target.value=""})}function he(e){const i=String(e??"");return i.includes(",")||i.includes('"')||i.includes(`
`)?'"'+i.replace(/"/g,'""')+'"':i}function W(e,i){const t=new Blob(["\uFEFF"+i],{type:"text/csv;charset=utf-8;"}),n=URL.createObjectURL(t),c=document.createElement("a");c.href=n,c.download=e,c.click(),URL.revokeObjectURL(n)}function Ee(e){e=e.replace(/^\uFEFF/,"").replace(/\r\n/g,`
`).replace(/\r/g,`
`);const i=e.split(`
`).filter(o=>o.trim());if(i.length<2)return[];const t=i[0],n=t.split(";").length>t.split(",").length?";":",",c=we(t,n).map(o=>o.trim().replace(/^\uFEFF/,"").toLowerCase()),l=[];for(let o=1;o<i.length;o++){if(!i[o].trim())continue;const r=we(i[o],n),d={};c.forEach((p,m)=>{d[p]=(r[m]||"").trim()}),!Object.values(d).every(p=>!p)&&l.push(d)}return l}function we(e,i=","){const t=[];let n="",c=!1;for(let l=0;l<e.length;l++){const o=e[l];c?o==='"'?e[l+1]==='"'?(n+='"',l++):c=!1:n+=o:o==='"'?c=!0:o===i?(t.push(n),n=""):n+=o}return t.push(n),t}const ct={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};async function pt(e,{toast:i}){const t=Object.keys(oe);let n=t[0];const{data:c}=await y.from("formation_links_overrides").select("formation, links"),l={};(c||[]).forEach(m=>{l[m.formation]=m.links});let o=new Set;function r(m,s){return[m,s].sort().join("-")}function d(m){const s=l[m]||oe[m]||[];o=new Set(s.map(([a,f])=>r(a,f)))}d(n),e.innerHTML=`
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;gap:12px;flex-wrap:wrap">
  ...
  `;function p(){const m=Je[n]||{},s=ge(n),a=320,f=400,g=22;function E(b){const h=m[b];return h?{x:h.x*a,y:h.y*f}:null}let _=`<svg width="${a}" height="${f}" viewBox="0 0 ${a} ${f}" xmlns="http://www.w3.org/2000/svg" style="display:block">`;s.forEach(([b,h],v)=>{const x=E(b),w=E(h);if(!x||!w)return;const B=r(b,h),$=o.has(B),S=$?"#3b82f6":"#999",j=$?.95:.35,M=$?4:3;_+=`<line x1="${x.x}" y1="${x.y}" x2="${w.x}" y2="${w.y}"
        ...
        opacity="${j}" pointer-events="none" data-line-key="${B}"/>`});for(const b of Object.keys(m)){const h=E(b);if(!h)continue;const v=b.replace(/\d+/,""),x=ct[v]||"#555";_+=`<circle cx="${h.x}" cy="${h.y}" r="${g}" fill="${x}" stroke="rgba(255,255,255,0.7)" stroke-width="2" pointer-events="none"/>`,_+=`<text x="${h.x}" y="${h.y+4}" text-anchor="middle" font-size="10" font-weight="900" fill="white" font-family="Arial Black,Arial" pointer-events="none">${b}</text>`}_+="</svg>",document.getElementById("field-wrap").innerHTML=_;const u=document.getElementById("links-list");u.innerHTML=s.map(([b,h])=>{const v=r(b,h),x=o.has(v);return`
        ...
        </button>`}).join("");const I=e.querySelector('div[style*="font-weight:700;font-size:13px;margin-bottom:8px"]');I&&(I.textContent=`Liens (${o.size}/${s.length} actifs)`);function k(b){o.has(b)?o.delete(b):o.add(b),p()}e.querySelectorAll(".link-hit").forEach(b=>{b.addEventListener("click",()=>k(b.dataset.key))}),e.querySelectorAll(".link-toggle").forEach(b=>{b.addEventListener("click",()=>k(b.dataset.key))})}p(),document.getElementById("formation-select").addEventListener("change",m=>{n=m.target.value,d(n),p()}),document.getElementById("reset-btn").addEventListener("click",()=>{const m=oe[n]||[];o=new Set(m.map(([s,a])=>r(s,a))),p(),i("Liens réinitialisés aux valeurs par défaut (non sauvegardé)","info")}),document.getElementById("save-btn").addEventListener("click",async()=>{const s=ge(n).filter(([f,g])=>o.has(r(f,g))),{error:a}=await y.from("formation_links_overrides").upsert({formation:n,links:s,updated_at:new Date().toISOString()});if(a){i(a.message,"error");return}l[n]=s,i(`Liens enregistrés pour ${n} (${s.length} actifs)`,"success")})}const ut=[{value:"player",label:"Joueur"},{value:"formation",label:"Formation"},{value:"game_changer",label:"Game Changer"},{value:"ultra_game_changer",label:"Ultra Game Changer"},{value:"game_helper",label:"Game Helper"},{value:"stadium",label:"🏟️ Stade"}],mt=[{value:"",label:"Toutes"},{value:"normal",label:"Normal"},{value:"pepite",label:"Pépite"},{value:"papyte",label:"Papyte"},{value:"legende",label:"Légende"}],gt=[{value:"credits",label:"Crédits"},{value:"pub",label:"Publicité"},{value:"free",label:"Gratuit"}];async function bt(e){e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">Chargement...</div>';const{data:i}=await y.from("booster_configs").select("*").order("sort_order");let t=null,n=[],c=null;const l=()=>window.innerWidth<700;async function o(){if(!t){n=[];return}if(t!==c){const{data:s}=await y.from("booster_drop_rates").select("*").eq("booster_id",t).order("sort_order");n=s||[],c=t}}function r(){return`
    ...
    </div>`}function d(){const s=t?(i||[]).find(g=>g.id===t):null;if(!s)return`<div style="display:flex;align-items:center;justify-content:center;flex:1;color:#bbb;font-size:14px;padding:40px;text-align:center">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
          <div>
            ...
            <select id="f-price-type" style="width:100%;padding:9px;border:1px solid #ddd;border-radius:6px;font-size:13px">
              ${gt.map(g=>`<option value="${g.value}" ${s.price_type===g.value?"selected":""}>${g.label}</option>`).join("")}
            </select>
          </div>
        ...
        ${n.length===0?'<div style="text-align:center;padding:16px;color:#aaa;font-size:13px">Aucun taux. Cliquez + Ligne.</div>':n.map((g,E)=>`
          <div style="display:grid;grid-template-columns:2fr 1.5fr 50px 50px 60px 32px;gap:4px;align-items:center;margin-bottom:6px;padding:6px;background:#f9f9f9;border-radius:6px" data-rate-idx="${E}">
            <select class="rate-type" data-idx="${E}" style="padding:6px 4px;border:1px solid #ddd;border-radius:4px;font-size:12px;width:100%">
              ${ut.map(_=>`<option value="${_.value}" ${g.card_type===_.value?"selected":""}>${_.label}</option>`).join("")}
            </select>
            <select class="rate-rarity" data-idx="${E}" style="padding:6px 4px;border:1px solid #ddd;border-radius:4px;font-size:12px;width:100%">
              ${mt.map(_=>`<option value="${_.value}" ${(g.rarity||"")===_.value?"selected":""}>${_.label}</option>`).join("")}
            </select>
    ...
    </div>`}async function p(s=!1){s||await o();const a=!t&&l(),f=t&&l();e.innerHTML=`
    <div style="display:flex;height:calc(100vh - 56px);overflow:hidden">
      ${!l()||a||!t?r():""}
      ${!l()||f?d():""}
    </div>`,m()}function m(){var f,g,E,_,u,I,k;const s=b=>e.querySelector(b);e.querySelectorAll(".booster-row").forEach(b=>{b.addEventListener("click",h=>{h.target.closest(".btn-delete")||(t=b.dataset.id,c=null,p())})}),(f=s("#btn-back"))==null||f.addEventListener("click",()=>{t=null,p()}),(g=s("#btn-new"))==null||g.addEventListener("click",async()=>{const b=prompt("Nom du nouveau booster :");if(!(b!=null&&b.trim()))return;const{data:h,error:v}=await y.from("booster_configs").insert({name:b.trim(),price_type:"credits",price_credits:1e3,card_count:5}).select().single();if(v){alert(v.message);return}i.push(h),t=h.id,c=null,p()}),e.querySelectorAll(".btn-delete").forEach(b=>{b.addEventListener("click",async h=>{if(h.stopPropagation(),!confirm("Supprimer ce booster ?"))return;await y.from("booster_configs").delete().eq("id",b.dataset.id);const v=i.findIndex(x=>x.id===b.dataset.id);v>-1&&i.splice(v,1),t===b.dataset.id&&(t=null,c=null),p()})}),(E=s("#btn-cancel"))==null||E.addEventListener("click",()=>{t=null,p()}),(_=s("#f-price-type"))==null||_.addEventListener("change",b=>{const h=s("#credits-field");h&&(h.style.opacity=b.target.value!=="credits"?"0.4":"1")}),(u=s("#btn-pick-icon"))==null||u.addEventListener("click",async()=>{var h;const b=s("#icon-picker-grid");if(b){if(b.style.display!=="none"){b.style.display="none";return}b.innerHTML='<div style="padding:10px;text-align:center;color:#888;font-size:12px">⏳ Chargement...</div>',b.style.display="block";try{const x=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),w=Array.isArray(x)?x.filter($=>$.name.startsWith("booster")&&/\.(png|jpg|svg|webp)$/i.test($.name)):[];if(!w.length){b.innerHTML='<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Aucune icône "booster*" trouvée.</div>';return}const B=((h=s("#f-image-url"))==null?void 0:h.value)||"";b.innerHTML=`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(72px,1fr));gap:8px">
          ${w.map($=>`
        ...
        </div>`,b.querySelectorAll(".icon-pick-item").forEach($=>{$.addEventListener("click",()=>{const S=s("#f-image-url");S&&(S.value=$.dataset.name),b.style.display="none"})})}catch(v){b.innerHTML=`<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Erreur API GitHub : ${v.message}</div>`}}});function a(){e.querySelectorAll("[data-rate-idx]").forEach(b=>{var v,x,w,B,$;const h=Number(b.dataset.rateIdx);h>=0&&h<n.length&&(n[h].card_type=((v=b.querySelector(".rate-type"))==null?void 0:v.value)||"player",n[h].rarity=((x=b.querySelector(".rate-rarity"))==null?void 0:x.value)||null,n[h].note_min=Number((w=b.querySelector(".rate-note-min"))==null?void 0:w.value)||null,n[h].note_max=Number((B=b.querySelector(".rate-note-max"))==null?void 0:B.value)||null,n[h].percentage=Number(($=b.querySelector(".rate-pct"))==null?void 0:$.value)||0)})}(I=s("#btn-add-rate"))==null||I.addEventListener("click",()=>{a(),n.push({id:null,booster_id:t,card_type:"player",rarity:"normal",note_min:null,note_max:null,percentage:10,sort_order:n.length}),p(!0)}),e.querySelectorAll(".btn-del-rate").forEach(b=>{b.addEventListener("click",()=>{a(),n.splice(Number(b.dataset.idx),1),p(!0)})}),(k=s("#btn-save"))==null||k.addEventListener("click",async()=>{var B,$,S,j,M,D,F,H,U,G,V,K,C;const b=(i||[]).find(L=>L.id===t);if(!b)return;const h=[];e.querySelectorAll("[data-rate-idx]").forEach(L=>{var se,ce,pe,ue,me;const ne=Number(L.dataset.rateIdx);h.push({booster_id:t,card_type:((se=L.querySelector(".rate-type"))==null?void 0:se.value)||"player",rarity:((ce=L.querySelector(".rate-rarity"))==null?void 0:ce.value)||null,note_min:Number((pe=L.querySelector(".rate-note-min"))==null?void 0:pe.value)||null,note_max:Number((ue=L.querySelector(".rate-note-max"))==null?void 0:ue.value)||null,percentage:Number((me=L.querySelector(".rate-pct"))==null?void 0:me.value)||0,sort_order:ne});const q=h[h.length-1];q.rarity||(q.rarity=null),q.note_min||(q.note_min=null),q.note_max||(q.note_max=null)});const v=h.reduce((L,ne)=>L+ne.percentage,0);if(h.length&&Math.abs(v-100)>.5){alert(`La somme doit faire 100% (actuellement ${v.toFixed(1)}%)`);return}const x={name:(($=(B=s("#f-name"))==null?void 0:B.value)==null?void 0:$.trim())||b.name,image_url:((j=(S=s("#f-image-url"))==null?void 0:S.value)==null?void 0:j.trim())||null,price_type:(M=s("#f-price-type"))==null?void 0:M.value,price_credits:Number((D=s("#f-price-credits"))==null?void 0:D.value)||0,card_count:Number((F=s("#f-card-count"))==null?void 0:F.value)||5,is_active:((H=s("#f-active"))==null?void 0:H.checked)??b.is_active,allow_duplicates:((U=s("#f-allow-dup"))==null?void 0:U.checked)??!0,sort_order:Number((G=s("#f-sort"))==null?void 0:G.value)||0,max_per_user:(V=s("#f-max-per-user"))!=null&&V.value?Number(s("#f-max-per-user").value):null,available_from:((K=s("#f-available-from"))==null?void 0:K.value)||null,available_until:((C=s("#f-available-until"))==null?void 0:C.value)||null},{error:w}=await y.from("booster_configs").update(x).eq("id",t);if(w){alert(w.message);return}if(Object.assign(b,x),await y.from("booster_drop_rates").delete().eq("booster_id",t),h.length){const{error:L}=await y.from("booster_drop_rates").insert(h);if(L){alert(L.message);return}}n=h,c=t,alert("✅ Booster enregistré !"),p(!0)})}p()}const Re=[{value:"normal",label:"Normal",color:"#888"},{value:"pepite",label:"Pépite",color:"#D4A017"},{value:"papyte",label:"Papyte",color:"#909090"},{value:"legende",label:"Légende",color:"#7a28b8"}];async function ft(e){await re(e)}async function re(e){const{data:i,error:t}=await y.from("sell_price_configs").select("*").order("rarity").order("note_min");if(t){e.innerHTML=`<div style="padding:24px;color:red">Erreur : ${t.message}</div>`;return}e.innerHTML=`
    ...
    </div>`,document.getElementById("add-config").addEventListener("click",()=>_e(null)),document.getElementById("form-cancel").addEventListener("click",()=>{document.getElementById("config-form").style.display="none"}),document.getElementById("form-save").addEventListener("click",()=>vt(e)),e.querySelectorAll("[data-edit]").forEach(n=>{const c=(i||[]).find(l=>l.id===n.dataset.edit);c&&n.addEventListener("click",()=>_e(c))}),e.querySelectorAll("[data-delete]").forEach(n=>{n.addEventListener("click",async()=>{confirm("Supprimer cette règle ?")&&(await y.from("sell_price_configs").delete().eq("id",n.dataset.delete),await re(e))})})}function xt(e){const i=Re.find(t=>t.value===e.rarity);return`
    ...
    </tr>`}function _e(e){document.getElementById("config-form").style.display="block",document.getElementById("form-title").textContent=e?"Modifier la règle":"Nouvelle règle",document.getElementById("form-id").value=(e==null?void 0:e.id)||"",document.getElementById("form-rarity").value=(e==null?void 0:e.rarity)||"normal",document.getElementById("form-price").value=(e==null?void 0:e.price)||1e3,document.getElementById("form-note-min").value=(e==null?void 0:e.note_min)??1,document.getElementById("form-note-max").value=(e==null?void 0:e.note_max)??10,document.getElementById("config-form").scrollIntoView({behavior:"smooth"})}async function vt(e){const i=document.getElementById("form-id").value,t=document.getElementById("form-rarity").value,n=parseInt(document.getElementById("form-price").value)||0,c=parseInt(document.getElementById("form-note-min").value)||1,l=parseInt(document.getElementById("form-note-max").value)||10;if(c>l){alert("Note min doit être ≤ note max");return}const o={rarity:t,price:n,note_min:c,note_max:l,updated_at:new Date().toISOString()};let r;if(i?{error:r}=await y.from("sell_price_configs").update(o).eq("id",i):{error:r}=await y.from("sell_price_configs").insert(o),r){alert("Erreur : "+r.message);return}document.getElementById("config-form").style.display="none",await re(e)}async function yt(e){await le(e)}async function le(e){const{data:i}=await y.from("patch_notes").select("*").order("published_at",{ascending:!1});e.innerHTML=`
    <div style="padding:20px;max-width:760px;margin:0 auto">
      ...
      <div id="articles-list" style="display:flex;flex-direction:column;gap:12px">
        ${(i||[]).map(t=>ht(t)).join("")||`<div style="color:#aaa;padding:24px;text-align:center">Aucun article pour l'instant.</div>`}
      </div>
    ...
    </div>`,document.getElementById("add-article").addEventListener("click",()=>$e(null)),document.getElementById("form-cancel").addEventListener("click",()=>{document.getElementById("article-form").style.display="none"}),document.getElementById("form-save").addEventListener("click",()=>Et(e)),e.querySelectorAll("[data-edit]").forEach(t=>{const n=(i||[]).find(c=>c.id===t.dataset.edit);n&&t.addEventListener("click",()=>$e(n))}),e.querySelectorAll("[data-delete]").forEach(t=>{t.addEventListener("click",async()=>{confirm("Supprimer cet article ?")&&(await y.from("patch_notes").delete().eq("id",t.dataset.delete),await le(e))})})}function ht(e){const i=new Date(e.published_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"short",year:"numeric"});return`
    ...
    </div>`}function $e(e){const i=document.getElementById("article-form");i.style.display="block",document.getElementById("form-title").textContent=e?"Modifier l'article":"Nouvel article",document.getElementById("form-id").value=(e==null?void 0:e.id)||"",document.getElementById("form-art-title").value=(e==null?void 0:e.title)||"",document.getElementById("form-art-desc").value=(e==null?void 0:e.description)||"",document.getElementById("form-art-img").value=(e==null?void 0:e.image_url)||"",document.getElementById("form-art-pub").checked=e?e.is_published:!0;const t=new Date((e==null?void 0:e.published_at)||Date.now()),n=new Date(t.getTime()-t.getTimezoneOffset()*6e4).toISOString().slice(0,16);document.getElementById("form-art-date").value=n,i.scrollIntoView({behavior:"smooth"})}async function Et(e){const i=document.getElementById("form-id").value,t=document.getElementById("form-art-title").value.trim(),n=document.getElementById("form-art-desc").value.trim(),c=document.getElementById("form-art-img").value.trim()||null,l=document.getElementById("form-art-pub").checked,o=document.getElementById("form-art-date").value;if(!t){alert("Le titre est obligatoire.");return}if(!n){alert("La description est obligatoire.");return}const r=o?new Date(o).toISOString():new Date().toISOString(),d={title:t,description:n,image_url:c,is_published:l,published_at:r};let p;if(i?{error:p}=await y.from("patch_notes").update(d).eq("id",i):{error:p}=await y.from("patch_notes").insert(d),p){alert("Erreur : "+p.message);return}document.getElementById("article-form").style.display="none",await le(e)}async function wt(e){await O(e)}async function O(e){var m,s,a,f,g,E,_,u,I,k,b,h;const{data:i}=await y.from("tutorial_steps").select("*").order("step_order");e.innerHTML=`
    ...
    </div>`,(m=document.getElementById("ts-add"))==null||m.addEventListener("click",()=>ke(null,(i==null?void 0:i.length)||0)),(s=document.getElementById("ts-reset"))==null||s.addEventListener("click",async()=>{if(!confirm("Remettre tutorial_done = false pour TOUS les joueurs ? Ils reverront le tutoriel à leur prochaine connexion."))return;const{error:v}=await y.from("users").update({tutorial_done:!1}).neq("id","00000000-0000-0000-0000-000000000000");alert(v?"Erreur : "+v.message:"✅ Tutorial_done réinitialisé pour tous les joueurs.")}),(a=document.getElementById("ts-cancel"))==null||a.addEventListener("click",Ne),(f=document.getElementById("ts-save"))==null||f.addEventListener("click",()=>kt(e)),(g=document.getElementById("ts-preview-btn"))==null||g.addEventListener("click",$t);const t=document.getElementById("ts-editor"),n=()=>{const v=document.getElementById("ts-content");v&&t&&(v.value=t.innerHTML)};t==null||t.addEventListener("input",n),t==null||t.addEventListener("blur",n);const c=()=>{document.activeElement!==t&&(t==null||t.focus())};(E=document.getElementById("ts-toolbar"))==null||E.querySelectorAll("[data-cmd]").forEach(v=>{v.addEventListener("mousedown",x=>{x.preventDefault(),c();const w=v.dataset.cmd;w.startsWith("formatBlock:")?document.execCommand("formatBlock",!1,w.split(":")[1]):document.execCommand(w,!1,null),n()})}),(_=document.getElementById("ts-insert-color"))==null||_.addEventListener("mousedown",v=>{v.preventDefault(),c();const x=prompt("Couleur hex (ex: #D4A017 pour or, #1A6B3C pour vert, #cc2222 pour rouge) :");x&&(document.execCommand("foreColor",!1,x),n())}),(u=document.getElementById("ts-clear-format"))==null||u.addEventListener("mousedown",v=>{v.preventDefault(),c(),document.execCommand("removeFormat",!1,null),n()}),t==null||t.addEventListener("focus",()=>{t&&(t.style.borderColor="#1A6B3C")}),t==null||t.addEventListener("blur",()=>{t&&(t.style.borderColor="#ddd")});const l=document.getElementById("ts-image"),o=document.getElementById("ts-img-preview"),r=document.getElementById("ts-img-preview-el"),d=document.getElementById("ts-img-picker-grid"),p=()=>{var x;const v=(x=l==null?void 0:l.value)==null?void 0:x.trim();if(v){const w="/";r.src=`${w}icons/${v}`,o.style.display="block"}else o.style.display="none"};l==null||l.addEventListener("input",p),(I=document.getElementById("ts-img-clear"))==null||I.addEventListener("click",()=>{l&&(l.value=""),o.style.display="none",d.style.display="none"}),(k=document.getElementById("ts-img-pick"))==null||k.addEventListener("click",async()=>{if(d.style.display!=="none"){d.style.display="none";return}d.innerHTML='<div style="color:#aaa;font-size:12px">Chargement…</div>',d.style.display="block";try{const x=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),w=Array.isArray(x)?x.filter($=>$.name.startsWith("tuto_")):[];if(!w.length){d.innerHTML='<div style="color:#aaa;font-size:12px">Aucun fichier tuto_* trouvé dans /icons/.<br>Uploade des images nommées tuto_xxx.png dans public/icons/</div>';return}const B="/";d.innerHTML='<div style="display:flex;flex-wrap:wrap;gap:8px">'+w.map($=>`
  ...
  <div style="background:#fff;border-radius:10px;padding:12px 16px;box-shadow:0 1px 4px rgba(0,0,0,0.08);display:flex;align-items:center;gap:12px">
  ...
  </div>`}function ke(e,i=0){document.getElementById("ts-form").style.display="block",document.getElementById("ts-form-title").textContent=e?"Modifier l'étape":"Nouvelle étape",document.getElementById("ts-id").value=(e==null?void 0:e.id)||"",document.getElementById("ts-emoji").value=(e==null?void 0:e.emoji)||"⚽",document.getElementById("ts-title").value=(e==null?void 0:e.title)||"",document.getElementById("ts-content").value=(e==null?void 0:e.content)||"";const t=document.getElementById("ts-editor");t&&(t.innerHTML=(e==null?void 0:e.content)||"<p>Texte de l'étape…</p>"),document.getElementById("ts-order").value=(e==null?void 0:e.step_order)??i,document.getElementById("ts-active").checked=e?e.is_active:!0;const n=(e==null?void 0:e.color)||"#1A6B3C";document.getElementById("ts-color").value=n,document.getElementById("ts-color-hex").value=n;const c=(e==null?void 0:e.image_url)||"",l=document.getElementById("ts-image");l&&(l.value=c);const o=document.getElementById("ts-img-preview"),r=document.getElementById("ts-img-preview-el");if(c&&o&&r){const d="/";r.src=`${d}icons/${c}`,o.style.display="block"}else o&&(o.style.display="none");document.getElementById("ts-img-picker-grid").style.display="none",document.getElementById("ts-preview-area").innerHTML="",document.getElementById("ts-form").scrollIntoView({behavior:"smooth"})}function Ne(){document.getElementById("ts-form").style.display="none",document.getElementById("ts-preview-area").innerHTML=""}function $t(){var p,m;const e=document.getElementById("ts-emoji").value||"⚽",i=document.getElementById("ts-title").value||"Titre",t=document.getElementById("ts-editor"),n=t?t.innerHTML:document.getElementById("ts-content").value||"",c=document.getElementById("ts-color-hex").value||"#1A6B3C",l=(m=(p=document.getElementById("ts-image"))==null?void 0:p.value)==null?void 0:m.trim(),r=l?`<div style="padding:0 20px 12px;text-align:center"><img src="/icons/${l}" style="max-height:140px;max-width:100%;border-radius:10px;object-fit:contain"></div>`:"",d=document.getElementById("ts-preview-area");d.innerHTML=`
    ...
    </div>`,d.querySelectorAll("ul,ol").forEach(s=>{s.style.paddingLeft="20px",s.style.margin="6px 0"}),d.querySelectorAll("li").forEach(s=>{s.style.marginBottom="4px"}),d.querySelectorAll("p").forEach(s=>{s.style.marginBottom="8px"})}async function kt(e){var s,a;const i=document.getElementById("ts-id").value,t=document.getElementById("ts-emoji").value.trim()||"⚽",n=document.getElementById("ts-title").value.trim(),c=document.getElementById("ts-editor"),l=(c?c.innerHTML:document.getElementById("ts-content").value).trim(),o=document.getElementById("ts-color-hex").value.trim()||"#1A6B3C",r=parseInt(document.getElementById("ts-order").value)||0,d=document.getElementById("ts-active").checked;if(!n||!l){alert("Titre et contenu sont obligatoires.");return}const p={emoji:t,title:n,content:l,color:o,step_order:r,is_active:d,image_url:((a=(s=document.getElementById("ts-image"))==null?void 0:s.value)==null?void 0:a.trim())||null};let m;if(i?{error:m}=await y.from("tutorial_steps").update(p).eq("id",i):{error:m}=await y.from("tutorial_steps").insert(p),m){alert("Erreur : "+m.message);return}Ne(),O(e)}const It="/",Pe="#E87722",Bt=[["MA","Maroc"],["FR","France"],["AR","Argentine"],["PT","Portugal"],["BR","Brésil"],["ES","Espagne"],["DE","Allemagne"],["GB","Angleterre"],["IT","Italie"],["CM","Cameroun"],["SN","Sénégal"],["NG","Nigéria"],["DK","Danemark"],["NL","Pays-Bas"],["BE","Belgique"],["CI","Côte d'Ivoire"],["AL","Albanie"],["HR","Croatie"],["RS","Serbie"],["TR","Turquie"],["MW","Malawi"],["GH","Ghana"],["ZA","Afrique du Sud"],["EG","Égypte"],["US","États-Unis"],["MX","Mexique"],["CO","Colombie"],["UY","Uruguay"],["CL","Chili"],["JP","Japon"],["KR","Corée du Sud"],["CN","Chine"],["AU","Australie"],["SW","Suisse"],["SE","Suède"],["NO","Norvège"],["PL","Pologne"]];async function Lt(e){await de(e)}async function de(e){var n,c,l,o;const[{data:i},{data:t}]=await Promise.all([y.from("stadium_definitions").select("*, club:clubs(encoded_name, logo_url)").order("name"),y.from("clubs").select("id, encoded_name, logo_url").order("encoded_name")]);e.innerHTML=`
    <div style="padding:20px;max-width:820px;margin:0 auto">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px">
      ...
      <div style="display:flex;flex-direction:column;gap:10px" id="st-list">
        ${(i||[]).map(r=>zt(r)).join("")||'<div style="color:#aaa;padding:24px;text-align:center">Aucun stade créé.</div>'}
      </div>
          ...
          <div style="flex:1;min-width:220px;display:flex;flex-direction:column;gap:12px">
            ...
            <div>
              ...
              <select id="st-country" style="width:100%;box-sizing:border-box;padding:9px;border:1.5px solid #ddd;border-radius:8px;font-size:13px;background:#fff">
                ...
                ${Bt.map(([r,d])=>`<option value="${r}">${d} (${r})</option>`).join("")}
              </select>
            </div>
            <div>
    ...
    </div>`,(n=document.getElementById("st-add-btn"))==null||n.addEventListener("click",()=>Ie(null)),(c=document.getElementById("st-cancel"))==null||c.addEventListener("click",()=>{document.getElementById("st-form").style.display="none"}),(l=document.getElementById("st-save"))==null||l.addEventListener("click",()=>St(e)),(o=document.getElementById("st-preview-btn"))==null||o.addEventListener("click",()=>te()),["st-name","st-club","st-country","st-image"].forEach(r=>{var d,p;(d=document.getElementById(r))==null||d.addEventListener("input",te),(p=document.getElementById(r))==null||p.addEventListener("change",te)}),e.querySelectorAll("[data-edit]").forEach(r=>{const d=(i||[]).find(p=>p.id===r.dataset.edit);d&&r.addEventListener("click",()=>Ie(d))}),e.querySelectorAll("[data-delete]").forEach(r=>{r.addEventListener("click",async()=>{confirm("Supprimer ce stade ?")&&(await y.from("stadium_definitions").delete().eq("id",r.dataset.delete),de(e))})})}function zt(e){var t,n;const i=(t=e.club)!=null&&t.logo_url?`<img src="${e.club.logo_url}" style="width:32px;height:32px;object-fit:contain">`:'<span style="font-size:20px">🌍</span>';return`
    <div style="background:#fff;border-radius:10px;padding:12px 16px;box-shadow:0 1px 4px rgba(0,0,0,0.08);display:flex;align-items:center;gap:12px">
      ...
      <button data-edit="${e.id}" class="btn btn-ghost btn-sm">✏️</button>
      <button data-delete="${e.id}" class="btn btn-ghost btn-sm" style="color:#cc2222">🗑️</button>
    </div>`}function Ie(e,i){var n,c;document.getElementById("st-form").style.display="block",document.getElementById("st-form-title").textContent=e?"Modifier le stade":"Nouveau stade",document.getElementById("st-id").value=(e==null?void 0:e.id)||"",document.getElementById("st-name").value=(e==null?void 0:e.name)||"",document.getElementById("st-club").value=(e==null?void 0:e.club_id)||"";const t=document.getElementById("st-country");t&&(t.value=(e==null?void 0:e.country_code)||""),document.getElementById("st-image").value=(e==null?void 0:e.image_url)||"",document.getElementById("st-preview-card").innerHTML="",te((n=e==null?void 0:e.club)==null?void 0:n.logo_url,(c=e==null?void 0:e.club)==null?void 0:c.encoded_name),document.getElementById("st-form").scrollIntoView({behavior:"smooth"})}function te(){var p,m,s,a,f,g,E;const e=((p=document.getElementById("st-name"))==null?void 0:p.value)||"NOM DU STADE",i=(s=(m=document.getElementById("st-image"))==null?void 0:m.value)==null?void 0:s.trim(),t=(f=(a=document.getElementById("st-country"))==null?void 0:a.value)==null?void 0:f.trim(),n=document.getElementById("st-club"),c=(n==null?void 0:n.selectedIndex)||0,l=n&&c>0?n.options[c].text:"",o=((E=(g=n==null?void 0:n.options[c])==null?void 0:g.getAttribute)==null?void 0:E.call(g,"data-logo"))||"";let r;i?r=`<img src="${It}icons/${i}" style="width:64px;height:64px;object-fit:contain;display:block;margin:0 auto">`:o?r=`<img src="${o}" style="width:64px;height:64px;object-fit:contain;display:block;margin:0 auto" onerror="this.style.display='none'">`:t?r=`<img src="${`https://flagcdn.com/w80/${t.toLowerCase()}.png`}" style="width:80px;height:60px;object-fit:contain;display:block;margin:0 auto;border-radius:4px">`:r='<div style="font-size:36px;text-align:center">🏟️</div>';const d=l||t||"—";document.getElementById("st-preview-card").innerHTML=At(e,r,d,!0)}function At(e,i,t,n=!1){return`<div style="width:140px;background:linear-gradient(160deg,${Pe},#c45a00);border-radius:12px;padding:6px;display:flex;flex-direction:column;gap:0;box-shadow:0 4px 12px rgba(232,119,34,0.4);font-family:Arial,sans-serif">
  ...
  </div>`}async function St(e){const i=document.getElementById("st-id").value,t=document.getElementById("st-name").value.trim(),n=document.getElementById("st-club").value||null,c=document.getElementById("st-country").value.trim().toUpperCase()||null,l=document.getElementById("st-image").value.trim()||null;if(!t){alert("Le nom est obligatoire");return}const o={name:t,club_id:n,country_code:c,image_url:l},{error:r}=i?await y.from("stadium_definitions").update(o).eq("id",i):await y.from("stadium_definitions").insert(o);if(r){alert("Erreur : "+r.message);return}document.getElementById("st-form").style.display="none",de(e)}const R=[{value:"game_changer",label:"⚡ Game Changer",border:"#7a28b8",bg:"linear-gradient(135deg,#4a0080,#7a28b8)"},{value:"ultra_game_changer",label:"💎 Ultra Game Changer",border:"#00bcd4",bg:"linear-gradient(135deg,#006080,#00bcd4)"}],N=[{value:"purple",label:"Violet",hex:"#7a28b8"},{value:"light_blue",label:"Bleu clair",hex:"#00bcd4"}];async function Ct(e){e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">Chargement...</div>';const{data:i}=await y.from("gc_definitions").select("*").order("sort_order").order("created_at");let t=null;const n=[{value:"BOOST_STAT",label:"⚡ Boost stats (+N)",hasValue:!0,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"DEBUFF_STAT",label:"💀 Debuff stats (-N)",hasValue:!0,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"GRAY_PLAYER",label:"❄️ Griser un joueur",hasValue:!1,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"REVIVE_PLAYER",label:"💫 Ressusciter un joueur",hasValue:!1,hasTarget:!1,hasCount:!0,hasRoles:!1},{value:"REMOVE_GOAL",label:"🚫 Retirer un but adverse",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1},{value:"ADD_GOAL_DRAW",label:"⚽ +1 but si duel nul",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1},{value:"ADD_SUB",label:"🔄 Ajouter un remplacement",hasValue:!1,hasTarget:!1,hasCount:!0,hasRoles:!1},{value:"CUSTOM",label:"🛠️ Effet custom (hardcodé)",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1}],c=["GK","DEF","MIL","ATT"],l=()=>window.innerWidth<700,o=a=>e.querySelector(a);function r(a,f=130){if(!a)return"";const g=R.find(k=>k.value===a.gc_type)||R[0],E=N.find(k=>k.value===a.color)||N[0],_=Math.round(f*.55),u=Math.round(f*.15),I=Math.round(f*.3);return`<div style="width:${f}px;height:${Math.round(f*1.4)}px;border-radius:10px;border:3px solid ${E.hex};background:${g.bg};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 16px ${E.hex}55;flex-shrink:0">
    ...
    </div>`}function p(){const a=t?(i||[]).find(g=>g.id===t):null;if(!a)return`<div style="flex:1;display:flex;align-items:center;justify-content:center;color:#bbb;font-size:14px;padding:40px;text-align:center">
    ...
    <div style="display:flex;height:calc(100vh - 56px);overflow:hidden">
      ${!l()||!t?d():""}
      ${!l()||t?p():""}
    </div>`,s()}function s(){var a,f,g,E,_;e.querySelectorAll(".gc-row").forEach(u=>{u.addEventListener("click",I=>{I.target.closest(".btn-toggle-gc,.btn-delete-gc")||(t=u.dataset.id,m())})}),(a=o("#btn-gc-back"))==null||a.addEventListener("click",()=>{t=null,m()}),(f=o("#btn-cancel-gc"))==null||f.addEventListener("click",()=>{t=null,m()}),(g=o("#btn-new-gc"))==null||g.addEventListener("click",async()=>{const u=prompt("Nom de la carte Game Changer :");if(!(u!=null&&u.trim()))return;const{data:I,error:k}=await y.from("gc_definitions").insert({name:u.trim(),gc_type:"game_changer",color:"purple"}).select().single();if(k){alert(k.message);return}i.push(I),t=I.id,m()}),e.querySelectorAll(".btn-toggle-gc").forEach(u=>{u.addEventListener("click",async I=>{I.stopPropagation();const k=u.dataset.active!=="true";await y.from("gc_definitions").update({is_active:k}).eq("id",u.dataset.id);const b=i.find(h=>h.id===u.dataset.id);b&&(b.is_active=k),m()})}),e.querySelectorAll(".btn-delete-gc").forEach(u=>{u.addEventListener("click",async I=>{if(I.stopPropagation(),!confirm("Supprimer cette carte Game Changer ?"))return;await y.from("gc_definitions").delete().eq("id",u.dataset.id);const k=i.findIndex(b=>b.id===u.dataset.id);k>-1&&i.splice(k,1),t===u.dataset.id&&(t=null),m()})}),(E=o("#btn-pick-gc-icon"))==null||E.addEventListener("click",async()=>{var I;const u=o("#gc-icon-picker");if(u){if(u.style.display!=="none"){u.style.display="none";return}u.innerHTML='<div style="padding:10px;text-align:center;color:#888;font-size:12px">⏳ Chargement...</div>',u.style.display="block";try{const b=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),h=Array.isArray(b)?b.filter(x=>x.name.startsWith("gamechanger-")&&/\.(png|jpg|svg|webp)$/i.test(x.name)):[];if(!h.length){u.innerHTML='<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Aucun fichier "gamechanger-*" trouvé dans public/icons/</div>';return}const v=((I=o("#gc-image-url"))==null?void 0:I.value)||"";u.innerHTML=`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(72px,1fr));gap:8px">
          ${h.map(x=>`
        ...
        </div>`,u.querySelectorAll(".gc-icon-item").forEach(x=>{x.addEventListener("click",()=>{const w=o("#gc-image-url");w&&(w.value=x.dataset.name),u.style.display="none"})})}catch(k){u.innerHTML=`<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Erreur API GitHub : ${k.message}</div>`}}}),(_=o("#btn-save-gc"))==null||_.addEventListener("click",async()=>{var x,w,B,$,S,j,M,D,F,H,U,G,V,K;const u=i.find(C=>C.id===t);if(!u)return;const I=n.find(C=>{var L;return C.value===(((L=o("#gc-effect-type"))==null?void 0:L.value)||"BOOST_STAT")})||n[0],k=I.hasRoles?[...e.querySelectorAll(".gc-p-role:checked")].map(C=>C.value):null,b={...I.hasValue?{value:Number((x=o("#gc-p-value"))==null?void 0:x.value)||2}:{},...I.hasCount?{count:Number((w=o("#gc-p-count"))==null?void 0:w.value)||1}:{},...I.hasTarget?{target:((B=o("#gc-p-target"))==null?void 0:B.value)||"home"}:{},...I.hasRoles?{roles:k!=null&&k.length?k:null}:{}},h={name:((S=($=o("#gc-name"))==null?void 0:$.value)==null?void 0:S.trim())||u.name,effect:((M=(j=o("#gc-effect"))==null?void 0:j.value)==null?void 0:M.trim())||null,image_url:((F=(D=o("#gc-image-url"))==null?void 0:D.value)==null?void 0:F.trim())||null,gc_type:((H=o("#gc-type"))==null?void 0:H.value)||"game_changer",color:((U=o("#gc-color"))==null?void 0:U.value)||"purple",sort_order:Number((G=o("#gc-sort"))==null?void 0:G.value)||0,is_active:((V=o("#gc-active"))==null?void 0:V.checked)??u.is_active,effect_type:((K=o("#gc-effect-type"))==null?void 0:K.value)||"BOOST_STAT",effect_params:b},{error:v}=await y.from("gc_definitions").update(h).eq("id",t);if(v){alert(v.message);return}Object.assign(u,h),alert("✅ Carte enregistrée !"),m()})}m()}We(Se);function Tt(e,i="info"){const t=document.getElementById("toast");t.textContent=e,t.className=`show ${i}`,clearTimeout(t._t),t._t=setTimeout(()=>{t.className=""},3e3)}function jt(e,i,t=""){document.getElementById("modal-title").textContent=e,document.getElementById("modal-body").innerHTML=i,document.getElementById("modal-footer").innerHTML=t,document.getElementById("modal-overlay").classList.remove("hidden")}function ae(){document.getElementById("modal-overlay").classList.add("hidden")}const Be={dashboard:{title:"Dashboard",fn:Xe},players:{title:"Joueurs & Cartes",fn:Ze},clubs:{title:"Clubs",fn:et},"card-builder":{title:"Card Builder",fn:ot},users:{title:"Managers",fn:je},market:{title:"Marché des transferts",fn:Me},"import-export":{title:"Import / Export CSV",fn:st},formations:{title:"Formations & Liens",fn:pt},"boosters-config":{title:"Boosters",fn:bt},"sell-price":{title:"Prix vente directe",fn:ft},journal:{title:"Journal",fn:yt},tutorial:{title:"Tutoriel",fn:wt},stadiums:{title:"Stades",fn:Lt},"gc-cards":{title:"Game Changers",fn:Ct}};async function ie(e){document.querySelectorAll(".admin-sidebar nav a").forEach(c=>{c.classList.toggle("active",c.dataset.page===e)});const i=document.getElementById("mobile-page-select");i&&(i.value=e);const t=Be[e]||Be.dashboard;document.getElementById("page-title").textContent=t.title;const n=document.getElementById("page-content");n.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">Chargement…</div>';try{await t.fn(n,{toast:Tt,openModal:jt,closeModal:ae,navigate:ie})}catch(c){n.innerHTML=`<div style="padding:40px;color:#c0392b">Erreur: ${c.message}</div>`,console.error(c)}}async function Mt(){var i;const{data:{session:e}}=await y.auth.getSession();e&&await Le(e.user),document.getElementById("auth-btn").addEventListener("click",async()=>{const t=document.getElementById("auth-email").value.trim(),n=document.getElementById("auth-password").value,c=document.getElementById("auth-error");if(c.textContent="",!t||!n){c.textContent="Remplissez tous les champs.";return}const{data:l,error:o}=await y.auth.signInWithPassword({email:t,password:n});if(o){c.textContent=o.message;return}await Le(l.user)}),document.getElementById("auth-password").addEventListener("keydown",t=>{t.key==="Enter"&&document.getElementById("auth-btn").click()}),document.getElementById("logout-btn").addEventListener("click",async()=>{await y.auth.signOut(),document.getElementById("auth-screen").style.display="flex",document.getElementById("admin-app").style.display="none"}),document.getElementById("modal-close").addEventListener("click",ae),document.getElementById("modal-overlay").addEventListener("click",t=>{t.target===t.currentTarget&&ae()}),document.querySelectorAll(".admin-sidebar nav a").forEach(t=>{t.addEventListener("click",n=>{n.preventDefault(),ie(t.dataset.page)})}),(i=document.getElementById("mobile-page-select"))==null||i.addEventListener("change",t=>{ie(t.target.value)})}async function Le(e){const{data:i,error:t}=await y.from("users").select("*").eq("id",e.id).single(),n=document.getElementById("auth-error");if(t||!i){n.textContent="Profil introuvable.";return}if(!i.is_admin){n.textContent="Accès refusé. Vous n'êtes pas administrateur.",await y.auth.signOut();return}document.getElementById("auth-screen").style.display="none",document.getElementById("admin-app").style.display="flex",document.getElementById("admin-user").textContent=`Admin: ${i.pseudo}`,ie("dashboard")}Mt();
