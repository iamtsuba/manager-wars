import{s as f,g as pe,F as P,a as J,b as me}from"./formation-links-CDBKB_z4.js";async function be(e){const[{count:n},{count:t},{count:i},{count:c},{count:l}]=await Promise.all([f.from("players").select("*",{count:"exact",head:!0}),f.from("clubs").select("*",{count:"exact",head:!0}),f.from("users").select("*",{count:"exact",head:!0}),f.from("cards").select("*",{count:"exact",head:!0}),f.from("matches").select("*",{count:"exact",head:!0})]);e.innerHTML=`
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:12px;margin-bottom:24px">
      ${j("🃏","Joueurs (cartes)",n??"–","#1A6B3C")}
      ${j("🏟️","Clubs",t??"–","#D4A017")}
      ${j("👥","Managers",i??"–","#7a28b8")}
      ${j("📦","Cartes possédées",c??"–","#2a8f52")}
      ${j("⚽","Matchs joués",l??"–","#bb2020")}
    </div>
  ...
  `,window.adminNav=a=>{var o;(o=document.querySelector(`[data-page="${a}"]`))==null||o.click()}}function j(e,n,t,i){return`<div class="card-panel" style="text-align:center">
    ...
    <div class="mw-inner" style="${a!=="normal"?`border:3px solid ${o};border-radius:8px`:""}">
      ...
      <div class="mw-stars">
        <div class="mw-band" style="background:${u}"></div>
        ${Q(u,d,52)}
        ${r&&m?Q(m,b,32):""}
      </div>
  ...
  </div>`}function T(e){const n={A:"E",E:"I",I:"O",O:"U",U:"A",a:"e",e:"i",i:"o",o:"u",u:"a"};return(e||"").replace(/[AEIOUaeiou]/g,t=>n[t]||t)}const W={normal:"Normal",pepite:"Pépite",papyte:"Papyte",legende:"Légende"},ve={normal:"#aaa",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"};async function ye(e,n){await z(e,n)}async function z(e,n){const{toast:t,openModal:i,closeModal:c}=n,[{data:l,error:a},{data:o}]=await Promise.all([f.from("players").select("*, clubs(encoded_name,logo_url)").order("surname_encoded"),f.from("clubs").select("id,encoded_name").order("encoded_name")]);if(a){e.innerHTML=`<p style="color:red">${a.message}</p>`;return}const s={};(o||[]).forEach(b=>{s[b.id]=b.encoded_name}),e.innerHTML=`
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;gap:12px;flex-wrap:wrap">
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        ...
        <select id="filter-rarity" style="width:130px">
          ...
          ${Object.entries(W).map(([b,u])=>`<option value="${b}">${u}</option>`).join("")}
        </select>
  ...
  `,r(l);function r(b){document.getElementById("count-label").textContent=`${b.length} joueur(s)`,document.getElementById("players-tbody").innerHTML=b.map(u=>{var m;return`
      <tr>
        <td>
          <div style="font-weight:600">${u.firstname} ${u.surname_encoded}</div>
          <div style="font-size:11px;color:var(--gray-600)">${u.firstname} ${u.surname_real}</div>
        </td>
        <td>
          <img src="https://flagsapi.com/${u.country_code}/flat/32.png" alt="${u.country_code}" style="height:16px;vertical-align:middle"> ${u.country_code}
        </td>
        <td style="font-size:12px">${((m=u.clubs)==null?void 0:m.encoded_name)||"—"}</td>
        <td>
          <span style="font-weight:700;color:${u.job==="ATT"?"#1A6B3C":u.job==="MIL"?"#D4A017":u.job==="DEF"?"#bb2020":"#111"}">
            ${u.job}
          </span>
          ${u.job2?`<span style="font-size:11px;color:#aaa"> / ${u.job2}</span>`:""}
        </td>
        <td style="font-size:12px;font-family:monospace">
          ${u.note_g} / ${u.note_d} / ${u.note_m} / ${u.note_a}
        </td>
        <td>
          <span style="background:${ve[u.rarity]};color:#fff;padding:2px 6px;border-radius:3px;font-size:10px;font-weight:700">
            ${W[u.rarity]||u.rarity}
          </span>
        </td>
        <td>
    ...
    `}).join(""),document.querySelectorAll("[data-edit]").forEach(u=>{u.addEventListener("click",()=>{const m=l.find(g=>g.id===u.dataset.edit);X(m,o||[],{toast:t,openModal:i,closeModal:c,reload:()=>z(e,n)})})}),document.querySelectorAll("[data-del]").forEach(u=>{u.addEventListener("click",async()=>{if(!confirm("Supprimer ce joueur ? Les cartes associées resteront en jeu."))return;const{error:m}=await f.from("players").delete().eq("id",u.dataset.del);m?t(m.message,"error"):(t("Joueur supprimé","success"),z(e,n))})})}function d(){const b=document.getElementById("search-players").value.toLowerCase(),u=document.getElementById("filter-rarity").value,m=document.getElementById("filter-job").value;r(l.filter(g=>(!b||`${g.firstname} ${g.surname_encoded} ${g.surname_real}`.toLowerCase().includes(b))&&(!u||g.rarity===u)&&(!m||g.job===m)))}document.getElementById("search-players").addEventListener("input",d),document.getElementById("filter-rarity").addEventListener("change",d),document.getElementById("filter-job").addEventListener("change",d),document.getElementById("add-player-btn").addEventListener("click",()=>{X(null,o||[],{toast:t,openModal:i,closeModal:c,reload:()=>z(e,n)})})}function X(e,n,{toast:t,openModal:i,closeModal:c,reload:l}){const a=!!e,o='<option value="">— Club —</option>'+n.map(s=>`<option value="${s.id}" ${(e==null?void 0:e.club_id)===s.id?"selected":""}>${s.encoded_name}</option>`).join("");i(a?`Modifier ${e.firstname} ${e.surname_encoded}`:"Nouveau joueur",`<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
  ...
  `,c(R),document.getElementById("search-clubs").addEventListener("input",l=>{const a=l.target.value.toLowerCase();c(R.filter(o=>o.encoded_name.toLowerCase().includes(a)||o.real_name.toLowerCase().includes(a)))}),document.getElementById("add-club-btn").addEventListener("click",()=>{Z(null,{toast:n,openModal:t,closeModal:i,reload:()=>M(e,{toast:n,openModal:t,closeModal:i})})});function c(l){document.getElementById("clubs-tbody").innerHTML=l.map(a=>`
    ...
    `).join(""),document.querySelectorAll("[data-edit]").forEach(a=>{a.addEventListener("click",()=>{const o=R.find(s=>s.id===a.dataset.edit);Z(o,{toast:n,openModal:t,closeModal:i,reload:()=>M(e,{toast:n,openModal:t,closeModal:i})})})}),document.querySelectorAll("[data-del]").forEach(a=>{a.addEventListener("click",async()=>{if(!confirm("Supprimer ce club ?"))return;const{error:o}=await f.from("clubs").delete().eq("id",a.dataset.del);o?n(o.message,"error"):(n("Club supprimé","success"),M(e,{toast:n,openModal:t,closeModal:i}))})})}}function Z(e,{toast:n,openModal:t,closeModal:i,reload:c}){const l=!!e;t(l?"Modifier le club":"Nouveau club",`<div style="display:flex;flex-direction:column;gap:12px">
     ...
     <button class="btn btn-primary" id="m-save">${l?"Enregistrer":"Créer"}</button>`),document.getElementById("m-cancel").addEventListener("click",i),document.getElementById("auto-encode").addEventListener("click",()=>{const a=document.getElementById("m-real").value,o=a.split(" ")[0]||a;document.getElementById("m-encoded").value=o.toUpperCase()+" FC"}),document.getElementById("m-real").addEventListener("input",()=>{const a=document.getElementById("m-real").value;if(!document.getElementById("m-encoded").value){const o=a.split(" ")[0]||a;document.getElementById("m-encoded").value=o.toUpperCase()+" FC"}}),document.getElementById("m-save").addEventListener("click",async()=>{const a={real_name:document.getElementById("m-real").value.trim(),encoded_name:document.getElementById("m-encoded").value.trim().toUpperCase(),country_code:document.getElementById("m-country").value.trim().toUpperCase(),logo_url:document.getElementById("m-logo").value.trim()||null};if(!a.real_name||!a.encoded_name||!a.country_code){n("Remplissez tous les champs obligatoires","error");return}const{error:o}=l?await f.from("clubs").update(a).eq("id",e.id):await f.from("clubs").insert(a);if(o){n(o.message,"error");return}n(l?"Club modifié":"Club créé","success"),i(),c()})}const _e=["blanc","metisse","typ","noir"],$e=["blond","marron","noir","chauve"],Ee=["rase","court","milong","long"],we=["normal","pepite","papyte","legende"],ee=["GK","DEF","MIL","ATT"],Ie=[["MA","Maroc"],["FR","France"],["AR","Argentine"],["PT","Portugal"],["BR","Brésil"],["ES","Espagne"],["DE","Allemagne"],["GB","Angleterre"],["IT","Italie"],["CM","Cameroun"],["SN","Sénégal"],["NG","Nigéria"],["DK","Danemark"],["NL","Pays-Bas"],["BE","Belgique"],["CI","Côte d'Ivoire"],["AL","Albanie"],["HR","Croatie"],["RS","Serbie"],["TR","Turquie"],["MW","Malawi"]];async function Le(e,{toast:n}){const{data:t}=await f.from("clubs").select("id,encoded_name,logo_url").order("encoded_name");e.innerHTML=ke(t||[]),Ae(e,t||[],n),C()}function te(){var i;const e=((i=document.getElementById("cb-surname-real"))==null?void 0:i.value)||"",n=T(e),t=document.getElementById("cb-surname-enc");t&&(t.value=n)}function C(){var d,b,u,m;const e=g=>{var y;return((y=document.getElementById(g))==null?void 0:y.value)||""},n={firstname:e("cb-firstname"),surname_encoded:e("cb-surname-enc")||T(e("cb-surname-real")),country_code:e("cb-country"),club_encoded_name:((b=(d=document.getElementById("cb-club"))==null?void 0:d.selectedOptions[0])==null?void 0:b.text)||"",job:e("cb-job"),job2:e("cb-job2")||null,note_g:parseInt(e("cb-note-g"))||0,note_d:parseInt(e("cb-note-d"))||0,note_m:parseInt(e("cb-note-m"))||0,note_a:parseInt(e("cb-note-a"))||0,rarity:e("cb-rarity"),skin:e("cb-skin"),hair:e("cb-hair"),hair_length:e("cb-hair-length")},t=pe(n.skin,n.hair,n.hair_length);e("cb-club");const i=document.getElementById("cb-club"),c=((m=(u=i==null?void 0:i.selectedOptions[0])==null?void 0:u.dataset)==null?void 0:m.logo)||null,l=fe(n,{portraitUrl:t,clubLogoUrl:c,showNotes:!0}),a=document.getElementById("card-preview");a&&(a.innerHTML=l);const o=e("cb-surname-real"),s=n.surname_encoded,r=document.getElementById("encode-summary");r&&o&&(r.textContent=`${e("cb-firstname")} ${o} → ${e("cb-firstname")} ${s}`)}function Ae(e,n,t){var i,c,l,a;e.querySelectorAll("input,select").forEach(o=>{o.addEventListener("input",C),o.addEventListener("change",C)}),(i=document.getElementById("cb-surname-real"))==null||i.addEventListener("input",()=>{te(),C()}),(c=document.getElementById("btn-encode"))==null||c.addEventListener("click",()=>{te(),C()}),(l=document.getElementById("btn-save-player"))==null||l.addEventListener("click",async()=>{await Be(n,t)}),(a=document.getElementById("btn-reset"))==null||a.addEventListener("click",()=>{e.querySelectorAll("input").forEach(o=>o.value=""),e.querySelectorAll("select").forEach(o=>o.selectedIndex=0),C()})}async function Be(e,n){const t=s=>{var r;return((r=document.getElementById(s))==null?void 0:r.value)||""},i=t("cb-surname-real").trim(),c=t("cb-surname-enc").trim()||T(i);if(!t("cb-firstname")||!i||!t("cb-country")||!t("cb-job")){n("Remplissez les champs obligatoires (prénom, nom, pays, poste)","error");return}const l=t("cb-club")||null,a={firstname:t("cb-firstname").trim(),surname_real:i,surname_encoded:c,country_code:t("cb-country"),club_id:l||null,job:t("cb-job"),job2:t("cb-job2")||null,note_g:parseInt(t("cb-note-g"))||0,note_d:parseInt(t("cb-note-d"))||0,note_m:parseInt(t("cb-note-m"))||0,note_a:parseInt(t("cb-note-a"))||0,rarity:t("cb-rarity")||"normal",note_min:parseInt(t("cb-note-min"))||null,note_max:parseInt(t("cb-note-max"))||null,skin:t("cb-skin")||"blanc",hair:t("cb-hair")||"noir",hair_length:t("cb-hair-length")||"court",sell_price:parseInt(t("cb-price"))||0},{error:o}=await f.from("players").insert(a);if(o){n("Erreur: "+o.message,"error");return}n(`✅ Joueur "${a.firstname} ${a.surname_encoded}" enregistré !`,"success")}function ke(e){const n=Ie.map(([r,d])=>`<option value="${r}">${d} (${r})</option>`).join(""),t='<option value="">— Sélectionner un club —</option>'+e.map(r=>`<option value="${r.id}" data-logo="${r.logo_url||""}">${r.encoded_name}</option>`).join(""),i=ee.map(r=>`<option value="${r}">${r}</option>`).join(""),c='<option value="">Aucun</option>'+ee.map(r=>`<option value="${r}">${r}</option>`).join(""),l=we.map(r=>`<option value="${r}">${r.charAt(0).toUpperCase()+r.slice(1)}</option>`).join(""),a=_e.map(r=>`<option value="${r}">${r.charAt(0).toUpperCase()+r.slice(1)}</option>`).join(""),o=$e.map(r=>`<option value="${r}">${r.charAt(0).toUpperCase()+r.slice(1)}</option>`).join(""),s=Ee.map(r=>`<option value="${r}">${r.charAt(0).toUpperCase()+r.slice(1)}</option>`).join("");return`
  ...
  `}async function le(e,{toast:n}){const{data:t,error:i}=await f.from("users").select("id,pseudo,club_name,credits,level,wins,draws,losses,trophies_top1,trophies_top2,trophies_top3,is_admin,created_at").order("created_at",{ascending:!1});if(i){e.innerHTML=`<p style="color:red">${i.message}</p>`;return}e.innerHTML=`
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px">
  ...
  `;const c=t||[];l(c),document.getElementById("search-users").addEventListener("input",a=>{const o=a.target.value.toLowerCase();l(c.filter(s=>{var r;return s.pseudo.toLowerCase().includes(o)||((r=s.club_name)==null?void 0:r.toLowerCase().includes(o))}))});function l(a){document.getElementById("users-tbody").innerHTML=a.map(o=>`
    ...
    `).join(""),document.querySelectorAll("[data-toggle-admin]").forEach(o=>{o.addEventListener("click",async()=>{const s=o.dataset.isAdmin!=="true";if(!confirm(`Voulez-vous ${s?"rendre admin":"retirer le rôle admin"} de ce Manager ?`))return;const{error:d}=await f.from("users").update({is_admin:s}).eq("id",o.dataset.toggleAdmin);d?n(d.message,"error"):(n("Rôle mis à jour ✅","success"),le(e,{toast:n}))})})}}async function de(e,{toast:n}){const{data:t,error:i}=await f.from("market_listings").select(`
    ...
    <div class="card-panel">
      <div class="table-wrap">
        <table>
          ...
          <tbody>
            ${c.map(o=>{var b,u,m,g;const s=(b=o.card)==null?void 0:b.player,r=s?`${s.firstname} ${s.surname_encoded}`:((u=o.card)==null?void 0:u.card_type)||"—",d={active:"#1A6B3C",sold:"#D4A017",cancelled:"#aaa"};return`<tr>
                <td><b>${r}</b>${s?`<div style="font-size:10px;color:#999">${s.rarity} · ${s.job}</div>`:""}</td>
                <td style="font-size:12px">${((m=o.seller)==null?void 0:m.pseudo)||"—"}</td>
  ...
  `,document.querySelectorAll("[data-cancel]").forEach(o=>{o.addEventListener("click",async()=>{if(!confirm("Annuler cette annonce ?"))return;const{error:s}=await f.from("market_listings").update({status:"cancelled"}).eq("id",o.dataset.cancel);s?n(s.message,"error"):(n("Annonce annulée","success"),de(e,{toast:n}))})})}async function Ce(e,{toast:n}){e.innerHTML=`
  ...
  `,document.getElementById("export-clubs-template").addEventListener("click",()=>{S("clubs_template.csv",`real_name,encoded_name,country_code,logo_url
Paris Saint-Germain,PARIS FC,FR,
Real Madrid,MADRID FC,ES,
`)}),document.getElementById("export-players-template").addEventListener("click",()=>{S("players_template.csv",`firstname,surname_real,surname_encoded,country_code,club_encoded_name,job,job2,note_g,note_d,note_m,note_a,rarity,note_min,note_max,skin,hair,hair_length,sell_price
...
`}),S("players_export.csv",c),n(`${t.length} joueurs exportés`,"success")}),document.getElementById("import-clubs").addEventListener("change",async t=>{const i=t.target.files[0];if(!i)return;const c=document.getElementById("clubs-import-result");c.innerHTML='<span style="color:var(--gray-600)">Import en cours...</span>';try{const l=await i.text(),a=oe(l);if(a.length===0){c.innerHTML='<span style="color:#c0392b">Fichier vide</span>';return}let o=0,s=0;const r=[];for(const d of a){if(!d.real_name||!d.encoded_name||!d.country_code){s++,r.push(`Ligne ignorée (champs manquants): ${d.real_name||"?"}`);continue}const b={real_name:d.real_name,encoded_name:d.encoded_name.toUpperCase(),country_code:d.country_code.toUpperCase().slice(0,2),logo_url:d.logo_url||null},{error:u}=await f.from("clubs").upsert(b,{onConflict:"encoded_name"});u?(s++,r.push(`${d.encoded_name}: ${u.message}`)):o++}c.innerHTML=`<div style="color:var(--green)">✅ ${o} clubs importés</div>
        ${s>0?`<div style="color:#c0392b">❌ ${s} erreurs</div><div style="font-size:11px;color:var(--gray-600);max-height:100px;overflow-y:auto">${r.slice(0,10).join("<br>")}</div>`:""}`,n(`${o} clubs importés`,"success")}catch(l){c.innerHTML=`<span style="color:#c0392b">Erreur : ${l.message}</span>`}t.target.value=""}),document.getElementById("import-players").addEventListener("change",async t=>{const i=t.target.files[0];if(!i)return;const c=document.getElementById("players-import-result");c.innerHTML='<span style="color:var(--gray-600)">Import en cours...</span>';try{const l=await i.text(),a=oe(l);if(a.length===0){c.innerHTML='<span style="color:#c0392b">Fichier vide</span>';return}const{data:o}=await f.from("clubs").select("id,encoded_name"),s={};(o||[]).forEach(p=>{s[p.encoded_name.toUpperCase()]=p.id});let r=0,d=0;const b=[],u=["GK","DEF","MIL","ATT"],m=["normal","pepite","papyte","legende"],g=["blanc","metisse","typ","noir"],y=["blond","marron","noir","chauve"],x=["rase","court","milong","long"];for(const p of a){if(!p.firstname||!p.surname_real||!p.country_code||!p.job){d++,b.push(`Ligne ignorée (champs requis manquants): ${p.firstname||"?"}`);continue}if(!u.includes(p.job)){d++,b.push(`${p.firstname}: job invalide "${p.job}"`);continue}const E=p.club_encoded_name&&s[p.club_encoded_name.toUpperCase()]||null,w={firstname:p.firstname,surname_real:p.surname_real,surname_encoded:p.surname_encoded||T(p.surname_real),country_code:p.country_code.toUpperCase().slice(0,2),club_id:E,job:p.job,job2:u.includes(p.job2)?p.job2:null,note_g:parseInt(p.note_g)||0,note_d:parseInt(p.note_d)||0,note_m:parseInt(p.note_m)||0,note_a:parseInt(p.note_a)||0,rarity:m.includes(p.rarity)?p.rarity:"normal",note_min:p.note_min!==""&&p.note_min!=null?parseInt(p.note_min):null,note_max:p.note_max!==""&&p.note_max!=null?parseInt(p.note_max):null,skin:g.includes(p.skin)?p.skin:"blanc",hair:y.includes(p.hair)?p.hair:"noir",hair_length:x.includes(p.hair_length)?p.hair_length:"court",sell_price:parseInt(p.sell_price)||0},{error:v}=await f.from("players").insert(w);v?(d++,b.push(`${p.firstname} ${p.surname_real}: ${v.message}`)):r++}c.innerHTML=`<div style="color:var(--green)">✅ ${r} joueurs importés</div>
...
`)?'"'+n.replace(/"/g,'""')+'"':n}function S(e,n){const t=new Blob(["\uFEFF"+n],{type:"text/csv;charset=utf-8;"}),i=URL.createObjectURL(t),c=document.createElement("a");c.href=i,c.download=e,c.click(),URL.revokeObjectURL(i)}function oe(e){e=e.replace(/^\uFEFF/,"").replace(/\r\n/g,`
`).replace(/\r/g,`
`);const n=e.split(`
`).filter(a=>a.trim());if(n.length<2)return[];const t=n[0],i=t.split(";").length>t.split(",").length?";":",",c=ie(t,i).map(a=>a.trim().replace(/^\uFEFF/,"").toLowerCase()),l=[];for(let a=1;a<n.length;a++){if(!n[a].trim())continue;const o=ie(n[a],i),s={};c.forEach((r,d)=>{s[r]=(o[d]||"").trim()}),!Object.values(s).every(r=>!r)&&l.push(s)}return l}function ie(e,n=","){const t=[];let i="",c=!1;for(let l=0;l<e.length;l++){const a=e[l];c?a==='"'?e[l+1]==='"'?(i+='"',l++):c=!1:i+=a:a==='"'?c=!0:a===n?(t.push(i),i=""):i+=a}return t.push(i),t}const je={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};async function Te(e,{toast:n}){const t=Object.keys(P);let i=t[0];const{data:c}=await f.from("formation_links_overrides").select("formation, links"),l={};(c||[]).forEach(d=>{l[d.formation]=d.links});let a=new Set;function o(d,b){return[d,b].sort().join("-")}function s(d){const b=l[d]||P[d]||[];a=new Set(b.map(([u,m])=>o(u,m)))}s(i),e.innerHTML=`
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;gap:12px;flex-wrap:wrap">
  ...
  `;function r(){const d=me[i]||{},b=J(i),u=320,m=400,g=22;function y(v){const h=d[v];return h?{x:h.x*u,y:h.y*m}:null}let x=`<svg width="${u}" height="${m}" viewBox="0 0 ${u} ${m}" xmlns="http://www.w3.org/2000/svg" style="display:block">`;b.forEach(([v,h],I)=>{const _=y(v),L=y(h);if(!_||!L)return;const B=o(v,h),A=a.has(B),$=A?"#3b82f6":"#999",k=A?.95:.35,N=A?4:3;x+=`<line x1="${_.x}" y1="${_.y}" x2="${L.x}" y2="${L.y}"
        stroke="transparent" stroke-width="14" class="link-hit" data-key="${B}" style="cursor:pointer"/>`,x+=`<line x1="${_.x}" y1="${_.y}" x2="${L.x}" y2="${L.y}"
        stroke="${$}" stroke-width="${N}" stroke-dasharray="${A?"none":"4,3"}"
        opacity="${k}" pointer-events="none" data-line-key="${B}"/>`});for(const v of Object.keys(d)){const h=y(v);if(!h)continue;const I=v.replace(/\d+/,""),_=je[I]||"#555";x+=`<circle cx="${h.x}" cy="${h.y}" r="${g}" fill="${_}" stroke="rgba(255,255,255,0.7)" stroke-width="2" pointer-events="none"/>`,x+=`<text x="${h.x}" y="${h.y+4}" text-anchor="middle" font-size="10" font-weight="900" fill="white" font-family="Arial Black,Arial" pointer-events="none">${v}</text>`}x+="</svg>",document.getElementById("field-wrap").innerHTML=x;const p=document.getElementById("links-list");p.innerHTML=b.map(([v,h])=>{const I=o(v,h),_=a.has(I);return`
        <button class="link-toggle" data-key="${I}" style="
          display:flex;justify-content:space-between;align-items:center;
          padding:6px 10px;border-radius:8px;font-size:12px;font-weight:600;cursor:pointer;
          border:1.5px solid ${_?"#3b82f6":"#ddd"};
          background:${_?"#eaf2ff":"#fafafa"};
          color:${_?"#1d4ed8":"#888"}">
          <span>${v} ↔ ${h}</span>
          <span>${_?"● Actif":"○ Inactif"}</span>
        </button>`}).join("");const E=e.querySelector('div[style*="font-weight:700;font-size:13px;margin-bottom:8px"]');E&&(E.textContent=`Liens (${a.size}/${b.length} actifs)`);function w(v){a.has(v)?a.delete(v):a.add(v),r()}e.querySelectorAll(".link-hit").forEach(v=>{v.addEventListener("click",()=>w(v.dataset.key))}),e.querySelectorAll(".link-toggle").forEach(v=>{v.addEventListener("click",()=>w(v.dataset.key))})}r(),document.getElementById("formation-select").addEventListener("change",d=>{i=d.target.value,s(i),r()}),document.getElementById("reset-btn").addEventListener("click",()=>{const d=P[i]||[];a=new Set(d.map(([b,u])=>o(b,u))),r(),n("Liens réinitialisés aux valeurs par défaut (non sauvegardé)","info")}),document.getElementById("save-btn").addEventListener("click",async()=>{const b=J(i).filter(([m,g])=>a.has(o(m,g))),{error:u}=await f.from("formation_links_overrides").upsert({formation:i,links:b,updated_at:new Date().toISOString()});if(u){n(u.message,"error");return}l[i]=b,n(`Liens enregistrés pour ${i} (${b.length} actifs)`,"success")})}const Se=[{value:"player",label:"Joueur"},{value:"formation",label:"Formation"},{value:"game_changer",label:"Game Changer"},{value:"game_helper",label:"Game Helper"}],ze=[{value:"",label:"Toutes"},{value:"normal",label:"Normal"},{value:"pépite",label:"Pépite"},{value:"papyte",label:"Papyte"},{value:"légende",label:"Légende"}],Re=[{value:"credits",label:"Crédits"},{value:"pub",label:"Publicité"},{value:"free",label:"Gratuit"}];async function Me(e){e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">Chargement...</div>';const{data:n}=await f.from("booster_configs").select("*").order("sort_order");let t=null,i=[];async function c(o){const{data:s}=await f.from("booster_drop_rates").select("*").eq("booster_id",o).order("sort_order");return s||[]}async function l(){const o=t?(n||[]).find(r=>r.id===t):null;o&&(i=await c(o.id));const s=i.reduce((r,d)=>r+Number(d.percentage||0),0);e.innerHTML=`
    <div style="display:grid;grid-template-columns:300px 1fr;gap:0;height:calc(100vh - 60px)">
      ...
      <div style="border-right:1px solid #e0e0e0;overflow-y:auto;background:#fff">
        ...
        <div class="booster-row" data-id="${r.id}"
          ...
          ${r.image_url?`<img src="${f.storage.from("assets").getPublicUrl("boosters/"+r.image_url).data.publicUrl}" style="width:36px;height:36px;object-fit:contain;border-radius:6px">`:'<div style="width:36px;height:36px;background:#eee;border-radius:6px;display:flex;align-items:center;justify-content:center;font-size:18px">📦</div>'}
          ...
          <div style="background:#fff;border-radius:10px;padding:16px;margin-bottom:16px;display:grid;grid-template-columns:1fr 1fr;gap:14px">
            ...
            <div>
              ...
              <div style="display:flex;gap:8px;align-items:center;margin-top:4px">
                <input id="f-image-url" value="${o.image_url||""}" placeholder="nom-du-fichier.png" style="flex:1;padding:8px;border:1px solid #ddd;border-radius:6px;font-size:13px">
              ...
              ${o.image_url?`<img src="${f.storage.from("assets").getPublicUrl("boosters/"+o.image_url).data.publicUrl}" style="height:50px;margin-top:8px;border-radius:6px;object-fit:contain">`:""}
                  ...
                  <tr style="border-bottom:1px solid #f0f0f0" data-rate-idx="${d}">
                    <td style="padding:6px 10px">
                      <select class="rate-type" data-idx="${d}" style="padding:5px;border:1px solid #ddd;border-radius:4px;font-size:12px;min-width:110px">
                        ${Se.map(b=>`<option value="${b.value}" ${r.card_type===b.value?"selected":""}>${b.label}</option>`).join("")}
                      </select>
                    </td>
                    <td style="padding:6px 10px">
    ...
    </div>`,a()}function a(){var o,s,r,d,b,u;e.querySelectorAll(".booster-row").forEach(m=>{m.addEventListener("click",g=>{g.target.closest(".btn-delete")||(t=m.dataset.id,l())})}),(o=document.getElementById("btn-new"))==null||o.addEventListener("click",async()=>{const m=prompt("Nom du nouveau booster :");if(!(m!=null&&m.trim()))return;const{data:g,error:y}=await f.from("booster_configs").insert({name:m.trim(),price_type:"credits",price_credits:1e3,card_count:5}).select().single();if(y){alert(y.message);return}n.push(g),t=g.id,l()}),e.querySelectorAll(".btn-delete").forEach(m=>{m.addEventListener("click",async g=>{if(g.stopPropagation(),!confirm("Supprimer ce booster ?"))return;const y=m.dataset.id;await f.from("booster_configs").delete().eq("id",y);const x=n.findIndex(p=>p.id===y);x>-1&&n.splice(x,1),t===y&&(t=null),l()})}),(s=document.getElementById("btn-cancel"))==null||s.addEventListener("click",()=>{t=null,l()}),(r=document.getElementById("f-price-type"))==null||r.addEventListener("change",m=>{const g=document.getElementById("credits-field");g&&(g.style.cssText=m.target.value!=="credits"?"opacity:0.4;pointer-events:none":"")}),(d=document.getElementById("f-image-file"))==null||d.addEventListener("change",async m=>{const g=m.target.files[0];if(!g)return;const y=document.getElementById("upload-status");y.textContent="⏳ Upload en cours...";const x=g.name.split(".").pop(),p=`booster-${t}.${x}`,{error:E}=await f.storage.from("assets").upload(`boosters/${p}`,g,{upsert:!0,contentType:g.type});if(E){y.textContent="❌ "+E.message;return}document.getElementById("f-image-url").value=p,y.textContent="✅ Uploadé : "+p}),(b=document.getElementById("btn-add-rate"))==null||b.addEventListener("click",()=>{i.push({id:null,booster_id:t,card_type:"player",rarity:"normal",note_min:1,note_max:10,percentage:10,sort_order:i.length}),l()}),e.querySelectorAll(".btn-del-rate").forEach(m=>{m.addEventListener("click",()=>{i.splice(Number(m.dataset.idx),1),l()})}),(u=document.getElementById("btn-save"))==null||u.addEventListener("click",async()=>{var E,w,v,h,I,_,L,B,A;const m=(n||[]).find($=>$.id===t);if(!m)return;const g=[];e.querySelectorAll("tr[data-rate-idx]").forEach($=>{var q,H,G,V,K;const k=Number($.dataset.rateIdx);i[k];const N=((q=$.querySelector(".rate-type"))==null?void 0:q.value)||"player",ce=((H=$.querySelector(".rate-rarity"))==null?void 0:H.value)||"",O=(G=$.querySelector(".rate-note-min"))==null?void 0:G.value,U=(V=$.querySelector(".rate-note-max"))==null?void 0:V.value,ue=(K=$.querySelector(".rate-pct"))==null?void 0:K.value;g.push({booster_id:t,card_type:N,rarity:ce||null,note_min:O?Number(O):null,note_max:U?Number(U):null,percentage:Number(ue)||0,sort_order:k})});const y=g.reduce(($,k)=>$+k.percentage,0);if(Math.abs(y-100)>.5){alert(`La somme des pourcentages doit faire 100%. Actuellement : ${y.toFixed(1)}%`);return}const x={name:((w=(E=document.getElementById("f-name"))==null?void 0:E.value)==null?void 0:w.trim())||m.name,image_url:((h=(v=document.getElementById("f-image-url"))==null?void 0:v.value)==null?void 0:h.trim())||null,price_type:(I=document.getElementById("f-price-type"))==null?void 0:I.value,price_credits:Number((_=document.getElementById("f-price-credits"))==null?void 0:_.value)||0,card_count:Number((L=document.getElementById("f-card-count"))==null?void 0:L.value)||5,is_active:(B=document.getElementById("f-active"))==null?void 0:B.checked,sort_order:Number((A=document.getElementById("f-sort"))==null?void 0:A.value)||0},{error:p}=await f.from("booster_configs").update(x).eq("id",t);if(p){alert(p.message);return}if(Object.assign(m,x),await f.from("booster_drop_rates").delete().eq("booster_id",t),g.length){const{error:$}=await f.from("booster_drop_rates").insert(g);if($){alert($.message);return}}i=g,alert("✅ Booster enregistré !"),l()})}l()}function Ne(e,n="info"){const t=document.getElementById("toast");t.textContent=e,t.className=`show ${n}`,clearTimeout(t._t),t._t=setTimeout(()=>{t.className=""},3e3)}function Pe(e,n,t=""){document.getElementById("modal-title").textContent=e,document.getElementById("modal-body").innerHTML=n,document.getElementById("modal-footer").innerHTML=t,document.getElementById("modal-overlay").classList.remove("hidden")}function D(){document.getElementById("modal-overlay").classList.add("hidden")}const ae={dashboard:{title:"Dashboard",fn:be},players:{title:"Joueurs & Cartes",fn:ye},clubs:{title:"Clubs",fn:xe},"card-builder":{title:"Card Builder",fn:Le},users:{title:"Managers",fn:le},market:{title:"Marché des transferts",fn:de},"import-export":{title:"Import / Export CSV",fn:Ce},formations:{title:"Formations & Liens",fn:Te},"boosters-config":{title:"Boosters",fn:Me}};async function F(e){document.querySelectorAll(".admin-sidebar nav a").forEach(i=>{i.classList.toggle("active",i.dataset.page===e)});const n=ae[e]||ae.dashboard;document.getElementById("page-title").textContent=n.title;const t=document.getElementById("page-content");t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">Chargement…</div>';try{await n.fn(t,{toast:Ne,openModal:Pe,closeModal:D,navigate:F})}catch(i){t.innerHTML=`<div style="padding:40px;color:#c0392b">Erreur: ${i.message}</div>`,console.error(i)}}async function De(){const{data:{session:e}}=await f.auth.getSession();e&&await re(e.user),document.getElementById("auth-btn").addEventListener("click",async()=>{const n=document.getElementById("auth-email").value.trim(),t=document.getElementById("auth-password").value,i=document.getElementById("auth-error");if(i.textContent="",!n||!t){i.textContent="Remplissez tous les champs.";return}const{data:c,error:l}=await f.auth.signInWithPassword({email:n,password:t});if(l){i.textContent=l.message;return}await re(c.user)}),document.getElementById("auth-password").addEventListener("keydown",n=>{n.key==="Enter"&&document.getElementById("auth-btn").click()}),document.getElementById("logout-btn").addEventListener("click",async()=>{await f.auth.signOut(),document.getElementById("auth-screen").style.display="flex",document.getElementById("admin-app").style.display="none"}),document.getElementById("modal-close").addEventListener("click",D),document.getElementById("modal-overlay").addEventListener("click",n=>{n.target===n.currentTarget&&D()}),document.querySelectorAll(".admin-sidebar nav a").forEach(n=>{n.addEventListener("click",t=>{t.preventDefault(),F(n.dataset.page)})})}async function re(e){const{data:n,error:t}=await f.from("users").select("*").eq("id",e.id).single(),i=document.getElementById("auth-error");if(t||!n){i.textContent="Profil introuvable.";return}if(!n.is_admin){i.textContent="Accès refusé. Vous n'êtes pas administrateur.",await f.auth.signOut();return}document.getElementById("auth-screen").style.display="none",document.getElementById("admin-app").style.display="flex",document.getElementById("admin-user").textContent=`Admin: ${n.pseudo}`,F("dashboard")}De();
