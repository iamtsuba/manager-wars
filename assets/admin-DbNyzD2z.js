import{s as x,g as ve,F as Y,a as te,b as xe}from"./formation-links-CDBKB_z4.js";async function ye(e){const[{count:n},{count:t},{count:o},{count:m},{count:s}]=await Promise.all([x.from("players").select("*",{count:"exact",head:!0}),x.from("clubs").select("*",{count:"exact",head:!0}),x.from("users").select("*",{count:"exact",head:!0}),x.from("cards").select("*",{count:"exact",head:!0}),x.from("matches").select("*",{count:"exact",head:!0})]);e.innerHTML=`
  ...
  </div>`}const ne={ATT:"#1A6B3C",MIL:"#D4A017",DEF:"#bb2020",GK:"#111111"},ge={normal:"#cccccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"},he={MA:"MAROC",FR:"FRANCE",AR:"ARGENTINE",PT:"PORTUGAL",BR:"BRESIL",ES:"ESPAGNE",DE:"ALLEMAGNE",GB:"ANGLETERRE",IT:"ITALIE",CM:"CAMEROUN",SN:"SENEGAL",NG:"NIGERIA",DK:"DANEMARK",NL:"PAYS-BAS",BE:"BELGIQUE",CI:"CÔTE D'IVOIRE",MA:"MAROC",AL:"ALBANIE",HR:"CROATIE",RS:"SERBIE",TR:"TURQUIE",MA:"MAROC"};function ie(e,n,t=52){const o=t===52,m=o?"26,3 31.5,18 48,18 35,29 39.5,46 26,36 12.5,46 17,29 4,18 20.5,18":"16,2 19.5,11 30,11 22,17.5 25,27 16,21.5 7,27 10,17.5 2,11 12.5,11",s=o?26:16,i=o?32:20,r=o?16:9,l=o?50:31;return`<svg width="${t}" height="${l}" viewBox="0 0 ${t} ${l}" style="display:block">
    <polygon points="${m}" fill="${e}" stroke="#0005" stroke-width="${o?2:1.5}"/>
    <text x="${s}" y="${i}" text-anchor="middle" font-size="${r}" font-weight="900"
      fill="white" font-family="Arial Black,Arial">${n}</text>
  </svg>`}function _e(e,n={}){const{size:t="md",showNotes:o=!1,portraitUrl:m=null,clubLogoUrl:s=null}=n,i=e.rarity||"normal",r=ge[i],l=e.job||"ATT",c=e.job2,b=e.current_note??(l==="ATT"?e.note_a:l==="MIL"?e.note_m:l==="DEF"?e.note_d:e.note_g),d=c?c==="ATT"?e.note_a:c==="MIL"?e.note_m:c==="DEF"?e.note_d:e.note_g:null,a=ne[l],f=c?ne[c]:null,g=e.country_code||"",y=he[g]||g,_=t==="sm"?120:160,p=m?`<img src="${m}" alt="" style="width:100%;height:100%;object-fit:cover;object-position:center top;display:block">`:'<div style="width:100%;height:100%;background:#c5d8ec;display:flex;align-items:center;justify-content:center;font-size:32px;color:#8fa5be">👤</div>',$=s?`<img src="${s}" alt="" class="mw-club-logo">`:`<div class="mw-club-badge">${e.club_encoded_name||"CLUB"}</div>`,E=i==="legende"?'<div style="background:#f5c518;color:#7a5800;border-radius:3px;padding:1px 5px;font-size:6px;font-weight:800;font-family:Arial">LÉGENDE</div>':i==="pepite"?'<div style="background:#D4A017;color:#fff;border-radius:3px;padding:1px 5px;font-size:6px;font-weight:800;font-family:Arial">PÉPITE</div>':i==="papyte"?'<div style="background:#909090;color:#fff;border-radius:3px;padding:1px 5px;font-size:6px;font-weight:800;font-family:Arial">PAPYTE</div>':$;return`<div class="mw-card" style="background:${r};width:${_}px;${t==="sm"?"transform:scale(0.75);transform-origin:top left;":""}">
  ...
  </div>`}function G(e){const n={A:"E",E:"I",I:"O",O:"U",U:"A",a:"e",e:"i",i:"o",o:"u",u:"a"};return(e||"").replace(/[AEIOUaeiou]/g,t=>n[t]||t)}const oe={normal:"Normal",pepite:"Pépite",papyte:"Papyte",legende:"Légende"},$e={normal:"#aaa",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"};async function Ee(e,n){await V(e,n)}async function V(e,n){const{toast:t,openModal:o,closeModal:m}=n,[{data:s,error:i},{data:r}]=await Promise.all([x.from("players").select("*, clubs(encoded_name,logo_url)").order("surname_encoded"),x.from("clubs").select("id,encoded_name").order("encoded_name")]);if(i){e.innerHTML=`<p style="color:red">${i.message}</p>`;return}const l={};(r||[]).forEach(d=>{l[d.id]=d.encoded_name}),e.innerHTML=`
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;gap:12px;flex-wrap:wrap">
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        ...
        <select id="filter-rarity" style="width:130px">
          ...
          ${Object.entries(oe).map(([d,a])=>`<option value="${d}">${a}</option>`).join("")}
        </select>
  ...
  `,c(s);function c(d){document.getElementById("count-label").textContent=`${d.length} joueur(s)`,document.getElementById("players-tbody").innerHTML=d.map(a=>{var f;return`
      <tr>
        <td>
        ...
        <td>
          <span style="background:${$e[a.rarity]};color:#fff;padding:2px 6px;border-radius:3px;font-size:10px;font-weight:700">
            ${oe[a.rarity]||a.rarity}
          </span>
        </td>
        <td>
    ...
    `}).join(""),document.querySelectorAll("[data-edit]").forEach(a=>{a.addEventListener("click",()=>{const f=s.find(g=>g.id===a.dataset.edit);ae(f,r||[],{toast:t,openModal:o,closeModal:m,reload:()=>V(e,n)})})}),document.querySelectorAll("[data-del]").forEach(a=>{a.addEventListener("click",async()=>{if(!confirm("Supprimer ce joueur ? Les cartes associées resteront en jeu."))return;const{error:f}=await x.from("players").delete().eq("id",a.dataset.del);f?t(f.message,"error"):(t("Joueur supprimé","success"),V(e,n))})})}function b(){const d=document.getElementById("search-players").value.toLowerCase(),a=document.getElementById("filter-rarity").value,f=document.getElementById("filter-job").value;c(s.filter(g=>(!d||`${g.firstname} ${g.surname_encoded} ${g.surname_real}`.toLowerCase().includes(d))&&(!a||g.rarity===a)&&(!f||g.job===f)))}document.getElementById("search-players").addEventListener("input",b),document.getElementById("filter-rarity").addEventListener("change",b),document.getElementById("filter-job").addEventListener("change",b),document.getElementById("add-player-btn").addEventListener("click",()=>{ae(null,r||[],{toast:t,openModal:o,closeModal:m,reload:()=>V(e,n)})})}function ae(e,n,{toast:t,openModal:o,closeModal:m,reload:s}){const i=!!e,r='<option value="">— Club —</option>'+n.map(l=>`<option value="${l.id}" ${(e==null?void 0:e.club_id)===l.id?"selected":""}>${l.encoded_name}</option>`).join("");o(i?`Modifier ${e.firstname} ${e.surname_encoded}`:"Nouveau joueur",`<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
  ...
  `,m(K),document.getElementById("search-clubs").addEventListener("input",s=>{const i=s.target.value.toLowerCase();m(K.filter(r=>r.encoded_name.toLowerCase().includes(i)||r.real_name.toLowerCase().includes(i)))}),document.getElementById("add-club-btn").addEventListener("click",()=>{re(null,{toast:n,openModal:t,closeModal:o,reload:()=>J(e,{toast:n,openModal:t,closeModal:o})})});function m(s){document.getElementById("clubs-tbody").innerHTML=s.map(i=>`
      <tr>
        <td>
    ...
    `).join(""),document.querySelectorAll("[data-edit]").forEach(i=>{i.addEventListener("click",()=>{const r=K.find(l=>l.id===i.dataset.edit);re(r,{toast:n,openModal:t,closeModal:o,reload:()=>J(e,{toast:n,openModal:t,closeModal:o})})})}),document.querySelectorAll("[data-del]").forEach(i=>{i.addEventListener("click",async()=>{if(!confirm("Supprimer ce club ?"))return;const{error:r}=await x.from("clubs").delete().eq("id",i.dataset.del);r?n(r.message,"error"):(n("Club supprimé","success"),J(e,{toast:n,openModal:t,closeModal:o}))})})}}function re(e,{toast:n,openModal:t,closeModal:o,reload:m}){const s=!!e;t(s?"Modifier le club":"Nouveau club",`<div style="display:flex;flex-direction:column;gap:12px">
     ...
     <button class="btn btn-primary" id="m-save">${s?"Enregistrer":"Créer"}</button>`),document.getElementById("m-cancel").addEventListener("click",o),document.getElementById("auto-encode").addEventListener("click",()=>{const i=document.getElementById("m-real").value,r=i.split(" ")[0]||i;document.getElementById("m-encoded").value=r.toUpperCase()+" FC"}),document.getElementById("m-real").addEventListener("input",()=>{const i=document.getElementById("m-real").value;if(!document.getElementById("m-encoded").value){const r=i.split(" ")[0]||i;document.getElementById("m-encoded").value=r.toUpperCase()+" FC"}}),document.getElementById("m-save").addEventListener("click",async()=>{const i={real_name:document.getElementById("m-real").value.trim(),encoded_name:document.getElementById("m-encoded").value.trim().toUpperCase(),country_code:document.getElementById("m-country").value.trim().toUpperCase(),logo_url:document.getElementById("m-logo").value.trim()||null};if(!i.real_name||!i.encoded_name||!i.country_code){n("Remplissez tous les champs obligatoires","error");return}const{error:r}=s?await x.from("clubs").update(i).eq("id",e.id):await x.from("clubs").insert(i);if(r){n(r.message,"error");return}n(s?"Club modifié":"Club créé","success"),o(),m()})}const Ie=["blanc","metisse","typ","noir"],Le=["blond","marron","noir","chauve"],Ae=["rase","court","milong","long"],Te=["normal","pepite","papyte","legende"],se=["GK","DEF","MIL","ATT"],ze=[["MA","Maroc"],["FR","France"],["AR","Argentine"],["PT","Portugal"],["BR","Brésil"],["ES","Espagne"],["DE","Allemagne"],["GB","Angleterre"],["IT","Italie"],["CM","Cameroun"],["SN","Sénégal"],["NG","Nigéria"],["DK","Danemark"],["NL","Pays-Bas"],["BE","Belgique"],["CI","Côte d'Ivoire"],["AL","Albanie"],["HR","Croatie"],["RS","Serbie"],["TR","Turquie"],["MW","Malawi"]];async function Ce(e,{toast:n}){const{data:t}=await x.from("clubs").select("id,encoded_name,logo_url").order("encoded_name");e.innerHTML=Be(t||[]),je(e,t||[],n),N()}function le(){var o;const e=((o=document.getElementById("cb-surname-real"))==null?void 0:o.value)||"",n=G(e),t=document.getElementById("cb-surname-enc");t&&(t.value=n)}function N(){var b,d,a,f;const e=g=>{var y;return((y=document.getElementById(g))==null?void 0:y.value)||""},n={firstname:e("cb-firstname"),surname_encoded:e("cb-surname-enc")||G(e("cb-surname-real")),country_code:e("cb-country"),club_encoded_name:((d=(b=document.getElementById("cb-club"))==null?void 0:b.selectedOptions[0])==null?void 0:d.text)||"",job:e("cb-job"),job2:e("cb-job2")||null,note_g:parseInt(e("cb-note-g"))||0,note_d:parseInt(e("cb-note-d"))||0,note_m:parseInt(e("cb-note-m"))||0,note_a:parseInt(e("cb-note-a"))||0,rarity:e("cb-rarity"),skin:e("cb-skin"),hair:e("cb-hair"),hair_length:e("cb-hair-length")},t=ve(n.skin,n.hair,n.hair_length);e("cb-club");const o=document.getElementById("cb-club"),m=((f=(a=o==null?void 0:o.selectedOptions[0])==null?void 0:a.dataset)==null?void 0:f.logo)||null,s=_e(n,{portraitUrl:t,clubLogoUrl:m,showNotes:!0}),i=document.getElementById("card-preview");i&&(i.innerHTML=s);const r=e("cb-surname-real"),l=n.surname_encoded,c=document.getElementById("encode-summary");c&&r&&(c.textContent=`${e("cb-firstname")} ${r} → ${e("cb-firstname")} ${l}`)}function je(e,n,t){var o,m,s,i;e.querySelectorAll("input,select").forEach(r=>{r.addEventListener("input",N),r.addEventListener("change",N)}),(o=document.getElementById("cb-surname-real"))==null||o.addEventListener("input",()=>{le(),N()}),(m=document.getElementById("btn-encode"))==null||m.addEventListener("click",()=>{le(),N()}),(s=document.getElementById("btn-save-player"))==null||s.addEventListener("click",async()=>{await Se(n,t)}),(i=document.getElementById("btn-reset"))==null||i.addEventListener("click",()=>{e.querySelectorAll("input").forEach(r=>r.value=""),e.querySelectorAll("select").forEach(r=>r.selectedIndex=0),N()})}async function Se(e,n){const t=l=>{var c;return((c=document.getElementById(l))==null?void 0:c.value)||""},o=t("cb-surname-real").trim(),m=t("cb-surname-enc").trim()||G(o);if(!t("cb-firstname")||!o||!t("cb-country")||!t("cb-job")){n("Remplissez les champs obligatoires (prénom, nom, pays, poste)","error");return}const s=t("cb-club")||null,i={firstname:t("cb-firstname").trim(),surname_real:o,surname_encoded:m,country_code:t("cb-country"),club_id:s||null,job:t("cb-job"),job2:t("cb-job2")||null,note_g:parseInt(t("cb-note-g"))||0,note_d:parseInt(t("cb-note-d"))||0,note_m:parseInt(t("cb-note-m"))||0,note_a:parseInt(t("cb-note-a"))||0,rarity:t("cb-rarity")||"normal",note_min:parseInt(t("cb-note-min"))||null,note_max:parseInt(t("cb-note-max"))||null,skin:t("cb-skin")||"blanc",hair:t("cb-hair")||"noir",hair_length:t("cb-hair-length")||"court",sell_price:parseInt(t("cb-price"))||0},{error:r}=await x.from("players").insert(i);if(r){n("Erreur: "+r.message,"error");return}n(`✅ Joueur "${i.firstname} ${i.surname_encoded}" enregistré !`,"success")}function Be(e){const n=ze.map(([c,b])=>`<option value="${c}">${b} (${c})</option>`).join(""),t='<option value="">— Sélectionner un club —</option>'+e.map(c=>`<option value="${c.id}" data-logo="${c.logo_url||""}">${c.encoded_name}</option>`).join(""),o=se.map(c=>`<option value="${c}">${c}</option>`).join(""),m='<option value="">Aucun</option>'+se.map(c=>`<option value="${c}">${c}</option>`).join(""),s=Te.map(c=>`<option value="${c}">${c.charAt(0).toUpperCase()+c.slice(1)}</option>`).join(""),i=Ie.map(c=>`<option value="${c}">${c.charAt(0).toUpperCase()+c.slice(1)}</option>`).join(""),r=Le.map(c=>`<option value="${c}">${c.charAt(0).toUpperCase()+c.slice(1)}</option>`).join(""),l=Ae.map(c=>`<option value="${c}">${c.charAt(0).toUpperCase()+c.slice(1)}</option>`).join("");return`
      ...
      <div class="card-panel">
        ...
        <div style="margin-top:8px;display:flex;gap:8px">
          ${["normal","pepite","papyte","legende"].map(c=>`
            <div style="background:${ge[c]};color:#fff;padding:3px 8px;border-radius:4px;font-size:11px;font-weight:700">
              ${c.toUpperCase()}
            </div>`).join("")}
        </div>
      </div>
  ...
  `}async function be(e,{toast:n}){const{data:t,error:o}=await x.from("users").select("id,pseudo,club_name,credits,level,wins,draws,losses,trophies_top1,trophies_top2,trophies_top3,is_admin,created_at").order("created_at",{ascending:!1});if(o){e.innerHTML=`<p style="color:red">${o.message}</p>`;return}e.innerHTML=`
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px">
  ...
  `;const m=t||[];s(m),document.getElementById("search-users").addEventListener("input",i=>{const r=i.target.value.toLowerCase();s(m.filter(l=>{var c;return l.pseudo.toLowerCase().includes(r)||((c=l.club_name)==null?void 0:c.toLowerCase().includes(r))}))});function s(i){document.getElementById("users-tbody").innerHTML=i.map(r=>`
      <tr>
        <td><b>${r.pseudo}</b></td>
    ...
    `).join(""),document.querySelectorAll("[data-toggle-admin]").forEach(r=>{r.addEventListener("click",async()=>{const l=r.dataset.isAdmin!=="true";if(!confirm(`Voulez-vous ${l?"rendre admin":"retirer le rôle admin"} de ce Manager ?`))return;const{error:b}=await x.from("users").update({is_admin:l}).eq("id",r.dataset.toggleAdmin);b?n(b.message,"error"):(n("Rôle mis à jour ✅","success"),be(e,{toast:n}))})})}}async function fe(e,{toast:n}){const{data:t,error:o}=await x.from("market_listings").select(`
    ...
    `).order("listed_at",{ascending:!1}).limit(100);if(o){e.innerHTML=`<p style="color:red">${o.message}</p>`;return}const m=t||[],s=m.filter(r=>r.status==="active").length,i=m.filter(r=>r.status==="sold").length;e.innerHTML=`
    ...
    <div class="card-panel">
      <div class="table-wrap">
        <table>
          ...
          <tbody>
            ${m.map(r=>{var d,a,f,g;const l=(d=r.card)==null?void 0:d.player,c=l?`${l.firstname} ${l.surname_encoded}`:((a=r.card)==null?void 0:a.card_type)||"—",b={active:"#1A6B3C",sold:"#D4A017",cancelled:"#aaa"};return`<tr>
  ...
  `,document.querySelectorAll("[data-cancel]").forEach(r=>{r.addEventListener("click",async()=>{if(!confirm("Annuler cette annonce ?"))return;const{error:l}=await x.from("market_listings").update({status:"cancelled"}).eq("id",r.dataset.cancel);l?n(l.message,"error"):(n("Annonce annulée","success"),fe(e,{toast:n}))})})}async function Re(e,{toast:n}){e.innerHTML=`
...
`)}),document.getElementById("export-clubs").addEventListener("click",async()=>{const{data:t,error:o}=await x.from("clubs").select("real_name,encoded_name,country_code,logo_url").order("encoded_name");if(o){n(o.message,"error");return}if(!t||t.length===0){n("Aucun club à exporter","info");return}let m=`real_name,encoded_name,country_code,logo_url
`;t.forEach(s=>{m+=[s.real_name,s.encoded_name,s.country_code,s.logo_url||""].map(de).join(",")+`
`}),H("clubs_export.csv",m),n(`${t.length} clubs exportés`,"success")}),document.getElementById("export-players").addEventListener("click",async()=>{const{data:t,error:o}=await x.from("players").select("firstname,surname_real,surname_encoded,country_code,job,job2,note_g,note_d,note_m,note_a,rarity,note_min,note_max,skin,hair,hair_length,sell_price,clubs(encoded_name)").order("surname_encoded");if(o){n(o.message,"error");return}if(!t||t.length===0){n("Aucun joueur à exporter","info");return}let m=`firstname,surname_real,surname_encoded,country_code,club_encoded_name,job,job2,note_g,note_d,note_m,note_a,rarity,note_min,note_max,skin,hair,hair_length,sell_price
`;t.forEach(s=>{var i;m+=[s.firstname,s.surname_real,s.surname_encoded,s.country_code,((i=s.clubs)==null?void 0:i.encoded_name)||"",s.job,s.job2||"",s.note_g,s.note_d,s.note_m,s.note_a,s.rarity,s.note_min??"",s.note_max??"",s.skin,s.hair,s.hair_length,s.sell_price].map(de).join(",")+`
`}),H("players_export.csv",m),n(`${t.length} joueurs exportés`,"success")}),document.getElementById("import-clubs").addEventListener("change",async t=>{const o=t.target.files[0];if(!o)return;const m=document.getElementById("clubs-import-result");m.innerHTML='<span style="color:var(--gray-600)">Import en cours...</span>';try{const s=await o.text(),i=ce(s);if(i.length===0){m.innerHTML='<span style="color:#c0392b">Fichier vide</span>';return}let r=0,l=0;const c=[];for(const b of i){if(!b.real_name||!b.encoded_name||!b.country_code){l++,c.push(`Ligne ignorée (champs manquants): ${b.real_name||"?"}`);continue}const d={real_name:b.real_name,encoded_name:b.encoded_name.toUpperCase(),country_code:b.country_code.toUpperCase().slice(0,2),logo_url:b.logo_url||null},{error:a}=await x.from("clubs").upsert(d,{onConflict:"encoded_name"});a?(l++,c.push(`${b.encoded_name}: ${a.message}`)):r++}m.innerHTML=`<div style="color:var(--green)">✅ ${r} clubs importés</div>
        ${l>0?`<div style="color:#c0392b">❌ ${l} erreurs</div><div style="font-size:11px;color:var(--gray-600);max-height:100px;overflow-y:auto">${c.slice(0,10).join("<br>")}</div>`:""}`,n(`${r} clubs importés`,"success")}catch(s){m.innerHTML=`<span style="color:#c0392b">Erreur : ${s.message}</span>`}t.target.value=""}),document.getElementById("import-players").addEventListener("change",async t=>{const o=t.target.files[0];if(!o)return;const m=document.getElementById("players-import-result");m.innerHTML='<span style="color:var(--gray-600)">Import en cours...</span>';try{const s=await o.text(),i=ce(s);if(i.length===0){m.innerHTML='<span style="color:#c0392b">Fichier vide</span>';return}const{data:r}=await x.from("clubs").select("id,encoded_name"),l={};(r||[]).forEach(p=>{l[p.encoded_name.toUpperCase()]=p.id});let c=0,b=0;const d=[],a=["GK","DEF","MIL","ATT"],f=["normal","pepite","papyte","legende"],g=["blanc","metisse","typ","noir"],y=["blond","marron","noir","chauve"],_=["rase","court","milong","long"];for(const p of i){if(!p.firstname||!p.surname_real||!p.country_code||!p.job){b++,d.push(`Ligne ignorée (champs requis manquants): ${p.firstname||"?"}`);continue}if(!a.includes(p.job)){b++,d.push(`${p.firstname}: job invalide "${p.job}"`);continue}const $=p.club_encoded_name&&l[p.club_encoded_name.toUpperCase()]||null,E={firstname:p.firstname,surname_real:p.surname_real,surname_encoded:p.surname_encoded||G(p.surname_real),country_code:p.country_code.toUpperCase().slice(0,2),club_id:$,job:p.job,job2:a.includes(p.job2)?p.job2:null,note_g:parseInt(p.note_g)||0,note_d:parseInt(p.note_d)||0,note_m:parseInt(p.note_m)||0,note_a:parseInt(p.note_a)||0,rarity:f.includes(p.rarity)?p.rarity:"normal",note_min:p.note_min!==""&&p.note_min!=null?parseInt(p.note_min):null,note_max:p.note_max!==""&&p.note_max!=null?parseInt(p.note_max):null,skin:g.includes(p.skin)?p.skin:"blanc",hair:y.includes(p.hair)?p.hair:"noir",hair_length:_.includes(p.hair_length)?p.hair_length:"court",sell_price:parseInt(p.sell_price)||0},{error:u}=await x.from("players").insert(E);u?(b++,d.push(`${p.firstname} ${p.surname_real}: ${u.message}`)):c++}m.innerHTML=`<div style="color:var(--green)">✅ ${c} joueurs importés</div>
        ${b>0?`<div style="color:#c0392b">❌ ${b} erreurs</div><div style="font-size:11px;color:var(--gray-600);max-height:100px;overflow-y:auto">${d.slice(0,10).join("<br>")}</div>`:""}`,n(`${c} joueurs importés`,"success")}catch(s){m.innerHTML=`<span style="color:#c0392b">Erreur : ${s.message}</span>`}t.target.value=""})}function de(e){const n=String(e??"");return n.includes(",")||n.includes('"')||n.includes(`
`)?'"'+n.replace(/"/g,'""')+'"':n}function H(e,n){const t=new Blob(["\uFEFF"+n],{type:"text/csv;charset=utf-8;"}),o=URL.createObjectURL(t),m=document.createElement("a");m.href=o,m.download=e,m.click(),URL.revokeObjectURL(o)}function ce(e){e=e.replace(/^\uFEFF/,"").replace(/\r\n/g,`
`).replace(/\r/g,`
`);const n=e.split(`
`).filter(i=>i.trim());if(n.length<2)return[];const t=n[0],o=t.split(";").length>t.split(",").length?";":",",m=pe(t,o).map(i=>i.trim().replace(/^\uFEFF/,"").toLowerCase()),s=[];for(let i=1;i<n.length;i++){if(!n[i].trim())continue;const r=pe(n[i],o),l={};m.forEach((c,b)=>{l[c]=(r[b]||"").trim()}),!Object.values(l).every(c=>!c)&&s.push(l)}return s}function pe(e,n=","){const t=[];let o="",m=!1;for(let s=0;s<e.length;s++){const i=e[s];m?i==='"'?e[s+1]==='"'?(o+='"',s++):m=!1:o+=i:i==='"'?m=!0:i===n?(t.push(o),o=""):o+=i}return t.push(o),t}const Me={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};async function Ne(e,{toast:n}){const t=Object.keys(Y);let o=t[0];const{data:m}=await x.from("formation_links_overrides").select("formation, links"),s={};(m||[]).forEach(b=>{s[b.formation]=b.links});let i=new Set;function r(b,d){return[b,d].sort().join("-")}function l(b){const d=s[b]||Y[b]||[];i=new Set(d.map(([a,f])=>r(a,f)))}l(o),e.innerHTML=`
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;gap:12px;flex-wrap:wrap">
  ...
  `;function c(){const b=xe[o]||{},d=te(o),a=320,f=400,g=22;function y(u){const v=b[u];return v?{x:v.x*a,y:v.y*f}:null}let _=`<svg width="${a}" height="${f}" viewBox="0 0 ${a} ${f}" xmlns="http://www.w3.org/2000/svg" style="display:block">`;d.forEach(([u,v],w)=>{const h=y(u),I=y(v);if(!h||!I)return;const A=r(u,v),k=i.has(A),T=k?"#3b82f6":"#999",j=k?.95:.35,S=k?4:3;_+=`<line x1="${h.x}" y1="${h.y}" x2="${I.x}" y2="${I.y}"
        stroke="transparent" stroke-width="14" class="link-hit" data-key="${A}" style="cursor:pointer"/>`,_+=`<line x1="${h.x}" y1="${h.y}" x2="${I.x}" y2="${I.y}"
        stroke="${T}" stroke-width="${S}" stroke-dasharray="${k?"none":"4,3"}"
        opacity="${j}" pointer-events="none" data-line-key="${A}"/>`});for(const u of Object.keys(b)){const v=y(u);if(!v)continue;const w=u.replace(/\d+/,""),h=Me[w]||"#555";_+=`<circle cx="${v.x}" cy="${v.y}" r="${g}" fill="${h}" stroke="rgba(255,255,255,0.7)" stroke-width="2" pointer-events="none"/>`,_+=`<text x="${v.x}" y="${v.y+4}" text-anchor="middle" font-size="10" font-weight="900" fill="white" font-family="Arial Black,Arial" pointer-events="none">${u}</text>`}_+="</svg>",document.getElementById("field-wrap").innerHTML=_;const p=document.getElementById("links-list");p.innerHTML=d.map(([u,v])=>{const w=r(u,v),h=i.has(w);return`
        ...
        </button>`}).join("");const $=e.querySelector('div[style*="font-weight:700;font-size:13px;margin-bottom:8px"]');$&&($.textContent=`Liens (${i.size}/${d.length} actifs)`);function E(u){i.has(u)?i.delete(u):i.add(u),c()}e.querySelectorAll(".link-hit").forEach(u=>{u.addEventListener("click",()=>E(u.dataset.key))}),e.querySelectorAll(".link-toggle").forEach(u=>{u.addEventListener("click",()=>E(u.dataset.key))})}c(),document.getElementById("formation-select").addEventListener("change",b=>{o=b.target.value,l(o),c()}),document.getElementById("reset-btn").addEventListener("click",()=>{const b=Y[o]||[];i=new Set(b.map(([d,a])=>r(d,a))),c(),n("Liens réinitialisés aux valeurs par défaut (non sauvegardé)","info")}),document.getElementById("save-btn").addEventListener("click",async()=>{const d=te(o).filter(([f,g])=>i.has(r(f,g))),{error:a}=await x.from("formation_links_overrides").upsert({formation:o,links:d,updated_at:new Date().toISOString()});if(a){n(a.message,"error");return}s[o]=d,n(`Liens enregistrés pour ${o} (${d.length} actifs)`,"success")})}const Pe=[{value:"player",label:"Joueur"},{value:"formation",label:"Formation"},{value:"game_changer",label:"Game Changer"},{value:"ultra_game_changer",label:"Ultra Game Changer"},{value:"game_helper",label:"Game Helper"}],Oe=[{value:"",label:"Toutes"},{value:"normal",label:"Normal"},{value:"pépite",label:"Pépite"},{value:"papyte",label:"Papyte"},{value:"légende",label:"Légende"}],De=[{value:"credits",label:"Crédits"},{value:"pub",label:"Publicité"},{value:"free",label:"Gratuit"}];async function qe(e){e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">Chargement...</div>';const{data:n}=await x.from("booster_configs").select("*").order("sort_order");let t=null,o=[],m=null;const s=()=>window.innerWidth<700;async function i(){if(!t){o=[];return}if(t!==m){const{data:d}=await x.from("booster_drop_rates").select("*").eq("booster_id",t).order("sort_order");o=d||[],m=t}}function r(){return`
    ...
    </div>`}function l(){const d=t?(n||[]).find(g=>g.id===t):null;if(!d)return`<div style="display:flex;align-items:center;justify-content:center;flex:1;color:#bbb;font-size:14px;padding:40px;text-align:center">
      ← Sélectionnez un booster ou créez-en un nouveau
    </div>`;const a=o.reduce((g,y)=>g+Number(y.percentage||0),0),f=Math.abs(a-100)<.1;return`
    <div id="booster-editor" style="flex:1;overflow-y:auto;background:#f8f8f8;padding:14px">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
          <div>
            ...
            <select id="f-price-type" style="width:100%;padding:9px;border:1px solid #ddd;border-radius:6px;font-size:13px">
              ${De.map(g=>`<option value="${g.value}" ${d.price_type===g.value?"selected":""}>${g.label}</option>`).join("")}
            </select>
          </div>
      ...
      <div style="background:#fff;border-radius:10px;padding:14px;margin-bottom:14px">
        ...
        ${o.length===0?'<div style="text-align:center;padding:16px;color:#aaa;font-size:13px">Aucun taux. Cliquez + Ligne.</div>':o.map((g,y)=>`
          <div style="display:grid;grid-template-columns:2fr 1.5fr 50px 50px 60px 32px;gap:4px;align-items:center;margin-bottom:6px;padding:6px;background:#f9f9f9;border-radius:6px" data-rate-idx="${y}">
            <select class="rate-type" data-idx="${y}" style="padding:6px 4px;border:1px solid #ddd;border-radius:4px;font-size:12px;width:100%">
              ${Pe.map(_=>`<option value="${_.value}" ${g.card_type===_.value?"selected":""}>${_.label}</option>`).join("")}
            </select>
            <select class="rate-rarity" data-idx="${y}" style="padding:6px 4px;border:1px solid #ddd;border-radius:4px;font-size:12px;width:100%">
              ${Oe.map(_=>`<option value="${_.value}" ${(g.rarity||"")===_.value?"selected":""}>${_.label}</option>`).join("")}
            </select>
    ...
    </div>`}async function c(d=!1){d||await i();const a=!t&&s(),f=t&&s();e.innerHTML=`
    <div style="display:flex;height:calc(100vh - 56px);overflow:hidden">
      ${!s()||a||!t?r():""}
      ${!s()||f?l():""}
    </div>`,b()}function b(){var f,g,y,_,p,$,E;const d=u=>e.querySelector(u);e.querySelectorAll(".booster-row").forEach(u=>{u.addEventListener("click",v=>{v.target.closest(".btn-delete")||(t=u.dataset.id,m=null,c())})}),(f=d("#btn-back"))==null||f.addEventListener("click",()=>{t=null,c()}),(g=d("#btn-new"))==null||g.addEventListener("click",async()=>{const u=prompt("Nom du nouveau booster :");if(!(u!=null&&u.trim()))return;const{data:v,error:w}=await x.from("booster_configs").insert({name:u.trim(),price_type:"credits",price_credits:1e3,card_count:5}).select().single();if(w){alert(w.message);return}n.push(v),t=v.id,m=null,c()}),e.querySelectorAll(".btn-delete").forEach(u=>{u.addEventListener("click",async v=>{if(v.stopPropagation(),!confirm("Supprimer ce booster ?"))return;await x.from("booster_configs").delete().eq("id",u.dataset.id);const w=n.findIndex(h=>h.id===u.dataset.id);w>-1&&n.splice(w,1),t===u.dataset.id&&(t=null,m=null),c()})}),(y=d("#btn-cancel"))==null||y.addEventListener("click",()=>{t=null,c()}),(_=d("#f-price-type"))==null||_.addEventListener("change",u=>{const v=d("#credits-field");v&&(v.style.opacity=u.target.value!=="credits"?"0.4":"1")}),(p=d("#btn-pick-icon"))==null||p.addEventListener("click",async()=>{var v;const u=d("#icon-picker-grid");if(u){if(u.style.display!=="none"){u.style.display="none";return}u.innerHTML='<div style="padding:10px;text-align:center;color:#888;font-size:12px">⏳ Chargement...</div>',u.style.display="block";try{const h=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),I=Array.isArray(h)?h.filter(k=>k.name.startsWith("booster")&&/\.(png|jpg|svg|webp)$/i.test(k.name)):[];if(!I.length){u.innerHTML='<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Aucune icône "booster*" trouvée.</div>';return}const A=((v=d("#f-image-url"))==null?void 0:v.value)||"";u.innerHTML=`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(72px,1fr));gap:8px">
          ${I.map(k=>`
        ...
        </div>`,u.querySelectorAll(".icon-pick-item").forEach(k=>{k.addEventListener("click",()=>{const T=d("#f-image-url");T&&(T.value=k.dataset.name),u.style.display="none"})})}catch(w){u.innerHTML=`<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Erreur API GitHub : ${w.message}</div>`}}});function a(){e.querySelectorAll("[data-rate-idx]").forEach(u=>{var w,h,I,A,k;const v=Number(u.dataset.rateIdx);v>=0&&v<o.length&&(o[v].card_type=((w=u.querySelector(".rate-type"))==null?void 0:w.value)||"player",o[v].rarity=((h=u.querySelector(".rate-rarity"))==null?void 0:h.value)||null,o[v].note_min=Number((I=u.querySelector(".rate-note-min"))==null?void 0:I.value)||null,o[v].note_max=Number((A=u.querySelector(".rate-note-max"))==null?void 0:A.value)||null,o[v].percentage=Number((k=u.querySelector(".rate-pct"))==null?void 0:k.value)||0)})}($=d("#btn-add-rate"))==null||$.addEventListener("click",()=>{a(),o.push({id:null,booster_id:t,card_type:"player",rarity:"normal",note_min:null,note_max:null,percentage:10,sort_order:o.length}),c(!0)}),e.querySelectorAll(".btn-del-rate").forEach(u=>{u.addEventListener("click",()=>{a(),o.splice(Number(u.dataset.idx),1),c(!0)})}),(E=d("#btn-save"))==null||E.addEventListener("click",async()=>{var A,k,T,j,S,P,O,D,q;const u=(n||[]).find(L=>L.id===t);if(!u)return;const v=[];e.querySelectorAll("[data-rate-idx]").forEach(L=>{var C,F,X,Z,ee;const M=Number(L.dataset.rateIdx);v.push({booster_id:t,card_type:((C=L.querySelector(".rate-type"))==null?void 0:C.value)||"player",rarity:((F=L.querySelector(".rate-rarity"))==null?void 0:F.value)||null,note_min:Number((X=L.querySelector(".rate-note-min"))==null?void 0:X.value)||null,note_max:Number((Z=L.querySelector(".rate-note-max"))==null?void 0:Z.value)||null,percentage:Number((ee=L.querySelector(".rate-pct"))==null?void 0:ee.value)||0,sort_order:M});const z=v[v.length-1];z.rarity||(z.rarity=null),z.note_min||(z.note_min=null),z.note_max||(z.note_max=null)});const w=v.reduce((L,M)=>L+M.percentage,0);if(v.length&&Math.abs(w-100)>.5){alert(`La somme doit faire 100% (actuellement ${w.toFixed(1)}%)`);return}const h={name:((k=(A=d("#f-name"))==null?void 0:A.value)==null?void 0:k.trim())||u.name,image_url:((j=(T=d("#f-image-url"))==null?void 0:T.value)==null?void 0:j.trim())||null,price_type:(S=d("#f-price-type"))==null?void 0:S.value,price_credits:Number((P=d("#f-price-credits"))==null?void 0:P.value)||0,card_count:Number((O=d("#f-card-count"))==null?void 0:O.value)||5,is_active:((D=d("#f-active"))==null?void 0:D.checked)??u.is_active,sort_order:Number((q=d("#f-sort"))==null?void 0:q.value)||0},{error:I}=await x.from("booster_configs").update(h).eq("id",t);if(I){alert(I.message);return}if(Object.assign(u,h),await x.from("booster_drop_rates").delete().eq("booster_id",t),v.length){const{error:L}=await x.from("booster_drop_rates").insert(v);if(L){alert(L.message);return}}o=v,m=t,alert("✅ Booster enregistré !"),c(!0)})}c()}const B=[{value:"game_changer",label:"⚡ Game Changer",border:"#7a28b8",bg:"linear-gradient(135deg,#4a0080,#7a28b8)"},{value:"ultra_game_changer",label:"💎 Ultra Game Changer",border:"#00bcd4",bg:"linear-gradient(135deg,#006080,#00bcd4)"}],R=[{value:"purple",label:"Violet",hex:"#7a28b8"},{value:"light_blue",label:"Bleu clair",hex:"#00bcd4"}];async function Fe(e){e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">Chargement...</div>';const{data:n}=await x.from("gc_definitions").select("*").order("sort_order").order("created_at");let t=null;const o=[{value:"BOOST_STAT",label:"⚡ Boost stats (+N)",hasValue:!0,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"DEBUFF_STAT",label:"💀 Debuff stats (-N)",hasValue:!0,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"GRAY_PLAYER",label:"❄️ Griser un joueur",hasValue:!1,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"REVIVE_PLAYER",label:"💫 Ressusciter un joueur",hasValue:!1,hasTarget:!1,hasCount:!0,hasRoles:!1},{value:"REMOVE_GOAL",label:"🚫 Retirer un but adverse",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1},{value:"ADD_GOAL_DRAW",label:"⚽ +1 but si duel nul",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1},{value:"ADD_SUB",label:"🔄 Ajouter un remplacement",hasValue:!1,hasTarget:!1,hasCount:!0,hasRoles:!1},{value:"CUSTOM",label:"🛠️ Effet custom (hardcodé)",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1}],m=["GK","DEF","MIL","ATT"],s=()=>window.innerWidth<700,i=a=>e.querySelector(a);function r(a,f=130){if(!a)return"";const g=B.find(E=>E.value===a.gc_type)||B[0],y=R.find(E=>E.value===a.color)||R[0],_=Math.round(f*.55),p=Math.round(f*.15),$=Math.round(f*.3);return`<div style="width:${f}px;height:${Math.round(f*1.4)}px;border-radius:10px;border:3px solid ${y.hex};background:${g.bg};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 16px ${y.hex}55;flex-shrink:0">
    ...
    <div id="gc-list" style="background:#fff;${s()?"":"border-right:1px solid #e0e0e0;width:280px;flex-shrink:0;"}overflow-y:auto;height:100%">
      ...
      ${(n||[]).map(a=>{const f=B.find(y=>y.value===a.gc_type)||B[0],g=R.find(y=>y.value===a.color)||R[0];return`
    ...
    </div>`}function c(){const a=t?(n||[]).find(g=>g.id===t):null;if(!a)return`<div style="flex:1;display:flex;align-items:center;justify-content:center;color:#bbb;font-size:14px;padding:40px;text-align:center">
      ← Sélectionnez une carte ou créez-en une nouvelle
    </div>`;const f=B.find(g=>g.value===a.gc_type)||B[0];return R.find(g=>g.value===a.color)||R[0],`
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
          <div>
            ...
            <select id="gc-type" style="width:100%;padding:9px;border:1px solid #ddd;border-radius:6px;font-size:13px">
              ${B.map(g=>`<option value="${g.value}" ${a.gc_type===g.value?"selected":""}>${g.label}</option>`).join("")}
            </select>
          </div>
          <div>
            ...
            <select id="gc-color" style="width:100%;padding:9px;border:1px solid #ddd;border-radius:6px;font-size:13px">
              ${R.map(g=>`<option value="${g.value}" ${a.color===g.value?"selected":""}>${g.label}</option>`).join("")}
            </select>
          </div>
        </div>
        ...
        <div style="border-top:1px solid #eee;padding-top:12px;margin-top:4px">
          ...
          <div>
            ...
            <select id="gc-effect-type" style="width:100%;padding:9px;border:1px solid #ddd;border-radius:6px;font-size:13px;margin-bottom:10px">
              ${o.map(g=>`<option value="${g.value}" ${(a.effect_type||"BOOST_STAT")===g.value?"selected":""}>${g.label}</option>`).join("")}
            </select>
          </div>
          ${(()=>{const g=a.effect_params||{},y=o.find(_=>_.value===(a.effect_type||"BOOST_STAT"))||o[0];return`
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
              ...
              ${y.hasRoles?`<div>
                ...
                <div style="display:flex;gap:4px;flex-wrap:wrap">
                  ${m.map(_=>`<label style="display:flex;align-items:center;gap:2px;font-size:11px;cursor:pointer">
    ...
    <div style="display:flex;height:calc(100vh - 56px);overflow:hidden">
      ${!s()||!t?l():""}
      ${!s()||t?c():""}
    </div>`,d()}function d(){var a,f,g,y,_;e.querySelectorAll(".gc-row").forEach(p=>{p.addEventListener("click",$=>{$.target.closest(".btn-toggle-gc,.btn-delete-gc")||(t=p.dataset.id,b())})}),(a=i("#btn-gc-back"))==null||a.addEventListener("click",()=>{t=null,b()}),(f=i("#btn-cancel-gc"))==null||f.addEventListener("click",()=>{t=null,b()}),(g=i("#btn-new-gc"))==null||g.addEventListener("click",async()=>{const p=prompt("Nom de la carte Game Changer :");if(!(p!=null&&p.trim()))return;const{data:$,error:E}=await x.from("gc_definitions").insert({name:p.trim(),gc_type:"game_changer",color:"purple"}).select().single();if(E){alert(E.message);return}n.push($),t=$.id,b()}),e.querySelectorAll(".btn-toggle-gc").forEach(p=>{p.addEventListener("click",async $=>{$.stopPropagation();const E=p.dataset.active!=="true";await x.from("gc_definitions").update({is_active:E}).eq("id",p.dataset.id);const u=n.find(v=>v.id===p.dataset.id);u&&(u.is_active=E),b()})}),e.querySelectorAll(".btn-delete-gc").forEach(p=>{p.addEventListener("click",async $=>{if($.stopPropagation(),!confirm("Supprimer cette carte Game Changer ?"))return;await x.from("gc_definitions").delete().eq("id",p.dataset.id);const E=n.findIndex(u=>u.id===p.dataset.id);E>-1&&n.splice(E,1),t===p.dataset.id&&(t=null),b()})}),(y=i("#btn-pick-gc-icon"))==null||y.addEventListener("click",async()=>{var $;const p=i("#gc-icon-picker");if(p){if(p.style.display!=="none"){p.style.display="none";return}p.innerHTML='<div style="padding:10px;text-align:center;color:#888;font-size:12px">⏳ Chargement...</div>',p.style.display="block";try{const u=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),v=Array.isArray(u)?u.filter(h=>h.name.startsWith("gamechanger-")&&/\.(png|jpg|svg|webp)$/i.test(h.name)):[];if(!v.length){p.innerHTML='<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Aucun fichier "gamechanger-*" trouvé dans public/icons/</div>';return}const w=(($=i("#gc-image-url"))==null?void 0:$.value)||"";p.innerHTML=`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(72px,1fr));gap:8px">
          ${v.map(h=>`
        ...
        </div>`,p.querySelectorAll(".gc-icon-item").forEach(h=>{h.addEventListener("click",()=>{const I=i("#gc-image-url");I&&(I.value=h.dataset.name),p.style.display="none"})})}catch(E){p.innerHTML=`<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Erreur API GitHub : ${E.message}</div>`}}}),(_=i("#btn-save-gc"))==null||_.addEventListener("click",async()=>{var h,I,A,k,T,j,S,P,O,D,q,L,M,z;const p=n.find(C=>C.id===t);if(!p)return;const $=o.find(C=>{var F;return C.value===(((F=i("#gc-effect-type"))==null?void 0:F.value)||"BOOST_STAT")})||o[0],E=$.hasRoles?[...e.querySelectorAll(".gc-p-role:checked")].map(C=>C.value):null,u={...$.hasValue?{value:Number((h=i("#gc-p-value"))==null?void 0:h.value)||2}:{},...$.hasCount?{count:Number((I=i("#gc-p-count"))==null?void 0:I.value)||1}:{},...$.hasTarget?{target:((A=i("#gc-p-target"))==null?void 0:A.value)||"home"}:{},...$.hasRoles?{roles:E!=null&&E.length?E:null}:{}},v={name:((T=(k=i("#gc-name"))==null?void 0:k.value)==null?void 0:T.trim())||p.name,effect:((S=(j=i("#gc-effect"))==null?void 0:j.value)==null?void 0:S.trim())||null,image_url:((O=(P=i("#gc-image-url"))==null?void 0:P.value)==null?void 0:O.trim())||null,gc_type:((D=i("#gc-type"))==null?void 0:D.value)||"game_changer",color:((q=i("#gc-color"))==null?void 0:q.value)||"purple",sort_order:Number((L=i("#gc-sort"))==null?void 0:L.value)||0,is_active:((M=i("#gc-active"))==null?void 0:M.checked)??p.is_active,effect_type:((z=i("#gc-effect-type"))==null?void 0:z.value)||"BOOST_STAT",effect_params:u},{error:w}=await x.from("gc_definitions").update(v).eq("id",t);if(w){alert(w.message);return}Object.assign(p,v),alert("✅ Carte enregistrée !"),b()})}b()}function Ue(e,n="info"){const t=document.getElementById("toast");t.textContent=e,t.className=`show ${n}`,clearTimeout(t._t),t._t=setTimeout(()=>{t.className=""},3e3)}function Ge(e,n,t=""){document.getElementById("modal-title").textContent=e,document.getElementById("modal-body").innerHTML=n,document.getElementById("modal-footer").innerHTML=t,document.getElementById("modal-overlay").classList.remove("hidden")}function W(){document.getElementById("modal-overlay").classList.add("hidden")}const ue={dashboard:{title:"Dashboard",fn:ye},players:{title:"Joueurs & Cartes",fn:Ee},clubs:{title:"Clubs",fn:we},"card-builder":{title:"Card Builder",fn:Ce},users:{title:"Managers",fn:be},market:{title:"Marché des transferts",fn:fe},"import-export":{title:"Import / Export CSV",fn:Re},formations:{title:"Formations & Liens",fn:Ne},"boosters-config":{title:"Boosters",fn:qe},"gc-cards":{title:"Game Changers",fn:Fe}};async function Q(e){document.querySelectorAll(".admin-sidebar nav a").forEach(o=>{o.classList.toggle("active",o.dataset.page===e)});const n=ue[e]||ue.dashboard;document.getElementById("page-title").textContent=n.title;const t=document.getElementById("page-content");t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">Chargement…</div>';try{await n.fn(t,{toast:Ue,openModal:Ge,closeModal:W,navigate:Q})}catch(o){t.innerHTML=`<div style="padding:40px;color:#c0392b">Erreur: ${o.message}</div>`,console.error(o)}}async function He(){const{data:{session:e}}=await x.auth.getSession();e&&await me(e.user),document.getElementById("auth-btn").addEventListener("click",async()=>{const n=document.getElementById("auth-email").value.trim(),t=document.getElementById("auth-password").value,o=document.getElementById("auth-error");if(o.textContent="",!n||!t){o.textContent="Remplissez tous les champs.";return}const{data:m,error:s}=await x.auth.signInWithPassword({email:n,password:t});if(s){o.textContent=s.message;return}await me(m.user)}),document.getElementById("auth-password").addEventListener("keydown",n=>{n.key==="Enter"&&document.getElementById("auth-btn").click()}),document.getElementById("logout-btn").addEventListener("click",async()=>{await x.auth.signOut(),document.getElementById("auth-screen").style.display="flex",document.getElementById("admin-app").style.display="none"}),document.getElementById("modal-close").addEventListener("click",W),document.getElementById("modal-overlay").addEventListener("click",n=>{n.target===n.currentTarget&&W()}),document.querySelectorAll(".admin-sidebar nav a").forEach(n=>{n.addEventListener("click",t=>{t.preventDefault(),Q(n.dataset.page)})})}async function me(e){const{data:n,error:t}=await x.from("users").select("*").eq("id",e.id).single(),o=document.getElementById("auth-error");if(t||!n){o.textContent="Profil introuvable.";return}if(!n.is_admin){o.textContent="Accès refusé. Vous n'êtes pas administrateur.",await x.auth.signOut();return}document.getElementById("auth-screen").style.display="none",document.getElementById("admin-app").style.display="flex",document.getElementById("admin-user").textContent=`Admin: ${n.pseudo}`,Q("dashboard")}He();
