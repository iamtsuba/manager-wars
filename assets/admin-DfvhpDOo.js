import{s as _,r as Ee,a as _e,A as vt,l as xt,g as ht,K as Me,D as z,b as Et,S as _t,c as R,H as wt,R as It,e as pe,d as Ze,f as $t,h as kt,E as Lt,i as Bt,N as At,M as St,B as Ct,j as Tt,F as me,k as ze,m as Mt,n as Qe,o as fe,p as zt}from"./special-cards-Cc3e36Hs.js";async function Rt(e){var h;const[{count:t},{count:n},{count:o},{count:l},{count:r},{data:g}]=await Promise.all([_.from("players").select("*",{count:"exact",head:!0}),_.from("clubs").select("*",{count:"exact",head:!0}),_.from("users").select("*",{count:"exact",head:!0}),_.from("stadium_definitions").select("*",{count:"exact",head:!0}),_.from("users").select("*",{count:"exact",head:!0}).gt("last_seen_at",new Date(Date.now()-18e4).toISOString()),_.rpc("get_signup_password")]),b=g||"";e.innerHTML=`
  ...
  `,window.adminNav=i=>{var m;(m=document.querySelector(`[data-page="${i}"]`))==null||m.click()},(h=document.getElementById("signup-pwd-save"))==null||h.addEventListener("click",async()=>{const i=document.getElementById("signup-pwd-input").value.trim(),m=document.getElementById("signup-pwd-status");if(!i){m.textContent="Le code ne peut pas être vide.",m.style.color="#ff6b6b";return}const a=document.getElementById("signup-pwd-save");a.disabled=!0,a.textContent="⏳...";const{error:d}=await _.rpc("set_signup_password",{new_password:i});if(a.disabled=!1,a.textContent="💾 Enregistrer",d){m.textContent=d.message,m.style.color="#ff6b6b";return}m.textContent="✅ Code mis à jour.",m.style.color="#2ecc71"})}const Xe={normal:"Normal",pepite:"Pépite",papyte:"Papyte",legende:"Légende"},jt={GK:"#aaaaaa",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};async function Dt(){const e={};for(const t of vt)e[t]=await xt(t);return e}async function Nt(e,t){const{toast:n}=t;if(!confirm(`Réattribuer une nouvelle photo à tous les joueurs ayant encore l'ancien format de chemin (public/faces/...) ?

Cette action est irréversible.`))return;const o=document.getElementById("fix-old-faces-btn");o&&(o.disabled=!0,o.textContent="⏳ En cours...");try{const{data:l,error:r}=await _.from("players").select("id, country_code, face").like("face","public/faces/%");if(r){n(r.message,"error");return}if(!(l!=null&&l.length)){n("Aucun joueur avec l'ancien format — rien à faire ✅","success");return}const{data:g}=await _.from("players").select("face").not("face","is",null),b=new Set((g||[]).map(m=>m.face).filter(m=>m&&!m.startsWith("public/faces/")));let h=0,i=0;for(const m of l){const a=await _e(m.country_code,b);if(!a){i++;continue}const{error:d}=await _.from("players").update({face:a}).eq("id",m.id);if(d){i++;continue}b.add(a),h++}n(`${h} photo(s) réattribuée(s)${i?`, ${i} échec(s)`:""} ✅`,i?"error":"success")}finally{o&&(o.disabled=!1,o.textContent="🖼️ Réattribuer anciennes photos")}}async function Pt(e,t){await se(e,t)}async function se(e,t,n=null){var i,m,a,d,p;const{toast:o}=t;n||(n={search:((i=document.getElementById("search-players"))==null?void 0:i.value)||"",job:((m=document.getElementById("filter-job"))==null?void 0:m.value)||"",rarity:((a=document.getElementById("filter-rarity"))==null?void 0:a.value)||"",club:((d=document.getElementById("filter-club"))==null?void 0:d.value)||"",country:((p=document.getElementById("filter-country"))==null?void 0:p.value)||""});const[{data:l,error:r},{data:g}]=await Promise.all([_.from("players").select("*, clubs(id,encoded_name,logo_url)"),_.from("clubs").select("id,encoded_name").order("encoded_name")]);if(r){e.innerHTML=`<p style="color:red">${r.message}</p>`;return}const b={GK:0,DEF:1,MIL:2,ATT:3},h=(l||[]).sort((s,f)=>{const c=(b[s.job]??4)-(b[f.job]??4);return c!==0?c:(s.surname_real||"").localeCompare(f.surname_real||"")});Ft(e,h,g||[],t,n)}function Ft(e,t,n,o,l=null){var m,a;const{toast:r}=o;if(e.innerHTML=`
    <div style="display:flex;flex-direction:column;gap:10px">
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        ...
        <select id="filter-rarity" style="width:110px">
          ...
          ${Object.entries(Xe).map(([d,p])=>`<option value="${d}">${p}</option>`).join("")}
        </select>
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        <select id="filter-club" style="flex:1;min-width:140px">
          ...
          ${n.map(d=>`<option value="${d.id}">${d.encoded_name}</option>`).join("")}
        </select>
        <select id="filter-country" style="flex:1;min-width:120px">
          <option value="">Tous les pays</option>
          ${[...new Set(t.map(d=>d.country_code).filter(Boolean))].sort().map(d=>`<option value="${d}">${d}</option>`).join("")}
        </select>
      </div>
    ...
    </div>`,l){const d=l,p=s=>document.getElementById(s);d.search&&p("search-players")&&(p("search-players").value=d.search),d.job&&p("filter-job")&&(p("filter-job").value=d.job),d.rarity&&p("filter-rarity")&&(p("filter-rarity").value=d.rarity),d.club&&p("filter-club")&&(p("filter-club").value=d.club),d.country&&p("filter-country")&&(p("filter-country").value=d.country)}if(l){const d=l,p=s=>document.getElementById(s);d.search&&p("search-players")&&(p("search-players").value=d.search),d.job&&p("filter-job")&&(p("filter-job").value=d.job),d.rarity&&p("filter-rarity")&&(p("filter-rarity").value=d.rarity),d.club&&p("filter-club")&&(p("filter-club").value=d.club),d.country&&p("filter-country")&&(p("filter-country").value=d.country)}function g(){const d=document.getElementById("search-players").value.toLowerCase(),p=document.getElementById("filter-job").value,s=document.getElementById("filter-rarity").value,f=document.getElementById("filter-club").value,c=document.getElementById("filter-country").value;return t.filter(y=>(!d||`${y.firstname} ${y.surname_real}`.toLowerCase().includes(d))&&(!p||y.job===p)&&(!s||y.rarity===s)&&(!f||y.club_id===f)&&(!c||y.country_code===c))}const b=new Set;function h(){const d=document.getElementById("bulk-bar"),p=document.getElementById("bulk-count");d&&(d.style.display=b.size>0?"flex":"none",p&&(p.textContent=`${b.size} joueur(s) sélectionné(s)`))}function i(){const d=g();document.getElementById("count-label").textContent=`${d.length} joueur(s)`;const p=document.getElementById("players-list");if(!d.length){p.innerHTML='<div style="color:var(--gray-600);padding:20px;text-align:center">Aucun joueur.</div>';return}p.innerHTML=d.map(s=>{const f={...s,clubs:s.clubs,face:s.face||null},c=Ee(f,{width:120});return`<div style="position:relative;cursor:pointer" data-edit="${s.id}">
        ${c}
      ...
      </div>`}).join(""),p.querySelectorAll("[data-edit]").forEach(s=>{s.addEventListener("click",()=>{const f=t.find(c=>c.id===s.dataset.edit);f&&Re(f,n,e,o)})}),p.querySelectorAll(".btn-del-player").forEach(s=>{s.addEventListener("click",async f=>{if(f.stopPropagation(),!confirm("Supprimer ce joueur ?"))return;const{error:c}=await _.from("players").delete().eq("id",s.dataset.del);c?r(c.message,"error"):(r("Joueur supprimé ✅","success"),se(e,o))})})}i(),document.getElementById("search-players").addEventListener("input",i),document.getElementById("filter-job").addEventListener("change",i),document.getElementById("filter-rarity").addEventListener("change",i),document.getElementById("filter-club").addEventListener("change",i),document.getElementById("filter-country").addEventListener("change",i),(m=document.getElementById("bulk-cancel-btn"))==null||m.addEventListener("click",()=>{b.clear(),h(),i()}),(a=document.getElementById("bulk-delete-btn"))==null||a.addEventListener("click",async()=>{var f,c,y,I,$;if(!b.size||!confirm(`Supprimer ${b.size} joueur(s) ?`))return;const d=[...b],{error:p}=await _.from("players").delete().in("id",d);if(p){r(p.message,"error");return}r(`${d.length} joueur(s) supprimé(s) ✅`,"success"),b.clear();const s={search:((f=document.getElementById("search-players"))==null?void 0:f.value)||"",job:((c=document.getElementById("filter-job"))==null?void 0:c.value)||"",rarity:((y=document.getElementById("filter-rarity"))==null?void 0:y.value)||"",club:((I=document.getElementById("filter-club"))==null?void 0:I.value)||"",country:(($=document.getElementById("filter-country"))==null?void 0:$.value)||""};se(e,o,s)}),document.getElementById("add-player-btn").addEventListener("click",()=>Re(null,n,e,o)),document.getElementById("fix-old-faces-btn").addEventListener("click",()=>Nt(e,o))}async function Re(e,t,n,o){const{toast:l,openModal:r,closeModal:g}=o,b=!!e,h=await Dt(),{data:i}=await _.from("players").select("face").not("face","is",null),m=new Set((i||[]).map(s=>s.face).filter(Boolean));e!=null&&e.face&&m.delete(e.face);const a='<option value="">— Club —</option>'+t.map(s=>`<option value="${s.id}" ${(e==null?void 0:e.club_id)===s.id?"selected":""}>${s.encoded_name}</option>`).join(""),d=e!=null&&e.face?e.face.split("/")[0]:"",p=Object.keys(h).map(s=>`<option value="${s}" ${d===s?"selected":""}>${s}</option>`).join("");r(b?`✏️ ${e.firstname} ${e.surname_real}`:"➕ Nouveau joueur",`<div style="display:flex;gap:20px;align-items:flex-start">
      ...
      <div style="flex:1;min-width:300px;display:flex;flex-direction:column;gap:12px">
        ...
        <div style="border-top:1px solid var(--gray-200);padding-top:10px">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:8px">
            ${[["GK","pm-g","note_g"],["DEF","pm-d","note_d"],["MIL","pm-m","note_m"],["ATT","pm-a","note_a"]].map(([s,f,c])=>`
              <div class="form-group" style="text-align:center">
                <label style="color:${jt[s]};font-weight:700">${s}</label>
                <input id="${f}" type="number" min="0" max="20" value="${(e==null?void 0:e[c])??0}" style="text-align:center;font-size:18px;font-weight:900;padding:6px 4px">
              </div>`).join("")}
          </div>
    ...
    </div>`,g(re),document.getElementById("search-clubs").addEventListener("input",b=>{const h=b.target.value.toLowerCase();g(re.filter(i=>i.real_name.toLowerCase().includes(h)||i.encoded_name.toLowerCase().includes(h)))}),document.getElementById("add-club-btn").addEventListener("click",()=>je(null,e,t));function g(b){const h=document.getElementById("clubs-list");if(!b.length){h.innerHTML='<p style="color:var(--tile-fg-dim);padding:20px;text-align:center">Aucun club.</p>';return}const i={};b.forEach(d=>{const p=d.country_code||"—";i[p]||(i[p]=[]),i[p].push(d)});const m=Object.keys(i).sort(),a=d=>{const p=st(d),s=d.logo_url?`<img src="${d.logo_url}" style="width:36px;height:36px;object-fit:contain;border-radius:8px;flex-shrink:0">`:`<div style="width:36px;height:36px;background:linear-gradient(135deg,${p.color1},${p.color2});border-radius:8px;display:flex;align-items:center;justify-content:center;color:#fff;font-size:9px;font-weight:900;flex-shrink:0">${d.encoded_name.slice(0,3)}</div>`,f=n[d.id]||0;return`
        <div class="club-row" data-club-id="${d.id}"
          ...
          ${s}
          <img src="https://flagsapi.com/${d.country_code}/flat/24.png" style="width:18px;height:13px;object-fit:cover;border-radius:2px;flex-shrink:0" onerror="this.style.display='none'">
          <div style="flex:1;min-width:0">
            <div style="font-weight:900;font-size:12px;color:var(--tile-fg-on-page);font-family:monospace;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${d.encoded_name}</div>
            <div style="font-size:10px;color:${f===0?"#e67e22":"var(--tile-fg-dim)"}">${f===0?"⚠️ 0 joueur":`👥 ${f}`}</div>
          </div>
          <button class="btn-del-club" data-del="${d.id}"
            ...
            onclick="event.stopPropagation()">✕</button>
        </div>`};h.innerHTML=m.map(d=>`
      <div style="display:flex;align-items:center;gap:8px;padding:8px 4px 4px;margin-top:6px">
        ${d!=="—"?`<img src="https://flagsapi.com/${d}/flat/24.png" style="width:20px;height:15px;object-fit:cover;border-radius:2px" onerror="this.style.display='none'">`:'<span style="font-size:14px">🌍</span>'}
        <span style="font-size:11px;font-weight:900;color:var(--tile-fg-dim);letter-spacing:1px">${d}</span>
        <span style="font-size:11px;color:var(--tile-fg-dim);background:var(--tile-bg);border:1px solid var(--tile-border);border-radius:10px;padding:1px 8px">${i[d].length}</span>
        <div style="flex:1;height:1px;background:var(--tile-border)"></div>
      </div>
      <div style="display:flex;flex-direction:column;gap:6px;margin-bottom:4px">
        ${i[d].map(a).join("")}
      </div>
    `).join(""),h.querySelectorAll(".club-row").forEach(d=>{d.addEventListener("click",()=>{const p=re.find(s=>s.id===d.dataset.clubId);p&&je(p,e,t),h.querySelectorAll(".club-row").forEach(s=>s.style.background="var(--tile-bg)"),d.style.background="rgba(26,107,60,0.18)"})}),h.querySelectorAll(".btn-del-club").forEach(d=>{d.addEventListener("click",async()=>{if(!confirm("Supprimer ce club ?"))return;const{error:p}=await _.from("clubs").delete().eq("id",d.dataset.del);p?o(p.message,"error"):(o("Club supprimé","success"),ue(e,t))})})}}async function je(e,t,n){var c,y,I,$,E,v;const{toast:o}=n,l=!!e,r=e?st(e):{...z},g=document.getElementById("club-panel");if(!g)return;const b=Object.entries(Me).map(([u,x])=>`<option value="${u}" ${r.style===u?"selected":""}>${x.label}</option>`).join(""),h=we.map(u=>`<option value="${u}" ${((e==null?void 0:e.country_code)||"FR")===u?"selected":""}>${u}</option>`).join("");g.style.display="block",g.style.alignItems="",g.style.justifyContent="",g.innerHTML=`
      ...
      <div style="border-top:1px solid var(--tile-border);padding-top:12px">
        ...
        <div style="display:flex;gap:12px;align-items:flex-start;flex-wrap:wrap">
          <div style="flex:1;min-width:200px;display:flex;flex-direction:column;gap:8px">
            ...
            ${[["Couleur 1","m-kit-color1",r.color1,!1],["Couleur 2","m-kit-color2",r.color2,!1],["Couleur 3","m-kit-color3",r.color3,!0],["Short","m-kit-shorts",r.shorts,!1],["Chaussettes","m-kit-socks",r.socks,!1]].map(([u,x,w,k])=>`
  ...
  `,(c=document.getElementById("m-logo-file"))==null||c.addEventListener("change",u=>{var k;const x=(k=u.target.files)==null?void 0:k[0];if(!x)return;const w=new FileReader;w.onload=()=>{document.getElementById("logo-preview").innerHTML=`<img src="${w.result}" style="width:100%;height:100%;object-fit:contain">`},w.readAsDataURL(x)}),ne();function i(){var k,L;const u=((k=document.getElementById("m-kit-style"))==null?void 0:k.value)||"uni",x=((L=Me[u])==null?void 0:L.colors)===3,w=document.getElementById("wrap-m-kit-color3");w&&(w.style.display=x?"":"none")}i(),["m-kit-color1","m-kit-color2","m-kit-color3","m-kit-shorts","m-kit-socks"].forEach(u=>{const x=document.getElementById(u),w=document.getElementById(u+"-txt");!x||!w||(x.addEventListener("input",()=>{w.value=x.value,ne()}),w.addEventListener("input",()=>{const k=w.value.trim();/^#[0-9a-fA-F]{6}$/.test(k)&&(x.value=k,ne())}),w.addEventListener("change",()=>{let k=w.value.trim();k.startsWith("#")||(k="#"+k),/^#[0-9a-fA-F]{6}$/.test(k)&&(x.value=k,w.value=k,ne())}))}),(y=document.getElementById("m-kit-style"))==null||y.addEventListener("change",()=>{i(),ne()});const m=document.getElementById("m-real"),a=document.getElementById("m-encoded");function d(){if(!m||!a||a.value)return;const u=m.value.trim().split(/\s+/),x=u.length===1?u[0].toUpperCase().slice(0,6):u.filter(w=>w.length>2).map(w=>w[0].toUpperCase()).join("")||u[0].toUpperCase().slice(0,4);a.value=x}m==null||m.addEventListener("input",d),(I=document.getElementById("auto-encode-btn"))==null||I.addEventListener("click",()=>{a&&(a.value=""),d()}),($=document.getElementById("m-save"))==null||$.addEventListener("click",()=>an(e,l,t,n));const p=document.getElementById("m-gen-squad"),s=document.getElementById("m-gen-strong-label");function f(){if(s&&(s.style.display=p!=null&&p.checked?"flex":"none",!(p!=null&&p.checked))){const u=document.getElementById("m-gen-strong");u&&(u.checked=!1)}}p==null||p.addEventListener("change",f),f(),(E=document.getElementById("btn-gen-squad-panel"))==null||E.addEventListener("click",()=>{Xt(e,n,t)}),(v=document.getElementById("btn-gen-one-player"))==null||v.addEventListener("click",()=>{tn(e,n)}),l&&$e(e.id)}async function $e(e){const{data:t}=await _.from("players").select("*, clubs(encoded_name, logo_url, kit_style, kit_color1, kit_color2, kit_color3, kit_shorts, kit_socks)").eq("club_id",e).order("surname_real"),n=document.getElementById("club-players-grid"),o=document.getElementById("club-players-count");if(o&&(o.textContent=(t||[]).length),!!n){if(!(t!=null&&t.length)){n.innerHTML='<div style="color:var(--tile-fg-dim);padding:20px;font-size:13px">Aucun joueur dans ce club.</div>';return}n.innerHTML=t.map(l=>Ee(l,{width:100})).join("")}}function Xt(e,t,n){const{openModal:o,closeModal:l,toast:r}=t;o(`⚽ Générer — ${e.real_name}`,`<div style="display:flex;flex-direction:column;gap:14px">
    ...
    </div>`),setTimeout(()=>{var g,b;(g=document.getElementById("qg-cancel"))==null||g.addEventListener("click",()=>l()),(b=document.getElementById("qg-ok"))==null||b.addEventListener("click",async()=>{var i;const h=((i=document.getElementById("qg-strong"))==null?void 0:i.checked)??!1;l(),r("Génération en cours…","info"),await lt(e.id,e.country_code,r,h),r("Effectif généré ✅","success"),$e(e.id),ue(n,t)})},50)}const dt=["GK","DEF","MIL","ATT"],en=[{value:"normal",label:"Normal"},{value:"pepite",label:"Pépite"},{value:"papyte",label:"Papyte"},{value:"legende",label:"Légende"}];function tn(e,t){const{openModal:n,closeModal:o,toast:l}=t,r=we.map(i=>`<option value="${i}">${i}</option>`).join(""),g=i=>dt.map(m=>`<option value="${m}" ${m===i?"selected":""}>${m}</option>`).join(""),b=`
  ...
  `),setTimeout(()=>{var i,m;document.querySelectorAll('input[name="gen-one-mode"]').forEach(a=>{a.addEventListener("change",()=>{document.getElementById("gen-one-custom-fields").style.display=a.value==="custom"&&a.checked?"flex":"none"})}),(i=document.getElementById("gen-one-cancel"))==null||i.addEventListener("click",()=>o()),(m=document.getElementById("gen-one-ok"))==null||m.addEventListener("click",async()=>{var s,f,c,y,I;const a=((s=document.querySelector('input[name="gen-one-mode"]:checked'))==null?void 0:s.value)||"random",d=document.getElementById("gen-one-ok");d.disabled=!0,d.textContent="⏳ Génération…";const p={};a==="custom"&&(p.job=((f=document.getElementById("gen-one-job1"))==null?void 0:f.value)||null,p.job2=((c=document.getElementById("gen-one-job2"))==null?void 0:c.value)||null,p.rarity=((y=document.getElementById("gen-one-rarity"))==null?void 0:y.value)||null,p.country=((I=document.getElementById("gen-one-country"))==null?void 0:I.value)||null);try{await nn(e,p),l("Joueur généré ✅","success"),o(),$e(e.id)}catch($){l("Erreur : "+$.message,"error"),d.disabled=!1,d.textContent="➕ Générer"}})},50)}async function nn(e,t={}){const n=t.country||(Math.random()<.5?e.country_code:nt(e.country_code)),o=t.job||q(dt),l=t.job2||null,r=t.rarity||(()=>{const B=Math.random()*100;return B<1.5?"legende":B<3?"pepite":B<4.5?"papyte":"normal"})(),g=r==="pepite"?J(10,15):r==="papyte"?J(15,20):it(),b=l?Math.max(0,g-2):0;let h=0,i=0,m=0,a=0;o==="GK"&&(h=g),o==="DEF"&&(i=g),o==="MIL"&&(m=g),o==="ATT"&&(a=g),l==="GK"&&(h=b),l==="DEF"&&(i=b),l==="MIL"&&(m=b),l==="ATT"&&(a=b);const d=Ie(n),p=tt(n),s=q(et),{data:f}=await _.from("players").select("surname_real").not("surname_real","is",null),c=new Set((f||[]).map(B=>B.surname_real).filter(Boolean)),y=rt(n),I=y.filter(B=>!c.has(B)),$=q(I.length?I:y),E={job:o,job2:l,firstname:at(n),surname_real:$,country_code:n,club_id:e.id,note_g:h,note_d:i,note_m:m,note_a:a,skin:d,hair:p,hair_length:s,rarity:r,sell_price:0,ethnie:ye(n)},{data:v}=await _.from("players").select("face").not("face","is",null),u=new Set((v||[]).map(B=>B.face).filter(Boolean)),x=await _e(n,u);x&&(E.face=x);const{data:w,error:k}=await _.from("players").insert(E).select("id").single();if(k)throw k;const{error:L}=await _.from("cards").insert({card_type:"player",player_id:w.id});L&&console.warn("[GenOnePlayer] Erreur carte:",L.message)}function ct(){var e,t,n,o,l,r;return{style:((e=document.getElementById("m-kit-style"))==null?void 0:e.value)||"uni",color1:((t=document.getElementById("m-kit-color1"))==null?void 0:t.value)||"#1A6B3C",color2:((n=document.getElementById("m-kit-color2"))==null?void 0:n.value)||"#ffffff",color3:((o=document.getElementById("m-kit-color3"))==null?void 0:o.value)||"#000000",shorts:((l=document.getElementById("m-kit-shorts"))==null?void 0:l.value)||"#111111",socks:((r=document.getElementById("m-kit-socks"))==null?void 0:r.value)||"#ffffff"}}function ne(){const e=document.getElementById("kit-preview-wrap");e&&(e.innerHTML=Et(ct(),"panel"))}async function on(e,t){const n=(e.name.split(".").pop()||"png").toLowerCase(),o=`clubs/${t||"new"}-${Date.now()}.${n}`,{error:l}=await _.storage.from("assets").upload(o,e,{upsert:!0,cacheControl:"3600"});if(l)throw l;const{data:r}=_.storage.from("assets").getPublicUrl(o);return r.publicUrl}async function an(e,t,n,o){var $,E,v,u,x,w,k,L,B;const{toast:l}=o,r=document.getElementById("m-error"),g=document.getElementById("m-save"),b=($=document.getElementById("m-real"))==null?void 0:$.value.trim(),h=(E=document.getElementById("m-encoded"))==null?void 0:E.value.trim().toUpperCase(),i=(v=document.getElementById("m-country"))==null?void 0:v.value.trim().toUpperCase(),m=((x=(u=document.getElementById("m-logo-file"))==null?void 0:u.files)==null?void 0:x[0])||null,a=((w=document.getElementById("m-logo-url-current"))==null?void 0:w.value)||null,d=((k=document.getElementById("m-gen-stadium"))==null?void 0:k.checked)??!1,p=((L=document.getElementById("m-gen-squad"))==null?void 0:L.checked)??!1,s=((B=document.getElementById("m-gen-strong"))==null?void 0:B.checked)??!1,f=ct();if(!b){r.textContent="Le nom du club est requis.";return}if(!h){r.textContent="Le nom encodé est requis.";return}if(!i){r.textContent="Le pays est requis.";return}g.disabled=!0,g.textContent="Enregistrement…";let c=a;if(m)try{g.textContent="📤 Envoi du logo…",c=await on(m,e==null?void 0:e.id)}catch(A){r.textContent="Erreur upload logo : "+A.message,g.disabled=!1,g.textContent=t?"💾 Enregistrer":"✅ Créer le club";return}const y={real_name:b,encoded_name:h,country_code:i,logo_url:c,kit_style:f.style,kit_color1:f.color1,kit_color2:f.color2,kit_color3:f.color3,kit_shorts:f.shorts,kit_socks:f.socks};let I=e==null?void 0:e.id;if(t){const{error:A}=await _.from("clubs").update(y).eq("id",I);if(A){r.textContent=A.message,g.disabled=!1,g.textContent="💾 Enregistrer";return}}else{const{data:A,error:S}=await _.from("clubs").insert(y).select().single();if(S){r.textContent=S.message,g.disabled=!1,g.textContent="✅ Créer le club";return}if(I=A.id,d){g.textContent="🏟️ Création du stade…";const{data:C,error:M}=await _.from("stadium_definitions").insert({name:`Stade de ${b}`,club_id:I,country_code:null}).select().single();M?console.warn("[Stadium] Erreur def stade:",M.message):C&&await _.from("cards").insert({card_type:"stadium",stadium_id:C.id})}p&&(g.textContent="⚽ Génération des joueurs…",await lt(I,i,l,s))}l(t?"Club modifié ✅":"Club créé ✅","success"),ue(n,o)}const rn=["normal","pepite","papyte","legende"],De=["GK","DEF","MIL","ATT"],ln=[["MA","Maroc"],["FR","France"],["AR","Argentine"],["PT","Portugal"],["BR","Brésil"],["ES","Espagne"],["DE","Allemagne"],["GB","Angleterre"],["IT","Italie"],["CM","Cameroun"],["SN","Sénégal"],["NG","Nigéria"],["DK","Danemark"],["NL","Pays-Bas"],["BE","Belgique"],["CI","Côte d'Ivoire"],["AL","Albanie"],["HR","Croatie"],["RS","Serbie"],["TR","Turquie"],["MW","Malawi"]];let K={...R},le=[];async function sn(e,{toast:t}){const{data:n}=await _.from("clubs").select("id,encoded_name,logo_url,kit_style,kit_color1,kit_color2,kit_shorts,kit_socks").order("encoded_name");le=n||[],K={...R},e.innerHTML=un(le),cn(e,le,t),P()}function Ne(){var n;const e=((n=document.getElementById("cb-surname-real"))==null?void 0:n.value)||"",t=document.getElementById("cb-surname-enc");t&&(t.value=pe(e))}function dn(){var n;const e=(n=document.getElementById("cb-club"))==null?void 0:n.value,t=le.find(o=>String(o.id)===String(e));return t?{style:t.kit_style||z.style,color1:t.kit_color1||z.color1,color2:t.kit_color2||z.color2,shorts:t.kit_shorts||z.shorts,socks:t.kit_socks||z.socks}:{...z}}function P(){var d,p,s,f;const e=c=>{var y;return((y=document.getElementById(c))==null?void 0:y.value)||""},t={firstname:e("cb-firstname"),surname_real:e("cb-surname-enc")||pe(e("cb-surname-real")),country_code:e("cb-country"),club_encoded_name:((p=(d=document.getElementById("cb-club"))==null?void 0:d.selectedOptions[0])==null?void 0:p.text)||"",job:e("cb-job"),job2:e("cb-job2")||null,note_g:parseInt(e("cb-note-g"))||0,note_d:parseInt(e("cb-note-d"))||0,note_m:parseInt(e("cb-note-m"))||0,note_a:parseInt(e("cb-note-a"))||0,rarity:e("cb-rarity")},n=dn(),o=document.getElementById("cb-club"),l=((f=(s=o==null?void 0:o.selectedOptions[0])==null?void 0:s.dataset)==null?void 0:f.logo)||null,r=Ze(K,n,120),g=`data:image/svg+xml;charset=utf-8,${encodeURIComponent(r)}`,b=$t(t,{portraitUrl:g,clubLogoUrl:l,showNotes:!0}),h=document.getElementById("card-preview");h&&(h.innerHTML=b);const i=document.getElementById("avatar-preview-wrap");i&&(i.innerHTML=r);const m=e("cb-surname-real"),a=document.getElementById("encode-summary");a&&m&&(a.textContent=`${e("cb-firstname")} ${m} → ${e("cb-firstname")} ${t.surname_real}`)}function pt(){const e=t=>{var n;return((n=document.getElementById(t))==null?void 0:n.value)||""};K={skin:e("av-skin"),hair:e("av-hair"),hairColor:e("av-hairColor"),eyes:e("av-eyes"),nose:e("av-nose"),mouth:e("av-mouth"),eyebrows:e("av-eyebrows"),beard:e("av-beard")}}function cn(e,t,n){var l,r,g,b,h;e.querySelectorAll("input,select").forEach(i=>{i.addEventListener("input",P),i.addEventListener("change",P)});const o=["av-skin","av-hair","av-hairColor","av-eyes","av-nose","av-mouth","av-eyebrows","av-beard"];o.forEach(i=>{var m;(m=document.getElementById(i))==null||m.addEventListener("change",()=>{pt(),P()})}),(l=document.getElementById("cb-surname-real"))==null||l.addEventListener("input",()=>{Ne(),P()}),(r=document.getElementById("btn-encode"))==null||r.addEventListener("click",()=>{Ne(),P()}),(g=document.getElementById("cb-club"))==null||g.addEventListener("change",P),(b=document.getElementById("btn-save-player"))==null||b.addEventListener("click",()=>pn(n)),(h=document.getElementById("btn-reset"))==null||h.addEventListener("click",()=>{e.querySelectorAll("input").forEach(i=>i.value=""),e.querySelectorAll("select").forEach(i=>i.selectedIndex=0),K={...R},o.forEach(i=>{const m=i.replace("av-",""),a=document.getElementById(i);a&&(a.value=R[m]||"")}),P()})}async function pn(e){const t=g=>{var b;return((b=document.getElementById(g))==null?void 0:b.value)||""},n=t("cb-surname-real").trim(),o=t("cb-surname-enc").trim()||pe(n);if(!t("cb-firstname")||!n||!t("cb-country")||!t("cb-job")){e("Remplissez les champs obligatoires (prénom, nom, pays, poste)","error");return}pt();const l={firstname:t("cb-firstname").trim(),surname_real:n,surname_real:o,country_code:t("cb-country"),club_id:t("cb-club")||null,job:t("cb-job"),job2:t("cb-job2")||null,note_g:parseInt(t("cb-note-g"))||0,note_d:parseInt(t("cb-note-d"))||0,note_m:parseInt(t("cb-note-m"))||0,note_a:parseInt(t("cb-note-a"))||0,rarity:t("cb-rarity")||"normal",note_min:parseInt(t("cb-note-min"))||null,note_max:parseInt(t("cb-note-max"))||null,sell_price:parseInt(t("cb-price"))||0,avatar_config:K,skin:K.skin||"blanc",hair:K.hairColor||"marron",hair_length:"court"},{error:r}=await _.from("players").insert(l);if(r){e("Erreur: "+r.message,"error");return}e(`✅ Joueur "${l.firstname} ${l.surname_real}" enregistré !`,"success")}function un(e){const t=ln.map(([i,m])=>`<option value="${i}">${m} (${i})</option>`).join(""),n='<option value="">— Sélectionner un club —</option>'+e.map(i=>`<option value="${i.id}" data-logo="${i.logo_url||""}">${i.encoded_name}</option>`).join(""),o=De.map(i=>`<option value="${i}">${i}</option>`).join(""),l='<option value="">Aucun</option>'+De.map(i=>`<option value="${i}">${i}</option>`).join(""),r=rn.map(i=>`<option value="${i}">${i.charAt(0).toUpperCase()+i.slice(1)}</option>`).join(""),g=(i,m,a)=>Object.entries(m).map(([d,p])=>`<option value="${d}" ${d===(a||"")?"selected":""}>${p.label}</option>`).join(""),b=Object.keys(_t).map(i=>`<option value="${i}" ${i===R.skin?"selected":""}>${i.charAt(0).toUpperCase()+i.slice(1)}</option>`).join(""),h=Object.keys(wt).map(i=>`<option value="${i}" ${i===R.hairColor?"selected":""}>${i.charAt(0).toUpperCase()+i.slice(1)}</option>`).join("");return`
        ...
        <div style="display:grid;grid-template-columns:1fr 120px;gap:16px;align-items:start">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
            ...
            <div>
              ...
              <select id="av-hair">${g("av-hair",kt,R.hair)}</select>
            </div>
            <div>
            ...
            <div>
              ...
              <select id="av-eyebrows">${g("av-eyebrows",Lt,R.eyebrows)}</select>
            </div>
            <div>
              <label>Yeux</label>
              <select id="av-eyes">${g("av-eyes",Bt,R.eyes)}</select>
            </div>
            <div>
              <label>Nez</label>
              <select id="av-nose">${g("av-nose",At,R.nose)}</select>
            </div>
            <div>
              <label>Bouche</label>
              <select id="av-mouth">${g("av-mouth",St,R.mouth)}</select>
            </div>
            <div>
              <label>Barbe</label>
              <select id="av-beard">${g("av-beard",Ct,R.beard)}</select>
            </div>
          </div>
  ...
  `}async function ut(e,{toast:t}){const[{data:n,error:o},{data:l}]=await Promise.all([_.from("users").select("id,pseudo,club_name,credits,level,wins,draws,losses,trophies_top1,trophies_top2,trophies_top3,is_admin,created_at,mmr,mmr_deviation,rank_tier,placement_matches,ranked_wins,ranked_losses,ranked_draws").order("created_at",{ascending:!1}),_.from("user_solo_progress").select("user_id, unlocked_level")]);if(o){e.innerHTML=`<p style="color:red">${o.message}</p>`;return}const r={};(l||[]).forEach(h=>{r[h.user_id]=h.unlocked_level});const g=n||[];e.innerHTML=`
  ...
  `,b(g),document.getElementById("search-users").addEventListener("input",h=>{const i=h.target.value.toLowerCase();b(g.filter(m=>{var a;return m.pseudo.toLowerCase().includes(i)||((a=m.club_name)==null?void 0:a.toLowerCase().includes(i))}))});function b(h){document.getElementById("users-tbody").innerHTML=h.map(i=>{const m=i.mmr??1e3,a=i.mmr_deviation??350,d=i.placement_matches??0,p=Tt(m),s=i.ranked_wins??0,f=i.ranked_losses??0,c=i.ranked_draws??0,y=s+f+c,I=y>0?Math.round(s/y*100):null,$=d<10,E=a<80?"#1A6B3C":a<150?"#e67e22":"#bb2020";return`
        <tr>
          <td>
          ...
          <td style="font-size:12px">
            ...
            ${$?`<div style="font-size:10px;color:#e67e22">${d}/10 placement${d<10?` (${10-d} restants)`:""}</div>`:`<div style="font-size:10px;color:var(--gray-600)">${s}V ${c}N ${f}D${I!==null?` · ${I}% WR`:""}</div>`}
          </td>
        ...
        </tr>`}).join(""),document.querySelectorAll(".btn-edit-credits").forEach(i=>{i.addEventListener("click",async()=>{const m=i.dataset.user,a=Number(i.dataset.current)||0,d=prompt("Nouveau solde de crédits :",a);if(d===null)return;const p=parseInt(d.replace(/\s/g,""),10);if(isNaN(p)||p<0){t("Valeur invalide","error");return}const{data:s,error:f}=await _.rpc("admin_update_user_credits",{p_user_id:m,p_new_credits:p});if(f){t(f.message,"error");return}if(!(s!=null&&s.success)){t((s==null?void 0:s.error)||"Échec de la mise à jour","error");return}t("Crédits mis à jour ✅","success");const c=document.getElementById(`credits-display-${m}`);c&&(c.textContent=`${p.toLocaleString("fr")} cr.`),i.dataset.current=p})}),document.querySelectorAll("[data-toggle-admin]").forEach(i=>{i.addEventListener("click",async()=>{const m=i.dataset.isAdmin!=="true";if(!confirm(`Voulez-vous ${m?"rendre admin":"retirer le rôle admin"} de ce Manager ?`))return;const{error:a}=await _.from("users").update({is_admin:m}).eq("id",i.dataset.toggleAdmin);a?t(a.message,"error"):(t("Rôle mis à jour ✅","success"),ut(e,{toast:t}))})})}}const mn=[{value:"normal",label:"Normal",color:"#ccc"},{value:"pepite",label:"Pépite",color:"#D4A017"},{value:"papyte",label:"Papyte",color:"#909090"},{value:"legende",label:"Légende",color:"#7a28b8"}];async function mt(e,t){var h;const{toast:n}=t,{data:o,error:l}=await _.from("market_listings").select(`
    ...
    `).order("listed_at",{ascending:!1}).limit(100);if(l){e.innerHTML=`<p style="color:var(--danger)">${l.message}</p>`;return}const r=o||[],g=r.filter(i=>i.status==="active").length,b=r.filter(i=>i.status==="sold").length;e.innerHTML=`
    ...
    <div class="card-panel">
      <div class="table-wrap">
        <table>
          ...
          <tbody>
            ${r.map(i=>{var p,s,f,c;const m=(p=i.card)==null?void 0:p.player,a=m?`${m.firstname} ${m.surname_real}`:((s=i.card)==null?void 0:s.card_type)||"—",d={active:"#1A6B3C",sold:"#D4A017",cancelled:"#888"};return`<tr>
                <td><b>${a}</b>${m?`<div style="font-size:10px;color:var(--tile-fg-dim)">${m.rarity} · ${m.job}</div>`:""}</td>
                <td style="font-size:12px">${((f=i.seller)==null?void 0:f.pseudo)||"—"}</td>
                <td style="font-size:12px">${((c=i.buyer)==null?void 0:c.pseudo)||"—"}</td>
                <td style="font-size:13px;font-weight:600">${(i.price||0).toLocaleString("fr")} cr.</td>
                <td>
                  <span style="background:${d[i.status]};color:#fff;padding:2px 6px;border-radius:3px;font-size:10px;font-weight:700">
  ...
  `,document.querySelectorAll("[data-cancel]").forEach(i=>{i.addEventListener("click",async()=>{if(!confirm("Annuler cette annonce ?"))return;const{error:m}=await _.from("market_listings").update({status:"cancelled"}).eq("id",i.dataset.cancel);m?n(m.message,"error"):(n("Annonce annulée","success"),mt(e,t))})}),(h=document.getElementById("btn-price-grid"))==null||h.addEventListener("click",()=>gn(t))}async function gn(e){await be(e)}function fn(){const e=document.getElementById("modal-overlay");if(!e||e._pgBlocker)return;const t=n=>{n.target===n.currentTarget&&n.stopImmediatePropagation()};e.addEventListener("click",t,!0),e._pgBlocker=t}function ge(){const e=document.getElementById("modal-overlay");e!=null&&e._pgBlocker&&(e.removeEventListener("click",e._pgBlocker,!0),delete e._pgBlocker)}async function be(e){var p,s;const{openModal:t,closeModal:n,toast:o}=e,{data:l,error:r}=await _.from("sell_price_configs").select("*").order("rarity").order("note_min");if(r){o(r.message,"error");return}const g=f=>mn.map(c=>`<option value="${c.value}" ${c.value===f?"selected":""}>${c.label}</option>`).join(""),b=f=>`
    <tr data-row="${f.id}">
    ...
    <div style="max-height:400px;overflow-y:auto;margin-bottom:14px">
      <table>
        ...
        <tbody id="pg-tbody">
          ${(l||[]).map(b).join("")||'<tr><td colspan="6" style="text-align:center;color:#aaa;padding:20px">Aucune règle définie.</td></tr>'}
        </tbody>
      </table>
    </div>
  ...
  `),fn(),(p=document.getElementById("pg-close"))==null||p.addEventListener("click",()=>{ge(),n()});function m(f){const c=f.dataset.id,y=f.dataset.field,I=y==="rarity"?f.value:Number(f.value)||0;return a(c,{[y]:I},f)}async function a(f,c,y){const I=document.querySelector(`tr[data-row="${f}"]`),$=k=>I.querySelector(`[data-field="${k}"]`),E="note_min"in c?c.note_min:Number($("note_min").value),v="note_max"in c?c.note_max:Number($("note_max").value),u="price_min"in c?c.price_min:Number($("price_min").value),x="price_max"in c?c.price_max:Number($("price_max").value);if(E>v){o("Note min doit être ≤ note max","error");return}if(u>x){o("Prix min doit être ≤ prix max","error");return}const{error:w}=await _.from("sell_price_configs").update(c).eq("id",f);if(w){o(w.message,"error");return}y&&(y.style.transition="background .3s",y.style.background="#e8f8ee",setTimeout(()=>{y.style.background=""},500))}function d(){document.querySelectorAll("#pg-tbody [data-field]").forEach(f=>{f.addEventListener("change",()=>m(f))})}d(),document.querySelectorAll("[data-del-row]").forEach(f=>{f.addEventListener("click",async()=>{confirm("Supprimer cette ligne ?")&&(await _.from("sell_price_configs").delete().eq("id",f.dataset.delRow),o("Ligne supprimée","success"),ge(),be(e))})}),(s=document.getElementById("pg-add-row"))==null||s.addEventListener("click",async()=>{var $;const f={rarity:"normal",note_min:0,note_max:20,price_min:2500,price_max:15e3},{data:c,error:y}=await _.from("sell_price_configs").insert(f).select().single();if(y){o(y.message,"error");return}const I=document.getElementById("pg-tbody");I.querySelector("td[colspan]")&&(I.innerHTML=""),I.insertAdjacentHTML("beforeend",b(c)),d(),($=document.querySelector(`[data-del-row="${c.id}"]`))==null||$.addEventListener("click",async function(){confirm("Supprimer cette ligne ?")&&(await _.from("sell_price_configs").delete().eq("id",c.id),o("Ligne supprimée","success"),ge(),be(e))}),o("Ligne ajoutée — modifie les valeurs directement","success")})}async function yn(e,{toast:t}){e.innerHTML=`
...
`;n.forEach(r=>{l+=[r.real_name,r.encoded_name,r.country_code,r.logo_url||""].map(Pe).join(",")+`
`}),oe("clubs_export.csv",l),t(`${n.length} clubs exportés`,"success")}),document.getElementById("export-players").addEventListener("click",async()=>{const{data:n,error:o}=await _.from("players").select("firstname,surname_real,surname_real,country_code,job,job2,note_g,note_d,note_m,note_a,rarity,note_min,note_max,skin,hair,hair_length,sell_price,clubs(encoded_name)").order("surname_real");if(o){t(o.message,"error");return}if(!n||n.length===0){t("Aucun joueur à exporter","info");return}let l=`firstname,surname_real,surname_real,country_code,club_encoded_name,job,job2,note_g,note_d,note_m,note_a,rarity,note_min,note_max,skin,hair,hair_length,sell_price
`;n.forEach(r=>{var g;l+=[r.firstname,r.surname_real,r.surname_real,r.country_code,((g=r.clubs)==null?void 0:g.encoded_name)||"",r.job,r.job2||"",r.note_g,r.note_d,r.note_m,r.note_a,r.rarity,r.note_min??"",r.note_max??"",r.skin,r.hair,r.hair_length,r.sell_price].map(Pe).join(",")+`
`}),oe("players_export.csv",l),t(`${n.length} joueurs exportés`,"success")}),document.getElementById("import-clubs").addEventListener("change",async n=>{const o=n.target.files[0];if(!o)return;const l=document.getElementById("clubs-import-result");l.innerHTML='<span style="color:var(--gray-600)">Import en cours...</span>';try{const r=await o.text(),g=Fe(r);if(g.length===0){l.innerHTML='<span style="color:#c0392b">Fichier vide</span>';return}let b=0,h=0;const i=[];for(const m of g){if(!m.real_name||!m.encoded_name||!m.country_code){h++,i.push(`Ligne ignorée (champs manquants): ${m.real_name||"?"}`);continue}const a={real_name:m.real_name,encoded_name:m.encoded_name.toUpperCase(),country_code:m.country_code.toUpperCase().slice(0,2),logo_url:m.logo_url||null},{error:d}=await _.from("clubs").upsert(a,{onConflict:"encoded_name"});d?(h++,i.push(`${m.encoded_name}: ${d.message}`)):b++}l.innerHTML=`<div style="color:var(--green)">✅ ${b} clubs importés</div>
        ${h>0?`<div style="color:#c0392b">❌ ${h} erreurs</div><div style="font-size:11px;color:var(--gray-600);max-height:100px;overflow-y:auto">${i.slice(0,10).join("<br>")}</div>`:""}`,t(`${b} clubs importés`,"success")}catch(r){l.innerHTML=`<span style="color:#c0392b">Erreur : ${r.message}</span>`}n.target.value=""}),document.getElementById("import-players").addEventListener("change",async n=>{const o=n.target.files[0];if(!o)return;const l=document.getElementById("players-import-result");l.innerHTML='<span style="color:var(--gray-600)">Import en cours...</span>';try{const r=await o.text(),g=Fe(r);if(g.length===0){l.innerHTML='<span style="color:#c0392b">Fichier vide</span>';return}const{data:b}=await _.from("clubs").select("id,encoded_name"),h={};(b||[]).forEach(y=>{h[y.encoded_name.toUpperCase()]=y.id});let i=0,m=0;const a=[],d=["GK","DEF","MIL","ATT"],p=["normal","pepite","papyte","legende"],s=["blanc","metisse","typ","noir"],f=["blond","marron","noir","chauve"],c=["rase","court","milong","long"];for(const y of g){if(!y.firstname||!y.surname_real||!y.country_code||!y.job){m++,a.push(`Ligne ignorée (champs requis manquants): ${y.firstname||"?"}`);continue}if(!d.includes(y.job)){m++,a.push(`${y.firstname}: job invalide "${y.job}"`);continue}const I=y.club_encoded_name&&h[y.club_encoded_name.toUpperCase()]||null,$={firstname:y.firstname,surname_real:y.surname_real,surname_real:y.surname_real||pe(y.surname_real),country_code:y.country_code.toUpperCase().slice(0,2),club_id:I,job:y.job,job2:d.includes(y.job2)?y.job2:null,note_g:parseInt(y.note_g)||0,note_d:parseInt(y.note_d)||0,note_m:parseInt(y.note_m)||0,note_a:parseInt(y.note_a)||0,rarity:p.includes(y.rarity)?y.rarity:"normal",note_min:y.note_min!==""&&y.note_min!=null?parseInt(y.note_min):null,note_max:y.note_max!==""&&y.note_max!=null?parseInt(y.note_max):null,skin:s.includes(y.skin)?y.skin:"blanc",hair:f.includes(y.hair)?y.hair:"noir",hair_length:c.includes(y.hair_length)?y.hair_length:"court",sell_price:parseInt(y.sell_price)||0},{error:E}=await _.from("players").insert($);E?(m++,a.push(`${y.firstname} ${y.surname_real}: ${E.message}`)):i++}l.innerHTML=`<div style="color:var(--green)">✅ ${i} joueurs importés</div>
        ${m>0?`<div style="color:#c0392b">❌ ${m} erreurs</div><div style="font-size:11px;color:var(--gray-600);max-height:100px;overflow-y:auto">${a.slice(0,10).join("<br>")}</div>`:""}`,t(`${i} joueurs importés`,"success")}catch(r){l.innerHTML=`<span style="color:#c0392b">Erreur : ${r.message}</span>`}n.target.value=""})}function Pe(e){const t=String(e??"");return t.includes(",")||t.includes('"')||t.includes(`
`)?'"'+t.replace(/"/g,'""')+'"':t}function oe(e,t){const n=new Blob(["\uFEFF"+t],{type:"text/csv;charset=utf-8;"}),o=URL.createObjectURL(n),l=document.createElement("a");l.href=o,l.download=e,l.click(),URL.revokeObjectURL(o)}function Fe(e){e=e.replace(/^\uFEFF/,"").replace(/\r\n/g,`
`).replace(/\r/g,`
`);const t=e.split(`
`).filter(g=>g.trim());if(t.length<2)return[];const n=t[0],o=n.split(";").length>n.split(",").length?";":",",l=Oe(n,o).map(g=>g.trim().replace(/^\uFEFF/,"").toLowerCase()),r=[];for(let g=1;g<t.length;g++){if(!t[g].trim())continue;const b=Oe(t[g],o),h={};l.forEach((i,m)=>{h[i]=(b[m]||"").trim()}),!Object.values(h).every(i=>!i)&&r.push(h)}return r}function Oe(e,t=","){const n=[];let o="",l=!1;for(let r=0;r<e.length;r++){const g=e[r];l?g==='"'?e[r+1]==='"'?(o+='"',r++):l=!1:o+=g:g==='"'?l=!0:g===t?(n.push(o),o=""):o+=g}return n.push(o),n}const bn={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};async function vn(e,{toast:t}){const n=Object.keys(me);let o=n[0];const{data:l}=await _.from("formation_links_overrides").select("formation, links"),r={};(l||[]).forEach(m=>{r[m.formation]=m.links});let g=new Set;function b(m,a){return[m,a].sort().join("-")}function h(m){const a=r[m]||me[m]||[];g=new Set(a.map(([d,p])=>b(d,p)))}h(o),e.innerHTML=`
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;gap:12px;flex-wrap:wrap">
      <div style="display:flex;align-items:center;gap:10px">
        ...
        <select id="formation-select" style="padding:6px 10px;font-size:14px;border-radius:8px;border:1.5px solid #ccc">
          ${n.map(m=>`<option value="${m}" ${m===o?"selected":""}>${m}</option>`).join("")}
        </select>
  ...
  `;function i(){const m=Mt[o]||{},a=ze(o),d=320,p=400,s=22;function f(E){const v=m[E];return v?{x:v.x*d,y:v.y*p}:null}let c=`<svg width="${d}" height="${p}" viewBox="0 0 ${d} ${p}" xmlns="http://www.w3.org/2000/svg" style="display:block">`;a.forEach(([E,v],u)=>{const x=f(E),w=f(v);if(!x||!w)return;const k=b(E,v),L=g.has(k),B=L?"#3b82f6":"#999",A=L?.95:.35,S=L?4:3;c+=`<line x1="${x.x}" y1="${x.y}" x2="${w.x}" y2="${w.y}"
        stroke="transparent" stroke-width="14" class="link-hit" data-key="${k}" style="cursor:pointer"/>`,c+=`<line x1="${x.x}" y1="${x.y}" x2="${w.x}" y2="${w.y}"
        stroke="${B}" stroke-width="${S}" stroke-dasharray="${L?"none":"4,3"}"
        opacity="${A}" pointer-events="none" data-line-key="${k}"/>`});for(const E of Object.keys(m)){const v=f(E);if(!v)continue;const u=E.replace(/\d+/,""),x=bn[u]||"#555";c+=`<circle cx="${v.x}" cy="${v.y}" r="${s}" fill="${x}" stroke="rgba(255,255,255,0.7)" stroke-width="2" pointer-events="none"/>`,c+=`<text x="${v.x}" y="${v.y+4}" text-anchor="middle" font-size="10" font-weight="900" fill="white" font-family="Arial Black,Arial" pointer-events="none">${E}</text>`}c+="</svg>",document.getElementById("field-wrap").innerHTML=c;const y=document.getElementById("links-list");y.innerHTML=a.map(([E,v])=>{const u=b(E,v),x=g.has(u);return`
        <button class="link-toggle" data-key="${u}" style="
        ...
        </button>`}).join("");const I=e.querySelector('div[style*="font-weight:700;font-size:13px;margin-bottom:8px"]');I&&(I.textContent=`Liens (${g.size}/${a.length} actifs)`);function $(E){g.has(E)?g.delete(E):g.add(E),i()}e.querySelectorAll(".link-hit").forEach(E=>{E.addEventListener("click",()=>$(E.dataset.key))}),e.querySelectorAll(".link-toggle").forEach(E=>{E.addEventListener("click",()=>$(E.dataset.key))})}i(),document.getElementById("formation-select").addEventListener("change",m=>{o=m.target.value,h(o),i()}),document.getElementById("reset-btn").addEventListener("click",()=>{const m=me[o]||[];g=new Set(m.map(([a,d])=>b(a,d))),i(),t("Liens réinitialisés aux valeurs par défaut (non sauvegardé)","info")}),document.getElementById("save-btn").addEventListener("click",async()=>{const a=ze(o).filter(([p,s])=>g.has(b(p,s))),{error:d}=await _.from("formation_links_overrides").upsert({formation:o,links:a,updated_at:new Date().toISOString()});if(d){t(d.message,"error");return}r[o]=a,t(`Liens enregistrés pour ${o} (${a.length} actifs)`,"success")})}const xn=[{value:"player",label:"Joueur"},{value:"formation",label:"Formation"},{value:"game_changer",label:"Game Changer"},{value:"ultra_game_changer",label:"Ultra Game Changer"},{value:"game_helper",label:"Game Helper"},{value:"stadium",label:"🏟️ Stade"}],hn=[{value:"",label:"Toutes"},{value:"normal",label:"Normal"},{value:"pepite",label:"Pépite"},{value:"papyte",label:"Papyte"},{value:"legende",label:"Légende"}],En=[{value:"credits",label:"Crédits"},{value:"pub",label:"Publicité"},{value:"free",label:"Gratuit"}];async function _n(e){e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">Chargement...</div>';const{data:t}=await _.from("booster_configs").select("*").order("sort_order");let n=null,o=[],l=null;const r=()=>window.innerWidth<700;async function g(){if(!n){o=[];return}if(n!==l){const{data:a}=await _.from("booster_drop_rates").select("*").eq("booster_id",n).order("sort_order");o=a||[],l=n}}function b(){return`
    ...
    </div>`}function h(){const a=n?(t||[]).find(s=>s.id===n):null;if(!a)return`<div style="display:flex;align-items:center;justify-content:center;flex:1;color:#bbb;font-size:14px;padding:40px;text-align:center">
      ← Sélectionnez un booster ou créez-en un nouveau
    </div>`;const d=o.reduce((s,f)=>s+Number(f.percentage||0),0),p=Math.abs(d-100)<.1;return`
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
          <div>
            ...
            <select id="f-price-type" style="width:100%;padding:9px;border:1px solid #ddd;border-radius:6px;font-size:13px">
              ${En.map(s=>`<option value="${s.value}" ${a.price_type===s.value?"selected":""}>${s.label}</option>`).join("")}
            </select>
          </div>
        ...
        ${o.length===0?'<div style="text-align:center;padding:16px;color:#aaa;font-size:13px">Aucun taux. Cliquez + Ligne.</div>':o.map((s,f)=>`
          <div style="display:grid;grid-template-columns:2fr 1.5fr 50px 50px 60px 32px;gap:4px;align-items:center;margin-bottom:6px;padding:6px;background:#f9f9f9;border-radius:6px" data-rate-idx="${f}">
            <select class="rate-type" data-idx="${f}" style="padding:6px 4px;border:1px solid #ddd;border-radius:4px;font-size:12px;width:100%">
              ${xn.map(c=>`<option value="${c.value}" ${s.card_type===c.value?"selected":""}>${c.label}</option>`).join("")}
            </select>
            <select class="rate-rarity" data-idx="${f}" style="padding:6px 4px;border:1px solid #ddd;border-radius:4px;font-size:12px;width:100%">
              ${hn.map(c=>`<option value="${c.value}" ${(s.rarity||"")===c.value?"selected":""}>${c.label}</option>`).join("")}
            </select>
    ...
    </div>`}async function i(a=!1){a||await g();const d=!n&&r(),p=n&&r();e.innerHTML=`
    <div style="display:flex;height:calc(100vh - 56px);overflow:hidden">
      ${!r()||d||!n?b():""}
      ${!r()||p?h():""}
    </div>`,m()}function m(){var p,s,f,c,y,I,$;const a=E=>e.querySelector(E);e.querySelectorAll(".booster-row").forEach(E=>{E.addEventListener("click",v=>{v.target.closest(".btn-delete")||(n=E.dataset.id,l=null,i())})}),(p=a("#btn-back"))==null||p.addEventListener("click",()=>{n=null,i()}),(s=a("#btn-new"))==null||s.addEventListener("click",async()=>{const E=prompt("Nom du nouveau booster :");if(!(E!=null&&E.trim()))return;const{data:v,error:u}=await _.from("booster_configs").insert({name:E.trim(),price_type:"credits",price_credits:1e3,card_count:5}).select().single();if(u){alert(u.message);return}t.push(v),n=v.id,l=null,i()}),e.querySelectorAll(".btn-delete").forEach(E=>{E.addEventListener("click",async v=>{if(v.stopPropagation(),!confirm("Supprimer ce booster ?"))return;await _.from("booster_configs").delete().eq("id",E.dataset.id);const u=t.findIndex(x=>x.id===E.dataset.id);u>-1&&t.splice(u,1),n===E.dataset.id&&(n=null,l=null),i()})}),(f=a("#btn-cancel"))==null||f.addEventListener("click",()=>{n=null,i()}),(c=a("#f-price-type"))==null||c.addEventListener("change",E=>{const v=a("#credits-field");v&&(v.style.opacity=E.target.value!=="credits"?"0.4":"1")}),(y=a("#btn-pick-icon"))==null||y.addEventListener("click",async()=>{var v;const E=a("#icon-picker-grid");if(E){if(E.style.display!=="none"){E.style.display="none";return}E.innerHTML='<div style="padding:10px;text-align:center;color:#888;font-size:12px">⏳ Chargement...</div>',E.style.display="block";try{const x=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),w=Array.isArray(x)?x.filter(L=>L.name.startsWith("booster")&&/\.(png|jpg|svg|webp)$/i.test(L.name)):[];if(!w.length){E.innerHTML='<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Aucune icône "booster*" trouvée.</div>';return}const k=((v=a("#f-image-url"))==null?void 0:v.value)||"";E.innerHTML=`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(72px,1fr));gap:8px">
          ${w.map(L=>`
    ...
    </div>`,document.getElementById("add-config").addEventListener("click",()=>Ue(null)),document.getElementById("form-cancel").addEventListener("click",()=>{document.getElementById("config-form").style.display="none"}),document.getElementById("form-save").addEventListener("click",()=>$n(e)),e.querySelectorAll("[data-edit]").forEach(o=>{const l=(t||[]).find(r=>r.id===o.dataset.edit);l&&o.addEventListener("click",()=>Ue(l))}),e.querySelectorAll("[data-delete]").forEach(o=>{o.addEventListener("click",async()=>{confirm("Supprimer cette règle ?")&&(await _.from("sell_price_configs").delete().eq("id",o.dataset.delete),await ke(e))})})}function In(e){const t=gt.find(n=>n.value===e.rarity);return`
    ...
    </tr>`}function Ue(e){document.getElementById("config-form").style.display="block",document.getElementById("form-title").textContent=e?"Modifier la règle":"Nouvelle règle",document.getElementById("form-id").value=(e==null?void 0:e.id)||"",document.getElementById("form-rarity").value=(e==null?void 0:e.rarity)||"normal",document.getElementById("form-price").value=(e==null?void 0:e.price)||1e3,document.getElementById("form-note-min").value=(e==null?void 0:e.note_min)??1,document.getElementById("form-note-max").value=(e==null?void 0:e.note_max)??10,document.getElementById("config-form").scrollIntoView({behavior:"smooth"})}async function $n(e){const t=document.getElementById("form-id").value,n=document.getElementById("form-rarity").value,o=parseInt(document.getElementById("form-price").value)||0,l=parseInt(document.getElementById("form-note-min").value)||1,r=parseInt(document.getElementById("form-note-max").value)||10;if(l>r){alert("Note min doit être ≤ note max");return}const g={rarity:n,price:o,note_min:l,note_max:r,updated_at:new Date().toISOString()};let b;if(t?{error:b}=await _.from("sell_price_configs").update(g).eq("id",t):{error:b}=await _.from("sell_price_configs").insert(g),b){alert("Erreur : "+b.message);return}document.getElementById("config-form").style.display="none",await ke(e)}async function kn(e){await Le(e)}async function Le(e){const{data:t}=await _.from("patch_notes").select("*").order("published_at",{ascending:!1});e.innerHTML=`
      ...
      <div style="width:420px;flex-shrink:0;overflow-y:auto">
        ...
        <div style="margin-bottom:16px">
          ...
          <div id="ts-toolbar" style="display:flex;flex-wrap:wrap;gap:4px;padding:6px 8px;background:#f5f5f5;border:1.5px solid #ddd;border-bottom:none;border-radius:8px 8px 0 0">
            ${[["B","bold","<strong>Gras</strong>"],["I","italic","<em>Italique</em>"],["U","underline","Souligné"],["¶","insertParagraph","Paragraphe"],["•","insertUnorderedList","Liste à puces"],["1.","insertOrderedList","Liste numérotée"],["H2","formatBlock:h2","Titre H2"],["H3","formatBlock:h3","Titre H3"],["—","insertHorizontalRule","Séparateur"]].map(([u,x,w])=>`<button type="button" data-cmd="${x}" title="${w}" style="padding:4px 8px;border:1px solid #ddd;border-radius:4px;background:#fff;font-size:12px;font-weight:700;cursor:pointer;line-height:1;min-width:28px">${u}</button>`).join("")}
      ...
      <div style="flex:1;overflow-y:auto">
        ...
        <div id="ts-list" style="display:flex;flex-direction:column;gap:10px">
          ${(t||[]).length===0?`
            ...
            </div>`:(t||[]).map(u=>Cn(u)).join("")}
        </div>
      </div><!-- /colonne droite -->
    </div>`,(m=document.getElementById("ts-add"))==null||m.addEventListener("click",()=>He(null,(t==null?void 0:t.length)||0)),(a=document.getElementById("ts-reset"))==null||a.addEventListener("click",async()=>{if(!confirm("Remettre tutorial_done = false pour TOUS les joueurs ? Ils reverront le tutoriel à leur prochaine connexion."))return;const{error:u}=await _.from("users").update({tutorial_done:!1}).neq("id","00000000-0000-0000-0000-000000000000");alert(u?"Erreur : "+u.message:"✅ Tutorial_done réinitialisé pour tous les joueurs.")}),(d=document.getElementById("ts-cancel"))==null||d.addEventListener("click",ft),(p=document.getElementById("ts-save"))==null||p.addEventListener("click",()=>Mn(e)),(s=document.getElementById("ts-preview-btn"))==null||s.addEventListener("click",Tn);const n=document.getElementById("ts-editor"),o=()=>{const u=document.getElementById("ts-content");u&&n&&(u.value=n.innerHTML)};n==null||n.addEventListener("input",o),n==null||n.addEventListener("blur",o);const l=()=>{document.activeElement!==n&&(n==null||n.focus())};(f=document.getElementById("ts-toolbar"))==null||f.querySelectorAll("[data-cmd]").forEach(u=>{u.addEventListener("mousedown",x=>{x.preventDefault(),l();const w=u.dataset.cmd;w.startsWith("formatBlock:")?document.execCommand("formatBlock",!1,w.split(":")[1]):document.execCommand(w,!1,null),o()})}),(c=document.getElementById("ts-insert-color"))==null||c.addEventListener("mousedown",u=>{u.preventDefault(),l();const x=prompt("Couleur hex (ex: #D4A017 pour or, #1A6B3C pour vert, #cc2222 pour rouge) :");x&&(document.execCommand("foreColor",!1,x),o())}),(y=document.getElementById("ts-clear-format"))==null||y.addEventListener("mousedown",u=>{u.preventDefault(),l(),document.execCommand("removeFormat",!1,null),o()}),n==null||n.addEventListener("focus",()=>{n&&(n.style.borderColor="#1A6B3C")}),n==null||n.addEventListener("blur",()=>{n&&(n.style.borderColor="#ddd")});const r=document.getElementById("ts-image"),g=document.getElementById("ts-img-preview"),b=document.getElementById("ts-img-preview-el"),h=document.getElementById("ts-img-picker-grid"),i=()=>{var x;const u=(x=r==null?void 0:r.value)==null?void 0:x.trim();if(u){const w="/";b.src=`${w}icons/${u}`,g.style.display="block"}else g.style.display="none"};r==null||r.addEventListener("input",i),(I=document.getElementById("ts-img-clear"))==null||I.addEventListener("click",()=>{r&&(r.value=""),g.style.display="none",h.style.display="none"}),($=document.getElementById("ts-img-pick"))==null||$.addEventListener("click",async()=>{if(h.style.display!=="none"){h.style.display="none";return}h.innerHTML='<div style="color:#aaa;font-size:12px">Chargement…</div>',h.style.display="block";try{const x=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),w=Array.isArray(x)?x.filter(L=>L.name.startsWith("tuto_")):[];if(!w.length){h.innerHTML='<div style="color:#aaa;font-size:12px">Aucun fichier tuto_* trouvé dans /icons/.<br>Uploade des images nommées tuto_xxx.png dans public/icons/</div>';return}const k="/";h.innerHTML='<div style="display:flex;flex-wrap:wrap;gap:8px">'+w.map(L=>`
          ...
          </div>`).join("")+"</div>",h.querySelectorAll("[data-pick]").forEach(L=>{L.addEventListener("click",()=>{r&&(r.value=L.dataset.pick),i(),h.style.display="none"})})}catch(u){h.innerHTML='<div style="color:#cc2222;font-size:12px">Erreur chargement : '+u.message+"</div>"}}),(E=document.getElementById("ts-color"))==null||E.addEventListener("input",u=>{const x=document.getElementById("ts-color-hex");x&&(x.value=u.target.value)}),(v=document.getElementById("ts-color-hex"))==null||v.addEventListener("input",u=>{const x=u.target.value;if(/^#[0-9a-fA-F]{6}$/.test(x)){const w=document.getElementById("ts-color");w&&(w.value=x)}}),e.querySelectorAll("[data-edit-row]").forEach(u=>{const x=(t||[]).find(w=>w.id===u.dataset.editRow);x&&u.addEventListener("click",()=>He(x))}),e.querySelectorAll("[data-delete]").forEach(u=>{u.addEventListener("click",async()=>{confirm("Supprimer cette étape ?")&&(await _.from("tutorial_steps").delete().eq("id",u.dataset.delete),W(e))})}),e.querySelectorAll("[data-toggle]").forEach(u=>{u.addEventListener("click",async()=>{const x=(t||[]).find(w=>w.id===u.dataset.toggle);x&&(await _.from("tutorial_steps").update({is_active:!x.is_active}).eq("id",x.id),W(e))})}),e.querySelectorAll("[data-up]").forEach(u=>{u.addEventListener("click",async()=>{const x=t||[],w=x.findIndex(k=>k.id===u.dataset.up);w<=0||(await Promise.all([_.from("tutorial_steps").update({step_order:x[w-1].step_order}).eq("id",x[w].id),_.from("tutorial_steps").update({step_order:x[w].step_order}).eq("id",x[w-1].id)]),W(e))})}),e.querySelectorAll("[data-down]").forEach(u=>{u.addEventListener("click",async()=>{const x=t||[],w=x.findIndex(k=>k.id===u.dataset.down);w<0||w>=x.length-1||(await Promise.all([_.from("tutorial_steps").update({step_order:x[w+1].step_order}).eq("id",x[w].id),_.from("tutorial_steps").update({step_order:x[w].step_order}).eq("id",x[w+1].id)]),W(e))})})}function Cn(e){return`
  <div data-edit-row="${e.id}" style="background:#fff;border-radius:10px;padding:12px 16px;box-shadow:0 1px 4px rgba(0,0,0,0.08);display:flex;align-items:center;gap:12px;cursor:pointer">
  ...
  </div>`}function He(e,t=0){document.getElementById("ts-form-empty").style.display="none",document.getElementById("ts-form-content").style.display="block",document.getElementById("ts-form-title").textContent=e?"Modifier l'étape":"Nouvelle étape",document.getElementById("ts-id").value=(e==null?void 0:e.id)||"",document.getElementById("ts-emoji").value=(e==null?void 0:e.emoji)||"⚽",document.getElementById("ts-title").value=(e==null?void 0:e.title)||"",document.getElementById("ts-content").value=(e==null?void 0:e.content)||"";const n=document.getElementById("ts-editor");n&&(n.innerHTML=(e==null?void 0:e.content)||"<p>Texte de l'étape…</p>"),document.getElementById("ts-order").value=(e==null?void 0:e.step_order)??t,document.getElementById("ts-active").checked=e?e.is_active:!0;const o=(e==null?void 0:e.color)||"#1A6B3C";document.getElementById("ts-color").value=o,document.getElementById("ts-color-hex").value=o;const l=(e==null?void 0:e.image_url)||"",r=document.getElementById("ts-image");r&&(r.value=l);const g=document.getElementById("ts-img-preview"),b=document.getElementById("ts-img-preview-el");if(l&&g&&b){const h="/";b.src=`${h}icons/${l}`,g.style.display="block"}else g&&(g.style.display="none");document.getElementById("ts-img-picker-grid").style.display="none",document.getElementById("ts-preview-area").innerHTML="",document.getElementById("ts-form").scrollIntoView({behavior:"smooth"})}function ft(){document.getElementById("ts-form-content").style.display="none",document.getElementById("ts-form-empty").style.display="block",document.getElementById("ts-preview-area").innerHTML=""}function Tn(){var i,m;const e=document.getElementById("ts-emoji").value||"⚽",t=document.getElementById("ts-title").value||"Titre",n=document.getElementById("ts-editor"),o=n?n.innerHTML:document.getElementById("ts-content").value||"",l=document.getElementById("ts-color-hex").value||"#1A6B3C",r=(m=(i=document.getElementById("ts-image"))==null?void 0:i.value)==null?void 0:m.trim(),b=r?`<div style="padding:0 20px 12px;text-align:center"><img src="/icons/${r}" style="max-height:140px;max-width:100%;border-radius:10px;object-fit:contain"></div>`:"",h=document.getElementById("ts-preview-area");h.innerHTML=`
    ...
    </div>`,h.querySelectorAll("ul,ol").forEach(a=>{a.style.paddingLeft="20px",a.style.margin="6px 0"}),h.querySelectorAll("li").forEach(a=>{a.style.marginBottom="4px"}),h.querySelectorAll("p").forEach(a=>{a.style.marginBottom="8px"})}async function Mn(e){var a,d;const t=document.getElementById("ts-id").value,n=document.getElementById("ts-emoji").value.trim()||"⚽",o=document.getElementById("ts-title").value.trim(),l=document.getElementById("ts-editor"),r=(l?l.innerHTML:document.getElementById("ts-content").value).trim(),g=document.getElementById("ts-color-hex").value.trim()||"#1A6B3C",b=parseInt(document.getElementById("ts-order").value)||0,h=document.getElementById("ts-active").checked;if(!o||!r){alert("Titre et contenu sont obligatoires.");return}const i={emoji:n,title:o,content:r,color:g,step_order:b,is_active:h,image_url:((d=(a=document.getElementById("ts-image"))==null?void 0:a.value)==null?void 0:d.trim())||null};let m;if(t?{error:m}=await _.from("tutorial_steps").update(i).eq("id",t):{error:m}=await _.from("tutorial_steps").insert(i),m){alert("Erreur : "+m.message);return}ft(),W(e)}const yt="/",zn=[["MA","Maroc"],["FR","France"],["AR","Argentine"],["PT","Portugal"],["BR","Brésil"],["ES","Espagne"],["DE","Allemagne"],["GB","Angleterre"],["IT","Italie"],["CM","Cameroun"],["SN","Sénégal"],["NG","Nigéria"],["DK","Danemark"],["NL","Pays-Bas"],["BE","Belgique"],["CI","Côte d'Ivoire"],["AL","Albanie"],["HR","Croatie"],["RS","Serbie"],["TR","Turquie"],["MW","Malawi"],["GH","Ghana"],["ZA","Afrique du Sud"],["EG","Égypte"],["US","États-Unis"],["MX","Mexique"],["CO","Colombie"],["UY","Uruguay"],["CL","Chili"],["JP","Japon"],["KR","Corée du Sud"],["CN","Chine"],["AU","Australie"],["SW","Suisse"],["SE","Suède"],["NO","Norvège"],["PL","Pologne"]];async function Rn(e,t){await ve(e,t)}async function ve(e,t){var l;const[{data:n},{data:o}]=await Promise.all([_.from("stadium_definitions").select("*, club:clubs(encoded_name, logo_url)").order("name"),_.from("clubs").select("id, encoded_name, logo_url").order("encoded_name")]);e.innerHTML=`
      ...
      <div style="flex:1;min-width:220px;display:flex;flex-direction:column;gap:12px">
        ...
        <div>
          ...
          <select id="st-country">
            ...
            ${zn.map(([p,s])=>`<option value="${p}" ${(e==null?void 0:e.country_code)===p?"selected":""}>${s} (${p})</option>`).join("")}
          </select>
        </div>
        <div>
  ...
  `);const i=()=>{var x,w,k,L,B,A,S;const p=((x=document.getElementById("st-name"))==null?void 0:x.value)||"NOM DU STADE",s=(k=(w=document.getElementById("st-image"))==null?void 0:w.value)==null?void 0:k.trim(),f=(B=(L=document.getElementById("st-country"))==null?void 0:L.value)==null?void 0:B.trim(),c=document.getElementById("st-club"),y=(c==null?void 0:c.selectedIndex)||0,I=c&&y>0?c.options[y].text:"",$=((S=(A=c==null?void 0:c.options[y])==null?void 0:A.getAttribute)==null?void 0:S.call(A,"data-logo"))||"";let E=null;s?E=s.startsWith("http")?s:`${yt}icons/${s}`:$?E=$:f&&(E=`https://flagsapi.com/${f.toLowerCase().slice(0,2).toUpperCase()}/flat/64.png`);const v=I||f||"—",u=document.getElementById("st-preview-card");u&&(u.innerHTML=Qe(p,E,`${v}<br>+10 ⭐`,{width:140}))};i(),["st-name","st-club","st-country","st-image"].forEach(p=>{var s,f;(s=document.getElementById(p))==null||s.addEventListener("input",i),(f=document.getElementById(p))==null||f.addEventListener("change",i)}),(m=document.getElementById("st-cancel"))==null||m.addEventListener("click",()=>r()),(a=document.getElementById("st-save"))==null||a.addEventListener("click",async()=>{const p=document.getElementById("st-name").value.trim(),s=document.getElementById("st-club").value||null,f=document.getElementById("st-country").value.trim().toUpperCase()||null,c=document.getElementById("st-image").value.trim()||null;if(!p){g("Le nom est obligatoire","error");return}const y={name:p,club_id:s,country_code:f,image_url:c},{error:I}=e?await _.from("stadium_definitions").update(y).eq("id",e.id):await _.from("stadium_definitions").insert(y);if(I){g("Erreur : "+I.message,"error");return}g(e?"Stade modifié ✅":"Stade créé ✅","success"),r(),ve(n,o)}),(d=document.getElementById("st-delete"))==null||d.addEventListener("click",async()=>{confirm("Supprimer ce stade ?")&&(await _.from("stadium_definitions").delete().eq("id",e.id),g("Stade supprimé","success"),r(),ve(n,o))})}const H=[{value:"game_changer",label:"⚡ Game Changer",border:"#7a28b8",bg:"linear-gradient(135deg,#4a0080,#7a28b8)"},{value:"ultra_game_changer",label:"💎 Ultra Game Changer",border:"#00bcd4",bg:"linear-gradient(135deg,#006080,#00bcd4)"}],Dn=[{value:"purple",label:"Violet",hex:"#7a28b8"},{value:"light_blue",label:"Bleu clair",hex:"#00bcd4"}],G=[{value:"BOOST_STAT",label:"⚡ Boost stats (+N)",hasValue:!0,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"DEBUFF_STAT",label:"💀 Debuff stats (-N)",hasValue:!0,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"GRAY_PLAYER",label:"❄️ Griser un joueur",hasValue:!1,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"REVIVE_PLAYER",label:"💫 Ressusciter un joueur",hasValue:!1,hasTarget:!1,hasCount:!0,hasRoles:!1},{value:"REMOVE_GOAL",label:"🚫 Retirer un but adverse",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1},{value:"ADD_GOAL_DRAW",label:"⚽ +1 but si duel nul",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1},{value:"ADD_SUB",label:"🔄 Ajouter un remplacement",hasValue:!1,hasTarget:!1,hasCount:!0,hasRoles:!1},{value:"CUSTOM",label:"🛠️ Effet custom (hardcodé)",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1}],Ke=["GK","DEF","MIL","ATT"];async function Nn(e,t){await Be(e,t)}async function Be(e,t){var o;const{data:n}=await _.from("gc_definitions").select("*").order("sort_order").order("created_at");e.innerHTML=`
    ...
    </div>`,(o=document.getElementById("gc-add-btn"))==null||o.addEventListener("click",()=>Je(null,e,t)),e.querySelectorAll("[data-edit-gc]").forEach(l=>{l.addEventListener("click",()=>{const r=(n||[]).find(g=>g.id===l.dataset.editGc);r&&Je(r,e,t)})}),e.querySelectorAll(".btn-del-gc").forEach(l=>{l.addEventListener("click",async r=>{r.stopPropagation(),confirm("Supprimer cette carte Game Changer ?")&&(await _.from("gc_definitions").delete().eq("id",l.dataset.del),Be(e,t))})})}function Pn(e){const t=H.find(l=>l.value===e.gc_type)||H[0],n=e.image_url?`/icons/${e.image_url}`:null,o=fe(e.name||"?",n,t.label.split(" ")[0],e.effect||"",{width:130});return`<div data-edit-gc="${e.id}" style="position:relative;cursor:pointer">
    ${o}
    <div style="position:absolute;top:4px;left:4px;z-index:10;display:flex;gap:3px">
  ...
  </div>`}function Je(e,t,n){var d,p,s,f;const{openModal:o,closeModal:l,toast:r}=n,g=!e,b=e||{name:"",gc_type:"game_changer",color:"purple",effect:"",image_url:"",sort_order:0,is_active:!0,effect_type:"BOOST_STAT",effect_params:{}},h=(()=>{const c=b.effect_params||{},y=G.find(I=>I.value===(b.effect_type||"BOOST_STAT"))||G[0];return`
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
      ${y.hasValue?`<div>
      ...
      ${y.hasRoles?`<div>
        ...
        <div style="display:flex;gap:4px;flex-wrap:wrap">
          ${Ke.map(I=>`<label style="display:flex;align-items:center;gap:2px;font-size:11px;cursor:pointer;color:#555">
            <input type="checkbox" class="gc-p-role" value="${I}" ${!c.roles||c.roles.includes(I)?"checked":""}> ${I}
          </label>`).join("")}
        </div>
      </div>`:""}
    </div>`})(),i=`
    <div style="display:flex;justify-content:center;margin-bottom:16px;padding:20px;background:#1a1a2e;border-radius:12px">
      <div id="gc-modal-preview">${fe(b.name||"?",b.image_url?`/icons/${b.image_url}`:null,(H.find(c=>c.value===b.gc_type)||H[0]).label.split(" ")[0],b.effect||"",{width:150})}</div>
    </div>
    ...
    <div style="display:flex;flex-direction:column;gap:12px">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
        <div>
          <label>TYPE</label>
          <select id="gc-type">
            ${H.map(c=>`<option value="${c.value}" ${b.gc_type===c.value?"selected":""}>${c.label}</option>`).join("")}
          </select>
        </div>
        <div>
          <label>COULEUR</label>
          <select id="gc-color">
            ${Dn.map(c=>`<option value="${c.value}" ${b.color===c.value?"selected":""}>${c.label}</option>`).join("")}
          </select>
        </div>
      </div>
      <div>
      ...
      <div style="border-top:1px solid #eee;padding-top:12px;margin-top:4px">
        ...
        <div>
          ...
          <select id="gc-effect-type" style="margin-bottom:10px">
            ${G.map(c=>`<option value="${c.value}" ${(b.effect_type||"BOOST_STAT")===c.value?"selected":""}>${c.label}</option>`).join("")}
          </select>
        </div>
  ...
  `);const a=()=>{var u,x,w,k,L;const c=((u=document.getElementById("gc-name"))==null?void 0:u.value)||"?",y=((x=document.getElementById("gc-effect"))==null?void 0:x.value)||"",I=((w=document.getElementById("gc-type"))==null?void 0:w.value)||"game_changer",$=(L=(k=document.getElementById("gc-image-url"))==null?void 0:k.value)==null?void 0:L.trim(),E=H.find(B=>B.value===I)||H[0],v=document.getElementById("gc-modal-preview");v&&(v.innerHTML=fe(c,$?`/icons/${$}`:null,E.label.split(" ")[0],y,{width:150}))};["gc-name","gc-effect","gc-type","gc-image-url"].forEach(c=>{var y,I;(y=document.getElementById(c))==null||y.addEventListener("input",a),(I=document.getElementById(c))==null||I.addEventListener("change",a)}),(d=document.getElementById("gc-effect-type"))==null||d.addEventListener("change",()=>{const c=document.getElementById("gc-effect-type").value,y=G.find($=>$.value===c)||G[0],I=document.getElementById("gc-params-wrap");I.innerHTML=`
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
    ...
    </div>`}),(p=document.getElementById("btn-pick-gc-icon"))==null||p.addEventListener("click",async()=>{var y;const c=document.getElementById("gc-icon-picker");if(c){if(c.style.display!=="none"){c.style.display="none";return}c.innerHTML='<div style="padding:10px;text-align:center;color:#888;font-size:12px">⏳ Chargement...</div>',c.style.display="block";try{const $=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),E=Array.isArray($)?$.filter(u=>u.name.startsWith("gamechanger-")&&/\.(png|jpg|svg|webp)$/i.test(u.name)):[];if(!E.length){c.innerHTML='<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Aucun fichier "gamechanger-*" trouvé dans public/icons/</div>';return}const v=((y=document.getElementById("gc-image-url"))==null?void 0:y.value)||"";c.innerHTML=`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(72px,1fr));gap:8px">
        ${E.map(u=>`
        <div class="gc-icon-item" data-name="${u.name}"
          style="cursor:pointer;text-align:center;padding:6px;border-radius:6px;border:2px solid ${u.name===v?"#7a28b8":"#ddd"};background:${u.name===v?"#f5f0ff":"#fff"}">
      ...
      </div>`,c.querySelectorAll(".gc-icon-item").forEach(u=>{u.addEventListener("click",()=>{const x=document.getElementById("gc-image-url");x&&(x.value=u.dataset.name),c.style.display="none",a()})})}catch(I){c.innerHTML=`<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Erreur API GitHub : ${I.message}</div>`}}}),(s=document.getElementById("gc-cancel"))==null||s.addEventListener("click",()=>l()),(f=document.getElementById("gc-save"))==null||f.addEventListener("click",async()=>{var v,u,x,w,k,L,B,A,S,C,M,F,O,U;const c=G.find(N=>{var j;return N.value===(((j=document.getElementById("gc-effect-type"))==null?void 0:j.value)||"BOOST_STAT")})||G[0],y=c.hasRoles?[...document.querySelectorAll(".gc-p-role:checked")].map(N=>N.value):null,I={...c.hasValue?{value:Number((v=document.getElementById("gc-p-value"))==null?void 0:v.value)||2}:{},...c.hasCount?{count:Number((u=document.getElementById("gc-p-count"))==null?void 0:u.value)||1}:{},...c.hasTarget?{target:((x=document.getElementById("gc-p-target"))==null?void 0:x.value)||"home"}:{},...c.hasRoles?{roles:y!=null&&y.length?y:null}:{}},$={name:((k=(w=document.getElementById("gc-name"))==null?void 0:w.value)==null?void 0:k.trim())||"",effect:((B=(L=document.getElementById("gc-effect"))==null?void 0:L.value)==null?void 0:B.trim())||null,image_url:((S=(A=document.getElementById("gc-image-url"))==null?void 0:A.value)==null?void 0:S.trim())||null,gc_type:((C=document.getElementById("gc-type"))==null?void 0:C.value)||"game_changer",color:((M=document.getElementById("gc-color"))==null?void 0:M.value)||"purple",sort_order:Number((F=document.getElementById("gc-sort"))==null?void 0:F.value)||0,is_active:((O=document.getElementById("gc-active"))==null?void 0:O.checked)??!0,effect_type:((U=document.getElementById("gc-effect-type"))==null?void 0:U.value)||"BOOST_STAT",effect_params:I};if(!$.name){r("Le nom est obligatoire","error");return}const{error:E}=g?await _.from("gc_definitions").insert($):await _.from("gc_definitions").update($).eq("id",b.id);if(E){r(E.message,"error");return}r(g?"Carte créée ✅":"Carte modifiée ✅","success"),l(),Be(t,n)})}async function Fn(e,{toast:t,openModal:n,closeModal:o}){await Y(e,{toast:t,openModal:n,closeModal:o})}async function Y(e,t){var E;const{toast:n,openModal:o,closeModal:l}=t,[{data:r,error:g},{data:b}]=await Promise.all([_.from("ranked_seasons").select("*").order("start_at",{ascending:!1}),_.from("users").select("mmr,rank_tier,placement_matches").gte("placement_matches",1)]);if(g){e.innerHTML=`<p style="color:red">${g.message}</p>`;return}const h=r||[],i=h.find(v=>v.is_active),m=(b||[]).filter(v=>(v.placement_matches||0)>=1),a=m.filter(v=>(v.placement_matches||0)>=10),d=["bronze","silver","gold","platinum","diamond","master"],p={bronze:"Bronze",silver:"Argent",gold:"Or",platinum:"Platine",diamond:"Diamant",master:"Maître"},s={bronze:"#cd7f32",silver:"#a0a0a0",gold:"#D4A017",platinum:"#00e5ff",diamond:"#5b9bd5",master:"#9b59b6"},f={bronze:"🥉",silver:"🥈",gold:"🥇",platinum:"💎",diamond:"🔷",master:"👑"},c={};d.forEach(v=>{c[v]=0}),a.forEach(v=>{const u=v.rank_tier||"bronze";c[u]!==void 0&&c[u]++});const y=a.length?Math.round(a.reduce((v,u)=>v+(u.mmr||1e3),0)/a.length):0;function I(v){return v?new Date(v).toLocaleDateString("fr",{day:"numeric",month:"short",year:"numeric"}):"—"}function $(v){const u=new Date;return v.is_active?'<span style="background:#1A6B3C;color:#fff;padding:2px 8px;border-radius:10px;font-size:11px;font-weight:700">● ACTIVE</span>':new Date(v.end_at)<u?'<span style="background:#555;color:#ccc;padding:2px 8px;border-radius:10px;font-size:11px">Terminée</span>':'<span style="background:#e67e22;color:#fff;padding:2px 8px;border-radius:10px;font-size:11px">À venir</span>'}e.innerHTML=`
    <div style="display:flex;flex-direction:column;gap:20px">
      ...
      <div class="card-panel" style="background:linear-gradient(135deg,#0a3d1e,#1a6b3c20);border:1.5px solid #1A6B3C40">
        ...
        <div style="display:flex;flex-direction:column;gap:4px">
          ${d.map(v=>{const u=c[v],x=a.length>0?Math.round(u/a.length*100):0;return`
              <div style="display:flex;align-items:center;gap:8px;font-size:12px">
                <span style="width:80px">${f[v]} ${p[v]}</span>
                ...
                <span style="width:40px;text-align:right;color:var(--gray-600)">${u} (${x}%)</span>
              </div>`}).join("")}
        ...
        <div class="table-wrap">
          <table>
            ...
            <tbody id="seasons-tbody">
              ${h.length===0?'<tr><td colspan="6" style="text-align:center;color:var(--gray-600);padding:24px">Aucune saison créée.</td></tr>':h.map(v=>{const u=Math.round((new Date(v.end_at)-new Date(v.start_at))/864e5),x=new Date(v.end_at)<new Date&&!v.is_active;return`
                    <tr>
                      <td><b>${v.name}</b></td>
                      <td style="font-size:12px">${I(v.start_at)}</td>
                      <td style="font-size:12px">${I(v.end_at)}</td>
                      <td style="font-size:12px">${u} jours</td>
                      <td>${$(v)}</td>
    ...
    </div>`,(E=document.getElementById("create-season-btn"))==null||E.addEventListener("click",()=>{Ve(null,{toast:n,openModal:o,closeModal:l,reload:()=>Y(e,t)})}),e.querySelectorAll("[data-edit]").forEach(v=>{const u=h.find(x=>x.id==v.dataset.edit);v.addEventListener("click",()=>{Ve(u,{toast:n,openModal:o,closeModal:l,reload:()=>Y(e,t)})})}),e.querySelectorAll("[data-activate]").forEach(v=>{v.addEventListener("click",async()=>{const u=parseInt(v.dataset.activate);if(!confirm("Activer cette saison ? La saison actuellement active sera désactivée."))return;const{error:x}=await _.from("ranked_seasons").update({is_active:!1}).neq("id",0);if(x){n(x.message,"error");return}const{error:w}=await _.from("ranked_seasons").update({is_active:!0}).eq("id",u);if(w){n(w.message,"error");return}await _.from("users").update({current_season_id:u}).gt("placement_matches",0),n("Saison activée ✅","success"),Y(e,t)})}),e.querySelectorAll("[data-deactivate]").forEach(v=>{v.addEventListener("click",async()=>{if(!confirm("Désactiver la saison active ? Le mode Ranked sera suspendu."))return;const{error:u}=await _.from("ranked_seasons").update({is_active:!1}).eq("id",parseInt(v.dataset.deactivate));if(u){n(u.message,"error");return}n("Saison désactivée","success"),Y(e,t)})}),e.querySelectorAll("[data-delete]").forEach(v=>{v.addEventListener("click",async()=>{if(!confirm("Supprimer cette saison archivée ?"))return;const{error:u}=await _.from("ranked_seasons").delete().eq("id",parseInt(v.dataset.delete));if(u){n(u.message,"error");return}n("Saison supprimée","success"),Y(e,t)})})}function Ve(e,{toast:t,openModal:n,closeModal:o,reload:l}){const r=!!e,g=new Date().toISOString().slice(0,16),b=new Date(Date.now()+90*864e5).toISOString().slice(0,16),h=e?new Date(e.start_at).toISOString().slice(0,16):g,i=e?new Date(e.end_at).toISOString().slice(0,16):b,m=(e==null?void 0:e.name)||"";n(r?"✏️ Modifier la saison":"➕ Nouvelle saison",`<div style="display:flex;flex-direction:column;gap:14px">
      <div class="form-group">
    ...
    </div>`);function a(){var c,y;const d=(c=document.getElementById("season-start"))==null?void 0:c.value,p=(y=document.getElementById("season-end"))==null?void 0:y.value,s=document.getElementById("season-duration");if(!d||!p||!s)return;const f=Math.round((new Date(p)-new Date(d))/864e5);s.textContent=f>0?`Durée : ${f} jour${f>1?"s":""}`:"⚠️ La fin doit être après le début",s.style.color=f>0?"var(--gray-600)":"#bb2020"}setTimeout(()=>{var d,p,s;(d=document.getElementById("season-start"))==null||d.addEventListener("input",a),(p=document.getElementById("season-end"))==null||p.addEventListener("input",a),a(),(s=document.getElementById("season-save-btn"))==null||s.addEventListener("click",async()=>{var u,x,w;const f=(u=document.getElementById("season-name"))==null?void 0:u.value.trim(),c=(x=document.getElementById("season-start"))==null?void 0:x.value,y=(w=document.getElementById("season-end"))==null?void 0:w.value,I=document.getElementById("season-error");if(!f){I.textContent="Le nom est requis.";return}if(!c){I.textContent="La date de début est requise.";return}if(!y){I.textContent="La date de fin est requise.";return}if(new Date(y)<=new Date(c)){I.textContent="La date de fin doit être après le début.";return}const $=document.getElementById("season-save-btn");$.disabled=!0,$.textContent="Enregistrement…";const E={name:f,start_at:new Date(c).toISOString(),end_at:new Date(y).toISOString()};let v;if(r?{error:v}=await _.from("ranked_seasons").update(E).eq("id",e.id):{error:v}=await _.from("ranked_seasons").insert({...E,is_active:!1}),v){I.textContent=v.message,$.disabled=!1,$.textContent=r?"💾 Enregistrer":"✅ Créer la saison";return}t(r?"Saison modifiée ✅":"Saison créée ✅","success"),o(),l()})},50)}async function xe(e,t){var h;const{toast:n}=t,{data:o,error:l}=await _.from("solo_levels").select("*").order("level_number");if(l){e.innerHTML=`<p style="color:var(--danger)">${l.message}</p>`;return}const r=i=>`
    <tr data-row="${i.id}">
  ...
  `;function g(){document.querySelectorAll("#solo-tbody [data-field]").forEach(i=>{i.addEventListener("change",()=>b(i))})}async function b(i){const m=i.dataset.id,a=i.dataset.field,d=a==="is_active"?i.checked:Number(i.value)||0,{error:p}=await _.from("solo_levels").update({[a]:d}).eq("id",m);if(p){n(p.message,"error");return}i.style.transition="background .3s",i.style.background="#e8f8ee",setTimeout(()=>{i.style.background=""},500)}g(),document.querySelectorAll("[data-del-row]").forEach(i=>{i.addEventListener("click",async()=>{confirm("Supprimer ce niveau ? Les joueurs ayant progressé au-delà seront affectés.")&&(await _.from("solo_levels").delete().eq("id",i.dataset.delRow),n("Niveau supprimé","success"),xe(e,t))})}),(h=document.getElementById("solo-add-row"))==null||h.addEventListener("click",async()=>{const m={level_number:Math.max(0,...(o||[]).map(p=>p.level_number))+1,target_note_avg:10,nb_liens_jaune:2,nb_liens_vert:1,nb_joueurs_stade:2,reward_credits:500,is_active:!0},{data:a,error:d}=await _.from("solo_levels").insert(m).select().single();if(d){n(d.message,"error");return}n("Niveau ajouté — modifie les valeurs directement","success"),xe(e,t)})}zt(Ze);function On(e,t="info"){const n=document.getElementById("toast");n.textContent=e,n.className=`show ${t}`,clearTimeout(n._t),n._t=setTimeout(()=>{n.className=""},3e3)}function Un(e,t,n=""){document.getElementById("modal-title").textContent=e,document.getElementById("modal-body").innerHTML=t,document.getElementById("modal-footer").innerHTML=n,document.getElementById("modal-overlay").classList.remove("hidden")}function he(){document.getElementById("modal-overlay").classList.add("hidden")}const Ye={dashboard:{title:"Dashboard",fn:Rt},players:{title:"Joueurs & Cartes",fn:Pt},clubs:{title:"Clubs",fn:Zt},"card-builder":{title:"Card Builder",fn:sn},users:{title:"Managers",fn:ut},market:{title:"Mercato",fn:mt},"import-export":{title:"Import / Export CSV",fn:yn},formations:{title:"Formations & Liens",fn:vn},"boosters-config":{title:"Boosters",fn:_n},"sell-price":{title:"Prix vente directe",fn:wn},journal:{title:"Actualités",fn:kn},tutorial:{title:"Tutoriel",fn:Sn},stadiums:{title:"Stades",fn:Rn},"gc-cards":{title:"Game Changers",fn:Nn},"ranked-seasons":{title:"Saisons Ranked",fn:Fn},"solo-mode":{title:"Solo Mode",fn:xe}};async function ce(e){document.querySelectorAll(".admin-sidebar nav a").forEach(l=>{l.classList.toggle("active",l.dataset.page===e)});const t=document.getElementById("mobile-page-select");t&&(t.value=e);const n=Ye[e]||Ye.dashboard;document.getElementById("page-title").textContent=n.title;const o=document.getElementById("page-content");o.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">Chargement…</div>';try{await n.fn(o,{toast:On,openModal:Un,closeModal:he,navigate:ce})}catch(l){o.innerHTML=`<div style="padding:40px;color:#c0392b">Erreur: ${l.message}</div>`,console.error(l)}}async function Gn(){var t;const{data:{session:e}}=await _.auth.getSession();e&&await We(e.user),document.getElementById("auth-btn").addEventListener("click",async()=>{const n=document.getElementById("auth-email").value.trim(),o=document.getElementById("auth-password").value,l=document.getElementById("auth-error");if(l.textContent="",!n||!o){l.textContent="Remplissez tous les champs.";return}const{data:r,error:g}=await _.auth.signInWithPassword({email:n,password:o});if(g){l.textContent=g.message;return}await We(r.user)}),document.getElementById("auth-password").addEventListener("keydown",n=>{n.key==="Enter"&&document.getElementById("auth-btn").click()}),document.getElementById("logout-btn").addEventListener("click",async()=>{await _.auth.signOut(),document.getElementById("auth-screen").style.display="flex",document.getElementById("admin-app").style.display="none"}),document.getElementById("modal-close").addEventListener("click",he),document.getElementById("modal-overlay").addEventListener("click",n=>{n.target===n.currentTarget&&he()}),document.querySelectorAll(".admin-sidebar nav a").forEach(n=>{n.addEventListener("click",o=>{o.preventDefault(),ce(n.dataset.page)})}),(t=document.getElementById("mobile-page-select"))==null||t.addEventListener("change",n=>{ce(n.target.value)})}async function We(e){const{data:t,error:n}=await _.from("users").select("*").eq("id",e.id).single(),o=document.getElementById("auth-error");if(n||!t){o.textContent="Profil introuvable.";return}if(!t.is_admin){o.textContent="Accès refusé. Vous n'êtes pas administrateur.",await _.auth.signOut();return}document.getElementById("auth-screen").style.display="none",document.getElementById("admin-app").style.display="flex",document.getElementById("admin-user").textContent=`Admin: ${t.pseudo}`,ce("dashboard")}Gn();
