import{s as E,r as Fe,a as Pe,A as tt,l as nt,g as it,b as Oe,K as _e,D as R,S as ot,c as D,H as at,R as rt,e as pe,d as Ue,f as lt,h as st,E as dt,i as ct,N as pt,M as ut,B as mt,j as gt,F as ue,k as we,m as ft,n as bt,o as yt,p as xt}from"./special-cards-CrbcHJPn.js";async function vt(e){var y;const[{count:t},{count:n},{count:i},{count:u},{count:d},{data:p}]=await Promise.all([E.from("players").select("*",{count:"exact",head:!0}),E.from("clubs").select("*",{count:"exact",head:!0}),E.from("users").select("*",{count:"exact",head:!0}),E.from("stadium_definitions").select("*",{count:"exact",head:!0}),E.from("users").select("*",{count:"exact",head:!0}).gt("last_seen_at",new Date(Date.now()-18e4).toISOString()),E.rpc("get_signup_password")]),l=p||"";e.innerHTML=`
    ...
    <div class="card-panel">
      ...
      <div style="display:flex;gap:8px;max-width:420px">
        <input id="signup-pwd-input" type="text" value="${l.replace(/"/g,"&quot;")}" style="flex:1">
  ...
  `,window.adminNav=o=>{var c;(c=document.querySelector(`[data-page="${o}"]`))==null||c.click()},(y=document.getElementById("signup-pwd-save"))==null||y.addEventListener("click",async()=>{const o=document.getElementById("signup-pwd-input").value.trim(),c=document.getElementById("signup-pwd-status");if(!o){c.textContent="Le code ne peut pas être vide.",c.style.color="#ff6b6b";return}const r=document.getElementById("signup-pwd-save");r.disabled=!0,r.textContent="⏳...";const{error:a}=await E.rpc("set_signup_password",{new_password:o});if(r.disabled=!1,r.textContent="💾 Enregistrer",a){c.textContent=a.message,c.style.color="#ff6b6b";return}c.textContent="✅ Code mis à jour.",c.style.color="#2ecc71"})}const Ge={normal:"Normal",pepite:"Pépite",papyte:"Papyte",legende:"Légende"},ht={GK:"#aaaaaa",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};async function Et(){const e={};for(const t of tt)e[t]=await nt(t);return e}async function _t(e,t){const{toast:n}=t;if(!confirm(`Réattribuer une nouvelle photo à tous les joueurs ayant encore l'ancien format de chemin (public/faces/...) ?

Cette action est irréversible.`))return;const i=document.getElementById("fix-old-faces-btn");i&&(i.disabled=!0,i.textContent="⏳ En cours...");try{const{data:u,error:d}=await E.from("players").select("id, country_code, face").like("face","public/faces/%");if(d){n(d.message,"error");return}if(!(u!=null&&u.length)){n("Aucun joueur avec l'ancien format — rien à faire ✅","success");return}const{data:p}=await E.from("players").select("face").not("face","is",null),l=new Set((p||[]).map(c=>c.face).filter(c=>c&&!c.startsWith("public/faces/")));let y=0,o=0;for(const c of u){const r=await Pe(c.country_code,l);if(!r){o++;continue}const{error:a}=await E.from("players").update({face:r}).eq("id",c.id);if(a){o++;continue}l.add(r),y++}n(`${y} photo(s) réattribuée(s)${o?`, ${o} échec(s)`:""} ✅`,o?"error":"success")}finally{i&&(i.disabled=!1,i.textContent="🖼️ Réattribuer anciennes photos")}}async function wt(e,t){await se(e,t)}async function se(e,t,n=null){var o,c,r,a,m;const{toast:i}=t;n||(n={search:((o=document.getElementById("search-players"))==null?void 0:o.value)||"",job:((c=document.getElementById("filter-job"))==null?void 0:c.value)||"",rarity:((r=document.getElementById("filter-rarity"))==null?void 0:r.value)||"",club:((a=document.getElementById("filter-club"))==null?void 0:a.value)||"",country:((m=document.getElementById("filter-country"))==null?void 0:m.value)||""});const[{data:u,error:d},{data:p}]=await Promise.all([E.from("players").select("*, clubs(id,encoded_name,logo_url)"),E.from("clubs").select("id,encoded_name").order("encoded_name")]);if(d){e.innerHTML=`<p style="color:red">${d.message}</p>`;return}const l={GK:0,DEF:1,MIL:2,ATT:3},y=(u||[]).sort((s,h)=>{const _=(l[s.job]??4)-(l[h.job]??4);return _!==0?_:(s.surname_real||"").localeCompare(h.surname_real||"")});kt(e,y,p||[],t,n)}function kt(e,t,n,i,u=null){var c,r;const{toast:d}=i;if(e.innerHTML=`
    <div style="display:flex;flex-direction:column;gap:10px">
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        ...
        <select id="filter-rarity" style="width:110px">
          ...
          ${Object.entries(Ge).map(([a,m])=>`<option value="${a}">${m}</option>`).join("")}
        </select>
      ...
      <div style="flex:1;min-width:300px;display:flex;flex-direction:column;gap:12px">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:8px">
          ...
          <div class="form-group">
            <label>Rareté</label>
            <select id="pm-rarity">
              ${["normal","pepite","papyte","legende"].map(s=>`<option value="${s}" ${(e==null?void 0:e.rarity)===s?"selected":""}>${Ge[s]}</option>`).join("")}
            </select>
          </div>
          <div class="form-group">
            <label>Pays</label>
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:8px">
            ${[["GK","pm-g","note_g"],["DEF","pm-d","note_d"],["MIL","pm-m","note_m"],["ATT","pm-a","note_a"]].map(([s,h,_])=>`
              <div class="form-group" style="text-align:center">
                <label style="color:${ht[s]};font-weight:700">${s}</label>
    ...
    <div id="clubs-list" style="display:flex;flex-direction:column;gap:8px"></div>`,p(ae),document.getElementById("search-clubs").addEventListener("input",l=>{const y=l.target.value.toLowerCase();p(ae.filter(o=>o.real_name.toLowerCase().includes(y)||o.encoded_name.toLowerCase().includes(y)))}),document.getElementById("add-club-btn").addEventListener("click",()=>$e(null,e,t));function p(l){const y=document.getElementById("clubs-list");if(!l.length){y.innerHTML='<p style="color:var(--gray-600);padding:20px">Aucun club.</p>';return}y.innerHTML=l.map(o=>{const c=Je(o),r=Oe(c,o.id).replace("<svg ",'<svg style="width:40px;height:48px" '),a=o.logo_url?`<img src="${o.logo_url}" style="width:40px;height:40px;object-fit:contain;border-radius:8px">`:`<div style="width:40px;height:40px;background:linear-gradient(135deg,${c.color1},${c.color2});border-radius:8px;display:flex;align-items:center;justify-content:center;color:#fff;font-size:10px;font-weight:900">${o.encoded_name.slice(0,3)}</div>`,m=n[o.id]||0;return`
    ...
    </div>`),setTimeout(()=>{var b,I,k;ee();function c(){var v,w;const x=((v=document.getElementById("m-kit-style"))==null?void 0:v.value)||"uni",g=((w=_e[x])==null?void 0:w.colors)===3,f=document.getElementById("wrap-m-kit-color3");f&&(f.style.display=g?"":"none")}c(),["m-kit-color1","m-kit-color2","m-kit-color3","m-kit-shorts","m-kit-socks"].forEach(x=>{const g=document.getElementById(x),f=document.getElementById(x+"-txt");!g||!f||(g.addEventListener("input",()=>{f.value=g.value,ee()}),f.addEventListener("input",()=>{const v=f.value.trim();/^#[0-9a-fA-F]{6}$/.test(v)&&(g.value=v,ee())}),f.addEventListener("change",()=>{let v=f.value.trim();v.startsWith("#")||(v="#"+v),/^#[0-9a-fA-F]{6}$/.test(v)&&(g.value=v,f.value=v,ee())}))}),(b=document.getElementById("m-kit-style"))==null||b.addEventListener("change",()=>{c(),ee()});const r=document.getElementById("m-real"),a=document.getElementById("m-encoded");function m(){if(!r||!a||a.value)return;const x=r.value.trim().split(/\s+/),g=x.length===1?x[0].toUpperCase().slice(0,6):x.filter(f=>f.length>2).map(f=>f[0].toUpperCase()).join("")||x[0].toUpperCase().slice(0,4);a.value=g}r==null||r.addEventListener("input",m),(I=document.getElementById("auto-encode-btn"))==null||I.addEventListener("click",()=>{a&&(a.value=""),m()}),(k=document.getElementById("m-save"))==null||k.addEventListener("click",()=>Pt(e,p,t,n));const s=document.getElementById("m-gen-squad"),h=document.getElementById("m-gen-strong-label");function _(){if(h&&(h.style.display=s!=null&&s.checked?"flex":"none",!(s!=null&&s.checked))){const x=document.getElementById("m-gen-strong");x&&(x.checked=!1)}}s==null||s.addEventListener("change",_),_()},50)}function Ve(){var e,t,n,i,u,d;return{style:((e=document.getElementById("m-kit-style"))==null?void 0:e.value)||"uni",color1:((t=document.getElementById("m-kit-color1"))==null?void 0:t.value)||"#1A6B3C",color2:((n=document.getElementById("m-kit-color2"))==null?void 0:n.value)||"#ffffff",color3:((i=document.getElementById("m-kit-color3"))==null?void 0:i.value)||"#000000",shorts:((u=document.getElementById("m-kit-shorts"))==null?void 0:u.value)||"#111111",socks:((d=document.getElementById("m-kit-socks"))==null?void 0:d.value)||"#ffffff"}}function ee(){const e=document.getElementById("kit-preview-wrap");e&&(e.innerHTML=Oe(Ve(),"modal"))}async function Pt(e,t,n,i){var I,k,x,g,f,v,w;const{toast:u,closeModal:d}=i,p=document.getElementById("m-error"),l=document.getElementById("m-save"),y=(I=document.getElementById("m-real"))==null?void 0:I.value.trim(),o=(k=document.getElementById("m-encoded"))==null?void 0:k.value.trim().toUpperCase(),c=(x=document.getElementById("m-country"))==null?void 0:x.value.trim().toUpperCase(),r=((g=document.getElementById("m-logo"))==null?void 0:g.value.trim())||null,a=((f=document.getElementById("m-gen-stadium"))==null?void 0:f.checked)??!1,m=((v=document.getElementById("m-gen-squad"))==null?void 0:v.checked)??!1,s=((w=document.getElementById("m-gen-strong"))==null?void 0:w.checked)??!1,h=Ve();if(!y){p.textContent="Le nom du club est requis.";return}if(!o){p.textContent="Le nom encodé est requis.";return}if(!c){p.textContent="Le pays est requis.";return}l.disabled=!0,l.textContent="Enregistrement…";const _={real_name:y,encoded_name:o,country_code:c,logo_url:r,kit_style:h.style,kit_color1:h.color1,kit_color2:h.color2,kit_color3:h.color3,kit_shorts:h.shorts,kit_socks:h.socks};let b=e==null?void 0:e.id;if(t){const{error:A}=await E.from("clubs").update(_).eq("id",b);if(A){p.textContent=A.message,l.disabled=!1,l.textContent="💾 Enregistrer";return}}else{const{data:A,error:$}=await E.from("clubs").insert(_).select().single();if($){p.textContent=$.message,l.disabled=!1,l.textContent="✅ Créer le club";return}if(b=A.id,a){l.textContent="🏟️ Création du stade…";const{data:L,error:B}=await E.from("stadium_definitions").insert({name:`Stade de ${y}`,club_id:b,country_code:c}).select().single();B?console.warn("[Stadium] Erreur def stade:",B.message):L&&await E.from("cards").insert({card_type:"stadium",stadium_id:L.id})}m&&(l.textContent="⚽ Génération des joueurs…",await Ke(b,c,u,s))}u(t?"Club modifié ✅":"Club créé ✅","success"),d(),de(n,i)}const Ot=["normal","pepite","papyte","legende"],Ae=["GK","DEF","MIL","ATT"],Ut=[["MA","Maroc"],["FR","France"],["AR","Argentine"],["PT","Portugal"],["BR","Brésil"],["ES","Espagne"],["DE","Allemagne"],["GB","Angleterre"],["IT","Italie"],["CM","Cameroun"],["SN","Sénégal"],["NG","Nigéria"],["DK","Danemark"],["NL","Pays-Bas"],["BE","Belgique"],["CI","Côte d'Ivoire"],["AL","Albanie"],["HR","Croatie"],["RS","Serbie"],["TR","Turquie"],["MW","Malawi"]];let q={...D},re=[];async function Gt(e,{toast:t}){const{data:n}=await E.from("clubs").select("id,encoded_name,logo_url,kit_style,kit_color1,kit_color2,kit_shorts,kit_socks").order("encoded_name");re=n||[],q={...D},e.innerHTML=Jt(re),qt(e,re,t),G()}function Le(){var n;const e=((n=document.getElementById("cb-surname-real"))==null?void 0:n.value)||"",t=document.getElementById("cb-surname-enc");t&&(t.value=pe(e))}function Ht(){var n;const e=(n=document.getElementById("cb-club"))==null?void 0:n.value,t=re.find(i=>String(i.id)===String(e));return t?{style:t.kit_style||R.style,color1:t.kit_color1||R.color1,color2:t.kit_color2||R.color2,shorts:t.kit_shorts||R.shorts,socks:t.kit_socks||R.socks}:{...R}}function G(){var a,m,s,h;const e=_=>{var b;return((b=document.getElementById(_))==null?void 0:b.value)||""},t={firstname:e("cb-firstname"),surname_real:e("cb-surname-enc")||pe(e("cb-surname-real")),country_code:e("cb-country"),club_encoded_name:((m=(a=document.getElementById("cb-club"))==null?void 0:a.selectedOptions[0])==null?void 0:m.text)||"",job:e("cb-job"),job2:e("cb-job2")||null,note_g:parseInt(e("cb-note-g"))||0,note_d:parseInt(e("cb-note-d"))||0,note_m:parseInt(e("cb-note-m"))||0,note_a:parseInt(e("cb-note-a"))||0,rarity:e("cb-rarity")},n=Ht(),i=document.getElementById("cb-club"),u=((h=(s=i==null?void 0:i.selectedOptions[0])==null?void 0:s.dataset)==null?void 0:h.logo)||null,d=Ue(q,n,120),p=`data:image/svg+xml;charset=utf-8,${encodeURIComponent(d)}`,l=lt(t,{portraitUrl:p,clubLogoUrl:u,showNotes:!0}),y=document.getElementById("card-preview");y&&(y.innerHTML=l);const o=document.getElementById("avatar-preview-wrap");o&&(o.innerHTML=d);const c=e("cb-surname-real"),r=document.getElementById("encode-summary");r&&c&&(r.textContent=`${e("cb-firstname")} ${c} → ${e("cb-firstname")} ${t.surname_real}`)}function Ye(){const e=t=>{var n;return((n=document.getElementById(t))==null?void 0:n.value)||""};q={skin:e("av-skin"),hair:e("av-hair"),hairColor:e("av-hairColor"),eyes:e("av-eyes"),nose:e("av-nose"),mouth:e("av-mouth"),eyebrows:e("av-eyebrows"),beard:e("av-beard")}}function qt(e,t,n){var u,d,p,l,y;e.querySelectorAll("input,select").forEach(o=>{o.addEventListener("input",G),o.addEventListener("change",G)});const i=["av-skin","av-hair","av-hairColor","av-eyes","av-nose","av-mouth","av-eyebrows","av-beard"];i.forEach(o=>{var c;(c=document.getElementById(o))==null||c.addEventListener("change",()=>{Ye(),G()})}),(u=document.getElementById("cb-surname-real"))==null||u.addEventListener("input",()=>{Le(),G()}),(d=document.getElementById("btn-encode"))==null||d.addEventListener("click",()=>{Le(),G()}),(p=document.getElementById("cb-club"))==null||p.addEventListener("change",G),(l=document.getElementById("btn-save-player"))==null||l.addEventListener("click",()=>Kt(n)),(y=document.getElementById("btn-reset"))==null||y.addEventListener("click",()=>{e.querySelectorAll("input").forEach(o=>o.value=""),e.querySelectorAll("select").forEach(o=>o.selectedIndex=0),q={...D},i.forEach(o=>{const c=o.replace("av-",""),r=document.getElementById(o);r&&(r.value=D[c]||"")}),G()})}async function Kt(e){const t=p=>{var l;return((l=document.getElementById(p))==null?void 0:l.value)||""},n=t("cb-surname-real").trim(),i=t("cb-surname-enc").trim()||pe(n);if(!t("cb-firstname")||!n||!t("cb-country")||!t("cb-job")){e("Remplissez les champs obligatoires (prénom, nom, pays, poste)","error");return}Ye();const u={firstname:t("cb-firstname").trim(),surname_real:n,surname_real:i,country_code:t("cb-country"),club_id:t("cb-club")||null,job:t("cb-job"),job2:t("cb-job2")||null,note_g:parseInt(t("cb-note-g"))||0,note_d:parseInt(t("cb-note-d"))||0,note_m:parseInt(t("cb-note-m"))||0,note_a:parseInt(t("cb-note-a"))||0,rarity:t("cb-rarity")||"normal",note_min:parseInt(t("cb-note-min"))||null,note_max:parseInt(t("cb-note-max"))||null,sell_price:parseInt(t("cb-price"))||0,avatar_config:q,skin:q.skin||"blanc",hair:q.hairColor||"marron",hair_length:"court"},{error:d}=await E.from("players").insert(u);if(d){e("Erreur: "+d.message,"error");return}e(`✅ Joueur "${u.firstname} ${u.surname_real}" enregistré !`,"success")}function Jt(e){const t=Ut.map(([o,c])=>`<option value="${o}">${c} (${o})</option>`).join(""),n='<option value="">— Sélectionner un club —</option>'+e.map(o=>`<option value="${o.id}" data-logo="${o.logo_url||""}">${o.encoded_name}</option>`).join(""),i=Ae.map(o=>`<option value="${o}">${o}</option>`).join(""),u='<option value="">Aucun</option>'+Ae.map(o=>`<option value="${o}">${o}</option>`).join(""),d=Ot.map(o=>`<option value="${o}">${o.charAt(0).toUpperCase()+o.slice(1)}</option>`).join(""),p=(o,c,r)=>Object.entries(c).map(([a,m])=>`<option value="${a}" ${a===(r||"")?"selected":""}>${m.label}</option>`).join(""),l=Object.keys(ot).map(o=>`<option value="${o}" ${o===D.skin?"selected":""}>${o.charAt(0).toUpperCase()+o.slice(1)}</option>`).join(""),y=Object.keys(at).map(o=>`<option value="${o}" ${o===D.hairColor?"selected":""}>${o.charAt(0).toUpperCase()+o.slice(1)}</option>`).join("");return`
        ...
        <div style="display:grid;grid-template-columns:1fr 120px;gap:16px;align-items:start">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
            ...
            <div>
              ...
              <select id="av-hair">${p("av-hair",st,D.hair)}</select>
            </div>
            <div>
            ...
            <div>
              ...
              <select id="av-eyebrows">${p("av-eyebrows",dt,D.eyebrows)}</select>
            </div>
            <div>
              <label>Yeux</label>
              <select id="av-eyes">${p("av-eyes",ct,D.eyes)}</select>
            </div>
            <div>
              <label>Nez</label>
              <select id="av-nose">${p("av-nose",pt,D.nose)}</select>
            </div>
            <div>
              <label>Bouche</label>
              <select id="av-mouth">${p("av-mouth",ut,D.mouth)}</select>
            </div>
            <div>
              <label>Barbe</label>
              <select id="av-beard">${p("av-beard",mt,D.beard)}</select>
            </div>
          </div>
  ...
  `,d(u),document.getElementById("search-users").addEventListener("input",p=>{const l=p.target.value.toLowerCase();d(u.filter(y=>{var o;return y.pseudo.toLowerCase().includes(l)||((o=y.club_name)==null?void 0:o.toLowerCase().includes(l))}))});function d(p){document.getElementById("users-tbody").innerHTML=p.map(l=>{const y=l.mmr??1e3,o=l.mmr_deviation??350,c=l.placement_matches??0,r=gt(y),a=l.ranked_wins??0,m=l.ranked_losses??0,s=l.ranked_draws??0,h=a+m+s,_=h>0?Math.round(a/h*100):null,b=c<10,I=o<80?"#1A6B3C":o<150?"#e67e22":"#bb2020";return`
        <tr>
          <td>
  ...
  `,document.querySelectorAll("[data-cancel]").forEach(l=>{l.addEventListener("click",async()=>{if(!confirm("Annuler cette annonce ?"))return;const{error:y}=await E.from("market_listings").update({status:"cancelled"}).eq("id",l.dataset.cancel);y?t(y.message,"error"):(t("Annonce annulée","success"),Ze(e,{toast:t}))})})}async function Vt(e,{toast:t}){e.innerHTML=`
...
`)?'"'+t.replace(/"/g,'""')+'"':t}function ne(e,t){const n=new Blob(["\uFEFF"+t],{type:"text/csv;charset=utf-8;"}),i=URL.createObjectURL(n),u=document.createElement("a");u.href=i,u.download=e,u.click(),URL.revokeObjectURL(i)}function Se(e){e=e.replace(/^\uFEFF/,"").replace(/\r\n/g,`
`).replace(/\r/g,`
`);const t=e.split(`
`).filter(p=>p.trim());if(t.length<2)return[];const n=t[0],i=n.split(";").length>n.split(",").length?";":",",u=Ce(n,i).map(p=>p.trim().replace(/^\uFEFF/,"").toLowerCase()),d=[];for(let p=1;p<t.length;p++){if(!t[p].trim())continue;const l=Ce(t[p],i),y={};u.forEach((o,c)=>{y[o]=(l[c]||"").trim()}),!Object.values(y).every(o=>!o)&&d.push(y)}return d}function Ce(e,t=","){const n=[];let i="",u=!1;for(let d=0;d<e.length;d++){const p=e[d];u?p==='"'?e[d+1]==='"'?(i+='"',d++):u=!1:i+=p:p==='"'?u=!0:p===t?(n.push(i),i=""):i+=p}return n.push(i),n}const Yt={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};async function Wt(e,{toast:t}){const n=Object.keys(ue);let i=n[0];const{data:u}=await E.from("formation_links_overrides").select("formation, links"),d={};(u||[]).forEach(c=>{d[c.formation]=c.links});let p=new Set;function l(c,r){return[c,r].sort().join("-")}function y(c){const r=d[c]||ue[c]||[];p=new Set(r.map(([a,m])=>l(a,m)))}y(i),e.innerHTML=`
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;gap:12px;flex-wrap:wrap">
  ...
  `;function o(){const c=ft[i]||{},r=we(i),a=320,m=400,s=22;function h(x){const g=c[x];return g?{x:g.x*a,y:g.y*m}:null}let _=`<svg width="${a}" height="${m}" viewBox="0 0 ${a} ${m}" xmlns="http://www.w3.org/2000/svg" style="display:block">`;r.forEach(([x,g],f)=>{const v=h(x),w=h(g);if(!v||!w)return;const A=l(x,g),$=p.has(A),L=$?"#3b82f6":"#999",B=$?.95:.35,S=$?4:3;_+=`<line x1="${v.x}" y1="${v.y}" x2="${w.x}" y2="${w.y}"
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
          <div>
            ...
            <select id="f-price-type" style="width:100%;padding:9px;border:1px solid #ddd;border-radius:6px;font-size:13px">
              ${Xt.map(s=>`<option value="${s.value}" ${r.price_type===s.value?"selected":""}>${s.label}</option>`).join("")}
            </select>
          </div>
        ...
        ${i.length===0?'<div style="text-align:center;padding:16px;color:#aaa;font-size:13px">Aucun taux. Cliquez + Ligne.</div>':i.map((s,h)=>`
          <div style="display:grid;grid-template-columns:2fr 1.5fr 50px 50px 60px 32px;gap:4px;align-items:center;margin-bottom:6px;padding:6px;background:#f9f9f9;border-radius:6px" data-rate-idx="${h}">
            <select class="rate-type" data-idx="${h}" style="padding:6px 4px;border:1px solid #ddd;border-radius:4px;font-size:12px;width:100%">
              ${Zt.map(_=>`<option value="${_.value}" ${s.card_type===_.value?"selected":""}>${_.label}</option>`).join("")}
            </select>
            <select class="rate-rarity" data-idx="${h}" style="padding:6px 4px;border:1px solid #ddd;border-radius:4px;font-size:12px;width:100%">
              ${Qt.map(_=>`<option value="${_.value}" ${(s.rarity||"")===_.value?"selected":""}>${_.label}</option>`).join("")}
            </select>
    ...
    </div>`,document.getElementById("add-config").addEventListener("click",()=>ze(null)),document.getElementById("form-cancel").addEventListener("click",()=>{document.getElementById("config-form").style.display="none"}),document.getElementById("form-save").addEventListener("click",()=>on(e)),e.querySelectorAll("[data-edit]").forEach(i=>{const u=(t||[]).find(d=>d.id===i.dataset.edit);u&&i.addEventListener("click",()=>ze(u))}),e.querySelectorAll("[data-delete]").forEach(i=>{i.addEventListener("click",async()=>{confirm("Supprimer cette règle ?")&&(await E.from("sell_price_configs").delete().eq("id",i.dataset.delete),await fe(e))})})}function nn(e){const t=Qe.find(n=>n.value===e.rarity);return`
    ...
    </tr>`}function ze(e){document.getElementById("config-form").style.display="block",document.getElementById("form-title").textContent=e?"Modifier la règle":"Nouvelle règle",document.getElementById("form-id").value=(e==null?void 0:e.id)||"",document.getElementById("form-rarity").value=(e==null?void 0:e.rarity)||"normal",document.getElementById("form-price").value=(e==null?void 0:e.price)||1e3,document.getElementById("form-note-min").value=(e==null?void 0:e.note_min)??1,document.getElementById("form-note-max").value=(e==null?void 0:e.note_max)??10,document.getElementById("config-form").scrollIntoView({behavior:"smooth"})}async function on(e){const t=document.getElementById("form-id").value,n=document.getElementById("form-rarity").value,i=parseInt(document.getElementById("form-price").value)||0,u=parseInt(document.getElementById("form-note-min").value)||1,d=parseInt(document.getElementById("form-note-max").value)||10;if(u>d){alert("Note min doit être ≤ note max");return}const p={rarity:n,price:i,note_min:u,note_max:d,updated_at:new Date().toISOString()};let l;if(t?{error:l}=await E.from("sell_price_configs").update(p).eq("id",t):{error:l}=await E.from("sell_price_configs").insert(p),l){alert("Erreur : "+l.message);return}document.getElementById("config-form").style.display="none",await fe(e)}async function an(e){await be(e)}async function be(e){const{data:t}=await E.from("patch_notes").select("*").order("published_at",{ascending:!1});e.innerHTML=`
    ...
    </div>`,document.getElementById("add-article").addEventListener("click",()=>Te(null)),document.getElementById("form-cancel").addEventListener("click",()=>{document.getElementById("article-form").style.display="none"}),document.getElementById("form-save").addEventListener("click",()=>ln(e)),e.querySelectorAll("[data-edit]").forEach(n=>{const i=(t||[]).find(u=>u.id===n.dataset.edit);i&&n.addEventListener("click",()=>Te(i))}),e.querySelectorAll("[data-delete]").forEach(n=>{n.addEventListener("click",async()=>{confirm("Supprimer cet article ?")&&(await E.from("patch_notes").delete().eq("id",n.dataset.delete),await be(e))})})}function rn(e){const t=new Date(e.published_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"short",year:"numeric"});return`
  ...
  <div style="background:#fff;border-radius:10px;padding:12px 16px;box-shadow:0 1px 4px rgba(0,0,0,0.08);display:flex;align-items:center;gap:12px">
    ...
    <div style="padding:20px;max-width:820px;margin:0 auto">
      ...
      <div style="display:flex;flex-direction:column;gap:10px" id="st-list">
        ${(t||[]).map(l=>fn(l)).join("")||'<div style="color:#aaa;padding:24px;text-align:center">Aucun stade créé.</div>'}
      </div>
          ...
          <div style="flex:1;min-width:220px;display:flex;flex-direction:column;gap:12px">
            ...
            <div>
              ...
              <select id="st-country" style="width:100%;box-sizing:border-box;padding:9px;border:1.5px solid #ddd;border-radius:8px;font-size:13px;background:#fff">
                ...
                ${mn.map(([l,y])=>`<option value="${l}">${y} (${l})</option>`).join("")}
              </select>
            </div>
            <div>
    ...
    </div>`,(i=document.getElementById("st-add-btn"))==null||i.addEventListener("click",()=>Re(null)),(u=document.getElementById("st-cancel"))==null||u.addEventListener("click",()=>{document.getElementById("st-form").style.display="none"}),(d=document.getElementById("st-save"))==null||d.addEventListener("click",()=>bn(e)),(p=document.getElementById("st-preview-btn"))==null||p.addEventListener("click",()=>le()),["st-name","st-club","st-country","st-image"].forEach(l=>{var y,o;(y=document.getElementById(l))==null||y.addEventListener("input",le),(o=document.getElementById(l))==null||o.addEventListener("change",le)}),e.querySelectorAll("[data-edit]").forEach(l=>{const y=(t||[]).find(o=>o.id===l.dataset.edit);y&&l.addEventListener("click",()=>Re(y))}),e.querySelectorAll("[data-delete]").forEach(l=>{l.addEventListener("click",async()=>{confirm("Supprimer ce stade ?")&&(await E.from("stadium_definitions").delete().eq("id",l.dataset.delete),ye(e))})})}function fn(e){var n,i;let t;return(n=e.club)!=null&&n.logo_url?t=`<img src="${e.club.logo_url}" style="width:32px;height:32px;object-fit:contain">`:e.country_code?t=`<img src="https://flagsapi.com/${e.country_code.slice(0,2).toUpperCase()}/flat/32.png" style="width:32px;height:24px;object-fit:contain;border-radius:3px" onerror="this.style.display='none'">`:t='<span style="font-size:20px">🌍</span>',`
    <div style="background:#fff;border-radius:10px;padding:12px 16px;box-shadow:0 1px 4px rgba(0,0,0,0.08);display:flex;align-items:center;gap:12px">
      ...
      <button data-edit="${e.id}" class="btn btn-ghost btn-sm">✏️</button>
      <button data-delete="${e.id}" class="btn btn-ghost btn-sm" style="color:#cc2222">🗑️</button>
    </div>`}function Re(e,t){var i,u;document.getElementById("st-form").style.display="block",document.getElementById("st-form-title").textContent=e?"Modifier le stade":"Nouveau stade",document.getElementById("st-id").value=(e==null?void 0:e.id)||"",document.getElementById("st-name").value=(e==null?void 0:e.name)||"",document.getElementById("st-club").value=(e==null?void 0:e.club_id)||"";const n=document.getElementById("st-country");n&&(n.value=(e==null?void 0:e.country_code)||""),document.getElementById("st-image").value=(e==null?void 0:e.image_url)||"",document.getElementById("st-preview-card").innerHTML="",le((i=e==null?void 0:e.club)==null?void 0:i.logo_url,(u=e==null?void 0:e.club)==null?void 0:u.encoded_name),document.getElementById("st-form").scrollIntoView({behavior:"smooth"})}function le(){var o,c,r,a,m,s,h;const e=((o=document.getElementById("st-name"))==null?void 0:o.value)||"NOM DU STADE",t=(r=(c=document.getElementById("st-image"))==null?void 0:c.value)==null?void 0:r.trim(),n=(m=(a=document.getElementById("st-country"))==null?void 0:a.value)==null?void 0:m.trim(),i=document.getElementById("st-club"),u=(i==null?void 0:i.selectedIndex)||0,d=i&&u>0?i.options[u].text:"",p=((h=(s=i==null?void 0:i.options[u])==null?void 0:s.getAttribute)==null?void 0:h.call(s,"data-logo"))||"";let l=null;t?l=`${un}icons/${t}`:p?l=p:n&&(l=`https://flagsapi.com/${n.toLowerCase().slice(0,2).toUpperCase()}/flat/64.png`);const y=d||n||"—";document.getElementById("st-preview-card").innerHTML=bt(e,l,`${y}<br>+10 ⭐`,{width:140})}async function bn(e){const t=document.getElementById("st-id").value,n=document.getElementById("st-name").value.trim(),i=document.getElementById("st-club").value||null,u=document.getElementById("st-country").value.trim().toUpperCase()||null,d=document.getElementById("st-image").value.trim()||null;if(!n){alert("Le nom est obligatoire");return}const p={name:n,club_id:i,country_code:u,image_url:d},{error:l}=t?await E.from("stadium_definitions").update(p).eq("id",t):await E.from("stadium_definitions").insert(p);if(l){alert("Erreur : "+l.message);return}document.getElementById("st-form").style.display="none",ye(e)}const H=[{value:"game_changer",label:"⚡ Game Changer",border:"#7a28b8",bg:"linear-gradient(135deg,#4a0080,#7a28b8)"},{value:"ultra_game_changer",label:"💎 Ultra Game Changer",border:"#00bcd4",bg:"linear-gradient(135deg,#006080,#00bcd4)"}],te=[{value:"purple",label:"Violet",hex:"#7a28b8"},{value:"light_blue",label:"Bleu clair",hex:"#00bcd4"}];async function yn(e){e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">Chargement...</div>';const{data:t}=await E.from("gc_definitions").select("*").order("sort_order").order("created_at");let n=null;const i=[{value:"BOOST_STAT",label:"⚡ Boost stats (+N)",hasValue:!0,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"DEBUFF_STAT",label:"💀 Debuff stats (-N)",hasValue:!0,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"GRAY_PLAYER",label:"❄️ Griser un joueur",hasValue:!1,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"REVIVE_PLAYER",label:"💫 Ressusciter un joueur",hasValue:!1,hasTarget:!1,hasCount:!0,hasRoles:!1},{value:"REMOVE_GOAL",label:"🚫 Retirer un but adverse",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1},{value:"ADD_GOAL_DRAW",label:"⚽ +1 but si duel nul",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1},{value:"ADD_SUB",label:"🔄 Ajouter un remplacement",hasValue:!1,hasTarget:!1,hasCount:!0,hasRoles:!1},{value:"CUSTOM",label:"🛠️ Effet custom (hardcodé)",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1}],u=["GK","DEF","MIL","ATT"],d=()=>window.innerWidth<700,p=a=>e.querySelector(a);function l(a,m=130){if(!a)return"";const s=H.find(_=>_.value===a.gc_type)||H[0],h=a.image_url?`/icons/${a.image_url}`:null;return yt(a.name||"?",h,s.label.split(" ")[0],a.effect||"",{width:m})}function y(){return`
    <div id="gc-list" style="background:#fff;${d()?"":"border-right:1px solid #e0e0e0;width:280px;flex-shrink:0;"}overflow-y:auto;height:100%">
      ...
      ${(t||[]).map(a=>{const m=H.find(h=>h.value===a.gc_type)||H[0],s=te.find(h=>h.value===a.color)||te[0];return`
    ...
    </div>`}function o(){const a=n?(t||[]).find(m=>m.id===n):null;return a?(H.find(m=>m.value===a.gc_type)||H[0],te.find(m=>m.value===a.color)||te[0],`
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
          ...
          <div>
            ...
            <select id="gc-color" style="width:100%;padding:9px;border:1px solid #ddd;border-radius:6px;font-size:13px">
              ${te.map(m=>`<option value="${m.value}" ${a.color===m.value?"selected":""}>${m.label}</option>`).join("")}
            </select>
          </div>
        </div>
