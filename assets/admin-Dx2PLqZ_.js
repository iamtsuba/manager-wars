import{s as _,i as Et,r as _e,a as we,A as _t,l as wt,g as It,K as ze,D as P,b as $t,S as kt,c as F,H as Lt,R as Bt,e as ue,d as Xe,f as At,h as St,E as Ct,j as Tt,N as Mt,M as zt,B as Rt,k as Dt,F as ge,m as Re,n as jt,o as Qe,p as be,q as Pt}from"./special-cards-DNsLLa5x.js";async function et(e){var o;const[{count:t},{count:n},{count:i},{count:d},{count:s},{data:g},{data:x}]=await Promise.all([_.from("players").select("*",{count:"exact",head:!0}),_.from("clubs").select("*",{count:"exact",head:!0}),_.from("users").select("*",{count:"exact",head:!0}),_.from("stadium_definitions").select("*",{count:"exact",head:!0}),_.from("users").select("*",{count:"exact",head:!0}).gt("last_seen_at",new Date(Date.now()-18e4).toISOString()),_.rpc("get_signup_password"),_.from("app_feature_flags").select("*").order("key")]),E=g||"";e.innerHTML=`
  ...
  `,window.adminNav=l=>{var a;(a=document.querySelector(`[data-page="${l}"]`))==null||a.click()},e.querySelectorAll(".feature-flag-toggle").forEach(l=>{l.addEventListener("click",async()=>{const a=l.dataset.key,r=l.dataset.enabled!=="true";l.disabled=!0;const{error:p}=await _.from("app_feature_flags").update({enabled:r,updated_at:new Date().toISOString()}).eq("key",a);if(l.disabled=!1,p){alert(p.message);return}Et(),et(e)})}),(o=document.getElementById("signup-pwd-save"))==null||o.addEventListener("click",async()=>{const l=document.getElementById("signup-pwd-input").value.trim(),a=document.getElementById("signup-pwd-status");if(!l){a.textContent="Le code ne peut pas être vide.",a.style.color="#ff6b6b";return}const r=document.getElementById("signup-pwd-save");r.disabled=!0,r.textContent="⏳...";const{error:p}=await _.rpc("set_signup_password",{new_password:l});if(r.disabled=!1,r.textContent="💾 Enregistrer",p){a.textContent=p.message,a.style.color="#ff6b6b";return}a.textContent="✅ Code mis à jour.",a.style.color="#2ecc71"})}const tt={normal:"Normal",pepite:"Pépite",papyte:"Papyte",legende:"Légende"},Nt={GK:"#aaaaaa",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};async function Ft(){const e={};for(const t of _t)e[t]=await wt(t);return e}async function Ot(e,t){const{toast:n}=t;if(!confirm(`Réattribuer une nouvelle photo à tous les joueurs ayant encore l'ancien format de chemin (public/faces/...) ?

Cette action est irréversible.`))return;const i=document.getElementById("fix-old-faces-btn");i&&(i.disabled=!0,i.textContent="⏳ En cours...");try{const{data:d,error:s}=await _.from("players").select("id, country_code, face").like("face","public/faces/%");if(s){n(s.message,"error");return}if(!(d!=null&&d.length)){n("Aucun joueur avec l'ancien format — rien à faire ✅","success");return}const{data:g}=await _.from("players").select("face").not("face","is",null),x=new Set((g||[]).map(l=>l.face).filter(l=>l&&!l.startsWith("public/faces/")));let E=0,o=0;for(const l of d){const a=await we(l.country_code,x);if(!a){o++;continue}const{error:r}=await _.from("players").update({face:a}).eq("id",l.id);if(r){o++;continue}x.add(a),E++}n(`${E} photo(s) réattribuée(s)${o?`, ${o} échec(s)`:""} ✅`,o?"error":"success")}finally{i&&(i.disabled=!1,i.textContent="🖼️ Réattribuer anciennes photos")}}async function Ut(e,t){await se(e,t)}async function se(e,t,n=null){var o,l,a,r,p;const{toast:i}=t;n||(n={search:((o=document.getElementById("search-players"))==null?void 0:o.value)||"",job:((l=document.getElementById("filter-job"))==null?void 0:l.value)||"",rarity:((a=document.getElementById("filter-rarity"))==null?void 0:a.value)||"",club:((r=document.getElementById("filter-club"))==null?void 0:r.value)||"",country:((p=document.getElementById("filter-country"))==null?void 0:p.value)||""});const[{data:d,error:s},{data:g}]=await Promise.all([_.from("players").select("*, clubs(id,encoded_name,logo_url)"),_.from("clubs").select("id,encoded_name").order("encoded_name")]);if(s){e.innerHTML=`<p style="color:red">${s.message}</p>`;return}const x={GK:0,DEF:1,MIL:2,ATT:3},E=(d||[]).sort((c,f)=>{const m=(x[c.job]??4)-(x[f.job]??4);return m!==0?m:(c.surname_real||"").localeCompare(f.surname_real||"")});qt(e,E,g||[],t,n)}function qt(e,t,n,i,d=null){var l,a;const{toast:s}=i;if(e.innerHTML=`
    <div style="display:flex;flex-direction:column;gap:10px">
    ...
    </div>`,d){const r=d,p=c=>document.getElementById(c);r.search&&p("search-players")&&(p("search-players").value=r.search),r.job&&p("filter-job")&&(p("filter-job").value=r.job),r.rarity&&p("filter-rarity")&&(p("filter-rarity").value=r.rarity),r.club&&p("filter-club")&&(p("filter-club").value=r.club),r.country&&p("filter-country")&&(p("filter-country").value=r.country)}if(d){const r=d,p=c=>document.getElementById(c);r.search&&p("search-players")&&(p("search-players").value=r.search),r.job&&p("filter-job")&&(p("filter-job").value=r.job),r.rarity&&p("filter-rarity")&&(p("filter-rarity").value=r.rarity),r.club&&p("filter-club")&&(p("filter-club").value=r.club),r.country&&p("filter-country")&&(p("filter-country").value=r.country)}function g(){const r=document.getElementById("search-players").value.toLowerCase(),p=document.getElementById("filter-job").value,c=document.getElementById("filter-rarity").value,f=document.getElementById("filter-club").value,m=document.getElementById("filter-country").value;return t.filter(y=>(!r||`${y.firstname} ${y.surname_real}`.toLowerCase().includes(r))&&(!p||y.job===p)&&(!c||y.rarity===c)&&(!f||y.club_id===f)&&(!m||y.country_code===m))}const x=new Set;function E(){const r=document.getElementById("bulk-bar"),p=document.getElementById("bulk-count");r&&(r.style.display=x.size>0?"flex":"none",p&&(p.textContent=`${x.size} joueur(s) sélectionné(s)`))}function o(){const r=g();document.getElementById("count-label").textContent=`${r.length} joueur(s)`;const p=document.getElementById("players-list");if(!r.length){p.innerHTML='<div style="color:var(--gray-600);padding:20px;text-align:center">Aucun joueur.</div>';return}p.innerHTML=r.map(c=>{const f={...c,clubs:c.clubs,face:c.face||null},m=_e(f,{width:120});return`<div style="position:relative;cursor:pointer" data-edit="${c.id}">
        ${m}
      ...
      </div>`}).join(""),p.querySelectorAll("[data-edit]").forEach(c=>{c.addEventListener("click",()=>{const f=t.find(m=>m.id===c.dataset.edit);f&&De(f,n,e,i)})}),p.querySelectorAll(".btn-del-player").forEach(c=>{c.addEventListener("click",async f=>{if(f.stopPropagation(),!confirm("Supprimer ce joueur ?"))return;const{error:m}=await _.from("players").delete().eq("id",c.dataset.del);m?s(m.message,"error"):(s("Joueur supprimé ✅","success"),se(e,i))})})}o(),document.getElementById("search-players").addEventListener("input",o),document.getElementById("filter-job").addEventListener("change",o),document.getElementById("filter-rarity").addEventListener("change",o),document.getElementById("filter-club").addEventListener("change",o),document.getElementById("filter-country").addEventListener("change",o),(l=document.getElementById("bulk-cancel-btn"))==null||l.addEventListener("click",()=>{x.clear(),E(),o()}),(a=document.getElementById("bulk-delete-btn"))==null||a.addEventListener("click",async()=>{var f,m,y,k,A;if(!x.size||!confirm(`Supprimer ${x.size} joueur(s) ?`))return;const r=[...x],{error:p}=await _.from("players").delete().in("id",r);if(p){s(p.message,"error");return}s(`${r.length} joueur(s) supprimé(s) ✅`,"success"),x.clear();const c={search:((f=document.getElementById("search-players"))==null?void 0:f.value)||"",job:((m=document.getElementById("filter-job"))==null?void 0:m.value)||"",rarity:((y=document.getElementById("filter-rarity"))==null?void 0:y.value)||"",club:((k=document.getElementById("filter-club"))==null?void 0:k.value)||"",country:((A=document.getElementById("filter-country"))==null?void 0:A.value)||""};se(e,i,c)}),document.getElementById("add-player-btn").addEventListener("click",()=>De(null,n,e,i)),document.getElementById("fix-old-faces-btn").addEventListener("click",()=>Ot(e,i))}async function De(e,t,n,i){const{toast:d,openModal:s,closeModal:g}=i,x=!!e,E=await Ft(),{data:o}=await _.from("players").select("face").not("face","is",null),l=new Set((o||[]).map(c=>c.face).filter(Boolean));e!=null&&e.face&&l.delete(e.face);const a='<option value="">— Club —</option>'+t.map(c=>`<option value="${c.id}" ${(e==null?void 0:e.club_id)===c.id?"selected":""}>${c.encoded_name}</option>`).join(""),r=e!=null&&e.face?e.face.split("/")[0]:"",p=Object.keys(E).map(c=>`<option value="${c}" ${r===c?"selected":""}>${c}</option>`).join("");s(x?`✏️ ${e.firstname} ${e.surname_real}`:"➕ Nouveau joueur",`<div style="display:flex;gap:20px;align-items:flex-start">
      ...
      <div style="flex:1;min-width:300px;display:flex;flex-direction:column;gap:12px">
        ...
        <div style="border-top:1px solid var(--gray-200);padding-top:10px">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:8px">
            ${[["GK","pm-g","note_g"],["DEF","pm-d","note_d"],["MIL","pm-m","note_m"],["ATT","pm-a","note_a"]].map(([c,f,m])=>`
              <div class="form-group" style="text-align:center">
                <label style="color:${Nt[c]};font-weight:700">${c}</label>
    ...
    </div>`,g(re),document.getElementById("search-clubs").addEventListener("input",x=>{const E=x.target.value.toLowerCase();g(re.filter(o=>o.real_name.toLowerCase().includes(E)||o.encoded_name.toLowerCase().includes(E)))}),document.getElementById("add-club-btn").addEventListener("click",()=>je(null,e,t));function g(x){const E=document.getElementById("clubs-list");if(!x.length){E.innerHTML='<p style="color:var(--tile-fg-dim);padding:20px;text-align:center">Aucun club.</p>';return}const o={};x.forEach(r=>{const p=r.country_code||"—";o[p]||(o[p]=[]),o[p].push(r)});const l=Object.keys(o).sort(),a=r=>{const p=ct(r),c=r.logo_url?`<img src="${r.logo_url}" style="width:36px;height:36px;object-fit:contain;border-radius:8px;flex-shrink:0">`:`<div style="width:36px;height:36px;background:linear-gradient(135deg,${p.color1},${p.color2});border-radius:8px;display:flex;align-items:center;justify-content:center;color:#fff;font-size:9px;font-weight:900;flex-shrink:0">${r.encoded_name.slice(0,3)}</div>`,f=n[r.id]||0;return`
    ...
    `).join(""),E.querySelectorAll(".club-row").forEach(r=>{r.addEventListener("click",()=>{const p=re.find(c=>c.id===r.dataset.clubId);p&&je(p,e,t),E.querySelectorAll(".club-row").forEach(c=>c.style.background="var(--tile-bg)"),r.style.background="rgba(26,107,60,0.18)"})}),E.querySelectorAll(".btn-del-club").forEach(r=>{r.addEventListener("click",async()=>{if(!confirm("Supprimer ce club ?"))return;const{error:p}=await _.from("clubs").delete().eq("id",r.dataset.del);p?i(p.message,"error"):(i("Club supprimé","success"),me(e,t))})})}}function nn(e){var w,L,S,T;const{openModal:t,closeModal:n}=e;let i=6;const d=36;function s(B){const C=[];for(let M=0;M<B;M++)for(let z=0;z<B;z++)C.push((M+z)%2===0?"#00AEEF":"#ffffff");return C}let g=s(i),x="OP",E="#D4A017",o=50,l=0,a=0,r="",p="#ffffff",c=25,f=0,m=35,y=null,k=null;function A(){const B=d*i;let C="";for(let z=0;z<i;z++)for(let R=0;R<i;R++){const N=z*i+R;C+=`<rect x="${R*d}" y="${z*d}" width="${d}" height="${d}" fill="${g[N]}"/>`}const M=(z,R,N,O,W)=>{if(!z)return"";const D=B/2+O/100*B,q=B/2+W/100*B,U=N/100*B;return`<text x="${D}" y="${q}" text-anchor="middle" dominant-baseline="central"
        font-family="Arial Black, Arial, sans-serif" font-weight="900" font-size="${U}"
        fill="${R}">${z.toUpperCase().slice(0,3)}</text>`};return`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${B} ${B}">
      ${C}
      ${M(x,E,o,l,a)}
      ${M(r,p,c,f,m)}
    </svg>`}function v(B){return"data:image/svg+xml;base64,"+btoa(unescape(encodeURIComponent(B)))}const I=`
      ...
      <div style="flex:0 0 260px;display:flex;flex-direction:column;gap:14px;align-items:center">
        ...
        <div style="width:100%">
          ...
          <div style="display:flex;gap:6px">
            ${[6,9,12].map(B=>`<button type="button" class="btn ${B===i?"btn-primary":"btn-ghost"} btn-sm flag-grid-size" data-n="${B}" style="flex:1">${B}×${B}</button>`).join("")}
          </div>
        </div>

        <div style="width:100%">
          <label>TEXTE 1 (3 caractères max)</label>
          <input id="flag-text" maxlength="3" value="${x}" style="text-align:center;font-weight:900;text-transform:uppercase">
        </div>
        ...
        <div style="width:100%">
          <label>Taille (<span id="flag-size-val">${o}</span>%)</label>
          <input type="range" id="flag-text-size" min="10" max="120" value="${o}" style="width:100%">
        </div>
        <div style="width:100%">
          <label>Position H (<span id="flag-x-val">${l}</span>)</label>
          <input type="range" id="flag-text-x" min="-50" max="50" value="${l}" style="width:100%">
        </div>
        <div style="width:100%">
          <label>Position V (<span id="flag-y-val">${a}</span>)</label>
        ...
        <div style="width:100%;border-top:1px solid #eee;margin-top:4px;padding-top:12px">
          ...
          <input id="flag-text2" maxlength="3" value="${r}" placeholder="Vide = désactivé" style="text-align:center;font-weight:900;text-transform:uppercase">
        </div>
        <div style="display:flex;align-items:center;gap:8px;width:100%">
          <label style="margin:0;white-space:nowrap">Couleur</label>
          <input type="color" id="flag-text2-color" value="${p}" style="width:44px;height:32px;padding:2px;cursor:pointer">
        </div>
        <div style="width:100%">
          <label>Taille (<span id="flag-size2-val">${c}</span>%)</label>
          <input type="range" id="flag-text2-size" min="10" max="120" value="${c}" style="width:100%">
        </div>
        <div style="width:100%">
          <label>Position H (<span id="flag-x2-val">${f}</span>)</label>
          <input type="range" id="flag-text2-x" min="-50" max="50" value="${f}" style="width:100%">
        </div>
        <div style="width:100%">
          <label>Position V (<span id="flag-y2-val">${m}</span>)</label>
          <input type="range" id="flag-text2-y" min="-50" max="50" value="${m}" style="width:100%">
        </div>
      </div>

      <div style="flex:1;min-width:280px">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
          <label style="margin:0">Couleurs des ${i*i} carrés — clique un carré pour le sélectionner</label>
        </div>
  ...
  `;t(`🎨 Générer un logo (${i}×${i})`,I,`
    <button id="flag-cancel" class="btn btn-ghost">Annuler</button>
    <button id="flag-use" class="btn btn-primary">✅ Utiliser ce logo</button>
  `);function b(){const B=document.getElementById("flag-squares-grid");if(!B)return;const C=i>=12?26:i>=9?32:40;B.style.gridTemplateColumns=`repeat(${i}, ${C}px)`,B.innerHTML=g.map((M,z)=>`
      <div class="flag-square" data-i="${z}" style="width:${C}px;height:${C}px;border-radius:4px;background:${M};cursor:pointer;
        box-sizing:border-box;border:${y===z?"3px solid #1A6B3C":"1px solid rgba(0,0,0,0.15)"}"></div>
    `).join(""),B.querySelectorAll(".flag-square").forEach(M=>{M.addEventListener("click",()=>{y=parseInt(M.dataset.i),b(),$()}),M.addEventListener("dblclick",()=>{const z=parseInt(M.dataset.i),R=document.createElement("input");R.type="color",R.value=g[z],R.style.position="fixed",R.style.opacity="0",document.body.appendChild(R),R.addEventListener("input",()=>{g[z]=R.value,b(),u()}),R.addEventListener("change",()=>R.remove()),R.click()})})}function $(){const B=document.getElementById("flag-copy-btn"),C=document.getElementById("flag-paste-btn");B.disabled=y===null,C.disabled=y===null||k===null}(w=document.getElementById("flag-copy-btn"))==null||w.addEventListener("click",()=>{if(y===null)return;k=g[y];const B=document.getElementById("flag-clipboard-preview"),C=document.getElementById("flag-clipboard-swatch");B&&(B.style.display="inline-flex"),C&&(C.style.background=k),$()}),(L=document.getElementById("flag-paste-btn"))==null||L.addEventListener("click",()=>{y===null||k===null||(g[y]=k,b(),u())}),document.querySelectorAll(".flag-grid-size").forEach(B=>{B.addEventListener("click",()=>{const C=parseInt(B.dataset.n);C!==i&&(i=C,g=s(i),y=null,k=null,document.getElementById("flag-clipboard-preview").style.display="none",b(),$(),u(),document.querySelectorAll(".flag-grid-size").forEach(M=>{M.classList.toggle("btn-primary",parseInt(M.dataset.n)===i),M.classList.toggle("btn-ghost",parseInt(M.dataset.n)!==i)}))})});function u(){const B=document.getElementById("flag-preview");B&&(B.innerHTML=A())}b(),$(),u(),document.getElementById("flag-text").addEventListener("input",B=>{x=B.target.value.toUpperCase().slice(0,3),B.target.value=x,u()}),document.getElementById("flag-text-color").addEventListener("input",B=>{E=B.target.value,u()}),document.getElementById("flag-text-size").addEventListener("input",B=>{o=parseInt(B.target.value),document.getElementById("flag-size-val").textContent=o,u()}),document.getElementById("flag-text-x").addEventListener("input",B=>{l=parseInt(B.target.value),document.getElementById("flag-x-val").textContent=l,u()}),document.getElementById("flag-text-y").addEventListener("input",B=>{a=parseInt(B.target.value),document.getElementById("flag-y-val").textContent=a,u()}),document.getElementById("flag-text2").addEventListener("input",B=>{r=B.target.value.toUpperCase().slice(0,3),B.target.value=r,u()}),document.getElementById("flag-text2-color").addEventListener("input",B=>{p=B.target.value,u()}),document.getElementById("flag-text2-size").addEventListener("input",B=>{c=parseInt(B.target.value),document.getElementById("flag-size2-val").textContent=c,u()}),document.getElementById("flag-text2-x").addEventListener("input",B=>{f=parseInt(B.target.value),document.getElementById("flag-x2-val").textContent=f,u()}),document.getElementById("flag-text2-y").addEventListener("input",B=>{m=parseInt(B.target.value),document.getElementById("flag-y2-val").textContent=m,u()}),(S=document.getElementById("flag-cancel"))==null||S.addEventListener("click",()=>n()),(T=document.getElementById("flag-use"))==null||T.addEventListener("click",()=>{const B=v(A()),C=document.getElementById("m-logo-url-current");C&&(C.value=B);const M=document.getElementById("logo-preview");M&&(M.innerHTML=`<img src="${B}" style="width:100%;height:100%;object-fit:contain">`),n()})}async function je(e,t,n){var m,y,k,A,v,I,h;const{toast:i}=n,d=!!e,s=e?ct(e):{...P},g=document.getElementById("club-panel");if(!g)return;const x=Object.entries(ze).map(([b,$])=>`<option value="${b}" ${s.style===b?"selected":""}>${$.label}</option>`).join(""),E=Ie.map(b=>`<option value="${b}" ${((e==null?void 0:e.country_code)||"FR")===b?"selected":""}>${b}</option>`).join("");g.style.display="block",g.style.alignItems="",g.style.justifyContent="",g.innerHTML=`
      ...
      <div style="border-top:1px solid var(--tile-border);padding-top:12px">
        ...
        <div style="display:flex;gap:12px;align-items:flex-start;flex-wrap:wrap">
          <div style="flex:1;min-width:200px;display:flex;flex-direction:column;gap:8px">
            ...
            ${[["Couleur 1","m-kit-color1",s.color1,!1],["Couleur 2","m-kit-color2",s.color2,!1],["Couleur 3","m-kit-color3",s.color3,!0],["Short","m-kit-shorts",s.shorts,!1],["Chaussettes","m-kit-socks",s.socks,!1]].map(([b,$,u,w])=>`
              <div class="form-group" id="wrap-${$}" ${w?'style="display:none"':""}>
                <label>${b}</label>
                <div style="display:flex;gap:6px;align-items:center">
                  <input type="color" id="${$}" value="${u||"#000000"}" style="width:38px;height:32px;padding:2px;border:1px solid var(--tile-border);border-radius:6px;cursor:pointer;flex-shrink:0">
                  <input id="${$}-txt" value="${u||"#000000"}" maxlength="7" style="flex:1;font-family:monospace;font-size:12px">
                </div>
              </div>`).join("")}
          </div>
  ...
  `,(m=document.getElementById("m-logo-file"))==null||m.addEventListener("change",b=>{var w;const $=(w=b.target.files)==null?void 0:w[0];if(!$)return;const u=new FileReader;u.onload=()=>{document.getElementById("logo-preview").innerHTML=`<img src="${u.result}" style="width:100%;height:100%;object-fit:contain">`},u.readAsDataURL($)}),(y=document.getElementById("m-open-flag-builder"))==null||y.addEventListener("click",()=>{nn(n)}),ie();function o(){var w,L;const b=((w=document.getElementById("m-kit-style"))==null?void 0:w.value)||"uni",$=((L=ze[b])==null?void 0:L.colors)===3,u=document.getElementById("wrap-m-kit-color3");u&&(u.style.display=$?"":"none")}o(),["m-kit-color1","m-kit-color2","m-kit-color3","m-kit-shorts","m-kit-socks"].forEach(b=>{const $=document.getElementById(b),u=document.getElementById(b+"-txt");!$||!u||($.addEventListener("input",()=>{u.value=$.value,ie()}),u.addEventListener("input",()=>{const w=u.value.trim();/^#[0-9a-fA-F]{6}$/.test(w)&&($.value=w,ie())}),u.addEventListener("change",()=>{let w=u.value.trim();w.startsWith("#")||(w="#"+w),/^#[0-9a-fA-F]{6}$/.test(w)&&($.value=w,u.value=w,ie())}))}),(k=document.getElementById("m-kit-style"))==null||k.addEventListener("change",()=>{o(),ie()});const l=document.getElementById("m-real"),a=document.getElementById("m-encoded");function r(){if(!l||!a||a.value)return;const b=l.value.trim().split(/\s+/),$=b.length===1?b[0].toUpperCase().slice(0,6):b.filter(u=>u.length>2).map(u=>u[0].toUpperCase()).join("")||b[0].toUpperCase().slice(0,4);a.value=$}l==null||l.addEventListener("input",r),(A=document.getElementById("auto-encode-btn"))==null||A.addEventListener("click",()=>{a&&(a.value=""),r()}),(v=document.getElementById("m-save"))==null||v.addEventListener("click",()=>dn(e,d,t,n));const p=document.getElementById("m-gen-squad"),c=document.getElementById("m-gen-strong-label");function f(){if(c&&(c.style.display=p!=null&&p.checked?"flex":"none",!(p!=null&&p.checked))){const b=document.getElementById("m-gen-strong");b&&(b.checked=!1)}}p==null||p.addEventListener("change",f),f(),(I=document.getElementById("btn-gen-squad-panel"))==null||I.addEventListener("click",()=>{an(e,n,t)}),(h=document.getElementById("btn-gen-one-player"))==null||h.addEventListener("click",()=>{rn(e,n)}),d&&ke(e.id)}async function ke(e){const{data:t}=await _.from("players").select("*, clubs(encoded_name, logo_url, kit_style, kit_color1, kit_color2, kit_color3, kit_shorts, kit_socks)").eq("club_id",e).order("surname_real"),n=document.getElementById("club-players-grid"),i=document.getElementById("club-players-count");if(i&&(i.textContent=(t||[]).length),!!n){if(!(t!=null&&t.length)){n.innerHTML='<div style="color:var(--tile-fg-dim);padding:20px;font-size:13px">Aucun joueur dans ce club.</div>';return}n.innerHTML=t.map(d=>_e(d,{width:100})).join("")}}function an(e,t,n){const{openModal:i,closeModal:d,toast:s}=t;i(`⚽ Générer — ${e.real_name}`,`<div style="display:flex;flex-direction:column;gap:14px">
  ...
  `),setTimeout(()=>{var o,l;document.querySelectorAll('input[name="gen-one-mode"]').forEach(a=>{a.addEventListener("change",()=>{document.getElementById("gen-one-custom-fields").style.display=a.value==="custom"&&a.checked?"flex":"none"})}),(o=document.getElementById("gen-one-cancel"))==null||o.addEventListener("click",()=>i()),(l=document.getElementById("gen-one-ok"))==null||l.addEventListener("click",async()=>{var c,f,m,y,k;const a=((c=document.querySelector('input[name="gen-one-mode"]:checked'))==null?void 0:c.value)||"random",r=document.getElementById("gen-one-ok");r.disabled=!0,r.textContent="⏳ Génération…";const p={};a==="custom"&&(p.job=((f=document.getElementById("gen-one-job1"))==null?void 0:f.value)||null,p.job2=((m=document.getElementById("gen-one-job2"))==null?void 0:m.value)||null,p.rarity=((y=document.getElementById("gen-one-rarity"))==null?void 0:y.value)||null,p.country=((k=document.getElementById("gen-one-country"))==null?void 0:k.value)||null);try{await ln(e,p),d("Joueur généré ✅","success"),i(),ke(e.id)}catch(A){d("Erreur : "+A.message,"error"),r.disabled=!1,r.textContent="➕ Générer"}})},50)}async function ln(e,t={}){const n=t.country||(Math.random()<.5?e.country_code:at(e.country_code)),i=t.job||J(pt),d=t.job2||null,s=t.rarity||(()=>{const L=Math.random()*100;return L<1.5?"legende":L<3?"pepite":L<4.5?"papyte":"normal"})(),g=s==="pepite"?Y(10,15):s==="papyte"?Y(15,20):ot(),x=d?Math.max(0,g-2):0;let E=0,o=0,l=0,a=0;i==="GK"&&(E=g),i==="DEF"&&(o=g),i==="MIL"&&(l=g),i==="ATT"&&(a=g),d==="GK"&&(E=x),d==="DEF"&&(o=x),d==="MIL"&&(l=x),d==="ATT"&&(a=x);const r=$e(n),p=it(n),c=J(nt),{data:f}=await _.from("players").select("surname_real").not("surname_real","is",null),m=new Set((f||[]).map(L=>L.surname_real).filter(Boolean)),y=st(n),k=y.filter(L=>!m.has(L)),A=J(k.length?k:y),v={job:i,job2:d,firstname:lt(n),surname_real:A,country_code:n,club_id:e.id,note_g:E,note_d:o,note_m:l,note_a:a,skin:r,hair:p,hair_length:c,rarity:s,sell_price:0,ethnie:ve(n)},{data:I}=await _.from("players").select("face").not("face","is",null),h=new Set((I||[]).map(L=>L.face).filter(Boolean)),b=await we(n,h);b&&(v.face=b);const{data:$,error:u}=await _.from("players").insert(v).select("id").single();if(u)throw u;const{error:w}=await _.from("cards").insert({card_type:"player",player_id:$.id});w&&console.warn("[GenOnePlayer] Erreur carte:",w.message)}function ut(){var e,t,n,i,d,s;return{style:((e=document.getElementById("m-kit-style"))==null?void 0:e.value)||"uni",color1:((t=document.getElementById("m-kit-color1"))==null?void 0:t.value)||"#1A6B3C",color2:((n=document.getElementById("m-kit-color2"))==null?void 0:n.value)||"#ffffff",color3:((i=document.getElementById("m-kit-color3"))==null?void 0:i.value)||"#000000",shorts:((d=document.getElementById("m-kit-shorts"))==null?void 0:d.value)||"#111111",socks:((s=document.getElementById("m-kit-socks"))==null?void 0:s.value)||"#ffffff"}}function ie(){const e=document.getElementById("kit-preview-wrap");e&&(e.innerHTML=$t(ut(),"panel"))}async function sn(e,t){const n=(e.name.split(".").pop()||"png").toLowerCase(),i=`clubs/${t||"new"}-${Date.now()}.${n}`,{error:d}=await _.storage.from("assets").upload(i,e,{upsert:!0,cacheControl:"3600"});if(d)throw d;const{data:s}=_.storage.from("assets").getPublicUrl(i);return s.publicUrl}async function dn(e,t,n,i){var A,v,I,h,b,$,u,w,L;const{toast:d}=i,s=document.getElementById("m-error"),g=document.getElementById("m-save"),x=(A=document.getElementById("m-real"))==null?void 0:A.value.trim(),E=(v=document.getElementById("m-encoded"))==null?void 0:v.value.trim().toUpperCase(),o=(I=document.getElementById("m-country"))==null?void 0:I.value.trim().toUpperCase(),l=((b=(h=document.getElementById("m-logo-file"))==null?void 0:h.files)==null?void 0:b[0])||null,a=(($=document.getElementById("m-logo-url-current"))==null?void 0:$.value)||null,r=((u=document.getElementById("m-gen-stadium"))==null?void 0:u.checked)??!1,p=((w=document.getElementById("m-gen-squad"))==null?void 0:w.checked)??!1,c=((L=document.getElementById("m-gen-strong"))==null?void 0:L.checked)??!1,f=ut();if(!x){s.textContent="Le nom du club est requis.";return}if(!E){s.textContent="Le nom encodé est requis.";return}if(!o){s.textContent="Le pays est requis.";return}g.disabled=!0,g.textContent="Enregistrement…";let m=a;if(l)try{g.textContent="📤 Envoi du logo…",m=await sn(l,e==null?void 0:e.id)}catch(S){s.textContent="Erreur upload logo : "+S.message,g.disabled=!1,g.textContent=t?"💾 Enregistrer":"✅ Créer le club";return}const y={real_name:x,encoded_name:E,country_code:o,logo_url:m,kit_style:f.style,kit_color1:f.color1,kit_color2:f.color2,kit_color3:f.color3,kit_shorts:f.shorts,kit_socks:f.socks};let k=e==null?void 0:e.id;if(t){const{error:S}=await _.from("clubs").update(y).eq("id",k);if(S){s.textContent=S.message,g.disabled=!1,g.textContent="💾 Enregistrer";return}}else{const{data:S,error:T}=await _.from("clubs").insert(y).select().single();if(T){s.textContent=T.message,g.disabled=!1,g.textContent="✅ Créer le club";return}if(k=S.id,r){g.textContent="🏟️ Création du stade…";const{data:B,error:C}=await _.from("stadium_definitions").insert({name:`Stade de ${x}`,club_id:k,country_code:null}).select().single();C?console.warn("[Stadium] Erreur def stade:",C.message):B&&await _.from("cards").insert({card_type:"stadium",stadium_id:B.id})}p&&(g.textContent="⚽ Génération des joueurs…",await dt(k,o,d,c))}d(t?"Club modifié ✅":"Club créé ✅","success"),me(n,i)}const cn=["normal","pepite","papyte","legende"],Pe=["GK","DEF","MIL","ATT"],pn=[["MA","Maroc"],["FR","France"],["AR","Argentine"],["PT","Portugal"],["BR","Brésil"],["ES","Espagne"],["DE","Allemagne"],["GB","Angleterre"],["IT","Italie"],["CM","Cameroun"],["SN","Sénégal"],["NG","Nigéria"],["DK","Danemark"],["NL","Pays-Bas"],["BE","Belgique"],["CI","Côte d'Ivoire"],["AL","Albanie"],["HR","Croatie"],["RS","Serbie"],["TR","Turquie"],["MW","Malawi"]];let V={...F},le=[];async function un(e,{toast:t}){const{data:n}=await _.from("clubs").select("id,encoded_name,logo_url,kit_style,kit_color1,kit_color2,kit_shorts,kit_socks").order("encoded_name");le=n||[],V={...F},e.innerHTML=yn(le),gn(e,le,t),H()}function Ne(){var n;const e=((n=document.getElementById("cb-surname-real"))==null?void 0:n.value)||"",t=document.getElementById("cb-surname-enc");t&&(t.value=ue(e))}function mn(){var n;const e=(n=document.getElementById("cb-club"))==null?void 0:n.value,t=le.find(i=>String(i.id)===String(e));return t?{style:t.kit_style||P.style,color1:t.kit_color1||P.color1,color2:t.kit_color2||P.color2,shorts:t.kit_shorts||P.shorts,socks:t.kit_socks||P.socks}:{...P}}function H(){var r,p,c,f;const e=m=>{var y;return((y=document.getElementById(m))==null?void 0:y.value)||""},t={firstname:e("cb-firstname"),surname_real:e("cb-surname-enc")||ue(e("cb-surname-real")),country_code:e("cb-country"),club_encoded_name:((p=(r=document.getElementById("cb-club"))==null?void 0:r.selectedOptions[0])==null?void 0:p.text)||"",job:e("cb-job"),job2:e("cb-job2")||null,note_g:parseInt(e("cb-note-g"))||0,note_d:parseInt(e("cb-note-d"))||0,note_m:parseInt(e("cb-note-m"))||0,note_a:parseInt(e("cb-note-a"))||0,rarity:e("cb-rarity")},n=mn(),i=document.getElementById("cb-club"),d=((f=(c=i==null?void 0:i.selectedOptions[0])==null?void 0:c.dataset)==null?void 0:f.logo)||null,s=Xe(V,n,120),g=`data:image/svg+xml;charset=utf-8,${encodeURIComponent(s)}`,x=At(t,{portraitUrl:g,clubLogoUrl:d,showNotes:!0}),E=document.getElementById("card-preview");E&&(E.innerHTML=x);const o=document.getElementById("avatar-preview-wrap");o&&(o.innerHTML=s);const l=e("cb-surname-real"),a=document.getElementById("encode-summary");a&&l&&(a.textContent=`${e("cb-firstname")} ${l} → ${e("cb-firstname")} ${t.surname_real}`)}function mt(){const e=t=>{var n;return((n=document.getElementById(t))==null?void 0:n.value)||""};V={skin:e("av-skin"),hair:e("av-hair"),hairColor:e("av-hairColor"),eyes:e("av-eyes"),nose:e("av-nose"),mouth:e("av-mouth"),eyebrows:e("av-eyebrows"),beard:e("av-beard")}}function gn(e,t,n){var d,s,g,x,E;e.querySelectorAll("input,select").forEach(o=>{o.addEventListener("input",H),o.addEventListener("change",H)});const i=["av-skin","av-hair","av-hairColor","av-eyes","av-nose","av-mouth","av-eyebrows","av-beard"];i.forEach(o=>{var l;(l=document.getElementById(o))==null||l.addEventListener("change",()=>{mt(),H()})}),(d=document.getElementById("cb-surname-real"))==null||d.addEventListener("input",()=>{Ne(),H()}),(s=document.getElementById("btn-encode"))==null||s.addEventListener("click",()=>{Ne(),H()}),(g=document.getElementById("cb-club"))==null||g.addEventListener("change",H),(x=document.getElementById("btn-save-player"))==null||x.addEventListener("click",()=>fn(n)),(E=document.getElementById("btn-reset"))==null||E.addEventListener("click",()=>{e.querySelectorAll("input").forEach(o=>o.value=""),e.querySelectorAll("select").forEach(o=>o.selectedIndex=0),V={...F},i.forEach(o=>{const l=o.replace("av-",""),a=document.getElementById(o);a&&(a.value=F[l]||"")}),H()})}async function fn(e){const t=g=>{var x;return((x=document.getElementById(g))==null?void 0:x.value)||""},n=t("cb-surname-real").trim(),i=t("cb-surname-enc").trim()||ue(n);if(!t("cb-firstname")||!n||!t("cb-country")||!t("cb-job")){e("Remplissez les champs obligatoires (prénom, nom, pays, poste)","error");return}mt();const d={firstname:t("cb-firstname").trim(),surname_real:n,surname_real:i,country_code:t("cb-country"),club_id:t("cb-club")||null,job:t("cb-job"),job2:t("cb-job2")||null,note_g:parseInt(t("cb-note-g"))||0,note_d:parseInt(t("cb-note-d"))||0,note_m:parseInt(t("cb-note-m"))||0,note_a:parseInt(t("cb-note-a"))||0,rarity:t("cb-rarity")||"normal",note_min:parseInt(t("cb-note-min"))||null,note_max:parseInt(t("cb-note-max"))||null,sell_price:parseInt(t("cb-price"))||0,avatar_config:V,skin:V.skin||"blanc",hair:V.hairColor||"marron",hair_length:"court"},{error:s}=await _.from("players").insert(d);if(s){e("Erreur: "+s.message,"error");return}e(`✅ Joueur "${d.firstname} ${d.surname_real}" enregistré !`,"success")}function yn(e){const t=pn.map(([o,l])=>`<option value="${o}">${l} (${o})</option>`).join(""),n='<option value="">— Sélectionner un club —</option>'+e.map(o=>`<option value="${o.id}" data-logo="${o.logo_url||""}">${o.encoded_name}</option>`).join(""),i=Pe.map(o=>`<option value="${o}">${o}</option>`).join(""),d='<option value="">Aucun</option>'+Pe.map(o=>`<option value="${o}">${o}</option>`).join(""),s=cn.map(o=>`<option value="${o}">${o.charAt(0).toUpperCase()+o.slice(1)}</option>`).join(""),g=(o,l,a)=>Object.entries(l).map(([r,p])=>`<option value="${r}" ${r===(a||"")?"selected":""}>${p.label}</option>`).join(""),x=Object.keys(kt).map(o=>`<option value="${o}" ${o===F.skin?"selected":""}>${o.charAt(0).toUpperCase()+o.slice(1)}</option>`).join(""),E=Object.keys(Lt).map(o=>`<option value="${o}" ${o===F.hairColor?"selected":""}>${o.charAt(0).toUpperCase()+o.slice(1)}</option>`).join("");return`
        ...
        <div style="display:grid;grid-template-columns:1fr 120px;gap:16px;align-items:start">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
            ...
            <div>
              ...
              <select id="av-hair">${g("av-hair",St,F.hair)}</select>
            </div>
            <div>
            ...
            <div>
              ...
              <select id="av-eyebrows">${g("av-eyebrows",Ct,F.eyebrows)}</select>
            </div>
            <div>
              <label>Yeux</label>
              <select id="av-eyes">${g("av-eyes",Tt,F.eyes)}</select>
            </div>
            <div>
              <label>Nez</label>
              <select id="av-nose">${g("av-nose",Mt,F.nose)}</select>
            </div>
            <div>
              <label>Bouche</label>
              <select id="av-mouth">${g("av-mouth",zt,F.mouth)}</select>
            </div>
            <div>
              <label>Barbe</label>
              <select id="av-beard">${g("av-beard",Rt,F.beard)}</select>
            </div>
          </div>
  ...
  `,o(E),document.getElementById("search-users").addEventListener("input",l=>{const a=l.target.value.toLowerCase();o(E.filter(r=>{var p;return r.pseudo.toLowerCase().includes(a)||((p=r.club_name)==null?void 0:p.toLowerCase().includes(a))}))});function o(l){document.getElementById("users-tbody").innerHTML=l.map(a=>{const r=a.mmr??1e3,p=a.mmr_deviation??350,c=a.placement_matches??0,f=Dt(r),m=a.ranked_wins??0,y=a.ranked_losses??0,k=a.ranked_draws??0,A=m+y+k,v=A>0?Math.round(m/A*100):null,I=c<10,h=p<80?"#1A6B3C":p<150?"#e67e22":"#bb2020";return`
        <tr>
          <td>
          ...
          <td style="font-size:12px">
            ...
            ${I?`<div style="font-size:10px;color:#e67e22">${c}/10 placement${c<10?` (${10-c} restants)`:""}</div>`:`<div style="font-size:10px;color:var(--gray-600)">${m}V ${k}N ${y}D${v!==null?` · ${v}% WR`:""}</div>`}
          </td>
    ...
    <div class="card-panel">
      <div class="table-wrap">
        <table>
          ...
          <tbody>
            ${s.map(o=>{var p,c,f,m;const l=(p=o.card)==null?void 0:p.player,a=l?`${l.firstname} ${l.surname_real}`:((c=o.card)==null?void 0:c.card_type)||"—",r={active:"#1A6B3C",sold:"#D4A017",cancelled:"#888"};return`<tr>
  ...
  `),xn(),(p=document.getElementById("pg-close"))==null||p.addEventListener("click",()=>{fe(),n()});function l(f){const m=f.dataset.id,y=f.dataset.field,k=y==="rarity"?f.value:Number(f.value)||0;return a(m,{[y]:k},f)}async function a(f,m,y){const k=document.querySelector(`tr[data-row="${f}"]`),A=u=>k.querySelector(`[data-field="${u}"]`),v="note_min"in m?m.note_min:Number(A("note_min").value),I="note_max"in m?m.note_max:Number(A("note_max").value),h="price_min"in m?m.price_min:Number(A("price_min").value),b="price_max"in m?m.price_max:Number(A("price_max").value);if(v>I){i("Note min doit être ≤ note max","error");return}if(h>b){i("Prix min doit être ≤ prix max","error");return}const{error:$}=await _.from("sell_price_configs").update(m).eq("id",f);if($){i($.message,"error");return}y&&(y.style.transition="background .3s",y.style.background="#e8f8ee",setTimeout(()=>{y.style.background=""},500))}function r(){document.querySelectorAll("#pg-tbody [data-field]").forEach(f=>{f.addEventListener("change",()=>l(f))})}r(),document.querySelectorAll("[data-del-row]").forEach(f=>{f.addEventListener("click",async()=>{confirm("Supprimer cette ligne ?")&&(await _.from("sell_price_configs").delete().eq("id",f.dataset.delRow),i("Ligne supprimée","success"),fe(),xe(e))})}),(c=document.getElementById("pg-add-row"))==null||c.addEventListener("click",async()=>{var A;const f={rarity:"normal",note_min:0,note_max:20,price_min:2500,price_max:15e3},{data:m,error:y}=await _.from("sell_price_configs").insert(f).select().single();if(y){i(y.message,"error");return}const k=document.getElementById("pg-tbody");k.querySelector("td[colspan]")&&(k.innerHTML=""),k.insertAdjacentHTML("beforeend",x(m)),r(),(A=document.querySelector(`[data-del-row="${m.id}"]`))==null||A.addEventListener("click",async function(){confirm("Supprimer cette ligne ?")&&(await _.from("sell_price_configs").delete().eq("id",m.id),i("Ligne supprimée","success"),fe(),xe(e))}),i("Ligne ajoutée — modifie les valeurs directement","success")})}async function hn(e,{toast:t}){e.innerHTML=`
...
`}),ae("players_export.csv",d),t(`${n.length} joueurs exportés`,"success")}),document.getElementById("import-clubs").addEventListener("change",async n=>{const i=n.target.files[0];if(!i)return;const d=document.getElementById("clubs-import-result");d.innerHTML='<span style="color:var(--gray-600)">Import en cours...</span>';try{const s=await i.text(),g=Oe(s);if(g.length===0){d.innerHTML='<span style="color:#c0392b">Fichier vide</span>';return}let x=0,E=0;const o=[];for(const l of g){if(!l.real_name||!l.encoded_name||!l.country_code){E++,o.push(`Ligne ignorée (champs manquants): ${l.real_name||"?"}`);continue}const a={real_name:l.real_name,encoded_name:l.encoded_name.toUpperCase(),country_code:l.country_code.toUpperCase().slice(0,2),logo_url:l.logo_url||null},{error:r}=await _.from("clubs").upsert(a,{onConflict:"encoded_name"});r?(E++,o.push(`${l.encoded_name}: ${r.message}`)):x++}d.innerHTML=`<div style="color:var(--green)">✅ ${x} clubs importés</div>
        ${E>0?`<div style="color:#c0392b">❌ ${E} erreurs</div><div style="font-size:11px;color:var(--gray-600);max-height:100px;overflow-y:auto">${o.slice(0,10).join("<br>")}</div>`:""}`,t(`${x} clubs importés`,"success")}catch(s){d.innerHTML=`<span style="color:#c0392b">Erreur : ${s.message}</span>`}n.target.value=""}),document.getElementById("import-players").addEventListener("change",async n=>{const i=n.target.files[0];if(!i)return;const d=document.getElementById("players-import-result");d.innerHTML='<span style="color:var(--gray-600)">Import en cours...</span>';try{const s=await i.text(),g=Oe(s);if(g.length===0){d.innerHTML='<span style="color:#c0392b">Fichier vide</span>';return}const{data:x}=await _.from("clubs").select("id,encoded_name"),E={};(x||[]).forEach(y=>{E[y.encoded_name.toUpperCase()]=y.id});let o=0,l=0;const a=[],r=["GK","DEF","MIL","ATT"],p=["normal","pepite","papyte","legende"],c=["blanc","metisse","typ","noir"],f=["blond","marron","noir","chauve"],m=["rase","court","milong","long"];for(const y of g){if(!y.firstname||!y.surname_real||!y.country_code||!y.job){l++,a.push(`Ligne ignorée (champs requis manquants): ${y.firstname||"?"}`);continue}if(!r.includes(y.job)){l++,a.push(`${y.firstname}: job invalide "${y.job}"`);continue}const k=y.club_encoded_name&&E[y.club_encoded_name.toUpperCase()]||null,A={firstname:y.firstname,surname_real:y.surname_real,surname_real:y.surname_real||ue(y.surname_real),country_code:y.country_code.toUpperCase().slice(0,2),club_id:k,job:y.job,job2:r.includes(y.job2)?y.job2:null,note_g:parseInt(y.note_g)||0,note_d:parseInt(y.note_d)||0,note_m:parseInt(y.note_m)||0,note_a:parseInt(y.note_a)||0,rarity:p.includes(y.rarity)?y.rarity:"normal",note_min:y.note_min!==""&&y.note_min!=null?parseInt(y.note_min):null,note_max:y.note_max!==""&&y.note_max!=null?parseInt(y.note_max):null,skin:c.includes(y.skin)?y.skin:"blanc",hair:f.includes(y.hair)?y.hair:"noir",hair_length:m.includes(y.hair_length)?y.hair_length:"court",sell_price:parseInt(y.sell_price)||0},{error:v}=await _.from("players").insert(A);v?(l++,a.push(`${y.firstname} ${y.surname_real}: ${v.message}`)):o++}d.innerHTML=`<div style="color:var(--green)">✅ ${o} joueurs importés</div>
...
`).filter(g=>g.trim());if(t.length<2)return[];const n=t[0],i=n.split(";").length>n.split(",").length?";":",",d=Ue(n,i).map(g=>g.trim().replace(/^\uFEFF/,"").toLowerCase()),s=[];for(let g=1;g<t.length;g++){if(!t[g].trim())continue;const x=Ue(t[g],i),E={};d.forEach((o,l)=>{E[o]=(x[l]||"").trim()}),!Object.values(E).every(o=>!o)&&s.push(E)}return s}function Ue(e,t=","){const n=[];let i="",d=!1;for(let s=0;s<e.length;s++){const g=e[s];d?g==='"'?e[s+1]==='"'?(i+='"',s++):d=!1:i+=g:g==='"'?d=!0:g===t?(n.push(i),i=""):i+=g}return n.push(i),n}const En={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};async function _n(e,{toast:t}){const n=Object.keys(ge);let i=n[0];const{data:d}=await _.from("formation_links_overrides").select("formation, links"),s={};(d||[]).forEach(l=>{s[l.formation]=l.links});let g=new Set;function x(l,a){return[l,a].sort().join("-")}function E(l){const a=s[l]||ge[l]||[];g=new Set(a.map(([r,p])=>x(r,p)))}E(i),e.innerHTML=`
  ...
  `;function o(){const l=jt[i]||{},a=Re(i),r=320,p=400,c=22;function f(v){const I=l[v];return I?{x:I.x*r,y:I.y*p}:null}let m=`<svg width="${r}" height="${p}" viewBox="0 0 ${r} ${p}" xmlns="http://www.w3.org/2000/svg" style="display:block">`;a.forEach(([v,I],h)=>{const b=f(v),$=f(I);if(!b||!$)return;const u=x(v,I),w=g.has(u),L=w?"#3b82f6":"#999",S=w?.95:.35,T=w?4:3;m+=`<line x1="${b.x}" y1="${b.y}" x2="${$.x}" y2="${$.y}"
        stroke="transparent" stroke-width="14" class="link-hit" data-key="${u}" style="cursor:pointer"/>`,m+=`<line x1="${b.x}" y1="${b.y}" x2="${$.x}" y2="${$.y}"
        stroke="${L}" stroke-width="${T}" stroke-dasharray="${w?"none":"4,3"}"
        opacity="${S}" pointer-events="none" data-line-key="${u}"/>`});for(const v of Object.keys(l)){const I=f(v);if(!I)continue;const h=v.replace(/\d+/,""),b=En[h]||"#555";m+=`<circle cx="${I.x}" cy="${I.y}" r="${c}" fill="${b}" stroke="rgba(255,255,255,0.7)" stroke-width="2" pointer-events="none"/>`,m+=`<text x="${I.x}" y="${I.y+4}" text-anchor="middle" font-size="10" font-weight="900" fill="white" font-family="Arial Black,Arial" pointer-events="none">${v}</text>`}m+="</svg>",document.getElementById("field-wrap").innerHTML=m;const y=document.getElementById("links-list");y.innerHTML=a.map(([v,I])=>{const h=x(v,I),b=g.has(h);return`
        ...
        </button>`}).join("");const k=e.querySelector('div[style*="font-weight:700;font-size:13px;margin-bottom:8px"]');k&&(k.textContent=`Liens (${g.size}/${a.length} actifs)`);function A(v){g.has(v)?g.delete(v):g.add(v),o()}e.querySelectorAll(".link-hit").forEach(v=>{v.addEventListener("click",()=>A(v.dataset.key))}),e.querySelectorAll(".link-toggle").forEach(v=>{v.addEventListener("click",()=>A(v.dataset.key))})}o(),document.getElementById("formation-select").addEventListener("change",l=>{i=l.target.value,E(i),o()}),document.getElementById("reset-btn").addEventListener("click",()=>{const l=ge[i]||[];g=new Set(l.map(([a,r])=>x(a,r))),o(),t("Liens réinitialisés aux valeurs par défaut (non sauvegardé)","info")}),document.getElementById("save-btn").addEventListener("click",async()=>{const a=Re(i).filter(([p,c])=>g.has(x(p,c))),{error:r}=await _.from("formation_links_overrides").upsert({formation:i,links:a,updated_at:new Date().toISOString()});if(r){t(r.message,"error");return}s[i]=a,t(`Liens enregistrés pour ${i} (${a.length} actifs)`,"success")})}const wn=[{value:"player",label:"Joueur"},{value:"formation",label:"Formation"},{value:"game_changer",label:"Game Changer"},{value:"ultra_game_changer",label:"Ultra Game Changer"},{value:"game_helper",label:"Game Helper"},{value:"stadium",label:"🏟️ Stade"}],In=[{value:"",label:"Toutes"},{value:"normal",label:"Normal"},{value:"pepite",label:"Pépite"},{value:"papyte",label:"Papyte"},{value:"legende",label:"Légende"}],$n=[{value:"credits",label:"Crédits"},{value:"pub",label:"Publicité"},{value:"free",label:"Gratuit"}];async function kn(e){e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">Chargement...</div>';const{data:t}=await _.from("booster_configs").select("*").order("sort_order");let n=null,i=[],d=null;const s=()=>window.innerWidth<700;async function g(){if(!n){i=[];return}if(n!==d){const{data:a}=await _.from("booster_drop_rates").select("*").eq("booster_id",n).order("sort_order");i=a||[],d=n}}function x(){return`
    ...
    </div>`;const r=i.reduce((c,f)=>c+Number(f.percentage||0),0),p=Math.abs(r-100)<.1;return`
        ...
        ${i.length===0?'<div style="text-align:center;padding:16px;color:#aaa;font-size:13px">Aucun taux. Cliquez + Ligne.</div>':i.map((c,f)=>`
          <div style="display:grid;grid-template-columns:2fr 1.5fr 50px 50px 60px 32px;gap:4px;align-items:center;margin-bottom:6px;padding:6px;background:#f9f9f9;border-radius:6px" data-rate-idx="${f}">
            <select class="rate-type" data-idx="${f}" style="padding:6px 4px;border:1px solid #ddd;border-radius:4px;font-size:12px;width:100%">
              ${wn.map(m=>`<option value="${m.value}" ${c.card_type===m.value?"selected":""}>${m.label}</option>`).join("")}
            </select>
            <select class="rate-rarity" data-idx="${f}" style="padding:6px 4px;border:1px solid #ddd;border-radius:4px;font-size:12px;width:100%">
              ${In.map(m=>`<option value="${m.value}" ${(c.rarity||"")===m.value?"selected":""}>${m.label}</option>`).join("")}
            </select>
    ...
    </div>`}async function o(a=!1){a||await g();const r=!n&&s(),p=n&&s();e.innerHTML=`
    <div style="display:flex;height:calc(100vh - 56px);overflow:hidden">
      ${!s()||r||!n?x():""}
      ${!s()||p?E():""}
    </div>`,l()}function l(){var p,c,f,m,y,k,A;const a=v=>e.querySelector(v);e.querySelectorAll(".booster-row").forEach(v=>{v.addEventListener("click",I=>{I.target.closest(".btn-delete")||(n=v.dataset.id,d=null,o())})}),(p=a("#btn-back"))==null||p.addEventListener("click",()=>{n=null,o()}),(c=a("#btn-new"))==null||c.addEventListener("click",async()=>{const v=prompt("Nom du nouveau booster :");if(!(v!=null&&v.trim()))return;const{data:I,error:h}=await _.from("booster_configs").insert({name:v.trim(),price_type:"credits",price_credits:1e3,card_count:5}).select().single();if(h){alert(h.message);return}t.push(I),n=I.id,d=null,o()}),e.querySelectorAll(".btn-delete").forEach(v=>{v.addEventListener("click",async I=>{if(I.stopPropagation(),!confirm("Supprimer ce booster ?"))return;await _.from("booster_configs").delete().eq("id",v.dataset.id);const h=t.findIndex(b=>b.id===v.dataset.id);h>-1&&t.splice(h,1),n===v.dataset.id&&(n=null,d=null),o()})}),(f=a("#btn-cancel"))==null||f.addEventListener("click",()=>{n=null,o()}),(m=a("#f-price-type"))==null||m.addEventListener("change",v=>{const I=a("#credits-field");I&&(I.style.opacity=v.target.value!=="credits"?"0.4":"1")}),(y=a("#btn-pick-icon"))==null||y.addEventListener("click",async()=>{var I;const v=a("#icon-picker-grid");if(v){if(v.style.display!=="none"){v.style.display="none";return}v.innerHTML='<div style="padding:10px;text-align:center;color:#888;font-size:12px">⏳ Chargement...</div>',v.style.display="block";try{const b=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),$=Array.isArray(b)?b.filter(w=>w.name.startsWith("booster")&&/\.(png|jpg|svg|webp)$/i.test(w.name)):[];if(!$.length){v.innerHTML='<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Aucune icône "booster*" trouvée.</div>';return}const u=((I=a("#f-image-url"))==null?void 0:I.value)||"";v.innerHTML=`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(72px,1fr));gap:8px">
          ${$.map(w=>`
          <div class="icon-pick-item" data-name="${w.name}"
            style="cursor:pointer;text-align:center;padding:6px;border-radius:6px;border:2px solid ${w.name===u?"#1A6B3C":"#ddd"};background:${w.name===u?"#f0f7f0":"#fff"}">
        ...
        </div>`,v.querySelectorAll(".icon-pick-item").forEach(w=>{w.addEventListener("click",()=>{const L=a("#f-image-url");L&&(L.value=w.dataset.name),v.style.display="none"})})}catch(h){v.innerHTML=`<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Erreur API GitHub : ${h.message}</div>`}}});function r(){e.querySelectorAll("[data-rate-idx]").forEach(v=>{var h,b,$,u,w;const I=Number(v.dataset.rateIdx);I>=0&&I<i.length&&(i[I].card_type=((h=v.querySelector(".rate-type"))==null?void 0:h.value)||"player",i[I].rarity=((b=v.querySelector(".rate-rarity"))==null?void 0:b.value)||null,i[I].note_min=Number(($=v.querySelector(".rate-note-min"))==null?void 0:$.value)||null,i[I].note_max=Number((u=v.querySelector(".rate-note-max"))==null?void 0:u.value)||null,i[I].percentage=Number((w=v.querySelector(".rate-pct"))==null?void 0:w.value)||0)})}(k=a("#btn-add-rate"))==null||k.addEventListener("click",()=>{r(),i.push({id:null,booster_id:n,card_type:"player",rarity:"normal",note_min:null,note_max:null,percentage:10,sort_order:i.length}),o(!0)}),e.querySelectorAll(".btn-del-rate").forEach(v=>{v.addEventListener("click",()=>{r(),i.splice(Number(v.dataset.idx),1),o(!0)})}),(A=a("#btn-save"))==null||A.addEventListener("click",async()=>{var u,w,L,S,T,B,C,M,z,R,N,O,W;const v=(t||[]).find(D=>D.id===n);if(!v)return;const I=[];e.querySelectorAll("[data-rate-idx]").forEach(D=>{var X,Q,ee,te,ne;const q=Number(D.dataset.rateIdx);I.push({booster_id:n,card_type:((X=D.querySelector(".rate-type"))==null?void 0:X.value)||"player",rarity:((Q=D.querySelector(".rate-rarity"))==null?void 0:Q.value)||null,note_min:Number((ee=D.querySelector(".rate-note-min"))==null?void 0:ee.value)||null,note_max:Number((te=D.querySelector(".rate-note-max"))==null?void 0:te.value)||null,percentage:Number((ne=D.querySelector(".rate-pct"))==null?void 0:ne.value)||0,sort_order:q});const U=I[I.length-1];U.rarity||(U.rarity=null),U.note_min||(U.note_min=null),U.note_max||(U.note_max=null)});const h=I.reduce((D,q)=>D+q.percentage,0);if(I.length&&Math.abs(h-100)>.5){alert(`La somme doit faire 100% (actuellement ${h.toFixed(1)}%)`);return}const b={name:((w=(u=a("#f-name"))==null?void 0:u.value)==null?void 0:w.trim())||v.name,image_url:((S=(L=a("#f-image-url"))==null?void 0:L.value)==null?void 0:S.trim())||null,price_type:(T=a("#f-price-type"))==null?void 0:T.value,price_credits:Number((B=a("#f-price-credits"))==null?void 0:B.value)||0,card_count:Number((C=a("#f-card-count"))==null?void 0:C.value)||5,is_active:((M=a("#f-active"))==null?void 0:M.checked)??v.is_active,allow_duplicates:((z=a("#f-allow-dup"))==null?void 0:z.checked)??!0,sort_order:Number((R=a("#f-sort"))==null?void 0:R.value)||0,max_per_user:(N=a("#f-max-per-user"))!=null&&N.value?Number(a("#f-max-per-user").value):null,available_from:((O=a("#f-available-from"))==null?void 0:O.value)||null,available_until:((W=a("#f-available-until"))==null?void 0:W.value)||null},{error:$}=await _.from("booster_configs").update(b).eq("id",n);if($){alert($.message);return}if(Object.assign(v,b),await _.from("booster_drop_rates").delete().eq("booster_id",n),I.length){const{error:D}=await _.from("booster_drop_rates").insert(I);if(D){alert(D.message);return}}i=I,d=n,alert("✅ Booster enregistré !"),o(!0)})}o()}const yt=[{value:"normal",label:"Normal",color:"#888"},{value:"pepite",label:"Pépite",color:"#D4A017"},{value:"papyte",label:"Papyte",color:"#909090"},{value:"legende",label:"Légende",color:"#7a28b8"}];async function Ln(e){await Le(e)}async function Le(e){const{data:t,error:n}=await _.from("sell_price_configs").select("*").order("rarity").order("note_min");if(n){e.innerHTML=`<div style="padding:24px;color:red">Erreur : ${n.message}</div>`;return}e.innerHTML=`
    ...
    </div>`,document.getElementById("add-config").addEventListener("click",()=>qe(null)),document.getElementById("form-cancel").addEventListener("click",()=>{document.getElementById("config-form").style.display="none"}),document.getElementById("form-save").addEventListener("click",()=>An(e)),e.querySelectorAll("[data-edit]").forEach(i=>{const d=(t||[]).find(s=>s.id===i.dataset.edit);d&&i.addEventListener("click",()=>qe(d))}),e.querySelectorAll("[data-delete]").forEach(i=>{i.addEventListener("click",async()=>{confirm("Supprimer cette règle ?")&&(await _.from("sell_price_configs").delete().eq("id",i.dataset.delete),await Le(e))})})}function Bn(e){const t=yt.find(n=>n.value===e.rarity);return`
    ...
    </tr>`}function qe(e){document.getElementById("config-form").style.display="block",document.getElementById("form-title").textContent=e?"Modifier la règle":"Nouvelle règle",document.getElementById("form-id").value=(e==null?void 0:e.id)||"",document.getElementById("form-rarity").value=(e==null?void 0:e.rarity)||"normal",document.getElementById("form-price").value=(e==null?void 0:e.price)||1e3,document.getElementById("form-note-min").value=(e==null?void 0:e.note_min)??1,document.getElementById("form-note-max").value=(e==null?void 0:e.note_max)??10,document.getElementById("config-form").scrollIntoView({behavior:"smooth"})}async function An(e){const t=document.getElementById("form-id").value,n=document.getElementById("form-rarity").value,i=parseInt(document.getElementById("form-price").value)||0,d=parseInt(document.getElementById("form-note-min").value)||1,s=parseInt(document.getElementById("form-note-max").value)||10;if(d>s){alert("Note min doit être ≤ note max");return}const g={rarity:n,price:i,note_min:d,note_max:s,updated_at:new Date().toISOString()};let x;if(t?{error:x}=await _.from("sell_price_configs").update(g).eq("id",t):{error:x}=await _.from("sell_price_configs").insert(g),x){alert("Erreur : "+x.message);return}document.getElementById("config-form").style.display="none",await Le(e)}async function Sn(e){await Be(e)}async function Be(e){const{data:t}=await _.from("patch_notes").select("*").order("published_at",{ascending:!1});e.innerHTML=`
      ...
      <div style="width:420px;flex-shrink:0;overflow-y:auto">
        ...
        <div style="margin-bottom:16px">
          ...
          <div id="ts-toolbar" style="display:flex;flex-wrap:wrap;gap:4px;padding:6px 8px;background:#f5f5f5;border:1.5px solid #ddd;border-bottom:none;border-radius:8px 8px 0 0">
            ${[["B","bold","<strong>Gras</strong>"],["I","italic","<em>Italique</em>"],["U","underline","Souligné"],["¶","insertParagraph","Paragraphe"],["•","insertUnorderedList","Liste à puces"],["1.","insertOrderedList","Liste numérotée"],["H2","formatBlock:h2","Titre H2"],["H3","formatBlock:h3","Titre H3"],["—","insertHorizontalRule","Séparateur"]].map(([h,b,$])=>`<button type="button" data-cmd="${b}" title="${$}" style="padding:4px 8px;border:1px solid #ddd;border-radius:4px;background:#fff;font-size:12px;font-weight:700;cursor:pointer;line-height:1;min-width:28px">${h}</button>`).join("")}
      ...
      <div style="flex:1;overflow-y:auto">
        ...
        <div id="ts-list" style="display:flex;flex-direction:column;gap:10px">
    ...
    </div>`,(l=document.getElementById("ts-add"))==null||l.addEventListener("click",()=>Ge(null,(t==null?void 0:t.length)||0)),(a=document.getElementById("ts-reset"))==null||a.addEventListener("click",async()=>{if(!confirm("Remettre tutorial_done = false pour TOUS les joueurs ? Ils reverront le tutoriel à leur prochaine connexion."))return;const{error:h}=await _.from("users").update({tutorial_done:!1}).neq("id","00000000-0000-0000-0000-000000000000");alert(h?"Erreur : "+h.message:"✅ Tutorial_done réinitialisé pour tous les joueurs.")}),(r=document.getElementById("ts-cancel"))==null||r.addEventListener("click",bt),(p=document.getElementById("ts-save"))==null||p.addEventListener("click",()=>jn(e)),(c=document.getElementById("ts-preview-btn"))==null||c.addEventListener("click",Dn);const n=document.getElementById("ts-editor"),i=()=>{const h=document.getElementById("ts-content");h&&n&&(h.value=n.innerHTML)};n==null||n.addEventListener("input",i),n==null||n.addEventListener("blur",i);const d=()=>{document.activeElement!==n&&(n==null||n.focus())};(f=document.getElementById("ts-toolbar"))==null||f.querySelectorAll("[data-cmd]").forEach(h=>{h.addEventListener("mousedown",b=>{b.preventDefault(),d();const $=h.dataset.cmd;$.startsWith("formatBlock:")?document.execCommand("formatBlock",!1,$.split(":")[1]):document.execCommand($,!1,null),i()})}),(m=document.getElementById("ts-insert-color"))==null||m.addEventListener("mousedown",h=>{h.preventDefault(),d();const b=prompt("Couleur hex (ex: #D4A017 pour or, #1A6B3C pour vert, #cc2222 pour rouge) :");b&&(document.execCommand("foreColor",!1,b),i())}),(y=document.getElementById("ts-clear-format"))==null||y.addEventListener("mousedown",h=>{h.preventDefault(),d(),document.execCommand("removeFormat",!1,null),i()}),n==null||n.addEventListener("focus",()=>{n&&(n.style.borderColor="#1A6B3C")}),n==null||n.addEventListener("blur",()=>{n&&(n.style.borderColor="#ddd")});const s=document.getElementById("ts-image"),g=document.getElementById("ts-img-preview"),x=document.getElementById("ts-img-preview-el"),E=document.getElementById("ts-img-picker-grid"),o=()=>{var b;const h=(b=s==null?void 0:s.value)==null?void 0:b.trim();if(h){const $="/";x.src=`${$}icons/${h}`,g.style.display="block"}else g.style.display="none"};s==null||s.addEventListener("input",o),(k=document.getElementById("ts-img-clear"))==null||k.addEventListener("click",()=>{s&&(s.value=""),g.style.display="none",E.style.display="none"}),(A=document.getElementById("ts-img-pick"))==null||A.addEventListener("click",async()=>{if(E.style.display!=="none"){E.style.display="none";return}E.innerHTML='<div style="color:#aaa;font-size:12px">Chargement…</div>',E.style.display="block";try{const b=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),$=Array.isArray(b)?b.filter(w=>w.name.startsWith("tuto_")):[];if(!$.length){E.innerHTML='<div style="color:#aaa;font-size:12px">Aucun fichier tuto_* trouvé dans /icons/.<br>Uploade des images nommées tuto_xxx.png dans public/icons/</div>';return}const u="/";E.innerHTML='<div style="display:flex;flex-wrap:wrap;gap:8px">'+$.map(w=>`
          <div data-pick="${w.name}" style="cursor:pointer;border:2px solid transparent;border-radius:8px;padding:4px;text-align:center;transition:border-color .15s" onmouseover="this.style.borderColor='#1A6B3C'" onmouseout="this.style.borderColor='transparent'">
            <img src="${u}icons/${w.name}" style="width:70px;height:56px;object-fit:contain;display:block;border-radius:4px">
          ...
          </div>`).join("")+"</div>",E.querySelectorAll("[data-pick]").forEach(w=>{w.addEventListener("click",()=>{s&&(s.value=w.dataset.pick),o(),E.style.display="none"})})}catch(h){E.innerHTML='<div style="color:#cc2222;font-size:12px">Erreur chargement : '+h.message+"</div>"}}),(v=document.getElementById("ts-color"))==null||v.addEventListener("input",h=>{const b=document.getElementById("ts-color-hex");b&&(b.value=h.target.value)}),(I=document.getElementById("ts-color-hex"))==null||I.addEventListener("input",h=>{const b=h.target.value;if(/^#[0-9a-fA-F]{6}$/.test(b)){const $=document.getElementById("ts-color");$&&($.value=b)}}),e.querySelectorAll("[data-edit-row]").forEach(h=>{const b=(t||[]).find($=>$.id===h.dataset.editRow);b&&h.addEventListener("click",()=>Ge(b))}),e.querySelectorAll("[data-delete]").forEach(h=>{h.addEventListener("click",async()=>{confirm("Supprimer cette étape ?")&&(await _.from("tutorial_steps").delete().eq("id",h.dataset.delete),Z(e))})}),e.querySelectorAll("[data-toggle]").forEach(h=>{h.addEventListener("click",async()=>{const b=(t||[]).find($=>$.id===h.dataset.toggle);b&&(await _.from("tutorial_steps").update({is_active:!b.is_active}).eq("id",b.id),Z(e))})}),e.querySelectorAll("[data-up]").forEach(h=>{h.addEventListener("click",async()=>{const b=t||[],$=b.findIndex(u=>u.id===h.dataset.up);$<=0||(await Promise.all([_.from("tutorial_steps").update({step_order:b[$-1].step_order}).eq("id",b[$].id),_.from("tutorial_steps").update({step_order:b[$].step_order}).eq("id",b[$-1].id)]),Z(e))})}),e.querySelectorAll("[data-down]").forEach(h=>{h.addEventListener("click",async()=>{const b=t||[],$=b.findIndex(u=>u.id===h.dataset.down);$<0||$>=b.length-1||(await Promise.all([_.from("tutorial_steps").update({step_order:b[$+1].step_order}).eq("id",b[$].id),_.from("tutorial_steps").update({step_order:b[$].step_order}).eq("id",b[$+1].id)]),Z(e))})})}function Rn(e){return`
  <div data-edit-row="${e.id}" style="background:#fff;border-radius:10px;padding:12px 16px;box-shadow:0 1px 4px rgba(0,0,0,0.08);display:flex;align-items:center;gap:12px;cursor:pointer">
    ...
    </div>`,E.querySelectorAll("ul,ol").forEach(a=>{a.style.paddingLeft="20px",a.style.margin="6px 0"}),E.querySelectorAll("li").forEach(a=>{a.style.marginBottom="4px"}),E.querySelectorAll("p").forEach(a=>{a.style.marginBottom="8px"})}async function jn(e){var a,r;const t=document.getElementById("ts-id").value,n=document.getElementById("ts-emoji").value.trim()||"⚽",i=document.getElementById("ts-title").value.trim(),d=document.getElementById("ts-editor"),s=(d?d.innerHTML:document.getElementById("ts-content").value).trim(),g=document.getElementById("ts-color-hex").value.trim()||"#1A6B3C",x=parseInt(document.getElementById("ts-order").value)||0,E=document.getElementById("ts-active").checked;if(!i||!s){alert("Titre et contenu sont obligatoires.");return}const o={emoji:n,title:i,content:s,color:g,step_order:x,is_active:E,image_url:((r=(a=document.getElementById("ts-image"))==null?void 0:a.value)==null?void 0:r.trim())||null};let l;if(t?{error:l}=await _.from("tutorial_steps").update(o).eq("id",t):{error:l}=await _.from("tutorial_steps").insert(o),l){alert("Erreur : "+l.message);return}bt(),Z(e)}const vt="/",xt=[["MA","Maroc"],["FR","France"],["AR","Argentine"],["PT","Portugal"],["BR","Brésil"],["ES","Espagne"],["DE","Allemagne"],["GB","Angleterre"],["IT","Italie"],["CM","Cameroun"],["SN","Sénégal"],["NG","Nigéria"],["DK","Danemark"],["NL","Pays-Bas"],["BE","Belgique"],["CI","Côte d'Ivoire"],["AL","Albanie"],["HR","Croatie"],["RS","Serbie"],["TR","Turquie"],["MW","Malawi"],["GH","Ghana"],["ZA","Afrique du Sud"],["EG","Égypte"],["US","États-Unis"],["MX","Mexique"],["CO","Colombie"],["UY","Uruguay"],["CL","Chili"],["JP","Japon"],["KR","Corée du Sud"],["CN","Chine"],["AU","Australie"],["SW","Suisse"],["SE","Suède"],["NO","Norvège"],["PL","Pologne"]];async function Pn(e,t){await ce(e,t)}async function ce(e,t){var d,s;const[{data:n},{data:i}]=await Promise.all([_.from("stadium_definitions").select("*, club:clubs(encoded_name, logo_url)").order("name"),_.from("clubs").select("id, encoded_name, logo_url").order("encoded_name")]);e.innerHTML=`
    <div style="padding:20px;max-width:1100px;margin:0 auto">
      ...
      <div style="display:flex;flex-wrap:wrap;gap:14px" id="st-list">
        ${(n||[]).length?(n||[]).map(g=>Nn(g)).join(""):'<div style="color:var(--tile-fg-dim);padding:24px;text-align:center;width:100%">Aucun stade créé.</div>'}
      </div>
    </div>`,(d=document.getElementById("st-add-btn"))==null||d.addEventListener("click",()=>Ke(null,i,e,t)),(s=document.getElementById("st-gen-countries-btn"))==null||s.addEventListener("click",async()=>{const{toast:g}=t,x=new Set((n||[]).filter(r=>!r.club_id&&r.country_code).map(r=>r.country_code)),E=xt.filter(([r])=>!x.has(r));if(!E.length){g("Tous les pays ont déjà un stade.","info");return}if(!confirm(`Créer ${E.length} stade(s) "Pays" manquant(s) ?`))return;const o=E.map(([r,p])=>({name:`Stade ${p}`,club_id:null,country_code:r})),{data:l,error:a}=await _.from("stadium_definitions").insert(o).select();if(a){g("Erreur : "+a.message,"error");return}l!=null&&l.length&&await _.from("cards").insert(l.map(r=>({card_type:"stadium",stadium_id:r.id}))),g(`${(l==null?void 0:l.length)||0} stade(s) Pays créé(s) ✅`,"success"),ce(e,t)}),e.querySelectorAll("[data-edit-stadium]").forEach(g=>{g.addEventListener("click",()=>{const x=(n||[]).find(E=>E.id===g.dataset.editStadium);x&&Ke(x,i,e,t)})})}function Nn(e){var d,s;let t=null;e.image_url?t=e.image_url.startsWith("http")?e.image_url:`${vt}icons/${e.image_url}`:(d=e.club)!=null&&d.logo_url?t=e.club.logo_url:e.country_code&&(t=`https://flagsapi.com/${e.country_code.slice(0,2).toUpperCase()}/flat/64.png`);const n=((s=e.club)==null?void 0:s.encoded_name)||e.country_code||"—",i=Qe(e.name,t,`${n}<br>+10 ⭐`,{width:140});return`<div data-edit-stadium="${e.id}" style="cursor:pointer;transition:transform .15s" onmouseover="this.style.transform='translateY(-3px)'" onmouseout="this.style.transform=''">
    ${i}
  ...
  `);const o=()=>{var b,$,u,w,L,S,T;const p=((b=document.getElementById("st-name"))==null?void 0:b.value)||"NOM DU STADE",c=(u=($=document.getElementById("st-image"))==null?void 0:$.value)==null?void 0:u.trim(),f=(L=(w=document.getElementById("st-country"))==null?void 0:w.value)==null?void 0:L.trim(),m=document.getElementById("st-club"),y=(m==null?void 0:m.selectedIndex)||0,k=m&&y>0?m.options[y].text:"",A=((T=(S=m==null?void 0:m.options[y])==null?void 0:S.getAttribute)==null?void 0:T.call(S,"data-logo"))||"";let v=null;c?v=c.startsWith("http")?c:`${vt}icons/${c}`:A?v=A:f&&(v=`https://flagsapi.com/${f.toLowerCase().slice(0,2).toUpperCase()}/flat/64.png`);const I=k||f||"—",h=document.getElementById("st-preview-card");h&&(h.innerHTML=Qe(p,v,`${I}<br>+10 ⭐`,{width:140}))};o(),["st-name","st-club","st-country","st-image"].forEach(p=>{var c,f;(c=document.getElementById(p))==null||c.addEventListener("input",o),(f=document.getElementById(p))==null||f.addEventListener("change",o)}),(l=document.getElementById("st-cancel"))==null||l.addEventListener("click",()=>s()),(a=document.getElementById("st-save"))==null||a.addEventListener("click",async()=>{const p=document.getElementById("st-name").value.trim(),c=document.getElementById("st-club").value||null,f=document.getElementById("st-country").value.trim().toUpperCase()||null,m=document.getElementById("st-image").value.trim()||null;if(!p){g("Le nom est obligatoire","error");return}const y={name:p,club_id:c,country_code:f,image_url:m},{error:k}=e?await _.from("stadium_definitions").update(y).eq("id",e.id):await _.from("stadium_definitions").insert(y);if(k){g("Erreur : "+k.message,"error");return}g(e?"Stade modifié ✅":"Stade créé ✅","success"),s(),ce(n,i)}),(r=document.getElementById("st-delete"))==null||r.addEventListener("click",async()=>{confirm("Supprimer ce stade ?")&&(await _.from("stadium_definitions").delete().eq("id",e.id),g("Stade supprimé","success"),s(),ce(n,i))})}const K=[{value:"game_changer",label:"⚡ Game Changer",border:"#7a28b8",bg:"linear-gradient(135deg,#4a0080,#7a28b8)"},{value:"ultra_game_changer",label:"💎 Ultra Game Changer",border:"#00bcd4",bg:"linear-gradient(135deg,#006080,#00bcd4)"}],Fn=[{value:"purple",label:"Violet",hex:"#7a28b8"},{value:"light_blue",label:"Bleu clair",hex:"#00bcd4"}],G=[{value:"BOOST_STAT",label:"⚡ Boost stats (+N)",hasValue:!0,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"DEBUFF_STAT",label:"💀 Debuff stats (-N)",hasValue:!0,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"GRAY_PLAYER",label:"❄️ Griser un joueur",hasValue:!1,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"REVIVE_PLAYER",label:"💫 Ressusciter un joueur",hasValue:!1,hasTarget:!1,hasCount:!0,hasRoles:!1},{value:"REMOVE_GOAL",label:"🚫 Retirer un but adverse",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1},{value:"ADD_GOAL_DRAW",label:"⚽ +1 but si duel nul",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1},{value:"ADD_SUB",label:"🔄 Ajouter un remplacement",hasValue:!1,hasTarget:!1,hasCount:!0,hasRoles:!1},{value:"CUSTOM",label:"🛠️ Effet custom (hardcodé)",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1}],Je=["GK","DEF","MIL","ATT"];async function On(e,t){await Ae(e,t)}async function Ae(e,t){var i;const{data:n}=await _.from("gc_definitions").select("*").order("sort_order").order("created_at");e.innerHTML=`
    <div style="padding:20px;max-width:1100px;margin:0 auto">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px">
    ...
    </div>`,(i=document.getElementById("gc-add-btn"))==null||i.addEventListener("click",()=>Ve(null,e,t)),e.querySelectorAll("[data-edit-gc]").forEach(d=>{d.addEventListener("click",()=>{const s=(n||[]).find(g=>g.id===d.dataset.editGc);s&&Ve(s,e,t)})}),e.querySelectorAll(".btn-del-gc").forEach(d=>{d.addEventListener("click",async s=>{s.stopPropagation(),confirm("Supprimer cette carte Game Changer ?")&&(await _.from("gc_definitions").delete().eq("id",d.dataset.del),Ae(e,t))})})}function Un(e){const t=K.find(d=>d.value===e.gc_type)||K[0],n=e.image_url?`/icons/${e.image_url}`:null,i=be(e.name||"?",n,t.label.split(" ")[0],e.effect||"",{width:130});return`<div data-edit-gc="${e.id}" style="position:relative;cursor:pointer">
    ${i}
    <div style="position:absolute;top:4px;left:4px;z-index:10;display:flex;gap:3px">
  ...
  </div>`}function Ve(e,t,n){var r,p,c,f;const{openModal:i,closeModal:d,toast:s}=n,g=!e,x=e||{name:"",gc_type:"game_changer",color:"purple",effect:"",image_url:"",sort_order:0,is_active:!0,effect_type:"BOOST_STAT",effect_params:{}},E=(()=>{const m=x.effect_params||{},y=G.find(k=>k.value===(x.effect_type||"BOOST_STAT"))||G[0];return`
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
      ${y.hasValue?`<div>
    ...
    <div style="display:flex;justify-content:center;margin-bottom:16px;padding:20px;background:#1a1a2e;border-radius:12px">
      <div id="gc-modal-preview">${be(x.name||"?",x.image_url?`/icons/${x.image_url}`:null,(K.find(m=>m.value===x.gc_type)||K[0]).label.split(" ")[0],x.effect||"",{width:150})}</div>
    </div>
    ...
    <div style="display:flex;flex-direction:column;gap:12px">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
        <div>
          <label>TYPE</label>
          <select id="gc-type">
            ${K.map(m=>`<option value="${m.value}" ${x.gc_type===m.value?"selected":""}>${m.label}</option>`).join("")}
          </select>
        </div>
        <div>
          <label>COULEUR</label>
          <select id="gc-color">
            ${Fn.map(m=>`<option value="${m.value}" ${x.color===m.value?"selected":""}>${m.label}</option>`).join("")}
          </select>
        </div>
      </div>
      <div>
  ...
  `);const a=()=>{var h,b,$,u,w;const m=((h=document.getElementById("gc-name"))==null?void 0:h.value)||"?",y=((b=document.getElementById("gc-effect"))==null?void 0:b.value)||"",k=(($=document.getElementById("gc-type"))==null?void 0:$.value)||"game_changer",A=(w=(u=document.getElementById("gc-image-url"))==null?void 0:u.value)==null?void 0:w.trim(),v=K.find(L=>L.value===k)||K[0],I=document.getElementById("gc-modal-preview");I&&(I.innerHTML=be(m,A?`/icons/${A}`:null,v.label.split(" ")[0],y,{width:150}))};["gc-name","gc-effect","gc-type","gc-image-url"].forEach(m=>{var y,k;(y=document.getElementById(m))==null||y.addEventListener("input",a),(k=document.getElementById(m))==null||k.addEventListener("change",a)}),(r=document.getElementById("gc-effect-type"))==null||r.addEventListener("change",()=>{const m=document.getElementById("gc-effect-type").value,y=G.find(A=>A.value===m)||G[0],k=document.getElementById("gc-params-wrap");k.innerHTML=`
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
    ...
    </div>`}),(p=document.getElementById("btn-pick-gc-icon"))==null||p.addEventListener("click",async()=>{var y;const m=document.getElementById("gc-icon-picker");if(m){if(m.style.display!=="none"){m.style.display="none";return}m.innerHTML='<div style="padding:10px;text-align:center;color:#888;font-size:12px">⏳ Chargement...</div>',m.style.display="block";try{const A=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),v=Array.isArray(A)?A.filter(h=>h.name.startsWith("gamechanger-")&&/\.(png|jpg|svg|webp)$/i.test(h.name)):[];if(!v.length){m.innerHTML='<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Aucun fichier "gamechanger-*" trouvé dans public/icons/</div>';return}const I=((y=document.getElementById("gc-image-url"))==null?void 0:y.value)||"";m.innerHTML=`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(72px,1fr));gap:8px">
        ${v.map(h=>`
      ...
      </div>`,m.querySelectorAll(".gc-icon-item").forEach(h=>{h.addEventListener("click",()=>{const b=document.getElementById("gc-image-url");b&&(b.value=h.dataset.name),m.style.display="none",a()})})}catch(k){m.innerHTML=`<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Erreur API GitHub : ${k.message}</div>`}}}),(c=document.getElementById("gc-cancel"))==null||c.addEventListener("click",()=>d()),(f=document.getElementById("gc-save"))==null||f.addEventListener("click",async()=>{var I,h,b,$,u,w,L,S,T,B,C,M,z,R;const m=G.find(N=>{var O;return N.value===(((O=document.getElementById("gc-effect-type"))==null?void 0:O.value)||"BOOST_STAT")})||G[0],y=m.hasRoles?[...document.querySelectorAll(".gc-p-role:checked")].map(N=>N.value):null,k={...m.hasValue?{value:Number((I=document.getElementById("gc-p-value"))==null?void 0:I.value)||2}:{},...m.hasCount?{count:Number((h=document.getElementById("gc-p-count"))==null?void 0:h.value)||1}:{},...m.hasTarget?{target:((b=document.getElementById("gc-p-target"))==null?void 0:b.value)||"home"}:{},...m.hasRoles?{roles:y!=null&&y.length?y:null}:{}},A={name:((u=($=document.getElementById("gc-name"))==null?void 0:$.value)==null?void 0:u.trim())||"",effect:((L=(w=document.getElementById("gc-effect"))==null?void 0:w.value)==null?void 0:L.trim())||null,image_url:((T=(S=document.getElementById("gc-image-url"))==null?void 0:S.value)==null?void 0:T.trim())||null,gc_type:((B=document.getElementById("gc-type"))==null?void 0:B.value)||"game_changer",color:((C=document.getElementById("gc-color"))==null?void 0:C.value)||"purple",sort_order:Number((M=document.getElementById("gc-sort"))==null?void 0:M.value)||0,is_active:((z=document.getElementById("gc-active"))==null?void 0:z.checked)??!0,effect_type:((R=document.getElementById("gc-effect-type"))==null?void 0:R.value)||"BOOST_STAT",effect_params:k};if(!A.name){s("Le nom est obligatoire","error");return}const{error:v}=g?await _.from("gc_definitions").insert(A):await _.from("gc_definitions").update(A).eq("id",x.id);if(v){s(v.message,"error");return}s(g?"Carte créée ✅":"Carte modifiée ✅","success"),d(),Ae(t,n)})}async function qn(e,{toast:t,openModal:n,closeModal:i}){await j(e,{toast:t,openModal:n,closeModal:i})}async function j(e,t){var h,b,$;const{toast:n,openModal:i,closeModal:d}=t,[{data:s,error:g},{data:x},{data:E},{data:o}]=await Promise.all([_.from("ranked_seasons").select("*").order("start_at",{ascending:!1}),_.from("users").select("mmr,rank_tier,placement_matches").gte("placement_matches",1),_.from("season_reward_tiers").select("*").order("rank_min"),_.from("booster_configs").select("id,name").order("sort_order")]);if(g){e.innerHTML=`<p style="color:red">${g.message}</p>`;return}const l=s||[],a=l.find(u=>u.is_active),r=(x||[]).filter(u=>(u.placement_matches||0)>=1),p=r.filter(u=>(u.placement_matches||0)>=10),c=["bronze","silver","gold","platinum","diamond","master"],f={bronze:"Bronze",silver:"Argent",gold:"Or",platinum:"Platine",diamond:"Diamant",master:"Maître"},m={bronze:"#cd7f32",silver:"#a0a0a0",gold:"#D4A017",platinum:"#00e5ff",diamond:"#5b9bd5",master:"#9b59b6"},y={bronze:"🥉",silver:"🥈",gold:"🥇",platinum:"💎",diamond:"🔷",master:"👑"},k={};c.forEach(u=>{k[u]=0}),p.forEach(u=>{const w=u.rank_tier||"bronze";k[w]!==void 0&&k[w]++});const A=p.length?Math.round(p.reduce((u,w)=>u+(w.mmr||1e3),0)/p.length):0;function v(u){return u?new Date(u).toLocaleDateString("fr",{day:"numeric",month:"short",year:"numeric"}):"—"}function I(u){const w=new Date;return u.is_active?'<span style="background:#1A6B3C;color:#fff;padding:2px 8px;border-radius:10px;font-size:11px;font-weight:700">● ACTIVE</span>':new Date(u.end_at)<w?'<span style="background:#555;color:#ccc;padding:2px 8px;border-radius:10px;font-size:11px">Terminée</span>':'<span style="background:#e67e22;color:#fff;padding:2px 8px;border-radius:10px;font-size:11px">À venir</span>'}e.innerHTML=`
    <div style="display:flex;flex-direction:column;gap:20px">
      ...
      <div class="card-panel" style="background:linear-gradient(135deg,#0a3d1e,#1a6b3c20);border:1.5px solid #1A6B3C40">
        ...
        <div style="display:flex;flex-direction:column;gap:4px">
          ${c.map(u=>{const w=k[u],L=p.length>0?Math.round(w/p.length*100):0;return`
              <div style="display:flex;align-items:center;gap:8px;font-size:12px">
                <span style="width:80px">${y[u]} ${f[u]}</span>
                <div style="flex:1;background:rgba(255,255,255,0.08);border-radius:4px;height:14px;overflow:hidden">
                  <div style="height:100%;width:${L}%;background:${m[u]};border-radius:4px;transition:width 0.5s"></div>
                </div>
        ...
        <div class="table-wrap">
          <table>
            ...
            <tbody id="seasons-tbody">
              ${l.length===0?'<tr><td colspan="6" style="text-align:center;color:var(--gray-600);padding:24px">Aucune saison créée.</td></tr>':l.map(u=>{const w=Math.round((new Date(u.end_at)-new Date(u.start_at))/864e5),L=new Date(u.end_at)<new Date&&!u.is_active;return`
                    <tr>
                      <td><b>${u.name}</b></td>
                      <td style="font-size:12px">${v(u.start_at)}</td>
                      <td style="font-size:12px">${v(u.end_at)}</td>
                      <td style="font-size:12px">${w} jours</td>
                      <td>${I(u)}</td>
                      <td style="display:flex;gap:6px;flex-wrap:wrap">
                        <button class="btn btn-ghost btn-sm" data-edit="${u.id}">✏️ Modifier</button>
                        <button class="btn btn-ghost btn-sm" data-rewards-for="${u.id}" style="color:#D4A017;border-color:#D4A017">🏆 Récompenses</button>
                        ${u.is_active?"":`<button class="btn btn-yellow btn-sm" data-launch="${u.id}">▶ Lancer la saison</button>`}
                        ...
                        ${L?`<button class="btn btn-danger btn-sm" data-delete="${u.id}">🗑</button>`:""}
                      </td>
                    </tr>`}).join("")}
            </tbody>
          </table>
      ...
      <div class="card-panel" id="rewards-section">
        ...
        <div class="table-wrap">
          <table>
            ...
            <tbody>
              ${(E||[]).map(u=>{var S;const w=!u.activate_at||new Date(u.activate_at)<=new Date;return`<tr>
                  <td style="font-size:12px">${((S=l.find(T=>T.id===u.season_id))==null?void 0:S.name)||"—"}</td>
                  <td><b>${u.label}</b></td>
                  <td>${u.rank_min===u.rank_max?`#${u.rank_min}`:`#${u.rank_min}–${u.rank_max}`}</td>
                  <td style="text-align:right">${(u.credits||0).toLocaleString("fr")}</td>
                  <td style="text-align:center">${(u.player_ids||[]).length}</td>
                  <td style="text-align:center">${(u.booster_config_ids||[]).length}</td>
                  <td style="font-size:12px">${u.activate_at?v(u.activate_at):"Immédiat"}</td>
                  <td>
                    ${u.distributed_at?`<span style="background:#555;color:#ccc;padding:2px 8px;border-radius:10px;font-size:11px">Distribué le ${v(u.distributed_at)}</span>`:w?'<span style="background:#1A6B3C;color:#fff;padding:2px 8px;border-radius:10px;font-size:11px">● Prêt</span>':'<span style="background:#e67e22;color:#fff;padding:2px 8px;border-radius:10px;font-size:11px">⏳ Programmé</span>'}
                  </td>
                  <td style="white-space:nowrap">
                    <button class="btn btn-ghost btn-sm" data-edit-tier="${u.id}">✏️</button>
                    <button class="btn btn-primary btn-sm" data-distribute-tier="${u.id}" ${w?"":"disabled"}>🎁 Distribuer</button>
                    <button class="btn btn-danger btn-sm" data-delete-tier="${u.id}">🗑️</button>
                  </td>
    ...
    </div>`,(h=document.getElementById("create-season-btn"))==null||h.addEventListener("click",()=>{Ye(null,{toast:n,openModal:i,closeModal:d,reload:()=>j(e,t)})}),(b=document.getElementById("start-new-season-btn"))==null||b.addEventListener("click",()=>{Hn(a,{toast:n,openModal:i,closeModal:d,reload:()=>j(e,t)})}),e.querySelectorAll("[data-edit]").forEach(u=>{const w=l.find(L=>L.id==u.dataset.edit);u.addEventListener("click",()=>{Ye(w,{toast:n,openModal:i,closeModal:d,reload:()=>j(e,t)})})}),e.querySelectorAll("[data-activate]").forEach(u=>{u.addEventListener("click",async()=>{const w=parseInt(u.dataset.activate);if(!confirm("Activer cette saison ? La saison actuellement active sera désactivée."))return;const{error:L}=await _.from("ranked_seasons").update({is_active:!1}).neq("id",0);if(L){n(L.message,"error");return}const{error:S}=await _.from("ranked_seasons").update({is_active:!0}).eq("id",w);if(S){n(S.message,"error");return}await _.from("users").update({current_season_id:w}).gt("placement_matches",0),n("Saison activée ✅","success"),j(e,t)})}),e.querySelectorAll("[data-rewards-for]").forEach(u=>{u.addEventListener("click",()=>{var S;const w=parseInt(u.dataset.rewardsFor);(S=document.getElementById("rewards-section"))==null||S.scrollIntoView({behavior:"smooth",block:"start"});const L=l.find(T=>T.id===w);ye(null,l,L,o||[],{toast:n,openModal:i,closeModal:d,reload:()=>j(e,t)})})}),e.querySelectorAll("[data-launch]").forEach(u=>{u.addEventListener("click",async()=>{const w=parseInt(u.dataset.launch),L=l.find(B=>B.id===w);if(!confirm(`Lancer "${L==null?void 0:L.name}" ?
...
Action irréversible. Continuer ?`))return;u.disabled=!0,u.textContent="⏳ Lancement...";const{data:S,error:T}=await _.rpc("admin_launch_season",{p_season_id:w});if(u.disabled=!1,u.textContent="▶ Lancer la saison",T){n(T.message,"error");return}if(!(S!=null&&S.success)){n((S==null?void 0:S.error)||"Échec du lancement","error");return}n(`Saison lancée ✅ (${S.logged} classement(s) archivé(s), ${S.reset} joueur(s) recalculé(s))`,"success"),j(e,t)})}),e.querySelectorAll("[data-deactivate]").forEach(u=>{u.addEventListener("click",async()=>{if(!confirm("Désactiver la saison active ? Le mode Ranked sera suspendu."))return;const{error:w}=await _.from("ranked_seasons").update({is_active:!1}).eq("id",parseInt(u.dataset.deactivate));if(w){n(w.message,"error");return}n("Saison désactivée","success"),j(e,t)})}),e.querySelectorAll("[data-delete]").forEach(u=>{u.addEventListener("click",async()=>{if(!confirm("Supprimer cette saison archivée ?"))return;const{error:w}=await _.from("ranked_seasons").delete().eq("id",parseInt(u.dataset.delete));if(w){n(w.message,"error");return}n("Saison supprimée","success"),j(e,t)})}),($=document.getElementById("add-reward-tier-btn"))==null||$.addEventListener("click",()=>{ye(null,l,a,o||[],{toast:n,openModal:i,closeModal:d,reload:()=>j(e,t)})}),e.querySelectorAll("[data-edit-tier]").forEach(u=>{const w=(E||[]).find(L=>L.id===u.dataset.editTier);u.addEventListener("click",()=>{ye(w,l,a,o||[],{toast:n,openModal:i,closeModal:d,reload:()=>j(e,t)})})}),e.querySelectorAll("[data-distribute-tier]").forEach(u=>{u.addEventListener("click",async()=>{const w=(E||[]).find(T=>T.id===u.dataset.distributeTier);if(!confirm(`Distribuer les récompenses du palier "${w==null?void 0:w.label}" à tous les joueurs concernés ?`))return;u.disabled=!0,u.textContent="⏳...";const{data:L,error:S}=await _.rpc("admin_distribute_season_reward",{p_tier_id:u.dataset.distributeTier});if(S){n(S.message,"error"),u.disabled=!1,u.textContent="🎁 Distribuer";return}if(!(L!=null&&L.success)){n((L==null?void 0:L.error)||"Échec de la distribution","error"),u.disabled=!1,u.textContent="🎁 Distribuer";return}n(`Récompenses distribuées à ${L.rewarded} joueur(s) ✅`,"success"),j(e,t)})}),e.querySelectorAll("[data-delete-tier]").forEach(u=>{u.addEventListener("click",async()=>{if(!confirm("Supprimer ce palier de récompense ?"))return;const{error:w}=await _.from("season_reward_tiers").delete().eq("id",u.dataset.deleteTier);if(w){n(w.message,"error");return}n("Palier supprimé","success"),j(e,t)})})}function ye(e,t,n,i,{toast:d,openModal:s,closeModal:g,reload:x}){var f,m,y,k,A;const E=!!e,o=(e==null?void 0:e.season_id)??(n==null?void 0:n.id)??((f=t[0])==null?void 0:f.id)??null;let l=[];const a=v=>{if(!v)return"";const I=new Date(v);return new Date(I.getTime()-I.getTimezoneOffset()*6e4).toISOString().slice(0,16)},r=`
    <div style="display:flex;flex-direction:column;gap:12px">
      <div>
        <label>SAISON RANKED</label>
        <select id="rt-season">
          ${t.map(v=>`<option value="${v.id}" ${v.id===o?"selected":""}>${v.name}${v.is_active?" (active)":""}</option>`).join("")}
        </select>
      </div>
      <div>
        <label>LIBELLÉ</label>
  ...
  `);function c(){const v=document.getElementById("rt-player-chips");v&&(v.innerHTML=l.map(I=>`
      ...
      </span>`).join(""),v.querySelectorAll("[data-remove-player]").forEach(I=>{I.addEventListener("click",()=>{l=l.filter(h=>h.id!==I.dataset.removePlayer),c()})}))}(m=e==null?void 0:e.player_ids)!=null&&m.length&&_.from("players").select("id,firstname,surname_real").in("id",e.player_ids).then(({data:v})=>{l=(v||[]).map(I=>({id:I.id,label:`${I.firstname} ${I.surname_real}`})),c()}),(y=document.getElementById("rt-player-search-btn"))==null||y.addEventListener("click",async()=>{const v=document.getElementById("rt-player-search").value.trim();if(v.length<2)return;const{data:I}=await _.from("players").select("id,firstname,surname_real,rarity").or(`firstname.ilike.%${v}%,surname_real.ilike.%${v}%`).limit(10),h=document.getElementById("rt-player-results");if(!(I!=null&&I.length)){h.style.display="flex",h.innerHTML='<div style="color:#999;font-size:12px;padding:4px">Aucun résultat.</div>';return}h.style.display="flex",h.innerHTML=I.map(b=>`
      ...
      </div>`).join(""),h.querySelectorAll("[data-add-player]").forEach(b=>{b.addEventListener("click",()=>{const $=b.dataset.addPlayer;l.some(u=>u.id===$)||l.push({id:$,label:b.dataset.label}),c(),h.style.display="none",document.getElementById("rt-player-search").value=""})})}),(k=document.getElementById("rt-cancel"))==null||k.addEventListener("click",()=>g()),(A=document.getElementById("rt-save"))==null||A.addEventListener("click",async()=>{const v=document.getElementById("rt-error"),I=parseInt(document.getElementById("rt-season").value),h=document.getElementById("rt-label").value.trim(),b=parseInt(document.getElementById("rt-rank-min").value)||1,$=parseInt(document.getElementById("rt-rank-max").value)||1,u=parseInt(document.getElementById("rt-credits").value)||0,w=document.getElementById("rt-activate-at").value,L=w?new Date(w).toISOString():null,S=[...document.querySelectorAll(".rt-booster-cb:checked")].map(C=>C.value);if(!h){v.textContent="Le libellé est obligatoire.";return}if(b>$){v.textContent="Le classement min doit être ≤ au max.";return}if(!I){v.textContent="Choisis une saison.";return}const T={season_id:I,label:h,rank_min:b,rank_max:$,credits:u,booster_config_ids:S,player_ids:l.map(C=>C.id),activate_at:L},{error:B}=E?await _.from("season_reward_tiers").update(T).eq("id",e.id):await _.from("season_reward_tiers").insert(T);if(B){v.textContent=B.message;return}d(E?"Palier modifié ✅":"Palier créé ✅","success"),g(),x()})}function Hn(e,{toast:t,openModal:n,closeModal:i,reload:d}){var l,a;const s=new Date,g=new Date(s.getTime()+30*864e5),x=r=>r.toISOString().slice(0,10),E=`
    <div style="display:flex;flex-direction:column;gap:12px">
  ...
  `),(l=document.getElementById("sns-cancel"))==null||l.addEventListener("click",()=>i()),(a=document.getElementById("sns-launch"))==null||a.addEventListener("click",async()=>{const r=document.getElementById("sns-error"),p=document.getElementById("sns-name").value.trim(),c=document.getElementById("sns-start").value,f=document.getElementById("sns-end").value,m=document.getElementById("sns-confirm").checked;if(!p){r.textContent="Le nom de la saison est requis.";return}if(!c||!f){r.textContent="Les deux dates sont requises.";return}if(new Date(c)>=new Date(f)){r.textContent="La date de fin doit être après la date de début.";return}if(!m){r.textContent="Coche la case de confirmation pour continuer.";return}const y=document.getElementById("sns-launch");y.disabled=!0,y.textContent="⏳ Recalcul en cours...";const{data:k,error:A}=await _.rpc("admin_start_new_season",{p_name:p,p_start_at:new Date(c).toISOString(),p_end_at:new Date(f).toISOString()});if(y.disabled=!1,y.textContent="🚀 Démarrer la saison",A){r.textContent=A.message;return}if(!(k!=null&&k.success)){r.textContent=(k==null?void 0:k.error)||"Échec de l'opération.";return}t(`Nouvelle saison démarrée ✅ (${k.users_reset} joueur(s) recalculé(s))`,"success"),i(),d()})}function Ye(e,{toast:t,openModal:n,closeModal:i,reload:d}){const s=!!e,g=new Date().toISOString().slice(0,16),x=new Date(Date.now()+90*864e5).toISOString().slice(0,16),E=e?new Date(e.start_at).toISOString().slice(0,16):g,o=e?new Date(e.end_at).toISOString().slice(0,16):x,l=(e==null?void 0:e.name)||"";n(s?"✏️ Modifier la saison":"➕ Nouvelle saison",`<div style="display:flex;flex-direction:column;gap:14px">
    ...
    </div>`);function a(){var m,y;const r=(m=document.getElementById("season-start"))==null?void 0:m.value,p=(y=document.getElementById("season-end"))==null?void 0:y.value,c=document.getElementById("season-duration");if(!r||!p||!c)return;const f=Math.round((new Date(p)-new Date(r))/864e5);c.textContent=f>0?`Durée : ${f} jour${f>1?"s":""}`:"⚠️ La fin doit être après le début",c.style.color=f>0?"var(--gray-600)":"#bb2020"}setTimeout(()=>{var r,p,c;(r=document.getElementById("season-start"))==null||r.addEventListener("input",a),(p=document.getElementById("season-end"))==null||p.addEventListener("input",a),a(),(c=document.getElementById("season-save-btn"))==null||c.addEventListener("click",async()=>{var h,b,$;const f=(h=document.getElementById("season-name"))==null?void 0:h.value.trim(),m=(b=document.getElementById("season-start"))==null?void 0:b.value,y=($=document.getElementById("season-end"))==null?void 0:$.value,k=document.getElementById("season-error");if(!f){k.textContent="Le nom est requis.";return}if(!m){k.textContent="La date de début est requise.";return}if(!y){k.textContent="La date de fin est requise.";return}if(new Date(y)<=new Date(m)){k.textContent="La date de fin doit être après le début.";return}const A=document.getElementById("season-save-btn");A.disabled=!0,A.textContent="Enregistrement…";const v={name:f,start_at:new Date(m).toISOString(),end_at:new Date(y).toISOString()};let I;if(s?{error:I}=await _.from("ranked_seasons").update(v).eq("id",e.id):{error:I}=await _.from("ranked_seasons").insert({...v,is_active:!1}),I){k.textContent=I.message,A.disabled=!1,A.textContent=s?"💾 Enregistrer":"✅ Créer la saison";return}t(s?"Saison modifiée ✅":"Saison créée ✅","success"),i(),d()})},50)}async function he(e,t){var E;const{toast:n}=t,{data:i,error:d}=await _.from("solo_levels").select("*").order("level_number");if(d){e.innerHTML=`<p style="color:var(--danger)">${d.message}</p>`;return}const s=o=>`
    <tr data-row="${o.id}">
