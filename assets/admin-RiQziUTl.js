import{s as y,e as P,r as je,g as Re,K as Ze,D as A,S as Xe,a as z,H as Qe,R as et,b as Ne,c as tt,E as nt,d as it,N as ot,M as at,B as rt,f as lt,F as se,h as he,i as st,j as dt}from"./formation-links-DwOY6Nx_.js";async function ct(e){const[{count:n},{count:t},{count:i},{count:c},{count:d}]=await Promise.all([y.from("players").select("*",{count:"exact",head:!0}),y.from("clubs").select("*",{count:"exact",head:!0}),y.from("users").select("*",{count:"exact",head:!0}),y.from("cards").select("*",{count:"exact",head:!0}),y.from("matches").select("*",{count:"exact",head:!0})]);e.innerHTML=`
  ...
  `,window.adminNav=s=>{var a;(a=document.querySelector(`[data-page="${s}"]`))==null||a.click()}}function Z(e,n,t,i){return`<div class="card-panel" style="text-align:center">
    ...
    <div style="display:flex;flex-direction:column;gap:10px">
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        ...
        <select id="filter-rarity" style="width:110px">
          ...
          ${Object.entries(de).map(([a,p])=>`<option value="${a}">${p}</option>`).join("")}
        </select>
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        <select id="filter-club" style="flex:1;min-width:140px">
          ...
          ${t.map(a=>`<option value="${a.id}">${a.encoded_name}</option>`).join("")}
        </select>
        <select id="filter-country" style="flex:1;min-width:120px">
          <option value="">Tous les pays</option>
          ${[...new Set(n.map(a=>a.country_code).filter(Boolean))].sort().map(a=>`<option value="${a}">${a}</option>`).join("")}
        </select>
      </div>
      ...
      <div id="players-list" style="display:flex;flex-direction:column;gap:6px"></div>
    </div>`;function d(){const a=document.getElementById("search-players").value.toLowerCase(),p=document.getElementById("filter-job").value,o=document.getElementById("filter-rarity").value,r=document.getElementById("filter-club").value,l=document.getElementById("filter-country").value;return n.filter(u=>(!a||`${u.firstname} ${u.surname_encoded} ${u.surname_real||""}`.toLowerCase().includes(a))&&(!p||u.job===p)&&(!o||u.rarity===o)&&(!r||u.club_id===r)&&(!l||u.country_code===l))}function s(){const a=d();document.getElementById("count-label").textContent=`${a.length} joueur(s)`;const p=document.getElementById("players-list");if(!a.length){p.innerHTML='<div style="color:var(--gray-600);padding:20px;text-align:center">Aucun joueur.</div>';return}p.innerHTML=a.map(o=>{const r=pt[o.rarity]||"#aaa",l=De[o.job]||"#aaa";return`
        <div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px">
          <!-- Note + poste -->
          <div style="flex-shrink:0;width:40px;height:40px;border-radius:10px;border:2px solid ${r};
          ...
          <div style="flex:1;min-width:0">
            ...
            <div style="font-size:10px;color:${r};font-weight:700;margin-top:2px">${de[o.rarity]||o.rarity}</div>
          </div>
          <!-- Notes secondaires -->
        ...
        </div>`}).join(""),p.querySelectorAll("[data-edit]").forEach(o=>{const r=n.find(l=>l.id===o.dataset.edit);o.addEventListener("click",()=>Ee(r,t,e,i))}),p.querySelectorAll("[data-del]").forEach(o=>{o.addEventListener("click",async()=>{if(!confirm("Supprimer ce joueur ?"))return;const{error:r}=await y.from("players").delete().eq("id",o.dataset.del);r?c(r.message,"error"):(c("Joueur supprimé ✅","success"),pe(e,i))})})}s(),document.getElementById("search-players").addEventListener("input",s),document.getElementById("filter-job").addEventListener("change",s),document.getElementById("filter-rarity").addEventListener("change",s),document.getElementById("filter-club").addEventListener("change",s),document.getElementById("filter-country").addEventListener("change",s),document.getElementById("add-player-btn").addEventListener("click",()=>Ee(null,t,e,i))}function ft(e){return e.job==="GK"?e.note_g:e.job==="DEF"?e.note_d:e.job==="MIL"?e.note_m:e.note_a}function Ee(e,n,t,i){const{toast:c,openModal:d,closeModal:s}=i,a=!!e,p='<option value="">— Club —</option>'+n.map(r=>`<option value="${r.id}" ${(e==null?void 0:e.club_id)===r.id?"selected":""}>${r.encoded_name}</option>`).join(""),o=ut.map(r=>`<option value="${r}" ${((e==null?void 0:e.country_code)||"FR")===r?"selected":""}>${r}</option>`).join("");d(a?`✏️ ${e.firstname} ${e.surname_encoded}`:"➕ Nouveau joueur",`<div style="display:flex;flex-direction:column;gap:12px">
      ...
      <div style="border-top:1px solid var(--gray-200);padding-top:10px">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:8px">
          ${[["GK","pm-g","note_g"],["DEF","pm-d","note_d"],["MIL","pm-m","note_m"],["ATT","pm-a","note_a"]].map(([r,l,u])=>`
            <div class="form-group" style="text-align:center">
              <label style="color:${De[r]};font-weight:700">${r}</label>
...
Payload:`,JSON.stringify(s)),d=p.message;continue}c++,await y.from("cards").insert({card_type:"player",player_id:a.id})}console.log("[GenSquad] Créés:",c,"/",i.length),c>0?t(`${c} joueurs générés ✅`,"success"):t("Erreur: "+(d||"inconnue"),"error")}function He(e){return{style:e.kit_style||A.style,color1:e.kit_color1||A.color1,color2:e.kit_color2||A.color2,shorts:e.kit_shorts||A.shorts,socks:e.kit_socks||A.socks}}let ne=[];async function $t(e,n){await ae(e,n)}async function ae(e,n){const[{data:t,error:i},{data:c}]=await Promise.all([y.from("clubs").select("*").order("real_name"),y.from("players").select("club_id")]);if(i){e.innerHTML=`<p style="color:red">${i.message}</p>`;return}ne=t||[];const d={};(c||[]).forEach(s=>{d[s.club_id]=(d[s.club_id]||0)+1}),kt(e,n,d)}function kt(e,n,t={}){const{toast:i,openModal:c,closeModal:d}=n;e.innerHTML=`
    ...
    <div id="clubs-list" style="display:flex;flex-direction:column;gap:8px"></div>`,s(ne),document.getElementById("search-clubs").addEventListener("input",a=>{const p=a.target.value.toLowerCase();s(ne.filter(o=>o.real_name.toLowerCase().includes(p)||o.encoded_name.toLowerCase().includes(p)))}),document.getElementById("add-club-btn").addEventListener("click",()=>we(null,e,n));function s(a){const p=document.getElementById("clubs-list");if(!a.length){p.innerHTML='<p style="color:var(--gray-600);padding:20px">Aucun club.</p>';return}p.innerHTML=a.map(o=>{const r=He(o),l=Re(r,o.id).replace("<svg ",'<svg style="width:40px;height:48px" '),u=o.logo_url?`<img src="${o.logo_url}" style="width:40px;height:40px;object-fit:contain;border-radius:8px">`:`<div style="width:40px;height:40px;background:linear-gradient(135deg,${r.color1},${r.color2});border-radius:8px;display:flex;align-items:center;justify-content:center;color:#fff;font-size:10px;font-weight:900">${o.encoded_name.slice(0,3)}</div>`,h=t[o.id]||0;return`
        <div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px">
          ${u}
          <div style="flex:1;min-width:0">
        ...
        </div>`}).join(""),p.querySelectorAll("[data-gen]").forEach(o=>{o.addEventListener("click",async()=>{confirm(`Générer 20 joueurs pour ${o.dataset.name} ?`)&&(o.disabled=!0,o.textContent="⏳",await Fe(o.dataset.gen,o.dataset.cc,i),ae(e,n))})}),p.querySelectorAll("[data-edit]").forEach(o=>{const r=ne.find(l=>l.id===o.dataset.edit);o.addEventListener("click",()=>we(r,e,n))}),p.querySelectorAll("[data-del]").forEach(o=>{o.addEventListener("click",async()=>{if(!confirm("Supprimer ce club ?"))return;const{error:r}=await y.from("clubs").delete().eq("id",o.dataset.del);r?i(r.message,"error"):(i("Club supprimé","success"),ae(e,n))})})}}function we(e,n,t){const{toast:i,openModal:c,closeModal:d}=t,s=!!e,a=e?He(e):{...A},p=Object.entries(Ze).map(([r,l])=>`<option value="${r}" ${a.style===r?"selected":""}>${l.label}</option>`).join(""),o=qe.map(r=>`<option value="${r}" ${((e==null?void 0:e.country_code)||"FR")===r?"selected":""}>${r}</option>`).join("");c(s?`✏️ ${e.real_name}`:"➕ Nouveau club",`<div style="display:flex;flex-direction:column;gap:14px">
      ...
      <div style="border-top:1px solid var(--gray-200);padding-top:12px">
        ...
        <div style="display:flex;gap:12px;align-items:flex-start">
          ...
          <div style="flex:1;display:flex;flex-direction:column;gap:8px">
            ...
            ${[["Couleur principale","m-kit-color1",a.color1],["Couleur secondaire","m-kit-color2",a.color2],["Short","m-kit-shorts",a.shorts],["Chaussettes","m-kit-socks",a.socks]].map(([r,l,u])=>`
              <div class="form-group">
                <label>${r}</label>
    ...
    </div>`),setTimeout(()=>{var h,f,E;Q(),["m-kit-color1","m-kit-color2","m-kit-shorts","m-kit-socks"].forEach(w=>{const m=document.getElementById(w),_=document.getElementById(w+"-txt");!m||!_||(m.addEventListener("input",()=>{_.value=m.value,Q()}),_.addEventListener("input",()=>{const $=_.value.trim();/^#[0-9a-fA-F]{6}$/.test($)&&(m.value=$,Q())}),_.addEventListener("change",()=>{let $=_.value.trim();$.startsWith("#")||($="#"+$),/^#[0-9a-fA-F]{6}$/.test($)&&(m.value=$,_.value=$,Q())}))}),(h=document.getElementById("m-kit-style"))==null||h.addEventListener("change",Q);const r=document.getElementById("m-real"),l=document.getElementById("m-encoded");function u(){if(!r||!l||l.value)return;const w=r.value.trim().split(/\s+/),m=w.length===1?w[0].toUpperCase().slice(0,6):w.filter(_=>_.length>2).map(_=>_[0].toUpperCase()).join("")||w[0].toUpperCase().slice(0,4);l.value=m}r==null||r.addEventListener("input",u),(f=document.getElementById("auto-encode-btn"))==null||f.addEventListener("click",()=>{l&&(l.value=""),u()}),(E=document.getElementById("m-save"))==null||E.addEventListener("click",()=>It(e,s,n,t))},50)}function Ge(){var e,n,t,i,c;return{style:((e=document.getElementById("m-kit-style"))==null?void 0:e.value)||"uni",color1:((n=document.getElementById("m-kit-color1"))==null?void 0:n.value)||"#1A6B3C",color2:((t=document.getElementById("m-kit-color2"))==null?void 0:t.value)||"#ffffff",shorts:((i=document.getElementById("m-kit-shorts"))==null?void 0:i.value)||"#111111",socks:((c=document.getElementById("m-kit-socks"))==null?void 0:c.value)||"#ffffff"}}function Q(){const e=document.getElementById("kit-preview-wrap");e&&(e.innerHTML=Re(Ge(),"modal"))}async function It(e,n,t,i){var m,_,$,v,g,b;const{toast:c,closeModal:d}=i,s=document.getElementById("m-error"),a=document.getElementById("m-save"),p=(m=document.getElementById("m-real"))==null?void 0:m.value.trim(),o=(_=document.getElementById("m-encoded"))==null?void 0:_.value.trim().toUpperCase(),r=($=document.getElementById("m-country"))==null?void 0:$.value.trim().toUpperCase(),l=((v=document.getElementById("m-logo"))==null?void 0:v.value.trim())||null,u=((g=document.getElementById("m-gen-stadium"))==null?void 0:g.checked)??!1,h=((b=document.getElementById("m-gen-squad"))==null?void 0:b.checked)??!1,f=Ge();if(!p){s.textContent="Le nom du club est requis.";return}if(!o){s.textContent="Le nom encodé est requis.";return}if(!r){s.textContent="Le pays est requis.";return}a.disabled=!0,a.textContent="Enregistrement…";const E={real_name:p,encoded_name:o,country_code:r,logo_url:l,kit_style:f.style,kit_color1:f.color1,kit_color2:f.color2,kit_shorts:f.shorts,kit_socks:f.socks};let w=e==null?void 0:e.id;if(n){const{error:x}=await y.from("clubs").update(E).eq("id",w);if(x){s.textContent=x.message,a.disabled=!1,a.textContent="💾 Enregistrer";return}}else{const{data:x,error:k}=await y.from("clubs").insert(E).select().single();if(k){s.textContent=k.message,a.disabled=!1,a.textContent="✅ Créer le club";return}if(w=x.id,u){a.textContent="🏟️ Création du stade…";const{data:L,error:I}=await y.from("stadium_definitions").insert({name:`Stade de ${p}`,club_id:w,country_code:r}).select().single();I?console.warn("[Stadium] Erreur def stade:",I.message):L&&await y.from("cards").insert({card_type:"stadium",stadium_id:L.id})}h&&(a.textContent="⚽ Génération des joueurs…",await Fe(w,r,c))}c(n?"Club modifié ✅":"Club créé ✅","success"),d(),ae(t,i)}const Lt=["normal","pepite","papyte","legende"],_e=["GK","DEF","MIL","ATT"],Bt=[["MA","Maroc"],["FR","France"],["AR","Argentine"],["PT","Portugal"],["BR","Brésil"],["ES","Espagne"],["DE","Allemagne"],["GB","Angleterre"],["IT","Italie"],["CM","Cameroun"],["SN","Sénégal"],["NG","Nigéria"],["DK","Danemark"],["NL","Pays-Bas"],["BE","Belgique"],["CI","Côte d'Ivoire"],["AL","Albanie"],["HR","Croatie"],["RS","Serbie"],["TR","Turquie"],["MW","Malawi"]];let D={...z},ie=[];async function zt(e,{toast:n}){const{data:t}=await y.from("clubs").select("id,encoded_name,logo_url,kit_style,kit_color1,kit_color2,kit_shorts,kit_socks").order("encoded_name");ie=t||[],D={...z},e.innerHTML=Tt(ie),Ct(e,ie,n),T()}function $e(){var t;const e=((t=document.getElementById("cb-surname-real"))==null?void 0:t.value)||"",n=document.getElementById("cb-surname-enc");n&&(n.value=P(e))}function At(){var t;const e=(t=document.getElementById("cb-club"))==null?void 0:t.value,n=ie.find(i=>String(i.id)===String(e));return n?{style:n.kit_style||A.style,color1:n.kit_color1||A.color1,color2:n.kit_color2||A.color2,shorts:n.kit_shorts||A.shorts,socks:n.kit_socks||A.socks}:{...A}}function T(){var u,h,f,E;const e=w=>{var m;return((m=document.getElementById(w))==null?void 0:m.value)||""},n={firstname:e("cb-firstname"),surname_encoded:e("cb-surname-enc")||P(e("cb-surname-real")),country_code:e("cb-country"),club_encoded_name:((h=(u=document.getElementById("cb-club"))==null?void 0:u.selectedOptions[0])==null?void 0:h.text)||"",job:e("cb-job"),job2:e("cb-job2")||null,note_g:parseInt(e("cb-note-g"))||0,note_d:parseInt(e("cb-note-d"))||0,note_m:parseInt(e("cb-note-m"))||0,note_a:parseInt(e("cb-note-a"))||0,rarity:e("cb-rarity")},t=At(),i=document.getElementById("cb-club"),c=((E=(f=i==null?void 0:i.selectedOptions[0])==null?void 0:f.dataset)==null?void 0:E.logo)||null,d=Ne(D,t,120),s=`data:image/svg+xml;charset=utf-8,${encodeURIComponent(d)}`,a=je(n,{portraitUrl:s,clubLogoUrl:c,showNotes:!0}),p=document.getElementById("card-preview");p&&(p.innerHTML=a);const o=document.getElementById("avatar-preview-wrap");o&&(o.innerHTML=d);const r=e("cb-surname-real"),l=document.getElementById("encode-summary");l&&r&&(l.textContent=`${e("cb-firstname")} ${r} → ${e("cb-firstname")} ${n.surname_encoded}`)}function Ue(){const e=n=>{var t;return((t=document.getElementById(n))==null?void 0:t.value)||""};D={skin:e("av-skin"),hair:e("av-hair"),hairColor:e("av-hairColor"),eyes:e("av-eyes"),nose:e("av-nose"),mouth:e("av-mouth"),eyebrows:e("av-eyebrows"),beard:e("av-beard")}}function Ct(e,n,t){var c,d,s,a,p;e.querySelectorAll("input,select").forEach(o=>{o.addEventListener("input",T),o.addEventListener("change",T)});const i=["av-skin","av-hair","av-hairColor","av-eyes","av-nose","av-mouth","av-eyebrows","av-beard"];i.forEach(o=>{var r;(r=document.getElementById(o))==null||r.addEventListener("change",()=>{Ue(),T()})}),(c=document.getElementById("cb-surname-real"))==null||c.addEventListener("input",()=>{$e(),T()}),(d=document.getElementById("btn-encode"))==null||d.addEventListener("click",()=>{$e(),T()}),(s=document.getElementById("cb-club"))==null||s.addEventListener("change",T),(a=document.getElementById("btn-save-player"))==null||a.addEventListener("click",()=>St(t)),(p=document.getElementById("btn-reset"))==null||p.addEventListener("click",()=>{e.querySelectorAll("input").forEach(o=>o.value=""),e.querySelectorAll("select").forEach(o=>o.selectedIndex=0),D={...z},i.forEach(o=>{const r=o.replace("av-",""),l=document.getElementById(o);l&&(l.value=z[r]||"")}),T()})}async function St(e){const n=s=>{var a;return((a=document.getElementById(s))==null?void 0:a.value)||""},t=n("cb-surname-real").trim(),i=n("cb-surname-enc").trim()||P(t);if(!n("cb-firstname")||!t||!n("cb-country")||!n("cb-job")){e("Remplissez les champs obligatoires (prénom, nom, pays, poste)","error");return}Ue();const c={firstname:n("cb-firstname").trim(),surname_real:t,surname_encoded:i,country_code:n("cb-country"),club_id:n("cb-club")||null,job:n("cb-job"),job2:n("cb-job2")||null,note_g:parseInt(n("cb-note-g"))||0,note_d:parseInt(n("cb-note-d"))||0,note_m:parseInt(n("cb-note-m"))||0,note_a:parseInt(n("cb-note-a"))||0,rarity:n("cb-rarity")||"normal",note_min:parseInt(n("cb-note-min"))||null,note_max:parseInt(n("cb-note-max"))||null,sell_price:parseInt(n("cb-price"))||0,avatar_config:D,skin:D.skin||"blanc",hair:D.hairColor||"marron",hair_length:"court"},{error:d}=await y.from("players").insert(c);if(d){e("Erreur: "+d.message,"error");return}e(`✅ Joueur "${c.firstname} ${c.surname_encoded}" enregistré !`,"success")}function Tt(e){const n=Bt.map(([o,r])=>`<option value="${o}">${r} (${o})</option>`).join(""),t='<option value="">— Sélectionner un club —</option>'+e.map(o=>`<option value="${o.id}" data-logo="${o.logo_url||""}">${o.encoded_name}</option>`).join(""),i=_e.map(o=>`<option value="${o}">${o}</option>`).join(""),c='<option value="">Aucun</option>'+_e.map(o=>`<option value="${o}">${o}</option>`).join(""),d=Lt.map(o=>`<option value="${o}">${o.charAt(0).toUpperCase()+o.slice(1)}</option>`).join(""),s=(o,r,l)=>Object.entries(r).map(([u,h])=>`<option value="${u}" ${u===(l||"")?"selected":""}>${h.label}</option>`).join(""),a=Object.keys(Xe).map(o=>`<option value="${o}" ${o===z.skin?"selected":""}>${o.charAt(0).toUpperCase()+o.slice(1)}</option>`).join(""),p=Object.keys(Qe).map(o=>`<option value="${o}" ${o===z.hairColor?"selected":""}>${o.charAt(0).toUpperCase()+o.slice(1)}</option>`).join("");return`
  ...
  `,d(c),document.getElementById("search-users").addEventListener("input",s=>{const a=s.target.value.toLowerCase();d(c.filter(p=>{var o;return p.pseudo.toLowerCase().includes(a)||((o=p.club_name)==null?void 0:o.toLowerCase().includes(a))}))});function d(s){document.getElementById("users-tbody").innerHTML=s.map(a=>{const p=a.mmr??1e3,o=a.mmr_deviation??350,r=a.placement_matches??0,l=lt(p),u=a.ranked_wins??0,h=a.ranked_losses??0,f=a.ranked_draws??0,E=u+h+f,w=E>0?Math.round(u/E*100):null,m=r<10,_=o<80?"#1A6B3C":o<150?"#e67e22":"#bb2020";return`
        <tr>
          <td>
          ...
          <td style="font-size:12px">
            ...
            ${m?`<div style="font-size:10px;color:#e67e22">${r}/10 placement${r<10?` (${10-r} restants)`:""}</div>`:`<div style="font-size:10px;color:var(--gray-600)">${u}V ${f}N ${h}D${w!==null?` · ${w}% WR`:""}</div>`}
          </td>
        ...
        </tr>`}).join(""),document.querySelectorAll("[data-toggle-admin]").forEach(a=>{a.addEventListener("click",async()=>{const p=a.dataset.isAdmin!=="true";if(!confirm(`Voulez-vous ${p?"rendre admin":"retirer le rôle admin"} de ce Manager ?`))return;const{error:o}=await y.from("users").update({is_admin:p}).eq("id",a.dataset.toggleAdmin);o?n(o.message,"error"):(n("Rôle mis à jour ✅","success"),Ve(e,{toast:n}))})})}}async function Ke(e,{toast:n}){const{data:t,error:i}=await y.from("market_listings").select(`
    ...
    `).order("listed_at",{ascending:!1}).limit(100);if(i){e.innerHTML=`<p style="color:red">${i.message}</p>`;return}const c=t||[],d=c.filter(a=>a.status==="active").length,s=c.filter(a=>a.status==="sold").length;e.innerHTML=`
    ...
    <div class="card-panel">
      <div class="table-wrap">
        <table>
          ...
          <tbody>
            ${c.map(a=>{var l,u,h,f;const p=(l=a.card)==null?void 0:l.player,o=p?`${p.firstname} ${p.surname_encoded}`:((u=a.card)==null?void 0:u.card_type)||"—",r={active:"#1A6B3C",sold:"#D4A017",cancelled:"#aaa"};return`<tr>
  ...
  `,document.querySelectorAll("[data-cancel]").forEach(a=>{a.addEventListener("click",async()=>{if(!confirm("Annuler cette annonce ?"))return;const{error:p}=await y.from("market_listings").update({status:"cancelled"}).eq("id",a.dataset.cancel);p?n(p.message,"error"):(n("Annonce annulée","success"),Ke(e,{toast:n}))})})}async function Mt(e,{toast:n}){e.innerHTML=`
...
`).replace(/\r/g,`
`);const n=e.split(`
`).filter(s=>s.trim());if(n.length<2)return[];const t=n[0],i=t.split(";").length>t.split(",").length?";":",",c=Le(t,i).map(s=>s.trim().replace(/^\uFEFF/,"").toLowerCase()),d=[];for(let s=1;s<n.length;s++){if(!n[s].trim())continue;const a=Le(n[s],i),p={};c.forEach((o,r)=>{p[o]=(a[r]||"").trim()}),!Object.values(p).every(o=>!o)&&d.push(p)}return d}function Le(e,n=","){const t=[];let i="",c=!1;for(let d=0;d<e.length;d++){const s=e[d];c?s==='"'?e[d+1]==='"'?(i+='"',d++):c=!1:i+=s:s==='"'?c=!0:s===n?(t.push(i),i=""):i+=s}return t.push(i),t}const jt={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};async function Rt(e,{toast:n}){const t=Object.keys(se);let i=t[0];const{data:c}=await y.from("formation_links_overrides").select("formation, links"),d={};(c||[]).forEach(r=>{d[r.formation]=r.links});let s=new Set;function a(r,l){return[r,l].sort().join("-")}function p(r){const l=d[r]||se[r]||[];s=new Set(l.map(([u,h])=>a(u,h)))}p(i),e.innerHTML=`
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;gap:12px;flex-wrap:wrap">
  ...
  `;function o(){const r=st[i]||{},l=he(i),u=320,h=400,f=22;function E(v){const g=r[v];return g?{x:g.x*u,y:g.y*h}:null}let w=`<svg width="${u}" height="${h}" viewBox="0 0 ${u} ${h}" xmlns="http://www.w3.org/2000/svg" style="display:block">`;l.forEach(([v,g],b)=>{const x=E(v),k=E(g);if(!x||!k)return;const L=a(v,g),I=s.has(L),C=I?"#3b82f6":"#999",M=I?.95:.35,j=I?4:3;w+=`<line x1="${x.x}" y1="${x.y}" x2="${k.x}" y2="${k.y}"
        ...
        opacity="${M}" pointer-events="none" data-line-key="${L}"/>`});for(const v of Object.keys(r)){const g=E(v);if(!g)continue;const b=v.replace(/\d+/,""),x=jt[b]||"#555";w+=`<circle cx="${g.x}" cy="${g.y}" r="${f}" fill="${x}" stroke="rgba(255,255,255,0.7)" stroke-width="2" pointer-events="none"/>`,w+=`<text x="${g.x}" y="${g.y+4}" text-anchor="middle" font-size="10" font-weight="900" fill="white" font-family="Arial Black,Arial" pointer-events="none">${v}</text>`}w+="</svg>",document.getElementById("field-wrap").innerHTML=w;const m=document.getElementById("links-list");m.innerHTML=l.map(([v,g])=>{const b=a(v,g),x=s.has(b);return`
        ...
        </button>`}).join("");const _=e.querySelector('div[style*="font-weight:700;font-size:13px;margin-bottom:8px"]');_&&(_.textContent=`Liens (${s.size}/${l.length} actifs)`);function $(v){s.has(v)?s.delete(v):s.add(v),o()}e.querySelectorAll(".link-hit").forEach(v=>{v.addEventListener("click",()=>$(v.dataset.key))}),e.querySelectorAll(".link-toggle").forEach(v=>{v.addEventListener("click",()=>$(v.dataset.key))})}o(),document.getElementById("formation-select").addEventListener("change",r=>{i=r.target.value,p(i),o()}),document.getElementById("reset-btn").addEventListener("click",()=>{const r=se[i]||[];s=new Set(r.map(([l,u])=>a(l,u))),o(),n("Liens réinitialisés aux valeurs par défaut (non sauvegardé)","info")}),document.getElementById("save-btn").addEventListener("click",async()=>{const l=he(i).filter(([h,f])=>s.has(a(h,f))),{error:u}=await y.from("formation_links_overrides").upsert({formation:i,links:l,updated_at:new Date().toISOString()});if(u){n(u.message,"error");return}d[i]=l,n(`Liens enregistrés pour ${i} (${l.length} actifs)`,"success")})}const Nt=[{value:"player",label:"Joueur"},{value:"formation",label:"Formation"},{value:"game_changer",label:"Game Changer"},{value:"ultra_game_changer",label:"Ultra Game Changer"},{value:"game_helper",label:"Game Helper"},{value:"stadium",label:"🏟️ Stade"}],Dt=[{value:"",label:"Toutes"},{value:"normal",label:"Normal"},{value:"pepite",label:"Pépite"},{value:"papyte",label:"Papyte"},{value:"legende",label:"Légende"}],Pt=[{value:"credits",label:"Crédits"},{value:"pub",label:"Publicité"},{value:"free",label:"Gratuit"}];async function qt(e){e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">Chargement...</div>';const{data:n}=await y.from("booster_configs").select("*").order("sort_order");let t=null,i=[],c=null;const d=()=>window.innerWidth<700;async function s(){if(!t){i=[];return}if(t!==c){const{data:l}=await y.from("booster_drop_rates").select("*").eq("booster_id",t).order("sort_order");i=l||[],c=t}}function a(){return`
    ...
    <div style="display:flex;height:calc(100vh - 56px);overflow:hidden">
      ${!d()||u||!t?a():""}
      ${!d()||h?p():""}
    </div>`,r()}function r(){var h,f,E,w,m,_,$;const l=v=>e.querySelector(v);e.querySelectorAll(".booster-row").forEach(v=>{v.addEventListener("click",g=>{g.target.closest(".btn-delete")||(t=v.dataset.id,c=null,o())})}),(h=l("#btn-back"))==null||h.addEventListener("click",()=>{t=null,o()}),(f=l("#btn-new"))==null||f.addEventListener("click",async()=>{const v=prompt("Nom du nouveau booster :");if(!(v!=null&&v.trim()))return;const{data:g,error:b}=await y.from("booster_configs").insert({name:v.trim(),price_type:"credits",price_credits:1e3,card_count:5}).select().single();if(b){alert(b.message);return}n.push(g),t=g.id,c=null,o()}),e.querySelectorAll(".btn-delete").forEach(v=>{v.addEventListener("click",async g=>{if(g.stopPropagation(),!confirm("Supprimer ce booster ?"))return;await y.from("booster_configs").delete().eq("id",v.dataset.id);const b=n.findIndex(x=>x.id===v.dataset.id);b>-1&&n.splice(b,1),t===v.dataset.id&&(t=null,c=null),o()})}),(E=l("#btn-cancel"))==null||E.addEventListener("click",()=>{t=null,o()}),(w=l("#f-price-type"))==null||w.addEventListener("change",v=>{const g=l("#credits-field");g&&(g.style.opacity=v.target.value!=="credits"?"0.4":"1")}),(m=l("#btn-pick-icon"))==null||m.addEventListener("click",async()=>{var g;const v=l("#icon-picker-grid");if(v){if(v.style.display!=="none"){v.style.display="none";return}v.innerHTML='<div style="padding:10px;text-align:center;color:#888;font-size:12px">⏳ Chargement...</div>',v.style.display="block";try{const x=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),k=Array.isArray(x)?x.filter(I=>I.name.startsWith("booster")&&/\.(png|jpg|svg|webp)$/i.test(I.name)):[];if(!k.length){v.innerHTML='<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Aucune icône "booster*" trouvée.</div>';return}const L=((g=l("#f-image-url"))==null?void 0:g.value)||"";v.innerHTML=`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(72px,1fr));gap:8px">
          ${k.map(I=>`
  ...
  <div style="background:#fff;border-radius:10px;padding:12px 16px;box-shadow:0 1px 4px rgba(0,0,0,0.08);display:flex;align-items:center;gap:12px">
  ...
  </div>`}function Ae(e,n=0){document.getElementById("ts-form").style.display="block",document.getElementById("ts-form-title").textContent=e?"Modifier l'étape":"Nouvelle étape",document.getElementById("ts-id").value=(e==null?void 0:e.id)||"",document.getElementById("ts-emoji").value=(e==null?void 0:e.emoji)||"⚽",document.getElementById("ts-title").value=(e==null?void 0:e.title)||"",document.getElementById("ts-content").value=(e==null?void 0:e.content)||"";const t=document.getElementById("ts-editor");t&&(t.innerHTML=(e==null?void 0:e.content)||"<p>Texte de l'étape…</p>"),document.getElementById("ts-order").value=(e==null?void 0:e.step_order)??n,document.getElementById("ts-active").checked=e?e.is_active:!0;const i=(e==null?void 0:e.color)||"#1A6B3C";document.getElementById("ts-color").value=i,document.getElementById("ts-color-hex").value=i;const c=(e==null?void 0:e.image_url)||"",d=document.getElementById("ts-image");d&&(d.value=c);const s=document.getElementById("ts-img-preview"),a=document.getElementById("ts-img-preview-el");if(c&&s&&a){const p="/";a.src=`${p}icons/${c}`,s.style.display="block"}else s&&(s.style.display="none");document.getElementById("ts-img-picker-grid").style.display="none",document.getElementById("ts-preview-area").innerHTML="",document.getElementById("ts-form").scrollIntoView({behavior:"smooth"})}function Ye(){document.getElementById("ts-form").style.display="none",document.getElementById("ts-preview-area").innerHTML=""}function Yt(){var o,r;const e=document.getElementById("ts-emoji").value||"⚽",n=document.getElementById("ts-title").value||"Titre",t=document.getElementById("ts-editor"),i=t?t.innerHTML:document.getElementById("ts-content").value||"",c=document.getElementById("ts-color-hex").value||"#1A6B3C",d=(r=(o=document.getElementById("ts-image"))==null?void 0:o.value)==null?void 0:r.trim(),a=d?`<div style="padding:0 20px 12px;text-align:center"><img src="/icons/${d}" style="max-height:140px;max-width:100%;border-radius:10px;object-fit:contain"></div>`:"",p=document.getElementById("ts-preview-area");p.innerHTML=`
    ...
    </div>`,p.querySelectorAll("ul,ol").forEach(l=>{l.style.paddingLeft="20px",l.style.margin="6px 0"}),p.querySelectorAll("li").forEach(l=>{l.style.marginBottom="4px"}),p.querySelectorAll("p").forEach(l=>{l.style.marginBottom="8px"})}async function Wt(e){var l,u;const n=document.getElementById("ts-id").value,t=document.getElementById("ts-emoji").value.trim()||"⚽",i=document.getElementById("ts-title").value.trim(),c=document.getElementById("ts-editor"),d=(c?c.innerHTML:document.getElementById("ts-content").value).trim(),s=document.getElementById("ts-color-hex").value.trim()||"#1A6B3C",a=parseInt(document.getElementById("ts-order").value)||0,p=document.getElementById("ts-active").checked;if(!i||!d){alert("Titre et contenu sont obligatoires.");return}const o={emoji:t,title:i,content:d,color:s,step_order:a,is_active:p,image_url:((u=(l=document.getElementById("ts-image"))==null?void 0:l.value)==null?void 0:u.trim())||null};let r;if(n?{error:r}=await y.from("tutorial_steps").update(o).eq("id",n):{error:r}=await y.from("tutorial_steps").insert(o),r){alert("Erreur : "+r.message);return}Ye(),H(e)}const Zt="/",We="#E87722",Xt=[["MA","Maroc"],["FR","France"],["AR","Argentine"],["PT","Portugal"],["BR","Brésil"],["ES","Espagne"],["DE","Allemagne"],["GB","Angleterre"],["IT","Italie"],["CM","Cameroun"],["SN","Sénégal"],["NG","Nigéria"],["DK","Danemark"],["NL","Pays-Bas"],["BE","Belgique"],["CI","Côte d'Ivoire"],["AL","Albanie"],["HR","Croatie"],["RS","Serbie"],["TR","Turquie"],["MW","Malawi"],["GH","Ghana"],["ZA","Afrique du Sud"],["EG","Égypte"],["US","États-Unis"],["MX","Mexique"],["CO","Colombie"],["UY","Uruguay"],["CL","Chili"],["JP","Japon"],["KR","Corée du Sud"],["CN","Chine"],["AU","Australie"],["SW","Suisse"],["SE","Suède"],["NO","Norvège"],["PL","Pologne"]];async function Qt(e){await ge(e)}async function ge(e){var i,c,d,s;const[{data:n},{data:t}]=await Promise.all([y.from("stadium_definitions").select("*, club:clubs(encoded_name, logo_url)").order("name"),y.from("clubs").select("id, encoded_name, logo_url").order("encoded_name")]);e.innerHTML=`
    <div style="padding:20px;max-width:820px;margin:0 auto">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px">
          ...
          <div style="flex:1;min-width:220px;display:flex;flex-direction:column;gap:12px">
            ...
            <div>
              ...
              <select id="st-country" style="width:100%;box-sizing:border-box;padding:9px;border:1.5px solid #ddd;border-radius:8px;font-size:13px;background:#fff">
                ...
                ${Xt.map(([a,p])=>`<option value="${a}">${p} (${a})</option>`).join("")}
              </select>
            </div>
            <div>
    ...
    </div>`,(i=document.getElementById("st-add-btn"))==null||i.addEventListener("click",()=>Ce(null)),(c=document.getElementById("st-cancel"))==null||c.addEventListener("click",()=>{document.getElementById("st-form").style.display="none"}),(d=document.getElementById("st-save"))==null||d.addEventListener("click",()=>nn(e)),(s=document.getElementById("st-preview-btn"))==null||s.addEventListener("click",()=>oe()),["st-name","st-club","st-country","st-image"].forEach(a=>{var p,o;(p=document.getElementById(a))==null||p.addEventListener("input",oe),(o=document.getElementById(a))==null||o.addEventListener("change",oe)}),e.querySelectorAll("[data-edit]").forEach(a=>{const p=(n||[]).find(o=>o.id===a.dataset.edit);p&&a.addEventListener("click",()=>Ce(p))}),e.querySelectorAll("[data-delete]").forEach(a=>{a.addEventListener("click",async()=>{confirm("Supprimer ce stade ?")&&(await y.from("stadium_definitions").delete().eq("id",a.dataset.delete),ge(e))})})}function en(e){var t,i;const n=(t=e.club)!=null&&t.logo_url?`<img src="${e.club.logo_url}" style="width:32px;height:32px;object-fit:contain">`:'<span style="font-size:20px">🌍</span>';return`
    <div style="background:#fff;border-radius:10px;padding:12px 16px;box-shadow:0 1px 4px rgba(0,0,0,0.08);display:flex;align-items:center;gap:12px">
    ...
    <div style="display:flex;height:calc(100vh - 56px);overflow:hidden">
      ${!d()||!t?p():""}
      ${!d()||t?o():""}
    </div>`,l()}function l(){var u,h,f,E,w;e.querySelectorAll(".gc-row").forEach(m=>{m.addEventListener("click",_=>{_.target.closest(".btn-toggle-gc,.btn-delete-gc")||(t=m.dataset.id,r())})}),(u=s("#btn-gc-back"))==null||u.addEventListener("click",()=>{t=null,r()}),(h=s("#btn-cancel-gc"))==null||h.addEventListener("click",()=>{t=null,r()}),(f=s("#btn-new-gc"))==null||f.addEventListener("click",async()=>{const m=prompt("Nom de la carte Game Changer :");if(!(m!=null&&m.trim()))return;const{data:_,error:$}=await y.from("gc_definitions").insert({name:m.trim(),gc_type:"game_changer",color:"purple"}).select().single();if($){alert($.message);return}n.push(_),t=_.id,r()}),e.querySelectorAll(".btn-toggle-gc").forEach(m=>{m.addEventListener("click",async _=>{_.stopPropagation();const $=m.dataset.active!=="true";await y.from("gc_definitions").update({is_active:$}).eq("id",m.dataset.id);const v=n.find(g=>g.id===m.dataset.id);v&&(v.is_active=$),r()})}),e.querySelectorAll(".btn-delete-gc").forEach(m=>{m.addEventListener("click",async _=>{if(_.stopPropagation(),!confirm("Supprimer cette carte Game Changer ?"))return;await y.from("gc_definitions").delete().eq("id",m.dataset.id);const $=n.findIndex(v=>v.id===m.dataset.id);$>-1&&n.splice($,1),t===m.dataset.id&&(t=null),r()})}),(E=s("#btn-pick-gc-icon"))==null||E.addEventListener("click",async()=>{var _;const m=s("#gc-icon-picker");if(m){if(m.style.display!=="none"){m.style.display="none";return}m.innerHTML='<div style="padding:10px;text-align:center;color:#888;font-size:12px">⏳ Chargement...</div>',m.style.display="block";try{const v=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),g=Array.isArray(v)?v.filter(x=>x.name.startsWith("gamechanger-")&&/\.(png|jpg|svg|webp)$/i.test(x.name)):[];if(!g.length){m.innerHTML='<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Aucun fichier "gamechanger-*" trouvé dans public/icons/</div>';return}const b=((_=s("#gc-image-url"))==null?void 0:_.value)||"";m.innerHTML=`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(72px,1fr));gap:8px">
          ${g.map(x=>`
