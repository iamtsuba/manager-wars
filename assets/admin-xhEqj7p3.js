import{s as _,r as ve,a as qe,A as st,l as dt,g as ct,K as ke,D as R,b as ut,S as pt,c as D,H as mt,R as gt,e as de,d as Ke,f as ft,h as bt,E as yt,i as vt,N as xt,M as ht,B as Et,j as _t,F as ue,k as Be,m as wt,n as Je,o as me,p as It}from"./special-cards-hvDhyFrj.js";async function $t(e){var h;const[{count:t},{count:n},{count:i},{count:r},{count:a},{data:m}]=await Promise.all([_.from("players").select("*",{count:"exact",head:!0}),_.from("clubs").select("*",{count:"exact",head:!0}),_.from("users").select("*",{count:"exact",head:!0}),_.from("stadium_definitions").select("*",{count:"exact",head:!0}),_.from("users").select("*",{count:"exact",head:!0}).gt("last_seen_at",new Date(Date.now()-18e4).toISOString()),_.rpc("get_signup_password")]),g=m||"";e.innerHTML=`
    ...
    <div class="card-panel">
      ...
      <div style="display:flex;gap:8px;max-width:420px">
        <input id="signup-pwd-input" type="text" value="${g.replace(/"/g,"&quot;")}" style="flex:1">
  ...
  `,window.adminNav=o=>{var u;(u=document.querySelector(`[data-page="${o}"]`))==null||u.click()},(h=document.getElementById("signup-pwd-save"))==null||h.addEventListener("click",async()=>{const o=document.getElementById("signup-pwd-input").value.trim(),u=document.getElementById("signup-pwd-status");if(!o){u.textContent="Le code ne peut pas être vide.",u.style.color="#ff6b6b";return}const l=document.getElementById("signup-pwd-save");l.disabled=!0,l.textContent="⏳...";const{error:b}=await _.rpc("set_signup_password",{new_password:o});if(l.disabled=!1,l.textContent="💾 Enregistrer",b){u.textContent=b.message,u.style.color="#ff6b6b";return}u.textContent="✅ Code mis à jour.",u.style.color="#2ecc71"})}const Ve={normal:"Normal",pepite:"Pépite",papyte:"Papyte",legende:"Légende"},kt={GK:"#aaaaaa",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};async function Bt(){const e={};for(const t of st)e[t]=await dt(t);return e}async function Lt(e,t){const{toast:n}=t;if(!confirm(`Réattribuer une nouvelle photo à tous les joueurs ayant encore l'ancien format de chemin (public/faces/...) ?

Cette action est irréversible.`))return;const i=document.getElementById("fix-old-faces-btn");i&&(i.disabled=!0,i.textContent="⏳ En cours...");try{const{data:r,error:a}=await _.from("players").select("id, country_code, face").like("face","public/faces/%");if(a){n(a.message,"error");return}if(!(r!=null&&r.length)){n("Aucun joueur avec l'ancien format — rien à faire ✅","success");return}const{data:m}=await _.from("players").select("face").not("face","is",null),g=new Set((m||[]).map(u=>u.face).filter(u=>u&&!u.startsWith("public/faces/")));let h=0,o=0;for(const u of r){const l=await qe(u.country_code,g);if(!l){o++;continue}const{error:b}=await _.from("players").update({face:l}).eq("id",u.id);if(b){o++;continue}g.add(l),h++}n(`${h} photo(s) réattribuée(s)${o?`, ${o} échec(s)`:""} ✅`,o?"error":"success")}finally{i&&(i.disabled=!1,i.textContent="🖼️ Réattribuer anciennes photos")}}async function At(e,t){await le(e,t)}async function le(e,t,n=null){var o,u,l,b,p;const{toast:i}=t;n||(n={search:((o=document.getElementById("search-players"))==null?void 0:o.value)||"",job:((u=document.getElementById("filter-job"))==null?void 0:u.value)||"",rarity:((l=document.getElementById("filter-rarity"))==null?void 0:l.value)||"",club:((b=document.getElementById("filter-club"))==null?void 0:b.value)||"",country:((p=document.getElementById("filter-country"))==null?void 0:p.value)||""});const[{data:r,error:a},{data:m}]=await Promise.all([_.from("players").select("*, clubs(id,encoded_name,logo_url)"),_.from("clubs").select("id,encoded_name").order("encoded_name")]);if(a){e.innerHTML=`<p style="color:red">${a.message}</p>`;return}const g={GK:0,DEF:1,MIL:2,ATT:3},h=(r||[]).sort((s,E)=>{const f=(g[s.job]??4)-(g[E.job]??4);return f!==0?f:(s.surname_real||"").localeCompare(E.surname_real||"")});St(e,h,m||[],t,n)}function St(e,t,n,i,r=null){var u,l;const{toast:a}=i;if(e.innerHTML=`
    <div style="display:flex;flex-direction:column;gap:10px">
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        ...
        <select id="filter-rarity" style="width:110px">
          ...
          ${Object.entries(Ve).map(([b,p])=>`<option value="${b}">${p}</option>`).join("")}
        </select>
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        <select id="filter-club" style="flex:1;min-width:140px">
          ...
          ${n.map(b=>`<option value="${b.id}">${b.encoded_name}</option>`).join("")}
        </select>
        <select id="filter-country" style="flex:1;min-width:120px">
          <option value="">Tous les pays</option>
          ${[...new Set(t.map(b=>b.country_code).filter(Boolean))].sort().map(b=>`<option value="${b}">${b}</option>`).join("")}
        </select>
      </div>
    ...
    </div>`,r){const b=r,p=s=>document.getElementById(s);b.search&&p("search-players")&&(p("search-players").value=b.search),b.job&&p("filter-job")&&(p("filter-job").value=b.job),b.rarity&&p("filter-rarity")&&(p("filter-rarity").value=b.rarity),b.club&&p("filter-club")&&(p("filter-club").value=b.club),b.country&&p("filter-country")&&(p("filter-country").value=b.country)}if(r){const b=r,p=s=>document.getElementById(s);b.search&&p("search-players")&&(p("search-players").value=b.search),b.job&&p("filter-job")&&(p("filter-job").value=b.job),b.rarity&&p("filter-rarity")&&(p("filter-rarity").value=b.rarity),b.club&&p("filter-club")&&(p("filter-club").value=b.club),b.country&&p("filter-country")&&(p("filter-country").value=b.country)}function m(){const b=document.getElementById("search-players").value.toLowerCase(),p=document.getElementById("filter-job").value,s=document.getElementById("filter-rarity").value,E=document.getElementById("filter-club").value,f=document.getElementById("filter-country").value;return t.filter(v=>(!b||`${v.firstname} ${v.surname_real}`.toLowerCase().includes(b))&&(!p||v.job===p)&&(!s||v.rarity===s)&&(!E||v.club_id===E)&&(!f||v.country_code===f))}const g=new Set;function h(){const b=document.getElementById("bulk-bar"),p=document.getElementById("bulk-count");b&&(b.style.display=g.size>0?"flex":"none",p&&(p.textContent=`${g.size} joueur(s) sélectionné(s)`))}function o(){const b=m();document.getElementById("count-label").textContent=`${b.length} joueur(s)`;const p=document.getElementById("players-list");if(!b.length){p.innerHTML='<div style="color:var(--gray-600);padding:20px;text-align:center">Aucun joueur.</div>';return}p.innerHTML=b.map(s=>{const E={...s,clubs:s.clubs,face:s.face||null},f=ve(E,{width:120});return`<div style="position:relative;cursor:pointer" data-edit="${s.id}">
        ${f}
      ...
      </div>`}).join(""),p.querySelectorAll("[data-edit]").forEach(s=>{s.addEventListener("click",()=>{const E=t.find(f=>f.id===s.dataset.edit);E&&Le(E,n,e,i)})}),p.querySelectorAll(".btn-del-player").forEach(s=>{s.addEventListener("click",async E=>{if(E.stopPropagation(),!confirm("Supprimer ce joueur ?"))return;const{error:f}=await _.from("players").delete().eq("id",s.dataset.del);f?a(f.message,"error"):(a("Joueur supprimé ✅","success"),le(e,i))})})}o(),document.getElementById("search-players").addEventListener("input",o),document.getElementById("filter-job").addEventListener("change",o),document.getElementById("filter-rarity").addEventListener("change",o),document.getElementById("filter-club").addEventListener("change",o),document.getElementById("filter-country").addEventListener("change",o),(u=document.getElementById("bulk-cancel-btn"))==null||u.addEventListener("click",()=>{g.clear(),h(),o()}),(l=document.getElementById("bulk-delete-btn"))==null||l.addEventListener("click",async()=>{var E,f,v,$,I;if(!g.size||!confirm(`Supprimer ${g.size} joueur(s) ?`))return;const b=[...g],{error:p}=await _.from("players").delete().in("id",b);if(p){a(p.message,"error");return}a(`${b.length} joueur(s) supprimé(s) ✅`,"success"),g.clear();const s={search:((E=document.getElementById("search-players"))==null?void 0:E.value)||"",job:((f=document.getElementById("filter-job"))==null?void 0:f.value)||"",rarity:((v=document.getElementById("filter-rarity"))==null?void 0:v.value)||"",club:(($=document.getElementById("filter-club"))==null?void 0:$.value)||"",country:((I=document.getElementById("filter-country"))==null?void 0:I.value)||""};le(e,i,s)}),document.getElementById("add-player-btn").addEventListener("click",()=>Le(null,n,e,i)),document.getElementById("fix-old-faces-btn").addEventListener("click",()=>Lt(e,i))}async function Le(e,t,n,i){const{toast:r,openModal:a,closeModal:m}=i,g=!!e,h=await Bt(),{data:o}=await _.from("players").select("face").not("face","is",null),u=new Set((o||[]).map(s=>s.face).filter(Boolean));e!=null&&e.face&&u.delete(e.face);const l='<option value="">— Club —</option>'+t.map(s=>`<option value="${s.id}" ${(e==null?void 0:e.club_id)===s.id?"selected":""}>${s.encoded_name}</option>`).join(""),b=e!=null&&e.face?e.face.split("/")[0]:"",p=Object.keys(h).map(s=>`<option value="${s}" ${b===s?"selected":""}>${s}</option>`).join("");a(g?`✏️ ${e.firstname} ${e.surname_real}`:"➕ Nouveau joueur",`<div style="display:flex;gap:20px;align-items:flex-start">
      ...
      <div style="flex:1;min-width:300px;display:flex;flex-direction:column;gap:12px">
        ...
        <div style="border-top:1px solid var(--gray-200);padding-top:10px">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:8px">
            ${[["GK","pm-g","note_g"],["DEF","pm-d","note_d"],["MIL","pm-m","note_m"],["ATT","pm-a","note_a"]].map(([s,E,f])=>`
      ...
      <div id="club-panel" class="card-panel" style="flex:1;overflow-y:auto;display:flex;align-items:center;justify-content:center">
    ...
    </div>`,m(ae),document.getElementById("search-clubs").addEventListener("input",g=>{const h=g.target.value.toLowerCase();m(ae.filter(o=>o.real_name.toLowerCase().includes(h)||o.encoded_name.toLowerCase().includes(h)))}),document.getElementById("add-club-btn").addEventListener("click",()=>Se(null,e,t));function m(g){const h=document.getElementById("clubs-list");if(!g.length){h.innerHTML='<p style="color:var(--tile-fg-dim);padding:20px;text-align:center">Aucun club.</p>';return}h.innerHTML=g.map(o=>{const u=Xe(o),l=o.logo_url?`<img src="${o.logo_url}" style="width:36px;height:36px;object-fit:contain;border-radius:8px;flex-shrink:0">`:`<div style="width:36px;height:36px;background:linear-gradient(135deg,${u.color1},${u.color2});border-radius:8px;display:flex;align-items:center;justify-content:center;color:#fff;font-size:9px;font-weight:900;flex-shrink:0">${o.encoded_name.slice(0,3)}</div>`,b=n[o.id]||0;return`
        <div class="club-row" data-club-id="${o.id}"
          ...
          <div style="flex:1;min-width:0">
            ...
            <div style="font-size:10px;color:${b===0?"#e67e22":"var(--tile-fg-dim)"}">${b===0?"⚠️ 0 joueur":`👥 ${b}`}</div>
          </div>
  ...
  `,(f=document.getElementById("m-logo-file"))==null||f.addEventListener("change",d=>{var w;const c=(w=d.target.files)==null?void 0:w[0];if(!c)return;const y=new FileReader;y.onload=()=>{document.getElementById("logo-preview").innerHTML=`<img src="${y.result}" style="width:100%;height:100%;object-fit:contain">`},y.readAsDataURL(c)}),ee();function o(){var w,B;const d=((w=document.getElementById("m-kit-style"))==null?void 0:w.value)||"uni",c=((B=ke[d])==null?void 0:B.colors)===3,y=document.getElementById("wrap-m-kit-color3");y&&(y.style.display=c?"":"none")}o(),["m-kit-color1","m-kit-color2","m-kit-color3","m-kit-shorts","m-kit-socks"].forEach(d=>{const c=document.getElementById(d),y=document.getElementById(d+"-txt");!c||!y||(c.addEventListener("input",()=>{y.value=c.value,ee()}),y.addEventListener("input",()=>{const w=y.value.trim();/^#[0-9a-fA-F]{6}$/.test(w)&&(c.value=w,ee())}),y.addEventListener("change",()=>{let w=y.value.trim();w.startsWith("#")||(w="#"+w),/^#[0-9a-fA-F]{6}$/.test(w)&&(c.value=w,y.value=w,ee())}))}),(v=document.getElementById("m-kit-style"))==null||v.addEventListener("change",()=>{o(),ee()});const u=document.getElementById("m-real"),l=document.getElementById("m-encoded");function b(){if(!u||!l||l.value)return;const d=u.value.trim().split(/\s+/),c=d.length===1?d[0].toUpperCase().slice(0,6):d.filter(y=>y.length>2).map(y=>y[0].toUpperCase()).join("")||d[0].toUpperCase().slice(0,4);l.value=c}u==null||u.addEventListener("input",b),($=document.getElementById("auto-encode-btn"))==null||$.addEventListener("click",()=>{l&&(l.value=""),b()}),(I=document.getElementById("m-save"))==null||I.addEventListener("click",()=>Vt(e,r,t,n));const p=document.getElementById("m-gen-squad"),s=document.getElementById("m-gen-strong-label");function E(){if(s&&(s.style.display=p!=null&&p.checked?"flex":"none",!(p!=null&&p.checked))){const d=document.getElementById("m-gen-strong");d&&(d.checked=!1)}}p==null||p.addEventListener("change",E),E(),(x=document.getElementById("btn-gen-squad-panel"))==null||x.addEventListener("click",()=>{Kt(e,n,t)}),r&&Qe(e.id)}async function Qe(e){const{data:t}=await _.from("players").select("*, clubs(encoded_name, logo_url, kit_style, kit_color1, kit_color2, kit_color3, kit_shorts, kit_socks)").eq("club_id",e).order("surname_real"),n=document.getElementById("club-players-grid"),i=document.getElementById("club-players-count");if(i&&(i.textContent=(t||[]).length),!!n){if(!(t!=null&&t.length)){n.innerHTML='<div style="color:var(--tile-fg-dim);padding:20px;font-size:13px">Aucun joueur dans ce club.</div>';return}n.innerHTML=t.map(r=>ve(r,{width:100})).join("")}}function Kt(e,t,n){const{openModal:i,closeModal:r,toast:a}=t;i(`⚽ Générer — ${e.real_name}`,`<div style="display:flex;flex-direction:column;gap:14px">
    ...
    </div>`),setTimeout(()=>{var m,g;(m=document.getElementById("qg-cancel"))==null||m.addEventListener("click",()=>r()),(g=document.getElementById("qg-ok"))==null||g.addEventListener("click",async()=>{var o;const h=((o=document.getElementById("qg-strong"))==null?void 0:o.checked)??!1;r(),a("Génération en cours…","info"),await Ze(e.id,e.country_code,a,h),a("Effectif généré ✅","success"),Qe(e.id),ce(n,t)})},50)}function et(){var e,t,n,i,r,a;return{style:((e=document.getElementById("m-kit-style"))==null?void 0:e.value)||"uni",color1:((t=document.getElementById("m-kit-color1"))==null?void 0:t.value)||"#1A6B3C",color2:((n=document.getElementById("m-kit-color2"))==null?void 0:n.value)||"#ffffff",color3:((i=document.getElementById("m-kit-color3"))==null?void 0:i.value)||"#000000",shorts:((r=document.getElementById("m-kit-shorts"))==null?void 0:r.value)||"#111111",socks:((a=document.getElementById("m-kit-socks"))==null?void 0:a.value)||"#ffffff"}}function ee(){const e=document.getElementById("kit-preview-wrap");e&&(e.innerHTML=ut(et(),"panel"))}async function Jt(e,t){const n=(e.name.split(".").pop()||"png").toLowerCase(),i=`clubs/${t||"new"}-${Date.now()}.${n}`,{error:r}=await _.storage.from("assets").upload(i,e,{upsert:!0,cacheControl:"3600"});if(r)throw r;const{data:a}=_.storage.from("assets").getPublicUrl(i);return a.publicUrl}async function Vt(e,t,n,i){var I,x,d,c,y,w,B,k,A;const{toast:r}=i,a=document.getElementById("m-error"),m=document.getElementById("m-save"),g=(I=document.getElementById("m-real"))==null?void 0:I.value.trim(),h=(x=document.getElementById("m-encoded"))==null?void 0:x.value.trim().toUpperCase(),o=(d=document.getElementById("m-country"))==null?void 0:d.value.trim().toUpperCase(),u=((y=(c=document.getElementById("m-logo-file"))==null?void 0:c.files)==null?void 0:y[0])||null,l=((w=document.getElementById("m-logo-url-current"))==null?void 0:w.value)||null,b=((B=document.getElementById("m-gen-stadium"))==null?void 0:B.checked)??!1,p=((k=document.getElementById("m-gen-squad"))==null?void 0:k.checked)??!1,s=((A=document.getElementById("m-gen-strong"))==null?void 0:A.checked)??!1,E=et();if(!g){a.textContent="Le nom du club est requis.";return}if(!h){a.textContent="Le nom encodé est requis.";return}if(!o){a.textContent="Le pays est requis.";return}m.disabled=!0,m.textContent="Enregistrement…";let f=l;if(u)try{m.textContent="📤 Envoi du logo…",f=await Jt(u,e==null?void 0:e.id)}catch(L){a.textContent="Erreur upload logo : "+L.message,m.disabled=!1,m.textContent=t?"💾 Enregistrer":"✅ Créer le club";return}const v={real_name:g,encoded_name:h,country_code:o,logo_url:f,kit_style:E.style,kit_color1:E.color1,kit_color2:E.color2,kit_color3:E.color3,kit_shorts:E.shorts,kit_socks:E.socks};let $=e==null?void 0:e.id;if(t){const{error:L}=await _.from("clubs").update(v).eq("id",$);if(L){a.textContent=L.message,m.disabled=!1,m.textContent="💾 Enregistrer";return}}else{const{data:L,error:S}=await _.from("clubs").insert(v).select().single();if(S){a.textContent=S.message,m.disabled=!1,m.textContent="✅ Créer le club";return}if($=L.id,b){m.textContent="🏟️ Création du stade…";const{data:C,error:T}=await _.from("stadium_definitions").insert({name:`Stade de ${g}`,club_id:$,country_code:o}).select().single();T?console.warn("[Stadium] Erreur def stade:",T.message):C&&await _.from("cards").insert({card_type:"stadium",stadium_id:C.id})}p&&(m.textContent="⚽ Génération des joueurs…",await Ze($,o,r,s))}r(t?"Club modifié ✅":"Club créé ✅","success"),ce(n,i)}const Yt=["normal","pepite","papyte","legende"],Ce=["GK","DEF","MIL","ATT"],Wt=[["MA","Maroc"],["FR","France"],["AR","Argentine"],["PT","Portugal"],["BR","Brésil"],["ES","Espagne"],["DE","Allemagne"],["GB","Angleterre"],["IT","Italie"],["CM","Cameroun"],["SN","Sénégal"],["NG","Nigéria"],["DK","Danemark"],["NL","Pays-Bas"],["BE","Belgique"],["CI","Côte d'Ivoire"],["AL","Albanie"],["HR","Croatie"],["RS","Serbie"],["TR","Turquie"],["MW","Malawi"]];let q={...D},re=[];async function Zt(e,{toast:t}){const{data:n}=await _.from("clubs").select("id,encoded_name,logo_url,kit_style,kit_color1,kit_color2,kit_shorts,kit_socks").order("encoded_name");re=n||[],q={...D},e.innerHTML=tn(re),Qt(e,re,t),O()}function Te(){var n;const e=((n=document.getElementById("cb-surname-real"))==null?void 0:n.value)||"",t=document.getElementById("cb-surname-enc");t&&(t.value=de(e))}function Xt(){var n;const e=(n=document.getElementById("cb-club"))==null?void 0:n.value,t=re.find(i=>String(i.id)===String(e));return t?{style:t.kit_style||R.style,color1:t.kit_color1||R.color1,color2:t.kit_color2||R.color2,shorts:t.kit_shorts||R.shorts,socks:t.kit_socks||R.socks}:{...R}}function O(){var b,p,s,E;const e=f=>{var v;return((v=document.getElementById(f))==null?void 0:v.value)||""},t={firstname:e("cb-firstname"),surname_real:e("cb-surname-enc")||de(e("cb-surname-real")),country_code:e("cb-country"),club_encoded_name:((p=(b=document.getElementById("cb-club"))==null?void 0:b.selectedOptions[0])==null?void 0:p.text)||"",job:e("cb-job"),job2:e("cb-job2")||null,note_g:parseInt(e("cb-note-g"))||0,note_d:parseInt(e("cb-note-d"))||0,note_m:parseInt(e("cb-note-m"))||0,note_a:parseInt(e("cb-note-a"))||0,rarity:e("cb-rarity")},n=Xt(),i=document.getElementById("cb-club"),r=((E=(s=i==null?void 0:i.selectedOptions[0])==null?void 0:s.dataset)==null?void 0:E.logo)||null,a=Ke(q,n,120),m=`data:image/svg+xml;charset=utf-8,${encodeURIComponent(a)}`,g=ft(t,{portraitUrl:m,clubLogoUrl:r,showNotes:!0}),h=document.getElementById("card-preview");h&&(h.innerHTML=g);const o=document.getElementById("avatar-preview-wrap");o&&(o.innerHTML=a);const u=e("cb-surname-real"),l=document.getElementById("encode-summary");l&&u&&(l.textContent=`${e("cb-firstname")} ${u} → ${e("cb-firstname")} ${t.surname_real}`)}function tt(){const e=t=>{var n;return((n=document.getElementById(t))==null?void 0:n.value)||""};q={skin:e("av-skin"),hair:e("av-hair"),hairColor:e("av-hairColor"),eyes:e("av-eyes"),nose:e("av-nose"),mouth:e("av-mouth"),eyebrows:e("av-eyebrows"),beard:e("av-beard")}}function Qt(e,t,n){var r,a,m,g,h;e.querySelectorAll("input,select").forEach(o=>{o.addEventListener("input",O),o.addEventListener("change",O)});const i=["av-skin","av-hair","av-hairColor","av-eyes","av-nose","av-mouth","av-eyebrows","av-beard"];i.forEach(o=>{var u;(u=document.getElementById(o))==null||u.addEventListener("change",()=>{tt(),O()})}),(r=document.getElementById("cb-surname-real"))==null||r.addEventListener("input",()=>{Te(),O()}),(a=document.getElementById("btn-encode"))==null||a.addEventListener("click",()=>{Te(),O()}),(m=document.getElementById("cb-club"))==null||m.addEventListener("change",O),(g=document.getElementById("btn-save-player"))==null||g.addEventListener("click",()=>en(n)),(h=document.getElementById("btn-reset"))==null||h.addEventListener("click",()=>{e.querySelectorAll("input").forEach(o=>o.value=""),e.querySelectorAll("select").forEach(o=>o.selectedIndex=0),q={...D},i.forEach(o=>{const u=o.replace("av-",""),l=document.getElementById(o);l&&(l.value=D[u]||"")}),O()})}async function en(e){const t=m=>{var g;return((g=document.getElementById(m))==null?void 0:g.value)||""},n=t("cb-surname-real").trim(),i=t("cb-surname-enc").trim()||de(n);if(!t("cb-firstname")||!n||!t("cb-country")||!t("cb-job")){e("Remplissez les champs obligatoires (prénom, nom, pays, poste)","error");return}tt();const r={firstname:t("cb-firstname").trim(),surname_real:n,surname_real:i,country_code:t("cb-country"),club_id:t("cb-club")||null,job:t("cb-job"),job2:t("cb-job2")||null,note_g:parseInt(t("cb-note-g"))||0,note_d:parseInt(t("cb-note-d"))||0,note_m:parseInt(t("cb-note-m"))||0,note_a:parseInt(t("cb-note-a"))||0,rarity:t("cb-rarity")||"normal",note_min:parseInt(t("cb-note-min"))||null,note_max:parseInt(t("cb-note-max"))||null,sell_price:parseInt(t("cb-price"))||0,avatar_config:q,skin:q.skin||"blanc",hair:q.hairColor||"marron",hair_length:"court"},{error:a}=await _.from("players").insert(r);if(a){e("Erreur: "+a.message,"error");return}e(`✅ Joueur "${r.firstname} ${r.surname_real}" enregistré !`,"success")}function tn(e){const t=Wt.map(([o,u])=>`<option value="${o}">${u} (${o})</option>`).join(""),n='<option value="">— Sélectionner un club —</option>'+e.map(o=>`<option value="${o.id}" data-logo="${o.logo_url||""}">${o.encoded_name}</option>`).join(""),i=Ce.map(o=>`<option value="${o}">${o}</option>`).join(""),r='<option value="">Aucun</option>'+Ce.map(o=>`<option value="${o}">${o}</option>`).join(""),a=Yt.map(o=>`<option value="${o}">${o.charAt(0).toUpperCase()+o.slice(1)}</option>`).join(""),m=(o,u,l)=>Object.entries(u).map(([b,p])=>`<option value="${b}" ${b===(l||"")?"selected":""}>${p.label}</option>`).join(""),g=Object.keys(pt).map(o=>`<option value="${o}" ${o===D.skin?"selected":""}>${o.charAt(0).toUpperCase()+o.slice(1)}</option>`).join(""),h=Object.keys(mt).map(o=>`<option value="${o}" ${o===D.hairColor?"selected":""}>${o.charAt(0).toUpperCase()+o.slice(1)}</option>`).join("");return`
        ...
        <div style="display:grid;grid-template-columns:1fr 120px;gap:16px;align-items:start">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
            ...
            <div>
              ...
              <select id="av-skin">${g}</select>
            </div>
            <div>
              <label>Style de cheveux</label>
              <select id="av-hair">${m("av-hair",bt,D.hair)}</select>
            </div>
            <div>
            ...
            <div>
              ...
              <select id="av-eyebrows">${m("av-eyebrows",yt,D.eyebrows)}</select>
            </div>
            <div>
              <label>Yeux</label>
              <select id="av-eyes">${m("av-eyes",vt,D.eyes)}</select>
            </div>
            <div>
              <label>Nez</label>
              <select id="av-nose">${m("av-nose",xt,D.nose)}</select>
            </div>
            <div>
              <label>Bouche</label>
              <select id="av-mouth">${m("av-mouth",ht,D.mouth)}</select>
            </div>
            <div>
              <label>Barbe</label>
              <select id="av-beard">${m("av-beard",Et,D.beard)}</select>
            </div>
          </div>
  ...
  `,a(r),document.getElementById("search-users").addEventListener("input",m=>{const g=m.target.value.toLowerCase();a(r.filter(h=>{var o;return h.pseudo.toLowerCase().includes(g)||((o=h.club_name)==null?void 0:o.toLowerCase().includes(g))}))});function a(m){document.getElementById("users-tbody").innerHTML=m.map(g=>{const h=g.mmr??1e3,o=g.mmr_deviation??350,u=g.placement_matches??0,l=_t(h),b=g.ranked_wins??0,p=g.ranked_losses??0,s=g.ranked_draws??0,E=b+p+s,f=E>0?Math.round(b/E*100):null,v=u<10,$=o<80?"#1A6B3C":o<150?"#e67e22":"#bb2020";return`
        <tr>
          <td>
          ...
          <td style="font-size:12px">
            <div>${(g.credits||0).toLocaleString("fr")} cr.</div>
            <div style="color:var(--gray-600)">Niv. ${g.level}</div>
          </td>
          <td style="font-size:12px">${g.wins}V / ${g.draws}N / ${g.losses}D</td>
          <td style="font-size:12px">🥇${g.trophies_top1} 🥈${g.trophies_top2} 🥉${g.trophies_top3}</td>
          <td style="font-size:12px">
            <div style="display:flex;align-items:center;gap:5px">
              <span style="font-size:16px">${l.emoji}</span>
              <span style="font-weight:700;color:${l.color}">${l.label}</span>
            </div>
            ${v?`<div style="font-size:10px;color:#e67e22">${u}/10 placement${u<10?` (${10-u} restants)`:""}</div>`:`<div style="font-size:10px;color:var(--gray-600)">${b}V ${s}N ${p}D${f!==null?` · ${f}% WR`:""}</div>`}
          </td>
          ...
          <td>
            ${g.is_admin?'<span style="color:#1A6B3C;font-weight:700;font-size:12px">✅ Admin</span>':'<span style="color:#aaa;font-size:12px">Manager</span>'}
          </td>
          <td>
            <button class="btn btn-ghost btn-sm" data-toggle-admin="${g.id}" data-is-admin="${g.is_admin}">
              ${g.is_admin?"🔒 Retirer":"🔓 Admin"}
            </button>
          </td>
        </tr>`}).join(""),document.querySelectorAll("[data-toggle-admin]").forEach(g=>{g.addEventListener("click",async()=>{const h=g.dataset.isAdmin!=="true";if(!confirm(`Voulez-vous ${h?"rendre admin":"retirer le rôle admin"} de ce Manager ?`))return;const{error:o}=await _.from("users").update({is_admin:h}).eq("id",g.dataset.toggleAdmin);o?t(o.message,"error"):(t("Rôle mis à jour ✅","success"),nt(e,{toast:t}))})})}}const pe=[{value:"normal",label:"Normal",color:"#ccc"},{value:"pepite",label:"Pépite",color:"#D4A017"},{value:"papyte",label:"Papyte",color:"#909090"},{value:"legende",label:"Légende",color:"#7a28b8"}];async function it(e,t){var h;const{toast:n}=t,{data:i,error:r}=await _.from("market_listings").select(`
    ...
    `).order("listed_at",{ascending:!1}).limit(100);if(r){e.innerHTML=`<p style="color:var(--danger)">${r.message}</p>`;return}const a=i||[],m=a.filter(o=>o.status==="active").length,g=a.filter(o=>o.status==="sold").length;e.innerHTML=`
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;flex-wrap:wrap;gap:10px">
      ...
      <div style="display:flex;gap:12px">
        ...
        <div class="card-panel" style="min-width:120px;text-align:center;padding:12px 20px">
          <div style="font-size:22px;font-weight:700;color:#D4A017">${g}</div>
    ...
    <div class="card-panel">
      <div class="table-wrap">
        <table>
          ...
          <tbody>
            ${a.map(o=>{var p,s,E,f;const u=(p=o.card)==null?void 0:p.player,l=u?`${u.firstname} ${u.surname_real}`:((s=o.card)==null?void 0:s.card_type)||"—",b={active:"#1A6B3C",sold:"#D4A017",cancelled:"#888"};return`<tr>
  ...
  `,document.querySelectorAll("[data-cancel]").forEach(o=>{o.addEventListener("click",async()=>{if(!confirm("Annuler cette annonce ?"))return;const{error:u}=await _.from("market_listings").update({status:"cancelled"}).eq("id",o.dataset.cancel);u?n(u.message,"error"):(n("Annonce annulée","success"),it(e,t))})}),(h=document.getElementById("btn-price-grid"))==null||h.addEventListener("click",()=>nn(t))}async function nn(e){const{openModal:t,closeModal:n,toast:i}=e;await fe(e)}async function fe(e){var o,u,l,b;const{openModal:t,closeModal:n,toast:i}=e,{data:r,error:a}=await _.from("sell_price_configs").select("*").order("rarity").order("note_min");if(a){i(a.message,"error");return}const m=p=>{const s=pe.find(E=>E.value===p.rarity)||pe[0];return`<tr data-row="${p.id}">
    ...
    <div style="max-height:320px;overflow-y:auto;margin-bottom:14px">
      <table>
        ...
        <tbody id="pg-tbody">
          ${(r||[]).map(m).join("")||'<tr><td colspan="6" style="text-align:center;color:#aaa;padding:20px">Aucune règle définie.</td></tr>'}
        </tbody>
      </table>
    </div>
...
`;n.forEach(a=>{var m;r+=[a.firstname,a.surname_real,a.surname_real,a.country_code,((m=a.clubs)==null?void 0:m.encoded_name)||"",a.job,a.job2||"",a.note_g,a.note_d,a.note_m,a.note_a,a.rarity,a.note_min??"",a.note_max??"",a.skin,a.hair,a.hair_length,a.sell_price].map(Me).join(",")+`
`}),ne("players_export.csv",r),t(`${n.length} joueurs exportés`,"success")}),document.getElementById("import-clubs").addEventListener("change",async n=>{const i=n.target.files[0];if(!i)return;const r=document.getElementById("clubs-import-result");r.innerHTML='<span style="color:var(--gray-600)">Import en cours...</span>';try{const a=await i.text(),m=ze(a);if(m.length===0){r.innerHTML='<span style="color:#c0392b">Fichier vide</span>';return}let g=0,h=0;const o=[];for(const u of m){if(!u.real_name||!u.encoded_name||!u.country_code){h++,o.push(`Ligne ignorée (champs manquants): ${u.real_name||"?"}`);continue}const l={real_name:u.real_name,encoded_name:u.encoded_name.toUpperCase(),country_code:u.country_code.toUpperCase().slice(0,2),logo_url:u.logo_url||null},{error:b}=await _.from("clubs").upsert(l,{onConflict:"encoded_name"});b?(h++,o.push(`${u.encoded_name}: ${b.message}`)):g++}r.innerHTML=`<div style="color:var(--green)">✅ ${g} clubs importés</div>
        ${h>0?`<div style="color:#c0392b">❌ ${h} erreurs</div><div style="font-size:11px;color:var(--gray-600);max-height:100px;overflow-y:auto">${o.slice(0,10).join("<br>")}</div>`:""}`,t(`${g} clubs importés`,"success")}catch(a){r.innerHTML=`<span style="color:#c0392b">Erreur : ${a.message}</span>`}n.target.value=""}),document.getElementById("import-players").addEventListener("change",async n=>{const i=n.target.files[0];if(!i)return;const r=document.getElementById("players-import-result");r.innerHTML='<span style="color:var(--gray-600)">Import en cours...</span>';try{const a=await i.text(),m=ze(a);if(m.length===0){r.innerHTML='<span style="color:#c0392b">Fichier vide</span>';return}const{data:g}=await _.from("clubs").select("id,encoded_name"),h={};(g||[]).forEach(v=>{h[v.encoded_name.toUpperCase()]=v.id});let o=0,u=0;const l=[],b=["GK","DEF","MIL","ATT"],p=["normal","pepite","papyte","legende"],s=["blanc","metisse","typ","noir"],E=["blond","marron","noir","chauve"],f=["rase","court","milong","long"];for(const v of m){if(!v.firstname||!v.surname_real||!v.country_code||!v.job){u++,l.push(`Ligne ignorée (champs requis manquants): ${v.firstname||"?"}`);continue}if(!b.includes(v.job)){u++,l.push(`${v.firstname}: job invalide "${v.job}"`);continue}const $=v.club_encoded_name&&h[v.club_encoded_name.toUpperCase()]||null,I={firstname:v.firstname,surname_real:v.surname_real,surname_real:v.surname_real||de(v.surname_real),country_code:v.country_code.toUpperCase().slice(0,2),club_id:$,job:v.job,job2:b.includes(v.job2)?v.job2:null,note_g:parseInt(v.note_g)||0,note_d:parseInt(v.note_d)||0,note_m:parseInt(v.note_m)||0,note_a:parseInt(v.note_a)||0,rarity:p.includes(v.rarity)?v.rarity:"normal",note_min:v.note_min!==""&&v.note_min!=null?parseInt(v.note_min):null,note_max:v.note_max!==""&&v.note_max!=null?parseInt(v.note_max):null,skin:s.includes(v.skin)?v.skin:"blanc",hair:E.includes(v.hair)?v.hair:"noir",hair_length:f.includes(v.hair_length)?v.hair_length:"court",sell_price:parseInt(v.sell_price)||0},{error:x}=await _.from("players").insert(I);x?(u++,l.push(`${v.firstname} ${v.surname_real}: ${x.message}`)):o++}r.innerHTML=`<div style="color:var(--green)">✅ ${o} joueurs importés</div>
...
`).replace(/\r/g,`
`);const t=e.split(`
`).filter(m=>m.trim());if(t.length<2)return[];const n=t[0],i=n.split(";").length>n.split(",").length?";":",",r=Re(n,i).map(m=>m.trim().replace(/^\uFEFF/,"").toLowerCase()),a=[];for(let m=1;m<t.length;m++){if(!t[m].trim())continue;const g=Re(t[m],i),h={};r.forEach((o,u)=>{h[o]=(g[u]||"").trim()}),!Object.values(h).every(o=>!o)&&a.push(h)}return a}function Re(e,t=","){const n=[];let i="",r=!1;for(let a=0;a<e.length;a++){const m=e[a];r?m==='"'?e[a+1]==='"'?(i+='"',a++):r=!1:i+=m:m==='"'?r=!0:m===t?(n.push(i),i=""):i+=m}return n.push(i),n}const an={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};async function rn(e,{toast:t}){const n=Object.keys(ue);let i=n[0];const{data:r}=await _.from("formation_links_overrides").select("formation, links"),a={};(r||[]).forEach(u=>{a[u.formation]=u.links});let m=new Set;function g(u,l){return[u,l].sort().join("-")}function h(u){const l=a[u]||ue[u]||[];m=new Set(l.map(([b,p])=>g(b,p)))}h(i),e.innerHTML=`
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;gap:12px;flex-wrap:wrap">
  ...
  `;function o(){const u=wt[i]||{},l=Be(i),b=320,p=400,s=22;function E(x){const d=u[x];return d?{x:d.x*b,y:d.y*p}:null}let f=`<svg width="${b}" height="${p}" viewBox="0 0 ${b} ${p}" xmlns="http://www.w3.org/2000/svg" style="display:block">`;l.forEach(([x,d],c)=>{const y=E(x),w=E(d);if(!y||!w)return;const B=g(x,d),k=m.has(B),A=k?"#3b82f6":"#999",L=k?.95:.35,S=k?4:3;f+=`<line x1="${y.x}" y1="${y.y}" x2="${w.x}" y2="${w.y}"
        stroke="transparent" stroke-width="14" class="link-hit" data-key="${B}" style="cursor:pointer"/>`,f+=`<line x1="${y.x}" y1="${y.y}" x2="${w.x}" y2="${w.y}"
        stroke="${A}" stroke-width="${S}" stroke-dasharray="${k?"none":"4,3"}"
        opacity="${L}" pointer-events="none" data-line-key="${B}"/>`});for(const x of Object.keys(u)){const d=E(x);if(!d)continue;const c=x.replace(/\d+/,""),y=an[c]||"#555";f+=`<circle cx="${d.x}" cy="${d.y}" r="${s}" fill="${y}" stroke="rgba(255,255,255,0.7)" stroke-width="2" pointer-events="none"/>`,f+=`<text x="${d.x}" y="${d.y+4}" text-anchor="middle" font-size="10" font-weight="900" fill="white" font-family="Arial Black,Arial" pointer-events="none">${x}</text>`}f+="</svg>",document.getElementById("field-wrap").innerHTML=f;const v=document.getElementById("links-list");v.innerHTML=l.map(([x,d])=>{const c=g(x,d),y=m.has(c);return`
        ...
        </button>`}).join("");const $=e.querySelector('div[style*="font-weight:700;font-size:13px;margin-bottom:8px"]');$&&($.textContent=`Liens (${m.size}/${l.length} actifs)`);function I(x){m.has(x)?m.delete(x):m.add(x),o()}e.querySelectorAll(".link-hit").forEach(x=>{x.addEventListener("click",()=>I(x.dataset.key))}),e.querySelectorAll(".link-toggle").forEach(x=>{x.addEventListener("click",()=>I(x.dataset.key))})}o(),document.getElementById("formation-select").addEventListener("change",u=>{i=u.target.value,h(i),o()}),document.getElementById("reset-btn").addEventListener("click",()=>{const u=ue[i]||[];m=new Set(u.map(([l,b])=>g(l,b))),o(),t("Liens réinitialisés aux valeurs par défaut (non sauvegardé)","info")}),document.getElementById("save-btn").addEventListener("click",async()=>{const l=Be(i).filter(([p,s])=>m.has(g(p,s))),{error:b}=await _.from("formation_links_overrides").upsert({formation:i,links:l,updated_at:new Date().toISOString()});if(b){t(b.message,"error");return}a[i]=l,t(`Liens enregistrés pour ${i} (${l.length} actifs)`,"success")})}const ln=[{value:"player",label:"Joueur"},{value:"formation",label:"Formation"},{value:"game_changer",label:"Game Changer"},{value:"ultra_game_changer",label:"Ultra Game Changer"},{value:"game_helper",label:"Game Helper"},{value:"stadium",label:"🏟️ Stade"}],sn=[{value:"",label:"Toutes"},{value:"normal",label:"Normal"},{value:"pepite",label:"Pépite"},{value:"papyte",label:"Papyte"},{value:"legende",label:"Légende"}],dn=[{value:"credits",label:"Crédits"},{value:"pub",label:"Publicité"},{value:"free",label:"Gratuit"}];async function cn(e){e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">Chargement...</div>';const{data:t}=await _.from("booster_configs").select("*").order("sort_order");let n=null,i=[],r=null;const a=()=>window.innerWidth<700;async function m(){if(!n){i=[];return}if(n!==r){const{data:l}=await _.from("booster_drop_rates").select("*").eq("booster_id",n).order("sort_order");i=l||[],r=n}}function g(){return`
    ...
    </div>`;const b=i.reduce((s,E)=>s+Number(E.percentage||0),0),p=Math.abs(b-100)<.1;return`
        ...
        ${i.length===0?'<div style="text-align:center;padding:16px;color:#aaa;font-size:13px">Aucun taux. Cliquez + Ligne.</div>':i.map((s,E)=>`
          <div style="display:grid;grid-template-columns:2fr 1.5fr 50px 50px 60px 32px;gap:4px;align-items:center;margin-bottom:6px;padding:6px;background:#f9f9f9;border-radius:6px" data-rate-idx="${E}">
            <select class="rate-type" data-idx="${E}" style="padding:6px 4px;border:1px solid #ddd;border-radius:4px;font-size:12px;width:100%">
              ${ln.map(f=>`<option value="${f.value}" ${s.card_type===f.value?"selected":""}>${f.label}</option>`).join("")}
            </select>
            <select class="rate-rarity" data-idx="${E}" style="padding:6px 4px;border:1px solid #ddd;border-radius:4px;font-size:12px;width:100%">
              ${sn.map(f=>`<option value="${f.value}" ${(s.rarity||"")===f.value?"selected":""}>${f.label}</option>`).join("")}
            </select>
    ...
    </div>`}async function o(l=!1){l||await m();const b=!n&&a(),p=n&&a();e.innerHTML=`
    <div style="display:flex;height:calc(100vh - 56px);overflow:hidden">
      ${!a()||b||!n?g():""}
      ${!a()||p?h():""}
    </div>`,u()}function u(){var p,s,E,f,v,$,I;const l=x=>e.querySelector(x);e.querySelectorAll(".booster-row").forEach(x=>{x.addEventListener("click",d=>{d.target.closest(".btn-delete")||(n=x.dataset.id,r=null,o())})}),(p=l("#btn-back"))==null||p.addEventListener("click",()=>{n=null,o()}),(s=l("#btn-new"))==null||s.addEventListener("click",async()=>{const x=prompt("Nom du nouveau booster :");if(!(x!=null&&x.trim()))return;const{data:d,error:c}=await _.from("booster_configs").insert({name:x.trim(),price_type:"credits",price_credits:1e3,card_count:5}).select().single();if(c){alert(c.message);return}t.push(d),n=d.id,r=null,o()}),e.querySelectorAll(".btn-delete").forEach(x=>{x.addEventListener("click",async d=>{if(d.stopPropagation(),!confirm("Supprimer ce booster ?"))return;await _.from("booster_configs").delete().eq("id",x.dataset.id);const c=t.findIndex(y=>y.id===x.dataset.id);c>-1&&t.splice(c,1),n===x.dataset.id&&(n=null,r=null),o()})}),(E=l("#btn-cancel"))==null||E.addEventListener("click",()=>{n=null,o()}),(f=l("#f-price-type"))==null||f.addEventListener("change",x=>{const d=l("#credits-field");d&&(d.style.opacity=x.target.value!=="credits"?"0.4":"1")}),(v=l("#btn-pick-icon"))==null||v.addEventListener("click",async()=>{var d;const x=l("#icon-picker-grid");if(x){if(x.style.display!=="none"){x.style.display="none";return}x.innerHTML='<div style="padding:10px;text-align:center;color:#888;font-size:12px">⏳ Chargement...</div>',x.style.display="block";try{const y=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),w=Array.isArray(y)?y.filter(k=>k.name.startsWith("booster")&&/\.(png|jpg|svg|webp)$/i.test(k.name)):[];if(!w.length){x.innerHTML='<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Aucune icône "booster*" trouvée.</div>';return}const B=((d=l("#f-image-url"))==null?void 0:d.value)||"";x.innerHTML=`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(72px,1fr));gap:8px">
          ${w.map(k=>`
    ...
    </tr>`}function De(e){document.getElementById("config-form").style.display="block",document.getElementById("form-title").textContent=e?"Modifier la règle":"Nouvelle règle",document.getElementById("form-id").value=(e==null?void 0:e.id)||"",document.getElementById("form-rarity").value=(e==null?void 0:e.rarity)||"normal",document.getElementById("form-price").value=(e==null?void 0:e.price)||1e3,document.getElementById("form-note-min").value=(e==null?void 0:e.note_min)??1,document.getElementById("form-note-max").value=(e==null?void 0:e.note_max)??10,document.getElementById("config-form").scrollIntoView({behavior:"smooth"})}async function mn(e){const t=document.getElementById("form-id").value,n=document.getElementById("form-rarity").value,i=parseInt(document.getElementById("form-price").value)||0,r=parseInt(document.getElementById("form-note-min").value)||1,a=parseInt(document.getElementById("form-note-max").value)||10;if(r>a){alert("Note min doit être ≤ note max");return}const m={rarity:n,price:i,note_min:r,note_max:a,updated_at:new Date().toISOString()};let g;if(t?{error:g}=await _.from("sell_price_configs").update(m).eq("id",t):{error:g}=await _.from("sell_price_configs").insert(m),g){alert("Erreur : "+g.message);return}document.getElementById("config-form").style.display="none",await xe(e)}async function gn(e){await he(e)}async function he(e){const{data:t}=await _.from("patch_notes").select("*").order("published_at",{ascending:!1});e.innerHTML=`
    ...
    </div>`,document.getElementById("add-article").addEventListener("click",()=>je(null)),document.getElementById("form-cancel").addEventListener("click",()=>{document.getElementById("article-form").style.display="none"}),document.getElementById("form-save").addEventListener("click",()=>bn(e)),e.querySelectorAll("[data-edit]").forEach(n=>{const i=(t||[]).find(r=>r.id===n.dataset.edit);i&&n.addEventListener("click",()=>je(i))}),e.querySelectorAll("[data-delete]").forEach(n=>{n.addEventListener("click",async()=>{confirm("Supprimer cet article ?")&&(await _.from("patch_notes").delete().eq("id",n.dataset.delete),await he(e))})})}function fn(e){const t=new Date(e.published_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"short",year:"numeric"});return`
    ...
    </div>`}function je(e){const t=document.getElementById("article-form");t.style.display="block",document.getElementById("form-title").textContent=e?"Modifier l'article":"Nouvel article",document.getElementById("form-id").value=(e==null?void 0:e.id)||"",document.getElementById("form-art-title").value=(e==null?void 0:e.title)||"",document.getElementById("form-art-desc").value=(e==null?void 0:e.description)||"",document.getElementById("form-art-img").value=(e==null?void 0:e.image_url)||"",document.getElementById("form-art-pub").checked=e?e.is_published:!0;const n=new Date((e==null?void 0:e.published_at)||Date.now()),i=new Date(n.getTime()-n.getTimezoneOffset()*6e4).toISOString().slice(0,16);document.getElementById("form-art-date").value=i,t.scrollIntoView({behavior:"smooth"})}async function bn(e){const t=document.getElementById("form-id").value,n=document.getElementById("form-art-title").value.trim(),i=document.getElementById("form-art-desc").value.trim(),r=document.getElementById("form-art-img").value.trim()||null,a=document.getElementById("form-art-pub").checked,m=document.getElementById("form-art-date").value;if(!n){alert("Le titre est obligatoire.");return}if(!i){alert("La description est obligatoire.");return}const g=m?new Date(m).toISOString():new Date().toISOString(),h={title:n,description:i,image_url:r,is_published:a,published_at:g};let o;if(t?{error:o}=await _.from("patch_notes").update(h).eq("id",t):{error:o}=await _.from("patch_notes").insert(h),o){alert("Erreur : "+o.message);return}document.getElementById("article-form").style.display="none",await he(e)}async function yn(e){await V(e)}async function V(e){var u,l,b,p,s,E,f,v,$,I,x,d;const{data:t}=await _.from("tutorial_steps").select("*").order("step_order");e.innerHTML=`
  ...
  <div style="background:#fff;border-radius:10px;padding:12px 16px;box-shadow:0 1px 4px rgba(0,0,0,0.08);display:flex;align-items:center;gap:12px">
  ...
  </div>`}function Ne(e,t=0){document.getElementById("ts-form").style.display="block",document.getElementById("ts-form-title").textContent=e?"Modifier l'étape":"Nouvelle étape",document.getElementById("ts-id").value=(e==null?void 0:e.id)||"",document.getElementById("ts-emoji").value=(e==null?void 0:e.emoji)||"⚽",document.getElementById("ts-title").value=(e==null?void 0:e.title)||"",document.getElementById("ts-content").value=(e==null?void 0:e.content)||"";const n=document.getElementById("ts-editor");n&&(n.innerHTML=(e==null?void 0:e.content)||"<p>Texte de l'étape…</p>"),document.getElementById("ts-order").value=(e==null?void 0:e.step_order)??t,document.getElementById("ts-active").checked=e?e.is_active:!0;const i=(e==null?void 0:e.color)||"#1A6B3C";document.getElementById("ts-color").value=i,document.getElementById("ts-color-hex").value=i;const r=(e==null?void 0:e.image_url)||"",a=document.getElementById("ts-image");a&&(a.value=r);const m=document.getElementById("ts-img-preview"),g=document.getElementById("ts-img-preview-el");if(r&&m&&g){const h="/";g.src=`${h}icons/${r}`,m.style.display="block"}else m&&(m.style.display="none");document.getElementById("ts-img-picker-grid").style.display="none",document.getElementById("ts-preview-area").innerHTML="",document.getElementById("ts-form").scrollIntoView({behavior:"smooth"})}function at(){document.getElementById("ts-form").style.display="none",document.getElementById("ts-preview-area").innerHTML=""}function xn(){var o,u;const e=document.getElementById("ts-emoji").value||"⚽",t=document.getElementById("ts-title").value||"Titre",n=document.getElementById("ts-editor"),i=n?n.innerHTML:document.getElementById("ts-content").value||"",r=document.getElementById("ts-color-hex").value||"#1A6B3C",a=(u=(o=document.getElementById("ts-image"))==null?void 0:o.value)==null?void 0:u.trim(),g=a?`<div style="padding:0 20px 12px;text-align:center"><img src="/icons/${a}" style="max-height:140px;max-width:100%;border-radius:10px;object-fit:contain"></div>`:"",h=document.getElementById("ts-preview-area");h.innerHTML=`
    ...
    </div>`,h.querySelectorAll("ul,ol").forEach(l=>{l.style.paddingLeft="20px",l.style.margin="6px 0"}),h.querySelectorAll("li").forEach(l=>{l.style.marginBottom="4px"}),h.querySelectorAll("p").forEach(l=>{l.style.marginBottom="8px"})}async function hn(e){var l,b;const t=document.getElementById("ts-id").value,n=document.getElementById("ts-emoji").value.trim()||"⚽",i=document.getElementById("ts-title").value.trim(),r=document.getElementById("ts-editor"),a=(r?r.innerHTML:document.getElementById("ts-content").value).trim(),m=document.getElementById("ts-color-hex").value.trim()||"#1A6B3C",g=parseInt(document.getElementById("ts-order").value)||0,h=document.getElementById("ts-active").checked;if(!i||!a){alert("Titre et contenu sont obligatoires.");return}const o={emoji:n,title:i,content:a,color:m,step_order:g,is_active:h,image_url:((b=(l=document.getElementById("ts-image"))==null?void 0:l.value)==null?void 0:b.trim())||null};let u;if(t?{error:u}=await _.from("tutorial_steps").update(o).eq("id",t):{error:u}=await _.from("tutorial_steps").insert(o),u){alert("Erreur : "+u.message);return}at(),V(e)}const rt="/",En=[["MA","Maroc"],["FR","France"],["AR","Argentine"],["PT","Portugal"],["BR","Brésil"],["ES","Espagne"],["DE","Allemagne"],["GB","Angleterre"],["IT","Italie"],["CM","Cameroun"],["SN","Sénégal"],["NG","Nigéria"],["DK","Danemark"],["NL","Pays-Bas"],["BE","Belgique"],["CI","Côte d'Ivoire"],["AL","Albanie"],["HR","Croatie"],["RS","Serbie"],["TR","Turquie"],["MW","Malawi"],["GH","Ghana"],["ZA","Afrique du Sud"],["EG","Égypte"],["US","États-Unis"],["MX","Mexique"],["CO","Colombie"],["UY","Uruguay"],["CL","Chili"],["JP","Japon"],["KR","Corée du Sud"],["CN","Chine"],["AU","Australie"],["SW","Suisse"],["SE","Suède"],["NO","Norvège"],["PL","Pologne"]];async function _n(e,t){await be(e,t)}async function be(e,t){var r;const[{data:n},{data:i}]=await Promise.all([_.from("stadium_definitions").select("*, club:clubs(encoded_name, logo_url)").order("name"),_.from("clubs").select("id, encoded_name, logo_url").order("encoded_name")]);e.innerHTML=`
      ...
      <div style="flex:1;min-width:220px;display:flex;flex-direction:column;gap:12px">
        ...
        <div>
          ...
          <select id="st-country">
            ...
            ${En.map(([p,s])=>`<option value="${p}" ${(e==null?void 0:e.country_code)===p?"selected":""}>${s} (${p})</option>`).join("")}
          </select>
        </div>
        <div>
  ...
  `);const o=()=>{var y,w,B,k,A,L,S;const p=((y=document.getElementById("st-name"))==null?void 0:y.value)||"NOM DU STADE",s=(B=(w=document.getElementById("st-image"))==null?void 0:w.value)==null?void 0:B.trim(),E=(A=(k=document.getElementById("st-country"))==null?void 0:k.value)==null?void 0:A.trim(),f=document.getElementById("st-club"),v=(f==null?void 0:f.selectedIndex)||0,$=f&&v>0?f.options[v].text:"",I=((S=(L=f==null?void 0:f.options[v])==null?void 0:L.getAttribute)==null?void 0:S.call(L,"data-logo"))||"";let x=null;s?x=s.startsWith("http")?s:`${rt}icons/${s}`:I?x=I:E&&(x=`https://flagsapi.com/${E.toLowerCase().slice(0,2).toUpperCase()}/flat/64.png`);const d=$||E||"—",c=document.getElementById("st-preview-card");c&&(c.innerHTML=Je(p,x,`${d}<br>+10 ⭐`,{width:140}))};o(),["st-name","st-club","st-country","st-image"].forEach(p=>{var s,E;(s=document.getElementById(p))==null||s.addEventListener("input",o),(E=document.getElementById(p))==null||E.addEventListener("change",o)}),(u=document.getElementById("st-cancel"))==null||u.addEventListener("click",()=>a()),(l=document.getElementById("st-save"))==null||l.addEventListener("click",async()=>{const p=document.getElementById("st-name").value.trim(),s=document.getElementById("st-club").value||null,E=document.getElementById("st-country").value.trim().toUpperCase()||null,f=document.getElementById("st-image").value.trim()||null;if(!p){m("Le nom est obligatoire","error");return}const v={name:p,club_id:s,country_code:E,image_url:f},{error:$}=e?await _.from("stadium_definitions").update(v).eq("id",e.id):await _.from("stadium_definitions").insert(v);if($){m("Erreur : "+$.message,"error");return}m(e?"Stade modifié ✅":"Stade créé ✅","success"),a(),be(n,i)}),(b=document.getElementById("st-delete"))==null||b.addEventListener("click",async()=>{confirm("Supprimer ce stade ?")&&(await _.from("stadium_definitions").delete().eq("id",e.id),m("Stade supprimé","success"),a(),be(n,i))})}const H=[{value:"game_changer",label:"⚡ Game Changer",border:"#7a28b8",bg:"linear-gradient(135deg,#4a0080,#7a28b8)"},{value:"ultra_game_changer",label:"💎 Ultra Game Changer",border:"#00bcd4",bg:"linear-gradient(135deg,#006080,#00bcd4)"}],In=[{value:"purple",label:"Violet",hex:"#7a28b8"},{value:"light_blue",label:"Bleu clair",hex:"#00bcd4"}],G=[{value:"BOOST_STAT",label:"⚡ Boost stats (+N)",hasValue:!0,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"DEBUFF_STAT",label:"💀 Debuff stats (-N)",hasValue:!0,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"GRAY_PLAYER",label:"❄️ Griser un joueur",hasValue:!1,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"REVIVE_PLAYER",label:"💫 Ressusciter un joueur",hasValue:!1,hasTarget:!1,hasCount:!0,hasRoles:!1},{value:"REMOVE_GOAL",label:"🚫 Retirer un but adverse",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1},{value:"ADD_GOAL_DRAW",label:"⚽ +1 but si duel nul",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1},{value:"ADD_SUB",label:"🔄 Ajouter un remplacement",hasValue:!1,hasTarget:!1,hasCount:!0,hasRoles:!1},{value:"CUSTOM",label:"🛠️ Effet custom (hardcodé)",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1}],Fe=["GK","DEF","MIL","ATT"];async function $n(e,t){await Ee(e,t)}async function Ee(e,t){var i;const{data:n}=await _.from("gc_definitions").select("*").order("sort_order").order("created_at");e.innerHTML=`
    <div style="padding:20px;max-width:1100px;margin:0 auto">
      ...
      <div style="display:flex;flex-wrap:wrap;gap:14px" id="gc-list">
        ${(n||[]).length?(n||[]).map(r=>kn(r)).join(""):'<div style="color:var(--tile-fg-dim);padding:24px;text-align:center;width:100%">Aucune carte Game Changer.</div>'}
      </div>
    </div>`,(i=document.getElementById("gc-add-btn"))==null||i.addEventListener("click",()=>Ue(null,e,t)),e.querySelectorAll("[data-edit-gc]").forEach(r=>{r.addEventListener("click",()=>{const a=(n||[]).find(m=>m.id===r.dataset.editGc);a&&Ue(a,e,t)})}),e.querySelectorAll(".btn-del-gc").forEach(r=>{r.addEventListener("click",async a=>{a.stopPropagation(),confirm("Supprimer cette carte Game Changer ?")&&(await _.from("gc_definitions").delete().eq("id",r.dataset.del),Ee(e,t))})})}function kn(e){const t=H.find(r=>r.value===e.gc_type)||H[0],n=e.image_url?`/icons/${e.image_url}`:null,i=me(e.name||"?",n,t.label.split(" ")[0],e.effect||"",{width:130});return`<div data-edit-gc="${e.id}" style="position:relative;cursor:pointer">
    ${i}
    <div style="position:absolute;top:4px;left:4px;z-index:10;display:flex;gap:3px">
  ...
  </div>`}function Ue(e,t,n){var b,p,s,E;const{openModal:i,closeModal:r,toast:a}=n,m=!e,g=e||{name:"",gc_type:"game_changer",color:"purple",effect:"",image_url:"",sort_order:0,is_active:!0,effect_type:"BOOST_STAT",effect_params:{}},h=(()=>{const f=g.effect_params||{},v=G.find($=>$.value===(g.effect_type||"BOOST_STAT"))||G[0];return`
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
      ${v.hasValue?`<div>
    ...
    <div style="display:flex;justify-content:center;margin-bottom:16px;padding:20px;background:#1a1a2e;border-radius:12px">
      <div id="gc-modal-preview">${me(g.name||"?",g.image_url?`/icons/${g.image_url}`:null,(H.find(f=>f.value===g.gc_type)||H[0]).label.split(" ")[0],g.effect||"",{width:150})}</div>
    </div>

    <div style="display:flex;flex-direction:column;gap:12px">
      <div>
        <label>NOM</label>
        <input id="gc-name" value="${g.name}">
      </div>
      <div>
        <label>EFFET / DESCRIPTION</label>
        <textarea id="gc-effect" rows="3">${g.effect||""}</textarea>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
        <div>
          <label>TYPE</label>
          <select id="gc-type">
            ${H.map(f=>`<option value="${f.value}" ${g.gc_type===f.value?"selected":""}>${f.label}</option>`).join("")}
          </select>
        </div>
        <div>
          <label>COULEUR</label>
          <select id="gc-color">
            ${In.map(f=>`<option value="${f.value}" ${g.color===f.value?"selected":""}>${f.label}</option>`).join("")}
          </select>
        </div>
      </div>
      <div>
        ...
        <div style="display:flex;gap:8px;align-items:center">
          <input id="gc-image-url" value="${g.image_url||""}" placeholder="gamechanger-nom.png" style="flex:1">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
        <div>
          <label>ORDRE</label>
          <input id="gc-sort" type="number" value="${g.sort_order||0}">
        </div>
        <div style="display:flex;align-items:center;gap:8px;padding-top:20px">
          <input type="checkbox" id="gc-active" ${g.is_active?"checked":""} style="width:18px;height:18px;cursor:pointer">
      ...
      <div style="border-top:1px solid #eee;padding-top:12px;margin-top:4px">
        ...
        <div>
          ...
          <select id="gc-effect-type" style="margin-bottom:10px">
            ${G.map(f=>`<option value="${f.value}" ${(g.effect_type||"BOOST_STAT")===f.value?"selected":""}>${f.label}</option>`).join("")}
          </select>
        </div>
        <div id="gc-params-wrap">${h}</div>
      </div>
    </div>
  `;i(m?"Nouvelle carte Game Changer":`Modifier : ${g.name}`,o,`
    <button id="gc-cancel" class="btn btn-ghost">Annuler</button>
    <button id="gc-save" class="btn btn-primary">💾 Enregistrer</button>
  `);const l=()=>{var c,y,w,B,k;const f=((c=document.getElementById("gc-name"))==null?void 0:c.value)||"?",v=((y=document.getElementById("gc-effect"))==null?void 0:y.value)||"",$=((w=document.getElementById("gc-type"))==null?void 0:w.value)||"game_changer",I=(k=(B=document.getElementById("gc-image-url"))==null?void 0:B.value)==null?void 0:k.trim(),x=H.find(A=>A.value===$)||H[0],d=document.getElementById("gc-modal-preview");d&&(d.innerHTML=me(f,I?`/icons/${I}`:null,x.label.split(" ")[0],v,{width:150}))};["gc-name","gc-effect","gc-type","gc-image-url"].forEach(f=>{var v,$;(v=document.getElementById(f))==null||v.addEventListener("input",l),($=document.getElementById(f))==null||$.addEventListener("change",l)}),(b=document.getElementById("gc-effect-type"))==null||b.addEventListener("change",()=>{const f=document.getElementById("gc-effect-type").value,v=G.find(I=>I.value===f)||G[0],$=document.getElementById("gc-params-wrap");$.innerHTML=`
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
    ...
    </div>`}),(p=document.getElementById("btn-pick-gc-icon"))==null||p.addEventListener("click",async()=>{var v;const f=document.getElementById("gc-icon-picker");if(f){if(f.style.display!=="none"){f.style.display="none";return}f.innerHTML='<div style="padding:10px;text-align:center;color:#888;font-size:12px">⏳ Chargement...</div>',f.style.display="block";try{const I=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),x=Array.isArray(I)?I.filter(c=>c.name.startsWith("gamechanger-")&&/\.(png|jpg|svg|webp)$/i.test(c.name)):[];if(!x.length){f.innerHTML='<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Aucun fichier "gamechanger-*" trouvé dans public/icons/</div>';return}const d=((v=document.getElementById("gc-image-url"))==null?void 0:v.value)||"";f.innerHTML=`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(72px,1fr));gap:8px">
        ${x.map(c=>`
      ...
      </div>`,f.querySelectorAll(".gc-icon-item").forEach(c=>{c.addEventListener("click",()=>{const y=document.getElementById("gc-image-url");y&&(y.value=c.dataset.name),f.style.display="none",l()})})}catch($){f.innerHTML=`<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Erreur API GitHub : ${$.message}</div>`}}}),(s=document.getElementById("gc-cancel"))==null||s.addEventListener("click",()=>r()),(E=document.getElementById("gc-save"))==null||E.addEventListener("click",async()=>{var d,c,y,w,B,k,A,L,S,C,T,z,j,N;const f=G.find(P=>{var F;return P.value===(((F=document.getElementById("gc-effect-type"))==null?void 0:F.value)||"BOOST_STAT")})||G[0],v=f.hasRoles?[...document.querySelectorAll(".gc-p-role:checked")].map(P=>P.value):null,$={...f.hasValue?{value:Number((d=document.getElementById("gc-p-value"))==null?void 0:d.value)||2}:{},...f.hasCount?{count:Number((c=document.getElementById("gc-p-count"))==null?void 0:c.value)||1}:{},...f.hasTarget?{target:((y=document.getElementById("gc-p-target"))==null?void 0:y.value)||"home"}:{},...f.hasRoles?{roles:v!=null&&v.length?v:null}:{}},I={name:((B=(w=document.getElementById("gc-name"))==null?void 0:w.value)==null?void 0:B.trim())||"",effect:((A=(k=document.getElementById("gc-effect"))==null?void 0:k.value)==null?void 0:A.trim())||null,image_url:((S=(L=document.getElementById("gc-image-url"))==null?void 0:L.value)==null?void 0:S.trim())||null,gc_type:((C=document.getElementById("gc-type"))==null?void 0:C.value)||"game_changer",color:((T=document.getElementById("gc-color"))==null?void 0:T.value)||"purple",sort_order:Number((z=document.getElementById("gc-sort"))==null?void 0:z.value)||0,is_active:((j=document.getElementById("gc-active"))==null?void 0:j.checked)??!0,effect_type:((N=document.getElementById("gc-effect-type"))==null?void 0:N.value)||"BOOST_STAT",effect_params:$};if(!I.name){a("Le nom est obligatoire","error");return}const{error:x}=m?await _.from("gc_definitions").insert(I):await _.from("gc_definitions").update(I).eq("id",g.id);if(x){a(x.message,"error");return}a(m?"Carte créée ✅":"Carte modifiée ✅","success"),r(),Ee(t,n)})}async function Bn(e,{toast:t,openModal:n,closeModal:i}){await J(e,{toast:t,openModal:n,closeModal:i})}async function J(e,t){var x;const{toast:n,openModal:i,closeModal:r}=t,[{data:a,error:m},{data:g}]=await Promise.all([_.from("ranked_seasons").select("*").order("start_at",{ascending:!1}),_.from("users").select("mmr,rank_tier,placement_matches").gte("placement_matches",1)]);if(m){e.innerHTML=`<p style="color:red">${m.message}</p>`;return}const h=a||[],o=h.find(d=>d.is_active),u=(g||[]).filter(d=>(d.placement_matches||0)>=1),l=u.filter(d=>(d.placement_matches||0)>=10),b=["bronze","silver","gold","platinum","diamond","master"],p={bronze:"Bronze",silver:"Argent",gold:"Or",platinum:"Platine",diamond:"Diamant",master:"Maître"},s={bronze:"#cd7f32",silver:"#a0a0a0",gold:"#D4A017",platinum:"#00e5ff",diamond:"#5b9bd5",master:"#9b59b6"},E={bronze:"🥉",silver:"🥈",gold:"🥇",platinum:"💎",diamond:"🔷",master:"👑"},f={};b.forEach(d=>{f[d]=0}),l.forEach(d=>{const c=d.rank_tier||"bronze";f[c]!==void 0&&f[c]++});const v=l.length?Math.round(l.reduce((d,c)=>d+(c.mmr||1e3),0)/l.length):0;function $(d){return d?new Date(d).toLocaleDateString("fr",{day:"numeric",month:"short",year:"numeric"}):"—"}function I(d){const c=new Date;return d.is_active?'<span style="background:#1A6B3C;color:#fff;padding:2px 8px;border-radius:10px;font-size:11px;font-weight:700">● ACTIVE</span>':new Date(d.end_at)<c?'<span style="background:#555;color:#ccc;padding:2px 8px;border-radius:10px;font-size:11px">Terminée</span>':'<span style="background:#e67e22;color:#fff;padding:2px 8px;border-radius:10px;font-size:11px">À venir</span>'}e.innerHTML=`
    <div style="display:flex;flex-direction:column;gap:20px">
      ...
      <div class="card-panel" style="background:linear-gradient(135deg,#0a3d1e,#1a6b3c20);border:1.5px solid #1A6B3C40">
        ...
        <div style="display:flex;flex-direction:column;gap:4px">
          ${b.map(d=>{const c=f[d],y=l.length>0?Math.round(c/l.length*100):0;return`
