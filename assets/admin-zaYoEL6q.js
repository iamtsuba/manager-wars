import{s as _,i as Et,r as _e,a as we,A as _t,l as wt,g as $t,K as ze,D as F,b as It,S as kt,c as O,H as Lt,R as Bt,e as pe,d as Qe,f as At,h as St,E as Ct,j as Tt,N as Mt,M as zt,B as Rt,k as Dt,F as ge,m as Re,n as jt,o as Xe,p as be,q as Nt}from"./special-cards-DNsLLa5x.js";async function et(e){var o;const[{count:t},{count:n},{count:i},{count:d},{count:s},{data:g},{data:v}]=await Promise.all([_.from("players").select("*",{count:"exact",head:!0}),_.from("clubs").select("*",{count:"exact",head:!0}),_.from("users").select("*",{count:"exact",head:!0}),_.from("stadium_definitions").select("*",{count:"exact",head:!0}),_.from("users").select("*",{count:"exact",head:!0}).gt("last_seen_at",new Date(Date.now()-18e4).toISOString()),_.rpc("get_signup_password"),_.from("app_feature_flags").select("*").order("key")]),E=g||"";e.innerHTML=`
...
Cette action est irréversible.`))return;const i=document.getElementById("fix-old-faces-btn");i&&(i.disabled=!0,i.textContent="⏳ En cours...");try{const{data:d,error:s}=await _.from("players").select("id, country_code, face").like("face","public/faces/%");if(s){n(s.message,"error");return}if(!(d!=null&&d.length)){n("Aucun joueur avec l'ancien format — rien à faire ✅","success");return}const{data:g}=await _.from("players").select("face").not("face","is",null),v=new Set((g||[]).map(l=>l.face).filter(l=>l&&!l.startsWith("public/faces/")));let E=0,o=0;for(const l of d){const a=await we(l.country_code,v);if(!a){o++;continue}const{error:r}=await _.from("players").update({face:a}).eq("id",l.id);if(r){o++;continue}v.add(a),E++}n(`${E} photo(s) réattribuée(s)${o?`, ${o} échec(s)`:""} ✅`,o?"error":"success")}finally{i&&(i.disabled=!1,i.textContent="🖼️ Réattribuer anciennes photos")}}async function Ut(e,t){await se(e,t)}async function se(e,t,n=null){var o,l,a,r,u;const{toast:i}=t;n||(n={search:((o=document.getElementById("search-players"))==null?void 0:o.value)||"",job:((l=document.getElementById("filter-job"))==null?void 0:l.value)||"",rarity:((a=document.getElementById("filter-rarity"))==null?void 0:a.value)||"",club:((r=document.getElementById("filter-club"))==null?void 0:r.value)||"",country:((u=document.getElementById("filter-country"))==null?void 0:u.value)||""});const[{data:d,error:s},{data:g}]=await Promise.all([_.from("players").select("*, clubs(id,encoded_name,logo_url)"),_.from("clubs").select("id,encoded_name").order("encoded_name")]);if(s){e.innerHTML=`<p style="color:red">${s.message}</p>`;return}const v={GK:0,DEF:1,MIL:2,ATT:3},E=(d||[]).sort((c,f)=>{const p=(v[c.job]??4)-(v[f.job]??4);return p!==0?p:(c.surname_real||"").localeCompare(f.surname_real||"")});qt(e,E,g||[],t,n)}function qt(e,t,n,i,d=null){var l,a;const{toast:s}=i;if(e.innerHTML=`
    ...
    </div>`,d){const r=d,u=c=>document.getElementById(c);r.search&&u("search-players")&&(u("search-players").value=r.search),r.job&&u("filter-job")&&(u("filter-job").value=r.job),r.rarity&&u("filter-rarity")&&(u("filter-rarity").value=r.rarity),r.club&&u("filter-club")&&(u("filter-club").value=r.club),r.country&&u("filter-country")&&(u("filter-country").value=r.country)}if(d){const r=d,u=c=>document.getElementById(c);r.search&&u("search-players")&&(u("search-players").value=r.search),r.job&&u("filter-job")&&(u("filter-job").value=r.job),r.rarity&&u("filter-rarity")&&(u("filter-rarity").value=r.rarity),r.club&&u("filter-club")&&(u("filter-club").value=r.club),r.country&&u("filter-country")&&(u("filter-country").value=r.country)}function g(){const r=document.getElementById("search-players").value.toLowerCase(),u=document.getElementById("filter-job").value,c=document.getElementById("filter-rarity").value,f=document.getElementById("filter-club").value,p=document.getElementById("filter-country").value;return t.filter(y=>(!r||`${y.firstname} ${y.surname_real}`.toLowerCase().includes(r))&&(!u||y.job===u)&&(!c||y.rarity===c)&&(!f||y.club_id===f)&&(!p||y.country_code===p))}const v=new Set;function E(){const r=document.getElementById("bulk-bar"),u=document.getElementById("bulk-count");r&&(r.style.display=v.size>0?"flex":"none",u&&(u.textContent=`${v.size} joueur(s) sélectionné(s)`))}function o(){const r=g();document.getElementById("count-label").textContent=`${r.length} joueur(s)`;const u=document.getElementById("players-list");if(!r.length){u.innerHTML='<div style="color:var(--gray-600);padding:20px;text-align:center">Aucun joueur.</div>';return}u.innerHTML=r.map(c=>{const f={...c,clubs:c.clubs,face:c.face||null},p=_e(f,{width:120});return`<div style="position:relative;cursor:pointer" data-edit="${c.id}">
        ${p}
    ...
    `).join(""),E.querySelectorAll(".club-row").forEach(r=>{r.addEventListener("click",()=>{const u=re.find(c=>c.id===r.dataset.clubId);u&&je(u,e,t),E.querySelectorAll(".club-row").forEach(c=>c.style.background="var(--tile-bg)"),r.style.background="rgba(26,107,60,0.18)"})}),E.querySelectorAll(".btn-del-club").forEach(r=>{r.addEventListener("click",async()=>{if(!confirm("Supprimer ce club ?"))return;const{error:u}=await _.from("clubs").delete().eq("id",r.dataset.del);u?i(u.message,"error"):(i("Club supprimé","success"),me(e,t))})})}}function nn(e){var j,U,q,H;const{openModal:t,closeModal:n}=e;let i=6;const d=36;function s(B){return new Array(B*B).fill("#ffffff")}let g=s(i),v="OP",E="#D4A017",o=50,l=0,a=0,r="",u="#ffffff",c=25,f=0,p=35,y=null,k=null;function A(){const B=d*i;let T="";for(let M=0;M<i;M++)for(let R=0;R<i;R++){const N=M*i+R;T+=`<rect x="${R*d}" y="${M*d}" width="${d}" height="${d}" fill="${g[N]}"/>`}const z=(M,R,N,G,V)=>{if(!M)return"";const J=B/2+G/100*B,Y=B/2+V/100*B,W=N/100*B*(M.length>3?3/M.length:1);return`<text x="${J}" y="${Y}" text-anchor="middle" dominant-baseline="central"
        font-family="Arial Black, Arial, sans-serif" font-weight="900" font-size="${W}"
        fill="${R}">${M}</text>`};return`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${B} ${B}">
      ${T}
      ${z(v,E,o,l,a)}
      ${z(r,u,c,f,p)}
    </svg>`}function b(B){return"data:image/svg+xml;base64,"+btoa(unescape(encodeURIComponent(B)))}const $=(B,T,z,M,R,N,G)=>`
    <div style="background:#f7f7f7;border-radius:10px;padding:12px;display:flex;flex-direction:column;gap:8px">
      <div style="font-weight:700;font-size:12px;color:#333">${T}</div>
      ...
      <div style="display:flex;align-items:center;gap:8px">
        ...
        <input type="color" id="flag-text${B}-color" value="${M}" style="width:40px;height:28px;padding:2px;cursor:pointer">
      </div>
      <div>
  ...
  `,h=`
    ...
    <div style="display:grid;grid-template-columns:200px 1fr 1fr;gap:16px;margin-bottom:16px">
      ...
      ${$(1,"TEXTE 1 (20 caractères max)",v,E,o,l,a)}
      ${$(2,"TEXTE 2 — optionnel (20 caractères max)",r,u,c,f,p)}
    </div>

    <div>
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;flex-wrap:wrap;gap:8px">
        <label style="margin:0;font-size:12px">Couleurs des ${i*i} carrés — clique un carré, puis Ctrl+C / Ctrl+V pour copier-coller sa couleur</label>
  ...
  `;t(`🎨 Générer un logo (${i}×${i})`,h,`
    <button id="flag-cancel" class="btn btn-ghost">Annuler</button>
    <button id="flag-use" class="btn btn-primary">✅ Utiliser ce logo</button>
  `);function I(){const B=document.getElementById("flag-squares-grid");if(!B)return;const T=i>=12?30:i>=9?36:44;B.style.gridTemplateColumns=`repeat(${i}, ${T}px)`,B.innerHTML=g.map((z,M)=>`
      <div class="flag-square" data-i="${M}" style="width:${T}px;height:${T}px;border-radius:4px;background:${z};cursor:pointer;
        box-sizing:border-box;border:${y===M?"3px solid #1A6B3C":"1px solid rgba(0,0,0,0.15)"}"></div>
    `).join(""),B.querySelectorAll(".flag-square").forEach(z=>{z.addEventListener("click",()=>{y=parseInt(z.dataset.i),I(),m()}),z.addEventListener("dblclick",()=>{const M=parseInt(z.dataset.i),R=document.createElement("input");R.type="color",R.value=g[M],R.style.position="fixed",R.style.opacity="0",document.body.appendChild(R),R.addEventListener("input",()=>{g[M]=R.value,I(),C()}),R.addEventListener("change",()=>R.remove()),R.click()})})}function m(){const B=document.getElementById("flag-copy-btn"),T=document.getElementById("flag-paste-btn");B.disabled=y===null,T.disabled=y===null||k===null}function w(){if(y===null)return;k=g[y];const B=document.getElementById("flag-clipboard-preview"),T=document.getElementById("flag-clipboard-swatch");B&&(B.style.display="inline-flex"),T&&(T.style.background=k),m()}function L(){y===null||k===null||(g[y]=k,I(),C())}(j=document.getElementById("flag-copy-btn"))==null||j.addEventListener("click",w),(U=document.getElementById("flag-paste-btn"))==null||U.addEventListener("click",L);function S(B){if(!document.getElementById("flag-squares-grid")){document.removeEventListener("keydown",S);return}!(B.ctrlKey||B.metaKey)||y===null||(B.key==="c"||B.key==="C"?(B.preventDefault(),w()):(B.key==="v"||B.key==="V")&&(B.preventDefault(),L()))}document.addEventListener("keydown",S),document.querySelectorAll(".flag-grid-size").forEach(B=>{B.addEventListener("click",()=>{const T=parseInt(B.dataset.n);if(T===i)return;i=T,g=s(i),y=null,k=null;const z=document.getElementById("flag-clipboard-preview");z&&(z.style.display="none"),I(),m(),C(),document.querySelectorAll(".flag-grid-size").forEach(M=>{M.classList.toggle("btn-primary",parseInt(M.dataset.n)===i),M.classList.toggle("btn-ghost",parseInt(M.dataset.n)!==i)})})});function C(){const B=document.getElementById("flag-preview");B&&(B.innerHTML=A())}I(),m(),C();function D(B){document.getElementById(`flag-text${B}`).addEventListener("input",T=>{const z=T.target.value.slice(0,20);B===1?v=z:r=z,C()}),document.getElementById(`flag-text${B}-color`).addEventListener("input",T=>{B===1?E=T.target.value:u=T.target.value,C()}),document.getElementById(`flag-text${B}-size`).addEventListener("input",T=>{const z=parseInt(T.target.value);B===1?o=z:c=z,document.getElementById(`flag-size${B}-val`).textContent=z,C()}),document.getElementById(`flag-text${B}-x`).addEventListener("input",T=>{const z=parseInt(T.target.value);B===1?l=z:f=z,document.getElementById(`flag-x${B}-val`).textContent=z,C()}),document.getElementById(`flag-text${B}-y`).addEventListener("input",T=>{const z=parseInt(T.target.value);B===1?a=z:p=z,document.getElementById(`flag-y${B}-val`).textContent=z,C()})}D(1),D(2),(q=document.getElementById("flag-cancel"))==null||q.addEventListener("click",()=>n()),(H=document.getElementById("flag-use"))==null||H.addEventListener("click",()=>{const B=b(A()),T=document.getElementById("m-logo-url-current");T&&(T.value=B);const z=document.getElementById("logo-preview");z&&(z.innerHTML=`<img src="${B}" style="width:100%;height:100%;object-fit:contain">`),n()})}async function je(e,t,n){var p,y,k,A,b,$,h;const{toast:i}=n,d=!!e,s=e?ct(e):{...F},g=document.getElementById("club-panel");if(!g)return;const v=Object.entries(ze).map(([x,I])=>`<option value="${x}" ${s.style===x?"selected":""}>${I.label}</option>`).join(""),E=$e.map(x=>`<option value="${x}" ${((e==null?void 0:e.country_code)||"FR")===x?"selected":""}>${x}</option>`).join("");g.style.display="block",g.style.alignItems="",g.style.justifyContent="",g.innerHTML=`
      ...
      <div style="border-top:1px solid var(--tile-border);padding-top:12px">
        ...
        <div style="display:flex;gap:12px;align-items:flex-start;flex-wrap:wrap">
          <div style="flex:1;min-width:200px;display:flex;flex-direction:column;gap:8px">
            ...
            ${[["Couleur 1","m-kit-color1",s.color1,!1],["Couleur 2","m-kit-color2",s.color2,!1],["Couleur 3","m-kit-color3",s.color3,!0],["Short","m-kit-shorts",s.shorts,!1],["Chaussettes","m-kit-socks",s.socks,!1]].map(([x,I,m,w])=>`
  ...
  `,(p=document.getElementById("m-logo-file"))==null||p.addEventListener("change",x=>{var w;const I=(w=x.target.files)==null?void 0:w[0];if(!I)return;const m=new FileReader;m.onload=()=>{document.getElementById("logo-preview").innerHTML=`<img src="${m.result}" style="width:100%;height:100%;object-fit:contain">`},m.readAsDataURL(I)}),(y=document.getElementById("m-open-flag-builder"))==null||y.addEventListener("click",()=>{nn(n)}),ie();function o(){var w,L;const x=((w=document.getElementById("m-kit-style"))==null?void 0:w.value)||"uni",I=((L=ze[x])==null?void 0:L.colors)===3,m=document.getElementById("wrap-m-kit-color3");m&&(m.style.display=I?"":"none")}o(),["m-kit-color1","m-kit-color2","m-kit-color3","m-kit-shorts","m-kit-socks"].forEach(x=>{const I=document.getElementById(x),m=document.getElementById(x+"-txt");!I||!m||(I.addEventListener("input",()=>{m.value=I.value,ie()}),m.addEventListener("input",()=>{const w=m.value.trim();/^#[0-9a-fA-F]{6}$/.test(w)&&(I.value=w,ie())}),m.addEventListener("change",()=>{let w=m.value.trim();w.startsWith("#")||(w="#"+w),/^#[0-9a-fA-F]{6}$/.test(w)&&(I.value=w,m.value=w,ie())}))}),(k=document.getElementById("m-kit-style"))==null||k.addEventListener("change",()=>{o(),ie()});const l=document.getElementById("m-real"),a=document.getElementById("m-encoded");function r(){if(!l||!a||a.value)return;const x=l.value.trim().split(/\s+/),I=x.length===1?x[0].toUpperCase().slice(0,6):x.filter(m=>m.length>2).map(m=>m[0].toUpperCase()).join("")||x[0].toUpperCase().slice(0,4);a.value=I}l==null||l.addEventListener("input",r),(A=document.getElementById("auto-encode-btn"))==null||A.addEventListener("click",()=>{a&&(a.value=""),r()}),(b=document.getElementById("m-save"))==null||b.addEventListener("click",()=>dn(e,d,t,n));const u=document.getElementById("m-gen-squad"),c=document.getElementById("m-gen-strong-label");function f(){if(c&&(c.style.display=u!=null&&u.checked?"flex":"none",!(u!=null&&u.checked))){const x=document.getElementById("m-gen-strong");x&&(x.checked=!1)}}u==null||u.addEventListener("change",f),f(),($=document.getElementById("btn-gen-squad-panel"))==null||$.addEventListener("click",()=>{an(e,n,t)}),(h=document.getElementById("btn-gen-one-player"))==null||h.addEventListener("click",()=>{rn(e,n)}),d&&ke(e.id)}async function ke(e){const{data:t}=await _.from("players").select("*, clubs(encoded_name, logo_url, kit_style, kit_color1, kit_color2, kit_color3, kit_shorts, kit_socks)").eq("club_id",e).order("surname_real"),n=document.getElementById("club-players-grid"),i=document.getElementById("club-players-count");if(i&&(i.textContent=(t||[]).length),!!n){if(!(t!=null&&t.length)){n.innerHTML='<div style="color:var(--tile-fg-dim);padding:20px;font-size:13px">Aucun joueur dans ce club.</div>';return}n.innerHTML=t.map(d=>_e(d,{width:100})).join("")}}function an(e,t,n){const{openModal:i,closeModal:d,toast:s}=t;i(`⚽ Générer — ${e.real_name}`,`<div style="display:flex;flex-direction:column;gap:14px">
    ...
    </div>`),setTimeout(()=>{var g,v;(g=document.getElementById("qg-cancel"))==null||g.addEventListener("click",()=>d()),(v=document.getElementById("qg-ok"))==null||v.addEventListener("click",async()=>{var o;const E=((o=document.getElementById("qg-strong"))==null?void 0:o.checked)??!1;d(),s("Génération en cours…","info"),await dt(e.id,e.country_code,s,E),s("Effectif généré ✅","success"),ke(e.id),me(n,t)})},50)}const ut=["GK","DEF","MIL","ATT"],on=[{value:"normal",label:"Normal"},{value:"pepite",label:"Pépite"},{value:"papyte",label:"Papyte"},{value:"legende",label:"Légende"}];function rn(e,t){const{openModal:n,closeModal:i,toast:d}=t,s=$e.map(o=>`<option value="${o}">${o}</option>`).join(""),g=o=>ut.map(l=>`<option value="${l}" ${l===o?"selected":""}>${l}</option>`).join(""),v=`
  ...
  `),setTimeout(()=>{var o,l;document.querySelectorAll('input[name="gen-one-mode"]').forEach(a=>{a.addEventListener("change",()=>{document.getElementById("gen-one-custom-fields").style.display=a.value==="custom"&&a.checked?"flex":"none"})}),(o=document.getElementById("gen-one-cancel"))==null||o.addEventListener("click",()=>i()),(l=document.getElementById("gen-one-ok"))==null||l.addEventListener("click",async()=>{var c,f,p,y,k;const a=((c=document.querySelector('input[name="gen-one-mode"]:checked'))==null?void 0:c.value)||"random",r=document.getElementById("gen-one-ok");r.disabled=!0,r.textContent="⏳ Génération…";const u={};a==="custom"&&(u.job=((f=document.getElementById("gen-one-job1"))==null?void 0:f.value)||null,u.job2=((p=document.getElementById("gen-one-job2"))==null?void 0:p.value)||null,u.rarity=((y=document.getElementById("gen-one-rarity"))==null?void 0:y.value)||null,u.country=((k=document.getElementById("gen-one-country"))==null?void 0:k.value)||null);try{await ln(e,u),d("Joueur généré ✅","success"),i(),ke(e.id)}catch(A){d("Erreur : "+A.message,"error"),r.disabled=!1,r.textContent="➕ Générer"}})},50)}async function ln(e,t={}){const n=t.country||(Math.random()<.5?e.country_code:at(e.country_code)),i=t.job||X(ut),d=t.job2||null,s=t.rarity||(()=>{const L=Math.random()*100;return L<1.5?"legende":L<3?"pepite":L<4.5?"papyte":"normal"})(),g=s==="pepite"?te(10,15):s==="papyte"?te(15,20):ot(),v=d?Math.max(0,g-2):0;let E=0,o=0,l=0,a=0;i==="GK"&&(E=g),i==="DEF"&&(o=g),i==="MIL"&&(l=g),i==="ATT"&&(a=g),d==="GK"&&(E=v),d==="DEF"&&(o=v),d==="MIL"&&(l=v),d==="ATT"&&(a=v);const r=Ie(n),u=it(n),c=X(nt),{data:f}=await _.from("players").select("surname_real").not("surname_real","is",null),p=new Set((f||[]).map(L=>L.surname_real).filter(Boolean)),y=st(n),k=y.filter(L=>!p.has(L)),A=X(k.length?k:y),b={job:i,job2:d,firstname:lt(n),surname_real:A,country_code:n,club_id:e.id,note_g:E,note_d:o,note_m:l,note_a:a,skin:r,hair:u,hair_length:c,rarity:s,sell_price:0,ethnie:ve(n)},{data:$}=await _.from("players").select("face").not("face","is",null),h=new Set(($||[]).map(L=>L.face).filter(Boolean)),x=await we(n,h);x&&(b.face=x);const{data:I,error:m}=await _.from("players").insert(b).select("id").single();if(m)throw m;const{error:w}=await _.from("cards").insert({card_type:"player",player_id:I.id});w&&console.warn("[GenOnePlayer] Erreur carte:",w.message)}function pt(){var e,t,n,i,d,s;return{style:((e=document.getElementById("m-kit-style"))==null?void 0:e.value)||"uni",color1:((t=document.getElementById("m-kit-color1"))==null?void 0:t.value)||"#1A6B3C",color2:((n=document.getElementById("m-kit-color2"))==null?void 0:n.value)||"#ffffff",color3:((i=document.getElementById("m-kit-color3"))==null?void 0:i.value)||"#000000",shorts:((d=document.getElementById("m-kit-shorts"))==null?void 0:d.value)||"#111111",socks:((s=document.getElementById("m-kit-socks"))==null?void 0:s.value)||"#ffffff"}}function ie(){const e=document.getElementById("kit-preview-wrap");e&&(e.innerHTML=It(pt(),"panel"))}async function sn(e,t){const n=(e.name.split(".").pop()||"png").toLowerCase(),i=`clubs/${t||"new"}-${Date.now()}.${n}`,{error:d}=await _.storage.from("assets").upload(i,e,{upsert:!0,cacheControl:"3600"});if(d)throw d;const{data:s}=_.storage.from("assets").getPublicUrl(i);return s.publicUrl}async function dn(e,t,n,i){var A,b,$,h,x,I,m,w,L;const{toast:d}=i,s=document.getElementById("m-error"),g=document.getElementById("m-save"),v=(A=document.getElementById("m-real"))==null?void 0:A.value.trim(),E=(b=document.getElementById("m-encoded"))==null?void 0:b.value.trim().toUpperCase(),o=($=document.getElementById("m-country"))==null?void 0:$.value.trim().toUpperCase(),l=((x=(h=document.getElementById("m-logo-file"))==null?void 0:h.files)==null?void 0:x[0])||null,a=((I=document.getElementById("m-logo-url-current"))==null?void 0:I.value)||null,r=((m=document.getElementById("m-gen-stadium"))==null?void 0:m.checked)??!1,u=((w=document.getElementById("m-gen-squad"))==null?void 0:w.checked)??!1,c=((L=document.getElementById("m-gen-strong"))==null?void 0:L.checked)??!1,f=pt();if(!v){s.textContent="Le nom du club est requis.";return}if(!E){s.textContent="Le nom encodé est requis.";return}if(!o){s.textContent="Le pays est requis.";return}g.disabled=!0,g.textContent="Enregistrement…";let p=a;if(l)try{g.textContent="📤 Envoi du logo…",p=await sn(l,e==null?void 0:e.id)}catch(S){s.textContent="Erreur upload logo : "+S.message,g.disabled=!1,g.textContent=t?"💾 Enregistrer":"✅ Créer le club";return}const y={real_name:v,encoded_name:E,country_code:o,logo_url:p,kit_style:f.style,kit_color1:f.color1,kit_color2:f.color2,kit_color3:f.color3,kit_shorts:f.shorts,kit_socks:f.socks};let k=e==null?void 0:e.id;if(t){const{error:S}=await _.from("clubs").update(y).eq("id",k);if(S){s.textContent=S.message,g.disabled=!1,g.textContent="💾 Enregistrer";return}}else{const{data:S,error:C}=await _.from("clubs").insert(y).select().single();if(C){s.textContent=C.message,g.disabled=!1,g.textContent="✅ Créer le club";return}if(k=S.id,r){g.textContent="🏟️ Création du stade…";const{data:D,error:j}=await _.from("stadium_definitions").insert({name:`Stade de ${v}`,club_id:k,country_code:null}).select().single();j?console.warn("[Stadium] Erreur def stade:",j.message):D&&await _.from("cards").insert({card_type:"stadium",stadium_id:D.id})}u&&(g.textContent="⚽ Génération des joueurs…",await dt(k,o,d,c))}d(t?"Club modifié ✅":"Club créé ✅","success"),me(n,i)}const cn=["normal","pepite","papyte","legende"],Ne=["GK","DEF","MIL","ATT"],un=[["MA","Maroc"],["FR","France"],["AR","Argentine"],["PT","Portugal"],["BR","Brésil"],["ES","Espagne"],["DE","Allemagne"],["GB","Angleterre"],["IT","Italie"],["CM","Cameroun"],["SN","Sénégal"],["NG","Nigéria"],["DK","Danemark"],["NL","Pays-Bas"],["BE","Belgique"],["CI","Côte d'Ivoire"],["AL","Albanie"],["HR","Croatie"],["RS","Serbie"],["TR","Turquie"],["MW","Malawi"]];let ee={...O},le=[];async function pn(e,{toast:t}){const{data:n}=await _.from("clubs").select("id,encoded_name,logo_url,kit_style,kit_color1,kit_color2,kit_shorts,kit_socks").order("encoded_name");le=n||[],ee={...O},e.innerHTML=yn(le),gn(e,le,t),K()}function Pe(){var n;const e=((n=document.getElementById("cb-surname-real"))==null?void 0:n.value)||"",t=document.getElementById("cb-surname-enc");t&&(t.value=pe(e))}function mn(){var n;const e=(n=document.getElementById("cb-club"))==null?void 0:n.value,t=le.find(i=>String(i.id)===String(e));return t?{style:t.kit_style||F.style,color1:t.kit_color1||F.color1,color2:t.kit_color2||F.color2,shorts:t.kit_shorts||F.shorts,socks:t.kit_socks||F.socks}:{...F}}function K(){var r,u,c,f;const e=p=>{var y;return((y=document.getElementById(p))==null?void 0:y.value)||""},t={firstname:e("cb-firstname"),surname_real:e("cb-surname-enc")||pe(e("cb-surname-real")),country_code:e("cb-country"),club_encoded_name:((u=(r=document.getElementById("cb-club"))==null?void 0:r.selectedOptions[0])==null?void 0:u.text)||"",job:e("cb-job"),job2:e("cb-job2")||null,note_g:parseInt(e("cb-note-g"))||0,note_d:parseInt(e("cb-note-d"))||0,note_m:parseInt(e("cb-note-m"))||0,note_a:parseInt(e("cb-note-a"))||0,rarity:e("cb-rarity")},n=mn(),i=document.getElementById("cb-club"),d=((f=(c=i==null?void 0:i.selectedOptions[0])==null?void 0:c.dataset)==null?void 0:f.logo)||null,s=Qe(ee,n,120),g=`data:image/svg+xml;charset=utf-8,${encodeURIComponent(s)}`,v=At(t,{portraitUrl:g,clubLogoUrl:d,showNotes:!0}),E=document.getElementById("card-preview");E&&(E.innerHTML=v);const o=document.getElementById("avatar-preview-wrap");o&&(o.innerHTML=s);const l=e("cb-surname-real"),a=document.getElementById("encode-summary");a&&l&&(a.textContent=`${e("cb-firstname")} ${l} → ${e("cb-firstname")} ${t.surname_real}`)}function mt(){const e=t=>{var n;return((n=document.getElementById(t))==null?void 0:n.value)||""};ee={skin:e("av-skin"),hair:e("av-hair"),hairColor:e("av-hairColor"),eyes:e("av-eyes"),nose:e("av-nose"),mouth:e("av-mouth"),eyebrows:e("av-eyebrows"),beard:e("av-beard")}}function gn(e,t,n){var d,s,g,v,E;e.querySelectorAll("input,select").forEach(o=>{o.addEventListener("input",K),o.addEventListener("change",K)});const i=["av-skin","av-hair","av-hairColor","av-eyes","av-nose","av-mouth","av-eyebrows","av-beard"];i.forEach(o=>{var l;(l=document.getElementById(o))==null||l.addEventListener("change",()=>{mt(),K()})}),(d=document.getElementById("cb-surname-real"))==null||d.addEventListener("input",()=>{Pe(),K()}),(s=document.getElementById("btn-encode"))==null||s.addEventListener("click",()=>{Pe(),K()}),(g=document.getElementById("cb-club"))==null||g.addEventListener("change",K),(v=document.getElementById("btn-save-player"))==null||v.addEventListener("click",()=>fn(n)),(E=document.getElementById("btn-reset"))==null||E.addEventListener("click",()=>{e.querySelectorAll("input").forEach(o=>o.value=""),e.querySelectorAll("select").forEach(o=>o.selectedIndex=0),ee={...O},i.forEach(o=>{const l=o.replace("av-",""),a=document.getElementById(o);a&&(a.value=O[l]||"")}),K()})}async function fn(e){const t=g=>{var v;return((v=document.getElementById(g))==null?void 0:v.value)||""},n=t("cb-surname-real").trim(),i=t("cb-surname-enc").trim()||pe(n);if(!t("cb-firstname")||!n||!t("cb-country")||!t("cb-job")){e("Remplissez les champs obligatoires (prénom, nom, pays, poste)","error");return}mt();const d={firstname:t("cb-firstname").trim(),surname_real:n,surname_real:i,country_code:t("cb-country"),club_id:t("cb-club")||null,job:t("cb-job"),job2:t("cb-job2")||null,note_g:parseInt(t("cb-note-g"))||0,note_d:parseInt(t("cb-note-d"))||0,note_m:parseInt(t("cb-note-m"))||0,note_a:parseInt(t("cb-note-a"))||0,rarity:t("cb-rarity")||"normal",note_min:parseInt(t("cb-note-min"))||null,note_max:parseInt(t("cb-note-max"))||null,sell_price:parseInt(t("cb-price"))||0,avatar_config:ee,skin:ee.skin||"blanc",hair:ee.hairColor||"marron",hair_length:"court"},{error:s}=await _.from("players").insert(d);if(s){e("Erreur: "+s.message,"error");return}e(`✅ Joueur "${d.firstname} ${d.surname_real}" enregistré !`,"success")}function yn(e){const t=un.map(([o,l])=>`<option value="${o}">${l} (${o})</option>`).join(""),n='<option value="">— Sélectionner un club —</option>'+e.map(o=>`<option value="${o.id}" data-logo="${o.logo_url||""}">${o.encoded_name}</option>`).join(""),i=Ne.map(o=>`<option value="${o}">${o}</option>`).join(""),d='<option value="">Aucun</option>'+Ne.map(o=>`<option value="${o}">${o}</option>`).join(""),s=cn.map(o=>`<option value="${o}">${o.charAt(0).toUpperCase()+o.slice(1)}</option>`).join(""),g=(o,l,a)=>Object.entries(l).map(([r,u])=>`<option value="${r}" ${r===(a||"")?"selected":""}>${u.label}</option>`).join(""),v=Object.keys(kt).map(o=>`<option value="${o}" ${o===O.skin?"selected":""}>${o.charAt(0).toUpperCase()+o.slice(1)}</option>`).join(""),E=Object.keys(Lt).map(o=>`<option value="${o}" ${o===O.hairColor?"selected":""}>${o.charAt(0).toUpperCase()+o.slice(1)}</option>`).join("");return`
  ...
  `}async function gt(e,{toast:t}){const[{data:n,error:i},{data:d},{data:s}]=await Promise.all([_.from("users").select("id,pseudo,club_name,credits,level,wins,draws,losses,trophies_top1,trophies_top2,trophies_top3,is_admin,created_at,mmr,mmr_deviation,rank_tier,placement_matches,ranked_wins,ranked_losses,ranked_draws").order("created_at",{ascending:!1}),_.from("user_solo_progress").select("user_id, unlocked_level"),_.rpc("admin_get_user_emails")]);if(i){e.innerHTML=`<p style="color:red">${i.message}</p>`;return}const g={};(d||[]).forEach(l=>{g[l.user_id]=l.unlocked_level});const v={};(s||[]).forEach(l=>{v[l.id]=l.email});const E=n||[];e.innerHTML=`
  ...
  `,o(E),document.getElementById("search-users").addEventListener("input",l=>{const a=l.target.value.toLowerCase();o(E.filter(r=>{var u;return r.pseudo.toLowerCase().includes(a)||((u=r.club_name)==null?void 0:u.toLowerCase().includes(a))}))});function o(l){document.getElementById("users-tbody").innerHTML=l.map(a=>{const r=a.mmr??1e3,u=a.mmr_deviation??350,c=a.placement_matches??0,f=Dt(r),p=a.ranked_wins??0,y=a.ranked_losses??0,k=a.ranked_draws??0,A=p+y+k,b=A>0?Math.round(p/A*100):null,$=c<10,h=u<80?"#1A6B3C":u<150?"#e67e22":"#bb2020";return`
        <tr>
          ...
          <td style="font-size:12px">
            ...
            ${$?`<div style="font-size:10px;color:#e67e22">${c}/10 placement${c<10?` (${10-c} restants)`:""}</div>`:`<div style="font-size:10px;color:var(--gray-600)">${p}V ${k}N ${y}D${b!==null?` · ${b}% WR`:""}</div>`}
          </td>
    ...
    `).order("listed_at",{ascending:!1}).limit(100);if(d){e.innerHTML=`<p style="color:var(--danger)">${d.message}</p>`;return}const s=i||[],g=s.filter(o=>o.status==="active").length,v=s.filter(o=>o.status==="sold").length;e.innerHTML=`
...
`).replace(/\r/g,`
`);const t=e.split(`
`).filter(g=>g.trim());if(t.length<2)return[];const n=t[0],i=n.split(";").length>n.split(",").length?";":",",d=Ue(n,i).map(g=>g.trim().replace(/^\uFEFF/,"").toLowerCase()),s=[];for(let g=1;g<t.length;g++){if(!t[g].trim())continue;const v=Ue(t[g],i),E={};d.forEach((o,l)=>{E[o]=(v[l]||"").trim()}),!Object.values(E).every(o=>!o)&&s.push(E)}return s}function Ue(e,t=","){const n=[];let i="",d=!1;for(let s=0;s<e.length;s++){const g=e[s];d?g==='"'?e[s+1]==='"'?(i+='"',s++):d=!1:i+=g:g==='"'?d=!0:g===t?(n.push(i),i=""):i+=g}return n.push(i),n}const En={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};async function _n(e,{toast:t}){const n=Object.keys(ge);let i=n[0];const{data:d}=await _.from("formation_links_overrides").select("formation, links"),s={};(d||[]).forEach(l=>{s[l.formation]=l.links});let g=new Set;function v(l,a){return[l,a].sort().join("-")}function E(l){const a=s[l]||ge[l]||[];g=new Set(a.map(([r,u])=>v(r,u)))}E(i),e.innerHTML=`
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;gap:12px;flex-wrap:wrap">
  ...
  `;function o(){const l=jt[i]||{},a=Re(i),r=320,u=400,c=22;function f(b){const $=l[b];return $?{x:$.x*r,y:$.y*u}:null}let p=`<svg width="${r}" height="${u}" viewBox="0 0 ${r} ${u}" xmlns="http://www.w3.org/2000/svg" style="display:block">`;a.forEach(([b,$],h)=>{const x=f(b),I=f($);if(!x||!I)return;const m=v(b,$),w=g.has(m),L=w?"#3b82f6":"#999",S=w?.95:.35,C=w?4:3;p+=`<line x1="${x.x}" y1="${x.y}" x2="${I.x}" y2="${I.y}"
        ...
        opacity="${S}" pointer-events="none" data-line-key="${m}"/>`});for(const b of Object.keys(l)){const $=f(b);if(!$)continue;const h=b.replace(/\d+/,""),x=En[h]||"#555";p+=`<circle cx="${$.x}" cy="${$.y}" r="${c}" fill="${x}" stroke="rgba(255,255,255,0.7)" stroke-width="2" pointer-events="none"/>`,p+=`<text x="${$.x}" y="${$.y+4}" text-anchor="middle" font-size="10" font-weight="900" fill="white" font-family="Arial Black,Arial" pointer-events="none">${b}</text>`}p+="</svg>",document.getElementById("field-wrap").innerHTML=p;const y=document.getElementById("links-list");y.innerHTML=a.map(([b,$])=>{const h=v(b,$),x=g.has(h);return`
        ...
        </button>`}).join("");const k=e.querySelector('div[style*="font-weight:700;font-size:13px;margin-bottom:8px"]');k&&(k.textContent=`Liens (${g.size}/${a.length} actifs)`);function A(b){g.has(b)?g.delete(b):g.add(b),o()}e.querySelectorAll(".link-hit").forEach(b=>{b.addEventListener("click",()=>A(b.dataset.key))}),e.querySelectorAll(".link-toggle").forEach(b=>{b.addEventListener("click",()=>A(b.dataset.key))})}o(),document.getElementById("formation-select").addEventListener("change",l=>{i=l.target.value,E(i),o()}),document.getElementById("reset-btn").addEventListener("click",()=>{const l=ge[i]||[];g=new Set(l.map(([a,r])=>v(a,r))),o(),t("Liens réinitialisés aux valeurs par défaut (non sauvegardé)","info")}),document.getElementById("save-btn").addEventListener("click",async()=>{const a=Re(i).filter(([u,c])=>g.has(v(u,c))),{error:r}=await _.from("formation_links_overrides").upsert({formation:i,links:a,updated_at:new Date().toISOString()});if(r){t(r.message,"error");return}s[i]=a,t(`Liens enregistrés pour ${i} (${a.length} actifs)`,"success")})}const wn=[{value:"player",label:"Joueur"},{value:"formation",label:"Formation"},{value:"game_changer",label:"Game Changer"},{value:"ultra_game_changer",label:"Ultra Game Changer"},{value:"game_helper",label:"Game Helper"},{value:"stadium",label:"🏟️ Stade"}],$n=[{value:"",label:"Toutes"},{value:"normal",label:"Normal"},{value:"pepite",label:"Pépite"},{value:"papyte",label:"Papyte"},{value:"legende",label:"Légende"}],In=[{value:"credits",label:"Crédits"},{value:"pub",label:"Publicité"},{value:"free",label:"Gratuit"}];async function kn(e){e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">Chargement...</div>';const{data:t}=await _.from("booster_configs").select("*").order("sort_order");let n=null,i=[],d=null;const s=()=>window.innerWidth<700;async function g(){if(!n){i=[];return}if(n!==d){const{data:a}=await _.from("booster_drop_rates").select("*").eq("booster_id",n).order("sort_order");i=a||[],d=n}}function v(){return`
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
          <div>
            ...
            <select id="f-price-type" style="width:100%;padding:9px;border:1px solid #ddd;border-radius:6px;font-size:13px">
              ${In.map(c=>`<option value="${c.value}" ${a.price_type===c.value?"selected":""}>${c.label}</option>`).join("")}
            </select>
          </div>
        ...
        ${i.length===0?'<div style="text-align:center;padding:16px;color:#aaa;font-size:13px">Aucun taux. Cliquez + Ligne.</div>':i.map((c,f)=>`
          <div style="display:grid;grid-template-columns:2fr 1.5fr 50px 50px 60px 32px;gap:4px;align-items:center;margin-bottom:6px;padding:6px;background:#f9f9f9;border-radius:6px" data-rate-idx="${f}">
            ...
            <select class="rate-rarity" data-idx="${f}" style="padding:6px 4px;border:1px solid #ddd;border-radius:4px;font-size:12px;width:100%">
              ${$n.map(p=>`<option value="${p.value}" ${(c.rarity||"")===p.value?"selected":""}>${p.label}</option>`).join("")}
            </select>
    ...
    <div style="display:flex;height:calc(100vh - 56px);overflow:hidden">
      ${!s()||r||!n?v():""}
      ${!s()||u?E():""}
    </div>`,l()}function l(){var u,c,f,p,y,k,A;const a=b=>e.querySelector(b);e.querySelectorAll(".booster-row").forEach(b=>{b.addEventListener("click",$=>{$.target.closest(".btn-delete")||(n=b.dataset.id,d=null,o())})}),(u=a("#btn-back"))==null||u.addEventListener("click",()=>{n=null,o()}),(c=a("#btn-new"))==null||c.addEventListener("click",async()=>{const b=prompt("Nom du nouveau booster :");if(!(b!=null&&b.trim()))return;const{data:$,error:h}=await _.from("booster_configs").insert({name:b.trim(),price_type:"credits",price_credits:1e3,card_count:5}).select().single();if(h){alert(h.message);return}t.push($),n=$.id,d=null,o()}),e.querySelectorAll(".btn-delete").forEach(b=>{b.addEventListener("click",async $=>{if($.stopPropagation(),!confirm("Supprimer ce booster ?"))return;await _.from("booster_configs").delete().eq("id",b.dataset.id);const h=t.findIndex(x=>x.id===b.dataset.id);h>-1&&t.splice(h,1),n===b.dataset.id&&(n=null,d=null),o()})}),(f=a("#btn-cancel"))==null||f.addEventListener("click",()=>{n=null,o()}),(p=a("#f-price-type"))==null||p.addEventListener("change",b=>{const $=a("#credits-field");$&&($.style.opacity=b.target.value!=="credits"?"0.4":"1")}),(y=a("#btn-pick-icon"))==null||y.addEventListener("click",async()=>{var $;const b=a("#icon-picker-grid");if(b){if(b.style.display!=="none"){b.style.display="none";return}b.innerHTML='<div style="padding:10px;text-align:center;color:#888;font-size:12px">⏳ Chargement...</div>',b.style.display="block";try{const x=await(await fetch("https://api.github.com/repos/iamtsuba/manager-wars/contents/public/icons")).json(),I=Array.isArray(x)?x.filter(w=>w.name.startsWith("booster")&&/\.(png|jpg|svg|webp)$/i.test(w.name)):[];if(!I.length){b.innerHTML='<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Aucune icône "booster*" trouvée.</div>';return}const m=(($=a("#f-image-url"))==null?void 0:$.value)||"";b.innerHTML=`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(72px,1fr));gap:8px">
          ${I.map(w=>`
        ...
        </div>`,b.querySelectorAll(".icon-pick-item").forEach(w=>{w.addEventListener("click",()=>{const L=a("#f-image-url");L&&(L.value=w.dataset.name),b.style.display="none"})})}catch(h){b.innerHTML=`<div style="padding:10px;text-align:center;color:#e74c3c;font-size:12px">Erreur API GitHub : ${h.message}</div>`}}});function r(){e.querySelectorAll("[data-rate-idx]").forEach(b=>{var h,x,I,m,w;const $=Number(b.dataset.rateIdx);$>=0&&$<i.length&&(i[$].card_type=((h=b.querySelector(".rate-type"))==null?void 0:h.value)||"player",i[$].rarity=((x=b.querySelector(".rate-rarity"))==null?void 0:x.value)||null,i[$].note_min=Number((I=b.querySelector(".rate-note-min"))==null?void 0:I.value)||null,i[$].note_max=Number((m=b.querySelector(".rate-note-max"))==null?void 0:m.value)||null,i[$].percentage=Number((w=b.querySelector(".rate-pct"))==null?void 0:w.value)||0)})}(k=a("#btn-add-rate"))==null||k.addEventListener("click",()=>{r(),i.push({id:null,booster_id:n,card_type:"player",rarity:"normal",note_min:null,note_max:null,percentage:10,sort_order:i.length}),o(!0)}),e.querySelectorAll(".btn-del-rate").forEach(b=>{b.addEventListener("click",()=>{r(),i.splice(Number(b.dataset.idx),1),o(!0)})}),(A=a("#btn-save"))==null||A.addEventListener("click",async()=>{var m,w,L,S,C,D,j,U,q,H,B,T,z;const b=(t||[]).find(M=>M.id===n);if(!b)return;const $=[];e.querySelectorAll("[data-rate-idx]").forEach(M=>{var G,V,J,Y,W;const R=Number(M.dataset.rateIdx);$.push({booster_id:n,card_type:((G=M.querySelector(".rate-type"))==null?void 0:G.value)||"player",rarity:((V=M.querySelector(".rate-rarity"))==null?void 0:V.value)||null,note_min:Number((J=M.querySelector(".rate-note-min"))==null?void 0:J.value)||null,note_max:Number((Y=M.querySelector(".rate-note-max"))==null?void 0:Y.value)||null,percentage:Number((W=M.querySelector(".rate-pct"))==null?void 0:W.value)||0,sort_order:R});const N=$[$.length-1];N.rarity||(N.rarity=null),N.note_min||(N.note_min=null),N.note_max||(N.note_max=null)});const h=$.reduce((M,R)=>M+R.percentage,0);if($.length&&Math.abs(h-100)>.5){alert(`La somme doit faire 100% (actuellement ${h.toFixed(1)}%)`);return}const x={name:((w=(m=a("#f-name"))==null?void 0:m.value)==null?void 0:w.trim())||b.name,image_url:((S=(L=a("#f-image-url"))==null?void 0:L.value)==null?void 0:S.trim())||null,price_type:(C=a("#f-price-type"))==null?void 0:C.value,price_credits:Number((D=a("#f-price-credits"))==null?void 0:D.value)||0,card_count:Number((j=a("#f-card-count"))==null?void 0:j.value)||5,is_active:((U=a("#f-active"))==null?void 0:U.checked)??b.is_active,allow_duplicates:((q=a("#f-allow-dup"))==null?void 0:q.checked)??!0,sort_order:Number((H=a("#f-sort"))==null?void 0:H.value)||0,max_per_user:(B=a("#f-max-per-user"))!=null&&B.value?Number(a("#f-max-per-user").value):null,available_from:((T=a("#f-available-from"))==null?void 0:T.value)||null,available_until:((z=a("#f-available-until"))==null?void 0:z.value)||null},{error:I}=await _.from("booster_configs").update(x).eq("id",n);if(I){alert(I.message);return}if(Object.assign(b,x),await _.from("booster_drop_rates").delete().eq("booster_id",n),$.length){const{error:M}=await _.from("booster_drop_rates").insert($);if(M){alert(M.message);return}}i=$,d=n,alert("✅ Booster enregistré !"),o(!0)})}o()}const yt=[{value:"normal",label:"Normal",color:"#888"},{value:"pepite",label:"Pépite",color:"#D4A017"},{value:"papyte",label:"Papyte",color:"#909090"},{value:"legende",label:"Légende",color:"#7a28b8"}];async function Ln(e){await Le(e)}async function Le(e){const{data:t,error:n}=await _.from("sell_price_configs").select("*").order("rarity").order("note_min");if(n){e.innerHTML=`<div style="padding:24px;color:red">Erreur : ${n.message}</div>`;return}e.innerHTML=`
    ...
    </tr>`}function qe(e){document.getElementById("config-form").style.display="block",document.getElementById("form-title").textContent=e?"Modifier la règle":"Nouvelle règle",document.getElementById("form-id").value=(e==null?void 0:e.id)||"",document.getElementById("form-rarity").value=(e==null?void 0:e.rarity)||"normal",document.getElementById("form-price").value=(e==null?void 0:e.price)||1e3,document.getElementById("form-note-min").value=(e==null?void 0:e.note_min)??1,document.getElementById("form-note-max").value=(e==null?void 0:e.note_max)??10,document.getElementById("config-form").scrollIntoView({behavior:"smooth"})}async function An(e){const t=document.getElementById("form-id").value,n=document.getElementById("form-rarity").value,i=parseInt(document.getElementById("form-price").value)||0,d=parseInt(document.getElementById("form-note-min").value)||1,s=parseInt(document.getElementById("form-note-max").value)||10;if(d>s){alert("Note min doit être ≤ note max");return}const g={rarity:n,price:i,note_min:d,note_max:s,updated_at:new Date().toISOString()};let v;if(t?{error:v}=await _.from("sell_price_configs").update(g).eq("id",t):{error:v}=await _.from("sell_price_configs").insert(g),v){alert("Erreur : "+v.message);return}document.getElementById("config-form").style.display="none",await Le(e)}async function Sn(e){await Be(e)}async function Be(e){const{data:t}=await _.from("patch_notes").select("*").order("published_at",{ascending:!1});e.innerHTML=`
  ...
  <div data-edit-row="${e.id}" style="background:#fff;border-radius:10px;padding:12px 16px;box-shadow:0 1px 4px rgba(0,0,0,0.08);display:flex;align-items:center;gap:12px;cursor:pointer">
  ...
  </div>`}function Ge(e,t=0){document.getElementById("ts-form-empty").style.display="none",document.getElementById("ts-form-content").style.display="block",document.getElementById("ts-form-title").textContent=e?"Modifier l'étape":"Nouvelle étape",document.getElementById("ts-id").value=(e==null?void 0:e.id)||"",document.getElementById("ts-emoji").value=(e==null?void 0:e.emoji)||"⚽",document.getElementById("ts-title").value=(e==null?void 0:e.title)||"",document.getElementById("ts-content").value=(e==null?void 0:e.content)||"";const n=document.getElementById("ts-editor");n&&(n.innerHTML=(e==null?void 0:e.content)||"<p>Texte de l'étape…</p>"),document.getElementById("ts-order").value=(e==null?void 0:e.step_order)??t,document.getElementById("ts-active").checked=e?e.is_active:!0;const i=(e==null?void 0:e.color)||"#1A6B3C";document.getElementById("ts-color").value=i,document.getElementById("ts-color-hex").value=i;const d=(e==null?void 0:e.image_url)||"",s=document.getElementById("ts-image");s&&(s.value=d);const g=document.getElementById("ts-img-preview"),v=document.getElementById("ts-img-preview-el");if(d&&g&&v){const E="/";v.src=`${E}icons/${d}`,g.style.display="block"}else g&&(g.style.display="none");document.getElementById("ts-img-picker-grid").style.display="none",document.getElementById("ts-preview-area").innerHTML="",document.getElementById("ts-form").scrollIntoView({behavior:"smooth"})}function bt(){document.getElementById("ts-form-content").style.display="none",document.getElementById("ts-form-empty").style.display="block",document.getElementById("ts-preview-area").innerHTML=""}function Dn(){var o,l;const e=document.getElementById("ts-emoji").value||"⚽",t=document.getElementById("ts-title").value||"Titre",n=document.getElementById("ts-editor"),i=n?n.innerHTML:document.getElementById("ts-content").value||"",d=document.getElementById("ts-color-hex").value||"#1A6B3C",s=(l=(o=document.getElementById("ts-image"))==null?void 0:o.value)==null?void 0:l.trim(),v=s?`<div style="padding:0 20px 12px;text-align:center"><img src="/icons/${s}" style="max-height:140px;max-width:100%;border-radius:10px;object-fit:contain"></div>`:"",E=document.getElementById("ts-preview-area");E.innerHTML=`
    ...
    </div>`,(d=document.getElementById("st-add-btn"))==null||d.addEventListener("click",()=>Ke(null,i,e,t)),(s=document.getElementById("st-gen-countries-btn"))==null||s.addEventListener("click",async()=>{const{toast:g}=t,v=new Set((n||[]).filter(r=>!r.club_id&&r.country_code).map(r=>r.country_code)),E=xt.filter(([r])=>!v.has(r));if(!E.length){g("Tous les pays ont déjà un stade.","info");return}if(!confirm(`Créer ${E.length} stade(s) "Pays" manquant(s) ?`))return;const o=E.map(([r,u])=>({name:`Stade ${u}`,club_id:null,country_code:r})),{data:l,error:a}=await _.from("stadium_definitions").insert(o).select();if(a){g("Erreur : "+a.message,"error");return}l!=null&&l.length&&await _.from("cards").insert(l.map(r=>({card_type:"stadium",stadium_id:r.id}))),g(`${(l==null?void 0:l.length)||0} stade(s) Pays créé(s) ✅`,"success"),ce(e,t)}),e.querySelectorAll("[data-edit-stadium]").forEach(g=>{g.addEventListener("click",()=>{const v=(n||[]).find(E=>E.id===g.dataset.editStadium);v&&Ke(v,i,e,t)})})}function Pn(e){var d,s;let t=null;e.image_url?t=e.image_url.startsWith("http")?e.image_url:`${vt}icons/${e.image_url}`:(d=e.club)!=null&&d.logo_url?t=e.club.logo_url:e.country_code&&(t=`https://flagsapi.com/${e.country_code.slice(0,2).toUpperCase()}/flat/64.png`);const n=((s=e.club)==null?void 0:s.encoded_name)||e.country_code||"—",i=Xe(e.name,t,`${n}<br>+10 ⭐`,{width:140});return`<div data-edit-stadium="${e.id}" style="cursor:pointer;transition:transform .15s" onmouseover="this.style.transform='translateY(-3px)'" onmouseout="this.style.transform=''">
    ${i}
  </div>`}function Ke(e,t,n,i){var l,a,r;const{openModal:d,closeModal:s,toast:g}=i,v=`
  ...
  `);const o=()=>{var x,I,m,w,L,S,C;const u=((x=document.getElementById("st-name"))==null?void 0:x.value)||"NOM DU STADE",c=(m=(I=document.getElementById("st-image"))==null?void 0:I.value)==null?void 0:m.trim(),f=(L=(w=document.getElementById("st-country"))==null?void 0:w.value)==null?void 0:L.trim(),p=document.getElementById("st-club"),y=(p==null?void 0:p.selectedIndex)||0,k=p&&y>0?p.options[y].text:"",A=((C=(S=p==null?void 0:p.options[y])==null?void 0:S.getAttribute)==null?void 0:C.call(S,"data-logo"))||"";let b=null;c?b=c.startsWith("http")?c:`${vt}icons/${c}`:A?b=A:f&&(b=`https://flagsapi.com/${f.toLowerCase().slice(0,2).toUpperCase()}/flat/64.png`);const $=k||f||"—",h=document.getElementById("st-preview-card");h&&(h.innerHTML=Xe(u,b,`${$}<br>+10 ⭐`,{width:140}))};o(),["st-name","st-club","st-country","st-image"].forEach(u=>{var c,f;(c=document.getElementById(u))==null||c.addEventListener("input",o),(f=document.getElementById(u))==null||f.addEventListener("change",o)}),(l=document.getElementById("st-cancel"))==null||l.addEventListener("click",()=>s()),(a=document.getElementById("st-save"))==null||a.addEventListener("click",async()=>{const u=document.getElementById("st-name").value.trim(),c=document.getElementById("st-club").value||null,f=document.getElementById("st-country").value.trim().toUpperCase()||null,p=document.getElementById("st-image").value.trim()||null;if(!u){g("Le nom est obligatoire","error");return}const y={name:u,club_id:c,country_code:f,image_url:p},{error:k}=e?await _.from("stadium_definitions").update(y).eq("id",e.id):await _.from("stadium_definitions").insert(y);if(k){g("Erreur : "+k.message,"error");return}g(e?"Stade modifié ✅":"Stade créé ✅","success"),s(),ce(n,i)}),(r=document.getElementById("st-delete"))==null||r.addEventListener("click",async()=>{confirm("Supprimer ce stade ?")&&(await _.from("stadium_definitions").delete().eq("id",e.id),g("Stade supprimé","success"),s(),ce(n,i))})}const Q=[{value:"game_changer",label:"⚡ Game Changer",border:"#7a28b8",bg:"linear-gradient(135deg,#4a0080,#7a28b8)"},{value:"ultra_game_changer",label:"💎 Ultra Game Changer",border:"#00bcd4",bg:"linear-gradient(135deg,#006080,#00bcd4)"}],Fn=[{value:"purple",label:"Violet",hex:"#7a28b8"},{value:"light_blue",label:"Bleu clair",hex:"#00bcd4"}],Z=[{value:"BOOST_STAT",label:"⚡ Boost stats (+N)",hasValue:!0,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"DEBUFF_STAT",label:"💀 Debuff stats (-N)",hasValue:!0,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"GRAY_PLAYER",label:"❄️ Griser un joueur",hasValue:!1,hasTarget:!0,hasCount:!0,hasRoles:!0},{value:"REVIVE_PLAYER",label:"💫 Ressusciter un joueur",hasValue:!1,hasTarget:!1,hasCount:!0,hasRoles:!1},{value:"REMOVE_GOAL",label:"🚫 Retirer un but adverse",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1},{value:"ADD_GOAL_DRAW",label:"⚽ +1 but si duel nul",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1},{value:"ADD_SUB",label:"🔄 Ajouter un remplacement",hasValue:!1,hasTarget:!1,hasCount:!0,hasRoles:!1},{value:"CUSTOM",label:"🛠️ Effet custom (hardcodé)",hasValue:!1,hasTarget:!1,hasCount:!1,hasRoles:!1}],Ve=["GK","DEF","MIL","ATT"];async function On(e,t){await Ae(e,t)}async function Ae(e,t){var i;const{data:n}=await _.from("gc_definitions").select("*").order("sort_order").order("created_at");e.innerHTML=`
    ...
    </div>`,(i=document.getElementById("gc-add-btn"))==null||i.addEventListener("click",()=>Je(null,e,t)),e.querySelectorAll("[data-edit-gc]").forEach(d=>{d.addEventListener("click",()=>{const s=(n||[]).find(g=>g.id===d.dataset.editGc);s&&Je(s,e,t)})}),e.querySelectorAll(".btn-del-gc").forEach(d=>{d.addEventListener("click",async s=>{s.stopPropagation(),confirm("Supprimer cette carte Game Changer ?")&&(await _.from("gc_definitions").delete().eq("id",d.dataset.del),Ae(e,t))})})}function Un(e){const t=Q.find(d=>d.value===e.gc_type)||Q[0],n=e.image_url?`/icons/${e.image_url}`:null,i=be(e.name||"?",n,t.label.split(" ")[0],e.effect||"",{width:130});return`<div data-edit-gc="${e.id}" style="position:relative;cursor:pointer">
    ${i}
    <div style="position:absolute;top:4px;left:4px;z-index:10;display:flex;gap:3px">
  ...
  </div>`}function Je(e,t,n){var r,u,c,f;const{openModal:i,closeModal:d,toast:s}=n,g=!e,v=e||{name:"",gc_type:"game_changer",color:"purple",effect:"",image_url:"",sort_order:0,is_active:!0,effect_type:"BOOST_STAT",effect_params:{}},E=(()=>{const p=v.effect_params||{},y=Z.find(k=>k.value===(v.effect_type||"BOOST_STAT"))||Z[0];return`
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
      ${y.hasValue?`<div>
    ...
    <div style="display:flex;flex-direction:column;gap:20px">
      ...
      <div class="card-panel" style="background:linear-gradient(135deg,#0a3d1e,#1a6b3c20);border:1.5px solid #1A6B3C40">
        ...
        <div style="display:flex;flex-direction:column;gap:4px">
          ${c.map(m=>{const w=k[m],L=u.length>0?Math.round(w/u.length*100):0;return`
...
Action irréversible. Continuer ?`))return;m.disabled=!0,m.textContent="⏳ Lancement...";const{data:S,error:C}=await _.rpc("admin_launch_season",{p_season_id:w});if(m.disabled=!1,m.textContent="▶ Lancer la saison",C){n(C.message,"error");return}if(!(S!=null&&S.success)){n((S==null?void 0:S.error)||"Échec du lancement","error");return}n(`Saison lancée ✅ (${S.logged} classement(s) archivé(s), ${S.reset} joueur(s) recalculé(s))`,"success"),P(e,t)})}),e.querySelectorAll("[data-deactivate]").forEach(m=>{m.addEventListener("click",async()=>{if(!confirm("Désactiver la saison active ? Le mode Ranked sera suspendu."))return;const{error:w}=await _.from("ranked_seasons").update({is_active:!1}).eq("id",parseInt(m.dataset.deactivate));if(w){n(w.message,"error");return}n("Saison désactivée","success"),P(e,t)})}),e.querySelectorAll("[data-delete]").forEach(m=>{m.addEventListener("click",async()=>{if(!confirm("Supprimer cette saison archivée ?"))return;const{error:w}=await _.from("ranked_seasons").delete().eq("id",parseInt(m.dataset.delete));if(w){n(w.message,"error");return}n("Saison supprimée","success"),P(e,t)})}),(I=document.getElementById("add-reward-tier-btn"))==null||I.addEventListener("click",()=>{ye(null,l,a,o||[],{toast:n,openModal:i,closeModal:d,reload:()=>P(e,t)})}),e.querySelectorAll("[data-edit-tier]").forEach(m=>{const w=(E||[]).find(L=>L.id===m.dataset.editTier);m.addEventListener("click",()=>{ye(w,l,a,o||[],{toast:n,openModal:i,closeModal:d,reload:()=>P(e,t)})})}),e.querySelectorAll("[data-distribute-tier]").forEach(m=>{m.addEventListener("click",async()=>{const w=(E||[]).find(C=>C.id===m.dataset.distributeTier);if(!confirm(`Distribuer les récompenses du palier "${w==null?void 0:w.label}" à tous les joueurs concernés ?`))return;m.disabled=!0,m.textContent="⏳...";const{data:L,error:S}=await _.rpc("admin_distribute_season_reward",{p_tier_id:m.dataset.distributeTier});if(S){n(S.message,"error"),m.disabled=!1,m.textContent="🎁 Distribuer";return}if(!(L!=null&&L.success)){n((L==null?void 0:L.error)||"Échec de la distribution","error"),m.disabled=!1,m.textContent="🎁 Distribuer";return}n(`Récompenses distribuées à ${L.rewarded} joueur(s) ✅`,"success"),P(e,t)})}),e.querySelectorAll("[data-delete-tier]").forEach(m=>{m.addEventListener("click",async()=>{if(!confirm("Supprimer ce palier de récompense ?"))return;const{error:w}=await _.from("season_reward_tiers").delete().eq("id",m.dataset.deleteTier);if(w){n(w.message,"error");return}n("Palier supprimé","success"),P(e,t)})})}function ye(e,t,n,i,{toast:d,openModal:s,closeModal:g,reload:v}){var f,p,y,k,A;const E=!!e,o=(e==null?void 0:e.season_id)??(n==null?void 0:n.id)??((f=t[0])==null?void 0:f.id)??null;let l=[];const a=b=>{if(!b)return"";const $=new Date(b);return new Date($.getTime()-$.getTimezoneOffset()*6e4).toISOString().slice(0,16)},r=`
  ...
  `);function c(){const b=document.getElementById("rt-player-chips");b&&(b.innerHTML=l.map($=>`
      ...
      </div>`).join(""),h.querySelectorAll("[data-add-player]").forEach(x=>{x.addEventListener("click",()=>{const I=x.dataset.addPlayer;l.some(m=>m.id===I)||l.push({id:I,label:x.dataset.label}),c(),h.style.display="none",document.getElementById("rt-player-search").value=""})})}),(k=document.getElementById("rt-cancel"))==null||k.addEventListener("click",()=>g()),(A=document.getElementById("rt-save"))==null||A.addEventListener("click",async()=>{const b=document.getElementById("rt-error"),$=parseInt(document.getElementById("rt-season").value),h=document.getElementById("rt-label").value.trim(),x=parseInt(document.getElementById("rt-rank-min").value)||1,I=parseInt(document.getElementById("rt-rank-max").value)||1,m=parseInt(document.getElementById("rt-credits").value)||0,w=document.getElementById("rt-activate-at").value,L=w?new Date(w).toISOString():null,S=[...document.querySelectorAll(".rt-booster-cb:checked")].map(j=>j.value);if(!h){b.textContent="Le libellé est obligatoire.";return}if(x>I){b.textContent="Le classement min doit être ≤ au max.";return}if(!$){b.textContent="Choisis une saison.";return}const C={season_id:$,label:h,rank_min:x,rank_max:I,credits:m,booster_config_ids:S,player_ids:l.map(j=>j.id),activate_at:L},{error:D}=E?await _.from("season_reward_tiers").update(C).eq("id",e.id):await _.from("season_reward_tiers").insert(C);if(D){b.textContent=D.message;return}d(E?"Palier modifié ✅":"Palier créé ✅","success"),g(),v()})}function Hn(e,{toast:t,openModal:n,closeModal:i,reload:d}){var l,a;const s=new Date,g=new Date(s.getTime()+30*864e5),v=r=>r.toISOString().slice(0,10),E=`
    <div style="display:flex;flex-direction:column;gap:12px">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
        <div>
          ...
          <input id="sns-start" type="date" value="${v(s)}">
        </div>
        <div>
