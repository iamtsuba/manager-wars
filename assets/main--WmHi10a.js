import{s as T,F as xi,h as Nt,j as Yi,l as at,i as zn,k as Ln,b as Sn}from"./formation-links-DJC25wD7.js";const In="/manager-wars/";function Li(e,{navigate:t,toast:i}){let a="login";const l=()=>{var s,c,p,n,o,f;const d=a==="login";e.innerHTML=`
    <div style="
  ...
  `;function c(){var A;const n=document.getElementById("logo-preview"),o=document.getElementById("logo-initials"),f=((A=document.getElementById("setup-club"))==null?void 0:A.value)||"MW",v=f.trim().split(" ").filter(Boolean),w=v.length>=2?(v[0][0]+v[1][0]).toUpperCase():f.slice(0,2).toUpperCase();n&&(n.style.background=s,n.style.borderColor=d),o&&(o.textContent=w,o.style.color=d)}document.getElementById("color1").addEventListener("input",n=>{d=n.target.value,document.getElementById("swatch1").style.background=d,c()}),document.getElementById("color2").addEventListener("input",n=>{s=n.target.value,document.getElementById("swatch2").style.background=s,c()});function p(n){document.querySelectorAll(".setup-step").forEach(o=>o.classList.remove("active")),document.getElementById(`step-${n}`).classList.add("active"),document.getElementById("step-num").textContent=n,document.getElementById("progress-fill").style.width=`${Math.round(n/3*100)}%`,n===3&&c()}document.getElementById("step1-next").addEventListener("click",async()=>{const n=document.getElementById("setup-pseudo").value.trim(),o=document.getElementById("step1-error");if(o.textContent="",n.length<3){o.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:f}=await T.from("users").select("id").eq("pseudo",n).maybeSingle();if(f){o.textContent="Ce pseudo est déjà pris.";return}p(2)}),document.getElementById("step2-back").addEventListener("click",()=>p(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const n=document.getElementById("setup-club").value.trim(),o=document.getElementById("step2-error");if(o.textContent="",n.length<2){o.textContent="Nom trop court (min. 2 caractères).";return}const{data:f}=await T.from("users").select("id").eq("club_name",n).maybeSingle();if(f){o.textContent="Ce nom de club est déjà pris.";return}p(3)}),document.getElementById("step3-back").addEventListener("click",()=>p(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const n=document.getElementById("setup-pseudo").value.trim(),o=document.getElementById("setup-club").value.trim(),f=document.getElementById("step3-error"),v=document.getElementById("step3-finish");f.textContent="",v.disabled=!0,v.textContent="Création en cours…";try{const{error:w}=await T.from("users").insert({id:t.user.id,pseudo:n,club_name:o,club_color1:d,club_color2:s,credits:1e4});if(w)throw w;await Cn(t.user.id),await l(),a(`Bienvenue ${n} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(w){f.textContent=w.message,v.disabled=!1,v.textContent="🚀 Créer mon profil !"}})}async function Cn(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await T.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const jn="modulepreload",qn=function(e){return"/manager-wars/"+e},Si={},yi=function(t,i,a){let l=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const s=document.querySelector("meta[property=csp-nonce]"),c=(s==null?void 0:s.nonce)||(s==null?void 0:s.getAttribute("nonce"));l=Promise.allSettled(i.map(p=>{if(p=qn(p),p in Si)return;Si[p]=!0;const n=p.endsWith(".css"),o=n?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${p}"]${o}`))return;const f=document.createElement("link");if(f.rel=n?"stylesheet":jn,n||(f.as="script"),f.crossOrigin="",f.href=p,c&&f.setAttribute("nonce",c),document.head.appendChild(f),n)return new Promise((v,w)=>{f.addEventListener("load",v),f.addEventListener("error",()=>w(new Error(`Unable to preload CSS for ${p}`)))})}))}function d(s){const c=new Event("vite:preloadError",{cancelable:!0});if(c.payload=s,window.dispatchEvent(c),!c.defaultPrevented)throw s}return l.then(s=>{for(const c of s||[])c.status==="rejected"&&d(c.reason);return t().catch(d)})},Dt="#1A6B3C",Gt="#cc2222",Fn="#D4A017",Ii="#888";async function Bn(e,t){const{state:i,toast:a}=t;e.innerHTML=`
    ...
    </div>`,await Wi(i,a,t),document.getElementById("btn-add-friend").addEventListener("click",()=>Gn(i,a)),document.getElementById("btn-accept-friend").addEventListener("click",()=>Ji(i,a,null,t))}async function Wi(e,t,i={}){const{navigate:a}=i,l=e.user.id,{data:d,error:s}=await T.from("friendships").select("id, requester_id, addressee_id").eq("status","accepted").or(`requester_id.eq.${l},addressee_id.eq.${l}`),{count:c}=await T.from("friendships").select("id",{count:"exact",head:!0}).eq("addressee_id",l).eq("status","pending"),p=document.getElementById("pending-badge");p&&(c>0?(p.style.display="flex",p.textContent=c):p.style.display="none");const n=document.getElementById("friends-list");if(!n)return;if(s){console.error("[Friends] Erreur:",s),n.innerHTML=`<div style="color:${Gt};text-align:center;padding:16px">Erreur chargement : ${s.message}</div>`;return}const o=(d||[]).map(w=>w.requester_id===l?w.addressee_id:w.requester_id);let f={};if(o.length){const{data:w}=await T.from("users").select("id, pseudo, club_name, last_seen_at, club_color1, club_color2").in("id",o);(w||[]).forEach(A=>{f[A.id]=A})}const v=(d||[]).map(w=>({friendshipId:w.id,friend:f[w.requester_id===l?w.addressee_id:w.requester_id]||{pseudo:"?"}}));if(!v.length){n.innerHTML=`
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      ${v.map(({friendshipId:w,friend:A})=>Dn(A,w)).join("")}
    </div>`,n.querySelectorAll("[data-stats]").forEach(w=>{w.addEventListener("click",()=>Pn(e,w.dataset.stats,w.dataset.friendName))}),n.querySelectorAll("[data-match]").forEach(w=>{w.addEventListener("click",()=>{const A=w.dataset.friendId,r=w.dataset.friendName;console.log("[Friends] clic match",{fid:A,fname:r,hasNavigate:typeof a}),typeof a=="function"?a("match",{matchMode:"friend",friendId:A,friendName:r}):t("Erreur navigation","error")})})}function Dn(e,t){const i=e.club_name||e.pseudo||"?",a=e.pseudo||"",l=(a||i).slice(0,2).toUpperCase(),d=e.club_color2||Dt,s=e.club_color1||"#ffffff",c=e.last_seen_at?new Date(e.last_seen_at):null,p=c&&Date.now()-c.getTime()<3*60*1e3;return`
    ...
    ${hi()}`,document.body.appendChild(i);const a=i.querySelector("#friend-pseudo-input"),l=i.querySelector("#add-friend-error"),d=()=>i.remove();a.focus(),i.querySelector("#add-cancel").addEventListener("click",d),i.addEventListener("click",s=>{s.target===i&&d()}),i.querySelector("#add-ok").addEventListener("click",async()=>{const s=a.value.trim();if(!s){l.textContent="Entre un pseudo";return}l.textContent="";const{data:c}=await T.from("users").select("id, pseudo").ilike("pseudo",s).single();if(!c){l.textContent="Utilisateur introuvable";return}if(c.id===e.user.id){l.textContent="Tu ne peux pas t'ajouter toi-même";return}const{data:p}=await T.from("friendships").select("id, status").or(`and(requester_id.eq.${e.user.id},addressee_id.eq.${c.id}),and(requester_id.eq.${c.id},addressee_id.eq.${e.user.id})`).single();if(p){const o=p.status==="accepted"?"Vous êtes déjà amis !":p.status==="pending"?"Demande déjà envoyée":"Une demande existe déjà";l.textContent=o;return}const{error:n}=await T.from("friendships").insert({requester_id:e.user.id,addressee_id:c.id,status:"pending"});if(n){l.textContent="Erreur : "+n.message;return}d(),t(`✅ Demande envoyée à ${c.pseudo} !`,"success")})}async function Ji(e,t,i=null,a={}){const l=e.user.id,{data:d}=await T.from("friendships").select("id, requester_id").eq("addressee_id",l).eq("status","pending").order("created_at",{ascending:!1}),s=(d||[]).map(v=>v.requester_id);let c={};if(s.length){const{data:v}=await T.from("users").select("id, pseudo, club_name").in("id",s);(v||[]).forEach(w=>{c[w.id]=w})}const p=(d||[]).map(v=>({...v,requester:c[v.requester_id]||{pseudo:"?"}})),n=bi(),o=p||[];n.innerHTML=`
    <div class="popup-box">
    ...
    ${hi()}`,document.body.appendChild(n);const f=()=>n.remove();n.querySelector("#pending-close").addEventListener("click",f),n.addEventListener("click",v=>{v.target===n&&f()}),n.querySelectorAll("[data-accept]").forEach(v=>{v.addEventListener("click",async()=>{const{error:w}=await T.from("friendships").update({status:"accepted"}).eq("id",v.dataset.accept);if(w){t("Erreur : "+w.message,"error");return}v.closest("div[style]").remove(),t("✅ Ami accepté !","success"),Wi(e,t,a),i&&i()})}),n.querySelectorAll("[data-decline]").forEach(v=>{v.addEventListener("click",async()=>{await T.from("friendships").delete().eq("id",v.dataset.decline),v.closest("div[style]").remove(),t("Demande refusée","info"),i&&i()})})}async function Pn(e,t,i){const a=e.user.id,[l,d]=[a,t].sort(),s=a===l,{data:c}=await T.from("friend_match_stats").select("*").eq("player1_id",l).eq("player2_id",d).single(),p=e.profile.club_name||e.profile.pseudo||"Moi",n=c||{},o=s?n.wins_p1||0:n.wins_p2||0,f=s?n.wins_p2||0:n.wins_p1||0,v=n.draws||0,w=s?n.goals_p1||0:n.goals_p2||0,A=s?n.goals_p2||0:n.goals_p1||0,r=s?n.gc_used_p1||0:n.gc_used_p2||0,u=s?n.gc_used_p2||0:n.gc_used_p1||0,I=n.matches_total||0,z=(H,B,j,U=Dt,oe=Gt)=>`
    ...
    </style>`}async function Xi({player1Id:e,player2Id:t,score1:i,score2:a,gc1:l,gc2:d}){const[s,c]=[e,t].sort(),p=e!==s,n=p?a:i,o=p?i:a,f=p?d:l,v=p?l:d,w=n>o?1:0,A=o>n?1:0,r=n===o?1:0,{data:u}=await T.from("friend_match_stats").select("*").eq("player1_id",s).eq("player2_id",c).single();u?await T.from("friend_match_stats").update({wins_p1:u.wins_p1+w,wins_p2:u.wins_p2+A,draws:u.draws+r,goals_p1:u.goals_p1+n,goals_p2:u.goals_p2+o,gc_used_p1:u.gc_used_p1+f,gc_used_p2:u.gc_used_p2+v,matches_total:u.matches_total+1}).eq("player1_id",s).eq("player2_id",c):await T.from("friend_match_stats").insert({player1_id:s,player2_id:c,wins_p1:w,wins_p2:A,draws:r,goals_p1:n,goals_p2:o,gc_used_p1:f,gc_used_p2:v,matches_total:1})}const Nn="2026.07.02-0135";async function Mi(e,{state:t,navigate:i,toast:a}){var d,s;const l=t.profile;l&&(e.innerHTML=`
  <style>
  ...
  `,e.querySelectorAll("[data-nav]").forEach(c=>{c.addEventListener("click",p=>{p.preventDefault(),i(c.dataset.nav)})}),(d=document.getElementById("nav-rankings"))==null||d.addEventListener("click",()=>i("rankings")),(s=document.getElementById("nav-matches"))==null||s.addEventListener("click",()=>i("matches")),e.querySelectorAll("[data-action]").forEach(c=>{c.addEventListener("click",()=>{c.classList.add("tapped"),setTimeout(()=>c.classList.remove("tapped"),200);const p=c.dataset.action;if(p==="match-ai"){Un(i);return}if(p==="match-random"){i("match",{matchMode:"random"});return}if(p==="match-friend"){i("friends");return}if(p==="mini-league"){i("mini-league");return}if(p==="ranked"){ctx.toast("Mode Ranked — Bientôt disponible !","info");return}a("Bientôt disponible","info")})}),window.innerWidth<768&&requestAnimationFrame(()=>{var H,B,j,U,oe,ce;const c=((H=window.visualViewport)==null?void 0:H.height)||window.innerHeight,p=((B=document.querySelector(".top-nav"))==null?void 0:B.offsetHeight)||56,n=((j=document.querySelector(".bottom-nav"))==null?void 0:j.offsetHeight)||60,o=((U=e.querySelector(".hero-compact"))==null?void 0:U.offsetHeight)||52,f=e.querySelector(".ranked-banner");e.querySelector(".play-grid");const v=((ce=(oe=e.querySelector("#logout-btn"))==null?void 0:oe.closest("div"))==null?void 0:ce.offsetHeight)||44,w=["friend-requests-banner","match-invite-banner","ongoing-match-banner"].reduce((re,D)=>{var ie;return re+(((ie=document.getElementById(D))==null?void 0:ie.offsetHeight)||0)},0),A=14*5,r=c-p-n-o-v-w-A,u=Math.max(80,Math.round(r*.28)),I=Math.max(160,Math.round(r*.72)),z=Math.floor((I-10)/2);f&&(f.style.minHeight=f.style.maxHeight=u+"px"),e.querySelectorAll(".play-grid .play-card").forEach(re=>{re.style.minHeight=re.style.height=z+"px"});const C=Math.round(z*.55);e.querySelectorAll(".play-card .play-icon").forEach(re=>{re.style.height=C+"px"})}),document.getElementById("logout-btn").addEventListener("click",async()=>{await T.auth.signOut(),window.location.reload()}),Qi(t,a),Hn(t,a,i),Zi(t,a,i))}async function Zi(e,t,i){const a=document.getElementById("ongoing-match-banner");if(!a)return;const l=e.profile.id,{data:d}=await T.from("matches").select("id, home_id, away_id, status, mode").eq("status","active").or(`home_id.eq.${l},away_id.eq.${l}`).order("created_at",{ascending:!1});if(!(d!=null&&d.length)){a.innerHTML="";return}const s=d.map(p=>p.home_id===l?p.away_id:p.home_id).filter(Boolean);let c={};if(s.length){const{data:p}=await T.from("users").select("id, pseudo, club_name").in("id",s);(p||[]).forEach(n=>{c[n.id]=n.club_name||n.pseudo})}a.innerHTML=d.map(p=>{const n=p.home_id===l?p.away_id:p.home_id,o=c[n]||"Adversaire",f=p.mode==="mini_league";return`
    ...
    </div>`,document.body.appendChild(t),t.querySelector("#ab-cancel").addEventListener("click",()=>t.remove()),t.querySelector("#ab-ok").addEventListener("click",()=>{t.remove(),e()}),t.addEventListener("click",i=>{i.target===t&&t.remove()})}async function Hn(e,t,i){var c,p,n,o;const a=document.getElementById("match-invite-banner");if(!a)return;const{data:l}=await T.from("friend_match_invites").select("id, inviter_id, inviter:users!inviter_id(pseudo, club_name)").eq("invitee_id",e.user.id).eq("status","pending").order("created_at",{ascending:!1}).limit(1).maybeSingle();if(!l){a.innerHTML="";return}const d=((c=l.inviter)==null?void 0:c.club_name)||((p=l.inviter)==null?void 0:p.pseudo)||"?",s=l.inviter_id;a.innerHTML=`
    ...
    </div>`,(n=document.getElementById("match-inv-accept"))==null||n.addEventListener("click",()=>{a.innerHTML="",i("match",{matchMode:"friend",friendId:s,friendName:d})}),(o=document.getElementById("match-inv-decline"))==null||o.addEventListener("click",async()=>{await T.from("friend_match_invites").update({status:"declined"}).eq("id",l.id),a.innerHTML="",t("Invitation refusée","info")})}async function Qi(e,t){const i=document.getElementById("friend-requests-banner");if(!i)return;const{data:a,error:l}=await T.from("friendships").select("id, requester:users!requester_id(pseudo, club_name)").eq("addressee_id",e.user.id).eq("status","pending");if(l||!(a!=null&&a.length)){i.innerHTML="";return}const d=a.length,s=a.slice(0,2).map(p=>{var n;return((n=p.requester)==null?void 0:n.pseudo)||"?"}).join(", "),c=d>2?` +${d-2}`:"";i.innerHTML=`
  ...
  `,document.body.appendChild(i);const a=()=>i.remove();document.getElementById("diff-cancel").addEventListener("click",a),i.addEventListener("click",l=>{l.target===i&&a()}),i.querySelectorAll("[data-mode]").forEach(l=>{l.addEventListener("click",()=>{a(),e("match",{matchMode:l.dataset.mode})})})}const Ue={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé pour ce match."},"Double attaque":{icon:"⚡",desc:"La note d'attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la note d'un joueur adverse."},Gel:{icon:"❄️",desc:"Bloque le meilleur attaquant IA."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function $e(e,t){if(!e)return 0;switch(t){case"GK":return Number(e.note_g)||0;case"DEF":return Number(e.note_d)||0;case"MIL":return Number(e.note_m)||0;case"ATT":return Number(e.note_a)||0;default:return 0}}const Ci=["ATT","MIL","DEF","GK"];function en(e){let t=0;const i=e.length;for(let a=0;a<i;a++)for(let l=a+1;l<i;l++){const d=e[a],s=e[l];if(!d||!s)continue;const c=d._col!=null&&s._col!=null&&d._col===s._col,p=d._col!=null&&s._col!=null&&Math.abs(d._col-s._col)===1,n=Ci.indexOf(d._line||d.job),o=Ci.indexOf(s._line||s.job),f=Math.abs(n-o)===1;if(!((d._line||d.job)===(s._line||s.job)&&p||c&&f))continue;const A=d.country_code&&s.country_code&&d.country_code===s.country_code,r=d.club_id&&s.club_id&&d.club_id===s.club_id;A&&r?t+=2:(A||r)&&(t+=1)}return t}function Rt(e,t={}){const i=e.reduce((d,s)=>{const c=s._line||s.job;return d+(Number(c==="MIL"?s.note_m:s.note_a)||0)+(s.boost||0)},0),a=en(e);let l=i+a;return t.doubleAttack&&(l*=2),t.stolenNote&&(l-=t.stolenNote),{base:i,links:a,total:Math.max(0,l)}}function Ot(e,t={}){const i=e.reduce((d,s)=>{const c=s._line||s.job;let p=0;return c==="GK"?p=Number(s.note_g)||0:c==="MIL"?p=Number(s.note_m)||0:p=Number(s.note_d)||0,d+p+(s.boost||0)},0),a=en(e);let l=i+a;return t.stolenNote&&(l-=t.stolenNote),{base:i,links:a,total:Math.max(0,l)}}function Ht(e,t,i={}){return i.shield?{goal:!1,shielded:!0}:{goal:e>t,shielded:!1}}function tn(e,t,i="easy"){const a=e.filter(s=>!s.used);if(!a.length)return[];const l=[...a].sort((s,c)=>{const p=t==="attack"?$e(s,"ATT"):s._line==="GK"?$e(s,"GK"):$e(s,"DEF");return(t==="attack"?$e(c,"ATT"):c._line==="GK"?$e(c,"GK"):$e(c,"DEF"))-p});let d=i==="easy"?1+Math.floor(Math.random()*2):i==="medium"?2+Math.floor(Math.random()*2):3;return l.slice(0,Math.min(d,l.length,3))}function Vn(e,t){const i={vs_ai_easy:{victoire:500,nul:250,defaite:50},vs_ai_medium:{victoire:1e3,nul:500,defaite:50},vs_ai_hard:{victoire:1500,nul:750,defaite:100},vs_ai_club:{victoire:2500,nul:1250,defaite:100}};return(i[e]||i.vs_ai_easy)[t]||0}const nn={pepite:{win:2,loss:-1,boosterStart:"min"},papyte:{win:1,loss:-2,boosterStart:"max"}};function vi(e,t){const i=e.player;if(!i)return 0;const a=i.rarity;if(a!=="pepite"&&a!=="papyte")return Number(i[t])||0;const l=wi(i),d=Number(i[t])||0;if(d<=0)return 0;const s=i.note_min??1,c=i.note_max??10,n=(e.current_note??l)-l;return Math.min(c,Math.max(s,d+n))}function wi(e){const t=e.job||"ATT";return Number(t==="GK"?e.note_g:t==="DEF"?e.note_d:t==="MIL"?e.note_m:e.note_a)||0}async function _i(e,t){!e||!t||await Promise.all([ji(e,"win"),ji(t,"loss")])}async function ji(e,t){const{data:i}=await T.from("cards").select("id, current_note, player:players(rarity, job, note_g, note_d, note_m, note_a, note_min, note_max)").eq("owner_id",e).eq("card_type","player");if(!(i!=null&&i.length))return;const a=i.filter(l=>{var d,s;return((d=l.player)==null?void 0:d.rarity)==="pepite"||((s=l.player)==null?void 0:s.rarity)==="papyte"});a.length&&await Promise.all(a.map(l=>{const d=nn[l.player.rarity],s=t==="win"?d.win:d.loss,c=l.player.note_min??1,p=l.player.note_max??10,n=wi(l.player),o=l.current_note??n,f=Math.min(p,Math.max(c,o+s));return T.from("cards").update({current_note:f}).eq("id",l.id)}))}const on=Object.freeze(Object.defineProperty({__proto__:null,EVOLUTIVE_RULES:nn,currentSecondaryNote:vi,getBaseMainNote:wi,updateEvolutiveCards:_i},Symbol.toStringTag,{value:"Module"})),rn={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"},qt={GK:"#111111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},Vt=["GK","DEF","MIL","ATT"],Kn=["Tous","GK","DEF","MIL","ATT"],Yn={normal:1e3,pepite:5e3,papyte:5e3,legende:1e4};function qi(e){const t=e.player;if(!t)return null;const i=t.rarity;return(i==="pepite"||i==="papyte")&&e.current_note!=null?e.current_note:Math.max(Number(t.note_g)||0,Number(t.note_d)||0,Number(t.note_m)||0,Number(t.note_a)||0)}function Wn(e){return e.length?e.reduce((t,i)=>qi(i)>qi(t)?i:t,e[0]):e[0]}function an(e){return e==="GK"?"note_g":e==="DEF"?"note_d":e==="MIL"?"note_m":"note_a"}const $i={MA:"MAROC",FR:"FRANCE",AR:"ARGENTINE",PT:"PORTUGAL",BR:"BRESIL",ES:"ESPAGNE",DE:"ALLEMAGNE",GB:"ANGLETERRE",IT:"ITALIE",CM:"CAMEROUN",SN:"SENEGAL",NG:"NIGERIA",DK:"DANEMARK",NL:"PAYS-BAS",BE:"BELGIQUE",CI:"CÔTE D'IVOIRE",AL:"ALBANIE",HR:"CROATIE",RS:"SERBIE",TR:"TURQUIE"};function sn(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";if(!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function Pt(e,t){return e&&Number(t==="GK"?e.note_g:t==="DEF"?e.note_d:t==="MIL"?e.note_m:e.note_a)||0}function ai(e,t=""){var v,w;const i=e.player;if(!i)return"";const a=i.job||"ATT",l=qt[a],d=rn[i.rarity]||"#ccc",s=i.rarity==="pepite"||i.rarity==="papyte",c=s&&e.current_note!=null?e.current_note:Pt(i,a),p=i.job2?s?vi(e,an(i.job2)):Pt(i,i.job2):null,n=i.job2?qt[i.job2]:null,o=sn(i),f=$i[i.country_code]||i.country_code||"";return`
  <div style="
  ...
  </div>`}function Fi(e){const t=e.job||"ATT",i=Pt(e,t),a=$i[e.country_code]||e.country_code||"";return`
  <div style="
    width:140px;border-radius:12px;padding:6px;
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
  ...
  </div>`}async function Jn(e,t){const{state:i,navigate:a,toast:l,openModal:d,closeModal:s}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:c}=await T.from("cards").select(`id, card_type, current_note, gc_type, formation, is_for_sale, sale_price,
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),n=(c||[]).filter(Y=>Y.card_type==="player"&&Y.player),o=(c||[]).filter(Y=>Y.card_type==="game_changer"),f=(c||[]).filter(Y=>Y.card_type==="formation"),{data:v}=await T.from("gc_definitions").select("name,gc_type,color,effect,image_url"),w={};(v||[]).forEach(Y=>{w[Y.name]=Y});const A=Object.keys(xi),r=Object.keys(Ue),u={};n.forEach(Y=>{const J=Y.player.id;u[J]=(u[J]||0)+1}),new Set(Object.keys(u).map(Y=>String(Y)));const I=new Set(f.map(Y=>Y.formation)),z=new Set(o.map(Y=>Y.gc_type));let C="player",H="Tous",B="",j=!1;function U(){return[...n].sort((Y,J)=>{const N=Vt.indexOf(Y.player.job),R=Vt.indexOf(J.player.job);return N!==R?N-R:(Y.player.surname_encoded||"").localeCompare(J.player.surname_encoded||"")})}function oe(){return[...p||[]].sort((Y,J)=>{const N=Vt.indexOf(Y.job),R=Vt.indexOf(J.job);return N!==R?N-R:(Y.surname_encoded||"").localeCompare(J.surname_encoded||"")})}function ce(){return U().filter(Y=>{const J=Y.player,N=H==="Tous"||J.job===H,R=!B||`${J.firstname} ${J.surname_encoded}`.toLowerCase().includes(B);return N&&R})}function re(){return oe().filter(Y=>{const J=H==="Tous"||Y.job===H,N=!B||`${Y.firstname} ${Y.surname_encoded}`.toLowerCase().includes(B);return J&&N})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Kt=1e3;function Zn(e,t,i,a){var A,r,u;const{state:l,toast:d,closeModal:s,navigate:c,refreshProfile:p}=i,n=e.formation,o={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function f(){const I=Nt[n]||{},z=xi[n]||[],C=290,H=360,B=20;function j(oe){const ce=I[oe];return ce?{x:ce.x*C,y:ce.y*H}:null}let U=`<svg width="${C}" height="${H}" viewBox="0 0 ${C} ${H}" xmlns="http://www.w3.org/2000/svg">`;for(const[oe,ce]of z){const re=j(oe),D=j(ce);!re||!D||(U+=`<line x1="${re.x}" y1="${re.y}" x2="${D.x}" y2="${D.y}"
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(A=document.getElementById("direct-sell-form-btn"))==null||A.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${n} pour ${Kt.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const I=v.find(C=>!C.is_for_sale)||v[0];if(!I){d("Aucune carte à vendre","error");return}await T.from("market_listings").delete().eq("card_id",I.id),await T.from("transfer_history").delete().eq("card_id",I.id);const{error:z}=await T.from("cards").delete().eq("id",I.id);if(z){d(z.message,"error");return}await T.from("users").update({credits:(l.profile.credits||0)+Kt}).eq("id",l.profile.id),await p(),d(`+${Kt.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),s(),c("collection")}),(r=document.getElementById("market-sell-form-btn"))==null||r.addEventListener("click",async()=>{const I=parseInt(document.getElementById("sell-price-form").value);if(!I||I<1){d("Prix invalide","error");return}await T.from("cards").update({is_for_sale:!0,sale_price:I}).eq("id",e.id),await T.from("market_listings").insert({seller_id:l.profile.id,card_id:e.id,price:I}),d("Carte mise en vente sur le marché !","success"),s(),c("collection")}),(u=document.getElementById("cancel-sell-form-btn"))==null||u.addEventListener("click",async()=>{await T.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await T.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),d("Annonce retirée","success"),s(),c("collection")})}async function Bi(e,t,i,a){var fe,xe,Ee,_e,he,Y,J;const{state:l,toast:d,openModal:s,closeModal:c,navigate:p,refreshProfile:n}=a,o=e.player,f=t.filter(N=>N.player.id===o.id),v=f.length,w=Math.max(Number(o.note_g)||0,Number(o.note_d)||0,Number(o.note_m)||0,Number(o.note_a)||0),A=o.rarity||"normal",{data:r}=await T.from("sell_price_configs").select("*").eq("rarity",A).lte("note_min",w).gte("note_max",w).order("note_min",{ascending:!1}).limit(1),u=((fe=r==null?void 0:r[0])==null?void 0:fe.price)??Yn[A]??1e3,I=o.rarity!=="legende",z=sn(o),C=(o.rarity==="pepite"||o.rarity==="papyte")&&e.current_note!=null?e.current_note:Pt(o,o.job),H=o.rarity==="pepite"||o.rarity==="papyte",B=o.job2?H?vi(e,an(o.job2)):Pt(o,o.job2):null,j=qt[o.job]||"#1A6B3C",U=o.job2?qt[o.job2]:null,oe=rn[o.rarity]||"#ccc",ce=$i[o.country_code]||o.country_code||"",re=f.map(N=>N.id);let D={};if(re.length){const{data:N}=await T.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",re).order("transferred_at",{ascending:!0});(N||[]).forEach(R=>{D[R.card_id]||(D[R.card_id]=[]),D[R.card_id].push(R)})}const ie=N=>{const R=N.transferred_at?new Date(N.transferred_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"2-digit",year:"numeric"}):"",Q=N.source==="booster"?"Booster":N.price?N.price.toLocaleString("fr"):"—";return`
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",o.note_g],["DEF",o.note_d],["MIL",o.note_m],["ATT",o.note_a]].map(([N,R])=>{const Q=qt[N],ge=Number(R)||0;return`<div style="display:flex;flex-direction:column;align-items:center;gap:2px">
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(_e=document.getElementById("close-detail"))==null||_e.addEventListener("click",c);let G=new Set;const se=()=>{const N=G.size,R=document.getElementById("sell-action-panel");R&&(R.style.display=N>0?"block":"none",document.getElementById("sell-selected-count").textContent=N,document.getElementById("sell-direct-total").textContent=(N*u).toLocaleString("fr")+" cr.")};document.querySelectorAll(".expl-check").forEach(N=>{N.addEventListener("change",()=>{const R=N.dataset.id;N.checked?G.add(R):G.delete(R);const Q=N.closest(".exemplaire-row");Q&&(Q.style.borderColor=N.checked?"#1A6B3C":"#eee"),se()})}),document.querySelectorAll(".exemplaire-row").forEach(N=>{N.addEventListener("click",R=>{if(R.target.closest("button")||R.target.tagName==="INPUT")return;const Q=N.querySelector(".expl-check");Q&&!Q.disabled&&(Q.checked=!Q.checked,Q.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(N=>{N.addEventListener("click",R=>{R.stopPropagation();const Q=document.querySelector(`.expl-hist[data-hist="${N.dataset.idx}"]`);Q&&(Q.style.display=Q.style.display==="none"?"flex":"none")})}),(he=document.getElementById("direct-sell-btn"))==null||he.addEventListener("click",async()=>{const N=[...G];if(!N.length)return;const R=N.length*u;if(!confirm(`Vendre ${N.length} carte${N.length>1?"s":""} ${o.surname_encoded} pour ${R.toLocaleString("fr")} crédits ? Action irréversible.`))return;await T.from("market_listings").delete().in("card_id",N),await T.from("transfer_history").delete().in("card_id",N);const{error:Q}=await T.from("cards").delete().in("id",N);if(Q){d(Q.message,"error");return}await T.from("users").update({credits:(l.profile.credits||0)+R}).eq("id",l.profile.id),await n();const ge=document.getElementById("nav-credits");ge&&(ge.textContent=`💰 ${(l.profile.credits||0).toLocaleString("fr")}`),d(`+${R.toLocaleString("fr")} crédits ! ${N.length} carte${N.length>1?"s":""} vendue${N.length>1?"s":""}.`,"success"),c(),p("collection")}),(Y=document.getElementById("market-sell-btn"))==null||Y.addEventListener("click",async()=>{var Me;const N=[...G];if(!N.length){d("Sélectionne au moins un exemplaire","warning");return}const R=parseInt((Me=document.getElementById("sell-market-price"))==null?void 0:Me.value);if(!R||R<1){d("Prix invalide","error");return}const{error:Q}=await T.from("cards").update({is_for_sale:!0,sale_price:R}).in("id",N);if(Q){d(Q.message,"error");return}const ge=N.map(Ce=>({seller_id:l.profile.id,card_id:Ce,price:R,status:"active"})),{error:pe}=await T.from("market_listings").insert(ge);pe&&console.warn("[Market] insert listings:",pe.message),d(`${N.length} carte${N.length>1?"s":""} mise${N.length>1?"s":""} en vente à ${R.toLocaleString("fr")} cr. chacune !`,"success"),c(),p("collection")}),(J=document.getElementById("cancel-sell-btn"))==null||J.addEventListener("click",async()=>{await T.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await T.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),d("Annonce retirée","success"),c(),p("collection")})}const Xt={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},vt={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function ti(e){const t=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function dn(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function ln(e){var i;const t="https://fcnwclxlkytdfjotqkta.supabase.co";return(i=e==null?void 0:e.clubs)!=null&&i.logo_url?e.clubs.logo_url.startsWith("http")?e.clubs.logo_url:`${t}/storage/v1/object/public/assets/clubs/${e.clubs.logo_url}`:null}function Qn(e,t=44,i=58){var w;const a=e?ti(e):null,l=e?ln(e):null,d=dn(e==null?void 0:e.country_code),s=(e==null?void 0:e.job)||"MIL",c=vt[s]||"#555",p={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",n=Number(s==="GK"?e==null?void 0:e.note_g:s==="DEF"?e==null?void 0:e.note_d:s==="MIL"?e==null?void 0:e.note_m:e==null?void 0:e.note_a)||0,o=Math.round(i*.19),f=Math.round(i*.25),v=i-o-f;return e?`<div style="width:${t}px;height:${i}px;border-radius:5px;border:2px solid ${p};background:${c};position:relative;overflow:hidden;flex-shrink:0">
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;height:${f}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ...
      ${l?`<img src="${l}" style="width:${f-4}px;height:${f-4}px;object-fit:contain">`:(w=e==null?void 0:e.clubs)!=null&&w.encoded_name?`<span style="font-size:${Math.max(4,f-8)}px;font-weight:700;color:#333">${(e.clubs.encoded_name||"").slice(0,3).toUpperCase()}</span>`:"<span></span>"}
    </div>
  ...
  </div>`,document.getElementById("new-deck-btn").addEventListener("click",async()=>{const s=prompt("Nom du deck :",`Deck ${((d==null?void 0:d.length)||0)+1}`);if(!s)return;const{data:c,error:p}=await T.from("decks").insert({owner_id:i.profile.id,name:s}).select().single();if(p){l(p.message,"error");return}l("Deck créé !","success"),Di(c.id,e,t)}),e.querySelectorAll("[data-open-deck]").forEach(s=>{s.addEventListener("click",()=>Di(s.dataset.openDeck,e,t))}),e.querySelectorAll("[data-rename]").forEach(s=>{s.addEventListener("click",async()=>{const c=prompt("Nouveau nom :",s.dataset.name);if(!c||c===s.dataset.name)return;const{error:p}=await T.from("decks").update({name:c}).eq("id",s.dataset.rename);if(p){l(p.message,"error");return}l("Deck renommé !","success"),li(e,t)})}),e.querySelectorAll("[data-delete]").forEach(s=>{s.addEventListener("click",async()=>{if(!confirm(`Supprimer le deck "${s.dataset.name}" ? Cette action est irréversible.`))return;await T.from("deck_cards").delete().eq("deck_id",s.dataset.delete);const{error:c}=await T.from("decks").delete().eq("id",s.dataset.delete);if(c){l(c.message,"error");return}l("Deck supprimé.","success"),li(e,t)})})}async function Di(e,t,i){const{state:a,toast:l}=i;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:d}=await T.from("decks").select("*").eq("id",e).single(),{data:s}=await T.from("cards").select(`id, card_type, formation,
      player:players(id, firstname, surname_encoded, country_code, club_id, job, job2,
        note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length,
        clubs(encoded_name, logo_url))`).eq("owner_id",a.profile.id),c=(s||[]).filter(w=>w.card_type==="player"&&w.player),p=(s||[]).filter(w=>w.card_type==="formation"),n=p.map(w=>w.formation).filter(Boolean),{data:o}=await T.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",e);let f=d.formation||"4-4-2";n.length>0&&!n.includes(f)&&(f=n[0]);const v={deckId:e,name:d.name,formation:f,formationCardId:d.formation_card_id,slots:{},subs:[],playerCards:c,formationCards:p,availableFormations:n};(o||[]).forEach(w=>{w.is_starter?v.slots[w.position]=w.card_id:v.subs.includes(w.card_id)||v.subs.push(w.card_id)}),zt(t,v,i)}function zt(e,t,i){var p;const{navigate:a}=i;Xt[t.formation];const l=Gi(t.formation),d=l.filter(n=>t.slots[n]).length,s=t.availableFormations.length>0?t.availableFormations:Object.keys(Xt),c=t.subs.map(n=>t.playerCards.find(o=>o.id===n)).filter(Boolean);[...Object.values(t.slots),...t.subs],e.innerHTML=`
  <div style="height:100%;overflow-y:auto;padding-bottom:80px;background:var(--page-bg)">
    ...
    <div style="padding:10px 12px;background:rgba(0,0,0,0.25);border-top:1px solid rgba(255,255,255,0.1)">
      ...
      <div style="display:flex;gap:8px;align-items:flex-end;overflow-x:auto;padding-bottom:4px" id="subs-list">
        ${c.map(n=>{const o=n.player;return`<div style="position:relative;flex-shrink:0">
            ${Qn(o,44,58)}
    ...
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(w=document.getElementById("close-selector"))==null||w.addEventListener("click",d),(A=document.getElementById("remove-player"))==null||A.addEventListener("click",()=>{delete t.slots[e],d(),zt(i,t,a)}),document.querySelectorAll(".player-option").forEach(r=>{r.addEventListener("click",()=>{t.slots[e]=r.dataset.cardId,d(),zt(i,t,a)})})}function io(e,t,i){var p;const{openModal:a,closeModal:l}=i,d=new Set;Object.values(e.slots).filter(Boolean).forEach(n=>{var f;const o=e.playerCards.find(v=>v.id===n);(f=o==null?void 0:o.player)!=null&&f.id&&d.add(o.player.id)}),e.subs.forEach(n=>{var f;const o=e.playerCards.find(v=>v.id===n);(f=o==null?void 0:o.player)!=null&&f.id&&d.add(o.player.id)});const s=new Set,c=e.playerCards.filter(n=>{var o,f,v;return d.has((o=n.player)==null?void 0:o.id)||s.has((f=n.player)==null?void 0:f.id)?!1:(s.add((v=n.player)==null?void 0:v.id),!0)});a("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${c.length>0?c.map(n=>{var w;const o=n.player,f=ti(o),v=o.job==="GK"?o.note_g:o.job==="DEF"?o.note_d:o.job==="MIL"?o.note_m:o.note_a;return`<div class="player-option" data-card-id="${n.id}"
          style="display:flex;align-items:center;gap:10px;padding:8px;border:1.5px solid var(--gray-200);border-radius:10px;cursor:pointer">
    ...
    </div>`,'<button class="btn btn-ghost" id="close-sub-selector">Fermer</button>'),(p=document.getElementById("close-sub-selector"))==null||p.addEventListener("click",l),document.querySelectorAll(".player-option").forEach(n=>{n.addEventListener("click",()=>{e.subs.push(n.dataset.cardId),l(),zt(t,e,i)})})}async function no(e,t){const{state:i,toast:a,navigate:l}=t,d=e.formationCards.find(p=>p.formation===e.formation),s=(d==null?void 0:d.id)||e.formationCardId;await T.from("decks").update({formation:e.formation,formation_card_id:s||null}).eq("id",e.deckId),await T.from("deck_cards").delete().eq("deck_id",e.deckId);const c=[];if(Object.entries(e.slots).forEach(([p,n],o)=>{c.push({deck_id:e.deckId,card_id:n,position:p,is_starter:!0,slot_order:o})}),e.subs.forEach((p,n)=>{c.push({deck_id:e.deckId,card_id:p,position:`SUB${n+1}`,is_starter:!1,slot_order:100+n})}),c.length>0){const{error:p}=await T.from("deck_cards").insert(c);if(p){a(p.message,"error");return}}a("Deck enregistré ✅","success"),l("decks")}function Gi(e){const t=Xt[e]||Xt["4-4-2"],i=["GK1"];for(let a=1;a<=t.DEF;a++)i.push(`DEF${a}`);for(let a=1;a<=t.MIL;a++)i.push(`MIL${a}`);for(let a=1;a<=t.ATT;a++)i.push(`ATT${a}`);return i}async function cn(){const{data:e}=await T.from("booster_configs").select("*").eq("is_active",!0).order("sort_order");if(!(e!=null&&e.length))return[];const{data:t}=await T.from("booster_drop_rates").select("*").in("booster_id",e.map(i=>i.id)).order("sort_order");return e.map(i=>({...i,rates:(t||[]).filter(a=>a.booster_id===i.id)}))}function oo(e){if(!(e!=null&&e.length))return null;const t=e.reduce((a,l)=>a+Number(l.percentage),0);let i=Math.random()*t;for(const a of e)if(i-=Number(a.percentage),i<=0)return a;return e[e.length-1]}const pn=()=>Object.keys(Nt),ro=[{id:"players_std",img:"/manager-wars/icons/booster-players.png",name:"Players",sub:"5 cartes joueurs",cost:5e3,costLabel:"5 000 crédits",cardCount:5,type:"player"},{id:"players_pub",img:"/manager-wars/icons/booster-silver.png",name:"Players (pub)",sub:"3 cartes joueurs",cost:0,costLabel:"1 pub",cardCount:3,type:"player"},{id:"game_changer",img:"/manager-wars/icons/booster-gamechanger.png",name:"Game Changer",sub:"3 cartes spéciales",cost:1e4,costLabel:"10 000 crédits",cardCount:3,type:"game_changer"},{id:"formation",img:"/manager-wars/icons/booster-formation.png",name:"Formation",sub:"1 carte formation",cost:1e4,costLabel:"10 000 crédits",cardCount:1,type:"formation"}],ci={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé."},"Double attaque":{icon:"⚡",desc:"La prochaine attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la prochaine action IA."},Gel:{icon:"❄️",desc:"Bloque le meilleur attaquant IA."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function ao(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";if(!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}const Pi={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},so={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"},lo={MA:"MAROC",FR:"FRANCE",AR:"ARGENTINE",PT:"PORTUGAL",BR:"BRESIL",ES:"ESPAGNE",DE:"ALLEMAGNE",GB:"ANGLETERRE",IT:"ITALIE",CM:"CAMEROUN",SN:"SENEGAL",NG:"NIGERIA",DK:"DANEMARK",NL:"PAYS-BAS",BE:"BELGIQUE",CI:"CÔTE D'IVOIRE",AL:"ALBANIE",HR:"CROATIE",RS:"SERBIE",TR:"TURQUIE"};function Ni(e,t){return e&&Number(t==="GK"?e.note_g:t==="DEF"?e.note_d:t==="MIL"?e.note_m:e.note_a)||0}function co(e){var o,f;const t=e.player;if(!t)return"";const i=t.job||"ATT",a=Pi[i],l=so[t.rarity]||"#ccc",d=Ni(t,i),s=t.job2?Ni(t,t.job2):null,c=t.job2?Pi[t.job2]:null,p=ao(t),n=lo[t.country_code]||t.country_code||"";return`
  ...
  </div>`}function un(e){var l;const t={};(e.rates||[]).forEach(d=>{t[d.card_type]=(t[d.card_type]||0)+Number(d.percentage||0)});const i=((l=Object.entries(t).sort((d,s)=>s[1]-d[1])[0])==null?void 0:l[0])||"player",a=e.image_url||"booster-players.png";return{id:e.id,img:"/manager-wars/icons/"+a,name:e.name,sub:`${e.card_count} carte(s)`,cost:e.price_type==="credits"&&e.price_credits||0,costLabel:e.price_type==="credits"?`${(e.price_credits||0).toLocaleString("fr")} crédits`:e.price_type==="pub"?"1 pub":"Gratuit",cardCount:e.card_count||5,type:i,isPub:e.price_type==="pub",rates:e.rates||[],_raw:e}}async function po(e,{state:t,navigate:i,toast:a}){var s;const l=((s=t.profile)==null?void 0:s.credits)||0;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⏳ Chargement...</div>';let d=[];try{d=(await cn()).map(un)}catch(c){console.warn("Erreur chargement boosters DB, fallback hardcodé",c)}d.length||(d=ro.map(c=>({...c,rates:[],isPub:c.id==="players_pub"}))),e.innerHTML=`
  <div class="page">
    <div class="page-header">
      <h2>📦 Boosters</h2>
    ...
    <div class="page-body">
      <div class="booster-grid">
        ${d.map(c=>{const p=c.cost===0||l>=c.cost;return`<div class="booster-card ${p?"":"disabled"}" data-booster="${c.id}" style="position:relative">
  ...
  </div>`,e.querySelectorAll(".booster-card:not(.disabled)").forEach(c=>{c.addEventListener("click",async()=>{const p=d.find(n=>n.id===c.dataset.booster);if(p){c.style.opacity="0.5",c.style.pointerEvents="none";try{await uo(p,{state:t,toast:a,navigate:i,container:e})}catch(n){a(n.message,"error"),c.style.opacity="",c.style.pointerEvents=""}}})}),e.querySelectorAll(".booster-info-btn").forEach(c=>{c.addEventListener("click",p=>{p.stopPropagation();const n=d.find(o=>o.id===c.dataset.boosterId);xo(n)})})}async function uo(e,{state:t,toast:i,navigate:a,container:l}){var o;if(e.cost>0&&t.profile.credits<e.cost){i("Crédits insuffisants","error");return}if(e.isPub)try{await ho()}catch(f){i(f.message||"Regardez la pub entièrement pour ouvrir le booster","error");return}const{data:d}=await T.from("cards").select("card_type, player_id, formation").eq("owner_id",t.profile.id),s=new Set((d||[]).filter(f=>f.card_type==="player").map(f=>f.player_id)),c=new Set((d||[]).filter(f=>f.card_type==="formation").map(f=>f.formation));let p=[];if((o=e.rates)!=null&&o.length)p=await pi(t.profile,e);else{const f=e.type||"player";f==="player"?p=await fn(t.profile,e.cardCount,e.cost):f==="game_changer"?p=await gn(t.profile,e.cardCount,e.cost):f==="formation"?p=await mn(t.profile,e.cost):p=await pi(t.profile,e)}p.forEach(f=>{f.card_type==="player"&&f.player?f.isDuplicate=s.has(f.player.id):f.card_type==="formation"&&(f.isDuplicate=c.has(f.formation))});const{data:n}=await T.from("users").select("*").eq("id",t.profile.id).single();n&&(t.profile=n),xn(p,e,a)}function fo(){const e=Math.random()*100;return e<.5?"legende":e<2?"special":e<10?"normal_high":"normal_low"}function mt(e){return Math.max(Number(e.note_g)||0,Number(e.note_d)||0,Number(e.note_m)||0,Number(e.note_a)||0)}function go(e,t){let i;switch(t){case"legende":i=e.filter(a=>a.rarity==="legende"),i.length||(i=e.filter(a=>a.rarity==="pepite"||a.rarity==="papyte")),i.length||(i=e.filter(a=>mt(a)>=6));break;case"special":i=e.filter(a=>a.rarity==="pepite"||a.rarity==="papyte"),i.length||(i=e.filter(a=>mt(a)>=6));break;case"normal_high":i=e.filter(a=>a.rarity==="normal"&&mt(a)>=6),i.length||(i=e.filter(a=>mt(a)>=6));break;default:i=e.filter(a=>a.rarity==="normal"&&mt(a)>=1&&mt(a)<=5),i.length||(i=e.filter(a=>a.rarity==="normal"));break}return i.length||(i=e),i[Math.floor(Math.random()*i.length)]}async function pi(e,t){if(t.cost>0){const{error:c}=await T.from("users").update({credits:e.credits-t.cost}).eq("id",e.id);if(c)throw c}const{data:i}=await T.from("cards").select("player_id, card_type, formation").eq("owner_id",e.id),a=new Set((i||[]).filter(c=>c.card_type==="player").map(c=>c.player_id)),l=new Set((i||[]).filter(c=>c.card_type==="formation").map(c=>c.formation)),d=new Set,s=[];for(let c=0;c<(t.cardCount||5);c++){const p=oo(t.rates);if(p){if(p.card_type==="player"){const n=z=>({légende:"legende",pépite:"pepite",pépites:"pepite"})[z]||z,o=p.rarity?n(p.rarity):null;let f=T.from("players").select("id,job,firstname,surname_encoded,country_code,club_id,rarity,note_g,note_d,note_m,note_a,skin,hair,hair_length,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);o&&(f=f.eq("rarity",o));const{data:v}=await f;let w=v||[];if((p.note_min||p.note_max)&&(w=w.filter(z=>{const C=Math.max(Number(z.note_g)||0,Number(z.note_d)||0,Number(z.note_m)||0,Number(z.note_a)||0);return(!p.note_min||C>=p.note_min)&&(!p.note_max||C<=p.note_max)})),w.length||(p.note_min||p.note_max?(w=v||[],console.warn("[Booster] Aucun joueur avec note",p.note_min,"-",p.note_max,"— fallback rareté uniquement")):w=v||[]),!w.length)continue;let A=w.filter(z=>!d.has(z.id));A.length||(A=w);const r=A[Math.floor(Math.random()*A.length)];d.add(r.id);const u=a.has(r.id),{data:I}=await T.from("cards").insert({owner_id:e.id,player_id:r.id,card_type:"player"}).select().single();I&&(s.push({...I,player:r,isDuplicate:u}),T.rpc("record_transfer",{p_card_id:I.id,p_player_id:r.id,p_club_name:e.club_name||e.pseudo,p_manager_name:e.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{}))}else if(p.card_type==="game_changer"){const{data:n}=await T.from("gc_definitions").select("id,name").eq("is_active",!0).eq("gc_type","game_changer"),o=n!=null&&n.length?n:[{name:"Ressusciter"},{name:"Double attaque"},{name:"Bouclier"},{name:"Vol de note"},{name:"Gel"}],v=o[Math.floor(Math.random()*o.length)].name,{data:w}=await T.from("cards").insert({owner_id:e.id,card_type:"game_changer",gc_type:v}).select().single();w&&s.push(w)}else if(p.card_type==="formation"){const n=pn(),o=n[Math.floor(Math.random()*n.length)],f=l.has(o),{data:v}=await T.from("cards").insert({owner_id:e.id,card_type:"formation",formation:o}).select();v!=null&&v[0]&&s.push({...v[0],isDuplicate:f})}}}return s}async function fn(e,t,i){if(i>0){const{error:n}=await T.from("users").update({credits:e.credits-i}).eq("id",e.id);if(n)throw n}const{data:a}=await T.from("players").select("id,job,firstname,surname_encoded,country_code,club_id,rarity,note_g,note_d,note_m,note_a,note_min,note_max,skin,hair,hair_length,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);if(!(a!=null&&a.length))throw new Error("Pas de joueurs en BDD — ajoutes-en via le panel admin !");const l=a.filter(n=>n.job==="GK"),d=a.filter(n=>n.job!=="GK"),s=!e.first_booster_opened&&l.length>0,c=[];for(let n=0;n<t;n++){const o=n===0&&s?l:n===0?d:a,f=fo(),v=go(o,f);v&&c.push(v)}s&&await T.from("users").update({first_booster_opened:!0}).eq("id",e.id);const{data:p}=await T.from("cards").insert(c.map(n=>({owner_id:e.id,player_id:n.id,card_type:"player"}))).select();return(p||[]).forEach((n,o)=>{T.rpc("record_transfer",{p_card_id:n.id,p_player_id:c[o].id,p_club_name:e.club_name||e.pseudo,p_manager_name:e.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{})}),c.map((n,o)=>({...p[o],player:n}))}async function gn(e,t,i){const{error:a}=await T.from("users").update({credits:e.credits-i}).eq("id",e.id);if(a)throw a;const{data:l}=await T.from("gc_definitions").select("id,name,gc_type,color,effect,image_url").eq("is_active",!0),d=l!=null&&l.length?l:Object.keys(ci).map(o=>({name:o,gc_type:"game_changer"})),s=Array.from({length:t},()=>{const o=d[Math.floor(Math.random()*d.length)];return{owner_id:e.id,card_type:"game_changer",gc_type:o.name,gc_definition_id:o.id||null}}),{data:c,error:p}=await T.from("cards").insert(s).select();return p&&console.error("[Booster GC] Erreur insert:",p.message,p),(c||[]).map(o=>{const f=l==null?void 0:l.find(v=>v.name===o.gc_type||v.id===o.gc_definition_id);return{...o,_gcDef:f||null}})}async function mn(e,t){const{error:i}=await T.from("users").update({credits:e.credits-t}).eq("id",e.id);if(i)throw i;const{data:a}=await T.from("cards").select("formation").eq("owner_id",e.id).eq("card_type","formation"),l=new Set((a||[]).map(o=>o.formation)),d=pn(),s=d[Math.floor(Math.random()*d.length)],c=l.has(s),{data:p,error:n}=await T.from("cards").insert({owner_id:e.id,card_type:"formation",formation:s}).select();return n&&console.error("[Booster Formation] Erreur insert:",n.message,n),(p||[]).map(o=>({...o,isDuplicate:c}))}function xn(e,t,i,a=null){var ie,W;if(!e||e.length===0){const G=document.createElement("div");G.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:3000;gap:16px;color:#fff;padding:24px;text-align:center",G.innerHTML=`
      <div style="font-size:48px">😕</div>
      <div style="font-size:20px;font-weight:900">Aucune carte obtenue</div>
      <div style="font-size:13px;color:rgba(255,255,255,0.5)">Erreur lors du tirage (permissions DB ou colonne manquante)</div>
      <button style="margin-top:10px;padding:12px 28px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer" id="anim-close-err">Fermer</button>`,document.body.appendChild(G),(ie=G.querySelector("#anim-close-err"))==null||ie.addEventListener("click",()=>G.remove());return}e=[...e].sort((G,se)=>{const fe=G.player?mt(G.player):-1;return(se.player?mt(se.player):-1)-fe});const l=document.createElement("div");l.id="booster-anim-overlay",l.innerHTML=`
    <style>
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
  ...
  </div>`;const o=async()=>{await T.from("users").update({pending_boosters:s}).eq("id",t.user.id)};async function f(){var z;if(n>=p||!s.length){await T.from("users").update({pending_boosters:[],onboarding_done:!0}).eq("id",t.user.id),l&&await l(),a("Tous tes boosters sont ouverts ! Bon jeu 🎮","success",4e3),i("home");return}const A=s[0],{data:r}=await T.from("users").select("*").eq("id",t.user.id).single();r&&(t.profile=r);let u=[];try{if(A.type==="formation")u=await mn(t.profile,0);else if(A.type==="game_changer")u=await gn(t.profile,A.count||3,0);else if(c&&((z=c.rates)!=null&&z.length)){const C={...c,cost:0,cardCount:A.count||c.cardCount||5};u=await pi(t.profile,C),A.guaranteeGK&&!t.profile.first_booster_opened&&(u.some(B=>B.player&&B.player.job==="GK")||await $o(t.profile,u),await T.from("users").update({first_booster_opened:!0}).eq("id",t.profile.id))}else u=await fn(t.profile,A.count||5,0)}catch(C){a(C.message||"Erreur ouverture booster","error");return}s.shift(),n++,await o();const I=A.type==="formation"?{name:"Booster Formation",img:"/manager-wars/icons/booster-formation.png"}:A.type==="game_changer"?{name:"Booster Game Changer",img:"/manager-wars/icons/booster-gamechanger.png"}:{name:`Booster Joueurs (${n}/${p})`,img:(c==null?void 0:c.img)||"/manager-wars/icons/booster-players.png"};xn(u,I,i,()=>{f()})}(w=document.getElementById("onboard-start"))==null||w.addEventListener("click",()=>f())}async function $o(e,t){try{const{data:i}=await T.from("players").select("id,job,firstname,surname_encoded,country_code,club_id,rarity,note_g,note_d,note_m,note_a,skin,hair,hair_length,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0).eq("job","GK");if(!(i!=null&&i.length))return;const a=i[Math.floor(Math.random()*i.length)],l=t.findIndex(s=>s.player);if(l===-1)return;const d=t[l];await T.from("cards").update({player_id:a.id}).eq("id",d.id),t[l]={...d,player_id:a.id,player:a}}catch(i){console.warn("[Onboarding] ensureGK échec",i)}}const Lt={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},St={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function wt(e,t,i,a,l){var d;e.innerHTML=`<div class="match-screen" style="display:flex;align-items:center;justify-content:center;min-height:100vh">
    <div style="text-align:center;padding:40px;color:#fff">
      <div style="font-size:48px;margin-bottom:16px">${t}</div>
      <p style="margin-bottom:16px">${i}</p>
      <button class="btn btn-primary" id="msg-btn">${a}</button>
    </div>
  </div>`,(d=document.getElementById("msg-btn"))==null||d.addEventListener("click",l)}function qe(e){const t=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function ui(e,t){var a,l;const i=e.player;return{cardId:e.id,position:t||null,id:i.id,firstname:i.firstname,name:i.surname_encoded,country_code:i.country_code,club_id:i.club_id,job:i.job,job2:i.job2,note_g:Number(i.note_g)||0,note_d:Number(i.note_d)||0,note_m:Number(i.note_m)||0,note_a:Number(i.note_a)||0,rarity:i.rarity,skin:i.skin,hair:i.hair,hair_length:i.hair_length,clubName:((a=i.clubs)==null?void 0:a.encoded_name)||null,clubLogo:((l=i.clubs)==null?void 0:l.logo_url)||null,boost:0,used:!1,_line:null,_col:null}}function lt(e){return e===1?[1]:e===2?[0,2]:e===3?[0,1,2]:e===4?[0,1,1,2]:e===5?[0,1,1,1,2]:[1]}function ii(){const e=Math.random()*100;return e<10?3:e<30?2:1}function yt(e,t){const i=Lt[t]||Lt["4-4-2"],a={GK:[],DEF:[],MIL:[],ATT:[]};if(e.length&&e.every(s=>s.position)){for(const s of["GK","DEF","MIL","ATT"]){const c=e.filter(n=>n.position&&n.position.replace(/\d+$/,"")===s).sort((n,o)=>parseInt(n.position.replace(/\D+/g,""),10)-parseInt(o.position.replace(/\D+/g,""),10)).map(n=>({...n,_line:s})),p=lt(c.length);c.forEach((n,o)=>{n._col=p[o]}),a[s]=c}return a}const d=[...e];for(const s of["GK","DEF","MIL","ATT"]){const c=[];for(let n=0;n<i[s];n++){let o=d.findIndex(f=>f.job===s);if(o===-1&&(o=d.findIndex(f=>f.job2===s)),o===-1&&(o=0),d[o]){const f={...d[o],_line:s};c.push(f),d.splice(o,1)}}const p=lt(c.length);c.forEach((n,o)=>{n._col=p[o]}),a[s]=c}return a}function et(e){document.querySelectorAll(".top-nav, .bottom-nav").forEach(t=>{t.style.setProperty("display","none","important"),t.dataset.matchHidden="1"}),e&&e.style.setProperty("padding-bottom","0","important")}function Ye(e){document.querySelectorAll(".top-nav, .bottom-nav").forEach(t=>{t.style.removeProperty("display"),delete t.dataset.matchHidden}),e&&e.style.removeProperty("padding-bottom")}function ni(e,t,i){const l=new Set,d=t.filter(o=>{const f=o.gc_type||o.id;return l.has(f)?!1:(l.add(f),!0)});let s=[];function c(o,f){const v=o._gcDef,w={purple:"linear-gradient(135deg,#3d0a7a,#7a28b8)",light_blue:"linear-gradient(135deg,#006080,#00bcd4)"},A={purple:"#9b59b6",light_blue:"#00bcd4"},r=w[v==null?void 0:v.color]||w.purple,u=A[v==null?void 0:v.color]||A.purple;return`<div class="gc-select-card" data-id="${o.id}"
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      <div style="flex:1;overflow-y:auto;display:flex;flex-wrap:wrap;gap:10px;justify-content:center;align-content:flex-start;padding:8px 16px 16px">
        ${d.map(A=>{const r=s.find(u=>u.gc_type===A.gc_type);return c(A,!!r)}).join("")}
      </div>
    ...
    </div>`,e.querySelectorAll(".gc-select-card").forEach(A=>{A.addEventListener("click",()=>{const r=A.dataset.id,u=d.find(z=>z.id===r);if(!u)return;const I=s.findIndex(z=>z.gc_type===u.gc_type);I>-1?s.splice(I,1):s.length<3&&s.push(u),n()})}),(f=e.querySelector("#gc-launch"))==null||f.addEventListener("click",()=>{o&&p(s)}),(v=e.querySelector("#gc-no-gc"))==null||v.addEventListener("click",()=>p([])),(w=e.querySelector("#gc-reset"))==null||w.addEventListener("click",()=>{s.length&&(s=[],n())})}n()}function ko(e){var a;const t=((a=e==null?void 0:e.state)==null?void 0:a.params)||{},i=t.matchMode||"vs_ai_easy";return i==="friend"?`Match vs ${t.friendName||"Ami"}`:i==="random"?"Match vs Random":`Match vs IA — ${i.replace("vs_ai_","").toUpperCase()}`}async function Eo(e,t,i){const{state:a,navigate:l}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:d}=await T.from("decks").select("id,name,is_active,formation").eq("owner_id",a.profile.id).order("created_at",{ascending:!1});if(!d||d.length===0){wt(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>l("decks"));return}const s=d.map(o=>o.id),{data:c}=await T.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",s).order("slot_order");let p=0;function n(){var z,C,H,B,j;const o=d[p],f=(c||[]).filter(U=>U.deck_id===o.id),v=f.filter(U=>{var oe;return U.is_starter&&((oe=U.card)==null?void 0:oe.player)}).map(U=>ui(U.card,U.position)),w=f.find(U=>{var oe;return((oe=U.card)==null?void 0:oe.card_type)==="formation"}),A=o.formation||((z=w==null?void 0:w.card)==null?void 0:z.formation)||"4-4-2",r=v.length>=11?yt(v,A):null,u=v.length>=11;et(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;height:${f}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ...
      ${l?`<img src="${l}" style="width:${f-4}px;height:${f-4}px;object-fit:contain">`:e!=null&&e.clubName?`<span style="font-size:${Math.max(4,f-8)}px;font-weight:700;color:#333">${(e.clubName||"").slice(0,3).toUpperCase()}</span>`:""}
    </div>
  </div>`}function nt(e,t,i){if(!(e!=null&&e.length))return"";const a=e.slice(0,5);let l='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return a.forEach((d,s)=>{if(l+=Ke(d,40,52),s<a.length-1){const c=at(d,a[s+1]);l+=`<div style="width:7px;height:3px;background:${c==="#ff3333"||c==="#cc2222"?"rgba(255,255,255,0.12)":c};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(l+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),l+="</div>",l}function Zt(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function hn(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function ut(e,t,i,a,l=310,d=310,s=[]){var C;const c=Nt[t]||{},p=Yi(t)||xi[t]||[],n={},o=["ATT","MIL","DEF","GK"];for(const H of o)(e[H]||[]).forEach((j,U)=>{n[`${H}${U+1}`]=j});function f(H){const B=c[H];return B?{x:B.x*l,y:B.y*d}:null}let v="";for(const[H,B]of p){const j=f(H),U=f(B);if(!j||!U)continue;const oe=n[H],ce=n[B],re=at(oe,ce);re==="#00ff88"||re==="#FFD700"?(v+=`<line x1="${j.x.toFixed(1)}" y1="${j.y.toFixed(1)}" x2="${U.x.toFixed(1)}" y2="${U.y.toFixed(1)}"
  ...
  </div>`}async function oi(e,t,i,a){var B;const{state:l,navigate:d,toast:s}=t;et(e);const c=l.params||{};if(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!c.deckId)return Eo(e,t,i);const p=c.deckId;let n,o,f,v;try{const j=await Promise.all([T.from("decks").select("formation,name").eq("id",p).single(),T.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation,
            player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
              clubs(encoded_name,logo_url)))`).eq("deck_id",p).order("slot_order")]);n=j[0].data,f=j[0].error,o=j[1].data,v=j[1].error}catch(j){console.error("[Match] Exception chargement deck:",j),wt(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>d("home"));return}if(f||v){console.error("[Match] Erreur Supabase:",f||v),wt(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>d("home"));return}const w=(o||[]).filter(j=>{var U;return j.is_starter&&((U=j.card)==null?void 0:U.player)}).map(j=>ui(j.card,j.position)),A=(o||[]).filter(j=>{var U;return!j.is_starter&&((U=j.card)==null?void 0:U.player)}).map(j=>ui(j.card,j.position));if(w.length<11){wt(e,"⚠️",`Deck incomplet (${w.length}/11).`,"Compléter",()=>d("decks"));return}const r=(o||[]).find(j=>{var U;return((U=j.card)==null?void 0:U.card_type)==="formation"}),u=(n==null?void 0:n.formation)||((B=r==null?void 0:r.card)==null?void 0:B.formation)||"4-4-2",{data:I,error:z}=await T.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",l.profile.id).eq("card_type","game_changer"),{data:C}=await T.from("gc_definitions").select("*").eq("is_active",!0),H=(I||[]).map(j=>({...j,_gcDef:(C==null?void 0:C.find(U=>U.name===j.gc_type||U.id===j.gc_definition_id))||null}));a({deckId:p,formation:u,starters:w,subsRaw:A,gcCardsEnriched:H,gcDefs:C||[]})}async function To(e,t){const{state:i}=t,l=(i.params||{}).matchMode||"vs_ai_easy",d=l.replace("vs_ai_",""),s=l;await oi(e,t,l,async({deckId:c,formation:p,starters:n,subsRaw:o,gcCardsEnriched:f,gcDefs:v})=>{try{const w=yt(n,p),A=await Ao(p,d),r=async u=>{try{const{data:I,error:z}=await T.from("matches").insert({home_id:i.profile.id,away_id:null,mode:s,home_deck_id:c,status:"in_progress"}).select().single();if(z){console.error("[MatchIA] Erreur création match:",z),wt(e,"⚠️","Impossible de créer le match ("+z.message+").","Retour",()=>t.navigate("home"));return}const C={gcDefs:v||[],matchId:I==null?void 0:I.id,mode:s,difficulty:d,formation:p,homeTeam:w,aiTeam:A,homeSubs:o,subsUsed:0,maxSubs:Math.min(o.length,3),homeScore:0,aiScore:0,gcCards:u,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Lo(e,C,t)}catch(I){console.error("[MatchIA] Exception launchMatch:",I),wt(e,"⚠️","Erreur au lancement du match : "+I.message,"Retour",()=>t.navigate("home"))}};if(!f.length){r([]);return}ni(e,f,r)}catch(w){console.error("[MatchIA] Exception setup:",w),wt(e,"⚠️","Erreur de préparation du match : "+w.message,"Retour",()=>t.navigate("home"))}})}async function Ao(e,t){var s,c;const{data:i}=await T.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(60);if(!i||i.length<11)return zo(e);const a=Lt[e]||Lt["4-4-2"],l={GK:[],DEF:[],MIL:[],ATT:[]},d=[...i];for(const p of["GK","DEF","MIL","ATT"]){const n=d.filter(A=>A.job===p),o=d.filter(A=>A.job!==p),f=[...n,...o],v=[];for(let A=0;A<a[p];A++){const r=f[A]||d[A];r&&v.push({cardId:"ai-"+r.id+"-"+A,id:r.id,firstname:r.firstname,name:r.surname_encoded,country_code:r.country_code,club_id:r.club_id,job:r.job,job2:r.job2,note_g:Number(r.note_g)||0,note_d:Number(r.note_d)||0,note_m:Number(r.note_m)||0,note_a:Number(r.note_a)||0,rarity:r.rarity,skin:r.skin,hair:r.hair,hair_length:r.hair_length,clubName:((s=r.clubs)==null?void 0:s.encoded_name)||null,clubLogo:((c=r.clubs)==null?void 0:c.logo_url)||null,boost:0,used:!1,_line:p})}const w=lt(v.length);v.forEach((A,r)=>{A._col=w[r]}),l[p]=v}return l}function zo(e){const t=Lt[e]||Lt["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},a=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let l=0;for(const d of["GK","DEF","MIL","ATT"]){const s=[];for(let p=0;p<t[d];p++){const n=3+Math.floor(Math.random()*5);s.push({cardId:"fake-"+l,id:"fake-"+l,firstname:"IA",name:a[l%a.length],country_code:"XX",club_id:null,job:d,job2:null,note_g:d==="GK"?n:2,note_d:d==="DEF"?n:2,note_m:d==="MIL"?n:2,note_a:d==="ATT"?n:2,rarity:"normal",boost:0,used:!1,_line:d}),l++}const c=lt(s.length);s.forEach((p,n)=>{p._col=c[n]}),i[d]=s}return i}function Lo(e,t,i){e.innerHTML=`
  ...
  </div>`;function I(){const D=e.querySelector(".match-screen");if(!D)return;const ie=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);D.style.bottom="auto",D.style.height=ie+"px",D.style.minHeight=ie+"px",D.style.maxHeight=ie+"px",D.style.overflow="hidden";const W=e.querySelector("#mobile-action-bar"),G=e.querySelector("#mobile-play-area");W&&G&&(G.style.paddingBottom=W.offsetHeight+"px")}if(I(),setTimeout(I,120),setTimeout(I,400),setTimeout(I,1e3),t._vvBound||(t._vvBound=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",I),window.visualViewport.addEventListener("scroll",I)),window.addEventListener("resize",I)),function(){const ie=e.querySelector(".terrain-wrapper svg");ie&&(ie.removeAttribute("width"),ie.removeAttribute("height"),ie.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",ie.setAttribute("viewBox","-26 -26 352 352"),ie.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),t._resizeBound||(t._resizeBound=!0,window.addEventListener("resize",()=>{const D=e.querySelector(".terrain-wrapper svg");D&&(D.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0")})),t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase==="attack"||t.phase==="defense"){let D=!1,ie=30;const W=()=>document.getElementById("match-timer"),G=()=>{const se=W();if(!se)return;const fe=String(Math.floor(ie/60)).padStart(2,"0"),xe=String(ie%60).padStart(2,"0");se.textContent=` ${fe}:${xe}`,se.style.color=D?"#ff2222":"#ff9500",se.style.fontWeight="900"};G(),t._timerInt=setInterval(()=>{if(ie--,ie<0)if(!D)D=!0,ie=15,G();else{clearInterval(t._timerInt),t._timerInt=null,t.homeScore=0,t.aiScore=3;const se=document.createElement("div");se.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;color:#fff;padding:24px;text-align:center",se.innerHTML='<div style="font-size:56px">⏱️</div><div style="font-size:24px;font-weight:900;color:#ff4444">MATCH PERDU PAR FORFAIT</div><div style="font-size:14px;color:rgba(255,255,255,0.6)">Temps écoulé</div>',document.body.appendChild(se),setTimeout(()=>{se.remove(),Ft(e,t,i)},2500)}else G()},1e3)}(C=document.getElementById("match-quit"))==null||C.addEventListener("click",()=>{Ye(e),confirm("Abandonner ? Résultat : défaite 3-0")&&(t.homeScore=0,t.aiScore=3,Ft(e,t,i))}),(H=document.getElementById("view-ai"))==null||H.addEventListener("click",()=>No(t,i)),(B=document.getElementById("toggle-history"))==null||B.addEventListener("click",()=>{var D;(D=document.getElementById("match-history-panel"))==null||D.classList.add("open")}),(j=document.getElementById("close-history"))==null||j.addEventListener("click",()=>{var D;(D=document.getElementById("match-history-panel"))==null||D.classList.remove("open")}),(U=document.getElementById("btn-action"))==null||U.addEventListener("click",()=>{t.selected.length!==0&&(v?Co(e,t,i):w&&jo(e,t,i))}),(oe=document.getElementById("btn-results"))==null||oe.addEventListener("click",()=>Ft(e,t,i)),e.querySelectorAll(".match-slot-hit").forEach(D=>{D.addEventListener("click",()=>Mo(D,t,e,i))}),e.querySelectorAll(".match-used-hit").forEach(D=>{D.addEventListener("click",()=>si(e,t,i,null,D.dataset.cardId))}),e.querySelectorAll(".gc-mini").forEach(D=>{D.addEventListener("click",()=>Bo(D.dataset.gcId,D.dataset.gcType,e,t,i))}),(ce=document.getElementById("boost-card"))==null||ce.addEventListener("click",()=>Po(e,t,i)),e.querySelectorAll(".sub-btn-col").forEach(D=>{D.addEventListener("click",()=>si(e,t,i,D.dataset.subId))}),(re=document.getElementById("sub-btn-main"))==null||re.addEventListener("click",()=>si(e,t,i))}function Mo(e,t,i,a){const l=e.dataset.cardId,d=e.dataset.role,s=t.selected.findIndex(c=>c.cardId===l);if(s!==-1)t.selected.splice(s,1);else{if(t.selected.length>=3){a.toast("Maximum 3 joueurs","error");return}const c=(t.homeTeam[d]||[]).find(p=>p.cardId===l);c&&t.selected.push({...c,_role:d,_line:d})}Je(i,t,a)}function Ei(e,t,i){e.matchId&&T.from("matches").update({last_player_id:i}).eq("id",e.matchId).then(()=>{})}function Co(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),Ei(t,i,i.state.profile.id);const a=!["GK","DEF","MIL","ATT"].some(s=>(t.aiTeam[s]||[]).some(c=>!c.used)),l=t.selected.map(s=>{const c=(t.homeTeam[s._role]||[]).find(n=>n.cardId===s.cardId)||s,p=a&&["GK","DEF"].includes(s._role);return{...c,_line:s._role,...p?{note_a:Math.max(1,Number(c.note_a)||0)}:{}}}),d=Rt(l,t.modifiers.home);t.pendingAttack={...d,players:[...l],side:"home"},t.selected.forEach(s=>{const c=(t.homeTeam[s._role]||[]).find(p=>p.cardId===s.cardId);c&&(c.used=!0)}),t.log.push({text:`⚔️ Vous attaquez : ${d.total} (base ${d.base}${d.links?` +${d.links} liens`:""}) — ${t.selected.map(s=>s.name).join(", ")}`,type:"info"}),t.selected=[],t.modifiers.home={},t.phase="ai-defense",Je(e,t,i),setTimeout(()=>qo(e,t,i),1200)}function jo(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),Ei(t,i,i.state.profile.id);const a=t.selected.map(c=>({...(t.homeTeam[c._role]||[]).find(n=>n.cardId===c.cardId)||c,_line:c._role})),l=Ot(a,t.modifiers.home);t.selected.forEach(c=>{const p=(t.homeTeam[c._role]||[]).find(n=>n.cardId===c.cardId);p&&(p.used=!0)});const d=Ht(t.pendingAttack.total,l.total,t.modifiers.home),s={type:"duel",title:"Défense",aiPlayers:(t.pendingAttack.players||[]).map(c=>({name:c.name,note:c._line==="MIL"?c.note_m:c.note_a,portrait:qe(c),job:c.job,country_code:c.country_code,rarity:c.rarity,clubName:c.clubName,clubLogo:c.clubLogo})),homePlayers:t.selected.map(c=>{const p=(t.homeTeam[c._role]||[]).find(n=>n.cardId===c.cardId)||c;return{name:p.name,note:(p._line==="GK"?Number(p.note_g)||0:p._line==="MIL"?Number(p.note_m)||0:Number(p.note_d)||0)+(p.boost||0),portrait:qe(p),job:p.job,country_code:p.country_code,rarity:p.rarity,clubName:p.clubName,clubLogo:p.clubLogo}}),homeTotal:l.total,aiTotal:t.pendingAttack.total,isGoal:!1,homeScored:!1,text:""};if(d.shielded)s.text="🛡️ Bouclier ! But annulé.",t.log.push(s);else if(d.goal){t.aiScore++,s.isGoal=!0,s.homeScored=!1,s.text=`⚽ BUT IA ! (${t.pendingAttack.total} > ${l.total})`,t.log.push(s),t.selected=[],t.modifiers.home={},t.pendingAttack=null,Je(e,t,i),Qt(s.aiPlayers,t.homeScore,t.aiScore,!1,()=>{Tt(e,t,i,"home-attack")});return}else s.text=`🧤 Défense réussie ! (${l.total} ≥ ${t.pendingAttack.total})`,t.log.push(s);t.selected=[],t.modifiers.home={},t.pendingAttack=null,Tt(e,t,i,"home-attack")}function fi(e,t,i){Ei(t,i,null);const a=[...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]],l=tn(a,"attack",t.difficulty);if(!l.length){Jt(e,t,i);return}const d=Rt(l,t.modifiers.ai);t.pendingAttack={...d,players:l,side:"ai"},l.forEach(n=>{n.used=!0}),t.log.push({text:`🤖 IA attaque : ${d.total} (${l.map(n=>n.name).join(", ")})`,type:"info"}),t.modifiers.ai={};const s=[...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(n=>!n.used),p=(t.homeSubs||[]).filter(n=>!(t.usedSubIds||[]).includes(n.cardId)).length>0&&t.subsUsed<t.maxSubs;if(s.length===0&&!p){t.aiScore++;const n={type:"duel",isGoal:!0,homeScored:!1,aiPlayers:l.map(o=>({name:o.name,note:o._line==="MIL"?o.note_m:o.note_a,portrait:qe(o),job:o.job,country_code:o.country_code,rarity:o.rarity,clubName:o.clubName,clubLogo:o.clubLogo})),aiTotal:d.total,text:"⚽ BUT IA ! (aucun défenseur disponible)"};t.log.push(n),t.pendingAttack=null,Je(e,t,i),Qt(n.aiPlayers,t.homeScore,t.aiScore,!1,()=>{Tt(e,t,i,"home-attack")});return}t.phase="defense",Je(e,t,i)}function qo(e,t,i){var n,o;const a=[...t.aiTeam.GK||[],...t.aiTeam.DEF||[],...t.aiTeam.MIL||[]],l=tn(a,"defense",t.difficulty);if(!["GK","DEF","MIL","ATT"].flatMap(f=>(t.aiTeam[f]||[]).filter(v=>!v.used)).length){t.homeScore++;const v={type:"duel",isGoal:!0,homeScored:!0,homePlayers:(((n=t.pendingAttack)==null?void 0:n.players)||[]).map(w=>({name:w.name,note:$e(w,w._line||w.job),portrait:qe(w),job:w.job,country_code:w.country_code,rarity:w.rarity,clubName:w.clubName,clubLogo:w.clubLogo})),homeTotal:((o=t.pendingAttack)==null?void 0:o.total)||0,aiTotal:0,text:"⚽ BUT ! L'IA n'a plus de joueurs — but automatique !"};t.log.push(v),t.modifiers.ai={},t.pendingAttack=null,Je(e,t,i),Qt(v.homePlayers,t.homeScore,t.aiScore,!0,()=>{Tt(e,t,i,"ai-attack")});return}const s=l.length>0?Ot(l,t.modifiers.ai).total:0;l.forEach(f=>{f.used=!0});const c=Ht(t.pendingAttack.total,s,t.modifiers.ai),p={type:"duel",title:"Attaque",homePlayers:(t.pendingAttack.players||[]).map(f=>({name:f.name,note:f._line==="MIL"?f.note_m:f.note_a,portrait:qe(f),job:f.job,country_code:f.country_code,rarity:f.rarity,clubName:f.clubName,clubLogo:f.clubLogo})),aiPlayers:l.map(f=>({name:f.name,note:f._line==="GK"?f.note_g:f._line==="MIL"?f.note_m:f.note_d,portrait:qe(f),job:f.job,country_code:f.country_code,rarity:f.rarity,clubName:f.clubName,clubLogo:f.clubLogo})),homeTotal:t.pendingAttack.total,aiTotal:s,isGoal:!1,homeScored:!1,text:""};if(c.shielded)p.text="🛡️ Bouclier IA !",t.log.push(p);else if(c.goal){t.homeScore++,p.isGoal=!0,p.homeScored=!0,p.text=`⚽ BUT ! (${t.pendingAttack.total} > ${s})`,t.log.push(p),t.modifiers.ai={},t.pendingAttack=null,Je(e,t,i),Qt(p.homePlayers,t.homeScore,t.aiScore,!0,()=>{Tt(e,t,i,"ai-attack")});return}else p.text=`🧤 IA défend (${s} ≥ ${t.pendingAttack.total})`,t.log.push(p);t.modifiers.ai={},t.pendingAttack=null,Tt(e,t,i,"ai-attack")}function Tt(e,t,i,a){if(t.round++,vn(t)){Ft(e,t,i);return}if(a==="home-attack"){if(![...t.homeTeam.MIL||[],...t.homeTeam.ATT||[]].filter(d=>!d.used).length){if(![...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(c=>!c.used).length){Jt(e,t,i);return}if(![...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]].filter(c=>!c.used).length){Jt(e,t,i);return}setTimeout(()=>fi(e,t,i),100);return}t.phase="attack",Je(e,t,i)}else{if(![...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]].filter(d=>!d.used).length){Jt(e,t,i);return}t.phase="ai-attack",Je(e,t,i),setTimeout(()=>fi(e,t,i),800)}}function vn(e){const t=["MIL","ATT","GK","DEF"].some(a=>(e.homeTeam[a]||[]).some(l=>!l.used)),i=["MIL","ATT","GK","DEF"].some(a=>(e.aiTeam[a]||[]).some(l=>!l.used));return!t&&!i}function Jt(e,t,i){vn(t)?Ft(e,t,i):(t.phase="attack",Je(e,t,i))}function Fo(e,t,i){const a=document.createElement("div");a.style.cssText=`
    position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:800;
    display:flex;flex-direction:column;align-items:center;justify-content:center;gap:20px;
    animation:subFadeIn 0.2s ease;
  `;const l=qe(e),d=qe(t),s=St[e.job]||"#555",c=St[t.job]||"#555",p=e.job==="GK"?e.note_g:e.job==="DEF"?e.note_d:e.job==="MIL"?e.note_m:e.note_a,n=t.job==="GK"?t.note_g:t.job==="DEF"?t.note_d:t.job==="MIL"?t.note_m:t.note_a;a.innerHTML=`
    <style>
  ...
  `,document.body.appendChild(a);let o=!1;const f=()=>{o||(o=!0,a.remove(),i())};a.addEventListener("click",f),setTimeout(f,2e3)}function It(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function si(e,t,i,a=null,l=null){var w,A;if(t.phase!=="attack"){It("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){It(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const d=Object.entries(t.homeTeam).flatMap(([r,u])=>(u||[]).filter(I=>I.used).map(I=>({...I,_line:I._line||r}))),s=t.homeSubs.filter(r=>!t.usedSubIds.includes(r.cardId));if(!d.length){It("Aucun joueur utilisé à remplacer");return}if(!s.length){It("Aucun remplaçant disponible");return}let c=Math.max(0,d.findIndex(r=>r.cardId===l));const p=((w=d[c])==null?void 0:w._line)||((A=d[c])==null?void 0:A.job);let n=a?Math.max(0,s.findIndex(r=>r.cardId===a)):Math.max(0,s.findIndex(r=>r.job===p)),o=!1;const f=document.createElement("div");f.id="sub-overlay",f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function v(){var B,j,U,oe,ce,re;const r=d[c],u=s[n],I=Math.min(130,Math.round((window.innerWidth-90)/2)),z=Math.round(I*1.35),C=D=>`background:rgba(255,255,255,0.12);border:none;color:${D?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${D?"default":"pointer"};flex-shrink:0`;f.innerHTML=`
    ...
    </div>`,(B=f.querySelector("#sub-close"))==null||B.addEventListener("click",()=>f.remove()),(j=f.querySelector("#out-up"))==null||j.addEventListener("click",()=>{c>0&&(c--,v())}),(U=f.querySelector("#out-down"))==null||U.addEventListener("click",()=>{c<d.length-1&&(c++,v())}),(oe=f.querySelector("#in-up"))==null||oe.addEventListener("click",()=>{n>0&&(n--,v())}),(ce=f.querySelector("#in-down"))==null||ce.addEventListener("click",()=>{n<s.length-1&&(n++,v())});const H=(D,ie,W,G)=>{const se=f.querySelector("#"+D);if(!se)return;let fe=0;se.addEventListener("touchstart",xe=>{fe=xe.touches[0].clientY},{passive:!0}),se.addEventListener("touchend",xe=>{const Ee=xe.changedTouches[0].clientY-fe;if(Math.abs(Ee)<30)return;const _e=ie();Ee<0&&_e<G-1?(W(_e+1),v()):Ee>0&&_e>0&&(W(_e-1),v())},{passive:!0})};H("in-panel",()=>n,D=>n=D,s.length),H("out-panel",()=>c,D=>c=D,d.length),(re=f.querySelector("#sub-confirm"))==null||re.addEventListener("click",D=>{if(D.preventDefault(),D.stopPropagation(),o)return;o=!0;const ie=d[c],W=s[n];if(!ie||!W)return;let G=null,se=-1;for(const[xe,Ee]of Object.entries(t.homeTeam)){const _e=(Ee||[]).findIndex(he=>he.cardId===ie.cardId);if(_e!==-1){G=xe,se=_e;break}}if(se===-1||!G){It("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),f.remove();return}const fe={...W,_line:G,_col:ie._col||0,used:!1,boost:0};t.homeTeam[G].splice(se,1,fe),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(W.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:ie.name,firstname:ie.firstname,note:$e(ie,G),portrait:qe(ie),job:ie.job,country_code:ie.country_code,rarity:ie.rarity,clubName:ie.clubName,clubLogo:ie.clubLogo},inPlayer:{name:W.name,firstname:W.firstname,note:$e(W,G),portrait:qe(W),job:W.job,country_code:W.country_code,rarity:W.rarity,clubName:W.clubName,clubLogo:W.clubLogo},text:`🔄 ${W.firstname} ${W.name} remplace ${ie.firstname} ${ie.name}`}),f.remove(),Fo(ie,W,()=>Je(e,t,i))})}document.body.appendChild(f),v()}function Bo(e,t,i,a,l){var A,r;const d=(a.gcDefs||[]).find(u=>u.name===t),s=Ue[t]||{icon:"⚡",desc:"Carte spéciale."},c={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[d==null?void 0:d.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",p={purple:"#b06ce0",light_blue:"#00d4ef"}[d==null?void 0:d.color]||"#b06ce0",n=(d==null?void 0:d.name)||t,o=(d==null?void 0:d.effect)||s.desc,f=d!=null&&d.image_url?`/manager-wars/icons/${d.image_url}`:null,v=s.icon||"⚡",w=document.createElement("div");w.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",w.innerHTML=`
    ...
    </div>`,document.body.appendChild(w),(A=w.querySelector("#gc-back"))==null||A.addEventListener("click",()=>w.remove()),(r=w.querySelector("#gc-use"))==null||r.addEventListener("click",()=>{w.remove(),Go(e,t,i,a,l)})}function Yt(e,t,i,a,l,d){const s=document.createElement("div");s.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let c=[];function p(){var n,o;s.innerHTML=`
    ...
    <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
      ${e.map(f=>{const v=f._line||f.job||"MIL",w={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[v]||"#555",A=$e(f,v)+(f.boost||0),r=c.find(u=>u.cardId===f.cardId);return`<div class="gc-pick-item" data-cid="${f.cardId}"
    ...
    </div>`,(n=s.querySelector("#gc-picker-close"))==null||n.addEventListener("click",()=>s.remove()),s.querySelectorAll(".gc-pick-item").forEach(f=>{f.addEventListener("click",()=>{const v=f.dataset.cid,w=e.find(r=>r.cardId===v);if(!w)return;const A=c.findIndex(r=>r.cardId===v);A>-1?c.splice(A,1):c.length<t&&c.push(w),p()})}),(o=s.querySelector("#gc-picker-confirm"))==null||o.addEventListener("click",()=>{s.remove(),d(c)})}p(),document.body.appendChild(s)}const Do={BOOST_STAT:({value:e=1,count:t=1,roles:i=[]},a,l,d)=>{const s=Object.entries(a.homeTeam).filter(([c])=>!i.length||i.includes(c)).flatMap(([c,p])=>p.filter(n=>!n.used).map(n=>({...n,_line:c})));return s.length?(Yt(s,t,`Choisir ${t} joueur(s) à booster (+${e})`,l,a,c=>{c.forEach(p=>{const n=(a.homeTeam[p._line]||[]).find(o=>o.cardId===p.cardId);n&&(n.boost=(n.boost||0)+e,a.log.push({text:`⚡ +${e} sur ${n.name}`,type:"info"}))}),Je(l,a,d)}),!0):(a.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),Je(l,a,d),!0)},DEBUFF_STAT:({value:e=1,count:t=1,roles:i=[],target:a="ai"},l,d,s)=>{const c=a==="home"?l.homeTeam:l.aiTeam,p=a==="ai"?"adverse":"allié",n=Object.entries(c).filter(([o])=>!i.length||i.includes(o)).flatMap(([o,f])=>f.filter(v=>!v.used).map(v=>({...v,_line:o})));return n.length?(Yt(n,t,`Choisir ${t} joueur(s) ${p}(s) à débuffer (-${e})`,d,l,o=>{o.forEach(f=>{const w=((a==="home"?l.homeTeam:l.aiTeam)[f._line]||[]).find(A=>A.cardId===f.cardId);w&&(w.boost=(w.boost||0)-e,l.log.push({text:`🎯 -${e} sur ${w.name}${a==="ai"?" (IA)":""}`,type:"info"}))}),Je(d,l,s)}),!0):(l.log.push({text:`🎯 Aucun joueur ${p} disponible`,type:"info"}),Je(d,l,s),!0)},GRAY_PLAYER:({count:e=1,roles:t=[],target:i="ai"},a,l,d)=>{const s=i==="home"?a.homeTeam:a.aiTeam,c=i==="ai"?"adverse":"allié",p=Object.entries(s).filter(([n])=>!t.length||t.includes(n)).flatMap(([n,o])=>o.filter(f=>!f.used).map(f=>({...f,_line:n})));return p.length?(Yt(p,e,`Choisir ${e} joueur(s) ${c}(s) à exclure`,l,a,n=>{n.forEach(o=>{const v=((i==="home"?a.homeTeam:a.aiTeam)[o._line]||[]).find(w=>w.cardId===o.cardId);v&&(v.used=!0,a.log.push({text:`❌ ${v.name}${i==="ai"?" (IA)":""} exclu !`,type:"info"}))}),Je(l,a,d)}),!0):(a.log.push({text:`❌ Aucun joueur ${c} à exclure`,type:"info"}),Je(l,a,d),!0)},REVIVE_PLAYER:({count:e=1,roles:t=[]},i,a,l)=>{const d=Object.entries(i.homeTeam).filter(([s])=>!t.length||t.includes(s)).flatMap(([s,c])=>c.filter(p=>p.used).map(p=>({...p,_line:s})));return d.length?(Yt(d,e,`Choisir ${e} joueur(s) à ressusciter`,a,i,s=>{s.forEach(c=>{const p=(i.homeTeam[c._line]||[]).find(n=>n.cardId===c.cardId);p&&(p.used=!1,i.log.push({text:`💫 ${p.name} ressuscité !`,type:"info"}))}),Je(a,i,l)}),!0):(i.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),Je(a,i,l),!0)},REMOVE_GOAL:({},e)=>(e.aiScore>0?(e.aiScore--,e.log.push({text:"🚫 Dernier but IA annulé !",type:"info"})):e.log.push({text:"🚫 Aucun but à annuler",type:"info"}),!1),ADD_GOAL_DRAW:({},e)=>(e.homeScore===e.aiScore?(e.homeScore++,e.log.push({text:"🎯 But bonus (match nul) !",type:"info"})):e.log.push({text:"🎯 But bonus : non applicable (pas de match nul)",type:"info"}),!1),ADD_SUB:({value:e=1},t)=>(t.maxSubs=(t.maxSubs||3)+e,t.log.push({text:`🔄 +${e} remplacement(s) débloqué(s)`,type:"info"}),!1),CUSTOM:()=>!1};function Go(e,t,i,a,l){a.usedGc.push(e);const d=a.gcDefs||[],s=d.find(p=>p.name===t)||d.find(p=>{var n;return((n=p.name)==null?void 0:n.toLowerCase().trim())===(t==null?void 0:t.toLowerCase().trim())});let c=!1;if(s!=null&&s.effect_type&&s.effect_type!=="CUSTOM"){const p=Do[s.effect_type];p?p(s.effect_params||{},a,i,l)||(c=!0):(l.toast(`Effet "${s.effect_type}" non implémenté`,"error"),c=!0)}else{switch(t){case"Double attaque":a.modifiers.home.doubleAttack=!0,a.log.push({text:"⚡ Double attaque activée !",type:"info"});break;case"Bouclier":a.modifiers.home.shield=!0,a.log.push({text:"🛡️ Bouclier activé !",type:"info"});break;case"Ressusciter":{const p=Object.entries(a.homeTeam).flatMap(([n,o])=>(o||[]).filter(f=>f.used).map(f=>({...f,_line:n})));p.length?(p[0].used=!1,a.log.push({text:`💫 ${p[0].name} ressuscité !`,type:"info"})):a.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"});break}case"Vol de note":a.modifiers.ai.stolenNote=(a.modifiers.ai.stolenNote||0)+1,a.log.push({text:"🎯 -1 à la prochaine attaque IA",type:"info"});break;case"Gel":{const p=[...a.aiTeam.ATT||[],...a.aiTeam.MIL||[]].filter(n=>!n.used);if(p.length){const n=p.sort((o,f)=>$e(f,"ATT")-$e(o,"ATT"))[0];n.used=!0,a.log.push({text:`❄️ ${n.name} (IA) gelé !`,type:"info"})}break}case"Remplacement+":a.maxSubs++,a.log.push({text:"🔄 +1 remplacement débloqué",type:"info"});break}c=!0}T.from("cards").delete().eq("id",e).then(()=>{}),c&&Je(i,a,l)}function Po(e,t,i){const a=Object.values(t.homeTeam).flat().filter(l=>!l.used);if(!a.length){i.toast("Aucun joueur actif à booster","error");return}i.openModal("⚡ Utiliser le Boost",`<div style="margin-bottom:12px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border-radius:10px;padding:12px;text-align:center;color:#000">
    ...
    <div style="display:flex;flex-direction:column;gap:6px">
      ${a.map(l=>`
        <div class="player-boost-opt" data-card-id="${l.cardId}"
          ...
          <div style="width:32px;height:32px;background:${St[l.job]||"#888"};border-radius:6px;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:900;font-size:13px">${$e(l,l._line||l.job)}</div>
          <div style="flex:1"><b>${l.firstname} ${l.name}</b><div style="font-size:11px;color:#888">${l._line||l.job}</div></div>
          <div style="color:#87CEEB;font-weight:700">+${t.boostCard.value}</div>
        </div>`).join("")}
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(l=>{l.addEventListener("click",()=>{const d=l.dataset.cardId;for(const s of["GK","DEF","MIL","ATT"]){const c=(t.homeTeam[s]||[]).find(p=>p.cardId===d);if(c){c.boost=(c.boost||0)+t.boostCard.value,t.log.push({text:`⚡ Boost +${t.boostCard.value} appliqué à ${c.name}`,type:"info"});break}}t.boostUsed=!0,i.closeModal(),Je(e,t,i)})})}function Qt(e,t,i,a,l){const d=document.createElement("div");d.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:900;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;overflow:hidden;cursor:pointer";const s=Array.from({length:10},(n,o)=>`
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(d);let c=!1;const p=()=>{c||(c=!0,d.remove(),setTimeout(()=>l(),50))};d.addEventListener("click",p),setTimeout(p,3500)}async function Ft(e,t,i){var o,f;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:a}=i,l=t.homeScore>t.aiScore,d=t.homeScore===t.aiScore,s=l?"victoire":d?"nul":"defaite",c=Vn(t.mode,s);t.matchId&&await T.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:l?a.profile.id:null,home_credits_reward:c,played_at:new Date().toISOString()}).eq("id",t.matchId);const p={credits:(a.profile.credits||0)+c,matches_played:(a.profile.matches_played||0)+1};l?p.wins=(a.profile.wins||0)+1:d?p.draws=(a.profile.draws||0)+1:p.losses=(a.profile.losses||0)+1,await T.from("users").update(p).eq("id",a.profile.id),await i.refreshProfile();const n=document.createElement("div");n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",n.innerHTML=`
    ...
    </div>`,`<button class="btn btn-primary" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}async function wn(e,t){var i,a;try{const l=(a=(i=t==null?void 0:t.state)==null?void 0:i.profile)==null?void 0:a.id;try{(T.getChannels?T.getChannels():[]).forEach(s=>{const c=s.topic||"";(c.includes("matchmaking")||c.includes("pvp-match")||c.includes("friend-invite"))&&T.removeChannel(s)})}catch(d){console.warn("[Random] cleanup canaux:",d)}l&&await T.rpc("cancel_matchmaking",{p_user_id:l}).catch(()=>{})}catch{}await oi(e,t,"random",({deckId:l,formation:d,starters:s,subsRaw:c,gcCardsEnriched:p,gcDefs:n})=>{const o=f=>Ro(e,t,l,d,s,c,f,n||[]);if(!p.length){o([]);return}ni(e,p,o)})}async function Ro(e,t,i,a,l,d,s=[],c=[]){var z;const{state:p,navigate:n,toast:o}=t;let f=!1,v=null;et(e),e.innerHTML=`
    ...
    </div>`,(g=document.getElementById("pvp-home"))==null||g.addEventListener("click",()=>{try{T.removeChannel(j)}catch{}Ye(e),c("home")})}G()}const Ho=Object.freeze(Object.defineProperty({__proto__:null,renderMatchRandom:wn,resumePvpMatch:Oo},Symbol.toStringTag,{value:"Module"}));async function Uo(e,t,i,a){var l,d;try{const s=(d=(l=t==null?void 0:t.state)==null?void 0:l.profile)==null?void 0:d.id;try{(T.getChannels?T.getChannels():[]).forEach(p=>{const n=p.topic||"";(n.includes("matchmaking")||n.includes("pvp-match"))&&T.removeChannel(p)})}catch(c){console.warn("[FriendMatch] cleanup canaux:",c)}s&&(await T.rpc("cancel_matchmaking",{p_user_id:s}).catch(()=>{}),await T.from("matchmaking_queue").delete().eq("user_id",s).then(()=>{},()=>{}))}catch{}await oi(e,t,"random",({deckId:s,formation:c,starters:p,subsRaw:n,gcCardsEnriched:o,gcDefs:f})=>{const v=w=>Vo(e,t,s,c,p,n,w,f||[],i,a);if(!o.length){v([]);return}ni(e,o,v)})}async function Vo(e,t,i,a,l,d,s=[],c=[],p,n){var ce;const{state:o,navigate:f,toast:v}=t,w=o.profile.id;let A=!1,r=null;et(e);try{await T.rpc("cancel_matchmaking",{p_user_id:w})}catch{}try{await T.from("matchmaking_queue").delete().eq("user_id",w)}catch{}const u=(re,D,ie,W)=>{var G;e.innerHTML=`
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      ...
      <button id="pvp-home" style="padding:16px 40px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">🏠 Retour</button>
    </div>`,(x=document.getElementById("pvp-home"))==null||x.addEventListener("click",()=>{try{T.removeChannel(oe)}catch{}Ye(e),c("home")})}fe()}const At="#1A6B3C",xt="#D4A017";async function Yo(e,t){var a;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const i=(a=t.state.params)==null?void 0:a.openLeagueId;if(i){t.state.params.openLeagueId=null,await bt(e,t,i);return}await _t(e,t)}async function _t(e,t,i="waiting"){var I;const{state:a}=t,l=a.profile.id,{data:d}=await T.from("mini_league_members").select("league_id").eq("user_id",l),s=(d||[]).map(z=>z.league_id),{data:c,error:p}=await T.from("mini_leagues").select("*, mini_league_members(count)").eq("status","waiting").eq("is_archived",!1).order("created_at",{ascending:!1}).limit(30),n=p?(await T.from("mini_leagues").select("*, mini_league_members(count)").eq("status","waiting").order("created_at",{ascending:!1}).limit(30)).data||[]:c||[],f=(s.length?await T.from("mini_leagues").select("*, mini_league_members(count)").in("id",s).order("created_at",{ascending:!1}):{data:[]}).data||[],v=f.filter(z=>z.status==="waiting"&&!z.is_archived),w=f.filter(z=>z.status==="active"&&!z.is_archived),A=f.filter(z=>z.is_archived||z.status==="finished"),r=n.filter(z=>!s.includes(z.id)),u=[{key:"waiting",label:"🟡 En attente",count:v.length+r.length},{key:"active",label:"🟢 En cours",count:w.length},{key:"archived",label:"📁 Archivées",count:A.length}];e.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:10px">
      ${i==="waiting"?Wo(v,r,l):i==="active"?Jo(w,l):Xo(A,l)}
    </div>
  </div>`,(I=document.getElementById("ml-create-btn"))==null||I.addEventListener("click",()=>Zo(e,t)),e.querySelectorAll(".ml-tab").forEach(z=>z.addEventListener("click",()=>_t(e,t,z.dataset.tab))),e.querySelectorAll("[data-league-id]").forEach(z=>z.addEventListener("click",()=>bt(e,t,z.dataset.leagueId))),e.querySelectorAll("[data-join]").forEach(z=>z.addEventListener("click",C=>{C.stopPropagation(),$n(e,t,z.dataset.join,z.dataset.type)})),e.querySelectorAll("[data-delete]").forEach(z=>z.addEventListener("click",C=>{C.stopPropagation(),gi(e,t,z.dataset.delete,z.dataset.name,i)}))}function ei(e,t,i=!1){var s,c;const a=e.creator_id===t,l=e.pot||0,d=((c=(s=e.mini_league_members)==null?void 0:s[0])==null?void 0:c.count)||0;return`<div data-league-id="${e.id}" style="background:#fff;border-radius:12px;padding:14px 16px;box-shadow:0 1px 6px rgba(0,0,0,0.08);cursor:pointer;margin-bottom:8px">
    ...
    <div style="padding:20px;display:flex;flex-direction:column;gap:14px">
      ...
      </div>`,document.body.appendChild(t);const i=t.querySelector("#ml-pwd-input"),a=l=>{t.remove(),e(l)};setTimeout(()=>i==null?void 0:i.focus(),80),t.querySelector("#pwd-cancel").addEventListener("click",()=>a(null)),t.querySelector("#pwd-ok").addEventListener("click",()=>a(i.value.trim())),i.addEventListener("keydown",l=>{l.key==="Enter"&&a(i.value.trim())}),t.addEventListener("click",l=>{l.target===t&&a(null)})})}async function $n(e,t,i,a){const{toast:l,state:d}=t,s=d.profile.id,{data:c}=await T.from("mini_leagues").select("password,status,entry_fee,pot,max_players").eq("id",i).single();if(!c){l("Mini League introuvable","error");return}if(c.status!=="waiting"){l("Cette Mini League a déjà démarré","warning");return}const{count:p}=await T.from("mini_league_members").select("id",{count:"exact",head:!0}).eq("league_id",i);if(p>=c.max_players){l("La Mini League est complète","warning");return}if(a==="private"){const r=await Qo();if(r===null)return;if(c.password!==r){l("Mot de passe incorrect","error");return}}const n=c.entry_fee||100,{data:o}=await T.from("users").select("credits").eq("id",s).single();if(((o==null?void 0:o.credits)||0)<n){l(`Crédits insuffisants — il te faut ${n.toLocaleString("fr")} cr. (solde : ${((o==null?void 0:o.credits)||0).toLocaleString("fr")} cr.)`,"error");return}const{error:f}=await T.from("mini_league_members").insert({league_id:i,user_id:s});if(f){l("Erreur inscription : "+f.message,"error");return}const{error:v}=await T.from("users").update({credits:o.credits-n}).eq("id",s),{error:w}=await T.from("mini_leagues").update({pot:(c.pot||0)+n}).eq("id",i);(v||w)&&console.error("Erreur post-inscription:",v,w),d.profile&&(d.profile.credits=o.credits-n);const A=document.getElementById("nav-credits");A&&(A.textContent=`💰 ${(d.profile.credits||0).toLocaleString("fr")}`),l(`✅ Inscrit ! −${n.toLocaleString("fr")} cr. · Pot : ${((c.pot||0)+n).toLocaleString("fr")} cr.`,"success"),bt(e,t,i)}async function er(e,t,i,a){const{toast:l,state:d}=t,s=d.profile.id;if(!confirm(`Te désinscrire et récupérer ${a.toLocaleString("fr")} cr. ?`))return;const{data:c}=await T.from("mini_leagues").select("pot,status").eq("id",i).single();if(!c||c.status!=="waiting"){l("Impossible — la league a déjà démarré","error");return}const{data:p}=await T.from("users").select("credits").eq("id",s).single();await T.from("mini_leagues").update({pot:Math.max(0,(c.pot||0)-a)}).eq("id",i),await T.from("users").update({credits:((p==null?void 0:p.credits)||0)+a}).eq("id",s),await T.from("mini_league_members").delete().eq("league_id",i).eq("user_id",s),d.profile&&(d.profile.credits=((p==null?void 0:p.credits)||0)+a);const n=document.getElementById("nav-credits");n&&(n.textContent=`💰 ${(d.profile.credits||0).toLocaleString("fr")}`),l(`↩️ Désinscrit · +${a.toLocaleString("fr")} cr. remboursés`,"success"),_t(e,t,"waiting")}async function gi(e,t,i,a,l){const{toast:d}=t;if(!confirm(`Supprimer définitivement "${a}" et tous ses matchs/membres ? Action irréversible.`))return;await T.from("mini_league_matches").delete().eq("league_id",i),await T.from("mini_league_members").delete().eq("league_id",i);const{error:s}=await T.from("mini_leagues").delete().eq("id",i);if(s){d("Erreur suppression ("+s.message+")","error");return}d("Mini League supprimée avec succès","success"),_t(e,t,l)}async function tr(e,t,i){await T.from("mini_leagues").update({is_archived:!0}).eq("id",i),t.toast("Mini League archivée","success"),_t(e,t,"archived")}async function bt(e,t,i){var H,B,j,U,oe,ce,re,D,ie;const{state:a,toast:l}=t,d=a.profile.id,[{data:s},{data:c},{data:p}]=await Promise.all([T.from("mini_leagues").select("*").eq("id",i).single(),T.from("mini_league_members").select("*, user:users(id,pseudo,club_name,club_color1,club_color2)").eq("league_id",i),T.from("mini_league_matches").select("*").eq("league_id",i).order("matchday").order("created_at")]);if(!s){l("Introuvable","error"),await _t(e,t);return}const n=(c||[]).some(W=>W.user_id===d),o=s.creator_id===d,f=(c||[]).map(W=>W.user).filter(Boolean),v=En(f,p||[]),w=(p||[]).filter(W=>W.matchday===s.current_day),A=s.pot||0,r=s.entry_fee||100,u=w.length>0&&w.every(W=>W.status==="finished"||W.status==="bye"),I=s.current_day>=s.total_days,z=(c||[]).find(W=>W.user_id===d);e.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
  ...
  </div>`}async function ir(e,t,i,a){const{toast:l,state:d}=t,s=rr(a.map(n=>n.id),i.mode),c=[];s.forEach((n,o)=>n.forEach(f=>c.push({league_id:i.id,matchday:o+1,home_id:f.home||f.bye,away_id:f.away||null,is_bye:!!f.bye,status:f.bye?"bye":"pending"})));const{error:p}=await T.from("mini_league_matches").insert(c);if(p){l("Erreur calendrier : "+p.message,"error");return}await T.from("mini_leagues").update({status:"active",current_day:1,total_days:s.length}).eq("id",i.id),l(`🚀 Lancée ! Pot : ${(i.pot||0).toLocaleString("fr")} cr.`,"success"),bt(e,t,i.id)}async function nr(e,t,i){const{data:a}=await T.from("mini_leagues").select("current_day,total_days,pot").eq("id",i).single(),l=(a.current_day||0)+1;if(l>(a.total_days||0)){await kn(e,t,i,a.pot||0,null,null);return}await T.from("mini_leagues").update({current_day:l}).eq("id",i),t.toast(`Journée ${l} commencée !`,"success"),bt(e,t,i)}async function kn(e,t,i,a,l,d){const{toast:s,state:c}=t;let p=l,n=d;if(!p){const{data:f}=await T.from("mini_league_matches").select("*").eq("league_id",i),{data:v}=await T.from("mini_league_members").select("*, user:users(id,pseudo,club_name)").eq("league_id",i);n=(v||[]).map(w=>w.user).filter(Boolean),p=En(n,f||[])}const o=[Math.floor(a*.7),Math.floor(a*.2),Math.floor(a*.1)];await Promise.all(p.slice(0,3).map((f,v)=>o[v]?T.from("mini_league_members").update({prize_amount:o[v],prize_claimed:!1}).eq("league_id",i).eq("user_id",f.userId):Promise.resolve())),await T.from("mini_leagues").update({status:"finished"}).eq("id",i),s("🏆 Mini League terminée ! Les gagnants peuvent récupérer leurs crédits.","success"),bt(e,t,i)}async function or(e,t,i,a,l){var v,w;const{state:d,toast:s}=t,c=[Math.floor((i.pot||0)*.7),Math.floor((i.pot||0)*.2),Math.floor((i.pot||0)*.1)],p=["🥇","🥈","🥉"][l],n=a.prize_amount||c[l]||0,o=a.prize_claimed,f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",f.innerHTML=`
    <div style="background:linear-gradient(160deg,#1a0a2e,#3b1a6e);border-radius:20px;padding:32px 24px;max-width:320px;width:100%;text-align:center;color:#fff;box-shadow:0 8px 40px rgba(0,0,0,0.6)">
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">🏆 Retour à la Mini League</button>`,document.body.appendChild(h),(q=h.querySelector("#pvp-end-home"))==null||q.addEventListener("click",()=>{h.remove(),Ye(e),n?c("mini-league",{openLeagueId:n}):c("mini-league")})}const ce=T.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},g=>{const m=g.new;try{if(m.status==="finished"||m.forfeit){if(I||(I=!0,H&&(clearInterval(H),H=null),m.game_state&&(u=m.game_state),u.phase==="finished"&&!m.forfeit&&document.getElementById("pvp-home")))return;oe(m);return}if(m.game_state){const x=u;u=m.game_state;const y=u._lastGC;if(y&&y.seq&&!U.has(y.seq)&&(U.add(y.seq),y.by!==o)){J(y.type,y.by,()=>xe());return}const b=x[o+"Score"]||0,h=x[f+"Score"]||0,E=u[o+"Score"]||0,k=u[f+"Score"]||0,L=E>b,S=k>h;if((L||S)&&!j.has(u.round)){j.add(u.round);const q=[...u.log||[]].reverse().find(O=>O.isGoal),F=((q==null?void 0:q.homePlayers)||[]).map(O=>({name:O.name,note:O.note,portrait:O.portrait,job:O.job}));Y(F,E,k,L,()=>xe());return}xe()}}catch(x){console.error("[PvP] crash:",x)}}).subscribe();async function re(g){Object.assign(u,g),u.lastActionAt=new Date().toISOString();const{error:m}=await T.from("matches").update({game_state:u}).eq("id",i);m&&p("Erreur de synchronisation","error");try{xe()}catch(x){console.error("[PvP] renderPvpScreen crash:",x)}}async function D(){if(I)return;I=!0,H&&(clearInterval(H),H=null);const g=a?A.away_id:A.home_id,m=a?"p2":"p1",x=a?"p1":"p2",y={...u,[m+"Score"]:3,[x+"Score"]:0,phase:"finished"};try{await T.from("matches").update({status:"finished",forfeit:!0,winner_id:g,home_score:y.p1Score||0,away_score:y.p2Score||0,game_state:y}).eq("id",i)}catch{}try{T.removeChannel(ce)}catch{}setTimeout(()=>{Ye(e),c("mini-league")},800)}const ie={BOOST_STAT:({value:g=1,count:m=1,roles:x=[]},y,b)=>{const h=y[o+"Team"],E=Object.entries(h).filter(([k])=>!x.length||x.includes(k)).flatMap(([k,L])=>L.filter(S=>!S.used).map(S=>({...S,_line:k})));if(!E.length){y.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),b(y);return}W(E,m,`Choisir ${m} joueur(s) à booster (+${g})`,k=>{k.forEach(L=>{const S=(h[L._line]||[]).find(q=>q.cardId===L.cardId);S&&(S.boost=(S.boost||0)+g,y.log.push({text:`⚡ +${g} sur ${S.name}`,type:"info"}))}),y[o+"Team"]=h,b(y)})},DEBUFF_STAT:({value:g=1,count:m=1,roles:x=[],target:y="ai"},b,h)=>{const E=y==="home"?o:f,k=b[E+"Team"],L=y==="home"?"allié":"adverse",S=Object.entries(k).filter(([q])=>!x.length||x.includes(q)).flatMap(([q,F])=>F.map(O=>({...O,_line:q})));if(!S.length){b.log.push({text:`🎯 Aucun joueur ${L}`,type:"info"}),h(b);return}W(S,m,`Choisir ${m} joueur(s) ${L}(s) (-${g})`,q=>{q.forEach(F=>{const O=(k[F._line]||[]).find(V=>V.cardId===F.cardId);O&&(O.boost=(O.boost||0)-g,b.log.push({text:`🎯 -${g} sur ${O.name}`,type:"info"}))}),b[E+"Team"]=k,h(b)})},GRAY_PLAYER:({count:g=1,roles:m=[],target:x="ai"},y,b)=>{const h=x==="home"?o:f,E=y[h+"Team"],k=x==="home"?"allié":"adverse",L=Object.entries(E).filter(([S])=>!m.length||m.includes(S)).flatMap(([S,q])=>q.filter(F=>!F.used).map(F=>({...F,_line:S})));if(!L.length){y.log.push({text:`❌ Aucun joueur ${k}`,type:"info"}),b(y);return}W(L,g,`Choisir ${g} joueur(s) ${k}(s) à exclure`,S=>{const q="usedCardIds_"+h,F=new Set(y[q]||[]);S.forEach(O=>{const V=(E[O._line]||[]).find(X=>X.cardId===O.cardId);V&&(V.used=!0,F.add(O.cardId),y.log.push({text:`❌ ${V.name} exclu !`,type:"info"}))}),y[q]=[...F],y[h+"Team"]=E,b(y)})},REVIVE_PLAYER:({count:g=1,roles:m=[]},x,y)=>{const b=x[o+"Team"],h=Object.entries(b).filter(([E])=>!m.length||m.includes(E)).flatMap(([E,k])=>k.filter(L=>L.used).map(L=>({...L,_line:E})));if(!h.length){x.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),y(x);return}W(h,g,`Choisir ${g} joueur(s) à ressusciter`,E=>{E.forEach(k=>{const L=(b[k._line]||[]).find(S=>S.cardId===k.cardId);L&&(L.used=!1,x.log.push({text:`💫 ${L.name} ressuscité !`,type:"info"}))}),x[o+"Team"]=b,y(x)})},REMOVE_GOAL:({},g,m)=>{const x=f+"Score";g[x]>0?(g[x]--,g.log.push({text:"🚫 Dernier but annulé !",type:"info"})):g.log.push({text:"🚫 Aucun but à annuler",type:"info"}),m(g)},ADD_GOAL_DRAW:({},g,m)=>{g[o+"Score"]===g[f+"Score"]?(g[o+"Score"]++,g.log.push({text:"🎯 But bonus !",type:"info"})):g.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),m(g)},ADD_SUB:({value:g=1},m,x)=>{m.maxSubs=(m.maxSubs||3)+g,m.log.push({text:`🔄 +${g} remplacement(s)`,type:"info"}),x(m)},CUSTOM:({},g,m)=>m(g)};function W(g,m,x,y){const b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let h=[];function E(){var L,S;const k=g.map(q=>{const F={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[q._line]||"#555",O=$e(q,q._line)+(q.boost||0),X=h.find(te=>te.cardId===q.cardId)?"#FFD700":"rgba(255,255,255,0.25)",K=q.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${q.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${X};background:${F};overflow:hidden;cursor:pointer;${K}">
        ...
        <div style="height:${pt}px;background:rgba(255,255,255,0.14);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 3px">
          <span style="font-size:${Ut}px;font-weight:900;color:#fff;text-align:center;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${de-6}px">${rt}</span>
          <span style="font-size:6px;color:rgba(255,255,255,0.45)">⚡ GC</span>
        </div>
    ...
    </div>`,(b=document.getElementById("pvp-home"))==null||b.addEventListener("click",()=>{try{T.removeChannel(ce)}catch{}Ye(e),c("mini-league",n?{openLeagueId:n}:{})})}xe()}const Mt=[{emoji:"⚽",title:"Bienvenue dans Manager Wars !",color:"#1A6B3C",content:`<p>Tu es désormais un <strong>manager de football</strong> virtuel.</p>
    ...
    <p style="margin-top:12px;font-size:13px;color:#888">Tu peux revoir ce tutoriel depuis les paramètres à tout moment.</p>`}];function dr(e,t,i){let a=0;const l=document.createElement("div");l.id="tutorial-overlay",l.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.65);z-index:9900;display:flex;align-items:center;justify-content:center;padding:16px";const d=()=>{var o,f,v;const c=Mt[a],p=a===Mt.length-1,n=Math.round((a+1)/Mt.length*100);l.innerHTML=`
      <div style="background:#fff;border-radius:20px;width:100%;max-width:420px;max-height:90vh;display:flex;flex-direction:column;overflow:hidden;box-shadow:0 16px 64px rgba(0,0,0,0.4)">
        ...
        <div style="padding:24px 24px 0;text-align:center">
          ...
          <div style="font-size:11px;color:#aaa">${a+1} / ${Mt.length}</div>
        </div>
        <!-- Contenu -->
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    </div>`}function w(){const r=document.getElementById("mkt-content"),u=document.getElementById("mkt-filters");if(r){if(u.style.display=p==="buy"?"flex":"none",p==="buy"){const I=o(s);r.innerHTML=I.length?I.map(f).join(""):'<div style="text-align:center;color:#aaa;padding:40px">Aucune carte trouvée.</div>'}else{const I=c.filter(C=>C.status==="active").sort((C,H)=>new Date(H.listed_at)-new Date(C.listed_at)),z=c.filter(C=>C.status==="sold").sort((C,H)=>new Date(H.sold_at||H.listed_at)-new Date(C.sold_at||C.listed_at));r.innerHTML=(I.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:4px 0 6px;text-transform:uppercase;letter-spacing:1px">🟢 En vente (${I.length})</div>`+I.map(v).join(""):"")+(z.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:12px 0 6px;text-transform:uppercase;letter-spacing:1px">✅ Ventes réussies (${z.length})</div>`+z.map(v).join(""):"")+(!I.length&&!z.length?'<div style="text-align:center;color:#aaa;padding:40px">Aucune vente pour le moment.</div>':"")}r.querySelectorAll("[data-buy]").forEach(I=>I.addEventListener("click",()=>ur(I.dataset.buy,s,e,t))),r.querySelectorAll("[data-cancel]").forEach(I=>I.addEventListener("click",()=>gr(I.dataset.cancel,e,t)))}}e.querySelectorAll(".mkt-tab").forEach(r=>{r.addEventListener("click",()=>{p=r.dataset.tab,e.querySelectorAll(".mkt-tab").forEach(u=>{const I=u===r;u.style.background=I?"var(--green)":"#fff",u.style.color=I?"#fff":"var(--gray-600)",u.style.borderColor=I?"var(--green)":"var(--gray-200)"}),w()})});let A;["flt-name","flt-club","flt-country","flt-job","flt-rarity","flt-note1","flt-note2"].forEach(r=>{var u;(u=document.getElementById(r))==null||u.addEventListener("input",()=>{clearTimeout(A),A=setTimeout(w,250)})}),w()}async function ur(e,t,i,a){const{state:l,toast:d,refreshProfile:s}=a,c=t.find(o=>o.id===e);if(!c)return;const p=c.price;if((l.profile.credits||0)<p){d("Crédits insuffisants","error");return}fr(c,async()=>{const{error:o}=await T.rpc("buy_market_card",{p_listing_id:e,p_buyer_id:l.profile.id});if(o){d("Erreur achat : "+o.message,"error");return}await s();const f=document.getElementById("nav-credits");f&&(f.textContent=`💰 ${(l.profile.credits||0).toLocaleString("fr")}`),d("✅ Carte achetée !","success"),await Ti(i,a)})}function fr(e,t){var s;const i=(s=e.card)==null?void 0:s.player,a=i?`${i.firstname} ${i.surname_encoded}`:"cette carte",l=document.createElement("div");l.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",l.innerHTML=`
  ...
  `}async function xr(e,{state:t,navigate:i,toast:a}){const l=t.profile;if(!l)return;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:d}=await T.from("matches").select(`id,home_id,away_id,home_score,away_score,status,mode,winner_id,created_at,played_at,
      home:users!home_id(pseudo,club_name),
      away:users!away_id(pseudo,club_name)`).or(`home_id.eq.${l.id},away_id.eq.${l.id}`).order("created_at",{ascending:!1}).limit(50),s={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},c=(d||[]).filter(o=>o.status==="finished"),p=(d||[]).filter(o=>o.status==="in_progress");function n(o){const f=o.home_id===l.id;f?o.home_score:o.away_score,f?o.away_score:o.home_score;const v=o.winner_id===l.id,w=o.home_score===o.away_score&&o.status==="finished",A=o.status!=="finished"?"…":w?"N":v?"V":"D",r=o.status!=="finished"||w?"#888":v?"#1A6B3C":"#c0392b";let u=s[o.mode]||o.mode;o.away_id===null&&!u.startsWith("IA")&&(u="IA");const z=o.home_id===l.id?o.away:o.home;let C;o.away_id===null?C=u:z?C=`${z.club_name||z.pseudo} (${z.pseudo})`:C="Adversaire";let H="";o.status==="in_progress"&&Date.now()-new Date(o.created_at).getTime()>3600*1e3&&(H=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const B=new Date(o.created_at),j=B.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+B.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),U=o.status==="finished"?`${o.home_score} - ${o.away_score}`:`${o.home_score||0} - ${o.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
  ...
  </div>`}zn(Sn);const je={user:null,profile:null,page:"home",params:{}};function Ct(e,t="info",i=3e3){const a=document.getElementById("toast");a&&(a.textContent=e,a.className=`show ${t}`,clearTimeout(a._t),a._t=setTimeout(()=>{a.className=""},i))}function yr(e,t,i=""){document.getElementById("modal-title").textContent=e,document.getElementById("modal-body").innerHTML=t,document.getElementById("modal-footer").innerHTML=i,document.getElementById("modal-overlay").classList.remove("hidden")}function mi(){document.getElementById("modal-overlay").classList.add("hidden")}async function jt(){if(!je.user)return;const{data:e}=await T.from("users").select("*").eq("id",je.user.id).single();e&&(je.profile=e)}function Bt(e,t={}){je.page=e,je.params=t,Tn()}async function Tn(){var a,l,d,s;const e=document.getElementById("page-content");if(!e)return;document.querySelectorAll(".bottom-nav a").forEach(c=>{c.classList.toggle("active",c.dataset.page===je.page)});const t=document.getElementById("nav-credits");t&&je.profile&&(t.textContent=`💰 ${(je.profile.credits||0).toLocaleString("fr")}`);const i={state:je,navigate:Bt,toast:Ct,openModal:yr,closeModal:mi,refreshProfile:jt};switch(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽</div>',je.page){case"home":await Mi(e,i);break;case"collection":await Jn(e,i);break;case"decks":await li(e,i);break;case"boosters":await po(e,i);break;case"match":{const c=je.params&&je.params.matchMode||"vs_ai_easy";c==="random"?await wn(e,i):c==="friend"?await Uo(e,i,(a=je.params)==null?void 0:a.friendId,(l=je.params)==null?void 0:l.friendName):c==="mini-league"?await Oi(e,i,(d=je.params)==null?void 0:d.mlMatchId,(s=je.params)==null?void 0:s.leagueId):await To(e,i);break}case"market":await pr(e,i);break;case"rankings":await mr(e,i);break;case"matches":await xr(e,i);break;case"friends":await Bn(e,i);break;case"mini-league":await Yo(e,i);break;case"match-mini-league":{const c=je.params||{};await Oi(e,i,c.mlMatchId,c.leagueId);break}default:await Mi(e,i)}}function br(){var a;const e=document.getElementById("app"),t=je.profile;if(!t)return;const i="/manager-wars/icons/";e.innerHTML=`
    <nav class="top-nav">
  ...
  `,document.querySelectorAll(".bottom-nav a").forEach(l=>{l.addEventListener("click",d=>{d.preventDefault(),Bt(l.dataset.page)})}),document.getElementById("nav-logo").addEventListener("click",()=>Bt("home")),document.getElementById("nav-credits").addEventListener("click",()=>Bt("boosters")),(a=document.getElementById("journal-btn"))==null||a.addEventListener("click",()=>hr())}async function hr(){const{data:e}=await T.from("patch_notes").select("*").eq("is_published",!0).order("published_at",{ascending:!1}).limit(20),t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9000;display:flex;align-items:center;justify-content:center;padding:16px";const i=(e||[]).map(a=>{const l=new Date(a.published_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"long",year:"numeric"});return`<div style="padding:14px 0;border-bottom:1px solid #f0f0f0">
