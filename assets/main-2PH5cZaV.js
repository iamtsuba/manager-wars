import{s as E,F as xi,h as Pt,j as Yi,l as at,i as zn,k as Sn,b as Ln}from"./formation-links-DJC25wD7.js";const In="/manager-wars/";function Li(e,{navigate:t,toast:i}){let s="login";const c=()=>{var a,l,p,n,o,g;const d=s==="login";e.innerHTML=`
  ...
  `;function l(){var T;const n=document.getElementById("logo-preview"),o=document.getElementById("logo-initials"),g=((T=document.getElementById("setup-club"))==null?void 0:T.value)||"MW",v=g.trim().split(" ").filter(Boolean),y=v.length>=2?(v[0][0]+v[1][0]).toUpperCase():g.slice(0,2).toUpperCase();n&&(n.style.background=a,n.style.borderColor=d),o&&(o.textContent=y,o.style.color=d)}document.getElementById("color1").addEventListener("input",n=>{d=n.target.value,document.getElementById("swatch1").style.background=d,l()}),document.getElementById("color2").addEventListener("input",n=>{a=n.target.value,document.getElementById("swatch2").style.background=a,l()});function p(n){document.querySelectorAll(".setup-step").forEach(o=>o.classList.remove("active")),document.getElementById(`step-${n}`).classList.add("active"),document.getElementById("step-num").textContent=n,document.getElementById("progress-fill").style.width=`${Math.round(n/3*100)}%`,n===3&&l()}document.getElementById("step1-next").addEventListener("click",async()=>{const n=document.getElementById("setup-pseudo").value.trim(),o=document.getElementById("step1-error");if(o.textContent="",n.length<3){o.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:g}=await E.from("users").select("id").eq("pseudo",n).maybeSingle();if(g){o.textContent="Ce pseudo est déjà pris.";return}p(2)}),document.getElementById("step2-back").addEventListener("click",()=>p(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const n=document.getElementById("setup-club").value.trim(),o=document.getElementById("step2-error");if(o.textContent="",n.length<2){o.textContent="Nom trop court (min. 2 caractères).";return}const{data:g}=await E.from("users").select("id").eq("club_name",n).maybeSingle();if(g){o.textContent="Ce nom de club est déjà pris.";return}p(3)}),document.getElementById("step3-back").addEventListener("click",()=>p(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const n=document.getElementById("setup-pseudo").value.trim(),o=document.getElementById("setup-club").value.trim(),g=document.getElementById("step3-error"),v=document.getElementById("step3-finish");g.textContent="",v.disabled=!0,v.textContent="Création en cours…";try{const{error:y}=await E.from("users").insert({id:t.user.id,pseudo:n,club_name:o,club_color1:d,club_color2:a,credits:1e4});if(y)throw y;await Cn(t.user.id),await c(),s(`Bienvenue ${n} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(y){g.textContent=y.message,v.disabled=!1,v.textContent="🚀 Créer mon profil !"}})}async function Cn(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await E.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const jn="modulepreload",qn=function(e){return"/manager-wars/"+e},Ii={},yi=function(t,i,s){let c=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),l=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));c=Promise.allSettled(i.map(p=>{if(p=qn(p),p in Ii)return;Ii[p]=!0;const n=p.endsWith(".css"),o=n?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${p}"]${o}`))return;const g=document.createElement("link");if(g.rel=n?"stylesheet":jn,n||(g.as="script"),g.crossOrigin="",g.href=p,l&&g.setAttribute("nonce",l),document.head.appendChild(g),n)return new Promise((v,y)=>{g.addEventListener("load",v),g.addEventListener("error",()=>y(new Error(`Unable to preload CSS for ${p}`)))})}))}function d(a){const l=new Event("vite:preloadError",{cancelable:!0});if(l.payload=a,window.dispatchEvent(l),!l.defaultPrevented)throw a}return c.then(a=>{for(const l of a||[])l.status==="rejected"&&d(l.reason);return t().catch(d)})},Bt="#1A6B3C",Dt="#cc2222",Fn="#D4A017",Mi="#888";async function Bn(e,t){const{state:i,toast:s}=t;e.innerHTML=`
    ...
    <div style="display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:8px;padding:10px 0;border-bottom:1px solid #f0f0f0">
      ...
      <div style="text-align:left;font-size:18px;font-weight:900;color:${Z}">${j}</div>
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),n=(l||[]).filter(R=>R.card_type==="player"&&R.player),o=(l||[]).filter(R=>R.card_type==="game_changer"),g=(l||[]).filter(R=>R.card_type==="formation"),v=(l||[]).filter(R=>R.card_type==="stadium"),{data:y}=await E.from("gc_definitions").select("name,gc_type,color,effect,image_url"),T={};(y||[]).forEach(R=>{T[R.name]=R});const{data:r}=await E.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),u={};(r||[]).forEach(R=>{u[R.id]=R}),v.forEach(R=>{R.stadium_def&&(u[R.stadium_id]=R.stadium_def)});const M=Object.keys(xi),L=Object.keys(Ke),N={};n.forEach(R=>{const Q=R.player.id;N[Q]=(N[Q]||0)+1}),new Set(Object.keys(N).map(R=>String(R)));const K=new Set(g.map(R=>R.formation)),C=new Set(o.map(R=>R.gc_type));let j="player",D="Tous",Z="",se=!1;function re(){return[...n].sort((R,Q)=>{const ue=Ut.indexOf(R.player.job),me=Ut.indexOf(Q.player.job);return ue!==me?ue-me:(R.player.surname_encoded||"").localeCompare(Q.player.surname_encoded||"")})}function P(){return[...p||[]].sort((R,Q)=>{const ue=Ut.indexOf(R.job),me=Ut.indexOf(Q.job);return ue!==me?ue-me:(R.surname_encoded||"").localeCompare(Q.surname_encoded||"")})}function Y(){return re().filter(R=>{const Q=R.player,ue=D==="Tous"||Q.job===D,me=!Z||`${Q.firstname} ${Q.surname_encoded}`.toLowerCase().includes(Z);return ue&&me})}function U(){return P().filter(R=>{const Q=D==="Tous"||R.job===D,ue=!Z||`${R.firstname} ${R.surname_encoded}`.toLowerCase().includes(Z);return Q&&ue})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
  ...
  </div>`;function G(){const R=document.getElementById("col-filters");R&&(j==="player"?(R.innerHTML=`
          ...
          ${Yn.map(Q=>`
            <button class="filter-btn" data-job="${Q}"
              style="flex-shrink:0;padding:4px 12px;border-radius:20px;font-size:12px;font-weight:600;cursor:pointer;
                border:1.5px solid ${Q===D?"var(--green)":"var(--gray-200)"};
                background:${Q===D?"var(--green)":"#fff"};
                color:${Q===D?"#fff":"var(--gray-600)"}">
              ${Q}
            </button>`).join("")}
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",Q=>{Z=Q.target.value.toLowerCase(),ne()}),e.querySelectorAll(".filter-btn").forEach(Q=>{Q.addEventListener("click",()=>{D=Q.dataset.job,G(),ne()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{se=!se,G(),ne()})):(R.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{se=!se,G(),ne()})))}function ne(){const R=document.getElementById("col-grid");R&&(j==="player"?$e(R):j==="formation"?Ae(R):j==="stadium"?ae(R):ie(R))}function pe(R,Q,ue,me,X){const be=document.getElementById("col-grid"),ge=document.getElementById("col-big");if(!be||!ge)return;var Se=0;function Ce(){const Be=window.innerWidth>=768,qe=document.getElementById("col-big"),De=document.getElementById("col-grid");Be&&qe&&(qe.style.minHeight="420px",qe.style.flex="1 1 auto"),Be&&De&&(De.style.height=Math.round(310*.76+16)+"px",De.style.flexShrink="0",De.style.overflowX="auto",De.style.overflowY="hidden",De.style.alignItems="center",De.style.padding="8px 16px"),ge.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+Q(R[Se])+"</div>";var I=ge.querySelector("[data-card-id],[data-form-id],[data-gc-id]");I&&I.addEventListener("click",function(){me(R[Se])}),requestAnimationFrame(function(){var F=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!F||!ge)){var w=ge.clientHeight-8,k=ge.clientWidth-24,f=F.offsetHeight,m=F.offsetWidth;if(f>0&&m>0&&w>40){var x=Be?2.2:1,b=Math.min(w/f,k/m,x);F.style.transform="scale("+b.toFixed(3)+")",F.style.transformOrigin="top center"}}}),be.innerHTML=R.map(function(F,w){var k=w===Se,f="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+(k?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+w+'" style="'+f+'">'+ue(F,k)+"</div>"}).join(""),be.querySelectorAll(".col-mini-item").forEach(function(F){F.addEventListener("click",function(){Se=Number(F.dataset.idx),Ce(),F.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}Ce()}function ye(R){var Q=window.innerWidth>=768?.76:.54,ue;if(!R||R._fake){var me=R?R.player:null;if(!me)return"";ue=Fi(me)}else ue=ri(R,"");return'<div style="display:inline-block;zoom:'+Q+';pointer-events:none;line-height:0">'+ue+"</div>"}function Te(R,Q,ue){Q=Q||100,ue=ue||140;var me=Pt[R]||{},X={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},be=Math.max(3,Math.round(Q*.06)),ge=Object.entries(me).map(function(Ce){var Be=Ce[0],qe=Ce[1],De=Be.replace(/\d+$/,""),I=X[De]||"#888",F=Math.round(qe.x*Q),w=Math.round(qe.y*ue);return'<circle cx="'+F+'" cy="'+w+'" r="'+be+'" fill="'+I+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),Se=Math.max(1,Math.round(Q/50));return'<svg viewBox="0 0 '+Q+" "+ue+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+Q+'" height="'+ue+'" fill="#1A6B3C"/><rect x="'+Math.round(Q*.2)+'" y="'+Math.round(ue*.02)+'" width="'+Math.round(Q*.6)+'" height="'+Math.round(ue*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+Se+'"/><line x1="0" y1="'+Math.round(ue*.5)+'" x2="'+Q+'" y2="'+Math.round(ue*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+Se+'"/><ellipse cx="'+Math.round(Q*.5)+'" cy="'+Math.round(ue*.5)+'" rx="'+Math.round(Q*.18)+'" ry="'+Math.round(ue*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+Se+'"/><rect x="'+Math.round(Q*.2)+'" y="'+Math.round(ue*.82)+'" width="'+Math.round(Q*.6)+'" height="'+Math.round(ue*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+Se+'"/>'+ge+"</svg>"}function Ee(R,Q,ue){var me=ue>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+ue+"</div>":"",X=Q?'data-form-id="'+Q.id+'"':"",be=R.length>7?14:R.length>5?16:19,ge=!!Q;return"<div "+X+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(ge?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(ge?"":"filter:grayscale(1);opacity:0.5")+'">'+me+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(ge?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+be+"px;font-weight:900;color:"+(ge?"#1A6B3C":"#aaa")+';line-height:1">'+R+'</div></div><div style="flex:1;overflow:hidden;background:'+(ge?"#1A6B3C":"#ccc")+'">'+Te(R,140,220)+"</div></div>"}function he(R,Q){var ue=window.innerWidth>=768?.76:.54,me=140,X=305,be=Math.round(X*.22),ge=X-be,Se=R.length>7?10:13,Ce=Te(R,me,ge),Be=Q?"1.5px solid #2a7a40":"1px solid #ddd",qe=Q?"":"filter:grayscale(1);opacity:0.45;",De=Q?"#1A6B3C":"#bbb",I="#fff";return'<div style="display:inline-block;zoom:'+ue+';line-height:0;pointer-events:none"><div style="width:'+me+"px;height:"+X+"px;border-radius:6px;border:"+Be+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+qe+'"><div style="height:'+be+"px;background:"+De+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+Se+"px;font-weight:900;color:"+I+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(me-4)+'px">'+R+'</span></div><div style="height:'+ge+'px;overflow:hidden;flex-shrink:0">'+Ce+"</div></div></div>"}function $e(R){if(se){const Q=U();if(!Q.length){R.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const ue=Q.map(me=>n.find(X=>X.player.id===me.id)||{_fake:!0,player:me,id:"fake-"+me.id});pe(ue,me=>me._fake?Fi(me.player):ri(me,""),me=>me._fake?ye({player:me.player,id:"x",_fake:!0}):ye(me),me=>{me._fake||Bi(me,n,N,t)})}else{const Q=Y();if(!Q.length){R.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const ue={};Q.forEach(X=>{const be=X.player.id;ue[be]||(ue[be]=[]),ue[be].push(X)});const me=Object.values(ue).map(X=>Jn(X));pe(me,X=>{const be=N[X.player.id]||1,ge=be>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${be}</div>`:"",Ce=n.filter(Be=>Be.player.id===X.player.id&&Be.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return ri(X,ge+Ce)},X=>ye(X),X=>Bi(X,n,N,t))}}function Ae(R){const Q=se?M:[...K];if(!Q.length){R.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const ue=Q.map(me=>({formation:me,card:g.find(X=>X.formation===me)||null,owned:K.has(me)}));pe(ue,({formation:me,card:X,owned:be})=>Ee(me,be?X:null,be?g.filter(ge=>ge.formation===me).length:0),({formation:me,owned:X})=>he(me,X),({card:me,owned:X})=>{X&&me&&Qn(me,g,t,d)})}function ie(R){const Q=Object.keys(T),ue=se?Q.length?Q:L:[...C];if(!ue.length){R.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const me=ue.map(X=>({type:X,gc:Ke[X]||{icon:"⚡",desc:""},def:T[X]||null,owned:C.has(X),card:o.find(be=>be.gc_type===X)||null}));pe(me,({type:X,gc:be,def:ge,owned:Se,card:Ce})=>{const Be=Se?o.filter(x=>x.gc_type===X).length:0,qe=Be>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${Be}</div>`:"",De=(ge==null?void 0:ge.gc_type)==="ultra_game_changer",I={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},F={purple:"#b06ce0",light_blue:"#00d4ef"},w=I[ge==null?void 0:ge.color]||I.purple,k=F[ge==null?void 0:ge.color]||F.purple,f=(ge==null?void 0:ge.effect)||be.desc||"",m=ge!=null&&ge.image_url?`/manager-wars/icons/${ge.image_url}`:null;return Se&&Ce?`<div data-gc-id="${Ce.id}" data-gc-type="${X}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${k};background:${w};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${k}66;cursor:pointer">
          ${qe}
        ...
        </div>`},null)}e.querySelectorAll(".col-tab-btn").forEach(R=>{R.addEventListener("click",()=>{j=R.dataset.tab,D="Tous",Z="",se=!1,e.querySelectorAll(".col-tab-btn").forEach(Q=>{const ue=Q.dataset.tab===j;Q.style.borderBottomColor=ue?"var(--green)":"transparent",Q.style.color=ue?"var(--green)":"var(--gray-600)"}),G(),ne()})}),G(),ne()}function Zn(e,t,i){const s=Ke[e]||{icon:"⚡",desc:"Effet spécial."},c=(t==null?void 0:t.gc_type)==="ultra_game_changer",d={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},a={purple:"#b06ce0",light_blue:"#00d4ef"},l=d[t==null?void 0:t.color]||d.purple,p=a[t==null?void 0:t.color]||a.purple,n=(t==null?void 0:t.name)||e,o=(t==null?void 0:t.effect)||s.desc,g=t!=null&&t.image_url?`/manager-wars/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Vt=1e3;function Qn(e,t,i,s){var T,r,u;const{state:c,toast:d,closeModal:a,navigate:l,refreshProfile:p}=i,n=e.formation,o={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function g(){const M=Pt[n]||{},L=xi[n]||[],N=290,K=360,C=20;function j(Z){const se=M[Z];return se?{x:se.x*N,y:se.y*K}:null}let D=`<svg width="${N}" height="${K}" viewBox="0 0 ${N} ${K}" xmlns="http://www.w3.org/2000/svg">`;for(const[Z,se]of L){const re=j(Z),P=j(se);!re||!P||(D+=`<line x1="${re.x}" y1="${re.y}" x2="${P.x}" y2="${P.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const Z of Object.keys(M)){const se=j(Z);if(!se)continue;const re=Z.replace(/\d+/,""),P=o[re]||"#555";D+=`<circle cx="${se.x}" cy="${se.y}" r="${C}" fill="${P}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,D+=`<text x="${se.x}" y="${se.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${re}</text>`}return D+="</svg>",D}const v=t.filter(M=>M.formation===n);v.length;const y=!e.is_for_sale;s(`Formation ${n}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(T=document.getElementById("direct-sell-form-btn"))==null||T.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${n} pour ${Vt.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const M=v.find(N=>!N.is_for_sale)||v[0];if(!M){d("Aucune carte à vendre","error");return}await E.from("market_listings").delete().eq("card_id",M.id),await E.from("transfer_history").delete().eq("card_id",M.id);const{error:L}=await E.from("cards").delete().eq("id",M.id);if(L){d(L.message,"error");return}await E.from("users").update({credits:(c.profile.credits||0)+Vt}).eq("id",c.profile.id),await p(),d(`+${Vt.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),a(),l("collection")}),(r=document.getElementById("market-sell-form-btn"))==null||r.addEventListener("click",async()=>{const M=parseInt(document.getElementById("sell-price-form").value);if(!M||M<1){d("Prix invalide","error");return}await E.from("cards").update({is_for_sale:!0,sale_price:M}).eq("id",e.id),await E.from("market_listings").insert({seller_id:c.profile.id,card_id:e.id,price:M}),d("Carte mise en vente sur le marché !","success"),a(),l("collection")}),(u=document.getElementById("cancel-sell-form-btn"))==null||u.addEventListener("click",async()=>{await E.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await E.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),d("Annonce retirée","success"),a(),l("collection")})}async function Bi(e,t,i,s){var pe,ye,Te,Ee,he,$e,Ae;const{state:c,toast:d,openModal:a,closeModal:l,navigate:p,refreshProfile:n}=s,o=e.player,g=t.filter(ie=>ie.player.id===o.id),v=g.length,y=Math.max(Number(o.note_g)||0,Number(o.note_d)||0,Number(o.note_m)||0,Number(o.note_a)||0),T=o.rarity||"normal",{data:r}=await E.from("sell_price_configs").select("*").eq("rarity",T).lte("note_min",y).gte("note_max",y).order("note_min",{ascending:!1}).limit(1),u=((pe=r==null?void 0:r[0])==null?void 0:pe.price)??Wn[T]??1e3,M=o.rarity!=="legende",L=sn(o),N=(o.rarity==="pepite"||o.rarity==="papyte")&&e.current_note!=null?e.current_note:Gt(o,o.job),K=o.rarity==="pepite"||o.rarity==="papyte",C=o.job2?K?vi(e,an(o.job2)):Gt(o,o.job2):null,j=jt[o.job]||"#1A6B3C",D=o.job2?jt[o.job2]:null,Z=rn[o.rarity]||"#ccc",se=$i[o.country_code]||o.country_code||"",re=g.map(ie=>ie.id);let P={};if(re.length){const{data:ie}=await E.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",re).order("transferred_at",{ascending:!0});(ie||[]).forEach(ae=>{P[ae.card_id]||(P[ae.card_id]=[]),P[ae.card_id].push(ae)})}const Y=ie=>{const ae=ie.transferred_at?new Date(ie.transferred_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"2-digit",year:"numeric"}):"",R=ie.source==="booster"?"Booster":ie.price?ie.price.toLocaleString("fr"):"—";return`
    <div style="display:flex;justify-content:space-between;align-items:flex-start;background:#f9f9f9;border-radius:8px;padding:8px 12px;gap:8px">
      <div style="flex:1;min-width:0">
        <div style="font-size:13px;font-weight:700;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${ie.club_name}</div>
        <div style="font-size:11px;color:var(--gray-600)">(${ie.manager_name})</div>
      </div>
      <div style="text-align:right;flex-shrink:0">
        <div style="font-size:13px;font-weight:700;color:${ie.source==="booster"?"var(--gray-600)":"var(--yellow)"}">${R}</div>
    ...
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${g.map((ie,ae)=>{const R=P[ie.id]||[],Q=ie.is_for_sale,ue=R.length?R[R.length-1]:null,X=(o.rarity==="pepite"||o.rarity==="papyte")&&ie.current_note!=null?` (☆${ie.current_note})`:"";return`
            <div data-card-id="${ie.id}" data-card-idx="${ae}" class="exemplaire-row"
              style="border:2px solid #eee;border-radius:10px;padding:10px;cursor:pointer;transition:border-color .15s;${Q?"opacity:0.6":""}">
              <div style="display:flex;align-items:center;gap:10px">
                <input type="checkbox" class="expl-check" data-id="${ie.id}" ${Q?"disabled":""} style="width:18px;height:18px;cursor:pointer;accent-color:#1A6B3C;flex-shrink:0">
                <div style="flex:1;min-width:0">
                  <div style="font-size:12px;font-weight:700">Exemplaire ${ae+1}${X}${Q?" 🏷️ En vente":""}</div>
      ...
      <!-- Carte visuelle -->
      <div style="width:140px;border-radius:12px;padding:6px;background:${Z};flex-shrink:0">
        <div style="background:#f2e8d2;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
          <div style="padding:5px 6px 2px;text-align:center">
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        <div>
          ...
          <div style="font-weight:700;color:${Z}">${o.rarity.toUpperCase()}</div>
          ${o.rarity==="pepite"||o.rarity==="papyte"?`
          <div style="margin-top:6px;background:${Z}18;border-left:3px solid ${Z};border-radius:0 6px 6px 0;padding:6px 10px">
            <div style="font-size:11px;font-weight:700;color:${Z};margin-bottom:2px">Carte évolutive</div>
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",o.note_g],["DEF",o.note_d],["MIL",o.note_m],["ATT",o.note_a]].map(([ie,ae])=>{const R=jt[ie],Q=Number(ae)||0;return`<div style="display:flex;flex-direction:column;align-items:center;gap:2px">
                <svg width="38" height="37" viewBox="0 0 38 37" style="display:block">
                  ...
                  <text x="19" y="23" text-anchor="middle" font-size="13" font-weight="900" fill="white" font-family="Arial Black,Arial">${Q}</text>
                </svg>
                <span style="font-size:9px;font-weight:700;color:var(--gray-600)">${ie}</span>
              </div>`}).join("")}
          </div>
        </div>
        <div>
    ...
    ${U}
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(Ee=document.getElementById("close-detail"))==null||Ee.addEventListener("click",l);let G=new Set;const ne=()=>{const ie=G.size,ae=document.getElementById("sell-action-panel");ae&&(ae.style.display=ie>0?"block":"none",document.getElementById("sell-selected-count").textContent=ie,document.getElementById("sell-direct-total").textContent=(ie*u).toLocaleString("fr")+" cr.")};document.querySelectorAll(".expl-check").forEach(ie=>{ie.addEventListener("change",()=>{const ae=ie.dataset.id;ie.checked?G.add(ae):G.delete(ae);const R=ie.closest(".exemplaire-row");R&&(R.style.borderColor=ie.checked?"#1A6B3C":"#eee"),ne()})}),document.querySelectorAll(".exemplaire-row").forEach(ie=>{ie.addEventListener("click",ae=>{if(ae.target.closest("button")||ae.target.tagName==="INPUT")return;const R=ie.querySelector(".expl-check");R&&!R.disabled&&(R.checked=!R.checked,R.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(ie=>{ie.addEventListener("click",ae=>{ae.stopPropagation();const R=document.querySelector(`.expl-hist[data-hist="${ie.dataset.idx}"]`);R&&(R.style.display=R.style.display==="none"?"flex":"none")})}),(he=document.getElementById("direct-sell-btn"))==null||he.addEventListener("click",async()=>{const ie=[...G];if(!ie.length)return;const ae=ie.length*u;if(!confirm(`Vendre ${ie.length} carte${ie.length>1?"s":""} ${o.surname_encoded} pour ${ae.toLocaleString("fr")} crédits ? Action irréversible.`))return;await E.from("market_listings").delete().in("card_id",ie),await E.from("transfer_history").delete().in("card_id",ie);const{error:R}=await E.from("cards").delete().in("id",ie);if(R){d(R.message,"error");return}await E.from("users").update({credits:(c.profile.credits||0)+ae}).eq("id",c.profile.id),await n();const Q=document.getElementById("nav-credits");Q&&(Q.textContent=`💰 ${(c.profile.credits||0).toLocaleString("fr")}`),d(`+${ae.toLocaleString("fr")} crédits ! ${ie.length} carte${ie.length>1?"s":""} vendue${ie.length>1?"s":""}.`,"success"),l(),p("collection")}),($e=document.getElementById("market-sell-btn"))==null||$e.addEventListener("click",async()=>{var me;const ie=[...G];if(!ie.length){d("Sélectionne au moins un exemplaire","warning");return}const ae=parseInt((me=document.getElementById("sell-market-price"))==null?void 0:me.value);if(!ae||ae<1){d("Prix invalide","error");return}const{error:R}=await E.from("cards").update({is_for_sale:!0,sale_price:ae}).in("id",ie);if(R){d(R.message,"error");return}const Q=ie.map(X=>({seller_id:c.profile.id,card_id:X,price:ae,status:"active"})),{error:ue}=await E.from("market_listings").insert(Q);ue&&console.warn("[Market] insert listings:",ue.message),d(`${ie.length} carte${ie.length>1?"s":""} mise${ie.length>1?"s":""} en vente à ${ae.toLocaleString("fr")} cr. chacune !`,"success"),l(),p("collection")}),(Ae=document.getElementById("cancel-sell-btn"))==null||Ae.addEventListener("click",async()=>{await E.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await E.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),d("Annonce retirée","success"),l(),p("collection")})}const Jt={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},vt={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function ei(e){const t=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function dn(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function ln(e){var i;const t="https://fcnwclxlkytdfjotqkta.supabase.co";return(i=e==null?void 0:e.clubs)!=null&&i.logo_url?e.clubs.logo_url.startsWith("http")?e.clubs.logo_url:`${t}/storage/v1/object/public/assets/clubs/${e.clubs.logo_url}`:null}function eo(e,t=44,i=58){var y;const s=e?ei(e):null,c=e?ln(e):null,d=dn(e==null?void 0:e.country_code),a=(e==null?void 0:e.job)||"MIL",l=vt[a]||"#555",p={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",n=Number(a==="GK"?e==null?void 0:e.note_g:a==="DEF"?e==null?void 0:e.note_d:a==="MIL"?e==null?void 0:e.note_m:e==null?void 0:e.note_a)||0,o=Math.round(i*.19),g=Math.round(i*.25),v=i-o-g;return e?`<div style="width:${t}px;height:${i}px;border-radius:5px;border:2px solid ${p};background:${l};position:relative;overflow:hidden;flex-shrink:0">
  ...
  </div>`,to(e,t,c,i),document.getElementById("builder-back").addEventListener("click",()=>s("decks")),document.getElementById("formation-select").addEventListener("change",o=>{t.formation=o.target.value;const g=Gi(t.formation),v={};g.forEach(y=>{t.slots[y]&&(v[y]=t.slots[y])}),t.slots=v,St(e,t,i)}),(p=document.getElementById("stadium-select"))==null||p.addEventListener("change",o=>{t.stadiumCardId=o.target.value||null}),document.getElementById("save-deck").addEventListener("click",()=>oo(t,i)),e.querySelectorAll("[data-remove-sub]").forEach(o=>{o.addEventListener("click",()=>{t.subs=t.subs.filter(g=>g!==o.dataset.removeSub),St(e,t,i)})}),(n=document.getElementById("add-sub-btn"))==null||n.addEventListener("click",()=>{no(t,e,i)})}function to(e,t,i,s){var N,K;const c=e.querySelector("#deck-field");if(!c)return;const d=(N=t.stadiumCards)==null?void 0:N.find(C=>C.id===t.stadiumCardId),a=d&&((K=t.stadDefMap)==null?void 0:K[d.stadium_id])||null,l=Pt[t.formation]||{},p=Yi(t.formation)||[],n={};for(const C of i){const j=t.slots[C],D=j?t.playerCards.find(Z=>Z.id===j):null;n[C]=D?D.player:null}const o=300,g=300,v=48,y=64,T=13,r=16,u=38;function M(C){const j=l[C];return j?{x:j.x*o,y:j.y*g}:null}let L="";for(const[C,j]of p){const D=M(C),Z=M(j);if(!D||!Z)continue;const se=n[C]?{...n[C],club_id:n[C].club_id,country_code:n[C].country_code,rarity:n[C].rarity}:null,re=n[j]?{...n[j],club_id:n[j].club_id,country_code:n[j].country_code,rarity:n[j].rarity}:null,P=at(se,re);P==="#ff3333"||P==="#cc2222"?L+=`<line x1="${D.x.toFixed(1)}" y1="${D.y.toFixed(1)}" x2="${Z.x.toFixed(1)}" y2="${Z.y.toFixed(1)}" stroke="${P}" stroke-width="2.5" stroke-linecap="round" opacity="0.4"/>`:(L+=`<line x1="${D.x.toFixed(1)}" y1="${D.y.toFixed(1)}" x2="${Z.x.toFixed(1)}" y2="${Z.y.toFixed(1)}" stroke="${P}" stroke-width="8" stroke-linecap="round" opacity="0.2"/>`,L+=`<line x1="${D.x.toFixed(1)}" y1="${D.y.toFixed(1)}" x2="${Z.x.toFixed(1)}" y2="${Z.y.toFixed(1)}" stroke="${P}" stroke-width="2.5" stroke-linecap="round" opacity="0.9"/>`)}for(const C of i){const j=M(C);if(!j)continue;const D=n[C],Z=C.replace(/\d+/,""),se=vt[Z]||"#555",re=(j.x-v/2).toFixed(1),P=(j.y-y/2).toFixed(1),Y={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[D==null?void 0:D.rarity]||"#aaa";if(D){const U=ei(D),G=ln(D),ne=dn(D.country_code),pe=Number(Z==="GK"?D.note_g:Z==="DEF"?D.note_d:Z==="MIL"?D.note_m:D.note_a)||0,ye=a&&(a.club_id&&String(D.club_id)===String(a.club_id)||a.country_code&&D.country_code===a.country_code),Te=ye?pe+10:pe,Ee=y-T-r;L+=`<defs><clipPath id="dcp-${C}"><rect x="${re}" y="${(j.y-y/2+T).toFixed(1)}" width="${v}" height="${Ee}"/></clipPath></defs>`,L+=`<rect x="${re}" y="${P}" width="${v}" height="${y}" rx="5" fill="${se}"/>`,U&&(L+=`<image href="${U}" x="${re}" y="${(j.y-y/2+T).toFixed(1)}" width="${v}" height="${Ee}" clip-path="url(#dcp-${C})" preserveAspectRatio="xMidYMin slice"/>`),L+=`<rect x="${re}" y="${P}" width="${v}" height="${T}" fill="rgba(255,255,255,0.93)"/>`,L+=`<text x="${j.x.toFixed(1)}" y="${(j.y-y/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(D.surname_encoded||"").slice(0,9)}</text>`;const he=(j.y+y/2-r).toFixed(1);L+=`<rect x="${re}" y="${he}" width="${v}" height="${r}" fill="rgba(255,255,255,0.93)"/>`,ne&&(L+=`<image href="${ne}" x="${(j.x-21).toFixed(1)}" y="${(j.y+y/2-r+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`),L+=`<text x="${j.x.toFixed(1)}" y="${(j.y+y/2-r/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${Te}</text>`,G&&(L+=`<image href="${G}" x="${(j.x+v/2-14).toFixed(1)}" y="${(j.y+y/2-r+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`),L+=`<rect x="${re}" y="${P}" width="${v}" height="${y}" rx="5" fill="none" stroke="${ye?"#E87722":Y}" stroke-width="${ye?"2.5":"2"}"/>`,ye&&(L+=`<rect x="${(j.x+v/2-13).toFixed(1)}" y="${P}" width="13" height="9" rx="3" fill="#E87722"/>`,L+=`<text x="${(j.x+v/2-6.5).toFixed(1)}" y="${(j.y-y/2+4.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="900" fill="#fff" font-family="Arial">+10</text>`)}else L+=`<rect x="${re}" y="${P}" width="${v}" height="${y}" rx="5" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="5,3"/>`,L+=`<text x="${j.x.toFixed(1)}" y="${j.y.toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="22" fill="rgba(255,255,255,0.35)">+</text>`,L+=`<text x="${j.x.toFixed(1)}" y="${(j.y+16).toFixed(1)}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.3)">${Z}</text>`;L+=`<rect x="${re}" y="${P}" width="${v}" height="${y}" rx="5" fill="rgba(0,0,0,0.01)" class="deck-slot-hit" data-pos="${C}" style="cursor:pointer"/>`}c.innerHTML=`<svg viewBox="${-u} ${-u} ${o+u*2} ${g+u*2}" width="100%" style="display:block;max-width:440px;margin:0 auto">${L}</svg>`,c.querySelectorAll(".deck-slot-hit").forEach(C=>{C.addEventListener("click",()=>io(C.dataset.pos,t,e,s))})}function io(e,t,i,s){var v,y,T;const{openModal:c,closeModal:d}=s,a=e.replace(/\d+/,""),l=t.slots[e],p=l?t.playerCards.find(r=>r.id===l):null;(v=p==null?void 0:p.player)==null||v.id;const n=new Set;Object.entries(t.slots).forEach(([r,u])=>{var L;if(r===e||!u)return;const M=t.playerCards.find(N=>N.id===u);(L=M==null?void 0:M.player)!=null&&L.id&&n.add(M.player.id)}),t.subs.forEach(r=>{var M;const u=t.playerCards.find(L=>L.id===r);(M=u==null?void 0:u.player)!=null&&M.id&&n.add(u.player.id)});const o=new Set,g=t.playerCards.filter(r=>{const u=r.player;return!(u.job===a||u.job2===a)||n.has(u.id)||o.has(u.id)?!1:(o.add(u.id),!0)});g.sort((r,u)=>{const M=a==="GK"?r.player.note_g:a==="DEF"?r.player.note_d:a==="MIL"?r.player.note_m:r.player.note_a;return(a==="GK"?u.player.note_g:a==="DEF"?u.player.note_d:a==="MIL"?u.player.note_m:u.player.note_a)-M}),c(`Choisir ${a} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
  ...
  `,document.body.appendChild(c);let d=!1;const a=document.getElementById("pack-cut-zone"),l=document.getElementById("pack-blade");let p=!1;const n=G=>G.touches&&G.touches[0]?G.touches[0].clientX:G.clientX;function o(G){d||(p=!0,l.style.opacity="1",g(G))}function g(G){if(!p||d)return;const ne=a.getBoundingClientRect(),pe=n(G)-ne.left,ye=Math.max(0,Math.min(1,pe/ne.width));l.style.width=ye*ne.width+"px",ye>=.82&&y()}function v(){d||(p=!1,l.style.transition="width .2s ease, opacity .2s ease",l.style.width="0",l.style.opacity="0",setTimeout(()=>{d||(l.style.transition="")},220))}function y(){var ne;if(d)return;d=!0,p=!1,l.style.width="100%",l.style.opacity="1",(ne=document.getElementById("cut-flash"))==null||ne.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const G=document.getElementById("cut-hint");G&&(G.style.opacity="0"),a.classList.add("pack-cut"),setTimeout(()=>{l.style.opacity="0",document.getElementById("pack-phase").style.display="none",M(0)},620)}a.addEventListener("pointerdown",o),window.addEventListener("pointermove",g),window.addEventListener("pointerup",v),a.addEventListener("touchstart",o,{passive:!0}),window.addEventListener("touchmove",g,{passive:!0}),window.addEventListener("touchend",v);let T=0,r=!1;const u=new Set;function M(G){T=G,document.getElementById("reveal-phase").style.display="flex",L(),N(G,0),Z()}function L(){const G=document.getElementById("card-dots");G&&(G.innerHTML=e.map((ne,pe)=>`<div class="card-dot" data-i="${pe}" style="width:8px;height:8px;border-radius:50%;background:${pe===T?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),G.querySelectorAll(".card-dot").forEach(ne=>ne.addEventListener("click",()=>C(parseInt(ne.dataset.i)))))}function N(G,ne){var ae;const pe=e[G],ye=document.getElementById("card-counter"),Te=document.getElementById("card-track");ye&&(ye.textContent=`Carte ${G+1} / ${e.length}`);const Ee=document.getElementById("reveal-btns");Ee&&(Ee.style.display=G===e.length-1?"flex":"none");const he=pe.card_type==="player"&&((ae=pe.player)==null?void 0:ae.rarity)==="legende",$e=!u.has(G);u.add(G);let Ae=0;if(pe.card_type==="player"&&pe.player){const R=pe.player,Q=R.job||"ATT";Ae=Number(Q==="GK"?R.note_g:Q==="DEF"?R.note_d:Q==="MIL"?R.note_m:R.note_a)||0}const ie=R=>{Te.innerHTML=`
        ...
        </div>`;const Q=document.getElementById("current-card-wrap");ne!==0?(Q.style.transition="none",Q.style.transform=`translateX(${ne>0?100:-100}%)`,requestAnimationFrame(()=>{Q.style.transition="transform .28s cubic-bezier(.25,1,.5,1)",Q.style.transform="translateX(0)"})):Q.animate([{opacity:0,transform:"scale(.7)"},{opacity:1,transform:"scale(1)"}],{duration:300,easing:"cubic-bezier(.34,1.56,.64,1)"}),R||he?re():P(),L()};$e&&Ae>6&&pe.card_type==="player"&&pe.player?K(pe,()=>ie(!0)):ie(!1)}function K(G,ne){var Ae;r=!0;const pe=G.player,ye=`https://flagsapi.com/${pe.country_code}/flat/64.png`,Te=(Ae=pe.clubs)==null?void 0:Ae.logo_url,Ee=document.getElementById("walkout-overlay"),he=document.getElementById("walkout-stage");if(!Ee||!he){r=!1,ne();return}Ee.style.display="flex";const $e=()=>{const ie=he.firstElementChild;ie&&(ie.classList.remove("wo-in"),ie.classList.add("wo-out"))};he.innerHTML=`<img class="wo-in" src="${ye}" style="height:130px;border-radius:10px;box-shadow:0 10px 36px rgba(0,0,0,.6)" onerror="this.style.display='none'">`,navigator.vibrate&&navigator.vibrate(30),setTimeout($e,2e3),setTimeout(()=>{var ie;he.innerHTML=Te?`<img class="wo-in" src="${Te}" style="max-height:160px;max-width:210px;object-fit:contain">`:`<div class="wo-in" style="font-size:34px;font-weight:900;color:#fff;text-align:center">${((ie=pe.clubs)==null?void 0:ie.encoded_name)||"CLUB"}</div>`,navigator.vibrate&&navigator.vibrate(30)},2450),setTimeout($e,4450),setTimeout(()=>{Ee.style.display="none",he.innerHTML="",r=!1,navigator.vibrate&&navigator.vibrate([40,30,80]),ne()},4900)}function C(G){if(r||G<0||G>=e.length||G===T)return;const ne=G>T?1:-1;T=G,N(G,ne)}function j(){C(T+1)}function D(){C(T-1)}function Z(){const G=document.getElementById("card-viewport");if(!G||G._swipeBound)return;G._swipeBound=!0;let ne=0,pe=0,ye=0,Te=!1;const Ee=ae=>ae.touches?ae.touches[0].clientX:ae.clientX,he=ae=>ae.touches?ae.touches[0].clientY:ae.clientY,$e=ae=>{Te=!0,ne=Ee(ae),pe=he(ae),ye=0},Ae=ae=>{if(!Te)return;ye=Ee(ae)-ne;const R=he(ae)-pe;if(Math.abs(ye)<Math.abs(R))return;const Q=document.getElementById("current-card-wrap");Q&&(Q.style.transition="none",Q.style.transform=`translateX(${ye*.6}px) rotate(${ye*.02}deg)`)},ie=()=>{if(!Te)return;Te=!1;const ae=document.getElementById("current-card-wrap"),R=55;ye<=-R&&T<e.length-1?j():ye>=R&&T>0?D():ae&&(ae.style.transition="transform .2s ease",ae.style.transform="translateX(0)")};G.addEventListener("pointerdown",$e),G.addEventListener("pointermove",Ae),G.addEventListener("pointerup",ie),G.addEventListener("pointercancel",ie),G.addEventListener("touchstart",$e,{passive:!0}),G.addEventListener("touchmove",Ae,{passive:!0}),G.addEventListener("touchend",ie),G.addEventListener("click",ae=>{if(Math.abs(ye)>8)return;const R=G.getBoundingClientRect();ae.clientX-R.left>R.width/2?j():D()})}let se=null;function re(){const G=document.getElementById("fireworks-canvas");if(!G)return;G.width=window.innerWidth,G.height=window.innerHeight;const ne=G.getContext("2d"),pe=[];function ye(){const Ee=Math.random()*G.width,he=Math.random()*G.height*.6,$e=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],Ae=$e[Math.floor(Math.random()*$e.length)];for(let ie=0;ie<60;ie++){const ae=Math.PI*2/60*ie,R=2+Math.random()*5;pe.push({x:Ee,y:he,vx:Math.cos(ae)*R,vy:Math.sin(ae)*R,alpha:1,color:Ae,size:2+Math.random()*3})}}ye(),se=setInterval(ye,600);function Te(){if(document.getElementById("fireworks-canvas")){ne.clearRect(0,0,G.width,G.height);for(let Ee=pe.length-1;Ee>=0;Ee--){const he=pe[Ee];if(he.x+=he.vx,he.y+=he.vy+.08,he.vy*=.98,he.alpha-=.018,he.alpha<=0){pe.splice(Ee,1);continue}ne.globalAlpha=he.alpha,ne.fillStyle=he.color,ne.beginPath(),ne.arc(he.x,he.y,he.size,0,Math.PI*2),ne.fill()}ne.globalAlpha=1,(se!==null||pe.length>0)&&requestAnimationFrame(Te)}}Te()}function P(){se!==null&&(clearInterval(se),se=null);const G=document.getElementById("fireworks-canvas");G&&G.getContext("2d").clearRect(0,0,G.width,G.height)}if(s){const G=document.getElementById("reveal-btns");G&&(G.innerHTML='<button class="btn btn-primary" id="reveal-next" style="flex:1">Continuer →</button>'),(U=document.getElementById("reveal-next"))==null||U.addEventListener("click",()=>{P(),c.remove(),s()})}else document.getElementById("reveal-collection").addEventListener("click",()=>{P(),c.remove(),i("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{P(),c.remove(),i("boosters")})}function xo(e){var t,i,s,c;if(e.card_type==="player"&&e.player)return po(e);if(e.card_type==="game_changer"){const d=e._gcDef,a=(d==null?void 0:d.gc_type)==="ultra_game_changer",l={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},p={purple:"#b06ce0",light_blue:"#00d4ef"},n=l[d==null?void 0:d.color]||l.purple,o=p[d==null?void 0:d.color]||p.purple,g=(d==null?void 0:d.name)||e.gc_type||"Game Changer",v=(d==null?void 0:d.effect)||((t=ci[e.gc_type])==null?void 0:t.desc)||"",y=d!=null&&d.image_url?`/manager-wars/icons/${d.image_url}`:null,T=((i=ci[e.gc_type])==null?void 0:i.icon)||"⚡";return`<div style="width:170px;height:240px;background:${n};border-radius:14px;border:3px solid ${o};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${o}88;flex-shrink:0">
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      card:cards(id,card_type,formation,stadium_id,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order"),p=[...new Set((l||[]).filter(v=>{var y,T;return((y=v.card)==null?void 0:y.card_type)==="stadium"&&((T=v.card)==null?void 0:T.stadium_id)}).map(v=>v.card.stadium_id))],n={};if(p.length){const{data:v}=await E.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",p);(v||[]).forEach(y=>{n[y.id]=y}),(l||[]).forEach(y=>{var T,r;((T=y.card)==null?void 0:T.card_type)==="stadium"&&((r=y.card)!=null&&r.stadium_id)&&(y.card._stadiumDef=n[y.card.stadium_id]||null)})}let o=0;function g(){var C,j,D,Z,se,re,P;const v=d[o],y=(l||[]).filter(Y=>Y.deck_id===v.id),T=y.filter(Y=>{var U;return Y.is_starter&&((U=Y.card)==null?void 0:U.player)}).map(Y=>ui(Y.card,Y.position)),r=y.find(Y=>{var U;return((U=Y.card)==null?void 0:U.card_type)==="formation"}),u=v.formation||((C=r==null?void 0:r.card)==null?void 0:C.formation)||"4-4-2";let M=T.length>=11?yt(T,u):null,L=null;if(v.stadium_card_id&&M){const Y=y.find(U=>{var G;return((G=U.card)==null?void 0:G.id)===v.stadium_card_id});(j=Y==null?void 0:Y.card)!=null&&j.stadium_id&&(L=Y.card._stadiumDef||null),L&&(M=Ei(M,L))}const N=T.length>=11;et(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
  ...
  </div>`}function nt(e,t,i){if(!(e!=null&&e.length))return"";const s=e.slice(0,5);let c='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return s.forEach((d,a)=>{if(c+=Ye(d,40,52),a<s.length-1){const l=at(d,s[a+1]);c+=`<div style="width:7px;height:3px;background:${l==="#ff3333"||l==="#cc2222"?"rgba(255,255,255,0.12)":l};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(c+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),c+="</div>",c}function Xt(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function hn(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function ut(e,t,i,s,c=310,d=310,a=[]){var N;const l=Pt[t]||{},p=Yi(t)||xi[t]||[],n={},o=["ATT","MIL","DEF","GK"];for(const K of o)(e[K]||[]).forEach((j,D)=>{n[`${K}${D+1}`]=j});function g(K){const C=l[K];return C?{x:C.x*c,y:C.y*d}:null}let v="";for(const[K,C]of p){const j=g(K),D=g(C);if(!j||!D)continue;const Z=n[K],se=n[C],re=at(Z,se);re==="#00ff88"||re==="#FFD700"?(v+=`<line x1="${j.x.toFixed(1)}" y1="${j.y.toFixed(1)}" x2="${D.x.toFixed(1)}" y2="${D.y.toFixed(1)}"
        stroke="${re}" stroke-width="10" stroke-linecap="round" opacity="0.22"/>`,v+=`<line x1="${j.x.toFixed(1)}" y1="${j.y.toFixed(1)}" x2="${D.x.toFixed(1)}" y2="${D.y.toFixed(1)}"
        stroke="${re}" stroke-width="3.5" stroke-linecap="round" opacity="0.95"/>`):v+=`<line x1="${j.x.toFixed(1)}" y1="${j.y.toFixed(1)}" x2="${D.x.toFixed(1)}" y2="${D.y.toFixed(1)}"
        stroke="${re}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const y=48,T=64,r=13,u=16,M={normal:"#aaaaaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#111111",legende:"#7a28b8",légende:"#7a28b8"};for(const[K,C]of Object.entries(n)){const j=g(K);if(!j||!C)continue;const D=K.replace(/[0-9]/g,""),Z=It[D]||"#555",se=a.includes(C.cardId),re=i==="attack"&&(["MIL","ATT"].includes(D)||se)&&!C.used||i==="defense"&&["GK","DEF","MIL"].includes(D)&&!C.used,P=s.includes(C.cardId);let Y;i==="attack"?Y=se?Math.max(1,Number(C.note_a)||0):D==="MIL"?Number(C.note_m)||0:Number(C.note_a)||0:i==="defense"?Y=D==="GK"?Number(C.note_g)||0:D==="MIL"?Number(C.note_m)||0:Number(C.note_d)||0:Y=Number(D==="GK"?C.note_g:D==="DEF"?C.note_d:D==="MIL"?C.note_m:C.note_a)||0,Y=Y+(C.boost||0);const U=(j.x-y/2).toFixed(1),G=(j.y-T/2).toFixed(1),ne=M[C==null?void 0:C.rarity]||M.normal;if(C.used){const Ae=typeof import.meta<"u"&&"/manager-wars/"||"/",ae=`${typeof window<"u"&&((N=window.location)==null?void 0:N.origin)||""}${Ae}icons/carte-dos.png`.replace(/([^:])\/\//g,"$1/");v+=`<rect x="${U}" y="${G}" width="${y}" height="${T}" rx="5" fill="#161616"/>`,v+=`<image href="${ae}" xlink:href="${ae}" x="${U}" y="${G}" width="${y}" height="${T}" preserveAspectRatio="xMidYMid slice"/>`,v+=`<rect x="${U}" y="${G}" width="${y}" height="${T}" rx="5" fill="none" stroke="${ne}" stroke-width="2" opacity="0.7"/>`,v+=`<rect x="${U}" y="${G}" width="${y}" height="${T}" rx="5" fill="rgba(0,0,0,0.01)" class="match-used-hit" data-card-id="${C.cardId}" data-role="${D}" style="cursor:pointer"/>`;continue}const pe=P?.45:1,ye=P?"#FFD700":ne,Te=P?3:(C==null?void 0:C.rarity)==="legende"||(C==null?void 0:C.rarity)==="légende"||(C==null?void 0:C.rarity)==="pepite"||(C==null?void 0:C.rarity)==="pépite"?2.5:2,Ee=T-r-u;v+=`<defs><clipPath id="cp-${K}"><rect x="${U}" y="${(j.y-T/2+r).toFixed(1)}" width="${y}" height="${Ee}"/></clipPath></defs>`,v+=`<rect x="${U}" y="${G}" width="${y}" height="${T}" rx="5" fill="${Z}" opacity="${pe}"/>`;const he=Pe(C);he&&(v+=`<image href="${he}" xlink:href="${he}" x="${U}" y="${(j.y-T/2+r).toFixed(1)}" width="${y}" height="${Ee}" clip-path="url(#cp-${K})" preserveAspectRatio="xMidYMin slice"/>`),v+=`<rect x="${U}" y="${G}" width="${y}" height="${r}" rx="4" fill="rgba(255,255,255,0.92)"/>`,v+=`<text x="${j.x.toFixed(1)}" y="${(j.y-T/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(C.name||"").slice(0,9)}</text>`;const $e=(j.y+T/2-u).toFixed(1);v+=`<rect x="${U}" y="${$e}" width="${y}" height="${u}" fill="rgba(255,255,255,0.92)"/>`;{const Ae=Xt(C.country_code);Ae?v+=`<image href="${Ae}" xlink:href="${Ae}" x="${(j.x-20).toFixed(1)}" y="${(j.y+T/2-u+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:v+=`<text x="${(j.x-13).toFixed(1)}" y="${(j.y+T/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${hn(C.country_code)}</text>`,v+=`<text x="${j.x.toFixed(1)}" y="${(j.y+T/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${Y}</text>`;const ie=bn(C);ie?v+=`<image href="${ie}" xlink:href="${ie}" x="${(j.x+y/2-14).toFixed(1)}" y="${(j.y+T/2-u+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:C.clubName&&(v+=`<text x="${(j.x+14).toFixed(1)}" y="${(j.y+T/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(C.clubName||"").slice(0,3).toUpperCase()}</text>`),C.boost&&(v+=`<rect x="${(j.x+y/2-12).toFixed(1)}" y="${(j.y-T/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,v+=`<text x="${(j.x+y/2-5).toFixed(1)}" y="${(j.y-T/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${C.boost}</text>`)}v+=`<rect x="${U}" y="${G}" width="${y}" height="${T}" rx="5" fill="${P?"rgba(255,255,255,0.12)":"none"}" stroke="${ye}" stroke-width="${Te}" opacity="${pe}"/>`,re&&(v+=`<rect x="${U}" y="${G}" width="${y}" height="${T}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${P?"selected":""}" data-card-id="${C.cardId}" data-role="${D}" style="cursor:pointer"/>`)}const L=38;return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-L} ${-L} ${c+L*2} ${d+L*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${v}
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${r.map((N,K)=>{const C=K<r.length-1?at(N,r[K+1]):null,j=!C||C==="#ff3333"||C==="#cc2222",D=C==="#00ff88"?"+2":C==="#FFD700"?"+1":"";return`
          ...
          ${K<r.length-1?`<div class="duel-link duel-link-${L}" data-idx="${K}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${j?"rgba(255,255,255,0.12)":C};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${j?"none":`0 0 8px ${C}`}">
  ...
  <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div id="duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center, rgba(10,61,30,0.4), rgba(10,61,30,0.92))"></div>
          ...
          <button id="start-match-btn" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,0.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">
            ▶ Commencer le match
          </button>`,C.style.transition="opacity .45s ease",C.style.opacity="1",C.style.pointerEvents="auto",(j=document.getElementById("start-match-btn"))==null||j.addEventListener("click",T))},600)},700)},2800)}function Mo(e){if(e.type==="duel"&&(e.homeTotal!=null||e.aiTotal!=null)){const t=(e.homeTotal||0)>=(e.aiTotal||0);return`
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    ${(()=>{const P=window.innerWidth>=700,Y=($e,Ae,ie)=>{var qe,De;const ae=(t.gcDefs||[]).find(I=>I.name===$e.gc_type),R={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ae==null?void 0:ae.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",Q={purple:"#b06ce0",light_blue:"#00d4ef"}[ae==null?void 0:ae.color]||"#b06ce0",ue=(ae==null?void 0:ae.name)||$e.gc_type,me=(ae==null?void 0:ae.effect)||((qe=Ke[$e.gc_type])==null?void 0:qe.desc)||"",X=ae!=null&&ae.image_url?`/manager-wars/icons/${ae.image_url}`:null,be=((De=Ke[$e.gc_type])==null?void 0:De.icon)||"⚡",ge=Math.round(ie*.22),Se=Math.round(ie*.22),Ce=ie-ge-Se,Be=ue.length>12?7:9;return`<div class="gc-mini" data-gc-id="${$e.id}" data-gc-type="${$e.gc_type}"
          style="box-sizing:border-box;width:${Ae}px;height:${ie}px;border-radius:10px;border:2px solid ${Q};background:${R};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
          <div style="height:${ge}px;background:rgba(255,255,255,0.14);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 3px">
            <span style="font-size:${Be}px;font-weight:900;color:#fff;text-align:center;line-height:1.1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${Ae-6}px">${ue}</span>
          ...
          <div style="height:${Ce}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
            ${X?`<img src="${X}" style="max-width:${Ae-10}px;max-height:${Ce-6}px;object-fit:contain">`:`<span style="font-size:${Math.round(Ce*.55)}px">${be}</span>`}
          </div>
        ...
        </div>`},U=($e,Ae)=>{var ie;return`<div id="boost-card"
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(P=>{var Y,U,G;if(P.type==="duel"){const ne=P.isGoal,pe=P.homeScored?"#FFD700":ne?"#ff6b6b":"rgba(255,255,255,0.3)",ye=P.homeScored?"⚽ BUT !":ne?"⚽ BUT IA !":(Y=P.homePlayers)!=null&&Y.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${ne?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${pe};margin-bottom:4px">
  ...
  `,document.body.appendChild(s);let o=!1;const g=()=>{o||(o=!0,s.remove(),i())};s.addEventListener("click",g),setTimeout(g,2e3)}function Mt(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function ai(e,t,i,s=null,c=null){var y,T;if(t.phase!=="attack"){Mt("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){Mt(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const d=Object.entries(t.homeTeam).flatMap(([r,u])=>(u||[]).filter(M=>M.used).map(M=>({...M,_line:M._line||r}))),a=t.homeSubs.filter(r=>!t.usedSubIds.includes(r.cardId));if(!d.length){Mt("Aucun joueur utilisé à remplacer");return}if(!a.length){Mt("Aucun remplaçant disponible");return}let l=Math.max(0,d.findIndex(r=>r.cardId===c));const p=((y=d[l])==null?void 0:y._line)||((T=d[l])==null?void 0:T.job);let n=s?Math.max(0,a.findIndex(r=>r.cardId===s)):Math.max(0,a.findIndex(r=>r.job===p)),o=!1;const g=document.createElement("div");g.id="sub-overlay",g.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function v(){var C,j,D,Z,se,re;const r=d[l],u=a[n],M=Math.min(130,Math.round((window.innerWidth-90)/2)),L=Math.round(M*1.35),N=P=>`background:rgba(255,255,255,0.12);border:none;color:${P?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${P?"default":"pointer"};flex-shrink:0`;g.innerHTML=`
    ...
    </div>`,(C=g.querySelector("#sub-close"))==null||C.addEventListener("click",()=>g.remove()),(j=g.querySelector("#out-up"))==null||j.addEventListener("click",()=>{l>0&&(l--,v())}),(D=g.querySelector("#out-down"))==null||D.addEventListener("click",()=>{l<d.length-1&&(l++,v())}),(Z=g.querySelector("#in-up"))==null||Z.addEventListener("click",()=>{n>0&&(n--,v())}),(se=g.querySelector("#in-down"))==null||se.addEventListener("click",()=>{n<a.length-1&&(n++,v())});const K=(P,Y,U,G)=>{const ne=g.querySelector("#"+P);if(!ne)return;let pe=0;ne.addEventListener("touchstart",ye=>{pe=ye.touches[0].clientY},{passive:!0}),ne.addEventListener("touchend",ye=>{const Te=ye.changedTouches[0].clientY-pe;if(Math.abs(Te)<30)return;const Ee=Y();Te<0&&Ee<G-1?(U(Ee+1),v()):Te>0&&Ee>0&&(U(Ee-1),v())},{passive:!0})};K("in-panel",()=>n,P=>n=P,a.length),K("out-panel",()=>l,P=>l=P,d.length),(re=g.querySelector("#sub-confirm"))==null||re.addEventListener("click",P=>{if(P.preventDefault(),P.stopPropagation(),o)return;o=!0;const Y=d[l],U=a[n];if(!Y||!U)return;let G=null,ne=-1;for(const[ye,Te]of Object.entries(t.homeTeam)){const Ee=(Te||[]).findIndex(he=>he.cardId===Y.cardId);if(Ee!==-1){G=ye,ne=Ee;break}}if(ne===-1||!G){Mt("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),g.remove();return}const pe={...U,_line:G,_col:Y._col||0,used:!1,boost:0};t.homeTeam[G].splice(ne,1,pe),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(U.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:Y.name,firstname:Y.firstname,note:ze(Y,G),portrait:Pe(Y),job:Y.job,country_code:Y.country_code,rarity:Y.rarity,clubName:Y.clubName,clubLogo:Y.clubLogo},inPlayer:{name:U.name,firstname:U.firstname,note:ze(U,G),portrait:Pe(U),job:U.job,country_code:U.country_code,rarity:U.rarity,clubName:U.clubName,clubLogo:U.clubLogo},text:`🔄 ${U.firstname} ${U.name} remplace ${Y.firstname} ${Y.name}`}),g.remove(),Bo(Y,U,()=>Xe(e,t,i))})}document.body.appendChild(g),v()}function Do(e,t,i,s,c){var T,r;const d=(s.gcDefs||[]).find(u=>u.name===t),a=Ke[t]||{icon:"⚡",desc:"Carte spéciale."},l={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[d==null?void 0:d.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",p={purple:"#b06ce0",light_blue:"#00d4ef"}[d==null?void 0:d.color]||"#b06ce0",n=(d==null?void 0:d.name)||t,o=(d==null?void 0:d.effect)||a.desc,g=d!=null&&d.image_url?`/manager-wars/icons/${d.image_url}`:null,v=a.icon||"⚡",y=document.createElement("div");y.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",y.innerHTML=`
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(d);let l=!1;const p=()=>{l||(l=!0,d.remove(),setTimeout(()=>c(),50))};d.addEventListener("click",p),setTimeout(p,3500)}async function qt(e,t,i){var o,g;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:s}=i,c=t.homeScore>t.aiScore,d=t.homeScore===t.aiScore,a=c?"victoire":d?"nul":"defaite",l=Vn(t.mode,a);t.matchId&&await E.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:c?s.profile.id:null,home_credits_reward:l,played_at:new Date().toISOString()}).eq("id",t.matchId);const p={credits:(s.profile.credits||0)+l,matches_played:(s.profile.matches_played||0)+1};c?p.wins=(s.profile.wins||0)+1:d?p.draws=(s.profile.draws||0)+1:p.losses=(s.profile.losses||0)+1,await E.from("users").update(p).eq("id",s.profile.id),await i.refreshProfile();const n=document.createElement("div");n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",n.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const T=async(K=!0)=>{v=!0,y&&(E.removeChannel(y),y=null),K&&await E.rpc("cancel_matchmaking",{p_user_id:n.profile.id}).catch(()=>{})};(N=document.getElementById("mm-cancel"))==null||N.addEventListener("click",async()=>{try{await T(!0)}catch{}We(e),o("home")});const r=async(K,C,j)=>{if(v)return;v=!0,y&&(E.removeChannel(y),y=null);const D=document.getElementById("mm-status");D&&(D.textContent="Adversaire trouvé !"),await new Promise(Z=>setTimeout(Z,600)),e.isConnected&&_n(e,t,K,j,a,l)},{data:u,error:M}=await E.rpc("try_matchmake",{p_user_id:n.profile.id,p_deck_id:i});if(M||!(u!=null&&u.success)){g("Erreur de matchmaking","error"),We(e),o("home");return}if(u.matched){r(u.match_id,u.opponent_id,!1);return}const L=document.getElementById("mm-status");L&&(L.textContent="En attente d'un autre joueur..."),y=E.channel("matchmaking-"+n.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${n.profile.id}`},K=>{const C=K.new;C.status==="matched"&&C.match_id&&r(C.match_id,C.matched_with,!0)}).subscribe()}async function Ho(e,t,i){const{state:s,navigate:c,toast:d}=t;try{(E.getChannels?E.getChannels():[]).forEach(n=>{const o=n.topic||"";(o.includes("matchmaking")||o.includes("pvp-match")||o.includes("friend-invite"))&&E.removeChannel(n)})}catch{}const{data:a}=await E.from("matches").select("home_id, away_id, status").eq("id",i).single();if(!a){d("Match introuvable","error"),c("home");return}if(a.status==="finished"){d("Ce match est terminé","info"),c("home");return}const l=a.home_id===s.profile.id;_n(e,t,i,l,[],[])}async function _n(e,t,i,s,c=[],d=[]){const{state:a,navigate:l,toast:p}=t,n=s?"p1":"p2",o=s?"p2":"p1",g=(c||[]).map(I=>I.id),v=(c||[]).map(I=>({id:I.id,gc_type:I.gc_type,_gcDef:I._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:y}=await E.from("matches").select("*").eq("id",i).single();if(!y){p("Match introuvable","error"),l("home");return}async function T(){const[{data:I},{data:F},{data:w},{data:k}]=await Promise.all([E.rpc("get_deck_for_match",{p_deck_id:y.home_deck_id}),E.rpc("get_deck_for_match",{p_deck_id:y.away_deck_id}),E.from("users").select("id,pseudo,club_name").eq("id",y.home_id).single(),E.from("users").select("id,pseudo,club_name").eq("id",y.away_id).single()]),f=_=>({cardId:_.card_id,position:_.position,id:_.id,firstname:_.firstname,name:_.surname_encoded,country_code:_.country_code,club_id:_.club_id,job:_.job,job2:_.job2,note_g:Number(_.note_g)||0,note_d:Number(_.note_d)||0,note_m:Number(_.note_m)||0,note_a:Number(_.note_a)||0,rarity:_.rarity,skin:_.skin,hair:_.hair,hair_length:_.hair_length,clubName:_.club_encoded_name||null,clubLogo:_.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}),m=((I==null?void 0:I.starters)||[]).map(_=>f(_)),x=((F==null?void 0:F.starters)||[]).map(_=>f(_)),b=(I==null?void 0:I.formation)||"4-4-2",h=(F==null?void 0:F.formation)||"4-4-2";let $=yt(m,b),A=yt(x,h);return stadiumDef&&s&&($=Ei($,stadiumDef)),{p1Team:$,p2Team:A,p1Subs:((I==null?void 0:I.subs)||[]).map(_=>f(_)),p2Subs:((F==null?void 0:F.subs)||[]).map(_=>f(_)),p1Formation:b,p2Formation:h,p1Name:(w==null?void 0:w.club_name)||(w==null?void 0:w.pseudo)||"Joueur 1",p2Name:(k==null?void 0:k.club_name)||(k==null?void 0:k.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:s?g:[],gc_p2:s?[]:g,gcCardsFull_p1:s?v:[],gcCardsFull_p2:s?[]:v,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:d||[],lastActionAt:new Date().toISOString()}}let r=y.game_state&&Object.keys(y.game_state).length?y.game_state:null;if(!r)if(s){r=await T();const{data:I}=await E.from("matches").select("game_state").eq("id",i).single();!(I!=null&&I.game_state)||!Object.keys(I.game_state).length?await E.from("matches").update({game_state:r,turn_user_id:y.home_id}).eq("id",i):r=I.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let I=0;I<30&&!r;I++){await new Promise(w=>setTimeout(w,400));const{data:F}=await E.from("matches").select("game_state").eq("id",i).single();F!=null&&F.game_state&&Object.keys(F.game_state).length&&(r=F.game_state)}if(!r){p("Erreur de synchronisation","error"),l("home");return}r.gc_p2=g,r.gcCardsFull_p2=v,await E.from("matches").update({game_state:r}).eq("id",i)}let u=!1,M=null,L=!1;const N=new Set,K=new Set;function C(I){var $,A;try{E.removeChannel(j)}catch{}const F=r[n+"Score"]||0,w=r[o+"Score"]||0;let k,f;I.winner_id?(k=I.winner_id===a.profile.id,f=!1):I.forfeit?(k=F>w,f=!1):(f=F===w,k=F>w),($=document.getElementById("pvp-end-overlay"))==null||$.remove();const m=document.createElement("div");m.id="pvp-end-overlay",m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const x=f?"🤝":k?"🏆":"😞",b=f?"MATCH NUL":k?"VICTOIRE !":"DÉFAITE",h=f?"#fff":k?"#FFD700":"#ff6b6b";m.innerHTML=`
      <div style="font-size:64px">${x}</div>
      <div style="font-size:26px;font-weight:900;color:${h}">${b}</div>
      <div style="font-size:18px">${r[n+"Name"]} ${F} – ${w} ${r[o+"Name"]}</div>
      ${I.forfeit?`<div style="font-size:13px;color:rgba(255,255,255,0.5)">${k?"L'adversaire a quitté":"Perdu par forfait"}</div>`:""}
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(m),(A=m.querySelector("#pvp-end-home"))==null||A.addEventListener("click",()=>{m.remove(),We(e),l("home")})}const j=E.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},I=>{const F=I.new;try{if(F.status==="finished"||F.forfeit){if(u)return;u=!0,M&&(clearInterval(M),M=null),F.game_state&&(r=F.game_state),C(F);return}if(F.game_state){const w=r;r=F.game_state;const k=r._lastGC;if(k&&k.seq&&!K.has(k.seq)&&(K.add(k.seq),k.by!==n)){Ee(k.type,k.by,()=>G());return}const f=w[n+"Score"]||0,m=w[o+"Score"]||0,x=r[n+"Score"]||0,b=r[o+"Score"]||0,h=x>f,$=b>m;if((h||$)&&!N.has(r.round)){N.add(r.round);const A=[...r.log||[]].reverse().find(z=>z.isGoal),_=((A==null?void 0:A.homePlayers)||[]).map(z=>({name:z.name,note:z.note,portrait:z.portrait,job:z.job}));Te(_,x,b,h,()=>G());return}G()}}catch(w){console.error("[PvP] crash:",w)}}).subscribe();async function D(I){Object.assign(r,I),r.lastActionAt=new Date().toISOString();const{error:F}=await E.from("matches").update({game_state:r}).eq("id",i);F&&p("Erreur de synchronisation","error");try{G()}catch(w){console.error("[PvP] renderPvpScreen crash:",w)}}async function Z(){if(u)return;u=!0,M&&(clearInterval(M),M=null);const I=s?y.away_id:y.home_id,F=s?"p2":"p1",w=s?"p1":"p2",k={...r,[F+"Score"]:3,[w+"Score"]:0,phase:"finished"};try{await E.from("matches").update({status:"finished",forfeit:!0,winner_id:I,home_score:k.p1Score||0,away_score:k.p2Score||0,game_state:k}).eq("id",i)}catch{}try{E.removeChannel(j)}catch{}setTimeout(()=>{We(e),l("home")},800)}const se={BOOST_STAT:({value:I=1,count:F=1,roles:w=[]},k,f)=>{const m=k[n+"Team"],x=Object.entries(m).filter(([b])=>!w.length||w.includes(b)).flatMap(([b,h])=>h.filter($=>!$.used).map($=>({...$,_line:b})));if(!x.length){k.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),f(k);return}re(x,F,`Choisir ${F} joueur(s) à booster (+${I})`,b=>{b.forEach(h=>{const $=(m[h._line]||[]).find(A=>A.cardId===h.cardId);$&&($.boost=($.boost||0)+I,k.log.push({text:`⚡ +${I} sur ${$.name}`,type:"info"}))}),k[n+"Team"]=m,f(k)})},DEBUFF_STAT:({value:I=1,count:F=1,roles:w=[],target:k="ai"},f,m)=>{const x=k==="home"?n:o,b=f[x+"Team"],h=k==="home"?"allié":"adverse",$=Object.entries(b).filter(([A])=>!w.length||w.includes(A)).flatMap(([A,_])=>_.map(z=>({...z,_line:A})));if(!$.length){f.log.push({text:`🎯 Aucun joueur ${h}`,type:"info"}),m(f);return}re($,F,`Choisir ${F} joueur(s) ${h}(s) (-${I})`,A=>{A.forEach(_=>{const z=(b[_._line]||[]).find(S=>S.cardId===_.cardId);z&&(z.boost=(z.boost||0)-I,f.log.push({text:`🎯 -${I} sur ${z.name}`,type:"info"}))}),f[x+"Team"]=b,m(f)})},GRAY_PLAYER:({count:I=1,roles:F=[],target:w="ai"},k,f)=>{const m=w==="home"?n:o,x=k[m+"Team"],b=w==="home"?"allié":"adverse",h=Object.entries(x).filter(([$])=>!F.length||F.includes($)).flatMap(([$,A])=>A.filter(_=>!_.used).map(_=>({..._,_line:$})));if(!h.length){k.log.push({text:`❌ Aucun joueur ${b}`,type:"info"}),f(k);return}re(h,I,`Choisir ${I} joueur(s) ${b}(s) à exclure`,$=>{const A="usedCardIds_"+m,_=new Set(k[A]||[]);$.forEach(z=>{const S=(x[z._line]||[]).find(q=>q.cardId===z.cardId);S&&(S.used=!0,_.add(z.cardId),k.log.push({text:`❌ ${S.name} exclu !`,type:"info"}))}),k[A]=[..._],k[m+"Team"]=x,f(k)})},REVIVE_PLAYER:({count:I=1,roles:F=[]},w,k)=>{const f=w[n+"Team"],m=Object.entries(f).filter(([x])=>!F.length||F.includes(x)).flatMap(([x,b])=>b.filter(h=>h.used).map(h=>({...h,_line:x})));if(!m.length){w.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),k(w);return}re(m,I,`Choisir ${I} joueur(s) à ressusciter`,x=>{x.forEach(b=>{const h=(f[b._line]||[]).find($=>$.cardId===b.cardId);h&&(h.used=!1,w.log.push({text:`💫 ${h.name} ressuscité !`,type:"info"}))}),w[n+"Team"]=f,k(w)})},REMOVE_GOAL:({},I,F)=>{const w=o+"Score";I[w]>0?(I[w]--,I.log.push({text:"🚫 Dernier but annulé !",type:"info"})):I.log.push({text:"🚫 Aucun but à annuler",type:"info"}),F(I)},ADD_GOAL_DRAW:({},I,F)=>{I[n+"Score"]===I[o+"Score"]?(I[n+"Score"]++,I.log.push({text:"🎯 But bonus !",type:"info"})):I.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),F(I)},ADD_SUB:({value:I=1},F,w)=>{F.maxSubs=(F.maxSubs||3)+I,F.log.push({text:`🔄 +${I} remplacement(s)`,type:"info"}),w(F)},CUSTOM:({},I,F)=>F(I)};function re(I,F,w,k){const f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let m=[];function x(){var h,$;const b=I.map(A=>{const _={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[A._line]||"#555",z=ze(A,A._line)+(A.boost||0),q=m.find(O=>O.cardId===A.cardId)?"#FFD700":"rgba(255,255,255,0.25)",B=A.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${A.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${q};background:${_};overflow:hidden;cursor:pointer;${B}">
        ...
        <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
          ...
          <span style="font-size:12px;color:rgba(255,255,255,0.5)">${m.length}/${F}</span>
        ...
        <div style="padding:12px;background:rgba(0,0,0,0.4);flex-shrink:0">
          <button id="pp-confirm" ${m.length===0?'disabled style="opacity:0.4"':""} style="width:100%;padding:13px;border-radius:10px;border:none;background:#7a28b8;color:#fff;font-size:15px;font-weight:900;cursor:pointer">
            ✅ Confirmer (${m.length}/${F})
          </button>
        </div>`,(h=f.querySelector("#pp-close"))==null||h.addEventListener("click",()=>f.remove()),f.querySelectorAll(".pp-item").forEach(A=>{A.addEventListener("click",()=>{const _=A.dataset.cid,z=I.find(q=>q.cardId===_),S=m.findIndex(q=>q.cardId===_);z&&(S>-1?m.splice(S,1):m.length<F&&m.push(z),x())})}),($=f.querySelector("#pp-confirm"))==null||$.addEventListener("click",()=>{f.remove(),k(m)})}x(),document.body.appendChild(f)}async function P(I,F){const k=(r["gcCardsFull_"+n]||[]).find(b=>b.id===I),f=(k==null?void 0:k._gcDef)||(r.gcDefs||[]).find(b=>{var h;return b.name===F||((h=b.name)==null?void 0:h.toLowerCase().trim())===(F==null?void 0:F.toLowerCase().trim())}),m=[...r["usedGc_"+n]||[],I],x={type:F,by:n,seq:(r._gcAnimSeq||0)+1};K.add(x.seq),Ee(F,n,async()=>{if(f!=null&&f.effect_type&&f.effect_type!=="CUSTOM"){const h=se[f.effect_type];if(h){const $={...r};h(f.effect_params||{},$,async A=>{A["usedGc_"+n]=m,A._lastGC=x,A._gcAnimSeq=x.seq,await D(A)});return}}const b={...r};switch(F){case"Remplacement+":b.maxSubs=(b.maxSubs||3)+1,b.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const h=o+"Score";b[h]>0&&(b[h]--,b.log.push({text:"🚫 But annulé",type:"info"}));break}}b["usedGc_"+n]=m,b._lastGC=x,b._gcAnimSeq=x.seq,await D(b)})}function Y(I,F){const w="usedCardIds_"+I,k=new Set(r[w]||[]);F.forEach(f=>k.add(f)),r[w]=[...k]}function U(){for(const I of["p1","p2"]){const F=new Set(r["usedCardIds_"+I]||[]),w=r[I+"Team"];if(w)for(const k of["GK","DEF","MIL","ATT"])(w[k]||[]).forEach(f=>{f.used=F.has(f.cardId)})}}function G(){var Ue,Je,Ze,Qe,ot,gt;if(r.phase==="reveal")return ne();if(r.phase==="midfield")return ye();if(r.phase==="finished")return De();const I=r[n+"Team"],F=r[o+"Team"];U();const w=r[n+"Score"],k=r[o+"Score"],f=r[n+"Name"],m=r[o+"Name"],x=r.phase===n+"-attack",b=r.phase===n+"-defense",h=Array.isArray(r["selected_"+n])?r["selected_"+n]:[],$=h.map(de=>de.cardId),A=window.innerWidth>=700,_=r[n+"Subs"]||[],z=r["usedSubIds_"+n]||[],S=_.filter(de=>!z.includes(de.cardId)),q=r["gcCardsFull_"+n]||[],B=r["usedGc_"+n]||[],O=q.filter(de=>!B.includes(de.id)),H=r.boostOwner===n&&!r.boostUsed,W=!["GK","DEF","MIL","ATT"].some(de=>(F[de]||[]).some(_e=>!_e.used)),V=[...I.MIL||[],...I.ATT||[]].filter(de=>!de.used),te=x&&W&&V.length===0?[...I.GK||[],...I.DEF||[]].filter(de=>!de.used).map(de=>de.cardId):[];function oe(de,_e,ee){var pt,ht;const ce=de._gcDef,J={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ce==null?void 0:ce.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",le={purple:"#b06ce0",light_blue:"#00d4ef"}[ce==null?void 0:ce.color]||"#b06ce0",Me=(ce==null?void 0:ce.name)||de.gc_type,ke=(ce==null?void 0:ce.effect)||((pt=Ke[de.gc_type])==null?void 0:pt.desc)||"",tt=ce!=null&&ce.image_url?`/manager-wars/icons/${ce.image_url}`:null,it=((ht=Ke[de.gc_type])==null?void 0:ht.icon)||"⚡",rt=Math.round(ee*.22),st=Math.round(ee*.22),ct=ee-rt-st,$t=Me.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${de.id}" data-gc-type="${de.gc_type}"
        style="box-sizing:border-box;width:${_e}px;height:${ee}px;border-radius:10px;border:2px solid ${le};background:${J};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${ft(I,r[n+"Formation"],je,$,300,300,te)}
      </div>
          ...
          <div style="display:flex;align-items:center;gap:3px;max-height:46px;overflow:hidden">
            <div style="flex:1;display:flex;gap:2px;justify-content:flex-end;flex-wrap:nowrap;overflow-x:auto;overflow-y:hidden">
              ${(de.homePlayers||[]).map(ee=>dt(ee)).join("")}
            </div>
            <div style="text-align:center;padding:0 6px;flex-shrink:0">
        ...
        </div>`}if(x&&((ee=r.pendingAttack)!=null&&ee.players)){const ce=r.pendingAttack;return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #00ff88">
          <div style="font-size:8px;color:#00ff88;letter-spacing:2px;margin-bottom:4px;text-transform:uppercase">⚔️ VOUS ATTAQUEZ</div>
      ...
      </div>`;function He(){const de=e.querySelector(".match-screen");if(!de)return;const _e=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);de.style.height=_e+"px",de.style.minHeight=_e+"px",de.style.maxHeight=_e+"px",de.style.overflow="hidden";const ee=e.querySelector("#mobile-action-bar"),ce=e.querySelector("#mobile-play-area");ee&&ce&&(ce.style.paddingBottom=ee.offsetHeight+"px")}if(He(),setTimeout(He,120),setTimeout(He,400),L||(L=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",He),window.visualViewport.addEventListener("scroll",He)),window.addEventListener("resize",He)),function(){const _e=e.querySelector(".terrain-wrapper svg");_e&&(_e.removeAttribute("width"),_e.removeAttribute("height"),_e.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",_e.setAttribute("viewBox","-26 -26 352 352"),_e.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),e.querySelectorAll(".match-slot-hit").forEach(de=>{de.addEventListener("click",()=>{if(!x&&!b)return;const _e=de.dataset.cardId,ee=de.dataset.role,ce=(I[ee]||[]).find(ke=>ke.cardId===_e);if(!ce||ce.used)return;const J=te.includes(_e);if(x&&!["MIL","ATT"].includes(ee)&&!J)return;Array.isArray(r["selected_"+n])||(r["selected_"+n]=[]);const le=r["selected_"+n],Me=le.findIndex(ke=>ke.cardId===_e);Me>-1?le.splice(Me,1):le.length<3&&le.push({...ce,_role:ee}),G()})}),e.querySelectorAll(".match-used-hit").forEach(de=>{de.addEventListener("click",()=>Ae(de.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(de=>{de.addEventListener("click",()=>Ae())}),(Ue=e.querySelector("#pvp-sub-open"))==null||Ue.addEventListener("click",()=>Ae()),e.querySelectorAll(".pvp-gc-mini").forEach(de=>{de.addEventListener("click",()=>he(de.dataset.gcId,de.dataset.gcType))}),(Je=e.querySelector("#pvp-boost-card"))==null||Je.addEventListener("click",()=>$e()),(Ze=e.querySelector("#pvp-action"))==null||Ze.addEventListener("click",de=>{x?de.currentTarget.dataset.pass==="1"||!Se(n)?Q():ue():b&&me()}),(Qe=e.querySelector("#pvp-quit"))==null||Qe.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&Z()}),(ot=e.querySelector("#pvp-view-opp"))==null||ot.addEventListener("click",()=>ae()),(gt=e.querySelector("#pvp-toggle-history"))==null||gt.addEventListener("click",()=>R()),M&&(clearInterval(M),M=null),(x||b)&&!u){let de=30,_e=!1;const ee=()=>document.getElementById("pvp-timer"),ce=()=>{ee()&&(ee().textContent=de+"s",ee().style.color=_e?"#ff4444":"#fff")};ce(),M=setInterval(()=>{de--,de<0?_e?(clearInterval(M),M=null,x&&!Se(n)?Q():Z()):(_e=!0,de=15,ce()):ce()},1e3)}}function ne(){et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
      ...
      <div style="width:min(90vw,420px)">${ut(r[o+"Team"],r[o+"Formation"],null,[],300,300)}</div>
    </div>`,n==="p1"&&setTimeout(async()=>{await D({phase:"midfield"})},5e3)}let pe=!1;function ye(){if(pe)return;const I=r[n+"Team"].MIL||[],F=r[o+"Team"].MIL||[];function w(O){return O.reduce((H,W)=>H+ze(W,"MIL"),0)}function k(O){let H=0;for(let W=0;W<O.length-1;W++){const V=at(O[W],O[W+1]);V==="#00ff88"?H+=2:V==="#FFD700"&&(H+=1)}return H}const f=w(I)+k(I),m=w(F)+k(F),x=f>=m;function b(O,H,W){return`<div id="duel-row-${W}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
        ...
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${O.map((V,te)=>{const oe=te<O.length-1?at(V,O[te+1]):null,fe=!oe||oe==="#ff3333"||oe==="#cc2222",xe=oe==="#00ff88"?"+2":oe==="#FFD700"?"+1":"";return`<div class="duel-card duel-card-${W}" data-idx="${te}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease,transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
              ${Ye({...V,note:ze(V,"MIL"),_line:"MIL"},58,78)}
            </div>
            ${te<O.length-1?`<div class="duel-link duel-link-${W}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${fe?"rgba(255,255,255,0.12)":oe};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${fe?"none":`0 0 8px ${oe}`}">
              ${xe?`<span style="position:absolute;top:-13px;left:50%;transform:translateX(-50%);font-size:8px;font-weight:900;color:${oe}">${xe}</span>`:""}
            </div>`:""}`}).join("")}
        </div>
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      ${b(F,r[o+"Name"]||"Adversaire","opp")}
      <div id="duel-shock" style="position:absolute;left:50%;top:50%;width:120px;height:120px;border-radius:50%;border:6px solid #FFD700;opacity:0;pointer-events:none"></div>
      <div id="pvp-duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center,rgba(10,61,30,.4),rgba(10,61,30,.92))"></div>
    </div>`;const h=(O,H)=>e.querySelectorAll(O).forEach((W,V)=>{setTimeout(()=>{W.style.opacity="1",W.style.transform="translateY(0) scale(1)"},H+V*90)});h(".duel-card-me",150),h(".duel-card-opp",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((O,H)=>setTimeout(()=>{O.style.opacity="1"},H*70)),900),setTimeout(()=>{const O=e.querySelector("#pvp-vs");O&&(O.style.opacity="1",O.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(H=>H.style.opacity="1")},1250);function $(O,H,W){const V=document.getElementById(O);if(!V)return;const te=performance.now(),oe=fe=>{const xe=Math.min(1,(fe-te)/W);V.textContent=Math.round(H*(1-Math.pow(1-xe,3))),xe<1?requestAnimationFrame(oe):V.textContent=H};requestAnimationFrame(oe)}setTimeout(()=>{$("pvp-score-me",f,800),$("pvp-score-opp",m,800)},1500);const A=r.p1Team.MIL||[],_=r.p2Team.MIL||[],z=w(A)+k(A),S=w(_)+k(_),q=z>=S?"p1":"p2";let B=r.boostValue;B==null&&(B=ti(),r.boostValue=B,r.boostOwner=q,r.boostUsed=!1),pe=!0,setTimeout(()=>{const O=e.querySelector("#duel-row-"+(x?"me":"opp")),H=e.querySelector("#duel-row-"+(x?"opp":"me")),W=document.getElementById("pvp-score-me"),V=document.getElementById("pvp-score-opp"),te=x?W:V,oe=x?V:W;te&&(te.style.fontSize="80px",te.style.color=x?"#FFD700":"#ff6b6b",te.style.animation="duelPulse .5s ease"+(x?",duelGlow 1.5s ease infinite .5s":"")),oe&&(oe.style.opacity="0.25"),setTimeout(()=>{O&&(O.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",O.style.zIndex="5"),setTimeout(()=>{const fe=document.getElementById("duel-shock");fe&&(fe.style.animation="shockwave .5s ease-out forwards"),H&&(H.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var we;const fe=document.getElementById("pvp-duel-finale");if(!fe)return;const xe=r.boostOwner===n?'<div style="background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:3px solid #cdeffd;border-radius:18px;padding:20px 34px;text-align:center;animation:boostFlipIn .7s cubic-bezier(.34,1.56,.64,1) both;box-shadow:0 10px 36px rgba(135,206,235,.5)"><div style="font-size:10px;color:rgba(0,0,0,.6);letter-spacing:2px;text-transform:uppercase;margin-bottom:6px;font-weight:700">Carte Boost obtenue</div><div style="font-size:46px;line-height:1">⚡</div><div style="font-size:50px;font-weight:900;color:#063;line-height:1.1">+'+B+`</div><div style="font-size:10px;color:rgba(0,0,0,.55);margin-top:4px">Applicable sur n'importe quel joueur</div></div>`:"",ve=n==="p1"?'<button id="pvp-start-match" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">▶ Commencer le match</button>':`<div style="font-size:14px;color:rgba(255,255,255,0.5);text-align:center;margin-top:8px;animation:fadeUp .4s ease both">⏳ En attente de l'adversaire...</div>`;fe.innerHTML='<div style="font-size:22px;font-weight:900;color:#fff;text-align:center;animation:fadeUp .4s ease both;text-shadow:0 2px 12px rgba(0,0,0,.5)">'+(x?"⚽ "+r[n+"Name"]+"<br>gagne le milieu et attaque !":"😔 "+r[o+"Name"]+"<br>gagne l'engagement et attaque !")+"</div>"+xe+ve,fe.style.transition="opacity .45s ease",fe.style.opacity="1",fe.style.pointerEvents="auto",(we=document.getElementById("pvp-start-match"))==null||we.addEventListener("click",async()=>{const Ie=q;await D({phase:Ie+"-attack",attacker:Ie,round:1,boostValue:B,boostUsed:!1,boostOwner:Ie})})},600)},700)},2800)}function Te(I,F,w,k,f){const m=document.createElement("div");m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:900;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;overflow:hidden;cursor:pointer";const x=Array.from({length:10},(A,_)=>`<div style="position:absolute;font-size:${16+Math.floor(Math.random()*24)}px;top:${5+Math.floor(Math.random()*65)}%;left:${3+Math.floor(Math.random()*94)}%;animation:fw${_%2===0?"A":"B"} ${.7+Math.random()*.7}s ease ${Math.random()*.9}s both;opacity:0">${["✨","🌟","⭐","💥","🎇","🎆","🔥","🌈"][_%8]}</div>`).join(""),b={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};m.innerHTML=`
      <style>
        @keyframes butPop{0%{transform:scale(0) rotate(-8deg);opacity:0}55%{transform:scale(1.25) rotate(2deg)}85%{transform:scale(0.92) rotate(-1deg)}100%{transform:scale(1);opacity:1}}
        @keyframes ballIn{0%{transform:translate(-70px,18px);opacity:0}65%{opacity:1}100%{transform:translate(26px,-8px);opacity:1}}
        @keyframes scoreIn{from{opacity:0;transform:translateY(-12px)}to{opacity:1;transform:translateY(0)}}
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(m);let h=!1;const $=()=>{h||(h=!0,m.remove(),setTimeout(()=>f(),50))};m.addEventListener("click",$),setTimeout($,3500)}function Ee(I,F,w){var B,O;const k=(r.gcDefs||[]).find(H=>{var W;return H.name===I||((W=H.name)==null?void 0:W.toLowerCase().trim())===(I==null?void 0:I.toLowerCase().trim())}),f={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[k==null?void 0:k.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",m={purple:"#b06ce0",light_blue:"#00d4ef"}[k==null?void 0:k.color]||"#b06ce0",x=(k==null?void 0:k.name)||I,b=(k==null?void 0:k.effect)||((B=Ke[I])==null?void 0:B.desc)||"",h=k!=null&&k.image_url?`/manager-wars/icons/${k.image_url}`:null,$=((O=Ke[I])==null?void 0:O.icon)||"⚡",_=F===n?"Vous":r[F+"Name"]||"Adversaire",z=document.createElement("div");z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",z.innerHTML=`
      <style>
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(z);let S=!1;const q=()=>{S||(S=!0,z.remove(),setTimeout(()=>w&&w(),50))};z.addEventListener("click",q),setTimeout(q,3e3)}function he(I,F){var z,S,q,B;const k=(r["gcCardsFull_"+n]||[]).find(O=>O.id===I),f=k==null?void 0:k._gcDef,m={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[f==null?void 0:f.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",x={purple:"#b06ce0",light_blue:"#00d4ef"}[f==null?void 0:f.color]||"#b06ce0",b=(f==null?void 0:f.name)||F,h=(f==null?void 0:f.effect)||((z=Ke[F])==null?void 0:z.desc)||"Carte spéciale.",$=f!=null&&f.image_url?`/manager-wars/icons/${f.image_url}`:null,A=((S=Ke[F])==null?void 0:S.icon)||"⚡",_=document.createElement("div");_.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",_.innerHTML=`
      ...
      </div>`,document.body.appendChild(_),(q=_.querySelector("#pvp-gc-back"))==null||q.addEventListener("click",()=>_.remove()),(B=_.querySelector("#pvp-gc-use"))==null||B.addEventListener("click",()=>{_.remove(),P(I,F)})}function $e(){var k;const I=r[n+"Team"],F=Object.entries(I).flatMap(([f,m])=>(m||[]).filter(x=>!x.used).map(x=>({...x,_line:f})));if(!F.length)return;const w=document.createElement("div");w.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",w.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
        <div style="flex:1;font-size:14px;font-weight:700;color:#fff">⚡ Choisir un joueur pour +${r.boostValue}</div>
        <button id="bp-close" style="background:none;border:none;color:rgba(255,255,255,0.5);font-size:22px;cursor:pointer">✕</button>
      </div>
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${F.map(f=>{const m={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[f._line]||"#555",x=ze(f,f._line)+(f.boost||0);return`<div class="bp-item" data-cid="${f.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${m};overflow:hidden;cursor:pointer">
      ...
      </div>`,document.body.appendChild(w),(k=w.querySelector("#bp-close"))==null||k.addEventListener("click",()=>w.remove()),w.querySelectorAll(".bp-item").forEach(f=>{f.addEventListener("click",async()=>{const m=f.dataset.cid,x=F.find(h=>h.cardId===m);if(!x)return;const b=(I[x._line]||[]).find(h=>h.cardId===m);b&&(b.boost=(b.boost||0)+r.boostValue),w.remove(),await D({[n+"Team"]:I,boostUsed:!0})})})}function Ae(I=null){var S,q;if(!(r.phase===n+"-attack")){p("Remplacement uniquement avant votre attaque","warning");return}const w=r[n+"Team"],k=r["usedSubIds_"+n]||[],f=r.maxSubs||3;if(k.length>=f){p(`Maximum ${f} remplacements atteint`,"warning");return}const m=Object.entries(w).flatMap(([B,O])=>(O||[]).filter(H=>H.used).map(H=>({...H,_line:B}))),x=(r[n+"Subs"]||[]).filter(B=>!k.includes(B.cardId));if(!m.length){p("Aucun joueur utilisé à remplacer","warning");return}if(!x.length){p("Aucun remplaçant disponible","warning");return}let b=Math.max(0,m.findIndex(B=>B.cardId===I));const h=((S=m[b])==null?void 0:S._line)||((q=m[b])==null?void 0:q.job);let $=Math.max(0,x.findIndex(B=>B.job===h)),A=!1;const _=document.createElement("div");_.id="pvp-sub-overlay",_.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function z(){var oe,fe,xe,ve,we,Ie;const B=m[b],O=x[$],H=Math.min(130,Math.round((window.innerWidth-90)/2)),W=Math.round(H*1.35),V=Le=>`background:rgba(255,255,255,0.12);border:none;color:${Le?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Le?"default":"pointer"};flex-shrink:0`;_.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      </div>`,(oe=_.querySelector("#psub-close"))==null||oe.addEventListener("click",()=>_.remove()),(fe=_.querySelector("#pout-up"))==null||fe.addEventListener("click",()=>{b>0&&(b--,z())}),(xe=_.querySelector("#pout-down"))==null||xe.addEventListener("click",()=>{b<m.length-1&&(b++,z())}),(ve=_.querySelector("#pin-up"))==null||ve.addEventListener("click",()=>{$>0&&($--,z())}),(we=_.querySelector("#pin-down"))==null||we.addEventListener("click",()=>{$<x.length-1&&($++,z())});const te=(Le,Re,je,Oe)=>{const Fe=_.querySelector("#"+Le);if(!Fe)return;let Ve=0;Fe.addEventListener("touchstart",Ne=>{Ve=Ne.touches[0].clientY},{passive:!0}),Fe.addEventListener("touchend",Ne=>{const He=Ne.changedTouches[0].clientY-Ve;if(Math.abs(He)<30)return;const Ue=Re();He<0&&Ue<Oe-1?(je(Ue+1),z()):He>0&&Ue>0&&(je(Ue-1),z())},{passive:!0})};te("pin-panel",()=>$,Le=>$=Le,x.length),te("pout-panel",()=>b,Le=>b=Le,m.length),(Ie=_.querySelector("#psub-confirm"))==null||Ie.addEventListener("click",async Le=>{if(Le.preventDefault(),Le.stopPropagation(),A)return;A=!0;const Re=m[b],je=x[$];if(!Re||!je)return;const Oe=Re._line,Fe=(w[Oe]||[]).findIndex(He=>He.cardId===Re.cardId);if(Fe===-1){p("Erreur : joueur introuvable","error"),_.remove();return}const Ve={...je,_line:Oe,position:Re.position,used:!1,boost:0};w[Oe].splice(Fe,1,Ve);const Ne=[...k,je.cardId];_.remove(),ie(Re,je,async()=>{await D({[n+"Team"]:w,[o+"Team"]:r[o+"Team"],["usedSubIds_"+n]:Ne})})})}document.body.appendChild(_),z()}function ie(I,F,w){const k={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const m=(h,$,A)=>`<div style="text-align:center">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:6px">Appuyer pour continuer</div>`,document.body.appendChild(f);let x=!1;const b=()=>{x||(x=!0,f.remove(),setTimeout(()=>w(),50))};f.addEventListener("click",b),setTimeout(b,2200)}function ae(){var F;const I=document.createElement("div");I.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:20px;overflow-y:auto",I.innerHTML=`
      <div style="font-size:12px;color:rgba(255,255,255,0.5);letter-spacing:2px;text-transform:uppercase">Équipe adverse</div>
      <div style="font-size:18px;font-weight:900;color:#ff6b6b">${r[o+"Name"]}</div>
      <div style="width:min(90vw,420px)">${ut(r[o+"Team"],r[o+"Formation"],null,[],300,300)}</div>
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(I),(F=I.querySelector("#pvp-opp-close"))==null||F.addEventListener("click",()=>I.remove())}function R(){var w;const I=r.log||[],F=document.createElement("div");F.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column",F.innerHTML=`
      ...
      </div>`,document.body.appendChild(F),(w=F.querySelector("#pvp-hist-close"))==null||w.addEventListener("click",()=>F.remove())}async function Q(){if(r.phase!==n+"-attack")return;const I=n==="p1"?"p2":"p1",F=(r.round||0)+1,w=[...r.log||[]];w.push({type:"info",text:`⏭️ ${r[n+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const k=Ce(r),f=Se(I),m=k||!f?"finished":I+"-attack";await D({["selected_"+n]:[],modifiers:{...r.modifiers,[n]:{}},pendingAttack:null,phase:m,attacker:I,round:F,log:w}),m==="finished"&&await qe(r)}async function ue(){const I=r[n+"Team"],F=!["GK","DEF","MIL","ATT"].some(m=>(r[o+"Team"][m]||[]).some(x=>!x.used)),w=(r["selected_"+n]||[]).map(m=>{const x=(I[m._role]||[]).find(z=>z.cardId===m.cardId)||m,b=F&&["GK","DEF"].includes(m._role),h=I[m._role]||[],$=h.findIndex(z=>z.cardId===m.cardId),A=lt(h.length),_=$>=0?A[$]:x._col??1;return{...x,_line:m._role,_col:_,...b?{note_a:Math.max(1,Number(x.note_a)||0)}:{}}});if(!w.length)return;const k=Nt(w,r.modifiers[n]||{});Y(n,w.map(m=>m.cardId)),w.forEach(m=>{const x=(I[m._role]||[]).find(b=>b.cardId===m.cardId);x&&(x.used=!0)}),r["selected_"+n]=[],G();const f=[...r.log||[]];if(F){const m=(r[n+"Score"]||0)+1,x=w.map(_=>({name:_.name,note:ze(_,_._line||"ATT"),portrait:Pe(_),job:_.job}));f.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:x,homeTotal:k.total,aiTotal:0});const b=(r.round||0)+1,h=n==="p1"?"p2":"p1",$={...r,[n+"Team"]:I,[n+"Score"]:m},A=Ce($);N.add(b),Te(x,m,r[o+"Score"]||0,!0,async()=>{await D({[n+"Team"]:I,[n+"Score"]:m,["selected_"+n]:[],modifiers:{...r.modifiers,[n]:{}},pendingAttack:null,phase:A?"finished":h+"-attack",attacker:h,round:b,log:f}),A&&await qe({...r,[n+"Score"]:m})});return}f.push({type:"pending",text:`⚔️ ${r[n+"Name"]} attaque (${k.total})`}),await D({[n+"Team"]:I,[o+"Team"]:r[o+"Team"],pendingAttack:{...k,players:w,side:n},["selected_"+n]:[],modifiers:{...r.modifiers,[n]:{}},phase:o+"-defense",log:f})}async function me(){const I=r[n+"Team"],F=(r["selected_"+n]||[]).map(B=>{const O=(I[B._role]||[]).find(oe=>oe.cardId===B.cardId)||B,H=I[B._role]||[],W=H.findIndex(oe=>oe.cardId===B.cardId),V=lt(H.length),te=W>=0?V[W]:O._col??1;return{...O,_line:B._role,_col:te}}),w=Rt(F,r.modifiers[n]||{});Y(n,F.map(B=>B.cardId)),F.forEach(B=>{const O=(I[B._role]||[]).find(H=>H.cardId===B.cardId);O&&(O.used=!0)}),r["selected_"+n]=[],G();const k=Ot(r.pendingAttack.total,w.total,r.modifiers[n]||{}),f=r.pendingAttack.side,m=k==="attack"||(k==null?void 0:k.goal),x=f==="p1"?"p2":"p1",b=(r.round||0)+1,h=(r.pendingAttack.players||[]).map(B=>({name:B.name,note:ze(B,B._line||"ATT"),portrait:Pe(B),job:B.job})),$=[...r.log||[]];$.push({type:"duel",isGoal:m,homeScored:m&&f===n,text:m?`⚽ BUT de ${r[f+"Name"]} ! (${r.pendingAttack.total} vs ${w.total})`:`✋ Attaque stoppée (${r.pendingAttack.total} vs ${w.total})`,homePlayers:h,aiPlayers:F.map(B=>({name:B.name,note:ze(B,B._line||"DEF"),portrait:Pe(B),job:B.job})),homeTotal:r.pendingAttack.total,aiTotal:w.total});const A=m?(r[f+"Score"]||0)+1:r[f+"Score"]||0,_={...r,[n+"Team"]:I,[f+"Score"]:A},z=Ce(_),S=z?"finished":x+"-attack",q=async()=>{await D({[n+"Team"]:I,[o+"Team"]:r[o+"Team"],[f+"Score"]:A,["selected_"+n]:[],modifiers:{...r.modifiers,[n]:{}},pendingAttack:null,phase:S,attacker:x,round:b,log:$}),(S==="finished"||z)&&await qe({...r,[f+"Score"]:A})};if(m){const B=f===n,O=B?A:r[n+"Score"]||0,H=B?r[o+"Score"]||0:A;N.add(b),Te(h,O,H,B,q)}else await q()}function X(I){return["MIL","ATT"].some(F=>(I[F]||[]).some(w=>!w.used))}function be(I){return["GK","DEF","MIL","ATT"].some(F=>(I[F]||[]).some(w=>!w.used))}function ge(I){return be(I)&&!X(I)}function Se(I){const F=r[I+"Team"],w=r[(I==="p1"?"p2":"p1")+"Team"];return!!(X(F)||!be(w)&&ge(F))}function Ce(I){const F=["MIL","ATT"].some($=>(I.p1Team[$]||[]).some(A=>!A.used)),w=["MIL","ATT"].some($=>(I.p2Team[$]||[]).some(A=>!A.used)),k=be(I.p1Team),f=be(I.p2Team);return!F&&!(!f&&k)&&(!w&&!(!k&&f))}function Be(I){const F=I.p1Score||0,w=I.p2Score||0;return F===w?null:F>w?y.home_id:y.away_id}async function qe(I){try{const F=Be(I),w=F?y.home_id===F?y.away_id:y.home_id:null;await E.from("matches").update({status:"finished",winner_id:F,home_score:I.p1Score||0,away_score:I.p2Score||0}).eq("id",i),F&&w&&updateEvolutiveCards(F,w).catch(()=>{})}catch(F){console.error("[PvP] finishMatch:",F)}}function De(){var f;const I=r[n+"Score"],F=r[o+"Score"],w=I>F,k=I===F;et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(b),(z=b.querySelector("#pvp-end-home"))==null||z.addEventListener("click",()=>{b.remove(),We(e),l("home")})}const Z=E.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},w=>{const k=w.new;try{if(k.status==="finished"||k.forfeit){if(u)return;u=!0,N&&(clearInterval(N),N=null),k.game_state&&(r=k.game_state),D(k);return}if(k.game_state){const f=r;r=k.game_state;const m=r._lastGC;if(m&&m.seq&&!j.has(m.seq)&&(j.add(m.seq),m.by!==n)){$e(m.type,m.by,()=>pe());return}const x=f[n+"Score"]||0,b=f[o+"Score"]||0,h=r[n+"Score"]||0,$=r[o+"Score"]||0,A=h>x,_=$>b;if((A||_)&&!C.has(r.round)){C.add(r.round);const z=[...r.log||[]].reverse().find(q=>q.isGoal),S=((z==null?void 0:z.homePlayers)||[]).map(q=>({name:q.name,note:q.note,portrait:q.portrait,job:q.job}));he(S,h,$,A,()=>pe());return}pe()}}catch(f){console.error("[PvP] crash:",f)}}).subscribe();async function se(w){Object.assign(r,w),r.lastActionAt=new Date().toISOString();const{error:k}=await E.from("matches").update({game_state:r}).eq("id",i);k&&p("Erreur de synchronisation","error");try{pe()}catch(f){console.error("[PvP] renderPvpScreen crash:",f)}}async function re(){if(u)return;u=!0,N&&(clearInterval(N),N=null);const w=s?y.away_id:y.home_id,k=s?"p2":"p1",f=s?"p1":"p2",m={...r,[k+"Score"]:3,[f+"Score"]:0,phase:"finished"};try{await E.from("matches").update({status:"finished",forfeit:!0,winner_id:w,home_score:m.p1Score||0,away_score:m.p2Score||0,game_state:m}).eq("id",i)}catch{}try{E.removeChannel(Z)}catch{}setTimeout(()=>{We(e),l("home")},800)}const P={BOOST_STAT:({value:w=1,count:k=1,roles:f=[]},m,x)=>{const b=m[n+"Team"],h=Object.entries(b).filter(([$])=>!f.length||f.includes($)).flatMap(([$,A])=>A.filter(_=>!_.used).map(_=>({..._,_line:$})));if(!h.length){m.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),x(m);return}Y(h,k,`Choisir ${k} joueur(s) à booster (+${w})`,$=>{$.forEach(A=>{const _=(b[A._line]||[]).find(z=>z.cardId===A.cardId);_&&(_.boost=(_.boost||0)+w,m.log.push({text:`⚡ +${w} sur ${_.name}`,type:"info"}))}),m[n+"Team"]=b,x(m)})},DEBUFF_STAT:({value:w=1,count:k=1,roles:f=[],target:m="ai"},x,b)=>{const h=m==="home"?n:o,$=x[h+"Team"],A=m==="home"?"allié":"adverse",_=Object.entries($).filter(([z])=>!f.length||f.includes(z)).flatMap(([z,S])=>S.map(q=>({...q,_line:z})));if(!_.length){x.log.push({text:`🎯 Aucun joueur ${A}`,type:"info"}),b(x);return}Y(_,k,`Choisir ${k} joueur(s) ${A}(s) (-${w})`,z=>{z.forEach(S=>{const q=($[S._line]||[]).find(B=>B.cardId===S.cardId);q&&(q.boost=(q.boost||0)-w,x.log.push({text:`🎯 -${w} sur ${q.name}`,type:"info"}))}),x[h+"Team"]=$,b(x)})},GRAY_PLAYER:({count:w=1,roles:k=[],target:f="ai"},m,x)=>{const b=f==="home"?n:o,h=m[b+"Team"],$=f==="home"?"allié":"adverse",A=Object.entries(h).filter(([_])=>!k.length||k.includes(_)).flatMap(([_,z])=>z.filter(S=>!S.used).map(S=>({...S,_line:_})));if(!A.length){m.log.push({text:`❌ Aucun joueur ${$}`,type:"info"}),x(m);return}Y(A,w,`Choisir ${w} joueur(s) ${$}(s) à exclure`,_=>{const z="usedCardIds_"+b,S=new Set(m[z]||[]);_.forEach(q=>{const B=(h[q._line]||[]).find(O=>O.cardId===q.cardId);B&&(B.used=!0,S.add(q.cardId),m.log.push({text:`❌ ${B.name} exclu !`,type:"info"}))}),m[z]=[...S],m[b+"Team"]=h,x(m)})},REVIVE_PLAYER:({count:w=1,roles:k=[]},f,m)=>{const x=f[n+"Team"],b=Object.entries(x).filter(([h])=>!k.length||k.includes(h)).flatMap(([h,$])=>$.filter(A=>A.used).map(A=>({...A,_line:h})));if(!b.length){f.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),m(f);return}Y(b,w,`Choisir ${w} joueur(s) à ressusciter`,h=>{h.forEach($=>{const A=(x[$._line]||[]).find(_=>_.cardId===$.cardId);A&&(A.used=!1,f.log.push({text:`💫 ${A.name} ressuscité !`,type:"info"}))}),f[n+"Team"]=x,m(f)})},REMOVE_GOAL:({},w,k)=>{const f=o+"Score";w[f]>0?(w[f]--,w.log.push({text:"🚫 Dernier but annulé !",type:"info"})):w.log.push({text:"🚫 Aucun but à annuler",type:"info"}),k(w)},ADD_GOAL_DRAW:({},w,k)=>{w[n+"Score"]===w[o+"Score"]?(w[n+"Score"]++,w.log.push({text:"🎯 But bonus !",type:"info"})):w.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),k(w)},ADD_SUB:({value:w=1},k,f)=>{k.maxSubs=(k.maxSubs||3)+w,k.log.push({text:`🔄 +${w} remplacement(s)`,type:"info"}),f(k)},CUSTOM:({},w,k)=>k(w)};function Y(w,k,f,m){const x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let b=[];function h(){var A,_;const $=w.map(z=>{const S={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[z._line]||"#555",q=ze(z,z._line)+(z.boost||0),O=b.find(W=>W.cardId===z.cardId)?"#FFD700":"rgba(255,255,255,0.25)",H=z.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${z.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${O};background:${S};overflow:hidden;cursor:pointer;${H}">
        ...
        </div>`,(A=x.querySelector("#pp-close"))==null||A.addEventListener("click",()=>x.remove()),x.querySelectorAll(".pp-item").forEach(z=>{z.addEventListener("click",()=>{const S=z.dataset.cid,q=w.find(O=>O.cardId===S),B=b.findIndex(O=>O.cardId===S);q&&(B>-1?b.splice(B,1):b.length<k&&b.push(q),h())})}),(_=x.querySelector("#pp-confirm"))==null||_.addEventListener("click",()=>{x.remove(),m(b)})}h(),document.body.appendChild(x)}async function U(w,k){const m=(r["gcCardsFull_"+n]||[]).find($=>$.id===w),x=(m==null?void 0:m._gcDef)||(r.gcDefs||[]).find($=>{var A;return $.name===k||((A=$.name)==null?void 0:A.toLowerCase().trim())===(k==null?void 0:k.toLowerCase().trim())}),b=[...r["usedGc_"+n]||[],w],h={type:k,by:n,seq:(r._gcAnimSeq||0)+1};j.add(h.seq),$e(k,n,async()=>{if(x!=null&&x.effect_type&&x.effect_type!=="CUSTOM"){const A=P[x.effect_type];if(A){const _={...r};A(x.effect_params||{},_,async z=>{z["usedGc_"+n]=b,z._lastGC=h,z._gcAnimSeq=h.seq,await se(z)});return}}const $={...r};switch(k){case"Remplacement+":$.maxSubs=($.maxSubs||3)+1,$.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const A=o+"Score";$[A]>0&&($[A]--,$.log.push({text:"🚫 But annulé",type:"info"}));break}}$["usedGc_"+n]=b,$._lastGC=h,$._gcAnimSeq=h.seq,await se($)})}function G(w,k){const f="usedCardIds_"+w,m=new Set(r[f]||[]);k.forEach(x=>m.add(x)),r[f]=[...m]}function ne(){for(const w of["p1","p2"]){const k=new Set(r["usedCardIds_"+w]||[]),f=r[w+"Team"];if(f)for(const m of["GK","DEF","MIL","ATT"])(f[m]||[]).forEach(x=>{x.used=k.has(x.cardId)})}}function pe(){var Ze,Qe,ot,gt,de,_e;if(r.phase==="reveal")return ye();if(r.phase==="midfield")return Ee();if(r.phase==="finished")return F();const w=r[n+"Team"],k=r[o+"Team"];ne();const f=r[n+"Score"],m=r[o+"Score"],x=r[n+"Name"],b=r[o+"Name"],h=r.phase===n+"-attack",$=r.phase===n+"-defense",A=Array.isArray(r["selected_"+n])?r["selected_"+n]:[],_=A.map(ee=>ee.cardId),z=window.innerWidth>=700,S=r[n+"Subs"]||[],q=r["usedSubIds_"+n]||[],B=S.filter(ee=>!q.includes(ee.cardId)),O=r["gcCardsFull_"+n]||[],H=r["usedGc_"+n]||[],W=O.filter(ee=>!H.includes(ee.id)),V=r.boostOwner===n&&!r.boostUsed,te=!["GK","DEF","MIL","ATT"].some(ee=>(k[ee]||[]).some(ce=>!ce.used)),oe=[...w.MIL||[],...w.ATT||[]].filter(ee=>!ee.used),fe=h&&te&&oe.length===0?[...w.GK||[],...w.DEF||[]].filter(ee=>!ee.used).map(ee=>ee.cardId):[];function xe(ee,ce,J){var kt,Ht;const le=ee._gcDef,Me={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[le==null?void 0:le.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",ke={purple:"#b06ce0",light_blue:"#00d4ef"}[le==null?void 0:le.color]||"#b06ce0",tt=(le==null?void 0:le.name)||ee.gc_type,it=(le==null?void 0:le.effect)||((kt=Ke[ee.gc_type])==null?void 0:kt.desc)||"",rt=le!=null&&le.image_url?`/manager-wars/icons/${le.image_url}`:null,st=((Ht=Ke[ee.gc_type])==null?void 0:Ht.icon)||"⚡",ct=Math.round(J*.22),$t=Math.round(J*.22),pt=J-ct-$t,ht=tt.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${ee.id}" data-gc-type="${ee.gc_type}"
      ...
      </div>`}function ve(ee,ce){return`<div id="pvp-boost-card"
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${ft(w,r[n+"Formation"],Fe,_,300,300,fe)}
    ...
    </div>`;function Ne(ee){if(ee.type==="duel"&&(ee.homeTotal!=null||ee.aiTotal!=null)){const ce=(ee.homeTotal||0)>=(ee.aiTotal||0);return`<div style="background:rgba(255,255,255,0.05);border-radius:8px;padding:5px 6px;border:1px solid rgba(255,255,255,0.08)">
          ...
          <div style="display:flex;align-items:center;gap:3px;max-height:46px;overflow:hidden">
            <div style="flex:1;display:flex;gap:2px;justify-content:flex-end;flex-wrap:nowrap;overflow-x:auto;overflow-y:hidden">
              ${(ee.homePlayers||[]).map(J=>dt(J)).join("")}
            </div>
            <div style="text-align:center;padding:0 6px;flex-shrink:0">
      ...
      </div>`;function Je(){const ee=e.querySelector(".match-screen");if(!ee)return;const ce=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);ee.style.height=ce+"px",ee.style.minHeight=ce+"px",ee.style.maxHeight=ce+"px",ee.style.overflow="hidden";const J=e.querySelector("#mobile-action-bar"),le=e.querySelector("#mobile-play-area");J&&le&&(le.style.paddingBottom=J.offsetHeight+"px")}if(Je(),setTimeout(Je,120),setTimeout(Je,400),K||(K=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",Je),window.visualViewport.addEventListener("scroll",Je)),window.addEventListener("resize",Je)),function(){const ce=e.querySelector(".terrain-wrapper svg");ce&&(ce.removeAttribute("width"),ce.removeAttribute("height"),ce.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",ce.setAttribute("viewBox","-26 -26 352 352"),ce.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),e.querySelectorAll(".match-slot-hit").forEach(ee=>{ee.addEventListener("click",()=>{if(!h&&!$)return;const ce=ee.dataset.cardId,J=ee.dataset.role,le=(w[J]||[]).find(it=>it.cardId===ce);if(!le||le.used)return;const Me=fe.includes(ce);if(h&&!["MIL","ATT"].includes(J)&&!Me)return;Array.isArray(r["selected_"+n])||(r["selected_"+n]=[]);const ke=r["selected_"+n],tt=ke.findIndex(it=>it.cardId===ce);tt>-1?ke.splice(tt,1):ke.length<3&&ke.push({...le,_role:J}),pe()})}),e.querySelectorAll(".match-used-hit").forEach(ee=>{ee.addEventListener("click",()=>ae(ee.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(ee=>{ee.addEventListener("click",()=>ae())}),(Ze=e.querySelector("#pvp-sub-open"))==null||Ze.addEventListener("click",()=>ae()),e.querySelectorAll(".pvp-gc-mini").forEach(ee=>{ee.addEventListener("click",()=>Ae(ee.dataset.gcId,ee.dataset.gcType))}),(Qe=e.querySelector("#pvp-boost-card"))==null||Qe.addEventListener("click",()=>ie()),(ot=e.querySelector("#pvp-action"))==null||ot.addEventListener("click",ee=>{h?ee.currentTarget.dataset.pass==="1"||!Be(n)?me():X():$&&be()}),(gt=e.querySelector("#pvp-quit"))==null||gt.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&re()}),(de=e.querySelector("#pvp-view-opp"))==null||de.addEventListener("click",()=>Q()),(_e=e.querySelector("#pvp-toggle-history"))==null||_e.addEventListener("click",()=>ue()),N&&(clearInterval(N),N=null),(h||$)&&!u){let ee=30,ce=!1;const J=()=>document.getElementById("pvp-timer"),le=()=>{J()&&(J().textContent=ee+"s",J().style.color=ce?"#ff4444":"#fff")};le(),N=setInterval(()=>{ee--,ee<0?ce?(clearInterval(N),N=null,h&&!Be(n)?me():re()):(ce=!0,ee=15,le()):le()},1e3)}}function ye(){et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
      <div style="font-size:20px;font-weight:900;color:#ff6b6b">${r[o+"Name"]||"Adversaire"}</div>
      <div style="width:min(90vw,420px)">${ut(r[o+"Team"],r[o+"Formation"],null,[],300,300)}</div>
    </div>`,n==="p1"&&setTimeout(async()=>{await se({phase:"midfield"})},5e3)}let Te=!1;function Ee(){if(Te)return;const w=r[n+"Team"].MIL||[],k=r[o+"Team"].MIL||[];function f(W){return W.reduce((V,te)=>V+ze(te,"MIL"),0)}function m(W){let V=0;for(let te=0;te<W.length-1;te++){const oe=at(W[te],W[te+1]);oe==="#00ff88"?V+=2:oe==="#FFD700"&&(V+=1)}return V}const x=f(w)+m(w),b=f(k)+m(k),h=x>=b;function $(W,V,te){return`<div id="duel-row-${te}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
        ...
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${W.map((oe,fe)=>{const xe=fe<W.length-1?at(oe,W[fe+1]):null,ve=!xe||xe==="#ff3333"||xe==="#cc2222",we=xe==="#00ff88"?"+2":xe==="#FFD700"?"+1":"";return`<div class="duel-card duel-card-${te}" data-idx="${fe}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease,transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
              ${Ye({...oe,note:ze(oe,"MIL"),_line:"MIL"},58,78)}
            </div>
            ${fe<W.length-1?`<div class="duel-link duel-link-${te}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${ve?"rgba(255,255,255,0.12)":xe};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${ve?"none":`0 0 8px ${xe}`}">
              ${we?`<span style="position:absolute;top:-13px;left:50%;transform:translateX(-50%);font-size:8px;font-weight:900;color:${xe}">${we}</span>`:""}
            </div>`:""}`}).join("")}
        </div>
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(b);let A=!1;const _=()=>{A||(A=!0,b.remove(),setTimeout(()=>x(),50))};b.addEventListener("click",_),setTimeout(_,3500)}function $e(w,k,f){var H,W;const m=(r.gcDefs||[]).find(V=>{var te;return V.name===w||((te=V.name)==null?void 0:te.toLowerCase().trim())===(w==null?void 0:w.toLowerCase().trim())}),x={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[m==null?void 0:m.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",b={purple:"#b06ce0",light_blue:"#00d4ef"}[m==null?void 0:m.color]||"#b06ce0",h=(m==null?void 0:m.name)||w,$=(m==null?void 0:m.effect)||((H=Ke[w])==null?void 0:H.desc)||"",A=m!=null&&m.image_url?`/manager-wars/icons/${m.image_url}`:null,_=((W=Ke[w])==null?void 0:W.icon)||"⚡",S=k===n?"Vous":r[k+"Name"]||"Adversaire",q=document.createElement("div");q.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",q.innerHTML=`
      <style>
        @keyframes gcFlipIn{0%{transform:perspective(800px) rotateY(90deg) scale(.7);opacity:0}55%{transform:perspective(800px) rotateY(-12deg) scale(1.08);opacity:1}100%{transform:perspective(800px) rotateY(0) scale(1);opacity:1}}
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(q);let B=!1;const O=()=>{B||(B=!0,q.remove(),setTimeout(()=>f&&f(),50))};q.addEventListener("click",O),setTimeout(O,3e3)}function Ae(w,k){var q,B,O,H;const m=(r["gcCardsFull_"+n]||[]).find(W=>W.id===w),x=m==null?void 0:m._gcDef,b={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[x==null?void 0:x.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",h={purple:"#b06ce0",light_blue:"#00d4ef"}[x==null?void 0:x.color]||"#b06ce0",$=(x==null?void 0:x.name)||k,A=(x==null?void 0:x.effect)||((q=Ke[k])==null?void 0:q.desc)||"Carte spéciale.",_=x!=null&&x.image_url?`/manager-wars/icons/${x.image_url}`:null,z=((B=Ke[k])==null?void 0:B.icon)||"⚡",S=document.createElement("div");S.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",S.innerHTML=`
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        <div id="pin-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
          <div style="font-size:9px;color:#00ff88;letter-spacing:2px;text-transform:uppercase;font-weight:700">Joueur qui entre</div>
          <button id="pin-up" style="${oe(_===0)}" ${_===0?"disabled":""}>▲</button>
          <div>${W?Ye({...W,used:!1,boost:0},V,te):"<div>—</div>"}</div>
          <button id="pin-down" style="${oe(_>=h.length-1)}" ${_>=h.length-1?"disabled":""}>▼</button>
          <div style="font-size:10px;color:rgba(255,255,255,0.35)">${_+1}/${h.length}</div>
        </div>
        <div id="pout-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
          <div style="font-size:9px;color:#ff6b6b;letter-spacing:2px;text-transform:uppercase;font-weight:700">Joueur qui sort</div>
          <button id="pout-up" style="${oe($===0)}" ${$===0?"disabled":""}>▲</button>
          <div>${H?Ye({...H,used:!1,boost:0},V,te):"<div>—</div>"}</div>
          <button id="pout-down" style="${oe($>=b.length-1)}" ${$>=b.length-1?"disabled":""}>▼</button>
          <div style="font-size:10px;color:rgba(255,255,255,0.35)">${$+1}/${b.length}</div>
        </div>
      </div>
      <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
        <button id="psub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
      </div>`,(xe=S.querySelector("#psub-close"))==null||xe.addEventListener("click",()=>S.remove()),(ve=S.querySelector("#pout-up"))==null||ve.addEventListener("click",()=>{$>0&&($--,q())}),(we=S.querySelector("#pout-down"))==null||we.addEventListener("click",()=>{$<b.length-1&&($++,q())}),(Ie=S.querySelector("#pin-up"))==null||Ie.addEventListener("click",()=>{_>0&&(_--,q())}),(Le=S.querySelector("#pin-down"))==null||Le.addEventListener("click",()=>{_<h.length-1&&(_++,q())});const fe=(je,Oe,Fe,Ve)=>{const Ne=S.querySelector("#"+je);if(!Ne)return;let He=0;Ne.addEventListener("touchstart",Ue=>{He=Ue.touches[0].clientY},{passive:!0}),Ne.addEventListener("touchend",Ue=>{const Je=Ue.changedTouches[0].clientY-He;if(Math.abs(Je)<30)return;const Ze=Oe();Je<0&&Ze<Ve-1?(Fe(Ze+1),q()):Je>0&&Ze>0&&(Fe(Ze-1),q())},{passive:!0})};fe("pin-panel",()=>_,je=>_=je,h.length),fe("pout-panel",()=>$,je=>$=je,b.length),(Re=S.querySelector("#psub-confirm"))==null||Re.addEventListener("click",async je=>{if(je.preventDefault(),je.stopPropagation(),z)return;z=!0;const Oe=b[$],Fe=h[_];if(!Oe||!Fe)return;const Ve=Oe._line,Ne=(f[Ve]||[]).findIndex(Je=>Je.cardId===Oe.cardId);if(Ne===-1){p("Erreur : joueur introuvable","error"),S.remove();return}const He={...Fe,_line:Ve,position:Oe.position,used:!1,boost:0};f[Ve].splice(Ne,1,He);const Ue=[...m,Fe.cardId];S.remove(),R(Oe,Fe,async()=>{await se({[n+"Team"]:f,[o+"Team"]:r[o+"Team"],["usedSubIds_"+n]:Ue})})})}document.body.appendChild(S),q()}function R(w,k,f){const m={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const b=(A,_,z)=>`<div style="text-align:center">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:6px">Appuyer pour continuer</div>`,document.body.appendChild(x);let h=!1;const $=()=>{h||(h=!0,x.remove(),setTimeout(()=>f(),50))};x.addEventListener("click",$),setTimeout($,2200)}function Q(){var k;const w=document.createElement("div");w.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:20px;overflow-y:auto",w.innerHTML=`
      <div style="font-size:12px;color:rgba(255,255,255,0.5);letter-spacing:2px;text-transform:uppercase">Équipe adverse</div>
      <div style="font-size:18px;font-weight:900;color:#ff6b6b">${r[o+"Name"]}</div>
      <div style="width:min(90vw,420px)">${ut(r[o+"Team"],r[o+"Formation"],null,[],300,300)}</div>
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(w),(k=w.querySelector("#pvp-opp-close"))==null||k.addEventListener("click",()=>w.remove())}function ue(){var m;const w=r.log||[],k=document.createElement("div");k.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column";const f=x=>{var h,$,A;if(x.type==="duel"){const _=x.isGoal,z=x.homeScored&&n==="p1"||!x.homeScored&&_&&n==="p2",S=x.homeScored?"#FFD700":_?"#ff6b6b":"rgba(255,255,255,0.3)",q=_?z?"⚽ BUT !":"⚽ BUT adversaire !":(h=x.homePlayers)!=null&&h.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${_?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${S};margin-bottom:4px">
      ...
      </div>`,document.body.appendChild(k),(m=k.querySelector("#pvp-hist-close"))==null||m.addEventListener("click",()=>k.remove())}async function me(){if(r.phase!==n+"-attack")return;const w=n==="p1"?"p2":"p1",k=(r.round||0)+1,f=[...r.log||[]];f.push({type:"info",text:`⏭️ ${r[n+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const m=qe(r),x=Be(w),b=m||!x?"finished":w+"-attack";await se({["selected_"+n]:[],modifiers:{...r.modifiers,[n]:{}},pendingAttack:null,phase:b,attacker:w,round:k,log:f}),b==="finished"&&await I(r)}async function X(){const w=r[n+"Team"],k=!["GK","DEF","MIL","ATT"].some(b=>(r[o+"Team"][b]||[]).some(h=>!h.used)),f=(r["selected_"+n]||[]).map(b=>{const h=(w[b._role]||[]).find(q=>q.cardId===b.cardId)||b,$=k&&["GK","DEF"].includes(b._role),A=w[b._role]||[],_=A.findIndex(q=>q.cardId===b.cardId),z=lt(A.length),S=_>=0?z[_]:h._col??1;return{...h,_line:b._role,_col:S,...$?{note_a:Math.max(1,Number(h.note_a)||0)}:{}}});if(!f.length)return;const m=Nt(f,r.modifiers[n]||{});G(n,f.map(b=>b.cardId)),f.forEach(b=>{const h=(w[b._role]||[]).find($=>$.cardId===b.cardId);h&&(h.used=!0)}),r["selected_"+n]=[],pe();const x=[...r.log||[]];if(k){const b=(r[n+"Score"]||0)+1,h=f.map(S=>({name:S.name,note:ze(S,S._line||"ATT"),portrait:Pe(S),job:S.job}));x.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:h,homeTotal:m.total,aiTotal:0});const $=(r.round||0)+1,A=n==="p1"?"p2":"p1",_={...r,[n+"Team"]:w,[n+"Score"]:b},z=qe(_);C.add($),he(h,b,r[o+"Score"]||0,!0,async()=>{await se({[n+"Team"]:w,[n+"Score"]:b,["selected_"+n]:[],modifiers:{...r.modifiers,[n]:{}},pendingAttack:null,phase:z?"finished":A+"-attack",attacker:A,round:$,log:x}),z&&await I({...r,[n+"Score"]:b})});return}x.push({type:"pending",text:`⚔️ ${r[n+"Name"]} attaque (${m.total})`}),await se({[n+"Team"]:w,[o+"Team"]:r[o+"Team"],pendingAttack:{...m,players:f,side:n},["selected_"+n]:[],modifiers:{...r.modifiers,[n]:{}},phase:o+"-defense",log:x})}async function be(){const w=r[n+"Team"],k=(r["selected_"+n]||[]).map(H=>{const W=(w[H._role]||[]).find(xe=>xe.cardId===H.cardId)||H,V=w[H._role]||[],te=V.findIndex(xe=>xe.cardId===H.cardId),oe=lt(V.length),fe=te>=0?oe[te]:W._col??1;return{...W,_line:H._role,_col:fe}}),f=Rt(k,r.modifiers[n]||{});G(n,k.map(H=>H.cardId)),k.forEach(H=>{const W=(w[H._role]||[]).find(V=>V.cardId===H.cardId);W&&(W.used=!0)}),r["selected_"+n]=[],pe();const m=Ot(r.pendingAttack.total,f.total,r.modifiers[n]||{}),x=r.pendingAttack.side,b=m==="attack"||(m==null?void 0:m.goal),h=x==="p1"?"p2":"p1",$=(r.round||0)+1,A=(r.pendingAttack.players||[]).map(H=>({name:H.name,note:ze(H,H._line||"ATT"),portrait:Pe(H),job:H.job})),_=[...r.log||[]];_.push({type:"duel",isGoal:b,homeScored:b&&x===n,text:b?`⚽ BUT de ${r[x+"Name"]} ! (${r.pendingAttack.total} vs ${f.total})`:`✋ Attaque stoppée (${r.pendingAttack.total} vs ${f.total})`,homePlayers:A,aiPlayers:k.map(H=>({name:H.name,note:ze(H,H._line||"DEF"),portrait:Pe(H),job:H.job})),homeTotal:r.pendingAttack.total,aiTotal:f.total});const z=b?(r[x+"Score"]||0)+1:r[x+"Score"]||0,S={...r,[n+"Team"]:w,[x+"Score"]:z},q=qe(S),B=q?"finished":h+"-attack",O=async()=>{await se({[n+"Team"]:w,[o+"Team"]:r[o+"Team"],[x+"Score"]:z,["selected_"+n]:[],modifiers:{...r.modifiers,[n]:{}},pendingAttack:null,phase:B,attacker:h,round:$,log:_}),(B==="finished"||q)&&await I({...r,[x+"Score"]:z})};if(b){const H=x===n,W=H?z:r[n+"Score"]||0,V=H?r[o+"Score"]||0:z;C.add($),he(A,W,V,H,O)}else await O()}function ge(w){return["MIL","ATT"].some(k=>(w[k]||[]).some(f=>!f.used))}function Se(w){return["GK","DEF","MIL","ATT"].some(k=>(w[k]||[]).some(f=>!f.used))}function Ce(w){return Se(w)&&!ge(w)}function Be(w){const k=r[w+"Team"],f=r[(w==="p1"?"p2":"p1")+"Team"];return!!(ge(k)||!Se(f)&&Ce(k))}function qe(w){const k=["MIL","ATT"].some(_=>(w.p1Team[_]||[]).some(z=>!z.used)),f=["MIL","ATT"].some(_=>(w.p2Team[_]||[]).some(z=>!z.used)),m=Se(w.p1Team),x=Se(w.p2Team);return!k&&!(!x&&m)&&(!f&&!(!m&&x))}function De(w){const k=w.p1Score||0,f=w.p2Score||0;return k===f?null:k>f?y.home_id:y.away_id}async function I(w){try{const k=De(w),f=k?y.home_id===k?y.away_id:y.home_id:null;await E.from("matches").update({status:"finished",winner_id:k,home_score:w.p1Score||0,away_score:w.p2Score||0}).eq("id",i),k&&f&&_i(k,f).catch(()=>{})}catch(k){console.error("[PvP] finishMatch:",k)}}function F(){var x;const w=r[n+"Score"],k=r[o+"Score"],f=w>k,m=w===k;et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>${v.map((U,G)=>{const ne=T>0&&a.status==="finished"?G===0?Math.floor(T*.7):G===1?Math.floor(T*.2):G===2?Math.floor(T*.1):0:0;return`<tr style="border-bottom:1px solid #f5f5f5;${U.userId===d?"background:#f0fdf4;":""}">
              ...
              ${T>0&&a.status==="finished"?`<td style="text-align:right;font-weight:700;color:${xt}">${ne?ne.toLocaleString("fr")+" cr.":"—"}</td>`:""}
            </tr>`}).join("")}</tbody>
        </table>
      </div>`:""}
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        ${Array.from({length:Math.max(0,a.status==="active"?a.current_day-1:a.current_day)},(U,G)=>G+1).reverse().map(U=>{const G=(p||[]).filter(ne=>ne.matchday===U);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:#888;margin-bottom:6px">Journée ${U}</div>${G.map(ne=>Ri(ne,g,d,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
    ...
    <div style="background:linear-gradient(160deg,#1a0a2e,#3b1a6e);border-radius:20px;padding:32px 24px;max-width:320px;width:100%;text-align:center;color:#fff;box-shadow:0 8px 40px rgba(0,0,0,0.6)">
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">🏆 Retour à la Mini League</button>`,document.body.appendChild($),(q=$.querySelector("#pvp-end-home"))==null||q.addEventListener("click",()=>{$.remove(),We(e),n?l("mini-league",{openLeagueId:n}):l("mini-league")})}const se=E.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},f=>{const m=f.new;try{if(m.status==="finished"||m.forfeit){if(M||(M=!0,K&&(clearInterval(K),K=null),m.game_state&&(u=m.game_state),u.phase==="finished"&&!m.forfeit&&document.getElementById("pvp-home")))return;Z(m);return}if(m.game_state){const x=u;u=m.game_state;const b=u._lastGC;if(b&&b.seq&&!D.has(b.seq)&&(D.add(b.seq),b.by!==o)){Ae(b.type,b.by,()=>ye());return}const h=x[o+"Score"]||0,$=x[g+"Score"]||0,A=u[o+"Score"]||0,_=u[g+"Score"]||0,z=A>h,S=_>$;if((z||S)&&!j.has(u.round)){j.add(u.round);const q=[...u.log||[]].reverse().find(O=>O.isGoal),B=((q==null?void 0:q.homePlayers)||[]).map(O=>({name:O.name,note:O.note,portrait:O.portrait,job:O.job}));$e(B,A,_,z,()=>ye());return}ye()}}catch(x){console.error("[PvP] crash:",x)}}).subscribe();async function re(f){Object.assign(u,f),u.lastActionAt=new Date().toISOString();const{error:m}=await E.from("matches").update({game_state:u}).eq("id",i);m&&p("Erreur de synchronisation","error");try{ye()}catch(x){console.error("[PvP] renderPvpScreen crash:",x)}}async function P(){if(M)return;M=!0,K&&(clearInterval(K),K=null);const f=s?T.away_id:T.home_id,m=s?"p2":"p1",x=s?"p1":"p2",b={...u,[m+"Score"]:3,[x+"Score"]:0,phase:"finished"};try{await E.from("matches").update({status:"finished",forfeit:!0,winner_id:f,home_score:b.p1Score||0,away_score:b.p2Score||0,game_state:b}).eq("id",i)}catch{}try{E.removeChannel(se)}catch{}setTimeout(()=>{We(e),l("mini-league")},800)}const Y={BOOST_STAT:({value:f=1,count:m=1,roles:x=[]},b,h)=>{const $=b[o+"Team"],A=Object.entries($).filter(([_])=>!x.length||x.includes(_)).flatMap(([_,z])=>z.filter(S=>!S.used).map(S=>({...S,_line:_})));if(!A.length){b.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),h(b);return}U(A,m,`Choisir ${m} joueur(s) à booster (+${f})`,_=>{_.forEach(z=>{const S=($[z._line]||[]).find(q=>q.cardId===z.cardId);S&&(S.boost=(S.boost||0)+f,b.log.push({text:`⚡ +${f} sur ${S.name}`,type:"info"}))}),b[o+"Team"]=$,h(b)})},DEBUFF_STAT:({value:f=1,count:m=1,roles:x=[],target:b="ai"},h,$)=>{const A=b==="home"?o:g,_=h[A+"Team"],z=b==="home"?"allié":"adverse",S=Object.entries(_).filter(([q])=>!x.length||x.includes(q)).flatMap(([q,B])=>B.map(O=>({...O,_line:q})));if(!S.length){h.log.push({text:`🎯 Aucun joueur ${z}`,type:"info"}),$(h);return}U(S,m,`Choisir ${m} joueur(s) ${z}(s) (-${f})`,q=>{q.forEach(B=>{const O=(_[B._line]||[]).find(H=>H.cardId===B.cardId);O&&(O.boost=(O.boost||0)-f,h.log.push({text:`🎯 -${f} sur ${O.name}`,type:"info"}))}),h[A+"Team"]=_,$(h)})},GRAY_PLAYER:({count:f=1,roles:m=[],target:x="ai"},b,h)=>{const $=x==="home"?o:g,A=b[$+"Team"],_=x==="home"?"allié":"adverse",z=Object.entries(A).filter(([S])=>!m.length||m.includes(S)).flatMap(([S,q])=>q.filter(B=>!B.used).map(B=>({...B,_line:S})));if(!z.length){b.log.push({text:`❌ Aucun joueur ${_}`,type:"info"}),h(b);return}U(z,f,`Choisir ${f} joueur(s) ${_}(s) à exclure`,S=>{const q="usedCardIds_"+$,B=new Set(b[q]||[]);S.forEach(O=>{const H=(A[O._line]||[]).find(W=>W.cardId===O.cardId);H&&(H.used=!0,B.add(O.cardId),b.log.push({text:`❌ ${H.name} exclu !`,type:"info"}))}),b[q]=[...B],b[$+"Team"]=A,h(b)})},REVIVE_PLAYER:({count:f=1,roles:m=[]},x,b)=>{const h=x[o+"Team"],$=Object.entries(h).filter(([A])=>!m.length||m.includes(A)).flatMap(([A,_])=>_.filter(z=>z.used).map(z=>({...z,_line:A})));if(!$.length){x.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),b(x);return}U($,f,`Choisir ${f} joueur(s) à ressusciter`,A=>{A.forEach(_=>{const z=(h[_._line]||[]).find(S=>S.cardId===_.cardId);z&&(z.used=!1,x.log.push({text:`💫 ${z.name} ressuscité !`,type:"info"}))}),x[o+"Team"]=h,b(x)})},REMOVE_GOAL:({},f,m)=>{const x=g+"Score";f[x]>0?(f[x]--,f.log.push({text:"🚫 Dernier but annulé !",type:"info"})):f.log.push({text:"🚫 Aucun but à annuler",type:"info"}),m(f)},ADD_GOAL_DRAW:({},f,m)=>{f[o+"Score"]===f[g+"Score"]?(f[o+"Score"]++,f.log.push({text:"🎯 But bonus !",type:"info"})):f.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),m(f)},ADD_SUB:({value:f=1},m,x)=>{m.maxSubs=(m.maxSubs||3)+f,m.log.push({text:`🔄 +${f} remplacement(s)`,type:"info"}),x(m)},CUSTOM:({},f,m)=>m(f)};function U(f,m,x,b){const h=document.createElement("div");h.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let $=[];function A(){var z,S;const _=f.map(q=>{const B={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[q._line]||"#555",O=ze(q,q._line)+(q.boost||0),W=$.find(te=>te.cardId===q.cardId)?"#FFD700":"rgba(255,255,255,0.25)",V=q.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${q.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${W};background:${B};overflow:hidden;cursor:pointer;${V}">
        ...
        </div>`,(z=h.querySelector("#pp-close"))==null||z.addEventListener("click",()=>h.remove()),h.querySelectorAll(".pp-item").forEach(q=>{q.addEventListener("click",()=>{const B=q.dataset.cid,O=f.find(W=>W.cardId===B),H=$.findIndex(W=>W.cardId===B);O&&(H>-1?$.splice(H,1):$.length<m&&$.push(O),A())})}),(S=h.querySelector("#pp-confirm"))==null||S.addEventListener("click",()=>{h.remove(),b($)})}A(),document.body.appendChild(h)}async function G(f,m){const b=(u["gcCardsFull_"+o]||[]).find(_=>_.id===f),h=(b==null?void 0:b._gcDef)||(u.gcDefs||[]).find(_=>{var z;return _.name===m||((z=_.name)==null?void 0:z.toLowerCase().trim())===(m==null?void 0:m.toLowerCase().trim())}),$=[...u["usedGc_"+o]||[],f],A={type:m,by:o,seq:(u._gcAnimSeq||0)+1};D.add(A.seq),Ae(m,o,async()=>{if(h!=null&&h.effect_type&&h.effect_type!=="CUSTOM"){const z=Y[h.effect_type];if(z){const S={...u};z(h.effect_params||{},S,async q=>{q["usedGc_"+o]=$,q._lastGC=A,q._gcAnimSeq=A.seq,await re(q)});return}}const _={...u};switch(m){case"Remplacement+":_.maxSubs=(_.maxSubs||3)+1,_.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const z=g+"Score";_[z]>0&&(_[z]--,_.log.push({text:"🚫 But annulé",type:"info"}));break}}_["usedGc_"+o]=$,_._lastGC=A,_._gcAnimSeq=A.seq,await re(_)})}function ne(f,m){const x="usedCardIds_"+f,b=new Set(u[x]||[]);m.forEach(h=>b.add(h)),u[x]=[...b]}function pe(){for(const f of["p1","p2"]){const m=new Set(u["usedCardIds_"+f]||[]),x=u[f+"Team"];if(x)for(const b of["GK","DEF","MIL","ATT"])(x[b]||[]).forEach(h=>{h.used=m.has(h.cardId)})}}function ye(){var ot,gt,de,_e,ee,ce;if(u.phase==="reveal")return Te();if(u.phase==="midfield")return he();if(u.phase==="finished")return w();const f=u[o+"Team"],m=u[g+"Team"];pe();const x=u[o+"Score"],b=u[g+"Score"],h=u[o+"Name"],$=u[g+"Name"],A=u.phase===o+"-attack",_=u.phase===o+"-defense",z=Array.isArray(u["selected_"+o])?u["selected_"+o]:[],S=z.map(J=>J.cardId),q=window.innerWidth>=700,B=u[o+"Subs"]||[],O=u["usedSubIds_"+o]||[],H=B.filter(J=>!O.includes(J.cardId)),W=u["gcCardsFull_"+o]||[],V=u["usedGc_"+o]||[],te=W.filter(J=>!V.includes(J.id)),oe=u.boostOwner===o&&!u.boostUsed,fe=!["GK","DEF","MIL","ATT"].some(J=>(m[J]||[]).some(le=>!le.used)),xe=[...f.MIL||[],...f.ATT||[]].filter(J=>!J.used),ve=A&&fe&&xe.length===0?[...f.GK||[],...f.DEF||[]].filter(J=>!J.used).map(J=>J.cardId):[];function we(J,le,Me){var zi,Si;const ke=J._gcDef,tt={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ke==null?void 0:ke.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",it={purple:"#b06ce0",light_blue:"#00d4ef"}[ke==null?void 0:ke.color]||"#b06ce0",rt=(ke==null?void 0:ke.name)||J.gc_type,st=(ke==null?void 0:ke.effect)||((zi=Ke[J.gc_type])==null?void 0:zi.desc)||"",ct=ke!=null&&ke.image_url?`/manager-wars/icons/${ke.image_url}`:null,$t=((Si=Ke[J.gc_type])==null?void 0:Si.icon)||"⚡",pt=Math.round(Me*.22),ht=Math.round(Me*.22),kt=Me-pt-ht,Ht=rt.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${J.id}" data-gc-type="${J.gc_type}"
      ...
      </div>`}const Le=(J,le)=>le?Ie(130,175):we(J,130,175),Re=(J,le)=>le?Ie(68,95):we(J,68,95),je=q?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",Oe=A?qe(o)?`<button id="pvp-action" style="${je};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${z.length===0?"disabled":""}>⚔️ ATTAQUEZ <span id="pvp-timer"></span></button>`:`<button id="pvp-action" data-pass="1" style="${je};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER <span id="pvp-timer"></span></button>`:_?`<button id="pvp-action" style="${je};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${z.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="pvp-timer"></span></button>`:`<div style="font-size:11px;color:rgba(255,255,255,0.3);text-align:center;padding:4px">⏳ Tour de ${$}</div>`,Fe=A&&!qe(o)?'<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">Aucun attaquant — passez la main</div>':A||_?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${z.length}/3 sélectionné(s)</div>`:"",Ve=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${q?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
      ...
      </div>`;function Qe(){const J=e.querySelector(".match-screen");if(!J)return;const le=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);J.style.height=le+"px",J.style.minHeight=le+"px",J.style.maxHeight=le+"px",J.style.overflow="hidden";const Me=e.querySelector("#mobile-action-bar"),ke=e.querySelector("#mobile-play-area");Me&&ke&&(ke.style.paddingBottom=Me.offsetHeight+"px")}if(Qe(),setTimeout(Qe,120),setTimeout(Qe,400),C||(C=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",Qe),window.visualViewport.addEventListener("scroll",Qe)),window.addEventListener("resize",Qe)),function(){const le=e.querySelector(".terrain-wrapper svg");le&&(le.removeAttribute("width"),le.removeAttribute("height"),le.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",le.setAttribute("viewBox","-26 -26 352 352"),le.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),e.querySelectorAll(".match-slot-hit").forEach(J=>{J.addEventListener("click",()=>{if(!A&&!_)return;const le=J.dataset.cardId,Me=J.dataset.role,ke=(f[Me]||[]).find(st=>st.cardId===le);if(!ke||ke.used)return;const tt=ve.includes(le);if(A&&!["MIL","ATT"].includes(Me)&&!tt)return;Array.isArray(u["selected_"+o])||(u["selected_"+o]=[]);const it=u["selected_"+o],rt=it.findIndex(st=>st.cardId===le);rt>-1?it.splice(rt,1):it.length<3&&it.push({...ke,_role:Me}),ye()})}),e.querySelectorAll(".match-used-hit").forEach(J=>{J.addEventListener("click",()=>R(J.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(J=>{J.addEventListener("click",()=>R())}),(ot=e.querySelector("#pvp-sub-open"))==null||ot.addEventListener("click",()=>R()),e.querySelectorAll(".pvp-gc-mini").forEach(J=>{J.addEventListener("click",()=>ie(J.dataset.gcId,J.dataset.gcType))}),(gt=e.querySelector("#pvp-boost-card"))==null||gt.addEventListener("click",()=>ae()),(de=e.querySelector("#pvp-action"))==null||de.addEventListener("click",J=>{A?J.currentTarget.dataset.pass==="1"||!qe(o)?X():be():_&&ge()}),(_e=e.querySelector("#pvp-quit"))==null||_e.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&P()}),(ee=e.querySelector("#pvp-view-opp"))==null||ee.addEventListener("click",()=>ue()),(ce=e.querySelector("#pvp-toggle-history"))==null||ce.addEventListener("click",()=>me()),K&&(clearInterval(K),K=null),(A||_)&&!M){let J=30,le=!1;const Me=()=>document.getElementById("pvp-timer"),ke=()=>{Me()&&(Me().textContent=J+"s",Me().style.color=le?"#ff4444":"#fff")};ke(),K=setInterval(()=>{J--,J<0?le?(clearInterval(K),K=null,A&&!qe(o)?X():P()):(le=!0,J=15,ke()):ke()},1e3)}}function Te(){et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
      ...
      <div style="width:min(90vw,420px)">${ut(u[g+"Team"],u[g+"Formation"],null,[],300,300)}</div>
    </div>`,o==="p1"&&setTimeout(async()=>{await re({phase:"midfield"})},5e3)}let Ee=!1;function he(){if(Ee)return;const f=u[o+"Team"].MIL||[],m=u[g+"Team"].MIL||[];function x(te){return te.reduce((oe,fe)=>oe+ze(fe,"MIL"),0)}function b(te){let oe=0;for(let fe=0;fe<te.length-1;fe++){const xe=at(te[fe],te[fe+1]);xe==="#00ff88"?oe+=2:xe==="#FFD700"&&(oe+=1)}return oe}const h=x(f)+b(f),$=x(m)+b(m),A=h>=$;function _(te,oe,fe){return`<div id="duel-row-${fe}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
        ...
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${te.map((xe,ve)=>{const we=ve<te.length-1?at(xe,te[ve+1]):null,Ie=!we||we==="#ff3333"||we==="#cc2222",Le=we==="#00ff88"?"+2":we==="#FFD700"?"+1":"";return`<div class="duel-card duel-card-${fe}" data-idx="${ve}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease,transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
              ${Ye({...xe,note:ze(xe,"MIL"),_line:"MIL"},58,78)}
            </div>
            ${ve<te.length-1?`<div class="duel-link duel-link-${fe}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${Ie?"rgba(255,255,255,0.12)":we};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${Ie?"none":`0 0 8px ${we}`}">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div id="duel-shock" style="position:absolute;left:50%;top:50%;width:120px;height:120px;border-radius:50%;border:6px solid #FFD700;opacity:0;pointer-events:none"></div>
      <div id="pvp-duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center,rgba(10,61,30,.4),rgba(10,61,30,.92))"></div>
    </div>`;const z=(te,oe)=>e.querySelectorAll(te).forEach((fe,xe)=>{setTimeout(()=>{fe.style.opacity="1",fe.style.transform="translateY(0) scale(1)"},oe+xe*90)});z(".duel-card-me",150),z(".duel-card-opp",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((te,oe)=>setTimeout(()=>{te.style.opacity="1"},oe*70)),900),setTimeout(()=>{const te=e.querySelector("#pvp-vs");te&&(te.style.opacity="1",te.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(oe=>oe.style.opacity="1")},1250);function S(te,oe,fe){const xe=document.getElementById(te);if(!xe)return;const ve=performance.now(),we=Ie=>{const Le=Math.min(1,(Ie-ve)/fe);xe.textContent=Math.round(oe*(1-Math.pow(1-Le,3))),Le<1?requestAnimationFrame(we):xe.textContent=oe};requestAnimationFrame(we)}setTimeout(()=>{S("pvp-score-me",h,800),S("pvp-score-opp",$,800)},1500);const q=u.p1Team.MIL||[],B=u.p2Team.MIL||[],O=x(q)+b(q),H=x(B)+b(B),W=O>=H?"p1":"p2";let V=u.boostValue;V==null&&(V=ti(),u.boostValue=V,u.boostOwner=W,u.boostUsed=!1),Ee=!0,setTimeout(()=>{const te=e.querySelector("#duel-row-"+(A?"me":"opp")),oe=e.querySelector("#duel-row-"+(A?"opp":"me")),fe=document.getElementById("pvp-score-me"),xe=document.getElementById("pvp-score-opp"),ve=A?fe:xe,we=A?xe:fe;ve&&(ve.style.fontSize="80px",ve.style.color=A?"#FFD700":"#ff6b6b",ve.style.animation="duelPulse .5s ease"+(A?",duelGlow 1.5s ease infinite .5s":"")),we&&(we.style.opacity="0.25"),setTimeout(()=>{te&&(te.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",te.style.zIndex="5"),setTimeout(()=>{const Ie=document.getElementById("duel-shock");Ie&&(Ie.style.animation="shockwave .5s ease-out forwards"),oe&&(oe.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var je;const Ie=document.getElementById("pvp-duel-finale");if(!Ie)return;const Le=u.boostOwner===o?'<div style="background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:3px solid #cdeffd;border-radius:18px;padding:20px 34px;text-align:center;animation:boostFlipIn .7s cubic-bezier(.34,1.56,.64,1) both;box-shadow:0 10px 36px rgba(135,206,235,.5)"><div style="font-size:10px;color:rgba(0,0,0,.6);letter-spacing:2px;text-transform:uppercase;margin-bottom:6px;font-weight:700">Carte Boost obtenue</div><div style="font-size:46px;line-height:1">⚡</div><div style="font-size:50px;font-weight:900;color:#063;line-height:1.1">+'+V+`</div><div style="font-size:10px;color:rgba(0,0,0,.55);margin-top:4px">Applicable sur n'importe quel joueur</div></div>`:"",Re=o==="p1"?'<button id="pvp-start-match" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">▶ Commencer le match</button>':`<div style="font-size:14px;color:rgba(255,255,255,0.5);text-align:center;margin-top:8px;animation:fadeUp .4s ease both">⏳ En attente de l'adversaire...</div>`;Ie.innerHTML='<div style="font-size:22px;font-weight:900;color:#fff;text-align:center;animation:fadeUp .4s ease both;text-shadow:0 2px 12px rgba(0,0,0,.5)">'+(A?"⚽ "+u[o+"Name"]+"<br>gagne le milieu et attaque !":"😔 "+u[g+"Name"]+"<br>gagne l'engagement et attaque !")+"</div>"+Le+Re,Ie.style.transition="opacity .45s ease",Ie.style.opacity="1",Ie.style.pointerEvents="auto",(je=document.getElementById("pvp-start-match"))==null||je.addEventListener("click",async()=>{const Oe=W;await re({phase:Oe+"-attack",attacker:Oe,round:1,boostValue:V,boostUsed:!1,boostOwner:Oe})})},600)},700)},2800)}function $e(f,m,x,b,h){const $=document.createElement("div");$.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:900;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;overflow:hidden;cursor:pointer";const A=Array.from({length:10},(q,B)=>`<div style="position:absolute;font-size:${16+Math.floor(Math.random()*24)}px;top:${5+Math.floor(Math.random()*65)}%;left:${3+Math.floor(Math.random()*94)}%;animation:fw${B%2===0?"A":"B"} ${.7+Math.random()*.7}s ease ${Math.random()*.9}s both;opacity:0">${["✨","🌟","⭐","💥","🎇","🎆","🔥","🌈"][B%8]}</div>`).join(""),_={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};$.innerHTML=`
      <style>
        @keyframes butPop{0%{transform:scale(0) rotate(-8deg);opacity:0}55%{transform:scale(1.25) rotate(2deg)}85%{transform:scale(0.92) rotate(-1deg)}100%{transform:scale(1);opacity:1}}
        @keyframes ballIn{0%{transform:translate(-70px,18px);opacity:0}65%{opacity:1}100%{transform:translate(26px,-8px);opacity:1}}
        @keyframes scoreIn{from{opacity:0;transform:translateY(-12px)}to{opacity:1;transform:translateY(0)}}
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild($);let z=!1;const S=()=>{z||(z=!0,$.remove(),setTimeout(()=>h(),50))};$.addEventListener("click",S),setTimeout(S,3500)}function Ae(f,m,x){var V,te;const b=(u.gcDefs||[]).find(oe=>{var fe;return oe.name===f||((fe=oe.name)==null?void 0:fe.toLowerCase().trim())===(f==null?void 0:f.toLowerCase().trim())}),h={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[b==null?void 0:b.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",$={purple:"#b06ce0",light_blue:"#00d4ef"}[b==null?void 0:b.color]||"#b06ce0",A=(b==null?void 0:b.name)||f,_=(b==null?void 0:b.effect)||((V=Ke[f])==null?void 0:V.desc)||"",z=b!=null&&b.image_url?`/manager-wars/icons/${b.image_url}`:null,S=((te=Ke[f])==null?void 0:te.icon)||"⚡",B=m===o?"Vous":u[m+"Name"]||"Adversaire",O=document.createElement("div");O.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",O.innerHTML=`
      <style>
        @keyframes gcFlipIn{0%{transform:perspective(800px) rotateY(90deg) scale(.7);opacity:0}55%{transform:perspective(800px) rotateY(-12deg) scale(1.08);opacity:1}100%{transform:perspective(800px) rotateY(0) scale(1);opacity:1}}
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(O);let H=!1;const W=()=>{H||(H=!0,O.remove(),setTimeout(()=>x&&x(),50))};O.addEventListener("click",W),setTimeout(W,3e3)}function ie(f,m){var O,H,W,V;const b=(u["gcCardsFull_"+o]||[]).find(te=>te.id===f),h=b==null?void 0:b._gcDef,$={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[h==null?void 0:h.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",A={purple:"#b06ce0",light_blue:"#00d4ef"}[h==null?void 0:h.color]||"#b06ce0",_=(h==null?void 0:h.name)||m,z=(h==null?void 0:h.effect)||((O=Ke[m])==null?void 0:O.desc)||"Carte spéciale.",S=h!=null&&h.image_url?`/manager-wars/icons/${h.image_url}`:null,q=((H=Ke[m])==null?void 0:H.icon)||"⚡",B=document.createElement("div");B.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",B.innerHTML=`
      ...
      </div>`,document.body.appendChild(x),(b=x.querySelector("#bp-close"))==null||b.addEventListener("click",()=>x.remove()),x.querySelectorAll(".bp-item").forEach(h=>{h.addEventListener("click",async()=>{const $=h.dataset.cid,A=m.find(z=>z.cardId===$);if(!A)return;const _=(f[A._line]||[]).find(z=>z.cardId===$);_&&(_.boost=(_.boost||0)+u.boostValue),x.remove(),await re({[o+"Team"]:f,boostUsed:!0})})})}function R(f=null){var H,W;if(!(u.phase===o+"-attack")){p("Remplacement uniquement avant votre attaque","warning");return}const x=u[o+"Team"],b=u["usedSubIds_"+o]||[],h=u.maxSubs||3;if(b.length>=h){p(`Maximum ${h} remplacements atteint`,"warning");return}const $=Object.entries(x).flatMap(([V,te])=>(te||[]).filter(oe=>oe.used).map(oe=>({...oe,_line:V}))),A=(u[o+"Subs"]||[]).filter(V=>!b.includes(V.cardId));if(!$.length){p("Aucun joueur utilisé à remplacer","warning");return}if(!A.length){p("Aucun remplaçant disponible","warning");return}let _=Math.max(0,$.findIndex(V=>V.cardId===f));const z=((H=$[_])==null?void 0:H._line)||((W=$[_])==null?void 0:W.job);let S=Math.max(0,A.findIndex(V=>V.job===z)),q=!1;const B=document.createElement("div");B.id="pvp-sub-overlay",B.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function O(){var we,Ie,Le,Re,je,Oe;const V=$[_],te=A[S],oe=Math.min(130,Math.round((window.innerWidth-90)/2)),fe=Math.round(oe*1.35),xe=Fe=>`background:rgba(255,255,255,0.12);border:none;color:${Fe?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Fe?"default":"pointer"};flex-shrink:0`;B.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        <div id="pin-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
          <div style="font-size:9px;color:#00ff88;letter-spacing:2px;text-transform:uppercase;font-weight:700">Joueur qui entre</div>
          <button id="pin-up" style="${xe(S===0)}" ${S===0?"disabled":""}>▲</button>
          <div>${te?Ye({...te,used:!1,boost:0},oe,fe):"<div>—</div>"}</div>
          <button id="pin-down" style="${xe(S>=A.length-1)}" ${S>=A.length-1?"disabled":""}>▼</button>
      ...
      <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
        <button id="psub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
      </div>`,(we=B.querySelector("#psub-close"))==null||we.addEventListener("click",()=>B.remove()),(Ie=B.querySelector("#pout-up"))==null||Ie.addEventListener("click",()=>{_>0&&(_--,O())}),(Le=B.querySelector("#pout-down"))==null||Le.addEventListener("click",()=>{_<$.length-1&&(_++,O())}),(Re=B.querySelector("#pin-up"))==null||Re.addEventListener("click",()=>{S>0&&(S--,O())}),(je=B.querySelector("#pin-down"))==null||je.addEventListener("click",()=>{S<A.length-1&&(S++,O())});const ve=(Fe,Ve,Ne,He)=>{const Ue=B.querySelector("#"+Fe);if(!Ue)return;let Je=0;Ue.addEventListener("touchstart",Ze=>{Je=Ze.touches[0].clientY},{passive:!0}),Ue.addEventListener("touchend",Ze=>{const Qe=Ze.changedTouches[0].clientY-Je;if(Math.abs(Qe)<30)return;const ot=Ve();Qe<0&&ot<He-1?(Ne(ot+1),O()):Qe>0&&ot>0&&(Ne(ot-1),O())},{passive:!0})};ve("pin-panel",()=>S,Fe=>S=Fe,A.length),ve("pout-panel",()=>_,Fe=>_=Fe,$.length),(Oe=B.querySelector("#psub-confirm"))==null||Oe.addEventListener("click",async Fe=>{if(Fe.preventDefault(),Fe.stopPropagation(),q)return;q=!0;const Ve=$[_],Ne=A[S];if(!Ve||!Ne)return;const He=Ve._line,Ue=(x[He]||[]).findIndex(Qe=>Qe.cardId===Ve.cardId);if(Ue===-1){p("Erreur : joueur introuvable","error"),B.remove();return}const Je={...Ne,_line:He,position:Ve.position,used:!1,boost:0};x[He].splice(Ue,1,Je);const Ze=[...b,Ne.cardId];B.remove(),Q(Ve,Ne,async()=>{await re({[o+"Team"]:x,[g+"Team"]:u[g+"Team"],["usedSubIds_"+o]:Ze})})})}document.body.appendChild(B),O()}function Q(f,m,x){const b={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},h=document.createElement("div");h.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const $=(z,S,q)=>`<div style="text-align:center">
      ...
      <div style="width:min(90vw,420px)">${ut(u[g+"Team"],u[g+"Formation"],null,[],300,300)}</div>
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(f),(m=f.querySelector("#pvp-opp-close"))==null||m.addEventListener("click",()=>f.remove())}function me(){var b;const f=u.log||[],m=document.createElement("div");m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column";const x=h=>{var A,_,z;if(h.type==="duel"){const S=h.isGoal,q=h.homeScored&&o==="p1"||!h.homeScored&&S&&o==="p2",B=h.homeScored?"#FFD700":S?"#ff6b6b":"rgba(255,255,255,0.3)",O=S?q?"⚽ BUT !":"⚽ BUT adversaire !":(A=h.homePlayers)!=null&&A.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${S?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${B};margin-bottom:4px">
      ...
      </div>`,document.body.appendChild(m),(b=m.querySelector("#pvp-hist-close"))==null||b.addEventListener("click",()=>m.remove())}async function X(){if(u.phase!==o+"-attack")return;const f=o==="p1"?"p2":"p1",m=(u.round||0)+1,x=[...u.log||[]];x.push({type:"info",text:`⏭️ ${u[o+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const b=De(u),h=qe(f),$=b||!h?"finished":f+"-attack";await re({["selected_"+o]:[],modifiers:{...u.modifiers,[o]:{}},pendingAttack:null,phase:$,attacker:f,round:m,log:x}),$==="finished"&&await F(u)}async function be(){const f=u[o+"Team"],m=!["GK","DEF","MIL","ATT"].some($=>(u[g+"Team"][$]||[]).some(A=>!A.used)),x=(u["selected_"+o]||[]).map($=>{const A=(f[$._role]||[]).find(O=>O.cardId===$.cardId)||$,_=m&&["GK","DEF"].includes($._role),z=f[$._role]||[],S=z.findIndex(O=>O.cardId===$.cardId),q=lt(z.length),B=S>=0?q[S]:A._col??1;return{...A,_line:$._role,_col:B,..._?{note_a:Math.max(1,Number(A.note_a)||0)}:{}}});if(!x.length)return;const b=Nt(x,u.modifiers[o]||{});ne(o,x.map($=>$.cardId)),x.forEach($=>{const A=(f[$._role]||[]).find(_=>_.cardId===$.cardId);A&&(A.used=!0)}),u["selected_"+o]=[],ye();const h=[...u.log||[]];if(m){const $=(u[o+"Score"]||0)+1,A=x.map(B=>({name:B.name,note:ze(B,B._line||"ATT"),portrait:Pe(B),job:B.job}));h.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:A,homeTotal:b.total,aiTotal:0});const _=(u.round||0)+1,z=o==="p1"?"p2":"p1",S={...u,[o+"Team"]:f,[o+"Score"]:$},q=De(S);j.add(_),$e(A,$,u[g+"Score"]||0,!0,async()=>{await re({[o+"Team"]:f,[o+"Score"]:$,["selected_"+o]:[],modifiers:{...u.modifiers,[o]:{}},pendingAttack:null,phase:q?"finished":z+"-attack",attacker:z,round:_,log:h}),q&&await F({...u,[o+"Score"]:$})});return}h.push({type:"pending",text:`⚔️ ${u[o+"Name"]} attaque (${b.total})`}),await re({[o+"Team"]:f,[g+"Team"]:u[g+"Team"],pendingAttack:{...b,players:x,side:o},["selected_"+o]:[],modifiers:{...u.modifiers,[o]:{}},phase:g+"-defense",log:h})}async function ge(){const f=u[o+"Team"],m=(u["selected_"+o]||[]).map(V=>{const te=(f[V._role]||[]).find(we=>we.cardId===V.cardId)||V,oe=f[V._role]||[],fe=oe.findIndex(we=>we.cardId===V.cardId),xe=lt(oe.length),ve=fe>=0?xe[fe]:te._col??1;return{...te,_line:V._role,_col:ve}}),x=Rt(m,u.modifiers[o]||{});ne(o,m.map(V=>V.cardId)),m.forEach(V=>{const te=(f[V._role]||[]).find(oe=>oe.cardId===V.cardId);te&&(te.used=!0)}),u["selected_"+o]=[],ye();const b=Ot(u.pendingAttack.total,x.total,u.modifiers[o]||{}),h=u.pendingAttack.side,$=b==="attack"||(b==null?void 0:b.goal),A=h==="p1"?"p2":"p1",_=(u.round||0)+1,z=(u.pendingAttack.players||[]).map(V=>({name:V.name,note:ze(V,V._line||"ATT"),portrait:Pe(V),job:V.job})),S=[...u.log||[]];S.push({type:"duel",isGoal:$,homeScored:$&&h===o,text:$?`⚽ BUT de ${u[h+"Name"]} ! (${u.pendingAttack.total} vs ${x.total})`:`✋ Attaque stoppée (${u.pendingAttack.total} vs ${x.total})`,homePlayers:z,aiPlayers:m.map(V=>({name:V.name,note:ze(V,V._line||"DEF"),portrait:Pe(V),job:V.job})),homeTotal:u.pendingAttack.total,aiTotal:x.total});const q=$?(u[h+"Score"]||0)+1:u[h+"Score"]||0,B={...u,[o+"Team"]:f,[h+"Score"]:q},O=De(B),H=O?"finished":A+"-attack",W=async()=>{await re({[o+"Team"]:f,[g+"Team"]:u[g+"Team"],[h+"Score"]:q,["selected_"+o]:[],modifiers:{...u.modifiers,[o]:{}},pendingAttack:null,phase:H,attacker:A,round:_,log:S}),(H==="finished"||O)&&await F({...u,[h+"Score"]:q})};if($){const V=h===o,te=V?q:u[o+"Score"]||0,oe=V?u[g+"Score"]||0:q;j.add(_),$e(z,te,oe,V,W)}else await W()}function Se(f){return["MIL","ATT"].some(m=>(f[m]||[]).some(x=>!x.used))}function Ce(f){return["GK","DEF","MIL","ATT"].some(m=>(f[m]||[]).some(x=>!x.used))}function Be(f){return Ce(f)&&!Se(f)}function qe(f){const m=u[f+"Team"],x=u[(f==="p1"?"p2":"p1")+"Team"];return!!(Se(m)||!Ce(x)&&Be(m))}function De(f){const m=["MIL","ATT"].some(S=>(f.p1Team[S]||[]).some(q=>!q.used)),x=["MIL","ATT"].some(S=>(f.p2Team[S]||[]).some(q=>!q.used)),b=Ce(f.p1Team),h=Ce(f.p2Team);return!m&&!(!h&&b)&&(!x&&!(!b&&h))}function I(f){const m=f.p1Score||0,x=f.p2Score||0;return m===x?null:m>x?T.home_id:T.away_id}async function F(f){try{const m=I(f),x=m?T.home_id===m?T.away_id:T.home_id:null,b=f.p1Score||0,h=f.p2Score||0;await E.from("matches").update({status:"finished",winner_id:m,home_score:b,away_score:h}).eq("id",i);const{data:$}=await E.from("mini_league_matches").select("id, league_id, matchday").eq("match_id",i).single();if($){await E.from("mini_league_matches").update({home_score:b,away_score:h,status:"finished"}).eq("id",$.id);const{data:A}=await E.from("mini_league_matches").select("id, status").eq("league_id",$.league_id).eq("matchday",$.matchday);if((A||[]).every(z=>z.status==="finished"||z.status==="bye")){const{data:z}=await E.from("mini_leagues").select("current_day,total_days").eq("id",$.league_id).single();if(z){const S=(z.current_day||0)+1,q=S>(z.total_days||0);await E.from("mini_leagues").update({current_day:q?z.total_days:S,status:q?"finished":"active"}).eq("id",$.league_id)}}}m&&x&&_i(m,x).catch(()=>{})}catch(m){console.error("[ML] finishMatch:",m)}}function w(){var h;if(M&&document.getElementById("pvp-end-overlay"))return;M=!0;const f=u[o+"Score"],m=u[g+"Score"],x=f>m,b=f===m;et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
    ...
    </div>`,(h=document.getElementById("pvp-home"))==null||h.addEventListener("click",()=>{try{E.removeChannel(se)}catch{}We(e),l("mini-league",n?{openLeagueId:n}:{})})}ye()}const lr="/manager-wars/",cr=[{emoji:"⚽",title:"Bienvenue dans Manager Wars !",color:"#1A6B3C",content:`<p>Tu es désormais un <strong>manager de football</strong> virtuel.</p>
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div id="mkt-content" style="padding:10px 16px;display:flex;flex-direction:column;gap:8px"></div>
    ...
    </div>`}function v(r){var K,C,j;const u=(K=r.card)==null?void 0:K.player;if(!u)return"";const M=Et(u,u.job),L=u.job2?Et(u,u.job2):0,N=r.status==="sold";return`<div class="card-panel" style="display:flex;align-items:center;gap:8px;padding:10px 12px;overflow:hidden;padding-left:6px;${N?"opacity:0.7":""}">
      ${Hi(u.rarity)}
      ${Yt(M,u.job)}
      ${Yt(L,u.job2||null)}
      ${Ui(u.country_code)}
      ...
      <div style="flex:1;min-width:0">
        ...
        <div style="font-size:10px;color:${N?"#22c55e":"#999"};margin-top:1px">
          ${N?`✅ Vendu à ${((j=r.buyer)==null?void 0:j.pseudo)||"—"} · ${r.sold_at?new Date(r.sold_at).toLocaleDateString("fr"):""}`:`🟢 En vente depuis le ${new Date(r.listed_at).toLocaleDateString("fr")}`}
        </div>
      </div>
      <div style="text-align:right;flex-shrink:0">
        <div style="font-size:14px;font-weight:900;color:#D4A017">${r.price.toLocaleString("fr")}</div>
