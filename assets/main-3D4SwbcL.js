import{s as k,F as Je,b as Te,c as Tt,l as fe,d as Wt}from"./formation-links-CDBKB_z4.js";function ft(e,{navigate:t,toast:i}){e.innerHTML=`
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
  ...
  </div>`}async function ii(e,t){const{state:i,navigate:l,toast:p,openModal:n,closeModal:a}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await k.from("cards").select(`id, card_type, current_note, gc_type, formation, is_for_sale, sale_price,
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),d=(o||[]).filter(_=>_.card_type==="player"&&_.player),s=(o||[]).filter(_=>_.card_type==="game_changer"),r=(o||[]).filter(_=>_.card_type==="formation"),{data:f}=await k.from("gc_definitions").select("name,gc_type,color,effect,image_url"),u={};(f||[]).forEach(_=>{u[_.name]=_});const g=Object.keys(Je),m=Object.keys(de),b={};d.forEach(_=>{const y=_.player.id;b[y]=(b[y]||0)+1}),new Set(Object.keys(b).map(_=>String(_)));const I=new Set(r.map(_=>_.formation)),w=new Set(s.map(_=>_.gc_type));let x="player",A="Tous",D="",v=!1;function E(){return[...d].sort((_,y)=>{const j=Le.indexOf(_.player.job),F=Le.indexOf(y.player.job);return j!==F?j-F:(_.player.surname_encoded||"").localeCompare(y.player.surname_encoded||"")})}function L(){return[...c||[]].sort((_,y)=>{const j=Le.indexOf(_.job),F=Le.indexOf(y.job);return j!==F?j-F:(_.surname_encoded||"").localeCompare(y.surname_encoded||"")})}function h(){return E().filter(_=>{const y=_.player,j=A==="Tous"||y.job===A,F=!D||`${y.firstname} ${y.surname_encoded}`.toLowerCase().includes(D);return j&&F})}function $(){return L().filter(_=>{const y=A==="Tous"||_.job===A,j=!D||`${_.firstname} ${_.surname_encoded}`.toLowerCase().includes(D);return y&&j})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",y=>{D=y.target.value.toLowerCase(),S()}),e.querySelectorAll(".filter-btn").forEach(y=>{y.addEventListener("click",()=>{A=y.dataset.job,T(),S()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{v=!v,T(),S()})):(_.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{v=!v,T(),S()})))}function S(){const _=document.getElementById("col-grid");_&&(x==="player"?M(_):x==="formation"?P(_):H(_))}function z(_,y,j,F,q){q=q||"#7a28b8";const O=document.getElementById("col-grid"),R=document.getElementById("col-big");if(!O||!R)return;var X=0;function Y(){R.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+y(_[X])+"</div>";var W=R.querySelector("[data-card-id],[data-form-id],[data-gc-id]");W&&W.addEventListener("click",function(){F(_[X])}),requestAnimationFrame(function(){var U=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!U||!R)){var ee=R.clientHeight-8,oe=R.clientWidth-24,re=U.offsetHeight,ae=U.offsetWidth;if(re>0&&ae>0&&ee>40){var se=Math.min(ee/re,oe/ae,1);U.style.transform="scale("+se.toFixed(3)+")",U.style.transformOrigin="top center"}}}),O.innerHTML=_.map(function(U,ee){return'<div class="col-mini-item" data-idx="'+ee+'" style="flex-shrink:0;cursor:pointer;border-radius:8px;border:1.5px solid '+(ee===X?q:"transparent")+';transition:border-color .15s;overflow:hidden">'+j(U,ee===X)+"</div>"}).join(""),O.querySelectorAll(".col-mini-item").forEach(function(U){U.addEventListener("click",function(){X=Number(U.dataset.idx),Y(),U.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}Y()}function B(_){var y=.54,j=Math.round(140*y),F=Math.round(310*y),q;if(!_||_._fake){var O=_?_.player:null;if(!O)return"";q=yt(O)}else q=Pe(_,"");return'<div style="width:'+j+"px;height:"+F+'px;overflow:hidden;position:relative;flex-shrink:0"><div style="transform:scale('+y+');transform-origin:top left;position:absolute;top:0;left:0;pointer-events:none">'+q+"</div></div>"}function C(_,y,j){y=y||100,j=j||140;var F=Te[_]||{},q={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},O=Math.max(3,Math.round(y*.06)),R=Object.entries(F).map(function(Y){var W=Y[0],U=Y[1],ee=W.replace(/\d+$/,""),oe=q[ee]||"#888",re=Math.round(U.x*y),ae=Math.round(U.y*j);return'<circle cx="'+re+'" cy="'+ae+'" r="'+O+'" fill="'+oe+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),X=Math.max(1,Math.round(y/50));return'<svg viewBox="0 0 '+y+" "+j+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+y+'" height="'+j+'" fill="#1A6B3C"/><rect x="'+Math.round(y*.2)+'" y="'+Math.round(j*.02)+'" width="'+Math.round(y*.6)+'" height="'+Math.round(j*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+X+'"/><line x1="0" y1="'+Math.round(j*.5)+'" x2="'+y+'" y2="'+Math.round(j*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+X+'"/><ellipse cx="'+Math.round(y*.5)+'" cy="'+Math.round(j*.5)+'" rx="'+Math.round(y*.18)+'" ry="'+Math.round(j*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+X+'"/><rect x="'+Math.round(y*.2)+'" y="'+Math.round(j*.82)+'" width="'+Math.round(y*.6)+'" height="'+Math.round(j*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+X+'"/>'+R+"</svg>"}function N(_,y,j){var F=j>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+j+"</div>":"",q=y?'data-form-id="'+y.id+'"':"",O=_.length>7?14:_.length>5?16:19,R=!!y;return"<div "+q+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(R?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(R?"":"filter:grayscale(1);opacity:0.5")+'">'+F+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(R?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+O+"px;font-weight:900;color:"+(R?"#1A6B3C":"#aaa")+';line-height:1">'+_+'</div></div><div style="flex:1;overflow:hidden;background:'+(R?"#1A6B3C":"#ccc")+'">'+C(_,140,220)+"</div></div>"}function G(_,y){var j=.54,F=Math.round(140*j),q=Math.round(305*j),O=Math.round(q*.22),R=q-O,X=_.length>7?5:7,Y=C(_,F,R),W=y?"1.5px solid #2a7a40":"1px solid #ddd",U=y?"":"filter:grayscale(1);opacity:0.45;",ee=y?"#1A6B3C":"#bbb",oe="#fff";return'<div style="width:'+F+"px;height:"+q+"px;border-radius:6px;border:"+W+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+U+'"><div style="height:'+O+"px;background:"+ee+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+X+"px;font-weight:900;color:"+oe+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(F-4)+'px">'+_+'</span></div><div style="height:'+R+'px;overflow:hidden;flex-shrink:0">'+Y+"</div></div>"}function M(_){if(v){const y=$();if(!y.length){_.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const j=y.map(F=>d.find(q=>q.player.id===F.id)||{_fake:!0,player:F,id:"fake-"+F.id});z(j,F=>F._fake?yt(F.player):Pe(F,""),F=>F._fake?B({player:F.player,id:"x",_fake:!0}):B(F),F=>{F._fake||ht(F,d,b,t)},"#1A6B3C")}else{const y=h();if(!y.length){_.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const j={},F=[];y.forEach(q=>{j[q.player.id]||(j[q.player.id]=!0,F.push(q))}),z(F,q=>{const O=b[q.player.id]||1,R=O>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${O}</div>`:"",Y=d.filter(W=>W.player.id===q.player.id&&W.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Pe(q,R+Y)},q=>B(q),q=>ht(q,d,b,t),"#1A6B3C")}}function P(_){const y=v?g:[...I];if(!y.length){_.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const j=y.map(F=>({formation:F,card:r.find(q=>q.formation===F)||null,owned:I.has(F)}));z(j,({formation:F,card:q,owned:O})=>N(F,O?q:null,O?r.filter(R=>R.formation===F).length:0),({formation:F,owned:q})=>G(F,q),({card:F,owned:q})=>{q&&F&&oi(F,r,t,n)},"#1A6B3C")}function H(_){const y=Object.keys(u),j=v?y.length?y:m:[...w];if(!j.length){_.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const F=j.map(q=>({type:q,gc:de[q]||{icon:"⚡",desc:""},def:u[q]||null,owned:w.has(q),card:s.find(O=>O.gc_type===q)||null}));z(F,({type:q,gc:O,def:R,owned:X,card:Y})=>{const W=X?s.filter(K=>K.gc_type===q).length:0,U=W>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${W}</div>`:"",ee=(R==null?void 0:R.gc_type)==="ultra_game_changer",oe={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},re={purple:"#b06ce0",light_blue:"#00d4ef"},ae=oe[R==null?void 0:R.color]||oe.purple,se=re[R==null?void 0:R.color]||re.purple,ce=(R==null?void 0:R.effect)||O.desc||"",we=R!=null&&R.image_url?`/manager-wars/icons/${R.image_url}`:null;return X&&Y?`<div data-gc-id="${Y.id}" data-gc-type="${q}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${se};background:${ae};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${se}66;cursor:pointer">
          ${U}
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const _e=1e3;function oi(e,t,i,l){var m,b,I;const{state:p,toast:n,closeModal:a,navigate:o,refreshProfile:c}=i,d=e.formation,s={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function r(){const w=Te[d]||{},x=Je[d]||[],A=290,D=360,v=20;function E(h){const $=w[h];return $?{x:$.x*A,y:$.y*D}:null}let L=`<svg width="${A}" height="${D}" viewBox="0 0 ${A} ${D}" xmlns="http://www.w3.org/2000/svg">`;for(const[h,$]of x){const T=E(h),S=E($);!T||!S||(L+=`<line x1="${T.x}" y1="${T.y}" x2="${S.x}" y2="${S.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const h of Object.keys(w)){const $=E(h);if(!$)continue;const T=h.replace(/\d+/,""),S=s[T]||"#555";L+=`<circle cx="${$.x}" cy="${$.y}" r="${v}" fill="${S}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,L+=`<text x="${$.x}" y="${$.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${T}</text>`}return L+="</svg>",L}const f=t.filter(w=>w.formation===d),u=f.length,g=!e.is_for_sale;l(`Formation ${d}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
  ...
  </div>`,ai(e,t,p,i),document.getElementById("builder-back").addEventListener("click",()=>l("decks")),document.getElementById("formation-select").addEventListener("change",d=>{t.formation=d.target.value;const s=bt(t.formation),r={};s.forEach(f=>{t.slots[f]&&(r[f]=t.slots[f])}),t.slots=r,he(e,t,i)}),document.getElementById("save-deck").addEventListener("click",()=>li(t,i)),e.querySelectorAll("[data-remove-sub]").forEach(d=>{d.addEventListener("click",()=>{t.subs=t.subs.filter(s=>s!==d.dataset.removeSub),he(e,t,i)})}),(c=document.getElementById("add-sub-btn"))==null||c.addEventListener("click",()=>{di(t,e,i)})}function ai(e,t,i,l){const p=e.querySelector("#deck-field");if(!p)return;const n=Te[t.formation]||{},a=Tt(t.formation)||[],o={};for(const I of i){const w=t.slots[I],x=w?t.playerCards.find(A=>A.id===w):null;o[I]=x?x.player:null}const c=300,d=300,s=48,r=64,f=13,u=16,g=38;function m(I){const w=n[I];return w?{x:w.x*c,y:w.y*d}:null}let b="";for(const[I,w]of a){const x=m(I),A=m(w);if(!x||!A)continue;const D=o[I]?{...o[I],club_id:o[I].club_id,country_code:o[I].country_code,rarity:o[I].rarity}:null,v=o[w]?{...o[w],club_id:o[w].club_id,country_code:o[w].country_code,rarity:o[w].rarity}:null,E=fe(D,v);E==="#ff3333"||E==="#cc2222"?b+=`<line x1="${x.x.toFixed(1)}" y1="${x.y.toFixed(1)}" x2="${A.x.toFixed(1)}" y2="${A.y.toFixed(1)}" stroke="${E}" stroke-width="2.5" stroke-linecap="round" opacity="0.4"/>`:(b+=`<line x1="${x.x.toFixed(1)}" y1="${x.y.toFixed(1)}" x2="${A.x.toFixed(1)}" y2="${A.y.toFixed(1)}" stroke="${E}" stroke-width="8" stroke-linecap="round" opacity="0.2"/>`,b+=`<line x1="${x.x.toFixed(1)}" y1="${x.y.toFixed(1)}" x2="${A.x.toFixed(1)}" y2="${A.y.toFixed(1)}" stroke="${E}" stroke-width="2.5" stroke-linecap="round" opacity="0.9"/>`)}for(const I of i){const w=m(I);if(!w)continue;const x=o[I],A=I.replace(/\d+/,""),D=pe[A]||"#555",v=(w.x-s/2).toFixed(1),E=(w.y-r/2).toFixed(1),L={normal:"#aaa",pépite:"#D4A017",papyte:"#222",légende:"#7a28b8"}[x==null?void 0:x.rarity]||"#aaa";if(x){const h=Re(x),$=jt(x),T=Ct(x.country_code),S=Number(A==="GK"?x.note_g:A==="DEF"?x.note_d:A==="MIL"?x.note_m:x.note_a)||0,z=r-f-u;b+=`<defs><clipPath id="dcp-${I}"><rect x="${v}" y="${(w.y-r/2+f).toFixed(1)}" width="${s}" height="${z}"/></clipPath></defs>`,b+=`<rect x="${v}" y="${E}" width="${s}" height="${r}" rx="5" fill="${D}"/>`,h&&(b+=`<image href="${h}" x="${v}" y="${(w.y-r/2+f).toFixed(1)}" width="${s}" height="${z}" clip-path="url(#dcp-${I})" preserveAspectRatio="xMidYMin slice"/>`),b+=`<rect x="${v}" y="${E}" width="${s}" height="${f}" fill="rgba(255,255,255,0.93)"/>`,b+=`<text x="${w.x.toFixed(1)}" y="${(w.y-r/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(x.surname_encoded||"").slice(0,9)}</text>`;const B=(w.y+r/2-u).toFixed(1);b+=`<rect x="${v}" y="${B}" width="${s}" height="${u}" fill="rgba(255,255,255,0.93)"/>`,T&&(b+=`<image href="${T}" x="${(w.x-21).toFixed(1)}" y="${(w.y+r/2-u+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`),b+=`<text x="${w.x.toFixed(1)}" y="${(w.y+r/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${S}</text>`,$&&(b+=`<image href="${$}" x="${(w.x+s/2-14).toFixed(1)}" y="${(w.y+r/2-u+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`),b+=`<rect x="${v}" y="${E}" width="${s}" height="${r}" rx="5" fill="none" stroke="${L}" stroke-width="2"/>`}else b+=`<rect x="${v}" y="${E}" width="${s}" height="${r}" rx="5" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="5,3"/>`,b+=`<text x="${w.x.toFixed(1)}" y="${w.y.toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="22" fill="rgba(255,255,255,0.35)">+</text>`,b+=`<text x="${w.x.toFixed(1)}" y="${(w.y+16).toFixed(1)}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.3)">${A}</text>`;b+=`<rect x="${v}" y="${E}" width="${s}" height="${r}" rx="5" fill="rgba(0,0,0,0.01)" class="deck-slot-hit" data-pos="${I}" style="cursor:pointer"/>`}p.innerHTML=`<svg viewBox="${-g} ${-g} ${c+g*2} ${d+g*2}" width="100%" style="display:block;max-width:440px;margin:0 auto">${b}</svg>`,p.querySelectorAll(".deck-slot-hit").forEach(I=>{I.addEventListener("click",()=>si(I.dataset.pos,t,e,l))})}function si(e,t,i,l){var f,u,g;const{openModal:p,closeModal:n}=l,a=e.replace(/\d+/,""),o=t.slots[e],c=o?t.playerCards.find(m=>m.id===o):null;(f=c==null?void 0:c.player)==null||f.id;const d=new Set;Object.entries(t.slots).forEach(([m,b])=>{var w;if(m===e||!b)return;const I=t.playerCards.find(x=>x.id===b);(w=I==null?void 0:I.player)!=null&&w.id&&d.add(I.player.id)}),t.subs.forEach(m=>{var I;const b=t.playerCards.find(w=>w.id===m);(I=b==null?void 0:b.player)!=null&&I.id&&d.add(b.player.id)});const s=new Set,r=t.playerCards.filter(m=>{const b=m.player;return!(b.job===a||b.job2===a)||d.has(b.id)||s.has(b.id)?!1:(s.add(b.id),!0)});r.sort((m,b)=>{const I=a==="GK"?m.player.note_g:a==="DEF"?m.player.note_d:a==="MIL"?m.player.note_m:m.player.note_a;return(a==="GK"?b.player.note_g:a==="DEF"?b.player.note_d:a==="MIL"?b.player.note_m:b.player.note_a)-I}),p(`Choisir ${a} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${r.length>0?r.map(m=>{var A,D;const b=m.player,I=a==="GK"?b.note_g:a==="DEF"?b.note_d:a==="MIL"?b.note_m:b.note_a,w=Re(b),x={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[b.rarity];return`<div class="player-option" data-card-id="${m.id}"
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}const zi="simulation",Ft="VOTRE_ZONE_ID";function Ci(){switch(zi){case"propellerads":return ji();case"adsense":return Si();default:return it()}}function it(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:#111;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:14px";let i=5;t.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const r=()=>document.getElementById("mm-status"),f=async(I=!0)=>{d=!0,s&&(k.removeChannel(s),s=null),I&&await k.rpc("cancel_matchmaking",{p_user_id:a.profile.id}).catch(()=>{})};(b=document.getElementById("mm-cancel"))==null||b.addEventListener("click",async()=>{try{await f(!0)}catch(I){console.warn("[Matchmaking] cleanup error:",I)}le(),o("home")});const u=async(I,w,x)=>{d||(d=!0,s&&(k.removeChannel(s),s=null),r()&&(r().textContent="Adversaire trouvé !"),await new Promise(A=>setTimeout(A,600)),e.isConnected!==!1&&Gi(e,t,I,x))},{data:g,error:m}=await k.rpc("try_matchmake",{p_user_id:a.profile.id,p_deck_id:i});if(m||!(g!=null&&g.success)){c("Erreur de matchmaking","error"),le(),o("home");return}if(g.matched){u(g.match_id,g.opponent_id,!1);return}r()&&(r().textContent="En attente d'un autre joueur..."),s=k.channel("matchmaking-"+a.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${a.profile.id}`},I=>{const w=I.new;w.status==="matched"&&w.match_id&&u(w.match_id,w.matched_with,!0)}).subscribe()}async function Gi(e,t,i,l){const{state:p,navigate:n,toast:a}=t,o=l?"p1":"p2",c=l?"p2":"p1";e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation du match...</div>';const{data:d}=await k.from("matches").select("*").eq("id",i).single();if(!d){a("Match introuvable","error"),n("home");return}async function s(){const[{data:h,error:$},{data:T,error:S},{data:z},{data:B}]=await Promise.all([k.rpc("get_deck_for_match",{p_deck_id:d.home_deck_id}),k.rpc("get_deck_for_match",{p_deck_id:d.away_deck_id}),k.from("users").select("id,pseudo,club_name").eq("id",d.home_id).single(),k.from("users").select("id,pseudo,club_name").eq("id",d.away_id).single()]);$&&console.error("[PvP] get_deck_for_match p1 error:",$),S&&console.error("[PvP] get_deck_for_match p2 error:",S);function C(y,j){return{cardId:y.card_id,position:j||y.position,id:y.id,firstname:y.firstname,name:y.surname_encoded,country_code:y.country_code,club_id:y.club_id,job:y.job,job2:y.job2,note_g:Number(y.note_g)||0,note_d:Number(y.note_d)||0,note_m:Number(y.note_m)||0,note_a:Number(y.note_a)||0,rarity:y.rarity,skin:y.skin,hair:y.hair,hair_length:y.hair_length,clubName:y.club_encoded_name||null,clubLogo:y.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}}const N=((h==null?void 0:h.starters)||[]).map(y=>C(y,y.position)),G=((T==null?void 0:T.starters)||[]).map(y=>C(y,y.position)),M=((h==null?void 0:h.subs)||[]).map(y=>C(y,y.position)),P=((T==null?void 0:T.subs)||[]).map(y=>C(y,y.position)),H=(h==null?void 0:h.formation)||"4-4-2",_=(T==null?void 0:T.formation)||"4-4-2";return console.log("[PvP] p1Formation:",H,"p1 starters:",N.length,"positions:",N.map(y=>y.position)),console.log("[PvP] p2Formation:",_,"p2 starters:",G.length,"positions:",G.map(y=>y.position)),{p1Team:Ge(N,H),p2Team:Ge(G,_),p1Subs:M,p2Subs:P,p1Formation:H,p2Formation:_,p1Name:(z==null?void 0:z.club_name)||(z==null?void 0:z.pseudo)||"Joueur 1",p2Name:(B==null?void 0:B.club_name)||(B==null?void 0:B.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"gc-select",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:[],gc_p2:[],gcReady_p1:!1,gcReady_p2:!1,usedGc_p1:[],usedGc_p2:[],lastActionAt:new Date().toISOString()}}let r=d.game_state&&Object.keys(d.game_state).length?d.game_state:null;if(!r)if(l){r=await s();const{data:h}=await k.from("matches").select("game_state").eq("id",i).single();!(h!=null&&h.game_state)||!Object.keys(h.game_state).length?await k.from("matches").update({game_state:r,turn_user_id:d.home_id}).eq("id",i):r=h.game_state}else{e.innerHTML=`<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation avec l'adversaire...</div>`;for(let h=0;h<30&&!r;h++){await new Promise(T=>setTimeout(T,400));const{data:$}=await k.from("matches").select("game_state").eq("id",i).single();$!=null&&$.game_state&&Object.keys($.game_state).length&&(r=$.game_state)}if(!r){a("Erreur de synchronisation","error"),n("home");return}}const f=k.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},h=>{const $=h.new;try{$.game_state&&(r=$.game_state,m()),($.status==="finished"||$.forfeit)&&m()}catch(T){console.error("[PvP] Realtime render crash:",T,"gameState:",r),a("Erreur de synchro temps réel : "+T.message,"error")}}).subscribe();async function u(h){Object.assign(r,h),r.lastActionAt=new Date().toISOString();const{error:$}=await k.from("matches").update({game_state:r}).eq("id",i);$&&(console.error("[PvP] pushState DB error:",$.message,$),a("Erreur de synchronisation : "+$.message,"error"));try{m()}catch(T){console.error("[PvP] renderPvpScreen crash après pushState:",T),a("Erreur d'affichage : "+T.message,"error")}}async function g(){const h=l?d.away_id:d.home_id;await k.from("matches").update({status:"finished",forfeit:!0,winner_id:h}).eq("id",i),k.removeChannel(f),le(),n("home")}function m(){var ae,se,ce,we;const h=r[o+"Team"];r[c+"Team"];const $=r[o+"Score"],T=r[c+"Score"],S=r[o+"Name"],z=r[c+"Name"];if(r.phase==="gc-select")return b();if(r.phase==="reveal")return I();if(r.phase==="midfield")return w();if(r.phase==="finished")return v();const B=r.phase===o+"-attack",C=r.phase===o+"-defense",N=r.phase===c+"-attack"||r.phase===c+"-defense",G=Array.isArray(r["selected_"+o])?r["selected_"+o]:[],M=G.map(K=>K.cardId);e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column";const P=window.innerWidth>=700,H=r[o+"Subs"]||[],_=r["usedSubIds_"+o]||[],y=H.filter(K=>!_.includes(K.cardId)),j=r["gcCardsFull_"+o]||[],F=r["usedGc_"+o]||[],q=j.filter(K=>!F.includes(K.id)),O=r.boostOwner===o&&!r.boostUsed,R=(K,Z,ie)=>{var pt,ut;const V=K._gcDef,ge={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[V==null?void 0:V.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",Ie={purple:"#b06ce0",light_blue:"#00d4ef"}[V==null?void 0:V.color]||"#b06ce0",xe=(V==null?void 0:V.name)||K.gc_type,Ut=(V==null?void 0:V.effect)||((pt=de[K.gc_type])==null?void 0:pt.desc)||"",dt=V!=null&&V.image_url?`/manager-wars/icons/${V.image_url}`:null,Vt=((ut=de[K.gc_type])==null?void 0:ut.icon)||"⚡",lt=Math.round(ie*.22),ct=Math.round(ie*.22),Ne=ie-lt-ct,Yt=xe.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${K.id}" data-gc-type="${K.gc_type}"
      ...
      <div style="font-size:${P?11:9}px;color:#000;font-weight:900">+${r.boostValue}</div>
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 14px;background:rgba(0,0,0,0.25);flex-shrink:0">
        ...
        <div style="flex:1;text-align:center;color:#fff;font-size:14px;font-weight:700">
          ${S} <span style="color:#FFD700;font-size:18px">${$}</span> – <span style="color:#FFD700;font-size:18px">${T}</span> ${z}
        </div>
      ...
      </div>`,e.querySelectorAll(".match-slot-hit").forEach(K=>{K.addEventListener("click",()=>{if(!B&&!C)return;const Z=K.dataset.cardId,ie=K.dataset.role,V=(h[ie]||[]).find(xe=>xe.cardId===Z);if(!V||V.used)return;Array.isArray(r["selected_"+o])||(r["selected_"+o]=[]);const ge=r["selected_"+o],Ie=ge.findIndex(xe=>xe.cardId===Z);Ie>-1?ge.splice(Ie,1):ge.length<3&&ge.push({...V,_role:ie}),m()})}),(ae=e.querySelector("#pvp-boost-card"))==null||ae.addEventListener("click",()=>{x()}),e.querySelectorAll(".pvp-gc-mini").forEach(K=>{K.addEventListener("click",()=>{D(K.dataset.gcId,K.dataset.gcType)})}),(se=e.querySelector("#pvp-action"))==null||se.addEventListener("click",()=>{B?E():C&&L()}),(ce=e.querySelector("#pvp-quit"))==null||ce.addEventListener("click",()=>{confirm("Quitter le match ? Vous perdrez par forfait.")&&g()}),(we=e.querySelector("#pvp-view-opp"))==null||we.addEventListener("click",()=>{A()}),function(){const Z=e.querySelector(".terrain-wrapper svg");Z&&(Z.removeAttribute("width"),Z.removeAttribute("height"),Z.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",Z.setAttribute("viewBox","-26 -26 352 352"),Z.setAttribute("preserveAspectRatio","xMidYMid meet"));const ie=e.querySelector(".match-screen");if(ie){const V=e.clientHeight;V>50&&(ie.style.height=V+"px",ie.style.maxHeight=V+"px",ie.style.overflow="hidden")}}(),r._timerInt&&(clearInterval(r._timerInt),r._timerInt=null),B||C){let K=30,Z=!1;const ie=()=>document.getElementById("pvp-timer"),V=()=>{ie()&&(ie().textContent=K+"s",ie().style.color=Z?"#ff4444":"#fff")};V(),r._timerInt=setInterval(()=>{K--,K<=15&&!Z&&(Z=!0),V(),K<=0&&(clearInterval(r._timerInt),r._timerInt=null,g())},1e3)}}function b(){var C;const h=r["gcReady_"+o],$=r["gcReady_"+c];Array.isArray(r["gc_"+o])||(r["gc_"+o]=[]);let T=r["gc_"+o];const S=3;function z(N,G){const M=N._gcDef,P={purple:"linear-gradient(135deg,#3d0a7a,#7a28b8)",light_blue:"linear-gradient(135deg,#006080,#00bcd4)"},H={purple:"#9b59b6",light_blue:"#00bcd4"},_=P[M==null?void 0:M.color]||P.purple,y=H[M==null?void 0:M.color]||H.purple;return`<div class="pvp-gc-card" data-id="${N.id}"
    ...
    <div id="pvp-gc-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      <div style="text-align:center;padding:12px 16px 8px;flex-shrink:0">
        ...
        <div style="font-size:13px;color:rgba(255,255,255,0.5);margin-top:3px">
          Sélectionne <b style="color:#FFD700">${S}</b> cartes · ${T.length}/${S}
        </div>
      </div>
      ${h?`
    ...
    </div>`,h)return;const B=o==="p1"?d.home_id:d.away_id;k.from("cards").select("id,gc_type").eq("owner_id",B).eq("card_type","game_changer").then(async({data:N})=>{const{data:G}=await k.from("gc_definitions").select("*").eq("is_active",!0),M=new Set,P=(N||[]).filter(_=>M.has(_.gc_type)?!1:(M.add(_.gc_type),!0)).map(_=>({..._,_gcDef:(G==null?void 0:G.find(y=>y.name===_.gc_type))||null})),H=document.getElementById("pvp-gc-grid");if(H){if(!P.length){H.innerHTML='<div style="color:rgba(255,255,255,0.4);font-size:13px;text-align:center;margin-top:30px">Aucune carte Game Changer disponible.</div>';return}H.innerHTML=P.map(_=>z(_,T.includes(_.id))).join(""),H.querySelectorAll(".pvp-gc-card").forEach(_=>{_.addEventListener("click",()=>{const y=_.dataset.id,j=T.indexOf(y);j>-1?T.splice(j,1):T.length<S&&T.push(y),m()})})}}),(C=e.querySelector("#pvp-gc-validate"))==null||C.addEventListener("click",async()=>{const N=o==="p1"?d.home_id:d.away_id,{data:G}=await k.from("cards").select("id,gc_type").eq("owner_id",N).eq("card_type","game_changer").in("id",T),{data:M}=await k.from("gc_definitions").select("*").eq("is_active",!0),P=(G||[]).map(H=>({...H,_gcDef:(M==null?void 0:M.find(_=>_.name===H.gc_type))||null}));await u({["gcReady_"+o]:!0,["gcCardsFull_"+o]:P}),r["gcReady_"+c]&&await u({phase:"reveal"})})}function I(){e.style.overflow="hidden",e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      ...
      <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
    </div>`,o==="p1"&&setTimeout(async()=>{await u({phase:"midfield"})},5e3)}function w(){const h=r[o+"Team"].MIL||[],$=r[c+"Team"].MIL||[];function T(G){return G.reduce((M,P)=>M+Q(P,"MIL"),0)}function S(G){let M=0;for(let P=0;P<G.length-1;P++){const H=fe(G[P],G[P+1]);H==="#00ff88"?M+=2:H==="#FFD700"&&(M+=1)}return M}const z=T(h)+S(h),B=T($)+S($),C=z>=B;function N(G,M,P){return`<div style="text-align:center">
        ...
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${G.map((H,_)=>{const y=ne(H),j=_<G.length-1?fe(H,G[_+1]):null,F=j&&j!=="#ff3333"&&j!=="#cc2222";return`
        ...
        <div style="margin-top:6px;font-size:11px;color:rgba(255,255,255,0.5)">
          Score: ${T(G)} + ${S(G)} liens = <b style="color:#fff">${T(G)+S(G)}</b>
        </div>
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div id="pvp-midfield-result" style="opacity:0;text-align:center;transition:opacity 0.5s;color:#fff;max-width:320px"></div>
    </div>`,setTimeout(()=>{const G=document.getElementById("pvp-score-me"),M=document.getElementById("pvp-score-opp"),P=document.getElementById("pvp-midfield-result");G&&M&&(C?(G.style.fontSize="80px",G.style.color="#FFD700",M.style.opacity="0.25"):(M.style.fontSize="80px",M.style.color="#ff6b6b",G.style.opacity="0.25"));const H=r.p1Team.MIL||[],_=r.p2Team.MIL||[],y=T(H)+S(H),j=T(_)+S(_),F=y>=j?"p1":"p2";r[F+"Name"],P&&(P.style.opacity="1",P.innerHTML=`<div style="font-size:20px;font-weight:900;margin-bottom:10px">
        ...
        </div>`:""}`),setTimeout(async()=>{if(o!=="p1")return;const q=F,O=Gt();await u({phase:q+"-attack",attacker:q,round:1,boostValue:O,boostUsed:!1,boostOwner:q})},1800)},600)}function x(){var S;const h=r[o+"Team"],$=Object.entries(h).flatMap(([z,B])=>(B||[]).filter(C=>!C.used).map(C=>({...C,_line:z})));if(!$.length)return;const T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",T.innerHTML=`
      ...
      </div>`,document.body.appendChild(T),(S=T.querySelector("#boost-picker-close"))==null||S.addEventListener("click",()=>T.remove()),T.querySelectorAll(".boost-pick-item").forEach(z=>{z.addEventListener("click",async()=>{const B=z.dataset.cid,C=$.find(M=>M.cardId===B);if(!C)return;const N=r[o+"Team"],G=(N[C._line]||[]).find(M=>M.cardId===B);G&&(G.boost=(G.boost||0)+r.boostValue),T.remove(),await u({[o+"Team"]:N,boostUsed:!0})})})}function A(){var $;const h=document.createElement("div");h.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:20px;overflow-y:auto",h.innerHTML=`
      <div style="font-size:12px;color:rgba(255,255,255,0.5);letter-spacing:2px;text-transform:uppercase">Équipe adverse</div>
      <div style="font-size:18px;font-weight:900;color:#ff6b6b">${r[c+"Name"]}</div>
      <div style="width:min(90vw,420px)">${Ee(r[c+"Team"],r[c+"Formation"],null,[],300,300)}</div>
      <button id="pvp-view-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(h),($=h.querySelector("#pvp-view-opp-close"))==null||$.addEventListener("click",()=>h.remove())}function D(h,$){var y,j;const S=(r["gcCardsFull_"+o]||[]).find(F=>F.id===h),z=S==null?void 0:S._gcDef,B=de[$]||{icon:"⚡",desc:"Carte spéciale."},C={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[z==null?void 0:z.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",N={purple:"#b06ce0",light_blue:"#00d4ef"}[z==null?void 0:z.color]||"#b06ce0",G=(z==null?void 0:z.name)||$,M=(z==null?void 0:z.effect)||B.desc,P=z!=null&&z.image_url?`/manager-wars/icons/${z.image_url}`:null,H=B.icon||"⚡",_=document.createElement("div");_.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",_.innerHTML=`
      ...
      </div>`,document.body.appendChild(_),(y=_.querySelector("#pvp-gc-back"))==null||y.addEventListener("click",()=>_.remove()),(j=_.querySelector("#pvp-gc-use"))==null||j.addEventListener("click",async()=>{_.remove();const F=r["usedGc_"+o]||[];F.push(h),await u({["usedGc_"+o]:F})})}function v(){var z;const h=r[o+"Score"],$=r[c+"Score"],T=h>$,S=h===$;e.style.overflow="hidden",e.innerHTML=`
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;height:${r}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ...
      ${p?`<img src="${p}" style="width:${r-4}px;height:${r-4}px;object-fit:contain">`:e!=null&&e.clubName?`<span style="font-size:${Math.max(4,r-8)}px;font-weight:700;color:#333">${(e.clubName||"").slice(0,3).toUpperCase()}</span>`:""}
        ...
        stroke="${L}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const f=48,u=64,g=13,m=16,b={normal:"#aaaaaa",pépite:"#D4A017",papyte:"#111111",légende:"#7a28b8"};for(const[w,x]of Object.entries(c)){const A=s(w);if(!A||!x)continue;const D=w.replace(/[0-9]/g,""),v=be[D]||"#555",E=i==="attack"&&["MIL","ATT"].includes(D)&&!x.used||i==="defense"&&["GK","DEF","MIL"].includes(D)&&!x.used,L=l.includes(x.cardId);let h;i==="attack"?h=D==="MIL"?Number(x.note_m)||0:Number(x.note_a)||0:i==="defense"?h=D==="GK"?Number(x.note_g)||0:D==="MIL"?Number(x.note_m)||0:Number(x.note_d)||0:h=Number(D==="GK"?x.note_g:D==="DEF"?x.note_d:D==="MIL"?x.note_m:x.note_a)||0,h=h+(x.boost||0);const $=(A.x-f/2).toFixed(1),T=(A.y-u/2).toFixed(1),S=x.used?.25:1,z=b[x==null?void 0:x.rarity]||b.normal,B=L?"#ff3030":z,C=L?3:(x==null?void 0:x.rarity)==="légende"||(x==null?void 0:x.rarity)==="pépite"?2.5:2,N=u-g-m;r+=`<defs><clipPath id="cp-${w}"><rect x="${$}" y="${(A.y-u/2+g).toFixed(1)}" width="${f}" height="${N}"/></clipPath></defs>`,r+=`<rect x="${$}" y="${T}" width="${f}" height="${u}" rx="5" fill="${v}" opacity="${S}"/>`;const G=ne(x);G&&!x.used&&(r+=`<image href="${G}" x="${$}" y="${(A.y-u/2+g).toFixed(1)}" width="${f}" height="${N}" clip-path="url(#cp-${w})" preserveAspectRatio="xMidYMin slice"/>`),r+=`<rect x="${$}" y="${T}" width="${f}" height="${g}" rx="4" fill="rgba(255,255,255,0.92)"/>`,r+=`<text x="${A.x.toFixed(1)}" y="${(A.y-u/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(x.name||"").slice(0,9)}</text>`;const M=(A.y+u/2-m).toFixed(1);if(r+=`<rect x="${$}" y="${M}" width="${f}" height="${m}" fill="rgba(255,255,255,0.92)"/>`,x.used)r+=`<text x="${A.x.toFixed(1)}" y="${(A.y+u/2-m/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="13" font-weight="900" fill="rgba(0,0,0,0.4)" font-family="Arial Black">–</text>`;else{const P=Ve(x.country_code);P?r+=`<image href="${P}" x="${(A.x-20).toFixed(1)}" y="${(A.y+u/2-m+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:r+=`<text x="${(A.x-13).toFixed(1)}" y="${(A.y+u/2-m/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${Nt(x.country_code)}</text>`,r+=`<text x="${A.x.toFixed(1)}" y="${(A.y+u/2-m/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${h}</text>`;const H=Rt(x);H?r+=`<image href="${H}" x="${(A.x+f/2-14).toFixed(1)}" y="${(A.y+u/2-m+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:x.clubName&&(r+=`<text x="${(A.x+14).toFixed(1)}" y="${(A.y+u/2-m/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(x.clubName||"").slice(0,3).toUpperCase()}</text>`),x.boost&&(r+=`<rect x="${(A.x+f/2-12).toFixed(1)}" y="${(A.y-u/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,r+=`<text x="${(A.x+f/2-5).toFixed(1)}" y="${(A.y-u/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${x.boost}</text>`)}r+=`<rect x="${$}" y="${T}" width="${f}" height="${u}" rx="5" fill="${L?"rgba(255,255,255,0.12)":"none"}" stroke="${B}" stroke-width="${C}" opacity="${S}"/>`,E&&(r+=`<rect x="${$}" y="${T}" width="${f}" height="${u}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${L?"selected":""}" data-card-id="${x.cardId}" data-role="${D}" style="cursor:pointer"/>`)}const I=38;return`<svg viewBox="${-I} ${-I} ${p+I*2} ${n+I*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${r}
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;height:100%;width:100%">
    ...
    ${(()=>{const v=window.innerWidth>=700,E=(C,N,G)=>{var Y,W;const M=(t.gcDefs||[]).find(U=>U.name===C.gc_type),P={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[M==null?void 0:M.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",H={purple:"#b06ce0",light_blue:"#00d4ef"}[M==null?void 0:M.color]||"#b06ce0",_=(M==null?void 0:M.name)||C.gc_type,y=(M==null?void 0:M.effect)||((Y=de[C.gc_type])==null?void 0:Y.desc)||"",j=M!=null&&M.image_url?`/manager-wars/icons/${M.image_url}`:null,F=((W=de[C.gc_type])==null?void 0:W.icon)||"⚡",q=Math.round(G*.22),O=Math.round(G*.22),R=G-q-O,X=_.length>12?7:9;return`<div class="gc-mini" data-gc-id="${C.id}" data-gc-type="${C.gc_type}"
          style="width:${N}px;height:${G}px;border-radius:10px;border:2px solid ${H};background:${P};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(v=>{var E,L,h;if(v.type==="duel"){const $=v.isGoal,T=v.homeScored?"#FFD700":$?"#ff6b6b":"rgba(255,255,255,0.3)",S=v.homeScored?"⚽ BUT !":$?"⚽ BUT IA !":(E=v.homePlayers)!=null&&E.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${$?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${T};margin-bottom:4px">
                <div style="font-size:9px;color:${T};letter-spacing:1px;margin-bottom:5px;font-weight:700;text-transform:uppercase">${S}</div>
  ...
  `,document.body.appendChild(l);let s=!1;const r=()=>{s||(s=!0,l.remove(),i())};l.addEventListener("click",r),setTimeout(r,2e3)}function $e(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function $t(e,t,i,l=null){if(t.phase!=="attack"){$e("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){$e(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const p=Object.values(t.homeTeam).flat().filter(r=>r.used),n=t.homeSubs.filter(r=>!t.usedSubIds.includes(r.cardId));if(!p.length){$e("Aucun joueur utilisé à remplacer");return}if(!n.length){$e("Aucun remplaçant disponible");return}let a=0,o=Math.max(0,n.findIndex(r=>r.cardId===l)),c=!1;const d=document.createElement("div");d.id="sub-overlay",d.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function s(){var I,w,x,A,D,v;const r=p[a],f=n[o],u=Math.min(130,Math.round((window.innerWidth-90)/2)),g=Math.round(u*1.35),m=E=>`background:rgba(255,255,255,0.12);border:none;color:${E?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${E?"default":"pointer"};flex-shrink:0`;d.innerHTML=`
    ...
    </div>`,(I=d.querySelector("#sub-close"))==null||I.addEventListener("click",()=>d.remove()),(w=d.querySelector("#out-up"))==null||w.addEventListener("click",()=>{a>0&&(a--,s())}),(x=d.querySelector("#out-down"))==null||x.addEventListener("click",()=>{a<p.length-1&&(a++,s())}),(A=d.querySelector("#in-up"))==null||A.addEventListener("click",()=>{o>0&&(o--,s())}),(D=d.querySelector("#in-down"))==null||D.addEventListener("click",()=>{o<n.length-1&&(o++,s())});const b=(E,L,h,$)=>{const T=d.querySelector("#"+E);if(!T)return;let S=0;T.addEventListener("touchstart",z=>{S=z.touches[0].clientY},{passive:!0}),T.addEventListener("touchend",z=>{const B=z.changedTouches[0].clientY-S;if(Math.abs(B)<30)return;const C=L();B<0&&C<$-1?(h(C+1),s()):B>0&&C>0&&(h(C-1),s())},{passive:!0})};b("in-panel",()=>o,E=>o=E,n.length),b("out-panel",()=>a,E=>a=E,p.length),(v=d.querySelector("#sub-confirm"))==null||v.addEventListener("click",E=>{if(E.preventDefault(),E.stopPropagation(),c)return;c=!0;const L=p[a],h=n[o];if(!L||!h)return;let $=null,T=-1;for(const[z,B]of Object.entries(t.homeTeam)){const C=(B||[]).findIndex(N=>N.cardId===L.cardId);if(C!==-1){$=z,T=C;break}}if(T===-1||!$){$e("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),d.remove();return}const S={...h,_line:$,_col:L._col||0,used:!1,boost:0};t.homeTeam[$].splice(T,1,S),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(h.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:L.name,firstname:L.firstname,note:Q(L,$),portrait:ne(L),job:L.job,country_code:L.country_code,rarity:L.rarity,clubName:L.clubName,clubLogo:L.clubLogo},inPlayer:{name:h.name,firstname:h.firstname,note:Q(h,$),portrait:ne(h),job:h.job,country_code:h.country_code,rarity:h.rarity,clubName:h.clubName,clubLogo:h.clubLogo},text:`🔄 ${h.firstname} ${h.name} remplace ${L.firstname} ${L.name}`}),d.remove(),Wi(L,h,()=>te(e,t,i))})}document.body.appendChild(d),s()}function Xi(e,t,i,l,p){var g,m;const n=(l.gcDefs||[]).find(b=>b.name===t),a=de[t]||{icon:"⚡",desc:"Carte spéciale."},o={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[n==null?void 0:n.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",c={purple:"#b06ce0",light_blue:"#00d4ef"}[n==null?void 0:n.color]||"#b06ce0",d=(n==null?void 0:n.name)||t,s=(n==null?void 0:n.effect)||a.desc,r=n!=null&&n.image_url?`/manager-wars/icons/${n.image_url}`:null,f=a.icon||"⚡",u=document.createElement("div");u.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",u.innerHTML=`
    ...
    </div>`,document.body.appendChild(u),(g=u.querySelector("#gc-back"))==null||g.addEventListener("click",()=>u.remove()),(m=u.querySelector("#gc-use"))==null||m.addEventListener("click",()=>{u.remove(),Ji(e,t,i,l,p)})}function Oe(e,t,i,l,p,n){const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let o=[];function c(){var d,s;a.innerHTML=`
    <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(p=>{p.addEventListener("click",()=>{const n=p.dataset.cardId;for(const a of["GK","DEF","MIL","ATT"]){const o=(t.homeTeam[a]||[]).find(c=>c.cardId===n);if(o){o.boost=(o.boost||0)+t.boostCard.value,t.log.push({text:`⚡ Boost +${t.boostCard.value} appliqué à ${o.name}`,type:"info"});break}}t.boostUsed=!0,i.closeModal(),te(e,t,i)})})}function rt(e,t,i,l,p){const n=document.createElement("div");n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:900;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;overflow:hidden;cursor:pointer";const a=Array.from({length:10},(d,s)=>`
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(n);let o=!1;const c=()=>{o||(o=!0,n.remove(),setTimeout(()=>p(),50))};n.addEventListener("click",c),setTimeout(c,3500)}async function ye(e,t,i){var s,r;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:l}=i,p=t.homeScore>t.aiScore,n=t.homeScore===t.aiScore,a=p?"victoire":n?"nul":"defaite",o=Zt(t.mode,a);t.matchId&&await k.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:p?l.profile.id:null,home_credits_reward:o,played_at:new Date().toISOString()}).eq("id",t.matchId);const c={credits:(l.profile.credits||0)+o,matches_played:(l.profile.matches_played||0)+1};p?c.wins=(l.profile.wins||0)+1:n?c.draws=(l.profile.draws||0)+1:c.losses=(l.profile.losses||0)+1,await k.from("users").update(c).eq("id",l.profile.id),await i.refreshProfile();const d=document.createElement("div");d.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",d.innerHTML=`
    ...
    </div>`,document.body.appendChild(d),(s=document.getElementById("res-home"))==null||s.addEventListener("click",()=>{d.remove(),le(),i.navigate("home")}),(r=document.getElementById("res-replay"))==null||r.addEventListener("click",()=>{d.remove(),le(),i.navigate("match",{matchMode:t.mode})})}function Zi(e,t){t.openModal("Équipe adverse (IA)",`<div style="background:#0a3d1e;padding:12px;border-radius:8px">
  ...
  `;function c(d){const s=document.getElementById("mkt-content"),r=d==="buy"?a:o;if(r.length===0){s.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      ${f.map(u=>{var x,A,D,v;const g=(x=u.card)==null?void 0:x.player;if(!g)return"";const m=g.job==="GK"?g.note_g:g.job==="DEF"?g.note_d:g.job==="MIL"?g.note_m:g.note_a,b=en[g.rarity],I=(i.profile.credits||0)>=u.price,w=u.status==="sold";return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${w?"opacity:0.65":""}">
  ...
  `}async function sn(e,{state:t,navigate:i,toast:l}){const p=t.profile;if(!p)return;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:n}=await k.from("matches").select("id,home_id,away_id,home_score,away_score,status,mode,winner_id,created_at,played_at").or(`home_id.eq.${p.id},away_id.eq.${p.id}`).order("created_at",{ascending:!1}).limit(50),a={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},o=(n||[]).filter(s=>s.status==="finished"),c=(n||[]).filter(s=>s.status==="in_progress");function d(s){const r=s.home_id===p.id;r?s.home_score:s.away_score,r?s.away_score:s.home_score;const f=s.winner_id===p.id,u=s.home_score===s.away_score&&s.status==="finished",g=s.status!=="finished"?"…":u?"N":f?"V":"D",m=s.status!=="finished"||u?"#888":f?"#1A6B3C":"#c0392b";let b=a[s.mode]||s.mode;s.away_id===null&&!b.startsWith("IA")&&(b="IA");let I="";s.status==="in_progress"&&Date.now()-new Date(s.created_at).getTime()>3600*1e3&&(I=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const w=new Date(s.created_at),x=w.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+w.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),A=s.status==="finished"?`${s.home_score} - ${s.away_score}`:`${s.home_score||0} - ${s.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
        <div style="font-size:13px;font-weight:600">${b}${I}</div>
