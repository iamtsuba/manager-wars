import{s as $,l as ae,F as Ge,b as xe,c as st,d as Mt}from"./formation-links-CDBKB_z4.js";function Ye(e,{navigate:t,toast:i}){e.innerHTML=`
  ...
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const ce=1e3;function Rt(e,t,i,a){var m,h,_;const{state:d,toast:o,closeModal:s,navigate:l,refreshProfile:r}=i,n=e.formation,c={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function p(){const v=xe[n]||{},x=Ge[n]||[],w=290,I=360,T=20;function g(A){const z=v[A];return z?{x:z.x*w,y:z.y*I}:null}let b=`<svg width="${w}" height="${I}" viewBox="0 0 ${w} ${I}" xmlns="http://www.w3.org/2000/svg">`;for(const[A,z]of x){const j=g(A),S=g(z);!j||!S||(b+=`<line x1="${j.x}" y1="${j.y}" x2="${S.x}" y2="${S.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const A of Object.keys(v)){const z=g(A);if(!z)continue;const j=A.replace(/\d+/,""),S=c[j]||"#555";b+=`<circle cx="${z.x}" cy="${z.y}" r="${T}" fill="${S}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,b+=`<text x="${z.x}" y="${z.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${j}</text>`}return b+="</svg>",b}const u=t.filter(v=>v.formation===n),f=u.length,y=!e.is_for_sale;a(`Formation ${n}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(m=document.getElementById("direct-sell-form-btn"))==null||m.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${n} pour ${ce.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const v=u.find(w=>!w.is_for_sale)||u[0];if(!v){o("Aucune carte à vendre","error");return}const{error:x}=await $.from("cards").delete().eq("id",v.id);if(x){o(x.message,"error");return}await $.from("users").update({credits:(d.profile.credits||0)+ce}).eq("id",d.profile.id),await r(),o(`+${ce.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),s(),l("collection")}),(h=document.getElementById("market-sell-form-btn"))==null||h.addEventListener("click",async()=>{const v=parseInt(document.getElementById("sell-price-form").value);if(!v||v<1){o("Prix invalide","error");return}await $.from("cards").update({is_for_sale:!0,sale_price:v}).eq("id",e.id),await $.from("market_listings").insert({seller_id:d.profile.id,card_id:e.id,price:v}),o("Carte mise en vente sur le marché !","success"),s(),l("collection")}),(_=document.getElementById("cancel-sell-form-btn"))==null||_.addEventListener("click",async()=>{await $.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await $.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),o("Annonce retirée","success"),s(),l("collection")})}function Ze(e,t,i,a){var T,g,b,A,z,j;const{state:d,toast:o,openModal:s,closeModal:l,navigate:r,refreshProfile:n}=a,c=e.player,p=t.filter(S=>S.player.id===c.id),u=p.length,f=Ft[c.rarity]||1e3,y=c.rarity!=="legende",m=gt(c),h=fe(c,c.job),_=c.job2?fe(c,c.job2):null,v=Ee[c.job]||"#1A6B3C",x=c.job2?Ee[c.job2]:null,w=ft[c.rarity]||"#ccc",I=Re[c.country_code]||c.country_code||"";s(`${c.firstname} ${c.surname_encoded}`,`<div style="display:flex;gap:16px;flex-wrap:wrap;justify-content:center">
  ...
  </div>`,qt(e,t,d,i),document.getElementById("builder-back").addEventListener("click",()=>a("decks")),document.getElementById("formation-select").addEventListener("change",n=>{t.formation=n.target.value;const c=et(t.formation),p={};c.forEach(u=>{t.slots[u]&&(p[u]=t.slots[u])}),t.slots=p,se(e,t,i)}),document.getElementById("save-deck").addEventListener("click",()=>Ht(t,i)),e.querySelectorAll("[data-remove-sub]").forEach(n=>{n.addEventListener("click",()=>{t.subs=t.subs.filter(c=>c!==n.dataset.removeSub),se(e,t,i)})}),(r=document.getElementById("add-sub-btn"))==null||r.addEventListener("click",()=>{Pt(t,e,i)})}function qt(e,t,i,a){const d=e.querySelector("#deck-field");if(!d)return;const o=xe[t.formation]||{},s=st(t.formation)||[],l={};for(const _ of i){const v=t.slots[_],x=v?t.playerCards.find(w=>w.id===v):null;l[_]=x?x.player:null}const r=300,n=300,c=48,p=64,u=13,f=16,y=38;function m(_){const v=o[_];return v?{x:v.x*r,y:v.y*n}:null}let h="";for(const[_,v]of s){const x=m(_),w=m(v);if(!x||!w)continue;const I=l[_]?{...l[_],club_id:l[_].club_id,country_code:l[_].country_code,rarity:l[_].rarity}:null,T=l[v]?{...l[v],club_id:l[v].club_id,country_code:l[v].country_code,rarity:l[v].rarity}:null,g=ae(I,T);g==="#ff3333"||g==="#cc2222"?h+=`<line x1="${x.x.toFixed(1)}" y1="${x.y.toFixed(1)}" x2="${w.x.toFixed(1)}" y2="${w.y.toFixed(1)}" stroke="${g}" stroke-width="2.5" stroke-linecap="round" opacity="0.4"/>`:(h+=`<line x1="${x.x.toFixed(1)}" y1="${x.y.toFixed(1)}" x2="${w.x.toFixed(1)}" y2="${w.y.toFixed(1)}" stroke="${g}" stroke-width="8" stroke-linecap="round" opacity="0.2"/>`,h+=`<line x1="${x.x.toFixed(1)}" y1="${x.y.toFixed(1)}" x2="${w.x.toFixed(1)}" y2="${w.y.toFixed(1)}" stroke="${g}" stroke-width="2.5" stroke-linecap="round" opacity="0.9"/>`)}for(const _ of i){const v=m(_);if(!v)continue;const x=l[_],w=_.replace(/\d+/,""),I=te[w]||"#555",T=(v.x-c/2).toFixed(1),g=(v.y-p/2).toFixed(1),b={normal:"#aaa",pépite:"#D4A017",papyte:"#222",légende:"#7a28b8"}[x==null?void 0:x.rarity]||"#aaa";if(x){const A=Te(x),z=mt(x),j=xt(x.country_code),S=Number(w==="GK"?x.note_g:w==="DEF"?x.note_d:w==="MIL"?x.note_m:x.note_a)||0,D=p-u-f;h+=`<defs><clipPath id="dcp-${_}"><rect x="${T}" y="${(v.y-p/2+u).toFixed(1)}" width="${c}" height="${D}"/></clipPath></defs>`,h+=`<rect x="${T}" y="${g}" width="${c}" height="${p}" rx="5" fill="${I}"/>`,A&&(h+=`<image href="${A}" x="${T}" y="${(v.y-p/2+u).toFixed(1)}" width="${c}" height="${D}" clip-path="url(#dcp-${_})" preserveAspectRatio="xMidYMin slice"/>`),h+=`<rect x="${T}" y="${g}" width="${c}" height="${u}" fill="rgba(255,255,255,0.93)"/>`,h+=`<text x="${v.x.toFixed(1)}" y="${(v.y-p/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(x.surname_encoded||"").slice(0,9)}</text>`;const N=(v.y+p/2-f).toFixed(1);h+=`<rect x="${T}" y="${N}" width="${c}" height="${f}" fill="rgba(255,255,255,0.93)"/>`,j&&(h+=`<image href="${j}" x="${(v.x-21).toFixed(1)}" y="${(v.y+p/2-f+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`),h+=`<text x="${v.x.toFixed(1)}" y="${(v.y+p/2-f/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${S}</text>`,z&&(h+=`<image href="${z}" x="${(v.x+c/2-14).toFixed(1)}" y="${(v.y+p/2-f+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`),h+=`<rect x="${T}" y="${g}" width="${c}" height="${p}" rx="5" fill="none" stroke="${b}" stroke-width="2"/>`}else h+=`<rect x="${T}" y="${g}" width="${c}" height="${p}" rx="5" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="5,3"/>`,h+=`<text x="${v.x.toFixed(1)}" y="${v.y.toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="22" fill="rgba(255,255,255,0.35)">+</text>`,h+=`<text x="${v.x.toFixed(1)}" y="${(v.y+16).toFixed(1)}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.3)">${w}</text>`;h+=`<rect x="${T}" y="${g}" width="${c}" height="${p}" rx="5" fill="rgba(0,0,0,0.01)" class="deck-slot-hit" data-pos="${_}" style="cursor:pointer"/>`}d.innerHTML=`<svg viewBox="${-y} ${-y} ${r+y*2} ${n+y*2}" width="100%" style="display:block;max-width:440px;margin:0 auto">${h}</svg>`,d.querySelectorAll(".deck-slot-hit").forEach(_=>{_.addEventListener("click",()=>Ot(_.dataset.pos,t,e,a))})}function Ot(e,t,i,a){var u,f,y;const{openModal:d,closeModal:o}=a,s=e.replace(/\d+/,""),l=t.slots[e],r=l?t.playerCards.find(m=>m.id===l):null;(u=r==null?void 0:r.player)==null||u.id;const n=new Set;Object.entries(t.slots).forEach(([m,h])=>{var v;if(m===e||!h)return;const _=t.playerCards.find(x=>x.id===h);(v=_==null?void 0:_.player)!=null&&v.id&&n.add(_.player.id)}),t.subs.forEach(m=>{var _;const h=t.playerCards.find(v=>v.id===m);(_=h==null?void 0:h.player)!=null&&_.id&&n.add(h.player.id)});const c=new Set,p=t.playerCards.filter(m=>{const h=m.player;return!(h.job===s||h.job2===s)||n.has(h.id)||c.has(h.id)?!1:(c.add(h.id),!0)});p.sort((m,h)=>{const _=s==="GK"?m.player.note_g:s==="DEF"?m.player.note_d:s==="MIL"?m.player.note_m:m.player.note_a;return(s==="GK"?h.player.note_g:s==="DEF"?h.player.note_d:s==="MIL"?h.player.note_m:h.player.note_a)-_}),d(`Choisir ${s} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${p.length>0?p.map(m=>{var w,I;const h=m.player,_=s==="GK"?h.note_g:s==="DEF"?h.note_d:s==="MIL"?h.note_m:h.note_a,v=Te(h),x={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[h.rarity];return`<div class="player-option" data-card-id="${m.id}"
          ...
          <div style="width:44px;height:44px;border-radius:8px;overflow:hidden;flex-shrink:0;background:#dde;border:2px solid ${te[s]}">
            ${v?`<img src="${v}" style="width:100%;height:100%;object-fit:cover">`:`<div style="width:100%;height:100%;background:${te[s]};display:flex;align-items:center;justify-content:center;color:#fff;font-size:10px;font-weight:700">${s}</div>`}
          </div>
          <!-- Infos -->
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}const ui="simulation",vt="VOTRE_ZONE_ID";function fi(){switch(ui){case"propellerads":return gi();case"adsense":return xi();default:return Ne()}}function Ne(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:#111;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:14px";let i=5;t.innerHTML=`
    ...
    </div>`}function s(){var n,c,p;e.style.overflow="hidden",e.style.position="relative";const l=e.getBoundingClientRect().top,r=(((n=window.visualViewport)==null?void 0:n.height)||window.innerHeight)-l;e.style.height=Math.max(200,r)+"px",e.innerHTML=`
    <div style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      <div style="flex:1;overflow-y:auto;display:flex;flex-wrap:wrap;gap:10px;justify-content:center;align-content:flex-start;padding:8px 16px">
        ${t.map(u=>{const f=d.find(y=>y.id===u.id);return`<div style="position:relative">${o(u,!!f)}</div>`}).join("")}
      </div>
    ...
    </div>`,e.querySelectorAll(".gc-select-card").forEach(u=>{u.addEventListener("click",()=>{const f=u.dataset.id,y=d.findIndex(m=>m.id===f);if(y>-1)d.splice(y,1);else{if(d.length>=3)return;const m=t.find(h=>h.id===f);m&&d.push(m)}s()})}),(c=e.querySelector("#gc-sel-confirm"))==null||c.addEventListener("click",()=>{d.length&&i(d)}),(p=e.querySelector("#gc-sel-skip"))==null||p.addEventListener("click",()=>{i([])})}s()}async function wi(e,t,i){const{state:a,navigate:d}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await $.from("decks").select("id,name,is_active,formation").eq("owner_id",a.profile.id).order("created_at",{ascending:!1});if(!o||o.length===0){bt(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>d("decks"));return}const s=o.map(c=>c.id),{data:l}=await $.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",s).order("slot_order");let r=0;function n(){var v,x,w,I,T;const c=o[r],p=(l||[]).filter(g=>g.deck_id===c.id),u=p.filter(g=>{var b;return g.is_starter&&((b=g.card)==null?void 0:b.player)}).map(g=>je(g.card)),f=p.find(g=>{var b;return((b=g.card)==null?void 0:b.card_type)==="formation"}),y=c.formation||((v=f==null?void 0:f.card)==null?void 0:v.formation)||"4-4-2",m=u.length>=11?wt(u,y):null,h=u.length>=11;e.innerHTML=`
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${p.map((y,m)=>{const h=W(y),_=m<p.length-1?ae(y,p[m+1]):null,v=_&&_!=="#ff3333"&&_!=="#cc2222";return`
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;height:${p}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ...
      ${d?`<img src="${d}" style="width:${p-4}px;height:${p-4}px;object-fit:contain">`:e!=null&&e.clubName?`<span style="font-size:${Math.max(4,p-8)}px;font-weight:700;color:#333">${(e.clubName||"").slice(0,3).toUpperCase()}</span>`:""}
  ...
  </div>`}function be(e,t,i){if(!(e!=null&&e.length))return"";const a=e.slice(0,5);let d='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return a.forEach((o,s)=>{if(d+=ne(o,40,52),s<a.length-1){const l=ae(o,a[s+1]);d+=`<div style="width:7px;height:3px;background:${l==="#ff3333"||l==="#cc2222"?"rgba(255,255,255,0.12)":l};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(d+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),d+="</div>",d}function Be(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function _t(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function Oe(e,t,i,a,d=310,o=310){const s=xe[t]||{},l=st(t)||Ge[t]||[],r={},n=["ATT","MIL","DEF","GK"];for(const v of n)(e[v]||[]).forEach((w,I)=>{r[`${v}${I+1}`]=w});function c(v){const x=s[v];return x?{x:x.x*d,y:x.y*o}:null}let p="";for(const[v,x]of l){const w=c(v),I=c(x);if(!w||!I)continue;const T=r[v],g=r[x],b=ae(T,g);b==="#00ff88"||b==="#FFD700"?(p+=`<line x1="${w.x.toFixed(1)}" y1="${w.y.toFixed(1)}" x2="${I.x.toFixed(1)}" y2="${I.y.toFixed(1)}"
        ...
        stroke="${b}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const u=48,f=64,y=13,m=16,h={normal:"#aaaaaa",pépite:"#D4A017",papyte:"#111111",légende:"#7a28b8"};for(const[v,x]of Object.entries(r)){const w=c(v);if(!w||!x)continue;const I=v.replace(/[0-9]/g,""),T=le[I]||"#555",g=i==="attack"&&["MIL","ATT"].includes(I)&&!x.used||i==="defense"&&["GK","DEF","MIL"].includes(I)&&!x.used,b=a.includes(x.cardId);let A;i==="attack"?A=I==="MIL"?Number(x.note_m)||0:Number(x.note_a)||0:i==="defense"?A=I==="GK"?Number(x.note_g)||0:I==="MIL"?Number(x.note_m)||0:Number(x.note_d)||0:A=Number(I==="GK"?x.note_g:I==="DEF"?x.note_d:I==="MIL"?x.note_m:x.note_a)||0,A=A+(x.boost||0);const z=(w.x-u/2).toFixed(1),j=(w.y-f/2).toFixed(1),S=x.used?.25:1,D=h[x==null?void 0:x.rarity]||h.normal,N=b?"#ff3030":D,U=b?3:(x==null?void 0:x.rarity)==="légende"||(x==null?void 0:x.rarity)==="pépite"?2.5:2,Q=f-y-m;p+=`<defs><clipPath id="cp-${v}"><rect x="${z}" y="${(w.y-f/2+y).toFixed(1)}" width="${u}" height="${Q}"/></clipPath></defs>`,p+=`<rect x="${z}" y="${j}" width="${u}" height="${f}" rx="5" fill="${T}" opacity="${S}"/>`;const J=W(x);J&&!x.used&&(p+=`<image href="${J}" x="${z}" y="${(w.y-f/2+y).toFixed(1)}" width="${u}" height="${Q}" clip-path="url(#cp-${v})" preserveAspectRatio="xMidYMin slice"/>`),p+=`<rect x="${z}" y="${j}" width="${u}" height="${y}" rx="4" fill="rgba(255,255,255,0.92)"/>`,p+=`<text x="${w.x.toFixed(1)}" y="${(w.y-f/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(x.name||"").slice(0,9)}</text>`;const V=(w.y+f/2-m).toFixed(1);if(p+=`<rect x="${z}" y="${V}" width="${u}" height="${m}" fill="rgba(255,255,255,0.92)"/>`,x.used)p+=`<text x="${w.x.toFixed(1)}" y="${(w.y+f/2-m/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="13" font-weight="900" fill="rgba(0,0,0,0.4)" font-family="Arial Black">–</text>`;else{const me=Be(x.country_code);me?p+=`<image href="${me}" x="${(w.x-20).toFixed(1)}" y="${(w.y+f/2-m+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:p+=`<text x="${(w.x-13).toFixed(1)}" y="${(w.y+f/2-m/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${_t(x.country_code)}</text>`,p+=`<text x="${w.x.toFixed(1)}" y="${(w.y+f/2-m/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${A}</text>`;const ye=$t(x);ye?p+=`<image href="${ye}" x="${(w.x+u/2-14).toFixed(1)}" y="${(w.y+f/2-m+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:x.clubName&&(p+=`<text x="${(w.x+14).toFixed(1)}" y="${(w.y+f/2-m/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(x.clubName||"").slice(0,3).toUpperCase()}</text>`),x.boost&&(p+=`<rect x="${(w.x+u/2-12).toFixed(1)}" y="${(w.y-f/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,p+=`<text x="${(w.x+u/2-5).toFixed(1)}" y="${(w.y-f/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${x.boost}</text>`)}p+=`<rect x="${z}" y="${j}" width="${u}" height="${f}" rx="5" fill="${b?"rgba(255,255,255,0.12)":"none"}" stroke="${N}" stroke-width="${U}" opacity="${S}"/>`,g&&(p+=`<rect x="${z}" y="${j}" width="${u}" height="${f}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${b?"selected":""}" data-card-id="${x.cardId}" data-role="${I}" style="cursor:pointer"/>`)}const _=38;return`<svg viewBox="${-_} ${-_} ${d+_*2} ${o+_*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${p}
      ...
      <div style="flex:1;display:flex;gap:4px;overflow-x:auto;scrollbar-width:none">
        ${p.map(g=>{var b;return`
          <div class="gc-mini" data-gc-id="${g.id}" data-gc-type="${g.gc_type}"
            ...
            <div style="font-size:18px;line-height:1">${((b=ue[g.gc_type])==null?void 0:b.icon)||"⚡"}</div>
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(g=>{var b,A,z;if(g.type==="duel"){const j=g.isGoal,S=g.homeScored?"#FFD700":j?"#ff6b6b":"rgba(255,255,255,0.3)",D=g.homeScored?"⚽ BUT !":j?"⚽ BUT IA !":(b=g.homePlayers)!=null&&b.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${j?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${S};margin-bottom:4px">
                ...
                ${(z=g.aiPlayers)!=null&&z.length?`<div style="opacity:0.7">${be(g.aiPlayers,"#ff6b6b",g.aiTotal)}</div>`:""}
  ...
  `,document.body.appendChild(a);let c=!1;const p=()=>{c||(c=!0,a.remove(),i())};a.addEventListener("click",p),setTimeout(p,2e3)}function pe(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function ot(e,t,i,a=null){if(t.phase!=="attack"){pe("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){pe(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const d=Object.values(t.homeTeam).flat().filter(p=>p.used),o=t.homeSubs.filter(p=>!t.usedSubIds.includes(p.cardId));if(!d.length){pe("Aucun joueur utilisé à remplacer");return}if(!o.length){pe("Aucun remplaçant disponible");return}let s=0,l=Math.max(0,o.findIndex(p=>p.cardId===a)),r=!1;const n=document.createElement("div");n.id="sub-overlay",n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function c(){var _,v,x,w,I,T;const p=d[s],u=o[l],f=Math.min(130,Math.round((window.innerWidth-90)/2)),y=Math.round(f*1.35),m=g=>`background:rgba(255,255,255,0.12);border:none;color:${g?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${g?"default":"pointer"};flex-shrink:0`;n.innerHTML=`
    ...
    </div>`,(_=n.querySelector("#sub-close"))==null||_.addEventListener("click",()=>n.remove()),(v=n.querySelector("#out-up"))==null||v.addEventListener("click",()=>{s>0&&(s--,c())}),(x=n.querySelector("#out-down"))==null||x.addEventListener("click",()=>{s<d.length-1&&(s++,c())}),(w=n.querySelector("#in-up"))==null||w.addEventListener("click",()=>{l>0&&(l--,c())}),(I=n.querySelector("#in-down"))==null||I.addEventListener("click",()=>{l<o.length-1&&(l++,c())});const h=(g,b,A,z)=>{const j=n.querySelector("#"+g);if(!j)return;let S=0;j.addEventListener("touchstart",D=>{S=D.touches[0].clientY},{passive:!0}),j.addEventListener("touchend",D=>{const N=D.changedTouches[0].clientY-S;if(Math.abs(N)<30)return;const U=b();N<0&&U<z-1?(A(U+1),c()):N>0&&U>0&&(A(U-1),c())},{passive:!0})};h("in-panel",()=>l,g=>l=g,o.length),h("out-panel",()=>s,g=>s=g,d.length),(T=n.querySelector("#sub-confirm"))==null||T.addEventListener("click",g=>{if(g.preventDefault(),g.stopPropagation(),r)return;r=!0;const b=d[s],A=o[l];if(!b||!A)return;let z=null,j=-1;for(const[D,N]of Object.entries(t.homeTeam)){const U=(N||[]).findIndex(Q=>Q.cardId===b.cardId);if(U!==-1){z=D,j=U;break}}if(j===-1||!z){pe("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),n.remove();return}const S={...A,_line:z,_col:b._col||0,used:!1,boost:0};t.homeTeam[z].splice(j,1,S),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(A.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:b.name,firstname:b.firstname,note:H(b,z),portrait:W(b),job:b.job,country_code:b.country_code,rarity:b.rarity,clubName:b.clubName,clubLogo:b.clubLogo},inPlayer:{name:A.name,firstname:A.firstname,note:H(A,z),portrait:W(A),job:A.job,country_code:A.country_code,rarity:A.rarity,clubName:A.clubName,clubLogo:A.clubLogo},text:`🔄 ${A.firstname} ${A.name} remplace ${b.firstname} ${b.name}`}),n.remove(),Li(b,A,()=>K(e,t,i))})}document.body.appendChild(n),c()}function Mi(e,t,i,a,d){var l,r;const o=ue[t]||{icon:"⚡",desc:"Carte spéciale."},s=document.createElement("div");s.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;align-items:center;justify-content:center;padding:24px",s.innerHTML=`
  ...
  `;function l(r){const n=document.getElementById("mkt-content"),c=r==="buy"?s:o;if(c.length===0){n.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      ${c.map(p=>{var h,_,v;const u=(h=p.card)==null?void 0:h.player;if(!u)return"";const f=u.job==="GK"?u.note_g:u.job==="DEF"?u.note_d:u.job==="MIL"?u.note_m:u.note_a,y=Bi[u.rarity],m=(i.profile.credits||0)>=p.price;return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px">
