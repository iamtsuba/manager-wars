...
import{s as T,F as $i,i as Ut,k as Vi,l as nt,m as Ii,f as Nt,n as vo,T as li,j as wo,o as _o,b as $o}from"./formation-links-BZe0pGr4.js";const ko="/";function Mi(e,{navigate:t,toast:i}){let n="login";const d=()=>{var r,l,f,a,o,x;const s=n==="login";e.innerHTML=`
    ...
    </style>`,(r=document.getElementById("tab-login-btn"))==null||r.addEventListener("click",()=>{n="login",d()}),(l=document.getElementById("tab-reg-btn"))==null||l.addEventListener("click",()=>{n="register",d()}),s?((f=document.getElementById("login-password"))==null||f.addEventListener("keydown",u=>{var h;u.key==="Enter"&&((h=document.getElementById("login-btn"))==null||h.click())}),(a=document.getElementById("login-btn"))==null||a.addEventListener("click",async()=>{const u=document.getElementById("login-email").value.trim(),h=document.getElementById("login-password").value,$=document.getElementById("login-error");if($.textContent="",!u||!h){$.textContent="Remplissez tous les champs.";return}const m=document.getElementById("login-btn");m.textContent="⏳ Connexion…",m.disabled=!0;const{error:c}=await T.auth.signInWithPassword({email:u,password:h});if(m.textContent="⚽ Se connecter",m.disabled=!1,c){$.textContent=c.message.includes("Invalid")?"Email ou mot de passe incorrect.":c.message;return}window.location.reload()})):((o=document.getElementById("reg-confirm"))==null||o.addEventListener("keydown",u=>{var h;u.key==="Enter"&&((h=document.getElementById("reg-btn"))==null||h.click())}),(x=document.getElementById("reg-btn"))==null||x.addEventListener("click",async()=>{const u=document.getElementById("reg-email").value.trim(),h=document.getElementById("reg-password").value,$=document.getElementById("reg-confirm").value,m=document.getElementById("reg-error");if(m.textContent="",!u||!h||!$){m.textContent="Remplissez tous les champs.";return}if(h.length<6){m.textContent="Mot de passe trop court (min. 6 caractères).";return}if(h!==$){m.textContent="Les mots de passe ne correspondent pas.";return}const c=document.getElementById("reg-btn");c.textContent="⏳ Création…",c.disabled=!0;const{error:A}=await T.auth.signUp({email:u,password:h});if(c.textContent="🚀 Créer mon compte",c.disabled=!1,A){m.textContent=A.message;return}i("Compte créé ! Connecte-toi pour commencer.","success",4e3),n="login",d(),setTimeout(()=>{const p=document.getElementById("login-email");p&&(p.value=u)},50)}))};d()}function Eo(e,{state:t,navigate:i,toast:n,refreshProfile:d}){let s="#1A6B3C",r="#D4A017";e.innerHTML=`
  ...
  `;function l(){var $;const a=document.getElementById("logo-preview"),o=document.getElementById("logo-initials"),x=(($=document.getElementById("setup-club"))==null?void 0:$.value)||"MW",u=x.trim().split(" ").filter(Boolean),h=u.length>=2?(u[0][0]+u[1][0]).toUpperCase():x.slice(0,2).toUpperCase();a&&(a.style.background=r,a.style.borderColor=s),o&&(o.textContent=h,o.style.color=s)}document.getElementById("color1").addEventListener("input",a=>{s=a.target.value,document.getElementById("swatch1").style.background=s,l()}),document.getElementById("color2").addEventListener("input",a=>{r=a.target.value,document.getElementById("swatch2").style.background=r,l()});function f(a){document.querySelectorAll(".setup-step").forEach(o=>o.classList.remove("active")),document.getElementById(`step-${a}`).classList.add("active"),document.getElementById("step-num").textContent=a,document.getElementById("progress-fill").style.width=`${Math.round(a/3*100)}%`,a===3&&l()}document.getElementById("step1-next").addEventListener("click",async()=>{const a=document.getElementById("setup-pseudo").value.trim(),o=document.getElementById("step1-error");if(o.textContent="",a.length<3){o.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:x}=await T.from("users").select("id").eq("pseudo",a).maybeSingle();if(x){o.textContent="Ce pseudo est déjà pris.";return}f(2)}),document.getElementById("step2-back").addEventListener("click",()=>f(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const a=document.getElementById("setup-club").value.trim(),o=document.getElementById("step2-error");if(o.textContent="",a.length<2){o.textContent="Nom trop court (min. 2 caractères).";return}const{data:x}=await T.from("users").select("id").eq("club_name",a).maybeSingle();if(x){o.textContent="Ce nom de club est déjà pris.";return}f(3)}),document.getElementById("step3-back").addEventListener("click",()=>f(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const a=document.getElementById("setup-pseudo").value.trim(),o=document.getElementById("setup-club").value.trim(),x=document.getElementById("step3-error"),u=document.getElementById("step3-finish");x.textContent="",u.disabled=!0,u.textContent="Création en cours…";try{const{error:h}=await T.from("users").insert({id:t.user.id,pseudo:a,club_name:o,club_color1:s,club_color2:r,credits:1e4});if(h)throw h;await To(t.user.id),await d(),n(`Bienvenue ${a} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(h){x.textContent=h.message,u.disabled=!1,u.textContent="🚀 Créer mon profil !"}})}async function To(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await T.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const zo="modulepreload",So=function(e){return"/"+e},ji={},ni=function(t,i,n){let d=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const r=document.querySelector("meta[property=csp-nonce]"),l=(r==null?void 0:r.nonce)||(r==null?void 0:r.getAttribute("nonce"));d=Promise.allSettled(i.map(f=>{if(f=So(f),f in ji)return;ji[f]=!0;const a=f.endsWith(".css"),o=a?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${f}"]${o}`))return;const x=document.createElement("link");if(x.rel=a?"stylesheet":zo,a||(x.as="script"),x.crossOrigin="",x.href=f,l&&x.setAttribute("nonce",l),document.head.appendChild(x),a)return new Promise((u,h)=>{x.addEventListener("load",u),x.addEventListener("error",()=>h(new Error(`Unable to preload CSS for ${f}`)))})}))}function s(r){const l=new Event("vite:preloadError",{cancelable:!0});if(l.payload=r,window.dispatchEvent(l),!l.defaultPrevented)throw r}return d.then(r=>{for(const l of r||[])l.status==="rejected"&&s(l.reason);return t().catch(s)})},Rt="#1A6B3C",Ot="#cc2222",Ao="#D4A017",Ci="#888";async function Lo(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      ${u.map(({friendshipId:h,friend:$})=>Io($,h)).join("")}
    </div>`,a.querySelectorAll("[data-stats]").forEach(h=>{h.addEventListener("click",()=>jo(e,h.dataset.stats,h.dataset.friendName))}),a.querySelectorAll("[data-match]").forEach(h=>{h.addEventListener("click",()=>{const $=h.dataset.friendId,m=h.dataset.friendName;console.log("[Friends] clic match",{fid:$,fname:m,hasNavigate:typeof n}),typeof n=="function"?n("match",{matchMode:"friend",friendId:$,friendName:m}):t("Erreur navigation","error")})})}function Io(e,t){const i=e.club_name||e.pseudo||"?",n=e.pseudo||"",d=(n||i).slice(0,2).toUpperCase(),s=e.club_color2||Rt,r=e.club_color1||"#ffffff",l=e.last_seen_at?new Date(e.last_seen_at):null,f=l&&Date.now()-l.getTime()<3*60*1e3;return`
    ...
    <div class="popup-box">
      ...
      ${o.length?`<div style="display:flex;flex-direction:column;gap:8px;max-height:50vh;overflow-y:auto;margin-bottom:14px">
            ${o.map(u=>{var h,$,m;return`
              <div style="display:flex;align-items:center;gap:10px;background:#f9f9f9;border-radius:10px;padding:10px 12px">
                <div style="flex:1;font-size:14px;font-weight:700">${((h=u.requester)==null?void 0:h.club_name)||(($=u.requester)==null?void 0:$.pseudo)||"?"}
    ...
    </style>`}async function Ji({player1Id:e,player2Id:t,score1:i,score2:n,gc1:d,gc2:s}){const[r,l]=[e,t].sort(),f=e!==r,a=f?n:i,o=f?i:n,x=f?s:d,u=f?d:s,h=a>o?1:0,$=o>a?1:0,m=a===o?1:0,{data:c}=await T.from("friend_match_stats").select("*").eq("player1_id",r).eq("player2_id",l).single();c?await T.from("friend_match_stats").update({wins_p1:c.wins_p1+h,wins_p2:c.wins_p2+$,draws:c.draws+m,goals_p1:c.goals_p1+a,goals_p2:c.goals_p2+o,gc_used_p1:c.gc_used_p1+x,gc_used_p2:c.gc_used_p2+u,matches_total:c.matches_total+1}).eq("player1_id",r).eq("player2_id",l):await T.from("friend_match_stats").insert({player1_id:r,player2_id:l,wins_p1:h,wins_p2:$,draws:m,goals_p1:a,goals_p2:o,gc_used_p1:x,gc_used_p2:u,matches_total:1})}const Co="2026.07.07-2309";async function qi(e,{state:t,navigate:i,toast:n}){var s,r;const d=t.profile;d&&(e.innerHTML=`
  <style>
  ...
  <img src="${$}" style="position:absolute;inset:0;width:100%;height:100%;object-fit:fill" draggable="false">
  ...
  ${P?`<img src="${P}"
    ...
    width:${R}px;height:${D}px;object-fit:cover;object-position:top center;z-index:2"
  ...
  <div style="position:absolute;left:${ce}px;top:${se}px;width:${Ee}px;height:${ve}px;z-index:3;
    ...
    ${bt(e,{width:140})}
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),a=(l||[]).filter(H=>H.card_type==="player"&&H.player),o=(l||[]).filter(H=>H.card_type==="game_changer"),x=(l||[]).filter(H=>H.card_type==="formation"),u=(l||[]).filter(H=>H.card_type==="stadium"),{data:h}=await T.from("gc_definitions").select("name,gc_type,color,effect,image_url"),$={};(h||[]).forEach(H=>{$[H.name]=H});const{data:m}=await T.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),c={};(m||[]).forEach(H=>{c[H.id]=H}),u.forEach(H=>{H.stadium_def&&(c[H.stadium_id]=H.stadium_def)});const A=Object.keys($i),p=Object.keys(Oe),I={};a.forEach(H=>{const Z=H.player.id;I[Z]=(I[Z]||0)+1}),new Set(Object.keys(I).map(H=>String(H)));const q=new Set(x.map(H=>H.formation)),j=new Set(o.map(H=>H.gc_type));let P="player",C="Tous",N="",V=!1;function K(){return[...a].sort((H,Z)=>{const ge=Wt.indexOf(H.player.job),de=Wt.indexOf(Z.player.job);return ge!==de?ge-de:(H.player.surname_encoded||"").localeCompare(Z.player.surname_encoded||"")})}function ne(){return[...f||[]].sort((H,Z)=>{const ge=Wt.indexOf(H.job),de=Wt.indexOf(Z.job);return ge!==de?ge-de:(H.surname_encoded||"").localeCompare(Z.surname_encoded||"")})}function ie(){return K().filter(H=>{const Z=H.player,ge=C==="Tous"||Z.job===C,de=!N||`${Z.firstname} ${Z.surname_encoded}`.toLowerCase().includes(N);return ge&&de})}function Q(){return ne().filter(H=>{const Z=C==="Tous"||H.job===C,ge=!N||`${H.firstname} ${H.surname_encoded}`.toLowerCase().includes(N);return Z&&ge})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
  ...
  </div>`;function R(){const H=document.getElementById("col-filters");H&&(P==="player"?(H.innerHTML=`
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",Z=>{N=Z.target.value.toLowerCase(),D()}),e.querySelectorAll(".filter-btn").forEach(Z=>{Z.addEventListener("click",()=>{C=Z.dataset.job,R(),D()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{V=!V,R(),D()})):(H.innerHTML=`
        ...
        </div>`},null)}e.querySelectorAll(".col-tab-btn").forEach(H=>{H.addEventListener("click",()=>{P=H.dataset.tab,C="Tous",N="",V=!1,e.querySelectorAll(".col-tab-btn").forEach(Z=>{const ge=Z.dataset.tab===P;Z.style.borderBottomColor=ge?"var(--green)":"transparent",Z.style.color=ge?"var(--green)":"var(--gray-600)"}),R(),D()})}),R(),D()}function en(e,t,i){const n=Oe[e]||{icon:"⚡",desc:"Effet spécial."},d=(t==null?void 0:t.gc_type)==="ultra_game_changer",s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},r={purple:"#b06ce0",light_blue:"#00d4ef"},l=s[t==null?void 0:t.color]||s.purple,f=r[t==null?void 0:t.color]||r.purple,a=(t==null?void 0:t.name)||e,o=(t==null?void 0:t.effect)||n.desc,x=t!=null&&t.image_url?`/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),($=document.getElementById("direct-sell-form-btn"))==null||$.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${a} pour ${Jt.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const A=u.find(I=>!I.is_for_sale)||u[0];if(!A){s("Aucune carte à vendre","error");return}await T.from("market_listings").delete().eq("card_id",A.id),await T.from("transfer_history").delete().eq("card_id",A.id);const{error:p}=await T.from("cards").delete().eq("id",A.id);if(p){s(p.message,"error");return}await T.from("users").update({credits:(d.profile.credits||0)+Jt}).eq("id",d.profile.id),await f(),s(`+${Jt.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),r(),l("collection")}),(m=document.getElementById("market-sell-form-btn"))==null||m.addEventListener("click",async()=>{const A=parseInt(document.getElementById("sell-price-form").value);if(!A||A<1){s("Prix invalide","error");return}await T.from("cards").update({is_for_sale:!0,sale_price:A}).eq("id",e.id),await T.from("market_listings").insert({seller_id:d.profile.id,card_id:e.id,price:A}),s("Carte mise en vente sur le marché !","success"),r(),l("collection")}),(c=document.getElementById("cancel-sell-form-btn"))==null||c.addEventListener("click",async()=>{await T.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await T.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),s("Annonce retirée","success"),r(),l("collection")})}async function Ni(e,t,i,n){var xe,_e,we,ve,Ee;const{state:d,toast:s,openModal:r,closeModal:l,navigate:f,refreshProfile:a}=n,o=e.player,x=t.filter(se=>se.player.id===o.id),u=x.length,h=e.evolution_bonus||0,$=Math.max((Number(o.note_g)||0)+(o.job==="GK"||o.job2==="GK"?h:0),(Number(o.note_d)||0)+(o.job==="DEF"||o.job2==="DEF"?h:0),(Number(o.note_m)||0)+(o.job==="MIL"||o.job2==="MIL"?h:0),(Number(o.note_a)||0)+(o.job==="ATT"||o.job2==="ATT"?h:0)),m=o.rarity||"normal",{data:c}=await T.from("sell_price_configs").select("*").eq("rarity",m).lte("note_min",$).gte("note_max",$).order("note_min",{ascending:!1}).limit(1);((xe=c==null?void 0:c[0])==null?void 0:xe.price)??Yo[m];const A=o.rarity!=="legende";Qo(o);const p=((o.rarity==="pepite"||o.rarity==="papyte")&&e.current_note!=null?e.current_note:pi(o,o.job))+h,I=o.rarity==="pepite"||o.rarity==="papyte",q=o.job2?(I?io(e,Jo(o.job2)):pi(o,o.job2))+(pi(o,o.job2)>0?h:0):null;ci[o.job],o.job2&&ci[o.job2];const j=Ko[o.rarity]||"#ccc";Xo[o.country_code]||o.country_code;const P=e.evolution_bonus||0,N=p+P,V=q||0,K=V>0?V+P:0,ne=x.map(se=>se.id);let ie={};if(ne.length){const{data:se}=await T.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",ne).order("transferred_at",{ascending:!0});(se||[]).forEach(pe=>{ie[pe.card_id]||(ie[pe.card_id]=[]),ie[pe.card_id].push(pe)})}const Q=se=>{const pe=se.transferred_at?new Date(se.transferred_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"2-digit",year:"numeric"}):"",ce=se.source==="booster"?"Booster":se.price?se.price.toLocaleString("fr"):"—";return`
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(_e=document.getElementById("close-detail"))==null||_e.addEventListener("click",l);let D=new Set;const re=()=>{const se=D.size,pe=document.getElementById("sell-action-panel");if(!pe)return;pe.style.display=se>0?"block":"none",document.getElementById("sell-selected-count").textContent=se;const ce=document.getElementById("evolve-btn");ce&&(ce.textContent=`⬆️ Évoluer ${se>1?"(+"+se+")":""}`)};document.querySelectorAll(".expl-check").forEach(se=>{se.addEventListener("change",()=>{const pe=se.dataset.id;se.checked?D.add(pe):D.delete(pe);const ce=se.closest(".exemplaire-row");ce&&(ce.style.borderColor=se.checked?"#1A6B3C":"#eee"),re()})}),document.querySelectorAll(".exemplaire-row").forEach(se=>{se.addEventListener("click",pe=>{if(pe.target.closest("button")||pe.target.tagName==="INPUT")return;const ce=se.querySelector(".expl-check");ce&&!ce.disabled&&(ce.checked=!ce.checked,ce.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(se=>{se.addEventListener("click",pe=>{pe.stopPropagation();const ce=document.querySelector(`.expl-hist[data-hist="${se.dataset.idx}"]`);ce&&(ce.style.display=ce.style.display==="none"?"flex":"none")})}),(we=document.getElementById("evolve-btn"))==null||we.addEventListener("click",async()=>{if(u<=1)return;const se=[...D];if(!se.length)return;if(D.has(e.id)){const me=document.createElement("div");me.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",me.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;padding-bottom:80px;background:var(--page-bg)">
    ...
    <div class="page-body">
      ...
      stadium_def:stadium_definitions(id, name, club_id, country_code, image_url,
        club:clubs(encoded_name, logo_url))`).eq("owner_id",n.profile.id).eq("card_type","stadium"),f=(r||[]).filter(A=>A.card_type==="player"&&A.player),a=(r||[]).filter(A=>A.card_type==="formation"),o=(l||[]).filter(A=>A.card_type==="stadium"),x=[...new Set(o.map(A=>A.stadium_id).filter(Boolean))];let u={};if(o.forEach(A=>{A.stadium_def&&A.stadium_id&&(u[A.stadium_id]=A.stadium_def)}),x.length&&Object.keys(u).length<x.length){const{data:A}=await T.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",x);(A||[]).forEach(p=>{u[p.id]=p})}const h=a.map(A=>A.formation).filter(Boolean),{data:$}=await T.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",e);let m=s.formation||"4-4-2";h.length>0&&!h.includes(m)&&(m=h[0]);const c={deckId:e,name:s.name,formation:m,formationCardId:s.formation_card_id,stadiumCardId:s.stadium_card_id||null,slots:{},subs:[],playerCards:f,formationCards:a,stadiumCards:o,stadDefMap:u,availableFormations:h};($||[]).forEach(A=>{A.is_starter?c.slots[A.position]=A.card_id:c.subs.includes(A.card_id)||c.subs.push(A.card_id)}),ht(t,c,i)}function ht(e,t,i){var $,m,c,A;const{navigate:n}=i;ei[t.formation];const d=Oi(t.formation),s=d.filter(p=>t.slots[p]).length,r=t.availableFormations.length>0?t.availableFormations:Object.keys(ei),l=($=t.stadiumCards)==null?void 0:$.find(p=>p.id===t.stadiumCardId),f=l&&((m=t.stadDefMap)==null?void 0:m[l.stadium_id])||null,a=t.subs.map(p=>t.playerCards.find(I=>I.id===p)).filter(Boolean);[...Object.values(t.slots),...t.subs],e.innerHTML=`
  <div style="height:100%;overflow-y:auto;padding-bottom:80px;background:var(--page-bg)">
    ...
    <div style="position:relative;width:${h}px;height:${$}px;margin:0 auto">
      <svg style="position:absolute;inset:0;width:100%;height:100%;pointer-events:none" viewBox="0 0 ${h} ${$}">${c}</svg>
      ${A}
  ...
  <div class="page">
    ...
    <div class="page-body">
      <div class="booster-grid">
        ${s.map($=>{const m=$.cost===0||d>=$.cost,c=o[$.id]===!0;return`<div class="booster-card ${!m||c?"disabled":""}" data-booster="${$.id}" style="position:relative">
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      <div style="flex:1;overflow-y:auto;display:flex;flex-wrap:wrap;gap:10px;justify-content:center;align-content:flex-start;padding:8px 16px 16px">
        ${s.map($=>{const m=r.find(c=>c.gc_type===$.gc_type);return l($,!!m)}).join("")}
      </div>
      ...
      card:cards(id,card_type,formation,stadium_id,evolution_bonus,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
          clubs(encoded_name,logo_url)))`).in("deck_id",r).order("slot_order"),f=[...new Set((l||[]).filter(u=>{var h,$;return((h=u.card)==null?void 0:h.card_type)==="stadium"&&(($=u.card)==null?void 0:$.stadium_id)}).map(u=>u.card.stadium_id))],a={};if(f.length){const{data:u}=await T.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",f);(u||[]).forEach(h=>{a[h.id]=h}),(l||[]).forEach(h=>{var $,m;(($=h.card)==null?void 0:$.card_type)==="stadium"&&((m=h.card)!=null&&m.stadium_id)&&(h.card._stadiumDef=a[h.card.stadium_id]||null)})}let o=0;function x(){var j,P,C,N,V,K,ne;const u=s[o],h=(l||[]).filter(ie=>ie.deck_id===u.id),$=h.filter(ie=>{var Q;return ie.is_starter&&((Q=ie.card)==null?void 0:Q.player)}).map(ie=>hi(ie.card,ie.position)),m=h.find(ie=>{var Q;return((Q=ie.card)==null?void 0:Q.card_type)==="formation"}),c=u.formation||((j=m==null?void 0:m.card)==null?void 0:j.formation)||"4-4-2";let A=$.length>=11?vt($,c):null,p=((P=u.stadium_card)==null?void 0:P.stadium_def)||null;p&&A&&(A=Tt(A,p));const I=$.length>=11;et(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div style="display:flex;align-items:center;gap:8px;padding:8px;flex-shrink:0">
        ...
        <div style="flex:1;text-align:center">
          ...
          <div style="font-size:11px;opacity:.6;margin-top:2px">${c} · ${$.length}/11 ${u.is_active?"· ⭐ Actif":""}</div>
        </div>
    ...
    </div>`,function(){const Q=e.querySelector(".deck-preview-wrap svg");Q&&(Q.removeAttribute("width"),Q.removeAttribute("height"),Q.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",Q.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),(N=document.getElementById("prev-deck"))==null||N.addEventListener("click",()=>{o>0&&(o--,x())}),(V=document.getElementById("next-deck"))==null||V.addEventListener("click",()=>{o<s.length-1&&(o++,x())}),(K=document.getElementById("validate-deck"))==null||K.addEventListener("click",()=>{if(!I)return;const ie=t.state.params||{};t.navigate("match",{...ie,matchMode:ie.matchMode||i,deckId:u.id})}),(ne=document.getElementById("cancel-deck-select"))==null||ne.addEventListener("click",()=>{Ve(e),d("home")});const q=document.getElementById("deck-swipe-zone");if(q){let ie=0,Q=0;q.addEventListener("touchstart",R=>{ie=R.touches[0].clientX,Q=R.touches[0].clientY},{passive:!0}),q.addEventListener("touchend",R=>{const D=R.changedTouches[0].clientX-ie,re=R.changedTouches[0].clientY-Q;Math.abs(D)<40||Math.abs(D)<Math.abs(re)||(D<0&&o<s.length-1?(o++,x()):D>0&&o>0&&(o--,x()))},{passive:!0})}}x()}function kn(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";return e!=null&&e.clubLogo?e.clubLogo.startsWith("http")?e.clubLogo:`${t}/storage/v1/object/public/assets/clubs/${e.clubLogo}`:null}function Xe(e,t=44,i=58,n=null){return bt(e,{width:t,showStad:!!n,stadDef:n,used:e==null?void 0:e.used})}function it(e,t,i){if(!(e!=null&&e.length))return"";const n=e.slice(0,5);let d='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return n.forEach((s,r)=>{if(d+=Xe(s,40,52),r<n.length-1){const l=nt(s,n[r+1]);d+=`<div style="width:7px;height:3px;background:${l==="#ff3333"||l==="#cc2222"?"rgba(255,255,255,0.12)":l};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(d+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),d+="</div>",d}function En(e){return!e||e.length<2?null:`https://flagsapi.com/${e.slice(0,2).toUpperCase()}/flat/64.png`}function Tn(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function lt(e,t,i,n,d=310,s=310,r=[]){var I;const l=Ut[t]||{},f=Vi(t)||$i[t]||[],a={},o=["ATT","MIL","DEF","GK"];for(const q of o)(e[q]||[]).forEach((P,C)=>{a[`${q}${C+1}`]=P});function x(q){const j=l[q];return j?{x:j.x*d,y:j.y*s}:null}let u="";for(const[q,j]of f){const P=x(q),C=x(j);if(!P||!C)continue;const N=a[q],V=a[j],K=nt(N,V);K==="#00ff88"||K==="#FFD700"?(u+=`<line x1="${P.x.toFixed(1)}" y1="${P.y.toFixed(1)}" x2="${C.x.toFixed(1)}" y2="${C.y.toFixed(1)}"
        stroke="${K}" stroke-width="10" stroke-linecap="round" opacity="0.22"/>`,u+=`<line x1="${P.x.toFixed(1)}" y1="${P.y.toFixed(1)}" x2="${C.x.toFixed(1)}" y2="${C.y.toFixed(1)}"
        stroke="${K}" stroke-width="3.5" stroke-linecap="round" opacity="0.95"/>`):u+=`<line x1="${P.x.toFixed(1)}" y1="${P.y.toFixed(1)}" x2="${C.x.toFixed(1)}" y2="${C.y.toFixed(1)}"
        stroke="${K}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const h=48,$=64,m=13,c=16,A={normal:"#aaaaaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#111111",legende:"#7a28b8",légende:"#7a28b8"};for(const[q,j]of Object.entries(a)){const P=x(q);if(!P||!j)continue;const C=q.replace(/[0-9]/g,""),N=Ht[C]||"#555",V=r.includes(j.cardId),K=i==="attack"&&(["MIL","ATT"].includes(C)||V)&&!j.used||i==="defense"&&["GK","DEF","MIL"].includes(C)&&!j.used,ne=n.includes(j.cardId);let ie;i==="attack"?ie=V?Math.max(1,Number(j.note_a)||0):C==="MIL"?Number(j.note_m)||0:Number(j.note_a)||0:i==="defense"?ie=C==="GK"?Number(j.note_g)||0:C==="MIL"?Number(j.note_m)||0:Number(j.note_d)||0:ie=Number(C==="GK"?j.note_g:C==="DEF"?j.note_d:C==="MIL"?j.note_m:j.note_a)||0,ie=ie+(j.boost||0),j.stadiumBonus&&(i==="attack"&&(C==="ATT"||C==="MIL")||i==="defense"&&(C==="GK"||C==="DEF"||C==="MIL")?ie+=10:i||(ie+=10));const Q=(P.x-h/2).toFixed(1),R=(P.y-$/2).toFixed(1),D=A[j==null?void 0:j.rarity]||A.normal;if(j.used){const se=typeof import.meta<"u"&&"/"||"/",ce=`${typeof window<"u"&&((I=window.location)==null?void 0:I.origin)||""}${se}icons/carte-dos.png`.replace(/([^:])\/\//g,"$1/");u+=`<rect x="${Q}" y="${R}" width="${h}" height="${$}" rx="5" fill="#161616"/>`,u+=`<image href="${ce}" xlink:href="${ce}" x="${Q}" y="${R}" width="${h}" height="${$}" preserveAspectRatio="xMidYMid slice"/>`,u+=`<rect x="${Q}" y="${R}" width="${h}" height="${$}" rx="5" fill="none" stroke="${D}" stroke-width="2" opacity="0.7"/>`,u+=`<rect x="${Q}" y="${R}" width="${h}" height="${$}" rx="5" fill="rgba(0,0,0,0.01)" class="match-used-hit" data-card-id="${j.cardId}" data-role="${C}" style="cursor:pointer"/>`;continue}const re=ne?.45:1,xe=ne?"#FFD700":D,_e=ne?3:(j==null?void 0:j.rarity)==="legende"||(j==null?void 0:j.rarity)==="légende"||(j==null?void 0:j.rarity)==="pepite"||(j==null?void 0:j.rarity)==="pépite"?2.5:2,we=$-m-c;u+=`<defs><clipPath id="cp-${q}"><rect x="${Q}" y="${(P.y-$/2+m).toFixed(1)}" width="${h}" height="${we}"/></clipPath></defs>`,u+=`<rect x="${Q}" y="${R}" width="${h}" height="${$}" rx="5" fill="${N}" opacity="${re}"/>`;const ve=He(j);ve&&(u+=`<image href="${ve}" xlink:href="${ve}" x="${Q}" y="${(P.y-$/2+m).toFixed(1)}" width="${h}" height="${we}" clip-path="url(#cp-${q})" preserveAspectRatio="xMidYMin slice"/>`),u+=`<rect x="${Q}" y="${R}" width="${h}" height="${m}" rx="4" fill="rgba(255,255,255,0.92)"/>`,u+=`<text x="${P.x.toFixed(1)}" y="${(P.y-$/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(j.name||"").slice(0,9)}</text>`;const Ee=(P.y+$/2-c).toFixed(1);u+=`<rect x="${Q}" y="${Ee}" width="${h}" height="${c}" fill="rgba(255,255,255,0.92)"/>`;{const se=En(j.country_code);se?u+=`<image href="${se}" xlink:href="${se}" x="${(P.x-20).toFixed(1)}" y="${(P.y+$/2-c+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:u+=`<text x="${(P.x-13).toFixed(1)}" y="${(P.y+$/2-c/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${Tn(j.country_code)}</text>`;const pe=j.stadiumBonus?"#E87722":"#111";u+=`<text x="${P.x.toFixed(1)}" y="${(P.y+$/2-c/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="${pe}" font-family="Arial Black">${ie}</text>`;const ce=kn(j);ce?u+=`<image href="${ce}" xlink:href="${ce}" x="${(P.x+h/2-14).toFixed(1)}" y="${(P.y+$/2-c+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:j.clubName&&(u+=`<text x="${(P.x+14).toFixed(1)}" y="${(P.y+$/2-c/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(j.clubName||"").slice(0,3).toUpperCase()}</text>`),j.stadiumBonus&&(u+=`<rect x="${(P.x-h/2).toFixed(1)}" y="${(P.y-$/2).toFixed(1)}" width="${h}" height="${$}" rx="5" fill="none" stroke="#E87722" stroke-width="2" opacity="0.8"/>`)}u+=`<rect x="${Q}" y="${R}" width="${h}" height="${$}" rx="5" fill="${ne?"rgba(255,255,255,0.12)":"none"}" stroke="${xe}" stroke-width="${_e}" opacity="${re}"/>`,K&&(u+=`<rect x="${Q}" y="${R}" width="${h}" height="${$}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${ne?"selected":""}" data-card-id="${j.cardId}" data-role="${C}" style="cursor:pointer"/>`)}const p=38;return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-p} ${-p} ${d+p*2} ${s+p*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${u}
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    ${(()=>{const D=window.innerWidth>=700,re=(H,Z,ge)=>{var B,_;const de=(t.gcDefs||[]).find(g=>g.name===H.gc_type),me={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[de==null?void 0:de.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",oe={purple:"#b06ce0",light_blue:"#00d4ef"}[de==null?void 0:de.color]||"#b06ce0",ue=(de==null?void 0:de.name)||H.gc_type,Le=(de==null?void 0:de.effect)||((B=Oe[H.gc_type])==null?void 0:B.desc)||"",Ie=de!=null&&de.image_url?`/icons/${de.image_url}`:null,Ce=((_=Oe[H.gc_type])==null?void 0:_.icon)||"⚡",Te=Math.round(ge*.22),qe=Math.round(ge*.22),Pe=ge-Te-qe,ze=ue.length>12?7:9;return`<div class="gc-mini" data-gc-id="${H.id}" data-gc-type="${H.gc_type}"
  ...
  `,document.body.appendChild(n);let o=!1;const x=()=>{o||(o=!0,n.remove(),i())};n.addEventListener("click",x),setTimeout(x,2e3)}function qt(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function fi(e,t,i,n=null,d=null){var h,$;if(t.phase!=="attack"){qt("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){qt(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const s=Object.entries(t.homeTeam).flatMap(([m,c])=>(c||[]).filter(A=>A.used).map(A=>({...A,_line:A._line||m}))),r=t.homeSubs.filter(m=>!t.usedSubIds.includes(m.cardId));if(!s.length){qt("Aucun joueur utilisé à remplacer");return}if(!r.length){qt("Aucun remplaçant disponible");return}let l=Math.max(0,s.findIndex(m=>m.cardId===d));const f=((h=s[l])==null?void 0:h._line)||(($=s[l])==null?void 0:$.job);let a=n?Math.max(0,r.findIndex(m=>m.cardId===n)):Math.max(0,r.findIndex(m=>m.job===f)),o=!1;const x=document.createElement("div");x.id="sub-overlay",x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function u(){var j,P,C,N,V,K;const m=s[l],c=r[a],A=Math.min(130,Math.round((window.innerWidth-90)/2)),p=Math.round(A*1.35),I=ne=>`background:rgba(255,255,255,0.12);border:none;color:${ne?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${ne?"default":"pointer"};flex-shrink:0`;x.innerHTML=`
    ...
    </div>`,(j=x.querySelector("#sub-close"))==null||j.addEventListener("click",()=>x.remove()),(P=x.querySelector("#out-up"))==null||P.addEventListener("click",()=>{l>0&&(l--,u())}),(C=x.querySelector("#out-down"))==null||C.addEventListener("click",()=>{l<s.length-1&&(l++,u())}),(N=x.querySelector("#in-up"))==null||N.addEventListener("click",()=>{a>0&&(a--,u())}),(V=x.querySelector("#in-down"))==null||V.addEventListener("click",()=>{a<r.length-1&&(a++,u())});const q=(ne,ie,Q,R)=>{const D=x.querySelector("#"+ne);if(!D)return;let re=0;D.addEventListener("touchstart",xe=>{re=xe.touches[0].clientY},{passive:!0}),D.addEventListener("touchend",xe=>{const _e=xe.changedTouches[0].clientY-re;if(Math.abs(_e)<30)return;const we=ie();_e<0&&we<R-1?(Q(we+1),u()):_e>0&&we>0&&(Q(we-1),u())},{passive:!0})};q("in-panel",()=>a,ne=>a=ne,r.length),q("out-panel",()=>l,ne=>l=ne,s.length),(K=x.querySelector("#sub-confirm"))==null||K.addEventListener("click",ne=>{if(ne.preventDefault(),ne.stopPropagation(),o)return;o=!0;const ie=s[l],Q=r[a];if(!ie||!Q)return;let R=null,D=-1;for(const[xe,_e]of Object.entries(t.homeTeam)){const we=(_e||[]).findIndex(ve=>ve.cardId===ie.cardId);if(we!==-1){R=xe,D=we;break}}if(D===-1||!R){qt("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),x.remove();return}const re={...Q,_line:R,_col:ie._col||0,used:!1,boost:0};t.homeTeam[R].splice(D,1,re),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(Q.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:ie.name,firstname:ie.firstname,note:Se(ie,R),portrait:He(ie),job:ie.job,country_code:ie.country_code,rarity:ie.rarity,clubName:ie.clubName,clubLogo:ie.clubLogo},inPlayer:{name:Q.name,firstname:Q.firstname,note:Se(Q,R),portrait:He(Q),job:Q.job,country_code:Q.country_code,rarity:Q.rarity,clubName:Q.clubName,clubLogo:Q.clubLogo},text:`🔄 ${Q.firstname} ${Q.name} remplace ${ie.firstname} ${ie.name}`}),x.remove(),Pn(ie,Q,()=>Je(e,t,i))})}document.body.appendChild(x),u()}function Gn(e,t,i,n,d){var $,m;const s=(n.gcDefs||[]).find(c=>c.name===t),r=Oe[t]||{icon:"⚡",desc:"Carte spéciale."},l={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[s==null?void 0:s.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",f={purple:"#b06ce0",light_blue:"#00d4ef"}[s==null?void 0:s.color]||"#b06ce0",a=(s==null?void 0:s.name)||t,o=(s==null?void 0:s.effect)||r.desc,x=s!=null&&s.image_url?`/icons/${s.image_url}`:null,u=r.icon||"⚡",h=document.createElement("div");h.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",h.innerHTML=`
    ...
    <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
      ${e.map(x=>{const u=x._line||x.job||"MIL",h={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[u]||"#555",$=Se(x,u)+(x.boost||0),m=l.find(c=>c.cardId===x.cardId);return`<div class="gc-pick-item" data-cid="${x.cardId}"
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(s);let l=!1;const f=()=>{l||(l=!0,s.remove(),setTimeout(()=>d(),50))};s.addEventListener("click",f),setTimeout(f,3500)}async function Pt(e,t,i){var o,x;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:n}=i,d=t.homeScore>t.aiScore,s=t.homeScore===t.aiScore,r=d?"victoire":s?"nul":"defaite",l=Ho(t.mode,r);t.matchId&&await T.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:d?n.profile.id:null,home_credits_reward:l,played_at:new Date().toISOString()}).eq("id",t.matchId);const f={credits:(n.profile.credits||0)+l,matches_played:(n.profile.matches_played||0)+1};d?f.wins=(n.profile.wins||0)+1:s?f.draws=(n.profile.draws||0)+1:f.losses=(n.profile.losses||0)+1,await T.from("users").update(f).eq("id",n.profile.id),await i.refreshProfile();const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",a.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const q=async(N=!0)=>{$=!0,m&&(T.removeChannel(m),m=null),N&&await T.rpc("cancel_matchmaking",{p_user_id:x.profile.id}).catch(()=>{})};(C=document.getElementById("mm-cancel"))==null||C.addEventListener("click",async()=>{try{await q(!0)}catch{}Ve(e),u("home")});const j=async(N,V,K)=>{if($)return;$=!0,m&&(T.removeChannel(m),m=null);const ne=document.getElementById("mm-status");ne&&(ne.textContent="Adversaire trouvé !"),await new Promise(ie=>setTimeout(ie,600)),e.isConnected&&go(e,t,N,K,r,l,a,o,f)};if(a){const N=[{range:50,delay:0},{range:100,delay:15e3},{range:200,delay:3e4},{range:400,delay:45e3},{range:800,delay:6e4}];let V=0,K=!1;const ne=async()=>{if($||K)return;const{range:ie}=N[V],Q=document.getElementById("mm-range");Q&&(Q.textContent=`Plage MMR : ±${ie}`);const{data:R,error:D}=await T.rpc("try_matchmake_ranked",{p_user_id:x.profile.id,p_deck_id:i,p_mmr:c,p_range:ie});if(console.log("[Ranked] try_matchmake_ranked result:",R,"error:",JSON.stringify(D)),$)return;if(D||!(R!=null&&R.success)){h("Erreur matchmaking ranked","error"),Ve(e),u("home");return}if(R.matched){K=!0,j(R.match_id,R.opponent_id,!1);return}const re=document.getElementById("mm-status");if(re&&(re.textContent="En attente d'un adversaire..."),m||(m=T.channel("matchmaking-"+x.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${x.profile.id}`},xe=>{const _e=xe.new;_e.status==="matched"&&_e.match_id&&!K&&(K=!0,j(_e.match_id,_e.matched_with,!0))}).subscribe()),V<N.length-1){const xe=N[V+1].delay-N[V].delay;V++,setTimeout(()=>{!$&&!K&&ne()},xe)}};await ne()}else{const{data:N,error:V}=await T.rpc("try_matchmake",{p_user_id:x.profile.id,p_deck_id:i});if(V||!(N!=null&&N.success)){h("Erreur de matchmaking","error"),Ve(e),u("home");return}if(N.matched){j(N.match_id,N.opponent_id,!1);return}const K=document.getElementById("mm-status");K&&(K.textContent="En attente d'un autre joueur..."),m=T.channel("matchmaking-"+x.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${x.profile.id}`},ne=>{const ie=ne.new;ie.status==="matched"&&ie.match_id&&j(ie.match_id,ie.matched_with,!0)}).subscribe()}}async function Kn(e,t,i){const{state:n,navigate:d,toast:s}=t;try{(T.getChannels?T.getChannels():[]).forEach(a=>{const o=a.topic||"";(o.includes("matchmaking")||o.includes("pvp-match")||o.includes("friend-invite"))&&T.removeChannel(a)})}catch{}const{data:r}=await T.from("matches").select("home_id, away_id, status").eq("id",i).single();if(!r){s("Match introuvable","error"),d("home");return}if(r.status==="finished"){s("Ce match est terminé","info"),d("home");return}const l=r.home_id===n.profile.id;go(e,t,i,l,[],[])}async function go(e,t,i,n,d=[],s=[],r=!1,l=null,f=null){const{state:a,navigate:o,toast:x}=t,u=n?"p1":"p2",h=n?"p2":"p1",$=(d||[]).map(_=>_.id),m=(d||[]).map(_=>({id:_.id,gc_type:_.gc_type,_gcDef:_._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:c}=await T.from("matches").select("*").eq("id",i).single();if(!c){x("Match introuvable","error"),o("home");return}async function A(){var W,te;console.log("[PvP] buildGameState match:",{id:c.id,home_deck_id:c.home_deck_id,away_deck_id:c.away_deck_id,mode:c.mode,is_ranked:c.is_ranked});const[{data:_,error:g},{data:y,error:b},{data:v},{data:w}]=await Promise.all([T.rpc("get_deck_for_match",{p_deck_id:c.home_deck_id}),T.rpc("get_deck_for_match",{p_deck_id:c.away_deck_id}),T.from("users").select("id,pseudo,club_name").eq("id",c.home_id).single(),T.from("users").select("id,pseudo,club_name").eq("id",c.away_id).single()]);console.log("[PvP] get_deck_for_match p1:",g==null?void 0:g.message,"p2:",b==null?void 0:b.message,"p1D:",(W=_==null?void 0:_.starters)==null?void 0:W.length,"p2D:",(te=y==null?void 0:y.starters)==null?void 0:te.length);const k=ee=>{const fe=Number(ee.evolution_bonus)||0;return{cardId:ee.card_id,position:ee.position,id:ee.id,firstname:ee.firstname,name:ee.surname_encoded,country_code:ee.country_code,club_id:ee.club_id,job:ee.job,job2:ee.job2,note_g:(Number(ee.note_g)||0)+(ee.job==="GK"||ee.job2==="GK"&&Number(ee.note_g)>0?fe:0),note_d:(Number(ee.note_d)||0)+(ee.job==="DEF"||ee.job2==="DEF"&&Number(ee.note_d)>0?fe:0),note_m:(Number(ee.note_m)||0)+(ee.job==="MIL"||ee.job2==="MIL"&&Number(ee.note_m)>0?fe:0),note_a:(Number(ee.note_a)||0)+(ee.job==="ATT"||ee.job2==="ATT"&&Number(ee.note_a)>0?fe:0),evolution_bonus:fe,rarity:ee.rarity,skin:ee.skin,hair:ee.hair,hair_length:ee.hair_length,clubName:ee.club_encoded_name||null,clubLogo:ee.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},z=((_==null?void 0:_.starters)||[]).map(ee=>k(ee)),E=((y==null?void 0:y.starters)||[]).map(ee=>k(ee)),L=(_==null?void 0:_.formation)||"4-4-2",S=(y==null?void 0:y.formation)||"4-4-2";let M=vt(z,L),F=vt(E,S);const Y=((_==null?void 0:_.subs)||[]).map(ee=>k(ee)),J=((y==null?void 0:y.subs)||[]).map(ee=>k(ee)),U=(_==null?void 0:_.stadium_def)||(n?f:null),G=(y==null?void 0:y.stadium_def)||(n?null:f);return U&&(M=Tt(M,U),jt(Y,U)),G&&(F=Tt(F,G),jt(J,G)),{p1Team:M,p2Team:F,p1Subs:Y,p2Subs:J,p1Formation:L,p2Formation:S,p1Name:(v==null?void 0:v.club_name)||(v==null?void 0:v.pseudo)||"Joueur 1",p2Name:(w==null?void 0:w.club_name)||(w==null?void 0:w.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:n?$:[],gc_p2:n?[]:$,gcCardsFull_p1:n?m:[],gcCardsFull_p2:n?[]:m,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:s||[],lastActionAt:new Date().toISOString()}}let p=c.game_state&&Object.keys(c.game_state).length?c.game_state:null;if(console.log("[PvP] init - amIHome:",n,"gameState exists:",!!p,"match.status:",c.status,"home_id:",c.home_id,"away_id:",c.away_id,"myId:",a.profile.id),!p)if(n){p=await A(),console.log("[PvP] home - gameState built, p1Team keys:",Object.keys((p==null?void 0:p.p1Team)||{}));const{data:_}=await T.from("matches").select("game_state").eq("id",i).single();!(_!=null&&_.game_state)||!Object.keys(_.game_state).length?await T.from("matches").update({game_state:p,turn_user_id:c.home_id}).eq("id",i):p=_.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>',console.log("[PvP] away - waiting for home to write game_state...");for(let _=0;_<30&&!p;_++){await new Promise(y=>setTimeout(y,400));const{data:g}=await T.from("matches").select("game_state").eq("id",i).single();g!=null&&g.game_state&&Object.keys(g.game_state).length&&(p=g.game_state),_%5===0&&console.log("[PvP] away - poll",_,"game_state:",!!(g!=null&&g.game_state))}if(!p){x("Erreur de synchronisation","error"),o("home");return}p.gc_p2=$,p.gcCardsFull_p2=m,await T.from("matches").update({game_state:p}).eq("id",i)}let I=!1,q=null,j=!1;const P=new Set,C=new Set;async function N(_){var S,M;try{T.removeChannel(V)}catch{}const g=p[u+"Score"]||0,y=p[h+"Score"]||0;let b,v;_.winner_id?(b=_.winner_id===a.profile.id,v=!1):_.forfeit?(b=g>y,v=!1):(v=g===y,b=g>y);let w="";if(r&&n)try{const{data:F}=await T.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",c.home_id).single(),{data:Y}=await T.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",c.away_id).single();if(F&&Y){const J=b?1:v?.5:0,U=1-J,G=F.placement_matches<10,W=Y.placement_matches<10,te=Ii(F.mmr,F.mmr_deviation,F.mmr_volatility,Y.mmr,Y.mmr_deviation,J===1?1:J===0?0:.5,G),ee=Ii(Y.mmr,Y.mmr_deviation,Y.mmr_volatility,F.mmr,F.mmr_deviation,U===1?1:U===0?0:.5,W);await T.rpc("update_mmr_after_ranked",{p_match_id:i,p_winner_id:v?null:b?c.home_id:c.away_id,p_home_id:c.home_id,p_away_id:c.away_id,p_home_delta:te.delta,p_away_delta:ee.delta,p_home_new_rd:te.newRd,p_away_new_rd:ee.newRd,p_home_new_vol:te.newSigma,p_away_new_vol:ee.newSigma});const fe=te.delta,ye=te.newMmr,be=Nt(ye),ke=fe>=0?"+":"",Be=fe>=0?"#4caf50":"#ff6b6b",Fe=Nt(F.mmr);w=`
      ...
      ${w}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${r?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(k),(M=k.querySelector("#pvp-end-home"))==null||M.addEventListener("click",()=>{k.remove(),Ve(e),o(r?"ranked":"home")})}const V=T.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},_=>{const g=_.new;try{if(g.status==="finished"||g.forfeit){if(I)return;I=!0,q&&(clearInterval(q),q=null),g.game_state&&(p=g.game_state),N(g);return}if(g.game_state){const y=p;p=g.game_state;const b=p._lastGC;if(b&&b.seq&&!C.has(b.seq)&&(C.add(b.seq),b.by!==u)){se(b.type,b.by,()=>xe());return}const v=y[u+"Score"]||0,w=y[h+"Score"]||0,k=p[u+"Score"]||0,z=p[h+"Score"]||0,E=k>v,L=z>w;if((E||L)&&!P.has(p.round)){P.add(p.round);const S=[...p.log||[]].reverse().find(F=>F.isGoal),M=((S==null?void 0:S.homePlayers)||[]).map(F=>({name:F.name,note:F.note,portrait:F.portrait,job:F.job}));Ee(M,k,z,E,()=>xe());return}xe()}}catch(y){console.error("[PvP] crash:",y)}}).subscribe();async function K(_){Object.assign(p,_),p.lastActionAt=new Date().toISOString();const{error:g}=await T.from("matches").update({game_state:p}).eq("id",i);g&&x("Erreur de synchronisation","error");try{xe()}catch(y){console.error("[PvP] renderPvpScreen crash:",y)}}async function ne(){if(I)return;I=!0,q&&(clearInterval(q),q=null);const _=n?c.away_id:c.home_id,g=n?"p2":"p1",y=n?"p1":"p2",b={...p,[g+"Score"]:3,[y+"Score"]:0,phase:"finished"};try{await T.from("matches").update({status:"finished",forfeit:!0,winner_id:_,home_score:b.p1Score||0,away_score:b.p2Score||0,game_state:b}).eq("id",i)}catch{}try{T.removeChannel(V)}catch{}setTimeout(()=>{Ve(e),o("home")},800)}const ie={BOOST_STAT:({value:_=1,count:g=1,roles:y=[]},b,v)=>{const w=b[u+"Team"],k=Object.entries(w).filter(([z])=>!y.length||y.includes(z)).flatMap(([z,E])=>E.filter(L=>!L.used).map(L=>({...L,_line:z})));if(!k.length){b.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),v(b);return}Q(k,g,`Choisir ${g} joueur(s) à booster (+${_})`,z=>{z.forEach(E=>{const L=(w[E._line]||[]).find(S=>S.cardId===E.cardId);L&&(L.boost=(L.boost||0)+_,b.log.push({text:`⚡ +${_} sur ${L.name}`,type:"info"}))}),b[u+"Team"]=w,v(b)})},DEBUFF_STAT:({value:_=1,count:g=1,roles:y=[],target:b="ai"},v,w)=>{const k=b==="home"?u:h,z=v[k+"Team"],E=b==="home"?"allié":"adverse",L=Object.entries(z).filter(([S])=>!y.length||y.includes(S)).flatMap(([S,M])=>M.map(F=>({...F,_line:S})));if(!L.length){v.log.push({text:`🎯 Aucun joueur ${E}`,type:"info"}),w(v);return}Q(L,g,`Choisir ${g} joueur(s) ${E}(s) (-${_})`,S=>{S.forEach(M=>{const F=(z[M._line]||[]).find(Y=>Y.cardId===M.cardId);F&&(F.boost=(F.boost||0)-_,v.log.push({text:`🎯 -${_} sur ${F.name}`,type:"info"}))}),v[k+"Team"]=z,w(v)})},GRAY_PLAYER:({count:_=1,roles:g=[],target:y="ai"},b,v)=>{const w=y==="home"?u:h,k=b[w+"Team"],z=y==="home"?"allié":"adverse",E=Object.entries(k).filter(([L])=>!g.length||g.includes(L)).flatMap(([L,S])=>S.filter(M=>!M.used).map(M=>({...M,_line:L})));if(!E.length){b.log.push({text:`❌ Aucun joueur ${z}`,type:"info"}),v(b);return}Q(E,_,`Choisir ${_} joueur(s) ${z}(s) à exclure`,L=>{const S="usedCardIds_"+w,M=new Set(b[S]||[]);L.forEach(F=>{const Y=(k[F._line]||[]).find(J=>J.cardId===F.cardId);Y&&(Y.used=!0,M.add(F.cardId),b.log.push({text:`❌ ${Y.name} exclu !`,type:"info"}))}),b[S]=[...M],b[w+"Team"]=k,v(b)})},REVIVE_PLAYER:({count:_=1,roles:g=[]},y,b)=>{const v=y[u+"Team"],w=Object.entries(v).filter(([k])=>!g.length||g.includes(k)).flatMap(([k,z])=>z.filter(E=>E.used).map(E=>({...E,_line:k})));if(!w.length){y.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),b(y);return}Q(w,_,`Choisir ${_} joueur(s) à ressusciter`,k=>{k.forEach(z=>{const E=(v[z._line]||[]).find(L=>L.cardId===z.cardId);E&&(E.used=!1,y.log.push({text:`💫 ${E.name} ressuscité !`,type:"info"}))}),y[u+"Team"]=v,b(y)})},REMOVE_GOAL:({},_,g)=>{const y=h+"Score";_[y]>0?(_[y]--,_.log.push({text:"🚫 Dernier but annulé !",type:"info"})):_.log.push({text:"🚫 Aucun but à annuler",type:"info"}),g(_)},ADD_GOAL_DRAW:({},_,g)=>{_[u+"Score"]===_[h+"Score"]?(_[u+"Score"]++,_.log.push({text:"🎯 But bonus !",type:"info"})):_.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),g(_)},ADD_SUB:({value:_=1},g,y)=>{g.maxSubs=(g.maxSubs||3)+_,g.log.push({text:`🔄 +${_} remplacement(s)`,type:"info"}),y(g)},CUSTOM:({},_,g)=>g(_)};function Q(_,g,y,b){const v=document.createElement("div");v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let w=[];function k(){var E,L;const z=_.map(S=>{const M={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[S._line]||"#555",F=Se(S,S._line)+(S.boost||0),J=w.find(G=>G.cardId===S.cardId)?"#FFD700":"rgba(255,255,255,0.25)",U=S.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${S.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${J};background:${M};overflow:hidden;cursor:pointer;${U}">
        ...
        </div>`,(E=v.querySelector("#pp-close"))==null||E.addEventListener("click",()=>v.remove()),v.querySelectorAll(".pp-item").forEach(S=>{S.addEventListener("click",()=>{const M=S.dataset.cid,F=_.find(J=>J.cardId===M),Y=w.findIndex(J=>J.cardId===M);F&&(Y>-1?w.splice(Y,1):w.length<g&&w.push(F),k())})}),(L=v.querySelector("#pp-confirm"))==null||L.addEventListener("click",()=>{v.remove(),b(w)})}k(),document.body.appendChild(v)}async function R(_,g){const b=(p["gcCardsFull_"+u]||[]).find(z=>z.id===_),v=(b==null?void 0:b._gcDef)||(p.gcDefs||[]).find(z=>{var E;return z.name===g||((E=z.name)==null?void 0:E.toLowerCase().trim())===(g==null?void 0:g.toLowerCase().trim())}),w=[...p["usedGc_"+u]||[],_],k={type:g,by:u,seq:(p._gcAnimSeq||0)+1};C.add(k.seq),se(g,u,async()=>{if(v!=null&&v.effect_type&&v.effect_type!=="CUSTOM"){const E=ie[v.effect_type];if(E){const L={...p};E(v.effect_params||{},L,async S=>{S["usedGc_"+u]=w,S._lastGC=k,S._gcAnimSeq=k.seq,await K(S)});return}}const z={...p};switch(g){case"Remplacement+":z.maxSubs=(z.maxSubs||3)+1,z.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const E=h+"Score";z[E]>0&&(z[E]--,z.log.push({text:"🚫 But annulé",type:"info"}));break}}z["usedGc_"+u]=w,z._lastGC=k,z._gcAnimSeq=k.seq,await K(z)})}function D(_,g){const y="usedCardIds_"+_,b=new Set(p[y]||[]);g.forEach(v=>b.add(v)),p[y]=[...b]}function re(){for(const _ of["p1","p2"]){const g=new Set(p["usedCardIds_"+_]||[]),y=p[_+"Team"];if(y)for(const b of["GK","DEF","MIL","ATT"])(y[b]||[]).forEach(v=>{v.used=g.has(v.cardId)})}}function xe(){var We,ot,pt,ut,ft,le;if(p.phase==="reveal")return _e();if(p.phase==="midfield")return ve();if(p.phase==="finished")return B();const _=p[u+"Team"],g=p[h+"Team"];re();const y=p[u+"Score"],b=p[h+"Score"],v=p[u+"Name"],w=p[h+"Name"],k=p.phase===u+"-attack",z=p.phase===u+"-defense",E=Array.isArray(p["selected_"+u])?p["selected_"+u]:[],L=E.map(X=>X.cardId),S=window.innerWidth>=700,M=p[u+"Subs"]||[],F=p["usedSubIds_"+u]||[],Y=M.filter(X=>!F.includes(X.cardId)),J=p["gcCardsFull_"+u]||[],U=p["usedGc_"+u]||[],G=J.filter(X=>!U.includes(X.id)),W=p.boostOwner===u&&!p.boostUsed,te=!["GK","DEF","MIL","ATT"].some(X=>(g[X]||[]).some(O=>!O.used)),ee=[..._.MIL||[],..._.ATT||[]].filter(X=>!X.used),fe=k&&te&&ee.length===0?[..._.GK||[],..._.DEF||[]].filter(X=>!X.used).map(X=>X.cardId):[];function ye(X,O,ae){var kt,Ct;const he=X._gcDef,$e={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[he==null?void 0:he.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",Qe={purple:"#b06ce0",light_blue:"#00d4ef"}[he==null?void 0:he.color]||"#b06ce0",Ze=(he==null?void 0:he.name)||X.gc_type,tt=(he==null?void 0:he.effect)||((kt=Oe[X.gc_type])==null?void 0:kt.desc)||"",rt=he!=null&&he.image_url?`/icons/${he.image_url}`:null,_t=((Ct=Oe[X.gc_type])==null?void 0:Ct.icon)||"⚡",$t=Math.round(ae*.22),dt=Math.round(ae*.22),gt=ae-$t-dt,mt=Ze.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${X.id}" data-gc-type="${X.gc_type}"
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${ct(_,p[u+"Formation"],Ne,L,300,300,fe)}
      </div>
    ...
    </div>`,u==="p1"&&setTimeout(async()=>{await K({phase:"midfield"})},5e3)}let we=!1;function ve(){if(we)return;const _=p[u+"Team"].MIL||[],g=p[h+"Team"].MIL||[];function y(G){return G.reduce((W,te)=>W+Se(te,"MIL"),0)}function b(G){let W=0;for(let te=0;te<G.length-1;te++){const ee=nt(G[te],G[te+1]);ee==="#00ff88"?W+=2:ee==="#FFD700"&&(W+=1)}return W}const v=y(_)+b(_),w=y(g)+b(g),k=v>=w;function z(G,W,te){return`<div id="duel-row-${te}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(w);let E=!1;const L=()=>{E||(E=!0,w.remove(),setTimeout(()=>v(),50))};w.addEventListener("click",L),setTimeout(L,3500)}function se(_,g,y){var U,G;const b=(p.gcDefs||[]).find(W=>{var te;return W.name===_||((te=W.name)==null?void 0:te.toLowerCase().trim())===(_==null?void 0:_.toLowerCase().trim())}),v={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[b==null?void 0:b.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",w={purple:"#b06ce0",light_blue:"#00d4ef"}[b==null?void 0:b.color]||"#b06ce0",k=(b==null?void 0:b.name)||_,z=(b==null?void 0:b.effect)||((U=Oe[_])==null?void 0:U.desc)||"",E=b!=null&&b.image_url?`/icons/${b.image_url}`:null,L=((G=Oe[_])==null?void 0:G.icon)||"⚡",M=g===u?"Vous":p[g+"Name"]||"Adversaire",F=document.createElement("div");F.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",F.innerHTML=`
      <style>
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:6px">Appuyer pour continuer</div>`,document.body.appendChild(v);let k=!1;const z=()=>{k||(k=!0,v.remove(),setTimeout(()=>y(),50))};v.addEventListener("click",z),setTimeout(z,2200)}function ge(){var g;const _=document.createElement("div");_.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:20px;overflow-y:auto",_.innerHTML=`
      <div style="font-size:12px;color:rgba(255,255,255,0.5);letter-spacing:2px;text-transform:uppercase">Équipe adverse</div>
      <div style="font-size:18px;font-weight:900;color:#ff6b6b">${p[h+"Name"]}</div>
      <div style="width:min(90vw,420px)">${lt(p[h+"Team"],p[h+"Formation"],null,[],300,300)}</div>
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(_),(g=_.querySelector("#pvp-opp-close"))==null||g.addEventListener("click",()=>_.remove())}function de(){var y;const _=p.log||[],g=document.createElement("div");g.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column",g.innerHTML=`
      ...
      </div>`,document.body.appendChild(g),(y=g.querySelector("#pvp-hist-close"))==null||y.addEventListener("click",()=>g.remove())}async function me(){if(p.phase!==u+"-attack")return;const _=u==="p1"?"p2":"p1",g=(p.round||0)+1,y=[...p.log||[]];y.push({type:"info",text:`⏭️ ${p[u+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const b=qe(p),v=Te(_),w=b||!v?"finished":_+"-attack";await K({["selected_"+u]:[],modifiers:{...p.modifiers,[u]:{}},pendingAttack:null,phase:w,attacker:_,round:g,log:y}),w==="finished"&&await ze(p)}async function oe(){const _=p[u+"Team"],g=!["GK","DEF","MIL","ATT"].some(w=>(p[h+"Team"][w]||[]).some(k=>!k.used)),y=(p["selected_"+u]||[]).map(w=>{const k=(_[w._role]||[]).find(F=>F.cardId===w.cardId)||w,z=g&&["GK","DEF"].includes(w._role),E=_[w._role]||[],L=E.findIndex(F=>F.cardId===w.cardId),S=st(E.length),M=L>=0?S[L]:k._col??1;return{...k,_line:w._role,_col:M,...z?{note_a:Math.max(1,Number(k.note_a)||0)}:{}}});if(!y.length)return;const b=Kt(y,p.modifiers[u]||{});D(u,y.map(w=>w.cardId)),y.forEach(w=>{const k=(_[w._role]||[]).find(z=>z.cardId===w.cardId);k&&(k.used=!0)}),p["selected_"+u]=[],xe();const v=[...p.log||[]];if(g){const w=(p[u+"Score"]||0)+1,k=y.map(M=>({name:M.name,note:Se(M,M._line||"ATT"),portrait:He(M),job:M.job}));v.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:k,homeTotal:b.total,aiTotal:0});const z=(p.round||0)+1,E=u==="p1"?"p2":"p1",L={...p,[u+"Team"]:_,[u+"Score"]:w},S=qe(L);P.add(z),Ee(k,w,p[h+"Score"]||0,!0,async()=>{await K({[u+"Team"]:_,[u+"Score"]:w,["selected_"+u]:[],modifiers:{...p.modifiers,[u]:{}},pendingAttack:null,phase:S?"finished":E+"-attack",attacker:E,round:z,log:v}),S&&await ze({...p,[u+"Score"]:w})});return}v.push({type:"pending",text:`⚔️ ${p[u+"Name"]} attaque (${b.total})`}),await K({[u+"Team"]:_,[h+"Team"]:p[h+"Team"],pendingAttack:{...b,players:y,side:u},["selected_"+u]:[],modifiers:{...p.modifiers,[u]:{}},phase:h+"-defense",log:v})}async function ue(){const _=p[u+"Team"],g=(p["selected_"+u]||[]).map(U=>{const G=(_[U._role]||[]).find(ye=>ye.cardId===U.cardId)||U,W=_[U._role]||[],te=W.findIndex(ye=>ye.cardId===U.cardId),ee=st(W.length),fe=te>=0?ee[te]:G._col??1;return{...G,_line:U._role,_col:fe}}),y=Vt(g,p.modifiers[u]||{});D(u,g.map(U=>U.cardId)),g.forEach(U=>{const G=(_[U._role]||[]).find(W=>W.cardId===U.cardId);G&&(G.used=!0)}),p["selected_"+u]=[],xe();const b=Yt(p.pendingAttack.total,y.total,p.modifiers[u]||{}),v=p.pendingAttack.side,w=b==="attack"||(b==null?void 0:b.goal),k=v==="p1"?"p2":"p1",z=(p.round||0)+1,E=(p.pendingAttack.players||[]).map(U=>({name:U.name,note:Se(U,U._line||"ATT"),portrait:He(U),job:U.job})),L=[...p.log||[]];L.push({type:"duel",isGoal:w,homeScored:w&&v===u,text:w?`⚽ BUT de ${p[v+"Name"]} ! (${p.pendingAttack.total} vs ${y.total})`:`✋ Attaque stoppée (${p.pendingAttack.total} vs ${y.total})`,homePlayers:E,aiPlayers:g.map(U=>({name:U.name,note:Se(U,U._line||"DEF"),portrait:He(U),job:U.job})),homeTotal:p.pendingAttack.total,aiTotal:y.total});const S=w?(p[v+"Score"]||0)+1:p[v+"Score"]||0,M={...p,[u+"Team"]:_,[v+"Score"]:S},F=qe(M),Y=F?"finished":k+"-attack",J=async()=>{await K({[u+"Team"]:_,[h+"Team"]:p[h+"Team"],[v+"Score"]:S,["selected_"+u]:[],modifiers:{...p.modifiers,[u]:{}},pendingAttack:null,phase:Y,attacker:k,round:z,log:L}),(Y==="finished"||F)&&await ze({...p,[v+"Score"]:S})};if(w){const U=v===u,G=U?S:p[u+"Score"]||0,W=U?p[h+"Score"]||0:S;P.add(z),Ee(E,G,W,U,J)}else await J()}function Le(_){return["MIL","ATT"].some(g=>(_[g]||[]).some(y=>!y.used))}function Ie(_){return["GK","DEF","MIL","ATT"].some(g=>(_[g]||[]).some(y=>!y.used))}function Ce(_){return Ie(_)&&!Le(_)}function Te(_){const g=p[_+"Team"],y=p[(_==="p1"?"p2":"p1")+"Team"];return!!(Le(g)||!Ie(y)&&Ce(g))}function qe(_){const g=["MIL","ATT"].some(L=>(_.p1Team[L]||[]).some(S=>!S.used)),y=["MIL","ATT"].some(L=>(_.p2Team[L]||[]).some(S=>!S.used)),b=Ie(_.p1Team),v=Ie(_.p2Team);return!g&&!(!v&&b)&&(!y&&!(!b&&v))}function Pe(_){const g=_.p1Score||0,y=_.p2Score||0;return g===y?null:g>y?c.home_id:c.away_id}async function ze(_){try{const g=Pe(_),y=g?c.home_id===g?c.away_id:c.home_id:null;await T.from("matches").update({status:"finished",winner_id:g,home_score:_.p1Score||0,away_score:_.p2Score||0}).eq("id",i),g&&y&&updateEvolutiveCards(g,y).catch(()=>{})}catch(g){console.error("[PvP] finishMatch:",g)}}function B(){var v;const _=p[u+"Score"],g=p[h+"Score"],y=_>g,b=_===g;et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      ...
      </div>`,(R=document.getElementById("lobby-cancel"))==null||R.addEventListener("click",async()=>{$=!0,m&&(T.removeChannel(m),m=null),ie&&await T.from("friend_match_invites").update({status:"declined"}).eq("id",ie),Ve(e),x("home")})},A=async(K,ne)=>{$=!0,m&&(T.removeChannel(m),m=null),await new Promise(ie=>setTimeout(ie,600)),e.isConnected&&Jn(e,t,K,ne,r,l)},{data:p}=await T.from("friend_match_invites").select("*").eq("status","pending").or(`and(inviter_id.eq.${h},invitee_id.eq.${f}),and(inviter_id.eq.${f},invitee_id.eq.${h})`).order("created_at",{ascending:!1}).limit(1).maybeSingle();let I,q;if(p&&p.inviter_id===f)q=!1,I=p.id,await T.from("friend_match_invites").update({invitee_deck_id:i,invitee_ready:!0,status:"accepted"}).eq("id",I);else{q=!0;const{data:K,error:ne}=await T.from("friend_match_invites").insert({inviter_id:h,invitee_id:f,friend_id:f,inviter_deck_id:i,inviter_ready:!0,status:"pending"}).select().single();if(ne||!K){u("Erreur création invitation","error"),Ve(e),x("home");return}I=K.id}if(c(!0,!q,I),!q){const K={home_id:f,away_id:h,home_deck_id:p.inviter_deck_id,away_deck_id:i,status:"active",mode:"friend"},ne=await T.from("matches").insert(K).select().single(),ie=ne.data;if(!ie){u("Erreur création match: "+(((V=ne.error)==null?void 0:V.message)||""),"error"),Ve(e),x("home");return}await T.from("friend_match_invites").update({match_id:ie.id,status:"matched"}).eq("id",I),A(ie.id,!1);return}let j=!1;const P=K=>{$||j||(K.status==="matched"&&K.match_id?(j=!0,clearInterval(C),clearInterval(N),A(K.match_id,!0)):K.status==="declined"?(clearInterval(C),clearInterval(N),u(`${a} a décliné l'invitation`,"warning"),Ve(e),x("home")):K.invitee_ready&&c(!0,!0,I))},C=setInterval(()=>{if($){clearInterval(C);return}T.from("matchmaking_queue").delete().eq("user_id",h).then(()=>{},()=>{})},3e3),N=setInterval(async()=>{if($||j){clearInterval(N);return}const{data:K}=await T.from("friend_match_invites").select("*").eq("id",I).maybeSingle();K&&P(K)},1200);m=T.channel("friend-invite-"+I).on("postgres_changes",{event:"UPDATE",schema:"public",table:"friend_match_invites",filter:`id=eq.${I}`},K=>P(K.new)).subscribe()}async function Jn(e,t,i,n,d=[],s=[]){const{state:r,navigate:l,toast:f}=t,a=n?"p1":"p2",o=n?"p2":"p1",x=(d||[]).map(B=>B.id),u=(d||[]).map(B=>({id:B.id,gc_type:B.gc_type,_gcDef:B._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:h}=await T.from("matches").select("*").eq("id",i).single();if(!h){f("Match introuvable","error"),l("home");return}async function $(){const[{data:B},{data:_},{data:g},{data:y}]=await Promise.all([T.rpc("get_deck_for_match",{p_deck_id:h.home_deck_id}),T.rpc("get_deck_for_match",{p_deck_id:h.away_deck_id}),T.from("users").select("id,pseudo,club_name").eq("id",h.home_id).single(),T.from("users").select("id,pseudo,club_name").eq("id",h.away_id).single()]),b=E=>{const L=Number(E.evolution_bonus)||0;return{cardId:E.card_id,position:E.position,id:E.id,firstname:E.firstname,name:E.surname_encoded,country_code:E.country_code,club_id:E.club_id,job:E.job,job2:E.job2,note_g:(Number(E.note_g)||0)+(E.job==="GK"||E.job2==="GK"&&Number(E.note_g)>0?L:0),note_d:(Number(E.note_d)||0)+(E.job==="DEF"||E.job2==="DEF"&&Number(E.note_d)>0?L:0),note_m:(Number(E.note_m)||0)+(E.job==="MIL"||E.job2==="MIL"&&Number(E.note_m)>0?L:0),note_a:(Number(E.note_a)||0)+(E.job==="ATT"||E.job2==="ATT"&&Number(E.note_a)>0?L:0),evolution_bonus:L,rarity:E.rarity,skin:E.skin,hair:E.hair,hair_length:E.hair_length,clubName:E.club_encoded_name||null,clubLogo:E.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},v=((B==null?void 0:B.starters)||[]).map(E=>b(E)),w=((_==null?void 0:_.starters)||[]).map(E=>b(E)),k=(B==null?void 0:B.formation)||"4-4-2",z=(_==null?void 0:_.formation)||"4-4-2";return{p1Team:(()=>{const E=vt(v,k);return stadiumDef?Tt(E,stadiumDef):E})(),p2Team:vt(w,z),p1Subs:(()=>{const E=((B==null?void 0:B.subs)||[]).map(L=>b(L));return stadiumDef&&jt(E,stadiumDef),E})(),p2Subs:((_==null?void 0:_.subs)||[]).map(E=>b(E)),p1Formation:k,p2Formation:z,p1Name:(g==null?void 0:g.club_name)||(g==null?void 0:g.pseudo)||"Joueur 1",p2Name:(y==null?void 0:y.club_name)||(y==null?void 0:y.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:n?x:[],gc_p2:n?[]:x,gcCardsFull_p1:n?u:[],gcCardsFull_p2:n?[]:u,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:s||[],lastActionAt:new Date().toISOString()}}let m=h.game_state&&Object.keys(h.game_state).length?h.game_state:null;if(!m)if(n){m=await $();const{data:B}=await T.from("matches").select("game_state").eq("id",i).single();!(B!=null&&B.game_state)||!Object.keys(B.game_state).length?await T.from("matches").update({game_state:m,turn_user_id:h.home_id}).eq("id",i):m=B.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let B=0;B<60&&!m;B++){await new Promise(g=>setTimeout(g,400));const{data:_}=await T.from("matches").select("game_state").eq("id",i).single();_!=null&&_.game_state&&Object.keys(_.game_state).length&&(m=_.game_state)}if(!m){f("Erreur de synchronisation","error"),l("home");return}m.gc_p2=x,m.gcCardsFull_p2=u,await T.from("matches").update({game_state:m}).eq("id",i)}let c=!1,A=!1,p=!1,I=null,q=!1;const j=new Set,P=new Set;function C(B){var E,L;try{T.removeChannel(N)}catch{}const _=m[a+"Score"]||0,g=m[o+"Score"]||0;if(!p){p=!0;const S=B.winner_id||(_>g?r.profile.id:g>_?"opp":null),M=S===r.profile.id?"win":S?"loss":null;M&&ni(()=>Promise.resolve().then(()=>oo),void 0).then(F=>F.applyOwnEvolution(r.profile.id,M)).catch(()=>{})}if(!A){const S=m.p1Score||0,M=m.p2Score||0,F=(m.usedGc_p1||[]).length,Y=(m.usedGc_p2||[]).length,J=B.winner_id||(S>M?h.home_id:M>S?h.away_id:null);(J?r.profile.id===J:r.profile.id<(n?h.away_id:h.home_id))&&(A=!0,Ji({player1Id:h.home_id,player2Id:h.away_id,score1:S,score2:M,gc1:F,gc2:Y}).catch(G=>console.warn("[FriendMatch] updateStats:",G)))}let y,b;B.winner_id?(y=B.winner_id===r.profile.id,b=!1):B.forfeit?(y=_>g,b=!1):(b=_===g,y=_>g),(E=document.getElementById("pvp-end-overlay"))==null||E.remove();const v=document.createElement("div");v.id="pvp-end-overlay",v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const w=b?"🤝":y?"🏆":"😞",k=b?"MATCH NUL":y?"VICTOIRE !":"DÉFAITE",z=b?"#fff":y?"#FFD700":"#ff6b6b";v.innerHTML=`
      <div style="font-size:64px">${w}</div>
      <div style="font-size:26px;font-weight:900;color:${z}">${k}</div>
      <div style="font-size:18px">${m[a+"Name"]} ${_} – ${g} ${m[o+"Name"]}</div>
      ${B.forfeit?`<div style="font-size:13px;color:rgba(255,255,255,0.5)">${y?"L'adversaire a quitté":"Perdu par forfait"}</div>`:""}
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(v),(L=v.querySelector("#pvp-end-home"))==null||L.addEventListener("click",()=>{v.remove(),Ve(e),l("home")})}const N=T.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},B=>{const _=B.new;try{if(_.status==="finished"||_.forfeit){if(c)return;c=!0,I&&(clearInterval(I),I=null),_.game_state&&(m=_.game_state),C(_);return}if(_.game_state){const g=m;m=_.game_state;const y=m._lastGC;if(y&&y.seq&&!P.has(y.seq)&&(P.add(y.seq),y.by!==a)){Ee(y.type,y.by,()=>re());return}const b=g[a+"Score"]||0,v=g[o+"Score"]||0,w=m[a+"Score"]||0,k=m[o+"Score"]||0,z=w>b,E=k>v;if((z||E)&&!j.has(m.round)){j.add(m.round);const L=[...m.log||[]].reverse().find(M=>M.isGoal),S=((L==null?void 0:L.homePlayers)||[]).map(M=>({name:M.name,note:M.note,portrait:M.portrait,job:M.job}));ve(S,w,k,z,()=>re());return}re()}}catch(g){console.error("[PvP] crash:",g)}}).subscribe();async function V(B){Object.assign(m,B),m.lastActionAt=new Date().toISOString();const{error:_}=await T.from("matches").update({game_state:m}).eq("id",i);_&&f("Erreur de synchronisation","error");try{re()}catch(g){console.error("[PvP] renderPvpScreen crash:",g)}}async function K(){if(c)return;c=!0,I&&(clearInterval(I),I=null);const B=n?h.away_id:h.home_id,_=n?"p2":"p1",g=n?"p1":"p2",y={...m,[_+"Score"]:3,[g+"Score"]:0,phase:"finished"};try{await T.from("matches").update({status:"finished",forfeit:!0,winner_id:B,home_score:y.p1Score||0,away_score:y.p2Score||0,game_state:y}).eq("id",i)}catch{}try{T.removeChannel(N)}catch{}setTimeout(()=>{Ve(e),l("home")},800)}const ne={BOOST_STAT:({value:B=1,count:_=1,roles:g=[]},y,b)=>{const v=y[a+"Team"],w=Object.entries(v).filter(([k])=>!g.length||g.includes(k)).flatMap(([k,z])=>z.filter(E=>!E.used).map(E=>({...E,_line:k})));if(!w.length){y.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),b(y);return}ie(w,_,`Choisir ${_} joueur(s) à booster (+${B})`,k=>{k.forEach(z=>{const E=(v[z._line]||[]).find(L=>L.cardId===z.cardId);E&&(E.boost=(E.boost||0)+B,y.log.push({text:`⚡ +${B} sur ${E.name}`,type:"info"}))}),y[a+"Team"]=v,b(y)})},DEBUFF_STAT:({value:B=1,count:_=1,roles:g=[],target:y="ai"},b,v)=>{const w=y==="home"?a:o,k=b[w+"Team"],z=y==="home"?"allié":"adverse",E=Object.entries(k).filter(([L])=>!g.length||g.includes(L)).flatMap(([L,S])=>S.map(M=>({...M,_line:L})));if(!E.length){b.log.push({text:`🎯 Aucun joueur ${z}`,type:"info"}),v(b);return}ie(E,_,`Choisir ${_} joueur(s) ${z}(s) (-${B})`,L=>{L.forEach(S=>{const M=(k[S._line]||[]).find(F=>F.cardId===S.cardId);M&&(M.boost=(M.boost||0)-B,b.log.push({text:`🎯 -${B} sur ${M.name}`,type:"info"}))}),b[w+"Team"]=k,v(b)})},GRAY_PLAYER:({count:B=1,roles:_=[],target:g="ai"},y,b)=>{const v=g==="home"?a:o,w=y[v+"Team"],k=g==="home"?"allié":"adverse",z=Object.entries(w).filter(([E])=>!_.length||_.includes(E)).flatMap(([E,L])=>L.filter(S=>!S.used).map(S=>({...S,_line:E})));if(!z.length){y.log.push({text:`❌ Aucun joueur ${k}`,type:"info"}),b(y);return}ie(z,B,`Choisir ${B} joueur(s) ${k}(s) à exclure`,E=>{const L="usedCardIds_"+v,S=new Set(y[L]||[]);E.forEach(M=>{const F=(w[M._line]||[]).find(Y=>Y.cardId===M.cardId);F&&(F.used=!0,S.add(M.cardId),y.log.push({text:`❌ ${F.name} exclu !`,type:"info"}))}),y[L]=[...S],y[v+"Team"]=w,b(y)})},REVIVE_PLAYER:({count:B=1,roles:_=[]},g,y)=>{const b=g[a+"Team"],v=Object.entries(b).filter(([w])=>!_.length||_.includes(w)).flatMap(([w,k])=>k.filter(z=>z.used).map(z=>({...z,_line:w})));if(!v.length){g.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),y(g);return}ie(v,B,`Choisir ${B} joueur(s) à ressusciter`,w=>{w.forEach(k=>{const z=(b[k._line]||[]).find(E=>E.cardId===k.cardId);z&&(z.used=!1,g.log.push({text:`💫 ${z.name} ressuscité !`,type:"info"}))}),g[a+"Team"]=b,y(g)})},REMOVE_GOAL:({},B,_)=>{const g=o+"Score";B[g]>0?(B[g]--,B.log.push({text:"🚫 Dernier but annulé !",type:"info"})):B.log.push({text:"🚫 Aucun but à annuler",type:"info"}),_(B)},ADD_GOAL_DRAW:({},B,_)=>{B[a+"Score"]===B[o+"Score"]?(B[a+"Score"]++,B.log.push({text:"🎯 But bonus !",type:"info"})):B.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),_(B)},ADD_SUB:({value:B=1},_,g)=>{_.maxSubs=(_.maxSubs||3)+B,_.log.push({text:`🔄 +${B} remplacement(s)`,type:"info"}),g(_)},CUSTOM:({},B,_)=>_(B)};function ie(B,_,g,y){const b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let v=[];function w(){var z,E;const k=B.map(L=>{const S={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[L._line]||"#555",M=Se(L,L._line)+(L.boost||0),Y=v.find(U=>U.cardId===L.cardId)?"#FFD700":"rgba(255,255,255,0.25)",J=L.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${L.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${Y};background:${S};overflow:hidden;cursor:pointer;${J}">
        ...
        <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
          ...
          <span style="font-size:12px;color:rgba(255,255,255,0.5)">${v.length}/${_}</span>
          <button id="pp-close" style="background:none;border:none;color:rgba(255,255,255,0.5);font-size:22px;cursor:pointer">✕</button>
        </div>
        <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
          ${k}
        ...
        <div style="padding:12px;background:rgba(0,0,0,0.4);flex-shrink:0">
          <button id="pp-confirm" ${v.length===0?'disabled style="opacity:0.4"':""} style="width:100%;padding:13px;border-radius:10px;border:none;background:#7a28b8;color:#fff;font-size:15px;font-weight:900;cursor:pointer">
            ✅ Confirmer (${v.length}/${_})
          </button>
        </div>`,(z=b.querySelector("#pp-close"))==null||z.addEventListener("click",()=>b.remove()),b.querySelectorAll(".pp-item").forEach(L=>{L.addEventListener("click",()=>{const S=L.dataset.cid,M=B.find(Y=>Y.cardId===S),F=v.findIndex(Y=>Y.cardId===S);M&&(F>-1?v.splice(F,1):v.length<_&&v.push(M),w())})}),(E=b.querySelector("#pp-confirm"))==null||E.addEventListener("click",()=>{b.remove(),y(v)})}w(),document.body.appendChild(b)}async function Q(B,_){const y=(m["gcCardsFull_"+a]||[]).find(k=>k.id===B),b=(y==null?void 0:y._gcDef)||(m.gcDefs||[]).find(k=>{var z;return k.name===_||((z=k.name)==null?void 0:z.toLowerCase().trim())===(_==null?void 0:_.toLowerCase().trim())}),v=[...m["usedGc_"+a]||[],B],w={type:_,by:a,seq:(m._gcAnimSeq||0)+1};P.add(w.seq),Ee(_,a,async()=>{if(b!=null&&b.effect_type&&b.effect_type!=="CUSTOM"){const z=ne[b.effect_type];if(z){const E={...m};z(b.effect_params||{},E,async L=>{L["usedGc_"+a]=v,L._lastGC=w,L._gcAnimSeq=w.seq,await V(L)});return}}const k={...m};switch(_){case"Remplacement+":k.maxSubs=(k.maxSubs||3)+1,k.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const z=o+"Score";k[z]>0&&(k[z]--,k.log.push({text:"🚫 But annulé",type:"info"}));break}}k["usedGc_"+a]=v,k._lastGC=w,k._gcAnimSeq=w.seq,await V(k)})}function R(B,_){const g="usedCardIds_"+B,y=new Set(m[g]||[]);_.forEach(b=>y.add(b)),m[g]=[...y]}function D(){for(const B of["p1","p2"]){const _=new Set(m["usedCardIds_"+B]||[]),g=m[B+"Team"];if(g)for(const y of["GK","DEF","MIL","ATT"])(g[y]||[]).forEach(b=>{b.used=_.has(b.cardId)})}}function re(){var Ge,We,ot,pt,ut,ft;if(m.phase==="reveal")return xe();if(m.phase==="midfield")return we();if(m.phase==="finished")return ze();const B=m[a+"Team"],_=m[o+"Team"];D();const g=m[a+"Score"],y=m[o+"Score"],b=m[a+"Name"],v=m[o+"Name"],w=m.phase===a+"-attack",k=m.phase===a+"-defense",z=Array.isArray(m["selected_"+a])?m["selected_"+a]:[],E=z.map(le=>le.cardId),L=window.innerWidth>=700,S=m[a+"Subs"]||[],M=m["usedSubIds_"+a]||[],F=S.filter(le=>!M.includes(le.cardId)),Y=m["gcCardsFull_"+a]||[],J=m["usedGc_"+a]||[],U=Y.filter(le=>!J.includes(le.id)),G=m.boostOwner===a&&!m.boostUsed,W=!["GK","DEF","MIL","ATT"].some(le=>(_[le]||[]).some(X=>!X.used)),te=[...B.MIL||[],...B.ATT||[]].filter(le=>!le.used),ee=w&&W&&te.length===0?[...B.GK||[],...B.DEF||[]].filter(le=>!le.used).map(le=>le.cardId):[];function fe(le,X,O){var mt,kt;const ae=le._gcDef,he={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ae==null?void 0:ae.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",$e={purple:"#b06ce0",light_blue:"#00d4ef"}[ae==null?void 0:ae.color]||"#b06ce0",Qe=(ae==null?void 0:ae.name)||le.gc_type,Ze=(ae==null?void 0:ae.effect)||((mt=Oe[le.gc_type])==null?void 0:mt.desc)||"",tt=ae!=null&&ae.image_url?`/icons/${ae.image_url}`:null,rt=((kt=Oe[le.gc_type])==null?void 0:kt.icon)||"⚡",_t=Math.round(O*.22),$t=Math.round(O*.22),dt=O-_t-$t,gt=Qe.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${le.id}" data-gc-type="${le.gc_type}"
    ...
    </div>`,a==="p1"&&setTimeout(async()=>{await V({phase:"midfield"})},5e3)}let _e=!1;function we(){if(_e)return;const B=m[a+"Team"].MIL||[],_=m[o+"Team"].MIL||[];function g(U){return U.reduce((G,W)=>G+Se(W,"MIL"),0)}function y(U){let G=0;for(let W=0;W<U.length-1;W++){const te=nt(U[W],U[W+1]);te==="#00ff88"?G+=2:te==="#FFD700"&&(G+=1)}return G}const b=g(B)+y(B),v=g(_)+y(_),w=b>=v;function k(U,G,W){return`<div id="duel-row-${W}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(v);let z=!1;const E=()=>{z||(z=!0,v.remove(),setTimeout(()=>b(),50))};v.addEventListener("click",E),setTimeout(E,3500)}function Ee(B,_,g){var J,U;const y=(m.gcDefs||[]).find(G=>{var W;return G.name===B||((W=G.name)==null?void 0:W.toLowerCase().trim())===(B==null?void 0:B.toLowerCase().trim())}),b={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[y==null?void 0:y.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",v={purple:"#b06ce0",light_blue:"#00d4ef"}[y==null?void 0:y.color]||"#b06ce0",w=(y==null?void 0:y.name)||B,k=(y==null?void 0:y.effect)||((J=Oe[B])==null?void 0:J.desc)||"",z=y!=null&&y.image_url?`/icons/${y.image_url}`:null,E=((U=Oe[B])==null?void 0:U.icon)||"⚡",S=_===a?"Vous":m[_+"Name"]||"Adversaire",M=document.createElement("div");M.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",M.innerHTML=`
      <style>
        @keyframes gcFlipIn{0%{transform:perspective(800px) rotateY(90deg) scale(.7);opacity:0}55%{transform:perspective(800px) rotateY(-12deg) scale(1.08);opacity:1}100%{transform:perspective(800px) rotateY(0) scale(1);opacity:1}}
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(M);let F=!1;const Y=()=>{F||(F=!0,M.remove(),setTimeout(()=>g&&g(),50))};M.addEventListener("click",Y),setTimeout(Y,3e3)}function se(B,_){var M,F,Y,J;const y=(m["gcCardsFull_"+a]||[]).find(U=>U.id===B),b=y==null?void 0:y._gcDef,v={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[b==null?void 0:b.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",w={purple:"#b06ce0",light_blue:"#00d4ef"}[b==null?void 0:b.color]||"#b06ce0",k=(b==null?void 0:b.name)||_,z=(b==null?void 0:b.effect)||((M=Oe[_])==null?void 0:M.desc)||"Carte spéciale.",E=b!=null&&b.image_url?`/icons/${b.image_url}`:null,L=((F=Oe[_])==null?void 0:F.icon)||"⚡",S=document.createElement("div");S.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",S.innerHTML=`
      ...
      <div style="display:flex;gap:12px;width:190px">
        <button id="pvp-gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
        <button id="pvp-gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
      </div>`,document.body.appendChild(S),(Y=S.querySelector("#pvp-gc-back"))==null||Y.addEventListener("click",()=>S.remove()),(J=S.querySelector("#pvp-gc-use"))==null||J.addEventListener("click",()=>{S.remove(),Q(B,_)})}function pe(){var y;const B=m[a+"Team"],_=Object.entries(B).flatMap(([b,v])=>(v||[]).filter(w=>!w.used).map(w=>({...w,_line:b})));if(!_.length)return;const g=document.createElement("div");g.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",g.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
        <div style="flex:1;font-size:14px;font-weight:700;color:#fff">⚡ Choisir un joueur pour +${m.boostValue}</div>
        <button id="bp-close" style="background:none;border:none;color:rgba(255,255,255,0.5);font-size:22px;cursor:pointer">✕</button>
      </div>
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${_.map(b=>{const v={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[b._line]||"#555",w=Se(b,b._line)+(b.boost||0);return`<div class="bp-item" data-cid="${b.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${v};overflow:hidden;cursor:pointer">
      ...
      </div>`,document.body.appendChild(_),(y=_.querySelector("#pvp-hist-close"))==null||y.addEventListener("click",()=>_.remove())}async function de(){if(m.phase!==a+"-attack")return;const B=a==="p1"?"p2":"p1",_=(m.round||0)+1,g=[...m.log||[]];g.push({type:"info",text:`⏭️ ${m[a+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const y=Te(m),b=Ce(B),v=y||!b?"finished":B+"-attack";await V({["selected_"+a]:[],modifiers:{...m.modifiers,[a]:{}},pendingAttack:null,phase:v,attacker:B,round:_,log:g}),v==="finished"&&await Pe(m)}async function me(){const B=m[a+"Team"],_=!["GK","DEF","MIL","ATT"].some(v=>(m[o+"Team"][v]||[]).some(w=>!w.used)),g=(m["selected_"+a]||[]).map(v=>{const w=(B[v._role]||[]).find(M=>M.cardId===v.cardId)||v,k=_&&["GK","DEF"].includes(v._role),z=B[v._role]||[],E=z.findIndex(M=>M.cardId===v.cardId),L=st(z.length),S=E>=0?L[E]:w._col??1;return{...w,_line:v._role,_col:S,...k?{note_a:Math.max(1,Number(w.note_a)||0)}:{}}});if(!g.length)return;const y=Kt(g,m.modifiers[a]||{});R(a,g.map(v=>v.cardId)),g.forEach(v=>{const w=(B[v._role]||[]).find(k=>k.cardId===v.cardId);w&&(w.used=!0)}),m["selected_"+a]=[],re();const b=[...m.log||[]];if(_){const v=(m[a+"Score"]||0)+1,w=g.map(S=>({name:S.name,note:Se(S,S._line||"ATT"),portrait:He(S),job:S.job}));b.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:w,homeTotal:y.total,aiTotal:0});const k=(m.round||0)+1,z=a==="p1"?"p2":"p1",E={...m,[a+"Team"]:B,[a+"Score"]:v},L=Te(E);j.add(k),ve(w,v,m[o+"Score"]||0,!0,async()=>{await V({[a+"Team"]:B,[a+"Score"]:v,["selected_"+a]:[],modifiers:{...m.modifiers,[a]:{}},pendingAttack:null,phase:L?"finished":z+"-attack",attacker:z,round:k,log:b}),L&&await Pe({...m,[a+"Score"]:v})});return}b.push({type:"pending",text:`⚔️ ${m[a+"Name"]} attaque (${y.total})`}),await V({[a+"Team"]:B,[o+"Team"]:m[o+"Team"],pendingAttack:{...y,players:g,side:a},["selected_"+a]:[],modifiers:{...m.modifiers,[a]:{}},phase:o+"-defense",log:b})}async function oe(){const B=m[a+"Team"],_=(m["selected_"+a]||[]).map(J=>{const U=(B[J._role]||[]).find(fe=>fe.cardId===J.cardId)||J,G=B[J._role]||[],W=G.findIndex(fe=>fe.cardId===J.cardId),te=st(G.length),ee=W>=0?te[W]:U._col??1;return{...U,_line:J._role,_col:ee}}),g=Vt(_,m.modifiers[a]||{});R(a,_.map(J=>J.cardId)),_.forEach(J=>{const U=(B[J._role]||[]).find(G=>G.cardId===J.cardId);U&&(U.used=!0)}),m["selected_"+a]=[],re();const y=Yt(m.pendingAttack.total,g.total,m.modifiers[a]||{}),b=m.pendingAttack.side,v=y==="attack"||(y==null?void 0:y.goal),w=b==="p1"?"p2":"p1",k=(m.round||0)+1,z=(m.pendingAttack.players||[]).map(J=>({name:J.name,note:Se(J,J._line||"ATT"),portrait:He(J),job:J.job})),E=[...m.log||[]];E.push({type:"duel",isGoal:v,homeScored:v&&b===a,text:v?`⚽ BUT de ${m[b+"Name"]} ! (${m.pendingAttack.total} vs ${g.total})`:`✋ Attaque stoppée (${m.pendingAttack.total} vs ${g.total})`,homePlayers:z,aiPlayers:_.map(J=>({name:J.name,note:Se(J,J._line||"DEF"),portrait:He(J),job:J.job})),homeTotal:m.pendingAttack.total,aiTotal:g.total});const L=v?(m[b+"Score"]||0)+1:m[b+"Score"]||0,S={...m,[a+"Team"]:B,[b+"Score"]:L},M=Te(S),F=M?"finished":w+"-attack",Y=async()=>{await V({[a+"Team"]:B,[o+"Team"]:m[o+"Team"],[b+"Score"]:L,["selected_"+a]:[],modifiers:{...m.modifiers,[a]:{}},pendingAttack:null,phase:F,attacker:w,round:k,log:E}),(F==="finished"||M)&&await Pe({...m,[b+"Score"]:L})};if(v){const J=b===a,U=J?L:m[a+"Score"]||0,G=J?m[o+"Score"]||0:L;j.add(k),ve(z,U,G,J,Y)}else await Y()}function ue(B){return["MIL","ATT"].some(_=>(B[_]||[]).some(g=>!g.used))}function Le(B){return["GK","DEF","MIL","ATT"].some(_=>(B[_]||[]).some(g=>!g.used))}function Ie(B){return Le(B)&&!ue(B)}function Ce(B){const _=m[B+"Team"],g=m[(B==="p1"?"p2":"p1")+"Team"];return!!(ue(_)||!Le(g)&&Ie(_))}function Te(B){const _=["MIL","ATT"].some(E=>(B.p1Team[E]||[]).some(L=>!L.used)),g=["MIL","ATT"].some(E=>(B.p2Team[E]||[]).some(L=>!L.used)),y=Le(B.p1Team),b=Le(B.p2Team);return!_&&!(!b&&y)&&(!g&&!(!y&&b))}function qe(B){const _=B.p1Score||0,g=B.p2Score||0;return _===g?null:_>g?h.home_id:h.away_id}async function Pe(B){try{const _=qe(B),g=_?h.home_id===_?h.away_id:h.home_id:null;await T.from("matches").update({status:"finished",winner_id:_,home_score:B.p1Score||0,away_score:B.p2Score||0}).eq("id",i),_&&g&&zi(_,g).catch(()=>{})}catch(_){console.error("[PvP] finishMatch:",_)}}function ze(){var b;const B=m[a+"Score"],_=m[o+"Score"],g=B>_,y=B===_;et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      <div style="font-size:64px">${g?"🏆":y?"🤝":"😤"}</div>
      <div style="font-size:24px;font-weight:900;color:#fff">${g?"Victoire !":y?"Match nul":"Défaite"}</div>
      <div style="font-size:32px;font-weight:900;color:#FFD700">${B} - ${_}</div>
      <button id="pvp-home" style="padding:16px 40px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">🏠 Retour</button>
    </div>`,(b=document.getElementById("pvp-home"))==null||b.addEventListener("click",()=>{try{T.removeChannel(N)}catch{}Ve(e),l("home")})}re()}const It="#1A6B3C",yt="#D4A017";async function Xn(e,t){var n;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const i=(n=t.state.params)==null?void 0:n.openLeagueId;if(i){t.state.params.openLeagueId=null,await wt(e,t,i);return}await zt(e,t)}async function zt(e,t,i="waiting"){var A;const{state:n}=t,d=n.profile.id,{data:s}=await T.from("mini_league_members").select("league_id").eq("user_id",d),r=(s||[]).map(p=>p.league_id),{data:l,error:f}=await T.from("mini_leagues").select("*, mini_league_members(count)").eq("status","waiting").eq("is_archived",!1).order("created_at",{ascending:!1}).limit(30),a=f?(await T.from("mini_leagues").select("*, mini_league_members(count)").eq("status","waiting").order("created_at",{ascending:!1}).limit(30)).data||[]:l||[],x=(r.length?await T.from("mini_leagues").select("*, mini_league_members(count)").in("id",r).order("created_at",{ascending:!1}):{data:[]}).data||[],u=x.filter(p=>p.status==="waiting"&&!p.is_archived),h=x.filter(p=>p.status==="active"&&!p.is_archived),$=x.filter(p=>p.is_archived||p.status==="finished"),m=a.filter(p=>!r.includes(p.id)),c=[{key:"waiting",label:"🟡 En attente",count:u.length+m.length},{key:"active",label:"🟢 En cours",count:h.length},{key:"archived",label:"📁 Archivées",count:$.length}];e.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:10px">
      ${i==="waiting"?Qn(u,m,d):i==="active"?Zn(h,d):er($,d)}
    </div>
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>${u.map((Q,R)=>{const D=$>0&&r.status==="finished"?R===0?Math.floor($*.7):R===1?Math.floor($*.2):R===2?Math.floor($*.1):0:0;return`<tr style="border-bottom:1px solid #f5f5f5;${Q.userId===s?"background:#f0fdf4;":""}">
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        ${Array.from({length:Math.max(0,r.status==="active"?r.current_day-1:r.current_day)},(Q,R)=>R+1).reverse().map(Q=>{const R=(f||[]).filter(D=>D.matchday===Q);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:#888;margin-bottom:6px">Journée ${Q}</div>${R.map(D=>Hi(D,x,s,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
    ...
    </div>`,document.body.appendChild(x),(u=x.querySelector("#prize-close"))==null||u.addEventListener("click",()=>x.remove()),x.addEventListener("click",$=>{$.target===x&&x.remove()}),(h=x.querySelector("#claim-prize-btn"))==null||h.addEventListener("click",async()=>{const{data:$}=await T.from("users").select("credits").eq("id",s.profile.id).single();await T.from("users").update({credits:(($==null?void 0:$.credits)||0)+a}).eq("id",s.profile.id),await T.from("mini_league_members").update({prize_claimed:!0}).eq("league_id",i.id).eq("user_id",s.profile.id),s.profile&&(s.profile.credits=(($==null?void 0:$.credits)||0)+a);const m=document.getElementById("nav-credits");m&&(m.textContent=`💰 ${s.profile.credits.toLocaleString("fr")}`),r(`💰 +${a.toLocaleString("fr")} cr. ajoutés à ton solde !`,"success"),x.remove(),wt(e,t,i.id)})}function dr(e,t){const n=e.length%2===0?[...e]:[...e,null],d=n.length;let s=n.slice(1);const r=[];for(let l=0;l<d-1;l++){const f=[],a=[n[0],...s];for(let o=0;o<d/2;o++){const x=a[o],u=a[d-1-o];x===null?f.push({bye:u}):u===null?f.push({bye:x}):f.push({home:x,away:u})}r.push(f),s=[s[s.length-1],...s.slice(0,-1)]}return t==="aller-retour"?[...r,...r.map(l=>l.map(f=>f.bye?f:{home:f.away,away:f.home}))]:r}function yo(e,t){const i={};return e.forEach(n=>{i[n.id]={userId:n.id,pseudo:n.pseudo,clubName:n.club_name||n.pseudo,played:0,won:0,drawn:0,lost:0,goalsFor:0,goalsAgainst:0,goalDiff:0,points:0}}),t.filter(n=>n.status==="finished"&&!n.is_bye&&n.home_score!=null).forEach(n=>{const d=i[n.home_id],s=i[n.away_id];!d||!s||(d.played++,s.played++,d.goalsFor+=n.home_score,d.goalsAgainst+=n.away_score,s.goalsFor+=n.away_score,s.goalsAgainst+=n.home_score,n.home_score>n.away_score?(d.won++,d.points+=3,s.lost++):n.home_score<n.away_score?(s.won++,s.points+=3,d.lost++):(d.drawn++,d.points++,s.drawn++,s.points++),d.goalDiff=d.goalsFor-d.goalsAgainst,s.goalDiff=s.goalsFor-s.goalsAgainst)}),Object.values(i).sort((n,d)=>d.points-n.points||d.goalDiff-n.goalDiff||d.goalsFor-n.goalsFor)}async function Ui(e,t,i,n){var d,s;t.state.params={...t.state.params,matchMode:"mini-league",mlMatchId:i,leagueId:n};try{const r=(s=(d=t==null?void 0:t.state)==null?void 0:d.profile)==null?void 0:s.id;try{(T.getChannels?T.getChannels():[]).forEach(f=>{const a=f.topic||"";(a.includes("matchmaking")||a.includes("pvp-match"))&&T.removeChannel(f)})}catch(l){console.warn("[FriendMatch] cleanup canaux:",l)}r&&(await T.rpc("cancel_matchmaking",{p_user_id:r}).catch(()=>{}),await T.from("matchmaking_queue").delete().eq("user_id",r).then(()=>{},()=>{}))}catch{}await si(e,t,"random",({deckId:r,formation:l,starters:f,subsRaw:a,gcCardsEnriched:o,gcDefs:x,stadiumDef:u})=>{const h=$=>lr(e,t,r,l,f,a,$,x||[],i);if(!o.length){h([]);return}ai(e,o,h)})}async function lr(e,t,i,n,d,s,r=[],l=[],f,a){const{state:o,navigate:x,toast:u}=t,h=o.profile.id;let $=!1,m=null;et(e);try{await T.rpc("cancel_matchmaking",{p_user_id:h})}catch{}try{await T.from("matchmaking_queue").delete().eq("user_id",h)}catch{}const{data:c}=await T.from("mini_league_matches").select("*, home:users!home_id(id,pseudo,club_name), away:users!away_id(id,pseudo,club_name)").eq("id",f).single();if(!c){u("Match introuvable","error"),Ve(e),x("mini-league");return}if(c.home_id!==h&&c.away_id!==h){u("Tu ne fais pas partie de ce match","error"),Ve(e),x("mini-league");return}const A=c.home_id===h,p=A?c.away:c.home,I=A?c.away_id:c.home_id,q=(P,C)=>{var N;e.innerHTML=`
      <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:20px;background:linear-gradient(180deg,#0a1628,#1a0a2e);padding:24px;text-align:center;color:#fff">
        ...
        <button id="lobby-cancel" style="padding:10px 24px;border-radius:10px;border:1.5px solid rgba(255,255,255,0.2);background:transparent;color:rgba(255,255,255,0.6);font-size:13px;cursor:pointer">Annuler</button>
        <style>@keyframes lobbyPulse{0%,100%{opacity:1}50%{opacity:0.4}}</style>
      </div>`,(N=document.getElementById("lobby-cancel"))==null||N.addEventListener("click",()=>{$=!0,clearInterval(m),Ve(e),x("mini-league")})},j=async(P,C)=>{$=!0,clearInterval(m),await new Promise(N=>setTimeout(N,600)),e.isConnected&&cr(e,t,P,C,r,l)};if(q(),A){let P=c.match_id;if(!P){const{data:N,error:V}=await T.from("matches").insert({home_id:h,away_id:I,home_deck_id:i,away_deck_id:null,status:"active",mode:"mini_league"}).select().single();if(V||!N){u("Erreur création match","error"),Ve(e),x("mini-league");return}P=N.id,await T.from("mini_league_matches").update({match_id:P,status:"playing"}).eq("id",f)}const C=P;m=setInterval(async()=>{if($){clearInterval(m);return}const{data:N}=await T.from("matches").select("away_deck_id").eq("id",C).single();N!=null&&N.away_deck_id&&(clearInterval(m),j(C,!0))},1500)}else{let P=c.match_id;m=setInterval(async()=>{if($){clearInterval(m);return}const{data:C}=await T.from("mini_league_matches").select("match_id").eq("id",f).single();C!=null&&C.match_id&&(clearInterval(m),P=C.match_id,await T.from("matches").update({away_deck_id:i}).eq("id",P),j(P,!1))},1500)}}async function cr(e,t,i,n,d=[],s=[]){var _;const{state:r,navigate:l,toast:f}=t,a=((_=r.params)==null?void 0:_.leagueId)||null,o=n?"p1":"p2",x=n?"p2":"p1",u=(d||[]).map(g=>g.id),h=(d||[]).map(g=>({id:g.id,gc_type:g.gc_type,_gcDef:g._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:$}=await T.from("matches").select("*").eq("id",i).single();if(!$){f("Match introuvable","error"),l("mini-league");return}async function m(){const[{data:g},{data:y},{data:b},{data:v}]=await Promise.all([T.rpc("get_deck_for_match",{p_deck_id:$.home_deck_id}),T.rpc("get_deck_for_match",{p_deck_id:$.away_deck_id}),T.from("users").select("id,pseudo,club_name").eq("id",$.home_id).single(),T.from("users").select("id,pseudo,club_name").eq("id",$.away_id).single()]),w=S=>{const M=Number(S.evolution_bonus)||0;return{cardId:S.card_id,position:S.position,id:S.id,firstname:S.firstname,name:S.surname_encoded,country_code:S.country_code,club_id:S.club_id,job:S.job,job2:S.job2,note_g:(Number(S.note_g)||0)+(S.job==="GK"||S.job2==="GK"&&Number(S.note_g)>0?M:0),note_d:(Number(S.note_d)||0)+(S.job==="DEF"||S.job2==="DEF"&&Number(S.note_d)>0?M:0),note_m:(Number(S.note_m)||0)+(S.job==="MIL"||S.job2==="MIL"&&Number(S.note_m)>0?M:0),note_a:(Number(S.note_a)||0)+(S.job==="ATT"||S.job2==="ATT"&&Number(S.note_a)>0?M:0),evolution_bonus:M,rarity:S.rarity,skin:S.skin,hair:S.hair,hair_length:S.hair_length,clubName:S.club_encoded_name||null,clubLogo:S.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},k=((g==null?void 0:g.starters)||[]).map(S=>w(S)),z=((y==null?void 0:y.starters)||[]).map(S=>w(S)),E=(g==null?void 0:g.formation)||"4-4-2",L=(y==null?void 0:y.formation)||"4-4-2";return{p1Team:(()=>{const S=vt(k,E);return stadiumDef?Tt(S,stadiumDef):S})(),p2Team:vt(z,L),p1Subs:(()=>{const S=((g==null?void 0:g.subs)||[]).map(M=>w(M));return stadiumDef&&jt(S,stadiumDef),S})(),p2Subs:((y==null?void 0:y.subs)||[]).map(S=>w(S)),p1Formation:E,p2Formation:L,p1Name:(b==null?void 0:b.club_name)||(b==null?void 0:b.pseudo)||"Joueur 1",p2Name:(v==null?void 0:v.club_name)||(v==null?void 0:v.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:n?u:[],gc_p2:n?[]:u,gcCardsFull_p1:n?h:[],gcCardsFull_p2:n?[]:h,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:s||[],lastActionAt:new Date().toISOString()}}let c=$.game_state&&Object.keys($.game_state).length?$.game_state:null;if(!c)if(n){c=await m();const{data:g}=await T.from("matches").select("game_state").eq("id",i).single();!(g!=null&&g.game_state)||!Object.keys(g.game_state).length?await T.from("matches").update({game_state:c,turn_user_id:$.home_id}).eq("id",i):c=g.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let g=0;g<60&&!c;g++){await new Promise(b=>setTimeout(b,400));const{data:y}=await T.from("matches").select("game_state").eq("id",i).single();y!=null&&y.game_state&&Object.keys(y.game_state).length&&(c=y.game_state)}if(!c){f("Erreur de synchronisation","error"),l("mini-league");return}c.gc_p2=u,c.gcCardsFull_p2=h,await T.from("matches").update({game_state:c}).eq("id",i)}let A=!1,p=!1,I=!1,q=null,j=!1;const P=new Set,C=new Set;function N(g){var S,M;try{T.removeChannel(V)}catch{}const y=c[o+"Score"]||0,b=c[x+"Score"]||0;if(!I){I=!0;const F=g.winner_id||(y>b?r.profile.id:b>y?"opp":null),Y=F===r.profile.id?"win":F?"loss":null;Y&&ni(()=>Promise.resolve().then(()=>oo),void 0).then(J=>J.applyOwnEvolution(r.profile.id,Y)).catch(()=>{})}if(!p){const F=c.p1Score||0,Y=c.p2Score||0,J=(c.usedGc_p1||[]).length,U=(c.usedGc_p2||[]).length,G=g.winner_id||(F>Y?$.home_id:Y>F?$.away_id:null);(G?r.profile.id===G:r.profile.id<(n?$.away_id:$.home_id))&&(p=!0,Ji({player1Id:$.home_id,player2Id:$.away_id,score1:F,score2:Y,gc1:J,gc2:U}).catch(te=>console.warn("[FriendMatch] updateStats:",te)))}let v,w;g.winner_id?(v=g.winner_id===r.profile.id,w=!1):g.forfeit?(v=y>b,w=!1):(w=y===b,v=y>b),(S=document.getElementById("pvp-end-overlay"))==null||S.remove();const k=document.createElement("div");k.id="pvp-end-overlay",k.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const z=w?"🤝":v?"🏆":"😞",E=w?"MATCH NUL":v?"VICTOIRE !":"DÉFAITE",L=w?"#fff":v?"#FFD700":"#ff6b6b";k.innerHTML=`
      <div style="font-size:64px">${z}</div>
      <div style="font-size:26px;font-weight:900;color:${L}">${E}</div>
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center">
      ...
      `).join(""):'<div style="text-align:center;color:var(--gray-600);padding:40px">Aucun manager.</div>'}else{const A=r||[];$.innerHTML=A.length>0?A.map((p,I)=>{const q=Nt(p.mmr??1e3),j=(p.ranked_wins||0)+(p.ranked_losses||0)+(p.ranked_draws||0),P=j>0?Math.round((p.ranked_wins||0)/j*100):0,C=p.id===i.profile.id,N=(p.placement_matches||0)<10;return`
  ...
  <div class="page">
    ...
    </div>`,(P=document.getElementById("ranked-back"))==null||P.addEventListener("click",()=>n("home"));return}const f=l.mmr??1e3,a=l.mmr_deviation??350,o=l.mmr_volatility??.06,x=l.placement_matches??0,u=x<10,h=Math.max(0,10-x),$=Nt(f),m=vo(f),c=li.findIndex(K=>K.id===$.id),A=li[c+1]||null,p={bronze:"linear-gradient(160deg,#3d1c00,#7a3e00)",silver:"linear-gradient(160deg,#1a1a2e,#3a3a5e)",gold:"linear-gradient(160deg,#1a1200,#4a3500)",platinum:"linear-gradient(160deg,#001a20,#003040)",diamond:"linear-gradient(160deg,#001030,#1a2860)",master:"linear-gradient(160deg,#1a0030,#3d0070)"},I=(l.ranked_wins||0)+(l.ranked_losses||0)+(l.ranked_draws||0),q=I>0?Math.round((l.ranked_wins||0)/I*100):0,j=li.map(K=>`
    <div style="display:flex;flex-direction:column;align-items:center;gap:2px;opacity:${$.id===K.id?1:.35};
      transform:${$.id===K.id?"scale(1.15)":"scale(1)"};transition:all 0.3s">
      <div style="font-size:${$.id===K.id?"28px":"20px"}">${K.emoji}</div>
      <div style="font-size:9px;color:${K.color};font-weight:${$.id===K.id?"900":"400"};letter-spacing:0.5px">${K.label.toUpperCase()}</div>
    </div>
  `).join("");e.innerHTML=`
  <div style="min-height:100%;background:${p[$.id]};padding:16px;overflow-y:auto;display:flex;flex-direction:column;gap:16px">
    ...
    <div style="background:rgba(0,0,0,0.3);border-radius:12px;padding:12px 16px">
      ...
      <div style="background:rgba(255,255,255,0.1);border-radius:6px;height:10px;overflow:hidden">
        <div style="height:100%;width:${m}%;background:linear-gradient(90deg,${$.color},${$.color}aa);border-radius:6px;transition:width 0.8s ease"></div>
      </div>
    ...
    <div style="display:flex;flex-direction:column;gap:10px;margin-top:auto;padding-top:8px">
      ...
      away:users!away_id(pseudo,club_name)`).or(`home_id.eq.${d.id},away_id.eq.${d.id}`).order("created_at",{ascending:!1}).limit(50),r={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},l=(s||[]).filter(o=>o.status==="finished"),f=(s||[]).filter(o=>o.status==="in_progress");function a(o){const x=o.home_id===d.id;x?o.home_score:o.away_score,x?o.away_score:o.home_score;const u=o.winner_id===d.id,h=o.home_score===o.away_score&&o.status==="finished",$=o.status!=="finished"?"…":h?"N":u?"V":"D",m=o.status!=="finished"||h?"#888":u?"#1A6B3C":"#c0392b";let c=r[o.mode]||o.mode;o.away_id===null&&!c.startsWith("IA")&&(c="IA");const p=o.home_id===d.id?o.away:o.home;let I;o.away_id===null?I=c:p?I=`${p.club_name||p.pseudo} (${p.pseudo})`:I="Adversaire";let q="";o.status==="in_progress"&&Date.now()-new Date(o.created_at).getTime()>3600*1e3&&(q=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const j=new Date(o.created_at),P=j.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+j.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),C=o.status==="finished"?`${o.home_score} - ${o.away_score}`:`${o.home_score||0} - ${o.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
      ...
      <div style="display:flex;align-items:center;gap:8px">
        ...
        <span style="background:${m};color:#fff;width:22px;height:22px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:900">${$}</span>
      </div>
    </div>`}e.innerHTML=`
  <div class="page">
    <div class="page-header">
