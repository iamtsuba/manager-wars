import{s as $,F as oi,h as zt,j as Li,l as Ze,i as tn,k as nn,b as on}from"./formation-links-CEzN0rcz.js";function xi(e,{navigate:t,toast:n}){e.innerHTML=`
  ...
  `;function d(){var k;const i=document.getElementById("logo-preview"),r=document.getElementById("logo-initials"),u=((k=document.getElementById("setup-club"))==null?void 0:k.value)||"MW",m=u.trim().split(" ").filter(Boolean),g=m.length>=2?(m[0][0]+m[1][0]).toUpperCase():u.slice(0,2).toUpperCase();i&&(i.style.background=s,i.style.borderColor=l),r&&(r.textContent=g,r.style.color=l)}document.getElementById("color1").addEventListener("input",i=>{l=i.target.value,document.getElementById("swatch1").style.background=l,d()}),document.getElementById("color2").addEventListener("input",i=>{s=i.target.value,document.getElementById("swatch2").style.background=s,d()});function c(i){document.querySelectorAll(".setup-step").forEach(r=>r.classList.remove("active")),document.getElementById(`step-${i}`).classList.add("active"),document.getElementById("step-num").textContent=i,document.getElementById("progress-fill").style.width=`${Math.round(i/3*100)}%`,i===3&&d()}document.getElementById("step1-next").addEventListener("click",async()=>{const i=document.getElementById("setup-pseudo").value.trim(),r=document.getElementById("step1-error");if(r.textContent="",i.length<3){r.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:u}=await $.from("users").select("id").eq("pseudo",i).maybeSingle();if(u){r.textContent="Ce pseudo est déjà pris.";return}c(2)}),document.getElementById("step2-back").addEventListener("click",()=>c(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const i=document.getElementById("setup-club").value.trim(),r=document.getElementById("step2-error");if(r.textContent="",i.length<2){r.textContent="Nom trop court (min. 2 caractères).";return}const{data:u}=await $.from("users").select("id").eq("club_name",i).maybeSingle();if(u){r.textContent="Ce nom de club est déjà pris.";return}c(3)}),document.getElementById("step3-back").addEventListener("click",()=>c(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const i=document.getElementById("setup-pseudo").value.trim(),r=document.getElementById("setup-club").value.trim(),u=document.getElementById("step3-error"),m=document.getElementById("step3-finish");u.textContent="",m.disabled=!0,m.textContent="Création en cours…";try{const{error:g}=await $.from("users").insert({id:t.user.id,pseudo:i,club_name:r,club_color1:l,club_color2:s,credits:1e4});if(g)throw g;await an(t.user.id),await p(),a(`Bienvenue ${i} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(g){u.textContent=g.message,m.disabled=!1,m.textContent="🚀 Créer mon profil !"}})}async function an(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await $.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(n){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",n)}}const sn="modulepreload",dn=function(e){return"/manager-wars/"+e},yi={},ln=function(t,n,a){let p=Promise.resolve();if(n&&n.length>0){document.getElementsByTagName("link");const s=document.querySelector("meta[property=csp-nonce]"),d=(s==null?void 0:s.nonce)||(s==null?void 0:s.getAttribute("nonce"));p=Promise.allSettled(n.map(c=>{if(c=dn(c),c in yi)return;yi[c]=!0;const i=c.endsWith(".css"),r=i?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${c}"]${r}`))return;const u=document.createElement("link");if(u.rel=i?"stylesheet":sn,i||(u.as="script"),u.crossOrigin="",u.href=c,d&&u.setAttribute("nonce",d),document.head.appendChild(u),i)return new Promise((m,g)=>{u.addEventListener("load",m),u.addEventListener("error",()=>g(new Error(`Unable to preload CSS for ${c}`)))})}))}function l(s){const d=new Event("vite:preloadError",{cancelable:!0});if(d.payload=s,window.dispatchEvent(d),!d.defaultPrevented)throw s}return p.then(s=>{for(const d of s||[])d.status==="rejected"&&l(d.reason);return t().catch(l)})},Tt="#1A6B3C",It="#cc2222",cn="#D4A017",hi="#888";async function pn(e,t){const{state:n,toast:a}=t;e.innerHTML=`
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      ${m.map(({friendshipId:g,friend:k})=>un(k,g)).join("")}
    </div>`,i.querySelectorAll("[data-stats]").forEach(g=>{g.addEventListener("click",()=>gn(e,g.dataset.stats,g.dataset.friendName))}),i.querySelectorAll("[data-match]").forEach(g=>{g.addEventListener("click",()=>{const k=g.dataset.friendId,o=g.dataset.friendName;console.log("[Friends] clic match",{fid:k,fname:o,hasNavigate:typeof a}),typeof a=="function"?a("match",{matchMode:"friend",friendId:k,friendName:o}):t("Erreur navigation","error")})})}function un(e,t){const n=e.club_name||e.pseudo||"?",a=e.pseudo||"",p=(a||n).slice(0,2).toUpperCase(),l=e.club_color2||Tt,s=e.club_color1||"#ffffff",d=e.last_seen_at?new Date(e.last_seen_at):null,c=d&&Date.now()-d.getTime()<3*60*1e3;return`
    ...
    ${ai()}`,document.body.appendChild(n);const a=n.querySelector("#friend-pseudo-input"),p=n.querySelector("#add-friend-error"),l=()=>n.remove();a.focus(),n.querySelector("#add-cancel").addEventListener("click",l),n.addEventListener("click",s=>{s.target===n&&l()}),n.querySelector("#add-ok").addEventListener("click",async()=>{const s=a.value.trim();if(!s){p.textContent="Entre un pseudo";return}p.textContent="";const{data:d}=await $.from("users").select("id, pseudo").ilike("pseudo",s).single();if(!d){p.textContent="Utilisateur introuvable";return}if(d.id===e.user.id){p.textContent="Tu ne peux pas t'ajouter toi-même";return}const{data:c}=await $.from("friendships").select("id, status").or(`and(requester_id.eq.${e.user.id},addressee_id.eq.${d.id}),and(requester_id.eq.${d.id},addressee_id.eq.${e.user.id})`).single();if(c){const r=c.status==="accepted"?"Vous êtes déjà amis !":c.status==="pending"?"Demande déjà envoyée":"Une demande existe déjà";p.textContent=r;return}const{error:i}=await $.from("friendships").insert({requester_id:e.user.id,addressee_id:d.id,status:"pending"});if(i){p.textContent="Erreur : "+i.message;return}l(),t(`✅ Demande envoyée à ${d.pseudo} !`,"success")})}async function Mi(e,t,n=null,a={}){const p=e.user.id,{data:l}=await $.from("friendships").select("id, requester_id").eq("addressee_id",p).eq("status","pending").order("created_at",{ascending:!1}),s=(l||[]).map(m=>m.requester_id);let d={};if(s.length){const{data:m}=await $.from("users").select("id, pseudo, club_name").in("id",s);(m||[]).forEach(g=>{d[g.id]=g})}const c=(l||[]).map(m=>({...m,requester:d[m.requester_id]||{pseudo:"?"}})),i=ri(),r=c||[];i.innerHTML=`
    <div class="popup-box">
      ...
      ${r.length?`<div style="display:flex;flex-direction:column;gap:8px;max-height:50vh;overflow-y:auto;margin-bottom:14px">
            ${r.map(m=>{var g,k,o;return`
              <div style="display:flex;align-items:center;gap:10px;background:#f9f9f9;border-radius:10px;padding:10px 12px">
                <div style="flex:1;font-size:14px;font-weight:700">${((g=m.requester)==null?void 0:g.club_name)||((k=m.requester)==null?void 0:k.pseudo)||"?"}
    ...
    ${ai()}`,document.body.appendChild(i);const u=()=>i.remove();i.querySelector("#pending-close").addEventListener("click",u),i.addEventListener("click",m=>{m.target===i&&u()}),i.querySelectorAll("[data-accept]").forEach(m=>{m.addEventListener("click",async()=>{const{error:g}=await $.from("friendships").update({status:"accepted"}).eq("id",m.dataset.accept);if(g){t("Erreur : "+g.message,"error");return}m.closest("div[style]").remove(),t("✅ Ami accepté !","success"),zi(e,t,a),n&&n()})}),i.querySelectorAll("[data-decline]").forEach(m=>{m.addEventListener("click",async()=>{await $.from("friendships").delete().eq("id",m.dataset.decline),m.closest("div[style]").remove(),t("Demande refusée","info"),n&&n()})})}async function gn(e,t,n){const a=e.user.id,[p,l]=[a,t].sort(),s=a===p,{data:d}=await $.from("friend_match_stats").select("*").eq("player1_id",p).eq("player2_id",l).single(),c=e.profile.club_name||e.profile.pseudo||"Moi",i=d||{},r=s?i.wins_p1||0:i.wins_p2||0,u=s?i.wins_p2||0:i.wins_p1||0,m=i.draws||0,g=s?i.goals_p1||0:i.goals_p2||0,k=s?i.goals_p2||0:i.goals_p1||0,o=s?i.gc_used_p1||0:i.gc_used_p2||0,_=s?i.gc_used_p2||0:i.gc_used_p1||0,z=i.matches_total||0,M=(U,q,j,R=Tt,te=It)=>`
    ...
    </style>`}async function mn({player1Id:e,player2Id:t,score1:n,score2:a,gc1:p,gc2:l}){const[s,d]=[e,t].sort(),c=e!==s,i=c?a:n,r=c?n:a,u=c?l:p,m=c?p:l,g=i>r?1:0,k=r>i?1:0,o=i===r?1:0,{data:_}=await $.from("friend_match_stats").select("*").eq("player1_id",s).eq("player2_id",d).single();_?await $.from("friend_match_stats").update({wins_p1:_.wins_p1+g,wins_p2:_.wins_p2+k,draws:_.draws+o,goals_p1:_.goals_p1+i,goals_p2:_.goals_p2+r,gc_used_p1:_.gc_used_p1+u,gc_used_p2:_.gc_used_p2+m,matches_total:_.matches_total+1}).eq("player1_id",s).eq("player2_id",d):await $.from("friend_match_stats").insert({player1_id:s,player2_id:d,wins_p1:g,wins_p2:k,draws:o,goals_p1:i,goals_p2:r,gc_used_p1:u,gc_used_p2:m,matches_total:1})}const xn="2026.06.28-2332";async function bi(e,{state:t,navigate:n,toast:a}){var l,s;const p=t.profile;p&&(e.innerHTML=`
  <div class="page">
    <div class="page-body">
  ...
  `,e.querySelectorAll("[data-nav]").forEach(d=>{d.addEventListener("click",c=>{c.preventDefault(),n(d.dataset.nav)})}),(l=document.getElementById("nav-rankings"))==null||l.addEventListener("click",()=>n("rankings")),(s=document.getElementById("nav-matches"))==null||s.addEventListener("click",()=>n("matches")),e.querySelectorAll("[data-action]").forEach(d=>{d.addEventListener("click",()=>{d.classList.add("tapped"),setTimeout(()=>d.classList.remove("tapped"),200);const c=d.dataset.action;if(c==="match-ai"){vn(n);return}if(c==="match-random"){n("match",{matchMode:"random"});return}if(c==="match-friend"){n("friends");return}a("Bientôt disponible","info")})}),document.getElementById("logout-btn").addEventListener("click",async()=>{await $.auth.signOut(),window.location.reload()}),Ci(t,a),bn(t,a,n),Si(t,a,n))}async function Si(e,t,n){const a=document.getElementById("ongoing-match-banner");if(!a)return;const p=e.profile.id,{data:l}=await $.from("matches").select("id, home_id, away_id, status, mode").eq("status","active").or(`home_id.eq.${p},away_id.eq.${p}`).order("created_at",{ascending:!1});if(!(l!=null&&l.length)){a.innerHTML="";return}const s=l.map(c=>c.home_id===p?c.away_id:c.home_id).filter(Boolean);let d={};if(s.length){const{data:c}=await $.from("users").select("id, pseudo, club_name").in("id",s);(c||[]).forEach(i=>{d[i.id]=i.club_name||i.pseudo})}a.innerHTML=l.map(c=>{const i=c.home_id===p?c.away_id:c.home_id,r=d[i]||"Adversaire";return`
    ...
    </div>`,(i=document.getElementById("match-inv-accept"))==null||i.addEventListener("click",()=>{a.innerHTML="",n("match",{matchMode:"friend",friendId:s,friendName:l})}),(r=document.getElementById("match-inv-decline"))==null||r.addEventListener("click",async()=>{await $.from("friend_match_invites").update({status:"declined"}).eq("id",p.id),a.innerHTML="",t("Invitation refusée","info")})}async function Ci(e,t){const n=document.getElementById("friend-requests-banner");if(!n)return;const{data:a,error:p}=await $.from("friendships").select("id, requester:users!requester_id(pseudo, club_name)").eq("addressee_id",e.user.id).eq("status","pending");if(p||!(a!=null&&a.length)){n.innerHTML="";return}const l=a.length,s=a.slice(0,2).map(c=>{var i;return((i=c.requester)==null?void 0:i.pseudo)||"?"}).join(", "),d=l>2?` +${l-2}`:"";n.innerHTML=`
  ...
  </div>`}async function kn(e,t){const{state:n,navigate:a,toast:p,openModal:l,closeModal:s}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:d}=await $.from("cards").select(`id, card_type, current_note, gc_type, formation, is_for_sale, sale_price,
      player:players(id, firstname, surname_encoded, country_code, club_id, job, job2,
        note_g, note_d, note_m, note_a, rarity, note_min, note_max, skin, hair, hair_length, sell_price,
        clubs(encoded_name, logo_url))`).eq("owner_id",n.profile.id),{data:c}=await $.from("players").select(`id, firstname, surname_encoded, country_code, club_id, job, job2,
      note_g, note_d, note_m, note_a, rarity, note_min, note_max, skin, hair, hair_length,
      clubs(encoded_name, logo_url)`).eq("is_active",!0),i=(d||[]).filter(O=>O.card_type==="player"&&O.player),r=(d||[]).filter(O=>O.card_type==="game_changer"),u=(d||[]).filter(O=>O.card_type==="formation"),{data:m}=await $.from("gc_definitions").select("name,gc_type,color,effect,image_url"),g={};(m||[]).forEach(O=>{g[O.name]=O});const k=Object.keys(oi),o=Object.keys(Ge),_={};i.forEach(O=>{const L=O.player.id;_[L]=(_[L]||0)+1}),new Set(Object.keys(_).map(O=>String(O)));const z=new Set(u.map(O=>O.formation)),M=new Set(r.map(O=>O.gc_type));let F="player",U="Tous",q="",j=!1;function R(){return[...i].sort((O,L)=>{const N=jt.indexOf(O.player.job),B=jt.indexOf(L.player.job);return N!==B?N-B:(O.player.surname_encoded||"").localeCompare(L.player.surname_encoded||"")})}function te(){return[...c||[]].sort((O,L)=>{const N=jt.indexOf(O.job),B=jt.indexOf(L.job);return N!==B?N-B:(O.surname_encoded||"").localeCompare(L.surname_encoded||"")})}function ce(){return R().filter(O=>{const L=O.player,N=U==="Tous"||L.job===U,B=!q||`${L.firstname} ${L.surname_encoded}`.toLowerCase().includes(q);return N&&B})}function ne(){return te().filter(O=>{const L=U==="Tous"||O.job===U,N=!q||`${O.firstname} ${O.surname_encoded}`.toLowerCase().includes(q);return L&&N})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
  ...
  </div>`;function S(){const O=document.getElementById("col-filters");O&&(F==="player"?(O.innerHTML=`
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",L=>{q=L.target.value.toLowerCase(),Y()}),e.querySelectorAll(".filter-btn").forEach(L=>{L.addEventListener("click",()=>{U=L.dataset.job,S(),Y()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{j=!j,S(),Y()})):(O.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{j=!j,S(),Y()})))}function Y(){const O=document.getElementById("col-grid");O&&(F==="player"?he(O):F==="formation"?xe(O):ge(O))}function ee(O,L,N,B,V){V=V||"#7a28b8";const re=document.getElementById("col-grid"),ie=document.getElementById("col-big");if(!re||!ie)return;var ke=0;function Ee(){ie.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+L(O[ke])+"</div>";var $e=ie.querySelector("[data-card-id],[data-form-id],[data-gc-id]");$e&&$e.addEventListener("click",function(){B(O[ke])}),requestAnimationFrame(function(){var _e=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!_e||!ie)){var Ae=ie.clientHeight-8,Te=ie.clientWidth-24,qe=_e.offsetHeight,Ne=_e.offsetWidth;if(qe>0&&Ne>0&&Ae>40){var He=Math.min(Ae/qe,Te/Ne,1);_e.style.transform="scale("+He.toFixed(3)+")",_e.style.transformOrigin="top center"}}}),re.innerHTML=O.map(function(_e,Ae){return'<div class="col-mini-item" data-idx="'+Ae+'" style="flex-shrink:0;cursor:pointer;border-radius:8px;border:1.5px solid '+(Ae===ke?V:"transparent")+';transition:border-color .15s;overflow:hidden">'+N(_e,Ae===ke)+"</div>"}).join(""),re.querySelectorAll(".col-mini-item").forEach(function(_e){_e.addEventListener("click",function(){ke=Number(_e.dataset.idx),Ee(),_e.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}Ee()}function D(O){var L=.54,N=Math.round(140*L),B=Math.round(310*L),V;if(!O||O._fake){var re=O?O.player:null;if(!re)return"";V=wi(re)}else V=Vt(O,"");return'<div style="width:'+N+"px;height:"+B+'px;overflow:hidden;position:relative;flex-shrink:0"><div style="transform:scale('+L+');transform-origin:top left;position:absolute;top:0;left:0;pointer-events:none">'+V+"</div></div>"}function Q(O,L,N){L=L||100,N=N||140;var B=zt[O]||{},V={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},re=Math.max(3,Math.round(L*.06)),ie=Object.entries(B).map(function(Ee){var $e=Ee[0],_e=Ee[1],Ae=$e.replace(/\d+$/,""),Te=V[Ae]||"#888",qe=Math.round(_e.x*L),Ne=Math.round(_e.y*N);return'<circle cx="'+qe+'" cy="'+Ne+'" r="'+re+'" fill="'+Te+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),ke=Math.max(1,Math.round(L/50));return'<svg viewBox="0 0 '+L+" "+N+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+L+'" height="'+N+'" fill="#1A6B3C"/><rect x="'+Math.round(L*.2)+'" y="'+Math.round(N*.02)+'" width="'+Math.round(L*.6)+'" height="'+Math.round(N*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+ke+'"/><line x1="0" y1="'+Math.round(N*.5)+'" x2="'+L+'" y2="'+Math.round(N*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+ke+'"/><ellipse cx="'+Math.round(L*.5)+'" cy="'+Math.round(N*.5)+'" rx="'+Math.round(L*.18)+'" ry="'+Math.round(N*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+ke+'"/><rect x="'+Math.round(L*.2)+'" y="'+Math.round(N*.82)+'" width="'+Math.round(L*.6)+'" height="'+Math.round(N*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+ke+'"/>'+ie+"</svg>"}function ae(O,L,N){var B=N>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+N+"</div>":"",V=L?'data-form-id="'+L.id+'"':"",re=O.length>7?14:O.length>5?16:19,ie=!!L;return"<div "+V+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(ie?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(ie?"":"filter:grayscale(1);opacity:0.5")+'">'+B+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(ie?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+re+"px;font-weight:900;color:"+(ie?"#1A6B3C":"#aaa")+';line-height:1">'+O+'</div></div><div style="flex:1;overflow:hidden;background:'+(ie?"#1A6B3C":"#ccc")+'">'+Q(O,140,220)+"</div></div>"}function pe(O,L){var N=.54,B=Math.round(140*N),V=Math.round(305*N),re=Math.round(V*.22),ie=V-re,ke=O.length>7?5:7,Ee=Q(O,B,ie),$e=L?"1.5px solid #2a7a40":"1px solid #ddd",_e=L?"":"filter:grayscale(1);opacity:0.45;",Ae=L?"#1A6B3C":"#bbb",Te="#fff";return'<div style="width:'+B+"px;height:"+V+"px;border-radius:6px;border:"+$e+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+_e+'"><div style="height:'+re+"px;background:"+Ae+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+ke+"px;font-weight:900;color:"+Te+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(B-4)+'px">'+O+'</span></div><div style="height:'+ie+'px;overflow:hidden;flex-shrink:0">'+Ee+"</div></div>"}function he(O){if(j){const L=ne();if(!L.length){O.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const N=L.map(B=>i.find(V=>V.player.id===B.id)||{_fake:!0,player:B,id:"fake-"+B.id});ee(N,B=>B._fake?wi(B.player):Vt(B,""),B=>B._fake?D({player:B.player,id:"x",_fake:!0}):D(B),B=>{B._fake||_i(B,i,_,t)},"#1A6B3C")}else{const L=ce();if(!L.length){O.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const N={},B=[];L.forEach(V=>{N[V.player.id]||(N[V.player.id]=!0,B.push(V))}),ee(B,V=>{const re=_[V.player.id]||1,ie=re>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${re}</div>`:"",Ee=i.filter($e=>$e.player.id===V.player.id&&$e.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Vt(V,ie+Ee)},V=>D(V),V=>_i(V,i,_,t),"#1A6B3C")}}function xe(O){const L=j?k:[...z];if(!L.length){O.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const N=L.map(B=>({formation:B,card:u.find(V=>V.formation===B)||null,owned:z.has(B)}));ee(N,({formation:B,card:V,owned:re})=>ae(B,re?V:null,re?u.filter(ie=>ie.formation===B).length:0),({formation:B,owned:V})=>pe(B,V),({card:B,owned:V})=>{V&&B&&An(B,u,t,l)},"#1A6B3C")}function ge(O){const L=Object.keys(g),N=j?L.length?L:o:[...M];if(!N.length){O.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const B=N.map(V=>({type:V,gc:Ge[V]||{icon:"⚡",desc:""},def:g[V]||null,owned:M.has(V),card:r.find(re=>re.gc_type===V)||null}));ee(B,({type:V,gc:re,def:ie,owned:ke,card:Ee})=>{const $e=ke?r.filter(x=>x.gc_type===V).length:0,_e=$e>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${$e}</div>`:"",Ae=(ie==null?void 0:ie.gc_type)==="ultra_game_changer",Te={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},qe={purple:"#b06ce0",light_blue:"#00d4ef"},Ne=Te[ie==null?void 0:ie.color]||Te.purple,He=qe[ie==null?void 0:ie.color]||qe.purple,T=(ie==null?void 0:ie.effect)||re.desc||"",f=ie!=null&&ie.image_url?`/manager-wars/icons/${ie.image_url}`:null;return ke&&Ee?`<div data-gc-id="${Ee.id}" data-gc-type="${V}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${He};background:${Ne};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${He}66;cursor:pointer">
          ${_e}
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Bt=1e3;function An(e,t,n,a){var k,o,_;const{state:p,toast:l,closeModal:s,navigate:d,refreshProfile:c}=n,i=e.formation,r={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function u(){const z=zt[i]||{},M=oi[i]||[],F=290,U=360,q=20;function j(te){const ce=z[te];return ce?{x:ce.x*F,y:ce.y*U}:null}let R=`<svg width="${F}" height="${U}" viewBox="0 0 ${F} ${U}" xmlns="http://www.w3.org/2000/svg">`;for(const[te,ce]of M){const ne=j(te),S=j(ce);!ne||!S||(R+=`<line x1="${ne.x}" y1="${ne.y}" x2="${S.x}" y2="${S.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const te of Object.keys(z)){const ce=j(te);if(!ce)continue;const ne=te.replace(/\d+/,""),S=r[ne]||"#555";R+=`<circle cx="${ce.x}" cy="${ce.y}" r="${q}" fill="${S}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,R+=`<text x="${ce.x}" y="${ce.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${ne}</text>`}return R+="</svg>",R}const m=t.filter(z=>z.formation===i);m.length;const g=!e.is_for_sale;a(`Formation ${i}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(k=document.getElementById("direct-sell-form-btn"))==null||k.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${i} pour ${Bt.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const z=m.find(F=>!F.is_for_sale)||m[0];if(!z){l("Aucune carte à vendre","error");return}await $.from("market_listings").update({status:"cancelled"}).eq("card_id",z.id).eq("status","active"),await $.from("transfer_history").delete().eq("card_id",z.id);const{error:M}=await $.from("cards").delete().eq("id",z.id);if(M){l(M.message,"error");return}await $.from("users").update({credits:(p.profile.credits||0)+Bt}).eq("id",p.profile.id),await c(),l(`+${Bt.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),s(),d("collection")}),(o=document.getElementById("market-sell-form-btn"))==null||o.addEventListener("click",async()=>{const z=parseInt(document.getElementById("sell-price-form").value);if(!z||z<1){l("Prix invalide","error");return}await $.from("cards").update({is_for_sale:!0,sale_price:z}).eq("id",e.id),await $.from("market_listings").insert({seller_id:p.profile.id,card_id:e.id,price:z}),l("Carte mise en vente sur le marché !","success"),s(),d("collection")}),(_=document.getElementById("cancel-sell-form-btn"))==null||_.addEventListener("click",async()=>{await $.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await $.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),l("Annonce retirée","success"),s(),d("collection")})}async function _i(e,t,n,a){var Q,ae,pe,he,xe,ge,O;const{state:p,toast:l,openModal:s,closeModal:d,navigate:c,refreshProfile:i}=a,r=e.player,u=t.filter(L=>L.player.id===r.id),m=u.length,g=Math.max(Number(r.note_g)||0,Number(r.note_d)||0,Number(r.note_m)||0,Number(r.note_a)||0),k=r.rarity||"normal",{data:o}=await $.from("sell_price_configs").select("*").eq("rarity",k).lte("note_min",g).gte("note_max",g).order("note_min",{ascending:!1}).limit(1),_=((Q=o==null?void 0:o[0])==null?void 0:Q.price)??$n[k]??1e3,z=r.rarity!=="legende",M=qi(r),F=Lt(r,r.job),U=r.job2?Lt(r,r.job2):null,q=Et[r.job]||"#1A6B3C",j=r.job2?Et[r.job2]:null,R=Fi[r.rarity]||"#ccc",te=si[r.country_code]||r.country_code||"",ce=u.map(L=>L.id);let ne={};if(ce.length){const{data:L}=await $.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",ce).order("transferred_at",{ascending:!0});(L||[]).forEach(N=>{ne[N.card_id]||(ne[N.card_id]=[]),ne[N.card_id].push(N)})}const S=L=>{const N=L.transferred_at?new Date(L.transferred_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"2-digit",year:"numeric"}):"",B=L.source==="booster"?"Booster":L.price?L.price.toLocaleString("fr"):"—";return`
    <div style="display:flex;justify-content:space-between;align-items:flex-start;background:#f9f9f9;border-radius:8px;padding:8px 12px;gap:8px">
      ...
      <div style="text-align:right;flex-shrink:0">
        <div style="font-size:13px;font-weight:700;color:${L.source==="booster"?"var(--gray-600)":"var(--yellow)"}">${B}</div>
    ...
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${u.map((L,N)=>{const B=ne[L.id]||[],V=L.is_for_sale,re=B.length?B[B.length-1]:null;return`
            <div data-card-id="${L.id}" data-card-idx="${N}" class="exemplaire-row"
              ...
              <div style="display:flex;align-items:center;gap:10px">
                ...
                <div style="flex:1;min-width:0">
                  <div style="font-size:12px;font-weight:700">Exemplaire ${N+1}${V?" 🏷️ En vente":""}</div>
                ...
                <button class="expl-hist-toggle" data-idx="${N}" style="font-size:11px;color:#999;border:none;background:none;cursor:pointer;padding:2px 6px;flex-shrink:0">
                  ${B.length?`${B.length} club${B.length>1?"s":""}`:""} ▾
                </button>
              </div>
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",r.note_g],["DEF",r.note_d],["MIL",r.note_m],["ATT",r.note_a]].map(([L,N])=>{const B=Et[L],V=Number(N)||0;return`<div style="display:flex;flex-direction:column;align-items:center;gap:2px">
                <svg width="38" height="37" viewBox="0 0 38 37" style="display:block">
                  <polygon points="19,2 24,13 36,13 26,21 30,33 19,26 8,33 12,21 2,13 14,13" fill="${B}" stroke="white" stroke-width="1.5"/>
    ...
    ${Y}
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(he=document.getElementById("close-detail"))==null||he.addEventListener("click",d);let ee=new Set;const D=()=>{const L=ee.size,N=document.getElementById("sell-action-panel");N&&(N.style.display=L>0?"block":"none",document.getElementById("sell-selected-count").textContent=L,document.getElementById("sell-direct-total").textContent=(L*_).toLocaleString("fr")+" cr.")};document.querySelectorAll(".expl-check").forEach(L=>{L.addEventListener("change",()=>{const N=L.dataset.id;L.checked?ee.add(N):ee.delete(N);const B=L.closest(".exemplaire-row");B&&(B.style.borderColor=L.checked?"#1A6B3C":"#eee"),D()})}),document.querySelectorAll(".exemplaire-row").forEach(L=>{L.addEventListener("click",N=>{if(N.target.closest("button")||N.target.tagName==="INPUT")return;const B=L.querySelector(".expl-check");B&&!B.disabled&&(B.checked=!B.checked,B.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(L=>{L.addEventListener("click",N=>{N.stopPropagation();const B=document.querySelector(`.expl-hist[data-hist="${L.dataset.idx}"]`);B&&(B.style.display=B.style.display==="none"?"flex":"none")})}),(xe=document.getElementById("direct-sell-btn"))==null||xe.addEventListener("click",async()=>{const L=[...ee];if(!L.length)return;const N=L.length*_;if(!confirm(`Vendre ${L.length} carte${L.length>1?"s":""} ${r.surname_encoded} pour ${N.toLocaleString("fr")} crédits ? Action irréversible.`))return;await $.from("market_listings").update({status:"cancelled"}).in("card_id",L).eq("status","active"),await $.from("transfer_history").delete().in("card_id",L);const{error:B}=await $.from("cards").delete().in("id",L);if(B){l(B.message,"error");return}await $.from("users").update({credits:(p.profile.credits||0)+N}).eq("id",p.profile.id),await i();const V=document.getElementById("nav-credits");V&&(V.textContent=`💰 ${(p.profile.credits||0).toLocaleString("fr")}`),l(`+${N.toLocaleString("fr")} crédits ! ${L.length} carte${L.length>1?"s":""} vendue${L.length>1?"s":""}.`,"success"),d(),c("collection")}),(ge=document.getElementById("market-sell-btn"))==null||ge.addEventListener("click",async()=>{var ie;const L=[...ee];if(!L.length){l("Sélectionne au moins un exemplaire","warning");return}const N=parseInt((ie=document.getElementById("sell-market-price"))==null?void 0:ie.value);if(!N||N<1){l("Prix invalide","error");return}const{error:B}=await $.from("cards").update({is_for_sale:!0,sale_price:N}).in("id",L);if(B){l(B.message,"error");return}const V=L.map(ke=>({seller_id:p.profile.id,card_id:ke,price:N,status:"active"})),{error:re}=await $.from("market_listings").insert(V);re&&console.warn("[Market] insert listings:",re.message),l(`${L.length} carte${L.length>1?"s":""} mise${L.length>1?"s":""} en vente à ${N.toLocaleString("fr")} cr. chacune !`,"success"),d(),c("collection")}),(O=document.getElementById("cancel-sell-btn"))==null||O.addEventListener("click",async()=>{await $.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await $.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),l("Annonce retirée","success"),d(),c("collection")})}const Gt={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},rt={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function Ut(e){const t=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const n=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${n}.png`}function Di(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function Gi(e){var n;const t="https://fcnwclxlkytdfjotqkta.supabase.co";return(n=e==null?void 0:e.clubs)!=null&&n.logo_url?e.clubs.logo_url.startsWith("http")?e.clubs.logo_url:`${t}/storage/v1/object/public/assets/clubs/${e.clubs.logo_url}`:null}function Tn(e,t=44,n=58){var g;const a=e?Ut(e):null,p=e?Gi(e):null,l=Di(e==null?void 0:e.country_code),s=(e==null?void 0:e.job)||"MIL",d=rt[s]||"#555",c={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",i=Number(s==="GK"?e==null?void 0:e.note_g:s==="DEF"?e==null?void 0:e.note_d:s==="MIL"?e==null?void 0:e.note_m:e==null?void 0:e.note_a)||0,r=Math.round(n*.19),u=Math.round(n*.25),m=n-r-u;return e?`<div style="width:${t}px;height:${n}px;border-radius:5px;border:2px solid ${c};background:${d};position:relative;overflow:hidden;flex-shrink:0">
  ...
  </div>`,document.getElementById("new-deck-btn").addEventListener("click",async()=>{const s=prompt("Nom du deck :",`Deck ${((l==null?void 0:l.length)||0)+1}`);if(!s)return;const{data:d,error:c}=await $.from("decks").insert({owner_id:n.profile.id,name:s}).select().single();if(c){p(c.message,"error");return}p("Deck créé !","success"),$i(d.id,e,t)}),e.querySelectorAll("[data-open-deck]").forEach(s=>{s.addEventListener("click",()=>$i(s.dataset.openDeck,e,t))}),e.querySelectorAll("[data-rename]").forEach(s=>{s.addEventListener("click",async()=>{const d=prompt("Nouveau nom :",s.dataset.name);if(!d||d===s.dataset.name)return;const{error:c}=await $.from("decks").update({name:d}).eq("id",s.dataset.rename);if(c){p(c.message,"error");return}p("Deck renommé !","success"),Xt(e,t)})}),e.querySelectorAll("[data-delete]").forEach(s=>{s.addEventListener("click",async()=>{if(!confirm(`Supprimer le deck "${s.dataset.name}" ? Cette action est irréversible.`))return;await $.from("deck_cards").delete().eq("deck_id",s.dataset.delete);const{error:d}=await $.from("decks").delete().eq("id",s.dataset.delete);if(d){p(d.message,"error");return}p("Deck supprimé.","success"),Xt(e,t)})})}async function $i(e,t,n){const{state:a,toast:p}=n;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:l}=await $.from("decks").select("*").eq("id",e).single(),{data:s}=await $.from("cards").select(`id, card_type, formation,
      player:players(id, firstname, surname_encoded, country_code, club_id, job, job2,
        note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length,
        clubs(encoded_name, logo_url))`).eq("owner_id",a.profile.id),d=(s||[]).filter(g=>g.card_type==="player"&&g.player),c=(s||[]).filter(g=>g.card_type==="formation"),i=c.map(g=>g.formation).filter(Boolean),{data:r}=await $.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",e);let u=l.formation||"4-4-2";i.length>0&&!i.includes(u)&&(u=i[0]);const m={deckId:e,name:l.name,formation:u,formationCardId:l.formation_card_id,slots:{},subs:[],playerCards:d,formationCards:c,availableFormations:i};(r||[]).forEach(g=>{g.is_starter?m.slots[g.position]=g.card_id:m.subs.includes(g.card_id)||m.subs.push(g.card_id)}),pt(t,m,n)}function pt(e,t,n){var c;const{navigate:a}=n;Gt[t.formation];const p=ki(t.formation),l=p.filter(i=>t.slots[i]).length,s=t.availableFormations.length>0?t.availableFormations:Object.keys(Gt),d=t.subs.map(i=>t.playerCards.find(r=>r.id===i)).filter(Boolean);[...Object.values(t.slots),...t.subs],e.innerHTML=`
  <div style="height:100%;overflow-y:auto;padding-bottom:80px;background:var(--page-bg)">
  ...
  </div>`,In(e,t,p,n),document.getElementById("builder-back").addEventListener("click",()=>a("decks")),document.getElementById("formation-select").addEventListener("change",i=>{t.formation=i.target.value;const r=ki(t.formation),u={};r.forEach(m=>{t.slots[m]&&(u[m]=t.slots[m])}),t.slots=u,pt(e,t,n)}),document.getElementById("save-deck").addEventListener("click",()=>Mn(t,n)),e.querySelectorAll("[data-remove-sub]").forEach(i=>{i.addEventListener("click",()=>{t.subs=t.subs.filter(r=>r!==i.dataset.removeSub),pt(e,t,n)})}),(c=document.getElementById("add-sub-btn"))==null||c.addEventListener("click",()=>{zn(t,e,n)})}function In(e,t,n,a){const p=e.querySelector("#deck-field");if(!p)return;const l=zt[t.formation]||{},s=Li(t.formation)||[],d={};for(const z of n){const M=t.slots[z],F=M?t.playerCards.find(U=>U.id===M):null;d[z]=F?F.player:null}const c=300,i=300,r=48,u=64,m=13,g=16,k=38;function o(z){const M=l[z];return M?{x:M.x*c,y:M.y*i}:null}let _="";for(const[z,M]of s){const F=o(z),U=o(M);if(!F||!U)continue;const q=d[z]?{...d[z],club_id:d[z].club_id,country_code:d[z].country_code,rarity:d[z].rarity}:null,j=d[M]?{...d[M],club_id:d[M].club_id,country_code:d[M].country_code,rarity:d[M].rarity}:null,R=Ze(q,j);R==="#ff3333"||R==="#cc2222"?_+=`<line x1="${F.x.toFixed(1)}" y1="${F.y.toFixed(1)}" x2="${U.x.toFixed(1)}" y2="${U.y.toFixed(1)}" stroke="${R}" stroke-width="2.5" stroke-linecap="round" opacity="0.4"/>`:(_+=`<line x1="${F.x.toFixed(1)}" y1="${F.y.toFixed(1)}" x2="${U.x.toFixed(1)}" y2="${U.y.toFixed(1)}" stroke="${R}" stroke-width="8" stroke-linecap="round" opacity="0.2"/>`,_+=`<line x1="${F.x.toFixed(1)}" y1="${F.y.toFixed(1)}" x2="${U.x.toFixed(1)}" y2="${U.y.toFixed(1)}" stroke="${R}" stroke-width="2.5" stroke-linecap="round" opacity="0.9"/>`)}for(const z of n){const M=o(z);if(!M)continue;const F=d[z],U=z.replace(/\d+/,""),q=rt[U]||"#555",j=(M.x-r/2).toFixed(1),R=(M.y-u/2).toFixed(1),te={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[F==null?void 0:F.rarity]||"#aaa";if(F){const ce=Ut(F),ne=Gi(F),S=Di(F.country_code),Y=Number(U==="GK"?F.note_g:U==="DEF"?F.note_d:U==="MIL"?F.note_m:F.note_a)||0,ee=u-m-g;_+=`<defs><clipPath id="dcp-${z}"><rect x="${j}" y="${(M.y-u/2+m).toFixed(1)}" width="${r}" height="${ee}"/></clipPath></defs>`,_+=`<rect x="${j}" y="${R}" width="${r}" height="${u}" rx="5" fill="${q}"/>`,ce&&(_+=`<image href="${ce}" x="${j}" y="${(M.y-u/2+m).toFixed(1)}" width="${r}" height="${ee}" clip-path="url(#dcp-${z})" preserveAspectRatio="xMidYMin slice"/>`),_+=`<rect x="${j}" y="${R}" width="${r}" height="${m}" fill="rgba(255,255,255,0.93)"/>`,_+=`<text x="${M.x.toFixed(1)}" y="${(M.y-u/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(F.surname_encoded||"").slice(0,9)}</text>`;const D=(M.y+u/2-g).toFixed(1);_+=`<rect x="${j}" y="${D}" width="${r}" height="${g}" fill="rgba(255,255,255,0.93)"/>`,S&&(_+=`<image href="${S}" x="${(M.x-21).toFixed(1)}" y="${(M.y+u/2-g+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`),_+=`<text x="${M.x.toFixed(1)}" y="${(M.y+u/2-g/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${Y}</text>`,ne&&(_+=`<image href="${ne}" x="${(M.x+r/2-14).toFixed(1)}" y="${(M.y+u/2-g+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`),_+=`<rect x="${j}" y="${R}" width="${r}" height="${u}" rx="5" fill="none" stroke="${te}" stroke-width="2"/>`}else _+=`<rect x="${j}" y="${R}" width="${r}" height="${u}" rx="5" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="5,3"/>`,_+=`<text x="${M.x.toFixed(1)}" y="${M.y.toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="22" fill="rgba(255,255,255,0.35)">+</text>`,_+=`<text x="${M.x.toFixed(1)}" y="${(M.y+16).toFixed(1)}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.3)">${U}</text>`;_+=`<rect x="${j}" y="${R}" width="${r}" height="${u}" rx="5" fill="rgba(0,0,0,0.01)" class="deck-slot-hit" data-pos="${z}" style="cursor:pointer"/>`}p.innerHTML=`<svg viewBox="${-k} ${-k} ${c+k*2} ${i+k*2}" width="100%" style="display:block;max-width:440px;margin:0 auto">${_}</svg>`,p.querySelectorAll(".deck-slot-hit").forEach(z=>{z.addEventListener("click",()=>Ln(z.dataset.pos,t,e,a))})}function Ln(e,t,n,a){var m,g,k;const{openModal:p,closeModal:l}=a,s=e.replace(/\d+/,""),d=t.slots[e],c=d?t.playerCards.find(o=>o.id===d):null;(m=c==null?void 0:c.player)==null||m.id;const i=new Set;Object.entries(t.slots).forEach(([o,_])=>{var M;if(o===e||!_)return;const z=t.playerCards.find(F=>F.id===_);(M=z==null?void 0:z.player)!=null&&M.id&&i.add(z.player.id)}),t.subs.forEach(o=>{var z;const _=t.playerCards.find(M=>M.id===o);(z=_==null?void 0:_.player)!=null&&z.id&&i.add(_.player.id)});const r=new Set,u=t.playerCards.filter(o=>{const _=o.player;return!(_.job===s||_.job2===s)||i.has(_.id)||r.has(_.id)?!1:(r.add(_.id),!0)});u.sort((o,_)=>{const z=s==="GK"?o.player.note_g:s==="DEF"?o.player.note_d:s==="MIL"?o.player.note_m:o.player.note_a;return(s==="GK"?_.player.note_g:s==="DEF"?_.player.note_d:s==="MIL"?_.player.note_m:_.player.note_a)-z}),p(`Choisir ${s} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${u.length>0?u.map(o=>{var U,q;const _=o.player,z=s==="GK"?_.note_g:s==="DEF"?_.note_d:s==="MIL"?_.note_m:_.note_a,M=Ut(_),F={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[_.rarity];return`<div class="player-option" data-card-id="${o.id}"
          ...
          <div style="flex:1;min-width:0">
            ...
            <div style="display:flex;align-items:center;gap:6px;margin-top:3px">
              ...
              ${(U=_.clubs)!=null&&U.logo_url?`<img src="${_.clubs.logo_url}" style="width:18px;height:18px;object-fit:contain">`:`<span style="font-size:10px;color:var(--gray-600)">${((q=_.clubs)==null?void 0:q.encoded_name)||"—"}</span>`}
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      <div style="flex:1;overflow-y:auto;display:flex;flex-wrap:wrap;gap:10px;justify-content:center;align-content:flex-start;padding:8px 16px 16px">
        ${l.map(k=>{const o=s.find(_=>_.gc_type===k.gc_type);return d(k,!!o)}).join("")}
      </div>
    ...
    </div>`,e.querySelectorAll(".gc-select-card").forEach(k=>{k.addEventListener("click",()=>{const o=k.dataset.id,_=l.find(M=>M.id===o);if(!_)return;const z=s.findIndex(M=>M.gc_type===_.gc_type);z>-1?s.splice(z,1):s.length<3&&s.push(_),i()})}),(u=e.querySelector("#gc-launch"))==null||u.addEventListener("click",()=>{r&&c(s)}),(m=e.querySelector("#gc-no-gc"))==null||m.addEventListener("click",()=>c([])),(g=e.querySelector("#gc-reset"))==null||g.addEventListener("click",()=>{s.length&&(s=[],i())})}i()}function Jn(e){var a;const t=((a=e==null?void 0:e.state)==null?void 0:a.params)||{},n=t.matchMode||"vs_ai_easy";return n==="friend"?`Match vs ${t.friendName||"Ami"}`:n==="random"?"Match vs Random":`Match vs IA — ${n.replace("vs_ai_","").toUpperCase()}`}async function Qn(e,t,n){const{state:a,navigate:p}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:l}=await $.from("decks").select("id,name,is_active,formation").eq("owner_id",a.profile.id).order("created_at",{ascending:!1});if(!l||l.length===0){at(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>p("decks"));return}const s=l.map(r=>r.id),{data:d}=await $.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",s).order("slot_order");let c=0;function i(){var M,F,U,q,j;const r=l[c],u=(d||[]).filter(R=>R.deck_id===r.id),m=u.filter(R=>{var te;return R.is_starter&&((te=R.card)==null?void 0:te.player)}).map(R=>Zt(R.card,R.position)),g=u.find(R=>{var te;return((te=R.card)==null?void 0:te.card_type)==="formation"}),k=r.formation||((M=g==null?void 0:g.card)==null?void 0:M.formation)||"4-4-2",o=m.length>=11?gt(m,k):null,_=m.length>=11;Xe(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;height:${u}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ...
      ${p?`<img src="${p}" style="width:${u-4}px;height:${u-4}px;object-fit:contain">`:e!=null&&e.clubName?`<span style="font-size:${Math.max(4,u-8)}px;font-weight:700;color:#333">${(e.clubName||"").slice(0,3).toUpperCase()}</span>`:""}
        ...
        stroke="${ne}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const g=48,k=64,o=13,_=16,z={normal:"#aaaaaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#111111",legende:"#7a28b8",légende:"#7a28b8"};for(const[U,q]of Object.entries(i)){const j=u(U);if(!j||!q)continue;const R=U.replace(/[0-9]/g,""),te=ft[R]||"#555",ce=s.includes(q.cardId),ne=n==="attack"&&(["MIL","ATT"].includes(R)||ce)&&!q.used||n==="defense"&&["GK","DEF","MIL"].includes(R)&&!q.used,S=a.includes(q.cardId);let Y;n==="attack"?Y=ce?Math.max(1,Number(q.note_a)||0):R==="MIL"?Number(q.note_m)||0:Number(q.note_a)||0:n==="defense"?Y=R==="GK"?Number(q.note_g)||0:R==="MIL"?Number(q.note_m)||0:Number(q.note_d)||0:Y=Number(R==="GK"?q.note_g:R==="DEF"?q.note_d:R==="MIL"?q.note_m:q.note_a)||0,Y=Y+(q.boost||0);const ee=(j.x-g/2).toFixed(1),D=(j.y-k/2).toFixed(1),Q=z[q==null?void 0:q.rarity]||z.normal;if(q.used){const L=typeof import.meta<"u"&&"/manager-wars/"||"/",B=`${typeof window<"u"&&((F=window.location)==null?void 0:F.origin)||""}${L}icons/carte-dos.png`.replace(/([^:])\/\//g,"$1/");m+=`<rect x="${ee}" y="${D}" width="${g}" height="${k}" rx="5" fill="#161616"/>`,m+=`<image href="${B}" xlink:href="${B}" x="${ee}" y="${D}" width="${g}" height="${k}" preserveAspectRatio="xMidYMid slice"/>`,m+=`<rect x="${ee}" y="${D}" width="${g}" height="${k}" rx="5" fill="none" stroke="${Q}" stroke-width="2" opacity="0.7"/>`,m+=`<rect x="${ee}" y="${D}" width="${g}" height="${k}" rx="5" fill="rgba(0,0,0,0.01)" class="match-used-hit" data-card-id="${q.cardId}" data-role="${R}" style="cursor:pointer"/>`;continue}const ae=S?.45:1,pe=S?"#FFD700":Q,he=S?3:(q==null?void 0:q.rarity)==="legende"||(q==null?void 0:q.rarity)==="légende"||(q==null?void 0:q.rarity)==="pepite"||(q==null?void 0:q.rarity)==="pépite"?2.5:2,xe=k-o-_;m+=`<defs><clipPath id="cp-${U}"><rect x="${ee}" y="${(j.y-k/2+o).toFixed(1)}" width="${g}" height="${xe}"/></clipPath></defs>`,m+=`<rect x="${ee}" y="${D}" width="${g}" height="${k}" rx="5" fill="${te}" opacity="${ae}"/>`;const ge=ze(q);ge&&(m+=`<image href="${ge}" xlink:href="${ge}" x="${ee}" y="${(j.y-k/2+o).toFixed(1)}" width="${g}" height="${xe}" clip-path="url(#cp-${U})" preserveAspectRatio="xMidYMin slice"/>`),m+=`<rect x="${ee}" y="${D}" width="${g}" height="${o}" rx="4" fill="rgba(255,255,255,0.92)"/>`,m+=`<text x="${j.x.toFixed(1)}" y="${(j.y-k/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(q.name||"").slice(0,9)}</text>`;const O=(j.y+k/2-_).toFixed(1);m+=`<rect x="${ee}" y="${O}" width="${g}" height="${_}" fill="rgba(255,255,255,0.92)"/>`;{const L=ei(q.country_code);L?m+=`<image href="${L}" xlink:href="${L}" x="${(j.x-20).toFixed(1)}" y="${(j.y+k/2-_+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:m+=`<text x="${(j.x-13).toFixed(1)}" y="${(j.y+k/2-_/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${Wi(q.country_code)}</text>`,m+=`<text x="${j.x.toFixed(1)}" y="${(j.y+k/2-_/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${Y}</text>`;const N=Yi(q);N?m+=`<image href="${N}" xlink:href="${N}" x="${(j.x+g/2-14).toFixed(1)}" y="${(j.y+k/2-_+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:q.clubName&&(m+=`<text x="${(j.x+14).toFixed(1)}" y="${(j.y+k/2-_/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(q.clubName||"").slice(0,3).toUpperCase()}</text>`),q.boost&&(m+=`<rect x="${(j.x+g/2-12).toFixed(1)}" y="${(j.y-k/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,m+=`<text x="${(j.x+g/2-5).toFixed(1)}" y="${(j.y-k/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${q.boost}</text>`)}m+=`<rect x="${ee}" y="${D}" width="${g}" height="${k}" rx="5" fill="${S?"rgba(255,255,255,0.12)":"none"}" stroke="${pe}" stroke-width="${he}" opacity="${ae}"/>`,ne&&(m+=`<rect x="${ee}" y="${D}" width="${g}" height="${k}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${S?"selected":""}" data-card-id="${q.cardId}" data-role="${R}" style="cursor:pointer"/>`)}const M=38;return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-M} ${-M} ${p+M*2} ${l+M*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${m}
  ...
  </div>`}async function pi(e,t,n,a){var q;const{state:p,navigate:l,toast:s}=t;Xe(e);const d=p.params||{};if(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!d.deckId)return Qn(e,t,n);const c=d.deckId;let i,r,u,m;try{const j=await Promise.all([$.from("decks").select("formation,name").eq("id",c).single(),$.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation,
            player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
              clubs(encoded_name,logo_url)))`).eq("deck_id",c).order("slot_order")]);i=j[0].data,u=j[0].error,r=j[1].data,m=j[1].error}catch(j){console.error("[Match] Exception chargement deck:",j),at(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>l("home"));return}if(u||m){console.error("[Match] Erreur Supabase:",u||m),at(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>l("home"));return}const g=(r||[]).filter(j=>{var R;return j.is_starter&&((R=j.card)==null?void 0:R.player)}).map(j=>Zt(j.card,j.position)),k=(r||[]).filter(j=>{var R;return!j.is_starter&&((R=j.card)==null?void 0:R.player)}).map(j=>Zt(j.card,j.position));if(g.length<11){at(e,"⚠️",`Deck incomplet (${g.length}/11).`,"Compléter",()=>l("decks"));return}const o=(r||[]).find(j=>{var R;return((R=j.card)==null?void 0:R.card_type)==="formation"}),_=(i==null?void 0:i.formation)||((q=o==null?void 0:o.card)==null?void 0:q.formation)||"4-4-2",{data:z,error:M}=await $.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",p.profile.id).eq("card_type","game_changer"),{data:F}=await $.from("gc_definitions").select("*").eq("is_active",!0),U=(z||[]).map(j=>({...j,_gcDef:(F==null?void 0:F.find(R=>R.name===j.gc_type||R.id===j.gc_definition_id))||null}));a({deckId:c,formation:_,starters:g,subsRaw:k,gcCardsEnriched:U,gcDefs:F||[]})}async function Zn(e,t){const{state:n}=t,p=(n.params||{}).matchMode||"vs_ai_easy",l=p.replace("vs_ai_",""),s=p;await pi(e,t,p,async({deckId:d,formation:c,starters:i,subsRaw:r,gcCardsEnriched:u,gcDefs:m})=>{try{const g=gt(i,c),k=await eo(c,l),o=async _=>{try{const{data:z,error:M}=await $.from("matches").insert({home_id:n.profile.id,away_id:null,mode:s,home_deck_id:d,status:"in_progress"}).select().single();if(M){console.error("[MatchIA] Erreur création match:",M),at(e,"⚠️","Impossible de créer le match ("+M.message+").","Retour",()=>t.navigate("home"));return}const F={gcDefs:m||[],matchId:z==null?void 0:z.id,mode:s,difficulty:l,formation:c,homeTeam:g,aiTeam:k,homeSubs:r,subsUsed:0,maxSubs:Math.min(r.length,3),homeScore:0,aiScore:0,gcCards:_,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:n.profile.club_name||"Vous"};io(e,F,t)}catch(z){console.error("[MatchIA] Exception launchMatch:",z),at(e,"⚠️","Erreur au lancement du match : "+z.message,"Retour",()=>t.navigate("home"))}};if(!u.length){o([]);return}ci(e,u,o)}catch(g){console.error("[MatchIA] Exception setup:",g),at(e,"⚠️","Erreur de préparation du match : "+g.message,"Retour",()=>t.navigate("home"))}})}async function eo(e,t){var s,d;const{data:n}=await $.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(60);if(!n||n.length<11)return to(e);const a=ut[e]||ut["4-4-2"],p={GK:[],DEF:[],MIL:[],ATT:[]},l=[...n];for(const c of["GK","DEF","MIL","ATT"]){const i=l.filter(k=>k.job===c),r=l.filter(k=>k.job!==c),u=[...i,...r],m=[];for(let k=0;k<a[c];k++){const o=u[k]||l[k];o&&m.push({cardId:"ai-"+o.id+"-"+k,id:o.id,firstname:o.firstname,name:o.surname_encoded,country_code:o.country_code,club_id:o.club_id,job:o.job,job2:o.job2,note_g:Number(o.note_g)||0,note_d:Number(o.note_d)||0,note_m:Number(o.note_m)||0,note_a:Number(o.note_a)||0,rarity:o.rarity,skin:o.skin,hair:o.hair,hair_length:o.hair_length,clubName:((s=o.clubs)==null?void 0:s.encoded_name)||null,clubLogo:((d=o.clubs)==null?void 0:d.logo_url)||null,boost:0,used:!1,_line:c})}const g=ot(m.length);m.forEach((k,o)=>{k._col=g[o]}),p[c]=m}return p}function to(e){const t=ut[e]||ut["4-4-2"],n={GK:[],DEF:[],MIL:[],ATT:[]},a=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let p=0;for(const l of["GK","DEF","MIL","ATT"]){const s=[];for(let c=0;c<t[l];c++){const i=3+Math.floor(Math.random()*5);s.push({cardId:"fake-"+p,id:"fake-"+p,firstname:"IA",name:a[p%a.length],country_code:"XX",club_id:null,job:l,job2:null,note_g:l==="GK"?i:2,note_d:l==="DEF"?i:2,note_m:l==="MIL"?i:2,note_a:l==="ATT"?i:2,rarity:"normal",boost:0,used:!1,_line:l}),p++}const d=ot(s.length);s.forEach((c,i)=>{c._col=d[i]}),n[l]=s}return n}function io(e,t,n){e.innerHTML=`
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${o.map((F,U)=>{const q=U<o.length-1?Ze(F,o[U+1]):null,j=!q||q==="#ff3333"||q==="#cc2222",R=q==="#00ff88"?"+2":q==="#FFD700"?"+1":"";return`
          ...
          ${U<o.length-1?`<div class="duel-link duel-link-${M}" data-idx="${U}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${j?"rgba(255,255,255,0.12)":q};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${j?"none":`0 0 8px ${q}`}">
  ...
  <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div id="duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center, rgba(10,61,30,0.4), rgba(10,61,30,0.92))"></div>
  </div>`;const u=()=>{const o=(_,z)=>e.querySelectorAll(_).forEach((M,F)=>{setTimeout(()=>{M.style.opacity="1",M.style.transform="translateY(0) scale(1)"},z+F*90)});o(".duel-card-home",150),o(".duel-card-ai",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((_,z)=>{setTimeout(()=>{_.style.opacity="1"},z*70)}),900),setTimeout(()=>{const _=e.querySelector("#vs-label");_&&(_.style.opacity="1",_.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(z=>z.style.opacity="1")},1250),setTimeout(()=>{m("score-home",d,800),m("score-ai",c,800)},1500)};function m(o,_,z){const M=document.getElementById(o);if(!M)return;const F=performance.now(),U=q=>{const j=Math.min(1,(q-F)/z);M.textContent=Math.round(_*(1-Math.pow(1-j,3))),j<1?requestAnimationFrame(U):M.textContent=_};requestAnimationFrame(U)}requestAnimationFrame(u),t.attacker=i?"home":"ai";const g=i?li():null;i&&(t.boostCard={value:g}),t.log.push({type:"duel",title:"Milieu de Terrain",homePlayers:a.map(o=>({name:o.name,note:we(o,"MIL"),portrait:ze(o),job:o.job,country_code:o.country_code,rarity:o.rarity,clubName:o.clubName,clubLogo:o.clubLogo})),aiPlayers:p.map(o=>({name:o.name,note:we(o,"MIL"),portrait:ze(o),job:o.job,country_code:o.country_code,rarity:o.rarity,clubName:o.clubName,clubLogo:o.clubLogo})),homeTotal:d,aiTotal:c,text:`Duel milieu : ${t.clubName} ${d} – ${c} IA → ${i?t.clubName+" attaque":"IA attaque"}`});const k=()=>{t.phase=t.attacker==="home"?"attack":"ai-attack",Me(e,t,n),t.attacker==="ai"&&setTimeout(()=>ti(e,t,n),800)};setTimeout(()=>{const o=document.getElementById("score-home"),_=document.getElementById("score-ai"),z=document.getElementById(i?"duel-row-home":"duel-row-ai"),M=document.getElementById(i?"duel-row-ai":"duel-row-home"),F=i?o:_,U=i?_:o;F&&(F.style.fontSize="80px",F.style.color=i?"#FFD700":"#ff6b6b",F.style.animation="duelPulse .5s ease"+(i?", duelGlow 1.5s ease infinite .5s":"")),U&&(U.style.opacity="0.25"),setTimeout(()=>{z&&(z.style.transformOrigin="center",z.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",z.style.zIndex="5"),setTimeout(()=>{var j;const q=document.getElementById("duel-shock");if(q){const R=(j=M||z)==null?void 0:j.getBoundingClientRect(),te=e.querySelector(".match-screen").getBoundingClientRect();R&&(q.style.top=R.top-te.top+R.height/2+"px"),q.style.animation="shockwave .5s ease-out forwards"}M&&(M.style.transformOrigin="center",M.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var j;const q=document.getElementById("duel-finale");q&&(q.innerHTML=`
          ...
          <button id="start-match-btn" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,0.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">
            ▶ Commencer le match
          </button>`,q.style.transition="opacity .45s ease",q.style.opacity="1",q.style.pointerEvents="auto",(j=document.getElementById("start-match-btn"))==null||j.addEventListener("click",k))},600)},700)},2800)}function oo(e){if(e.type==="duel"&&(e.homeTotal!=null||e.aiTotal!=null)){const t=(e.homeTotal||0)>=(e.aiTotal||0);return`
    ...
    <div style="display:flex;align-items:center;gap:4px;${t?"flex-direction:row-reverse":""};background:rgba(255,255,255,0.04);border-radius:8px;padding:5px 8px;border:1px solid rgba(255,255,255,0.07)">
      ...
      ${nt(e.inPlayer||{})}
    </div>`}return`<div style="font-size:11px;color:${e.type==="goal"?"#FFD700":"rgba(255,255,255,0.65)"};font-weight:${e.type==="goal"?700:400};padding:3px 2px">${e.text||""}</div>`}function Me(e,t,n){var F,U,q,j,R,te,ce,ne;const a=t.selected.map(S=>S.cardId),p=t.usedSubIds||[],l=t.homeSubs.filter(S=>!p.includes(S.cardId)),d=Object.values(t.homeTeam).flat().filter(S=>S.used).length>0&&l.length>0&&t.subsUsed<t.maxSubs,c=!["GK","DEF","MIL","ATT"].some(S=>(t.aiTeam[S]||[]).some(Y=>!Y.used)),i=[...t.homeTeam.MIL||[],...t.homeTeam.ATT||[]].filter(S=>!S.used),r=t.phase==="attack"&&c&&i.length===0?[...t.homeTeam.GK||[],...t.homeTeam.DEF||[]].filter(S=>!S.used).map(S=>S.cardId):[];t.log[t.log.length-1];const u=t.phase==="ai-attack"||t.phase==="ai-defense",m=t.phase==="attack",g=t.phase==="defense",k=t.phase==="finished",o=t.gcCards.filter(S=>!t.usedGc.includes(S.id)),_=t.boostCard&&!t.boostUsed;e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column",e.innerHTML=`
  <style>
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    ${(()=>{const S=window.innerWidth>=700,Y=(O,L,N)=>{var Ne,He;const B=(t.gcDefs||[]).find(T=>T.name===O.gc_type),V={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[B==null?void 0:B.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",re={purple:"#b06ce0",light_blue:"#00d4ef"}[B==null?void 0:B.color]||"#b06ce0",ie=(B==null?void 0:B.name)||O.gc_type,ke=(B==null?void 0:B.effect)||((Ne=Ge[O.gc_type])==null?void 0:Ne.desc)||"",Ee=B!=null&&B.image_url?`/manager-wars/icons/${B.image_url}`:null,$e=((He=Ge[O.gc_type])==null?void 0:He.icon)||"⚡",_e=Math.round(N*.22),Ae=Math.round(N*.22),Te=N-_e-Ae,qe=ie.length>12?7:9;return`<div class="gc-mini" data-gc-id="${O.id}" data-gc-type="${O.gc_type}"
          style="box-sizing:border-box;width:${L}px;height:${N}px;border-radius:10px;border:2px solid ${re};background:${V};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
  ...
  `,document.body.appendChild(a);let r=!1;const u=()=>{r||(r=!0,a.remove(),n())};a.addEventListener("click",u),setTimeout(u,2e3)}function _t(e,t="rgba(0,0,0,0.8)"){const n=document.createElement("div");n.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,n.textContent=e,document.body.appendChild(n),setTimeout(()=>n.remove(),2200)}function Yt(e,t,n,a=null,p=null){var g,k;if(t.phase!=="attack"){_t("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){_t(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const l=Object.entries(t.homeTeam).flatMap(([o,_])=>(_||[]).filter(z=>z.used).map(z=>({...z,_line:z._line||o}))),s=t.homeSubs.filter(o=>!t.usedSubIds.includes(o.cardId));if(!l.length){_t("Aucun joueur utilisé à remplacer");return}if(!s.length){_t("Aucun remplaçant disponible");return}let d=Math.max(0,l.findIndex(o=>o.cardId===p));const c=((g=l[d])==null?void 0:g._line)||((k=l[d])==null?void 0:k.job);let i=a?Math.max(0,s.findIndex(o=>o.cardId===a)):Math.max(0,s.findIndex(o=>o.job===c)),r=!1;const u=document.createElement("div");u.id="sub-overlay",u.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function m(){var q,j,R,te,ce,ne;const o=l[d],_=s[i],z=Math.min(130,Math.round((window.innerWidth-90)/2)),M=Math.round(z*1.35),F=S=>`background:rgba(255,255,255,0.12);border:none;color:${S?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${S?"default":"pointer"};flex-shrink:0`;u.innerHTML=`
    ...
    <div style="flex:1;display:flex;gap:0;overflow:hidden">
      ...
      <div id="in-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
        ...
        <div>${_?Ve({..._,used:!1,boost:0},z,M):"<div>—</div>"}</div>
    ...
    </div>`,document.body.appendChild(g),(k=g.querySelector("#gc-back"))==null||k.addEventListener("click",()=>g.remove()),(o=g.querySelector("#gc-use"))==null||o.addEventListener("click",()=>{g.remove(),fo(e,t,n,a,p)})}function Ft(e,t,n,a,p,l){const s=document.createElement("div");s.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let d=[];function c(){var i,r;s.innerHTML=`
    ...
    <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
      ${e.map(u=>{const m=u._line||u.job||"MIL",g={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[m]||"#555",k=we(u,m)+(u.boost||0),o=d.find(_=>_.cardId===u.cardId);return`<div class="gc-pick-item" data-cid="${u.cardId}"
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(l);let d=!1;const c=()=>{d||(d=!0,l.remove(),setTimeout(()=>p(),50))};l.addEventListener("click",c),setTimeout(c,3500)}async function At(e,t,n){var r,u;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:a}=n,p=t.homeScore>t.aiScore,l=t.homeScore===t.aiScore,s=p?"victoire":l?"nul":"defaite",d=wn(t.mode,s);t.matchId&&await $.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:p?a.profile.id:null,home_credits_reward:d,played_at:new Date().toISOString()}).eq("id",t.matchId);const c={credits:(a.profile.credits||0)+d,matches_played:(a.profile.matches_played||0)+1};p?c.wins=(a.profile.wins||0)+1:l?c.draws=(a.profile.draws||0)+1:c.losses=(a.profile.losses||0)+1,await $.from("users").update(c).eq("id",a.profile.id),await n.refreshProfile();const i=document.createElement("div");i.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",i.innerHTML=`
    ...
    </div>`,`<button class="btn btn-primary" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}async function Ji(e,t){var n,a;try{const p=(a=(n=t==null?void 0:t.state)==null?void 0:n.profile)==null?void 0:a.id;try{($.getChannels?$.getChannels():[]).forEach(s=>{const d=s.topic||"";(d.includes("matchmaking")||d.includes("pvp-match")||d.includes("friend-invite"))&&$.removeChannel(s)})}catch(l){console.warn("[Random] cleanup canaux:",l)}p&&await $.rpc("cancel_matchmaking",{p_user_id:p}).catch(()=>{})}catch{}await pi(e,t,"random",({deckId:p,formation:l,starters:s,subsRaw:d,gcCardsEnriched:c,gcDefs:i})=>{const r=u=>xo(e,t,p,l,s,d,u,i||[]);if(!c.length){r([]);return}ci(e,c,r)})}async function xo(e,t,n,a,p,l,s=[],d=[]){var M;const{state:c,navigate:i,toast:r}=t;let u=!1,m=null;Xe(e),e.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const g=async(F=!0)=>{u=!0,m&&($.removeChannel(m),m=null),F&&await $.rpc("cancel_matchmaking",{p_user_id:c.profile.id}).catch(()=>{})};(M=document.getElementById("mm-cancel"))==null||M.addEventListener("click",async()=>{try{await g(!0)}catch{}Oe(e),i("home")});const k=async(F,U,q)=>{if(u)return;u=!0,m&&($.removeChannel(m),m=null);const j=document.getElementById("mm-status");j&&(j.textContent="Adversaire trouvé !"),await new Promise(R=>setTimeout(R,600)),e.isConnected&&Qi(e,t,F,q,s,d)},{data:o,error:_}=await $.rpc("try_matchmake",{p_user_id:c.profile.id,p_deck_id:n});if(_||!(o!=null&&o.success)){r("Erreur de matchmaking","error"),Oe(e),i("home");return}if(o.matched){k(o.match_id,o.opponent_id,!1);return}const z=document.getElementById("mm-status");z&&(z.textContent="En attente d'un autre joueur..."),m=$.channel("matchmaking-"+c.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${c.profile.id}`},F=>{const U=F.new;U.status==="matched"&&U.match_id&&k(U.match_id,U.matched_with,!0)}).subscribe()}async function yo(e,t,n){const{state:a,navigate:p,toast:l}=t;try{($.getChannels?$.getChannels():[]).forEach(i=>{const r=i.topic||"";(r.includes("matchmaking")||r.includes("pvp-match")||r.includes("friend-invite"))&&$.removeChannel(i)})}catch{}const{data:s}=await $.from("matches").select("home_id, away_id, status").eq("id",n).single();if(!s){l("Match introuvable","error"),p("home");return}if(s.status==="finished"){l("Ce match est terminé","info"),p("home");return}const d=s.home_id===a.profile.id;Qi(e,t,n,d,[],[])}async function Qi(e,t,n,a,p=[],l=[]){const{state:s,navigate:d,toast:c}=t,i=a?"p1":"p2",r=a?"p2":"p1",u=(p||[]).map(T=>T.id),m=(p||[]).map(T=>({id:T.id,gc_type:T.gc_type,_gcDef:T._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:g}=await $.from("matches").select("*").eq("id",n).single();if(!g){c("Match introuvable","error"),d("home");return}async function k(){const[{data:T},{data:f},{data:x},{data:h}]=await Promise.all([$.rpc("get_deck_for_match",{p_deck_id:g.home_deck_id}),$.rpc("get_deck_for_match",{p_deck_id:g.away_deck_id}),$.from("users").select("id,pseudo,club_name").eq("id",g.home_id).single(),$.from("users").select("id,pseudo,club_name").eq("id",g.away_id).single()]),y=E=>({cardId:E.card_id,position:E.position,id:E.id,firstname:E.firstname,name:E.surname_encoded,country_code:E.country_code,club_id:E.club_id,job:E.job,job2:E.job2,note_g:Number(E.note_g)||0,note_d:Number(E.note_d)||0,note_m:Number(E.note_m)||0,note_a:Number(E.note_a)||0,rarity:E.rarity,skin:E.skin,hair:E.hair,hair_length:E.hair_length,clubName:E.club_encoded_name||null,clubLogo:E.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}),b=((T==null?void 0:T.starters)||[]).map(E=>y(E)),v=((f==null?void 0:f.starters)||[]).map(E=>y(E)),w=(T==null?void 0:T.formation)||"4-4-2",I=(f==null?void 0:f.formation)||"4-4-2";return{p1Team:gt(b,w),p2Team:gt(v,I),p1Subs:((T==null?void 0:T.subs)||[]).map(E=>y(E)),p2Subs:((f==null?void 0:f.subs)||[]).map(E=>y(E)),p1Formation:w,p2Formation:I,p1Name:(x==null?void 0:x.club_name)||(x==null?void 0:x.pseudo)||"Joueur 1",p2Name:(h==null?void 0:h.club_name)||(h==null?void 0:h.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:a?u:[],gc_p2:a?[]:u,gcCardsFull_p1:a?m:[],gcCardsFull_p2:a?[]:m,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:l||[],lastActionAt:new Date().toISOString()}}let o=g.game_state&&Object.keys(g.game_state).length?g.game_state:null;if(!o)if(a){o=await k();const{data:T}=await $.from("matches").select("game_state").eq("id",n).single();!(T!=null&&T.game_state)||!Object.keys(T.game_state).length?await $.from("matches").update({game_state:o,turn_user_id:g.home_id}).eq("id",n):o=T.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let T=0;T<30&&!o;T++){await new Promise(x=>setTimeout(x,400));const{data:f}=await $.from("matches").select("game_state").eq("id",n).single();f!=null&&f.game_state&&Object.keys(f.game_state).length&&(o=f.game_state)}if(!o){c("Erreur de synchronisation","error"),d("home");return}o.gc_p2=u,o.gcCardsFull_p2=m,await $.from("matches").update({game_state:o}).eq("id",n)}let _=!1,z=null,M=!1;const F=new Set,U=new Set;function q(T){var E,A;try{$.removeChannel(j)}catch{}const f=o[i+"Score"]||0,x=o[r+"Score"]||0;let h,y;T.winner_id?(h=T.winner_id===s.profile.id,y=!1):T.forfeit?(h=f>x,y=!1):(y=f===x,h=f>x),(E=document.getElementById("pvp-end-overlay"))==null||E.remove();const b=document.createElement("div");b.id="pvp-end-overlay",b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const v=y?"🤝":h?"🏆":"😞",w=y?"MATCH NUL":h?"VICTOIRE !":"DÉFAITE",I=y?"#fff":h?"#FFD700":"#ff6b6b";b.innerHTML=`
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:6px">Appuyer pour continuer</div>`,document.body.appendChild(y);let v=!1;const w=()=>{v||(v=!0,y.remove(),setTimeout(()=>x(),50))};y.addEventListener("click",w),setTimeout(w,2200)}function B(){var f;const T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:20px;overflow-y:auto",T.innerHTML=`
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
    ...
    </div>`,(y=document.getElementById("pvp-home"))==null||y.addEventListener("click",()=>{try{$.removeChannel(j)}catch{}Oe(e),d("home")})}D()}const ho=Object.freeze(Object.defineProperty({__proto__:null,renderMatchRandom:Ji,resumePvpMatch:yo},Symbol.toStringTag,{value:"Module"}));async function bo(e,t,n,a){var p,l;try{const s=(l=(p=t==null?void 0:t.state)==null?void 0:p.profile)==null?void 0:l.id;try{($.getChannels?$.getChannels():[]).forEach(c=>{const i=c.topic||"";(i.includes("matchmaking")||i.includes("pvp-match"))&&$.removeChannel(c)})}catch(d){console.warn("[FriendMatch] cleanup canaux:",d)}s&&(await $.rpc("cancel_matchmaking",{p_user_id:s}).catch(()=>{}),await $.from("matchmaking_queue").delete().eq("user_id",s).then(()=>{},()=>{}))}catch{}await pi(e,t,"random",({deckId:s,formation:d,starters:c,subsRaw:i,gcCardsEnriched:r,gcDefs:u})=>{const m=g=>vo(e,t,s,d,c,i,g,u||[],n,a);if(!r.length){m([]);return}ci(e,r,m)})}async function vo(e,t,n,a,p,l,s=[],d=[],c,i){var ce;const{state:r,navigate:u,toast:m}=t,g=r.profile.id;let k=!1,o=null;Xe(e);try{await $.rpc("cancel_matchmaking",{p_user_id:g})}catch{}try{await $.from("matchmaking_queue").delete().eq("user_id",g)}catch{}const _=(ne,S,Y,ee)=>{var D;e.innerHTML=`
      ...
      </div>`,(D=document.getElementById("lobby-cancel"))==null||D.addEventListener("click",async()=>{k=!0,o&&($.removeChannel(o),o=null),Y&&await $.from("friend_match_invites").update({status:"declined"}).eq("id",Y),Oe(e),u("home")})},z=async(ne,S)=>{k=!0,o&&($.removeChannel(o),o=null),await new Promise(Y=>setTimeout(Y,600)),e.isConnected&&wo(e,t,ne,S,s,d)},{data:M}=await $.from("friend_match_invites").select("*").eq("status","pending").or(`and(inviter_id.eq.${g},invitee_id.eq.${c}),and(inviter_id.eq.${c},invitee_id.eq.${g})`).order("created_at",{ascending:!1}).limit(1).maybeSingle();let F,U;if(M&&M.inviter_id===c)U=!1,F=M.id,await $.from("friend_match_invites").update({invitee_deck_id:n,invitee_ready:!0,status:"accepted"}).eq("id",F);else{U=!0;const{data:ne,error:S}=await $.from("friend_match_invites").insert({inviter_id:g,invitee_id:c,friend_id:c,inviter_deck_id:n,inviter_ready:!0,status:"pending"}).select().single();if(S||!ne){m("Erreur création invitation","error"),Oe(e),u("home");return}F=ne.id}if(_(!0,!U,F),!U){const ne={home_id:c,away_id:g,home_deck_id:M.inviter_deck_id,away_deck_id:n,status:"active",mode:"friend"},S=await $.from("matches").insert(ne).select().single(),Y=S.data;if(!Y){m("Erreur création match: "+(((ce=S.error)==null?void 0:ce.message)||""),"error"),Oe(e),u("home");return}await $.from("friend_match_invites").update({match_id:Y.id,status:"matched"}).eq("id",F),z(Y.id,!1);return}let q=!1;const j=ne=>{k||q||(ne.status==="matched"&&ne.match_id?(q=!0,clearInterval(R),clearInterval(te),z(ne.match_id,!0)):ne.status==="declined"?(clearInterval(R),clearInterval(te),m(`${i} a décliné l'invitation`,"warning"),Oe(e),u("home")):ne.invitee_ready&&_(!0,!0,F))},R=setInterval(()=>{if(k){clearInterval(R);return}$.from("matchmaking_queue").delete().eq("user_id",g).then(()=>{},()=>{})},3e3),te=setInterval(async()=>{if(k||q){clearInterval(te);return}const{data:ne}=await $.from("friend_match_invites").select("*").eq("id",F).maybeSingle();ne&&j(ne)},1200);o=$.channel("friend-invite-"+F).on("postgres_changes",{event:"UPDATE",schema:"public",table:"friend_match_invites",filter:`id=eq.${F}`},ne=>j(ne.new)).subscribe()}async function wo(e,t,n,a,p=[],l=[]){const{state:s,navigate:d,toast:c}=t,i=a?"p1":"p2",r=a?"p2":"p1",u=(p||[]).map(f=>f.id),m=(p||[]).map(f=>({id:f.id,gc_type:f.gc_type,_gcDef:f._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:g}=await $.from("matches").select("*").eq("id",n).single();if(!g){c("Match introuvable","error"),d("home");return}async function k(){const[{data:f},{data:x},{data:h},{data:y}]=await Promise.all([$.rpc("get_deck_for_match",{p_deck_id:g.home_deck_id}),$.rpc("get_deck_for_match",{p_deck_id:g.away_deck_id}),$.from("users").select("id,pseudo,club_name").eq("id",g.home_id).single(),$.from("users").select("id,pseudo,club_name").eq("id",g.away_id).single()]),b=A=>({cardId:A.card_id,position:A.position,id:A.id,firstname:A.firstname,name:A.surname_encoded,country_code:A.country_code,club_id:A.club_id,job:A.job,job2:A.job2,note_g:Number(A.note_g)||0,note_d:Number(A.note_d)||0,note_m:Number(A.note_m)||0,note_a:Number(A.note_a)||0,rarity:A.rarity,skin:A.skin,hair:A.hair,hair_length:A.hair_length,clubName:A.club_encoded_name||null,clubLogo:A.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}),v=((f==null?void 0:f.starters)||[]).map(A=>b(A)),w=((x==null?void 0:x.starters)||[]).map(A=>b(A)),I=(f==null?void 0:f.formation)||"4-4-2",E=(x==null?void 0:x.formation)||"4-4-2";return{p1Team:gt(v,I),p2Team:gt(w,E),p1Subs:((f==null?void 0:f.subs)||[]).map(A=>b(A)),p2Subs:((x==null?void 0:x.subs)||[]).map(A=>b(A)),p1Formation:I,p2Formation:E,p1Name:(h==null?void 0:h.club_name)||(h==null?void 0:h.pseudo)||"Joueur 1",p2Name:(y==null?void 0:y.club_name)||(y==null?void 0:y.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:a?u:[],gc_p2:a?[]:u,gcCardsFull_p1:a?m:[],gcCardsFull_p2:a?[]:m,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:l||[],lastActionAt:new Date().toISOString()}}let o=g.game_state&&Object.keys(g.game_state).length?g.game_state:null;if(!o)if(a){o=await k();const{data:f}=await $.from("matches").select("game_state").eq("id",n).single();!(f!=null&&f.game_state)||!Object.keys(f.game_state).length?await $.from("matches").update({game_state:o,turn_user_id:g.home_id}).eq("id",n):o=f.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let f=0;f<60&&!o;f++){await new Promise(h=>setTimeout(h,400));const{data:x}=await $.from("matches").select("game_state").eq("id",n).single();x!=null&&x.game_state&&Object.keys(x.game_state).length&&(o=x.game_state)}if(!o){c("Erreur de synchronisation","error"),d("home");return}o.gc_p2=u,o.gcCardsFull_p2=m,await $.from("matches").update({game_state:o}).eq("id",n)}let _=!1,z=!1,M=null,F=!1;const U=new Set,q=new Set;function j(f){var A,C;try{$.removeChannel(R)}catch{}const x=o[i+"Score"]||0,h=o[r+"Score"]||0;if(!z){const P=o.p1Score||0,W=o.p2Score||0,le=(o.usedGc_p1||[]).length,J=(o.usedGc_p2||[]).length,H=f.winner_id||(P>W?g.home_id:W>P?g.away_id:null);(H?s.profile.id===H:s.profile.id<(a?g.away_id:g.home_id))&&(z=!0,mn({player1Id:g.home_id,player2Id:g.away_id,score1:P,score2:W,gc1:le,gc2:J}).catch(Z=>console.warn("[FriendMatch] updateStats:",Z)))}let y,b;f.winner_id?(y=f.winner_id===s.profile.id,b=!1):f.forfeit?(y=x>h,b=!1):(b=x===h,y=x>h),(A=document.getElementById("pvp-end-overlay"))==null||A.remove();const v=document.createElement("div");v.id="pvp-end-overlay",v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const w=b?"🤝":y?"🏆":"😞",I=b?"MATCH NUL":y?"VICTOIRE !":"DÉFAITE",E=b?"#fff":y?"#FFD700":"#ff6b6b";v.innerHTML=`
      ...
      </div>`;function Be(){const G=e.querySelector(".match-screen");if(!G)return;const oe=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);G.style.height=oe+"px",G.style.minHeight=oe+"px",G.style.maxHeight=oe+"px",G.style.overflow="hidden";const se=e.querySelector("#mobile-action-bar"),ye=e.querySelector("#mobile-play-area");se&&ye&&(ye.style.paddingBottom=se.offsetHeight+"px")}if(Be(),setTimeout(Be,120),setTimeout(Be,400),F||(F=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",Be),window.visualViewport.addEventListener("scroll",Be)),window.addEventListener("resize",Be)),function(){const oe=e.querySelector(".terrain-wrapper svg");oe&&(oe.removeAttribute("width"),oe.removeAttribute("height"),oe.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",oe.setAttribute("viewBox","-26 -26 352 352"),oe.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),e.querySelectorAll(".match-slot-hit").forEach(G=>{G.addEventListener("click",()=>{if(!w&&!I)return;const oe=G.dataset.cardId,se=G.dataset.role,ye=(f[se]||[]).find(et=>et.cardId===oe);if(!ye||ye.used)return;const Ye=fe.includes(oe);if(w&&!["MIL","ATT"].includes(se)&&!Ye)return;Array.isArray(o["selected_"+i])||(o["selected_"+i]=[]);const We=o["selected_"+i],Je=We.findIndex(et=>et.cardId===oe);Je>-1?We.splice(Je,1):We.length<3&&We.push({...ye,_role:se}),Q()})}),e.querySelectorAll(".match-used-hit").forEach(G=>{G.addEventListener("click",()=>N(G.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(G=>{G.addEventListener("click",()=>N())}),(Qe=e.querySelector("#pvp-sub-open"))==null||Qe.addEventListener("click",()=>N()),e.querySelectorAll(".pvp-gc-mini").forEach(G=>{G.addEventListener("click",()=>O(G.dataset.gcId,G.dataset.gcType))}),(mt=e.querySelector("#pvp-boost-card"))==null||mt.addEventListener("click",()=>L()),(xt=e.querySelector("#pvp-action"))==null||xt.addEventListener("click",G=>{w?G.currentTarget.dataset.pass==="1"||!Te(i)?ie():ke():I&&Ee()}),(yt=e.querySelector("#pvp-quit"))==null||yt.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&ce()}),(ht=e.querySelector("#pvp-view-opp"))==null||ht.addEventListener("click",()=>V()),(X=e.querySelector("#pvp-toggle-history"))==null||X.addEventListener("click",()=>re()),M&&(clearInterval(M),M=null),(w||I)&&!_){let G=30,oe=!1;const se=()=>document.getElementById("pvp-timer"),ye=()=>{se()&&(se().textContent=G+"s",se().style.color=oe?"#ff4444":"#fff")};ye(),M=setInterval(()=>{G--,G<0?oe?(clearInterval(M),M=null,w&&!Te(i)?ie():ce()):(oe=!0,G=15,ye()):ye()},1e3)}}function ae(){Xe(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
      <div style="font-size:20px;font-weight:900;color:#ff6b6b">${o[r+"Name"]||"Adversaire"}</div>
      <div style="width:min(90vw,420px)">${st(o[r+"Team"],o[r+"Formation"],null,[],300,300)}</div>
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(W);let le=!1;const J=()=>{le||(le=!0,W.remove(),setTimeout(()=>h&&h(),50))};W.addEventListener("click",J),setTimeout(J,3e3)}function O(f,x){var W,le,J,H;const y=(o["gcCardsFull_"+i]||[]).find(K=>K.id===f),b=y==null?void 0:y._gcDef,v={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[b==null?void 0:b.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",w={purple:"#b06ce0",light_blue:"#00d4ef"}[b==null?void 0:b.color]||"#b06ce0",I=(b==null?void 0:b.name)||x,E=(b==null?void 0:b.effect)||((W=Ge[x])==null?void 0:W.desc)||"Carte spéciale.",A=b!=null&&b.image_url?`/manager-wars/icons/${b.image_url}`:null,C=((le=Ge[x])==null?void 0:le.icon)||"⚡",P=document.createElement("div");P.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",P.innerHTML=`
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      ...
      <button id="pvp-home" style="padding:16px 40px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">🏠 Retour</button>
    </div>`,(b=document.getElementById("pvp-home"))==null||b.addEventListener("click",()=>{try{$.removeChannel(R)}catch{}Oe(e),d("home")})}Q()}const _o={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"};async function $o(e,t){const{state:n,toast:a}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement du marché...</div>',await fi(e,t)}async function fi(e,t){const{state:n,toast:a}=t,{data:p}=await $.from("market_listings").select(`id, price, status, listed_at, seller_id,
      seller:users!seller_id(pseudo),
      card:cards(id, card_type,
        player:players(id, firstname, surname_encoded, country_code, job, job2,
          note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length, clubs(encoded_name, logo_url)))`).eq("status","active").order("listed_at",{ascending:!1}).limit(60),{data:l}=await $.from("market_listings").select(`id, price, status, listed_at, sold_at, seller_id, buyer_id,
  ...
  `;function c(i){const r=document.getElementById("mkt-content"),u=i==="buy"?s:d;if(u.length===0){r.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      </div>`;return}const m=i==="mine"?[...u].sort((g,k)=>g.status!==k.status?g.status==="active"?-1:1:new Date(k.listed_at)-new Date(g.listed_at)):u;r.innerHTML=`<div style="display:flex;flex-direction:column;gap:10px">
      ${m.map(g=>{var F,U,q,j;const k=(F=g.card)==null?void 0:F.player;if(!k)return"";const o=k.job==="GK"?k.note_g:k.job==="DEF"?k.note_d:k.job==="MIL"?k.note_m:k.note_a,_=_o[k.rarity],z=(n.profile.credits||0)>=g.price,M=g.status==="sold";return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${M?"opacity:0.65":""}">
          <div style="width:44px;height:44px;border-radius:8px;background:${To(k.job)};color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:18px;border:2px solid ${_};flex-shrink:0">${o}</div>
          <div style="flex:1;min-width:0">
            ...
            ${i==="buy"?`<div style="font-size:11px;color:var(--gray-600)">Vendeur : ${((q=g.seller)==null?void 0:q.pseudo)||"—"}</div>`:M?`<div style="font-size:11px;color:var(--green)">✅ Vendu à ${((j=g.buyer)==null?void 0:j.pseudo)||"—"} · ${g.sold_at?new Date(g.sold_at).toLocaleDateString("fr"):""}</div>`:`<div style="font-size:11px;color:var(--gray-600)">🟢 En vente depuis le ${new Date(g.listed_at).toLocaleDateString("fr")}</div>`}
          </div>
          <div style="text-align:right;flex-shrink:0">
    ...
    </div>`,r.querySelectorAll("[data-buy]").forEach(g=>{g.addEventListener("click",()=>ko(g.dataset.buy,u,e,t))}),r.querySelectorAll("[data-cancel]").forEach(g=>{g.addEventListener("click",()=>Ao(g.dataset.cancel,e,t))})}c("buy"),e.querySelectorAll(".mkt-tab").forEach(i=>{i.addEventListener("click",()=>{e.querySelectorAll(".mkt-tab").forEach(r=>{const u=r===i;r.style.background=u?"var(--green)":"#fff",r.style.color=u?"#fff":"var(--gray-600)",r.style.borderColor=u?"var(--green)":"var(--gray-200)"}),c(i.dataset.tab)})})}async function ko(e,t,n,a){var g;const{state:p,toast:l,refreshProfile:s}=a,d=t.find(k=>k.id===e);if(!d)return;const c=d.price,i=p.profile.credits||0,r=(g=d.card)==null?void 0:g.player;if(i<c){l("Crédits insuffisants","error");return}if(!await Eo(r,c))return;const m=document.querySelector(`[data-buy="${e}"]`);m&&(m.disabled=!0,m.textContent="⏳");try{const{data:k,error:o}=await $.rpc("buy_market_card",{p_listing_id:e,p_buyer_id:p.profile.id});if(o)throw new Error(o.message);if(!(k!=null&&k.success))throw new Error((k==null?void 0:k.error)||"Achat impossible");p.profile.credits=i-c;const _=document.getElementById("nav-credits");_&&(_.textContent=`💰 ${(i-c).toLocaleString("fr")}`),l(`✅ ${r==null?void 0:r.surname_encoded} ajouté à ta collection !`,"success"),fi(n,a)}catch(k){l("❌ "+k.message,"error"),m&&(m.disabled=!1,m.textContent="Acheter")}}function Eo(e,t){return new Promise(n=>{const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",a.innerHTML=`
      ...
      </div>`,document.body.appendChild(a);const p=l=>{a.remove(),n(l)};a.querySelector("#buy-ok").addEventListener("click",()=>p(!0)),a.querySelector("#buy-cancel").addEventListener("click",()=>p(!1)),a.addEventListener("click",l=>{l.target===a&&p(!1)})})}async function Ao(e,t,n){const{toast:a}=n,{data:p}=await $.from("market_listings").select("card_id").eq("id",e).single();if(await $.from("market_listings").update({status:"cancelled"}).eq("id",e),p!=null&&p.card_id){const{count:l}=await $.from("market_listings").select("id",{count:"exact",head:!0}).eq("card_id",p.card_id).eq("status","active");l||await $.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",p.card_id)}a("Annonce retirée","success"),fi(t,n)}function To(e){return{GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[e]||"#888"}async function Io(e,{state:t,navigate:n,toast:a}){e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:p}=await $.from("users").select("id,pseudo,club_name,trophies_top1,trophies_top2,trophies_top3,wins,level").order("trophies_top1",{ascending:!1}).limit(100);e.innerHTML=`
  <div class="page">
    <div class="page-header">
  ...
  `}async function Lo(e,{state:t,navigate:n,toast:a}){const p=t.profile;if(!p)return;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:l}=await $.from("matches").select(`id,home_id,away_id,home_score,away_score,status,mode,winner_id,created_at,played_at,
      home:users!home_id(pseudo,club_name),
      away:users!away_id(pseudo,club_name)`).or(`home_id.eq.${p.id},away_id.eq.${p.id}`).order("created_at",{ascending:!1}).limit(50),s={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},d=(l||[]).filter(r=>r.status==="finished"),c=(l||[]).filter(r=>r.status==="in_progress");function i(r){const u=r.home_id===p.id;u?r.home_score:r.away_score,u?r.away_score:r.home_score;const m=r.winner_id===p.id,g=r.home_score===r.away_score&&r.status==="finished",k=r.status!=="finished"?"…":g?"N":m?"V":"D",o=r.status!=="finished"||g?"#888":m?"#1A6B3C":"#c0392b";let _=s[r.mode]||r.mode;r.away_id===null&&!_.startsWith("IA")&&(_="IA");const M=r.home_id===p.id?r.away:r.home;let F;r.away_id===null?F=_:M?F=`${M.club_name||M.pseudo} (${M.pseudo})`:F="Adversaire";let U="";r.status==="in_progress"&&Date.now()-new Date(r.created_at).getTime()>3600*1e3&&(U=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const q=new Date(r.created_at),j=q.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+q.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),R=r.status==="finished"?`${r.home_score} - ${r.away_score}`:`${r.home_score||0} - ${r.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
        <div style="font-size:13px;font-weight:600">${F}${U}</div>
  ...
  `,document.querySelectorAll(".bottom-nav a").forEach(a=>{a.addEventListener("click",p=>{p.preventDefault(),Dt(a.dataset.page)})}),document.getElementById("nav-logo").addEventListener("click",()=>Dt("home")),document.getElementById("nav-credits").addEventListener("click",()=>Dt("boosters")),document.getElementById("theme-toggle").addEventListener("click",()=>{const p=Pt()==="dark"?"light":"dark";Mo(p),Ti(p)}),Ti(Pt())}function Ti(e){const t=document.getElementById("theme-icon");t&&(t.textContent=e==="dark"?"☀️":"🌙")}async function Co(){ni(Pt()),document.getElementById("modal-overlay").addEventListener("click",n=>{n.target===n.currentTarget&&ii()}),document.getElementById("modal-close").addEventListener("click",ii);const{data:{session:e}}=await $.auth.getSession();if(!e){Ii(),xi(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:$t});return}ve.user=e.user,await kt(),Ii();try{const{data:n}=await $.from("formation_links_overrides").select("formation, links"),a={};(n||[]).forEach(p=>{a[p.formation]=p.links}),nn(a)}catch(n){console.warn("Impossible de charger les overrides de formation:",n)}if(!ve.profile){rn(document.getElementById("app"),{state:ve,navigate:async()=>{await kt(),Wt()},toast:$t,refreshProfile:kt});return}const t=Array.isArray(ve.profile.pending_boosters)?ve.profile.pending_boosters:[];if(!ve.profile.onboarding_done&&t.length>0){Wn(document.getElementById("app"),{state:ve,navigate:()=>Wt(),toast:$t,refreshProfile:kt});return}ve.profile.theme&&ve.profile.theme!==Pt()&&(localStorage.setItem(gi,ve.profile.theme),ni(ve.profile.theme)),Wt(),$.auth.onAuthStateChange(async(n,a)=>{n==="SIGNED_OUT"&&(ve.user=null,ve.profile=null,document.getElementById("app").innerHTML="",xi(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:$t}))})}function jo(){return Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight)}function Kt(){const e=document.getElementById("app");e&&(e.style.height=jo()+"px")}window.addEventListener("resize",Kt);window.addEventListener("orientationchange",()=>setTimeout(Kt,150));window.visualViewport&&window.visualViewport.addEventListener("resize",Kt);function Wt(){const e=()=>{var n;(n=ve.user)!=null&&n.id&&$.from("users").update({last_seen_at:new Date().toISOString()}).eq("id",ve.user.id).then(()=>{})};e(),window._presencePingInterval&&clearInterval(window._presencePingInterval),window._presencePingInterval=setInterval(e,6e4);const t=document.getElementById("app");t.style.display="flex",t.style.flexDirection="column",Kt(),So(),Zi()}function Ii(){const e=document.getElementById("app-loader"),t=document.getElementById("app");t&&(t.style.display=""),e&&(e.classList.add("zoom-out"),setTimeout(()=>e.style.display="none",500))}function en(e){var a;const t=document.getElementById("app-loader");if(t&&(t.style.display="none"),document.getElementById("boot-error"))return;const n=document.createElement("div");n.id="boot-error",n.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:99999;gap:16px;color:#fff;padding:24px;text-align:center",n.innerHTML=`
