import{s as T,F as xi,h as Pt,j as Yi,l as at,i as zn,k as Ln,b as Sn}from"./formation-links-DJC25wD7.js";const In="/manager-wars/";function Li(e,{navigate:t,toast:i}){let a="login";const c=()=>{var s,l,p,n,o,f;const d=a==="login";e.innerHTML=`
    <div style="
    ...
    </style>`,(s=document.getElementById("tab-login-btn"))==null||s.addEventListener("click",()=>{a="login",c()}),(l=document.getElementById("tab-reg-btn"))==null||l.addEventListener("click",()=>{a="register",c()}),d?((p=document.getElementById("login-password"))==null||p.addEventListener("keydown",v=>{var _;v.key==="Enter"&&((_=document.getElementById("login-btn"))==null||_.click())}),(n=document.getElementById("login-btn"))==null||n.addEventListener("click",async()=>{const v=document.getElementById("login-email").value.trim(),_=document.getElementById("login-password").value,A=document.getElementById("login-error");if(A.textContent="",!v||!_){A.textContent="Remplissez tous les champs.";return}const r=document.getElementById("login-btn");r.textContent="⏳ Connexion…",r.disabled=!0;const{error:u}=await T.auth.signInWithPassword({email:v,password:_});if(r.textContent="⚽ Se connecter",r.disabled=!1,u){A.textContent=u.message.includes("Invalid")?"Email ou mot de passe incorrect.":u.message;return}window.location.reload()})):((o=document.getElementById("reg-confirm"))==null||o.addEventListener("keydown",v=>{var _;v.key==="Enter"&&((_=document.getElementById("reg-btn"))==null||_.click())}),(f=document.getElementById("reg-btn"))==null||f.addEventListener("click",async()=>{const v=document.getElementById("reg-email").value.trim(),_=document.getElementById("reg-password").value,A=document.getElementById("reg-confirm").value,r=document.getElementById("reg-error");if(r.textContent="",!v||!_||!A){r.textContent="Remplissez tous les champs.";return}if(_.length<6){r.textContent="Mot de passe trop court (min. 6 caractères).";return}if(_!==A){r.textContent="Les mots de passe ne correspondent pas.";return}const u=document.getElementById("reg-btn");u.textContent="⏳ Création…",u.disabled=!0;const{error:I}=await T.auth.signUp({email:v,password:_});if(u.textContent="🚀 Créer mon compte",u.disabled=!1,I){r.textContent=I.message;return}i("Compte créé ! Connecte-toi pour commencer.","success",4e3),a="login",c(),setTimeout(()=>{const z=document.getElementById("login-email");z&&(z.value=v)},50)}))};c()}function Mn(e,{state:t,navigate:i,toast:a,refreshProfile:c}){let d="#1A6B3C",s="#D4A017";e.innerHTML=`
  ...
  `;function l(){var A;const n=document.getElementById("logo-preview"),o=document.getElementById("logo-initials"),f=((A=document.getElementById("setup-club"))==null?void 0:A.value)||"MW",v=f.trim().split(" ").filter(Boolean),_=v.length>=2?(v[0][0]+v[1][0]).toUpperCase():f.slice(0,2).toUpperCase();n&&(n.style.background=s,n.style.borderColor=d),o&&(o.textContent=_,o.style.color=d)}document.getElementById("color1").addEventListener("input",n=>{d=n.target.value,document.getElementById("swatch1").style.background=d,l()}),document.getElementById("color2").addEventListener("input",n=>{s=n.target.value,document.getElementById("swatch2").style.background=s,l()});function p(n){document.querySelectorAll(".setup-step").forEach(o=>o.classList.remove("active")),document.getElementById(`step-${n}`).classList.add("active"),document.getElementById("step-num").textContent=n,document.getElementById("progress-fill").style.width=`${Math.round(n/3*100)}%`,n===3&&l()}document.getElementById("step1-next").addEventListener("click",async()=>{const n=document.getElementById("setup-pseudo").value.trim(),o=document.getElementById("step1-error");if(o.textContent="",n.length<3){o.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:f}=await T.from("users").select("id").eq("pseudo",n).maybeSingle();if(f){o.textContent="Ce pseudo est déjà pris.";return}p(2)}),document.getElementById("step2-back").addEventListener("click",()=>p(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const n=document.getElementById("setup-club").value.trim(),o=document.getElementById("step2-error");if(o.textContent="",n.length<2){o.textContent="Nom trop court (min. 2 caractères).";return}const{data:f}=await T.from("users").select("id").eq("club_name",n).maybeSingle();if(f){o.textContent="Ce nom de club est déjà pris.";return}p(3)}),document.getElementById("step3-back").addEventListener("click",()=>p(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const n=document.getElementById("setup-pseudo").value.trim(),o=document.getElementById("setup-club").value.trim(),f=document.getElementById("step3-error"),v=document.getElementById("step3-finish");f.textContent="",v.disabled=!0,v.textContent="Création en cours…";try{const{error:_}=await T.from("users").insert({id:t.user.id,pseudo:n,club_name:o,club_color1:d,club_color2:s,credits:1e4});if(_)throw _;await Cn(t.user.id),await c(),a(`Bienvenue ${n} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(_){f.textContent=_.message,v.disabled=!1,v.textContent="🚀 Créer mon profil !"}})}async function Cn(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await T.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const jn="modulepreload",qn=function(e){return"/manager-wars/"+e},Si={},yi=function(t,i,a){let c=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const s=document.querySelector("meta[property=csp-nonce]"),l=(s==null?void 0:s.nonce)||(s==null?void 0:s.getAttribute("nonce"));c=Promise.allSettled(i.map(p=>{if(p=qn(p),p in Si)return;Si[p]=!0;const n=p.endsWith(".css"),o=n?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${p}"]${o}`))return;const f=document.createElement("link");if(f.rel=n?"stylesheet":jn,n||(f.as="script"),f.crossOrigin="",f.href=p,l&&f.setAttribute("nonce",l),document.head.appendChild(f),n)return new Promise((v,_)=>{f.addEventListener("load",v),f.addEventListener("error",()=>_(new Error(`Unable to preload CSS for ${p}`)))})}))}function d(s){const l=new Event("vite:preloadError",{cancelable:!0});if(l.payload=s,window.dispatchEvent(l),!l.defaultPrevented)throw s}return c.then(s=>{for(const l of s||[])l.status==="rejected"&&d(l.reason);return t().catch(d)})},Bt="#1A6B3C",Dt="#cc2222",Fn="#D4A017",Ii="#888";async function Bn(e,t){const{state:i,toast:a}=t;e.innerHTML=`
    ...
    </div>`,await Wi(i,a,t),document.getElementById("btn-add-friend").addEventListener("click",()=>Gn(i,a)),document.getElementById("btn-accept-friend").addEventListener("click",()=>Ji(i,a,null,t))}async function Wi(e,t,i={}){const{navigate:a}=i,c=e.user.id,{data:d,error:s}=await T.from("friendships").select("id, requester_id, addressee_id").eq("status","accepted").or(`requester_id.eq.${c},addressee_id.eq.${c}`),{count:l}=await T.from("friendships").select("id",{count:"exact",head:!0}).eq("addressee_id",c).eq("status","pending"),p=document.getElementById("pending-badge");p&&(l>0?(p.style.display="flex",p.textContent=l):p.style.display="none");const n=document.getElementById("friends-list");if(!n)return;if(s){console.error("[Friends] Erreur:",s),n.innerHTML=`<div style="color:${Dt};text-align:center;padding:16px">Erreur chargement : ${s.message}</div>`;return}const o=(d||[]).map(_=>_.requester_id===c?_.addressee_id:_.requester_id);let f={};if(o.length){const{data:_}=await T.from("users").select("id, pseudo, club_name, last_seen_at, club_color1, club_color2").in("id",o);(_||[]).forEach(A=>{f[A.id]=A})}const v=(d||[]).map(_=>({friendshipId:_.id,friend:f[_.requester_id===c?_.addressee_id:_.requester_id]||{pseudo:"?"}}));if(!v.length){n.innerHTML=`
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      ${v.map(({friendshipId:_,friend:A})=>Dn(A,_)).join("")}
    </div>`,n.querySelectorAll("[data-stats]").forEach(_=>{_.addEventListener("click",()=>Pn(e,_.dataset.stats,_.dataset.friendName))}),n.querySelectorAll("[data-match]").forEach(_=>{_.addEventListener("click",()=>{const A=_.dataset.friendId,r=_.dataset.friendName;console.log("[Friends] clic match",{fid:A,fname:r,hasNavigate:typeof a}),typeof a=="function"?a("match",{matchMode:"friend",friendId:A,friendName:r}):t("Erreur navigation","error")})})}function Dn(e,t){const i=e.club_name||e.pseudo||"?",a=e.pseudo||"",c=(a||i).slice(0,2).toUpperCase(),d=e.club_color2||Bt,s=e.club_color1||"#ffffff",l=e.last_seen_at?new Date(e.last_seen_at):null,p=l&&Date.now()-l.getTime()<3*60*1e3;return`
    ...
    ${hi()}`,document.body.appendChild(i);const a=i.querySelector("#friend-pseudo-input"),c=i.querySelector("#add-friend-error"),d=()=>i.remove();a.focus(),i.querySelector("#add-cancel").addEventListener("click",d),i.addEventListener("click",s=>{s.target===i&&d()}),i.querySelector("#add-ok").addEventListener("click",async()=>{const s=a.value.trim();if(!s){c.textContent="Entre un pseudo";return}c.textContent="";const{data:l}=await T.from("users").select("id, pseudo").ilike("pseudo",s).single();if(!l){c.textContent="Utilisateur introuvable";return}if(l.id===e.user.id){c.textContent="Tu ne peux pas t'ajouter toi-même";return}const{data:p}=await T.from("friendships").select("id, status").or(`and(requester_id.eq.${e.user.id},addressee_id.eq.${l.id}),and(requester_id.eq.${l.id},addressee_id.eq.${e.user.id})`).single();if(p){const o=p.status==="accepted"?"Vous êtes déjà amis !":p.status==="pending"?"Demande déjà envoyée":"Une demande existe déjà";c.textContent=o;return}const{error:n}=await T.from("friendships").insert({requester_id:e.user.id,addressee_id:l.id,status:"pending"});if(n){c.textContent="Erreur : "+n.message;return}d(),t(`✅ Demande envoyée à ${l.pseudo} !`,"success")})}async function Ji(e,t,i=null,a={}){const c=e.user.id,{data:d}=await T.from("friendships").select("id, requester_id").eq("addressee_id",c).eq("status","pending").order("created_at",{ascending:!1}),s=(d||[]).map(v=>v.requester_id);let l={};if(s.length){const{data:v}=await T.from("users").select("id, pseudo, club_name").in("id",s);(v||[]).forEach(_=>{l[_.id]=_})}const p=(d||[]).map(v=>({...v,requester:l[v.requester_id]||{pseudo:"?"}})),n=bi(),o=p||[];n.innerHTML=`
    <div class="popup-box">
      ...
      ${o.length?`<div style="display:flex;flex-direction:column;gap:8px;max-height:50vh;overflow-y:auto;margin-bottom:14px">
            ${o.map(v=>{var _,A,r;return`
              <div style="display:flex;align-items:center;gap:10px;background:#f9f9f9;border-radius:10px;padding:10px 12px">
                <div style="flex:1;font-size:14px;font-weight:700">${((_=v.requester)==null?void 0:_.club_name)||((A=v.requester)==null?void 0:A.pseudo)||"?"}
    ...
    </style>`}async function Xi({player1Id:e,player2Id:t,score1:i,score2:a,gc1:c,gc2:d}){const[s,l]=[e,t].sort(),p=e!==s,n=p?a:i,o=p?i:a,f=p?d:c,v=p?c:d,_=n>o?1:0,A=o>n?1:0,r=n===o?1:0,{data:u}=await T.from("friend_match_stats").select("*").eq("player1_id",s).eq("player2_id",l).single();u?await T.from("friend_match_stats").update({wins_p1:u.wins_p1+_,wins_p2:u.wins_p2+A,draws:u.draws+r,goals_p1:u.goals_p1+n,goals_p2:u.goals_p2+o,gc_used_p1:u.gc_used_p1+f,gc_used_p2:u.gc_used_p2+v,matches_total:u.matches_total+1}).eq("player1_id",s).eq("player2_id",l):await T.from("friend_match_stats").insert({player1_id:s,player2_id:l,wins_p1:_,wins_p2:A,draws:r,goals_p1:n,goals_p2:o,gc_used_p1:f,gc_used_p2:v,matches_total:1})}const Nn="2026.07.02-0109";async function Mi(e,{state:t,navigate:i,toast:a}){var d,s;const c=t.profile;c&&(e.innerHTML=`
  <style>
  ...
  `,e.querySelectorAll("[data-nav]").forEach(l=>{l.addEventListener("click",p=>{p.preventDefault(),i(l.dataset.nav)})}),(d=document.getElementById("nav-rankings"))==null||d.addEventListener("click",()=>i("rankings")),(s=document.getElementById("nav-matches"))==null||s.addEventListener("click",()=>i("matches")),e.querySelectorAll("[data-action]").forEach(l=>{l.addEventListener("click",()=>{l.classList.add("tapped"),setTimeout(()=>l.classList.remove("tapped"),200);const p=l.dataset.action;if(p==="match-ai"){Un(i);return}if(p==="match-random"){i("match",{matchMode:"random"});return}if(p==="match-friend"){i("friends");return}if(p==="mini-league"){i("mini-league");return}if(p==="ranked"){ctx.toast("Mode Ranked — Bientôt disponible !","info");return}a("Bientôt disponible","info")})}),window.innerWidth<768&&requestAnimationFrame(()=>{var H,B,j,U,oe,ce;const l=((H=window.visualViewport)==null?void 0:H.height)||window.innerHeight,p=((B=document.querySelector(".top-nav"))==null?void 0:B.offsetHeight)||56,n=((j=document.querySelector(".bottom-nav"))==null?void 0:j.offsetHeight)||60,o=((U=e.querySelector(".hero-compact"))==null?void 0:U.offsetHeight)||52,f=e.querySelector(".ranked-banner");e.querySelector(".play-grid");const v=((ce=(oe=e.querySelector("#logout-btn"))==null?void 0:oe.closest("div"))==null?void 0:ce.offsetHeight)||44,_=["friend-requests-banner","match-invite-banner","ongoing-match-banner"].reduce((re,D)=>{var ie;return re+(((ie=document.getElementById(D))==null?void 0:ie.offsetHeight)||0)},0),A=14*5,r=l-p-n-o-v-_-A,u=Math.max(80,Math.round(r*.28)),I=Math.max(160,Math.round(r*.72)),z=Math.floor((I-10)/2);f&&(f.style.minHeight=f.style.maxHeight=u+"px"),e.querySelectorAll(".play-grid .play-card").forEach(re=>{re.style.minHeight=re.style.height=z+"px"});const C=Math.round(z*.55);e.querySelectorAll(".play-card .play-icon").forEach(re=>{re.style.height=C+"px"})}),document.getElementById("logout-btn").addEventListener("click",async()=>{await T.auth.signOut(),window.location.reload()}),Qi(t,a),Hn(t,a,i),Zi(t,a,i))}async function Zi(e,t,i){const a=document.getElementById("ongoing-match-banner");if(!a)return;const c=e.profile.id,{data:d}=await T.from("matches").select("id, home_id, away_id, status, mode").eq("status","active").or(`home_id.eq.${c},away_id.eq.${c}`).order("created_at",{ascending:!1});if(!(d!=null&&d.length)){a.innerHTML="";return}const s=d.map(p=>p.home_id===c?p.away_id:p.home_id).filter(Boolean);let l={};if(s.length){const{data:p}=await T.from("users").select("id, pseudo, club_name").in("id",s);(p||[]).forEach(n=>{l[n.id]=n.club_name||n.pseudo})}a.innerHTML=d.map(p=>{const n=p.home_id===c?p.away_id:p.home_id,o=l[n]||"Adversaire",f=p.mode==="mini_league";return`
      ...
      </div>`}).join(""),a.querySelectorAll("[data-resume]").forEach(p=>{p.addEventListener("click",async()=>{const n=document.getElementById("page-content")||document.getElementById("app");if(p.dataset.mini==="1"){const{data:o}=await T.from("mini_league_matches").select("id, league_id").eq("match_id",p.dataset.resume).single();o?i("match-mini-league",{mlMatchId:o.id,leagueId:o.league_id}):i("mini-league")}else{const{resumePvpMatch:o}=await yi(async()=>{const{resumePvpMatch:f}=await Promise.resolve().then(()=>Ho);return{resumePvpMatch:f}},void 0);o(n,{state:e,navigate:i,toast:t,openModal:null,closeModal:null,refreshProfile:null},p.dataset.resume)}})}),a.querySelectorAll("[data-abandon]").forEach(p=>{p.addEventListener("click",()=>{On(async()=>{await Rn(p.dataset.abandon,p.dataset.opp,c),t("Match abandonné (défaite 3-0)","info"),Zi(e,t,i)})})})}async function Rn(e,t,i){const{data:a}=await T.from("matches").select("home_id, away_id, game_state").eq("id",e).single();if(!a)return;const c=a.home_id===i,d=c?0:3,s=c?3:0,l=a.game_state||{};l.p1Score=d,l.p2Score=s,l.phase="finished",l.forfeit=!0,await T.from("matches").update({status:"finished",forfeit:!0,winner_id:t,home_score:d,away_score:s,game_state:l}).eq("id",e)}function On(e){const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",t.innerHTML=`
  ...
  `,document.body.appendChild(i);const a=()=>i.remove();document.getElementById("diff-cancel").addEventListener("click",a),i.addEventListener("click",c=>{c.target===i&&a()}),i.querySelectorAll("[data-mode]").forEach(c=>{c.addEventListener("click",()=>{a(),e("match",{matchMode:c.dataset.mode})})})}const Ue={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé pour ce match."},"Double attaque":{icon:"⚡",desc:"La note d'attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la note d'un joueur adverse."},Gel:{icon:"❄️",desc:"Bloque le meilleur attaquant IA."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function $e(e,t){if(!e)return 0;switch(t){case"GK":return Number(e.note_g)||0;case"DEF":return Number(e.note_d)||0;case"MIL":return Number(e.note_m)||0;case"ATT":return Number(e.note_a)||0;default:return 0}}const Ci=["ATT","MIL","DEF","GK"];function en(e){let t=0;const i=e.length;for(let a=0;a<i;a++)for(let c=a+1;c<i;c++){const d=e[a],s=e[c];if(!d||!s)continue;const l=d._col!=null&&s._col!=null&&d._col===s._col,p=d._col!=null&&s._col!=null&&Math.abs(d._col-s._col)===1,n=Ci.indexOf(d._line||d.job),o=Ci.indexOf(s._line||s.job),f=Math.abs(n-o)===1;if(!((d._line||d.job)===(s._line||s.job)&&p||l&&f))continue;const A=d.country_code&&s.country_code&&d.country_code===s.country_code,r=d.club_id&&s.club_id&&d.club_id===s.club_id;A&&r?t+=2:(A||r)&&(t+=1)}return t}function Nt(e,t={}){const i=e.reduce((d,s)=>{const l=s._line||s.job;return d+(Number(l==="MIL"?s.note_m:s.note_a)||0)+(s.boost||0)},0),a=en(e);let c=i+a;return t.doubleAttack&&(c*=2),t.stolenNote&&(c-=t.stolenNote),{base:i,links:a,total:Math.max(0,c)}}function Rt(e,t={}){const i=e.reduce((d,s)=>{const l=s._line||s.job;let p=0;return l==="GK"?p=Number(s.note_g)||0:l==="MIL"?p=Number(s.note_m)||0:p=Number(s.note_d)||0,d+p+(s.boost||0)},0),a=en(e);let c=i+a;return t.stolenNote&&(c-=t.stolenNote),{base:i,links:a,total:Math.max(0,c)}}function Ot(e,t,i={}){return i.shield?{goal:!1,shielded:!0}:{goal:e>t,shielded:!1}}function tn(e,t,i="easy"){const a=e.filter(s=>!s.used);if(!a.length)return[];const c=[...a].sort((s,l)=>{const p=t==="attack"?$e(s,"ATT"):s._line==="GK"?$e(s,"GK"):$e(s,"DEF");return(t==="attack"?$e(l,"ATT"):l._line==="GK"?$e(l,"GK"):$e(l,"DEF"))-p});let d=i==="easy"?1+Math.floor(Math.random()*2):i==="medium"?2+Math.floor(Math.random()*2):3;return c.slice(0,Math.min(d,c.length,3))}function Vn(e,t){const i={vs_ai_easy:{victoire:500,nul:250,defaite:50},vs_ai_medium:{victoire:1e3,nul:500,defaite:50},vs_ai_hard:{victoire:1500,nul:750,defaite:100},vs_ai_club:{victoire:2500,nul:1250,defaite:100}};return(i[e]||i.vs_ai_easy)[t]||0}const nn={pepite:{win:2,loss:-1,boosterStart:"min"},papyte:{win:1,loss:-2,boosterStart:"max"}};function vi(e,t){const i=e.player;if(!i)return 0;const a=i.rarity;if(a!=="pepite"&&a!=="papyte")return Number(i[t])||0;const c=wi(i),d=Number(i[t])||0;if(d<=0)return 0;const s=i.note_min??1,l=i.note_max??10,n=(e.current_note??c)-c;return Math.min(l,Math.max(s,d+n))}function wi(e){const t=e.job||"ATT";return Number(t==="GK"?e.note_g:t==="DEF"?e.note_d:t==="MIL"?e.note_m:e.note_a)||0}async function _i(e,t){!e||!t||await Promise.all([ji(e,"win"),ji(t,"loss")])}async function ji(e,t){const{data:i}=await T.from("cards").select("id, current_note, player:players(rarity, job, note_g, note_d, note_m, note_a, note_min, note_max)").eq("owner_id",e).eq("card_type","player");if(!(i!=null&&i.length))return;const a=i.filter(c=>{var d,s;return((d=c.player)==null?void 0:d.rarity)==="pepite"||((s=c.player)==null?void 0:s.rarity)==="papyte"});a.length&&await Promise.all(a.map(c=>{const d=nn[c.player.rarity],s=t==="win"?d.win:d.loss,l=c.player.note_min??1,p=c.player.note_max??10,n=wi(c.player),o=c.current_note??n,f=Math.min(p,Math.max(l,o+s));return T.from("cards").update({current_note:f}).eq("id",c.id)}))}const on=Object.freeze(Object.defineProperty({__proto__:null,EVOLUTIVE_RULES:nn,currentSecondaryNote:vi,getBaseMainNote:wi,updateEvolutiveCards:_i},Symbol.toStringTag,{value:"Module"})),rn={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"},jt={GK:"#111111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},Ut=["GK","DEF","MIL","ATT"],Kn=["Tous","GK","DEF","MIL","ATT"],Yn={normal:1e3,pepite:5e3,papyte:5e3,legende:1e4};function qi(e){const t=e.player;if(!t)return null;const i=t.rarity;return(i==="pepite"||i==="papyte")&&e.current_note!=null?e.current_note:Math.max(Number(t.note_g)||0,Number(t.note_d)||0,Number(t.note_m)||0,Number(t.note_a)||0)}function Wn(e){return e.length?e.reduce((t,i)=>qi(i)>qi(t)?i:t,e[0]):e[0]}function an(e){return e==="GK"?"note_g":e==="DEF"?"note_d":e==="MIL"?"note_m":"note_a"}const $i={MA:"MAROC",FR:"FRANCE",AR:"ARGENTINE",PT:"PORTUGAL",BR:"BRESIL",ES:"ESPAGNE",DE:"ALLEMAGNE",GB:"ANGLETERRE",IT:"ITALIE",CM:"CAMEROUN",SN:"SENEGAL",NG:"NIGERIA",DK:"DANEMARK",NL:"PAYS-BAS",BE:"BELGIQUE",CI:"CÔTE D'IVOIRE",AL:"ALBANIE",HR:"CROATIE",RS:"SERBIE",TR:"TURQUIE"};function sn(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";if(!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function Gt(e,t){return e&&Number(t==="GK"?e.note_g:t==="DEF"?e.note_d:t==="MIL"?e.note_m:e.note_a)||0}function ai(e,t=""){var v,_;const i=e.player;if(!i)return"";const a=i.job||"ATT",c=jt[a],d=rn[i.rarity]||"#ccc",s=i.rarity==="pepite"||i.rarity==="papyte",l=s&&e.current_note!=null?e.current_note:Gt(i,a),p=i.job2?s?vi(e,an(i.job2)):Gt(i,i.job2):null,n=i.job2?jt[i.job2]:null,o=sn(i),f=$i[i.country_code]||i.country_code||"";return`
  <div style="
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
  ...
  </div>`}async function Jn(e,t){const{state:i,navigate:a,toast:c,openModal:d,closeModal:s}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:l}=await T.from("cards").select(`id, card_type, current_note, gc_type, formation, is_for_sale, sale_price,
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),n=(l||[]).filter(Y=>Y.card_type==="player"&&Y.player),o=(l||[]).filter(Y=>Y.card_type==="game_changer"),f=(l||[]).filter(Y=>Y.card_type==="formation"),{data:v}=await T.from("gc_definitions").select("name,gc_type,color,effect,image_url"),_={};(v||[]).forEach(Y=>{_[Y.name]=Y});const A=Object.keys(xi),r=Object.keys(Ue),u={};n.forEach(Y=>{const J=Y.player.id;u[J]=(u[J]||0)+1}),new Set(Object.keys(u).map(Y=>String(Y)));const I=new Set(f.map(Y=>Y.formation)),z=new Set(o.map(Y=>Y.gc_type));let C="player",H="Tous",B="",j=!1;function U(){return[...n].sort((Y,J)=>{const N=Ut.indexOf(Y.player.job),R=Ut.indexOf(J.player.job);return N!==R?N-R:(Y.player.surname_encoded||"").localeCompare(J.player.surname_encoded||"")})}function oe(){return[...p||[]].sort((Y,J)=>{const N=Ut.indexOf(Y.job),R=Ut.indexOf(J.job);return N!==R?N-R:(Y.surname_encoded||"").localeCompare(J.surname_encoded||"")})}function ce(){return U().filter(Y=>{const J=Y.player,N=H==="Tous"||J.job===H,R=!B||`${J.firstname} ${J.surname_encoded}`.toLowerCase().includes(B);return N&&R})}function re(){return oe().filter(Y=>{const J=H==="Tous"||Y.job===H,N=!B||`${Y.firstname} ${Y.surname_encoded}`.toLowerCase().includes(B);return J&&N})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(A=document.getElementById("direct-sell-form-btn"))==null||A.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${n} pour ${Vt.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const I=v.find(C=>!C.is_for_sale)||v[0];if(!I){d("Aucune carte à vendre","error");return}await T.from("market_listings").delete().eq("card_id",I.id),await T.from("transfer_history").delete().eq("card_id",I.id);const{error:z}=await T.from("cards").delete().eq("id",I.id);if(z){d(z.message,"error");return}await T.from("users").update({credits:(c.profile.credits||0)+Vt}).eq("id",c.profile.id),await p(),d(`+${Vt.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),s(),l("collection")}),(r=document.getElementById("market-sell-form-btn"))==null||r.addEventListener("click",async()=>{const I=parseInt(document.getElementById("sell-price-form").value);if(!I||I<1){d("Prix invalide","error");return}await T.from("cards").update({is_for_sale:!0,sale_price:I}).eq("id",e.id),await T.from("market_listings").insert({seller_id:c.profile.id,card_id:e.id,price:I}),d("Carte mise en vente sur le marché !","success"),s(),l("collection")}),(u=document.getElementById("cancel-sell-form-btn"))==null||u.addEventListener("click",async()=>{await T.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await T.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),d("Annonce retirée","success"),s(),l("collection")})}async function Bi(e,t,i,a){var fe,xe,Ee,_e,he,Y,J;const{state:c,toast:d,openModal:s,closeModal:l,navigate:p,refreshProfile:n}=a,o=e.player,f=t.filter(N=>N.player.id===o.id),v=f.length,_=Math.max(Number(o.note_g)||0,Number(o.note_d)||0,Number(o.note_m)||0,Number(o.note_a)||0),A=o.rarity||"normal",{data:r}=await T.from("sell_price_configs").select("*").eq("rarity",A).lte("note_min",_).gte("note_max",_).order("note_min",{ascending:!1}).limit(1),u=((fe=r==null?void 0:r[0])==null?void 0:fe.price)??Yn[A]??1e3,I=o.rarity!=="legende",z=sn(o),C=(o.rarity==="pepite"||o.rarity==="papyte")&&e.current_note!=null?e.current_note:Gt(o,o.job),H=o.rarity==="pepite"||o.rarity==="papyte",B=o.job2?H?vi(e,an(o.job2)):Gt(o,o.job2):null,j=jt[o.job]||"#1A6B3C",U=o.job2?jt[o.job2]:null,oe=rn[o.rarity]||"#ccc",ce=$i[o.country_code]||o.country_code||"",re=f.map(N=>N.id);let D={};if(re.length){const{data:N}=await T.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",re).order("transferred_at",{ascending:!0});(N||[]).forEach(R=>{D[R.card_id]||(D[R.card_id]=[]),D[R.card_id].push(R)})}const ie=N=>{const R=N.transferred_at?new Date(N.transferred_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"2-digit",year:"numeric"}):"",Q=N.source==="booster"?"Booster":N.price?N.price.toLocaleString("fr"):"—";return`
    ...
    ${W}
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(_e=document.getElementById("close-detail"))==null||_e.addEventListener("click",l);let G=new Set;const se=()=>{const N=G.size,R=document.getElementById("sell-action-panel");R&&(R.style.display=N>0?"block":"none",document.getElementById("sell-selected-count").textContent=N,document.getElementById("sell-direct-total").textContent=(N*u).toLocaleString("fr")+" cr.")};document.querySelectorAll(".expl-check").forEach(N=>{N.addEventListener("change",()=>{const R=N.dataset.id;N.checked?G.add(R):G.delete(R);const Q=N.closest(".exemplaire-row");Q&&(Q.style.borderColor=N.checked?"#1A6B3C":"#eee"),se()})}),document.querySelectorAll(".exemplaire-row").forEach(N=>{N.addEventListener("click",R=>{if(R.target.closest("button")||R.target.tagName==="INPUT")return;const Q=N.querySelector(".expl-check");Q&&!Q.disabled&&(Q.checked=!Q.checked,Q.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(N=>{N.addEventListener("click",R=>{R.stopPropagation();const Q=document.querySelector(`.expl-hist[data-hist="${N.dataset.idx}"]`);Q&&(Q.style.display=Q.style.display==="none"?"flex":"none")})}),(he=document.getElementById("direct-sell-btn"))==null||he.addEventListener("click",async()=>{const N=[...G];if(!N.length)return;const R=N.length*u;if(!confirm(`Vendre ${N.length} carte${N.length>1?"s":""} ${o.surname_encoded} pour ${R.toLocaleString("fr")} crédits ? Action irréversible.`))return;await T.from("market_listings").delete().in("card_id",N),await T.from("transfer_history").delete().in("card_id",N);const{error:Q}=await T.from("cards").delete().in("id",N);if(Q){d(Q.message,"error");return}await T.from("users").update({credits:(c.profile.credits||0)+R}).eq("id",c.profile.id),await n();const ge=document.getElementById("nav-credits");ge&&(ge.textContent=`💰 ${(c.profile.credits||0).toLocaleString("fr")}`),d(`+${R.toLocaleString("fr")} crédits ! ${N.length} carte${N.length>1?"s":""} vendue${N.length>1?"s":""}.`,"success"),l(),p("collection")}),(Y=document.getElementById("market-sell-btn"))==null||Y.addEventListener("click",async()=>{var Me;const N=[...G];if(!N.length){d("Sélectionne au moins un exemplaire","warning");return}const R=parseInt((Me=document.getElementById("sell-market-price"))==null?void 0:Me.value);if(!R||R<1){d("Prix invalide","error");return}const{error:Q}=await T.from("cards").update({is_for_sale:!0,sale_price:R}).in("id",N);if(Q){d(Q.message,"error");return}const ge=N.map(Ce=>({seller_id:c.profile.id,card_id:Ce,price:R,status:"active"})),{error:pe}=await T.from("market_listings").insert(ge);pe&&console.warn("[Market] insert listings:",pe.message),d(`${N.length} carte${N.length>1?"s":""} mise${N.length>1?"s":""} en vente à ${R.toLocaleString("fr")} cr. chacune !`,"success"),l(),p("collection")}),(J=document.getElementById("cancel-sell-btn"))==null||J.addEventListener("click",async()=>{await T.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await T.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),d("Annonce retirée","success"),l(),p("collection")})}const Xt={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},vt={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function ti(e){const t=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function dn(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function ln(e){var i;const t="https://fcnwclxlkytdfjotqkta.supabase.co";return(i=e==null?void 0:e.clubs)!=null&&i.logo_url?e.clubs.logo_url.startsWith("http")?e.clubs.logo_url:`${t}/storage/v1/object/public/assets/clubs/${e.clubs.logo_url}`:null}function Qn(e,t=44,i=58){var _;const a=e?ti(e):null,c=e?ln(e):null,d=dn(e==null?void 0:e.country_code),s=(e==null?void 0:e.job)||"MIL",l=vt[s]||"#555",p={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",n=Number(s==="GK"?e==null?void 0:e.note_g:s==="DEF"?e==null?void 0:e.note_d:s==="MIL"?e==null?void 0:e.note_m:e==null?void 0:e.note_a)||0,o=Math.round(i*.19),f=Math.round(i*.25),v=i-o-f;return e?`<div style="width:${t}px;height:${i}px;border-radius:5px;border:2px solid ${p};background:${l};position:relative;overflow:hidden;flex-shrink:0">
  ...
  </div>`,document.getElementById("new-deck-btn").addEventListener("click",async()=>{const s=prompt("Nom du deck :",`Deck ${((d==null?void 0:d.length)||0)+1}`);if(!s)return;const{data:l,error:p}=await T.from("decks").insert({owner_id:i.profile.id,name:s}).select().single();if(p){c(p.message,"error");return}c("Deck créé !","success"),Di(l.id,e,t)}),e.querySelectorAll("[data-open-deck]").forEach(s=>{s.addEventListener("click",()=>Di(s.dataset.openDeck,e,t))}),e.querySelectorAll("[data-rename]").forEach(s=>{s.addEventListener("click",async()=>{const l=prompt("Nouveau nom :",s.dataset.name);if(!l||l===s.dataset.name)return;const{error:p}=await T.from("decks").update({name:l}).eq("id",s.dataset.rename);if(p){c(p.message,"error");return}c("Deck renommé !","success"),li(e,t)})}),e.querySelectorAll("[data-delete]").forEach(s=>{s.addEventListener("click",async()=>{if(!confirm(`Supprimer le deck "${s.dataset.name}" ? Cette action est irréversible.`))return;await T.from("deck_cards").delete().eq("deck_id",s.dataset.delete);const{error:l}=await T.from("decks").delete().eq("id",s.dataset.delete);if(l){c(l.message,"error");return}c("Deck supprimé.","success"),li(e,t)})})}async function Di(e,t,i){const{state:a,toast:c}=i;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:d}=await T.from("decks").select("*").eq("id",e).single(),{data:s}=await T.from("cards").select(`id, card_type, formation,
      player:players(id, firstname, surname_encoded, country_code, club_id, job, job2,
        note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length,
        clubs(encoded_name, logo_url))`).eq("owner_id",a.profile.id),l=(s||[]).filter(_=>_.card_type==="player"&&_.player),p=(s||[]).filter(_=>_.card_type==="formation"),n=p.map(_=>_.formation).filter(Boolean),{data:o}=await T.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",e);let f=d.formation||"4-4-2";n.length>0&&!n.includes(f)&&(f=n[0]);const v={deckId:e,name:d.name,formation:f,formationCardId:d.formation_card_id,slots:{},subs:[],playerCards:l,formationCards:p,availableFormations:n};(o||[]).forEach(_=>{_.is_starter?v.slots[_.position]=_.card_id:v.subs.includes(_.card_id)||v.subs.push(_.card_id)}),zt(t,v,i)}function zt(e,t,i){var p;const{navigate:a}=i;Xt[t.formation];const c=Gi(t.formation),d=c.filter(n=>t.slots[n]).length,s=t.availableFormations.length>0?t.availableFormations:Object.keys(Xt),l=t.subs.map(n=>t.playerCards.find(o=>o.id===n)).filter(Boolean);[...Object.values(t.slots),...t.subs],e.innerHTML=`
  <div style="height:100%;overflow-y:auto;padding-bottom:80px;background:var(--page-bg)">
    ...
    <div style="padding:10px 12px;background:rgba(0,0,0,0.25);border-top:1px solid rgba(255,255,255,0.1)">
      ...
      <div style="display:flex;gap:8px;align-items:flex-end;overflow-x:auto;padding-bottom:4px" id="subs-list">
        ${l.map(n=>{const o=n.player;return`<div style="position:relative;flex-shrink:0">
            ${Qn(o,44,58)}
  ...
  </div>`,eo(e,t,c,i),document.getElementById("builder-back").addEventListener("click",()=>a("decks")),document.getElementById("formation-select").addEventListener("change",n=>{t.formation=n.target.value;const o=Gi(t.formation),f={};o.forEach(v=>{t.slots[v]&&(f[v]=t.slots[v])}),t.slots=f,zt(e,t,i)}),document.getElementById("save-deck").addEventListener("click",()=>no(t,i)),e.querySelectorAll("[data-remove-sub]").forEach(n=>{n.addEventListener("click",()=>{t.subs=t.subs.filter(o=>o!==n.dataset.removeSub),zt(e,t,i)})}),(p=document.getElementById("add-sub-btn"))==null||p.addEventListener("click",()=>{io(t,e,i)})}function eo(e,t,i,a){const c=e.querySelector("#deck-field");if(!c)return;const d=Pt[t.formation]||{},s=Yi(t.formation)||[],l={};for(const I of i){const z=t.slots[I],C=z?t.playerCards.find(H=>H.id===z):null;l[I]=C?C.player:null}const p=300,n=300,o=48,f=64,v=13,_=16,A=38;function r(I){const z=d[I];return z?{x:z.x*p,y:z.y*n}:null}let u="";for(const[I,z]of s){const C=r(I),H=r(z);if(!C||!H)continue;const B=l[I]?{...l[I],club_id:l[I].club_id,country_code:l[I].country_code,rarity:l[I].rarity}:null,j=l[z]?{...l[z],club_id:l[z].club_id,country_code:l[z].country_code,rarity:l[z].rarity}:null,U=at(B,j);U==="#ff3333"||U==="#cc2222"?u+=`<line x1="${C.x.toFixed(1)}" y1="${C.y.toFixed(1)}" x2="${H.x.toFixed(1)}" y2="${H.y.toFixed(1)}" stroke="${U}" stroke-width="2.5" stroke-linecap="round" opacity="0.4"/>`:(u+=`<line x1="${C.x.toFixed(1)}" y1="${C.y.toFixed(1)}" x2="${H.x.toFixed(1)}" y2="${H.y.toFixed(1)}" stroke="${U}" stroke-width="8" stroke-linecap="round" opacity="0.2"/>`,u+=`<line x1="${C.x.toFixed(1)}" y1="${C.y.toFixed(1)}" x2="${H.x.toFixed(1)}" y2="${H.y.toFixed(1)}" stroke="${U}" stroke-width="2.5" stroke-linecap="round" opacity="0.9"/>`)}for(const I of i){const z=r(I);if(!z)continue;const C=l[I],H=I.replace(/\d+/,""),B=vt[H]||"#555",j=(z.x-o/2).toFixed(1),U=(z.y-f/2).toFixed(1),oe={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[C==null?void 0:C.rarity]||"#aaa";if(C){const ce=ti(C),re=ln(C),D=dn(C.country_code),ie=Number(H==="GK"?C.note_g:H==="DEF"?C.note_d:H==="MIL"?C.note_m:C.note_a)||0,W=f-v-_;u+=`<defs><clipPath id="dcp-${I}"><rect x="${j}" y="${(z.y-f/2+v).toFixed(1)}" width="${o}" height="${W}"/></clipPath></defs>`,u+=`<rect x="${j}" y="${U}" width="${o}" height="${f}" rx="5" fill="${B}"/>`,ce&&(u+=`<image href="${ce}" x="${j}" y="${(z.y-f/2+v).toFixed(1)}" width="${o}" height="${W}" clip-path="url(#dcp-${I})" preserveAspectRatio="xMidYMin slice"/>`),u+=`<rect x="${j}" y="${U}" width="${o}" height="${v}" fill="rgba(255,255,255,0.93)"/>`,u+=`<text x="${z.x.toFixed(1)}" y="${(z.y-f/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(C.surname_encoded||"").slice(0,9)}</text>`;const G=(z.y+f/2-_).toFixed(1);u+=`<rect x="${j}" y="${G}" width="${o}" height="${_}" fill="rgba(255,255,255,0.93)"/>`,D&&(u+=`<image href="${D}" x="${(z.x-21).toFixed(1)}" y="${(z.y+f/2-_+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`),u+=`<text x="${z.x.toFixed(1)}" y="${(z.y+f/2-_/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${ie}</text>`,re&&(u+=`<image href="${re}" x="${(z.x+o/2-14).toFixed(1)}" y="${(z.y+f/2-_+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`),u+=`<rect x="${j}" y="${U}" width="${o}" height="${f}" rx="5" fill="none" stroke="${oe}" stroke-width="2"/>`}else u+=`<rect x="${j}" y="${U}" width="${o}" height="${f}" rx="5" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="5,3"/>`,u+=`<text x="${z.x.toFixed(1)}" y="${z.y.toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="22" fill="rgba(255,255,255,0.35)">+</text>`,u+=`<text x="${z.x.toFixed(1)}" y="${(z.y+16).toFixed(1)}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.3)">${H}</text>`;u+=`<rect x="${j}" y="${U}" width="${o}" height="${f}" rx="5" fill="rgba(0,0,0,0.01)" class="deck-slot-hit" data-pos="${I}" style="cursor:pointer"/>`}c.innerHTML=`<svg viewBox="${-A} ${-A} ${p+A*2} ${n+A*2}" width="100%" style="display:block;max-width:440px;margin:0 auto">${u}</svg>`,c.querySelectorAll(".deck-slot-hit").forEach(I=>{I.addEventListener("click",()=>to(I.dataset.pos,t,e,a))})}function to(e,t,i,a){var v,_,A;const{openModal:c,closeModal:d}=a,s=e.replace(/\d+/,""),l=t.slots[e],p=l?t.playerCards.find(r=>r.id===l):null;(v=p==null?void 0:p.player)==null||v.id;const n=new Set;Object.entries(t.slots).forEach(([r,u])=>{var z;if(r===e||!u)return;const I=t.playerCards.find(C=>C.id===u);(z=I==null?void 0:I.player)!=null&&z.id&&n.add(I.player.id)}),t.subs.forEach(r=>{var I;const u=t.playerCards.find(z=>z.id===r);(I=u==null?void 0:u.player)!=null&&I.id&&n.add(u.player.id)});const o=new Set,f=t.playerCards.filter(r=>{const u=r.player;return!(u.job===s||u.job2===s)||n.has(u.id)||o.has(u.id)?!1:(o.add(u.id),!0)});f.sort((r,u)=>{const I=s==="GK"?r.player.note_g:s==="DEF"?r.player.note_d:s==="MIL"?r.player.note_m:r.player.note_a;return(s==="GK"?u.player.note_g:s==="DEF"?u.player.note_d:s==="MIL"?u.player.note_m:u.player.note_a)-I}),c(`Choisir ${s} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${f.length>0?f.map(r=>{var H,B;const u=r.player,I=s==="GK"?u.note_g:s==="DEF"?u.note_d:s==="MIL"?u.note_m:u.note_a,z=ti(u),C={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[u.rarity];return`<div class="player-option" data-card-id="${r.id}"
    ...
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(_=document.getElementById("close-selector"))==null||_.addEventListener("click",d),(A=document.getElementById("remove-player"))==null||A.addEventListener("click",()=>{delete t.slots[e],d(),zt(i,t,a)}),document.querySelectorAll(".player-option").forEach(r=>{r.addEventListener("click",()=>{t.slots[e]=r.dataset.cardId,d(),zt(i,t,a)})})}function io(e,t,i){var p;const{openModal:a,closeModal:c}=i,d=new Set;Object.values(e.slots).filter(Boolean).forEach(n=>{var f;const o=e.playerCards.find(v=>v.id===n);(f=o==null?void 0:o.player)!=null&&f.id&&d.add(o.player.id)}),e.subs.forEach(n=>{var f;const o=e.playerCards.find(v=>v.id===n);(f=o==null?void 0:o.player)!=null&&f.id&&d.add(o.player.id)});const s=new Set,l=e.playerCards.filter(n=>{var o,f,v;return d.has((o=n.player)==null?void 0:o.id)||s.has((f=n.player)==null?void 0:f.id)?!1:(s.add((v=n.player)==null?void 0:v.id),!0)});a("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${l.length>0?l.map(n=>{var _;const o=n.player,f=ti(o),v=o.job==="GK"?o.note_g:o.job==="DEF"?o.note_d:o.job==="MIL"?o.note_m:o.note_a;return`<div class="player-option" data-card-id="${n.id}"
          style="display:flex;align-items:center;gap:10px;padding:8px;border:1.5px solid var(--gray-200);border-radius:10px;cursor:pointer">
    ...
    </div>`,'<button class="btn btn-ghost" id="close-sub-selector">Fermer</button>'),(p=document.getElementById("close-sub-selector"))==null||p.addEventListener("click",c),document.querySelectorAll(".player-option").forEach(n=>{n.addEventListener("click",()=>{e.subs.push(n.dataset.cardId),c(),zt(t,e,i)})})}async function no(e,t){const{state:i,toast:a,navigate:c}=t,d=e.formationCards.find(p=>p.formation===e.formation),s=(d==null?void 0:d.id)||e.formationCardId;await T.from("decks").update({formation:e.formation,formation_card_id:s||null}).eq("id",e.deckId),await T.from("deck_cards").delete().eq("deck_id",e.deckId);const l=[];if(Object.entries(e.slots).forEach(([p,n],o)=>{l.push({deck_id:e.deckId,card_id:n,position:p,is_starter:!0,slot_order:o})}),e.subs.forEach((p,n)=>{l.push({deck_id:e.deckId,card_id:p,position:`SUB${n+1}`,is_starter:!1,slot_order:100+n})}),l.length>0){const{error:p}=await T.from("deck_cards").insert(l);if(p){a(p.message,"error");return}}a("Deck enregistré ✅","success"),c("decks")}function Gi(e){const t=Xt[e]||Xt["4-4-2"],i=["GK1"];for(let a=1;a<=t.DEF;a++)i.push(`DEF${a}`);for(let a=1;a<=t.MIL;a++)i.push(`MIL${a}`);for(let a=1;a<=t.ATT;a++)i.push(`ATT${a}`);return i}async function cn(){const{data:e}=await T.from("booster_configs").select("*").eq("is_active",!0).order("sort_order");if(!(e!=null&&e.length))return[];const{data:t}=await T.from("booster_drop_rates").select("*").in("booster_id",e.map(i=>i.id)).order("sort_order");return e.map(i=>({...i,rates:(t||[]).filter(a=>a.booster_id===i.id)}))}function oo(e){if(!(e!=null&&e.length))return null;const t=e.reduce((a,c)=>a+Number(c.percentage),0);let i=Math.random()*t;for(const a of e)if(i-=Number(a.percentage),i<=0)return a;return e[e.length-1]}const pn=()=>Object.keys(Pt),ro=[{id:"players_std",img:"/manager-wars/icons/booster-players.png",name:"Players",sub:"5 cartes joueurs",cost:5e3,costLabel:"5 000 crédits",cardCount:5,type:"player"},{id:"players_pub",img:"/manager-wars/icons/booster-silver.png",name:"Players (pub)",sub:"3 cartes joueurs",cost:0,costLabel:"1 pub",cardCount:3,type:"player"},{id:"game_changer",img:"/manager-wars/icons/booster-gamechanger.png",name:"Game Changer",sub:"3 cartes spéciales",cost:1e4,costLabel:"10 000 crédits",cardCount:3,type:"game_changer"},{id:"formation",img:"/manager-wars/icons/booster-formation.png",name:"Formation",sub:"1 carte formation",cost:1e4,costLabel:"10 000 crédits",cardCount:1,type:"formation"}],ci={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé."},"Double attaque":{icon:"⚡",desc:"La prochaine attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la prochaine action IA."},Gel:{icon:"❄️",desc:"Bloque le meilleur attaquant IA."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function ao(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";if(!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}const Pi={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},so={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"},lo={MA:"MAROC",FR:"FRANCE",AR:"ARGENTINE",PT:"PORTUGAL",BR:"BRESIL",ES:"ESPAGNE",DE:"ALLEMAGNE",GB:"ANGLETERRE",IT:"ITALIE",CM:"CAMEROUN",SN:"SENEGAL",NG:"NIGERIA",DK:"DANEMARK",NL:"PAYS-BAS",BE:"BELGIQUE",CI:"CÔTE D'IVOIRE",AL:"ALBANIE",HR:"CROATIE",RS:"SERBIE",TR:"TURQUIE"};function Ni(e,t){return e&&Number(t==="GK"?e.note_g:t==="DEF"?e.note_d:t==="MIL"?e.note_m:e.note_a)||0}function co(e){var o,f;const t=e.player;if(!t)return"";const i=t.job||"ATT",a=Pi[i],c=so[t.rarity]||"#ccc",d=Ni(t,i),s=t.job2?Ni(t,t.job2):null,l=t.job2?Pi[t.job2]:null,p=ao(t),n=lo[t.country_code]||t.country_code||"";return`
  ...
  </div>`,e.querySelectorAll(".booster-card:not(.disabled)").forEach(l=>{l.addEventListener("click",async()=>{const p=d.find(n=>n.id===l.dataset.booster);if(p){l.style.opacity="0.5",l.style.pointerEvents="none";try{await uo(p,{state:t,toast:a,navigate:i,container:e})}catch(n){a(n.message,"error"),l.style.opacity="",l.style.pointerEvents=""}}})}),e.querySelectorAll(".booster-info-btn").forEach(l=>{l.addEventListener("click",p=>{p.stopPropagation();const n=d.find(o=>o.id===l.dataset.boosterId);xo(n)})})}async function uo(e,{state:t,toast:i,navigate:a,container:c}){var o;if(e.cost>0&&t.profile.credits<e.cost){i("Crédits insuffisants","error");return}if(e.isPub)try{await ho()}catch(f){i(f.message||"Regardez la pub entièrement pour ouvrir le booster","error");return}const{data:d}=await T.from("cards").select("card_type, player_id, formation").eq("owner_id",t.profile.id),s=new Set((d||[]).filter(f=>f.card_type==="player").map(f=>f.player_id)),l=new Set((d||[]).filter(f=>f.card_type==="formation").map(f=>f.formation));let p=[];if((o=e.rates)!=null&&o.length)p=await pi(t.profile,e);else{const f=e.type||"player";f==="player"?p=await fn(t.profile,e.cardCount,e.cost):f==="game_changer"?p=await gn(t.profile,e.cardCount,e.cost):f==="formation"?p=await mn(t.profile,e.cost):p=await pi(t.profile,e)}p.forEach(f=>{f.card_type==="player"&&f.player?f.isDuplicate=s.has(f.player.id):f.card_type==="formation"&&(f.isDuplicate=l.has(f.formation))});const{data:n}=await T.from("users").select("*").eq("id",t.profile.id).single();n&&(t.profile=n),xn(p,e,a)}function fo(){const e=Math.random()*100;return e<.5?"legende":e<2?"special":e<10?"normal_high":"normal_low"}function mt(e){return Math.max(Number(e.note_g)||0,Number(e.note_d)||0,Number(e.note_m)||0,Number(e.note_a)||0)}function go(e,t){let i;switch(t){case"legende":i=e.filter(a=>a.rarity==="legende"),i.length||(i=e.filter(a=>a.rarity==="pepite"||a.rarity==="papyte")),i.length||(i=e.filter(a=>mt(a)>=6));break;case"special":i=e.filter(a=>a.rarity==="pepite"||a.rarity==="papyte"),i.length||(i=e.filter(a=>mt(a)>=6));break;case"normal_high":i=e.filter(a=>a.rarity==="normal"&&mt(a)>=6),i.length||(i=e.filter(a=>mt(a)>=6));break;default:i=e.filter(a=>a.rarity==="normal"&&mt(a)>=1&&mt(a)<=5),i.length||(i=e.filter(a=>a.rarity==="normal"));break}return i.length||(i=e),i[Math.floor(Math.random()*i.length)]}async function pi(e,t){if(t.cost>0){const{error:l}=await T.from("users").update({credits:e.credits-t.cost}).eq("id",e.id);if(l)throw l}const{data:i}=await T.from("cards").select("player_id, card_type, formation").eq("owner_id",e.id),a=new Set((i||[]).filter(l=>l.card_type==="player").map(l=>l.player_id)),c=new Set((i||[]).filter(l=>l.card_type==="formation").map(l=>l.formation)),d=new Set,s=[];for(let l=0;l<(t.cardCount||5);l++){const p=oo(t.rates);if(p){if(p.card_type==="player"){const n=z=>({légende:"legende",pépite:"pepite",pépites:"pepite"})[z]||z,o=p.rarity?n(p.rarity):null;let f=T.from("players").select("id,job,firstname,surname_encoded,country_code,club_id,rarity,note_g,note_d,note_m,note_a,skin,hair,hair_length,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);o&&(f=f.eq("rarity",o));const{data:v}=await f;let _=v||[];if((p.note_min||p.note_max)&&(_=_.filter(z=>{const C=Math.max(Number(z.note_g)||0,Number(z.note_d)||0,Number(z.note_m)||0,Number(z.note_a)||0);return(!p.note_min||C>=p.note_min)&&(!p.note_max||C<=p.note_max)})),_.length||(p.note_min||p.note_max?(_=v||[],console.warn("[Booster] Aucun joueur avec note",p.note_min,"-",p.note_max,"— fallback rareté uniquement")):_=v||[]),!_.length)continue;let A=_.filter(z=>!d.has(z.id));A.length||(A=_);const r=A[Math.floor(Math.random()*A.length)];d.add(r.id);const u=a.has(r.id),{data:I}=await T.from("cards").insert({owner_id:e.id,player_id:r.id,card_type:"player"}).select().single();I&&(s.push({...I,player:r,isDuplicate:u}),T.rpc("record_transfer",{p_card_id:I.id,p_player_id:r.id,p_club_name:e.club_name||e.pseudo,p_manager_name:e.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{}))}else if(p.card_type==="game_changer"){const{data:n}=await T.from("gc_definitions").select("id,name").eq("is_active",!0).eq("gc_type","game_changer"),o=n!=null&&n.length?n:[{name:"Ressusciter"},{name:"Double attaque"},{name:"Bouclier"},{name:"Vol de note"},{name:"Gel"}],v=o[Math.floor(Math.random()*o.length)].name,{data:_}=await T.from("cards").insert({owner_id:e.id,card_type:"game_changer",gc_type:v}).select().single();_&&s.push(_)}else if(p.card_type==="formation"){const n=pn(),o=n[Math.floor(Math.random()*n.length)],f=c.has(o),{data:v}=await T.from("cards").insert({owner_id:e.id,card_type:"formation",formation:o}).select();v!=null&&v[0]&&s.push({...v[0],isDuplicate:f})}}}return s}async function fn(e,t,i){if(i>0){const{error:n}=await T.from("users").update({credits:e.credits-i}).eq("id",e.id);if(n)throw n}const{data:a}=await T.from("players").select("id,job,firstname,surname_encoded,country_code,club_id,rarity,note_g,note_d,note_m,note_a,note_min,note_max,skin,hair,hair_length,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);if(!(a!=null&&a.length))throw new Error("Pas de joueurs en BDD — ajoutes-en via le panel admin !");const c=a.filter(n=>n.job==="GK"),d=a.filter(n=>n.job!=="GK"),s=!e.first_booster_opened&&c.length>0,l=[];for(let n=0;n<t;n++){const o=n===0&&s?c:n===0?d:a,f=fo(),v=go(o,f);v&&l.push(v)}s&&await T.from("users").update({first_booster_opened:!0}).eq("id",e.id);const{data:p}=await T.from("cards").insert(l.map(n=>({owner_id:e.id,player_id:n.id,card_type:"player"}))).select();return(p||[]).forEach((n,o)=>{T.rpc("record_transfer",{p_card_id:n.id,p_player_id:l[o].id,p_club_name:e.club_name||e.pseudo,p_manager_name:e.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{})}),l.map((n,o)=>({...p[o],player:n}))}async function gn(e,t,i){const{error:a}=await T.from("users").update({credits:e.credits-i}).eq("id",e.id);if(a)throw a;const{data:c}=await T.from("gc_definitions").select("id,name,gc_type,color,effect,image_url").eq("is_active",!0),d=c!=null&&c.length?c:Object.keys(ci).map(o=>({name:o,gc_type:"game_changer"})),s=Array.from({length:t},()=>{const o=d[Math.floor(Math.random()*d.length)];return{owner_id:e.id,card_type:"game_changer",gc_type:o.name,gc_definition_id:o.id||null}}),{data:l,error:p}=await T.from("cards").insert(s).select();return p&&console.error("[Booster GC] Erreur insert:",p.message,p),(l||[]).map(o=>{const f=c==null?void 0:c.find(v=>v.name===o.gc_type||v.id===o.gc_definition_id);return{...o,_gcDef:f||null}})}async function mn(e,t){const{error:i}=await T.from("users").update({credits:e.credits-t}).eq("id",e.id);if(i)throw i;const{data:a}=await T.from("cards").select("formation").eq("owner_id",e.id).eq("card_type","formation"),c=new Set((a||[]).map(o=>o.formation)),d=pn(),s=d[Math.floor(Math.random()*d.length)],l=c.has(s),{data:p,error:n}=await T.from("cards").insert({owner_id:e.id,card_type:"formation",formation:s}).select();return n&&console.error("[Booster Formation] Erreur insert:",n.message,n),(p||[]).map(o=>({...o,isDuplicate:l}))}function xn(e,t,i,a=null){var ie,W;if(!e||e.length===0){const G=document.createElement("div");G.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:3000;gap:16px;color:#fff;padding:24px;text-align:center",G.innerHTML=`
  ...
  `,document.body.appendChild(c);let d=!1;const s=document.getElementById("pack-cut-zone"),l=document.getElementById("pack-blade");let p=!1;const n=G=>G.touches&&G.touches[0]?G.touches[0].clientX:G.clientX;function o(G){d||(p=!0,l.style.opacity="1",f(G))}function f(G){if(!p||d)return;const se=s.getBoundingClientRect(),fe=n(G)-se.left,xe=Math.max(0,Math.min(1,fe/se.width));l.style.width=xe*se.width+"px",xe>=.82&&_()}function v(){d||(p=!1,l.style.transition="width .2s ease, opacity .2s ease",l.style.width="0",l.style.opacity="0",setTimeout(()=>{d||(l.style.transition="")},220))}function _(){var se;if(d)return;d=!0,p=!1,l.style.width="100%",l.style.opacity="1",(se=document.getElementById("cut-flash"))==null||se.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const G=document.getElementById("cut-hint");G&&(G.style.opacity="0"),s.classList.add("pack-cut"),setTimeout(()=>{l.style.opacity="0",document.getElementById("pack-phase").style.display="none",I(0)},620)}s.addEventListener("pointerdown",o),window.addEventListener("pointermove",f),window.addEventListener("pointerup",v),s.addEventListener("touchstart",o,{passive:!0}),window.addEventListener("touchmove",f,{passive:!0}),window.addEventListener("touchend",v);let A=0,r=!1;const u=new Set;function I(G){A=G,document.getElementById("reveal-phase").style.display="flex",z(),C(G,0),oe()}function z(){const G=document.getElementById("card-dots");G&&(G.innerHTML=e.map((se,fe)=>`<div class="card-dot" data-i="${fe}" style="width:8px;height:8px;border-radius:50%;background:${fe===A?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),G.querySelectorAll(".card-dot").forEach(se=>se.addEventListener("click",()=>B(parseInt(se.dataset.i)))))}function C(G,se){var R;const fe=e[G],xe=document.getElementById("card-counter"),Ee=document.getElementById("card-track");xe&&(xe.textContent=`Carte ${G+1} / ${e.length}`);const _e=document.getElementById("reveal-btns");_e&&(_e.style.display=G===e.length-1?"flex":"none");const he=fe.card_type==="player"&&((R=fe.player)==null?void 0:R.rarity)==="legende",Y=!u.has(G);u.add(G);let J=0;if(fe.card_type==="player"&&fe.player){const Q=fe.player,ge=Q.job||"ATT";J=Number(ge==="GK"?Q.note_g:ge==="DEF"?Q.note_d:ge==="MIL"?Q.note_m:Q.note_a)||0}const N=Q=>{Ee.innerHTML=`
        <div id="current-card-wrap" style="position:relative;display:flex;flex-direction:column;align-items:center;gap:8px;${he?"filter:drop-shadow(0 0 20px #7a28b8)":""}">
          <div style="transform:scale(1.25);transform-origin:center">${mo(fe)}</div>
      ...
      </div>`,document.body.appendChild(a),a.addEventListener("click",s=>{s.target===a&&a.remove()}),(i=document.getElementById("odds-close"))==null||i.addEventListener("click",()=>a.remove());return}yo()}function yo(){const e=document.createElement("div");e.style.cssText=`position:fixed;inset:0;background:rgba(0,0,0,0.6);display:flex;
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}const bo="simulation",yn="VOTRE_ZONE_ID";function ho(){switch(bo){case"propellerads":return vo();case"adsense":return wo();default:return ki()}}function ki(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:#111;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:14px";let i=5;t.innerHTML=`
      ...
      <button id="ad-skip" disabled style="margin-top:8px;padding:10px 24px;border-radius:20px;border:none;background:rgba(255,255,255,0.15);color:rgba(255,255,255,0.4);font-size:13px;cursor:default">
        Passer (5)
      </button>`,document.body.appendChild(t);const a=setInterval(()=>{i--;const c=document.getElementById("ad-cd"),d=document.getElementById("ad-skip");c&&(c.textContent=i),d&&(d.textContent=i>0?`Passer (${i})`:"✓ Continuer"),i<=0&&(clearInterval(a),d&&(d.disabled=!1,d.style.cssText=d.style.cssText.replace("rgba(255,255,255,0.15)","#1A6B3C").replace("rgba(255,255,255,0.4)","#fff").replace("default","pointer")),d==null||d.addEventListener("click",()=>{t.remove(),e(!0)},{once:!0}))},1e3)})}function vo(){return new Promise((e,t)=>{if(typeof window.propeller>"u")return console.warn("[Ad] PropellerAds non chargé → simulation"),e(ki());window.propeller.push({zone_id:yn,onComplete:()=>e(!0),onSkip:()=>t(new Error("Publicité ignorée")),onError:()=>t(new Error("Erreur publicité"))})})}function wo(){return new Promise((e,t)=>{if(typeof window.adsbygoogle>"u")return console.warn("[Ad] AdSense non chargé → simulation"),e(ki());try{(window.adsbygoogle=window.adsbygoogle||[]).push({google_ad_client:"ca-pub-VOTRE_PUBLISHER_ID",google_ad_slot:yn,google_ad_format:"rewarded",on_reward:()=>e(!0),on_dismiss:()=>t(new Error("Publicité fermée"))})}catch(i){t(i)}})}async function _o(e,{state:t,navigate:i,toast:a,refreshProfile:c}){var v,_;const{data:d}=await T.from("users").select("*").eq("id",t.user.id).single();d&&(t.profile=d);let s=Array.isArray((v=t.profile)==null?void 0:v.pending_boosters)?[...t.profile.pending_boosters]:[];if(!s.length){await T.from("users").update({onboarding_done:!0}).eq("id",t.user.id),i("home");return}let l=null;try{const r=(await cn()).find(u=>(u.name||"").toLowerCase().includes("new player"));r&&(l=un(r))}catch(A){console.warn('[Onboarding] Config "Booster (new player)" introuvable, fallback taux par défaut',A)}const p=s.length;let n=0;e.innerHTML=`
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
  ...
  </div>`;const o=async()=>{await T.from("users").update({pending_boosters:s}).eq("id",t.user.id)};async function f(){var z;if(n>=p||!s.length){await T.from("users").update({pending_boosters:[],onboarding_done:!0}).eq("id",t.user.id),c&&await c(),a("Tous tes boosters sont ouverts ! Bon jeu 🎮","success",4e3),i("home");return}const A=s[0],{data:r}=await T.from("users").select("*").eq("id",t.user.id).single();r&&(t.profile=r);let u=[];try{if(A.type==="formation")u=await mn(t.profile,0);else if(A.type==="game_changer")u=await gn(t.profile,A.count||3,0);else if(l&&((z=l.rates)!=null&&z.length)){const C={...l,cost:0,cardCount:A.count||l.cardCount||5};u=await pi(t.profile,C),A.guaranteeGK&&!t.profile.first_booster_opened&&(u.some(B=>B.player&&B.player.job==="GK")||await $o(t.profile,u),await T.from("users").update({first_booster_opened:!0}).eq("id",t.profile.id))}else u=await fn(t.profile,A.count||5,0)}catch(C){a(C.message||"Erreur ouverture booster","error");return}s.shift(),n++,await o();const I=A.type==="formation"?{name:"Booster Formation",img:"/manager-wars/icons/booster-formation.png"}:A.type==="game_changer"?{name:"Booster Game Changer",img:"/manager-wars/icons/booster-gamechanger.png"}:{name:`Booster Joueurs (${n}/${p})`,img:(l==null?void 0:l.img)||"/manager-wars/icons/booster-players.png"};xn(u,I,i,()=>{f()})}(_=document.getElementById("onboard-start"))==null||_.addEventListener("click",()=>f())}async function $o(e,t){try{const{data:i}=await T.from("players").select("id,job,firstname,surname_encoded,country_code,club_id,rarity,note_g,note_d,note_m,note_a,skin,hair,hair_length,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0).eq("job","GK");if(!(i!=null&&i.length))return;const a=i[Math.floor(Math.random()*i.length)],c=t.findIndex(s=>s.player);if(c===-1)return;const d=t[c];await T.from("cards").update({player_id:a.id}).eq("id",d.id),t[c]={...d,player_id:a.id,player:a}}catch(i){console.warn("[Onboarding] ensureGK échec",i)}}const Lt={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},St={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function wt(e,t,i,a,c){var d;e.innerHTML=`<div class="match-screen" style="display:flex;align-items:center;justify-content:center;min-height:100vh">
    <div style="text-align:center;padding:40px;color:#fff">
      <div style="font-size:48px;margin-bottom:16px">${t}</div>
      <p style="margin-bottom:16px">${i}</p>
      <button class="btn btn-primary" id="msg-btn">${a}</button>
    </div>
  </div>`,(d=document.getElementById("msg-btn"))==null||d.addEventListener("click",c)}function qe(e){const t=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function ui(e,t){var a,c;const i=e.player;return{cardId:e.id,position:t||null,id:i.id,firstname:i.firstname,name:i.surname_encoded,country_code:i.country_code,club_id:i.club_id,job:i.job,job2:i.job2,note_g:Number(i.note_g)||0,note_d:Number(i.note_d)||0,note_m:Number(i.note_m)||0,note_a:Number(i.note_a)||0,rarity:i.rarity,skin:i.skin,hair:i.hair,hair_length:i.hair_length,clubName:((a=i.clubs)==null?void 0:a.encoded_name)||null,clubLogo:((c=i.clubs)==null?void 0:c.logo_url)||null,boost:0,used:!1,_line:null,_col:null}}function lt(e){return e===1?[1]:e===2?[0,2]:e===3?[0,1,2]:e===4?[0,1,1,2]:e===5?[0,1,1,1,2]:[1]}function ii(){const e=Math.random()*100;return e<10?3:e<30?2:1}function yt(e,t){const i=Lt[t]||Lt["4-4-2"],a={GK:[],DEF:[],MIL:[],ATT:[]};if(e.length&&e.every(s=>s.position)){for(const s of["GK","DEF","MIL","ATT"]){const l=e.filter(n=>n.position&&n.position.replace(/\d+$/,"")===s).sort((n,o)=>parseInt(n.position.replace(/\D+/g,""),10)-parseInt(o.position.replace(/\D+/g,""),10)).map(n=>({...n,_line:s})),p=lt(l.length);l.forEach((n,o)=>{n._col=p[o]}),a[s]=l}return a}const d=[...e];for(const s of["GK","DEF","MIL","ATT"]){const l=[];for(let n=0;n<i[s];n++){let o=d.findIndex(f=>f.job===s);if(o===-1&&(o=d.findIndex(f=>f.job2===s)),o===-1&&(o=0),d[o]){const f={...d[o],_line:s};l.push(f),d.splice(o,1)}}const p=lt(l.length);l.forEach((n,o)=>{n._col=p[o]}),a[s]=l}return a}function et(e){document.querySelectorAll(".top-nav, .bottom-nav").forEach(t=>{t.style.setProperty("display","none","important"),t.dataset.matchHidden="1"}),e&&e.style.setProperty("padding-bottom","0","important")}function Ye(e){document.querySelectorAll(".top-nav, .bottom-nav").forEach(t=>{t.style.removeProperty("display"),delete t.dataset.matchHidden}),e&&e.style.removeProperty("padding-bottom")}function ni(e,t,i){const c=new Set,d=t.filter(o=>{const f=o.gc_type||o.id;return c.has(f)?!1:(c.add(f),!0)});let s=[];function l(o,f){const v=o._gcDef,_={purple:"linear-gradient(135deg,#3d0a7a,#7a28b8)",light_blue:"linear-gradient(135deg,#006080,#00bcd4)"},A={purple:"#9b59b6",light_blue:"#00bcd4"},r=_[v==null?void 0:v.color]||_.purple,u=A[v==null?void 0:v.color]||A.purple;return`<div class="gc-select-card" data-id="${o.id}"
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      <div style="flex:1;overflow-y:auto;display:flex;flex-wrap:wrap;gap:10px;justify-content:center;align-content:flex-start;padding:8px 16px 16px">
        ${d.map(A=>{const r=s.find(u=>u.gc_type===A.gc_type);return l(A,!!r)}).join("")}
      </div>
    ...
    </div>`,e.querySelectorAll(".gc-select-card").forEach(A=>{A.addEventListener("click",()=>{const r=A.dataset.id,u=d.find(z=>z.id===r);if(!u)return;const I=s.findIndex(z=>z.gc_type===u.gc_type);I>-1?s.splice(I,1):s.length<3&&s.push(u),n()})}),(f=e.querySelector("#gc-launch"))==null||f.addEventListener("click",()=>{o&&p(s)}),(v=e.querySelector("#gc-no-gc"))==null||v.addEventListener("click",()=>p([])),(_=e.querySelector("#gc-reset"))==null||_.addEventListener("click",()=>{s.length&&(s=[],n())})}n()}function ko(e){var a;const t=((a=e==null?void 0:e.state)==null?void 0:a.params)||{},i=t.matchMode||"vs_ai_easy";return i==="friend"?`Match vs ${t.friendName||"Ami"}`:i==="random"?"Match vs Random":`Match vs IA — ${i.replace("vs_ai_","").toUpperCase()}`}async function Eo(e,t,i){const{state:a,navigate:c}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:d}=await T.from("decks").select("id,name,is_active,formation").eq("owner_id",a.profile.id).order("created_at",{ascending:!1});if(!d||d.length===0){wt(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>c("decks"));return}const s=d.map(o=>o.id),{data:l}=await T.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",s).order("slot_order");let p=0;function n(){var z,C,H,B,j;const o=d[p],f=(l||[]).filter(U=>U.deck_id===o.id),v=f.filter(U=>{var oe;return U.is_starter&&((oe=U.card)==null?void 0:oe.player)}).map(U=>ui(U.card,U.position)),_=f.find(U=>{var oe;return((oe=U.card)==null?void 0:oe.card_type)==="formation"}),A=o.formation||((z=_==null?void 0:_.card)==null?void 0:z.formation)||"4-4-2",r=v.length>=11?yt(v,A):null,u=v.length>=11;et(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">
        ${d.map((U,oe)=>`<div style="width:7px;height:7px;border-radius:50%;background:${oe===p?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}
      </div>`:""}
    ...
    </div>`,function(){const oe=e.querySelector(".deck-preview-wrap svg");oe&&(oe.removeAttribute("width"),oe.removeAttribute("height"),oe.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",oe.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),(C=document.getElementById("prev-deck"))==null||C.addEventListener("click",()=>{p>0&&(p--,n())}),(H=document.getElementById("next-deck"))==null||H.addEventListener("click",()=>{p<d.length-1&&(p++,n())}),(B=document.getElementById("validate-deck"))==null||B.addEventListener("click",()=>{if(!u)return;const U=t.state.params||{};t.navigate("match",{...U,matchMode:U.matchMode||i,deckId:o.id})}),(j=document.getElementById("cancel-deck-select"))==null||j.addEventListener("click",()=>{Ye(e),c("home")});const I=document.getElementById("deck-swipe-zone");if(I){let U=0,oe=0;I.addEventListener("touchstart",ce=>{U=ce.touches[0].clientX,oe=ce.touches[0].clientY},{passive:!0}),I.addEventListener("touchend",ce=>{const re=ce.changedTouches[0].clientX-U,D=ce.changedTouches[0].clientY-oe;Math.abs(re)<40||Math.abs(re)<Math.abs(D)||(re<0&&p<d.length-1?(p++,n()):re>0&&p>0&&(p--,n()))},{passive:!0})}}n()}function bn(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";return e!=null&&e.clubLogo?e.clubLogo.startsWith("http")?e.clubLogo:`${t}/storage/v1/object/public/assets/clubs/${e.clubLogo}`:null}function Ke(e,t=44,i=58){if(!e)return`<div style="width:${t}px;height:${i}px;border:1.5px dashed rgba(255,255,255,0.2);border-radius:5px"></div>`;const a=typeof e.portrait=="string"&&e.portrait.startsWith("http")?e.portrait:qe(e),c=bn(e),d=e._line||e.job||"MIL",s=St[d]||"#555",l={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",p=e.note!==void 0?Number(e.note)||0:(Number($e(e,d))||0)+(e.boost||0),n=hn(e==null?void 0:e.country_code),o=Math.round(i*.19),f=Math.round(i*.25),v=i-Math.round(i*.19)-Math.round(i*.25),_=e!=null&&e.used?.28:1;return`<div style="width:${t}px;height:${i}px;border-radius:5px;border:2px solid ${l};background:${s};position:relative;overflow:hidden;flex-shrink:0;opacity:${_}">
    <div style="position:absolute;top:0;left:0;right:0;height:${o}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:center;z-index:2">
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;height:${f}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ...
      ${c?`<img src="${c}" style="width:${f-4}px;height:${f-4}px;object-fit:contain">`:e!=null&&e.clubName?`<span style="font-size:${Math.max(4,f-8)}px;font-weight:700;color:#333">${(e.clubName||"").slice(0,3).toUpperCase()}</span>`:""}
  ...
  </div>`}function nt(e,t,i){if(!(e!=null&&e.length))return"";const a=e.slice(0,5);let c='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return a.forEach((d,s)=>{if(c+=Ke(d,40,52),s<a.length-1){const l=at(d,a[s+1]);c+=`<div style="width:7px;height:3px;background:${l==="#ff3333"||l==="#cc2222"?"rgba(255,255,255,0.12)":l};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(c+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),c+="</div>",c}function Zt(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function hn(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function ut(e,t,i,a,c=310,d=310,s=[]){var C;const l=Pt[t]||{},p=Yi(t)||xi[t]||[],n={},o=["ATT","MIL","DEF","GK"];for(const H of o)(e[H]||[]).forEach((j,U)=>{n[`${H}${U+1}`]=j});function f(H){const B=l[H];return B?{x:B.x*c,y:B.y*d}:null}let v="";for(const[H,B]of p){const j=f(H),U=f(B);if(!j||!U)continue;const oe=n[H],ce=n[B],re=at(oe,ce);re==="#00ff88"||re==="#FFD700"?(v+=`<line x1="${j.x.toFixed(1)}" y1="${j.y.toFixed(1)}" x2="${U.x.toFixed(1)}" y2="${U.y.toFixed(1)}"
        ...
        stroke="${re}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const _=48,A=64,r=13,u=16,I={normal:"#aaaaaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#111111",legende:"#7a28b8",légende:"#7a28b8"};for(const[H,B]of Object.entries(n)){const j=f(H);if(!j||!B)continue;const U=H.replace(/[0-9]/g,""),oe=St[U]||"#555",ce=s.includes(B.cardId),re=i==="attack"&&(["MIL","ATT"].includes(U)||ce)&&!B.used||i==="defense"&&["GK","DEF","MIL"].includes(U)&&!B.used,D=a.includes(B.cardId);let ie;i==="attack"?ie=ce?Math.max(1,Number(B.note_a)||0):U==="MIL"?Number(B.note_m)||0:Number(B.note_a)||0:i==="defense"?ie=U==="GK"?Number(B.note_g)||0:U==="MIL"?Number(B.note_m)||0:Number(B.note_d)||0:ie=Number(U==="GK"?B.note_g:U==="DEF"?B.note_d:U==="MIL"?B.note_m:B.note_a)||0,ie=ie+(B.boost||0);const W=(j.x-_/2).toFixed(1),G=(j.y-A/2).toFixed(1),se=I[B==null?void 0:B.rarity]||I.normal;if(B.used){const J=typeof import.meta<"u"&&"/manager-wars/"||"/",R=`${typeof window<"u"&&((C=window.location)==null?void 0:C.origin)||""}${J}icons/carte-dos.png`.replace(/([^:])\/\//g,"$1/");v+=`<rect x="${W}" y="${G}" width="${_}" height="${A}" rx="5" fill="#161616"/>`,v+=`<image href="${R}" xlink:href="${R}" x="${W}" y="${G}" width="${_}" height="${A}" preserveAspectRatio="xMidYMid slice"/>`,v+=`<rect x="${W}" y="${G}" width="${_}" height="${A}" rx="5" fill="none" stroke="${se}" stroke-width="2" opacity="0.7"/>`,v+=`<rect x="${W}" y="${G}" width="${_}" height="${A}" rx="5" fill="rgba(0,0,0,0.01)" class="match-used-hit" data-card-id="${B.cardId}" data-role="${U}" style="cursor:pointer"/>`;continue}const fe=D?.45:1,xe=D?"#FFD700":se,Ee=D?3:(B==null?void 0:B.rarity)==="legende"||(B==null?void 0:B.rarity)==="légende"||(B==null?void 0:B.rarity)==="pepite"||(B==null?void 0:B.rarity)==="pépite"?2.5:2,_e=A-r-u;v+=`<defs><clipPath id="cp-${H}"><rect x="${W}" y="${(j.y-A/2+r).toFixed(1)}" width="${_}" height="${_e}"/></clipPath></defs>`,v+=`<rect x="${W}" y="${G}" width="${_}" height="${A}" rx="5" fill="${oe}" opacity="${fe}"/>`;const he=qe(B);he&&(v+=`<image href="${he}" xlink:href="${he}" x="${W}" y="${(j.y-A/2+r).toFixed(1)}" width="${_}" height="${_e}" clip-path="url(#cp-${H})" preserveAspectRatio="xMidYMin slice"/>`),v+=`<rect x="${W}" y="${G}" width="${_}" height="${r}" rx="4" fill="rgba(255,255,255,0.92)"/>`,v+=`<text x="${j.x.toFixed(1)}" y="${(j.y-A/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(B.name||"").slice(0,9)}</text>`;const Y=(j.y+A/2-u).toFixed(1);v+=`<rect x="${W}" y="${Y}" width="${_}" height="${u}" fill="rgba(255,255,255,0.92)"/>`;{const J=Zt(B.country_code);J?v+=`<image href="${J}" xlink:href="${J}" x="${(j.x-20).toFixed(1)}" y="${(j.y+A/2-u+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:v+=`<text x="${(j.x-13).toFixed(1)}" y="${(j.y+A/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${hn(B.country_code)}</text>`,v+=`<text x="${j.x.toFixed(1)}" y="${(j.y+A/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${ie}</text>`;const N=bn(B);N?v+=`<image href="${N}" xlink:href="${N}" x="${(j.x+_/2-14).toFixed(1)}" y="${(j.y+A/2-u+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:B.clubName&&(v+=`<text x="${(j.x+14).toFixed(1)}" y="${(j.y+A/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(B.clubName||"").slice(0,3).toUpperCase()}</text>`),B.boost&&(v+=`<rect x="${(j.x+_/2-12).toFixed(1)}" y="${(j.y-A/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,v+=`<text x="${(j.x+_/2-5).toFixed(1)}" y="${(j.y-A/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${B.boost}</text>`)}v+=`<rect x="${W}" y="${G}" width="${_}" height="${A}" rx="5" fill="${D?"rgba(255,255,255,0.12)":"none"}" stroke="${xe}" stroke-width="${Ee}" opacity="${fe}"/>`,re&&(v+=`<rect x="${W}" y="${G}" width="${_}" height="${A}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${D?"selected":""}" data-card-id="${B.cardId}" data-role="${U}" style="cursor:pointer"/>`)}const z=38;return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-z} ${-z} ${c+z*2} ${d+z*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${v}
  </svg>`}function ft(e,t,i,a,c=300,d=300,s=[]){return`<div id="match-terrain-wrap" style="position:relative;padding:0 4px">
    ${ut(e,t,i,a,c,d,s)}
  ...
  </div>`}async function oi(e,t,i,a){var B;const{state:c,navigate:d,toast:s}=t;et(e);const l=c.params||{};if(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!l.deckId)return Eo(e,t,i);const p=l.deckId;let n,o,f,v;try{const j=await Promise.all([T.from("decks").select("formation,name").eq("id",p).single(),T.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation,
            player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
              clubs(encoded_name,logo_url)))`).eq("deck_id",p).order("slot_order")]);n=j[0].data,f=j[0].error,o=j[1].data,v=j[1].error}catch(j){console.error("[Match] Exception chargement deck:",j),wt(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>d("home"));return}if(f||v){console.error("[Match] Erreur Supabase:",f||v),wt(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>d("home"));return}const _=(o||[]).filter(j=>{var U;return j.is_starter&&((U=j.card)==null?void 0:U.player)}).map(j=>ui(j.card,j.position)),A=(o||[]).filter(j=>{var U;return!j.is_starter&&((U=j.card)==null?void 0:U.player)}).map(j=>ui(j.card,j.position));if(_.length<11){wt(e,"⚠️",`Deck incomplet (${_.length}/11).`,"Compléter",()=>d("decks"));return}const r=(o||[]).find(j=>{var U;return((U=j.card)==null?void 0:U.card_type)==="formation"}),u=(n==null?void 0:n.formation)||((B=r==null?void 0:r.card)==null?void 0:B.formation)||"4-4-2",{data:I,error:z}=await T.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",c.profile.id).eq("card_type","game_changer"),{data:C}=await T.from("gc_definitions").select("*").eq("is_active",!0),H=(I||[]).map(j=>({...j,_gcDef:(C==null?void 0:C.find(U=>U.name===j.gc_type||U.id===j.gc_definition_id))||null}));a({deckId:p,formation:u,starters:_,subsRaw:A,gcCardsEnriched:H,gcDefs:C||[]})}async function To(e,t){const{state:i}=t,c=(i.params||{}).matchMode||"vs_ai_easy",d=c.replace("vs_ai_",""),s=c;await oi(e,t,c,async({deckId:l,formation:p,starters:n,subsRaw:o,gcCardsEnriched:f,gcDefs:v})=>{try{const _=yt(n,p),A=await Ao(p,d),r=async u=>{try{const{data:I,error:z}=await T.from("matches").insert({home_id:i.profile.id,away_id:null,mode:s,home_deck_id:l,status:"in_progress"}).select().single();if(z){console.error("[MatchIA] Erreur création match:",z),wt(e,"⚠️","Impossible de créer le match ("+z.message+").","Retour",()=>t.navigate("home"));return}const C={gcDefs:v||[],matchId:I==null?void 0:I.id,mode:s,difficulty:d,formation:p,homeTeam:_,aiTeam:A,homeSubs:o,subsUsed:0,maxSubs:Math.min(o.length,3),homeScore:0,aiScore:0,gcCards:u,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Lo(e,C,t)}catch(I){console.error("[MatchIA] Exception launchMatch:",I),wt(e,"⚠️","Erreur au lancement du match : "+I.message,"Retour",()=>t.navigate("home"))}};if(!f.length){r([]);return}ni(e,f,r)}catch(_){console.error("[MatchIA] Exception setup:",_),wt(e,"⚠️","Erreur de préparation du match : "+_.message,"Retour",()=>t.navigate("home"))}})}async function Ao(e,t){var s,l;const{data:i}=await T.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(60);if(!i||i.length<11)return zo(e);const a=Lt[e]||Lt["4-4-2"],c={GK:[],DEF:[],MIL:[],ATT:[]},d=[...i];for(const p of["GK","DEF","MIL","ATT"]){const n=d.filter(A=>A.job===p),o=d.filter(A=>A.job!==p),f=[...n,...o],v=[];for(let A=0;A<a[p];A++){const r=f[A]||d[A];r&&v.push({cardId:"ai-"+r.id+"-"+A,id:r.id,firstname:r.firstname,name:r.surname_encoded,country_code:r.country_code,club_id:r.club_id,job:r.job,job2:r.job2,note_g:Number(r.note_g)||0,note_d:Number(r.note_d)||0,note_m:Number(r.note_m)||0,note_a:Number(r.note_a)||0,rarity:r.rarity,skin:r.skin,hair:r.hair,hair_length:r.hair_length,clubName:((s=r.clubs)==null?void 0:s.encoded_name)||null,clubLogo:((l=r.clubs)==null?void 0:l.logo_url)||null,boost:0,used:!1,_line:p})}const _=lt(v.length);v.forEach((A,r)=>{A._col=_[r]}),c[p]=v}return c}function zo(e){const t=Lt[e]||Lt["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},a=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let c=0;for(const d of["GK","DEF","MIL","ATT"]){const s=[];for(let p=0;p<t[d];p++){const n=3+Math.floor(Math.random()*5);s.push({cardId:"fake-"+c,id:"fake-"+c,firstname:"IA",name:a[c%a.length],country_code:"XX",club_id:null,job:d,job2:null,note_g:d==="GK"?n:2,note_d:d==="DEF"?n:2,note_m:d==="MIL"?n:2,note_a:d==="ATT"?n:2,rarity:"normal",boost:0,used:!1,_line:d}),c++}const l=lt(s.length);s.forEach((p,n)=>{p._col=l[n]}),i[d]=s}return i}function Lo(e,t,i){e.innerHTML=`
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
      ...
      <div class="duel-score-line duel-score-line-${z}" style="margin-top:10px;font-size:12px;color:rgba(255,255,255,0.55);opacity:0;transition:opacity .4s ease">
        Score: ${d(r)} + ${s(r)} liens = <b style="color:#fff">${d(r)+s(r)}</b>
      </div>
    </div>`}e.innerHTML=`
  <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div id="duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center, rgba(10,61,30,0.4), rgba(10,61,30,0.92))"></div>
          ...
          <button id="start-match-btn" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,0.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">
            ▶ Commencer le match
          </button>`,B.style.transition="opacity .45s ease",B.style.opacity="1",B.style.pointerEvents="auto",(j=document.getElementById("start-match-btn"))==null||j.addEventListener("click",A))},600)},700)},2800)}function Io(e){if(e.type==="duel"&&(e.homeTotal!=null||e.aiTotal!=null)){const t=(e.homeTotal||0)>=(e.aiTotal||0);return`
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const ie=t.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
          ...
          </div>`}const D=t.log[t.log.length-1];return D?'<div style="padding:2px 4px">'+Io(D)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})()}
    </div>
    ...
    ${(()=>{const D=window.innerWidth>=700,ie=(Y,J,N)=>{var Re,Ze;const R=(t.gcDefs||[]).find(M=>M.name===Y.gc_type),Q={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[R==null?void 0:R.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",ge={purple:"#b06ce0",light_blue:"#00d4ef"}[R==null?void 0:R.color]||"#b06ce0",pe=(R==null?void 0:R.name)||Y.gc_type,Me=(R==null?void 0:R.effect)||((Re=Ue[Y.gc_type])==null?void 0:Re.desc)||"",Ce=R!=null&&R.image_url?`/manager-wars/icons/${R.image_url}`:null,Be=((Ze=Ue[Y.gc_type])==null?void 0:Ze.icon)||"⚡",Ve=Math.round(N*.22),Se=Math.round(N*.22),Ne=N-Ve-Se,Ie=pe.length>12?7:9;return`<div class="gc-mini" data-gc-id="${Y.id}" data-gc-type="${Y.gc_type}"
        ...
        ${d.length===0?'<div style="font-size:7px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':d.map(Y=>`<div class="sub-btn-col" data-sub-id="${Y.cardId}" style="cursor:pointer;flex-shrink:0">${Ke(Y,76,100)}</div>`).join("")}
  ...
  </div>`;function I(){const D=e.querySelector(".match-screen");if(!D)return;const ie=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);D.style.bottom="auto",D.style.height=ie+"px",D.style.minHeight=ie+"px",D.style.maxHeight=ie+"px",D.style.overflow="hidden";const W=e.querySelector("#mobile-action-bar"),G=e.querySelector("#mobile-play-area");W&&G&&(G.style.paddingBottom=W.offsetHeight+"px")}if(I(),setTimeout(I,120),setTimeout(I,400),setTimeout(I,1e3),t._vvBound||(t._vvBound=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",I),window.visualViewport.addEventListener("scroll",I)),window.addEventListener("resize",I)),function(){const ie=e.querySelector(".terrain-wrapper svg");ie&&(ie.removeAttribute("width"),ie.removeAttribute("height"),ie.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",ie.setAttribute("viewBox","-26 -26 352 352"),ie.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),t._resizeBound||(t._resizeBound=!0,window.addEventListener("resize",()=>{const D=e.querySelector(".terrain-wrapper svg");D&&(D.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0")})),t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase==="attack"||t.phase==="defense"){let D=!1,ie=30;const W=()=>document.getElementById("match-timer"),G=()=>{const se=W();if(!se)return;const fe=String(Math.floor(ie/60)).padStart(2,"0"),xe=String(ie%60).padStart(2,"0");se.textContent=` ${fe}:${xe}`,se.style.color=D?"#ff2222":"#ff9500",se.style.fontWeight="900"};G(),t._timerInt=setInterval(()=>{if(ie--,ie<0)if(!D)D=!0,ie=15,G();else{clearInterval(t._timerInt),t._timerInt=null,t.homeScore=0,t.aiScore=3;const se=document.createElement("div");se.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;color:#fff;padding:24px;text-align:center",se.innerHTML='<div style="font-size:56px">⏱️</div><div style="font-size:24px;font-weight:900;color:#ff4444">MATCH PERDU PAR FORFAIT</div><div style="font-size:14px;color:rgba(255,255,255,0.6)">Temps écoulé</div>',document.body.appendChild(se),setTimeout(()=>{se.remove(),qt(e,t,i)},2500)}else G()},1e3)}(C=document.getElementById("match-quit"))==null||C.addEventListener("click",()=>{Ye(e),confirm("Abandonner ? Résultat : défaite 3-0")&&(t.homeScore=0,t.aiScore=3,qt(e,t,i))}),(H=document.getElementById("view-ai"))==null||H.addEventListener("click",()=>No(t,i)),(B=document.getElementById("toggle-history"))==null||B.addEventListener("click",()=>{var D;(D=document.getElementById("match-history-panel"))==null||D.classList.add("open")}),(j=document.getElementById("close-history"))==null||j.addEventListener("click",()=>{var D;(D=document.getElementById("match-history-panel"))==null||D.classList.remove("open")}),(U=document.getElementById("btn-action"))==null||U.addEventListener("click",()=>{t.selected.length!==0&&(v?Co(e,t,i):_&&jo(e,t,i))}),(oe=document.getElementById("btn-results"))==null||oe.addEventListener("click",()=>qt(e,t,i)),e.querySelectorAll(".match-slot-hit").forEach(D=>{D.addEventListener("click",()=>Mo(D,t,e,i))}),e.querySelectorAll(".match-used-hit").forEach(D=>{D.addEventListener("click",()=>si(e,t,i,null,D.dataset.cardId))}),e.querySelectorAll(".gc-mini").forEach(D=>{D.addEventListener("click",()=>Bo(D.dataset.gcId,D.dataset.gcType,e,t,i))}),(ce=document.getElementById("boost-card"))==null||ce.addEventListener("click",()=>Po(e,t,i)),e.querySelectorAll(".sub-btn-col").forEach(D=>{D.addEventListener("click",()=>si(e,t,i,D.dataset.subId))}),(re=document.getElementById("sub-btn-main"))==null||re.addEventListener("click",()=>si(e,t,i))}function Mo(e,t,i,a){const c=e.dataset.cardId,d=e.dataset.role,s=t.selected.findIndex(l=>l.cardId===c);if(s!==-1)t.selected.splice(s,1);else{if(t.selected.length>=3){a.toast("Maximum 3 joueurs","error");return}const l=(t.homeTeam[d]||[]).find(p=>p.cardId===c);l&&t.selected.push({...l,_role:d,_line:d})}Je(i,t,a)}function Ei(e,t,i){e.matchId&&T.from("matches").update({last_player_id:i}).eq("id",e.matchId).then(()=>{})}function Co(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),Ei(t,i,i.state.profile.id);const a=!["GK","DEF","MIL","ATT"].some(s=>(t.aiTeam[s]||[]).some(l=>!l.used)),c=t.selected.map(s=>{const l=(t.homeTeam[s._role]||[]).find(n=>n.cardId===s.cardId)||s,p=a&&["GK","DEF"].includes(s._role);return{...l,_line:s._role,...p?{note_a:Math.max(1,Number(l.note_a)||0)}:{}}}),d=Nt(c,t.modifiers.home);t.pendingAttack={...d,players:[...c],side:"home"},t.selected.forEach(s=>{const l=(t.homeTeam[s._role]||[]).find(p=>p.cardId===s.cardId);l&&(l.used=!0)}),t.log.push({text:`⚔️ Vous attaquez : ${d.total} (base ${d.base}${d.links?` +${d.links} liens`:""}) — ${t.selected.map(s=>s.name).join(", ")}`,type:"info"}),t.selected=[],t.modifiers.home={},t.phase="ai-defense",Je(e,t,i),setTimeout(()=>qo(e,t,i),1200)}function jo(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),Ei(t,i,i.state.profile.id);const a=t.selected.map(l=>({...(t.homeTeam[l._role]||[]).find(n=>n.cardId===l.cardId)||l,_line:l._role})),c=Rt(a,t.modifiers.home);t.selected.forEach(l=>{const p=(t.homeTeam[l._role]||[]).find(n=>n.cardId===l.cardId);p&&(p.used=!0)});const d=Ot(t.pendingAttack.total,c.total,t.modifiers.home),s={type:"duel",title:"Défense",aiPlayers:(t.pendingAttack.players||[]).map(l=>({name:l.name,note:l._line==="MIL"?l.note_m:l.note_a,portrait:qe(l),job:l.job,country_code:l.country_code,rarity:l.rarity,clubName:l.clubName,clubLogo:l.clubLogo})),homePlayers:t.selected.map(l=>{const p=(t.homeTeam[l._role]||[]).find(n=>n.cardId===l.cardId)||l;return{name:p.name,note:(p._line==="GK"?Number(p.note_g)||0:p._line==="MIL"?Number(p.note_m)||0:Number(p.note_d)||0)+(p.boost||0),portrait:qe(p),job:p.job,country_code:p.country_code,rarity:p.rarity,clubName:p.clubName,clubLogo:p.clubLogo}}),homeTotal:c.total,aiTotal:t.pendingAttack.total,isGoal:!1,homeScored:!1,text:""};if(d.shielded)s.text="🛡️ Bouclier ! But annulé.",t.log.push(s);else if(d.goal){t.aiScore++,s.isGoal=!0,s.homeScored=!1,s.text=`⚽ BUT IA ! (${t.pendingAttack.total} > ${c.total})`,t.log.push(s),t.selected=[],t.modifiers.home={},t.pendingAttack=null,Je(e,t,i),Qt(s.aiPlayers,t.homeScore,t.aiScore,!1,()=>{Tt(e,t,i,"home-attack")});return}else s.text=`🧤 Défense réussie ! (${c.total} ≥ ${t.pendingAttack.total})`,t.log.push(s);t.selected=[],t.modifiers.home={},t.pendingAttack=null,Tt(e,t,i,"home-attack")}function fi(e,t,i){Ei(t,i,null);const a=[...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]],c=tn(a,"attack",t.difficulty);if(!c.length){Jt(e,t,i);return}const d=Nt(c,t.modifiers.ai);t.pendingAttack={...d,players:c,side:"ai"},c.forEach(n=>{n.used=!0}),t.log.push({text:`🤖 IA attaque : ${d.total} (${c.map(n=>n.name).join(", ")})`,type:"info"}),t.modifiers.ai={};const s=[...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(n=>!n.used),p=(t.homeSubs||[]).filter(n=>!(t.usedSubIds||[]).includes(n.cardId)).length>0&&t.subsUsed<t.maxSubs;if(s.length===0&&!p){t.aiScore++;const n={type:"duel",isGoal:!0,homeScored:!1,aiPlayers:c.map(o=>({name:o.name,note:o._line==="MIL"?o.note_m:o.note_a,portrait:qe(o),job:o.job,country_code:o.country_code,rarity:o.rarity,clubName:o.clubName,clubLogo:o.clubLogo})),aiTotal:d.total,text:"⚽ BUT IA ! (aucun défenseur disponible)"};t.log.push(n),t.pendingAttack=null,Je(e,t,i),Qt(n.aiPlayers,t.homeScore,t.aiScore,!1,()=>{Tt(e,t,i,"home-attack")});return}t.phase="defense",Je(e,t,i)}function qo(e,t,i){var n,o;const a=[...t.aiTeam.GK||[],...t.aiTeam.DEF||[],...t.aiTeam.MIL||[]],c=tn(a,"defense",t.difficulty);if(!["GK","DEF","MIL","ATT"].flatMap(f=>(t.aiTeam[f]||[]).filter(v=>!v.used)).length){t.homeScore++;const v={type:"duel",isGoal:!0,homeScored:!0,homePlayers:(((n=t.pendingAttack)==null?void 0:n.players)||[]).map(_=>({name:_.name,note:$e(_,_._line||_.job),portrait:qe(_),job:_.job,country_code:_.country_code,rarity:_.rarity,clubName:_.clubName,clubLogo:_.clubLogo})),homeTotal:((o=t.pendingAttack)==null?void 0:o.total)||0,aiTotal:0,text:"⚽ BUT ! L'IA n'a plus de joueurs — but automatique !"};t.log.push(v),t.modifiers.ai={},t.pendingAttack=null,Je(e,t,i),Qt(v.homePlayers,t.homeScore,t.aiScore,!0,()=>{Tt(e,t,i,"ai-attack")});return}const s=c.length>0?Rt(c,t.modifiers.ai).total:0;c.forEach(f=>{f.used=!0});const l=Ot(t.pendingAttack.total,s,t.modifiers.ai),p={type:"duel",title:"Attaque",homePlayers:(t.pendingAttack.players||[]).map(f=>({name:f.name,note:f._line==="MIL"?f.note_m:f.note_a,portrait:qe(f),job:f.job,country_code:f.country_code,rarity:f.rarity,clubName:f.clubName,clubLogo:f.clubLogo})),aiPlayers:c.map(f=>({name:f.name,note:f._line==="GK"?f.note_g:f._line==="MIL"?f.note_m:f.note_d,portrait:qe(f),job:f.job,country_code:f.country_code,rarity:f.rarity,clubName:f.clubName,clubLogo:f.clubLogo})),homeTotal:t.pendingAttack.total,aiTotal:s,isGoal:!1,homeScored:!1,text:""};if(l.shielded)p.text="🛡️ Bouclier IA !",t.log.push(p);else if(l.goal){t.homeScore++,p.isGoal=!0,p.homeScored=!0,p.text=`⚽ BUT ! (${t.pendingAttack.total} > ${s})`,t.log.push(p),t.modifiers.ai={},t.pendingAttack=null,Je(e,t,i),Qt(p.homePlayers,t.homeScore,t.aiScore,!0,()=>{Tt(e,t,i,"ai-attack")});return}else p.text=`🧤 IA défend (${s} ≥ ${t.pendingAttack.total})`,t.log.push(p);t.modifiers.ai={},t.pendingAttack=null,Tt(e,t,i,"ai-attack")}function Tt(e,t,i,a){if(t.round++,vn(t)){qt(e,t,i);return}if(a==="home-attack"){if(![...t.homeTeam.MIL||[],...t.homeTeam.ATT||[]].filter(d=>!d.used).length){if(![...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(l=>!l.used).length){Jt(e,t,i);return}if(![...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]].filter(l=>!l.used).length){Jt(e,t,i);return}setTimeout(()=>fi(e,t,i),100);return}t.phase="attack",Je(e,t,i)}else{if(![...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]].filter(d=>!d.used).length){Jt(e,t,i);return}t.phase="ai-attack",Je(e,t,i),setTimeout(()=>fi(e,t,i),800)}}function vn(e){const t=["MIL","ATT","GK","DEF"].some(a=>(e.homeTeam[a]||[]).some(c=>!c.used)),i=["MIL","ATT","GK","DEF"].some(a=>(e.aiTeam[a]||[]).some(c=>!c.used));return!t&&!i}function Jt(e,t,i){vn(t)?qt(e,t,i):(t.phase="attack",Je(e,t,i))}function Fo(e,t,i){const a=document.createElement("div");a.style.cssText=`
    position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:800;
    display:flex;flex-direction:column;align-items:center;justify-content:center;gap:20px;
    animation:subFadeIn 0.2s ease;
  `;const c=qe(e),d=qe(t),s=St[e.job]||"#555",l=St[t.job]||"#555",p=e.job==="GK"?e.note_g:e.job==="DEF"?e.note_d:e.job==="MIL"?e.note_m:e.note_a,n=t.job==="GK"?t.note_g:t.job==="DEF"?t.note_d:t.job==="MIL"?t.note_m:t.note_a;a.innerHTML=`
    <style>
  ...
  `,document.body.appendChild(a);let o=!1;const f=()=>{o||(o=!0,a.remove(),i())};a.addEventListener("click",f),setTimeout(f,2e3)}function It(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function si(e,t,i,a=null,c=null){var _,A;if(t.phase!=="attack"){It("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){It(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const d=Object.entries(t.homeTeam).flatMap(([r,u])=>(u||[]).filter(I=>I.used).map(I=>({...I,_line:I._line||r}))),s=t.homeSubs.filter(r=>!t.usedSubIds.includes(r.cardId));if(!d.length){It("Aucun joueur utilisé à remplacer");return}if(!s.length){It("Aucun remplaçant disponible");return}let l=Math.max(0,d.findIndex(r=>r.cardId===c));const p=((_=d[l])==null?void 0:_._line)||((A=d[l])==null?void 0:A.job);let n=a?Math.max(0,s.findIndex(r=>r.cardId===a)):Math.max(0,s.findIndex(r=>r.job===p)),o=!1;const f=document.createElement("div");f.id="sub-overlay",f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function v(){var B,j,U,oe,ce,re;const r=d[l],u=s[n],I=Math.min(130,Math.round((window.innerWidth-90)/2)),z=Math.round(I*1.35),C=D=>`background:rgba(255,255,255,0.12);border:none;color:${D?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${D?"default":"pointer"};flex-shrink:0`;f.innerHTML=`
    ...
    </div>`,(B=f.querySelector("#sub-close"))==null||B.addEventListener("click",()=>f.remove()),(j=f.querySelector("#out-up"))==null||j.addEventListener("click",()=>{l>0&&(l--,v())}),(U=f.querySelector("#out-down"))==null||U.addEventListener("click",()=>{l<d.length-1&&(l++,v())}),(oe=f.querySelector("#in-up"))==null||oe.addEventListener("click",()=>{n>0&&(n--,v())}),(ce=f.querySelector("#in-down"))==null||ce.addEventListener("click",()=>{n<s.length-1&&(n++,v())});const H=(D,ie,W,G)=>{const se=f.querySelector("#"+D);if(!se)return;let fe=0;se.addEventListener("touchstart",xe=>{fe=xe.touches[0].clientY},{passive:!0}),se.addEventListener("touchend",xe=>{const Ee=xe.changedTouches[0].clientY-fe;if(Math.abs(Ee)<30)return;const _e=ie();Ee<0&&_e<G-1?(W(_e+1),v()):Ee>0&&_e>0&&(W(_e-1),v())},{passive:!0})};H("in-panel",()=>n,D=>n=D,s.length),H("out-panel",()=>l,D=>l=D,d.length),(re=f.querySelector("#sub-confirm"))==null||re.addEventListener("click",D=>{if(D.preventDefault(),D.stopPropagation(),o)return;o=!0;const ie=d[l],W=s[n];if(!ie||!W)return;let G=null,se=-1;for(const[xe,Ee]of Object.entries(t.homeTeam)){const _e=(Ee||[]).findIndex(he=>he.cardId===ie.cardId);if(_e!==-1){G=xe,se=_e;break}}if(se===-1||!G){It("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),f.remove();return}const fe={...W,_line:G,_col:ie._col||0,used:!1,boost:0};t.homeTeam[G].splice(se,1,fe),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(W.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:ie.name,firstname:ie.firstname,note:$e(ie,G),portrait:qe(ie),job:ie.job,country_code:ie.country_code,rarity:ie.rarity,clubName:ie.clubName,clubLogo:ie.clubLogo},inPlayer:{name:W.name,firstname:W.firstname,note:$e(W,G),portrait:qe(W),job:W.job,country_code:W.country_code,rarity:W.rarity,clubName:W.clubName,clubLogo:W.clubLogo},text:`🔄 ${W.firstname} ${W.name} remplace ${ie.firstname} ${ie.name}`}),f.remove(),Fo(ie,W,()=>Je(e,t,i))})}document.body.appendChild(f),v()}function Bo(e,t,i,a,c){var A,r;const d=(a.gcDefs||[]).find(u=>u.name===t),s=Ue[t]||{icon:"⚡",desc:"Carte spéciale."},l={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[d==null?void 0:d.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",p={purple:"#b06ce0",light_blue:"#00d4ef"}[d==null?void 0:d.color]||"#b06ce0",n=(d==null?void 0:d.name)||t,o=(d==null?void 0:d.effect)||s.desc,f=d!=null&&d.image_url?`/manager-wars/icons/${d.image_url}`:null,v=s.icon||"⚡",_=document.createElement("div");_.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",_.innerHTML=`
    ...
    </div>`,document.body.appendChild(_),(A=_.querySelector("#gc-back"))==null||A.addEventListener("click",()=>_.remove()),(r=_.querySelector("#gc-use"))==null||r.addEventListener("click",()=>{_.remove(),Go(e,t,i,a,c)})}function Kt(e,t,i,a,c,d){const s=document.createElement("div");s.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let l=[];function p(){var n,o;s.innerHTML=`
    ...
    <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
      ${e.map(f=>{const v=f._line||f.job||"MIL",_={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[v]||"#555",A=$e(f,v)+(f.boost||0),r=l.find(u=>u.cardId===f.cardId);return`<div class="gc-pick-item" data-cid="${f.cardId}"
    ...
    </div>`,(n=s.querySelector("#gc-picker-close"))==null||n.addEventListener("click",()=>s.remove()),s.querySelectorAll(".gc-pick-item").forEach(f=>{f.addEventListener("click",()=>{const v=f.dataset.cid,_=e.find(r=>r.cardId===v);if(!_)return;const A=l.findIndex(r=>r.cardId===v);A>-1?l.splice(A,1):l.length<t&&l.push(_),p()})}),(o=s.querySelector("#gc-picker-confirm"))==null||o.addEventListener("click",()=>{s.remove(),d(l)})}p(),document.body.appendChild(s)}const Do={BOOST_STAT:({value:e=1,count:t=1,roles:i=[]},a,c,d)=>{const s=Object.entries(a.homeTeam).filter(([l])=>!i.length||i.includes(l)).flatMap(([l,p])=>p.filter(n=>!n.used).map(n=>({...n,_line:l})));return s.length?(Kt(s,t,`Choisir ${t} joueur(s) à booster (+${e})`,c,a,l=>{l.forEach(p=>{const n=(a.homeTeam[p._line]||[]).find(o=>o.cardId===p.cardId);n&&(n.boost=(n.boost||0)+e,a.log.push({text:`⚡ +${e} sur ${n.name}`,type:"info"}))}),Je(c,a,d)}),!0):(a.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),Je(c,a,d),!0)},DEBUFF_STAT:({value:e=1,count:t=1,roles:i=[],target:a="ai"},c,d,s)=>{const l=a==="home"?c.homeTeam:c.aiTeam,p=a==="ai"?"adverse":"allié",n=Object.entries(l).filter(([o])=>!i.length||i.includes(o)).flatMap(([o,f])=>f.filter(v=>!v.used).map(v=>({...v,_line:o})));return n.length?(Kt(n,t,`Choisir ${t} joueur(s) ${p}(s) à débuffer (-${e})`,d,c,o=>{o.forEach(f=>{const _=((a==="home"?c.homeTeam:c.aiTeam)[f._line]||[]).find(A=>A.cardId===f.cardId);_&&(_.boost=(_.boost||0)-e,c.log.push({text:`🎯 -${e} sur ${_.name}${a==="ai"?" (IA)":""}`,type:"info"}))}),Je(d,c,s)}),!0):(c.log.push({text:`🎯 Aucun joueur ${p} disponible`,type:"info"}),Je(d,c,s),!0)},GRAY_PLAYER:({count:e=1,roles:t=[],target:i="ai"},a,c,d)=>{const s=i==="home"?a.homeTeam:a.aiTeam,l=i==="ai"?"adverse":"allié",p=Object.entries(s).filter(([n])=>!t.length||t.includes(n)).flatMap(([n,o])=>o.filter(f=>!f.used).map(f=>({...f,_line:n})));return p.length?(Kt(p,e,`Choisir ${e} joueur(s) ${l}(s) à exclure`,c,a,n=>{n.forEach(o=>{const v=((i==="home"?a.homeTeam:a.aiTeam)[o._line]||[]).find(_=>_.cardId===o.cardId);v&&(v.used=!0,a.log.push({text:`❌ ${v.name}${i==="ai"?" (IA)":""} exclu !`,type:"info"}))}),Je(c,a,d)}),!0):(a.log.push({text:`❌ Aucun joueur ${l} à exclure`,type:"info"}),Je(c,a,d),!0)},REVIVE_PLAYER:({count:e=1,roles:t=[]},i,a,c)=>{const d=Object.entries(i.homeTeam).filter(([s])=>!t.length||t.includes(s)).flatMap(([s,l])=>l.filter(p=>p.used).map(p=>({...p,_line:s})));return d.length?(Kt(d,e,`Choisir ${e} joueur(s) à ressusciter`,a,i,s=>{s.forEach(l=>{const p=(i.homeTeam[l._line]||[]).find(n=>n.cardId===l.cardId);p&&(p.used=!1,i.log.push({text:`💫 ${p.name} ressuscité !`,type:"info"}))}),Je(a,i,c)}),!0):(i.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),Je(a,i,c),!0)},REMOVE_GOAL:({},e)=>(e.aiScore>0?(e.aiScore--,e.log.push({text:"🚫 Dernier but IA annulé !",type:"info"})):e.log.push({text:"🚫 Aucun but à annuler",type:"info"}),!1),ADD_GOAL_DRAW:({},e)=>(e.homeScore===e.aiScore?(e.homeScore++,e.log.push({text:"🎯 But bonus (match nul) !",type:"info"})):e.log.push({text:"🎯 But bonus : non applicable (pas de match nul)",type:"info"}),!1),ADD_SUB:({value:e=1},t)=>(t.maxSubs=(t.maxSubs||3)+e,t.log.push({text:`🔄 +${e} remplacement(s) débloqué(s)`,type:"info"}),!1),CUSTOM:()=>!1};function Go(e,t,i,a,c){a.usedGc.push(e);const d=a.gcDefs||[],s=d.find(p=>p.name===t)||d.find(p=>{var n;return((n=p.name)==null?void 0:n.toLowerCase().trim())===(t==null?void 0:t.toLowerCase().trim())});let l=!1;if(s!=null&&s.effect_type&&s.effect_type!=="CUSTOM"){const p=Do[s.effect_type];p?p(s.effect_params||{},a,i,c)||(l=!0):(c.toast(`Effet "${s.effect_type}" non implémenté`,"error"),l=!0)}else{switch(t){case"Double attaque":a.modifiers.home.doubleAttack=!0,a.log.push({text:"⚡ Double attaque activée !",type:"info"});break;case"Bouclier":a.modifiers.home.shield=!0,a.log.push({text:"🛡️ Bouclier activé !",type:"info"});break;case"Ressusciter":{const p=Object.entries(a.homeTeam).flatMap(([n,o])=>(o||[]).filter(f=>f.used).map(f=>({...f,_line:n})));p.length?(p[0].used=!1,a.log.push({text:`💫 ${p[0].name} ressuscité !`,type:"info"})):a.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"});break}case"Vol de note":a.modifiers.ai.stolenNote=(a.modifiers.ai.stolenNote||0)+1,a.log.push({text:"🎯 -1 à la prochaine attaque IA",type:"info"});break;case"Gel":{const p=[...a.aiTeam.ATT||[],...a.aiTeam.MIL||[]].filter(n=>!n.used);if(p.length){const n=p.sort((o,f)=>$e(f,"ATT")-$e(o,"ATT"))[0];n.used=!0,a.log.push({text:`❄️ ${n.name} (IA) gelé !`,type:"info"})}break}case"Remplacement+":a.maxSubs++,a.log.push({text:"🔄 +1 remplacement débloqué",type:"info"});break}l=!0}T.from("cards").delete().eq("id",e).then(()=>{}),l&&Je(i,a,c)}function Po(e,t,i){const a=Object.values(t.homeTeam).flat().filter(c=>!c.used);if(!a.length){i.toast("Aucun joueur actif à booster","error");return}i.openModal("⚡ Utiliser le Boost",`<div style="margin-bottom:12px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border-radius:10px;padding:12px;text-align:center;color:#000">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(c=>{c.addEventListener("click",()=>{const d=c.dataset.cardId;for(const s of["GK","DEF","MIL","ATT"]){const l=(t.homeTeam[s]||[]).find(p=>p.cardId===d);if(l){l.boost=(l.boost||0)+t.boostCard.value,t.log.push({text:`⚡ Boost +${t.boostCard.value} appliqué à ${l.name}`,type:"info"});break}}t.boostUsed=!0,i.closeModal(),Je(e,t,i)})})}function Qt(e,t,i,a,c){const d=document.createElement("div");d.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:900;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;overflow:hidden;cursor:pointer";const s=Array.from({length:10},(n,o)=>`
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(d);let l=!1;const p=()=>{l||(l=!0,d.remove(),setTimeout(()=>c(),50))};d.addEventListener("click",p),setTimeout(p,3500)}async function qt(e,t,i){var o,f;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:a}=i,c=t.homeScore>t.aiScore,d=t.homeScore===t.aiScore,s=c?"victoire":d?"nul":"defaite",l=Vn(t.mode,s);t.matchId&&await T.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:c?a.profile.id:null,home_credits_reward:l,played_at:new Date().toISOString()}).eq("id",t.matchId);const p={credits:(a.profile.credits||0)+l,matches_played:(a.profile.matches_played||0)+1};c?p.wins=(a.profile.wins||0)+1:d?p.draws=(a.profile.draws||0)+1:p.losses=(a.profile.losses||0)+1,await T.from("users").update(p).eq("id",a.profile.id),await i.refreshProfile();const n=document.createElement("div");n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",n.innerHTML=`
    ...
    </div>`,`<button class="btn btn-primary" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}async function wn(e,t){var i,a;try{const c=(a=(i=t==null?void 0:t.state)==null?void 0:i.profile)==null?void 0:a.id;try{(T.getChannels?T.getChannels():[]).forEach(s=>{const l=s.topic||"";(l.includes("matchmaking")||l.includes("pvp-match")||l.includes("friend-invite"))&&T.removeChannel(s)})}catch(d){console.warn("[Random] cleanup canaux:",d)}c&&await T.rpc("cancel_matchmaking",{p_user_id:c}).catch(()=>{})}catch{}await oi(e,t,"random",({deckId:c,formation:d,starters:s,subsRaw:l,gcCardsEnriched:p,gcDefs:n})=>{const o=f=>Ro(e,t,c,d,s,l,f,n||[]);if(!p.length){o([]);return}ni(e,p,o)})}async function Ro(e,t,i,a,c,d,s=[],l=[]){var z;const{state:p,navigate:n,toast:o}=t;let f=!1,v=null;et(e),e.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const _=async(C=!0)=>{f=!0,v&&(T.removeChannel(v),v=null),C&&await T.rpc("cancel_matchmaking",{p_user_id:p.profile.id}).catch(()=>{})};(z=document.getElementById("mm-cancel"))==null||z.addEventListener("click",async()=>{try{await _(!0)}catch{}Ye(e),n("home")});const A=async(C,H,B)=>{if(f)return;f=!0,v&&(T.removeChannel(v),v=null);const j=document.getElementById("mm-status");j&&(j.textContent="Adversaire trouvé !"),await new Promise(U=>setTimeout(U,600)),e.isConnected&&_n(e,t,C,B,s,l)},{data:r,error:u}=await T.rpc("try_matchmake",{p_user_id:p.profile.id,p_deck_id:i});if(u||!(r!=null&&r.success)){o("Erreur de matchmaking","error"),Ye(e),n("home");return}if(r.matched){A(r.match_id,r.opponent_id,!1);return}const I=document.getElementById("mm-status");I&&(I.textContent="En attente d'un autre joueur..."),v=T.channel("matchmaking-"+p.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${p.profile.id}`},C=>{const H=C.new;H.status==="matched"&&H.match_id&&A(H.match_id,H.matched_with,!0)}).subscribe()}async function Oo(e,t,i){const{state:a,navigate:c,toast:d}=t;try{(T.getChannels?T.getChannels():[]).forEach(n=>{const o=n.topic||"";(o.includes("matchmaking")||o.includes("pvp-match")||o.includes("friend-invite"))&&T.removeChannel(n)})}catch{}const{data:s}=await T.from("matches").select("home_id, away_id, status").eq("id",i).single();if(!s){d("Match introuvable","error"),c("home");return}if(s.status==="finished"){d("Ce match est terminé","info"),c("home");return}const l=s.home_id===a.profile.id;_n(e,t,i,l,[],[])}async function _n(e,t,i,a,c=[],d=[]){const{state:s,navigate:l,toast:p}=t,n=a?"p1":"p2",o=a?"p2":"p1",f=(c||[]).map(M=>M.id),v=(c||[]).map(M=>({id:M.id,gc_type:M.gc_type,_gcDef:M._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:_}=await T.from("matches").select("*").eq("id",i).single();if(!_){p("Match introuvable","error"),l("home");return}async function A(){const[{data:M},{data:P},{data:w},{data:$}]=await Promise.all([T.rpc("get_deck_for_match",{p_deck_id:_.home_deck_id}),T.rpc("get_deck_for_match",{p_deck_id:_.away_deck_id}),T.from("users").select("id,pseudo,club_name").eq("id",_.home_id).single(),T.from("users").select("id,pseudo,club_name").eq("id",_.away_id).single()]),g=h=>({cardId:h.card_id,position:h.position,id:h.id,firstname:h.firstname,name:h.surname_encoded,country_code:h.country_code,club_id:h.club_id,job:h.job,job2:h.job2,note_g:Number(h.note_g)||0,note_d:Number(h.note_d)||0,note_m:Number(h.note_m)||0,note_a:Number(h.note_a)||0,rarity:h.rarity,skin:h.skin,hair:h.hair,hair_length:h.hair_length,clubName:h.club_encoded_name||null,clubLogo:h.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}),m=((M==null?void 0:M.starters)||[]).map(h=>g(h)),x=((P==null?void 0:P.starters)||[]).map(h=>g(h)),y=(M==null?void 0:M.formation)||"4-4-2",b=(P==null?void 0:P.formation)||"4-4-2";return{p1Team:yt(m,y),p2Team:yt(x,b),p1Subs:((M==null?void 0:M.subs)||[]).map(h=>g(h)),p2Subs:((P==null?void 0:P.subs)||[]).map(h=>g(h)),p1Formation:y,p2Formation:b,p1Name:(w==null?void 0:w.club_name)||(w==null?void 0:w.pseudo)||"Joueur 1",p2Name:($==null?void 0:$.club_name)||($==null?void 0:$.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:a?f:[],gc_p2:a?[]:f,gcCardsFull_p1:a?v:[],gcCardsFull_p2:a?[]:v,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:d||[],lastActionAt:new Date().toISOString()}}let r=_.game_state&&Object.keys(_.game_state).length?_.game_state:null;if(!r)if(a){r=await A();const{data:M}=await T.from("matches").select("game_state").eq("id",i).single();!(M!=null&&M.game_state)||!Object.keys(M.game_state).length?await T.from("matches").update({game_state:r,turn_user_id:_.home_id}).eq("id",i):r=M.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let M=0;M<30&&!r;M++){await new Promise(w=>setTimeout(w,400));const{data:P}=await T.from("matches").select("game_state").eq("id",i).single();P!=null&&P.game_state&&Object.keys(P.game_state).length&&(r=P.game_state)}if(!r){p("Erreur de synchronisation","error"),l("home");return}r.gc_p2=f,r.gcCardsFull_p2=v,await T.from("matches").update({game_state:r}).eq("id",i)}let u=!1,I=null,z=!1;const C=new Set,H=new Set;function B(M){var h,E;try{T.removeChannel(j)}catch{}const P=r[n+"Score"]||0,w=r[o+"Score"]||0;let $,g;M.winner_id?($=M.winner_id===s.profile.id,g=!1):M.forfeit?($=P>w,g=!1):(g=P===w,$=P>w),(h=document.getElementById("pvp-end-overlay"))==null||h.remove();const m=document.createElement("div");m.id="pvp-end-overlay",m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const x=g?"🤝":$?"🏆":"😞",y=g?"MATCH NUL":$?"VICTOIRE !":"DÉFAITE",b=g?"#fff":$?"#FFD700":"#ff6b6b";m.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(m),(E=m.querySelector("#pvp-end-home"))==null||E.addEventListener("click",()=>{m.remove(),Ye(e),l("home")})}const j=T.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},M=>{const P=M.new;try{if(P.status==="finished"||P.forfeit){if(u)return;u=!0,I&&(clearInterval(I),I=null),P.game_state&&(r=P.game_state),B(P);return}if(P.game_state){const w=r;r=P.game_state;const $=r._lastGC;if($&&$.seq&&!H.has($.seq)&&(H.add($.seq),$.by!==n)){_e($.type,$.by,()=>G());return}const g=w[n+"Score"]||0,m=w[o+"Score"]||0,x=r[n+"Score"]||0,y=r[o+"Score"]||0,b=x>g,h=y>m;if((b||h)&&!C.has(r.round)){C.add(r.round);const E=[...r.log||[]].reverse().find(L=>L.isGoal),k=((E==null?void 0:E.homePlayers)||[]).map(L=>({name:L.name,note:L.note,portrait:L.portrait,job:L.job}));Ee(k,x,y,b,()=>G());return}G()}}catch(w){console.error("[PvP] crash:",w)}}).subscribe();async function U(M){Object.assign(r,M),r.lastActionAt=new Date().toISOString();const{error:P}=await T.from("matches").update({game_state:r}).eq("id",i);P&&p("Erreur de synchronisation","error");try{G()}catch(w){console.error("[PvP] renderPvpScreen crash:",w)}}async function oe(){if(u)return;u=!0,I&&(clearInterval(I),I=null);const M=a?_.away_id:_.home_id,P=a?"p2":"p1",w=a?"p1":"p2",$={...r,[P+"Score"]:3,[w+"Score"]:0,phase:"finished"};try{await T.from("matches").update({status:"finished",forfeit:!0,winner_id:M,home_score:$.p1Score||0,away_score:$.p2Score||0,game_state:$}).eq("id",i)}catch{}try{T.removeChannel(j)}catch{}setTimeout(()=>{Ye(e),l("home")},800)}const ce={BOOST_STAT:({value:M=1,count:P=1,roles:w=[]},$,g)=>{const m=$[n+"Team"],x=Object.entries(m).filter(([y])=>!w.length||w.includes(y)).flatMap(([y,b])=>b.filter(h=>!h.used).map(h=>({...h,_line:y})));if(!x.length){$.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),g($);return}re(x,P,`Choisir ${P} joueur(s) à booster (+${M})`,y=>{y.forEach(b=>{const h=(m[b._line]||[]).find(E=>E.cardId===b.cardId);h&&(h.boost=(h.boost||0)+M,$.log.push({text:`⚡ +${M} sur ${h.name}`,type:"info"}))}),$[n+"Team"]=m,g($)})},DEBUFF_STAT:({value:M=1,count:P=1,roles:w=[],target:$="ai"},g,m)=>{const x=$==="home"?n:o,y=g[x+"Team"],b=$==="home"?"allié":"adverse",h=Object.entries(y).filter(([E])=>!w.length||w.includes(E)).flatMap(([E,k])=>k.map(L=>({...L,_line:E})));if(!h.length){g.log.push({text:`🎯 Aucun joueur ${b}`,type:"info"}),m(g);return}re(h,P,`Choisir ${P} joueur(s) ${b}(s) (-${M})`,E=>{E.forEach(k=>{const L=(y[k._line]||[]).find(S=>S.cardId===k.cardId);L&&(L.boost=(L.boost||0)-M,g.log.push({text:`🎯 -${M} sur ${L.name}`,type:"info"}))}),g[x+"Team"]=y,m(g)})},GRAY_PLAYER:({count:M=1,roles:P=[],target:w="ai"},$,g)=>{const m=w==="home"?n:o,x=$[m+"Team"],y=w==="home"?"allié":"adverse",b=Object.entries(x).filter(([h])=>!P.length||P.includes(h)).flatMap(([h,E])=>E.filter(k=>!k.used).map(k=>({...k,_line:h})));if(!b.length){$.log.push({text:`❌ Aucun joueur ${y}`,type:"info"}),g($);return}re(b,M,`Choisir ${M} joueur(s) ${y}(s) à exclure`,h=>{const E="usedCardIds_"+m,k=new Set($[E]||[]);h.forEach(L=>{const S=(x[L._line]||[]).find(q=>q.cardId===L.cardId);S&&(S.used=!0,k.add(L.cardId),$.log.push({text:`❌ ${S.name} exclu !`,type:"info"}))}),$[E]=[...k],$[m+"Team"]=x,g($)})},REVIVE_PLAYER:({count:M=1,roles:P=[]},w,$)=>{const g=w[n+"Team"],m=Object.entries(g).filter(([x])=>!P.length||P.includes(x)).flatMap(([x,y])=>y.filter(b=>b.used).map(b=>({...b,_line:x})));if(!m.length){w.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),$(w);return}re(m,M,`Choisir ${M} joueur(s) à ressusciter`,x=>{x.forEach(y=>{const b=(g[y._line]||[]).find(h=>h.cardId===y.cardId);b&&(b.used=!1,w.log.push({text:`💫 ${b.name} ressuscité !`,type:"info"}))}),w[n+"Team"]=g,$(w)})},REMOVE_GOAL:({},M,P)=>{const w=o+"Score";M[w]>0?(M[w]--,M.log.push({text:"🚫 Dernier but annulé !",type:"info"})):M.log.push({text:"🚫 Aucun but à annuler",type:"info"}),P(M)},ADD_GOAL_DRAW:({},M,P)=>{M[n+"Score"]===M[o+"Score"]?(M[n+"Score"]++,M.log.push({text:"🎯 But bonus !",type:"info"})):M.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),P(M)},ADD_SUB:({value:M=1},P,w)=>{P.maxSubs=(P.maxSubs||3)+M,P.log.push({text:`🔄 +${M} remplacement(s)`,type:"info"}),w(P)},CUSTOM:({},M,P)=>P(M)};function re(M,P,w,$){const g=document.createElement("div");g.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let m=[];function x(){var b,h;const y=M.map(E=>{const k={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[E._line]||"#555",L=$e(E,E._line)+(E.boost||0),q=m.find(O=>O.cardId===E.cardId)?"#FFD700":"rgba(255,255,255,0.25)",F=E.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${E.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${q};background:${k};overflow:hidden;cursor:pointer;${F}">
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
    ...
    </div>`,(g=document.getElementById("pvp-home"))==null||g.addEventListener("click",()=>{try{T.removeChannel(j)}catch{}Ye(e),l("home")})}G()}const Ho=Object.freeze(Object.defineProperty({__proto__:null,renderMatchRandom:wn,resumePvpMatch:Oo},Symbol.toStringTag,{value:"Module"}));async function Uo(e,t,i,a){var c,d;try{const s=(d=(c=t==null?void 0:t.state)==null?void 0:c.profile)==null?void 0:d.id;try{(T.getChannels?T.getChannels():[]).forEach(p=>{const n=p.topic||"";(n.includes("matchmaking")||n.includes("pvp-match"))&&T.removeChannel(p)})}catch(l){console.warn("[FriendMatch] cleanup canaux:",l)}s&&(await T.rpc("cancel_matchmaking",{p_user_id:s}).catch(()=>{}),await T.from("matchmaking_queue").delete().eq("user_id",s).then(()=>{},()=>{}))}catch{}await oi(e,t,"random",({deckId:s,formation:l,starters:p,subsRaw:n,gcCardsEnriched:o,gcDefs:f})=>{const v=_=>Vo(e,t,s,l,p,n,_,f||[],i,a);if(!o.length){v([]);return}ni(e,o,v)})}async function Vo(e,t,i,a,c,d,s=[],l=[],p,n){var ce;const{state:o,navigate:f,toast:v}=t,_=o.profile.id;let A=!1,r=null;et(e);try{await T.rpc("cancel_matchmaking",{p_user_id:_})}catch{}try{await T.from("matchmaking_queue").delete().eq("user_id",_)}catch{}const u=(re,D,ie,W)=>{var G;e.innerHTML=`
      ...
      </div>`,(G=document.getElementById("lobby-cancel"))==null||G.addEventListener("click",async()=>{A=!0,r&&(T.removeChannel(r),r=null),ie&&await T.from("friend_match_invites").update({status:"declined"}).eq("id",ie),Ye(e),f("home")})},I=async(re,D)=>{A=!0,r&&(T.removeChannel(r),r=null),await new Promise(ie=>setTimeout(ie,600)),e.isConnected&&Ko(e,t,re,D,s,l)},{data:z}=await T.from("friend_match_invites").select("*").eq("status","pending").or(`and(inviter_id.eq.${_},invitee_id.eq.${p}),and(inviter_id.eq.${p},invitee_id.eq.${_})`).order("created_at",{ascending:!1}).limit(1).maybeSingle();let C,H;if(z&&z.inviter_id===p)H=!1,C=z.id,await T.from("friend_match_invites").update({invitee_deck_id:i,invitee_ready:!0,status:"accepted"}).eq("id",C);else{H=!0;const{data:re,error:D}=await T.from("friend_match_invites").insert({inviter_id:_,invitee_id:p,friend_id:p,inviter_deck_id:i,inviter_ready:!0,status:"pending"}).select().single();if(D||!re){v("Erreur création invitation","error"),Ye(e),f("home");return}C=re.id}if(u(!0,!H,C),!H){const re={home_id:p,away_id:_,home_deck_id:z.inviter_deck_id,away_deck_id:i,status:"active",mode:"friend"},D=await T.from("matches").insert(re).select().single(),ie=D.data;if(!ie){v("Erreur création match: "+(((ce=D.error)==null?void 0:ce.message)||""),"error"),Ye(e),f("home");return}await T.from("friend_match_invites").update({match_id:ie.id,status:"matched"}).eq("id",C),I(ie.id,!1);return}let B=!1;const j=re=>{A||B||(re.status==="matched"&&re.match_id?(B=!0,clearInterval(U),clearInterval(oe),I(re.match_id,!0)):re.status==="declined"?(clearInterval(U),clearInterval(oe),v(`${n} a décliné l'invitation`,"warning"),Ye(e),f("home")):re.invitee_ready&&u(!0,!0,C))},U=setInterval(()=>{if(A){clearInterval(U);return}T.from("matchmaking_queue").delete().eq("user_id",_).then(()=>{},()=>{})},3e3),oe=setInterval(async()=>{if(A||B){clearInterval(oe);return}const{data:re}=await T.from("friend_match_invites").select("*").eq("id",C).maybeSingle();re&&j(re)},1200);r=T.channel("friend-invite-"+C).on("postgres_changes",{event:"UPDATE",schema:"public",table:"friend_match_invites",filter:`id=eq.${C}`},re=>j(re.new)).subscribe()}async function Ko(e,t,i,a,c=[],d=[]){const{state:s,navigate:l,toast:p}=t,n=a?"p1":"p2",o=a?"p2":"p1",f=(c||[]).map(w=>w.id),v=(c||[]).map(w=>({id:w.id,gc_type:w.gc_type,_gcDef:w._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:_}=await T.from("matches").select("*").eq("id",i).single();if(!_){p("Match introuvable","error"),l("home");return}async function A(){const[{data:w},{data:$},{data:g},{data:m}]=await Promise.all([T.rpc("get_deck_for_match",{p_deck_id:_.home_deck_id}),T.rpc("get_deck_for_match",{p_deck_id:_.away_deck_id}),T.from("users").select("id,pseudo,club_name").eq("id",_.home_id).single(),T.from("users").select("id,pseudo,club_name").eq("id",_.away_id).single()]),x=k=>({cardId:k.card_id,position:k.position,id:k.id,firstname:k.firstname,name:k.surname_encoded,country_code:k.country_code,club_id:k.club_id,job:k.job,job2:k.job2,note_g:Number(k.note_g)||0,note_d:Number(k.note_d)||0,note_m:Number(k.note_m)||0,note_a:Number(k.note_a)||0,rarity:k.rarity,skin:k.skin,hair:k.hair,hair_length:k.hair_length,clubName:k.club_encoded_name||null,clubLogo:k.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}),y=((w==null?void 0:w.starters)||[]).map(k=>x(k)),b=(($==null?void 0:$.starters)||[]).map(k=>x(k)),h=(w==null?void 0:w.formation)||"4-4-2",E=($==null?void 0:$.formation)||"4-4-2";return{p1Team:yt(y,h),p2Team:yt(b,E),p1Subs:((w==null?void 0:w.subs)||[]).map(k=>x(k)),p2Subs:(($==null?void 0:$.subs)||[]).map(k=>x(k)),p1Formation:h,p2Formation:E,p1Name:(g==null?void 0:g.club_name)||(g==null?void 0:g.pseudo)||"Joueur 1",p2Name:(m==null?void 0:m.club_name)||(m==null?void 0:m.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:a?f:[],gc_p2:a?[]:f,gcCardsFull_p1:a?v:[],gcCardsFull_p2:a?[]:v,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:d||[],lastActionAt:new Date().toISOString()}}let r=_.game_state&&Object.keys(_.game_state).length?_.game_state:null;if(!r)if(a){r=await A();const{data:w}=await T.from("matches").select("game_state").eq("id",i).single();!(w!=null&&w.game_state)||!Object.keys(w.game_state).length?await T.from("matches").update({game_state:r,turn_user_id:_.home_id}).eq("id",i):r=w.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let w=0;w<60&&!r;w++){await new Promise(g=>setTimeout(g,400));const{data:$}=await T.from("matches").select("game_state").eq("id",i).single();$!=null&&$.game_state&&Object.keys($.game_state).length&&(r=$.game_state)}if(!r){p("Erreur de synchronisation","error"),l("home");return}r.gc_p2=f,r.gcCardsFull_p2=v,await T.from("matches").update({game_state:r}).eq("id",i)}let u=!1,I=!1,z=!1,C=null,H=!1;const B=new Set,j=new Set;function U(w){var k,L;try{T.removeChannel(oe)}catch{}const $=r[n+"Score"]||0,g=r[o+"Score"]||0;if(!z){z=!0;const S=w.winner_id||($>g?s.profile.id:g>$?"opp":null),q=S===s.profile.id?"win":S?"loss":null;q&&yi(()=>Promise.resolve().then(()=>on),void 0).then(F=>F.applyOwnEvolution(s.profile.id,q)).catch(()=>{})}if(!I){const S=r.p1Score||0,q=r.p2Score||0,F=(r.usedGc_p1||[]).length,O=(r.usedGc_p2||[]).length,V=w.winner_id||(S>q?_.home_id:q>S?_.away_id:null);(V?s.profile.id===V:s.profile.id<(a?_.away_id:_.home_id))&&(I=!0,Xi({player1Id:_.home_id,player2Id:_.away_id,score1:S,score2:q,gc1:F,gc2:O}).catch(K=>console.warn("[FriendMatch] updateStats:",K)))}let m,x;w.winner_id?(m=w.winner_id===s.profile.id,x=!1):w.forfeit?(m=$>g,x=!1):(x=$===g,m=$>g),(k=document.getElementById("pvp-end-overlay"))==null||k.remove();const y=document.createElement("div");y.id="pvp-end-overlay",y.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const b=x?"🤝":m?"🏆":"😞",h=x?"MATCH NUL":m?"VICTOIRE !":"DÉFAITE",E=x?"#fff":m?"#FFD700":"#ff6b6b";y.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(y),(L=y.querySelector("#pvp-end-home"))==null||L.addEventListener("click",()=>{y.remove(),Ye(e),l("home")})}const oe=T.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},w=>{const $=w.new;try{if($.status==="finished"||$.forfeit){if(u)return;u=!0,C&&(clearInterval(C),C=null),$.game_state&&(r=$.game_state),U($);return}if($.game_state){const g=r;r=$.game_state;const m=r._lastGC;if(m&&m.seq&&!j.has(m.seq)&&(j.add(m.seq),m.by!==n)){Y(m.type,m.by,()=>fe());return}const x=g[n+"Score"]||0,y=g[o+"Score"]||0,b=r[n+"Score"]||0,h=r[o+"Score"]||0,E=b>x,k=h>y;if((E||k)&&!B.has(r.round)){B.add(r.round);const L=[...r.log||[]].reverse().find(q=>q.isGoal),S=((L==null?void 0:L.homePlayers)||[]).map(q=>({name:q.name,note:q.note,portrait:q.portrait,job:q.job}));he(S,b,h,E,()=>fe());return}fe()}}catch(g){console.error("[PvP] crash:",g)}}).subscribe();async function ce(w){Object.assign(r,w),r.lastActionAt=new Date().toISOString();const{error:$}=await T.from("matches").update({game_state:r}).eq("id",i);$&&p("Erreur de synchronisation","error");try{fe()}catch(g){console.error("[PvP] renderPvpScreen crash:",g)}}async function re(){if(u)return;u=!0,C&&(clearInterval(C),C=null);const w=a?_.away_id:_.home_id,$=a?"p2":"p1",g=a?"p1":"p2",m={...r,[$+"Score"]:3,[g+"Score"]:0,phase:"finished"};try{await T.from("matches").update({status:"finished",forfeit:!0,winner_id:w,home_score:m.p1Score||0,away_score:m.p2Score||0,game_state:m}).eq("id",i)}catch{}try{T.removeChannel(oe)}catch{}setTimeout(()=>{Ye(e),l("home")},800)}const D={BOOST_STAT:({value:w=1,count:$=1,roles:g=[]},m,x)=>{const y=m[n+"Team"],b=Object.entries(y).filter(([h])=>!g.length||g.includes(h)).flatMap(([h,E])=>E.filter(k=>!k.used).map(k=>({...k,_line:h})));if(!b.length){m.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),x(m);return}ie(b,$,`Choisir ${$} joueur(s) à booster (+${w})`,h=>{h.forEach(E=>{const k=(y[E._line]||[]).find(L=>L.cardId===E.cardId);k&&(k.boost=(k.boost||0)+w,m.log.push({text:`⚡ +${w} sur ${k.name}`,type:"info"}))}),m[n+"Team"]=y,x(m)})},DEBUFF_STAT:({value:w=1,count:$=1,roles:g=[],target:m="ai"},x,y)=>{const b=m==="home"?n:o,h=x[b+"Team"],E=m==="home"?"allié":"adverse",k=Object.entries(h).filter(([L])=>!g.length||g.includes(L)).flatMap(([L,S])=>S.map(q=>({...q,_line:L})));if(!k.length){x.log.push({text:`🎯 Aucun joueur ${E}`,type:"info"}),y(x);return}ie(k,$,`Choisir ${$} joueur(s) ${E}(s) (-${w})`,L=>{L.forEach(S=>{const q=(h[S._line]||[]).find(F=>F.cardId===S.cardId);q&&(q.boost=(q.boost||0)-w,x.log.push({text:`🎯 -${w} sur ${q.name}`,type:"info"}))}),x[b+"Team"]=h,y(x)})},GRAY_PLAYER:({count:w=1,roles:$=[],target:g="ai"},m,x)=>{const y=g==="home"?n:o,b=m[y+"Team"],h=g==="home"?"allié":"adverse",E=Object.entries(b).filter(([k])=>!$.length||$.includes(k)).flatMap(([k,L])=>L.filter(S=>!S.used).map(S=>({...S,_line:k})));if(!E.length){m.log.push({text:`❌ Aucun joueur ${h}`,type:"info"}),x(m);return}ie(E,w,`Choisir ${w} joueur(s) ${h}(s) à exclure`,k=>{const L="usedCardIds_"+y,S=new Set(m[L]||[]);k.forEach(q=>{const F=(b[q._line]||[]).find(O=>O.cardId===q.cardId);F&&(F.used=!0,S.add(q.cardId),m.log.push({text:`❌ ${F.name} exclu !`,type:"info"}))}),m[L]=[...S],m[y+"Team"]=b,x(m)})},REVIVE_PLAYER:({count:w=1,roles:$=[]},g,m)=>{const x=g[n+"Team"],y=Object.entries(x).filter(([b])=>!$.length||$.includes(b)).flatMap(([b,h])=>h.filter(E=>E.used).map(E=>({...E,_line:b})));if(!y.length){g.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),m(g);return}ie(y,w,`Choisir ${w} joueur(s) à ressusciter`,b=>{b.forEach(h=>{const E=(x[h._line]||[]).find(k=>k.cardId===h.cardId);E&&(E.used=!1,g.log.push({text:`💫 ${E.name} ressuscité !`,type:"info"}))}),g[n+"Team"]=x,m(g)})},REMOVE_GOAL:({},w,$)=>{const g=o+"Score";w[g]>0?(w[g]--,w.log.push({text:"🚫 Dernier but annulé !",type:"info"})):w.log.push({text:"🚫 Aucun but à annuler",type:"info"}),$(w)},ADD_GOAL_DRAW:({},w,$)=>{w[n+"Score"]===w[o+"Score"]?(w[n+"Score"]++,w.log.push({text:"🎯 But bonus !",type:"info"})):w.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),$(w)},ADD_SUB:({value:w=1},$,g)=>{$.maxSubs=($.maxSubs||3)+w,$.log.push({text:`🔄 +${w} remplacement(s)`,type:"info"}),g($)},CUSTOM:({},w,$)=>$(w)};function ie(w,$,g,m){const x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let y=[];function b(){var E,k;const h=w.map(L=>{const S={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[L._line]||"#555",q=$e(L,L._line)+(L.boost||0),O=y.find(X=>X.cardId===L.cardId)?"#FFD700":"rgba(255,255,255,0.25)",V=L.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${L.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${O};background:${S};overflow:hidden;cursor:pointer;${V}">
        ...
        </div>`,(E=x.querySelector("#pp-close"))==null||E.addEventListener("click",()=>x.remove()),x.querySelectorAll(".pp-item").forEach(L=>{L.addEventListener("click",()=>{const S=L.dataset.cid,q=w.find(O=>O.cardId===S),F=y.findIndex(O=>O.cardId===S);q&&(F>-1?y.splice(F,1):y.length<$&&y.push(q),b())})}),(k=x.querySelector("#pp-confirm"))==null||k.addEventListener("click",()=>{x.remove(),m(y)})}b(),document.body.appendChild(x)}async function W(w,$){const m=(r["gcCardsFull_"+n]||[]).find(h=>h.id===w),x=(m==null?void 0:m._gcDef)||(r.gcDefs||[]).find(h=>{var E;return h.name===$||((E=h.name)==null?void 0:E.toLowerCase().trim())===($==null?void 0:$.toLowerCase().trim())}),y=[...r["usedGc_"+n]||[],w],b={type:$,by:n,seq:(r._gcAnimSeq||0)+1};j.add(b.seq),Y($,n,async()=>{if(x!=null&&x.effect_type&&x.effect_type!=="CUSTOM"){const E=D[x.effect_type];if(E){const k={...r};E(x.effect_params||{},k,async L=>{L["usedGc_"+n]=y,L._lastGC=b,L._gcAnimSeq=b.seq,await ce(L)});return}}const h={...r};switch($){case"Remplacement+":h.maxSubs=(h.maxSubs||3)+1,h.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const E=o+"Score";h[E]>0&&(h[E]--,h.log.push({text:"🚫 But annulé",type:"info"}));break}}h["usedGc_"+n]=y,h._lastGC=b,h._gcAnimSeq=b.seq,await ce(h)})}function G(w,$){const g="usedCardIds_"+w,m=new Set(r[g]||[]);$.forEach(x=>m.add(x)),r[g]=[...m]}function se(){for(const w of["p1","p2"]){const $=new Set(r["usedCardIds_"+w]||[]),g=r[w+"Team"];if(g)for(const m of["GK","DEF","MIL","ATT"])(g[m]||[]).forEach(x=>{x.used=$.has(x.cardId)})}}function fe(){var Xe,Qe,ot,gt,ae,ve;if(r.phase==="reveal")return xe();if(r.phase==="midfield")return _e();if(r.phase==="finished")return P();const w=r[n+"Team"],$=r[o+"Team"];se();const g=r[n+"Score"],m=r[o+"Score"],x=r[n+"Name"],y=r[o+"Name"],b=r.phase===n+"-attack",h=r.phase===n+"-defense",E=Array.isArray(r["selected_"+n])?r["selected_"+n]:[],k=E.map(ee=>ee.cardId),L=window.innerWidth>=700,S=r[n+"Subs"]||[],q=r["usedSubIds_"+n]||[],F=S.filter(ee=>!q.includes(ee.cardId)),O=r["gcCardsFull_"+n]||[],V=r["usedGc_"+n]||[],X=O.filter(ee=>!V.includes(ee.id)),K=r.boostOwner===n&&!r.boostUsed,te=!["GK","DEF","MIL","ATT"].some(ee=>($[ee]||[]).some(le=>!le.used)),ne=[...w.MIL||[],...w.ATT||[]].filter(ee=>!ee.used),ue=b&&te&&ne.length===0?[...w.GK||[],...w.DEF||[]].filter(ee=>!ee.used).map(ee=>ee.cardId):[];function me(ee,le,Z){var kt,Ht;const de=ee._gcDef,Ae={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[de==null?void 0:de.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",we={purple:"#b06ce0",light_blue:"#00d4ef"}[de==null?void 0:de.color]||"#b06ce0",tt=(de==null?void 0:de.name)||ee.gc_type,it=(de==null?void 0:de.effect)||((kt=Ue[ee.gc_type])==null?void 0:kt.desc)||"",rt=de!=null&&de.image_url?`/manager-wars/icons/${de.image_url}`:null,st=((Ht=Ue[ee.gc_type])==null?void 0:Ht.icon)||"⚡",ct=Math.round(Z*.22),$t=Math.round(Z*.22),pt=Z-ct-$t,ht=tt.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${ee.id}" data-gc-type="${ee.gc_type}"
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${ft(w,r[n+"Formation"],Le,k,300,300,ue)}
      </div>
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      ${w!=null&&w.length?`<div style="display:flex;gap:10px;animation:scoreIn .4s ease 1s both;position:relative;z-index:1">
        ${w.map(L=>`<div style="text-align:center">
      ...
      <div style="display:flex;gap:12px;width:190px">
        <button id="pvp-gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
        <button id="pvp-gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
      </div>`,document.body.appendChild(S),(O=S.querySelector("#pvp-gc-back"))==null||O.addEventListener("click",()=>S.remove()),(V=S.querySelector("#pvp-gc-use"))==null||V.addEventListener("click",()=>{S.remove(),W(w,$)})}function N(){var m;const w=r[n+"Team"],$=Object.entries(w).flatMap(([x,y])=>(y||[]).filter(b=>!b.used).map(b=>({...b,_line:x})));if(!$.length)return;const g=document.createElement("div");g.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",g.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
        <div style="flex:1;font-size:14px;font-weight:700;color:#fff">⚡ Choisir un joueur pour +${r.boostValue}</div>
        <button id="bp-close" style="background:none;border:none;color:rgba(255,255,255,0.5);font-size:22px;cursor:pointer">✕</button>
      </div>
      ...
      </div>`,document.body.appendChild(g),(m=g.querySelector("#bp-close"))==null||m.addEventListener("click",()=>g.remove()),g.querySelectorAll(".bp-item").forEach(x=>{x.addEventListener("click",async()=>{const y=x.dataset.cid,b=$.find(E=>E.cardId===y);if(!b)return;const h=(w[b._line]||[]).find(E=>E.cardId===y);h&&(h.boost=(h.boost||0)+r.boostValue),g.remove(),await ce({[n+"Team"]:w,boostUsed:!0})})})}function R(w=null){var F,O;if(!(r.phase===n+"-attack")){p("Remplacement uniquement avant votre attaque","warning");return}const g=r[n+"Team"],m=r["usedSubIds_"+n]||[],x=r.maxSubs||3;if(m.length>=x){p(`Maximum ${x} remplacements atteint`,"warning");return}const y=Object.entries(g).flatMap(([V,X])=>(X||[]).filter(K=>K.used).map(K=>({...K,_line:V}))),b=(r[n+"Subs"]||[]).filter(V=>!m.includes(V.cardId));if(!y.length){p("Aucun joueur utilisé à remplacer","warning");return}if(!b.length){p("Aucun remplaçant disponible","warning");return}let h=Math.max(0,y.findIndex(V=>V.cardId===w));const E=((F=y[h])==null?void 0:F._line)||((O=y[h])==null?void 0:O.job);let k=Math.max(0,b.findIndex(V=>V.job===E)),L=!1;const S=document.createElement("div");S.id="pvp-sub-overlay",S.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function q(){var me,ye,be,Te,ke,De;const V=y[h],X=b[k],K=Math.min(130,Math.round((window.innerWidth-90)/2)),te=Math.round(K*1.35),ne=ze=>`background:rgba(255,255,255,0.12);border:none;color:${ze?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${ze?"default":"pointer"};flex-shrink:0`;S.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:6px">Appuyer pour continuer</div>`,document.body.appendChild(x);let b=!1;const h=()=>{b||(b=!0,x.remove(),setTimeout(()=>g(),50))};x.addEventListener("click",h),setTimeout(h,2200)}function ge(){var $;const w=document.createElement("div");w.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:20px;overflow-y:auto",w.innerHTML=`
      <div style="font-size:12px;color:rgba(255,255,255,0.5);letter-spacing:2px;text-transform:uppercase">Équipe adverse</div>
      <div style="font-size:18px;font-weight:900;color:#ff6b6b">${r[o+"Name"]}</div>
      <div style="width:min(90vw,420px)">${ut(r[o+"Team"],r[o+"Formation"],null,[],300,300)}</div>
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(w),($=w.querySelector("#pvp-opp-close"))==null||$.addEventListener("click",()=>w.remove())}function pe(){var m;const w=r.log||[],$=document.createElement("div");$.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column";const g=x=>{var b,h,E;if(x.type==="duel"){const k=x.isGoal,L=x.homeScored&&n==="p1"||!x.homeScored&&k&&n==="p2",S=x.homeScored?"#FFD700":k?"#ff6b6b":"rgba(255,255,255,0.3)",q=k?L?"⚽ BUT !":"⚽ BUT adversaire !":(b=x.homePlayers)!=null&&b.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${k?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${S};margin-bottom:4px">
      ...
      </div>`,document.body.appendChild($),(m=$.querySelector("#pvp-hist-close"))==null||m.addEventListener("click",()=>$.remove())}async function Me(){if(r.phase!==n+"-attack")return;const w=n==="p1"?"p2":"p1",$=(r.round||0)+1,g=[...r.log||[]];g.push({type:"info",text:`⏭️ ${r[n+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const m=Re(r),x=Ie(w),y=m||!x?"finished":w+"-attack";await ce({["selected_"+n]:[],modifiers:{...r.modifiers,[n]:{}},pendingAttack:null,phase:y,attacker:w,round:$,log:g}),y==="finished"&&await M(r)}async function Ce(){const w=r[n+"Team"],$=!["GK","DEF","MIL","ATT"].some(y=>(r[o+"Team"][y]||[]).some(b=>!b.used)),g=(r["selected_"+n]||[]).map(y=>{const b=(w[y._role]||[]).find(q=>q.cardId===y.cardId)||y,h=$&&["GK","DEF"].includes(y._role),E=w[y._role]||[],k=E.findIndex(q=>q.cardId===y.cardId),L=lt(E.length),S=k>=0?L[k]:b._col??1;return{...b,_line:y._role,_col:S,...h?{note_a:Math.max(1,Number(b.note_a)||0)}:{}}});if(!g.length)return;const m=Nt(g,r.modifiers[n]||{});G(n,g.map(y=>y.cardId)),g.forEach(y=>{const b=(w[y._role]||[]).find(h=>h.cardId===y.cardId);b&&(b.used=!0)}),r["selected_"+n]=[],fe();const x=[...r.log||[]];if($){const y=(r[n+"Score"]||0)+1,b=g.map(S=>({name:S.name,note:$e(S,S._line||"ATT"),portrait:qe(S),job:S.job}));x.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:b,homeTotal:m.total,aiTotal:0});const h=(r.round||0)+1,E=n==="p1"?"p2":"p1",k={...r,[n+"Team"]:w,[n+"Score"]:y},L=Re(k);B.add(h),he(b,y,r[o+"Score"]||0,!0,async()=>{await ce({[n+"Team"]:w,[n+"Score"]:y,["selected_"+n]:[],modifiers:{...r.modifiers,[n]:{}},pendingAttack:null,phase:L?"finished":E+"-attack",attacker:E,round:h,log:x}),L&&await M({...r,[n+"Score"]:y})});return}x.push({type:"pending",text:`⚔️ ${r[n+"Name"]} attaque (${m.total})`}),await ce({[n+"Team"]:w,[o+"Team"]:r[o+"Team"],pendingAttack:{...m,players:g,side:n},["selected_"+n]:[],modifiers:{...r.modifiers,[n]:{}},phase:o+"-defense",log:x})}async function Be(){const w=r[n+"Team"],$=(r["selected_"+n]||[]).map(V=>{const X=(w[V._role]||[]).find(me=>me.cardId===V.cardId)||V,K=w[V._role]||[],te=K.findIndex(me=>me.cardId===V.cardId),ne=lt(K.length),ue=te>=0?ne[te]:X._col??1;return{...X,_line:V._role,_col:ue}}),g=Rt($,r.modifiers[n]||{});G(n,$.map(V=>V.cardId)),$.forEach(V=>{const X=(w[V._role]||[]).find(K=>K.cardId===V.cardId);X&&(X.used=!0)}),r["selected_"+n]=[],fe();const m=Ot(r.pendingAttack.total,g.total,r.modifiers[n]||{}),x=r.pendingAttack.side,y=m==="attack"||(m==null?void 0:m.goal),b=x==="p1"?"p2":"p1",h=(r.round||0)+1,E=(r.pendingAttack.players||[]).map(V=>({name:V.name,note:$e(V,V._line||"ATT"),portrait:qe(V),job:V.job})),k=[...r.log||[]];k.push({type:"duel",isGoal:y,homeScored:y&&x===n,text:y?`⚽ BUT de ${r[x+"Name"]} ! (${r.pendingAttack.total} vs ${g.total})`:`✋ Attaque stoppée (${r.pendingAttack.total} vs ${g.total})`,homePlayers:E,aiPlayers:$.map(V=>({name:V.name,note:$e(V,V._line||"DEF"),portrait:qe(V),job:V.job})),homeTotal:r.pendingAttack.total,aiTotal:g.total});const L=y?(r[x+"Score"]||0)+1:r[x+"Score"]||0,S={...r,[n+"Team"]:w,[x+"Score"]:L},q=Re(S),F=q?"finished":b+"-attack",O=async()=>{await ce({[n+"Team"]:w,[o+"Team"]:r[o+"Team"],[x+"Score"]:L,["selected_"+n]:[],modifiers:{...r.modifiers,[n]:{}},pendingAttack:null,phase:F,attacker:b,round:h,log:k}),(F==="finished"||q)&&await M({...r,[x+"Score"]:L})};if(y){const V=x===n,X=V?L:r[n+"Score"]||0,K=V?r[o+"Score"]||0:L;B.add(h),he(E,X,K,V,O)}else await O()}function Ve(w){return["MIL","ATT"].some($=>(w[$]||[]).some(g=>!g.used))}function Se(w){return["GK","DEF","MIL","ATT"].some($=>(w[$]||[]).some(g=>!g.used))}function Ne(w){return Se(w)&&!Ve(w)}function Ie(w){const $=r[w+"Team"],g=r[(w==="p1"?"p2":"p1")+"Team"];return!!(Ve($)||!Se(g)&&Ne($))}function Re(w){const $=["MIL","ATT"].some(k=>(w.p1Team[k]||[]).some(L=>!L.used)),g=["MIL","ATT"].some(k=>(w.p2Team[k]||[]).some(L=>!L.used)),m=Se(w.p1Team),x=Se(w.p2Team);return!$&&!(!x&&m)&&(!g&&!(!m&&x))}function Ze(w){const $=w.p1Score||0,g=w.p2Score||0;return $===g?null:$>g?_.home_id:_.away_id}async function M(w){try{const $=Ze(w),g=$?_.home_id===$?_.away_id:_.home_id:null;await T.from("matches").update({status:"finished",winner_id:$,home_score:w.p1Score||0,away_score:w.p2Score||0}).eq("id",i),$&&g&&_i($,g).catch(()=>{})}catch($){console.error("[PvP] finishMatch:",$)}}function P(){var x;const w=r[n+"Score"],$=r[o+"Score"],g=w>$,m=w===$;et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      ...
      <button id="pvp-home" style="padding:16px 40px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">🏠 Retour</button>
    </div>`,(x=document.getElementById("pvp-home"))==null||x.addEventListener("click",()=>{try{T.removeChannel(oe)}catch{}Ye(e),l("home")})}fe()}const At="#1A6B3C",xt="#D4A017";async function Yo(e,t){var a;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const i=(a=t.state.params)==null?void 0:a.openLeagueId;if(i){t.state.params.openLeagueId=null,await bt(e,t,i);return}await _t(e,t)}async function _t(e,t,i="waiting"){var I;const{state:a}=t,c=a.profile.id,{data:d}=await T.from("mini_league_members").select("league_id").eq("user_id",c),s=(d||[]).map(z=>z.league_id),{data:l,error:p}=await T.from("mini_leagues").select("*, mini_league_members(count)").eq("status","waiting").eq("is_archived",!1).order("created_at",{ascending:!1}).limit(30),n=p?(await T.from("mini_leagues").select("*, mini_league_members(count)").eq("status","waiting").order("created_at",{ascending:!1}).limit(30)).data||[]:l||[],f=(s.length?await T.from("mini_leagues").select("*, mini_league_members(count)").in("id",s).order("created_at",{ascending:!1}):{data:[]}).data||[],v=f.filter(z=>z.status==="waiting"&&!z.is_archived),_=f.filter(z=>z.status==="active"&&!z.is_archived),A=f.filter(z=>z.is_archived||z.status==="finished"),r=n.filter(z=>!s.includes(z.id)),u=[{key:"waiting",label:"🟡 En attente",count:v.length+r.length},{key:"active",label:"🟢 En cours",count:_.length},{key:"archived",label:"📁 Archivées",count:A.length}];e.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:10px">
      ${i==="waiting"?Wo(v,r,c):i==="active"?Jo(_,c):Xo(A,c)}
    </div>
  </div>`,(I=document.getElementById("ml-create-btn"))==null||I.addEventListener("click",()=>Zo(e,t)),e.querySelectorAll(".ml-tab").forEach(z=>z.addEventListener("click",()=>_t(e,t,z.dataset.tab))),e.querySelectorAll("[data-league-id]").forEach(z=>z.addEventListener("click",()=>bt(e,t,z.dataset.leagueId))),e.querySelectorAll("[data-join]").forEach(z=>z.addEventListener("click",C=>{C.stopPropagation(),$n(e,t,z.dataset.join,z.dataset.type)})),e.querySelectorAll("[data-delete]").forEach(z=>z.addEventListener("click",C=>{C.stopPropagation(),gi(e,t,z.dataset.delete,z.dataset.name,i)}))}function ei(e,t,i=!1){var s,l;const a=e.creator_id===t,c=e.pot||0,d=((l=(s=e.mini_league_members)==null?void 0:s[0])==null?void 0:l.count)||0;return`<div data-league-id="${e.id}" style="background:#fff;border-radius:12px;padding:14px 16px;box-shadow:0 1px 6px rgba(0,0,0,0.08);cursor:pointer;margin-bottom:8px">
  ...
  </div>`}function Wo(e,t,i){const a=[];return e.length&&(a.push('<div style="font-size:11px;font-weight:700;color:#555;text-transform:uppercase;letter-spacing:1px">Mes leagues en attente</div>'),a.push(...e.map(c=>ei(c,i,!1)))),t.length&&(a.push('<div style="font-size:11px;font-weight:700;color:#555;text-transform:uppercase;letter-spacing:1px;margin-top:4px">Rejoindre</div>'),a.push(...t.map(c=>ei(c,i,!0)))),a.length?a.join(""):'<div style="text-align:center;padding:40px;color:#aaa">🏆<br>Aucune mini league.<br>Crée la première !</div>'}function Jo(e,t){return e.length?e.map(i=>ei(i,t)).join(""):'<div style="text-align:center;padding:40px;color:#aaa">Aucune mini league en cours.</div>'}function Xo(e,t){return e.length?e.map(i=>ei(i,t)).join(""):'<div style="text-align:center;padding:40px;color:#aaa">Aucune mini league archivée.</div>'}function Zo(e,t){const i=document.createElement("div");i.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9000;display:flex;align-items:center;justify-content:center;padding:16px",i.innerHTML=`<div style="background:#fff;border-radius:16px;width:100%;max-width:400px;max-height:90vh;overflow-y:auto">
    ...
    <div style="padding:20px;display:flex;flex-direction:column;gap:14px">
    ...
    </div></div>`,document.body.appendChild(i),i.querySelector("#ml-form-close").addEventListener("click",()=>i.remove()),i.addEventListener("click",a=>{a.target===i&&i.remove()}),i.querySelectorAll('input[name="ml-type"]').forEach(a=>a.addEventListener("change",()=>{document.getElementById("ml-pwd-block").style.display=a.value==="private"?"block":"none"})),i.querySelector("#ml-create-confirm").addEventListener("click",async()=>{var A,r,u,I;const{toast:a}=t,c=document.getElementById("ml-name").value.trim(),d=parseInt(document.getElementById("ml-max").value)||6,s=parseInt(document.getElementById("ml-fee").value)||500,l=((A=i.querySelector('input[name="ml-type"]:checked'))==null?void 0:A.value)||"public",p=((r=i.querySelector('input[name="ml-mode"]:checked'))==null?void 0:r.value)||"aller",n=((I=(u=document.getElementById("ml-pwd"))==null?void 0:u.value)==null?void 0:I.trim())||null;if(!c){a("Le nom est obligatoire","error");return}if(s<100){a("Mise minimum : 100 crédits","error");return}if(l==="private"&&!n){a("Mot de passe requis","error");return}const{data:o,error:f}=await T.from("mini_leagues").insert({name:c,creator_id:t.state.profile.id,type:l,password:n,mode:p,max_players:d,entry_fee:s}).select().single();if(f){a("Erreur : "+f.message,"error");return}const{data:v}=await T.from("users").select("credits").eq("id",t.state.profile.id).single();if(((v==null?void 0:v.credits)||0)<s){await T.from("mini_leagues").delete().eq("id",o.id),a(`Crédits insuffisants pour la mise (${s.toLocaleString("fr")} cr.)`,"error");return}await T.from("users").update({credits:v.credits-s}).eq("id",t.state.profile.id),await T.from("mini_leagues").update({pot:s}).eq("id",o.id),await T.from("mini_league_members").insert({league_id:o.id,user_id:t.state.profile.id}),t.state.profile&&(t.state.profile.credits=v.credits-s);const _=document.getElementById("nav-credits");_&&(_.textContent=`💰 ${(t.state.profile.credits||0).toLocaleString("fr")}`),i.remove(),a(`Mini League créée ! ${s.toLocaleString("fr")} cr. déduits.`,"success"),bt(e,t,o.id)})}function Qo(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9500;display:flex;align-items:center;justify-content:center;padding:20px",t.innerHTML=`
      ...
      </div>`,document.body.appendChild(t);const i=t.querySelector("#ml-pwd-input"),a=c=>{t.remove(),e(c)};setTimeout(()=>i==null?void 0:i.focus(),80),t.querySelector("#pwd-cancel").addEventListener("click",()=>a(null)),t.querySelector("#pwd-ok").addEventListener("click",()=>a(i.value.trim())),i.addEventListener("keydown",c=>{c.key==="Enter"&&a(i.value.trim())}),t.addEventListener("click",c=>{c.target===t&&a(null)})})}async function $n(e,t,i,a){const{toast:c,state:d}=t,s=d.profile.id,{data:l}=await T.from("mini_leagues").select("password,status,entry_fee,pot,max_players").eq("id",i).single();if(!l){c("Mini League introuvable","error");return}if(l.status!=="waiting"){c("Cette Mini League a déjà démarré","warning");return}const{count:p}=await T.from("mini_league_members").select("id",{count:"exact",head:!0}).eq("league_id",i);if(p>=l.max_players){c("La Mini League est complète","warning");return}if(a==="private"){const r=await Qo();if(r===null)return;if(l.password!==r){c("Mot de passe incorrect","error");return}}const n=l.entry_fee||100,{data:o}=await T.from("users").select("credits").eq("id",s).single();if(((o==null?void 0:o.credits)||0)<n){c(`Crédits insuffisants — il te faut ${n.toLocaleString("fr")} cr. (solde : ${((o==null?void 0:o.credits)||0).toLocaleString("fr")} cr.)`,"error");return}const{error:f}=await T.from("mini_league_members").insert({league_id:i,user_id:s});if(f){c("Erreur inscription : "+f.message,"error");return}const{error:v}=await T.from("users").update({credits:o.credits-n}).eq("id",s),{error:_}=await T.from("mini_leagues").update({pot:(l.pot||0)+n}).eq("id",i);(v||_)&&console.error("Erreur post-inscription:",v,_),d.profile&&(d.profile.credits=o.credits-n);const A=document.getElementById("nav-credits");A&&(A.textContent=`💰 ${(d.profile.credits||0).toLocaleString("fr")}`),c(`✅ Inscrit ! −${n.toLocaleString("fr")} cr. · Pot : ${((l.pot||0)+n).toLocaleString("fr")} cr.`,"success"),bt(e,t,i)}async function er(e,t,i,a){const{toast:c,state:d}=t,s=d.profile.id;if(!confirm(`Te désinscrire et récupérer ${a.toLocaleString("fr")} cr. ?`))return;const{data:l}=await T.from("mini_leagues").select("pot,status").eq("id",i).single();if(!l||l.status!=="waiting"){c("Impossible — la league a déjà démarré","error");return}const{data:p}=await T.from("users").select("credits").eq("id",s).single();await T.from("mini_leagues").update({pot:Math.max(0,(l.pot||0)-a)}).eq("id",i),await T.from("users").update({credits:((p==null?void 0:p.credits)||0)+a}).eq("id",s),await T.from("mini_league_members").delete().eq("league_id",i).eq("user_id",s),d.profile&&(d.profile.credits=((p==null?void 0:p.credits)||0)+a);const n=document.getElementById("nav-credits");n&&(n.textContent=`💰 ${(d.profile.credits||0).toLocaleString("fr")}`),c(`↩️ Désinscrit · +${a.toLocaleString("fr")} cr. remboursés`,"success"),_t(e,t,"waiting")}async function gi(e,t,i,a,c){const{toast:d}=t;if(!confirm(`Supprimer définitivement "${a}" et tous ses matchs/membres ? Action irréversible.`))return;await T.from("mini_league_matches").delete().eq("league_id",i),await T.from("mini_league_members").delete().eq("league_id",i);const{error:s}=await T.from("mini_leagues").delete().eq("id",i);if(s){d("Erreur suppression ("+s.message+")","error");return}d("Mini League supprimée avec succès","success"),_t(e,t,c)}async function tr(e,t,i){await T.from("mini_leagues").update({is_archived:!0}).eq("id",i),t.toast("Mini League archivée","success"),_t(e,t,"archived")}async function bt(e,t,i){var H,B,j,U,oe,ce,re,D,ie;const{state:a,toast:c}=t,d=a.profile.id,[{data:s},{data:l},{data:p}]=await Promise.all([T.from("mini_leagues").select("*").eq("id",i).single(),T.from("mini_league_members").select("*, user:users(id,pseudo,club_name,club_color1,club_color2)").eq("league_id",i),T.from("mini_league_matches").select("*").eq("league_id",i).order("matchday").order("created_at")]);if(!s){c("Introuvable","error"),await _t(e,t);return}const n=(l||[]).some(W=>W.user_id===d),o=s.creator_id===d,f=(l||[]).map(W=>W.user).filter(Boolean),v=En(f,p||[]),_=(p||[]).filter(W=>W.matchday===s.current_day),A=s.pot||0,r=s.entry_fee||100,u=_.length>0&&_.every(W=>W.status==="finished"||W.status==="bye"),I=s.current_day>=s.total_days,z=(l||[]).find(W=>W.user_id===d);e.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>${v.map((W,G)=>{const se=A>0&&s.status==="finished"?G===0?Math.floor(A*.7):G===1?Math.floor(A*.2):G===2?Math.floor(A*.1):0:0;return`<tr style="border-bottom:1px solid #f5f5f5;${W.userId===d?"background:#f0fdf4;":""}">
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        ${Array.from({length:Math.max(0,s.status==="active"?s.current_day-1:s.current_day)},(W,G)=>G+1).reverse().map(W=>{const G=(p||[]).filter(se=>se.matchday===W);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:#888;margin-bottom:6px">Journée ${W}</div>${G.map(se=>Ri(se,f,d,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
  ...
  </div>`;const C=s.status==="waiting"?"waiting":s.status==="active"?"active":"archived";if((H=document.getElementById("ml-back"))==null||H.addEventListener("click",()=>_t(e,t,C)),(B=document.getElementById("ml-start-btn"))==null||B.addEventListener("click",()=>ir(e,t,s,f)),(j=document.getElementById("ml-next-day"))==null||j.addEventListener("click",()=>nr(e,t,i)),(U=document.getElementById("ml-finish-btn"))==null||U.addEventListener("click",()=>kn(e,t,i,A,v,f)),(oe=document.getElementById("ml-join-now"))==null||oe.addEventListener("click",()=>$n(e,t,i,s.type)),(ce=document.getElementById("ml-leave-btn"))==null||ce.addEventListener("click",()=>er(e,t,i,r)),(re=document.getElementById("ml-delete-btn"))==null||re.addEventListener("click",()=>gi(e,t,i,s.name,"waiting")),(D=document.getElementById("ml-delete-now"))==null||D.addEventListener("click",()=>gi(e,t,i,s.name,C)),(ie=document.getElementById("ml-archive-btn"))==null||ie.addEventListener("click",()=>tr(e,t,i)),e.querySelectorAll("[data-play-match]").forEach(W=>{W.addEventListener("click",()=>{const G=_.find(se=>se.id===W.dataset.playMatch);G&&t.navigate("match-mini-league",{mlMatchId:G.id,leagueId:i})})}),s.status==="finished"&&z){const W=v.findIndex(G=>G.userId===d);W>=0&&W<3&&z.prize_amount>0&&setTimeout(()=>or(e,t,s,z,W),400)}}function Ri(e,t,i,a,c=!1){const d=f=>t.find(v=>v.id===f);if(e.is_bye){const f=d(e.home_id);return`<div style="padding:8px;border-radius:8px;background:#f9f9f9;margin-bottom:6px;font-size:12px;color:#888;text-align:center">🔵 ${(f==null?void 0:f.club_name)||(f==null?void 0:f.pseudo)||"?"} exempté(e)</div>`}const s=d(e.home_id),l=d(e.away_id),p=e.home_id===i||e.away_id===i,n=p&&e.status==="pending"&&a&&!c,o=e.status==="finished"?`${e.home_score} - ${e.away_score}`:"vs";return`<div style="display:flex;align-items:center;gap:8px;padding:10px;border-radius:8px;background:${p?"#f0fdf4":"#f9f9f9"};margin-bottom:6px;border:1px solid ${p?"#86efac":"#f0f0f0"}">
  ...
  </div>`}async function ir(e,t,i,a){const{toast:c,state:d}=t,s=rr(a.map(n=>n.id),i.mode),l=[];s.forEach((n,o)=>n.forEach(f=>l.push({league_id:i.id,matchday:o+1,home_id:f.home||f.bye,away_id:f.away||null,is_bye:!!f.bye,status:f.bye?"bye":"pending"})));const{error:p}=await T.from("mini_league_matches").insert(l);if(p){c("Erreur calendrier : "+p.message,"error");return}await T.from("mini_leagues").update({status:"active",current_day:1,total_days:s.length}).eq("id",i.id),c(`🚀 Lancée ! Pot : ${(i.pot||0).toLocaleString("fr")} cr.`,"success"),bt(e,t,i.id)}async function nr(e,t,i){const{data:a}=await T.from("mini_leagues").select("current_day,total_days,pot").eq("id",i).single(),c=(a.current_day||0)+1;if(c>(a.total_days||0)){await kn(e,t,i,a.pot||0,null,null);return}await T.from("mini_leagues").update({current_day:c}).eq("id",i),t.toast(`Journée ${c} commencée !`,"success"),bt(e,t,i)}async function kn(e,t,i,a,c,d){const{toast:s,state:l}=t;let p=c,n=d;if(!p){const{data:f}=await T.from("mini_league_matches").select("*").eq("league_id",i),{data:v}=await T.from("mini_league_members").select("*, user:users(id,pseudo,club_name)").eq("league_id",i);n=(v||[]).map(_=>_.user).filter(Boolean),p=En(n,f||[])}const o=[Math.floor(a*.7),Math.floor(a*.2),Math.floor(a*.1)];await Promise.all(p.slice(0,3).map((f,v)=>o[v]?T.from("mini_league_members").update({prize_amount:o[v],prize_claimed:!1}).eq("league_id",i).eq("user_id",f.userId):Promise.resolve())),await T.from("mini_leagues").update({status:"finished"}).eq("id",i),s("🏆 Mini League terminée ! Les gagnants peuvent récupérer leurs crédits.","success"),bt(e,t,i)}async function or(e,t,i,a,c){var v,_;const{state:d,toast:s}=t,l=[Math.floor((i.pot||0)*.7),Math.floor((i.pot||0)*.2),Math.floor((i.pot||0)*.1)],p=["🥇","🥈","🥉"][c],n=a.prize_amount||l[c]||0,o=a.prize_claimed,f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",f.innerHTML=`
    ...
    </div>`,document.body.appendChild(f),(v=f.querySelector("#prize-close"))==null||v.addEventListener("click",()=>f.remove()),f.addEventListener("click",A=>{A.target===f&&f.remove()}),(_=f.querySelector("#claim-prize-btn"))==null||_.addEventListener("click",async()=>{const{data:A}=await T.from("users").select("credits").eq("id",d.profile.id).single();await T.from("users").update({credits:((A==null?void 0:A.credits)||0)+n}).eq("id",d.profile.id),await T.from("mini_league_members").update({prize_claimed:!0}).eq("league_id",i.id).eq("user_id",d.profile.id),d.profile&&(d.profile.credits=((A==null?void 0:A.credits)||0)+n);const r=document.getElementById("nav-credits");r&&(r.textContent=`💰 ${d.profile.credits.toLocaleString("fr")}`),s(`💰 +${n.toLocaleString("fr")} cr. ajoutés à ton solde !`,"success"),f.remove(),bt(e,t,i.id)})}function rr(e,t){const a=e.length%2===0?[...e]:[...e,null],c=a.length;let d=a.slice(1);const s=[];for(let l=0;l<c-1;l++){const p=[],n=[a[0],...d];for(let o=0;o<c/2;o++){const f=n[o],v=n[c-1-o];f===null?p.push({bye:v}):v===null?p.push({bye:f}):p.push({home:f,away:v})}s.push(p),d=[d[d.length-1],...d.slice(0,-1)]}return t==="aller-retour"?[...s,...s.map(l=>l.map(p=>p.bye?p:{home:p.away,away:p.home}))]:s}function En(e,t){const i={};return e.forEach(a=>{i[a.id]={userId:a.id,pseudo:a.pseudo,clubName:a.club_name||a.pseudo,played:0,won:0,drawn:0,lost:0,goalsFor:0,goalsAgainst:0,goalDiff:0,points:0}}),t.filter(a=>a.status==="finished"&&!a.is_bye&&a.home_score!=null).forEach(a=>{const c=i[a.home_id],d=i[a.away_id];!c||!d||(c.played++,d.played++,c.goalsFor+=a.home_score,c.goalsAgainst+=a.away_score,d.goalsFor+=a.away_score,d.goalsAgainst+=a.home_score,a.home_score>a.away_score?(c.won++,c.points+=3,d.lost++):a.home_score<a.away_score?(d.won++,d.points+=3,c.lost++):(c.drawn++,c.points++,d.drawn++,d.points++),c.goalDiff=c.goalsFor-c.goalsAgainst,d.goalDiff=d.goalsFor-d.goalsAgainst)}),Object.values(i).sort((a,c)=>c.points-a.points||c.goalDiff-a.goalDiff||c.goalsFor-a.goalsFor)}async function Oi(e,t,i,a){var c,d;t.state.params={...t.state.params,matchMode:"mini-league",mlMatchId:i,leagueId:a};try{const s=(d=(c=t==null?void 0:t.state)==null?void 0:c.profile)==null?void 0:d.id;try{(T.getChannels?T.getChannels():[]).forEach(p=>{const n=p.topic||"";(n.includes("matchmaking")||n.includes("pvp-match"))&&T.removeChannel(p)})}catch(l){console.warn("[FriendMatch] cleanup canaux:",l)}s&&(await T.rpc("cancel_matchmaking",{p_user_id:s}).catch(()=>{}),await T.from("matchmaking_queue").delete().eq("user_id",s).then(()=>{},()=>{}))}catch{}await oi(e,t,"random",({deckId:s,formation:l,starters:p,subsRaw:n,gcCardsEnriched:o,gcDefs:f})=>{const v=_=>ar(e,t,s,l,p,n,_,f||[],i);if(!o.length){v([]);return}ni(e,o,v)})}async function ar(e,t,i,a,c,d,s=[],l=[],p,n){const{state:o,navigate:f,toast:v}=t,_=o.profile.id;let A=!1,r=null;et(e);try{await T.rpc("cancel_matchmaking",{p_user_id:_})}catch{}try{await T.from("matchmaking_queue").delete().eq("user_id",_)}catch{}const{data:u}=await T.from("mini_league_matches").select("*, home:users!home_id(id,pseudo,club_name), away:users!away_id(id,pseudo,club_name)").eq("id",p).single();if(!u){v("Match introuvable","error"),Ye(e),f("mini-league");return}if(u.home_id!==_&&u.away_id!==_){v("Tu ne fais pas partie de ce match","error"),Ye(e),f("mini-league");return}const I=u.home_id===_,z=I?u.away:u.home,C=I?u.away_id:u.home_id,H=(j,U)=>{var oe;e.innerHTML=`
      <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:20px;background:linear-gradient(180deg,#0a1628,#1a0a2e);padding:24px;text-align:center;color:#fff">
    ...
    </div>`,(b=document.getElementById("pvp-home"))==null||b.addEventListener("click",()=>{try{T.removeChannel(ce)}catch{}Ye(e),l("mini-league",n?{openLeagueId:n}:{})})}xe()}const Yt=[{emoji:"⚽",title:"Bienvenue dans Manager Wars !",color:"#1A6B3C",content:`<p>Tu es désormais un <strong>manager de football</strong> virtuel.</p>
    ...
    <p style="margin-top:12px;font-size:13px;color:#888">Tu peux revoir ce tutoriel depuis les paramètres à tout moment.</p>`}];function dr(e,t){let i=0;const a=document.createElement("div");a.id="tutorial-overlay",a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.65);z-index:9900;display:flex;align-items:center;justify-content:center;padding:16px";const c=()=>{var n,o,f;const s=Yt[i],l=i===Yt.length-1,p=Math.round((i+1)/Yt.length*100);a.innerHTML=`
    ...
    `,a.querySelectorAll("ul,ol").forEach(v=>{v.style.paddingLeft="20px",v.style.marginTop="6px",v.style.marginBottom="6px"}),a.querySelectorAll("li").forEach(v=>{v.style.marginBottom="4px"}),a.querySelectorAll("p").forEach(v=>{v.style.marginBottom="8px"}),(n=a.querySelector("#tuto-prev"))==null||n.addEventListener("click",()=>{i--,c()}),(o=a.querySelector("#tuto-next"))==null||o.addEventListener("click",()=>{l?d():(i++,c())}),(f=a.querySelector("#tuto-skip"))==null||f.addEventListener("click",()=>{confirm("Passer le tutoriel ? Tu pourras le revoir plus tard depuis les paramètres.")&&d()})},d=async()=>{a.remove(),e!=null&&e.id&&await T.from("users").update({tutorial_done:!0}).eq("id",e.id),t==null||t()};document.body.appendChild(a),c()}async function lr(e,t){!e||e.tutorial_done||dr(e,()=>t("boosters"))}const cr={GK:"#111111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function Et(e,t){return t&&Number(t==="GK"?e.note_g:t==="DEF"?e.note_d:t==="MIL"?e.note_m:e.note_a)||0}function Hi(e){return`<div style="width:6px;height:100%;background:${{normal:"#e0e0e0",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[e]||"#e0e0e0"};border-radius:4px 0 0 4px;flex-shrink:0;align-self:stretch;min-height:52px"></div>`}function Wt(e,t){const a=t?cr[t]||"#bbb":"#d0d0d0",c=e>0?e:t||"—";return`<div style="width:32px;height:32px;border-radius:6px;background:${a};display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:13px;font-weight:900;color:#fff;text-shadow:0 1px 2px rgba(0,0,0,0.4)">${c}</div>`}function Ui(e){const i=Zt(e);return i?`<div style="width:32px;height:32px;border-radius:6px;overflow:hidden;flex-shrink:0;display:flex;align-items:center;justify-content:center;background:#f0f0f0"><img src="${i}" style="width:100%;height:100%;object-fit:cover"></div>`:'<div style="width:32px;height:32px;border-radius:6px;background:#eee;display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:16px">🌍</div>'}function Vi(e){return e?`<div style="width:32px;height:32px;border-radius:6px;overflow:hidden;flex-shrink:0;display:flex;align-items:center;justify-content:center;background:#f0f0f0"><img src="${e}" style="width:28px;height:28px;object-fit:contain" onerror="this.parentElement.innerHTML='🏟️'"></div>`:'<div style="width:32px;height:32px;border-radius:6px;background:#f0f0f0;display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:16px">🏟️</div>'}async function pr(e,t){e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',await Ti(e,t)}async function Ti(e,t){const{state:i,toast:a}=t,{data:c}=await T.from("market_listings").select(`id, price, status, listed_at, seller_id,
      ...
      card:cards(id, card_type, current_note,
        player:players(id, firstname, surname_encoded, country_code, job, job2,
          ...
          clubs(encoded_name, logo_url)))`).eq("seller_id",i.profile.id).in("status",["active","sold"]).order("listed_at",{ascending:!1}).limit(100),s=(c||[]).filter(r=>r.seller_id!==i.profile.id),l=d||[];e.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    <!-- Header -->
    ...
    </div>`}function _(){const r=document.getElementById("mkt-content"),u=document.getElementById("mkt-filters");if(r){if(u.style.display=p==="buy"?"flex":"none",p==="buy"){const I=o(s);r.innerHTML=I.length?I.map(f).join(""):'<div style="text-align:center;color:#aaa;padding:40px">Aucune carte trouvée.</div>'}else{const I=l.filter(C=>C.status==="active").sort((C,H)=>new Date(H.listed_at)-new Date(C.listed_at)),z=l.filter(C=>C.status==="sold").sort((C,H)=>new Date(H.sold_at||H.listed_at)-new Date(C.sold_at||C.listed_at));r.innerHTML=(I.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:4px 0 6px;text-transform:uppercase;letter-spacing:1px">🟢 En vente (${I.length})</div>`+I.map(v).join(""):"")+(z.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:12px 0 6px;text-transform:uppercase;letter-spacing:1px">✅ Ventes réussies (${z.length})</div>`+z.map(v).join(""):"")+(!I.length&&!z.length?'<div style="text-align:center;color:#aaa;padding:40px">Aucune vente pour le moment.</div>':"")}r.querySelectorAll("[data-buy]").forEach(I=>I.addEventListener("click",()=>ur(I.dataset.buy,s,e,t))),r.querySelectorAll("[data-cancel]").forEach(I=>I.addEventListener("click",()=>gr(I.dataset.cancel,e,t)))}}e.querySelectorAll(".mkt-tab").forEach(r=>{r.addEventListener("click",()=>{p=r.dataset.tab,e.querySelectorAll(".mkt-tab").forEach(u=>{const I=u===r;u.style.background=I?"var(--green)":"#fff",u.style.color=I?"#fff":"var(--gray-600)",u.style.borderColor=I?"var(--green)":"var(--gray-200)"}),_()})});let A;["flt-name","flt-club","flt-country","flt-job","flt-rarity","flt-note1","flt-note2"].forEach(r=>{var u;(u=document.getElementById(r))==null||u.addEventListener("input",()=>{clearTimeout(A),A=setTimeout(_,250)})}),_()}async function ur(e,t,i,a){const{state:c,toast:d,refreshProfile:s}=a,l=t.find(o=>o.id===e);if(!l)return;const p=l.price;if((c.profile.credits||0)<p){d("Crédits insuffisants","error");return}fr(l,async()=>{const{error:o}=await T.rpc("buy_market_card",{p_listing_id:e,p_buyer_id:c.profile.id});if(o){d("Erreur achat : "+o.message,"error");return}await s();const f=document.getElementById("nav-credits");f&&(f.textContent=`💰 ${(c.profile.credits||0).toLocaleString("fr")}`),d("✅ Carte achetée !","success"),await Ti(i,a)})}function fr(e,t){var s;const i=(s=e.card)==null?void 0:s.player,a=i?`${i.firstname} ${i.surname_encoded}`:"cette carte",c=document.createElement("div");c.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",c.innerHTML=`
    ...
    </div>`,document.body.appendChild(c);const d=l=>{c.remove(),l&&t()};c.querySelector("#buy-cancel").addEventListener("click",()=>d(!1)),c.querySelector("#buy-ok").addEventListener("click",()=>d(!0)),c.addEventListener("click",l=>{l.target===c&&d(!1)})}async function gr(e,t,i){const{toast:a}=i,{data:c}=await T.from("market_listings").select("card_id").eq("id",e).single();if(await T.from("market_listings").update({status:"cancelled"}).eq("id",e),c!=null&&c.card_id){const{count:d}=await T.from("market_listings").select("id",{count:"exact",head:!0}).eq("card_id",c.card_id).eq("status","active");d||await T.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",c.card_id)}a("Annonce retirée","success"),Ti(t,i)}async function mr(e,{state:t,navigate:i,toast:a}){e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:c}=await T.from("users").select("id,pseudo,club_name,trophies_top1,trophies_top2,trophies_top3,wins,level").order("trophies_top1",{ascending:!1}).limit(100);e.innerHTML=`
  <div class="page">
    <div class="page-header">
  ...
  `}async function xr(e,{state:t,navigate:i,toast:a}){const c=t.profile;if(!c)return;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:d}=await T.from("matches").select(`id,home_id,away_id,home_score,away_score,status,mode,winner_id,created_at,played_at,
      home:users!home_id(pseudo,club_name),
      away:users!away_id(pseudo,club_name)`).or(`home_id.eq.${c.id},away_id.eq.${c.id}`).order("created_at",{ascending:!1}).limit(50),s={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},l=(d||[]).filter(o=>o.status==="finished"),p=(d||[]).filter(o=>o.status==="in_progress");function n(o){const f=o.home_id===c.id;f?o.home_score:o.away_score,f?o.away_score:o.home_score;const v=o.winner_id===c.id,_=o.home_score===o.away_score&&o.status==="finished",A=o.status!=="finished"?"…":_?"N":v?"V":"D",r=o.status!=="finished"||_?"#888":v?"#1A6B3C":"#c0392b";let u=s[o.mode]||o.mode;o.away_id===null&&!u.startsWith("IA")&&(u="IA");const z=o.home_id===c.id?o.away:o.home;let C;o.away_id===null?C=u:z?C=`${z.club_name||z.pseudo} (${z.pseudo})`:C="Adversaire";let H="";o.status==="in_progress"&&Date.now()-new Date(o.created_at).getTime()>3600*1e3&&(H=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const B=new Date(o.created_at),j=B.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+B.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),U=o.status==="finished"?`${o.home_score} - ${o.away_score}`:`${o.home_score||0} - ${o.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
  ...
  </div>`}zn(Sn);const je={user:null,profile:null,page:"home",params:{}};function Mt(e,t="info",i=3e3){const a=document.getElementById("toast");a&&(a.textContent=e,a.className=`show ${t}`,clearTimeout(a._t),a._t=setTimeout(()=>{a.className=""},i))}function yr(e,t,i=""){document.getElementById("modal-title").textContent=e,document.getElementById("modal-body").innerHTML=t,document.getElementById("modal-footer").innerHTML=i,document.getElementById("modal-overlay").classList.remove("hidden")}function mi(){document.getElementById("modal-overlay").classList.add("hidden")}async function Ct(){if(!je.user)return;const{data:e}=await T.from("users").select("*").eq("id",je.user.id).single();e&&(je.profile=e)}function Ft(e,t={}){je.page=e,je.params=t,Tn()}async function Tn(){var a,c,d,s;const e=document.getElementById("page-content");if(!e)return;document.querySelectorAll(".bottom-nav a").forEach(l=>{l.classList.toggle("active",l.dataset.page===je.page)});const t=document.getElementById("nav-credits");t&&je.profile&&(t.textContent=`💰 ${(je.profile.credits||0).toLocaleString("fr")}`);const i={state:je,navigate:Ft,toast:Mt,openModal:yr,closeModal:mi,refreshProfile:Ct};switch(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽</div>',je.page){case"home":await Mi(e,i);break;case"collection":await Jn(e,i);break;case"decks":await li(e,i);break;case"boosters":await po(e,i);break;case"match":{const l=je.params&&je.params.matchMode||"vs_ai_easy";l==="random"?await wn(e,i):l==="friend"?await Uo(e,i,(a=je.params)==null?void 0:a.friendId,(c=je.params)==null?void 0:c.friendName):l==="mini-league"?await Oi(e,i,(d=je.params)==null?void 0:d.mlMatchId,(s=je.params)==null?void 0:s.leagueId):await To(e,i);break}case"market":await pr(e,i);break;case"rankings":await mr(e,i);break;case"matches":await xr(e,i);break;case"friends":await Bn(e,i);break;case"mini-league":await Yo(e,i);break;case"match-mini-league":{const l=je.params||{};await Oi(e,i,l.mlMatchId,l.leagueId);break}default:await Mi(e,i)}}function br(){var a;const e=document.getElementById("app"),t=je.profile;if(!t)return;const i="/manager-wars/icons/";e.innerHTML=`
    <nav class="top-nav">
  ...
  `,document.querySelectorAll(".bottom-nav a").forEach(c=>{c.addEventListener("click",d=>{d.preventDefault(),Ft(c.dataset.page)})}),document.getElementById("nav-logo").addEventListener("click",()=>Ft("home")),document.getElementById("nav-credits").addEventListener("click",()=>Ft("boosters")),(a=document.getElementById("journal-btn"))==null||a.addEventListener("click",()=>hr())}async function hr(){const{data:e}=await T.from("patch_notes").select("*").eq("is_published",!0).order("published_at",{ascending:!1}).limit(20),t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9000;display:flex;align-items:center;justify-content:center;padding:16px";const i=(e||[]).map(a=>{const c=new Date(a.published_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"long",year:"numeric"});return`<div style="padding:14px 0;border-bottom:1px solid #f0f0f0">
    ...
    </div>`,document.body.appendChild(t),t.querySelector("#journal-close").addEventListener("click",()=>t.remove()),t.addEventListener("click",a=>{a.target===t&&t.remove()})}async function vr(){document.documentElement.setAttribute("data-theme","light"),document.getElementById("modal-overlay").addEventListener("click",i=>{i.target===i.currentTarget&&mi()}),document.getElementById("modal-close").addEventListener("click",mi);const{data:{session:e}}=await T.auth.getSession();if(!e){Ki(),Li(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:Mt});return}je.user=e.user,await Ct(),Ki();try{const{data:i}=await T.from("formation_links_overrides").select("formation, links"),a={};(i||[]).forEach(c=>{a[c.formation]=c.links}),Ln(a)}catch(i){console.warn("Impossible de charger les overrides de formation:",i)}if(!je.profile){Mn(document.getElementById("app"),{state:je,navigate:async()=>{await Ct(),di()},toast:Mt,refreshProfile:Ct});return}const t=Array.isArray(je.profile.pending_boosters)?je.profile.pending_boosters:[];if(!je.profile.onboarding_done&&t.length>0){_o(document.getElementById("app"),{state:je,navigate:()=>di(),toast:Mt,refreshProfile:Ct});return}di(),setTimeout(()=>lr(je.profile,Ft),800),T.auth.onAuthStateChange(async(i,a)=>{i==="SIGNED_OUT"&&(je.user=null,je.profile=null,document.getElementById("app").innerHTML="",Li(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:Mt}))})}function wr(){return Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight)}function ri(){const e=document.getElementById("app");e&&(e.style.height=wr()+"px")}window.addEventListener("resize",ri);window.addEventListener("orientationchange",()=>setTimeout(ri,150));window.visualViewport&&window.visualViewport.addEventListener("resize",ri);function di(){const e=()=>{var i;(i=je.user)!=null&&i.id&&T.from("users").update({last_seen_at:new Date().toISOString()}).eq("id",je.user.id).then(()=>{})};e(),window._presencePingInterval&&clearInterval(window._presencePingInterval),window._presencePingInterval=setInterval(e,6e4);const t=document.getElementById("app");t.style.display="flex",t.style.flexDirection="column",ri(),br(),Tn()}function Ki(){const e=document.getElementById("app-loader"),t=document.getElementById("app");t&&(t.style.display=""),e&&(e.classList.add("zoom-out"),setTimeout(()=>e.style.display="none",500))}function An(e){var a;const t=document.getElementById("app-loader");if(t&&(t.style.display="none"),document.getElementById("boot-error"))return;const i=document.createElement("div");i.id="boot-error",i.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:99999;gap:16px;color:#fff;padding:24px;text-align:center",i.innerHTML=`
    <div style="font-size:42px">📡</div>
    <div style="font-size:18px;font-weight:900">Connexion impossible</div>
    <div style="font-size:13px;color:rgba(255,255,255,0.6);max-width:280px">${e||"Le chargement a échoué. Vérifie ta connexion et réessaie."}</div>
    <button id="boot-retry" style="margin-top:8px;padding:12px 30px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer">Réessayer</button>`,document.body.appendChild(i),(a=document.getElementById("boot-retry"))==null||a.addEventListener("click",()=>window.location.reload())}vr().catch(e=>{console.error("Échec du démarrage:",e),An()});setTimeout(()=>{const e=document.getElementById("app-loader");e&&e.style.display!=="none"&&!e.classList.contains("zoom-out")&&!document.getElementById("boot-error")&&An("Le serveur met trop de temps à répondre.")},12e3);
