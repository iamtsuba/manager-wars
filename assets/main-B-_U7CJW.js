...
import{s as _,q as Ut,t as Ri,F as Oi,o as tt,n as ti,g as At,r as Ie,m as zt,u as zi,j as Hi,v as ro,w as ao,T as Li,p as so,x as lo,d as co}from"./special-cards-BY0MTcrl.js";const po="/";function ln(e,{navigate:t,toast:i}){let n="login";const r=()=>{var a,s,d,c,l,m,b;const o=n==="login";if(e.innerHTML=`
    <div style="
    ...
    ">
      ...
      <div style="margin-bottom:28px;text-align:center;animation:authFadeIn .5s ease">
        <img src="${po}icons/logo-withname.png" alt="Manager Wars" style="height:72px;width:auto;filter:drop-shadow(0 4px 16px rgba(212,160,23,0.4))">
  ...
  `;function s(){var g;const c=document.getElementById("logo-preview"),l=document.getElementById("logo-initials"),m=((g=document.getElementById("setup-club"))==null?void 0:g.value)||"MW",b=m.trim().split(" ").filter(Boolean),y=b.length>=2?(b[0][0]+b[1][0]).toUpperCase():m.slice(0,2).toUpperCase();c&&(c.style.background=a,c.style.borderColor=o),l&&(l.textContent=y,l.style.color=o)}document.getElementById("color1").addEventListener("input",c=>{o=c.target.value,document.getElementById("swatch1").style.background=o,s()}),document.getElementById("color2").addEventListener("input",c=>{a=c.target.value,document.getElementById("swatch2").style.background=a,s()});function d(c){document.querySelectorAll(".setup-step").forEach(l=>l.classList.remove("active")),document.getElementById(`step-${c}`).classList.add("active"),document.getElementById("step-num").textContent=c,document.getElementById("progress-fill").style.width=`${Math.round(c/3*100)}%`,c===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("step1-error");if(l.textContent="",c.length<3){l.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:m}=await _.from("users").select("id").eq("pseudo",c).maybeSingle();if(m){l.textContent="Ce pseudo est déjà pris.";return}d(2)}),document.getElementById("step2-back").addEventListener("click",()=>d(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const c=document.getElementById("setup-club").value.trim(),l=document.getElementById("step2-error");if(l.textContent="",c.length<2){l.textContent="Nom trop court (min. 2 caractères).";return}const{data:m}=await _.from("users").select("id").eq("club_name",c).maybeSingle();if(m){l.textContent="Ce nom de club est déjà pris.";return}d(3)}),document.getElementById("step3-back").addEventListener("click",()=>d(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("setup-club").value.trim(),m=document.getElementById("step3-error"),b=document.getElementById("step3-finish");m.textContent="",b.disabled=!0,b.textContent="Création en cours…";try{const{error:y}=await _.from("users").insert({id:t.user.id,pseudo:c,club_name:l,club_color1:o,club_color2:a,credits:1e4});if(y)throw y;await fo(t.user.id),await r(),n(`Bienvenue ${c} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(y){m.textContent=y.message,b.disabled=!1,b.textContent="🚀 Créer mon profil !"}})}async function fo(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await _.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const mo="modulepreload",go=function(e){return"/"+e},cn={},Ln=function(t,i,n){let r=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),s=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));r=Promise.allSettled(i.map(d=>{if(d=go(d),d in cn)return;cn[d]=!0;const c=d.endsWith(".css"),l=c?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${l}`))return;const m=document.createElement("link");if(m.rel=c?"stylesheet":mo,c||(m.as="script"),m.crossOrigin="",m.href=d,s&&m.setAttribute("nonce",s),document.head.appendChild(m),c)return new Promise((b,y)=>{m.addEventListener("load",b),m.addEventListener("error",()=>y(new Error(`Unable to preload CSS for ${d}`)))})}))}function o(a){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=a,window.dispatchEvent(s),!s.defaultPrevented)throw a}return r.then(a=>{for(const s of a||[])s.status==="rejected"&&o(s.reason);return t().catch(o)})},ii="#1A6B3C",ni="#cc2222",yo="#D4A017",pn="#888";async function xo(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      ${b.map(({friendshipId:y,friend:g})=>bo(g,y)).join("")}
    </div>`,c.querySelectorAll("[data-stats]").forEach(y=>{y.addEventListener("click",()=>ho(e,y.dataset.stats,y.dataset.friendName))}),c.querySelectorAll("[data-match]").forEach(y=>{y.addEventListener("click",async()=>{const g=y.dataset.friendId,p=y.dataset.friendName;if(typeof n!="function"){t("Erreur navigation","error");return}const u=e.user.id,{data:x}=await _.from("friend_match_invites").select("id").eq("inviter_id",g).eq("invitee_id",u).eq("status","pending").order("created_at",{ascending:!1}).limit(1).maybeSingle(),h=!!x;console.log("[Friends] clic match",{fid:g,fname:p,isAccepting:h}),n("match",{matchMode:"friend",friendId:g,friendName:p,isAccepting:h})})})}function bo(e,t){const i=e.club_name||e.pseudo||"?",n=e.pseudo||"",r=(n||i).slice(0,2).toUpperCase(),o=e.club_color2||ii,a=e.club_color1||"#ffffff",s=e.last_seen_at?new Date(e.last_seen_at):null,d=s&&Date.now()-s.getTime()<3*60*1e3;return`
    <div style="display:flex;align-items:center;gap:12px;background:var(--tile-bg);border-radius:12px;padding:12px 14px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
      ...
      <div style="display:flex;gap:8px;flex-shrink:0">
        ...
        <button data-stats="${e.id}" data-friend-name="${i}" title="Voir les stats"
          style="width:38px;height:38px;border-radius:50%;border:2px solid ${yo};background:var(--tile-bg);font-size:16px;cursor:pointer;display:flex;align-items:center;justify-content:center">📊</button>
      </div>
    </div>`}function vo(e,t){const i=Ui();i.innerHTML=`
    <div class="popup-box">
    ...
    </style>`}const Sn="mw_sound_volume";function Vi(){const e=localStorage.getItem(Sn);if(e===null)return 100;const t=parseInt(e,10);return Number.isFinite(t)?Math.max(0,Math.min(100,t)):100}function _o(e){localStorage.setItem(Sn,String(Math.max(0,Math.min(100,Math.round(e)))))}function Yi(){return Vi()===0}function fi(e){return Math.max(0,Math.min(1,e*(Vi()/100)))}function Wi(e,t=1){if(Yi())return null;try{const i=new Audio(e);return i.volume=fi(t),i.play().catch(()=>{}),i}catch{return null}}let gt=null,Mi=null,un=.3;function An(e,t=.3){if(un=t,gt&&Mi===e&&!gt.paused){gt.volume=fi(un);return}if(Gt(),!Yi())try{const i=new Audio(e);i.loop=!0,i.volume=fi(t),i.play().catch(()=>{}),gt=i,Mi=e}catch{}}function Gt(){if(gt)try{gt.pause(),gt.currentTime=0}catch{}gt=null,Mi=null}let Xt=null;function zn(e,t=.6){if(Ue(),!Yi())try{const i=new Audio(e);i.volume=fi(t),i.play().catch(()=>{}),Xt=i}catch{}}function Ue(){if(Xt)try{Xt.pause(),Xt.currentTime=0}catch{}Xt=null}const wo="2026.07.23-1123";async function fn(e,{state:t,navigate:i,toast:n}){var o,a;const r=t.profile;r&&(e.innerHTML=`
  <style>
    .home-dark {
      min-height: 100%;
  ...
  <div class="home-dark" id="home-dark">
    <div class="home-inner">
      ...
      <div class="home-footer">
        ...
        <div style="font-size:10px;color:rgba(255,255,255,0.25);font-family:monospace">build ${wo}</div>`:""}
      </div>

    </div>
  </div>`,requestAnimationFrame(()=>{var m,b,y,g,p;const s=((m=window.visualViewport)==null?void 0:m.height)||window.innerHeight,d=((b=document.querySelector(".top-nav"))==null?void 0:b.offsetHeight)||56,c=((y=document.querySelector(".bottom-nav"))==null?void 0:y.offsetHeight)||60;if(e.querySelector(".home-inner")){const u=s-d-c;e.querySelector(".home-dark").style.minHeight=u+"px"}if(window.innerWidth<768){const u=((g=e.querySelector(".home-hero"))==null?void 0:g.offsetHeight)||60,x=e.querySelector(".ranked-tile");e.querySelector(".play-grid");const h=((p=e.querySelector(".home-footer"))==null?void 0:p.offsetHeight)||44,v=["friend-requests-banner","match-invite-banner","ongoing-match-banner"].reduce((B,ee)=>{var te;return B+(((te=document.getElementById(ee))==null?void 0:te.offsetHeight)||0)},0),$=12*5,f=s-d-c-u-h-v-$-32,M=Math.max(80,Math.round(f*.28)),k=Math.max(160,Math.round(f*.72)),I=Math.floor((k-10)/2);x&&(x.style.minHeight=x.style.maxHeight=M+"px"),e.querySelectorAll(".play-tile").forEach(B=>{B.style.minHeight=B.style.height=I+"px"}),e.querySelectorAll(".play-tile .play-icon").forEach(B=>{B.style.height=Math.round(I*.55)+"px"})}}),(o=document.getElementById("nav-rankings"))==null||o.addEventListener("click",()=>i("rankings")),(a=document.getElementById("nav-matches"))==null||a.addEventListener("click",()=>i("matches")),e.querySelectorAll("[data-action]").forEach(s=>{s.addEventListener("click",()=>{s.style.transform="scale(.96)",setTimeout(()=>s.style.transform="",180);const d=s.dataset.action;if(d==="match-ai"){To(i);return}if(d==="match-random"){i("match",{matchMode:"random"});return}if(d==="match-friend"){i("friends");return}if(d==="mini-league"){i("mini-league");return}if(d==="ranked"){i("ranked");return}n("Bientôt disponible","info")})}),document.getElementById("logout-btn").addEventListener("click",async()=>{await _.auth.signOut(),window.location.reload()}),Cn(t,n),Lo(t,n,i),Mn(t,n,i),ko(t,n))}async function ko(e,t){const i=e.profile.id,{data:n}=await _.from("mini_league_members").select("league_id, prize_amount, mini_leagues(name)").eq("user_id",i).eq("prize_claimed",!1).gt("prize_amount",0);if(!(n!=null&&n.length))return;const r=n.reduce((a,s)=>a+(s.prize_amount||0),0),o=document.createElement("div");o.className="modal-overlay",o.style.zIndex="2200",o.innerHTML=`<div class="modal" style="max-width:400px">
    ...
    </div>`}).join(""),n.querySelectorAll("[data-resume]").forEach(d=>{d.addEventListener("click",async()=>{const c=document.getElementById("page-content")||document.getElementById("app");if(d.dataset.mini==="1"){const{data:l}=await _.from("mini_league_matches").select("id, league_id").eq("match_id",d.dataset.resume).single();l?i("match-mini-league",{mlMatchId:l.id,leagueId:l.league_id}):i("mini-league")}else{const{resumePvpMatch:l}=await Ln(async()=>{const{resumePvpMatch:m}=await Promise.resolve().then(()=>zr);return{resumePvpMatch:m}},void 0);l(c,{state:e,navigate:i,toast:t,openModal:null,closeModal:null,refreshProfile:null},d.dataset.resume)}})}),n.querySelectorAll("[data-abandon]").forEach(d=>{d.addEventListener("click",()=>{Eo(async()=>{await $o(d.dataset.abandon,d.dataset.opp,r),t("Match abandonné (défaite 3-0)","info"),Mn(e,t,i)})})})}async function $o(e,t,i){Gt();const{data:n}=await _.from("matches").select("home_id, away_id, game_state, mode").eq("id",e).single();if(!n)return;const r=n.home_id===i,o=r?0:3,a=r?3:0,s=n.game_state||{};s.p1Score=o,s.p2Score=a,s.phase="finished",s.forfeit=!0,await _.from("matches").update({status:"finished",forfeit:!0,winner_id:t,home_score:o,away_score:a,game_state:s}).eq("id",e),n.mode==="mini_league"&&await _.from("mini_league_matches").update({status:"finished",home_score:o,away_score:a}).eq("match_id",e)}function Eo(e){const t=document.createElement("div");t.className="modal-overlay",t.innerHTML=`<div class="modal" style="max-width:340px">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    ${Ie(e,{width:140})}
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),c=(s||[]).filter(E=>E.card_type==="player"&&E.player),l=(s||[]).filter(E=>E.card_type==="game_changer"),m=(s||[]).filter(E=>E.card_type==="formation"),b=(s||[]).filter(E=>E.card_type==="stadium"),{data:y}=await _.from("gc_definitions").select("name,gc_type,color,effect,image_url"),g={};(y||[]).forEach(E=>{g[E.name]=E});const{data:p}=await _.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),u={};(p||[]).forEach(E=>{u[E.id]=E}),b.forEach(E=>{E.stadium_def&&(u[E.stadium_id]=E.stadium_def)});const x=Object.keys(Oi),h=Object.keys(je),v={};c.forEach(E=>{const F=E.player.id;v[F]=(v[F]||0)+1}),new Set(Object.keys(v).map(E=>String(E)));const $=new Set(m.map(E=>E.formation)),f=new Set(l.map(E=>E.gc_type));let M="player",k="Tous",I="",B=!1;function ee(){return[...c].sort((E,F)=>{const H=di.indexOf(E.player.job),R=di.indexOf(F.player.job);return H!==R?H-R:(E.player.surname_real||"").localeCompare(F.player.surname_real||"")})}function te(){return[...d||[]].sort((E,F)=>{const H=di.indexOf(E.job),R=di.indexOf(F.job);return H!==R?H-R:(E.surname_real||"").localeCompare(F.surname_real||"")})}function ne(){return ee().filter(E=>{const F=E.player,H=k==="Tous"||F.job===k,R=!I||`${F.firstname} ${F.surname_real}`.toLowerCase().includes(I);return H&&R})}function z(){return te().filter(E=>{const F=k==="Tous"||E.job===k,H=!I||`${E.firstname} ${E.surname_real}`.toLowerCase().includes(I);return F&&H})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:0 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none;align-items:center"></div>
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",F=>{I=F.target.value.toLowerCase(),K()}),e.querySelectorAll(".filter-btn").forEach(F=>{F.addEventListener("click",()=>{k=F.dataset.job,S(),K()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{B=!B,S(),K()})):(E.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{B=!B,S(),K()})))}function K(){const E=document.getElementById("col-grid");E&&(M==="player"?Y(E):M==="formation"?pe(E):M==="stadium"?V(E):U(E))}function J(E,F,H,R,re,q=0){const G=document.getElementById("col-grid"),se=document.getElementById("col-big");if(!G||!se)return;var de=0;function he(){var ct;const Te=window.innerWidth>=768,ve=document.getElementById("col-big"),Se=document.getElementById("col-gap"),Ae=((ct=document.getElementById("col-grid"))==null?void 0:ct.parentElement)||null,Be=ve?ve.closest(".page"):null;if(ve&&Ae&&Be){let $e=0;Array.from(Be.children).forEach(function(j){j!==ve&&j!==Ae&&j!==Se&&($e+=j.offsetHeight)});const Ce=Math.max(0,Be.clientHeight-$e),qe=Math.round(Ce*(q/100)),Ge=Math.max(0,Ce-qe),w=Te?50/71:45/63,L=Math.round(Ge*w),A=Math.max(0,Ge-L);Se&&(Se.style.height=qe+"px"),ve.style.flex="none",ve.style.height=L+"px",ve.style.minHeight="0",Ae.style.flex="none",Ae.style.height=A+"px",Ae.style.minHeight="0",Ae.style.display="flex",Ae.style.overflow="hidden"}const ke=document.getElementById("col-grid");ke&&(ke.style.height="100%",ke.style.flexShrink="0",ke.style.overflowX="auto",ke.style.overflowY="hidden",ke.style.alignItems="center",ke.style.width="100%"),se.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:center center">'+F(E[de])+"</div>";var wt=se.querySelector("[data-card-id],[data-form-id],[data-gc-id]");wt&&wt.addEventListener("click",function(){R(E[de])}),requestAnimationFrame(function(){var $e=document.getElementById("big-card-inner");if(!(!$e||!se)){var Ce=se.clientHeight,qe=se.clientWidth-16,Ge=$e.offsetHeight,w=$e.offsetWidth;if(Ge>0&&w>0&&Ce>40){var L=Math.min(Ce/Ge,qe/w);$e.style.transform="scale("+L.toFixed(3)+")",$e.style.transformOrigin="center center"}}}),G.innerHTML=E.map(function($e,Ce){var qe=Ce===de,Ge="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+(qe?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+Ce+'" style="'+Ge+'">'+H($e,qe)+"</div>"}).join(""),G.querySelectorAll(".col-mini-item").forEach(function($e){$e.addEventListener("click",function(){de=Number($e.dataset.idx),he(),$e.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})}),requestAnimationFrame(function(){var $e=ke?ke.clientHeight:0,Ce=G.querySelector(".col-mini-item > div");if(!(!$e||!Ce)){var qe=Ce.style.zoom;Ce.style.zoom="1";var Ge=Ce.offsetHeight;if(Ce.style.zoom=qe,!(Ge<=0)){var w=$e/Ge;G.querySelectorAll(".col-mini-item > div").forEach(function(L){L.style.zoom=w.toFixed(4)})}}})}he()}function ue(E,F){var H=window.innerWidth>=768?.76:.54,R;if(!E||E._fake){var re=E?E.player:null;if(!re)return"";R=yn(re)}else R=Ii(E,"");var q=E&&!E._fake?v[E.player&&E.player.id]||1:0,G=q>0?'<div style="position:absolute;top:6px;right:6px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:11px;font-weight:900;padding:2px 7px;z-index:3;box-shadow:0 1px 4px rgba(0,0,0,0.4)">×'+q+"</div>":"";return'<div style="display:inline-block;position:relative;zoom:'+H+';pointer-events:none;line-height:0">'+G+R+"</div>"}function fe(E,F,H){var R=H>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+H+"</div>":"",re=!!F,q=zi(E,zt[E],{width:160});return re||(q='<div style="filter:grayscale(1);opacity:0.5">'+q+"</div>"),"<div "+(F?'data-form-id="'+F.id+'"':"")+' style="position:relative;cursor:pointer">'+R+q+"</div>"}function C(E,F){var H=window.innerWidth>=768?.76:.54,R=zi(E,zt[E],{width:140});return F||(R='<div style="filter:grayscale(1);opacity:0.45">'+R+"</div>"),'<div style="display:inline-block;zoom:'+H+';line-height:0;pointer-events:none">'+R+"</div>"}function Y(E){if(B){const F=z();if(!F.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucun joueur.</div>';return}const H=F.map(R=>c.find(re=>re.player.id===R.id)||{_fake:!0,player:R,id:"fake-"+R.id});J(H,R=>R._fake?yn(R.player):Ii(R,""),R=>R._fake?ue({player:R.player,id:"x",_fake:!0}):ue(R),R=>{R._fake||xn(R,c,v,t)})}else{const F=ne();if(!F.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const H={};F.forEach(re=>{const q=re.player.id;H[q]||(H[q]=[]),H[q].push(re)});const R=Object.values(H).map(re=>Mo(re));J(R,re=>{const q=v[re.player.id]||1,G=q>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${q}</div>`:"",de=c.filter(he=>he.player.id===re.player.id&&he.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Ii(re,G+de)},re=>ue(re),re=>xn(re,c,v,t))}}function pe(E){const F=B?x:[...$];if(!F.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const H=F.map(R=>({formation:R,card:m.find(re=>re.formation===R)||null,owned:$.has(R)}));J(H,({formation:R,card:re,owned:q})=>fe(R,q?re:null,q?m.filter(G=>G.formation===R).length:0),({formation:R,owned:re})=>C(R,re),({card:R,owned:re})=>{re&&R&&qo(R,m,t,o)},"#1A6B3C",5)}function U(E){const F=Object.keys(g),H=B?F.length?F:h:[...f];if(!H.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const R=H.map(re=>({type:re,gc:je[re]||{icon:"⚡",desc:""},def:g[re]||null,owned:f.has(re),card:l.find(q=>q.gc_type===re)||null}));J(R,({type:re,gc:q,def:G,owned:se,card:de})=>{var ke;const he=(G==null?void 0:G.name)||re,Te=se?l.filter(wt=>wt.gc_type===re).length:0,ve=Te>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${Te}</div>`:"",Se=(G==null?void 0:G.effect)||q.desc||"",Ae=G!=null&&G.image_url?`/icons/${G.image_url}`:((ke=G==null?void 0:G.club)==null?void 0:ke.logo_url)||(G!=null&&G.country_code?`https://flagsapi.com/${G.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null);let Be=tt(he,Ae,q.icon,Se,{width:160,onClick:se});return se||(Be=`<div style="filter:grayscale(1);opacity:0.5">${Be}</div>`),`<div ${se&&de?`data-gc-id="${de.id}" data-gc-type="${re}"`:""} style="position:relative">${ve}${Be}</div>`},({type:re,gc:q,def:G,owned:se})=>{const de=window.innerWidth>=768?.76:.54,he=(G==null?void 0:G.name)||re,Te=(G==null?void 0:G.effect)||q.desc||"",ve=G!=null&&G.image_url?`/icons/${G.image_url}`:null;let Se=tt(he,ve,q.icon,Te,{width:140});return se||(Se=`<div style="filter:grayscale(1);opacity:0.45">${Se}</div>`),`<div style="display:inline-block;zoom:${de};line-height:0;pointer-events:none">${Se}</div>`},({type:re,owned:q,def:G})=>{q&&Bo(re,G,o)},"#7a28b8",5)}function V(E){const F="#4FC3F7",H="/";if(!b.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Stade.<br><small>Ouvre un booster Stade !</small></div>';return}const R={};b.forEach(q=>{const G=q.stadium_id||"?";(R[G]=R[G]||[]).push(q)});const re=Object.entries(R).map(([q,G])=>({sid:q,def:u[q]||null,count:G.length,card:G[0]}));J(re,({def:q,count:G})=>{var Se,Ae;const se=(q==null?void 0:q.name)||"?",de=((Se=q==null?void 0:q.club)==null?void 0:Se.encoded_name)||(q==null?void 0:q.country_code)||"—",he=q!=null&&q.image_url?`${H}icons/${q.image_url}`:((Ae=q==null?void 0:q.club)==null?void 0:Ae.logo_url)||(q!=null&&q.country_code?`https://flagsapi.com/${q.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),Te=G>1?`<div style="position:absolute;top:8px;right:8px;background:#333;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${G}</div>`:"",ve=`${de}<br>+10 ⭐ joueurs alliés`;return`<div style="position:relative">${Te}${ti(se,he,ve,{width:160})}</div>`},({def:q})=>{var ve,Se;const G=window.innerWidth>=768?.76:.54,se=(q==null?void 0:q.name)||"?",de=((ve=q==null?void 0:q.club)==null?void 0:ve.encoded_name)||(q==null?void 0:q.country_code)||"—",he=q!=null&&q.image_url?`${H}icons/${q.image_url}`:((Se=q==null?void 0:q.club)==null?void 0:Se.logo_url)||(q!=null&&q.country_code?`https://flagsapi.com/${q.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),Te=`${de}<br>+10 ⭐`;return`<div style="display:inline-block;zoom:${G};line-height:0;pointer-events:none">${ti(se,he,Te,{width:140})}</div>`},null,F,5)}e.querySelectorAll(".col-tab-btn").forEach(E=>{E.addEventListener("click",()=>{M=E.dataset.tab,k="Tous",I="",B=!1,e.querySelectorAll(".col-tab-btn").forEach(F=>{const H=F.dataset.tab===M;F.style.borderBottomColor=H?"var(--green)":"transparent",F.style.color=H?"var(--green)":"var(--tile-fg-dim)"}),S(),K()})}),S(),K()}function Bo(e,t,i){const n=je[e]||{icon:"⚡",desc:"Effet spécial."},r=(t==null?void 0:t.name)||e,o=(t==null?void 0:t.effect)||n.desc,a=t!=null&&t.image_url?`/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
      ${tt(r,a,n.icon,o,{width:200})}
      <div style="background:#fff3cd;border-radius:10px;padding:10px 14px;width:100%">
        <div style="font-size:12px;color:#856404">⚠️ Cette carte est à <b>usage unique</b>. Une fois jouée en match, elle est définitivement supprimée de ta collection.</div>
      </div>
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const ci=1e3;function qo(e,t,i,n){var g,p,u;const{state:r,toast:o,closeModal:a,navigate:s,refreshProfile:d}=i,c=e.formation,l={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function m(){const x=zt[c]||{},h=Oi[c]||[],v=290,$=360,f=20;function M(I){const B=x[I];return B?{x:B.x*v,y:B.y*$}:null}let k=`<svg width="${v}" height="${$}" viewBox="0 0 ${v} ${$}" xmlns="http://www.w3.org/2000/svg">`;for(const[I,B]of h){const ee=M(I),te=M(B);!ee||!te||(k+=`<line x1="${ee.x}" y1="${ee.y}" x2="${te.x}" y2="${te.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const I of Object.keys(x)){const B=M(I);if(!B)continue;const ee=I.replace(/\d+/,""),te=l[ee]||"#555";k+=`<circle cx="${B.x}" cy="${B.y}" r="${f}" fill="${te}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,k+=`<text x="${B.x}" y="${B.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${ee}</text>`}return k+="</svg>",k}const b=t.filter(x=>x.formation===c);b.length;const y=!e.is_for_sale;n(`Formation ${c}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",l.note_g],["DEF",l.note_d],["MIL",l.note_m],["ATT",l.note_a]].map(([U,V])=>{const E=Ti[U],F=U==="GK"?"#fff":E,H=U===l.job||U===l.job2,R=(Number(V)||0)+(H&&f>0?f:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
    ...
    ${ne}
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(fe=document.getElementById("close-detail"))==null||fe.addEventListener("click",s);let z=new Set;const S=l.rarity==="pepite"?1.3:l.rarity==="papyte"?.7:1;function K(){let U=0;return document.querySelectorAll(".expl-check:checked").forEach(V=>{if(V.dataset.id===e.id)return;const F=Number(V.dataset.evo)||0,H=Number(V.dataset.note)||0;U+=H+F}),Math.round(U*S)}const J=()=>{const U=z.size,V=document.getElementById("sell-action-panel");if(!V)return;V.style.display=U>0?"block":"none",document.getElementById("sell-selected-count").textContent=U;const E=document.getElementById("evolve-btn");if(E){const F=K();E.textContent=F>0?`⬆️ Évoluer (+${F})`:"⬆️ Évoluer"}};document.querySelectorAll(".expl-check").forEach(U=>{U.addEventListener("change",()=>{const V=U.dataset.id;U.checked?z.add(V):z.delete(V);const E=U.closest(".exemplaire-row");if(E){const F=E.querySelector(".expl-sel-overlay"),H=E.querySelector(".expl-sel-check");F&&(F.style.display=U.checked?"block":"none"),H&&(H.style.display=U.checked?"flex":"none"),E.style.transform=U.checked?"scale(1.05)":"scale(1)"}J()})}),document.querySelectorAll(".exemplaire-row").forEach(U=>{U.addEventListener("click",V=>{if(V.target.tagName==="INPUT")return;const E=U.querySelector(".expl-check");E&&!E.disabled&&(E.checked=!E.checked,E.dispatchEvent(new Event("change")))})}),(C=document.getElementById("evolve-btn"))==null||C.addEventListener("click",async()=>{if(b<=1)return;const U=[...z];if(!U.length)return;if(z.has(e.id)){const G=document.createElement("div");G.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",G.innerHTML=`
        ...
        </div>`,document.body.appendChild(G),G.querySelector("#err-close").addEventListener("click",()=>G.remove()),G.addEventListener("click",se=>{se.target===G&&G.remove()});return}const V=U.filter(G=>G!==e.id);if(!V.length){o("Sélectionne des copies à sacrifier","warning");return}const E=V.reduce((G,se)=>{const de=document.querySelector(`.expl-check[data-id="${se}"]`),he=de&&Number(de.dataset.evo)||0,Te=de&&Number(de.dataset.note)||0;return G+Te+he},0),F=Math.round(E*S),H=l.rarity==="pepite"?" (+30% pépite ✨)":l.rarity==="papyte"?" (-30% papyte)":"";if(!await new Promise(G=>{const se=document.createElement("div");se.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",se.innerHTML=`
        <div style="background:var(--tile-bg);border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center;box-shadow:0 16px 48px rgba(0,0,0,0.3)">
          ...
          <div style="background:#f0fdf4;border-radius:10px;padding:12px;margin-bottom:16px;font-size:13px;color:#333">
            ...
            📊 Note finale : <strong>${h}</strong> → <strong style="color:#1A6B3C">${h+F}</strong>
        ...
        </div>`,document.body.appendChild(se),se.querySelector("#sac-cancel").addEventListener("click",()=>{se.remove(),G(!1)}),se.querySelector("#sac-ok").addEventListener("click",()=>{se.remove(),G(!0)}),se.addEventListener("click",de=>{de.target===se&&(se.remove(),G(!1))})}))return;if(V.length){await _.from("market_listings").delete().in("card_id",V),await _.from("deck_cards").delete().in("card_id",V),await _.from("transfer_history").delete().in("card_id",V),await _.from("decks").update({stadium_card_id:null}).in("stadium_card_id",V);const{error:G}=await _.from("cards").delete().in("id",V);if(G){o("Erreur suppression : "+G.message,"error");return}}const re=(e.evolution_bonus||0)+F,{error:q}=await _.from("cards").update({evolution_bonus:re}).eq("id",e.id);if(q){o("Erreur évolution : "+q.message,"error");return}o(`⬆️ ${l.firstname} ${l.surname_real} : note ${h} → ${h+re} (+${F}) !`,"success",4e3),s(),d("collection")}),(Y=document.getElementById("market-sell-btn"))==null||Y.addEventListener("click",async()=>{var R;const U=[...z];if(!U.length){o("Sélectionne au moins un exemplaire","warning");return}const V=parseInt((R=document.getElementById("sell-market-price"))==null?void 0:R.value);if(!V||V<1){o("Prix invalide","error");return}const{error:E}=await _.from("cards").update({is_for_sale:!0,sale_price:V}).in("id",U);if(E){o(E.message,"error");return}const F=U.map(re=>({seller_id:r.profile.id,card_id:re,price:V,status:"active"})),{error:H}=await _.from("market_listings").insert(F);H&&console.warn("[Market] insert listings:",H.message),o(`${U.length} carte${U.length>1?"s":""} mise${U.length>1?"s":""} en vente à ${V.toLocaleString("fr")} cr. chacune !`,"success"),s(),d("collection")}),(pe=document.getElementById("cancel-sell-btn"))==null||pe.addEventListener("click",async()=>{await _.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await _.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),o("Annonce retirée","success"),s(),d("collection")})}function bn(e,t=""){return new Promise(i=>{const n=document.createElement("div");n.className="modal-overlay",n.style.zIndex="2100",n.innerHTML=`<div class="modal" style="max-width:360px">
  ...
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
  ...
  </div>`;const l=window.innerWidth>=900,m=e.querySelector(".deck-pc-layout"),b=e.querySelector(".deck-mobile-layout");m&&(m.style.display=l?"block":"none"),b&&(b.style.display=l?"none":"block"),Fo(e,t,r,i),e.querySelectorAll("#builder-back").forEach(u=>u.addEventListener("click",()=>n("decks")));const y=()=>{const{openModal:u,closeModal:x}=i,h=`<div style="display:flex;flex-wrap:wrap;gap:8px;padding:8px">
    ...
    </div>`;u("⚽ Choisir une formation",h),document.querySelectorAll("#modal-body [data-forma]").forEach(v=>{v.addEventListener("click",()=>{t.formation=v.dataset.forma;const $=hn(t.formation),f={};$.forEach(M=>{t.slots[M]&&(f[M]=t.slots[M])}),t.slots=f,x(),_t(e,t,i)})})};e.querySelectorAll("#formation-mobile-btn, #formation-pc-btn").forEach(u=>u.addEventListener("click",y)),e.querySelectorAll("#add-stad-btn-pc, #add-stad-btn").forEach(u=>u.addEventListener("click",()=>Po(t,e,i))),e.querySelectorAll("#save-deck").forEach(u=>u.addEventListener("click",()=>Ro(t,i))),e.querySelectorAll("[data-remove-sub]").forEach(u=>{u.addEventListener("click",()=>{t.subs=t.subs.filter(x=>x!==u.dataset.removeSub),_t(e,t,i)})}),e.querySelectorAll("#add-sub-btn").forEach(u=>u.addEventListener("click",()=>No(t,e,i)))}function Fo(e,t,i,n){var v,$,f;const r=window.innerWidth>=900,o=e.querySelector(r?"#deck-field-pc":"#deck-field-mobile");if(!o)return;const a=(v=t.stadiumCards)==null?void 0:v.find(M=>M.id===t.stadiumCardId),s=a&&(($=t.stadDefMap)==null?void 0:$[a.stadium_id])||null,d=zt[t.formation]||{},c=Ri(t.formation)||[],l={};for(const M of i){const k=t.slots[M],I=k?t.playerCards.find(B=>B.id===k):null;I!=null&&I.player?l[M]={...I.player,_evolution_bonus:I.evolution_bonus||0,face:I.player.face||null}:l[M]=null}const m=window.innerWidth>=900,b=m?window.innerWidth-280:window.innerWidth-20,y=m?Math.min(b,860):b,g=Math.round(m?y*.82:y*.85),p=m?84:44;let u="";for(const[M,k]of c){const I=d[M],B=d[k];if(!I||!B)continue;const ee=I.x*y,te=Math.round(.03*g+I.y*.85*g),ne=B.x*y,z=Math.round(.03*g+B.y*.85*g),S=l[M],K=l[k],J=Ut(S,K);J==="#ff3333"||J==="#cc2222"?u+=`<line x1="${ee.toFixed(1)}" y1="${te.toFixed(1)}" x2="${ne.toFixed(1)}" y2="${z.toFixed(1)}" stroke="${J}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(u+=`<line x1="${ee.toFixed(1)}" y1="${te.toFixed(1)}" x2="${ne.toFixed(1)}" y2="${z.toFixed(1)}" stroke="${J}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,u+=`<line x1="${ee.toFixed(1)}" y1="${te.toFixed(1)}" x2="${ne.toFixed(1)}" y2="${z.toFixed(1)}" stroke="${J}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let x="";const h=Math.round(p*657/507);for(const M of i){const k=d[M];if(!k)continue;const I=l[M],B=k.x*y;k.y*g;const ee=Math.round(.03*g+k.y*(.85*g)),te=Math.round(B-p/2),ne=Math.round(ee-h/2);if(I){const z=M.replace(/\d+/,""),S=s&&(s.club_id&&String(I.club_id)===String(s.club_id)||s.country_code&&I.country_code===s.country_code),K=Ie({...I,_evolution_bonus:I._evolution_bonus||0},{width:p,showStad:!0,stadDef:s,role:z});S&&((f=s.club)!=null&&f.logo_url||s.image_url),x+=`<div style="position:absolute;left:${te}px;top:${ne}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${M}">
        <div style="position:relative">${K}</div>
      </div>`}else{const z=M.replace(/\d+/,"");x+=`<div style="position:absolute;left:${te}px;top:${ne}px;width:${p}px;height:${h}px;
    ...
    </div>`),(a=document.getElementById("stad-none"))==null||a.addEventListener("click",()=>{e.stadiumCardId=null,r(),_t(t,e,i)}),document.querySelectorAll(".stad-choice").forEach(s=>{s.addEventListener("click",()=>{e.stadiumCardId=s.dataset.stadId,r(),_t(t,e,i)})})}function Go(e,t,i,n){var g,p,u,x,h;const{openModal:r,closeModal:o}=n,a=e.replace(/\d+/,""),s=(g=t.stadiumCards)==null?void 0:g.find(v=>v.id===t.stadiumCardId),d=s&&((p=t.stadDefMap)==null?void 0:p[s.stadium_id])||null,c=t.slots[e],l=c?t.playerCards.find(v=>v.id===c):null;(u=l==null?void 0:l.player)==null||u.id;const m=new Set;Object.entries(t.slots).forEach(([v,$])=>{var M;if(v===e||!$)return;const f=t.playerCards.find(k=>k.id===$);(M=f==null?void 0:f.player)!=null&&M.id&&m.add(f.player.id)}),t.subs.forEach(v=>{var f;const $=t.playerCards.find(M=>M.id===v);(f=$==null?void 0:$.player)!=null&&f.id&&m.add($.player.id)});const b=new Set,y=t.playerCards.filter(v=>{const $=v.player;return!($.job===a||$.job2===a)||m.has($.id)||b.has($.id)?!1:(b.add($.id),!0)});y.sort((v,$)=>{const f=v.evolution_bonus||0,M=$.evolution_bonus||0,k=(a==="GK"?v.player.note_g:a==="DEF"?v.player.note_d:a==="MIL"?v.player.note_m:v.player.note_a)+(a===v.player.job||a===v.player.job2?f:0);return(a==="GK"?$.player.note_g:a==="DEF"?$.player.note_d:a==="MIL"?$.player.note_m:$.player.note_a)+(a===$.player.job||a===$.player.job2?M:0)-k}),r(`Choisir ${a} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${y.length>0?'<div style="display:flex;flex-wrap:wrap;gap:8px;justify-content:center">'+y.map(v=>{const $={...v.player,_evolution_bonus:v.evolution_bonus||0};return`<div class="player-option" data-card-id="${v.id}" style="cursor:pointer">
    ...
    </div>`,'<button class="btn btn-ghost" id="close-sub-selector">Fermer</button>'),(b=document.getElementById("close-sub-selector"))==null||b.addEventListener("click",r),document.querySelectorAll(".player-option").forEach(y=>{y.addEventListener("click",()=>{e.subs.push(y.dataset.cardId),r(),_t(t,e,i)})})}async function Ro(e,t){const{state:i,toast:n,navigate:r}=t,o=e.formationCards.find(d=>d.formation===e.formation),a=(o==null?void 0:o.id)||e.formationCardId;await _.from("decks").update({formation:e.formation,formation_card_id:a||null,stadium_card_id:e.stadiumCardId||null}).eq("id",e.deckId),await _.from("deck_cards").delete().eq("deck_id",e.deckId);const s=[];if(Object.entries(e.slots).forEach(([d,c],l)=>{s.push({deck_id:e.deckId,card_id:c,position:d,is_starter:!0,slot_order:l})}),e.subs.forEach((d,c)=>{s.push({deck_id:e.deckId,card_id:d,position:`SUB${c+1}`,is_starter:!1,slot_order:100+c})}),s.length>0){const{error:d}=await _.from("deck_cards").insert(s);if(d){n(d.message,"error");return}}n("Deck enregistré ✅","success"),r("decks")}function hn(e){const t=mi[e]||mi["4-4-2"],i=["GK1"];for(let n=1;n<=t.DEF;n++)i.push(`DEF${n}`);for(let n=1;n<=t.MIL;n++)i.push(`MIL${n}`);for(let n=1;n<=t.ATT;n++)i.push(`ATT${n}`);return i}async function Fn(e=null){const t=new Date().toISOString().slice(0,10),{data:i}=await _.from("booster_configs").select("*").eq("is_active",!0).order("sort_order");if(!(i!=null&&i.length))return[];const n=i.filter(a=>!(a.available_from&&t<a.available_from||a.available_until&&t>a.available_until));if(!n.length)return[];let r=n;if(e){const a=n.filter(s=>s.max_per_user!=null);if(a.length){const{data:s}=await _.from("booster_claims").select("booster_id").eq("user_id",e).in("booster_id",a.map(c=>c.id)),d={};(s||[]).forEach(c=>{d[c.booster_id]=(d[c.booster_id]||0)+1}),r=n.filter(c=>c.max_per_user==null?!0:(d[c.id]||0)<c.max_per_user)}}if(!r.length)return[];const{data:o}=await _.from("booster_drop_rates").select("*").in("booster_id",r.map(a=>a.id)).order("sort_order");return r.map(a=>({...a,rates:(o||[]).filter(s=>s.booster_id===a.id)}))}async function Oo(e,t){const{data:i}=await _.from("booster_configs").select("max_per_user").eq("id",t).single();i!=null&&i.max_per_user&&await _.from("booster_claims").insert({user_id:e,booster_id:t})}function Ho(e){if(!(e!=null&&e.length))return null;const t=e.reduce((n,r)=>n+Number(r.percentage),0);let i=Math.random()*t;for(const n of e)if(i-=Number(n.percentage),i<=0)return n;return e[e.length-1]}const Pn=()=>Object.keys(zt),Uo=[{id:"players_std",img:"/icons/booster-players.png",name:"Players",sub:"5 cartes joueurs",cost:5e3,costLabel:"5 000 crédits",cardCount:5,type:"player"},{id:"players_pub",img:"/icons/booster-silver.png",name:"Players (pub)",sub:"3 cartes joueurs",cost:0,costLabel:"1 pub",cardCount:3,type:"player"},{id:"game_changer",img:"/icons/booster-gamechanger.png",name:"Game Changer",sub:"3 cartes spéciales",cost:1e4,costLabel:"10 000 crédits",cardCount:3,type:"game_changer"},{id:"formation",img:"/icons/booster-formation.png",name:"Formation",sub:"1 carte formation",cost:1e4,costLabel:"10 000 crédits",cardCount:1,type:"formation"}],ji={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé."},"Double attaque":{icon:"⚡",desc:"La prochaine attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la prochaine action IA."},Gel:{icon:"❄️",desc:"Bloque le meilleur attaquant IA."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function Ko(e){const t=e.player;if(!t)return"";const i=e.evolution_bonus||0;return Ie({...t,_evolution_bonus:i},{width:140})}function Gn(e){var r;const t={};(e.rates||[]).forEach(o=>{t[o.card_type]=(t[o.card_type]||0)+Number(o.percentage||0)});const i=((r=Object.entries(t).sort((o,a)=>a[1]-o[1])[0])==null?void 0:r[0])||"player",n=e.image_url||"booster-players.png";return{id:e.id,img:"/icons/"+n,name:e.name,sub:`${e.card_count} carte(s)`,cost:e.price_type==="credits"&&e.price_credits||0,costLabel:e.price_type==="credits"?`${(e.price_credits||0).toLocaleString("fr")} crédits`:e.price_type==="pub"?"1 pub":"Gratuit",cardCount:e.card_count||5,type:i,isPub:e.price_type==="pub",rates:e.rates||[],allow_duplicates:e.allow_duplicates!==!1,_boosterId:e.id,_raw:e}}async function Vo(e,{state:t,navigate:i,toast:n}){var m,b,y;const r=((m=t.profile)==null?void 0:m.credits)||0;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⏳ Chargement...</div>';let o=[];try{o=(await Fn((b=t.user)==null?void 0:b.id)).map(Gn)}catch(g){console.warn("Erreur chargement boosters DB, fallback hardcodé",g)}o.length||(o=Uo.map(g=>({...g,rates:[],isPub:g.id==="players_pub"})));const a=await _.from("cards").select("player_id, card_type, formation, stadium_id, gc_type").eq("owner_id",t.profile.id).then(g=>g.data||[]),s=new Set(a.filter(g=>g.card_type==="stadium").map(g=>g.stadium_id)),d=new Set(a.filter(g=>g.card_type==="formation").map(g=>g.formation)),c=new Set(a.filter(g=>g.card_type==="game_changer").map(g=>g.gc_type)),l={};for(const g of o){if(g.allow_duplicates!==!1||!((y=g.rates)!=null&&y.length))continue;const p=[...new Set((g.rates||[]).map(x=>x.card_type))];let u=!1;for(const x of p)if(x==="stadium"){const{data:h}=await _.from("stadium_definitions").select("id");if((h||[]).some(v=>!s.has(v.id))){u=!0;break}}else if(x==="game_changer"){const{data:h}=await _.from("gc_definitions").select("name").eq("is_active",!0);if((h||[]).some(v=>!c.has(v.name))){u=!0;break}}else if(x==="formation"){const{FORMATION_LINKS:h}=await Ln(async()=>{const{FORMATION_LINKS:v}=await import("./special-cards-BY0MTcrl.js").then($=>$.y);return{FORMATION_LINKS:v}},__vite__mapDeps([0,1]));if(Object.keys(h).some(v=>!d.has(v))){u=!0;break}}else{u=!0;break}u||(l[g.id]=!0)}e.innerHTML=`
  <div class="page">
    <div class="page-header">
      <h2>📦 Boosters</h2>
  ...
  </div>`,e.querySelectorAll(".booster-card:not(.disabled)").forEach(g=>{g.addEventListener("click",async()=>{const p=o.find(u=>u.id===g.dataset.booster);if(p){g.style.opacity="0.5",g.style.pointerEvents="none";try{await Yo(p,{state:t,toast:n,navigate:i,container:e})}catch(u){n(u.message,"error"),g.style.opacity="",g.style.pointerEvents=""}}})}),e.querySelectorAll(".booster-info-btn").forEach(g=>{g.addEventListener("click",p=>{p.stopPropagation();const u=o.find(x=>x.id===g.dataset.boosterId);Qo(u)})})}async function Yo(e,{state:t,toast:i,navigate:n,container:r}){var m,b;if(e.cost>0&&t.profile.credits<e.cost){i("Crédits insuffisants","error");return}e.isPub&&await er();const{data:o}=await _.from("cards").select("card_type, player_id, formation").eq("owner_id",t.profile.id),a=new Set((o||[]).filter(y=>y.card_type==="player").map(y=>y.player_id)),s=new Set((o||[]).filter(y=>y.card_type==="formation").map(y=>y.formation));let d=[],c=null;try{if((m=e.rates)!=null&&m.length)d=await Bi(t.profile,e);else{const y=e.type||"player";y==="player"?d=await Nn(t.profile,e.cardCount,e.cost):y==="game_changer"?d=await Rn(t.profile,e.cardCount,e.cost):y==="formation"?d=await On(t.profile,e.cost):d=await Bi(t.profile,e)}}catch(y){c=y.message||String(y),console.error("[Booster] Erreur:",y)}if(d!=null&&d.length&&e._boosterId&&await Oo(t.user.id,e._boosterId),!(d!=null&&d.length)){const y=document.createElement("div");y.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:3000;gap:16px;color:#fff;padding:24px;text-align:center",y.innerHTML=`
      <div style="font-size:48px">😕</div>
      <div style="font-size:20px;font-weight:900">Aucune carte obtenue</div>
      <div style="font-size:12px;color:rgba(255,255,255,0.6);max-width:320px;word-break:break-all;background:rgba(255,255,255,0.05);padding:10px;border-radius:8px;margin-top:6px">
        ${c||"Vérifie la console (F12) pour plus de détails"}
      </div>
      <button style="margin-top:10px;padding:12px 28px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer" id="anim-close-err">Fermer</button>`,document.body.appendChild(y),(b=y.querySelector("#anim-close-err"))==null||b.addEventListener("click",()=>y.remove());return}d.forEach(y=>{y.card_type==="player"&&y.player?y.isDuplicate=a.has(y.player.id):y.card_type==="formation"&&(y.isDuplicate=s.has(y.formation))});const{data:l}=await _.from("users").select("*").eq("id",t.profile.id).single();l&&(t.profile=l),Hn(d,e,n)}function Wo(){const e=Math.random()*100;return e<.5?"legende":e<2?"special":e<10?"normal_high":"normal_low"}function mt(e){return Math.max(Number(e.note_g)||0,Number(e.note_d)||0,Number(e.note_m)||0,Number(e.note_a)||0)}function Xo(e,t){let i;switch(t){case"legende":i=e.filter(n=>n.rarity==="legende"),i.length||(i=e.filter(n=>n.rarity==="pepite"||n.rarity==="papyte")),i.length||(i=e.filter(n=>mt(n)>=6));break;case"special":i=e.filter(n=>n.rarity==="pepite"||n.rarity==="papyte"),i.length||(i=e.filter(n=>mt(n)>=6));break;case"normal_high":i=e.filter(n=>n.rarity==="normal"&&mt(n)>=6),i.length||(i=e.filter(n=>mt(n)>=6));break;default:i=e.filter(n=>n.rarity==="normal"&&mt(n)>=1&&mt(n)<=5),i.length||(i=e.filter(n=>n.rarity==="normal"));break}return i.length||(i=e),i[Math.floor(Math.random()*i.length)]}async function Bi(e,t){if(t.cost>0){const{error:b}=await _.from("users").update({credits:e.credits-t.cost}).eq("id",e.id);if(b)throw b}const i=t.allow_duplicates!==!1;let n=[];const{data:r,error:o}=await _.from("cards").select("player_id, card_type, formation, stadium_id, gc_type").eq("owner_id",e.id);if(o){const{data:b}=await _.from("cards").select("player_id, card_type, formation, gc_type").eq("owner_id",e.id);n=b||[]}else n=r||[];const a=new Set(n.filter(b=>b.card_type==="player").map(b=>b.player_id)),s=new Set(n.filter(b=>b.card_type==="formation").map(b=>b.formation)),d=new Set(n.filter(b=>b.card_type==="game_changer").map(b=>b.gc_type)),c=new Set(n.filter(b=>b.card_type==="stadium").map(b=>b.stadium_id).filter(Boolean)),l=new Set,m=[];for(let b=0;b<(t.cardCount||5);b++){const y=Ho(t.rates);if(y){if(y.card_type==="player"){const g=k=>({légende:"legende",pépite:"pepite",pépites:"pepite"})[k]||k,p=y.rarity?g(y.rarity):null;let u=_.from("players").select("id,job,firstname,surname_real,country_code,club_id,rarity,note_g,note_d,note_m,note_a,skin,hair,hair_length,face,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);p&&(u=u.eq("rarity",p));const{data:x}=await u;let h=x||[];if((y.note_min||y.note_max)&&(h=h.filter(k=>{const I=Math.max(Number(k.note_g)||0,Number(k.note_d)||0,Number(k.note_m)||0,Number(k.note_a)||0);return(!y.note_min||I>=y.note_min)&&(!y.note_max||I<=y.note_max)})),h.length||(y.note_min||y.note_max?(h=x||[],console.warn("[Booster] Aucun joueur avec note",y.note_min,"-",y.note_max,"— fallback rareté uniquement")):h=x||[]),!h.length)continue;let v=h.filter(k=>!l.has(k.id));if(i)v.length||(v=h);else if(v=v.filter(k=>!a.has(k.id)),!v.length)continue;const $=v[Math.floor(Math.random()*v.length)];l.add($.id);const f=a.has($.id),{data:M}=await _.from("cards").insert({owner_id:e.id,player_id:$.id,card_type:"player"}).select().single();M&&(m.push({...M,player:$,isDuplicate:f}),_.rpc("record_transfer",{p_card_id:M.id,p_player_id:$.id,p_club_name:e.club_name||e.pseudo,p_manager_name:e.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{}))}else if(y.card_type==="game_changer"){const{data:g}=await _.from("gc_definitions").select("id,name,color,effect,image_url,gc_type").eq("is_active",!0).eq("gc_type","game_changer"),p=g!=null&&g.length?g:[{name:"Ressusciter"},{name:"Double attaque"},{name:"Bouclier"},{name:"Vol de note"},{name:"Gel"}],u=i?p:p.filter($=>!d.has($.name));if(!i&&!u.length)continue;const x=u[Math.floor(Math.random()*u.length)],h=x.name,{data:v}=await _.from("cards").insert({owner_id:e.id,card_type:"game_changer",gc_type:h,gc_definition_id:x.id||null}).select().single();v&&m.push({...v,_gcDef:x})}else if(y.card_type==="formation"){const g=Pn(),p=i?g:g.filter(v=>!s.has(v));if(!i&&!p.length)continue;const u=p[Math.floor(Math.random()*p.length)],x=s.has(u),{data:h}=await _.from("cards").insert({owner_id:e.id,card_type:"formation",formation:u}).select();h!=null&&h[0]&&m.push({...h[0],isDuplicate:x})}else if(y.card_type==="stadium"){const{data:g,error:p}=await _.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)");if(p){console.error("[Booster] stadium_definitions:",p.message);continue}if(!(g!=null&&g.length)){console.warn("[Booster] Aucun stade en DB");continue}const u=i?g:g.filter($=>!c.has($.id));if(!i&&!u.length)continue;const x=u[Math.floor(Math.random()*u.length)],{data:h,error:v}=await _.from("cards").insert({owner_id:e.id,card_type:"stadium",stadium_id:x.id}).select("id,card_type,stadium_id").single();if(v){console.error("[Booster] insert stadium card:",v.message);continue}h&&m.push({...h,rarity:"normal",_stadiumDef:x})}}}return m}async function Nn(e,t,i){if(i>0){const{error:c}=await _.from("users").update({credits:e.credits-i}).eq("id",e.id);if(c)throw c}const{data:n}=await _.from("players").select("id,job,firstname,surname_real,country_code,club_id,rarity,note_g,note_d,note_m,note_a,note_min,note_max,skin,hair,hair_length,face,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);if(!(n!=null&&n.length))throw new Error("Pas de joueurs en BDD — ajoutes-en via le panel admin !");const r=n.filter(c=>c.job==="GK"),o=n.filter(c=>c.job!=="GK"),a=!e.first_booster_opened&&r.length>0,s=[];for(let c=0;c<t;c++){const l=c===0&&a?r:c===0?o:n,m=Wo(),b=Xo(l,m);b&&s.push(b)}a&&await _.from("users").update({first_booster_opened:!0}).eq("id",e.id);const{data:d}=await _.from("cards").insert(s.map(c=>({owner_id:e.id,player_id:c.id,card_type:"player"}))).select();return(d||[]).forEach((c,l)=>{_.rpc("record_transfer",{p_card_id:c.id,p_player_id:s[l].id,p_club_name:e.club_name||e.pseudo,p_manager_name:e.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{})}),s.map((c,l)=>({...d[l],player:c}))}async function Rn(e,t,i){const{error:n}=await _.from("users").update({credits:e.credits-i}).eq("id",e.id);if(n)throw n;const{data:r}=await _.from("gc_definitions").select("id,name,gc_type,color,effect,image_url").eq("is_active",!0),o=r!=null&&r.length?r:Object.keys(ji).map(m=>({name:m,gc_type:"game_changer"})),a=Array.from({length:t},()=>o[Math.floor(Math.random()*o.length)]),s=a.map(m=>({owner_id:e.id,card_type:"game_changer",gc_type:m.name,gc_definition_id:m.id||null})),{data:d,error:c}=await _.from("cards").insert(s).select();return c&&console.error("[Booster GC] Erreur insert:",c.message,c),(d||[]).map((m,b)=>({...m,_gcDef:a[b]||null}))}async function On(e,t){const{error:i}=await _.from("users").update({credits:e.credits-t}).eq("id",e.id);if(i)throw i;const{data:n}=await _.from("cards").select("formation").eq("owner_id",e.id).eq("card_type","formation"),r=new Set((n||[]).map(l=>l.formation)),o=Pn(),a=o[Math.floor(Math.random()*o.length)],s=r.has(a),{data:d,error:c}=await _.from("cards").insert({owner_id:e.id,card_type:"formation",formation:a}).select();return c&&console.error("[Booster Formation] Erreur insert:",c.message,c),(d||[]).map(l=>({...l,isDuplicate:s}))}function Hn(e,t,i,n=null){var ne,z;if(!e||e.length===0){const S=document.createElement("div");S.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:3000;gap:16px;color:#fff;padding:24px;text-align:center",S.innerHTML=`
      <div style="font-size:48px">😕</div>
      <div style="font-size:20px;font-weight:900">Aucune carte obtenue</div>
      <div style="font-size:13px;color:rgba(255,255,255,0.5)">Erreur lors du tirage (permissions DB ou colonne manquante)</div>
      <button style="margin-top:10px;padding:12px 28px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer" id="anim-close-err">Fermer</button>`,document.body.appendChild(S),(ne=S.querySelector("#anim-close-err"))==null||ne.addEventListener("click",()=>S.remove());return}e=[...e].sort((S,K)=>{const J=S.player?mt(S.player):-1;return(K.player?mt(K.player):-1)-J});const r=document.createElement("div");r.id="booster-anim-overlay",r.innerHTML=`
    <style>
  ...
  `,document.body.appendChild(r);let o=!1;const a=document.getElementById("pack-cut-zone"),s=document.getElementById("pack-blade");let d=!1;const c=S=>S.touches&&S.touches[0]?S.touches[0].clientX:S.clientX;function l(S){o||(d=!0,s.style.opacity="1",m(S))}function m(S){if(!d||o)return;const K=a.getBoundingClientRect(),J=c(S)-K.left,ue=Math.max(0,Math.min(1,J/K.width));s.style.width=ue*K.width+"px",ue>=.82&&y()}function b(){o||(d=!1,s.style.transition="width .2s ease, opacity .2s ease",s.style.width="0",s.style.opacity="0",setTimeout(()=>{o||(s.style.transition="")},220))}function y(){var K;if(o)return;o=!0,d=!1,s.style.width="100%",s.style.opacity="1",(K=document.getElementById("cut-flash"))==null||K.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const S=document.getElementById("cut-hint");S&&(S.style.opacity="0"),a.classList.add("pack-cut"),setTimeout(()=>{s.style.opacity="0",document.getElementById("pack-phase").style.display="none",x(0)},620)}a.addEventListener("pointerdown",l),window.addEventListener("pointermove",m),window.addEventListener("pointerup",b),a.addEventListener("touchstart",l,{passive:!0}),window.addEventListener("touchmove",m,{passive:!0}),window.addEventListener("touchend",b);let g=0,p=!1;const u=new Set;function x(S){g=S,document.getElementById("reveal-phase").style.display="flex",h(),v(S,0),I()}function h(){const S=document.getElementById("card-dots");S&&(S.innerHTML=e.map((K,J)=>`<div class="card-dot" data-i="${J}" style="width:8px;height:8px;border-radius:50%;background:${J===g?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),S.querySelectorAll(".card-dot").forEach(K=>K.addEventListener("click",()=>f(parseInt(K.dataset.i)))))}function v(S,K){var E,F;const J=e[S],ue=document.getElementById("card-counter"),fe=document.getElementById("card-track");ue&&(ue.textContent=`Carte ${S+1} / ${e.length}`);const C=document.getElementById("reveal-btns");C&&(C.style.display=S===e.length-1?"flex":"none");const Y=J.card_type==="player"&&((E=J.player)==null?void 0:E.rarity)==="legende",pe=!u.has(S);u.add(S);let U=0;if(J.card_type==="player"&&J.player){const H=J.player,R=H.job||"ATT";U=(Number(R==="GK"?H.note_g:R==="DEF"?H.note_d:R==="MIL"?H.note_m:H.note_a)||0)+(J.evolution_bonus||0)}const V=H=>{fe.innerHTML=`
        <div id="current-card-wrap" style="position:relative;display:flex;flex-direction:column;align-items:center;gap:8px;${Y?"filter:drop-shadow(0 0 20px #7a28b8)":""}">
          <div style="transform:scale(1.25);transform-origin:center">${Jo(J)}</div>
        ...
        </div>`;const R=document.getElementById("current-card-wrap");K!==0?(R.style.transition="none",R.style.transform=`translateX(${K>0?100:-100}%)`,requestAnimationFrame(()=>{R.style.transition="transform .28s cubic-bezier(.25,1,.5,1)",R.style.transform="translateX(0)"})):R.animate([{opacity:0,transform:"scale(.7)"},{opacity:1,transform:"scale(1)"}],{duration:300,easing:"cubic-bezier(.34,1.56,.64,1)"}),H||Y?ee():te(),h()};pe&&(((F=J.player)==null?void 0:F.rarity)==="legende"||U>=18)&&J.card_type==="player"&&J.player?$(J,()=>V(!0)):V(!1)}function $(S,K){var Te;p=!0;const J=S.player,ue=`https://flagsapi.com/${J.country_code}/flat/64.png`,fe=(Te=J.clubs)==null?void 0:Te.logo_url,C=At(J),Y=J.job||"ATT",pe=Number(Y==="GK"?J.note_g:Y==="DEF"?J.note_d:Y==="MIL"?J.note_m:J.note_a)||0,U=S.evolution_bonus||0,V=pe+U,E=V>=18&&V<=20,F=V>=18,H=document.getElementById("walkout-overlay"),R=document.getElementById("walkout-stage");if(!H||!R){p=!1,K();return}let re=null;E&&(re=Wi("/sounds/Legendary.mp3",.8)),H.style.display="flex";const q=()=>{const ve=R.firstElementChild;ve&&(ve.classList.remove("wo-in"),ve.classList.add("wo-out"))},G=1800,se=400;R.innerHTML=`<img class="wo-in" src="${ue}" style="height:130px;border-radius:10px;box-shadow:0 10px 36px rgba(0,0,0,.6)" onerror="this.style.display='none'">`,navigator.vibrate&&navigator.vibrate(30),setTimeout(q,G),setTimeout(()=>{var ve;R.innerHTML=fe?`<img class="wo-in" src="${fe}" style="max-height:160px;max-width:210px;object-fit:contain">`:`<div class="wo-in" style="font-size:34px;font-weight:900;color:#fff;text-align:center">${((ve=J.clubs)==null?void 0:ve.encoded_name)||"CLUB"}</div>`,navigator.vibrate&&navigator.vibrate(30)},G+se),setTimeout(q,G*2+se),setTimeout(()=>{R.innerHTML=C?`<img class="wo-in" src="${C}" style="height:200px;border-radius:50%;box-shadow:0 0 40px rgba(255,255,255,0.3);object-fit:cover;object-position:top">`:'<div class="wo-in" style="font-size:80px">👤</div>',navigator.vibrate&&navigator.vibrate(30)},(G+se)*2),setTimeout(q,(G+se)*2+G);const he=F?"#FFD700":{GK:"#c0c0c0",DEF:"#e03030",MIL:"#D4A017",ATT:"#3fbf5f"}[Y]||"#fff";setTimeout(()=>{R.innerHTML=`<div class="wo-in" style="
        ...
        ${V}
      </div>`,F&&navigator.vibrate&&navigator.vibrate([50,30,100,30,200])},(G+se)*3),setTimeout(q,(G+se)*3+G),setTimeout(()=>{H.style.display="none",R.innerHTML="",p=!1,re&&!E&&re.pause(),navigator.vibrate&&navigator.vibrate([40,30,80]),K()},(G+se)*4)}function f(S){if(p||S<0||S>=e.length||S===g)return;const K=S>g?1:-1;g=S,v(S,K)}function M(){f(g+1)}function k(){f(g-1)}function I(){const S=document.getElementById("card-viewport");if(!S||S._swipeBound)return;S._swipeBound=!0;let K=0,J=0,ue=0,fe=!1;const C=E=>E.touches?E.touches[0].clientX:E.clientX,Y=E=>E.touches?E.touches[0].clientY:E.clientY,pe=E=>{fe=!0,K=C(E),J=Y(E),ue=0},U=E=>{if(!fe)return;ue=C(E)-K;const F=Y(E)-J;if(Math.abs(ue)<Math.abs(F))return;const H=document.getElementById("current-card-wrap");H&&(H.style.transition="none",H.style.transform=`translateX(${ue*.6}px) rotate(${ue*.02}deg)`)},V=()=>{if(!fe)return;fe=!1;const E=document.getElementById("current-card-wrap"),F=55;ue<=-F&&g<e.length-1?M():ue>=F&&g>0?k():E&&(E.style.transition="transform .2s ease",E.style.transform="translateX(0)")};S.addEventListener("pointerdown",pe),S.addEventListener("pointermove",U),S.addEventListener("pointerup",V),S.addEventListener("pointercancel",V),S.addEventListener("touchstart",pe,{passive:!0}),S.addEventListener("touchmove",U,{passive:!0}),S.addEventListener("touchend",V),S.addEventListener("click",E=>{if(Math.abs(ue)>8)return;const F=S.getBoundingClientRect();E.clientX-F.left>F.width/2?M():k()})}let B=null;function ee(){const S=document.getElementById("fireworks-canvas");if(!S)return;S.width=window.innerWidth,S.height=window.innerHeight;const K=S.getContext("2d"),J=[];function ue(){const C=Math.random()*S.width,Y=Math.random()*S.height*.6,pe=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],U=pe[Math.floor(Math.random()*pe.length)];for(let V=0;V<60;V++){const E=Math.PI*2/60*V,F=2+Math.random()*5;J.push({x:C,y:Y,vx:Math.cos(E)*F,vy:Math.sin(E)*F,alpha:1,color:U,size:2+Math.random()*3})}}ue(),B=setInterval(ue,600);function fe(){if(document.getElementById("fireworks-canvas")){K.clearRect(0,0,S.width,S.height);for(let C=J.length-1;C>=0;C--){const Y=J[C];if(Y.x+=Y.vx,Y.y+=Y.vy+.08,Y.vy*=.98,Y.alpha-=.018,Y.alpha<=0){J.splice(C,1);continue}K.globalAlpha=Y.alpha,K.fillStyle=Y.color,K.beginPath(),K.arc(Y.x,Y.y,Y.size,0,Math.PI*2),K.fill()}K.globalAlpha=1,(B!==null||J.length>0)&&requestAnimationFrame(fe)}}fe()}function te(){B!==null&&(clearInterval(B),B=null);const S=document.getElementById("fireworks-canvas");S&&S.getContext("2d").clearRect(0,0,S.width,S.height)}if(n){const S=document.getElementById("reveal-btns");S&&(S.innerHTML='<button class="btn btn-primary" id="reveal-next" style="flex:1">Continuer →</button>'),(z=document.getElementById("reveal-next"))==null||z.addEventListener("click",()=>{te(),r.remove(),n()})}else document.getElementById("reveal-collection").addEventListener("click",()=>{te(),r.remove(),i("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{te(),r.remove(),i("boosters")})}function Jo(e){var t,i,n,r;if(e.card_type==="player"&&e.player)return Ko(e);if(e.card_type==="game_changer"){const o=e._gcDef,a=(o==null?void 0:o.name)||e.gc_type||"Game Changer",s=(o==null?void 0:o.effect)||((t=ji[e.gc_type])==null?void 0:t.desc)||"",d=o!=null&&o.image_url?`/icons/${o.image_url}`:null,c=((i=ji[e.gc_type])==null?void 0:i.icon)||"⚡";return tt(a,d,c,s,{width:170})}if(e.card_type==="formation")return zi(e.formation,zt[e.formation],{width:160});if(e.card_type==="stadium"){const o=e._stadiumDef,a=(o==null?void 0:o.name)||"Stade",s=((n=o==null?void 0:o.club)==null?void 0:n.encoded_name)||(o==null?void 0:o.country_code)||"—",d=o!=null&&o.image_url?`/icons/${o.image_url}`:((r=o==null?void 0:o.club)==null?void 0:r.logo_url)||(o!=null&&o.country_code?`https://flagsapi.com/${o.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),c=`${s}<br>+10 ⭐ joueurs alliés`;return ti(a,d,c,{width:170})}return'<div style="width:140px;height:200px;background:#333;border-radius:12px"></div>'}function Qo(e){var t,i;if((t=e==null?void 0:e.rates)!=null&&t.length){const n=document.createElement("div");n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.6);display:flex;align-items:center;justify-content:center;z-index:4000;padding:16px";const r={normal:"#ccc",pepite:"#D4A017",pépite:"#D4A017",papyte:"#909090",legende:"#7a28b8",légende:"#7a28b8"},o={player:"Joueur",formation:"Formation",game_changer:"Game Changer",game_helper:"Game Helper"};n.innerHTML=`
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}function er(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText=`
    ...
    `,document.body.appendChild(t);let i=5;const n=setInterval(()=>{i--;const r=document.getElementById("mw-ad-cd");r&&(r.textContent=i),i<=0&&(clearInterval(n),t.remove(),e(!0))},1e3)})}async function tr(e,{state:t,navigate:i,toast:n,refreshProfile:r}){var b,y;const{data:o}=await _.from("users").select("*").eq("id",t.user.id).single();o&&(t.profile=o);let a=Array.isArray((b=t.profile)==null?void 0:b.pending_boosters)?[...t.profile.pending_boosters]:[];if(!a.length){await _.from("users").update({onboarding_done:!0}).eq("id",t.user.id),i("home");return}let s=null;try{const p=(await Fn()).find(u=>(u.name||"").toLowerCase().includes("new player"));p&&(s=Gn(p))}catch(g){console.warn('[Onboarding] Config "Booster (new player)" introuvable, fallback taux par défaut',g)}const d=a.length;let c=0;e.innerHTML=`
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      card:cards(id,card_type,formation,stadium_id,evolution_bonus,
        player:players(id,firstname,surname_real,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order"),d=[...new Set((s||[]).filter(b=>{var y,g;return((y=b.card)==null?void 0:y.card_type)==="stadium"&&((g=b.card)==null?void 0:g.stadium_id)}).map(b=>b.card.stadium_id))],c={};if(d.length){const{data:b}=await _.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",d);(b||[]).forEach(y=>{c[y.id]=y}),(s||[]).forEach(y=>{var g,p;((g=y.card)==null?void 0:g.card_type)==="stadium"&&((p=y.card)!=null&&p.stadium_id)&&(y.card._stadiumDef=c[y.card.stadium_id]||null)})}let l=0;function m(){var f,M,k,I,B,ee,te;const b=o[l],y=(s||[]).filter(ne=>ne.deck_id===b.id),g=y.filter(ne=>{var z;return ne.is_starter&&((z=ne.card)==null?void 0:z.player)}).map(ne=>qi(ne.card,ne.position)),p=y.find(ne=>{var z;return((z=ne.card)==null?void 0:z.card_type)==="formation"}),u=b.formation||((f=p==null?void 0:p.card)==null?void 0:f.formation)||"4-4-2";let x=g.length>=11?yi(g,u):null,h=((M=b.stadium_card)==null?void 0:M.stadium_def)||null;h&&x&&(x=ai(x,h));const v=g.length>=11;st(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff;position:relative">
      ...
      <!-- Pagination -->
      ${o.length>1?`<div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">${o.map((ne,z)=>`<div style="width:6px;height:6px;border-radius:50%;background:${z===l?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}</div>`:""}
    ...
    </div>`,requestAnimationFrame(()=>requestAnimationFrame(function ne(){const z=e.querySelector(".deck-preview-wrap"),S=e.querySelector("#deck-swipe-zone");if(!z||!S||!x)return;const K=S.clientWidth>=900,J=Math.max(200,S.clientHeight-(K?60:40)),fe=Math.max(200,S.clientWidth-16),C=K?Math.min(117,Math.max(52,Math.round(fe*.22))):Math.max(44,Math.round(fe*.168));if(J<220||fe<220){requestAnimationFrame(ne);return}const Y=K?null:Math.round(C*.55);z.innerHTML=Ot(x,u,null,[],fe,J,[],Y),z.style.cssText=`width:${fe}px;height:${J}px;overflow:hidden;margin:${K?0:60}px auto 0`;const pe=z.querySelector("svg");pe&&(pe.style.cssText="display:block;width:100%;height:100%",pe.setAttribute("preserveAspectRatio",K?"xMidYMid meet":"none"))})),(I=document.getElementById("prev-deck"))==null||I.addEventListener("click",()=>{l>0&&(l--,m())}),(B=document.getElementById("next-deck"))==null||B.addEventListener("click",()=>{l<o.length-1&&(l++,m())}),(ee=document.getElementById("validate-deck"))==null||ee.addEventListener("click",()=>{if(!v)return;const ne=t.state.params||{};t.navigate("match",{...ne,matchMode:ne.matchMode||i,deckId:b.id})}),(te=document.getElementById("cancel-deck-select"))==null||te.addEventListener("click",()=>{it(e),r("home")});const $=document.getElementById("deck-swipe-zone");if($){let ne=0,z=0;$.addEventListener("touchstart",S=>{ne=S.touches[0].clientX,z=S.touches[0].clientY},{passive:!0}),$.addEventListener("touchend",S=>{const K=S.changedTouches[0].clientX-ne,J=S.changedTouches[0].clientY-z;Math.abs(K)<40||Math.abs(K)<Math.abs(J)||(K<0&&l<o.length-1?(l++,m()):K>0&&l>0&&(l--,m()))},{passive:!0})}}m()}function ht(e,t=44,i=58,n=null){return Ie(e,{width:t,showStad:!!n,stadDef:n,used:e==null?void 0:e.used})}function Je(e,t,i,n,r){if(!(e!=null&&e.length))return"";const o=e.slice(0,5);let a='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return o.forEach((s,d)=>{const c=s._line||s.job||"MIL";let l=s.boost||0;if(s.stadiumBonus&&(n==="attack"&&(c==="ATT"||c==="MIL")||n==="defense"&&(c==="GK"||c==="DEF"||c==="MIL")?l+=10:n||(l+=10)),a+=Ie(s,{width:40,role:c,extraNote:l}),d<o.length-1){const m=o[d+1],b=Bn(s,m,r)?Ut(s,m):null;a+=`<div style="width:7px;height:3px;background:${!b||b==="#ff3333"||b==="#cc2222"?"rgba(255,255,255,0.12)":b};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(a+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),a+="</div>",a}function _i(e,t,i,n,r=310,o=310,a=[],s=null){const d=zt[t]||{},c=Ri(t)||Oi[t]||[],l={},m=["ATT","MIL","DEF","GK"];for(const x of m)(e[x]||[]).forEach((v,$)=>{l[`${x}${$+1}`]=v});function b(x){const h=d[x];return h?{x:h.x*r,y:h.y*o}:null}let y="";for(const[x,h]of c){const v=b(x),$=b(h);if(!v||!$)continue;const f=l[x],M=l[h],k=Ut(f,M);k==="#00ff88"||k==="#FFD700"?(y+=`<line x1="${v.x.toFixed(1)}" y1="${v.y.toFixed(1)}" x2="${$.x.toFixed(1)}" y2="${$.y.toFixed(1)}"
        stroke="${k}" stroke-width="10" stroke-linecap="round" opacity="0.22"/>`,y+=`<line x1="${v.x.toFixed(1)}" y1="${v.y.toFixed(1)}" x2="${$.x.toFixed(1)}" y2="${$.y.toFixed(1)}"
        stroke="${k}" stroke-width="3.5" stroke-linecap="round" opacity="0.95"/>`):y+=`<line x1="${v.x.toFixed(1)}" y1="${v.y.toFixed(1)}" x2="${$.x.toFixed(1)}" y2="${$.y.toFixed(1)}"
        stroke="${k}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const g=typeof window<"u"&&window.innerWidth>=900?Math.min(Math.max(81,Math.round(r*.225)),117):Math.max(44,Math.round(r*.168)),p=Math.round(g*657/507);for(const[x,h]of Object.entries(l)){const v=b(x);if(!v||!h)continue;const $=x.replace(/[0-9]/g,""),f=a.includes(h.cardId),M=i==="attack"&&(["MIL","ATT"].includes($)||f)&&!h.used||i==="defense"&&["GK","DEF","MIL"].includes($)&&!h.used,k=n.includes(h.cardId);let I=h.boost||0,B=!1;h.stadiumBonus&&(i==="attack"&&($==="ATT"||$==="MIL")||i==="defense"&&($==="GK"||$==="DEF"||$==="MIL")?(I+=10,B=!0):i||(I+=10,B=!0));const ee=Math.round(v.x-g/2),te=Math.round(v.y-p/2);if(h.used){y+=`<image href="${`${typeof import.meta<"u"&&"/"||"/"}icons/carte-dos.png`}" x="${ee}" y="${te}" width="${g}" height="${p}" preserveAspectRatio="xMidYMid slice" class="match-used-hit" data-card-id="${h.cardId}" data-role="${$}" style="cursor:pointer"/>`;continue}const ne=Ie({...h,_evolution_bonus:0,stadiumBonus:!1},{width:g,showStad:!1,stadDef:null,role:$,extraNote:I,_cardOffset:30,_forceStadColor:B}),z=k?`position:absolute;top:30px;left:0;width:${g}px;height:${p}px;outline:3px solid #FFD700;outline-offset:2px;border-radius:8px;pointer-events:none;`:"";y+=`<foreignObject x="${ee-2}" y="${te-30}" width="${g+8}" height="${p+60}" style="overflow:visible">
      <div xmlns="http://www.w3.org/1999/xhtml" style="position:relative">
        ${ne}
        ${k?`<div style="${z}"></div>`:""}
      </div>
    </foreignObject>`,M&&(y+=`<rect x="${ee}" y="${te}" width="${g}" height="${p}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${k?"selected":""}" data-card-id="${h.cardId}" data-role="${$}" style="cursor:pointer"/>`)}const u=s!==null?s:Math.round(Math.max(g*.7,80));return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-u} ${-u} ${r+u*2} ${o+u*2}" width="100%" style="display:block;width:100%;margin:0 auto">
    ${y}
  </svg>`}function Ot(e,t,i,n,r=300,o=300,a=[],s=null){return`<div id="match-terrain-wrap" style="position:relative;padding:0 4px">
    ${_i(e,t,i,n,r,o,a,s)}
  </div>`}async function wi(e,t,i,n){var M;const{state:r,navigate:o,toast:a}=t;st(e);const s=r.params||{};if(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!s.deckId)return rr(e,t,i);const d=s.deckId;let c,l,m,b;try{const k=await Promise.all([_.from("decks").select("formation,name,stadium_card_id").eq("id",d).single(),_.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation, evolution_bonus,
            player:players(id,firstname,surname_real,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
              clubs(encoded_name,logo_url)))`).eq("deck_id",d).order("slot_order")]);c=k[0].data,m=k[0].error,l=k[1].data,b=k[1].error}catch(k){console.error("[Match] Exception chargement deck:",k),Tt(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>o("home"));return}if(m||b){console.error("[Match] Erreur Supabase:",m||b),Tt(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>o("home"));return}const y=(l||[]).filter(k=>{var I;return k.is_starter&&((I=k.card)==null?void 0:I.player)}).map(k=>qi(k.card,k.position)),g=(l||[]).filter(k=>{var I;return!k.is_starter&&((I=k.card)==null?void 0:I.player)}).map(k=>qi(k.card,k.position));if(y.length<11){Tt(e,"⚠️",`Deck incomplet (${y.length}/11).`,"Compléter",()=>o("decks"));return}const p=(l||[]).find(k=>{var I;return((I=k.card)==null?void 0:I.card_type)==="formation"}),u=(c==null?void 0:c.formation)||((M=p==null?void 0:p.card)==null?void 0:M.formation)||"4-4-2",{data:x,error:h}=await _.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",r.profile.id).eq("card_type","game_changer"),{data:v}=await _.from("gc_definitions").select("*").eq("is_active",!0),$=(x||[]).map(k=>({...k,_gcDef:(v==null?void 0:v.find(I=>I.name===k.gc_type||I.id===k.gc_definition_id))||null}));let f=null;if(c!=null&&c.stadium_card_id){const{data:k}=await _.from("cards").select("stadium_id").eq("id",c.stadium_card_id).single();if(k!=null&&k.stadium_id){const{data:I}=await _.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").eq("id",k.stadium_id).single();f=I||null}}n({deckId:d,formation:u,starters:y,subsRaw:g,gcCardsEnriched:$,gcDefs:v||[],stadiumDef:f})}function It(){return Math.min(window.innerWidth-40,860)}function Nt(){return Math.round(It()*1.1)}function Kn(e){var r,o;if(!e)return null;const t=e._line||e.job||"MIL",i=t==="GK"?e.note_g||0:t==="DEF"?e.note_d||0:t==="MIL"?e.note_m||0:e.note_a||0,n=e.stadiumBonus?10:0;return{name:e.name,firstname:e.firstname||"",note:i+(e.boost||0)+n,note_g:e.note_g||0,note_d:e.note_d||0,note_m:e.note_m||0,note_a:e.note_a||0,_evolution_bonus:0,stadiumBonus:e.stadiumBonus||!1,boost:e.boost||0,job:e.job,job2:e.job2||null,_line:e._line||e.job,_col:e._col,country_code:e.country_code,club_id:e.club_id,rarity:e.rarity,clubName:e.clubName||((r=e.clubs)==null?void 0:r.encoded_name)||null,clubLogo:e.clubLogo||((o=e.clubs)==null?void 0:o.logo_url)||null,face:e.face||null,portrait:At(e)}}function ar(e,t,i,n="Adversaire"){An("/sounds/match-opening.mp3",.3);const r=It(),o=Nt();return`
    ...
    </div>`}function Vn(e){var o,a,s;if(!e)return"";const t=d=>d?Ie({...d,_evolution_bonus:0},{width:52,role:d._line||d.job,showStad:!!d.stadiumBonus,extraNote:d.boost||0}):"",n={goal:"⚽","goal-home":"⚽","goal-ai":"⚽",duel:"⚔️",midfield:"🎯",sub:"🔄",gc:"⚡",boost:"💥","defense-won":"🛡️","attack-won":"⚔️","defense-lost":"😓","attack-lost":"😓"}[e.type]||"📋";return`
    ...
    pointer-events:none;white-space:nowrap;max-width:90vw;text-align:center`,r.textContent=e,!document.getElementById("game-toast-style")){const o=document.createElement("style");o.id="game-toast-style",o.textContent=`
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${e.map((o,a)=>{const s=a<e.length-1?Ut(o,e[a+1]):null,d=!s||s==="#ff3333"||s==="#cc2222",c=s==="#00ff88"?"+10":s==="#FFD700"?"+5":"";return Yn(o,r),o.stadiumBonus||r&&(r.club_id&&String(o.club_id)===String(r.club_id)||r.country_code&&(o.country_code,r.country_code)),`
    ...
    </div>`}async function dr(e,t){const{state:i}=t,r=(i.params||{}).matchMode||"vs_ai_easy",o=r.replace("vs_ai_",""),a=r;await wi(e,t,r,async({deckId:s,formation:d,starters:c,subsRaw:l,gcCardsEnriched:m,gcDefs:b,stadiumDef:y})=>{try{let g=yi(c,d);y&&(g=ai(g,y),gi(l,y));const p=await lr(d,o),u=p.lines||p,x=async h=>{try{const v=a==="vs_ai_club"?"club":a,{data:$,error:f}=await _.from("matches").insert({home_id:i.profile.id,away_id:null,mode:v,home_deck_id:s,status:"in_progress"}).select().single();if(f){console.error("[MatchIA] Erreur création match:",f),Tt(e,"⚠️","Impossible de créer le match ("+f.message+").","Retour",()=>t.navigate("home"));return}const M=p.stadiumDef||null;M&&u&&(ai(u,M),gi(p.subs||[],M));const k={gcDefs:b||[],matchId:$==null?void 0:$.id,mode:a,difficulty:o,formation:d,homeTeam:g,aiTeam:u,homeSubs:l,subsUsed:0,maxSubs:Math.min(l.length,3),aiSubs:p.subs||[],aiSubsUsed:0,aiMaxSubs:Math.min((p.subs||[]).length,3),aiUsedSubIds:[],aiGcCards:p.gcCards||[],aiUsedGc:[],aiStadiumDef:M,homeScore:0,aiScore:0,gcCards:h,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};pr(e,k,t)}catch(v){console.error("[MatchIA] Exception launchMatch:",v),Tt(e,"⚠️","Erreur au lancement du match : "+v.message,"Retour",()=>t.navigate("home"))}};if(!m.length){x([]);return}hi(e,m,x)}catch(g){console.error("[MatchIA] Exception setup:",g),Tt(e,"⚠️","Erreur de préparation du match : "+g.message,"Retour",()=>t.navigate("home"))}})}async function lr(e,t){var u;const{data:i}=await _.from("players").select("id,firstname,surname_real,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(80);if(!i||i.length<11)return{lines:cr(e),subs:[],gcCards:[],stadiumDef:null};const n=Rt[e]||Rt["4-4-2"],r={GK:[],DEF:[],MIL:[],ATT:[]},o=new Set;function a(x,h,v){var $,f;return o.add(x.id),{cardId:"ai-"+x.id+"-"+v,id:x.id,firstname:x.firstname,name:x.surname_real,country_code:x.country_code,club_id:x.club_id,job:x.job,job2:x.job2,note_g:Number(x.note_g)||0,note_d:Number(x.note_d)||0,note_m:Number(x.note_m)||0,note_a:Number(x.note_a)||0,rarity:x.rarity,skin:x.skin,hair:x.hair,hair_length:x.hair_length,clubName:(($=x.clubs)==null?void 0:$.encoded_name)||null,clubLogo:((f=x.clubs)==null?void 0:f.logo_url)||null,boost:0,used:!1,_line:h}}for(const x of["GK","DEF","MIL","ATT"]){const h=i.filter(k=>k.job===x&&!o.has(k.id)),v=i.filter(k=>k.job!==x&&!o.has(k.id)),$=[...h,...v],f=[];for(let k=0;k<n[x];k++){const I=$[k];I&&f.push(a(I,x,k))}const M=yt(f.length);f.forEach((k,I)=>{k._col=M[I]}),r[x]=f}const d=i.filter(x=>!o.has(x.id)).slice(0,5).map((x,h)=>a(x,x.job,100+h)),m=Object.keys(je).sort(()=>Math.random()-.5).slice(0,3).map((x,h)=>{var v,$;return{id:"ai-gc-"+h,gc_type:x,name:((v=je[x])==null?void 0:v.name)||x,icon:(($=je[x])==null?void 0:$.icon)||"⚡"}}),b=Object.values(r).flat(),y={};b.forEach(x=>{x.club_id&&(y[x.club_id]=(y[x.club_id]||0)+1)});const g=(u=Object.entries(y).sort((x,h)=>h[1]-x[1])[0])==null?void 0:u[0];let p=null;if(g){const{data:x}=await _.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",g).single();x&&(p={club_id:x.id,country_code:null,name:x.encoded_name+" Stadium",club:{encoded_name:x.encoded_name,logo_url:x.logo_url}})}return{lines:r,subs:d,gcCards:m,stadiumDef:p}}function cr(e){const t=Rt[e]||Rt["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},n=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let r=0;for(const o of["GK","DEF","MIL","ATT"]){const a=[];for(let d=0;d<t[o];d++){const c=3+Math.floor(Math.random()*5);a.push({cardId:"fake-"+r,id:"fake-"+r,firstname:"IA",name:n[r%n.length],country_code:"XX",club_id:null,job:o,job2:null,note_g:o==="GK"?c:2,note_d:o==="DEF"?c:2,note_m:o==="MIL"?c:2,note_a:o==="ATT"?c:2,rarity:"normal",boost:0,used:!1,_line:o}),r++}const s=yt(a.length);a.forEach((d,c)=>{d._col=s[c]}),i[o]=a}return i}function pr(e,t,i){var r;An("/sounds/match-opening.mp3",.3);const n=`<div style="position:relative;width:22px;height:22px;flex-shrink:0;display:flex;align-items:center;justify-content:center">
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const Y=t.pendingAttack;let pe="";if(t.selected.length>0){const U=t.selected.map(E=>({...(t.homeTeam[E._role]||[]).find(H=>H.cardId===E.cardId)||E,_line:E._role})),V=ri(U,t.modifiers.home,t.formation);pe=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
          ...
          </div>`}const C=t.log[t.log.length-1];return C?'<div style="padding:2px 4px">'+Vn(C)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})()}
    </div>
    ...
    ${(()=>{const C=window.innerWidth>=700,Y=(q,G,se)=>{var Ae,Be;const de=(t.gcDefs||[]).find(ke=>ke.name===q.gc_type),he=(de==null?void 0:de.name)||q.gc_type,Te=(de==null?void 0:de.effect)||((Ae=je[q.gc_type])==null?void 0:Ae.desc)||"",ve=de!=null&&de.image_url?`/icons/${de.image_url}`:null,Se=((Be=je[q.gc_type])==null?void 0:Be.icon)||"⚡";return`<div class="gc-mini" data-gc-id="${q.id}" data-gc-type="${q.gc_type}" style="flex-shrink:0;cursor:pointer">
          ${tt(he,ve,Se,Te,{width:G})}
          ...
          </div>`},U=(q,G)=>G?pe(95,162):Y(q,95),V=(q,G)=>G?pe(68,116):Y(q,68),E=C?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",F=h?`<button id="btn-results" style="${E};background:linear-gradient(135deg,#D4A017,#FFD700);border:none;color:#000">🏁 Résultats</button>`:p?`<div style="${E};background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);color:rgba(255,255,255,0.4)">⏳ Tour IA</div>`:f?`<button id="btn-pass" style="${E};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER (plus d'attaquants)</button>`:u?`<button id="btn-action" style="${E};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${t.selected.length===0?"disabled":""}> ⚔️ ATTAQUEZ <span id="match-timer" style="font-weight:900"></span></button>`:x?`<button id="btn-action" style="${E};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${t.selected.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="match-timer" style="font-weight:900"></span></button>`:`<div style="${E};background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1)"></div>`,H=u||x?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${t.selected.length}/3 sélectionné(s)</div>`:"",R=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${C?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
        ${o.length===0?'<div style="font-size:7px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':o.map(q=>`<div class="sub-btn-col" data-sub-id="${q.cardId}" style="cursor:pointer;flex-shrink:0">${ht(q,76,100)}</div>`).join("")}
        ...
        <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
          ${Ot(t.homeTeam,t.formation,t.phase,n,C?1300:It(),C?600:Nt(),g)}
        </div>
      </div>`;return C?`
  ...
  <div id="match-history-panel">
    ...
    </div>`,(f=m.querySelector("#sub-close"))==null||f.addEventListener("click",()=>m.remove()),(M=m.querySelector("#out-up"))==null||M.addEventListener("click",()=>{s>0&&(s--,b())}),(k=m.querySelector("#out-down"))==null||k.addEventListener("click",()=>{s<o.length-1&&(s++,b())}),(I=m.querySelector("#in-up"))==null||I.addEventListener("click",()=>{c>0&&(c--,b())}),(B=m.querySelector("#in-down"))==null||B.addEventListener("click",()=>{c<a.length-1&&(c++,b())});const $=(te,ne,z,S)=>{const K=m.querySelector("#"+te);if(!K)return;let J=0;K.addEventListener("touchstart",ue=>{J=ue.touches[0].clientY},{passive:!0}),K.addEventListener("touchend",ue=>{const fe=ue.changedTouches[0].clientY-J;if(Math.abs(fe)<30)return;const C=ne();fe<0&&C<S-1?(z(C+1),b()):fe>0&&C>0&&(z(C-1),b())},{passive:!0})};$("in-panel",()=>c,te=>c=te,a.length),$("out-panel",()=>s,te=>s=te,o.length),(ee=m.querySelector("#sub-confirm"))==null||ee.addEventListener("click",te=>{if(te.preventDefault(),te.stopPropagation(),l)return;l=!0;const ne=o[s],z=a[c];if(!ne||!z)return;let S=null,K=-1;for(const[ue,fe]of Object.entries(t.homeTeam)){const C=(fe||[]).findIndex(Y=>Y.cardId===ne.cardId);if(C!==-1){S=ue,K=C;break}}if(K===-1||!S){$t("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),m.remove();return}const J={...z,_line:S,_col:ne._col||0,used:!1,boost:0};t.homeTeam[S].splice(K,1,J),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(z.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:ne.name,firstname:ne.firstname,note:Pe(ne,S),portrait:At(ne),job:ne.job,country_code:ne.country_code,rarity:ne.rarity,clubName:ne.clubName,clubLogo:ne.clubLogo},inPlayer:{name:z.name,firstname:z.firstname,note:Pe(z,S),portrait:At(z),job:z.job,country_code:z.country_code,rarity:z.rarity,clubName:z.clubName,clubLogo:z.clubLogo},text:`🔄 ${z.firstname} ${z.name} remplace ${ne.firstname} ${ne.name}`}),m.remove(),sr(ne,z,()=>we(e,t,i))})}document.body.appendChild(m),b()}function vr(e,t,i,n,r){var b,y;const o=(n.gcDefs||[]).find(g=>g.name===t),a=je[t]||{icon:"⚡",desc:"Carte spéciale."},s=(o==null?void 0:o.name)||t,d=(o==null?void 0:o.effect)||a.desc,c=o!=null&&o.image_url?`/icons/${o.image_url}`:null,l=a.icon||"⚡",m=document.createElement("div");m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",m.innerHTML=`
    ${tt(s,c,l,d,{width:190})}
    <!-- Boutons -->
    <div style="display:flex;gap:12px;width:190px">
      <button id="gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
      <button id="gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
    </div>`,document.body.appendChild(m),(b=m.querySelector("#gc-back"))==null||b.addEventListener("click",()=>m.remove()),(y=m.querySelector("#gc-use"))==null||y.addEventListener("click",()=>{m.remove(),_r(e,t,i,n,r)})}function pi(e,t,i,n,r,o){const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let s=[];function d(){var c,l;a.innerHTML=`
    ...
    <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start;justify-content:center">
      ${e.map(m=>{const b=m._line||m.job||"MIL",y=s.find(p=>p.cardId===m.cardId),g=Ie({...m,_evolution_bonus:0},{width:90,showStad:!0,role:b,extraNote:m.boost||0});return`<div class="gc-pick-item" data-cid="${m.cardId}"
    ...
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      ...
      ${_i(e.aiTeam,e.formation,null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.1))}
    </div>`,`<button class="btn btn-primary" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const $r={pepite:{win:2,loss:-1,boosterStart:"min"},papyte:{win:1,loss:-2,boosterStart:"max"}};function Er(e){const t=e.job||"ATT";return Number(t==="GK"?e.note_g:t==="DEF"?e.note_d:t==="MIL"?e.note_m:e.note_a)||0}async function Lr(e,t){!e||!t||await Promise.all([wn(e,"win"),wn(t,"loss")])}async function wn(e,t){const{data:i}=await _.from("cards").select("id, current_note, player:players(rarity, job, note_g, note_d, note_m, note_a, note_min, note_max)").eq("owner_id",e).eq("card_type","player");if(!(i!=null&&i.length))return;const n=i.filter(r=>{var o,a;return((o=r.player)==null?void 0:o.rarity)==="pepite"||((a=r.player)==null?void 0:a.rarity)==="papyte"});n.length&&await Promise.all(n.map(r=>{const o=$r[r.player.rarity],a=t==="win"?o.win:o.loss,s=r.player.note_min??1,d=r.player.note_max??10,c=Er(r.player),l=r.current_note??c,m=Math.min(d,Math.max(s,l+a));return _.from("cards").update({current_note:m}).eq("id",r.id)}))}const qt=e=>Kn(e);async function Mt(e,t,i,n,r={}){return Xn(e,t,i,n,r.myGC||[],r.gcDefs||[],r.isRanked||!1,r.rankedData||null,r.stadiumDef||null,r.onMatchEnd||null,r.mlLeagueId||null,r.mlMatchId||null)}async function Tr(e,t,i){const{data:n}=await _.from("matches").select("home_id,away_id,mode,is_ranked").eq("id",i).single();if(!n){t.toast("Match introuvable","error"),t.navigate("home");return}const r=n.home_id===t.state.user.id;let o=null,a=null;if(n.mode==="mini_league"){const{data:s}=await _.from("mini_league_matches").select("id, league_id").eq("match_id",i).maybeSingle();s&&(o=s.league_id,a=s.id)}return Xn(e,t,i,r,[],[],n.is_ranked||!1,null,null,null,o,a)}async function Xn(e,t,i,n,r=[],o=[],a=!1,s=null,d=null,c=null,l=null,m=null){const{state:b,navigate:y,toast:g}=t,p=n?"p1":"p2",u=n?"p2":"p1",x=(r||[]).map(w=>w.id),h=(r||[]).map(w=>({id:w.id,gc_type:w.gc_type,_gcDef:w._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:v}=await _.from("matches").select("*").eq("id",i).single();if(!v){g("Match introuvable","error"),y("home");return}async function $(){var ye,be;console.log("[PvP] buildGameState match:",{id:v.id,home_deck_id:v.home_deck_id,away_deck_id:v.away_deck_id,mode:v.mode,is_ranked:v.is_ranked});const[{data:w,error:L},{data:A,error:j},{data:T},{data:N}]=await Promise.all([_.rpc("get_deck_for_match",{p_deck_id:v.home_deck_id}),_.rpc("get_deck_for_match",{p_deck_id:v.away_deck_id}),_.from("users").select("id,pseudo,club_name").eq("id",v.home_id).single(),_.from("users").select("id,pseudo,club_name").eq("id",v.away_id).single()]);console.log("[PvP] get_deck_for_match p1:",L==null?void 0:L.message,"p2:",j==null?void 0:j.message,"p1D:",(ye=w==null?void 0:w.starters)==null?void 0:ye.length,"p2D:",(be=A==null?void 0:A.starters)==null?void 0:be.length);const D=oe=>{const Ee=Number(oe.evolution_bonus)||0;return{cardId:oe.card_id,position:oe.position,id:oe.id,firstname:oe.firstname,name:oe.surname_real,country_code:oe.country_code,club_id:oe.club_id,job:oe.job,job2:oe.job2,note_g:(Number(oe.note_g)||0)+(oe.job==="GK"||oe.job2==="GK"&&Number(oe.note_g)>0?Ee:0),note_d:(Number(oe.note_d)||0)+(oe.job==="DEF"||oe.job2==="DEF"&&Number(oe.note_d)>0?Ee:0),note_m:(Number(oe.note_m)||0)+(oe.job==="MIL"||oe.job2==="MIL"&&Number(oe.note_m)>0?Ee:0),note_a:(Number(oe.note_a)||0)+(oe.job==="ATT"||oe.job2==="ATT"&&Number(oe.note_a)>0?Ee:0),evolution_bonus:Ee,rarity:oe.rarity,skin:oe.skin,hair:oe.hair,hair_length:oe.hair_length,face:oe.face||null,clubName:oe.club_encoded_name||null,clubLogo:oe.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},P=((w==null?void 0:w.starters)||[]).map(oe=>D(oe)),O=((A==null?void 0:A.starters)||[]).map(oe=>D(oe)),X=(w==null?void 0:w.formation)||"4-4-2",ie=(A==null?void 0:A.formation)||"4-4-2";let Z=yi(P,X),Q=yi(O,ie);const ae=((w==null?void 0:w.subs)||[]).map(oe=>D(oe)),le=((A==null?void 0:A.subs)||[]).map(oe=>D(oe)),ge=(w==null?void 0:w.stadium_def)||(n?d:null),ce=(A==null?void 0:A.stadium_def)||(n?null:d);return ge&&(Z=ai(Z,ge),gi(ae,ge)),ce&&(Q=ai(Q,ce),gi(le,ce)),{p1Team:Z,p2Team:Q,p1Subs:ae,p2Subs:le,p1Formation:X,p2Formation:ie,p1Name:(T==null?void 0:T.club_name)||(T==null?void 0:T.pseudo)||"Joueur 1",p2Name:(N==null?void 0:N.club_name)||(N==null?void 0:N.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:n?x:[],gc_p2:n?[]:x,gcCardsFull_p1:n?h:[],gcCardsFull_p2:n?[]:h,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:o||[],lastActionAt:new Date().toISOString()}}let f=v.game_state&&Object.keys(v.game_state).length?v.game_state:null;if(console.log("[PvP] init - amIHome:",n,"gameState exists:",!!f,"match.status:",v.status,"home_id:",v.home_id,"away_id:",v.away_id,"myId:",b.profile.id),!f)if(n){f=await $(),console.log("[PvP] home - gameState built, p1Team keys:",Object.keys((f==null?void 0:f.p1Team)||{}));const{data:w}=await _.from("matches").select("game_state").eq("id",i).single();!(w!=null&&w.game_state)||!Object.keys(w.game_state).length?await _.from("matches").update({game_state:f,turn_user_id:v.home_id}).eq("id",i):f=w.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>',console.log("[PvP] away - waiting for home to write game_state...");for(let w=0;w<30&&!f;w++){await new Promise(A=>setTimeout(A,400));const{data:L}=await _.from("matches").select("game_state").eq("id",i).single();L!=null&&L.game_state&&Object.keys(L.game_state).length&&(f=L.game_state),w%5===0&&console.log("[PvP] away - poll",w,"game_state:",!!(L!=null&&L.game_state))}if(!f){g("Erreur de synchronisation","error"),y("home");return}f.gc_p2=x,f.gcCardsFull_p2=h,await _.from("matches").update({game_state:f}).eq("id",i)}let M=!1,k=null,I=!1;const B=new Set,ee=new Set;async function te(w){var ie,Z;Gt();try{_.removeChannel(ne)}catch{}const L=f[p+"Score"]||0,A=f[u+"Score"]||0;let j,T;w.winner_id?(j=w.winner_id===b.profile.id,T=!1):w.forfeit?(j=L>A,T=!1):(T=L===A,j=L>A);let N="";if(a&&n)try{const{data:Q}=await _.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",v.home_id).single(),{data:ae}=await _.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",v.away_id).single();if(Q&&ae){const le=j?1:T?.5:0,ge=1-le,ce=Q.placement_matches<10,ye=ae.placement_matches<10,be=computeGlicko2(Q.mmr,Q.mmr_deviation,Q.mmr_volatility,ae.mmr,ae.mmr_deviation,le===1?1:le===0?0:.5,ce),oe=computeGlicko2(ae.mmr,ae.mmr_deviation,ae.mmr_volatility,Q.mmr,Q.mmr_deviation,ge===1?1:ge===0?0:.5,ye);await _.rpc("update_mmr_after_ranked",{p_match_id:i,p_winner_id:T?null:j?v.home_id:v.away_id,p_home_id:v.home_id,p_away_id:v.away_id,p_home_delta:be.delta,p_away_delta:oe.delta,p_home_new_rd:be.newRd,p_away_new_rd:oe.newRd,p_home_new_vol:be.newSigma,p_away_new_vol:oe.newSigma});const Ee=be.delta,Ne=be.newMmr,Le=getTier(Ne),Ve=Ee>=0?"+":"",Oe=Ee>=0?"#4caf50":"#ff6b6b",nt=getTier(Q.mmr);N=`
            ...
            </div>`}}catch(Q){console.error("[Ranked] MMR update error:",Q)}(ie=document.getElementById("pvp-end-overlay"))==null||ie.remove();const D=document.createElement("div");D.id="pvp-end-overlay",D.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;color:#fff;padding:24px;text-align:center;overflow-y:auto";const P=T?"🤝":j?"🏆":"😞",O=T?"MATCH NUL":j?"VICTOIRE !":"DÉFAITE",X=T?"#fff":j?"#FFD700":"#ff6b6b";D.innerHTML=`
      <div style="font-size:64px">${P}</div>
      <div style="font-size:26px;font-weight:900;color:${X}">${O}</div>
      ...
      ${w.forfeit?`<div style="font-size:13px;color:rgba(255,255,255,0.5)">${j?"L'adversaire a quitté":"Perdu par forfait"}</div>`:""}
      ${N}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${l?"🏆 Retour à la Mini League":a?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(D),(Z=D.querySelector("#pvp-end-home"))==null||Z.addEventListener("click",()=>{D.remove(),it(e),l?y("mini-league",{openLeagueId:l}):y(a?"ranked":"home")})}const ne=_.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},w=>{var A;const L=w.new;try{if(L.status==="finished"||L.forfeit){if(M)return;M=!0,k&&(clearInterval(k),k=null),Ue(),L.game_state&&(f=L.game_state),te(L);return}if(L.game_state){const j=f;f=L.game_state;const T=f._lastGC;if(T&&T.seq&&!ee.has(T.seq)&&(ee.add(T.seq),T.by!==p)){E(T.type,T.by,()=>Y());return}const N=j[p+"Score"]||0,D=j[u+"Score"]||0,P=f[p+"Score"]||0,O=f[u+"Score"]||0,X=P>N,ie=O>D;if((X||ie)&&!B.has(f.round)){B.add(f.round);const Z=[...f.log||[]].reverse().find(le=>le.isGoal),Q=((Z==null?void 0:Z.homePlayers)||[]).map(le=>({name:le.name,note:le.note,portrait:le.portrait,job:le.job}));!!((A=Z==null?void 0:Z.text)!=null&&A.includes("DERNIER CORNER"))?Te().then(()=>{Qe(Q,P,O,X,()=>Y())}):Qe(Q,P,O,X,()=>Y());return}Y()}}catch(j){console.error("[PvP] crash:",j)}}).subscribe();async function z(w){Object.assign(f,w),f.lastActionAt=new Date().toISOString();const{error:L}=await _.from("matches").update({game_state:f}).eq("id",i);L&&g("Erreur de synchronisation","error");try{Y()}catch(A){console.error("[PvP] renderPvpScreen crash:",A)}}async function S(){if(M)return;M=!0,Gt(),Ue(),k&&(clearInterval(k),k=null);const w=n?v.away_id:v.home_id,L=n?"p2":"p1",A=n?"p1":"p2",j={...f,[L+"Score"]:3,[A+"Score"]:0,phase:"finished"},T=j.p1Score||0,N=j.p2Score||0;try{await _.from("matches").update({game_state:j}).eq("id",i);const{data:D,error:P}=await _.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:w,p_home_score:T,p_away_score:N,p_forfeit:!0});if(P&&console.error("[PvP] finish_pvp_match (forfeit):",P),l)try{await _.rpc("check_and_finish_mini_league",{p_league_id:l})}catch(O){console.error("[PvP] check_and_finish_mini_league (forfeit):",O)}try{const{data:O}=await _.rpc("apply_match_rewards",{p_match_id:i});O!=null&&O.success&&!(O!=null&&O.skipped)&&typeof t.refreshProfile=="function"&&await t.refreshProfile()}catch(O){console.error("[PvP] apply_match_rewards (forfeit):",O)}}catch{}try{_.removeChannel(ne)}catch{}setTimeout(()=>{it(e),l?y("mini-league",{openLeagueId:l}):y("home")},800)}const K={BOOST_STAT:({value:w=1,count:L=1,roles:A=[]},j,T)=>{const N=j[p+"Team"],D=Object.entries(N).filter(([P])=>!A.length||A.includes(P)).flatMap(([P,O])=>O.filter(X=>!X.used).map(X=>({...X,_line:P})));if(!D.length){j.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),T(j);return}J(D,L,`Choisir ${L} joueur(s) à booster (+${w})`,P=>{P.forEach(O=>{const X=(N[O._line]||[]).find(ie=>ie.cardId===O.cardId);X&&(X.boost=(X.boost||0)+w,j.log.push({text:`⚡ +${w} sur ${X.name}`,type:"info"}))}),j[p+"Team"]=N,T(j)})},DEBUFF_STAT:({value:w=1,count:L=1,roles:A=[],target:j="ai"},T,N)=>{const D=j==="home"?p:u,P=T[D+"Team"],O=j==="home"?"allié":"adverse",X=Object.entries(P).filter(([ie])=>!A.length||A.includes(ie)).flatMap(([ie,Z])=>Z.map(Q=>({...Q,_line:ie})));if(!X.length){T.log.push({text:`🎯 Aucun joueur ${O}`,type:"info"}),N(T);return}J(X,L,`Choisir ${L} joueur(s) ${O}(s) (-${w})`,ie=>{ie.forEach(Z=>{const Q=(P[Z._line]||[]).find(ae=>ae.cardId===Z.cardId);Q&&(Q.boost=(Q.boost||0)-w,T.log.push({text:`🎯 -${w} sur ${Q.name}`,type:"info"}))}),T[D+"Team"]=P,N(T)})},GRAY_PLAYER:({count:w=1,roles:L=[],target:A="ai"},j,T)=>{const N=A==="home"?p:u,D=j[N+"Team"],P=A==="home"?"allié":"adverse",O=Object.entries(D).filter(([X])=>!L.length||L.includes(X)).flatMap(([X,ie])=>ie.filter(Z=>!Z.used).map(Z=>({...Z,_line:X})));if(!O.length){j.log.push({text:`❌ Aucun joueur ${P}`,type:"info"}),T(j);return}J(O,w,`Choisir ${w} joueur(s) ${P}(s) à exclure`,X=>{const ie="usedCardIds_"+N,Z=new Set(j[ie]||[]);X.forEach(Q=>{const ae=(D[Q._line]||[]).find(le=>le.cardId===Q.cardId);ae&&(ae.used=!0,Z.add(Q.cardId),j.log.push({text:`❌ ${ae.name} exclu !`,type:"info"}))}),j[ie]=[...Z],j[N+"Team"]=D,T(j)})},REVIVE_PLAYER:({count:w=1,roles:L=[]},A,j)=>{const T=A[p+"Team"],N=Object.entries(T).filter(([D])=>!L.length||L.includes(D)).flatMap(([D,P])=>P.filter(O=>O.used).map(O=>({...O,_line:D})));if(!N.length){A.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),j(A);return}J(N,w,`Choisir ${w} joueur(s) à ressusciter`,D=>{D.forEach(P=>{const O=(T[P._line]||[]).find(X=>X.cardId===P.cardId);O&&(O.used=!1,A.log.push({text:`💫 ${O.name} ressuscité !`,type:"info"}))}),A[p+"Team"]=T,j(A)})},REMOVE_GOAL:({},w,L)=>{const A=u+"Score";w[A]>0?(w[A]--,w.log.push({text:"🚫 Dernier but annulé !",type:"info"})):w.log.push({text:"🚫 Aucun but à annuler",type:"info"}),L(w)},ADD_GOAL_DRAW:({},w,L)=>{w[p+"Score"]===w[u+"Score"]?(w[p+"Score"]++,w.log.push({text:"🎯 But bonus !",type:"info"})):w.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),L(w)},ADD_SUB:({value:w=1},L,A)=>{L.maxSubs=(L.maxSubs||3)+w,L.log.push({text:`🔄 +${w} remplacement(s)`,type:"info"}),A(L)},CUSTOM:({},w,L)=>L(w)};function J(w,L,A,j){const T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let N=[];function D(){var O,X;const P=w.map(ie=>{const Z=ie._line||ie.job||"MIL",Q=N.find(le=>le.cardId===ie.cardId),ae=Ie({...ie,_evolution_bonus:0},{width:90,showStad:!0,role:Z,extraNote:ie.boost||0});return`<div class="pp-item" data-cid="${ie.cardId}"
        ...
        </div>`,(O=T.querySelector("#pp-close"))==null||O.addEventListener("click",()=>T.remove()),T.querySelectorAll(".pp-item").forEach(ie=>{ie.addEventListener("click",()=>{const Z=ie.dataset.cid,Q=w.find(le=>le.cardId===Z),ae=N.findIndex(le=>le.cardId===Z);Q&&(ae>-1?N.splice(ae,1):N.length<L&&N.push(Q),D())})}),(X=T.querySelector("#pp-confirm"))==null||X.addEventListener("click",()=>{T.remove(),j(N)})}D(),document.body.appendChild(T)}async function ue(w,L){const j=(f["gcCardsFull_"+p]||[]).find(P=>P.id===w),T=(j==null?void 0:j._gcDef)||(f.gcDefs||[]).find(P=>{var O;return P.name===L||((O=P.name)==null?void 0:O.toLowerCase().trim())===(L==null?void 0:L.toLowerCase().trim())}),N=[...f["usedGc_"+p]||[],w],D={type:L,by:p,seq:(f._gcAnimSeq||0)+1};ee.add(D.seq),E(L,p,async()=>{if(T!=null&&T.effect_type&&T.effect_type!=="CUSTOM"){const O=K[T.effect_type];if(O){const X={...f};O(T.effect_params||{},X,async ie=>{ie["usedGc_"+p]=N,ie._lastGC=D,ie._gcAnimSeq=D.seq,await z(ie)});return}}const P={...f};switch(L){case"Remplacement+":P.maxSubs=(P.maxSubs||3)+1,P.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const O=u+"Score";P[O]>0&&(P[O]--,P.log.push({text:"🚫 But annulé",type:"info"}));break}}P["usedGc_"+p]=N,P._lastGC=D,P._gcAnimSeq=D.seq,await z(P)})}function fe(w,L){const A="usedCardIds_"+w,j=new Set(f[A]||[]);L.forEach(T=>j.add(T)),f[A]=[...j]}function C(){for(const w of["p1","p2"]){const L=new Set(f["usedCardIds_"+w]||[]),A=f[w+"Team"];if(A)for(const j of["GK","DEF","MIL","ATT"])(A[j]||[]).forEach(T=>{T.used=L.has(T.cardId)})}}function Y(){var nn,on,rn,an,sn,dn;if(f.phase==="reveal")return pe();if(f.phase==="midfield")return V();if(f.phase==="finished")return Ge();const w=f[p+"Team"],L=f[u+"Team"];C();const A=f[p+"Score"],j=f[u+"Score"],T=f[p+"Name"],N=f[u+"Name"],D=f.phase===p+"-attack",P=f.phase===p+"-defense",O=Array.isArray(f["selected_"+p])?f["selected_"+p]:[],X=O.map(W=>W.cardId),ie=window.innerWidth>=700,Z=f[p+"Subs"]||[],Q=f["usedSubIds_"+p]||[],ae=Z.filter(W=>!Q.includes(W.cardId)),le=f["gcCardsFull_"+p]||[],ge=f["usedGc_"+p]||[],ce=le.filter(W=>!ge.includes(W.id)),ye=f.boostOwner===p&&!f.boostUsed,be=[...w.MIL||[],...w.ATT||[]].filter(W=>!W.used),oe=[...L.MIL||[],...L.ATT||[]].filter(W=>!W.used),Ee=!ke(L),Ne=be.length===0&&oe.length===0&&!Ee,Le=(w.DEF||[]).filter(W=>!W.used),Ve=(w.GK||[]).filter(W=>!W.used);let Oe=[];D&&be.length===0&&(Ee?(Oe=Le.map(W=>W.cardId),Le.length===0&&(Oe=Oe.concat(Ve.map(W=>W.cardId)))):Ne&&(Oe=[...Le,...Ve].map(W=>W.cardId)));function nt(W,me,ze){var Me,ft;const _e=W._gcDef,Bt=(_e==null?void 0:_e.name)||W.gc_type,at=(_e==null?void 0:_e.effect)||((Me=je[W.gc_type])==null?void 0:Me.desc)||"",Fe=_e!=null&&_e.image_url?`/icons/${_e.image_url}`:null,Re=((ft=je[W.gc_type])==null?void 0:ft.icon)||"⚡";return`<div class="pvp-gc-mini" data-gc-id="${W.id}" data-gc-type="${W.gc_type}" style="flex-shrink:0;cursor:pointer">
      ...
      </div>`}const De=(W,me)=>me?Kt(95,162):nt(W,95),Ye=(W,me)=>me?Kt(68,116):nt(W,68),He=ie?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",pt=D?ct(p)?`<button id="pvp-action" style="${He};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${O.length===0?"disabled":""}>⚔️ ATTAQUEZ <span id="pvp-timer"></span></button>`:`<button id="pvp-action" data-pass="1" style="${He};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER <span id="pvp-timer"></span></button>`:P?`<button id="pvp-action" style="${He};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${O.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="pvp-timer"></span></button>`:`<div style="font-size:11px;color:rgba(255,255,255,0.3);text-align:center;padding:4px">⏳ Tour de ${N}</div>`,ot=D&&!ct(p)?'<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">Aucun attaquant — passez la main</div>':D||P?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${O.length}/3 sélectionné(s)</div>`:"",Ct=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${ie?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
      ${ae.length===0?'<div style="font-size:7px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':ae.map(W=>`<div class="pvp-sub-btn" data-sub-id="${W.cardId}" style="cursor:pointer;flex-shrink:0">${ht(W,ie?76:44,ie?100:58)}</div>`).join("")}
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ...
        </div>`}const W=(f.log||[]).slice(-1)[0];return W?'<div style="padding:2px 4px">'+Vn(W)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3);text-align:center">⏳ Match en cours...</div>'})(),tn=`
      <div style="display:flex;align-items:center;padding:8px 10px;background:rgba(0,0,0,0.5);gap:6px;flex-shrink:0">
      ...
      <div style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">${oo}</div>
      ...
      <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
        ...
        <div id="mobile-play-area" style="flex:1;min-height:0;display:flex;overflow:hidden">
          <div id="match-field" style="flex:1;min-width:0;min-height:0;overflow:hidden">
            <div class="terrain-wrapper" style="width:100%;height:100%;overflow:hidden">
              ${Ot(w,f[p+"Formation"],ut,X,It(),Nt(),Oe)}
            </div>
          </div>
        </div>
        <div id="mobile-action-bar" style="position:absolute;left:0;right:0;bottom:0;z-index:20;background:rgba(0,0,0,0.55);padding:6px 8px 8px;display:flex;flex-direction:column;gap:6px;box-shadow:0 -4px 16px rgba(0,0,0,0.5)">
          <div style="display:flex;gap:6px;overflow-x:auto;align-items:flex-end;min-height:96px;padding-bottom:2px">
            ${ce.map(W=>Ye(W,!1)).join("")}
            ${ye?Ye(null,!0):""}
      ...
      </div>`;function jt(){const W=e.querySelector(".match-screen");if(!W)return;const me=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);W.style.height=me+"px",W.style.minHeight=me+"px",W.style.maxHeight=me+"px",W.style.overflow="hidden";const ze=e.querySelector("#mobile-action-bar"),_e=e.querySelector("#mobile-play-area");ze&&_e&&(_e.style.paddingBottom=ze.offsetHeight+"px")}if(jt(),setTimeout(jt,120),setTimeout(jt,400),I||(I=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",jt),window.visualViewport.addEventListener("scroll",jt)),window.addEventListener("resize",jt)),function(){const me=e.querySelector("#match-field .terrain-wrapper svg")||e.querySelector(".terrain-wrapper svg");if(!me)return;const ze=me.closest("#match-terrain-wrap");ze&&(ze.style.cssText="position:relative;width:100%;height:100%;padding:0"),me.removeAttribute("width"),me.removeAttribute("height"),me.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",me.setAttribute("preserveAspectRatio","xMidYMid meet")}(),f._pvpResizeBound||(f._pvpResizeBound=!0,window.addEventListener("resize",()=>{const W=e.querySelector(".terrain-wrapper svg");if(W){const me=W.closest("#match-terrain-wrap");me&&(me.style.cssText="position:relative;width:100%;height:100%;padding:0"),W.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0"}})),e.querySelectorAll(".match-slot-hit").forEach(W=>{W.addEventListener("click",()=>{if(!D&&!P)return;const me=W.dataset.cardId,ze=W.dataset.role,_e=(w[ze]||[]).find(Re=>Re.cardId===me);if(!_e||_e.used)return;const Bt=Oe.includes(me);if(D&&!["MIL","ATT"].includes(ze)&&!Bt)return;Array.isArray(f["selected_"+p])||(f["selected_"+p]=[]);const at=f["selected_"+p],Fe=at.findIndex(Re=>Re.cardId===me);Fe>-1?at.splice(Fe,1):at.length<3&&at.push({..._e,_role:ze}),Y()})}),e.querySelectorAll(".match-used-hit").forEach(W=>{W.addEventListener("click",()=>R(W.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(W=>{W.addEventListener("click",()=>R())}),(nn=e.querySelector("#pvp-sub-open"))==null||nn.addEventListener("click",()=>R()),e.querySelectorAll(".pvp-gc-mini").forEach(W=>{D?W.addEventListener("click",()=>F(W.dataset.gcId,W.dataset.gcType)):(W.style.opacity="0.35",W.style.cursor="default",W.addEventListener("click",()=>$t("⚡ Les Game Changers ne sont utilisables qu'en attaque","rgba(180,100,0,0.9)")))}),(on=e.querySelector("#pvp-boost-card"))==null||on.addEventListener("click",()=>H()),(rn=e.querySelector("#pvp-action"))==null||rn.addEventListener("click",W=>{D?W.currentTarget.dataset.pass==="1"||!ct(p)?se():Se():P&&Ae()}),(an=e.querySelector("#pvp-quit"))==null||an.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&S()}),(sn=e.querySelector("#pvp-view-opp"))==null||sn.addEventListener("click",()=>q()),(dn=e.querySelector("#pvp-toggle-history"))==null||dn.addEventListener("click",()=>G()),k&&(clearInterval(k),k=null),Ue(),(D||P)&&!M){let W=30,me=!1;const ze=()=>document.getElementById("pvp-timer"),_e=()=>{ze()&&(ze().textContent=W+"s",ze().style.color=me?"#ff4444":"#fff")};_e(),k=setInterval(()=>{W--,W<0?me?(clearInterval(k),k=null,Ue(),D&&!ct(p)?se():S()):(me=!0,W=15,_e(),zn("/sounds/timer-urgent.mp3",.6)):_e()},1e3)}}function pe(){st(e),e.innerHTML=`
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="animation:gcFlipIn .7s cubic-bezier(.34,1.56,.64,1) both,gcGlow 1.8s ease infinite .7s">
        ${tt(N,P,O,D,{width:200})}
      </div>
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(Z);let Q=!1;const ae=()=>{Q||(Q=!0,Z.remove(),setTimeout(()=>A&&A(),50))};Z.addEventListener("click",ae),setTimeout(ae,3e3)}function F(w,L){var ie,Z,Q,ae;const j=(f["gcCardsFull_"+p]||[]).find(le=>le.id===w),T=j==null?void 0:j._gcDef,N=(T==null?void 0:T.name)||L,D=(T==null?void 0:T.effect)||((ie=je[L])==null?void 0:ie.desc)||"Carte spéciale.",P=T!=null&&T.image_url?`/icons/${T.image_url}`:null,O=((Z=je[L])==null?void 0:Z.icon)||"⚡",X=document.createElement("div");X.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",X.innerHTML=`
      ${tt(N,P,O,D,{width:190})}
      <div style="display:flex;gap:12px;width:190px">
        <button id="pvp-gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
        <button id="pvp-gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
      </div>`,document.body.appendChild(X),(Q=X.querySelector("#pvp-gc-back"))==null||Q.addEventListener("click",()=>X.remove()),(ae=X.querySelector("#pvp-gc-use"))==null||ae.addEventListener("click",()=>{X.remove(),ue(w,L)})}function H(){var j;const w=f[p+"Team"],L=Object.entries(w).flatMap(([T,N])=>(N||[]).filter(D=>!D.used).map(D=>({...D,_line:T})));if(!L.length)return;const A=document.createElement("div");A.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",A.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
      ...
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${L.map(T=>{const N={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[T._line]||"#555",D=Pe(T,T._line)+(T.boost||0);return`<div class="bp-item" data-cid="${T.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${N};overflow:hidden;cursor:pointer">
      ...
      </div>`,document.body.appendChild(A),(j=A.querySelector("#bp-close"))==null||j.addEventListener("click",()=>A.remove()),A.querySelectorAll(".bp-item").forEach(T=>{T.addEventListener("click",async()=>{const N=T.dataset.cid,D=L.find(O=>O.cardId===N);if(!D)return;const P=(w[D._line]||[]).find(O=>O.cardId===N);P&&(P.boost=(P.boost||0)+f.boostValue),A.remove(),await z({[p+"Team"]:w,boostUsed:!0})})})}function R(w=null){var ae,le;if(!(f.phase===p+"-attack")){g("Remplacement uniquement avant votre attaque","warning");return}const A=f[p+"Team"],j=f["usedSubIds_"+p]||[],T=f.maxSubs||3;if(j.length>=T){g(`Maximum ${T} remplacements atteint`,"warning");return}const N=Object.entries(A).flatMap(([ge,ce])=>(ce||[]).filter(ye=>ye.used).map(ye=>({...ye,_line:ge}))),D=(f[p+"Subs"]||[]).filter(ge=>!j.includes(ge.cardId));if(!N.length){g("Aucun joueur utilisé à remplacer","warning");return}if(!D.length){g("Aucun remplaçant disponible","warning");return}let P=Math.max(0,N.findIndex(ge=>ge.cardId===w));const O=((ae=N[P])==null?void 0:ae._line)||((le=N[P])==null?void 0:le.job);let X=Math.max(0,D.findIndex(ge=>ge.job===O)),ie=!1;const Z=document.createElement("div");Z.id="pvp-sub-overlay",Z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function Q(){var Ne,Le,Ve,Oe,nt,Kt;const ge=N[P],ce=D[X],ye=Math.min(130,Math.round((window.innerWidth-90)/2)),be=Math.round(ye*1.35),oe=De=>`background:rgba(255,255,255,0.12);border:none;color:${De?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${De?"default":"pointer"};flex-shrink:0`;Z.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        <div id="pin-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
          ...
          <div>${ce?ht({...ce,used:!1,boost:0},ye,be):"<div>—</div>"}</div>
      ...
      </div>`,(Ne=Z.querySelector("#psub-close"))==null||Ne.addEventListener("click",()=>Z.remove()),(Le=Z.querySelector("#pout-up"))==null||Le.addEventListener("click",()=>{P>0&&(P--,Q())}),(Ve=Z.querySelector("#pout-down"))==null||Ve.addEventListener("click",()=>{P<N.length-1&&(P++,Q())}),(Oe=Z.querySelector("#pin-up"))==null||Oe.addEventListener("click",()=>{X>0&&(X--,Q())}),(nt=Z.querySelector("#pin-down"))==null||nt.addEventListener("click",()=>{X<D.length-1&&(X++,Q())});const Ee=(De,Ye,He,pt)=>{const ot=Z.querySelector("#"+De);if(!ot)return;let Ct=0;ot.addEventListener("touchstart",ut=>{Ct=ut.touches[0].clientY},{passive:!0}),ot.addEventListener("touchend",ut=>{const kt=ut.changedTouches[0].clientY-Ct;if(Math.abs(kt)<30)return;const rt=Ye();kt<0&&rt<pt-1?(He(rt+1),Q()):kt>0&&rt>0&&(He(rt-1),Q())},{passive:!0})};Ee("pin-panel",()=>X,De=>X=De,D.length),Ee("pout-panel",()=>P,De=>P=De,N.length),(Kt=Z.querySelector("#psub-confirm"))==null||Kt.addEventListener("click",async De=>{if(De.preventDefault(),De.stopPropagation(),ie)return;ie=!0;const Ye=N[P],He=D[X];if(!Ye||!He)return;const pt=Ye._line,ot=(A[pt]||[]).findIndex(kt=>kt.cardId===Ye.cardId);if(ot===-1){g("Erreur : joueur introuvable","error"),Z.remove();return}const Ct={...He,_line:pt,position:Ye.position,used:!1,boost:0};A[pt].splice(ot,1,Ct);const ut=[...j,He.cardId];Z.remove(),re(Ye,He,async()=>{await z({[p+"Team"]:A,[u+"Team"]:f[u+"Team"],["usedSubIds_"+p]:ut})})})}document.body.appendChild(Z),Q()}function re(w,L,A){const j={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const N=(O,X,ie)=>`<div style="text-align:center">
      <div style="font-size:9px;color:${X};letter-spacing:2px;text-transform:uppercase;font-weight:700;margin-bottom:6px">${ie}</div>
      <div style="width:70px;height:70px;border-radius:50%;background:${j[O.job]||"#555"};border:3px solid ${X};position:relative;overflow:hidden;margin:0 auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:6px">Appuyer pour continuer</div>`,document.body.appendChild(T);let D=!1;const P=()=>{D||(D=!0,T.remove(),setTimeout(()=>A(),50))};T.addEventListener("click",P),setTimeout(P,2200)}function q(){var L;const w=document.createElement("div");w.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:20px;overflow-y:auto",w.innerHTML=`
      ...
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(w),(L=w.querySelector("#pvp-opp-close"))==null||L.addEventListener("click",()=>w.remove())}function G(){var A;const w=f.log||[],L=document.createElement("div");L.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column",L.innerHTML=`
      <div style="display:flex;align-items:center;padding:14px 16px;border-bottom:1px solid rgba(255,255,255,0.1);flex-shrink:0">
        <div style="flex:1;font-size:14px;font-weight:700;color:#fff">📋 Historique</div>
        <button id="pvp-hist-close" style="background:none;border:none;color:rgba(255,255,255,0.6);font-size:20px;cursor:pointer">✕</button>
      </div>
      <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
        ${w.length===0?'<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action</div>':[...w].reverse().map(j=>`<div style="padding:8px 10px;border-radius:8px;background:rgba(255,255,255,0.04);border-left:3px solid ${j.type==="goal"?"#FFD700":j.type==="stop"?"#00ff88":"rgba(255,255,255,0.5)"}"><div style="font-size:12px;color:#fff">${j.text||""}</div></div>`).join("")}
      </div>`,document.body.appendChild(L),(A=L.querySelector("#pvp-hist-close"))==null||A.addEventListener("click",()=>L.remove())}async function se(){if(f.phase!==p+"-attack")return;const w=p==="p1"?"p2":"p1",L=(f.round||0)+1,A=[...f.log||[]];A.push({type:"info",text:`⏭️ ${f[p+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const j=$e(f),T=ct(w),N=j||!T?"finished":w+"-attack";await z({["selected_"+p]:[],modifiers:{...f.modifiers,[p]:{}},pendingAttack:null,phase:N,attacker:w,round:L,log:A}),N==="finished"&&await qe(f)}function de(w){const L=(w.GK||[]).some(j=>!j.used),A=["DEF","MIL","ATT"].some(j=>(w[j]||[]).some(T=>!T.used));return L&&!A}function he(w){return(w.p1Score||0)!==(w.p2Score||0)?null:de(w.p1Team)&&!ke(w.p2Team)?{side:"p1",gk:(w.p1Team.GK||[]).find(L=>!L.used)}:de(w.p2Team)&&!ke(w.p1Team)?{side:"p2",gk:(w.p2Team.GK||[]).find(L=>!L.used)}:null}async function Te(){const w=document.createElement("div");w.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:950;display:flex;align-items:center;justify-content:center;overflow:hidden;text-align:center;padding:24px",document.body.appendChild(w);const L=(A,j)=>new Promise(T=>{w.innerHTML=`<div style="font-size:32px;font-weight:900;color:${j};letter-spacing:2px;animation:lcFade 1.4s ease both">${A}</div>
      <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(T,1400)});await L("⚽ DERNIER CORNER","#FFD700"),await L("🧤 LE GARDIEN MONTE !","#4fc3f7"),w.remove()}async function ve(w,L){const{side:A,gk:j}=w;if(!j)return;const T=A+"Score",N=(L[T]||0)+1;j.used=!0;const D=[...L.log||[]];D.push({type:"duel",isGoal:!0,homeScored:A===p,homePlayers:A==="p1"?[qt(j)]:[],aiPlayers:A==="p2"?[qt(j)]:[],text:`⚽ DERNIER CORNER — Le gardien de ${L[A+"Name"]} marque !`});const P=(L.round||0)+1,O=A==="p1"?"p2":"p1",X={...L,[T]:N},ie=$e(X);B.add(P),await Te();const Z=A===p?N:L[p+"Score"]||0,Q=A===p?L[u+"Score"]||0:N;Qe([qt(j)],Z,Q,A===p,async()=>{await z({[T]:N,log:D,round:P,pendingAttack:null,phase:ie?"finished":O+"-attack",attacker:O,["selected_"+p]:[],modifiers:{...f.modifiers,[p]:{}}}),ie&&await qe({...f,[T]:N})})}async function Se(){const w=f[p+"Team"],L=!["GK","DEF","MIL","ATT"].some(N=>(f[u+"Team"][N]||[]).some(D=>!D.used)),A=(f["selected_"+p]||[]).map(N=>{const D=(w[N._role]||[]).find(le=>le.cardId===N.cardId)||N,P=["GK","DEF"].includes(N._role),O=w[N._role]||[],X=O.findIndex(le=>le.cardId===N.cardId),ie=yt(O.length),Z=X>=0?ie[X]:D._col??1,Q=f.stadiumDef||f.homeStadiumDef||null,ae=D.stadiumBonus||Q&&(Q.club_id&&String(D.club_id)===String(Q.club_id)||Q.country_code&&D.country_code===Q.country_code)||!1;return{...D,_line:N._role,_col:Z,stadiumBonus:ae,...P?{note_a:Math.max(1,Number(D.note_a)||0)}:{}}});if(!A.length)return;const j=oi(A,f.modifiers[p]||{},f[p+"Formation"]);fe(p,A.map(N=>N.cardId)),A.forEach(N=>{const D=(w[N._role]||[]).find(P=>P.cardId===N.cardId);D&&(D.used=!0)}),f["selected_"+p]=[],Y();const T=[...f.log||[]];if(L){if(A.length===1&&(A[0]._line==="GK"||A[0].job==="GK")&&(f.p1Score||0)===(f.p2Score||0)){await ve({side:p,gk:A[0]},{...f,[p+"Team"]:w,log:T});return}const D=(f[p+"Score"]||0)+1,P=A.map(Q=>qt(Q));T.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:P,homeTotal:j.total,aiTotal:0});const O=(f.round||0)+1,X=p==="p1"?"p2":"p1",ie={...f,[p+"Team"]:w,[p+"Score"]:D},Z=$e(ie);B.add(O),Qe(P,D,f[u+"Score"]||0,!0,async()=>{await z({[p+"Team"]:w,[p+"Score"]:D,["selected_"+p]:[],modifiers:{...f.modifiers,[p]:{}},pendingAttack:null,phase:Z?"finished":X+"-attack",attacker:X,round:O,log:T}),Z&&await qe({...f,[p+"Score"]:D})});return}T.push({type:"pending",text:`⚔️ ${f[p+"Name"]} attaque (${j.total})`}),await z({[p+"Team"]:w,[u+"Team"]:f[u+"Team"],pendingAttack:{...j,players:A,side:p},["selected_"+p]:[],modifiers:{...f.modifiers,[p]:{}},phase:u+"-defense",log:T})}async function Ae(){const w=f[p+"Team"],L=(f["selected_"+p]||[]).map(ce=>{const ye=(w[ce._role]||[]).find(Le=>Le.cardId===ce.cardId)||ce,be=w[ce._role]||[],oe=be.findIndex(Le=>Le.cardId===ce.cardId),Ee=yt(be.length),Ne=oe>=0?Ee[oe]:ye._col??1;return{...ye,_line:ce._role,_col:Ne}}),A=ri(L,f.modifiers[p]||{},f[p+"Formation"]);fe(p,L.map(ce=>ce.cardId)),L.forEach(ce=>{const ye=(w[ce._role]||[]).find(be=>be.cardId===ce.cardId);ye&&(ye.used=!0)}),f["selected_"+p]=[],Y();const j=Xi(f.pendingAttack.total,A.total,f.modifiers[p]||{}),T=f.pendingAttack.side,N=j==="attack"||(j==null?void 0:j.goal),D=T==="p1"?"p2":"p1",P=(f.round||0)+1,O=(f.pendingAttack.players||[]).map(ce=>qt(ce)),X=[...f.log||[]];X.push({type:"duel",isGoal:N,homeScored:N&&T===p,text:N?`⚽ BUT de ${f[T+"Name"]} ! (${f.pendingAttack.total} vs ${A.total})`:`✋ Attaque stoppée (${f.pendingAttack.total} vs ${A.total})`,homePlayers:O,aiPlayers:L.map(ce=>qt(ce)),homeTotal:f.pendingAttack.total,aiTotal:A.total});const ie=N?(f[T+"Score"]||0)+1:f[T+"Score"]||0,Z={...f,[p+"Team"]:w,[T+"Score"]:ie},Q=$e(Z),ae=Q?"finished":D+"-attack",le=!Q&&!N?he(Z):null,ge=async()=>{if(le){await ve(le,Z);return}await z({[p+"Team"]:w,[u+"Team"]:f[u+"Team"],[T+"Score"]:ie,["selected_"+p]:[],modifiers:{...f.modifiers,[p]:{}},pendingAttack:null,phase:ae,attacker:D,round:P,log:X}),(ae==="finished"||Q)&&await qe({...f,[T+"Score"]:ie})};if(N){const ce=T===p,ye=ce?ie:f[p+"Score"]||0,be=ce?f[u+"Score"]||0:ie;B.add(P),Qe(O,ye,be,ce,ge)}else await ge()}function Be(w){return["MIL","ATT"].some(L=>(w[L]||[]).some(A=>!A.used))}function ke(w){return["GK","DEF","MIL","ATT"].some(L=>(w[L]||[]).some(A=>!A.used))}function wt(w){return ke(w)&&!Be(w)}function ct(w){const L=f[w+"Team"],A=f[(w==="p1"?"p2":"p1")+"Team"];return!!(Be(L)||wt(L)&&!ke(A)||wt(L)&&!Be(A)&&ke(A))}function $e(w){return!ke(w.p1Team)&&!ke(w.p2Team)}function Ce(w){const L=w.p1Score||0,A=w.p2Score||0;return L===A?null:L>A?v.home_id:v.away_id}async function qe(w){Gt(),Ue();try{const L=Ce(w),A=L?v.home_id===L?v.away_id:v.home_id:null,j=w.p1Score||0,T=w.p2Score||0,{data:N,error:D}=await _.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:L,p_home_score:j,p_away_score:T,p_forfeit:!1});if(D&&console.error("[PvP] finish_pvp_match:",D),l)try{await _.rpc("check_and_finish_mini_league",{p_league_id:l})}catch(P){console.error("[PvP] check_and_finish_mini_league:",P)}L&&A&&Lr(L,A).catch(()=>{});try{const{data:P}=await _.rpc("apply_match_rewards",{p_match_id:i});P!=null&&P.success&&!(P!=null&&P.skipped)&&typeof t.refreshProfile=="function"&&await t.refreshProfile()}catch(P){console.error("[PvP] apply_match_rewards:",P)}if(typeof c=="function")try{await c({homeScore:j,awayScore:T,winnerId:L,homeId:v.home_id,awayId:v.away_id,matchId:i})}catch(P){console.error("[PvP] onMatchEnd:",P)}}catch(L){console.error("[PvP] finishMatch:",L)}}function Ge(){var T;const w=f[p+"Score"],L=f[u+"Score"],A=w>L,j=w===L;st(e),e.innerHTML=`
    ...
    </div>`,(T=document.getElementById("pvp-home"))==null||T.addEventListener("click",()=>{try{_.removeChannel(ne)}catch{}it(e),l?y("mini-league",{openLeagueId:l}):y("home")})}Y()}async function Pi(e,t,i=!1){var s,d;const{state:n,navigate:r,toast:o}=t,a=i&&((d=(s=t==null?void 0:t.state)==null?void 0:s.params)==null?void 0:d.rankedData)||null;await wi(e,t,i?"ranked":"random",async({deckId:c,formation:l,starters:m,subsRaw:b,gcCardsEnriched:y,gcDefs:g,stadiumDef:p})=>{st(e);const u=async x=>{await Sr(e,t,c,l,m,b,x||[],g,p,i,a)};if(!(y!=null&&y.length)){await u([]);return}hi(e,y,u)})}async function Ir(e,t,i){return Tr(e,t,i)}async function Sr(e,t,i,n,r,o,a=[],s=[],d=null,c=!1,l=null){var te,ne;const{state:m,navigate:b,toast:y}=t,g=(l==null?void 0:l.mmr)??((te=m.profile)==null?void 0:te.mmr)??1e3,p=c?Hi(g):null,u=c?"linear-gradient(135deg, #1a0a2e 0%, #2d1060 50%, #1a0a2e 100%)":"linear-gradient(135deg, #0a1a2e 0%, #0d3d1e 100%)",x=c?(p==null?void 0:p.color)||"#D4A017":"#FFD700";let h=!1,v=null,$=null;e.innerHTML=`
    ...
    </div>`;const f=async()=>{h=!0,v&&(v.unsubscribe(),v=null),$&&(clearInterval($),$=null);try{await _.rpc("cancel_matchmaking",{p_user_id:m.profile.id})}catch(z){console.error("[Matchmaking] cancel error:",z)}};(ne=document.getElementById("cancel-mm"))==null||ne.addEventListener("click",async()=>{await f(),it(e),b("home")});const M=async(z,S)=>{h||(h=!0,v&&(v.unsubscribe(),v=null),$&&(clearInterval($),$=null),await Mt(e,t,z,S,{myGC:a,gcDefs:s,stadiumDef:d,isRanked:c,rankedData:l,onMatchEnd:c?Ar:null}))},k=m.user.id,{data:I,error:B}=c?await _.rpc("try_matchmake_ranked",{p_user_id:k,p_deck_id:i,p_mmr:g}):await _.rpc("try_matchmake",{p_user_id:k,p_deck_id:i});if(B||!(I!=null&&I.success)){console.error("[Matchmaking] try_matchmake error:",B||I),y("Erreur de matchmaking","error"),it(e),b("home");return}if(I.matched){await M(I.match_id,!1);return}const ee=async()=>{if(h)return;const{data:z}=await _.from("matchmaking_queue").select("status, match_id").eq("user_id",k).maybeSingle();(z==null?void 0:z.status)==="matched"&&z.match_id&&await M(z.match_id,!0)};v=_.channel(`mm_${k}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${k}`},z=>{const S=z.new;S.status==="matched"&&S.match_id&&M(S.match_id,!0)}).subscribe(),$=setInterval(ee,3e3),setTimeout(()=>{h||f()},12e4)}async function Ar(e,t){const{state:i,toast:n}=t,{winnerId:r,homeId:o,awayId:a,homeScore:s,awayScore:d,matchId:c}=e,l=i.user.id,m=o===l,b=r===l,{data:y}=await _.from("users").select("mmr, mmr_rd, mmr_v").eq("id",l).single(),{data:g}=await _.from("users").select("mmr, mmr_rd, mmr_v").eq("id",m?a:o).single();if(y&&g){const p=ro(y.mmr,y.mmr_rd,y.mmr_v,g.mmr,g.mmr_rd,b?1:0);await _.from("users").update({mmr:p.mmr,mmr_rd:p.rd,mmr_v:p.v,mmr_wins:b?_.sql`mmr_wins + 1`:void 0,mmr_losses:b?void 0:_.sql`mmr_losses + 1`}).eq("id",l),n(b?`+MMR ↑ ${Math.round(p.mmr-y.mmr)}`:`-MMR ↓ ${Math.round(y.mmr-p.mmr)}`,b?"success":"error",4e3)}}const zr=Object.freeze(Object.defineProperty({__proto__:null,renderMatchRandom:Pi,resumePvpMatch:Ir},Symbol.toStringTag,{value:"Module"}));async function Mr(e,t){const{state:i,navigate:n,toast:r}=t,o=i.params||{},a=o.friendId||null,s=o.friendName||"Ami",d=!!o.isAccepting;await wi(e,t,"friend",async({deckId:c,formation:l,starters:m,subsRaw:b,gcCardsEnriched:y,gcDefs:g,stadiumDef:p})=>{st(e);const u=async x=>{const h=x||[];d?await jr(e,t,c,h,g,p):await Cr(e,t,c,h,g,p,a,s)};if(!(y!=null&&y.length)){await u([]);return}hi(e,y,u)})}async function Cr(e,t,i,n,r,o,a,s){var h;const{state:d,navigate:c,toast:l}=t,m=d.user.id,{data:b,error:y}=await _.from("friend_match_invites").insert({inviter_id:m,invitee_id:a,friend_id:a,status:"pending",inviter_deck_id:i,expires_at:new Date(Date.now()+2*60*1e3).toISOString()}).select("id").single();if(y||!b){console.error("[Friend] Erreur création invitation:",y),l("Impossible de créer l'invitation","error"),c("home");return}e.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    <div style="padding:14px 16px;background:var(--nav-bg,#0d1a0f);border-bottom:1px solid ${Lt};display:flex;align-items:center;justify-content:space-between">
      <div>
        ...
        <div style="font-size:12px;color:${Qn}">Championnats 3 à 8 joueurs</div>
      </div>
      <div style="display:flex;align-items:center;gap:8px">
    ...
    <div style="display:flex;background:var(--nav-bg,#0d1a0f);border-bottom:1px solid ${Lt}">
      ${h.map(f=>`<button class="ml-tab" data-tab="${f.key}" style="flex:1;padding:11px 4px;border:none;border-bottom:2px solid ${i===f.key?Jn:"transparent"};background:none;font-size:12px;font-weight:${i===f.key?"900":"600"};color:${i===f.key?"#4ade80":Br};cursor:pointer">${f.label}${f.count?` (${f.count})`:""}</button>`).join("")}
    </div>
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:10px">
      ${i==="waiting"?Dr(y,x,r):i==="active"?Fr(g,r):Pr(u,r,s)}
    </div>
  </div>`,(v=document.getElementById("ml-create-btn"))==null||v.addEventListener("click",()=>Gr(e,t)),($=document.getElementById("ml-refresh-list"))==null||$.addEventListener("click",()=>et(e,t,i)),e.querySelectorAll(".ml-tab").forEach(f=>f.addEventListener("click",()=>et(e,t,f.dataset.tab))),e.querySelectorAll("[data-league-id]").forEach(f=>f.addEventListener("click",()=>lt(e,t,f.dataset.leagueId))),e.querySelectorAll("[data-join]").forEach(f=>f.addEventListener("click",M=>{M.stopPropagation(),Zn(e,t,f.dataset.join,f.dataset.type)})),e.querySelectorAll("[data-delete]").forEach(f=>f.addEventListener("click",M=>{M.stopPropagation(),Gi(e,t,f.dataset.delete,f.dataset.name,i)})),e.querySelectorAll("[data-claim-league]").forEach(f=>f.addEventListener("click",async M=>{M.stopPropagation(),f.disabled=!0,f.textContent="...";const{data:k,error:I}=await _.rpc("claim_mini_league_prize",{p_league_id:f.dataset.claimLeague,p_user_id:n.profile.id});if(I||!(k!=null&&k.success)){t.toast((k==null?void 0:k.error)||"Erreur lors de la récupération","error"),et(e,t,i);return}if(!k.already_claimed){n.profile.credits=(n.profile.credits||0)+k.prize;const B=document.getElementById("nav-credits");B&&(B.textContent=`💰 ${n.profile.credits.toLocaleString("fr")}`),t.toast(`💰 +${k.prize.toLocaleString("fr")} cr. ajoutés !`,"success")}et(e,t,i)}))}function vi(e,t,i=!1,n=null){var d,c;const r=e.creator_id===t,o=e.pot||0,a=((c=(d=e.mini_league_members)==null?void 0:d[0])==null?void 0:c.count)||0,s=n&&n.amount>0&&!n.claimed;return`<div data-league-id="${e.id}" style="background:${Jt};border:1px solid ${s?"rgba(212,160,23,0.5)":Qt};border-radius:12px;padding:14px 16px;cursor:pointer;margin-bottom:8px">
    ...
    <div style="padding:20px;display:flex;flex-direction:column;gap:14px">
      ...
      </div>`,document.body.appendChild(t);const i=t.querySelector("#ml-pwd-input"),n=r=>{t.remove(),e(r)};setTimeout(()=>i==null?void 0:i.focus(),80),t.querySelector("#pwd-cancel").addEventListener("click",()=>n(null)),t.querySelector("#pwd-ok").addEventListener("click",()=>n(i.value.trim())),i.addEventListener("keydown",r=>{r.key==="Enter"&&n(i.value.trim())})})}async function Zn(e,t,i,n){var m;const{toast:r,state:o}=t,a=o.profile.id;let s=null;if(n==="private"&&(s=await Nr(),s===null))return;const{data:d,error:c}=await _.rpc("join_mini_league",{p_league_id:i,p_user_id:a,p_password:s});if(c||!(d!=null&&d.success)){console.error("[MiniLeague] join_mini_league:",c||d),r((d==null?void 0:d.error)||"Erreur lors de l'inscription","error");return}if(typeof t.refreshProfile=="function")await t.refreshProfile();else{const{data:b}=await _.from("users").select("credits").eq("id",a).single();o.profile&&b&&(o.profile.credits=b.credits)}const l=document.getElementById("nav-credits");l&&(l.textContent=`💰 ${(((m=o.profile)==null?void 0:m.credits)||0).toLocaleString("fr")}`),r("✅ Inscrit ! Pot mis à jour.","success"),lt(e,t,i)}async function Rr(e,t,i,n){const{toast:r,state:o}=t,a=o.profile.id;if(!await new Promise(b=>{const y=document.createElement("div");y.className="modal-overlay",y.style.zIndex="2100",y.innerHTML=`<div class="modal" style="max-width:360px">
    ...
    </div>`,document.body.appendChild(y),y.querySelector("#lv-ok").onclick=()=>{y.remove(),b(!0)},y.querySelector("#lv-cancel").onclick=()=>{y.remove(),b(!1)}}))return;const{data:d,error:c}=await _.rpc("leave_mini_league",{p_league_id:i,p_user_id:a});if(c||!(d!=null&&d.success)){r((d==null?void 0:d.error)||"Erreur lors de la désinscription","error");return}const l=d.refund||n;o.profile&&(o.profile.credits=(o.profile.credits||0)+l);const m=document.getElementById("nav-credits");m&&(m.textContent=`💰 ${(o.profile.credits||0).toLocaleString("fr")}`),r(`↩️ Désinscrit · +${l.toLocaleString("fr")} cr. remboursés`,"success"),et(e,t,"waiting")}async function Gi(e,t,i,n,r){const{toast:o}=t;if(!confirm(`Supprimer définitivement "${n}" et tous ses matchs/membres ? Action irréversible.`))return;await _.from("mini_league_matches").delete().eq("league_id",i),await _.from("mini_league_members").delete().eq("league_id",i);const{error:a}=await _.from("mini_leagues").delete().eq("id",i);if(a){o("Erreur suppression ("+a.message+")","error");return}o("Mini League supprimée avec succès","success"),et(e,t,r)}async function Or(e,t,i){await _.from("mini_leagues").update({is_archived:!0}).eq("id",i),t.toast("Mini League archivée","success"),et(e,t,"archived")}async function lt(e,t,i){var $,f,M,k,I,B,ee,te,ne;const{state:n,toast:r}=t,o=n.profile.id,[{data:a},{data:s},{data:d}]=await Promise.all([_.from("mini_leagues").select("*").eq("id",i).single(),_.from("mini_league_members").select("*, user:users(id,pseudo,club_name,club_color1,club_color2)").eq("league_id",i),_.from("mini_league_matches").select("*").eq("league_id",i).order("matchday").order("created_at")]);if(!a){r("Introuvable","error"),await et(e,t);return}const c=(s||[]).some(z=>z.user_id===o),l=a.creator_id===o,m=(s||[]).map(z=>z.user).filter(Boolean),b=Yr(m,d||[]),y=(d||[]).filter(z=>z.matchday===a.current_day),g=a.pot||0,p=a.entry_fee||100,u=y.length>0&&y.every(z=>z.status==="finished"||z.status==="bye"),x=a.current_day>=a.total_days,h=(s||[]).find(z=>z.user_id===o);e.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    <div style="padding:14px 16px;background:var(--nav-bg,#0d1a0f);border-bottom:1px solid ${Lt};display:flex;align-items:center;gap:10px">
      ...
      <div style="flex:1">
        ...
        <div style="font-size:11px;color:${Qn}">${a.mode==="aller-retour"?"Aller-Retour":"Aller"} · max ${a.max_players} · 💰 ${p} cr./joueur</div>
      </div>
    ...
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:${Jt};border:1px solid ${Qt};border-radius:12px;padding:16px">
        ...
        ${m.map(z=>`
          <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid ${Lt}">
            <div style="width:36px;height:36px;border-radius:50%;background:${z.club_color2||Jn};display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:900;color:${z.club_color1||"#fff"}">${(z.pseudo||"?").slice(0,2).toUpperCase()}</div>
            <div style="flex:1"><div style="font-size:13px;font-weight:700;color:${Xe}">${z.club_name||z.pseudo}</div><div style="font-size:11px;color:${Ze}">@${z.pseudo}</div></div>
            ${z.id===a.creator_id?'<span style="font-size:10px;color:#D4A017;font-weight:700">👑</span>':""}
          </div>`).join("")}
      ...
      <div style="background:${Jt};border:1px solid ${Qt};border-radius:12px;padding:16px">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>${b.map((z,S)=>{const K=g>0&&a.status==="finished"?S===0?Math.floor(g*.7):S===1?Math.floor(g*.2):S===2?Math.floor(g*.1):0:0;return`<tr style="border-bottom:1px solid ${Lt};${z.userId===o?"background:rgba(74,222,128,0.08);":""}">
              <td style="padding:7px 3px 7px 0;font-weight:700;color:${S===0?dt:S<3?"#4ade80":Et}">${["🥇","🥈","🥉"][S]||S+1}</td>
              <td style="padding:7px 3px"><div style="font-weight:700;color:${Xe}">${z.clubName}</div><div style="font-size:10px;color:${Ze}">@${z.pseudo}</div></td>
              <td style="text-align:center;color:${Et}">${z.played}</td><td style="text-align:center;color:${Et}">${z.won}-${z.drawn}-${z.lost}</td>
              <td style="text-align:center;color:${z.goalDiff>=0?"#4ade80":"#ff6b6b"}">${z.goalDiff>=0?"+":""}${z.goalDiff}</td>
              <td style="text-align:center;font-weight:900;font-size:14px;color:${Xe}">${z.points}</td>
      ...
      <div style="background:${Jt};border:1px solid ${Qt};border-radius:12px;padding:16px">
        ...
        ${Array.from({length:Math.max(0,a.status==="active"?a.current_day-1:a.current_day)},(z,S)=>S+1).reverse().map(z=>{const S=(d||[]).filter(K=>K.matchday===z);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:${Ze};margin-bottom:6px">Journée ${z}</div>${S.map(K=>kn(K,m,o,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
  ...
  </div>`;const v=a.status==="waiting"?"waiting":a.status==="active"?"active":"archived";if(($=document.getElementById("ml-back"))==null||$.addEventListener("click",()=>et(e,t,v)),(f=document.getElementById("ml-refresh"))==null||f.addEventListener("click",async z=>{const S=z.currentTarget;S.style.opacity="0.5",await lt(e,t,i)}),(M=document.getElementById("ml-start-btn"))==null||M.addEventListener("click",()=>Hr(e,t,a,m)),(k=document.getElementById("ml-next-day"))==null||k.addEventListener("click",()=>Ur(e,t,i)),(I=document.getElementById("ml-join-now"))==null||I.addEventListener("click",()=>Zn(e,t,i,a.type)),(B=document.getElementById("ml-leave-btn"))==null||B.addEventListener("click",()=>Rr(e,t,i,p)),(ee=document.getElementById("ml-delete-btn"))==null||ee.addEventListener("click",()=>Gi(e,t,i,a.name,"waiting")),(te=document.getElementById("ml-delete-now"))==null||te.addEventListener("click",()=>Gi(e,t,i,a.name,v)),(ne=document.getElementById("ml-archive-btn"))==null||ne.addEventListener("click",()=>Or(e,t,i)),e.querySelectorAll("[data-play-match]").forEach(z=>{z.addEventListener("click",()=>{const S=y.find(K=>K.id===z.dataset.playMatch);S&&t.navigate("match-mini-league",{mlMatchId:S.id,leagueId:i})})}),a.status==="finished"&&h){const z=b.findIndex(S=>S.userId===o);z>=0&&z<3&&h.prize_amount>0&&!h.prize_claimed&&setTimeout(()=>Kr(e,t,a,h,z),400)}}function kn(e,t,i,n,r=!1){const o=m=>t.find(b=>b.id===m);if(e.is_bye){const m=o(e.home_id);return`<div style="padding:8px;border-radius:8px;background:rgba(255,255,255,0.03);margin-bottom:6px;font-size:12px;color:${Et};text-align:center">🔵 ${(m==null?void 0:m.club_name)||(m==null?void 0:m.pseudo)||"?"} exempté(e)</div>`}const a=o(e.home_id),s=o(e.away_id),d=e.home_id===i||e.away_id===i,c=d&&e.status==="pending"&&n&&!r,l=e.status==="finished"?`${e.home_score} - ${e.away_score}`:"vs";return`<div style="display:flex;align-items:center;gap:8px;padding:10px;border-radius:8px;background:${d?"rgba(26,107,60,0.16)":"rgba(255,255,255,0.03)"};margin-bottom:6px;border:1px solid ${d?"rgba(74,222,128,0.35)":Lt}">
  ...
  </div>`}async function Hr(e,t,i,n){const{toast:r,state:o}=t,a=Vr(n.map(c=>c.id),i.mode),s=[];a.forEach((c,l)=>c.forEach(m=>s.push({league_id:i.id,matchday:l+1,home_id:m.home||m.bye,away_id:m.away||null,is_bye:!!m.bye,status:m.bye?"bye":"pending"})));const{error:d}=await _.from("mini_league_matches").insert(s);if(d){r("Erreur calendrier : "+d.message,"error");return}await _.from("mini_leagues").update({status:"active",current_day:1,total_days:a.length}).eq("id",i.id),r(`🚀 Lancée ! Pot : ${(i.pot||0).toLocaleString("fr")} cr.`,"success"),lt(e,t,i.id)}async function Ur(e,t,i){const{data:n}=await _.from("mini_leagues").select("current_day,total_days,pot").eq("id",i).single(),r=(n.current_day||0)+1;if(r>(n.total_days||0)){lt(e,t,i);return}await _.from("mini_leagues").update({current_day:r}).eq("id",i),t.toast(`Journée ${r} commencée !`,"success"),lt(e,t,i)}async function Kr(e,t,i,n,r){var b,y;const{state:o,toast:a}=t,s=[Math.floor((i.pot||0)*.7),Math.floor((i.pot||0)*.2),Math.floor((i.pot||0)*.1)],d=["🥇","🥈","🥉"][r],c=n.prize_amount||s[r]||0,l=n.prize_claimed,m=document.createElement("div");m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",m.innerHTML=`
    ...
    <p style="margin-top:12px;font-size:13px;color:#888">Tu peux revoir ce tutoriel depuis les paramètres à tout moment.</p>`}];function Qr(e,t,i){let n=0;const r=document.createElement("div");r.id="tutorial-overlay",r.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.65);z-index:9900;display:flex;align-items:center;justify-content:center;padding:16px";const o=()=>{var l,m,b;const s=t[n],d=n===t.length-1,c=Math.round((n+1)/t.length*100);r.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div id="mkt-content" style="padding:10px 16px;display:flex;flex-direction:column;gap:8px"></div>
  </div>`;let d="buy";const c=()=>{var p,u,x,h,v,$,f;return{name:(((p=document.getElementById("flt-name"))==null?void 0:p.value)||"").toLowerCase().trim(),club:(((u=document.getElementById("flt-club"))==null?void 0:u.value)||"").toLowerCase().trim(),country:(((x=document.getElementById("flt-country"))==null?void 0:x.value)||"").toLowerCase().trim(),job:((h=document.getElementById("flt-job"))==null?void 0:h.value)||"",rarity:((v=document.getElementById("flt-rarity"))==null?void 0:v.value)||"",note1:parseInt(($=document.getElementById("flt-note1"))==null?void 0:$.value)||0,note2:parseInt((f=document.getElementById("flt-note2"))==null?void 0:f.value)||0}};function l(p){const u=c();return p.filter(x=>{var B,ee,te;const h=(B=x.card)==null?void 0:B.player;if(!h)return!1;const v=`${h.firstname} ${h.surname_real}`.toLowerCase(),$=(((ee=h.clubs)==null?void 0:ee.encoded_name)||"").toLowerCase(),f=(h.country_code||"").toLowerCase(),M=((te=x.card)==null?void 0:te.evolution_bonus)||0,k=Dt(h,h.job,M),I=h.job2?Dt(h,h.job2,M):0;return!(u.name&&!v.includes(u.name)||u.club&&!$.includes(u.club)||u.country&&!f.includes(u.country)||u.job&&h.job!==u.job||u.rarity&&h.rarity!==u.rarity||u.note1&&k<u.note1||u.note2&&I<u.note2)})}function m(p){var I,B,ee,te;const u=(I=p.card)==null?void 0:I.player;if(!u)return"";const x=((B=p.card)==null?void 0:B.evolution_bonus)||0,h=Dt(u,u.job,x),v=u.job2?Dt(u,u.job2,x):0,$=(i.profile.credits||0)>=p.price,f=u.country_code?`https://flagsapi.com/${u.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,M=ui[u.job]||"#bbb",k=u.job2?ui[u.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden">
      <!-- Drapeau -->
      ${f?`<img src="${f}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      <!-- Logo club -->
      ${(ee=u.clubs)!=null&&ee.logo_url?`<img src="${u.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <!-- Notes -->
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${M};display:flex;align-items:center;justify-content:center">
          <span style="font-size:14px;font-weight:900;color:${M};font-family:Arial Black,Arial">${h}</span>
        </div>
    ...
    </div>`}function b(p){var I,B,ee,te;const u=(I=p.card)==null?void 0:I.player;if(!u)return"";const x=((B=p.card)==null?void 0:B.evolution_bonus)||0,h=Dt(u,u.job,x),v=u.job2?Dt(u,u.job2,x):0,$=p.status==="sold",f=u.country_code?`https://flagsapi.com/${u.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,M=ui[u.job]||"#bbb",k=u.job2?ui[u.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${$?"opacity:0.7":""}">
      ${f?`<img src="${f}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      ${(ee=u.clubs)!=null&&ee.logo_url?`<img src="${u.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${M};display:flex;align-items:center;justify-content:center">
    ...
    </div>`}function y(){const p=document.getElementById("mkt-content"),u=document.getElementById("mkt-filters");if(p){if(u.style.display=d==="buy"?"flex":"none",d==="buy"){const x=l(a);p.innerHTML=x.length?x.map(m).join(""):'<div style="text-align:center;color:#aaa;padding:40px">Aucune carte trouvée.</div>'}else{const x=s.filter(v=>v.status==="active").sort((v,$)=>new Date($.listed_at)-new Date(v.listed_at)),h=s.filter(v=>v.status==="sold").sort((v,$)=>new Date($.sold_at||$.listed_at)-new Date(v.sold_at||v.listed_at));p.innerHTML=(x.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:4px 0 6px;text-transform:uppercase;letter-spacing:1px">🟢 En vente (${x.length})</div>`+x.map(b).join(""):"")+(h.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:12px 0 6px;text-transform:uppercase;letter-spacing:1px">✅ Ventes réussies (${h.length})</div>`+h.map(b).join(""):"")+(!x.length&&!h.length?'<div style="text-align:center;color:#aaa;padding:40px">Aucune vente pour le moment.</div>':"")}p.querySelectorAll("[data-buy]").forEach(x=>x.addEventListener("click",()=>ta(x.dataset.buy,a,e,t))),p.querySelectorAll("[data-cancel]").forEach(x=>x.addEventListener("click",()=>na(x.dataset.cancel,e,t)))}}e.querySelectorAll(".mkt-tab").forEach(p=>{p.addEventListener("click",()=>{d=p.dataset.tab,e.querySelectorAll(".mkt-tab").forEach(u=>{const x=u===p;u.style.background=x?"var(--green)":"var(--tile-bg)",u.style.color=x?"#fff":"var(--tile-fg-dim)",u.style.borderColor=x?"var(--green)":"var(--tile-border)"}),y()})});let g;["flt-name","flt-club","flt-country","flt-job","flt-rarity","flt-note1","flt-note2"].forEach(p=>{var u;(u=document.getElementById(p))==null||u.addEventListener("input",()=>{clearTimeout(g),g=setTimeout(y,250)})}),y()}async function ta(e,t,i,n){const{state:r,toast:o,refreshProfile:a}=n,s=t.find(l=>l.id===e);if(!s)return;const d=s.price;if((r.profile.credits||0)<d){o("Crédits insuffisants","error");return}ia(s,async()=>{const{error:l}=await _.rpc("buy_market_card",{p_listing_id:e,p_buyer_id:r.profile.id});if(l){o("Erreur achat : "+l.message,"error");return}await a();const m=document.getElementById("nav-credits");m&&(m.textContent=`💰 ${(r.profile.credits||0).toLocaleString("fr")}`),o("✅ Carte achetée !","success"),await en(i,n)})}function ia(e,t){var a;const i=(a=e.card)==null?void 0:a.player,n=i?`${i.firstname} ${i.surname_real}`:"cette carte",r=document.createElement("div");r.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",r.innerHTML=`
    ...
    </div>`,document.body.appendChild(r);const o=s=>{r.remove(),s&&t()};r.querySelector("#buy-cancel").addEventListener("click",()=>o(!1)),r.querySelector("#buy-ok").addEventListener("click",()=>o(!0)),r.addEventListener("click",s=>{s.target===r&&o(!1)})}async function na(e,t,i){const{toast:n}=i,{data:r}=await _.from("market_listings").select("card_id").eq("id",e).single();if(await _.from("market_listings").update({status:"cancelled"}).eq("id",e),r!=null&&r.card_id){const{count:o}=await _.from("market_listings").select("id",{count:"exact",head:!0}).eq("card_id",r.card_id).eq("status","active");o||await _.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",r.card_id)}n("Annonce retirée","success"),en(t,i)}async function oa(e,t){var y,g,p,u,x;const{state:i,navigate:n}=t,r=((g=(y=t==null?void 0:t.state)==null?void 0:y.params)==null?void 0:g.tab)||"ranked";e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:var(--tile-fg-dim)">⚽ Chargement...</div>';const[{data:o},{data:a},{data:s}]=await Promise.all([_.from("users").select("id,pseudo,club_name,mmr,rank_tier,ranked_wins,ranked_losses,ranked_draws,placement_matches").gte("placement_matches",1).order("mmr",{ascending:!1}).limit(100),_.rpc("get_mini_league_leaderboard"),_.rpc("get_random_leaderboard")]);let d=r;const c=h=>h<3?["#D4A017","#a0a0a0","#cd7f32"][h]:"var(--green)",l=h=>h<3?"#000":"#fff";function m(){var $,f,M;const h=document.getElementById("rankings-legend");h&&(h.innerHTML=d==="random"?'<div style="font-size:11px;color:var(--tile-fg-dim);text-align:center;margin-bottom:10px">📊 = score de classement fiable (tient compte du nombre de matchs joués, pas seulement du %)</div>':"");const v=document.getElementById("rankings-list");if(v){if(d==="ranked"){const k=o||[];v.innerHTML=k.length>0?k.map((I,B)=>{const ee=Hi(I.mmr??1e3),te=(I.ranked_wins||0)+(I.ranked_losses||0)+(I.ranked_draws||0),ne=te>0?Math.round((I.ranked_wins||0)/te*100):0,z=I.id===i.profile.id,S=(I.placement_matches||0)<10;return`
          <div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${z?"border:2px solid var(--yellow)":""}">
            ...
            <div style="flex:1;min-width:0">
              ...
              <div style="font-size:11px;color:var(--tile-fg-dim)">${I.club_name} · ${ee.label}</div>
            </div>
            <div style="text-align:right;flex-shrink:0">
              <div style="font-size:20px">${S?"❓":ee.emoji}</div>
              <div style="font-size:11px;font-weight:700;color:${ee.color}">${S?"Placement":ee.label}</div>
              ${S?"":`<div style="font-size:10px;color:var(--tile-fg-dim)">${ne}% WR</div>`}
            </div>
  ...
  <div class="page">
    ...
    </div>`,(M=document.getElementById("ranked-back"))==null||M.addEventListener("click",()=>n("home"));return}const d=s.mmr??1e3,c=s.mmr_deviation??350,l=s.mmr_volatility??.06,m=s.placement_matches??0,b=m<10,y=Math.max(0,10-m),g=Hi(d),p=ao(d),u=Li.findIndex(ee=>ee.id===g.id),x=Li[u+1]||null,h={bronze:"linear-gradient(160deg,#3d1c00,#7a3e00)",silver:"linear-gradient(160deg,#1a1a2e,#3a3a5e)",gold:"linear-gradient(160deg,#1a1200,#4a3500)",platinum:"linear-gradient(160deg,#001a20,#003040)",diamond:"linear-gradient(160deg,#001030,#1a2860)",master:"linear-gradient(160deg,#1a0030,#3d0070)"},v=(s.ranked_wins||0)+(s.ranked_losses||0)+(s.ranked_draws||0),$=v>0?Math.round((s.ranked_wins||0)/v*100):0,f=Li.map(ee=>`
  ...
  </div>`,(k=document.getElementById("ranked-back"))==null||k.addEventListener("click",()=>n("home")),(I=document.getElementById("ranked-leaderboard-btn"))==null||I.addEventListener("click",()=>n("rankings",{tab:"ranked"})),(B=document.getElementById("ranked-play-btn"))==null||B.addEventListener("click",()=>{n("match",{matchMode:"ranked",rankedData:{mmr:d,rd:c,sigma:l,isPlacement:b}})})}async function aa(e,{state:t,navigate:i,toast:n}){const r=t.profile;if(!r)return;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await _.from("matches").select(`id,home_id,away_id,home_score,away_score,status,mode,winner_id,created_at,played_at,
      home:users!home_id(pseudo,club_name),
      away:users!away_id(pseudo,club_name)`).or(`home_id.eq.${r.id},away_id.eq.${r.id}`).order("created_at",{ascending:!1}).limit(50),a={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},s=(o||[]).filter(l=>l.status==="finished"),d=(o||[]).filter(l=>l.status==="in_progress");function c(l){const m=l.home_id===r.id;m?l.home_score:l.away_score,m?l.away_score:l.home_score;const b=l.winner_id===r.id,y=l.home_score===l.away_score&&l.status==="finished",g=l.status!=="finished"?"…":y?"N":b?"V":"D",p=l.status!=="finished"||y?"#888":b?"#1A6B3C":"#c0392b";let u=a[l.mode]||l.mode;l.away_id===null&&!u.startsWith("IA")&&(u="IA");const h=l.home_id===r.id?l.away:l.home;let v;l.away_id===null?v=u:h?v=`${h.club_name||h.pseudo} (${h.pseudo})`:v="Adversaire";let $="";l.status==="in_progress"&&Date.now()-new Date(l.created_at).getTime()>3600*1e3&&($=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const f=new Date(l.created_at),M=f.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+f.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),k=l.status==="finished"?`${l.home_score} - ${l.away_score}`:`${l.home_score||0} - ${l.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
  ...
  </div>`}so(co);const xe={user:null,profile:null,page:"home",params:{}};function Ft(e,t="info",i=3e3){const n=document.getElementById("toast");n&&(n.textContent=e,n.className=`show ${t}`,clearTimeout(n._t),n._t=setTimeout(()=>{n.className=""},i))}function sa(e,t,i=""){document.getElementById("modal-title").textContent=e,document.getElementById("modal-body").innerHTML=t,document.getElementById("modal-footer").innerHTML=i,document.getElementById("modal-overlay").classList.remove("hidden")}function Ni(){document.getElementById("modal-overlay").classList.add("hidden")}async function Zt(){if(!xe.user)return;const{data:e}=await _.from("users").select("*").eq("id",xe.user.id).single();e&&(xe.profile=e)}const eo="mw_theme";function to(){return localStorage.getItem(eo)||"dark"}function da(e){var t;localStorage.setItem(eo,e),la(e),(t=xe.profile)!=null&&t.id&&_.from("users").update({theme:e}).eq("id",xe.profile.id).then(()=>{})}function la(e){document.documentElement.setAttribute("data-theme",e)}function Pt(e,t={}){xe.page=e,xe.params=t,io()}async function io(){var n,r,o,a;const e=document.getElementById("page-content");if(!e)return;document.querySelectorAll(".bottom-nav a").forEach(s=>{s.classList.toggle("active",s.dataset.page===xe.page)});const t=document.getElementById("nav-credits");t&&xe.profile&&(t.textContent=`💰 ${(xe.profile.credits||0).toLocaleString("fr")}`);const i={state:xe,navigate:Pt,toast:Ft,openModal:sa,closeModal:Ni,refreshProfile:Zt};switch(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽</div>',xe.page){case"home":await fn(e,i);break;case"settings":await jn(e,i);break;case"collection":await jo(e,i);break;case"decks":await Ci(e,i);break;case"boosters":await Vo(e,i);break;case"ranked":await ra(e,i);break;case"match":{const s=xe.params&&xe.params.matchMode||"vs_ai_easy";s==="random"?await Pi(e,i,!1):s==="ranked"?await Pi(e,i,!0):s==="friend"?await Mr(e,i,(n=xe.params)==null?void 0:n.friendId,(r=xe.params)==null?void 0:r.friendName):s==="mini_league"||s==="mini-league"?await $n(e,i,(o=xe.params)==null?void 0:o.mlMatchId,(a=xe.params)==null?void 0:a.leagueId):await dr(e,i);break}case"market":await ea(e,i);break;case"rankings":await oa(e,i);break;case"matches":await aa(e,i);break;case"friends":await xo(e,i);break;case"mini-league":await qr(e,i);break;case"match-mini-league":{const s=xe.params||{};await $n(e,i,s.mlMatchId,s.leagueId);break}default:await fn(e,i)}}function ca(){var n,r;const e=document.getElementById("app"),t=xe.profile;if(!t)return;const i="/icons/";e.innerHTML=`
    <nav class="top-nav">
      <div class="logo" id="nav-logo" title="Manager Wars v2026.07.23-1123" style="cursor:pointer">
  ...
  `,document.querySelectorAll(".bottom-nav a").forEach(o=>{o.addEventListener("click",a=>{a.preventDefault(),Pt(o.dataset.page)})}),document.getElementById("nav-logo").addEventListener("click",()=>Pt("home")),document.getElementById("nav-credits").addEventListener("click",()=>Pt("boosters")),(n=document.getElementById("journal-btn"))==null||n.addEventListener("click",()=>pa()),(r=document.getElementById("settings-btn"))==null||r.addEventListener("click",()=>Pt("settings"))}async function pa(){const{data:e}=await _.from("patch_notes").select("*").eq("is_published",!0).order("published_at",{ascending:!1}).limit(20),t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9000;display:flex;align-items:center;justify-content:center;padding:16px";const i=(e||[]).map(n=>{const r=new Date(n.published_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"long",year:"numeric"});return`<div style="padding:14px 0;border-bottom:1px solid #f0f0f0">
    ...
    </div>`,document.body.appendChild(t),t.querySelector("#journal-close").addEventListener("click",()=>t.remove()),t.addEventListener("click",n=>{n.target===t&&t.remove()})}function ua(e,{onPlay:t}){var n;const i="/icons/";e.style.cssText="",e.innerHTML=`
  <style>
    #public-landing{
  ...
  </div>`,(n=e.querySelector("#pl-play-btn"))==null||n.addEventListener("click",t)}async function fa(){document.documentElement.setAttribute("data-theme",to()),document.getElementById("modal-overlay").addEventListener("click",i=>{i.target===i.currentTarget&&Ni()}),document.getElementById("modal-close").addEventListener("click",Ni);const{data:{session:e}}=await _.auth.getSession();if(!e){En(),ua(document.getElementById("app"),{onPlay:()=>ln(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:Ft})});return}xe.user=e.user,await Zt(),En();try{const{data:i}=await _.from("formation_links_overrides").select("formation, links"),n={};(i||[]).forEach(r=>{n[r.formation]=r.links}),lo(n)}catch(i){console.warn("Impossible de charger les overrides de formation:",i)}if(!xe.profile){uo(document.getElementById("app"),{state:xe,navigate:async()=>{await Zt(),Ai()},toast:Ft,refreshProfile:Zt});return}const t=Array.isArray(xe.profile.pending_boosters)?xe.profile.pending_boosters:[];if(!xe.profile.onboarding_done&&t.length>0){tr(document.getElementById("app"),{state:xe,navigate:()=>Ai(),toast:Ft,refreshProfile:Zt});return}Ai(),setTimeout(()=>Zr(xe.profile,Pt,Ft),800),_.auth.onAuthStateChange(async(i,n)=>{i==="SIGNED_OUT"&&(xe.user=null,xe.profile=null,document.getElementById("app").innerHTML="",ln(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:Ft}))})}function ma(){return Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight)}function ki(){const e=document.getElementById("app");e&&(e.style.height=ma()+"px")}window.addEventListener("resize",ki);window.addEventListener("orientationchange",()=>setTimeout(ki,150));window.visualViewport&&window.visualViewport.addEventListener("resize",ki);function Ai(){const e=()=>{var i;(i=xe.user)!=null&&i.id&&_.from("users").update({last_seen_at:new Date().toISOString()}).eq("id",xe.user.id).then(()=>{})};e(),window._presencePingInterval&&clearInterval(window._presencePingInterval),window._presencePingInterval=setInterval(e,6e4);const t=document.getElementById("app");t.style.display="flex",t.style.flexDirection="column",ki(),ca(),io()}function En(){const e=document.getElementById("app-loader"),t=document.getElementById("app");t&&(t.style.display=""),e&&(e.classList.add("zoom-out"),setTimeout(()=>e.style.display="none",500))}function no(e){var n;const t=document.getElementById("app-loader");if(t&&(t.style.display="none"),document.getElementById("boot-error"))return;const i=document.createElement("div");i.id="boot-error",i.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:99999;gap:16px;color:#fff;padding:24px;text-align:center",i.innerHTML=`
    <div style="font-size:42px">📡</div>
    <div style="font-size:18px;font-weight:900">Connexion impossible</div>
    <div style="font-size:13px;color:rgba(255,255,255,0.6);max-width:280px">${e||"Le chargement a échoué. Vérifie ta connexion et réessaie."}</div>
    <button id="boot-retry" style="margin-top:8px;padding:12px 30px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer">Réessayer</button>`,document.body.appendChild(i),(n=document.getElementById("boot-retry"))==null||n.addEventListener("click",()=>window.location.reload())}fa().catch(e=>{console.error("Échec du démarrage:",e),no()});setTimeout(()=>{const e=document.getElementById("app-loader");e&&e.style.display!=="none"&&!e.classList.contains("zoom-out")&&!document.getElementById("boot-error")&&no("Le serveur met trop de temps à répondre.")},12e3);
