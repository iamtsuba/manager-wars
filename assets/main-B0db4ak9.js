...
import{s as _,q as Ut,t as Ri,F as Oi,o as tt,n as ti,g as At,r as Ie,m as zt,u as zi,j as Hi,v as ao,w as so,T as Li,p as lo,x as co,d as po}from"./special-cards-DZxbxWIu.js";const uo="/";function ln(e,{navigate:t,toast:i}){let n="login";const r=()=>{var a,s,d,c,l,m,b;const o=n==="login";if(e.innerHTML=`
    <div style="
  ...
  `;function s(){var g;const c=document.getElementById("logo-preview"),l=document.getElementById("logo-initials"),m=((g=document.getElementById("setup-club"))==null?void 0:g.value)||"MW",b=m.trim().split(" ").filter(Boolean),y=b.length>=2?(b[0][0]+b[1][0]).toUpperCase():m.slice(0,2).toUpperCase();c&&(c.style.background=a,c.style.borderColor=o),l&&(l.textContent=y,l.style.color=o)}document.getElementById("color1").addEventListener("input",c=>{o=c.target.value,document.getElementById("swatch1").style.background=o,s()}),document.getElementById("color2").addEventListener("input",c=>{a=c.target.value,document.getElementById("swatch2").style.background=a,s()});function d(c){document.querySelectorAll(".setup-step").forEach(l=>l.classList.remove("active")),document.getElementById(`step-${c}`).classList.add("active"),document.getElementById("step-num").textContent=c,document.getElementById("progress-fill").style.width=`${Math.round(c/3*100)}%`,c===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("step1-error");if(l.textContent="",c.length<3){l.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:m}=await _.from("users").select("id").eq("pseudo",c).maybeSingle();if(m){l.textContent="Ce pseudo est déjà pris.";return}d(2)}),document.getElementById("step2-back").addEventListener("click",()=>d(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const c=document.getElementById("setup-club").value.trim(),l=document.getElementById("step2-error");if(l.textContent="",c.length<2){l.textContent="Nom trop court (min. 2 caractères).";return}const{data:m}=await _.from("users").select("id").eq("club_name",c).maybeSingle();if(m){l.textContent="Ce nom de club est déjà pris.";return}d(3)}),document.getElementById("step3-back").addEventListener("click",()=>d(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("setup-club").value.trim(),m=document.getElementById("step3-error"),b=document.getElementById("step3-finish");m.textContent="",b.disabled=!0,b.textContent="Création en cours…";try{const{error:y}=await _.from("users").insert({id:t.user.id,pseudo:c,club_name:l,club_color1:o,club_color2:a,credits:1e4});if(y)throw y;await mo(t.user.id),await r(),n(`Bienvenue ${c} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(y){m.textContent=y.message,b.disabled=!1,b.textContent="🚀 Créer mon profil !"}})}async function mo(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await _.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const go="modulepreload",yo=function(e){return"/"+e},cn={},Ln=function(t,i,n){let r=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),s=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));r=Promise.allSettled(i.map(d=>{if(d=yo(d),d in cn)return;cn[d]=!0;const c=d.endsWith(".css"),l=c?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${l}`))return;const m=document.createElement("link");if(m.rel=c?"stylesheet":go,c||(m.as="script"),m.crossOrigin="",m.href=d,s&&m.setAttribute("nonce",s),document.head.appendChild(m),c)return new Promise((b,y)=>{m.addEventListener("load",b),m.addEventListener("error",()=>y(new Error(`Unable to preload CSS for ${d}`)))})}))}function o(a){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=a,window.dispatchEvent(s),!s.defaultPrevented)throw a}return r.then(a=>{for(const s of a||[])s.status==="rejected"&&o(s.reason);return t().catch(o)})},ii="#1A6B3C",ni="#cc2222",xo="#D4A017",pn="#888";async function bo(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    </style>`}const Sn="mw_sound_volume";function Vi(){const e=localStorage.getItem(Sn);if(e===null)return 100;const t=parseInt(e,10);return Number.isFinite(t)?Math.max(0,Math.min(100,t)):100}function wo(e){localStorage.setItem(Sn,String(Math.max(0,Math.min(100,Math.round(e)))))}function Yi(){return Vi()===0}function fi(e){return Math.max(0,Math.min(1,e*(Vi()/100)))}function Wi(e,t=1){if(Yi())return null;try{const i=new Audio(e);return i.volume=fi(t),i.play().catch(()=>{}),i}catch{return null}}let gt=null,Mi=null,un=.3;function An(e,t=.3){if(un=t,gt&&Mi===e&&!gt.paused){gt.volume=fi(un);return}if(Gt(),!Yi())try{const i=new Audio(e);i.loop=!0,i.volume=fi(t),i.play().catch(()=>{}),gt=i,Mi=e}catch{}}function Gt(){if(gt)try{gt.pause(),gt.currentTime=0}catch{}gt=null,Mi=null}let Xt=null;function zn(e,t=.6){if(Ue(),!Yi())try{const i=new Audio(e);i.volume=fi(t),i.play().catch(()=>{}),Xt=i}catch{}}function Ue(){if(Xt)try{Xt.pause(),Xt.currentTime=0}catch{}Xt=null}const ko="2026.07.22-1649";async function fn(e,{state:t,navigate:i,toast:n}){var o,a;const r=t.profile;r&&(e.innerHTML=`
  <style>
    .home-dark {
      min-height: 100%;
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    ${Ie(e,{width:140})}
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),c=(s||[]).filter($=>$.card_type==="player"&&$.player),l=(s||[]).filter($=>$.card_type==="game_changer"),m=(s||[]).filter($=>$.card_type==="formation"),b=(s||[]).filter($=>$.card_type==="stadium"),{data:y}=await _.from("gc_definitions").select("name,gc_type,color,effect,image_url"),g={};(y||[]).forEach($=>{g[$.name]=$});const{data:p}=await _.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),u={};(p||[]).forEach($=>{u[$.id]=$}),b.forEach($=>{$.stadium_def&&(u[$.stadium_id]=$.stadium_def)});const x=Object.keys(Oi),h=Object.keys(je),v={};c.forEach($=>{const q=$.player.id;v[q]=(v[q]||0)+1}),new Set(Object.keys(v).map($=>String($)));const E=new Set(m.map($=>$.formation)),f=new Set(l.map($=>$.gc_type));let z="player",k="Tous",S="",D=!1;function te(){return[...c].sort(($,q)=>{const U=di.indexOf($.player.job),R=di.indexOf(q.player.job);return U!==R?U-R:($.player.surname_real||"").localeCompare(q.player.surname_real||"")})}function ie(){return[...d||[]].sort(($,q)=>{const U=di.indexOf($.job),R=di.indexOf(q.job);return U!==R?U-R:($.surname_real||"").localeCompare(q.surname_real||"")})}function Q(){return te().filter($=>{const q=$.player,U=k==="Tous"||q.job===k,R=!S||`${q.firstname} ${q.surname_real}`.toLowerCase().includes(S);return U&&R})}function B(){return ie().filter($=>{const q=k==="Tous"||$.job===k,U=!S||`${$.firstname} ${$.surname_real}`.toLowerCase().includes(S);return q&&U})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:0 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none;align-items:center"></div>
  ...
  </div>`,function(q){const U=document.getElementById(q);if(!U)return;U.addEventListener("wheel",de=>{Math.abs(de.deltaY)<=Math.abs(de.deltaX)||(de.preventDefault(),U.scrollLeft+=de.deltaY)},{passive:!1});let R=!1,oe=0,C=0,O=!1,le=!1;U.addEventListener("mousedown",de=>{R=!0,O=!1,oe=de.pageX,C=U.scrollLeft}),window.addEventListener("mouseup",()=>{R=!1,O&&(U.style.cursor="",le=!0),O=!1}),window.addEventListener("mousemove",de=>{if(!R)return;const he=de.pageX-oe;!O&&Math.abs(he)<6||(O=!0,U.style.cursor="grabbing",de.preventDefault(),U.scrollLeft=C-he)}),U.addEventListener("click",de=>{le&&(de.stopPropagation(),de.preventDefault(),le=!1)},!0)}("col-grid");function I(){const $=document.getElementById("col-filters");$&&(z==="player"?($.innerHTML=`
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",q=>{S=q.target.value.toLowerCase(),K()}),e.querySelectorAll(".filter-btn").forEach(q=>{q.addEventListener("click",()=>{k=q.dataset.job,I(),K()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{D=!D,I(),K()})):($.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{D=!D,I(),K()})))}function K(){const $=document.getElementById("col-grid");$&&(z==="player"?V($):z==="formation"?pe($):z==="stadium"?N($):ce($))}function J($,q,U,R,oe,C=0){const O=document.getElementById("col-grid"),le=document.getElementById("col-big");if(!O||!le)return;var de=0;function he(){var ct;const Se=window.innerWidth>=768,ve=document.getElementById("col-big"),Te=document.getElementById("col-gap"),Ae=((ct=document.getElementById("col-grid"))==null?void 0:ct.parentElement)||null,Be=ve?ve.closest(".page"):null;if(ve&&Ae&&Be){let $e=0;Array.from(Be.children).forEach(function(j){j!==ve&&j!==Ae&&j!==Te&&($e+=j.offsetHeight)});const Ce=Math.max(0,Be.clientHeight-$e),qe=Math.round(Ce*(C/100)),Ge=Math.max(0,Ce-qe),w=Se?50/71:45/63,L=Math.round(Ge*w),A=Math.max(0,Ge-L);Te&&(Te.style.height=qe+"px"),ve.style.flex="none",ve.style.height=L+"px",ve.style.minHeight="0",Ae.style.flex="none",Ae.style.height=A+"px",Ae.style.minHeight="0",Ae.style.display="flex",Ae.style.overflow="hidden"}const ke=document.getElementById("col-grid");ke&&(ke.style.height="100%",ke.style.flexShrink="0",ke.style.overflowX="auto",ke.style.overflowY="hidden",ke.style.alignItems="center",ke.style.width="100%"),le.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:center center">'+q($[de])+"</div>";var wt=le.querySelector("[data-card-id],[data-form-id],[data-gc-id]");wt&&wt.addEventListener("click",function(){R($[de])}),requestAnimationFrame(function(){var $e=document.getElementById("big-card-inner");if(!(!$e||!le)){var Ce=le.clientHeight,qe=le.clientWidth-16,Ge=$e.offsetHeight,w=$e.offsetWidth;if(Ge>0&&w>0&&Ce>40){var L=Math.min(Ce/Ge,qe/w);$e.style.transform="scale("+L.toFixed(3)+")",$e.style.transformOrigin="center center"}}}),O.innerHTML=$.map(function($e,Ce){var qe=Ce===de,Ge="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+(qe?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+Ce+'" style="'+Ge+'">'+U($e,qe)+"</div>"}).join(""),O.querySelectorAll(".col-mini-item").forEach(function($e){$e.addEventListener("click",function(){de=Number($e.dataset.idx),he(),$e.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})}),requestAnimationFrame(function(){var $e=ke?ke.clientHeight:0,Ce=O.querySelector(".col-mini-item > div");if(!(!$e||!Ce)){var qe=Ce.style.zoom;Ce.style.zoom="1";var Ge=Ce.offsetHeight;if(Ce.style.zoom=qe,!(Ge<=0)){var w=$e/Ge;O.querySelectorAll(".col-mini-item > div").forEach(function(L){L.style.zoom=w.toFixed(4)})}}})}he()}function ue($){var q=window.innerWidth>=768?.76:.54,U;if(!$||$._fake){var R=$?$.player:null;if(!R)return"";U=xn(R)}else U=Ii($,"");return'<div style="display:inline-block;zoom:'+q+';pointer-events:none;line-height:0">'+U+"</div>"}function fe($,q,U){var R=U>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+U+"</div>":"",oe=!!q,C=zi($,zt[$],{width:160});return oe||(C='<div style="filter:grayscale(1);opacity:0.5">'+C+"</div>"),"<div "+(q?'data-form-id="'+q.id+'"':"")+' style="position:relative;cursor:pointer">'+R+C+"</div>"}function M($,q){var U=window.innerWidth>=768?.76:.54,R=zi($,zt[$],{width:140});return q||(R='<div style="filter:grayscale(1);opacity:0.45">'+R+"</div>"),'<div style="display:inline-block;zoom:'+U+';line-height:0;pointer-events:none">'+R+"</div>"}function V($){if(D){const q=B();if(!q.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucun joueur.</div>';return}const U=q.map(R=>c.find(oe=>oe.player.id===R.id)||{_fake:!0,player:R,id:"fake-"+R.id});J(U,R=>R._fake?xn(R.player):Ii(R,""),R=>R._fake?ue({player:R.player,id:"x",_fake:!0}):ue(R),R=>{R._fake||bn(R,c,v,t)})}else{const q=Q();if(!q.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const U={};q.forEach(oe=>{const C=oe.player.id;U[C]||(U[C]=[]),U[C].push(oe)});const R=Object.values(U).map(oe=>qo(oe));J(R,oe=>{const C=v[oe.player.id]||1,O=C>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${C}</div>`:"",de=c.filter(he=>he.player.id===oe.player.id&&he.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Ii(oe,O+de)},oe=>ue(oe),oe=>bn(oe,c,v,t))}}function pe($){const q=D?x:[...E];if(!q.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const U=q.map(R=>({formation:R,card:m.find(oe=>oe.formation===R)||null,owned:E.has(R)}));J(U,({formation:R,card:oe,owned:C})=>fe(R,C?oe:null,C?m.filter(O=>O.formation===R).length:0),({formation:R,owned:oe})=>M(R,oe),({card:R,owned:oe})=>{oe&&R&&No(R,m,t,o)},"#1A6B3C",5)}function ce($){const q=Object.keys(g),U=D?q.length?q:h:[...f];if(!U.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const R=U.map(oe=>({type:oe,gc:je[oe]||{icon:"⚡",desc:""},def:g[oe]||null,owned:f.has(oe),card:l.find(C=>C.gc_type===oe)||null}));J(R,({type:oe,gc:C,def:O,owned:le,card:de})=>{var ke;const he=(O==null?void 0:O.name)||oe,Se=le?l.filter(wt=>wt.gc_type===oe).length:0,ve=Se>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${Se}</div>`:"",Te=(O==null?void 0:O.effect)||C.desc||"",Ae=O!=null&&O.image_url?`/icons/${O.image_url}`:((ke=O==null?void 0:O.club)==null?void 0:ke.logo_url)||(O!=null&&O.country_code?`https://flagsapi.com/${O.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null);let Be=tt(he,Ae,C.icon,Te,{width:160,onClick:le});return le||(Be=`<div style="filter:grayscale(1);opacity:0.5">${Be}</div>`),`<div ${le&&de?`data-gc-id="${de.id}" data-gc-type="${oe}"`:""} style="position:relative">${ve}${Be}</div>`},({type:oe,gc:C,def:O,owned:le})=>{const de=window.innerWidth>=768?.76:.54,he=(O==null?void 0:O.name)||oe,Se=(O==null?void 0:O.effect)||C.desc||"",ve=O!=null&&O.image_url?`/icons/${O.image_url}`:null;let Te=tt(he,ve,C.icon,Se,{width:140});return le||(Te=`<div style="filter:grayscale(1);opacity:0.45">${Te}</div>`),`<div style="display:inline-block;zoom:${de};line-height:0;pointer-events:none">${Te}</div>`},({type:oe,owned:C,def:O})=>{C&&Go(oe,O,o)},"#7a28b8",5)}function N($){const q="#4FC3F7",U="/";if(!b.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Stade.<br><small>Ouvre un booster Stade !</small></div>';return}const R={};b.forEach(C=>{const O=C.stadium_id||"?";(R[O]=R[O]||[]).push(C)});const oe=Object.entries(R).map(([C,O])=>({sid:C,def:u[C]||null,count:O.length,card:O[0]}));J(oe,({def:C,count:O})=>{var Te,Ae;const le=(C==null?void 0:C.name)||"?",de=((Te=C==null?void 0:C.club)==null?void 0:Te.encoded_name)||(C==null?void 0:C.country_code)||"—",he=C!=null&&C.image_url?`${U}icons/${C.image_url}`:((Ae=C==null?void 0:C.club)==null?void 0:Ae.logo_url)||(C!=null&&C.country_code?`https://flagsapi.com/${C.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),Se=O>1?`<div style="position:absolute;top:8px;right:8px;background:#333;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${O}</div>`:"",ve=`${de}<br>+10 ⭐ joueurs alliés`;return`<div style="position:relative">${Se}${ti(le,he,ve,{width:160})}</div>`},({def:C})=>{var ve,Te;const O=window.innerWidth>=768?.76:.54,le=(C==null?void 0:C.name)||"?",de=((ve=C==null?void 0:C.club)==null?void 0:ve.encoded_name)||(C==null?void 0:C.country_code)||"—",he=C!=null&&C.image_url?`${U}icons/${C.image_url}`:((Te=C==null?void 0:C.club)==null?void 0:Te.logo_url)||(C!=null&&C.country_code?`https://flagsapi.com/${C.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),Se=`${de}<br>+10 ⭐`;return`<div style="display:inline-block;zoom:${O};line-height:0;pointer-events:none">${ti(le,he,Se,{width:140})}</div>`},null,q,5)}e.querySelectorAll(".col-tab-btn").forEach($=>{$.addEventListener("click",()=>{z=$.dataset.tab,k="Tous",S="",D=!1,e.querySelectorAll(".col-tab-btn").forEach(q=>{const U=q.dataset.tab===z;q.style.borderBottomColor=U?"var(--green)":"transparent",q.style.color=U?"var(--green)":"var(--tile-fg-dim)"}),I(),K()})}),I(),K()}function Go(e,t,i){const n=je[e]||{icon:"⚡",desc:"Effet spécial."},r=(t==null?void 0:t.name)||e,o=(t==null?void 0:t.effect)||n.desc,a=t!=null&&t.image_url?`/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
      ${tt(r,a,n.icon,o,{width:200})}
      <div style="background:#fff3cd;border-radius:10px;padding:10px 14px;width:100%">
        <div style="font-size:12px;color:#856404">⚠️ Cette carte est à <b>usage unique</b>. Une fois jouée en match, elle est définitivement supprimée de ta collection.</div>
      </div>
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const ci=1e3;function No(e,t,i,n){var g,p,u;const{state:r,toast:o,closeModal:a,navigate:s,refreshProfile:d}=i,c=e.formation,l={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function m(){const x=zt[c]||{},h=Oi[c]||[],v=290,E=360,f=20;function z(S){const D=x[S];return D?{x:D.x*v,y:D.y*E}:null}let k=`<svg width="${v}" height="${E}" viewBox="0 0 ${v} ${E}" xmlns="http://www.w3.org/2000/svg">`;for(const[S,D]of h){const te=z(S),ie=z(D);!te||!ie||(k+=`<line x1="${te.x}" y1="${te.y}" x2="${ie.x}" y2="${ie.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const S of Object.keys(x)){const D=z(S);if(!D)continue;const te=S.replace(/\d+/,""),ie=l[te]||"#555";k+=`<circle cx="${D.x}" cy="${D.y}" r="${f}" fill="${ie}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,k+=`<text x="${D.x}" y="${D.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${te}</text>`}return k+="</svg>",k}const b=t.filter(x=>x.formation===c);b.length;const y=!e.is_for_sale;n(`Formation ${c}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(g=document.getElementById("direct-sell-form-btn"))==null||g.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${c} pour ${ci.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const x=b.find(v=>!v.is_for_sale)||b[0];if(!x){o("Aucune carte à vendre","error");return}await _.from("market_listings").delete().eq("card_id",x.id),await _.from("transfer_history").delete().eq("card_id",x.id);const{error:h}=await _.from("cards").delete().eq("id",x.id);if(h){o(h.message,"error");return}await _.from("users").update({credits:(r.profile.credits||0)+ci}).eq("id",r.profile.id),await d(),o(`+${ci.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),a(),s("collection")}),(p=document.getElementById("market-sell-form-btn"))==null||p.addEventListener("click",async()=>{const x=parseInt(document.getElementById("sell-price-form").value);if(!x||x<1){o("Prix invalide","error");return}await _.from("cards").update({is_for_sale:!0,sale_price:x}).eq("id",e.id),await _.from("market_listings").insert({seller_id:r.profile.id,card_id:e.id,price:x}),o("Carte mise en vente sur le marché !","success"),a(),s("collection")}),(u=document.getElementById("cancel-sell-form-btn"))==null||u.addEventListener("click",async()=>{await _.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await _.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),o("Annonce retirée","success"),a(),s("collection")})}async function bn(e,t,i,n){var fe,M,V,pe,ce;const{state:r,toast:o,openModal:a,closeModal:s,navigate:d,refreshProfile:c}=n,l=e.player,m=t.filter(N=>N.player.id===l.id),b=m.length,y=e.evolution_bonus||0,g=Math.max((Number(l.note_g)||0)+(l.job==="GK"||l.job2==="GK"?y:0),(Number(l.note_d)||0)+(l.job==="DEF"||l.job2==="DEF"?y:0),(Number(l.note_m)||0)+(l.job==="MIL"||l.job2==="MIL"?y:0),(Number(l.note_a)||0)+(l.job==="ATT"||l.job2==="ATT"?y:0)),p=l.rarity||"normal",{data:u}=await _.from("sell_price_configs").select("*").eq("rarity",p).lte("note_min",g).gte("note_max",g).order("note_min",{ascending:!1}).limit(1);((fe=u==null?void 0:u[0])==null?void 0:fe.price)??Bo[p];const x=l.rarity!=="legende";At(l);const h=((l.rarity==="pepite"||l.rarity==="papyte")&&e.current_note!=null?e.current_note:li(l,l.job))+y,v=l.rarity==="pepite"||l.rarity==="papyte",E=l.job2?(v?zo(e,Do(l.job2)):li(l,l.job2))+(li(l,l.job2)>0?y:0):null;Ti[l.job],l.job2&&Ti[l.job2];const f=Co[l.rarity]||"#ccc";Fo[l.country_code]||l.country_code;const z=e.evolution_bonus||0,S=h+z,D=E||0,te=D>0?D+z:0,ie=m.map(N=>N.id);let Q={};if(ie.length){const{data:N}=await _.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",ie).order("transferred_at",{ascending:!0});(N||[]).forEach($=>{Q[$.card_id]||(Q[$.card_id]=[]),Q[$.card_id].push($)})}const B=N=>{const $=N.transferred_at?new Date(N.transferred_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"2-digit",year:"numeric"}):"",q=N.source==="booster"?"Booster":N.price?N.price.toLocaleString("fr"):"—";return`
    <div style="display:flex;justify-content:space-between;align-items:flex-start;background:#f9f9f9;border-radius:8px;padding:8px 12px;gap:8px">
      ...
      <div style="text-align:right;flex-shrink:0">
        <div style="font-size:13px;font-weight:700;color:${N.source==="booster"?"var(--tile-fg-dim)":"var(--yellow)"}">${q}</div>
        <div style="font-size:11px;color:var(--tile-fg-dim)">${$}</div>
      </div>
    </div>`},I=ie.length?`
    <div style="margin-top:16px;border-top:1px solid var(--tile-border);padding-top:14px">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${m.map((N,$)=>{const q=Q[N.id]||[],U=N.is_for_sale,R=q.length?q[q.length-1]:null,C=(l.rarity==="pepite"||l.rarity==="papyte")&&N.current_note!=null?` (☆${N.current_note})`:"";return`
            <div data-card-id="${N.id}" data-card-idx="${$}" class="exemplaire-row"
              style="border:2px solid #eee;border-radius:10px;padding:10px;cursor:pointer;transition:border-color .15s;${U?"opacity:0.6":""}">
              <div style="display:flex;align-items:center;gap:10px">
                <input type="checkbox" class="expl-check" data-id="${N.id}" data-evo="${N.evolution_bonus||0}" data-note="${li(l,l.job)}" ${U?"disabled":""} style="width:18px;height:18px;cursor:pointer;accent-color:#1A6B3C;flex-shrink:0">
                <div style="flex:1;min-width:0">
                  <div style="font-size:12px;font-weight:700">Exemplaire ${$+1}${C}${U?" 🏷️ En vente":""}</div>
                  ${R?`<div style="font-size:11px;color:#888;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">
                    ${R.club_name} · ${R.source==="booster"?"Booster":R.price?R.price.toLocaleString("fr")+" cr.":"—"}
                  </div>`:""}
                </div>
                <button class="expl-hist-toggle" data-idx="${$}" style="font-size:11px;color:#999;border:none;background:none;cursor:pointer;padding:2px 6px;flex-shrink:0">
                  ${q.length?`${q.length} club${q.length>1?"s":""}`:""} ▾
                </button>
              </div>
              <div class="expl-hist" data-hist="${$}" style="display:none;margin-top:8px;display:flex;flex-direction:column;gap:4px">
                ${q.map(B).join("")}
              </div>
            </div>`}).join("")}
      </div>
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;background:#f0fdf4;border-radius:10px;padding:10px 14px">
          <div>
            ...
            <div style="font-size:11px;color:#555;margin-top:2px">
              ${(e.evolution_bonus||0)>0?`+${e.evolution_bonus} appliqué${e.evolution_bonus>1?"s":""} · Note : ${S}${l.job2&&D>0?` / ${te}`:""}`:`Note actuelle : ${S}${l.job2&&D>0?` / ${te}`:""}`}
            </div>
          </div>
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",l.note_g],["DEF",l.note_d],["MIL",l.note_m],["ATT",l.note_a]].map(([N,$])=>{const q=Ti[N],U=N==="GK"?"#fff":q,R=N===l.job||N===l.job2,oe=(Number($)||0)+(R&&z>0?z:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
                <div style="width:48px;height:48px;border-radius:8px;background:#111;border:2.5px solid ${q};
                  display:flex;align-items:center;justify-content:center">
                  <span style="font-size:20px;font-weight:900;color:${U};font-family:Arial Black,Arial;line-height:1">${oe}</span>
                </div>
                <span style="font-size:10px;font-weight:700;color:${U}">${N}</span>
              </div>`}).join("")}
          </div>
        </div>
        <div>
    ...
    ${I}
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(M=document.getElementById("close-detail"))==null||M.addEventListener("click",s);let K=new Set;function J(){let N=0;return document.querySelectorAll(".expl-check:checked").forEach($=>{if($.dataset.id===e.id)return;const U=Number($.dataset.evo)||0,R=Number($.dataset.note)||0;N+=R+U}),N}const ue=()=>{const N=K.size,$=document.getElementById("sell-action-panel");if(!$)return;$.style.display=N>0?"block":"none",document.getElementById("sell-selected-count").textContent=N;const q=document.getElementById("evolve-btn");if(q){const U=J();q.textContent=U>0?`⬆️ Évoluer (+${U})`:"⬆️ Évoluer"}};document.querySelectorAll(".expl-check").forEach(N=>{N.addEventListener("change",()=>{const $=N.dataset.id;N.checked?K.add($):K.delete($);const q=N.closest(".exemplaire-row");q&&(q.style.borderColor=N.checked?"#1A6B3C":"#eee"),ue()})}),document.querySelectorAll(".exemplaire-row").forEach(N=>{N.addEventListener("click",$=>{if($.target.closest("button")||$.target.tagName==="INPUT")return;const q=N.querySelector(".expl-check");q&&!q.disabled&&(q.checked=!q.checked,q.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(N=>{N.addEventListener("click",$=>{$.stopPropagation();const q=document.querySelector(`.expl-hist[data-hist="${N.dataset.idx}"]`);q&&(q.style.display=q.style.display==="none"?"flex":"none")})}),(V=document.getElementById("evolve-btn"))==null||V.addEventListener("click",async()=>{if(b<=1)return;const N=[...K];if(!N.length)return;if(K.has(e.id)){const C=document.createElement("div");C.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",C.innerHTML=`
        ...
        </div>`,document.body.appendChild(C),C.querySelector("#err-close").addEventListener("click",()=>C.remove()),C.addEventListener("click",O=>{O.target===C&&C.remove()});return}const $=N.filter(C=>C!==e.id);if(!$.length){o("Sélectionne des copies à sacrifier","warning");return}const q=$.reduce((C,O)=>{const le=document.querySelector(`.expl-check[data-id="${O}"]`),de=le&&Number(le.dataset.evo)||0,he=le&&Number(le.dataset.note)||0;return C+he+de},0);if(!await new Promise(C=>{const O=document.createElement("div");O.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",O.innerHTML=`
        <div style="background:var(--tile-bg);border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center;box-shadow:0 16px 48px rgba(0,0,0,0.3)">
          ...
          <div style="background:#f0fdf4;border-radius:10px;padding:12px;margin-bottom:16px;font-size:13px;color:#333">
            ...
            ⬆️ Bonus gagné : <strong style="color:#1A6B3C">+${q}</strong> <span style="font-size:11px;color:#888">(Σ note + évolution de chaque copie)</span><br>
            📈 Évolution : <strong>${e.evolution_bonus||0}</strong> → <strong style="color:#1A6B3C">${(e.evolution_bonus||0)+q}</strong><br>
            📊 Note finale : <strong>${h}</strong> → <strong style="color:#1A6B3C">${h+q}</strong>
            ${E&&E>0?`<br>📊 Note 2 finale : <strong>${E}</strong> → <strong style="color:#1A6B3C">${E+q}</strong>`:""}
          </div>
        ...
        </div>`,document.body.appendChild(O),O.querySelector("#sac-cancel").addEventListener("click",()=>{O.remove(),C(!1)}),O.querySelector("#sac-ok").addEventListener("click",()=>{O.remove(),C(!0)}),O.addEventListener("click",le=>{le.target===O&&(O.remove(),C(!1))})}))return;if($.length){await _.from("market_listings").delete().in("card_id",$),await _.from("deck_cards").delete().in("card_id",$),await _.from("transfer_history").delete().in("card_id",$),await _.from("decks").update({stadium_card_id:null}).in("stadium_card_id",$);const{error:C}=await _.from("cards").delete().in("id",$);if(C){o("Erreur suppression : "+C.message,"error");return}}const R=(e.evolution_bonus||0)+q,{error:oe}=await _.from("cards").update({evolution_bonus:R}).eq("id",e.id);if(oe){o("Erreur évolution : "+oe.message,"error");return}o(`⬆️ ${l.firstname} ${l.surname_real} : note ${h} → ${h+R} (+${q}) !`,"success",4e3),s(),d("collection")}),(pe=document.getElementById("market-sell-btn"))==null||pe.addEventListener("click",async()=>{var oe;const N=[...K];if(!N.length){o("Sélectionne au moins un exemplaire","warning");return}const $=parseInt((oe=document.getElementById("sell-market-price"))==null?void 0:oe.value);if(!$||$<1){o("Prix invalide","error");return}const{error:q}=await _.from("cards").update({is_for_sale:!0,sale_price:$}).in("id",N);if(q){o(q.message,"error");return}const U=N.map(C=>({seller_id:r.profile.id,card_id:C,price:$,status:"active"})),{error:R}=await _.from("market_listings").insert(U);R&&console.warn("[Market] insert listings:",R.message),o(`${N.length} carte${N.length>1?"s":""} mise${N.length>1?"s":""} en vente à ${$.toLocaleString("fr")} cr. chacune !`,"success"),s(),d("collection")}),(ce=document.getElementById("cancel-sell-btn"))==null||ce.addEventListener("click",async()=>{await _.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await _.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),o("Annonce retirée","success"),s(),d("collection")})}function vn(e,t=""){return new Promise(i=>{const n=document.createElement("div");n.className="modal-overlay",n.style.zIndex="2100",n.innerHTML=`<div class="modal" style="max-width:360px">
  ...
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
    ...
    <div class="page-body" style="padding-bottom:20px">
      ...
      ${a.map(v=>`<div data-forma="${v}" style="cursor:pointer;padding:10px 16px;border-radius:8px;background:${v===t.formation?"#1A6B3C":"#f0f0f0"};color:${v===t.formation?"#fff":"#111"};font-weight:900;font-size:16px;border:2px solid ${v===t.formation?"#1A6B3C":"#ddd"}">${v}</div>`).join("")}
    </div>`;u("⚽ Choisir une formation",h),document.querySelectorAll("#modal-body [data-forma]").forEach(v=>{v.addEventListener("click",()=>{t.formation=v.dataset.forma;const E=_n(t.formation),f={};E.forEach(z=>{t.slots[z]&&(f[z]=t.slots[z])}),t.slots=f,x(),_t(e,t,i)})})};e.querySelectorAll("#formation-mobile-btn, #formation-pc-btn").forEach(u=>u.addEventListener("click",y)),e.querySelectorAll("#add-stad-btn-pc, #add-stad-btn").forEach(u=>u.addEventListener("click",()=>Ho(t,e,i))),e.querySelectorAll("#save-deck").forEach(u=>u.addEventListener("click",()=>Vo(t,i))),e.querySelectorAll("[data-remove-sub]").forEach(u=>{u.addEventListener("click",()=>{t.subs=t.subs.filter(x=>x!==u.dataset.removeSub),_t(e,t,i)})}),e.querySelectorAll("#add-sub-btn").forEach(u=>u.addEventListener("click",()=>Ko(t,e,i)))}function Oo(e,t,i,n){var v,E,f;const r=window.innerWidth>=900,o=e.querySelector(r?"#deck-field-pc":"#deck-field-mobile");if(!o)return;const a=(v=t.stadiumCards)==null?void 0:v.find(z=>z.id===t.stadiumCardId),s=a&&((E=t.stadDefMap)==null?void 0:E[a.stadium_id])||null,d=zt[t.formation]||{},c=Ri(t.formation)||[],l={};for(const z of i){const k=t.slots[z],S=k?t.playerCards.find(D=>D.id===k):null;S!=null&&S.player?l[z]={...S.player,_evolution_bonus:S.evolution_bonus||0,face:S.player.face||null}:l[z]=null}const m=window.innerWidth>=900,b=m?window.innerWidth-280:window.innerWidth-20,y=m?Math.min(b,860):b,g=Math.round(m?y*.82:y*.85),p=m?84:44;let u="";for(const[z,k]of c){const S=d[z],D=d[k];if(!S||!D)continue;const te=S.x*y,ie=Math.round(.03*g+S.y*.85*g),Q=D.x*y,B=Math.round(.03*g+D.y*.85*g),I=l[z],K=l[k],J=Ut(I,K);J==="#ff3333"||J==="#cc2222"?u+=`<line x1="${te.toFixed(1)}" y1="${ie.toFixed(1)}" x2="${Q.toFixed(1)}" y2="${B.toFixed(1)}" stroke="${J}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(u+=`<line x1="${te.toFixed(1)}" y1="${ie.toFixed(1)}" x2="${Q.toFixed(1)}" y2="${B.toFixed(1)}" stroke="${J}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,u+=`<line x1="${te.toFixed(1)}" y1="${ie.toFixed(1)}" x2="${Q.toFixed(1)}" y2="${B.toFixed(1)}" stroke="${J}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let x="";const h=Math.round(p*657/507);for(const z of i){const k=d[z];if(!k)continue;const S=l[z],D=k.x*y;k.y*g;const te=Math.round(.03*g+k.y*(.85*g)),ie=Math.round(D-p/2),Q=Math.round(te-h/2);if(S){const B=z.replace(/\d+/,""),I=s&&(s.club_id&&String(S.club_id)===String(s.club_id)||s.country_code&&S.country_code===s.country_code),K=Ie({...S,_evolution_bonus:S._evolution_bonus||0},{width:p,showStad:!0,stadDef:s,role:B});I&&((f=s.club)!=null&&f.logo_url||s.image_url),x+=`<div style="position:absolute;left:${ie}px;top:${Q}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${z}">
        <div style="position:relative">${K}</div>
      </div>`}else{const B=z.replace(/\d+/,"");x+=`<div style="position:absolute;left:${ie}px;top:${Q}px;width:${p}px;height:${h}px;
        ...
        <span style="font-size:8px;color:rgba(255,255,255,0.3);margin-top:2px">${B}</span>
      </div>`}}o.innerHTML=`
  ...
  `,document.body.appendChild(r);let o=!1;const a=document.getElementById("pack-cut-zone"),s=document.getElementById("pack-blade");let d=!1;const c=I=>I.touches&&I.touches[0]?I.touches[0].clientX:I.clientX;function l(I){o||(d=!0,s.style.opacity="1",m(I))}function m(I){if(!d||o)return;const K=a.getBoundingClientRect(),J=c(I)-K.left,ue=Math.max(0,Math.min(1,J/K.width));s.style.width=ue*K.width+"px",ue>=.82&&y()}function b(){o||(d=!1,s.style.transition="width .2s ease, opacity .2s ease",s.style.width="0",s.style.opacity="0",setTimeout(()=>{o||(s.style.transition="")},220))}function y(){var K;if(o)return;o=!0,d=!1,s.style.width="100%",s.style.opacity="1",(K=document.getElementById("cut-flash"))==null||K.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const I=document.getElementById("cut-hint");I&&(I.style.opacity="0"),a.classList.add("pack-cut"),setTimeout(()=>{s.style.opacity="0",document.getElementById("pack-phase").style.display="none",x(0)},620)}a.addEventListener("pointerdown",l),window.addEventListener("pointermove",m),window.addEventListener("pointerup",b),a.addEventListener("touchstart",l,{passive:!0}),window.addEventListener("touchmove",m,{passive:!0}),window.addEventListener("touchend",b);let g=0,p=!1;const u=new Set;function x(I){g=I,document.getElementById("reveal-phase").style.display="flex",h(),v(I,0),S()}function h(){const I=document.getElementById("card-dots");I&&(I.innerHTML=e.map((K,J)=>`<div class="card-dot" data-i="${J}" style="width:8px;height:8px;border-radius:50%;background:${J===g?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),I.querySelectorAll(".card-dot").forEach(K=>K.addEventListener("click",()=>f(parseInt(K.dataset.i)))))}function v(I,K){var $,q;const J=e[I],ue=document.getElementById("card-counter"),fe=document.getElementById("card-track");ue&&(ue.textContent=`Carte ${I+1} / ${e.length}`);const M=document.getElementById("reveal-btns");M&&(M.style.display=I===e.length-1?"flex":"none");const V=J.card_type==="player"&&(($=J.player)==null?void 0:$.rarity)==="legende",pe=!u.has(I);u.add(I);let ce=0;if(J.card_type==="player"&&J.player){const U=J.player,R=U.job||"ATT";ce=Number(R==="GK"?U.note_g:R==="DEF"?U.note_d:R==="MIL"?U.note_m:U.note_a)||0}const N=U=>{fe.innerHTML=`
        <div id="current-card-wrap" style="position:relative;display:flex;flex-direction:column;align-items:center;gap:8px;${V?"filter:drop-shadow(0 0 20px #7a28b8)":""}">
          <div style="transform:scale(1.25);transform-origin:center">${ir(J)}</div>
        ...
        </div>`;const R=document.getElementById("current-card-wrap");K!==0?(R.style.transition="none",R.style.transform=`translateX(${K>0?100:-100}%)`,requestAnimationFrame(()=>{R.style.transition="transform .28s cubic-bezier(.25,1,.5,1)",R.style.transform="translateX(0)"})):R.animate([{opacity:0,transform:"scale(.7)"},{opacity:1,transform:"scale(1)"}],{duration:300,easing:"cubic-bezier(.34,1.56,.64,1)"}),U||V?te():ie(),h()};pe&&(((q=J.player)==null?void 0:q.rarity)==="legende"||ce>=8)&&J.card_type==="player"&&J.player?E(J,()=>N(!0)):N(!1)}function E(I,K){var Se;p=!0;const J=I.player,ue=`https://flagsapi.com/${J.country_code}/flat/64.png`,fe=(Se=J.clubs)==null?void 0:Se.logo_url,M=At(J),V=J.job||"ATT",pe=Number(V==="GK"?J.note_g:V==="DEF"?J.note_d:V==="MIL"?J.note_m:J.note_a)||0,ce=I.evolution_bonus||0,N=pe+ce,$=N>=18&&N<=20,q=N>=18,U=document.getElementById("walkout-overlay"),R=document.getElementById("walkout-stage");if(!U||!R){p=!1,K();return}let oe=null;$&&(oe=Wi("/sounds/Legendary.mp3",.8)),U.style.display="flex";const C=()=>{const ve=R.firstElementChild;ve&&(ve.classList.remove("wo-in"),ve.classList.add("wo-out"))},O=1800,le=400;R.innerHTML=`<img class="wo-in" src="${ue}" style="height:130px;border-radius:10px;box-shadow:0 10px 36px rgba(0,0,0,.6)" onerror="this.style.display='none'">`,navigator.vibrate&&navigator.vibrate(30),setTimeout(C,O),setTimeout(()=>{var ve;R.innerHTML=fe?`<img class="wo-in" src="${fe}" style="max-height:160px;max-width:210px;object-fit:contain">`:`<div class="wo-in" style="font-size:34px;font-weight:900;color:#fff;text-align:center">${((ve=J.clubs)==null?void 0:ve.encoded_name)||"CLUB"}</div>`,navigator.vibrate&&navigator.vibrate(30)},O+le),setTimeout(C,O*2+le),setTimeout(()=>{R.innerHTML=M?`<img class="wo-in" src="${M}" style="height:200px;border-radius:50%;box-shadow:0 0 40px rgba(255,255,255,0.3);object-fit:cover;object-position:top">`:'<div class="wo-in" style="font-size:80px">👤</div>',navigator.vibrate&&navigator.vibrate(30)},(O+le)*2),setTimeout(C,(O+le)*2+O);const he=q?"#FFD700":{GK:"#c0c0c0",DEF:"#e03030",MIL:"#D4A017",ATT:"#3fbf5f"}[V]||"#fff";setTimeout(()=>{R.innerHTML=`<div class="wo-in" style="
        ...
        ${N}
      </div>`,q&&navigator.vibrate&&navigator.vibrate([50,30,100,30,200])},(O+le)*3),setTimeout(C,(O+le)*3+O),setTimeout(()=>{U.style.display="none",R.innerHTML="",p=!1,oe&&!$&&oe.pause(),navigator.vibrate&&navigator.vibrate([40,30,80]),K()},(O+le)*4)}function f(I){if(p||I<0||I>=e.length||I===g)return;const K=I>g?1:-1;g=I,v(I,K)}function z(){f(g+1)}function k(){f(g-1)}function S(){const I=document.getElementById("card-viewport");if(!I||I._swipeBound)return;I._swipeBound=!0;let K=0,J=0,ue=0,fe=!1;const M=$=>$.touches?$.touches[0].clientX:$.clientX,V=$=>$.touches?$.touches[0].clientY:$.clientY,pe=$=>{fe=!0,K=M($),J=V($),ue=0},ce=$=>{if(!fe)return;ue=M($)-K;const q=V($)-J;if(Math.abs(ue)<Math.abs(q))return;const U=document.getElementById("current-card-wrap");U&&(U.style.transition="none",U.style.transform=`translateX(${ue*.6}px) rotate(${ue*.02}deg)`)},N=()=>{if(!fe)return;fe=!1;const $=document.getElementById("current-card-wrap"),q=55;ue<=-q&&g<e.length-1?z():ue>=q&&g>0?k():$&&($.style.transition="transform .2s ease",$.style.transform="translateX(0)")};I.addEventListener("pointerdown",pe),I.addEventListener("pointermove",ce),I.addEventListener("pointerup",N),I.addEventListener("pointercancel",N),I.addEventListener("touchstart",pe,{passive:!0}),I.addEventListener("touchmove",ce,{passive:!0}),I.addEventListener("touchend",N),I.addEventListener("click",$=>{if(Math.abs(ue)>8)return;const q=I.getBoundingClientRect();$.clientX-q.left>q.width/2?z():k()})}let D=null;function te(){const I=document.getElementById("fireworks-canvas");if(!I)return;I.width=window.innerWidth,I.height=window.innerHeight;const K=I.getContext("2d"),J=[];function ue(){const M=Math.random()*I.width,V=Math.random()*I.height*.6,pe=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],ce=pe[Math.floor(Math.random()*pe.length)];for(let N=0;N<60;N++){const $=Math.PI*2/60*N,q=2+Math.random()*5;J.push({x:M,y:V,vx:Math.cos($)*q,vy:Math.sin($)*q,alpha:1,color:ce,size:2+Math.random()*3})}}ue(),D=setInterval(ue,600);function fe(){if(document.getElementById("fireworks-canvas")){K.clearRect(0,0,I.width,I.height);for(let M=J.length-1;M>=0;M--){const V=J[M];if(V.x+=V.vx,V.y+=V.vy+.08,V.vy*=.98,V.alpha-=.018,V.alpha<=0){J.splice(M,1);continue}K.globalAlpha=V.alpha,K.fillStyle=V.color,K.beginPath(),K.arc(V.x,V.y,V.size,0,Math.PI*2),K.fill()}K.globalAlpha=1,(D!==null||J.length>0)&&requestAnimationFrame(fe)}}fe()}function ie(){D!==null&&(clearInterval(D),D=null);const I=document.getElementById("fireworks-canvas");I&&I.getContext("2d").clearRect(0,0,I.width,I.height)}if(n){const I=document.getElementById("reveal-btns");I&&(I.innerHTML='<button class="btn btn-primary" id="reveal-next" style="flex:1">Continuer →</button>'),(B=document.getElementById("reveal-next"))==null||B.addEventListener("click",()=>{ie(),r.remove(),n()})}else document.getElementById("reveal-collection").addEventListener("click",()=>{ie(),r.remove(),i("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{ie(),r.remove(),i("boosters")})}function ir(e){var t,i,n,r;if(e.card_type==="player"&&e.player)return Jo(e);if(e.card_type==="game_changer"){const o=e._gcDef,a=(o==null?void 0:o.name)||e.gc_type||"Game Changer",s=(o==null?void 0:o.effect)||((t=ji[e.gc_type])==null?void 0:t.desc)||"",d=o!=null&&o.image_url?`/icons/${o.image_url}`:null,c=((i=ji[e.gc_type])==null?void 0:i.icon)||"⚡";return tt(a,d,c,s,{width:170})}if(e.card_type==="formation")return zi(e.formation,zt[e.formation],{width:160});if(e.card_type==="stadium"){const o=e._stadiumDef,a=(o==null?void 0:o.name)||"Stade",s=((n=o==null?void 0:o.club)==null?void 0:n.encoded_name)||(o==null?void 0:o.country_code)||"—",d=o!=null&&o.image_url?`/icons/${o.image_url}`:((r=o==null?void 0:o.club)==null?void 0:r.logo_url)||(o!=null&&o.country_code?`https://flagsapi.com/${o.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),c=`${s}<br>+10 ⭐ joueurs alliés`;return ti(a,d,c,{width:170})}return'<div style="width:140px;height:200px;background:#333;border-radius:12px"></div>'}function nr(e){var t,i;if((t=e==null?void 0:e.rates)!=null&&t.length){const n=document.createElement("div");n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.6);display:flex;align-items:center;justify-content:center;z-index:4000;padding:16px";const r={normal:"#ccc",pepite:"#D4A017",pépite:"#D4A017",papyte:"#909090",legende:"#7a28b8",légende:"#7a28b8"},o={player:"Joueur",formation:"Formation",game_changer:"Game Changer",game_helper:"Game Helper"};n.innerHTML=`
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff;position:relative">
      ...
      <!-- Pagination -->
      ${o.length>1?`<div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">${o.map((Q,B)=>`<div style="width:6px;height:6px;border-radius:50%;background:${B===l?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}</div>`:""}
      ...
      <div id="deck-bottom-bar" style="flex-shrink:0;padding:10px 14px 14px;display:flex;flex-direction:column;gap:8px;background:rgba(0,0,0,0.2)">
        ...
        stroke="${k}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const g=typeof window<"u"&&window.innerWidth>=900?Math.min(Math.max(81,Math.round(r*.225)),117):Math.max(44,Math.round(r*.168)),p=Math.round(g*657/507);for(const[x,h]of Object.entries(l)){const v=b(x);if(!v||!h)continue;const E=x.replace(/[0-9]/g,""),f=a.includes(h.cardId),z=i==="attack"&&(["MIL","ATT"].includes(E)||f)&&!h.used||i==="defense"&&["GK","DEF","MIL"].includes(E)&&!h.used,k=n.includes(h.cardId);let S=h.boost||0,D=!1;h.stadiumBonus&&(i==="attack"&&(E==="ATT"||E==="MIL")||i==="defense"&&(E==="GK"||E==="DEF"||E==="MIL")?(S+=10,D=!0):i||(S+=10,D=!0));const te=Math.round(v.x-g/2),ie=Math.round(v.y-p/2);if(h.used){y+=`<image href="${`${typeof import.meta<"u"&&"/"||"/"}icons/carte-dos.png`}" x="${te}" y="${ie}" width="${g}" height="${p}" preserveAspectRatio="xMidYMid slice" class="match-used-hit" data-card-id="${h.cardId}" data-role="${E}" style="cursor:pointer"/>`;continue}const Q=Ie({...h,_evolution_bonus:0,stadiumBonus:!1},{width:g,showStad:!1,stadDef:null,role:E,extraNote:S,_cardOffset:30,_forceStadColor:D}),B=k?`position:absolute;top:30px;left:0;width:${g}px;height:${p}px;outline:3px solid #FFD700;outline-offset:2px;border-radius:8px;pointer-events:none;`:"";y+=`<foreignObject x="${te-2}" y="${ie-30}" width="${g+8}" height="${p+60}" style="overflow:visible">
      <div xmlns="http://www.w3.org/1999/xhtml" style="position:relative">
        ${Q}
        ${k?`<div style="${B}"></div>`:""}
      </div>
    </foreignObject>`,z&&(y+=`<rect x="${te}" y="${ie}" width="${g}" height="${p}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${k?"selected":""}" data-card-id="${h.cardId}" data-role="${E}" style="cursor:pointer"/>`)}const u=s!==null?s:Math.round(Math.max(g*.7,80));return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-u} ${-u} ${r+u*2} ${o+u*2}" width="100%" style="display:block;width:100%;margin:0 auto">
    ${y}
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const V=t.pendingAttack;let pe="";if(t.selected.length>0){const ce=t.selected.map($=>({...(t.homeTeam[$._role]||[]).find(U=>U.cardId===$.cardId)||$,_line:$._role})),N=ri(ce,t.modifiers.home,t.formation);pe=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
              ...
              <div style="display:flex;justify-content:center">${Je(ce.map($=>({...$,used:!1})),"#3a7bd5",N.total,"defense",t.formation)}</div>
            ...
            <div style="display:flex;justify-content:center">${Je((V.players||[]).map(ce=>({...ce,used:!1})),"#ff6b6b",V.total,"attack",t.formation)}</div>
            ${pe}
            ...
            <div style="display:flex;justify-content:center">${Je((V.players||[]).map(pe=>({...pe,used:!1})),"#00ff88",V.total,"attack",t.formation)}</div>
          </div>`}if(t.phase==="attack"&&t.selected.length>0){const V=t.selected.map(ce=>{const N=(t.homeTeam[ce._role]||[]).find(q=>q.cardId===ce.cardId)||ce,$=["GK","DEF"].includes(ce._role);return{...N,_line:ce._role,...$?{note_a:Math.max(1,Number(N.note_a)||0)}:{}}}),pe=oi(V,t.modifiers.home,t.formation);return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #FFD700;text-align:center">
            ...
            <div style="display:flex;justify-content:center">${Je(V.map(ce=>({...ce,used:!1})),"#FFD700",pe.total,"attack",t.formation)}</div>
    ...
    ${(()=>{const M=window.innerWidth>=700,V=(C,O,le)=>{var Ae,Be;const de=(t.gcDefs||[]).find(ke=>ke.name===C.gc_type),he=(de==null?void 0:de.name)||C.gc_type,Se=(de==null?void 0:de.effect)||((Ae=je[C.gc_type])==null?void 0:Ae.desc)||"",ve=de!=null&&de.image_url?`/icons/${de.image_url}`:null,Te=((Be=je[C.gc_type])==null?void 0:Be.icon)||"⚡";return`<div class="gc-mini" data-gc-id="${C.id}" data-gc-type="${C.gc_type}" style="flex-shrink:0;cursor:pointer">
          ${tt(he,ve,Te,Se,{width:O})}
        </div>`},pe=(C,O)=>{var le;return`<div id="boost-card"
          style="box-sizing:border-box;width:${C}px;height:${O}px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:2px solid #87CEEB;border-radius:10px;cursor:pointer;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:${Math.round(O*.04)}px;text-align:center;flex-shrink:0">
          ...
          </div>`},ce=(C,O)=>O?pe(95,162):V(C,95),N=(C,O)=>O?pe(68,116):V(C,68),$=M?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",q=h?`<button id="btn-results" style="${$};background:linear-gradient(135deg,#D4A017,#FFD700);border:none;color:#000">🏁 Résultats</button>`:p?`<div style="${$};background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);color:rgba(255,255,255,0.4)">⏳ Tour IA</div>`:f?`<button id="btn-pass" style="${$};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER (plus d'attaquants)</button>`:u?`<button id="btn-action" style="${$};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${t.selected.length===0?"disabled":""}> ⚔️ ATTAQUEZ <span id="match-timer" style="font-weight:900"></span></button>`:x?`<button id="btn-action" style="${$};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${t.selected.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="match-timer" style="font-weight:900"></span></button>`:`<div style="${$};background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1)"></div>`,U=u||x?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${t.selected.length}/3 sélectionné(s)</div>`:"",R=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${M?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
        ${o.length===0?'<div style="font-size:7px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':o.map(C=>`<div class="sub-btn-col" data-sub-id="${C.cardId}" style="cursor:pointer;flex-shrink:0">${ht(C,76,100)}</div>`).join("")}
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(M=>{var V,pe,ce;if(M.type==="duel"){const N=M.isGoal,$=M.homeScored?"#FFD700":N?"#ff6b6b":"rgba(255,255,255,0.3)",q=M.homeScored?"⚽ BUT !":N?"⚽ BUT IA !":(V=M.homePlayers)!=null&&V.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${N?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${$};margin-bottom:4px">
                ...
                ${(ce=M.aiPlayers)!=null&&ce.length?`<div style="opacity:0.7">${Je(M.aiPlayers,"#ff6b6b",M.aiTotal,void 0,t.formation)}</div>`:""}
    ...
    </div>`,(f=m.querySelector("#sub-close"))==null||f.addEventListener("click",()=>m.remove()),(z=m.querySelector("#out-up"))==null||z.addEventListener("click",()=>{s>0&&(s--,b())}),(k=m.querySelector("#out-down"))==null||k.addEventListener("click",()=>{s<o.length-1&&(s++,b())}),(S=m.querySelector("#in-up"))==null||S.addEventListener("click",()=>{c>0&&(c--,b())}),(D=m.querySelector("#in-down"))==null||D.addEventListener("click",()=>{c<a.length-1&&(c++,b())});const E=(ie,Q,B,I)=>{const K=m.querySelector("#"+ie);if(!K)return;let J=0;K.addEventListener("touchstart",ue=>{J=ue.touches[0].clientY},{passive:!0}),K.addEventListener("touchend",ue=>{const fe=ue.changedTouches[0].clientY-J;if(Math.abs(fe)<30)return;const M=Q();fe<0&&M<I-1?(B(M+1),b()):fe>0&&M>0&&(B(M-1),b())},{passive:!0})};E("in-panel",()=>c,ie=>c=ie,a.length),E("out-panel",()=>s,ie=>s=ie,o.length),(te=m.querySelector("#sub-confirm"))==null||te.addEventListener("click",ie=>{if(ie.preventDefault(),ie.stopPropagation(),l)return;l=!0;const Q=o[s],B=a[c];if(!Q||!B)return;let I=null,K=-1;for(const[ue,fe]of Object.entries(t.homeTeam)){const M=(fe||[]).findIndex(V=>V.cardId===Q.cardId);if(M!==-1){I=ue,K=M;break}}if(K===-1||!I){$t("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),m.remove();return}const J={...B,_line:I,_col:Q._col||0,used:!1,boost:0};t.homeTeam[I].splice(K,1,J),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(B.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:Q.name,firstname:Q.firstname,note:Pe(Q,I),portrait:At(Q),job:Q.job,country_code:Q.country_code,rarity:Q.rarity,clubName:Q.clubName,clubLogo:Q.clubLogo},inPlayer:{name:B.name,firstname:B.firstname,note:Pe(B,I),portrait:At(B),job:B.job,country_code:B.country_code,rarity:B.rarity,clubName:B.clubName,clubLogo:B.clubLogo},text:`🔄 ${B.firstname} ${B.name} remplace ${Q.firstname} ${Q.name}`}),m.remove(),ur(Q,B,()=>we(e,t,i))})}document.body.appendChild(m),b()}function $r(e,t,i,n,r){var b,y;const o=(n.gcDefs||[]).find(g=>g.name===t),a=je[t]||{icon:"⚡",desc:"Carte spéciale."},s=(o==null?void 0:o.name)||t,d=(o==null?void 0:o.effect)||a.desc,c=o!=null&&o.image_url?`/icons/${o.image_url}`:null,l=a.icon||"⚡",m=document.createElement("div");m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",m.innerHTML=`
    ...
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      ...
      ${_i(e.aiTeam,e.formation,null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.1))}
            ...
            <div style="background:rgba(255,255,255,0.08);border-radius:14px;padding:14px 20px;text-align:center;min-width:220px">
              ...
              ${Le.id!==nt.id?`<div style="font-size:20px;font-weight:900;color:${Le.color}">
                    ${Ne>X.mmr?"📈":"📉"} ${nt.emoji} ${nt.label} → ${Le.emoji} ${Le.label}
                  </div>
                  <div style="font-size:13px;color:rgba(255,255,255,0.5);margin-top:4px">${Ne>X.mmr?"Promotion !":"Rétrogradation"}</div>`:`<div style="font-size:36px">${Ee>=0?"📈":"📉"}</div>
                   <div style="font-size:18px;font-weight:900;color:${Le.color}">${Le.emoji} ${Le.label}</div>`}
              ${se?`<div style="font-size:11px;color:rgba(255,255,255,0.4);margin-top:6px">Match de placement (${X.placement_matches+1}/10)</div>`:""}
            </div>`}}catch(X){console.error("[Ranked] MMR update error:",X)}(ee=document.getElementById("pvp-end-overlay"))==null||ee.remove();const F=document.createElement("div");F.id="pvp-end-overlay",F.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;color:#fff;padding:24px;text-align:center;overflow-y:auto";const P=T?"🤝":j?"🏆":"😞",H=T?"MATCH NUL":j?"VICTOIRE !":"DÉFAITE",W=T?"#fff":j?"#FFD700":"#ff6b6b";F.innerHTML=`
      ...
      ${G}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${l?"🏆 Retour à la Mini League":a?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(F),(Z=F.querySelector("#pvp-end-home"))==null||Z.addEventListener("click",()=>{F.remove(),it(e),l?y("mini-league",{openLeagueId:l}):y(a?"ranked":"home")})}const Q=_.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},w=>{var A;const L=w.new;try{if(L.status==="finished"||L.forfeit){if(z)return;z=!0,k&&(clearInterval(k),k=null),Ue(),L.game_state&&(f=L.game_state),ie(L);return}if(L.game_state){const j=f;f=L.game_state;const T=f._lastGC;if(T&&T.seq&&!te.has(T.seq)&&(te.add(T.seq),T.by!==p)){$(T.type,T.by,()=>V());return}const G=j[p+"Score"]||0,F=j[u+"Score"]||0,P=f[p+"Score"]||0,H=f[u+"Score"]||0,W=P>G,ee=H>F;if((W||ee)&&!D.has(f.round)){D.add(f.round);const Z=[...f.log||[]].reverse().find(ae=>ae.isGoal),X=((Z==null?void 0:Z.homePlayers)||[]).map(ae=>({name:ae.name,note:ae.note,portrait:ae.portrait,job:ae.job}));!!((A=Z==null?void 0:Z.text)!=null&&A.includes("DERNIER CORNER"))?Se().then(()=>{Qe(X,P,H,W,()=>V())}):Qe(X,P,H,W,()=>V());return}V()}}catch(j){console.error("[PvP] crash:",j)}}).subscribe();async function B(w){Object.assign(f,w),f.lastActionAt=new Date().toISOString();const{error:L}=await _.from("matches").update({game_state:f}).eq("id",i);L&&g("Erreur de synchronisation","error");try{V()}catch(A){console.error("[PvP] renderPvpScreen crash:",A)}}async function I(){if(z)return;z=!0,Gt(),Ue(),k&&(clearInterval(k),k=null);const w=n?v.away_id:v.home_id,L=n?"p2":"p1",A=n?"p1":"p2",j={...f,[L+"Score"]:3,[A+"Score"]:0,phase:"finished"},T=j.p1Score||0,G=j.p2Score||0;try{await _.from("matches").update({game_state:j}).eq("id",i);const{data:F,error:P}=await _.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:w,p_home_score:T,p_away_score:G,p_forfeit:!0});if(P&&console.error("[PvP] finish_pvp_match (forfeit):",P),l)try{await _.rpc("check_and_finish_mini_league",{p_league_id:l})}catch(H){console.error("[PvP] check_and_finish_mini_league (forfeit):",H)}try{const{data:H}=await _.rpc("apply_match_rewards",{p_match_id:i});H!=null&&H.success&&!(H!=null&&H.skipped)&&typeof t.refreshProfile=="function"&&await t.refreshProfile()}catch(H){console.error("[PvP] apply_match_rewards (forfeit):",H)}}catch{}try{_.removeChannel(Q)}catch{}setTimeout(()=>{it(e),l?y("mini-league",{openLeagueId:l}):y("home")},800)}const K={BOOST_STAT:({value:w=1,count:L=1,roles:A=[]},j,T)=>{const G=j[p+"Team"],F=Object.entries(G).filter(([P])=>!A.length||A.includes(P)).flatMap(([P,H])=>H.filter(W=>!W.used).map(W=>({...W,_line:P})));if(!F.length){j.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),T(j);return}J(F,L,`Choisir ${L} joueur(s) à booster (+${w})`,P=>{P.forEach(H=>{const W=(G[H._line]||[]).find(ee=>ee.cardId===H.cardId);W&&(W.boost=(W.boost||0)+w,j.log.push({text:`⚡ +${w} sur ${W.name}`,type:"info"}))}),j[p+"Team"]=G,T(j)})},DEBUFF_STAT:({value:w=1,count:L=1,roles:A=[],target:j="ai"},T,G)=>{const F=j==="home"?p:u,P=T[F+"Team"],H=j==="home"?"allié":"adverse",W=Object.entries(P).filter(([ee])=>!A.length||A.includes(ee)).flatMap(([ee,Z])=>Z.map(X=>({...X,_line:ee})));if(!W.length){T.log.push({text:`🎯 Aucun joueur ${H}`,type:"info"}),G(T);return}J(W,L,`Choisir ${L} joueur(s) ${H}(s) (-${w})`,ee=>{ee.forEach(Z=>{const X=(P[Z._line]||[]).find(re=>re.cardId===Z.cardId);X&&(X.boost=(X.boost||0)-w,T.log.push({text:`🎯 -${w} sur ${X.name}`,type:"info"}))}),T[F+"Team"]=P,G(T)})},GRAY_PLAYER:({count:w=1,roles:L=[],target:A="ai"},j,T)=>{const G=A==="home"?p:u,F=j[G+"Team"],P=A==="home"?"allié":"adverse",H=Object.entries(F).filter(([W])=>!L.length||L.includes(W)).flatMap(([W,ee])=>ee.filter(Z=>!Z.used).map(Z=>({...Z,_line:W})));if(!H.length){j.log.push({text:`❌ Aucun joueur ${P}`,type:"info"}),T(j);return}J(H,w,`Choisir ${w} joueur(s) ${P}(s) à exclure`,W=>{const ee="usedCardIds_"+G,Z=new Set(j[ee]||[]);W.forEach(X=>{const re=(F[X._line]||[]).find(ae=>ae.cardId===X.cardId);re&&(re.used=!0,Z.add(X.cardId),j.log.push({text:`❌ ${re.name} exclu !`,type:"info"}))}),j[ee]=[...Z],j[G+"Team"]=F,T(j)})},REVIVE_PLAYER:({count:w=1,roles:L=[]},A,j)=>{const T=A[p+"Team"],G=Object.entries(T).filter(([F])=>!L.length||L.includes(F)).flatMap(([F,P])=>P.filter(H=>H.used).map(H=>({...H,_line:F})));if(!G.length){A.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),j(A);return}J(G,w,`Choisir ${w} joueur(s) à ressusciter`,F=>{F.forEach(P=>{const H=(T[P._line]||[]).find(W=>W.cardId===P.cardId);H&&(H.used=!1,A.log.push({text:`💫 ${H.name} ressuscité !`,type:"info"}))}),A[p+"Team"]=T,j(A)})},REMOVE_GOAL:({},w,L)=>{const A=u+"Score";w[A]>0?(w[A]--,w.log.push({text:"🚫 Dernier but annulé !",type:"info"})):w.log.push({text:"🚫 Aucun but à annuler",type:"info"}),L(w)},ADD_GOAL_DRAW:({},w,L)=>{w[p+"Score"]===w[u+"Score"]?(w[p+"Score"]++,w.log.push({text:"🎯 But bonus !",type:"info"})):w.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),L(w)},ADD_SUB:({value:w=1},L,A)=>{L.maxSubs=(L.maxSubs||3)+w,L.log.push({text:`🔄 +${w} remplacement(s)`,type:"info"}),A(L)},CUSTOM:({},w,L)=>L(w)};function J(w,L,A,j){const T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let G=[];function F(){var H,W;const P=w.map(ee=>{const Z=ee._line||ee.job||"MIL",X=G.find(ae=>ae.cardId===ee.cardId),re=Ie({...ee,_evolution_bonus:0},{width:90,showStad:!0,role:Z,extraNote:ee.boost||0});return`<div class="pp-item" data-cid="${ee.cardId}"
          style="position:relative;border-radius:8px;${X?"outline:3px solid #FFD700;outline-offset:2px;":""}cursor:pointer;flex-shrink:0;${ee.used?"opacity:0.3;pointer-events:none":""}">
          ${re}
        ...
        </div>`,(H=T.querySelector("#pp-close"))==null||H.addEventListener("click",()=>T.remove()),T.querySelectorAll(".pp-item").forEach(ee=>{ee.addEventListener("click",()=>{const Z=ee.dataset.cid,X=w.find(ae=>ae.cardId===Z),re=G.findIndex(ae=>ae.cardId===Z);X&&(re>-1?G.splice(re,1):G.length<L&&G.push(X),F())})}),(W=T.querySelector("#pp-confirm"))==null||W.addEventListener("click",()=>{T.remove(),j(G)})}F(),document.body.appendChild(T)}async function ue(w,L){const j=(f["gcCardsFull_"+p]||[]).find(P=>P.id===w),T=(j==null?void 0:j._gcDef)||(f.gcDefs||[]).find(P=>{var H;return P.name===L||((H=P.name)==null?void 0:H.toLowerCase().trim())===(L==null?void 0:L.toLowerCase().trim())}),G=[...f["usedGc_"+p]||[],w],F={type:L,by:p,seq:(f._gcAnimSeq||0)+1};te.add(F.seq),$(L,p,async()=>{if(T!=null&&T.effect_type&&T.effect_type!=="CUSTOM"){const H=K[T.effect_type];if(H){const W={...f};H(T.effect_params||{},W,async ee=>{ee["usedGc_"+p]=G,ee._lastGC=F,ee._gcAnimSeq=F.seq,await B(ee)});return}}const P={...f};switch(L){case"Remplacement+":P.maxSubs=(P.maxSubs||3)+1,P.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const H=u+"Score";P[H]>0&&(P[H]--,P.log.push({text:"🚫 But annulé",type:"info"}));break}}P["usedGc_"+p]=G,P._lastGC=F,P._gcAnimSeq=F.seq,await B(P)})}function fe(w,L){const A="usedCardIds_"+w,j=new Set(f[A]||[]);L.forEach(T=>j.add(T)),f[A]=[...j]}function M(){for(const w of["p1","p2"]){const L=new Set(f["usedCardIds_"+w]||[]),A=f[w+"Team"];if(A)for(const j of["GK","DEF","MIL","ATT"])(A[j]||[]).forEach(T=>{T.used=L.has(T.cardId)})}}function V(){var nn,on,rn,an,sn,dn;if(f.phase==="reveal")return pe();if(f.phase==="midfield")return N();if(f.phase==="finished")return Ge();const w=f[p+"Team"],L=f[u+"Team"];M();const A=f[p+"Score"],j=f[u+"Score"],T=f[p+"Name"],G=f[u+"Name"],F=f.phase===p+"-attack",P=f.phase===p+"-defense",H=Array.isArray(f["selected_"+p])?f["selected_"+p]:[],W=H.map(Y=>Y.cardId),ee=window.innerWidth>=700,Z=f[p+"Subs"]||[],X=f["usedSubIds_"+p]||[],re=Z.filter(Y=>!X.includes(Y.cardId)),ae=f["gcCardsFull_"+p]||[],ge=f["usedGc_"+p]||[],se=ae.filter(Y=>!ge.includes(Y.id)),ye=f.boostOwner===p&&!f.boostUsed,be=[...w.MIL||[],...w.ATT||[]].filter(Y=>!Y.used),ne=[...L.MIL||[],...L.ATT||[]].filter(Y=>!Y.used),Ee=!ke(L),Ne=be.length===0&&ne.length===0&&!Ee,Le=(w.DEF||[]).filter(Y=>!Y.used),Ve=(w.GK||[]).filter(Y=>!Y.used);let Oe=[];F&&be.length===0&&(Ee?(Oe=Le.map(Y=>Y.cardId),Le.length===0&&(Oe=Oe.concat(Ve.map(Y=>Y.cardId)))):Ne&&(Oe=[...Le,...Ve].map(Y=>Y.cardId)));function nt(Y,me,ze){var Me,ft;const _e=Y._gcDef,Bt=(_e==null?void 0:_e.name)||Y.gc_type,at=(_e==null?void 0:_e.effect)||((Me=je[Y.gc_type])==null?void 0:Me.desc)||"",Fe=_e!=null&&_e.image_url?`/icons/${_e.image_url}`:null,Re=((ft=je[Y.gc_type])==null?void 0:ft.icon)||"⚡";return`<div class="pvp-gc-mini" data-gc-id="${Y.id}" data-gc-type="${Y.gc_type}" style="flex-shrink:0;cursor:pointer">
        ${tt(Bt,Fe,Re,at,{width:me})}
      </div>`}function Kt(Y,me){return`<div id="pvp-boost-card"
        style="box-sizing:border-box;width:${Y}px;height:${me}px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:2px solid #87CEEB;border-radius:10px;cursor:pointer;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:${Math.round(me*.04)}px;text-align:center;flex-shrink:0">
      ...
      </div>`}const De=(Y,me)=>me?Kt(95,162):nt(Y,95),Ye=(Y,me)=>me?Kt(68,116):nt(Y,68),He=ee?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",pt=F?ct(p)?`<button id="pvp-action" style="${He};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${H.length===0?"disabled":""}>⚔️ ATTAQUEZ <span id="pvp-timer"></span></button>`:`<button id="pvp-action" data-pass="1" style="${He};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER <span id="pvp-timer"></span></button>`:P?`<button id="pvp-action" style="${He};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${H.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="pvp-timer"></span></button>`:`<div style="font-size:11px;color:rgba(255,255,255,0.3);text-align:center;padding:4px">⏳ Tour de ${G}</div>`,ot=F&&!ct(p)?'<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">Aucun attaquant — passez la main</div>':F||P?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${H.length}/3 sélectionné(s)</div>`:"",Ct=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${ee?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
      ${re.length===0?'<div style="font-size:7px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':re.map(Y=>`<div class="pvp-sub-btn" data-sub-id="${Y.cardId}" style="cursor:pointer;flex-shrink:0">${ht(Y,ee?76:44,ee?100:58)}</div>`).join("")}
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${Ot(w,f[p+"Formation"],ut,W,ee?1300:It(),ee?600:Nt(),Oe)}
      </div>
    </div>`,rt=f[p+"Team"],ro=(()=>{var me,ze,_e,Bt,at;if(P&&((me=f.pendingAttack)!=null&&me.players)){const Fe=f.pendingAttack;let Re="";if(H.length>0){const Me=H.map(We=>{const Vt=(rt[We._role]||[]).find(Ei=>Ei.cardId===We.cardId)||We,Yt=rt[We._role]||[],si=Yt.findIndex(Ei=>Ei.cardId===We.cardId),$i=yt(Yt.length),Wt=si>=0?$i[si]:Vt._col??1;return{...Vt,_line:We._role,_col:Wt}}),ft=ri(Me,((ze=f.modifiers)==null?void 0:ze[p])||{},f[p+"Formation"]);Re=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
        ...
        </div>`}const Y=(f.log||[]).slice(-1)[0];return Y?'<div style="padding:2px 4px">'+Yn(Y)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3);text-align:center">⏳ Match en cours...</div>'})(),tn=`
      <div style="display:flex;align-items:center;padding:8px 10px;background:rgba(0,0,0,0.5);gap:6px;flex-shrink:0">
      ...
      </button>`;st(e),e.style.overflow="hidden",ee?e.innerHTML=`
      <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
        ${tn}
        <div style="display:flex;flex:1;min-height:0;overflow:hidden">
          ${Ct}
      ...
      <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
        ...
        <div id="mobile-play-area" style="flex:1;min-height:0;display:flex;overflow:hidden">
          <div id="match-field" style="flex:1;min-width:0;min-height:0;overflow:hidden">
            <div class="terrain-wrapper" style="width:100%;height:100%;overflow:hidden">
              ${Ot(w,f[p+"Formation"],ut,W,It(),Nt(),Oe)}
            </div>
          </div>
        </div>
        <div id="mobile-action-bar" style="position:absolute;left:0;right:0;bottom:0;z-index:20;background:rgba(0,0,0,0.55);padding:6px 8px 8px;display:flex;flex-direction:column;gap:6px;box-shadow:0 -4px 16px rgba(0,0,0,0.5)">
          <div style="display:flex;gap:6px;overflow-x:auto;align-items:flex-end;min-height:96px;padding-bottom:2px">
            ${se.map(Y=>Ye(Y,!1)).join("")}
            ${ye?Ye(null,!0):""}
      ...
      </div>`;function jt(){const Y=e.querySelector(".match-screen");if(!Y)return;const me=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);Y.style.height=me+"px",Y.style.minHeight=me+"px",Y.style.maxHeight=me+"px",Y.style.overflow="hidden";const ze=e.querySelector("#mobile-action-bar"),_e=e.querySelector("#mobile-play-area");ze&&_e&&(_e.style.paddingBottom=ze.offsetHeight+"px")}if(jt(),setTimeout(jt,120),setTimeout(jt,400),S||(S=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",jt),window.visualViewport.addEventListener("scroll",jt)),window.addEventListener("resize",jt)),function(){const me=e.querySelector("#match-field .terrain-wrapper svg")||e.querySelector(".terrain-wrapper svg");if(!me)return;const ze=me.closest("#match-terrain-wrap");ze&&(ze.style.cssText="position:relative;width:100%;height:100%;padding:0"),me.removeAttribute("width"),me.removeAttribute("height"),me.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",me.setAttribute("preserveAspectRatio","xMidYMid meet")}(),f._pvpResizeBound||(f._pvpResizeBound=!0,window.addEventListener("resize",()=>{const Y=e.querySelector(".terrain-wrapper svg");if(Y){const me=Y.closest("#match-terrain-wrap");me&&(me.style.cssText="position:relative;width:100%;height:100%;padding:0"),Y.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0"}})),e.querySelectorAll(".match-slot-hit").forEach(Y=>{Y.addEventListener("click",()=>{if(!F&&!P)return;const me=Y.dataset.cardId,ze=Y.dataset.role,_e=(w[ze]||[]).find(Re=>Re.cardId===me);if(!_e||_e.used)return;const Bt=Oe.includes(me);if(F&&!["MIL","ATT"].includes(ze)&&!Bt)return;Array.isArray(f["selected_"+p])||(f["selected_"+p]=[]);const at=f["selected_"+p],Fe=at.findIndex(Re=>Re.cardId===me);Fe>-1?at.splice(Fe,1):at.length<3&&at.push({..._e,_role:ze}),V()})}),e.querySelectorAll(".match-used-hit").forEach(Y=>{Y.addEventListener("click",()=>R(Y.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(Y=>{Y.addEventListener("click",()=>R())}),(nn=e.querySelector("#pvp-sub-open"))==null||nn.addEventListener("click",()=>R()),e.querySelectorAll(".pvp-gc-mini").forEach(Y=>{F?Y.addEventListener("click",()=>q(Y.dataset.gcId,Y.dataset.gcType)):(Y.style.opacity="0.35",Y.style.cursor="default",Y.addEventListener("click",()=>$t("⚡ Les Game Changers ne sont utilisables qu'en attaque","rgba(180,100,0,0.9)")))}),(on=e.querySelector("#pvp-boost-card"))==null||on.addEventListener("click",()=>U()),(rn=e.querySelector("#pvp-action"))==null||rn.addEventListener("click",Y=>{F?Y.currentTarget.dataset.pass==="1"||!ct(p)?le():Te():P&&Ae()}),(an=e.querySelector("#pvp-quit"))==null||an.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&I()}),(sn=e.querySelector("#pvp-view-opp"))==null||sn.addEventListener("click",()=>C()),(dn=e.querySelector("#pvp-toggle-history"))==null||dn.addEventListener("click",()=>O()),k&&(clearInterval(k),k=null),Ue(),(F||P)&&!z){let Y=30,me=!1;const ze=()=>document.getElementById("pvp-timer"),_e=()=>{ze()&&(ze().textContent=Y+"s",ze().style.color=me?"#ff4444":"#fff")};_e(),k=setInterval(()=>{Y--,Y<0?me?(clearInterval(k),k=null,Ue(),F&&!ct(p)?le():I()):(me=!0,Y=15,_e(),zn("/sounds/timer-urgent.mp3",.6)):_e()},1e3)}}function pe(){st(e),e.innerHTML=`
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    </div>`;const G=(re,ae)=>e.querySelectorAll(re).forEach((ge,se)=>{setTimeout(()=>{ge.style.opacity="1",ge.style.transform="translateY(0) scale(1)"},ae+se*90)});G(".duel-card-me",150),G(".duel-card-opp",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((re,ae)=>setTimeout(()=>{re.style.opacity="1"},ae*70)),900),setTimeout(()=>{const re=e.querySelector("#pvp-vs");re&&(re.style.opacity="1",re.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(ae=>ae.style.opacity="1")},1250);function F(re,ae,ge){const se=document.getElementById(re);if(!se)return;const ye=performance.now(),be=ne=>{const Ee=Math.min(1,(ne-ye)/ge);se.textContent=Math.round(ae*(1-Math.pow(1-Ee,3))),Ee<1?requestAnimationFrame(be):se.textContent=ae};requestAnimationFrame(be)}setTimeout(()=>{F("pvp-score-me",A,800),F("pvp-score-opp",j,800)},1500);const P=f.p1Team.MIL||[],H=f.p2Team.MIL||[],W=xt(P)+bt(P),ee=xt(H)+bt(H),Z=W>=ee?"p1":"p2";let X=f.boostValue;X==null&&(X=Kn(),f.boostValue=X,f.boostOwner=Z,f.boostUsed=!1),ce=!0,setTimeout(()=>{const re=e.querySelector("#duel-row-"+(T?"me":"opp")),ae=e.querySelector("#duel-row-"+(T?"opp":"me")),ge=document.getElementById("pvp-score-me"),se=document.getElementById("pvp-score-opp"),ye=T?ge:se,be=T?se:ge;ye&&(ye.style.fontSize="80px",ye.style.color=T?"#FFD700":"#ff6b6b",ye.style.animation="duelPulse .5s ease"+(T?",duelGlow 1.5s ease infinite .5s":"")),be&&(be.style.opacity="0.25"),setTimeout(()=>{re&&(re.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",re.style.zIndex="5"),setTimeout(()=>{const ne=document.getElementById("duel-shock");ne&&(ne.style.animation="shockwave .5s ease-out forwards"),ae&&(ae.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var Le;const ne=document.getElementById("pvp-duel-finale");if(!ne)return;const Ee=f.boostOwner===p?'<div style="background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:3px solid #cdeffd;border-radius:18px;padding:20px 34px;text-align:center;animation:boostFlipIn .7s cubic-bezier(.34,1.56,.64,1) both;box-shadow:0 10px 36px rgba(135,206,235,.5)"><div style="font-size:10px;color:rgba(0,0,0,.6);letter-spacing:2px;text-transform:uppercase;margin-bottom:6px;font-weight:700">Carte Boost obtenue</div><div style="font-size:46px;line-height:1">⚡</div><div style="font-size:50px;font-weight:900;color:#063;line-height:1.1">+'+X+`</div><div style="font-size:10px;color:rgba(0,0,0,.55);margin-top:4px">Applicable sur n'importe quel joueur</div></div>`:"",Ne=p==="p1"?'<button id="pvp-start-match" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">▶ Commencer le match</button>':`<div style="font-size:14px;color:rgba(255,255,255,0.5);text-align:center;margin-top:8px;animation:fadeUp .4s ease both">⏳ En attente de l'adversaire...</div>`;ne.innerHTML='<div style="font-size:22px;font-weight:900;color:#fff;text-align:center;animation:fadeUp .4s ease both;text-shadow:0 2px 12px rgba(0,0,0,.5)">'+(T?"⚽ "+f[p+"Name"]+"<br>gagne le milieu et attaque !":"😔 "+f[u+"Name"]+"<br>gagne l'engagement et attaque !")+"</div>"+Ee+Ne,ne.style.transition="opacity .45s ease",ne.style.opacity="1",ne.style.pointerEvents="auto",(Le=document.getElementById("pvp-start-match"))==null||Le.addEventListener("click",async()=>{const Ve=Z;await B({phase:Ve+"-attack",attacker:Ve,round:1,boostValue:X,boostUsed:!1,boostOwner:Ve})})},600)},700)},2800)}function $(w,L,A){var ae,ge;const j=(f.gcDefs||[]).find(se=>{var ye;return se.name===w||((ye=se.name)==null?void 0:ye.toLowerCase().trim())===(w==null?void 0:w.toLowerCase().trim())}),T={purple:"#b06ce0",light_blue:"#00d4ef"}[j==null?void 0:j.color]||"#b06ce0",G=(j==null?void 0:j.name)||w,F=(j==null?void 0:j.effect)||((ae=je[w])==null?void 0:ae.desc)||"",P=j!=null&&j.image_url?`/icons/${j.image_url}`:null,H=((ge=je[w])==null?void 0:ge.icon)||"⚡",ee=L===p?"Vous":f[L+"Name"]||"Adversaire",Z=document.createElement("div");Z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",Z.innerHTML=`
      <style>
      ...
      <div style="font-size:11px;color:${T};letter-spacing:3px;text-transform:uppercase;font-weight:700;animation:gcLabel .4s ease both">${ee} joue une carte</div>
      <div style="animation:gcFlipIn .7s cubic-bezier(.34,1.56,.64,1) both,gcGlow 1.8s ease infinite .7s">
        ${tt(G,P,H,F,{width:200})}
      </div>
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(Z);let X=!1;const re=()=>{X||(X=!0,Z.remove(),setTimeout(()=>A&&A(),50))};Z.addEventListener("click",re),setTimeout(re,3e3)}function q(w,L){var ee,Z,X,re;const j=(f["gcCardsFull_"+p]||[]).find(ae=>ae.id===w),T=j==null?void 0:j._gcDef,G=(T==null?void 0:T.name)||L,F=(T==null?void 0:T.effect)||((ee=je[L])==null?void 0:ee.desc)||"Carte spéciale.",P=T!=null&&T.image_url?`/icons/${T.image_url}`:null,H=((Z=je[L])==null?void 0:Z.icon)||"⚡",W=document.createElement("div");W.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",W.innerHTML=`
      ${tt(G,P,H,F,{width:190})}
      <div style="display:flex;gap:12px;width:190px">
        <button id="pvp-gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
        <button id="pvp-gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
      </div>`,document.body.appendChild(W),(X=W.querySelector("#pvp-gc-back"))==null||X.addEventListener("click",()=>W.remove()),(re=W.querySelector("#pvp-gc-use"))==null||re.addEventListener("click",()=>{W.remove(),ue(w,L)})}function U(){var j;const w=f[p+"Team"],L=Object.entries(w).flatMap(([T,G])=>(G||[]).filter(F=>!F.used).map(F=>({...F,_line:T})));if(!L.length)return;const A=document.createElement("div");A.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",A.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
      ...
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${L.map(T=>{const G={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[T._line]||"#555",F=Pe(T,T._line)+(T.boost||0);return`<div class="bp-item" data-cid="${T.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${G};overflow:hidden;cursor:pointer">
      ...
      </div>`,document.body.appendChild(A),(j=A.querySelector("#bp-close"))==null||j.addEventListener("click",()=>A.remove()),A.querySelectorAll(".bp-item").forEach(T=>{T.addEventListener("click",async()=>{const G=T.dataset.cid,F=L.find(H=>H.cardId===G);if(!F)return;const P=(w[F._line]||[]).find(H=>H.cardId===G);P&&(P.boost=(P.boost||0)+f.boostValue),A.remove(),await B({[p+"Team"]:w,boostUsed:!0})})})}function R(w=null){var re,ae;if(!(f.phase===p+"-attack")){g("Remplacement uniquement avant votre attaque","warning");return}const A=f[p+"Team"],j=f["usedSubIds_"+p]||[],T=f.maxSubs||3;if(j.length>=T){g(`Maximum ${T} remplacements atteint`,"warning");return}const G=Object.entries(A).flatMap(([ge,se])=>(se||[]).filter(ye=>ye.used).map(ye=>({...ye,_line:ge}))),F=(f[p+"Subs"]||[]).filter(ge=>!j.includes(ge.cardId));if(!G.length){g("Aucun joueur utilisé à remplacer","warning");return}if(!F.length){g("Aucun remplaçant disponible","warning");return}let P=Math.max(0,G.findIndex(ge=>ge.cardId===w));const H=((re=G[P])==null?void 0:re._line)||((ae=G[P])==null?void 0:ae.job);let W=Math.max(0,F.findIndex(ge=>ge.job===H)),ee=!1;const Z=document.createElement("div");Z.id="pvp-sub-overlay",Z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function X(){var Ne,Le,Ve,Oe,nt,Kt;const ge=G[P],se=F[W],ye=Math.min(130,Math.round((window.innerWidth-90)/2)),be=Math.round(ye*1.35),ne=De=>`background:rgba(255,255,255,0.12);border:none;color:${De?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${De?"default":"pointer"};flex-shrink:0`;Z.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        <div id="pin-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
          <div style="font-size:9px;color:#00ff88;letter-spacing:2px;text-transform:uppercase;font-weight:700">Joueur qui entre</div>
          <button id="pin-up" style="${ne(W===0)}" ${W===0?"disabled":""}>▲</button>
          <div>${se?ht({...se,used:!1,boost:0},ye,be):"<div>—</div>"}</div>
          <button id="pin-down" style="${ne(W>=F.length-1)}" ${W>=F.length-1?"disabled":""}>▼</button>
          <div style="font-size:10px;color:rgba(255,255,255,0.35)">${W+1}/${F.length}</div>
        </div>
      ...
      </div>`,(Ne=Z.querySelector("#psub-close"))==null||Ne.addEventListener("click",()=>Z.remove()),(Le=Z.querySelector("#pout-up"))==null||Le.addEventListener("click",()=>{P>0&&(P--,X())}),(Ve=Z.querySelector("#pout-down"))==null||Ve.addEventListener("click",()=>{P<G.length-1&&(P++,X())}),(Oe=Z.querySelector("#pin-up"))==null||Oe.addEventListener("click",()=>{W>0&&(W--,X())}),(nt=Z.querySelector("#pin-down"))==null||nt.addEventListener("click",()=>{W<F.length-1&&(W++,X())});const Ee=(De,Ye,He,pt)=>{const ot=Z.querySelector("#"+De);if(!ot)return;let Ct=0;ot.addEventListener("touchstart",ut=>{Ct=ut.touches[0].clientY},{passive:!0}),ot.addEventListener("touchend",ut=>{const kt=ut.changedTouches[0].clientY-Ct;if(Math.abs(kt)<30)return;const rt=Ye();kt<0&&rt<pt-1?(He(rt+1),X()):kt>0&&rt>0&&(He(rt-1),X())},{passive:!0})};Ee("pin-panel",()=>W,De=>W=De,F.length),Ee("pout-panel",()=>P,De=>P=De,G.length),(Kt=Z.querySelector("#psub-confirm"))==null||Kt.addEventListener("click",async De=>{if(De.preventDefault(),De.stopPropagation(),ee)return;ee=!0;const Ye=G[P],He=F[W];if(!Ye||!He)return;const pt=Ye._line,ot=(A[pt]||[]).findIndex(kt=>kt.cardId===Ye.cardId);if(ot===-1){g("Erreur : joueur introuvable","error"),Z.remove();return}const Ct={...He,_line:pt,position:Ye.position,used:!1,boost:0};A[pt].splice(ot,1,Ct);const ut=[...j,He.cardId];Z.remove(),oe(Ye,He,async()=>{await B({[p+"Team"]:A,[u+"Team"]:f[u+"Team"],["usedSubIds_"+p]:ut})})})}document.body.appendChild(Z),X()}function oe(w,L,A){const j={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const G=(H,W,ee)=>`<div style="text-align:center">
      <div style="font-size:9px;color:${W};letter-spacing:2px;text-transform:uppercase;font-weight:700;margin-bottom:6px">${ee}</div>
      <div style="width:70px;height:70px;border-radius:50%;background:${j[H.job]||"#555"};border:3px solid ${W};position:relative;overflow:hidden;margin:0 auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:6px">Appuyer pour continuer</div>`,document.body.appendChild(T);let F=!1;const P=()=>{F||(F=!0,T.remove(),setTimeout(()=>A(),50))};T.addEventListener("click",P),setTimeout(P,2200)}function C(){var L;const w=document.createElement("div");w.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:20px;overflow-y:auto",w.innerHTML=`
      ...
      </div>`,document.body.appendChild(L),(A=L.querySelector("#pvp-hist-close"))==null||A.addEventListener("click",()=>L.remove())}async function le(){if(f.phase!==p+"-attack")return;const w=p==="p1"?"p2":"p1",L=(f.round||0)+1,A=[...f.log||[]];A.push({type:"info",text:`⏭️ ${f[p+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const j=$e(f),T=ct(w),G=j||!T?"finished":w+"-attack";await B({["selected_"+p]:[],modifiers:{...f.modifiers,[p]:{}},pendingAttack:null,phase:G,attacker:w,round:L,log:A}),G==="finished"&&await qe(f)}function de(w){const L=(w.GK||[]).some(j=>!j.used),A=["DEF","MIL","ATT"].some(j=>(w[j]||[]).some(T=>!T.used));return L&&!A}function he(w){return(w.p1Score||0)!==(w.p2Score||0)?null:de(w.p1Team)&&!ke(w.p2Team)?{side:"p1",gk:(w.p1Team.GK||[]).find(L=>!L.used)}:de(w.p2Team)&&!ke(w.p1Team)?{side:"p2",gk:(w.p2Team.GK||[]).find(L=>!L.used)}:null}async function Se(){const w=document.createElement("div");w.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:950;display:flex;align-items:center;justify-content:center;overflow:hidden;text-align:center;padding:24px",document.body.appendChild(w);const L=(A,j)=>new Promise(T=>{w.innerHTML=`<div style="font-size:32px;font-weight:900;color:${j};letter-spacing:2px;animation:lcFade 1.4s ease both">${A}</div>
      <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(T,1400)});await L("⚽ DERNIER CORNER","#FFD700"),await L("🧤 LE GARDIEN MONTE !","#4fc3f7"),w.remove()}async function ve(w,L){const{side:A,gk:j}=w;if(!j)return;const T=A+"Score",G=(L[T]||0)+1;j.used=!0;const F=[...L.log||[]];F.push({type:"duel",isGoal:!0,homeScored:A===p,homePlayers:A==="p1"?[qt(j)]:[],aiPlayers:A==="p2"?[qt(j)]:[],text:`⚽ DERNIER CORNER — Le gardien de ${L[A+"Name"]} marque !`});const P=(L.round||0)+1,H=A==="p1"?"p2":"p1",W={...L,[T]:G},ee=$e(W);D.add(P),await Se();const Z=A===p?G:L[p+"Score"]||0,X=A===p?L[u+"Score"]||0:G;Qe([qt(j)],Z,X,A===p,async()=>{await B({[T]:G,log:F,round:P,pendingAttack:null,phase:ee?"finished":H+"-attack",attacker:H,["selected_"+p]:[],modifiers:{...f.modifiers,[p]:{}}}),ee&&await qe({...f,[T]:G})})}async function Te(){const w=f[p+"Team"],L=!["GK","DEF","MIL","ATT"].some(G=>(f[u+"Team"][G]||[]).some(F=>!F.used)),A=(f["selected_"+p]||[]).map(G=>{const F=(w[G._role]||[]).find(ae=>ae.cardId===G.cardId)||G,P=["GK","DEF"].includes(G._role),H=w[G._role]||[],W=H.findIndex(ae=>ae.cardId===G.cardId),ee=yt(H.length),Z=W>=0?ee[W]:F._col??1,X=f.stadiumDef||f.homeStadiumDef||null,re=F.stadiumBonus||X&&(X.club_id&&String(F.club_id)===String(X.club_id)||X.country_code&&F.country_code===X.country_code)||!1;return{...F,_line:G._role,_col:Z,stadiumBonus:re,...P?{note_a:Math.max(1,Number(F.note_a)||0)}:{}}});if(!A.length)return;const j=oi(A,f.modifiers[p]||{},f[p+"Formation"]);fe(p,A.map(G=>G.cardId)),A.forEach(G=>{const F=(w[G._role]||[]).find(P=>P.cardId===G.cardId);F&&(F.used=!0)}),f["selected_"+p]=[],V();const T=[...f.log||[]];if(L){if(A.length===1&&(A[0]._line==="GK"||A[0].job==="GK")&&(f.p1Score||0)===(f.p2Score||0)){await ve({side:p,gk:A[0]},{...f,[p+"Team"]:w,log:T});return}const F=(f[p+"Score"]||0)+1,P=A.map(X=>qt(X));T.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:P,homeTotal:j.total,aiTotal:0});const H=(f.round||0)+1,W=p==="p1"?"p2":"p1",ee={...f,[p+"Team"]:w,[p+"Score"]:F},Z=$e(ee);D.add(H),Qe(P,F,f[u+"Score"]||0,!0,async()=>{await B({[p+"Team"]:w,[p+"Score"]:F,["selected_"+p]:[],modifiers:{...f.modifiers,[p]:{}},pendingAttack:null,phase:Z?"finished":W+"-attack",attacker:W,round:H,log:T}),Z&&await qe({...f,[p+"Score"]:F})});return}T.push({type:"pending",text:`⚔️ ${f[p+"Name"]} attaque (${j.total})`}),await B({[p+"Team"]:w,[u+"Team"]:f[u+"Team"],pendingAttack:{...j,players:A,side:p},["selected_"+p]:[],modifiers:{...f.modifiers,[p]:{}},phase:u+"-defense",log:T})}async function Ae(){const w=f[p+"Team"],L=(f["selected_"+p]||[]).map(se=>{const ye=(w[se._role]||[]).find(Le=>Le.cardId===se.cardId)||se,be=w[se._role]||[],ne=be.findIndex(Le=>Le.cardId===se.cardId),Ee=yt(be.length),Ne=ne>=0?Ee[ne]:ye._col??1;return{...ye,_line:se._role,_col:Ne}}),A=ri(L,f.modifiers[p]||{},f[p+"Formation"]);fe(p,L.map(se=>se.cardId)),L.forEach(se=>{const ye=(w[se._role]||[]).find(be=>be.cardId===se.cardId);ye&&(ye.used=!0)}),f["selected_"+p]=[],V();const j=Xi(f.pendingAttack.total,A.total,f.modifiers[p]||{}),T=f.pendingAttack.side,G=j==="attack"||(j==null?void 0:j.goal),F=T==="p1"?"p2":"p1",P=(f.round||0)+1,H=(f.pendingAttack.players||[]).map(se=>qt(se)),W=[...f.log||[]];W.push({type:"duel",isGoal:G,homeScored:G&&T===p,text:G?`⚽ BUT de ${f[T+"Name"]} ! (${f.pendingAttack.total} vs ${A.total})`:`✋ Attaque stoppée (${f.pendingAttack.total} vs ${A.total})`,homePlayers:H,aiPlayers:L.map(se=>qt(se)),homeTotal:f.pendingAttack.total,aiTotal:A.total});const ee=G?(f[T+"Score"]||0)+1:f[T+"Score"]||0,Z={...f,[p+"Team"]:w,[T+"Score"]:ee},X=$e(Z),re=X?"finished":F+"-attack",ae=!X&&!G?he(Z):null,ge=async()=>{if(ae){await ve(ae,Z);return}await B({[p+"Team"]:w,[u+"Team"]:f[u+"Team"],[T+"Score"]:ee,["selected_"+p]:[],modifiers:{...f.modifiers,[p]:{}},pendingAttack:null,phase:re,attacker:F,round:P,log:W}),(re==="finished"||X)&&await qe({...f,[T+"Score"]:ee})};if(G){const se=T===p,ye=se?ee:f[p+"Score"]||0,be=se?f[u+"Score"]||0:ee;D.add(P),Qe(H,ye,be,se,ge)}else await ge()}function Be(w){return["MIL","ATT"].some(L=>(w[L]||[]).some(A=>!A.used))}function ke(w){return["GK","DEF","MIL","ATT"].some(L=>(w[L]||[]).some(A=>!A.used))}function wt(w){return ke(w)&&!Be(w)}function ct(w){const L=f[w+"Team"],A=f[(w==="p1"?"p2":"p1")+"Team"];return!!(Be(L)||wt(L)&&!ke(A)||wt(L)&&!Be(A)&&ke(A))}function $e(w){return!ke(w.p1Team)&&!ke(w.p2Team)}function Ce(w){const L=w.p1Score||0,A=w.p2Score||0;return L===A?null:L>A?v.home_id:v.away_id}async function qe(w){Gt(),Ue();try{const L=Ce(w),A=L?v.home_id===L?v.away_id:v.home_id:null,j=w.p1Score||0,T=w.p2Score||0,{data:G,error:F}=await _.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:L,p_home_score:j,p_away_score:T,p_forfeit:!1});if(F&&console.error("[PvP] finish_pvp_match:",F),l)try{await _.rpc("check_and_finish_mini_league",{p_league_id:l})}catch(P){console.error("[PvP] check_and_finish_mini_league:",P)}L&&A&&Mo(L,A).catch(()=>{});try{const{data:P}=await _.rpc("apply_match_rewards",{p_match_id:i});P!=null&&P.success&&!(P!=null&&P.skipped)&&typeof t.refreshProfile=="function"&&await t.refreshProfile()}catch(P){console.error("[PvP] apply_match_rewards:",P)}if(typeof c=="function")try{await c({homeScore:j,awayScore:T,winnerId:L,homeId:v.home_id,awayId:v.away_id,matchId:i})}catch(P){console.error("[PvP] onMatchEnd:",P)}}catch(L){console.error("[PvP] finishMatch:",L)}}function Ge(){var T;const w=f[p+"Score"],L=f[u+"Score"],A=w>L,j=w===L;st(e),e.innerHTML=`
    ...
    </div>`,(T=document.getElementById("pvp-home"))==null||T.addEventListener("click",()=>{try{_.removeChannel(Q)}catch{}it(e),l?y("mini-league",{openLeagueId:l}):y("home")})}V()}async function Pi(e,t,i=!1){var s,d;const{state:n,navigate:r,toast:o}=t,a=i&&((d=(s=t==null?void 0:t.state)==null?void 0:s.params)==null?void 0:d.rankedData)||null;await wi(e,t,i?"ranked":"random",async({deckId:c,formation:l,starters:m,subsRaw:b,gcCardsEnriched:y,gcDefs:g,stadiumDef:p})=>{st(e);const u=async x=>{await zr(e,t,c,l,m,b,x||[],g,p,i,a)};if(!(y!=null&&y.length)){await u([]);return}hi(e,y,u)})}async function Ar(e,t,i){return Sr(e,t,i)}async function zr(e,t,i,n,r,o,a=[],s=[],d=null,c=!1,l=null){var ie,Q;const{state:m,navigate:b,toast:y}=t,g=(l==null?void 0:l.mmr)??((ie=m.profile)==null?void 0:ie.mmr)??1e3,p=c?Hi(g):null,u=c?"linear-gradient(135deg, #1a0a2e 0%, #2d1060 50%, #1a0a2e 100%)":"linear-gradient(135deg, #0a1a2e 0%, #0d3d1e 100%)",x=c?(p==null?void 0:p.color)||"#D4A017":"#FFD700";let h=!1,v=null,E=null;e.innerHTML=`
    ...
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:${Jt};border:1px solid ${Qt};border-radius:12px;padding:16px">
        ...
        ${m.map(B=>`
          <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid ${Lt}">
            <div style="width:36px;height:36px;border-radius:50%;background:${B.club_color2||Qn};display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:900;color:${B.club_color1||"#fff"}">${(B.pseudo||"?").slice(0,2).toUpperCase()}</div>
            <div style="flex:1"><div style="font-size:13px;font-weight:700;color:${Xe}">${B.club_name||B.pseudo}</div><div style="font-size:11px;color:${Ze}">@${B.pseudo}</div></div>
      ...
      <div style="background:${Jt};border:1px solid ${Qt};border-radius:12px;padding:16px">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>${b.map((B,I)=>{const K=g>0&&a.status==="finished"?I===0?Math.floor(g*.7):I===1?Math.floor(g*.2):I===2?Math.floor(g*.1):0:0;return`<tr style="border-bottom:1px solid ${Lt};${B.userId===o?"background:rgba(74,222,128,0.08);":""}">
              <td style="padding:7px 3px 7px 0;font-weight:700;color:${I===0?dt:I<3?"#4ade80":Et}">${["🥇","🥈","🥉"][I]||I+1}</td>
              <td style="padding:7px 3px"><div style="font-weight:700;color:${Xe}">${B.clubName}</div><div style="font-size:10px;color:${Ze}">@${B.pseudo}</div></td>
              <td style="text-align:center;color:${Et}">${B.played}</td><td style="text-align:center;color:${Et}">${B.won}-${B.drawn}-${B.lost}</td>
              <td style="text-align:center;color:${B.goalDiff>=0?"#4ade80":"#ff6b6b"}">${B.goalDiff>=0?"+":""}${B.goalDiff}</td>
              <td style="text-align:center;font-weight:900;font-size:14px;color:${Xe}">${B.points}</td>
      ...
      <div style="background:${Jt};border:1px solid ${Qt};border-radius:12px;padding:16px">
        ...
        ${Array.from({length:Math.max(0,a.status==="active"?a.current_day-1:a.current_day)},(B,I)=>I+1).reverse().map(B=>{const I=(d||[]).filter(K=>K.matchday===B);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:${Ze};margin-bottom:6px">Journée ${B}</div>${I.map(K=>kn(K,m,o,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
  ...
  </div>`;const v=a.status==="waiting"?"waiting":a.status==="active"?"active":"archived";if((E=document.getElementById("ml-back"))==null||E.addEventListener("click",()=>et(e,t,v)),(f=document.getElementById("ml-refresh"))==null||f.addEventListener("click",async B=>{const I=B.currentTarget;I.style.opacity="0.5",await lt(e,t,i)}),(z=document.getElementById("ml-start-btn"))==null||z.addEventListener("click",()=>Kr(e,t,a,m)),(k=document.getElementById("ml-next-day"))==null||k.addEventListener("click",()=>Vr(e,t,i)),(S=document.getElementById("ml-join-now"))==null||S.addEventListener("click",()=>eo(e,t,i,a.type)),(D=document.getElementById("ml-leave-btn"))==null||D.addEventListener("click",()=>Hr(e,t,i,p)),(te=document.getElementById("ml-delete-btn"))==null||te.addEventListener("click",()=>Gi(e,t,i,a.name,"waiting")),(ie=document.getElementById("ml-delete-now"))==null||ie.addEventListener("click",()=>Gi(e,t,i,a.name,v)),(Q=document.getElementById("ml-archive-btn"))==null||Q.addEventListener("click",()=>Ur(e,t,i)),e.querySelectorAll("[data-play-match]").forEach(B=>{B.addEventListener("click",()=>{const I=y.find(K=>K.id===B.dataset.playMatch);I&&t.navigate("match-mini-league",{mlMatchId:I.id,leagueId:i})})}),a.status==="finished"&&h){const B=b.findIndex(I=>I.userId===o);B>=0&&B<3&&h.prize_amount>0&&!h.prize_claimed&&setTimeout(()=>Yr(e,t,a,h,B),400)}}function kn(e,t,i,n,r=!1){const o=m=>t.find(b=>b.id===m);if(e.is_bye){const m=o(e.home_id);return`<div style="padding:8px;border-radius:8px;background:rgba(255,255,255,0.03);margin-bottom:6px;font-size:12px;color:${Et};text-align:center">🔵 ${(m==null?void 0:m.club_name)||(m==null?void 0:m.pseudo)||"?"} exempté(e)</div>`}const a=o(e.home_id),s=o(e.away_id),d=e.home_id===i||e.away_id===i,c=d&&e.status==="pending"&&n&&!r,l=e.status==="finished"?`${e.home_score} - ${e.away_score}`:"vs";return`<div style="display:flex;align-items:center;gap:8px;padding:10px;border-radius:8px;background:${d?"rgba(26,107,60,0.16)":"rgba(255,255,255,0.03)"};margin-bottom:6px;border:1px solid ${d?"rgba(74,222,128,0.35)":Lt}">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
  ...
  </div>`;let d="buy";const c=()=>{var p,u,x,h,v,E,f;return{name:(((p=document.getElementById("flt-name"))==null?void 0:p.value)||"").toLowerCase().trim(),club:(((u=document.getElementById("flt-club"))==null?void 0:u.value)||"").toLowerCase().trim(),country:(((x=document.getElementById("flt-country"))==null?void 0:x.value)||"").toLowerCase().trim(),job:((h=document.getElementById("flt-job"))==null?void 0:h.value)||"",rarity:((v=document.getElementById("flt-rarity"))==null?void 0:v.value)||"",note1:parseInt((E=document.getElementById("flt-note1"))==null?void 0:E.value)||0,note2:parseInt((f=document.getElementById("flt-note2"))==null?void 0:f.value)||0}};function l(p){const u=c();return p.filter(x=>{var D,te,ie;const h=(D=x.card)==null?void 0:D.player;if(!h)return!1;const v=`${h.firstname} ${h.surname_real}`.toLowerCase(),E=(((te=h.clubs)==null?void 0:te.encoded_name)||"").toLowerCase(),f=(h.country_code||"").toLowerCase(),z=((ie=x.card)==null?void 0:ie.evolution_bonus)||0,k=Dt(h,h.job,z),S=h.job2?Dt(h,h.job2,z):0;return!(u.name&&!v.includes(u.name)||u.club&&!E.includes(u.club)||u.country&&!f.includes(u.country)||u.job&&h.job!==u.job||u.rarity&&h.rarity!==u.rarity||u.note1&&k<u.note1||u.note2&&S<u.note2)})}function m(p){var S,D,te,ie;const u=(S=p.card)==null?void 0:S.player;if(!u)return"";const x=((D=p.card)==null?void 0:D.evolution_bonus)||0,h=Dt(u,u.job,x),v=u.job2?Dt(u,u.job2,x):0,E=(i.profile.credits||0)>=p.price,f=u.country_code?`https://flagsapi.com/${u.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,z=ui[u.job]||"#bbb",k=u.job2?ui[u.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden">
      <!-- Drapeau -->
    ...
    </div>`}function b(p){var S,D,te,ie;const u=(S=p.card)==null?void 0:S.player;if(!u)return"";const x=((D=p.card)==null?void 0:D.evolution_bonus)||0,h=Dt(u,u.job,x),v=u.job2?Dt(u,u.job2,x):0,E=p.status==="sold",f=u.country_code?`https://flagsapi.com/${u.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,z=ui[u.job]||"#bbb",k=u.job2?ui[u.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${E?"opacity:0.7":""}">
      ${f?`<img src="${f}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      ${(te=u.clubs)!=null&&te.logo_url?`<img src="${u.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${z};display:flex;align-items:center;justify-content:center">
    ...
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center">
      ...
      <div style="display:flex;gap:10px">
        ...
        <button id="buy-ok" style="flex:1;padding:12px;border-radius:10px;border:none;background:var(--green);color:#fff;font-size:14px;font-weight:900;cursor:pointer">Confirmer</button>
          ...
          </div>`}).join(""):'<div style="text-align:center;color:var(--tile-fg-dim);padding:40px">Aucun joueur classé.</div>'}else if(d==="mini-league"){const k=a||[];v.innerHTML=k.length>0?k.map((S,D)=>`
      ...
      `).join(""):`<div style="text-align:center;color:var(--tile-fg-dim);padding:40px">Aucun résultat de Mini League pour l'instant.</div>`}else{const k=s||[];v.innerHTML=k.length>0?k.map((S,D)=>`
        <div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${S.user_id===i.profile.id?"border:2px solid var(--yellow)":""}">
          <div style="width:32px;height:32px;border-radius:50%;background:${c(D)};color:${l(D)};display:flex;align-items:center;justify-content:center;font-weight:900;flex-shrink:0;font-size:${D<3?"16":"13"}px">${D<3?["🥇","🥈","🥉"][D]:D+1}</div>
          <div style="flex:1;min-width:0">
            <div style="font-weight:700;color:var(--tile-fg-on-page);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${S.pseudo}</div>
  ...
  <div class="page">
    ...
    </div>`,(z=document.getElementById("ranked-back"))==null||z.addEventListener("click",()=>n("home"));return}const d=s.mmr??1e3,c=s.mmr_deviation??350,l=s.mmr_volatility??.06,m=s.placement_matches??0,b=m<10,y=Math.max(0,10-m),g=Hi(d),p=so(d),u=Li.findIndex(te=>te.id===g.id),x=Li[u+1]||null,h={bronze:"linear-gradient(160deg,#3d1c00,#7a3e00)",silver:"linear-gradient(160deg,#1a1a2e,#3a3a5e)",gold:"linear-gradient(160deg,#1a1200,#4a3500)",platinum:"linear-gradient(160deg,#001a20,#003040)",diamond:"linear-gradient(160deg,#001030,#1a2860)",master:"linear-gradient(160deg,#1a0030,#3d0070)"},v=(s.ranked_wins||0)+(s.ranked_losses||0)+(s.ranked_draws||0),E=v>0?Math.round((s.ranked_wins||0)/v*100):0,f=Li.map(te=>`
    <div style="display:flex;flex-direction:column;align-items:center;gap:2px;opacity:${g.id===te.id?1:.35};
      transform:${g.id===te.id?"scale(1.15)":"scale(1)"};transition:all 0.3s">
      <div style="font-size:${g.id===te.id?"28px":"20px"}">${te.emoji}</div>
      <div style="font-size:9px;color:${te.color};font-weight:${g.id===te.id?"900":"400"};letter-spacing:0.5px">${te.label.toUpperCase()}</div>
    </div>
  `).join("");e.innerHTML=`
  ...
  <div class="page">
    ...
    <nav class="top-nav">
      <div class="logo" id="nav-logo" title="Manager Wars v2026.07.22-1649" style="cursor:pointer">
