...
import{s as T,F as $i,i as Ut,k as Vi,l as ot,m as Ii,f as Nt,n as vn,T as li,j as wn,o as _n,b as $n}from"./formation-links-BZe0pGr4.js";const kn="/";function Mi(e,{navigate:t,toast:i}){let o="login";const d=()=>{var r,l,u,a,n,x;const s=o==="login";e.innerHTML=`
  ...
  <div style="position:absolute;top:${n(48)};left:0;right:0;text-align:center;z-index:4;padding:0 ${n(50)}">
    <span style="font-size:${n(20)};font-weight:700;color:${f};letter-spacing:${n(2)};line-height:1;text-shadow:0 1px 4px #000;display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${C}</span>
  </div>
  ...
  <div style="position:absolute;left:${Ee}px;top:${we}px;width:${ve}px;height:${_e}px;z-index:3;
    ...
    ${j?`<img src="${j}" style="width:100%;height:100%;object-fit:cover;border-radius:${pe}px">`:`<span style="font-size:${n(22)}">🌍</span>`}
  </div>
  ...
  <div style="position:absolute;left:${se}px;top:${we}px;width:${ve}px;height:${_e}px;z-index:3;
    ...
    ${bt(e,{width:140})}
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),a=(l||[]).filter(O=>O.card_type==="player"&&O.player),n=(l||[]).filter(O=>O.card_type==="game_changer"),x=(l||[]).filter(O=>O.card_type==="formation"),f=(l||[]).filter(O=>O.card_type==="stadium"),{data:h}=await T.from("gc_definitions").select("name,gc_type,color,effect,image_url"),_={};(h||[]).forEach(O=>{_[O.name]=O});const{data:m}=await T.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),c={};(m||[]).forEach(O=>{c[O.id]=O}),f.forEach(O=>{O.stadium_def&&(c[O.stadium_id]=O.stadium_def)});const A=Object.keys($i),p=Object.keys(Oe),I={};a.forEach(O=>{const ee=O.player.id;I[ee]=(I[ee]||0)+1}),new Set(Object.keys(I).map(O=>String(O)));const q=new Set(x.map(O=>O.formation)),j=new Set(n.map(O=>O.gc_type));let P="player",C="Tous",N="",V=!1;function K(){return[...a].sort((O,ee)=>{const xe=Wt.indexOf(O.player.job),le=Wt.indexOf(ee.player.job);return xe!==le?xe-le:(O.player.surname_encoded||"").localeCompare(ee.player.surname_encoded||"")})}function oe(){return[...u||[]].sort((O,ee)=>{const xe=Wt.indexOf(O.job),le=Wt.indexOf(ee.job);return xe!==le?xe-le:(O.surname_encoded||"").localeCompare(ee.surname_encoded||"")})}function ie(){return K().filter(O=>{const ee=O.player,xe=C==="Tous"||ee.job===C,le=!N||`${ee.firstname} ${ee.surname_encoded}`.toLowerCase().includes(N);return xe&&le})}function Q(){return oe().filter(O=>{const ee=C==="Tous"||O.job===C,xe=!N||`${O.firstname} ${O.surname_encoded}`.toLowerCase().includes(N);return ee&&xe})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",ee=>{N=ee.target.value.toLowerCase(),D()}),e.querySelectorAll(".filter-btn").forEach(ee=>{ee.addEventListener("click",()=>{C=ee.dataset.job,R(),D()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{V=!V,R(),D()})):(O.innerHTML=`
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Jt=1e3;function to(e,t,i,o){var _,m,c;const{state:d,toast:s,closeModal:r,navigate:l,refreshProfile:u}=i,a=e.formation,n={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function x(){const A=Ut[a]||{},p=$i[a]||[],I=290,q=360,j=20;function P(N){const V=A[N];return V?{x:V.x*I,y:V.y*q}:null}let C=`<svg width="${I}" height="${q}" viewBox="0 0 ${I} ${q}" xmlns="http://www.w3.org/2000/svg">`;for(const[N,V]of p){const K=P(N),oe=P(V);!K||!oe||(C+=`<line x1="${K.x}" y1="${K.y}" x2="${oe.x}" y2="${oe.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const N of Object.keys(A)){const V=P(N);if(!V)continue;const K=N.replace(/\d+/,""),oe=n[K]||"#555";C+=`<circle cx="${V.x}" cy="${V.y}" r="${j}" fill="${oe}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,C+=`<text x="${V.x}" y="${V.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${K}</text>`}return C+="</svg>",C}const f=t.filter(A=>A.formation===a);f.length;const h=!e.is_for_sale;o(`Formation ${a}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(_=document.getElementById("direct-sell-form-btn"))==null||_.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${a} pour ${Jt.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const A=f.find(I=>!I.is_for_sale)||f[0];if(!A){s("Aucune carte à vendre","error");return}await T.from("market_listings").delete().eq("card_id",A.id),await T.from("transfer_history").delete().eq("card_id",A.id);const{error:p}=await T.from("cards").delete().eq("id",A.id);if(p){s(p.message,"error");return}await T.from("users").update({credits:(d.profile.credits||0)+Jt}).eq("id",d.profile.id),await u(),s(`+${Jt.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),r(),l("collection")}),(m=document.getElementById("market-sell-form-btn"))==null||m.addEventListener("click",async()=>{const A=parseInt(document.getElementById("sell-price-form").value);if(!A||A<1){s("Prix invalide","error");return}await T.from("cards").update({is_for_sale:!0,sale_price:A}).eq("id",e.id),await T.from("market_listings").insert({seller_id:d.profile.id,card_id:e.id,price:A}),s("Carte mise en vente sur le marché !","success"),r(),l("collection")}),(c=document.getElementById("cancel-sell-form-btn"))==null||c.addEventListener("click",async()=>{await T.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await T.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),s("Annonce retirée","success"),r(),l("collection")})}async function Ni(e,t,i,o){var me,_e,ve,we,Ee;const{state:d,toast:s,openModal:r,closeModal:l,navigate:u,refreshProfile:a}=o,n=e.player,x=t.filter(se=>se.player.id===n.id),f=x.length,h=e.evolution_bonus||0,_=Math.max((Number(n.note_g)||0)+(n.job==="GK"||n.job2==="GK"?h:0),(Number(n.note_d)||0)+(n.job==="DEF"||n.job2==="DEF"?h:0),(Number(n.note_m)||0)+(n.job==="MIL"||n.job2==="MIL"?h:0),(Number(n.note_a)||0)+(n.job==="ATT"||n.job2==="ATT"?h:0)),m=n.rarity||"normal",{data:c}=await T.from("sell_price_configs").select("*").eq("rarity",m).lte("note_min",_).gte("note_max",_).order("note_min",{ascending:!1}).limit(1);((me=c==null?void 0:c[0])==null?void 0:me.price)??Yn[m];const A=n.rarity!=="legende";Qn(n);const p=((n.rarity==="pepite"||n.rarity==="papyte")&&e.current_note!=null?e.current_note:pi(n,n.job))+h,I=n.rarity==="pepite"||n.rarity==="papyte",q=n.job2?(I?nn(e,Jn(n.job2)):pi(n,n.job2))+(pi(n,n.job2)>0?h:0):null;ci[n.job],n.job2&&ci[n.job2];const j=Kn[n.rarity]||"#ccc";Xn[n.country_code]||n.country_code;const P=e.evolution_bonus||0,N=p+P,V=q||0,K=V>0?V+P:0,oe=x.map(se=>se.id);let ie={};if(oe.length){const{data:se}=await T.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",oe).order("transferred_at",{ascending:!0});(se||[]).forEach(pe=>{ie[pe.card_id]||(ie[pe.card_id]=[]),ie[pe.card_id].push(pe)})}const Q=se=>{const pe=se.transferred_at?new Date(se.transferred_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"2-digit",year:"numeric"}):"",de=se.source==="booster"?"Booster":se.price?se.price.toLocaleString("fr"):"—";return`
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(_e=document.getElementById("close-detail"))==null||_e.addEventListener("click",l);let D=new Set;const re=()=>{const se=D.size,pe=document.getElementById("sell-action-panel");if(!pe)return;pe.style.display=se>0?"block":"none",document.getElementById("sell-selected-count").textContent=se;const de=document.getElementById("evolve-btn");de&&(de.textContent=`⬆️ Évoluer ${se>1?"(+"+se+")":""}`)};document.querySelectorAll(".expl-check").forEach(se=>{se.addEventListener("change",()=>{const pe=se.dataset.id;se.checked?D.add(pe):D.delete(pe);const de=se.closest(".exemplaire-row");de&&(de.style.borderColor=se.checked?"#1A6B3C":"#eee"),re()})}),document.querySelectorAll(".exemplaire-row").forEach(se=>{se.addEventListener("click",pe=>{if(pe.target.closest("button")||pe.target.tagName==="INPUT")return;const de=se.querySelector(".expl-check");de&&!de.disabled&&(de.checked=!de.checked,de.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(se=>{se.addEventListener("click",pe=>{pe.stopPropagation();const de=document.querySelector(`.expl-hist[data-hist="${se.dataset.idx}"]`);de&&(de.style.display=de.style.display==="none"?"flex":"none")})}),(ve=document.getElementById("evolve-btn"))==null||ve.addEventListener("click",async()=>{if(f<=1)return;const se=[...D];if(!se.length)return;if(D.has(e.id)){const ge=document.createElement("div");ge.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",ge.innerHTML=`
        ...
        <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center;box-shadow:0 16px 48px rgba(0,0,0,0.3)">
          ...
          <div style="background:#f0fdf4;border-radius:10px;padding:12px;margin-bottom:16px;font-size:13px;color:#333">
            ...
            ${q&&q>0?`<br>📈 Note 2 : <strong>${q+(e.evolution_bonus||0)}</strong> → <strong style="color:#1A6B3C">${q+(e.evolution_bonus||0)+pe.length*de}</strong>`:""}
  ...
  <div style="height:100%;overflow-y:auto;padding-bottom:80px;background:var(--page-bg)">
    ...
    <div class="page-body">
      ...
      stadium_def:stadium_definitions(id, name, club_id, country_code, image_url,
        club:clubs(encoded_name, logo_url))`).eq("owner_id",o.profile.id).eq("card_type","stadium"),u=(r||[]).filter(A=>A.card_type==="player"&&A.player),a=(r||[]).filter(A=>A.card_type==="formation"),n=(l||[]).filter(A=>A.card_type==="stadium"),x=[...new Set(n.map(A=>A.stadium_id).filter(Boolean))];let f={};if(n.forEach(A=>{A.stadium_def&&A.stadium_id&&(f[A.stadium_id]=A.stadium_def)}),x.length&&Object.keys(f).length<x.length){const{data:A}=await T.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",x);(A||[]).forEach(p=>{f[p.id]=p})}const h=a.map(A=>A.formation).filter(Boolean),{data:_}=await T.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",e);let m=s.formation||"4-4-2";h.length>0&&!h.includes(m)&&(m=h[0]);const c={deckId:e,name:s.name,formation:m,formationCardId:s.formation_card_id,stadiumCardId:s.stadium_card_id||null,slots:{},subs:[],playerCards:u,formationCards:a,stadiumCards:n,stadDefMap:f,availableFormations:h};(_||[]).forEach(A=>{A.is_starter?c.slots[A.position]=A.card_id:c.subs.includes(A.card_id)||c.subs.push(A.card_id)}),ht(t,c,i)}function ht(e,t,i){var _,m,c,A;const{navigate:o}=i;ei[t.formation];const d=Oi(t.formation),s=d.filter(p=>t.slots[p]).length,r=t.availableFormations.length>0?t.availableFormations:Object.keys(ei),l=(_=t.stadiumCards)==null?void 0:_.find(p=>p.id===t.stadiumCardId),u=l&&((m=t.stadDefMap)==null?void 0:m[l.stadium_id])||null,a=t.subs.map(p=>t.playerCards.find(I=>I.id===p)).filter(Boolean);[...Object.values(t.slots),...t.subs],e.innerHTML=`
  <div style="height:100%;overflow-y:auto;padding-bottom:80px;background:var(--page-bg)">
        ...
        <div style="width:115px;flex-shrink:0;background:rgba(0,0,0,0.3);display:flex;flex-direction:column;align-items:center;padding:12px 8px;gap:10px;border-right:1px solid rgba(255,255,255,0.1)">
          ...
          <div style="display:flex;flex-direction:column;gap:6px;align-items:center" id="subs-list">
            ${a.map(p=>{const I={...p.player,_evolution_bonus:p.evolution_bonus||0};return`<div style="position:relative;flex-shrink:0;overflow:visible">
                ${bt({...I,_evolution_bonus:I._evolution_bonus||0},{width:60,showStad:!0,stadDef:u})}
        ...
        <div style="width:130px;flex-shrink:0;background:rgba(0,0,0,0.3);display:flex;flex-direction:column;align-items:center;padding:12px 8px;gap:12px;border-left:1px solid rgba(255,255,255,0.1)">
          ...
          <div style="width:100%;text-align:center">
            ...
            <div id="add-stad-btn" style="cursor:pointer;margin:0 auto;width:fit-content">
              ${l?(()=>{var q;const p=t.stadDefMap[l.stadium_id],I=((q=p==null?void 0:p.club)==null?void 0:q.logo_url)||null;return`<div style="width:100px;height:130px;border-radius:8px;background:linear-gradient(135deg,#1a3a6b,#0a1a3a);border:2px solid #E87722;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px">
        ...
        <div style="display:flex;gap:10px;align-items:flex-start">
          ...
          <div style="flex:1;min-width:0">
            ...
            <div style="display:flex;gap:4px;align-items:flex-end;flex-wrap:nowrap;overflow-x:auto" id="subs-list">
              ${a.map(p=>{const I={...p.player,_evolution_bonus:p.evolution_bonus||0};return`<div style="position:relative;flex-shrink:0;overflow:visible">
                  ${bt({...I,_evolution_bonus:I._evolution_bonus||0},{width:44,showStad:!0,stadDef:u})}
                  <button data-remove-sub="${p.id}"
  ...
  </div>`;const n=window.innerWidth>=768,x=e.querySelector(".deck-pc-layout"),f=e.querySelector(".deck-mobile-layout"),h=e.querySelector(".formation-mobile-only");x&&(x.style.display=n?"block":"none"),f&&(f.style.display=n?"none":"block"),h&&(h.style.display=n?"none":"block"),io(e,t,d,i),document.getElementById("builder-back").addEventListener("click",()=>o("decks")),document.getElementById("formation-select").addEventListener("change",p=>{t.formation=p.target.value;const I=Oi(t.formation),q={};I.forEach(j=>{t.slots[j]&&(q[j]=t.slots[j])}),t.slots=q,ht(e,t,i)}),(c=document.getElementById("add-stad-btn"))==null||c.addEventListener("click",()=>{no(t,e,i)}),document.getElementById("save-deck").addEventListener("click",()=>ao(t,i)),e.querySelectorAll("[data-remove-sub]").forEach(p=>{p.addEventListener("click",()=>{t.subs=t.subs.filter(I=>I!==p.dataset.removeSub),ht(e,t,i)})}),(A=document.getElementById("add-sub-btn"))==null||A.addEventListener("click",()=>{ro(t,e,i)})}function io(e,t,i,o){var I,q;const d=window.innerWidth>=768,s=e.querySelector(d?"#deck-field-pc":"#deck-field-mobile");if(!s)return;const r=(I=t.stadiumCards)==null?void 0:I.find(j=>j.id===t.stadiumCardId),l=r&&((q=t.stadDefMap)==null?void 0:q[r.stadium_id])||null,u=Ut[t.formation]||{},a=Vi(t.formation)||[],n={};for(const j of i){const P=t.slots[j],C=P?t.playerCards.find(N=>N.id===P):null;C!=null&&C.player?n[j]={...C.player,_evolution_bonus:C.evolution_bonus||0,face:C.player.face||null}:n[j]=null}const x=window.innerWidth>=768,f=x?window.innerWidth-200:window.innerWidth-20,h=x?Math.min(f,900):f,_=Math.round(x?h*1.05:h*1.35),m=x?75:44;let c="";for(const[j,P]of a){const C=u[j],N=u[P];if(!C||!N)continue;const V=C.x*h,K=C.y*_,oe=N.x*h,ie=N.y*_,Q=n[j],R=n[P],D=ot(Q,R);D==="#ff3333"||D==="#cc2222"?c+=`<line x1="${V.toFixed(1)}" y1="${K.toFixed(1)}" x2="${oe.toFixed(1)}" y2="${ie.toFixed(1)}" stroke="${D}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(c+=`<line x1="${V.toFixed(1)}" y1="${K.toFixed(1)}" x2="${oe.toFixed(1)}" y2="${ie.toFixed(1)}" stroke="${D}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,c+=`<line x1="${V.toFixed(1)}" y1="${K.toFixed(1)}" x2="${oe.toFixed(1)}" y2="${ie.toFixed(1)}" stroke="${D}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let A="";const p=Math.round(m*657/507);for(const j of i){const P=u[j];if(!P)continue;const C=n[j],N=P.x*h,V=P.y*_,K=Math.round(N-m/2),oe=Math.round(V-p/2);if(C){const ie=bt({...C,_evolution_bonus:C._evolution_bonus||0},{width:m,showStad:!0,stadDef:l});A+=`<div style="position:absolute;left:${K}px;top:${oe}px;cursor:pointer;z-index:2" class="deck-slot-hit" data-pos="${j}">${ie}</div>`}else{const ie=j.replace(/\d+/,"");A+=`<div style="position:absolute;left:${K}px;top:${oe}px;width:${m}px;height:${p}px;
    ...
    <div style="position:relative;width:${h}px;height:${_}px;margin:0 auto">
      ...
      ${A}
    </div>`,s.querySelectorAll(".deck-slot-hit").forEach(j=>{j.addEventListener("click",()=>oo(j.dataset.pos,t,e,o))})}function no(e,t,i){var r;const{openModal:o,closeModal:d}=i,s=e.stadiumCards||[];o("🏟️ Choisir un stade",`<div style="display:flex;flex-wrap:wrap;gap:10px;padding:8px">
      ...
      ${x.length>0?x.map(m=>{var j,P;const c=m.player,A=m.evolution_bonus||0,p=(r==="GK"?c.note_g:r==="DEF"?c.note_d:r==="MIL"?c.note_m:c.note_a)+(r===c.job||r===c.job2?A:0),I=rn(c),q={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[c.rarity];return`<div class="player-option" data-card-id="${m.id}"
          ...
          <div style="flex:1;min-width:0">
            ...
            <div style="display:flex;align-items:center;gap:6px;margin-top:3px">
              ...
              ${(j=c.clubs)!=null&&j.logo_url?`<img src="${c.clubs.logo_url}" style="width:18px;height:18px;object-fit:contain">`:`<span style="font-size:10px;color:var(--gray-600)">${((P=c.clubs)==null?void 0:P.encoded_name)||"—"}</span>`}
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      card:cards(id,card_type,formation,stadium_id,evolution_bonus,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
          clubs(encoded_name,logo_url)))`).in("deck_id",r).order("slot_order"),u=[...new Set((l||[]).filter(f=>{var h,_;return((h=f.card)==null?void 0:h.card_type)==="stadium"&&((_=f.card)==null?void 0:_.stadium_id)}).map(f=>f.card.stadium_id))],a={};if(u.length){const{data:f}=await T.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",u);(f||[]).forEach(h=>{a[h.id]=h}),(l||[]).forEach(h=>{var _,m;((_=h.card)==null?void 0:_.card_type)==="stadium"&&((m=h.card)!=null&&m.stadium_id)&&(h.card._stadiumDef=a[h.card.stadium_id]||null)})}let n=0;function x(){var j,P,C,N,V,K,oe;const f=s[n],h=(l||[]).filter(ie=>ie.deck_id===f.id),_=h.filter(ie=>{var Q;return ie.is_starter&&((Q=ie.card)==null?void 0:Q.player)}).map(ie=>hi(ie.card,ie.position)),m=h.find(ie=>{var Q;return((Q=ie.card)==null?void 0:Q.card_type)==="formation"}),c=f.formation||((j=m==null?void 0:m.card)==null?void 0:j.formation)||"4-4-2";let A=_.length>=11?vt(_,c):null,p=((P=f.stadium_card)==null?void 0:P.stadium_def)||null;p&&A&&(A=Tt(A,p));const I=_.length>=11;et(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div style="display:flex;align-items:center;gap:8px;padding:6px 14px;background:linear-gradient(90deg,rgba(232,119,34,0.3),rgba(196,90,0,0.2));border-top:1px solid rgba(232,119,34,0.4);flex-shrink:0">
        ...
        <div style="font-size:11px;font-weight:900;color:#FFD700">+10 aux joueurs ${((C=p.club)==null?void 0:C.encoded_name)||p.country_code||""}</div>
      </div>`:""}
    ...
    </div>`,function(){const Q=e.querySelector(".deck-preview-wrap svg");Q&&(Q.removeAttribute("width"),Q.removeAttribute("height"),Q.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",Q.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),(N=document.getElementById("prev-deck"))==null||N.addEventListener("click",()=>{n>0&&(n--,x())}),(V=document.getElementById("next-deck"))==null||V.addEventListener("click",()=>{n<s.length-1&&(n++,x())}),(K=document.getElementById("validate-deck"))==null||K.addEventListener("click",()=>{if(!I)return;const ie=t.state.params||{};t.navigate("match",{...ie,matchMode:ie.matchMode||i,deckId:f.id})}),(oe=document.getElementById("cancel-deck-select"))==null||oe.addEventListener("click",()=>{Ve(e),d("home")});const q=document.getElementById("deck-swipe-zone");if(q){let ie=0,Q=0;q.addEventListener("touchstart",R=>{ie=R.touches[0].clientX,Q=R.touches[0].clientY},{passive:!0}),q.addEventListener("touchend",R=>{const D=R.changedTouches[0].clientX-ie,re=R.changedTouches[0].clientY-Q;Math.abs(D)<40||Math.abs(D)<Math.abs(re)||(D<0&&n<s.length-1?(n++,x()):D>0&&n>0&&(n--,x()))},{passive:!0})}}x()}function ko(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";return e!=null&&e.clubLogo?e.clubLogo.startsWith("http")?e.clubLogo:`${t}/storage/v1/object/public/assets/clubs/${e.clubLogo}`:null}function Xe(e,t=44,i=58,o=null){return bt(e,{width:t,showStad:!!o,stadDef:o,used:e==null?void 0:e.used})}function it(e,t,i){if(!(e!=null&&e.length))return"";const o=e.slice(0,5);let d='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return o.forEach((s,r)=>{if(d+=Xe(s,40,52),r<o.length-1){const l=ot(s,o[r+1]);d+=`<div style="width:7px;height:3px;background:${l==="#ff3333"||l==="#cc2222"?"rgba(255,255,255,0.12)":l};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(d+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),d+="</div>",d}function Eo(e){return!e||e.length<2?null:`https://flagsapi.com/${e.slice(0,2).toUpperCase()}/flat/64.png`}function To(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function lt(e,t,i,o,d=310,s=310,r=[]){var I;const l=Ut[t]||{},u=Vi(t)||$i[t]||[],a={},n=["ATT","MIL","DEF","GK"];for(const q of n)(e[q]||[]).forEach((P,C)=>{a[`${q}${C+1}`]=P});function x(q){const j=l[q];return j?{x:j.x*d,y:j.y*s}:null}let f="";for(const[q,j]of u){const P=x(q),C=x(j);if(!P||!C)continue;const N=a[q],V=a[j],K=ot(N,V);K==="#00ff88"||K==="#FFD700"?(f+=`<line x1="${P.x.toFixed(1)}" y1="${P.y.toFixed(1)}" x2="${C.x.toFixed(1)}" y2="${C.y.toFixed(1)}"
        stroke="${K}" stroke-width="10" stroke-linecap="round" opacity="0.22"/>`,f+=`<line x1="${P.x.toFixed(1)}" y1="${P.y.toFixed(1)}" x2="${C.x.toFixed(1)}" y2="${C.y.toFixed(1)}"
        stroke="${K}" stroke-width="3.5" stroke-linecap="round" opacity="0.95"/>`):f+=`<line x1="${P.x.toFixed(1)}" y1="${P.y.toFixed(1)}" x2="${C.x.toFixed(1)}" y2="${C.y.toFixed(1)}"
        stroke="${K}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const h=48,_=64,m=13,c=16,A={normal:"#aaaaaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#111111",legende:"#7a28b8",légende:"#7a28b8"};for(const[q,j]of Object.entries(a)){const P=x(q);if(!P||!j)continue;const C=q.replace(/[0-9]/g,""),N=Ht[C]||"#555",V=r.includes(j.cardId),K=i==="attack"&&(["MIL","ATT"].includes(C)||V)&&!j.used||i==="defense"&&["GK","DEF","MIL"].includes(C)&&!j.used,oe=o.includes(j.cardId);let ie;i==="attack"?ie=V?Math.max(1,Number(j.note_a)||0):C==="MIL"?Number(j.note_m)||0:Number(j.note_a)||0:i==="defense"?ie=C==="GK"?Number(j.note_g)||0:C==="MIL"?Number(j.note_m)||0:Number(j.note_d)||0:ie=Number(C==="GK"?j.note_g:C==="DEF"?j.note_d:C==="MIL"?j.note_m:j.note_a)||0,ie=ie+(j.boost||0),j.stadiumBonus&&(i==="attack"&&(C==="ATT"||C==="MIL")||i==="defense"&&(C==="GK"||C==="DEF"||C==="MIL")?ie+=10:i||(ie+=10));const Q=(P.x-h/2).toFixed(1),R=(P.y-_/2).toFixed(1),D=A[j==null?void 0:j.rarity]||A.normal;if(j.used){const se=typeof import.meta<"u"&&"/"||"/",de=`${typeof window<"u"&&((I=window.location)==null?void 0:I.origin)||""}${se}icons/carte-dos.png`.replace(/([^:])\/\//g,"$1/");f+=`<rect x="${Q}" y="${R}" width="${h}" height="${_}" rx="5" fill="#161616"/>`,f+=`<image href="${de}" xlink:href="${de}" x="${Q}" y="${R}" width="${h}" height="${_}" preserveAspectRatio="xMidYMid slice"/>`,f+=`<rect x="${Q}" y="${R}" width="${h}" height="${_}" rx="5" fill="none" stroke="${D}" stroke-width="2" opacity="0.7"/>`,f+=`<rect x="${Q}" y="${R}" width="${h}" height="${_}" rx="5" fill="rgba(0,0,0,0.01)" class="match-used-hit" data-card-id="${j.cardId}" data-role="${C}" style="cursor:pointer"/>`;continue}const re=oe?.45:1,me=oe?"#FFD700":D,_e=oe?3:(j==null?void 0:j.rarity)==="legende"||(j==null?void 0:j.rarity)==="légende"||(j==null?void 0:j.rarity)==="pepite"||(j==null?void 0:j.rarity)==="pépite"?2.5:2,ve=_-m-c;f+=`<defs><clipPath id="cp-${q}"><rect x="${Q}" y="${(P.y-_/2+m).toFixed(1)}" width="${h}" height="${ve}"/></clipPath></defs>`,f+=`<rect x="${Q}" y="${R}" width="${h}" height="${_}" rx="5" fill="${N}" opacity="${re}"/>`;const we=He(j);we&&(f+=`<image href="${we}" xlink:href="${we}" x="${Q}" y="${(P.y-_/2+m).toFixed(1)}" width="${h}" height="${ve}" clip-path="url(#cp-${q})" preserveAspectRatio="xMidYMin slice"/>`),f+=`<rect x="${Q}" y="${R}" width="${h}" height="${m}" rx="4" fill="rgba(255,255,255,0.92)"/>`,f+=`<text x="${P.x.toFixed(1)}" y="${(P.y-_/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(j.name||"").slice(0,9)}</text>`;const Ee=(P.y+_/2-c).toFixed(1);f+=`<rect x="${Q}" y="${Ee}" width="${h}" height="${c}" fill="rgba(255,255,255,0.92)"/>`;{const se=Eo(j.country_code);se?f+=`<image href="${se}" xlink:href="${se}" x="${(P.x-20).toFixed(1)}" y="${(P.y+_/2-c+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:f+=`<text x="${(P.x-13).toFixed(1)}" y="${(P.y+_/2-c/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${To(j.country_code)}</text>`;const pe=j.stadiumBonus?"#E87722":"#111";f+=`<text x="${P.x.toFixed(1)}" y="${(P.y+_/2-c/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="${pe}" font-family="Arial Black">${ie}</text>`;const de=ko(j);de?f+=`<image href="${de}" xlink:href="${de}" x="${(P.x+h/2-14).toFixed(1)}" y="${(P.y+_/2-c+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:j.clubName&&(f+=`<text x="${(P.x+14).toFixed(1)}" y="${(P.y+_/2-c/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(j.clubName||"").slice(0,3).toUpperCase()}</text>`),j.stadiumBonus&&(f+=`<rect x="${(P.x-h/2).toFixed(1)}" y="${(P.y-_/2).toFixed(1)}" width="${h}" height="${_}" rx="5" fill="none" stroke="#E87722" stroke-width="2" opacity="0.8"/>`)}f+=`<rect x="${Q}" y="${R}" width="${h}" height="${_}" rx="5" fill="${oe?"rgba(255,255,255,0.12)":"none"}" stroke="${me}" stroke-width="${_e}" opacity="${re}"/>`,K&&(f+=`<rect x="${Q}" y="${R}" width="${h}" height="${_}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${oe?"selected":""}" data-card-id="${j.cardId}" data-role="${C}" style="cursor:pointer"/>`)}const p=38;return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-p} ${-p} ${d+p*2} ${s+p*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${f}
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
  </div>`,setTimeout(()=>Io(e,t,i),5e3)}function Io(e,t,i){const o=t.homeTeam.MIL||[],d=t.aiTeam.MIL||[],s=t.stadiumDef||null,r=t.aiStadiumDef||null;function l(p,I){const q=Se(p,"MIL"),j=p.evolution_bonus||p._evolution_bonus||0,P=p.job==="MIL"||p.job2==="MIL"?j:0,C=p.stadiumBonus||I&&(I.club_id&&String(p.club_id)===String(I.club_id)||I.country_code&&p.country_code===I.country_code)?10:0;return q+P+C}function u(p,I){return p.reduce((q,j)=>q+l(j,I),0)}function a(p){let I=0;for(let q=0;q<p.length-1;q++){const j=ot(p[q],p[q+1]);j==="#00ff88"?I+=2:j==="#FFD700"&&(I+=1)}return I}const n=u(o,s)+a(o),x=u(d,r)+a(d),f=n>=x;function h(p,I,q,j,P){return`<div id="duel-row-${j}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0), opacity .5s ease;transform-origin:center">
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${p.map((C,N)=>{const V=N<p.length-1?ot(C,p[N+1]):null,K=!V||V==="#ff3333"||V==="#cc2222",oe=V==="#00ff88"?"+2":V==="#FFD700"?"+1":"";l(C,P),C.stadiumBonus||P&&(P.club_id&&String(C.club_id)===String(P.club_id)||P.country_code&&(C.country_code,P.country_code));const ie=C.evolution_bonus||C._evolution_bonus||0;return`
          <div class="duel-card duel-card-${j}" data-idx="${N}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease, transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
            ${Xe({...C,_evolution_bonus:ie},58,78,P)}
          </div>
          ${N<p.length-1?`<div class="duel-link duel-link-${j}" data-idx="${N}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${K?"rgba(255,255,255,0.12)":V};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${K?"none":`0 0 8px ${V}`}">
  ...
  </div>`;const _=()=>{const p=(I,q)=>e.querySelectorAll(I).forEach((j,P)=>{setTimeout(()=>{j.style.opacity="1",j.style.transform="translateY(0) scale(1)"},q+P*90)});p(".duel-card-home",150),p(".duel-card-ai",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((I,q)=>{setTimeout(()=>{I.style.opacity="1"},q*70)}),900),setTimeout(()=>{const I=e.querySelector("#vs-label");I&&(I.style.opacity="1",I.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(q=>q.style.opacity="1")},1250),setTimeout(()=>{m("score-home",n,800),m("score-ai",x,800)},1500)};function m(p,I,q){const j=document.getElementById(p);if(!j)return;const P=performance.now(),C=N=>{const V=Math.min(1,(N-P)/q);j.textContent=Math.round(I*(1-Math.pow(1-V,3))),V<1?requestAnimationFrame(C):j.textContent=I};requestAnimationFrame(C)}requestAnimationFrame(_),t.attacker=f?"home":"ai";const c=f?ri():null;f&&(t.boostCard={value:c}),t.log.push({type:"duel",title:"Milieu de Terrain",homePlayers:o.map(p=>({name:p.name,note:Se(p,"MIL"),portrait:He(p),job:p.job,country_code:p.country_code,rarity:p.rarity,clubName:p.clubName,clubLogo:p.clubLogo})),aiPlayers:d.map(p=>({name:p.name,note:Se(p,"MIL"),portrait:He(p),job:p.job,country_code:p.country_code,rarity:p.rarity,clubName:p.clubName,clubLogo:p.clubLogo})),homeTotal:n,aiTotal:x,text:`Duel milieu : ${t.clubName} ${n} – ${x} IA → ${f?t.clubName+" attaque":"IA attaque"}`});const A=()=>{t.phase=t.attacker==="home"?"attack":"ai-attack",Je(e,t,i),t.attacker==="ai"&&setTimeout(()=>ti(e,t,i),800)};setTimeout(()=>{const p=document.getElementById("score-home"),I=document.getElementById("score-ai"),q=document.getElementById(f?"duel-row-home":"duel-row-ai"),j=document.getElementById(f?"duel-row-ai":"duel-row-home"),P=f?p:I,C=f?I:p;P&&(P.style.fontSize="80px",P.style.color=f?"#FFD700":"#ff6b6b",P.style.animation="duelPulse .5s ease"+(f?", duelGlow 1.5s ease infinite .5s":"")),C&&(C.style.opacity="0.25"),setTimeout(()=>{q&&(q.style.transformOrigin="center",q.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",q.style.zIndex="5"),setTimeout(()=>{var V;const N=document.getElementById("duel-shock");if(N){const K=(V=j||q)==null?void 0:V.getBoundingClientRect(),oe=e.querySelector(".match-screen").getBoundingClientRect();K&&(N.style.top=K.top-oe.top+K.height/2+"px"),N.style.animation="shockwave .5s ease-out forwards"}j&&(j.style.transformOrigin="center",j.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var V;const N=document.getElementById("duel-finale");N&&(N.innerHTML=`
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    ${(()=>{const D=window.innerWidth>=700,re=(O,ee,xe)=>{var B,$;const le=(t.gcDefs||[]).find(g=>g.name===O.gc_type),ge={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[le==null?void 0:le.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",ne={purple:"#b06ce0",light_blue:"#00d4ef"}[le==null?void 0:le.color]||"#b06ce0",ue=(le==null?void 0:le.name)||O.gc_type,Le=(le==null?void 0:le.effect)||((B=Oe[O.gc_type])==null?void 0:B.desc)||"",Ie=le!=null&&le.image_url?`/icons/${le.image_url}`:null,Ce=(($=Oe[O.gc_type])==null?void 0:$.icon)||"⚡",Te=Math.round(xe*.22),qe=Math.round(xe*.22),Pe=xe-Te-qe,ze=ue.length>12?7:9;return`<div class="gc-mini" data-gc-id="${O.id}" data-gc-type="${O.gc_type}"
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(s);let l=!1;const u=()=>{l||(l=!0,s.remove(),setTimeout(()=>d(),50))};s.addEventListener("click",u),setTimeout(u,3500)}async function Pt(e,t,i){var n,x;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:o}=i,d=t.homeScore>t.aiScore,s=t.homeScore===t.aiScore,r=d?"victoire":s?"nul":"defaite",l=Hn(t.mode,r);t.matchId&&await T.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:d?o.profile.id:null,home_credits_reward:l,played_at:new Date().toISOString()}).eq("id",t.matchId);const u={credits:(o.profile.credits||0)+l,matches_played:(o.profile.matches_played||0)+1};d?u.wins=(o.profile.wins||0)+1:s?u.draws=(o.profile.draws||0)+1:u.losses=(o.profile.losses||0)+1,await T.from("users").update(u).eq("id",o.profile.id),await i.refreshProfile();const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",a.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const q=async(N=!0)=>{_=!0,m&&(T.removeChannel(m),m=null),N&&await T.rpc("cancel_matchmaking",{p_user_id:x.profile.id}).catch(()=>{})};(C=document.getElementById("mm-cancel"))==null||C.addEventListener("click",async()=>{try{await q(!0)}catch{}Ve(e),f("home")});const j=async(N,V,K)=>{if(_)return;_=!0,m&&(T.removeChannel(m),m=null);const oe=document.getElementById("mm-status");oe&&(oe.textContent="Adversaire trouvé !"),await new Promise(ie=>setTimeout(ie,600)),e.isConnected&&gn(e,t,N,K,r,l,a,n,u)};if(a){const N=[{range:50,delay:0},{range:100,delay:15e3},{range:200,delay:3e4},{range:400,delay:45e3},{range:800,delay:6e4}];let V=0,K=!1;const oe=async()=>{if(_||K)return;const{range:ie}=N[V],Q=document.getElementById("mm-range");Q&&(Q.textContent=`Plage MMR : ±${ie}`);const{data:R,error:D}=await T.rpc("try_matchmake_ranked",{p_user_id:x.profile.id,p_deck_id:i,p_mmr:c,p_range:ie});if(console.log("[Ranked] try_matchmake_ranked result:",R,"error:",JSON.stringify(D)),_)return;if(D||!(R!=null&&R.success)){h("Erreur matchmaking ranked","error"),Ve(e),f("home");return}if(R.matched){K=!0,j(R.match_id,R.opponent_id,!1);return}const re=document.getElementById("mm-status");if(re&&(re.textContent="En attente d'un adversaire..."),m||(m=T.channel("matchmaking-"+x.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${x.profile.id}`},me=>{const _e=me.new;_e.status==="matched"&&_e.match_id&&!K&&(K=!0,j(_e.match_id,_e.matched_with,!0))}).subscribe()),V<N.length-1){const me=N[V+1].delay-N[V].delay;V++,setTimeout(()=>{!_&&!K&&oe()},me)}};await oe()}else{const{data:N,error:V}=await T.rpc("try_matchmake",{p_user_id:x.profile.id,p_deck_id:i});if(V||!(N!=null&&N.success)){h("Erreur de matchmaking","error"),Ve(e),f("home");return}if(N.matched){j(N.match_id,N.opponent_id,!1);return}const K=document.getElementById("mm-status");K&&(K.textContent="En attente d'un autre joueur..."),m=T.channel("matchmaking-"+x.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${x.profile.id}`},oe=>{const ie=oe.new;ie.status==="matched"&&ie.match_id&&j(ie.match_id,ie.matched_with,!0)}).subscribe()}}async function Ko(e,t,i){const{state:o,navigate:d,toast:s}=t;try{(T.getChannels?T.getChannels():[]).forEach(a=>{const n=a.topic||"";(n.includes("matchmaking")||n.includes("pvp-match")||n.includes("friend-invite"))&&T.removeChannel(a)})}catch{}const{data:r}=await T.from("matches").select("home_id, away_id, status").eq("id",i).single();if(!r){s("Match introuvable","error"),d("home");return}if(r.status==="finished"){s("Ce match est terminé","info"),d("home");return}const l=r.home_id===o.profile.id;gn(e,t,i,l,[],[])}async function gn(e,t,i,o,d=[],s=[],r=!1,l=null,u=null){const{state:a,navigate:n,toast:x}=t,f=o?"p1":"p2",h=o?"p2":"p1",_=(d||[]).map($=>$.id),m=(d||[]).map($=>({id:$.id,gc_type:$.gc_type,_gcDef:$._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:c}=await T.from("matches").select("*").eq("id",i).single();if(!c){x("Match introuvable","error"),n("home");return}async function A(){var W,te;console.log("[PvP] buildGameState match:",{id:c.id,home_deck_id:c.home_deck_id,away_deck_id:c.away_deck_id,mode:c.mode,is_ranked:c.is_ranked});const[{data:$,error:g},{data:y,error:b},{data:v},{data:w}]=await Promise.all([T.rpc("get_deck_for_match",{p_deck_id:c.home_deck_id}),T.rpc("get_deck_for_match",{p_deck_id:c.away_deck_id}),T.from("users").select("id,pseudo,club_name").eq("id",c.home_id).single(),T.from("users").select("id,pseudo,club_name").eq("id",c.away_id).single()]);console.log("[PvP] get_deck_for_match p1:",g==null?void 0:g.message,"p2:",b==null?void 0:b.message,"p1D:",(W=$==null?void 0:$.starters)==null?void 0:W.length,"p2D:",(te=y==null?void 0:y.starters)==null?void 0:te.length);const k=Z=>{const fe=Number(Z.evolution_bonus)||0;return{cardId:Z.card_id,position:Z.position,id:Z.id,firstname:Z.firstname,name:Z.surname_encoded,country_code:Z.country_code,club_id:Z.club_id,job:Z.job,job2:Z.job2,note_g:(Number(Z.note_g)||0)+(Z.job==="GK"||Z.job2==="GK"&&Number(Z.note_g)>0?fe:0),note_d:(Number(Z.note_d)||0)+(Z.job==="DEF"||Z.job2==="DEF"&&Number(Z.note_d)>0?fe:0),note_m:(Number(Z.note_m)||0)+(Z.job==="MIL"||Z.job2==="MIL"&&Number(Z.note_m)>0?fe:0),note_a:(Number(Z.note_a)||0)+(Z.job==="ATT"||Z.job2==="ATT"&&Number(Z.note_a)>0?fe:0),evolution_bonus:fe,rarity:Z.rarity,skin:Z.skin,hair:Z.hair,hair_length:Z.hair_length,clubName:Z.club_encoded_name||null,clubLogo:Z.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},z=(($==null?void 0:$.starters)||[]).map(Z=>k(Z)),E=((y==null?void 0:y.starters)||[]).map(Z=>k(Z)),L=($==null?void 0:$.formation)||"4-4-2",S=(y==null?void 0:y.formation)||"4-4-2";let M=vt(z,L),F=vt(E,S);const Y=(($==null?void 0:$.subs)||[]).map(Z=>k(Z)),J=((y==null?void 0:y.subs)||[]).map(Z=>k(Z)),U=($==null?void 0:$.stadium_def)||(o?u:null),G=(y==null?void 0:y.stadium_def)||(o?null:u);return U&&(M=Tt(M,U),jt(Y,U)),G&&(F=Tt(F,G),jt(J,G)),{p1Team:M,p2Team:F,p1Subs:Y,p2Subs:J,p1Formation:L,p2Formation:S,p1Name:(v==null?void 0:v.club_name)||(v==null?void 0:v.pseudo)||"Joueur 1",p2Name:(w==null?void 0:w.club_name)||(w==null?void 0:w.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:o?_:[],gc_p2:o?[]:_,gcCardsFull_p1:o?m:[],gcCardsFull_p2:o?[]:m,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:s||[],lastActionAt:new Date().toISOString()}}let p=c.game_state&&Object.keys(c.game_state).length?c.game_state:null;if(console.log("[PvP] init - amIHome:",o,"gameState exists:",!!p,"match.status:",c.status,"home_id:",c.home_id,"away_id:",c.away_id,"myId:",a.profile.id),!p)if(o){p=await A(),console.log("[PvP] home - gameState built, p1Team keys:",Object.keys((p==null?void 0:p.p1Team)||{}));const{data:$}=await T.from("matches").select("game_state").eq("id",i).single();!($!=null&&$.game_state)||!Object.keys($.game_state).length?await T.from("matches").update({game_state:p,turn_user_id:c.home_id}).eq("id",i):p=$.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>',console.log("[PvP] away - waiting for home to write game_state...");for(let $=0;$<30&&!p;$++){await new Promise(y=>setTimeout(y,400));const{data:g}=await T.from("matches").select("game_state").eq("id",i).single();g!=null&&g.game_state&&Object.keys(g.game_state).length&&(p=g.game_state),$%5===0&&console.log("[PvP] away - poll",$,"game_state:",!!(g!=null&&g.game_state))}if(!p){x("Erreur de synchronisation","error"),n("home");return}p.gc_p2=_,p.gcCardsFull_p2=m,await T.from("matches").update({game_state:p}).eq("id",i)}let I=!1,q=null,j=!1;const P=new Set,C=new Set;async function N($){var S,M;try{T.removeChannel(V)}catch{}const g=p[f+"Score"]||0,y=p[h+"Score"]||0;let b,v;$.winner_id?(b=$.winner_id===a.profile.id,v=!1):$.forfeit?(b=g>y,v=!1):(v=g===y,b=g>y);let w="";if(r&&o)try{const{data:F}=await T.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",c.home_id).single(),{data:Y}=await T.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",c.away_id).single();if(F&&Y){const J=b?1:v?.5:0,U=1-J,G=F.placement_matches<10,W=Y.placement_matches<10,te=Ii(F.mmr,F.mmr_deviation,F.mmr_volatility,Y.mmr,Y.mmr_deviation,J===1?1:J===0?0:.5,G),Z=Ii(Y.mmr,Y.mmr_deviation,Y.mmr_volatility,F.mmr,F.mmr_deviation,U===1?1:U===0?0:.5,W);await T.rpc("update_mmr_after_ranked",{p_match_id:i,p_winner_id:v?null:b?c.home_id:c.away_id,p_home_id:c.home_id,p_away_id:c.away_id,p_home_delta:te.delta,p_away_delta:Z.delta,p_home_new_rd:te.newRd,p_away_new_rd:Z.newRd,p_home_new_vol:te.newSigma,p_away_new_vol:Z.newSigma});const fe=te.delta,ye=te.newMmr,be=Nt(ye),ke=fe>=0?"+":"",Be=fe>=0?"#4caf50":"#ff6b6b",Fe=Nt(F.mmr);w=`
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ...
        </div>`}return`<div style="font-size:11px;color:${X.type==="goal"?"#FFD700":"rgba(255,255,255,0.65)"};font-weight:${X.type==="goal"?700:400};padding:3px 2px">${X.text||""}</div>`}const Ke=(()=>{var H,ae;if(z&&((H=p.pendingAttack)!=null&&H.players)){const he=p.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
      ...
      </div>`;function Ge(){const X=e.querySelector(".match-screen");if(!X)return;const H=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);X.style.height=H+"px",X.style.minHeight=H+"px",X.style.maxHeight=H+"px",X.style.overflow="hidden";const ae=e.querySelector("#mobile-action-bar"),he=e.querySelector("#mobile-play-area");ae&&he&&(he.style.paddingBottom=ae.offsetHeight+"px")}if(Ge(),setTimeout(Ge,120),setTimeout(Ge,400),j||(j=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",Ge),window.visualViewport.addEventListener("scroll",Ge)),window.addEventListener("resize",Ge)),function(){const H=e.querySelector(".terrain-wrapper svg");H&&(H.removeAttribute("width"),H.removeAttribute("height"),H.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",H.setAttribute("viewBox","-26 -26 352 352"),H.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),e.querySelectorAll(".match-slot-hit").forEach(X=>{X.addEventListener("click",()=>{if(!k&&!z)return;const H=X.dataset.cardId,ae=X.dataset.role,he=($[ae]||[]).find(tt=>tt.cardId===H);if(!he||he.used)return;const $e=fe.includes(H);if(k&&!["MIL","ATT"].includes(ae)&&!$e)return;Array.isArray(p["selected_"+f])||(p["selected_"+f]=[]);const Qe=p["selected_"+f],Ze=Qe.findIndex(tt=>tt.cardId===H);Ze>-1?Qe.splice(Ze,1):Qe.length<3&&Qe.push({...he,_role:ae}),me()})}),e.querySelectorAll(".match-used-hit").forEach(X=>{X.addEventListener("click",()=>O(X.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(X=>{X.addEventListener("click",()=>O())}),(We=e.querySelector("#pvp-sub-open"))==null||We.addEventListener("click",()=>O()),e.querySelectorAll(".pvp-gc-mini").forEach(X=>{X.addEventListener("click",()=>pe(X.dataset.gcId,X.dataset.gcType))}),(nt=e.querySelector("#pvp-boost-card"))==null||nt.addEventListener("click",()=>de()),(pt=e.querySelector("#pvp-action"))==null||pt.addEventListener("click",X=>{k?X.currentTarget.dataset.pass==="1"||!Te(f)?ge():ne():z&&ue()}),(ut=e.querySelector("#pvp-quit"))==null||ut.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&oe()}),(ft=e.querySelector("#pvp-view-opp"))==null||ft.addEventListener("click",()=>xe()),(ce=e.querySelector("#pvp-toggle-history"))==null||ce.addEventListener("click",()=>le()),q&&(clearInterval(q),q=null),(k||z)&&!I){let X=30,H=!1;const ae=()=>document.getElementById("pvp-timer"),he=()=>{ae()&&(ae().textContent=X+"s",ae().style.color=H?"#ff4444":"#fff")};he(),q=setInterval(()=>{X--,X<0?H?(clearInterval(q),q=null,k&&!Te(f)?ge():oe()):(H=!0,X=15,he()):he()},1e3)}}function _e(){et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(w);let E=!1;const L=()=>{E||(E=!0,w.remove(),setTimeout(()=>v(),50))};w.addEventListener("click",L),setTimeout(L,3500)}function se($,g,y){var U,G;const b=(p.gcDefs||[]).find(W=>{var te;return W.name===$||((te=W.name)==null?void 0:te.toLowerCase().trim())===($==null?void 0:$.toLowerCase().trim())}),v={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[b==null?void 0:b.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",w={purple:"#b06ce0",light_blue:"#00d4ef"}[b==null?void 0:b.color]||"#b06ce0",k=(b==null?void 0:b.name)||$,z=(b==null?void 0:b.effect)||((U=Oe[$])==null?void 0:U.desc)||"",E=b!=null&&b.image_url?`/icons/${b.image_url}`:null,L=((G=Oe[$])==null?void 0:G.icon)||"⚡",M=g===f?"Vous":p[g+"Name"]||"Adversaire",F=document.createElement("div");F.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",F.innerHTML=`
      <style>
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(v),(L=v.querySelector("#pvp-end-home"))==null||L.addEventListener("click",()=>{v.remove(),Ve(e),l("home")})}const N=T.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},B=>{const $=B.new;try{if($.status==="finished"||$.forfeit){if(c)return;c=!0,I&&(clearInterval(I),I=null),$.game_state&&(m=$.game_state),C($);return}if($.game_state){const g=m;m=$.game_state;const y=m._lastGC;if(y&&y.seq&&!P.has(y.seq)&&(P.add(y.seq),y.by!==a)){Ee(y.type,y.by,()=>re());return}const b=g[a+"Score"]||0,v=g[n+"Score"]||0,w=m[a+"Score"]||0,k=m[n+"Score"]||0,z=w>b,E=k>v;if((z||E)&&!j.has(m.round)){j.add(m.round);const L=[...m.log||[]].reverse().find(M=>M.isGoal),S=((L==null?void 0:L.homePlayers)||[]).map(M=>({name:M.name,note:M.note,portrait:M.portrait,job:M.job}));we(S,w,k,z,()=>re());return}re()}}catch(g){console.error("[PvP] crash:",g)}}).subscribe();async function V(B){Object.assign(m,B),m.lastActionAt=new Date().toISOString();const{error:$}=await T.from("matches").update({game_state:m}).eq("id",i);$&&u("Erreur de synchronisation","error");try{re()}catch(g){console.error("[PvP] renderPvpScreen crash:",g)}}async function K(){if(c)return;c=!0,I&&(clearInterval(I),I=null);const B=o?h.away_id:h.home_id,$=o?"p2":"p1",g=o?"p1":"p2",y={...m,[$+"Score"]:3,[g+"Score"]:0,phase:"finished"};try{await T.from("matches").update({status:"finished",forfeit:!0,winner_id:B,home_score:y.p1Score||0,away_score:y.p2Score||0,game_state:y}).eq("id",i)}catch{}try{T.removeChannel(N)}catch{}setTimeout(()=>{Ve(e),l("home")},800)}const oe={BOOST_STAT:({value:B=1,count:$=1,roles:g=[]},y,b)=>{const v=y[a+"Team"],w=Object.entries(v).filter(([k])=>!g.length||g.includes(k)).flatMap(([k,z])=>z.filter(E=>!E.used).map(E=>({...E,_line:k})));if(!w.length){y.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),b(y);return}ie(w,$,`Choisir ${$} joueur(s) à booster (+${B})`,k=>{k.forEach(z=>{const E=(v[z._line]||[]).find(L=>L.cardId===z.cardId);E&&(E.boost=(E.boost||0)+B,y.log.push({text:`⚡ +${B} sur ${E.name}`,type:"info"}))}),y[a+"Team"]=v,b(y)})},DEBUFF_STAT:({value:B=1,count:$=1,roles:g=[],target:y="ai"},b,v)=>{const w=y==="home"?a:n,k=b[w+"Team"],z=y==="home"?"allié":"adverse",E=Object.entries(k).filter(([L])=>!g.length||g.includes(L)).flatMap(([L,S])=>S.map(M=>({...M,_line:L})));if(!E.length){b.log.push({text:`🎯 Aucun joueur ${z}`,type:"info"}),v(b);return}ie(E,$,`Choisir ${$} joueur(s) ${z}(s) (-${B})`,L=>{L.forEach(S=>{const M=(k[S._line]||[]).find(F=>F.cardId===S.cardId);M&&(M.boost=(M.boost||0)-B,b.log.push({text:`🎯 -${B} sur ${M.name}`,type:"info"}))}),b[w+"Team"]=k,v(b)})},GRAY_PLAYER:({count:B=1,roles:$=[],target:g="ai"},y,b)=>{const v=g==="home"?a:n,w=y[v+"Team"],k=g==="home"?"allié":"adverse",z=Object.entries(w).filter(([E])=>!$.length||$.includes(E)).flatMap(([E,L])=>L.filter(S=>!S.used).map(S=>({...S,_line:E})));if(!z.length){y.log.push({text:`❌ Aucun joueur ${k}`,type:"info"}),b(y);return}ie(z,B,`Choisir ${B} joueur(s) ${k}(s) à exclure`,E=>{const L="usedCardIds_"+v,S=new Set(y[L]||[]);E.forEach(M=>{const F=(w[M._line]||[]).find(Y=>Y.cardId===M.cardId);F&&(F.used=!0,S.add(M.cardId),y.log.push({text:`❌ ${F.name} exclu !`,type:"info"}))}),y[L]=[...S],y[v+"Team"]=w,b(y)})},REVIVE_PLAYER:({count:B=1,roles:$=[]},g,y)=>{const b=g[a+"Team"],v=Object.entries(b).filter(([w])=>!$.length||$.includes(w)).flatMap(([w,k])=>k.filter(z=>z.used).map(z=>({...z,_line:w})));if(!v.length){g.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),y(g);return}ie(v,B,`Choisir ${B} joueur(s) à ressusciter`,w=>{w.forEach(k=>{const z=(b[k._line]||[]).find(E=>E.cardId===k.cardId);z&&(z.used=!1,g.log.push({text:`💫 ${z.name} ressuscité !`,type:"info"}))}),g[a+"Team"]=b,y(g)})},REMOVE_GOAL:({},B,$)=>{const g=n+"Score";B[g]>0?(B[g]--,B.log.push({text:"🚫 Dernier but annulé !",type:"info"})):B.log.push({text:"🚫 Aucun but à annuler",type:"info"}),$(B)},ADD_GOAL_DRAW:({},B,$)=>{B[a+"Score"]===B[n+"Score"]?(B[a+"Score"]++,B.log.push({text:"🎯 But bonus !",type:"info"})):B.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),$(B)},ADD_SUB:({value:B=1},$,g)=>{$.maxSubs=($.maxSubs||3)+B,$.log.push({text:`🔄 +${B} remplacement(s)`,type:"info"}),g($)},CUSTOM:({},B,$)=>$(B)};function ie(B,$,g,y){const b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let v=[];function w(){var z,E;const k=B.map(L=>{const S={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[L._line]||"#555",M=Se(L,L._line)+(L.boost||0),Y=v.find(U=>U.cardId===L.cardId)?"#FFD700":"rgba(255,255,255,0.25)",J=L.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${L.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${Y};background:${S};overflow:hidden;cursor:pointer;${J}">
        ...
        </div>`,(z=b.querySelector("#pp-close"))==null||z.addEventListener("click",()=>b.remove()),b.querySelectorAll(".pp-item").forEach(L=>{L.addEventListener("click",()=>{const S=L.dataset.cid,M=B.find(Y=>Y.cardId===S),F=v.findIndex(Y=>Y.cardId===S);M&&(F>-1?v.splice(F,1):v.length<$&&v.push(M),w())})}),(E=b.querySelector("#pp-confirm"))==null||E.addEventListener("click",()=>{b.remove(),y(v)})}w(),document.body.appendChild(b)}async function Q(B,$){const y=(m["gcCardsFull_"+a]||[]).find(k=>k.id===B),b=(y==null?void 0:y._gcDef)||(m.gcDefs||[]).find(k=>{var z;return k.name===$||((z=k.name)==null?void 0:z.toLowerCase().trim())===($==null?void 0:$.toLowerCase().trim())}),v=[...m["usedGc_"+a]||[],B],w={type:$,by:a,seq:(m._gcAnimSeq||0)+1};P.add(w.seq),Ee($,a,async()=>{if(b!=null&&b.effect_type&&b.effect_type!=="CUSTOM"){const z=oe[b.effect_type];if(z){const E={...m};z(b.effect_params||{},E,async L=>{L["usedGc_"+a]=v,L._lastGC=w,L._gcAnimSeq=w.seq,await V(L)});return}}const k={...m};switch($){case"Remplacement+":k.maxSubs=(k.maxSubs||3)+1,k.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const z=n+"Score";k[z]>0&&(k[z]--,k.log.push({text:"🚫 But annulé",type:"info"}));break}}k["usedGc_"+a]=v,k._lastGC=w,k._gcAnimSeq=w.seq,await V(k)})}function R(B,$){const g="usedCardIds_"+B,y=new Set(m[g]||[]);$.forEach(b=>y.add(b)),m[g]=[...y]}function D(){for(const B of["p1","p2"]){const $=new Set(m["usedCardIds_"+B]||[]),g=m[B+"Team"];if(g)for(const y of["GK","DEF","MIL","ATT"])(g[y]||[]).forEach(b=>{b.used=$.has(b.cardId)})}}function re(){var Ge,We,nt,pt,ut,ft;if(m.phase==="reveal")return me();if(m.phase==="midfield")return ve();if(m.phase==="finished")return ze();const B=m[a+"Team"],$=m[n+"Team"];D();const g=m[a+"Score"],y=m[n+"Score"],b=m[a+"Name"],v=m[n+"Name"],w=m.phase===a+"-attack",k=m.phase===a+"-defense",z=Array.isArray(m["selected_"+a])?m["selected_"+a]:[],E=z.map(ce=>ce.cardId),L=window.innerWidth>=700,S=m[a+"Subs"]||[],M=m["usedSubIds_"+a]||[],F=S.filter(ce=>!M.includes(ce.cardId)),Y=m["gcCardsFull_"+a]||[],J=m["usedGc_"+a]||[],U=Y.filter(ce=>!J.includes(ce.id)),G=m.boostOwner===a&&!m.boostUsed,W=!["GK","DEF","MIL","ATT"].some(ce=>($[ce]||[]).some(X=>!X.used)),te=[...B.MIL||[],...B.ATT||[]].filter(ce=>!ce.used),Z=w&&W&&te.length===0?[...B.GK||[],...B.DEF||[]].filter(ce=>!ce.used).map(ce=>ce.cardId):[];function fe(ce,X,H){var mt,kt;const ae=ce._gcDef,he={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ae==null?void 0:ae.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",$e={purple:"#b06ce0",light_blue:"#00d4ef"}[ae==null?void 0:ae.color]||"#b06ce0",Qe=(ae==null?void 0:ae.name)||ce.gc_type,Ze=(ae==null?void 0:ae.effect)||((mt=Oe[ce.gc_type])==null?void 0:mt.desc)||"",tt=ae!=null&&ae.image_url?`/icons/${ae.image_url}`:null,rt=((kt=Oe[ce.gc_type])==null?void 0:kt.icon)||"⚡",_t=Math.round(H*.22),$t=Math.round(H*.22),dt=H-_t-$t,gt=Qe.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${ce.id}" data-gc-type="${ce.gc_type}"
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${ct(B,m[a+"Formation"],Ae,E,300,300,Z)}
      </div>
  ...
  </div>`,(A=document.getElementById("ml-create-btn"))==null||A.addEventListener("click",()=>tr(e,t)),e.querySelectorAll(".ml-tab").forEach(p=>p.addEventListener("click",()=>zt(e,t,p.dataset.tab))),e.querySelectorAll("[data-league-id]").forEach(p=>p.addEventListener("click",()=>wt(e,t,p.dataset.leagueId))),e.querySelectorAll("[data-join]").forEach(p=>p.addEventListener("click",I=>{I.stopPropagation(),mn(e,t,p.dataset.join,p.dataset.type)})),e.querySelectorAll("[data-delete]").forEach(p=>p.addEventListener("click",I=>{I.stopPropagation(),wi(e,t,p.dataset.delete,p.dataset.name,i)}))}function ni(e,t,i=!1){var r,l;const o=e.creator_id===t,d=e.pot||0,s=((l=(r=e.mini_league_members)==null?void 0:r[0])==null?void 0:l.count)||0;return`<div data-league-id="${e.id}" style="background:#fff;border-radius:12px;padding:14px 16px;box-shadow:0 1px 6px rgba(0,0,0,0.08);cursor:pointer;margin-bottom:8px">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
  ...
  </div>`;let u="buy";const a=()=>{var m,c,A,p,I,q,j;return{name:(((m=document.getElementById("flt-name"))==null?void 0:m.value)||"").toLowerCase().trim(),club:(((c=document.getElementById("flt-club"))==null?void 0:c.value)||"").toLowerCase().trim(),country:(((A=document.getElementById("flt-country"))==null?void 0:A.value)||"").toLowerCase().trim(),job:((p=document.getElementById("flt-job"))==null?void 0:p.value)||"",rarity:((I=document.getElementById("flt-rarity"))==null?void 0:I.value)||"",note1:parseInt((q=document.getElementById("flt-note1"))==null?void 0:q.value)||0,note2:parseInt((j=document.getElementById("flt-note2"))==null?void 0:j.value)||0}};function n(m){const c=a();return m.filter(A=>{var V,K,oe;const p=(V=A.card)==null?void 0:V.player;if(!p)return!1;const I=`${p.firstname} ${p.surname_encoded}`.toLowerCase(),q=(((K=p.clubs)==null?void 0:K.encoded_name)||"").toLowerCase(),j=(p.country_code||"").toLowerCase(),P=((oe=A.card)==null?void 0:oe.evolution_bonus)||0,C=St(p,p.job,P),N=p.job2?St(p,p.job2,P):0;return!(c.name&&!I.includes(c.name)||c.club&&!q.includes(c.club)||c.country&&!j.includes(c.country)||c.job&&p.job!==c.job||c.rarity&&p.rarity!==c.rarity||c.note1&&C<c.note1||c.note2&&N<c.note2)})}function x(m){var N,V,K,oe;const c=(N=m.card)==null?void 0:N.player;if(!c)return"";const A=((V=m.card)==null?void 0:V.evolution_bonus)||0,p=St(c,c.job,A),I=c.job2?St(c,c.job2,A):0,q=(i.profile.credits||0)>=m.price,j=c.country_code?`https://flagsapi.com/${c.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,P=Qt[c.job]||"#bbb",C=c.job2?Qt[c.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden">
      <!-- Drapeau -->
    ...
    </div>`}function f(m){var N,V,K,oe;const c=(N=m.card)==null?void 0:N.player;if(!c)return"";const A=((V=m.card)==null?void 0:V.evolution_bonus)||0,p=St(c,c.job,A),I=c.job2?St(c,c.job2,A):0,q=m.status==="sold",j=c.country_code?`https://flagsapi.com/${c.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,P=Qt[c.job]||"#bbb",C=c.job2?Qt[c.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${q?"opacity:0.7":""}">
      ${j?`<img src="${j}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      ${(K=c.clubs)!=null&&K.logo_url?`<img src="${c.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${P};display:flex;align-items:center;justify-content:center">
    ...
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center">
      ...
      `).join(""):'<div style="text-align:center;color:var(--gray-600);padding:40px">Aucun manager.</div>'}else{const A=r||[];_.innerHTML=A.length>0?A.map((p,I)=>{const q=Nt(p.mmr??1e3),j=(p.ranked_wins||0)+(p.ranked_losses||0)+(p.ranked_draws||0),P=j>0?Math.round((p.ranked_wins||0)/j*100):0,C=p.id===i.profile.id,N=(p.placement_matches||0)<10;return`
          <div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${C?"border:2px solid var(--yellow)":""}">
            ...
            <div style="text-align:right;flex-shrink:0">
              <div style="font-size:20px">${N?"❓":q.emoji}</div>
              <div style="font-size:11px;font-weight:700;color:${q.color}">${N?"Placement":q.label}</div>
