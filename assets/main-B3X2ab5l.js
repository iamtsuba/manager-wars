...
import{s as v,F as ai,r as qe,j as Ut,l as It,m as fn,h as si,n as gn,o as mn,T as Yt,k as xn,p as bn,b as yn}from"./formation-links-C5ScSaLv.js";const hn="/";function Li(e,{navigate:t,toast:i}){let n="login";const a=()=>{var r,s,c,l,d,u;const o=n==="login";e.innerHTML=`
  ...
  `;function s(){var g;const l=document.getElementById("logo-preview"),d=document.getElementById("logo-initials"),u=((g=document.getElementById("setup-club"))==null?void 0:g.value)||"MW",f=u.trim().split(" ").filter(Boolean),x=f.length>=2?(f[0][0]+f[1][0]).toUpperCase():u.slice(0,2).toUpperCase();l&&(l.style.background=r,l.style.borderColor=o),d&&(d.textContent=x,d.style.color=o)}document.getElementById("color1").addEventListener("input",l=>{o=l.target.value,document.getElementById("swatch1").style.background=o,s()}),document.getElementById("color2").addEventListener("input",l=>{r=l.target.value,document.getElementById("swatch2").style.background=r,s()});function c(l){document.querySelectorAll(".setup-step").forEach(d=>d.classList.remove("active")),document.getElementById(`step-${l}`).classList.add("active"),document.getElementById("step-num").textContent=l,document.getElementById("progress-fill").style.width=`${Math.round(l/3*100)}%`,l===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const l=document.getElementById("setup-pseudo").value.trim(),d=document.getElementById("step1-error");if(d.textContent="",l.length<3){d.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:u}=await v.from("users").select("id").eq("pseudo",l).maybeSingle();if(u){d.textContent="Ce pseudo est déjà pris.";return}c(2)}),document.getElementById("step2-back").addEventListener("click",()=>c(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const l=document.getElementById("setup-club").value.trim(),d=document.getElementById("step2-error");if(d.textContent="",l.length<2){d.textContent="Nom trop court (min. 2 caractères).";return}const{data:u}=await v.from("users").select("id").eq("club_name",l).maybeSingle();if(u){d.textContent="Ce nom de club est déjà pris.";return}c(3)}),document.getElementById("step3-back").addEventListener("click",()=>c(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const l=document.getElementById("setup-pseudo").value.trim(),d=document.getElementById("setup-club").value.trim(),u=document.getElementById("step3-error"),f=document.getElementById("step3-finish");u.textContent="",f.disabled=!0,f.textContent="Création en cours…";try{const{error:x}=await v.from("users").insert({id:t.user.id,pseudo:l,club_name:d,club_color1:o,club_color2:r,credits:1e4});if(x)throw x;await wn(t.user.id),await a(),n(`Bienvenue ${l} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(x){u.textContent=x.message,f.disabled=!1,f.textContent="🚀 Créer mon profil !"}})}async function wn(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await v.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const _n="modulepreload",kn=function(e){return"/"+e},Ti={},Pi=function(t,i,n){let a=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const r=document.querySelector("meta[property=csp-nonce]"),s=(r==null?void 0:r.nonce)||(r==null?void 0:r.getAttribute("nonce"));a=Promise.allSettled(i.map(c=>{if(c=kn(c),c in Ti)return;Ti[c]=!0;const l=c.endsWith(".css"),d=l?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${c}"]${d}`))return;const u=document.createElement("link");if(u.rel=l?"stylesheet":_n,l||(u.as="script"),u.crossOrigin="",u.href=c,s&&u.setAttribute("nonce",s),document.head.appendChild(u),l)return new Promise((f,x)=>{u.addEventListener("load",f),u.addEventListener("error",()=>x(new Error(`Unable to preload CSS for ${c}`)))})}))}function o(r){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=r,window.dispatchEvent(s),!s.defaultPrevented)throw r}return a.then(r=>{for(const s of r||[])s.status==="rejected"&&o(s.reason);return t().catch(o)})},$t="#1A6B3C",Et="#cc2222",$n="#D4A017",zi="#888";async function En(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    </style>`}const In="2026.07.13-0938";async function Ii(e,{state:t,navigate:i,toast:n}){var o,r;const a=t.profile;a&&(e.innerHTML=`
  <style>
    .home-dark {
      min-height: 100%;
  ...
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
  ...
  </div>`;function I(){const T=document.getElementById("col-filters");T&&(R==="player"?(T.innerHTML=`
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",M=>{G=M.target.value.toLowerCase(),k()}),e.querySelectorAll(".filter-btn").forEach(M=>{M.addEventListener("click",()=>{L=M.dataset.job,I(),k()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{O=!O,I(),k()})):(T.innerHTML=`
        ...
        </div>`},null)}e.querySelectorAll(".col-tab-btn").forEach(T=>{T.addEventListener("click",()=>{R=T.dataset.tab,L="Tous",G="",O=!1,e.querySelectorAll(".col-tab-btn").forEach(M=>{const P=M.dataset.tab===R;M.style.borderBottomColor=P?"var(--green)":"transparent",M.style.color=P?"var(--green)":"var(--gray-600)"}),I(),k()})}),I(),k()}function Un(e,t,i){const n=Ae[e]||{icon:"⚡",desc:"Effet spécial."},a=(t==null?void 0:t.gc_type)==="ultra_game_changer",o={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},r={purple:"#b06ce0",light_blue:"#00d4ef"},s=o[t==null?void 0:t.color]||o.purple,c=r[t==null?void 0:t.color]||r.purple,l=(t==null?void 0:t.name)||e,d=(t==null?void 0:t.effect)||n.desc,u=t!=null&&t.image_url?`/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(pe=document.getElementById("close-detail"))==null||pe.addEventListener("click",s);let k=new Set;const D=()=>{const K=k.size,Q=document.getElementById("sell-action-panel");if(!Q)return;Q.style.display=K>0?"block":"none",document.getElementById("sell-selected-count").textContent=K;const V=document.getElementById("evolve-btn");V&&(V.textContent=`⬆️ Évoluer ${K>1?"(+"+K+")":""}`)};document.querySelectorAll(".expl-check").forEach(K=>{K.addEventListener("change",()=>{const Q=K.dataset.id;K.checked?k.add(Q):k.delete(Q);const V=K.closest(".exemplaire-row");V&&(V.style.borderColor=K.checked?"#1A6B3C":"#eee"),D()})}),document.querySelectorAll(".exemplaire-row").forEach(K=>{K.addEventListener("click",Q=>{if(Q.target.closest("button")||Q.target.tagName==="INPUT")return;const V=K.querySelector(".expl-check");V&&!V.disabled&&(V.checked=!V.checked,V.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(K=>{K.addEventListener("click",Q=>{Q.stopPropagation();const V=document.querySelector(`.expl-hist[data-hist="${K.dataset.idx}"]`);V&&(V.style.display=V.style.display==="none"?"flex":"none")})}),(le=document.getElementById("evolve-btn"))==null||le.addEventListener("click",async()=>{if(f<=1)return;const K=[...k];if(!K.length)return;if(k.has(e.id)){const U=document.createElement("div");U.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",U.innerHTML=`
  ...
  <div class="page">
    ...
    <div class="page-body">
      ...
      <button style="margin-top:10px;padding:12px 28px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer" id="anim-close-err">Fermer</button>`,document.body.appendChild(I),(te=I.querySelector("#anim-close-err"))==null||te.addEventListener("click",()=>I.remove());return}e=[...e].sort((I,k)=>{const D=I.player?Ye(I.player):-1;return(k.player?Ye(k.player):-1)-D});const a=document.createElement("div");a.id="booster-anim-overlay",a.innerHTML=`
    <style>
  ...
  `,document.body.appendChild(a);let o=!1;const r=document.getElementById("pack-cut-zone"),s=document.getElementById("pack-blade");let c=!1;const l=I=>I.touches&&I.touches[0]?I.touches[0].clientX:I.clientX;function d(I){o||(c=!0,s.style.opacity="1",u(I))}function u(I){if(!c||o)return;const k=r.getBoundingClientRect(),D=l(I)-k.left,ae=Math.max(0,Math.min(1,D/k.width));s.style.width=ae*k.width+"px",ae>=.82&&x()}function f(){o||(c=!1,s.style.transition="width .2s ease, opacity .2s ease",s.style.width="0",s.style.opacity="0",setTimeout(()=>{o||(s.style.transition="")},220))}function x(){var k;if(o)return;o=!0,c=!1,s.style.width="100%",s.style.opacity="1",(k=document.getElementById("cut-flash"))==null||k.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const I=document.getElementById("cut-hint");I&&(I.style.opacity="0"),r.classList.add("pack-cut"),setTimeout(()=>{s.style.opacity="0",document.getElementById("pack-phase").style.display="none",y(0)},620)}r.addEventListener("pointerdown",d),window.addEventListener("pointermove",u),window.addEventListener("pointerup",f),r.addEventListener("touchstart",d,{passive:!0}),window.addEventListener("touchmove",u,{passive:!0}),window.addEventListener("touchend",f);let g=0,p=!1;const m=new Set;function y(I){g=I,document.getElementById("reveal-phase").style.display="flex",h(),w(I,0),G()}function h(){const I=document.getElementById("card-dots");I&&(I.innerHTML=e.map((k,D)=>`<div class="card-dot" data-i="${D}" style="width:8px;height:8px;border-radius:50%;background:${D===g?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),I.querySelectorAll(".card-dot").forEach(k=>k.addEventListener("click",()=>b(parseInt(k.dataset.i)))))}function w(I,k){var V,T;const D=e[I],ae=document.getElementById("card-counter"),pe=document.getElementById("card-track");ae&&(ae.textContent=`Carte ${I+1} / ${e.length}`);const le=document.getElementById("reveal-btns");le&&(le.style.display=I===e.length-1?"flex":"none");const se=D.card_type==="player"&&((V=D.player)==null?void 0:V.rarity)==="legende",ke=!m.has(I);m.add(I);let K=0;if(D.card_type==="player"&&D.player){const M=D.player,P=M.job||"ATT";K=Number(P==="GK"?M.note_g:P==="DEF"?M.note_d:P==="MIL"?M.note_m:M.note_a)||0}const Q=M=>{pe.innerHTML=`
        ...
        ${Q}
      </div>`,T&&navigator.vibrate&&navigator.vibrate([50,30,100,30,200])},(C+N)*3),setTimeout(U,(C+N)*3+C),setTimeout(()=>{M.style.display="none",P.innerHTML="",p=!1,H&&!V&&H.pause(),navigator.vibrate&&navigator.vibrate([40,30,80]),k()},(C+N)*4)}function b(I){if(p||I<0||I>=e.length||I===g)return;const k=I>g?1:-1;g=I,w(I,k)}function R(){b(g+1)}function L(){b(g-1)}function G(){const I=document.getElementById("card-viewport");if(!I||I._swipeBound)return;I._swipeBound=!0;let k=0,D=0,ae=0,pe=!1;const le=V=>V.touches?V.touches[0].clientX:V.clientX,se=V=>V.touches?V.touches[0].clientY:V.clientY,ke=V=>{pe=!0,k=le(V),D=se(V),ae=0},K=V=>{if(!pe)return;ae=le(V)-k;const T=se(V)-D;if(Math.abs(ae)<Math.abs(T))return;const M=document.getElementById("current-card-wrap");M&&(M.style.transition="none",M.style.transform=`translateX(${ae*.6}px) rotate(${ae*.02}deg)`)},Q=()=>{if(!pe)return;pe=!1;const V=document.getElementById("current-card-wrap"),T=55;ae<=-T&&g<e.length-1?R():ae>=T&&g>0?L():V&&(V.style.transition="transform .2s ease",V.style.transform="translateX(0)")};I.addEventListener("pointerdown",ke),I.addEventListener("pointermove",K),I.addEventListener("pointerup",Q),I.addEventListener("pointercancel",Q),I.addEventListener("touchstart",ke,{passive:!0}),I.addEventListener("touchmove",K,{passive:!0}),I.addEventListener("touchend",Q),I.addEventListener("click",V=>{if(Math.abs(ae)>8)return;const T=I.getBoundingClientRect();V.clientX-T.left>T.width/2?R():L()})}let O=null;function ee(){const I=document.getElementById("fireworks-canvas");if(!I)return;I.width=window.innerWidth,I.height=window.innerHeight;const k=I.getContext("2d"),D=[];function ae(){const le=Math.random()*I.width,se=Math.random()*I.height*.6,ke=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],K=ke[Math.floor(Math.random()*ke.length)];for(let Q=0;Q<60;Q++){const V=Math.PI*2/60*Q,T=2+Math.random()*5;D.push({x:le,y:se,vx:Math.cos(V)*T,vy:Math.sin(V)*T,alpha:1,color:K,size:2+Math.random()*3})}}ae(),O=setInterval(ae,600);function pe(){if(document.getElementById("fireworks-canvas")){k.clearRect(0,0,I.width,I.height);for(let le=D.length-1;le>=0;le--){const se=D[le];if(se.x+=se.vx,se.y+=se.vy+.08,se.vy*=.98,se.alpha-=.018,se.alpha<=0){D.splice(le,1);continue}k.globalAlpha=se.alpha,k.fillStyle=se.color,k.beginPath(),k.arc(se.x,se.y,se.size,0,Math.PI*2),k.fill()}k.globalAlpha=1,(O!==null||D.length>0)&&requestAnimationFrame(pe)}}pe()}function oe(){O!==null&&(clearInterval(O),O=null);const I=document.getElementById("fireworks-canvas");I&&I.getContext("2d").clearRect(0,0,I.width,I.height)}if(n){const I=document.getElementById("reveal-btns");I&&(I.innerHTML='<button class="btn btn-primary" id="reveal-next" style="flex:1">Continuer →</button>'),(j=document.getElementById("reveal-next"))==null||j.addEventListener("click",()=>{oe(),a.remove(),n()})}else document.getElementById("reveal-collection").addEventListener("click",()=>{oe(),a.remove(),i("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{oe(),a.remove(),i("boosters")})}function co(e){var t,i,n,a;if(e.card_type==="player"&&e.player)return oo(e);if(e.card_type==="game_changer"){const o=e._gcDef,r=(o==null?void 0:o.gc_type)==="ultra_game_changer",s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},c={purple:"#b06ce0",light_blue:"#00d4ef"},l=s[o==null?void 0:o.color]||s.purple,d=c[o==null?void 0:o.color]||c.purple,u=(o==null?void 0:o.name)||e.gc_type||"Game Changer",f=(o==null?void 0:o.effect)||((t=ti[e.gc_type])==null?void 0:t.desc)||"",x=o!=null&&o.image_url?`/icons/${o.image_url}`:null,g=((i=ti[e.gc_type])==null?void 0:i.icon)||"⚡";return`<div style="width:170px;height:240px;background:${l};border-radius:14px;border:3px solid ${d};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${d}88;flex-shrink:0">
  ...
  <div id="match-history-panel">
    ...
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      ...
      ${zt(e.aiTeam,e.formation,null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.1))}
            ...
            </div>`}}catch(J){console.error("[Ranked] MMR update error:",J)}(X=document.getElementById("pvp-end-overlay"))==null||X.remove();const B=document.createElement("div");B.id="pvp-end-overlay",B.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;color:#fff;padding:24px;text-align:center;overflow-y:auto";const F=z?"🤝":S?"🏆":"😞",Y=z?"MATCH NUL":S?"VICTOIRE !":"DÉFAITE",W=z?"#fff":S?"#FFD700":"#ff6b6b";B.innerHTML=`
      ...
      ${q}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${r?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(B),(re=B.querySelector("#pvp-end-home"))==null||re.addEventListener("click",()=>{B.remove(),Ue(e),x(r?"ranked":"home")})}const te=v.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},_=>{const E=_.new;try{if(E.status==="finished"||E.forfeit){if(R)return;R=!0,L&&(clearInterval(L),L=null),E.game_state&&(b=E.game_state),oe(E);return}if(E.game_state){const A=b;b=E.game_state;const S=b._lastGC;if(S&&S.seq&&!ee.has(S.seq)&&(ee.add(S.seq),S.by!==p)){V(S.type,S.by,()=>se());return}const z=A[p+"Score"]||0,q=A[m+"Score"]||0,B=b[p+"Score"]||0,F=b[m+"Score"]||0,Y=B>z,W=F>q;if((Y||W)&&!O.has(b.round)){O.add(b.round);const X=[...b.log||[]].reverse().find(J=>J.isGoal),re=((X==null?void 0:X.homePlayers)||[]).map(J=>({name:J.name,note:J.note,portrait:J.portrait,job:J.job}));at(re,B,F,Y,()=>se());return}se()}}catch(A){console.error("[PvP] crash:",A)}}).subscribe();async function j(_){Object.assign(b,_),b.lastActionAt=new Date().toISOString();const{error:E}=await v.from("matches").update({game_state:b}).eq("id",i);E&&g("Erreur de synchronisation","error");try{se()}catch(A){console.error("[PvP] renderPvpScreen crash:",A)}}async function I(){if(R)return;R=!0,L&&(clearInterval(L),L=null);const _=n?w.away_id:w.home_id,E=n?"p2":"p1",A=n?"p1":"p2",S={...b,[E+"Score"]:3,[A+"Score"]:0,phase:"finished"};try{await v.from("matches").update({status:"finished",forfeit:!0,winner_id:_,home_score:S.p1Score||0,away_score:S.p2Score||0,game_state:S}).eq("id",i)}catch{}try{v.removeChannel(te)}catch{}setTimeout(()=>{Ue(e),x("home")},800)}const k={BOOST_STAT:({value:_=1,count:E=1,roles:A=[]},S,z)=>{const q=S[p+"Team"],B=Object.entries(q).filter(([F])=>!A.length||A.includes(F)).flatMap(([F,Y])=>Y.filter(W=>!W.used).map(W=>({...W,_line:F})));if(!B.length){S.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),z(S);return}D(B,E,`Choisir ${E} joueur(s) à booster (+${_})`,F=>{F.forEach(Y=>{const W=(q[Y._line]||[]).find(X=>X.cardId===Y.cardId);W&&(W.boost=(W.boost||0)+_,S.log.push({text:`⚡ +${_} sur ${W.name}`,type:"info"}))}),S[p+"Team"]=q,z(S)})},DEBUFF_STAT:({value:_=1,count:E=1,roles:A=[],target:S="ai"},z,q)=>{const B=S==="home"?p:m,F=z[B+"Team"],Y=S==="home"?"allié":"adverse",W=Object.entries(F).filter(([X])=>!A.length||A.includes(X)).flatMap(([X,re])=>re.map(J=>({...J,_line:X})));if(!W.length){z.log.push({text:`🎯 Aucun joueur ${Y}`,type:"info"}),q(z);return}D(W,E,`Choisir ${E} joueur(s) ${Y}(s) (-${_})`,X=>{X.forEach(re=>{const J=(F[re._line]||[]).find(ce=>ce.cardId===re.cardId);J&&(J.boost=(J.boost||0)-_,z.log.push({text:`🎯 -${_} sur ${J.name}`,type:"info"}))}),z[B+"Team"]=F,q(z)})},GRAY_PLAYER:({count:_=1,roles:E=[],target:A="ai"},S,z)=>{const q=A==="home"?p:m,B=S[q+"Team"],F=A==="home"?"allié":"adverse",Y=Object.entries(B).filter(([W])=>!E.length||E.includes(W)).flatMap(([W,X])=>X.filter(re=>!re.used).map(re=>({...re,_line:W})));if(!Y.length){S.log.push({text:`❌ Aucun joueur ${F}`,type:"info"}),z(S);return}D(Y,_,`Choisir ${_} joueur(s) ${F}(s) à exclure`,W=>{const X="usedCardIds_"+q,re=new Set(S[X]||[]);W.forEach(J=>{const ce=(B[J._line]||[]).find(de=>de.cardId===J.cardId);ce&&(ce.used=!0,re.add(J.cardId),S.log.push({text:`❌ ${ce.name} exclu !`,type:"info"}))}),S[X]=[...re],S[q+"Team"]=B,z(S)})},REVIVE_PLAYER:({count:_=1,roles:E=[]},A,S)=>{const z=A[p+"Team"],q=Object.entries(z).filter(([B])=>!E.length||E.includes(B)).flatMap(([B,F])=>F.filter(Y=>Y.used).map(Y=>({...Y,_line:B})));if(!q.length){A.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),S(A);return}D(q,_,`Choisir ${_} joueur(s) à ressusciter`,B=>{B.forEach(F=>{const Y=(z[F._line]||[]).find(W=>W.cardId===F.cardId);Y&&(Y.used=!1,A.log.push({text:`💫 ${Y.name} ressuscité !`,type:"info"}))}),A[p+"Team"]=z,S(A)})},REMOVE_GOAL:({},_,E)=>{const A=m+"Score";_[A]>0?(_[A]--,_.log.push({text:"🚫 Dernier but annulé !",type:"info"})):_.log.push({text:"🚫 Aucun but à annuler",type:"info"}),E(_)},ADD_GOAL_DRAW:({},_,E)=>{_[p+"Score"]===_[m+"Score"]?(_[p+"Score"]++,_.log.push({text:"🎯 But bonus !",type:"info"})):_.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),E(_)},ADD_SUB:({value:_=1},E,A)=>{E.maxSubs=(E.maxSubs||3)+_,E.log.push({text:`🔄 +${_} remplacement(s)`,type:"info"}),A(E)},CUSTOM:({},_,E)=>E(_)};function D(_,E,A,S){const z=document.createElement("div");z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let q=[];function B(){var Y,W;const F=_.map(X=>{const re={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[X._line]||"#555",J=Be(X,X._line)+(X.boost||0),de=q.find(ue=>ue.cardId===X.cardId)?"#FFD700":"rgba(255,255,255,0.25)",ne=X.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${X.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${de};background:${re};overflow:hidden;cursor:pointer;${ne}">
        ...
        </div>`,(Y=z.querySelector("#pp-close"))==null||Y.addEventListener("click",()=>z.remove()),z.querySelectorAll(".pp-item").forEach(X=>{X.addEventListener("click",()=>{const re=X.dataset.cid,J=_.find(de=>de.cardId===re),ce=q.findIndex(de=>de.cardId===re);J&&(ce>-1?q.splice(ce,1):q.length<E&&q.push(J),B())})}),(W=z.querySelector("#pp-confirm"))==null||W.addEventListener("click",()=>{z.remove(),S(q)})}B(),document.body.appendChild(z)}async function ae(_,E){const S=(b["gcCardsFull_"+p]||[]).find(F=>F.id===_),z=(S==null?void 0:S._gcDef)||(b.gcDefs||[]).find(F=>{var Y;return F.name===E||((Y=F.name)==null?void 0:Y.toLowerCase().trim())===(E==null?void 0:E.toLowerCase().trim())}),q=[...b["usedGc_"+p]||[],_],B={type:E,by:p,seq:(b._gcAnimSeq||0)+1};ee.add(B.seq),V(E,p,async()=>{if(z!=null&&z.effect_type&&z.effect_type!=="CUSTOM"){const Y=k[z.effect_type];if(Y){const W={...b};Y(z.effect_params||{},W,async X=>{X["usedGc_"+p]=q,X._lastGC=B,X._gcAnimSeq=B.seq,await j(X)});return}}const F={...b};switch(E){case"Remplacement+":F.maxSubs=(F.maxSubs||3)+1,F.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const Y=m+"Score";F[Y]>0&&(F[Y]--,F.log.push({text:"🚫 But annulé",type:"info"}));break}}F["usedGc_"+p]=q,F._lastGC=B,F._gcAnimSeq=B.seq,await j(F)})}function pe(_,E){const A="usedCardIds_"+_,S=new Set(b[A]||[]);E.forEach(z=>S.add(z)),b[A]=[...S]}function le(){for(const _ of["p1","p2"]){const E=new Set(b["usedCardIds_"+_]||[]),A=b[_+"Team"];if(A)for(const S of["GK","DEF","MIL","ATT"])(A[S]||[]).forEach(z=>{z.used=E.has(z.cardId)})}}function se(){var Ve,it,bi,yi,hi,vi;if(b.phase==="reveal")return ke();if(b.phase==="midfield")return Q();if(b.phase==="finished")return Fe();const _=b[p+"Team"],E=b[m+"Team"];le();const A=b[p+"Score"],S=b[m+"Score"],z=b[p+"Name"],q=b[m+"Name"],B=b.phase===p+"-attack",F=b.phase===p+"-defense",Y=Array.isArray(b["selected_"+p])?b["selected_"+p]:[],W=Y.map(ie=>ie.cardId),X=window.innerWidth>=700,re=b[p+"Subs"]||[],J=b["usedSubIds_"+p]||[],ce=re.filter(ie=>!J.includes(ie.cardId)),de=b["gcCardsFull_"+p]||[],ne=b["usedGc_"+p]||[],ue=de.filter(ie=>!ne.includes(ie.id)),fe=b.boostOwner===p&&!b.boostUsed,xe=!["GK","DEF","MIL","ATT"].some(ie=>(E[ie]||[]).some(ge=>!ge.used)),Z=[..._.MIL||[],..._.ATT||[]].filter(ie=>!ie.used),ye=B&&xe&&Z.length===0?[..._.GK||[],..._.DEF||[]].filter(ie=>!ie.used).map(ie=>ie.cardId):[];function ze(ie,ge,Ie){var $i,Ei;const be=ie._gcDef,nt={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[be==null?void 0:be.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",dt={purple:"#b06ce0",light_blue:"#00d4ef"}[be==null?void 0:be.color]||"#b06ce0",yt=(be==null?void 0:be.name)||ie.gc_type,ht=(be==null?void 0:be.effect)||(($i=Ae[ie.gc_type])==null?void 0:$i.desc)||"",wi=be!=null&&be.image_url?`/icons/${be.image_url}`:null,pn=((Ei=Ae[ie.gc_type])==null?void 0:Ei.icon)||"⚡",_i=Math.round(Ie*.22),ki=Math.round(Ie*.22),Vt=Ie-_i-ki,un=yt.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${ie.id}" data-gc-type="${ie.gc_type}"
    ...
    </div>`,p==="p1"&&setTimeout(async()=>{await j({phase:"midfield"})},5e3)}let K=!1;function Q(){if(K)return;const _=b[p+"Team"].MIL||[],E=b[m+"Team"].MIL||[],A=milScore(_)+milLinks(_),S=milScore(E)+milLinks(E),z=A>=S;function q(de,ne,ue){return`<div id="duel-row-${ue}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      </div>`,document.body.appendChild(E),(A=E.querySelector("#pvp-hist-close"))==null||A.addEventListener("click",()=>E.remove())}async function N(){if(b.phase!==p+"-attack")return;const _=p==="p1"?"p2":"p1",E=(b.round||0)+1,A=[...b.log||[]];A.push({type:"info",text:`⏭️ ${b[p+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const S=he(b),z=$e(_),q=S||!z?"finished":_+"-attack";await j({["selected_"+p]:[],modifiers:{...b.modifiers,[p]:{}},pendingAttack:null,phase:q,attacker:_,round:E,log:A}),q==="finished"&&await Ce(b)}async function Ee(){const _=b[p+"Team"],E=!["GK","DEF","MIL","ATT"].some(q=>(b[m+"Team"][q]||[]).some(B=>!B.used)),A=(b["selected_"+p]||[]).map(q=>{const B=(_[q._role]||[]).find(de=>de.cardId===q.cardId)||q,F=E&&["GK","DEF"].includes(q._role),Y=_[q._role]||[],W=Y.findIndex(de=>de.cardId===q.cardId),X=mt(Y.length),re=W>=0?X[W]:B._col??1,J=b.stadiumDef||b.homeStadiumDef||null,ce=B.stadiumBonus||J&&(J.club_id&&String(B.club_id)===String(J.club_id)||J.country_code&&B.country_code===J.country_code)||!1;return{...B,_line:q._role,_col:re,stadiumBonus:ce,...F?{note_a:Math.max(1,Number(B.note_a)||0)}:{}}});if(!A.length)return;const S=ci(A,b.modifiers[p]||{});pe(p,A.map(q=>q.cardId)),A.forEach(q=>{const B=(_[q._role]||[]).find(F=>F.cardId===q.cardId);B&&(B.used=!0)}),b["selected_"+p]=[],se();const z=[...b.log||[]];if(E){const q=(b[p+"Score"]||0)+1,B=A.map(re=>histPlayer(re));z.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:B,homeTotal:S.total,aiTotal:0});const F=(b.round||0)+1,Y=p==="p1"?"p2":"p1",W={...b,[p+"Team"]:_,[p+"Score"]:q},X=he(W);O.add(F),at(B,q,b[m+"Score"]||0,!0,async()=>{await j({[p+"Team"]:_,[p+"Score"]:q,["selected_"+p]:[],modifiers:{...b.modifiers,[p]:{}},pendingAttack:null,phase:X?"finished":Y+"-attack",attacker:Y,round:F,log:z}),X&&await Ce({...b,[p+"Score"]:q})});return}z.push({type:"pending",text:`⚔️ ${b[p+"Name"]} attaque (${S.total})`}),await j({[p+"Team"]:_,[m+"Team"]:b[m+"Team"],pendingAttack:{...S,players:A,side:p},["selected_"+p]:[],modifiers:{...b.modifiers,[p]:{}},phase:m+"-defense",log:z})}async function _e(){const _=b[p+"Team"],E=(b["selected_"+p]||[]).map(ne=>{const ue=(_[ne._role]||[]).find(ze=>ze.cardId===ne.cardId)||ne,fe=_[ne._role]||[],xe=fe.findIndex(ze=>ze.cardId===ne.cardId),Z=mt(fe.length),ye=xe>=0?Z[xe]:ue._col??1;return{...ue,_line:ne._role,_col:ye}}),A=pi(E,b.modifiers[p]||{});pe(p,E.map(ne=>ne.cardId)),E.forEach(ne=>{const ue=(_[ne._role]||[]).find(fe=>fe.cardId===ne.cardId);ue&&(ue.used=!0)}),b["selected_"+p]=[],se();const S=ui(b.pendingAttack.total,A.total,b.modifiers[p]||{}),z=b.pendingAttack.side,q=S==="attack"||(S==null?void 0:S.goal),B=z==="p1"?"p2":"p1",F=(b.round||0)+1,Y=(b.pendingAttack.players||[]).map(ne=>histPlayer(ne)),W=[...b.log||[]];W.push({type:"duel",isGoal:q,homeScored:q&&z===p,text:q?`⚽ BUT de ${b[z+"Name"]} ! (${b.pendingAttack.total} vs ${A.total})`:`✋ Attaque stoppée (${b.pendingAttack.total} vs ${A.total})`,homePlayers:Y,aiPlayers:E.map(ne=>histPlayer(ne)),homeTotal:b.pendingAttack.total,aiTotal:A.total});const X=q?(b[z+"Score"]||0)+1:b[z+"Score"]||0,re={...b,[p+"Team"]:_,[z+"Score"]:X},J=he(re),ce=J?"finished":B+"-attack",de=async()=>{await j({[p+"Team"]:_,[m+"Team"]:b[m+"Team"],[z+"Score"]:X,["selected_"+p]:[],modifiers:{...b.modifiers,[p]:{}},pendingAttack:null,phase:ce,attacker:B,round:F,log:W}),(ce==="finished"||J)&&await Ce({...b,[z+"Score"]:X})};if(q){const ne=z===p,ue=ne?X:b[p+"Score"]||0,fe=ne?b[m+"Score"]||0:X;O.add(F),at(Y,ue,fe,ne,de)}else await de()}function we(_){return["MIL","ATT"].some(E=>(_[E]||[]).some(A=>!A.used))}function me(_){return["GK","DEF","MIL","ATT"].some(E=>(_[E]||[]).some(A=>!A.used))}function Le(_){return me(_)&&!we(_)}function $e(_){const E=b[_+"Team"],A=b[(_==="p1"?"p2":"p1")+"Team"];return!!(we(E)||!me(A)&&Le(E))}function he(_){const E=["MIL","ATT"].some(W=>(_.p1Team[W]||[]).some(X=>!X.used)),A=["MIL","ATT"].some(W=>(_.p2Team[W]||[]).some(X=>!X.used)),S=me(_.p1Team),z=me(_.p2Team);return!E&&!(!z&&S)&&(!A&&!(!S&&z))}function Se(_){const E=_.p1Score||0,A=_.p2Score||0;return E===A?null:E>A?w.home_id:w.away_id}async function Ce(_){try{const E=Se(_),A=E?w.home_id===E?w.away_id:w.home_id:null;await v.from("matches").update({status:"finished",winner_id:E,home_score:_.p1Score||0,away_score:_.p2Score||0}).eq("id",i),E&&A&&updateEvolutiveCards(E,A).catch(()=>{})}catch(E){console.error("[PvP] finishMatch:",E)}}function Fe(){var z;const _=b[p+"Score"],E=b[m+"Score"],A=_>E,S=_===E;He(e),e.innerHTML=`
    ...
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>${f.map((j,I)=>{const k=g>0&&r.status==="finished"?I===0?Math.floor(g*.7):I===1?Math.floor(g*.2):I===2?Math.floor(g*.1):0:0;return`<tr style="border-bottom:1px solid #f5f5f5;${j.userId===o?"background:#f0fdf4;":""}">
              <td style="padding:7px 3px 7px 0;font-weight:700;color:${I===0?Xe:I<3?ft:"#555"}">${["🥇","🥈","🥉"][I]||I+1}</td>
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        ${Array.from({length:Math.max(0,r.status==="active"?r.current_day-1:r.current_day)},(j,I)=>I+1).reverse().map(j=>{const I=(c||[]).filter(k=>k.matchday===j);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:#888;margin-bottom:6px">Journée ${j}</div>${I.map(k=>Di(k,u,o,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
