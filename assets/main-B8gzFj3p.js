...
import{s as w,l as Gt,m as Ii,F as Si,r as Se,j as Jt,h as Ai,n as An,o as Mn,T as gi,k as jn,p as Cn,b as Bn}from"./formation-links-yJiMLlPw.js";const qn="/";function Ni(e,{navigate:t,toast:i}){let n="login";const o=()=>{var a,s,l,c,d,f;const r=n==="login";e.innerHTML=`
  ...
  <div class="home-dark" id="home-dark">
    ...
    ${Se(e,{width:140})}
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),c=(s||[]).filter(S=>S.card_type==="player"&&S.player),d=(s||[]).filter(S=>S.card_type==="game_changer"),f=(s||[]).filter(S=>S.card_type==="formation"),x=(s||[]).filter(S=>S.card_type==="stadium"),{data:m}=await w.from("gc_definitions").select("name,gc_type,color,effect,image_url"),g={};(m||[]).forEach(S=>{g[S.name]=S});const{data:p}=await w.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),u={};(p||[]).forEach(S=>{u[S.id]=S}),x.forEach(S=>{S.stadium_def&&(u[S.stadium_id]=S.stadium_def)});const h=Object.keys(Si),v=Object.keys(Me),b={};c.forEach(S=>{const q=S.player.id;b[q]=(b[q]||0)+1}),new Set(Object.keys(b).map(S=>String(S)));const $=new Set(f.map(S=>S.formation)),y=new Set(d.map(S=>S.gc_type));let B="player",E="Tous",G="",N=!1;function ne(){return[...c].sort((S,q)=>{const P=Zt.indexOf(S.player.job),X=Zt.indexOf(q.player.job);return P!==X?P-X:(S.player.surname_real||"").localeCompare(q.player.surname_real||"")})}function ie(){return[...l||[]].sort((S,q)=>{const P=Zt.indexOf(S.job),X=Zt.indexOf(q.job);return P!==X?P-X:(S.surname_real||"").localeCompare(q.surname_real||"")})}function Q(){return ne().filter(S=>{const q=S.player,P=E==="Tous"||q.job===E,X=!G||`${q.firstname} ${q.surname_real}`.toLowerCase().includes(G);return P&&X})}function M(){return ie().filter(S=>{const q=E==="Tous"||S.job===E,P=!G||`${S.firstname} ${S.surname_real}`.toLowerCase().includes(G);return q&&P})}e.innerHTML=`
  ...
  </div>`;function _(){const S=document.getElementById("col-filters");S&&(B==="player"?(S.innerHTML=`
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",d.note_g],["DEF",d.note_d],["MIL",d.note_m],["ATT",d.note_a]].map(([W,J])=>{const R=mi[W],S=W==="GK"?"#fff":R,q=W===d.job||W===d.job2,P=(Number(J)||0)+(q&&B>0?B:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(ae=document.getElementById("close-detail"))==null||ae.addEventListener("click",s);let j=new Set;const U=()=>{const W=j.size,J=document.getElementById("sell-action-panel");if(!J)return;J.style.display=W>0?"block":"none",document.getElementById("sell-selected-count").textContent=W;const R=document.getElementById("evolve-btn");R&&(R.textContent=`⬆️ Évoluer ${W>1?"(+"+W+")":""}`)};document.querySelectorAll(".expl-check").forEach(W=>{W.addEventListener("change",()=>{const J=W.dataset.id;W.checked?j.add(J):j.delete(J);const R=W.closest(".exemplaire-row");R&&(R.style.borderColor=W.checked?"#1A6B3C":"#eee"),U()})}),document.querySelectorAll(".exemplaire-row").forEach(W=>{W.addEventListener("click",J=>{if(J.target.closest("button")||J.target.tagName==="INPUT")return;const R=W.querySelector(".expl-check");R&&!R.disabled&&(R.checked=!R.checked,R.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(W=>{W.addEventListener("click",J=>{J.stopPropagation();const R=document.querySelector(`.expl-hist[data-hist="${W.dataset.idx}"]`);R&&(R.style.display=R.style.display==="none"?"flex":"none")})}),(se=document.getElementById("evolve-btn"))==null||se.addEventListener("click",async()=>{if(x<=1)return;const W=[...j];if(!W.length)return;if(j.has(e.id)){const V=document.createElement("div");V.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",V.innerHTML=`
  ...
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
    ...
    </div>`;u("⚽ Choisir une formation",v),document.querySelectorAll("#modal-body [data-forma]").forEach(b=>{b.addEventListener("click",()=>{t.formation=b.dataset.forma;const $=Zi(t.formation),y={};$.forEach(B=>{t.slots[B]&&(y[B]=t.slots[B])}),t.slots=y,h(),xt(e,t,i)})})};e.querySelectorAll("#formation-mobile-btn, #formation-pc-btn").forEach(u=>u.addEventListener("click",m)),e.querySelectorAll("#add-stad-btn-pc, #add-stad-btn").forEach(u=>u.addEventListener("click",()=>go(t,e,i))),e.querySelectorAll("#save-deck").forEach(u=>u.addEventListener("click",()=>yo(t,i))),e.querySelectorAll("[data-remove-sub]").forEach(u=>{u.addEventListener("click",()=>{t.subs=t.subs.filter(h=>h!==u.dataset.removeSub),xt(e,t,i)})}),e.querySelectorAll("#add-sub-btn").forEach(u=>u.addEventListener("click",()=>xo(t,e,i)))}function fo(e,t,i,n){var b,$;const o=window.innerWidth>=900,r=e.querySelector(o?"#deck-field-pc":"#deck-field-mobile");if(!r)return;const a=(b=t.stadiumCards)==null?void 0:b.find(y=>y.id===t.stadiumCardId),s=a&&(($=t.stadDefMap)==null?void 0:$[a.stadium_id])||null,l=Jt[t.formation]||{},c=Ii(t.formation)||[],d={};for(const y of i){const B=t.slots[y],E=B?t.playerCards.find(G=>G.id===B):null;E!=null&&E.player?d[y]={...E.player,_evolution_bonus:E.evolution_bonus||0,face:E.player.face||null}:d[y]=null}const f=window.innerWidth>=900,x=f?window.innerWidth-280:window.innerWidth-20,m=f?Math.min(x,860):x,g=Math.round(f?m*.82:m*.85),p=f?84:44;let u="";for(const[y,B]of c){const E=l[y],G=l[B];if(!E||!G)continue;const N=E.x*m,ne=Math.round(.03*g+E.y*.85*g),ie=G.x*m,Q=Math.round(.03*g+G.y*.85*g),M=d[y],_=d[B],j=Gt(M,_);j==="#ff3333"||j==="#cc2222"?u+=`<line x1="${N.toFixed(1)}" y1="${ne.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${Q.toFixed(1)}" stroke="${j}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(u+=`<line x1="${N.toFixed(1)}" y1="${ne.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${Q.toFixed(1)}" stroke="${j}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,u+=`<line x1="${N.toFixed(1)}" y1="${ne.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${Q.toFixed(1)}" stroke="${j}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let h="";const v=Math.round(p*657/507);for(const y of i){const B=l[y];if(!B)continue;const E=d[y],G=B.x*m;B.y*g;const N=Math.round(.03*g+B.y*(.85*g)),ne=Math.round(G-p/2),ie=Math.round(N-v/2);if(E){const Q=y.replace(/\d+/,""),M=s&&(s.club_id&&String(E.club_id)===String(s.club_id)||s.country_code&&E.country_code===s.country_code),_=Se({...E,_evolution_bonus:E._evolution_bonus||0},{width:p,showStad:!0,stadDef:s,role:Q});h+=`<div style="position:absolute;left:${ne}px;top:${ie}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${y}">
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      card:cards(id,card_type,formation,stadium_id,evolution_bonus,
        player:players(id,firstname,surname_real,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order"),l=[...new Set((s||[]).filter(x=>{var m,g;return((m=x.card)==null?void 0:m.card_type)==="stadium"&&((g=x.card)==null?void 0:g.stadium_id)}).map(x=>x.card.stadium_id))],c={};if(l.length){const{data:x}=await w.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",l);(x||[]).forEach(m=>{c[m.id]=m}),(s||[]).forEach(m=>{var g,p;((g=m.card)==null?void 0:g.card_type)==="stadium"&&((p=m.card)!=null&&p.stadium_id)&&(m.card._stadiumDef=c[m.card.stadium_id]||null)})}let d=0;function f(){var y,B,E,G,N,ne,ie;const x=r[d],m=(s||[]).filter(Q=>Q.deck_id===x.id),g=m.filter(Q=>{var M;return Q.is_starter&&((M=Q.card)==null?void 0:M.player)}).map(Q=>ki(Q.card,Q.position)),p=m.find(Q=>{var M;return((M=Q.card)==null?void 0:M.card_type)==="formation"}),u=x.formation||((y=p==null?void 0:p.card)==null?void 0:y.formation)||"4-4-2";let h=g.length>=11?ri(g,u):null,v=((B=x.stadium_card)==null?void 0:B.stadium_def)||null;v&&h&&(h=Xt(h,v));const b=g.length>=11;nt(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
  ...
  <div id="match-history-panel">
    ...
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      ...
      ${ci(e.aiTeam,e.formation,null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.1))}
            ...
            </div>`}}catch(H){console.error("[Ranked] MMR update error:",H)}(oe=document.getElementById("pvp-end-overlay"))==null||oe.remove();const F=document.createElement("div");F.id="pvp-end-overlay",F.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;color:#fff;padding:24px;text-align:center;overflow-y:auto";const C=T?"🤝":I?"🏆":"😞",K=T?"MATCH NUL":I?"VICTOIRE !":"DÉFAITE",Y=T?"#fff":I?"#FFD700":"#ff6b6b";F.innerHTML=`
      ...
      ${A}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${a?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(F),(Z=F.querySelector("#pvp-end-home"))==null||Z.addEventListener("click",()=>{F.remove(),We(e),m(a?"ranked":"home")})}const Q=w.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},k=>{const L=k.new;try{if(L.status==="finished"||L.forfeit){if(B)return;B=!0,E&&(clearInterval(E),E=null),L.game_state&&(y=L.game_state),ie(L);return}if(L.game_state){const z=y;y=L.game_state;const I=y._lastGC;if(I&&I.seq&&!ne.has(I.seq)&&(ne.add(I.seq),I.by!==p)){R(I.type,I.by,()=>de());return}const T=z[p+"Score"]||0,A=z[u+"Score"]||0,F=y[p+"Score"]||0,C=y[u+"Score"]||0,K=F>T,Y=C>A;if((K||Y)&&!N.has(y.round)){N.add(y.round);const oe=[...y.log||[]].reverse().find(H=>H.isGoal),Z=((oe==null?void 0:oe.homePlayers)||[]).map(H=>({name:H.name,note:H.note,portrait:H.portrait,job:H.job}));ot(Z,F,C,K,()=>de());return}de()}}catch(z){console.error("[PvP] crash:",z)}}).subscribe();async function M(k){Object.assign(y,k),y.lastActionAt=new Date().toISOString();const{error:L}=await w.from("matches").update({game_state:y}).eq("id",i);L&&g("Erreur de synchronisation","error");try{de()}catch(z){console.error("[PvP] renderPvpScreen crash:",z)}}async function _(){if(B)return;B=!0,E&&(clearInterval(E),E=null);const k=n?b.away_id:b.home_id,L=n?"p2":"p1",z=n?"p1":"p2",I={...y,[L+"Score"]:3,[z+"Score"]:0,phase:"finished"},T=I.p1Score||0,A=I.p2Score||0;try{await w.from("matches").update({game_state:I}).eq("id",i);const{data:F,error:C}=await w.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:k,p_home_score:T,p_away_score:A,p_forfeit:!0});if(C&&console.error("[PvP] finish_pvp_match (forfeit):",C),d)try{await w.rpc("check_and_finish_mini_league",{p_league_id:d})}catch(K){console.error("[PvP] check_and_finish_mini_league (forfeit):",K)}try{const{data:K}=await w.rpc("apply_match_rewards",{p_match_id:i});K!=null&&K.success&&!(K!=null&&K.skipped)&&typeof t.refreshProfile=="function"&&await t.refreshProfile()}catch(K){console.error("[PvP] apply_match_rewards (forfeit):",K)}}catch{}try{w.removeChannel(Q)}catch{}setTimeout(()=>{We(e),d?m("mini-league",{openLeagueId:d}):m("home")},800)}const j={BOOST_STAT:({value:k=1,count:L=1,roles:z=[]},I,T)=>{const A=I[p+"Team"],F=Object.entries(A).filter(([C])=>!z.length||z.includes(C)).flatMap(([C,K])=>K.filter(Y=>!Y.used).map(Y=>({...Y,_line:C})));if(!F.length){I.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),T(I);return}U(F,L,`Choisir ${L} joueur(s) à booster (+${k})`,C=>{C.forEach(K=>{const Y=(A[K._line]||[]).find(oe=>oe.cardId===K.cardId);Y&&(Y.boost=(Y.boost||0)+k,I.log.push({text:`⚡ +${k} sur ${Y.name}`,type:"info"}))}),I[p+"Team"]=A,T(I)})},DEBUFF_STAT:({value:k=1,count:L=1,roles:z=[],target:I="ai"},T,A)=>{const F=I==="home"?p:u,C=T[F+"Team"],K=I==="home"?"allié":"adverse",Y=Object.entries(C).filter(([oe])=>!z.length||z.includes(oe)).flatMap(([oe,Z])=>Z.map(H=>({...H,_line:oe})));if(!Y.length){T.log.push({text:`🎯 Aucun joueur ${K}`,type:"info"}),A(T);return}U(Y,L,`Choisir ${L} joueur(s) ${K}(s) (-${k})`,oe=>{oe.forEach(Z=>{const H=(C[Z._line]||[]).find(le=>le.cardId===Z.cardId);H&&(H.boost=(H.boost||0)-k,T.log.push({text:`🎯 -${k} sur ${H.name}`,type:"info"}))}),T[F+"Team"]=C,A(T)})},GRAY_PLAYER:({count:k=1,roles:L=[],target:z="ai"},I,T)=>{const A=z==="home"?p:u,F=I[A+"Team"],C=z==="home"?"allié":"adverse",K=Object.entries(F).filter(([Y])=>!L.length||L.includes(Y)).flatMap(([Y,oe])=>oe.filter(Z=>!Z.used).map(Z=>({...Z,_line:Y})));if(!K.length){I.log.push({text:`❌ Aucun joueur ${C}`,type:"info"}),T(I);return}U(K,k,`Choisir ${k} joueur(s) ${C}(s) à exclure`,Y=>{const oe="usedCardIds_"+A,Z=new Set(I[oe]||[]);Y.forEach(H=>{const le=(F[H._line]||[]).find(pe=>pe.cardId===H.cardId);le&&(le.used=!0,Z.add(H.cardId),I.log.push({text:`❌ ${le.name} exclu !`,type:"info"}))}),I[oe]=[...Z],I[A+"Team"]=F,T(I)})},REVIVE_PLAYER:({count:k=1,roles:L=[]},z,I)=>{const T=z[p+"Team"],A=Object.entries(T).filter(([F])=>!L.length||L.includes(F)).flatMap(([F,C])=>C.filter(K=>K.used).map(K=>({...K,_line:F})));if(!A.length){z.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),I(z);return}U(A,k,`Choisir ${k} joueur(s) à ressusciter`,F=>{F.forEach(C=>{const K=(T[C._line]||[]).find(Y=>Y.cardId===C.cardId);K&&(K.used=!1,z.log.push({text:`💫 ${K.name} ressuscité !`,type:"info"}))}),z[p+"Team"]=T,I(z)})},REMOVE_GOAL:({},k,L)=>{const z=u+"Score";k[z]>0?(k[z]--,k.log.push({text:"🚫 Dernier but annulé !",type:"info"})):k.log.push({text:"🚫 Aucun but à annuler",type:"info"}),L(k)},ADD_GOAL_DRAW:({},k,L)=>{k[p+"Score"]===k[u+"Score"]?(k[p+"Score"]++,k.log.push({text:"🎯 But bonus !",type:"info"})):k.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),L(k)},ADD_SUB:({value:k=1},L,z)=>{L.maxSubs=(L.maxSubs||3)+k,L.log.push({text:`🔄 +${k} remplacement(s)`,type:"info"}),z(L)},CUSTOM:({},k,L)=>L(k)};function U(k,L,z,I){const T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let A=[];function F(){var K,Y;const C=k.map(oe=>{const Z=oe._line||oe.job||"MIL",H=A.find(pe=>pe.cardId===oe.cardId),le=Se({...oe,_evolution_bonus:0},{width:90,showStad:!0,role:Z,extraNote:oe.boost||0});return`<div class="pp-item" data-cid="${oe.cardId}"
        ...
        </div>`,(K=T.querySelector("#pp-close"))==null||K.addEventListener("click",()=>T.remove()),T.querySelectorAll(".pp-item").forEach(oe=>{oe.addEventListener("click",()=>{const Z=oe.dataset.cid,H=k.find(pe=>pe.cardId===Z),le=A.findIndex(pe=>pe.cardId===Z);H&&(le>-1?A.splice(le,1):A.length<L&&A.push(H),F())})}),(Y=T.querySelector("#pp-confirm"))==null||Y.addEventListener("click",()=>{T.remove(),I(A)})}F(),document.body.appendChild(T)}async function re(k,L){const I=(y["gcCardsFull_"+p]||[]).find(C=>C.id===k),T=(I==null?void 0:I._gcDef)||(y.gcDefs||[]).find(C=>{var K;return C.name===L||((K=C.name)==null?void 0:K.toLowerCase().trim())===(L==null?void 0:L.toLowerCase().trim())}),A=[...y["usedGc_"+p]||[],k],F={type:L,by:p,seq:(y._gcAnimSeq||0)+1};ne.add(F.seq),R(L,p,async()=>{if(T!=null&&T.effect_type&&T.effect_type!=="CUSTOM"){const K=j[T.effect_type];if(K){const Y={...y};K(T.effect_params||{},Y,async oe=>{oe["usedGc_"+p]=A,oe._lastGC=F,oe._gcAnimSeq=F.seq,await M(oe)});return}}const C={...y};switch(L){case"Remplacement+":C.maxSubs=(C.maxSubs||3)+1,C.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const K=u+"Score";C[K]>0&&(C[K]--,C.log.push({text:"🚫 But annulé",type:"info"}));break}}C["usedGc_"+p]=A,C._lastGC=F,C._gcAnimSeq=F.seq,await M(C)})}function ae(k,L){const z="usedCardIds_"+k,I=new Set(y[z]||[]);L.forEach(T=>I.add(T)),y[z]=[...I]}function se(){for(const k of["p1","p2"]){const L=new Set(y["usedCardIds_"+k]||[]),z=y[k+"Team"];if(z)for(const I of["GK","DEF","MIL","ATT"])(z[I]||[]).forEach(T=>{T.used=L.has(T.cardId)})}}function de(){var at,st,vt,Pi,Gi,Ri;if(y.phase==="reveal")return ke();if(y.phase==="midfield")return J();if(y.phase==="finished")return Rt();const k=y[p+"Team"];y[u+"Team"],se();const L=y[p+"Score"],z=y[u+"Score"],I=y[p+"Name"],T=y[u+"Name"],A=y.phase===p+"-attack",F=y.phase===p+"-defense",C=Array.isArray(y["selected_"+p])?y["selected_"+p]:[],K=C.map(ee=>ee.cardId),Y=window.innerWidth>=700,oe=y[p+"Subs"]||[],Z=y["usedSubIds_"+p]||[],H=oe.filter(ee=>!Z.includes(ee.cardId)),le=y["gcCardsFull_"+p]||[],pe=y["usedGc_"+p]||[],ue=le.filter(ee=>!pe.includes(ee.id)),ce=y.boostOwner===p&&!y.boostUsed,me=[...k.MIL||[],...k.ATT||[]].filter(ee=>!ee.used),xe=A&&me.length===0?[...k.GK||[],...k.DEF||[]].filter(ee=>!ee.used).map(ee=>ee.cardId):[];function te(ee,fe,ze){var Mt,lt;const he=ee._gcDef,St={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[he==null?void 0:he.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",et={purple:"#b06ce0",light_blue:"#00d4ef"}[he==null?void 0:he.color]||"#b06ce0",De=(he==null?void 0:he.name)||ee.gc_type,Ne=(he==null?void 0:he.effect)||((Mt=Me[ee.gc_type])==null?void 0:Mt.desc)||"",Ae=he!=null&&he.image_url?`/icons/${he.image_url}`:null,wt=((lt=Me[ee.gc_type])==null?void 0:lt.icon)||"⚡",Pe=Math.round(ze*.22),dt=Math.round(ze*.22),tt=ze-Pe-dt,At=De.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${ee.id}" data-gc-type="${ee.gc_type}"
      ...
      </div>`}const Fe=(ee,fe)=>fe?Te(130,175):te(ee,130,175),Ie=(ee,fe)=>fe?Te(68,95):te(ee,68,95),He=Y?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",It=A?je(p)?`<button id="pvp-action" style="${He};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${C.length===0?"disabled":""}>⚔️ ATTAQUEZ <span id="pvp-timer"></span></button>`:`<button id="pvp-action" data-pass="1" style="${He};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER <span id="pvp-timer"></span></button>`:F?`<button id="pvp-action" style="${He};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${C.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="pvp-timer"></span></button>`:`<div style="font-size:11px;color:rgba(255,255,255,0.3);text-align:center;padding:4px">⏳ Tour de ${T}</div>`,Je=A&&!je(p)?'<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">Aucun attaquant — passez la main</div>':A||F?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${C.length}/3 sélectionné(s)</div>`:"",Qt=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${Y?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
    ...
    </div>`,Re=y[p+"Team"],ht=(()=>{var fe,ze,he,St,et;if(F&&((fe=y.pendingAttack)!=null&&fe.players)){const De=y.pendingAttack;let Ne="";if(C.length>0){const Ae=C.map(Pe=>{const dt=(Re[Pe._role]||[]).find(fi=>fi.cardId===Pe.cardId)||Pe,tt=Re[Pe._role]||[],At=tt.findIndex(fi=>fi.cardId===Pe.cardId),Mt=pt(tt.length),lt=At>=0?Mt[At]:dt._col??1;return{...dt,_line:Pe._role,_col:lt}}),wt=Yt(Ae,((ze=y.modifiers)==null?void 0:ze[p])||{},y[p+"Formation"]);Ne=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
            <div style="font-size:8px;color:#3a7bd5;letter-spacing:2px;margin-bottom:4px;text-transform:uppercase">🛡️ Votre défense (${C.length}/3)</div>
        ...
        </div>`}if(A&&!((St=y.pendingAttack)!=null&&St.players)&&C.length>0){const De=C.map(Ae=>{const wt=(Re[Ae._role]||[]).find(lt=>lt.cardId===Ae.cardId)||Ae,Pe=["GK","DEF"].includes(Ae._role),dt=Re[Ae._role]||[],tt=dt.findIndex(lt=>lt.cardId===Ae.cardId),At=pt(dt.length),Mt=tt>=0?At[tt]:wt._col??1;return{...wt,_line:Ae._role,_col:Mt,...Pe?{note_a:Math.max(1,Number(wt.note_a)||0)}:{}}}),Ne=Vt(De,((et=y.modifiers)==null?void 0:et[p])||{},y[p+"Formation"]);return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #FFD700;text-align:center">
          <div style="font-size:8px;color:#FFD700;letter-spacing:2px;margin-bottom:4px;text-transform:uppercase">⚔️ Votre sélection (${C.length}/3)</div>
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        ...
        <div id="pout-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
          <div style="font-size:9px;color:#ff6b6b;letter-spacing:2px;text-transform:uppercase;font-weight:700">Joueur qui sort</div>
          <button id="pout-up" style="${te(C===0)}" ${C===0?"disabled":""}>▲</button>
          <div>${ue?mt({...ue,used:!1,boost:0},me,xe):"<div>—</div>"}</div>
          <button id="pout-down" style="${te(C>=A.length-1)}" ${C>=A.length-1?"disabled":""}>▼</button>
          <div style="font-size:10px;color:rgba(255,255,255,0.35)">${C+1}/${A.length}</div>
        </div>
      </div>
      <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
        <button id="psub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
      </div>`,(Fe=Z.querySelector("#psub-close"))==null||Fe.addEventListener("click",()=>Z.remove()),(Ie=Z.querySelector("#pout-up"))==null||Ie.addEventListener("click",()=>{C>0&&(C--,H())}),(He=Z.querySelector("#pout-down"))==null||He.addEventListener("click",()=>{C<A.length-1&&(C++,H())}),(It=Z.querySelector("#pin-up"))==null||It.addEventListener("click",()=>{Y>0&&(Y--,H())}),(Je=Z.querySelector("#pin-down"))==null||Je.addEventListener("click",()=>{Y<F.length-1&&(Y++,H())});const Te=(qe,Qe,Re,ht)=>{const Ze=Z.querySelector("#"+qe);if(!Ze)return;let Ue=0;Ze.addEventListener("touchstart",at=>{Ue=at.touches[0].clientY},{passive:!0}),Ze.addEventListener("touchend",at=>{const st=at.changedTouches[0].clientY-Ue;if(Math.abs(st)<30)return;const vt=Qe();st<0&&vt<ht-1?(Re(vt+1),H()):st>0&&vt>0&&(Re(vt-1),H())},{passive:!0})};Te("pin-panel",()=>Y,qe=>Y=qe,F.length),Te("pout-panel",()=>C,qe=>C=qe,A.length),(Qt=Z.querySelector("#psub-confirm"))==null||Qt.addEventListener("click",async qe=>{if(qe.preventDefault(),qe.stopPropagation(),oe)return;oe=!0;const Qe=A[C],Re=F[Y];if(!Qe||!Re)return;const ht=Qe._line,Ze=(z[ht]||[]).findIndex(st=>st.cardId===Qe.cardId);if(Ze===-1){g("Erreur : joueur introuvable","error"),Z.remove();return}const Ue={...Re,_line:ht,position:Qe.position,used:!1,boost:0};z[ht].splice(Ze,1,Ue);const at=[...I,Re.cardId];Z.remove(),X(Qe,Re,async()=>{await M({[p+"Team"]:z,[u+"Team"]:y[u+"Team"],["usedSubIds_"+p]:at})})})}document.body.appendChild(Z),H()}function X(k,L,z){const I={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const A=(K,Y,oe)=>`<div style="text-align:center">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:6px">Appuyer pour continuer</div>`,document.body.appendChild(T);let F=!1;const C=()=>{F||(F=!0,T.remove(),setTimeout(()=>z(),50))};T.addEventListener("click",C),setTimeout(C,2200)}function V(){var L;const k=document.createElement("div");k.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:20px;overflow-y:auto",k.innerHTML=`
      ...
      </div>`,document.body.appendChild(L),(z=L.querySelector("#pvp-hist-close"))==null||z.addEventListener("click",()=>L.remove())}async function O(){if(y.phase!==p+"-attack")return;const k=p==="p1"?"p2":"p1",L=(y.round||0)+1,z=[...y.log||[]];z.push({type:"info",text:`⏭️ ${y[p+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const I=Oe(y),T=je(k),A=I||!T?"finished":k+"-attack";await M({["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:A,attacker:k,round:L,log:z}),A==="finished"&&await Xe(y)}function we(k){const L=(k.GK||[]).some(I=>!I.used),z=["DEF","MIL","ATT"].some(I=>(k[I]||[]).some(T=>!T.used));return L&&!z}function _e(k){return(k.p1Score||0)!==(k.p2Score||0)?null:we(k.p1Team)&&!ge(k.p2Team)?{side:"p1",gk:(k.p1Team.GK||[]).find(L=>!L.used)}:we(k.p2Team)&&!ge(k.p1Team)?{side:"p2",gk:(k.p2Team.GK||[]).find(L=>!L.used)}:null}async function ye(k,L){const{side:z,gk:I}=k;if(!I)return;const T=z+"Score",A=(L[T]||0)+1;I.used=!0;const F=[...L.log||[]];F.push({type:"duel",isGoal:!0,homeScored:z===p,homePlayers:z==="p1"?[jt(I)]:[],aiPlayers:z==="p2"?[jt(I)]:[],text:`⚽ DERNIER CORNER — Le gardien de ${L[z+"Name"]} marque !`});const C=(L.round||0)+1,K=z==="p1"?"p2":"p1",Y={...L,[T]:A},oe=Oe(Y);N.add(C);const Z=document.createElement("div");Z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:950;display:flex;align-items:center;justify-content:center;overflow:hidden;text-align:center;padding:24px",document.body.appendChild(Z);const H=(ue,ce)=>new Promise(me=>{Z.innerHTML=`<div style="font-size:32px;font-weight:900;color:${ce};letter-spacing:2px;animation:lcFade 1.4s ease both">${ue}</div>
      <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(me,1400)});await H("⚽ DERNIER CORNER","#FFD700"),await H("🧤 LE GARDIEN MONTE !","#4fc3f7"),Z.remove();const le=z===p?A:L[p+"Score"]||0,pe=z===p?L[u+"Score"]||0:A;ot([jt(I)],le,pe,z===p,async()=>{await M({[T]:A,log:F,round:C,pendingAttack:null,phase:oe?"finished":K+"-attack",attacker:K,["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}}}),oe&&await Xe({...y,[T]:A})})}async function be(){const k=y[p+"Team"],L=!["GK","DEF","MIL","ATT"].some(A=>(y[u+"Team"][A]||[]).some(F=>!F.used)),z=(y["selected_"+p]||[]).map(A=>{const F=(k[A._role]||[]).find(pe=>pe.cardId===A.cardId)||A,C=["GK","DEF"].includes(A._role),K=k[A._role]||[],Y=K.findIndex(pe=>pe.cardId===A.cardId),oe=pt(K.length),Z=Y>=0?oe[Y]:F._col??1,H=y.stadiumDef||y.homeStadiumDef||null,le=F.stadiumBonus||H&&(H.club_id&&String(F.club_id)===String(H.club_id)||H.country_code&&F.country_code===H.country_code)||!1;return{...F,_line:A._role,_col:Z,stadiumBonus:le,...C?{note_a:Math.max(1,Number(F.note_a)||0)}:{}}});if(!z.length)return;const I=Vt(z,y.modifiers[p]||{},y[p+"Formation"]);ae(p,z.map(A=>A.cardId)),z.forEach(A=>{const F=(k[A._role]||[]).find(C=>C.cardId===A.cardId);F&&(F.used=!0)}),y["selected_"+p]=[],de();const T=[...y.log||[]];if(L){if(z.length===1&&(z[0]._line==="GK"||z[0].job==="GK")&&(y.p1Score||0)===(y.p2Score||0)){await ye({side:p,gk:z[0]},{...y,[p+"Team"]:k,log:T});return}const F=(y[p+"Score"]||0)+1,C=z.map(H=>jt(H));T.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:C,homeTotal:I.total,aiTotal:0});const K=(y.round||0)+1,Y=p==="p1"?"p2":"p1",oe={...y,[p+"Team"]:k,[p+"Score"]:F},Z=Oe(oe);N.add(K),ot(C,F,y[u+"Score"]||0,!0,async()=>{await M({[p+"Team"]:k,[p+"Score"]:F,["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:Z?"finished":Y+"-attack",attacker:Y,round:K,log:T}),Z&&await Xe({...y,[p+"Score"]:F})});return}T.push({type:"pending",text:`⚔️ ${y[p+"Name"]} attaque (${I.total})`}),await M({[p+"Team"]:k,[u+"Team"]:y[u+"Team"],pendingAttack:{...I,players:z,side:p},["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},phase:u+"-defense",log:T})}async function $e(){const k=y[p+"Team"],L=(y["selected_"+p]||[]).map(ce=>{const me=(k[ce._role]||[]).find(Ie=>Ie.cardId===ce.cardId)||ce,xe=k[ce._role]||[],te=xe.findIndex(Ie=>Ie.cardId===ce.cardId),Te=pt(xe.length),Fe=te>=0?Te[te]:me._col??1;return{...me,_line:ce._role,_col:Fe}}),z=Yt(L,y.modifiers[p]||{},y[p+"Formation"]);ae(p,L.map(ce=>ce.cardId)),L.forEach(ce=>{const me=(k[ce._role]||[]).find(xe=>xe.cardId===ce.cardId);me&&(me.used=!0)}),y["selected_"+p]=[],de();const I=Ci(y.pendingAttack.total,z.total,y.modifiers[p]||{}),T=y.pendingAttack.side,A=I==="attack"||(I==null?void 0:I.goal),F=T==="p1"?"p2":"p1",C=(y.round||0)+1,K=(y.pendingAttack.players||[]).map(ce=>jt(ce)),Y=[...y.log||[]];Y.push({type:"duel",isGoal:A,homeScored:A&&T===p,text:A?`⚽ BUT de ${y[T+"Name"]} ! (${y.pendingAttack.total} vs ${z.total})`:`✋ Attaque stoppée (${y.pendingAttack.total} vs ${z.total})`,homePlayers:K,aiPlayers:L.map(ce=>jt(ce)),homeTotal:y.pendingAttack.total,aiTotal:z.total});const oe=A?(y[T+"Score"]||0)+1:y[T+"Score"]||0,Z={...y,[p+"Team"]:k,[T+"Score"]:oe},H=Oe(Z),le=H?"finished":F+"-attack",pe=!H&&!A?_e(Z):null,ue=async()=>{if(pe){await ye(pe,Z);return}await M({[p+"Team"]:k,[u+"Team"]:y[u+"Team"],[T+"Score"]:oe,["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:le,attacker:F,round:C,log:Y}),(le==="finished"||H)&&await Xe({...y,[T+"Score"]:oe})};if(A){const ce=T===p,me=ce?oe:y[p+"Score"]||0,xe=ce?y[u+"Score"]||0:oe;N.add(C),ot(K,me,xe,ce,ue)}else await ue()}function Le(k){return["MIL","ATT"].some(L=>(k[L]||[]).some(z=>!z.used))}function ge(k){return["GK","DEF","MIL","ATT"].some(L=>(k[L]||[]).some(z=>!z.used))}function Be(k){return ge(k)&&!Le(k)}function je(k){const L=y[k+"Team"];return!!(Le(L)||Be(L))}function Oe(k){return!ge(k.p1Team)&&!ge(k.p2Team)}function bt(k){const L=k.p1Score||0,z=k.p2Score||0;return L===z?null:L>z?b.home_id:b.away_id}async function Xe(k){try{const L=bt(k),z=L?b.home_id===L?b.away_id:b.home_id:null,I=k.p1Score||0,T=k.p2Score||0,{data:A,error:F}=await w.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:L,p_home_score:I,p_away_score:T,p_forfeit:!1});if(F&&console.error("[PvP] finish_pvp_match:",F),d)try{await w.rpc("check_and_finish_mini_league",{p_league_id:d})}catch(C){console.error("[PvP] check_and_finish_mini_league:",C)}L&&z&&eo(L,z).catch(()=>{});try{const{data:C}=await w.rpc("apply_match_rewards",{p_match_id:i});C!=null&&C.success&&!(C!=null&&C.skipped)&&typeof t.refreshProfile=="function"&&await t.refreshProfile()}catch(C){console.error("[PvP] apply_match_rewards:",C)}if(typeof c=="function")try{await c({homeScore:I,awayScore:T,winnerId:L,homeId:b.home_id,awayId:b.away_id,matchId:i})}catch(C){console.error("[PvP] onMatchEnd:",C)}}catch(L){console.error("[PvP] finishMatch:",L)}}function Rt(){var T;const k=y[p+"Score"],L=y[u+"Score"],z=k>L,I=k===L;nt(e),e.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    </div>`;let d=!1;const f=async u=>{if(!d){if(d=!0,clearInterval(g),m)try{m.unsubscribe()}catch{}u&&await zt(e,t,u,o,{mlLeagueId:n,mlMatchId:i,myGC:r||[],gcDefs:a,stadiumDef:s}),l()}};(p=document.getElementById("wait-back"))==null||p.addEventListener("click",()=>{if(d=!0,clearInterval(g),m)try{m.unsubscribe()}catch{}c("mini-league",{openLeagueId:n}),l()});const x=async()=>{if(d)return;const{data:u}=await w.from("mini_league_matches").select("match_id").eq("id",i).single();u!=null&&u.match_id&&f(u.match_id)},m=w.channel(`ml_match_${i}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"mini_league_matches",filter:`id=eq.${i}`},u=>{var h;(h=u.new)!=null&&h.match_id&&f(u.new.match_id)}).subscribe(),g=setInterval(x,3e3);x()})}const wr="/",_r=[{emoji:"⚽",title:"Bienvenue dans Manager Wars !",color:"#1A6B3C",content:`<p>Tu es désormais un <strong>manager de football</strong> virtuel.</p>
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
  ...
  </div>`;let l="buy";const c=()=>{var p,u,h,v,b,$,y;return{name:(((p=document.getElementById("flt-name"))==null?void 0:p.value)||"").toLowerCase().trim(),club:(((u=document.getElementById("flt-club"))==null?void 0:u.value)||"").toLowerCase().trim(),country:(((h=document.getElementById("flt-country"))==null?void 0:h.value)||"").toLowerCase().trim(),job:((v=document.getElementById("flt-job"))==null?void 0:v.value)||"",rarity:((b=document.getElementById("flt-rarity"))==null?void 0:b.value)||"",note1:parseInt(($=document.getElementById("flt-note1"))==null?void 0:$.value)||0,note2:parseInt((y=document.getElementById("flt-note2"))==null?void 0:y.value)||0}};function d(p){const u=c();return p.filter(h=>{var N,ne,ie;const v=(N=h.card)==null?void 0:N.player;if(!v)return!1;const b=`${v.firstname} ${v.surname_real}`.toLowerCase(),$=(((ne=v.clubs)==null?void 0:ne.encoded_name)||"").toLowerCase(),y=(v.country_code||"").toLowerCase(),B=((ie=h.card)==null?void 0:ie.evolution_bonus)||0,E=Ct(v,v.job,B),G=v.job2?Ct(v,v.job2,B):0;return!(u.name&&!b.includes(u.name)||u.club&&!$.includes(u.club)||u.country&&!y.includes(u.country)||u.job&&v.job!==u.job||u.rarity&&v.rarity!==u.rarity||u.note1&&E<u.note1||u.note2&&G<u.note2)})}function f(p){var G,N,ne,ie;const u=(G=p.card)==null?void 0:G.player;if(!u)return"";const h=((N=p.card)==null?void 0:N.evolution_bonus)||0,v=Ct(u,u.job,h),b=u.job2?Ct(u,u.job2,h):0,$=(i.profile.credits||0)>=p.price,y=u.country_code?`https://flagsapi.com/${u.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,B=ii[u.job]||"#bbb",E=u.job2?ii[u.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden">
      <!-- Drapeau -->
    ...
    </div>`}function x(p){var G,N,ne,ie;const u=(G=p.card)==null?void 0:G.player;if(!u)return"";const h=((N=p.card)==null?void 0:N.evolution_bonus)||0,v=Ct(u,u.job,h),b=u.job2?Ct(u,u.job2,h):0,$=p.status==="sold",y=u.country_code?`https://flagsapi.com/${u.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,B=ii[u.job]||"#bbb",E=u.job2?ii[u.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${$?"opacity:0.7":""}">
      ${y?`<img src="${y}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      ${(ne=u.clubs)!=null&&ne.logo_url?`<img src="${u.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${B};display:flex;align-items:center;justify-content:center">
    ...
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center">
      ...
      `).join(""):'<div style="text-align:center;color:var(--gray-600);padding:40px">Aucun manager.</div>'}else{const h=a||[];g.innerHTML=h.length>0?h.map((v,b)=>{const $=Ai(v.mmr??1e3),y=(v.ranked_wins||0)+(v.ranked_losses||0)+(v.ranked_draws||0),B=y>0?Math.round((v.ranked_wins||0)/y*100):0,E=v.id===i.profile.id,G=(v.placement_matches||0)<10;return`
  ...
  <div style="min-height:100%;background:${v[g.id]};padding:16px;overflow-y:auto;display:flex;flex-direction:column;gap:16px">
    ...
    <div style="display:flex;flex-direction:column;gap:10px;margin-top:auto;padding-top:8px">
      ...
      <div style="flex:1">
        ...
        <div style="font-size:11px;color:var(--gray-600)">${u} · ${B}${d.status==="in_progress"?" · en cours":""}</div>
      </div>
      <div style="display:flex;align-items:center;gap:8px">
