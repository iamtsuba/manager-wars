const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/special-cards-CrbcHJPn.js","assets/special-cards-C2pAaCmQ.css"])))=>i.map(i=>d[i]);
import{s as v,j as hi,q as zn,t as Ht,u as Ni,F as Oi,o as tt,n as ei,g as At,r as Te,m as Mt,v as Ai,w as uo,T as Li,p as fo,x as mo,d as go}from"./special-cards-CrbcHJPn.js";const xo="/";function cn(e,{navigate:t,toast:i}){let n="login";const o=()=>{var a,s,d,c,l,f,b;const r=n==="login";if(e.innerHTML=`
    <div style="
  ...
  </div>`,requestAnimationFrame(()=>{var fe;const L=((fe=window.visualViewport)==null?void 0:fe.height)||window.innerHeight,T=document.getElementById("home2-chrome-header"),K=T?T.offsetHeight:0,V=L-K,ce=e.querySelector(".home-dark");ce&&(ce.style.minHeight=V+"px")});const E=()=>{const L=document.getElementById("mode-indicator");L&&(L.textContent=window.innerWidth>=1024?"🖥️ Mode PC":"📱 Mode Mobile")};if(E(),window.addEventListener("resize",E),(m=document.getElementById("nav-edit-btn"))==null||m.addEventListener("click",()=>i("settings")),(M=document.getElementById("nav-profile-btn"))==null||M.addEventListener("click",()=>i("settings")),(k=document.getElementById("nav-rankings-link"))==null||k.addEventListener("click",()=>i("rankings")),(S=document.getElementById("nav-rankings-cta"))==null||S.addEventListener("click",()=>i("rankings")),(P=document.getElementById("rank-inline-link-btn"))==null||P.addEventListener("click",()=>i("rankings")),(J=document.getElementById("nav-journal-link"))==null||J.addEventListener("click",()=>Go()),e.querySelectorAll("[data-action]").forEach(L=>{L.addEventListener("click",()=>{L.style.transform="scale(.96)",setTimeout(()=>L.style.transform="",180);const T=L.dataset.action;if(T==="match-ai"){Fn(i);return}if(T==="match-random"){i("match",{matchMode:"random"});return}if(T==="match-friend"){i("friends");return}if(T==="mini-league"){i("mini-league");return}if(T==="ranked"){i("ranked");return}n("Bientôt disponible","info")})}),p.length>1){const L=document.getElementById("promo-slide-content"),T=()=>Array.from(document.querySelectorAll(".promo-dot")),K=V=>{w=V,L&&(L.innerHTML=h(p[w])),T().forEach((ce,fe)=>ce.classList.toggle("active",fe===w))};setInterval(()=>K((w+1)%p.length),5e3),document.querySelectorAll(".promo-dot").forEach(V=>{V.addEventListener("click",()=>K(Number(V.dataset.dot)))})}(ie=document.getElementById("promo-cta-btn"))==null||ie.addEventListener("click",()=>i("boosters")),(Q=document.getElementById("back-to-v1-btn"))==null||Q.addEventListener("click",()=>{jo(),i("home")}),Dn(t,n),Uo(t,n,i),qn(t,n,i),No(t,n)}async function No(e,t){const i=e.profile.id,{data:n}=await v.from("mini_league_members").select("league_id, prize_amount, mini_leagues(name)").eq("user_id",i).eq("prize_claimed",!1).gt("prize_amount",0);if(!(n!=null&&n.length))return;const o=n.reduce((a,s)=>a+(s.prize_amount||0),0),r=document.createElement("div");r.className="modal-overlay",r.style.zIndex="2200",r.innerHTML=`<div class="modal" style="max-width:400px">
    <div class="modal-header"><h2 style="color:#1A6B3C">🏆 Gains à récupérer</h2></div>
    <div class="modal-body" style="padding:18px 20px">
      <p style="font-size:14px;color:#555;margin:0 0 14px">Tu as terminé sur le podium de ${n.length>1?"plusieurs mini leagues":"une mini league"} !</p>
      <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:16px">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    ${Te(e,{width:140})}
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),c=(s||[]).filter($=>$.card_type==="player"&&$.player),l=(s||[]).filter($=>$.card_type==="game_changer"),f=(s||[]).filter($=>$.card_type==="formation"),b=(s||[]).filter($=>$.card_type==="stadium"),{data:x}=await v.from("gc_definitions").select("name,gc_type,color,effect,image_url"),g={};(x||[]).forEach($=>{g[$.name]=$});const{data:p}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),u={};(p||[]).forEach($=>{u[$.id]=$}),b.forEach($=>{$.stadium_def&&(u[$.stadium_id]=$.stadium_def)});const y=Object.keys(Oi),w=Object.keys(je),h={};c.forEach($=>{const B=$.player.id;h[B]=(h[B]||0)+1}),new Set(Object.keys(h).map($=>String($)));const E=new Set(f.map($=>$.formation)),m=new Set(l.map($=>$.gc_type));let M="player",k="Tous",S="",P=!1;function J(){return[...c].sort(($,B)=>{const N=si.indexOf($.player.job),R=si.indexOf(B.player.job);return N!==R?N-R:($.player.surname_real||"").localeCompare(B.player.surname_real||"")})}function ie(){return[...d||[]].sort(($,B)=>{const N=si.indexOf($.job),R=si.indexOf(B.job);return N!==R?N-R:($.surname_real||"").localeCompare(B.surname_real||"")})}function Q(){return J().filter($=>{const B=$.player,N=k==="Tous"||B.job===k,R=!S||`${B.firstname} ${B.surname_real}`.toLowerCase().includes(S);return N&&R})}function L(){return ie().filter($=>{const B=k==="Tous"||$.job===k,N=!S||`${$.firstname} ${$.surname_real}`.toLowerCase().includes(S);return B&&N})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:0 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none;align-items:center"></div>
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",B=>{S=B.target.value.toLowerCase(),K()}),e.querySelectorAll(".filter-btn").forEach(B=>{B.addEventListener("click",()=>{k=B.dataset.job,T(),K()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{P=!P,T(),K()})):($.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{P=!P,T(),K()})))}function K(){const $=document.getElementById("col-grid");$&&(M==="player"?Y($):M==="formation"?ue($):M==="stadium"?O($):pe($))}function V($,B,N,R,oe,q=0){const U=document.getElementById("col-grid"),re=document.getElementById("col-big");if(!U||!re)return;var ae=0;function ge(){var ct;const ze=window.innerWidth>=768,he=document.getElementById("col-big"),Ie=document.getElementById("col-gap"),Se=((ct=document.getElementById("col-grid"))==null?void 0:ct.parentElement)||null,Be=he?he.closest(".page"):null;if(he&&Se&&Be){let $e=0;Array.from(Be.children).forEach(function(j){j!==he&&j!==Se&&j!==Ie&&($e+=j.offsetHeight)});const Ce=Math.max(0,Be.clientHeight-$e),qe=Math.round(Ce*(q/100)),Ge=Math.max(0,Ce-qe),_=ze?50/71:45/63,z=Math.round(Ge*_),A=Math.max(0,Ge-z);Ie&&(Ie.style.height=qe+"px"),he.style.flex="none",he.style.height=z+"px",he.style.minHeight="0",Se.style.flex="none",Se.style.height=A+"px",Se.style.minHeight="0",Se.style.display="flex",Se.style.overflow="hidden"}const ke=document.getElementById("col-grid");ke&&(ke.style.height="100%",ke.style.flexShrink="0",ke.style.overflowX="auto",ke.style.overflowY="hidden",ke.style.alignItems="center",ke.style.width="100%"),re.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:center center">'+B($[ae])+"</div>";var _t=re.querySelector("[data-card-id],[data-form-id],[data-gc-id]");_t&&_t.addEventListener("click",function(){R($[ae])}),requestAnimationFrame(function(){var $e=document.getElementById("big-card-inner");if(!(!$e||!re)){var Ce=re.clientHeight,qe=re.clientWidth-16,Ge=$e.offsetHeight,_=$e.offsetWidth;if(Ge>0&&_>0&&Ce>40){var z=Math.min(Ce/Ge,qe/_);$e.style.transform="scale("+z.toFixed(3)+")",$e.style.transformOrigin="center center"}}}),U.innerHTML=$.map(function($e,Ce){var qe=Ce===ae,Ge="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+(qe?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+Ce+'" style="'+Ge+'">'+N($e,qe)+"</div>"}).join(""),U.querySelectorAll(".col-mini-item").forEach(function($e){$e.addEventListener("click",function(){ae=Number($e.dataset.idx),ge(),$e.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})}),requestAnimationFrame(function(){var $e=ke?ke.clientHeight:0,Ce=U.querySelector(".col-mini-item > div");if(!(!$e||!Ce)){var qe=Ce.style.zoom;Ce.style.zoom="1";var Ge=Ce.offsetHeight;if(Ce.style.zoom=qe,!(Ge<=0)){var _=$e/Ge;U.querySelectorAll(".col-mini-item > div").forEach(function(z){z.style.zoom=_.toFixed(4)})}}})}ge()}function ce($,B){var N=window.innerWidth>=768?.76:.54,R;if(!$||$._fake){var oe=$?$.player:null;if(!oe)return"";R=bn(oe)}else R=zi($,"");var q=$&&!$._fake?h[$.player&&$.player.id]||1:0,U=q>0?'<div style="position:absolute;top:6px;right:6px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:11px;font-weight:900;padding:2px 7px;z-index:3;box-shadow:0 1px 4px rgba(0,0,0,0.4)">×'+q+"</div>":"";return'<div style="display:inline-block;position:relative;zoom:'+N+';pointer-events:none;line-height:0">'+U+R+"</div>"}function fe($,B,N){var R=N>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+N+"</div>":"",oe=!!B,q=Ai($,Mt[$],{width:160});return oe||(q='<div style="filter:grayscale(1);opacity:0.5">'+q+"</div>"),"<div "+(B?'data-form-id="'+B.id+'"':"")+' style="position:relative;cursor:pointer">'+R+q+"</div>"}function C($,B){var N=window.innerWidth>=768?.76:.54,R=Ai($,Mt[$],{width:140});return B||(R='<div style="filter:grayscale(1);opacity:0.45">'+R+"</div>"),'<div style="display:inline-block;zoom:'+N+';line-height:0;pointer-events:none">'+R+"</div>"}function Y($){if(P){const B=L();if(!B.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucun joueur.</div>';return}const N=B.map(R=>c.find(oe=>oe.player.id===R.id)||{_fake:!0,player:R,id:"fake-"+R.id});V(N,R=>R._fake?bn(R.player):zi(R,""),R=>R._fake?ce({player:R.player,id:"x",_fake:!0}):ce(R),R=>{R._fake||yn(R,c,h,t)})}else{const B=Q();if(!B.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const N={};B.forEach(oe=>{const q=oe.player.id;N[q]||(N[q]=[]),N[q].push(oe)});const R=Object.values(N).map(oe=>Jo(oe));V(R,oe=>{const q=h[oe.player.id]||1,U=q>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${q}</div>`:"",ae=c.filter(ge=>ge.player.id===oe.player.id&&ge.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return zi(oe,U+ae)},oe=>ce(oe),oe=>yn(oe,c,h,t))}}function ue($){const B=P?y:[...E];if(!B.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const N=B.map(R=>({formation:R,card:f.find(oe=>oe.formation===R)||null,owned:E.has(R)}));V(N,({formation:R,card:oe,owned:q})=>fe(R,q?oe:null,q?f.filter(U=>U.formation===R).length:0),({formation:R,owned:oe})=>C(R,oe),({card:R,owned:oe})=>{oe&&R&&tr(R,f,t,r)},"#1A6B3C",5)}function pe($){const B=Object.keys(g),N=P?B.length?B:w:[...m];if(!N.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const R=N.map(oe=>({type:oe,gc:je[oe]||{icon:"⚡",desc:""},def:g[oe]||null,owned:m.has(oe),card:l.find(q=>q.gc_type===oe)||null}));V(R,({type:oe,gc:q,def:U,owned:re,card:ae})=>{var ke;const ge=(U==null?void 0:U.name)||oe,ze=re?l.filter(_t=>_t.gc_type===oe).length:0,he=ze>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${ze}</div>`:"",Ie=(U==null?void 0:U.effect)||q.desc||"",Se=U!=null&&U.image_url?`/icons/${U.image_url}`:((ke=U==null?void 0:U.club)==null?void 0:ke.logo_url)||(U!=null&&U.country_code?`https://flagsapi.com/${U.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null);let Be=tt(ge,Se,q.icon,Ie,{width:160,onClick:re});return re||(Be=`<div style="filter:grayscale(1);opacity:0.5">${Be}</div>`),`<div ${re&&ae?`data-gc-id="${ae.id}" data-gc-type="${oe}"`:""} style="position:relative">${he}${Be}</div>`},({type:oe,gc:q,def:U,owned:re})=>{const ae=window.innerWidth>=768?.76:.54,ge=(U==null?void 0:U.name)||oe,ze=(U==null?void 0:U.effect)||q.desc||"",he=U!=null&&U.image_url?`/icons/${U.image_url}`:null;let Ie=tt(ge,he,q.icon,ze,{width:140});return re||(Ie=`<div style="filter:grayscale(1);opacity:0.45">${Ie}</div>`),`<div style="display:inline-block;zoom:${ae};line-height:0;pointer-events:none">${Ie}</div>`},({type:oe,owned:q,def:U})=>{q&&er(oe,U,r)},"#7a28b8",5)}function O($){const B="#4FC3F7",N="/";if(!b.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Stade.<br><small>Ouvre un booster Stade !</small></div>';return}const R={};b.forEach(q=>{const U=q.stadium_id||"?";(R[U]=R[U]||[]).push(q)});const oe=Object.entries(R).map(([q,U])=>({sid:q,def:u[q]||null,count:U.length,card:U[0]}));V(oe,({def:q,count:U})=>{var Ie,Se;const re=(q==null?void 0:q.name)||"?",ae=((Ie=q==null?void 0:q.club)==null?void 0:Ie.encoded_name)||(q==null?void 0:q.country_code)||"—",ge=q!=null&&q.image_url?`${N}icons/${q.image_url}`:((Se=q==null?void 0:q.club)==null?void 0:Se.logo_url)||(q!=null&&q.country_code?`https://flagsapi.com/${q.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),ze=U>1?`<div style="position:absolute;top:8px;right:8px;background:#333;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${U}</div>`:"",he=`${ae}<br>+10 ⭐ joueurs alliés`;return`<div style="position:relative">${ze}${ei(re,ge,he,{width:160})}</div>`},({def:q})=>{var he,Ie;const U=window.innerWidth>=768?.76:.54,re=(q==null?void 0:q.name)||"?",ae=((he=q==null?void 0:q.club)==null?void 0:he.encoded_name)||(q==null?void 0:q.country_code)||"—",ge=q!=null&&q.image_url?`${N}icons/${q.image_url}`:((Ie=q==null?void 0:q.club)==null?void 0:Ie.logo_url)||(q!=null&&q.country_code?`https://flagsapi.com/${q.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),ze=`${ae}<br>+10 ⭐`;return`<div style="display:inline-block;zoom:${U};line-height:0;pointer-events:none">${ei(re,ge,ze,{width:140})}</div>`},null,B,5)}e.querySelectorAll(".col-tab-btn").forEach($=>{$.addEventListener("click",()=>{M=$.dataset.tab,k="Tous",S="",P=!1,e.querySelectorAll(".col-tab-btn").forEach(B=>{const N=B.dataset.tab===M;B.style.borderBottomColor=N?"var(--green)":"transparent",B.style.color=N?"var(--green)":"var(--tile-fg-dim)"}),T(),K()})}),T(),K()}function er(e,t,i){const n=je[e]||{icon:"⚡",desc:"Effet spécial."},o=(t==null?void 0:t.name)||e,r=(t==null?void 0:t.effect)||n.desc,a=t!=null&&t.image_url?`/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
      ${tt(o,a,n.icon,r,{width:200})}
      <div style="background:#fff3cd;border-radius:10px;padding:10px 14px;width:100%">
        <div style="font-size:12px;color:#856404">⚠️ Cette carte est à <b>usage unique</b>. Une fois jouée en match, elle est définitivement supprimée de ta collection.</div>
      </div>
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const li=1e3;function tr(e,t,i,n){var g,p,u;const{state:o,toast:r,closeModal:a,navigate:s,refreshProfile:d}=i,c=e.formation,l={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function f(){const y=Mt[c]||{},w=Oi[c]||[],h=290,E=360,m=20;function M(S){const P=y[S];return P?{x:P.x*h,y:P.y*E}:null}let k=`<svg width="${h}" height="${E}" viewBox="0 0 ${h} ${E}" xmlns="http://www.w3.org/2000/svg">`;for(const[S,P]of w){const J=M(S),ie=M(P);!J||!ie||(k+=`<line x1="${J.x}" y1="${J.y}" x2="${ie.x}" y2="${ie.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const S of Object.keys(y)){const P=M(S);if(!P)continue;const J=S.replace(/\d+/,""),ie=l[J]||"#555";k+=`<circle cx="${P.x}" cy="${P.y}" r="${m}" fill="${ie}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,k+=`<text x="${P.x}" y="${P.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${J}</text>`}return k+="</svg>",k}const b=t.filter(y=>y.formation===c);b.length;const x=!e.is_for_sale;n(`Formation ${c}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    <div style="margin-top:16px;border-top:1px solid var(--tile-border);padding-top:14px">
      ...
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px">
        ${f.filter(O=>O.id!==e.id).map((O,$)=>{const B=ie[O.id]||[],N=O.is_for_sale,R=B.length?B[B.length-1]:null,oe=O.evolution_bonus||0,q=R?R.source==="booster"?"Booster":R.price?R.price.toLocaleString("fr")+" cr.":"—":"—",U=R?new Date(R.transferred_at).toLocaleDateString("fr",{day:"2-digit",month:"2-digit",year:"numeric"}):"",re=80,ae=Math.round(re*657/507),ge=Te({...l,_evolution_bonus:oe},{width:re});return`
            <div class="exemplaire-row" data-card-id="${O.id}" data-card-idx="${$}"
              ...
              <input type="checkbox" class="expl-check"
                data-id="${O.id}" data-evo="${oe}" data-note="${di(l,l.job)}"
                ${N?"disabled":""}
                style="display:none">
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(fe=document.getElementById("close-detail"))==null||fe.addEventListener("click",s);let L=new Set;const T=l.rarity==="pepite"?1.3:l.rarity==="papyte"?.7:1;function K(){let O=0;return document.querySelectorAll(".expl-check:checked").forEach($=>{if($.dataset.id===e.id)return;const N=Number($.dataset.evo)||0,R=Number($.dataset.note)||0;O+=R+N}),Math.round(O*T)}const V=()=>{const O=L.size,$=document.getElementById("sell-action-panel");if(!$)return;$.style.display=O>0?"block":"none",document.getElementById("sell-selected-count").textContent=O;const B=document.getElementById("evolve-btn");if(B){const N=K();B.textContent=N>0?`⬆️ Évoluer (+${N})`:"⬆️ Évoluer"}};document.querySelectorAll(".expl-check").forEach(O=>{O.addEventListener("change",()=>{const $=O.dataset.id;O.checked?L.add($):L.delete($);const B=O.closest(".exemplaire-row");if(B){const N=B.querySelector(".expl-sel-overlay"),R=B.querySelector(".expl-sel-check");N&&(N.style.display=O.checked?"block":"none"),R&&(R.style.display=O.checked?"flex":"none"),B.style.transform=O.checked?"scale(1.05)":"scale(1)"}V()})}),document.querySelectorAll(".exemplaire-row").forEach(O=>{O.addEventListener("click",$=>{if($.target.tagName==="INPUT")return;const B=O.querySelector(".expl-check");B&&!B.disabled&&(B.checked=!B.checked,B.dispatchEvent(new Event("change")))})}),(C=document.getElementById("evolve-btn"))==null||C.addEventListener("click",async()=>{if(b<=1)return;const O=[...L];if(!O.length)return;if(L.has(e.id)){const re=document.createElement("div");re.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",re.innerHTML=`
  ...
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
    ...
    </div>`;u("⚽ Choisir une formation",w),document.querySelectorAll("#modal-body [data-forma]").forEach(h=>{h.addEventListener("click",()=>{t.formation=h.dataset.forma;const E=wn(t.formation),m={};E.forEach(M=>{t.slots[M]&&(m[M]=t.slots[M])}),t.slots=m,y(),wt(e,t,i)})})};e.querySelectorAll("#formation-mobile-btn, #formation-pc-btn").forEach(u=>u.addEventListener("click",x)),e.querySelectorAll("#add-stad-btn-pc, #add-stad-btn").forEach(u=>u.addEventListener("click",()=>or(t,e,i))),e.querySelectorAll("#save-deck").forEach(u=>u.addEventListener("click",()=>sr(t,i))),e.querySelectorAll("[data-remove-sub]").forEach(u=>{u.addEventListener("click",()=>{t.subs=t.subs.filter(y=>y!==u.dataset.removeSub),wt(e,t,i)})}),e.querySelectorAll("#add-sub-btn").forEach(u=>u.addEventListener("click",()=>ar(t,e,i)))}function nr(e,t,i,n){var h,E,m;const o=window.innerWidth>=900,r=e.querySelector(o?"#deck-field-pc":"#deck-field-mobile");if(!r)return;const a=(h=t.stadiumCards)==null?void 0:h.find(M=>M.id===t.stadiumCardId),s=a&&((E=t.stadDefMap)==null?void 0:E[a.stadium_id])||null,d=Mt[t.formation]||{},c=Ni(t.formation)||[],l={};for(const M of i){const k=t.slots[M],S=k?t.playerCards.find(P=>P.id===k):null;S!=null&&S.player?l[M]={...S.player,_evolution_bonus:S.evolution_bonus||0,face:S.player.face||null}:l[M]=null}const f=window.innerWidth>=900,b=f?window.innerWidth-280:window.innerWidth-20,x=f?Math.min(b,860):b,g=Math.round(f?x*.82:x*.85),p=f?84:44;let u="";for(const[M,k]of c){const S=d[M],P=d[k];if(!S||!P)continue;const J=S.x*x,ie=Math.round(.03*g+S.y*.85*g),Q=P.x*x,L=Math.round(.03*g+P.y*.85*g),T=l[M],K=l[k],V=Ht(T,K);V==="#ff3333"||V==="#cc2222"?u+=`<line x1="${J.toFixed(1)}" y1="${ie.toFixed(1)}" x2="${Q.toFixed(1)}" y2="${L.toFixed(1)}" stroke="${V}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(u+=`<line x1="${J.toFixed(1)}" y1="${ie.toFixed(1)}" x2="${Q.toFixed(1)}" y2="${L.toFixed(1)}" stroke="${V}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,u+=`<line x1="${J.toFixed(1)}" y1="${ie.toFixed(1)}" x2="${Q.toFixed(1)}" y2="${L.toFixed(1)}" stroke="${V}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let y="";const w=Math.round(p*657/507);for(const M of i){const k=d[M];if(!k)continue;const S=l[M],P=k.x*x;k.y*g;const J=Math.round(.03*g+k.y*(.85*g)),ie=Math.round(P-p/2),Q=Math.round(J-w/2);if(S){const L=M.replace(/\d+/,""),T=s&&(s.club_id&&String(S.club_id)===String(s.club_id)||s.country_code&&S.country_code===s.country_code),K=Te({...S,_evolution_bonus:S._evolution_bonus||0},{width:p,showStad:!0,stadDef:s,role:L});T&&((m=s.club)!=null&&m.logo_url||s.image_url),y+=`<div style="position:absolute;left:${ie}px;top:${Q}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${M}">
        <div style="position:relative">${K}</div>
      </div>`}else{const L=M.replace(/\d+/,"");y+=`<div style="position:absolute;left:${ie}px;top:${Q}px;width:${p}px;height:${w}px;
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff;position:relative">
      ...
      <div id="deck-bottom-bar" style="flex-shrink:0;padding:10px 14px 14px;display:flex;flex-direction:column;gap:8px;background:rgba(0,0,0,0.2)">
        ...
        stroke="${k}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const g=typeof window<"u"&&window.innerWidth>=900?Math.min(Math.max(81,Math.round(o*.225)),117):Math.max(44,Math.round(o*.168)),p=Math.round(g*657/507);for(const[y,w]of Object.entries(l)){const h=b(y);if(!h||!w)continue;const E=y.replace(/[0-9]/g,""),m=a.includes(w.cardId),M=i==="attack"&&(["MIL","ATT"].includes(E)||m)&&!w.used||i==="defense"&&["GK","DEF","MIL"].includes(E)&&!w.used,k=n.includes(w.cardId);let S=w.boost||0,P=!1;w.stadiumBonus&&(i==="attack"&&(E==="ATT"||E==="MIL")||i==="defense"&&(E==="GK"||E==="DEF"||E==="MIL")?(S+=10,P=!0):i||(S+=10,P=!0));const J=Math.round(h.x-g/2),ie=Math.round(h.y-p/2);if(w.used){x+=`<image href="${`${typeof import.meta<"u"&&"/"||"/"}icons/carte-dos.png`}" x="${J}" y="${ie}" width="${g}" height="${p}" preserveAspectRatio="xMidYMid slice" class="match-used-hit" data-card-id="${w.cardId}" data-role="${E}" style="cursor:pointer"/>`;continue}const Q=Te({...w,_evolution_bonus:0,stadiumBonus:!1},{width:g,showStad:!1,stadDef:null,role:E,extraNote:S,_cardOffset:30,_forceStadColor:P}),L=k?`position:absolute;top:30px;left:0;width:${g}px;height:${p}px;outline:3px solid #FFD700;outline-offset:2px;border-radius:8px;pointer-events:none;`:"";x+=`<foreignObject x="${J-2}" y="${ie-30}" width="${g+8}" height="${p+60}" style="overflow:visible">
      <div xmlns="http://www.w3.org/1999/xhtml" style="position:relative">
        ${Q}
        ${k?`<div style="${L}"></div>`:""}
      </div>
    </foreignObject>`,M&&(x+=`<rect x="${J}" y="${ie}" width="${g}" height="${p}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${k?"selected":""}" data-card-id="${w.cardId}" data-role="${E}" style="cursor:pointer"/>`)}const u=s!==null?s:Math.round(Math.max(g*.7,80));return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-u} ${-u} ${o+u*2} ${r+u*2}" width="100%" style="display:block;width:100%;margin:0 auto">
    ${x}
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const Y=t.pendingAttack;let ue="";if(t.selected.length>0){const pe=t.selected.map($=>({...(t.homeTeam[$._role]||[]).find(N=>N.cardId===$.cardId)||$,_line:$._role})),O=oi(pe,t.modifiers.home,t.formation);ue=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
    ...
    ${(()=>{const C=window.innerWidth>=700,Y=(q,U,re)=>{var Se,Be;const ae=(t.gcDefs||[]).find(ke=>ke.name===q.gc_type),ge=(ae==null?void 0:ae.name)||q.gc_type,ze=(ae==null?void 0:ae.effect)||((Se=je[q.gc_type])==null?void 0:Se.desc)||"",he=ae!=null&&ae.image_url?`/icons/${ae.image_url}`:null,Ie=((Be=je[q.gc_type])==null?void 0:Be.icon)||"⚡";return`<div class="gc-mini" data-gc-id="${q.id}" data-gc-type="${q.gc_type}" style="flex-shrink:0;cursor:pointer">
        ...
        <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
          ${Nt(t.homeTeam,t.formation,t.phase,n,C?1300:It(),C?600:Gt(),g)}
        </div>
      </div>`;return C?`
  ...
  <div id="match-history-panel">
    ...
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      ...
      ${wi(e.aiTeam,e.formation,null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.1))}
            ...
            </div>`}}catch(Z){console.error("[Ranked] MMR update error:",Z)}(te=document.getElementById("pvp-end-overlay"))==null||te.remove();const D=document.createElement("div");D.id="pvp-end-overlay",D.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;color:#fff;padding:24px;text-align:center;overflow-y:auto";const F=I?"🤝":j?"🏆":"😞",H=I?"MATCH NUL":j?"VICTOIRE !":"DÉFAITE",X=I?"#fff":j?"#FFD700":"#ff6b6b";D.innerHTML=`
      ...
      ${G}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${l?"🏆 Retour à la Mini League":a?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(D),(ee=D.querySelector("#pvp-end-home"))==null||ee.addEventListener("click",()=>{D.remove(),it(e),l?x("mini-league",{openLeagueId:l}):x(a?"ranked":"home")})}const Q=v.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},_=>{var A;const z=_.new;try{if(z.status==="finished"||z.forfeit){if(M)return;M=!0,k&&(clearInterval(k),k=null),Ue(),z.game_state&&(m=z.game_state),ie(z);return}if(z.game_state){const j=m;m=z.game_state;const I=m._lastGC;if(I&&I.seq&&!J.has(I.seq)&&(J.add(I.seq),I.by!==p)){$(I.type,I.by,()=>Y());return}const G=j[p+"Score"]||0,D=j[u+"Score"]||0,F=m[p+"Score"]||0,H=m[u+"Score"]||0,X=F>G,te=H>D;if((X||te)&&!P.has(m.round)){P.add(m.round);const ee=[...m.log||[]].reverse().find(de=>de.isGoal),Z=((ee==null?void 0:ee.homePlayers)||[]).map(de=>({name:de.name,note:de.note,portrait:de.portrait,job:de.job}));!!((A=ee==null?void 0:ee.text)!=null&&A.includes("DERNIER CORNER"))?ze().then(()=>{Qe(Z,F,H,X,()=>Y())}):Qe(Z,F,H,X,()=>Y());return}Y()}}catch(j){console.error("[PvP] crash:",j)}}).subscribe();async function L(_){Object.assign(m,_),m.lastActionAt=new Date().toISOString();const{error:z}=await v.from("matches").update({game_state:m}).eq("id",i);z&&g("Erreur de synchronisation","error");try{Y()}catch(A){console.error("[PvP] renderPvpScreen crash:",A)}}async function T(){if(M)return;M=!0,Tt(),Ue(),k&&(clearInterval(k),k=null);const _=n?h.away_id:h.home_id,z=n?"p2":"p1",A=n?"p1":"p2",j={...m,[z+"Score"]:3,[A+"Score"]:0,phase:"finished"},I=j.p1Score||0,G=j.p2Score||0;try{await v.from("matches").update({game_state:j}).eq("id",i);const{data:D,error:F}=await v.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:_,p_home_score:I,p_away_score:G,p_forfeit:!0});if(F&&console.error("[PvP] finish_pvp_match (forfeit):",F),l)try{await v.rpc("check_and_finish_mini_league",{p_league_id:l})}catch(H){console.error("[PvP] check_and_finish_mini_league (forfeit):",H)}try{const{data:H}=await v.rpc("apply_match_rewards",{p_match_id:i});H!=null&&H.success&&!(H!=null&&H.skipped)&&typeof t.refreshProfile=="function"&&await t.refreshProfile()}catch(H){console.error("[PvP] apply_match_rewards (forfeit):",H)}}catch{}try{v.removeChannel(Q)}catch{}setTimeout(()=>{it(e),l?x("mini-league",{openLeagueId:l}):x("home")},800)}const K={BOOST_STAT:({value:_=1,count:z=1,roles:A=[]},j,I)=>{const G=j[p+"Team"],D=Object.entries(G).filter(([F])=>!A.length||A.includes(F)).flatMap(([F,H])=>H.filter(X=>!X.used).map(X=>({...X,_line:F})));if(!D.length){j.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),I(j);return}V(D,z,`Choisir ${z} joueur(s) à booster (+${_})`,F=>{F.forEach(H=>{const X=(G[H._line]||[]).find(te=>te.cardId===H.cardId);X&&(X.boost=(X.boost||0)+_,j.log.push({text:`⚡ +${_} sur ${X.name}`,type:"info"}))}),j[p+"Team"]=G,I(j)})},DEBUFF_STAT:({value:_=1,count:z=1,roles:A=[],target:j="ai"},I,G)=>{const D=j==="home"?p:u,F=I[D+"Team"],H=j==="home"?"allié":"adverse",X=Object.entries(F).filter(([te])=>!A.length||A.includes(te)).flatMap(([te,ee])=>ee.map(Z=>({...Z,_line:te})));if(!X.length){I.log.push({text:`🎯 Aucun joueur ${H}`,type:"info"}),G(I);return}V(X,z,`Choisir ${z} joueur(s) ${H}(s) (-${_})`,te=>{te.forEach(ee=>{const Z=(F[ee._line]||[]).find(se=>se.cardId===ee.cardId);Z&&(Z.boost=(Z.boost||0)-_,I.log.push({text:`🎯 -${_} sur ${Z.name}`,type:"info"}))}),I[D+"Team"]=F,G(I)})},GRAY_PLAYER:({count:_=1,roles:z=[],target:A="ai"},j,I)=>{const G=A==="home"?p:u,D=j[G+"Team"],F=A==="home"?"allié":"adverse",H=Object.entries(D).filter(([X])=>!z.length||z.includes(X)).flatMap(([X,te])=>te.filter(ee=>!ee.used).map(ee=>({...ee,_line:X})));if(!H.length){j.log.push({text:`❌ Aucun joueur ${F}`,type:"info"}),I(j);return}V(H,_,`Choisir ${_} joueur(s) ${F}(s) à exclure`,X=>{const te="usedCardIds_"+G,ee=new Set(j[te]||[]);X.forEach(Z=>{const se=(D[Z._line]||[]).find(de=>de.cardId===Z.cardId);se&&(se.used=!0,ee.add(Z.cardId),j.log.push({text:`❌ ${se.name} exclu !`,type:"info"}))}),j[te]=[...ee],j[G+"Team"]=D,I(j)})},REVIVE_PLAYER:({count:_=1,roles:z=[]},A,j)=>{const I=A[p+"Team"],G=Object.entries(I).filter(([D])=>!z.length||z.includes(D)).flatMap(([D,F])=>F.filter(H=>H.used).map(H=>({...H,_line:D})));if(!G.length){A.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),j(A);return}V(G,_,`Choisir ${_} joueur(s) à ressusciter`,D=>{D.forEach(F=>{const H=(I[F._line]||[]).find(X=>X.cardId===F.cardId);H&&(H.used=!1,A.log.push({text:`💫 ${H.name} ressuscité !`,type:"info"}))}),A[p+"Team"]=I,j(A)})},REMOVE_GOAL:({},_,z)=>{const A=u+"Score";_[A]>0?(_[A]--,_.log.push({text:"🚫 Dernier but annulé !",type:"info"})):_.log.push({text:"🚫 Aucun but à annuler",type:"info"}),z(_)},ADD_GOAL_DRAW:({},_,z)=>{_[p+"Score"]===_[u+"Score"]?(_[p+"Score"]++,_.log.push({text:"🎯 But bonus !",type:"info"})):_.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),z(_)},ADD_SUB:({value:_=1},z,A)=>{z.maxSubs=(z.maxSubs||3)+_,z.log.push({text:`🔄 +${_} remplacement(s)`,type:"info"}),A(z)},CUSTOM:({},_,z)=>z(_)};function V(_,z,A,j){const I=document.createElement("div");I.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let G=[];function D(){var H,X;const F=_.map(te=>{const ee=te._line||te.job||"MIL",Z=G.find(de=>de.cardId===te.cardId),se=Te({...te,_evolution_bonus:0},{width:90,showStad:!0,role:ee,extraNote:te.boost||0});return`<div class="pp-item" data-cid="${te.cardId}"
        ...
        </div>`,(H=I.querySelector("#pp-close"))==null||H.addEventListener("click",()=>I.remove()),I.querySelectorAll(".pp-item").forEach(te=>{te.addEventListener("click",()=>{const ee=te.dataset.cid,Z=_.find(de=>de.cardId===ee),se=G.findIndex(de=>de.cardId===ee);Z&&(se>-1?G.splice(se,1):G.length<z&&G.push(Z),D())})}),(X=I.querySelector("#pp-confirm"))==null||X.addEventListener("click",()=>{I.remove(),j(G)})}D(),document.body.appendChild(I)}async function ce(_,z){const j=(m["gcCardsFull_"+p]||[]).find(F=>F.id===_),I=(j==null?void 0:j._gcDef)||(m.gcDefs||[]).find(F=>{var H;return F.name===z||((H=F.name)==null?void 0:H.toLowerCase().trim())===(z==null?void 0:z.toLowerCase().trim())}),G=[...m["usedGc_"+p]||[],_],D={type:z,by:p,seq:(m._gcAnimSeq||0)+1};J.add(D.seq),$(z,p,async()=>{if(I!=null&&I.effect_type&&I.effect_type!=="CUSTOM"){const H=K[I.effect_type];if(H){const X={...m};H(I.effect_params||{},X,async te=>{te["usedGc_"+p]=G,te._lastGC=D,te._gcAnimSeq=D.seq,await L(te)});return}}const F={...m};switch(z){case"Remplacement+":F.maxSubs=(F.maxSubs||3)+1,F.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const H=u+"Score";F[H]>0&&(F[H]--,F.log.push({text:"🚫 But annulé",type:"info"}));break}}F["usedGc_"+p]=G,F._lastGC=D,F._gcAnimSeq=D.seq,await L(F)})}function fe(_,z){const A="usedCardIds_"+_,j=new Set(m[A]||[]);z.forEach(I=>j.add(I)),m[A]=[...j]}function C(){for(const _ of["p1","p2"]){const z=new Set(m["usedCardIds_"+_]||[]),A=m[_+"Team"];if(A)for(const j of["GK","DEF","MIL","ATT"])(A[j]||[]).forEach(I=>{I.used=z.has(I.cardId)})}}function Y(){var on,rn,an,sn,dn,ln;if(m.phase==="reveal")return ue();if(m.phase==="midfield")return O();if(m.phase==="finished")return Ge();const _=m[p+"Team"],z=m[u+"Team"];C();const A=m[p+"Score"],j=m[u+"Score"],I=m[p+"Name"],G=m[u+"Name"],D=m.phase===p+"-attack",F=m.phase===p+"-defense",H=Array.isArray(m["selected_"+p])?m["selected_"+p]:[],X=H.map(W=>W.cardId),te=window.innerWidth>=700,ee=m[p+"Subs"]||[],Z=m["usedSubIds_"+p]||[],se=ee.filter(W=>!Z.includes(W.cardId)),de=m["gcCardsFull_"+p]||[],xe=m["usedGc_"+p]||[],le=de.filter(W=>!xe.includes(W.id)),be=m.boostOwner===p&&!m.boostUsed,ve=[..._.MIL||[],..._.ATT||[]].filter(W=>!W.used),ne=[...z.MIL||[],...z.ATT||[]].filter(W=>!W.used),Ee=!ke(z),Re=ve.length===0&&ne.length===0&&!Ee,Le=(_.DEF||[]).filter(W=>!W.used),Ve=(_.GK||[]).filter(W=>!W.used);let Oe=[];D&&ve.length===0&&(Ee?(Oe=Le.map(W=>W.cardId),Le.length===0&&(Oe=Oe.concat(Ve.map(W=>W.cardId)))):Re&&(Oe=[...Le,...Ve].map(W=>W.cardId)));function nt(W,me,Ae){var Me,ft;const we=W._gcDef,qt=(we==null?void 0:we.name)||W.gc_type,at=(we==null?void 0:we.effect)||((Me=je[W.gc_type])==null?void 0:Me.desc)||"",Fe=we!=null&&we.image_url?`/icons/${we.image_url}`:null,Ne=((ft=je[W.gc_type])==null?void 0:ft.icon)||"⚡";return`<div class="pvp-gc-mini" data-gc-id="${W.id}" data-gc-type="${W.gc_type}" style="flex-shrink:0;cursor:pointer">
      ...
      </div>`;function Bt(){const W=e.querySelector(".match-screen");if(!W)return;const me=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);W.style.height=me+"px",W.style.minHeight=me+"px",W.style.maxHeight=me+"px",W.style.overflow="hidden";const Ae=e.querySelector("#mobile-action-bar"),we=e.querySelector("#mobile-play-area");Ae&&we&&(we.style.paddingBottom=Ae.offsetHeight+"px")}if(Bt(),setTimeout(Bt,120),setTimeout(Bt,400),S||(S=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",Bt),window.visualViewport.addEventListener("scroll",Bt)),window.addEventListener("resize",Bt)),function(){const me=e.querySelector("#match-field .terrain-wrapper svg")||e.querySelector(".terrain-wrapper svg");if(!me)return;const Ae=me.closest("#match-terrain-wrap");Ae&&(Ae.style.cssText="position:relative;width:100%;height:100%;padding:0"),me.removeAttribute("width"),me.removeAttribute("height"),me.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",me.setAttribute("preserveAspectRatio","xMidYMid meet")}(),m._pvpResizeBound||(m._pvpResizeBound=!0,window.addEventListener("resize",()=>{const W=e.querySelector(".terrain-wrapper svg");if(W){const me=W.closest("#match-terrain-wrap");me&&(me.style.cssText="position:relative;width:100%;height:100%;padding:0"),W.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0"}})),e.querySelectorAll(".match-slot-hit").forEach(W=>{W.addEventListener("click",()=>{if(!D&&!F)return;const me=W.dataset.cardId,Ae=W.dataset.role,we=(_[Ae]||[]).find(Ne=>Ne.cardId===me);if(!we||we.used)return;const qt=Oe.includes(me);if(D&&!["MIL","ATT"].includes(Ae)&&!qt)return;Array.isArray(m["selected_"+p])||(m["selected_"+p]=[]);const at=m["selected_"+p],Fe=at.findIndex(Ne=>Ne.cardId===me);Fe>-1?at.splice(Fe,1):at.length<3&&at.push({...we,_role:Ae}),Y()})}),e.querySelectorAll(".match-used-hit").forEach(W=>{W.addEventListener("click",()=>R(W.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(W=>{W.addEventListener("click",()=>R())}),(on=e.querySelector("#pvp-sub-open"))==null||on.addEventListener("click",()=>R()),e.querySelectorAll(".pvp-gc-mini").forEach(W=>{D?W.addEventListener("click",()=>B(W.dataset.gcId,W.dataset.gcType)):(W.style.opacity="0.35",W.style.cursor="default",W.addEventListener("click",()=>$t("⚡ Les Game Changers ne sont utilisables qu'en attaque","rgba(180,100,0,0.9)")))}),(rn=e.querySelector("#pvp-boost-card"))==null||rn.addEventListener("click",()=>N()),(an=e.querySelector("#pvp-action"))==null||an.addEventListener("click",W=>{D?W.currentTarget.dataset.pass==="1"||!ct(p)?re():Ie():F&&Se()}),(sn=e.querySelector("#pvp-quit"))==null||sn.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&T()}),(dn=e.querySelector("#pvp-view-opp"))==null||dn.addEventListener("click",()=>q()),(ln=e.querySelector("#pvp-toggle-history"))==null||ln.addEventListener("click",()=>U()),k&&(clearInterval(k),k=null),Ue(),(D||F)&&!M){let W=30,me=!1;const Ae=()=>document.getElementById("pvp-timer"),we=()=>{Ae()&&(Ae().textContent=W+"s",Ae().style.color=me?"#ff4444":"#fff")};we(),k=setInterval(()=>{W--,W<0?me?(clearInterval(k),k=null,Ue(),D&&!ct(p)?re():T()):(me=!0,W=15,we(),Mn("/sounds/timer-urgent.mp3",.6)):we()},1e3)}}function ue(){st(e),e.innerHTML=`
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="animation:gcFlipIn .7s cubic-bezier(.34,1.56,.64,1) both,gcGlow 1.8s ease infinite .7s">
        ${tt(G,F,H,D,{width:200})}
      </div>
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(ee);let Z=!1;const se=()=>{Z||(Z=!0,ee.remove(),setTimeout(()=>A&&A(),50))};ee.addEventListener("click",se),setTimeout(se,3e3)}function B(_,z){var te,ee,Z,se;const j=(m["gcCardsFull_"+p]||[]).find(de=>de.id===_),I=j==null?void 0:j._gcDef,G=(I==null?void 0:I.name)||z,D=(I==null?void 0:I.effect)||((te=je[z])==null?void 0:te.desc)||"Carte spéciale.",F=I!=null&&I.image_url?`/icons/${I.image_url}`:null,H=((ee=je[z])==null?void 0:ee.icon)||"⚡",X=document.createElement("div");X.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",X.innerHTML=`
      ${tt(G,F,H,D,{width:190})}
      <div style="display:flex;gap:12px;width:190px">
        <button id="pvp-gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
        <button id="pvp-gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
      </div>`,document.body.appendChild(X),(Z=X.querySelector("#pvp-gc-back"))==null||Z.addEventListener("click",()=>X.remove()),(se=X.querySelector("#pvp-gc-use"))==null||se.addEventListener("click",()=>{X.remove(),ce(_,z)})}function N(){var j;const _=m[p+"Team"],z=Object.entries(_).flatMap(([I,G])=>(G||[]).filter(D=>!D.used).map(D=>({...D,_line:I})));if(!z.length)return;const A=document.createElement("div");A.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",A.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
      ...
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${z.map(I=>{const G={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[I._line]||"#555",D=Pe(I,I._line)+(I.boost||0);return`<div class="bp-item" data-cid="${I.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${G};overflow:hidden;cursor:pointer">
      ...
      </div>`,document.body.appendChild(A),(j=A.querySelector("#bp-close"))==null||j.addEventListener("click",()=>A.remove()),A.querySelectorAll(".bp-item").forEach(I=>{I.addEventListener("click",async()=>{const G=I.dataset.cid,D=z.find(H=>H.cardId===G);if(!D)return;const F=(_[D._line]||[]).find(H=>H.cardId===G);F&&(F.boost=(F.boost||0)+m.boostValue),A.remove(),await L({[p+"Team"]:_,boostUsed:!0})})})}function R(_=null){var se,de;if(!(m.phase===p+"-attack")){g("Remplacement uniquement avant votre attaque","warning");return}const A=m[p+"Team"],j=m["usedSubIds_"+p]||[],I=m.maxSubs||3;if(j.length>=I){g(`Maximum ${I} remplacements atteint`,"warning");return}const G=Object.entries(A).flatMap(([xe,le])=>(le||[]).filter(be=>be.used).map(be=>({...be,_line:xe}))),D=(m[p+"Subs"]||[]).filter(xe=>!j.includes(xe.cardId));if(!G.length){g("Aucun joueur utilisé à remplacer","warning");return}if(!D.length){g("Aucun remplaçant disponible","warning");return}let F=Math.max(0,G.findIndex(xe=>xe.cardId===_));const H=((se=G[F])==null?void 0:se._line)||((de=G[F])==null?void 0:de.job);let X=Math.max(0,D.findIndex(xe=>xe.job===H)),te=!1;const ee=document.createElement("div");ee.id="pvp-sub-overlay",ee.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function Z(){var Re,Le,Ve,Oe,nt,Ut;const xe=G[F],le=D[X],be=Math.min(130,Math.round((window.innerWidth-90)/2)),ve=Math.round(be*1.35),ne=De=>`background:rgba(255,255,255,0.12);border:none;color:${De?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${De?"default":"pointer"};flex-shrink:0`;ee.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        <div id="pin-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
          <div style="font-size:9px;color:#00ff88;letter-spacing:2px;text-transform:uppercase;font-weight:700">Joueur qui entre</div>
          <button id="pin-up" style="${ne(X===0)}" ${X===0?"disabled":""}>▲</button>
          <div>${le?vt({...le,used:!1,boost:0},be,ve):"<div>—</div>"}</div>
          <button id="pin-down" style="${ne(X>=D.length-1)}" ${X>=D.length-1?"disabled":""}>▼</button>
          <div style="font-size:10px;color:rgba(255,255,255,0.35)">${X+1}/${D.length}</div>
        </div>
        <div id="pout-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
          <div style="font-size:9px;color:#ff6b6b;letter-spacing:2px;text-transform:uppercase;font-weight:700">Joueur qui sort</div>
          <button id="pout-up" style="${ne(F===0)}" ${F===0?"disabled":""}>▲</button>
          <div>${xe?vt({...xe,used:!1,boost:0},be,ve):"<div>—</div>"}</div>
          <button id="pout-down" style="${ne(F>=G.length-1)}" ${F>=G.length-1?"disabled":""}>▼</button>
          <div style="font-size:10px;color:rgba(255,255,255,0.35)">${F+1}/${G.length}</div>
        </div>
      </div>
      <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
        <button id="psub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
      </div>`,(Re=ee.querySelector("#psub-close"))==null||Re.addEventListener("click",()=>ee.remove()),(Le=ee.querySelector("#pout-up"))==null||Le.addEventListener("click",()=>{F>0&&(F--,Z())}),(Ve=ee.querySelector("#pout-down"))==null||Ve.addEventListener("click",()=>{F<G.length-1&&(F++,Z())}),(Oe=ee.querySelector("#pin-up"))==null||Oe.addEventListener("click",()=>{X>0&&(X--,Z())}),(nt=ee.querySelector("#pin-down"))==null||nt.addEventListener("click",()=>{X<D.length-1&&(X++,Z())});const Ee=(De,Ye,He,pt)=>{const ot=ee.querySelector("#"+De);if(!ot)return;let jt=0;ot.addEventListener("touchstart",ut=>{jt=ut.touches[0].clientY},{passive:!0}),ot.addEventListener("touchend",ut=>{const kt=ut.changedTouches[0].clientY-jt;if(Math.abs(kt)<30)return;const rt=Ye();kt<0&&rt<pt-1?(He(rt+1),Z()):kt>0&&rt>0&&(He(rt-1),Z())},{passive:!0})};Ee("pin-panel",()=>X,De=>X=De,D.length),Ee("pout-panel",()=>F,De=>F=De,G.length),(Ut=ee.querySelector("#psub-confirm"))==null||Ut.addEventListener("click",async De=>{if(De.preventDefault(),De.stopPropagation(),te)return;te=!0;const Ye=G[F],He=D[X];if(!Ye||!He)return;const pt=Ye._line,ot=(A[pt]||[]).findIndex(kt=>kt.cardId===Ye.cardId);if(ot===-1){g("Erreur : joueur introuvable","error"),ee.remove();return}const jt={...He,_line:pt,position:Ye.position,used:!1,boost:0};A[pt].splice(ot,1,jt);const ut=[...j,He.cardId];ee.remove(),oe(Ye,He,async()=>{await L({[p+"Team"]:A,[u+"Team"]:m[u+"Team"],["usedSubIds_"+p]:ut})})})}document.body.appendChild(ee),Z()}function oe(_,z,A){const j={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},I=document.createElement("div");I.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const G=(H,X,te)=>`<div style="text-align:center">
      <div style="font-size:9px;color:${X};letter-spacing:2px;text-transform:uppercase;font-weight:700;margin-bottom:6px">${te}</div>
      <div style="width:70px;height:70px;border-radius:50%;background:${j[H.job]||"#555"};border:3px solid ${X};position:relative;overflow:hidden;margin:0 auto">
      ...
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(_),(z=_.querySelector("#pvp-opp-close"))==null||z.addEventListener("click",()=>_.remove())}function U(){var A;const _=m.log||[],z=document.createElement("div");z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column",z.innerHTML=`
      ...
      </div>`,document.body.appendChild(z),(A=z.querySelector("#pvp-hist-close"))==null||A.addEventListener("click",()=>z.remove())}async function re(){if(m.phase!==p+"-attack")return;const _=p==="p1"?"p2":"p1",z=(m.round||0)+1,A=[...m.log||[]];A.push({type:"info",text:`⏭️ ${m[p+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const j=$e(m),I=ct(_),G=j||!I?"finished":_+"-attack";await L({["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},pendingAttack:null,phase:G,attacker:_,round:z,log:A}),G==="finished"&&await qe(m)}function ae(_){const z=(_.GK||[]).some(j=>!j.used),A=["DEF","MIL","ATT"].some(j=>(_[j]||[]).some(I=>!I.used));return z&&!A}function ge(_){return(_.p1Score||0)!==(_.p2Score||0)?null:ae(_.p1Team)&&!ke(_.p2Team)?{side:"p1",gk:(_.p1Team.GK||[]).find(z=>!z.used)}:ae(_.p2Team)&&!ke(_.p1Team)?{side:"p2",gk:(_.p2Team.GK||[]).find(z=>!z.used)}:null}async function ze(){const _=document.createElement("div");_.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:950;display:flex;align-items:center;justify-content:center;overflow:hidden;text-align:center;padding:24px",document.body.appendChild(_);const z=(A,j)=>new Promise(I=>{_.innerHTML=`<div style="font-size:32px;font-weight:900;color:${j};letter-spacing:2px;animation:lcFade 1.4s ease both">${A}</div>
      <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(I,1400)});await z("⚽ DERNIER CORNER","#FFD700"),await z("🧤 LE GARDIEN MONTE !","#4fc3f7"),_.remove()}async function he(_,z){const{side:A,gk:j}=_;if(!j)return;const I=A+"Score",G=(z[I]||0)+1;j.used=!0;const D=[...z.log||[]];D.push({type:"duel",isGoal:!0,homeScored:A===p,homePlayers:A==="p1"?[Dt(j)]:[],aiPlayers:A==="p2"?[Dt(j)]:[],text:`⚽ DERNIER CORNER — Le gardien de ${z[A+"Name"]} marque !`});const F=(z.round||0)+1,H=A==="p1"?"p2":"p1",X={...z,[I]:G},te=$e(X);P.add(F),await ze();const ee=A===p?G:z[p+"Score"]||0,Z=A===p?z[u+"Score"]||0:G;Qe([Dt(j)],ee,Z,A===p,async()=>{await L({[I]:G,log:D,round:F,pendingAttack:null,phase:te?"finished":H+"-attack",attacker:H,["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}}}),te&&await qe({...m,[I]:G})})}async function Ie(){const _=m[p+"Team"],z=!["GK","DEF","MIL","ATT"].some(G=>(m[u+"Team"][G]||[]).some(D=>!D.used)),A=(m["selected_"+p]||[]).map(G=>{const D=(_[G._role]||[]).find(de=>de.cardId===G.cardId)||G,F=["GK","DEF"].includes(G._role),H=_[G._role]||[],X=H.findIndex(de=>de.cardId===G.cardId),te=xt(H.length),ee=X>=0?te[X]:D._col??1,Z=m.stadiumDef||m.homeStadiumDef||null,se=D.stadiumBonus||Z&&(Z.club_id&&String(D.club_id)===String(Z.club_id)||Z.country_code&&D.country_code===Z.country_code)||!1;return{...D,_line:G._role,_col:ee,stadiumBonus:se,...F?{note_a:Math.max(1,Number(D.note_a)||0)}:{}}});if(!A.length)return;const j=ni(A,m.modifiers[p]||{},m[p+"Formation"]);fe(p,A.map(G=>G.cardId)),A.forEach(G=>{const D=(_[G._role]||[]).find(F=>F.cardId===G.cardId);D&&(D.used=!0)}),m["selected_"+p]=[],Y();const I=[...m.log||[]];if(z){if(A.length===1&&(A[0]._line==="GK"||A[0].job==="GK")&&(m.p1Score||0)===(m.p2Score||0)){await he({side:p,gk:A[0]},{...m,[p+"Team"]:_,log:I});return}const D=(m[p+"Score"]||0)+1,F=A.map(Z=>Dt(Z));I.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:F,homeTotal:j.total,aiTotal:0});const H=(m.round||0)+1,X=p==="p1"?"p2":"p1",te={...m,[p+"Team"]:_,[p+"Score"]:D},ee=$e(te);P.add(H),Qe(F,D,m[u+"Score"]||0,!0,async()=>{await L({[p+"Team"]:_,[p+"Score"]:D,["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},pendingAttack:null,phase:ee?"finished":X+"-attack",attacker:X,round:H,log:I}),ee&&await qe({...m,[p+"Score"]:D})});return}I.push({type:"pending",text:`⚔️ ${m[p+"Name"]} attaque (${j.total})`}),await L({[p+"Team"]:_,[u+"Team"]:m[u+"Team"],pendingAttack:{...j,players:A,side:p},["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},phase:u+"-defense",log:I})}async function Se(){const _=m[p+"Team"],z=(m["selected_"+p]||[]).map(le=>{const be=(_[le._role]||[]).find(Le=>Le.cardId===le.cardId)||le,ve=_[le._role]||[],ne=ve.findIndex(Le=>Le.cardId===le.cardId),Ee=xt(ve.length),Re=ne>=0?Ee[ne]:be._col??1;return{...be,_line:le._role,_col:Re}}),A=oi(z,m.modifiers[p]||{},m[p+"Formation"]);fe(p,z.map(le=>le.cardId)),z.forEach(le=>{const be=(_[le._role]||[]).find(ve=>ve.cardId===le.cardId);be&&(be.used=!0)}),m["selected_"+p]=[],Y();const j=Ji(m.pendingAttack.total,A.total,m.modifiers[p]||{}),I=m.pendingAttack.side,G=j==="attack"||(j==null?void 0:j.goal),D=I==="p1"?"p2":"p1",F=(m.round||0)+1,H=(m.pendingAttack.players||[]).map(le=>Dt(le)),X=[...m.log||[]];X.push({type:"duel",isGoal:G,homeScored:G&&I===p,text:G?`⚽ BUT de ${m[I+"Name"]} ! (${m.pendingAttack.total} vs ${A.total})`:`✋ Attaque stoppée (${m.pendingAttack.total} vs ${A.total})`,homePlayers:H,aiPlayers:z.map(le=>Dt(le)),homeTotal:m.pendingAttack.total,aiTotal:A.total});const te=G?(m[I+"Score"]||0)+1:m[I+"Score"]||0,ee={...m,[p+"Team"]:_,[I+"Score"]:te},Z=$e(ee),se=Z?"finished":D+"-attack",de=!Z&&!G?ge(ee):null,xe=async()=>{if(de){await he(de,ee);return}await L({[p+"Team"]:_,[u+"Team"]:m[u+"Team"],[I+"Score"]:te,["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},pendingAttack:null,phase:se,attacker:D,round:F,log:X}),(se==="finished"||Z)&&await qe({...m,[I+"Score"]:te})};if(G){const le=I===p,be=le?te:m[p+"Score"]||0,ve=le?m[u+"Score"]||0:te;P.add(F),Qe(H,be,ve,le,xe)}else await xe()}function Be(_){return["MIL","ATT"].some(z=>(_[z]||[]).some(A=>!A.used))}function ke(_){return["GK","DEF","MIL","ATT"].some(z=>(_[z]||[]).some(A=>!A.used))}function _t(_){return ke(_)&&!Be(_)}function ct(_){const z=m[_+"Team"],A=m[(_==="p1"?"p2":"p1")+"Team"];return!!(Be(z)||_t(z)&&!ke(A)||_t(z)&&!Be(A)&&ke(A))}function $e(_){return!ke(_.p1Team)&&!ke(_.p2Team)}function Ce(_){const z=_.p1Score||0,A=_.p2Score||0;return z===A?null:z>A?h.home_id:h.away_id}async function qe(_){Tt(),Ue();try{const z=Ce(_),A=z?h.home_id===z?h.away_id:h.home_id:null,j=_.p1Score||0,I=_.p2Score||0,{data:G,error:D}=await v.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:z,p_home_score:j,p_away_score:I,p_forfeit:!1});if(D&&console.error("[PvP] finish_pvp_match:",D),l)try{await v.rpc("check_and_finish_mini_league",{p_league_id:l})}catch(F){console.error("[PvP] check_and_finish_mini_league:",F)}z&&A&&Ur(z,A).catch(()=>{});try{const{data:F}=await v.rpc("apply_match_rewards",{p_match_id:i});F!=null&&F.success&&!(F!=null&&F.skipped)&&typeof t.refreshProfile=="function"&&await t.refreshProfile()}catch(F){console.error("[PvP] apply_match_rewards:",F)}if(typeof c=="function")try{await c({homeScore:j,awayScore:I,winnerId:z,homeId:h.home_id,awayId:h.away_id,matchId:i})}catch(F){console.error("[PvP] onMatchEnd:",F)}}catch(z){console.error("[PvP] finishMatch:",z)}}function Ge(){var I;const _=m[p+"Score"],z=m[u+"Score"],A=_>z,j=_===z;st(e),e.innerHTML=`
    ...
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:${Xt};border:1px solid ${Jt};border-radius:12px;padding:16px">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>${b.map((L,T)=>{const K=g>0&&a.status==="finished"?T===0?Math.floor(g*.7):T===1?Math.floor(g*.2):T===2?Math.floor(g*.1):0:0;return`<tr style="border-bottom:1px solid ${Lt};${L.userId===r?"background:rgba(74,222,128,0.08);":""}">
      ...
      <div style="background:${Xt};border:1px solid ${Jt};border-radius:12px;padding:16px">
        ...
        ${Array.from({length:Math.max(0,a.status==="active"?a.current_day-1:a.current_day)},(L,T)=>T+1).reverse().map(L=>{const T=(d||[]).filter(K=>K.matchday===L);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:${Ze};margin-bottom:6px">Journée ${L}</div>${T.map(K=>$n(K,f,r,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    </div>`}function b(p){var S,P,J,ie;const u=(S=p.card)==null?void 0:S.player;if(!u)return"";const y=((P=p.card)==null?void 0:P.evolution_bonus)||0,w=pi(u,u.job,y),h=u.job2?pi(u,u.job2,y):0,E=p.status==="sold",m=u.country_code?`https://flagsapi.com/${u.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,M=Ln[u.job]||"#bbb",k=u.job2?Ln[u.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${E?"opacity:0.7":""}">
    ...
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center">
      ...
      <div style="display:flex;gap:10px">
        ...
        <button id="buy-ok" style="flex:1;padding:12px;border-radius:10px;border:none;background:var(--green);color:#fff;font-size:14px;font-weight:900;cursor:pointer">Confirmer</button>
          ...
          </div>`}).join(""):'<div style="text-align:center;color:var(--tile-fg-dim);padding:40px">Aucun joueur classé.</div>'}else if(d==="mini-league"){const k=a||[];h.innerHTML=k.length>0?k.map((S,P)=>`
      ...
      `).join(""):`<div style="text-align:center;color:var(--tile-fg-dim);padding:40px">Aucun résultat de Mini League pour l'instant.</div>`}else{const k=s||[];h.innerHTML=k.length>0?k.map((S,P)=>`
        <div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${S.user_id===i.profile.id?"border:2px solid var(--yellow)":""}">
          <div style="width:32px;height:32px;border-radius:50%;background:${c(P)};color:${l(P)};display:flex;align-items:center;justify-content:center;font-weight:900;flex-shrink:0;font-size:${P<3?"16":"13"}px">${P<3?["🥇","🥈","🥉"][P]:P+1}</div>
          <div style="flex:1;min-width:0">
            <div style="font-weight:700;color:var(--tile-fg-on-page);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${S.pseudo}</div>
  ...
  <div class="page">
    ...
    <nav class="top-nav">
      <div class="logo" id="nav-logo" title="Manager Wars v2026.07.24-1042" style="cursor:pointer">
