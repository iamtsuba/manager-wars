...
import{s as _,q as Ut,t as Ri,F as Oi,o as tt,n as ti,g as At,r as Te,m as zt,u as zi,j as Hi,v as ao,w as so,T as Li,p as lo,x as co,d as po}from"./special-cards-DZxbxWIu.js";const uo="/";function ln(e,{navigate:t,toast:i}){let n="login";const r=()=>{var a,s,d,c,l,m,b;const o=n==="login";if(e.innerHTML=`
  ...
  `;function s(){var g;const c=document.getElementById("logo-preview"),l=document.getElementById("logo-initials"),m=((g=document.getElementById("setup-club"))==null?void 0:g.value)||"MW",b=m.trim().split(" ").filter(Boolean),y=b.length>=2?(b[0][0]+b[1][0]).toUpperCase():m.slice(0,2).toUpperCase();c&&(c.style.background=a,c.style.borderColor=o),l&&(l.textContent=y,l.style.color=o)}document.getElementById("color1").addEventListener("input",c=>{o=c.target.value,document.getElementById("swatch1").style.background=o,s()}),document.getElementById("color2").addEventListener("input",c=>{a=c.target.value,document.getElementById("swatch2").style.background=a,s()});function d(c){document.querySelectorAll(".setup-step").forEach(l=>l.classList.remove("active")),document.getElementById(`step-${c}`).classList.add("active"),document.getElementById("step-num").textContent=c,document.getElementById("progress-fill").style.width=`${Math.round(c/3*100)}%`,c===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("step1-error");if(l.textContent="",c.length<3){l.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:m}=await _.from("users").select("id").eq("pseudo",c).maybeSingle();if(m){l.textContent="Ce pseudo est déjà pris.";return}d(2)}),document.getElementById("step2-back").addEventListener("click",()=>d(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const c=document.getElementById("setup-club").value.trim(),l=document.getElementById("step2-error");if(l.textContent="",c.length<2){l.textContent="Nom trop court (min. 2 caractères).";return}const{data:m}=await _.from("users").select("id").eq("club_name",c).maybeSingle();if(m){l.textContent="Ce nom de club est déjà pris.";return}d(3)}),document.getElementById("step3-back").addEventListener("click",()=>d(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("setup-club").value.trim(),m=document.getElementById("step3-error"),b=document.getElementById("step3-finish");m.textContent="",b.disabled=!0,b.textContent="Création en cours…";try{const{error:y}=await _.from("users").insert({id:t.user.id,pseudo:c,club_name:l,club_color1:o,club_color2:a,credits:1e4});if(y)throw y;await mo(t.user.id),await r(),n(`Bienvenue ${c} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(y){m.textContent=y.message,b.disabled=!1,b.textContent="🚀 Créer mon profil !"}})}async function mo(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await _.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const go="modulepreload",yo=function(e){return"/"+e},cn={},Ln=function(t,i,n){let r=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),s=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));r=Promise.allSettled(i.map(d=>{if(d=yo(d),d in cn)return;cn[d]=!0;const c=d.endsWith(".css"),l=c?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${l}`))return;const m=document.createElement("link");if(m.rel=c?"stylesheet":go,c||(m.as="script"),m.crossOrigin="",m.href=d,s&&m.setAttribute("nonce",s),document.head.appendChild(m),c)return new Promise((b,y)=>{m.addEventListener("load",b),m.addEventListener("error",()=>y(new Error(`Unable to preload CSS for ${d}`)))})}))}function o(a){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=a,window.dispatchEvent(s),!s.defaultPrevented)throw a}return r.then(a=>{for(const s of a||[])s.status==="rejected"&&o(s.reason);return t().catch(o)})},ii="#1A6B3C",ni="#cc2222",xo="#D4A017",pn="#888";async function bo(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    </style>`}const Sn="mw_sound_volume";function Vi(){const e=localStorage.getItem(Sn);if(e===null)return 100;const t=parseInt(e,10);return Number.isFinite(t)?Math.max(0,Math.min(100,t)):100}function wo(e){localStorage.setItem(Sn,String(Math.max(0,Math.min(100,Math.round(e)))))}function Yi(){return Vi()===0}function fi(e){return Math.max(0,Math.min(1,e*(Vi()/100)))}function Wi(e,t=1){if(Yi())return null;try{const i=new Audio(e);return i.volume=fi(t),i.play().catch(()=>{}),i}catch{return null}}let gt=null,Mi=null,un=.3;function An(e,t=.3){if(un=t,gt&&Mi===e&&!gt.paused){gt.volume=fi(un);return}if(Gt(),!Yi())try{const i=new Audio(e);i.loop=!0,i.volume=fi(t),i.play().catch(()=>{}),gt=i,Mi=e}catch{}}function Gt(){if(gt)try{gt.pause(),gt.currentTime=0}catch{}gt=null,Mi=null}let Xt=null;function zn(e,t=.6){if(Ue(),!Yi())try{const i=new Audio(e);i.volume=fi(t),i.play().catch(()=>{}),Xt=i}catch{}}function Ue(){if(Xt)try{Xt.pause(),Xt.currentTime=0}catch{}Xt=null}const ko="2026.07.22-1723";async function fn(e,{state:t,navigate:i,toast:n}){var o,a;const r=t.profile;r&&(e.innerHTML=`
  <style>
    .home-dark {
      min-height: 100%;
  ...
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:0 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none;align-items:center"></div>
  ...
  </div>`,function(G){const O=document.getElementById(G);if(!O)return;O.addEventListener("wheel",de=>{Math.abs(de.deltaY)<=Math.abs(de.deltaX)||(de.preventDefault(),O.scrollLeft+=de.deltaY)},{passive:!1});let N=!1,W=0,B=0,R=!1,pe=!1;O.addEventListener("mousedown",de=>{N=!0,R=!1,W=de.pageX,B=O.scrollLeft}),window.addEventListener("mouseup",()=>{N=!1,R&&(O.style.cursor="",pe=!0),R=!1}),window.addEventListener("mousemove",de=>{if(!N)return;const he=de.pageX-W;!R&&Math.abs(he)<6||(R=!0,O.style.cursor="grabbing",de.preventDefault(),O.scrollLeft=B-he)}),O.addEventListener("click",de=>{pe&&(de.stopPropagation(),de.preventDefault(),pe=!1)},!0)}("col-grid");function T(){const E=document.getElementById("col-filters");E&&(z==="player"?(E.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{q=!q,T(),K()})))}function K(){const E=document.getElementById("col-grid");E&&(z==="player"?Y(E):z==="formation"?ce(E):z==="stadium"?V(E):U(E))}function Z(E,G,O,N,W,B=0){const R=document.getElementById("col-grid"),pe=document.getElementById("col-big");if(!R||!pe)return;var de=0;function he(){var ct;const Se=window.innerWidth>=768,ve=document.getElementById("col-big"),Ie=document.getElementById("col-gap"),Ae=((ct=document.getElementById("col-grid"))==null?void 0:ct.parentElement)||null,Be=ve?ve.closest(".page"):null;if(ve&&Ae&&Be){let $e=0;Array.from(Be.children).forEach(function(C){C!==ve&&C!==Ae&&C!==Ie&&($e+=C.offsetHeight)});const Ce=Math.max(0,Be.clientHeight-$e),qe=Math.round(Ce*(B/100)),Ge=Math.max(0,Ce-qe),w=Se?50/71:45/63,L=Math.round(Ge*w),A=Math.max(0,Ge-L);Ie&&(Ie.style.height=qe+"px"),ve.style.flex="none",ve.style.height=L+"px",ve.style.minHeight="0",Ae.style.flex="none",Ae.style.height=A+"px",Ae.style.minHeight="0",Ae.style.display="flex",Ae.style.overflow="hidden"}const ke=document.getElementById("col-grid");ke&&(ke.style.height="100%",ke.style.flexShrink="0",ke.style.overflowX="auto",ke.style.overflowY="hidden",ke.style.alignItems="center",ke.style.width="100%"),pe.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:center center">'+G(E[de])+"</div>";var wt=pe.querySelector("[data-card-id],[data-form-id],[data-gc-id]");wt&&wt.addEventListener("click",function(){N(E[de])}),requestAnimationFrame(function(){var $e=document.getElementById("big-card-inner");if(!(!$e||!pe)){var Ce=pe.clientHeight,qe=pe.clientWidth-16,Ge=$e.offsetHeight,w=$e.offsetWidth;if(Ge>0&&w>0&&Ce>40){var L=Math.min(Ce/Ge,qe/w);$e.style.transform="scale("+L.toFixed(3)+")",$e.style.transformOrigin="center center"}}}),R.innerHTML=E.map(function($e,Ce){var qe=Ce===de,Ge="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+(qe?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+Ce+'" style="'+Ge+'">'+O($e,qe)+"</div>"}).join(""),R.querySelectorAll(".col-mini-item").forEach(function($e){$e.addEventListener("click",function(){de=Number($e.dataset.idx),he(),$e.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})}),requestAnimationFrame(function(){var $e=ke?ke.clientHeight:0,Ce=R.querySelector(".col-mini-item > div");if(!(!$e||!Ce)){var qe=Ce.style.zoom;Ce.style.zoom="1";var Ge=Ce.offsetHeight;if(Ce.style.zoom=qe,!(Ge<=0)){var w=$e/Ge;R.querySelectorAll(".col-mini-item > div").forEach(function(L){L.style.zoom=w.toFixed(4)})}}})}he()}function ue(E){var G=window.innerWidth>=768?.76:.54,O;if(!E||E._fake){var N=E?E.player:null;if(!N)return"";O=xn(N)}else O=Ii(E,"");return'<div style="display:inline-block;zoom:'+G+';pointer-events:none;line-height:0">'+O+"</div>"}function fe(E,G,O){var N=O>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+O+"</div>":"",W=!!G,B=zi(E,zt[E],{width:160});return W||(B='<div style="filter:grayscale(1);opacity:0.5">'+B+"</div>"),"<div "+(G?'data-form-id="'+G.id+'"':"")+' style="position:relative;cursor:pointer">'+N+B+"</div>"}function M(E,G){var O=window.innerWidth>=768?.76:.54,N=zi(E,zt[E],{width:140});return G||(N='<div style="filter:grayscale(1);opacity:0.45">'+N+"</div>"),'<div style="display:inline-block;zoom:'+O+';line-height:0;pointer-events:none">'+N+"</div>"}function Y(E){if(q){const G=j();if(!G.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucun joueur.</div>';return}const O=G.map(N=>c.find(W=>W.player.id===N.id)||{_fake:!0,player:N,id:"fake-"+N.id});Z(O,N=>N._fake?xn(N.player):Ii(N,""),N=>N._fake?ue({player:N.player,id:"x",_fake:!0}):ue(N),N=>{N._fake||bn(N,c,v,t)})}else{const G=ee();if(!G.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const O={};G.forEach(W=>{const B=W.player.id;O[B]||(O[B]=[]),O[B].push(W)});const N=Object.values(O).map(W=>qo(W));Z(N,W=>{const B=v[W.player.id]||1,R=B>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${B}</div>`:"",de=c.filter(he=>he.player.id===W.player.id&&he.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Ii(W,R+de)},W=>ue(W),W=>bn(W,c,v,t))}}function ce(E){const G=q?x:[...$];if(!G.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const O=G.map(N=>({formation:N,card:m.find(W=>W.formation===N)||null,owned:$.has(N)}));Z(O,({formation:N,card:W,owned:B})=>fe(N,B?W:null,B?m.filter(R=>R.formation===N).length:0),({formation:N,owned:W})=>M(N,W),({card:N,owned:W})=>{W&&N&&No(N,m,t,o)},"#1A6B3C",5)}function U(E){const G=Object.keys(g),O=q?G.length?G:h:[...f];if(!O.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const N=O.map(W=>({type:W,gc:je[W]||{icon:"⚡",desc:""},def:g[W]||null,owned:f.has(W),card:l.find(B=>B.gc_type===W)||null}));Z(N,({type:W,gc:B,def:R,owned:pe,card:de})=>{var ke;const he=(R==null?void 0:R.name)||W,Se=pe?l.filter(wt=>wt.gc_type===W).length:0,ve=Se>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${Se}</div>`:"",Ie=(R==null?void 0:R.effect)||B.desc||"",Ae=R!=null&&R.image_url?`/icons/${R.image_url}`:((ke=R==null?void 0:R.club)==null?void 0:ke.logo_url)||(R!=null&&R.country_code?`https://flagsapi.com/${R.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null);let Be=tt(he,Ae,B.icon,Ie,{width:160,onClick:pe});return pe||(Be=`<div style="filter:grayscale(1);opacity:0.5">${Be}</div>`),`<div ${pe&&de?`data-gc-id="${de.id}" data-gc-type="${W}"`:""} style="position:relative">${ve}${Be}</div>`},({type:W,gc:B,def:R,owned:pe})=>{const de=window.innerWidth>=768?.76:.54,he=(R==null?void 0:R.name)||W,Se=(R==null?void 0:R.effect)||B.desc||"",ve=R!=null&&R.image_url?`/icons/${R.image_url}`:null;let Ie=tt(he,ve,B.icon,Se,{width:140});return pe||(Ie=`<div style="filter:grayscale(1);opacity:0.45">${Ie}</div>`),`<div style="display:inline-block;zoom:${de};line-height:0;pointer-events:none">${Ie}</div>`},({type:W,owned:B,def:R})=>{B&&Go(W,R,o)},"#7a28b8",5)}function V(E){const G="#4FC3F7",O="/";if(!b.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Stade.<br><small>Ouvre un booster Stade !</small></div>';return}const N={};b.forEach(B=>{const R=B.stadium_id||"?";(N[R]=N[R]||[]).push(B)});const W=Object.entries(N).map(([B,R])=>({sid:B,def:u[B]||null,count:R.length,card:R[0]}));Z(W,({def:B,count:R})=>{var Ie,Ae;const pe=(B==null?void 0:B.name)||"?",de=((Ie=B==null?void 0:B.club)==null?void 0:Ie.encoded_name)||(B==null?void 0:B.country_code)||"—",he=B!=null&&B.image_url?`${O}icons/${B.image_url}`:((Ae=B==null?void 0:B.club)==null?void 0:Ae.logo_url)||(B!=null&&B.country_code?`https://flagsapi.com/${B.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),Se=R>1?`<div style="position:absolute;top:8px;right:8px;background:#333;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${R}</div>`:"",ve=`${de}<br>+10 ⭐ joueurs alliés`;return`<div style="position:relative">${Se}${ti(pe,he,ve,{width:160})}</div>`},({def:B})=>{var ve,Ie;const R=window.innerWidth>=768?.76:.54,pe=(B==null?void 0:B.name)||"?",de=((ve=B==null?void 0:B.club)==null?void 0:ve.encoded_name)||(B==null?void 0:B.country_code)||"—",he=B!=null&&B.image_url?`${O}icons/${B.image_url}`:((Ie=B==null?void 0:B.club)==null?void 0:Ie.logo_url)||(B!=null&&B.country_code?`https://flagsapi.com/${B.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),Se=`${de}<br>+10 ⭐`;return`<div style="display:inline-block;zoom:${R};line-height:0;pointer-events:none">${ti(pe,he,Se,{width:140})}</div>`},null,G,5)}e.querySelectorAll(".col-tab-btn").forEach(E=>{E.addEventListener("click",()=>{z=E.dataset.tab,k="Tous",S="",q=!1,e.querySelectorAll(".col-tab-btn").forEach(G=>{const O=G.dataset.tab===z;G.style.borderBottomColor=O?"var(--green)":"transparent",G.style.color=O?"var(--green)":"var(--tile-fg-dim)"}),T(),K()})}),T(),K()}function Go(e,t,i){const n=je[e]||{icon:"⚡",desc:"Effet spécial."},r=(t==null?void 0:t.name)||e,o=(t==null?void 0:t.effect)||n.desc,a=t!=null&&t.image_url?`/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    <div style="margin-top:16px;border-top:1px solid var(--tile-border);padding-top:14px">
      ...
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px">
        ${m.filter(U=>U.id!==e.id).map((U,V)=>{const E=ee[U.id]||[],G=U.is_for_sale,O=E.length?E[E.length-1]:null,N=U.evolution_bonus||0,W=O?O.source==="booster"?"Booster":O.price?O.price.toLocaleString("fr")+" cr.":"—":"—",B=O?new Date(O.transferred_at).toLocaleDateString("fr",{day:"2-digit",month:"2-digit",year:"numeric"}):"",R=80,pe=Math.round(R*657/507),de=Te({...l,_evolution_bonus:N},{width:R});return`
            <div class="exemplaire-row" data-card-id="${U.id}" data-card-idx="${V}"
              ...
              <div class="expl-mini-card" style="position:relative;width:${R}px;height:${pe}px;border-radius:8px;overflow:hidden">
                ${de}
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",l.note_g],["DEF",l.note_d],["MIL",l.note_m],["ATT",l.note_a]].map(([U,V])=>{const E=Ti[U],G=U==="GK"?"#fff":E,O=U===l.job||U===l.job2,N=(Number(V)||0)+(O&&z>0?z:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(fe=document.getElementById("close-detail"))==null||fe.addEventListener("click",s);let T=new Set;function K(){let U=0;return document.querySelectorAll(".expl-check:checked").forEach(V=>{if(V.dataset.id===e.id)return;const G=Number(V.dataset.evo)||0,O=Number(V.dataset.note)||0;U+=O+G}),U}const Z=()=>{const U=T.size,V=document.getElementById("sell-action-panel");if(!V)return;V.style.display=U>0?"block":"none",document.getElementById("sell-selected-count").textContent=U;const E=document.getElementById("evolve-btn");if(E){const G=K();E.textContent=G>0?`⬆️ Évoluer (+${G})`:"⬆️ Évoluer"}};document.querySelectorAll(".expl-check").forEach(U=>{U.addEventListener("change",()=>{const V=U.dataset.id;U.checked?T.add(V):T.delete(V);const E=U.closest(".exemplaire-row");if(E){const G=E.querySelector(".expl-sel-overlay"),O=E.querySelector(".expl-sel-check");G&&(G.style.display=U.checked?"block":"none"),O&&(O.style.display=U.checked?"flex":"none"),E.style.transform=U.checked?"scale(1.05)":"scale(1)"}Z()})}),document.querySelectorAll(".exemplaire-row").forEach(U=>{U.addEventListener("click",V=>{if(V.target.tagName==="INPUT")return;const E=U.querySelector(".expl-check");E&&!E.disabled&&(E.checked=!E.checked,E.dispatchEvent(new Event("change")))})}),(M=document.getElementById("evolve-btn"))==null||M.addEventListener("click",async()=>{if(b<=1)return;const U=[...T];if(!U.length)return;if(T.has(e.id)){const W=document.createElement("div");W.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",W.innerHTML=`
        ...
        </div>`,document.body.appendChild(B),B.querySelector("#sac-cancel").addEventListener("click",()=>{B.remove(),W(!1)}),B.querySelector("#sac-ok").addEventListener("click",()=>{B.remove(),W(!0)}),B.addEventListener("click",R=>{R.target===B&&(B.remove(),W(!1))})}))return;if(V.length){await _.from("market_listings").delete().in("card_id",V),await _.from("deck_cards").delete().in("card_id",V),await _.from("transfer_history").delete().in("card_id",V),await _.from("decks").update({stadium_card_id:null}).in("stadium_card_id",V);const{error:W}=await _.from("cards").delete().in("id",V);if(W){o("Erreur suppression : "+W.message,"error");return}}const O=(e.evolution_bonus||0)+E,{error:N}=await _.from("cards").update({evolution_bonus:O}).eq("id",e.id);if(N){o("Erreur évolution : "+N.message,"error");return}o(`⬆️ ${l.firstname} ${l.surname_real} : note ${h} → ${h+O} (+${E}) !`,"success",4e3),s(),d("collection")}),(Y=document.getElementById("market-sell-btn"))==null||Y.addEventListener("click",async()=>{var N;const U=[...T];if(!U.length){o("Sélectionne au moins un exemplaire","warning");return}const V=parseInt((N=document.getElementById("sell-market-price"))==null?void 0:N.value);if(!V||V<1){o("Prix invalide","error");return}const{error:E}=await _.from("cards").update({is_for_sale:!0,sale_price:V}).in("id",U);if(E){o(E.message,"error");return}const G=U.map(W=>({seller_id:r.profile.id,card_id:W,price:V,status:"active"})),{error:O}=await _.from("market_listings").insert(G);O&&console.warn("[Market] insert listings:",O.message),o(`${U.length} carte${U.length>1?"s":""} mise${U.length>1?"s":""} en vente à ${V.toLocaleString("fr")} cr. chacune !`,"success"),s(),d("collection")}),(ce=document.getElementById("cancel-sell-btn"))==null||ce.addEventListener("click",async()=>{await _.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await _.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),o("Annonce retirée","success"),s(),d("collection")})}function vn(e,t=""){return new Promise(i=>{const n=document.createElement("div");n.className="modal-overlay",n.style.zIndex="2100",n.innerHTML=`<div class="modal" style="max-width:360px">
  ...
  `,document.body.appendChild(r);let o=!1;const a=document.getElementById("pack-cut-zone"),s=document.getElementById("pack-blade");let d=!1;const c=T=>T.touches&&T.touches[0]?T.touches[0].clientX:T.clientX;function l(T){o||(d=!0,s.style.opacity="1",m(T))}function m(T){if(!d||o)return;const K=a.getBoundingClientRect(),Z=c(T)-K.left,ue=Math.max(0,Math.min(1,Z/K.width));s.style.width=ue*K.width+"px",ue>=.82&&y()}function b(){o||(d=!1,s.style.transition="width .2s ease, opacity .2s ease",s.style.width="0",s.style.opacity="0",setTimeout(()=>{o||(s.style.transition="")},220))}function y(){var K;if(o)return;o=!0,d=!1,s.style.width="100%",s.style.opacity="1",(K=document.getElementById("cut-flash"))==null||K.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const T=document.getElementById("cut-hint");T&&(T.style.opacity="0"),a.classList.add("pack-cut"),setTimeout(()=>{s.style.opacity="0",document.getElementById("pack-phase").style.display="none",x(0)},620)}a.addEventListener("pointerdown",l),window.addEventListener("pointermove",m),window.addEventListener("pointerup",b),a.addEventListener("touchstart",l,{passive:!0}),window.addEventListener("touchmove",m,{passive:!0}),window.addEventListener("touchend",b);let g=0,p=!1;const u=new Set;function x(T){g=T,document.getElementById("reveal-phase").style.display="flex",h(),v(T,0),S()}function h(){const T=document.getElementById("card-dots");T&&(T.innerHTML=e.map((K,Z)=>`<div class="card-dot" data-i="${Z}" style="width:8px;height:8px;border-radius:50%;background:${Z===g?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),T.querySelectorAll(".card-dot").forEach(K=>K.addEventListener("click",()=>f(parseInt(K.dataset.i)))))}function v(T,K){var E,G;const Z=e[T],ue=document.getElementById("card-counter"),fe=document.getElementById("card-track");ue&&(ue.textContent=`Carte ${T+1} / ${e.length}`);const M=document.getElementById("reveal-btns");M&&(M.style.display=T===e.length-1?"flex":"none");const Y=Z.card_type==="player"&&((E=Z.player)==null?void 0:E.rarity)==="legende",ce=!u.has(T);u.add(T);let U=0;if(Z.card_type==="player"&&Z.player){const O=Z.player,N=O.job||"ATT";U=Number(N==="GK"?O.note_g:N==="DEF"?O.note_d:N==="MIL"?O.note_m:O.note_a)||0}const V=O=>{fe.innerHTML=`
        ...
        </div>`;const N=document.getElementById("current-card-wrap");K!==0?(N.style.transition="none",N.style.transform=`translateX(${K>0?100:-100}%)`,requestAnimationFrame(()=>{N.style.transition="transform .28s cubic-bezier(.25,1,.5,1)",N.style.transform="translateX(0)"})):N.animate([{opacity:0,transform:"scale(.7)"},{opacity:1,transform:"scale(1)"}],{duration:300,easing:"cubic-bezier(.34,1.56,.64,1)"}),O||Y?ne():oe(),h()};ce&&(((G=Z.player)==null?void 0:G.rarity)==="legende"||U>=8)&&Z.card_type==="player"&&Z.player?$(Z,()=>V(!0)):V(!1)}function $(T,K){var Se;p=!0;const Z=T.player,ue=`https://flagsapi.com/${Z.country_code}/flat/64.png`,fe=(Se=Z.clubs)==null?void 0:Se.logo_url,M=At(Z),Y=Z.job||"ATT",ce=Number(Y==="GK"?Z.note_g:Y==="DEF"?Z.note_d:Y==="MIL"?Z.note_m:Z.note_a)||0,U=T.evolution_bonus||0,V=ce+U,E=V>=18&&V<=20,G=V>=18,O=document.getElementById("walkout-overlay"),N=document.getElementById("walkout-stage");if(!O||!N){p=!1,K();return}let W=null;E&&(W=Wi("/sounds/Legendary.mp3",.8)),O.style.display="flex";const B=()=>{const ve=N.firstElementChild;ve&&(ve.classList.remove("wo-in"),ve.classList.add("wo-out"))},R=1800,pe=400;N.innerHTML=`<img class="wo-in" src="${ue}" style="height:130px;border-radius:10px;box-shadow:0 10px 36px rgba(0,0,0,.6)" onerror="this.style.display='none'">`,navigator.vibrate&&navigator.vibrate(30),setTimeout(B,R),setTimeout(()=>{var ve;N.innerHTML=fe?`<img class="wo-in" src="${fe}" style="max-height:160px;max-width:210px;object-fit:contain">`:`<div class="wo-in" style="font-size:34px;font-weight:900;color:#fff;text-align:center">${((ve=Z.clubs)==null?void 0:ve.encoded_name)||"CLUB"}</div>`,navigator.vibrate&&navigator.vibrate(30)},R+pe),setTimeout(B,R*2+pe),setTimeout(()=>{N.innerHTML=M?`<img class="wo-in" src="${M}" style="height:200px;border-radius:50%;box-shadow:0 0 40px rgba(255,255,255,0.3);object-fit:cover;object-position:top">`:'<div class="wo-in" style="font-size:80px">👤</div>',navigator.vibrate&&navigator.vibrate(30)},(R+pe)*2),setTimeout(B,(R+pe)*2+R);const he=G?"#FFD700":{GK:"#c0c0c0",DEF:"#e03030",MIL:"#D4A017",ATT:"#3fbf5f"}[Y]||"#fff";setTimeout(()=>{N.innerHTML=`<div class="wo-in" style="
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const Y=t.pendingAttack;let ce="";if(t.selected.length>0){const U=t.selected.map(E=>({...(t.homeTeam[E._role]||[]).find(O=>O.cardId===E.cardId)||E,_line:E._role})),V=ri(U,t.modifiers.home,t.formation);ce=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
    ...
    ${(()=>{const M=window.innerWidth>=700,Y=(B,R,pe)=>{var Ae,Be;const de=(t.gcDefs||[]).find(ke=>ke.name===B.gc_type),he=(de==null?void 0:de.name)||B.gc_type,Se=(de==null?void 0:de.effect)||((Ae=je[B.gc_type])==null?void 0:Ae.desc)||"",ve=de!=null&&de.image_url?`/icons/${de.image_url}`:null,Ie=((Be=je[B.gc_type])==null?void 0:Be.icon)||"⚡";return`<div class="gc-mini" data-gc-id="${B.id}" data-gc-type="${B.gc_type}" style="flex-shrink:0;cursor:pointer">
          ${tt(he,ve,Ie,Se,{width:R})}
        </div>`},ce=(B,R)=>{var pe;return`<div id="boost-card"
          ...
          </div>`},U=(B,R)=>R?ce(95,162):Y(B,95),V=(B,R)=>R?ce(68,116):Y(B,68),E=M?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",G=h?`<button id="btn-results" style="${E};background:linear-gradient(135deg,#D4A017,#FFD700);border:none;color:#000">🏁 Résultats</button>`:p?`<div style="${E};background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);color:rgba(255,255,255,0.4)">⏳ Tour IA</div>`:f?`<button id="btn-pass" style="${E};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER (plus d'attaquants)</button>`:u?`<button id="btn-action" style="${E};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${t.selected.length===0?"disabled":""}> ⚔️ ATTAQUEZ <span id="match-timer" style="font-weight:900"></span></button>`:x?`<button id="btn-action" style="${E};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${t.selected.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="match-timer" style="font-weight:900"></span></button>`:`<div style="${E};background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1)"></div>`,O=u||x?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${t.selected.length}/3 sélectionné(s)</div>`:"",N=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${M?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
  ...
  <div class="page">
    ...
    <nav class="top-nav">
      <div class="logo" id="nav-logo" title="Manager Wars v2026.07.22-1723" style="cursor:pointer">
