...
import{s as T,F as _i,i as Ht,k as Vi,l as ot,m as Li,f as Gt,n as hn,T as di,j as vn,o as wn,b as _n}from"./formation-links-BZe0pGr4.js";const $n="/";function Ii(e,{navigate:t,toast:i}){let o="login";const d=()=>{var r,l,p,s,n,x;const a=o==="login";e.innerHTML=`
  ...
  `;function l(){var _;const s=document.getElementById("logo-preview"),n=document.getElementById("logo-initials"),x=((_=document.getElementById("setup-club"))==null?void 0:_.value)||"MW",u=x.trim().split(" ").filter(Boolean),h=u.length>=2?(u[0][0]+u[1][0]).toUpperCase():x.slice(0,2).toUpperCase();s&&(s.style.background=r,s.style.borderColor=a),n&&(n.textContent=h,n.style.color=a)}document.getElementById("color1").addEventListener("input",s=>{a=s.target.value,document.getElementById("swatch1").style.background=a,l()}),document.getElementById("color2").addEventListener("input",s=>{r=s.target.value,document.getElementById("swatch2").style.background=r,l()});function p(s){document.querySelectorAll(".setup-step").forEach(n=>n.classList.remove("active")),document.getElementById(`step-${s}`).classList.add("active"),document.getElementById("step-num").textContent=s,document.getElementById("progress-fill").style.width=`${Math.round(s/3*100)}%`,s===3&&l()}document.getElementById("step1-next").addEventListener("click",async()=>{const s=document.getElementById("setup-pseudo").value.trim(),n=document.getElementById("step1-error");if(n.textContent="",s.length<3){n.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:x}=await T.from("users").select("id").eq("pseudo",s).maybeSingle();if(x){n.textContent="Ce pseudo est déjà pris.";return}p(2)}),document.getElementById("step2-back").addEventListener("click",()=>p(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const s=document.getElementById("setup-club").value.trim(),n=document.getElementById("step2-error");if(n.textContent="",s.length<2){n.textContent="Nom trop court (min. 2 caractères).";return}const{data:x}=await T.from("users").select("id").eq("club_name",s).maybeSingle();if(x){n.textContent="Ce nom de club est déjà pris.";return}p(3)}),document.getElementById("step3-back").addEventListener("click",()=>p(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const s=document.getElementById("setup-pseudo").value.trim(),n=document.getElementById("setup-club").value.trim(),x=document.getElementById("step3-error"),u=document.getElementById("step3-finish");x.textContent="",u.disabled=!0,u.textContent="Création en cours…";try{const{error:h}=await T.from("users").insert({id:t.user.id,pseudo:s,club_name:n,club_color1:a,club_color2:r,credits:1e4});if(h)throw h;await En(t.user.id),await d(),o(`Bienvenue ${s} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(h){x.textContent=h.message,u.disabled=!1,u.textContent="🚀 Créer mon profil !"}})}async function En(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await T.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const Tn="modulepreload",Sn=function(e){return"/"+e},Mi={},ni=function(t,i,o){let d=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const r=document.querySelector("meta[property=csp-nonce]"),l=(r==null?void 0:r.nonce)||(r==null?void 0:r.getAttribute("nonce"));d=Promise.allSettled(i.map(p=>{if(p=Sn(p),p in Mi)return;Mi[p]=!0;const s=p.endsWith(".css"),n=s?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${p}"]${n}`))return;const x=document.createElement("link");if(x.rel=s?"stylesheet":Tn,s||(x.as="script"),x.crossOrigin="",x.href=p,l&&x.setAttribute("nonce",l),document.head.appendChild(x),s)return new Promise((u,h)=>{x.addEventListener("load",u),x.addEventListener("error",()=>h(new Error(`Unable to preload CSS for ${p}`)))})}))}function a(r){const l=new Event("vite:preloadError",{cancelable:!0});if(l.payload=r,window.dispatchEvent(l),!l.defaultPrevented)throw r}return d.then(r=>{for(const l of r||[])l.status==="rejected"&&a(l.reason);return t().catch(a)})},Nt="#1A6B3C",Rt="#cc2222",zn="#D4A017",ji="#888";async function An(e,t){const{state:i,toast:o}=t;e.innerHTML=`
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      ${u.map(({friendshipId:h,friend:_})=>Ln(_,h)).join("")}
    </div>`,s.querySelectorAll("[data-stats]").forEach(h=>{h.addEventListener("click",()=>Mn(e,h.dataset.stats,h.dataset.friendName))}),s.querySelectorAll("[data-match]").forEach(h=>{h.addEventListener("click",()=>{const _=h.dataset.friendId,m=h.dataset.friendName;console.log("[Friends] clic match",{fid:_,fname:m,hasNavigate:typeof o}),typeof o=="function"?o("match",{matchMode:"friend",friendId:_,friendName:m}):t("Erreur navigation","error")})})}function Ln(e,t){const i=e.club_name||e.pseudo||"?",o=e.pseudo||"",d=(o||i).slice(0,2).toUpperCase(),a=e.club_color2||Nt,r=e.club_color1||"#ffffff",l=e.last_seen_at?new Date(e.last_seen_at):null,p=l&&Date.now()-l.getTime()<3*60*1e3;return`
    ...
    ${ki()}`,document.body.appendChild(s);const x=()=>s.remove();s.querySelector("#pending-close").addEventListener("click",x),s.addEventListener("click",u=>{u.target===s&&x()}),s.querySelectorAll("[data-accept]").forEach(u=>{u.addEventListener("click",async()=>{const{error:h}=await T.from("friendships").update({status:"accepted"}).eq("id",u.dataset.accept);if(h){t("Erreur : "+h.message,"error");return}u.closest("div[style]").remove(),t("✅ Ami accepté !","success"),Yi(e,t,o),i&&i()})}),s.querySelectorAll("[data-decline]").forEach(u=>{u.addEventListener("click",async()=>{await T.from("friendships").delete().eq("id",u.dataset.decline),u.closest("div[style]").remove(),t("Demande refusée","info"),i&&i()})})}async function Mn(e,t,i){const o=e.user.id,[d,a]=[o,t].sort(),r=o===d,{data:l}=await T.from("friend_match_stats").select("*").eq("player1_id",d).eq("player2_id",a).single(),p=e.profile.club_name||e.profile.pseudo||"Moi",s=l||{},n=r?s.wins_p1||0:s.wins_p2||0,x=r?s.wins_p2||0:s.wins_p1||0,u=s.draws||0,h=r?s.goals_p1||0:s.goals_p2||0,_=r?s.goals_p2||0:s.goals_p1||0,m=r?s.gc_used_p1||0:s.gc_used_p2||0,c=r?s.gc_used_p2||0:s.gc_used_p1||0,A=s.matches_total||0,g=(q,B,C,M=Nt,G=Rt)=>`
    <div style="display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:8px;padding:10px 0;border-bottom:1px solid #f0f0f0">
      <div style="text-align:right;font-size:18px;font-weight:900;color:${M}">${B}</div>
    ...
    </style>`}async function Ji({player1Id:e,player2Id:t,score1:i,score2:o,gc1:d,gc2:a}){const[r,l]=[e,t].sort(),p=e!==r,s=p?o:i,n=p?i:o,x=p?a:d,u=p?d:a,h=s>n?1:0,_=n>s?1:0,m=s===n?1:0,{data:c}=await T.from("friend_match_stats").select("*").eq("player1_id",r).eq("player2_id",l).single();c?await T.from("friend_match_stats").update({wins_p1:c.wins_p1+h,wins_p2:c.wins_p2+_,draws:c.draws+m,goals_p1:c.goals_p1+s,goals_p2:c.goals_p2+n,gc_used_p1:c.gc_used_p1+x,gc_used_p2:c.gc_used_p2+u,matches_total:c.matches_total+1}).eq("player1_id",r).eq("player2_id",l):await T.from("friend_match_stats").insert({player1_id:r,player2_id:l,wins_p1:h,wins_p2:_,draws:m,goals_p1:s,goals_p2:n,gc_used_p1:x,gc_used_p2:u,matches_total:1})}const jn="2026.07.08-0010";async function Ci(e,{state:t,navigate:i,toast:o}){var a,r;const d=t.profile;d&&(e.innerHTML=`
  <style>
  ...
  <div style="position:absolute;top:${x(48)};left:0;right:0;text-align:center;z-index:4;padding:0 ${x(50)}">
    <span style="font-size:${x(20)};font-weight:700;color:${h};letter-spacing:${x(2)};line-height:1;text-shadow:0 1px 4px #000;display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${H}</span>
  </div>
  ...
  <div style="position:absolute;left:50%;transform:translateX(-50%);top:${x(530)};
    ...
    <span style="font-size:${x(32)};font-weight:900;color:#e03030;font-family:Arial Black,Arial;line-height:1">${B}</span>
  </div>`:""}

  <!-- Drapeau : sans fond -->
  <div style="position:absolute;left:${pe}px;top:${se}px;width:${Ee}px;height:${ve}px;z-index:3;
    display:flex;align-items:center;justify-content:center;overflow:hidden">
    ${M?`<img src="${M}" style="width:100%;height:100%;object-fit:cover;border-radius:${U}px">`:`<span style="font-size:${x(22)}">🌍</span>`}
  </div>
  ...
  <div style="position:absolute;left:${ce}px;top:${se}px;width:${Ee}px;height:${ve}px;z-index:3;
    ...
    ${st(e,{width:140})}
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),s=(l||[]).filter(U=>U.card_type==="player"&&U.player),n=(l||[]).filter(U=>U.card_type==="game_changer"),x=(l||[]).filter(U=>U.card_type==="formation"),u=(l||[]).filter(U=>U.card_type==="stadium"),{data:h}=await T.from("gc_definitions").select("name,gc_type,color,effect,image_url"),_={};(h||[]).forEach(U=>{_[U.name]=U});const{data:m}=await T.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),c={};(m||[]).forEach(U=>{c[U.id]=U}),u.forEach(U=>{U.stadium_def&&(c[U.stadium_id]=U.stadium_def)});const A=Object.keys(_i),g=Object.keys(Oe),I={};s.forEach(U=>{const Z=U.player.id;I[Z]=(I[Z]||0)+1}),new Set(Object.keys(I).map(U=>String(U)));const q=new Set(x.map(U=>U.formation)),B=new Set(n.map(U=>U.gc_type));let C="player",M="Tous",G="",H=!1;function V(){return[...s].sort((U,Z)=>{const ge=Yt.indexOf(U.player.job),de=Yt.indexOf(Z.player.job);return ge!==de?ge-de:(U.player.surname_encoded||"").localeCompare(Z.player.surname_encoded||"")})}function oe(){return[...p||[]].sort((U,Z)=>{const ge=Yt.indexOf(U.job),de=Yt.indexOf(Z.job);return ge!==de?ge-de:(U.surname_encoded||"").localeCompare(Z.surname_encoded||"")})}function ie(){return V().filter(U=>{const Z=U.player,ge=M==="Tous"||Z.job===M,de=!G||`${Z.firstname} ${Z.surname_encoded}`.toLowerCase().includes(G);return ge&&de})}function Q(){return oe().filter(U=>{const Z=M==="Tous"||U.job===M,ge=!G||`${U.firstname} ${U.surname_encoded}`.toLowerCase().includes(G);return Z&&ge})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
  ...
  </div>`;function R(){const U=document.getElementById("col-filters");U&&(C==="player"?(U.innerHTML=`
          ...
          ${Kn.map(Z=>`
            <button class="filter-btn" data-job="${Z}"
              style="flex-shrink:0;padding:4px 12px;border-radius:20px;font-size:12px;font-weight:600;cursor:pointer;
                border:1.5px solid ${Z===M?"var(--green)":"var(--gray-200)"};
                background:${Z===M?"var(--green)":"#fff"};
                color:${Z===M?"#fff":"var(--gray-600)"}">
              ${Z}
            </button>`).join("")}
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",Z=>{G=Z.target.value.toLowerCase(),P()}),e.querySelectorAll(".filter-btn").forEach(Z=>{Z.addEventListener("click",()=>{M=Z.dataset.job,R(),P()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{H=!H,R(),P()})):(U.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{H=!H,R(),P()})))}function P(){const U=document.getElementById("col-grid");U&&(C==="player"?Ee(U):C==="formation"?se(U):C==="stadium"?ce(U):pe(U))}function re(U,Z,ge,de,me){const ne=document.getElementById("col-grid"),ue=document.getElementById("col-big");if(!ne||!ue)return;var Le=0;function Ie(){const Ce=window.innerWidth>=768,Te=document.getElementById("col-big"),qe=document.getElementById("col-grid");Ce&&Te&&(Te.style.minHeight="420px",Te.style.flex="1 1 auto"),Ce&&qe&&(qe.style.height=Math.round(310*.76+16)+"px",qe.style.flexShrink="0",qe.style.overflowX="auto",qe.style.overflowY="hidden",qe.style.alignItems="center",qe.style.padding="8px 16px"),ue.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+Z(U[Le])+"</div>";var Pe=ue.querySelector("[data-card-id],[data-form-id],[data-gc-id]");Pe&&Pe.addEventListener("click",function(){de(U[Le])}),requestAnimationFrame(function(){var Se=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!Se||!ue)){var F=ue.clientHeight-8,$=ue.clientWidth-24,f=Se.offsetHeight,y=Se.offsetWidth;if(f>0&&y>0&&F>40){var b=Ce?2.2:1,v=Math.min(F/f,$/y,b);Se.style.transform="scale("+v.toFixed(3)+")",Se.style.transformOrigin="top center"}}}),ne.innerHTML=U.map(function(Se,F){var $=F===Le,f="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+($?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+F+'" style="'+f+'">'+ge(Se,$)+"</div>"}).join(""),ne.querySelectorAll(".col-mini-item").forEach(function(Se){Se.addEventListener("click",function(){Le=Number(Se.dataset.idx),Ie(),Se.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}Ie()}function xe(U){var Z=window.innerWidth>=768?.76:.54,ge;if(!U||U._fake){var de=U?U.player:null;if(!de)return"";ge=Pi(de)}else ge=pi(U,"");return'<div style="display:inline-block;zoom:'+Z+';pointer-events:none;line-height:0">'+ge+"</div>"}function _e(U,Z,ge){Z=Z||100,ge=ge||140;var de=Ht[U]||{},me={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},ne=Math.max(3,Math.round(Z*.06)),ue=Object.entries(de).map(function(Ie){var Ce=Ie[0],Te=Ie[1],qe=Ce.replace(/\d+$/,""),Pe=me[qe]||"#888",Se=Math.round(Te.x*Z),F=Math.round(Te.y*ge);return'<circle cx="'+Se+'" cy="'+F+'" r="'+ne+'" fill="'+Pe+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),Le=Math.max(1,Math.round(Z/50));return'<svg viewBox="0 0 '+Z+" "+ge+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+Z+'" height="'+ge+'" fill="#1A6B3C"/><rect x="'+Math.round(Z*.2)+'" y="'+Math.round(ge*.02)+'" width="'+Math.round(Z*.6)+'" height="'+Math.round(ge*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+Le+'"/><line x1="0" y1="'+Math.round(ge*.5)+'" x2="'+Z+'" y2="'+Math.round(ge*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+Le+'"/><ellipse cx="'+Math.round(Z*.5)+'" cy="'+Math.round(ge*.5)+'" rx="'+Math.round(Z*.18)+'" ry="'+Math.round(ge*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+Le+'"/><rect x="'+Math.round(Z*.2)+'" y="'+Math.round(ge*.82)+'" width="'+Math.round(Z*.6)+'" height="'+Math.round(ge*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+Le+'"/>'+ue+"</svg>"}function we(U,Z,ge){var de=ge>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+ge+"</div>":"",me=Z?'data-form-id="'+Z.id+'"':"",ne=U.length>7?14:U.length>5?16:19,ue=!!Z;return"<div "+me+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(ue?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(ue?"":"filter:grayscale(1);opacity:0.5")+'">'+de+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(ue?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+ne+"px;font-weight:900;color:"+(ue?"#1A6B3C":"#aaa")+';line-height:1">'+U+'</div></div><div style="flex:1;overflow:hidden;background:'+(ue?"#1A6B3C":"#ccc")+'">'+_e(U,140,220)+"</div></div>"}function ve(U,Z){var ge=window.innerWidth>=768?.76:.54,de=140,me=305,ne=Math.round(me*.22),ue=me-ne,Le=U.length>7?10:13,Ie=_e(U,de,ue),Ce=Z?"1.5px solid #2a7a40":"1px solid #ddd",Te=Z?"":"filter:grayscale(1);opacity:0.45;",qe=Z?"#1A6B3C":"#bbb",Pe="#fff";return'<div style="display:inline-block;zoom:'+ge+';line-height:0;pointer-events:none"><div style="width:'+de+"px;height:"+me+"px;border-radius:6px;border:"+Ce+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+Te+'"><div style="height:'+ne+"px;background:"+qe+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+Le+"px;font-weight:900;color:"+Pe+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(de-4)+'px">'+U+'</span></div><div style="height:'+ue+'px;overflow:hidden;flex-shrink:0">'+Ie+"</div></div></div>"}function Ee(U){if(H){const Z=Q();if(!Z.length){U.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const ge=Z.map(de=>s.find(me=>me.player.id===de.id)||{_fake:!0,player:de,id:"fake-"+de.id});re(ge,de=>de._fake?Pi(de.player):pi(de,""),de=>de._fake?xe({player:de.player,id:"x",_fake:!0}):xe(de),de=>{de._fake||Gi(de,s,I,t)})}else{const Z=ie();if(!Z.length){U.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const ge={};Z.forEach(me=>{const ne=me.player.id;ge[ne]||(ge[ne]=[]),ge[ne].push(me)});const de=Object.values(ge).map(me=>Yn(me));re(de,me=>{const ne=I[me.player.id]||1,ue=ne>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${ne}</div>`:"",Ie=s.filter(Ce=>Ce.player.id===me.player.id&&Ce.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return pi(me,ue+Ie)},me=>xe(me),me=>Gi(me,s,I,t))}}function se(U){const Z=H?A:[...q];if(!Z.length){U.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const ge=Z.map(de=>({formation:de,card:x.find(me=>me.formation===de)||null,owned:q.has(de)}));re(ge,({formation:de,card:me,owned:ne})=>we(de,ne?me:null,ne?x.filter(ue=>ue.formation===de).length:0),({formation:de,owned:me})=>ve(de,me),({card:de,owned:me})=>{me&&de&&eo(de,x,t,a)})}function pe(U){const Z=Object.keys(_),ge=H?Z.length?Z:g:[...B];if(!ge.length){U.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const de=ge.map(me=>({type:me,gc:Oe[me]||{icon:"⚡",desc:""},def:_[me]||null,owned:B.has(me),card:n.find(ne=>ne.gc_type===me)||null}));re(de,({type:me,gc:ne,def:ue,owned:Le,card:Ie})=>{var b;const Ce=Le?n.filter(v=>v.gc_type===me).length:0,Te=Ce>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${Ce}</div>`:"",qe=(ue==null?void 0:ue.gc_type)==="ultra_game_changer",Pe={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},Se={purple:"#b06ce0",light_blue:"#00d4ef"},F=Pe[ue==null?void 0:ue.color]||Pe.purple,$=Se[ue==null?void 0:ue.color]||Se.purple,f=(ue==null?void 0:ue.effect)||ne.desc||"",y=ue!=null&&ue.image_url?`/icons/${ue.image_url}`:((b=ue==null?void 0:ue.club)==null?void 0:b.logo_url)||(ue!=null&&ue.country_code?`https://flagsapi.com/${ue.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null);return Le&&Ie?`<div data-gc-id="${Ie.id}" data-gc-type="${me}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${$};background:${F};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${$}66;cursor:pointer">
          ${Te}
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Wt=1e3;function eo(e,t,i,o){var _,m,c;const{state:d,toast:a,closeModal:r,navigate:l,refreshProfile:p}=i,s=e.formation,n={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function x(){const A=Ht[s]||{},g=_i[s]||[],I=290,q=360,B=20;function C(G){const H=A[G];return H?{x:H.x*I,y:H.y*q}:null}let M=`<svg width="${I}" height="${q}" viewBox="0 0 ${I} ${q}" xmlns="http://www.w3.org/2000/svg">`;for(const[G,H]of g){const V=C(G),oe=C(H);!V||!oe||(M+=`<line x1="${V.x}" y1="${V.y}" x2="${oe.x}" y2="${oe.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const G of Object.keys(A)){const H=C(G);if(!H)continue;const V=G.replace(/\d+/,""),oe=n[V]||"#555";M+=`<circle cx="${H.x}" cy="${H.y}" r="${B}" fill="${oe}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,M+=`<text x="${H.x}" y="${H.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${V}</text>`}return M+="</svg>",M}const u=t.filter(A=>A.formation===s);u.length;const h=!e.is_for_sale;o(`Formation ${s}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${x.map((se,pe)=>{const ce=ie[se.id]||[],U=se.is_for_sale,Z=ce.length?ce[ce.length-1]:null,de=(n.rarity==="pepite"||n.rarity==="papyte")&&se.current_note!=null?` (☆${se.current_note})`:"";return`
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;background:#f0fdf4;border-radius:10px;padding:10px 14px">
          <div>
            ...
            <div style="font-size:11px;color:#555;margin-top:2px">
              ${(e.evolution_bonus||0)>0?`+${e.evolution_bonus} appliqué${e.evolution_bonus>1?"s":""} · Note : ${G}${n.job2&&H>0?` / ${V}`:""}`:`Note actuelle : ${G}${n.job2&&H>0?` / ${V}`:""}`}
            </div>
          </div>
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",n.note_g],["DEF",n.note_d],["MIL",n.note_m],["ATT",n.note_a]].map(([se,pe])=>{const ce=li[se],U=se==="GK"?"#fff":ce,Z=se===n.job||se===n.job2,ge=(Number(pe)||0)+(Z&&C>0?C:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
                <div style="width:48px;height:48px;border-radius:8px;background:#111;border:2.5px solid ${ce};
                  display:flex;align-items:center;justify-content:center">
                  <span style="font-size:20px;font-weight:900;color:${U};font-family:Arial Black,Arial;line-height:1">${ge}</span>
                </div>
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(_e=document.getElementById("close-detail"))==null||_e.addEventListener("click",l);let P=new Set;const re=()=>{const se=P.size,pe=document.getElementById("sell-action-panel");if(!pe)return;pe.style.display=se>0?"block":"none",document.getElementById("sell-selected-count").textContent=se;const ce=document.getElementById("evolve-btn");ce&&(ce.textContent=`⬆️ Évoluer ${se>1?"(+"+se+")":""}`)};document.querySelectorAll(".expl-check").forEach(se=>{se.addEventListener("change",()=>{const pe=se.dataset.id;se.checked?P.add(pe):P.delete(pe);const ce=se.closest(".exemplaire-row");ce&&(ce.style.borderColor=se.checked?"#1A6B3C":"#eee"),re()})}),document.querySelectorAll(".exemplaire-row").forEach(se=>{se.addEventListener("click",pe=>{if(pe.target.closest("button")||pe.target.tagName==="INPUT")return;const ce=se.querySelector(".expl-check");ce&&!ce.disabled&&(ce.checked=!ce.checked,ce.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(se=>{se.addEventListener("click",pe=>{pe.stopPropagation();const ce=document.querySelector(`.expl-hist[data-hist="${se.dataset.idx}"]`);ce&&(ce.style.display=ce.style.display==="none"?"flex":"none")})}),(we=document.getElementById("evolve-btn"))==null||we.addEventListener("click",async()=>{if(u<=1)return;const se=[...P];if(!se.length)return;if(P.has(e.id)){const me=document.createElement("div");me.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",me.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;padding-bottom:80px;background:var(--page-bg)">
      ...
      <div style="display:flex;gap:0;min-height:600px">
        ...
        <div style="width:130px;flex-shrink:0;background:rgba(0,0,0,0.3);display:flex;flex-direction:column;align-items:center;padding:12px 8px;gap:10px;border-right:1px solid rgba(255,255,255,0.1)">
          ...
          <div style="display:flex;flex-direction:column;gap:6px;align-items:center" id="subs-list">
            ${s.map(C=>{const M={...C.player,_evolution_bonus:C.evolution_bonus||0};return`<div style="position:relative;flex-shrink:0;overflow:visible">
                ${st({...M,_evolution_bonus:M._evolution_bonus||0},{width:60,showStad:!0,stadDef:p})}
        ...
        <div style="width:130px;flex-shrink:0;background:rgba(0,0,0,0.3);display:flex;flex-direction:column;align-items:center;padding:12px 8px;gap:12px;border-left:1px solid rgba(255,255,255,0.1)">
          ...
          <div style="width:100%;text-align:center">
            ...
            <div id="add-stad-btn-pc" style="cursor:pointer;margin:0 auto;width:fit-content">
              ${l?(()=>{var G;const C=t.stadDefMap[l.stadium_id],M=((G=C==null?void 0:C.club)==null?void 0:G.logo_url)||null;return`<div style="width:100px;height:130px;border-radius:8px;background:linear-gradient(135deg,#1a3a6b,#0a1a3a);border:2px solid #4fc3f7;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px">
                  ${M?`<img src="${M}" style="width:48px;height:48px;object-fit:contain">`:'<span style="font-size:36px">🏟️</span>'}
        ...
        <div style="display:flex;gap:10px;align-items:flex-start">
          ...
          <div style="flex:1;min-width:0">
            ...
            <div style="display:flex;gap:4px;align-items:flex-end;flex-wrap:nowrap;overflow-x:auto" id="subs-list">
              ${s.map(C=>{const M={...C.player,_evolution_bonus:C.evolution_bonus||0};return`<div style="position:relative;flex-shrink:0;overflow:visible">
                  ${st({...M,_evolution_bonus:M._evolution_bonus||0},{width:44,showStad:!0,stadDef:p})}
                  <button data-remove-sub="${C.id}"
          ...
          <div style="flex-shrink:0;text-align:center">
            ...
            <div id="add-stad-btn" style="cursor:pointer">
              ${l?(()=>{var G;const C=t.stadDefMap[l.stadium_id],M=((G=C==null?void 0:C.club)==null?void 0:G.logo_url)||null;return`<div style="width:50px;height:65px;border-radius:6px;background:linear-gradient(135deg,#1a3a6b,#0a1a3a);border:2px solid #4fc3f7;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px">
                  ${M?`<img src="${M}" style="width:26px;height:26px;object-fit:contain">`:'<span style="font-size:18px">🏟️</span>'}
    ...
    <div class="page-body" style="padding-bottom:20px">
  ...
  </div>`;const n=window.innerWidth>=900,x=e.querySelector(".deck-pc-layout"),u=e.querySelector(".deck-mobile-layout");x&&(x.style.display=n?"block":"none"),u&&(u.style.display=n?"none":"block"),to(e,t,d,i),(c=e.querySelector("#builder-back"))==null||c.addEventListener("click",()=>o("decks")),document.getElementById("formation-select").addEventListener("change",C=>{t.formation=C.target.value;const M=Oi(t.formation),G={};M.forEach(H=>{t.slots[H]&&(G[H]=t.slots[H])}),t.slots=G,ct(e,t,i)});const h=()=>{const{openModal:C,closeModal:M}=i,G=`<div style="display:flex;flex-wrap:wrap;gap:8px;padding:8px">
    ...
    </div>`;C("⚽ Choisir une formation",G),document.querySelectorAll("#modal-body [data-forma]").forEach(H=>{H.addEventListener("click",()=>{t.formation=H.dataset.forma,M(),ct(e,t,i)})})};(A=e.querySelector("#formation-mobile-btn"))==null||A.addEventListener("click",h),(g=e.querySelector("#formation-pc-btn"))==null||g.addEventListener("click",h),(I=e.querySelector("#add-stad-btn-pc"))==null||I.addEventListener("click",()=>Ri(t,e,i)),(q=e.querySelector("#add-stad-btn"))==null||q.addEventListener("click",()=>Ri(t,e,i)),(B=e.querySelector("#save-deck"))==null||B.addEventListener("click",()=>oo(t,i)),e.querySelectorAll("[data-remove-sub]").forEach(C=>{C.addEventListener("click",()=>{t.subs=t.subs.filter(M=>M!==C.dataset.removeSub),ct(e,t,i)})}),e.querySelectorAll("#add-sub-btn").forEach(C=>C.addEventListener("click",()=>no(t,e,i)))}function to(e,t,i,o){var I,q;const d=window.innerWidth>=900,a=e.querySelector(d?"#deck-field-pc":"#deck-field-mobile");if(!a)return;const r=(I=t.stadiumCards)==null?void 0:I.find(B=>B.id===t.stadiumCardId),l=r&&((q=t.stadDefMap)==null?void 0:q[r.stadium_id])||null,p=Ht[t.formation]||{},s=Vi(t.formation)||[],n={};for(const B of i){const C=t.slots[B],M=C?t.playerCards.find(G=>G.id===C):null;M!=null&&M.player?n[B]={...M.player,_evolution_bonus:M.evolution_bonus||0,face:M.player.face||null}:n[B]=null}const x=window.innerWidth>=900,u=x?window.innerWidth-280:window.innerWidth-20,h=x?Math.min(u,860):u,_=Math.round(x?h*.82:h*1.05),m=x?84:44;let c="";for(const[B,C]of s){const M=p[B],G=p[C];if(!M||!G)continue;const H=M.x*h,V=Math.round(.03*_+M.y*.85*_),oe=G.x*h,ie=Math.round(.03*_+G.y*.85*_),Q=n[B],R=n[C],P=ot(Q,R);P==="#ff3333"||P==="#cc2222"?c+=`<line x1="${H.toFixed(1)}" y1="${V.toFixed(1)}" x2="${oe.toFixed(1)}" y2="${ie.toFixed(1)}" stroke="${P}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(c+=`<line x1="${H.toFixed(1)}" y1="${V.toFixed(1)}" x2="${oe.toFixed(1)}" y2="${ie.toFixed(1)}" stroke="${P}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,c+=`<line x1="${H.toFixed(1)}" y1="${V.toFixed(1)}" x2="${oe.toFixed(1)}" y2="${ie.toFixed(1)}" stroke="${P}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let A="";const g=Math.round(m*657/507);for(const B of i){const C=p[B];if(!C)continue;const M=n[B],G=C.x*h;C.y*_;const H=Math.round(.03*_+C.y*(.85*_)),V=Math.round(G-m/2),oe=Math.round(H-g/2);if(M){const ie=B.replace(/\d+/,""),Q=l&&(l.club_id&&String(M.club_id)===String(l.club_id)||l.country_code&&M.country_code===l.country_code),R=st({...M,_evolution_bonus:M._evolution_bonus||0},{width:m,showStad:!1,stadDef:l,role:ie});A+=`<div style="position:absolute;left:${V}px;top:${oe}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${B}">
      ...
      </div>`}else{const ie=B.replace(/\d+/,"");A+=`<div style="position:absolute;left:${V}px;top:${oe}px;width:${m}px;height:${g}px;
    ...
    <div style="position:relative;width:${h}px;height:${_}px;margin:0 auto">
      ...
      ${A}
    </div>`,a.querySelectorAll(".deck-slot-hit").forEach(B=>{B.addEventListener("click",()=>io(B.dataset.pos,t,e,o))})}function Ri(e,t,i){var r;const{openModal:o,closeModal:d}=i,a=e.stadiumCards||[];o("🏟️ Choisir un stade",`<div style="display:flex;flex-wrap:wrap;gap:10px;padding:8px">
    ...
    </div>`),(r=document.getElementById("stad-none"))==null||r.addEventListener("click",()=>{e.stadiumCardId=null,d(),ct(t,e,i)}),document.querySelectorAll(".stad-choice").forEach(l=>{l.addEventListener("click",()=>{e.stadiumCardId=l.dataset.stadId,d(),ct(t,e,i)})})}function io(e,t,i,o){var _,m,c,A,g;const{openModal:d,closeModal:a}=o,r=e.replace(/\d+/,""),l=(_=t.stadiumCards)==null?void 0:_.find(I=>I.id===t.stadiumCardId),p=l&&((m=t.stadDefMap)==null?void 0:m[l.stadium_id])||null,s=t.slots[e],n=s?t.playerCards.find(I=>I.id===s):null;(c=n==null?void 0:n.player)==null||c.id;const x=new Set;Object.entries(t.slots).forEach(([I,q])=>{var C;if(I===e||!q)return;const B=t.playerCards.find(M=>M.id===q);(C=B==null?void 0:B.player)!=null&&C.id&&x.add(B.player.id)}),t.subs.forEach(I=>{var B;const q=t.playerCards.find(C=>C.id===I);(B=q==null?void 0:q.player)!=null&&B.id&&x.add(q.player.id)});const u=new Set,h=t.playerCards.filter(I=>{const q=I.player;return!(q.job===r||q.job2===r)||x.has(q.id)||u.has(q.id)?!1:(u.add(q.id),!0)});h.sort((I,q)=>{const B=I.evolution_bonus||0,C=q.evolution_bonus||0,M=(r==="GK"?I.player.note_g:r==="DEF"?I.player.note_d:r==="MIL"?I.player.note_m:I.player.note_a)+(r===I.player.job||r===I.player.job2?B:0);return(r==="GK"?q.player.note_g:r==="DEF"?q.player.note_d:r==="MIL"?q.player.note_m:q.player.note_a)+(r===q.player.job||r===q.player.job2?C:0)-M}),d(`Choisir ${r} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${h.length>0?'<div style="display:flex;flex-wrap:wrap;gap:8px;justify-content:center">'+h.map(I=>{const q={...I.player,_evolution_bonus:I.evolution_bonus||0};return`<div class="player-option" data-card-id="${I.id}" style="cursor:pointer">
          ${st(q,{width:100,showStad:!0,stadDef:p,role:r})}
  ...
  `,document.body.appendChild(d);let a=!1;const r=document.getElementById("pack-cut-zone"),l=document.getElementById("pack-blade");let p=!1;const s=R=>R.touches&&R.touches[0]?R.touches[0].clientX:R.clientX;function n(R){a||(p=!0,l.style.opacity="1",x(R))}function x(R){if(!p||a)return;const P=r.getBoundingClientRect(),re=s(R)-P.left,xe=Math.max(0,Math.min(1,re/P.width));l.style.width=xe*P.width+"px",xe>=.82&&h()}function u(){a||(p=!1,l.style.transition="width .2s ease, opacity .2s ease",l.style.width="0",l.style.opacity="0",setTimeout(()=>{a||(l.style.transition="")},220))}function h(){var P;if(a)return;a=!0,p=!1,l.style.width="100%",l.style.opacity="1",(P=document.getElementById("cut-flash"))==null||P.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const R=document.getElementById("cut-hint");R&&(R.style.opacity="0"),r.classList.add("pack-cut"),setTimeout(()=>{l.style.opacity="0",document.getElementById("pack-phase").style.display="none",A(0)},620)}r.addEventListener("pointerdown",n),window.addEventListener("pointermove",x),window.addEventListener("pointerup",u),r.addEventListener("touchstart",n,{passive:!0}),window.addEventListener("touchmove",x,{passive:!0}),window.addEventListener("touchend",u);let _=0,m=!1;const c=new Set;function A(R){_=R,document.getElementById("reveal-phase").style.display="flex",g(),I(R,0),G()}function g(){const R=document.getElementById("card-dots");R&&(R.innerHTML=e.map((P,re)=>`<div class="card-dot" data-i="${re}" style="width:8px;height:8px;border-radius:50%;background:${re===_?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),R.querySelectorAll(".card-dot").forEach(P=>P.addEventListener("click",()=>B(parseInt(P.dataset.i)))))}function I(R,P){var ce;const re=e[R],xe=document.getElementById("card-counter"),_e=document.getElementById("card-track");xe&&(xe.textContent=`Carte ${R+1} / ${e.length}`);const we=document.getElementById("reveal-btns");we&&(we.style.display=R===e.length-1?"flex":"none");const ve=re.card_type==="player"&&((ce=re.player)==null?void 0:ce.rarity)==="legende",Ee=!c.has(R);c.add(R);let se=0;if(re.card_type==="player"&&re.player){const U=re.player,Z=U.job||"ATT";se=Number(Z==="GK"?U.note_g:Z==="DEF"?U.note_d:Z==="MIL"?U.note_m:U.note_a)||0}const pe=U=>{_e.innerHTML=`
        ...
        </div>`;const Z=document.getElementById("current-card-wrap");P!==0?(Z.style.transition="none",Z.style.transform=`translateX(${P>0?100:-100}%)`,requestAnimationFrame(()=>{Z.style.transition="transform .28s cubic-bezier(.25,1,.5,1)",Z.style.transform="translateX(0)"})):Z.animate([{opacity:0,transform:"scale(.7)"},{opacity:1,transform:"scale(1)"}],{duration:300,easing:"cubic-bezier(.34,1.56,.64,1)"}),U||ve?V():oe(),g()};Ee&&se>6&&re.card_type==="player"&&re.player?q(re,()=>pe(!0)):pe(!1)}function q(R,P){var se;m=!0;const re=R.player,xe=`https://flagsapi.com/${re.country_code}/flat/64.png`,_e=(se=re.clubs)==null?void 0:se.logo_url,we=document.getElementById("walkout-overlay"),ve=document.getElementById("walkout-stage");if(!we||!ve){m=!1,P();return}we.style.display="flex";const Ee=()=>{const pe=ve.firstElementChild;pe&&(pe.classList.remove("wo-in"),pe.classList.add("wo-out"))};ve.innerHTML=`<img class="wo-in" src="${xe}" style="height:130px;border-radius:10px;box-shadow:0 10px 36px rgba(0,0,0,.6)" onerror="this.style.display='none'">`,navigator.vibrate&&navigator.vibrate(30),setTimeout(Ee,2e3),setTimeout(()=>{var pe;ve.innerHTML=_e?`<img class="wo-in" src="${_e}" style="max-height:160px;max-width:210px;object-fit:contain">`:`<div class="wo-in" style="font-size:34px;font-weight:900;color:#fff;text-align:center">${((pe=re.clubs)==null?void 0:pe.encoded_name)||"CLUB"}</div>`,navigator.vibrate&&navigator.vibrate(30)},2450),setTimeout(Ee,4450),setTimeout(()=>{we.style.display="none",ve.innerHTML="",m=!1,navigator.vibrate&&navigator.vibrate([40,30,80]),P()},4900)}function B(R){if(m||R<0||R>=e.length||R===_)return;const P=R>_?1:-1;_=R,I(R,P)}function C(){B(_+1)}function M(){B(_-1)}function G(){const R=document.getElementById("card-viewport");if(!R||R._swipeBound)return;R._swipeBound=!0;let P=0,re=0,xe=0,_e=!1;const we=ce=>ce.touches?ce.touches[0].clientX:ce.clientX,ve=ce=>ce.touches?ce.touches[0].clientY:ce.clientY,Ee=ce=>{_e=!0,P=we(ce),re=ve(ce),xe=0},se=ce=>{if(!_e)return;xe=we(ce)-P;const U=ve(ce)-re;if(Math.abs(xe)<Math.abs(U))return;const Z=document.getElementById("current-card-wrap");Z&&(Z.style.transition="none",Z.style.transform=`translateX(${xe*.6}px) rotate(${xe*.02}deg)`)},pe=()=>{if(!_e)return;_e=!1;const ce=document.getElementById("current-card-wrap"),U=55;xe<=-U&&_<e.length-1?C():xe>=U&&_>0?M():ce&&(ce.style.transition="transform .2s ease",ce.style.transform="translateX(0)")};R.addEventListener("pointerdown",Ee),R.addEventListener("pointermove",se),R.addEventListener("pointerup",pe),R.addEventListener("pointercancel",pe),R.addEventListener("touchstart",Ee,{passive:!0}),R.addEventListener("touchmove",se,{passive:!0}),R.addEventListener("touchend",pe),R.addEventListener("click",ce=>{if(Math.abs(xe)>8)return;const U=R.getBoundingClientRect();ce.clientX-U.left>U.width/2?C():M()})}let H=null;function V(){const R=document.getElementById("fireworks-canvas");if(!R)return;R.width=window.innerWidth,R.height=window.innerHeight;const P=R.getContext("2d"),re=[];function xe(){const we=Math.random()*R.width,ve=Math.random()*R.height*.6,Ee=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],se=Ee[Math.floor(Math.random()*Ee.length)];for(let pe=0;pe<60;pe++){const ce=Math.PI*2/60*pe,U=2+Math.random()*5;re.push({x:we,y:ve,vx:Math.cos(ce)*U,vy:Math.sin(ce)*U,alpha:1,color:se,size:2+Math.random()*3})}}xe(),H=setInterval(xe,600);function _e(){if(document.getElementById("fireworks-canvas")){P.clearRect(0,0,R.width,R.height);for(let we=re.length-1;we>=0;we--){const ve=re[we];if(ve.x+=ve.vx,ve.y+=ve.vy+.08,ve.vy*=.98,ve.alpha-=.018,ve.alpha<=0){re.splice(we,1);continue}P.globalAlpha=ve.alpha,P.fillStyle=ve.color,P.beginPath(),P.arc(ve.x,ve.y,ve.size,0,Math.PI*2),P.fill()}P.globalAlpha=1,(H!==null||re.length>0)&&requestAnimationFrame(_e)}}_e()}function oe(){H!==null&&(clearInterval(H),H=null);const R=document.getElementById("fireworks-canvas");R&&R.getContext("2d").clearRect(0,0,R.width,R.height)}if(o){const R=document.getElementById("reveal-btns");R&&(R.innerHTML='<button class="btn btn-primary" id="reveal-next" style="flex:1">Continuer →</button>'),(Q=document.getElementById("reveal-next"))==null||Q.addEventListener("click",()=>{oe(),d.remove(),o()})}else document.getElementById("reveal-collection").addEventListener("click",()=>{oe(),d.remove(),i("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{oe(),d.remove(),i("boosters")})}function go(e){var t,i,o,d;if(e.card_type==="player"&&e.player)return lo(e);if(e.card_type==="game_changer"){const a=e._gcDef,r=(a==null?void 0:a.gc_type)==="ultra_game_changer",l={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},p={purple:"#b06ce0",light_blue:"#00d4ef"},s=l[a==null?void 0:a.color]||l.purple,n=p[a==null?void 0:a.color]||p.purple,x=(a==null?void 0:a.name)||e.gc_type||"Game Changer",u=(a==null?void 0:a.effect)||((t=xi[e.gc_type])==null?void 0:t.desc)||"",h=a!=null&&a.image_url?`/icons/${a.image_url}`:null,_=((i=xi[e.gc_type])==null?void 0:i.icon)||"⚡";return`<div style="width:170px;height:240px;background:${s};border-radius:14px;border:3px solid ${n};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${n}88;flex-shrink:0">
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      card:cards(id,card_type,formation,stadium_id,evolution_bonus,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
          clubs(encoded_name,logo_url)))`).in("deck_id",r).order("slot_order"),p=[...new Set((l||[]).filter(u=>{var h,_;return((h=u.card)==null?void 0:h.card_type)==="stadium"&&((_=u.card)==null?void 0:_.stadium_id)}).map(u=>u.card.stadium_id))],s={};if(p.length){const{data:u}=await T.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",p);(u||[]).forEach(h=>{s[h.id]=h}),(l||[]).forEach(h=>{var _,m;((_=h.card)==null?void 0:_.card_type)==="stadium"&&((m=h.card)!=null&&m.stadium_id)&&(h.card._stadiumDef=s[h.card.stadium_id]||null)})}let n=0;function x(){var B,C,M,G,H,V,oe;const u=a[n],h=(l||[]).filter(ie=>ie.deck_id===u.id),_=h.filter(ie=>{var Q;return ie.is_starter&&((Q=ie.card)==null?void 0:Q.player)}).map(ie=>bi(ie.card,ie.position)),m=h.find(ie=>{var Q;return((Q=ie.card)==null?void 0:Q.card_type)==="formation"}),c=u.formation||((B=m==null?void 0:m.card)==null?void 0:B.formation)||"4-4-2";let A=_.length>=11?vt(_,c):null,g=((C=u.stadium_card)==null?void 0:C.stadium_def)||null;g&&A&&(A=Tt(A,g));const I=_.length>=11;et(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div style="display:flex;align-items:center;gap:8px;padding:6px 14px;background:linear-gradient(90deg,rgba(232,119,34,0.3),rgba(196,90,0,0.2));border-top:1px solid rgba(232,119,34,0.4);flex-shrink:0">
        ...
        <div style="font-size:11px;font-weight:900;color:#FFD700">+10 aux joueurs ${((M=g.club)==null?void 0:M.encoded_name)||g.country_code||""}</div>
      </div>`:""}
    ...
    </div>`,function(){const Q=e.querySelector(".deck-preview-wrap svg");Q&&(Q.removeAttribute("width"),Q.removeAttribute("height"),Q.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",Q.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),(G=document.getElementById("prev-deck"))==null||G.addEventListener("click",()=>{n>0&&(n--,x())}),(H=document.getElementById("next-deck"))==null||H.addEventListener("click",()=>{n<a.length-1&&(n++,x())}),(V=document.getElementById("validate-deck"))==null||V.addEventListener("click",()=>{if(!I)return;const ie=t.state.params||{};t.navigate("match",{...ie,matchMode:ie.matchMode||i,deckId:u.id})}),(oe=document.getElementById("cancel-deck-select"))==null||oe.addEventListener("click",()=>{Ve(e),d("home")});const q=document.getElementById("deck-swipe-zone");if(q){let ie=0,Q=0;q.addEventListener("touchstart",R=>{ie=R.touches[0].clientX,Q=R.touches[0].clientY},{passive:!0}),q.addEventListener("touchend",R=>{const P=R.changedTouches[0].clientX-ie,re=R.changedTouches[0].clientY-Q;Math.abs(P)<40||Math.abs(P)<Math.abs(re)||(P<0&&n<a.length-1?(n++,x()):P>0&&n>0&&(n--,x()))},{passive:!0})}}x()}function _o(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";return e!=null&&e.clubLogo?e.clubLogo.startsWith("http")?e.clubLogo:`${t}/storage/v1/object/public/assets/clubs/${e.clubLogo}`:null}function Xe(e,t=44,i=58,o=null){return st(e,{width:t,showStad:!!o,stadDef:o,used:e==null?void 0:e.used})}function it(e,t,i){if(!(e!=null&&e.length))return"";const o=e.slice(0,5);let d='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return o.forEach((a,r)=>{if(d+=Xe(a,40,52),r<o.length-1){const l=ot(a,o[r+1]);d+=`<div style="width:7px;height:3px;background:${l==="#ff3333"||l==="#cc2222"?"rgba(255,255,255,0.12)":l};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(d+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),d+="</div>",d}function $o(e){return!e||e.length<2?null:`https://flagsapi.com/${e.slice(0,2).toUpperCase()}/flat/64.png`}function ko(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function pt(e,t,i,o,d=310,a=310,r=[]){var I;const l=Ht[t]||{},p=Vi(t)||_i[t]||[],s={},n=["ATT","MIL","DEF","GK"];for(const q of n)(e[q]||[]).forEach((C,M)=>{s[`${q}${M+1}`]=C});function x(q){const B=l[q];return B?{x:B.x*d,y:B.y*a}:null}let u="";for(const[q,B]of p){const C=x(q),M=x(B);if(!C||!M)continue;const G=s[q],H=s[B],V=ot(G,H);V==="#00ff88"||V==="#FFD700"?(u+=`<line x1="${C.x.toFixed(1)}" y1="${C.y.toFixed(1)}" x2="${M.x.toFixed(1)}" y2="${M.y.toFixed(1)}"
        stroke="${V}" stroke-width="10" stroke-linecap="round" opacity="0.22"/>`,u+=`<line x1="${C.x.toFixed(1)}" y1="${C.y.toFixed(1)}" x2="${M.x.toFixed(1)}" y2="${M.y.toFixed(1)}"
        stroke="${V}" stroke-width="3.5" stroke-linecap="round" opacity="0.95"/>`):u+=`<line x1="${C.x.toFixed(1)}" y1="${C.y.toFixed(1)}" x2="${M.x.toFixed(1)}" y2="${M.y.toFixed(1)}"
        stroke="${V}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const h=48,_=64,m=13,c=16,A={normal:"#aaaaaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#111111",legende:"#7a28b8",légende:"#7a28b8"};for(const[q,B]of Object.entries(s)){const C=x(q);if(!C||!B)continue;const M=q.replace(/[0-9]/g,""),G=Ot[M]||"#555",H=r.includes(B.cardId),V=i==="attack"&&(["MIL","ATT"].includes(M)||H)&&!B.used||i==="defense"&&["GK","DEF","MIL"].includes(M)&&!B.used,oe=o.includes(B.cardId);let ie;i==="attack"?ie=H?Math.max(1,Number(B.note_a)||0):M==="MIL"?Number(B.note_m)||0:Number(B.note_a)||0:i==="defense"?ie=M==="GK"?Number(B.note_g)||0:M==="MIL"?Number(B.note_m)||0:Number(B.note_d)||0:ie=Number(M==="GK"?B.note_g:M==="DEF"?B.note_d:M==="MIL"?B.note_m:B.note_a)||0,ie=ie+(B.boost||0),B.stadiumBonus&&(i==="attack"&&(M==="ATT"||M==="MIL")||i==="defense"&&(M==="GK"||M==="DEF"||M==="MIL")?ie+=10:i||(ie+=10));const Q=(C.x-h/2).toFixed(1),R=(C.y-_/2).toFixed(1),P=A[B==null?void 0:B.rarity]||A.normal;if(B.used){const se=typeof import.meta<"u"&&"/"||"/",ce=`${typeof window<"u"&&((I=window.location)==null?void 0:I.origin)||""}${se}icons/carte-dos.png`.replace(/([^:])\/\//g,"$1/");u+=`<rect x="${Q}" y="${R}" width="${h}" height="${_}" rx="5" fill="#161616"/>`,u+=`<image href="${ce}" xlink:href="${ce}" x="${Q}" y="${R}" width="${h}" height="${_}" preserveAspectRatio="xMidYMid slice"/>`,u+=`<rect x="${Q}" y="${R}" width="${h}" height="${_}" rx="5" fill="none" stroke="${P}" stroke-width="2" opacity="0.7"/>`,u+=`<rect x="${Q}" y="${R}" width="${h}" height="${_}" rx="5" fill="rgba(0,0,0,0.01)" class="match-used-hit" data-card-id="${B.cardId}" data-role="${M}" style="cursor:pointer"/>`;continue}const re=oe?.45:1,xe=oe?"#FFD700":P,_e=oe?3:(B==null?void 0:B.rarity)==="legende"||(B==null?void 0:B.rarity)==="légende"||(B==null?void 0:B.rarity)==="pepite"||(B==null?void 0:B.rarity)==="pépite"?2.5:2,we=_-m-c;u+=`<defs><clipPath id="cp-${q}"><rect x="${Q}" y="${(C.y-_/2+m).toFixed(1)}" width="${h}" height="${we}"/></clipPath></defs>`,u+=`<rect x="${Q}" y="${R}" width="${h}" height="${_}" rx="5" fill="${G}" opacity="${re}"/>`;const ve=He(B);ve&&(u+=`<image href="${ve}" xlink:href="${ve}" x="${Q}" y="${(C.y-_/2+m).toFixed(1)}" width="${h}" height="${we}" clip-path="url(#cp-${q})" preserveAspectRatio="xMidYMin slice"/>`),u+=`<rect x="${Q}" y="${R}" width="${h}" height="${m}" rx="4" fill="rgba(255,255,255,0.92)"/>`,u+=`<text x="${C.x.toFixed(1)}" y="${(C.y-_/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(B.name||"").slice(0,9)}</text>`;const Ee=(C.y+_/2-c).toFixed(1);u+=`<rect x="${Q}" y="${Ee}" width="${h}" height="${c}" fill="rgba(255,255,255,0.92)"/>`;{const se=$o(B.country_code);se?u+=`<image href="${se}" xlink:href="${se}" x="${(C.x-20).toFixed(1)}" y="${(C.y+_/2-c+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:u+=`<text x="${(C.x-13).toFixed(1)}" y="${(C.y+_/2-c/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${ko(B.country_code)}</text>`;const pe=B.stadiumBonus?"#E87722":"#111";u+=`<text x="${C.x.toFixed(1)}" y="${(C.y+_/2-c/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="${pe}" font-family="Arial Black">${ie}</text>`;const ce=_o(B);ce?u+=`<image href="${ce}" xlink:href="${ce}" x="${(C.x+h/2-14).toFixed(1)}" y="${(C.y+_/2-c+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:B.clubName&&(u+=`<text x="${(C.x+14).toFixed(1)}" y="${(C.y+_/2-c/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(B.clubName||"").slice(0,3).toUpperCase()}</text>`),B.stadiumBonus&&(u+=`<rect x="${(C.x-h/2).toFixed(1)}" y="${(C.y-_/2).toFixed(1)}" width="${h}" height="${_}" rx="5" fill="none" stroke="#E87722" stroke-width="2" opacity="0.8"/>`)}u+=`<rect x="${Q}" y="${R}" width="${h}" height="${_}" rx="5" fill="${oe?"rgba(255,255,255,0.12)":"none"}" stroke="${xe}" stroke-width="${_e}" opacity="${re}"/>`,V&&(u+=`<rect x="${Q}" y="${R}" width="${h}" height="${_}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${oe?"selected":""}" data-card-id="${B.cardId}" data-role="${M}" style="cursor:pointer"/>`)}const g=38;return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-g} ${-g} ${d+g*2} ${a+g*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${u}
  ...
  </div>`}async function ai(e,t,i,o){var C;const{state:d,navigate:a,toast:r}=t;et(e);const l=d.params||{};if(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!l.deckId)return wo(e,t,i);const p=l.deckId;let s,n,x,u;try{const M=await Promise.all([T.from("decks").select("formation,name,stadium_card_id").eq("id",p).single(),T.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation, evolution_bonus,
            player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
              clubs(encoded_name,logo_url)))`).eq("deck_id",p).order("slot_order")]);s=M[0].data,x=M[0].error,n=M[1].data,u=M[1].error}catch(M){console.error("[Match] Exception chargement deck:",M),Et(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>a("home"));return}if(x||u){console.error("[Match] Erreur Supabase:",x||u),Et(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>a("home"));return}const h=(n||[]).filter(M=>{var G;return M.is_starter&&((G=M.card)==null?void 0:G.player)}).map(M=>bi(M.card,M.position)),_=(n||[]).filter(M=>{var G;return!M.is_starter&&((G=M.card)==null?void 0:G.player)}).map(M=>bi(M.card,M.position));if(h.length<11){Et(e,"⚠️",`Deck incomplet (${h.length}/11).`,"Compléter",()=>a("decks"));return}const m=(n||[]).find(M=>{var G;return((G=M.card)==null?void 0:G.card_type)==="formation"}),c=(s==null?void 0:s.formation)||((C=m==null?void 0:m.card)==null?void 0:C.formation)||"4-4-2",{data:A,error:g}=await T.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",d.profile.id).eq("card_type","game_changer"),{data:I}=await T.from("gc_definitions").select("*").eq("is_active",!0),q=(A||[]).map(M=>({...M,_gcDef:(I==null?void 0:I.find(G=>G.name===M.gc_type||G.id===M.gc_definition_id))||null}));let B=null;if(s!=null&&s.stadium_card_id){const{data:M}=await T.from("cards").select("stadium_id").eq("id",s.stadium_card_id).single();if(M!=null&&M.stadium_id){const{data:G}=await T.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").eq("id",M.stadium_id).single();B=G||null}}o({deckId:p,formation:c,starters:h,subsRaw:_,gcCardsEnriched:q,gcDefs:I||[],stadiumDef:B})}async function Eo(e,t){const{state:i}=t,d=(i.params||{}).matchMode||"vs_ai_easy",a=d.replace("vs_ai_",""),r=d;await ai(e,t,d,async({deckId:l,formation:p,starters:s,subsRaw:n,gcCardsEnriched:x,gcDefs:u,stadiumDef:h})=>{try{let _=vt(s,p);h&&(_=Tt(_,h),jt(n,h));const m=await To(p,a),c=m.lines||m,A=async g=>{try{const{data:I,error:q}=await T.from("matches").insert({home_id:i.profile.id,away_id:null,mode:r,home_deck_id:l,status:"in_progress"}).select().single();if(q){console.error("[MatchIA] Erreur création match:",q),Et(e,"⚠️","Impossible de créer le match ("+q.message+").","Retour",()=>t.navigate("home"));return}const B=m.stadiumDef||null;B&&c&&(Tt(c,B),jt(m.subs||[],B));const C={gcDefs:u||[],matchId:I==null?void 0:I.id,mode:r,difficulty:a,formation:p,homeTeam:_,aiTeam:c,homeSubs:n,subsUsed:0,maxSubs:Math.min(n.length,3),aiSubs:m.subs||[],aiSubsUsed:0,aiMaxSubs:Math.min((m.subs||[]).length,3),aiUsedSubIds:[],aiGcCards:m.gcCards||[],aiUsedGc:[],aiStadiumDef:B,homeScore:0,aiScore:0,gcCards:g,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};zo(e,C,t)}catch(I){console.error("[MatchIA] Exception launchMatch:",I),Et(e,"⚠️","Erreur au lancement du match : "+I.message,"Retour",()=>t.navigate("home"))}};if(!x.length){A([]);return}ri(e,x,A)}catch(_){console.error("[MatchIA] Exception setup:",_),Et(e,"⚠️","Erreur de préparation du match : "+_.message,"Retour",()=>t.navigate("home"))}})}async function To(e,t){var c;const{data:i}=await T.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(80);if(!i||i.length<11)return{lines:So(e),subs:[],gcCards:[],stadiumDef:null};const o=Mt[e]||Mt["4-4-2"],d={GK:[],DEF:[],MIL:[],ATT:[]},a=new Set;function r(A,g,I){var q,B;return a.add(A.id),{cardId:"ai-"+A.id+"-"+I,id:A.id,firstname:A.firstname,name:A.surname_encoded,country_code:A.country_code,club_id:A.club_id,job:A.job,job2:A.job2,note_g:Number(A.note_g)||0,note_d:Number(A.note_d)||0,note_m:Number(A.note_m)||0,note_a:Number(A.note_a)||0,rarity:A.rarity,skin:A.skin,hair:A.hair,hair_length:A.hair_length,clubName:((q=A.clubs)==null?void 0:q.encoded_name)||null,clubLogo:((B=A.clubs)==null?void 0:B.logo_url)||null,boost:0,used:!1,_line:g}}for(const A of["GK","DEF","MIL","ATT"]){const g=i.filter(M=>M.job===A&&!a.has(M.id)),I=i.filter(M=>M.job!==A&&!a.has(M.id)),q=[...g,...I],B=[];for(let M=0;M<o[A];M++){const G=q[M];G&&B.push(r(G,A,M))}const C=dt(B.length);B.forEach((M,G)=>{M._col=C[G]}),d[A]=B}const p=i.filter(A=>!a.has(A.id)).slice(0,5).map((A,g)=>r(A,A.job,100+g)),x=Object.keys(Oe).sort(()=>Math.random()-.5).slice(0,3).map((A,g)=>{var I,q;return{id:"ai-gc-"+g,gc_type:A,name:((I=Oe[A])==null?void 0:I.name)||A,icon:((q=Oe[A])==null?void 0:q.icon)||"⚡"}}),u=Object.values(d).flat(),h={};u.forEach(A=>{A.club_id&&(h[A.club_id]=(h[A.club_id]||0)+1)});const _=(c=Object.entries(h).sort((A,g)=>g[1]-A[1])[0])==null?void 0:c[0];let m=null;if(_){const{data:A}=await T.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",_).single();A&&(m={club_id:A.id,country_code:null,name:A.encoded_name+" Stadium",club:{encoded_name:A.encoded_name,logo_url:A.logo_url}})}return{lines:d,subs:p,gcCards:x,stadiumDef:m}}function So(e){const t=Mt[e]||Mt["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},o=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let d=0;for(const a of["GK","DEF","MIL","ATT"]){const r=[];for(let p=0;p<t[a];p++){const s=3+Math.floor(Math.random()*5);r.push({cardId:"fake-"+d,id:"fake-"+d,firstname:"IA",name:o[d%o.length],country_code:"XX",club_id:null,job:a,job2:null,note_g:a==="GK"?s:2,note_d:a==="DEF"?s:2,note_m:a==="MIL"?s:2,note_a:a==="ATT"?s:2,rarity:"normal",boost:0,used:!1,_line:a}),d++}const l=dt(r.length);r.forEach((p,s)=>{p._col=l[s]}),i[a]=r}return i}function zo(e,t,i){var o;e.innerHTML=`
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
  </div>`,setTimeout(()=>Ao(e,t,i),5e3)}function Ao(e,t,i){const o=t.homeTeam.MIL||[],d=t.aiTeam.MIL||[],a=t.stadiumDef||null,r=t.aiStadiumDef||null;function l(g,I){const q=ze(g,"MIL"),B=g.evolution_bonus||g._evolution_bonus||0,C=g.job==="MIL"||g.job2==="MIL"?B:0,M=g.stadiumBonus||I&&(I.club_id&&String(g.club_id)===String(I.club_id)||I.country_code&&g.country_code===I.country_code)?10:0;return q+C+M}function p(g,I){return g.reduce((q,B)=>q+l(B,I),0)}function s(g){let I=0;for(let q=0;q<g.length-1;q++){const B=ot(g[q],g[q+1]);B==="#00ff88"?I+=2:B==="#FFD700"&&(I+=1)}return I}const n=p(o,a)+s(o),x=p(d,r)+s(d),u=n>=x;function h(g,I,q,B,C){return`<div id="duel-row-${B}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0), opacity .5s ease;transform-origin:center">
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${g.map((M,G)=>{const H=G<g.length-1?ot(M,g[G+1]):null,V=!H||H==="#ff3333"||H==="#cc2222",oe=H==="#00ff88"?"+2":H==="#FFD700"?"+1":"";l(M,C),M.stadiumBonus||C&&(C.club_id&&String(M.club_id)===String(C.club_id)||C.country_code&&(M.country_code,C.country_code));const ie=M.evolution_bonus||M._evolution_bonus||0;return`
          <div class="duel-card duel-card-${B}" data-idx="${G}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease, transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
            ${Xe({...M,_evolution_bonus:ie},58,78,C)}
          </div>
          ${G<g.length-1?`<div class="duel-link duel-link-${B}" data-idx="${G}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${V?"rgba(255,255,255,0.12)":H};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${V?"none":`0 0 8px ${H}`}">
            ${oe?`<span style="position:absolute;top:-13px;left:50%;transform:translateX(-50%);font-size:8px;font-weight:900;color:${H}">${oe}</span>`:""}
          </div>`:""}
          `}).join("")}
      </div>
      <div class="duel-score-line duel-score-line-${B}" style="margin-top:10px;font-size:12px;color:rgba(255,255,255,0.55);opacity:0;transition:opacity .4s ease">
        Score: ${p(g,C)} + ${s(g)} liens = <b style="color:#fff">${p(g,C)+s(g)}</b>
      </div>
    </div>`}e.innerHTML=`
  <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div id="duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center, rgba(10,61,30,0.4), rgba(10,61,30,0.92))"></div>
  </div>`;const _=()=>{const g=(I,q)=>e.querySelectorAll(I).forEach((B,C)=>{setTimeout(()=>{B.style.opacity="1",B.style.transform="translateY(0) scale(1)"},q+C*90)});g(".duel-card-home",150),g(".duel-card-ai",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((I,q)=>{setTimeout(()=>{I.style.opacity="1"},q*70)}),900),setTimeout(()=>{const I=e.querySelector("#vs-label");I&&(I.style.opacity="1",I.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(q=>q.style.opacity="1")},1250),setTimeout(()=>{m("score-home",n,800),m("score-ai",x,800)},1500)};function m(g,I,q){const B=document.getElementById(g);if(!B)return;const C=performance.now(),M=G=>{const H=Math.min(1,(G-C)/q);B.textContent=Math.round(I*(1-Math.pow(1-H,3))),H<1?requestAnimationFrame(M):B.textContent=I};requestAnimationFrame(M)}requestAnimationFrame(_),t.attacker=u?"home":"ai";const c=u?oi():null;u&&(t.boostCard={value:c}),t.log.push({type:"duel",title:"Milieu de Terrain",homePlayers:o.map(g=>({name:g.name,note:ze(g,"MIL"),portrait:He(g),job:g.job,country_code:g.country_code,rarity:g.rarity,clubName:g.clubName,clubLogo:g.clubLogo})),aiPlayers:d.map(g=>({name:g.name,note:ze(g,"MIL"),portrait:He(g),job:g.job,country_code:g.country_code,rarity:g.rarity,clubName:g.clubName,clubLogo:g.clubLogo})),homeTotal:n,aiTotal:x,text:`Duel milieu : ${t.clubName} ${n} – ${x} IA → ${u?t.clubName+" attaque":"IA attaque"}`});const A=()=>{t.phase=t.attacker==="home"?"attack":"ai-attack",Je(e,t,i),t.attacker==="ai"&&setTimeout(()=>ei(e,t,i),800)};setTimeout(()=>{const g=document.getElementById("score-home"),I=document.getElementById("score-ai"),q=document.getElementById(u?"duel-row-home":"duel-row-ai"),B=document.getElementById(u?"duel-row-ai":"duel-row-home"),C=u?g:I,M=u?I:g;C&&(C.style.fontSize="80px",C.style.color=u?"#FFD700":"#ff6b6b",C.style.animation="duelPulse .5s ease"+(u?", duelGlow 1.5s ease infinite .5s":"")),M&&(M.style.opacity="0.25"),setTimeout(()=>{q&&(q.style.transformOrigin="center",q.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",q.style.zIndex="5"),setTimeout(()=>{var H;const G=document.getElementById("duel-shock");if(G){const V=(H=B||q)==null?void 0:H.getBoundingClientRect(),oe=e.querySelector(".match-screen").getBoundingClientRect();V&&(G.style.top=V.top-oe.top+V.height/2+"px"),G.style.animation="shockwave .5s ease-out forwards"}B&&(B.style.transformOrigin="center",B.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var H;const G=document.getElementById("duel-finale");G&&(G.innerHTML=`
          ...
          <button id="start-match-btn" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,0.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">
            ▶ Commencer le match
          </button>`,G.style.transition="opacity .45s ease",G.style.opacity="1",G.style.pointerEvents="auto",(H=document.getElementById("start-match-btn"))==null||H.addEventListener("click",A))},600)},700)},2800)}function Lo(e){if(e.type==="duel"&&(e.homeTotal!=null||e.aiTotal!=null)){const t=(e.homeTotal||0)>=(e.aiTotal||0);return`
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    ${(()=>{const P=window.innerWidth>=700,re=(U,Z,ge)=>{var F,$;const de=(t.gcDefs||[]).find(f=>f.name===U.gc_type),me={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[de==null?void 0:de.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",ne={purple:"#b06ce0",light_blue:"#00d4ef"}[de==null?void 0:de.color]||"#b06ce0",ue=(de==null?void 0:de.name)||U.gc_type,Le=(de==null?void 0:de.effect)||((F=Oe[U.gc_type])==null?void 0:F.desc)||"",Ie=de!=null&&de.image_url?`/icons/${de.image_url}`:null,Ce=(($=Oe[U.gc_type])==null?void 0:$.icon)||"⚡",Te=Math.round(ge*.22),qe=Math.round(ge*.22),Pe=ge-Te-qe,Se=ue.length>12?7:9;return`<div class="gc-mini" data-gc-id="${U.id}" data-gc-type="${U.gc_type}"
        ...
        </div>`},xe=(U,Z)=>{var ge;return`<div id="boost-card"
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(a);let l=!1;const p=()=>{l||(l=!0,a.remove(),setTimeout(()=>d(),50))};a.addEventListener("click",p),setTimeout(p,3500)}async function Dt(e,t,i){var n,x;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:o}=i,d=t.homeScore>t.aiScore,a=t.homeScore===t.aiScore,r=d?"victoire":a?"nul":"defaite",l=On(t.mode,r);t.matchId&&await T.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:d?o.profile.id:null,home_credits_reward:l,played_at:new Date().toISOString()}).eq("id",t.matchId);const p={credits:(o.profile.credits||0)+l,matches_played:(o.profile.matches_played||0)+1};d?p.wins=(o.profile.wins||0)+1:a?p.draws=(o.profile.draws||0)+1:p.losses=(o.profile.losses||0)+1,await T.from("users").update(p).eq("id",o.profile.id),await i.refreshProfile();const s=document.createElement("div");s.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",s.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const q=async(G=!0)=>{_=!0,m&&(T.removeChannel(m),m=null),G&&await T.rpc("cancel_matchmaking",{p_user_id:x.profile.id}).catch(()=>{})};(M=document.getElementById("mm-cancel"))==null||M.addEventListener("click",async()=>{try{await q(!0)}catch{}Ve(e),u("home")});const B=async(G,H,V)=>{if(_)return;_=!0,m&&(T.removeChannel(m),m=null);const oe=document.getElementById("mm-status");oe&&(oe.textContent="Adversaire trouvé !"),await new Promise(ie=>setTimeout(ie,600)),e.isConnected&&fn(e,t,G,V,r,l,s,n,p)};if(s){const G=[{range:50,delay:0},{range:100,delay:15e3},{range:200,delay:3e4},{range:400,delay:45e3},{range:800,delay:6e4}];let H=0,V=!1;const oe=async()=>{if(_||V)return;const{range:ie}=G[H],Q=document.getElementById("mm-range");Q&&(Q.textContent=`Plage MMR : ±${ie}`);const{data:R,error:P}=await T.rpc("try_matchmake_ranked",{p_user_id:x.profile.id,p_deck_id:i,p_mmr:c,p_range:ie});if(console.log("[Ranked] try_matchmake_ranked result:",R,"error:",JSON.stringify(P)),_)return;if(P||!(R!=null&&R.success)){h("Erreur matchmaking ranked","error"),Ve(e),u("home");return}if(R.matched){V=!0,B(R.match_id,R.opponent_id,!1);return}const re=document.getElementById("mm-status");if(re&&(re.textContent="En attente d'un adversaire..."),m||(m=T.channel("matchmaking-"+x.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${x.profile.id}`},xe=>{const _e=xe.new;_e.status==="matched"&&_e.match_id&&!V&&(V=!0,B(_e.match_id,_e.matched_with,!0))}).subscribe()),H<G.length-1){const xe=G[H+1].delay-G[H].delay;H++,setTimeout(()=>{!_&&!V&&oe()},xe)}};await oe()}else{const{data:G,error:H}=await T.rpc("try_matchmake",{p_user_id:x.profile.id,p_deck_id:i});if(H||!(G!=null&&G.success)){h("Erreur de matchmaking","error"),Ve(e),u("home");return}if(G.matched){B(G.match_id,G.opponent_id,!1);return}const V=document.getElementById("mm-status");V&&(V.textContent="En attente d'un autre joueur..."),m=T.channel("matchmaking-"+x.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${x.profile.id}`},oe=>{const ie=oe.new;ie.status==="matched"&&ie.match_id&&B(ie.match_id,ie.matched_with,!0)}).subscribe()}}async function Ho(e,t,i){const{state:o,navigate:d,toast:a}=t;try{(T.getChannels?T.getChannels():[]).forEach(s=>{const n=s.topic||"";(n.includes("matchmaking")||n.includes("pvp-match")||n.includes("friend-invite"))&&T.removeChannel(s)})}catch{}const{data:r}=await T.from("matches").select("home_id, away_id, status").eq("id",i).single();if(!r){a("Match introuvable","error"),d("home");return}if(r.status==="finished"){a("Ce match est terminé","info"),d("home");return}const l=r.home_id===o.profile.id;fn(e,t,i,l,[],[])}async function fn(e,t,i,o,d=[],a=[],r=!1,l=null,p=null){const{state:s,navigate:n,toast:x}=t,u=o?"p1":"p2",h=o?"p2":"p1",_=(d||[]).map($=>$.id),m=(d||[]).map($=>({id:$.id,gc_type:$.gc_type,_gcDef:$._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:c}=await T.from("matches").select("*").eq("id",i).single();if(!c){x("Match introuvable","error"),n("home");return}async function A(){var W,te;console.log("[PvP] buildGameState match:",{id:c.id,home_deck_id:c.home_deck_id,away_deck_id:c.away_deck_id,mode:c.mode,is_ranked:c.is_ranked});const[{data:$,error:f},{data:y,error:b},{data:v},{data:w}]=await Promise.all([T.rpc("get_deck_for_match",{p_deck_id:c.home_deck_id}),T.rpc("get_deck_for_match",{p_deck_id:c.away_deck_id}),T.from("users").select("id,pseudo,club_name").eq("id",c.home_id).single(),T.from("users").select("id,pseudo,club_name").eq("id",c.away_id).single()]);console.log("[PvP] get_deck_for_match p1:",f==null?void 0:f.message,"p2:",b==null?void 0:b.message,"p1D:",(W=$==null?void 0:$.starters)==null?void 0:W.length,"p2D:",(te=y==null?void 0:y.starters)==null?void 0:te.length);const k=ee=>{const fe=Number(ee.evolution_bonus)||0;return{cardId:ee.card_id,position:ee.position,id:ee.id,firstname:ee.firstname,name:ee.surname_encoded,country_code:ee.country_code,club_id:ee.club_id,job:ee.job,job2:ee.job2,note_g:(Number(ee.note_g)||0)+(ee.job==="GK"||ee.job2==="GK"&&Number(ee.note_g)>0?fe:0),note_d:(Number(ee.note_d)||0)+(ee.job==="DEF"||ee.job2==="DEF"&&Number(ee.note_d)>0?fe:0),note_m:(Number(ee.note_m)||0)+(ee.job==="MIL"||ee.job2==="MIL"&&Number(ee.note_m)>0?fe:0),note_a:(Number(ee.note_a)||0)+(ee.job==="ATT"||ee.job2==="ATT"&&Number(ee.note_a)>0?fe:0),evolution_bonus:fe,rarity:ee.rarity,skin:ee.skin,hair:ee.hair,hair_length:ee.hair_length,clubName:ee.club_encoded_name||null,clubLogo:ee.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},S=(($==null?void 0:$.starters)||[]).map(ee=>k(ee)),E=((y==null?void 0:y.starters)||[]).map(ee=>k(ee)),L=($==null?void 0:$.formation)||"4-4-2",z=(y==null?void 0:y.formation)||"4-4-2";let j=vt(S,L),D=vt(E,z);const Y=(($==null?void 0:$.subs)||[]).map(ee=>k(ee)),J=((y==null?void 0:y.subs)||[]).map(ee=>k(ee)),K=($==null?void 0:$.stadium_def)||(o?p:null),N=(y==null?void 0:y.stadium_def)||(o?null:p);return K&&(j=Tt(j,K),jt(Y,K)),N&&(D=Tt(D,N),jt(J,N)),{p1Team:j,p2Team:D,p1Subs:Y,p2Subs:J,p1Formation:L,p2Formation:z,p1Name:(v==null?void 0:v.club_name)||(v==null?void 0:v.pseudo)||"Joueur 1",p2Name:(w==null?void 0:w.club_name)||(w==null?void 0:w.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:o?_:[],gc_p2:o?[]:_,gcCardsFull_p1:o?m:[],gcCardsFull_p2:o?[]:m,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:a||[],lastActionAt:new Date().toISOString()}}let g=c.game_state&&Object.keys(c.game_state).length?c.game_state:null;if(console.log("[PvP] init - amIHome:",o,"gameState exists:",!!g,"match.status:",c.status,"home_id:",c.home_id,"away_id:",c.away_id,"myId:",s.profile.id),!g)if(o){g=await A(),console.log("[PvP] home - gameState built, p1Team keys:",Object.keys((g==null?void 0:g.p1Team)||{}));const{data:$}=await T.from("matches").select("game_state").eq("id",i).single();!($!=null&&$.game_state)||!Object.keys($.game_state).length?await T.from("matches").update({game_state:g,turn_user_id:c.home_id}).eq("id",i):g=$.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>',console.log("[PvP] away - waiting for home to write game_state...");for(let $=0;$<30&&!g;$++){await new Promise(y=>setTimeout(y,400));const{data:f}=await T.from("matches").select("game_state").eq("id",i).single();f!=null&&f.game_state&&Object.keys(f.game_state).length&&(g=f.game_state),$%5===0&&console.log("[PvP] away - poll",$,"game_state:",!!(f!=null&&f.game_state))}if(!g){x("Erreur de synchronisation","error"),n("home");return}g.gc_p2=_,g.gcCardsFull_p2=m,await T.from("matches").update({game_state:g}).eq("id",i)}let I=!1,q=null,B=!1;const C=new Set,M=new Set;async function G($){var z,j;try{T.removeChannel(H)}catch{}const f=g[u+"Score"]||0,y=g[h+"Score"]||0;let b,v;$.winner_id?(b=$.winner_id===s.profile.id,v=!1):$.forfeit?(b=f>y,v=!1):(v=f===y,b=f>y);let w="";if(r&&o)try{const{data:D}=await T.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",c.home_id).single(),{data:Y}=await T.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",c.away_id).single();if(D&&Y){const J=b?1:v?.5:0,K=1-J,N=D.placement_matches<10,W=Y.placement_matches<10,te=Li(D.mmr,D.mmr_deviation,D.mmr_volatility,Y.mmr,Y.mmr_deviation,J===1?1:J===0?0:.5,N),ee=Li(Y.mmr,Y.mmr_deviation,Y.mmr_volatility,D.mmr,D.mmr_deviation,K===1?1:K===0?0:.5,W);await T.rpc("update_mmr_after_ranked",{p_match_id:i,p_winner_id:v?null:b?c.home_id:c.away_id,p_home_id:c.home_id,p_away_id:c.away_id,p_home_delta:te.delta,p_away_delta:ee.delta,p_home_new_rd:te.newRd,p_away_new_rd:ee.newRd,p_home_new_vol:te.newSigma,p_away_new_vol:ee.newSigma});const fe=te.delta,ye=te.newMmr,be=Gt(ye),ke=fe>=0?"+":"",Be=fe>=0?"#4caf50":"#ff6b6b",Fe=Gt(D.mmr);w=`
      ...
      ${w}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${r?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(k),(j=k.querySelector("#pvp-end-home"))==null||j.addEventListener("click",()=>{k.remove(),Ve(e),n(r?"ranked":"home")})}const H=T.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},$=>{const f=$.new;try{if(f.status==="finished"||f.forfeit){if(I)return;I=!0,q&&(clearInterval(q),q=null),f.game_state&&(g=f.game_state),G(f);return}if(f.game_state){const y=g;g=f.game_state;const b=g._lastGC;if(b&&b.seq&&!M.has(b.seq)&&(M.add(b.seq),b.by!==u)){se(b.type,b.by,()=>xe());return}const v=y[u+"Score"]||0,w=y[h+"Score"]||0,k=g[u+"Score"]||0,S=g[h+"Score"]||0,E=k>v,L=S>w;if((E||L)&&!C.has(g.round)){C.add(g.round);const z=[...g.log||[]].reverse().find(D=>D.isGoal),j=((z==null?void 0:z.homePlayers)||[]).map(D=>({name:D.name,note:D.note,portrait:D.portrait,job:D.job}));Ee(j,k,S,E,()=>xe());return}xe()}}catch(y){console.error("[PvP] crash:",y)}}).subscribe();async function V($){Object.assign(g,$),g.lastActionAt=new Date().toISOString();const{error:f}=await T.from("matches").update({game_state:g}).eq("id",i);f&&x("Erreur de synchronisation","error");try{xe()}catch(y){console.error("[PvP] renderPvpScreen crash:",y)}}async function oe(){if(I)return;I=!0,q&&(clearInterval(q),q=null);const $=o?c.away_id:c.home_id,f=o?"p2":"p1",y=o?"p1":"p2",b={...g,[f+"Score"]:3,[y+"Score"]:0,phase:"finished"};try{await T.from("matches").update({status:"finished",forfeit:!0,winner_id:$,home_score:b.p1Score||0,away_score:b.p2Score||0,game_state:b}).eq("id",i)}catch{}try{T.removeChannel(H)}catch{}setTimeout(()=>{Ve(e),n("home")},800)}const ie={BOOST_STAT:({value:$=1,count:f=1,roles:y=[]},b,v)=>{const w=b[u+"Team"],k=Object.entries(w).filter(([S])=>!y.length||y.includes(S)).flatMap(([S,E])=>E.filter(L=>!L.used).map(L=>({...L,_line:S})));if(!k.length){b.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),v(b);return}Q(k,f,`Choisir ${f} joueur(s) à booster (+${$})`,S=>{S.forEach(E=>{const L=(w[E._line]||[]).find(z=>z.cardId===E.cardId);L&&(L.boost=(L.boost||0)+$,b.log.push({text:`⚡ +${$} sur ${L.name}`,type:"info"}))}),b[u+"Team"]=w,v(b)})},DEBUFF_STAT:({value:$=1,count:f=1,roles:y=[],target:b="ai"},v,w)=>{const k=b==="home"?u:h,S=v[k+"Team"],E=b==="home"?"allié":"adverse",L=Object.entries(S).filter(([z])=>!y.length||y.includes(z)).flatMap(([z,j])=>j.map(D=>({...D,_line:z})));if(!L.length){v.log.push({text:`🎯 Aucun joueur ${E}`,type:"info"}),w(v);return}Q(L,f,`Choisir ${f} joueur(s) ${E}(s) (-${$})`,z=>{z.forEach(j=>{const D=(S[j._line]||[]).find(Y=>Y.cardId===j.cardId);D&&(D.boost=(D.boost||0)-$,v.log.push({text:`🎯 -${$} sur ${D.name}`,type:"info"}))}),v[k+"Team"]=S,w(v)})},GRAY_PLAYER:({count:$=1,roles:f=[],target:y="ai"},b,v)=>{const w=y==="home"?u:h,k=b[w+"Team"],S=y==="home"?"allié":"adverse",E=Object.entries(k).filter(([L])=>!f.length||f.includes(L)).flatMap(([L,z])=>z.filter(j=>!j.used).map(j=>({...j,_line:L})));if(!E.length){b.log.push({text:`❌ Aucun joueur ${S}`,type:"info"}),v(b);return}Q(E,$,`Choisir ${$} joueur(s) ${S}(s) à exclure`,L=>{const z="usedCardIds_"+w,j=new Set(b[z]||[]);L.forEach(D=>{const Y=(k[D._line]||[]).find(J=>J.cardId===D.cardId);Y&&(Y.used=!0,j.add(D.cardId),b.log.push({text:`❌ ${Y.name} exclu !`,type:"info"}))}),b[z]=[...j],b[w+"Team"]=k,v(b)})},REVIVE_PLAYER:({count:$=1,roles:f=[]},y,b)=>{const v=y[u+"Team"],w=Object.entries(v).filter(([k])=>!f.length||f.includes(k)).flatMap(([k,S])=>S.filter(E=>E.used).map(E=>({...E,_line:k})));if(!w.length){y.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),b(y);return}Q(w,$,`Choisir ${$} joueur(s) à ressusciter`,k=>{k.forEach(S=>{const E=(v[S._line]||[]).find(L=>L.cardId===S.cardId);E&&(E.used=!1,y.log.push({text:`💫 ${E.name} ressuscité !`,type:"info"}))}),y[u+"Team"]=v,b(y)})},REMOVE_GOAL:({},$,f)=>{const y=h+"Score";$[y]>0?($[y]--,$.log.push({text:"🚫 Dernier but annulé !",type:"info"})):$.log.push({text:"🚫 Aucun but à annuler",type:"info"}),f($)},ADD_GOAL_DRAW:({},$,f)=>{$[u+"Score"]===$[h+"Score"]?($[u+"Score"]++,$.log.push({text:"🎯 But bonus !",type:"info"})):$.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),f($)},ADD_SUB:({value:$=1},f,y)=>{f.maxSubs=(f.maxSubs||3)+$,f.log.push({text:`🔄 +${$} remplacement(s)`,type:"info"}),y(f)},CUSTOM:({},$,f)=>f($)};function Q($,f,y,b){const v=document.createElement("div");v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let w=[];function k(){var E,L;const S=$.map(z=>{const j={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[z._line]||"#555",D=ze(z,z._line)+(z.boost||0),J=w.find(N=>N.cardId===z.cardId)?"#FFD700":"rgba(255,255,255,0.25)",K=z.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${z.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${J};background:${j};overflow:hidden;cursor:pointer;${K}">
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${ut($,g[u+"Formation"],Ne,L,300,300,fe)}
      </div>
      ...
      </div>`;function Ge(){const X=e.querySelector(".match-screen");if(!X)return;const O=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);X.style.height=O+"px",X.style.minHeight=O+"px",X.style.maxHeight=O+"px",X.style.overflow="hidden";const ae=e.querySelector("#mobile-action-bar"),he=e.querySelector("#mobile-play-area");ae&&he&&(he.style.paddingBottom=ae.offsetHeight+"px")}if(Ge(),setTimeout(Ge,120),setTimeout(Ge,400),B||(B=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",Ge),window.visualViewport.addEventListener("scroll",Ge)),window.addEventListener("resize",Ge)),function(){const O=e.querySelector(".terrain-wrapper svg");O&&(O.removeAttribute("width"),O.removeAttribute("height"),O.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",O.setAttribute("viewBox","-26 -26 352 352"),O.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),e.querySelectorAll(".match-slot-hit").forEach(X=>{X.addEventListener("click",()=>{if(!k&&!S)return;const O=X.dataset.cardId,ae=X.dataset.role,he=($[ae]||[]).find(tt=>tt.cardId===O);if(!he||he.used)return;const $e=fe.includes(O);if(k&&!["MIL","ATT"].includes(ae)&&!$e)return;Array.isArray(g["selected_"+u])||(g["selected_"+u]=[]);const Qe=g["selected_"+u],Ze=Qe.findIndex(tt=>tt.cardId===O);Ze>-1?Qe.splice(Ze,1):Qe.length<3&&Qe.push({...he,_role:ae}),xe()})}),e.querySelectorAll(".match-used-hit").forEach(X=>{X.addEventListener("click",()=>U(X.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(X=>{X.addEventListener("click",()=>U())}),(We=e.querySelector("#pvp-sub-open"))==null||We.addEventListener("click",()=>U()),e.querySelectorAll(".pvp-gc-mini").forEach(X=>{X.addEventListener("click",()=>pe(X.dataset.gcId,X.dataset.gcType))}),(nt=e.querySelector("#pvp-boost-card"))==null||nt.addEventListener("click",()=>ce()),(ft=e.querySelector("#pvp-action"))==null||ft.addEventListener("click",X=>{k?X.currentTarget.dataset.pass==="1"||!Te(u)?me():ne():S&&ue()}),(gt=e.querySelector("#pvp-quit"))==null||gt.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&oe()}),(mt=e.querySelector("#pvp-view-opp"))==null||mt.addEventListener("click",()=>ge()),(le=e.querySelector("#pvp-toggle-history"))==null||le.addEventListener("click",()=>de()),q&&(clearInterval(q),q=null),(k||S)&&!I){let X=30,O=!1;const ae=()=>document.getElementById("pvp-timer"),he=()=>{ae()&&(ae().textContent=X+"s",ae().style.color=O?"#ff4444":"#fff")};he(),q=setInterval(()=>{X--,X<0?O?(clearInterval(q),q=null,k&&!Te(u)?me():oe()):(O=!0,X=15,he()):he()},1e3)}}function _e(){et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
      <div style="font-size:20px;font-weight:900;color:#ff6b6b">${g[h+"Name"]||"Adversaire"}</div>
      <div style="width:min(90vw,420px)">${pt(g[h+"Team"],g[h+"Formation"],null,[],300,300)}</div>
    </div>`,u==="p1"&&setTimeout(async()=>{await V({phase:"midfield"})},5e3)}let we=!1;function ve(){if(we)return;const $=g[u+"Team"].MIL||[],f=g[h+"Team"].MIL||[];function y(N){return N.reduce((W,te)=>W+ze(te,"MIL"),0)}function b(N){let W=0;for(let te=0;te<N.length-1;te++){const ee=ot(N[te],N[te+1]);ee==="#00ff88"?W+=2:ee==="#FFD700"&&(W+=1)}return W}const v=y($)+b($),w=y(f)+b(f),k=v>=w;function S(N,W,te){return`<div id="duel-row-${te}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(w);let E=!1;const L=()=>{E||(E=!0,w.remove(),setTimeout(()=>v(),50))};w.addEventListener("click",L),setTimeout(L,3500)}function se($,f,y){var K,N;const b=(g.gcDefs||[]).find(W=>{var te;return W.name===$||((te=W.name)==null?void 0:te.toLowerCase().trim())===($==null?void 0:$.toLowerCase().trim())}),v={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[b==null?void 0:b.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",w={purple:"#b06ce0",light_blue:"#00d4ef"}[b==null?void 0:b.color]||"#b06ce0",k=(b==null?void 0:b.name)||$,S=(b==null?void 0:b.effect)||((K=Oe[$])==null?void 0:K.desc)||"",E=b!=null&&b.image_url?`/icons/${b.image_url}`:null,L=((N=Oe[$])==null?void 0:N.icon)||"⚡",j=f===u?"Vous":g[f+"Name"]||"Adversaire",D=document.createElement("div");D.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",D.innerHTML=`
      <style>
      ...
      </div>`,document.body.appendChild(y),(b=y.querySelector("#bp-close"))==null||b.addEventListener("click",()=>y.remove()),y.querySelectorAll(".bp-item").forEach(v=>{v.addEventListener("click",async()=>{const w=v.dataset.cid,k=f.find(E=>E.cardId===w);if(!k)return;const S=($[k._line]||[]).find(E=>E.cardId===w);S&&(S.boost=(S.boost||0)+g.boostValue),y.remove(),await V({[u+"Team"]:$,boostUsed:!0})})})}function U($=null){var Y,J;if(!(g.phase===u+"-attack")){x("Remplacement uniquement avant votre attaque","warning");return}const y=g[u+"Team"],b=g["usedSubIds_"+u]||[],v=g.maxSubs||3;if(b.length>=v){x(`Maximum ${v} remplacements atteint`,"warning");return}const w=Object.entries(y).flatMap(([K,N])=>(N||[]).filter(W=>W.used).map(W=>({...W,_line:K}))),k=(g[u+"Subs"]||[]).filter(K=>!b.includes(K.cardId));if(!w.length){x("Aucun joueur utilisé à remplacer","warning");return}if(!k.length){x("Aucun remplaçant disponible","warning");return}let S=Math.max(0,w.findIndex(K=>K.cardId===$));const E=((Y=w[S])==null?void 0:Y._line)||((J=w[S])==null?void 0:J.job);let L=Math.max(0,k.findIndex(K=>K.job===E)),z=!1;const j=document.createElement("div");j.id="pvp-sub-overlay",j.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function D(){var ye,be,ke,Be,Fe,Me;const K=w[S],N=k[L],W=Math.min(130,Math.round((window.innerWidth-90)/2)),te=Math.round(W*1.35),ee=je=>`background:rgba(255,255,255,0.12);border:none;color:${je?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${je?"default":"pointer"};flex-shrink:0`;j.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        ...
        <div id="pout-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
          ...
          <div>${K?Xe({...K,used:!1,boost:0},W,te):"<div>—</div>"}</div>
      ...
      </div>`,(ye=j.querySelector("#psub-close"))==null||ye.addEventListener("click",()=>j.remove()),(be=j.querySelector("#pout-up"))==null||be.addEventListener("click",()=>{S>0&&(S--,D())}),(ke=j.querySelector("#pout-down"))==null||ke.addEventListener("click",()=>{S<w.length-1&&(S++,D())}),(Be=j.querySelector("#pin-up"))==null||Be.addEventListener("click",()=>{L>0&&(L--,D())}),(Fe=j.querySelector("#pin-down"))==null||Fe.addEventListener("click",()=>{L<k.length-1&&(L++,D())});const fe=(je,Ae,Ne,Ue)=>{const Ye=j.querySelector("#"+je);if(!Ye)return;let Ke=0;Ye.addEventListener("touchstart",Re=>{Ke=Re.touches[0].clientY},{passive:!0}),Ye.addEventListener("touchend",Re=>{const Ge=Re.changedTouches[0].clientY-Ke;if(Math.abs(Ge)<30)return;const We=Ae();Ge<0&&We<Ue-1?(Ne(We+1),D()):Ge>0&&We>0&&(Ne(We-1),D())},{passive:!0})};fe("pin-panel",()=>L,je=>L=je,k.length),fe("pout-panel",()=>S,je=>S=je,w.length),(Me=j.querySelector("#psub-confirm"))==null||Me.addEventListener("click",async je=>{if(je.preventDefault(),je.stopPropagation(),z)return;z=!0;const Ae=w[S],Ne=k[L];if(!Ae||!Ne)return;const Ue=Ae._line,Ye=(y[Ue]||[]).findIndex(Ge=>Ge.cardId===Ae.cardId);if(Ye===-1){x("Erreur : joueur introuvable","error"),j.remove();return}const Ke={...Ne,_line:Ue,position:Ae.position,used:!1,boost:0};y[Ue].splice(Ye,1,Ke);const Re=[...b,Ne.cardId];j.remove(),Z(Ae,Ne,async()=>{await V({[u+"Team"]:y,[h+"Team"]:g[h+"Team"],["usedSubIds_"+u]:Re})})})}document.body.appendChild(j),D()}function Z($,f,y){const b={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},v=document.createElement("div");v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const w=(E,L,z)=>`<div style="text-align:center">
      ...
      </div>`,document.body.appendChild(f),(y=f.querySelector("#pvp-hist-close"))==null||y.addEventListener("click",()=>f.remove())}async function me(){if(g.phase!==u+"-attack")return;const $=u==="p1"?"p2":"p1",f=(g.round||0)+1,y=[...g.log||[]];y.push({type:"info",text:`⏭️ ${g[u+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const b=qe(g),v=Te($),w=b||!v?"finished":$+"-attack";await V({["selected_"+u]:[],modifiers:{...g.modifiers,[u]:{}},pendingAttack:null,phase:w,attacker:$,round:f,log:y}),w==="finished"&&await Se(g)}async function ne(){const $=g[u+"Team"],f=!["GK","DEF","MIL","ATT"].some(w=>(g[h+"Team"][w]||[]).some(k=>!k.used)),y=(g["selected_"+u]||[]).map(w=>{const k=($[w._role]||[]).find(D=>D.cardId===w.cardId)||w,S=f&&["GK","DEF"].includes(w._role),E=$[w._role]||[],L=E.findIndex(D=>D.cardId===w.cardId),z=dt(E.length),j=L>=0?z[L]:k._col??1;return{...k,_line:w._role,_col:j,...S?{note_a:Math.max(1,Number(k.note_a)||0)}:{}}});if(!y.length)return;const b=Ut(y,g.modifiers[u]||{});P(u,y.map(w=>w.cardId)),y.forEach(w=>{const k=($[w._role]||[]).find(S=>S.cardId===w.cardId);k&&(k.used=!0)}),g["selected_"+u]=[],xe();const v=[...g.log||[]];if(f){const w=(g[u+"Score"]||0)+1,k=y.map(j=>({name:j.name,note:ze(j,j._line||"ATT"),portrait:He(j),job:j.job}));v.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:k,homeTotal:b.total,aiTotal:0});const S=(g.round||0)+1,E=u==="p1"?"p2":"p1",L={...g,[u+"Team"]:$,[u+"Score"]:w},z=qe(L);C.add(S),Ee(k,w,g[h+"Score"]||0,!0,async()=>{await V({[u+"Team"]:$,[u+"Score"]:w,["selected_"+u]:[],modifiers:{...g.modifiers,[u]:{}},pendingAttack:null,phase:z?"finished":E+"-attack",attacker:E,round:S,log:v}),z&&await Se({...g,[u+"Score"]:w})});return}v.push({type:"pending",text:`⚔️ ${g[u+"Name"]} attaque (${b.total})`}),await V({[u+"Team"]:$,[h+"Team"]:g[h+"Team"],pendingAttack:{...b,players:y,side:u},["selected_"+u]:[],modifiers:{...g.modifiers,[u]:{}},phase:h+"-defense",log:v})}async function ue(){const $=g[u+"Team"],f=(g["selected_"+u]||[]).map(K=>{const N=($[K._role]||[]).find(ye=>ye.cardId===K.cardId)||K,W=$[K._role]||[],te=W.findIndex(ye=>ye.cardId===K.cardId),ee=dt(W.length),fe=te>=0?ee[te]:N._col??1;return{...N,_line:K._role,_col:fe}}),y=Kt(f,g.modifiers[u]||{});P(u,f.map(K=>K.cardId)),f.forEach(K=>{const N=($[K._role]||[]).find(W=>W.cardId===K.cardId);N&&(N.used=!0)}),g["selected_"+u]=[],xe();const b=Vt(g.pendingAttack.total,y.total,g.modifiers[u]||{}),v=g.pendingAttack.side,w=b==="attack"||(b==null?void 0:b.goal),k=v==="p1"?"p2":"p1",S=(g.round||0)+1,E=(g.pendingAttack.players||[]).map(K=>({name:K.name,note:ze(K,K._line||"ATT"),portrait:He(K),job:K.job})),L=[...g.log||[]];L.push({type:"duel",isGoal:w,homeScored:w&&v===u,text:w?`⚽ BUT de ${g[v+"Name"]} ! (${g.pendingAttack.total} vs ${y.total})`:`✋ Attaque stoppée (${g.pendingAttack.total} vs ${y.total})`,homePlayers:E,aiPlayers:f.map(K=>({name:K.name,note:ze(K,K._line||"DEF"),portrait:He(K),job:K.job})),homeTotal:g.pendingAttack.total,aiTotal:y.total});const z=w?(g[v+"Score"]||0)+1:g[v+"Score"]||0,j={...g,[u+"Team"]:$,[v+"Score"]:z},D=qe(j),Y=D?"finished":k+"-attack",J=async()=>{await V({[u+"Team"]:$,[h+"Team"]:g[h+"Team"],[v+"Score"]:z,["selected_"+u]:[],modifiers:{...g.modifiers,[u]:{}},pendingAttack:null,phase:Y,attacker:k,round:S,log:L}),(Y==="finished"||D)&&await Se({...g,[v+"Score"]:z})};if(w){const K=v===u,N=K?z:g[u+"Score"]||0,W=K?g[h+"Score"]||0:z;C.add(S),Ee(E,N,W,K,J)}else await J()}function Le($){return["MIL","ATT"].some(f=>($[f]||[]).some(y=>!y.used))}function Ie($){return["GK","DEF","MIL","ATT"].some(f=>($[f]||[]).some(y=>!y.used))}function Ce($){return Ie($)&&!Le($)}function Te($){const f=g[$+"Team"],y=g[($==="p1"?"p2":"p1")+"Team"];return!!(Le(f)||!Ie(y)&&Ce(f))}function qe($){const f=["MIL","ATT"].some(L=>($.p1Team[L]||[]).some(z=>!z.used)),y=["MIL","ATT"].some(L=>($.p2Team[L]||[]).some(z=>!z.used)),b=Ie($.p1Team),v=Ie($.p2Team);return!f&&!(!v&&b)&&(!y&&!(!b&&v))}function Pe($){const f=$.p1Score||0,y=$.p2Score||0;return f===y?null:f>y?c.home_id:c.away_id}async function Se($){try{const f=Pe($),y=f?c.home_id===f?c.away_id:c.home_id:null;await T.from("matches").update({status:"finished",winner_id:f,home_score:$.p1Score||0,away_score:$.p2Score||0}).eq("id",i),f&&y&&updateEvolutiveCards(f,y).catch(()=>{})}catch(f){console.error("[PvP] finishMatch:",f)}}function F(){var v;const $=g[u+"Score"],f=g[h+"Score"],y=$>f,b=$===f;et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      ...
      </div>`,(R=document.getElementById("lobby-cancel"))==null||R.addEventListener("click",async()=>{_=!0,m&&(T.removeChannel(m),m=null),ie&&await T.from("friend_match_invites").update({status:"declined"}).eq("id",ie),Ve(e),x("home")})},A=async(V,oe)=>{_=!0,m&&(T.removeChannel(m),m=null),await new Promise(ie=>setTimeout(ie,600)),e.isConnected&&Yo(e,t,V,oe,r,l)},{data:g}=await T.from("friend_match_invites").select("*").eq("status","pending").or(`and(inviter_id.eq.${h},invitee_id.eq.${p}),and(inviter_id.eq.${p},invitee_id.eq.${h})`).order("created_at",{ascending:!1}).limit(1).maybeSingle();let I,q;if(g&&g.inviter_id===p)q=!1,I=g.id,await T.from("friend_match_invites").update({invitee_deck_id:i,invitee_ready:!0,status:"accepted"}).eq("id",I);else{q=!0;const{data:V,error:oe}=await T.from("friend_match_invites").insert({inviter_id:h,invitee_id:p,friend_id:p,inviter_deck_id:i,inviter_ready:!0,status:"pending"}).select().single();if(oe||!V){u("Erreur création invitation","error"),Ve(e),x("home");return}I=V.id}if(c(!0,!q,I),!q){const V={home_id:p,away_id:h,home_deck_id:g.inviter_deck_id,away_deck_id:i,status:"active",mode:"friend"},oe=await T.from("matches").insert(V).select().single(),ie=oe.data;if(!ie){u("Erreur création match: "+(((H=oe.error)==null?void 0:H.message)||""),"error"),Ve(e),x("home");return}await T.from("friend_match_invites").update({match_id:ie.id,status:"matched"}).eq("id",I),A(ie.id,!1);return}let B=!1;const C=V=>{_||B||(V.status==="matched"&&V.match_id?(B=!0,clearInterval(M),clearInterval(G),A(V.match_id,!0)):V.status==="declined"?(clearInterval(M),clearInterval(G),u(`${s} a décliné l'invitation`,"warning"),Ve(e),x("home")):V.invitee_ready&&c(!0,!0,I))},M=setInterval(()=>{if(_){clearInterval(M);return}T.from("matchmaking_queue").delete().eq("user_id",h).then(()=>{},()=>{})},3e3),G=setInterval(async()=>{if(_||B){clearInterval(G);return}const{data:V}=await T.from("friend_match_invites").select("*").eq("id",I).maybeSingle();V&&C(V)},1200);m=T.channel("friend-invite-"+I).on("postgres_changes",{event:"UPDATE",schema:"public",table:"friend_match_invites",filter:`id=eq.${I}`},V=>C(V.new)).subscribe()}async function Yo(e,t,i,o,d=[],a=[]){const{state:r,navigate:l,toast:p}=t,s=o?"p1":"p2",n=o?"p2":"p1",x=(d||[]).map(F=>F.id),u=(d||[]).map(F=>({id:F.id,gc_type:F.gc_type,_gcDef:F._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:h}=await T.from("matches").select("*").eq("id",i).single();if(!h){p("Match introuvable","error"),l("home");return}async function _(){const[{data:F},{data:$},{data:f},{data:y}]=await Promise.all([T.rpc("get_deck_for_match",{p_deck_id:h.home_deck_id}),T.rpc("get_deck_for_match",{p_deck_id:h.away_deck_id}),T.from("users").select("id,pseudo,club_name").eq("id",h.home_id).single(),T.from("users").select("id,pseudo,club_name").eq("id",h.away_id).single()]),b=E=>{const L=Number(E.evolution_bonus)||0;return{cardId:E.card_id,position:E.position,id:E.id,firstname:E.firstname,name:E.surname_encoded,country_code:E.country_code,club_id:E.club_id,job:E.job,job2:E.job2,note_g:(Number(E.note_g)||0)+(E.job==="GK"||E.job2==="GK"&&Number(E.note_g)>0?L:0),note_d:(Number(E.note_d)||0)+(E.job==="DEF"||E.job2==="DEF"&&Number(E.note_d)>0?L:0),note_m:(Number(E.note_m)||0)+(E.job==="MIL"||E.job2==="MIL"&&Number(E.note_m)>0?L:0),note_a:(Number(E.note_a)||0)+(E.job==="ATT"||E.job2==="ATT"&&Number(E.note_a)>0?L:0),evolution_bonus:L,rarity:E.rarity,skin:E.skin,hair:E.hair,hair_length:E.hair_length,clubName:E.club_encoded_name||null,clubLogo:E.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},v=((F==null?void 0:F.starters)||[]).map(E=>b(E)),w=(($==null?void 0:$.starters)||[]).map(E=>b(E)),k=(F==null?void 0:F.formation)||"4-4-2",S=($==null?void 0:$.formation)||"4-4-2";return{p1Team:(()=>{const E=vt(v,k);return stadiumDef?Tt(E,stadiumDef):E})(),p2Team:vt(w,S),p1Subs:(()=>{const E=((F==null?void 0:F.subs)||[]).map(L=>b(L));return stadiumDef&&jt(E,stadiumDef),E})(),p2Subs:(($==null?void 0:$.subs)||[]).map(E=>b(E)),p1Formation:k,p2Formation:S,p1Name:(f==null?void 0:f.club_name)||(f==null?void 0:f.pseudo)||"Joueur 1",p2Name:(y==null?void 0:y.club_name)||(y==null?void 0:y.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:o?x:[],gc_p2:o?[]:x,gcCardsFull_p1:o?u:[],gcCardsFull_p2:o?[]:u,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:a||[],lastActionAt:new Date().toISOString()}}let m=h.game_state&&Object.keys(h.game_state).length?h.game_state:null;if(!m)if(o){m=await _();const{data:F}=await T.from("matches").select("game_state").eq("id",i).single();!(F!=null&&F.game_state)||!Object.keys(F.game_state).length?await T.from("matches").update({game_state:m,turn_user_id:h.home_id}).eq("id",i):m=F.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let F=0;F<60&&!m;F++){await new Promise(f=>setTimeout(f,400));const{data:$}=await T.from("matches").select("game_state").eq("id",i).single();$!=null&&$.game_state&&Object.keys($.game_state).length&&(m=$.game_state)}if(!m){p("Erreur de synchronisation","error"),l("home");return}m.gc_p2=x,m.gcCardsFull_p2=u,await T.from("matches").update({game_state:m}).eq("id",i)}let c=!1,A=!1,g=!1,I=null,q=!1;const B=new Set,C=new Set;function M(F){var E,L;try{T.removeChannel(G)}catch{}const $=m[s+"Score"]||0,f=m[n+"Score"]||0;if(!g){g=!0;const z=F.winner_id||($>f?r.profile.id:f>$?"opp":null),j=z===r.profile.id?"win":z?"loss":null;j&&ni(()=>Promise.resolve().then(()=>on),void 0).then(D=>D.applyOwnEvolution(r.profile.id,j)).catch(()=>{})}if(!A){const z=m.p1Score||0,j=m.p2Score||0,D=(m.usedGc_p1||[]).length,Y=(m.usedGc_p2||[]).length,J=F.winner_id||(z>j?h.home_id:j>z?h.away_id:null);(J?r.profile.id===J:r.profile.id<(o?h.away_id:h.home_id))&&(A=!0,Ji({player1Id:h.home_id,player2Id:h.away_id,score1:z,score2:j,gc1:D,gc2:Y}).catch(N=>console.warn("[FriendMatch] updateStats:",N)))}let y,b;F.winner_id?(y=F.winner_id===r.profile.id,b=!1):F.forfeit?(y=$>f,b=!1):(b=$===f,y=$>f),(E=document.getElementById("pvp-end-overlay"))==null||E.remove();const v=document.createElement("div");v.id="pvp-end-overlay",v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const w=b?"🤝":y?"🏆":"😞",k=b?"MATCH NUL":y?"VICTOIRE !":"DÉFAITE",S=b?"#fff":y?"#FFD700":"#ff6b6b";v.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(v),(L=v.querySelector("#pvp-end-home"))==null||L.addEventListener("click",()=>{v.remove(),Ve(e),l("home")})}const G=T.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},F=>{const $=F.new;try{if($.status==="finished"||$.forfeit){if(c)return;c=!0,I&&(clearInterval(I),I=null),$.game_state&&(m=$.game_state),M($);return}if($.game_state){const f=m;m=$.game_state;const y=m._lastGC;if(y&&y.seq&&!C.has(y.seq)&&(C.add(y.seq),y.by!==s)){Ee(y.type,y.by,()=>re());return}const b=f[s+"Score"]||0,v=f[n+"Score"]||0,w=m[s+"Score"]||0,k=m[n+"Score"]||0,S=w>b,E=k>v;if((S||E)&&!B.has(m.round)){B.add(m.round);const L=[...m.log||[]].reverse().find(j=>j.isGoal),z=((L==null?void 0:L.homePlayers)||[]).map(j=>({name:j.name,note:j.note,portrait:j.portrait,job:j.job}));ve(z,w,k,S,()=>re());return}re()}}catch(f){console.error("[PvP] crash:",f)}}).subscribe();async function H(F){Object.assign(m,F),m.lastActionAt=new Date().toISOString();const{error:$}=await T.from("matches").update({game_state:m}).eq("id",i);$&&p("Erreur de synchronisation","error");try{re()}catch(f){console.error("[PvP] renderPvpScreen crash:",f)}}async function V(){if(c)return;c=!0,I&&(clearInterval(I),I=null);const F=o?h.away_id:h.home_id,$=o?"p2":"p1",f=o?"p1":"p2",y={...m,[$+"Score"]:3,[f+"Score"]:0,phase:"finished"};try{await T.from("matches").update({status:"finished",forfeit:!0,winner_id:F,home_score:y.p1Score||0,away_score:y.p2Score||0,game_state:y}).eq("id",i)}catch{}try{T.removeChannel(G)}catch{}setTimeout(()=>{Ve(e),l("home")},800)}const oe={BOOST_STAT:({value:F=1,count:$=1,roles:f=[]},y,b)=>{const v=y[s+"Team"],w=Object.entries(v).filter(([k])=>!f.length||f.includes(k)).flatMap(([k,S])=>S.filter(E=>!E.used).map(E=>({...E,_line:k})));if(!w.length){y.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),b(y);return}ie(w,$,`Choisir ${$} joueur(s) à booster (+${F})`,k=>{k.forEach(S=>{const E=(v[S._line]||[]).find(L=>L.cardId===S.cardId);E&&(E.boost=(E.boost||0)+F,y.log.push({text:`⚡ +${F} sur ${E.name}`,type:"info"}))}),y[s+"Team"]=v,b(y)})},DEBUFF_STAT:({value:F=1,count:$=1,roles:f=[],target:y="ai"},b,v)=>{const w=y==="home"?s:n,k=b[w+"Team"],S=y==="home"?"allié":"adverse",E=Object.entries(k).filter(([L])=>!f.length||f.includes(L)).flatMap(([L,z])=>z.map(j=>({...j,_line:L})));if(!E.length){b.log.push({text:`🎯 Aucun joueur ${S}`,type:"info"}),v(b);return}ie(E,$,`Choisir ${$} joueur(s) ${S}(s) (-${F})`,L=>{L.forEach(z=>{const j=(k[z._line]||[]).find(D=>D.cardId===z.cardId);j&&(j.boost=(j.boost||0)-F,b.log.push({text:`🎯 -${F} sur ${j.name}`,type:"info"}))}),b[w+"Team"]=k,v(b)})},GRAY_PLAYER:({count:F=1,roles:$=[],target:f="ai"},y,b)=>{const v=f==="home"?s:n,w=y[v+"Team"],k=f==="home"?"allié":"adverse",S=Object.entries(w).filter(([E])=>!$.length||$.includes(E)).flatMap(([E,L])=>L.filter(z=>!z.used).map(z=>({...z,_line:E})));if(!S.length){y.log.push({text:`❌ Aucun joueur ${k}`,type:"info"}),b(y);return}ie(S,F,`Choisir ${F} joueur(s) ${k}(s) à exclure`,E=>{const L="usedCardIds_"+v,z=new Set(y[L]||[]);E.forEach(j=>{const D=(w[j._line]||[]).find(Y=>Y.cardId===j.cardId);D&&(D.used=!0,z.add(j.cardId),y.log.push({text:`❌ ${D.name} exclu !`,type:"info"}))}),y[L]=[...z],y[v+"Team"]=w,b(y)})},REVIVE_PLAYER:({count:F=1,roles:$=[]},f,y)=>{const b=f[s+"Team"],v=Object.entries(b).filter(([w])=>!$.length||$.includes(w)).flatMap(([w,k])=>k.filter(S=>S.used).map(S=>({...S,_line:w})));if(!v.length){f.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),y(f);return}ie(v,F,`Choisir ${F} joueur(s) à ressusciter`,w=>{w.forEach(k=>{const S=(b[k._line]||[]).find(E=>E.cardId===k.cardId);S&&(S.used=!1,f.log.push({text:`💫 ${S.name} ressuscité !`,type:"info"}))}),f[s+"Team"]=b,y(f)})},REMOVE_GOAL:({},F,$)=>{const f=n+"Score";F[f]>0?(F[f]--,F.log.push({text:"🚫 Dernier but annulé !",type:"info"})):F.log.push({text:"🚫 Aucun but à annuler",type:"info"}),$(F)},ADD_GOAL_DRAW:({},F,$)=>{F[s+"Score"]===F[n+"Score"]?(F[s+"Score"]++,F.log.push({text:"🎯 But bonus !",type:"info"})):F.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),$(F)},ADD_SUB:({value:F=1},$,f)=>{$.maxSubs=($.maxSubs||3)+F,$.log.push({text:`🔄 +${F} remplacement(s)`,type:"info"}),f($)},CUSTOM:({},F,$)=>$(F)};function ie(F,$,f,y){const b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let v=[];function w(){var S,E;const k=F.map(L=>{const z={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[L._line]||"#555",j=ze(L,L._line)+(L.boost||0),Y=v.find(K=>K.cardId===L.cardId)?"#FFD700":"rgba(255,255,255,0.25)",J=L.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${L.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${Y};background:${z};overflow:hidden;cursor:pointer;${J}">
        ...
        </div>`,(S=b.querySelector("#pp-close"))==null||S.addEventListener("click",()=>b.remove()),b.querySelectorAll(".pp-item").forEach(L=>{L.addEventListener("click",()=>{const z=L.dataset.cid,j=F.find(Y=>Y.cardId===z),D=v.findIndex(Y=>Y.cardId===z);j&&(D>-1?v.splice(D,1):v.length<$&&v.push(j),w())})}),(E=b.querySelector("#pp-confirm"))==null||E.addEventListener("click",()=>{b.remove(),y(v)})}w(),document.body.appendChild(b)}async function Q(F,$){const y=(m["gcCardsFull_"+s]||[]).find(k=>k.id===F),b=(y==null?void 0:y._gcDef)||(m.gcDefs||[]).find(k=>{var S;return k.name===$||((S=k.name)==null?void 0:S.toLowerCase().trim())===($==null?void 0:$.toLowerCase().trim())}),v=[...m["usedGc_"+s]||[],F],w={type:$,by:s,seq:(m._gcAnimSeq||0)+1};C.add(w.seq),Ee($,s,async()=>{if(b!=null&&b.effect_type&&b.effect_type!=="CUSTOM"){const S=oe[b.effect_type];if(S){const E={...m};S(b.effect_params||{},E,async L=>{L["usedGc_"+s]=v,L._lastGC=w,L._gcAnimSeq=w.seq,await H(L)});return}}const k={...m};switch($){case"Remplacement+":k.maxSubs=(k.maxSubs||3)+1,k.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const S=n+"Score";k[S]>0&&(k[S]--,k.log.push({text:"🚫 But annulé",type:"info"}));break}}k["usedGc_"+s]=v,k._lastGC=w,k._gcAnimSeq=w.seq,await H(k)})}function R(F,$){const f="usedCardIds_"+F,y=new Set(m[f]||[]);$.forEach(b=>y.add(b)),m[f]=[...y]}function P(){for(const F of["p1","p2"]){const $=new Set(m["usedCardIds_"+F]||[]),f=m[F+"Team"];if(f)for(const y of["GK","DEF","MIL","ATT"])(f[y]||[]).forEach(b=>{b.used=$.has(b.cardId)})}}function re(){var Ge,We,nt,ft,gt,mt;if(m.phase==="reveal")return xe();if(m.phase==="midfield")return we();if(m.phase==="finished")return Se();const F=m[s+"Team"],$=m[n+"Team"];P();const f=m[s+"Score"],y=m[n+"Score"],b=m[s+"Name"],v=m[n+"Name"],w=m.phase===s+"-attack",k=m.phase===s+"-defense",S=Array.isArray(m["selected_"+s])?m["selected_"+s]:[],E=S.map(le=>le.cardId),L=window.innerWidth>=700,z=m[s+"Subs"]||[],j=m["usedSubIds_"+s]||[],D=z.filter(le=>!j.includes(le.cardId)),Y=m["gcCardsFull_"+s]||[],J=m["usedGc_"+s]||[],K=Y.filter(le=>!J.includes(le.id)),N=m.boostOwner===s&&!m.boostUsed,W=!["GK","DEF","MIL","ATT"].some(le=>($[le]||[]).some(X=>!X.used)),te=[...F.MIL||[],...F.ATT||[]].filter(le=>!le.used),ee=w&&W&&te.length===0?[...F.GK||[],...F.DEF||[]].filter(le=>!le.used).map(le=>le.cardId):[];function fe(le,X,O){var yt,kt;const ae=le._gcDef,he={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ae==null?void 0:ae.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",$e={purple:"#b06ce0",light_blue:"#00d4ef"}[ae==null?void 0:ae.color]||"#b06ce0",Qe=(ae==null?void 0:ae.name)||le.gc_type,Ze=(ae==null?void 0:ae.effect)||((yt=Oe[le.gc_type])==null?void 0:yt.desc)||"",tt=ae!=null&&ae.image_url?`/icons/${ae.image_url}`:null,rt=((kt=Oe[le.gc_type])==null?void 0:kt.icon)||"⚡",_t=Math.round(O*.22),$t=Math.round(O*.22),lt=O-_t-$t,xt=Qe.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${le.id}" data-gc-type="${le.gc_type}"
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${ut(F,m[s+"Formation"],Ae,E,300,300,ee)}
      </div>
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="width:min(90vw,420px)">${pt(m[n+"Team"],m[n+"Formation"],null,[],300,300)}</div>
        ...
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${K.map((te,ee)=>{const fe=ee<K.length-1?ot(te,K[ee+1]):null,ye=!fe||fe==="#ff3333"||fe==="#cc2222",be=fe==="#00ff88"?"+2":fe==="#FFD700"?"+1":"";return`<div class="duel-card duel-card-${W}" data-idx="${ee}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease,transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
              ${Xe({...te,note:ze(te,"MIL"),_line:"MIL"},58,78)}
            </div>
            ${ee<K.length-1?`<div class="duel-link duel-link-${W}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${ye?"rgba(255,255,255,0.12)":fe};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${ye?"none":`0 0 8px ${fe}`}">
        ...
        <div class="duel-score-line duel-score-line-${W}" style="margin-top:10px;font-size:12px;color:rgba(255,255,255,0.55);opacity:0;transition:opacity .4s ease">
          Score: ${f(K)} + ${y(K)} liens = <b style="color:#fff">${f(K)+y(K)}</b>
        </div>
      </div>`}et(e),e.innerHTML=`
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      <style>
      ...
      <div id="duel-shock" style="position:absolute;left:50%;top:50%;width:120px;height:120px;border-radius:50%;border:6px solid #FFD700;opacity:0;pointer-events:none"></div>
      <div id="pvp-duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center,rgba(10,61,30,.4),rgba(10,61,30,.92))"></div>
    </div>`;const S=(K,N)=>e.querySelectorAll(K).forEach((W,te)=>{setTimeout(()=>{W.style.opacity="1",W.style.transform="translateY(0) scale(1)"},N+te*90)});S(".duel-card-me",150),S(".duel-card-opp",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((K,N)=>setTimeout(()=>{K.style.opacity="1"},N*70)),900),setTimeout(()=>{const K=e.querySelector("#pvp-vs");K&&(K.style.opacity="1",K.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(N=>N.style.opacity="1")},1250);function E(K,N,W){const te=document.getElementById(K);if(!te)return;const ee=performance.now(),fe=ye=>{const be=Math.min(1,(ye-ee)/W);te.textContent=Math.round(N*(1-Math.pow(1-be,3))),be<1?requestAnimationFrame(fe):te.textContent=N};requestAnimationFrame(fe)}setTimeout(()=>{E("pvp-score-me",b,800),E("pvp-score-opp",v,800)},1500);const L=m.p1Team.MIL||[],z=m.p2Team.MIL||[],j=f(L)+y(L),D=f(z)+y(z),Y=j>=D?"p1":"p2";let J=m.boostValue;J==null&&(J=oi(),m.boostValue=J,m.boostOwner=Y,m.boostUsed=!1),_e=!0,setTimeout(()=>{const K=e.querySelector("#duel-row-"+(w?"me":"opp")),N=e.querySelector("#duel-row-"+(w?"opp":"me")),W=document.getElementById("pvp-score-me"),te=document.getElementById("pvp-score-opp"),ee=w?W:te,fe=w?te:W;ee&&(ee.style.fontSize="80px",ee.style.color=w?"#FFD700":"#ff6b6b",ee.style.animation="duelPulse .5s ease"+(w?",duelGlow 1.5s ease infinite .5s":"")),fe&&(fe.style.opacity="0.25"),setTimeout(()=>{K&&(K.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",K.style.zIndex="5"),setTimeout(()=>{const ye=document.getElementById("duel-shock");ye&&(ye.style.animation="shockwave .5s ease-out forwards"),N&&(N.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var Be;const ye=document.getElementById("pvp-duel-finale");if(!ye)return;const be=m.boostOwner===s?'<div style="background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:3px solid #cdeffd;border-radius:18px;padding:20px 34px;text-align:center;animation:boostFlipIn .7s cubic-bezier(.34,1.56,.64,1) both;box-shadow:0 10px 36px rgba(135,206,235,.5)"><div style="font-size:10px;color:rgba(0,0,0,.6);letter-spacing:2px;text-transform:uppercase;margin-bottom:6px;font-weight:700">Carte Boost obtenue</div><div style="font-size:46px;line-height:1">⚡</div><div style="font-size:50px;font-weight:900;color:#063;line-height:1.1">+'+J+`</div><div style="font-size:10px;color:rgba(0,0,0,.55);margin-top:4px">Applicable sur n'importe quel joueur</div></div>`:"",ke=s==="p1"?'<button id="pvp-start-match" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">▶ Commencer le match</button>':`<div style="font-size:14px;color:rgba(255,255,255,0.5);text-align:center;margin-top:8px;animation:fadeUp .4s ease both">⏳ En attente de l'adversaire...</div>`;ye.innerHTML='<div style="font-size:22px;font-weight:900;color:#fff;text-align:center;animation:fadeUp .4s ease both;text-shadow:0 2px 12px rgba(0,0,0,.5)">'+(w?"⚽ "+m[s+"Name"]+"<br>gagne le milieu et attaque !":"😔 "+m[n+"Name"]+"<br>gagne l'engagement et attaque !")+"</div>"+be+ke,ye.style.transition="opacity .45s ease",ye.style.opacity="1",ye.style.pointerEvents="auto",(Be=document.getElementById("pvp-start-match"))==null||Be.addEventListener("click",async()=>{const Fe=Y;await H({phase:Fe+"-attack",attacker:Fe,round:1,boostValue:J,boostUsed:!1,boostOwner:Fe})})},600)},700)},2800)}function ve(F,$,f,y,b){const v=document.createElement("div");v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:900;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;overflow:hidden;cursor:pointer";const w=Array.from({length:10},(L,z)=>`<div style="position:absolute;font-size:${16+Math.floor(Math.random()*24)}px;top:${5+Math.floor(Math.random()*65)}%;left:${3+Math.floor(Math.random()*94)}%;animation:fw${z%2===0?"A":"B"} ${.7+Math.random()*.7}s ease ${Math.random()*.9}s both;opacity:0">${["✨","🌟","⭐","💥","🎇","🎆","🔥","🌈"][z%8]}</div>`).join(""),k={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};v.innerHTML=`
      <style>
        @keyframes butPop{0%{transform:scale(0) rotate(-8deg);opacity:0}55%{transform:scale(1.25) rotate(2deg)}85%{transform:scale(0.92) rotate(-1deg)}100%{transform:scale(1);opacity:1}}
        @keyframes ballIn{0%{transform:translate(-70px,18px);opacity:0}65%{opacity:1}100%{transform:translate(26px,-8px);opacity:1}}
        @keyframes scoreIn{from{opacity:0;transform:translateY(-12px)}to{opacity:1;transform:translateY(0)}}
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(v);let S=!1;const E=()=>{S||(S=!0,v.remove(),setTimeout(()=>b(),50))};v.addEventListener("click",E),setTimeout(E,3500)}function Ee(F,$,f){var J,K;const y=(m.gcDefs||[]).find(N=>{var W;return N.name===F||((W=N.name)==null?void 0:W.toLowerCase().trim())===(F==null?void 0:F.toLowerCase().trim())}),b={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[y==null?void 0:y.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",v={purple:"#b06ce0",light_blue:"#00d4ef"}[y==null?void 0:y.color]||"#b06ce0",w=(y==null?void 0:y.name)||F,k=(y==null?void 0:y.effect)||((J=Oe[F])==null?void 0:J.desc)||"",S=y!=null&&y.image_url?`/icons/${y.image_url}`:null,E=((K=Oe[F])==null?void 0:K.icon)||"⚡",z=$===s?"Vous":m[$+"Name"]||"Adversaire",j=document.createElement("div");j.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",j.innerHTML=`
      <style>
        @keyframes gcFlipIn{0%{transform:perspective(800px) rotateY(90deg) scale(.7);opacity:0}55%{transform:perspective(800px) rotateY(-12deg) scale(1.08);opacity:1}100%{transform:perspective(800px) rotateY(0) scale(1);opacity:1}}
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(j);let D=!1;const Y=()=>{D||(D=!0,j.remove(),setTimeout(()=>f&&f(),50))};j.addEventListener("click",Y),setTimeout(Y,3e3)}function se(F,$){var j,D,Y,J;const y=(m["gcCardsFull_"+s]||[]).find(K=>K.id===F),b=y==null?void 0:y._gcDef,v={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[b==null?void 0:b.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",w={purple:"#b06ce0",light_blue:"#00d4ef"}[b==null?void 0:b.color]||"#b06ce0",k=(b==null?void 0:b.name)||$,S=(b==null?void 0:b.effect)||((j=Oe[$])==null?void 0:j.desc)||"Carte spéciale.",E=b!=null&&b.image_url?`/icons/${b.image_url}`:null,L=((D=Oe[$])==null?void 0:D.icon)||"⚡",z=document.createElement("div");z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",z.innerHTML=`
      ...
      <div style="display:flex;gap:12px;width:190px">
        <button id="pvp-gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
        <button id="pvp-gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
      </div>`,document.body.appendChild(z),(Y=z.querySelector("#pvp-gc-back"))==null||Y.addEventListener("click",()=>z.remove()),(J=z.querySelector("#pvp-gc-use"))==null||J.addEventListener("click",()=>{z.remove(),Q(F,$)})}function pe(){var y;const F=m[s+"Team"],$=Object.entries(F).flatMap(([b,v])=>(v||[]).filter(w=>!w.used).map(w=>({...w,_line:b})));if(!$.length)return;const f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",f.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
      ...
      </div>`,document.body.appendChild(f),(y=f.querySelector("#bp-close"))==null||y.addEventListener("click",()=>f.remove()),f.querySelectorAll(".bp-item").forEach(b=>{b.addEventListener("click",async()=>{const v=b.dataset.cid,w=$.find(S=>S.cardId===v);if(!w)return;const k=(F[w._line]||[]).find(S=>S.cardId===v);k&&(k.boost=(k.boost||0)+m.boostValue),f.remove(),await H({[s+"Team"]:F,boostUsed:!0})})})}function ce(F=null){var D,Y;if(!(m.phase===s+"-attack")){p("Remplacement uniquement avant votre attaque","warning");return}const f=m[s+"Team"],y=m["usedSubIds_"+s]||[],b=m.maxSubs||3;if(y.length>=b){p(`Maximum ${b} remplacements atteint`,"warning");return}const v=Object.entries(f).flatMap(([J,K])=>(K||[]).filter(N=>N.used).map(N=>({...N,_line:J}))),w=(m[s+"Subs"]||[]).filter(J=>!y.includes(J.cardId));if(!v.length){p("Aucun joueur utilisé à remplacer","warning");return}if(!w.length){p("Aucun remplaçant disponible","warning");return}let k=Math.max(0,v.findIndex(J=>J.cardId===F));const S=((D=v[k])==null?void 0:D._line)||((Y=v[k])==null?void 0:Y.job);let E=Math.max(0,w.findIndex(J=>J.job===S)),L=!1;const z=document.createElement("div");z.id="pvp-sub-overlay",z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function j(){var fe,ye,be,ke,Be,Fe;const J=v[k],K=w[E],N=Math.min(130,Math.round((window.innerWidth-90)/2)),W=Math.round(N*1.35),te=Me=>`background:rgba(255,255,255,0.12);border:none;color:${Me?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Me?"default":"pointer"};flex-shrink:0`;z.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        <div id="pin-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
          ...
          <div>${K?Xe({...K,used:!1,boost:0},N,W):"<div>—</div>"}</div>
      ...
      <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
        <button id="psub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
      </div>`,(fe=z.querySelector("#psub-close"))==null||fe.addEventListener("click",()=>z.remove()),(ye=z.querySelector("#pout-up"))==null||ye.addEventListener("click",()=>{k>0&&(k--,j())}),(be=z.querySelector("#pout-down"))==null||be.addEventListener("click",()=>{k<v.length-1&&(k++,j())}),(ke=z.querySelector("#pin-up"))==null||ke.addEventListener("click",()=>{E>0&&(E--,j())}),(Be=z.querySelector("#pin-down"))==null||Be.addEventListener("click",()=>{E<w.length-1&&(E++,j())});const ee=(Me,je,Ae,Ne)=>{const Ue=z.querySelector("#"+Me);if(!Ue)return;let Ye=0;Ue.addEventListener("touchstart",Ke=>{Ye=Ke.touches[0].clientY},{passive:!0}),Ue.addEventListener("touchend",Ke=>{const Re=Ke.changedTouches[0].clientY-Ye;if(Math.abs(Re)<30)return;const Ge=je();Re<0&&Ge<Ne-1?(Ae(Ge+1),j()):Re>0&&Ge>0&&(Ae(Ge-1),j())},{passive:!0})};ee("pin-panel",()=>E,Me=>E=Me,w.length),ee("pout-panel",()=>k,Me=>k=Me,v.length),(Fe=z.querySelector("#psub-confirm"))==null||Fe.addEventListener("click",async Me=>{if(Me.preventDefault(),Me.stopPropagation(),L)return;L=!0;const je=v[k],Ae=w[E];if(!je||!Ae)return;const Ne=je._line,Ue=(f[Ne]||[]).findIndex(Re=>Re.cardId===je.cardId);if(Ue===-1){p("Erreur : joueur introuvable","error"),z.remove();return}const Ye={...Ae,_line:Ne,position:je.position,used:!1,boost:0};f[Ne].splice(Ue,1,Ye);const Ke=[...y,Ae.cardId];z.remove(),U(je,Ae,async()=>{await H({[s+"Team"]:f,[n+"Team"]:m[n+"Team"],["usedSubIds_"+s]:Ke})})})}document.body.appendChild(z),j()}function U(F,$,f){const y={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const v=(S,E,L)=>`<div style="text-align:center">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:6px">Appuyer pour continuer</div>`,document.body.appendChild(b);let w=!1;const k=()=>{w||(w=!0,b.remove(),setTimeout(()=>f(),50))};b.addEventListener("click",k),setTimeout(k,2200)}function Z(){var $;const F=document.createElement("div");F.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:20px;overflow-y:auto",F.innerHTML=`
      <div style="font-size:12px;color:rgba(255,255,255,0.5);letter-spacing:2px;text-transform:uppercase">Équipe adverse</div>
      <div style="font-size:18px;font-weight:900;color:#ff6b6b">${m[n+"Name"]}</div>
      <div style="width:min(90vw,420px)">${pt(m[n+"Team"],m[n+"Formation"],null,[],300,300)}</div>
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(F),($=F.querySelector("#pvp-opp-close"))==null||$.addEventListener("click",()=>F.remove())}function ge(){var y;const F=m.log||[],$=document.createElement("div");$.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column";const f=b=>{var w,k,S;if(b.type==="duel"){const E=b.isGoal,L=b.homeScored&&s==="p1"||!b.homeScored&&E&&s==="p2",z=b.homeScored?"#FFD700":E?"#ff6b6b":"rgba(255,255,255,0.3)",j=E?L?"⚽ BUT !":"⚽ BUT adversaire !":(w=b.homePlayers)!=null&&w.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${E?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${z};margin-bottom:4px">
          <div style="font-size:9px;color:${z};letter-spacing:1px;margin-bottom:5px;font-weight:700;text-transform:uppercase">${j}</div>
      ...
      <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
        ${F.length===0?'<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action</div>':[...F].reverse().map(f).join("")}
      </div>`,document.body.appendChild($),(y=$.querySelector("#pvp-hist-close"))==null||y.addEventListener("click",()=>$.remove())}async function de(){if(m.phase!==s+"-attack")return;const F=s==="p1"?"p2":"p1",$=(m.round||0)+1,f=[...m.log||[]];f.push({type:"info",text:`⏭️ ${m[s+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const y=Te(m),b=Ce(F),v=y||!b?"finished":F+"-attack";await H({["selected_"+s]:[],modifiers:{...m.modifiers,[s]:{}},pendingAttack:null,phase:v,attacker:F,round:$,log:f}),v==="finished"&&await Pe(m)}async function me(){const F=m[s+"Team"],$=!["GK","DEF","MIL","ATT"].some(v=>(m[n+"Team"][v]||[]).some(w=>!w.used)),f=(m["selected_"+s]||[]).map(v=>{const w=(F[v._role]||[]).find(j=>j.cardId===v.cardId)||v,k=$&&["GK","DEF"].includes(v._role),S=F[v._role]||[],E=S.findIndex(j=>j.cardId===v.cardId),L=dt(S.length),z=E>=0?L[E]:w._col??1;return{...w,_line:v._role,_col:z,...k?{note_a:Math.max(1,Number(w.note_a)||0)}:{}}});if(!f.length)return;const y=Ut(f,m.modifiers[s]||{});R(s,f.map(v=>v.cardId)),f.forEach(v=>{const w=(F[v._role]||[]).find(k=>k.cardId===v.cardId);w&&(w.used=!0)}),m["selected_"+s]=[],re();const b=[...m.log||[]];if($){const v=(m[s+"Score"]||0)+1,w=f.map(z=>({name:z.name,note:ze(z,z._line||"ATT"),portrait:He(z),job:z.job}));b.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:w,homeTotal:y.total,aiTotal:0});const k=(m.round||0)+1,S=s==="p1"?"p2":"p1",E={...m,[s+"Team"]:F,[s+"Score"]:v},L=Te(E);B.add(k),ve(w,v,m[n+"Score"]||0,!0,async()=>{await H({[s+"Team"]:F,[s+"Score"]:v,["selected_"+s]:[],modifiers:{...m.modifiers,[s]:{}},pendingAttack:null,phase:L?"finished":S+"-attack",attacker:S,round:k,log:b}),L&&await Pe({...m,[s+"Score"]:v})});return}b.push({type:"pending",text:`⚔️ ${m[s+"Name"]} attaque (${y.total})`}),await H({[s+"Team"]:F,[n+"Team"]:m[n+"Team"],pendingAttack:{...y,players:f,side:s},["selected_"+s]:[],modifiers:{...m.modifiers,[s]:{}},phase:n+"-defense",log:b})}async function ne(){const F=m[s+"Team"],$=(m["selected_"+s]||[]).map(J=>{const K=(F[J._role]||[]).find(fe=>fe.cardId===J.cardId)||J,N=F[J._role]||[],W=N.findIndex(fe=>fe.cardId===J.cardId),te=dt(N.length),ee=W>=0?te[W]:K._col??1;return{...K,_line:J._role,_col:ee}}),f=Kt($,m.modifiers[s]||{});R(s,$.map(J=>J.cardId)),$.forEach(J=>{const K=(F[J._role]||[]).find(N=>N.cardId===J.cardId);K&&(K.used=!0)}),m["selected_"+s]=[],re();const y=Vt(m.pendingAttack.total,f.total,m.modifiers[s]||{}),b=m.pendingAttack.side,v=y==="attack"||(y==null?void 0:y.goal),w=b==="p1"?"p2":"p1",k=(m.round||0)+1,S=(m.pendingAttack.players||[]).map(J=>({name:J.name,note:ze(J,J._line||"ATT"),portrait:He(J),job:J.job})),E=[...m.log||[]];E.push({type:"duel",isGoal:v,homeScored:v&&b===s,text:v?`⚽ BUT de ${m[b+"Name"]} ! (${m.pendingAttack.total} vs ${f.total})`:`✋ Attaque stoppée (${m.pendingAttack.total} vs ${f.total})`,homePlayers:S,aiPlayers:$.map(J=>({name:J.name,note:ze(J,J._line||"DEF"),portrait:He(J),job:J.job})),homeTotal:m.pendingAttack.total,aiTotal:f.total});const L=v?(m[b+"Score"]||0)+1:m[b+"Score"]||0,z={...m,[s+"Team"]:F,[b+"Score"]:L},j=Te(z),D=j?"finished":w+"-attack",Y=async()=>{await H({[s+"Team"]:F,[n+"Team"]:m[n+"Team"],[b+"Score"]:L,["selected_"+s]:[],modifiers:{...m.modifiers,[s]:{}},pendingAttack:null,phase:D,attacker:w,round:k,log:E}),(D==="finished"||j)&&await Pe({...m,[b+"Score"]:L})};if(v){const J=b===s,K=J?L:m[s+"Score"]||0,N=J?m[n+"Score"]||0:L;B.add(k),ve(S,K,N,J,Y)}else await Y()}function ue(F){return["MIL","ATT"].some($=>(F[$]||[]).some(f=>!f.used))}function Le(F){return["GK","DEF","MIL","ATT"].some($=>(F[$]||[]).some(f=>!f.used))}function Ie(F){return Le(F)&&!ue(F)}function Ce(F){const $=m[F+"Team"],f=m[(F==="p1"?"p2":"p1")+"Team"];return!!(ue($)||!Le(f)&&Ie($))}function Te(F){const $=["MIL","ATT"].some(E=>(F.p1Team[E]||[]).some(L=>!L.used)),f=["MIL","ATT"].some(E=>(F.p2Team[E]||[]).some(L=>!L.used)),y=Le(F.p1Team),b=Le(F.p2Team);return!$&&!(!b&&y)&&(!f&&!(!y&&b))}function qe(F){const $=F.p1Score||0,f=F.p2Score||0;return $===f?null:$>f?h.home_id:h.away_id}async function Pe(F){try{const $=qe(F),f=$?h.home_id===$?h.away_id:h.home_id:null;await T.from("matches").update({status:"finished",winner_id:$,home_score:F.p1Score||0,away_score:F.p2Score||0}).eq("id",i),$&&f&&Ti($,f).catch(()=>{})}catch($){console.error("[PvP] finishMatch:",$)}}function Se(){var b;const F=m[s+"Score"],$=m[n+"Score"],f=F>$,y=F===$;et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
  ...
  </div>`,(A=document.getElementById("ml-create-btn"))==null||A.addEventListener("click",()=>Zo(e,t)),e.querySelectorAll(".ml-tab").forEach(g=>g.addEventListener("click",()=>St(e,t,g.dataset.tab))),e.querySelectorAll("[data-league-id]").forEach(g=>g.addEventListener("click",()=>wt(e,t,g.dataset.leagueId))),e.querySelectorAll("[data-join]").forEach(g=>g.addEventListener("click",I=>{I.stopPropagation(),gn(e,t,g.dataset.join,g.dataset.type)})),e.querySelectorAll("[data-delete]").forEach(g=>g.addEventListener("click",I=>{I.stopPropagation(),vi(e,t,g.dataset.delete,g.dataset.name,i)}))}function ii(e,t,i=!1){var r,l;const o=e.creator_id===t,d=e.pot||0,a=((l=(r=e.mini_league_members)==null?void 0:r[0])==null?void 0:l.count)||0;return`<div data-league-id="${e.id}" style="background:#fff;border-radius:12px;padding:14px 16px;box-shadow:0 1px 6px rgba(0,0,0,0.08);cursor:pointer;margin-bottom:8px">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="background:linear-gradient(160deg,#1a0a2e,#3b1a6e);border-radius:20px;padding:32px 24px;max-width:320px;width:100%;text-align:center;color:#fff;box-shadow:0 8px 40px rgba(0,0,0,0.6)">
      ...
      <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:20px;background:linear-gradient(180deg,#0a1628,#1a0a2e);padding:24px;text-align:center;color:#fff">
        ...
        <button id="lobby-cancel" style="padding:10px 24px;border-radius:10px;border:1.5px solid rgba(255,255,255,0.2);background:transparent;color:rgba(255,255,255,0.6);font-size:13px;cursor:pointer">Annuler</button>
        <style>@keyframes lobbyPulse{0%,100%{opacity:1}50%{opacity:0.4}}</style>
      </div>`,(G=document.getElementById("lobby-cancel"))==null||G.addEventListener("click",()=>{_=!0,clearInterval(m),Ve(e),x("mini-league")})},B=async(C,M)=>{_=!0,clearInterval(m),await new Promise(G=>setTimeout(G,600)),e.isConnected&&dr(e,t,C,M,r,l)};if(q(),A){let C=c.match_id;if(!C){const{data:G,error:H}=await T.from("matches").insert({home_id:h,away_id:I,home_deck_id:i,away_deck_id:null,status:"active",mode:"mini_league"}).select().single();if(H||!G){u("Erreur création match","error"),Ve(e),x("mini-league");return}C=G.id,await T.from("mini_league_matches").update({match_id:C,status:"playing"}).eq("id",p)}const M=C;m=setInterval(async()=>{if(_){clearInterval(m);return}const{data:G}=await T.from("matches").select("away_deck_id").eq("id",M).single();G!=null&&G.away_deck_id&&(clearInterval(m),B(M,!0))},1500)}else{let C=c.match_id;m=setInterval(async()=>{if(_){clearInterval(m);return}const{data:M}=await T.from("mini_league_matches").select("match_id").eq("id",p).single();M!=null&&M.match_id&&(clearInterval(m),C=M.match_id,await T.from("matches").update({away_deck_id:i}).eq("id",C),B(C,!1))},1500)}}async function dr(e,t,i,o,d=[],a=[]){var $;const{state:r,navigate:l,toast:p}=t,s=(($=r.params)==null?void 0:$.leagueId)||null,n=o?"p1":"p2",x=o?"p2":"p1",u=(d||[]).map(f=>f.id),h=(d||[]).map(f=>({id:f.id,gc_type:f.gc_type,_gcDef:f._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:_}=await T.from("matches").select("*").eq("id",i).single();if(!_){p("Match introuvable","error"),l("mini-league");return}async function m(){const[{data:f},{data:y},{data:b},{data:v}]=await Promise.all([T.rpc("get_deck_for_match",{p_deck_id:_.home_deck_id}),T.rpc("get_deck_for_match",{p_deck_id:_.away_deck_id}),T.from("users").select("id,pseudo,club_name").eq("id",_.home_id).single(),T.from("users").select("id,pseudo,club_name").eq("id",_.away_id).single()]),w=z=>{const j=Number(z.evolution_bonus)||0;return{cardId:z.card_id,position:z.position,id:z.id,firstname:z.firstname,name:z.surname_encoded,country_code:z.country_code,club_id:z.club_id,job:z.job,job2:z.job2,note_g:(Number(z.note_g)||0)+(z.job==="GK"||z.job2==="GK"&&Number(z.note_g)>0?j:0),note_d:(Number(z.note_d)||0)+(z.job==="DEF"||z.job2==="DEF"&&Number(z.note_d)>0?j:0),note_m:(Number(z.note_m)||0)+(z.job==="MIL"||z.job2==="MIL"&&Number(z.note_m)>0?j:0),note_a:(Number(z.note_a)||0)+(z.job==="ATT"||z.job2==="ATT"&&Number(z.note_a)>0?j:0),evolution_bonus:j,rarity:z.rarity,skin:z.skin,hair:z.hair,hair_length:z.hair_length,clubName:z.club_encoded_name||null,clubLogo:z.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},k=((f==null?void 0:f.starters)||[]).map(z=>w(z)),S=((y==null?void 0:y.starters)||[]).map(z=>w(z)),E=(f==null?void 0:f.formation)||"4-4-2",L=(y==null?void 0:y.formation)||"4-4-2";return{p1Team:(()=>{const z=vt(k,E);return stadiumDef?Tt(z,stadiumDef):z})(),p2Team:vt(S,L),p1Subs:(()=>{const z=((f==null?void 0:f.subs)||[]).map(j=>w(j));return stadiumDef&&jt(z,stadiumDef),z})(),p2Subs:((y==null?void 0:y.subs)||[]).map(z=>w(z)),p1Formation:E,p2Formation:L,p1Name:(b==null?void 0:b.club_name)||(b==null?void 0:b.pseudo)||"Joueur 1",p2Name:(v==null?void 0:v.club_name)||(v==null?void 0:v.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:o?u:[],gc_p2:o?[]:u,gcCardsFull_p1:o?h:[],gcCardsFull_p2:o?[]:h,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:a||[],lastActionAt:new Date().toISOString()}}let c=_.game_state&&Object.keys(_.game_state).length?_.game_state:null;if(!c)if(o){c=await m();const{data:f}=await T.from("matches").select("game_state").eq("id",i).single();!(f!=null&&f.game_state)||!Object.keys(f.game_state).length?await T.from("matches").update({game_state:c,turn_user_id:_.home_id}).eq("id",i):c=f.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let f=0;f<60&&!c;f++){await new Promise(b=>setTimeout(b,400));const{data:y}=await T.from("matches").select("game_state").eq("id",i).single();y!=null&&y.game_state&&Object.keys(y.game_state).length&&(c=y.game_state)}if(!c){p("Erreur de synchronisation","error"),l("mini-league");return}c.gc_p2=u,c.gcCardsFull_p2=h,await T.from("matches").update({game_state:c}).eq("id",i)}let A=!1,g=!1,I=!1,q=null,B=!1;const C=new Set,M=new Set;function G(f){var z,j;try{T.removeChannel(H)}catch{}const y=c[n+"Score"]||0,b=c[x+"Score"]||0;if(!I){I=!0;const D=f.winner_id||(y>b?r.profile.id:b>y?"opp":null),Y=D===r.profile.id?"win":D?"loss":null;Y&&ni(()=>Promise.resolve().then(()=>on),void 0).then(J=>J.applyOwnEvolution(r.profile.id,Y)).catch(()=>{})}if(!g){const D=c.p1Score||0,Y=c.p2Score||0,J=(c.usedGc_p1||[]).length,K=(c.usedGc_p2||[]).length,N=f.winner_id||(D>Y?_.home_id:Y>D?_.away_id:null);(N?r.profile.id===N:r.profile.id<(o?_.away_id:_.home_id))&&(g=!0,Ji({player1Id:_.home_id,player2Id:_.away_id,score1:D,score2:Y,gc1:J,gc2:K}).catch(te=>console.warn("[FriendMatch] updateStats:",te)))}let v,w;f.winner_id?(v=f.winner_id===r.profile.id,w=!1):f.forfeit?(v=y>b,w=!1):(w=y===b,v=y>b),(z=document.getElementById("pvp-end-overlay"))==null||z.remove();const k=document.createElement("div");k.id="pvp-end-overlay",k.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const S=w?"🤝":v?"🏆":"😞",E=w?"MATCH NUL":v?"VICTOIRE !":"DÉFAITE",L=w?"#fff":v?"#FFD700":"#ff6b6b";k.innerHTML=`
      <div style="font-size:64px">${S}</div>
      <div style="font-size:26px;font-weight:900;color:${L}">${E}</div>
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">🏆 Retour à la Mini League</button>`,document.body.appendChild(k),(j=k.querySelector("#pvp-end-home"))==null||j.addEventListener("click",()=>{k.remove(),Ve(e),s?l("mini-league",{openLeagueId:s}):l("mini-league")})}const H=T.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},f=>{const y=f.new;try{if(y.status==="finished"||y.forfeit){if(A||(A=!0,q&&(clearInterval(q),q=null),y.game_state&&(c=y.game_state),c.phase==="finished"&&!y.forfeit&&document.getElementById("pvp-home")))return;G(y);return}if(y.game_state){const b=c;c=y.game_state;const v=c._lastGC;if(v&&v.seq&&!M.has(v.seq)&&(M.add(v.seq),v.by!==n)){se(v.type,v.by,()=>xe());return}const w=b[n+"Score"]||0,k=b[x+"Score"]||0,S=c[n+"Score"]||0,E=c[x+"Score"]||0,L=S>w,z=E>k;if((L||z)&&!C.has(c.round)){C.add(c.round);const j=[...c.log||[]].reverse().find(Y=>Y.isGoal),D=((j==null?void 0:j.homePlayers)||[]).map(Y=>({name:Y.name,note:Y.note,portrait:Y.portrait,job:Y.job}));Ee(D,S,E,L,()=>xe());return}xe()}}catch(b){console.error("[PvP] crash:",b)}}).subscribe();async function V(f){Object.assign(c,f),c.lastActionAt=new Date().toISOString();const{error:y}=await T.from("matches").update({game_state:c}).eq("id",i);y&&p("Erreur de synchronisation","error");try{xe()}catch(b){console.error("[PvP] renderPvpScreen crash:",b)}}async function oe(){if(A)return;A=!0,q&&(clearInterval(q),q=null);const f=o?_.away_id:_.home_id,y=o?"p2":"p1",b=o?"p1":"p2",v={...c,[y+"Score"]:3,[b+"Score"]:0,phase:"finished"};try{await T.from("matches").update({status:"finished",forfeit:!0,winner_id:f,home_score:v.p1Score||0,away_score:v.p2Score||0,game_state:v}).eq("id",i)}catch{}try{T.removeChannel(H)}catch{}setTimeout(()=>{Ve(e),l("mini-league")},800)}const ie={BOOST_STAT:({value:f=1,count:y=1,roles:b=[]},v,w)=>{const k=v[n+"Team"],S=Object.entries(k).filter(([E])=>!b.length||b.includes(E)).flatMap(([E,L])=>L.filter(z=>!z.used).map(z=>({...z,_line:E})));if(!S.length){v.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),w(v);return}Q(S,y,`Choisir ${y} joueur(s) à booster (+${f})`,E=>{E.forEach(L=>{const z=(k[L._line]||[]).find(j=>j.cardId===L.cardId);z&&(z.boost=(z.boost||0)+f,v.log.push({text:`⚡ +${f} sur ${z.name}`,type:"info"}))}),v[n+"Team"]=k,w(v)})},DEBUFF_STAT:({value:f=1,count:y=1,roles:b=[],target:v="ai"},w,k)=>{const S=v==="home"?n:x,E=w[S+"Team"],L=v==="home"?"allié":"adverse",z=Object.entries(E).filter(([j])=>!b.length||b.includes(j)).flatMap(([j,D])=>D.map(Y=>({...Y,_line:j})));if(!z.length){w.log.push({text:`🎯 Aucun joueur ${L}`,type:"info"}),k(w);return}Q(z,y,`Choisir ${y} joueur(s) ${L}(s) (-${f})`,j=>{j.forEach(D=>{const Y=(E[D._line]||[]).find(J=>J.cardId===D.cardId);Y&&(Y.boost=(Y.boost||0)-f,w.log.push({text:`🎯 -${f} sur ${Y.name}`,type:"info"}))}),w[S+"Team"]=E,k(w)})},GRAY_PLAYER:({count:f=1,roles:y=[],target:b="ai"},v,w)=>{const k=b==="home"?n:x,S=v[k+"Team"],E=b==="home"?"allié":"adverse",L=Object.entries(S).filter(([z])=>!y.length||y.includes(z)).flatMap(([z,j])=>j.filter(D=>!D.used).map(D=>({...D,_line:z})));if(!L.length){v.log.push({text:`❌ Aucun joueur ${E}`,type:"info"}),w(v);return}Q(L,f,`Choisir ${f} joueur(s) ${E}(s) à exclure`,z=>{const j="usedCardIds_"+k,D=new Set(v[j]||[]);z.forEach(Y=>{const J=(S[Y._line]||[]).find(K=>K.cardId===Y.cardId);J&&(J.used=!0,D.add(Y.cardId),v.log.push({text:`❌ ${J.name} exclu !`,type:"info"}))}),v[j]=[...D],v[k+"Team"]=S,w(v)})},REVIVE_PLAYER:({count:f=1,roles:y=[]},b,v)=>{const w=b[n+"Team"],k=Object.entries(w).filter(([S])=>!y.length||y.includes(S)).flatMap(([S,E])=>E.filter(L=>L.used).map(L=>({...L,_line:S})));if(!k.length){b.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),v(b);return}Q(k,f,`Choisir ${f} joueur(s) à ressusciter`,S=>{S.forEach(E=>{const L=(w[E._line]||[]).find(z=>z.cardId===E.cardId);L&&(L.used=!1,b.log.push({text:`💫 ${L.name} ressuscité !`,type:"info"}))}),b[n+"Team"]=w,v(b)})},REMOVE_GOAL:({},f,y)=>{const b=x+"Score";f[b]>0?(f[b]--,f.log.push({text:"🚫 Dernier but annulé !",type:"info"})):f.log.push({text:"🚫 Aucun but à annuler",type:"info"}),y(f)},ADD_GOAL_DRAW:({},f,y)=>{f[n+"Score"]===f[x+"Score"]?(f[n+"Score"]++,f.log.push({text:"🎯 But bonus !",type:"info"})):f.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),y(f)},ADD_SUB:({value:f=1},y,b)=>{y.maxSubs=(y.maxSubs||3)+f,y.log.push({text:`🔄 +${f} remplacement(s)`,type:"info"}),b(y)},CUSTOM:({},f,y)=>y(f)};function Q(f,y,b,v){const w=document.createElement("div");w.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let k=[];function S(){var L,z;const E=f.map(j=>{const D={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[j._line]||"#555",Y=ze(j,j._line)+(j.boost||0),K=k.find(W=>W.cardId===j.cardId)?"#FFD700":"rgba(255,255,255,0.25)",N=j.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${j.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${K};background:${D};overflow:hidden;cursor:pointer;${N}">
        ...
        </div>`,(L=w.querySelector("#pp-close"))==null||L.addEventListener("click",()=>w.remove()),w.querySelectorAll(".pp-item").forEach(j=>{j.addEventListener("click",()=>{const D=j.dataset.cid,Y=f.find(K=>K.cardId===D),J=k.findIndex(K=>K.cardId===D);Y&&(J>-1?k.splice(J,1):k.length<y&&k.push(Y),S())})}),(z=w.querySelector("#pp-confirm"))==null||z.addEventListener("click",()=>{w.remove(),v(k)})}S(),document.body.appendChild(w)}async function R(f,y){const v=(c["gcCardsFull_"+n]||[]).find(E=>E.id===f),w=(v==null?void 0:v._gcDef)||(c.gcDefs||[]).find(E=>{var L;return E.name===y||((L=E.name)==null?void 0:L.toLowerCase().trim())===(y==null?void 0:y.toLowerCase().trim())}),k=[...c["usedGc_"+n]||[],f],S={type:y,by:n,seq:(c._gcAnimSeq||0)+1};M.add(S.seq),se(y,n,async()=>{if(w!=null&&w.effect_type&&w.effect_type!=="CUSTOM"){const L=ie[w.effect_type];if(L){const z={...c};L(w.effect_params||{},z,async j=>{j["usedGc_"+n]=k,j._lastGC=S,j._gcAnimSeq=S.seq,await V(j)});return}}const E={...c};switch(y){case"Remplacement+":E.maxSubs=(E.maxSubs||3)+1,E.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const L=x+"Score";E[L]>0&&(E[L]--,E.log.push({text:"🚫 But annulé",type:"info"}));break}}E["usedGc_"+n]=k,E._lastGC=S,E._gcAnimSeq=S.seq,await V(E)})}function P(f,y){const b="usedCardIds_"+f,v=new Set(c[b]||[]);y.forEach(w=>v.add(w)),c[b]=[...v]}function re(){for(const f of["p1","p2"]){const y=new Set(c["usedCardIds_"+f]||[]),b=c[f+"Team"];if(b)for(const v of["GK","DEF","MIL","ATT"])(b[v]||[]).forEach(w=>{w.used=y.has(w.cardId)})}}function xe(){var nt,ft,gt,mt,le,X;if(c.phase==="reveal")return _e();if(c.phase==="midfield")return ve();if(c.phase==="finished")return F();const f=c[n+"Team"],y=c[x+"Team"];re();const b=c[n+"Score"],v=c[x+"Score"],w=c[n+"Name"],k=c[x+"Name"],S=c.phase===n+"-attack",E=c.phase===n+"-defense",L=Array.isArray(c["selected_"+n])?c["selected_"+n]:[],z=L.map(O=>O.cardId),j=window.innerWidth>=700,D=c[n+"Subs"]||[],Y=c["usedSubIds_"+n]||[],J=D.filter(O=>!Y.includes(O.cardId)),K=c["gcCardsFull_"+n]||[],N=c["usedGc_"+n]||[],W=K.filter(O=>!N.includes(O.id)),te=c.boostOwner===n&&!c.boostUsed,ee=!["GK","DEF","MIL","ATT"].some(O=>(y[O]||[]).some(ae=>!ae.used)),fe=[...f.MIL||[],...f.ATT||[]].filter(O=>!O.used),ye=S&&ee&&fe.length===0?[...f.GK||[],...f.DEF||[]].filter(O=>!O.used).map(O=>O.cardId):[];function be(O,ae,he){var Ct,Ai;const $e=O._gcDef,Qe={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[$e==null?void 0:$e.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",Ze={purple:"#b06ce0",light_blue:"#00d4ef"}[$e==null?void 0:$e.color]||"#b06ce0",tt=($e==null?void 0:$e.name)||O.gc_type,rt=($e==null?void 0:$e.effect)||((Ct=Oe[O.gc_type])==null?void 0:Ct.desc)||"",_t=$e!=null&&$e.image_url?`/icons/${$e.image_url}`:null,$t=((Ai=Oe[O.gc_type])==null?void 0:Ai.icon)||"⚡",lt=Math.round(he*.22),xt=Math.round(he*.22),yt=he-lt-xt,kt=tt.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${O.id}" data-gc-type="${O.gc_type}"
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${ut(f,c[n+"Formation"],Ue,z,300,300,ye)}
      </div>
      ...
      </div>`;function We(){const O=e.querySelector(".match-screen");if(!O)return;const ae=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);O.style.height=ae+"px",O.style.minHeight=ae+"px",O.style.maxHeight=ae+"px",O.style.overflow="hidden";const he=e.querySelector("#mobile-action-bar"),$e=e.querySelector("#mobile-play-area");he&&$e&&($e.style.paddingBottom=he.offsetHeight+"px")}if(We(),setTimeout(We,120),setTimeout(We,400),B||(B=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",We),window.visualViewport.addEventListener("scroll",We)),window.addEventListener("resize",We)),function(){const ae=e.querySelector(".terrain-wrapper svg");ae&&(ae.removeAttribute("width"),ae.removeAttribute("height"),ae.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",ae.setAttribute("viewBox","-26 -26 352 352"),ae.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),e.querySelectorAll(".match-slot-hit").forEach(O=>{O.addEventListener("click",()=>{if(!S&&!E)return;const ae=O.dataset.cardId,he=O.dataset.role,$e=(f[he]||[]).find(rt=>rt.cardId===ae);if(!$e||$e.used)return;const Qe=ye.includes(ae);if(S&&!["MIL","ATT"].includes(he)&&!Qe)return;Array.isArray(c["selected_"+n])||(c["selected_"+n]=[]);const Ze=c["selected_"+n],tt=Ze.findIndex(rt=>rt.cardId===ae);tt>-1?Ze.splice(tt,1):Ze.length<3&&Ze.push({...$e,_role:he}),xe()})}),e.querySelectorAll(".match-used-hit").forEach(O=>{O.addEventListener("click",()=>U(O.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(O=>{O.addEventListener("click",()=>U())}),(nt=e.querySelector("#pvp-sub-open"))==null||nt.addEventListener("click",()=>U()),e.querySelectorAll(".pvp-gc-mini").forEach(O=>{O.addEventListener("click",()=>pe(O.dataset.gcId,O.dataset.gcType))}),(ft=e.querySelector("#pvp-boost-card"))==null||ft.addEventListener("click",()=>ce()),(gt=e.querySelector("#pvp-action"))==null||gt.addEventListener("click",O=>{S?O.currentTarget.dataset.pass==="1"||!Te(n)?me():ne():E&&ue()}),(mt=e.querySelector("#pvp-quit"))==null||mt.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&oe()}),(le=e.querySelector("#pvp-view-opp"))==null||le.addEventListener("click",()=>ge()),(X=e.querySelector("#pvp-toggle-history"))==null||X.addEventListener("click",()=>de()),q&&(clearInterval(q),q=null),(S||E)&&!A){let O=30,ae=!1;const he=()=>document.getElementById("pvp-timer"),$e=()=>{he()&&(he().textContent=O+"s",he().style.color=ae?"#ff4444":"#fff")};$e(),q=setInterval(()=>{O--,O<0?ae?(clearInterval(q),q=null,S&&!Te(n)?me():oe()):(ae=!0,O=15,$e()):$e()},1e3)}}function _e(){et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(k);let L=!1;const z=()=>{L||(L=!0,k.remove(),setTimeout(()=>w(),50))};k.addEventListener("click",z),setTimeout(z,3500)}function se(f,y,b){var N,W;const v=(c.gcDefs||[]).find(te=>{var ee;return te.name===f||((ee=te.name)==null?void 0:ee.toLowerCase().trim())===(f==null?void 0:f.toLowerCase().trim())}),w={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[v==null?void 0:v.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",k={purple:"#b06ce0",light_blue:"#00d4ef"}[v==null?void 0:v.color]||"#b06ce0",S=(v==null?void 0:v.name)||f,E=(v==null?void 0:v.effect)||((N=Oe[f])==null?void 0:N.desc)||"",L=v!=null&&v.image_url?`/icons/${v.image_url}`:null,z=((W=Oe[f])==null?void 0:W.icon)||"⚡",D=y===n?"Vous":c[y+"Name"]||"Adversaire",Y=document.createElement("div");Y.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",Y.innerHTML=`
      <style>
        @keyframes gcFlipIn{0%{transform:perspective(800px) rotateY(90deg) scale(.7);opacity:0}55%{transform:perspective(800px) rotateY(-12deg) scale(1.08);opacity:1}100%{transform:perspective(800px) rotateY(0) scale(1);opacity:1}}
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(Y);let J=!1;const K=()=>{J||(J=!0,Y.remove(),setTimeout(()=>b&&b(),50))};Y.addEventListener("click",K),setTimeout(K,3e3)}function pe(f,y){var Y,J,K,N;const v=(c["gcCardsFull_"+n]||[]).find(W=>W.id===f),w=v==null?void 0:v._gcDef,k={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[w==null?void 0:w.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",S={purple:"#b06ce0",light_blue:"#00d4ef"}[w==null?void 0:w.color]||"#b06ce0",E=(w==null?void 0:w.name)||y,L=(w==null?void 0:w.effect)||((Y=Oe[y])==null?void 0:Y.desc)||"Carte spéciale.",z=w!=null&&w.image_url?`/icons/${w.image_url}`:null,j=((J=Oe[y])==null?void 0:J.icon)||"⚡",D=document.createElement("div");D.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",D.innerHTML=`
      <div style="width:190px;border-radius:16px;border:3px solid ${S};background:${k};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 40px ${S}66">
        ...
        <div style="height:160px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
          ${z?`<img src="${z}" style="max-width:150px;max-height:150px;object-fit:contain">`:`<span style="font-size:72px">${j}</span>`}
        </div>
        <div style="padding:10px;background:rgba(0,0,0,0.38);text-align:center">
      ...
      </div>`,document.body.appendChild(b),(v=b.querySelector("#bp-close"))==null||v.addEventListener("click",()=>b.remove()),b.querySelectorAll(".bp-item").forEach(w=>{w.addEventListener("click",async()=>{const k=w.dataset.cid,S=y.find(L=>L.cardId===k);if(!S)return;const E=(f[S._line]||[]).find(L=>L.cardId===k);E&&(E.boost=(E.boost||0)+c.boostValue),b.remove(),await V({[n+"Team"]:f,boostUsed:!0})})})}function U(f=null){var J,K;if(!(c.phase===n+"-attack")){p("Remplacement uniquement avant votre attaque","warning");return}const b=c[n+"Team"],v=c["usedSubIds_"+n]||[],w=c.maxSubs||3;if(v.length>=w){p(`Maximum ${w} remplacements atteint`,"warning");return}const k=Object.entries(b).flatMap(([N,W])=>(W||[]).filter(te=>te.used).map(te=>({...te,_line:N}))),S=(c[n+"Subs"]||[]).filter(N=>!v.includes(N.cardId));if(!k.length){p("Aucun joueur utilisé à remplacer","warning");return}if(!S.length){p("Aucun remplaçant disponible","warning");return}let E=Math.max(0,k.findIndex(N=>N.cardId===f));const L=((J=k[E])==null?void 0:J._line)||((K=k[E])==null?void 0:K.job);let z=Math.max(0,S.findIndex(N=>N.job===L)),j=!1;const D=document.createElement("div");D.id="pvp-sub-overlay",D.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function Y(){var be,ke,Be,Fe,Me,je;const N=k[E],W=S[z],te=Math.min(130,Math.round((window.innerWidth-90)/2)),ee=Math.round(te*1.35),fe=Ae=>`background:rgba(255,255,255,0.12);border:none;color:${Ae?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Ae?"default":"pointer"};flex-shrink:0`;D.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="width:min(90vw,420px)">${pt(c[x+"Team"],c[x+"Formation"],null,[],300,300)}</div>
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(f),(y=f.querySelector("#pvp-opp-close"))==null||y.addEventListener("click",()=>f.remove())}function de(){var v;const f=c.log||[],y=document.createElement("div");y.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column";const b=w=>{var S,E,L;if(w.type==="duel"){const z=w.isGoal,j=w.homeScored&&n==="p1"||!w.homeScored&&z&&n==="p2",D=w.homeScored?"#FFD700":z?"#ff6b6b":"rgba(255,255,255,0.3)",Y=z?j?"⚽ BUT !":"⚽ BUT adversaire !":(S=w.homePlayers)!=null&&S.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${z?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${D};margin-bottom:4px">
      ...
      </div>`,document.body.appendChild(y),(v=y.querySelector("#pvp-hist-close"))==null||v.addEventListener("click",()=>y.remove())}async function me(){if(c.phase!==n+"-attack")return;const f=n==="p1"?"p2":"p1",y=(c.round||0)+1,b=[...c.log||[]];b.push({type:"info",text:`⏭️ ${c[n+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const v=qe(c),w=Te(f),k=v||!w?"finished":f+"-attack";await V({["selected_"+n]:[],modifiers:{...c.modifiers,[n]:{}},pendingAttack:null,phase:k,attacker:f,round:y,log:b}),k==="finished"&&await Se(c)}async function ne(){const f=c[n+"Team"],y=!["GK","DEF","MIL","ATT"].some(k=>(c[x+"Team"][k]||[]).some(S=>!S.used)),b=(c["selected_"+n]||[]).map(k=>{const S=(f[k._role]||[]).find(Y=>Y.cardId===k.cardId)||k,E=y&&["GK","DEF"].includes(k._role),L=f[k._role]||[],z=L.findIndex(Y=>Y.cardId===k.cardId),j=dt(L.length),D=z>=0?j[z]:S._col??1;return{...S,_line:k._role,_col:D,...E?{note_a:Math.max(1,Number(S.note_a)||0)}:{}}});if(!b.length)return;const v=Ut(b,c.modifiers[n]||{});P(n,b.map(k=>k.cardId)),b.forEach(k=>{const S=(f[k._role]||[]).find(E=>E.cardId===k.cardId);S&&(S.used=!0)}),c["selected_"+n]=[],xe();const w=[...c.log||[]];if(y){const k=(c[n+"Score"]||0)+1,S=b.map(D=>({name:D.name,note:ze(D,D._line||"ATT"),portrait:He(D),job:D.job}));w.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:S,homeTotal:v.total,aiTotal:0});const E=(c.round||0)+1,L=n==="p1"?"p2":"p1",z={...c,[n+"Team"]:f,[n+"Score"]:k},j=qe(z);C.add(E),Ee(S,k,c[x+"Score"]||0,!0,async()=>{await V({[n+"Team"]:f,[n+"Score"]:k,["selected_"+n]:[],modifiers:{...c.modifiers,[n]:{}},pendingAttack:null,phase:j?"finished":L+"-attack",attacker:L,round:E,log:w}),j&&await Se({...c,[n+"Score"]:k})});return}w.push({type:"pending",text:`⚔️ ${c[n+"Name"]} attaque (${v.total})`}),await V({[n+"Team"]:f,[x+"Team"]:c[x+"Team"],pendingAttack:{...v,players:b,side:n},["selected_"+n]:[],modifiers:{...c.modifiers,[n]:{}},phase:x+"-defense",log:w})}async function ue(){const f=c[n+"Team"],y=(c["selected_"+n]||[]).map(N=>{const W=(f[N._role]||[]).find(be=>be.cardId===N.cardId)||N,te=f[N._role]||[],ee=te.findIndex(be=>be.cardId===N.cardId),fe=dt(te.length),ye=ee>=0?fe[ee]:W._col??1;return{...W,_line:N._role,_col:ye}}),b=Kt(y,c.modifiers[n]||{});P(n,y.map(N=>N.cardId)),y.forEach(N=>{const W=(f[N._role]||[]).find(te=>te.cardId===N.cardId);W&&(W.used=!0)}),c["selected_"+n]=[],xe();const v=Vt(c.pendingAttack.total,b.total,c.modifiers[n]||{}),w=c.pendingAttack.side,k=v==="attack"||(v==null?void 0:v.goal),S=w==="p1"?"p2":"p1",E=(c.round||0)+1,L=(c.pendingAttack.players||[]).map(N=>({name:N.name,note:ze(N,N._line||"ATT"),portrait:He(N),job:N.job})),z=[...c.log||[]];z.push({type:"duel",isGoal:k,homeScored:k&&w===n,text:k?`⚽ BUT de ${c[w+"Name"]} ! (${c.pendingAttack.total} vs ${b.total})`:`✋ Attaque stoppée (${c.pendingAttack.total} vs ${b.total})`,homePlayers:L,aiPlayers:y.map(N=>({name:N.name,note:ze(N,N._line||"DEF"),portrait:He(N),job:N.job})),homeTotal:c.pendingAttack.total,aiTotal:b.total});const j=k?(c[w+"Score"]||0)+1:c[w+"Score"]||0,D={...c,[n+"Team"]:f,[w+"Score"]:j},Y=qe(D),J=Y?"finished":S+"-attack",K=async()=>{await V({[n+"Team"]:f,[x+"Team"]:c[x+"Team"],[w+"Score"]:j,["selected_"+n]:[],modifiers:{...c.modifiers,[n]:{}},pendingAttack:null,phase:J,attacker:S,round:E,log:z}),(J==="finished"||Y)&&await Se({...c,[w+"Score"]:j})};if(k){const N=w===n,W=N?j:c[n+"Score"]||0,te=N?c[x+"Score"]||0:j;C.add(E),Ee(L,W,te,N,K)}else await K()}function Le(f){return["MIL","ATT"].some(y=>(f[y]||[]).some(b=>!b.used))}function Ie(f){return["GK","DEF","MIL","ATT"].some(y=>(f[y]||[]).some(b=>!b.used))}function Ce(f){return Ie(f)&&!Le(f)}function Te(f){const y=c[f+"Team"],b=c[(f==="p1"?"p2":"p1")+"Team"];return!!(Le(y)||!Ie(b)&&Ce(y))}function qe(f){const y=["MIL","ATT"].some(z=>(f.p1Team[z]||[]).some(j=>!j.used)),b=["MIL","ATT"].some(z=>(f.p2Team[z]||[]).some(j=>!j.used)),v=Ie(f.p1Team),w=Ie(f.p2Team);return!y&&!(!w&&v)&&(!b&&!(!v&&w))}function Pe(f){const y=f.p1Score||0,b=f.p2Score||0;return y===b?null:y>b?_.home_id:_.away_id}async function Se(f){try{const y=Pe(f),b=y?_.home_id===y?_.away_id:_.home_id:null,v=f.p1Score||0,w=f.p2Score||0;await T.from("matches").update({status:"finished",winner_id:y,home_score:v,away_score:w}).eq("id",i);const{data:k}=await T.from("mini_league_matches").select("id, league_id, matchday").eq("match_id",i).single();if(k){await T.from("mini_league_matches").update({home_score:v,away_score:w,status:"finished"}).eq("id",k.id);const{data:S}=await T.from("mini_league_matches").select("id, status").eq("league_id",k.league_id).eq("matchday",k.matchday);if((S||[]).every(L=>L.status==="finished"||L.status==="bye")){const{data:L}=await T.from("mini_leagues").select("current_day,total_days").eq("id",k.league_id).single();if(L){const z=(L.current_day||0)+1,j=z>(L.total_days||0);await T.from("mini_leagues").update({current_day:j?L.total_days:z,status:j?"finished":"active"}).eq("id",k.league_id)}}}y&&b&&Ti(y,b).catch(()=>{})}catch(y){console.error("[ML] finishMatch:",y)}}function F(){var w;if(A&&document.getElementById("pvp-end-overlay"))return;A=!0;const f=c[n+"Score"],y=c[x+"Score"],b=f>y,v=f===y;et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
    ...
    </div>`,(w=document.getElementById("pvp-home"))==null||w.addEventListener("click",()=>{try{T.removeChannel(H)}catch{}Ve(e),l("mini-league",s?{openLeagueId:s}:{})})}xe()}const lr="/",cr=[{emoji:"⚽",title:"Bienvenue dans Manager Wars !",color:"#1A6B3C",content:`<p>Tu es désormais un <strong>manager de football</strong> virtuel.</p>
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div id="mkt-content" style="padding:10px 16px;display:flex;flex-direction:column;gap:8px"></div>
  </div>`;let p="buy";const s=()=>{var m,c,A,g,I,q,B;return{name:(((m=document.getElementById("flt-name"))==null?void 0:m.value)||"").toLowerCase().trim(),club:(((c=document.getElementById("flt-club"))==null?void 0:c.value)||"").toLowerCase().trim(),country:(((A=document.getElementById("flt-country"))==null?void 0:A.value)||"").toLowerCase().trim(),job:((g=document.getElementById("flt-job"))==null?void 0:g.value)||"",rarity:((I=document.getElementById("flt-rarity"))==null?void 0:I.value)||"",note1:parseInt((q=document.getElementById("flt-note1"))==null?void 0:q.value)||0,note2:parseInt((B=document.getElementById("flt-note2"))==null?void 0:B.value)||0}};function n(m){const c=s();return m.filter(A=>{var H,V,oe;const g=(H=A.card)==null?void 0:H.player;if(!g)return!1;const I=`${g.firstname} ${g.surname_encoded}`.toLowerCase(),q=(((V=g.clubs)==null?void 0:V.encoded_name)||"").toLowerCase(),B=(g.country_code||"").toLowerCase(),C=((oe=A.card)==null?void 0:oe.evolution_bonus)||0,M=zt(g,g.job,C),G=g.job2?zt(g,g.job2,C):0;return!(c.name&&!I.includes(c.name)||c.club&&!q.includes(c.club)||c.country&&!B.includes(c.country)||c.job&&g.job!==c.job||c.rarity&&g.rarity!==c.rarity||c.note1&&M<c.note1||c.note2&&G<c.note2)})}function x(m){var G,H,V,oe;const c=(G=m.card)==null?void 0:G.player;if(!c)return"";const A=((H=m.card)==null?void 0:H.evolution_bonus)||0,g=zt(c,c.job,A),I=c.job2?zt(c,c.job2,A):0,q=(i.profile.credits||0)>=m.price,B=c.country_code?`https://flagsapi.com/${c.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,C=Xt[c.job]||"#bbb",M=c.job2?Xt[c.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden">
      <!-- Drapeau -->
      ...
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${C};display:flex;align-items:center;justify-content:center">
          <span style="font-size:14px;font-weight:900;color:${C};font-family:Arial Black,Arial">${g}</span>
        </div>
        ${I?`<div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${M};display:flex;align-items:center;justify-content:center">
          <span style="font-size:14px;font-weight:900;color:${M};font-family:Arial Black,Arial">${I}</span>
        </div>`:""}
      </div>
      <!-- Nom -->
      <div style="flex:1;min-width:0">
    ...
    </div>`}function u(m){var G,H,V,oe;const c=(G=m.card)==null?void 0:G.player;if(!c)return"";const A=((H=m.card)==null?void 0:H.evolution_bonus)||0,g=zt(c,c.job,A),I=c.job2?zt(c,c.job2,A):0,q=m.status==="sold",B=c.country_code?`https://flagsapi.com/${c.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,C=Xt[c.job]||"#bbb",M=c.job2?Xt[c.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${q?"opacity:0.7":""}">
      ${B?`<img src="${B}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      ${(V=c.clubs)!=null&&V.logo_url?`<img src="${c.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${C};display:flex;align-items:center;justify-content:center">
      ...
      <div style="flex:1;min-width:0">
        ...
        <div style="font-size:10px;color:${q?"#22c55e":"#999"};margin-top:1px">
          ${q?`✅ Vendu à ${((oe=m.buyer)==null?void 0:oe.pseudo)||"—"} · ${m.sold_at?new Date(m.sold_at).toLocaleDateString("fr"):""}`:`🟢 En vente depuis le ${new Date(m.listed_at).toLocaleDateString("fr")}`}
        </div>
      </div>
      <div style="text-align:right;flex-shrink:0">
        <div style="font-size:14px;font-weight:900;color:#D4A017">${m.price.toLocaleString("fr")}</div>
    ...
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center">
      ...
      `).join(""):'<div style="text-align:center;color:var(--gray-600);padding:40px">Aucun manager.</div>'}else{const A=r||[];_.innerHTML=A.length>0?A.map((g,I)=>{const q=Gt(g.mmr??1e3),B=(g.ranked_wins||0)+(g.ranked_losses||0)+(g.ranked_draws||0),C=B>0?Math.round((g.ranked_wins||0)/B*100):0,M=g.id===i.profile.id,G=(g.placement_matches||0)<10;return`
          <div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${M?"border:2px solid var(--yellow)":""}">
            ...
            <div style="flex:1;min-width:0">
              ...
              <div style="font-size:11px;color:var(--gray-600)">${g.club_name} · ${q.label}</div>
            </div>
            <div style="text-align:right;flex-shrink:0">
              <div style="font-size:20px">${G?"❓":q.emoji}</div>
              <div style="font-size:11px;font-weight:700;color:${q.color}">${G?"Placement":q.label}</div>
