import{s as $,F as Qe,b as Me,c as Bt,l as ge,d as ri}from"./formation-links-CDBKB_z4.js";function wt(e,{navigate:t,toast:i}){e.innerHTML=`
  ...
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",b=>{F=b.target.value.toLowerCase(),V()}),e.querySelectorAll(".filter-btn").forEach(b=>{b.addEventListener("click",()=>{k=b.dataset.job,P(),V()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{m=!m,P(),V()})):(_.innerHTML=`
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const $e=1e3;function xi(e,t,i,s){var u,h,T;const{state:c,toast:o,closeModal:a,navigate:d,refreshProfile:l}=i,n=e.formation,r={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function p(){const v=Me[n]||{},x=Qe[n]||[],k=290,F=360,m=20;function A(q){const G=v[q];return G?{x:G.x*k,y:G.y*F}:null}let M=`<svg width="${k}" height="${F}" viewBox="0 0 ${k} ${F}" xmlns="http://www.w3.org/2000/svg">`;for(const[q,G]of x){const P=A(q),V=A(G);!P||!V||(M+=`<line x1="${P.x}" y1="${P.y}" x2="${V.x}" y2="${V.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const q of Object.keys(v)){const G=A(q);if(!G)continue;const P=q.replace(/\d+/,""),V=r[P]||"#555";M+=`<circle cx="${G.x}" cy="${G.y}" r="${m}" fill="${V}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,M+=`<text x="${G.x}" y="${G.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${P}</text>`}return M+="</svg>",M}const g=t.filter(v=>v.formation===n),f=g.length,y=!e.is_for_sale;s(`Formation ${n}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}const Pi="simulation",Ht="VOTRE_ZONE_ID";function Oi(){switch(Pi){case"propellerads":return Hi();case"adsense":return Ki();default:return nt()}}function nt(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:#111;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:14px";let i=5;t.innerHTML=`
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order");let l=0;function n(){var v,x,k,F,m;const r=o[l],p=(d||[]).filter(A=>A.deck_id===r.id),g=p.filter(A=>{var M;return A.is_starter&&((M=A.card)==null?void 0:M.player)}).map(A=>Ye(A.card,A.position)),f=p.find(A=>{var M;return((M=A.card)==null?void 0:M.card_type)==="formation"}),y=r.formation||((v=f==null?void 0:f.card)==null?void 0:v.formation)||"4-4-2",u=g.length>=11?Ne(g,y):null,h=g.length>=11;Ut(),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;height:${p}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ...
      ${c?`<img src="${c}" style="width:${p-4}px;height:${p-4}px;object-fit:contain">`:e!=null&&e.clubName?`<span style="font-size:${Math.max(4,p-8)}px;font-weight:700;color:#333">${(e.clubName||"").slice(0,3).toUpperCase()}</span>`:""}
        ...
        stroke="${M}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const g=48,f=64,y=13,u=16,h={normal:"#aaaaaa",pépite:"#D4A017",papyte:"#111111",légende:"#7a28b8"};for(const[v,x]of Object.entries(l)){const k=r(v);if(!k||!x)continue;const F=v.replace(/[0-9]/g,""),m=_e[F]||"#555",A=i==="attack"&&["MIL","ATT"].includes(F)&&!x.used||i==="defense"&&["GK","DEF","MIL"].includes(F)&&!x.used,M=s.includes(x.cardId);let q;i==="attack"?q=F==="MIL"?Number(x.note_m)||0:Number(x.note_a)||0:i==="defense"?q=F==="GK"?Number(x.note_g)||0:F==="MIL"?Number(x.note_m)||0:Number(x.note_d)||0:q=Number(F==="GK"?x.note_g:F==="DEF"?x.note_d:F==="MIL"?x.note_m:x.note_a)||0,q=q+(x.boost||0);const G=(k.x-g/2).toFixed(1),P=(k.y-f/2).toFixed(1),V=x.used?.25:1,Z=h[x==null?void 0:x.rarity]||h.normal,Y=M?"#ff3030":Z,U=M?3:(x==null?void 0:x.rarity)==="légende"||(x==null?void 0:x.rarity)==="pépite"?2.5:2,L=f-y-u;p+=`<defs><clipPath id="cp-${v}"><rect x="${G}" y="${(k.y-f/2+y).toFixed(1)}" width="${g}" height="${L}"/></clipPath></defs>`,p+=`<rect x="${G}" y="${P}" width="${g}" height="${f}" rx="5" fill="${m}" opacity="${V}"/>`;const j=oe(x);j&&!x.used&&(p+=`<image href="${j}" x="${G}" y="${(k.y-f/2+y).toFixed(1)}" width="${g}" height="${L}" clip-path="url(#cp-${v})" preserveAspectRatio="xMidYMin slice"/>`),p+=`<rect x="${G}" y="${P}" width="${g}" height="${y}" rx="4" fill="rgba(255,255,255,0.92)"/>`,p+=`<text x="${k.x.toFixed(1)}" y="${(k.y-f/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(x.name||"").slice(0,9)}</text>`;const z=(k.y+f/2-u).toFixed(1);if(p+=`<rect x="${G}" y="${z}" width="${g}" height="${u}" fill="rgba(255,255,255,0.92)"/>`,x.used)p+=`<text x="${k.x.toFixed(1)}" y="${(k.y+f/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="13" font-weight="900" fill="rgba(0,0,0,0.4)" font-family="Arial Black">–</text>`;else{const N=We(x.country_code);N?p+=`<image href="${N}" x="${(k.x-20).toFixed(1)}" y="${(k.y+f/2-u+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:p+=`<text x="${(k.x-13).toFixed(1)}" y="${(k.y+f/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${Wt(x.country_code)}</text>`,p+=`<text x="${k.x.toFixed(1)}" y="${(k.y+f/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${q}</text>`;const D=Yt(x);D?p+=`<image href="${D}" x="${(k.x+g/2-14).toFixed(1)}" y="${(k.y+f/2-u+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:x.clubName&&(p+=`<text x="${(k.x+14).toFixed(1)}" y="${(k.y+f/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(x.clubName||"").slice(0,3).toUpperCase()}</text>`),x.boost&&(p+=`<rect x="${(k.x+g/2-12).toFixed(1)}" y="${(k.y-f/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,p+=`<text x="${(k.x+g/2-5).toFixed(1)}" y="${(k.y-f/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${x.boost}</text>`)}p+=`<rect x="${G}" y="${P}" width="${g}" height="${f}" rx="5" fill="${M?"rgba(255,255,255,0.12)":"none"}" stroke="${Y}" stroke-width="${U}" opacity="${V}"/>`,A&&(p+=`<rect x="${G}" y="${P}" width="${g}" height="${f}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${M?"selected":""}" data-card-id="${x.cardId}" data-role="${F}" style="cursor:pointer"/>`)}const T=38;return`<svg viewBox="${-T} ${-T} ${c+T*2} ${o+T*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${p}
  ...
  </div>`,setTimeout(()=>Ji(e,t,i),5e3)}function Ji(e,t,i){const s=t.homeTeam.MIL||[],c=t.aiTeam.MIL||[];function o(p){return p.reduce((g,f)=>g+te(f,"MIL"),0)}function a(p){let g=0;for(let f=0;f<p.length-1;f++){const y=ge(p[f],p[f+1]);y==="#00ff88"?g+=2:y==="#FFD700"&&(g+=1)}return g}const d=o(s)+a(s),l=o(c)+a(c),n=d>=l;function r(p,g,f){return`<div style="text-align:center">
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div id="midfield-result" style="opacity:0;text-align:center;transition:opacity 0.5s;color:#fff;max-width:320px"></div>
      ...
      </div>`,(h=document.getElementById("start-match-btn"))==null||h.addEventListener("click",()=>{t.phase=t.attacker==="home"?"attack":"ai-attack",ne(e,t,i),t.attacker==="ai"&&setTimeout(()=>Xe(e,t,i),800)})},100)},5e3)}function Zi(e){if(e.type==="duel"){const t=e.homeTotal>=e.aiTotal;return`
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;height:100%;width:100%">
    ...
    ${(()=>{const m=window.innerWidth>=700,A=(U,L,j)=>{var S,H;const z=(t.gcDefs||[]).find(K=>K.name===U.gc_type),N={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[z==null?void 0:z.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",D={purple:"#b06ce0",light_blue:"#00d4ef"}[z==null?void 0:z.color]||"#b06ce0",_=(z==null?void 0:z.name)||U.gc_type,b=(z==null?void 0:z.effect)||((S=de[U.gc_type])==null?void 0:S.desc)||"",C=z!=null&&z.image_url?`/manager-wars/icons/${z.image_url}`:null,E=((H=de[U.gc_type])==null?void 0:H.icon)||"⚡",w=Math.round(j*.22),B=Math.round(j*.22),I=j-w-B,R=_.length>12?7:9;return`<div class="gc-mini" data-gc-id="${U.id}" data-gc-type="${U.gc_type}"
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(m=>{var A,M,q;if(m.type==="duel"){const G=m.isGoal,P=m.homeScored?"#FFD700":G?"#ff6b6b":"rgba(255,255,255,0.3)",V=m.homeScored?"⚽ BUT !":G?"⚽ BUT IA !":(A=m.homePlayers)!=null&&A.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${G?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${P};margin-bottom:4px">
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(o);let d=!1;const l=()=>{d||(d=!0,o.remove(),setTimeout(()=>c(),50))};o.addEventListener("click",l),setTimeout(l,3500)}async function ve(e,t,i){var r,p;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:s}=i,c=t.homeScore>t.aiScore,o=t.homeScore===t.aiScore,a=c?"victoire":o?"nul":"defaite",d=ci(t.mode,a);t.matchId&&await $.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:c?s.profile.id:null,home_credits_reward:d,played_at:new Date().toISOString()}).eq("id",t.matchId);const l={credits:(s.profile.credits||0)+d,matches_played:(s.profile.matches_played||0)+1};c?l.wins=(s.profile.wins||0)+1:o?l.draws=(s.profile.draws||0)+1:l.losses=(s.profile.losses||0)+1,await $.from("users").update(l).eq("id",s.profile.id),await i.refreshProfile();const n=document.createElement("div");n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",n.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const f=()=>document.getElementById("mm-status"),y=async(x=!0)=>{p=!0,g&&($.removeChannel(g),g=null),x&&await $.rpc("cancel_matchmaking",{p_user_id:l.profile.id}).catch(()=>{})};(v=document.getElementById("mm-cancel"))==null||v.addEventListener("click",async()=>{try{await y(!0)}catch(x){console.warn("[Matchmaking] cleanup error:",x)}le(),n("home")});const u=async(x,k,F)=>{p||(p=!0,g&&($.removeChannel(g),g=null),f()&&(f().textContent="Adversaire trouvé !"),await new Promise(m=>setTimeout(m,600)),e.isConnected!==!1&&po(e,t,x,F,a,d))},{data:h,error:T}=await $.rpc("try_matchmake",{p_user_id:l.profile.id,p_deck_id:i});if(T||!(h!=null&&h.success)){r("Erreur de matchmaking","error"),le(),n("home");return}if(h.matched){u(h.match_id,h.opponent_id,!1);return}f()&&(f().textContent="En attente d'un autre joueur..."),g=$.channel("matchmaking-"+l.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${l.profile.id}`},x=>{const k=x.new;k.status==="matched"&&k.match_id&&u(k.match_id,k.matched_with,!0)}).subscribe()}async function po(e,t,i,s,c=[],o=[]){const{state:a,navigate:d,toast:l}=t,n=s?"p1":"p2",r=s?"p2":"p1",p=(c||[]).map(L=>L.id),g=(c||[]).map(L=>({id:L.id,gc_type:L.gc_type,_gcDef:L._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation du match...</div>';const{data:f}=await $.from("matches").select("*").eq("id",i).single();if(!f){l("Match introuvable","error"),d("home");return}async function y(){const[{data:L,error:j},{data:z,error:N},{data:D},{data:_}]=await Promise.all([$.rpc("get_deck_for_match",{p_deck_id:f.home_deck_id}),$.rpc("get_deck_for_match",{p_deck_id:f.away_deck_id}),$.from("users").select("id,pseudo,club_name").eq("id",f.home_id).single(),$.from("users").select("id,pseudo,club_name").eq("id",f.away_id).single()]);j&&console.error("[PvP] get_deck_for_match p1 error:",j),N&&console.error("[PvP] get_deck_for_match p2 error:",N);function b(S,H){return{cardId:S.card_id,position:H||S.position,id:S.id,firstname:S.firstname,name:S.surname_encoded,country_code:S.country_code,club_id:S.club_id,job:S.job,job2:S.job2,note_g:Number(S.note_g)||0,note_d:Number(S.note_d)||0,note_m:Number(S.note_m)||0,note_a:Number(S.note_a)||0,rarity:S.rarity,skin:S.skin,hair:S.hair,hair_length:S.hair_length,clubName:S.club_encoded_name||null,clubLogo:S.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}}const C=((L==null?void 0:L.starters)||[]).map(S=>b(S,S.position)),E=((z==null?void 0:z.starters)||[]).map(S=>b(S,S.position)),w=((L==null?void 0:L.subs)||[]).map(S=>b(S,S.position)),B=((z==null?void 0:z.subs)||[]).map(S=>b(S,S.position)),I=(L==null?void 0:L.formation)||"4-4-2",R=(z==null?void 0:z.formation)||"4-4-2";return console.log("[PvP] p1Formation:",I,"p1 starters:",C.length,"positions:",C.map(S=>S.position)),console.log("[PvP] p2Formation:",R,"p2 starters:",E.length,"positions:",E.map(S=>S.position)),{p1Team:Ne(C,I),p2Team:Ne(E,R),p1Subs:w,p2Subs:B,p1Formation:I,p2Formation:R,p1Name:(D==null?void 0:D.club_name)||(D==null?void 0:D.pseudo)||"Joueur 1",p2Name:(_==null?void 0:_.club_name)||(_==null?void 0:_.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:s?p:[],gc_p2:s?[]:p,gcCardsFull_p1:s?g:[],gcCardsFull_p2:s?[]:g,gcReady_p1:!0,gcReady_p2:!0,usedGc_p1:[],usedGc_p2:[],lastActionAt:new Date().toISOString()}}let u=f.game_state&&Object.keys(f.game_state).length?f.game_state:null;if(!u)if(s){u=await y();const{data:L}=await $.from("matches").select("game_state").eq("id",i).single();!(L!=null&&L.game_state)||!Object.keys(L.game_state).length?await $.from("matches").update({game_state:u,turn_user_id:f.home_id}).eq("id",i):u=L.game_state}else{e.innerHTML=`<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation avec l'adversaire...</div>`;for(let L=0;L<30&&!u;L++){await new Promise(z=>setTimeout(z,400));const{data:j}=await $.from("matches").select("game_state").eq("id",i).single();j!=null&&j.game_state&&Object.keys(j.game_state).length&&(u=j.game_state)}if(!u){l("Erreur de synchronisation","error"),d("home");return}u.gc_p2=p,u.gcCardsFull_p2=g,await $.from("matches").update({game_state:u}).eq("id",i)}let h=!1;function T(L){var C;try{$.removeChannel(v)}catch{}const j=a.profile.id,z=L.winner_id===j,N=!!L.forfeit,D=u[n+"Score"]??0,_=u[r+"Score"]??0,b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center",b.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(b),(C=b.querySelector("#pvp-end-home"))==null||C.addEventListener("click",()=>{b.remove(),le(),d("home")})}const v=$.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},L=>{const j=L.new;try{if(j.status==="finished"||j.forfeit){if(h)return;h=!0,u._timerInt&&(clearInterval(u._timerInt),u._timerInt=null),j.game_state&&(u=j.game_state),T(j);return}j.game_state&&(u=j.game_state,F())}catch(z){console.error("[PvP] Realtime render crash:",z,"gameState:",u)}}).subscribe();async function x(L){Object.assign(u,L),u.lastActionAt=new Date().toISOString();const{error:j}=await $.from("matches").update({game_state:u}).eq("id",i);j&&(console.error("[PvP] pushState DB error:",j.message,j),l("Erreur de synchronisation : "+j.message,"error"));try{F()}catch(z){console.error("[PvP] renderPvpScreen crash après pushState:",z),l("Erreur d'affichage : "+z.message,"error")}}async function k(){if(h)return;h=!0,u._timerInt&&(clearInterval(u._timerInt),u._timerInt=null);const L=s?f.away_id:f.home_id;try{await $.from("matches").update({status:"finished",forfeit:!0,winner_id:L}).eq("id",i)}catch(j){console.warn("[PvP] forfeit DB error:",j)}try{$.removeChannel(v)}catch{}le(),d("home")}function F(){var pt,ut,ft,gt,xt;const L=u[n+"Team"];u[r+"Team"];const j=u[n+"Score"],z=u[r+"Score"],N=u[n+"Name"],D=u[r+"Name"];if(u.phase==="gc-select")return m();if(u.phase==="reveal")return A();if(u.phase==="midfield")return M();if(u.phase==="finished")return Z();const _=u.phase===n+"-attack",b=u.phase===n+"-defense",C=u.phase===r+"-attack"||u.phase===r+"-defense",E=Array.isArray(u["selected_"+n])?u["selected_"+n]:[],w=E.map(O=>O.cardId);e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column";const B=window.innerWidth>=700,I=u[n+"Subs"]||[],R=u["usedSubIds_"+n]||[],S=I.filter(O=>!R.includes(O.cardId)),H=u["gcCardsFull_"+n]||[],K=u["usedGc_"+n]||[],Q=H.filter(O=>!K.includes(O.id)),ie=u.boostOwner===n&&!u.boostUsed,re=(O,X,ee)=>{var vt,bt;const W=O._gcDef,ce={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[W==null?void 0:W.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",Ce={purple:"#b06ce0",light_blue:"#00d4ef"}[W==null?void 0:W.color]||"#b06ce0",ye=(W==null?void 0:W.name)||O.gc_type,ii=(W==null?void 0:W.effect)||((vt=de[O.gc_type])==null?void 0:vt.desc)||"",mt=W!=null&&W.image_url?`/manager-wars/icons/${W.image_url}`:null,oi=((bt=de[O.gc_type])==null?void 0:bt.icon)||"⚡",yt=Math.round(ee*.22),ht=Math.round(ee*.22),He=ee-yt-ht,ni=ye.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${O.id}" data-gc-type="${O.gc_type}"
        ...
        <div style="height:${yt}px;background:rgba(255,255,255,0.14);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 3px">
          <span style="font-size:${ni}px;font-weight:900;color:#fff;text-align:center;line-height:1.1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${X-6}px">${ye}</span>
          <span style="font-size:6px;color:rgba(255,255,255,0.45)">⚡ GC</span>
        </div>
        <div style="height:${He}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
          ${mt?`<img src="${mt}" style="max-width:${X-10}px;max-height:${He-6}px;object-fit:contain">`:`<span style="font-size:${Math.round(He*.55)}px">${oi}</span>`}
        </div>
      ...
      ${S.length===0?'<div style="font-size:7px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':S.map(O=>`<div class="pvp-sub-btn" data-sub-id="${O.cardId}" style="cursor:pointer;flex-shrink:0">${fe(O,B?60:44,B?78:58)}</div>`).join("")}
    </div>`,xe=`<div style="overflow:hidden;min-width:0;flex:1;min-height:0" id="match-field">
      <div class="terrain-wrapper" style="width:100%;height:100%;overflow:hidden">
        ${Te(L,u[n+"Formation"],_?"attack":b?"defense":"idle",w,300,300)}
      </div>
        ...
        </div>`;const X=Array.isArray(u.log)?u.log:[],ee=X[X.length-1];return ee?`<div style="padding:7px 10px;border-left:3px solid ${ee.type==="goal"?"#FFD700":ee.type==="stop"?"#00ff88":ee.type==="attack"?"#ff6b6b":"rgba(255,255,255,0.6)"};font-size:12px;color:#fff">${ee.text||""}</div>`:'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})(),ti=(Array.isArray(u.log)?u.log:[]).length,ct=`
      <!-- Score -->
      ...
      </div>`;const O=e.querySelector(".terrain-wrapper");O&&(O.innerHTML=Te(L,u[n+"Formation"],_?"attack":b?"defense":"idle",w,300,300))}if(e.querySelectorAll(".match-slot-hit").forEach(O=>{O.addEventListener("click",()=>{if(!_&&!b)return;const X=O.dataset.cardId,ee=O.dataset.role,W=(L[ee]||[]).find(ye=>ye.cardId===X);if(!W||W.used)return;Array.isArray(u["selected_"+n])||(u["selected_"+n]=[]);const ce=u["selected_"+n],Ce=ce.findIndex(ye=>ye.cardId===X);Ce>-1?ce.splice(Ce,1):ce.length<3&&ce.push({...W,_role:ee}),F()})}),(pt=e.querySelector("#pvp-boost-card"))==null||pt.addEventListener("click",()=>{q()}),e.querySelectorAll(".pvp-gc-mini").forEach(O=>{O.addEventListener("click",()=>{V(O.dataset.gcId,O.dataset.gcType)})}),(ut=e.querySelector("#pvp-action"))==null||ut.addEventListener("click",()=>{_?Y():b&&U()}),(ft=e.querySelector("#pvp-quit"))==null||ft.addEventListener("click",()=>{confirm("Quitter le match ? Vous perdrez par forfait.")&&k()}),(gt=e.querySelector("#pvp-view-opp"))==null||gt.addEventListener("click",()=>{P()}),(xt=e.querySelector("#pvp-toggle-history"))==null||xt.addEventListener("click",()=>{G()}),function(){const X=e.querySelector(".terrain-wrapper svg");X&&(X.removeAttribute("width"),X.removeAttribute("height"),X.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",X.setAttribute("viewBox","-26 -26 352 352"),X.setAttribute("preserveAspectRatio","xMidYMid meet"));const ee=e.querySelector(".match-screen");if(ee){const W=e.clientHeight;W>50&&(ee.style.height=W+"px",ee.style.maxHeight=W+"px",ee.style.overflow="hidden")}}(),u._timerInt&&(clearInterval(u._timerInt),u._timerInt=null),(_||b)&&!h){let O=30,X=!1;const ee=()=>document.getElementById("pvp-timer"),W=()=>{ee()&&(ee().textContent=O+"s",ee().style.color=X?"#ff4444":"#fff")};W(),u._timerInt=setInterval(()=>{O--,O<0?X?(clearInterval(u._timerInt),u._timerInt=null,k()):(X=!0,O=15,W()):W()},1e3)}}function m(){var b;const L=u["gcReady_"+n],j=u["gcReady_"+r];Array.isArray(u["gc_"+n])||(u["gc_"+n]=[]);let z=u["gc_"+n];const N=3;function D(C,E){const w=C._gcDef,B={purple:"linear-gradient(135deg,#3d0a7a,#7a28b8)",light_blue:"linear-gradient(135deg,#006080,#00bcd4)"},I={purple:"#9b59b6",light_blue:"#00bcd4"},R=B[w==null?void 0:w.color]||B.purple,S=I[w==null?void 0:w.color]||I.purple;return`<div class="pvp-gc-card" data-id="${C.id}"
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    </div>`,setTimeout(()=>{const E=document.getElementById("pvp-score-me"),w=document.getElementById("pvp-score-opp"),B=document.getElementById("pvp-midfield-result");E&&w&&(b?(E.style.fontSize="80px",E.style.color="#FFD700",w.style.opacity="0.25"):(w.style.fontSize="80px",w.style.color="#ff6b6b",E.style.opacity="0.25"));const I=u.p1Team.MIL||[],R=u.p2Team.MIL||[],S=z(I)+N(I),H=z(R)+N(R),K=S>=H?"p1":"p2";u[K+"Name"];let Q=u.boostValue;Q==null&&(Q=Kt(),u.boostValue=Q,u.boostOwner=K,u.boostUsed=!1),B&&(B.style.opacity="1",B.innerHTML=`<div style="font-size:20px;font-weight:900;margin-bottom:10px">
      ...
      </div>`,document.body.appendChild(z),(N=z.querySelector("#boost-picker-close"))==null||N.addEventListener("click",()=>z.remove()),z.querySelectorAll(".boost-pick-item").forEach(D=>{D.addEventListener("click",async()=>{const _=D.dataset.cid,b=j.find(w=>w.cardId===_);if(!b)return;const C=u[n+"Team"],E=(C[b._line]||[]).find(w=>w.cardId===_);E&&(E.boost=(E.boost||0)+u.boostValue),z.remove(),await x({[n+"Team"]:C,boostUsed:!0})})})}function G(){var z;const L=Array.isArray(u.log)?u.log:[],j=document.createElement("div");j.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column",j.innerHTML=`
      ...
      </div>`,document.body.appendChild(R),(S=R.querySelector("#pvp-gc-back"))==null||S.addEventListener("click",()=>R.remove()),(H=R.querySelector("#pvp-gc-use"))==null||H.addEventListener("click",async()=>{R.remove();const K=u["usedGc_"+n]||[];K.push(L),await x({["usedGc_"+n]:K})})}function Z(){var D;const L=u[n+"Score"],j=u[r+"Score"],z=L>j,N=L===j;e.style.overflow="hidden",e.innerHTML=`
    ...
    </div>`,(D=document.getElementById("pvp-home"))==null||D.addEventListener("click",()=>{$.removeChannel(v),le(),d("home")})}async function Y(){const L=(u["selected_"+n]||[]).map(N=>({...N,_line:N._role}));if(!L.length)return;const j=et(L,u.modifiers[n]||{});L.forEach(N=>{const D=(u[n+"Team"][N._role]||[]).find(_=>_.cardId===N.cardId);D&&(D.used=!0)});const z=Array.isArray(u.log)?u.log:[];z.push({type:"attack",text:`⚔️ ${u[n+"Name"]} attaque (${j.total})`,players:L,total:j.total,side:n}),await x({pendingAttack:{...j,players:L,side:n},["selected_"+n]:[],modifiers:{...u.modifiers,[n]:{}},phase:r+"-defense",log:z})}async function U(){const L=(u["selected_"+n]||[]).map(B=>({...B,_line:B._role})),j=tt(L,u.modifiers[n]||{});L.forEach(B=>{const I=(u[n+"Team"][B._role]||[]).find(R=>R.cardId===B.cardId);I&&(I.used=!0)});const z=it(u.pendingAttack.total,j.total,u.modifiers[n]||{}),N=u.pendingAttack.side,D=z==="attack",_=u[N+"Score"]+(D?1:0),b=N==="p1"?"p2":"p1",C=(u.round||0)+1,E=C>10,w=Array.isArray(u.log)?u.log:[];w.push({type:"defense",text:`🛡️ ${u[n+"Name"]} défend (${j.total})`,players:L,total:j.total,side:n}),w.push({type:D?"goal":"stop",text:D?`⚽ BUT de ${u[N+"Name"]} ! (${u.pendingAttack.total} vs ${j.total})`:`✋ Attaque stoppée (${u.pendingAttack.total} vs ${j.total})`}),await x({[N+"Score"]:_,["selected_"+n]:[],modifiers:{...u.modifiers,[n]:{}},pendingAttack:null,phase:E?"finished":b+"-attack",attacker:b,round:C,log:w}),E&&await $.from("matches").update({status:"finished"}).eq("id",i)}F()}const uo={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"};async function fo(e,t){const{state:i,toast:s}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement du marché...</div>',await st(e,t)}async function st(e,t){const{state:i,toast:s}=t,{data:c}=await $.from("market_listings").select(`id, price, status, listed_at, seller_id,
  ...
  </div>`}const J={user:null,profile:null,page:"home",params:{}};function Be(e,t="info",i=3e3){const s=document.getElementById("toast");s&&(s.textContent=e,s.className=`show ${t}`,clearTimeout(s._t),s._t=setTimeout(()=>{s.className=""},i))}function vo(e,t,i=""){document.getElementById("modal-title").textContent=e,document.getElementById("modal-body").innerHTML=t,document.getElementById("modal-footer").innerHTML=i,document.getElementById("modal-overlay").classList.remove("hidden")}function Je(){document.getElementById("modal-overlay").classList.add("hidden")}async function Fe(){if(!J.user)return;const{data:e}=await $.from("users").select("*").eq("id",J.user.id).single();e&&(J.profile=e)}const dt="mw_theme";function Pe(){return localStorage.getItem(dt)||"light"}function bo(e){var t;localStorage.setItem(dt,e),Ze(e),(t=J.profile)!=null&&t.id&&$.from("users").update({theme:e}).eq("id",J.profile.id).then(()=>{})}function Ze(e){document.documentElement.setAttribute("data-theme",e)}function De(e,t={}){J.page=e,J.params=t,Qt()}async function Qt(){const e=document.getElementById("page-content");if(!e)return;document.querySelectorAll(".bottom-nav a").forEach(s=>{s.classList.toggle("active",s.dataset.page===J.page)});const t=document.getElementById("nav-credits");t&&J.profile&&(t.textContent=`💰 ${(J.profile.credits||0).toLocaleString("fr")}`);const i={state:J,navigate:De,toast:Be,openModal:vo,closeModal:Je,refreshProfile:Fe};switch(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽</div>',J.page){case"home":await $t(e,i);break;case"collection":await fi(e,i);break;case"decks":await Ue(e,i);break;case"boosters":await zi(e,i);break;case"match":{(J.params&&J.params.matchMode||"vs_ai_easy")==="random"?await lo(e,i):await Vi(e,i);break}case"market":await fo(e,i);break;case"rankings":await yo(e,i);break;case"matches":await ho(e,i);break;default:await $t(e,i)}}function wo(){const e=document.getElementById("app"),t=J.profile;if(!t)return;const i="/manager-wars/icons/";e.innerHTML=`
    <nav class="top-nav">
