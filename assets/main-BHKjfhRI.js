import{s as _,F as oi,h as zt,j as Mi,l as Ze,i as rn,k as an,b as sn}from"./formation-links-CEzN0rcz.js";function xi(e,{navigate:t,toast:n}){e.innerHTML=`
  ...
  `;function l(){var k;const i=document.getElementById("logo-preview"),r=document.getElementById("logo-initials"),u=((k=document.getElementById("setup-club"))==null?void 0:k.value)||"MW",x=u.trim().split(" ").filter(Boolean),g=x.length>=2?(x[0][0]+x[1][0]).toUpperCase():u.slice(0,2).toUpperCase();i&&(i.style.background=s,i.style.borderColor=d),r&&(r.textContent=g,r.style.color=d)}document.getElementById("color1").addEventListener("input",i=>{d=i.target.value,document.getElementById("swatch1").style.background=d,l()}),document.getElementById("color2").addEventListener("input",i=>{s=i.target.value,document.getElementById("swatch2").style.background=s,l()});function c(i){document.querySelectorAll(".setup-step").forEach(r=>r.classList.remove("active")),document.getElementById(`step-${i}`).classList.add("active"),document.getElementById("step-num").textContent=i,document.getElementById("progress-fill").style.width=`${Math.round(i/3*100)}%`,i===3&&l()}document.getElementById("step1-next").addEventListener("click",async()=>{const i=document.getElementById("setup-pseudo").value.trim(),r=document.getElementById("step1-error");if(r.textContent="",i.length<3){r.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:u}=await _.from("users").select("id").eq("pseudo",i).maybeSingle();if(u){r.textContent="Ce pseudo est déjà pris.";return}c(2)}),document.getElementById("step2-back").addEventListener("click",()=>c(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const i=document.getElementById("setup-club").value.trim(),r=document.getElementById("step2-error");if(r.textContent="",i.length<2){r.textContent="Nom trop court (min. 2 caractères).";return}const{data:u}=await _.from("users").select("id").eq("club_name",i).maybeSingle();if(u){r.textContent="Ce nom de club est déjà pris.";return}c(3)}),document.getElementById("step3-back").addEventListener("click",()=>c(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const i=document.getElementById("setup-pseudo").value.trim(),r=document.getElementById("setup-club").value.trim(),u=document.getElementById("step3-error"),x=document.getElementById("step3-finish");u.textContent="",x.disabled=!0,x.textContent="Création en cours…";try{const{error:g}=await _.from("users").insert({id:t.user.id,pseudo:i,club_name:r,club_color1:d,club_color2:s,credits:1e4});if(g)throw g;await ln(t.user.id),await p(),a(`Bienvenue ${i} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(g){u.textContent=g.message,x.disabled=!1,x.textContent="🚀 Créer mon profil !"}})}async function ln(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await _.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(n){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",n)}}const cn="modulepreload",pn=function(e){return"/manager-wars/"+e},yi={},un=function(t,n,a){let p=Promise.resolve();if(n&&n.length>0){document.getElementsByTagName("link");const s=document.querySelector("meta[property=csp-nonce]"),l=(s==null?void 0:s.nonce)||(s==null?void 0:s.getAttribute("nonce"));p=Promise.allSettled(n.map(c=>{if(c=pn(c),c in yi)return;yi[c]=!0;const i=c.endsWith(".css"),r=i?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${c}"]${r}`))return;const u=document.createElement("link");if(u.rel=i?"stylesheet":cn,i||(u.as="script"),u.crossOrigin="",u.href=c,l&&u.setAttribute("nonce",l),document.head.appendChild(u),i)return new Promise((x,g)=>{u.addEventListener("load",x),u.addEventListener("error",()=>g(new Error(`Unable to preload CSS for ${c}`)))})}))}function d(s){const l=new Event("vite:preloadError",{cancelable:!0});if(l.payload=s,window.dispatchEvent(l),!l.defaultPrevented)throw s}return p.then(s=>{for(const l of s||[])l.status==="rejected"&&d(l.reason);return t().catch(d)})},Tt="#1A6B3C",It="#cc2222",fn="#D4A017",hi="#888";async function gn(e,t){const{state:n,toast:a}=t;e.innerHTML=`
    ...
    </div>`,await Si(n,a,t),document.getElementById("btn-add-friend").addEventListener("click",()=>xn(n,a)),document.getElementById("btn-accept-friend").addEventListener("click",()=>Ci(n,a,null,t))}async function Si(e,t,n={}){const{navigate:a}=n,p=e.user.id,{data:d,error:s}=await _.from("friendships").select("id, requester_id, addressee_id").eq("status","accepted").or(`requester_id.eq.${p},addressee_id.eq.${p}`),{count:l}=await _.from("friendships").select("id",{count:"exact",head:!0}).eq("addressee_id",p).eq("status","pending"),c=document.getElementById("pending-badge");c&&(l>0?(c.style.display="flex",c.textContent=l):c.style.display="none");const i=document.getElementById("friends-list");if(!i)return;if(s){console.error("[Friends] Erreur:",s),i.innerHTML=`<div style="color:${It};text-align:center;padding:16px">Erreur chargement : ${s.message}</div>`;return}const r=(d||[]).map(g=>g.requester_id===p?g.addressee_id:g.requester_id);let u={};if(r.length){const{data:g}=await _.from("users").select("id, pseudo, club_name, last_seen_at, club_color1, club_color2").in("id",r);(g||[]).forEach(k=>{u[k.id]=k})}const x=(d||[]).map(g=>({friendshipId:g.id,friend:u[g.requester_id===p?g.addressee_id:g.requester_id]||{pseudo:"?"}}));if(!x.length){i.innerHTML=`
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      ${x.map(({friendshipId:g,friend:k})=>mn(k,g)).join("")}
    </div>`,i.querySelectorAll("[data-stats]").forEach(g=>{g.addEventListener("click",()=>yn(e,g.dataset.stats,g.dataset.friendName))}),i.querySelectorAll("[data-match]").forEach(g=>{g.addEventListener("click",()=>{const k=g.dataset.friendId,o=g.dataset.friendName;console.log("[Friends] clic match",{fid:k,fname:o,hasNavigate:typeof a}),typeof a=="function"?a("match",{matchMode:"friend",friendId:k,friendName:o}):t("Erreur navigation","error")})})}function mn(e,t){const n=e.club_name||e.pseudo||"?",a=e.pseudo||"",p=(a||n).slice(0,2).toUpperCase(),d=e.club_color2||Tt,s=e.club_color1||"#ffffff",l=e.last_seen_at?new Date(e.last_seen_at):null,c=l&&Date.now()-l.getTime()<3*60*1e3;return`
    ...
    </div>`}function xn(e,t){const n=ri();n.innerHTML=`
    <div class="popup-box">
    ...
    ${ai()}`,document.body.appendChild(i);const u=()=>i.remove();i.querySelector("#pending-close").addEventListener("click",u),i.addEventListener("click",x=>{x.target===i&&u()}),i.querySelectorAll("[data-accept]").forEach(x=>{x.addEventListener("click",async()=>{const{error:g}=await _.from("friendships").update({status:"accepted"}).eq("id",x.dataset.accept);if(g){t("Erreur : "+g.message,"error");return}x.closest("div[style]").remove(),t("✅ Ami accepté !","success"),Si(e,t,a),n&&n()})}),i.querySelectorAll("[data-decline]").forEach(x=>{x.addEventListener("click",async()=>{await _.from("friendships").delete().eq("id",x.dataset.decline),x.closest("div[style]").remove(),t("Demande refusée","info"),n&&n()})})}async function yn(e,t,n){const a=e.user.id,[p,d]=[a,t].sort(),s=a===p,{data:l}=await _.from("friend_match_stats").select("*").eq("player1_id",p).eq("player2_id",d).single(),c=e.profile.club_name||e.profile.pseudo||"Moi",i=l||{},r=s?i.wins_p1||0:i.wins_p2||0,u=s?i.wins_p2||0:i.wins_p1||0,x=i.draws||0,g=s?i.goals_p1||0:i.goals_p2||0,k=s?i.goals_p2||0:i.goals_p1||0,o=s?i.gc_used_p1||0:i.gc_used_p2||0,$=s?i.gc_used_p2||0:i.gc_used_p1||0,z=i.matches_total||0,M=(U,F,j,R=Tt,ie=It)=>`
    ...
    <div class="popup-box" style="max-width:380px">
      ...
      ${z===0?`
        ...
        ${M("Défaites",u,r)}
        ${M("Buts marqués",g,k)}
        ${M("Buts encaissés",k,g,It,Tt)}
        ${M("GC utilisés ⚡",o,$,"#7a28b8","#7a28b8")}
  ...
  `,e.querySelectorAll("[data-nav]").forEach(l=>{l.addEventListener("click",c=>{c.preventDefault(),n(l.dataset.nav)})}),(d=document.getElementById("nav-rankings"))==null||d.addEventListener("click",()=>n("rankings")),(s=document.getElementById("nav-matches"))==null||s.addEventListener("click",()=>n("matches")),e.querySelectorAll("[data-action]").forEach(l=>{l.addEventListener("click",()=>{l.classList.add("tapped"),setTimeout(()=>l.classList.remove("tapped"),200);const c=l.dataset.action;if(c==="match-ai"){$n(n);return}if(c==="match-random"){n("match",{matchMode:"random"});return}if(c==="match-friend"){n("friends");return}a("Bientôt disponible","info")})}),document.getElementById("logout-btn").addEventListener("click",async()=>{await _.auth.signOut(),window.location.reload()}),Bi(t,a),_n(t,a,n),ji(t,a,n))}async function ji(e,t,n){const a=document.getElementById("ongoing-match-banner");if(!a)return;const p=e.profile.id,{data:d}=await _.from("matches").select("id, home_id, away_id, status, mode").eq("status","active").or(`home_id.eq.${p},away_id.eq.${p}`).order("created_at",{ascending:!1});if(!(d!=null&&d.length)){a.innerHTML="";return}const s=d.map(c=>c.home_id===p?c.away_id:c.home_id).filter(Boolean);let l={};if(s.length){const{data:c}=await _.from("users").select("id, pseudo, club_name").in("id",s);(c||[]).forEach(i=>{l[i.id]=i.club_name||i.pseudo})}a.innerHTML=d.map(c=>{const i=c.home_id===p?c.away_id:c.home_id,r=l[i]||"Adversaire";return`
      ...
      </div>`}).join(""),a.querySelectorAll("[data-resume]").forEach(c=>{c.addEventListener("click",async()=>{const{resumePvpMatch:i}=await un(async()=>{const{resumePvpMatch:u}=await Promise.resolve().then(()=>$o);return{resumePvpMatch:u}},void 0),r=document.getElementById("page-content")||document.getElementById("app");i(r,{state:e,navigate:n,toast:t,openModal:null,closeModal:null,refreshProfile:null},c.dataset.resume)})}),a.querySelectorAll("[data-abandon]").forEach(c=>{c.addEventListener("click",()=>{wn(async()=>{await vn(c.dataset.abandon,c.dataset.opp,p),t("Match abandonné (défaite 3-0)","info"),ji(e,t,n)})})})}async function vn(e,t,n){const{data:a}=await _.from("matches").select("home_id, away_id, game_state").eq("id",e).single();if(!a)return;const p=a.home_id===n,d=p?0:3,s=p?3:0,l=a.game_state||{};l.p1Score=d,l.p2Score=s,l.phase="finished",l.forfeit=!0,await _.from("matches").update({status:"finished",forfeit:!0,winner_id:t,home_score:d,away_score:s,game_state:l}).eq("id",e)}function wn(e){const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",t.innerHTML=`
    ...
    </div>`,document.getElementById("friend-req-btn").addEventListener("click",()=>{Ci(e,t,()=>Bi(e,t))})}function $n(e){const t=[{mode:"vs_ai_easy",label:"Facile",sub:"Gain 500 cr.",icon:"🟢"},{mode:"vs_ai_medium",label:"Moyen",sub:"Gain 1 000 cr.",icon:"🟡"},{mode:"vs_ai_hard",label:"Difficile",sub:"Gain 1 500 cr.",icon:"🟠"},{mode:"vs_ai_club",label:"Club",sub:"Gain 2 500 cr.",icon:"🔴"}],n=document.createElement("div");n.className="modal-overlay",n.style.zIndex="2000",n.innerHTML=`
  ...
  </div>`}function _i(e){const t=e.job||"ATT",n=Lt(e,t),a=si[e.country_code]||e.country_code||"";return`
  <div style="
    width:140px;border-radius:12px;padding:6px;
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
  ...
  </div>`}async function In(e,t){const{state:n,navigate:a,toast:p,openModal:d,closeModal:s}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:l}=await _.from("cards").select(`id, card_type, current_note, gc_type, formation, is_for_sale, sale_price,
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),i=(l||[]).filter(O=>O.card_type==="player"&&O.player),r=(l||[]).filter(O=>O.card_type==="game_changer"),u=(l||[]).filter(O=>O.card_type==="formation"),{data:x}=await _.from("gc_definitions").select("name,gc_type,color,effect,image_url"),g={};(x||[]).forEach(O=>{g[O.name]=O});const k=Object.keys(oi),o=Object.keys(Ge),$={};i.forEach(O=>{const L=O.player.id;$[L]=($[L]||0)+1}),new Set(Object.keys($).map(O=>String(O)));const z=new Set(u.map(O=>O.formation)),M=new Set(r.map(O=>O.gc_type));let B="player",U="Tous",F="",j=!1;function R(){return[...i].sort((O,L)=>{const N=jt.indexOf(O.player.job),q=jt.indexOf(L.player.job);return N!==q?N-q:(O.player.surname_encoded||"").localeCompare(L.player.surname_encoded||"")})}function ie(){return[...c||[]].sort((O,L)=>{const N=jt.indexOf(O.job),q=jt.indexOf(L.job);return N!==q?N-q:(O.surname_encoded||"").localeCompare(L.surname_encoded||"")})}function ce(){return R().filter(O=>{const L=O.player,N=U==="Tous"||L.job===U,q=!F||`${L.firstname} ${L.surname_encoded}`.toLowerCase().includes(F);return N&&q})}function oe(){return ie().filter(O=>{const L=U==="Tous"||O.job===U,N=!F||`${O.firstname} ${O.surname_encoded}`.toLowerCase().includes(F);return L&&N})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{j=!j,S(),Y()})))}function Y(){const O=document.getElementById("col-grid");O&&(B==="player"?he(O):B==="formation"?xe(O):ge(O))}function ee(O,L,N,q,V){V=V||"#7a28b8";const ne=document.getElementById("col-grid"),te=document.getElementById("col-big");if(!ne||!te)return;var $e=0;function Ee(){te.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+L(O[$e])+"</div>";var ke=te.querySelector("[data-card-id],[data-form-id],[data-gc-id]");ke&&ke.addEventListener("click",function(){q(O[$e])}),requestAnimationFrame(function(){var _e=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!_e||!te)){var Ae=te.clientHeight-8,Te=te.clientWidth-24,qe=_e.offsetHeight,Ne=_e.offsetWidth;if(qe>0&&Ne>0&&Ae>40){var He=Math.min(Ae/qe,Te/Ne,1);_e.style.transform="scale("+He.toFixed(3)+")",_e.style.transformOrigin="top center"}}}),ne.innerHTML=O.map(function(_e,Ae){return'<div class="col-mini-item" data-idx="'+Ae+'" style="flex-shrink:0;cursor:pointer;border-radius:8px;border:1.5px solid '+(Ae===$e?V:"transparent")+';transition:border-color .15s;overflow:hidden">'+N(_e,Ae===$e)+"</div>"}).join(""),ne.querySelectorAll(".col-mini-item").forEach(function(_e){_e.addEventListener("click",function(){$e=Number(_e.dataset.idx),Ee(),_e.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}Ee()}function D(O){var L=.54,N=Math.round(140*L),q=Math.round(310*L),V;if(!O||O._fake){var ne=O?O.player:null;if(!ne)return"";V=_i(ne)}else V=Vt(O,"");return'<div style="width:'+N+"px;height:"+q+'px;overflow:hidden;position:relative;flex-shrink:0"><div style="transform:scale('+L+');transform-origin:top left;position:absolute;top:0;left:0;pointer-events:none">'+V+"</div></div>"}function Q(O,L,N){L=L||100,N=N||140;var q=zt[O]||{},V={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},ne=Math.max(3,Math.round(L*.06)),te=Object.entries(q).map(function(Ee){var ke=Ee[0],_e=Ee[1],Ae=ke.replace(/\d+$/,""),Te=V[Ae]||"#888",qe=Math.round(_e.x*L),Ne=Math.round(_e.y*N);return'<circle cx="'+qe+'" cy="'+Ne+'" r="'+ne+'" fill="'+Te+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),$e=Math.max(1,Math.round(L/50));return'<svg viewBox="0 0 '+L+" "+N+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+L+'" height="'+N+'" fill="#1A6B3C"/><rect x="'+Math.round(L*.2)+'" y="'+Math.round(N*.02)+'" width="'+Math.round(L*.6)+'" height="'+Math.round(N*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+$e+'"/><line x1="0" y1="'+Math.round(N*.5)+'" x2="'+L+'" y2="'+Math.round(N*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+$e+'"/><ellipse cx="'+Math.round(L*.5)+'" cy="'+Math.round(N*.5)+'" rx="'+Math.round(L*.18)+'" ry="'+Math.round(N*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+$e+'"/><rect x="'+Math.round(L*.2)+'" y="'+Math.round(N*.82)+'" width="'+Math.round(L*.6)+'" height="'+Math.round(N*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+$e+'"/>'+te+"</svg>"}function ae(O,L,N){var q=N>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+N+"</div>":"",V=L?'data-form-id="'+L.id+'"':"",ne=O.length>7?14:O.length>5?16:19,te=!!L;return"<div "+V+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(te?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(te?"":"filter:grayscale(1);opacity:0.5")+'">'+q+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(te?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+ne+"px;font-weight:900;color:"+(te?"#1A6B3C":"#aaa")+';line-height:1">'+O+'</div></div><div style="flex:1;overflow:hidden;background:'+(te?"#1A6B3C":"#ccc")+'">'+Q(O,140,220)+"</div></div>"}function pe(O,L){var N=.54,q=Math.round(140*N),V=Math.round(305*N),ne=Math.round(V*.22),te=V-ne,$e=O.length>7?5:7,Ee=Q(O,q,te),ke=L?"1.5px solid #2a7a40":"1px solid #ddd",_e=L?"":"filter:grayscale(1);opacity:0.45;",Ae=L?"#1A6B3C":"#bbb",Te="#fff";return'<div style="width:'+q+"px;height:"+V+"px;border-radius:6px;border:"+ke+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+_e+'"><div style="height:'+ne+"px;background:"+Ae+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+$e+"px;font-weight:900;color:"+Te+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(q-4)+'px">'+O+'</span></div><div style="height:'+te+'px;overflow:hidden;flex-shrink:0">'+Ee+"</div></div>"}function he(O){if(j){const L=oe();if(!L.length){O.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const N=L.map(q=>i.find(V=>V.player.id===q.id)||{_fake:!0,player:q,id:"fake-"+q.id});ee(N,q=>q._fake?_i(q.player):Vt(q,""),q=>q._fake?D({player:q.player,id:"x",_fake:!0}):D(q),q=>{q._fake||$i(q,i,$,t)},"#1A6B3C")}else{const L=ce();if(!L.length){O.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const N={};L.forEach(V=>{const ne=V.player.id;N[ne]||(N[ne]=[]),N[ne].push(V)});const q=Object.values(N).map(V=>Tn(V));ee(q,V=>{const ne=$[V.player.id]||1,te=ne>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${ne}</div>`:"",Ee=i.filter(ke=>ke.player.id===V.player.id&&ke.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Vt(V,te+Ee)},V=>D(V),V=>$i(V,i,$,t),"#1A6B3C")}}function xe(O){const L=j?k:[...z];if(!L.length){O.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const N=L.map(q=>({formation:q,card:u.find(V=>V.formation===q)||null,owned:z.has(q)}));ee(N,({formation:q,card:V,owned:ne})=>ae(q,ne?V:null,ne?u.filter(te=>te.formation===q).length:0),({formation:q,owned:V})=>pe(q,V),({card:q,owned:V})=>{V&&q&&zn(q,u,t,d)},"#1A6B3C")}function ge(O){const L=Object.keys(g),N=j?L.length?L:o:[...M];if(!N.length){O.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const q=N.map(V=>({type:V,gc:Ge[V]||{icon:"⚡",desc:""},def:g[V]||null,owned:M.has(V),card:r.find(ne=>ne.gc_type===V)||null}));ee(q,({type:V,gc:ne,def:te,owned:$e,card:Ee})=>{const ke=$e?r.filter(m=>m.gc_type===V).length:0,_e=ke>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${ke}</div>`:"",Ae=(te==null?void 0:te.gc_type)==="ultra_game_changer",Te={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},qe={purple:"#b06ce0",light_blue:"#00d4ef"},Ne=Te[te==null?void 0:te.color]||Te.purple,He=qe[te==null?void 0:te.color]||qe.purple,T=(te==null?void 0:te.effect)||ne.desc||"",f=te!=null&&te.image_url?`/manager-wars/icons/${te.image_url}`:null;return $e&&Ee?`<div data-gc-id="${Ee.id}" data-gc-type="${V}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${He};background:${Ne};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${He}66;cursor:pointer">
          ${_e}
        ...
        </div>`},({type:V,gc:ne,def:te,owned:$e})=>$e?(()=>{const Ee=Math.round(75.60000000000001),ke=Math.round(310*.54),_e=Math.round(ke*.65),Ae=Math.round(ke*.18),Te={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},qe={purple:"#9b59b6",light_blue:"#00bcd4"},Ne=Te[te==null?void 0:te.color]||Te.purple,He=qe[te==null?void 0:te.color]||qe.purple,T=te!=null&&te.image_url?`/manager-wars/icons/${te.image_url}`:null;return`<div style="width:${Ee}px;height:${ke}px;border-radius:8px;background:${Ne};border:1px solid ${He};display:flex;flex-direction:column;overflow:hidden"><div style="height:${Ae}px;background:rgba(255,255,255,0.15);display:flex;align-items:center;justify-content:center"><span style="font-size:7px;font-weight:900;color:#fff;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${Ee-6}px">${V}</span></div><div style="height:${_e}px;display:flex;align-items:center;justify-content:center">${T?`<img src="${T}" style="max-width:${Ee-8}px;max-height:${_e-4}px;object-fit:contain">`:`<span style="font-size:24px">${ne.icon}</span>`}</div><div style="flex:1;display:flex;align-items:center;justify-content:center"><span style="font-size:7px;color:rgba(255,255,255,0.7);text-align:center;padding:0 2px">${((te==null?void 0:te.effect)||ne.desc||"").slice(0,20)}</span></div></div>`})():(()=>{const Ee=Math.round(75.60000000000001),ke=Math.round(310*.54);return`<div style="width:${Ee}px;height:${ke}px;border-radius:8px;background:#eee;border:1px solid #ddd;display:flex;flex-direction:column;align-items:center;justify-content:center;filter:grayscale(1);opacity:0.45"><span style="font-size:22px">${ne.icon}</span><span style="font-size:7px;color:#aaa;margin-top:4px;text-align:center;padding:0 4px">${V}</span></div>`})(),({type:V,owned:ne,def:te})=>{ne&&Ln(V,te,d)},"#7a28b8")}e.querySelectorAll(".col-tab-btn").forEach(O=>{O.addEventListener("click",()=>{B=O.dataset.tab,U="Tous",F="",j=!1,e.querySelectorAll(".col-tab-btn").forEach(L=>{const N=L.dataset.tab===B;L.style.borderBottomColor=N?"var(--green)":"transparent",L.style.color=N?"var(--green)":"var(--gray-600)"}),S(),Y()})}),S(),Y()}function Ln(e,t,n){const a=Ge[e]||{icon:"⚡",desc:"Effet spécial."},p=(t==null?void 0:t.gc_type)==="ultra_game_changer",d={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},s={purple:"#b06ce0",light_blue:"#00d4ef"},l=d[t==null?void 0:t.color]||d.purple,c=s[t==null?void 0:t.color]||s.purple,i=(t==null?void 0:t.name)||e,r=(t==null?void 0:t.effect)||a.desc,u=t!=null&&t.image_url?`/manager-wars/icons/${t.image_url}`:null;n("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Bt=1e3;function zn(e,t,n,a){var k,o,$;const{state:p,toast:d,closeModal:s,navigate:l,refreshProfile:c}=n,i=e.formation,r={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function u(){const z=zt[i]||{},M=oi[i]||[],B=290,U=360,F=20;function j(ie){const ce=z[ie];return ce?{x:ce.x*B,y:ce.y*U}:null}let R=`<svg width="${B}" height="${U}" viewBox="0 0 ${B} ${U}" xmlns="http://www.w3.org/2000/svg">`;for(const[ie,ce]of M){const oe=j(ie),S=j(ce);!oe||!S||(R+=`<line x1="${oe.x}" y1="${oe.y}" x2="${S.x}" y2="${S.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const ie of Object.keys(z)){const ce=j(ie);if(!ce)continue;const oe=ie.replace(/\d+/,""),S=r[oe]||"#555";R+=`<circle cx="${ce.x}" cy="${ce.y}" r="${F}" fill="${S}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,R+=`<text x="${ce.x}" y="${ce.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${oe}</text>`}return R+="</svg>",R}const x=t.filter(z=>z.formation===i);x.length;const g=!e.is_for_sale;a(`Formation ${i}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(k=document.getElementById("direct-sell-form-btn"))==null||k.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${i} pour ${Bt.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const z=x.find(B=>!B.is_for_sale)||x[0];if(!z){d("Aucune carte à vendre","error");return}await _.from("market_listings").delete().eq("card_id",z.id),await _.from("transfer_history").delete().eq("card_id",z.id);const{error:M}=await _.from("cards").delete().eq("id",z.id);if(M){d(M.message,"error");return}await _.from("users").update({credits:(p.profile.credits||0)+Bt}).eq("id",p.profile.id),await c(),d(`+${Bt.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),s(),l("collection")}),(o=document.getElementById("market-sell-form-btn"))==null||o.addEventListener("click",async()=>{const z=parseInt(document.getElementById("sell-price-form").value);if(!z||z<1){d("Prix invalide","error");return}await _.from("cards").update({is_for_sale:!0,sale_price:z}).eq("id",e.id),await _.from("market_listings").insert({seller_id:p.profile.id,card_id:e.id,price:z}),d("Carte mise en vente sur le marché !","success"),s(),l("collection")}),($=document.getElementById("cancel-sell-form-btn"))==null||$.addEventListener("click",async()=>{await _.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await _.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),d("Annonce retirée","success"),s(),l("collection")})}async function $i(e,t,n,a){var Q,ae,pe,he,xe,ge,O;const{state:p,toast:d,openModal:s,closeModal:l,navigate:c,refreshProfile:i}=a,r=e.player,u=t.filter(L=>L.player.id===r.id),x=u.length,g=Math.max(Number(r.note_g)||0,Number(r.note_d)||0,Number(r.note_m)||0,Number(r.note_a)||0),k=r.rarity||"normal",{data:o}=await _.from("sell_price_configs").select("*").eq("rarity",k).lte("note_min",g).gte("note_max",g).order("note_min",{ascending:!1}).limit(1),$=((Q=o==null?void 0:o[0])==null?void 0:Q.price)??An[k]??1e3,z=r.rarity!=="legende",M=Gi(r),B=(r.rarity==="pepite"||r.rarity==="papyte")&&e.current_note!=null?e.current_note:Lt(r,r.job),U=r.rarity!=="pepite"&&r.rarity!=="papyte"&&r.job2?Lt(r,r.job2):null,F=Et[r.job]||"#1A6B3C",j=r.job2?Et[r.job2]:null,R=Di[r.rarity]||"#ccc",ie=si[r.country_code]||r.country_code||"",ce=u.map(L=>L.id);let oe={};if(ce.length){const{data:L}=await _.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",ce).order("transferred_at",{ascending:!0});(L||[]).forEach(N=>{oe[N.card_id]||(oe[N.card_id]=[]),oe[N.card_id].push(N)})}const S=L=>{const N=L.transferred_at?new Date(L.transferred_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"2-digit",year:"numeric"}):"",q=L.source==="booster"?"Booster":L.price?L.price.toLocaleString("fr"):"—";return`
    <div style="display:flex;justify-content:space-between;align-items:flex-start;background:#f9f9f9;border-radius:8px;padding:8px 12px;gap:8px">
      <div style="flex:1;min-width:0">
        <div style="font-size:13px;font-weight:700;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${L.club_name}</div>
        <div style="font-size:11px;color:var(--gray-600)">(${L.manager_name})</div>
      </div>
      <div style="text-align:right;flex-shrink:0">
        <div style="font-size:13px;font-weight:700;color:${L.source==="booster"?"var(--gray-600)":"var(--yellow)"}">${q}</div>
    ...
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${u.map((L,N)=>{const q=oe[L.id]||[],V=L.is_for_sale,ne=q.length?q[q.length-1]:null,$e=(r.rarity==="pepite"||r.rarity==="papyte")&&L.current_note!=null?` (☆${L.current_note})`:"";return`
            <div data-card-id="${L.id}" data-card-idx="${N}" class="exemplaire-row"
              ...
              <div style="display:flex;align-items:center;gap:10px">
                ...
                <div style="flex:1;min-width:0">
                  <div style="font-size:12px;font-weight:700">Exemplaire ${N+1}${$e}${V?" 🏷️ En vente":""}</div>
                  ${ne?`<div style="font-size:11px;color:#888;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">
                    ${ne.club_name} · ${ne.source==="booster"?"Booster":ne.price?ne.price.toLocaleString("fr")+" cr.":"—"}
                  </div>`:""}
                </div>
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",r.note_g],["DEF",r.note_d],["MIL",r.note_m],["ATT",r.note_a]].map(([L,N])=>{const q=Et[L],V=Number(N)||0;return`<div style="display:flex;flex-direction:column;align-items:center;gap:2px">
    ...
    ${Y}
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(he=document.getElementById("close-detail"))==null||he.addEventListener("click",l);let ee=new Set;const D=()=>{const L=ee.size,N=document.getElementById("sell-action-panel");N&&(N.style.display=L>0?"block":"none",document.getElementById("sell-selected-count").textContent=L,document.getElementById("sell-direct-total").textContent=(L*$).toLocaleString("fr")+" cr.")};document.querySelectorAll(".expl-check").forEach(L=>{L.addEventListener("change",()=>{const N=L.dataset.id;L.checked?ee.add(N):ee.delete(N);const q=L.closest(".exemplaire-row");q&&(q.style.borderColor=L.checked?"#1A6B3C":"#eee"),D()})}),document.querySelectorAll(".exemplaire-row").forEach(L=>{L.addEventListener("click",N=>{if(N.target.closest("button")||N.target.tagName==="INPUT")return;const q=L.querySelector(".expl-check");q&&!q.disabled&&(q.checked=!q.checked,q.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(L=>{L.addEventListener("click",N=>{N.stopPropagation();const q=document.querySelector(`.expl-hist[data-hist="${L.dataset.idx}"]`);q&&(q.style.display=q.style.display==="none"?"flex":"none")})}),(xe=document.getElementById("direct-sell-btn"))==null||xe.addEventListener("click",async()=>{const L=[...ee];if(!L.length)return;const N=L.length*$;if(!confirm(`Vendre ${L.length} carte${L.length>1?"s":""} ${r.surname_encoded} pour ${N.toLocaleString("fr")} crédits ? Action irréversible.`))return;await _.from("market_listings").delete().in("card_id",L),await _.from("transfer_history").delete().in("card_id",L);const{error:q}=await _.from("cards").delete().in("id",L);if(q){d(q.message,"error");return}await _.from("users").update({credits:(p.profile.credits||0)+N}).eq("id",p.profile.id),await i();const V=document.getElementById("nav-credits");V&&(V.textContent=`💰 ${(p.profile.credits||0).toLocaleString("fr")}`),d(`+${N.toLocaleString("fr")} crédits ! ${L.length} carte${L.length>1?"s":""} vendue${L.length>1?"s":""}.`,"success"),l(),c("collection")}),(ge=document.getElementById("market-sell-btn"))==null||ge.addEventListener("click",async()=>{var te;const L=[...ee];if(!L.length){d("Sélectionne au moins un exemplaire","warning");return}const N=parseInt((te=document.getElementById("sell-market-price"))==null?void 0:te.value);if(!N||N<1){d("Prix invalide","error");return}const{error:q}=await _.from("cards").update({is_for_sale:!0,sale_price:N}).in("id",L);if(q){d(q.message,"error");return}const V=L.map($e=>({seller_id:p.profile.id,card_id:$e,price:N,status:"active"})),{error:ne}=await _.from("market_listings").insert(V);ne&&console.warn("[Market] insert listings:",ne.message),d(`${L.length} carte${L.length>1?"s":""} mise${L.length>1?"s":""} en vente à ${N.toLocaleString("fr")} cr. chacune !`,"success"),l(),c("collection")}),(O=document.getElementById("cancel-sell-btn"))==null||O.addEventListener("click",async()=>{await _.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await _.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),d("Annonce retirée","success"),l(),c("collection")})}const Gt={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},rt={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function Ut(e){const t=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const n=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${n}.png`}function Ni(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function Pi(e){var n;const t="https://fcnwclxlkytdfjotqkta.supabase.co";return(n=e==null?void 0:e.clubs)!=null&&n.logo_url?e.clubs.logo_url.startsWith("http")?e.clubs.logo_url:`${t}/storage/v1/object/public/assets/clubs/${e.clubs.logo_url}`:null}function Mn(e,t=44,n=58){var g;const a=e?Ut(e):null,p=e?Pi(e):null,d=Ni(e==null?void 0:e.country_code),s=(e==null?void 0:e.job)||"MIL",l=rt[s]||"#555",c={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",i=Number(s==="GK"?e==null?void 0:e.note_g:s==="DEF"?e==null?void 0:e.note_d:s==="MIL"?e==null?void 0:e.note_m:e==null?void 0:e.note_a)||0,r=Math.round(n*.19),u=Math.round(n*.25),x=n-r-u;return e?`<div style="width:${t}px;height:${n}px;border-radius:5px;border:2px solid ${c};background:${l};position:relative;overflow:hidden;flex-shrink:0">
  ...
  </div>`,document.getElementById("new-deck-btn").addEventListener("click",async()=>{const s=prompt("Nom du deck :",`Deck ${((d==null?void 0:d.length)||0)+1}`);if(!s)return;const{data:l,error:c}=await _.from("decks").insert({owner_id:n.profile.id,name:s}).select().single();if(c){p(c.message,"error");return}p("Deck créé !","success"),ki(l.id,e,t)}),e.querySelectorAll("[data-open-deck]").forEach(s=>{s.addEventListener("click",()=>ki(s.dataset.openDeck,e,t))}),e.querySelectorAll("[data-rename]").forEach(s=>{s.addEventListener("click",async()=>{const l=prompt("Nouveau nom :",s.dataset.name);if(!l||l===s.dataset.name)return;const{error:c}=await _.from("decks").update({name:l}).eq("id",s.dataset.rename);if(c){p(c.message,"error");return}p("Deck renommé !","success"),Xt(e,t)})}),e.querySelectorAll("[data-delete]").forEach(s=>{s.addEventListener("click",async()=>{if(!confirm(`Supprimer le deck "${s.dataset.name}" ? Cette action est irréversible.`))return;await _.from("deck_cards").delete().eq("deck_id",s.dataset.delete);const{error:l}=await _.from("decks").delete().eq("id",s.dataset.delete);if(l){p(l.message,"error");return}p("Deck supprimé.","success"),Xt(e,t)})})}async function ki(e,t,n){const{state:a,toast:p}=n;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:d}=await _.from("decks").select("*").eq("id",e).single(),{data:s}=await _.from("cards").select(`id, card_type, formation,
      player:players(id, firstname, surname_encoded, country_code, club_id, job, job2,
        note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length,
        clubs(encoded_name, logo_url))`).eq("owner_id",a.profile.id),l=(s||[]).filter(g=>g.card_type==="player"&&g.player),c=(s||[]).filter(g=>g.card_type==="formation"),i=c.map(g=>g.formation).filter(Boolean),{data:r}=await _.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",e);let u=d.formation||"4-4-2";i.length>0&&!i.includes(u)&&(u=i[0]);const x={deckId:e,name:d.name,formation:u,formationCardId:d.formation_card_id,slots:{},subs:[],playerCards:l,formationCards:c,availableFormations:i};(r||[]).forEach(g=>{g.is_starter?x.slots[g.position]=g.card_id:x.subs.includes(g.card_id)||x.subs.push(g.card_id)}),pt(t,x,n)}function pt(e,t,n){var c;const{navigate:a}=n;Gt[t.formation];const p=Ei(t.formation),d=p.filter(i=>t.slots[i]).length,s=t.availableFormations.length>0?t.availableFormations:Object.keys(Gt),l=t.subs.map(i=>t.playerCards.find(r=>r.id===i)).filter(Boolean);[...Object.values(t.slots),...t.subs],e.innerHTML=`
  <div style="height:100%;overflow-y:auto;padding-bottom:80px;background:var(--page-bg)">
  ...
  </div>`,Sn(e,t,p,n),document.getElementById("builder-back").addEventListener("click",()=>a("decks")),document.getElementById("formation-select").addEventListener("change",i=>{t.formation=i.target.value;const r=Ei(t.formation),u={};r.forEach(x=>{t.slots[x]&&(u[x]=t.slots[x])}),t.slots=u,pt(e,t,n)}),document.getElementById("save-deck").addEventListener("click",()=>Bn(t,n)),e.querySelectorAll("[data-remove-sub]").forEach(i=>{i.addEventListener("click",()=>{t.subs=t.subs.filter(r=>r!==i.dataset.removeSub),pt(e,t,n)})}),(c=document.getElementById("add-sub-btn"))==null||c.addEventListener("click",()=>{jn(t,e,n)})}function Sn(e,t,n,a){const p=e.querySelector("#deck-field");if(!p)return;const d=zt[t.formation]||{},s=Mi(t.formation)||[],l={};for(const z of n){const M=t.slots[z],B=M?t.playerCards.find(U=>U.id===M):null;l[z]=B?B.player:null}const c=300,i=300,r=48,u=64,x=13,g=16,k=38;function o(z){const M=d[z];return M?{x:M.x*c,y:M.y*i}:null}let $="";for(const[z,M]of s){const B=o(z),U=o(M);if(!B||!U)continue;const F=l[z]?{...l[z],club_id:l[z].club_id,country_code:l[z].country_code,rarity:l[z].rarity}:null,j=l[M]?{...l[M],club_id:l[M].club_id,country_code:l[M].country_code,rarity:l[M].rarity}:null,R=Ze(F,j);R==="#ff3333"||R==="#cc2222"?$+=`<line x1="${B.x.toFixed(1)}" y1="${B.y.toFixed(1)}" x2="${U.x.toFixed(1)}" y2="${U.y.toFixed(1)}" stroke="${R}" stroke-width="2.5" stroke-linecap="round" opacity="0.4"/>`:($+=`<line x1="${B.x.toFixed(1)}" y1="${B.y.toFixed(1)}" x2="${U.x.toFixed(1)}" y2="${U.y.toFixed(1)}" stroke="${R}" stroke-width="8" stroke-linecap="round" opacity="0.2"/>`,$+=`<line x1="${B.x.toFixed(1)}" y1="${B.y.toFixed(1)}" x2="${U.x.toFixed(1)}" y2="${U.y.toFixed(1)}" stroke="${R}" stroke-width="2.5" stroke-linecap="round" opacity="0.9"/>`)}for(const z of n){const M=o(z);if(!M)continue;const B=l[z],U=z.replace(/\d+/,""),F=rt[U]||"#555",j=(M.x-r/2).toFixed(1),R=(M.y-u/2).toFixed(1),ie={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[B==null?void 0:B.rarity]||"#aaa";if(B){const ce=Ut(B),oe=Pi(B),S=Ni(B.country_code),Y=Number(U==="GK"?B.note_g:U==="DEF"?B.note_d:U==="MIL"?B.note_m:B.note_a)||0,ee=u-x-g;$+=`<defs><clipPath id="dcp-${z}"><rect x="${j}" y="${(M.y-u/2+x).toFixed(1)}" width="${r}" height="${ee}"/></clipPath></defs>`,$+=`<rect x="${j}" y="${R}" width="${r}" height="${u}" rx="5" fill="${F}"/>`,ce&&($+=`<image href="${ce}" x="${j}" y="${(M.y-u/2+x).toFixed(1)}" width="${r}" height="${ee}" clip-path="url(#dcp-${z})" preserveAspectRatio="xMidYMin slice"/>`),$+=`<rect x="${j}" y="${R}" width="${r}" height="${x}" fill="rgba(255,255,255,0.93)"/>`,$+=`<text x="${M.x.toFixed(1)}" y="${(M.y-u/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(B.surname_encoded||"").slice(0,9)}</text>`;const D=(M.y+u/2-g).toFixed(1);$+=`<rect x="${j}" y="${D}" width="${r}" height="${g}" fill="rgba(255,255,255,0.93)"/>`,S&&($+=`<image href="${S}" x="${(M.x-21).toFixed(1)}" y="${(M.y+u/2-g+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`),$+=`<text x="${M.x.toFixed(1)}" y="${(M.y+u/2-g/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${Y}</text>`,oe&&($+=`<image href="${oe}" x="${(M.x+r/2-14).toFixed(1)}" y="${(M.y+u/2-g+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`),$+=`<rect x="${j}" y="${R}" width="${r}" height="${u}" rx="5" fill="none" stroke="${ie}" stroke-width="2"/>`}else $+=`<rect x="${j}" y="${R}" width="${r}" height="${u}" rx="5" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="5,3"/>`,$+=`<text x="${M.x.toFixed(1)}" y="${M.y.toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="22" fill="rgba(255,255,255,0.35)">+</text>`,$+=`<text x="${M.x.toFixed(1)}" y="${(M.y+16).toFixed(1)}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.3)">${U}</text>`;$+=`<rect x="${j}" y="${R}" width="${r}" height="${u}" rx="5" fill="rgba(0,0,0,0.01)" class="deck-slot-hit" data-pos="${z}" style="cursor:pointer"/>`}p.innerHTML=`<svg viewBox="${-k} ${-k} ${c+k*2} ${i+k*2}" width="100%" style="display:block;max-width:440px;margin:0 auto">${$}</svg>`,p.querySelectorAll(".deck-slot-hit").forEach(z=>{z.addEventListener("click",()=>Cn(z.dataset.pos,t,e,a))})}function Cn(e,t,n,a){var x,g,k;const{openModal:p,closeModal:d}=a,s=e.replace(/\d+/,""),l=t.slots[e],c=l?t.playerCards.find(o=>o.id===l):null;(x=c==null?void 0:c.player)==null||x.id;const i=new Set;Object.entries(t.slots).forEach(([o,$])=>{var M;if(o===e||!$)return;const z=t.playerCards.find(B=>B.id===$);(M=z==null?void 0:z.player)!=null&&M.id&&i.add(z.player.id)}),t.subs.forEach(o=>{var z;const $=t.playerCards.find(M=>M.id===o);(z=$==null?void 0:$.player)!=null&&z.id&&i.add($.player.id)});const r=new Set,u=t.playerCards.filter(o=>{const $=o.player;return!($.job===s||$.job2===s)||i.has($.id)||r.has($.id)?!1:(r.add($.id),!0)});u.sort((o,$)=>{const z=s==="GK"?o.player.note_g:s==="DEF"?o.player.note_d:s==="MIL"?o.player.note_m:o.player.note_a;return(s==="GK"?$.player.note_g:s==="DEF"?$.player.note_d:s==="MIL"?$.player.note_m:$.player.note_a)-z}),p(`Choisir ${s} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${u.length>0?u.map(o=>{var U,F;const $=o.player,z=s==="GK"?$.note_g:s==="DEF"?$.note_d:s==="MIL"?$.note_m:$.note_a,M=Ut($),B={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[$.rarity];return`<div class="player-option" data-card-id="${o.id}"
          ...
          <div style="width:44px;height:44px;border-radius:8px;overflow:hidden;flex-shrink:0;background:#dde;border:2px solid ${rt[s]}">
            ${M?`<img src="${M}" style="width:100%;height:100%;object-fit:cover">`:`<div style="width:100%;height:100%;background:${rt[s]};display:flex;align-items:center;justify-content:center;color:#fff;font-size:10px;font-weight:700">${s}</div>`}
          </div>
          <!-- Infos -->
          <div style="flex:1;min-width:0">
            ...
            <div style="display:flex;align-items:center;gap:6px;margin-top:3px">
              ...
              ${(U=$.clubs)!=null&&U.logo_url?`<img src="${$.clubs.logo_url}" style="width:18px;height:18px;object-fit:contain">`:`<span style="font-size:10px;color:var(--gray-600)">${((F=$.clubs)==null?void 0:F.encoded_name)||"—"}</span>`}
    ...
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(g=document.getElementById("close-selector"))==null||g.addEventListener("click",d),(k=document.getElementById("remove-player"))==null||k.addEventListener("click",()=>{delete t.slots[e],d(),pt(n,t,a)}),document.querySelectorAll(".player-option").forEach(o=>{o.addEventListener("click",()=>{t.slots[e]=o.dataset.cardId,d(),pt(n,t,a)})})}function jn(e,t,n){var c;const{openModal:a,closeModal:p}=n,d=new Set;Object.values(e.slots).filter(Boolean).forEach(i=>{var u;const r=e.playerCards.find(x=>x.id===i);(u=r==null?void 0:r.player)!=null&&u.id&&d.add(r.player.id)}),e.subs.forEach(i=>{var u;const r=e.playerCards.find(x=>x.id===i);(u=r==null?void 0:r.player)!=null&&u.id&&d.add(r.player.id)});const s=new Set,l=e.playerCards.filter(i=>{var r,u,x;return d.has((r=i.player)==null?void 0:r.id)||s.has((u=i.player)==null?void 0:u.id)?!1:(s.add((x=i.player)==null?void 0:x.id),!0)});a("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${l.length>0?l.map(i=>{var g;const r=i.player,u=Ut(r),x=r.job==="GK"?r.note_g:r.job==="DEF"?r.note_d:r.job==="MIL"?r.note_m:r.note_a;return`<div class="player-option" data-card-id="${i.id}"
          style="display:flex;align-items:center;gap:10px;padding:8px;border:1.5px solid var(--gray-200);border-radius:10px;cursor:pointer">
    ...
    </div>`,'<button class="btn btn-ghost" id="close-sub-selector">Fermer</button>'),(c=document.getElementById("close-sub-selector"))==null||c.addEventListener("click",p),document.querySelectorAll(".player-option").forEach(i=>{i.addEventListener("click",()=>{e.subs.push(i.dataset.cardId),p(),pt(t,e,n)})})}async function Bn(e,t){const{state:n,toast:a,navigate:p}=t,d=e.formationCards.find(c=>c.formation===e.formation),s=(d==null?void 0:d.id)||e.formationCardId;await _.from("decks").update({formation:e.formation,formation_card_id:s||null}).eq("id",e.deckId),await _.from("deck_cards").delete().eq("deck_id",e.deckId);const l=[];if(Object.entries(e.slots).forEach(([c,i],r)=>{l.push({deck_id:e.deckId,card_id:i,position:c,is_starter:!0,slot_order:r})}),e.subs.forEach((c,i)=>{l.push({deck_id:e.deckId,card_id:c,position:`SUB${i+1}`,is_starter:!1,slot_order:100+i})}),l.length>0){const{error:c}=await _.from("deck_cards").insert(l);if(c){a(c.message,"error");return}}a("Deck enregistré ✅","success"),p("decks")}function Ei(e){const t=Gt[e]||Gt["4-4-2"],n=["GK1"];for(let a=1;a<=t.DEF;a++)n.push(`DEF${a}`);for(let a=1;a<=t.MIL;a++)n.push(`MIL${a}`);for(let a=1;a<=t.ATT;a++)n.push(`ATT${a}`);return n}async function Ri(){const{data:e}=await _.from("booster_configs").select("*").eq("is_active",!0).order("sort_order");if(!(e!=null&&e.length))return[];const{data:t}=await _.from("booster_drop_rates").select("*").in("booster_id",e.map(n=>n.id)).order("sort_order");return e.map(n=>({...n,rates:(t||[]).filter(a=>a.booster_id===n.id)}))}function Fn(e){if(!(e!=null&&e.length))return null;const t=e.reduce((a,p)=>a+Number(p.percentage),0);let n=Math.random()*t;for(const a of e)if(n-=Number(a.percentage),n<=0)return a;return e[e.length-1]}const Oi=()=>Object.keys(zt),qn=[{id:"players_std",img:"/manager-wars/icons/booster-players.png",name:"Players",sub:"5 cartes joueurs",cost:5e3,costLabel:"5 000 crédits",cardCount:5,type:"player"},{id:"players_pub",img:"/manager-wars/icons/booster-silver.png",name:"Players (pub)",sub:"3 cartes joueurs",cost:0,costLabel:"1 pub",cardCount:3,type:"player"},{id:"game_changer",img:"/manager-wars/icons/booster-gamechanger.png",name:"Game Changer",sub:"3 cartes spéciales",cost:1e4,costLabel:"10 000 crédits",cardCount:3,type:"game_changer"},{id:"formation",img:"/manager-wars/icons/booster-formation.png",name:"Formation",sub:"1 carte formation",cost:1e4,costLabel:"10 000 crédits",cardCount:1,type:"formation"}],Jt={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé."},"Double attaque":{icon:"⚡",desc:"La prochaine attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la prochaine action IA."},Gel:{icon:"❄️",desc:"Bloque le meilleur attaquant IA."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function Dn(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";if(!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const n=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${n}.png`}const Ai={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},Gn={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"},Nn={MA:"MAROC",FR:"FRANCE",AR:"ARGENTINE",PT:"PORTUGAL",BR:"BRESIL",ES:"ESPAGNE",DE:"ALLEMAGNE",GB:"ANGLETERRE",IT:"ITALIE",CM:"CAMEROUN",SN:"SENEGAL",NG:"NIGERIA",DK:"DANEMARK",NL:"PAYS-BAS",BE:"BELGIQUE",CI:"CÔTE D'IVOIRE",AL:"ALBANIE",HR:"CROATIE",RS:"SERBIE",TR:"TURQUIE"};function Ti(e,t){return e&&Number(t==="GK"?e.note_g:t==="DEF"?e.note_d:t==="MIL"?e.note_m:e.note_a)||0}function Pn(e){var r,u;const t=e.player;if(!t)return"";const n=t.job||"ATT",a=Ai[n],p=Gn[t.rarity]||"#ccc",d=Ti(t,n),s=t.job2?Ti(t,t.job2):null,l=t.job2?Ai[t.job2]:null,c=Dn(t),i=Nn[t.country_code]||t.country_code||"";return`
  ...
  </div>`,e.querySelectorAll(".booster-card:not(.disabled)").forEach(l=>{l.addEventListener("click",async()=>{const c=d.find(i=>i.id===l.dataset.booster);if(c){l.style.opacity="0.5",l.style.pointerEvents="none";try{await On(c,{state:t,toast:a,navigate:n,container:e})}catch(i){a(i.message,"error"),l.style.opacity="",l.style.pointerEvents=""}}})}),e.querySelectorAll(".booster-info-btn").forEach(l=>{l.addEventListener("click",c=>{c.stopPropagation();const i=d.find(r=>r.id===l.dataset.boosterId);Vn(i)})})}async function On(e,{state:t,toast:n,navigate:a,container:p}){var r;if(e.cost>0&&t.profile.credits<e.cost){n("Crédits insuffisants","error");return}if(e.isPub)try{await Xn()}catch(u){n(u.message||"Regardez la pub entièrement pour ouvrir le booster","error");return}const{data:d}=await _.from("cards").select("card_type, player_id, formation").eq("owner_id",t.profile.id),s=new Set((d||[]).filter(u=>u.card_type==="player").map(u=>u.player_id)),l=new Set((d||[]).filter(u=>u.card_type==="formation").map(u=>u.formation));let c=[];if((r=e.rates)!=null&&r.length)c=await Qt(t.profile,e);else{const u=e.type||"player";u==="player"?c=await Ui(t.profile,e.cardCount,e.cost):u==="game_changer"?c=await Ki(t.profile,e.cardCount,e.cost):u==="formation"?c=await Vi(t.profile,e.cost):c=await Qt(t.profile,e)}c.forEach(u=>{u.card_type==="player"&&u.player?u.isDuplicate=s.has(u.player.id):u.card_type==="formation"&&(u.isDuplicate=l.has(u.formation))});const{data:i}=await _.from("users").select("*").eq("id",t.profile.id).single();i&&(t.profile=i),Yi(c,e,a)}function Hn(){const e=Math.random()*100;return e<.5?"legende":e<2?"special":e<10?"normal_high":"normal_low"}function tt(e){return Math.max(Number(e.note_g)||0,Number(e.note_d)||0,Number(e.note_m)||0,Number(e.note_a)||0)}function Un(e,t){let n;switch(t){case"legende":n=e.filter(a=>a.rarity==="legende"),n.length||(n=e.filter(a=>a.rarity==="pepite"||a.rarity==="papyte")),n.length||(n=e.filter(a=>tt(a)>=6));break;case"special":n=e.filter(a=>a.rarity==="pepite"||a.rarity==="papyte"),n.length||(n=e.filter(a=>tt(a)>=6));break;case"normal_high":n=e.filter(a=>a.rarity==="normal"&&tt(a)>=6),n.length||(n=e.filter(a=>tt(a)>=6));break;default:n=e.filter(a=>a.rarity==="normal"&&tt(a)>=1&&tt(a)<=5),n.length||(n=e.filter(a=>a.rarity==="normal"));break}return n.length||(n=e),n[Math.floor(Math.random()*n.length)]}async function Qt(e,t){if(t.cost>0){const{error:l}=await _.from("users").update({credits:e.credits-t.cost}).eq("id",e.id);if(l)throw l}const{data:n}=await _.from("cards").select("player_id, card_type, formation").eq("owner_id",e.id),a=new Set((n||[]).filter(l=>l.card_type==="player").map(l=>l.player_id)),p=new Set((n||[]).filter(l=>l.card_type==="formation").map(l=>l.formation)),d=new Set,s=[];for(let l=0;l<(t.cardCount||5);l++){const c=Fn(t.rates);if(c){if(c.card_type==="player"){const i=M=>({légende:"legende",pépite:"pepite",pépites:"pepite"})[M]||M,r=c.rarity?i(c.rarity):null;let u=_.from("players").select("id,job,firstname,surname_encoded,country_code,club_id,rarity,note_g,note_d,note_m,note_a,skin,hair,hair_length,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);r&&(u=u.eq("rarity",r));const{data:x}=await u;let g=x||[];if((c.note_min||c.note_max)&&(g=g.filter(M=>{const B=Math.max(Number(M.note_g)||0,Number(M.note_d)||0,Number(M.note_m)||0,Number(M.note_a)||0);return(!c.note_min||B>=c.note_min)&&(!c.note_max||B<=c.note_max)})),g.length||(c.note_min||c.note_max?(g=x||[],console.warn("[Booster] Aucun joueur avec note",c.note_min,"-",c.note_max,"— fallback rareté uniquement")):g=x||[]),!g.length)continue;let k=g.filter(M=>!d.has(M.id));k.length||(k=g);const o=k[Math.floor(Math.random()*k.length)];d.add(o.id);const $=a.has(o.id),{data:z}=await _.from("cards").insert({owner_id:e.id,player_id:o.id,card_type:"player"}).select().single();z&&(s.push({...z,player:o,isDuplicate:$}),_.rpc("record_transfer",{p_card_id:z.id,p_player_id:o.id,p_club_name:e.club_name||e.pseudo,p_manager_name:e.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{}))}else if(c.card_type==="game_changer"){const{data:i}=await _.from("gc_definitions").select("id,name").eq("is_active",!0).eq("gc_type","game_changer"),r=i!=null&&i.length?i:[{name:"Ressusciter"},{name:"Double attaque"},{name:"Bouclier"},{name:"Vol de note"},{name:"Gel"}],x=r[Math.floor(Math.random()*r.length)].name,{data:g}=await _.from("cards").insert({owner_id:e.id,card_type:"game_changer",gc_type:x}).select().single();g&&s.push(g)}else if(c.card_type==="formation"){const i=Oi(),r=i[Math.floor(Math.random()*i.length)],u=p.has(r),{data:x}=await _.from("cards").insert({owner_id:e.id,card_type:"formation",formation:r}).select();x!=null&&x[0]&&s.push({...x[0],isDuplicate:u})}}}return s}async function Ui(e,t,n){if(n>0){const{error:i}=await _.from("users").update({credits:e.credits-n}).eq("id",e.id);if(i)throw i}const{data:a}=await _.from("players").select("id,job,firstname,surname_encoded,country_code,club_id,rarity,note_g,note_d,note_m,note_a,note_min,note_max,skin,hair,hair_length,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);if(!(a!=null&&a.length))throw new Error("Pas de joueurs en BDD — ajoutes-en via le panel admin !");const p=a.filter(i=>i.job==="GK"),d=a.filter(i=>i.job!=="GK"),s=!e.first_booster_opened&&p.length>0,l=[];for(let i=0;i<t;i++){const r=i===0&&s?p:i===0?d:a,u=Hn(),x=Un(r,u);x&&l.push(x)}s&&await _.from("users").update({first_booster_opened:!0}).eq("id",e.id);const{data:c}=await _.from("cards").insert(l.map(i=>({owner_id:e.id,player_id:i.id,card_type:"player"}))).select();return(c||[]).forEach((i,r)=>{_.rpc("record_transfer",{p_card_id:i.id,p_player_id:l[r].id,p_club_name:e.club_name||e.pseudo,p_manager_name:e.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{})}),l.map((i,r)=>({...c[r],player:i}))}async function Ki(e,t,n){const{error:a}=await _.from("users").update({credits:e.credits-n}).eq("id",e.id);if(a)throw a;const{data:p}=await _.from("gc_definitions").select("id,name,gc_type,color,effect,image_url").eq("is_active",!0),d=p!=null&&p.length?p:Object.keys(Jt).map(r=>({name:r,gc_type:"game_changer"})),s=Array.from({length:t},()=>{const r=d[Math.floor(Math.random()*d.length)];return{owner_id:e.id,card_type:"game_changer",gc_type:r.name,gc_definition_id:r.id||null}}),{data:l,error:c}=await _.from("cards").insert(s).select();return c&&console.error("[Booster GC] Erreur insert:",c.message,c),(l||[]).map(r=>{const u=p==null?void 0:p.find(x=>x.name===r.gc_type||x.id===r.gc_definition_id);return{...r,_gcDef:u||null}})}async function Vi(e,t){const{error:n}=await _.from("users").update({credits:e.credits-t}).eq("id",e.id);if(n)throw n;const{data:a}=await _.from("cards").select("formation").eq("owner_id",e.id).eq("card_type","formation"),p=new Set((a||[]).map(r=>r.formation)),d=Oi(),s=d[Math.floor(Math.random()*d.length)],l=p.has(s),{data:c,error:i}=await _.from("cards").insert({owner_id:e.id,card_type:"formation",formation:s}).select();return i&&console.error("[Booster Formation] Erreur insert:",i.message,i),(c||[]).map(r=>({...r,isDuplicate:l}))}function Yi(e,t,n,a=null){var Y,ee;if(!e||e.length===0){const D=document.createElement("div");D.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:3000;gap:16px;color:#fff;padding:24px;text-align:center",D.innerHTML=`
  ...
  `,document.body.appendChild(p);let d=!1;const s=document.getElementById("pack-cut-zone"),l=document.getElementById("pack-blade");let c=!1;const i=D=>D.touches&&D.touches[0]?D.touches[0].clientX:D.clientX;function r(D){d||(c=!0,l.style.opacity="1",u(D))}function u(D){if(!c||d)return;const Q=s.getBoundingClientRect(),ae=i(D)-Q.left,pe=Math.max(0,Math.min(1,ae/Q.width));l.style.width=pe*Q.width+"px",pe>=.82&&g()}function x(){d||(c=!1,l.style.transition="width .2s ease, opacity .2s ease",l.style.width="0",l.style.opacity="0",setTimeout(()=>{d||(l.style.transition="")},220))}function g(){var Q;if(d)return;d=!0,c=!1,l.style.width="100%",l.style.opacity="1",(Q=document.getElementById("cut-flash"))==null||Q.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const D=document.getElementById("cut-hint");D&&(D.style.opacity="0"),s.classList.add("pack-cut"),setTimeout(()=>{l.style.opacity="0",document.getElementById("pack-phase").style.display="none",z(0)},620)}s.addEventListener("pointerdown",r),window.addEventListener("pointermove",u),window.addEventListener("pointerup",x),s.addEventListener("touchstart",r,{passive:!0}),window.addEventListener("touchmove",u,{passive:!0}),window.addEventListener("touchend",x);let k=0,o=!1;const $=new Set;function z(D){k=D,document.getElementById("reveal-phase").style.display="flex",M(),B(D,0),ie()}function M(){const D=document.getElementById("card-dots");D&&(D.innerHTML=e.map((Q,ae)=>`<div class="card-dot" data-i="${ae}" style="width:8px;height:8px;border-radius:50%;background:${ae===k?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),D.querySelectorAll(".card-dot").forEach(Q=>Q.addEventListener("click",()=>F(parseInt(Q.dataset.i)))))}function B(D,Q){var q;const ae=e[D],pe=document.getElementById("card-counter"),he=document.getElementById("card-track");pe&&(pe.textContent=`Carte ${D+1} / ${e.length}`);const xe=document.getElementById("reveal-btns");xe&&(xe.style.display=D===e.length-1?"flex":"none");const ge=ae.card_type==="player"&&((q=ae.player)==null?void 0:q.rarity)==="legende",O=!$.has(D);$.add(D);let L=0;if(ae.card_type==="player"&&ae.player){const V=ae.player,ne=V.job||"ATT";L=Number(ne==="GK"?V.note_g:ne==="DEF"?V.note_d:ne==="MIL"?V.note_m:V.note_a)||0}const N=V=>{he.innerHTML=`
        <div id="current-card-wrap" style="position:relative;display:flex;flex-direction:column;align-items:center;gap:8px;${ge?"filter:drop-shadow(0 0 20px #7a28b8)":""}">
          <div style="transform:scale(1.25);transform-origin:center">${Kn(ae)}</div>
        ...
        </div>`;const ne=document.getElementById("current-card-wrap");Q!==0?(ne.style.transition="none",ne.style.transform=`translateX(${Q>0?100:-100}%)`,requestAnimationFrame(()=>{ne.style.transition="transform .28s cubic-bezier(.25,1,.5,1)",ne.style.transform="translateX(0)"})):ne.animate([{opacity:0,transform:"scale(.7)"},{opacity:1,transform:"scale(1)"}],{duration:300,easing:"cubic-bezier(.34,1.56,.64,1)"}),V||ge?oe():S(),M()};O&&L>6&&ae.card_type==="player"&&ae.player?U(ae,()=>N(!0)):N(!1)}function U(D,Q){var L;o=!0;const ae=D.player,pe=`https://flagsapi.com/${ae.country_code}/flat/64.png`,he=(L=ae.clubs)==null?void 0:L.logo_url,xe=document.getElementById("walkout-overlay"),ge=document.getElementById("walkout-stage");if(!xe||!ge){o=!1,Q();return}xe.style.display="flex";const O=()=>{const N=ge.firstElementChild;N&&(N.classList.remove("wo-in"),N.classList.add("wo-out"))};ge.innerHTML=`<img class="wo-in" src="${pe}" style="height:130px;border-radius:10px;box-shadow:0 10px 36px rgba(0,0,0,.6)" onerror="this.style.display='none'">`,navigator.vibrate&&navigator.vibrate(30),setTimeout(O,2e3),setTimeout(()=>{var N;ge.innerHTML=he?`<img class="wo-in" src="${he}" style="max-height:160px;max-width:210px;object-fit:contain">`:`<div class="wo-in" style="font-size:34px;font-weight:900;color:#fff;text-align:center">${((N=ae.clubs)==null?void 0:N.encoded_name)||"CLUB"}</div>`,navigator.vibrate&&navigator.vibrate(30)},2450),setTimeout(O,4450),setTimeout(()=>{xe.style.display="none",ge.innerHTML="",o=!1,navigator.vibrate&&navigator.vibrate([40,30,80]),Q()},4900)}function F(D){if(o||D<0||D>=e.length||D===k)return;const Q=D>k?1:-1;k=D,B(D,Q)}function j(){F(k+1)}function R(){F(k-1)}function ie(){const D=document.getElementById("card-viewport");if(!D||D._swipeBound)return;D._swipeBound=!0;let Q=0,ae=0,pe=0,he=!1;const xe=q=>q.touches?q.touches[0].clientX:q.clientX,ge=q=>q.touches?q.touches[0].clientY:q.clientY,O=q=>{he=!0,Q=xe(q),ae=ge(q),pe=0},L=q=>{if(!he)return;pe=xe(q)-Q;const V=ge(q)-ae;if(Math.abs(pe)<Math.abs(V))return;const ne=document.getElementById("current-card-wrap");ne&&(ne.style.transition="none",ne.style.transform=`translateX(${pe*.6}px) rotate(${pe*.02}deg)`)},N=()=>{if(!he)return;he=!1;const q=document.getElementById("current-card-wrap"),V=55;pe<=-V&&k<e.length-1?j():pe>=V&&k>0?R():q&&(q.style.transition="transform .2s ease",q.style.transform="translateX(0)")};D.addEventListener("pointerdown",O),D.addEventListener("pointermove",L),D.addEventListener("pointerup",N),D.addEventListener("pointercancel",N),D.addEventListener("touchstart",O,{passive:!0}),D.addEventListener("touchmove",L,{passive:!0}),D.addEventListener("touchend",N),D.addEventListener("click",q=>{if(Math.abs(pe)>8)return;const V=D.getBoundingClientRect();q.clientX-V.left>V.width/2?j():R()})}let ce=null;function oe(){const D=document.getElementById("fireworks-canvas");if(!D)return;D.width=window.innerWidth,D.height=window.innerHeight;const Q=D.getContext("2d"),ae=[];function pe(){const xe=Math.random()*D.width,ge=Math.random()*D.height*.6,O=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],L=O[Math.floor(Math.random()*O.length)];for(let N=0;N<60;N++){const q=Math.PI*2/60*N,V=2+Math.random()*5;ae.push({x:xe,y:ge,vx:Math.cos(q)*V,vy:Math.sin(q)*V,alpha:1,color:L,size:2+Math.random()*3})}}pe(),ce=setInterval(pe,600);function he(){if(document.getElementById("fireworks-canvas")){Q.clearRect(0,0,D.width,D.height);for(let xe=ae.length-1;xe>=0;xe--){const ge=ae[xe];if(ge.x+=ge.vx,ge.y+=ge.vy+.08,ge.vy*=.98,ge.alpha-=.018,ge.alpha<=0){ae.splice(xe,1);continue}Q.globalAlpha=ge.alpha,Q.fillStyle=ge.color,Q.beginPath(),Q.arc(ge.x,ge.y,ge.size,0,Math.PI*2),Q.fill()}Q.globalAlpha=1,(ce!==null||ae.length>0)&&requestAnimationFrame(he)}}he()}function S(){ce!==null&&(clearInterval(ce),ce=null);const D=document.getElementById("fireworks-canvas");D&&D.getContext("2d").clearRect(0,0,D.width,D.height)}if(a){const D=document.getElementById("reveal-btns");D&&(D.innerHTML='<button class="btn btn-primary" id="reveal-next" style="flex:1">Continuer →</button>'),(ee=document.getElementById("reveal-next"))==null||ee.addEventListener("click",()=>{S(),p.remove(),a()})}else document.getElementById("reveal-collection").addEventListener("click",()=>{S(),p.remove(),n("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{S(),p.remove(),n("boosters")})}function Kn(e){var t,n;if(e.card_type==="player"&&e.player)return Pn(e);if(e.card_type==="game_changer"){const a=e._gcDef,p=(a==null?void 0:a.gc_type)==="ultra_game_changer",d={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},s={purple:"#b06ce0",light_blue:"#00d4ef"},l=d[a==null?void 0:a.color]||d.purple,c=s[a==null?void 0:a.color]||s.purple,i=(a==null?void 0:a.name)||e.gc_type||"Game Changer",r=(a==null?void 0:a.effect)||((t=Jt[e.gc_type])==null?void 0:t.desc)||"",u=a!=null&&a.image_url?`/manager-wars/icons/${a.image_url}`:null,x=((n=Jt[e.gc_type])==null?void 0:n.icon)||"⚡";return`<div style="width:170px;height:240px;background:${l};border-radius:14px;border:3px solid ${c};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${c}88;flex-shrink:0">
      ...
      </div>`,document.body.appendChild(a),a.addEventListener("click",s=>{s.target===a&&a.remove()}),(n=document.getElementById("odds-close"))==null||n.addEventListener("click",()=>a.remove());return}Yn()}function Yn(){const e=document.createElement("div");e.style.cssText=`position:fixed;inset:0;background:rgba(0,0,0,0.6);display:flex;
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}const Wn="simulation",Wi="VOTRE_ZONE_ID";function Xn(){switch(Wn){case"propellerads":return Jn();case"adsense":return Qn();default:return di()}}function di(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:#111;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:14px";let n=5;t.innerHTML=`
      ...
      <button id="ad-skip" disabled style="margin-top:8px;padding:10px 24px;border-radius:20px;border:none;background:rgba(255,255,255,0.15);color:rgba(255,255,255,0.4);font-size:13px;cursor:default">
        Passer (5)
      </button>`,document.body.appendChild(t);const a=setInterval(()=>{n--;const p=document.getElementById("ad-cd"),d=document.getElementById("ad-skip");p&&(p.textContent=n),d&&(d.textContent=n>0?`Passer (${n})`:"✓ Continuer"),n<=0&&(clearInterval(a),d&&(d.disabled=!1,d.style.cssText=d.style.cssText.replace("rgba(255,255,255,0.15)","#1A6B3C").replace("rgba(255,255,255,0.4)","#fff").replace("default","pointer")),d==null||d.addEventListener("click",()=>{t.remove(),e(!0)},{once:!0}))},1e3)})}function Jn(){return new Promise((e,t)=>{if(typeof window.propeller>"u")return console.warn("[Ad] PropellerAds non chargé → simulation"),e(di());window.propeller.push({zone_id:Wi,onComplete:()=>e(!0),onSkip:()=>t(new Error("Publicité ignorée")),onError:()=>t(new Error("Erreur publicité"))})})}function Qn(){return new Promise((e,t)=>{if(typeof window.adsbygoogle>"u")return console.warn("[Ad] AdSense non chargé → simulation"),e(di());try{(window.adsbygoogle=window.adsbygoogle||[]).push({google_ad_client:"ca-pub-VOTRE_PUBLISHER_ID",google_ad_slot:Wi,google_ad_format:"rewarded",on_reward:()=>e(!0),on_dismiss:()=>t(new Error("Publicité fermée"))})}catch(n){t(n)}})}async function Zn(e,{state:t,navigate:n,toast:a,refreshProfile:p}){var x,g;const{data:d}=await _.from("users").select("*").eq("id",t.user.id).single();d&&(t.profile=d);let s=Array.isArray((x=t.profile)==null?void 0:x.pending_boosters)?[...t.profile.pending_boosters]:[];if(!s.length){await _.from("users").update({onboarding_done:!0}).eq("id",t.user.id),n("home");return}let l=null;try{const o=(await Ri()).find($=>($.name||"").toLowerCase().includes("new player"));o&&(l=Hi(o))}catch(k){console.warn('[Onboarding] Config "Booster (new player)" introuvable, fallback taux par défaut',k)}const c=s.length;let i=0;e.innerHTML=`
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    </div>`}const c=r=>{e.style.overflow="",e.style.height="",e.style.display="",e.style.flexDirection="",n(r)};function i(){var u,x,g;e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column";const r=s.length>0;e.innerHTML=`
    ...
    </div>`,e.querySelectorAll(".gc-select-card").forEach(k=>{k.addEventListener("click",()=>{const o=k.dataset.id,$=d.find(M=>M.id===o);if(!$)return;const z=s.findIndex(M=>M.gc_type===$.gc_type);z>-1?s.splice(z,1):s.length<3&&s.push($),i()})}),(u=e.querySelector("#gc-launch"))==null||u.addEventListener("click",()=>{r&&c(s)}),(x=e.querySelector("#gc-no-gc"))==null||x.addEventListener("click",()=>c([])),(g=e.querySelector("#gc-reset"))==null||g.addEventListener("click",()=>{s.length&&(s=[],i())})}i()}function to(e){var a;const t=((a=e==null?void 0:e.state)==null?void 0:a.params)||{},n=t.matchMode||"vs_ai_easy";return n==="friend"?`Match vs ${t.friendName||"Ami"}`:n==="random"?"Match vs Random":`Match vs IA — ${n.replace("vs_ai_","").toUpperCase()}`}async function io(e,t,n){const{state:a,navigate:p}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:d}=await _.from("decks").select("id,name,is_active,formation").eq("owner_id",a.profile.id).order("created_at",{ascending:!1});if(!d||d.length===0){at(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>p("decks"));return}const s=d.map(r=>r.id),{data:l}=await _.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",s).order("slot_order");let c=0;function i(){var M,B,U,F,j;const r=d[c],u=(l||[]).filter(R=>R.deck_id===r.id),x=u.filter(R=>{var ie;return R.is_starter&&((ie=R.card)==null?void 0:ie.player)}).map(R=>Zt(R.card,R.position)),g=u.find(R=>{var ie;return((ie=R.card)==null?void 0:ie.card_type)==="formation"}),k=r.formation||((M=g==null?void 0:g.card)==null?void 0:M.formation)||"4-4-2",o=x.length>=11?gt(x,k):null,$=x.length>=11;Xe(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">
        ${d.map((R,ie)=>`<div style="width:7px;height:7px;border-radius:50%;background:${ie===c?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}
      </div>`:""}
    ...
    </div>`,function(){const ie=e.querySelector(".deck-preview-wrap svg");ie&&(ie.removeAttribute("width"),ie.removeAttribute("height"),ie.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",ie.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),(B=document.getElementById("prev-deck"))==null||B.addEventListener("click",()=>{c>0&&(c--,i())}),(U=document.getElementById("next-deck"))==null||U.addEventListener("click",()=>{c<d.length-1&&(c++,i())}),(F=document.getElementById("validate-deck"))==null||F.addEventListener("click",()=>{if(!$)return;const R=t.state.params||{};t.navigate("match",{...R,matchMode:R.matchMode||n,deckId:r.id})}),(j=document.getElementById("cancel-deck-select"))==null||j.addEventListener("click",()=>{Oe(e),p("home")});const z=document.getElementById("deck-swipe-zone");if(z){let R=0,ie=0;z.addEventListener("touchstart",ce=>{R=ce.touches[0].clientX,ie=ce.touches[0].clientY},{passive:!0}),z.addEventListener("touchend",ce=>{const oe=ce.changedTouches[0].clientX-R,S=ce.changedTouches[0].clientY-ie;Math.abs(oe)<40||Math.abs(oe)<Math.abs(S)||(oe<0&&c<d.length-1?(c++,i()):oe>0&&c>0&&(c--,i()))},{passive:!0})}}i()}function Xi(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";return e!=null&&e.clubLogo?e.clubLogo.startsWith("http")?e.clubLogo:`${t}/storage/v1/object/public/assets/clubs/${e.clubLogo}`:null}function Ve(e,t=44,n=58){if(!e)return`<div style="width:${t}px;height:${n}px;border:1.5px dashed rgba(255,255,255,0.2);border-radius:5px"></div>`;const a=typeof e.portrait=="string"&&e.portrait.startsWith("http")?e.portrait:ze(e),p=Xi(e),d=e._line||e.job||"MIL",s=ft[d]||"#555",l={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",c=e.note!==void 0?Number(e.note)||0:(Number(we(e,d))||0)+(e.boost||0),i=Ji(e==null?void 0:e.country_code),r=Math.round(n*.19),u=Math.round(n*.25),x=n-Math.round(n*.19)-Math.round(n*.25),g=e!=null&&e.used?.28:1;return`<div style="width:${t}px;height:${n}px;border-radius:5px;border:2px solid ${l};background:${s};position:relative;overflow:hidden;flex-shrink:0;opacity:${g}">
    <div style="position:absolute;top:0;left:0;right:0;height:${r}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:center;z-index:2">
  ...
  </div>`}function it(e,t,n){if(!(e!=null&&e.length))return"";const a=e.slice(0,5);let p='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return a.forEach((d,s)=>{if(p+=Ve(d,40,52),s<a.length-1){const l=Ze(d,a[s+1]);p+=`<div style="width:7px;height:3px;background:${l==="#ff3333"||l==="#cc2222"?"rgba(255,255,255,0.12)":l};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),n!==void 0&&(p+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${n}</div>`),p+="</div>",p}function ei(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function Ji(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function st(e,t,n,a,p=310,d=310,s=[]){var B;const l=zt[t]||{},c=Mi(t)||oi[t]||[],i={},r=["ATT","MIL","DEF","GK"];for(const U of r)(e[U]||[]).forEach((j,R)=>{i[`${U}${R+1}`]=j});function u(U){const F=l[U];return F?{x:F.x*p,y:F.y*d}:null}let x="";for(const[U,F]of c){const j=u(U),R=u(F);if(!j||!R)continue;const ie=i[U],ce=i[F],oe=Ze(ie,ce);oe==="#00ff88"||oe==="#FFD700"?(x+=`<line x1="${j.x.toFixed(1)}" y1="${j.y.toFixed(1)}" x2="${R.x.toFixed(1)}" y2="${R.y.toFixed(1)}"
        stroke="${oe}" stroke-width="10" stroke-linecap="round" opacity="0.22"/>`,x+=`<line x1="${j.x.toFixed(1)}" y1="${j.y.toFixed(1)}" x2="${R.x.toFixed(1)}" y2="${R.y.toFixed(1)}"
        stroke="${oe}" stroke-width="3.5" stroke-linecap="round" opacity="0.95"/>`):x+=`<line x1="${j.x.toFixed(1)}" y1="${j.y.toFixed(1)}" x2="${R.x.toFixed(1)}" y2="${R.y.toFixed(1)}"
        stroke="${oe}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const g=48,k=64,o=13,$=16,z={normal:"#aaaaaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#111111",legende:"#7a28b8",légende:"#7a28b8"};for(const[U,F]of Object.entries(i)){const j=u(U);if(!j||!F)continue;const R=U.replace(/[0-9]/g,""),ie=ft[R]||"#555",ce=s.includes(F.cardId),oe=n==="attack"&&(["MIL","ATT"].includes(R)||ce)&&!F.used||n==="defense"&&["GK","DEF","MIL"].includes(R)&&!F.used,S=a.includes(F.cardId);let Y;n==="attack"?Y=ce?Math.max(1,Number(F.note_a)||0):R==="MIL"?Number(F.note_m)||0:Number(F.note_a)||0:n==="defense"?Y=R==="GK"?Number(F.note_g)||0:R==="MIL"?Number(F.note_m)||0:Number(F.note_d)||0:Y=Number(R==="GK"?F.note_g:R==="DEF"?F.note_d:R==="MIL"?F.note_m:F.note_a)||0,Y=Y+(F.boost||0);const ee=(j.x-g/2).toFixed(1),D=(j.y-k/2).toFixed(1),Q=z[F==null?void 0:F.rarity]||z.normal;if(F.used){const L=typeof import.meta<"u"&&"/manager-wars/"||"/",q=`${typeof window<"u"&&((B=window.location)==null?void 0:B.origin)||""}${L}icons/carte-dos.png`.replace(/([^:])\/\//g,"$1/");x+=`<rect x="${ee}" y="${D}" width="${g}" height="${k}" rx="5" fill="#161616"/>`,x+=`<image href="${q}" xlink:href="${q}" x="${ee}" y="${D}" width="${g}" height="${k}" preserveAspectRatio="xMidYMid slice"/>`,x+=`<rect x="${ee}" y="${D}" width="${g}" height="${k}" rx="5" fill="none" stroke="${Q}" stroke-width="2" opacity="0.7"/>`,x+=`<rect x="${ee}" y="${D}" width="${g}" height="${k}" rx="5" fill="rgba(0,0,0,0.01)" class="match-used-hit" data-card-id="${F.cardId}" data-role="${R}" style="cursor:pointer"/>`;continue}const ae=S?.45:1,pe=S?"#FFD700":Q,he=S?3:(F==null?void 0:F.rarity)==="legende"||(F==null?void 0:F.rarity)==="légende"||(F==null?void 0:F.rarity)==="pepite"||(F==null?void 0:F.rarity)==="pépite"?2.5:2,xe=k-o-$;x+=`<defs><clipPath id="cp-${U}"><rect x="${ee}" y="${(j.y-k/2+o).toFixed(1)}" width="${g}" height="${xe}"/></clipPath></defs>`,x+=`<rect x="${ee}" y="${D}" width="${g}" height="${k}" rx="5" fill="${ie}" opacity="${ae}"/>`;const ge=ze(F);ge&&(x+=`<image href="${ge}" xlink:href="${ge}" x="${ee}" y="${(j.y-k/2+o).toFixed(1)}" width="${g}" height="${xe}" clip-path="url(#cp-${U})" preserveAspectRatio="xMidYMin slice"/>`),x+=`<rect x="${ee}" y="${D}" width="${g}" height="${o}" rx="4" fill="rgba(255,255,255,0.92)"/>`,x+=`<text x="${j.x.toFixed(1)}" y="${(j.y-k/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(F.name||"").slice(0,9)}</text>`;const O=(j.y+k/2-$).toFixed(1);x+=`<rect x="${ee}" y="${O}" width="${g}" height="${$}" fill="rgba(255,255,255,0.92)"/>`;{const L=ei(F.country_code);L?x+=`<image href="${L}" xlink:href="${L}" x="${(j.x-20).toFixed(1)}" y="${(j.y+k/2-$+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:x+=`<text x="${(j.x-13).toFixed(1)}" y="${(j.y+k/2-$/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${Ji(F.country_code)}</text>`,x+=`<text x="${j.x.toFixed(1)}" y="${(j.y+k/2-$/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${Y}</text>`;const N=Xi(F);N?x+=`<image href="${N}" xlink:href="${N}" x="${(j.x+g/2-14).toFixed(1)}" y="${(j.y+k/2-$+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:F.clubName&&(x+=`<text x="${(j.x+14).toFixed(1)}" y="${(j.y+k/2-$/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(F.clubName||"").slice(0,3).toUpperCase()}</text>`),F.boost&&(x+=`<rect x="${(j.x+g/2-12).toFixed(1)}" y="${(j.y-k/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,x+=`<text x="${(j.x+g/2-5).toFixed(1)}" y="${(j.y-k/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${F.boost}</text>`)}x+=`<rect x="${ee}" y="${D}" width="${g}" height="${k}" rx="5" fill="${S?"rgba(255,255,255,0.12)":"none"}" stroke="${pe}" stroke-width="${he}" opacity="${ae}"/>`,oe&&(x+=`<rect x="${ee}" y="${D}" width="${g}" height="${k}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${S?"selected":""}" data-card-id="${F.cardId}" data-role="${R}" style="cursor:pointer"/>`)}const M=38;return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-M} ${-M} ${p+M*2} ${d+M*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${x}
  ...
  </div>`}async function pi(e,t,n,a){var F;const{state:p,navigate:d,toast:s}=t;Xe(e);const l=p.params||{};if(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!l.deckId)return io(e,t,n);const c=l.deckId;let i,r,u,x;try{const j=await Promise.all([_.from("decks").select("formation,name").eq("id",c).single(),_.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation,
            player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
              clubs(encoded_name,logo_url)))`).eq("deck_id",c).order("slot_order")]);i=j[0].data,u=j[0].error,r=j[1].data,x=j[1].error}catch(j){console.error("[Match] Exception chargement deck:",j),at(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>d("home"));return}if(u||x){console.error("[Match] Erreur Supabase:",u||x),at(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>d("home"));return}const g=(r||[]).filter(j=>{var R;return j.is_starter&&((R=j.card)==null?void 0:R.player)}).map(j=>Zt(j.card,j.position)),k=(r||[]).filter(j=>{var R;return!j.is_starter&&((R=j.card)==null?void 0:R.player)}).map(j=>Zt(j.card,j.position));if(g.length<11){at(e,"⚠️",`Deck incomplet (${g.length}/11).`,"Compléter",()=>d("decks"));return}const o=(r||[]).find(j=>{var R;return((R=j.card)==null?void 0:R.card_type)==="formation"}),$=(i==null?void 0:i.formation)||((F=o==null?void 0:o.card)==null?void 0:F.formation)||"4-4-2",{data:z,error:M}=await _.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",p.profile.id).eq("card_type","game_changer"),{data:B}=await _.from("gc_definitions").select("*").eq("is_active",!0),U=(z||[]).map(j=>({...j,_gcDef:(B==null?void 0:B.find(R=>R.name===j.gc_type||R.id===j.gc_definition_id))||null}));a({deckId:c,formation:$,starters:g,subsRaw:k,gcCardsEnriched:U,gcDefs:B||[]})}async function no(e,t){const{state:n}=t,p=(n.params||{}).matchMode||"vs_ai_easy",d=p.replace("vs_ai_",""),s=p;await pi(e,t,p,async({deckId:l,formation:c,starters:i,subsRaw:r,gcCardsEnriched:u,gcDefs:x})=>{try{const g=gt(i,c),k=await oo(c,d),o=async $=>{try{const{data:z,error:M}=await _.from("matches").insert({home_id:n.profile.id,away_id:null,mode:s,home_deck_id:l,status:"in_progress"}).select().single();if(M){console.error("[MatchIA] Erreur création match:",M),at(e,"⚠️","Impossible de créer le match ("+M.message+").","Retour",()=>t.navigate("home"));return}const B={gcDefs:x||[],matchId:z==null?void 0:z.id,mode:s,difficulty:d,formation:c,homeTeam:g,aiTeam:k,homeSubs:r,subsUsed:0,maxSubs:Math.min(r.length,3),homeScore:0,aiScore:0,gcCards:$,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:n.profile.club_name||"Vous"};ao(e,B,t)}catch(z){console.error("[MatchIA] Exception launchMatch:",z),at(e,"⚠️","Erreur au lancement du match : "+z.message,"Retour",()=>t.navigate("home"))}};if(!u.length){o([]);return}ci(e,u,o)}catch(g){console.error("[MatchIA] Exception setup:",g),at(e,"⚠️","Erreur de préparation du match : "+g.message,"Retour",()=>t.navigate("home"))}})}async function oo(e,t){var s,l;const{data:n}=await _.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(60);if(!n||n.length<11)return ro(e);const a=ut[e]||ut["4-4-2"],p={GK:[],DEF:[],MIL:[],ATT:[]},d=[...n];for(const c of["GK","DEF","MIL","ATT"]){const i=d.filter(k=>k.job===c),r=d.filter(k=>k.job!==c),u=[...i,...r],x=[];for(let k=0;k<a[c];k++){const o=u[k]||d[k];o&&x.push({cardId:"ai-"+o.id+"-"+k,id:o.id,firstname:o.firstname,name:o.surname_encoded,country_code:o.country_code,club_id:o.club_id,job:o.job,job2:o.job2,note_g:Number(o.note_g)||0,note_d:Number(o.note_d)||0,note_m:Number(o.note_m)||0,note_a:Number(o.note_a)||0,rarity:o.rarity,skin:o.skin,hair:o.hair,hair_length:o.hair_length,clubName:((s=o.clubs)==null?void 0:s.encoded_name)||null,clubLogo:((l=o.clubs)==null?void 0:l.logo_url)||null,boost:0,used:!1,_line:c})}const g=ot(x.length);x.forEach((k,o)=>{k._col=g[o]}),p[c]=x}return p}function ro(e){const t=ut[e]||ut["4-4-2"],n={GK:[],DEF:[],MIL:[],ATT:[]},a=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let p=0;for(const d of["GK","DEF","MIL","ATT"]){const s=[];for(let c=0;c<t[d];c++){const i=3+Math.floor(Math.random()*5);s.push({cardId:"fake-"+p,id:"fake-"+p,firstname:"IA",name:a[p%a.length],country_code:"XX",club_id:null,job:d,job2:null,note_g:d==="GK"?i:2,note_d:d==="DEF"?i:2,note_m:d==="MIL"?i:2,note_a:d==="ATT"?i:2,rarity:"normal",boost:0,used:!1,_line:d}),p++}const l=ot(s.length);s.forEach((c,i)=>{c._col=l[i]}),n[d]=s}return n}function ao(e,t,n){e.innerHTML=`
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
  </div>`,setTimeout(()=>so(e,t,n),5e3)}function so(e,t,n){const a=t.homeTeam.MIL||[],p=t.aiTeam.MIL||[];function d(o){return o.reduce(($,z)=>$+we(z,"MIL"),0)}function s(o){let $=0;for(let z=0;z<o.length-1;z++){const M=Ze(o[z],o[z+1]);M==="#00ff88"?$+=2:M==="#FFD700"&&($+=1)}return $}const l=d(a)+s(a),c=d(p)+s(p),i=l>=c;function r(o,$,z,M){return`<div id="duel-row-${M}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0), opacity .5s ease;transform-origin:center">
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${o.map((B,U)=>{const F=U<o.length-1?Ze(B,o[U+1]):null,j=!F||F==="#ff3333"||F==="#cc2222",R=F==="#00ff88"?"+2":F==="#FFD700"?"+1":"";return`
          <div class="duel-card duel-card-${M}" data-idx="${U}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease, transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
            ${Ve({...B,note:we(B,"MIL"),_line:"MIL"},58,78)}
          </div>
          ${U<o.length-1?`<div class="duel-link duel-link-${M}" data-idx="${U}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${j?"rgba(255,255,255,0.12)":F};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${j?"none":`0 0 8px ${F}`}">
  ...
  <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div id="duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center, rgba(10,61,30,0.4), rgba(10,61,30,0.92))"></div>
  </div>`;const u=()=>{const o=($,z)=>e.querySelectorAll($).forEach((M,B)=>{setTimeout(()=>{M.style.opacity="1",M.style.transform="translateY(0) scale(1)"},z+B*90)});o(".duel-card-home",150),o(".duel-card-ai",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach(($,z)=>{setTimeout(()=>{$.style.opacity="1"},z*70)}),900),setTimeout(()=>{const $=e.querySelector("#vs-label");$&&($.style.opacity="1",$.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(z=>z.style.opacity="1")},1250),setTimeout(()=>{x("score-home",l,800),x("score-ai",c,800)},1500)};function x(o,$,z){const M=document.getElementById(o);if(!M)return;const B=performance.now(),U=F=>{const j=Math.min(1,(F-B)/z);M.textContent=Math.round($*(1-Math.pow(1-j,3))),j<1?requestAnimationFrame(U):M.textContent=$};requestAnimationFrame(U)}requestAnimationFrame(u),t.attacker=i?"home":"ai";const g=i?li():null;i&&(t.boostCard={value:g}),t.log.push({type:"duel",title:"Milieu de Terrain",homePlayers:a.map(o=>({name:o.name,note:we(o,"MIL"),portrait:ze(o),job:o.job,country_code:o.country_code,rarity:o.rarity,clubName:o.clubName,clubLogo:o.clubLogo})),aiPlayers:p.map(o=>({name:o.name,note:we(o,"MIL"),portrait:ze(o),job:o.job,country_code:o.country_code,rarity:o.rarity,clubName:o.clubName,clubLogo:o.clubLogo})),homeTotal:l,aiTotal:c,text:`Duel milieu : ${t.clubName} ${l} – ${c} IA → ${i?t.clubName+" attaque":"IA attaque"}`});const k=()=>{t.phase=t.attacker==="home"?"attack":"ai-attack",Me(e,t,n),t.attacker==="ai"&&setTimeout(()=>ti(e,t,n),800)};setTimeout(()=>{const o=document.getElementById("score-home"),$=document.getElementById("score-ai"),z=document.getElementById(i?"duel-row-home":"duel-row-ai"),M=document.getElementById(i?"duel-row-ai":"duel-row-home"),B=i?o:$,U=i?$:o;B&&(B.style.fontSize="80px",B.style.color=i?"#FFD700":"#ff6b6b",B.style.animation="duelPulse .5s ease"+(i?", duelGlow 1.5s ease infinite .5s":"")),U&&(U.style.opacity="0.25"),setTimeout(()=>{z&&(z.style.transformOrigin="center",z.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",z.style.zIndex="5"),setTimeout(()=>{var j;const F=document.getElementById("duel-shock");if(F){const R=(j=M||z)==null?void 0:j.getBoundingClientRect(),ie=e.querySelector(".match-screen").getBoundingClientRect();R&&(F.style.top=R.top-ie.top+R.height/2+"px"),F.style.animation="shockwave .5s ease-out forwards"}M&&(M.style.transformOrigin="center",M.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var j;const F=document.getElementById("duel-finale");F&&(F.innerHTML=`
          ...
          <button id="start-match-btn" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,0.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">
            ▶ Commencer le match
          </button>`,F.style.transition="opacity .45s ease",F.style.opacity="1",F.style.pointerEvents="auto",(j=document.getElementById("start-match-btn"))==null||j.addEventListener("click",k))},600)},700)},2800)}function lo(e){if(e.type==="duel"&&(e.homeTotal!=null||e.aiTotal!=null)){const t=(e.homeTotal||0)>=(e.aiTotal||0);return`
    ...
    <div style="display:flex;align-items:center;gap:4px;${t?"flex-direction:row-reverse":""};background:rgba(255,255,255,0.04);border-radius:8px;padding:5px 8px;border:1px solid rgba(255,255,255,0.07)">
      ...
      ${nt(e.inPlayer||{})}
    </div>`}return`<div style="font-size:11px;color:${e.type==="goal"?"#FFD700":"rgba(255,255,255,0.65)"};font-weight:${e.type==="goal"?700:400};padding:3px 2px">${e.text||""}</div>`}function Me(e,t,n){var B,U,F,j,R,ie,ce,oe;const a=t.selected.map(S=>S.cardId),p=t.usedSubIds||[],d=t.homeSubs.filter(S=>!p.includes(S.cardId)),l=Object.values(t.homeTeam).flat().filter(S=>S.used).length>0&&d.length>0&&t.subsUsed<t.maxSubs,c=!["GK","DEF","MIL","ATT"].some(S=>(t.aiTeam[S]||[]).some(Y=>!Y.used)),i=[...t.homeTeam.MIL||[],...t.homeTeam.ATT||[]].filter(S=>!S.used),r=t.phase==="attack"&&c&&i.length===0?[...t.homeTeam.GK||[],...t.homeTeam.DEF||[]].filter(S=>!S.used).map(S=>S.cardId):[];t.log[t.log.length-1];const u=t.phase==="ai-attack"||t.phase==="ai-defense",x=t.phase==="attack",g=t.phase==="defense",k=t.phase==="finished",o=t.gcCards.filter(S=>!t.usedGc.includes(S.id)),$=t.boostCard&&!t.boostUsed;e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column",e.innerHTML=`
  <style>
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const Y=t.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
          ...
          </div>`}const S=t.log[t.log.length-1];return S?'<div style="padding:2px 4px">'+lo(S)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})()}
    </div>
    ...
    ${(()=>{const S=window.innerWidth>=700,Y=(O,L,N)=>{var Ne,He;const q=(t.gcDefs||[]).find(T=>T.name===O.gc_type),V={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[q==null?void 0:q.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",ne={purple:"#b06ce0",light_blue:"#00d4ef"}[q==null?void 0:q.color]||"#b06ce0",te=(q==null?void 0:q.name)||O.gc_type,$e=(q==null?void 0:q.effect)||((Ne=Ge[O.gc_type])==null?void 0:Ne.desc)||"",Ee=q!=null&&q.image_url?`/manager-wars/icons/${q.image_url}`:null,ke=((He=Ge[O.gc_type])==null?void 0:He.icon)||"⚡",_e=Math.round(N*.22),Ae=Math.round(N*.22),Te=N-_e-Ae,qe=te.length>12?7:9;return`<div class="gc-mini" data-gc-id="${O.id}" data-gc-type="${O.gc_type}"
          style="box-sizing:border-box;width:${L}px;height:${N}px;border-radius:10px;border:2px solid ${ne};background:${V};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
          <div style="height:${_e}px;background:rgba(255,255,255,0.14);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 3px">
            <span style="font-size:${qe}px;font-weight:900;color:#fff;text-align:center;line-height:1.1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${L-6}px">${te}</span>
          ...
          <div style="height:${Te}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
            ${Ee?`<img src="${Ee}" style="max-width:${L-10}px;max-height:${Te-6}px;object-fit:contain">`:`<span style="font-size:${Math.round(Te*.55)}px">${ke}</span>`}
          </div>
        ...
        </div>`},ee=(O,L)=>{var N;return`<div id="boost-card"
  ...
  </div>`;function z(){const S=e.querySelector(".match-screen");if(!S)return;const Y=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);S.style.bottom="auto",S.style.height=Y+"px",S.style.minHeight=Y+"px",S.style.maxHeight=Y+"px",S.style.overflow="hidden";const ee=e.querySelector("#mobile-action-bar"),D=e.querySelector("#mobile-play-area");ee&&D&&(D.style.paddingBottom=ee.offsetHeight+"px")}if(z(),setTimeout(z,120),setTimeout(z,400),setTimeout(z,1e3),t._vvBound||(t._vvBound=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",z),window.visualViewport.addEventListener("scroll",z)),window.addEventListener("resize",z)),function(){const Y=e.querySelector(".terrain-wrapper svg");Y&&(Y.removeAttribute("width"),Y.removeAttribute("height"),Y.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",Y.setAttribute("viewBox","-26 -26 352 352"),Y.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),t._resizeBound||(t._resizeBound=!0,window.addEventListener("resize",()=>{const S=e.querySelector(".terrain-wrapper svg");S&&(S.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0")})),t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase==="attack"||t.phase==="defense"){let S=!1,Y=30;const ee=()=>document.getElementById("match-timer"),D=()=>{const Q=ee();if(!Q)return;const ae=String(Math.floor(Y/60)).padStart(2,"0"),pe=String(Y%60).padStart(2,"0");Q.textContent=` ${ae}:${pe}`,Q.style.color=S?"#ff2222":"#ff9500",Q.style.fontWeight="900"};D(),t._timerInt=setInterval(()=>{if(Y--,Y<0)if(!S)S=!0,Y=15,D();else{clearInterval(t._timerInt),t._timerInt=null,t.homeScore=0,t.aiScore=3;const Q=document.createElement("div");Q.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;color:#fff;padding:24px;text-align:center",Q.innerHTML='<div style="font-size:56px">⏱️</div><div style="font-size:24px;font-weight:900;color:#ff4444">MATCH PERDU PAR FORFAIT</div><div style="font-size:14px;color:rgba(255,255,255,0.6)">Temps écoulé</div>',document.body.appendChild(Q),setTimeout(()=>{Q.remove(),At(e,t,n)},2500)}else D()},1e3)}(B=document.getElementById("match-quit"))==null||B.addEventListener("click",()=>{Oe(e),confirm("Abandonner ? Résultat : défaite 3-0")&&(t.homeScore=0,t.aiScore=3,At(e,t,n))}),(U=document.getElementById("view-ai"))==null||U.addEventListener("click",()=>bo(t,n)),(F=document.getElementById("toggle-history"))==null||F.addEventListener("click",()=>{var S;(S=document.getElementById("match-history-panel"))==null||S.classList.add("open")}),(j=document.getElementById("close-history"))==null||j.addEventListener("click",()=>{var S;(S=document.getElementById("match-history-panel"))==null||S.classList.remove("open")}),(R=document.getElementById("btn-action"))==null||R.addEventListener("click",()=>{t.selected.length!==0&&(x?po(e,t,n):g&&uo(e,t,n))}),(ie=document.getElementById("btn-results"))==null||ie.addEventListener("click",()=>At(e,t,n)),e.querySelectorAll(".match-slot-hit").forEach(S=>{S.addEventListener("click",()=>co(S,t,e,n))}),e.querySelectorAll(".match-used-hit").forEach(S=>{S.addEventListener("click",()=>Yt(e,t,n,null,S.dataset.cardId))}),e.querySelectorAll(".gc-mini").forEach(S=>{S.addEventListener("click",()=>mo(S.dataset.gcId,S.dataset.gcType,e,t,n))}),(ce=document.getElementById("boost-card"))==null||ce.addEventListener("click",()=>ho(e,t,n)),e.querySelectorAll(".sub-btn-col").forEach(S=>{S.addEventListener("click",()=>Yt(e,t,n,S.dataset.subId))}),(oe=document.getElementById("sub-btn-main"))==null||oe.addEventListener("click",()=>Yt(e,t,n))}function co(e,t,n,a){const p=e.dataset.cardId,d=e.dataset.role,s=t.selected.findIndex(l=>l.cardId===p);if(s!==-1)t.selected.splice(s,1);else{if(t.selected.length>=3){a.toast("Maximum 3 joueurs","error");return}const l=(t.homeTeam[d]||[]).find(c=>c.cardId===p);l&&t.selected.push({...l,_role:d,_line:d})}Me(n,t,a)}function ui(e,t,n){e.matchId&&_.from("matches").update({last_player_id:n}).eq("id",e.matchId).then(()=>{})}function po(e,t,n){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),ui(t,n,n.state.profile.id);const a=!["GK","DEF","MIL","ATT"].some(s=>(t.aiTeam[s]||[]).some(l=>!l.used)),p=t.selected.map(s=>{const l=(t.homeTeam[s._role]||[]).find(i=>i.cardId===s.cardId)||s,c=a&&["GK","DEF"].includes(s._role);return{...l,_line:s._role,...c?{note_a:Math.max(1,Number(l.note_a)||0)}:{}}}),d=Rt(p,t.modifiers.home);t.pendingAttack={...d,players:[...p],side:"home"},t.selected.forEach(s=>{const l=(t.homeTeam[s._role]||[]).find(c=>c.cardId===s.cardId);l&&(l.used=!0)}),t.log.push({text:`⚔️ Vous attaquez : ${d.total} (base ${d.base}${d.links?` +${d.links} liens`:""}) — ${t.selected.map(s=>s.name).join(", ")}`,type:"info"}),t.selected=[],t.modifiers.home={},t.phase="ai-defense",Me(e,t,n),setTimeout(()=>fo(e,t,n),1200)}function uo(e,t,n){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),ui(t,n,n.state.profile.id);const a=t.selected.map(l=>({...(t.homeTeam[l._role]||[]).find(i=>i.cardId===l.cardId)||l,_line:l._role})),p=Ot(a,t.modifiers.home);t.selected.forEach(l=>{const c=(t.homeTeam[l._role]||[]).find(i=>i.cardId===l.cardId);c&&(c.used=!0)});const d=Ht(t.pendingAttack.total,p.total,t.modifiers.home),s={type:"duel",title:"Défense",aiPlayers:(t.pendingAttack.players||[]).map(l=>({name:l.name,note:l._line==="MIL"?l.note_m:l.note_a,portrait:ze(l),job:l.job,country_code:l.country_code,rarity:l.rarity,clubName:l.clubName,clubLogo:l.clubLogo})),homePlayers:t.selected.map(l=>{const c=(t.homeTeam[l._role]||[]).find(i=>i.cardId===l.cardId)||l;return{name:c.name,note:(c._line==="GK"?Number(c.note_g)||0:c._line==="MIL"?Number(c.note_m)||0:Number(c.note_d)||0)+(c.boost||0),portrait:ze(c),job:c.job,country_code:c.country_code,rarity:c.rarity,clubName:c.clubName,clubLogo:c.clubLogo}}),homeTotal:p.total,aiTotal:t.pendingAttack.total,isGoal:!1,homeScored:!1,text:""};if(d.shielded)s.text="🛡️ Bouclier ! But annulé.",t.log.push(s);else if(d.goal){t.aiScore++,s.isGoal=!0,s.homeScored=!1,s.text=`⚽ BUT IA ! (${t.pendingAttack.total} > ${p.total})`,t.log.push(s),t.selected=[],t.modifiers.home={},t.pendingAttack=null,Me(e,t,n),Nt(s.aiPlayers,t.homeScore,t.aiScore,!1,()=>{ct(e,t,n,"home-attack")});return}else s.text=`🧤 Défense réussie ! (${p.total} ≥ ${t.pendingAttack.total})`,t.log.push(s);t.selected=[],t.modifiers.home={},t.pendingAttack=null,ct(e,t,n,"home-attack")}function ti(e,t,n){ui(t,n,null);const a=[...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]],p=qi(a,"attack",t.difficulty);if(!p.length){qt(e,t,n);return}const d=Rt(p,t.modifiers.ai);t.pendingAttack={...d,players:p,side:"ai"},p.forEach(i=>{i.used=!0}),t.log.push({text:`🤖 IA attaque : ${d.total} (${p.map(i=>i.name).join(", ")})`,type:"info"}),t.modifiers.ai={};const s=[...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(i=>!i.used),c=(t.homeSubs||[]).filter(i=>!(t.usedSubIds||[]).includes(i.cardId)).length>0&&t.subsUsed<t.maxSubs;if(s.length===0&&!c){t.aiScore++;const i={type:"duel",isGoal:!0,homeScored:!1,aiPlayers:p.map(r=>({name:r.name,note:r._line==="MIL"?r.note_m:r.note_a,portrait:ze(r),job:r.job,country_code:r.country_code,rarity:r.rarity,clubName:r.clubName,clubLogo:r.clubLogo})),aiTotal:d.total,text:"⚽ BUT IA ! (aucun défenseur disponible)"};t.log.push(i),t.pendingAttack=null,Me(e,t,n),Nt(i.aiPlayers,t.homeScore,t.aiScore,!1,()=>{ct(e,t,n,"home-attack")});return}t.phase="defense",Me(e,t,n)}function fo(e,t,n){var i,r;const a=[...t.aiTeam.GK||[],...t.aiTeam.DEF||[],...t.aiTeam.MIL||[]],p=qi(a,"defense",t.difficulty);if(!["GK","DEF","MIL","ATT"].flatMap(u=>(t.aiTeam[u]||[]).filter(x=>!x.used)).length){t.homeScore++;const x={type:"duel",isGoal:!0,homeScored:!0,homePlayers:(((i=t.pendingAttack)==null?void 0:i.players)||[]).map(g=>({name:g.name,note:we(g,g._line||g.job),portrait:ze(g),job:g.job,country_code:g.country_code,rarity:g.rarity,clubName:g.clubName,clubLogo:g.clubLogo})),homeTotal:((r=t.pendingAttack)==null?void 0:r.total)||0,aiTotal:0,text:"⚽ BUT ! L'IA n'a plus de joueurs — but automatique !"};t.log.push(x),t.modifiers.ai={},t.pendingAttack=null,Me(e,t,n),Nt(x.homePlayers,t.homeScore,t.aiScore,!0,()=>{ct(e,t,n,"ai-attack")});return}const s=p.length>0?Ot(p,t.modifiers.ai).total:0;p.forEach(u=>{u.used=!0});const l=Ht(t.pendingAttack.total,s,t.modifiers.ai),c={type:"duel",title:"Attaque",homePlayers:(t.pendingAttack.players||[]).map(u=>({name:u.name,note:u._line==="MIL"?u.note_m:u.note_a,portrait:ze(u),job:u.job,country_code:u.country_code,rarity:u.rarity,clubName:u.clubName,clubLogo:u.clubLogo})),aiPlayers:p.map(u=>({name:u.name,note:u._line==="GK"?u.note_g:u._line==="MIL"?u.note_m:u.note_d,portrait:ze(u),job:u.job,country_code:u.country_code,rarity:u.rarity,clubName:u.clubName,clubLogo:u.clubLogo})),homeTotal:t.pendingAttack.total,aiTotal:s,isGoal:!1,homeScored:!1,text:""};if(l.shielded)c.text="🛡️ Bouclier IA !",t.log.push(c);else if(l.goal){t.homeScore++,c.isGoal=!0,c.homeScored=!0,c.text=`⚽ BUT ! (${t.pendingAttack.total} > ${s})`,t.log.push(c),t.modifiers.ai={},t.pendingAttack=null,Me(e,t,n),Nt(c.homePlayers,t.homeScore,t.aiScore,!0,()=>{ct(e,t,n,"ai-attack")});return}else c.text=`🧤 IA défend (${s} ≥ ${t.pendingAttack.total})`,t.log.push(c);t.modifiers.ai={},t.pendingAttack=null,ct(e,t,n,"ai-attack")}function ct(e,t,n,a){if(t.round++,Qi(t)){At(e,t,n);return}if(a==="home-attack"){if(![...t.homeTeam.MIL||[],...t.homeTeam.ATT||[]].filter(d=>!d.used).length){if(![...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(l=>!l.used).length){qt(e,t,n);return}if(![...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]].filter(l=>!l.used).length){qt(e,t,n);return}setTimeout(()=>ti(e,t,n),100);return}t.phase="attack",Me(e,t,n)}else{if(![...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]].filter(d=>!d.used).length){qt(e,t,n);return}t.phase="ai-attack",Me(e,t,n),setTimeout(()=>ti(e,t,n),800)}}function Qi(e){const t=["MIL","ATT","GK","DEF"].some(a=>(e.homeTeam[a]||[]).some(p=>!p.used)),n=["MIL","ATT","GK","DEF"].some(a=>(e.aiTeam[a]||[]).some(p=>!p.used));return!t&&!n}function qt(e,t,n){Qi(t)?At(e,t,n):(t.phase="attack",Me(e,t,n))}function go(e,t,n){const a=document.createElement("div");a.style.cssText=`
  ...
  `,document.body.appendChild(a);let r=!1;const u=()=>{r||(r=!0,a.remove(),n())};a.addEventListener("click",u),setTimeout(u,2e3)}function _t(e,t="rgba(0,0,0,0.8)"){const n=document.createElement("div");n.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,n.textContent=e,document.body.appendChild(n),setTimeout(()=>n.remove(),2200)}function Yt(e,t,n,a=null,p=null){var g,k;if(t.phase!=="attack"){_t("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){_t(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const d=Object.entries(t.homeTeam).flatMap(([o,$])=>($||[]).filter(z=>z.used).map(z=>({...z,_line:z._line||o}))),s=t.homeSubs.filter(o=>!t.usedSubIds.includes(o.cardId));if(!d.length){_t("Aucun joueur utilisé à remplacer");return}if(!s.length){_t("Aucun remplaçant disponible");return}let l=Math.max(0,d.findIndex(o=>o.cardId===p));const c=((g=d[l])==null?void 0:g._line)||((k=d[l])==null?void 0:k.job);let i=a?Math.max(0,s.findIndex(o=>o.cardId===a)):Math.max(0,s.findIndex(o=>o.job===c)),r=!1;const u=document.createElement("div");u.id="sub-overlay",u.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function x(){var F,j,R,ie,ce,oe;const o=d[l],$=s[i],z=Math.min(130,Math.round((window.innerWidth-90)/2)),M=Math.round(z*1.35),B=S=>`background:rgba(255,255,255,0.12);border:none;color:${S?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${S?"default":"pointer"};flex-shrink:0`;u.innerHTML=`
    ...
    <div style="flex:1;display:flex;gap:0;overflow:hidden">
      ...
      <div id="in-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
        ...
        <div>${$?Ve({...$,used:!1,boost:0},z,M):"<div>—</div>"}</div>
      ...
      <div id="out-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
        ...
        <div>${o?Ve({...o,used:!1,boost:0},z,M):"<div>—</div>"}</div>
    ...
    </div>`,(F=u.querySelector("#sub-close"))==null||F.addEventListener("click",()=>u.remove()),(j=u.querySelector("#out-up"))==null||j.addEventListener("click",()=>{l>0&&(l--,x())}),(R=u.querySelector("#out-down"))==null||R.addEventListener("click",()=>{l<d.length-1&&(l++,x())}),(ie=u.querySelector("#in-up"))==null||ie.addEventListener("click",()=>{i>0&&(i--,x())}),(ce=u.querySelector("#in-down"))==null||ce.addEventListener("click",()=>{i<s.length-1&&(i++,x())});const U=(S,Y,ee,D)=>{const Q=u.querySelector("#"+S);if(!Q)return;let ae=0;Q.addEventListener("touchstart",pe=>{ae=pe.touches[0].clientY},{passive:!0}),Q.addEventListener("touchend",pe=>{const he=pe.changedTouches[0].clientY-ae;if(Math.abs(he)<30)return;const xe=Y();he<0&&xe<D-1?(ee(xe+1),x()):he>0&&xe>0&&(ee(xe-1),x())},{passive:!0})};U("in-panel",()=>i,S=>i=S,s.length),U("out-panel",()=>l,S=>l=S,d.length),(oe=u.querySelector("#sub-confirm"))==null||oe.addEventListener("click",S=>{if(S.preventDefault(),S.stopPropagation(),r)return;r=!0;const Y=d[l],ee=s[i];if(!Y||!ee)return;let D=null,Q=-1;for(const[pe,he]of Object.entries(t.homeTeam)){const xe=(he||[]).findIndex(ge=>ge.cardId===Y.cardId);if(xe!==-1){D=pe,Q=xe;break}}if(Q===-1||!D){_t("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),u.remove();return}const ae={...ee,_line:D,_col:Y._col||0,used:!1,boost:0};t.homeTeam[D].splice(Q,1,ae),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(ee.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:Y.name,firstname:Y.firstname,note:we(Y,D),portrait:ze(Y),job:Y.job,country_code:Y.country_code,rarity:Y.rarity,clubName:Y.clubName,clubLogo:Y.clubLogo},inPlayer:{name:ee.name,firstname:ee.firstname,note:we(ee,D),portrait:ze(ee),job:ee.job,country_code:ee.country_code,rarity:ee.rarity,clubName:ee.clubName,clubLogo:ee.clubLogo},text:`🔄 ${ee.firstname} ${ee.name} remplace ${Y.firstname} ${Y.name}`}),u.remove(),go(Y,ee,()=>Me(e,t,n))})}document.body.appendChild(u),x()}function mo(e,t,n,a,p){var k,o;const d=(a.gcDefs||[]).find($=>$.name===t),s=Ge[t]||{icon:"⚡",desc:"Carte spéciale."},l={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[d==null?void 0:d.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",c={purple:"#b06ce0",light_blue:"#00d4ef"}[d==null?void 0:d.color]||"#b06ce0",i=(d==null?void 0:d.name)||t,r=(d==null?void 0:d.effect)||s.desc,u=d!=null&&d.image_url?`/manager-wars/icons/${d.image_url}`:null,x=s.icon||"⚡",g=document.createElement("div");g.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",g.innerHTML=`
    ...
    </div>`,document.body.appendChild(g),(k=g.querySelector("#gc-back"))==null||k.addEventListener("click",()=>g.remove()),(o=g.querySelector("#gc-use"))==null||o.addEventListener("click",()=>{g.remove(),yo(e,t,n,a,p)})}function Ft(e,t,n,a,p,d){const s=document.createElement("div");s.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let l=[];function c(){var i,r;s.innerHTML=`
    ...
    <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
      ${e.map(u=>{const x=u._line||u.job||"MIL",g={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[x]||"#555",k=we(u,x)+(u.boost||0),o=l.find($=>$.cardId===u.cardId);return`<div class="gc-pick-item" data-cid="${u.cardId}"
    ...
    </div>`,(i=s.querySelector("#gc-picker-close"))==null||i.addEventListener("click",()=>s.remove()),s.querySelectorAll(".gc-pick-item").forEach(u=>{u.addEventListener("click",()=>{const x=u.dataset.cid,g=e.find(o=>o.cardId===x);if(!g)return;const k=l.findIndex(o=>o.cardId===x);k>-1?l.splice(k,1):l.length<t&&l.push(g),c()})}),(r=s.querySelector("#gc-picker-confirm"))==null||r.addEventListener("click",()=>{s.remove(),d(l)})}c(),document.body.appendChild(s)}const xo={BOOST_STAT:({value:e=1,count:t=1,roles:n=[]},a,p,d)=>{const s=Object.entries(a.homeTeam).filter(([l])=>!n.length||n.includes(l)).flatMap(([l,c])=>c.filter(i=>!i.used).map(i=>({...i,_line:l})));return s.length?(Ft(s,t,`Choisir ${t} joueur(s) à booster (+${e})`,p,a,l=>{l.forEach(c=>{const i=(a.homeTeam[c._line]||[]).find(r=>r.cardId===c.cardId);i&&(i.boost=(i.boost||0)+e,a.log.push({text:`⚡ +${e} sur ${i.name}`,type:"info"}))}),Me(p,a,d)}),!0):(a.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),Me(p,a,d),!0)},DEBUFF_STAT:({value:e=1,count:t=1,roles:n=[],target:a="ai"},p,d,s)=>{const l=a==="home"?p.homeTeam:p.aiTeam,c=a==="ai"?"adverse":"allié",i=Object.entries(l).filter(([r])=>!n.length||n.includes(r)).flatMap(([r,u])=>u.filter(x=>!x.used).map(x=>({...x,_line:r})));return i.length?(Ft(i,t,`Choisir ${t} joueur(s) ${c}(s) à débuffer (-${e})`,d,p,r=>{r.forEach(u=>{const g=((a==="home"?p.homeTeam:p.aiTeam)[u._line]||[]).find(k=>k.cardId===u.cardId);g&&(g.boost=(g.boost||0)-e,p.log.push({text:`🎯 -${e} sur ${g.name}${a==="ai"?" (IA)":""}`,type:"info"}))}),Me(d,p,s)}),!0):(p.log.push({text:`🎯 Aucun joueur ${c} disponible`,type:"info"}),Me(d,p,s),!0)},GRAY_PLAYER:({count:e=1,roles:t=[],target:n="ai"},a,p,d)=>{const s=n==="home"?a.homeTeam:a.aiTeam,l=n==="ai"?"adverse":"allié",c=Object.entries(s).filter(([i])=>!t.length||t.includes(i)).flatMap(([i,r])=>r.filter(u=>!u.used).map(u=>({...u,_line:i})));return c.length?(Ft(c,e,`Choisir ${e} joueur(s) ${l}(s) à exclure`,p,a,i=>{i.forEach(r=>{const x=((n==="home"?a.homeTeam:a.aiTeam)[r._line]||[]).find(g=>g.cardId===r.cardId);x&&(x.used=!0,a.log.push({text:`❌ ${x.name}${n==="ai"?" (IA)":""} exclu !`,type:"info"}))}),Me(p,a,d)}),!0):(a.log.push({text:`❌ Aucun joueur ${l} à exclure`,type:"info"}),Me(p,a,d),!0)},REVIVE_PLAYER:({count:e=1,roles:t=[]},n,a,p)=>{const d=Object.entries(n.homeTeam).filter(([s])=>!t.length||t.includes(s)).flatMap(([s,l])=>l.filter(c=>c.used).map(c=>({...c,_line:s})));return d.length?(Ft(d,e,`Choisir ${e} joueur(s) à ressusciter`,a,n,s=>{s.forEach(l=>{const c=(n.homeTeam[l._line]||[]).find(i=>i.cardId===l.cardId);c&&(c.used=!1,n.log.push({text:`💫 ${c.name} ressuscité !`,type:"info"}))}),Me(a,n,p)}),!0):(n.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),Me(a,n,p),!0)},REMOVE_GOAL:({},e)=>(e.aiScore>0?(e.aiScore--,e.log.push({text:"🚫 Dernier but IA annulé !",type:"info"})):e.log.push({text:"🚫 Aucun but à annuler",type:"info"}),!1),ADD_GOAL_DRAW:({},e)=>(e.homeScore===e.aiScore?(e.homeScore++,e.log.push({text:"🎯 But bonus (match nul) !",type:"info"})):e.log.push({text:"🎯 But bonus : non applicable (pas de match nul)",type:"info"}),!1),ADD_SUB:({value:e=1},t)=>(t.maxSubs=(t.maxSubs||3)+e,t.log.push({text:`🔄 +${e} remplacement(s) débloqué(s)`,type:"info"}),!1),CUSTOM:()=>!1};function yo(e,t,n,a,p){a.usedGc.push(e);const d=a.gcDefs||[],s=d.find(c=>c.name===t)||d.find(c=>{var i;return((i=c.name)==null?void 0:i.toLowerCase().trim())===(t==null?void 0:t.toLowerCase().trim())});let l=!1;if(s!=null&&s.effect_type&&s.effect_type!=="CUSTOM"){const c=xo[s.effect_type];c?c(s.effect_params||{},a,n,p)||(l=!0):(p.toast(`Effet "${s.effect_type}" non implémenté`,"error"),l=!0)}else{switch(t){case"Double attaque":a.modifiers.home.doubleAttack=!0,a.log.push({text:"⚡ Double attaque activée !",type:"info"});break;case"Bouclier":a.modifiers.home.shield=!0,a.log.push({text:"🛡️ Bouclier activé !",type:"info"});break;case"Ressusciter":{const c=Object.entries(a.homeTeam).flatMap(([i,r])=>(r||[]).filter(u=>u.used).map(u=>({...u,_line:i})));c.length?(c[0].used=!1,a.log.push({text:`💫 ${c[0].name} ressuscité !`,type:"info"})):a.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"});break}case"Vol de note":a.modifiers.ai.stolenNote=(a.modifiers.ai.stolenNote||0)+1,a.log.push({text:"🎯 -1 à la prochaine attaque IA",type:"info"});break;case"Gel":{const c=[...a.aiTeam.ATT||[],...a.aiTeam.MIL||[]].filter(i=>!i.used);if(c.length){const i=c.sort((r,u)=>we(u,"ATT")-we(r,"ATT"))[0];i.used=!0,a.log.push({text:`❄️ ${i.name} (IA) gelé !`,type:"info"})}break}case"Remplacement+":a.maxSubs++,a.log.push({text:"🔄 +1 remplacement débloqué",type:"info"});break}l=!0}_.from("cards").delete().eq("id",e).then(()=>{}),l&&Me(n,a,p)}function ho(e,t,n){const a=Object.values(t.homeTeam).flat().filter(p=>!p.used);if(!a.length){n.toast("Aucun joueur actif à booster","error");return}n.openModal("⚡ Utiliser le Boost",`<div style="margin-bottom:12px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border-radius:10px;padding:12px;text-align:center;color:#000">
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(d);let l=!1;const c=()=>{l||(l=!0,d.remove(),setTimeout(()=>p(),50))};d.addEventListener("click",c),setTimeout(c,3500)}async function At(e,t,n){var r,u;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:a}=n,p=t.homeScore>t.aiScore,d=t.homeScore===t.aiScore,s=p?"victoire":d?"nul":"defaite",l=kn(t.mode,s);t.matchId&&await _.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:p?a.profile.id:null,home_credits_reward:l,played_at:new Date().toISOString()}).eq("id",t.matchId);const c={credits:(a.profile.credits||0)+l,matches_played:(a.profile.matches_played||0)+1};p?c.wins=(a.profile.wins||0)+1:d?c.draws=(a.profile.draws||0)+1:c.losses=(a.profile.losses||0)+1,await _.from("users").update(c).eq("id",a.profile.id),await n.refreshProfile();const i=document.createElement("div");i.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",i.innerHTML=`
    ...
    </div>`,`<button class="btn btn-primary" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const vo={pepite:{win:2,loss:-1},papyte:{win:1,loss:-2}};async function Zi(e,t){!e&&!t||!e||!t||await Promise.all([Ii(e,"win"),Ii(t,"loss")])}async function Ii(e,t){const{data:n}=await _.from("cards").select(`
      id, current_note,
      player:players(rarity, note_min, note_max)
    `).eq("owner_id",e).eq("card_type","player");if(!(n!=null&&n.length))return;const a=n.filter(d=>{var s,l;return((s=d.player)==null?void 0:s.rarity)==="pepite"||((l=d.player)==null?void 0:l.rarity)==="papyte"});if(!a.length)return;const p=a.map(d=>{const s=vo[d.player.rarity],l=t==="win"?s.win:s.loss,c=d.player.note_min??1,i=d.player.note_max??10,r=d.current_note??i,u=Math.min(i,Math.max(c,r+l));return{id:d.id,current_note:u}});await Promise.all(p.map(d=>_.from("cards").update({current_note:d.current_note}).eq("id",d.id)))}async function en(e,t){var n,a;try{const p=(a=(n=t==null?void 0:t.state)==null?void 0:n.profile)==null?void 0:a.id;try{(_.getChannels?_.getChannels():[]).forEach(s=>{const l=s.topic||"";(l.includes("matchmaking")||l.includes("pvp-match")||l.includes("friend-invite"))&&_.removeChannel(s)})}catch(d){console.warn("[Random] cleanup canaux:",d)}p&&await _.rpc("cancel_matchmaking",{p_user_id:p}).catch(()=>{})}catch{}await pi(e,t,"random",({deckId:p,formation:d,starters:s,subsRaw:l,gcCardsEnriched:c,gcDefs:i})=>{const r=u=>wo(e,t,p,d,s,l,u,i||[]);if(!c.length){r([]);return}ci(e,c,r)})}async function wo(e,t,n,a,p,d,s=[],l=[]){var M;const{state:c,navigate:i,toast:r}=t;let u=!1,x=null;Xe(e),e.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const g=async(B=!0)=>{u=!0,x&&(_.removeChannel(x),x=null),B&&await _.rpc("cancel_matchmaking",{p_user_id:c.profile.id}).catch(()=>{})};(M=document.getElementById("mm-cancel"))==null||M.addEventListener("click",async()=>{try{await g(!0)}catch{}Oe(e),i("home")});const k=async(B,U,F)=>{if(u)return;u=!0,x&&(_.removeChannel(x),x=null);const j=document.getElementById("mm-status");j&&(j.textContent="Adversaire trouvé !"),await new Promise(R=>setTimeout(R,600)),e.isConnected&&tn(e,t,B,F,s,l)},{data:o,error:$}=await _.rpc("try_matchmake",{p_user_id:c.profile.id,p_deck_id:n});if($||!(o!=null&&o.success)){r("Erreur de matchmaking","error"),Oe(e),i("home");return}if(o.matched){k(o.match_id,o.opponent_id,!1);return}const z=document.getElementById("mm-status");z&&(z.textContent="En attente d'un autre joueur..."),x=_.channel("matchmaking-"+c.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${c.profile.id}`},B=>{const U=B.new;U.status==="matched"&&U.match_id&&k(U.match_id,U.matched_with,!0)}).subscribe()}async function _o(e,t,n){const{state:a,navigate:p,toast:d}=t;try{(_.getChannels?_.getChannels():[]).forEach(i=>{const r=i.topic||"";(r.includes("matchmaking")||r.includes("pvp-match")||r.includes("friend-invite"))&&_.removeChannel(i)})}catch{}const{data:s}=await _.from("matches").select("home_id, away_id, status").eq("id",n).single();if(!s){d("Match introuvable","error"),p("home");return}if(s.status==="finished"){d("Ce match est terminé","info"),p("home");return}const l=s.home_id===a.profile.id;tn(e,t,n,l,[],[])}async function tn(e,t,n,a,p=[],d=[]){const{state:s,navigate:l,toast:c}=t,i=a?"p1":"p2",r=a?"p2":"p1",u=(p||[]).map(T=>T.id),x=(p||[]).map(T=>({id:T.id,gc_type:T.gc_type,_gcDef:T._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:g}=await _.from("matches").select("*").eq("id",n).single();if(!g){c("Match introuvable","error"),l("home");return}async function k(){const[{data:T},{data:f},{data:m},{data:h}]=await Promise.all([_.rpc("get_deck_for_match",{p_deck_id:g.home_deck_id}),_.rpc("get_deck_for_match",{p_deck_id:g.away_deck_id}),_.from("users").select("id,pseudo,club_name").eq("id",g.home_id).single(),_.from("users").select("id,pseudo,club_name").eq("id",g.away_id).single()]),y=E=>({cardId:E.card_id,position:E.position,id:E.id,firstname:E.firstname,name:E.surname_encoded,country_code:E.country_code,club_id:E.club_id,job:E.job,job2:E.job2,note_g:Number(E.note_g)||0,note_d:Number(E.note_d)||0,note_m:Number(E.note_m)||0,note_a:Number(E.note_a)||0,rarity:E.rarity,skin:E.skin,hair:E.hair,hair_length:E.hair_length,clubName:E.club_encoded_name||null,clubLogo:E.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}),b=((T==null?void 0:T.starters)||[]).map(E=>y(E)),v=((f==null?void 0:f.starters)||[]).map(E=>y(E)),w=(T==null?void 0:T.formation)||"4-4-2",I=(f==null?void 0:f.formation)||"4-4-2";return{p1Team:gt(b,w),p2Team:gt(v,I),p1Subs:((T==null?void 0:T.subs)||[]).map(E=>y(E)),p2Subs:((f==null?void 0:f.subs)||[]).map(E=>y(E)),p1Formation:w,p2Formation:I,p1Name:(m==null?void 0:m.club_name)||(m==null?void 0:m.pseudo)||"Joueur 1",p2Name:(h==null?void 0:h.club_name)||(h==null?void 0:h.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:a?u:[],gc_p2:a?[]:u,gcCardsFull_p1:a?x:[],gcCardsFull_p2:a?[]:x,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:d||[],lastActionAt:new Date().toISOString()}}let o=g.game_state&&Object.keys(g.game_state).length?g.game_state:null;if(!o)if(a){o=await k();const{data:T}=await _.from("matches").select("game_state").eq("id",n).single();!(T!=null&&T.game_state)||!Object.keys(T.game_state).length?await _.from("matches").update({game_state:o,turn_user_id:g.home_id}).eq("id",n):o=T.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let T=0;T<30&&!o;T++){await new Promise(m=>setTimeout(m,400));const{data:f}=await _.from("matches").select("game_state").eq("id",n).single();f!=null&&f.game_state&&Object.keys(f.game_state).length&&(o=f.game_state)}if(!o){c("Erreur de synchronisation","error"),l("home");return}o.gc_p2=u,o.gcCardsFull_p2=x,await _.from("matches").update({game_state:o}).eq("id",n)}let $=!1,z=null,M=!1;const B=new Set,U=new Set;function F(T){var E,A;try{_.removeChannel(j)}catch{}const f=o[i+"Score"]||0,m=o[r+"Score"]||0;let h,y;T.winner_id?(h=T.winner_id===s.profile.id,y=!1):T.forfeit?(h=f>m,y=!1):(y=f===m,h=f>m),(E=document.getElementById("pvp-end-overlay"))==null||E.remove();const b=document.createElement("div");b.id="pvp-end-overlay",b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const v=y?"🤝":h?"🏆":"😞",w=y?"MATCH NUL":h?"VICTOIRE !":"DÉFAITE",I=y?"#fff":h?"#FFD700":"#ff6b6b";b.innerHTML=`
      <div style="font-size:64px">${v}</div>
      <div style="font-size:26px;font-weight:900;color:${I}">${w}</div>
      <div style="font-size:18px">${o[i+"Name"]} ${f} – ${m} ${o[r+"Name"]}</div>
      ${T.forfeit?`<div style="font-size:13px;color:rgba(255,255,255,0.5)">${h?"L'adversaire a quitté":"Perdu par forfait"}</div>`:""}
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(b),(A=b.querySelector("#pvp-end-home"))==null||A.addEventListener("click",()=>{b.remove(),Oe(e),l("home")})}const j=_.channel("pvp-match-"+n).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${n}`},T=>{const f=T.new;try{if(f.status==="finished"||f.forfeit){if($)return;$=!0,z&&(clearInterval(z),z=null),f.game_state&&(o=f.game_state),F(f);return}if(f.game_state){const m=o;o=f.game_state;const h=o._lastGC;if(h&&h.seq&&!U.has(h.seq)&&(U.add(h.seq),h.by!==i)){xe(h.type,h.by,()=>D());return}const y=m[i+"Score"]||0,b=m[r+"Score"]||0,v=o[i+"Score"]||0,w=o[r+"Score"]||0,I=v>y,E=w>b;if((I||E)&&!B.has(o.round)){B.add(o.round);const A=[...o.log||[]].reverse().find(P=>P.isGoal),C=((A==null?void 0:A.homePlayers)||[]).map(P=>({name:P.name,note:P.note,portrait:P.portrait,job:P.job}));he(C,v,w,I,()=>D());return}D()}}catch(m){console.error("[PvP] crash:",m)}}).subscribe();async function R(T){Object.assign(o,T),o.lastActionAt=new Date().toISOString();const{error:f}=await _.from("matches").update({game_state:o}).eq("id",n);f&&c("Erreur de synchronisation","error");try{D()}catch(m){console.error("[PvP] renderPvpScreen crash:",m)}}async function ie(){if($)return;$=!0,z&&(clearInterval(z),z=null);const T=a?g.away_id:g.home_id,f=a?"p2":"p1",m=a?"p1":"p2",h={...o,[f+"Score"]:3,[m+"Score"]:0,phase:"finished"};try{await _.from("matches").update({status:"finished",forfeit:!0,winner_id:T,home_score:h.p1Score||0,away_score:h.p2Score||0,game_state:h}).eq("id",n)}catch{}try{_.removeChannel(j)}catch{}setTimeout(()=>{Oe(e),l("home")},800)}const ce={BOOST_STAT:({value:T=1,count:f=1,roles:m=[]},h,y)=>{const b=h[i+"Team"],v=Object.entries(b).filter(([w])=>!m.length||m.includes(w)).flatMap(([w,I])=>I.filter(E=>!E.used).map(E=>({...E,_line:w})));if(!v.length){h.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),y(h);return}oe(v,f,`Choisir ${f} joueur(s) à booster (+${T})`,w=>{w.forEach(I=>{const E=(b[I._line]||[]).find(A=>A.cardId===I.cardId);E&&(E.boost=(E.boost||0)+T,h.log.push({text:`⚡ +${T} sur ${E.name}`,type:"info"}))}),h[i+"Team"]=b,y(h)})},DEBUFF_STAT:({value:T=1,count:f=1,roles:m=[],target:h="ai"},y,b)=>{const v=h==="home"?i:r,w=y[v+"Team"],I=h==="home"?"allié":"adverse",E=Object.entries(w).filter(([A])=>!m.length||m.includes(A)).flatMap(([A,C])=>C.filter(P=>!P.used).map(P=>({...P,_line:A})));if(!E.length){y.log.push({text:`🎯 Aucun joueur ${I}`,type:"info"}),b(y);return}oe(E,f,`Choisir ${f} joueur(s) ${I}(s) (-${T})`,A=>{A.forEach(C=>{const P=(w[C._line]||[]).find(W=>W.cardId===C.cardId);P&&(P.boost=(P.boost||0)-T,y.log.push({text:`🎯 -${T} sur ${P.name}`,type:"info"}))}),y[v+"Team"]=w,b(y)})},GRAY_PLAYER:({count:T=1,roles:f=[],target:m="ai"},h,y)=>{const b=m==="home"?i:r,v=h[b+"Team"],w=m==="home"?"allié":"adverse",I=Object.entries(v).filter(([E])=>!f.length||f.includes(E)).flatMap(([E,A])=>A.filter(C=>!C.used).map(C=>({...C,_line:E})));if(!I.length){h.log.push({text:`❌ Aucun joueur ${w}`,type:"info"}),y(h);return}oe(I,T,`Choisir ${T} joueur(s) ${w}(s) à exclure`,E=>{E.forEach(A=>{const C=(v[A._line]||[]).find(P=>P.cardId===A.cardId);C&&(C.used=!0,h.log.push({text:`❌ ${C.name} exclu !`,type:"info"}))}),h[b+"Team"]=v,y(h)})},REVIVE_PLAYER:({count:T=1,roles:f=[]},m,h)=>{const y=m[i+"Team"],b=Object.entries(y).filter(([v])=>!f.length||f.includes(v)).flatMap(([v,w])=>w.filter(I=>I.used).map(I=>({...I,_line:v})));if(!b.length){m.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),h(m);return}oe(b,T,`Choisir ${T} joueur(s) à ressusciter`,v=>{v.forEach(w=>{const I=(y[w._line]||[]).find(E=>E.cardId===w.cardId);I&&(I.used=!1,m.log.push({text:`💫 ${I.name} ressuscité !`,type:"info"}))}),m[i+"Team"]=y,h(m)})},REMOVE_GOAL:({},T,f)=>{const m=r+"Score";T[m]>0?(T[m]--,T.log.push({text:"🚫 Dernier but annulé !",type:"info"})):T.log.push({text:"🚫 Aucun but à annuler",type:"info"}),f(T)},ADD_GOAL_DRAW:({},T,f)=>{T[i+"Score"]===T[r+"Score"]?(T[i+"Score"]++,T.log.push({text:"🎯 But bonus !",type:"info"})):T.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),f(T)},ADD_SUB:({value:T=1},f,m)=>{f.maxSubs=(f.maxSubs||3)+T,f.log.push({text:`🔄 +${T} remplacement(s)`,type:"info"}),m(f)},CUSTOM:({},T,f)=>f(T)};function oe(T,f,m,h){const y=document.createElement("div");y.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let b=[];function v(){var I,E;const w=T.map(A=>{const C={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[A._line]||"#555",P=we(A,A._line)+(A.boost||0),le=b.find(H=>H.cardId===A.cardId)?"#FFD700":"rgba(255,255,255,0.25)",J=A.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${A.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${le};background:${C};overflow:hidden;cursor:pointer;${J}">
        ...
        </div>`,(I=y.querySelector("#pp-close"))==null||I.addEventListener("click",()=>y.remove()),y.querySelectorAll(".pp-item").forEach(A=>{A.addEventListener("click",()=>{const C=A.dataset.cid,P=T.find(le=>le.cardId===C),W=b.findIndex(le=>le.cardId===C);P&&(W>-1?b.splice(W,1):b.length<f&&b.push(P),v())})}),(E=y.querySelector("#pp-confirm"))==null||E.addEventListener("click",()=>{y.remove(),h(b)})}v(),document.body.appendChild(y)}async function S(T,f){const h=(o["gcCardsFull_"+i]||[]).find(w=>w.id===T),y=(h==null?void 0:h._gcDef)||(o.gcDefs||[]).find(w=>{var I;return w.name===f||((I=w.name)==null?void 0:I.toLowerCase().trim())===(f==null?void 0:f.toLowerCase().trim())}),b=[...o["usedGc_"+i]||[],T],v={type:f,by:i,seq:(o._gcAnimSeq||0)+1};U.add(v.seq),xe(f,i,async()=>{if(y!=null&&y.effect_type&&y.effect_type!=="CUSTOM"){const I=ce[y.effect_type];if(I){const E={...o};I(y.effect_params||{},E,async A=>{A["usedGc_"+i]=b,A._lastGC=v,A._gcAnimSeq=v.seq,await R(A)});return}}const w={...o};switch(f){case"Remplacement+":w.maxSubs=(w.maxSubs||3)+1,w.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const I=r+"Score";w[I]>0&&(w[I]--,w.log.push({text:"🚫 But annulé",type:"info"}));break}}w["usedGc_"+i]=b,w._lastGC=v,w._gcAnimSeq=v.seq,await R(w)})}function Y(T,f){const m="usedCardIds_"+T,h=new Set(o[m]||[]);f.forEach(y=>h.add(y)),o[m]=[...h]}function ee(){for(const T of["p1","p2"]){const f=new Set(o["usedCardIds_"+T]||[]),m=o[T+"Team"];if(m)for(const h of["GK","DEF","MIL","ATT"])(m[h]||[]).forEach(y=>{y.used=f.has(y.cardId)})}}function D(){var Be,Qe,mt,xt,yt,ht;if(o.phase==="reveal")return Q();if(o.phase==="midfield")return pe();if(o.phase==="finished")return He();const T=o[i+"Team"],f=o[r+"Team"];ee();const m=o[i+"Score"],h=o[r+"Score"],y=o[i+"Name"],b=o[r+"Name"],v=o.phase===i+"-attack",w=o.phase===i+"-defense",I=Array.isArray(o["selected_"+i])?o["selected_"+i]:[],E=I.map(X=>X.cardId),A=window.innerWidth>=700,C=o[i+"Subs"]||[],P=o["usedSubIds_"+i]||[],W=C.filter(X=>!P.includes(X.cardId)),le=o["gcCardsFull_"+i]||[],J=o["usedGc_"+i]||[],H=le.filter(X=>!J.includes(X.id)),K=o.boostOwner===i&&!o.boostUsed,Z=!["GK","DEF","MIL","ATT"].some(X=>(f[X]||[]).some(G=>!G.used)),de=[...T.MIL||[],...T.ATT||[]].filter(X=>!X.used),ue=v&&Z&&de.length===0?[...T.GK||[],...T.DEF||[]].filter(X=>!X.used).map(X=>X.cardId):[];function fe(X,G,re){var Ct,wt;const se=X._gcDef,ye={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[se==null?void 0:se.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",Ye={purple:"#b06ce0",light_blue:"#00d4ef"}[se==null?void 0:se.color]||"#b06ce0",We=(se==null?void 0:se.name)||X.gc_type,Je=(se==null?void 0:se.effect)||((Ct=Ge[X.gc_type])==null?void 0:Ct.desc)||"",et=se!=null&&se.image_url?`/manager-wars/icons/${se.image_url}`:null,Mt=((wt=Ge[X.gc_type])==null?void 0:wt.icon)||"⚡",St=Math.round(re*.22),bt=Math.round(re*.22),lt=re-St-bt,vt=We.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${X.id}" data-gc-type="${X.gc_type}"
    ...
    </div>`,i==="p1"&&setTimeout(async()=>{await R({phase:"midfield"})},5e3)}let ae=!1;function pe(){if(ae)return;const T=o[i+"Team"].MIL||[],f=o[r+"Team"].MIL||[];function m(H){return H.reduce((K,Z)=>K+we(Z,"MIL"),0)}function h(H){let K=0;for(let Z=0;Z<H.length-1;Z++){const de=Ze(H[Z],H[Z+1]);de==="#00ff88"?K+=2:de==="#FFD700"&&(K+=1)}return K}const y=m(T)+h(T),b=m(f)+h(f),v=y>=b;function w(H,K,Z){return`<div id="duel-row-${Z}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(b);let I=!1;const E=()=>{I||(I=!0,b.remove(),setTimeout(()=>y(),50))};b.addEventListener("click",E),setTimeout(E,3500)}function xe(T,f,m){var J,H;const h=(o.gcDefs||[]).find(K=>{var Z;return K.name===T||((Z=K.name)==null?void 0:Z.toLowerCase().trim())===(T==null?void 0:T.toLowerCase().trim())}),y={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[h==null?void 0:h.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",b={purple:"#b06ce0",light_blue:"#00d4ef"}[h==null?void 0:h.color]||"#b06ce0",v=(h==null?void 0:h.name)||T,w=(h==null?void 0:h.effect)||((J=Ge[T])==null?void 0:J.desc)||"",I=h!=null&&h.image_url?`/manager-wars/icons/${h.image_url}`:null,E=((H=Ge[T])==null?void 0:H.icon)||"⚡",C=f===i?"Vous":o[f+"Name"]||"Adversaire",P=document.createElement("div");P.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",P.innerHTML=`
      <style>
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(P);let W=!1;const le=()=>{W||(W=!0,P.remove(),setTimeout(()=>m&&m(),50))};P.addEventListener("click",le),setTimeout(le,3e3)}function ge(T,f){var P,W,le,J;const h=(o["gcCardsFull_"+i]||[]).find(H=>H.id===T),y=h==null?void 0:h._gcDef,b={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[y==null?void 0:y.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",v={purple:"#b06ce0",light_blue:"#00d4ef"}[y==null?void 0:y.color]||"#b06ce0",w=(y==null?void 0:y.name)||f,I=(y==null?void 0:y.effect)||((P=Ge[f])==null?void 0:P.desc)||"Carte spéciale.",E=y!=null&&y.image_url?`/manager-wars/icons/${y.image_url}`:null,A=((W=Ge[f])==null?void 0:W.icon)||"⚡",C=document.createElement("div");C.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",C.innerHTML=`
      ...
      </div>`,document.body.appendChild(C),(le=C.querySelector("#pvp-gc-back"))==null||le.addEventListener("click",()=>C.remove()),(J=C.querySelector("#pvp-gc-use"))==null||J.addEventListener("click",()=>{C.remove(),S(T,f)})}function O(){var h;const T=o[i+"Team"],f=Object.entries(T).flatMap(([y,b])=>(b||[]).filter(v=>!v.used).map(v=>({...v,_line:y})));if(!f.length)return;const m=document.createElement("div");m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",m.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
      ...
      </div>`,document.body.appendChild(m),(h=m.querySelector("#bp-close"))==null||h.addEventListener("click",()=>m.remove()),m.querySelectorAll(".bp-item").forEach(y=>{y.addEventListener("click",async()=>{const b=y.dataset.cid,v=f.find(I=>I.cardId===b);if(!v)return;const w=(T[v._line]||[]).find(I=>I.cardId===b);w&&(w.boost=(w.boost||0)+o.boostValue),m.remove(),await R({[i+"Team"]:T,boostUsed:!0})})})}function L(T=null){var W,le;if(!(o.phase===i+"-attack")){c("Remplacement uniquement avant votre attaque","warning");return}const m=o[i+"Team"],h=o["usedSubIds_"+i]||[],y=o.maxSubs||3;if(h.length>=y){c(`Maximum ${y} remplacements atteint`,"warning");return}const b=Object.entries(m).flatMap(([J,H])=>(H||[]).filter(K=>K.used).map(K=>({...K,_line:J}))),v=(o[i+"Subs"]||[]).filter(J=>!h.includes(J.cardId));if(!b.length){c("Aucun joueur utilisé à remplacer","warning");return}if(!v.length){c("Aucun remplaçant disponible","warning");return}let w=Math.max(0,b.findIndex(J=>J.cardId===T));const I=((W=b[w])==null?void 0:W._line)||((le=b[w])==null?void 0:le.job);let E=Math.max(0,v.findIndex(J=>J.job===I)),A=!1;const C=document.createElement("div");C.id="pvp-sub-overlay",C.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function P(){var fe,me,be,Se,Pe,Fe;const J=b[w],H=v[E],K=Math.min(130,Math.round((window.innerWidth-90)/2)),Z=Math.round(K*1.35),de=Ie=>`background:rgba(255,255,255,0.12);border:none;color:${Ie?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Ie?"default":"pointer"};flex-shrink:0`;C.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      </div>`,(fe=C.querySelector("#psub-close"))==null||fe.addEventListener("click",()=>C.remove()),(me=C.querySelector("#pout-up"))==null||me.addEventListener("click",()=>{w>0&&(w--,P())}),(be=C.querySelector("#pout-down"))==null||be.addEventListener("click",()=>{w<b.length-1&&(w++,P())}),(Se=C.querySelector("#pin-up"))==null||Se.addEventListener("click",()=>{E>0&&(E--,P())}),(Pe=C.querySelector("#pin-down"))==null||Pe.addEventListener("click",()=>{E<v.length-1&&(E++,P())});const ue=(Ie,Le,je,De)=>{const Re=C.querySelector("#"+Ie);if(!Re)return;let Ue=0;Re.addEventListener("touchstart",Ke=>{Ue=Ke.touches[0].clientY},{passive:!0}),Re.addEventListener("touchend",Ke=>{const Ce=Ke.changedTouches[0].clientY-Ue;if(Math.abs(Ce)<30)return;const Be=Le();Ce<0&&Be<De-1?(je(Be+1),P()):Ce>0&&Be>0&&(je(Be-1),P())},{passive:!0})};ue("pin-panel",()=>E,Ie=>E=Ie,v.length),ue("pout-panel",()=>w,Ie=>w=Ie,b.length),(Fe=C.querySelector("#psub-confirm"))==null||Fe.addEventListener("click",async Ie=>{if(Ie.preventDefault(),Ie.stopPropagation(),A)return;A=!0;const Le=b[w],je=v[E];if(!Le||!je)return;const De=Le._line,Re=(m[De]||[]).findIndex(Ce=>Ce.cardId===Le.cardId);if(Re===-1){c("Erreur : joueur introuvable","error"),C.remove();return}const Ue={...je,_line:De,position:Le.position,used:!1,boost:0};m[De].splice(Re,1,Ue);const Ke=[...h,je.cardId];C.remove(),N(Le,je,async()=>{await R({[i+"Team"]:m,[r+"Team"]:o[r+"Team"],["usedSubIds_"+i]:Ke})})})}document.body.appendChild(C),P()}function N(T,f,m){const h={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},y=document.createElement("div");y.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const b=(I,E,A)=>`<div style="text-align:center">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:6px">Appuyer pour continuer</div>`,document.body.appendChild(y);let v=!1;const w=()=>{v||(v=!0,y.remove(),setTimeout(()=>m(),50))};y.addEventListener("click",w),setTimeout(w,2200)}function q(){var f;const T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:20px;overflow-y:auto",T.innerHTML=`
      <div style="font-size:12px;color:rgba(255,255,255,0.5);letter-spacing:2px;text-transform:uppercase">Équipe adverse</div>
      <div style="font-size:18px;font-weight:900;color:#ff6b6b">${o[r+"Name"]}</div>
      <div style="width:min(90vw,420px)">${st(o[r+"Team"],o[r+"Formation"],null,[],300,300)}</div>
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(T),(f=T.querySelector("#pvp-opp-close"))==null||f.addEventListener("click",()=>T.remove())}function V(){var m;const T=o.log||[],f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column",f.innerHTML=`
      ...
      </div>`,document.body.appendChild(f),(m=f.querySelector("#pvp-hist-close"))==null||m.addEventListener("click",()=>f.remove())}async function ne(){if(o.phase!==i+"-attack")return;const T=i==="p1"?"p2":"p1",f=(o.round||0)+1,m=[...o.log||[]];m.push({type:"info",text:`⏭️ ${o[i+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const h=Te(o),y=Ae(T),b=h||!y?"finished":T+"-attack";await R({["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},pendingAttack:null,phase:b,attacker:T,round:f,log:m}),b==="finished"&&await Ne(o)}async function te(){const T=o[i+"Team"],f=!["GK","DEF","MIL","ATT"].some(b=>(o[r+"Team"][b]||[]).some(v=>!v.used)),m=(o["selected_"+i]||[]).map(b=>{const v=(T[b._role]||[]).find(P=>P.cardId===b.cardId)||b,w=f&&["GK","DEF"].includes(b._role),I=T[b._role]||[],E=I.findIndex(P=>P.cardId===b.cardId),A=ot(I.length),C=E>=0?A[E]:v._col??1;return{...v,_line:b._role,_col:C,...w?{note_a:Math.max(1,Number(v.note_a)||0)}:{}}});if(!m.length)return;const h=Rt(m,o.modifiers[i]||{});Y(i,m.map(b=>b.cardId)),m.forEach(b=>{const v=(T[b._role]||[]).find(w=>w.cardId===b.cardId);v&&(v.used=!0)}),o["selected_"+i]=[],D();const y=[...o.log||[]];if(f){const b=(o[i+"Score"]||0)+1,v=m.map(C=>({name:C.name,note:we(C,C._line||"ATT"),portrait:ze(C),job:C.job}));y.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:v,homeTotal:h.total,aiTotal:0});const w=(o.round||0)+1,I=i==="p1"?"p2":"p1",E={...o,[i+"Team"]:T,[i+"Score"]:b},A=Te(E);B.add(w),he(v,b,o[r+"Score"]||0,!0,async()=>{await R({[i+"Team"]:T,[i+"Score"]:b,["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},pendingAttack:null,phase:A?"finished":I+"-attack",attacker:I,round:w,log:y}),A&&await Ne({...o,[i+"Score"]:b})});return}y.push({type:"pending",text:`⚔️ ${o[i+"Name"]} attaque (${h.total})`}),await R({[i+"Team"]:T,[r+"Team"]:o[r+"Team"],pendingAttack:{...h,players:m,side:i},["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},phase:r+"-defense",log:y})}async function $e(){const T=o[i+"Team"],f=(o["selected_"+i]||[]).map(J=>{const H=(T[J._role]||[]).find(fe=>fe.cardId===J.cardId)||J,K=T[J._role]||[],Z=K.findIndex(fe=>fe.cardId===J.cardId),de=ot(K.length),ue=Z>=0?de[Z]:H._col??1;return{...H,_line:J._role,_col:ue}}),m=Ot(f,o.modifiers[i]||{});Y(i,f.map(J=>J.cardId)),f.forEach(J=>{const H=(T[J._role]||[]).find(K=>K.cardId===J.cardId);H&&(H.used=!0)}),o["selected_"+i]=[],D();const h=Ht(o.pendingAttack.total,m.total,o.modifiers[i]||{}),y=o.pendingAttack.side,b=h==="attack"||(h==null?void 0:h.goal),v=y==="p1"?"p2":"p1",w=(o.round||0)+1,I=(o.pendingAttack.players||[]).map(J=>({name:J.name,note:we(J,J._line||"ATT"),portrait:ze(J),job:J.job})),E=[...o.log||[]];E.push({type:"duel",isGoal:b,homeScored:b&&y===i,text:b?`⚽ BUT de ${o[y+"Name"]} ! (${o.pendingAttack.total} vs ${m.total})`:`✋ Attaque stoppée (${o.pendingAttack.total} vs ${m.total})`,homePlayers:I,aiPlayers:f.map(J=>({name:J.name,note:we(J,J._line||"DEF"),portrait:ze(J),job:J.job})),homeTotal:o.pendingAttack.total,aiTotal:m.total});const A=b?(o[y+"Score"]||0)+1:o[y+"Score"]||0,C={...o,[i+"Team"]:T,[y+"Score"]:A},P=Te(C),W=P?"finished":v+"-attack",le=async()=>{await R({[i+"Team"]:T,[r+"Team"]:o[r+"Team"],[y+"Score"]:A,["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},pendingAttack:null,phase:W,attacker:v,round:w,log:E}),(W==="finished"||P)&&await Ne({...o,[y+"Score"]:A})};if(b){const J=y===i,H=J?A:o[i+"Score"]||0,K=J?o[r+"Score"]||0:A;B.add(w),he(I,H,K,J,le)}else await le()}function Ee(T){return["MIL","ATT"].some(f=>(T[f]||[]).some(m=>!m.used))}function ke(T){return["GK","DEF","MIL","ATT"].some(f=>(T[f]||[]).some(m=>!m.used))}function _e(T){return ke(T)&&!Ee(T)}function Ae(T){const f=o[T+"Team"],m=o[(T==="p1"?"p2":"p1")+"Team"];return!!(Ee(f)||!ke(m)&&_e(f))}function Te(T){const f=["MIL","ATT"].some(E=>(T.p1Team[E]||[]).some(A=>!A.used)),m=["MIL","ATT"].some(E=>(T.p2Team[E]||[]).some(A=>!A.used)),h=ke(T.p1Team),y=ke(T.p2Team);return!f&&!(!y&&h)&&(!m&&!(!h&&y))}function qe(T){const f=T.p1Score||0,m=T.p2Score||0;return f===m?null:f>m?g.home_id:g.away_id}async function Ne(T){try{const f=qe(T),m=f?g.home_id===f?g.away_id:g.home_id:null;await _.from("matches").update({status:"finished",winner_id:f,home_score:T.p1Score||0,away_score:T.p2Score||0}).eq("id",n),f&&m&&Zi(f,m).catch(()=>{})}catch(f){console.error("[PvP] finishMatch:",f)}}function He(){var y;const T=o[i+"Score"],f=o[r+"Score"],m=T>f,h=T===f;Xe(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
    ...
    </div>`,(y=document.getElementById("pvp-home"))==null||y.addEventListener("click",()=>{try{_.removeChannel(j)}catch{}Oe(e),l("home")})}D()}const $o=Object.freeze(Object.defineProperty({__proto__:null,renderMatchRandom:en,resumePvpMatch:_o},Symbol.toStringTag,{value:"Module"}));async function ko(e,t,n,a){var p,d;try{const s=(d=(p=t==null?void 0:t.state)==null?void 0:p.profile)==null?void 0:d.id;try{(_.getChannels?_.getChannels():[]).forEach(c=>{const i=c.topic||"";(i.includes("matchmaking")||i.includes("pvp-match"))&&_.removeChannel(c)})}catch(l){console.warn("[FriendMatch] cleanup canaux:",l)}s&&(await _.rpc("cancel_matchmaking",{p_user_id:s}).catch(()=>{}),await _.from("matchmaking_queue").delete().eq("user_id",s).then(()=>{},()=>{}))}catch{}await pi(e,t,"random",({deckId:s,formation:l,starters:c,subsRaw:i,gcCardsEnriched:r,gcDefs:u})=>{const x=g=>Eo(e,t,s,l,c,i,g,u||[],n,a);if(!r.length){x([]);return}ci(e,r,x)})}async function Eo(e,t,n,a,p,d,s=[],l=[],c,i){var ce;const{state:r,navigate:u,toast:x}=t,g=r.profile.id;let k=!1,o=null;Xe(e);try{await _.rpc("cancel_matchmaking",{p_user_id:g})}catch{}try{await _.from("matchmaking_queue").delete().eq("user_id",g)}catch{}const $=(oe,S,Y,ee)=>{var D;e.innerHTML=`
      <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:20px;background:linear-gradient(180deg,#0a1628,#1a0a2e);padding:24px;text-align:center;color:#fff">
        ...
        ${!S&&oe?'<div style="font-size:13px;color:rgba(255,255,255,0.5);animation:lobbyPulse 1.5s ease-in-out infinite">En attente de ton ami...</div>':""}
      ...
      </div>`,(D=document.getElementById("lobby-cancel"))==null||D.addEventListener("click",async()=>{k=!0,o&&(_.removeChannel(o),o=null),Y&&await _.from("friend_match_invites").update({status:"declined"}).eq("id",Y),Oe(e),u("home")})},z=async(oe,S)=>{k=!0,o&&(_.removeChannel(o),o=null),await new Promise(Y=>setTimeout(Y,600)),e.isConnected&&Ao(e,t,oe,S,s,l)},{data:M}=await _.from("friend_match_invites").select("*").eq("status","pending").or(`and(inviter_id.eq.${g},invitee_id.eq.${c}),and(inviter_id.eq.${c},invitee_id.eq.${g})`).order("created_at",{ascending:!1}).limit(1).maybeSingle();let B,U;if(M&&M.inviter_id===c)U=!1,B=M.id,await _.from("friend_match_invites").update({invitee_deck_id:n,invitee_ready:!0,status:"accepted"}).eq("id",B);else{U=!0;const{data:oe,error:S}=await _.from("friend_match_invites").insert({inviter_id:g,invitee_id:c,friend_id:c,inviter_deck_id:n,inviter_ready:!0,status:"pending"}).select().single();if(S||!oe){x("Erreur création invitation","error"),Oe(e),u("home");return}B=oe.id}if($(!0,!U,B),!U){const oe={home_id:c,away_id:g,home_deck_id:M.inviter_deck_id,away_deck_id:n,status:"active",mode:"friend"},S=await _.from("matches").insert(oe).select().single(),Y=S.data;if(!Y){x("Erreur création match: "+(((ce=S.error)==null?void 0:ce.message)||""),"error"),Oe(e),u("home");return}await _.from("friend_match_invites").update({match_id:Y.id,status:"matched"}).eq("id",B),z(Y.id,!1);return}let F=!1;const j=oe=>{k||F||(oe.status==="matched"&&oe.match_id?(F=!0,clearInterval(R),clearInterval(ie),z(oe.match_id,!0)):oe.status==="declined"?(clearInterval(R),clearInterval(ie),x(`${i} a décliné l'invitation`,"warning"),Oe(e),u("home")):oe.invitee_ready&&$(!0,!0,B))},R=setInterval(()=>{if(k){clearInterval(R);return}_.from("matchmaking_queue").delete().eq("user_id",g).then(()=>{},()=>{})},3e3),ie=setInterval(async()=>{if(k||F){clearInterval(ie);return}const{data:oe}=await _.from("friend_match_invites").select("*").eq("id",B).maybeSingle();oe&&j(oe)},1200);o=_.channel("friend-invite-"+B).on("postgres_changes",{event:"UPDATE",schema:"public",table:"friend_match_invites",filter:`id=eq.${B}`},oe=>j(oe.new)).subscribe()}async function Ao(e,t,n,a,p=[],d=[]){const{state:s,navigate:l,toast:c}=t,i=a?"p1":"p2",r=a?"p2":"p1",u=(p||[]).map(f=>f.id),x=(p||[]).map(f=>({id:f.id,gc_type:f.gc_type,_gcDef:f._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:g}=await _.from("matches").select("*").eq("id",n).single();if(!g){c("Match introuvable","error"),l("home");return}async function k(){const[{data:f},{data:m},{data:h},{data:y}]=await Promise.all([_.rpc("get_deck_for_match",{p_deck_id:g.home_deck_id}),_.rpc("get_deck_for_match",{p_deck_id:g.away_deck_id}),_.from("users").select("id,pseudo,club_name").eq("id",g.home_id).single(),_.from("users").select("id,pseudo,club_name").eq("id",g.away_id).single()]),b=A=>({cardId:A.card_id,position:A.position,id:A.id,firstname:A.firstname,name:A.surname_encoded,country_code:A.country_code,club_id:A.club_id,job:A.job,job2:A.job2,note_g:Number(A.note_g)||0,note_d:Number(A.note_d)||0,note_m:Number(A.note_m)||0,note_a:Number(A.note_a)||0,rarity:A.rarity,skin:A.skin,hair:A.hair,hair_length:A.hair_length,clubName:A.club_encoded_name||null,clubLogo:A.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}),v=((f==null?void 0:f.starters)||[]).map(A=>b(A)),w=((m==null?void 0:m.starters)||[]).map(A=>b(A)),I=(f==null?void 0:f.formation)||"4-4-2",E=(m==null?void 0:m.formation)||"4-4-2";return{p1Team:gt(v,I),p2Team:gt(w,E),p1Subs:((f==null?void 0:f.subs)||[]).map(A=>b(A)),p2Subs:((m==null?void 0:m.subs)||[]).map(A=>b(A)),p1Formation:I,p2Formation:E,p1Name:(h==null?void 0:h.club_name)||(h==null?void 0:h.pseudo)||"Joueur 1",p2Name:(y==null?void 0:y.club_name)||(y==null?void 0:y.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:a?u:[],gc_p2:a?[]:u,gcCardsFull_p1:a?x:[],gcCardsFull_p2:a?[]:x,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:d||[],lastActionAt:new Date().toISOString()}}let o=g.game_state&&Object.keys(g.game_state).length?g.game_state:null;if(!o)if(a){o=await k();const{data:f}=await _.from("matches").select("game_state").eq("id",n).single();!(f!=null&&f.game_state)||!Object.keys(f.game_state).length?await _.from("matches").update({game_state:o,turn_user_id:g.home_id}).eq("id",n):o=f.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let f=0;f<60&&!o;f++){await new Promise(h=>setTimeout(h,400));const{data:m}=await _.from("matches").select("game_state").eq("id",n).single();m!=null&&m.game_state&&Object.keys(m.game_state).length&&(o=m.game_state)}if(!o){c("Erreur de synchronisation","error"),l("home");return}o.gc_p2=u,o.gcCardsFull_p2=x,await _.from("matches").update({game_state:o}).eq("id",n)}let $=!1,z=!1,M=null,B=!1;const U=new Set,F=new Set;function j(f){var A,C;try{_.removeChannel(R)}catch{}const m=o[i+"Score"]||0,h=o[r+"Score"]||0;if(!z){const P=o.p1Score||0,W=o.p2Score||0,le=(o.usedGc_p1||[]).length,J=(o.usedGc_p2||[]).length,H=f.winner_id||(P>W?g.home_id:W>P?g.away_id:null);(H?s.profile.id===H:s.profile.id<(a?g.away_id:g.home_id))&&(z=!0,hn({player1Id:g.home_id,player2Id:g.away_id,score1:P,score2:W,gc1:le,gc2:J}).catch(Z=>console.warn("[FriendMatch] updateStats:",Z)))}let y,b;f.winner_id?(y=f.winner_id===s.profile.id,b=!1):f.forfeit?(y=m>h,b=!1):(b=m===h,y=m>h),(A=document.getElementById("pvp-end-overlay"))==null||A.remove();const v=document.createElement("div");v.id="pvp-end-overlay",v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const w=b?"🤝":y?"🏆":"😞",I=b?"MATCH NUL":y?"VICTOIRE !":"DÉFAITE",E=b?"#fff":y?"#FFD700":"#ff6b6b";v.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(v),(C=v.querySelector("#pvp-end-home"))==null||C.addEventListener("click",()=>{v.remove(),Oe(e),l("home")})}const R=_.channel("pvp-match-"+n).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${n}`},f=>{const m=f.new;try{if(m.status==="finished"||m.forfeit){if($)return;$=!0,M&&(clearInterval(M),M=null),m.game_state&&(o=m.game_state),j(m);return}if(m.game_state){const h=o;o=m.game_state;const y=o._lastGC;if(y&&y.seq&&!F.has(y.seq)&&(F.add(y.seq),y.by!==i)){ge(y.type,y.by,()=>Q());return}const b=h[i+"Score"]||0,v=h[r+"Score"]||0,w=o[i+"Score"]||0,I=o[r+"Score"]||0,E=w>b,A=I>v;if((E||A)&&!U.has(o.round)){U.add(o.round);const C=[...o.log||[]].reverse().find(W=>W.isGoal),P=((C==null?void 0:C.homePlayers)||[]).map(W=>({name:W.name,note:W.note,portrait:W.portrait,job:W.job}));xe(P,w,I,E,()=>Q());return}Q()}}catch(h){console.error("[PvP] crash:",h)}}).subscribe();async function ie(f){Object.assign(o,f),o.lastActionAt=new Date().toISOString();const{error:m}=await _.from("matches").update({game_state:o}).eq("id",n);m&&c("Erreur de synchronisation","error");try{Q()}catch(h){console.error("[PvP] renderPvpScreen crash:",h)}}async function ce(){if($)return;$=!0,M&&(clearInterval(M),M=null);const f=a?g.away_id:g.home_id,m=a?"p2":"p1",h=a?"p1":"p2",y={...o,[m+"Score"]:3,[h+"Score"]:0,phase:"finished"};try{await _.from("matches").update({status:"finished",forfeit:!0,winner_id:f,home_score:y.p1Score||0,away_score:y.p2Score||0,game_state:y}).eq("id",n)}catch{}try{_.removeChannel(R)}catch{}setTimeout(()=>{Oe(e),l("home")},800)}const oe={BOOST_STAT:({value:f=1,count:m=1,roles:h=[]},y,b)=>{const v=y[i+"Team"],w=Object.entries(v).filter(([I])=>!h.length||h.includes(I)).flatMap(([I,E])=>E.filter(A=>!A.used).map(A=>({...A,_line:I})));if(!w.length){y.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),b(y);return}S(w,m,`Choisir ${m} joueur(s) à booster (+${f})`,I=>{I.forEach(E=>{const A=(v[E._line]||[]).find(C=>C.cardId===E.cardId);A&&(A.boost=(A.boost||0)+f,y.log.push({text:`⚡ +${f} sur ${A.name}`,type:"info"}))}),y[i+"Team"]=v,b(y)})},DEBUFF_STAT:({value:f=1,count:m=1,roles:h=[],target:y="ai"},b,v)=>{const w=y==="home"?i:r,I=b[w+"Team"],E=y==="home"?"allié":"adverse",A=Object.entries(I).filter(([C])=>!h.length||h.includes(C)).flatMap(([C,P])=>P.filter(W=>!W.used).map(W=>({...W,_line:C})));if(!A.length){b.log.push({text:`🎯 Aucun joueur ${E}`,type:"info"}),v(b);return}S(A,m,`Choisir ${m} joueur(s) ${E}(s) (-${f})`,C=>{C.forEach(P=>{const W=(I[P._line]||[]).find(le=>le.cardId===P.cardId);W&&(W.boost=(W.boost||0)-f,b.log.push({text:`🎯 -${f} sur ${W.name}`,type:"info"}))}),b[w+"Team"]=I,v(b)})},GRAY_PLAYER:({count:f=1,roles:m=[],target:h="ai"},y,b)=>{const v=h==="home"?i:r,w=y[v+"Team"],I=h==="home"?"allié":"adverse",E=Object.entries(w).filter(([A])=>!m.length||m.includes(A)).flatMap(([A,C])=>C.filter(P=>!P.used).map(P=>({...P,_line:A})));if(!E.length){y.log.push({text:`❌ Aucun joueur ${I}`,type:"info"}),b(y);return}S(E,f,`Choisir ${f} joueur(s) ${I}(s) à exclure`,A=>{A.forEach(C=>{const P=(w[C._line]||[]).find(W=>W.cardId===C.cardId);P&&(P.used=!0,y.log.push({text:`❌ ${P.name} exclu !`,type:"info"}))}),y[v+"Team"]=w,b(y)})},REVIVE_PLAYER:({count:f=1,roles:m=[]},h,y)=>{const b=h[i+"Team"],v=Object.entries(b).filter(([w])=>!m.length||m.includes(w)).flatMap(([w,I])=>I.filter(E=>E.used).map(E=>({...E,_line:w})));if(!v.length){h.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),y(h);return}S(v,f,`Choisir ${f} joueur(s) à ressusciter`,w=>{w.forEach(I=>{const E=(b[I._line]||[]).find(A=>A.cardId===I.cardId);E&&(E.used=!1,h.log.push({text:`💫 ${E.name} ressuscité !`,type:"info"}))}),h[i+"Team"]=b,y(h)})},REMOVE_GOAL:({},f,m)=>{const h=r+"Score";f[h]>0?(f[h]--,f.log.push({text:"🚫 Dernier but annulé !",type:"info"})):f.log.push({text:"🚫 Aucun but à annuler",type:"info"}),m(f)},ADD_GOAL_DRAW:({},f,m)=>{f[i+"Score"]===f[r+"Score"]?(f[i+"Score"]++,f.log.push({text:"🎯 But bonus !",type:"info"})):f.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),m(f)},ADD_SUB:({value:f=1},m,h)=>{m.maxSubs=(m.maxSubs||3)+f,m.log.push({text:`🔄 +${f} remplacement(s)`,type:"info"}),h(m)},CUSTOM:({},f,m)=>m(f)};function S(f,m,h,y){const b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let v=[];function w(){var E,A;const I=f.map(C=>{const P={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[C._line]||"#555",W=we(C,C._line)+(C.boost||0),J=v.find(K=>K.cardId===C.cardId)?"#FFD700":"rgba(255,255,255,0.25)",H=C.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${C.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${J};background:${P};overflow:hidden;cursor:pointer;${H}">
        ...
        </div>`,(E=b.querySelector("#pp-close"))==null||E.addEventListener("click",()=>b.remove()),b.querySelectorAll(".pp-item").forEach(C=>{C.addEventListener("click",()=>{const P=C.dataset.cid,W=f.find(J=>J.cardId===P),le=v.findIndex(J=>J.cardId===P);W&&(le>-1?v.splice(le,1):v.length<m&&v.push(W),w())})}),(A=b.querySelector("#pp-confirm"))==null||A.addEventListener("click",()=>{b.remove(),y(v)})}w(),document.body.appendChild(b)}async function Y(f,m){const y=(o["gcCardsFull_"+i]||[]).find(I=>I.id===f),b=(y==null?void 0:y._gcDef)||(o.gcDefs||[]).find(I=>{var E;return I.name===m||((E=I.name)==null?void 0:E.toLowerCase().trim())===(m==null?void 0:m.toLowerCase().trim())}),v=[...o["usedGc_"+i]||[],f],w={type:m,by:i,seq:(o._gcAnimSeq||0)+1};F.add(w.seq),ge(m,i,async()=>{if(b!=null&&b.effect_type&&b.effect_type!=="CUSTOM"){const E=oe[b.effect_type];if(E){const A={...o};E(b.effect_params||{},A,async C=>{C["usedGc_"+i]=v,C._lastGC=w,C._gcAnimSeq=w.seq,await ie(C)});return}}const I={...o};switch(m){case"Remplacement+":I.maxSubs=(I.maxSubs||3)+1,I.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const E=r+"Score";I[E]>0&&(I[E]--,I.log.push({text:"🚫 But annulé",type:"info"}));break}}I["usedGc_"+i]=v,I._lastGC=w,I._gcAnimSeq=w.seq,await ie(I)})}function ee(f,m){const h="usedCardIds_"+f,y=new Set(o[h]||[]);m.forEach(b=>y.add(b)),o[h]=[...y]}function D(){for(const f of["p1","p2"]){const m=new Set(o["usedCardIds_"+f]||[]),h=o[f+"Team"];if(h)for(const y of["GK","DEF","MIL","ATT"])(h[y]||[]).forEach(b=>{b.used=m.has(b.cardId)})}}function Q(){var Qe,mt,xt,yt,ht,X;if(o.phase==="reveal")return ae();if(o.phase==="midfield")return he();if(o.phase==="finished")return T();const f=o[i+"Team"],m=o[r+"Team"];D();const h=o[i+"Score"],y=o[r+"Score"],b=o[i+"Name"],v=o[r+"Name"],w=o.phase===i+"-attack",I=o.phase===i+"-defense",E=Array.isArray(o["selected_"+i])?o["selected_"+i]:[],A=E.map(G=>G.cardId),C=window.innerWidth>=700,P=o[i+"Subs"]||[],W=o["usedSubIds_"+i]||[],le=P.filter(G=>!W.includes(G.cardId)),J=o["gcCardsFull_"+i]||[],H=o["usedGc_"+i]||[],K=J.filter(G=>!H.includes(G.id)),Z=o.boostOwner===i&&!o.boostUsed,de=!["GK","DEF","MIL","ATT"].some(G=>(m[G]||[]).some(re=>!re.used)),ue=[...f.MIL||[],...f.ATT||[]].filter(G=>!G.used),fe=w&&de&&ue.length===0?[...f.GK||[],...f.DEF||[]].filter(G=>!G.used).map(G=>G.cardId):[];function me(G,re,se){var wt,mi;const ye=G._gcDef,Ye={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ye==null?void 0:ye.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",We={purple:"#b06ce0",light_blue:"#00d4ef"}[ye==null?void 0:ye.color]||"#b06ce0",Je=(ye==null?void 0:ye.name)||G.gc_type,et=(ye==null?void 0:ye.effect)||((wt=Ge[G.gc_type])==null?void 0:wt.desc)||"",Mt=ye!=null&&ye.image_url?`/manager-wars/icons/${ye.image_url}`:null,St=((mi=Ge[G.gc_type])==null?void 0:mi.icon)||"⚡",bt=Math.round(se*.22),lt=Math.round(se*.22),vt=se-bt-lt,Ct=Je.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${G.id}" data-gc-type="${G.gc_type}"
      ...
      </div>`;function Be(){const G=e.querySelector(".match-screen");if(!G)return;const re=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);G.style.height=re+"px",G.style.minHeight=re+"px",G.style.maxHeight=re+"px",G.style.overflow="hidden";const se=e.querySelector("#mobile-action-bar"),ye=e.querySelector("#mobile-play-area");se&&ye&&(ye.style.paddingBottom=se.offsetHeight+"px")}if(Be(),setTimeout(Be,120),setTimeout(Be,400),B||(B=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",Be),window.visualViewport.addEventListener("scroll",Be)),window.addEventListener("resize",Be)),function(){const re=e.querySelector(".terrain-wrapper svg");re&&(re.removeAttribute("width"),re.removeAttribute("height"),re.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",re.setAttribute("viewBox","-26 -26 352 352"),re.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),e.querySelectorAll(".match-slot-hit").forEach(G=>{G.addEventListener("click",()=>{if(!w&&!I)return;const re=G.dataset.cardId,se=G.dataset.role,ye=(f[se]||[]).find(et=>et.cardId===re);if(!ye||ye.used)return;const Ye=fe.includes(re);if(w&&!["MIL","ATT"].includes(se)&&!Ye)return;Array.isArray(o["selected_"+i])||(o["selected_"+i]=[]);const We=o["selected_"+i],Je=We.findIndex(et=>et.cardId===re);Je>-1?We.splice(Je,1):We.length<3&&We.push({...ye,_role:se}),Q()})}),e.querySelectorAll(".match-used-hit").forEach(G=>{G.addEventListener("click",()=>N(G.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(G=>{G.addEventListener("click",()=>N())}),(Qe=e.querySelector("#pvp-sub-open"))==null||Qe.addEventListener("click",()=>N()),e.querySelectorAll(".pvp-gc-mini").forEach(G=>{G.addEventListener("click",()=>O(G.dataset.gcId,G.dataset.gcType))}),(mt=e.querySelector("#pvp-boost-card"))==null||mt.addEventListener("click",()=>L()),(xt=e.querySelector("#pvp-action"))==null||xt.addEventListener("click",G=>{w?G.currentTarget.dataset.pass==="1"||!Te(i)?te():$e():I&&Ee()}),(yt=e.querySelector("#pvp-quit"))==null||yt.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&ce()}),(ht=e.querySelector("#pvp-view-opp"))==null||ht.addEventListener("click",()=>V()),(X=e.querySelector("#pvp-toggle-history"))==null||X.addEventListener("click",()=>ne()),M&&(clearInterval(M),M=null),(w||I)&&!$){let G=30,re=!1;const se=()=>document.getElementById("pvp-timer"),ye=()=>{se()&&(se().textContent=G+"s",se().style.color=re?"#ff4444":"#fff")};ye(),M=setInterval(()=>{G--,G<0?re?(clearInterval(M),M=null,w&&!Te(i)?te():ce()):(re=!0,G=15,ye()):ye()},1e3)}}function ae(){Xe(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
      <div style="font-size:20px;font-weight:900;color:#ff6b6b">${o[r+"Name"]||"Adversaire"}</div>
      <div style="width:min(90vw,420px)">${st(o[r+"Team"],o[r+"Formation"],null,[],300,300)}</div>
    </div>`,i==="p1"&&setTimeout(async()=>{await ie({phase:"midfield"})},5e3)}let pe=!1;function he(){if(pe)return;const f=o[i+"Team"].MIL||[],m=o[r+"Team"].MIL||[];function h(K){return K.reduce((Z,de)=>Z+we(de,"MIL"),0)}function y(K){let Z=0;for(let de=0;de<K.length-1;de++){const ue=Ze(K[de],K[de+1]);ue==="#00ff88"?Z+=2:ue==="#FFD700"&&(Z+=1)}return Z}const b=h(f)+y(f),v=h(m)+y(m),w=b>=v;function I(K,Z,de){return`<div id="duel-row-${de}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(v);let E=!1;const A=()=>{E||(E=!0,v.remove(),setTimeout(()=>b(),50))};v.addEventListener("click",A),setTimeout(A,3500)}function ge(f,m,h){var H,K;const y=(o.gcDefs||[]).find(Z=>{var de;return Z.name===f||((de=Z.name)==null?void 0:de.toLowerCase().trim())===(f==null?void 0:f.toLowerCase().trim())}),b={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[y==null?void 0:y.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",v={purple:"#b06ce0",light_blue:"#00d4ef"}[y==null?void 0:y.color]||"#b06ce0",w=(y==null?void 0:y.name)||f,I=(y==null?void 0:y.effect)||((H=Ge[f])==null?void 0:H.desc)||"",E=y!=null&&y.image_url?`/manager-wars/icons/${y.image_url}`:null,A=((K=Ge[f])==null?void 0:K.icon)||"⚡",P=m===i?"Vous":o[m+"Name"]||"Adversaire",W=document.createElement("div");W.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",W.innerHTML=`
      <style>
        @keyframes gcFlipIn{0%{transform:perspective(800px) rotateY(90deg) scale(.7);opacity:0}55%{transform:perspective(800px) rotateY(-12deg) scale(1.08);opacity:1}100%{transform:perspective(800px) rotateY(0) scale(1);opacity:1}}
        @keyframes gcGlow{0%,100%{box-shadow:0 0 30px ${v}66}50%{box-shadow:0 0 60px ${v}cc}}
        @keyframes gcLabel{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:translateY(0)}}
      </style>
      <div style="font-size:11px;color:${v};letter-spacing:3px;text-transform:uppercase;font-weight:700;animation:gcLabel .4s ease both">${P} joue une carte</div>
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(W);let le=!1;const J=()=>{le||(le=!0,W.remove(),setTimeout(()=>h&&h(),50))};W.addEventListener("click",J),setTimeout(J,3e3)}function O(f,m){var W,le,J,H;const y=(o["gcCardsFull_"+i]||[]).find(K=>K.id===f),b=y==null?void 0:y._gcDef,v={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[b==null?void 0:b.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",w={purple:"#b06ce0",light_blue:"#00d4ef"}[b==null?void 0:b.color]||"#b06ce0",I=(b==null?void 0:b.name)||m,E=(b==null?void 0:b.effect)||((W=Ge[m])==null?void 0:W.desc)||"Carte spéciale.",A=b!=null&&b.image_url?`/manager-wars/icons/${b.image_url}`:null,C=((le=Ge[m])==null?void 0:le.icon)||"⚡",P=document.createElement("div");P.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",P.innerHTML=`
      ...
      <div style="display:flex;gap:12px;width:190px">
        <button id="pvp-gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
        <button id="pvp-gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
      </div>`,document.body.appendChild(P),(J=P.querySelector("#pvp-gc-back"))==null||J.addEventListener("click",()=>P.remove()),(H=P.querySelector("#pvp-gc-use"))==null||H.addEventListener("click",()=>{P.remove(),Y(f,m)})}function L(){var y;const f=o[i+"Team"],m=Object.entries(f).flatMap(([b,v])=>(v||[]).filter(w=>!w.used).map(w=>({...w,_line:b})));if(!m.length)return;const h=document.createElement("div");h.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",h.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
        <div style="flex:1;font-size:14px;font-weight:700;color:#fff">⚡ Choisir un joueur pour +${o.boostValue}</div>
        <button id="bp-close" style="background:none;border:none;color:rgba(255,255,255,0.5);font-size:22px;cursor:pointer">✕</button>
      </div>
      ...
      </div>`,document.body.appendChild(h),(y=h.querySelector("#bp-close"))==null||y.addEventListener("click",()=>h.remove()),h.querySelectorAll(".bp-item").forEach(b=>{b.addEventListener("click",async()=>{const v=b.dataset.cid,w=m.find(E=>E.cardId===v);if(!w)return;const I=(f[w._line]||[]).find(E=>E.cardId===v);I&&(I.boost=(I.boost||0)+o.boostValue),h.remove(),await ie({[i+"Team"]:f,boostUsed:!0})})})}function N(f=null){var le,J;if(!(o.phase===i+"-attack")){c("Remplacement uniquement avant votre attaque","warning");return}const h=o[i+"Team"],y=o["usedSubIds_"+i]||[],b=o.maxSubs||3;if(y.length>=b){c(`Maximum ${b} remplacements atteint`,"warning");return}const v=Object.entries(h).flatMap(([H,K])=>(K||[]).filter(Z=>Z.used).map(Z=>({...Z,_line:H}))),w=(o[i+"Subs"]||[]).filter(H=>!y.includes(H.cardId));if(!v.length){c("Aucun joueur utilisé à remplacer","warning");return}if(!w.length){c("Aucun remplaçant disponible","warning");return}let I=Math.max(0,v.findIndex(H=>H.cardId===f));const E=((le=v[I])==null?void 0:le._line)||((J=v[I])==null?void 0:J.job);let A=Math.max(0,w.findIndex(H=>H.job===E)),C=!1;const P=document.createElement("div");P.id="pvp-sub-overlay",P.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function W(){var me,be,Se,Pe,Fe,Ie;const H=v[I],K=w[A],Z=Math.min(130,Math.round((window.innerWidth-90)/2)),de=Math.round(Z*1.35),ue=Le=>`background:rgba(255,255,255,0.12);border:none;color:${Le?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Le?"default":"pointer"};flex-shrink:0`;P.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
        <button id="psub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
      </div>`,(me=P.querySelector("#psub-close"))==null||me.addEventListener("click",()=>P.remove()),(be=P.querySelector("#pout-up"))==null||be.addEventListener("click",()=>{I>0&&(I--,W())}),(Se=P.querySelector("#pout-down"))==null||Se.addEventListener("click",()=>{I<v.length-1&&(I++,W())}),(Pe=P.querySelector("#pin-up"))==null||Pe.addEventListener("click",()=>{A>0&&(A--,W())}),(Fe=P.querySelector("#pin-down"))==null||Fe.addEventListener("click",()=>{A<w.length-1&&(A++,W())});const fe=(Le,je,De,Re)=>{const Ue=P.querySelector("#"+Le);if(!Ue)return;let Ke=0;Ue.addEventListener("touchstart",Ce=>{Ke=Ce.touches[0].clientY},{passive:!0}),Ue.addEventListener("touchend",Ce=>{const Be=Ce.changedTouches[0].clientY-Ke;if(Math.abs(Be)<30)return;const Qe=je();Be<0&&Qe<Re-1?(De(Qe+1),W()):Be>0&&Qe>0&&(De(Qe-1),W())},{passive:!0})};fe("pin-panel",()=>A,Le=>A=Le,w.length),fe("pout-panel",()=>I,Le=>I=Le,v.length),(Ie=P.querySelector("#psub-confirm"))==null||Ie.addEventListener("click",async Le=>{if(Le.preventDefault(),Le.stopPropagation(),C)return;C=!0;const je=v[I],De=w[A];if(!je||!De)return;const Re=je._line,Ue=(h[Re]||[]).findIndex(Be=>Be.cardId===je.cardId);if(Ue===-1){c("Erreur : joueur introuvable","error"),P.remove();return}const Ke={...De,_line:Re,position:je.position,used:!1,boost:0};h[Re].splice(Ue,1,Ke);const Ce=[...y,De.cardId];P.remove(),q(je,De,async()=>{await ie({[i+"Team"]:h,[r+"Team"]:o[r+"Team"],["usedSubIds_"+i]:Ce})})})}document.body.appendChild(P),W()}function q(f,m,h){const y={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const v=(E,A,C)=>`<div style="text-align:center">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:6px">Appuyer pour continuer</div>`,document.body.appendChild(b);let w=!1;const I=()=>{w||(w=!0,b.remove(),setTimeout(()=>h(),50))};b.addEventListener("click",I),setTimeout(I,2200)}function V(){var m;const f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:20px;overflow-y:auto",f.innerHTML=`
      <div style="font-size:12px;color:rgba(255,255,255,0.5);letter-spacing:2px;text-transform:uppercase">Équipe adverse</div>
      <div style="font-size:18px;font-weight:900;color:#ff6b6b">${o[r+"Name"]}</div>
      <div style="width:min(90vw,420px)">${st(o[r+"Team"],o[r+"Formation"],null,[],300,300)}</div>
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(f),(m=f.querySelector("#pvp-opp-close"))==null||m.addEventListener("click",()=>f.remove())}function ne(){var h;const f=o.log||[],m=document.createElement("div");m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column",m.innerHTML=`
      <div style="display:flex;align-items:center;padding:14px 16px;border-bottom:1px solid rgba(255,255,255,0.1);flex-shrink:0">
        <div style="flex:1;font-size:14px;font-weight:700;color:#fff">📋 Historique</div>
        <button id="pvp-hist-close" style="background:none;border:none;color:rgba(255,255,255,0.6);font-size:20px;cursor:pointer">✕</button>
      </div>
      ...
      </div>`,document.body.appendChild(m),(h=m.querySelector("#pvp-hist-close"))==null||h.addEventListener("click",()=>m.remove())}async function te(){if(o.phase!==i+"-attack")return;const f=i==="p1"?"p2":"p1",m=(o.round||0)+1,h=[...o.log||[]];h.push({type:"info",text:`⏭️ ${o[i+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const y=qe(o),b=Te(f),v=y||!b?"finished":f+"-attack";await ie({["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},pendingAttack:null,phase:v,attacker:f,round:m,log:h}),v==="finished"&&await He(o)}async function $e(){const f=o[i+"Team"],m=!["GK","DEF","MIL","ATT"].some(v=>(o[r+"Team"][v]||[]).some(w=>!w.used)),h=(o["selected_"+i]||[]).map(v=>{const w=(f[v._role]||[]).find(W=>W.cardId===v.cardId)||v,I=m&&["GK","DEF"].includes(v._role),E=f[v._role]||[],A=E.findIndex(W=>W.cardId===v.cardId),C=ot(E.length),P=A>=0?C[A]:w._col??1;return{...w,_line:v._role,_col:P,...I?{note_a:Math.max(1,Number(w.note_a)||0)}:{}}});if(!h.length)return;const y=Rt(h,o.modifiers[i]||{});ee(i,h.map(v=>v.cardId)),h.forEach(v=>{const w=(f[v._role]||[]).find(I=>I.cardId===v.cardId);w&&(w.used=!0)}),o["selected_"+i]=[],Q();const b=[...o.log||[]];if(m){const v=(o[i+"Score"]||0)+1,w=h.map(P=>({name:P.name,note:we(P,P._line||"ATT"),portrait:ze(P),job:P.job}));b.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:w,homeTotal:y.total,aiTotal:0});const I=(o.round||0)+1,E=i==="p1"?"p2":"p1",A={...o,[i+"Team"]:f,[i+"Score"]:v},C=qe(A);U.add(I),xe(w,v,o[r+"Score"]||0,!0,async()=>{await ie({[i+"Team"]:f,[i+"Score"]:v,["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},pendingAttack:null,phase:C?"finished":E+"-attack",attacker:E,round:I,log:b}),C&&await He({...o,[i+"Score"]:v})});return}b.push({type:"pending",text:`⚔️ ${o[i+"Name"]} attaque (${y.total})`}),await ie({[i+"Team"]:f,[r+"Team"]:o[r+"Team"],pendingAttack:{...y,players:h,side:i},["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},phase:r+"-defense",log:b})}async function Ee(){const f=o[i+"Team"],m=(o["selected_"+i]||[]).map(H=>{const K=(f[H._role]||[]).find(me=>me.cardId===H.cardId)||H,Z=f[H._role]||[],de=Z.findIndex(me=>me.cardId===H.cardId),ue=ot(Z.length),fe=de>=0?ue[de]:K._col??1;return{...K,_line:H._role,_col:fe}}),h=Ot(m,o.modifiers[i]||{});ee(i,m.map(H=>H.cardId)),m.forEach(H=>{const K=(f[H._role]||[]).find(Z=>Z.cardId===H.cardId);K&&(K.used=!0)}),o["selected_"+i]=[],Q();const y=Ht(o.pendingAttack.total,h.total,o.modifiers[i]||{}),b=o.pendingAttack.side,v=y==="attack"||(y==null?void 0:y.goal),w=b==="p1"?"p2":"p1",I=(o.round||0)+1,E=(o.pendingAttack.players||[]).map(H=>({name:H.name,note:we(H,H._line||"ATT"),portrait:ze(H),job:H.job})),A=[...o.log||[]];A.push({type:"duel",isGoal:v,homeScored:v&&b===i,text:v?`⚽ BUT de ${o[b+"Name"]} ! (${o.pendingAttack.total} vs ${h.total})`:`✋ Attaque stoppée (${o.pendingAttack.total} vs ${h.total})`,homePlayers:E,aiPlayers:m.map(H=>({name:H.name,note:we(H,H._line||"DEF"),portrait:ze(H),job:H.job})),homeTotal:o.pendingAttack.total,aiTotal:h.total});const C=v?(o[b+"Score"]||0)+1:o[b+"Score"]||0,P={...o,[i+"Team"]:f,[b+"Score"]:C},W=qe(P),le=W?"finished":w+"-attack",J=async()=>{await ie({[i+"Team"]:f,[r+"Team"]:o[r+"Team"],[b+"Score"]:C,["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},pendingAttack:null,phase:le,attacker:w,round:I,log:A}),(le==="finished"||W)&&await He({...o,[b+"Score"]:C})};if(v){const H=b===i,K=H?C:o[i+"Score"]||0,Z=H?o[r+"Score"]||0:C;U.add(I),xe(E,K,Z,H,J)}else await J()}function ke(f){return["MIL","ATT"].some(m=>(f[m]||[]).some(h=>!h.used))}function _e(f){return["GK","DEF","MIL","ATT"].some(m=>(f[m]||[]).some(h=>!h.used))}function Ae(f){return _e(f)&&!ke(f)}function Te(f){const m=o[f+"Team"],h=o[(f==="p1"?"p2":"p1")+"Team"];return!!(ke(m)||!_e(h)&&Ae(m))}function qe(f){const m=["MIL","ATT"].some(A=>(f.p1Team[A]||[]).some(C=>!C.used)),h=["MIL","ATT"].some(A=>(f.p2Team[A]||[]).some(C=>!C.used)),y=_e(f.p1Team),b=_e(f.p2Team);return!m&&!(!b&&y)&&(!h&&!(!y&&b))}function Ne(f){const m=f.p1Score||0,h=f.p2Score||0;return m===h?null:m>h?g.home_id:g.away_id}async function He(f){try{const m=Ne(f),h=m?g.home_id===m?g.away_id:g.home_id:null;await _.from("matches").update({status:"finished",winner_id:m,home_score:f.p1Score||0,away_score:f.p2Score||0}).eq("id",n),m&&h&&Zi(m,h).catch(()=>{})}catch(m){console.error("[PvP] finishMatch:",m)}}function T(){var b;const f=o[i+"Score"],m=o[r+"Score"],h=f>m,y=f===m;Xe(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      ...
      <button id="pvp-home" style="padding:16px 40px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">🏠 Retour</button>
    </div>`,(b=document.getElementById("pvp-home"))==null||b.addEventListener("click",()=>{try{_.removeChannel(R)}catch{}Oe(e),l("home")})}Q()}const To={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"};async function Io(e,t){const{state:n,toast:a}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement du marché...</div>',await fi(e,t)}async function fi(e,t){const{state:n,toast:a}=t,{data:p}=await _.from("market_listings").select(`id, price, status, listed_at, seller_id,
  ...
  `;function c(i){const r=document.getElementById("mkt-content"),u=i==="buy"?s:l;if(u.length===0){r.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
        ${i==="buy"?"Aucune carte en vente actuellement.":"Tu n'as aucune vente pour le moment."}
      </div>`;return}const x=i==="mine"?[...u].sort((g,k)=>g.status!==k.status?g.status==="active"?-1:1:new Date(k.listed_at)-new Date(g.listed_at)):u;r.innerHTML=`<div style="display:flex;flex-direction:column;gap:10px">
      ${x.map(g=>{var B,U,F,j;const k=(B=g.card)==null?void 0:B.player;if(!k)return"";const o=k.job==="GK"?k.note_g:k.job==="DEF"?k.note_d:k.job==="MIL"?k.note_m:k.note_a,$=To[k.rarity],z=(n.profile.credits||0)>=g.price,M=g.status==="sold";return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${M?"opacity:0.65":""}">
          <div style="width:44px;height:44px;border-radius:8px;background:${So(k.job)};color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:18px;border:2px solid ${$};flex-shrink:0">${o}</div>
          <div style="flex:1;min-width:0">
            <div style="font-weight:700;font-size:14px">${k.firstname} ${k.surname_encoded}</div>
            <div style="font-size:11px;color:var(--gray-600)">${k.country_code} · ${((U=k.clubs)==null?void 0:U.encoded_name)||"—"} · ${k.rarity} · ${k.job}</div>
            ${i==="buy"?`<div style="font-size:11px;color:var(--gray-600)">Vendeur : ${((F=g.seller)==null?void 0:F.pseudo)||"—"}</div>`:M?`<div style="font-size:11px;color:var(--green)">✅ Vendu à ${((j=g.buyer)==null?void 0:j.pseudo)||"—"} · ${g.sold_at?new Date(g.sold_at).toLocaleDateString("fr"):""}</div>`:`<div style="font-size:11px;color:var(--gray-600)">🟢 En vente depuis le ${new Date(g.listed_at).toLocaleDateString("fr")}</div>`}
          </div>
          <div style="text-align:right;flex-shrink:0">
            <div style="font-weight:900;color:var(--yellow);font-size:15px">${g.price.toLocaleString("fr")}</div>
            ${i==="buy"?`<button class="btn btn-primary btn-sm" data-buy="${g.id}" ${z?"":"disabled"} style="margin-top:4px">${z?"Acheter":"Trop cher"}</button>`:M?'<span style="display:inline-block;margin-top:4px;font-size:10px;font-weight:700;color:#fff;background:var(--green);padding:3px 10px;border-radius:10px">VENDU</span>':`<button class="btn btn-danger btn-sm" data-cancel="${g.id}" style="margin-top:4px">Retirer</button>`}
          </div>
        </div>`}).join("")}
    </div>`,r.querySelectorAll("[data-buy]").forEach(g=>{g.addEventListener("click",()=>Lo(g.dataset.buy,u,e,t))}),r.querySelectorAll("[data-cancel]").forEach(g=>{g.addEventListener("click",()=>Mo(g.dataset.cancel,e,t))})}c("buy"),e.querySelectorAll(".mkt-tab").forEach(i=>{i.addEventListener("click",()=>{e.querySelectorAll(".mkt-tab").forEach(r=>{const u=r===i;r.style.background=u?"var(--green)":"#fff",r.style.color=u?"#fff":"var(--gray-600)",r.style.borderColor=u?"var(--green)":"var(--gray-200)"}),c(i.dataset.tab)})})}async function Lo(e,t,n,a){var g;const{state:p,toast:d,refreshProfile:s}=a,l=t.find(k=>k.id===e);if(!l)return;const c=l.price,i=p.profile.credits||0,r=(g=l.card)==null?void 0:g.player;if(i<c){d("Crédits insuffisants","error");return}if(!await zo(r,c))return;const x=document.querySelector(`[data-buy="${e}"]`);x&&(x.disabled=!0,x.textContent="⏳");try{const{data:k,error:o}=await _.rpc("buy_market_card",{p_listing_id:e,p_buyer_id:p.profile.id});if(o)throw new Error(o.message);if(!(k!=null&&k.success))throw new Error((k==null?void 0:k.error)||"Achat impossible");p.profile.credits=i-c;const $=document.getElementById("nav-credits");$&&($.textContent=`💰 ${(i-c).toLocaleString("fr")}`),d(`✅ ${r==null?void 0:r.surname_encoded} ajouté à ta collection !`,"success"),fi(n,a)}catch(k){d("❌ "+k.message,"error"),x&&(x.disabled=!1,x.textContent="Acheter")}}function zo(e,t){return new Promise(n=>{const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",a.innerHTML=`
      ...
      </div>`,document.body.appendChild(a);const p=d=>{a.remove(),n(d)};a.querySelector("#buy-ok").addEventListener("click",()=>p(!0)),a.querySelector("#buy-cancel").addEventListener("click",()=>p(!1)),a.addEventListener("click",d=>{d.target===a&&p(!1)})})}async function Mo(e,t,n){const{toast:a}=n,{data:p}=await _.from("market_listings").select("card_id").eq("id",e).single();if(await _.from("market_listings").update({status:"cancelled"}).eq("id",e),p!=null&&p.card_id){const{count:d}=await _.from("market_listings").select("id",{count:"exact",head:!0}).eq("card_id",p.card_id).eq("status","active");d||await _.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",p.card_id)}a("Annonce retirée","success"),fi(t,n)}function So(e){return{GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[e]||"#888"}async function Co(e,{state:t,navigate:n,toast:a}){e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:p}=await _.from("users").select("id,pseudo,club_name,trophies_top1,trophies_top2,trophies_top3,wins,level").order("trophies_top1",{ascending:!1}).limit(100);e.innerHTML=`
  <div class="page">
    <div class="page-header">
  ...
  `}async function jo(e,{state:t,navigate:n,toast:a}){const p=t.profile;if(!p)return;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:d}=await _.from("matches").select(`id,home_id,away_id,home_score,away_score,status,mode,winner_id,created_at,played_at,
      home:users!home_id(pseudo,club_name),
      away:users!away_id(pseudo,club_name)`).or(`home_id.eq.${p.id},away_id.eq.${p.id}`).order("created_at",{ascending:!1}).limit(50),s={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},l=(d||[]).filter(r=>r.status==="finished"),c=(d||[]).filter(r=>r.status==="in_progress");function i(r){const u=r.home_id===p.id;u?r.home_score:r.away_score,u?r.away_score:r.home_score;const x=r.winner_id===p.id,g=r.home_score===r.away_score&&r.status==="finished",k=r.status!=="finished"?"…":g?"N":x?"V":"D",o=r.status!=="finished"||g?"#888":x?"#1A6B3C":"#c0392b";let $=s[r.mode]||r.mode;r.away_id===null&&!$.startsWith("IA")&&($="IA");const M=r.home_id===p.id?r.away:r.home;let B;r.away_id===null?B=$:M?B=`${M.club_name||M.pseudo} (${M.pseudo})`:B="Adversaire";let U="";r.status==="in_progress"&&Date.now()-new Date(r.created_at).getTime()>3600*1e3&&(U=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const F=new Date(r.created_at),j=F.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+F.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),R=r.status==="finished"?`${r.home_score} - ${r.away_score}`:`${r.home_score||0} - ${r.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
  ...
  </div>`}rn(sn);const ve={user:null,profile:null,page:"home",params:{}};function $t(e,t="info",n=3e3){const a=document.getElementById("toast");a&&(a.textContent=e,a.className=`show ${t}`,clearTimeout(a._t),a._t=setTimeout(()=>{a.className=""},n))}function Bo(e,t,n=""){document.getElementById("modal-title").textContent=e,document.getElementById("modal-body").innerHTML=t,document.getElementById("modal-footer").innerHTML=n,document.getElementById("modal-overlay").classList.remove("hidden")}function ii(){document.getElementById("modal-overlay").classList.add("hidden")}async function kt(){if(!ve.user)return;const{data:e}=await _.from("users").select("*").eq("id",ve.user.id).single();e&&(ve.profile=e)}const gi="mw_theme";function Pt(){return localStorage.getItem(gi)||"light"}function Fo(e){var t;localStorage.setItem(gi,e),ni(e),(t=ve.profile)!=null&&t.id&&_.from("users").update({theme:e}).eq("id",ve.profile.id).then(()=>{})}function ni(e){document.documentElement.setAttribute("data-theme",e)}function Dt(e,t={}){ve.page=e,ve.params=t,nn()}async function nn(){var a,p;const e=document.getElementById("page-content");if(!e)return;document.querySelectorAll(".bottom-nav a").forEach(d=>{d.classList.toggle("active",d.dataset.page===ve.page)});const t=document.getElementById("nav-credits");t&&ve.profile&&(t.textContent=`💰 ${(ve.profile.credits||0).toLocaleString("fr")}`);const n={state:ve,navigate:Dt,toast:$t,openModal:Bo,closeModal:ii,refreshProfile:kt};switch(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽</div>',ve.page){case"home":await bi(e,n);break;case"collection":await In(e,n);break;case"decks":await Xt(e,n);break;case"boosters":await Rn(e,n);break;case"match":{const d=ve.params&&ve.params.matchMode||"vs_ai_easy";d==="random"?await en(e,n):d==="friend"?await ko(e,n,(a=ve.params)==null?void 0:a.friendId,(p=ve.params)==null?void 0:p.friendName):await no(e,n);break}case"market":await Io(e,n);break;case"rankings":await Co(e,n);break;case"matches":await jo(e,n);break;case"friends":await gn(e,n);break;default:await bi(e,n)}}function qo(){const e=document.getElementById("app"),t=ve.profile;if(!t)return;const n="/manager-wars/icons/";e.innerHTML=`
    <nav class="top-nav">
  ...
  `,document.querySelectorAll(".bottom-nav a").forEach(a=>{a.addEventListener("click",p=>{p.preventDefault(),Dt(a.dataset.page)})}),document.getElementById("nav-logo").addEventListener("click",()=>Dt("home")),document.getElementById("nav-credits").addEventListener("click",()=>Dt("boosters")),document.getElementById("theme-toggle").addEventListener("click",()=>{const p=Pt()==="dark"?"light":"dark";Fo(p),Li(p)}),Li(Pt())}function Li(e){const t=document.getElementById("theme-icon");t&&(t.textContent=e==="dark"?"☀️":"🌙")}async function Do(){ni(Pt()),document.getElementById("modal-overlay").addEventListener("click",n=>{n.target===n.currentTarget&&ii()}),document.getElementById("modal-close").addEventListener("click",ii);const{data:{session:e}}=await _.auth.getSession();if(!e){zi(),xi(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:$t});return}ve.user=e.user,await kt(),zi();try{const{data:n}=await _.from("formation_links_overrides").select("formation, links"),a={};(n||[]).forEach(p=>{a[p.formation]=p.links}),an(a)}catch(n){console.warn("Impossible de charger les overrides de formation:",n)}if(!ve.profile){dn(document.getElementById("app"),{state:ve,navigate:async()=>{await kt(),Wt()},toast:$t,refreshProfile:kt});return}const t=Array.isArray(ve.profile.pending_boosters)?ve.profile.pending_boosters:[];if(!ve.profile.onboarding_done&&t.length>0){Zn(document.getElementById("app"),{state:ve,navigate:()=>Wt(),toast:$t,refreshProfile:kt});return}ve.profile.theme&&ve.profile.theme!==Pt()&&(localStorage.setItem(gi,ve.profile.theme),ni(ve.profile.theme)),Wt(),_.auth.onAuthStateChange(async(n,a)=>{n==="SIGNED_OUT"&&(ve.user=null,ve.profile=null,document.getElementById("app").innerHTML="",xi(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:$t}))})}function Go(){return Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight)}function Kt(){const e=document.getElementById("app");e&&(e.style.height=Go()+"px")}window.addEventListener("resize",Kt);window.addEventListener("orientationchange",()=>setTimeout(Kt,150));window.visualViewport&&window.visualViewport.addEventListener("resize",Kt);function Wt(){const e=()=>{var n;(n=ve.user)!=null&&n.id&&_.from("users").update({last_seen_at:new Date().toISOString()}).eq("id",ve.user.id).then(()=>{})};e(),window._presencePingInterval&&clearInterval(window._presencePingInterval),window._presencePingInterval=setInterval(e,6e4);const t=document.getElementById("app");t.style.display="flex",t.style.flexDirection="column",Kt(),qo(),nn()}function zi(){const e=document.getElementById("app-loader"),t=document.getElementById("app");t&&(t.style.display=""),e&&(e.classList.add("zoom-out"),setTimeout(()=>e.style.display="none",500))}function on(e){var a;const t=document.getElementById("app-loader");if(t&&(t.style.display="none"),document.getElementById("boot-error"))return;const n=document.createElement("div");n.id="boot-error",n.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:99999;gap:16px;color:#fff;padding:24px;text-align:center",n.innerHTML=`
    <div style="font-size:42px">📡</div>
    <div style="font-size:18px;font-weight:900">Connexion impossible</div>
    <div style="font-size:13px;color:rgba(255,255,255,0.6);max-width:280px">${e||"Le chargement a échoué. Vérifie ta connexion et réessaie."}</div>
    <button id="boot-retry" style="margin-top:8px;padding:12px 30px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer">Réessayer</button>`,document.body.appendChild(n),(a=document.getElementById("boot-retry"))==null||a.addEventListener("click",()=>window.location.reload())}Do().catch(e=>{console.error("Échec du démarrage:",e),on()});setTimeout(()=>{const e=document.getElementById("app-loader");e&&e.style.display!=="none"&&!e.classList.contains("zoom-out")&&!document.getElementById("boot-error")&&on("Le serveur met trop de temps à répondre.")},12e3);
