import{s as A,F as oi,h as St,j as Ai,l as Qe,i as Xi,k as Ji,b as Qi}from"./formation-links-CEzN0rcz.js";function mi(e,{navigate:t,toast:o}){e.innerHTML=`
  ...
  `;function l(){var k;const i=document.getElementById("logo-preview"),r=document.getElementById("logo-initials"),f=((k=document.getElementById("setup-club"))==null?void 0:k.value)||"MW",m=f.trim().split(" ").filter(Boolean),y=m.length>=2?(m[0][0]+m[1][0]).toUpperCase():f.slice(0,2).toUpperCase();i&&(i.style.background=d,i.style.borderColor=s),r&&(r.textContent=y,r.style.color=s)}document.getElementById("color1").addEventListener("input",i=>{s=i.target.value,document.getElementById("swatch1").style.background=s,l()}),document.getElementById("color2").addEventListener("input",i=>{d=i.target.value,document.getElementById("swatch2").style.background=d,l()});function u(i){document.querySelectorAll(".setup-step").forEach(r=>r.classList.remove("active")),document.getElementById(`step-${i}`).classList.add("active"),document.getElementById("step-num").textContent=i,document.getElementById("progress-fill").style.width=`${Math.round(i/3*100)}%`,i===3&&l()}document.getElementById("step1-next").addEventListener("click",async()=>{const i=document.getElementById("setup-pseudo").value.trim(),r=document.getElementById("step1-error");if(r.textContent="",i.length<3){r.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:f}=await A.from("users").select("id").eq("pseudo",i).maybeSingle();if(f){r.textContent="Ce pseudo est déjà pris.";return}u(2)}),document.getElementById("step2-back").addEventListener("click",()=>u(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const i=document.getElementById("setup-club").value.trim(),r=document.getElementById("step2-error");if(r.textContent="",i.length<2){r.textContent="Nom trop court (min. 2 caractères).";return}const{data:f}=await A.from("users").select("id").eq("club_name",i).maybeSingle();if(f){r.textContent="Ce nom de club est déjà pris.";return}u(3)}),document.getElementById("step3-back").addEventListener("click",()=>u(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const i=document.getElementById("setup-pseudo").value.trim(),r=document.getElementById("setup-club").value.trim(),f=document.getElementById("step3-error"),m=document.getElementById("step3-finish");f.textContent="",m.disabled=!0,m.textContent="Création en cours…";try{const{error:y}=await A.from("users").insert({id:t.user.id,pseudo:i,club_name:r,club_color1:s,club_color2:d,credits:1e4});if(y)throw y;await en(t.user.id),await p(),a(`Bienvenue ${i} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(y){f.textContent=y.message,m.disabled=!1,m.textContent="🚀 Créer mon profil !"}})}async function en(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await A.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(o){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",o)}}const Lt="#1A6B3C",zt="#cc2222",tn="#D4A017",xi="#888";async function nn(e,t){const{state:o,toast:a}=t;e.innerHTML=`
    ...
    </div>`,await Ii(o,a,t),document.getElementById("btn-add-friend").addEventListener("click",()=>rn(o,a)),document.getElementById("btn-accept-friend").addEventListener("click",()=>Li(o,a,null,t))}async function Ii(e,t,o={}){const{navigate:a}=o,p=e.user.id,{data:s,error:d}=await A.from("friendships").select("id, requester_id, addressee_id").eq("status","accepted").or(`requester_id.eq.${p},addressee_id.eq.${p}`),{count:l}=await A.from("friendships").select("id",{count:"exact",head:!0}).eq("addressee_id",p).eq("status","pending"),u=document.getElementById("pending-badge");u&&(l>0?(u.style.display="flex",u.textContent=l):u.style.display="none");const i=document.getElementById("friends-list");if(!i)return;if(d){console.error("[Friends] Erreur:",d),i.innerHTML=`<div style="color:${zt};text-align:center;padding:16px">Erreur chargement : ${d.message}</div>`;return}const r=(s||[]).map(y=>y.requester_id===p?y.addressee_id:y.requester_id);let f={};if(r.length){const{data:y}=await A.from("users").select("id, pseudo, club_name, last_seen_at").in("id",r);(y||[]).forEach(k=>{f[k.id]=k})}const m=(s||[]).map(y=>({friendshipId:y.id,friend:f[y.requester_id===p?y.addressee_id:y.requester_id]||{pseudo:"?"}}));if(!m.length){i.innerHTML=`
    ...
    ${ai()}`,document.body.appendChild(o);const a=o.querySelector("#friend-pseudo-input"),p=o.querySelector("#add-friend-error"),s=()=>o.remove();a.focus(),o.querySelector("#add-cancel").addEventListener("click",s),o.addEventListener("click",d=>{d.target===o&&s()}),o.querySelector("#add-ok").addEventListener("click",async()=>{const d=a.value.trim();if(!d){p.textContent="Entre un pseudo";return}p.textContent="";const{data:l}=await A.from("users").select("id, pseudo").ilike("pseudo",d).single();if(!l){p.textContent="Utilisateur introuvable";return}if(l.id===e.user.id){p.textContent="Tu ne peux pas t'ajouter toi-même";return}const{data:u}=await A.from("friendships").select("id, status").or(`and(requester_id.eq.${e.user.id},addressee_id.eq.${l.id}),and(requester_id.eq.${l.id},addressee_id.eq.${e.user.id})`).single();if(u){const r=u.status==="accepted"?"Vous êtes déjà amis !":u.status==="pending"?"Demande déjà envoyée":"Une demande existe déjà";p.textContent=r;return}const{error:i}=await A.from("friendships").insert({requester_id:e.user.id,addressee_id:l.id,status:"pending"});if(i){p.textContent="Erreur : "+i.message;return}s(),t(`✅ Demande envoyée à ${l.pseudo} !`,"success")})}async function Li(e,t,o=null,a={}){const p=e.user.id,{data:s}=await A.from("friendships").select("id, requester_id").eq("addressee_id",p).eq("status","pending").order("created_at",{ascending:!1}),d=(s||[]).map(m=>m.requester_id);let l={};if(d.length){const{data:m}=await A.from("users").select("id, pseudo, club_name").in("id",d);(m||[]).forEach(y=>{l[y.id]=y})}const u=(s||[]).map(m=>({...m,requester:l[m.requester_id]||{pseudo:"?"}})),i=ri(),r=u||[];i.innerHTML=`
    <div class="popup-box">
      ...
      ${r.length?`<div style="display:flex;flex-direction:column;gap:8px;max-height:50vh;overflow-y:auto;margin-bottom:14px">
            ${r.map(m=>{var y,k,n;return`
              <div style="display:flex;align-items:center;gap:10px;background:#f9f9f9;border-radius:10px;padding:10px 12px">
                <div style="flex:1;font-size:14px;font-weight:700">${((y=m.requester)==null?void 0:y.club_name)||((k=m.requester)==null?void 0:k.pseudo)||"?"}
    ...
    ${ai()}`,document.body.appendChild(i);const f=()=>i.remove();i.querySelector("#pending-close").addEventListener("click",f),i.addEventListener("click",m=>{m.target===i&&f()}),i.querySelectorAll("[data-accept]").forEach(m=>{m.addEventListener("click",async()=>{const{error:y}=await A.from("friendships").update({status:"accepted"}).eq("id",m.dataset.accept);if(y){t("Erreur : "+y.message,"error");return}m.closest("div[style]").remove(),t("✅ Ami accepté !","success"),Ii(e,t,a),o&&o()})}),i.querySelectorAll("[data-decline]").forEach(m=>{m.addEventListener("click",async()=>{await A.from("friendships").delete().eq("id",m.dataset.decline),m.closest("div[style]").remove(),t("Demande refusée","info"),o&&o()})})}async function an(e,t,o){const a=e.user.id,[p,s]=[a,t].sort(),d=a===p,{data:l}=await A.from("friend_match_stats").select("*").eq("player1_id",p).eq("player2_id",s).single(),u=e.profile.club_name||e.profile.pseudo||"Moi",i=l||{},r=d?i.wins_p1||0:i.wins_p2||0,f=d?i.wins_p2||0:i.wins_p1||0,m=i.draws||0,y=d?i.goals_p1||0:i.goals_p2||0,k=d?i.goals_p2||0:i.goals_p1||0,n=d?i.gc_used_p1||0:i.gc_used_p2||0,E=d?i.gc_used_p2||0:i.gc_used_p1||0,z=i.matches_total||0,M=(O,B,S,C=Lt,ee=zt)=>`
    <div style="display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:8px;padding:10px 0;border-bottom:1px solid #f0f0f0">
      ...
      <div style="text-align:left;font-size:18px;font-weight:900;color:${ee}">${S}</div>
    ...
    </style>`}const sn="2026.06.27-2041";async function yi(e,{state:t,navigate:o,toast:a}){var s,d;const p=t.profile;p&&(e.innerHTML=`
  <div class="page">
    <div class="page-body">
  ...
  `,e.querySelectorAll("[data-nav]").forEach(l=>{l.addEventListener("click",u=>{u.preventDefault(),o(l.dataset.nav)})}),(s=document.getElementById("nav-rankings"))==null||s.addEventListener("click",()=>o("rankings")),(d=document.getElementById("nav-matches"))==null||d.addEventListener("click",()=>o("matches")),e.querySelectorAll("[data-action]").forEach(l=>{l.addEventListener("click",()=>{l.classList.add("tapped"),setTimeout(()=>l.classList.remove("tapped"),200);const u=l.dataset.action;if(u==="match-ai"){ln(o);return}if(u==="match-random"){o("match",{matchMode:"random"});return}if(u==="match-friend"){o("friends");return}a("Bientôt disponible","info")})}),document.getElementById("logout-btn").addEventListener("click",async()=>{await A.auth.signOut(),window.location.reload()}),zi(t,a),dn(t,a,o))}async function dn(e,t,o){var l,u,i,r;const a=document.getElementById("match-invite-banner");if(!a)return;const{data:p}=await A.from("friend_match_invites").select("id, inviter_id, inviter:users!inviter_id(pseudo, club_name)").eq("invitee_id",e.user.id).eq("status","pending").order("created_at",{ascending:!1}).limit(1).maybeSingle();if(!p){a.innerHTML="";return}const s=((l=p.inviter)==null?void 0:l.club_name)||((u=p.inviter)==null?void 0:u.pseudo)||"?",d=p.inviter_id;a.innerHTML=`
    ...
    </div>`,(i=document.getElementById("match-inv-accept"))==null||i.addEventListener("click",()=>{a.innerHTML="",o("match",{matchMode:"friend",friendId:d,friendName:s})}),(r=document.getElementById("match-inv-decline"))==null||r.addEventListener("click",async()=>{await A.from("friend_match_invites").update({status:"declined"}).eq("id",p.id),a.innerHTML="",t("Invitation refusée","info")})}async function zi(e,t){const o=document.getElementById("friend-requests-banner");if(!o)return;const{data:a,error:p}=await A.from("friendships").select("id, requester:users!requester_id(pseudo, club_name)").eq("addressee_id",e.user.id).eq("status","pending");if(p||!(a!=null&&a.length)){o.innerHTML="";return}const s=a.length,d=a.slice(0,2).map(u=>{var i;return((i=u.requester)==null?void 0:i.pseudo)||"?"}).join(", "),l=s>2?` +${s-2}`:"";o.innerHTML=`
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
  ...
  </div>`}async function fn(e,t){const{state:o,navigate:a,toast:p,openModal:s,closeModal:d}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:l}=await A.from("cards").select(`id, card_type, current_note, gc_type, formation, is_for_sale, sale_price,
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),i=(l||[]).filter(R=>R.card_type==="player"&&R.player),r=(l||[]).filter(R=>R.card_type==="game_changer"),f=(l||[]).filter(R=>R.card_type==="formation"),{data:m}=await A.from("gc_definitions").select("name,gc_type,color,effect,image_url"),y={};(m||[]).forEach(R=>{y[R.name]=R});const k=Object.keys(oi),n=Object.keys(Be),E={};i.forEach(R=>{const N=R.player.id;E[N]=(E[N]||0)+1}),new Set(Object.keys(E).map(R=>String(R)));const z=new Set(f.map(R=>R.formation)),M=new Set(r.map(R=>R.gc_type));let j="player",O="Tous",B="",S=!1;function C(){return[...i].sort((R,N)=>{const W=Ct.indexOf(R.player.job),P=Ct.indexOf(N.player.job);return W!==P?W-P:(R.player.surname_encoded||"").localeCompare(N.player.surname_encoded||"")})}function ee(){return[...u||[]].sort((R,N)=>{const W=Ct.indexOf(R.job),P=Ct.indexOf(N.job);return W!==P?W-P:(R.surname_encoded||"").localeCompare(N.surname_encoded||"")})}function ue(){return C().filter(R=>{const N=R.player,W=O==="Tous"||N.job===O,P=!B||`${N.firstname} ${N.surname_encoded}`.toLowerCase().includes(B);return W&&P})}function re(){return ee().filter(R=>{const N=O==="Tous"||R.job===O,W=!B||`${R.firstname} ${R.surname_encoded}`.toLowerCase().includes(B);return N&&W})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
  ...
  </div>`;function D(){const R=document.getElementById("col-filters");R&&(j==="player"?(R.innerHTML=`
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",N=>{B=N.target.value.toLowerCase(),Y()}),e.querySelectorAll(".filter-btn").forEach(N=>{N.addEventListener("click",()=>{O=N.dataset.job,D(),Y()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{S=!S,D(),Y()})):(R.innerHTML=`
        <div style="display:flex;justify-content:flex-end">
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{S=!S,D(),Y()})))}function Y(){const R=document.getElementById("col-grid");R&&(j==="player"?ge(R):j==="formation"?me(R):xe(R))}function te(R,N,W,P,X){X=X||"#7a28b8";const pe=document.getElementById("col-grid"),ie=document.getElementById("col-big");if(!pe||!ie)return;var Ae=0;function ke(){ie.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+N(R[Ae])+"</div>";var ve=ie.querySelector("[data-card-id],[data-form-id],[data-gc-id]");ve&&ve.addEventListener("click",function(){P(R[Ae])}),requestAnimationFrame(function(){var _e=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!_e||!ie)){var we=ie.clientHeight-8,Le=ie.clientWidth-24,Ge=_e.offsetHeight,Se=_e.offsetWidth;if(Ge>0&&Se>0&&we>40){var Ke=Math.min(we/Ge,Le/Se,1);_e.style.transform="scale("+Ke.toFixed(3)+")",_e.style.transformOrigin="top center"}}}),pe.innerHTML=R.map(function(_e,we){return'<div class="col-mini-item" data-idx="'+we+'" style="flex-shrink:0;cursor:pointer;border-radius:8px;border:1.5px solid '+(we===Ae?X:"transparent")+';transition:border-color .15s;overflow:hidden">'+W(_e,we===Ae)+"</div>"}).join(""),pe.querySelectorAll(".col-mini-item").forEach(function(_e){_e.addEventListener("click",function(){Ae=Number(_e.dataset.idx),ke(),_e.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}ke()}function F(R){var N=.54,W=Math.round(140*N),P=Math.round(310*N),X;if(!R||R._fake){var pe=R?R.player:null;if(!pe)return"";X=bi(pe)}else X=Vt(R,"");return'<div style="width:'+W+"px;height:"+P+'px;overflow:hidden;position:relative;flex-shrink:0"><div style="transform:scale('+N+');transform-origin:top left;position:absolute;top:0;left:0;pointer-events:none">'+X+"</div></div>"}function Q(R,N,W){N=N||100,W=W||140;var P=St[R]||{},X={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},pe=Math.max(3,Math.round(N*.06)),ie=Object.entries(P).map(function(ke){var ve=ke[0],_e=ke[1],we=ve.replace(/\d+$/,""),Le=X[we]||"#888",Ge=Math.round(_e.x*N),Se=Math.round(_e.y*W);return'<circle cx="'+Ge+'" cy="'+Se+'" r="'+pe+'" fill="'+Le+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),Ae=Math.max(1,Math.round(N/50));return'<svg viewBox="0 0 '+N+" "+W+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+N+'" height="'+W+'" fill="#1A6B3C"/><rect x="'+Math.round(N*.2)+'" y="'+Math.round(W*.02)+'" width="'+Math.round(N*.6)+'" height="'+Math.round(W*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+Ae+'"/><line x1="0" y1="'+Math.round(W*.5)+'" x2="'+N+'" y2="'+Math.round(W*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+Ae+'"/><ellipse cx="'+Math.round(N*.5)+'" cy="'+Math.round(W*.5)+'" rx="'+Math.round(N*.18)+'" ry="'+Math.round(W*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+Ae+'"/><rect x="'+Math.round(N*.2)+'" y="'+Math.round(W*.82)+'" width="'+Math.round(N*.6)+'" height="'+Math.round(W*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+Ae+'"/>'+ie+"</svg>"}function K(R,N,W){var P=W>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+W+"</div>":"",X=N?'data-form-id="'+N.id+'"':"",pe=R.length>7?14:R.length>5?16:19,ie=!!N;return"<div "+X+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(ie?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(ie?"":"filter:grayscale(1);opacity:0.5")+'">'+P+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(ie?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+pe+"px;font-weight:900;color:"+(ie?"#1A6B3C":"#aaa")+';line-height:1">'+R+'</div></div><div style="flex:1;overflow:hidden;background:'+(ie?"#1A6B3C":"#ccc")+'">'+Q(R,140,220)+"</div></div>"}function ae(R,N){var W=.54,P=Math.round(140*W),X=Math.round(305*W),pe=Math.round(X*.22),ie=X-pe,Ae=R.length>7?5:7,ke=Q(R,P,ie),ve=N?"1.5px solid #2a7a40":"1px solid #ddd",_e=N?"":"filter:grayscale(1);opacity:0.45;",we=N?"#1A6B3C":"#bbb",Le="#fff";return'<div style="width:'+P+"px;height:"+X+"px;border-radius:6px;border:"+ve+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+_e+'"><div style="height:'+pe+"px;background:"+we+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+Ae+"px;font-weight:900;color:"+Le+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(P-4)+'px">'+R+'</span></div><div style="height:'+ie+'px;overflow:hidden;flex-shrink:0">'+ke+"</div></div>"}function ge(R){if(S){const N=re();if(!N.length){R.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const W=N.map(P=>i.find(X=>X.player.id===P.id)||{_fake:!0,player:P,id:"fake-"+P.id});te(W,P=>P._fake?bi(P.player):Vt(P,""),P=>P._fake?F({player:P.player,id:"x",_fake:!0}):F(P),P=>{P._fake||vi(P,i,E,t)},"#1A6B3C")}else{const N=ue();if(!N.length){R.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const W={},P=[];N.forEach(X=>{W[X.player.id]||(W[X.player.id]=!0,P.push(X))}),te(P,X=>{const pe=E[X.player.id]||1,ie=pe>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${pe}</div>`:"",ke=i.filter(ve=>ve.player.id===X.player.id&&ve.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Vt(X,ie+ke)},X=>F(X),X=>vi(X,i,E,t),"#1A6B3C")}}function me(R){const N=S?k:[...z];if(!N.length){R.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const W=N.map(P=>({formation:P,card:f.find(X=>X.formation===P)||null,owned:z.has(P)}));te(W,({formation:P,card:X,owned:pe})=>K(P,pe?X:null,pe?f.filter(ie=>ie.formation===P).length:0),({formation:P,owned:X})=>ae(P,X),({card:P,owned:X})=>{X&&P&&mn(P,f,t,s)},"#1A6B3C")}function xe(R){const N=Object.keys(y),W=S?N.length?N:n:[...M];if(!W.length){R.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const P=W.map(X=>({type:X,gc:Be[X]||{icon:"⚡",desc:""},def:y[X]||null,owned:M.has(X),card:r.find(pe=>pe.gc_type===X)||null}));te(P,({type:X,gc:pe,def:ie,owned:Ae,card:ke})=>{const ve=Ae?r.filter(b=>b.gc_type===X).length:0,_e=ve>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${ve}</div>`:"",we=(ie==null?void 0:ie.gc_type)==="ultra_game_changer",Le={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},Ge={purple:"#b06ce0",light_blue:"#00d4ef"},Se=Le[ie==null?void 0:ie.color]||Le.purple,Ke=Ge[ie==null?void 0:ie.color]||Ge.purple,c=(ie==null?void 0:ie.effect)||pe.desc||"",g=ie!=null&&ie.image_url?`/manager-wars/icons/${ie.image_url}`:null;return Ae&&ke?`<div data-gc-id="${ke.id}" data-gc-type="${X}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${Ke};background:${Se};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${Ke}66;cursor:pointer">
          ${_e}
          ...
          <div style="height:150px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
            ${g?`<img src="${g}" style="max-width:120px;max-height:120px;object-fit:contain;border-radius:6px">`:`<span style="font-size:64px">${pe.icon}</span>`}
          </div>
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const _t=1e3;function mn(e,t,o,a){var n,E,z;const{state:p,toast:s,closeModal:d,navigate:l,refreshProfile:u}=o,i=e.formation,r={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function f(){const M=St[i]||{},j=oi[i]||[],O=290,B=360,S=20;function C(ue){const re=M[ue];return re?{x:re.x*O,y:re.y*B}:null}let ee=`<svg width="${O}" height="${B}" viewBox="0 0 ${O} ${B}" xmlns="http://www.w3.org/2000/svg">`;for(const[ue,re]of j){const D=C(ue),Y=C(re);!D||!Y||(ee+=`<line x1="${D.x}" y1="${D.y}" x2="${Y.x}" y2="${Y.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const ue of Object.keys(M)){const re=C(ue);if(!re)continue;const D=ue.replace(/\d+/,""),Y=r[D]||"#555";ee+=`<circle cx="${re.x}" cy="${re.y}" r="${S}" fill="${Y}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,ee+=`<text x="${re.x}" y="${re.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${D}</text>`}return ee+="</svg>",ee}const m=t.filter(M=>M.formation===i),y=m.length,k=!e.is_for_sale;a(`Formation ${i}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(n=document.getElementById("direct-sell-form-btn"))==null||n.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${i} pour ${_t.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const M=m.find(O=>!O.is_for_sale)||m[0];if(!M){s("Aucune carte à vendre","error");return}const{error:j}=await A.from("cards").delete().eq("id",M.id);if(j){s(j.message,"error");return}await A.from("users").update({credits:(p.profile.credits||0)+_t}).eq("id",p.profile.id),await u(),s(`+${_t.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),d(),l("collection")}),(E=document.getElementById("market-sell-form-btn"))==null||E.addEventListener("click",async()=>{const M=parseInt(document.getElementById("sell-price-form").value);if(!M||M<1){s("Prix invalide","error");return}await A.from("cards").update({is_for_sale:!0,sale_price:M}).eq("id",e.id),await A.from("market_listings").insert({seller_id:p.profile.id,card_id:e.id,price:M}),s("Carte mise en vente sur le marché !","success"),d(),l("collection")}),(z=document.getElementById("cancel-sell-form-btn"))==null||z.addEventListener("click",async()=>{await A.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await A.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),s("Annonce retirée","success"),d(),l("collection")})}async function vi(e,t,o,a){var re,D,Y,te,F,Q;const{state:p,toast:s,openModal:d,closeModal:l,navigate:u,refreshProfile:i}=a,r=e.player,f=t.filter(K=>K.player.id===r.id),m=f.length,y=un[r.rarity]||1e3,k=r.rarity!=="legende",n=ji(r),E=Mt(r,r.job),z=r.job2?Mt(r,r.job2):null,M=At[r.job]||"#1A6B3C",j=r.job2?At[r.job2]:null,O=Ci[r.rarity]||"#ccc",B=si[r.country_code]||r.country_code||"",S=f.map(K=>K.id);let C={};if(S.length){const{data:K}=await A.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",S).order("transferred_at",{ascending:!0});(K||[]).forEach(ae=>{C[ae.card_id]||(C[ae.card_id]=[]),C[ae.card_id].push(ae)})}const ee=K=>`
    ...
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      ...
      <div style="display:flex;flex-direction:column;gap:12px">
        ${f.map((K,ae)=>{const ge=C[K.id]||[];return ge.length?`
            <div>
      ...
      <div style="width:140px;border-radius:12px;padding:6px;background:${O};flex-shrink:0">
        <div style="background:#f2e8d2;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
          ...
          <div style="background:#f2e8d2;padding:3px 6px;display:flex;align-items:center;justify-content:space-between;min-height:24px">
            ...
            ${(re=r.clubs)!=null&&re.logo_url?`<img src="${r.clubs.logo_url}" style="width:22px;height:16px;object-fit:contain">`:`<div style="background:#1a3a7a;color:#fff;border-radius:2px;padding:1px 3px;font-size:6px;font-weight:800">${(((D=r.clubs)==null?void 0:D.encoded_name)||"").slice(0,6)}</div>`}
          </div>
        </div>
      </div>
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",r.note_g],["DEF",r.note_d],["MIL",r.note_m],["ATT",r.note_a]].map(([K,ae])=>{const ge=At[K],me=Number(ae)||0;return`<div style="display:flex;flex-direction:column;align-items:center;gap:2px">
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;height:${f}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ...
      ${p?`<img src="${p}" style="width:${f-4}px;height:${f-4}px;object-fit:contain">`:(y=e==null?void 0:e.clubs)!=null&&y.encoded_name?`<span style="font-size:${Math.max(4,f-8)}px;font-weight:700;color:#333">${(e.clubs.encoded_name||"").slice(0,3).toUpperCase()}</span>`:"<span></span>"}
    </div>
  ...
  </div>`,document.getElementById("new-deck-btn").addEventListener("click",async()=>{const d=prompt("Nom du deck :",`Deck ${((s==null?void 0:s.length)||0)+1}`);if(!d)return;const{data:l,error:u}=await A.from("decks").insert({owner_id:o.profile.id,name:d}).select().single();if(u){p(u.message,"error");return}p("Deck créé !","success"),wi(l.id,e,t)}),e.querySelectorAll("[data-open-deck]").forEach(d=>{d.addEventListener("click",()=>wi(d.dataset.openDeck,e,t))}),e.querySelectorAll("[data-rename]").forEach(d=>{d.addEventListener("click",async()=>{const l=prompt("Nouveau nom :",d.dataset.name);if(!l||l===d.dataset.name)return;const{error:u}=await A.from("decks").update({name:l}).eq("id",d.dataset.rename);if(u){p(u.message,"error");return}p("Deck renommé !","success"),Xt(e,t)})}),e.querySelectorAll("[data-delete]").forEach(d=>{d.addEventListener("click",async()=>{if(!confirm(`Supprimer le deck "${d.dataset.name}" ? Cette action est irréversible.`))return;await A.from("deck_cards").delete().eq("deck_id",d.dataset.delete);const{error:l}=await A.from("decks").delete().eq("id",d.dataset.delete);if(l){p(l.message,"error");return}p("Deck supprimé.","success"),Xt(e,t)})})}async function wi(e,t,o){const{state:a,toast:p}=o;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:s}=await A.from("decks").select("*").eq("id",e).single(),{data:d}=await A.from("cards").select(`id, card_type, formation,
      player:players(id, firstname, surname_encoded, country_code, club_id, job, job2,
        note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length,
        clubs(encoded_name, logo_url))`).eq("owner_id",a.profile.id),l=(d||[]).filter(y=>y.card_type==="player"&&y.player),u=(d||[]).filter(y=>y.card_type==="formation"),i=u.map(y=>y.formation).filter(Boolean),{data:r}=await A.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",e);let f=s.formation||"4-4-2";i.length>0&&!i.includes(f)&&(f=i[0]);const m={deckId:e,name:s.name,formation:f,formationCardId:s.formation_card_id,slots:{},subs:[],playerCards:l,formationCards:u,availableFormations:i};(r||[]).forEach(y=>{y.is_starter?m.slots[y.position]=y.card_id:m.subs.includes(y.card_id)||m.subs.push(y.card_id)}),lt(t,m,o)}function lt(e,t,o){var u;const{navigate:a}=o;Dt[t.formation];const p=$i(t.formation),s=p.filter(i=>t.slots[i]).length,d=t.availableFormations.length>0?t.availableFormations:Object.keys(Dt),l=t.subs.map(i=>t.playerCards.find(r=>r.id===i)).filter(Boolean);[...Object.values(t.slots),...t.subs],e.innerHTML=`
  <div class="page">
  ...
  </div>`,yn(e,t,p,o),document.getElementById("builder-back").addEventListener("click",()=>a("decks")),document.getElementById("formation-select").addEventListener("change",i=>{t.formation=i.target.value;const r=$i(t.formation),f={};r.forEach(m=>{t.slots[m]&&(f[m]=t.slots[m])}),t.slots=f,lt(e,t,o)}),document.getElementById("save-deck").addEventListener("click",()=>vn(t,o)),e.querySelectorAll("[data-remove-sub]").forEach(i=>{i.addEventListener("click",()=>{t.subs=t.subs.filter(r=>r!==i.dataset.removeSub),lt(e,t,o)})}),(u=document.getElementById("add-sub-btn"))==null||u.addEventListener("click",()=>{bn(t,e,o)})}function yn(e,t,o,a){const p=e.querySelector("#deck-field");if(!p)return;const s=St[t.formation]||{},d=Ai(t.formation)||[],l={};for(const z of o){const M=t.slots[z],j=M?t.playerCards.find(O=>O.id===M):null;l[z]=j?j.player:null}const u=300,i=300,r=48,f=64,m=13,y=16,k=38;function n(z){const M=s[z];return M?{x:M.x*u,y:M.y*i}:null}let E="";for(const[z,M]of d){const j=n(z),O=n(M);if(!j||!O)continue;const B=l[z]?{...l[z],club_id:l[z].club_id,country_code:l[z].country_code,rarity:l[z].rarity}:null,S=l[M]?{...l[M],club_id:l[M].club_id,country_code:l[M].country_code,rarity:l[M].rarity}:null,C=Qe(B,S);C==="#ff3333"||C==="#cc2222"?E+=`<line x1="${j.x.toFixed(1)}" y1="${j.y.toFixed(1)}" x2="${O.x.toFixed(1)}" y2="${O.y.toFixed(1)}" stroke="${C}" stroke-width="2.5" stroke-linecap="round" opacity="0.4"/>`:(E+=`<line x1="${j.x.toFixed(1)}" y1="${j.y.toFixed(1)}" x2="${O.x.toFixed(1)}" y2="${O.y.toFixed(1)}" stroke="${C}" stroke-width="8" stroke-linecap="round" opacity="0.2"/>`,E+=`<line x1="${j.x.toFixed(1)}" y1="${j.y.toFixed(1)}" x2="${O.x.toFixed(1)}" y2="${O.y.toFixed(1)}" stroke="${C}" stroke-width="2.5" stroke-linecap="round" opacity="0.9"/>`)}for(const z of o){const M=n(z);if(!M)continue;const j=l[z],O=z.replace(/\d+/,""),B=ot[O]||"#555",S=(M.x-r/2).toFixed(1),C=(M.y-f/2).toFixed(1),ee={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[j==null?void 0:j.rarity]||"#aaa";if(j){const ue=Ot(j),re=Bi(j),D=Fi(j.country_code),Y=Number(O==="GK"?j.note_g:O==="DEF"?j.note_d:O==="MIL"?j.note_m:j.note_a)||0,te=f-m-y;E+=`<defs><clipPath id="dcp-${z}"><rect x="${S}" y="${(M.y-f/2+m).toFixed(1)}" width="${r}" height="${te}"/></clipPath></defs>`,E+=`<rect x="${S}" y="${C}" width="${r}" height="${f}" rx="5" fill="${B}"/>`,ue&&(E+=`<image href="${ue}" x="${S}" y="${(M.y-f/2+m).toFixed(1)}" width="${r}" height="${te}" clip-path="url(#dcp-${z})" preserveAspectRatio="xMidYMin slice"/>`),E+=`<rect x="${S}" y="${C}" width="${r}" height="${m}" fill="rgba(255,255,255,0.93)"/>`,E+=`<text x="${M.x.toFixed(1)}" y="${(M.y-f/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(j.surname_encoded||"").slice(0,9)}</text>`;const F=(M.y+f/2-y).toFixed(1);E+=`<rect x="${S}" y="${F}" width="${r}" height="${y}" fill="rgba(255,255,255,0.93)"/>`,D&&(E+=`<image href="${D}" x="${(M.x-21).toFixed(1)}" y="${(M.y+f/2-y+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`),E+=`<text x="${M.x.toFixed(1)}" y="${(M.y+f/2-y/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${Y}</text>`,re&&(E+=`<image href="${re}" x="${(M.x+r/2-14).toFixed(1)}" y="${(M.y+f/2-y+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`),E+=`<rect x="${S}" y="${C}" width="${r}" height="${f}" rx="5" fill="none" stroke="${ee}" stroke-width="2"/>`}else E+=`<rect x="${S}" y="${C}" width="${r}" height="${f}" rx="5" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="5,3"/>`,E+=`<text x="${M.x.toFixed(1)}" y="${M.y.toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="22" fill="rgba(255,255,255,0.35)">+</text>`,E+=`<text x="${M.x.toFixed(1)}" y="${(M.y+16).toFixed(1)}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.3)">${O}</text>`;E+=`<rect x="${S}" y="${C}" width="${r}" height="${f}" rx="5" fill="rgba(0,0,0,0.01)" class="deck-slot-hit" data-pos="${z}" style="cursor:pointer"/>`}p.innerHTML=`<svg viewBox="${-k} ${-k} ${u+k*2} ${i+k*2}" width="100%" style="display:block;max-width:440px;margin:0 auto">${E}</svg>`,p.querySelectorAll(".deck-slot-hit").forEach(z=>{z.addEventListener("click",()=>hn(z.dataset.pos,t,e,a))})}function hn(e,t,o,a){var m,y,k;const{openModal:p,closeModal:s}=a,d=e.replace(/\d+/,""),l=t.slots[e],u=l?t.playerCards.find(n=>n.id===l):null;(m=u==null?void 0:u.player)==null||m.id;const i=new Set;Object.entries(t.slots).forEach(([n,E])=>{var M;if(n===e||!E)return;const z=t.playerCards.find(j=>j.id===E);(M=z==null?void 0:z.player)!=null&&M.id&&i.add(z.player.id)}),t.subs.forEach(n=>{var z;const E=t.playerCards.find(M=>M.id===n);(z=E==null?void 0:E.player)!=null&&z.id&&i.add(E.player.id)});const r=new Set,f=t.playerCards.filter(n=>{const E=n.player;return!(E.job===d||E.job2===d)||i.has(E.id)||r.has(E.id)?!1:(r.add(E.id),!0)});f.sort((n,E)=>{const z=d==="GK"?n.player.note_g:d==="DEF"?n.player.note_d:d==="MIL"?n.player.note_m:n.player.note_a;return(d==="GK"?E.player.note_g:d==="DEF"?E.player.note_d:d==="MIL"?E.player.note_m:E.player.note_a)-z}),p(`Choisir ${d} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${f.length>0?f.map(n=>{var O,B;const E=n.player,z=d==="GK"?E.note_g:d==="DEF"?E.note_d:d==="MIL"?E.note_m:E.note_a,M=Ot(E),j={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[E.rarity];return`<div class="player-option" data-card-id="${n.id}"
          ...
          <div style="flex:1;min-width:0">
            ...
            <div style="display:flex;align-items:center;gap:6px;margin-top:3px">
              ...
              ${(O=E.clubs)!=null&&O.logo_url?`<img src="${E.clubs.logo_url}" style="width:18px;height:18px;object-fit:contain">`:`<span style="font-size:10px;color:var(--gray-600)">${((B=E.clubs)==null?void 0:B.encoded_name)||"—"}</span>`}
    ...
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(y=document.getElementById("close-selector"))==null||y.addEventListener("click",s),(k=document.getElementById("remove-player"))==null||k.addEventListener("click",()=>{delete t.slots[e],s(),lt(o,t,a)}),document.querySelectorAll(".player-option").forEach(n=>{n.addEventListener("click",()=>{t.slots[e]=n.dataset.cardId,s(),lt(o,t,a)})})}function bn(e,t,o){var u;const{openModal:a,closeModal:p}=o,s=new Set;Object.values(e.slots).filter(Boolean).forEach(i=>{var f;const r=e.playerCards.find(m=>m.id===i);(f=r==null?void 0:r.player)!=null&&f.id&&s.add(r.player.id)}),e.subs.forEach(i=>{var f;const r=e.playerCards.find(m=>m.id===i);(f=r==null?void 0:r.player)!=null&&f.id&&s.add(r.player.id)});const d=new Set,l=e.playerCards.filter(i=>{var r,f,m;return s.has((r=i.player)==null?void 0:r.id)||d.has((f=i.player)==null?void 0:f.id)?!1:(d.add((m=i.player)==null?void 0:m.id),!0)});a("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${l.length>0?l.map(i=>{var y;const r=i.player,f=Ot(r),m=r.job==="GK"?r.note_g:r.job==="DEF"?r.note_d:r.job==="MIL"?r.note_m:r.note_a;return`<div class="player-option" data-card-id="${i.id}"
          style="display:flex;align-items:center;gap:10px;padding:8px;border:1.5px solid var(--gray-200);border-radius:10px;cursor:pointer">
  ...
  `,document.body.appendChild(p);let s=!1;const d=document.getElementById("pack-cut-zone"),l=document.getElementById("pack-blade");let u=!1;const i=F=>F.touches&&F.touches[0]?F.touches[0].clientX:F.clientX;function r(F){s||(u=!0,l.style.opacity="1",f(F))}function f(F){if(!u||s)return;const Q=d.getBoundingClientRect(),K=i(F)-Q.left,ae=Math.max(0,Math.min(1,K/Q.width));l.style.width=ae*Q.width+"px",ae>=.82&&y()}function m(){s||(u=!1,l.style.transition="width .2s ease, opacity .2s ease",l.style.width="0",l.style.opacity="0",setTimeout(()=>{s||(l.style.transition="")},220))}function y(){var Q;if(s)return;s=!0,u=!1,l.style.width="100%",l.style.opacity="1",(Q=document.getElementById("cut-flash"))==null||Q.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const F=document.getElementById("cut-hint");F&&(F.style.opacity="0"),d.classList.add("pack-cut"),setTimeout(()=>{l.style.opacity="0",document.getElementById("pack-phase").style.display="none",z(0)},620)}d.addEventListener("pointerdown",r),window.addEventListener("pointermove",f),window.addEventListener("pointerup",m),d.addEventListener("touchstart",r,{passive:!0}),window.addEventListener("touchmove",f,{passive:!0}),window.addEventListener("touchend",m);let k=0,n=!1;const E=new Set;function z(F){k=F,document.getElementById("reveal-phase").style.display="flex",M(),j(F,0),ee()}function M(){const F=document.getElementById("card-dots");F&&(F.innerHTML=e.map((Q,K)=>`<div class="card-dot" data-i="${K}" style="width:8px;height:8px;border-radius:50%;background:${K===k?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),F.querySelectorAll(".card-dot").forEach(Q=>Q.addEventListener("click",()=>B(parseInt(Q.dataset.i)))))}function j(F,Q){var W;const K=e[F],ae=document.getElementById("card-counter"),ge=document.getElementById("card-track");ae&&(ae.textContent=`Carte ${F+1} / ${e.length}`);const me=K.card_type==="player"&&((W=K.player)==null?void 0:W.rarity)==="legende",xe=!E.has(F);E.add(F);let R=0;if(K.card_type==="player"&&K.player){const P=K.player,X=P.job||"ATT";R=Number(X==="GK"?P.note_g:X==="DEF"?P.note_d:X==="MIL"?P.note_m:P.note_a)||0}const N=P=>{ge.innerHTML=`
        ...
        </div>`;const X=document.getElementById("current-card-wrap");Q!==0?(X.style.transition="none",X.style.transform=`translateX(${Q>0?100:-100}%)`,requestAnimationFrame(()=>{X.style.transition="transform .28s cubic-bezier(.25,1,.5,1)",X.style.transform="translateX(0)"})):X.animate([{opacity:0,transform:"scale(.7)"},{opacity:1,transform:"scale(1)"}],{duration:300,easing:"cubic-bezier(.34,1.56,.64,1)"}),P||me?re():D(),M()};xe&&R>6&&K.card_type==="player"&&K.player?O(K,()=>N(!0)):N(!1)}function O(F,Q){var N;n=!0;const K=F.player,ae=`https://flagsapi.com/${K.country_code}/flat/64.png`,ge=(N=K.clubs)==null?void 0:N.logo_url,me=document.getElementById("walkout-overlay"),xe=document.getElementById("walkout-stage");if(!me||!xe){n=!1,Q();return}me.style.display="flex";const R=()=>{const W=xe.firstElementChild;W&&(W.classList.remove("wo-in"),W.classList.add("wo-out"))};xe.innerHTML=`<img class="wo-in" src="${ae}" style="height:130px;border-radius:10px;box-shadow:0 10px 36px rgba(0,0,0,.6)" onerror="this.style.display='none'">`,navigator.vibrate&&navigator.vibrate(30),setTimeout(R,2e3),setTimeout(()=>{var W;xe.innerHTML=ge?`<img class="wo-in" src="${ge}" style="max-height:160px;max-width:210px;object-fit:contain">`:`<div class="wo-in" style="font-size:34px;font-weight:900;color:#fff;text-align:center">${((W=K.clubs)==null?void 0:W.encoded_name)||"CLUB"}</div>`,navigator.vibrate&&navigator.vibrate(30)},2450),setTimeout(R,4450),setTimeout(()=>{me.style.display="none",xe.innerHTML="",n=!1,navigator.vibrate&&navigator.vibrate([40,30,80]),Q()},4900)}function B(F){if(n||F<0||F>=e.length||F===k)return;const Q=F>k?1:-1;k=F,j(F,Q)}function S(){B(k+1)}function C(){B(k-1)}function ee(){const F=document.getElementById("card-viewport");if(!F||F._swipeBound)return;F._swipeBound=!0;let Q=0,K=0,ae=0,ge=!1;const me=P=>P.touches?P.touches[0].clientX:P.clientX,xe=P=>P.touches?P.touches[0].clientY:P.clientY,R=P=>{ge=!0,Q=me(P),K=xe(P),ae=0},N=P=>{if(!ge)return;ae=me(P)-Q;const X=xe(P)-K;if(Math.abs(ae)<Math.abs(X))return;const pe=document.getElementById("current-card-wrap");pe&&(pe.style.transition="none",pe.style.transform=`translateX(${ae*.6}px) rotate(${ae*.02}deg)`)},W=()=>{if(!ge)return;ge=!1;const P=document.getElementById("current-card-wrap"),X=55;ae<=-X&&k<e.length-1?S():ae>=X&&k>0?C():P&&(P.style.transition="transform .2s ease",P.style.transform="translateX(0)")};F.addEventListener("pointerdown",R),F.addEventListener("pointermove",N),F.addEventListener("pointerup",W),F.addEventListener("pointercancel",W),F.addEventListener("touchstart",R,{passive:!0}),F.addEventListener("touchmove",N,{passive:!0}),F.addEventListener("touchend",W),F.addEventListener("click",P=>{if(Math.abs(ae)>8)return;const X=F.getBoundingClientRect();P.clientX-X.left>X.width/2?S():C()})}let ue=null;function re(){const F=document.getElementById("fireworks-canvas");if(!F)return;F.width=window.innerWidth,F.height=window.innerHeight;const Q=F.getContext("2d"),K=[];function ae(){const me=Math.random()*F.width,xe=Math.random()*F.height*.6,R=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],N=R[Math.floor(Math.random()*R.length)];for(let W=0;W<60;W++){const P=Math.PI*2/60*W,X=2+Math.random()*5;K.push({x:me,y:xe,vx:Math.cos(P)*X,vy:Math.sin(P)*X,alpha:1,color:N,size:2+Math.random()*3})}}ae(),ue=setInterval(ae,600);function ge(){if(document.getElementById("fireworks-canvas")){Q.clearRect(0,0,F.width,F.height);for(let me=K.length-1;me>=0;me--){const xe=K[me];if(xe.x+=xe.vx,xe.y+=xe.vy+.08,xe.vy*=.98,xe.alpha-=.018,xe.alpha<=0){K.splice(me,1);continue}Q.globalAlpha=xe.alpha,Q.fillStyle=xe.color,Q.beginPath(),Q.arc(xe.x,xe.y,xe.size,0,Math.PI*2),Q.fill()}Q.globalAlpha=1,(ue!==null||K.length>0)&&requestAnimationFrame(ge)}}ge()}function D(){ue!==null&&(clearInterval(ue),ue=null);const F=document.getElementById("fireworks-canvas");F&&F.getContext("2d").clearRect(0,0,F.width,F.height)}if(a){const F=document.getElementById("reveal-phase"),Q=F==null?void 0:F.querySelector('div[style*="display:flex"][style*="gap:10px"]');Q&&(Q.innerHTML='<button class="btn btn-primary" id="reveal-next" style="flex:1">Continuer →</button>'),(te=document.getElementById("reveal-next"))==null||te.addEventListener("click",()=>{D(),p.remove(),a()})}else document.getElementById("reveal-collection").addEventListener("click",()=>{D(),p.remove(),o("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{D(),p.remove(),o("boosters")})}function Mn(e){var t,o;if(e.card_type==="player"&&e.player)return Tn(e);if(e.card_type==="game_changer"){const a=e._gcDef,p=(a==null?void 0:a.gc_type)==="ultra_game_changer",s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},d={purple:"#b06ce0",light_blue:"#00d4ef"},l=s[a==null?void 0:a.color]||s.purple,u=d[a==null?void 0:a.color]||d.purple,i=(a==null?void 0:a.name)||e.gc_type||"Game Changer",r=(a==null?void 0:a.effect)||((t=Jt[e.gc_type])==null?void 0:t.desc)||"",f=a!=null&&a.image_url?`/manager-wars/icons/${a.image_url}`:null,m=((o=Jt[e.gc_type])==null?void 0:o.icon)||"⚡";return`<div style="width:170px;height:240px;background:${l};border-radius:14px;border:3px solid ${u};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${u}88;flex-shrink:0">
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    </div>`}const u=r=>{e.style.overflow="",e.style.height="",e.style.display="",e.style.flexDirection="",o(r)};function i(){var f,m,y;e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column";const r=d.length>0;e.innerHTML=`
    ...
    </div>`,e.querySelectorAll(".gc-select-card").forEach(k=>{k.addEventListener("click",()=>{const n=k.dataset.id,E=s.find(M=>M.id===n);if(!E)return;const z=d.findIndex(M=>M.gc_type===E.gc_type);z>-1?d.splice(z,1):d.length<3&&d.push(E),i()})}),(f=e.querySelector("#gc-launch"))==null||f.addEventListener("click",()=>{r&&u(d)}),(m=e.querySelector("#gc-no-gc"))==null||m.addEventListener("click",()=>u([])),(y=e.querySelector("#gc-reset"))==null||y.addEventListener("click",()=>{d.length&&(d=[],i())})}i()}async function Nn(e,t,o){const{state:a,navigate:p}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:s}=await A.from("decks").select("id,name,is_active,formation").eq("owner_id",a.profile.id).order("created_at",{ascending:!1});if(!s||s.length===0){rt(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>p("decks"));return}const d=s.map(r=>r.id),{data:l}=await A.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",d).order("slot_order");let u=0;function i(){var M,j,O,B,S;const r=s[u],f=(l||[]).filter(C=>C.deck_id===r.id),m=f.filter(C=>{var ee;return C.is_starter&&((ee=C.card)==null?void 0:ee.player)}).map(C=>Zt(C.card,C.position)),y=f.find(C=>{var ee;return((ee=C.card)==null?void 0:ee.card_type)==="formation"}),k=r.formation||((M=y==null?void 0:y.card)==null?void 0:M.formation)||"4-4-2",n=m.length>=11?ut(m,k):null,E=m.length>=11;We(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">
        ${s.map((C,ee)=>`<div style="width:7px;height:7px;border-radius:50%;background:${ee===u?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}
      </div>`:""}
    ...
    </div>`,function(){const ee=e.querySelector(".deck-preview-wrap svg");ee&&(ee.removeAttribute("width"),ee.removeAttribute("height"),ee.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",ee.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),(j=document.getElementById("prev-deck"))==null||j.addEventListener("click",()=>{u>0&&(u--,i())}),(O=document.getElementById("next-deck"))==null||O.addEventListener("click",()=>{u<s.length-1&&(u++,i())}),(B=document.getElementById("validate-deck"))==null||B.addEventListener("click",()=>{E&&t.navigate("match",{matchMode:o,deckId:r.id})}),(S=document.getElementById("cancel-deck-select"))==null||S.addEventListener("click",()=>{Ne(e),p("home")});const z=document.getElementById("deck-swipe-zone");if(z){let C=0,ee=0;z.addEventListener("touchstart",ue=>{C=ue.touches[0].clientX,ee=ue.touches[0].clientY},{passive:!0}),z.addEventListener("touchend",ue=>{const re=ue.changedTouches[0].clientX-C,D=ue.changedTouches[0].clientY-ee;Math.abs(re)<40||Math.abs(re)<Math.abs(D)||(re<0&&u<s.length-1?(u++,i()):re>0&&u>0&&(u--,i()))},{passive:!0})}}i()}function Ui(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";return e!=null&&e.clubLogo?e.clubLogo.startsWith("http")?e.clubLogo:`${t}/storage/v1/object/public/assets/clubs/${e.clubLogo}`:null}function Ue(e,t=44,o=58){if(!e)return`<div style="width:${t}px;height:${o}px;border:1.5px dashed rgba(255,255,255,0.2);border-radius:5px"></div>`;const a=typeof e.portrait=="string"&&e.portrait.startsWith("http")?e.portrait:Ie(e),p=Ui(e),s=e._line||e.job||"MIL",d=pt[s]||"#555",l={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",u=e.note!==void 0?Number(e.note)||0:(Number(be(e,s))||0)+(e.boost||0),i=Ki(e==null?void 0:e.country_code),r=Math.round(o*.19),f=Math.round(o*.25),m=o-Math.round(o*.19)-Math.round(o*.25),y=e!=null&&e.used?.28:1;return`<div style="width:${t}px;height:${o}px;border-radius:5px;border:2px solid ${l};background:${d};position:relative;overflow:hidden;flex-shrink:0;opacity:${y}">
    <div style="position:absolute;top:0;left:0;right:0;height:${r}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:center;z-index:2">
  ...
  </div>`}function et(e,t,o){if(!(e!=null&&e.length))return"";const a=e.slice(0,5);let p='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return a.forEach((s,d)=>{if(p+=Ue(s,40,52),d<a.length-1){const l=Qe(s,a[d+1]);p+=`<div style="width:7px;height:3px;background:${l==="#ff3333"||l==="#cc2222"?"rgba(255,255,255,0.12)":l};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),o!==void 0&&(p+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${o}</div>`),p+="</div>",p}function ei(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function Ki(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function at(e,t,o,a,p=310,s=310,d=[]){var j;const l=St[t]||{},u=Ai(t)||oi[t]||[],i={},r=["ATT","MIL","DEF","GK"];for(const O of r)(e[O]||[]).forEach((S,C)=>{i[`${O}${C+1}`]=S});function f(O){const B=l[O];return B?{x:B.x*p,y:B.y*s}:null}let m="";for(const[O,B]of u){const S=f(O),C=f(B);if(!S||!C)continue;const ee=i[O],ue=i[B],re=Qe(ee,ue);re==="#00ff88"||re==="#FFD700"?(m+=`<line x1="${S.x.toFixed(1)}" y1="${S.y.toFixed(1)}" x2="${C.x.toFixed(1)}" y2="${C.y.toFixed(1)}"
        stroke="${re}" stroke-width="10" stroke-linecap="round" opacity="0.22"/>`,m+=`<line x1="${S.x.toFixed(1)}" y1="${S.y.toFixed(1)}" x2="${C.x.toFixed(1)}" y2="${C.y.toFixed(1)}"
        stroke="${re}" stroke-width="3.5" stroke-linecap="round" opacity="0.95"/>`):m+=`<line x1="${S.x.toFixed(1)}" y1="${S.y.toFixed(1)}" x2="${C.x.toFixed(1)}" y2="${C.y.toFixed(1)}"
        stroke="${re}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const y=48,k=64,n=13,E=16,z={normal:"#aaaaaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#111111",legende:"#7a28b8",légende:"#7a28b8"};for(const[O,B]of Object.entries(i)){const S=f(O);if(!S||!B)continue;const C=O.replace(/[0-9]/g,""),ee=pt[C]||"#555",ue=d.includes(B.cardId),re=o==="attack"&&(["MIL","ATT"].includes(C)||ue)&&!B.used||o==="defense"&&["GK","DEF","MIL"].includes(C)&&!B.used,D=a.includes(B.cardId);let Y;o==="attack"?Y=ue?Math.max(1,Number(B.note_a)||0):C==="MIL"?Number(B.note_m)||0:Number(B.note_a)||0:o==="defense"?Y=C==="GK"?Number(B.note_g)||0:C==="MIL"?Number(B.note_m)||0:Number(B.note_d)||0:Y=Number(C==="GK"?B.note_g:C==="DEF"?B.note_d:C==="MIL"?B.note_m:B.note_a)||0,Y=Y+(B.boost||0);const te=(S.x-y/2).toFixed(1),F=(S.y-k/2).toFixed(1),Q=z[B==null?void 0:B.rarity]||z.normal;if(B.used){const N=typeof import.meta<"u"&&"/manager-wars/"||"/",P=`${typeof window<"u"&&((j=window.location)==null?void 0:j.origin)||""}${N}icons/carte-dos.png`.replace(/([^:])\/\//g,"$1/");m+=`<rect x="${te}" y="${F}" width="${y}" height="${k}" rx="5" fill="#161616"/>`,m+=`<image href="${P}" xlink:href="${P}" x="${te}" y="${F}" width="${y}" height="${k}" preserveAspectRatio="xMidYMid slice"/>`,m+=`<rect x="${te}" y="${F}" width="${y}" height="${k}" rx="5" fill="none" stroke="${Q}" stroke-width="2" opacity="0.7"/>`,m+=`<rect x="${te}" y="${F}" width="${y}" height="${k}" rx="5" fill="rgba(0,0,0,0.01)" class="match-used-hit" data-card-id="${B.cardId}" data-role="${C}" style="cursor:pointer"/>`;continue}const K=D?.45:1,ae=D?"#FFD700":Q,ge=D?3:(B==null?void 0:B.rarity)==="legende"||(B==null?void 0:B.rarity)==="légende"||(B==null?void 0:B.rarity)==="pepite"||(B==null?void 0:B.rarity)==="pépite"?2.5:2,me=k-n-E;m+=`<defs><clipPath id="cp-${O}"><rect x="${te}" y="${(S.y-k/2+n).toFixed(1)}" width="${y}" height="${me}"/></clipPath></defs>`,m+=`<rect x="${te}" y="${F}" width="${y}" height="${k}" rx="5" fill="${ee}" opacity="${K}"/>`;const xe=Ie(B);xe&&(m+=`<image href="${xe}" xlink:href="${xe}" x="${te}" y="${(S.y-k/2+n).toFixed(1)}" width="${y}" height="${me}" clip-path="url(#cp-${O})" preserveAspectRatio="xMidYMin slice"/>`),m+=`<rect x="${te}" y="${F}" width="${y}" height="${n}" rx="4" fill="rgba(255,255,255,0.92)"/>`,m+=`<text x="${S.x.toFixed(1)}" y="${(S.y-k/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(B.name||"").slice(0,9)}</text>`;const R=(S.y+k/2-E).toFixed(1);m+=`<rect x="${te}" y="${R}" width="${y}" height="${E}" fill="rgba(255,255,255,0.92)"/>`;{const N=ei(B.country_code);N?m+=`<image href="${N}" xlink:href="${N}" x="${(S.x-20).toFixed(1)}" y="${(S.y+k/2-E+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:m+=`<text x="${(S.x-13).toFixed(1)}" y="${(S.y+k/2-E/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${Ki(B.country_code)}</text>`,m+=`<text x="${S.x.toFixed(1)}" y="${(S.y+k/2-E/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${Y}</text>`;const W=Ui(B);W?m+=`<image href="${W}" xlink:href="${W}" x="${(S.x+y/2-14).toFixed(1)}" y="${(S.y+k/2-E+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:B.clubName&&(m+=`<text x="${(S.x+14).toFixed(1)}" y="${(S.y+k/2-E/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(B.clubName||"").slice(0,3).toUpperCase()}</text>`),B.boost&&(m+=`<rect x="${(S.x+y/2-12).toFixed(1)}" y="${(S.y-k/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,m+=`<text x="${(S.x+y/2-5).toFixed(1)}" y="${(S.y-k/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${B.boost}</text>`)}m+=`<rect x="${te}" y="${F}" width="${y}" height="${k}" rx="5" fill="${D?"rgba(255,255,255,0.12)":"none"}" stroke="${ae}" stroke-width="${ge}" opacity="${K}"/>`,re&&(m+=`<rect x="${te}" y="${F}" width="${y}" height="${k}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${D?"selected":""}" data-card-id="${B.cardId}" data-role="${C}" style="cursor:pointer"/>`)}const M=38;return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-M} ${-M} ${p+M*2} ${s+M*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${m}
  ...
  </div>`}async function pi(e,t,o,a){var B;const{state:p,navigate:s,toast:d}=t;We(e);const l=p.params||{};if(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!l.deckId)return Nn(e,t,o);const u=l.deckId;let i,r,f,m;try{const S=await Promise.all([A.from("decks").select("formation,name").eq("id",u).single(),A.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation,
            player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
              clubs(encoded_name,logo_url)))`).eq("deck_id",u).order("slot_order")]);i=S[0].data,f=S[0].error,r=S[1].data,m=S[1].error}catch(S){console.error("[Match] Exception chargement deck:",S),rt(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>s("home"));return}if(f||m){console.error("[Match] Erreur Supabase:",f||m),rt(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>s("home"));return}const y=(r||[]).filter(S=>{var C;return S.is_starter&&((C=S.card)==null?void 0:C.player)}).map(S=>Zt(S.card,S.position)),k=(r||[]).filter(S=>{var C;return!S.is_starter&&((C=S.card)==null?void 0:C.player)}).map(S=>Zt(S.card,S.position));if(y.length<11){rt(e,"⚠️",`Deck incomplet (${y.length}/11).`,"Compléter",()=>s("decks"));return}const n=(r||[]).find(S=>{var C;return((C=S.card)==null?void 0:C.card_type)==="formation"}),E=(i==null?void 0:i.formation)||((B=n==null?void 0:n.card)==null?void 0:B.formation)||"4-4-2",{data:z,error:M}=await A.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",p.profile.id).eq("card_type","game_changer"),{data:j}=await A.from("gc_definitions").select("*").eq("is_active",!0),O=(z||[]).map(S=>({...S,_gcDef:(j==null?void 0:j.find(C=>C.name===S.gc_type||C.id===S.gc_definition_id))||null}));a({deckId:u,formation:E,starters:y,subsRaw:k,gcCardsEnriched:O,gcDefs:j||[]})}async function Rn(e,t){const{state:o}=t,p=(o.params||{}).matchMode||"vs_ai_easy",s=p.replace("vs_ai_",""),d=p;await pi(e,t,p,async({deckId:l,formation:u,starters:i,subsRaw:r,gcCardsEnriched:f,gcDefs:m})=>{try{const y=ut(i,u),k=await Pn(u,s),n=async E=>{try{const{data:z,error:M}=await A.from("matches").insert({home_id:o.profile.id,away_id:null,mode:d,home_deck_id:l,status:"in_progress"}).select().single();if(M){console.error("[MatchIA] Erreur création match:",M),rt(e,"⚠️","Impossible de créer le match ("+M.message+").","Retour",()=>t.navigate("home"));return}const j={gcDefs:m||[],matchId:z==null?void 0:z.id,mode:d,difficulty:s,formation:u,homeTeam:y,aiTeam:k,homeSubs:r,subsUsed:0,maxSubs:Math.min(r.length,3),homeScore:0,aiScore:0,gcCards:E,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:o.profile.club_name||"Vous"};Hn(e,j,t)}catch(z){console.error("[MatchIA] Exception launchMatch:",z),rt(e,"⚠️","Erreur au lancement du match : "+z.message,"Retour",()=>t.navigate("home"))}};if(!f.length){n([]);return}ci(e,f,n)}catch(y){console.error("[MatchIA] Exception setup:",y),rt(e,"⚠️","Erreur de préparation du match : "+y.message,"Retour",()=>t.navigate("home"))}})}async function Pn(e,t){var d,l;const{data:o}=await A.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(60);if(!o||o.length<11)return On(e);const a=ct[e]||ct["4-4-2"],p={GK:[],DEF:[],MIL:[],ATT:[]},s=[...o];for(const u of["GK","DEF","MIL","ATT"]){const i=s.filter(k=>k.job===u),r=s.filter(k=>k.job!==u),f=[...i,...r],m=[];for(let k=0;k<a[u];k++){const n=f[k]||s[k];n&&m.push({cardId:"ai-"+n.id+"-"+k,id:n.id,firstname:n.firstname,name:n.surname_encoded,country_code:n.country_code,club_id:n.club_id,job:n.job,job2:n.job2,note_g:Number(n.note_g)||0,note_d:Number(n.note_d)||0,note_m:Number(n.note_m)||0,note_a:Number(n.note_a)||0,rarity:n.rarity,skin:n.skin,hair:n.hair,hair_length:n.hair_length,clubName:((d=n.clubs)==null?void 0:d.encoded_name)||null,clubLogo:((l=n.clubs)==null?void 0:l.logo_url)||null,boost:0,used:!1,_line:u})}const y=it(m.length);m.forEach((k,n)=>{k._col=y[n]}),p[u]=m}return p}function On(e){const t=ct[e]||ct["4-4-2"],o={GK:[],DEF:[],MIL:[],ATT:[]},a=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let p=0;for(const s of["GK","DEF","MIL","ATT"]){const d=[];for(let u=0;u<t[s];u++){const i=3+Math.floor(Math.random()*5);d.push({cardId:"fake-"+p,id:"fake-"+p,firstname:"IA",name:a[p%a.length],country_code:"XX",club_id:null,job:s,job2:null,note_g:s==="GK"?i:2,note_d:s==="DEF"?i:2,note_m:s==="MIL"?i:2,note_a:s==="ATT"?i:2,rarity:"normal",boost:0,used:!1,_line:s}),p++}const l=it(d.length);d.forEach((u,i)=>{u._col=l[i]}),o[s]=d}return o}function Hn(e,t,o){e.innerHTML=`
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${n.map((j,O)=>{const B=O<n.length-1?Qe(j,n[O+1]):null,S=!B||B==="#ff3333"||B==="#cc2222",C=B==="#00ff88"?"+2":B==="#FFD700"?"+1":"";return`
          ...
          ${O<n.length-1?`<div class="duel-link duel-link-${M}" data-idx="${O}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${S?"rgba(255,255,255,0.12)":B};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${S?"none":`0 0 8px ${B}`}">
            ${C?`<span style="position:absolute;top:-13px;left:50%;transform:translateX(-50%);font-size:8px;font-weight:900;color:${B}">${C}</span>`:""}
          </div>`:""}
          `}).join("")}
      </div>
  ...
  <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div id="duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center, rgba(10,61,30,0.4), rgba(10,61,30,0.92))"></div>
          ...
          <button id="start-match-btn" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,0.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">
            ▶ Commencer le match
          </button>`,B.style.transition="opacity .45s ease",B.style.opacity="1",B.style.pointerEvents="auto",(S=document.getElementById("start-match-btn"))==null||S.addEventListener("click",k))},600)},700)},2800)}function Kn(e){if(e.type==="duel"&&(e.homeTotal!=null||e.aiTotal!=null)){const t=(e.homeTotal||0)>=(e.aiTotal||0);return`
    ...
    <div style="display:flex;align-items:center;gap:4px;${t?"flex-direction:row-reverse":""};background:rgba(255,255,255,0.04);border-radius:8px;padding:5px 8px;border:1px solid rgba(255,255,255,0.07)">
      ...
      ${tt(e.inPlayer||{})}
    </div>`}return`<div style="font-size:11px;color:${e.type==="goal"?"#FFD700":"rgba(255,255,255,0.65)"};font-weight:${e.type==="goal"?700:400};padding:3px 2px">${e.text||""}</div>`}function ze(e,t,o){var j,O,B,S,C,ee,ue,re;const a=t.selected.map(D=>D.cardId),p=t.usedSubIds||[],s=t.homeSubs.filter(D=>!p.includes(D.cardId)),l=Object.values(t.homeTeam).flat().filter(D=>D.used).length>0&&s.length>0&&t.subsUsed<t.maxSubs,u=!["GK","DEF","MIL","ATT"].some(D=>(t.aiTeam[D]||[]).some(Y=>!Y.used)),i=[...t.homeTeam.MIL||[],...t.homeTeam.ATT||[]].filter(D=>!D.used),r=t.phase==="attack"&&u&&i.length===0?[...t.homeTeam.GK||[],...t.homeTeam.DEF||[]].filter(D=>!D.used).map(D=>D.cardId):[];t.log[t.log.length-1];const f=t.phase==="ai-attack"||t.phase==="ai-defense",m=t.phase==="attack",y=t.phase==="defense",k=t.phase==="finished",n=t.gcCards.filter(D=>!t.usedGc.includes(D.id)),E=t.boostCard&&!t.boostUsed;e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column",e.innerHTML=`
  <style>
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const Y=t.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
          ...
          </div>`}const D=t.log[t.log.length-1];return D?'<div style="padding:2px 4px">'+Kn(D)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})()}
    </div>
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(D=>{var Y,te,F;if(D.type==="duel"){const Q=D.isGoal,K=D.homeScored?"#FFD700":Q?"#ff6b6b":"rgba(255,255,255,0.3)",ae=D.homeScored?"⚽ BUT !":Q?"⚽ BUT IA !":(Y=D.homePlayers)!=null&&Y.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${Q?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${K};margin-bottom:4px">
                <div style="font-size:9px;color:${K};letter-spacing:1px;margin-bottom:5px;font-weight:700;text-transform:uppercase">${ae}</div>
                ${(te=D.homePlayers)!=null&&te.length?`<div style="margin-bottom:3px">${et(D.homePlayers,"rgba(255,255,255,0.7)",D.homeTotal)}</div>`:""}
                ${(F=D.aiPlayers)!=null&&F.length?`<div style="opacity:0.7">${et(D.aiPlayers,"#ff6b6b",D.aiTotal)}</div>`:""}
                ...
                <div style="display:flex;align-items:center;gap:8px">
                  ${D.outPlayer?Ue({...D.outPlayer,used:!0,_line:D.outPlayer.job,rarity:"normal"},38,50):""}
                  <span style="color:rgba(255,255,255,0.4);font-size:18px">→</span>
                  ${D.inPlayer?Ue({...D.inPlayer,_line:D.inPlayer.job,rarity:"normal"},38,50):""}
                </div>
  ...
  </div>`;function z(){const D=e.querySelector(".match-screen");if(!D)return;const Y=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);D.style.bottom="auto",D.style.height=Y+"px",D.style.minHeight=Y+"px",D.style.maxHeight=Y+"px",D.style.overflow="hidden";const te=e.querySelector("#mobile-action-bar"),F=e.querySelector("#mobile-play-area");te&&F&&(F.style.paddingBottom=te.offsetHeight+"px")}if(z(),setTimeout(z,120),setTimeout(z,400),setTimeout(z,1e3),t._vvBound||(t._vvBound=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",z),window.visualViewport.addEventListener("scroll",z)),window.addEventListener("resize",z)),function(){const Y=e.querySelector(".terrain-wrapper svg");Y&&(Y.removeAttribute("width"),Y.removeAttribute("height"),Y.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",Y.setAttribute("viewBox","-26 -26 352 352"),Y.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),t._resizeBound||(t._resizeBound=!0,window.addEventListener("resize",()=>{const D=e.querySelector(".terrain-wrapper svg");D&&(D.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0")})),t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase==="attack"||t.phase==="defense"){let D=!1,Y=30;const te=()=>document.getElementById("match-timer"),F=()=>{const Q=te();if(!Q)return;const K=String(Math.floor(Y/60)).padStart(2,"0"),ae=String(Y%60).padStart(2,"0");Q.textContent=` ${K}:${ae}`,Q.style.color=D?"#ff2222":"#ff9500",Q.style.fontWeight="900"};F(),t._timerInt=setInterval(()=>{if(Y--,Y<0)if(!D)D=!0,Y=15,F();else{clearInterval(t._timerInt),t._timerInt=null,t.homeScore=0,t.aiScore=3;const Q=document.createElement("div");Q.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;color:#fff;padding:24px;text-align:center",Q.innerHTML='<div style="font-size:56px">⏱️</div><div style="font-size:24px;font-weight:900;color:#ff4444">MATCH PERDU PAR FORFAIT</div><div style="font-size:14px;color:rgba(255,255,255,0.6)">Temps écoulé</div>',document.body.appendChild(Q),setTimeout(()=>{Q.remove(),It(e,t,o)},2500)}else F()},1e3)}(j=document.getElementById("match-quit"))==null||j.addEventListener("click",()=>{Ne(e),confirm("Abandonner ? Résultat : défaite 3-0")&&(t.homeScore=0,t.aiScore=3,It(e,t,o))}),(O=document.getElementById("view-ai"))==null||O.addEventListener("click",()=>io(t,o)),(B=document.getElementById("toggle-history"))==null||B.addEventListener("click",()=>{var D;(D=document.getElementById("match-history-panel"))==null||D.classList.add("open")}),(S=document.getElementById("close-history"))==null||S.addEventListener("click",()=>{var D;(D=document.getElementById("match-history-panel"))==null||D.classList.remove("open")}),(C=document.getElementById("btn-action"))==null||C.addEventListener("click",()=>{t.selected.length!==0&&(m?Yn(e,t,o):y&&Wn(e,t,o))}),(ee=document.getElementById("btn-results"))==null||ee.addEventListener("click",()=>It(e,t,o)),e.querySelectorAll(".match-slot-hit").forEach(D=>{D.addEventListener("click",()=>Vn(D,t,e,o))}),e.querySelectorAll(".match-used-hit").forEach(D=>{D.addEventListener("click",()=>Yt(e,t,o,null,D.dataset.cardId))}),e.querySelectorAll(".gc-mini").forEach(D=>{D.addEventListener("click",()=>Qn(D.dataset.gcId,D.dataset.gcType,e,t,o))}),(ue=document.getElementById("boost-card"))==null||ue.addEventListener("click",()=>to(e,t,o)),e.querySelectorAll(".sub-btn-col").forEach(D=>{D.addEventListener("click",()=>Yt(e,t,o,D.dataset.subId))}),(re=document.getElementById("sub-btn-main"))==null||re.addEventListener("click",()=>Yt(e,t,o))}function Vn(e,t,o,a){const p=e.dataset.cardId,s=e.dataset.role,d=t.selected.findIndex(l=>l.cardId===p);if(d!==-1)t.selected.splice(d,1);else{if(t.selected.length>=3){a.toast("Maximum 3 joueurs","error");return}const l=(t.homeTeam[s]||[]).find(u=>u.cardId===p);l&&t.selected.push({...l,_role:s,_line:s})}ze(o,t,a)}function ui(e,t,o){e.matchId&&A.from("matches").update({last_player_id:o}).eq("id",e.matchId).then(()=>{})}function Yn(e,t,o){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),ui(t,o,o.state.profile.id);const a=!["GK","DEF","MIL","ATT"].some(d=>(t.aiTeam[d]||[]).some(l=>!l.used)),p=t.selected.map(d=>{const l=(t.homeTeam[d._role]||[]).find(i=>i.cardId===d.cardId)||d,u=a&&["GK","DEF"].includes(d._role);return{...l,_line:d._role,...u?{note_a:Math.max(1,Number(l.note_a)||0)}:{}}}),s=Nt(p,t.modifiers.home);t.pendingAttack={...s,players:[...p],side:"home"},t.selected.forEach(d=>{const l=(t.homeTeam[d._role]||[]).find(u=>u.cardId===d.cardId);l&&(l.used=!0)}),t.log.push({text:`⚔️ Vous attaquez : ${s.total} (base ${s.base}${s.links?` +${s.links} liens`:""}) — ${t.selected.map(d=>d.name).join(", ")}`,type:"info"}),t.selected=[],t.modifiers.home={},t.phase="ai-defense",ze(e,t,o),setTimeout(()=>Xn(e,t,o),1200)}function Wn(e,t,o){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),ui(t,o,o.state.profile.id);const a=t.selected.map(l=>({...(t.homeTeam[l._role]||[]).find(i=>i.cardId===l.cardId)||l,_line:l._role})),p=Rt(a,t.modifiers.home);t.selected.forEach(l=>{const u=(t.homeTeam[l._role]||[]).find(i=>i.cardId===l.cardId);u&&(u.used=!0)});const s=Pt(t.pendingAttack.total,p.total,t.modifiers.home),d={type:"duel",title:"Défense",aiPlayers:(t.pendingAttack.players||[]).map(l=>({name:l.name,note:l._line==="MIL"?l.note_m:l.note_a,portrait:Ie(l),job:l.job,country_code:l.country_code,rarity:l.rarity,clubName:l.clubName,clubLogo:l.clubLogo})),homePlayers:t.selected.map(l=>{const u=(t.homeTeam[l._role]||[]).find(i=>i.cardId===l.cardId)||l;return{name:u.name,note:(u._line==="GK"?Number(u.note_g)||0:u._line==="MIL"?Number(u.note_m)||0:Number(u.note_d)||0)+(u.boost||0),portrait:Ie(u),job:u.job,country_code:u.country_code,rarity:u.rarity,clubName:u.clubName,clubLogo:u.clubLogo}}),homeTotal:p.total,aiTotal:t.pendingAttack.total,isGoal:!1,homeScored:!1,text:""};if(s.shielded)d.text="🛡️ Bouclier ! But annulé.",t.log.push(d);else if(s.goal){t.aiScore++,d.isGoal=!0,d.homeScored=!1,d.text=`⚽ BUT IA ! (${t.pendingAttack.total} > ${p.total})`,t.log.push(d),t.selected=[],t.modifiers.home={},t.pendingAttack=null,ze(e,t,o),qt(d.aiPlayers,t.homeScore,t.aiScore,!1,()=>{dt(e,t,o,"home-attack")});return}else d.text=`🧤 Défense réussie ! (${p.total} ≥ ${t.pendingAttack.total})`,t.log.push(d);t.selected=[],t.modifiers.home={},t.pendingAttack=null,dt(e,t,o,"home-attack")}function ti(e,t,o){ui(t,o,null);const a=[...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]],p=Si(a,"attack",t.difficulty);if(!p.length){Ft(e,t,o);return}const s=Nt(p,t.modifiers.ai);t.pendingAttack={...s,players:p,side:"ai"},p.forEach(i=>{i.used=!0}),t.log.push({text:`🤖 IA attaque : ${s.total} (${p.map(i=>i.name).join(", ")})`,type:"info"}),t.modifiers.ai={};const d=[...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(i=>!i.used),u=(t.homeSubs||[]).filter(i=>!(t.usedSubIds||[]).includes(i.cardId)).length>0&&t.subsUsed<t.maxSubs;if(d.length===0&&!u){t.aiScore++;const i={type:"duel",isGoal:!0,homeScored:!1,aiPlayers:p.map(r=>({name:r.name,note:r._line==="MIL"?r.note_m:r.note_a,portrait:Ie(r),job:r.job,country_code:r.country_code,rarity:r.rarity,clubName:r.clubName,clubLogo:r.clubLogo})),aiTotal:s.total,text:"⚽ BUT IA ! (aucun défenseur disponible)"};t.log.push(i),t.pendingAttack=null,ze(e,t,o),qt(i.aiPlayers,t.homeScore,t.aiScore,!1,()=>{dt(e,t,o,"home-attack")});return}t.phase="defense",ze(e,t,o)}function Xn(e,t,o){var i,r;const a=[...t.aiTeam.GK||[],...t.aiTeam.DEF||[],...t.aiTeam.MIL||[]],p=Si(a,"defense",t.difficulty);if(!["GK","DEF","MIL","ATT"].flatMap(f=>(t.aiTeam[f]||[]).filter(m=>!m.used)).length){t.homeScore++;const m={type:"duel",isGoal:!0,homeScored:!0,homePlayers:(((i=t.pendingAttack)==null?void 0:i.players)||[]).map(y=>({name:y.name,note:be(y,y._line||y.job),portrait:Ie(y),job:y.job,country_code:y.country_code,rarity:y.rarity,clubName:y.clubName,clubLogo:y.clubLogo})),homeTotal:((r=t.pendingAttack)==null?void 0:r.total)||0,aiTotal:0,text:"⚽ BUT ! L'IA n'a plus de joueurs — but automatique !"};t.log.push(m),t.modifiers.ai={},t.pendingAttack=null,ze(e,t,o),qt(m.homePlayers,t.homeScore,t.aiScore,!0,()=>{dt(e,t,o,"ai-attack")});return}const d=p.length>0?Rt(p,t.modifiers.ai).total:0;p.forEach(f=>{f.used=!0});const l=Pt(t.pendingAttack.total,d,t.modifiers.ai),u={type:"duel",title:"Attaque",homePlayers:(t.pendingAttack.players||[]).map(f=>({name:f.name,note:f._line==="MIL"?f.note_m:f.note_a,portrait:Ie(f),job:f.job,country_code:f.country_code,rarity:f.rarity,clubName:f.clubName,clubLogo:f.clubLogo})),aiPlayers:p.map(f=>({name:f.name,note:f._line==="GK"?f.note_g:f._line==="MIL"?f.note_m:f.note_d,portrait:Ie(f),job:f.job,country_code:f.country_code,rarity:f.rarity,clubName:f.clubName,clubLogo:f.clubLogo})),homeTotal:t.pendingAttack.total,aiTotal:d,isGoal:!1,homeScored:!1,text:""};if(l.shielded)u.text="🛡️ Bouclier IA !",t.log.push(u);else if(l.goal){t.homeScore++,u.isGoal=!0,u.homeScored=!0,u.text=`⚽ BUT ! (${t.pendingAttack.total} > ${d})`,t.log.push(u),t.modifiers.ai={},t.pendingAttack=null,ze(e,t,o),qt(u.homePlayers,t.homeScore,t.aiScore,!0,()=>{dt(e,t,o,"ai-attack")});return}else u.text=`🧤 IA défend (${d} ≥ ${t.pendingAttack.total})`,t.log.push(u);t.modifiers.ai={},t.pendingAttack=null,dt(e,t,o,"ai-attack")}function dt(e,t,o,a){if(t.round++,Vi(t)){It(e,t,o);return}if(a==="home-attack"){if(![...t.homeTeam.MIL||[],...t.homeTeam.ATT||[]].filter(s=>!s.used).length){if(![...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(l=>!l.used).length){Ft(e,t,o);return}if(![...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]].filter(l=>!l.used).length){Ft(e,t,o);return}setTimeout(()=>ti(e,t,o),100);return}t.phase="attack",ze(e,t,o)}else{if(![...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]].filter(s=>!s.used).length){Ft(e,t,o);return}t.phase="ai-attack",ze(e,t,o),setTimeout(()=>ti(e,t,o),800)}}function Vi(e){const t=["MIL","ATT","GK","DEF"].some(a=>(e.homeTeam[a]||[]).some(p=>!p.used)),o=["MIL","ATT","GK","DEF"].some(a=>(e.aiTeam[a]||[]).some(p=>!p.used));return!t&&!o}function Ft(e,t,o){Vi(t)?It(e,t,o):(t.phase="attack",ze(e,t,o))}function Jn(e,t,o){const a=document.createElement("div");a.style.cssText=`
  ...
  `,document.body.appendChild(a);let r=!1;const f=()=>{r||(r=!0,a.remove(),o())};a.addEventListener("click",f),setTimeout(f,2e3)}function kt(e,t="rgba(0,0,0,0.8)"){const o=document.createElement("div");o.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,o.textContent=e,document.body.appendChild(o),setTimeout(()=>o.remove(),2200)}function Yt(e,t,o,a=null,p=null){var y,k;if(t.phase!=="attack"){kt("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){kt(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const s=Object.entries(t.homeTeam).flatMap(([n,E])=>(E||[]).filter(z=>z.used).map(z=>({...z,_line:z._line||n}))),d=t.homeSubs.filter(n=>!t.usedSubIds.includes(n.cardId));if(!s.length){kt("Aucun joueur utilisé à remplacer");return}if(!d.length){kt("Aucun remplaçant disponible");return}let l=Math.max(0,s.findIndex(n=>n.cardId===p));const u=((y=s[l])==null?void 0:y._line)||((k=s[l])==null?void 0:k.job);let i=a?Math.max(0,d.findIndex(n=>n.cardId===a)):Math.max(0,d.findIndex(n=>n.job===u)),r=!1;const f=document.createElement("div");f.id="sub-overlay",f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function m(){var B,S,C,ee,ue,re;const n=s[l],E=d[i],z=Math.min(130,Math.round((window.innerWidth-90)/2)),M=Math.round(z*1.35),j=D=>`background:rgba(255,255,255,0.12);border:none;color:${D?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${D?"default":"pointer"};flex-shrink:0`;f.innerHTML=`
    ...
    </div>`,(B=f.querySelector("#sub-close"))==null||B.addEventListener("click",()=>f.remove()),(S=f.querySelector("#out-up"))==null||S.addEventListener("click",()=>{l>0&&(l--,m())}),(C=f.querySelector("#out-down"))==null||C.addEventListener("click",()=>{l<s.length-1&&(l++,m())}),(ee=f.querySelector("#in-up"))==null||ee.addEventListener("click",()=>{i>0&&(i--,m())}),(ue=f.querySelector("#in-down"))==null||ue.addEventListener("click",()=>{i<d.length-1&&(i++,m())});const O=(D,Y,te,F)=>{const Q=f.querySelector("#"+D);if(!Q)return;let K=0;Q.addEventListener("touchstart",ae=>{K=ae.touches[0].clientY},{passive:!0}),Q.addEventListener("touchend",ae=>{const ge=ae.changedTouches[0].clientY-K;if(Math.abs(ge)<30)return;const me=Y();ge<0&&me<F-1?(te(me+1),m()):ge>0&&me>0&&(te(me-1),m())},{passive:!0})};O("in-panel",()=>i,D=>i=D,d.length),O("out-panel",()=>l,D=>l=D,s.length),(re=f.querySelector("#sub-confirm"))==null||re.addEventListener("click",D=>{if(D.preventDefault(),D.stopPropagation(),r)return;r=!0;const Y=s[l],te=d[i];if(!Y||!te)return;let F=null,Q=-1;for(const[ae,ge]of Object.entries(t.homeTeam)){const me=(ge||[]).findIndex(xe=>xe.cardId===Y.cardId);if(me!==-1){F=ae,Q=me;break}}if(Q===-1||!F){kt("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),f.remove();return}const K={...te,_line:F,_col:Y._col||0,used:!1,boost:0};t.homeTeam[F].splice(Q,1,K),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(te.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:Y.name,firstname:Y.firstname,note:be(Y,F),portrait:Ie(Y),job:Y.job,country_code:Y.country_code,rarity:Y.rarity,clubName:Y.clubName,clubLogo:Y.clubLogo},inPlayer:{name:te.name,firstname:te.firstname,note:be(te,F),portrait:Ie(te),job:te.job,country_code:te.country_code,rarity:te.rarity,clubName:te.clubName,clubLogo:te.clubLogo},text:`🔄 ${te.firstname} ${te.name} remplace ${Y.firstname} ${Y.name}`}),f.remove(),Jn(Y,te,()=>ze(e,t,o))})}document.body.appendChild(f),m()}function Qn(e,t,o,a,p){var k,n;const s=(a.gcDefs||[]).find(E=>E.name===t),d=Be[t]||{icon:"⚡",desc:"Carte spéciale."},l={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[s==null?void 0:s.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",u={purple:"#b06ce0",light_blue:"#00d4ef"}[s==null?void 0:s.color]||"#b06ce0",i=(s==null?void 0:s.name)||t,r=(s==null?void 0:s.effect)||d.desc,f=s!=null&&s.image_url?`/manager-wars/icons/${s.image_url}`:null,m=d.icon||"⚡",y=document.createElement("div");y.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",y.innerHTML=`
    ...
    </div>`,document.body.appendChild(y),(k=y.querySelector("#gc-back"))==null||k.addEventListener("click",()=>y.remove()),(n=y.querySelector("#gc-use"))==null||n.addEventListener("click",()=>{y.remove(),eo(e,t,o,a,p)})}function jt(e,t,o,a,p,s){const d=document.createElement("div");d.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let l=[];function u(){var i,r;d.innerHTML=`
    ...
    <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
      ${e.map(f=>{const m=f._line||f.job||"MIL",y={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[m]||"#555",k=be(f,m)+(f.boost||0),n=l.find(E=>E.cardId===f.cardId);return`<div class="gc-pick-item" data-cid="${f.cardId}"
    ...
    </div>`,(i=d.querySelector("#gc-picker-close"))==null||i.addEventListener("click",()=>d.remove()),d.querySelectorAll(".gc-pick-item").forEach(f=>{f.addEventListener("click",()=>{const m=f.dataset.cid,y=e.find(n=>n.cardId===m);if(!y)return;const k=l.findIndex(n=>n.cardId===m);k>-1?l.splice(k,1):l.length<t&&l.push(y),u()})}),(r=d.querySelector("#gc-picker-confirm"))==null||r.addEventListener("click",()=>{d.remove(),s(l)})}u(),document.body.appendChild(d)}const Zn={BOOST_STAT:({value:e=1,count:t=1,roles:o=[]},a,p,s)=>{const d=Object.entries(a.homeTeam).filter(([l])=>!o.length||o.includes(l)).flatMap(([l,u])=>u.filter(i=>!i.used).map(i=>({...i,_line:l})));return d.length?(jt(d,t,`Choisir ${t} joueur(s) à booster (+${e})`,p,a,l=>{l.forEach(u=>{const i=(a.homeTeam[u._line]||[]).find(r=>r.cardId===u.cardId);i&&(i.boost=(i.boost||0)+e,a.log.push({text:`⚡ +${e} sur ${i.name}`,type:"info"}))}),ze(p,a,s)}),!0):(a.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),ze(p,a,s),!0)},DEBUFF_STAT:({value:e=1,count:t=1,roles:o=[],target:a="ai"},p,s,d)=>{const l=a==="home"?p.homeTeam:p.aiTeam,u=a==="ai"?"adverse":"allié",i=Object.entries(l).filter(([r])=>!o.length||o.includes(r)).flatMap(([r,f])=>f.filter(m=>!m.used).map(m=>({...m,_line:r})));return i.length?(jt(i,t,`Choisir ${t} joueur(s) ${u}(s) à débuffer (-${e})`,s,p,r=>{r.forEach(f=>{const y=((a==="home"?p.homeTeam:p.aiTeam)[f._line]||[]).find(k=>k.cardId===f.cardId);y&&(y.boost=(y.boost||0)-e,p.log.push({text:`🎯 -${e} sur ${y.name}${a==="ai"?" (IA)":""}`,type:"info"}))}),ze(s,p,d)}),!0):(p.log.push({text:`🎯 Aucun joueur ${u} disponible`,type:"info"}),ze(s,p,d),!0)},GRAY_PLAYER:({count:e=1,roles:t=[],target:o="ai"},a,p,s)=>{const d=o==="home"?a.homeTeam:a.aiTeam,l=o==="ai"?"adverse":"allié",u=Object.entries(d).filter(([i])=>!t.length||t.includes(i)).flatMap(([i,r])=>r.filter(f=>!f.used).map(f=>({...f,_line:i})));return u.length?(jt(u,e,`Choisir ${e} joueur(s) ${l}(s) à exclure`,p,a,i=>{i.forEach(r=>{const m=((o==="home"?a.homeTeam:a.aiTeam)[r._line]||[]).find(y=>y.cardId===r.cardId);m&&(m.used=!0,a.log.push({text:`❌ ${m.name}${o==="ai"?" (IA)":""} exclu !`,type:"info"}))}),ze(p,a,s)}),!0):(a.log.push({text:`❌ Aucun joueur ${l} à exclure`,type:"info"}),ze(p,a,s),!0)},REVIVE_PLAYER:({count:e=1,roles:t=[]},o,a,p)=>{const s=Object.entries(o.homeTeam).filter(([d])=>!t.length||t.includes(d)).flatMap(([d,l])=>l.filter(u=>u.used).map(u=>({...u,_line:d})));return s.length?(jt(s,e,`Choisir ${e} joueur(s) à ressusciter`,a,o,d=>{d.forEach(l=>{const u=(o.homeTeam[l._line]||[]).find(i=>i.cardId===l.cardId);u&&(u.used=!1,o.log.push({text:`💫 ${u.name} ressuscité !`,type:"info"}))}),ze(a,o,p)}),!0):(o.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),ze(a,o,p),!0)},REMOVE_GOAL:({},e)=>(e.aiScore>0?(e.aiScore--,e.log.push({text:"🚫 Dernier but IA annulé !",type:"info"})):e.log.push({text:"🚫 Aucun but à annuler",type:"info"}),!1),ADD_GOAL_DRAW:({},e)=>(e.homeScore===e.aiScore?(e.homeScore++,e.log.push({text:"🎯 But bonus (match nul) !",type:"info"})):e.log.push({text:"🎯 But bonus : non applicable (pas de match nul)",type:"info"}),!1),ADD_SUB:({value:e=1},t)=>(t.maxSubs=(t.maxSubs||3)+e,t.log.push({text:`🔄 +${e} remplacement(s) débloqué(s)`,type:"info"}),!1),CUSTOM:()=>!1};function eo(e,t,o,a,p){a.usedGc.push(e);const s=a.gcDefs||[],d=s.find(u=>u.name===t)||s.find(u=>{var i;return((i=u.name)==null?void 0:i.toLowerCase().trim())===(t==null?void 0:t.toLowerCase().trim())});let l=!1;if(d!=null&&d.effect_type&&d.effect_type!=="CUSTOM"){const u=Zn[d.effect_type];u?u(d.effect_params||{},a,o,p)||(l=!0):(p.toast(`Effet "${d.effect_type}" non implémenté`,"error"),l=!0)}else{switch(t){case"Double attaque":a.modifiers.home.doubleAttack=!0,a.log.push({text:"⚡ Double attaque activée !",type:"info"});break;case"Bouclier":a.modifiers.home.shield=!0,a.log.push({text:"🛡️ Bouclier activé !",type:"info"});break;case"Ressusciter":{const u=Object.entries(a.homeTeam).flatMap(([i,r])=>(r||[]).filter(f=>f.used).map(f=>({...f,_line:i})));u.length?(u[0].used=!1,a.log.push({text:`💫 ${u[0].name} ressuscité !`,type:"info"})):a.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"});break}case"Vol de note":a.modifiers.ai.stolenNote=(a.modifiers.ai.stolenNote||0)+1,a.log.push({text:"🎯 -1 à la prochaine attaque IA",type:"info"});break;case"Gel":{const u=[...a.aiTeam.ATT||[],...a.aiTeam.MIL||[]].filter(i=>!i.used);if(u.length){const i=u.sort((r,f)=>be(f,"ATT")-be(r,"ATT"))[0];i.used=!0,a.log.push({text:`❄️ ${i.name} (IA) gelé !`,type:"info"})}break}case"Remplacement+":a.maxSubs++,a.log.push({text:"🔄 +1 remplacement débloqué",type:"info"});break}l=!0}A.from("cards").delete().eq("id",e).then(()=>{}),l&&ze(o,a,p)}function to(e,t,o){const a=Object.values(t.homeTeam).flat().filter(p=>!p.used);if(!a.length){o.toast("Aucun joueur actif à booster","error");return}o.openModal("⚡ Utiliser le Boost",`<div style="margin-bottom:12px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border-radius:10px;padding:12px;text-align:center;color:#000">
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(s);let l=!1;const u=()=>{l||(l=!0,s.remove(),setTimeout(()=>p(),50))};s.addEventListener("click",u),setTimeout(u,3500)}async function It(e,t,o){var r,f;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:a}=o,p=t.homeScore>t.aiScore,s=t.homeScore===t.aiScore,d=p?"victoire":s?"nul":"defaite",l=cn(t.mode,d);t.matchId&&await A.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:p?a.profile.id:null,home_credits_reward:l,played_at:new Date().toISOString()}).eq("id",t.matchId);const u={credits:(a.profile.credits||0)+l,matches_played:(a.profile.matches_played||0)+1};p?u.wins=(a.profile.wins||0)+1:s?u.draws=(a.profile.draws||0)+1:u.losses=(a.profile.losses||0)+1,await A.from("users").update(u).eq("id",a.profile.id),await o.refreshProfile();const i=document.createElement("div");i.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",i.innerHTML=`
    ...
    </div>`,`<button class="btn btn-primary" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}async function no(e,t){await pi(e,t,"random",({deckId:o,formation:a,starters:p,subsRaw:s,gcCardsEnriched:d,gcDefs:l})=>{const u=i=>oo(e,t,o,a,p,s,i,l||[]);if(!d.length){u([]);return}ci(e,d,u)})}async function oo(e,t,o,a,p,s,d=[],l=[]){var M;const{state:u,navigate:i,toast:r}=t;let f=!1,m=null;We(e),e.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const y=async(j=!0)=>{f=!0,m&&(A.removeChannel(m),m=null),j&&await A.rpc("cancel_matchmaking",{p_user_id:u.profile.id}).catch(()=>{})};(M=document.getElementById("mm-cancel"))==null||M.addEventListener("click",async()=>{try{await y(!0)}catch{}Ne(e),i("home")});const k=async(j,O,B)=>{if(f)return;f=!0,m&&(A.removeChannel(m),m=null);const S=document.getElementById("mm-status");S&&(S.textContent="Adversaire trouvé !"),await new Promise(C=>setTimeout(C,600)),e.isConnected&&ro(e,t,j,B,d,l)},{data:n,error:E}=await A.rpc("try_matchmake",{p_user_id:u.profile.id,p_deck_id:o});if(E||!(n!=null&&n.success)){r("Erreur de matchmaking","error"),Ne(e),i("home");return}if(n.matched){k(n.match_id,n.opponent_id,!1);return}const z=document.getElementById("mm-status");z&&(z.textContent="En attente d'un autre joueur..."),m=A.channel("matchmaking-"+u.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${u.profile.id}`},j=>{const O=j.new;O.status==="matched"&&O.match_id&&k(O.match_id,O.matched_with,!0)}).subscribe()}async function ro(e,t,o,a,p=[],s=[]){const{state:d,navigate:l,toast:u}=t,i=a?"p1":"p2",r=a?"p2":"p1",f=(p||[]).map(c=>c.id),m=(p||[]).map(c=>({id:c.id,gc_type:c.gc_type,_gcDef:c._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:y}=await A.from("matches").select("*").eq("id",o).single();if(!y){u("Match introuvable","error"),l("home");return}async function k(){const[{data:c},{data:g},{data:b},{data:h}]=await Promise.all([A.rpc("get_deck_for_match",{p_deck_id:y.home_deck_id}),A.rpc("get_deck_for_match",{p_deck_id:y.away_deck_id}),A.from("users").select("id,pseudo,club_name").eq("id",y.home_id).single(),A.from("users").select("id,pseudo,club_name").eq("id",y.away_id).single()]),x=$=>({cardId:$.card_id,position:$.position,id:$.id,firstname:$.firstname,name:$.surname_encoded,country_code:$.country_code,club_id:$.club_id,job:$.job,job2:$.job2,note_g:Number($.note_g)||0,note_d:Number($.note_d)||0,note_m:Number($.note_m)||0,note_a:Number($.note_a)||0,rarity:$.rarity,skin:$.skin,hair:$.hair,hair_length:$.hair_length,clubName:$.club_encoded_name||null,clubLogo:$.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}),v=((c==null?void 0:c.starters)||[]).map($=>x($)),w=((g==null?void 0:g.starters)||[]).map($=>x($)),_=(c==null?void 0:c.formation)||"4-4-2",I=(g==null?void 0:g.formation)||"4-4-2";return{p1Team:ut(v,_),p2Team:ut(w,I),p1Subs:((c==null?void 0:c.subs)||[]).map($=>x($)),p2Subs:((g==null?void 0:g.subs)||[]).map($=>x($)),p1Formation:_,p2Formation:I,p1Name:(b==null?void 0:b.club_name)||(b==null?void 0:b.pseudo)||"Joueur 1",p2Name:(h==null?void 0:h.club_name)||(h==null?void 0:h.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:a?f:[],gc_p2:a?[]:f,gcCardsFull_p1:a?m:[],gcCardsFull_p2:a?[]:m,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:s||[],lastActionAt:new Date().toISOString()}}let n=y.game_state&&Object.keys(y.game_state).length?y.game_state:null;if(!n)if(a){n=await k();const{data:c}=await A.from("matches").select("game_state").eq("id",o).single();!(c!=null&&c.game_state)||!Object.keys(c.game_state).length?await A.from("matches").update({game_state:n,turn_user_id:y.home_id}).eq("id",o):n=c.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let c=0;c<30&&!n;c++){await new Promise(b=>setTimeout(b,400));const{data:g}=await A.from("matches").select("game_state").eq("id",o).single();g!=null&&g.game_state&&Object.keys(g.game_state).length&&(n=g.game_state)}if(!n){u("Erreur de synchronisation","error"),l("home");return}n.gc_p2=f,n.gcCardsFull_p2=m,await A.from("matches").update({game_state:n}).eq("id",o)}let E=!1,z=null,M=!1;const j=new Set,O=new Set;function B(c){var $,L;try{A.removeChannel(S)}catch{}const g=n[i+"Score"]||0,b=n[r+"Score"]||0;let h,x;c.winner_id?(h=c.winner_id===d.profile.id,x=!1):c.forfeit?(h=g>b,x=!1):(x=g===b,h=g>b),($=document.getElementById("pvp-end-overlay"))==null||$.remove();const v=document.createElement("div");v.id="pvp-end-overlay",v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const w=x?"🤝":h?"🏆":"😞",_=x?"MATCH NUL":h?"VICTOIRE !":"DÉFAITE",I=x?"#fff":h?"#FFD700":"#ff6b6b";v.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(v),(L=v.querySelector("#pvp-end-home"))==null||L.addEventListener("click",()=>{v.remove(),Ne(e),l("home")})}const S=A.channel("pvp-match-"+o).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${o}`},c=>{const g=c.new;try{if(g.status==="finished"||g.forfeit){if(E)return;E=!0,z&&(clearInterval(z),z=null),g.game_state&&(n=g.game_state),B(g);return}if(g.game_state){const b=n;n=g.game_state;const h=n._lastGC;if(h&&h.seq&&!O.has(h.seq)&&(O.add(h.seq),h.by!==i)){me(h.type,h.by,()=>F());return}const x=b[i+"Score"]||0,v=b[r+"Score"]||0,w=n[i+"Score"]||0,_=n[r+"Score"]||0,I=w>x,$=_>v;if((I||$)&&!j.has(n.round)){j.add(n.round);const L=[...n.log||[]].reverse().find(U=>U.isGoal),q=((L==null?void 0:L.homePlayers)||[]).map(U=>({name:U.name,note:U.note,portrait:U.portrait,job:U.job}));ge(q,w,_,I,()=>F());return}F()}}catch(b){console.error("[PvP] crash:",b)}}).subscribe();async function C(c){Object.assign(n,c),n.lastActionAt=new Date().toISOString();const{error:g}=await A.from("matches").update({game_state:n}).eq("id",o);g&&u("Erreur de synchronisation","error");try{F()}catch(b){console.error("[PvP] renderPvpScreen crash:",b)}}async function ee(){if(E)return;E=!0,z&&(clearInterval(z),z=null);const c=a?y.away_id:y.home_id,g=a?"p2":"p1",b=a?"p1":"p2",h={...n,[g+"Score"]:3,[b+"Score"]:0,phase:"finished"};try{await A.from("matches").update({status:"finished",forfeit:!0,winner_id:c,home_score:h.p1Score||0,away_score:h.p2Score||0,game_state:h}).eq("id",o)}catch{}try{A.removeChannel(S)}catch{}setTimeout(()=>{Ne(e),l("home")},800)}const ue={BOOST_STAT:({value:c=1,count:g=1,roles:b=[]},h,x)=>{const v=h[i+"Team"],w=Object.entries(v).filter(([_])=>!b.length||b.includes(_)).flatMap(([_,I])=>I.filter($=>!$.used).map($=>({...$,_line:_})));if(!w.length){h.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),x(h);return}re(w,g,`Choisir ${g} joueur(s) à booster (+${c})`,_=>{_.forEach(I=>{const $=(v[I._line]||[]).find(L=>L.cardId===I.cardId);$&&($.boost=($.boost||0)+c,h.log.push({text:`⚡ +${c} sur ${$.name}`,type:"info"}))}),h[i+"Team"]=v,x(h)})},DEBUFF_STAT:({value:c=1,count:g=1,roles:b=[],target:h="ai"},x,v)=>{const w=h==="home"?i:r,_=x[w+"Team"],I=h==="home"?"allié":"adverse",$=Object.entries(_).filter(([L])=>!b.length||b.includes(L)).flatMap(([L,q])=>q.filter(U=>!U.used).map(U=>({...U,_line:L})));if(!$.length){x.log.push({text:`🎯 Aucun joueur ${I}`,type:"info"}),v(x);return}re($,g,`Choisir ${g} joueur(s) ${I}(s) (-${c})`,L=>{L.forEach(q=>{const U=(_[q._line]||[]).find(oe=>oe.cardId===q.cardId);U&&(U.boost=(U.boost||0)-c,x.log.push({text:`🎯 -${c} sur ${U.name}`,type:"info"}))}),x[w+"Team"]=_,v(x)})},GRAY_PLAYER:({count:c=1,roles:g=[],target:b="ai"},h,x)=>{const v=b==="home"?i:r,w=h[v+"Team"],_=b==="home"?"allié":"adverse",I=Object.entries(w).filter(([$])=>!g.length||g.includes($)).flatMap(([$,L])=>L.filter(q=>!q.used).map(q=>({...q,_line:$})));if(!I.length){h.log.push({text:`❌ Aucun joueur ${_}`,type:"info"}),x(h);return}re(I,c,`Choisir ${c} joueur(s) ${_}(s) à exclure`,$=>{$.forEach(L=>{const q=(w[L._line]||[]).find(U=>U.cardId===L.cardId);q&&(q.used=!0,h.log.push({text:`❌ ${q.name} exclu !`,type:"info"}))}),h[v+"Team"]=w,x(h)})},REVIVE_PLAYER:({count:c=1,roles:g=[]},b,h)=>{const x=b[i+"Team"],v=Object.entries(x).filter(([w])=>!g.length||g.includes(w)).flatMap(([w,_])=>_.filter(I=>I.used).map(I=>({...I,_line:w})));if(!v.length){b.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),h(b);return}re(v,c,`Choisir ${c} joueur(s) à ressusciter`,w=>{w.forEach(_=>{const I=(x[_._line]||[]).find($=>$.cardId===_.cardId);I&&(I.used=!1,b.log.push({text:`💫 ${I.name} ressuscité !`,type:"info"}))}),b[i+"Team"]=x,h(b)})},REMOVE_GOAL:({},c,g)=>{const b=r+"Score";c[b]>0?(c[b]--,c.log.push({text:"🚫 Dernier but annulé !",type:"info"})):c.log.push({text:"🚫 Aucun but à annuler",type:"info"}),g(c)},ADD_GOAL_DRAW:({},c,g)=>{c[i+"Score"]===c[r+"Score"]?(c[i+"Score"]++,c.log.push({text:"🎯 But bonus !",type:"info"})):c.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),g(c)},ADD_SUB:({value:c=1},g,b)=>{g.maxSubs=(g.maxSubs||3)+c,g.log.push({text:`🔄 +${c} remplacement(s)`,type:"info"}),b(g)},CUSTOM:({},c,g)=>g(c)};function re(c,g,b,h){const x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let v=[];function w(){var I,$;const _=c.map(L=>{const q={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[L._line]||"#555",U=be(L,L._line)+(L.boost||0),le=v.find(H=>H.cardId===L.cardId)?"#FFD700":"rgba(255,255,255,0.25)",G=L.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${L.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${le};background:${q};overflow:hidden;cursor:pointer;${G}">
        ...
        </div>`,(I=x.querySelector("#pp-close"))==null||I.addEventListener("click",()=>x.remove()),x.querySelectorAll(".pp-item").forEach(L=>{L.addEventListener("click",()=>{const q=L.dataset.cid,U=c.find(le=>le.cardId===q),oe=v.findIndex(le=>le.cardId===q);U&&(oe>-1?v.splice(oe,1):v.length<g&&v.push(U),w())})}),($=x.querySelector("#pp-confirm"))==null||$.addEventListener("click",()=>{x.remove(),h(v)})}w(),document.body.appendChild(x)}async function D(c,g){const h=(n["gcCardsFull_"+i]||[]).find(_=>_.id===c),x=(h==null?void 0:h._gcDef)||(n.gcDefs||[]).find(_=>{var I;return _.name===g||((I=_.name)==null?void 0:I.toLowerCase().trim())===(g==null?void 0:g.toLowerCase().trim())}),v=[...n["usedGc_"+i]||[],c],w={type:g,by:i,seq:(n._gcAnimSeq||0)+1};O.add(w.seq),me(g,i,async()=>{if(x!=null&&x.effect_type&&x.effect_type!=="CUSTOM"){const I=ue[x.effect_type];if(I){const $={...n};I(x.effect_params||{},$,async L=>{L["usedGc_"+i]=v,L._lastGC=w,L._gcAnimSeq=w.seq,await C(L)});return}}const _={...n};switch(g){case"Remplacement+":_.maxSubs=(_.maxSubs||3)+1,_.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const I=r+"Score";_[I]>0&&(_[I]--,_.log.push({text:"🚫 But annulé",type:"info"}));break}}_["usedGc_"+i]=v,_._lastGC=w,_._gcAnimSeq=w.seq,await C(_)})}function Y(c,g){const b="usedCardIds_"+c,h=new Set(n[b]||[]);g.forEach(x=>h.add(x)),n[b]=[...h]}function te(){for(const c of["p1","p2"]){const g=new Set(n["usedCardIds_"+c]||[]),b=n[c+"Team"];if(b)for(const h of["GK","DEF","MIL","ATT"])(b[h]||[]).forEach(x=>{x.used=g.has(x.cardId)})}}function F(){var Oe,ft,gt,mt,xt,yt;if(n.phase==="reveal")return Q();if(n.phase==="midfield")return ae();if(n.phase==="finished")return Ke();const c=n[i+"Team"],g=n[r+"Team"];te();const b=n[i+"Score"],h=n[r+"Score"],x=n[i+"Name"],v=n[r+"Name"],w=n.phase===i+"-attack",_=n.phase===i+"-defense",I=Array.isArray(n["selected_"+i])?n["selected_"+i]:[],$=I.map(T=>T.cardId),L=window.innerWidth>=700,q=n[i+"Subs"]||[],U=n["usedSubIds_"+i]||[],oe=q.filter(T=>!U.includes(T.cardId)),le=n["gcCardsFull_"+i]||[],G=n["usedGc_"+i]||[],H=le.filter(T=>!G.includes(T.id)),J=n.boostOwner===i&&!n.boostUsed,ne=!["GK","DEF","MIL","ATT"].some(T=>(g[T]||[]).some(V=>!V.used)),se=[...c.MIL||[],...c.ATT||[]].filter(T=>!T.used),fe=w&&ne&&se.length===0?[...c.GK||[],...c.DEF||[]].filter(T=>!T.used).map(T=>T.cardId):[];function de(T,V,ce){var wt,$t;const Z=T._gcDef,He={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[Z==null?void 0:Z.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",Ye={purple:"#b06ce0",light_blue:"#00d4ef"}[Z==null?void 0:Z.color]||"#b06ce0",Xe=(Z==null?void 0:Z.name)||T.gc_type,Je=(Z==null?void 0:Z.effect)||((wt=Be[T.gc_type])==null?void 0:wt.desc)||"",ht=Z!=null&&Z.image_url?`/manager-wars/icons/${Z.image_url}`:null,Ut=(($t=Be[T.gc_type])==null?void 0:$t.icon)||"⚡",bt=Math.round(ce*.22),vt=Math.round(ce*.22),nt=ce-bt-vt,Kt=Xe.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${T.id}" data-gc-type="${T.gc_type}"
      ...
      </div>`;function Te(){const T=e.querySelector(".match-screen");if(!T)return;const V=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);T.style.height=V+"px",T.style.minHeight=V+"px",T.style.maxHeight=V+"px",T.style.overflow="hidden";const ce=e.querySelector("#mobile-action-bar"),Z=e.querySelector("#mobile-play-area");ce&&Z&&(Z.style.paddingBottom=ce.offsetHeight+"px")}if(Te(),setTimeout(Te,120),setTimeout(Te,400),M||(M=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",Te),window.visualViewport.addEventListener("scroll",Te)),window.addEventListener("resize",Te)),function(){const V=e.querySelector(".terrain-wrapper svg");V&&(V.removeAttribute("width"),V.removeAttribute("height"),V.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",V.setAttribute("viewBox","-26 -26 352 352"),V.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),e.querySelectorAll(".match-slot-hit").forEach(T=>{T.addEventListener("click",()=>{if(!w&&!_)return;const V=T.dataset.cardId,ce=T.dataset.role,Z=(c[ce]||[]).find(Je=>Je.cardId===V);if(!Z||Z.used)return;const He=fe.includes(V);if(w&&!["MIL","ATT"].includes(ce)&&!He)return;Array.isArray(n["selected_"+i])||(n["selected_"+i]=[]);const Ye=n["selected_"+i],Xe=Ye.findIndex(Je=>Je.cardId===V);Xe>-1?Ye.splice(Xe,1):Ye.length<3&&Ye.push({...Z,_role:ce}),F()})}),e.querySelectorAll(".match-used-hit").forEach(T=>{T.addEventListener("click",()=>N(T.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(T=>{T.addEventListener("click",()=>N())}),(Oe=e.querySelector("#pvp-sub-open"))==null||Oe.addEventListener("click",()=>N()),e.querySelectorAll(".pvp-gc-mini").forEach(T=>{T.addEventListener("click",()=>xe(T.dataset.gcId,T.dataset.gcType))}),(ft=e.querySelector("#pvp-boost-card"))==null||ft.addEventListener("click",()=>R()),(gt=e.querySelector("#pvp-action"))==null||gt.addEventListener("click",T=>{w?T.currentTarget.dataset.pass==="1"||!we(i)?pe():ie():_&&Ae()}),(mt=e.querySelector("#pvp-quit"))==null||mt.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&ee()}),(xt=e.querySelector("#pvp-view-opp"))==null||xt.addEventListener("click",()=>P()),(yt=e.querySelector("#pvp-toggle-history"))==null||yt.addEventListener("click",()=>X()),z&&(clearInterval(z),z=null),(w||_)&&!E){let T=30,V=!1;const ce=()=>document.getElementById("pvp-timer"),Z=()=>{ce()&&(ce().textContent=T+"s",ce().style.color=V?"#ff4444":"#fff")};Z(),z=setInterval(()=>{T--,T<0?V?(clearInterval(z),z=null,w&&!we(i)?pe():ee()):(V=!0,T=15,Z()):Z()},1e3)}}function Q(){We(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
      ...
      <div style="width:min(90vw,420px)">${at(n[r+"Team"],n[r+"Formation"],null,[],300,300)}</div>
    </div>`,i==="p1"&&setTimeout(async()=>{await C({phase:"midfield"})},5e3)}let K=!1;function ae(){if(K)return;const c=n[i+"Team"].MIL||[],g=n[r+"Team"].MIL||[];function b(H){return H.reduce((J,ne)=>J+be(ne,"MIL"),0)}function h(H){let J=0;for(let ne=0;ne<H.length-1;ne++){const se=Qe(H[ne],H[ne+1]);se==="#00ff88"?J+=2:se==="#FFD700"&&(J+=1)}return J}const x=b(c)+h(c),v=b(g)+h(g),w=x>=v;function _(H,J,ne){return`<div id="duel-row-${ne}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
        ...
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${H.map((se,fe)=>{const de=fe<H.length-1?Qe(se,H[fe+1]):null,ye=!de||de==="#ff3333"||de==="#cc2222",Ee=de==="#00ff88"?"+2":de==="#FFD700"?"+1":"";return`<div class="duel-card duel-card-${ne}" data-idx="${fe}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease,transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
              ${Ue({...se,note:be(se,"MIL"),_line:"MIL"},58,78)}
            </div>
            ${fe<H.length-1?`<div class="duel-link duel-link-${ne}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${ye?"rgba(255,255,255,0.12)":de};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${ye?"none":`0 0 8px ${de}`}">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    </div>`;const I=(H,J)=>e.querySelectorAll(H).forEach((ne,se)=>{setTimeout(()=>{ne.style.opacity="1",ne.style.transform="translateY(0) scale(1)"},J+se*90)});I(".duel-card-me",150),I(".duel-card-opp",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((H,J)=>setTimeout(()=>{H.style.opacity="1"},J*70)),900),setTimeout(()=>{const H=e.querySelector("#pvp-vs");H&&(H.style.opacity="1",H.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(J=>J.style.opacity="1")},1250);function $(H,J,ne){const se=document.getElementById(H);if(!se)return;const fe=performance.now(),de=ye=>{const Ee=Math.min(1,(ye-fe)/ne);se.textContent=Math.round(J*(1-Math.pow(1-Ee,3))),Ee<1?requestAnimationFrame(de):se.textContent=J};requestAnimationFrame(de)}setTimeout(()=>{$("pvp-score-me",x,800),$("pvp-score-opp",v,800)},1500);const L=n.p1Team.MIL||[],q=n.p2Team.MIL||[],U=b(L)+h(L),oe=b(q)+h(q),le=U>=oe?"p1":"p2";let G=n.boostValue;G==null&&(G=li(),n.boostValue=G,n.boostOwner=le,n.boostUsed=!1),K=!0,setTimeout(()=>{const H=e.querySelector("#duel-row-"+(w?"me":"opp")),J=e.querySelector("#duel-row-"+(w?"opp":"me")),ne=document.getElementById("pvp-score-me"),se=document.getElementById("pvp-score-opp"),fe=w?ne:se,de=w?se:ne;fe&&(fe.style.fontSize="80px",fe.style.color=w?"#FFD700":"#ff6b6b",fe.style.animation="duelPulse .5s ease"+(w?",duelGlow 1.5s ease infinite .5s":"")),de&&(de.style.opacity="0.25"),setTimeout(()=>{H&&(H.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",H.style.zIndex="5"),setTimeout(()=>{const ye=document.getElementById("duel-shock");ye&&(ye.style.animation="shockwave .5s ease-out forwards"),J&&(J.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var Ce;const ye=document.getElementById("pvp-duel-finale");if(!ye)return;const Ee=n.boostOwner===i?'<div style="background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:3px solid #cdeffd;border-radius:18px;padding:20px 34px;text-align:center;animation:boostFlipIn .7s cubic-bezier(.34,1.56,.64,1) both;box-shadow:0 10px 36px rgba(135,206,235,.5)"><div style="font-size:10px;color:rgba(0,0,0,.6);letter-spacing:2px;text-transform:uppercase;margin-bottom:6px;font-weight:700">Carte Boost obtenue</div><div style="font-size:46px;line-height:1">⚡</div><div style="font-size:50px;font-weight:900;color:#063;line-height:1.1">+'+G+`</div><div style="font-size:10px;color:rgba(0,0,0,.55);margin-top:4px">Applicable sur n'importe quel joueur</div></div>`:"",Re=i==="p1"?'<button id="pvp-start-match" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">▶ Commencer le match</button>':`<div style="font-size:14px;color:rgba(255,255,255,0.5);text-align:center;margin-top:8px;animation:fadeUp .4s ease both">⏳ En attente de l'adversaire...</div>`;ye.innerHTML='<div style="font-size:22px;font-weight:900;color:#fff;text-align:center;animation:fadeUp .4s ease both;text-shadow:0 2px 12px rgba(0,0,0,.5)">'+(w?"⚽ "+n[i+"Name"]+"<br>gagne le milieu et attaque !":"😔 "+n[r+"Name"]+"<br>gagne l'engagement et attaque !")+"</div>"+Ee+Re,ye.style.transition="opacity .45s ease",ye.style.opacity="1",ye.style.pointerEvents="auto",(Ce=document.getElementById("pvp-start-match"))==null||Ce.addEventListener("click",async()=>{const je=le;await C({phase:je+"-attack",attacker:je,round:1,boostValue:G,boostUsed:!1,boostOwner:je})})},600)},700)},2800)}function ge(c,g,b,h,x){const v=document.createElement("div");v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:900;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;overflow:hidden;cursor:pointer";const w=Array.from({length:10},(L,q)=>`<div style="position:absolute;font-size:${16+Math.floor(Math.random()*24)}px;top:${5+Math.floor(Math.random()*65)}%;left:${3+Math.floor(Math.random()*94)}%;animation:fw${q%2===0?"A":"B"} ${.7+Math.random()*.7}s ease ${Math.random()*.9}s both;opacity:0">${["✨","🌟","⭐","💥","🎇","🎆","🔥","🌈"][q%8]}</div>`).join(""),_={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};v.innerHTML=`
      <style>
        @keyframes butPop{0%{transform:scale(0) rotate(-8deg);opacity:0}55%{transform:scale(1.25) rotate(2deg)}85%{transform:scale(0.92) rotate(-1deg)}100%{transform:scale(1);opacity:1}}
        @keyframes ballIn{0%{transform:translate(-70px,18px);opacity:0}65%{opacity:1}100%{transform:translate(26px,-8px);opacity:1}}
        @keyframes scoreIn{from{opacity:0;transform:translateY(-12px)}to{opacity:1;transform:translateY(0)}}
      ...
      </div>`,document.body.appendChild(q),(le=q.querySelector("#pvp-gc-back"))==null||le.addEventListener("click",()=>q.remove()),(G=q.querySelector("#pvp-gc-use"))==null||G.addEventListener("click",()=>{q.remove(),D(c,g)})}function R(){var h;const c=n[i+"Team"],g=Object.entries(c).flatMap(([x,v])=>(v||[]).filter(w=>!w.used).map(w=>({...w,_line:x})));if(!g.length)return;const b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",b.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
      ...
      </div>`,document.body.appendChild(b),(h=b.querySelector("#bp-close"))==null||h.addEventListener("click",()=>b.remove()),b.querySelectorAll(".bp-item").forEach(x=>{x.addEventListener("click",async()=>{const v=x.dataset.cid,w=g.find(I=>I.cardId===v);if(!w)return;const _=(c[w._line]||[]).find(I=>I.cardId===v);_&&(_.boost=(_.boost||0)+n.boostValue),b.remove(),await C({[i+"Team"]:c,boostUsed:!0})})})}function N(c=null){var oe,le;if(!(n.phase===i+"-attack")){u("Remplacement uniquement avant votre attaque","warning");return}const b=n[i+"Team"],h=n["usedSubIds_"+i]||[],x=n.maxSubs||3;if(h.length>=x){u(`Maximum ${x} remplacements atteint`,"warning");return}const v=Object.entries(b).flatMap(([G,H])=>(H||[]).filter(J=>J.used).map(J=>({...J,_line:G}))),w=(n[i+"Subs"]||[]).filter(G=>!h.includes(G.cardId));if(!v.length){u("Aucun joueur utilisé à remplacer","warning");return}if(!w.length){u("Aucun remplaçant disponible","warning");return}let _=Math.max(0,v.findIndex(G=>G.cardId===c));const I=((oe=v[_])==null?void 0:oe._line)||((le=v[_])==null?void 0:le.job);let $=Math.max(0,w.findIndex(G=>G.job===I)),L=!1;const q=document.createElement("div");q.id="pvp-sub-overlay",q.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function U(){var de,ye,Ee,Re,Ce,je;const G=v[_],H=w[$],J=Math.min(130,Math.round((window.innerWidth-90)/2)),ne=Math.round(J*1.35),se=$e=>`background:rgba(255,255,255,0.12);border:none;color:${$e?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${$e?"default":"pointer"};flex-shrink:0`;q.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:6px">Appuyer pour continuer</div>`,document.body.appendChild(x);let w=!1;const _=()=>{w||(w=!0,x.remove(),setTimeout(()=>b(),50))};x.addEventListener("click",_),setTimeout(_,2200)}function P(){var g;const c=document.createElement("div");c.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:20px;overflow-y:auto",c.innerHTML=`
      ...
      </div>`,document.body.appendChild(g),(b=g.querySelector("#pvp-hist-close"))==null||b.addEventListener("click",()=>g.remove())}async function pe(){if(n.phase!==i+"-attack")return;const c=i==="p1"?"p2":"p1",g=(n.round||0)+1,b=[...n.log||[]];b.push({type:"info",text:`⏭️ ${n[i+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const h=Le(n),x=we(c),v=h||!x?"finished":c+"-attack";await C({["selected_"+i]:[],modifiers:{...n.modifiers,[i]:{}},pendingAttack:null,phase:v,attacker:c,round:g,log:b}),v==="finished"&&await Se(n)}async function ie(){const c=n[i+"Team"],g=!["GK","DEF","MIL","ATT"].some(v=>(n[r+"Team"][v]||[]).some(w=>!w.used)),b=(n["selected_"+i]||[]).map(v=>{const w=(c[v._role]||[]).find(U=>U.cardId===v.cardId)||v,_=g&&["GK","DEF"].includes(v._role),I=c[v._role]||[],$=I.findIndex(U=>U.cardId===v.cardId),L=it(I.length),q=$>=0?L[$]:w._col??1;return{...w,_line:v._role,_col:q,..._?{note_a:Math.max(1,Number(w.note_a)||0)}:{}}});if(!b.length)return;const h=Nt(b,n.modifiers[i]||{});Y(i,b.map(v=>v.cardId)),b.forEach(v=>{const w=(c[v._role]||[]).find(_=>_.cardId===v.cardId);w&&(w.used=!0)}),n["selected_"+i]=[],F();const x=[...n.log||[]];if(g){const v=(n[i+"Score"]||0)+1,w=b.map(q=>({name:q.name,note:be(q,q._line||"ATT"),portrait:Ie(q),job:q.job}));x.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:w,homeTotal:h.total,aiTotal:0});const _=(n.round||0)+1,I=i==="p1"?"p2":"p1",$={...n,[i+"Team"]:c,[i+"Score"]:v},L=Le($);j.add(_),ge(w,v,n[r+"Score"]||0,!0,async()=>{await C({[i+"Team"]:c,[i+"Score"]:v,["selected_"+i]:[],modifiers:{...n.modifiers,[i]:{}},pendingAttack:null,phase:L?"finished":I+"-attack",attacker:I,round:_,log:x}),L&&await Se({...n,[i+"Score"]:v})});return}x.push({type:"pending",text:`⚔️ ${n[i+"Name"]} attaque (${h.total})`}),await C({[i+"Team"]:c,[r+"Team"]:n[r+"Team"],pendingAttack:{...h,players:b,side:i},["selected_"+i]:[],modifiers:{...n.modifiers,[i]:{}},phase:r+"-defense",log:x})}async function Ae(){const c=n[i+"Team"],g=(n["selected_"+i]||[]).map(G=>{const H=(c[G._role]||[]).find(de=>de.cardId===G.cardId)||G,J=c[G._role]||[],ne=J.findIndex(de=>de.cardId===G.cardId),se=it(J.length),fe=ne>=0?se[ne]:H._col??1;return{...H,_line:G._role,_col:fe}}),b=Rt(g,n.modifiers[i]||{});Y(i,g.map(G=>G.cardId)),g.forEach(G=>{const H=(c[G._role]||[]).find(J=>J.cardId===G.cardId);H&&(H.used=!0)}),n["selected_"+i]=[],F();const h=Pt(n.pendingAttack.total,b.total,n.modifiers[i]||{}),x=n.pendingAttack.side,v=h==="attack"||(h==null?void 0:h.goal),w=x==="p1"?"p2":"p1",_=(n.round||0)+1,I=(n.pendingAttack.players||[]).map(G=>({name:G.name,note:be(G,G._line||"ATT"),portrait:Ie(G),job:G.job})),$=[...n.log||[]];$.push({type:"duel",isGoal:v,homeScored:v&&x===i,text:v?`⚽ BUT de ${n[x+"Name"]} ! (${n.pendingAttack.total} vs ${b.total})`:`✋ Attaque stoppée (${n.pendingAttack.total} vs ${b.total})`,homePlayers:I,aiPlayers:g.map(G=>({name:G.name,note:be(G,G._line||"DEF"),portrait:Ie(G),job:G.job})),homeTotal:n.pendingAttack.total,aiTotal:b.total});const L=v?(n[x+"Score"]||0)+1:n[x+"Score"]||0,q={...n,[i+"Team"]:c,[x+"Score"]:L},U=Le(q),oe=U?"finished":w+"-attack",le=async()=>{await C({[i+"Team"]:c,[r+"Team"]:n[r+"Team"],[x+"Score"]:L,["selected_"+i]:[],modifiers:{...n.modifiers,[i]:{}},pendingAttack:null,phase:oe,attacker:w,round:_,log:$}),(oe==="finished"||U)&&await Se({...n,[x+"Score"]:L})};if(v){const G=x===i,H=G?L:n[i+"Score"]||0,J=G?n[r+"Score"]||0:L;j.add(_),ge(I,H,J,G,le)}else await le()}function ke(c){return["MIL","ATT"].some(g=>(c[g]||[]).some(b=>!b.used))}function ve(c){return["GK","DEF","MIL","ATT"].some(g=>(c[g]||[]).some(b=>!b.used))}function _e(c){return ve(c)&&!ke(c)}function we(c){const g=n[c+"Team"],b=n[(c==="p1"?"p2":"p1")+"Team"];return!!(ke(g)||!ve(b)&&_e(g))}function Le(c){const g=["MIL","ATT"].some($=>(c.p1Team[$]||[]).some(L=>!L.used)),b=["MIL","ATT"].some($=>(c.p2Team[$]||[]).some(L=>!L.used)),h=ve(c.p1Team),x=ve(c.p2Team);return!g&&!(!x&&h)&&(!b&&!(!h&&x))}function Ge(c){const g=c.p1Score||0,b=c.p2Score||0;return g===b?null:g>b?y.home_id:y.away_id}async function Se(c){try{await A.from("matches").update({status:"finished",winner_id:Ge(c),home_score:c.p1Score||0,away_score:c.p2Score||0}).eq("id",o)}catch(g){console.error("[PvP] finishMatch:",g)}}function Ke(){var x;const c=n[i+"Score"],g=n[r+"Score"],b=c>g,h=c===g;We(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
    ...
    </div>`,(x=document.getElementById("pvp-home"))==null||x.addEventListener("click",()=>{try{A.removeChannel(S)}catch{}Ne(e),l("home")})}F()}async function ao(e,t,o,a){await pi(e,t,"random",({deckId:p,formation:s,starters:d,subsRaw:l,gcCardsEnriched:u,gcDefs:i})=>{const r=f=>so(e,t,p,s,d,l,f,i||[],o,a);if(!u.length){r([]);return}ci(e,u,r)})}async function so(e,t,o,a,p,s,d=[],l=[],u,i){var B;const{state:r,navigate:f,toast:m}=t,y=r.profile.id;let k=!1,n=null;We(e);const E=(S,C,ee,ue)=>{var re;e.innerHTML=`
      <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:20px;background:linear-gradient(180deg,#0a1628,#1a0a2e);padding:24px;text-align:center;color:#fff">
        ...
        <div style="display:flex;gap:24px;align-items:center">
          ...
          <div style="display:flex;flex-direction:column;align-items:center;gap:6px">
            <div style="width:52px;height:52px;border-radius:50%;background:${C?"#1A6B3C":"rgba(255,255,255,0.1)"};display:flex;align-items:center;justify-content:center;font-size:22px;border:2px solid ${C?"#22c55e":"rgba(255,255,255,0.2)"}">
              ${C?"✅":"⏳"}
            </div>
            <div style="font-size:12px;font-weight:700">${i}</div>
            <div style="font-size:11px;color:${C?"#22c55e":"#aaa"}">${C?"Prêt":"En attente"}</div>
          </div>
        </div>
        
        ${!C&&S?'<div style="font-size:13px;color:rgba(255,255,255,0.5);animation:lobbyPulse 1.5s ease-in-out infinite">En attente de ton ami...</div>':""}
      ...
      </div>`,(re=document.getElementById("lobby-cancel"))==null||re.addEventListener("click",async()=>{k=!0,n&&(A.removeChannel(n),n=null),ee&&await A.from("friend_match_invites").update({status:"declined"}).eq("id",ee),Ne(e),f("home")})},z=async(S,C)=>{k=!0,n&&(A.removeChannel(n),n=null),await new Promise(ee=>setTimeout(ee,600)),e.isConnected&&lo(e,t,S,C,d,l)},{data:M}=await A.from("friend_match_invites").select("*").eq("status","pending").or(`and(inviter_id.eq.${y},invitee_id.eq.${u}),and(inviter_id.eq.${u},invitee_id.eq.${y})`).order("created_at",{ascending:!1}).limit(1).maybeSingle();let j,O;if(M&&M.inviter_id===u)O=!1,j=M.id,await A.from("friend_match_invites").update({invitee_deck_id:o,invitee_ready:!0,status:"accepted"}).eq("id",j);else{O=!0;const{data:S,error:C}=await A.from("friend_match_invites").insert({inviter_id:y,invitee_id:u,friend_id:u,inviter_deck_id:o,inviter_ready:!0,status:"pending"}).select().single();if(C||!S){m("Erreur création invitation","error"),Ne(e),f("home");return}j=S.id}if(E(!0,!O,j),!O){let S=null;const C={home_id:u,away_id:y,home_deck_id:M.inviter_deck_id,away_deck_id:o,status:"active"};let ee=await A.from("matches").insert({...C,mode:"friend"}).select().single();if(ee.error&&(console.warn("[FriendMatch] insert avec mode échoué, retry sans mode:",ee.error.message),ee=await A.from("matches").insert(C).select().single()),S=ee.data,!S){m("Erreur création match: "+(((B=ee.error)==null?void 0:B.message)||""),"error"),Ne(e),f("home");return}await A.from("friend_match_invites").update({match_id:S.id,status:"matched"}).eq("id",j),z(S.id,!1);return}n=A.channel("friend-invite-"+j).on("postgres_changes",{event:"UPDATE",schema:"public",table:"friend_match_invites",filter:`id=eq.${j}`},async S=>{if(k)return;const C=S.new;C.status==="matched"&&C.match_id?z(C.match_id,!0):C.status==="declined"?(m(`${i} a décliné l'invitation`,"warning"),Ne(e),f("home")):C.invitee_ready&&E(!0,!0,j)}).subscribe()}async function lo(e,t,o,a,p=[],s=[]){const{state:d,navigate:l,toast:u}=t,i=a?"p1":"p2",r=a?"p2":"p1",f=(p||[]).map(c=>c.id),m=(p||[]).map(c=>({id:c.id,gc_type:c.gc_type,_gcDef:c._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:y}=await A.from("matches").select("*").eq("id",o).single();if(!y){u("Match introuvable","error"),l("home");return}async function k(){const[{data:c},{data:g},{data:b},{data:h}]=await Promise.all([A.rpc("get_deck_for_match",{p_deck_id:y.home_deck_id}),A.rpc("get_deck_for_match",{p_deck_id:y.away_deck_id}),A.from("users").select("id,pseudo,club_name").eq("id",y.home_id).single(),A.from("users").select("id,pseudo,club_name").eq("id",y.away_id).single()]),x=$=>({cardId:$.card_id,position:$.position,id:$.id,firstname:$.firstname,name:$.surname_encoded,country_code:$.country_code,club_id:$.club_id,job:$.job,job2:$.job2,note_g:Number($.note_g)||0,note_d:Number($.note_d)||0,note_m:Number($.note_m)||0,note_a:Number($.note_a)||0,rarity:$.rarity,skin:$.skin,hair:$.hair,hair_length:$.hair_length,clubName:$.club_encoded_name||null,clubLogo:$.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}),v=((c==null?void 0:c.starters)||[]).map($=>x($)),w=((g==null?void 0:g.starters)||[]).map($=>x($)),_=(c==null?void 0:c.formation)||"4-4-2",I=(g==null?void 0:g.formation)||"4-4-2";return{p1Team:ut(v,_),p2Team:ut(w,I),p1Subs:((c==null?void 0:c.subs)||[]).map($=>x($)),p2Subs:((g==null?void 0:g.subs)||[]).map($=>x($)),p1Formation:_,p2Formation:I,p1Name:(b==null?void 0:b.club_name)||(b==null?void 0:b.pseudo)||"Joueur 1",p2Name:(h==null?void 0:h.club_name)||(h==null?void 0:h.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:a?f:[],gc_p2:a?[]:f,gcCardsFull_p1:a?m:[],gcCardsFull_p2:a?[]:m,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:s||[],lastActionAt:new Date().toISOString()}}let n=y.game_state&&Object.keys(y.game_state).length?y.game_state:null;if(!n)if(a){n=await k();const{data:c}=await A.from("matches").select("game_state").eq("id",o).single();!(c!=null&&c.game_state)||!Object.keys(c.game_state).length?await A.from("matches").update({game_state:n,turn_user_id:y.home_id}).eq("id",o):n=c.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let c=0;c<30&&!n;c++){await new Promise(b=>setTimeout(b,400));const{data:g}=await A.from("matches").select("game_state").eq("id",o).single();g!=null&&g.game_state&&Object.keys(g.game_state).length&&(n=g.game_state)}if(!n){u("Erreur de synchronisation","error"),l("home");return}n.gc_p2=f,n.gcCardsFull_p2=m,await A.from("matches").update({game_state:n}).eq("id",o)}let E=!1,z=null,M=!1;const j=new Set,O=new Set;function B(c){var $,L;try{A.removeChannel(S)}catch{}const g=n[i+"Score"]||0,b=n[r+"Score"]||0;let h,x;c.winner_id?(h=c.winner_id===d.profile.id,x=!1):c.forfeit?(h=g>b,x=!1):(x=g===b,h=g>b),($=document.getElementById("pvp-end-overlay"))==null||$.remove();const v=document.createElement("div");v.id="pvp-end-overlay",v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const w=x?"🤝":h?"🏆":"😞",_=x?"MATCH NUL":h?"VICTOIRE !":"DÉFAITE",I=x?"#fff":h?"#FFD700":"#ff6b6b";v.innerHTML=`
      <div style="font-size:64px">${w}</div>
      <div style="font-size:26px;font-weight:900;color:${I}">${_}</div>
      <div style="font-size:18px">${n[i+"Name"]} ${g} – ${b} ${n[r+"Name"]}</div>
      ${c.forfeit?`<div style="font-size:13px;color:rgba(255,255,255,0.5)">${h?"L'adversaire a quitté":"Perdu par forfait"}</div>`:""}
  ...
  `;function u(i){const r=document.getElementById("mkt-content"),f=i==="buy"?d:l;if(f.length===0){r.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      </div>`;return}const m=i==="mine"?[...f].sort((y,k)=>y.status!==k.status?y.status==="active"?-1:1:new Date(k.listed_at)-new Date(y.listed_at)):f;r.innerHTML=`<div style="display:flex;flex-direction:column;gap:10px">
      ${m.map(y=>{var j,O,B,S;const k=(j=y.card)==null?void 0:j.player;if(!k)return"";const n=k.job==="GK"?k.note_g:k.job==="DEF"?k.note_d:k.job==="MIL"?k.note_m:k.note_a,E=co[k.rarity],z=(o.profile.credits||0)>=y.price,M=y.status==="sold";return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${M?"opacity:0.65":""}">
          <div style="width:44px;height:44px;border-radius:8px;background:${mo(k.job)};color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:18px;border:2px solid ${E};flex-shrink:0">${n}</div>
          <div style="flex:1;min-width:0">
            ...
            ${i==="buy"?`<div style="font-size:11px;color:var(--gray-600)">Vendeur : ${((B=y.seller)==null?void 0:B.pseudo)||"—"}</div>`:M?`<div style="font-size:11px;color:var(--green)">✅ Vendu à ${((S=y.buyer)==null?void 0:S.pseudo)||"—"} · ${y.sold_at?new Date(y.sold_at).toLocaleDateString("fr"):""}</div>`:`<div style="font-size:11px;color:var(--gray-600)">🟢 En vente depuis le ${new Date(y.listed_at).toLocaleDateString("fr")}</div>`}
          </div>
          <div style="text-align:right;flex-shrink:0">
            <div style="font-weight:900;color:var(--yellow);font-size:15px">${y.price.toLocaleString("fr")}</div>
            ${i==="buy"?`<button class="btn btn-primary btn-sm" data-buy="${y.id}" ${z?"":"disabled"} style="margin-top:4px">${z?"Acheter":"Trop cher"}</button>`:M?'<span style="display:inline-block;margin-top:4px;font-size:10px;font-weight:700;color:#fff;background:var(--green);padding:3px 10px;border-radius:10px">VENDU</span>':`<button class="btn btn-danger btn-sm" data-cancel="${y.id}" style="margin-top:4px">Retirer</button>`}
          </div>
        </div>`}).join("")}
    </div>`,r.querySelectorAll("[data-buy]").forEach(y=>{y.addEventListener("click",()=>uo(y.dataset.buy,f,e,t))}),r.querySelectorAll("[data-cancel]").forEach(y=>{y.addEventListener("click",()=>go(y.dataset.cancel,e,t))})}u("buy"),e.querySelectorAll(".mkt-tab").forEach(i=>{i.addEventListener("click",()=>{e.querySelectorAll(".mkt-tab").forEach(r=>{const f=r===i;r.style.background=f?"var(--green)":"#fff",r.style.color=f?"#fff":"var(--gray-600)",r.style.borderColor=f?"var(--green)":"var(--gray-200)"}),u(i.dataset.tab)})})}async function uo(e,t,o,a){var y;const{state:p,toast:s,refreshProfile:d}=a,l=t.find(k=>k.id===e);if(!l)return;const u=l.price,i=p.profile.credits||0,r=(y=l.card)==null?void 0:y.player;if(i<u){s("Crédits insuffisants","error");return}if(!await fo(r,u))return;const m=document.querySelector(`[data-buy="${e}"]`);m&&(m.disabled=!0,m.textContent="⏳");try{const{data:k,error:n}=await A.rpc("buy_market_card",{p_listing_id:e,p_buyer_id:p.profile.id});if(n)throw new Error(n.message);if(!(k!=null&&k.success))throw new Error((k==null?void 0:k.error)||"Achat impossible");p.profile.credits=i-u;const E=document.getElementById("nav-credits");E&&(E.textContent=`💰 ${(i-u).toLocaleString("fr")}`),s(`✅ ${r==null?void 0:r.surname_encoded} ajouté à ta collection !`,"success"),fi(o,a)}catch(k){s("❌ "+k.message,"error"),m&&(m.disabled=!1,m.textContent="Acheter")}}function fo(e,t){return new Promise(o=>{const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",a.innerHTML=`
      ...
      </div>`,document.body.appendChild(a);const p=s=>{a.remove(),o(s)};a.querySelector("#buy-ok").addEventListener("click",()=>p(!0)),a.querySelector("#buy-cancel").addEventListener("click",()=>p(!1)),a.addEventListener("click",s=>{s.target===a&&p(!1)})})}async function go(e,t,o){const{toast:a}=o,{data:p}=await A.from("market_listings").select("card_id").eq("id",e).single();await A.from("market_listings").update({status:"cancelled"}).eq("id",e),p&&await A.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",p.card_id),a("Annonce retirée","success"),fi(t,o)}function mo(e){return{GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[e]||"#888"}async function xo(e,{state:t,navigate:o,toast:a}){e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:p}=await A.from("users").select("id,pseudo,club_name,trophies_top1,trophies_top2,trophies_top3,wins,level").order("trophies_top1",{ascending:!1}).limit(100);e.innerHTML=`
  <div class="page">
    <div class="page-header">
  ...
  `}async function yo(e,{state:t,navigate:o,toast:a}){const p=t.profile;if(!p)return;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:s}=await A.from("matches").select(`id,home_id,away_id,home_score,away_score,status,mode,winner_id,created_at,played_at,
      home:users!home_id(pseudo,club_name),
      away:users!away_id(pseudo,club_name)`).or(`home_id.eq.${p.id},away_id.eq.${p.id}`).order("created_at",{ascending:!1}).limit(50),d={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},l=(s||[]).filter(r=>r.status==="finished"),u=(s||[]).filter(r=>r.status==="in_progress");function i(r){const f=r.home_id===p.id;f?r.home_score:r.away_score,f?r.away_score:r.home_score;const m=r.winner_id===p.id,y=r.home_score===r.away_score&&r.status==="finished",k=r.status!=="finished"?"…":y?"N":m?"V":"D",n=r.status!=="finished"||y?"#888":m?"#1A6B3C":"#c0392b";let E=d[r.mode]||r.mode;r.away_id===null&&!E.startsWith("IA")&&(E="IA");const M=r.home_id===p.id?r.away:r.home;let j;r.away_id===null?j=E:M?j=`${M.club_name||M.pseudo} (${M.pseudo})`:j="Adversaire";let O="";r.status==="in_progress"&&Date.now()-new Date(r.created_at).getTime()>3600*1e3&&(O=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const B=new Date(r.created_at),S=B.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+B.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),C=r.status==="finished"?`${r.home_score} - ${r.away_score}`:`${r.home_score||0} - ${r.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
  ...
  </div>`}Xi(Qi);const he={user:null,profile:null,page:"home",params:{}};function Et(e,t="info",o=3e3){const a=document.getElementById("toast");a&&(a.textContent=e,a.className=`show ${t}`,clearTimeout(a._t),a._t=setTimeout(()=>{a.className=""},o))}function ho(e,t,o=""){document.getElementById("modal-title").textContent=e,document.getElementById("modal-body").innerHTML=t,document.getElementById("modal-footer").innerHTML=o,document.getElementById("modal-overlay").classList.remove("hidden")}function ii(){document.getElementById("modal-overlay").classList.add("hidden")}async function Tt(){if(!he.user)return;const{data:e}=await A.from("users").select("*").eq("id",he.user.id).single();e&&(he.profile=e)}const gi="mw_theme";function Gt(){return localStorage.getItem(gi)||"light"}function bo(e){var t;localStorage.setItem(gi,e),ni(e),(t=he.profile)!=null&&t.id&&A.from("users").update({theme:e}).eq("id",he.profile.id).then(()=>{})}function ni(e){document.documentElement.setAttribute("data-theme",e)}function Bt(e,t={}){he.page=e,he.params=t,Yi()}async function Yi(){var a,p;const e=document.getElementById("page-content");if(!e)return;document.querySelectorAll(".bottom-nav a").forEach(s=>{s.classList.toggle("active",s.dataset.page===he.page)});const t=document.getElementById("nav-credits");t&&he.profile&&(t.textContent=`💰 ${(he.profile.credits||0).toLocaleString("fr")}`);const o={state:he,navigate:Bt,toast:Et,openModal:ho,closeModal:ii,refreshProfile:Tt};switch(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽</div>',he.page){case"home":await yi(e,o);break;case"collection":await fn(e,o);break;case"decks":await Xt(e,o);break;case"boosters":await An(e,o);break;case"match":{const s=he.params&&he.params.matchMode||"vs_ai_easy";s==="random"?await no(e,o):s==="friend"?await ao(e,o,(a=he.params)==null?void 0:a.friendId,(p=he.params)==null?void 0:p.friendName):await Rn(e,o);break}case"market":await po(e,o);break;case"rankings":await xo(e,o);break;case"matches":await yo(e,o);break;case"friends":await nn(e,o);break;default:await yi(e,o)}}function vo(){const e=document.getElementById("app"),t=he.profile;if(!t)return;const o="/manager-wars/icons/";e.innerHTML=`
    <nav class="top-nav">
  ...
  `,document.querySelectorAll(".bottom-nav a").forEach(a=>{a.addEventListener("click",p=>{p.preventDefault(),Bt(a.dataset.page)})}),document.getElementById("nav-logo").addEventListener("click",()=>Bt("home")),document.getElementById("nav-credits").addEventListener("click",()=>Bt("boosters")),document.getElementById("theme-toggle").addEventListener("click",()=>{const p=Gt()==="dark"?"light":"dark";bo(p),Ei(p)}),Ei(Gt())}function Ei(e){const t=document.getElementById("theme-icon");t&&(t.textContent=e==="dark"?"☀️":"🌙")}async function wo(){ni(Gt()),document.getElementById("modal-overlay").addEventListener("click",o=>{o.target===o.currentTarget&&ii()}),document.getElementById("modal-close").addEventListener("click",ii);const{data:{session:e}}=await A.auth.getSession();if(!e){Ti(),mi(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:Et});return}he.user=e.user,await Tt(),Ti();try{const{data:o}=await A.from("formation_links_overrides").select("formation, links"),a={};(o||[]).forEach(p=>{a[p.formation]=p.links}),Ji(a)}catch(o){console.warn("Impossible de charger les overrides de formation:",o)}if(!he.profile){Zi(document.getElementById("app"),{state:he,navigate:async()=>{await Tt(),Wt()},toast:Et,refreshProfile:Tt});return}const t=Array.isArray(he.profile.pending_boosters)?he.profile.pending_boosters:[];if(!he.profile.onboarding_done&&t.length>0){qn(document.getElementById("app"),{state:he,navigate:()=>Wt(),toast:Et,refreshProfile:Tt});return}he.profile.theme&&he.profile.theme!==Gt()&&(localStorage.setItem(gi,he.profile.theme),ni(he.profile.theme)),Wt(),A.auth.onAuthStateChange(async(o,a)=>{o==="SIGNED_OUT"&&(he.user=null,he.profile=null,document.getElementById("app").innerHTML="",mi(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:Et}))})}function $o(){return Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight)}function Ht(){const e=document.getElementById("app");e&&(e.style.height=$o()+"px")}window.addEventListener("resize",Ht);window.addEventListener("orientationchange",()=>setTimeout(Ht,150));window.visualViewport&&window.visualViewport.addEventListener("resize",Ht);function Wt(){const e=()=>{var o;(o=he.user)!=null&&o.id&&A.from("users").update({last_seen_at:new Date().toISOString()}).eq("id",he.user.id).then(()=>{})};e(),window._presencePingInterval&&clearInterval(window._presencePingInterval),window._presencePingInterval=setInterval(e,6e4);const t=document.getElementById("app");t.style.display="flex",t.style.flexDirection="column",Ht(),vo(),Yi()}function Ti(){const e=document.getElementById("app-loader"),t=document.getElementById("app");t&&(t.style.display=""),e&&(e.classList.add("zoom-out"),setTimeout(()=>e.style.display="none",500))}function Wi(e){var a;const t=document.getElementById("app-loader");if(t&&(t.style.display="none"),document.getElementById("boot-error"))return;const o=document.createElement("div");o.id="boot-error",o.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:99999;gap:16px;color:#fff;padding:24px;text-align:center",o.innerHTML=`
    <div style="font-size:42px">📡</div>
    <div style="font-size:18px;font-weight:900">Connexion impossible</div>
    <div style="font-size:13px;color:rgba(255,255,255,0.6);max-width:280px">${e||"Le chargement a échoué. Vérifie ta connexion et réessaie."}</div>
    <button id="boot-retry" style="margin-top:8px;padding:12px 30px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer">Réessayer</button>`,document.body.appendChild(o),(a=document.getElementById("boot-retry"))==null||a.addEventListener("click",()=>window.location.reload())}wo().catch(e=>{console.error("Échec du démarrage:",e),Wi()});setTimeout(()=>{const e=document.getElementById("app-loader");e&&e.style.display!=="none"&&!e.classList.contains("zoom-out")&&!document.getElementById("boot-error")&&Wi("Le serveur met trop de temps à répondre.")},12e3);
