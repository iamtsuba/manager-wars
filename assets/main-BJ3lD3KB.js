...
import{s as v,l as Ot,m as Bi,F as qi,r as Se,j as ti,h as Di,n as Kn,o as Vn,T as bi,k as Yn,p as Wn,b as Xn}from"./formation-links-Bb-Ce4un.js";const Jn="/";function Qi(e,{navigate:t,toast:i}){let n="login";const r=()=>{var a,s,d,c,l,m;const o=n==="login";e.innerHTML=`
  ...
  `;function s(){var x;const c=document.getElementById("logo-preview"),l=document.getElementById("logo-initials"),m=((x=document.getElementById("setup-club"))==null?void 0:x.value)||"MW",y=m.trim().split(" ").filter(Boolean),g=y.length>=2?(y[0][0]+y[1][0]).toUpperCase():m.slice(0,2).toUpperCase();c&&(c.style.background=a,c.style.borderColor=o),l&&(l.textContent=g,l.style.color=o)}document.getElementById("color1").addEventListener("input",c=>{o=c.target.value,document.getElementById("swatch1").style.background=o,s()}),document.getElementById("color2").addEventListener("input",c=>{a=c.target.value,document.getElementById("swatch2").style.background=a,s()});function d(c){document.querySelectorAll(".setup-step").forEach(l=>l.classList.remove("active")),document.getElementById(`step-${c}`).classList.add("active"),document.getElementById("step-num").textContent=c,document.getElementById("progress-fill").style.width=`${Math.round(c/3*100)}%`,c===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("step1-error");if(l.textContent="",c.length<3){l.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:m}=await v.from("users").select("id").eq("pseudo",c).maybeSingle();if(m){l.textContent="Ce pseudo est déjà pris.";return}d(2)}),document.getElementById("step2-back").addEventListener("click",()=>d(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const c=document.getElementById("setup-club").value.trim(),l=document.getElementById("step2-error");if(l.textContent="",c.length<2){l.textContent="Nom trop court (min. 2 caractères).";return}const{data:m}=await v.from("users").select("id").eq("club_name",c).maybeSingle();if(m){l.textContent="Ce nom de club est déjà pris.";return}d(3)}),document.getElementById("step3-back").addEventListener("click",()=>d(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("setup-club").value.trim(),m=document.getElementById("step3-error"),y=document.getElementById("step3-finish");m.textContent="",y.disabled=!0,y.textContent="Création en cours…";try{const{error:g}=await v.from("users").insert({id:t.user.id,pseudo:c,club_name:l,club_color1:o,club_color2:a,credits:1e4});if(g)throw g;await Zn(t.user.id),await r(),n(`Bienvenue ${c} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(g){m.textContent=g.message,y.disabled=!1,y.textContent="🚀 Créer mon profil !"}})}async function Zn(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await v.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const eo="modulepreload",to=function(e){return"/"+e},Zi={},mn=function(t,i,n){let r=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),s=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));r=Promise.allSettled(i.map(d=>{if(d=to(d),d in Zi)return;Zi[d]=!0;const c=d.endsWith(".css"),l=c?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${l}`))return;const m=document.createElement("link");if(m.rel=c?"stylesheet":eo,c||(m.as="script"),m.crossOrigin="",m.href=d,s&&m.setAttribute("nonce",s),document.head.appendChild(m),c)return new Promise((y,g)=>{m.addEventListener("load",y),m.addEventListener("error",()=>g(new Error(`Unable to preload CSS for ${d}`)))})}))}function o(a){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=a,window.dispatchEvent(s),!s.defaultPrevented)throw a}return r.then(a=>{for(const s of a||[])s.status==="rejected"&&o(s.reason);return t().catch(o)})},Wt="#1A6B3C",Xt="#cc2222",io="#D4A017",en="#888";async function no(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    </style>`}const so="2026.07.20-1732";async function tn(e,{state:t,navigate:i,toast:n}){var o,a;const r=t.profile;r&&(e.innerHTML=`
  <style>
    .home-dark {
      min-height: 100%;
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    ${Se(e,{width:140})}
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),c=(s||[]).filter(j=>j.card_type==="player"&&j.player),l=(s||[]).filter(j=>j.card_type==="game_changer"),m=(s||[]).filter(j=>j.card_type==="formation"),y=(s||[]).filter(j=>j.card_type==="stadium"),{data:g}=await v.from("gc_definitions").select("name,gc_type,color,effect,image_url"),x={};(g||[]).forEach(j=>{x[j.name]=j});const{data:p}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),u={};(p||[]).forEach(j=>{u[j.id]=j}),y.forEach(j=>{j.stadium_def&&(u[j.stadium_id]=j.stadium_def)});const b=Object.keys(qi),w=Object.keys(Ce),h={};c.forEach(j=>{const P=j.player.id;h[P]=(h[P]||0)+1}),new Set(Object.keys(h).map(j=>String(j)));const $=new Set(m.map(j=>j.formation)),f=new Set(l.map(j=>j.gc_type));let C="player",k="Tous",I="",D=!1;function te(){return[...c].sort((j,P)=>{const Y=ii.indexOf(j.player.job),ie=ii.indexOf(P.player.job);return Y!==ie?Y-ie:(j.player.surname_real||"").localeCompare(P.player.surname_real||"")})}function ne(){return[...d||[]].sort((j,P)=>{const Y=ii.indexOf(j.job),ie=ii.indexOf(P.job);return Y!==ie?Y-ie:(j.surname_real||"").localeCompare(P.surname_real||"")})}function Q(){return te().filter(j=>{const P=j.player,Y=k==="Tous"||P.job===k,ie=!I||`${P.firstname} ${P.surname_real}`.toLowerCase().includes(I);return Y&&ie})}function M(){return ne().filter(j=>{const P=k==="Tous"||j.job===k,Y=!I||`${j.firstname} ${j.surname_real}`.toLowerCase().includes(I);return P&&Y})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
  ...
  </div>`;function L(){const j=document.getElementById("col-filters");j&&(C==="player"?(j.innerHTML=`
          ...
          ${wo.map(P=>`
            <button class="filter-btn" data-job="${P}"
              style="flex-shrink:0;padding:4px 12px;border-radius:20px;font-size:12px;font-weight:600;cursor:pointer;
                border:1.5px solid ${P===k?"var(--green)":"var(--tile-border)"};
                background:${P===k?"var(--green)":"#fff"};
                color:${P===k?"#fff":"var(--tile-fg-dim)"}">
              ${P}
            </button>`).join("")}
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",P=>{I=P.target.value.toLowerCase(),V()}),e.querySelectorAll(".filter-btn").forEach(P=>{P.addEventListener("click",()=>{k=P.dataset.job,L(),V()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{D=!D,L(),V()})):(j.innerHTML=`
        <div style="display:flex;justify-content:flex-end">
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{D=!D,L(),V()})))}function V(){const j=document.getElementById("col-grid");j&&(C==="player"?pe(j):C==="formation"?R(j):C==="stadium"?O(j):K(j))}function oe(j,P,Y,ie,N){const B=document.getElementById("col-grid"),H=document.getElementById("col-big");if(!B||!H)return;var ce=0;function ke(){const _e=window.innerWidth>=768,he=document.getElementById("col-big"),Ee=document.getElementById("col-grid");_e&&he&&(he.style.minHeight="420px",he.style.flex="1 1 auto"),_e&&Ee&&(Ee.style.height=Math.round(310*.76+16)+"px",Ee.style.flexShrink="0",Ee.style.overflowX="auto",Ee.style.overflowY="hidden",Ee.style.alignItems="center",Ee.style.padding="8px 16px"),H.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+P(j[ce])+"</div>";var Te=H.querySelector("[data-card-id],[data-form-id],[data-gc-id]");Te&&Te.addEventListener("click",function(){ie(j[ce])}),requestAnimationFrame(function(){var ve=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!ve||!H)){var ze=H.clientHeight-8,De=H.clientWidth-24,je=ve.offsetHeight,Ge=ve.offsetWidth;if(je>0&&Ge>0&&ze>40){var tt=_e?2.2:1.6,Oe=Math.min(ze/je,De/Ge,tt);ve.style.transform="scale("+Oe.toFixed(3)+")",ve.style.transformOrigin="top center"}}}),B.innerHTML=j.map(function(ve,ze){var De=ze===ce,je="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+(De?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+ze+'" style="'+je+'">'+Y(ve,De)+"</div>"}).join(""),B.querySelectorAll(".col-mini-item").forEach(function(ve){ve.addEventListener("click",function(){ce=Number(ve.dataset.idx),ke(),ve.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}ke()}function ue(j){var P=window.innerWidth>=768?.76:.54,Y;if(!j||j._fake){var ie=j?j.player:null;if(!ie)return"";Y=an(ie)}else Y=wi(j,"");return'<div style="display:inline-block;zoom:'+P+';pointer-events:none;line-height:0">'+Y+"</div>"}function ge(j,P,Y){P=P||100,Y=Y||140;var ie=ti[j]||{},N={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},B=Math.max(3,Math.round(P*.06)),H=Object.entries(ie).map(function(ke){var _e=ke[0],he=ke[1],Ee=_e.replace(/\d+$/,""),Te=N[Ee]||"#888",ve=Math.round(he.x*P),ze=Math.round(he.y*Y);return'<circle cx="'+ve+'" cy="'+ze+'" r="'+B+'" fill="'+Te+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),ce=Math.max(1,Math.round(P/50));return'<svg viewBox="0 0 '+P+" "+Y+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+P+'" height="'+Y+'" fill="#1A6B3C"/><rect x="'+Math.round(P*.2)+'" y="'+Math.round(Y*.02)+'" width="'+Math.round(P*.6)+'" height="'+Math.round(Y*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+ce+'"/><line x1="0" y1="'+Math.round(Y*.5)+'" x2="'+P+'" y2="'+Math.round(Y*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+ce+'"/><ellipse cx="'+Math.round(P*.5)+'" cy="'+Math.round(Y*.5)+'" rx="'+Math.round(P*.18)+'" ry="'+Math.round(Y*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+ce+'"/><rect x="'+Math.round(P*.2)+'" y="'+Math.round(Y*.82)+'" width="'+Math.round(P*.6)+'" height="'+Math.round(Y*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+ce+'"/>'+H+"</svg>"}function A(j,P,Y){var ie=Y>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+Y+"</div>":"",N=P?'data-form-id="'+P.id+'"':"",B=j.length>7?14:j.length>5?16:19,H=!!P;return"<div "+N+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(H?"#2a7a40":"#bbb")+";background:var(--tile-bg);display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(H?"":"filter:grayscale(1);opacity:0.5")+'">'+ie+'<div style="padding:8px 6px 6px;background:var(--tile-bg);text-align:center;border-bottom:3px solid '+(H?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+B+"px;font-weight:900;color:"+(H?"#1A6B3C":"#aaa")+';line-height:1">'+j+'</div></div><div style="flex:1;overflow:hidden;background:'+(H?"#1A6B3C":"#ccc")+'">'+ge(j,140,220)+"</div></div>"}function W(j,P){var Y=window.innerWidth>=768?.76:.54,ie=140,N=305,B=Math.round(N*.22),H=N-B,ce=j.length>7?10:13,ke=ge(j,ie,H),_e=P?"1.5px solid #2a7a40":"1px solid #ddd",he=P?"":"filter:grayscale(1);opacity:0.45;",Ee=P?"#1A6B3C":"#bbb",Te="#fff";return'<div style="display:inline-block;zoom:'+Y+';line-height:0;pointer-events:none"><div style="width:'+ie+"px;height:"+N+"px;border-radius:6px;border:"+_e+";background:var(--tile-bg);display:flex;flex-direction:column;overflow:hidden;"+he+'"><div style="height:'+B+"px;background:"+Ee+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+ce+"px;font-weight:900;color:"+Te+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(ie-4)+'px">'+j+'</span></div><div style="height:'+H+'px;overflow:hidden;flex-shrink:0">'+ke+"</div></div></div>"}function pe(j){if(D){const P=M();if(!P.length){j.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucun joueur.</div>';return}const Y=P.map(ie=>c.find(N=>N.player.id===ie.id)||{_fake:!0,player:ie,id:"fake-"+ie.id});oe(Y,ie=>ie._fake?an(ie.player):wi(ie,""),ie=>ie._fake?ue({player:ie.player,id:"x",_fake:!0}):ue(ie),ie=>{ie._fake||sn(ie,c,h,t)})}else{const P=Q();if(!P.length){j.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const Y={};P.forEach(N=>{const B=N.player.id;Y[B]||(Y[B]=[]),Y[B].push(N)});const ie=Object.values(Y).map(N=>ko(N));oe(ie,N=>{const B=h[N.player.id]||1,H=B>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${B}</div>`:"",ke=c.filter(_e=>_e.player.id===N.player.id&&_e.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return wi(N,H+ke)},N=>ue(N),N=>sn(N,c,h,t))}}function R(j){const P=D?b:[...$];if(!P.length){j.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const Y=P.map(ie=>({formation:ie,card:m.find(N=>N.formation===ie)||null,owned:$.has(ie)}));oe(Y,({formation:ie,card:N,owned:B})=>A(ie,B?N:null,B?m.filter(H=>H.formation===ie).length:0),({formation:ie,owned:N})=>W(ie,N),({card:ie,owned:N})=>{N&&ie&&Io(ie,m,t,o)})}function K(j){const P=Object.keys(x),Y=D?P.length?P:w:[...f];if(!Y.length){j.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const ie=Y.map(N=>({type:N,gc:Ce[N]||{icon:"⚡",desc:""},def:x[N]||null,owned:f.has(N),card:l.find(B=>B.gc_type===N)||null}));oe(ie,({type:N,gc:B,def:H,owned:ce,card:ke})=>{var tt;const _e=ce?l.filter(Oe=>Oe.gc_type===N).length:0,he=_e>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${_e}</div>`:"",Ee=(H==null?void 0:H.gc_type)==="ultra_game_changer",Te={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},ve={purple:"#b06ce0",light_blue:"#00d4ef"},ze=Te[H==null?void 0:H.color]||Te.purple,De=ve[H==null?void 0:H.color]||ve.purple,je=(H==null?void 0:H.effect)||B.desc||"",Ge=H!=null&&H.image_url?`/icons/${H.image_url}`:((tt=H==null?void 0:H.club)==null?void 0:tt.logo_url)||(H!=null&&H.country_code?`https://flagsapi.com/${H.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null);return ce&&ke?`<div data-gc-id="${ke.id}" data-gc-type="${N}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${De};background:${ze};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${De}66;cursor:pointer">
          ${he}
          ...
          <div style="width:140px;height:310px;border-radius:8px;background:linear-gradient(160deg,${P},#c45a00);border:1px solid #c45a00;display:flex;flex-direction:column;overflow:hidden">
        ...
        </div>`},null)}e.querySelectorAll(".col-tab-btn").forEach(j=>{j.addEventListener("click",()=>{C=j.dataset.tab,k="Tous",I="",D=!1,e.querySelectorAll(".col-tab-btn").forEach(P=>{const Y=P.dataset.tab===C;P.style.borderBottomColor=Y?"var(--green)":"transparent",P.style.color=Y?"var(--green)":"var(--tile-fg-dim)"}),L(),V()})}),L(),V()}function zo(e,t,i){const n=Ce[e]||{icon:"⚡",desc:"Effet spécial."},r=(t==null?void 0:t.gc_type)==="ultra_game_changer",o={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},a={purple:"#b06ce0",light_blue:"#00d4ef"},s=o[t==null?void 0:t.color]||o.purple,d=a[t==null?void 0:t.color]||a.purple,c=(t==null?void 0:t.name)||e,l=(t==null?void 0:t.effect)||n.desc,m=t!=null&&t.image_url?`/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const ni=1e3;function Io(e,t,i,n){var x,p,u;const{state:r,toast:o,closeModal:a,navigate:s,refreshProfile:d}=i,c=e.formation,l={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function m(){const b=ti[c]||{},w=qi[c]||[],h=290,$=360,f=20;function C(I){const D=b[I];return D?{x:D.x*h,y:D.y*$}:null}let k=`<svg width="${h}" height="${$}" viewBox="0 0 ${h} ${$}" xmlns="http://www.w3.org/2000/svg">`;for(const[I,D]of w){const te=C(I),ne=C(D);!te||!ne||(k+=`<line x1="${te.x}" y1="${te.y}" x2="${ne.x}" y2="${ne.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const I of Object.keys(b)){const D=C(I);if(!D)continue;const te=I.replace(/\d+/,""),ne=l[te]||"#555";k+=`<circle cx="${D.x}" cy="${D.y}" r="${f}" fill="${ne}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,k+=`<text x="${D.x}" y="${D.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${te}</text>`}return k+="</svg>",k}const y=t.filter(b=>b.formation===c);y.length;const g=!e.is_for_sale;n(`Formation ${c}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    <div style="margin-top:16px;border-top:1px solid var(--tile-border);padding-top:14px">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${m.map((R,K)=>{const O=Q[R.id]||[],j=R.is_for_sale,P=O.length?O[O.length-1]:null,ie=(l.rarity==="pepite"||l.rarity==="papyte")&&R.current_note!=null?` (☆${R.current_note})`:"";return`
            <div data-card-id="${R.id}" data-card-idx="${K}" class="exemplaire-row"
              ...
              <div style="display:flex;align-items:center;gap:10px">
                <input type="checkbox" class="expl-check" data-id="${R.id}" ${j?"disabled":""} style="width:18px;height:18px;cursor:pointer;accent-color:#1A6B3C;flex-shrink:0">
                <div style="flex:1;min-width:0">
                  <div style="font-size:12px;font-weight:700">Exemplaire ${K+1}${ie}${j?" 🏷️ En vente":""}</div>
                  ${P?`<div style="font-size:11px;color:#888;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">
                    ${P.club_name} · ${P.source==="booster"?"Booster":P.price?P.price.toLocaleString("fr")+" cr.":"—"}
                  </div>`:""}
                </div>
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;background:#f0fdf4;border-radius:10px;padding:10px 14px">
          <div>
            ...
            <div style="font-size:11px;color:#555;margin-top:2px">
              ${(e.evolution_bonus||0)>0?`+${e.evolution_bonus} appliqué${e.evolution_bonus>1?"s":""} · Note : ${I}${l.job2&&D>0?` / ${te}`:""}`:`Note actuelle : ${I}${l.job2&&D>0?` / ${te}`:""}`}
            </div>
          </div>
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",l.note_g],["DEF",l.note_d],["MIL",l.note_m],["ATT",l.note_a]].map(([R,K])=>{const O=hi[R],j=R==="GK"?"#fff":O,P=R===l.job||R===l.job2,Y=(Number(K)||0)+(P&&C>0?C:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
                <div style="width:48px;height:48px;border-radius:8px;background:#111;border:2.5px solid ${O};
                  display:flex;align-items:center;justify-content:center">
                  <span style="font-size:20px;font-weight:900;color:${j};font-family:Arial Black,Arial;line-height:1">${Y}</span>
                </div>
                <span style="font-size:10px;font-weight:700;color:${j}">${R}</span>
              </div>`}).join("")}
          </div>
        </div>
        <div>
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(ge=document.getElementById("close-detail"))==null||ge.addEventListener("click",s);let V=new Set;const oe=()=>{const R=V.size,K=document.getElementById("sell-action-panel");if(!K)return;K.style.display=R>0?"block":"none",document.getElementById("sell-selected-count").textContent=R;const O=document.getElementById("evolve-btn");O&&(O.textContent=`⬆️ Évoluer ${R>1?"(+"+R+")":""}`)};document.querySelectorAll(".expl-check").forEach(R=>{R.addEventListener("change",()=>{const K=R.dataset.id;R.checked?V.add(K):V.delete(K);const O=R.closest(".exemplaire-row");O&&(O.style.borderColor=R.checked?"#1A6B3C":"#eee"),oe()})}),document.querySelectorAll(".exemplaire-row").forEach(R=>{R.addEventListener("click",K=>{if(K.target.closest("button")||K.target.tagName==="INPUT")return;const O=R.querySelector(".expl-check");O&&!O.disabled&&(O.checked=!O.checked,O.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(R=>{R.addEventListener("click",K=>{K.stopPropagation();const O=document.querySelector(`.expl-hist[data-hist="${R.dataset.idx}"]`);O&&(O.style.display=O.style.display==="none"?"flex":"none")})}),(A=document.getElementById("evolve-btn"))==null||A.addEventListener("click",async()=>{if(y<=1)return;const R=[...V];if(!R.length)return;if(V.has(e.id)){const N=document.createElement("div");N.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",N.innerHTML=`
  ...
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
    ...
    </div>`;u("⚽ Choisir une formation",w),document.querySelectorAll("#modal-body [data-forma]").forEach(h=>{h.addEventListener("click",()=>{t.formation=h.dataset.forma;const $=cn(t.formation),f={};$.forEach(C=>{t.slots[C]&&(f[C]=t.slots[C])}),t.slots=f,b(),vt(e,t,i)})})};e.querySelectorAll("#formation-mobile-btn, #formation-pc-btn").forEach(u=>u.addEventListener("click",g)),e.querySelectorAll("#add-stad-btn-pc, #add-stad-btn").forEach(u=>u.addEventListener("click",()=>Mo(t,e,i))),e.querySelectorAll("#save-deck").forEach(u=>u.addEventListener("click",()=>Bo(t,i))),e.querySelectorAll("[data-remove-sub]").forEach(u=>{u.addEventListener("click",()=>{t.subs=t.subs.filter(b=>b!==u.dataset.removeSub),vt(e,t,i)})}),e.querySelectorAll("#add-sub-btn").forEach(u=>u.addEventListener("click",()=>Co(t,e,i)))}function So(e,t,i,n){var h,$;const r=window.innerWidth>=900,o=e.querySelector(r?"#deck-field-pc":"#deck-field-mobile");if(!o)return;const a=(h=t.stadiumCards)==null?void 0:h.find(f=>f.id===t.stadiumCardId),s=a&&(($=t.stadDefMap)==null?void 0:$[a.stadium_id])||null,d=ti[t.formation]||{},c=Bi(t.formation)||[],l={};for(const f of i){const C=t.slots[f],k=C?t.playerCards.find(I=>I.id===C):null;k!=null&&k.player?l[f]={...k.player,_evolution_bonus:k.evolution_bonus||0,face:k.player.face||null}:l[f]=null}const m=window.innerWidth>=900,y=m?window.innerWidth-280:window.innerWidth-20,g=m?Math.min(y,860):y,x=Math.round(m?g*.82:g*.85),p=m?84:44;let u="";for(const[f,C]of c){const k=d[f],I=d[C];if(!k||!I)continue;const D=k.x*g,te=Math.round(.03*x+k.y*.85*x),ne=I.x*g,Q=Math.round(.03*x+I.y*.85*x),M=l[f],L=l[C],V=Ot(M,L);V==="#ff3333"||V==="#cc2222"?u+=`<line x1="${D.toFixed(1)}" y1="${te.toFixed(1)}" x2="${ne.toFixed(1)}" y2="${Q.toFixed(1)}" stroke="${V}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(u+=`<line x1="${D.toFixed(1)}" y1="${te.toFixed(1)}" x2="${ne.toFixed(1)}" y2="${Q.toFixed(1)}" stroke="${V}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,u+=`<line x1="${D.toFixed(1)}" y1="${te.toFixed(1)}" x2="${ne.toFixed(1)}" y2="${Q.toFixed(1)}" stroke="${V}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let b="";const w=Math.round(p*657/507);for(const f of i){const C=d[f];if(!C)continue;const k=l[f],I=C.x*g;C.y*x;const D=Math.round(.03*x+C.y*(.85*x)),te=Math.round(I-p/2),ne=Math.round(D-w/2);if(k){const Q=f.replace(/\d+/,""),M=s&&(s.club_id&&String(k.club_id)===String(s.club_id)||s.country_code&&k.country_code===s.country_code),L=Se({...k,_evolution_bonus:k._evolution_bonus||0},{width:p,showStad:!0,stadDef:s,role:Q});b+=`<div style="position:absolute;left:${te}px;top:${ne}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${f}">
        <div style="position:relative">${M?'<div style="position:absolute;top:-18px;left:0;right:0;text-align:center;font-size:14px;z-index:5;line-height:1">🏟️</div>':""}${L}</div>
      </div>`}else{const Q=f.replace(/\d+/,"");b+=`<div style="position:absolute;left:${te}px;top:${ne}px;width:${p}px;height:${w}px;
  ...
  `,document.body.appendChild(r);let o=!1;const a=document.getElementById("pack-cut-zone"),s=document.getElementById("pack-blade");let d=!1;const c=L=>L.touches&&L.touches[0]?L.touches[0].clientX:L.clientX;function l(L){o||(d=!0,s.style.opacity="1",m(L))}function m(L){if(!d||o)return;const V=a.getBoundingClientRect(),oe=c(L)-V.left,ue=Math.max(0,Math.min(1,oe/V.width));s.style.width=ue*V.width+"px",ue>=.82&&g()}function y(){o||(d=!1,s.style.transition="width .2s ease, opacity .2s ease",s.style.width="0",s.style.opacity="0",setTimeout(()=>{o||(s.style.transition="")},220))}function g(){var V;if(o)return;o=!0,d=!1,s.style.width="100%",s.style.opacity="1",(V=document.getElementById("cut-flash"))==null||V.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const L=document.getElementById("cut-hint");L&&(L.style.opacity="0"),a.classList.add("pack-cut"),setTimeout(()=>{s.style.opacity="0",document.getElementById("pack-phase").style.display="none",b(0)},620)}a.addEventListener("pointerdown",l),window.addEventListener("pointermove",m),window.addEventListener("pointerup",y),a.addEventListener("touchstart",l,{passive:!0}),window.addEventListener("touchmove",m,{passive:!0}),window.addEventListener("touchend",y);let x=0,p=!1;const u=new Set;function b(L){x=L,document.getElementById("reveal-phase").style.display="flex",w(),h(L,0),I()}function w(){const L=document.getElementById("card-dots");L&&(L.innerHTML=e.map((V,oe)=>`<div class="card-dot" data-i="${oe}" style="width:8px;height:8px;border-radius:50%;background:${oe===x?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),L.querySelectorAll(".card-dot").forEach(V=>V.addEventListener("click",()=>f(parseInt(V.dataset.i)))))}function h(L,V){var O,j;const oe=e[L],ue=document.getElementById("card-counter"),ge=document.getElementById("card-track");ue&&(ue.textContent=`Carte ${L+1} / ${e.length}`);const A=document.getElementById("reveal-btns");A&&(A.style.display=L===e.length-1?"flex":"none");const W=oe.card_type==="player"&&((O=oe.player)==null?void 0:O.rarity)==="legende",pe=!u.has(L);u.add(L);let R=0;if(oe.card_type==="player"&&oe.player){const P=oe.player,Y=P.job||"ATT";R=Number(Y==="GK"?P.note_g:Y==="DEF"?P.note_d:Y==="MIL"?P.note_m:P.note_a)||0}const K=P=>{ge.innerHTML=`
        ...
        </div>`;const Y=document.getElementById("current-card-wrap");V!==0?(Y.style.transition="none",Y.style.transform=`translateX(${V>0?100:-100}%)`,requestAnimationFrame(()=>{Y.style.transition="transform .28s cubic-bezier(.25,1,.5,1)",Y.style.transform="translateX(0)"})):Y.animate([{opacity:0,transform:"scale(.7)"},{opacity:1,transform:"scale(1)"}],{duration:300,easing:"cubic-bezier(.34,1.56,.64,1)"}),P||W?te():ne(),w()};pe&&(((j=oe.player)==null?void 0:j.rarity)==="legende"||R>=8)&&oe.card_type==="player"&&oe.player?$(oe,()=>K(!0)):K(!1)}function $(L,V){var _e;p=!0;const oe=L.player,ue=`https://flagsapi.com/${oe.country_code}/flat/64.png`,ge=(_e=oe.clubs)==null?void 0:_e.logo_url,A=oe.face?"/"+oe.face.replace(/^public\//,"").replace(/^\//,""):null,W=oe.job||"ATT",pe=Number(W==="GK"?oe.note_g:W==="DEF"?oe.note_d:W==="MIL"?oe.note_m:oe.note_a)||0,R=L.evolution_bonus||0,K=pe+R,O=oe.rarity==="legende",j=K>=18,P=document.getElementById("walkout-overlay"),Y=document.getElementById("walkout-stage");if(!P||!Y){p=!1,V();return}let ie=null;O&&(ie=mo("/sounds/Legendary.mp3",.8)),P.style.display="flex";const N=()=>{const he=Y.firstElementChild;he&&(he.classList.remove("wo-in"),he.classList.add("wo-out"))},B=1800,H=400;Y.innerHTML=`<img class="wo-in" src="${ue}" style="height:130px;border-radius:10px;box-shadow:0 10px 36px rgba(0,0,0,.6)" onerror="this.style.display='none'">`,navigator.vibrate&&navigator.vibrate(30),setTimeout(N,B),setTimeout(()=>{var he;Y.innerHTML=ge?`<img class="wo-in" src="${ge}" style="max-height:160px;max-width:210px;object-fit:contain">`:`<div class="wo-in" style="font-size:34px;font-weight:900;color:#fff;text-align:center">${((he=oe.clubs)==null?void 0:he.encoded_name)||"CLUB"}</div>`,navigator.vibrate&&navigator.vibrate(30)},B+H),setTimeout(N,B*2+H),setTimeout(()=>{Y.innerHTML=A?`<img class="wo-in" src="${A}" style="height:200px;border-radius:50%;box-shadow:0 0 40px rgba(255,255,255,0.3);object-fit:cover;object-position:top">`:'<div class="wo-in" style="font-size:80px">👤</div>',navigator.vibrate&&navigator.vibrate(30)},(B+H)*2),setTimeout(N,(B+H)*2+B);const ke=j?"#FFD700":{GK:"#c0c0c0",DEF:"#e03030",MIL:"#D4A017",ATT:"#3fbf5f"}[W]||"#fff";setTimeout(()=>{Y.innerHTML=`<div class="wo-in" style="
        font-size:${j?"120px":"90px"};font-weight:900;color:${ke};
        ...
        ${K}
      </div>`,j&&navigator.vibrate&&navigator.vibrate([50,30,100,30,200])},(B+H)*3),setTimeout(N,(B+H)*3+B),setTimeout(()=>{P.style.display="none",Y.innerHTML="",p=!1,ie&&!O&&ie.pause(),navigator.vibrate&&navigator.vibrate([40,30,80]),V()},(B+H)*4)}function f(L){if(p||L<0||L>=e.length||L===x)return;const V=L>x?1:-1;x=L,h(L,V)}function C(){f(x+1)}function k(){f(x-1)}function I(){const L=document.getElementById("card-viewport");if(!L||L._swipeBound)return;L._swipeBound=!0;let V=0,oe=0,ue=0,ge=!1;const A=O=>O.touches?O.touches[0].clientX:O.clientX,W=O=>O.touches?O.touches[0].clientY:O.clientY,pe=O=>{ge=!0,V=A(O),oe=W(O),ue=0},R=O=>{if(!ge)return;ue=A(O)-V;const j=W(O)-oe;if(Math.abs(ue)<Math.abs(j))return;const P=document.getElementById("current-card-wrap");P&&(P.style.transition="none",P.style.transform=`translateX(${ue*.6}px) rotate(${ue*.02}deg)`)},K=()=>{if(!ge)return;ge=!1;const O=document.getElementById("current-card-wrap"),j=55;ue<=-j&&x<e.length-1?C():ue>=j&&x>0?k():O&&(O.style.transition="transform .2s ease",O.style.transform="translateX(0)")};L.addEventListener("pointerdown",pe),L.addEventListener("pointermove",R),L.addEventListener("pointerup",K),L.addEventListener("pointercancel",K),L.addEventListener("touchstart",pe,{passive:!0}),L.addEventListener("touchmove",R,{passive:!0}),L.addEventListener("touchend",K),L.addEventListener("click",O=>{if(Math.abs(ue)>8)return;const j=L.getBoundingClientRect();O.clientX-j.left>j.width/2?C():k()})}let D=null;function te(){const L=document.getElementById("fireworks-canvas");if(!L)return;L.width=window.innerWidth,L.height=window.innerHeight;const V=L.getContext("2d"),oe=[];function ue(){const A=Math.random()*L.width,W=Math.random()*L.height*.6,pe=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],R=pe[Math.floor(Math.random()*pe.length)];for(let K=0;K<60;K++){const O=Math.PI*2/60*K,j=2+Math.random()*5;oe.push({x:A,y:W,vx:Math.cos(O)*j,vy:Math.sin(O)*j,alpha:1,color:R,size:2+Math.random()*3})}}ue(),D=setInterval(ue,600);function ge(){if(document.getElementById("fireworks-canvas")){V.clearRect(0,0,L.width,L.height);for(let A=oe.length-1;A>=0;A--){const W=oe[A];if(W.x+=W.vx,W.y+=W.vy+.08,W.vy*=.98,W.alpha-=.018,W.alpha<=0){oe.splice(A,1);continue}V.globalAlpha=W.alpha,V.fillStyle=W.color,V.beginPath(),V.arc(W.x,W.y,W.size,0,Math.PI*2),V.fill()}V.globalAlpha=1,(D!==null||oe.length>0)&&requestAnimationFrame(ge)}}ge()}function ne(){D!==null&&(clearInterval(D),D=null);const L=document.getElementById("fireworks-canvas");L&&L.getContext("2d").clearRect(0,0,L.width,L.height)}if(n){const L=document.getElementById("reveal-btns");L&&(L.innerHTML='<button class="btn btn-primary" id="reveal-next" style="flex:1">Continuer →</button>'),(M=document.getElementById("reveal-next"))==null||M.addEventListener("click",()=>{ne(),r.remove(),n()})}else document.getElementById("reveal-collection").addEventListener("click",()=>{ne(),r.remove(),i("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{ne(),r.remove(),i("boosters")})}function Ho(e){var t,i,n,r;if(e.card_type==="player"&&e.player)return Po(e);if(e.card_type==="game_changer"){const o=e._gcDef,a=(o==null?void 0:o.gc_type)==="ultra_game_changer",s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},d={purple:"#b06ce0",light_blue:"#00d4ef"},c=s[o==null?void 0:o.color]||s.purple,l=d[o==null?void 0:o.color]||d.purple,m=(o==null?void 0:o.name)||e.gc_type||"Game Changer",y=(o==null?void 0:o.effect)||((t=Li[e.gc_type])==null?void 0:t.desc)||"",g=o!=null&&o.image_url?`/icons/${o.image_url}`:null,x=((i=Li[e.gc_type])==null?void 0:i.icon)||"⚡";return`<div style="width:170px;height:240px;background:${c};border-radius:14px;border:3px solid ${l};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${l}88;flex-shrink:0">
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff;position:relative">
      ...
      <!-- Pagination -->
      ${o.length>1?`<div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">${o.map((Q,M)=>`<div style="width:6px;height:6px;border-radius:50%;background:${M===l?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}</div>`:""}
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const W=t.pendingAttack;let pe="";if(t.selected.length>0){const R=t.selected.map(O=>({...(t.homeTeam[O._role]||[]).find(P=>P.cardId===O.cardId)||O,_line:O._role})),K=Qt(R,t.modifiers.home,t.formation);pe=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(A=>{var W,pe,R;if(A.type==="duel"){const K=A.isGoal,O=A.homeScored?"#FFD700":K?"#ff6b6b":"rgba(255,255,255,0.3)",j=A.homeScored?"⚽ BUT !":K?"⚽ BUT IA !":(W=A.homePlayers)!=null&&W.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${K?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${O};margin-bottom:4px">
                <div style="font-size:9px;color:${O};letter-spacing:1px;margin-bottom:5px;font-weight:700;text-transform:uppercase">${j}</div>
    ...
    </div>`,(f=m.querySelector("#sub-close"))==null||f.addEventListener("click",()=>m.remove()),(C=m.querySelector("#out-up"))==null||C.addEventListener("click",()=>{s>0&&(s--,y())}),(k=m.querySelector("#out-down"))==null||k.addEventListener("click",()=>{s<o.length-1&&(s++,y())}),(I=m.querySelector("#in-up"))==null||I.addEventListener("click",()=>{c>0&&(c--,y())}),(D=m.querySelector("#in-down"))==null||D.addEventListener("click",()=>{c<a.length-1&&(c++,y())});const $=(ne,Q,M,L)=>{const V=m.querySelector("#"+ne);if(!V)return;let oe=0;V.addEventListener("touchstart",ue=>{oe=ue.touches[0].clientY},{passive:!0}),V.addEventListener("touchend",ue=>{const ge=ue.changedTouches[0].clientY-oe;if(Math.abs(ge)<30)return;const A=Q();ge<0&&A<L-1?(M(A+1),y()):ge>0&&A>0&&(M(A-1),y())},{passive:!0})};$("in-panel",()=>c,ne=>c=ne,a.length),$("out-panel",()=>s,ne=>s=ne,o.length),(te=m.querySelector("#sub-confirm"))==null||te.addEventListener("click",ne=>{if(ne.preventDefault(),ne.stopPropagation(),l)return;l=!0;const Q=o[s],M=a[c];if(!Q||!M)return;let L=null,V=-1;for(const[ue,ge]of Object.entries(t.homeTeam)){const A=(ge||[]).findIndex(W=>W.cardId===Q.cardId);if(A!==-1){L=ue,V=A;break}}if(V===-1||!L){kt("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),m.remove();return}const oe={...M,_line:L,_col:Q._col||0,used:!1,boost:0};t.homeTeam[L].splice(V,1,oe),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(M.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:Q.name,firstname:Q.firstname,note:Pe(Q,L),portrait:Zt(Q),job:Q.job,country_code:Q.country_code,rarity:Q.rarity,clubName:Q.clubName,clubLogo:Q.clubLogo},inPlayer:{name:M.name,firstname:M.firstname,note:Pe(M,L),portrait:Zt(M),job:M.job,country_code:M.country_code,rarity:M.rarity,clubName:M.clubName,clubLogo:M.clubLogo},text:`🔄 ${M.firstname} ${M.name} remplace ${Q.firstname} ${Q.name}`}),m.remove(),er(Q,M,()=>$e(e,t,i))})}document.body.appendChild(m),y()}function ur(e,t,i,n,r){var x,p;const o=(n.gcDefs||[]).find(u=>u.name===t),a=Ce[t]||{icon:"⚡",desc:"Carte spéciale."},s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[o==null?void 0:o.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",d={purple:"#b06ce0",light_blue:"#00d4ef"}[o==null?void 0:o.color]||"#b06ce0",c=(o==null?void 0:o.name)||t,l=(o==null?void 0:o.effect)||a.desc,m=o!=null&&o.image_url?`/icons/${o.image_url}`:null,y=a.icon||"⚡",g=document.createElement("div");g.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",g.innerHTML=`
    ...
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      ...
      ${fi(e.aiTeam,e.formation,null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.1))}
            ...
            </div>`}}catch(J){console.error("[Ranked] MMR update error:",J)}(re=document.getElementById("pvp-end-overlay"))==null||re.remove();const q=document.createElement("div");q.id="pvp-end-overlay",q.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;color:#fff;padding:24px;text-align:center;overflow-y:auto";const F=T?"🤝":S?"🏆":"😞",U=T?"MATCH NUL":S?"VICTOIRE !":"DÉFAITE",Z=T?"#fff":S?"#FFD700":"#ff6b6b";q.innerHTML=`
      ...
      ${G}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${l?"🏆 Retour à la Mini League":a?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(q),(ee=q.querySelector("#pvp-end-home"))==null||ee.addEventListener("click",()=>{q.remove(),et(e),l?g("mini-league",{openLeagueId:l}):g(a?"ranked":"home")})}const Q=v.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},_=>{var z;const E=_.new;try{if(E.status==="finished"||E.forfeit){if(C)return;C=!0,k&&(clearInterval(k),k=null),E.game_state&&(f=E.game_state),ne(E);return}if(E.game_state){const S=f;f=E.game_state;const T=f._lastGC;if(T&&T.seq&&!te.has(T.seq)&&(te.add(T.seq),T.by!==p)){O(T.type,T.by,()=>W());return}const G=S[p+"Score"]||0,q=S[u+"Score"]||0,F=f[p+"Score"]||0,U=f[u+"Score"]||0,Z=F>G,re=U>q;if((Z||re)&&!D.has(f.round)){D.add(f.round);const ee=[...f.log||[]].reverse().find(de=>de.isGoal),J=((ee==null?void 0:ee.homePlayers)||[]).map(de=>({name:de.name,note:de.note,portrait:de.portrait,job:de.job}));!!((z=ee==null?void 0:ee.text)!=null&&z.includes("DERNIER CORNER"))?_e().then(()=>{Je(J,F,U,Z,()=>W())}):Je(J,F,U,Z,()=>W());return}W()}}catch(S){console.error("[PvP] crash:",S)}}).subscribe();async function M(_){Object.assign(f,_),f.lastActionAt=new Date().toISOString();const{error:E}=await v.from("matches").update({game_state:f}).eq("id",i);E&&x("Erreur de synchronisation","error");try{W()}catch(z){console.error("[PvP] renderPvpScreen crash:",z)}}async function L(){if(C)return;C=!0,k&&(clearInterval(k),k=null);const _=n?h.away_id:h.home_id,E=n?"p2":"p1",z=n?"p1":"p2",S={...f,[E+"Score"]:3,[z+"Score"]:0,phase:"finished"},T=S.p1Score||0,G=S.p2Score||0;try{await v.from("matches").update({game_state:S}).eq("id",i);const{data:q,error:F}=await v.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:_,p_home_score:T,p_away_score:G,p_forfeit:!0});if(F&&console.error("[PvP] finish_pvp_match (forfeit):",F),l)try{await v.rpc("check_and_finish_mini_league",{p_league_id:l})}catch(U){console.error("[PvP] check_and_finish_mini_league (forfeit):",U)}try{const{data:U}=await v.rpc("apply_match_rewards",{p_match_id:i});U!=null&&U.success&&!(U!=null&&U.skipped)&&typeof t.refreshProfile=="function"&&await t.refreshProfile()}catch(U){console.error("[PvP] apply_match_rewards (forfeit):",U)}}catch{}try{v.removeChannel(Q)}catch{}setTimeout(()=>{et(e),l?g("mini-league",{openLeagueId:l}):g("home")},800)}const V={BOOST_STAT:({value:_=1,count:E=1,roles:z=[]},S,T)=>{const G=S[p+"Team"],q=Object.entries(G).filter(([F])=>!z.length||z.includes(F)).flatMap(([F,U])=>U.filter(Z=>!Z.used).map(Z=>({...Z,_line:F})));if(!q.length){S.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),T(S);return}oe(q,E,`Choisir ${E} joueur(s) à booster (+${_})`,F=>{F.forEach(U=>{const Z=(G[U._line]||[]).find(re=>re.cardId===U.cardId);Z&&(Z.boost=(Z.boost||0)+_,S.log.push({text:`⚡ +${_} sur ${Z.name}`,type:"info"}))}),S[p+"Team"]=G,T(S)})},DEBUFF_STAT:({value:_=1,count:E=1,roles:z=[],target:S="ai"},T,G)=>{const q=S==="home"?p:u,F=T[q+"Team"],U=S==="home"?"allié":"adverse",Z=Object.entries(F).filter(([re])=>!z.length||z.includes(re)).flatMap(([re,ee])=>ee.map(J=>({...J,_line:re})));if(!Z.length){T.log.push({text:`🎯 Aucun joueur ${U}`,type:"info"}),G(T);return}oe(Z,E,`Choisir ${E} joueur(s) ${U}(s) (-${_})`,re=>{re.forEach(ee=>{const J=(F[ee._line]||[]).find(se=>se.cardId===ee.cardId);J&&(J.boost=(J.boost||0)-_,T.log.push({text:`🎯 -${_} sur ${J.name}`,type:"info"}))}),T[q+"Team"]=F,G(T)})},GRAY_PLAYER:({count:_=1,roles:E=[],target:z="ai"},S,T)=>{const G=z==="home"?p:u,q=S[G+"Team"],F=z==="home"?"allié":"adverse",U=Object.entries(q).filter(([Z])=>!E.length||E.includes(Z)).flatMap(([Z,re])=>re.filter(ee=>!ee.used).map(ee=>({...ee,_line:Z})));if(!U.length){S.log.push({text:`❌ Aucun joueur ${F}`,type:"info"}),T(S);return}oe(U,_,`Choisir ${_} joueur(s) ${F}(s) à exclure`,Z=>{const re="usedCardIds_"+G,ee=new Set(S[re]||[]);Z.forEach(J=>{const se=(q[J._line]||[]).find(de=>de.cardId===J.cardId);se&&(se.used=!0,ee.add(J.cardId),S.log.push({text:`❌ ${se.name} exclu !`,type:"info"}))}),S[re]=[...ee],S[G+"Team"]=q,T(S)})},REVIVE_PLAYER:({count:_=1,roles:E=[]},z,S)=>{const T=z[p+"Team"],G=Object.entries(T).filter(([q])=>!E.length||E.includes(q)).flatMap(([q,F])=>F.filter(U=>U.used).map(U=>({...U,_line:q})));if(!G.length){z.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),S(z);return}oe(G,_,`Choisir ${_} joueur(s) à ressusciter`,q=>{q.forEach(F=>{const U=(T[F._line]||[]).find(Z=>Z.cardId===F.cardId);U&&(U.used=!1,z.log.push({text:`💫 ${U.name} ressuscité !`,type:"info"}))}),z[p+"Team"]=T,S(z)})},REMOVE_GOAL:({},_,E)=>{const z=u+"Score";_[z]>0?(_[z]--,_.log.push({text:"🚫 Dernier but annulé !",type:"info"})):_.log.push({text:"🚫 Aucun but à annuler",type:"info"}),E(_)},ADD_GOAL_DRAW:({},_,E)=>{_[p+"Score"]===_[u+"Score"]?(_[p+"Score"]++,_.log.push({text:"🎯 But bonus !",type:"info"})):_.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),E(_)},ADD_SUB:({value:_=1},E,z)=>{E.maxSubs=(E.maxSubs||3)+_,E.log.push({text:`🔄 +${_} remplacement(s)`,type:"info"}),z(E)},CUSTOM:({},_,E)=>E(_)};function oe(_,E,z,S){const T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let G=[];function q(){var U,Z;const F=_.map(re=>{const ee=re._line||re.job||"MIL",J=G.find(de=>de.cardId===re.cardId),se=Se({...re,_evolution_bonus:0},{width:90,showStad:!0,role:ee,extraNote:re.boost||0});return`<div class="pp-item" data-cid="${re.cardId}"
        ...
        </div>`,(U=T.querySelector("#pp-close"))==null||U.addEventListener("click",()=>T.remove()),T.querySelectorAll(".pp-item").forEach(re=>{re.addEventListener("click",()=>{const ee=re.dataset.cid,J=_.find(de=>de.cardId===ee),se=G.findIndex(de=>de.cardId===ee);J&&(se>-1?G.splice(se,1):G.length<E&&G.push(J),q())})}),(Z=T.querySelector("#pp-confirm"))==null||Z.addEventListener("click",()=>{T.remove(),S(G)})}q(),document.body.appendChild(T)}async function ue(_,E){const S=(f["gcCardsFull_"+p]||[]).find(F=>F.id===_),T=(S==null?void 0:S._gcDef)||(f.gcDefs||[]).find(F=>{var U;return F.name===E||((U=F.name)==null?void 0:U.toLowerCase().trim())===(E==null?void 0:E.toLowerCase().trim())}),G=[...f["usedGc_"+p]||[],_],q={type:E,by:p,seq:(f._gcAnimSeq||0)+1};te.add(q.seq),O(E,p,async()=>{if(T!=null&&T.effect_type&&T.effect_type!=="CUSTOM"){const U=V[T.effect_type];if(U){const Z={...f};U(T.effect_params||{},Z,async re=>{re["usedGc_"+p]=G,re._lastGC=q,re._gcAnimSeq=q.seq,await M(re)});return}}const F={...f};switch(E){case"Remplacement+":F.maxSubs=(F.maxSubs||3)+1,F.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const U=u+"Score";F[U]>0&&(F[U]--,F.log.push({text:"🚫 But annulé",type:"info"}));break}}F["usedGc_"+p]=G,F._lastGC=q,F._gcAnimSeq=q.seq,await M(F)})}function ge(_,E){const z="usedCardIds_"+_,S=new Set(f[z]||[]);E.forEach(T=>S.add(T)),f[z]=[...S]}function A(){for(const _ of["p1","p2"]){const E=new Set(f["usedCardIds_"+_]||[]),z=f[_+"Team"];if(z)for(const S of["GK","DEF","MIL","ATT"])(z[S]||[]).forEach(T=>{T.used=E.has(T.cardId)})}}function W(){var Ki,Vi,Yi,Wi,Xi,Ji;if(f.phase==="reveal")return pe();if(f.phase==="midfield")return K();if(f.phase==="finished")return xi();const _=f[p+"Team"],E=f[u+"Team"];A();const z=f[p+"Score"],S=f[u+"Score"],T=f[p+"Name"],G=f[u+"Name"],q=f.phase===p+"-attack",F=f.phase===p+"-defense",U=Array.isArray(f["selected_"+p])?f["selected_"+p]:[],Z=U.map(X=>X.cardId),re=window.innerWidth>=700,ee=f[p+"Subs"]||[],J=f["usedSubIds_"+p]||[],se=ee.filter(X=>!J.includes(X.cardId)),de=f["gcCardsFull_"+p]||[],me=f["usedGc_"+p]||[],le=de.filter(X=>!me.includes(X.id)),xe=f.boostOwner===p&&!f.boostUsed,ye=[..._.MIL||[],..._.ATT||[]].filter(X=>!X.used),ae=[...E.MIL||[],...E.ATT||[]].filter(X=>!X.used),Le=!ze(E),Re=ye.length===0&&ae.length===0&&!Le,Ie=(_.DEF||[]).filter(X=>!X.used),Ve=(_.GK||[]).filter(X=>!X.used);let He=[];q&&ye.length===0&&(Le?(He=Ie.map(X=>X.cardId),Ie.length===0&&(He=He.concat(Ve.map(X=>X.cardId)))):Re&&(He=[...Ie,...Ve].map(X=>X.cardId)));function it(X,fe,Ae){var Ct,ft;const we=X._gcDef,Mt={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[we==null?void 0:we.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",rt={purple:"#b06ce0",light_blue:"#00d4ef"}[we==null?void 0:we.color]||"#b06ce0",qe=(we==null?void 0:we.name)||X.gc_type,Ne=(we==null?void 0:we.effect)||((Ct=Ce[X.gc_type])==null?void 0:Ct.desc)||"",Me=we!=null&&we.image_url?`/icons/${we.image_url}`:null,_t=((ft=Ce[X.gc_type])==null?void 0:ft.icon)||"⚡",Fe=Math.round(Ae*.22),ut=Math.round(Ae*.22),at=Ae-Fe-ut,jt=qe.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${X.id}" data-gc-type="${X.gc_type}"
      ...
      </div>`}const Be=(X,fe)=>fe?Ht(130,175):it(X,130,175),Ye=(X,fe)=>fe?Ht(68,95):it(X,68,95),Ue=re?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",ct=q?je(p)?`<button id="pvp-action" style="${Ue};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${U.length===0?"disabled":""}>⚔️ ATTAQUEZ <span id="pvp-timer"></span></button>`:`<button id="pvp-action" data-pass="1" style="${Ue};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER <span id="pvp-timer"></span></button>`:F?`<button id="pvp-action" style="${Ue};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${U.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="pvp-timer"></span></button>`:`<div style="font-size:11px;color:rgba(255,255,255,0.3);text-align:center;padding:4px">⏳ Tour de ${G}</div>`,nt=q&&!je(p)?'<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">Aucun attaquant — passez la main</div>':q||F?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${U.length}/3 sélectionné(s)</div>`:"",At=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${re?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
      ...
      <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
        ...
        <div id="mobile-play-area" style="flex:1;min-height:0;display:flex;overflow:hidden">
          <div id="match-field" style="flex:1;min-width:0;min-height:0;overflow:hidden">
            <div class="terrain-wrapper" style="width:100%;height:100%;overflow:hidden">
              ${Rt(_,f[p+"Formation"],pt,Z,Lt(),Pt(),He)}
            </div>
          </div>
        </div>
        <div id="mobile-action-bar" style="position:absolute;left:0;right:0;bottom:0;z-index:20;background:rgba(0,0,0,0.55);padding:6px 8px 8px;display:flex;flex-direction:column;gap:6px;box-shadow:0 -4px 16px rgba(0,0,0,0.5)">
      ...
      </div>`;function St(){const X=e.querySelector(".match-screen");if(!X)return;const fe=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);X.style.height=fe+"px",X.style.minHeight=fe+"px",X.style.maxHeight=fe+"px",X.style.overflow="hidden";const Ae=e.querySelector("#mobile-action-bar"),we=e.querySelector("#mobile-play-area");Ae&&we&&(we.style.paddingBottom=Ae.offsetHeight+"px")}if(St(),setTimeout(St,120),setTimeout(St,400),I||(I=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",St),window.visualViewport.addEventListener("scroll",St)),window.addEventListener("resize",St)),function(){const fe=e.querySelector("#match-field .terrain-wrapper svg")||e.querySelector(".terrain-wrapper svg");if(!fe)return;const Ae=fe.closest("#match-terrain-wrap");Ae&&(Ae.style.cssText="position:relative;width:100%;height:100%;padding:0"),fe.removeAttribute("width"),fe.removeAttribute("height"),fe.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",fe.setAttribute("preserveAspectRatio","xMidYMid meet")}(),f._pvpResizeBound||(f._pvpResizeBound=!0,window.addEventListener("resize",()=>{const X=e.querySelector(".terrain-wrapper svg");if(X){const fe=X.closest("#match-terrain-wrap");fe&&(fe.style.cssText="position:relative;width:100%;height:100%;padding:0"),X.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0"}})),e.querySelectorAll(".match-slot-hit").forEach(X=>{X.addEventListener("click",()=>{if(!q&&!F)return;const fe=X.dataset.cardId,Ae=X.dataset.role,we=(_[Ae]||[]).find(Ne=>Ne.cardId===fe);if(!we||we.used)return;const Mt=He.includes(fe);if(q&&!["MIL","ATT"].includes(Ae)&&!Mt)return;Array.isArray(f["selected_"+p])||(f["selected_"+p]=[]);const rt=f["selected_"+p],qe=rt.findIndex(Ne=>Ne.cardId===fe);qe>-1?rt.splice(qe,1):rt.length<3&&rt.push({...we,_role:Ae}),W()})}),e.querySelectorAll(".match-used-hit").forEach(X=>{X.addEventListener("click",()=>Y(X.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(X=>{X.addEventListener("click",()=>Y())}),(Ki=e.querySelector("#pvp-sub-open"))==null||Ki.addEventListener("click",()=>Y()),e.querySelectorAll(".pvp-gc-mini").forEach(X=>{q?X.addEventListener("click",()=>j(X.dataset.gcId,X.dataset.gcType)):(X.style.opacity="0.35",X.style.cursor="default",X.addEventListener("click",()=>kt("⚡ Les Game Changers ne sont utilisables qu'en attaque","rgba(180,100,0,0.9)")))}),(Vi=e.querySelector("#pvp-boost-card"))==null||Vi.addEventListener("click",()=>P()),(Yi=e.querySelector("#pvp-action"))==null||Yi.addEventListener("click",X=>{q?X.currentTarget.dataset.pass==="1"||!je(p)?H():Ee():F&&Te()}),(Wi=e.querySelector("#pvp-quit"))==null||Wi.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&L()}),(Xi=e.querySelector("#pvp-view-opp"))==null||Xi.addEventListener("click",()=>N()),(Ji=e.querySelector("#pvp-toggle-history"))==null||Ji.addEventListener("click",()=>B()),k&&(clearInterval(k),k=null),(q||F)&&!C){let X=30,fe=!1;const Ae=()=>document.getElementById("pvp-timer"),we=()=>{Ae()&&(Ae().textContent=X+"s",Ae().style.color=fe?"#ff4444":"#fff")};we(),k=setInterval(()=>{X--,X<0?fe?(clearInterval(k),k=null,q&&!je(p)?H():L()):(fe=!0,X=15,we()):we()},1e3)}}function pe(){st(e),e.innerHTML=`
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:${G};letter-spacing:3px;text-transform:uppercase;font-weight:700;animation:gcLabel .4s ease both">${ee} joue une carte</div>
      <div style="width:200px;border-radius:18px;border:3px solid ${G};background:${T};display:flex;flex-direction:column;overflow:hidden;animation:gcFlipIn .7s cubic-bezier(.34,1.56,.64,1) both,gcGlow 1.8s ease infinite .7s">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(J);let se=!1;const de=()=>{se||(se=!0,J.remove(),setTimeout(()=>z&&z(),50))};J.addEventListener("click",de),setTimeout(de,3e3)}function j(_,E){var J,se,de,me;const S=(f["gcCardsFull_"+p]||[]).find(le=>le.id===_),T=S==null?void 0:S._gcDef,G={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[T==null?void 0:T.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",q={purple:"#b06ce0",light_blue:"#00d4ef"}[T==null?void 0:T.color]||"#b06ce0",F=(T==null?void 0:T.name)||E,U=(T==null?void 0:T.effect)||((J=Ce[E])==null?void 0:J.desc)||"Carte spéciale.",Z=T!=null&&T.image_url?`/icons/${T.image_url}`:null,re=((se=Ce[E])==null?void 0:se.icon)||"⚡",ee=document.createElement("div");ee.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",ee.innerHTML=`
      <div style="width:190px;border-radius:16px;border:3px solid ${q};background:${G};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 40px ${q}66">
        ...
        <div style="height:160px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
          ${Z?`<img src="${Z}" style="max-width:150px;max-height:150px;object-fit:contain">`:`<span style="font-size:72px">${re}</span>`}
        </div>
        <div style="padding:10px;background:rgba(0,0,0,0.38);text-align:center">
      ...
      </div>`,document.body.appendChild(ee),(de=ee.querySelector("#pvp-gc-back"))==null||de.addEventListener("click",()=>ee.remove()),(me=ee.querySelector("#pvp-gc-use"))==null||me.addEventListener("click",()=>{ee.remove(),ue(_,E)})}function P(){var S;const _=f[p+"Team"],E=Object.entries(_).flatMap(([T,G])=>(G||[]).filter(q=>!q.used).map(q=>({...q,_line:T})));if(!E.length)return;const z=document.createElement("div");z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",z.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
      ...
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${E.map(T=>{const G={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[T._line]||"#555",q=Pe(T,T._line)+(T.boost||0);return`<div class="bp-item" data-cid="${T.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${G};overflow:hidden;cursor:pointer">
      ...
      </div>`,document.body.appendChild(z),(S=z.querySelector("#bp-close"))==null||S.addEventListener("click",()=>z.remove()),z.querySelectorAll(".bp-item").forEach(T=>{T.addEventListener("click",async()=>{const G=T.dataset.cid,q=E.find(U=>U.cardId===G);if(!q)return;const F=(_[q._line]||[]).find(U=>U.cardId===G);F&&(F.boost=(F.boost||0)+f.boostValue),z.remove(),await M({[p+"Team"]:_,boostUsed:!0})})})}function Y(_=null){var se,de;if(!(f.phase===p+"-attack")){x("Remplacement uniquement avant votre attaque","warning");return}const z=f[p+"Team"],S=f["usedSubIds_"+p]||[],T=f.maxSubs||3;if(S.length>=T){x(`Maximum ${T} remplacements atteint`,"warning");return}const G=Object.entries(z).flatMap(([me,le])=>(le||[]).filter(xe=>xe.used).map(xe=>({...xe,_line:me}))),q=(f[p+"Subs"]||[]).filter(me=>!S.includes(me.cardId));if(!G.length){x("Aucun joueur utilisé à remplacer","warning");return}if(!q.length){x("Aucun remplaçant disponible","warning");return}let F=Math.max(0,G.findIndex(me=>me.cardId===_));const U=((se=G[F])==null?void 0:se._line)||((de=G[F])==null?void 0:de.job);let Z=Math.max(0,q.findIndex(me=>me.job===U)),re=!1;const ee=document.createElement("div");ee.id="pvp-sub-overlay",ee.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function J(){var Re,Ie,Ve,He,it,Ht;const me=G[F],le=q[Z],xe=Math.min(130,Math.round((window.innerWidth-90)/2)),ye=Math.round(xe*1.35),ae=Be=>`background:rgba(255,255,255,0.12);border:none;color:${Be?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Be?"default":"pointer"};flex-shrink:0`;ee.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        ...
        <div id="pout-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
          ...
          <button id="pout-down" style="${ae(F>=G.length-1)}" ${F>=G.length-1?"disabled":""}>▼</button>
          <div style="font-size:10px;color:rgba(255,255,255,0.35)">${F+1}/${G.length}</div>
        </div>
      </div>
      <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
        <button id="psub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
      </div>`,(Re=ee.querySelector("#psub-close"))==null||Re.addEventListener("click",()=>ee.remove()),(Ie=ee.querySelector("#pout-up"))==null||Ie.addEventListener("click",()=>{F>0&&(F--,J())}),(Ve=ee.querySelector("#pout-down"))==null||Ve.addEventListener("click",()=>{F<G.length-1&&(F++,J())}),(He=ee.querySelector("#pin-up"))==null||He.addEventListener("click",()=>{Z>0&&(Z--,J())}),(it=ee.querySelector("#pin-down"))==null||it.addEventListener("click",()=>{Z<q.length-1&&(Z++,J())});const Le=(Be,Ye,Ue,ct)=>{const nt=ee.querySelector("#"+Be);if(!nt)return;let At=0;nt.addEventListener("touchstart",pt=>{At=pt.touches[0].clientY},{passive:!0}),nt.addEventListener("touchend",pt=>{const wt=pt.changedTouches[0].clientY-At;if(Math.abs(wt)<30)return;const ot=Ye();wt<0&&ot<ct-1?(Ue(ot+1),J()):wt>0&&ot>0&&(Ue(ot-1),J())},{passive:!0})};Le("pin-panel",()=>Z,Be=>Z=Be,q.length),Le("pout-panel",()=>F,Be=>F=Be,G.length),(Ht=ee.querySelector("#psub-confirm"))==null||Ht.addEventListener("click",async Be=>{if(Be.preventDefault(),Be.stopPropagation(),re)return;re=!0;const Ye=G[F],Ue=q[Z];if(!Ye||!Ue)return;const ct=Ye._line,nt=(z[ct]||[]).findIndex(wt=>wt.cardId===Ye.cardId);if(nt===-1){x("Erreur : joueur introuvable","error"),ee.remove();return}const At={...Ue,_line:ct,position:Ye.position,used:!1,boost:0};z[ct].splice(nt,1,At);const pt=[...S,Ue.cardId];ee.remove(),ie(Ye,Ue,async()=>{await M({[p+"Team"]:z,[u+"Team"]:f[u+"Team"],["usedSubIds_"+p]:pt})})})}document.body.appendChild(ee),J()}function ie(_,E,z){const S={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const G=(U,Z,re)=>`<div style="text-align:center">
      <div style="font-size:9px;color:${Z};letter-spacing:2px;text-transform:uppercase;font-weight:700;margin-bottom:6px">${re}</div>
      ...
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(_),(E=_.querySelector("#pvp-opp-close"))==null||E.addEventListener("click",()=>_.remove())}function B(){var z;const _=f.log||[],E=document.createElement("div");E.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column",E.innerHTML=`
      ...
      </div>`,document.body.appendChild(E),(z=E.querySelector("#pvp-hist-close"))==null||z.addEventListener("click",()=>E.remove())}async function H(){if(f.phase!==p+"-attack")return;const _=p==="p1"?"p2":"p1",E=(f.round||0)+1,z=[...f.log||[]];z.push({type:"info",text:`⏭️ ${f[p+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const S=Ge(f),T=je(_),G=S||!T?"finished":_+"-attack";await M({["selected_"+p]:[],modifiers:{...f.modifiers,[p]:{}},pendingAttack:null,phase:G,attacker:_,round:E,log:z}),G==="finished"&&await Oe(f)}function ce(_){const E=(_.GK||[]).some(S=>!S.used),z=["DEF","MIL","ATT"].some(S=>(_[S]||[]).some(T=>!T.used));return E&&!z}function ke(_){return(_.p1Score||0)!==(_.p2Score||0)?null:ce(_.p1Team)&&!ze(_.p2Team)?{side:"p1",gk:(_.p1Team.GK||[]).find(E=>!E.used)}:ce(_.p2Team)&&!ze(_.p1Team)?{side:"p2",gk:(_.p2Team.GK||[]).find(E=>!E.used)}:null}async function _e(){const _=document.createElement("div");_.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:950;display:flex;align-items:center;justify-content:center;overflow:hidden;text-align:center;padding:24px",document.body.appendChild(_);const E=(z,S)=>new Promise(T=>{_.innerHTML=`<div style="font-size:32px;font-weight:900;color:${S};letter-spacing:2px;animation:lcFade 1.4s ease both">${z}</div>
      <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(T,1400)});await E("⚽ DERNIER CORNER","#FFD700"),await E("🧤 LE GARDIEN MONTE !","#4fc3f7"),_.remove()}async function he(_,E){const{side:z,gk:S}=_;if(!S)return;const T=z+"Score",G=(E[T]||0)+1;S.used=!0;const q=[...E.log||[]];q.push({type:"duel",isGoal:!0,homeScored:z===p,homePlayers:z==="p1"?[Bt(S)]:[],aiPlayers:z==="p2"?[Bt(S)]:[],text:`⚽ DERNIER CORNER — Le gardien de ${E[z+"Name"]} marque !`});const F=(E.round||0)+1,U=z==="p1"?"p2":"p1",Z={...E,[T]:G},re=Ge(Z);D.add(F),await _e();const ee=z===p?G:E[p+"Score"]||0,J=z===p?E[u+"Score"]||0:G;Je([Bt(S)],ee,J,z===p,async()=>{await M({[T]:G,log:q,round:F,pendingAttack:null,phase:re?"finished":U+"-attack",attacker:U,["selected_"+p]:[],modifiers:{...f.modifiers,[p]:{}}}),re&&await Oe({...f,[T]:G})})}async function Ee(){const _=f[p+"Team"],E=!["GK","DEF","MIL","ATT"].some(G=>(f[u+"Team"][G]||[]).some(q=>!q.used)),z=(f["selected_"+p]||[]).map(G=>{const q=(_[G._role]||[]).find(de=>de.cardId===G.cardId)||G,F=["GK","DEF"].includes(G._role),U=_[G._role]||[],Z=U.findIndex(de=>de.cardId===G.cardId),re=mt(U.length),ee=Z>=0?re[Z]:q._col??1,J=f.stadiumDef||f.homeStadiumDef||null,se=q.stadiumBonus||J&&(J.club_id&&String(q.club_id)===String(J.club_id)||J.country_code&&q.country_code===J.country_code)||!1;return{...q,_line:G._role,_col:ee,stadiumBonus:se,...F?{note_a:Math.max(1,Number(q.note_a)||0)}:{}}});if(!z.length)return;const S=Jt(z,f.modifiers[p]||{},f[p+"Formation"]);ge(p,z.map(G=>G.cardId)),z.forEach(G=>{const q=(_[G._role]||[]).find(F=>F.cardId===G.cardId);q&&(q.used=!0)}),f["selected_"+p]=[],W();const T=[...f.log||[]];if(E){if(z.length===1&&(z[0]._line==="GK"||z[0].job==="GK")&&(f.p1Score||0)===(f.p2Score||0)){await he({side:p,gk:z[0]},{...f,[p+"Team"]:_,log:T});return}const q=(f[p+"Score"]||0)+1,F=z.map(J=>Bt(J));T.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:F,homeTotal:S.total,aiTotal:0});const U=(f.round||0)+1,Z=p==="p1"?"p2":"p1",re={...f,[p+"Team"]:_,[p+"Score"]:q},ee=Ge(re);D.add(U),Je(F,q,f[u+"Score"]||0,!0,async()=>{await M({[p+"Team"]:_,[p+"Score"]:q,["selected_"+p]:[],modifiers:{...f.modifiers,[p]:{}},pendingAttack:null,phase:ee?"finished":Z+"-attack",attacker:Z,round:U,log:T}),ee&&await Oe({...f,[p+"Score"]:q})});return}T.push({type:"pending",text:`⚔️ ${f[p+"Name"]} attaque (${S.total})`}),await M({[p+"Team"]:_,[u+"Team"]:f[u+"Team"],pendingAttack:{...S,players:z,side:p},["selected_"+p]:[],modifiers:{...f.modifiers,[p]:{}},phase:u+"-defense",log:T})}async function Te(){const _=f[p+"Team"],E=(f["selected_"+p]||[]).map(le=>{const xe=(_[le._role]||[]).find(Ie=>Ie.cardId===le.cardId)||le,ye=_[le._role]||[],ae=ye.findIndex(Ie=>Ie.cardId===le.cardId),Le=mt(ye.length),Re=ae>=0?Le[ae]:xe._col??1;return{...xe,_line:le._role,_col:Re}}),z=Qt(E,f.modifiers[p]||{},f[p+"Formation"]);ge(p,E.map(le=>le.cardId)),E.forEach(le=>{const xe=(_[le._role]||[]).find(ye=>ye.cardId===le.cardId);xe&&(xe.used=!0)}),f["selected_"+p]=[],W();const S=Gi(f.pendingAttack.total,z.total,f.modifiers[p]||{}),T=f.pendingAttack.side,G=S==="attack"||(S==null?void 0:S.goal),q=T==="p1"?"p2":"p1",F=(f.round||0)+1,U=(f.pendingAttack.players||[]).map(le=>Bt(le)),Z=[...f.log||[]];Z.push({type:"duel",isGoal:G,homeScored:G&&T===p,text:G?`⚽ BUT de ${f[T+"Name"]} ! (${f.pendingAttack.total} vs ${z.total})`:`✋ Attaque stoppée (${f.pendingAttack.total} vs ${z.total})`,homePlayers:U,aiPlayers:E.map(le=>Bt(le)),homeTotal:f.pendingAttack.total,aiTotal:z.total});const re=G?(f[T+"Score"]||0)+1:f[T+"Score"]||0,ee={...f,[p+"Team"]:_,[T+"Score"]:re},J=Ge(ee),se=J?"finished":q+"-attack",de=!J&&!G?ke(ee):null,me=async()=>{if(de){await he(de,ee);return}await M({[p+"Team"]:_,[u+"Team"]:f[u+"Team"],[T+"Score"]:re,["selected_"+p]:[],modifiers:{...f.modifiers,[p]:{}},pendingAttack:null,phase:se,attacker:q,round:F,log:Z}),(se==="finished"||J)&&await Oe({...f,[T+"Score"]:re})};if(G){const le=T===p,xe=le?re:f[p+"Score"]||0,ye=le?f[u+"Score"]||0:re;D.add(F),Je(U,xe,ye,le,me)}else await me()}function ve(_){return["MIL","ATT"].some(E=>(_[E]||[]).some(z=>!z.used))}function ze(_){return["GK","DEF","MIL","ATT"].some(E=>(_[E]||[]).some(z=>!z.used))}function De(_){return ze(_)&&!ve(_)}function je(_){const E=f[_+"Team"],z=f[(_==="p1"?"p2":"p1")+"Team"];return!!(ve(E)||De(E)&&!ze(z)||De(E)&&!ve(z)&&ze(z))}function Ge(_){return!ze(_.p1Team)&&!ze(_.p2Team)}function tt(_){const E=_.p1Score||0,z=_.p2Score||0;return E===z?null:E>z?h.home_id:h.away_id}async function Oe(_){try{const E=tt(_),z=E?h.home_id===E?h.away_id:h.home_id:null,S=_.p1Score||0,T=_.p2Score||0,{data:G,error:q}=await v.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:E,p_home_score:S,p_away_score:T,p_forfeit:!1});if(q&&console.error("[PvP] finish_pvp_match:",q),l)try{await v.rpc("check_and_finish_mini_league",{p_league_id:l})}catch(F){console.error("[PvP] check_and_finish_mini_league:",F)}E&&z&&ho(E,z).catch(()=>{});try{const{data:F}=await v.rpc("apply_match_rewards",{p_match_id:i});F!=null&&F.success&&!(F!=null&&F.skipped)&&typeof t.refreshProfile=="function"&&await t.refreshProfile()}catch(F){console.error("[PvP] apply_match_rewards:",F)}if(typeof c=="function")try{await c({homeScore:S,awayScore:T,winnerId:E,homeId:h.home_id,awayId:h.away_id,matchId:i})}catch(F){console.error("[PvP] onMatchEnd:",F)}}catch(E){console.error("[PvP] finishMatch:",E)}}function xi(){var T;const _=f[p+"Score"],E=f[u+"Score"],z=_>E,S=_===E;st(e),e.innerHTML=`
    ...
    </div>`,(T=document.getElementById("pvp-home"))==null||T.addEventListener("click",()=>{try{v.removeChannel(Q)}catch{}et(e),l?g("mini-league",{openLeagueId:l}):g("home")})}W()}async function Mi(e,t,i=!1){var s,d;const{state:n,navigate:r,toast:o}=t,a=i&&((d=(s=t==null?void 0:t.state)==null?void 0:s.params)==null?void 0:d.rankedData)||null;await gi(e,t,i?"ranked":"random",async({deckId:c,formation:l,starters:m,subsRaw:y,gcCardsEnriched:g,gcDefs:x,stadiumDef:p})=>{st(e);const u=async b=>{await hr(e,t,c,l,m,y,b||[],x,p,i,a)};if(!(g!=null&&g.length)){await u([]);return}ui(e,g,u)})}async function br(e,t,i){return yr(e,t,i)}async function hr(e,t,i,n,r,o,a=[],s=[],d=null,c=!1,l=null){var ne,Q;const{state:m,navigate:y,toast:g}=t,x=(l==null?void 0:l.mmr)??((ne=m.profile)==null?void 0:ne.mmr)??1e3,p=c?Di(x):null,u=c?"linear-gradient(135deg, #1a0a2e 0%, #2d1060 50%, #1a0a2e 100%)":"linear-gradient(135deg, #0a1a2e 0%, #0d3d1e 100%)",b=c?(p==null?void 0:p.color)||"#D4A017":"#FFD700";let w=!1,h=null,$=null;e.innerHTML=`
  ...
  </div>`,(h=document.getElementById("ml-create-btn"))==null||h.addEventListener("click",()=>Ar(e,t)),($=document.getElementById("ml-refresh-list"))==null||$.addEventListener("click",()=>Ze(e,t,i)),e.querySelectorAll(".ml-tab").forEach(f=>f.addEventListener("click",()=>Ze(e,t,f.dataset.tab))),e.querySelectorAll("[data-league-id]").forEach(f=>f.addEventListener("click",()=>lt(e,t,f.dataset.leagueId))),e.querySelectorAll("[data-join]").forEach(f=>f.addEventListener("click",C=>{C.stopPropagation(),Gn(e,t,f.dataset.join,f.dataset.type)})),e.querySelectorAll("[data-delete]").forEach(f=>f.addEventListener("click",C=>{C.stopPropagation(),ji(e,t,f.dataset.delete,f.dataset.name,i)})),e.querySelectorAll("[data-claim-league]").forEach(f=>f.addEventListener("click",async C=>{C.stopPropagation(),f.disabled=!0,f.textContent="...";const{data:k,error:I}=await v.rpc("claim_mini_league_prize",{p_league_id:f.dataset.claimLeague,p_user_id:n.profile.id});if(I||!(k!=null&&k.success)){t.toast((k==null?void 0:k.error)||"Erreur lors de la récupération","error"),Ze(e,t,i);return}if(!k.already_claimed){n.profile.credits=(n.profile.credits||0)+k.prize;const D=document.getElementById("nav-credits");D&&(D.textContent=`💰 ${n.profile.credits.toLocaleString("fr")}`),t.toast(`💰 +${k.prize.toLocaleString("fr")} cr. ajoutés !`,"success")}Ze(e,t,i)}))}function pi(e,t,i=!1,n=null){var d,c;const r=e.creator_id===t,o=e.pot||0,a=((c=(d=e.mini_league_members)==null?void 0:d[0])==null?void 0:c.count)||0,s=n&&n.amount>0&&!n.claimed;return`<div data-league-id="${e.id}" style="background:${Ut};border:1px solid ${s?"rgba(212,160,23,0.5)":Kt};border-radius:12px;padding:14px 16px;cursor:pointer;margin-bottom:8px">
    ...
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:${Ut};border:1px solid ${Kt};border-radius:12px;padding:16px">
        ...
        ${m.map(M=>`
          <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid ${Et}">
            <div style="width:36px;height:36px;border-radius:50%;background:${M.club_color2||Fn};display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:900;color:${M.club_color1||"#fff"}">${(M.pseudo||"?").slice(0,2).toUpperCase()}</div>
            <div style="flex:1"><div style="font-size:13px;font-weight:700;color:${We}">${M.club_name||M.pseudo}</div><div style="font-size:11px;color:${Qe}">@${M.pseudo}</div></div>
      ...
      <div style="background:${Ut};border:1px solid ${Kt};border-radius:12px;padding:16px">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>${y.map((M,L)=>{const V=x>0&&a.status==="finished"?L===0?Math.floor(x*.7):L===1?Math.floor(x*.2):L===2?Math.floor(x*.1):0:0;return`<tr style="border-bottom:1px solid ${Et};${M.userId===o?"background:rgba(74,222,128,0.08);":""}">
              <td style="padding:7px 3px 7px 0;font-weight:700;color:${L===0?dt:L<3?"#4ade80":$t}">${["🥇","🥈","🥉"][L]||L+1}</td>
              <td style="padding:7px 3px"><div style="font-weight:700;color:${We}">${M.clubName}</div><div style="font-size:10px;color:${Qe}">@${M.pseudo}</div></td>
              <td style="text-align:center;color:${$t}">${M.played}</td><td style="text-align:center;color:${$t}">${M.won}-${M.drawn}-${M.lost}</td>
              <td style="text-align:center;color:${M.goalDiff>=0?"#4ade80":"#ff6b6b"}">${M.goalDiff>=0?"+":""}${M.goalDiff}</td>
              <td style="text-align:center;font-weight:900;font-size:14px;color:${We}">${M.points}</td>
      ...
      <div style="background:${Ut};border:1px solid ${Kt};border-radius:12px;padding:16px">
        ...
        ${Array.from({length:Math.max(0,a.status==="active"?a.current_day-1:a.current_day)},(M,L)=>L+1).reverse().map(M=>{const L=(d||[]).filter(V=>V.matchday===M);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:${Qe};margin-bottom:6px">Journée ${M}</div>${L.map(V=>un(V,m,o,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
  ...
  </div>`;const h=a.status==="waiting"?"waiting":a.status==="active"?"active":"archived";if(($=document.getElementById("ml-back"))==null||$.addEventListener("click",()=>Ze(e,t,h)),(f=document.getElementById("ml-refresh"))==null||f.addEventListener("click",async M=>{const L=M.currentTarget;L.style.opacity="0.5",await lt(e,t,i)}),(C=document.getElementById("ml-start-btn"))==null||C.addEventListener("click",()=>Cr(e,t,a,m)),(k=document.getElementById("ml-next-day"))==null||k.addEventListener("click",()=>Br(e,t,i)),(I=document.getElementById("ml-join-now"))==null||I.addEventListener("click",()=>Gn(e,t,i,a.type)),(D=document.getElementById("ml-leave-btn"))==null||D.addEventListener("click",()=>Mr(e,t,i,p)),(te=document.getElementById("ml-delete-btn"))==null||te.addEventListener("click",()=>ji(e,t,i,a.name,"waiting")),(ne=document.getElementById("ml-delete-now"))==null||ne.addEventListener("click",()=>ji(e,t,i,a.name,h)),(Q=document.getElementById("ml-archive-btn"))==null||Q.addEventListener("click",()=>jr(e,t,i)),e.querySelectorAll("[data-play-match]").forEach(M=>{M.addEventListener("click",()=>{const L=g.find(V=>V.id===M.dataset.playMatch);L&&t.navigate("match-mini-league",{mlMatchId:L.id,leagueId:i})})}),a.status==="finished"&&w){const M=y.findIndex(L=>L.userId===o);M>=0&&M<3&&w.prize_amount>0&&!w.prize_claimed&&setTimeout(()=>qr(e,t,a,w,M),400)}}function un(e,t,i,n,r=!1){const o=m=>t.find(y=>y.id===m);if(e.is_bye){const m=o(e.home_id);return`<div style="padding:8px;border-radius:8px;background:rgba(255,255,255,0.03);margin-bottom:6px;font-size:12px;color:${$t};text-align:center">🔵 ${(m==null?void 0:m.club_name)||(m==null?void 0:m.pseudo)||"?"} exempté(e)</div>`}const a=o(e.home_id),s=o(e.away_id),d=e.home_id===i||e.away_id===i,c=d&&e.status==="pending"&&n&&!r,l=e.status==="finished"?`${e.home_score} - ${e.away_score}`:"vs";return`<div style="display:flex;align-items:center;gap:8px;padding:10px;border-radius:8px;background:${d?"rgba(26,107,60,0.16)":"rgba(255,255,255,0.03)"};margin-bottom:6px;border:1px solid ${d?"rgba(74,222,128,0.35)":Et}">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
  ...
  </div>`;let d="buy";const c=()=>{var p,u,b,w,h,$,f;return{name:(((p=document.getElementById("flt-name"))==null?void 0:p.value)||"").toLowerCase().trim(),club:(((u=document.getElementById("flt-club"))==null?void 0:u.value)||"").toLowerCase().trim(),country:(((b=document.getElementById("flt-country"))==null?void 0:b.value)||"").toLowerCase().trim(),job:((w=document.getElementById("flt-job"))==null?void 0:w.value)||"",rarity:((h=document.getElementById("flt-rarity"))==null?void 0:h.value)||"",note1:parseInt(($=document.getElementById("flt-note1"))==null?void 0:$.value)||0,note2:parseInt((f=document.getElementById("flt-note2"))==null?void 0:f.value)||0}};function l(p){const u=c();return p.filter(b=>{var D,te,ne;const w=(D=b.card)==null?void 0:D.player;if(!w)return!1;const h=`${w.firstname} ${w.surname_real}`.toLowerCase(),$=(((te=w.clubs)==null?void 0:te.encoded_name)||"").toLowerCase(),f=(w.country_code||"").toLowerCase(),C=((ne=b.card)==null?void 0:ne.evolution_bonus)||0,k=qt(w,w.job,C),I=w.job2?qt(w,w.job2,C):0;return!(u.name&&!h.includes(u.name)||u.club&&!$.includes(u.club)||u.country&&!f.includes(u.country)||u.job&&w.job!==u.job||u.rarity&&w.rarity!==u.rarity||u.note1&&k<u.note1||u.note2&&I<u.note2)})}function m(p){var I,D,te,ne;const u=(I=p.card)==null?void 0:I.player;if(!u)return"";const b=((D=p.card)==null?void 0:D.evolution_bonus)||0,w=qt(u,u.job,b),h=u.job2?qt(u,u.job2,b):0,$=(i.profile.credits||0)>=p.price,f=u.country_code?`https://flagsapi.com/${u.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,C=ri[u.job]||"#bbb",k=u.job2?ri[u.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden">
      <!-- Drapeau -->
    ...
    </div>`}function y(p){var I,D,te,ne;const u=(I=p.card)==null?void 0:I.player;if(!u)return"";const b=((D=p.card)==null?void 0:D.evolution_bonus)||0,w=qt(u,u.job,b),h=u.job2?qt(u,u.job2,b):0,$=p.status==="sold",f=u.country_code?`https://flagsapi.com/${u.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,C=ri[u.job]||"#bbb",k=u.job2?ri[u.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${$?"opacity:0.7":""}">
      ${f?`<img src="${f}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      ${(te=u.clubs)!=null&&te.logo_url?`<img src="${u.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${C};display:flex;align-items:center;justify-content:center">
        ...
        <button id="buy-ok" style="flex:1;padding:12px;border-radius:10px;border:none;background:var(--green);color:#fff;font-size:14px;font-weight:900;cursor:pointer">Confirmer</button>
          ...
          <div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${M?"border:2px solid var(--yellow)":""}">
            ...
            <div style="flex:1;min-width:0">
              ...
              <div style="font-size:11px;color:var(--tile-fg-dim)">${I.club_name} · ${te.label}</div>
            </div>
            <div style="text-align:right;flex-shrink:0">
              <div style="font-size:20px">${L?"❓":te.emoji}</div>
              <div style="font-size:11px;font-weight:700;color:${te.color}">${L?"Placement":te.label}</div>
              ${L?"":`<div style="font-size:10px;color:var(--tile-fg-dim)">${Q}% WR</div>`}
            </div>
  ...
  <div class="page">
    ...
    <nav class="top-nav">
      <div class="logo" id="nav-logo" title="Manager Wars v2026.07.20-1732" style="cursor:pointer">
