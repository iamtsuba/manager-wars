import{s as E,F as mi,h as Gt,j as Ui,l as at,i as An,k as Tn,b as zn}from"./formation-links-DJC25wD7.js";function Ai(e,{navigate:t,toast:i}){e.innerHTML=`
  ...
  `;function d(){var z;const n=document.getElementById("logo-preview"),o=document.getElementById("logo-initials"),g=((z=document.getElementById("setup-club"))==null?void 0:z.value)||"MW",w=g.trim().split(" ").filter(Boolean),$=w.length>=2?(w[0][0]+w[1][0]).toUpperCase():g.slice(0,2).toUpperCase();n&&(n.style.background=s,n.style.borderColor=l),o&&(o.textContent=$,o.style.color=l)}document.getElementById("color1").addEventListener("input",n=>{l=n.target.value,document.getElementById("swatch1").style.background=l,d()}),document.getElementById("color2").addEventListener("input",n=>{s=n.target.value,document.getElementById("swatch2").style.background=s,d()});function u(n){document.querySelectorAll(".setup-step").forEach(o=>o.classList.remove("active")),document.getElementById(`step-${n}`).classList.add("active"),document.getElementById("step-num").textContent=n,document.getElementById("progress-fill").style.width=`${Math.round(n/3*100)}%`,n===3&&d()}document.getElementById("step1-next").addEventListener("click",async()=>{const n=document.getElementById("setup-pseudo").value.trim(),o=document.getElementById("step1-error");if(o.textContent="",n.length<3){o.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:g}=await E.from("users").select("id").eq("pseudo",n).maybeSingle();if(g){o.textContent="Ce pseudo est déjà pris.";return}u(2)}),document.getElementById("step2-back").addEventListener("click",()=>u(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const n=document.getElementById("setup-club").value.trim(),o=document.getElementById("step2-error");if(o.textContent="",n.length<2){o.textContent="Nom trop court (min. 2 caractères).";return}const{data:g}=await E.from("users").select("id").eq("club_name",n).maybeSingle();if(g){o.textContent="Ce nom de club est déjà pris.";return}u(3)}),document.getElementById("step3-back").addEventListener("click",()=>u(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const n=document.getElementById("setup-pseudo").value.trim(),o=document.getElementById("setup-club").value.trim(),g=document.getElementById("step3-error"),w=document.getElementById("step3-finish");g.textContent="",w.disabled=!0,w.textContent="Création en cours…";try{const{error:$}=await E.from("users").insert({id:t.user.id,pseudo:n,club_name:o,club_color1:l,club_color2:s,credits:1e4});if($)throw $;await Sn(t.user.id),await c(),a(`Bienvenue ${n} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch($){g.textContent=$.message,w.disabled=!1,w.textContent="🚀 Créer mon profil !"}})}async function Sn(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await E.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const In="modulepreload",Mn=function(e){return"/manager-wars/"+e},Ti={},Vi=function(t,i,a){let c=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const s=document.querySelector("meta[property=csp-nonce]"),d=(s==null?void 0:s.nonce)||(s==null?void 0:s.getAttribute("nonce"));c=Promise.allSettled(i.map(u=>{if(u=Mn(u),u in Ti)return;Ti[u]=!0;const n=u.endsWith(".css"),o=n?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${u}"]${o}`))return;const g=document.createElement("link");if(g.rel=n?"stylesheet":In,n||(g.as="script"),g.crossOrigin="",g.href=u,d&&g.setAttribute("nonce",d),document.head.appendChild(g),n)return new Promise((w,$)=>{g.addEventListener("load",w),g.addEventListener("error",()=>$(new Error(`Unable to preload CSS for ${u}`)))})}))}function l(s){const d=new Event("vite:preloadError",{cancelable:!0});if(d.payload=s,window.dispatchEvent(d),!d.defaultPrevented)throw s}return c.then(s=>{for(const d of s||[])d.status==="rejected"&&l(d.reason);return t().catch(l)})},Ft="#1A6B3C",Bt="#cc2222",jn="#D4A017",zi="#888";async function Cn(e,t){const{state:i,toast:a}=t;e.innerHTML=`
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      ${w.map(({friendshipId:$,friend:z})=>qn(z,$)).join("")}
    </div>`,n.querySelectorAll("[data-stats]").forEach($=>{$.addEventListener("click",()=>Bn(e,$.dataset.stats,$.dataset.friendName))}),n.querySelectorAll("[data-match]").forEach($=>{$.addEventListener("click",()=>{const z=$.dataset.friendId,r=$.dataset.friendName;console.log("[Friends] clic match",{fid:z,fname:r,hasNavigate:typeof a}),typeof a=="function"?a("match",{matchMode:"friend",friendId:z,friendName:r}):t("Erreur navigation","error")})})}function qn(e,t){const i=e.club_name||e.pseudo||"?",a=e.pseudo||"",c=(a||i).slice(0,2).toUpperCase(),l=e.club_color2||Ft,s=e.club_color1||"#ffffff",d=e.last_seen_at?new Date(e.last_seen_at):null,u=d&&Date.now()-d.getTime()<3*60*1e3;return`
    ...
    <div class="popup-box">
      ...
      ${o.length?`<div style="display:flex;flex-direction:column;gap:8px;max-height:50vh;overflow-y:auto;margin-bottom:14px">
            ${o.map(w=>{var $,z,r;return`
              <div style="display:flex;align-items:center;gap:10px;background:#f9f9f9;border-radius:10px;padding:10px 12px">
                <div style="flex:1;font-size:14px;font-weight:700">${(($=w.requester)==null?void 0:$.club_name)||((z=w.requester)==null?void 0:z.pseudo)||"?"}
    ...
    </style>`}async function Wi({player1Id:e,player2Id:t,score1:i,score2:a,gc1:c,gc2:l}){const[s,d]=[e,t].sort(),u=e!==s,n=u?a:i,o=u?i:a,g=u?l:c,w=u?c:l,$=n>o?1:0,z=o>n?1:0,r=n===o?1:0,{data:p}=await E.from("friend_match_stats").select("*").eq("player1_id",s).eq("player2_id",d).single();p?await E.from("friend_match_stats").update({wins_p1:p.wins_p1+$,wins_p2:p.wins_p2+z,draws:p.draws+r,goals_p1:p.goals_p1+n,goals_p2:p.goals_p2+o,gc_used_p1:p.gc_used_p1+g,gc_used_p2:p.gc_used_p2+w,matches_total:p.matches_total+1}).eq("player1_id",s).eq("player2_id",d):await E.from("friend_match_stats").insert({player1_id:s,player2_id:d,wins_p1:$,wins_p2:z,draws:r,goals_p1:n,goals_p2:o,gc_used_p1:g,gc_used_p2:w,matches_total:1})}const Dn="2026.07.02-0021";async function Li(e,{state:t,navigate:i,toast:a}){var l,s;const c=t.profile;c&&(e.innerHTML=`
  <style>
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),n=(d||[]).filter(J=>J.card_type==="player"&&J.player),o=(d||[]).filter(J=>J.card_type==="game_changer"),g=(d||[]).filter(J=>J.card_type==="formation"),{data:w}=await E.from("gc_definitions").select("name,gc_type,color,effect,image_url"),$={};(w||[]).forEach(J=>{$[J.name]=J});const z=Object.keys(mi),r=Object.keys(Ve),p={};n.forEach(J=>{const Z=J.player.id;p[Z]=(p[Z]||0)+1}),new Set(Object.keys(p).map(J=>String(J)));const S=new Set(g.map(J=>J.formation)),M=new Set(o.map(J=>J.gc_type));let F="player",O="Tous",G="",q=!1;function H(){return[...n].sort((J,Z)=>{const N=Ot.indexOf(J.player.job),R=Ot.indexOf(Z.player.job);return N!==R?N-R:(J.player.surname_encoded||"").localeCompare(Z.player.surname_encoded||"")})}function ie(){return[...u||[]].sort((J,Z)=>{const N=Ot.indexOf(J.job),R=Ot.indexOf(Z.job);return N!==R?N-R:(J.surname_encoded||"").localeCompare(Z.surname_encoded||"")})}function ue(){return H().filter(J=>{const Z=J.player,N=O==="Tous"||Z.job===O,R=!G||`${Z.firstname} ${Z.surname_encoded}`.toLowerCase().includes(G);return N&&R})}function se(){return ie().filter(J=>{const Z=O==="Tous"||J.job===O,N=!G||`${J.firstname} ${J.surname_encoded}`.toLowerCase().includes(G);return Z&&N})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",Z=>{G=Z.target.value.toLowerCase(),ne()}),e.querySelectorAll(".filter-btn").forEach(Z=>{Z.addEventListener("click",()=>{O=Z.dataset.job,B(),ne()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{q=!q,B(),ne()})):(J.innerHTML=`
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Ht=1e3;function Jn(e,t,i,a){var z,r,p;const{state:c,toast:l,closeModal:s,navigate:d,refreshProfile:u}=i,n=e.formation,o={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function g(){const S=Gt[n]||{},M=mi[n]||[],F=290,O=360,G=20;function q(ie){const ue=S[ie];return ue?{x:ue.x*F,y:ue.y*O}:null}let H=`<svg width="${F}" height="${O}" viewBox="0 0 ${F} ${O}" xmlns="http://www.w3.org/2000/svg">`;for(const[ie,ue]of M){const se=q(ie),B=q(ue);!se||!B||(H+=`<line x1="${se.x}" y1="${se.y}" x2="${B.x}" y2="${B.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const ie of Object.keys(S)){const ue=q(ie);if(!ue)continue;const se=ie.replace(/\d+/,""),B=o[se]||"#555";H+=`<circle cx="${ue.x}" cy="${ue.y}" r="${G}" fill="${B}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,H+=`<text x="${ue.x}" y="${ue.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${se}</text>`}return H+="</svg>",H}const w=t.filter(S=>S.formation===n);w.length;const $=!e.is_for_sale;a(`Formation ${n}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(z=document.getElementById("direct-sell-form-btn"))==null||z.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${n} pour ${Ht.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const S=w.find(F=>!F.is_for_sale)||w[0];if(!S){l("Aucune carte à vendre","error");return}await E.from("market_listings").delete().eq("card_id",S.id),await E.from("transfer_history").delete().eq("card_id",S.id);const{error:M}=await E.from("cards").delete().eq("id",S.id);if(M){l(M.message,"error");return}await E.from("users").update({credits:(c.profile.credits||0)+Ht}).eq("id",c.profile.id),await u(),l(`+${Ht.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),s(),d("collection")}),(r=document.getElementById("market-sell-form-btn"))==null||r.addEventListener("click",async()=>{const S=parseInt(document.getElementById("sell-price-form").value);if(!S||S<1){l("Prix invalide","error");return}await E.from("cards").update({is_for_sale:!0,sale_price:S}).eq("id",e.id),await E.from("market_listings").insert({seller_id:c.profile.id,card_id:e.id,price:S}),l("Carte mise en vente sur le marché !","success"),s(),d("collection")}),(p=document.getElementById("cancel-sell-form-btn"))==null||p.addEventListener("click",async()=>{await E.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await E.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),l("Annonce retirée","success"),s(),d("collection")})}async function Ci(e,t,i,a){var fe,ye,Ae,ve,he,J,Z;const{state:c,toast:l,openModal:s,closeModal:d,navigate:u,refreshProfile:n}=a,o=e.player,g=t.filter(N=>N.player.id===o.id),w=g.length,$=Math.max(Number(o.note_g)||0,Number(o.note_d)||0,Number(o.note_m)||0,Number(o.note_a)||0),z=o.rarity||"normal",{data:r}=await E.from("sell_price_configs").select("*").eq("rarity",z).lte("note_min",$).gte("note_max",$).order("note_min",{ascending:!1}).limit(1),p=((fe=r==null?void 0:r[0])==null?void 0:fe.price)??Vn[z]??1e3,S=o.rarity!=="legende",M=on(o),F=(o.rarity==="pepite"||o.rarity==="papyte")&&e.current_note!=null?e.current_note:Dt(o,o.job),O=o.rarity==="pepite"||o.rarity==="papyte",G=o.job2?O?hi(e,nn(o.job2)):Dt(o,o.job2):null,q=Ct[o.job]||"#1A6B3C",H=o.job2?Ct[o.job2]:null,ie=tn[o.rarity]||"#ccc",ue=vi[o.country_code]||o.country_code||"",se=g.map(N=>N.id);let B={};if(se.length){const{data:N}=await E.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",se).order("transferred_at",{ascending:!0});(N||[]).forEach(R=>{B[R.card_id]||(B[R.card_id]=[]),B[R.card_id].push(R)})}const ne=N=>{const R=N.transferred_at?new Date(N.transferred_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"2-digit",year:"numeric"}):"",Q=N.source==="booster"?"Booster":N.price?N.price.toLocaleString("fr"):"—";return`
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(ve=document.getElementById("close-detail"))==null||ve.addEventListener("click",d);let P=new Set;const oe=()=>{const N=P.size,R=document.getElementById("sell-action-panel");R&&(R.style.display=N>0?"block":"none",document.getElementById("sell-selected-count").textContent=N,document.getElementById("sell-direct-total").textContent=(N*p).toLocaleString("fr")+" cr.")};document.querySelectorAll(".expl-check").forEach(N=>{N.addEventListener("change",()=>{const R=N.dataset.id;N.checked?P.add(R):P.delete(R);const Q=N.closest(".exemplaire-row");Q&&(Q.style.borderColor=N.checked?"#1A6B3C":"#eee"),oe()})}),document.querySelectorAll(".exemplaire-row").forEach(N=>{N.addEventListener("click",R=>{if(R.target.closest("button")||R.target.tagName==="INPUT")return;const Q=N.querySelector(".expl-check");Q&&!Q.disabled&&(Q.checked=!Q.checked,Q.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(N=>{N.addEventListener("click",R=>{R.stopPropagation();const Q=document.querySelector(`.expl-hist[data-hist="${N.dataset.idx}"]`);Q&&(Q.style.display=Q.style.display==="none"?"flex":"none")})}),(he=document.getElementById("direct-sell-btn"))==null||he.addEventListener("click",async()=>{const N=[...P];if(!N.length)return;const R=N.length*p;if(!confirm(`Vendre ${N.length} carte${N.length>1?"s":""} ${o.surname_encoded} pour ${R.toLocaleString("fr")} crédits ? Action irréversible.`))return;await E.from("market_listings").delete().in("card_id",N),await E.from("transfer_history").delete().in("card_id",N);const{error:Q}=await E.from("cards").delete().in("id",N);if(Q){l(Q.message,"error");return}await E.from("users").update({credits:(c.profile.credits||0)+R}).eq("id",c.profile.id),await n();const pe=document.getElementById("nav-credits");pe&&(pe.textContent=`💰 ${(c.profile.credits||0).toLocaleString("fr")}`),l(`+${R.toLocaleString("fr")} crédits ! ${N.length} carte${N.length>1?"s":""} vendue${N.length>1?"s":""}.`,"success"),d(),u("collection")}),(J=document.getElementById("market-sell-btn"))==null||J.addEventListener("click",async()=>{var je;const N=[...P];if(!N.length){l("Sélectionne au moins un exemplaire","warning");return}const R=parseInt((je=document.getElementById("sell-market-price"))==null?void 0:je.value);if(!R||R<1){l("Prix invalide","error");return}const{error:Q}=await E.from("cards").update({is_for_sale:!0,sale_price:R}).in("id",N);if(Q){l(Q.message,"error");return}const pe=N.map(Se=>({seller_id:c.profile.id,card_id:Se,price:R,status:"active"})),{error:le}=await E.from("market_listings").insert(pe);le&&console.warn("[Market] insert listings:",le.message),l(`${N.length} carte${N.length>1?"s":""} mise${N.length>1?"s":""} en vente à ${R.toLocaleString("fr")} cr. chacune !`,"success"),d(),u("collection")}),(Z=document.getElementById("cancel-sell-btn"))==null||Z.addEventListener("click",async()=>{await E.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await E.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),l("Annonce retirée","success"),d(),u("collection")})}const Wt={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},wt={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function ei(e){const t=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function rn(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function an(e){var i;const t="https://fcnwclxlkytdfjotqkta.supabase.co";return(i=e==null?void 0:e.clubs)!=null&&i.logo_url?e.clubs.logo_url.startsWith("http")?e.clubs.logo_url:`${t}/storage/v1/object/public/assets/clubs/${e.clubs.logo_url}`:null}function Xn(e,t=44,i=58){var $;const a=e?ei(e):null,c=e?an(e):null,l=rn(e==null?void 0:e.country_code),s=(e==null?void 0:e.job)||"MIL",d=wt[s]||"#555",u={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",n=Number(s==="GK"?e==null?void 0:e.note_g:s==="DEF"?e==null?void 0:e.note_d:s==="MIL"?e==null?void 0:e.note_m:e==null?void 0:e.note_a)||0,o=Math.round(i*.19),g=Math.round(i*.25),w=i-o-g;return e?`<div style="width:${t}px;height:${i}px;border-radius:5px;border:2px solid ${u};background:${d};position:relative;overflow:hidden;flex-shrink:0">
  ...
  </div>`,Zn(e,t,c,i),document.getElementById("builder-back").addEventListener("click",()=>a("decks")),document.getElementById("formation-select").addEventListener("change",n=>{t.formation=n.target.value;const o=Fi(t.formation),g={};o.forEach(w=>{t.slots[w]&&(g[w]=t.slots[w])}),t.slots=g,zt(e,t,i)}),document.getElementById("save-deck").addEventListener("click",()=>to(t,i)),e.querySelectorAll("[data-remove-sub]").forEach(n=>{n.addEventListener("click",()=>{t.subs=t.subs.filter(o=>o!==n.dataset.removeSub),zt(e,t,i)})}),(u=document.getElementById("add-sub-btn"))==null||u.addEventListener("click",()=>{eo(t,e,i)})}function Zn(e,t,i,a){const c=e.querySelector("#deck-field");if(!c)return;const l=Gt[t.formation]||{},s=Ui(t.formation)||[],d={};for(const S of i){const M=t.slots[S],F=M?t.playerCards.find(O=>O.id===M):null;d[S]=F?F.player:null}const u=300,n=300,o=48,g=64,w=13,$=16,z=38;function r(S){const M=l[S];return M?{x:M.x*u,y:M.y*n}:null}let p="";for(const[S,M]of s){const F=r(S),O=r(M);if(!F||!O)continue;const G=d[S]?{...d[S],club_id:d[S].club_id,country_code:d[S].country_code,rarity:d[S].rarity}:null,q=d[M]?{...d[M],club_id:d[M].club_id,country_code:d[M].country_code,rarity:d[M].rarity}:null,H=at(G,q);H==="#ff3333"||H==="#cc2222"?p+=`<line x1="${F.x.toFixed(1)}" y1="${F.y.toFixed(1)}" x2="${O.x.toFixed(1)}" y2="${O.y.toFixed(1)}" stroke="${H}" stroke-width="2.5" stroke-linecap="round" opacity="0.4"/>`:(p+=`<line x1="${F.x.toFixed(1)}" y1="${F.y.toFixed(1)}" x2="${O.x.toFixed(1)}" y2="${O.y.toFixed(1)}" stroke="${H}" stroke-width="8" stroke-linecap="round" opacity="0.2"/>`,p+=`<line x1="${F.x.toFixed(1)}" y1="${F.y.toFixed(1)}" x2="${O.x.toFixed(1)}" y2="${O.y.toFixed(1)}" stroke="${H}" stroke-width="2.5" stroke-linecap="round" opacity="0.9"/>`)}for(const S of i){const M=r(S);if(!M)continue;const F=d[S],O=S.replace(/\d+/,""),G=wt[O]||"#555",q=(M.x-o/2).toFixed(1),H=(M.y-g/2).toFixed(1),ie={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[F==null?void 0:F.rarity]||"#aaa";if(F){const ue=ei(F),se=an(F),B=rn(F.country_code),ne=Number(O==="GK"?F.note_g:O==="DEF"?F.note_d:O==="MIL"?F.note_m:F.note_a)||0,W=g-w-$;p+=`<defs><clipPath id="dcp-${S}"><rect x="${q}" y="${(M.y-g/2+w).toFixed(1)}" width="${o}" height="${W}"/></clipPath></defs>`,p+=`<rect x="${q}" y="${H}" width="${o}" height="${g}" rx="5" fill="${G}"/>`,ue&&(p+=`<image href="${ue}" x="${q}" y="${(M.y-g/2+w).toFixed(1)}" width="${o}" height="${W}" clip-path="url(#dcp-${S})" preserveAspectRatio="xMidYMin slice"/>`),p+=`<rect x="${q}" y="${H}" width="${o}" height="${w}" fill="rgba(255,255,255,0.93)"/>`,p+=`<text x="${M.x.toFixed(1)}" y="${(M.y-g/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(F.surname_encoded||"").slice(0,9)}</text>`;const P=(M.y+g/2-$).toFixed(1);p+=`<rect x="${q}" y="${P}" width="${o}" height="${$}" fill="rgba(255,255,255,0.93)"/>`,B&&(p+=`<image href="${B}" x="${(M.x-21).toFixed(1)}" y="${(M.y+g/2-$+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`),p+=`<text x="${M.x.toFixed(1)}" y="${(M.y+g/2-$/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${ne}</text>`,se&&(p+=`<image href="${se}" x="${(M.x+o/2-14).toFixed(1)}" y="${(M.y+g/2-$+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`),p+=`<rect x="${q}" y="${H}" width="${o}" height="${g}" rx="5" fill="none" stroke="${ie}" stroke-width="2"/>`}else p+=`<rect x="${q}" y="${H}" width="${o}" height="${g}" rx="5" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="5,3"/>`,p+=`<text x="${M.x.toFixed(1)}" y="${M.y.toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="22" fill="rgba(255,255,255,0.35)">+</text>`,p+=`<text x="${M.x.toFixed(1)}" y="${(M.y+16).toFixed(1)}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.3)">${O}</text>`;p+=`<rect x="${q}" y="${H}" width="${o}" height="${g}" rx="5" fill="rgba(0,0,0,0.01)" class="deck-slot-hit" data-pos="${S}" style="cursor:pointer"/>`}c.innerHTML=`<svg viewBox="${-z} ${-z} ${u+z*2} ${n+z*2}" width="100%" style="display:block;max-width:440px;margin:0 auto">${p}</svg>`,c.querySelectorAll(".deck-slot-hit").forEach(S=>{S.addEventListener("click",()=>Qn(S.dataset.pos,t,e,a))})}function Qn(e,t,i,a){var w,$,z;const{openModal:c,closeModal:l}=a,s=e.replace(/\d+/,""),d=t.slots[e],u=d?t.playerCards.find(r=>r.id===d):null;(w=u==null?void 0:u.player)==null||w.id;const n=new Set;Object.entries(t.slots).forEach(([r,p])=>{var M;if(r===e||!p)return;const S=t.playerCards.find(F=>F.id===p);(M=S==null?void 0:S.player)!=null&&M.id&&n.add(S.player.id)}),t.subs.forEach(r=>{var S;const p=t.playerCards.find(M=>M.id===r);(S=p==null?void 0:p.player)!=null&&S.id&&n.add(p.player.id)});const o=new Set,g=t.playerCards.filter(r=>{const p=r.player;return!(p.job===s||p.job2===s)||n.has(p.id)||o.has(p.id)?!1:(o.add(p.id),!0)});g.sort((r,p)=>{const S=s==="GK"?r.player.note_g:s==="DEF"?r.player.note_d:s==="MIL"?r.player.note_m:r.player.note_a;return(s==="GK"?p.player.note_g:s==="DEF"?p.player.note_d:s==="MIL"?p.player.note_m:p.player.note_a)-S}),c(`Choisir ${s} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${g.length>0?g.map(r=>{var O,G;const p=r.player,S=s==="GK"?p.note_g:s==="DEF"?p.note_d:s==="MIL"?p.note_m:p.note_a,M=ei(p),F={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[p.rarity];return`<div class="player-option" data-card-id="${r.id}"
          ...
          <div style="width:44px;height:44px;border-radius:8px;overflow:hidden;flex-shrink:0;background:#dde;border:2px solid ${wt[s]}">
            ${M?`<img src="${M}" style="width:100%;height:100%;object-fit:cover">`:`<div style="width:100%;height:100%;background:${wt[s]};display:flex;align-items:center;justify-content:center;color:#fff;font-size:10px;font-weight:700">${s}</div>`}
          </div>
          <!-- Infos -->
          <div style="flex:1;min-width:0">
          ...
          <!-- Note -->
          <div style="width:36px;height:36px;border-radius:8px;background:${wt[s]};color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:16px;border:2px solid ${F};flex-shrink:0">
            ${S}
          </div>
        </div>`}).join(""):'<div style="text-align:center;color:var(--gray-600);padding:20px">Aucun joueur pour ce poste.<br><small>Ouvre des boosters !</small></div>'}
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),($=document.getElementById("close-selector"))==null||$.addEventListener("click",l),(z=document.getElementById("remove-player"))==null||z.addEventListener("click",()=>{delete t.slots[e],l(),zt(i,t,a)}),document.querySelectorAll(".player-option").forEach(r=>{r.addEventListener("click",()=>{t.slots[e]=r.dataset.cardId,l(),zt(i,t,a)})})}function eo(e,t,i){var u;const{openModal:a,closeModal:c}=i,l=new Set;Object.values(e.slots).filter(Boolean).forEach(n=>{var g;const o=e.playerCards.find(w=>w.id===n);(g=o==null?void 0:o.player)!=null&&g.id&&l.add(o.player.id)}),e.subs.forEach(n=>{var g;const o=e.playerCards.find(w=>w.id===n);(g=o==null?void 0:o.player)!=null&&g.id&&l.add(o.player.id)});const s=new Set,d=e.playerCards.filter(n=>{var o,g,w;return l.has((o=n.player)==null?void 0:o.id)||s.has((g=n.player)==null?void 0:g.id)?!1:(s.add((w=n.player)==null?void 0:w.id),!0)});a("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
  ...
  `,document.body.appendChild(c);let l=!1;const s=document.getElementById("pack-cut-zone"),d=document.getElementById("pack-blade");let u=!1;const n=P=>P.touches&&P.touches[0]?P.touches[0].clientX:P.clientX;function o(P){l||(u=!0,d.style.opacity="1",g(P))}function g(P){if(!u||l)return;const oe=s.getBoundingClientRect(),fe=n(P)-oe.left,ye=Math.max(0,Math.min(1,fe/oe.width));d.style.width=ye*oe.width+"px",ye>=.82&&$()}function w(){l||(u=!1,d.style.transition="width .2s ease, opacity .2s ease",d.style.width="0",d.style.opacity="0",setTimeout(()=>{l||(d.style.transition="")},220))}function $(){var oe;if(l)return;l=!0,u=!1,d.style.width="100%",d.style.opacity="1",(oe=document.getElementById("cut-flash"))==null||oe.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const P=document.getElementById("cut-hint");P&&(P.style.opacity="0"),s.classList.add("pack-cut"),setTimeout(()=>{d.style.opacity="0",document.getElementById("pack-phase").style.display="none",S(0)},620)}s.addEventListener("pointerdown",o),window.addEventListener("pointermove",g),window.addEventListener("pointerup",w),s.addEventListener("touchstart",o,{passive:!0}),window.addEventListener("touchmove",g,{passive:!0}),window.addEventListener("touchend",w);let z=0,r=!1;const p=new Set;function S(P){z=P,document.getElementById("reveal-phase").style.display="flex",M(),F(P,0),ie()}function M(){const P=document.getElementById("card-dots");P&&(P.innerHTML=e.map((oe,fe)=>`<div class="card-dot" data-i="${fe}" style="width:8px;height:8px;border-radius:50%;background:${fe===z?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),P.querySelectorAll(".card-dot").forEach(oe=>oe.addEventListener("click",()=>G(parseInt(oe.dataset.i)))))}function F(P,oe){var R;const fe=e[P],ye=document.getElementById("card-counter"),Ae=document.getElementById("card-track");ye&&(ye.textContent=`Carte ${P+1} / ${e.length}`);const ve=document.getElementById("reveal-btns");ve&&(ve.style.display=P===e.length-1?"flex":"none");const he=fe.card_type==="player"&&((R=fe.player)==null?void 0:R.rarity)==="legende",J=!p.has(P);p.add(P);let Z=0;if(fe.card_type==="player"&&fe.player){const Q=fe.player,pe=Q.job||"ATT";Z=Number(pe==="GK"?Q.note_g:pe==="DEF"?Q.note_d:pe==="MIL"?Q.note_m:Q.note_a)||0}const N=Q=>{Ae.innerHTML=`
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      <div style="flex:1;overflow-y:auto;display:flex;flex-wrap:wrap;gap:10px;justify-content:center;align-content:flex-start;padding:8px 16px 16px">
        ${l.map(z=>{const r=s.find(p=>p.gc_type===z.gc_type);return d(z,!!r)}).join("")}
      </div>
    ...
    </div>`,e.querySelectorAll(".gc-select-card").forEach(z=>{z.addEventListener("click",()=>{const r=z.dataset.id,p=l.find(M=>M.id===r);if(!p)return;const S=s.findIndex(M=>M.gc_type===p.gc_type);S>-1?s.splice(S,1):s.length<3&&s.push(p),n()})}),(g=e.querySelector("#gc-launch"))==null||g.addEventListener("click",()=>{o&&u(s)}),(w=e.querySelector("#gc-no-gc"))==null||w.addEventListener("click",()=>u([])),($=e.querySelector("#gc-reset"))==null||$.addEventListener("click",()=>{s.length&&(s=[],n())})}n()}function _o(e){var a;const t=((a=e==null?void 0:e.state)==null?void 0:a.params)||{},i=t.matchMode||"vs_ai_easy";return i==="friend"?`Match vs ${t.friendName||"Ami"}`:i==="random"?"Match vs Random":`Match vs IA — ${i.replace("vs_ai_","").toUpperCase()}`}async function $o(e,t,i){const{state:a,navigate:c}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:l}=await E.from("decks").select("id,name,is_active,formation").eq("owner_id",a.profile.id).order("created_at",{ascending:!1});if(!l||l.length===0){_t(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>c("decks"));return}const s=l.map(o=>o.id),{data:d}=await E.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",s).order("slot_order");let u=0;function n(){var M,F,O,G,q;const o=l[u],g=(d||[]).filter(H=>H.deck_id===o.id),w=g.filter(H=>{var ie;return H.is_starter&&((ie=H.card)==null?void 0:ie.player)}).map(H=>pi(H.card,H.position)),$=g.find(H=>{var ie;return((ie=H.card)==null?void 0:ie.card_type)==="formation"}),z=o.formation||((M=$==null?void 0:$.card)==null?void 0:M.formation)||"4-4-2",r=w.length>=11?yt(w,z):null,p=w.length>=11;Qe(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div style="display:flex;align-items:center;gap:8px;padding:8px;flex-shrink:0">
        ...
        <div style="flex:1;text-align:center">
          ...
          <div style="font-size:11px;opacity:.6;margin-top:2px">${z} · ${w.length}/11 ${o.is_active?"· ⭐ Actif":""}</div>
        </div>
      ...
      <div id="deck-swipe-zone" style="flex:1;min-height:0;overflow:hidden;position:relative;touch-action:pan-y;display:flex;align-items:center;justify-content:center;padding:4px">
        ${r?`<div class="deck-preview-wrap" style="aspect-ratio:1/1;max-width:100%;max-height:100%;width:auto;height:100%;overflow:hidden">${ut(r,z,null,[],285,285)}</div>`:`<div style="display:flex;align-items:center;justify-content:center;height:100%;opacity:.4;flex-direction:column;gap:8px">
  ...
  </div>`}function nt(e,t,i){if(!(e!=null&&e.length))return"";const a=e.slice(0,5);let c='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return a.forEach((l,s)=>{if(c+=We(l,40,52),s<a.length-1){const d=at(l,a[s+1]);c+=`<div style="width:7px;height:3px;background:${d==="#ff3333"||d==="#cc2222"?"rgba(255,255,255,0.12)":d};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(c+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),c+="</div>",c}function Jt(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function xn(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function pt(e,t,i,a,c=310,l=310,s=[]){var F;const d=Gt[t]||{},u=Ui(t)||mi[t]||[],n={},o=["ATT","MIL","DEF","GK"];for(const O of o)(e[O]||[]).forEach((q,H)=>{n[`${O}${H+1}`]=q});function g(O){const G=d[O];return G?{x:G.x*c,y:G.y*l}:null}let w="";for(const[O,G]of u){const q=g(O),H=g(G);if(!q||!H)continue;const ie=n[O],ue=n[G],se=at(ie,ue);se==="#00ff88"||se==="#FFD700"?(w+=`<line x1="${q.x.toFixed(1)}" y1="${q.y.toFixed(1)}" x2="${H.x.toFixed(1)}" y2="${H.y.toFixed(1)}"
        stroke="${se}" stroke-width="10" stroke-linecap="round" opacity="0.22"/>`,w+=`<line x1="${q.x.toFixed(1)}" y1="${q.y.toFixed(1)}" x2="${H.x.toFixed(1)}" y2="${H.y.toFixed(1)}"
        stroke="${se}" stroke-width="3.5" stroke-linecap="round" opacity="0.95"/>`):w+=`<line x1="${q.x.toFixed(1)}" y1="${q.y.toFixed(1)}" x2="${H.x.toFixed(1)}" y2="${H.y.toFixed(1)}"
        stroke="${se}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const $=48,z=64,r=13,p=16,S={normal:"#aaaaaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#111111",legende:"#7a28b8",légende:"#7a28b8"};for(const[O,G]of Object.entries(n)){const q=g(O);if(!q||!G)continue;const H=O.replace(/[0-9]/g,""),ie=St[H]||"#555",ue=s.includes(G.cardId),se=i==="attack"&&(["MIL","ATT"].includes(H)||ue)&&!G.used||i==="defense"&&["GK","DEF","MIL"].includes(H)&&!G.used,B=a.includes(G.cardId);let ne;i==="attack"?ne=ue?Math.max(1,Number(G.note_a)||0):H==="MIL"?Number(G.note_m)||0:Number(G.note_a)||0:i==="defense"?ne=H==="GK"?Number(G.note_g)||0:H==="MIL"?Number(G.note_m)||0:Number(G.note_d)||0:ne=Number(H==="GK"?G.note_g:H==="DEF"?G.note_d:H==="MIL"?G.note_m:G.note_a)||0,ne=ne+(G.boost||0);const W=(q.x-$/2).toFixed(1),P=(q.y-z/2).toFixed(1),oe=S[G==null?void 0:G.rarity]||S.normal;if(G.used){const Z=typeof import.meta<"u"&&"/manager-wars/"||"/",R=`${typeof window<"u"&&((F=window.location)==null?void 0:F.origin)||""}${Z}icons/carte-dos.png`.replace(/([^:])\/\//g,"$1/");w+=`<rect x="${W}" y="${P}" width="${$}" height="${z}" rx="5" fill="#161616"/>`,w+=`<image href="${R}" xlink:href="${R}" x="${W}" y="${P}" width="${$}" height="${z}" preserveAspectRatio="xMidYMid slice"/>`,w+=`<rect x="${W}" y="${P}" width="${$}" height="${z}" rx="5" fill="none" stroke="${oe}" stroke-width="2" opacity="0.7"/>`,w+=`<rect x="${W}" y="${P}" width="${$}" height="${z}" rx="5" fill="rgba(0,0,0,0.01)" class="match-used-hit" data-card-id="${G.cardId}" data-role="${H}" style="cursor:pointer"/>`;continue}const fe=B?.45:1,ye=B?"#FFD700":oe,Ae=B?3:(G==null?void 0:G.rarity)==="legende"||(G==null?void 0:G.rarity)==="légende"||(G==null?void 0:G.rarity)==="pepite"||(G==null?void 0:G.rarity)==="pépite"?2.5:2,ve=z-r-p;w+=`<defs><clipPath id="cp-${O}"><rect x="${W}" y="${(q.y-z/2+r).toFixed(1)}" width="${$}" height="${ve}"/></clipPath></defs>`,w+=`<rect x="${W}" y="${P}" width="${$}" height="${z}" rx="5" fill="${ie}" opacity="${fe}"/>`;const he=Ge(G);he&&(w+=`<image href="${he}" xlink:href="${he}" x="${W}" y="${(q.y-z/2+r).toFixed(1)}" width="${$}" height="${ve}" clip-path="url(#cp-${O})" preserveAspectRatio="xMidYMin slice"/>`),w+=`<rect x="${W}" y="${P}" width="${$}" height="${r}" rx="4" fill="rgba(255,255,255,0.92)"/>`,w+=`<text x="${q.x.toFixed(1)}" y="${(q.y-z/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(G.name||"").slice(0,9)}</text>`;const J=(q.y+z/2-p).toFixed(1);w+=`<rect x="${W}" y="${J}" width="${$}" height="${p}" fill="rgba(255,255,255,0.92)"/>`;{const Z=Jt(G.country_code);Z?w+=`<image href="${Z}" xlink:href="${Z}" x="${(q.x-20).toFixed(1)}" y="${(q.y+z/2-p+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:w+=`<text x="${(q.x-13).toFixed(1)}" y="${(q.y+z/2-p/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${xn(G.country_code)}</text>`,w+=`<text x="${q.x.toFixed(1)}" y="${(q.y+z/2-p/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${ne}</text>`;const N=mn(G);N?w+=`<image href="${N}" xlink:href="${N}" x="${(q.x+$/2-14).toFixed(1)}" y="${(q.y+z/2-p+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:G.clubName&&(w+=`<text x="${(q.x+14).toFixed(1)}" y="${(q.y+z/2-p/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(G.clubName||"").slice(0,3).toUpperCase()}</text>`),G.boost&&(w+=`<rect x="${(q.x+$/2-12).toFixed(1)}" y="${(q.y-z/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,w+=`<text x="${(q.x+$/2-5).toFixed(1)}" y="${(q.y-z/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${G.boost}</text>`)}w+=`<rect x="${W}" y="${P}" width="${$}" height="${z}" rx="5" fill="${B?"rgba(255,255,255,0.12)":"none"}" stroke="${ye}" stroke-width="${Ae}" opacity="${fe}"/>`,se&&(w+=`<rect x="${W}" y="${P}" width="${$}" height="${z}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${B?"selected":""}" data-card-id="${G.cardId}" data-role="${H}" style="cursor:pointer"/>`)}const M=38;return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-M} ${-M} ${c+M*2} ${l+M*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${w}
  ...
  <div style="text-align:center;flex-shrink:0;width:38px">
    ...
    <div style="font-size:7px;color:rgba(255,255,255,0.5);margin-top:1px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${(e.name||"").slice(0,7)}</div>
          ...
          card:cards(id, card_type, formation,
            player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
              clubs(encoded_name,logo_url)))`).eq("deck_id",u).order("slot_order")]);n=q[0].data,g=q[0].error,o=q[1].data,w=q[1].error}catch(q){console.error("[Match] Exception chargement deck:",q),_t(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>l("home"));return}if(g||w){console.error("[Match] Erreur Supabase:",g||w),_t(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>l("home"));return}const $=(o||[]).filter(q=>{var H;return q.is_starter&&((H=q.card)==null?void 0:H.player)}).map(q=>pi(q.card,q.position)),z=(o||[]).filter(q=>{var H;return!q.is_starter&&((H=q.card)==null?void 0:H.player)}).map(q=>pi(q.card,q.position));if($.length<11){_t(e,"⚠️",`Deck incomplet (${$.length}/11).`,"Compléter",()=>l("decks"));return}const r=(o||[]).find(q=>{var H;return((H=q.card)==null?void 0:H.card_type)==="formation"}),p=(n==null?void 0:n.formation)||((G=r==null?void 0:r.card)==null?void 0:G.formation)||"4-4-2",{data:S,error:M}=await E.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",c.profile.id).eq("card_type","game_changer"),{data:F}=await E.from("gc_definitions").select("*").eq("is_active",!0),O=(S||[]).map(q=>({...q,_gcDef:(F==null?void 0:F.find(H=>H.name===q.gc_type||H.id===q.gc_definition_id))||null}));a({deckId:u,formation:p,starters:$,subsRaw:z,gcCardsEnriched:O,gcDefs:F||[]})}async function ko(e,t){const{state:i}=t,c=(i.params||{}).matchMode||"vs_ai_easy",l=c.replace("vs_ai_",""),s=c;await ni(e,t,c,async({deckId:d,formation:u,starters:n,subsRaw:o,gcCardsEnriched:g,gcDefs:w})=>{try{const $=yt(n,u),z=await Eo(u,l),r=async p=>{try{const{data:S,error:M}=await E.from("matches").insert({home_id:i.profile.id,away_id:null,mode:s,home_deck_id:d,status:"in_progress"}).select().single();if(M){console.error("[MatchIA] Erreur création match:",M),_t(e,"⚠️","Impossible de créer le match ("+M.message+").","Retour",()=>t.navigate("home"));return}const F={gcDefs:w||[],matchId:S==null?void 0:S.id,mode:s,difficulty:l,formation:u,homeTeam:$,aiTeam:z,homeSubs:o,subsUsed:0,maxSubs:Math.min(o.length,3),homeScore:0,aiScore:0,gcCards:p,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};To(e,F,t)}catch(S){console.error("[MatchIA] Exception launchMatch:",S),_t(e,"⚠️","Erreur au lancement du match : "+S.message,"Retour",()=>t.navigate("home"))}};if(!g.length){r([]);return}ii(e,g,r)}catch($){console.error("[MatchIA] Exception setup:",$),_t(e,"⚠️","Erreur de préparation du match : "+$.message,"Retour",()=>t.navigate("home"))}})}async function Eo(e,t){var s,d;const{data:i}=await E.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(60);if(!i||i.length<11)return Ao(e);const a=Lt[e]||Lt["4-4-2"],c={GK:[],DEF:[],MIL:[],ATT:[]},l=[...i];for(const u of["GK","DEF","MIL","ATT"]){const n=l.filter(z=>z.job===u),o=l.filter(z=>z.job!==u),g=[...n,...o],w=[];for(let z=0;z<a[u];z++){const r=g[z]||l[z];r&&w.push({cardId:"ai-"+r.id+"-"+z,id:r.id,firstname:r.firstname,name:r.surname_encoded,country_code:r.country_code,club_id:r.club_id,job:r.job,job2:r.job2,note_g:Number(r.note_g)||0,note_d:Number(r.note_d)||0,note_m:Number(r.note_m)||0,note_a:Number(r.note_a)||0,rarity:r.rarity,skin:r.skin,hair:r.hair,hair_length:r.hair_length,clubName:((s=r.clubs)==null?void 0:s.encoded_name)||null,clubLogo:((d=r.clubs)==null?void 0:d.logo_url)||null,boost:0,used:!1,_line:u})}const $=lt(w.length);w.forEach((z,r)=>{z._col=$[r]}),c[u]=w}return c}function Ao(e){const t=Lt[e]||Lt["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},a=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let c=0;for(const l of["GK","DEF","MIL","ATT"]){const s=[];for(let u=0;u<t[l];u++){const n=3+Math.floor(Math.random()*5);s.push({cardId:"fake-"+c,id:"fake-"+c,firstname:"IA",name:a[c%a.length],country_code:"XX",club_id:null,job:l,job2:null,note_g:l==="GK"?n:2,note_d:l==="DEF"?n:2,note_m:l==="MIL"?n:2,note_a:l==="ATT"?n:2,rarity:"normal",boost:0,used:!1,_line:l}),c++}const d=lt(s.length);s.forEach((u,n)=>{u._col=d[n]}),i[l]=s}return i}function To(e,t,i){e.innerHTML=`
  ...
  </div>`,setTimeout(()=>zo(e,t,i),5e3)}function zo(e,t,i){const a=t.homeTeam.MIL||[],c=t.aiTeam.MIL||[];function l(r){return r.reduce((p,S)=>p+ke(S,"MIL"),0)}function s(r){let p=0;for(let S=0;S<r.length-1;S++){const M=at(r[S],r[S+1]);M==="#00ff88"?p+=2:M==="#FFD700"&&(p+=1)}return p}const d=l(a)+s(a),u=l(c)+s(c),n=d>=u;function o(r,p,S,M){return`<div id="duel-row-${M}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0), opacity .5s ease;transform-origin:center">
  ...
  <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div id="duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center, rgba(10,61,30,0.4), rgba(10,61,30,0.92))"></div>
  </div>`;const g=()=>{const r=(p,S)=>e.querySelectorAll(p).forEach((M,F)=>{setTimeout(()=>{M.style.opacity="1",M.style.transform="translateY(0) scale(1)"},S+F*90)});r(".duel-card-home",150),r(".duel-card-ai",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((p,S)=>{setTimeout(()=>{p.style.opacity="1"},S*70)}),900),setTimeout(()=>{const p=e.querySelector("#vs-label");p&&(p.style.opacity="1",p.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(S=>S.style.opacity="1")},1250),setTimeout(()=>{w("score-home",d,800),w("score-ai",u,800)},1500)};function w(r,p,S){const M=document.getElementById(r);if(!M)return;const F=performance.now(),O=G=>{const q=Math.min(1,(G-F)/S);M.textContent=Math.round(p*(1-Math.pow(1-q,3))),q<1?requestAnimationFrame(O):M.textContent=p};requestAnimationFrame(O)}requestAnimationFrame(g),t.attacker=n?"home":"ai";const $=n?ti():null;n&&(t.boostCard={value:$}),t.log.push({type:"duel",title:"Milieu de Terrain",homePlayers:a.map(r=>({name:r.name,note:ke(r,"MIL"),portrait:Ge(r),job:r.job,country_code:r.country_code,rarity:r.rarity,clubName:r.clubName,clubLogo:r.clubLogo})),aiPlayers:c.map(r=>({name:r.name,note:ke(r,"MIL"),portrait:Ge(r),job:r.job,country_code:r.country_code,rarity:r.rarity,clubName:r.clubName,clubLogo:r.clubLogo})),homeTotal:d,aiTotal:u,text:`Duel milieu : ${t.clubName} ${d} – ${u} IA → ${n?t.clubName+" attaque":"IA attaque"}`});const z=()=>{t.phase=t.attacker==="home"?"attack":"ai-attack",Ye(e,t,i),t.attacker==="ai"&&setTimeout(()=>ui(e,t,i),800)};setTimeout(()=>{const r=document.getElementById("score-home"),p=document.getElementById("score-ai"),S=document.getElementById(n?"duel-row-home":"duel-row-ai"),M=document.getElementById(n?"duel-row-ai":"duel-row-home"),F=n?r:p,O=n?p:r;F&&(F.style.fontSize="80px",F.style.color=n?"#FFD700":"#ff6b6b",F.style.animation="duelPulse .5s ease"+(n?", duelGlow 1.5s ease infinite .5s":"")),O&&(O.style.opacity="0.25"),setTimeout(()=>{S&&(S.style.transformOrigin="center",S.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",S.style.zIndex="5"),setTimeout(()=>{var q;const G=document.getElementById("duel-shock");if(G){const H=(q=M||S)==null?void 0:q.getBoundingClientRect(),ie=e.querySelector(".match-screen").getBoundingClientRect();H&&(G.style.top=H.top-ie.top+H.height/2+"px"),G.style.animation="shockwave .5s ease-out forwards"}M&&(M.style.transformOrigin="center",M.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var q;const G=document.getElementById("duel-finale");G&&(G.innerHTML=`
          ...
          <button id="start-match-btn" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,0.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">
            ▶ Commencer le match
          </button>`,G.style.transition="opacity .45s ease",G.style.opacity="1",G.style.pointerEvents="auto",(q=document.getElementById("start-match-btn"))==null||q.addEventListener("click",z))},600)},700)},2800)}function Lo(e){if(e.type==="duel"&&(e.homeTotal!=null||e.aiTotal!=null)){const t=(e.homeTotal||0)>=(e.aiTotal||0);return`
    ...
    <div style="display:flex;align-items:center;gap:4px;${t?"flex-direction:row-reverse":""};background:rgba(255,255,255,0.04);border-radius:8px;padding:5px 8px;border:1px solid rgba(255,255,255,0.07)">
      ...
      ${dt(e.inPlayer||{})}
    </div>`}return`<div style="font-size:11px;color:${e.type==="goal"?"#FFD700":"rgba(255,255,255,0.65)"};font-weight:${e.type==="goal"?700:400};padding:3px 2px">${e.text||""}</div>`}function Ye(e,t,i){var F,O,G,q,H,ie,ue,se;const a=t.selected.map(B=>B.cardId),c=t.usedSubIds||[],l=t.homeSubs.filter(B=>!c.includes(B.cardId)),d=Object.values(t.homeTeam).flat().filter(B=>B.used).length>0&&l.length>0&&t.subsUsed<t.maxSubs,u=!["GK","DEF","MIL","ATT"].some(B=>(t.aiTeam[B]||[]).some(ne=>!ne.used)),n=[...t.homeTeam.MIL||[],...t.homeTeam.ATT||[]].filter(B=>!B.used),o=t.phase==="attack"&&u&&n.length===0?[...t.homeTeam.GK||[],...t.homeTeam.DEF||[]].filter(B=>!B.used).map(B=>B.cardId):[];t.log[t.log.length-1];const g=t.phase==="ai-attack"||t.phase==="ai-defense",w=t.phase==="attack",$=t.phase==="defense",z=t.phase==="finished",r=t.gcCards.filter(B=>!t.usedGc.includes(B.id)),p=t.boostCard&&!t.boostUsed;e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column",e.innerHTML=`
  <style>
  ...
  `,document.body.appendChild(a);let o=!1;const g=()=>{o||(o=!0,a.remove(),i())};a.addEventListener("click",g),setTimeout(g,2e3)}function It(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function ai(e,t,i,a=null,c=null){var $,z;if(t.phase!=="attack"){It("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){It(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const l=Object.entries(t.homeTeam).flatMap(([r,p])=>(p||[]).filter(S=>S.used).map(S=>({...S,_line:S._line||r}))),s=t.homeSubs.filter(r=>!t.usedSubIds.includes(r.cardId));if(!l.length){It("Aucun joueur utilisé à remplacer");return}if(!s.length){It("Aucun remplaçant disponible");return}let d=Math.max(0,l.findIndex(r=>r.cardId===c));const u=(($=l[d])==null?void 0:$._line)||((z=l[d])==null?void 0:z.job);let n=a?Math.max(0,s.findIndex(r=>r.cardId===a)):Math.max(0,s.findIndex(r=>r.job===u)),o=!1;const g=document.createElement("div");g.id="sub-overlay",g.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function w(){var G,q,H,ie,ue,se;const r=l[d],p=s[n],S=Math.min(130,Math.round((window.innerWidth-90)/2)),M=Math.round(S*1.35),F=B=>`background:rgba(255,255,255,0.12);border:none;color:${B?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${B?"default":"pointer"};flex-shrink:0`;g.innerHTML=`
    ...
    <div style="flex:1;display:flex;gap:0;overflow:hidden">
      ...
      <div id="in-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
        ...
        <div>${p?We({...p,used:!1,boost:0},S,M):"<div>—</div>"}</div>
      ...
      <div id="out-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
        ...
        <div>${r?We({...r,used:!1,boost:0},S,M):"<div>—</div>"}</div>
    ...
    </div>`,document.body.appendChild($),(z=$.querySelector("#gc-back"))==null||z.addEventListener("click",()=>$.remove()),(r=$.querySelector("#gc-use"))==null||r.addEventListener("click",()=>{$.remove(),Bo(e,t,i,a,c)})}function Ut(e,t,i,a,c,l){const s=document.createElement("div");s.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let d=[];function u(){var n,o;s.innerHTML=`
    ...
    <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
      ${e.map(g=>{const w=g._line||g.job||"MIL",$={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[w]||"#555",z=ke(g,w)+(g.boost||0),r=d.find(p=>p.cardId===g.cardId);return`<div class="gc-pick-item" data-cid="${g.cardId}"
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(l);let d=!1;const u=()=>{d||(d=!0,l.remove(),setTimeout(()=>c(),50))};l.addEventListener("click",u),setTimeout(u,3500)}async function qt(e,t,i){var o,g;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:a}=i,c=t.homeScore>t.aiScore,l=t.homeScore===t.aiScore,s=c?"victoire":l?"nul":"defaite",d=On(t.mode,s);t.matchId&&await E.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:c?a.profile.id:null,home_credits_reward:d,played_at:new Date().toISOString()}).eq("id",t.matchId);const u={credits:(a.profile.credits||0)+d,matches_played:(a.profile.matches_played||0)+1};c?u.wins=(a.profile.wins||0)+1:l?u.draws=(a.profile.draws||0)+1:u.losses=(a.profile.losses||0)+1,await E.from("users").update(u).eq("id",a.profile.id),await i.refreshProfile();const n=document.createElement("div");n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",n.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const $=async(F=!0)=>{g=!0,w&&(E.removeChannel(w),w=null),F&&await E.rpc("cancel_matchmaking",{p_user_id:u.profile.id}).catch(()=>{})};(M=document.getElementById("mm-cancel"))==null||M.addEventListener("click",async()=>{try{await $(!0)}catch{}Ke(e),n("home")});const z=async(F,O,G)=>{if(g)return;g=!0,w&&(E.removeChannel(w),w=null);const q=document.getElementById("mm-status");q&&(q.textContent="Adversaire trouvé !"),await new Promise(H=>setTimeout(H,600)),e.isConnected&&bn(e,t,F,G,s,d)},{data:r,error:p}=await E.rpc("try_matchmake",{p_user_id:u.profile.id,p_deck_id:i});if(p||!(r!=null&&r.success)){o("Erreur de matchmaking","error"),Ke(e),n("home");return}if(r.matched){z(r.match_id,r.opponent_id,!1);return}const S=document.getElementById("mm-status");S&&(S.textContent="En attente d'un autre joueur..."),w=E.channel("matchmaking-"+u.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${u.profile.id}`},F=>{const O=F.new;O.status==="matched"&&O.match_id&&z(O.match_id,O.matched_with,!0)}).subscribe()}async function No(e,t,i){const{state:a,navigate:c,toast:l}=t;try{(E.getChannels?E.getChannels():[]).forEach(n=>{const o=n.topic||"";(o.includes("matchmaking")||o.includes("pvp-match")||o.includes("friend-invite"))&&E.removeChannel(n)})}catch{}const{data:s}=await E.from("matches").select("home_id, away_id, status").eq("id",i).single();if(!s){l("Match introuvable","error"),c("home");return}if(s.status==="finished"){l("Ce match est terminé","info"),c("home");return}const d=s.home_id===a.profile.id;bn(e,t,i,d,[],[])}async function bn(e,t,i,a,c=[],l=[]){const{state:s,navigate:d,toast:u}=t,n=a?"p1":"p2",o=a?"p2":"p1",g=(c||[]).map(j=>j.id),w=(c||[]).map(j=>({id:j.id,gc_type:j.gc_type,_gcDef:j._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:$}=await E.from("matches").select("*").eq("id",i).single();if(!$){u("Match introuvable","error"),d("home");return}async function z(){const[{data:j},{data:b},{data:k},{data:A}]=await Promise.all([E.rpc("get_deck_for_match",{p_deck_id:$.home_deck_id}),E.rpc("get_deck_for_match",{p_deck_id:$.away_deck_id}),E.from("users").select("id,pseudo,club_name").eq("id",$.home_id).single(),E.from("users").select("id,pseudo,club_name").eq("id",$.away_id).single()]),f=v=>({cardId:v.card_id,position:v.position,id:v.id,firstname:v.firstname,name:v.surname_encoded,country_code:v.country_code,club_id:v.club_id,job:v.job,job2:v.job2,note_g:Number(v.note_g)||0,note_d:Number(v.note_d)||0,note_m:Number(v.note_m)||0,note_a:Number(v.note_a)||0,rarity:v.rarity,skin:v.skin,hair:v.hair,hair_length:v.hair_length,clubName:v.club_encoded_name||null,clubLogo:v.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}),m=((j==null?void 0:j.starters)||[]).map(v=>f(v)),x=((b==null?void 0:b.starters)||[]).map(v=>f(v)),y=(j==null?void 0:j.formation)||"4-4-2",h=(b==null?void 0:b.formation)||"4-4-2";return{p1Team:yt(m,y),p2Team:yt(x,h),p1Subs:((j==null?void 0:j.subs)||[]).map(v=>f(v)),p2Subs:((b==null?void 0:b.subs)||[]).map(v=>f(v)),p1Formation:y,p2Formation:h,p1Name:(k==null?void 0:k.club_name)||(k==null?void 0:k.pseudo)||"Joueur 1",p2Name:(A==null?void 0:A.club_name)||(A==null?void 0:A.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:a?g:[],gc_p2:a?[]:g,gcCardsFull_p1:a?w:[],gcCardsFull_p2:a?[]:w,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:l||[],lastActionAt:new Date().toISOString()}}let r=$.game_state&&Object.keys($.game_state).length?$.game_state:null;if(!r)if(a){r=await z();const{data:j}=await E.from("matches").select("game_state").eq("id",i).single();!(j!=null&&j.game_state)||!Object.keys(j.game_state).length?await E.from("matches").update({game_state:r,turn_user_id:$.home_id}).eq("id",i):r=j.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let j=0;j<30&&!r;j++){await new Promise(k=>setTimeout(k,400));const{data:b}=await E.from("matches").select("game_state").eq("id",i).single();b!=null&&b.game_state&&Object.keys(b.game_state).length&&(r=b.game_state)}if(!r){u("Erreur de synchronisation","error"),d("home");return}r.gc_p2=g,r.gcCardsFull_p2=w,await E.from("matches").update({game_state:r}).eq("id",i)}let p=!1,S=null,M=!1;const F=new Set,O=new Set;function G(j){var v,_;try{E.removeChannel(q)}catch{}const b=r[n+"Score"]||0,k=r[o+"Score"]||0;let A,f;j.winner_id?(A=j.winner_id===s.profile.id,f=!1):j.forfeit?(A=b>k,f=!1):(f=b===k,A=b>k),(v=document.getElementById("pvp-end-overlay"))==null||v.remove();const m=document.createElement("div");m.id="pvp-end-overlay",m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const x=f?"🤝":A?"🏆":"😞",y=f?"MATCH NUL":A?"VICTOIRE !":"DÉFAITE",h=f?"#fff":A?"#FFD700":"#ff6b6b";m.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(m),(_=m.querySelector("#pvp-end-home"))==null||_.addEventListener("click",()=>{m.remove(),Ke(e),d("home")})}const q=E.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},j=>{const b=j.new;try{if(b.status==="finished"||b.forfeit){if(p)return;p=!0,S&&(clearInterval(S),S=null),b.game_state&&(r=b.game_state),G(b);return}if(b.game_state){const k=r;r=b.game_state;const A=r._lastGC;if(A&&A.seq&&!O.has(A.seq)&&(O.add(A.seq),A.by!==n)){ve(A.type,A.by,()=>P());return}const f=k[n+"Score"]||0,m=k[o+"Score"]||0,x=r[n+"Score"]||0,y=r[o+"Score"]||0,h=x>f,v=y>m;if((h||v)&&!F.has(r.round)){F.add(r.round);const _=[...r.log||[]].reverse().find(L=>L.isGoal),T=((_==null?void 0:_.homePlayers)||[]).map(L=>({name:L.name,note:L.note,portrait:L.portrait,job:L.job}));Ae(T,x,y,h,()=>P());return}P()}}catch(k){console.error("[PvP] crash:",k)}}).subscribe();async function H(j){Object.assign(r,j),r.lastActionAt=new Date().toISOString();const{error:b}=await E.from("matches").update({game_state:r}).eq("id",i);b&&u("Erreur de synchronisation","error");try{P()}catch(k){console.error("[PvP] renderPvpScreen crash:",k)}}async function ie(){if(p)return;p=!0,S&&(clearInterval(S),S=null);const j=a?$.away_id:$.home_id,b=a?"p2":"p1",k=a?"p1":"p2",A={...r,[b+"Score"]:3,[k+"Score"]:0,phase:"finished"};try{await E.from("matches").update({status:"finished",forfeit:!0,winner_id:j,home_score:A.p1Score||0,away_score:A.p2Score||0,game_state:A}).eq("id",i)}catch{}try{E.removeChannel(q)}catch{}setTimeout(()=>{Ke(e),d("home")},800)}const ue={BOOST_STAT:({value:j=1,count:b=1,roles:k=[]},A,f)=>{const m=A[n+"Team"],x=Object.entries(m).filter(([y])=>!k.length||k.includes(y)).flatMap(([y,h])=>h.filter(v=>!v.used).map(v=>({...v,_line:y})));if(!x.length){A.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),f(A);return}se(x,b,`Choisir ${b} joueur(s) à booster (+${j})`,y=>{y.forEach(h=>{const v=(m[h._line]||[]).find(_=>_.cardId===h.cardId);v&&(v.boost=(v.boost||0)+j,A.log.push({text:`⚡ +${j} sur ${v.name}`,type:"info"}))}),A[n+"Team"]=m,f(A)})},DEBUFF_STAT:({value:j=1,count:b=1,roles:k=[],target:A="ai"},f,m)=>{const x=A==="home"?n:o,y=f[x+"Team"],h=A==="home"?"allié":"adverse",v=Object.entries(y).filter(([_])=>!k.length||k.includes(_)).flatMap(([_,T])=>T.filter(L=>!L.used).map(L=>({...L,_line:_})));if(!v.length){f.log.push({text:`🎯 Aucun joueur ${h}`,type:"info"}),m(f);return}se(v,b,`Choisir ${b} joueur(s) ${h}(s) (-${j})`,_=>{_.forEach(T=>{const L=(y[T._line]||[]).find(I=>I.cardId===T.cardId);L&&(L.boost=(L.boost||0)-j,f.log.push({text:`🎯 -${j} sur ${L.name}`,type:"info"}))}),f[x+"Team"]=y,m(f)})},GRAY_PLAYER:({count:j=1,roles:b=[],target:k="ai"},A,f)=>{const m=k==="home"?n:o,x=A[m+"Team"],y=k==="home"?"allié":"adverse",h=Object.entries(x).filter(([v])=>!b.length||b.includes(v)).flatMap(([v,_])=>_.filter(T=>!T.used).map(T=>({...T,_line:v})));if(!h.length){A.log.push({text:`❌ Aucun joueur ${y}`,type:"info"}),f(A);return}se(h,j,`Choisir ${j} joueur(s) ${y}(s) à exclure`,v=>{v.forEach(_=>{const T=(x[_._line]||[]).find(L=>L.cardId===_.cardId);T&&(T.used=!0,A.log.push({text:`❌ ${T.name} exclu !`,type:"info"}))}),A[m+"Team"]=x,f(A)})},REVIVE_PLAYER:({count:j=1,roles:b=[]},k,A)=>{const f=k[n+"Team"],m=Object.entries(f).filter(([x])=>!b.length||b.includes(x)).flatMap(([x,y])=>y.filter(h=>h.used).map(h=>({...h,_line:x})));if(!m.length){k.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),A(k);return}se(m,j,`Choisir ${j} joueur(s) à ressusciter`,x=>{x.forEach(y=>{const h=(f[y._line]||[]).find(v=>v.cardId===y.cardId);h&&(h.used=!1,k.log.push({text:`💫 ${h.name} ressuscité !`,type:"info"}))}),k[n+"Team"]=f,A(k)})},REMOVE_GOAL:({},j,b)=>{const k=o+"Score";j[k]>0?(j[k]--,j.log.push({text:"🚫 Dernier but annulé !",type:"info"})):j.log.push({text:"🚫 Aucun but à annuler",type:"info"}),b(j)},ADD_GOAL_DRAW:({},j,b)=>{j[n+"Score"]===j[o+"Score"]?(j[n+"Score"]++,j.log.push({text:"🎯 But bonus !",type:"info"})):j.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),b(j)},ADD_SUB:({value:j=1},b,k)=>{b.maxSubs=(b.maxSubs||3)+j,b.log.push({text:`🔄 +${j} remplacement(s)`,type:"info"}),k(b)},CUSTOM:({},j,b)=>b(j)};function se(j,b,k,A){const f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let m=[];function x(){var h,v;const y=j.map(_=>{const T={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[_._line]||"#555",L=ke(_,_._line)+(_.boost||0),U=m.find(C=>C.cardId===_.cardId)?"#FFD700":"rgba(255,255,255,0.25)",D=_.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${_.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${U};background:${T};overflow:hidden;cursor:pointer;${D}">
      ...
      ${I.length===0?'<div style="font-size:7px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':I.map(ae=>`<div class="pvp-sub-btn" data-sub-id="${ae.cardId}" style="cursor:pointer;flex-shrink:0">${We(ae,_?76:44,_?100:58)}</div>`).join("")}
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(m);let h=!1;const v=()=>{h||(h=!0,m.remove(),setTimeout(()=>f(),50))};m.addEventListener("click",v),setTimeout(v,3500)}function ve(j,b,k){var D,C;const A=(r.gcDefs||[]).find(V=>{var ee;return V.name===j||((ee=V.name)==null?void 0:ee.toLowerCase().trim())===(j==null?void 0:j.toLowerCase().trim())}),f={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[A==null?void 0:A.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",m={purple:"#b06ce0",light_blue:"#00d4ef"}[A==null?void 0:A.color]||"#b06ce0",x=(A==null?void 0:A.name)||j,y=(A==null?void 0:A.effect)||((D=Ve[j])==null?void 0:D.desc)||"",h=A!=null&&A.image_url?`/manager-wars/icons/${A.image_url}`:null,v=((C=Ve[j])==null?void 0:C.icon)||"⚡",T=b===n?"Vous":r[b+"Name"]||"Adversaire",L=document.createElement("div");L.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",L.innerHTML=`
      <style>
      ...
      </div>`,(re=T.querySelector("#psub-close"))==null||re.addEventListener("click",()=>T.remove()),(de=T.querySelector("#pout-up"))==null||de.addEventListener("click",()=>{y>0&&(y--,L())}),(me=T.querySelector("#pout-down"))==null||me.addEventListener("click",()=>{y<m.length-1&&(y++,L())}),(we=T.querySelector("#pin-up"))==null||we.addEventListener("click",()=>{v>0&&(v--,L())}),(_e=T.querySelector("#pin-down"))==null||_e.addEventListener("click",()=>{v<x.length-1&&(v++,L())});const te=(Ee,ze,Me,Ne)=>{const Le=T.querySelector("#"+Ee);if(!Le)return;let He=0;Le.addEventListener("touchstart",Re=>{He=Re.touches[0].clientY},{passive:!0}),Le.addEventListener("touchend",Re=>{const Fe=Re.changedTouches[0].clientY-He;if(Math.abs(Fe)<30)return;const Be=ze();Fe<0&&Be<Ne-1?(Me(Be+1),L()):Fe>0&&Be>0&&(Me(Be-1),L())},{passive:!0})};te("pin-panel",()=>v,Ee=>v=Ee,x.length),te("pout-panel",()=>y,Ee=>y=Ee,m.length),($e=T.querySelector("#psub-confirm"))==null||$e.addEventListener("click",async Ee=>{if(Ee.preventDefault(),Ee.stopPropagation(),_)return;_=!0;const ze=m[y],Me=x[v];if(!ze||!Me)return;const Ne=ze._line,Le=(k[Ne]||[]).findIndex(Fe=>Fe.cardId===ze.cardId);if(Le===-1){u("Erreur : joueur introuvable","error"),T.remove();return}const He={...Me,_line:Ne,position:ze.position,used:!1,boost:0};k[Ne].splice(Le,1,He);const Re=[...A,Me.cardId];T.remove(),N(ze,Me,async()=>{await H({[n+"Team"]:k,[o+"Team"]:r[o+"Team"],["usedSubIds_"+n]:Re})})})}document.body.appendChild(T),L()}function N(j,b,k){const A={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const m=(h,v,_)=>`<div style="text-align:center">
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(x),(T=x.querySelector("#pvp-end-home"))==null||T.addEventListener("click",()=>{x.remove(),Ke(e),d("home")})}const H=E.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},b=>{const k=b.new;try{if(k.status==="finished"||k.forfeit){if(p)return;p=!0,M&&(clearInterval(M),M=null),k.game_state&&(r=k.game_state),q(k);return}if(k.game_state){const A=r;r=k.game_state;const f=r._lastGC;if(f&&f.seq&&!G.has(f.seq)&&(G.add(f.seq),f.by!==n)){he(f.type,f.by,()=>oe());return}const m=A[n+"Score"]||0,x=A[o+"Score"]||0,y=r[n+"Score"]||0,h=r[o+"Score"]||0,v=y>m,_=h>x;if((v||_)&&!O.has(r.round)){O.add(r.round);const T=[...r.log||[]].reverse().find(I=>I.isGoal),L=((T==null?void 0:T.homePlayers)||[]).map(I=>({name:I.name,note:I.note,portrait:I.portrait,job:I.job}));ve(L,y,h,v,()=>oe());return}oe()}}catch(A){console.error("[PvP] crash:",A)}}).subscribe();async function ie(b){Object.assign(r,b),r.lastActionAt=new Date().toISOString();const{error:k}=await E.from("matches").update({game_state:r}).eq("id",i);k&&u("Erreur de synchronisation","error");try{oe()}catch(A){console.error("[PvP] renderPvpScreen crash:",A)}}async function ue(){if(p)return;p=!0,M&&(clearInterval(M),M=null);const b=a?$.away_id:$.home_id,k=a?"p2":"p1",A=a?"p1":"p2",f={...r,[k+"Score"]:3,[A+"Score"]:0,phase:"finished"};try{await E.from("matches").update({status:"finished",forfeit:!0,winner_id:b,home_score:f.p1Score||0,away_score:f.p2Score||0,game_state:f}).eq("id",i)}catch{}try{E.removeChannel(H)}catch{}setTimeout(()=>{Ke(e),d("home")},800)}const se={BOOST_STAT:({value:b=1,count:k=1,roles:A=[]},f,m)=>{const x=f[n+"Team"],y=Object.entries(x).filter(([h])=>!A.length||A.includes(h)).flatMap(([h,v])=>v.filter(_=>!_.used).map(_=>({..._,_line:h})));if(!y.length){f.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),m(f);return}B(y,k,`Choisir ${k} joueur(s) à booster (+${b})`,h=>{h.forEach(v=>{const _=(x[v._line]||[]).find(T=>T.cardId===v.cardId);_&&(_.boost=(_.boost||0)+b,f.log.push({text:`⚡ +${b} sur ${_.name}`,type:"info"}))}),f[n+"Team"]=x,m(f)})},DEBUFF_STAT:({value:b=1,count:k=1,roles:A=[],target:f="ai"},m,x)=>{const y=f==="home"?n:o,h=m[y+"Team"],v=f==="home"?"allié":"adverse",_=Object.entries(h).filter(([T])=>!A.length||A.includes(T)).flatMap(([T,L])=>L.filter(I=>!I.used).map(I=>({...I,_line:T})));if(!_.length){m.log.push({text:`🎯 Aucun joueur ${v}`,type:"info"}),x(m);return}B(_,k,`Choisir ${k} joueur(s) ${v}(s) (-${b})`,T=>{T.forEach(L=>{const I=(h[L._line]||[]).find(U=>U.cardId===L.cardId);I&&(I.boost=(I.boost||0)-b,m.log.push({text:`🎯 -${b} sur ${I.name}`,type:"info"}))}),m[y+"Team"]=h,x(m)})},GRAY_PLAYER:({count:b=1,roles:k=[],target:A="ai"},f,m)=>{const x=A==="home"?n:o,y=f[x+"Team"],h=A==="home"?"allié":"adverse",v=Object.entries(y).filter(([_])=>!k.length||k.includes(_)).flatMap(([_,T])=>T.filter(L=>!L.used).map(L=>({...L,_line:_})));if(!v.length){f.log.push({text:`❌ Aucun joueur ${h}`,type:"info"}),m(f);return}B(v,b,`Choisir ${b} joueur(s) ${h}(s) à exclure`,_=>{_.forEach(T=>{const L=(y[T._line]||[]).find(I=>I.cardId===T.cardId);L&&(L.used=!0,f.log.push({text:`❌ ${L.name} exclu !`,type:"info"}))}),f[x+"Team"]=y,m(f)})},REVIVE_PLAYER:({count:b=1,roles:k=[]},A,f)=>{const m=A[n+"Team"],x=Object.entries(m).filter(([y])=>!k.length||k.includes(y)).flatMap(([y,h])=>h.filter(v=>v.used).map(v=>({...v,_line:y})));if(!x.length){A.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),f(A);return}B(x,b,`Choisir ${b} joueur(s) à ressusciter`,y=>{y.forEach(h=>{const v=(m[h._line]||[]).find(_=>_.cardId===h.cardId);v&&(v.used=!1,A.log.push({text:`💫 ${v.name} ressuscité !`,type:"info"}))}),A[n+"Team"]=m,f(A)})},REMOVE_GOAL:({},b,k)=>{const A=o+"Score";b[A]>0?(b[A]--,b.log.push({text:"🚫 Dernier but annulé !",type:"info"})):b.log.push({text:"🚫 Aucun but à annuler",type:"info"}),k(b)},ADD_GOAL_DRAW:({},b,k)=>{b[n+"Score"]===b[o+"Score"]?(b[n+"Score"]++,b.log.push({text:"🎯 But bonus !",type:"info"})):b.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),k(b)},ADD_SUB:({value:b=1},k,A)=>{k.maxSubs=(k.maxSubs||3)+b,k.log.push({text:`🔄 +${b} remplacement(s)`,type:"info"}),A(k)},CUSTOM:({},b,k)=>k(b)};function B(b,k,A,f){const m=document.createElement("div");m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let x=[];function y(){var v,_;const h=b.map(T=>{const L={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[T._line]||"#555",I=ke(T,T._line)+(T.boost||0),D=x.find(V=>V.cardId===T.cardId)?"#FFD700":"rgba(255,255,255,0.25)",C=T.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${T.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${D};background:${L};overflow:hidden;cursor:pointer;${C}">
        ...
        </div>`,(v=m.querySelector("#pp-close"))==null||v.addEventListener("click",()=>m.remove()),m.querySelectorAll(".pp-item").forEach(T=>{T.addEventListener("click",()=>{const L=T.dataset.cid,I=b.find(D=>D.cardId===L),U=x.findIndex(D=>D.cardId===L);I&&(U>-1?x.splice(U,1):x.length<k&&x.push(I),y())})}),(_=m.querySelector("#pp-confirm"))==null||_.addEventListener("click",()=>{m.remove(),f(x)})}y(),document.body.appendChild(m)}async function ne(b,k){const f=(r["gcCardsFull_"+n]||[]).find(h=>h.id===b),m=(f==null?void 0:f._gcDef)||(r.gcDefs||[]).find(h=>{var v;return h.name===k||((v=h.name)==null?void 0:v.toLowerCase().trim())===(k==null?void 0:k.toLowerCase().trim())}),x=[...r["usedGc_"+n]||[],b],y={type:k,by:n,seq:(r._gcAnimSeq||0)+1};G.add(y.seq),he(k,n,async()=>{if(m!=null&&m.effect_type&&m.effect_type!=="CUSTOM"){const v=se[m.effect_type];if(v){const _={...r};v(m.effect_params||{},_,async T=>{T["usedGc_"+n]=x,T._lastGC=y,T._gcAnimSeq=y.seq,await ie(T)});return}}const h={...r};switch(k){case"Remplacement+":h.maxSubs=(h.maxSubs||3)+1,h.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const v=o+"Score";h[v]>0&&(h[v]--,h.log.push({text:"🚫 But annulé",type:"info"}));break}}h["usedGc_"+n]=x,h._lastGC=y,h._gcAnimSeq=y.seq,await ie(h)})}function W(b,k){const A="usedCardIds_"+b,f=new Set(r[A]||[]);k.forEach(m=>f.add(m)),r[A]=[...f]}function P(){for(const b of["p1","p2"]){const k=new Set(r["usedCardIds_"+b]||[]),A=r[b+"Team"];if(A)for(const f of["GK","DEF","MIL","ATT"])(A[f]||[]).forEach(m=>{m.used=k.has(m.cardId)})}}function oe(){var Ze,tt,Xe,it,ft,ae;if(r.phase==="reveal")return fe();if(r.phase==="midfield")return Ae();if(r.phase==="finished")return j();const b=r[n+"Team"],k=r[o+"Team"];P();const A=r[n+"Score"],f=r[o+"Score"],m=r[n+"Name"],x=r[o+"Name"],y=r.phase===n+"-attack",h=r.phase===n+"-defense",v=Array.isArray(r["selected_"+n])?r["selected_"+n]:[],_=v.map(K=>K.cardId),T=window.innerWidth>=700,L=r[n+"Subs"]||[],I=r["usedSubIds_"+n]||[],U=L.filter(K=>!I.includes(K.cardId)),D=r["gcCardsFull_"+n]||[],C=r["usedGc_"+n]||[],V=D.filter(K=>!C.includes(K.id)),ee=r.boostOwner===n&&!r.boostUsed,X=!["GK","DEF","MIL","ATT"].some(K=>(k[K]||[]).some(ce=>!ce.used)),te=[...b.MIL||[],...b.ATT||[]].filter(K=>!K.used),re=y&&X&&te.length===0?[...b.GK||[],...b.DEF||[]].filter(K=>!K.used).map(K=>K.cardId):[];function de(K,ce,ge){var gt,kt;const Y=K._gcDef,xe={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[Y==null?void 0:Y.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",Te={purple:"#b06ce0",light_blue:"#00d4ef"}[Y==null?void 0:Y.color]||"#b06ce0",be=(Y==null?void 0:Y.name)||K.gc_type,et=(Y==null?void 0:Y.effect)||((gt=Ve[K.gc_type])==null?void 0:gt.desc)||"",ot=Y!=null&&Y.image_url?`/manager-wars/icons/${Y.image_url}`:null,st=((kt=Ve[K.gc_type])==null?void 0:kt.icon)||"⚡",rt=Math.round(ge*.22),ct=Math.round(ge*.22),bt=ge-rt-ct,vt=be.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${K.id}" data-gc-type="${K.gc_type}"
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="width:min(90vw,420px)">${pt(r[o+"Team"],r[o+"Formation"],null,[],300,300)}</div>
        ...
        <div class="duel-score-line duel-score-line-${X}" style="margin-top:10px;font-size:12px;color:rgba(255,255,255,0.55);opacity:0;transition:opacity .4s ease">
          Score: ${A(V)} + ${f(V)} liens = <b style="color:#fff">${A(V)+f(V)}</b>
        </div>
      </div>`}Qe(e),e.innerHTML=`
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      <style>
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(I);let U=!1;const D=()=>{U||(U=!0,I.remove(),setTimeout(()=>A&&A(),50))};I.addEventListener("click",D),setTimeout(D,3e3)}function J(b,k){var I,U,D,C;const f=(r["gcCardsFull_"+n]||[]).find(V=>V.id===b),m=f==null?void 0:f._gcDef,x={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[m==null?void 0:m.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",y={purple:"#b06ce0",light_blue:"#00d4ef"}[m==null?void 0:m.color]||"#b06ce0",h=(m==null?void 0:m.name)||k,v=(m==null?void 0:m.effect)||((I=Ve[k])==null?void 0:I.desc)||"Carte spéciale.",_=m!=null&&m.image_url?`/manager-wars/icons/${m.image_url}`:null,T=((U=Ve[k])==null?void 0:U.icon)||"⚡",L=document.createElement("div");L.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",L.innerHTML=`
      ...
      <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
        <button id="psub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
      </div>`,(de=L.querySelector("#psub-close"))==null||de.addEventListener("click",()=>L.remove()),(me=L.querySelector("#pout-up"))==null||me.addEventListener("click",()=>{h>0&&(h--,I())}),(we=L.querySelector("#pout-down"))==null||we.addEventListener("click",()=>{h<x.length-1&&(h++,I())}),(_e=L.querySelector("#pin-up"))==null||_e.addEventListener("click",()=>{_>0&&(_--,I())}),($e=L.querySelector("#pin-down"))==null||$e.addEventListener("click",()=>{_<y.length-1&&(_++,I())});const re=(ze,Me,Ne,Le)=>{const He=L.querySelector("#"+ze);if(!He)return;let Re=0;He.addEventListener("touchstart",Fe=>{Re=Fe.touches[0].clientY},{passive:!0}),He.addEventListener("touchend",Fe=>{const Be=Fe.changedTouches[0].clientY-Re;if(Math.abs(Be)<30)return;const Ze=Me();Be<0&&Ze<Le-1?(Ne(Ze+1),I()):Be>0&&Ze>0&&(Ne(Ze-1),I())},{passive:!0})};re("pin-panel",()=>_,ze=>_=ze,y.length),re("pout-panel",()=>h,ze=>h=ze,x.length),(Ee=L.querySelector("#psub-confirm"))==null||Ee.addEventListener("click",async ze=>{if(ze.preventDefault(),ze.stopPropagation(),T)return;T=!0;const Me=x[h],Ne=y[_];if(!Me||!Ne)return;const Le=Me._line,He=(A[Le]||[]).findIndex(Be=>Be.cardId===Me.cardId);if(He===-1){u("Erreur : joueur introuvable","error"),L.remove();return}const Re={...Ne,_line:Le,position:Me.position,used:!1,boost:0};A[Le].splice(He,1,Re);const Fe=[...f,Ne.cardId];L.remove(),R(Me,Ne,async()=>{await ie({[n+"Team"]:A,[o+"Team"]:r[o+"Team"],["usedSubIds_"+n]:Fe})})})}document.body.appendChild(L),I()}function R(b,k,A){const f={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},m=document.createElement("div");m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const x=(v,_,T)=>`<div style="text-align:center">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="display:flex;background:#fff;border-bottom:1px solid #eee">
      ${z.map(p=>`<button class="ml-tab" data-tab="${p.key}" style="flex:1;padding:10px 4px;border:none;border-bottom:2px solid ${i===p.key?Tt:"transparent"};background:none;font-size:12px;font-weight:${i===p.key?"900":"600"};color:${i===p.key?Tt:"#888"};cursor:pointer">${p.label}${p.count?` (${p.count})`:""}</button>`).join("")}
    </div>
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:10px">
      ${i==="waiting"?Ko(o,$,c):i==="active"?Yo(g,c):Wo(w,c)}
    </div>
  </div>`,(r=document.getElementById("ml-create-btn"))==null||r.addEventListener("click",()=>Jo(e,t)),e.querySelectorAll(".ml-tab").forEach(p=>p.addEventListener("click",()=>$t(e,t,p.dataset.tab))),e.querySelectorAll("[data-league-id]").forEach(p=>p.addEventListener("click",()=>ht(e,t,p.dataset.leagueId))),e.querySelectorAll("[data-join]").forEach(p=>p.addEventListener("click",S=>{S.stopPropagation(),vn(e,t,p.dataset.join,p.dataset.type)})),e.querySelectorAll("[data-delete]").forEach(p=>p.addEventListener("click",S=>{S.stopPropagation(),fi(e,t,p.dataset.delete,p.dataset.name,i)}))}function Zt(e,t,i=!1){var s,d;const a=e.creator_id===t,c=e.pot||0,l=((d=(s=e.mini_league_members)==null?void 0:s[0])==null?void 0:d.count)||0;return`<div data-league-id="${e.id}" style="background:#fff;border-radius:12px;padding:14px 16px;box-shadow:0 1px 6px rgba(0,0,0,0.08);cursor:pointer;margin-bottom:8px">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        ${o&&p&&!S?'<button id="ml-next-day" class="btn btn-primary" style="width:100%;margin-top:12px">➡️ Journée suivante</button>':""}
        ${o&&p&&S?'<button id="ml-finish-btn" class="btn btn-primary" style="width:100%;margin-top:12px;background:#7a28b8">🏆 Terminer et distribuer le pot</button>':""}
      </div>`:""}
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          <thead><tr style="font-size:10px;color:#888;text-transform:uppercase;border-bottom:2px solid #f0f0f0">
            ...
            ${z>0&&s.status==="finished"?`<th style="text-align:right;padding:5px 0;color:${xt}">💰</th>`:""}
          </tr></thead>
          <tbody>${w.map((W,P)=>{const oe=z>0&&s.status==="finished"?P===0?Math.floor(z*.7):P===1?Math.floor(z*.2):P===2?Math.floor(z*.1):0:0;return`<tr style="border-bottom:1px solid #f5f5f5;${W.userId===l?"background:#f0fdf4;":""}">
              ...
              ${z>0&&s.status==="finished"?`<td style="text-align:right;font-weight:700;color:${xt}">${oe?oe.toLocaleString("fr")+" cr.":"—"}</td>`:""}
            </tr>`}).join("")}</tbody>
        </table>
      </div>`:""}
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        ${Array.from({length:Math.max(0,s.status==="active"?s.current_day-1:s.current_day)},(W,P)=>P+1).reverse().map(W=>{const P=(u||[]).filter(oe=>oe.matchday===W);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:#888;margin-bottom:6px">Journée ${W}</div>${P.map(oe=>Gi(oe,g,l,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
  ...
  </div>`;const F=s.status==="waiting"?"waiting":s.status==="active"?"active":"archived";if((O=document.getElementById("ml-back"))==null||O.addEventListener("click",()=>$t(e,t,F)),(G=document.getElementById("ml-start-btn"))==null||G.addEventListener("click",()=>Qo(e,t,s,g)),(q=document.getElementById("ml-next-day"))==null||q.addEventListener("click",()=>er(e,t,i)),(H=document.getElementById("ml-finish-btn"))==null||H.addEventListener("click",()=>wn(e,t,i,z,w,g)),(ie=document.getElementById("ml-join-now"))==null||ie.addEventListener("click",()=>vn(e,t,i,s.type)),(ue=document.getElementById("ml-leave-btn"))==null||ue.addEventListener("click",()=>Xo(e,t,i,r)),(se=document.getElementById("ml-delete-btn"))==null||se.addEventListener("click",()=>fi(e,t,i,s.name,"waiting")),(B=document.getElementById("ml-delete-now"))==null||B.addEventListener("click",()=>fi(e,t,i,s.name,F)),(ne=document.getElementById("ml-archive-btn"))==null||ne.addEventListener("click",()=>Zo(e,t,i)),e.querySelectorAll("[data-play-match]").forEach(W=>{W.addEventListener("click",()=>{const P=$.find(oe=>oe.id===W.dataset.playMatch);P&&t.navigate("match-mini-league",{mlMatchId:P.id,leagueId:i})})}),s.status==="finished"&&M){const W=w.findIndex(P=>P.userId===l);W>=0&&W<3&&M.prize_amount>0&&setTimeout(()=>tr(e,t,s,M,W),400)}}function Gi(e,t,i,a,c=!1){const l=g=>t.find(w=>w.id===g);if(e.is_bye){const g=l(e.home_id);return`<div style="padding:8px;border-radius:8px;background:#f9f9f9;margin-bottom:6px;font-size:12px;color:#888;text-align:center">🔵 ${(g==null?void 0:g.club_name)||(g==null?void 0:g.pseudo)||"?"} exempté(e)</div>`}const s=l(e.home_id),d=l(e.away_id),u=e.home_id===i||e.away_id===i,n=u&&e.status==="pending"&&a&&!c,o=e.status==="finished"?`${e.home_score} - ${e.away_score}`:"vs";return`<div style="display:flex;align-items:center;gap:8px;padding:10px;border-radius:8px;background:${u?"#f0fdf4":"#f9f9f9"};margin-bottom:6px;border:1px solid ${u?"#86efac":"#f0f0f0"}">
    ...
    <div style="background:linear-gradient(160deg,#1a0a2e,#3b1a6e);border-radius:20px;padding:32px 24px;max-width:320px;width:100%;text-align:center;color:#fff;box-shadow:0 8px 40px rgba(0,0,0,0.6)">
    ...
    </div>`,document.body.appendChild(g),(w=g.querySelector("#prize-close"))==null||w.addEventListener("click",()=>g.remove()),g.addEventListener("click",z=>{z.target===g&&g.remove()}),($=g.querySelector("#claim-prize-btn"))==null||$.addEventListener("click",async()=>{const{data:z}=await E.from("users").select("credits").eq("id",l.profile.id).single();await E.from("users").update({credits:((z==null?void 0:z.credits)||0)+n}).eq("id",l.profile.id),await E.from("mini_league_members").update({prize_claimed:!0}).eq("league_id",i.id).eq("user_id",l.profile.id),l.profile&&(l.profile.credits=((z==null?void 0:z.credits)||0)+n);const r=document.getElementById("nav-credits");r&&(r.textContent=`💰 ${l.profile.credits.toLocaleString("fr")}`),s(`💰 +${n.toLocaleString("fr")} cr. ajoutés à ton solde !`,"success"),g.remove(),ht(e,t,i.id)})}function ir(e,t){const a=e.length%2===0?[...e]:[...e,null],c=a.length;let l=a.slice(1);const s=[];for(let d=0;d<c-1;d++){const u=[],n=[a[0],...l];for(let o=0;o<c/2;o++){const g=n[o],w=n[c-1-o];g===null?u.push({bye:w}):w===null?u.push({bye:g}):u.push({home:g,away:w})}s.push(u),l=[l[l.length-1],...l.slice(0,-1)]}return t==="aller-retour"?[...s,...s.map(d=>d.map(u=>u.bye?u:{home:u.away,away:u.home}))]:s}function _n(e,t){const i={};return e.forEach(a=>{i[a.id]={userId:a.id,pseudo:a.pseudo,clubName:a.club_name||a.pseudo,played:0,won:0,drawn:0,lost:0,goalsFor:0,goalsAgainst:0,goalDiff:0,points:0}}),t.filter(a=>a.status==="finished"&&!a.is_bye&&a.home_score!=null).forEach(a=>{const c=i[a.home_id],l=i[a.away_id];!c||!l||(c.played++,l.played++,c.goalsFor+=a.home_score,c.goalsAgainst+=a.away_score,l.goalsFor+=a.away_score,l.goalsAgainst+=a.home_score,a.home_score>a.away_score?(c.won++,c.points+=3,l.lost++):a.home_score<a.away_score?(l.won++,l.points+=3,c.lost++):(c.drawn++,c.points++,l.drawn++,l.points++),c.goalDiff=c.goalsFor-c.goalsAgainst,l.goalDiff=l.goalsFor-l.goalsAgainst)}),Object.values(i).sort((a,c)=>c.points-a.points||c.goalDiff-a.goalDiff||c.goalsFor-a.goalsFor)}async function Pi(e,t,i,a){var c,l;t.state.params={...t.state.params,matchMode:"mini-league",mlMatchId:i,leagueId:a};try{const s=(l=(c=t==null?void 0:t.state)==null?void 0:c.profile)==null?void 0:l.id;try{(E.getChannels?E.getChannels():[]).forEach(u=>{const n=u.topic||"";(n.includes("matchmaking")||n.includes("pvp-match"))&&E.removeChannel(u)})}catch(d){console.warn("[FriendMatch] cleanup canaux:",d)}s&&(await E.rpc("cancel_matchmaking",{p_user_id:s}).catch(()=>{}),await E.from("matchmaking_queue").delete().eq("user_id",s).then(()=>{},()=>{}))}catch{}await ni(e,t,"random",({deckId:s,formation:d,starters:u,subsRaw:n,gcCardsEnriched:o,gcDefs:g})=>{const w=$=>nr(e,t,s,d,u,n,$,g||[],i);if(!o.length){w([]);return}ii(e,o,w)})}async function nr(e,t,i,a,c,l,s=[],d=[],u,n){const{state:o,navigate:g,toast:w}=t,$=o.profile.id;let z=!1,r=null;Qe(e);try{await E.rpc("cancel_matchmaking",{p_user_id:$})}catch{}try{await E.from("matchmaking_queue").delete().eq("user_id",$)}catch{}const{data:p}=await E.from("mini_league_matches").select("*, home:users!home_id(id,pseudo,club_name), away:users!away_id(id,pseudo,club_name)").eq("id",u).single();if(!p){w("Match introuvable","error"),Ke(e),g("mini-league");return}if(p.home_id!==$&&p.away_id!==$){w("Tu ne fais pas partie de ce match","error"),Ke(e),g("mini-league");return}const S=p.home_id===$,M=S?p.away:p.home,F=S?p.away_id:p.home_id,O=(q,H)=>{var ie;e.innerHTML=`
      <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:20px;background:linear-gradient(180deg,#0a1628,#1a0a2e);padding:24px;text-align:center;color:#fff">
        ...
        <button id="lobby-cancel" style="padding:10px 24px;border-radius:10px;border:1.5px solid rgba(255,255,255,0.2);background:transparent;color:rgba(255,255,255,0.6);font-size:13px;cursor:pointer">Annuler</button>
        <style>@keyframes lobbyPulse{0%,100%{opacity:1}50%{opacity:0.4}}</style>
      </div>`,(ie=document.getElementById("lobby-cancel"))==null||ie.addEventListener("click",()=>{z=!0,clearInterval(r),Ke(e),g("mini-league")})},G=async(q,H)=>{z=!0,clearInterval(r),await new Promise(ie=>setTimeout(ie,600)),e.isConnected&&or(e,t,q,H,s,d)};if(O(),S){let q=p.match_id;if(!q){const{data:ie,error:ue}=await E.from("matches").insert({home_id:$,away_id:F,home_deck_id:i,away_deck_id:null,status:"active",mode:"mini_league"}).select().single();if(ue||!ie){w("Erreur création match","error"),Ke(e),g("mini-league");return}q=ie.id,await E.from("mini_league_matches").update({match_id:q,status:"playing"}).eq("id",u)}const H=q;r=setInterval(async()=>{if(z){clearInterval(r);return}const{data:ie}=await E.from("matches").select("away_deck_id").eq("id",H).single();ie!=null&&ie.away_deck_id&&(clearInterval(r),G(H,!0))},1500)}else{let q=p.match_id;r=setInterval(async()=>{if(z){clearInterval(r);return}const{data:H}=await E.from("mini_league_matches").select("match_id").eq("id",u).single();H!=null&&H.match_id&&(clearInterval(r),q=H.match_id,await E.from("matches").update({away_deck_id:i}).eq("id",q),G(q,!1))},1500)}}async function or(e,t,i,a,c=[],l=[]){var A;const{state:s,navigate:d,toast:u}=t,n=((A=s.params)==null?void 0:A.leagueId)||null,o=a?"p1":"p2",g=a?"p2":"p1",w=(c||[]).map(f=>f.id),$=(c||[]).map(f=>({id:f.id,gc_type:f.gc_type,_gcDef:f._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:z}=await E.from("matches").select("*").eq("id",i).single();if(!z){u("Match introuvable","error"),d("mini-league");return}async function r(){const[{data:f},{data:m},{data:x},{data:y}]=await Promise.all([E.rpc("get_deck_for_match",{p_deck_id:z.home_deck_id}),E.rpc("get_deck_for_match",{p_deck_id:z.away_deck_id}),E.from("users").select("id,pseudo,club_name").eq("id",z.home_id).single(),E.from("users").select("id,pseudo,club_name").eq("id",z.away_id).single()]),h=I=>({cardId:I.card_id,position:I.position,id:I.id,firstname:I.firstname,name:I.surname_encoded,country_code:I.country_code,club_id:I.club_id,job:I.job,job2:I.job2,note_g:Number(I.note_g)||0,note_d:Number(I.note_d)||0,note_m:Number(I.note_m)||0,note_a:Number(I.note_a)||0,rarity:I.rarity,skin:I.skin,hair:I.hair,hair_length:I.hair_length,clubName:I.club_encoded_name||null,clubLogo:I.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}),v=((f==null?void 0:f.starters)||[]).map(I=>h(I)),_=((m==null?void 0:m.starters)||[]).map(I=>h(I)),T=(f==null?void 0:f.formation)||"4-4-2",L=(m==null?void 0:m.formation)||"4-4-2";return{p1Team:yt(v,T),p2Team:yt(_,L),p1Subs:((f==null?void 0:f.subs)||[]).map(I=>h(I)),p2Subs:((m==null?void 0:m.subs)||[]).map(I=>h(I)),p1Formation:T,p2Formation:L,p1Name:(x==null?void 0:x.club_name)||(x==null?void 0:x.pseudo)||"Joueur 1",p2Name:(y==null?void 0:y.club_name)||(y==null?void 0:y.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:a?w:[],gc_p2:a?[]:w,gcCardsFull_p1:a?$:[],gcCardsFull_p2:a?[]:$,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:l||[],lastActionAt:new Date().toISOString()}}let p=z.game_state&&Object.keys(z.game_state).length?z.game_state:null;if(!p)if(a){p=await r();const{data:f}=await E.from("matches").select("game_state").eq("id",i).single();!(f!=null&&f.game_state)||!Object.keys(f.game_state).length?await E.from("matches").update({game_state:p,turn_user_id:z.home_id}).eq("id",i):p=f.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let f=0;f<60&&!p;f++){await new Promise(x=>setTimeout(x,400));const{data:m}=await E.from("matches").select("game_state").eq("id",i).single();m!=null&&m.game_state&&Object.keys(m.game_state).length&&(p=m.game_state)}if(!p){u("Erreur de synchronisation","error"),d("mini-league");return}p.gc_p2=w,p.gcCardsFull_p2=$,await E.from("matches").update({game_state:p}).eq("id",i)}let S=!1,M=!1,F=!1,O=null,G=!1;const q=new Set,H=new Set;function ie(f){var I,U;try{E.removeChannel(ue)}catch{}const m=p[o+"Score"]||0,x=p[g+"Score"]||0;if(!F){F=!0;const D=f.winner_id||(m>x?s.profile.id:x>m?"opp":null),C=D===s.profile.id?"win":D?"loss":null;C&&Vi(()=>Promise.resolve().then(()=>Hn),void 0).then(V=>V.applyOwnEvolution(s.profile.id,C)).catch(()=>{})}if(!M){const D=p.p1Score||0,C=p.p2Score||0,V=(p.usedGc_p1||[]).length,ee=(p.usedGc_p2||[]).length,X=f.winner_id||(D>C?z.home_id:C>D?z.away_id:null);(X?s.profile.id===X:s.profile.id<(a?z.away_id:z.home_id))&&(M=!0,Wi({player1Id:z.home_id,player2Id:z.away_id,score1:D,score2:C,gc1:V,gc2:ee}).catch(re=>console.warn("[FriendMatch] updateStats:",re)))}let y,h;f.winner_id?(y=f.winner_id===s.profile.id,h=!1):f.forfeit?(y=m>x,h=!1):(h=m===x,y=m>x),(I=document.getElementById("pvp-end-overlay"))==null||I.remove();const v=document.createElement("div");v.id="pvp-end-overlay",v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const _=h?"🤝":y?"🏆":"😞",T=h?"MATCH NUL":y?"VICTOIRE !":"DÉFAITE",L=h?"#fff":y?"#FFD700":"#ff6b6b";v.innerHTML=`
      <div style="font-size:64px">${_}</div>
      <div style="font-size:26px;font-weight:900;color:${L}">${T}</div>
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">🏆 Retour à la Mini League</button>`,document.body.appendChild(v),(U=v.querySelector("#pvp-end-home"))==null||U.addEventListener("click",()=>{v.remove(),Ke(e),n?d("mini-league",{openLeagueId:n}):d("mini-league")})}const ue=E.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},f=>{const m=f.new;try{if(m.status==="finished"||m.forfeit){if(S||(S=!0,O&&(clearInterval(O),O=null),m.game_state&&(p=m.game_state),p.phase==="finished"&&!m.forfeit&&document.getElementById("pvp-home")))return;ie(m);return}if(m.game_state){const x=p;p=m.game_state;const y=p._lastGC;if(y&&y.seq&&!H.has(y.seq)&&(H.add(y.seq),y.by!==o)){Z(y.type,y.by,()=>ye());return}const h=x[o+"Score"]||0,v=x[g+"Score"]||0,_=p[o+"Score"]||0,T=p[g+"Score"]||0,L=_>h,I=T>v;if((L||I)&&!q.has(p.round)){q.add(p.round);const U=[...p.log||[]].reverse().find(C=>C.isGoal),D=((U==null?void 0:U.homePlayers)||[]).map(C=>({name:C.name,note:C.note,portrait:C.portrait,job:C.job}));J(D,_,T,L,()=>ye());return}ye()}}catch(x){console.error("[PvP] crash:",x)}}).subscribe();async function se(f){Object.assign(p,f),p.lastActionAt=new Date().toISOString();const{error:m}=await E.from("matches").update({game_state:p}).eq("id",i);m&&u("Erreur de synchronisation","error");try{ye()}catch(x){console.error("[PvP] renderPvpScreen crash:",x)}}async function B(){if(S)return;S=!0,O&&(clearInterval(O),O=null);const f=a?z.away_id:z.home_id,m=a?"p2":"p1",x=a?"p1":"p2",y={...p,[m+"Score"]:3,[x+"Score"]:0,phase:"finished"};try{await E.from("matches").update({status:"finished",forfeit:!0,winner_id:f,home_score:y.p1Score||0,away_score:y.p2Score||0,game_state:y}).eq("id",i)}catch{}try{E.removeChannel(ue)}catch{}setTimeout(()=>{Ke(e),d("mini-league")},800)}const ne={BOOST_STAT:({value:f=1,count:m=1,roles:x=[]},y,h)=>{const v=y[o+"Team"],_=Object.entries(v).filter(([T])=>!x.length||x.includes(T)).flatMap(([T,L])=>L.filter(I=>!I.used).map(I=>({...I,_line:T})));if(!_.length){y.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),h(y);return}W(_,m,`Choisir ${m} joueur(s) à booster (+${f})`,T=>{T.forEach(L=>{const I=(v[L._line]||[]).find(U=>U.cardId===L.cardId);I&&(I.boost=(I.boost||0)+f,y.log.push({text:`⚡ +${f} sur ${I.name}`,type:"info"}))}),y[o+"Team"]=v,h(y)})},DEBUFF_STAT:({value:f=1,count:m=1,roles:x=[],target:y="ai"},h,v)=>{const _=y==="home"?o:g,T=h[_+"Team"],L=y==="home"?"allié":"adverse",I=Object.entries(T).filter(([U])=>!x.length||x.includes(U)).flatMap(([U,D])=>D.filter(C=>!C.used).map(C=>({...C,_line:U})));if(!I.length){h.log.push({text:`🎯 Aucun joueur ${L}`,type:"info"}),v(h);return}W(I,m,`Choisir ${m} joueur(s) ${L}(s) (-${f})`,U=>{U.forEach(D=>{const C=(T[D._line]||[]).find(V=>V.cardId===D.cardId);C&&(C.boost=(C.boost||0)-f,h.log.push({text:`🎯 -${f} sur ${C.name}`,type:"info"}))}),h[_+"Team"]=T,v(h)})},GRAY_PLAYER:({count:f=1,roles:m=[],target:x="ai"},y,h)=>{const v=x==="home"?o:g,_=y[v+"Team"],T=x==="home"?"allié":"adverse",L=Object.entries(_).filter(([I])=>!m.length||m.includes(I)).flatMap(([I,U])=>U.filter(D=>!D.used).map(D=>({...D,_line:I})));if(!L.length){y.log.push({text:`❌ Aucun joueur ${T}`,type:"info"}),h(y);return}W(L,f,`Choisir ${f} joueur(s) ${T}(s) à exclure`,I=>{I.forEach(U=>{const D=(_[U._line]||[]).find(C=>C.cardId===U.cardId);D&&(D.used=!0,y.log.push({text:`❌ ${D.name} exclu !`,type:"info"}))}),y[v+"Team"]=_,h(y)})},REVIVE_PLAYER:({count:f=1,roles:m=[]},x,y)=>{const h=x[o+"Team"],v=Object.entries(h).filter(([_])=>!m.length||m.includes(_)).flatMap(([_,T])=>T.filter(L=>L.used).map(L=>({...L,_line:_})));if(!v.length){x.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),y(x);return}W(v,f,`Choisir ${f} joueur(s) à ressusciter`,_=>{_.forEach(T=>{const L=(h[T._line]||[]).find(I=>I.cardId===T.cardId);L&&(L.used=!1,x.log.push({text:`💫 ${L.name} ressuscité !`,type:"info"}))}),x[o+"Team"]=h,y(x)})},REMOVE_GOAL:({},f,m)=>{const x=g+"Score";f[x]>0?(f[x]--,f.log.push({text:"🚫 Dernier but annulé !",type:"info"})):f.log.push({text:"🚫 Aucun but à annuler",type:"info"}),m(f)},ADD_GOAL_DRAW:({},f,m)=>{f[o+"Score"]===f[g+"Score"]?(f[o+"Score"]++,f.log.push({text:"🎯 But bonus !",type:"info"})):f.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),m(f)},ADD_SUB:({value:f=1},m,x)=>{m.maxSubs=(m.maxSubs||3)+f,m.log.push({text:`🔄 +${f} remplacement(s)`,type:"info"}),x(m)},CUSTOM:({},f,m)=>m(f)};function W(f,m,x,y){const h=document.createElement("div");h.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let v=[];function _(){var L,I;const T=f.map(U=>{const D={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[U._line]||"#555",C=ke(U,U._line)+(U.boost||0),ee=v.find(te=>te.cardId===U.cardId)?"#FFD700":"rgba(255,255,255,0.25)",X=U.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${U.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${ee};background:${D};overflow:hidden;cursor:pointer;${X}">
        ...
        </div>`,(L=h.querySelector("#pp-close"))==null||L.addEventListener("click",()=>h.remove()),h.querySelectorAll(".pp-item").forEach(U=>{U.addEventListener("click",()=>{const D=U.dataset.cid,C=f.find(ee=>ee.cardId===D),V=v.findIndex(ee=>ee.cardId===D);C&&(V>-1?v.splice(V,1):v.length<m&&v.push(C),_())})}),(I=h.querySelector("#pp-confirm"))==null||I.addEventListener("click",()=>{h.remove(),y(v)})}_(),document.body.appendChild(h)}async function P(f,m){const y=(p["gcCardsFull_"+o]||[]).find(T=>T.id===f),h=(y==null?void 0:y._gcDef)||(p.gcDefs||[]).find(T=>{var L;return T.name===m||((L=T.name)==null?void 0:L.toLowerCase().trim())===(m==null?void 0:m.toLowerCase().trim())}),v=[...p["usedGc_"+o]||[],f],_={type:m,by:o,seq:(p._gcAnimSeq||0)+1};H.add(_.seq),Z(m,o,async()=>{if(h!=null&&h.effect_type&&h.effect_type!=="CUSTOM"){const L=ne[h.effect_type];if(L){const I={...p};L(h.effect_params||{},I,async U=>{U["usedGc_"+o]=v,U._lastGC=_,U._gcAnimSeq=_.seq,await se(U)});return}}const T={...p};switch(m){case"Remplacement+":T.maxSubs=(T.maxSubs||3)+1,T.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const L=g+"Score";T[L]>0&&(T[L]--,T.log.push({text:"🚫 But annulé",type:"info"}));break}}T["usedGc_"+o]=v,T._lastGC=_,T._gcAnimSeq=_.seq,await se(T)})}function oe(f,m){const x="usedCardIds_"+f,y=new Set(p[x]||[]);m.forEach(h=>y.add(h)),p[x]=[...y]}function fe(){for(const f of["p1","p2"]){const m=new Set(p["usedCardIds_"+f]||[]),x=p[f+"Team"];if(x)for(const y of["GK","DEF","MIL","ATT"])(x[y]||[]).forEach(h=>{h.used=m.has(h.cardId)})}}function ye(){var it,ft,ae,K,ce,ge;if(p.phase==="reveal")return Ae();if(p.phase==="midfield")return he();if(p.phase==="finished")return k();const f=p[o+"Team"],m=p[g+"Team"];fe();const x=p[o+"Score"],y=p[g+"Score"],h=p[o+"Name"],v=p[g+"Name"],_=p.phase===o+"-attack",T=p.phase===o+"-defense",L=Array.isArray(p["selected_"+o])?p["selected_"+o]:[],I=L.map(Y=>Y.cardId),U=window.innerWidth>=700,D=p[o+"Subs"]||[],C=p["usedSubIds_"+o]||[],V=D.filter(Y=>!C.includes(Y.cardId)),ee=p["gcCardsFull_"+o]||[],X=p["usedGc_"+o]||[],te=ee.filter(Y=>!X.includes(Y.id)),re=p.boostOwner===o&&!p.boostUsed,de=!["GK","DEF","MIL","ATT"].some(Y=>(m[Y]||[]).some(xe=>!xe.used)),me=[...f.MIL||[],...f.ATT||[]].filter(Y=>!Y.used),we=_&&de&&me.length===0?[...f.GK||[],...f.DEF||[]].filter(Y=>!Y.used).map(Y=>Y.cardId):[];function _e(Y,xe,Te){var ki,Ei;const be=Y._gcDef,et={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[be==null?void 0:be.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",ot={purple:"#b06ce0",light_blue:"#00d4ef"}[be==null?void 0:be.color]||"#b06ce0",st=(be==null?void 0:be.name)||Y.gc_type,rt=(be==null?void 0:be.effect)||((ki=Ve[Y.gc_type])==null?void 0:ki.desc)||"",ct=be!=null&&be.image_url?`/manager-wars/icons/${be.image_url}`:null,bt=((Ei=Ve[Y.gc_type])==null?void 0:Ei.icon)||"⚡",vt=Math.round(Te*.22),gt=Math.round(Te*.22),kt=Te-vt-gt,En=st.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${Y.id}" data-gc-type="${Y.gc_type}"
    ...
    </div>`,o==="p1"&&setTimeout(async()=>{await se({phase:"midfield"})},5e3)}let ve=!1;function he(){if(ve)return;const f=p[o+"Team"].MIL||[],m=p[g+"Team"].MIL||[];function x(te){return te.reduce((re,de)=>re+ke(de,"MIL"),0)}function y(te){let re=0;for(let de=0;de<te.length-1;de++){const me=at(te[de],te[de+1]);me==="#00ff88"?re+=2:me==="#FFD700"&&(re+=1)}return re}const h=x(f)+y(f),v=x(m)+y(m),_=h>=v;function T(te,re,de){return`<div id="duel-row-${de}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(C);let V=!1;const ee=()=>{V||(V=!0,C.remove(),setTimeout(()=>x&&x(),50))};C.addEventListener("click",ee),setTimeout(ee,3e3)}function N(f,m){var C,V,ee,X;const y=(p["gcCardsFull_"+o]||[]).find(te=>te.id===f),h=y==null?void 0:y._gcDef,v={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[h==null?void 0:h.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",_={purple:"#b06ce0",light_blue:"#00d4ef"}[h==null?void 0:h.color]||"#b06ce0",T=(h==null?void 0:h.name)||m,L=(h==null?void 0:h.effect)||((C=Ve[m])==null?void 0:C.desc)||"Carte spéciale.",I=h!=null&&h.image_url?`/manager-wars/icons/${h.image_url}`:null,U=((V=Ve[m])==null?void 0:V.icon)||"⚡",D=document.createElement("div");D.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",D.innerHTML=`
      ...
      <div style="display:flex;gap:12px;width:190px">
        <button id="pvp-gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
        <button id="pvp-gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
      </div>`,document.body.appendChild(D),(ee=D.querySelector("#pvp-gc-back"))==null||ee.addEventListener("click",()=>D.remove()),(X=D.querySelector("#pvp-gc-use"))==null||X.addEventListener("click",()=>{D.remove(),P(f,m)})}function R(){var y;const f=p[o+"Team"],m=Object.entries(f).flatMap(([h,v])=>(v||[]).filter(_=>!_.used).map(_=>({..._,_line:h})));if(!m.length)return;const x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",x.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
      ...
      </div>`,document.body.appendChild(x),(y=x.querySelector("#bp-close"))==null||y.addEventListener("click",()=>x.remove()),x.querySelectorAll(".bp-item").forEach(h=>{h.addEventListener("click",async()=>{const v=h.dataset.cid,_=m.find(L=>L.cardId===v);if(!_)return;const T=(f[_._line]||[]).find(L=>L.cardId===v);T&&(T.boost=(T.boost||0)+p.boostValue),x.remove(),await se({[o+"Team"]:f,boostUsed:!0})})})}function Q(f=null){var V,ee;if(!(p.phase===o+"-attack")){u("Remplacement uniquement avant votre attaque","warning");return}const x=p[o+"Team"],y=p["usedSubIds_"+o]||[],h=p.maxSubs||3;if(y.length>=h){u(`Maximum ${h} remplacements atteint`,"warning");return}const v=Object.entries(x).flatMap(([X,te])=>(te||[]).filter(re=>re.used).map(re=>({...re,_line:X}))),_=(p[o+"Subs"]||[]).filter(X=>!y.includes(X.cardId));if(!v.length){u("Aucun joueur utilisé à remplacer","warning");return}if(!_.length){u("Aucun remplaçant disponible","warning");return}let T=Math.max(0,v.findIndex(X=>X.cardId===f));const L=((V=v[T])==null?void 0:V._line)||((ee=v[T])==null?void 0:ee.job);let I=Math.max(0,_.findIndex(X=>X.job===L)),U=!1;const D=document.createElement("div");D.id="pvp-sub-overlay",D.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function C(){var _e,$e,Ee,ze,Me,Ne;const X=v[T],te=_[I],re=Math.min(130,Math.round((window.innerWidth-90)/2)),de=Math.round(re*1.35),me=Le=>`background:rgba(255,255,255,0.12);border:none;color:${Le?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Le?"default":"pointer"};flex-shrink:0`;D.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="width:70px;height:70px;border-radius:50%;background:${y[L.job]||"#555"};border:3px solid ${I};position:relative;overflow:hidden;margin:0 auto">
        ${Ge(L)?`<img src="${Ge(L)}" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover">`:""}
      </div>
      ...
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(f),(m=f.querySelector("#pvp-opp-close"))==null||m.addEventListener("click",()=>f.remove())}function je(){var y;const f=p.log||[],m=document.createElement("div");m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column";const x=h=>{var _,T,L;if(h.type==="duel"){const I=h.isGoal,U=h.homeScored&&o==="p1"||!h.homeScored&&I&&o==="p2",D=h.homeScored?"#FFD700":I?"#ff6b6b":"rgba(255,255,255,0.3)",C=I?U?"⚽ BUT !":"⚽ BUT adversaire !":(_=h.homePlayers)!=null&&_.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${I?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${D};margin-bottom:4px">
          ...
          ${(L=h.aiPlayers)!=null&&L.length?`<div style="opacity:0.7">${nt(h.aiPlayers,"#ff6b6b",h.aiTotal)}</div>`:""}
      ...
      </div>`,document.body.appendChild(m),(y=m.querySelector("#pvp-hist-close"))==null||y.addEventListener("click",()=>m.remove())}async function Se(){if(p.phase!==o+"-attack")return;const f=o==="p1"?"p2":"p1",m=(p.round||0)+1,x=[...p.log||[]];x.push({type:"info",text:`⏭️ ${p[o+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const y=Je(p),h=Ue(f),v=y||!h?"finished":f+"-attack";await se({["selected_"+o]:[],modifiers:{...p.modifiers,[o]:{}},pendingAttack:null,phase:v,attacker:f,round:m,log:x}),v==="finished"&&await b(p)}async function Pe(){const f=p[o+"Team"],m=!["GK","DEF","MIL","ATT"].some(v=>(p[g+"Team"][v]||[]).some(_=>!_.used)),x=(p["selected_"+o]||[]).map(v=>{const _=(f[v._role]||[]).find(C=>C.cardId===v.cardId)||v,T=m&&["GK","DEF"].includes(v._role),L=f[v._role]||[],I=L.findIndex(C=>C.cardId===v.cardId),U=lt(L.length),D=I>=0?U[I]:_._col??1;return{..._,_line:v._role,_col:D,...T?{note_a:Math.max(1,Number(_.note_a)||0)}:{}}});if(!x.length)return;const y=Pt(x,p.modifiers[o]||{});oe(o,x.map(v=>v.cardId)),x.forEach(v=>{const _=(f[v._role]||[]).find(T=>T.cardId===v.cardId);_&&(_.used=!0)}),p["selected_"+o]=[],ye();const h=[...p.log||[]];if(m){const v=(p[o+"Score"]||0)+1,_=x.map(D=>({name:D.name,note:ke(D,D._line||"ATT"),portrait:Ge(D),job:D.job}));h.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:_,homeTotal:y.total,aiTotal:0});const T=(p.round||0)+1,L=o==="p1"?"p2":"p1",I={...p,[o+"Team"]:f,[o+"Score"]:v},U=Je(I);q.add(T),J(_,v,p[g+"Score"]||0,!0,async()=>{await se({[o+"Team"]:f,[o+"Score"]:v,["selected_"+o]:[],modifiers:{...p.modifiers,[o]:{}},pendingAttack:null,phase:U?"finished":L+"-attack",attacker:L,round:T,log:h}),U&&await b({...p,[o+"Score"]:v})});return}h.push({type:"pending",text:`⚔️ ${p[o+"Name"]} attaque (${y.total})`}),await se({[o+"Team"]:f,[g+"Team"]:p[g+"Team"],pendingAttack:{...y,players:x,side:o},["selected_"+o]:[],modifiers:{...p.modifiers,[o]:{}},phase:g+"-defense",log:h})}async function Oe(){const f=p[o+"Team"],m=(p["selected_"+o]||[]).map(X=>{const te=(f[X._role]||[]).find(_e=>_e.cardId===X.cardId)||X,re=f[X._role]||[],de=re.findIndex(_e=>_e.cardId===X.cardId),me=lt(re.length),we=de>=0?me[de]:te._col??1;return{...te,_line:X._role,_col:we}}),x=Nt(m,p.modifiers[o]||{});oe(o,m.map(X=>X.cardId)),m.forEach(X=>{const te=(f[X._role]||[]).find(re=>re.cardId===X.cardId);te&&(te.used=!0)}),p["selected_"+o]=[],ye();const y=Rt(p.pendingAttack.total,x.total,p.modifiers[o]||{}),h=p.pendingAttack.side,v=y==="attack"||(y==null?void 0:y.goal),_=h==="p1"?"p2":"p1",T=(p.round||0)+1,L=(p.pendingAttack.players||[]).map(X=>({name:X.name,note:ke(X,X._line||"ATT"),portrait:Ge(X),job:X.job})),I=[...p.log||[]];I.push({type:"duel",isGoal:v,homeScored:v&&h===o,text:v?`⚽ BUT de ${p[h+"Name"]} ! (${p.pendingAttack.total} vs ${x.total})`:`✋ Attaque stoppée (${p.pendingAttack.total} vs ${x.total})`,homePlayers:L,aiPlayers:m.map(X=>({name:X.name,note:ke(X,X._line||"DEF"),portrait:Ge(X),job:X.job})),homeTotal:p.pendingAttack.total,aiTotal:x.total});const U=v?(p[h+"Score"]||0)+1:p[h+"Score"]||0,D={...p,[o+"Team"]:f,[h+"Score"]:U},C=Je(D),V=C?"finished":_+"-attack",ee=async()=>{await se({[o+"Team"]:f,[g+"Team"]:p[g+"Team"],[h+"Score"]:U,["selected_"+o]:[],modifiers:{...p.modifiers,[o]:{}},pendingAttack:null,phase:V,attacker:_,round:T,log:I}),(V==="finished"||C)&&await b({...p,[h+"Score"]:U})};if(v){const X=h===o,te=X?U:p[o+"Score"]||0,re=X?p[g+"Score"]||0:U;q.add(T),J(L,te,re,X,ee)}else await ee()}function Ce(f){return["MIL","ATT"].some(m=>(f[m]||[]).some(x=>!x.used))}function Ie(f){return["GK","DEF","MIL","ATT"].some(m=>(f[m]||[]).some(x=>!x.used))}function qe(f){return Ie(f)&&!Ce(f)}function Ue(f){const m=p[f+"Team"],x=p[(f==="p1"?"p2":"p1")+"Team"];return!!(Ce(m)||!Ie(x)&&qe(m))}function Je(f){const m=["MIL","ATT"].some(I=>(f.p1Team[I]||[]).some(U=>!U.used)),x=["MIL","ATT"].some(I=>(f.p2Team[I]||[]).some(U=>!U.used)),y=Ie(f.p1Team),h=Ie(f.p2Team);return!m&&!(!h&&y)&&(!x&&!(!y&&h))}function j(f){const m=f.p1Score||0,x=f.p2Score||0;return m===x?null:m>x?z.home_id:z.away_id}async function b(f){try{const m=j(f),x=m?z.home_id===m?z.away_id:z.home_id:null,y=f.p1Score||0,h=f.p2Score||0;await E.from("matches").update({status:"finished",winner_id:m,home_score:y,away_score:h}).eq("id",i);const{data:v}=await E.from("mini_league_matches").select("id, league_id, matchday").eq("match_id",i).single();if(v){await E.from("mini_league_matches").update({home_score:y,away_score:h,status:"finished"}).eq("id",v.id);const{data:_}=await E.from("mini_league_matches").select("id, status").eq("league_id",v.league_id).eq("matchday",v.matchday);if((_||[]).every(L=>L.status==="finished"||L.status==="bye")){const{data:L}=await E.from("mini_leagues").select("current_day,total_days").eq("id",v.league_id).single();if(L){const I=(L.current_day||0)+1,U=I>(L.total_days||0);await E.from("mini_leagues").update({current_day:U?L.total_days:I,status:U?"finished":"active"}).eq("id",v.league_id)}}}m&&x&&Qt(m,x).catch(()=>{})}catch(m){console.error("[ML] finishMatch:",m)}}function k(){var h;if(S&&document.getElementById("pvp-end-overlay"))return;S=!0;const f=p[o+"Score"],m=p[g+"Score"],x=f>m,y=f===m;Qe(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
  ...
  </div>`;let u="buy";const n=()=>{var r,p,S,M,F,O,G;return{name:(((r=document.getElementById("flt-name"))==null?void 0:r.value)||"").toLowerCase().trim(),club:(((p=document.getElementById("flt-club"))==null?void 0:p.value)||"").toLowerCase().trim(),country:(((S=document.getElementById("flt-country"))==null?void 0:S.value)||"").toLowerCase().trim(),job:((M=document.getElementById("flt-job"))==null?void 0:M.value)||"",rarity:((F=document.getElementById("flt-rarity"))==null?void 0:F.value)||"",note1:parseInt((O=document.getElementById("flt-note1"))==null?void 0:O.value)||0,note2:parseInt((G=document.getElementById("flt-note2"))==null?void 0:G.value)||0}};function o(r){const p=n();return r.filter(S=>{var ie,ue;const M=(ie=S.card)==null?void 0:ie.player;if(!M)return!1;const F=`${M.firstname} ${M.surname_encoded}`.toLowerCase(),O=(((ue=M.clubs)==null?void 0:ue.encoded_name)||"").toLowerCase(),G=(M.country_code||"").toLowerCase(),q=Et(M,M.job),H=M.job2?Et(M,M.job2):0;return!(p.name&&!F.includes(p.name)||p.club&&!O.includes(p.club)||p.country&&!G.includes(p.country)||p.job&&M.job!==p.job||p.rarity&&M.rarity!==p.rarity||p.note1&&q<p.note1||p.note2&&H<p.note2)})}function g(r){var O,G,q;const p=(O=r.card)==null?void 0:O.player;if(!p)return"";const S=Et(p,p.job),M=p.job2?Et(p,p.job2):0,F=(i.profile.credits||0)>=r.price;return`<div class="card-panel" style="display:flex;align-items:center;gap:8px;padding:10px 12px;overflow:hidden;padding-left:6px">
      ${Ni(p.rarity)}
      ${Vt(S,p.job)}
      ${Vt(M,p.job2||null)}
      ${Ri(p.country_code)}
    ...
    </div>`}function w(r){var O,G,q;const p=(O=r.card)==null?void 0:O.player;if(!p)return"";const S=Et(p,p.job),M=p.job2?Et(p,p.job2):0,F=r.status==="sold";return`<div class="card-panel" style="display:flex;align-items:center;gap:8px;padding:10px 12px;overflow:hidden;padding-left:6px;${F?"opacity:0.7":""}">
      ${Ni(p.rarity)}
      ${Vt(S,p.job)}
      ${Vt(M,p.job2||null)}
      ${Ri(p.country_code)}
    ...
    </div>`}function $(){const r=document.getElementById("mkt-content"),p=document.getElementById("mkt-filters");if(r){if(p.style.display=u==="buy"?"flex":"none",u==="buy"){const S=o(s);r.innerHTML=S.length?S.map(g).join(""):'<div style="text-align:center;color:#aaa;padding:40px">Aucune carte trouvée.</div>'}else{const S=d.filter(F=>F.status==="active").sort((F,O)=>new Date(O.listed_at)-new Date(F.listed_at)),M=d.filter(F=>F.status==="sold").sort((F,O)=>new Date(O.sold_at||O.listed_at)-new Date(F.sold_at||F.listed_at));r.innerHTML=(S.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:4px 0 6px;text-transform:uppercase;letter-spacing:1px">🟢 En vente (${S.length})</div>`+S.map(w).join(""):"")+(M.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:12px 0 6px;text-transform:uppercase;letter-spacing:1px">✅ Ventes réussies (${M.length})</div>`+M.map(w).join(""):"")+(!S.length&&!M.length?'<div style="text-align:center;color:#aaa;padding:40px">Aucune vente pour le moment.</div>':"")}r.querySelectorAll("[data-buy]").forEach(S=>S.addEventListener("click",()=>sr(S.dataset.buy,s,e,t))),r.querySelectorAll("[data-cancel]").forEach(S=>S.addEventListener("click",()=>lr(S.dataset.cancel,e,t)))}}e.querySelectorAll(".mkt-tab").forEach(r=>{r.addEventListener("click",()=>{u=r.dataset.tab,e.querySelectorAll(".mkt-tab").forEach(p=>{const S=p===r;p.style.background=S?"var(--green)":"#fff",p.style.color=S?"#fff":"var(--gray-600)",p.style.borderColor=S?"var(--green)":"var(--gray-200)"}),$()})});let z;["flt-name","flt-club","flt-country","flt-job","flt-rarity","flt-note1","flt-note2"].forEach(r=>{var p;(p=document.getElementById(r))==null||p.addEventListener("input",()=>{clearTimeout(z),z=setTimeout($,250)})}),$()}async function sr(e,t,i,a){const{state:c,toast:l,refreshProfile:s}=a,d=t.find(o=>o.id===e);if(!d)return;const u=d.price;if((c.profile.credits||0)<u){l("Crédits insuffisants","error");return}dr(d,async()=>{const{error:o}=await E.rpc("buy_market_card",{p_listing_id:e,p_buyer_id:c.profile.id});if(o){l("Erreur achat : "+o.message,"error");return}await s();const g=document.getElementById("nav-credits");g&&(g.textContent=`💰 ${(c.profile.credits||0).toLocaleString("fr")}`),l("✅ Carte achetée !","success"),await $i(i,a)})}function dr(e,t){var s;const i=(s=e.card)==null?void 0:s.player,a=i?`${i.firstname} ${i.surname_encoded}`:"cette carte",c=document.createElement("div");c.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",c.innerHTML=`
  ...
  `}async function pr(e,{state:t,navigate:i,toast:a}){const c=t.profile;if(!c)return;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:l}=await E.from("matches").select(`id,home_id,away_id,home_score,away_score,status,mode,winner_id,created_at,played_at,
      ...
      away:users!away_id(pseudo,club_name)`).or(`home_id.eq.${c.id},away_id.eq.${c.id}`).order("created_at",{ascending:!1}).limit(50),s={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},d=(l||[]).filter(o=>o.status==="finished"),u=(l||[]).filter(o=>o.status==="in_progress");function n(o){const g=o.home_id===c.id;g?o.home_score:o.away_score,g?o.away_score:o.home_score;const w=o.winner_id===c.id,$=o.home_score===o.away_score&&o.status==="finished",z=o.status!=="finished"?"…":$?"N":w?"V":"D",r=o.status!=="finished"||$?"#888":w?"#1A6B3C":"#c0392b";let p=s[o.mode]||o.mode;o.away_id===null&&!p.startsWith("IA")&&(p="IA");const M=o.home_id===c.id?o.away:o.home;let F;o.away_id===null?F=p:M?F=`${M.club_name||M.pseudo} (${M.pseudo})`:F="Adversaire";let O="";o.status==="in_progress"&&Date.now()-new Date(o.created_at).getTime()>3600*1e3&&(O=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const G=new Date(o.created_at),q=G.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+G.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),H=o.status==="finished"?`${o.home_score} - ${o.away_score}`:`${o.home_score||0} - ${o.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
