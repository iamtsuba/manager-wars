...
import{s as v,n as Ut,o as Fi,F as Pi,l as Jt,k as Qt,r as ze,j as zt,p as Li,h as Gi,q as Wn,t as Xn,T as vi,m as Jn,u as Qn,b as Zn}from"./special-cards-DGU8wjVP.js";const eo="/";function tn(e,{navigate:t,toast:i}){let n="login";const r=()=>{var a,s,d,c,l,g;const o=n==="login";e.innerHTML=`
    <div style="
    ...
    </style>`,(a=document.getElementById("tab-login-btn"))==null||a.addEventListener("click",()=>{n="login",r()}),(s=document.getElementById("tab-reg-btn"))==null||s.addEventListener("click",()=>{n="register",r()}),o?((d=document.getElementById("login-password"))==null||d.addEventListener("keydown",x=>{var f;x.key==="Enter"&&((f=document.getElementById("login-btn"))==null||f.click())}),(c=document.getElementById("login-btn"))==null||c.addEventListener("click",async()=>{const x=document.getElementById("login-email").value.trim(),f=document.getElementById("login-password").value,y=document.getElementById("login-error");if(y.textContent="",!x||!f){y.textContent="Remplissez tous les champs.";return}const p=document.getElementById("login-btn");p.textContent="⏳ Connexion…",p.disabled=!0;const{error:u}=await v.auth.signInWithPassword({email:x,password:f});if(p.textContent="⚽ Se connecter",p.disabled=!1,u){y.textContent=u.message.includes("Invalid")?"Email ou mot de passe incorrect.":u.message;return}window.location.reload()})):((l=document.getElementById("reg-confirm"))==null||l.addEventListener("keydown",x=>{var f;x.key==="Enter"&&((f=document.getElementById("reg-btn"))==null||f.click())}),(g=document.getElementById("reg-btn"))==null||g.addEventListener("click",async()=>{const x=document.getElementById("reg-email").value.trim(),f=document.getElementById("reg-password").value,y=document.getElementById("reg-confirm").value,p=document.getElementById("reg-error");if(p.textContent="",!x||!f||!y){p.textContent="Remplissez tous les champs.";return}if(f.length<6){p.textContent="Mot de passe trop court (min. 6 caractères).";return}if(f!==y){p.textContent="Les mots de passe ne correspondent pas.";return}const u=document.getElementById("reg-btn");u.textContent="⏳ Création…",u.disabled=!0;const{error:b}=await v.auth.signUp({email:x,password:f});if(u.textContent="🚀 Créer mon compte",u.disabled=!1,b){p.textContent=b.message;return}i("Compte créé ! Connecte-toi pour commencer.","success",4e3),n="login",r(),setTimeout(()=>{const w=document.getElementById("login-email");w&&(w.value=x)},50)}))};r()}function to(e,{state:t,navigate:i,toast:n,refreshProfile:r}){let o="#1A6B3C",a="#D4A017";e.innerHTML=`
  ...
  `;function s(){var y;const c=document.getElementById("logo-preview"),l=document.getElementById("logo-initials"),g=((y=document.getElementById("setup-club"))==null?void 0:y.value)||"MW",x=g.trim().split(" ").filter(Boolean),f=x.length>=2?(x[0][0]+x[1][0]).toUpperCase():g.slice(0,2).toUpperCase();c&&(c.style.background=a,c.style.borderColor=o),l&&(l.textContent=f,l.style.color=o)}document.getElementById("color1").addEventListener("input",c=>{o=c.target.value,document.getElementById("swatch1").style.background=o,s()}),document.getElementById("color2").addEventListener("input",c=>{a=c.target.value,document.getElementById("swatch2").style.background=a,s()});function d(c){document.querySelectorAll(".setup-step").forEach(l=>l.classList.remove("active")),document.getElementById(`step-${c}`).classList.add("active"),document.getElementById("step-num").textContent=c,document.getElementById("progress-fill").style.width=`${Math.round(c/3*100)}%`,c===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("step1-error");if(l.textContent="",c.length<3){l.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:g}=await v.from("users").select("id").eq("pseudo",c).maybeSingle();if(g){l.textContent="Ce pseudo est déjà pris.";return}d(2)}),document.getElementById("step2-back").addEventListener("click",()=>d(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const c=document.getElementById("setup-club").value.trim(),l=document.getElementById("step2-error");if(l.textContent="",c.length<2){l.textContent="Nom trop court (min. 2 caractères).";return}const{data:g}=await v.from("users").select("id").eq("club_name",c).maybeSingle();if(g){l.textContent="Ce nom de club est déjà pris.";return}d(3)}),document.getElementById("step3-back").addEventListener("click",()=>d(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("setup-club").value.trim(),g=document.getElementById("step3-error"),x=document.getElementById("step3-finish");g.textContent="",x.disabled=!0,x.textContent="Création en cours…";try{const{error:f}=await v.from("users").insert({id:t.user.id,pseudo:c,club_name:l,club_color1:o,club_color2:a,credits:1e4});if(f)throw f;await io(t.user.id),await r(),n(`Bienvenue ${c} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(f){g.textContent=f.message,x.disabled=!1,x.textContent="🚀 Créer mon profil !"}})}async function io(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await v.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const no="modulepreload",oo=function(e){return"/"+e},nn={},bn=function(t,i,n){let r=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),s=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));r=Promise.allSettled(i.map(d=>{if(d=oo(d),d in nn)return;nn[d]=!0;const c=d.endsWith(".css"),l=c?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${l}`))return;const g=document.createElement("link");if(g.rel=c?"stylesheet":no,c||(g.as="script"),g.crossOrigin="",g.href=d,s&&g.setAttribute("nonce",s),document.head.appendChild(g),c)return new Promise((x,f)=>{g.addEventListener("load",x),g.addEventListener("error",()=>f(new Error(`Unable to preload CSS for ${d}`)))})}))}function o(a){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=a,window.dispatchEvent(s),!s.defaultPrevented)throw a}return r.then(a=>{for(const s of a||[])s.status==="rejected"&&o(s.reason);return t().catch(o)})},Zt="#1A6B3C",ei="#cc2222",ro="#D4A017",on="#888";async function ao(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      ${x.map(({friendshipId:f,friend:y})=>so(y,f)).join("")}
    </div>`,c.querySelectorAll("[data-stats]").forEach(f=>{f.addEventListener("click",()=>co(e,f.dataset.stats,f.dataset.friendName))}),c.querySelectorAll("[data-match]").forEach(f=>{f.addEventListener("click",async()=>{const y=f.dataset.friendId,p=f.dataset.friendName;if(typeof n!="function"){t("Erreur navigation","error");return}const u=e.user.id,{data:b}=await v.from("friend_match_invites").select("id").eq("inviter_id",y).eq("invitee_id",u).eq("status","pending").order("created_at",{ascending:!1}).limit(1).maybeSingle(),w=!!b;console.log("[Friends] clic match",{fid:y,fname:p,isAccepting:w}),n("match",{matchMode:"friend",friendId:y,friendName:p,isAccepting:w})})})}function so(e,t){const i=e.club_name||e.pseudo||"?",n=e.pseudo||"",r=(n||i).slice(0,2).toUpperCase(),o=e.club_color2||Zt,a=e.club_color1||"#ffffff",s=e.last_seen_at?new Date(e.last_seen_at):null,d=s&&Date.now()-s.getTime()<3*60*1e3;return`
    ...
    <div class="popup-box">
      ...
      ${l.length?`<div style="display:flex;flex-direction:column;gap:8px;max-height:50vh;overflow-y:auto;margin-bottom:14px">
            ${l.map(x=>{var f,y,p;return`
              <div style="display:flex;align-items:center;gap:10px;background:#f9f9f9;border-radius:10px;padding:10px 12px">
                <div style="flex:1;font-size:14px;font-weight:700">${((f=x.requester)==null?void 0:f.club_name)||((y=x.requester)==null?void 0:y.pseudo)||"?"}
    ...
    ${Ri()}`,document.body.appendChild(c);const g=()=>c.remove();c.querySelector("#pending-close").addEventListener("click",g),c.addEventListener("click",x=>{x.target===c&&g()}),c.querySelectorAll("[data-accept]").forEach(x=>{x.addEventListener("click",async()=>{const{error:f}=await v.from("friendships").update({status:"accepted"}).eq("id",x.dataset.accept);if(f){t("Erreur : "+f.message,"error");return}x.closest("div[style]").remove(),t("✅ Ami accepté !","success"),hn(e,t,n),i&&i()})}),c.querySelectorAll("[data-decline]").forEach(x=>{x.addEventListener("click",async()=>{await v.from("friendships").delete().eq("id",x.dataset.decline),x.closest("div[style]").remove(),t("Demande refusée","info"),i&&i()})})}async function co(e,t,i){const n=e.user.id,[r,o]=[n,t].sort(),a=n===r,{data:s}=await v.from("friend_match_stats").select("*").eq("player1_id",r).eq("player2_id",o).single(),d=e.profile.club_name||e.profile.pseudo||"Moi",c=s||{},l=a?c.wins_p1||0:c.wins_p2||0,g=a?c.wins_p2||0:c.wins_p1||0,x=c.draws||0,f=a?c.goals_p1||0:c.goals_p2||0,y=a?c.goals_p2||0:c.goals_p1||0,p=a?c.gc_used_p1||0:c.gc_used_p2||0,u=a?c.gc_used_p2||0:c.gc_used_p1||0,b=c.matches_total||0,w=(E,m,M,k=Zt,z=ei)=>`
    ...
    </style>`}const po="2026.07.21-1212";async function rn(e,{state:t,navigate:i,toast:n}){var o,a;const r=t.profile;r&&(e.innerHTML=`
  <style>
    .home-dark {
      min-height: 100%;
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    ${ze(e,{width:140})}
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),c=(s||[]).filter($=>$.card_type==="player"&&$.player),l=(s||[]).filter($=>$.card_type==="game_changer"),g=(s||[]).filter($=>$.card_type==="formation"),x=(s||[]).filter($=>$.card_type==="stadium"),{data:f}=await v.from("gc_definitions").select("name,gc_type,color,effect,image_url"),y={};(f||[]).forEach($=>{y[$.name]=$});const{data:p}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),u={};(p||[]).forEach($=>{u[$.id]=$}),x.forEach($=>{$.stadium_def&&(u[$.stadium_id]=$.stadium_def)});const b=Object.keys(Pi),w=Object.keys(Be),h={};c.forEach($=>{const N=$.player.id;h[N]=(h[N]||0)+1}),new Set(Object.keys(h).map($=>String($)));const E=new Set(g.map($=>$.formation)),m=new Set(l.map($=>$.gc_type));let M="player",k="Tous",z="",q=!1;function te(){return[...c].sort(($,N)=>{const V=ri.indexOf($.player.job),U=ri.indexOf(N.player.job);return V!==U?V-U:($.player.surname_real||"").localeCompare(N.player.surname_real||"")})}function ie(){return[...d||[]].sort(($,N)=>{const V=ri.indexOf($.job),U=ri.indexOf(N.job);return V!==U?V-U:($.surname_real||"").localeCompare(N.surname_real||"")})}function ee(){return te().filter($=>{const N=$.player,V=k==="Tous"||N.job===k,U=!z||`${N.firstname} ${N.surname_real}`.toLowerCase().includes(z);return V&&U})}function j(){return ie().filter($=>{const N=k==="Tous"||$.job===k,V=!z||`${$.firstname} ${$.surname_real}`.toLowerCase().includes(z);return N&&V})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:0 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none;align-items:center"></div>
  ...
  </div>`,function(N){const V=document.getElementById(N);if(!V)return;V.addEventListener("wheel",de=>{Math.abs(de.deltaY)<=Math.abs(de.deltaX)||(de.preventDefault(),V.scrollLeft+=de.deltaY)},{passive:!1});let U=!1,re=0,B=0,R=!1,pe=!1;V.addEventListener("mousedown",de=>{U=!0,R=!1,re=de.pageX,B=V.scrollLeft}),window.addEventListener("mouseup",()=>{U=!1,R&&(V.style.cursor="",pe=!0),R=!1}),window.addEventListener("mousemove",de=>{if(!U)return;const we=de.pageX-re;!R&&Math.abs(we)<6||(R=!0,V.style.cursor="grabbing",de.preventDefault(),V.scrollLeft=B-we)}),V.addEventListener("click",de=>{pe&&(de.stopPropagation(),de.preventDefault(),pe=!1)},!0)}("col-grid");function I(){const $=document.getElementById("col-filters");$&&(M==="player"?($.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{q=!q,I(),K()})))}function K(){const $=document.getElementById("col-grid");$&&(M==="player"?Y($):M==="formation"?ue($):M==="stadium"?H($):G($))}function X($,N,V,U,re,B=0){const R=document.getElementById("col-grid"),pe=document.getElementById("col-big");if(!R||!pe)return;var de=0;function we(){var Re;const Ae=window.innerWidth>=768,ve=document.getElementById("col-big"),Ie=document.getElementById("col-gap"),Me=((Re=document.getElementById("col-grid"))==null?void 0:Re.parentElement)||null,Pe=ve?ve.closest(".page"):null;if(ve&&Me&&Pe){let ke=0;Array.from(Pe.children).forEach(function(A){A!==ve&&A!==Me&&A!==Ie&&(ke+=A.offsetHeight)});const Se=Math.max(0,Pe.clientHeight-ke),Ce=Math.round(Se*(B/100)),qe=Math.max(0,Se-Ce),_=Ae?50/71:45/63,L=Math.round(qe*_),S=Math.max(0,qe-L);Ie&&(Ie.style.height=Ce+"px"),ve.style.flex="none",ve.style.height=L+"px",ve.style.minHeight="0",Me.style.flex="none",Me.style.height=S+"px",Me.style.minHeight="0",Me.style.display="flex",Me.style.overflow="hidden"}const _e=document.getElementById("col-grid");_e&&(_e.style.height="100%",_e.style.flexShrink="0",_e.style.overflowX="auto",_e.style.overflowY="hidden",_e.style.alignItems="center",_e.style.width="100%"),pe.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:center center">'+N($[de])+"</div>";var Ye=pe.querySelector("[data-card-id],[data-form-id],[data-gc-id]");Ye&&Ye.addEventListener("click",function(){U($[de])}),requestAnimationFrame(function(){var ke=document.getElementById("big-card-inner");if(!(!ke||!pe)){var Se=pe.clientHeight,Ce=pe.clientWidth-16,qe=ke.offsetHeight,_=ke.offsetWidth;if(qe>0&&_>0&&Se>40){var L=Math.min(Se/qe,Ce/_);ke.style.transform="scale("+L.toFixed(3)+")",ke.style.transformOrigin="center center"}}}),R.innerHTML=$.map(function(ke,Se){var Ce=Se===de,qe="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+(Ce?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+Se+'" style="'+qe+'">'+V(ke,Ce)+"</div>"}).join(""),R.querySelectorAll(".col-mini-item").forEach(function(ke){ke.addEventListener("click",function(){de=Number(ke.dataset.idx),we(),ke.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})}),requestAnimationFrame(function(){var ke=_e?_e.clientHeight:0,Se=R.querySelector(".col-mini-item > div");if(!(!ke||!Se)){var Ce=Se.style.zoom;Se.style.zoom="1";var qe=Se.offsetHeight;if(Se.style.zoom=Ce,!(qe<=0)){var _=ke/qe;R.querySelectorAll(".col-mini-item > div").forEach(function(L){L.style.zoom=_.toFixed(4)})}}})}we()}function ce($){var N=window.innerWidth>=768?.76:.54,V;if(!$||$._fake){var U=$?$.player:null;if(!U)return"";V=ln(U)}else V=ki($,"");return'<div style="display:inline-block;zoom:'+N+';pointer-events:none;line-height:0">'+V+"</div>"}function fe($,N,V){var U=V>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+V+"</div>":"",re=!!N,B=Li($,zt[$],{width:160});return re||(B='<div style="filter:grayscale(1);opacity:0.5">'+B+"</div>"),"<div "+(N?'data-form-id="'+N.id+'"':"")+' style="position:relative;cursor:pointer">'+U+B+"</div>"}function C($,N){var V=window.innerWidth>=768?.76:.54,U=Li($,zt[$],{width:140});return N||(U='<div style="filter:grayscale(1);opacity:0.45">'+U+"</div>"),'<div style="display:inline-block;zoom:'+V+';line-height:0;pointer-events:none">'+U+"</div>"}function Y($){if(q){const N=j();if(!N.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucun joueur.</div>';return}const V=N.map(U=>c.find(re=>re.player.id===U.id)||{_fake:!0,player:U,id:"fake-"+U.id});X(V,U=>U._fake?ln(U.player):ki(U,""),U=>U._fake?ce({player:U.player,id:"x",_fake:!0}):ce(U),U=>{U._fake||cn(U,c,h,t)})}else{const N=ee();if(!N.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const V={};N.forEach(re=>{const B=re.player.id;V[B]||(V[B]=[]),V[B].push(re)});const U=Object.values(V).map(re=>Lo(re));X(U,re=>{const B=h[re.player.id]||1,R=B>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${B}</div>`:"",de=c.filter(we=>we.player.id===re.player.id&&we.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return ki(re,R+de)},re=>ce(re),re=>cn(re,c,h,t))}}function ue($){const N=q?b:[...E];if(!N.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const V=N.map(U=>({formation:U,card:g.find(re=>re.formation===U)||null,owned:E.has(U)}));X(V,({formation:U,card:re,owned:B})=>fe(U,B?re:null,B?g.filter(R=>R.formation===U).length:0),({formation:U,owned:re})=>C(U,re),({card:U,owned:re})=>{re&&U&&Mo(U,g,t,o)},"#1A6B3C",5)}function G($){const N=Object.keys(y),V=q?N.length?N:w:[...m];if(!V.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const U=V.map(re=>({type:re,gc:Be[re]||{icon:"⚡",desc:""},def:y[re]||null,owned:m.has(re),card:l.find(B=>B.gc_type===re)||null}));X(U,({type:re,gc:B,def:R,owned:pe,card:de})=>{var _e;const we=(R==null?void 0:R.name)||re,Ae=pe?l.filter(Ye=>Ye.gc_type===re).length:0,ve=Ae>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${Ae}</div>`:"",Ie=(R==null?void 0:R.effect)||B.desc||"",Me=R!=null&&R.image_url?`/icons/${R.image_url}`:((_e=R==null?void 0:R.club)==null?void 0:_e.logo_url)||(R!=null&&R.country_code?`https://flagsapi.com/${R.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null);let Pe=Jt(we,Me,B.icon,Ie,{width:160,onClick:pe});return pe||(Pe=`<div style="filter:grayscale(1);opacity:0.5">${Pe}</div>`),`<div ${pe&&de?`data-gc-id="${de.id}" data-gc-type="${re}"`:""} style="position:relative">${ve}${Pe}</div>`},({type:re,gc:B,def:R,owned:pe})=>{const de=window.innerWidth>=768?.76:.54,we=(R==null?void 0:R.name)||re,Ae=(R==null?void 0:R.effect)||B.desc||"",ve=R!=null&&R.image_url?`/icons/${R.image_url}`:null;let Ie=Jt(we,ve,B.icon,Ae,{width:140});return pe||(Ie=`<div style="filter:grayscale(1);opacity:0.45">${Ie}</div>`),`<div style="display:inline-block;zoom:${de};line-height:0;pointer-events:none">${Ie}</div>`},({type:re,owned:B,def:R})=>{B&&Ao(re,R,o)},"#7a28b8",5)}function H($){const N="#4FC3F7",V="/";if(!x.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Stade.<br><small>Ouvre un booster Stade !</small></div>';return}const U={};x.forEach(B=>{const R=B.stadium_id||"?";(U[R]=U[R]||[]).push(B)});const re=Object.entries(U).map(([B,R])=>({sid:B,def:u[B]||null,count:R.length,card:R[0]}));X(re,({def:B,count:R})=>{var Ie,Me;const pe=(B==null?void 0:B.name)||"?",de=((Ie=B==null?void 0:B.club)==null?void 0:Ie.encoded_name)||(B==null?void 0:B.country_code)||"—",we=B!=null&&B.image_url?`${V}icons/${B.image_url}`:((Me=B==null?void 0:B.club)==null?void 0:Me.logo_url)||(B!=null&&B.country_code?`https://flagsapi.com/${B.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),Ae=R>1?`<div style="position:absolute;top:8px;right:8px;background:#333;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${R}</div>`:"",ve=`${de}<br>+10 ⭐ joueurs alliés`;return`<div style="position:relative">${Ae}${Qt(pe,we,ve,{width:160})}</div>`},({def:B})=>{var ve,Ie;const R=window.innerWidth>=768?.76:.54,pe=(B==null?void 0:B.name)||"?",de=((ve=B==null?void 0:B.club)==null?void 0:ve.encoded_name)||(B==null?void 0:B.country_code)||"—",we=B!=null&&B.image_url?`${V}icons/${B.image_url}`:((Ie=B==null?void 0:B.club)==null?void 0:Ie.logo_url)||(B!=null&&B.country_code?`https://flagsapi.com/${B.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),Ae=`${de}<br>+10 ⭐`;return`<div style="display:inline-block;zoom:${R};line-height:0;pointer-events:none">${Qt(pe,we,Ae,{width:140})}</div>`},null,N,5)}e.querySelectorAll(".col-tab-btn").forEach($=>{$.addEventListener("click",()=>{M=$.dataset.tab,k="Tous",z="",q=!1,e.querySelectorAll(".col-tab-btn").forEach(N=>{const V=N.dataset.tab===M;N.style.borderBottomColor=V?"var(--green)":"transparent",N.style.color=V?"var(--green)":"var(--tile-fg-dim)"}),I(),K()})}),I(),K()}function Ao(e,t,i){const n=Be[e]||{icon:"⚡",desc:"Effet spécial."},r=(t==null?void 0:t.name)||e,o=(t==null?void 0:t.effect)||n.desc,a=t!=null&&t.image_url?`/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
      ${Jt(r,a,n.icon,o,{width:200})}
    ...
    <div style="margin-top:16px;border-top:1px solid var(--tile-border);padding-top:14px">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${g.map((G,H)=>{const $=ee[G.id]||[],N=G.is_for_sale,V=$.length?$[$.length-1]:null,re=(l.rarity==="pepite"||l.rarity==="papyte")&&G.current_note!=null?` (☆${G.current_note})`:"";return`
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",l.note_g],["DEF",l.note_d],["MIL",l.note_m],["ATT",l.note_a]].map(([G,H])=>{const $=wi[G],N=G==="GK"?"#fff":$,V=G===l.job||G===l.job2,U=(Number(H)||0)+(V&&M>0?M:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(fe=document.getElementById("close-detail"))==null||fe.addEventListener("click",s);let K=new Set;const X=()=>{const G=K.size,H=document.getElementById("sell-action-panel");if(!H)return;H.style.display=G>0?"block":"none",document.getElementById("sell-selected-count").textContent=G;const $=document.getElementById("evolve-btn");$&&($.textContent=`⬆️ Évoluer ${G>1?"(+"+G+")":""}`)};document.querySelectorAll(".expl-check").forEach(G=>{G.addEventListener("change",()=>{const H=G.dataset.id;G.checked?K.add(H):K.delete(H);const $=G.closest(".exemplaire-row");$&&($.style.borderColor=G.checked?"#1A6B3C":"#eee"),X()})}),document.querySelectorAll(".exemplaire-row").forEach(G=>{G.addEventListener("click",H=>{if(H.target.closest("button")||H.target.tagName==="INPUT")return;const $=G.querySelector(".expl-check");$&&!$.disabled&&($.checked=!$.checked,$.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(G=>{G.addEventListener("click",H=>{H.stopPropagation();const $=document.querySelector(`.expl-hist[data-hist="${G.dataset.idx}"]`);$&&($.style.display=$.style.display==="none"?"flex":"none")})}),(C=document.getElementById("evolve-btn"))==null||C.addEventListener("click",async()=>{if(x<=1)return;const G=[...K];if(!G.length)return;if(K.has(e.id)){const B=document.createElement("div");B.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",B.innerHTML=`
  ...
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
    ...
    <div class="page-body">
      ...
      stadium_def:stadium_definitions(id, name, club_id, country_code, image_url,
        club:clubs(encoded_name, logo_url))`).eq("owner_id",n.profile.id).eq("card_type","stadium"),d=(a||[]).filter(b=>b.card_type==="player"&&b.player),c=(a||[]).filter(b=>b.card_type==="formation"),l=(s||[]).filter(b=>b.card_type==="stadium"),g=[...new Set(l.map(b=>b.stadium_id).filter(Boolean))];let x={};if(l.forEach(b=>{b.stadium_def&&b.stadium_id&&(x[b.stadium_id]=b.stadium_def)}),g.length&&Object.keys(x).length<g.length){const{data:b}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",g);(b||[]).forEach(w=>{x[w.id]=w})}const f=c.map(b=>b.formation).filter(Boolean),{data:y}=await v.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",e);let p=o.formation||"4-4-2";f.length>0&&!f.includes(p)&&(p=f[0]);const u={deckId:e,name:o.name,formation:p,formationCardId:o.formation_card_id,stadiumCardId:o.stadium_card_id||null,slots:{},subs:[],playerCards:d,formationCards:c,stadiumCards:l,stadDefMap:x,availableFormations:f};(y||[]).forEach(b=>{b.is_starter?u.slots[b.position]=b.card_id:u.subs.includes(b.card_id)||u.subs.push(b.card_id)}),wt(t,u,i)}function wt(e,t,i){var y,p;const{navigate:n}=i;li[t.formation];const r=fn(t.formation),o=r.filter(u=>t.slots[u]).length,a=t.availableFormations.length>0?t.availableFormations:Object.keys(li),s=(y=t.stadiumCards)==null?void 0:y.find(u=>u.id===t.stadiumCardId),d=s&&((p=t.stadDefMap)==null?void 0:p[s.stadium_id])||null,c=t.subs.map(u=>t.playerCards.find(b=>b.id===u)).filter(Boolean);[...Object.values(t.slots),...t.subs],e.innerHTML=`
  ...
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
        ...
        <div style="width:130px;flex-shrink:0;background:rgba(0,0,0,0.3);display:flex;flex-direction:column;align-items:center;padding:12px 8px;gap:12px;border-left:1px solid rgba(255,255,255,0.1)">
          ...
          <div style="width:100%;text-align:center">
            ...
            <div id="add-stad-btn-pc" style="cursor:pointer;margin:0 auto;width:fit-content">
              ${s?(()=>{var w;const u=t.stadDefMap[s.stadium_id],b=((w=u==null?void 0:u.club)==null?void 0:w.logo_url)||(u==null?void 0:u.image_url)||null;return Qt((u==null?void 0:u.name)||"Stade",b,"+10 ⭐",{width:100})})():`<div style="width:100px;height:171px;border:2px dashed rgba(79,195,247,0.4);border-radius:8px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px">
        ...
        <div style="display:flex;gap:10px;align-items:flex-start">
          ...
          <div style="flex-shrink:0;text-align:center">
            ...
            <div id="add-stad-btn" style="cursor:pointer">
              ${s?(()=>{var w;const u=t.stadDefMap[s.stadium_id],b=((w=u==null?void 0:u.club)==null?void 0:w.logo_url)||(u==null?void 0:u.image_url)||null;return Qt((u==null?void 0:u.name)||"Stade",b,"+10⭐",{width:50})})():`<div style="width:50px;height:85px;border:2px dashed rgba(79,195,247,0.5);border-radius:6px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px">
  ...
  </div>`;const l=window.innerWidth>=900,g=e.querySelector(".deck-pc-layout"),x=e.querySelector(".deck-mobile-layout");g&&(g.style.display=l?"block":"none"),x&&(x.style.display=l?"none":"block"),jo(e,t,r,i),e.querySelectorAll("#builder-back").forEach(u=>u.addEventListener("click",()=>n("decks")));const f=()=>{const{openModal:u,closeModal:b}=i,w=`<div style="display:flex;flex-wrap:wrap;gap:8px;padding:8px">
    ...
    <div style="position:relative;width:${f}px;height:${y}px;margin:0 auto">
      <svg style="position:absolute;inset:0;width:100%;height:100%;pointer-events:none" viewBox="0 0 ${f} ${y}">${u}</svg>
      ${b}
      ...
      ${o.map(s=>{var b,w;const d=e.stadDefMap[s.stadium_id],c=((b=d==null?void 0:d.club)==null?void 0:b.logo_url)||null,l=d!=null&&d.image_url?"/icons/"+d.image_url:null,g=e.stadiumCardId===s.id,x=d!=null&&d.country_code&&!c?`https://flagsapi.com/${d.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,f=(d==null?void 0:d.name)||"Stade",y=f.match(/^(Stade\s+(?:de\s+)?(?:\w+)?)(.*)?$/i),p=y?y[1].trim():f.slice(0,8),u=((w=y==null?void 0:y[2])==null?void 0:w.trim())||"";return`<div class="stad-choice" data-stad-id="${s.id}" style="cursor:pointer;width:80px;text-align:center">
  ...
  `,document.body.appendChild(r);let o=!1;const a=document.getElementById("pack-cut-zone"),s=document.getElementById("pack-blade");let d=!1;const c=I=>I.touches&&I.touches[0]?I.touches[0].clientX:I.clientX;function l(I){o||(d=!0,s.style.opacity="1",g(I))}function g(I){if(!d||o)return;const K=a.getBoundingClientRect(),X=c(I)-K.left,ce=Math.max(0,Math.min(1,X/K.width));s.style.width=ce*K.width+"px",ce>=.82&&f()}function x(){o||(d=!1,s.style.transition="width .2s ease, opacity .2s ease",s.style.width="0",s.style.opacity="0",setTimeout(()=>{o||(s.style.transition="")},220))}function f(){var K;if(o)return;o=!0,d=!1,s.style.width="100%",s.style.opacity="1",(K=document.getElementById("cut-flash"))==null||K.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const I=document.getElementById("cut-hint");I&&(I.style.opacity="0"),a.classList.add("pack-cut"),setTimeout(()=>{s.style.opacity="0",document.getElementById("pack-phase").style.display="none",b(0)},620)}a.addEventListener("pointerdown",l),window.addEventListener("pointermove",g),window.addEventListener("pointerup",x),a.addEventListener("touchstart",l,{passive:!0}),window.addEventListener("touchmove",g,{passive:!0}),window.addEventListener("touchend",x);let y=0,p=!1;const u=new Set;function b(I){y=I,document.getElementById("reveal-phase").style.display="flex",w(),h(I,0),z()}function w(){const I=document.getElementById("card-dots");I&&(I.innerHTML=e.map((K,X)=>`<div class="card-dot" data-i="${X}" style="width:8px;height:8px;border-radius:50%;background:${X===y?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),I.querySelectorAll(".card-dot").forEach(K=>K.addEventListener("click",()=>m(parseInt(K.dataset.i)))))}function h(I,K){var $,N;const X=e[I],ce=document.getElementById("card-counter"),fe=document.getElementById("card-track");ce&&(ce.textContent=`Carte ${I+1} / ${e.length}`);const C=document.getElementById("reveal-btns");C&&(C.style.display=I===e.length-1?"flex":"none");const Y=X.card_type==="player"&&(($=X.player)==null?void 0:$.rarity)==="legende",ue=!u.has(I);u.add(I);let G=0;if(X.card_type==="player"&&X.player){const V=X.player,U=V.job||"ATT";G=Number(U==="GK"?V.note_g:U==="DEF"?V.note_d:U==="MIL"?V.note_m:V.note_a)||0}const H=V=>{fe.innerHTML=`
        ...
        </div>`;const U=document.getElementById("current-card-wrap");K!==0?(U.style.transition="none",U.style.transform=`translateX(${K>0?100:-100}%)`,requestAnimationFrame(()=>{U.style.transition="transform .28s cubic-bezier(.25,1,.5,1)",U.style.transform="translateX(0)"})):U.animate([{opacity:0,transform:"scale(.7)"},{opacity:1,transform:"scale(1)"}],{duration:300,easing:"cubic-bezier(.34,1.56,.64,1)"}),V||Y?te():ie(),w()};ue&&(((N=X.player)==null?void 0:N.rarity)==="legende"||G>=8)&&X.card_type==="player"&&X.player?E(X,()=>H(!0)):H(!1)}function E(I,K){var Ae;p=!0;const X=I.player,ce=`https://flagsapi.com/${X.country_code}/flat/64.png`,fe=(Ae=X.clubs)==null?void 0:Ae.logo_url,C=X.face?"/"+X.face.replace(/^public\//,"").replace(/^\//,""):null,Y=X.job||"ATT",ue=Number(Y==="GK"?X.note_g:Y==="DEF"?X.note_d:Y==="MIL"?X.note_m:X.note_a)||0,G=I.evolution_bonus||0,H=ue+G,$=X.rarity==="legende",N=H>=18,V=document.getElementById("walkout-overlay"),U=document.getElementById("walkout-stage");if(!V||!U){p=!1,K();return}let re=null;$&&(re=bo("/sounds/Legendary.mp3",.8)),V.style.display="flex";const B=()=>{const ve=U.firstElementChild;ve&&(ve.classList.remove("wo-in"),ve.classList.add("wo-out"))},R=1800,pe=400;U.innerHTML=`<img class="wo-in" src="${ce}" style="height:130px;border-radius:10px;box-shadow:0 10px 36px rgba(0,0,0,.6)" onerror="this.style.display='none'">`,navigator.vibrate&&navigator.vibrate(30),setTimeout(B,R),setTimeout(()=>{var ve;U.innerHTML=fe?`<img class="wo-in" src="${fe}" style="max-height:160px;max-width:210px;object-fit:contain">`:`<div class="wo-in" style="font-size:34px;font-weight:900;color:#fff;text-align:center">${((ve=X.clubs)==null?void 0:ve.encoded_name)||"CLUB"}</div>`,navigator.vibrate&&navigator.vibrate(30)},R+pe),setTimeout(B,R*2+pe),setTimeout(()=>{U.innerHTML=C?`<img class="wo-in" src="${C}" style="height:200px;border-radius:50%;box-shadow:0 0 40px rgba(255,255,255,0.3);object-fit:cover;object-position:top">`:'<div class="wo-in" style="font-size:80px">👤</div>',navigator.vibrate&&navigator.vibrate(30)},(R+pe)*2),setTimeout(B,(R+pe)*2+R);const we=N?"#FFD700":{GK:"#c0c0c0",DEF:"#e03030",MIL:"#D4A017",ATT:"#3fbf5f"}[Y]||"#fff";setTimeout(()=>{U.innerHTML=`<div class="wo-in" style="
        font-size:${N?"120px":"90px"};font-weight:900;color:${we};
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    </div>`}const d=l=>{e.style.overflow="",e.style.height="",e.style.display="",e.style.flexDirection="",i(l)};function c(){var g,x,f;e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column";const l=a.length>0;e.innerHTML=`
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
    ...
    </div>`,e.querySelectorAll(".gc-select-card").forEach(y=>{y.addEventListener("click",()=>{const p=y.dataset.id,u=o.find(w=>w.id===p);if(!u)return;const b=a.findIndex(w=>w.gc_type===u.gc_type);b>-1?a.splice(b,1):a.length<3&&a.push(u),c()})}),(g=e.querySelector("#gc-launch"))==null||g.addEventListener("click",()=>{l&&d(a)}),(x=e.querySelector("#gc-no-gc"))==null||x.addEventListener("click",()=>d([])),(f=e.querySelector("#gc-reset"))==null||f.addEventListener("click",()=>{a.length&&(a=[],c())})}c()}function er(e,t){var r;const i=((r=t==null?void 0:t.state)==null?void 0:r.params)||{},n=e||i.matchMode||"vs_ai_easy";return n==="friend"?`Match vs ${i.friendName||"Ami"}`:n==="random"?"Match vs Random":n==="ranked"?"Match Classé":n==="mini_league"||n==="mini-league"?"🏆 Match de Mini League":`Match vs IA — ${n.replace("vs_ai_","").toUpperCase()}`}async function tr(e,t,i){const{state:n,navigate:r}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await v.from("decks").select(`id,name,is_active,formation,stadium_card_id,
      ...
      card:cards(id,card_type,formation,stadium_id,evolution_bonus,
        player:players(id,firstname,surname_real,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order"),d=[...new Set((s||[]).filter(x=>{var f,y;return((f=x.card)==null?void 0:f.card_type)==="stadium"&&((y=x.card)==null?void 0:y.stadium_id)}).map(x=>x.card.stadium_id))],c={};if(d.length){const{data:x}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",d);(x||[]).forEach(f=>{c[f.id]=f}),(s||[]).forEach(f=>{var y,p;((y=f.card)==null?void 0:y.card_type)==="stadium"&&((p=f.card)!=null&&p.stadium_id)&&(f.card._stadiumDef=c[f.card.stadium_id]||null)})}let l=0;function g(){var m,M,k,z,q,te,ie;const x=o[l],f=(s||[]).filter(ee=>ee.deck_id===x.id),y=f.filter(ee=>{var j;return ee.is_starter&&((j=ee.card)==null?void 0:j.player)}).map(ee=>Mi(ee.card,ee.position)),p=f.find(ee=>{var j;return((j=ee.card)==null?void 0:j.card_type)==="formation"}),u=x.formation||((m=p==null?void 0:p.card)==null?void 0:m.formation)||"4-4-2";let b=y.length>=11?pi(y,u):null,w=((M=x.stadium_card)==null?void 0:M.stadium_def)||null;w&&b&&(b=oi(b,w));const h=y.length>=11;dt(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff;position:relative">
      ...
      <div id="deck-bottom-bar" style="flex-shrink:0;padding:10px 14px 14px;display:flex;flex-direction:column;gap:8px;background:rgba(0,0,0,0.2)">
        ...
        stroke="${k}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const y=typeof window<"u"&&window.innerWidth>=900?Math.min(Math.max(81,Math.round(r*.225)),117):Math.max(44,Math.round(r*.168)),p=Math.round(y*657/507);for(const[b,w]of Object.entries(l)){const h=x(b);if(!h||!w)continue;const E=b.replace(/[0-9]/g,""),m=a.includes(w.cardId),M=i==="attack"&&(["MIL","ATT"].includes(E)||m)&&!w.used||i==="defense"&&["GK","DEF","MIL"].includes(E)&&!w.used,k=n.includes(w.cardId);let z=w.boost||0,q=!1;w.stadiumBonus&&(i==="attack"&&(E==="ATT"||E==="MIL")||i==="defense"&&(E==="GK"||E==="DEF"||E==="MIL")?(z+=10,q=!0):i||(z+=10,q=!0));const te=Math.round(h.x-y/2),ie=Math.round(h.y-p/2);if(w.used){f+=`<image href="${`${typeof import.meta<"u"&&"/"||"/"}icons/carte-dos.png`}" x="${te}" y="${ie}" width="${y}" height="${p}" preserveAspectRatio="xMidYMid slice" class="match-used-hit" data-card-id="${w.cardId}" data-role="${E}" style="cursor:pointer"/>`;continue}const ee=ze({...w,_evolution_bonus:0,stadiumBonus:!1},{width:y,showStad:!1,stadDef:null,role:E,extraNote:z,_cardOffset:30,_forceStadColor:q}),j=k?`position:absolute;top:30px;left:0;width:${y}px;height:${p}px;outline:3px solid #FFD700;outline-offset:2px;border-radius:8px;pointer-events:none;`:"";f+=`<foreignObject x="${te-2}" y="${ie-30}" width="${y+8}" height="${p+60}" style="overflow:visible">
      <div xmlns="http://www.w3.org/1999/xhtml" style="position:relative">
        ${ee}
        ${k?`<div style="${j}"></div>`:""}
      </div>
    </foreignObject>`,M&&(f+=`<rect x="${te}" y="${ie}" width="${y}" height="${p}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${k?"selected":""}" data-card-id="${w.cardId}" data-role="${E}" style="cursor:pointer"/>`)}const u=s!==null?s:Math.round(Math.max(y*.7,80));return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-u} ${-u} ${r+u*2} ${o+u*2}" width="100%" style="display:block;width:100%;margin:0 auto">
    ${f}
  </svg>`}function Ot(e,t,i,n,r=300,o=300,a=[],s=null){return`<div id="match-terrain-wrap" style="position:relative;padding:0 4px">
    ${yi(e,t,i,n,r,o,a,s)}
  </div>`}async function xi(e,t,i,n){var M;const{state:r,navigate:o,toast:a}=t;dt(e);const s=r.params||{};if(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!s.deckId)return tr(e,t,i);const d=s.deckId;let c,l,g,x;try{const k=await Promise.all([v.from("decks").select("formation,name,stadium_card_id").eq("id",d).single(),v.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation, evolution_bonus,
            player:players(id,firstname,surname_real,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
              clubs(encoded_name,logo_url)))`).eq("deck_id",d).order("slot_order")]);c=k[0].data,g=k[0].error,l=k[1].data,x=k[1].error}catch(k){console.error("[Match] Exception chargement deck:",k),Tt(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>o("home"));return}if(g||x){console.error("[Match] Erreur Supabase:",g||x),Tt(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>o("home"));return}const f=(l||[]).filter(k=>{var z;return k.is_starter&&((z=k.card)==null?void 0:z.player)}).map(k=>Mi(k.card,k.position)),y=(l||[]).filter(k=>{var z;return!k.is_starter&&((z=k.card)==null?void 0:z.player)}).map(k=>Mi(k.card,k.position));if(f.length<11){Tt(e,"⚠️",`Deck incomplet (${f.length}/11).`,"Compléter",()=>o("decks"));return}const p=(l||[]).find(k=>{var z;return((z=k.card)==null?void 0:z.card_type)==="formation"}),u=(c==null?void 0:c.formation)||((M=p==null?void 0:p.card)==null?void 0:M.formation)||"4-4-2",{data:b,error:w}=await v.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",r.profile.id).eq("card_type","game_changer"),{data:h}=await v.from("gc_definitions").select("*").eq("is_active",!0),E=(b||[]).map(k=>({...k,_gcDef:(h==null?void 0:h.find(z=>z.name===k.gc_type||z.id===k.gc_definition_id))||null}));let m=null;if(c!=null&&c.stadium_card_id){const{data:k}=await v.from("cards").select("stadium_id").eq("id",c.stadium_card_id).single();if(k!=null&&k.stadium_id){const{data:z}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").eq("id",k.stadium_id).single();m=z||null}}n({deckId:d,formation:u,starters:f,subsRaw:y,gcCardsEnriched:E,gcDefs:h||[],stadiumDef:m})}function It(){return Math.min(window.innerWidth-40,860)}function Nt(){return Math.round(It()*1.1)}function qn(e){var r,o;if(!e)return null;const t=e._line||e.job||"MIL",i=t==="GK"?e.note_g||0:t==="DEF"?e.note_d||0:t==="MIL"?e.note_m||0:e.note_a||0,n=e.stadiumBonus?10:0;return{name:e.name,firstname:e.firstname||"",note:i+(e.boost||0)+n,note_g:e.note_g||0,note_d:e.note_d||0,note_m:e.note_m||0,note_a:e.note_a||0,_evolution_bonus:0,stadiumBonus:e.stadiumBonus||!1,boost:e.boost||0,job:e.job,job2:e.job2||null,_line:e._line||e.job,_col:e._col,country_code:e.country_code,club_id:e.club_id,rarity:e.rarity,clubName:e.clubName||((r=e.clubs)==null?void 0:r.encoded_name)||null,clubLogo:e.clubLogo||((o=e.clubs)==null?void 0:o.logo_url)||null,face:e.face||null,portrait:ni(e)}}function ir(e,t,i,n="Adversaire"){const r=It(),o=Nt();return`
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const Y=t.pendingAttack;let ue="";if(t.selected.length>0){const G=t.selected.map($=>({...(t.homeTeam[$._role]||[]).find(V=>V.cardId===$.cardId)||$,_line:$._role})),H=ii(G,t.modifiers.home,t.formation);ue=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
    ...
    ${(()=>{const C=window.innerWidth>=700,Y=(B,R,pe)=>{var Se,Ce;const de=(t.gcDefs||[]).find(qe=>qe.name===B.gc_type),we={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[de==null?void 0:de.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",Ae={purple:"#b06ce0",light_blue:"#00d4ef"}[de==null?void 0:de.color]||"#b06ce0",ve=(de==null?void 0:de.name)||B.gc_type,Ie=(de==null?void 0:de.effect)||((Se=Be[B.gc_type])==null?void 0:Se.desc)||"",Me=de!=null&&de.image_url?`/icons/${de.image_url}`:null,Pe=((Ce=Be[B.gc_type])==null?void 0:Ce.icon)||"⚡",_e=Math.round(pe*.22),Ye=Math.round(pe*.22),Re=pe-_e-Ye,ke=ve.length>12?7:9;return`<div class="gc-mini" data-gc-id="${B.id}" data-gc-type="${B.gc_type}"
          style="box-sizing:border-box;width:${R}px;height:${pe}px;border-radius:10px;border:2px solid ${Ae};background:${we};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
          <div style="height:${_e}px;background:rgba(255,255,255,0.14);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 3px">
            <span style="font-size:${ke}px;font-weight:900;color:#fff;text-align:center;line-height:1.1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${R-6}px">${ve}</span>
            <span style="font-size:6px;color:rgba(255,255,255,0.45)">⚡ GC</span>
          </div>
          <div style="height:${Re}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
            ${Me?`<img src="${Me}" style="max-width:${R-10}px;max-height:${Re-6}px;object-fit:contain">`:`<span style="font-size:${Math.round(Re*.55)}px">${Pe}</span>`}
          </div>
        ...
        </div>`},ue=(B,R)=>{var pe;return`<div id="boost-card"
        ...
        <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
          ${Ot(t.homeTeam,t.formation,t.phase,n,C?1300:It(),C?600:Nt(),y)}
        </div>
      </div>`;return C?`
  ...
  <div id="match-history-panel">
    ...
    </div>`,(m=g.querySelector("#sub-close"))==null||m.addEventListener("click",()=>g.remove()),(M=g.querySelector("#out-up"))==null||M.addEventListener("click",()=>{s>0&&(s--,x())}),(k=g.querySelector("#out-down"))==null||k.addEventListener("click",()=>{s<o.length-1&&(s++,x())}),(z=g.querySelector("#in-up"))==null||z.addEventListener("click",()=>{c>0&&(c--,x())}),(q=g.querySelector("#in-down"))==null||q.addEventListener("click",()=>{c<a.length-1&&(c++,x())});const E=(ie,ee,j,I)=>{const K=g.querySelector("#"+ie);if(!K)return;let X=0;K.addEventListener("touchstart",ce=>{X=ce.touches[0].clientY},{passive:!0}),K.addEventListener("touchend",ce=>{const fe=ce.changedTouches[0].clientY-X;if(Math.abs(fe)<30)return;const C=ee();fe<0&&C<I-1?(j(C+1),x()):fe>0&&C>0&&(j(C-1),x())},{passive:!0})};E("in-panel",()=>c,ie=>c=ie,a.length),E("out-panel",()=>s,ie=>s=ie,o.length),(te=g.querySelector("#sub-confirm"))==null||te.addEventListener("click",ie=>{if(ie.preventDefault(),ie.stopPropagation(),l)return;l=!0;const ee=o[s],j=a[c];if(!ee||!j)return;let I=null,K=-1;for(const[ce,fe]of Object.entries(t.homeTeam)){const C=(fe||[]).findIndex(Y=>Y.cardId===ee.cardId);if(C!==-1){I=ce,K=C;break}}if(K===-1||!I){$t("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),g.remove();return}const X={...j,_line:I,_col:ee._col||0,used:!1,boost:0};t.homeTeam[I].splice(K,1,X),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(j.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:ee.name,firstname:ee.firstname,note:Ne(ee,I),portrait:ni(ee),job:ee.job,country_code:ee.country_code,rarity:ee.rarity,clubName:ee.clubName,clubLogo:ee.clubLogo},inPlayer:{name:j.name,firstname:j.firstname,note:Ne(j,I),portrait:ni(j),job:j.job,country_code:j.country_code,rarity:j.rarity,clubName:j.clubName,clubLogo:j.clubLogo},text:`🔄 ${j.firstname} ${j.name} remplace ${ee.firstname} ${ee.name}`}),g.remove(),nr(ee,j,()=>$e(e,t,i))})}document.body.appendChild(g),x()}function gr(e,t,i,n,r){var y,p;const o=(n.gcDefs||[]).find(u=>u.name===t),a=Be[t]||{icon:"⚡",desc:"Carte spéciale."},s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[o==null?void 0:o.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",d={purple:"#b06ce0",light_blue:"#00d4ef"}[o==null?void 0:o.color]||"#b06ce0",c=(o==null?void 0:o.name)||t,l=(o==null?void 0:o.effect)||a.desc,g=o!=null&&o.image_url?`/icons/${o.image_url}`:null,x=a.icon||"⚡",f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",f.innerHTML=`
    ...
    </div>`,document.body.appendChild(f),(y=f.querySelector("#gc-back"))==null||y.addEventListener("click",()=>f.remove()),(p=f.querySelector("#gc-use"))==null||p.addEventListener("click",()=>{f.remove(),xr(e,t,i,n,r)})}function si(e,t,i,n,r,o){const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let s=[];function d(){var c,l;a.innerHTML=`
    ...
    <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start;justify-content:center">
      ${e.map(g=>{const x=g._line||g.job||"MIL",f=s.find(p=>p.cardId===g.cardId),y=ze({...g,_evolution_bonus:0},{width:90,showStad:!0,role:x,extraNote:g.boost||0});return`<div class="gc-pick-item" data-cid="${g.cardId}"
    ...
    </div>`,(c=a.querySelector("#gc-picker-close"))==null||c.addEventListener("click",()=>a.remove()),a.querySelectorAll(".gc-pick-item").forEach(g=>{g.addEventListener("click",()=>{const x=g.dataset.cid,f=e.find(p=>p.cardId===x);if(!f)return;const y=s.findIndex(p=>p.cardId===x);y>-1?s.splice(y,1):s.length<t&&s.push(f),d()})}),(l=a.querySelector("#gc-picker-confirm"))==null||l.addEventListener("click",()=>{a.remove(),o(s)})}d(),document.body.appendChild(a)}const yr={BOOST_STAT:({value:e=1,count:t=1,roles:i=[]},n,r,o)=>{const a=Object.entries(n.homeTeam).filter(([s])=>!i.length||i.includes(s)).flatMap(([s,d])=>d.filter(c=>!c.used).map(c=>({...c,_line:s})));return a.length?(si(a,t,`Choisir ${t} joueur(s) à booster (+${e})`,r,n,s=>{s.forEach(d=>{const c=(n.homeTeam[d._line]||[]).find(l=>l.cardId===d.cardId);c&&(c.boost=(c.boost||0)+e,n.log.push({text:`⚡ +${e} sur ${c.name}`,type:"info"}))}),$e(r,n,o)}),!0):(n.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),$e(r,n,o),!0)},DEBUFF_STAT:({value:e=1,count:t=1,roles:i=[],target:n="ai"},r,o,a)=>{const s=n==="home"?r.homeTeam:r.aiTeam,d=n==="ai"?"adverse":"allié",c=Object.entries(s).filter(([l])=>!i.length||i.includes(l)).flatMap(([l,g])=>g.filter(x=>!x.used).map(x=>({...x,_line:l})));return c.length?(si(c,t,`Choisir ${t} joueur(s) ${d}(s) à débuffer (-${e})`,o,r,l=>{l.forEach(g=>{const f=((n==="home"?r.homeTeam:r.aiTeam)[g._line]||[]).find(y=>y.cardId===g.cardId);f&&(f.boost=(f.boost||0)-e,r.log.push({text:`🎯 -${e} sur ${f.name}${n==="ai"?" (IA)":""}`,type:"info"}))}),$e(o,r,a)}),!0):(r.log.push({text:`🎯 Aucun joueur ${d} disponible`,type:"info"}),$e(o,r,a),!0)},GRAY_PLAYER:({count:e=1,roles:t=[],target:i="ai"},n,r,o)=>{const a=i==="home"?n.homeTeam:n.aiTeam,s=i==="ai"?"adverse":"allié",d=Object.entries(a).filter(([c])=>!t.length||t.includes(c)).flatMap(([c,l])=>l.filter(g=>!g.used).map(g=>({...g,_line:c})));return d.length?(si(d,e,`Choisir ${e} joueur(s) ${s}(s) à exclure`,r,n,c=>{c.forEach(l=>{const x=((i==="home"?n.homeTeam:n.aiTeam)[l._line]||[]).find(f=>f.cardId===l.cardId);x&&(x.used=!0,n.log.push({text:`❌ ${x.name}${i==="ai"?" (IA)":""} exclu !`,type:"info"}))}),$e(r,n,o)}),!0):(n.log.push({text:`❌ Aucun joueur ${s} à exclure`,type:"info"}),$e(r,n,o),!0)},REVIVE_PLAYER:({count:e=1,roles:t=[]},i,n,r)=>{const o=Object.entries(i.homeTeam).filter(([a])=>!t.length||t.includes(a)).flatMap(([a,s])=>s.filter(d=>d.used).map(d=>({...d,_line:a})));return o.length?(si(o,e,`Choisir ${e} joueur(s) à ressusciter`,n,i,a=>{a.forEach(s=>{const d=(i.homeTeam[s._line]||[]).find(c=>c.cardId===s.cardId);d&&(d.used=!1,i.log.push({text:`💫 ${d.name} ressuscité !`,type:"info"}))}),$e(n,i,r)}),!0):(i.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),$e(n,i,r),!0)},REMOVE_GOAL:({},e)=>(e.aiScore>0?(e.aiScore--,e.log.push({text:"🚫 Dernier but IA annulé !",type:"info"})):e.log.push({text:"🚫 Aucun but à annuler",type:"info"}),!1),ADD_GOAL_DRAW:({},e)=>(e.homeScore===e.aiScore?(e.homeScore++,e.log.push({text:"🎯 But bonus (match nul) !",type:"info"})):e.log.push({text:"🎯 But bonus : non applicable (pas de match nul)",type:"info"}),!1),ADD_SUB:({value:e=1},t)=>(t.maxSubs=(t.maxSubs||3)+e,t.log.push({text:`🔄 +${e} remplacement(s) débloqué(s)`,type:"info"}),!1),CUSTOM:()=>!1};function xr(e,t,i,n,r){n.usedGc.push(e);const o=n.gcDefs||[],a=o.find(d=>d.name===t)||o.find(d=>{var c;return((c=d.name)==null?void 0:c.toLowerCase().trim())===(t==null?void 0:t.toLowerCase().trim())});let s=!1;if(a!=null&&a.effect_type&&a.effect_type!=="CUSTOM"){const d=yr[a.effect_type];d?d(a.effect_params||{},n,i,r)||(s=!0):(r.toast(`Effet "${a.effect_type}" non implémenté`,"error"),s=!0)}else{switch(t){case"Double attaque":n.modifiers.home.doubleAttack=!0,n.log.push({text:"⚡ Double attaque activée !",type:"info"});break;case"Bouclier":n.modifiers.home.shield=!0,n.log.push({text:"🛡️ Bouclier activé !",type:"info"});break;case"Ressusciter":{const d=Object.entries(n.homeTeam).flatMap(([c,l])=>(l||[]).filter(g=>g.used).map(g=>({...g,_line:c})));d.length?(d[0].used=!1,n.log.push({text:`💫 ${d[0].name} ressuscité !`,type:"info"})):n.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"});break}case"Vol de note":n.modifiers.ai.stolenNote=(n.modifiers.ai.stolenNote||0)+1,n.log.push({text:"🎯 -1 à la prochaine attaque IA",type:"info"});break;case"Gel":{const d=[...n.aiTeam.ATT||[],...n.aiTeam.MIL||[]].filter(c=>!c.used);if(d.length){const c=d.sort((l,g)=>Ne(g,"ATT")-Ne(l,"ATT"))[0];c.used=!0,n.log.push({text:`❄️ ${c.name} (IA) gelé !`,type:"info"})}break}case"Remplacement+":n.maxSubs++,n.log.push({text:"🔄 +1 remplacement débloqué",type:"info"});break}s=!0}v.from("cards").delete().eq("id",e).then(()=>{}),s&&$e(i,n,r)}function br(e,t,i){const n=Object.values(t.homeTeam).flat().filter(r=>!r.used);if(!n.length){i.toast("Aucun joueur actif à booster","error");return}i.openModal("⚡ Utiliser le Boost",`<div style="margin-bottom:12px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border-radius:10px;padding:12px;text-align:center;color:#000">
    ...
    <div style="display:flex;flex-direction:column;gap:6px">
      ${n.map(r=>`
        <div class="player-boost-opt" data-card-id="${r.cardId}"
          ...
          <div style="width:32px;height:32px;background:${Zo[r.job]||"#888"};border-radius:6px;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:900;font-size:13px">${Ne(r,r._line||r.job)}</div>
    ...
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ...
        </div>`}if(D&&!((jt=m.pendingAttack)!=null&&jt.players)&&O.length>0){const Fe=O.map(je=>{const kt=(rt[je._role]||[]).find(mt=>mt.cardId===je.cardId)||je,Ge=["GK","DEF"].includes(je._role),ft=rt[je._role]||[],st=ft.findIndex(mt=>mt.cardId===je.cardId),Bt=yt(ft.length),qt=st>=0?Bt[st]:kt._col??1;return{...kt,_line:je._role,_col:qt,...Ge?{note_a:Math.max(1,Number(kt.note_a)||0)}:{}}}),He=ti(Fe,((at=m.modifiers)==null?void 0:at[p])||{},m[p+"Formation"]);return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #FFD700;text-align:center">
      ...
      <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
        ...
        <div id="mobile-play-area" style="flex:1;min-height:0;display:flex;overflow:hidden">
          <div id="match-field" style="flex:1;min-width:0;min-height:0;overflow:hidden">
            <div class="terrain-wrapper" style="width:100%;height:100%;overflow:hidden">
              ${Ot(_,m[p+"Formation"],ut,Q,It(),Nt(),Ue)}
            </div>
          </div>
        </div>
        <div id="mobile-action-bar" style="position:absolute;left:0;right:0;bottom:0;z-index:20;background:rgba(0,0,0,0.55);padding:6px 8px 8px;display:flex;flex-direction:column;gap:6px;box-shadow:0 -4px 16px rgba(0,0,0,0.5)">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${L.map(T=>{const P={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[T._line]||"#555",D=Ne(T,T._line)+(T.boost||0);return`<div class="bp-item" data-cid="${T.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${P};overflow:hidden;cursor:pointer">
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        <div id="pin-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
          ...
          <div>${le?vt({...le,used:!1,boost:0},ye,xe):"<div>—</div>"}</div>
        ...
        <div id="pout-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
          ...
          <div>${ge?vt({...ge,used:!1,boost:0},ye,xe):"<div>—</div>"}</div>
      ...
      <div style="width:min(90vw,420px)">${yi(m[u+"Team"],m[u+"Formation"],null,[],It(),Nt())}</div>
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(_),(L=_.querySelector("#pvp-opp-close"))==null||L.addEventListener("click",()=>_.remove())}function R(){var S;const _=m.log||[],L=document.createElement("div");L.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column",L.innerHTML=`
    ...
    </div>`,(T=document.getElementById("pvp-home"))==null||T.addEventListener("click",()=>{try{v.removeChannel(ee)}catch{}it(e),l?f("mini-league",{openLeagueId:l}):f("home")})}Y()}async function Bi(e,t,i=!1){var s,d;const{state:n,navigate:r,toast:o}=t,a=i&&((d=(s=t==null?void 0:t.state)==null?void 0:s.params)==null?void 0:d.rankedData)||null;await xi(e,t,i?"ranked":"random",async({deckId:c,formation:l,starters:g,subsRaw:x,gcCardsEnriched:f,gcDefs:y,stadiumDef:p})=>{dt(e);const u=async b=>{await _r(e,t,c,l,g,x,b||[],y,p,i,a)};if(!(f!=null&&f.length)){await u([]);return}gi(e,f,u)})}async function wr(e,t,i){return vr(e,t,i)}async function _r(e,t,i,n,r,o,a=[],s=[],d=null,c=!1,l=null){var ie,ee;const{state:g,navigate:x,toast:f}=t,y=(l==null?void 0:l.mmr)??((ie=g.profile)==null?void 0:ie.mmr)??1e3,p=c?Gi(y):null,u=c?"linear-gradient(135deg, #1a0a2e 0%, #2d1060 50%, #1a0a2e 100%)":"linear-gradient(135deg, #0a1a2e 0%, #0d3d1e 100%)",b=c?(p==null?void 0:p.color)||"#D4A017":"#FFD700";let w=!1,h=null,E=null;e.innerHTML=`
    ...
    </div>`;const m=async()=>{w=!0,h&&(h.unsubscribe(),h=null),E&&(clearInterval(E),E=null);try{await v.rpc("cancel_matchmaking",{p_user_id:g.profile.id})}catch(j){console.error("[Matchmaking] cancel error:",j)}};(ee=document.getElementById("cancel-mm"))==null||ee.addEventListener("click",async()=>{await m(),it(e),x("home")});const M=async(j,I)=>{w||(w=!0,h&&(h.unsubscribe(),h=null),E&&(clearInterval(E),E=null),await At(e,t,j,I,{myGC:a,gcDefs:s,stadiumDef:d,isRanked:c,rankedData:l,onMatchEnd:c?kr:null}))},k=g.user.id,{data:z,error:q}=c?await v.rpc("try_matchmake_ranked",{p_user_id:k,p_deck_id:i,p_mmr:y}):await v.rpc("try_matchmake",{p_user_id:k,p_deck_id:i});if(q||!(z!=null&&z.success)){console.error("[Matchmaking] try_matchmake error:",q||z),f("Erreur de matchmaking","error"),it(e),x("home");return}if(z.matched){await M(z.match_id,!1);return}const te=async()=>{if(w)return;const{data:j}=await v.from("matchmaking_queue").select("status, match_id").eq("user_id",k).maybeSingle();(j==null?void 0:j.status)==="matched"&&j.match_id&&await M(j.match_id,!0)};h=v.channel(`mm_${k}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${k}`},j=>{const I=j.new;I.status==="matched"&&I.match_id&&M(I.match_id,!0)}).subscribe(),E=setInterval(te,3e3),setTimeout(()=>{w||m()},12e4)}async function kr(e,t){const{state:i,toast:n}=t,{winnerId:r,homeId:o,awayId:a,homeScore:s,awayScore:d,matchId:c}=e,l=i.user.id,g=o===l,x=r===l,{data:f}=await v.from("users").select("mmr, mmr_rd, mmr_v").eq("id",l).single(),{data:y}=await v.from("users").select("mmr, mmr_rd, mmr_v").eq("id",g?a:o).single();if(f&&y){const p=Wn(f.mmr,f.mmr_rd,f.mmr_v,y.mmr,y.mmr_rd,x?1:0);await v.from("users").update({mmr:p.mmr,mmr_rd:p.rd,mmr_v:p.v,mmr_wins:x?v.sql`mmr_wins + 1`:void 0,mmr_losses:x?void 0:v.sql`mmr_losses + 1`}).eq("id",l),n(x?`+MMR ↑ ${Math.round(p.mmr-f.mmr)}`:`-MMR ↓ ${Math.round(f.mmr-p.mmr)}`,x?"success":"error",4e3)}}const $r=Object.freeze(Object.defineProperty({__proto__:null,renderMatchRandom:Bi,resumePvpMatch:wr},Symbol.toStringTag,{value:"Module"}));async function Er(e,t){const{state:i,navigate:n,toast:r}=t,o=i.params||{},a=o.friendId||null,s=o.friendName||"Ami",d=!!o.isAccepting;await xi(e,t,"friend",async({deckId:c,formation:l,starters:g,subsRaw:x,gcCardsEnriched:f,gcDefs:y,stadiumDef:p})=>{dt(e);const u=async b=>{const w=b||[];d?await Tr(e,t,c,w,y,p):await Lr(e,t,c,w,y,p,a,s)};if(!(f!=null&&f.length)){await u([]);return}gi(e,f,u)})}async function Lr(e,t,i,n,r,o,a,s){var w;const{state:d,navigate:c,toast:l}=t,g=d.user.id,{data:x,error:f}=await v.from("friend_match_invites").insert({inviter_id:g,invitee_id:a,friend_id:a,status:"pending",inviter_deck_id:i,expires_at:new Date(Date.now()+2*60*1e3).toISOString()}).select("id").single();if(f||!x){console.error("[Friend] Erreur création invitation:",f),l("Impossible de créer l'invitation","error"),c("home");return}e.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="display:flex;background:var(--nav-bg,#0d1a0f);border-bottom:1px solid ${Lt}">
      ${w.map(m=>`<button class="ml-tab" data-tab="${m.key}" style="flex:1;padding:11px 4px;border:none;border-bottom:2px solid ${i===m.key?Nn:"transparent"};background:none;font-size:12px;font-weight:${i===m.key?"900":"600"};color:${i===m.key?"#4ade80":Ir};cursor:pointer">${m.label}${m.count?` (${m.count})`:""}</button>`).join("")}
    </div>
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:10px">
      ${i==="waiting"?zr(f,b,r):i==="active"?Ar(y,r):Mr(u,r,s)}
    </div>
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
      ...
      <div style="background:${Vt};border:1px solid ${Yt};border-radius:12px;padding:16px">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>${x.map((j,I)=>{const K=y>0&&a.status==="finished"?I===0?Math.floor(y*.7):I===1?Math.floor(y*.2):I===2?Math.floor(y*.1):0:0;return`<tr style="border-bottom:1px solid ${Lt};${j.userId===o?"background:rgba(74,222,128,0.08);":""}">
              ...
              ${y>0&&a.status==="finished"?`<td style="text-align:right;font-weight:700;color:${lt}">${K?K.toLocaleString("fr")+" cr.":"—"}</td>`:""}
            </tr>`}).join("")}</tbody>
        </table>
      </div>`:""}
    ...
    </div>`;let l=!1;const g=async u=>{if(!l){if(l=!0,clearInterval(y),f)try{f.unsubscribe()}catch{}u&&await At(e,t,u,r,{mlLeagueId:n,mlMatchId:i,myGC:o||[],gcDefs:a,stadiumDef:s}),d()}};(p=document.getElementById("wait-back"))==null||p.addEventListener("click",()=>{if(l=!0,clearInterval(y),f)try{f.unsubscribe()}catch{}c("mini-league",{openLeagueId:n}),d()});const x=async()=>{if(l)return;const{data:u}=await v.from("mini_league_matches").select("match_id").eq("id",i).single();u!=null&&u.match_id&&g(u.match_id)},f=v.channel(`ml_match_${i}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"mini_league_matches",filter:`id=eq.${i}`},u=>{var b;(b=u.new)!=null&&b.match_id&&g(u.new.match_id)}).subscribe(),y=setInterval(x,3e3);x()})}const Or="/",Hr=[{emoji:"⚽",title:"Bienvenue dans Manager Wars !",color:"#1A6B3C",content:`<p>Tu es désormais un <strong>manager de football</strong> virtuel.</p>
    ...
    <p style="margin-top:12px;font-size:13px;color:#888">Tu peux revoir ce tutoriel depuis les paramètres à tout moment.</p>`}];function Ur(e,t,i){let n=0;const r=document.createElement("div");r.id="tutorial-overlay",r.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.65);z-index:9900;display:flex;align-items:center;justify-content:center;padding:16px";const o=()=>{var l,g,x;const s=t[n],d=n===t.length-1,c=Math.round((n+1)/t.length*100);r.innerHTML=`
    ...
    `,r.querySelectorAll("ul,ol").forEach(f=>{f.style.paddingLeft="20px",f.style.marginTop="6px",f.style.marginBottom="6px"}),r.querySelectorAll("li").forEach(f=>{f.style.marginBottom="4px"}),r.querySelectorAll("p").forEach(f=>{f.style.marginBottom="8px"}),(l=r.querySelector("#tuto-prev"))==null||l.addEventListener("click",()=>{n--,o()}),(g=r.querySelector("#tuto-next"))==null||g.addEventListener("click",()=>{d?a():(n++,o())}),(x=r.querySelector("#tuto-skip"))==null||x.addEventListener("click",()=>{confirm("Passer le tutoriel ? Tu pourras le revoir plus tard depuis les paramètres.")&&a()})},a=async()=>{r.remove(),e!=null&&e.id&&await v.from("users").update({tutorial_done:!0}).eq("id",e.id),i==null||i()};document.body.appendChild(r),o()}async function Kr(e,t,i){if(!e||e.tutorial_done)return;let n=[];const{data:r,error:o}=await v.rpc("get_tutorial_steps");if(!o&&(r==null?void 0:r.length)>0)n=r,console.log(`[Tutorial] RPC OK → ${n.length} étapes`);else{const{data:s,error:d}=await v.from("tutorial_steps").select("*").eq("is_active",!0).order("step_order");!d&&(s==null?void 0:s.length)>0?(n=s,console.log(`[Tutorial] Direct OK → ${n.length} étapes`)):(console.warn(`[Tutorial] Aucune étape DB (RPC: ${o==null?void 0:o.message}, Direct: ${d==null?void 0:d.message})`),i&&i("[Tutorial] DB vide ou inaccessible — tuto local utilisé","warning",5e3))}const a=n.length>0?n.map(s=>({emoji:s.emoji,title:s.title,color:s.color,content:s.content,image_url:s.image_url||null})):Hr;Ur(e,a,()=>t("boosters"))}const di={GK:"#111111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function Ft(e,t,i=0){return t?(Number(t==="GK"?e.note_g:t==="DEF"?e.note_d:t==="MIL"?e.note_m:e.note_a)||0)+(t===e.job||t===e.job2?i:0):0}async function Vr(e,t){e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',await Vi(e,t)}async function Vi(e,t){const{state:i,toast:n}=t,{data:r}=await v.from("market_listings").select(`id, price, status, listed_at, seller_id,
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    </div>`}function x(p){var z,q,te,ie;const u=(z=p.card)==null?void 0:z.player;if(!u)return"";const b=((q=p.card)==null?void 0:q.evolution_bonus)||0,w=Ft(u,u.job,b),h=u.job2?Ft(u,u.job2,b):0,E=p.status==="sold",m=u.country_code?`https://flagsapi.com/${u.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,M=di[u.job]||"#bbb",k=u.job2?di[u.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${E?"opacity:0.7":""}">
      ${m?`<img src="${m}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      ${(te=u.clubs)!=null&&te.logo_url?`<img src="${u.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${M};display:flex;align-items:center;justify-content:center">
    ...
    </div>`}function f(){const p=document.getElementById("mkt-content"),u=document.getElementById("mkt-filters");if(p){if(u.style.display=d==="buy"?"flex":"none",d==="buy"){const b=l(a);p.innerHTML=b.length?b.map(g).join(""):'<div style="text-align:center;color:#aaa;padding:40px">Aucune carte trouvée.</div>'}else{const b=s.filter(h=>h.status==="active").sort((h,E)=>new Date(E.listed_at)-new Date(h.listed_at)),w=s.filter(h=>h.status==="sold").sort((h,E)=>new Date(E.sold_at||E.listed_at)-new Date(h.sold_at||h.listed_at));p.innerHTML=(b.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:4px 0 6px;text-transform:uppercase;letter-spacing:1px">🟢 En vente (${b.length})</div>`+b.map(x).join(""):"")+(w.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:12px 0 6px;text-transform:uppercase;letter-spacing:1px">✅ Ventes réussies (${w.length})</div>`+w.map(x).join(""):"")+(!b.length&&!w.length?'<div style="text-align:center;color:#aaa;padding:40px">Aucune vente pour le moment.</div>':"")}p.querySelectorAll("[data-buy]").forEach(b=>b.addEventListener("click",()=>Yr(b.dataset.buy,a,e,t))),p.querySelectorAll("[data-cancel]").forEach(b=>b.addEventListener("click",()=>Xr(b.dataset.cancel,e,t)))}}e.querySelectorAll(".mkt-tab").forEach(p=>{p.addEventListener("click",()=>{d=p.dataset.tab,e.querySelectorAll(".mkt-tab").forEach(u=>{const b=u===p;u.style.background=b?"var(--green)":"var(--tile-bg)",u.style.color=b?"#fff":"var(--tile-fg-dim)",u.style.borderColor=b?"var(--green)":"var(--tile-border)"}),f()})});let y;["flt-name","flt-club","flt-country","flt-job","flt-rarity","flt-note1","flt-note2"].forEach(p=>{var u;(u=document.getElementById(p))==null||u.addEventListener("input",()=>{clearTimeout(y),y=setTimeout(f,250)})}),f()}async function Yr(e,t,i,n){const{state:r,toast:o,refreshProfile:a}=n,s=t.find(l=>l.id===e);if(!s)return;const d=s.price;if((r.profile.credits||0)<d){o("Crédits insuffisants","error");return}Wr(s,async()=>{const{error:l}=await v.rpc("buy_market_card",{p_listing_id:e,p_buyer_id:r.profile.id});if(l){o("Erreur achat : "+l.message,"error");return}await a();const g=document.getElementById("nav-credits");g&&(g.textContent=`💰 ${(r.profile.credits||0).toLocaleString("fr")}`),o("✅ Carte achetée !","success"),await Vi(i,n)})}function Wr(e,t){var a;const i=(a=e.card)==null?void 0:a.player,n=i?`${i.firstname} ${i.surname_real}`:"cette carte",r=document.createElement("div");r.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",r.innerHTML=`
  ...
  <div style="min-height:100%;background:${w[y.id]};padding:16px;overflow-y:auto;display:flex;flex-direction:column;gap:16px">
    ...
    <div style="display:flex;flex-direction:column;gap:10px;margin-top:auto;padding-top:8px">
      ...
      away:users!away_id(pseudo,club_name)`).or(`home_id.eq.${r.id},away_id.eq.${r.id}`).order("created_at",{ascending:!1}).limit(50),a={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},s=(o||[]).filter(l=>l.status==="finished"),d=(o||[]).filter(l=>l.status==="in_progress");function c(l){const g=l.home_id===r.id;g?l.home_score:l.away_score,g?l.away_score:l.home_score;const x=l.winner_id===r.id,f=l.home_score===l.away_score&&l.status==="finished",y=l.status!=="finished"?"…":f?"N":x?"V":"D",p=l.status!=="finished"||f?"#888":x?"#1A6B3C":"#c0392b";let u=a[l.mode]||l.mode;l.away_id===null&&!u.startsWith("IA")&&(u="IA");const w=l.home_id===r.id?l.away:l.home;let h;l.away_id===null?h=u:w?h=`${w.club_name||w.pseudo} (${w.pseudo})`:h="Adversaire";let E="";l.status==="in_progress"&&Date.now()-new Date(l.created_at).getTime()>3600*1e3&&(E=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const m=new Date(l.created_at),M=m.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+m.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),k=l.status==="finished"?`${l.home_score} - ${l.away_score}`:`${l.home_score||0} - ${l.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
  ...
  <div class="page">
    ...
    <nav class="top-nav">
      <div class="logo" id="nav-logo" title="Manager Wars v2026.07.21-1212" style="cursor:pointer">
