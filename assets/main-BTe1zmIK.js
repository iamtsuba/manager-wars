...
import{s as v,j as ki,q as zo,t as Bt,u as $i,F as Ei,o as it,n as Ht,g as Ct,r as Se,m as wt,v as gi,w as Mo,T as Ci,p as xn,x as bn,d as yn}from"./special-cards-lEGt-tGs.js";const hn="/";function uo(t,{navigate:e,toast:i}){let o="login";const r=()=>{var a,s,d,c,l,f,b;const n=o==="login";if(t.innerHTML=`
  ...
  <div class="home-dark" id="home-dark">
    ...
    <div style="display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:8px;padding:10px 0;border-bottom:1px solid #f0f0f0">
      ...
      <div style="text-align:left;font-size:18px;font-weight:900;color:${T}">${I}</div>
  ...
  </div>`,t.querySelectorAll("[data-action]").forEach(a=>{a.addEventListener("click",()=>{a.style.transform="scale(.96)",setTimeout(()=>a.style.transform="",180);const s=a.dataset.action;if(s==="match-ai"){No(i,e);return}if(s==="match-random"){$t(),i("match",{matchMode:"random"});return}if(s==="match-friend"){i("friends");return}if(s==="mini-league"){i("mini-league");return}if(s==="ranked"){i("ranked");return}o("Bientôt disponible","info")})})}async function Oo(t,e){var f;const{state:i,navigate:o}=e,r=un(),n=Yi(),a=(f=i==null?void 0:i.profile)==null?void 0:f.is_admin,s="2026.07.27-1603";t.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    ${Se(t,{width:140})}
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),c=(s||[]).filter(M=>M.card_type==="player"&&M.player),l=(s||[]).filter(M=>M.card_type==="game_changer"),f=(s||[]).filter(M=>M.card_type==="formation"),b=(s||[]).filter(M=>M.card_type==="stadium"),{data:x}=await v.from("gc_definitions").select("name,gc_type,color,effect,image_url"),g={};(x||[]).forEach(M=>{g[M.name]=M});const{data:p}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),u={};(p||[]).forEach(M=>{u[M.id]=M}),b.forEach(M=>{M.stadium_def&&(u[M.stadium_id]=M.stadium_def)});const y=Object.keys(Ei),w=Object.keys(ze),h={};c.forEach(M=>{const Y=M.player.id;h[Y]=(h[Y]||0)+1}),new Set(Object.keys(h).map(M=>String(M)));const E=new Set(f.map(M=>M.formation)),m=new Set(l.map(M=>M.gc_type));let I="player",$="Tous",T="",F=!1;function H(){return[...c].sort((M,Y)=>{const q=di.indexOf(M.player.job),j=di.indexOf(Y.player.job);return q!==j?q-j:(M.player.surname_real||"").localeCompare(Y.player.surname_real||"")})}function K(){return[...d||[]].sort((M,Y)=>{const q=di.indexOf(M.job),j=di.indexOf(Y.job);return q!==j?q-j:(M.surname_real||"").localeCompare(Y.surname_real||"")})}function X(){return H().filter(M=>{const Y=M.player,q=$==="Tous"||Y.job===$,j=!T||`${Y.firstname} ${Y.surname_real}`.toLowerCase().includes(T);return q&&j})}function S(){return K().filter(M=>{const Y=$==="Tous"||M.job===$,q=!T||`${M.firstname} ${M.surname_real}`.toLowerCase().includes(T);return Y&&q})}t.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:0 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none;align-items:center"></div>
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",Y=>{T=Y.target.value.toLowerCase(),O()}),t.querySelectorAll(".filter-btn").forEach(Y=>{Y.addEventListener("click",()=>{$=Y.dataset.job,_(),O()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{F=!F,_(),O()})):(M.innerHTML=`
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;background:#f0fdf4;border-radius:10px;padding:10px 14px">
          <div>
            ...
            <div style="font-size:11px;color:#555;margin-top:2px">
              ${(t.evolution_bonus||0)>0?`+${t.evolution_bonus} appliqué${t.evolution_bonus>1?"s":""} · Note : ${T}${l.job2&&F>0?` / ${H}`:""}`:`Note actuelle : ${T}${l.job2&&F>0?` / ${H}`:""}`}
            </div>
          </div>
      ...
      <div style="flex:1;min-width:220px;display:flex;flex-direction:column;gap:12px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",l.note_g],["DEF",l.note_d],["MIL",l.note_m],["ATT",l.note_a]].map(([q,j])=>{const W=Jt[q],C=q==="GK"?"#fff":W,R=q===l.job||q===l.job2,ce=(Number(j)||0)+(R&&I>0?I:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(de=document.getElementById("close-detail"))==null||de.addEventListener("click",s);let _=new Set;const O=l.rarity==="pepite"?1.3:l.rarity==="papyte"?.7:1;function J(){let q=0;return document.querySelectorAll(".expl-check:checked").forEach(j=>{if(j.dataset.id===t.id)return;const C=Number(j.dataset.evo)||0,R=Number(j.dataset.note)||0;q+=R+C}),Math.round(q*O)}const re=()=>{const q=_.size,j=document.getElementById("sell-action-panel");if(!j)return;j.style.display=q>0?"block":"none",document.getElementById("sell-selected-count").textContent=q;const W=document.getElementById("evolve-btn");if(W){const C=J();W.textContent=C>0?`⬆️ Évoluer (+${C})`:"⬆️ Évoluer"}};document.querySelectorAll(".expl-check").forEach(q=>{q.addEventListener("change",()=>{const j=q.dataset.id;q.checked?_.add(j):_.delete(j);const W=q.closest(".exemplaire-row");if(W){const C=W.querySelector(".expl-sel-overlay"),R=W.querySelector(".expl-sel-check");C&&(C.style.display=q.checked?"block":"none"),R&&(R.style.display=q.checked?"flex":"none"),W.style.transform=q.checked?"scale(1.05)":"scale(1)"}re()})}),document.querySelectorAll(".exemplaire-row").forEach(q=>{q.addEventListener("click",j=>{if(j.target.tagName==="INPUT")return;const W=q.querySelector(".expl-check");W&&!W.disabled&&(W.checked=!W.checked,W.dispatchEvent(new Event("change")))})}),(me=document.getElementById("evolve-btn"))==null||me.addEventListener("click",async()=>{if(b<=1)return;const q=[..._];if(!q.length)return;if(_.has(t.id)){const ge=document.createElement("div");ge.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",ge.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    </div>`;u("⚽ Choisir une formation",w),document.querySelectorAll("#modal-body [data-forma]").forEach(h=>{h.addEventListener("click",()=>{e.formation=h.dataset.forma;const E=_o(e.formation),m={};E.forEach(I=>{e.slots[I]&&(m[I]=e.slots[I])}),e.slots=m,y(),_t(t,e,i)})})};t.querySelectorAll("#formation-mobile-btn, #formation-pc-btn").forEach(u=>u.addEventListener("click",x)),t.querySelectorAll("#add-stad-btn-pc, #add-stad-btn").forEach(u=>u.addEventListener("click",()=>Zn(e,t,i))),t.querySelectorAll("#save-deck, #save-deck-pc").forEach(u=>u.addEventListener("click",()=>ir(e,i))),t.querySelectorAll("[data-remove-sub]").forEach(u=>{u.addEventListener("click",()=>{e.subs=e.subs.filter(y=>y!==u.dataset.removeSub),_t(t,e,i)})}),t.querySelectorAll("#add-sub-btn").forEach(u=>u.addEventListener("click",()=>tr(e,t,i)))}function Qn(t,e,i,o){var h,E,m;const r=window.innerWidth>=900,n=t.querySelector(r?"#deck-field-pc":"#deck-field-mobile");if(!n)return;const a=(h=e.stadiumCards)==null?void 0:h.find(I=>I.id===e.stadiumCardId),s=a&&((E=e.stadDefMap)==null?void 0:E[a.stadium_id])||null,d=wt[e.formation]||{},c=$i(e.formation)||[],l={};for(const I of i){const $=e.slots[I],T=$?e.playerCards.find(F=>F.id===$):null;T!=null&&T.player?l[I]={...T.player,_evolution_bonus:T.evolution_bonus||0,face:T.player.face||null}:l[I]=null}const f=window.innerWidth>=900,b=f?window.innerWidth-280:window.innerWidth-20,x=f?Math.min(b,860):b,g=Math.round(f?x*.82:x*.85),p=f?84:44;let u="";for(const[I,$]of c){const T=d[I],F=d[$];if(!T||!F)continue;const H=T.x*x,K=Math.round(.03*g+T.y*.85*g),X=F.x*x,S=Math.round(.03*g+F.y*.85*g),_=l[I],O=l[$],J=Bt(_,O);J==="#ff3333"||J==="#cc2222"?u+=`<line x1="${H.toFixed(1)}" y1="${K.toFixed(1)}" x2="${X.toFixed(1)}" y2="${S.toFixed(1)}" stroke="${J}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(u+=`<line x1="${H.toFixed(1)}" y1="${K.toFixed(1)}" x2="${X.toFixed(1)}" y2="${S.toFixed(1)}" stroke="${J}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,u+=`<line x1="${H.toFixed(1)}" y1="${K.toFixed(1)}" x2="${X.toFixed(1)}" y2="${S.toFixed(1)}" stroke="${J}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let y="";const w=Math.round(p*657/507);for(const I of i){const $=d[I];if(!$)continue;const T=l[I],F=$.x*x;$.y*g;const H=Math.round(.03*g+$.y*(.85*g)),K=Math.round(F-p/2),X=Math.round(H-w/2);if(T){const S=I.replace(/\d+/,""),_=s&&(s.club_id&&String(T.club_id)===String(s.club_id)||s.country_code&&T.country_code===s.country_code),O=Se({...T,_evolution_bonus:T._evolution_bonus||0},{width:p,showStad:!0,stadDef:s,role:S});_&&((m=s.club)!=null&&m.logo_url||s.image_url),y+=`<div style="position:absolute;left:${K}px;top:${X}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${I}">
        <div style="position:relative">${O}</div>
      </div>`}else{const S=I.replace(/\d+/,"");y+=`<div style="position:absolute;left:${K}px;top:${X}px;width:${p}px;height:${w}px;
    ...
    <div style="position:relative;width:${x}px;height:${g}px;margin:0 auto">
      ...
      ${y}
    </div>`,n.querySelectorAll(".deck-slot-hit").forEach(I=>{I.addEventListener("click",()=>er(I.dataset.pos,e,t,o))})}function Zn(t,e,i){var a;const{openModal:o,closeModal:r}=i,n=t.stadiumCards||[];o("🏟️ Choisir un stade",`<div style="display:flex;flex-wrap:wrap;gap:10px;padding:8px">
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      card:cards(id,card_type,formation,stadium_id,evolution_bonus,
        player:players(id,firstname,surname_real,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order"),d=[...new Set((s||[]).filter(b=>{var x,g;return((x=b.card)==null?void 0:x.card_type)==="stadium"&&((g=b.card)==null?void 0:g.stadium_id)}).map(b=>b.card.stadium_id))],c={};if(d.length){const{data:b}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",d);(b||[]).forEach(x=>{c[x.id]=x}),(s||[]).forEach(x=>{var g,p;((g=x.card)==null?void 0:g.card_type)==="stadium"&&((p=x.card)!=null&&p.stadium_id)&&(x.card._stadiumDef=c[x.card.stadium_id]||null)})}let l=0;function f(){var m,I,$,T,F,H,K;const b=n[l],x=(s||[]).filter(X=>X.deck_id===b.id),g=x.filter(X=>{var S;return X.is_starter&&((S=X.card)==null?void 0:S.player)}).map(X=>Ri(X.card,X.position)),p=x.find(X=>{var S;return((S=X.card)==null?void 0:S.card_type)==="formation"}),u=b.formation||((m=p==null?void 0:p.card)==null?void 0:m.formation)||"4-4-2";let y=g.length>=11?hi(g,u):null,w=((I=b.stadium_card)==null?void 0:I.stadium_def)||null;w&&y&&(y=si(y,w));const h=g.length>=11;lt(t),t.style.height="100%",t.style.overflow="hidden",t.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff;position:relative">
      ...
      <!-- Pagination -->
      ${n.length>1?`<div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">${n.map((X,S)=>`<div style="width:6px;height:6px;border-radius:50%;background:${S===l?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}</div>`:""}
    ...
    </div>`,requestAnimationFrame(()=>requestAnimationFrame(function X(){const S=t.querySelector(".deck-preview-wrap"),_=t.querySelector("#deck-swipe-zone");if(!S||!_||!y)return;const O=_.clientWidth>=900,J=Math.max(200,_.clientHeight-(O?60:40)),le=Math.max(200,_.clientWidth-16),B=O?Math.min(117,Math.max(52,Math.round(le*.22))):Math.max(44,Math.round(le*.168));if(J<220||le<220){requestAnimationFrame(X);return}const U=O?null:Math.round(B*.55);S.innerHTML=Ut(y,u,null,[],le,J,[],U),S.style.cssText=`width:${le}px;height:${J}px;overflow:hidden;margin:${O?0:60}px auto 0`;const de=S.querySelector("svg");de&&(de.style.cssText="display:block;width:100%;height:100%",de.setAttribute("preserveAspectRatio",O?"xMidYMid meet":"none"))})),(T=document.getElementById("prev-deck"))==null||T.addEventListener("click",()=>{l>0&&(l--,f())}),(F=document.getElementById("next-deck"))==null||F.addEventListener("click",()=>{l<n.length-1&&(l++,f())}),(H=document.getElementById("validate-deck"))==null||H.addEventListener("click",()=>{if(!h)return;const X=e.state.params||{};e.navigate("match",{...X,matchMode:X.matchMode||i,deckId:b.id})}),(K=document.getElementById("cancel-deck-select"))==null||K.addEventListener("click",()=>{Ke(t),r("home")});const E=document.getElementById("deck-swipe-zone");if(E){let X=0,S=0;E.addEventListener("touchstart",_=>{X=_.touches[0].clientX,S=_.touches[0].clientY},{passive:!0}),E.addEventListener("touchend",_=>{const O=_.changedTouches[0].clientX-X,J=_.changedTouches[0].clientY-S;Math.abs(O)<40||Math.abs(O)<Math.abs(J)||(O<0&&l<n.length-1?(l++,f()):O>0&&l>0&&(l--,f()))},{passive:!0})}}f()}function vt(t,e=44,i=58,o=null){return Se(t,{width:e,showStad:!!o,stadDef:o,used:t==null?void 0:t.used})}function Qe(t,e,i,o,r){if(!(t!=null&&t.length))return"";const n=t.slice(0,5);let a='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return n.forEach((s,d)=>{const c=s._line||s.job||"MIL";let l=s.boost||0;if(s.stadiumBonus&&(o==="attack"&&(c==="ATT"||c==="MIL")||o==="defense"&&(c==="GK"||c==="DEF"||c==="MIL")?l+=10:o||(l+=10)),a+=Se({...s,_evolution_bonus:0,evolution_bonus:0},{width:40,role:c,extraNote:l}),d<n.length-1){const f=n[d+1],b=Ho(s,f,r)?Bt(s,f):null;a+=`<div style="width:7px;height:3px;background:${!b||b==="#ff3333"||b==="#cc2222"?"rgba(255,255,255,0.12)":b};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(a+=`<div style="margin-left:6px;background:${e};color:${e==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),a+="</div>",a}function Ti(t,e,i,o,r=310,n=310,a=[],s=null){const d=wt[e]||{},c=$i(e)||Ei[e]||[],l={},f=["ATT","MIL","DEF","GK"];for(const y of f)(t[y]||[]).forEach((h,E)=>{l[`${y}${E+1}`]=h});function b(y){const w=d[y];return w?{x:w.x*r,y:w.y*n}:null}let x="";for(const[y,w]of c){const h=b(y),E=b(w);if(!h||!E)continue;const m=l[y],I=l[w],$=Bt(m,I);$==="#00ff88"||$==="#FFD700"?(x+=`<line x1="${h.x.toFixed(1)}" y1="${h.y.toFixed(1)}" x2="${E.x.toFixed(1)}" y2="${E.y.toFixed(1)}"
    ...
    </div>`}async function _r(t,e){const{state:i}=e,o=i.params||{},r=o.matchMode||"vs_ai_easy",n=r==="solo",a=r==="ranked_ai",s=o.soloLevel||1,d=r.replace("vs_ai_",""),c=r;console.log("[DEBUG renderMatchIA]",{matchMode:r,isRankedAI:a,hasPresetSetup:!!o.presetSetup,params:o});const l=document.createElement("div");l.style.cssText="position:fixed;top:4px;left:4px;z-index:99999;background:#000;color:#0f0;font-size:11px;padding:6px 10px;border-radius:6px;font-family:monospace;max-width:90vw;word-break:break-all",l.textContent=`DEBUG matchMode="${r}" isRankedAI=${a} presetSetup=${!!o.presetSetup}`,document.body.appendChild(l),setTimeout(()=>l.remove(),15e3);let f=null;if(n){const{data:p}=await v.from("solo_levels").select("*").eq("level_number",s).maybeSingle();f=p||{level_number:s,target_note_avg:10,nb_liens_jaune:2,nb_liens_vert:1,nb_joueurs_stade:2,reward_credits:500}}const b=o.rankedData||null,x=o.presetSetup||null;async function g({deckId:p,formation:u,starters:y,subsRaw:w,gcCardsEnriched:h,gcDefs:E,stadiumDef:m}){try{let I=hi(y,u);m&&(I=si(I,m),yi(w,m));let $=null;if(a){const K=$r(I),{jaune:X,vert:S}=Er(I,u),_=.9+Math.random()*.2;$={target_note_avg:Math.max(1,Math.round(K*_/16)),nb_liens_jaune:X,nb_liens_vert:S,nb_joueurs_stade:m?3:0}}const T=n?await $o(u,f):a?await $o(u,$):await kr(u,d),F=T.lines||T,H=async K=>{try{const X=c==="vs_ai_club"||c==="solo"||c==="ranked_ai"?"club":c,{data:S,error:_}=await v.from("matches").insert({home_id:i.profile.id,away_id:null,mode:X,home_deck_id:p,status:"in_progress"}).select().single();if(_){console.error("[MatchIA] Erreur création match:",_),At(t,"⚠️","Impossible de créer le match ("+_.message+").","Retour",()=>e.navigate("home"));return}const O=T.stadiumDef||null;O&&F&&(si(F,O),yi(T.subs||[],O));const J={gcDefs:E||[],matchId:S==null?void 0:S.id,mode:c,difficulty:d,formation:u,isSolo:n,soloLevel:s,soloLevelConfig:f,isRankedAI:a,rankedAIData:b,homeTeam:I,aiTeam:F,homeSubs:w,subsUsed:0,maxSubs:Math.min(w.length,3),aiSubs:T.subs||[],aiSubsUsed:0,aiMaxSubs:Math.min((T.subs||[]).length,3),aiUsedSubIds:[],aiGcCards:T.gcCards||[],aiUsedGc:[],aiStadiumDef:O,homeScore:0,aiScore:0,gcCards:K,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Lr(t,J,e)}catch(X){console.error("[MatchIA] Exception launchMatch:",X),At(t,"⚠️","Erreur au lancement du match : "+X.message,"Retour",()=>e.navigate("home"))}};if(a&&x){H(x.gcCardsEnriched||[]);return}if(!h.length){H([]);return}Ii(t,h,H)}catch(I){console.error("[MatchIA] Exception setup:",I),At(t,"⚠️","Erreur de préparation du match : "+I.message,"Retour",()=>e.navigate("home"))}}x?await g(x):await Si(t,e,r,g)}async function kr(t,e){var u;const{data:i}=await v.from("players").select("id,firstname,surname_real,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(80);if(!i||i.length<11)return{lines:rn(t),subs:[],gcCards:[],stadiumDef:null};const o=kt[t]||kt["4-4-2"],r={GK:[],DEF:[],MIL:[],ATT:[]},n=new Set;function a(y,w,h){var E,m;return n.add(y.id),{cardId:"ai-"+y.id+"-"+h,id:y.id,firstname:y.firstname,name:y.surname_real,country_code:y.country_code,club_id:y.club_id,job:y.job,job2:y.job2,note_g:Number(y.note_g)||0,note_d:Number(y.note_d)||0,note_m:Number(y.note_m)||0,note_a:Number(y.note_a)||0,rarity:y.rarity,skin:y.skin,hair:y.hair,hair_length:y.hair_length,face:y.face,clubName:((E=y.clubs)==null?void 0:E.encoded_name)||null,clubLogo:((m=y.clubs)==null?void 0:m.logo_url)||null,boost:0,used:!1,_line:w}}for(const y of["GK","DEF","MIL","ATT"]){const w=i.filter($=>$.job===y&&!n.has($.id)),h=i.filter($=>$.job!==y&&!n.has($.id)),E=[...w,...h],m=[];for(let $=0;$<o[y];$++){const T=E[$];T&&m.push(a(T,y,$))}const I=st(m.length);m.forEach(($,T)=>{$._col=I[T]}),r[y]=m}const d=i.filter(y=>!n.has(y.id)).slice(0,5).map((y,w)=>a(y,y.job,100+w)),f=Object.keys(ze).sort(()=>Math.random()-.5).slice(0,3).map((y,w)=>{var h,E;return{id:"ai-gc-"+w,gc_type:y,name:((h=ze[y])==null?void 0:h.name)||y,icon:((E=ze[y])==null?void 0:E.icon)||"⚡"}}),b=Object.values(r).flat(),x={};b.forEach(y=>{y.club_id&&(x[y.club_id]=(x[y.club_id]||0)+1)});const g=(u=Object.entries(x).sort((y,w)=>w[1]-y[1])[0])==null?void 0:u[0];let p=null;if(g){const{data:y}=await v.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",g).single();y&&(p={club_id:y.id,country_code:null,name:y.encoded_name+" Stadium",club:{encoded_name:y.encoded_name,logo_url:y.logo_url}})}return{lines:r,subs:d,gcCards:f,stadiumDef:p}}function $r(t){let e=0;for(const i of["GK","DEF","MIL","ATT"])(t[i]||[]).forEach(o=>{const r=o._line||i,n=Number(r==="GK"?o.note_g:r==="DEF"?o.note_d:r==="MIL"?o.note_m:o.note_a)||0;e+=n+(o.boost||0)});return e}function Er(t,e){const i={};for(const a of["GK","DEF","MIL","ATT"])(t[a]||[]).forEach((s,d)=>{i[`${a}${d+1}`]=s});const o=$i(e)||Ei[e]||[];let r=0,n=0;for(const[a,s]of o){const d=i[a],c=i[s];if(!d||!c)continue;const l=Bt(d,c);l==="#00ff88"?n++:l==="#FFD700"&&r++}return{jaune:r,vert:n}}function ko(t){const e=t.job||"ATT";return Number(e==="GK"?t.note_g:e==="DEF"?t.note_d:e==="MIL"?t.note_m:t.note_a)||0}function ui(t,e,i){if(i<=0||!t.length)return[];const o=[...t].sort((r,n)=>Math.abs(ko(r)-e)-Math.abs(ko(n)-e));return o.slice(0,Math.min(i,o.length))}async function $o(t,e){var X,S;const i=Number(e==null?void 0:e.target_note_avg)||10,o=Number(e==null?void 0:e.nb_liens_jaune)||0,r=Number(e==null?void 0:e.nb_liens_vert)||0,n=Number(e==null?void 0:e.nb_joueurs_stade)||0,{data:a}=await v.from("players").select("id,firstname,surname_real,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,clubs(encoded_name,logo_url,country_code)").eq("is_active",!0).limit(300);if(!a||a.length<16)return{lines:rn(t),subs:[],gcCards:[],stadiumDef:null};const s={};a.forEach(_=>{_.club_id&&(s[_.club_id]=(s[_.club_id]||0)+1)});const d=((X=Object.entries(s).sort((_,O)=>O[1]-_[1])[0])==null?void 0:X[0])||null,c=d?a.filter(_=>_.club_id===d):[],l=((S=c[0])==null?void 0:S.country_code)||null,f=l?a.filter(_=>_.country_code===l&&_.club_id!==d):[],b=Math.min(c.length,Math.max(2,r+1),16),x=Math.min(f.length,Math.max(0,o),16-b),g=[],p=new Set;ui(c,i,b).forEach(_=>{g.push(_),p.add(_.id)}),ui(f,i,x).forEach(_=>{g.push(_),p.add(_.id)});const u=Math.max(0,16-g.length),y=a.filter(_=>!p.has(_.id));ui(y,i,u).forEach(_=>{g.push(_),p.add(_.id)});const w=kt[t]||kt["4-4-2"],h={GK:[],DEF:[],MIL:[],ATT:[]},E=new Set;function m(_,O,J){var re,le;return E.add(_.id),{cardId:"ai-"+_.id+"-"+J,id:_.id,firstname:_.firstname,name:_.surname_real,country_code:_.country_code,club_id:_.club_id,job:_.job,job2:_.job2,note_g:Number(_.note_g)||0,note_d:Number(_.note_d)||0,note_m:Number(_.note_m)||0,note_a:Number(_.note_a)||0,rarity:_.rarity,skin:_.skin,hair:_.hair,hair_length:_.hair_length,face:_.face,clubName:((re=_.clubs)==null?void 0:re.encoded_name)||null,clubLogo:((le=_.clubs)==null?void 0:le.logo_url)||null,boost:0,used:!1,_line:O}}for(const _ of["GK","DEF","MIL","ATT"]){const O=g.filter(U=>U.job===_&&!E.has(U.id)),J=g.filter(U=>U.job!==_&&!E.has(U.id)),re=[...O,...J],le=[];for(let U=0;U<w[_];U++){const de=re[U];de&&le.push(m(de,_,U))}const B=st(le.length);le.forEach((U,de)=>{U._col=B[de]}),h[_]=le}let I=g.filter(_=>!E.has(_.id));if(I.length<5){const _=a.filter(O=>!E.has(O.id)&&!I.some(J=>J.id===O.id));I=I.concat(ui(_,i,5-I.length))}const $=I.slice(0,5).map((_,O)=>m(_,_.job,100+O)),H=Object.keys(ze).sort(()=>Math.random()-.5).slice(0,3).map((_,O)=>{var J,re;return{id:"ai-gc-"+O,gc_type:_,name:((J=ze[_])==null?void 0:J.name)||_,icon:((re=ze[_])==null?void 0:re.icon)||"⚡"}});let K=null;if(n>0&&d){const{data:_}=await v.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",d).single();_&&(K={club_id:_.id,country_code:null,name:_.encoded_name+" Stadium",club:{encoded_name:_.encoded_name,logo_url:_.logo_url}})}return{lines:h,subs:$,gcCards:H,stadiumDef:K}}function rn(t){const e=kt[t]||kt["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},o=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let r=0;for(const n of["GK","DEF","MIL","ATT"]){const a=[];for(let d=0;d<e[n];d++){const c=3+Math.floor(Math.random()*5);a.push({cardId:"fake-"+r,id:"fake-"+r,firstname:"IA",name:o[r%o.length],country_code:"XX",club_id:null,job:n,job2:null,note_g:n==="GK"?c:2,note_d:n==="DEF"?c:2,note_m:n==="MIL"?c:2,note_a:n==="ATT"?c:2,rarity:"normal",boost:0,used:!1,_line:n}),r++}const s=st(a.length);a.forEach((d,c)=>{d._col=s[c]}),i[n]=a}return i}function Lr(t,e,i){var r;Bo("/sounds/match-opening.mp3",.3);const o=`<div style="position:relative;width:22px;height:22px;flex-shrink:0;display:flex;align-items:center;justify-content:center">
  ...
  <div id="match-history-panel">
    ...
    </div>`,(m=f.querySelector("#sub-close"))==null||m.addEventListener("click",()=>f.remove()),(I=f.querySelector("#out-up"))==null||I.addEventListener("click",()=>{s>0&&(s--,b())}),($=f.querySelector("#out-down"))==null||$.addEventListener("click",()=>{s<n.length-1&&(s++,b())}),(T=f.querySelector("#in-up"))==null||T.addEventListener("click",()=>{c>0&&(c--,b())}),(F=f.querySelector("#in-down"))==null||F.addEventListener("click",()=>{c<a.length-1&&(c++,b())});const E=(K,X,S,_)=>{const O=f.querySelector("#"+K);if(!O)return;let J=0;O.addEventListener("touchstart",re=>{J=re.touches[0].clientY},{passive:!0}),O.addEventListener("touchend",re=>{const le=re.changedTouches[0].clientY-J;if(Math.abs(le)<30)return;const B=X();le<0&&B<_-1?(S(B+1),b()):le>0&&B>0&&(S(B-1),b())},{passive:!0})};E("in-panel",()=>c,K=>c=K,a.length),E("out-panel",()=>s,K=>s=K,n.length),(H=f.querySelector("#sub-confirm"))==null||H.addEventListener("click",K=>{if(K.preventDefault(),K.stopPropagation(),l)return;l=!0;const X=n[s],S=a[c];if(!X||!S)return;let _=null,O=-1;for(const[re,le]of Object.entries(e.homeTeam)){const B=(le||[]).findIndex(U=>U.cardId===X.cardId);if(B!==-1){_=re,O=B;break}}if(O===-1||!_){It("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),f.remove();return}const J={...S,_line:_,_col:X._col||0,used:!1,boost:0};e.homeTeam[_].splice(O,1,J),e.usedSubIds||(e.usedSubIds=[]),e.usedSubIds.push(S.cardId),e.subsUsed++,e.selected=[],e.log.push({type:"sub",subSide:"home",clubName:e.clubName,outPlayer:{name:X.name,firstname:X.firstname,note:Pe(X,_),portrait:Ct(X),job:X.job,country_code:X.country_code,rarity:X.rarity,clubName:X.clubName,clubLogo:X.clubLogo},inPlayer:{name:S.name,firstname:S.firstname,note:Pe(S,_),portrait:Ct(S),job:S.job,country_code:S.country_code,rarity:S.rarity,clubName:S.clubName,clubLogo:S.clubLogo},text:`🔄 ${S.firstname} ${S.name} remplace ${X.firstname} ${X.name}`}),f.remove(),wr(X,S,()=>$e(t,e,i))})}document.body.appendChild(f),b()}function jr(t,e,i,o,r){var b,x;const n=(o.gcDefs||[]).find(g=>g.name===e),a=ze[e]||{icon:"⚡",desc:"Carte spéciale."},s=(n==null?void 0:n.name)||e,d=(n==null?void 0:n.effect)||a.desc,c=n!=null&&n.image_url?`/icons/${n.image_url}`:null,l=a.icon||"⚡",f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",f.innerHTML=`
    ...
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      ...
      ${Ti(t.aiTeam,t.formation,null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.1))}
            ...
            </div>`}}catch(ee){console.error("[Ranked] MMR update error:",ee)}(ie=document.getElementById("pvp-end-overlay"))==null||ie.remove();const P=document.createElement("div");P.id="pvp-end-overlay",P.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;color:#fff;padding:24px;text-align:center;overflow-y:auto";const N=A?"🤝":D?"🏆":"😞",V=A?"MATCH NUL":D?"VICTOIRE !":"DÉFAITE",Z=A?"#fff":D?"#FFD700":"#ff6b6b";P.innerHTML=`
      ...
      ${G}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${l?"🏆 Retour à la Mini League":a?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(P),(te=P.querySelector("#pvp-end-home"))==null||te.addEventListener("click",()=>{P.remove(),Ke(t),l?x("mini-league",{openLeagueId:l}):x(a?"ranked":"home")})}const X=v.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},k=>{var z;const L=k.new;try{if(L.status==="finished"||L.forfeit){if(I)return;I=!0,$&&(clearInterval($),$=null),Ue(),L.game_state&&(m=L.game_state),K(L);return}if(L.game_state){const D=m;m=L.game_state;const A=m._lastGC;if(A&&A.seq&&!H.has(A.seq)&&(H.add(A.seq),A.by!==p)){M(A.type,A.by,()=>U());return}const G=D[p+"Score"]||0,P=D[u+"Score"]||0,N=m[p+"Score"]||0,V=m[u+"Score"]||0,Z=N>G,ie=V>P;if((Z||ie)&&!F.has(m.round)){F.add(m.round);const te=[...m.log||[]].reverse().find(se=>se.isGoal),ee=((te==null?void 0:te.homePlayers)||[]).map(se=>({name:se.name,note:se.note,portrait:se.portrait,job:se.job}));!!((z=te==null?void 0:te.text)!=null&&z.includes("DERNIER CORNER"))?ge().then(()=>{Ze(ee,N,V,Z,()=>U())}):Ze(ee,N,V,Z,()=>U());return}U()}}catch(D){console.error("[PvP] crash:",D)}}).subscribe();async function S(k){Object.assign(m,k),m.lastActionAt=new Date().toISOString();const{error:L}=await v.from("matches").update({game_state:m}).eq("id",i);L&&g("Erreur de synchronisation","error");try{U()}catch(z){console.error("[PvP] renderPvpScreen crash:",z)}}async function _(){if(I)return;I=!0,Rt(),Ue(),$&&(clearInterval($),$=null);const k=o?h.away_id:h.home_id,L=o?"p2":"p1",z=o?"p1":"p2",D={...m,[L+"Score"]:3,[z+"Score"]:0,phase:"finished"},A=D.p1Score||0,G=D.p2Score||0;try{await v.from("matches").update({game_state:D}).eq("id",i);const{data:P,error:N}=await v.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:k,p_home_score:A,p_away_score:G,p_forfeit:!0});if(N&&console.error("[PvP] finish_pvp_match (forfeit):",N),l)try{await v.rpc("check_and_finish_mini_league",{p_league_id:l})}catch(V){console.error("[PvP] check_and_finish_mini_league (forfeit):",V)}try{const{data:V}=await v.rpc("apply_match_rewards",{p_match_id:i});V!=null&&V.success&&!(V!=null&&V.skipped)&&typeof e.refreshProfile=="function"&&await e.refreshProfile()}catch(V){console.error("[PvP] apply_match_rewards (forfeit):",V)}}catch{}try{v.removeChannel(X)}catch{}setTimeout(()=>{Ke(t),l?x("mini-league",{openLeagueId:l}):x("home")},800)}const O={BOOST_STAT:({value:k=1,count:L=1,roles:z=[]},D,A)=>{const G=D[p+"Team"],P=Object.entries(G).filter(([N])=>!z.length||z.includes(N)).flatMap(([N,V])=>V.filter(Z=>!Z.used).map(Z=>({...Z,_line:N})));if(!P.length){D.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),A(D);return}J(P,L,`Choisir ${L} joueur(s) à booster (+${k})`,N=>{N.forEach(V=>{const Z=(G[V._line]||[]).find(ie=>ie.cardId===V.cardId);Z&&(Z.boost=(Z.boost||0)+k,D.log.push({text:`⚡ +${k} sur ${Z.name}`,type:"info"}))}),D[p+"Team"]=G,A(D)})},DEBUFF_STAT:({value:k=1,count:L=1,roles:z=[],target:D="ai"},A,G)=>{const P=D==="home"?p:u,N=A[P+"Team"],V=D==="home"?"allié":"adverse",Z=Object.entries(N).filter(([ie])=>!z.length||z.includes(ie)).flatMap(([ie,te])=>te.map(ee=>({...ee,_line:ie})));if(!Z.length){A.log.push({text:`🎯 Aucun joueur ${V}`,type:"info"}),G(A);return}J(Z,L,`Choisir ${L} joueur(s) ${V}(s) (-${k})`,ie=>{ie.forEach(te=>{const ee=(N[te._line]||[]).find(ne=>ne.cardId===te.cardId);ee&&(ee.boost=(ee.boost||0)-k,A.log.push({text:`🎯 -${k} sur ${ee.name}`,type:"info"}))}),A[P+"Team"]=N,G(A)})},GRAY_PLAYER:({count:k=1,roles:L=[],target:z="ai"},D,A)=>{const G=z==="home"?p:u,P=D[G+"Team"],N=z==="home"?"allié":"adverse",V=Object.entries(P).filter(([Z])=>!L.length||L.includes(Z)).flatMap(([Z,ie])=>ie.filter(te=>!te.used).map(te=>({...te,_line:Z})));if(!V.length){D.log.push({text:`❌ Aucun joueur ${N}`,type:"info"}),A(D);return}J(V,k,`Choisir ${k} joueur(s) ${N}(s) à exclure`,Z=>{const ie="usedCardIds_"+G,te=new Set(D[ie]||[]);Z.forEach(ee=>{const ne=(P[ee._line]||[]).find(se=>se.cardId===ee.cardId);ne&&(ne.used=!0,te.add(ee.cardId),D.log.push({text:`❌ ${ne.name} exclu !`,type:"info"}))}),D[ie]=[...te],D[G+"Team"]=P,A(D)})},REVIVE_PLAYER:({count:k=1,roles:L=[]},z,D)=>{const A=z[p+"Team"],G=Object.entries(A).filter(([P])=>!L.length||L.includes(P)).flatMap(([P,N])=>N.filter(V=>V.used).map(V=>({...V,_line:P})));if(!G.length){z.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),D(z);return}J(G,k,`Choisir ${k} joueur(s) à ressusciter`,P=>{P.forEach(N=>{const V=(A[N._line]||[]).find(Z=>Z.cardId===N.cardId);V&&(V.used=!1,z.log.push({text:`💫 ${V.name} ressuscité !`,type:"info"}))}),z[p+"Team"]=A,D(z)})},REMOVE_GOAL:({},k,L)=>{const z=u+"Score";k[z]>0?(k[z]--,k.log.push({text:"🚫 Dernier but annulé !",type:"info"})):k.log.push({text:"🚫 Aucun but à annuler",type:"info"}),L(k)},ADD_GOAL_DRAW:({},k,L)=>{k[p+"Score"]===k[u+"Score"]?(k[p+"Score"]++,k.log.push({text:"🎯 But bonus !",type:"info"})):k.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),L(k)},ADD_SUB:({value:k=1},L,z)=>{L.maxSubs=(L.maxSubs||3)+k,L.log.push({text:`🔄 +${k} remplacement(s)`,type:"info"}),z(L)},CUSTOM:({},k,L)=>L(k)};function J(k,L,z,D){const A=document.createElement("div");A.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let G=[];function P(){var V,Z;const N=k.map(ie=>{const te=ie._line||ie.job||"MIL",ee=G.find(se=>se.cardId===ie.cardId),ne=Se({...ie,_evolution_bonus:0},{width:90,showStad:!0,role:te,extraNote:ie.boost||0});return`<div class="pp-item" data-cid="${ie.cardId}"
        ...
        </div>`,(V=A.querySelector("#pp-close"))==null||V.addEventListener("click",()=>A.remove()),A.querySelectorAll(".pp-item").forEach(ie=>{ie.addEventListener("click",()=>{const te=ie.dataset.cid,ee=k.find(se=>se.cardId===te),ne=G.findIndex(se=>se.cardId===te);ee&&(ne>-1?G.splice(ne,1):G.length<L&&G.push(ee),P())})}),(Z=A.querySelector("#pp-confirm"))==null||Z.addEventListener("click",()=>{A.remove(),D(G)})}P(),document.body.appendChild(A)}async function re(k,L){const D=(m["gcCardsFull_"+p]||[]).find(N=>N.id===k),A=(D==null?void 0:D._gcDef)||(m.gcDefs||[]).find(N=>{var V;return N.name===L||((V=N.name)==null?void 0:V.toLowerCase().trim())===(L==null?void 0:L.toLowerCase().trim())}),G=[...m["usedGc_"+p]||[],k],P={type:L,by:p,seq:(m._gcAnimSeq||0)+1};H.add(P.seq),M(L,p,async()=>{if(A!=null&&A.effect_type&&A.effect_type!=="CUSTOM"){const V=O[A.effect_type];if(V){const Z={...m};V(A.effect_params||{},Z,async ie=>{ie["usedGc_"+p]=G,ie._lastGC=P,ie._gcAnimSeq=P.seq,await S(ie)});return}}const N={...m};switch(L){case"Remplacement+":N.maxSubs=(N.maxSubs||3)+1,N.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const V=u+"Score";N[V]>0&&(N[V]--,N.log.push({text:"🚫 But annulé",type:"info"}));break}}N["usedGc_"+p]=G,N._lastGC=P,N._gcAnimSeq=P.seq,await S(N)})}function le(k,L){const z="usedCardIds_"+k,D=new Set(m[z]||[]);L.forEach(A=>D.add(A)),m[z]=[...D]}function B(){for(const k of["p1","p2"]){const L=new Set(m["usedCardIds_"+k]||[]),z=m[k+"Team"];if(z)for(const D of["GK","DEF","MIL","ATT"])(z[D]||[]).forEach(A=>{A.used=L.has(A.cardId)})}}function U(){var ro,ao,so,lo,co,po;if(m.phase==="reveal")return de();if(m.phase==="midfield")return fe();if(m.phase==="finished")return Ne();const k=m[p+"Team"],L=m[u+"Team"];B();const z=m[p+"Score"],D=m[u+"Score"],A=m[p+"Name"],G=m[u+"Name"],P=m.phase===p+"-attack",N=m.phase===p+"-defense",V=Array.isArray(m["selected_"+p])?m["selected_"+p]:[],Z=V.map(Q=>Q.cardId),ie=window.innerWidth>=700,te=m[p+"Subs"]||[],ee=m["usedSubIds_"+p]||[],ne=te.filter(Q=>!ee.includes(Q.cardId)),se=m["gcCardsFull_"+p]||[],be=m["usedGc_"+p]||[],pe=se.filter(Q=>!be.includes(Q.id)),ye=m.boostOwner===p&&!m.boostUsed,we=[...k.MIL||[],...k.ATT||[]].filter(Q=>!Q.used),oe=[...L.MIL||[],...L.ATT||[]].filter(Q=>!Q.used),Ie=!Ee(L),Ge=we.length===0&&oe.length===0&&!Ie,Te=(k.DEF||[]).filter(Q=>!Q.used),Ye=(k.GK||[]).filter(Q=>!Q.used);let Oe=[];P&&we.length===0&&(Ie?(Oe=Te.map(Q=>Q.cardId),Te.length===0&&(Oe=Oe.concat(Ye.map(Q=>Q.cardId)))):Ge&&(Oe=[...Te,...Ye].map(Q=>Q.cardId)));function ot(Q,xe,Me){var Ce,mt;const ke=Q._gcDef,Ft=(ke==null?void 0:ke.name)||Q.gc_type,at=(ke==null?void 0:ke.effect)||((Ce=ze[Q.gc_type])==null?void 0:Ce.desc)||"",Fe=ke!=null&&ke.image_url?`/icons/${ke.image_url}`:null,Re=((mt=ze[Q.gc_type])==null?void 0:mt.icon)||"⚡";return`<div class="pvp-gc-mini" data-gc-id="${Q.id}" data-gc-type="${Q.gc_type}" style="flex-shrink:0;cursor:pointer">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(te);let ee=!1;const ne=()=>{ee||(ee=!0,te.remove(),setTimeout(()=>z&&z(),50))};te.addEventListener("click",ne),setTimeout(ne,3e3)}function Y(k,L){var ie,te,ee,ne;const D=(m["gcCardsFull_"+p]||[]).find(se=>se.id===k),A=D==null?void 0:D._gcDef,G=(A==null?void 0:A.name)||L,P=(A==null?void 0:A.effect)||((ie=ze[L])==null?void 0:ie.desc)||"Carte spéciale.",N=A!=null&&A.image_url?`/icons/${A.image_url}`:null,V=((te=ze[L])==null?void 0:te.icon)||"⚡",Z=document.createElement("div");Z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",Z.innerHTML=`
      ${it(G,N,V,P,{width:190})}
      <div style="display:flex;gap:12px;width:190px">
        <button id="pvp-gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
        <button id="pvp-gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
      </div>`,document.body.appendChild(Z),(ee=Z.querySelector("#pvp-gc-back"))==null||ee.addEventListener("click",()=>Z.remove()),(ne=Z.querySelector("#pvp-gc-use"))==null||ne.addEventListener("click",()=>{Z.remove(),re(k,L)})}function q(){var D;const k=m[p+"Team"],L=Object.entries(k).flatMap(([A,G])=>(G||[]).filter(P=>!P.used).map(P=>({...P,_line:A})));if(!L.length)return;const z=document.createElement("div");z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",z.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
      ...
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${L.map(A=>{const G={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[A._line]||"#555",P=Pe(A,A._line)+(A.boost||0);return`<div class="bp-item" data-cid="${A.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${G};overflow:hidden;cursor:pointer">
      ...
      </div>`,(Ge=te.querySelector("#psub-close"))==null||Ge.addEventListener("click",()=>te.remove()),(Te=te.querySelector("#pout-up"))==null||Te.addEventListener("click",()=>{N>0&&(N--,ee())}),(Ye=te.querySelector("#pout-down"))==null||Ye.addEventListener("click",()=>{N<G.length-1&&(N++,ee())}),(Oe=te.querySelector("#pin-up"))==null||Oe.addEventListener("click",()=>{Z>0&&(Z--,ee())}),(ot=te.querySelector("#pin-down"))==null||ot.addEventListener("click",()=>{Z<P.length-1&&(Z++,ee())});const Ie=(De,We,He,ut)=>{const nt=te.querySelector("#"+De);if(!nt)return;let qt=0;nt.addEventListener("touchstart",ft=>{qt=ft.touches[0].clientY},{passive:!0}),nt.addEventListener("touchend",ft=>{const Lt=ft.changedTouches[0].clientY-qt;if(Math.abs(Lt)<30)return;const rt=We();Lt<0&&rt<ut-1?(He(rt+1),ee()):Lt>0&&rt>0&&(He(rt-1),ee())},{passive:!0})};Ie("pin-panel",()=>Z,De=>Z=De,P.length),Ie("pout-panel",()=>N,De=>N=De,G.length),(Vt=te.querySelector("#psub-confirm"))==null||Vt.addEventListener("click",async De=>{if(De.preventDefault(),De.stopPropagation(),ie)return;ie=!0;const We=G[N],He=P[Z];if(!We||!He)return;const ut=We._line,nt=(z[ut]||[]).findIndex(Lt=>Lt.cardId===We.cardId);if(nt===-1){g("Erreur : joueur introuvable","error"),te.remove();return}const qt={...He,_line:ut,position:We.position,used:!1,boost:0};z[ut].splice(nt,1,qt);const ft=[...D,He.cardId];te.remove(),W(We,He,async()=>{await S({[p+"Team"]:z,[u+"Team"]:m[u+"Team"],["usedSubIds_"+p]:ft})})})}document.body.appendChild(te),ee()}function W(k,L,z){const D={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},A=document.createElement("div");A.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const G=(V,Z,ie)=>`<div style="text-align:center">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:6px">Appuyer pour continuer</div>`,document.body.appendChild(A);let P=!1;const N=()=>{P||(P=!0,A.remove(),setTimeout(()=>z(),50))};A.addEventListener("click",N),setTimeout(N,2200)}function C(){var L;const k=document.createElement("div");k.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:20px;overflow-y:auto",k.innerHTML=`
      ...
      </div>`,document.body.appendChild(L),(z=L.querySelector("#pvp-hist-close"))==null||z.addEventListener("click",()=>L.remove())}async function ce(){if(m.phase!==p+"-attack")return;const k=p==="p1"?"p2":"p1",L=(m.round||0)+1,z=[...m.log||[]];z.push({type:"info",text:`⏭️ ${m[p+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const D=Le(m),A=pt(k),G=D||!A?"finished":k+"-attack";await S({["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},pendingAttack:null,phase:G,attacker:k,round:L,log:z}),G==="finished"&&await qe(m)}function ae(k){const L=(k.GK||[]).some(D=>!D.used),z=["DEF","MIL","ATT"].some(D=>(k[D]||[]).some(A=>!A.used));return L&&!z}function he(k){return(k.p1Score||0)!==(k.p2Score||0)?null:ae(k.p1Team)&&!Ee(k.p2Team)?{side:"p1",gk:(k.p1Team.GK||[]).find(L=>!L.used)}:ae(k.p2Team)&&!Ee(k.p1Team)?{side:"p2",gk:(k.p2Team.GK||[]).find(L=>!L.used)}:null}async function ge(){const k=document.createElement("div");k.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:950;display:flex;align-items:center;justify-content:center;overflow:hidden;text-align:center;padding:24px",document.body.appendChild(k);const L=(z,D)=>new Promise(A=>{k.innerHTML=`<div style="font-size:32px;font-weight:900;color:${D};letter-spacing:2px;animation:lcFade 1.4s ease both">${z}</div>
      <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(A,1400)});await L("⚽ DERNIER CORNER","#FFD700"),await L("🧤 LE GARDIEN MONTE !","#4fc3f7"),k.remove()}async function ue(k,L){const{side:z,gk:D}=k;if(!D)return;const A=z+"Score",G=(L[A]||0)+1;D.used=!0;const P=[...L.log||[]];P.push({type:"duel",isGoal:!0,homeScored:z===p,homePlayers:z==="p1"?[Pt(D)]:[],aiPlayers:z==="p2"?[Pt(D)]:[],text:`⚽ DERNIER CORNER — Le gardien de ${L[z+"Name"]} marque !`});const N=(L.round||0)+1,V=z==="p1"?"p2":"p1",Z={...L,[A]:G},ie=Le(Z);F.add(N),await ge();const te=z===p?G:L[p+"Score"]||0,ee=z===p?L[u+"Score"]||0:G;Ze([Pt(D)],te,ee,z===p,async()=>{await S({[A]:G,log:P,round:N,pendingAttack:null,phase:ie?"finished":V+"-attack",attacker:V,["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}}}),ie&&await qe({...m,[A]:G})})}async function _e(){const k=m[p+"Team"],L=!["GK","DEF","MIL","ATT"].some(G=>(m[u+"Team"][G]||[]).some(P=>!P.used)),z=(m["selected_"+p]||[]).map(G=>{const P=(k[G._role]||[]).find(se=>se.cardId===G.cardId)||G,N=["GK","DEF"].includes(G._role),V=k[G._role]||[],Z=V.findIndex(se=>se.cardId===G.cardId),ie=st(V.length),te=Z>=0?ie[Z]:P._col??1,ee=m.stadiumDef||m.homeStadiumDef||null,ne=P.stadiumBonus||ee&&(ee.club_id&&String(P.club_id)===String(ee.club_id)||ee.country_code&&P.country_code===ee.country_code)||!1;return{...P,_line:G._role,_col:te,stadiumBonus:ne,...N?{note_a:Math.max(1,Number(P.note_a)||0)}:{}}});if(!z.length)return;const D=ri(z,m.modifiers[p]||{},m[p+"Formation"]);le(p,z.map(G=>G.cardId)),z.forEach(G=>{const P=(k[G._role]||[]).find(N=>N.cardId===G.cardId);P&&(P.used=!0)}),m["selected_"+p]=[],U();const A=[...m.log||[]];if(L){if(z.length===1&&(z[0]._line==="GK"||z[0].job==="GK")&&(m.p1Score||0)===(m.p2Score||0)){await ue({side:p,gk:z[0]},{...m,[p+"Team"]:k,log:A});return}const P=(m[p+"Score"]||0)+1,N=z.map(ee=>Pt(ee));A.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:N,homeTotal:D.total,aiTotal:0});const V=(m.round||0)+1,Z=p==="p1"?"p2":"p1",ie={...m,[p+"Team"]:k,[p+"Score"]:P},te=Le(ie);F.add(V),Ze(N,P,m[u+"Score"]||0,!0,async()=>{await S({[p+"Team"]:k,[p+"Score"]:P,["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},pendingAttack:null,phase:te?"finished":Z+"-attack",attacker:Z,round:V,log:A}),te&&await qe({...m,[p+"Score"]:P})});return}A.push({type:"pending",text:`⚔️ ${m[p+"Name"]} attaque (${D.total})`}),await S({[p+"Team"]:k,[u+"Team"]:m[u+"Team"],pendingAttack:{...D,players:z,side:p},["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},phase:u+"-defense",log:A})}async function Ae(){const k=m[p+"Team"],L=(m["selected_"+p]||[]).map(pe=>{const ye=(k[pe._role]||[]).find(Te=>Te.cardId===pe.cardId)||pe,we=k[pe._role]||[],oe=we.findIndex(Te=>Te.cardId===pe.cardId),Ie=st(we.length),Ge=oe>=0?Ie[oe]:ye._col??1;return{...ye,_line:pe._role,_col:Ge}}),z=ai(L,m.modifiers[p]||{},m[p+"Formation"]);le(p,L.map(pe=>pe.cardId)),L.forEach(pe=>{const ye=(k[pe._role]||[]).find(we=>we.cardId===pe.cardId);ye&&(ye.used=!0)}),m["selected_"+p]=[],U();const D=Zi(m.pendingAttack.total,z.total,m.modifiers[p]||{}),A=m.pendingAttack.side,G=D==="attack"||(D==null?void 0:D.goal),P=A==="p1"?"p2":"p1",N=(m.round||0)+1,V=(m.pendingAttack.players||[]).map(pe=>Pt(pe)),Z=[...m.log||[]];Z.push({type:"duel",isGoal:G,homeScored:G&&A===p,text:G?`⚽ BUT de ${m[A+"Name"]} ! (${m.pendingAttack.total} vs ${z.total})`:`✋ Attaque stoppée (${m.pendingAttack.total} vs ${z.total})`,homePlayers:V,aiPlayers:L.map(pe=>Pt(pe)),homeTotal:m.pendingAttack.total,aiTotal:z.total});const ie=G?(m[A+"Score"]||0)+1:m[A+"Score"]||0,te={...m,[p+"Team"]:k,[A+"Score"]:ie},ee=Le(te),ne=ee?"finished":P+"-attack",se=!ee&&!G?he(te):null,be=async()=>{if(se){await ue(se,te);return}await S({[p+"Team"]:k,[u+"Team"]:m[u+"Team"],[A+"Score"]:ie,["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},pendingAttack:null,phase:ne,attacker:P,round:N,log:Z}),(ne==="finished"||ee)&&await qe({...m,[A+"Score"]:ie})};if(G){const pe=A===p,ye=pe?ie:m[p+"Score"]||0,we=pe?m[u+"Score"]||0:ie;F.add(N),Ze(V,ye,we,pe,be)}else await be()}function je(k){return["MIL","ATT"].some(L=>(k[L]||[]).some(z=>!z.used))}function Ee(k){return["GK","DEF","MIL","ATT"].some(L=>(k[L]||[]).some(z=>!z.used))}function Et(k){return Ee(k)&&!je(k)}function pt(k){const L=m[k+"Team"],z=m[(k==="p1"?"p2":"p1")+"Team"];return!!(je(L)||Et(L)&&!Ee(z)||Et(L)&&!je(z)&&Ee(z))}function Le(k){return!Ee(k.p1Team)&&!Ee(k.p2Team)}function Be(k){const L=k.p1Score||0,z=k.p2Score||0;return L===z?null:L>z?h.home_id:h.away_id}async function qe(k){Rt(),Ue();try{const L=Be(k),z=L?h.home_id===L?h.away_id:h.home_id:null,D=k.p1Score||0,A=k.p2Score||0,{data:G,error:P}=await v.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:L,p_home_score:D,p_away_score:A,p_forfeit:!1});if(P&&console.error("[PvP] finish_pvp_match:",P),l)try{await v.rpc("check_and_finish_mini_league",{p_league_id:l})}catch(N){console.error("[PvP] check_and_finish_mini_league:",N)}L&&z&&Gr(L,z).catch(()=>{});try{const{data:N}=await v.rpc("apply_match_rewards",{p_match_id:i});N!=null&&N.success&&!(N!=null&&N.skipped)&&typeof e.refreshProfile=="function"&&await e.refreshProfile()}catch(N){console.error("[PvP] apply_match_rewards:",N)}if(typeof c=="function")try{await c({homeScore:D,awayScore:A,winnerId:L,homeId:h.home_id,awayId:h.away_id,matchId:i})}catch(N){console.error("[PvP] onMatchEnd:",N)}}catch(L){console.error("[PvP] finishMatch:",L)}}function Ne(){var A;const k=m[p+"Score"],L=m[u+"Score"],z=k>L,D=k===L;lt(t),t.innerHTML=`
    ...
    </div>`,(A=document.getElementById("pvp-home"))==null||A.addEventListener("click",()=>{try{v.removeChannel(X)}catch{}Ke(t),l?x("mini-league",{openLeagueId:l}):x("home")})}U()}async function Ui(t,e,i=!1){var s,d;const{state:o,navigate:r,toast:n}=e,a=i&&((d=(s=e==null?void 0:e.state)==null?void 0:s.params)==null?void 0:d.rankedData)||null;await Si(t,e,i?"ranked":"random",async({deckId:c,formation:l,starters:f,subsRaw:b,gcCardsEnriched:x,gcDefs:g,stadiumDef:p})=>{lt(t);const u=async y=>{await Hr(t,e,c,l,f,b,y||[],g,p,i,a)};if(!(x!=null&&x.length)){await u([]);return}Ii(t,x,u)})}async function Or(t,e,i){return Rr(t,e,i)}async function Hr(t,e,i,o,r,n,a=[],s=[],d=null,c=!1,l=null){var K,X;const{state:f,navigate:b,toast:x}=e,g=(l==null?void 0:l.mmr)??((K=f.profile)==null?void 0:K.mmr)??1e3,p=c?ki(g):null,u=c?"linear-gradient(135deg, #1a0a2e 0%, #2d1060 50%, #1a0a2e 100%)":"linear-gradient(135deg, #0a1a2e 0%, #0d3d1e 100%)",y=c?(p==null?void 0:p.color)||"#D4A017":"#FFD700";let w=!1,h=null,E=null;t.innerHTML=`
    ...
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:${Zt};border:1px solid ${ei};border-radius:12px;padding:16px">
        ...
        ${f.map(S=>`
          <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid ${St}">
            <div style="width:36px;height:36px;border-radius:50%;background:${S.club_color2||ln};display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:900;color:${S.club_color1||"#fff"}">${(S.pseudo||"?").slice(0,2).toUpperCase()}</div>
            <div style="flex:1"><div style="font-size:13px;font-weight:700;color:${Je}">${S.club_name||S.pseudo}</div><div style="font-size:11px;color:${et}">@${S.pseudo}</div></div>
      ...
      <div style="background:${Zt};border:1px solid ${ei};border-radius:12px;padding:16px">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>${b.map((S,_)=>{const O=g>0&&a.status==="finished"?_===0?Math.floor(g*.7):_===1?Math.floor(g*.2):_===2?Math.floor(g*.1):0:0;return`<tr style="border-bottom:1px solid ${St};${S.userId===n?"background:rgba(74,222,128,0.08);":""}">
              <td style="padding:7px 3px 7px 0;font-weight:700;color:${_===0?dt:_<3?"#4ade80":Tt}">${["🥇","🥈","🥉"][_]||_+1}</td>
              <td style="padding:7px 3px"><div style="font-weight:700;color:${Je}">${S.clubName}</div><div style="font-size:10px;color:${et}">@${S.pseudo}</div></td>
              <td style="text-align:center;color:${Tt}">${S.played}</td><td style="text-align:center;color:${Tt}">${S.won}-${S.drawn}-${S.lost}</td>
              <td style="text-align:center;color:${S.goalDiff>=0?"#4ade80":"#ff6b6b"}">${S.goalDiff>=0?"+":""}${S.goalDiff}</td>
              <td style="text-align:center;font-weight:900;font-size:14px;color:${Je}">${S.points}</td>
      ...
      <div style="background:${Zt};border:1px solid ${ei};border-radius:12px;padding:16px">
        ...
        ${Array.from({length:Math.max(0,a.status==="active"?a.current_day-1:a.current_day)},(S,_)=>_+1).reverse().map(S=>{const _=(d||[]).filter(O=>O.matchday===S);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:${et};margin-bottom:6px">Journée ${S}</div>${_.map(O=>Io(O,f,n,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    </div>`}function b(p){var T,F,H,K;const u=(T=p.card)==null?void 0:T.player;if(!u)return"";const y=((F=p.card)==null?void 0:F.evolution_bonus)||0,w=mi(u,u.job,y),h=u.job2?mi(u,u.job2,y):0,E=p.status==="sold",m=u.country_code?`https://flagsapi.com/${u.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,I=So[u.job]||"#bbb",$=u.job2?So[u.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${E?"opacity:0.7":""}">
    ...
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center">
        ...
        <button id="buy-ok" style="flex:1;padding:12px;border-radius:10px;border:none;background:var(--green);color:#fff;font-size:14px;font-weight:900;cursor:pointer">Confirmer</button>
          ...
          <div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${S?"border:2px solid var(--yellow)":""}">
            ...
            <div style="flex:1;min-width:0">
              ...
              <div style="font-size:11px;color:var(--tile-fg-dim)">${T.club_name} · ${H.label}</div>
            </div>
            <div style="text-align:right;flex-shrink:0">
          ...
          </div>`}).join(""):'<div style="text-align:center;color:var(--tile-fg-dim);padding:40px">Aucun joueur classé.</div>'}else if(d==="mini-league"){const $=a||[];h.innerHTML=$.length>0?$.map((T,F)=>`
        <div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${T.user_id===i.profile.id?"border:2px solid var(--yellow)":""}">
      ...
      `).join(""):`<div style="text-align:center;color:var(--tile-fg-dim);padding:40px">Aucun résultat de Mini League pour l'instant.</div>`}else{const $=s||[];h.innerHTML=$.length>0?$.map((T,F)=>`
        <div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${T.user_id===i.profile.id?"border:2px solid var(--yellow)":""}">
          <div style="width:32px;height:32px;border-radius:50%;background:${c(F)};color:${l(F)};display:flex;align-items:center;justify-content:center;font-weight:900;flex-shrink:0;font-size:${F<3?"16":"13"}px">${F<3?["🥇","🥈","🥉"][F]:F+1}</div>
          <div style="flex:1;min-width:0">
            <div style="font-weight:700;color:var(--tile-fg-on-page);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${T.pseudo}</div>
  ...
  <div class="page">
    ...
    <nav class="top-nav">
      <div class="logo" id="nav-logo" title="Manager Wars v2026.07.27-1603" style="cursor:pointer">
