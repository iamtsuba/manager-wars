import{s as k,F as et,b as Me,c as Ft,l as ge,d as ai}from"./formation-links-CDBKB_z4.js";function _t(e,{navigate:t,toast:i}){e.innerHTML=`
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),n=(d||[]).filter($=>$.card_type==="player"&&$.player),r=(d||[]).filter($=>$.card_type==="game_changer"),u=(d||[]).filter($=>$.card_type==="formation"),{data:m}=await k.from("gc_definitions").select("name,gc_type,color,effect,image_url"),f={};(m||[]).forEach($=>{f[$.name]=$});const h=Object.keys(et),p=Object.keys(de),x={};n.forEach($=>{const b=$.player.id;x[b]=(x[b]||0)+1}),new Set(Object.keys(x).map($=>String($)));const A=new Set(u.map($=>$.formation)),v=new Set(r.map($=>$.gc_type));let g="player",w="Tous",F="",M=!1;function y(){return[...n].sort(($,b)=>{const j=je.indexOf($.player.job),E=je.indexOf(b.player.job);return j!==E?j-E:($.player.surname_encoded||"").localeCompare(b.player.surname_encoded||"")})}function I(){return[...l||[]].sort(($,b)=>{const j=je.indexOf($.job),E=je.indexOf(b.job);return j!==E?j-E:($.surname_encoded||"").localeCompare(b.surname_encoded||"")})}function G(){return y().filter($=>{const b=$.player,j=w==="Tous"||b.job===w,E=!F||`${b.firstname} ${b.surname_encoded}`.toLowerCase().includes(F);return j&&E})}function R(){return I().filter($=>{const b=w==="Tous"||$.job===w,j=!F||`${$.firstname} ${$.surname_encoded}`.toLowerCase().includes(F);return b&&j})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{M=!M,P(),H()})))}function H(){const $=document.getElementById("col-grid");$&&(g==="player"?S($):g==="formation"?N($):C($))}function W($,b,j,E,_){_=_||"#7a28b8";const D=document.getElementById("col-grid"),T=document.getElementById("col-big");if(!D||!T)return;var q=0;function B(){T.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+b($[q])+"</div>";var K=T.querySelector("[data-card-id],[data-form-id],[data-gc-id]");K&&K.addEventListener("click",function(){E($[q])}),requestAnimationFrame(function(){var U=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!U||!T)){var J=T.clientHeight-8,te=T.clientWidth-24,re=U.offsetHeight,ae=U.offsetWidth;if(re>0&&ae>0&&J>40){var se=Math.min(J/re,te/ae,1);U.style.transform="scale("+se.toFixed(3)+")",U.style.transformOrigin="top center"}}}),D.innerHTML=$.map(function(U,J){return'<div class="col-mini-item" data-idx="'+J+'" style="flex-shrink:0;cursor:pointer;border-radius:8px;border:1.5px solid '+(J===q?_:"transparent")+';transition:border-color .15s;overflow:hidden">'+j(U,J===q)+"</div>"}).join(""),D.querySelectorAll(".col-mini-item").forEach(function(U){U.addEventListener("click",function(){q=Number(U.dataset.idx),B(),U.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}B()}function V($){var b=.54,j=Math.round(140*b),E=Math.round(310*b),_;if(!$||$._fake){var D=$?$.player:null;if(!D)return"";_=At(D)}else _=Ue($,"");return'<div style="width:'+j+"px;height:"+E+'px;overflow:hidden;position:relative;flex-shrink:0"><div style="transform:scale('+b+');transform-origin:top left;position:absolute;top:0;left:0;pointer-events:none">'+_+"</div></div>"}function ee($,b,j){b=b||100,j=j||140;var E=Me[$]||{},_={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},D=Math.max(3,Math.round(b*.06)),T=Object.entries(E).map(function(B){var K=B[0],U=B[1],J=K.replace(/\d+$/,""),te=_[J]||"#888",re=Math.round(U.x*b),ae=Math.round(U.y*j);return'<circle cx="'+re+'" cy="'+ae+'" r="'+D+'" fill="'+te+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),q=Math.max(1,Math.round(b/50));return'<svg viewBox="0 0 '+b+" "+j+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+b+'" height="'+j+'" fill="#1A6B3C"/><rect x="'+Math.round(b*.2)+'" y="'+Math.round(j*.02)+'" width="'+Math.round(b*.6)+'" height="'+Math.round(j*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+q+'"/><line x1="0" y1="'+Math.round(j*.5)+'" x2="'+b+'" y2="'+Math.round(j*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+q+'"/><ellipse cx="'+Math.round(b*.5)+'" cy="'+Math.round(j*.5)+'" rx="'+Math.round(b*.18)+'" ry="'+Math.round(j*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+q+'"/><rect x="'+Math.round(b*.2)+'" y="'+Math.round(j*.82)+'" width="'+Math.round(b*.6)+'" height="'+Math.round(j*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+q+'"/>'+T+"</svg>"}function z($,b,j){var E=j>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+j+"</div>":"",_=b?'data-form-id="'+b.id+'"':"",D=$.length>7?14:$.length>5?16:19,T=!!b;return"<div "+_+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(T?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(T?"":"filter:grayscale(1);opacity:0.5")+'">'+E+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(T?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+D+"px;font-weight:900;color:"+(T?"#1A6B3C":"#aaa")+';line-height:1">'+$+'</div></div><div style="flex:1;overflow:hidden;background:'+(T?"#1A6B3C":"#ccc")+'">'+ee($,140,220)+"</div></div>"}function L($,b){var j=.54,E=Math.round(140*j),_=Math.round(305*j),D=Math.round(_*.22),T=_-D,q=$.length>7?5:7,B=ee($,E,T),K=b?"1.5px solid #2a7a40":"1px solid #ddd",U=b?"":"filter:grayscale(1);opacity:0.45;",J=b?"#1A6B3C":"#bbb",te="#fff";return'<div style="width:'+E+"px;height:"+_+"px;border-radius:6px;border:"+K+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+U+'"><div style="height:'+D+"px;background:"+J+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+q+"px;font-weight:900;color:"+te+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(E-4)+'px">'+$+'</span></div><div style="height:'+T+'px;overflow:hidden;flex-shrink:0">'+B+"</div></div>"}function S($){if(M){const b=R();if(!b.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const j=b.map(E=>n.find(_=>_.player.id===E.id)||{_fake:!0,player:E,id:"fake-"+E.id});W(j,E=>E._fake?At(E.player):Ue(E,""),E=>E._fake?V({player:E.player,id:"x",_fake:!0}):V(E),E=>{E._fake||It(E,n,x,t)},"#1A6B3C")}else{const b=G();if(!b.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const j={},E=[];b.forEach(_=>{j[_.player.id]||(j[_.player.id]=!0,E.push(_))}),W(E,_=>{const D=x[_.player.id]||1,T=D>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${D}</div>`:"",B=n.filter(K=>K.player.id===_.player.id&&K.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Ue(_,T+B)},_=>V(_),_=>It(_,n,x,t),"#1A6B3C")}}function N($){const b=M?h:[...A];if(!b.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const j=b.map(E=>({formation:E,card:u.find(_=>_.formation===E)||null,owned:A.has(E)}));W(j,({formation:E,card:_,owned:D})=>z(E,D?_:null,D?u.filter(T=>T.formation===E).length:0),({formation:E,owned:_})=>L(E,_),({card:E,owned:_})=>{_&&E&&xi(E,u,t,o)},"#1A6B3C")}function C($){const b=Object.keys(f),j=M?b.length?b:p:[...v];if(!j.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const E=j.map(_=>({type:_,gc:de[_]||{icon:"⚡",desc:""},def:f[_]||null,owned:v.has(_),card:r.find(D=>D.gc_type===_)||null}));W(E,({type:_,gc:D,def:T,owned:q,card:B})=>{const K=q?r.filter(ze=>ze.gc_type===_).length:0,U=K>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${K}</div>`:"",J=(T==null?void 0:T.gc_type)==="ultra_game_changer",te={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},re={purple:"#b06ce0",light_blue:"#00d4ef"},ae=te[T==null?void 0:T.color]||te.purple,se=re[T==null?void 0:T.color]||re.purple,me=(T==null?void 0:T.effect)||D.desc||"",xe=T!=null&&T.image_url?`/manager-wars/icons/${T.image_url}`:null;return q&&B?`<div data-gc-id="${B.id}" data-gc-type="${_}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${se};background:${ae};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${se}66;cursor:pointer">
          ${U}
        ...
        </div>`},({type:_,gc:D,def:T,owned:q})=>q?(()=>{const B=Math.round(75.60000000000001),K=Math.round(310*.54),U=Math.round(K*.65),J=Math.round(K*.18),te={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},re={purple:"#9b59b6",light_blue:"#00bcd4"},ae=te[T==null?void 0:T.color]||te.purple,se=re[T==null?void 0:T.color]||re.purple,me=T!=null&&T.image_url?`/manager-wars/icons/${T.image_url}`:null;return`<div style="width:${B}px;height:${K}px;border-radius:8px;background:${ae};border:1px solid ${se};display:flex;flex-direction:column;overflow:hidden"><div style="height:${J}px;background:rgba(255,255,255,0.15);display:flex;align-items:center;justify-content:center"><span style="font-size:7px;font-weight:900;color:#fff;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${B-6}px">${_}</span></div><div style="height:${U}px;display:flex;align-items:center;justify-content:center">${me?`<img src="${me}" style="max-width:${B-8}px;max-height:${U-4}px;object-fit:contain">`:`<span style="font-size:24px">${D.icon}</span>`}</div><div style="flex:1;display:flex;align-items:center;justify-content:center"><span style="font-size:7px;color:rgba(255,255,255,0.7);text-align:center;padding:0 2px">${((T==null?void 0:T.effect)||D.desc||"").slice(0,20)}</span></div></div>`})():(()=>{const B=Math.round(75.60000000000001),K=Math.round(310*.54);return`<div style="width:${B}px;height:${K}px;border-radius:8px;background:#eee;border:1px solid #ddd;display:flex;flex-direction:column;align-items:center;justify-content:center;filter:grayscale(1);opacity:0.45"><span style="font-size:22px">${D.icon}</span><span style="font-size:7px;color:#aaa;margin-top:4px;text-align:center;padding:0 4px">${_}</span></div>`})(),({type:_,owned:D,def:T})=>{D&&mi(_,T,o)},"#7a28b8")}e.querySelectorAll(".col-tab-btn").forEach($=>{$.addEventListener("click",()=>{g=$.dataset.tab,w="Tous",F="",M=!1,e.querySelectorAll(".col-tab-btn").forEach(b=>{const j=b.dataset.tab===g;b.style.borderBottomColor=j?"var(--green)":"transparent",b.style.color=j?"var(--green)":"var(--gray-600)"}),P(),H()})}),P(),H()}function mi(e,t,i){const s=de[e]||{icon:"⚡",desc:"Effet spécial."},c=(t==null?void 0:t.gc_type)==="ultra_game_changer",o={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},a={purple:"#b06ce0",light_blue:"#00d4ef"},d=o[t==null?void 0:t.color]||o.purple,l=a[t==null?void 0:t.color]||a.purple,n=(t==null?void 0:t.name)||e,r=(t==null?void 0:t.effect)||s.desc,u=t!=null&&t.image_url?`/manager-wars/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const $e=1e3;function xi(e,t,i,s){var p,x,A;const{state:c,toast:o,closeModal:a,navigate:d,refreshProfile:l}=i,n=e.formation,r={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function u(){const v=Me[n]||{},g=et[n]||[],w=290,F=360,M=20;function y(G){const R=v[G];return R?{x:R.x*w,y:R.y*F}:null}let I=`<svg width="${w}" height="${F}" viewBox="0 0 ${w} ${F}" xmlns="http://www.w3.org/2000/svg">`;for(const[G,R]of g){const P=y(G),H=y(R);!P||!H||(I+=`<line x1="${P.x}" y1="${P.y}" x2="${H.x}" y2="${H.y}"
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}const Oi="simulation",Kt="VOTRE_ZONE_ID";function Hi(){switch(Oi){case"propellerads":return Ki();case"adsense":return Ui();default:return rt()}}function rt(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:#111;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:14px";let i=5;t.innerHTML=`
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order");let l=0;function n(){var v,g,w,F,M;const r=o[l],u=(d||[]).filter(y=>y.deck_id===r.id),m=u.filter(y=>{var I;return y.is_starter&&((I=y.card)==null?void 0:I.player)}).map(y=>We(y.card,y.position)),f=u.find(y=>{var I;return((I=y.card)==null?void 0:I.card_type)==="formation"}),h=r.formation||((v=f==null?void 0:f.card)==null?void 0:v.formation)||"4-4-2",p=m.length>=11?Ne(m,h):null,x=m.length>=11;Vt(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div id="deck-swipe-zone" style="flex:1;min-height:0;overflow:hidden;position:relative;touch-action:pan-y;display:flex;align-items:center;justify-content:center;padding:4px">
        ${p?`<div class="deck-preview-wrap" style="aspect-ratio:1/1;max-width:100%;max-height:100%;width:auto;height:100%;overflow:hidden">${Te(p,h,null,[],285,285)}</div>`:`<div style="display:flex;align-items:center;justify-content:center;height:100%;opacity:.4;flex-direction:column;gap:8px">
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;height:${u}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ${Je(e==null?void 0:e.country_code)?`<img src="${Je(e.country_code)}" style="width:${u+2}px;height:${u-3}px;object-fit:cover;border-radius:1px">`:`<span style="font-size:${u-4}px">${n}</span>`}
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div id="midfield-result" style="opacity:0;text-align:center;transition:opacity 0.5s;color:#fff;max-width:320px"></div>
      ...
      </div>`,(v=document.getElementById("start-match-btn"))==null||v.addEventListener("click",()=>{t.phase=t.attacker==="home"?"attack":"ai-attack",oe(e,t,i),t.attacker==="ai"&&setTimeout(()=>Xe(e,t,i),800)})},100)},2800)}function Qi(e){if(e.type==="duel"){const t=e.homeTotal>=e.aiTotal;return`
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:100px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const I=t.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
            ...
            ${Ee((I.players||[]).map(G=>({...G,used:!1})),"#ff6b6b",I.total)}
            ...
            ${Ee((I.players||[]).map(G=>({...G,used:!1})),"#00ff88",I.total)}
    ...
    ${(()=>{const y=window.innerWidth>=700,I=(L,S,N)=>{var U,J;const C=(t.gcDefs||[]).find(te=>te.name===L.gc_type),$={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[C==null?void 0:C.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",b={purple:"#b06ce0",light_blue:"#00d4ef"}[C==null?void 0:C.color]||"#b06ce0",j=(C==null?void 0:C.name)||L.gc_type,E=(C==null?void 0:C.effect)||((U=de[L.gc_type])==null?void 0:U.desc)||"",_=C!=null&&C.image_url?`/manager-wars/icons/${C.image_url}`:null,D=((J=de[L.gc_type])==null?void 0:J.icon)||"⚡",T=Math.round(N*.22),q=Math.round(N*.22),B=N-T-q,K=j.length>12?7:9;return`<div class="gc-mini" data-gc-id="${L.id}" data-gc-type="${L.gc_type}"
          style="box-sizing:border-box;width:${S}px;height:${N}px;border-radius:10px;border:2px solid ${b};background:${$};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
          <div style="height:${T}px;background:rgba(255,255,255,0.14);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 3px">
            <span style="font-size:${K}px;font-weight:900;color:#fff;text-align:center;line-height:1.1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${S-6}px">${j}</span>
          ...
          <div style="height:${B}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
            ${_?`<img src="${_}" style="max-width:${S-10}px;max-height:${B-6}px;object-fit:contain">`:`<span style="font-size:${Math.round(B*.55)}px">${D}</span>`}
          </div>
        ...
        </div>`},G=(L,S)=>{var N;return`<div id="boost-card"
          style="box-sizing:border-box;width:${L}px;height:${S}px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:2px solid #87CEEB;border-radius:10px;cursor:pointer;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:${Math.round(S*.04)}px;text-align:center;flex-shrink:0">
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(y=>{var I,G,R;if(y.type==="duel"){const P=y.isGoal,H=y.homeScored?"#FFD700":P?"#ff6b6b":"rgba(255,255,255,0.3)",W=y.homeScored?"⚽ BUT !":P?"⚽ BUT IA !":(I=y.homePlayers)!=null&&I.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${P?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${H};margin-bottom:4px">
                ...
                ${(G=y.homePlayers)!=null&&G.length?`<div style="margin-bottom:3px">${Ee(y.homePlayers,"rgba(255,255,255,0.7)",y.homeTotal)}</div>`:""}
                ${(R=y.aiPlayers)!=null&&R.length?`<div style="opacity:0.7">${Ee(y.aiPlayers,"#ff6b6b",y.aiTotal)}</div>`:""}
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(o);let d=!1;const l=()=>{d||(d=!0,o.remove(),setTimeout(()=>c(),50))};o.addEventListener("click",l),setTimeout(l,3500)}async function ve(e,t,i){var r,u;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:s}=i,c=t.homeScore>t.aiScore,o=t.homeScore===t.aiScore,a=c?"victoire":o?"nul":"defaite",d=pi(t.mode,a);t.matchId&&await k.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:c?s.profile.id:null,home_credits_reward:d,played_at:new Date().toISOString()}).eq("id",t.matchId);const l={credits:(s.profile.credits||0)+d,matches_played:(s.profile.matches_played||0)+1};c?l.wins=(s.profile.wins||0)+1:o?l.draws=(s.profile.draws||0)+1:l.losses=(s.profile.losses||0)+1,await k.from("users").update(l).eq("id",s.profile.id),await i.refreshProfile();const n=document.createElement("div");n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",n.innerHTML=`
    ...
    </div>`,`<button class="btn btn-primary" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}async function co(e,t){const{state:i}=t;await Xt(e,t,"random",({deckId:c,formation:o,starters:a,subsRaw:d,gcCardsEnriched:l,gcDefs:n})=>{const r=u=>{po(e,t,c,o,a,d,u,n||[])};if(!l.length){r([]);return}Yt(e,l,r)})}async function po(e,t,i,s,c,o,a=[],d=[]){var v;const{state:l,navigate:n,toast:r}=t;let u=!1,m=null;e.style.overflow="hidden",e.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const f=()=>document.getElementById("mm-status"),h=async(g=!0)=>{u=!0,m&&(k.removeChannel(m),m=null),g&&await k.rpc("cancel_matchmaking",{p_user_id:l.profile.id}).catch(()=>{})};(v=document.getElementById("mm-cancel"))==null||v.addEventListener("click",async()=>{try{await h(!0)}catch(g){console.warn("[Matchmaking] cleanup error:",g)}le(e),n("home")});const p=async(g,w,F)=>{u||(u=!0,m&&(k.removeChannel(m),m=null),f()&&(f().textContent="Adversaire trouvé !"),await new Promise(M=>setTimeout(M,600)),e.isConnected!==!1&&uo(e,t,g,F,a,d))},{data:x,error:A}=await k.rpc("try_matchmake",{p_user_id:l.profile.id,p_deck_id:i});if(A||!(x!=null&&x.success)){r("Erreur de matchmaking","error"),le(e),n("home");return}if(x.matched){p(x.match_id,x.opponent_id,!1);return}f()&&(f().textContent="En attente d'un autre joueur..."),m=k.channel("matchmaking-"+l.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${l.profile.id}`},g=>{const w=g.new;w.status==="matched"&&w.match_id&&p(w.match_id,w.matched_with,!0)}).subscribe()}async function uo(e,t,i,s,c=[],o=[]){const{state:a,navigate:d,toast:l}=t,n=s?"p1":"p2",r=s?"p2":"p1",u=(c||[]).map(z=>z.id),m=(c||[]).map(z=>({id:z.id,gc_type:z.gc_type,_gcDef:z._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation du match...</div>';const{data:f}=await k.from("matches").select("*").eq("id",i).single();if(!f){l("Match introuvable","error"),d("home");return}async function h(){const[{data:z,error:L},{data:S,error:N},{data:C},{data:$}]=await Promise.all([k.rpc("get_deck_for_match",{p_deck_id:f.home_deck_id}),k.rpc("get_deck_for_match",{p_deck_id:f.away_deck_id}),k.from("users").select("id,pseudo,club_name").eq("id",f.home_id).single(),k.from("users").select("id,pseudo,club_name").eq("id",f.away_id).single()]);L&&console.error("[PvP] get_deck_for_match p1 error:",L),N&&console.error("[PvP] get_deck_for_match p2 error:",N);function b(B,K){return{cardId:B.card_id,position:K||B.position,id:B.id,firstname:B.firstname,name:B.surname_encoded,country_code:B.country_code,club_id:B.club_id,job:B.job,job2:B.job2,note_g:Number(B.note_g)||0,note_d:Number(B.note_d)||0,note_m:Number(B.note_m)||0,note_a:Number(B.note_a)||0,rarity:B.rarity,skin:B.skin,hair:B.hair,hair_length:B.hair_length,clubName:B.club_encoded_name||null,clubLogo:B.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}}const j=((z==null?void 0:z.starters)||[]).map(B=>b(B,B.position)),E=((S==null?void 0:S.starters)||[]).map(B=>b(B,B.position)),_=((z==null?void 0:z.subs)||[]).map(B=>b(B,B.position)),D=((S==null?void 0:S.subs)||[]).map(B=>b(B,B.position)),T=(z==null?void 0:z.formation)||"4-4-2",q=(S==null?void 0:S.formation)||"4-4-2";return console.log("[PvP] p1Formation:",T,"p1 starters:",j.length,"positions:",j.map(B=>B.position)),console.log("[PvP] p2Formation:",q,"p2 starters:",E.length,"positions:",E.map(B=>B.position)),{p1Team:Ne(j,T),p2Team:Ne(E,q),p1Subs:_,p2Subs:D,p1Formation:T,p2Formation:q,p1Name:(C==null?void 0:C.club_name)||(C==null?void 0:C.pseudo)||"Joueur 1",p2Name:($==null?void 0:$.club_name)||($==null?void 0:$.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:s?u:[],gc_p2:s?[]:u,gcCardsFull_p1:s?m:[],gcCardsFull_p2:s?[]:m,gcReady_p1:!0,gcReady_p2:!0,usedGc_p1:[],usedGc_p2:[],lastActionAt:new Date().toISOString()}}let p=f.game_state&&Object.keys(f.game_state).length?f.game_state:null;if(!p)if(s){p=await h();const{data:z}=await k.from("matches").select("game_state").eq("id",i).single();!(z!=null&&z.game_state)||!Object.keys(z.game_state).length?await k.from("matches").update({game_state:p,turn_user_id:f.home_id}).eq("id",i):p=z.game_state}else{e.innerHTML=`<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation avec l'adversaire...</div>`;for(let z=0;z<30&&!p;z++){await new Promise(S=>setTimeout(S,400));const{data:L}=await k.from("matches").select("game_state").eq("id",i).single();L!=null&&L.game_state&&Object.keys(L.game_state).length&&(p=L.game_state)}if(!p){l("Erreur de synchronisation","error"),d("home");return}p.gc_p2=u,p.gcCardsFull_p2=m,await k.from("matches").update({game_state:p}).eq("id",i)}let x=!1;function A(z){var j;try{k.removeChannel(v)}catch{}const L=a.profile.id,S=z.winner_id===L,N=!!z.forfeit,C=p[n+"Score"]??0,$=p[r+"Score"]??0,b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center",b.innerHTML=`
      <div style="font-size:64px">${S?"🏆":"😞"}</div>
      <div style="font-size:26px;font-weight:900;color:${S?"#FFD700":"#ff6b6b"}">${S?"VICTOIRE !":"DÉFAITE"}</div>
      <div style="font-size:18px;color:#fff">${p[n+"Name"]} ${C} – ${$} ${p[r+"Name"]}</div>
      ${N?`<div style="font-size:13px;color:rgba(255,255,255,0.5)">${S?"L'adversaire a quitté la partie":"Match perdu par forfait"}</div>`:""}
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(b),(j=b.querySelector("#pvp-end-home"))==null||j.addEventListener("click",()=>{b.remove(),le(e),d("home")})}const v=k.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},z=>{const L=z.new;try{if(L.status==="finished"||L.forfeit){if(x)return;x=!0,p._timerInt&&(clearInterval(p._timerInt),p._timerInt=null),L.game_state&&(p=L.game_state),A(L);return}L.game_state&&(p=L.game_state,F())}catch(S){console.error("[PvP] Realtime render crash:",S,"gameState:",p)}}).subscribe();async function g(z){Object.assign(p,z),p.lastActionAt=new Date().toISOString();const{error:L}=await k.from("matches").update({game_state:p}).eq("id",i);L&&(console.error("[PvP] pushState DB error:",L.message,L),l("Erreur de synchronisation : "+L.message,"error"));try{F()}catch(S){console.error("[PvP] renderPvpScreen crash après pushState:",S),l("Erreur d'affichage : "+S.message,"error")}}async function w(){if(x)return;x=!0,p._timerInt&&(clearInterval(p._timerInt),p._timerInt=null);const z=s?f.away_id:f.home_id;try{await k.from("matches").update({status:"finished",forfeit:!0,winner_id:z}).eq("id",i)}catch(L){console.warn("[PvP] forfeit DB error:",L)}try{k.removeChannel(v)}catch{}le(e),d("home")}function F(){var ut,ft,gt,mt,xt;const z=p[n+"Team"];p[r+"Team"];const L=p[n+"Score"],S=p[r+"Score"],N=p[n+"Name"],C=p[r+"Name"];if(p.phase==="gc-select")return M();if(p.phase==="reveal")return y();if(p.phase==="midfield")return I();if(p.phase==="finished")return W();const $=p.phase===n+"-attack",b=p.phase===n+"-defense",j=p.phase===r+"-attack"||p.phase===r+"-defense",E=Array.isArray(p["selected_"+n])?p["selected_"+n]:[],_=E.map(O=>O.cardId);e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column";const D=window.innerWidth>=700,T=p[n+"Subs"]||[],q=p["usedSubIds_"+n]||[],B=T.filter(O=>!q.includes(O.cardId)),K=p["gcCardsFull_"+n]||[],U=p["usedGc_"+n]||[],J=K.filter(O=>!U.includes(O.id)),te=p.boostOwner===n&&!p.boostUsed,re=(O,X,Q)=>{var bt,wt;const Y=O._gcDef,ce={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[Y==null?void 0:Y.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",Ce={purple:"#b06ce0",light_blue:"#00d4ef"}[Y==null?void 0:Y.color]||"#b06ce0",ye=(Y==null?void 0:Y.name)||O.gc_type,oi=(Y==null?void 0:Y.effect)||((bt=de[O.gc_type])==null?void 0:bt.desc)||"",yt=Y!=null&&Y.image_url?`/manager-wars/icons/${Y.image_url}`:null,ni=((wt=de[O.gc_type])==null?void 0:wt.icon)||"⚡",ht=Math.round(Q*.22),vt=Math.round(Q*.22),Ke=Q-ht-vt,ri=ye.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${O.id}" data-gc-type="${O.gc_type}"
        style="width:${X}px;height:${Q}px;border-radius:10px;border:2px solid ${Ce};background:${ce};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
        <div style="height:${ht}px;background:rgba(255,255,255,0.14);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 3px">
          <span style="font-size:${ri}px;font-weight:900;color:#fff;text-align:center;line-height:1.1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${X-6}px">${ye}</span>
          <span style="font-size:6px;color:rgba(255,255,255,0.45)">⚡ GC</span>
        </div>
        <div style="height:${Ke}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
          ${yt?`<img src="${yt}" style="max-width:${X-10}px;max-height:${Ke-6}px;object-fit:contain">`:`<span style="font-size:${Math.round(Ke*.55)}px">${ni}</span>`}
        </div>
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 14px;background:rgba(0,0,0,0.25);flex-shrink:0">
        ...
        <div style="flex:1;text-align:center;color:#fff;font-size:14px;font-weight:700">
          ${N} <span style="color:#FFD700;font-size:18px">${L}</span> – <span style="color:#FFD700;font-size:18px">${S}</span> ${C}
        </div>
      ...
      <div style="text-align:center;padding:4px;background:rgba(0,0,0,0.15);font-size:11px;color:${j?"rgba(255,255,255,0.4)":"#FFD700"};font-weight:700;flex-shrink:0">
        ${j?`⏳ Tour de ${C}`:$?"⚔️ À vous d'attaquer !":b?"🛡️ À vous de défendre !":""}
      </div>
      <!-- Zone actions -->
      ...
      </div>`;const O=e.querySelector(".terrain-wrapper");O&&(O.innerHTML=Te(z,p[n+"Formation"],$?"attack":b?"defense":"idle",_,300,300))}if(e.querySelectorAll(".match-slot-hit").forEach(O=>{O.addEventListener("click",()=>{if(!$&&!b)return;const X=O.dataset.cardId,Q=O.dataset.role,Y=(z[Q]||[]).find(ye=>ye.cardId===X);if(!Y||Y.used)return;Array.isArray(p["selected_"+n])||(p["selected_"+n]=[]);const ce=p["selected_"+n],Ce=ce.findIndex(ye=>ye.cardId===X);Ce>-1?ce.splice(Ce,1):ce.length<3&&ce.push({...Y,_role:Q}),F()})}),(ut=e.querySelector("#pvp-boost-card"))==null||ut.addEventListener("click",()=>{G()}),e.querySelectorAll(".pvp-gc-mini").forEach(O=>{O.addEventListener("click",()=>{H(O.dataset.gcId,O.dataset.gcType)})}),(ft=e.querySelector("#pvp-action"))==null||ft.addEventListener("click",()=>{$?V():b&&ee()}),(gt=e.querySelector("#pvp-quit"))==null||gt.addEventListener("click",()=>{confirm("Quitter le match ? Vous perdrez par forfait.")&&w()}),(mt=e.querySelector("#pvp-view-opp"))==null||mt.addEventListener("click",()=>{P()}),(xt=e.querySelector("#pvp-toggle-history"))==null||xt.addEventListener("click",()=>{R()}),function(){const X=e.querySelector(".terrain-wrapper svg");X&&(X.removeAttribute("width"),X.removeAttribute("height"),X.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",X.setAttribute("viewBox","-26 -26 352 352"),X.setAttribute("preserveAspectRatio","xMidYMid meet"));const Q=e.querySelector(".match-screen");if(Q){const Y=e.clientHeight;Y>50&&(Q.style.height=Y+"px",Q.style.maxHeight=Y+"px",Q.style.overflow="hidden")}}(),p._timerInt&&(clearInterval(p._timerInt),p._timerInt=null),($||b)&&!x){let O=30,X=!1;const Q=()=>document.getElementById("pvp-timer"),Y=()=>{Q()&&(Q().textContent=O+"s",Q().style.color=X?"#ff4444":"#fff")};Y(),p._timerInt=setInterval(()=>{O--,O<0?X?(clearInterval(p._timerInt),p._timerInt=null,w()):(X=!0,O=15,Y()):Y()},1e3)}}function M(){var b;const z=p["gcReady_"+n],L=p["gcReady_"+r];Array.isArray(p["gc_"+n])||(p["gc_"+n]=[]);let S=p["gc_"+n];const N=3;function C(j,E){const _=j._gcDef,D={purple:"linear-gradient(135deg,#3d0a7a,#7a28b8)",light_blue:"linear-gradient(135deg,#006080,#00bcd4)"},T={purple:"#9b59b6",light_blue:"#00bcd4"},q=D[_==null?void 0:_.color]||D.purple,B=T[_==null?void 0:_.color]||T.purple;return`<div class="pvp-gc-card" data-id="${j.id}"
    ...
    <div id="pvp-gc-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      <div style="text-align:center;padding:12px 16px 8px;flex-shrink:0">
        ...
        <div style="font-size:20px;font-weight:900;color:#fff">Choisir ses Game Changers</div>
        <div style="font-size:13px;color:rgba(255,255,255,0.5);margin-top:3px">
          Sélectionne <b style="color:#FFD700">${N}</b> cartes · ${S.length}/${N}
        </div>
      </div>
      ${z?`
      ...
      <div style="padding:10px 16px 14px;flex-shrink:0">
        <button id="pvp-gc-validate" style="width:100%;padding:14px;border-radius:14px;border:none;background:linear-gradient(135deg,#5a0a9a,#9a28e8);color:#fff;font-size:15px;font-weight:900;cursor:pointer">
          ${S.length?`⚡ Valider (${S.length} GC)`:"▶ Continuer sans GC"}
        </button>
      </div>`}
    </div>`,z)return;const $=n==="p1"?f.home_id:f.away_id;k.from("cards").select("id,gc_type").eq("owner_id",$).eq("card_type","game_changer").then(async({data:j})=>{const{data:E}=await k.from("gc_definitions").select("*").eq("is_active",!0),_=new Set,D=(j||[]).filter(q=>_.has(q.gc_type)?!1:(_.add(q.gc_type),!0)).map(q=>({...q,_gcDef:(E==null?void 0:E.find(B=>B.name===q.gc_type))||null})),T=document.getElementById("pvp-gc-grid");if(T){if(!D.length){T.innerHTML='<div style="color:rgba(255,255,255,0.4);font-size:13px;text-align:center;margin-top:30px">Aucune carte Game Changer disponible.</div>';return}T.innerHTML=D.map(q=>C(q,S.includes(q.id))).join(""),T.querySelectorAll(".pvp-gc-card").forEach(q=>{q.addEventListener("click",()=>{const B=q.dataset.id,K=S.indexOf(B);K>-1?S.splice(K,1):S.length<N&&S.push(B),F()})})}}),(b=e.querySelector("#pvp-gc-validate"))==null||b.addEventListener("click",async()=>{const j=n==="p1"?f.home_id:f.away_id,{data:E}=await k.from("cards").select("id,gc_type").eq("owner_id",j).eq("card_type","game_changer").in("id",S),{data:_}=await k.from("gc_definitions").select("*").eq("is_active",!0),D=(E||[]).map(T=>({...T,_gcDef:(_==null?void 0:_.find(q=>q.name===T.gc_type))||null}));await g({["gcReady_"+n]:!0,["gcCardsFull_"+n]:D}),p["gcReady_"+r]&&await g({phase:"reveal"})})}function y(){e.style.overflow="hidden",e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
      ...
      <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
    </div>`,n==="p1"&&setTimeout(async()=>{await g({phase:"midfield"})},5e3)}function I(){const z=p[n+"Team"].MIL||[],L=p[r+"Team"].MIL||[];function S(E){return E.reduce((_,D)=>_+ie(D,"MIL"),0)}function N(E){let _=0;for(let D=0;D<E.length-1;D++){const T=ge(E[D],E[D+1]);T==="#00ff88"?_+=2:T==="#FFD700"&&(_+=1)}return _}const C=S(z)+N(z),$=S(L)+N(L),b=C>=$;function j(E,_,D){return`<div style="text-align:center">
        <div style="font-size:10px;color:rgba(255,255,255,0.5);letter-spacing:2px;margin-bottom:8px;text-transform:uppercase">${_}</div>
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${E.map((T,q)=>{const B=ne(T),K=q<E.length-1?ge(T,E[q+1]):null,U=K&&K!=="#ff3333"&&K!=="#cc2222";return`
        ...
        <div style="margin-top:6px;font-size:11px;color:rgba(255,255,255,0.5)">
          Score: ${S(E)} + ${N(E)} liens = <b style="color:#fff">${S(E)+N(E)}</b>
        </div>
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div id="pvp-midfield-result" style="opacity:0;text-align:center;transition:opacity 0.5s;color:#fff;max-width:320px"></div>
    </div>`,setTimeout(()=>{const E=document.getElementById("pvp-score-me"),_=document.getElementById("pvp-score-opp"),D=document.getElementById("pvp-midfield-result");E&&_&&(b?(E.style.fontSize="80px",E.style.color="#FFD700",_.style.opacity="0.25"):(_.style.fontSize="80px",_.style.color="#ff6b6b",E.style.opacity="0.25"));const T=p.p1Team.MIL||[],q=p.p2Team.MIL||[],B=S(T)+N(T),K=S(q)+N(q),U=B>=K?"p1":"p2";p[U+"Name"];let J=p.boostValue;J==null&&(J=Ut(),p.boostValue=J,p.boostOwner=U,p.boostUsed=!1),D&&(D.style.opacity="1",D.innerHTML=`<div style="font-size:20px;font-weight:900;margin-bottom:10px">
        ...
        <div style="background:rgba(135,206,235,0.15);border:2px solid #87CEEB;border-radius:14px;padding:14px 24px;display:inline-block;margin-top:4px" id="pvp-boost-display">
          ...
          <div style="font-size:10px;color:rgba(135,206,235,0.7)">Applicable sur n'importe quel joueur</div>
        </div>`:""}`),setTimeout(async()=>{if(n!=="p1")return;const te=U;await g({phase:te+"-attack",attacker:te,round:1,boostValue:p.boostValue,boostUsed:!1,boostOwner:te})},1800)},600)}function G(){var N;const z=p[n+"Team"],L=Object.entries(z).flatMap(([C,$])=>($||[]).filter(b=>!b.used).map(b=>({...b,_line:C})));if(!L.length)return;const S=document.createElement("div");S.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",S.innerHTML=`
      ...
      </div>`,document.body.appendChild(S),(N=S.querySelector("#boost-picker-close"))==null||N.addEventListener("click",()=>S.remove()),S.querySelectorAll(".boost-pick-item").forEach(C=>{C.addEventListener("click",async()=>{const $=C.dataset.cid,b=L.find(_=>_.cardId===$);if(!b)return;const j=p[n+"Team"],E=(j[b._line]||[]).find(_=>_.cardId===$);E&&(E.boost=(E.boost||0)+p.boostValue),S.remove(),await g({[n+"Team"]:j,boostUsed:!0})})})}function R(){var S;const z=Array.isArray(p.log)?p.log:[],L=document.createElement("div");L.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column",L.innerHTML=`
      ...
      </div>`,document.body.appendChild(q),(B=q.querySelector("#pvp-gc-back"))==null||B.addEventListener("click",()=>q.remove()),(K=q.querySelector("#pvp-gc-use"))==null||K.addEventListener("click",async()=>{q.remove();const U=p["usedGc_"+n]||[];U.push(z),await g({["usedGc_"+n]:U})})}function W(){var C;const z=p[n+"Score"],L=p[r+"Score"],S=z>L,N=z===L;e.style.overflow="hidden",e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      <div style="font-size:64px">${S?"🏆":N?"🤝":"😤"}</div>
      <div style="font-size:24px;font-weight:900;color:#fff">${S?"Victoire !":N?"Match nul":"Défaite"}</div>
    ...
    </div>`,(C=document.getElementById("pvp-home"))==null||C.addEventListener("click",()=>{k.removeChannel(v),le(e),d("home")})}async function V(){const z=(p["selected_"+n]||[]).map(N=>({...N,_line:N._role}));if(!z.length)return;const L=tt(z,p.modifiers[n]||{});z.forEach(N=>{const C=(p[n+"Team"][N._role]||[]).find($=>$.cardId===N.cardId);C&&(C.used=!0)});const S=Array.isArray(p.log)?p.log:[];S.push({type:"attack",text:`⚔️ ${p[n+"Name"]} attaque (${L.total})`,players:z,total:L.total,side:n}),await g({pendingAttack:{...L,players:z,side:n},["selected_"+n]:[],modifiers:{...p.modifiers,[n]:{}},phase:r+"-defense",log:S})}async function ee(){const z=(p["selected_"+n]||[]).map(D=>({...D,_line:D._role})),L=it(z,p.modifiers[n]||{});z.forEach(D=>{const T=(p[n+"Team"][D._role]||[]).find(q=>q.cardId===D.cardId);T&&(T.used=!0)});const S=ot(p.pendingAttack.total,L.total,p.modifiers[n]||{}),N=p.pendingAttack.side,C=S==="attack",$=p[N+"Score"]+(C?1:0),b=N==="p1"?"p2":"p1",j=(p.round||0)+1,E=j>10,_=Array.isArray(p.log)?p.log:[];_.push({type:"defense",text:`🛡️ ${p[n+"Name"]} défend (${L.total})`,players:z,total:L.total,side:n}),_.push({type:C?"goal":"stop",text:C?`⚽ BUT de ${p[N+"Name"]} ! (${p.pendingAttack.total} vs ${L.total})`:`✋ Attaque stoppée (${p.pendingAttack.total} vs ${L.total})`}),await g({[N+"Score"]:$,["selected_"+n]:[],modifiers:{...p.modifiers,[n]:{}},pendingAttack:null,phase:E?"finished":b+"-attack",attacker:b,round:j,log:_}),E&&await k.from("matches").update({status:"finished"}).eq("id",i)}F()}const fo={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"};async function go(e,t){const{state:i,toast:s}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement du marché...</div>',await dt(e,t)}async function dt(e,t){const{state:i,toast:s}=t,{data:c}=await k.from("market_listings").select(`id, price, status, listed_at, seller_id,
  ...
  `;function l(n){const r=document.getElementById("mkt-content"),u=n==="buy"?a:d;if(u.length===0){r.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      ${m.map(f=>{var g,w,F,M;const h=(g=f.card)==null?void 0:g.player;if(!h)return"";const p=h.job==="GK"?h.note_g:h.job==="DEF"?h.note_d:h.job==="MIL"?h.note_m:h.note_a,x=fo[h.rarity],A=(i.profile.credits||0)>=f.price,v=f.status==="sold";return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${v?"opacity:0.65":""}">
          <div style="width:44px;height:44px;border-radius:8px;background:${yo(h.job)};color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:18px;border:2px solid ${x};flex-shrink:0">${p}</div>
          <div style="flex:1;min-width:0">
