import{s as T,F as oi,h as St,j as Ai,l as Qe,i as Xi,k as Ji,b as Qi}from"./formation-links-CEzN0rcz.js";function mi(e,{navigate:t,toast:o}){e.innerHTML=`
  ...
  `,e.querySelectorAll(".auth-tab").forEach(a=>{a.addEventListener("click",()=>{e.querySelectorAll(".auth-tab").forEach(c=>c.classList.remove("active")),a.classList.add("active"),document.getElementById("tab-login").style.display=a.dataset.tab==="login"?"block":"none",document.getElementById("tab-register").style.display=a.dataset.tab==="register"?"block":"none"})}),document.getElementById("login-btn").addEventListener("click",async()=>{const a=document.getElementById("login-email").value.trim(),c=document.getElementById("login-password").value,s=document.getElementById("login-error");if(s.textContent="",!a||!c){s.textContent="Remplissez tous les champs.";return}const d=document.getElementById("login-btn");d.textContent="Connexion…",d.disabled=!0;const{error:l}=await T.auth.signInWithPassword({email:a,password:c});if(d.textContent="Se connecter",d.disabled=!1,l){s.textContent=l.message.includes("Invalid")?"Email ou mot de passe incorrect.":l.message;return}window.location.reload()}),document.getElementById("login-password").addEventListener("keydown",a=>{a.key==="Enter"&&document.getElementById("login-btn").click()}),document.getElementById("reg-btn").addEventListener("click",async()=>{const a=document.getElementById("reg-email").value.trim(),c=document.getElementById("reg-password").value,s=document.getElementById("reg-confirm").value,d=document.getElementById("reg-error");if(d.textContent="",!a||!c||!s){d.textContent="Remplissez tous les champs.";return}if(c.length<6){d.textContent="Mot de passe trop court (min. 6 caractères).";return}if(c!==s){d.textContent="Les mots de passe ne correspondent pas.";return}const l=document.getElementById("reg-btn");l.textContent="Création…",l.disabled=!0;const{error:u}=await T.auth.signUp({email:a,password:c});if(l.textContent="Créer mon compte",l.disabled=!1,u){d.textContent=u.message;return}o("Compte créé ! Connectez-vous.","success",4e3),document.querySelector('[data-tab="login"]').click(),document.getElementById("login-email").value=a})}function Zi(e,{state:t,navigate:o,toast:a,refreshProfile:c}){let s="#1A6B3C",d="#D4A017";e.innerHTML=`
  ...
  `;function l(){var k;const i=document.getElementById("logo-preview"),r=document.getElementById("logo-initials"),f=((k=document.getElementById("setup-club"))==null?void 0:k.value)||"MW",m=f.trim().split(" ").filter(Boolean),x=m.length>=2?(m[0][0]+m[1][0]).toUpperCase():f.slice(0,2).toUpperCase();i&&(i.style.background=d,i.style.borderColor=s),r&&(r.textContent=x,r.style.color=s)}document.getElementById("color1").addEventListener("input",i=>{s=i.target.value,document.getElementById("swatch1").style.background=s,l()}),document.getElementById("color2").addEventListener("input",i=>{d=i.target.value,document.getElementById("swatch2").style.background=d,l()});function u(i){document.querySelectorAll(".setup-step").forEach(r=>r.classList.remove("active")),document.getElementById(`step-${i}`).classList.add("active"),document.getElementById("step-num").textContent=i,document.getElementById("progress-fill").style.width=`${Math.round(i/3*100)}%`,i===3&&l()}document.getElementById("step1-next").addEventListener("click",async()=>{const i=document.getElementById("setup-pseudo").value.trim(),r=document.getElementById("step1-error");if(r.textContent="",i.length<3){r.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:f}=await T.from("users").select("id").eq("pseudo",i).maybeSingle();if(f){r.textContent="Ce pseudo est déjà pris.";return}u(2)}),document.getElementById("step2-back").addEventListener("click",()=>u(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const i=document.getElementById("setup-club").value.trim(),r=document.getElementById("step2-error");if(r.textContent="",i.length<2){r.textContent="Nom trop court (min. 2 caractères).";return}const{data:f}=await T.from("users").select("id").eq("club_name",i).maybeSingle();if(f){r.textContent="Ce nom de club est déjà pris.";return}u(3)}),document.getElementById("step3-back").addEventListener("click",()=>u(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const i=document.getElementById("setup-pseudo").value.trim(),r=document.getElementById("setup-club").value.trim(),f=document.getElementById("step3-error"),m=document.getElementById("step3-finish");f.textContent="",m.disabled=!0,m.textContent="Création en cours…";try{const{error:x}=await T.from("users").insert({id:t.user.id,pseudo:i,club_name:r,club_color1:s,club_color2:d,credits:1e4});if(x)throw x;await en(t.user.id),await c(),a(`Bienvenue ${i} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(x){f.textContent=x.message,m.disabled=!1,m.textContent="🚀 Créer mon profil !"}})}async function en(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await T.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(o){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",o)}}const Lt="#1A6B3C",zt="#cc2222",tn="#D4A017",xi="#888";async function nn(e,t){const{state:o,toast:a}=t;e.innerHTML=`
    ...
    </div>`,await Ii(o,a,t),document.getElementById("btn-add-friend").addEventListener("click",()=>rn(o,a)),document.getElementById("btn-accept-friend").addEventListener("click",()=>Li(o,a,null,t))}async function Ii(e,t,o={}){const{navigate:a}=o,c=e.user.id,{data:s,error:d}=await T.from("friendships").select("id, requester_id, addressee_id").eq("status","accepted").or(`requester_id.eq.${c},addressee_id.eq.${c}`),{count:l}=await T.from("friendships").select("id",{count:"exact",head:!0}).eq("addressee_id",c).eq("status","pending"),u=document.getElementById("pending-badge");u&&(l>0?(u.style.display="flex",u.textContent=l):u.style.display="none");const i=document.getElementById("friends-list");if(!i)return;if(d){console.error("[Friends] Erreur:",d),i.innerHTML=`<div style="color:${zt};text-align:center;padding:16px">Erreur chargement : ${d.message}</div>`;return}const r=(s||[]).map(x=>x.requester_id===c?x.addressee_id:x.requester_id);let f={};if(r.length){const{data:x}=await T.from("users").select("id, pseudo, club_name, last_seen_at").in("id",r);(x||[]).forEach(k=>{f[k.id]=k})}const m=(s||[]).map(x=>({friendshipId:x.id,friend:f[x.requester_id===c?x.addressee_id:x.requester_id]||{pseudo:"?"}}));if(!m.length){i.innerHTML=`
    ...
    ${ai()}`,document.body.appendChild(o);const a=o.querySelector("#friend-pseudo-input"),c=o.querySelector("#add-friend-error"),s=()=>o.remove();a.focus(),o.querySelector("#add-cancel").addEventListener("click",s),o.addEventListener("click",d=>{d.target===o&&s()}),o.querySelector("#add-ok").addEventListener("click",async()=>{const d=a.value.trim();if(!d){c.textContent="Entre un pseudo";return}c.textContent="";const{data:l}=await T.from("users").select("id, pseudo").ilike("pseudo",d).single();if(!l){c.textContent="Utilisateur introuvable";return}if(l.id===e.user.id){c.textContent="Tu ne peux pas t'ajouter toi-même";return}const{data:u}=await T.from("friendships").select("id, status").or(`and(requester_id.eq.${e.user.id},addressee_id.eq.${l.id}),and(requester_id.eq.${l.id},addressee_id.eq.${e.user.id})`).single();if(u){const r=u.status==="accepted"?"Vous êtes déjà amis !":u.status==="pending"?"Demande déjà envoyée":"Une demande existe déjà";c.textContent=r;return}const{error:i}=await T.from("friendships").insert({requester_id:e.user.id,addressee_id:l.id,status:"pending"});if(i){c.textContent="Erreur : "+i.message;return}s(),t(`✅ Demande envoyée à ${l.pseudo} !`,"success")})}async function Li(e,t,o=null,a={}){const c=e.user.id,{data:s}=await T.from("friendships").select("id, requester_id").eq("addressee_id",c).eq("status","pending").order("created_at",{ascending:!1}),d=(s||[]).map(m=>m.requester_id);let l={};if(d.length){const{data:m}=await T.from("users").select("id, pseudo, club_name").in("id",d);(m||[]).forEach(x=>{l[x.id]=x})}const u=(s||[]).map(m=>({...m,requester:l[m.requester_id]||{pseudo:"?"}})),i=ri(),r=u||[];i.innerHTML=`
    <div class="popup-box">
    ...
    </style>`}const sn="2026.06.27-2258";async function yi(e,{state:t,navigate:o,toast:a}){var s,d;const c=t.profile;c&&(e.innerHTML=`
  <div class="page">
    <div class="page-body">
  ...
  `,e.querySelectorAll("[data-nav]").forEach(l=>{l.addEventListener("click",u=>{u.preventDefault(),o(l.dataset.nav)})}),(s=document.getElementById("nav-rankings"))==null||s.addEventListener("click",()=>o("rankings")),(d=document.getElementById("nav-matches"))==null||d.addEventListener("click",()=>o("matches")),e.querySelectorAll("[data-action]").forEach(l=>{l.addEventListener("click",()=>{l.classList.add("tapped"),setTimeout(()=>l.classList.remove("tapped"),200);const u=l.dataset.action;if(u==="match-ai"){ln(o);return}if(u==="match-random"){o("match",{matchMode:"random"});return}if(u==="match-friend"){o("friends");return}a("Bientôt disponible","info")})}),document.getElementById("logout-btn").addEventListener("click",async()=>{await T.auth.signOut(),window.location.reload()}),zi(t,a),dn(t,a,o))}async function dn(e,t,o){var l,u,i,r;const a=document.getElementById("match-invite-banner");if(!a)return;const{data:c}=await T.from("friend_match_invites").select("id, inviter_id, inviter:users!inviter_id(pseudo, club_name)").eq("invitee_id",e.user.id).eq("status","pending").order("created_at",{ascending:!1}).limit(1).maybeSingle();if(!c){a.innerHTML="";return}const s=((l=c.inviter)==null?void 0:l.club_name)||((u=c.inviter)==null?void 0:u.pseudo)||"?",d=c.inviter_id;a.innerHTML=`
    ...
    </div>`,(i=document.getElementById("match-inv-accept"))==null||i.addEventListener("click",()=>{a.innerHTML="",o("match",{matchMode:"friend",friendId:d,friendName:s})}),(r=document.getElementById("match-inv-decline"))==null||r.addEventListener("click",async()=>{await T.from("friend_match_invites").update({status:"declined"}).eq("id",c.id),a.innerHTML="",t("Invitation refusée","info")})}async function zi(e,t){const o=document.getElementById("friend-requests-banner");if(!o)return;const{data:a,error:c}=await T.from("friendships").select("id, requester:users!requester_id(pseudo, club_name)").eq("addressee_id",e.user.id).eq("status","pending");if(c||!(a!=null&&a.length)){o.innerHTML="";return}const s=a.length,d=a.slice(0,2).map(u=>{var i;return((i=u.requester)==null?void 0:i.pseudo)||"?"}).join(", "),l=s>2?` +${s-2}`:"";o.innerHTML=`
  ...
  </div>`}async function fn(e,t){const{state:o,navigate:a,toast:c,openModal:s,closeModal:d}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:l}=await T.from("cards").select(`id, card_type, current_note, gc_type, formation, is_for_sale, sale_price,
  ...
  </div>`,document.getElementById("new-deck-btn").addEventListener("click",async()=>{const d=prompt("Nom du deck :",`Deck ${((s==null?void 0:s.length)||0)+1}`);if(!d)return;const{data:l,error:u}=await T.from("decks").insert({owner_id:o.profile.id,name:d}).select().single();if(u){c(u.message,"error");return}c("Deck créé !","success"),wi(l.id,e,t)}),e.querySelectorAll("[data-open-deck]").forEach(d=>{d.addEventListener("click",()=>wi(d.dataset.openDeck,e,t))}),e.querySelectorAll("[data-rename]").forEach(d=>{d.addEventListener("click",async()=>{const l=prompt("Nouveau nom :",d.dataset.name);if(!l||l===d.dataset.name)return;const{error:u}=await T.from("decks").update({name:l}).eq("id",d.dataset.rename);if(u){c(u.message,"error");return}c("Deck renommé !","success"),Xt(e,t)})}),e.querySelectorAll("[data-delete]").forEach(d=>{d.addEventListener("click",async()=>{if(!confirm(`Supprimer le deck "${d.dataset.name}" ? Cette action est irréversible.`))return;await T.from("deck_cards").delete().eq("deck_id",d.dataset.delete);const{error:l}=await T.from("decks").delete().eq("id",d.dataset.delete);if(l){c(l.message,"error");return}c("Deck supprimé.","success"),Xt(e,t)})})}async function wi(e,t,o){const{state:a,toast:c}=o;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:s}=await T.from("decks").select("*").eq("id",e).single(),{data:d}=await T.from("cards").select(`id, card_type, formation,
      player:players(id, firstname, surname_encoded, country_code, club_id, job, job2,
        note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length,
        clubs(encoded_name, logo_url))`).eq("owner_id",a.profile.id),l=(d||[]).filter(x=>x.card_type==="player"&&x.player),u=(d||[]).filter(x=>x.card_type==="formation"),i=u.map(x=>x.formation).filter(Boolean),{data:r}=await T.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",e);let f=s.formation||"4-4-2";i.length>0&&!i.includes(f)&&(f=i[0]);const m={deckId:e,name:s.name,formation:f,formationCardId:s.formation_card_id,slots:{},subs:[],playerCards:l,formationCards:u,availableFormations:i};(r||[]).forEach(x=>{x.is_starter?m.slots[x.position]=x.card_id:m.subs.includes(x.card_id)||m.subs.push(x.card_id)}),lt(t,m,o)}function lt(e,t,o){var u;const{navigate:a}=o;Dt[t.formation];const c=$i(t.formation),s=c.filter(i=>t.slots[i]).length,d=t.availableFormations.length>0?t.availableFormations:Object.keys(Dt),l=t.subs.map(i=>t.playerCards.find(r=>r.id===i)).filter(Boolean);[...Object.values(t.slots),...t.subs],e.innerHTML=`
  <div class="page">
    ...
    </div>`,'<button class="btn btn-ghost" id="close-sub-selector">Fermer</button>'),(u=document.getElementById("close-sub-selector"))==null||u.addEventListener("click",c),document.querySelectorAll(".player-option").forEach(i=>{i.addEventListener("click",()=>{e.subs.push(i.dataset.cardId),c(),lt(t,e,o)})})}async function vn(e,t){const{state:o,toast:a,navigate:c}=t,s=e.formationCards.find(u=>u.formation===e.formation),d=(s==null?void 0:s.id)||e.formationCardId;await T.from("decks").update({formation:e.formation,formation_card_id:d||null}).eq("id",e.deckId),await T.from("deck_cards").delete().eq("deck_id",e.deckId);const l=[];if(Object.entries(e.slots).forEach(([u,i],r)=>{l.push({deck_id:e.deckId,card_id:i,position:u,is_starter:!0,slot_order:r})}),e.subs.forEach((u,i)=>{l.push({deck_id:e.deckId,card_id:u,position:`SUB${i+1}`,is_starter:!1,slot_order:100+i})}),l.length>0){const{error:u}=await T.from("deck_cards").insert(l);if(u){a(u.message,"error");return}}a("Deck enregistré ✅","success"),c("decks")}function $i(e){const t=Dt[e]||Dt["4-4-2"],o=["GK1"];for(let a=1;a<=t.DEF;a++)o.push(`DEF${a}`);for(let a=1;a<=t.MIL;a++)o.push(`MIL${a}`);for(let a=1;a<=t.ATT;a++)o.push(`ATT${a}`);return o}async function Di(){const{data:e}=await T.from("booster_configs").select("*").eq("is_active",!0).order("sort_order");if(!(e!=null&&e.length))return[];const{data:t}=await T.from("booster_drop_rates").select("*").in("booster_id",e.map(o=>o.id)).order("sort_order");return e.map(o=>({...o,rates:(t||[]).filter(a=>a.booster_id===o.id)}))}function wn(e){if(!(e!=null&&e.length))return null;const t=e.reduce((a,c)=>a+Number(c.percentage),0);let o=Math.random()*t;for(const a of e)if(o-=Number(a.percentage),o<=0)return a;return e[e.length-1]}const qi=()=>Object.keys(St),$n=[{id:"players_std",img:"/manager-wars/icons/booster-players.png",name:"Players",sub:"5 cartes joueurs",cost:5e3,costLabel:"5 000 crédits",cardCount:5,type:"player"},{id:"players_pub",img:"/manager-wars/icons/booster-silver.png",name:"Players (pub)",sub:"3 cartes joueurs",cost:0,costLabel:"1 pub",cardCount:3,type:"player"},{id:"game_changer",img:"/manager-wars/icons/booster-gamechanger.png",name:"Game Changer",sub:"3 cartes spéciales",cost:1e4,costLabel:"10 000 crédits",cardCount:3,type:"game_changer"},{id:"formation",img:"/manager-wars/icons/booster-formation.png",name:"Formation",sub:"1 carte formation",cost:1e4,costLabel:"10 000 crédits",cardCount:1,type:"formation"}],Jt={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé."},"Double attaque":{icon:"⚡",desc:"La prochaine attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la prochaine action IA."},Gel:{icon:"❄️",desc:"Bloque le meilleur attaquant IA."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function _n(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";if(!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const o=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${o}.png`}const _i={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},kn={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"},En={MA:"MAROC",FR:"FRANCE",AR:"ARGENTINE",PT:"PORTUGAL",BR:"BRESIL",ES:"ESPAGNE",DE:"ALLEMAGNE",GB:"ANGLETERRE",IT:"ITALIE",CM:"CAMEROUN",SN:"SENEGAL",NG:"NIGERIA",DK:"DANEMARK",NL:"PAYS-BAS",BE:"BELGIQUE",CI:"CÔTE D'IVOIRE",AL:"ALBANIE",HR:"CROATIE",RS:"SERBIE",TR:"TURQUIE"};function ki(e,t){return e&&Number(t==="GK"?e.note_g:t==="DEF"?e.note_d:t==="MIL"?e.note_m:e.note_a)||0}function Tn(e){var r,f;const t=e.player;if(!t)return"";const o=t.job||"ATT",a=_i[o],c=kn[t.rarity]||"#ccc",s=ki(t,o),d=t.job2?ki(t,t.job2):null,l=t.job2?_i[t.job2]:null,u=_n(t),i=En[t.country_code]||t.country_code||"";return`
  ...
  </div>`}function et(e,t,o){if(!(e!=null&&e.length))return"";const a=e.slice(0,5);let c='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return a.forEach((s,d)=>{if(c+=Ue(s,40,52),d<a.length-1){const l=Qe(s,a[d+1]);c+=`<div style="width:7px;height:3px;background:${l==="#ff3333"||l==="#cc2222"?"rgba(255,255,255,0.12)":l};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),o!==void 0&&(c+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${o}</div>`),c+="</div>",c}function ei(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function Ki(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function at(e,t,o,a,c=310,s=310,d=[]){var j;const l=St[t]||{},u=Ai(t)||oi[t]||[],i={},r=["ATT","MIL","DEF","GK"];for(const O of r)(e[O]||[]).forEach((S,C)=>{i[`${O}${C+1}`]=S});function f(O){const F=l[O];return F?{x:F.x*c,y:F.y*s}:null}let m="";for(const[O,F]of u){const S=f(O),C=f(F);if(!S||!C)continue;const Z=i[O],ae=i[F],se=Qe(Z,ae);se==="#00ff88"||se==="#FFD700"?(m+=`<line x1="${S.x.toFixed(1)}" y1="${S.y.toFixed(1)}" x2="${C.x.toFixed(1)}" y2="${C.y.toFixed(1)}"
  ...
  `,document.body.appendChild(a);let r=!1;const f=()=>{r||(r=!0,a.remove(),o())};a.addEventListener("click",f),setTimeout(f,2e3)}function kt(e,t="rgba(0,0,0,0.8)"){const o=document.createElement("div");o.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,o.textContent=e,document.body.appendChild(o),setTimeout(()=>o.remove(),2200)}function Yt(e,t,o,a=null,c=null){var x,k;if(t.phase!=="attack"){kt("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){kt(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const s=Object.entries(t.homeTeam).flatMap(([n,E])=>(E||[]).filter(z=>z.used).map(z=>({...z,_line:z._line||n}))),d=t.homeSubs.filter(n=>!t.usedSubIds.includes(n.cardId));if(!s.length){kt("Aucun joueur utilisé à remplacer");return}if(!d.length){kt("Aucun remplaçant disponible");return}let l=Math.max(0,s.findIndex(n=>n.cardId===c));const u=((x=s[l])==null?void 0:x._line)||((k=s[l])==null?void 0:k.job);let i=a?Math.max(0,d.findIndex(n=>n.cardId===a)):Math.max(0,d.findIndex(n=>n.job===u)),r=!1;const f=document.createElement("div");f.id="sub-overlay",f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function m(){var F,S,C,Z,ae,se;const n=s[l],E=d[i],z=Math.min(130,Math.round((window.innerWidth-90)/2)),M=Math.round(z*1.35),j=B=>`background:rgba(255,255,255,0.12);border:none;color:${B?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${B?"default":"pointer"};flex-shrink:0`;f.innerHTML=`
    ...
    </div>`,document.body.appendChild(x),(k=x.querySelector("#gc-back"))==null||k.addEventListener("click",()=>x.remove()),(n=x.querySelector("#gc-use"))==null||n.addEventListener("click",()=>{x.remove(),to(e,t,o,a,c)})}function jt(e,t,o,a,c,s){const d=document.createElement("div");d.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let l=[];function u(){var i,r;d.innerHTML=`
    ...
    </div>`,(i=d.querySelector("#gc-picker-close"))==null||i.addEventListener("click",()=>d.remove()),d.querySelectorAll(".gc-pick-item").forEach(f=>{f.addEventListener("click",()=>{const m=f.dataset.cid,x=e.find(n=>n.cardId===m);if(!x)return;const k=l.findIndex(n=>n.cardId===m);k>-1?l.splice(k,1):l.length<t&&l.push(x),u()})}),(r=d.querySelector("#gc-picker-confirm"))==null||r.addEventListener("click",()=>{d.remove(),s(l)})}u(),document.body.appendChild(d)}const eo={BOOST_STAT:({value:e=1,count:t=1,roles:o=[]},a,c,s)=>{const d=Object.entries(a.homeTeam).filter(([l])=>!o.length||o.includes(l)).flatMap(([l,u])=>u.filter(i=>!i.used).map(i=>({...i,_line:l})));return d.length?(jt(d,t,`Choisir ${t} joueur(s) à booster (+${e})`,c,a,l=>{l.forEach(u=>{const i=(a.homeTeam[u._line]||[]).find(r=>r.cardId===u.cardId);i&&(i.boost=(i.boost||0)+e,a.log.push({text:`⚡ +${e} sur ${i.name}`,type:"info"}))}),ze(c,a,s)}),!0):(a.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),ze(c,a,s),!0)},DEBUFF_STAT:({value:e=1,count:t=1,roles:o=[],target:a="ai"},c,s,d)=>{const l=a==="home"?c.homeTeam:c.aiTeam,u=a==="ai"?"adverse":"allié",i=Object.entries(l).filter(([r])=>!o.length||o.includes(r)).flatMap(([r,f])=>f.filter(m=>!m.used).map(m=>({...m,_line:r})));return i.length?(jt(i,t,`Choisir ${t} joueur(s) ${u}(s) à débuffer (-${e})`,s,c,r=>{r.forEach(f=>{const x=((a==="home"?c.homeTeam:c.aiTeam)[f._line]||[]).find(k=>k.cardId===f.cardId);x&&(x.boost=(x.boost||0)-e,c.log.push({text:`🎯 -${e} sur ${x.name}${a==="ai"?" (IA)":""}`,type:"info"}))}),ze(s,c,d)}),!0):(c.log.push({text:`🎯 Aucun joueur ${u} disponible`,type:"info"}),ze(s,c,d),!0)},GRAY_PLAYER:({count:e=1,roles:t=[],target:o="ai"},a,c,s)=>{const d=o==="home"?a.homeTeam:a.aiTeam,l=o==="ai"?"adverse":"allié",u=Object.entries(d).filter(([i])=>!t.length||t.includes(i)).flatMap(([i,r])=>r.filter(f=>!f.used).map(f=>({...f,_line:i})));return u.length?(jt(u,e,`Choisir ${e} joueur(s) ${l}(s) à exclure`,c,a,i=>{i.forEach(r=>{const m=((o==="home"?a.homeTeam:a.aiTeam)[r._line]||[]).find(x=>x.cardId===r.cardId);m&&(m.used=!0,a.log.push({text:`❌ ${m.name}${o==="ai"?" (IA)":""} exclu !`,type:"info"}))}),ze(c,a,s)}),!0):(a.log.push({text:`❌ Aucun joueur ${l} à exclure`,type:"info"}),ze(c,a,s),!0)},REVIVE_PLAYER:({count:e=1,roles:t=[]},o,a,c)=>{const s=Object.entries(o.homeTeam).filter(([d])=>!t.length||t.includes(d)).flatMap(([d,l])=>l.filter(u=>u.used).map(u=>({...u,_line:d})));return s.length?(jt(s,e,`Choisir ${e} joueur(s) à ressusciter`,a,o,d=>{d.forEach(l=>{const u=(o.homeTeam[l._line]||[]).find(i=>i.cardId===l.cardId);u&&(u.used=!1,o.log.push({text:`💫 ${u.name} ressuscité !`,type:"info"}))}),ze(a,o,c)}),!0):(o.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),ze(a,o,c),!0)},REMOVE_GOAL:({},e)=>(e.aiScore>0?(e.aiScore--,e.log.push({text:"🚫 Dernier but IA annulé !",type:"info"})):e.log.push({text:"🚫 Aucun but à annuler",type:"info"}),!1),ADD_GOAL_DRAW:({},e)=>(e.homeScore===e.aiScore?(e.homeScore++,e.log.push({text:"🎯 But bonus (match nul) !",type:"info"})):e.log.push({text:"🎯 But bonus : non applicable (pas de match nul)",type:"info"}),!1),ADD_SUB:({value:e=1},t)=>(t.maxSubs=(t.maxSubs||3)+e,t.log.push({text:`🔄 +${e} remplacement(s) débloqué(s)`,type:"info"}),!1),CUSTOM:()=>!1};function to(e,t,o,a,c){a.usedGc.push(e);const s=a.gcDefs||[],d=s.find(u=>u.name===t)||s.find(u=>{var i;return((i=u.name)==null?void 0:i.toLowerCase().trim())===(t==null?void 0:t.toLowerCase().trim())});let l=!1;if(d!=null&&d.effect_type&&d.effect_type!=="CUSTOM"){const u=eo[d.effect_type];u?u(d.effect_params||{},a,o,c)||(l=!0):(c.toast(`Effet "${d.effect_type}" non implémenté`,"error"),l=!0)}else{switch(t){case"Double attaque":a.modifiers.home.doubleAttack=!0,a.log.push({text:"⚡ Double attaque activée !",type:"info"});break;case"Bouclier":a.modifiers.home.shield=!0,a.log.push({text:"🛡️ Bouclier activé !",type:"info"});break;case"Ressusciter":{const u=Object.entries(a.homeTeam).flatMap(([i,r])=>(r||[]).filter(f=>f.used).map(f=>({...f,_line:i})));u.length?(u[0].used=!1,a.log.push({text:`💫 ${u[0].name} ressuscité !`,type:"info"})):a.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"});break}case"Vol de note":a.modifiers.ai.stolenNote=(a.modifiers.ai.stolenNote||0)+1,a.log.push({text:"🎯 -1 à la prochaine attaque IA",type:"info"});break;case"Gel":{const u=[...a.aiTeam.ATT||[],...a.aiTeam.MIL||[]].filter(i=>!i.used);if(u.length){const i=u.sort((r,f)=>be(f,"ATT")-be(r,"ATT"))[0];i.used=!0,a.log.push({text:`❄️ ${i.name} (IA) gelé !`,type:"info"})}break}case"Remplacement+":a.maxSubs++,a.log.push({text:"🔄 +1 remplacement débloqué",type:"info"});break}l=!0}T.from("cards").delete().eq("id",e).then(()=>{}),l&&ze(o,a,c)}function io(e,t,o){const a=Object.values(t.homeTeam).flat().filter(c=>!c.used);if(!a.length){o.toast("Aucun joueur actif à booster","error");return}o.openModal("⚡ Utiliser le Boost",`<div style="margin-bottom:12px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border-radius:10px;padding:12px;text-align:center;color:#000">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(c=>{c.addEventListener("click",()=>{const s=c.dataset.cardId;for(const d of["GK","DEF","MIL","ATT"]){const l=(t.homeTeam[d]||[]).find(u=>u.cardId===s);if(l){l.boost=(l.boost||0)+t.boostCard.value,t.log.push({text:`⚡ Boost +${t.boostCard.value} appliqué à ${l.name}`,type:"info"});break}}t.boostUsed=!0,o.closeModal(),ze(e,t,o)})})}function qt(e,t,o,a,c){const s=document.createElement("div");s.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:900;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;overflow:hidden;cursor:pointer";const d=Array.from({length:10},(i,r)=>`
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(s);let l=!1;const u=()=>{l||(l=!0,s.remove(),setTimeout(()=>c(),50))};s.addEventListener("click",u),setTimeout(u,3500)}async function It(e,t,o){var r,f;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:a}=o,c=t.homeScore>t.aiScore,s=t.homeScore===t.aiScore,d=c?"victoire":s?"nul":"defaite",l=cn(t.mode,d);t.matchId&&await T.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:c?a.profile.id:null,home_credits_reward:l,played_at:new Date().toISOString()}).eq("id",t.matchId);const u={credits:(a.profile.credits||0)+l,matches_played:(a.profile.matches_played||0)+1};c?u.wins=(a.profile.wins||0)+1:s?u.draws=(a.profile.draws||0)+1:u.losses=(a.profile.losses||0)+1,await T.from("users").update(u).eq("id",a.profile.id),await o.refreshProfile();const i=document.createElement("div");i.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",i.innerHTML=`
    ...
    </div>`,`<button class="btn btn-primary" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}async function oo(e,t){var o,a;try{const c=(a=(o=t==null?void 0:t.state)==null?void 0:o.profile)==null?void 0:a.id;try{(T.getChannels?T.getChannels():[]).forEach(d=>{const l=d.topic||"";(l.includes("matchmaking")||l.includes("pvp-match")||l.includes("friend-invite"))&&T.removeChannel(d)})}catch(s){console.warn("[Random] cleanup canaux:",s)}c&&await T.rpc("cancel_matchmaking",{p_user_id:c}).catch(()=>{})}catch{}await pi(e,t,"random",({deckId:c,formation:s,starters:d,subsRaw:l,gcCardsEnriched:u,gcDefs:i})=>{const r=f=>ro(e,t,c,s,d,l,f,i||[]);if(!u.length){r([]);return}ci(e,u,r)})}async function ro(e,t,o,a,c,s,d=[],l=[]){var M;const{state:u,navigate:i,toast:r}=t;let f=!1,m=null;We(e),e.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const x=async(j=!0)=>{f=!0,m&&(T.removeChannel(m),m=null),j&&await T.rpc("cancel_matchmaking",{p_user_id:u.profile.id}).catch(()=>{})};(M=document.getElementById("mm-cancel"))==null||M.addEventListener("click",async()=>{try{await x(!0)}catch{}Ne(e),i("home")});const k=async(j,O,F)=>{if(f)return;f=!0,m&&(T.removeChannel(m),m=null);const S=document.getElementById("mm-status");S&&(S.textContent="Adversaire trouvé !"),await new Promise(C=>setTimeout(C,600)),e.isConnected&&ao(e,t,j,F,d,l)},{data:n,error:E}=await T.rpc("try_matchmake",{p_user_id:u.profile.id,p_deck_id:o});if(E||!(n!=null&&n.success)){r("Erreur de matchmaking","error"),Ne(e),i("home");return}if(n.matched){k(n.match_id,n.opponent_id,!1);return}const z=document.getElementById("mm-status");z&&(z.textContent="En attente d'un autre joueur..."),m=T.channel("matchmaking-"+u.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${u.profile.id}`},j=>{const O=j.new;O.status==="matched"&&O.match_id&&k(O.match_id,O.matched_with,!0)}).subscribe()}async function ao(e,t,o,a,c=[],s=[]){const{state:d,navigate:l,toast:u}=t,i=a?"p1":"p2",r=a?"p2":"p1",f=(c||[]).map(p=>p.id),m=(c||[]).map(p=>({id:p.id,gc_type:p.gc_type,_gcDef:p._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:x}=await T.from("matches").select("*").eq("id",o).single();if(!x){u("Match introuvable","error"),l("home");return}async function k(){const[{data:p},{data:g},{data:b},{data:h}]=await Promise.all([T.rpc("get_deck_for_match",{p_deck_id:x.home_deck_id}),T.rpc("get_deck_for_match",{p_deck_id:x.away_deck_id}),T.from("users").select("id,pseudo,club_name").eq("id",x.home_id).single(),T.from("users").select("id,pseudo,club_name").eq("id",x.away_id).single()]),y=$=>({cardId:$.card_id,position:$.position,id:$.id,firstname:$.firstname,name:$.surname_encoded,country_code:$.country_code,club_id:$.club_id,job:$.job,job2:$.job2,note_g:Number($.note_g)||0,note_d:Number($.note_d)||0,note_m:Number($.note_m)||0,note_a:Number($.note_a)||0,rarity:$.rarity,skin:$.skin,hair:$.hair,hair_length:$.hair_length,clubName:$.club_encoded_name||null,clubLogo:$.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}),v=((p==null?void 0:p.starters)||[]).map($=>y($)),w=((g==null?void 0:g.starters)||[]).map($=>y($)),_=(p==null?void 0:p.formation)||"4-4-2",I=(g==null?void 0:g.formation)||"4-4-2";return{p1Team:ut(v,_),p2Team:ut(w,I),p1Subs:((p==null?void 0:p.subs)||[]).map($=>y($)),p2Subs:((g==null?void 0:g.subs)||[]).map($=>y($)),p1Formation:_,p2Formation:I,p1Name:(b==null?void 0:b.club_name)||(b==null?void 0:b.pseudo)||"Joueur 1",p2Name:(h==null?void 0:h.club_name)||(h==null?void 0:h.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:a?f:[],gc_p2:a?[]:f,gcCardsFull_p1:a?m:[],gcCardsFull_p2:a?[]:m,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:s||[],lastActionAt:new Date().toISOString()}}let n=x.game_state&&Object.keys(x.game_state).length?x.game_state:null;if(!n)if(a){n=await k();const{data:p}=await T.from("matches").select("game_state").eq("id",o).single();!(p!=null&&p.game_state)||!Object.keys(p.game_state).length?await T.from("matches").update({game_state:n,turn_user_id:x.home_id}).eq("id",o):n=p.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let p=0;p<30&&!n;p++){await new Promise(b=>setTimeout(b,400));const{data:g}=await T.from("matches").select("game_state").eq("id",o).single();g!=null&&g.game_state&&Object.keys(g.game_state).length&&(n=g.game_state)}if(!n){u("Erreur de synchronisation","error"),l("home");return}n.gc_p2=f,n.gcCardsFull_p2=m,await T.from("matches").update({game_state:n}).eq("id",o)}let E=!1,z=null,M=!1;const j=new Set,O=new Set;function F(p){var $,L;try{T.removeChannel(S)}catch{}const g=n[i+"Score"]||0,b=n[r+"Score"]||0;let h,y;p.winner_id?(h=p.winner_id===d.profile.id,y=!1):p.forfeit?(h=g>b,y=!1):(y=g===b,h=g>b),($=document.getElementById("pvp-end-overlay"))==null||$.remove();const v=document.createElement("div");v.id="pvp-end-overlay",v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const w=y?"🤝":h?"🏆":"😞",_=y?"MATCH NUL":h?"VICTOIRE !":"DÉFAITE",I=y?"#fff":h?"#FFD700":"#ff6b6b";v.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(v),(L=v.querySelector("#pvp-end-home"))==null||L.addEventListener("click",()=>{v.remove(),Ne(e),l("home")})}const S=T.channel("pvp-match-"+o).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${o}`},p=>{const g=p.new;try{if(g.status==="finished"||g.forfeit){if(E)return;E=!0,z&&(clearInterval(z),z=null),g.game_state&&(n=g.game_state),F(g);return}if(g.game_state){const b=n;n=g.game_state;const h=n._lastGC;if(h&&h.seq&&!O.has(h.seq)&&(O.add(h.seq),h.by!==i)){me(h.type,h.by,()=>D());return}const y=b[i+"Score"]||0,v=b[r+"Score"]||0,w=n[i+"Score"]||0,_=n[r+"Score"]||0,I=w>y,$=_>v;if((I||$)&&!j.has(n.round)){j.add(n.round);const L=[...n.log||[]].reverse().find(U=>U.isGoal),q=((L==null?void 0:L.homePlayers)||[]).map(U=>({name:U.name,note:U.note,portrait:U.portrait,job:U.job}));ge(q,w,_,I,()=>D());return}D()}}catch(b){console.error("[PvP] crash:",b)}}).subscribe();async function C(p){Object.assign(n,p),n.lastActionAt=new Date().toISOString();const{error:g}=await T.from("matches").update({game_state:n}).eq("id",o);g&&u("Erreur de synchronisation","error");try{D()}catch(b){console.error("[PvP] renderPvpScreen crash:",b)}}async function Z(){if(E)return;E=!0,z&&(clearInterval(z),z=null);const p=a?x.away_id:x.home_id,g=a?"p2":"p1",b=a?"p1":"p2",h={...n,[g+"Score"]:3,[b+"Score"]:0,phase:"finished"};try{await T.from("matches").update({status:"finished",forfeit:!0,winner_id:p,home_score:h.p1Score||0,away_score:h.p2Score||0,game_state:h}).eq("id",o)}catch{}try{T.removeChannel(S)}catch{}setTimeout(()=>{Ne(e),l("home")},800)}const ae={BOOST_STAT:({value:p=1,count:g=1,roles:b=[]},h,y)=>{const v=h[i+"Team"],w=Object.entries(v).filter(([_])=>!b.length||b.includes(_)).flatMap(([_,I])=>I.filter($=>!$.used).map($=>({...$,_line:_})));if(!w.length){h.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),y(h);return}se(w,g,`Choisir ${g} joueur(s) à booster (+${p})`,_=>{_.forEach(I=>{const $=(v[I._line]||[]).find(L=>L.cardId===I.cardId);$&&($.boost=($.boost||0)+p,h.log.push({text:`⚡ +${p} sur ${$.name}`,type:"info"}))}),h[i+"Team"]=v,y(h)})},DEBUFF_STAT:({value:p=1,count:g=1,roles:b=[],target:h="ai"},y,v)=>{const w=h==="home"?i:r,_=y[w+"Team"],I=h==="home"?"allié":"adverse",$=Object.entries(_).filter(([L])=>!b.length||b.includes(L)).flatMap(([L,q])=>q.filter(U=>!U.used).map(U=>({...U,_line:L})));if(!$.length){y.log.push({text:`🎯 Aucun joueur ${I}`,type:"info"}),v(y);return}se($,g,`Choisir ${g} joueur(s) ${I}(s) (-${p})`,L=>{L.forEach(q=>{const U=(_[q._line]||[]).find(oe=>oe.cardId===q.cardId);U&&(U.boost=(U.boost||0)-p,y.log.push({text:`🎯 -${p} sur ${U.name}`,type:"info"}))}),y[w+"Team"]=_,v(y)})},GRAY_PLAYER:({count:p=1,roles:g=[],target:b="ai"},h,y)=>{const v=b==="home"?i:r,w=h[v+"Team"],_=b==="home"?"allié":"adverse",I=Object.entries(w).filter(([$])=>!g.length||g.includes($)).flatMap(([$,L])=>L.filter(q=>!q.used).map(q=>({...q,_line:$})));if(!I.length){h.log.push({text:`❌ Aucun joueur ${_}`,type:"info"}),y(h);return}se(I,p,`Choisir ${p} joueur(s) ${_}(s) à exclure`,$=>{$.forEach(L=>{const q=(w[L._line]||[]).find(U=>U.cardId===L.cardId);q&&(q.used=!0,h.log.push({text:`❌ ${q.name} exclu !`,type:"info"}))}),h[v+"Team"]=w,y(h)})},REVIVE_PLAYER:({count:p=1,roles:g=[]},b,h)=>{const y=b[i+"Team"],v=Object.entries(y).filter(([w])=>!g.length||g.includes(w)).flatMap(([w,_])=>_.filter(I=>I.used).map(I=>({...I,_line:w})));if(!v.length){b.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),h(b);return}se(v,p,`Choisir ${p} joueur(s) à ressusciter`,w=>{w.forEach(_=>{const I=(y[_._line]||[]).find($=>$.cardId===_.cardId);I&&(I.used=!1,b.log.push({text:`💫 ${I.name} ressuscité !`,type:"info"}))}),b[i+"Team"]=y,h(b)})},REMOVE_GOAL:({},p,g)=>{const b=r+"Score";p[b]>0?(p[b]--,p.log.push({text:"🚫 Dernier but annulé !",type:"info"})):p.log.push({text:"🚫 Aucun but à annuler",type:"info"}),g(p)},ADD_GOAL_DRAW:({},p,g)=>{p[i+"Score"]===p[r+"Score"]?(p[i+"Score"]++,p.log.push({text:"🎯 But bonus !",type:"info"})):p.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),g(p)},ADD_SUB:({value:p=1},g,b)=>{g.maxSubs=(g.maxSubs||3)+p,g.log.push({text:`🔄 +${p} remplacement(s)`,type:"info"}),b(g)},CUSTOM:({},p,g)=>g(p)};function se(p,g,b,h){const y=document.createElement("div");y.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let v=[];function w(){var I,$;const _=p.map(L=>{const q={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[L._line]||"#555",U=be(L,L._line)+(L.boost||0),ce=v.find(H=>H.cardId===L.cardId)?"#FFD700":"rgba(255,255,255,0.25)",G=L.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${L.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${ce};background:${q};overflow:hidden;cursor:pointer;${G}">
        ...
        </div>`,(I=y.querySelector("#pp-close"))==null||I.addEventListener("click",()=>y.remove()),y.querySelectorAll(".pp-item").forEach(L=>{L.addEventListener("click",()=>{const q=L.dataset.cid,U=p.find(ce=>ce.cardId===q),oe=v.findIndex(ce=>ce.cardId===q);U&&(oe>-1?v.splice(oe,1):v.length<g&&v.push(U),w())})}),($=y.querySelector("#pp-confirm"))==null||$.addEventListener("click",()=>{y.remove(),h(v)})}w(),document.body.appendChild(y)}async function B(p,g){const h=(n["gcCardsFull_"+i]||[]).find(_=>_.id===p),y=(h==null?void 0:h._gcDef)||(n.gcDefs||[]).find(_=>{var I;return _.name===g||((I=_.name)==null?void 0:I.toLowerCase().trim())===(g==null?void 0:g.toLowerCase().trim())}),v=[...n["usedGc_"+i]||[],p],w={type:g,by:i,seq:(n._gcAnimSeq||0)+1};O.add(w.seq),me(g,i,async()=>{if(y!=null&&y.effect_type&&y.effect_type!=="CUSTOM"){const I=ae[y.effect_type];if(I){const $={...n};I(y.effect_params||{},$,async L=>{L["usedGc_"+i]=v,L._lastGC=w,L._gcAnimSeq=w.seq,await C(L)});return}}const _={...n};switch(g){case"Remplacement+":_.maxSubs=(_.maxSubs||3)+1,_.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const I=r+"Score";_[I]>0&&(_[I]--,_.log.push({text:"🚫 But annulé",type:"info"}));break}}_["usedGc_"+i]=v,_._lastGC=w,_._gcAnimSeq=w.seq,await C(_)})}function Y(p,g){const b="usedCardIds_"+p,h=new Set(n[b]||[]);g.forEach(y=>h.add(y)),n[b]=[...h]}function te(){for(const p of["p1","p2"]){const g=new Set(n["usedCardIds_"+p]||[]),b=n[p+"Team"];if(b)for(const h of["GK","DEF","MIL","ATT"])(b[h]||[]).forEach(y=>{y.used=g.has(y.cardId)})}}function D(){var Oe,ft,gt,mt,xt,yt;if(n.phase==="reveal")return Q();if(n.phase==="midfield")return re();if(n.phase==="finished")return Ke();const p=n[i+"Team"],g=n[r+"Team"];te();const b=n[i+"Score"],h=n[r+"Score"],y=n[i+"Name"],v=n[r+"Name"],w=n.phase===i+"-attack",_=n.phase===i+"-defense",I=Array.isArray(n["selected_"+i])?n["selected_"+i]:[],$=I.map(A=>A.cardId),L=window.innerWidth>=700,q=n[i+"Subs"]||[],U=n["usedSubIds_"+i]||[],oe=q.filter(A=>!U.includes(A.cardId)),ce=n["gcCardsFull_"+i]||[],G=n["usedGc_"+i]||[],H=ce.filter(A=>!G.includes(A.id)),J=n.boostOwner===i&&!n.boostUsed,ne=!["GK","DEF","MIL","ATT"].some(A=>(g[A]||[]).some(V=>!V.used)),de=[...p.MIL||[],...p.ATT||[]].filter(A=>!A.used),fe=w&&ne&&de.length===0?[...p.GK||[],...p.DEF||[]].filter(A=>!A.used).map(A=>A.cardId):[];function le(A,V,pe){var wt,$t;const ee=A._gcDef,He={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ee==null?void 0:ee.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",Ye={purple:"#b06ce0",light_blue:"#00d4ef"}[ee==null?void 0:ee.color]||"#b06ce0",Xe=(ee==null?void 0:ee.name)||A.gc_type,Je=(ee==null?void 0:ee.effect)||((wt=Be[A.gc_type])==null?void 0:wt.desc)||"",ht=ee!=null&&ee.image_url?`/manager-wars/icons/${ee.image_url}`:null,Ut=(($t=Be[A.gc_type])==null?void 0:$t.icon)||"⚡",bt=Math.round(pe*.22),vt=Math.round(pe*.22),nt=pe-bt-vt,Kt=Xe.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${A.id}" data-gc-type="${A.gc_type}"
    ...
    </div>`,i==="p1"&&setTimeout(async()=>{await C({phase:"midfield"})},5e3)}let K=!1;function re(){if(K)return;const p=n[i+"Team"].MIL||[],g=n[r+"Team"].MIL||[];function b(H){return H.reduce((J,ne)=>J+be(ne,"MIL"),0)}function h(H){let J=0;for(let ne=0;ne<H.length-1;ne++){const de=Qe(H[ne],H[ne+1]);de==="#00ff88"?J+=2:de==="#FFD700"&&(J+=1)}return J}const y=b(p)+h(p),v=b(g)+h(g),w=y>=v;function _(H,J,ne){return`<div id="duel-row-${ne}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(v);let I=!1;const $=()=>{I||(I=!0,v.remove(),setTimeout(()=>y(),50))};v.addEventListener("click",$),setTimeout($,3500)}function me(p,g,b){var G,H;const h=(n.gcDefs||[]).find(J=>{var ne;return J.name===p||((ne=J.name)==null?void 0:ne.toLowerCase().trim())===(p==null?void 0:p.toLowerCase().trim())}),y={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[h==null?void 0:h.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",v={purple:"#b06ce0",light_blue:"#00d4ef"}[h==null?void 0:h.color]||"#b06ce0",w=(h==null?void 0:h.name)||p,_=(h==null?void 0:h.effect)||((G=Be[p])==null?void 0:G.desc)||"",I=h!=null&&h.image_url?`/manager-wars/icons/${h.image_url}`:null,$=((H=Be[p])==null?void 0:H.icon)||"⚡",q=g===i?"Vous":n[g+"Name"]||"Adversaire",U=document.createElement("div");U.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",U.innerHTML=`
      <style>
      ...
      </div>`,document.body.appendChild(q),(ce=q.querySelector("#pvp-gc-back"))==null||ce.addEventListener("click",()=>q.remove()),(G=q.querySelector("#pvp-gc-use"))==null||G.addEventListener("click",()=>{q.remove(),B(p,g)})}function R(){var h;const p=n[i+"Team"],g=Object.entries(p).flatMap(([y,v])=>(v||[]).filter(w=>!w.used).map(w=>({...w,_line:y})));if(!g.length)return;const b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",b.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
      ...
      </div>`,document.body.appendChild(b),(h=b.querySelector("#bp-close"))==null||h.addEventListener("click",()=>b.remove()),b.querySelectorAll(".bp-item").forEach(y=>{y.addEventListener("click",async()=>{const v=y.dataset.cid,w=g.find(I=>I.cardId===v);if(!w)return;const _=(p[w._line]||[]).find(I=>I.cardId===v);_&&(_.boost=(_.boost||0)+n.boostValue),b.remove(),await C({[i+"Team"]:p,boostUsed:!0})})})}function N(p=null){var oe,ce;if(!(n.phase===i+"-attack")){u("Remplacement uniquement avant votre attaque","warning");return}const b=n[i+"Team"],h=n["usedSubIds_"+i]||[],y=n.maxSubs||3;if(h.length>=y){u(`Maximum ${y} remplacements atteint`,"warning");return}const v=Object.entries(b).flatMap(([G,H])=>(H||[]).filter(J=>J.used).map(J=>({...J,_line:G}))),w=(n[i+"Subs"]||[]).filter(G=>!h.includes(G.cardId));if(!v.length){u("Aucun joueur utilisé à remplacer","warning");return}if(!w.length){u("Aucun remplaçant disponible","warning");return}let _=Math.max(0,v.findIndex(G=>G.cardId===p));const I=((oe=v[_])==null?void 0:oe._line)||((ce=v[_])==null?void 0:ce.job);let $=Math.max(0,w.findIndex(G=>G.job===I)),L=!1;const q=document.createElement("div");q.id="pvp-sub-overlay",q.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function U(){var le,ye,Ee,Re,Ce,je;const G=v[_],H=w[$],J=Math.min(130,Math.round((window.innerWidth-90)/2)),ne=Math.round(J*1.35),de=$e=>`background:rgba(255,255,255,0.12);border:none;color:${$e?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${$e?"default":"pointer"};flex-shrink:0`;q.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:6px">Appuyer pour continuer</div>`,document.body.appendChild(y);let w=!1;const _=()=>{w||(w=!0,y.remove(),setTimeout(()=>b(),50))};y.addEventListener("click",_),setTimeout(_,2200)}function P(){var g;const p=document.createElement("div");p.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:20px;overflow-y:auto",p.innerHTML=`
      ...
      </div>`,document.body.appendChild(g),(b=g.querySelector("#pvp-hist-close"))==null||b.addEventListener("click",()=>g.remove())}async function ue(){if(n.phase!==i+"-attack")return;const p=i==="p1"?"p2":"p1",g=(n.round||0)+1,b=[...n.log||[]];b.push({type:"info",text:`⏭️ ${n[i+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const h=Le(n),y=we(p),v=h||!y?"finished":p+"-attack";await C({["selected_"+i]:[],modifiers:{...n.modifiers,[i]:{}},pendingAttack:null,phase:v,attacker:p,round:g,log:b}),v==="finished"&&await Se(n)}async function ie(){const p=n[i+"Team"],g=!["GK","DEF","MIL","ATT"].some(v=>(n[r+"Team"][v]||[]).some(w=>!w.used)),b=(n["selected_"+i]||[]).map(v=>{const w=(p[v._role]||[]).find(U=>U.cardId===v.cardId)||v,_=g&&["GK","DEF"].includes(v._role),I=p[v._role]||[],$=I.findIndex(U=>U.cardId===v.cardId),L=it(I.length),q=$>=0?L[$]:w._col??1;return{...w,_line:v._role,_col:q,..._?{note_a:Math.max(1,Number(w.note_a)||0)}:{}}});if(!b.length)return;const h=Nt(b,n.modifiers[i]||{});Y(i,b.map(v=>v.cardId)),b.forEach(v=>{const w=(p[v._role]||[]).find(_=>_.cardId===v.cardId);w&&(w.used=!0)}),n["selected_"+i]=[],D();const y=[...n.log||[]];if(g){const v=(n[i+"Score"]||0)+1,w=b.map(q=>({name:q.name,note:be(q,q._line||"ATT"),portrait:Ie(q),job:q.job}));y.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:w,homeTotal:h.total,aiTotal:0});const _=(n.round||0)+1,I=i==="p1"?"p2":"p1",$={...n,[i+"Team"]:p,[i+"Score"]:v},L=Le($);j.add(_),ge(w,v,n[r+"Score"]||0,!0,async()=>{await C({[i+"Team"]:p,[i+"Score"]:v,["selected_"+i]:[],modifiers:{...n.modifiers,[i]:{}},pendingAttack:null,phase:L?"finished":I+"-attack",attacker:I,round:_,log:y}),L&&await Se({...n,[i+"Score"]:v})});return}y.push({type:"pending",text:`⚔️ ${n[i+"Name"]} attaque (${h.total})`}),await C({[i+"Team"]:p,[r+"Team"]:n[r+"Team"],pendingAttack:{...h,players:b,side:i},["selected_"+i]:[],modifiers:{...n.modifiers,[i]:{}},phase:r+"-defense",log:y})}async function Ae(){const p=n[i+"Team"],g=(n["selected_"+i]||[]).map(G=>{const H=(p[G._role]||[]).find(le=>le.cardId===G.cardId)||G,J=p[G._role]||[],ne=J.findIndex(le=>le.cardId===G.cardId),de=it(J.length),fe=ne>=0?de[ne]:H._col??1;return{...H,_line:G._role,_col:fe}}),b=Rt(g,n.modifiers[i]||{});Y(i,g.map(G=>G.cardId)),g.forEach(G=>{const H=(p[G._role]||[]).find(J=>J.cardId===G.cardId);H&&(H.used=!0)}),n["selected_"+i]=[],D();const h=Pt(n.pendingAttack.total,b.total,n.modifiers[i]||{}),y=n.pendingAttack.side,v=h==="attack"||(h==null?void 0:h.goal),w=y==="p1"?"p2":"p1",_=(n.round||0)+1,I=(n.pendingAttack.players||[]).map(G=>({name:G.name,note:be(G,G._line||"ATT"),portrait:Ie(G),job:G.job})),$=[...n.log||[]];$.push({type:"duel",isGoal:v,homeScored:v&&y===i,text:v?`⚽ BUT de ${n[y+"Name"]} ! (${n.pendingAttack.total} vs ${b.total})`:`✋ Attaque stoppée (${n.pendingAttack.total} vs ${b.total})`,homePlayers:I,aiPlayers:g.map(G=>({name:G.name,note:be(G,G._line||"DEF"),portrait:Ie(G),job:G.job})),homeTotal:n.pendingAttack.total,aiTotal:b.total});const L=v?(n[y+"Score"]||0)+1:n[y+"Score"]||0,q={...n,[i+"Team"]:p,[y+"Score"]:L},U=Le(q),oe=U?"finished":w+"-attack",ce=async()=>{await C({[i+"Team"]:p,[r+"Team"]:n[r+"Team"],[y+"Score"]:L,["selected_"+i]:[],modifiers:{...n.modifiers,[i]:{}},pendingAttack:null,phase:oe,attacker:w,round:_,log:$}),(oe==="finished"||U)&&await Se({...n,[y+"Score"]:L})};if(v){const G=y===i,H=G?L:n[i+"Score"]||0,J=G?n[r+"Score"]||0:L;j.add(_),ge(I,H,J,G,ce)}else await ce()}function ke(p){return["MIL","ATT"].some(g=>(p[g]||[]).some(b=>!b.used))}function ve(p){return["GK","DEF","MIL","ATT"].some(g=>(p[g]||[]).some(b=>!b.used))}function _e(p){return ve(p)&&!ke(p)}function we(p){const g=n[p+"Team"],b=n[(p==="p1"?"p2":"p1")+"Team"];return!!(ke(g)||!ve(b)&&_e(g))}function Le(p){const g=["MIL","ATT"].some($=>(p.p1Team[$]||[]).some(L=>!L.used)),b=["MIL","ATT"].some($=>(p.p2Team[$]||[]).some(L=>!L.used)),h=ve(p.p1Team),y=ve(p.p2Team);return!g&&!(!y&&h)&&(!b&&!(!h&&y))}function Ge(p){const g=p.p1Score||0,b=p.p2Score||0;return g===b?null:g>b?x.home_id:x.away_id}async function Se(p){try{await T.from("matches").update({status:"finished",winner_id:Ge(p),home_score:p.p1Score||0,away_score:p.p2Score||0}).eq("id",o)}catch(g){console.error("[PvP] finishMatch:",g)}}function Ke(){var y;const p=n[i+"Score"],g=n[r+"Score"],b=p>g,h=p===g;We(e),e.innerHTML=`
    ...
    </div>`,(y=document.getElementById("pvp-home"))==null||y.addEventListener("click",()=>{try{T.removeChannel(S)}catch{}Ne(e),l("home")})}D()}async function so(e,t,o,a){var c,s;try{const d=(s=(c=t==null?void 0:t.state)==null?void 0:c.profile)==null?void 0:s.id;try{(T.getChannels?T.getChannels():[]).forEach(u=>{const i=u.topic||"";(i.includes("matchmaking")||i.includes("pvp-match"))&&T.removeChannel(u)})}catch(l){console.warn("[FriendMatch] cleanup canaux:",l)}d&&(await T.rpc("cancel_matchmaking",{p_user_id:d}).catch(()=>{}),await T.from("matchmaking_queue").delete().eq("user_id",d).then(()=>{},()=>{}))}catch{}await pi(e,t,"random",({deckId:d,formation:l,starters:u,subsRaw:i,gcCardsEnriched:r,gcDefs:f})=>{const m=x=>lo(e,t,d,l,u,i,x,f||[],o,a);if(!r.length){m([]);return}ci(e,r,m)})}async function lo(e,t,o,a,c,s,d=[],l=[],u,i){var S;const{state:r,navigate:f,toast:m}=t,x=r.profile.id;let k=!1,n=null;We(e);try{await T.rpc("cancel_matchmaking",{p_user_id:x})}catch{}try{await T.from("matchmaking_queue").delete().eq("user_id",x)}catch{}const E=(C,Z,ae,se)=>{var B;e.innerHTML=`
  ...
  `;function u(i){const r=document.getElementById("mkt-content"),f=i==="buy"?d:l;if(f.length===0){r.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
    ...
    </div>`,r.querySelectorAll("[data-buy]").forEach(x=>{x.addEventListener("click",()=>fo(x.dataset.buy,f,e,t))}),r.querySelectorAll("[data-cancel]").forEach(x=>{x.addEventListener("click",()=>mo(x.dataset.cancel,e,t))})}u("buy"),e.querySelectorAll(".mkt-tab").forEach(i=>{i.addEventListener("click",()=>{e.querySelectorAll(".mkt-tab").forEach(r=>{const f=r===i;r.style.background=f?"var(--green)":"#fff",r.style.color=f?"#fff":"var(--gray-600)",r.style.borderColor=f?"var(--green)":"var(--gray-200)"}),u(i.dataset.tab)})})}async function fo(e,t,o,a){var x;const{state:c,toast:s,refreshProfile:d}=a,l=t.find(k=>k.id===e);if(!l)return;const u=l.price,i=c.profile.credits||0,r=(x=l.card)==null?void 0:x.player;if(i<u){s("Crédits insuffisants","error");return}if(!await go(r,u))return;const m=document.querySelector(`[data-buy="${e}"]`);m&&(m.disabled=!0,m.textContent="⏳");try{const{data:k,error:n}=await T.rpc("buy_market_card",{p_listing_id:e,p_buyer_id:c.profile.id});if(n)throw new Error(n.message);if(!(k!=null&&k.success))throw new Error((k==null?void 0:k.error)||"Achat impossible");c.profile.credits=i-u;const E=document.getElementById("nav-credits");E&&(E.textContent=`💰 ${(i-u).toLocaleString("fr")}`),s(`✅ ${r==null?void 0:r.surname_encoded} ajouté à ta collection !`,"success"),fi(o,a)}catch(k){s("❌ "+k.message,"error"),m&&(m.disabled=!1,m.textContent="Acheter")}}function go(e,t){return new Promise(o=>{const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",a.innerHTML=`
      ...
      </div>`,document.body.appendChild(a);const c=s=>{a.remove(),o(s)};a.querySelector("#buy-ok").addEventListener("click",()=>c(!0)),a.querySelector("#buy-cancel").addEventListener("click",()=>c(!1)),a.addEventListener("click",s=>{s.target===a&&c(!1)})})}async function mo(e,t,o){const{toast:a}=o,{data:c}=await T.from("market_listings").select("card_id").eq("id",e).single();await T.from("market_listings").update({status:"cancelled"}).eq("id",e),c&&await T.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",c.card_id),a("Annonce retirée","success"),fi(t,o)}function xo(e){return{GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[e]||"#888"}async function yo(e,{state:t,navigate:o,toast:a}){e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:c}=await T.from("users").select("id,pseudo,club_name,trophies_top1,trophies_top2,trophies_top3,wins,level").order("trophies_top1",{ascending:!1}).limit(100);e.innerHTML=`
  <div class="page">
    <div class="page-header">
  ...
  </div>`}Xi(Qi);const he={user:null,profile:null,page:"home",params:{}};function Et(e,t="info",o=3e3){const a=document.getElementById("toast");a&&(a.textContent=e,a.className=`show ${t}`,clearTimeout(a._t),a._t=setTimeout(()=>{a.className=""},o))}function bo(e,t,o=""){document.getElementById("modal-title").textContent=e,document.getElementById("modal-body").innerHTML=t,document.getElementById("modal-footer").innerHTML=o,document.getElementById("modal-overlay").classList.remove("hidden")}function ii(){document.getElementById("modal-overlay").classList.add("hidden")}async function Tt(){if(!he.user)return;const{data:e}=await T.from("users").select("*").eq("id",he.user.id).single();e&&(he.profile=e)}const gi="mw_theme";function Gt(){return localStorage.getItem(gi)||"light"}function vo(e){var t;localStorage.setItem(gi,e),ni(e),(t=he.profile)!=null&&t.id&&T.from("users").update({theme:e}).eq("id",he.profile.id).then(()=>{})}function ni(e){document.documentElement.setAttribute("data-theme",e)}function Bt(e,t={}){he.page=e,he.params=t,Yi()}async function Yi(){var a,c;const e=document.getElementById("page-content");if(!e)return;document.querySelectorAll(".bottom-nav a").forEach(s=>{s.classList.toggle("active",s.dataset.page===he.page)});const t=document.getElementById("nav-credits");t&&he.profile&&(t.textContent=`💰 ${(he.profile.credits||0).toLocaleString("fr")}`);const o={state:he,navigate:Bt,toast:Et,openModal:bo,closeModal:ii,refreshProfile:Tt};switch(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽</div>',he.page){case"home":await yi(e,o);break;case"collection":await fn(e,o);break;case"decks":await Xt(e,o);break;case"boosters":await An(e,o);break;case"match":{const s=he.params&&he.params.matchMode||"vs_ai_easy";s==="random"?await oo(e,o):s==="friend"?await so(e,o,(a=he.params)==null?void 0:a.friendId,(c=he.params)==null?void 0:c.friendName):await Pn(e,o);break}case"market":await uo(e,o);break;case"rankings":await yo(e,o);break;case"matches":await ho(e,o);break;case"friends":await nn(e,o);break;default:await yi(e,o)}}function wo(){const e=document.getElementById("app"),t=he.profile;if(!t)return;const o="/manager-wars/icons/";e.innerHTML=`
    <nav class="top-nav">
