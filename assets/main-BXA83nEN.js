import{s as k,F as it,b as Ce,c as Gt,l as ge,d as di}from"./formation-links-Bt44rMnM.js";function kt(e,{navigate:t,toast:i}){e.innerHTML=`
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
  ...
  </div>`}async function xi(e,t){const{state:i,navigate:s,toast:c,openModal:o,closeModal:a}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:d}=await k.from("cards").select(`id, card_type, current_note, gc_type, formation, is_for_sale, sale_price,
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),n=(d||[]).filter($=>$.card_type==="player"&&$.player),r=(d||[]).filter($=>$.card_type==="game_changer"),u=(d||[]).filter($=>$.card_type==="formation"),{data:m}=await k.from("gc_definitions").select("name,gc_type,color,effect,image_url"),f={};(m||[]).forEach($=>{f[$.name]=$});const h=Object.keys(it),p=Object.keys(de),x={};n.forEach($=>{const b=$.player.id;x[b]=(x[b]||0)+1}),new Set(Object.keys(x).map($=>String($)));const A=new Set(u.map($=>$.formation)),v=new Set(r.map($=>$.gc_type));let g="player",w="Tous",F="",M=!1;function y(){return[...n].sort(($,b)=>{const S=Be.indexOf($.player.job),E=Be.indexOf(b.player.job);return S!==E?S-E:($.player.surname_encoded||"").localeCompare(b.player.surname_encoded||"")})}function I(){return[...l||[]].sort(($,b)=>{const S=Be.indexOf($.job),E=Be.indexOf(b.job);return S!==E?S-E:($.surname_encoded||"").localeCompare(b.surname_encoded||"")})}function G(){return y().filter($=>{const b=$.player,S=w==="Tous"||b.job===w,E=!F||`${b.firstname} ${b.surname_encoded}`.toLowerCase().includes(F);return S&&E})}function R(){return I().filter($=>{const b=w==="Tous"||$.job===w,S=!F||`${$.firstname} ${$.surname_encoded}`.toLowerCase().includes(F);return b&&S})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
  ...
  </div>`;function P(){const $=document.getElementById("col-filters");$&&(g==="player"?($.innerHTML=`
          ...
          ${gi.map(b=>`
            <button class="filter-btn" data-job="${b}"
              style="flex-shrink:0;padding:4px 12px;border-radius:20px;font-size:12px;font-weight:600;cursor:pointer;
                border:1.5px solid ${b===w?"var(--green)":"var(--gray-200)"};
                background:${b===w?"var(--green)":"#fff"};
                color:${b===w?"#fff":"var(--gray-600)"}">
              ${b}
            </button>`).join("")}
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",b=>{F=b.target.value.toLowerCase(),H()}),e.querySelectorAll(".filter-btn").forEach(b=>{b.addEventListener("click",()=>{w=b.dataset.job,P(),H()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{M=!M,P(),H()})):($.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{M=!M,P(),H()})))}function H(){const $=document.getElementById("col-grid");$&&(g==="player"?j($):g==="formation"?N($):C($))}function W($,b,S,E,_){_=_||"#7a28b8";const D=document.getElementById("col-grid"),T=document.getElementById("col-big");if(!D||!T)return;var q=0;function B(){T.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+b($[q])+"</div>";var U=T.querySelector("[data-card-id],[data-form-id],[data-gc-id]");U&&U.addEventListener("click",function(){E($[q])}),requestAnimationFrame(function(){var K=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!K||!T)){var J=T.clientHeight-8,te=T.clientWidth-24,re=K.offsetHeight,ae=K.offsetWidth;if(re>0&&ae>0&&J>40){var se=Math.min(J/re,te/ae,1);K.style.transform="scale("+se.toFixed(3)+")",K.style.transformOrigin="top center"}}}),D.innerHTML=$.map(function(K,J){return'<div class="col-mini-item" data-idx="'+J+'" style="flex-shrink:0;cursor:pointer;border-radius:8px;border:1.5px solid '+(J===q?_:"transparent")+';transition:border-color .15s;overflow:hidden">'+S(K,J===q)+"</div>"}).join(""),D.querySelectorAll(".col-mini-item").forEach(function(K){K.addEventListener("click",function(){q=Number(K.dataset.idx),B(),K.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}B()}function V($){var b=.54,S=Math.round(140*b),E=Math.round(310*b),_;if(!$||$._fake){var D=$?$.player:null;if(!D)return"";_=Tt(D)}else _=Ye($,"");return'<div style="width:'+S+"px;height:"+E+'px;overflow:hidden;position:relative;flex-shrink:0"><div style="transform:scale('+b+');transform-origin:top left;position:absolute;top:0;left:0;pointer-events:none">'+_+"</div></div>"}function ee($,b,S){b=b||100,S=S||140;var E=Ce[$]||{},_={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},D=Math.max(3,Math.round(b*.06)),T=Object.entries(E).map(function(B){var U=B[0],K=B[1],J=U.replace(/\d+$/,""),te=_[J]||"#888",re=Math.round(K.x*b),ae=Math.round(K.y*S);return'<circle cx="'+re+'" cy="'+ae+'" r="'+D+'" fill="'+te+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),q=Math.max(1,Math.round(b/50));return'<svg viewBox="0 0 '+b+" "+S+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+b+'" height="'+S+'" fill="#1A6B3C"/><rect x="'+Math.round(b*.2)+'" y="'+Math.round(S*.02)+'" width="'+Math.round(b*.6)+'" height="'+Math.round(S*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+q+'"/><line x1="0" y1="'+Math.round(S*.5)+'" x2="'+b+'" y2="'+Math.round(S*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+q+'"/><ellipse cx="'+Math.round(b*.5)+'" cy="'+Math.round(S*.5)+'" rx="'+Math.round(b*.18)+'" ry="'+Math.round(S*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+q+'"/><rect x="'+Math.round(b*.2)+'" y="'+Math.round(S*.82)+'" width="'+Math.round(b*.6)+'" height="'+Math.round(S*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+q+'"/>'+T+"</svg>"}function z($,b,S){var E=S>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+S+"</div>":"",_=b?'data-form-id="'+b.id+'"':"",D=$.length>7?14:$.length>5?16:19,T=!!b;return"<div "+_+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(T?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(T?"":"filter:grayscale(1);opacity:0.5")+'">'+E+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(T?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+D+"px;font-weight:900;color:"+(T?"#1A6B3C":"#aaa")+';line-height:1">'+$+'</div></div><div style="flex:1;overflow:hidden;background:'+(T?"#1A6B3C":"#ccc")+'">'+ee($,140,220)+"</div></div>"}function L($,b){var S=.54,E=Math.round(140*S),_=Math.round(305*S),D=Math.round(_*.22),T=_-D,q=$.length>7?5:7,B=ee($,E,T),U=b?"1.5px solid #2a7a40":"1px solid #ddd",K=b?"":"filter:grayscale(1);opacity:0.45;",J=b?"#1A6B3C":"#bbb",te="#fff";return'<div style="width:'+E+"px;height:"+_+"px;border-radius:6px;border:"+U+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+K+'"><div style="height:'+D+"px;background:"+J+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+q+"px;font-weight:900;color:"+te+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(E-4)+'px">'+$+'</span></div><div style="height:'+T+'px;overflow:hidden;flex-shrink:0">'+B+"</div></div>"}function j($){if(M){const b=R();if(!b.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const S=b.map(E=>n.find(_=>_.player.id===E.id)||{_fake:!0,player:E,id:"fake-"+E.id});W(S,E=>E._fake?Tt(E.player):Ye(E,""),E=>E._fake?V({player:E.player,id:"x",_fake:!0}):V(E),E=>{E._fake||Lt(E,n,x,t)},"#1A6B3C")}else{const b=G();if(!b.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const S={},E=[];b.forEach(_=>{S[_.player.id]||(S[_.player.id]=!0,E.push(_))}),W(E,_=>{const D=x[_.player.id]||1,T=D>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${D}</div>`:"",B=n.filter(U=>U.player.id===_.player.id&&U.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Ye(_,T+B)},_=>V(_),_=>Lt(_,n,x,t),"#1A6B3C")}}function N($){const b=M?h:[...A];if(!b.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const S=b.map(E=>({formation:E,card:u.find(_=>_.formation===E)||null,owned:A.has(E)}));W(S,({formation:E,card:_,owned:D})=>z(E,D?_:null,D?u.filter(T=>T.formation===E).length:0),({formation:E,owned:_})=>L(E,_),({card:E,owned:_})=>{_&&E&&hi(E,u,t,o)},"#1A6B3C")}function C($){const b=Object.keys(f),S=M?b.length?b:p:[...v];if(!S.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const E=S.map(_=>({type:_,gc:de[_]||{icon:"⚡",desc:""},def:f[_]||null,owned:v.has(_),card:r.find(D=>D.gc_type===_)||null}));W(E,({type:_,gc:D,def:T,owned:q,card:B})=>{const U=q?r.filter(Se=>Se.gc_type===_).length:0,K=U>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${U}</div>`:"",J=(T==null?void 0:T.gc_type)==="ultra_game_changer",te={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},re={purple:"#b06ce0",light_blue:"#00d4ef"},ae=te[T==null?void 0:T.color]||te.purple,se=re[T==null?void 0:T.color]||re.purple,me=(T==null?void 0:T.effect)||D.desc||"",xe=T!=null&&T.image_url?`/manager-wars/icons/${T.image_url}`:null;return q&&B?`<div data-gc-id="${B.id}" data-gc-type="${_}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${se};background:${ae};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${se}66;cursor:pointer">
          ${K}
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Ee=1e3;function hi(e,t,i,s){var p,x,A;const{state:c,toast:o,closeModal:a,navigate:d,refreshProfile:l}=i,n=e.formation,r={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function u(){const v=Ce[n]||{},g=it[n]||[],w=290,F=360,M=20;function y(G){const R=v[G];return R?{x:R.x*w,y:R.y*F}:null}let I=`<svg width="${w}" height="${F}" viewBox="0 0 ${w} ${F}" xmlns="http://www.w3.org/2000/svg">`;for(const[G,R]of g){const P=y(G),H=y(R);!P||!H||(I+=`<line x1="${P.x}" y1="${P.y}" x2="${H.x}" y2="${H.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const G of Object.keys(v)){const R=y(G);if(!R)continue;const P=G.replace(/\d+/,""),H=r[P]||"#555";I+=`<circle cx="${R.x}" cy="${R.y}" r="${M}" fill="${H}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,I+=`<text x="${R.x}" y="${R.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${P}</text>`}return I+="</svg>",I}const m=t.filter(v=>v.formation===n),f=m.length,h=!e.is_for_sale;s(`Formation ${n}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(p=document.getElementById("direct-sell-form-btn"))==null||p.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${n} pour ${Ee.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const v=m.find(w=>!w.is_for_sale)||m[0];if(!v){o("Aucune carte à vendre","error");return}const{error:g}=await k.from("cards").delete().eq("id",v.id);if(g){o(g.message,"error");return}await k.from("users").update({credits:(c.profile.credits||0)+Ee}).eq("id",c.profile.id),await l(),o(`+${Ee.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),a(),d("collection")}),(x=document.getElementById("market-sell-form-btn"))==null||x.addEventListener("click",async()=>{const v=parseInt(document.getElementById("sell-price-form").value);if(!v||v<1){o("Prix invalide","error");return}await k.from("cards").update({is_for_sale:!0,sale_price:v}).eq("id",e.id),await k.from("market_listings").insert({seller_id:c.profile.id,card_id:e.id,price:v}),o("Carte mise en vente sur le marché !","success"),a(),d("collection")}),(A=document.getElementById("cancel-sell-form-btn"))==null||A.addEventListener("click",async()=>{await k.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await k.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),o("Annonce retirée","success"),a(),d("collection")})}async function Lt(e,t,i,s){var I,G,R,P,H,W;const{state:c,toast:o,openModal:a,closeModal:d,navigate:l,refreshProfile:n}=s,r=e.player,u=t.filter(V=>V.player.id===r.id),m=u.length,f=mi[r.rarity]||1e3,h=r.rarity!=="legende",p=Pt(),x=Te(r,r.job),A=r.job2?Te(r,r.job2):null,v=Re[r.job]||"#1A6B3C",g=r.job2?Re[r.job2]:null,w=Nt[r.rarity]||"#ccc",F=at[r.country_code]||r.country_code||"",{data:M}=await k.from("transfer_history").select("club_name, manager_name, source, price, transferred_at").eq("player_id",r.id).order("transferred_at",{ascending:!0}),y=M&&M.length?`
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      ...
      <div style="width:140px;border-radius:12px;padding:6px;background:${w};flex-shrink:0">
        <div style="background:#f2e8d2;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
          ...
          <div style="background:#f2e8d2;padding:3px 6px;display:flex;align-items:center;justify-content:space-between;min-height:24px">
            ...
            ${(I=r.clubs)!=null&&I.logo_url?`<img src="${r.clubs.logo_url}" style="width:22px;height:16px;object-fit:contain">`:`<div style="background:#1a3a7a;color:#fff;border-radius:2px;padding:1px 3px;font-size:6px;font-weight:800">${(((G=r.clubs)==null?void 0:G.encoded_name)||"").slice(0,6)}</div>`}
          </div>
        </div>
      </div>
    ...
    </div>`:""}`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(R=document.getElementById("close-detail"))==null||R.addEventListener("click",d),(P=document.getElementById("direct-sell-btn"))==null||P.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte ${r.surname_encoded} pour ${f.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const V=u.find(z=>!z.is_for_sale)||u[0];if(!V){o("Aucune carte à vendre","error");return}const{error:ee}=await k.from("cards").delete().eq("id",V.id);if(ee){o(ee.message,"error");return}await k.from("users").update({credits:(c.profile.credits||0)+f}).eq("id",c.profile.id),await n(),o(`+${f.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),d(),l("collection")}),(H=document.getElementById("market-sell-btn"))==null||H.addEventListener("click",async()=>{const V=parseInt(document.getElementById("sell-price").value);if(!V||V<1){o("Prix invalide","error");return}await k.from("cards").update({is_for_sale:!0,sale_price:V}).eq("id",e.id),await k.from("market_listings").insert({seller_id:c.profile.id,card_id:e.id,price:V}),o("Carte mise en vente sur le marché !","success"),d(),l("collection")}),(W=document.getElementById("cancel-sell-btn"))==null||W.addEventListener("click",async()=>{await k.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await k.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),o("Annonce retirée","success"),d(),l("collection")})}const ve={},Ne={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},ue={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function Ue(e){const t=typeof import.meta<"u"?ve==null?void 0:ve.VITE_SUPABASE_URL:"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function Ot(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function Ht(e){var i;const t=(ve==null?void 0:ve.VITE_SUPABASE_URL)||"";return(i=e==null?void 0:e.clubs)!=null&&i.logo_url?e.clubs.logo_url.startsWith("http")?e.clubs.logo_url:t?`${t}/storage/v1/object/public/assets/clubs/${e.clubs.logo_url}`:null:null}function vi(e,t=44,i=58){var f;const s=e?Ue(e):null,c=e?Ht(e):null,o=Ot(e==null?void 0:e.country_code),a=(e==null?void 0:e.job)||"MIL",d=ue[a]||"#555",l={normal:"#aaa",pépite:"#D4A017",papyte:"#222",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",n=Number(a==="GK"?e==null?void 0:e.note_g:a==="DEF"?e==null?void 0:e.note_d:a==="MIL"?e==null?void 0:e.note_m:e==null?void 0:e.note_a)||0,r=Math.round(i*.19),u=Math.round(i*.25),m=i-r-u;return e?`<div style="width:${t}px;height:${i}px;border-radius:5px;border:2px solid ${l};background:${d};position:relative;overflow:hidden;flex-shrink:0">
  ...
  <div class="page">
  ...
  </div>`,bi(e,t,c,i),document.getElementById("builder-back").addEventListener("click",()=>s("decks")),document.getElementById("formation-select").addEventListener("change",n=>{t.formation=n.target.value;const r=zt(t.formation),u={};r.forEach(m=>{t.slots[m]&&(u[m]=t.slots[m])}),t.slots=u,_e(e,t,i)}),document.getElementById("save-deck").addEventListener("click",()=>$i(t,i)),e.querySelectorAll("[data-remove-sub]").forEach(n=>{n.addEventListener("click",()=>{t.subs=t.subs.filter(r=>r!==n.dataset.removeSub),_e(e,t,i)})}),(l=document.getElementById("add-sub-btn"))==null||l.addEventListener("click",()=>{_i(t,e,i)})}function bi(e,t,i,s){const c=e.querySelector("#deck-field");if(!c)return;const o=Ce[t.formation]||{},a=Gt(t.formation)||[],d={};for(const A of i){const v=t.slots[A],g=v?t.playerCards.find(w=>w.id===v):null;d[A]=g?g.player:null}const l=300,n=300,r=48,u=64,m=13,f=16,h=38;function p(A){const v=o[A];return v?{x:v.x*l,y:v.y*n}:null}let x="";for(const[A,v]of a){const g=p(A),w=p(v);if(!g||!w)continue;const F=d[A]?{...d[A],club_id:d[A].club_id,country_code:d[A].country_code,rarity:d[A].rarity}:null,M=d[v]?{...d[v],club_id:d[v].club_id,country_code:d[v].country_code,rarity:d[v].rarity}:null,y=ge(F,M);y==="#ff3333"||y==="#cc2222"?x+=`<line x1="${g.x.toFixed(1)}" y1="${g.y.toFixed(1)}" x2="${w.x.toFixed(1)}" y2="${w.y.toFixed(1)}" stroke="${y}" stroke-width="2.5" stroke-linecap="round" opacity="0.4"/>`:(x+=`<line x1="${g.x.toFixed(1)}" y1="${g.y.toFixed(1)}" x2="${w.x.toFixed(1)}" y2="${w.y.toFixed(1)}" stroke="${y}" stroke-width="8" stroke-linecap="round" opacity="0.2"/>`,x+=`<line x1="${g.x.toFixed(1)}" y1="${g.y.toFixed(1)}" x2="${w.x.toFixed(1)}" y2="${w.y.toFixed(1)}" stroke="${y}" stroke-width="2.5" stroke-linecap="round" opacity="0.9"/>`)}for(const A of i){const v=p(A);if(!v)continue;const g=d[A],w=A.replace(/\d+/,""),F=ue[w]||"#555",M=(v.x-r/2).toFixed(1),y=(v.y-u/2).toFixed(1),I={normal:"#aaa",pépite:"#D4A017",papyte:"#222",légende:"#7a28b8"}[g==null?void 0:g.rarity]||"#aaa";if(g){const G=Ue(g),R=Ht(g),P=Ot(g.country_code),H=Number(w==="GK"?g.note_g:w==="DEF"?g.note_d:w==="MIL"?g.note_m:g.note_a)||0,W=u-m-f;x+=`<defs><clipPath id="dcp-${A}"><rect x="${M}" y="${(v.y-u/2+m).toFixed(1)}" width="${r}" height="${W}"/></clipPath></defs>`,x+=`<rect x="${M}" y="${y}" width="${r}" height="${u}" rx="5" fill="${F}"/>`,G&&(x+=`<image href="${G}" x="${M}" y="${(v.y-u/2+m).toFixed(1)}" width="${r}" height="${W}" clip-path="url(#dcp-${A})" preserveAspectRatio="xMidYMin slice"/>`),x+=`<rect x="${M}" y="${y}" width="${r}" height="${m}" fill="rgba(255,255,255,0.93)"/>`,x+=`<text x="${v.x.toFixed(1)}" y="${(v.y-u/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(g.surname_encoded||"").slice(0,9)}</text>`;const V=(v.y+u/2-f).toFixed(1);x+=`<rect x="${M}" y="${V}" width="${r}" height="${f}" fill="rgba(255,255,255,0.93)"/>`,P&&(x+=`<image href="${P}" x="${(v.x-21).toFixed(1)}" y="${(v.y+u/2-f+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`),x+=`<text x="${v.x.toFixed(1)}" y="${(v.y+u/2-f/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${H}</text>`,R&&(x+=`<image href="${R}" x="${(v.x+r/2-14).toFixed(1)}" y="${(v.y+u/2-f+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`),x+=`<rect x="${M}" y="${y}" width="${r}" height="${u}" rx="5" fill="none" stroke="${I}" stroke-width="2"/>`}else x+=`<rect x="${M}" y="${y}" width="${r}" height="${u}" rx="5" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="5,3"/>`,x+=`<text x="${v.x.toFixed(1)}" y="${v.y.toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="22" fill="rgba(255,255,255,0.35)">+</text>`,x+=`<text x="${v.x.toFixed(1)}" y="${(v.y+16).toFixed(1)}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.3)">${w}</text>`;x+=`<rect x="${M}" y="${y}" width="${r}" height="${u}" rx="5" fill="rgba(0,0,0,0.01)" class="deck-slot-hit" data-pos="${A}" style="cursor:pointer"/>`}c.innerHTML=`<svg viewBox="${-h} ${-h} ${l+h*2} ${n+h*2}" width="100%" style="display:block;max-width:440px;margin:0 auto">${x}</svg>`,c.querySelectorAll(".deck-slot-hit").forEach(A=>{A.addEventListener("click",()=>wi(A.dataset.pos,t,e,s))})}function wi(e,t,i,s){var m,f,h;const{openModal:c,closeModal:o}=s,a=e.replace(/\d+/,""),d=t.slots[e],l=d?t.playerCards.find(p=>p.id===d):null;(m=l==null?void 0:l.player)==null||m.id;const n=new Set;Object.entries(t.slots).forEach(([p,x])=>{var v;if(p===e||!x)return;const A=t.playerCards.find(g=>g.id===x);(v=A==null?void 0:A.player)!=null&&v.id&&n.add(A.player.id)}),t.subs.forEach(p=>{var A;const x=t.playerCards.find(v=>v.id===p);(A=x==null?void 0:x.player)!=null&&A.id&&n.add(x.player.id)});const r=new Set,u=t.playerCards.filter(p=>{const x=p.player;return!(x.job===a||x.job2===a)||n.has(x.id)||r.has(x.id)?!1:(r.add(x.id),!0)});u.sort((p,x)=>{const A=a==="GK"?p.player.note_g:a==="DEF"?p.player.note_d:a==="MIL"?p.player.note_m:p.player.note_a;return(a==="GK"?x.player.note_g:a==="DEF"?x.player.note_d:a==="MIL"?x.player.note_m:x.player.note_a)-A}),c(`Choisir ${a} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${u.length>0?u.map(p=>{var w,F;const x=p.player,A=a==="GK"?x.note_g:a==="DEF"?x.note_d:a==="MIL"?x.note_m:x.note_a,v=Ue(x),g={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[x.rarity];return`<div class="player-option" data-card-id="${p.id}"
          ...
          <div style="flex:1;min-width:0">
            ...
            <div style="display:flex;align-items:center;gap:6px;margin-top:3px">
              ...
              ${(w=x.clubs)!=null&&w.logo_url?`<img src="${x.clubs.logo_url}" style="width:18px;height:18px;object-fit:contain">`:`<span style="font-size:10px;color:var(--gray-600)">${((F=x.clubs)==null?void 0:F.encoded_name)||"—"}</span>`}
    ...
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(f=document.getElementById("close-selector"))==null||f.addEventListener("click",o),(h=document.getElementById("remove-player"))==null||h.addEventListener("click",()=>{delete t.slots[e],o(),_e(i,t,s)}),document.querySelectorAll(".player-option").forEach(p=>{p.addEventListener("click",()=>{t.slots[e]=p.dataset.cardId,o(),_e(i,t,s)})})}function _i(e,t,i){var l;const{openModal:s,closeModal:c}=i,o=new Set;Object.values(e.slots).filter(Boolean).forEach(n=>{var u;const r=e.playerCards.find(m=>m.id===n);(u=r==null?void 0:r.player)!=null&&u.id&&o.add(r.player.id)}),e.subs.forEach(n=>{var u;const r=e.playerCards.find(m=>m.id===n);(u=r==null?void 0:r.player)!=null&&u.id&&o.add(r.player.id)});const a=new Set,d=e.playerCards.filter(n=>{var r,u,m;return o.has((r=n.player)==null?void 0:r.id)||a.has((u=n.player)==null?void 0:u.id)?!1:(a.add((m=n.player)==null?void 0:m.id),!0)});s("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${d.length>0?d.map(n=>{var f;const r=n.player,u=Ue(r),m=r.job==="GK"?r.note_g:r.job==="DEF"?r.note_d:r.job==="MIL"?r.note_m:r.note_a;return`<div class="player-option" data-card-id="${n.id}"
          style="display:flex;align-items:center;gap:10px;padding:8px;border:1.5px solid var(--gray-200);border-radius:10px;cursor:pointer">
  ...
  `,document.body.appendChild(s);let c=!1;document.getElementById("pack-visual").addEventListener("click",()=>{if(c)return;c=!0;const f=document.getElementById("pack-visual");f.classList.add("shaking"),setTimeout(()=>{f.classList.add("pack-open"),setTimeout(()=>{document.getElementById("pack-phase").style.display="none",a(0)},600)},500)});let o=0;function a(f){o=f;const h=document.getElementById("reveal-phase");h.style.display="flex",d(f)}function d(f){var M;const h=e[f],p=document.getElementById("card-counter"),x=document.getElementById("single-card-slot"),A=document.getElementById("card-tap-hint");p&&(p.textContent=`Carte ${f+1} / ${e.length}`),A&&(A.textContent=f<e.length-1?"Appuie pour continuer →":"Appuie pour voir toutes tes cartes");const v=St(h),g=h.card_type==="player"&&((M=h.player)==null?void 0:M.rarity)==="legende";x.innerHTML=`
      ...
      </div>`,g&&r();const w=document.getElementById("current-card-wrap");let F=!1;w.addEventListener("click",()=>{if(F)return;F=!0;const y=f+1;y<e.length?(w.style.transition="all 0.25s ease",w.style.transform="translateX(-120%) rotate(-15deg)",w.style.opacity="0",setTimeout(()=>d(y),250)):l()})}function l(){u(),document.getElementById("reveal-phase").style.display="none";const f=document.getElementById("recap-phase");f.style.display="flex";const h=document.getElementById("recap-grid");h.innerHTML=e.map((p,x)=>`
      ...
      </div>`).join("")}let n=null;function r(){const f=document.getElementById("fireworks-canvas");if(!f)return;f.width=window.innerWidth,f.height=window.innerHeight;const h=f.getContext("2d"),p=[];function x(){const v=Math.random()*f.width,g=Math.random()*f.height*.6,w=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],F=w[Math.floor(Math.random()*w.length)];for(let M=0;M<60;M++){const y=Math.PI*2/60*M,I=2+Math.random()*5;p.push({x:v,y:g,vx:Math.cos(y)*I,vy:Math.sin(y)*I,alpha:1,color:F,size:2+Math.random()*3})}}x(),n=setInterval(x,600);function A(){if(document.getElementById("fireworks-canvas")){h.clearRect(0,0,f.width,f.height);for(let v=p.length-1;v>=0;v--){const g=p[v];if(g.x+=g.vx,g.y+=g.vy+.08,g.vy*=.98,g.alpha-=.018,g.alpha<=0){p.splice(v,1);continue}h.globalAlpha=g.alpha,h.fillStyle=g.color,h.beginPath(),h.arc(g.x,g.y,g.size,0,Math.PI*2),h.fill()}h.globalAlpha=1,(n!==null||p.length>0)&&requestAnimationFrame(A)}}A()}function u(){n!==null&&(clearInterval(n),n=null);const f=document.getElementById("fireworks-canvas");f&&f.getContext("2d").clearRect(0,0,f.width,f.height)}document.getElementById("reveal-collection").addEventListener("click",()=>{u(),s.remove(),i("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{u(),s.remove(),i("boosters")})}function St(e){var t,i,s,c;if(e.card_type==="player"&&e.player){const o=e.player,a=o.job||"ATT",d=zi[a]||"#1A6B3C",l=Ci[o.rarity]||"#ccc",n=a==="GK"?o.note_g:a==="DEF"?o.note_d:a==="MIL"?o.note_m:o.note_a,r=Mi(),u={MA:"MAROC",FR:"FRANCE",AR:"ARGENTINE",PT:"PORTUGAL",BR:"BRESIL",ES:"ESPAGNE",DE:"ALLEMAGNE",GB:"ANGLETERRE",IT:"ITALIE",CM:"CAMEROUN",SN:"SENEGAL"}[o.country_code]||o.country_code;return`<div style="width:140px;height:200px;background:#f2e8d2;border-radius:12px;border:3px solid ${l};overflow:hidden;display:flex;flex-direction:column">
      <!-- Nom -->
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}const Ui="simulation",Vt="VOTRE_ZONE_ID";function Ki(){switch(Ui){case"propellerads":return Vi();case"adsense":return Yi();default:return st()}}function st(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:#111;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:14px";let i=5;t.innerHTML=`
      ...
      <button id="ad-skip" disabled style="margin-top:8px;padding:10px 24px;border-radius:20px;border:none;background:rgba(255,255,255,0.15);color:rgba(255,255,255,0.4);font-size:13px;cursor:default">
        Passer (5)
      </button>`,document.body.appendChild(t);const s=setInterval(()=>{i--;const c=document.getElementById("ad-cd"),o=document.getElementById("ad-skip");c&&(c.textContent=i),o&&(o.textContent=i>0?`Passer (${i})`:"✓ Continuer"),i<=0&&(clearInterval(s),o&&(o.disabled=!1,o.style.cssText=o.style.cssText.replace("rgba(255,255,255,0.15)","#1A6B3C").replace("rgba(255,255,255,0.4)","#fff").replace("default","pointer")),o==null||o.addEventListener("click",()=>{t.remove(),e(!0)},{once:!0}))},1e3)})}function Vi(){return new Promise((e,t)=>{if(typeof window.propeller>"u")return console.warn("[Ad] PropellerAds non chargé → simulation"),e(st());window.propeller.push({zone_id:Vt,onComplete:()=>e(!0),onSkip:()=>t(new Error("Publicité ignorée")),onError:()=>t(new Error("Erreur publicité"))})})}function Yi(){return new Promise((e,t)=>{if(typeof window.adsbygoogle>"u")return console.warn("[Ad] AdSense non chargé → simulation"),e(st());try{(window.adsbygoogle=window.adsbygoogle||[]).push({google_ad_client:"ca-pub-VOTRE_PUBLISHER_ID",google_ad_slot:Vt,google_ad_format:"rewarded",on_reward:()=>e(!0),on_dismiss:()=>t(new Error("Publicité fermée"))})}catch(i){t(i)}})}const be={},$e={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},ke={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function fe(e,t,i,s,c){var o;e.innerHTML=`<div class="match-screen" style="display:flex;align-items:center;justify-content:center;min-height:100vh">
    <div style="text-align:center;padding:40px;color:#fff">
      <div style="font-size:48px;margin-bottom:16px">${t}</div>
      <p style="margin-bottom:16px">${i}</p>
      <button class="btn btn-primary" id="msg-btn">${s}</button>
    </div>
  </div>`,(o=document.getElementById("msg-btn"))==null||o.addEventListener("click",c)}function ne(e){const t=typeof import.meta<"u"?be==null?void 0:be.VITE_SUPABASE_URL:"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function Xe(e,t){var s,c;const i=e.player;return{cardId:e.id,position:t||null,id:i.id,firstname:i.firstname,name:i.surname_encoded,country_code:i.country_code,club_id:i.club_id,job:i.job,job2:i.job2,note_g:Number(i.note_g)||0,note_d:Number(i.note_d)||0,note_m:Number(i.note_m)||0,note_a:Number(i.note_a)||0,rarity:i.rarity,skin:i.skin,hair:i.hair,hair_length:i.hair_length,clubName:((s=i.clubs)==null?void 0:s.encoded_name)||null,clubLogo:((c=i.clubs)==null?void 0:c.logo_url)||null,boost:0,used:!1,_line:null,_col:null}}function Pe(e){return e===1?[1]:e===2?[0,2]:e===3?[0,1,2]:e===4?[0,1,1,2]:e===5?[0,1,1,1,2]:[1]}function Yt(){const e=Math.random()*100;return e<10?3:e<30?2:1}function Oe(e,t){const i=$e[t]||$e["4-4-2"],s={GK:[],DEF:[],MIL:[],ATT:[]};if(e.length&&e.every(a=>a.position)){for(const a of["GK","DEF","MIL","ATT"]){const d=e.filter(n=>n.position&&n.position.replace(/\d+$/,"")===a).sort((n,r)=>parseInt(n.position.replace(/\D+/g,""),10)-parseInt(r.position.replace(/\D+/g,""),10)).map(n=>({...n,_line:a})),l=Pe(d.length);d.forEach((n,r)=>{n._col=l[r]}),s[a]=d}return s}const o=[...e];for(const a of["GK","DEF","MIL","ATT"]){const d=[];for(let n=0;n<i[a];n++){let r=o.findIndex(u=>u.job===a);if(r===-1&&(r=o.findIndex(u=>u.job2===a)),r===-1&&(r=0),o[r]){const u={...o[r],_line:a};d.push(u),o.splice(r,1)}}const l=Pe(d.length);d.forEach((n,r)=>{n._col=l[r]}),s[a]=d}return s}function Wt(e){document.querySelectorAll(".top-nav, .bottom-nav").forEach(t=>{t.style.setProperty("display","none","important"),t.dataset.matchHidden="1"}),e&&e.style.setProperty("padding-bottom","0","important")}function le(e){document.querySelectorAll(".top-nav, .bottom-nav").forEach(t=>{t.style.removeProperty("display"),delete t.dataset.matchHidden}),e&&e.style.removeProperty("padding-bottom")}function Jt(e,t,i){const c=new Set,o=t.filter(r=>{const u=r.gc_type||r.id;return c.has(u)?!1:(c.add(u),!0)});let a=[];function d(r,u){const m=r._gcDef,f={purple:"linear-gradient(135deg,#3d0a7a,#7a28b8)",light_blue:"linear-gradient(135deg,#006080,#00bcd4)"},h={purple:"#9b59b6",light_blue:"#00bcd4"},p=f[m==null?void 0:m.color]||f.purple,x=h[m==null?void 0:m.color]||h.purple;return`<div class="gc-select-card" data-id="${r.id}"
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      <div style="flex:1;overflow-y:auto;display:flex;flex-wrap:wrap;gap:10px;justify-content:center;align-content:flex-start;padding:8px 16px 16px">
        ${o.map(h=>{const p=a.find(x=>x.gc_type===h.gc_type);return d(h,!!p)}).join("")}
      </div>
    ...
    </div>`,e.querySelectorAll(".gc-select-card").forEach(h=>{h.addEventListener("click",()=>{const p=h.dataset.id,x=o.find(v=>v.id===p);if(!x)return;const A=a.findIndex(v=>v.gc_type===x.gc_type);A>-1?a.splice(A,1):a.length<3&&a.push(x),n()})}),(u=e.querySelector("#gc-launch"))==null||u.addEventListener("click",()=>{r&&l(a)}),(m=e.querySelector("#gc-no-gc"))==null||m.addEventListener("click",()=>l([])),(f=e.querySelector("#gc-reset"))==null||f.addEventListener("click",()=>{a.length&&(a=[],n())})}n()}async function Wi(e,t,i){const{state:s,navigate:c}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await k.from("decks").select("id,name,is_active,formation").eq("owner_id",s.profile.id).order("created_at",{ascending:!1});if(!o||o.length===0){fe(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>c("decks"));return}const a=o.map(r=>r.id),{data:d}=await k.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order");let l=0;function n(){var v,g,w,F,M;const r=o[l],u=(d||[]).filter(y=>y.deck_id===r.id),m=u.filter(y=>{var I;return y.is_starter&&((I=y.card)==null?void 0:I.player)}).map(y=>Xe(y.card,y.position)),f=u.find(y=>{var I;return((I=y.card)==null?void 0:I.card_type)==="formation"}),h=r.formation||((v=f==null?void 0:f.card)==null?void 0:v.formation)||"4-4-2",p=m.length>=11?Oe(m,h):null,x=m.length>=11;Wt(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div style="display:flex;align-items:center;gap:8px;padding:8px;flex-shrink:0">
        ...
        <div style="flex:1;text-align:center">
          ...
          <div style="font-size:11px;opacity:.6;margin-top:2px">${h} · ${m.length}/11 ${r.is_active?"· ⭐ Actif":""}</div>
        </div>
      ...
      <div id="deck-swipe-zone" style="flex:1;min-height:0;overflow:hidden;position:relative;touch-action:pan-y;display:flex;align-items:center;justify-content:center;padding:4px">
        ${p?`<div class="deck-preview-wrap" style="aspect-ratio:1/1;max-width:100%;max-height:100%;width:auto;height:100%;overflow:hidden">${Me(p,h,null,[],285,285)}</div>`:`<div style="display:flex;align-items:center;justify-content:center;height:100%;opacity:.4;flex-direction:column;gap:8px">
      ...
      <div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">
        ${o.map((y,I)=>`<div style="width:7px;height:7px;border-radius:50%;background:${I===l?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}
      </div>`:""}
    ...
    </div>`,function(){const I=e.querySelector(".deck-preview-wrap svg");I&&(I.removeAttribute("width"),I.removeAttribute("height"),I.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",I.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),(g=document.getElementById("prev-deck"))==null||g.addEventListener("click",()=>{l>0&&(l--,n())}),(w=document.getElementById("next-deck"))==null||w.addEventListener("click",()=>{l<o.length-1&&(l++,n())}),(F=document.getElementById("validate-deck"))==null||F.addEventListener("click",()=>{x&&t.navigate("match",{matchMode:i,deckId:r.id})}),(M=document.getElementById("cancel-deck-select"))==null||M.addEventListener("click",()=>{le(e),c("home")});const A=document.getElementById("deck-swipe-zone");if(A){let y=0,I=0;A.addEventListener("touchstart",G=>{y=G.touches[0].clientX,I=G.touches[0].clientY},{passive:!0}),A.addEventListener("touchend",G=>{const R=G.changedTouches[0].clientX-y,P=G.changedTouches[0].clientY-I;Math.abs(R)<40||Math.abs(R)<Math.abs(P)||(R<0&&l<o.length-1?(l++,n()):R>0&&l>0&&(l--,n()))},{passive:!0})}}n()}function Xt(e){const t=(be==null?void 0:be.VITE_SUPABASE_URL)||"";return e!=null&&e.clubLogo?e.clubLogo.startsWith("http")?e.clubLogo:t?`${t}/storage/v1/object/public/assets/clubs/${e.clubLogo}`:null:null}function pe(e,t=44,i=58){if(!e)return`<div style="width:${t}px;height:${i}px;border:1.5px dashed rgba(255,255,255,0.2);border-radius:5px"></div>`;const s=typeof e.portrait=="string"&&e.portrait.startsWith("http")?e.portrait:ne(e),c=Xt(e),o=e._line||e.job||"MIL",a=ke[o]||"#555",d={normal:"#aaa",pépite:"#D4A017",papyte:"#222",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",l=e.note!==void 0?Number(e.note)||0:(Number(ie(e,o))||0)+(e.boost||0),n=Zt(e==null?void 0:e.country_code),r=Math.round(i*.19),u=Math.round(i*.25),m=i-Math.round(i*.19)-Math.round(i*.25),f=e!=null&&e.used?.28:1;return`<div style="width:${t}px;height:${i}px;border-radius:5px;border:2px solid ${d};background:${a};position:relative;overflow:hidden;flex-shrink:0;opacity:${f}">
    <div style="position:absolute;top:0;left:0;right:0;height:${r}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:center;z-index:2">
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;height:${u}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ${Ze(e==null?void 0:e.country_code)?`<img src="${Ze(e.country_code)}" style="width:${u+2}px;height:${u-3}px;object-fit:cover;border-radius:1px">`:`<span style="font-size:${u-4}px">${n}</span>`}
  ...
  </div>`}function Ie(e,t,i){if(!(e!=null&&e.length))return"";const s=e.slice(0,5);let c='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return s.forEach((o,a)=>{if(c+=pe(o,40,52),a<s.length-1){const d=ge(o,s[a+1]);c+=`<div style="width:7px;height:3px;background:${d==="#ff3333"||d==="#cc2222"?"rgba(255,255,255,0.12)":d};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(c+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),c+="</div>",c}function Ze(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function Zt(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function Le(e,t,i,s,c=310,o=310){const a=Ce[t]||{},d=Gt(t)||it[t]||[],l={},n=["ATT","MIL","DEF","GK"];for(const v of n)(e[v]||[]).forEach((w,F)=>{l[`${v}${F+1}`]=w});function r(v){const g=a[v];return g?{x:g.x*c,y:g.y*o}:null}let u="";for(const[v,g]of d){const w=r(v),F=r(g);if(!w||!F)continue;const M=l[v],y=l[g],I=ge(M,y);I==="#00ff88"||I==="#FFD700"?(u+=`<line x1="${w.x.toFixed(1)}" y1="${w.y.toFixed(1)}" x2="${F.x.toFixed(1)}" y2="${F.y.toFixed(1)}"
        stroke="${I}" stroke-width="10" stroke-linecap="round" opacity="0.22"/>`,u+=`<line x1="${w.x.toFixed(1)}" y1="${w.y.toFixed(1)}" x2="${F.x.toFixed(1)}" y2="${F.y.toFixed(1)}"
        stroke="${I}" stroke-width="3.5" stroke-linecap="round" opacity="0.95"/>`):u+=`<line x1="${w.x.toFixed(1)}" y1="${w.y.toFixed(1)}" x2="${F.x.toFixed(1)}" y2="${F.y.toFixed(1)}"
        stroke="${I}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const m=48,f=64,h=13,p=16,x={normal:"#aaaaaa",pépite:"#D4A017",papyte:"#111111",légende:"#7a28b8"};for(const[v,g]of Object.entries(l)){const w=r(v);if(!w||!g)continue;const F=v.replace(/[0-9]/g,""),M=ke[F]||"#555",y=i==="attack"&&["MIL","ATT"].includes(F)&&!g.used||i==="defense"&&["GK","DEF","MIL"].includes(F)&&!g.used,I=s.includes(g.cardId);let G;i==="attack"?G=F==="MIL"?Number(g.note_m)||0:Number(g.note_a)||0:i==="defense"?G=F==="GK"?Number(g.note_g)||0:F==="MIL"?Number(g.note_m)||0:Number(g.note_d)||0:G=Number(F==="GK"?g.note_g:F==="DEF"?g.note_d:F==="MIL"?g.note_m:g.note_a)||0,G=G+(g.boost||0);const R=(w.x-m/2).toFixed(1),P=(w.y-f/2).toFixed(1),H=g.used?.25:1,W=x[g==null?void 0:g.rarity]||x.normal,V=I?"#ff3030":W,ee=I?3:(g==null?void 0:g.rarity)==="légende"||(g==null?void 0:g.rarity)==="pépite"?2.5:2,z=f-h-p;u+=`<defs><clipPath id="cp-${v}"><rect x="${R}" y="${(w.y-f/2+h).toFixed(1)}" width="${m}" height="${z}"/></clipPath></defs>`,u+=`<rect x="${R}" y="${P}" width="${m}" height="${f}" rx="5" fill="${M}" opacity="${H}"/>`;const L=ne(g);L&&!g.used&&(u+=`<image href="${L}" x="${R}" y="${(w.y-f/2+h).toFixed(1)}" width="${m}" height="${z}" clip-path="url(#cp-${v})" preserveAspectRatio="xMidYMin slice"/>`),u+=`<rect x="${R}" y="${P}" width="${m}" height="${h}" rx="4" fill="rgba(255,255,255,0.92)"/>`,u+=`<text x="${w.x.toFixed(1)}" y="${(w.y-f/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(g.name||"").slice(0,9)}</text>`;const j=(w.y+f/2-p).toFixed(1);if(u+=`<rect x="${R}" y="${j}" width="${m}" height="${p}" fill="rgba(255,255,255,0.92)"/>`,g.used)u+=`<text x="${w.x.toFixed(1)}" y="${(w.y+f/2-p/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="13" font-weight="900" fill="rgba(0,0,0,0.4)" font-family="Arial Black">–</text>`;else{const N=Ze(g.country_code);N?u+=`<image href="${N}" x="${(w.x-20).toFixed(1)}" y="${(w.y+f/2-p+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:u+=`<text x="${(w.x-13).toFixed(1)}" y="${(w.y+f/2-p/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${Zt(g.country_code)}</text>`,u+=`<text x="${w.x.toFixed(1)}" y="${(w.y+f/2-p/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${G}</text>`;const C=Xt(g);C?u+=`<image href="${C}" x="${(w.x+m/2-14).toFixed(1)}" y="${(w.y+f/2-p+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:g.clubName&&(u+=`<text x="${(w.x+14).toFixed(1)}" y="${(w.y+f/2-p/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(g.clubName||"").slice(0,3).toUpperCase()}</text>`),g.boost&&(u+=`<rect x="${(w.x+m/2-12).toFixed(1)}" y="${(w.y-f/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,u+=`<text x="${(w.x+m/2-5).toFixed(1)}" y="${(w.y-f/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${g.boost}</text>`)}u+=`<rect x="${R}" y="${P}" width="${m}" height="${f}" rx="5" fill="${I?"rgba(255,255,255,0.12)":"none"}" stroke="${V}" stroke-width="${ee}" opacity="${H}"/>`,y&&(u+=`<rect x="${R}" y="${P}" width="${m}" height="${f}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${I?"selected":""}" data-card-id="${g.cardId}" data-role="${F}" style="cursor:pointer"/>`)}const A=38;return`<svg viewBox="${-A} ${-A} ${c+A*2} ${o+A*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${u}
  ...
  </div>`}async function Qt(e,t,i,s){var F;const{state:c,navigate:o,toast:a}=t;Wt(e);const d=c.params||{};if(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!d.deckId)return Wi(e,t,i);const l=d.deckId;let n,r,u,m;try{const M=await Promise.all([k.from("decks").select("formation,name").eq("id",l).single(),k.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation,
            player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
              clubs(encoded_name,logo_url)))`).eq("deck_id",l).order("slot_order")]);n=M[0].data,u=M[0].error,r=M[1].data,m=M[1].error}catch(M){console.error("[Match] Exception chargement deck:",M),fe(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>o("home"));return}if(u||m){console.error("[Match] Erreur Supabase:",u||m),fe(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>o("home"));return}const f=(r||[]).filter(M=>{var y;return M.is_starter&&((y=M.card)==null?void 0:y.player)}).map(M=>Xe(M.card,M.position)),h=(r||[]).filter(M=>{var y;return!M.is_starter&&((y=M.card)==null?void 0:y.player)}).map(M=>Xe(M.card,M.position));if(f.length<11){fe(e,"⚠️",`Deck incomplet (${f.length}/11).`,"Compléter",()=>o("decks"));return}const p=(r||[]).find(M=>{var y;return((y=M.card)==null?void 0:y.card_type)==="formation"}),x=(n==null?void 0:n.formation)||((F=p==null?void 0:p.card)==null?void 0:F.formation)||"4-4-2",{data:A,error:v}=await k.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",c.profile.id).eq("card_type","game_changer"),{data:g}=await k.from("gc_definitions").select("*").eq("is_active",!0),w=(A||[]).map(M=>({...M,_gcDef:(g==null?void 0:g.find(y=>y.name===M.gc_type||y.id===M.gc_definition_id))||null}));s({deckId:l,formation:x,starters:f,subsRaw:h,gcCardsEnriched:w,gcDefs:g||[]})}async function Ji(e,t){const{state:i}=t,c=(i.params||{}).matchMode||"vs_ai_easy",o=c.replace("vs_ai_",""),a=c;await Qt(e,t,c,async({deckId:d,formation:l,starters:n,subsRaw:r,gcCardsEnriched:u,gcDefs:m})=>{try{const f=Oe(n,l),h=await Xi(l,o),p=async x=>{try{const{data:A,error:v}=await k.from("matches").insert({home_id:i.profile.id,away_id:null,mode:a,home_deck_id:d,status:"in_progress"}).select().single();if(v){console.error("[MatchIA] Erreur création match:",v),fe(e,"⚠️","Impossible de créer le match ("+v.message+").","Retour",()=>t.navigate("home"));return}const g={gcDefs:m||[],matchId:A==null?void 0:A.id,mode:a,difficulty:o,formation:l,homeTeam:f,aiTeam:h,homeSubs:r,subsUsed:0,maxSubs:Math.min(r.length,3),homeScore:0,aiScore:0,gcCards:x,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Qi(e,g,t)}catch(A){console.error("[MatchIA] Exception launchMatch:",A),fe(e,"⚠️","Erreur au lancement du match : "+A.message,"Retour",()=>t.navigate("home"))}};if(!u.length){p([]);return}Jt(e,u,p)}catch(f){console.error("[MatchIA] Exception setup:",f),fe(e,"⚠️","Erreur de préparation du match : "+f.message,"Retour",()=>t.navigate("home"))}})}async function Xi(e,t){var a,d;const{data:i}=await k.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(60);if(!i||i.length<11)return Zi(e);const s=$e[e]||$e["4-4-2"],c={GK:[],DEF:[],MIL:[],ATT:[]},o=[...i];for(const l of["GK","DEF","MIL","ATT"]){const n=o.filter(h=>h.job===l),r=o.filter(h=>h.job!==l),u=[...n,...r],m=[];for(let h=0;h<s[l];h++){const p=u[h]||o[h];p&&m.push({cardId:"ai-"+p.id+"-"+h,id:p.id,firstname:p.firstname,name:p.surname_encoded,country_code:p.country_code,club_id:p.club_id,job:p.job,job2:p.job2,note_g:Number(p.note_g)||0,note_d:Number(p.note_d)||0,note_m:Number(p.note_m)||0,note_a:Number(p.note_a)||0,rarity:p.rarity,skin:p.skin,hair:p.hair,hair_length:p.hair_length,clubName:((a=p.clubs)==null?void 0:a.encoded_name)||null,clubLogo:((d=p.clubs)==null?void 0:d.logo_url)||null,boost:0,used:!1,_line:l})}const f=Pe(m.length);m.forEach((h,p)=>{h._col=f[p]}),c[l]=m}return c}function Zi(e){const t=$e[e]||$e["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},s=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let c=0;for(const o of["GK","DEF","MIL","ATT"]){const a=[];for(let l=0;l<t[o];l++){const n=3+Math.floor(Math.random()*5);a.push({cardId:"fake-"+c,id:"fake-"+c,firstname:"IA",name:s[c%s.length],country_code:"XX",club_id:null,job:o,job2:null,note_g:o==="GK"?n:2,note_d:o==="DEF"?n:2,note_m:o==="MIL"?n:2,note_a:o==="ATT"?n:2,rarity:"normal",boost:0,used:!1,_line:o}),c++}const d=Pe(a.length);a.forEach((l,n)=>{l._col=d[n]}),i[o]=a}return i}function Qi(e,t,i){e.innerHTML=`
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
  </div>`,setTimeout(()=>eo(e,t,i),5e3)}function eo(e,t,i){const s=t.homeTeam.MIL||[],c=t.aiTeam.MIL||[];function o(f){return f.reduce((h,p)=>h+ie(p,"MIL"),0)}function a(f){let h=0;for(let p=0;p<f.length-1;p++){const x=ge(f[p],f[p+1]);x==="#00ff88"?h+=2:x==="#FFD700"&&(h+=1)}return h}const d=o(s)+a(s),l=o(c)+a(c),n=d>=l;function r(f,h,p,x){return`<div style="text-align:center;width:100%">
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${f.map((A,v)=>{const g=v<f.length-1?ge(A,f[v+1]):null,w=!g||g==="#ff3333"||g==="#cc2222",F=g==="#00ff88"?"+2":g==="#FFD700"?"+1":"";return`
          <div class="duel-card duel-card-${x}" data-idx="${v}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease, transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
            ${pe({...A,note:ie(A,"MIL"),_line:"MIL"},58,78)}
          </div>
          ${v<f.length-1?`<div class="duel-link duel-link-${x}" data-idx="${v}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${w?"rgba(255,255,255,0.12)":g};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${w?"none":`0 0 8px ${g}`}">
            ${F?`<span style="position:absolute;top:-13px;left:50%;transform:translateX(-50%);font-size:8px;font-weight:900;color:${g}">${F}</span>`:""}
          </div>`:""}
          `}).join("")}
      </div>
      <div class="duel-score-line duel-score-line-${x}" style="margin-top:10px;font-size:12px;color:rgba(255,255,255,0.55);opacity:0;transition:opacity .4s ease">
        Score: ${o(f)} + ${a(f)} liens = <b style="color:#fff">${o(f)+a(f)}</b>
      </div>
    </div>`}e.innerHTML=`
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div id="midfield-result" style="opacity:0;text-align:center;transition:opacity 0.5s;color:#fff;max-width:320px"></div>
  </div>`;const u=()=>{const f=(h,p)=>e.querySelectorAll(h).forEach((x,A)=>{setTimeout(()=>{x.style.opacity="1",x.style.transform="translateY(0) scale(1)"},p+A*90)});f(".duel-card-home",150),f(".duel-card-ai",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((h,p)=>{setTimeout(()=>{h.style.opacity="1"},p*70)}),900),setTimeout(()=>{const h=e.querySelector("#vs-label");h&&(h.style.opacity="1",h.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(p=>p.style.opacity="1")},1250),setTimeout(()=>{m("score-home",d,800),m("score-ai",l,800)},1500)};function m(f,h,p){const x=document.getElementById(f);if(!x)return;const A=performance.now(),v=g=>{const w=Math.min(1,(g-A)/p);x.textContent=Math.round(h*(1-Math.pow(1-w,3))),w<1?requestAnimationFrame(v):x.textContent=h};requestAnimationFrame(v)}requestAnimationFrame(u),setTimeout(()=>{const f=document.getElementById("score-home"),h=document.getElementById("score-ai"),p=document.getElementById("midfield-result");if(f&&h&&(n?(f.style.fontSize="80px",f.style.color="#FFD700",f.style.animation="duelPulse .5s ease, duelGlow 1.5s ease infinite .5s",h.style.opacity="0.25"):(h.style.fontSize="80px",h.style.color="#ff6b6b",h.style.animation="duelPulse .5s ease",f.style.opacity="0.25")),p){const x=Yt();p.style.opacity="1",n&&t.clubName,p.innerHTML=`
      ...
      `,n&&(t.boostCard={value:x})}t.attacker=n?"home":"ai",t.log.push({type:"duel",title:"Milieu de Terrain",homePlayers:s.map(x=>({name:x.name,note:ie(x,"MIL"),portrait:ne(x),job:x.job,country_code:x.country_code,rarity:x.rarity,clubName:x.clubName,clubLogo:x.clubLogo})),aiPlayers:c.map(x=>({name:x.name,note:ie(x,"MIL"),portrait:ne(x),job:x.job,country_code:x.country_code,rarity:x.rarity,clubName:x.clubName,clubLogo:x.clubLogo})),homeTotal:d,aiTotal:l,text:`Duel milieu : ${t.clubName} ${d} – ${l} IA → ${n?t.clubName+" attaque":"IA attaque"}`}),setTimeout(()=>{var A,v;const x=(A=t.boostCard)==null?void 0:A.value;e.innerHTML=`
      ...
      </div>`,(v=document.getElementById("start-match-btn"))==null||v.addEventListener("click",()=>{t.phase=t.attacker==="home"?"attack":"ai-attack",oe(e,t,i),t.attacker==="ai"&&setTimeout(()=>Qe(e,t,i),800)})},100)},2800)}function to(e){if(e.type==="duel"){const t=e.homeTotal>=e.aiTotal;return`
    ...
    <div style="display:flex;align-items:center;gap:4px;${t?"flex-direction:row-reverse":""};background:rgba(255,255,255,0.04);border-radius:8px;padding:5px 8px;border:1px solid rgba(255,255,255,0.07)">
      ...
      ${Fe(e.inPlayer||{})}
    </div>`}return`<div style="font-size:11px;color:${e.type==="goal"?"#FFD700":"rgba(255,255,255,0.65)"};font-weight:${e.type==="goal"?700:400};padding:3px 2px">${e.text||""}</div>`}function oe(e,t,i){var p,x,A,v,g,w,F,M;const s=t.selected.map(y=>y.cardId),c=t.usedSubIds||[],o=t.homeSubs.filter(y=>!c.includes(y.cardId));Object.values(t.homeTeam).flat().filter(y=>y.used).length>0&&o.length>0&&t.subsUsed<t.maxSubs,t.log[t.log.length-1];const d=t.phase==="ai-attack"||t.phase==="ai-defense",l=t.phase==="attack",n=t.phase==="defense",r=t.phase==="finished",u=t.gcCards.filter(y=>!t.usedGc.includes(y.id)),m=t.boostCard&&!t.boostUsed;e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column",e.innerHTML=`
  <style>
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:100px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const I=t.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
            ...
            ${Ie((I.players||[]).map(G=>({...G,used:!1})),"#ff6b6b",I.total)}
            ...
            ${Ie((I.players||[]).map(G=>({...G,used:!1})),"#00ff88",I.total)}
    ...
    ${(()=>{const y=window.innerWidth>=700,I=(L,j,N)=>{var K,J;const C=(t.gcDefs||[]).find(te=>te.name===L.gc_type),$={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[C==null?void 0:C.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",b={purple:"#b06ce0",light_blue:"#00d4ef"}[C==null?void 0:C.color]||"#b06ce0",S=(C==null?void 0:C.name)||L.gc_type,E=(C==null?void 0:C.effect)||((K=de[L.gc_type])==null?void 0:K.desc)||"",_=C!=null&&C.image_url?`/manager-wars/icons/${C.image_url}`:null,D=((J=de[L.gc_type])==null?void 0:J.icon)||"⚡",T=Math.round(N*.22),q=Math.round(N*.22),B=N-T-q,U=S.length>12?7:9;return`<div class="gc-mini" data-gc-id="${L.id}" data-gc-type="${L.gc_type}"
          style="box-sizing:border-box;width:${j}px;height:${N}px;border-radius:10px;border:2px solid ${b};background:${$};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
          ...
          <div style="height:${B}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
            ${_?`<img src="${_}" style="max-width:${j-10}px;max-height:${B-6}px;object-fit:contain">`:`<span style="font-size:${Math.round(B*.55)}px">${D}</span>`}
          </div>
          ...
          </div>`},R=(L,j)=>j?G(110,150):I(L,110,150),P=(L,j)=>j?G(68,95):I(L,68,95),H=y?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",W=r?`<button id="btn-results" style="${H};background:linear-gradient(135deg,#D4A017,#FFD700);border:none;color:#000">🏁 Résultats</button>`:d?`<div style="${H};background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);color:rgba(255,255,255,0.4)">⏳ Tour IA</div>`:l?`<button id="btn-action" style="${H};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${t.selected.length===0?"disabled":""}> ⚔️ ATTAQUEZ <span id="match-timer" style="font-weight:900"></span></button>`:n?`<button id="btn-action" style="${H};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${t.selected.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="match-timer" style="font-weight:900"></span></button>`:`<div style="${H};background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1)"></div>`,V=l||n?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${t.selected.length}/3 sélectionné(s)</div>`:"",ee=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${y?56:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
        ${o.length===0?'<div style="font-size:7px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':o.map(L=>`<div class="sub-btn-col" data-sub-id="${L.cardId}" style="cursor:pointer;flex-shrink:0">${pe(L,44,58)}</div>`).join("")}
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(y=>{var I,G,R;if(y.type==="duel"){const P=y.isGoal,H=y.homeScored?"#FFD700":P?"#ff6b6b":"rgba(255,255,255,0.3)",W=y.homeScored?"⚽ BUT !":P?"⚽ BUT IA !":(I=y.homePlayers)!=null&&I.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${P?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${H};margin-bottom:4px">
                <div style="font-size:9px;color:${H};letter-spacing:1px;margin-bottom:5px;font-weight:700;text-transform:uppercase">${W}</div>
                ${(G=y.homePlayers)!=null&&G.length?`<div style="margin-bottom:3px">${Ie(y.homePlayers,"rgba(255,255,255,0.7)",y.homeTotal)}</div>`:""}
                ${(R=y.aiPlayers)!=null&&R.length?`<div style="opacity:0.7">${Ie(y.aiPlayers,"#ff6b6b",y.aiTotal)}</div>`:""}
  ...
  </div>`;function f(){const y=e.querySelector(".match-screen");if(!y)return;const I=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);y.style.bottom="auto",y.style.height=I+"px",y.style.minHeight=I+"px",y.style.maxHeight=I+"px",y.style.overflow="hidden";const G=e.querySelector("#mobile-action-bar"),R=e.querySelector("#mobile-play-area");G&&R&&(R.style.paddingBottom=G.offsetHeight+"px")}if(f(),setTimeout(f,120),setTimeout(f,400),setTimeout(f,1e3),t._vvBound||(t._vvBound=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",f),window.visualViewport.addEventListener("scroll",f)),window.addEventListener("resize",f)),function(){const I=e.querySelector(".terrain-wrapper svg");I&&(I.removeAttribute("width"),I.removeAttribute("height"),I.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",I.setAttribute("viewBox","-26 -26 352 352"),I.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),t._resizeBound||(t._resizeBound=!0,window.addEventListener("resize",()=>{const y=e.querySelector(".terrain-wrapper svg");y&&(y.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0")})),t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase==="attack"||t.phase==="defense"){let y=!1,I=30;const G=()=>document.getElementById("match-timer"),R=()=>{const P=G();if(!P)return;const H=String(Math.floor(I/60)).padStart(2,"0"),W=String(I%60).padStart(2,"0");P.textContent=` ${H}:${W}`,P.style.color=y?"#ff2222":"#ff9500",P.style.fontWeight="900"};R(),t._timerInt=setInterval(()=>{if(I--,I<0)if(!y)y=!0,I=15,R();else{clearInterval(t._timerInt),t._timerInt=null,t.homeScore=0,t.aiScore=3;const P=document.createElement("div");P.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;color:#fff;padding:24px;text-align:center",P.innerHTML='<div style="font-size:56px">⏱️</div><div style="font-size:24px;font-weight:900;color:#ff4444">MATCH PERDU PAR FORFAIT</div><div style="font-size:14px;color:rgba(255,255,255,0.6)">Temps écoulé</div>',document.body.appendChild(P),setTimeout(()=>{P.remove(),we(e,t,i)},2500)}else R()},1e3)}(p=document.getElementById("match-quit"))==null||p.addEventListener("click",()=>{le(e),confirm("Abandonner ? Résultat : défaite 3-0")&&(t.homeScore=0,t.aiScore=3,we(e,t,i))}),(x=document.getElementById("view-ai"))==null||x.addEventListener("click",()=>po(t,i)),(A=document.getElementById("toggle-history"))==null||A.addEventListener("click",()=>{var y;(y=document.getElementById("match-history-panel"))==null||y.classList.add("open")}),(v=document.getElementById("close-history"))==null||v.addEventListener("click",()=>{var y;(y=document.getElementById("match-history-panel"))==null||y.classList.remove("open")}),(g=document.getElementById("btn-action"))==null||g.addEventListener("click",()=>{t.selected.length!==0&&(l?oo(e,t,i):n&&no(e,t,i))}),(w=document.getElementById("btn-results"))==null||w.addEventListener("click",()=>we(e,t,i)),e.querySelectorAll(".match-slot-hit").forEach(y=>{y.addEventListener("click",()=>io(y,t,e,i))}),e.querySelectorAll(".gc-mini").forEach(y=>{y.addEventListener("click",()=>so(y.dataset.gcId,y.dataset.gcType,e,t,i))}),(F=document.getElementById("boost-card"))==null||F.addEventListener("click",()=>co(e,t,i)),e.querySelectorAll(".sub-btn-col").forEach(y=>{y.addEventListener("click",()=>jt(e,t,i,y.dataset.subId))}),(M=document.getElementById("sub-btn-main"))==null||M.addEventListener("click",()=>jt(e,t,i))}function io(e,t,i,s){const c=e.dataset.cardId,o=e.dataset.role,a=t.selected.findIndex(d=>d.cardId===c);if(a!==-1)t.selected.splice(a,1);else{if(t.selected.length>=3){s.toast("Maximum 3 joueurs","error");return}const d=(t.homeTeam[o]||[]).find(l=>l.cardId===c);d&&t.selected.push({...d,_role:o,_line:o})}oe(i,t,s)}function dt(e,t,i){e.matchId&&k.from("matches").update({last_player_id:i}).eq("id",e.matchId).then(()=>{})}function oo(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),dt(t,i,i.state.profile.id);const s=t.selected.map(o=>({...o,_line:o._role})),c=ot(s,t.modifiers.home);t.pendingAttack={...c,players:[...t.selected],side:"home"},t.selected.forEach(o=>{const a=(t.homeTeam[o._role]||[]).find(d=>d.cardId===o.cardId);a&&(a.used=!0)}),t.log.push({text:`⚔️ Vous attaquez : ${c.total} (base ${c.base}${c.links?` +${c.links} liens`:""}) — ${t.selected.map(o=>o.name).join(", ")}`,type:"info"}),t.selected=[],t.modifiers.home={},t.phase="ai-defense",oe(e,t,i),setTimeout(()=>ro(e,t,i),1200)}function no(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),dt(t,i,i.state.profile.id);const s=t.selected.map(d=>({...d,_line:d._role})),c=nt(s,t.modifiers.home);t.selected.forEach(d=>{const l=(t.homeTeam[d._role]||[]).find(n=>n.cardId===d.cardId);l&&(l.used=!0)});const o=rt(t.pendingAttack.total,c.total,t.modifiers.home),a={type:"duel",title:"Défense",aiPlayers:(t.pendingAttack.players||[]).map(d=>({name:d.name,note:d._line==="MIL"?d.note_m:d.note_a,portrait:ne(d),job:d.job,country_code:d.country_code,rarity:d.rarity,clubName:d.clubName,clubLogo:d.clubLogo})),homePlayers:t.selected.map(d=>{const l=(t.homeTeam[d._role]||[]).find(n=>n.cardId===d.cardId)||d;return{name:l.name,note:l._line==="GK"?l.note_g:l._line==="MIL"?l.note_m:l.note_d,portrait:ne(l),job:l.job,country_code:l.country_code,rarity:l.rarity,clubName:l.clubName,clubLogo:l.clubLogo}}),homeTotal:c.total,aiTotal:t.pendingAttack.total,isGoal:!1,homeScored:!1,text:""};if(o.shielded)a.text="🛡️ Bouclier ! But annulé.",t.log.push(a);else if(o.goal){t.aiScore++,a.isGoal=!0,a.homeScored=!1,a.text=`⚽ BUT IA ! (${t.pendingAttack.total} > ${c.total})`,t.log.push(a),t.selected=[],t.modifiers.home={},t.pendingAttack=null,oe(e,t,i),lt(a.aiPlayers,t.homeScore,t.aiScore,!1,()=>{ze(e,t,i,"home-attack")});return}else a.text=`🧤 Défense réussie ! (${c.total} ≥ ${t.pendingAttack.total})`,t.log.push(a);t.selected=[],t.modifiers.home={},t.pendingAttack=null,ze(e,t,i,"home-attack")}function Qe(e,t,i){dt(t,i,null);const s=[...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]],c=Rt(s,"attack",t.difficulty);if(!c.length){ti(e,t,i);return}const o=ot(c,t.modifiers.ai);t.pendingAttack={...o,players:c,side:"ai"},c.forEach(n=>{n.used=!0}),t.log.push({text:`🤖 IA attaque : ${o.total} (${c.map(n=>n.name).join(", ")})`,type:"info"}),t.modifiers.ai={};const a=[...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(n=>!n.used),l=(t.homeSubs||[]).filter(n=>!(t.usedSubIds||[]).includes(n.cardId)).length>0&&t.subsUsed<t.maxSubs;if(a.length===0&&!l){t.aiScore++;const n={type:"duel",isGoal:!0,homeScored:!1,aiPlayers:c.map(r=>({name:r.name,note:r._line==="MIL"?r.note_m:r.note_a,portrait:ne(r),job:r.job,country_code:r.country_code,rarity:r.rarity,clubName:r.clubName,clubLogo:r.clubLogo})),aiTotal:o.total,text:"⚽ BUT IA ! (aucun défenseur disponible)"};t.log.push(n),t.pendingAttack=null,oe(e,t,i),lt(n.aiPlayers,t.homeScore,t.aiScore,!1,()=>{ze(e,t,i,"home-attack")});return}t.phase="defense",oe(e,t,i)}function ro(e,t,i){const s=[...t.aiTeam.GK||[],...t.aiTeam.DEF||[],...t.aiTeam.MIL||[]],c=Rt(s,"defense",t.difficulty),o=c.length>0?nt(c,t.modifiers.ai).total:0;c.forEach(l=>{l.used=!0});const a=rt(t.pendingAttack.total,o,t.modifiers.ai),d={type:"duel",title:"Attaque",homePlayers:(t.pendingAttack.players||[]).map(l=>({name:l.name,note:l._line==="MIL"?l.note_m:l.note_a,portrait:ne(l),job:l.job,country_code:l.country_code,rarity:l.rarity,clubName:l.clubName,clubLogo:l.clubLogo})),aiPlayers:c.map(l=>({name:l.name,note:l._line==="GK"?l.note_g:l._line==="MIL"?l.note_m:l.note_d,portrait:ne(l),job:l.job,country_code:l.country_code,rarity:l.rarity,clubName:l.clubName,clubLogo:l.clubLogo})),homeTotal:t.pendingAttack.total,aiTotal:o,isGoal:!1,homeScored:!1,text:""};if(a.shielded)d.text="🛡️ Bouclier IA !",t.log.push(d);else if(a.goal){t.homeScore++,d.isGoal=!0,d.homeScored=!0,d.text=`⚽ BUT ! (${t.pendingAttack.total} > ${o})`,t.log.push(d),t.modifiers.ai={},t.pendingAttack=null,oe(e,t,i),lt(d.homePlayers,t.homeScore,t.aiScore,!0,()=>{ze(e,t,i,"ai-attack")});return}else d.text=`🧤 IA défend (${o} ≥ ${t.pendingAttack.total})`,t.log.push(d);t.modifiers.ai={},t.pendingAttack=null,ze(e,t,i,"ai-attack")}function ze(e,t,i,s){if(t.round++,ei(t)){we(e,t,i);return}if(s==="home-attack"){if(![...t.homeTeam.MIL||[],...t.homeTeam.ATT||[]].filter(o=>!o.used).length){if(![...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(a=>!a.used).length){we(e,t,i);return}t.phase="ai-attack",oe(e,t,i),setTimeout(()=>Qe(e,t,i),800);return}t.phase="attack",oe(e,t,i)}else{if(![...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]].filter(o=>!o.used).length){ti(e,t,i);return}t.phase="ai-attack",oe(e,t,i),setTimeout(()=>Qe(e,t,i),800)}}function ei(e){const t=["MIL","ATT","GK","DEF"].some(s=>(e.homeTeam[s]||[]).some(c=>!c.used)),i=["MIL","ATT","GK","DEF"].some(s=>(e.aiTeam[s]||[]).some(c=>!c.used));return!t&&!i}function ti(e,t,i){ei(t)?we(e,t,i):(t.phase="attack",oe(e,t,i))}function ao(e,t,i){const s=document.createElement("div");s.style.cssText=`
    position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:800;
    display:flex;flex-direction:column;align-items:center;justify-content:center;gap:20px;
    animation:subFadeIn 0.2s ease;
  `;const c=ne(e),o=ne(t),a=ke[e.job]||"#555",d=ke[t.job]||"#555",l=e.job==="GK"?e.note_g:e.job==="DEF"?e.note_d:e.job==="MIL"?e.note_m:e.note_a,n=t.job==="GK"?t.note_g:t.job==="DEF"?t.note_d:t.job==="MIL"?t.note_m:t.note_a;s.innerHTML=`
    <style>
  ...
  `,document.body.appendChild(s);let r=!1;const u=()=>{r||(r=!0,s.remove(),i())};s.addEventListener("click",u),setTimeout(u,2e3)}function Ae(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function jt(e,t,i,s=null){if(t.phase!=="attack"){Ae("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){Ae(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const c=Object.values(t.homeTeam).flat().filter(u=>u.used),o=t.homeSubs.filter(u=>!t.usedSubIds.includes(u.cardId));if(!c.length){Ae("Aucun joueur utilisé à remplacer");return}if(!o.length){Ae("Aucun remplaçant disponible");return}let a=0,d=Math.max(0,o.findIndex(u=>u.cardId===s)),l=!1;const n=document.createElement("div");n.id="sub-overlay",n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function r(){var A,v,g,w,F,M;const u=c[a],m=o[d],f=Math.min(130,Math.round((window.innerWidth-90)/2)),h=Math.round(f*1.35),p=y=>`background:rgba(255,255,255,0.12);border:none;color:${y?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${y?"default":"pointer"};flex-shrink:0`;n.innerHTML=`
    ...
    <div style="flex:1;display:flex;gap:0;overflow:hidden">
      ...
      <div id="in-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
        ...
        <div>${m?pe({...m,used:!1,boost:0},f,h):"<div>—</div>"}</div>
      ...
      <div id="out-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
        ...
        <div>${u?pe({...u,used:!1,boost:0},f,h):"<div>—</div>"}</div>
    ...
    </div>`,(A=n.querySelector("#sub-close"))==null||A.addEventListener("click",()=>n.remove()),(v=n.querySelector("#out-up"))==null||v.addEventListener("click",()=>{a>0&&(a--,r())}),(g=n.querySelector("#out-down"))==null||g.addEventListener("click",()=>{a<c.length-1&&(a++,r())}),(w=n.querySelector("#in-up"))==null||w.addEventListener("click",()=>{d>0&&(d--,r())}),(F=n.querySelector("#in-down"))==null||F.addEventListener("click",()=>{d<o.length-1&&(d++,r())});const x=(y,I,G,R)=>{const P=n.querySelector("#"+y);if(!P)return;let H=0;P.addEventListener("touchstart",W=>{H=W.touches[0].clientY},{passive:!0}),P.addEventListener("touchend",W=>{const V=W.changedTouches[0].clientY-H;if(Math.abs(V)<30)return;const ee=I();V<0&&ee<R-1?(G(ee+1),r()):V>0&&ee>0&&(G(ee-1),r())},{passive:!0})};x("in-panel",()=>d,y=>d=y,o.length),x("out-panel",()=>a,y=>a=y,c.length),(M=n.querySelector("#sub-confirm"))==null||M.addEventListener("click",y=>{if(y.preventDefault(),y.stopPropagation(),l)return;l=!0;const I=c[a],G=o[d];if(!I||!G)return;let R=null,P=-1;for(const[W,V]of Object.entries(t.homeTeam)){const ee=(V||[]).findIndex(z=>z.cardId===I.cardId);if(ee!==-1){R=W,P=ee;break}}if(P===-1||!R){Ae("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),n.remove();return}const H={...G,_line:R,_col:I._col||0,used:!1,boost:0};t.homeTeam[R].splice(P,1,H),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(G.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:I.name,firstname:I.firstname,note:ie(I,R),portrait:ne(I),job:I.job,country_code:I.country_code,rarity:I.rarity,clubName:I.clubName,clubLogo:I.clubLogo},inPlayer:{name:G.name,firstname:G.firstname,note:ie(G,R),portrait:ne(G),job:G.job,country_code:G.country_code,rarity:G.rarity,clubName:G.clubName,clubLogo:G.clubLogo},text:`🔄 ${G.firstname} ${G.name} remplace ${I.firstname} ${I.name}`}),n.remove(),ao(I,G,()=>oe(e,t,i))})}document.body.appendChild(n),r()}function so(e,t,i,s,c){var h,p;const o=(s.gcDefs||[]).find(x=>x.name===t),a=de[t]||{icon:"⚡",desc:"Carte spéciale."},d={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[o==null?void 0:o.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",l={purple:"#b06ce0",light_blue:"#00d4ef"}[o==null?void 0:o.color]||"#b06ce0",n=(o==null?void 0:o.name)||t,r=(o==null?void 0:o.effect)||a.desc,u=o!=null&&o.image_url?`/manager-wars/icons/${o.image_url}`:null,m=a.icon||"⚡",f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",f.innerHTML=`
    ...
    </div>`,document.body.appendChild(f),(h=f.querySelector("#gc-back"))==null||h.addEventListener("click",()=>f.remove()),(p=f.querySelector("#gc-use"))==null||p.addEventListener("click",()=>{f.remove(),lo(e,t,i,s,c)})}function lo(e,t,i,s,c){if(s.usedGc.includes(e))return;s.usedGc.push(e);const o=(s.gcDefs||[]).find(d=>d.name===t);let a=!1;if(o!=null&&o.effect_type&&o.effect_type!=="CUSTOM"){const d=GC_ENGINE[o.effect_type];d?d(o.effect_params||{},s,i,c)||(a=!0):(c.toast(`Effet "${o.effect_type}" non implémenté`,"error"),a=!0)}else{switch(t){case"Double attaque":s.modifiers.home.doubleAttack=!0,s.log.push({text:"⚡ Double attaque activée !",type:"info"});break;case"Bouclier":s.modifiers.home.shield=!0,s.log.push({text:"🛡️ Bouclier activé !",type:"info"});break;case"Ressusciter":{const d=Object.entries(s.homeTeam).flatMap(([l,n])=>(n||[]).filter(r=>r.used).map(r=>({...r,_line:l})));d.length?(d[0].used=!1,s.log.push({text:`💫 ${d[0].name} ressuscité !`,type:"info"})):s.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"});break}case"Vol de note":s.modifiers.ai.stolenNote=(s.modifiers.ai.stolenNote||0)+1,s.log.push({text:"🎯 -1 à la prochaine attaque IA",type:"info"});break;case"Gel":{const d=[...s.aiTeam.ATT||[],...s.aiTeam.MIL||[]].filter(l=>!l.used);if(d.length){const l=d.sort((n,r)=>ie(r,"ATT")-ie(n,"ATT"))[0];l.used=!0,s.log.push({text:`❄️ ${l.name} (IA) gelé !`,type:"info"})}break}case"Remplacement+":s.maxSubs++,s.log.push({text:"🔄 +1 remplacement débloqué",type:"info"});break}a=!0}k.from("cards").delete().eq("id",e).then(()=>{}),a&&oe(i,s,c)}function co(e,t,i){const s=Object.values(t.homeTeam).flat().filter(c=>!c.used);if(!s.length){i.toast("Aucun joueur actif à booster","error");return}i.openModal("⚡ Utiliser le Boost",`<div style="margin-bottom:12px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border-radius:10px;padding:12px;text-align:center;color:#000">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(c=>{c.addEventListener("click",()=>{const o=c.dataset.cardId;for(const a of["GK","DEF","MIL","ATT"]){const d=(t.homeTeam[a]||[]).find(l=>l.cardId===o);if(d){d.boost=(d.boost||0)+t.boostCard.value,t.log.push({text:`⚡ Boost +${t.boostCard.value} appliqué à ${d.name}`,type:"info"});break}}t.boostUsed=!0,i.closeModal(),oe(e,t,i)})})}function lt(e,t,i,s,c){const o=document.createElement("div");o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:900;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;overflow:hidden;cursor:pointer";const a=Array.from({length:10},(n,r)=>`
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(o);let d=!1;const l=()=>{d||(d=!0,o.remove(),setTimeout(()=>c(),50))};o.addEventListener("click",l),setTimeout(l,3500)}async function we(e,t,i){var r,u;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:s}=i,c=t.homeScore>t.aiScore,o=t.homeScore===t.aiScore,a=c?"victoire":o?"nul":"defaite",d=fi(t.mode,a);t.matchId&&await k.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:c?s.profile.id:null,home_credits_reward:d,played_at:new Date().toISOString()}).eq("id",t.matchId);const l={credits:(s.profile.credits||0)+d,matches_played:(s.profile.matches_played||0)+1};c?l.wins=(s.profile.wins||0)+1:o?l.draws=(s.profile.draws||0)+1:l.losses=(s.profile.losses||0)+1,await k.from("users").update(l).eq("id",s.profile.id),await i.refreshProfile();const n=document.createElement("div");n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",n.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const f=()=>document.getElementById("mm-status"),h=async(g=!0)=>{u=!0,m&&(k.removeChannel(m),m=null),g&&await k.rpc("cancel_matchmaking",{p_user_id:l.profile.id}).catch(()=>{})};(v=document.getElementById("mm-cancel"))==null||v.addEventListener("click",async()=>{try{await h(!0)}catch(g){console.warn("[Matchmaking] cleanup error:",g)}le(e),n("home")});const p=async(g,w,F)=>{u||(u=!0,m&&(k.removeChannel(m),m=null),f()&&(f().textContent="Adversaire trouvé !"),await new Promise(M=>setTimeout(M,600)),e.isConnected!==!1&&go(e,t,g,F,a,d))},{data:x,error:A}=await k.rpc("try_matchmake",{p_user_id:l.profile.id,p_deck_id:i});if(A||!(x!=null&&x.success)){r("Erreur de matchmaking","error"),le(e),n("home");return}if(x.matched){p(x.match_id,x.opponent_id,!1);return}f()&&(f().textContent="En attente d'un autre joueur..."),m=k.channel("matchmaking-"+l.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${l.profile.id}`},g=>{const w=g.new;w.status==="matched"&&w.match_id&&p(w.match_id,w.matched_with,!0)}).subscribe()}async function go(e,t,i,s,c=[],o=[]){const{state:a,navigate:d,toast:l}=t,n=s?"p1":"p2",r=s?"p2":"p1",u=(c||[]).map(z=>z.id),m=(c||[]).map(z=>({id:z.id,gc_type:z.gc_type,_gcDef:z._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation du match...</div>';const{data:f}=await k.from("matches").select("*").eq("id",i).single();if(!f){l("Match introuvable","error"),d("home");return}async function h(){const[{data:z,error:L},{data:j,error:N},{data:C},{data:$}]=await Promise.all([k.rpc("get_deck_for_match",{p_deck_id:f.home_deck_id}),k.rpc("get_deck_for_match",{p_deck_id:f.away_deck_id}),k.from("users").select("id,pseudo,club_name").eq("id",f.home_id).single(),k.from("users").select("id,pseudo,club_name").eq("id",f.away_id).single()]);L&&console.error("[PvP] get_deck_for_match p1 error:",L),N&&console.error("[PvP] get_deck_for_match p2 error:",N);function b(B,U){return{cardId:B.card_id,position:U||B.position,id:B.id,firstname:B.firstname,name:B.surname_encoded,country_code:B.country_code,club_id:B.club_id,job:B.job,job2:B.job2,note_g:Number(B.note_g)||0,note_d:Number(B.note_d)||0,note_m:Number(B.note_m)||0,note_a:Number(B.note_a)||0,rarity:B.rarity,skin:B.skin,hair:B.hair,hair_length:B.hair_length,clubName:B.club_encoded_name||null,clubLogo:B.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}}const S=((z==null?void 0:z.starters)||[]).map(B=>b(B,B.position)),E=((j==null?void 0:j.starters)||[]).map(B=>b(B,B.position)),_=((z==null?void 0:z.subs)||[]).map(B=>b(B,B.position)),D=((j==null?void 0:j.subs)||[]).map(B=>b(B,B.position)),T=(z==null?void 0:z.formation)||"4-4-2",q=(j==null?void 0:j.formation)||"4-4-2";return console.log("[PvP] p1Formation:",T,"p1 starters:",S.length,"positions:",S.map(B=>B.position)),console.log("[PvP] p2Formation:",q,"p2 starters:",E.length,"positions:",E.map(B=>B.position)),{p1Team:Oe(S,T),p2Team:Oe(E,q),p1Subs:_,p2Subs:D,p1Formation:T,p2Formation:q,p1Name:(C==null?void 0:C.club_name)||(C==null?void 0:C.pseudo)||"Joueur 1",p2Name:($==null?void 0:$.club_name)||($==null?void 0:$.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:s?u:[],gc_p2:s?[]:u,gcCardsFull_p1:s?m:[],gcCardsFull_p2:s?[]:m,gcReady_p1:!0,gcReady_p2:!0,usedGc_p1:[],usedGc_p2:[],lastActionAt:new Date().toISOString()}}let p=f.game_state&&Object.keys(f.game_state).length?f.game_state:null;if(!p)if(s){p=await h();const{data:z}=await k.from("matches").select("game_state").eq("id",i).single();!(z!=null&&z.game_state)||!Object.keys(z.game_state).length?await k.from("matches").update({game_state:p,turn_user_id:f.home_id}).eq("id",i):p=z.game_state}else{e.innerHTML=`<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation avec l'adversaire...</div>`;for(let z=0;z<30&&!p;z++){await new Promise(j=>setTimeout(j,400));const{data:L}=await k.from("matches").select("game_state").eq("id",i).single();L!=null&&L.game_state&&Object.keys(L.game_state).length&&(p=L.game_state)}if(!p){l("Erreur de synchronisation","error"),d("home");return}p.gc_p2=u,p.gcCardsFull_p2=m,await k.from("matches").update({game_state:p}).eq("id",i)}let x=!1;function A(z){var S;try{k.removeChannel(v)}catch{}const L=a.profile.id,j=z.winner_id===L,N=!!z.forfeit,C=p[n+"Score"]??0,$=p[r+"Score"]??0,b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center",b.innerHTML=`
      <div style="font-size:64px">${j?"🏆":"😞"}</div>
      <div style="font-size:26px;font-weight:900;color:${j?"#FFD700":"#ff6b6b"}">${j?"VICTOIRE !":"DÉFAITE"}</div>
      <div style="font-size:18px;color:#fff">${p[n+"Name"]} ${C} – ${$} ${p[r+"Name"]}</div>
      ${N?`<div style="font-size:13px;color:rgba(255,255,255,0.5)">${j?"L'adversaire a quitté la partie":"Match perdu par forfait"}</div>`:""}
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(b),(S=b.querySelector("#pvp-end-home"))==null||S.addEventListener("click",()=>{b.remove(),le(e),d("home")})}const v=k.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},z=>{const L=z.new;try{if(L.status==="finished"||L.forfeit){if(x)return;x=!0,p._timerInt&&(clearInterval(p._timerInt),p._timerInt=null),L.game_state&&(p=L.game_state),A(L);return}L.game_state&&(p=L.game_state,F())}catch(j){console.error("[PvP] Realtime render crash:",j,"gameState:",p)}}).subscribe();async function g(z){Object.assign(p,z),p.lastActionAt=new Date().toISOString();const{error:L}=await k.from("matches").update({game_state:p}).eq("id",i);L&&(console.error("[PvP] pushState DB error:",L.message,L),l("Erreur de synchronisation : "+L.message,"error"));try{F()}catch(j){console.error("[PvP] renderPvpScreen crash après pushState:",j),l("Erreur d'affichage : "+j.message,"error")}}async function w(){if(x)return;x=!0,p._timerInt&&(clearInterval(p._timerInt),p._timerInt=null);const z=s?f.away_id:f.home_id;try{await k.from("matches").update({status:"finished",forfeit:!0,winner_id:z}).eq("id",i)}catch(L){console.warn("[PvP] forfeit DB error:",L)}try{k.removeChannel(v)}catch{}le(e),d("home")}function F(){var gt,mt,xt,yt,ht;const z=p[n+"Team"];p[r+"Team"];const L=p[n+"Score"],j=p[r+"Score"],N=p[n+"Name"],C=p[r+"Name"];if(p.phase==="gc-select")return M();if(p.phase==="reveal")return y();if(p.phase==="midfield")return I();if(p.phase==="finished")return W();const $=p.phase===n+"-attack",b=p.phase===n+"-defense",S=p.phase===r+"-attack"||p.phase===r+"-defense",E=Array.isArray(p["selected_"+n])?p["selected_"+n]:[],_=E.map(O=>O.cardId);e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column";const D=window.innerWidth>=700,T=p[n+"Subs"]||[],q=p["usedSubIds_"+n]||[],B=T.filter(O=>!q.includes(O.cardId)),U=p["gcCardsFull_"+n]||[],K=p["usedGc_"+n]||[],J=U.filter(O=>!K.includes(O.id)),te=p.boostOwner===n&&!p.boostUsed,re=(O,X,Q)=>{var _t,$t;const Y=O._gcDef,ce={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[Y==null?void 0:Y.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",je={purple:"#b06ce0",light_blue:"#00d4ef"}[Y==null?void 0:Y.color]||"#b06ce0",ye=(Y==null?void 0:Y.name)||O.gc_type,ri=(Y==null?void 0:Y.effect)||((_t=de[O.gc_type])==null?void 0:_t.desc)||"",vt=Y!=null&&Y.image_url?`/manager-wars/icons/${Y.image_url}`:null,ai=(($t=de[O.gc_type])==null?void 0:$t.icon)||"⚡",bt=Math.round(Q*.22),wt=Math.round(Q*.22),Ve=Q-bt-wt,si=ye.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${O.id}" data-gc-type="${O.gc_type}"
      ...
      <div class="terrain-wrapper" style="width:100%;height:100%;overflow:hidden">
        ...
        </div>`;const X=Array.isArray(p.log)?p.log:[],Q=X[X.length-1];return Q?`<div style="padding:7px 10px;border-left:3px solid ${Q.type==="goal"?"#FFD700":Q.type==="stop"?"#00ff88":Q.type==="attack"?"#ff6b6b":"rgba(255,255,255,0.6)"};font-size:12px;color:#fff">${Q.text||""}</div>`:'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})(),ni=(Array.isArray(p.log)?p.log:[]).length,ft=`
      <!-- Score -->
      ...
      <div style="text-align:center;padding:4px;background:rgba(0,0,0,0.15);font-size:11px;color:${S?"rgba(255,255,255,0.4)":"#FFD700"};font-weight:700;flex-shrink:0">
        ${S?`⏳ Tour de ${C}`:$?"⚔️ À vous d'attaquer !":b?"🛡️ À vous de défendre !":""}
      </div>
      <!-- Zone actions -->
      ...
      </div>`;const O=e.querySelector(".terrain-wrapper");O&&(O.innerHTML=Me(z,p[n+"Formation"],$?"attack":b?"defense":"idle",_,300,300))}if(e.querySelectorAll(".match-slot-hit").forEach(O=>{O.addEventListener("click",()=>{if(!$&&!b)return;const X=O.dataset.cardId,Q=O.dataset.role,Y=(z[Q]||[]).find(ye=>ye.cardId===X);if(!Y||Y.used)return;Array.isArray(p["selected_"+n])||(p["selected_"+n]=[]);const ce=p["selected_"+n],je=ce.findIndex(ye=>ye.cardId===X);je>-1?ce.splice(je,1):ce.length<3&&ce.push({...Y,_role:Q}),F()})}),(gt=e.querySelector("#pvp-boost-card"))==null||gt.addEventListener("click",()=>{G()}),e.querySelectorAll(".pvp-gc-mini").forEach(O=>{O.addEventListener("click",()=>{H(O.dataset.gcId,O.dataset.gcType)})}),(mt=e.querySelector("#pvp-action"))==null||mt.addEventListener("click",()=>{$?V():b&&ee()}),(xt=e.querySelector("#pvp-quit"))==null||xt.addEventListener("click",()=>{confirm("Quitter le match ? Vous perdrez par forfait.")&&w()}),(yt=e.querySelector("#pvp-view-opp"))==null||yt.addEventListener("click",()=>{P()}),(ht=e.querySelector("#pvp-toggle-history"))==null||ht.addEventListener("click",()=>{R()}),function(){const X=e.querySelector(".terrain-wrapper svg");X&&(X.removeAttribute("width"),X.removeAttribute("height"),X.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",X.setAttribute("viewBox","-26 -26 352 352"),X.setAttribute("preserveAspectRatio","xMidYMid meet"));const Q=e.querySelector(".match-screen");if(Q){const Y=e.clientHeight;Y>50&&(Q.style.height=Y+"px",Q.style.maxHeight=Y+"px",Q.style.overflow="hidden")}}(),p._timerInt&&(clearInterval(p._timerInt),p._timerInt=null),($||b)&&!x){let O=30,X=!1;const Q=()=>document.getElementById("pvp-timer"),Y=()=>{Q()&&(Q().textContent=O+"s",Q().style.color=X?"#ff4444":"#fff")};Y(),p._timerInt=setInterval(()=>{O--,O<0?X?(clearInterval(p._timerInt),p._timerInt=null,w()):(X=!0,O=15,Y()):Y()},1e3)}}function M(){var b;const z=p["gcReady_"+n],L=p["gcReady_"+r];Array.isArray(p["gc_"+n])||(p["gc_"+n]=[]);let j=p["gc_"+n];const N=3;function C(S,E){const _=S._gcDef,D={purple:"linear-gradient(135deg,#3d0a7a,#7a28b8)",light_blue:"linear-gradient(135deg,#006080,#00bcd4)"},T={purple:"#9b59b6",light_blue:"#00bcd4"},q=D[_==null?void 0:_.color]||D.purple,B=T[_==null?void 0:_.color]||T.purple;return`<div class="pvp-gc-card" data-id="${S.id}"
    ...
    </div>`,z)return;const $=n==="p1"?f.home_id:f.away_id;k.from("cards").select("id,gc_type").eq("owner_id",$).eq("card_type","game_changer").then(async({data:S})=>{const{data:E}=await k.from("gc_definitions").select("*").eq("is_active",!0),_=new Set,D=(S||[]).filter(q=>_.has(q.gc_type)?!1:(_.add(q.gc_type),!0)).map(q=>({...q,_gcDef:(E==null?void 0:E.find(B=>B.name===q.gc_type))||null})),T=document.getElementById("pvp-gc-grid");if(T){if(!D.length){T.innerHTML='<div style="color:rgba(255,255,255,0.4);font-size:13px;text-align:center;margin-top:30px">Aucune carte Game Changer disponible.</div>';return}T.innerHTML=D.map(q=>C(q,j.includes(q.id))).join(""),T.querySelectorAll(".pvp-gc-card").forEach(q=>{q.addEventListener("click",()=>{const B=q.dataset.id,U=j.indexOf(B);U>-1?j.splice(U,1):j.length<N&&j.push(B),F()})})}}),(b=e.querySelector("#pvp-gc-validate"))==null||b.addEventListener("click",async()=>{const S=n==="p1"?f.home_id:f.away_id,{data:E}=await k.from("cards").select("id,gc_type").eq("owner_id",S).eq("card_type","game_changer").in("id",j),{data:_}=await k.from("gc_definitions").select("*").eq("is_active",!0),D=(E||[]).map(T=>({...T,_gcDef:(_==null?void 0:_.find(q=>q.name===T.gc_type))||null}));await g({["gcReady_"+n]:!0,["gcCardsFull_"+n]:D}),p["gcReady_"+r]&&await g({phase:"reveal"})})}function y(){e.style.overflow="hidden",e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
      ...
      <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
    </div>`,n==="p1"&&setTimeout(async()=>{await g({phase:"midfield"})},5e3)}function I(){const z=p[n+"Team"].MIL||[],L=p[r+"Team"].MIL||[];function j(E){return E.reduce((_,D)=>_+ie(D,"MIL"),0)}function N(E){let _=0;for(let D=0;D<E.length-1;D++){const T=ge(E[D],E[D+1]);T==="#00ff88"?_+=2:T==="#FFD700"&&(_+=1)}return _}const C=j(z)+N(z),$=j(L)+N(L),b=C>=$;function S(E,_,D){return`<div style="text-align:center">
        <div style="font-size:10px;color:rgba(255,255,255,0.5);letter-spacing:2px;margin-bottom:8px;text-transform:uppercase">${_}</div>
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${E.map((T,q)=>{const B=ne(T),U=q<E.length-1?ge(T,E[q+1]):null,K=U&&U!=="#ff3333"&&U!=="#cc2222";return`
            <div style="width:52px;height:52px;border-radius:8px;background:${D};position:relative;flex-shrink:0;overflow:hidden;border:2px solid rgba(255,255,255,0.3)">
              ...
              <div style="position:relative;z-index:1;font-size:15px;font-weight:900;color:#fff;text-shadow:0 1px 3px #000;text-align:center;padding-top:4px">${ie(T,"MIL")}</div>
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div id="pvp-midfield-result" style="opacity:0;text-align:center;transition:opacity 0.5s;color:#fff;max-width:320px"></div>
    </div>`,setTimeout(()=>{const E=document.getElementById("pvp-score-me"),_=document.getElementById("pvp-score-opp"),D=document.getElementById("pvp-midfield-result");E&&_&&(b?(E.style.fontSize="80px",E.style.color="#FFD700",_.style.opacity="0.25"):(_.style.fontSize="80px",_.style.color="#ff6b6b",E.style.opacity="0.25"));const T=p.p1Team.MIL||[],q=p.p2Team.MIL||[],B=j(T)+N(T),U=j(q)+N(q),K=B>=U?"p1":"p2";p[K+"Name"];let J=p.boostValue;J==null&&(J=Yt(),p.boostValue=J,p.boostOwner=K,p.boostUsed=!1),D&&(D.style.opacity="1",D.innerHTML=`<div style="font-size:20px;font-weight:900;margin-bottom:10px">
        ...
        <div style="background:rgba(135,206,235,0.15);border:2px solid #87CEEB;border-radius:14px;padding:14px 24px;display:inline-block;margin-top:4px" id="pvp-boost-display">
          ...
          <div style="font-size:10px;color:rgba(135,206,235,0.7)">Applicable sur n'importe quel joueur</div>
        </div>`:""}`),setTimeout(async()=>{if(n!=="p1")return;const te=K;await g({phase:te+"-attack",attacker:te,round:1,boostValue:p.boostValue,boostUsed:!1,boostOwner:te})},1800)},600)}function G(){var N;const z=p[n+"Team"],L=Object.entries(z).flatMap(([C,$])=>($||[]).filter(b=>!b.used).map(b=>({...b,_line:C})));if(!L.length)return;const j=document.createElement("div");j.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",j.innerHTML=`
      ...
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${L.map(C=>{const $={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[C._line]||"#555",b=ie(C,C._line)+(C.boost||0);return`<div class="boost-pick-item" data-cid="${C.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${$};overflow:hidden;cursor:pointer">
      ...
      </div>`,document.body.appendChild(j),(N=j.querySelector("#boost-picker-close"))==null||N.addEventListener("click",()=>j.remove()),j.querySelectorAll(".boost-pick-item").forEach(C=>{C.addEventListener("click",async()=>{const $=C.dataset.cid,b=L.find(_=>_.cardId===$);if(!b)return;const S=p[n+"Team"],E=(S[b._line]||[]).find(_=>_.cardId===$);E&&(E.boost=(E.boost||0)+p.boostValue),j.remove(),await g({[n+"Team"]:S,boostUsed:!0})})})}function R(){var j;const z=Array.isArray(p.log)?p.log:[],L=document.createElement("div");L.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column",L.innerHTML=`
      ...
      <div style="width:min(90vw,420px)">${Le(p[r+"Team"],p[r+"Formation"],null,[],300,300)}</div>
      <button id="pvp-view-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(z),(L=z.querySelector("#pvp-view-opp-close"))==null||L.addEventListener("click",()=>z.remove())}function H(z,L){var B,U;const N=(p["gcCardsFull_"+n]||[]).find(K=>K.id===z),C=N==null?void 0:N._gcDef,$=de[L]||{icon:"⚡",desc:"Carte spéciale."},b={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[C==null?void 0:C.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",S={purple:"#b06ce0",light_blue:"#00d4ef"}[C==null?void 0:C.color]||"#b06ce0",E=(C==null?void 0:C.name)||L,_=(C==null?void 0:C.effect)||$.desc,D=C!=null&&C.image_url?`/manager-wars/icons/${C.image_url}`:null,T=$.icon||"⚡",q=document.createElement("div");q.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",q.innerHTML=`
      <div style="width:190px;border-radius:16px;border:3px solid ${S};background:${b};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 40px ${S}66">
        ...
        <div style="height:160px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
          ${D?`<img src="${D}" style="max-width:150px;max-height:150px;object-fit:contain">`:`<span style="font-size:72px">${T}</span>`}
        </div>
        <div style="padding:10px;background:rgba(0,0,0,0.38);text-align:center">
      ...
      </div>`,document.body.appendChild(q),(B=q.querySelector("#pvp-gc-back"))==null||B.addEventListener("click",()=>q.remove()),(U=q.querySelector("#pvp-gc-use"))==null||U.addEventListener("click",async()=>{q.remove();const K=p["usedGc_"+n]||[];K.push(z),await g({["usedGc_"+n]:K})})}function W(){var C;const z=p[n+"Score"],L=p[r+"Score"],j=z>L,N=z===L;e.style.overflow="hidden",e.innerHTML=`
    ...
    </div>`,(C=document.getElementById("pvp-home"))==null||C.addEventListener("click",()=>{k.removeChannel(v),le(e),d("home")})}async function V(){const z=(p["selected_"+n]||[]).map(N=>({...N,_line:N._role}));if(!z.length)return;const L=ot(z,p.modifiers[n]||{});z.forEach(N=>{const C=(p[n+"Team"][N._role]||[]).find($=>$.cardId===N.cardId);C&&(C.used=!0)});const j=Array.isArray(p.log)?p.log:[];j.push({type:"attack",text:`⚔️ ${p[n+"Name"]} attaque (${L.total})`,players:z,total:L.total,side:n}),await g({pendingAttack:{...L,players:z,side:n},["selected_"+n]:[],modifiers:{...p.modifiers,[n]:{}},phase:r+"-defense",log:j})}async function ee(){const z=(p["selected_"+n]||[]).map(D=>({...D,_line:D._role})),L=nt(z,p.modifiers[n]||{});z.forEach(D=>{const T=(p[n+"Team"][D._role]||[]).find(q=>q.cardId===D.cardId);T&&(T.used=!0)});const j=rt(p.pendingAttack.total,L.total,p.modifiers[n]||{}),N=p.pendingAttack.side,C=j==="attack",$=p[N+"Score"]+(C?1:0),b=N==="p1"?"p2":"p1",S=(p.round||0)+1,E=S>10,_=Array.isArray(p.log)?p.log:[];_.push({type:"defense",text:`🛡️ ${p[n+"Name"]} défend (${L.total})`,players:z,total:L.total,side:n}),_.push({type:C?"goal":"stop",text:C?`⚽ BUT de ${p[N+"Name"]} ! (${p.pendingAttack.total} vs ${L.total})`:`✋ Attaque stoppée (${p.pendingAttack.total} vs ${L.total})`}),await g({[N+"Score"]:$,["selected_"+n]:[],modifiers:{...p.modifiers,[n]:{}},pendingAttack:null,phase:E?"finished":b+"-attack",attacker:b,round:S,log:_}),E&&await k.from("matches").update({status:"finished"}).eq("id",i)}F()}const mo={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"};async function xo(e,t){const{state:i,toast:s}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement du marché...</div>',await ct(e,t)}async function ct(e,t){const{state:i,toast:s}=t,{data:c}=await k.from("market_listings").select(`id, price, status, listed_at, seller_id,
  ...
  `;function l(n){const r=document.getElementById("mkt-content"),u=n==="buy"?a:d;if(u.length===0){r.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      </div>`;return}const m=n==="mine"?[...u].sort((f,h)=>f.status!==h.status?f.status==="active"?-1:1:new Date(h.listed_at)-new Date(f.listed_at)):u;r.innerHTML=`<div style="display:flex;flex-direction:column;gap:10px">
      ${m.map(f=>{var g,w,F,M;const h=(g=f.card)==null?void 0:g.player;if(!h)return"";const p=h.job==="GK"?h.note_g:h.job==="DEF"?h.note_d:h.job==="MIL"?h.note_m:h.note_a,x=mo[h.rarity],A=(i.profile.credits||0)>=f.price,v=f.status==="sold";return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${v?"opacity:0.65":""}">
          <div style="width:44px;height:44px;border-radius:8px;background:${vo(h.job)};color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:18px;border:2px solid ${x};flex-shrink:0">${p}</div>
          <div style="flex:1;min-width:0">
            <div style="font-weight:700;font-size:14px">${h.firstname} ${h.surname_encoded}</div>
            <div style="font-size:11px;color:var(--gray-600)">${h.country_code} · ${((w=h.clubs)==null?void 0:w.encoded_name)||"—"} · ${h.rarity} · ${h.job}</div>
            ${n==="buy"?`<div style="font-size:11px;color:var(--gray-600)">Vendeur : ${((F=f.seller)==null?void 0:F.pseudo)||"—"}</div>`:v?`<div style="font-size:11px;color:var(--green)">✅ Vendu à ${((M=f.buyer)==null?void 0:M.pseudo)||"—"} · ${f.sold_at?new Date(f.sold_at).toLocaleDateString("fr"):""}</div>`:`<div style="font-size:11px;color:var(--gray-600)">🟢 En vente depuis le ${new Date(f.listed_at).toLocaleDateString("fr")}</div>`}
          </div>
          <div style="text-align:right;flex-shrink:0">
            <div style="font-weight:900;color:var(--yellow);font-size:15px">${f.price.toLocaleString("fr")}</div>
            ${n==="buy"?`<button class="btn btn-primary btn-sm" data-buy="${f.id}" ${A?"":"disabled"} style="margin-top:4px">${A?"Acheter":"Trop cher"}</button>`:v?'<span style="display:inline-block;margin-top:4px;font-size:10px;font-weight:700;color:#fff;background:var(--green);padding:3px 10px;border-radius:10px">VENDU</span>':`<button class="btn btn-danger btn-sm" data-cancel="${f.id}" style="margin-top:4px">Retirer</button>`}
          </div>
        </div>`}).join("")}
    </div>`,r.querySelectorAll("[data-buy]").forEach(f=>{f.addEventListener("click",()=>yo(f.dataset.buy,u,e,t))}),r.querySelectorAll("[data-cancel]").forEach(f=>{f.addEventListener("click",()=>ho(f.dataset.cancel,e,t))})}l("buy"),e.querySelectorAll(".mkt-tab").forEach(n=>{n.addEventListener("click",()=>{e.querySelectorAll(".mkt-tab").forEach(r=>{const u=r===n;r.style.background=u?"var(--green)":"#fff",r.style.color=u?"#fff":"var(--gray-600)",r.style.borderColor=u?"var(--green)":"var(--gray-200)"}),l(n.dataset.tab)})})}async function yo(e,t,i,s){var u;const{state:c,toast:o}=s,a=t.find(m=>m.id===e);if(!a)return;const d=a.price,l=c.profile.credits||0,n=(u=a.card)==null?void 0:u.player;if(l<d){o("Crédits insuffisants","error");return}if(!confirm(`Acheter ${n==null?void 0:n.firstname} ${n==null?void 0:n.surname_encoded} pour ${d.toLocaleString("fr")} crédits ?`))return;const r=document.querySelector(`[data-buy="${e}"]`);r&&(r.disabled=!0,r.textContent="⏳");try{const{data:m,error:f}=await k.rpc("buy_market_card",{p_listing_id:e,p_buyer_id:c.profile.id});if(f)throw new Error(f.message);if(!(m!=null&&m.success))throw new Error((m==null?void 0:m.error)||"Achat impossible");c.profile.credits=l-d;const h=document.querySelector("[data-credits]")||document.querySelector(".credits-display");h&&(h.textContent=(l-d).toLocaleString("fr")+" cr."),o(`✅ ${n==null?void 0:n.surname_encoded} ajouté à ta collection !`,"success"),ct(i,s)}catch(m){o("❌ "+m.message,"error"),r&&(r.disabled=!1,r.textContent="Acheter")}}async function ho(e,t,i){const{toast:s}=i,{data:c}=await k.from("market_listings").select("card_id").eq("id",e).single();await k.from("market_listings").update({status:"cancelled"}).eq("id",e),c&&await k.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",c.card_id),s("Annonce retirée","success"),ct(t,i)}function vo(e){return{GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[e]||"#888"}async function bo(e,{state:t,navigate:i,toast:s}){e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:c}=await k.from("users").select("id,pseudo,club_name,trophies_top1,trophies_top2,trophies_top3,wins,level").order("trophies_top1",{ascending:!1}).limit(100);e.innerHTML=`
  <div class="page">
    <div class="page-header">
  ...
  `}async function wo(e,{state:t,navigate:i,toast:s}){const c=t.profile;if(!c)return;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await k.from("matches").select(`id,home_id,away_id,home_score,away_score,status,mode,winner_id,created_at,played_at,
      home:users!home_id(pseudo,club_name),
      away:users!away_id(pseudo,club_name)`).or(`home_id.eq.${c.id},away_id.eq.${c.id}`).order("created_at",{ascending:!1}).limit(50),a={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},d=(o||[]).filter(r=>r.status==="finished"),l=(o||[]).filter(r=>r.status==="in_progress");function n(r){const u=r.home_id===c.id;u?r.home_score:r.away_score,u?r.away_score:r.home_score;const m=r.winner_id===c.id,f=r.home_score===r.away_score&&r.status==="finished",h=r.status!=="finished"?"…":f?"N":m?"V":"D",p=r.status!=="finished"||f?"#888":m?"#1A6B3C":"#c0392b";let x=a[r.mode]||r.mode;r.away_id===null&&!x.startsWith("IA")&&(x="IA");const v=r.home_id===c.id?r.away:r.home;let g;r.away_id===null?g=x:v?g=`${v.club_name||v.pseudo} (${v.pseudo})`:g="Adversaire";let w="";r.status==="in_progress"&&Date.now()-new Date(r.created_at).getTime()>3600*1e3&&(w=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const F=new Date(r.created_at),M=F.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+F.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),y=r.status==="finished"?`${r.home_score} - ${r.away_score}`:`${r.home_score||0} - ${r.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
        <div style="font-size:13px;font-weight:600">${g}${w}</div>
      ...
      <div style="display:flex;align-items:center;gap:8px">
        ...
        <span style="background:${p};color:#fff;width:22px;height:22px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:900">${h}</span>
      </div>
    </div>`}e.innerHTML=`
  <div class="page">
    <div class="page-header">
  ...
  </div>`}const Z={user:null,profile:null,page:"home",params:{}};function De(e,t="info",i=3e3){const s=document.getElementById("toast");s&&(s.textContent=e,s.className=`show ${t}`,clearTimeout(s._t),s._t=setTimeout(()=>{s.className=""},i))}function _o(e,t,i=""){document.getElementById("modal-title").textContent=e,document.getElementById("modal-body").innerHTML=t,document.getElementById("modal-footer").innerHTML=i,document.getElementById("modal-overlay").classList.remove("hidden")}function et(){document.getElementById("modal-overlay").classList.add("hidden")}async function Ge(){if(!Z.user)return;const{data:e}=await k.from("users").select("*").eq("id",Z.user.id).single();e&&(Z.profile=e)}const pt="mw_theme";function He(){return localStorage.getItem(pt)||"light"}function $o(e){var t;localStorage.setItem(pt,e),tt(e),(t=Z.profile)!=null&&t.id&&k.from("users").update({theme:e}).eq("id",Z.profile.id).then(()=>{})}function tt(e){document.documentElement.setAttribute("data-theme",e)}function qe(e,t={}){Z.page=e,Z.params=t,ii()}async function ii(){const e=document.getElementById("page-content");if(!e)return;document.querySelectorAll(".bottom-nav a").forEach(s=>{s.classList.toggle("active",s.dataset.page===Z.page)});const t=document.getElementById("nav-credits");t&&Z.profile&&(t.textContent=`💰 ${(Z.profile.credits||0).toLocaleString("fr")}`);const i={state:Z,navigate:qe,toast:De,openModal:_o,closeModal:et,refreshProfile:Ge};switch(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽</div>',Z.page){case"home":await At(e,i);break;case"collection":await xi(e,i);break;case"decks":await We(e,i);break;case"boosters":await ji(e,i);break;case"match":{(Z.params&&Z.params.matchMode||"vs_ai_easy")==="random"?await uo(e,i):await Ji(e,i);break}case"market":await xo(e,i);break;case"rankings":await bo(e,i);break;case"matches":await wo(e,i);break;default:await At(e,i)}}function ko(){const e=document.getElementById("app"),t=Z.profile;if(!t)return;const i="/manager-wars/icons/";e.innerHTML=`
    <nav class="top-nav">
