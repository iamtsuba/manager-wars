...
import{s as h,t as Ho,u as Uo,n as ut,g as Bt,p as nt,v as si,o as Ut,r as ze,k as zt,w as Gi,x as Ft,y as Ii,F as Ai,z as Ko,T as Di,q as Tn,C as In,d as An}from"./special-cards-Bn7RhUuk.js";const zn="/";function _o(t,{navigate:e,toast:i}){let o="login";const r=()=>{var a,d,c,s,l,p,b;const n=o==="login";if(t.innerHTML=`
    <div style="
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    ${ze(t,{width:140})}
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),s=(d||[]).filter(j=>j.card_type==="player"&&j.player),l=(d||[]).filter(j=>j.card_type==="game_changer"),p=(d||[]).filter(j=>j.card_type==="formation"),b=(d||[]).filter(j=>j.card_type==="stadium"),{data:g}=await h.from("gc_definitions").select("name,gc_type,color,effect,image_url"),x={};(g||[]).forEach(j=>{x[j.name]=j});const{data:f}=await h.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),y={};(f||[]).forEach(j=>{y[j.id]=j}),b.forEach(j=>{j.stadium_def&&(y[j.stadium_id]=j.stadium_def)});const u=Object.keys(Ai),w=Object.keys(Oe),v={};s.forEach(j=>{const W=j.player.id;v[W]=(v[W]||0)+1}),new Set(Object.keys(v).map(j=>String(j)));const E=new Set(p.map(j=>j.formation)),m=new Set(l.map(j=>j.gc_type));let T="player",$="Tous",I="",B=!1,V=!1;function Y(){return[...s].sort((j,W)=>{const ee=mi.indexOf(j.player.job),ie=mi.indexOf(W.player.job);return ee!==ie?ee-ie:(j.player.surname_real||"").localeCompare(W.player.surname_real||"")})}function K(){return[...c||[]].sort((j,W)=>{const ee=mi.indexOf(j.job),ie=mi.indexOf(W.job);return ee!==ie?ee-ie:(j.surname_real||"").localeCompare(W.surname_real||"")})}function L(){return Y().filter(j=>{const W=j.player,ee=$==="Tous"||W.job===$,ie=!I||`${W.firstname} ${W.surname_real}`.toLowerCase().includes(I);return ee&&ie})}function _(){return K().filter(j=>{const W=$==="Tous"||j.job===$,ee=!I||`${j.firstname} ${j.surname_real}`.toLowerCase().includes(I);return W&&ee})}t.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="margin-top:16px;border-top:1px solid var(--tile-border);padding-top:14px">
      ...
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px">
        ${p.filter(F=>F.id!==t.id).map((F,H)=>{const ae=_[F.id]||[],ce=F.is_for_sale,be=ae.length?ae[ae.length-1]:null,ke=F.evolution_bonus||0,Ee=be?be.source==="booster"?"Booster":be.price?be.price.toLocaleString("fr")+" cr.":"—":"—",Se=be?new Date(be.transferred_at).toLocaleDateString("fr",{day:"2-digit",month:"2-digit",year:"numeric"}):"",ye=80,Le=Math.round(ye*657/507),De=ze({...l,_evolution_bonus:ke},{width:ye});return`
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(j=document.getElementById("close-detail"))==null||j.addEventListener("click",d);let G=new Set;const le=l.rarity==="pepite"?1.3:l.rarity==="papyte"?.7:1;function pe(){let F=0;return document.querySelectorAll(".expl-check:checked").forEach(H=>{if(H.dataset.id===t.id)return;const ce=Number(H.dataset.evo)||0,be=Number(H.dataset.note)||0;F+=be+ce}),Math.round(F*le)}const M=()=>{const F=G.size,H=document.getElementById("sell-action-panel");if(!H)return;H.style.display=F>0?"block":"none",document.getElementById("sell-selected-count").textContent=F;const ae=document.getElementById("evolve-btn");if(ae){const ce=pe();ae.textContent=ce>0?`⬆️ Évoluer (+${ce})`:"⬆️ Évoluer"}};document.querySelectorAll(".expl-check").forEach(F=>{F.addEventListener("change",()=>{const H=F.dataset.id;F.checked?G.add(H):G.delete(H);const ae=F.closest(".exemplaire-row");if(ae){const ce=ae.querySelector(".expl-sel-overlay"),be=ae.querySelector(".expl-sel-check");ce&&(ce.style.display=F.checked?"block":"none"),be&&(be.style.display=F.checked?"flex":"none"),ae.style.transform=F.checked?"scale(1.05)":"scale(1)"}M()})}),document.querySelectorAll(".exemplaire-row").forEach(F=>{F.addEventListener("click",H=>{if(H.target.tagName==="INPUT")return;const ae=F.querySelector(".expl-check");ae&&!ae.disabled&&(ae.checked=!ae.checked,ae.dispatchEvent(new Event("change")))})}),(W=document.getElementById("evolve-btn"))==null||W.addEventListener("click",async()=>{if(b<=1)return;const F=[...G];if(!F.length)return;if(G.has(t.id)){const ye=document.createElement("div");ye.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",ye.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    </div>`}function xn(t){var n,a,d;if(!t)return"";const e=c=>c?ze({...c,_evolution_bonus:0},{width:52,role:c._line||c.job,showStad:!!c.stadiumBonus,extraNote:c.boost||0}):"",o={goal:"⚽","goal-home":"⚽","goal-ai":"⚽",duel:"⚔️",midfield:"🎯",sub:"🔄",gc:"⚡",boost:"💥","defense-won":"🛡️","attack-won":"⚔️","defense-lost":"😓","attack-lost":"😓"}[t.type]||"📋";return`
  ...
  <div id="match-history-panel">
    ...
    <div style="text-align:center;padding:40px;color:#fff;max-width:380px;width:100%">
      ...
      ${Mi(t.aiTeam,t.formation,null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.1))}
            ...
            </div>`}}catch(te){console.error("[Ranked] MMR update error:",te)}(ne=document.getElementById("pvp-end-overlay"))==null||ne.remove();const q=document.createElement("div");q.id="pvp-end-overlay",q.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;color:#fff;padding:24px;text-align:center;overflow-y:auto";const D=A?"🤝":C?"🏆":"😞",U=A?"MATCH NUL":C?"VICTOIRE !":"DÉFAITE",Z=A?"#fff":C?"#FFD700":"#ff6b6b";q.innerHTML=`
      ...
      ${R}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${l?"🏆 Retour à la Mini League":a?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(q),(oe=q.querySelector("#pvp-end-home"))==null||oe.addEventListener("click",()=>{q.remove(),Ye(t),l?g("mini-league",{openLeagueId:l}):g(a?"ranked":"home")})}const K=h.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},k=>{var z;const S=k.new;try{if(S.status==="finished"||S.forfeit){if(T)return;T=!0,$&&(clearInterval($),$=null),Ve(),S.game_state&&(m=S.game_state),Y(S);return}if(S.game_state){const C=m;m=S.game_state;const A=m._lastGC;if(A&&A.seq&&!V.has(A.seq)&&(V.add(A.seq),A.by!==f)){de(A.type,A.by,()=>O());return}const R=C[f+"Score"]||0,q=C[y+"Score"]||0,D=m[f+"Score"]||0,U=m[y+"Score"]||0,Z=D>R,ne=U>q;if((Z||ne)&&!B.has(m.round)){B.add(m.round);const oe=[...m.log||[]].reverse().find(fe=>fe.isGoal),te=((oe==null?void 0:oe.homePlayers)||[]).map(fe=>({name:fe.name,note:fe.note,portrait:fe.portrait,job:fe.job}));!!((z=oe==null?void 0:oe.text)!=null&&z.includes("DERNIER CORNER"))?ae().then(()=>{tt(te,D,U,Z,()=>O())}):tt(te,D,U,Z,()=>O());return}O()}}catch(C){console.error("[PvP] crash:",C)}}).subscribe();async function L(k){Object.assign(m,k),m.lastActionAt=new Date().toISOString();const{error:S}=await h.from("matches").update({game_state:m}).eq("id",i);S&&x("Erreur de synchronisation","error");try{O()}catch(z){console.error("[PvP] renderPvpScreen crash:",z)}}async function _(){if(T)return;T=!0,Ot(),Ve(),$&&(clearInterval($),$=null);const k=o?v.away_id:v.home_id,S=o?"p2":"p1",z=o?"p1":"p2",C={...m,[S+"Score"]:3,[z+"Score"]:0,phase:"finished"},A=C.p1Score||0,R=C.p2Score||0;try{await h.from("matches").update({game_state:C}).eq("id",i);const{data:q,error:D}=await h.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:k,p_home_score:A,p_away_score:R,p_forfeit:!0});if(D&&console.error("[PvP] finish_pvp_match (forfeit):",D),l)try{await h.rpc("check_and_finish_mini_league",{p_league_id:l})}catch(U){console.error("[PvP] check_and_finish_mini_league (forfeit):",U)}try{const{data:U}=await h.rpc("apply_match_rewards",{p_match_id:i});U!=null&&U.success&&!(U!=null&&U.skipped)&&typeof e.refreshProfile=="function"&&await e.refreshProfile()}catch(U){console.error("[PvP] apply_match_rewards (forfeit):",U)}}catch{}try{h.removeChannel(K)}catch{}setTimeout(()=>{Ye(t),l?g("mini-league",{openLeagueId:l}):g("home")},800)}const N={BOOST_STAT:({value:k=1,count:S=1,roles:z=[]},C,A)=>{const R=C[f+"Team"],q=Object.entries(R).filter(([D])=>!z.length||z.includes(D)).flatMap(([D,U])=>U.filter(Z=>!Z.used).map(Z=>({...Z,_line:D})));if(!q.length){C.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),A(C);return}G(q,S,`Choisir ${S} joueur(s) à booster (+${k})`,D=>{D.forEach(U=>{const Z=(R[U._line]||[]).find(ne=>ne.cardId===U.cardId);Z&&(Z.boost=(Z.boost||0)+k,C.log.push({text:`⚡ +${k} sur ${Z.name}`,type:"info"}))}),C[f+"Team"]=R,A(C)})},DEBUFF_STAT:({value:k=1,count:S=1,roles:z=[],target:C="ai"},A,R)=>{const q=C==="home"?f:y,D=A[q+"Team"],U=C==="home"?"allié":"adverse",Z=Object.entries(D).filter(([ne])=>!z.length||z.includes(ne)).flatMap(([ne,oe])=>oe.map(te=>({...te,_line:ne})));if(!Z.length){A.log.push({text:`🎯 Aucun joueur ${U}`,type:"info"}),R(A);return}G(Z,S,`Choisir ${S} joueur(s) ${U}(s) (-${k})`,ne=>{ne.forEach(oe=>{const te=(D[oe._line]||[]).find(se=>se.cardId===oe.cardId);te&&(te.boost=(te.boost||0)-k,A.log.push({text:`🎯 -${k} sur ${te.name}`,type:"info"}))}),A[q+"Team"]=D,R(A)})},GRAY_PLAYER:({count:k=1,roles:S=[],target:z="ai"},C,A)=>{const R=z==="home"?f:y,q=C[R+"Team"],D=z==="home"?"allié":"adverse",U=Object.entries(q).filter(([Z])=>!S.length||S.includes(Z)).flatMap(([Z,ne])=>ne.filter(oe=>!oe.used).map(oe=>({...oe,_line:Z})));if(!U.length){C.log.push({text:`❌ Aucun joueur ${D}`,type:"info"}),A(C);return}G(U,k,`Choisir ${k} joueur(s) ${D}(s) à exclure`,Z=>{const ne="usedCardIds_"+R,oe=new Set(C[ne]||[]);Z.forEach(te=>{const se=(q[te._line]||[]).find(fe=>fe.cardId===te.cardId);se&&(se.used=!0,oe.add(te.cardId),C.log.push({text:`❌ ${se.name} exclu !`,type:"info"}))}),C[ne]=[...oe],C[R+"Team"]=q,A(C)})},REVIVE_PLAYER:({count:k=1,roles:S=[]},z,C)=>{const A=z[f+"Team"],R=Object.entries(A).filter(([q])=>!S.length||S.includes(q)).flatMap(([q,D])=>D.filter(U=>U.used).map(U=>({...U,_line:q})));if(!R.length){z.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),C(z);return}G(R,k,`Choisir ${k} joueur(s) à ressusciter`,q=>{q.forEach(D=>{const U=(A[D._line]||[]).find(Z=>Z.cardId===D.cardId);U&&(U.used=!1,z.log.push({text:`💫 ${U.name} ressuscité !`,type:"info"}))}),z[f+"Team"]=A,C(z)})},REMOVE_GOAL:({},k,S)=>{const z=y+"Score";k[z]>0?(k[z]--,k.log.push({text:"🚫 Dernier but annulé !",type:"info"})):k.log.push({text:"🚫 Aucun but à annuler",type:"info"}),S(k)},ADD_GOAL_DRAW:({},k,S)=>{k[f+"Score"]===k[y+"Score"]?(k[f+"Score"]++,k.log.push({text:"🎯 But bonus !",type:"info"})):k.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),S(k)},ADD_SUB:({value:k=1},S,z)=>{S.maxSubs=(S.maxSubs||3)+k,S.log.push({text:`🔄 +${k} remplacement(s)`,type:"info"}),z(S)},CUSTOM:({},k,S)=>S(k)};function G(k,S,z,C){const A=document.createElement("div");A.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let R=[];function q(){var U,Z;const D=k.map(ne=>{const oe=ne._line||ne.job||"MIL",te=R.find(fe=>fe.cardId===ne.cardId),se=ze({...ne,_evolution_bonus:0},{width:90,showStad:!0,role:oe,extraNote:ne.boost||0});return`<div class="pp-item" data-cid="${ne.cardId}"
        ...
        </div>`,(U=A.querySelector("#pp-close"))==null||U.addEventListener("click",()=>A.remove()),A.querySelectorAll(".pp-item").forEach(ne=>{ne.addEventListener("click",()=>{const oe=ne.dataset.cid,te=k.find(fe=>fe.cardId===oe),se=R.findIndex(fe=>fe.cardId===oe);te&&(se>-1?R.splice(se,1):R.length<S&&R.push(te),q())})}),(Z=A.querySelector("#pp-confirm"))==null||Z.addEventListener("click",()=>{A.remove(),C(R)})}q(),document.body.appendChild(A)}async function le(k,S){const C=(m["gcCardsFull_"+f]||[]).find(D=>D.id===k),A=(C==null?void 0:C._gcDef)||(m.gcDefs||[]).find(D=>{var U;return D.name===S||((U=D.name)==null?void 0:U.toLowerCase().trim())===(S==null?void 0:S.toLowerCase().trim())}),R=[...m["usedGc_"+f]||[],k],q={type:S,by:f,seq:(m._gcAnimSeq||0)+1};V.add(q.seq),de(S,f,async()=>{if(A!=null&&A.effect_type&&A.effect_type!=="CUSTOM"){const U=N[A.effect_type];if(U){const Z={...m};U(A.effect_params||{},Z,async ne=>{ne["usedGc_"+f]=R,ne._lastGC=q,ne._gcAnimSeq=q.seq,await L(ne)});return}}const D={...m};switch(S){case"Remplacement+":D.maxSubs=(D.maxSubs||3)+1,D.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const U=y+"Score";D[U]>0&&(D[U]--,D.log.push({text:"🚫 But annulé",type:"info"}));break}}D["usedGc_"+f]=R,D._lastGC=q,D._gcAnimSeq=q.seq,await L(D)})}function pe(k,S){const z="usedCardIds_"+k,C=new Set(m[z]||[]);S.forEach(A=>C.add(A)),m[z]=[...C]}function M(){for(const k of["p1","p2"]){const S=new Set(m["usedCardIds_"+k]||[]),z=m[k+"Team"];if(z)for(const C of["GK","DEF","MIL","ATT"])(z[C]||[]).forEach(A=>{A.used=S.has(A.cardId)})}}function O(){var xo,bo,yo,ho,vo,wo;if(m.phase==="reveal")return ue();if(m.phase==="midfield")return ge();if(m.phase==="finished")return He();const k=m[f+"Team"],S=m[y+"Team"];M();const z=m[f+"Score"],C=m[y+"Score"],A=m[f+"Name"],R=m[y+"Name"],q=m.phase===f+"-attack",D=m.phase===f+"-defense",U=Array.isArray(m["selected_"+f])?m["selected_"+f]:[],Z=U.map(Q=>Q.cardId),ne=window.innerWidth>=700,oe=m[f+"Subs"]||[],te=m["usedSubIds_"+f]||[],se=oe.filter(Q=>!te.includes(Q.cardId)),fe=m["gcCardsFull_"+f]||[],we=m["usedGc_"+f]||[],me=fe.filter(Q=>!we.includes(Q.id)),_e=m.boostOwner===f&&!m.boostUsed,$e=[...k.MIL||[],...k.ATT||[]].filter(Q=>!Q.used),re=[...S.MIL||[],...S.ATT||[]].filter(Q=>!Q.used),Ae=!Se(S),Ne=$e.length===0&&re.length===0&&!Ae,je=(k.DEF||[]).filter(Q=>!Q.used),Xe=(k.GK||[]).filter(Q=>!Q.used);let Ue=[];q&&$e.length===0&&(Ae?(Ue=je.map(Q=>Q.cardId),je.length===0&&(Ue=Ue.concat(Xe.map(Q=>Q.cardId)))):Ne&&(Ue=[...je,...Xe].map(Q=>Q.cardId)));function rt(Q,ve,qe){var Be,bt;const Te=Q._gcDef,Nt=(Te==null?void 0:Te.name)||Q.gc_type,dt=(Te==null?void 0:Te.effect)||((Be=Oe[Q.gc_type])==null?void 0:Be.desc)||"",Pe=Te!=null&&Te.image_url?`/icons/${Te.image_url}`:null,Ge=((bt=Oe[Q.gc_type])==null?void 0:bt.icon)||"⚡";return`<div class="pvp-gc-mini" data-gc-id="${Q.id}" data-gc-type="${Q.gc_type}" style="flex-shrink:0;cursor:pointer">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(oe);let te=!1;const se=()=>{te||(te=!0,oe.remove(),setTimeout(()=>z&&z(),50))};oe.addEventListener("click",se),setTimeout(se,3e3)}function j(k,S){var ne,oe,te,se;const C=(m["gcCardsFull_"+f]||[]).find(fe=>fe.id===k),A=C==null?void 0:C._gcDef,R=(A==null?void 0:A.name)||S,q=(A==null?void 0:A.effect)||((ne=Oe[S])==null?void 0:ne.desc)||"Carte spéciale.",D=A!=null&&A.image_url?`/icons/${A.image_url}`:null,U=((oe=Oe[S])==null?void 0:oe.icon)||"⚡",Z=document.createElement("div");Z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",Z.innerHTML=`
      ${nt(R,D,U,q,{width:190})}
      <div style="display:flex;gap:12px;width:190px">
        <button id="pvp-gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
        <button id="pvp-gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
      </div>`,document.body.appendChild(Z),(te=Z.querySelector("#pvp-gc-back"))==null||te.addEventListener("click",()=>Z.remove()),(se=Z.querySelector("#pvp-gc-use"))==null||se.addEventListener("click",()=>{Z.remove(),le(k,S)})}function W(){var C;const k=m[f+"Team"],S=Object.entries(k).flatMap(([A,R])=>(R||[]).filter(q=>!q.used).map(q=>({...q,_line:A})));if(!S.length)return;const z=document.createElement("div");z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",z.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
      ...
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${S.map(A=>{const R={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[A._line]||"#555",q=Re(A,A._line)+(A.boost||0);return`<div class="bp-item" data-cid="${A.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${R};overflow:hidden;cursor:pointer">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:6px">Appuyer pour continuer</div>`,document.body.appendChild(A);let q=!1;const D=()=>{q||(q=!0,A.remove(),setTimeout(()=>z(),50))};A.addEventListener("click",D),setTimeout(D,2200)}function X(){var S;const k=document.createElement("div");k.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:20px;overflow-y:auto",k.innerHTML=`
      ...
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(k),(S=k.querySelector("#pvp-opp-close"))==null||S.addEventListener("click",()=>k.remove())}function P(){var z;const k=m.log||[],S=document.createElement("div");S.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column",S.innerHTML=`
      ...
      </div>`,document.body.appendChild(S),(z=S.querySelector("#pvp-hist-close"))==null||z.addEventListener("click",()=>S.remove())}async function J(){if(m.phase!==f+"-attack")return;const k=f==="p1"?"p2":"p1",S=(m.round||0)+1,z=[...m.log||[]];z.push({type:"info",text:`⏭️ ${m[f+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const C=De(m),A=Le(k),R=C||!A?"finished":k+"-attack";await L({["selected_"+f]:[],modifiers:{...m.modifiers,[f]:{}},pendingAttack:null,phase:R,attacker:k,round:S,log:z}),R==="finished"&&await Ce(m)}function F(k){const S=(k.GK||[]).some(C=>!C.used),z=["DEF","MIL","ATT"].some(C=>(k[C]||[]).some(A=>!A.used));return S&&!z}function H(k){return(k.p1Score||0)!==(k.p2Score||0)?null:F(k.p1Team)&&!Se(k.p2Team)?{side:"p1",gk:(k.p1Team.GK||[]).find(S=>!S.used)}:F(k.p2Team)&&!Se(k.p1Team)?{side:"p2",gk:(k.p2Team.GK||[]).find(S=>!S.used)}:null}async function ae(){const k=document.createElement("div");k.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:950;display:flex;align-items:center;justify-content:center;overflow:hidden;text-align:center;padding:24px",document.body.appendChild(k);const S=(z,C)=>new Promise(A=>{k.innerHTML=`<div style="font-size:32px;font-weight:900;color:${C};letter-spacing:2px;animation:lcFade 1.4s ease both">${z}</div>
      <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(A,1400)});await S("⚽ DERNIER CORNER","#FFD700"),await S("🧤 LE GARDIEN MONTE !","#4fc3f7"),k.remove()}async function ce(k,S){const{side:z,gk:C}=k;if(!C)return;const A=z+"Score",R=(S[A]||0)+1;C.used=!0;const q=[...S.log||[]];q.push({type:"duel",isGoal:!0,homeScored:z===f,homePlayers:z==="p1"?[Gt(C)]:[],aiPlayers:z==="p2"?[Gt(C)]:[],text:`⚽ DERNIER CORNER — Le gardien de ${S[z+"Name"]} marque !`});const D=(S.round||0)+1,U=z==="p1"?"p2":"p1",Z={...S,[A]:R},ne=De(Z);B.add(D),await ae();const oe=z===f?R:S[f+"Score"]||0,te=z===f?S[y+"Score"]||0:R;tt([Gt(C)],oe,te,z===f,async()=>{await L({[A]:R,log:q,round:D,pendingAttack:null,phase:ne?"finished":U+"-attack",attacker:U,["selected_"+f]:[],modifiers:{...m.modifiers,[f]:{}}}),ne&&await Ce({...m,[A]:R})})}async function be(){const k=m[f+"Team"],S=!["GK","DEF","MIL","ATT"].some(R=>(m[y+"Team"][R]||[]).some(q=>!q.used)),z=(m["selected_"+f]||[]).map(R=>{const q=(k[R._role]||[]).find(fe=>fe.cardId===R.cardId)||R,D=["GK","DEF"].includes(R._role),U=k[R._role]||[],Z=U.findIndex(fe=>fe.cardId===R.cardId),ne=lt(U.length),oe=Z>=0?ne[Z]:q._col??1,te=m.stadiumDef||m.homeStadiumDef||null,se=q.stadiumBonus||te&&(te.club_id&&String(q.club_id)===String(te.club_id)||te.country_code&&q.country_code===te.country_code)||!1;return{...q,_line:R._role,_col:oe,stadiumBonus:se,...D?{note_a:Math.max(1,Number(q.note_a)||0)}:{}}});if(!z.length)return;const C=ci(z,m.modifiers[f]||{},m[f+"Formation"]);pe(f,z.map(R=>R.cardId)),z.forEach(R=>{const q=(k[R._role]||[]).find(D=>D.cardId===R.cardId);q&&(q.used=!0)}),m["selected_"+f]=[],O();const A=[...m.log||[]];if(S){if(z.length===1&&(z[0]._line==="GK"||z[0].job==="GK")&&(m.p1Score||0)===(m.p2Score||0)){await ce({side:f,gk:z[0]},{...m,[f+"Team"]:k,log:A});return}const q=(m[f+"Score"]||0)+1,D=z.map(te=>Gt(te));A.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:D,homeTotal:C.total,aiTotal:0});const U=(m.round||0)+1,Z=f==="p1"?"p2":"p1",ne={...m,[f+"Team"]:k,[f+"Score"]:q},oe=De(ne);B.add(U),tt(D,q,m[y+"Score"]||0,!0,async()=>{await L({[f+"Team"]:k,[f+"Score"]:q,["selected_"+f]:[],modifiers:{...m.modifiers,[f]:{}},pendingAttack:null,phase:oe?"finished":Z+"-attack",attacker:Z,round:U,log:A}),oe&&await Ce({...m,[f+"Score"]:q})});return}A.push({type:"pending",text:`⚔️ ${m[f+"Name"]} attaque (${C.total})`}),await L({[f+"Team"]:k,[y+"Team"]:m[y+"Team"],pendingAttack:{...C,players:z,side:f},["selected_"+f]:[],modifiers:{...m.modifiers,[f]:{}},phase:y+"-defense",log:A})}async function ke(){const k=m[f+"Team"],S=(m["selected_"+f]||[]).map(me=>{const _e=(k[me._role]||[]).find(je=>je.cardId===me.cardId)||me,$e=k[me._role]||[],re=$e.findIndex(je=>je.cardId===me.cardId),Ae=lt($e.length),Ne=re>=0?Ae[re]:_e._col??1;return{..._e,_line:me._role,_col:Ne}}),z=pi(S,m.modifiers[f]||{},m[f+"Formation"]);pe(f,S.map(me=>me.cardId)),S.forEach(me=>{const _e=(k[me._role]||[]).find($e=>$e.cardId===me.cardId);_e&&(_e.used=!0)}),m["selected_"+f]=[],O();const C=co(m.pendingAttack.total,z.total,m.modifiers[f]||{}),A=m.pendingAttack.side,R=C==="attack"||(C==null?void 0:C.goal),q=A==="p1"?"p2":"p1",D=(m.round||0)+1,U=(m.pendingAttack.players||[]).map(me=>Gt(me)),Z=[...m.log||[]];Z.push({type:"duel",isGoal:R,homeScored:R&&A===f,text:R?`⚽ BUT de ${m[A+"Name"]} ! (${m.pendingAttack.total} vs ${z.total})`:`✋ Attaque stoppée (${m.pendingAttack.total} vs ${z.total})`,homePlayers:U,aiPlayers:S.map(me=>Gt(me)),homeTotal:m.pendingAttack.total,aiTotal:z.total});const ne=R?(m[A+"Score"]||0)+1:m[A+"Score"]||0,oe={...m,[f+"Team"]:k,[A+"Score"]:ne},te=De(oe),se=te?"finished":q+"-attack",fe=!te&&!R?H(oe):null,we=async()=>{if(fe){await ce(fe,oe);return}await L({[f+"Team"]:k,[y+"Team"]:m[y+"Team"],[A+"Score"]:ne,["selected_"+f]:[],modifiers:{...m.modifiers,[f]:{}},pendingAttack:null,phase:se,attacker:q,round:D,log:Z}),(se==="finished"||te)&&await Ce({...m,[A+"Score"]:ne})};if(R){const me=A===f,_e=me?ne:m[f+"Score"]||0,$e=me?m[y+"Score"]||0:ne;B.add(D),tt(U,_e,$e,me,we)}else await we()}function Ee(k){return["MIL","ATT"].some(S=>(k[S]||[]).some(z=>!z.used))}function Se(k){return["GK","DEF","MIL","ATT"].some(S=>(k[S]||[]).some(z=>!z.used))}function ye(k){return Se(k)&&!Ee(k)}function Le(k){const S=m[k+"Team"],z=m[(k==="p1"?"p2":"p1")+"Team"];return!!(Ee(S)||ye(S)&&!Se(z)||ye(S)&&!Ee(z)&&Se(z))}function De(k){return!Se(k.p1Team)&&!Se(k.p2Team)}function Me(k){const S=k.p1Score||0,z=k.p2Score||0;return S===z?null:S>z?v.home_id:v.away_id}async function Ce(k){Ot(),Ve();try{const S=Me(k),z=S?v.home_id===S?v.away_id:v.home_id:null,C=k.p1Score||0,A=k.p2Score||0,{data:R,error:q}=await h.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:S,p_home_score:C,p_away_score:A,p_forfeit:!1});if(q&&console.error("[PvP] finish_pvp_match:",q),l)try{await h.rpc("check_and_finish_mini_league",{p_league_id:l})}catch(D){console.error("[PvP] check_and_finish_mini_league:",D)}S&&z&&aa(S,z).catch(()=>{});try{const{data:D}=await h.rpc("apply_match_rewards",{p_match_id:i});D!=null&&D.success&&!(D!=null&&D.skipped)&&typeof e.refreshProfile=="function"&&await e.refreshProfile()}catch(D){console.error("[PvP] apply_match_rewards:",D)}if(typeof s=="function")try{await s({homeScore:C,awayScore:A,winnerId:S,homeId:v.home_id,awayId:v.away_id,matchId:i})}catch(D){console.error("[PvP] onMatchEnd:",D)}}catch(S){console.error("[PvP] finishMatch:",S)}}function He(){var A;const k=m[f+"Score"],S=m[y+"Score"],z=k>S,C=k===S;ct(t),t.innerHTML=`
    ...
    </div>`,(A=document.getElementById("pvp-home"))==null||A.addEventListener("click",()=>{try{h.removeChannel(K)}catch{}Ye(t),l?g("mini-league",{openLeagueId:l}):g("home")})}O()}async function Xi(t,e,i=!1){var d,c;const{state:o,navigate:r,toast:n}=e,a=i&&((c=(d=e==null?void 0:e.state)==null?void 0:d.params)==null?void 0:c.rankedData)||null;await Ci(t,e,i?"ranked":"random",async({deckId:s,formation:l,starters:p,subsRaw:b,gcCardsEnriched:g,gcDefs:x,stadiumDef:f})=>{ct(t);const y=async u=>{await la(t,e,s,l,p,b,u||[],x,f,i,a)};if(!(g!=null&&g.length)){await y([]);return}zi(t,g,y)})}async function da(t,e,i){return sa(t,e,i)}async function la(t,e,i,o,r,n,a=[],d=[],c=null,s=!1,l=null){var Y,K;const{state:p,navigate:b,toast:g}=e,x=(l==null?void 0:l.mmr)??((Y=p.profile)==null?void 0:Y.mmr)??1e3,f=s?zt(x):null,y=s?"linear-gradient(135deg, #1a0a2e 0%, #2d1060 50%, #1a0a2e 100%)":"linear-gradient(135deg, #0a1a2e 0%, #0d3d1e 100%)",u=s?(f==null?void 0:f.color)||"#D4A017":"#FFD700";let w=!1,v=null,E=null;t.innerHTML=`
  ...
  <div class="page">
    ...
    <nav class="top-nav">
      <div class="logo" id="nav-logo" title="Manager Wars v2026.07.31-1930" style="cursor:pointer">
