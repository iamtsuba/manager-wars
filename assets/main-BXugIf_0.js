...
import{s as v,j as _i,q as Ao,t as Kt,u as Ui,F as Ki,o as it,n as Ot,g as Ct,r as Se,m as wt,v as mi,w as zo,T as Si,p as gn,x as xn,d as bn}from"./special-cards-lEGt-tGs.js";const yn="/";function po(t,{navigate:e,toast:i}){let o="login";const r=()=>{var a,s,d,c,l,f,b;const n=o==="login";if(t.innerHTML=`
  ...
  </div>`,t.querySelectorAll("[data-action]").forEach(a=>{a.addEventListener("click",()=>{a.style.transform="scale(.96)",setTimeout(()=>a.style.transform="",180);const s=a.dataset.action;if(s==="match-ai"){Po(i,e);return}if(s==="match-random"){$t(),i("match",{matchMode:"random"});return}if(s==="match-friend"){i("friends");return}if(s==="mini-league"){i("mini-league");return}if(s==="ranked"){i("ranked");return}o("Bientôt disponible","info")})})}async function Ro(t,e){var f;const{state:i,navigate:o}=e,r=pn(),n=Vi(),a=(f=i==null?void 0:i.profile)==null?void 0:f.is_admin,s="2026.07.27-0006";t.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    ${Se(t,{width:140})}
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),c=(s||[]).filter($=>$.card_type==="player"&&$.player),l=(s||[]).filter($=>$.card_type==="game_changer"),f=(s||[]).filter($=>$.card_type==="formation"),b=(s||[]).filter($=>$.card_type==="stadium"),{data:x}=await v.from("gc_definitions").select("name,gc_type,color,effect,image_url"),g={};(x||[]).forEach($=>{g[$.name]=$});const{data:p}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),u={};(p||[]).forEach($=>{u[$.id]=$}),b.forEach($=>{$.stadium_def&&(u[$.stadium_id]=$.stadium_def)});const y=Object.keys(Ki),w=Object.keys(Ae),h={};c.forEach($=>{const C=$.player.id;h[C]=(h[C]||0)+1}),new Set(Object.keys(h).map($=>String($)));const L=new Set(f.map($=>$.formation)),m=new Set(l.map($=>$.gc_type));let z="player",E="Tous",A="",D=!1;function O(){return[...c].sort(($,C)=>{const R=li.indexOf($.player.job),P=li.indexOf(C.player.job);return R!==P?R-P:($.player.surname_real||"").localeCompare(C.player.surname_real||"")})}function K(){return[...d||[]].sort(($,C)=>{const R=li.indexOf($.job),P=li.indexOf(C.job);return R!==P?R-P:($.surname_real||"").localeCompare(C.surname_real||"")})}function X(){return O().filter($=>{const C=$.player,R=E==="Tous"||C.job===E,P=!A||`${C.firstname} ${C.surname_real}`.toLowerCase().includes(A);return R&&P})}function S(){return K().filter($=>{const C=E==="Tous"||$.job===E,R=!A||`${$.firstname} ${$.surname_real}`.toLowerCase().includes(A);return C&&R})}t.innerHTML=`
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(j=document.getElementById("close-detail"))==null||j.addEventListener("click",s);let S=new Set;const _=l.rarity==="pepite"?1.3:l.rarity==="papyte"?.7:1;function H(){let $=0;return document.querySelectorAll(".expl-check:checked").forEach(C=>{if(C.dataset.id===t.id)return;const P=Number(C.dataset.evo)||0,J=Number(C.dataset.note)||0;$+=J+P}),Math.round($*_)}const W=()=>{const $=S.size,C=document.getElementById("sell-action-panel");if(!C)return;C.style.display=$>0?"block":"none",document.getElementById("sell-selected-count").textContent=$;const R=document.getElementById("evolve-btn");if(R){const P=H();R.textContent=P>0?`⬆️ Évoluer (+${P})`:"⬆️ Évoluer"}};document.querySelectorAll(".expl-check").forEach($=>{$.addEventListener("change",()=>{const C=$.dataset.id;$.checked?S.add(C):S.delete(C);const R=$.closest(".exemplaire-row");if(R){const P=R.querySelector(".expl-sel-overlay"),J=R.querySelector(".expl-sel-check");P&&(P.style.display=$.checked?"block":"none"),J&&(J.style.display=$.checked?"flex":"none"),R.style.transform=$.checked?"scale(1.05)":"scale(1)"}W()})}),document.querySelectorAll(".exemplaire-row").forEach($=>{$.addEventListener("click",C=>{if(C.target.tagName==="INPUT")return;const R=$.querySelector(".expl-check");R&&!R.disabled&&(R.checked=!R.checked,R.dispatchEvent(new Event("change")))})}),(U=document.getElementById("evolve-btn"))==null||U.addEventListener("click",async()=>{if(b<=1)return;const $=[...S];if(!$.length)return;if(S.has(t.id)){const oe=document.createElement("div");oe.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",oe.innerHTML=`
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    </div>`,requestAnimationFrame(()=>requestAnimationFrame(function X(){const S=t.querySelector(".deck-preview-wrap"),_=t.querySelector("#deck-swipe-zone");if(!S||!_||!y)return;const H=_.clientWidth>=900,W=Math.max(200,_.clientHeight-(H?60:40)),le=Math.max(200,_.clientWidth-16),j=H?Math.min(117,Math.max(52,Math.round(le*.22))):Math.max(44,Math.round(le*.168));if(W<220||le<220){requestAnimationFrame(X);return}const U=H?null:Math.round(j*.55);S.innerHTML=Ht(y,u,null,[],le,W,[],U),S.style.cssText=`width:${le}px;height:${W}px;overflow:hidden;margin:${H?0:60}px auto 0`;const de=S.querySelector("svg");de&&(de.style.cssText="display:block;width:100%;height:100%",de.setAttribute("preserveAspectRatio",H?"xMidYMid meet":"none"))})),(A=document.getElementById("prev-deck"))==null||A.addEventListener("click",()=>{l>0&&(l--,f())}),(D=document.getElementById("next-deck"))==null||D.addEventListener("click",()=>{l<n.length-1&&(l++,f())}),(O=document.getElementById("validate-deck"))==null||O.addEventListener("click",()=>{if(!h)return;const X=e.state.params||{};e.navigate("match",{...X,matchMode:X.matchMode||i,deckId:b.id})}),(K=document.getElementById("cancel-deck-select"))==null||K.addEventListener("click",()=>{Ke(t),r("home")});const L=document.getElementById("deck-swipe-zone");if(L){let X=0,S=0;L.addEventListener("touchstart",_=>{X=_.touches[0].clientX,S=_.touches[0].clientY},{passive:!0}),L.addEventListener("touchend",_=>{const H=_.changedTouches[0].clientX-X,W=_.changedTouches[0].clientY-S;Math.abs(H)<40||Math.abs(H)<Math.abs(W)||(H<0&&l<n.length-1?(l++,f()):H>0&&l>0&&(l--,f()))},{passive:!0})}}f()}function vt(t,e=44,i=58,o=null){return Se(t,{width:e,showStad:!!o,stadDef:o,used:t==null?void 0:t.used})}function Qe(t,e,i,o,r){if(!(t!=null&&t.length))return"";const n=t.slice(0,5);let a='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return n.forEach((s,d)=>{const c=s._line||s.job||"MIL";let l=s.boost||0;if(s.stadiumBonus&&(o==="attack"&&(c==="ATT"||c==="MIL")||o==="defense"&&(c==="GK"||c==="DEF"||c==="MIL")?l+=10:o||(l+=10)),a+=Se(s,{width:40,role:c,extraNote:l}),d<n.length-1){const f=n[d+1],b=Oo(s,f,r)?Kt(s,f):null;a+=`<div style="width:7px;height:3px;background:${!b||b==="#ff3333"||b==="#cc2222"?"rgba(255,255,255,0.12)":b};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(a+=`<div style="margin-left:6px;background:${e};color:${e==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),a+="</div>",a}function $i(t,e,i,o,r=310,n=310,a=[],s=null){const d=wt[e]||{},c=Ui(e)||Ki[e]||[],l={},f=["ATT","MIL","DEF","GK"];for(const y of f)(t[y]||[]).forEach((h,L)=>{l[`${y}${L+1}`]=h});function b(y){const w=d[y];return w?{x:w.x*r,y:w.y*n}:null}let x="";for(const[y,w]of c){const h=b(y),L=b(w);if(!h||!L)continue;const m=l[y],z=l[w],E=Kt(m,z);E==="#00ff88"||E==="#FFD700"?(x+=`<line x1="${h.x.toFixed(1)}" y1="${h.y.toFixed(1)}" x2="${L.x.toFixed(1)}" y2="${L.y.toFixed(1)}"
  ...
  <div id="match-history-panel">
    ...
    </div>`,(m=f.querySelector("#sub-close"))==null||m.addEventListener("click",()=>f.remove()),(z=f.querySelector("#out-up"))==null||z.addEventListener("click",()=>{s>0&&(s--,b())}),(E=f.querySelector("#out-down"))==null||E.addEventListener("click",()=>{s<n.length-1&&(s++,b())}),(A=f.querySelector("#in-up"))==null||A.addEventListener("click",()=>{c>0&&(c--,b())}),(D=f.querySelector("#in-down"))==null||D.addEventListener("click",()=>{c<a.length-1&&(c++,b())});const L=(K,X,S,_)=>{const H=f.querySelector("#"+K);if(!H)return;let W=0;H.addEventListener("touchstart",ae=>{W=ae.touches[0].clientY},{passive:!0}),H.addEventListener("touchend",ae=>{const le=ae.changedTouches[0].clientY-W;if(Math.abs(le)<30)return;const j=X();le<0&&j<_-1?(S(j+1),b()):le>0&&j>0&&(S(j-1),b())},{passive:!0})};L("in-panel",()=>c,K=>c=K,a.length),L("out-panel",()=>s,K=>s=K,n.length),(O=f.querySelector("#sub-confirm"))==null||O.addEventListener("click",K=>{if(K.preventDefault(),K.stopPropagation(),l)return;l=!0;const X=n[s],S=a[c];if(!X||!S)return;let _=null,H=-1;for(const[ae,le]of Object.entries(e.homeTeam)){const j=(le||[]).findIndex(U=>U.cardId===X.cardId);if(j!==-1){_=ae,H=j;break}}if(H===-1||!_){Tt("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),f.remove();return}const W={...S,_line:_,_col:X._col||0,used:!1,boost:0};e.homeTeam[_].splice(H,1,W),e.usedSubIds||(e.usedSubIds=[]),e.usedSubIds.push(S.cardId),e.subsUsed++,e.selected=[],e.log.push({type:"sub",subSide:"home",clubName:e.clubName,outPlayer:{name:X.name,firstname:X.firstname,note:Pe(X,_),portrait:Ct(X),job:X.job,country_code:X.country_code,rarity:X.rarity,clubName:X.clubName,clubLogo:X.clubLogo},inPlayer:{name:S.name,firstname:S.firstname,note:Pe(S,_),portrait:Ct(S),job:S.job,country_code:S.country_code,rarity:S.rarity,clubName:S.clubName,clubLogo:S.clubLogo},text:`🔄 ${S.firstname} ${S.name} remplace ${X.firstname} ${X.name}`}),f.remove(),vr(X,S,()=>ke(t,e,i))})}document.body.appendChild(f),b()}function zr(t,e,i,o,r){var b,x;const n=(o.gcDefs||[]).find(g=>g.name===e),a=Ae[e]||{icon:"⚡",desc:"Carte spéciale."},s=(n==null?void 0:n.name)||e,d=(n==null?void 0:n.effect)||a.desc,c=n!=null&&n.image_url?`/icons/${n.image_url}`:null,l=a.icon||"⚡",f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",f.innerHTML=`
    ...
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      ...
      ${$i(t.aiTeam,t.formation,null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.1))}
            ...
            </div>`}}catch(ee){console.error("[Ranked] MMR update error:",ee)}(ie=document.getElementById("pvp-end-overlay"))==null||ie.remove();const F=document.createElement("div");F.id="pvp-end-overlay",F.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;color:#fff;padding:24px;text-align:center;overflow-y:auto";const N=I?"🤝":B?"🏆":"😞",Y=I?"MATCH NUL":B?"VICTOIRE !":"DÉFAITE",Z=I?"#fff":B?"#FFD700":"#ff6b6b";F.innerHTML=`
      ...
      ${G}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${l?"🏆 Retour à la Mini League":a?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(F),(te=F.querySelector("#pvp-end-home"))==null||te.addEventListener("click",()=>{F.remove(),Ke(t),l?x("mini-league",{openLeagueId:l}):x(a?"ranked":"home")})}const X=v.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},k=>{var M;const T=k.new;try{if(T.status==="finished"||T.forfeit){if(z)return;z=!0,E&&(clearInterval(E),E=null),Ue(),T.game_state&&(m=T.game_state),K(T);return}if(T.game_state){const B=m;m=T.game_state;const I=m._lastGC;if(I&&I.seq&&!O.has(I.seq)&&(O.add(I.seq),I.by!==p)){$(I.type,I.by,()=>U());return}const G=B[p+"Score"]||0,F=B[u+"Score"]||0,N=m[p+"Score"]||0,Y=m[u+"Score"]||0,Z=N>G,ie=Y>F;if((Z||ie)&&!D.has(m.round)){D.add(m.round);const te=[...m.log||[]].reverse().find(se=>se.isGoal),ee=((te==null?void 0:te.homePlayers)||[]).map(se=>({name:se.name,note:se.note,portrait:se.portrait,job:se.job}));!!((M=te==null?void 0:te.text)!=null&&M.includes("DERNIER CORNER"))?we().then(()=>{Ze(ee,N,Y,Z,()=>U())}):Ze(ee,N,Y,Z,()=>U());return}U()}}catch(B){console.error("[PvP] crash:",B)}}).subscribe();async function S(k){Object.assign(m,k),m.lastActionAt=new Date().toISOString();const{error:T}=await v.from("matches").update({game_state:m}).eq("id",i);T&&g("Erreur de synchronisation","error");try{U()}catch(M){console.error("[PvP] renderPvpScreen crash:",M)}}async function _(){if(z)return;z=!0,Gt(),Ue(),E&&(clearInterval(E),E=null);const k=o?h.away_id:h.home_id,T=o?"p2":"p1",M=o?"p1":"p2",B={...m,[T+"Score"]:3,[M+"Score"]:0,phase:"finished"},I=B.p1Score||0,G=B.p2Score||0;try{await v.from("matches").update({game_state:B}).eq("id",i);const{data:F,error:N}=await v.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:k,p_home_score:I,p_away_score:G,p_forfeit:!0});if(N&&console.error("[PvP] finish_pvp_match (forfeit):",N),l)try{await v.rpc("check_and_finish_mini_league",{p_league_id:l})}catch(Y){console.error("[PvP] check_and_finish_mini_league (forfeit):",Y)}try{const{data:Y}=await v.rpc("apply_match_rewards",{p_match_id:i});Y!=null&&Y.success&&!(Y!=null&&Y.skipped)&&typeof e.refreshProfile=="function"&&await e.refreshProfile()}catch(Y){console.error("[PvP] apply_match_rewards (forfeit):",Y)}}catch{}try{v.removeChannel(X)}catch{}setTimeout(()=>{Ke(t),l?x("mini-league",{openLeagueId:l}):x("home")},800)}const H={BOOST_STAT:({value:k=1,count:T=1,roles:M=[]},B,I)=>{const G=B[p+"Team"],F=Object.entries(G).filter(([N])=>!M.length||M.includes(N)).flatMap(([N,Y])=>Y.filter(Z=>!Z.used).map(Z=>({...Z,_line:N})));if(!F.length){B.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),I(B);return}W(F,T,`Choisir ${T} joueur(s) à booster (+${k})`,N=>{N.forEach(Y=>{const Z=(G[Y._line]||[]).find(ie=>ie.cardId===Y.cardId);Z&&(Z.boost=(Z.boost||0)+k,B.log.push({text:`⚡ +${k} sur ${Z.name}`,type:"info"}))}),B[p+"Team"]=G,I(B)})},DEBUFF_STAT:({value:k=1,count:T=1,roles:M=[],target:B="ai"},I,G)=>{const F=B==="home"?p:u,N=I[F+"Team"],Y=B==="home"?"allié":"adverse",Z=Object.entries(N).filter(([ie])=>!M.length||M.includes(ie)).flatMap(([ie,te])=>te.map(ee=>({...ee,_line:ie})));if(!Z.length){I.log.push({text:`🎯 Aucun joueur ${Y}`,type:"info"}),G(I);return}W(Z,T,`Choisir ${T} joueur(s) ${Y}(s) (-${k})`,ie=>{ie.forEach(te=>{const ee=(N[te._line]||[]).find(re=>re.cardId===te.cardId);ee&&(ee.boost=(ee.boost||0)-k,I.log.push({text:`🎯 -${k} sur ${ee.name}`,type:"info"}))}),I[F+"Team"]=N,G(I)})},GRAY_PLAYER:({count:k=1,roles:T=[],target:M="ai"},B,I)=>{const G=M==="home"?p:u,F=B[G+"Team"],N=M==="home"?"allié":"adverse",Y=Object.entries(F).filter(([Z])=>!T.length||T.includes(Z)).flatMap(([Z,ie])=>ie.filter(te=>!te.used).map(te=>({...te,_line:Z})));if(!Y.length){B.log.push({text:`❌ Aucun joueur ${N}`,type:"info"}),I(B);return}W(Y,k,`Choisir ${k} joueur(s) ${N}(s) à exclure`,Z=>{const ie="usedCardIds_"+G,te=new Set(B[ie]||[]);Z.forEach(ee=>{const re=(F[ee._line]||[]).find(se=>se.cardId===ee.cardId);re&&(re.used=!0,te.add(ee.cardId),B.log.push({text:`❌ ${re.name} exclu !`,type:"info"}))}),B[ie]=[...te],B[G+"Team"]=F,I(B)})},REVIVE_PLAYER:({count:k=1,roles:T=[]},M,B)=>{const I=M[p+"Team"],G=Object.entries(I).filter(([F])=>!T.length||T.includes(F)).flatMap(([F,N])=>N.filter(Y=>Y.used).map(Y=>({...Y,_line:F})));if(!G.length){M.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),B(M);return}W(G,k,`Choisir ${k} joueur(s) à ressusciter`,F=>{F.forEach(N=>{const Y=(I[N._line]||[]).find(Z=>Z.cardId===N.cardId);Y&&(Y.used=!1,M.log.push({text:`💫 ${Y.name} ressuscité !`,type:"info"}))}),M[p+"Team"]=I,B(M)})},REMOVE_GOAL:({},k,T)=>{const M=u+"Score";k[M]>0?(k[M]--,k.log.push({text:"🚫 Dernier but annulé !",type:"info"})):k.log.push({text:"🚫 Aucun but à annuler",type:"info"}),T(k)},ADD_GOAL_DRAW:({},k,T)=>{k[p+"Score"]===k[u+"Score"]?(k[p+"Score"]++,k.log.push({text:"🎯 But bonus !",type:"info"})):k.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),T(k)},ADD_SUB:({value:k=1},T,M)=>{T.maxSubs=(T.maxSubs||3)+k,T.log.push({text:`🔄 +${k} remplacement(s)`,type:"info"}),M(T)},CUSTOM:({},k,T)=>T(k)};function W(k,T,M,B){const I=document.createElement("div");I.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let G=[];function F(){var Y,Z;const N=k.map(ie=>{const te=ie._line||ie.job||"MIL",ee=G.find(se=>se.cardId===ie.cardId),re=Se({...ie,_evolution_bonus:0},{width:90,showStad:!0,role:te,extraNote:ie.boost||0});return`<div class="pp-item" data-cid="${ie.cardId}"
        ...
        </div>`,(Y=I.querySelector("#pp-close"))==null||Y.addEventListener("click",()=>I.remove()),I.querySelectorAll(".pp-item").forEach(ie=>{ie.addEventListener("click",()=>{const te=ie.dataset.cid,ee=k.find(se=>se.cardId===te),re=G.findIndex(se=>se.cardId===te);ee&&(re>-1?G.splice(re,1):G.length<T&&G.push(ee),F())})}),(Z=I.querySelector("#pp-confirm"))==null||Z.addEventListener("click",()=>{I.remove(),B(G)})}F(),document.body.appendChild(I)}async function ae(k,T){const B=(m["gcCardsFull_"+p]||[]).find(N=>N.id===k),I=(B==null?void 0:B._gcDef)||(m.gcDefs||[]).find(N=>{var Y;return N.name===T||((Y=N.name)==null?void 0:Y.toLowerCase().trim())===(T==null?void 0:T.toLowerCase().trim())}),G=[...m["usedGc_"+p]||[],k],F={type:T,by:p,seq:(m._gcAnimSeq||0)+1};O.add(F.seq),$(T,p,async()=>{if(I!=null&&I.effect_type&&I.effect_type!=="CUSTOM"){const Y=H[I.effect_type];if(Y){const Z={...m};Y(I.effect_params||{},Z,async ie=>{ie["usedGc_"+p]=G,ie._lastGC=F,ie._gcAnimSeq=F.seq,await S(ie)});return}}const N={...m};switch(T){case"Remplacement+":N.maxSubs=(N.maxSubs||3)+1,N.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const Y=u+"Score";N[Y]>0&&(N[Y]--,N.log.push({text:"🚫 But annulé",type:"info"}));break}}N["usedGc_"+p]=G,N._lastGC=F,N._gcAnimSeq=F.seq,await S(N)})}function le(k,T){const M="usedCardIds_"+k,B=new Set(m[M]||[]);T.forEach(I=>B.add(I)),m[M]=[...B]}function j(){for(const k of["p1","p2"]){const T=new Set(m["usedCardIds_"+k]||[]),M=m[k+"Team"];if(M)for(const B of["GK","DEF","MIL","ATT"])(M[B]||[]).forEach(I=>{I.used=T.has(I.cardId)})}}function U(){var no,ro,ao,so,lo,co;if(m.phase==="reveal")return de();if(m.phase==="midfield")return ue();if(m.phase==="finished")return Ne();const k=m[p+"Team"],T=m[u+"Team"];j();const M=m[p+"Score"],B=m[u+"Score"],I=m[p+"Name"],G=m[u+"Name"],F=m.phase===p+"-attack",N=m.phase===p+"-defense",Y=Array.isArray(m["selected_"+p])?m["selected_"+p]:[],Z=Y.map(Q=>Q.cardId),ie=window.innerWidth>=700,te=m[p+"Subs"]||[],ee=m["usedSubIds_"+p]||[],re=te.filter(Q=>!ee.includes(Q.cardId)),se=m["gcCardsFull_"+p]||[],xe=m["usedGc_"+p]||[],ce=se.filter(Q=>!xe.includes(Q.id)),be=m.boostOwner===p&&!m.boostUsed,ve=[...k.MIL||[],...k.ATT||[]].filter(Q=>!Q.used),ne=[...T.MIL||[],...T.ATT||[]].filter(Q=>!Q.used),Le=!$e(T),Ge=ve.length===0&&ne.length===0&&!Le,Ie=(k.DEF||[]).filter(Q=>!Q.used),Ye=(k.GK||[]).filter(Q=>!Q.used);let Oe=[];F&&ve.length===0&&(Le?(Oe=Ie.map(Q=>Q.cardId),Ie.length===0&&(Oe=Oe.concat(Ye.map(Q=>Q.cardId)))):Ge&&(Oe=[...Ie,...Ye].map(Q=>Q.cardId)));function ot(Q,ge,Me){var Ce,mt;const _e=Q._gcDef,Dt=(_e==null?void 0:_e.name)||Q.gc_type,at=(_e==null?void 0:_e.effect)||((Ce=Ae[Q.gc_type])==null?void 0:Ce.desc)||"",Fe=_e!=null&&_e.image_url?`/icons/${_e.image_url}`:null,Re=((mt=Ae[Q.gc_type])==null?void 0:mt.icon)||"⚡";return`<div class="pvp-gc-mini" data-gc-id="${Q.id}" data-gc-type="${Q.gc_type}" style="flex-shrink:0;cursor:pointer">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(te);let ee=!1;const re=()=>{ee||(ee=!0,te.remove(),setTimeout(()=>M&&M(),50))};te.addEventListener("click",re),setTimeout(re,3e3)}function C(k,T){var ie,te,ee,re;const B=(m["gcCardsFull_"+p]||[]).find(se=>se.id===k),I=B==null?void 0:B._gcDef,G=(I==null?void 0:I.name)||T,F=(I==null?void 0:I.effect)||((ie=Ae[T])==null?void 0:ie.desc)||"Carte spéciale.",N=I!=null&&I.image_url?`/icons/${I.image_url}`:null,Y=((te=Ae[T])==null?void 0:te.icon)||"⚡",Z=document.createElement("div");Z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",Z.innerHTML=`
      ${it(G,N,Y,F,{width:190})}
      <div style="display:flex;gap:12px;width:190px">
        <button id="pvp-gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
        <button id="pvp-gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
      </div>`,document.body.appendChild(Z),(ee=Z.querySelector("#pvp-gc-back"))==null||ee.addEventListener("click",()=>Z.remove()),(re=Z.querySelector("#pvp-gc-use"))==null||re.addEventListener("click",()=>{Z.remove(),ae(k,T)})}function R(){var B;const k=m[p+"Team"],T=Object.entries(k).flatMap(([I,G])=>(G||[]).filter(F=>!F.used).map(F=>({...F,_line:I})));if(!T.length)return;const M=document.createElement("div");M.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",M.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
      ...
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${T.map(I=>{const G={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[I._line]||"#555",F=Pe(I,I._line)+(I.boost||0);return`<div class="bp-item" data-cid="${I.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${G};overflow:hidden;cursor:pointer">
      ...
      </div>`,(Ge=te.querySelector("#psub-close"))==null||Ge.addEventListener("click",()=>te.remove()),(Ie=te.querySelector("#pout-up"))==null||Ie.addEventListener("click",()=>{N>0&&(N--,ee())}),(Ye=te.querySelector("#pout-down"))==null||Ye.addEventListener("click",()=>{N<G.length-1&&(N++,ee())}),(Oe=te.querySelector("#pin-up"))==null||Oe.addEventListener("click",()=>{Z>0&&(Z--,ee())}),(ot=te.querySelector("#pin-down"))==null||ot.addEventListener("click",()=>{Z<F.length-1&&(Z++,ee())});const Le=(De,We,He,ut)=>{const nt=te.querySelector("#"+De);if(!nt)return;let Bt=0;nt.addEventListener("touchstart",ft=>{Bt=ft.touches[0].clientY},{passive:!0}),nt.addEventListener("touchend",ft=>{const Lt=ft.changedTouches[0].clientY-Bt;if(Math.abs(Lt)<30)return;const rt=We();Lt<0&&rt<ut-1?(He(rt+1),ee()):Lt>0&&rt>0&&(He(rt-1),ee())},{passive:!0})};Le("pin-panel",()=>Z,De=>Z=De,F.length),Le("pout-panel",()=>N,De=>N=De,G.length),(Vt=te.querySelector("#psub-confirm"))==null||Vt.addEventListener("click",async De=>{if(De.preventDefault(),De.stopPropagation(),ie)return;ie=!0;const We=G[N],He=F[Z];if(!We||!He)return;const ut=We._line,nt=(M[ut]||[]).findIndex(Lt=>Lt.cardId===We.cardId);if(nt===-1){g("Erreur : joueur introuvable","error"),te.remove();return}const Bt={...He,_line:ut,position:We.position,used:!1,boost:0};M[ut].splice(nt,1,Bt);const ft=[...B,He.cardId];te.remove(),J(We,He,async()=>{await S({[p+"Team"]:M,[u+"Team"]:m[u+"Team"],["usedSubIds_"+p]:ft})})})}document.body.appendChild(te),ee()}function J(k,T,M){const B={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},I=document.createElement("div");I.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const G=(Y,Z,ie)=>`<div style="text-align:center">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:6px">Appuyer pour continuer</div>`,document.body.appendChild(I);let F=!1;const N=()=>{F||(F=!0,I.remove(),setTimeout(()=>M(),50))};I.addEventListener("click",N),setTimeout(N,2200)}function q(){var T;const k=document.createElement("div");k.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:20px;overflow-y:auto",k.innerHTML=`
      ...
      </div>`,document.body.appendChild(T),(M=T.querySelector("#pvp-hist-close"))==null||M.addEventListener("click",()=>T.remove())}async function pe(){if(m.phase!==p+"-attack")return;const k=p==="p1"?"p2":"p1",T=(m.round||0)+1,M=[...m.log||[]];M.push({type:"info",text:`⏭️ ${m[p+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const B=Ee(m),I=pt(k),G=B||!I?"finished":k+"-attack";await S({["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},pendingAttack:null,phase:G,attacker:k,round:T,log:M}),G==="finished"&&await qe(m)}function oe(k){const T=(k.GK||[]).some(B=>!B.used),M=["DEF","MIL","ATT"].some(B=>(k[B]||[]).some(I=>!I.used));return T&&!M}function me(k){return(k.p1Score||0)!==(k.p2Score||0)?null:oe(k.p1Team)&&!$e(k.p2Team)?{side:"p1",gk:(k.p1Team.GK||[]).find(T=>!T.used)}:oe(k.p2Team)&&!$e(k.p1Team)?{side:"p2",gk:(k.p2Team.GK||[]).find(T=>!T.used)}:null}async function we(){const k=document.createElement("div");k.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:950;display:flex;align-items:center;justify-content:center;overflow:hidden;text-align:center;padding:24px",document.body.appendChild(k);const T=(M,B)=>new Promise(I=>{k.innerHTML=`<div style="font-size:32px;font-weight:900;color:${B};letter-spacing:2px;animation:lcFade 1.4s ease both">${M}</div>
      <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(I,1400)});await T("⚽ DERNIER CORNER","#FFD700"),await T("🧤 LE GARDIEN MONTE !","#4fc3f7"),k.remove()}async function he(k,T){const{side:M,gk:B}=k;if(!B)return;const I=M+"Score",G=(T[I]||0)+1;B.used=!0;const F=[...T.log||[]];F.push({type:"duel",isGoal:!0,homeScored:M===p,homePlayers:M==="p1"?[Ft(B)]:[],aiPlayers:M==="p2"?[Ft(B)]:[],text:`⚽ DERNIER CORNER — Le gardien de ${T[M+"Name"]} marque !`});const N=(T.round||0)+1,Y=M==="p1"?"p2":"p1",Z={...T,[I]:G},ie=Ee(Z);D.add(N),await we();const te=M===p?G:T[p+"Score"]||0,ee=M===p?T[u+"Score"]||0:G;Ze([Ft(B)],te,ee,M===p,async()=>{await S({[I]:G,log:F,round:N,pendingAttack:null,phase:ie?"finished":Y+"-attack",attacker:Y,["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}}}),ie&&await qe({...m,[I]:G})})}async function Te(){const k=m[p+"Team"],T=!["GK","DEF","MIL","ATT"].some(G=>(m[u+"Team"][G]||[]).some(F=>!F.used)),M=(m["selected_"+p]||[]).map(G=>{const F=(k[G._role]||[]).find(se=>se.cardId===G.cardId)||G,N=["GK","DEF"].includes(G._role),Y=k[G._role]||[],Z=Y.findIndex(se=>se.cardId===G.cardId),ie=st(Y.length),te=Z>=0?ie[Z]:F._col??1,ee=m.stadiumDef||m.homeStadiumDef||null,re=F.stadiumBonus||ee&&(ee.club_id&&String(F.club_id)===String(ee.club_id)||ee.country_code&&F.country_code===ee.country_code)||!1;return{...F,_line:G._role,_col:te,stadiumBonus:re,...N?{note_a:Math.max(1,Number(F.note_a)||0)}:{}}});if(!M.length)return;const B=ni(M,m.modifiers[p]||{},m[p+"Formation"]);le(p,M.map(G=>G.cardId)),M.forEach(G=>{const F=(k[G._role]||[]).find(N=>N.cardId===G.cardId);F&&(F.used=!0)}),m["selected_"+p]=[],U();const I=[...m.log||[]];if(T){if(M.length===1&&(M[0]._line==="GK"||M[0].job==="GK")&&(m.p1Score||0)===(m.p2Score||0)){await he({side:p,gk:M[0]},{...m,[p+"Team"]:k,log:I});return}const F=(m[p+"Score"]||0)+1,N=M.map(ee=>Ft(ee));I.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:N,homeTotal:B.total,aiTotal:0});const Y=(m.round||0)+1,Z=p==="p1"?"p2":"p1",ie={...m,[p+"Team"]:k,[p+"Score"]:F},te=Ee(ie);D.add(Y),Ze(N,F,m[u+"Score"]||0,!0,async()=>{await S({[p+"Team"]:k,[p+"Score"]:F,["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},pendingAttack:null,phase:te?"finished":Z+"-attack",attacker:Z,round:Y,log:I}),te&&await qe({...m,[p+"Score"]:F})});return}I.push({type:"pending",text:`⚔️ ${m[p+"Name"]} attaque (${B.total})`}),await S({[p+"Team"]:k,[u+"Team"]:m[u+"Team"],pendingAttack:{...B,players:M,side:p},["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},phase:u+"-defense",log:I})}async function ze(){const k=m[p+"Team"],T=(m["selected_"+p]||[]).map(ce=>{const be=(k[ce._role]||[]).find(Ie=>Ie.cardId===ce.cardId)||ce,ve=k[ce._role]||[],ne=ve.findIndex(Ie=>Ie.cardId===ce.cardId),Le=st(ve.length),Ge=ne>=0?Le[ne]:be._col??1;return{...be,_line:ce._role,_col:Ge}}),M=ri(T,m.modifiers[p]||{},m[p+"Formation"]);le(p,T.map(ce=>ce.cardId)),T.forEach(ce=>{const be=(k[ce._role]||[]).find(ve=>ve.cardId===ce.cardId);be&&(be.used=!0)}),m["selected_"+p]=[],U();const B=Qi(m.pendingAttack.total,M.total,m.modifiers[p]||{}),I=m.pendingAttack.side,G=B==="attack"||(B==null?void 0:B.goal),F=I==="p1"?"p2":"p1",N=(m.round||0)+1,Y=(m.pendingAttack.players||[]).map(ce=>Ft(ce)),Z=[...m.log||[]];Z.push({type:"duel",isGoal:G,homeScored:G&&I===p,text:G?`⚽ BUT de ${m[I+"Name"]} ! (${m.pendingAttack.total} vs ${M.total})`:`✋ Attaque stoppée (${m.pendingAttack.total} vs ${M.total})`,homePlayers:Y,aiPlayers:T.map(ce=>Ft(ce)),homeTotal:m.pendingAttack.total,aiTotal:M.total});const ie=G?(m[I+"Score"]||0)+1:m[I+"Score"]||0,te={...m,[p+"Team"]:k,[I+"Score"]:ie},ee=Ee(te),re=ee?"finished":F+"-attack",se=!ee&&!G?me(te):null,xe=async()=>{if(se){await he(se,te);return}await S({[p+"Team"]:k,[u+"Team"]:m[u+"Team"],[I+"Score"]:ie,["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},pendingAttack:null,phase:re,attacker:F,round:N,log:Z}),(re==="finished"||ee)&&await qe({...m,[I+"Score"]:ie})};if(G){const ce=I===p,be=ce?ie:m[p+"Score"]||0,ve=ce?m[u+"Score"]||0:ie;D.add(N),Ze(Y,be,ve,ce,xe)}else await xe()}function Be(k){return["MIL","ATT"].some(T=>(k[T]||[]).some(M=>!M.used))}function $e(k){return["GK","DEF","MIL","ATT"].some(T=>(k[T]||[]).some(M=>!M.used))}function Et(k){return $e(k)&&!Be(k)}function pt(k){const T=m[k+"Team"],M=m[(k==="p1"?"p2":"p1")+"Team"];return!!(Be(T)||Et(T)&&!$e(M)||Et(T)&&!Be(M)&&$e(M))}function Ee(k){return!$e(k.p1Team)&&!$e(k.p2Team)}function je(k){const T=k.p1Score||0,M=k.p2Score||0;return T===M?null:T>M?h.home_id:h.away_id}async function qe(k){Gt(),Ue();try{const T=je(k),M=T?h.home_id===T?h.away_id:h.home_id:null,B=k.p1Score||0,I=k.p2Score||0,{data:G,error:F}=await v.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:T,p_home_score:B,p_away_score:I,p_forfeit:!1});if(F&&console.error("[PvP] finish_pvp_match:",F),l)try{await v.rpc("check_and_finish_mini_league",{p_league_id:l})}catch(N){console.error("[PvP] check_and_finish_mini_league:",N)}T&&M&&Fr(T,M).catch(()=>{});try{const{data:N}=await v.rpc("apply_match_rewards",{p_match_id:i});N!=null&&N.success&&!(N!=null&&N.skipped)&&typeof e.refreshProfile=="function"&&await e.refreshProfile()}catch(N){console.error("[PvP] apply_match_rewards:",N)}if(typeof c=="function")try{await c({homeScore:B,awayScore:I,winnerId:T,homeId:h.home_id,awayId:h.away_id,matchId:i})}catch(N){console.error("[PvP] onMatchEnd:",N)}}catch(T){console.error("[PvP] finishMatch:",T)}}function Ne(){var I;const k=m[p+"Score"],T=m[u+"Score"],M=k>T,B=k===T;lt(t),t.innerHTML=`
    ...
    </div>`,(I=document.getElementById("pvp-home"))==null||I.addEventListener("click",()=>{try{v.removeChannel(X)}catch{}Ke(t),l?x("mini-league",{openLeagueId:l}):x("home")})}U()}async function Ri(t,e,i=!1){var s,d;const{state:o,navigate:r,toast:n}=e,a=i&&((d=(s=e==null?void 0:e.state)==null?void 0:s.params)==null?void 0:d.rankedData)||null;await Ei(t,e,i?"ranked":"random",async({deckId:c,formation:l,starters:f,subsRaw:b,gcCardsEnriched:x,gcDefs:g,stadiumDef:p})=>{lt(t);const u=async y=>{await Gr(t,e,c,l,f,b,y||[],g,p,i,a)};if(!(x!=null&&x.length)){await u([]);return}ki(t,x,u)})}async function Nr(t,e,i){return Pr(t,e,i)}async function Gr(t,e,i,o,r,n,a=[],s=[],d=null,c=!1,l=null){var K,X;const{state:f,navigate:b,toast:x}=e,g=(l==null?void 0:l.mmr)??((K=f.profile)==null?void 0:K.mmr)??1e3,p=c?_i(g):null,u=c?"linear-gradient(135deg, #1a0a2e 0%, #2d1060 50%, #1a0a2e 100%)":"linear-gradient(135deg, #0a1a2e 0%, #0d3d1e 100%)",y=c?(p==null?void 0:p.color)||"#D4A017":"#FFD700";let w=!1,h=null,L=null;t.innerHTML=`
    ...
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:${Qt};border:1px solid ${Zt};border-radius:12px;padding:16px">
        ...
        ${f.map(S=>`
          <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid ${St}">
            <div style="width:36px;height:36px;border-radius:50%;background:${S.club_color2||sn};display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:900;color:${S.club_color1||"#fff"}">${(S.pseudo||"?").slice(0,2).toUpperCase()}</div>
      ...
      <div style="background:${Qt};border:1px solid ${Zt};border-radius:12px;padding:16px">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>${b.map((S,_)=>{const H=g>0&&a.status==="finished"?_===0?Math.floor(g*.7):_===1?Math.floor(g*.2):_===2?Math.floor(g*.1):0:0;return`<tr style="border-bottom:1px solid ${St};${S.userId===n?"background:rgba(74,222,128,0.08);":""}">
      ...
      <div style="background:${Qt};border:1px solid ${Zt};border-radius:12px;padding:16px">
        ...
        ${Array.from({length:Math.max(0,a.status==="active"?a.current_day-1:a.current_day)},(S,_)=>_+1).reverse().map(S=>{const _=(d||[]).filter(H=>H.matchday===S);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:${et};margin-bottom:6px">Journée ${S}</div>${_.map(H=>Lo(H,f,n,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
  ...
  <div class="page">
    ...
    <nav class="top-nav">
      <div class="logo" id="nav-logo" title="Manager Wars v2026.07.27-0006" style="cursor:pointer">
