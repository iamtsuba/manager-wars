import{s as $,F as et,b as Me,c as Ft,l as ge,d as ai}from"./formation-links-CDBKB_z4.js";function _t(e,{navigate:t,toast:i}){e.innerHTML=`
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),n=(d||[]).filter(_=>_.card_type==="player"&&_.player),r=(d||[]).filter(_=>_.card_type==="game_changer"),p=(d||[]).filter(_=>_.card_type==="formation"),{data:g}=await $.from("gc_definitions").select("name,gc_type,color,effect,image_url"),f={};(g||[]).forEach(_=>{f[_.name]=_});const y=Object.keys(et),u=Object.keys(de),h={};n.forEach(_=>{const b=_.player.id;h[b]=(h[b]||0)+1}),new Set(Object.keys(h).map(_=>String(_)));const L=new Set(p.map(_=>_.formation)),v=new Set(r.map(_=>_.gc_type));let x="player",k="Tous",B="",m=!1;function A(){return[...n].sort((_,b)=>{const z=je.indexOf(_.player.job),E=je.indexOf(b.player.job);return z!==E?z-E:(_.player.surname_encoded||"").localeCompare(b.player.surname_encoded||"")})}function C(){return[...l||[]].sort((_,b)=>{const z=je.indexOf(_.job),E=je.indexOf(b.job);return z!==E?z-E:(_.surname_encoded||"").localeCompare(b.surname_encoded||"")})}function N(){return A().filter(_=>{const b=_.player,z=k==="Tous"||b.job===k,E=!B||`${b.firstname} ${b.surname_encoded}`.toLowerCase().includes(B);return z&&E})}function q(){return C().filter(_=>{const b=k==="Tous"||_.job===k,z=!B||`${_.firstname} ${_.surname_encoded}`.toLowerCase().includes(B);return b&&z})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{m=!m,P(),V()})))}function V(){const _=document.getElementById("col-grid");_&&(x==="player"?F(_):x==="formation"?G(_):D(_))}function Z(_,b,z,E,w){w=w||"#7a28b8";const j=document.getElementById("col-grid"),T=document.getElementById("col-big");if(!j||!T)return;var R=0;function S(){T.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+b(_[R])+"</div>";var H=T.querySelector("[data-card-id],[data-form-id],[data-gc-id]");H&&H.addEventListener("click",function(){E(_[R])}),requestAnimationFrame(function(){var K=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!K||!T)){var W=T.clientHeight-8,ie=T.clientWidth-24,re=K.offsetHeight,ae=K.offsetWidth;if(re>0&&ae>0&&W>40){var se=Math.min(W/re,ie/ae,1);K.style.transform="scale("+se.toFixed(3)+")",K.style.transformOrigin="top center"}}}),j.innerHTML=_.map(function(K,W){return'<div class="col-mini-item" data-idx="'+W+'" style="flex-shrink:0;cursor:pointer;border-radius:8px;border:1.5px solid '+(W===R?w:"transparent")+';transition:border-color .15s;overflow:hidden">'+z(K,W===R)+"</div>"}).join(""),j.querySelectorAll(".col-mini-item").forEach(function(K){K.addEventListener("click",function(){R=Number(K.dataset.idx),S(),K.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}S()}function U(_){var b=.54,z=Math.round(140*b),E=Math.round(310*b),w;if(!_||_._fake){var j=_?_.player:null;if(!j)return"";w=At(j)}else w=Ue(_,"");return'<div style="width:'+z+"px;height:"+E+'px;overflow:hidden;position:relative;flex-shrink:0"><div style="transform:scale('+b+');transform-origin:top left;position:absolute;top:0;left:0;pointer-events:none">'+w+"</div></div>"}function ee(_,b,z){b=b||100,z=z||140;var E=Me[_]||{},w={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},j=Math.max(3,Math.round(b*.06)),T=Object.entries(E).map(function(S){var H=S[0],K=S[1],W=H.replace(/\d+$/,""),ie=w[W]||"#888",re=Math.round(K.x*b),ae=Math.round(K.y*z);return'<circle cx="'+re+'" cy="'+ae+'" r="'+j+'" fill="'+ie+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),R=Math.max(1,Math.round(b/50));return'<svg viewBox="0 0 '+b+" "+z+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+b+'" height="'+z+'" fill="#1A6B3C"/><rect x="'+Math.round(b*.2)+'" y="'+Math.round(z*.02)+'" width="'+Math.round(b*.6)+'" height="'+Math.round(z*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+R+'"/><line x1="0" y1="'+Math.round(z*.5)+'" x2="'+b+'" y2="'+Math.round(z*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+R+'"/><ellipse cx="'+Math.round(b*.5)+'" cy="'+Math.round(z*.5)+'" rx="'+Math.round(b*.18)+'" ry="'+Math.round(z*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+R+'"/><rect x="'+Math.round(b*.2)+'" y="'+Math.round(z*.82)+'" width="'+Math.round(b*.6)+'" height="'+Math.round(z*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+R+'"/>'+T+"</svg>"}function I(_,b,z){var E=z>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+z+"</div>":"",w=b?'data-form-id="'+b.id+'"':"",j=_.length>7?14:_.length>5?16:19,T=!!b;return"<div "+w+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(T?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(T?"":"filter:grayscale(1);opacity:0.5")+'">'+E+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(T?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+j+"px;font-weight:900;color:"+(T?"#1A6B3C":"#aaa")+';line-height:1">'+_+'</div></div><div style="flex:1;overflow:hidden;background:'+(T?"#1A6B3C":"#ccc")+'">'+ee(_,140,220)+"</div></div>"}function M(_,b){var z=.54,E=Math.round(140*z),w=Math.round(305*z),j=Math.round(w*.22),T=w-j,R=_.length>7?5:7,S=ee(_,E,T),H=b?"1.5px solid #2a7a40":"1px solid #ddd",K=b?"":"filter:grayscale(1);opacity:0.45;",W=b?"#1A6B3C":"#bbb",ie="#fff";return'<div style="width:'+E+"px;height:"+w+"px;border-radius:6px;border:"+H+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+K+'"><div style="height:'+j+"px;background:"+W+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+R+"px;font-weight:900;color:"+ie+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(E-4)+'px">'+_+'</span></div><div style="height:'+T+'px;overflow:hidden;flex-shrink:0">'+S+"</div></div>"}function F(_){if(m){const b=q();if(!b.length){_.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const z=b.map(E=>n.find(w=>w.player.id===E.id)||{_fake:!0,player:E,id:"fake-"+E.id});Z(z,E=>E._fake?At(E.player):Ue(E,""),E=>E._fake?U({player:E.player,id:"x",_fake:!0}):U(E),E=>{E._fake||It(E,n,h,t)},"#1A6B3C")}else{const b=N();if(!b.length){_.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const z={},E=[];b.forEach(w=>{z[w.player.id]||(z[w.player.id]=!0,E.push(w))}),Z(E,w=>{const j=h[w.player.id]||1,T=j>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${j}</div>`:"",S=n.filter(H=>H.player.id===w.player.id&&H.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Ue(w,T+S)},w=>U(w),w=>It(w,n,h,t),"#1A6B3C")}}function G(_){const b=m?y:[...L];if(!b.length){_.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const z=b.map(E=>({formation:E,card:p.find(w=>w.formation===E)||null,owned:L.has(E)}));Z(z,({formation:E,card:w,owned:j})=>I(E,j?w:null,j?p.filter(T=>T.formation===E).length:0),({formation:E,owned:w})=>M(E,w),({card:E,owned:w})=>{w&&E&&mi(E,p,t,o)},"#1A6B3C")}function D(_){const b=Object.keys(f),z=m?b.length?b:u:[...v];if(!z.length){_.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const E=z.map(w=>({type:w,gc:de[w]||{icon:"⚡",desc:""},def:f[w]||null,owned:v.has(w),card:r.find(j=>j.gc_type===w)||null}));Z(E,({type:w,gc:j,def:T,owned:R,card:S})=>{const H=R?r.filter(ze=>ze.gc_type===w).length:0,K=H>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${H}</div>`:"",W=(T==null?void 0:T.gc_type)==="ultra_game_changer",ie={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},re={purple:"#b06ce0",light_blue:"#00d4ef"},ae=ie[T==null?void 0:T.color]||ie.purple,se=re[T==null?void 0:T.color]||re.purple,xe=(T==null?void 0:T.effect)||j.desc||"",me=T!=null&&T.image_url?`/manager-wars/icons/${T.image_url}`:null;return R&&S?`<div data-gc-id="${S.id}" data-gc-type="${w}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${se};background:${ae};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${se}66;cursor:pointer">
          ${K}
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const $e=1e3;function mi(e,t,i,s){var u,h,L;const{state:c,toast:o,closeModal:a,navigate:d,refreshProfile:l}=i,n=e.formation,r={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function p(){const v=Me[n]||{},x=et[n]||[],k=290,B=360,m=20;function A(N){const q=v[N];return q?{x:q.x*k,y:q.y*B}:null}let C=`<svg width="${k}" height="${B}" viewBox="0 0 ${k} ${B}" xmlns="http://www.w3.org/2000/svg">`;for(const[N,q]of x){const P=A(N),V=A(q);!P||!V||(C+=`<line x1="${P.x}" y1="${P.y}" x2="${V.x}" y2="${V.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const N of Object.keys(v)){const q=A(N);if(!q)continue;const P=N.replace(/\d+/,""),V=r[P]||"#555";C+=`<circle cx="${q.x}" cy="${q.y}" r="${m}" fill="${V}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,C+=`<text x="${q.x}" y="${q.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${P}</text>`}return C+="</svg>",C}const g=t.filter(v=>v.formation===n),f=g.length,y=!e.is_for_sale;s(`Formation ${n}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
      ...
      <div style="width:140px;border-radius:12px;padding:6px;background:${k};flex-shrink:0">
        <div style="background:#f2e8d2;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
          ...
          <div style="background:#f2e8d2;padding:3px 6px;display:flex;align-items:center;justify-content:space-between;min-height:24px">
            ...
            ${(C=r.clubs)!=null&&C.logo_url?`<img src="${r.clubs.logo_url}" style="width:22px;height:16px;object-fit:contain">`:`<div style="background:#1a3a7a;color:#fff;border-radius:2px;padding:1px 3px;font-size:6px;font-weight:800">${(((N=r.clubs)==null?void 0:N.encoded_name)||"").slice(0,6)}</div>`}
          </div>
        </div>
      </div>
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}const Oi="simulation",Kt="VOTRE_ZONE_ID";function Hi(){switch(Oi){case"propellerads":return Ki();case"adsense":return Ui();default:return rt()}}function rt(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:#111;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:14px";let i=5;t.innerHTML=`
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order");let l=0;function n(){var v,x,k,B,m;const r=o[l],p=(d||[]).filter(A=>A.deck_id===r.id),g=p.filter(A=>{var C;return A.is_starter&&((C=A.card)==null?void 0:C.player)}).map(A=>We(A.card,A.position)),f=p.find(A=>{var C;return((C=A.card)==null?void 0:C.card_type)==="formation"}),y=r.formation||((v=f==null?void 0:f.card)==null?void 0:v.formation)||"4-4-2",u=g.length>=11?Ne(g,y):null,h=g.length>=11;Vt(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">
        ${o.map((A,C)=>`<div style="width:7px;height:7px;border-radius:50%;background:${C===l?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}
      </div>`:""}
    ...
    </div>`,function(){const C=e.querySelector(".deck-preview-wrap svg");C&&(C.removeAttribute("width"),C.removeAttribute("height"),C.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",C.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),(x=document.getElementById("prev-deck"))==null||x.addEventListener("click",()=>{l>0&&(l--,n())}),(k=document.getElementById("next-deck"))==null||k.addEventListener("click",()=>{l<o.length-1&&(l++,n())}),(B=document.getElementById("validate-deck"))==null||B.addEventListener("click",()=>{h&&t.navigate("match",{matchMode:i,deckId:r.id})}),(m=document.getElementById("cancel-deck-select"))==null||m.addEventListener("click",()=>{le(e),c("home")});const L=document.getElementById("deck-swipe-zone");if(L){let A=0,C=0;L.addEventListener("touchstart",N=>{A=N.touches[0].clientX,C=N.touches[0].clientY},{passive:!0}),L.addEventListener("touchend",N=>{const q=N.changedTouches[0].clientX-A,P=N.changedTouches[0].clientY-C;Math.abs(q)<40||Math.abs(q)<Math.abs(P)||(q<0&&l<o.length-1?(l++,n()):q>0&&l>0&&(l--,n()))},{passive:!0})}}n()}function Wt(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";return e!=null&&e.clubLogo?e.clubLogo.startsWith("http")?e.clubLogo:`${t}/storage/v1/object/public/assets/clubs/${e.clubLogo}`:null}function fe(e,t=44,i=58){if(!e)return`<div style="width:${t}px;height:${i}px;border:1.5px dashed rgba(255,255,255,0.2);border-radius:5px"></div>`;const s=typeof e.portrait=="string"&&e.portrait.startsWith("http")?e.portrait:oe(e),c=Wt(e),o=e._line||e.job||"MIL",a=_e[o]||"#555",d={normal:"#aaa",pépite:"#D4A017",papyte:"#222",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",l=e.note!==void 0?Number(e.note)||0:(Number(te(e,o))||0)+(e.boost||0),n=Jt(e==null?void 0:e.country_code),r=Math.round(i*.19),p=Math.round(i*.25),g=i-Math.round(i*.19)-Math.round(i*.25),f=e!=null&&e.used?.28:1;return`<div style="width:${t}px;height:${i}px;border-radius:5px;border:2px solid ${d};background:${a};position:relative;overflow:hidden;flex-shrink:0;opacity:${f}">
    <div style="position:absolute;top:0;left:0;right:0;height:${r}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:center;z-index:2">
  ...
  <div class="match-screen" style="position:fixed;inset:0;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;height:100%;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:100px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const A=t.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
            ...
            ${Ee((A.players||[]).map(C=>({...C,used:!1})),"#ff6b6b",A.total)}
            ...
            ${Ee((A.players||[]).map(C=>({...C,used:!1})),"#00ff88",A.total)}
    ...
    ${(()=>{const m=window.innerWidth>=700,A=(I,M,F)=>{var H,K;const G=(t.gcDefs||[]).find(W=>W.name===I.gc_type),D={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[G==null?void 0:G.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",_={purple:"#b06ce0",light_blue:"#00d4ef"}[G==null?void 0:G.color]||"#b06ce0",b=(G==null?void 0:G.name)||I.gc_type,z=(G==null?void 0:G.effect)||((H=de[I.gc_type])==null?void 0:H.desc)||"",E=G!=null&&G.image_url?`/manager-wars/icons/${G.image_url}`:null,w=((K=de[I.gc_type])==null?void 0:K.icon)||"⚡",j=Math.round(F*.22),T=Math.round(F*.22),R=F-j-T,S=b.length>12?7:9;return`<div class="gc-mini" data-gc-id="${I.id}" data-gc-type="${I.gc_type}"
          ...
          <div style="height:${j}px;background:rgba(255,255,255,0.14);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 3px">
            <span style="font-size:${S}px;font-weight:900;color:#fff;text-align:center;line-height:1.1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${M-6}px">${b}</span>
          ...
          <div style="height:${R}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
            ${E?`<img src="${E}" style="max-width:${M-10}px;max-height:${R-6}px;object-fit:contain">`:`<span style="font-size:${Math.round(R*.55)}px">${w}</span>`}
          </div>
        ...
        </div>`},C=(I,M)=>{var F;return`<div id="boost-card"
        ...
        <div style="display:flex;flex:0.8;min-height:0;overflow:hidden">
          ...
          <div style="width:140px;flex-shrink:0;display:flex;flex-direction:column;padding:10px 8px;background:rgba(0,0,0,0.2)">
            ...
            <div style="width:100%;flex-shrink:0;padding-top:8px">${V}${Z}</div>
          </div>
        </div>`:`
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(m=>{var A,C,N;if(m.type==="duel"){const q=m.isGoal,P=m.homeScored?"#FFD700":q?"#ff6b6b":"rgba(255,255,255,0.3)",V=m.homeScored?"⚽ BUT !":q?"⚽ BUT IA !":(A=m.homePlayers)!=null&&A.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${q?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${P};margin-bottom:4px">
                <div style="font-size:9px;color:${P};letter-spacing:1px;margin-bottom:5px;font-weight:700;text-transform:uppercase">${V}</div>
                ${(C=m.homePlayers)!=null&&C.length?`<div style="margin-bottom:3px">${Ee(m.homePlayers,"rgba(255,255,255,0.7)",m.homeTotal)}</div>`:""}
                ${(N=m.aiPlayers)!=null&&N.length?`<div style="opacity:0.7">${Ee(m.aiPlayers,"#ff6b6b",m.aiTotal)}</div>`:""}
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(o);let d=!1;const l=()=>{d||(d=!0,o.remove(),setTimeout(()=>c(),50))};o.addEventListener("click",l),setTimeout(l,3500)}async function ve(e,t,i){var r,p;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:s}=i,c=t.homeScore>t.aiScore,o=t.homeScore===t.aiScore,a=c?"victoire":o?"nul":"defaite",d=pi(t.mode,a);t.matchId&&await $.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:c?s.profile.id:null,home_credits_reward:d,played_at:new Date().toISOString()}).eq("id",t.matchId);const l={credits:(s.profile.credits||0)+d,matches_played:(s.profile.matches_played||0)+1};c?l.wins=(s.profile.wins||0)+1:o?l.draws=(s.profile.draws||0)+1:l.losses=(s.profile.losses||0)+1,await $.from("users").update(l).eq("id",s.profile.id),await i.refreshProfile();const n=document.createElement("div");n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",n.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const f=()=>document.getElementById("mm-status"),y=async(x=!0)=>{p=!0,g&&($.removeChannel(g),g=null),x&&await $.rpc("cancel_matchmaking",{p_user_id:l.profile.id}).catch(()=>{})};(v=document.getElementById("mm-cancel"))==null||v.addEventListener("click",async()=>{try{await y(!0)}catch(x){console.warn("[Matchmaking] cleanup error:",x)}le(e),n("home")});const u=async(x,k,B)=>{p||(p=!0,g&&($.removeChannel(g),g=null),f()&&(f().textContent="Adversaire trouvé !"),await new Promise(m=>setTimeout(m,600)),e.isConnected!==!1&&uo(e,t,x,B,a,d))},{data:h,error:L}=await $.rpc("try_matchmake",{p_user_id:l.profile.id,p_deck_id:i});if(L||!(h!=null&&h.success)){r("Erreur de matchmaking","error"),le(e),n("home");return}if(h.matched){u(h.match_id,h.opponent_id,!1);return}f()&&(f().textContent="En attente d'un autre joueur..."),g=$.channel("matchmaking-"+l.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${l.profile.id}`},x=>{const k=x.new;k.status==="matched"&&k.match_id&&u(k.match_id,k.matched_with,!0)}).subscribe()}async function uo(e,t,i,s,c=[],o=[]){const{state:a,navigate:d,toast:l}=t,n=s?"p1":"p2",r=s?"p2":"p1",p=(c||[]).map(I=>I.id),g=(c||[]).map(I=>({id:I.id,gc_type:I.gc_type,_gcDef:I._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation du match...</div>';const{data:f}=await $.from("matches").select("*").eq("id",i).single();if(!f){l("Match introuvable","error"),d("home");return}async function y(){const[{data:I,error:M},{data:F,error:G},{data:D},{data:_}]=await Promise.all([$.rpc("get_deck_for_match",{p_deck_id:f.home_deck_id}),$.rpc("get_deck_for_match",{p_deck_id:f.away_deck_id}),$.from("users").select("id,pseudo,club_name").eq("id",f.home_id).single(),$.from("users").select("id,pseudo,club_name").eq("id",f.away_id).single()]);M&&console.error("[PvP] get_deck_for_match p1 error:",M),G&&console.error("[PvP] get_deck_for_match p2 error:",G);function b(S,H){return{cardId:S.card_id,position:H||S.position,id:S.id,firstname:S.firstname,name:S.surname_encoded,country_code:S.country_code,club_id:S.club_id,job:S.job,job2:S.job2,note_g:Number(S.note_g)||0,note_d:Number(S.note_d)||0,note_m:Number(S.note_m)||0,note_a:Number(S.note_a)||0,rarity:S.rarity,skin:S.skin,hair:S.hair,hair_length:S.hair_length,clubName:S.club_encoded_name||null,clubLogo:S.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}}const z=((I==null?void 0:I.starters)||[]).map(S=>b(S,S.position)),E=((F==null?void 0:F.starters)||[]).map(S=>b(S,S.position)),w=((I==null?void 0:I.subs)||[]).map(S=>b(S,S.position)),j=((F==null?void 0:F.subs)||[]).map(S=>b(S,S.position)),T=(I==null?void 0:I.formation)||"4-4-2",R=(F==null?void 0:F.formation)||"4-4-2";return console.log("[PvP] p1Formation:",T,"p1 starters:",z.length,"positions:",z.map(S=>S.position)),console.log("[PvP] p2Formation:",R,"p2 starters:",E.length,"positions:",E.map(S=>S.position)),{p1Team:Ne(z,T),p2Team:Ne(E,R),p1Subs:w,p2Subs:j,p1Formation:T,p2Formation:R,p1Name:(D==null?void 0:D.club_name)||(D==null?void 0:D.pseudo)||"Joueur 1",p2Name:(_==null?void 0:_.club_name)||(_==null?void 0:_.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:s?p:[],gc_p2:s?[]:p,gcCardsFull_p1:s?g:[],gcCardsFull_p2:s?[]:g,gcReady_p1:!0,gcReady_p2:!0,usedGc_p1:[],usedGc_p2:[],lastActionAt:new Date().toISOString()}}let u=f.game_state&&Object.keys(f.game_state).length?f.game_state:null;if(!u)if(s){u=await y();const{data:I}=await $.from("matches").select("game_state").eq("id",i).single();!(I!=null&&I.game_state)||!Object.keys(I.game_state).length?await $.from("matches").update({game_state:u,turn_user_id:f.home_id}).eq("id",i):u=I.game_state}else{e.innerHTML=`<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation avec l'adversaire...</div>`;for(let I=0;I<30&&!u;I++){await new Promise(F=>setTimeout(F,400));const{data:M}=await $.from("matches").select("game_state").eq("id",i).single();M!=null&&M.game_state&&Object.keys(M.game_state).length&&(u=M.game_state)}if(!u){l("Erreur de synchronisation","error"),d("home");return}u.gc_p2=p,u.gcCardsFull_p2=g,await $.from("matches").update({game_state:u}).eq("id",i)}let h=!1;function L(I){var z;try{$.removeChannel(v)}catch{}const M=a.profile.id,F=I.winner_id===M,G=!!I.forfeit,D=u[n+"Score"]??0,_=u[r+"Score"]??0,b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center",b.innerHTML=`
      ...
      <div style="text-align:center;padding:4px;background:rgba(0,0,0,0.15);font-size:11px;color:${z?"rgba(255,255,255,0.4)":"#FFD700"};font-weight:700;flex-shrink:0">
        ${z?`⏳ Tour de ${D}`:_?"⚔️ À vous d'attaquer !":b?"🛡️ À vous de défendre !":""}
      </div>
      <!-- Zone actions -->
      ...
      </div>`;const O=e.querySelector(".terrain-wrapper");O&&(O.innerHTML=Te(I,u[n+"Formation"],_?"attack":b?"defense":"idle",w,300,300))}if(e.querySelectorAll(".match-slot-hit").forEach(O=>{O.addEventListener("click",()=>{if(!_&&!b)return;const J=O.dataset.cardId,Q=O.dataset.role,Y=(I[Q]||[]).find(ye=>ye.cardId===J);if(!Y||Y.used)return;Array.isArray(u["selected_"+n])||(u["selected_"+n]=[]);const ce=u["selected_"+n],Ce=ce.findIndex(ye=>ye.cardId===J);Ce>-1?ce.splice(Ce,1):ce.length<3&&ce.push({...Y,_role:Q}),B()})}),(ut=e.querySelector("#pvp-boost-card"))==null||ut.addEventListener("click",()=>{N()}),e.querySelectorAll(".pvp-gc-mini").forEach(O=>{O.addEventListener("click",()=>{V(O.dataset.gcId,O.dataset.gcType)})}),(ft=e.querySelector("#pvp-action"))==null||ft.addEventListener("click",()=>{_?U():b&&ee()}),(gt=e.querySelector("#pvp-quit"))==null||gt.addEventListener("click",()=>{confirm("Quitter le match ? Vous perdrez par forfait.")&&k()}),(xt=e.querySelector("#pvp-view-opp"))==null||xt.addEventListener("click",()=>{P()}),(mt=e.querySelector("#pvp-toggle-history"))==null||mt.addEventListener("click",()=>{q()}),function(){const J=e.querySelector(".terrain-wrapper svg");J&&(J.removeAttribute("width"),J.removeAttribute("height"),J.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",J.setAttribute("viewBox","-26 -26 352 352"),J.setAttribute("preserveAspectRatio","xMidYMid meet"));const Q=e.querySelector(".match-screen");if(Q){const Y=e.clientHeight;Y>50&&(Q.style.height=Y+"px",Q.style.maxHeight=Y+"px",Q.style.overflow="hidden")}}(),u._timerInt&&(clearInterval(u._timerInt),u._timerInt=null),(_||b)&&!h){let O=30,J=!1;const Q=()=>document.getElementById("pvp-timer"),Y=()=>{Q()&&(Q().textContent=O+"s",Q().style.color=J?"#ff4444":"#fff")};Y(),u._timerInt=setInterval(()=>{O--,O<0?J?(clearInterval(u._timerInt),u._timerInt=null,k()):(J=!0,O=15,Y()):Y()},1e3)}}function m(){var b;const I=u["gcReady_"+n],M=u["gcReady_"+r];Array.isArray(u["gc_"+n])||(u["gc_"+n]=[]);let F=u["gc_"+n];const G=3;function D(z,E){const w=z._gcDef,j={purple:"linear-gradient(135deg,#3d0a7a,#7a28b8)",light_blue:"linear-gradient(135deg,#006080,#00bcd4)"},T={purple:"#9b59b6",light_blue:"#00bcd4"},R=j[w==null?void 0:w.color]||j.purple,S=T[w==null?void 0:w.color]||T.purple;return`<div class="pvp-gc-card" data-id="${z.id}"
        style="width:100px;border-radius:10px;border:3px solid ${E?"#FFD700":S};background:${R};
          display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0;position:relative;
          box-shadow:${E?"0 0 18px #FFD700":"0 2px 8px rgba(0,0,0,0.4)"};
          transform:${E?"scale(1.06)":"scale(1)"};transition:all 0.15s">
        <div style="padding:5px 6px;background:rgba(255,255,255,0.12);text-align:center;min-height:32px;display:flex;align-items:center;justify-content:center">
          <span style="font-size:${((w==null?void 0:w.name)||z.gc_type).length>12?8:10}px;font-weight:900;color:#fff;line-height:1.2;text-align:center">${(w==null?void 0:w.name)||z.gc_type}</span>
        </div>
        <div style="height:70px;display:flex;align-items:center;justify-content:center;padding:4px">
    ...
    </div>`,I)return;const _=n==="p1"?f.home_id:f.away_id;$.from("cards").select("id,gc_type").eq("owner_id",_).eq("card_type","game_changer").then(async({data:z})=>{const{data:E}=await $.from("gc_definitions").select("*").eq("is_active",!0),w=new Set,j=(z||[]).filter(R=>w.has(R.gc_type)?!1:(w.add(R.gc_type),!0)).map(R=>({...R,_gcDef:(E==null?void 0:E.find(S=>S.name===R.gc_type))||null})),T=document.getElementById("pvp-gc-grid");if(T){if(!j.length){T.innerHTML='<div style="color:rgba(255,255,255,0.4);font-size:13px;text-align:center;margin-top:30px">Aucune carte Game Changer disponible.</div>';return}T.innerHTML=j.map(R=>D(R,F.includes(R.id))).join(""),T.querySelectorAll(".pvp-gc-card").forEach(R=>{R.addEventListener("click",()=>{const S=R.dataset.id,H=F.indexOf(S);H>-1?F.splice(H,1):F.length<G&&F.push(S),B()})})}}),(b=e.querySelector("#pvp-gc-validate"))==null||b.addEventListener("click",async()=>{const z=n==="p1"?f.home_id:f.away_id,{data:E}=await $.from("cards").select("id,gc_type").eq("owner_id",z).eq("card_type","game_changer").in("id",F),{data:w}=await $.from("gc_definitions").select("*").eq("is_active",!0),j=(E||[]).map(T=>({...T,_gcDef:(w==null?void 0:w.find(R=>R.name===T.gc_type))||null}));await x({["gcReady_"+n]:!0,["gcCardsFull_"+n]:j}),u["gcReady_"+r]&&await x({phase:"reveal"})})}function A(){e.style.overflow="hidden",e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
      ...
      <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
    </div>`,n==="p1"&&setTimeout(async()=>{await x({phase:"midfield"})},5e3)}function C(){const I=u[n+"Team"].MIL||[],M=u[r+"Team"].MIL||[];function F(E){return E.reduce((w,j)=>w+te(j,"MIL"),0)}function G(E){let w=0;for(let j=0;j<E.length-1;j++){const T=ge(E[j],E[j+1]);T==="#00ff88"?w+=2:T==="#FFD700"&&(w+=1)}return w}const D=F(I)+G(I),_=F(M)+G(M),b=D>=_;function z(E,w,j){return`<div style="text-align:center">
        <div style="font-size:10px;color:rgba(255,255,255,0.5);letter-spacing:2px;margin-bottom:8px;text-transform:uppercase">${w}</div>
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${E.map((T,R)=>{const S=oe(T),H=R<E.length-1?ge(T,E[R+1]):null,K=H&&H!=="#ff3333"&&H!=="#cc2222";return`
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    </div>`,setTimeout(()=>{const E=document.getElementById("pvp-score-me"),w=document.getElementById("pvp-score-opp"),j=document.getElementById("pvp-midfield-result");E&&w&&(b?(E.style.fontSize="80px",E.style.color="#FFD700",w.style.opacity="0.25"):(w.style.fontSize="80px",w.style.color="#ff6b6b",E.style.opacity="0.25"));const T=u.p1Team.MIL||[],R=u.p2Team.MIL||[],S=F(T)+G(T),H=F(R)+G(R),K=S>=H?"p1":"p2";u[K+"Name"];let W=u.boostValue;W==null&&(W=Ut(),u.boostValue=W,u.boostOwner=K,u.boostUsed=!1),j&&(j.style.opacity="1",j.innerHTML=`<div style="font-size:20px;font-weight:900;margin-bottom:10px">
      ...
      </div>`,document.body.appendChild(F),(G=F.querySelector("#boost-picker-close"))==null||G.addEventListener("click",()=>F.remove()),F.querySelectorAll(".boost-pick-item").forEach(D=>{D.addEventListener("click",async()=>{const _=D.dataset.cid,b=M.find(w=>w.cardId===_);if(!b)return;const z=u[n+"Team"],E=(z[b._line]||[]).find(w=>w.cardId===_);E&&(E.boost=(E.boost||0)+u.boostValue),F.remove(),await x({[n+"Team"]:z,boostUsed:!0})})})}function q(){var F;const I=Array.isArray(u.log)?u.log:[],M=document.createElement("div");M.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column",M.innerHTML=`
      ...
      <div style="width:min(90vw,420px)">${Ie(u[r+"Team"],u[r+"Formation"],null,[],300,300)}</div>
      <button id="pvp-view-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(I),(M=I.querySelector("#pvp-view-opp-close"))==null||M.addEventListener("click",()=>I.remove())}function V(I,M){var S,H;const G=(u["gcCardsFull_"+n]||[]).find(K=>K.id===I),D=G==null?void 0:G._gcDef,_=de[M]||{icon:"⚡",desc:"Carte spéciale."},b={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[D==null?void 0:D.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",z={purple:"#b06ce0",light_blue:"#00d4ef"}[D==null?void 0:D.color]||"#b06ce0",E=(D==null?void 0:D.name)||M,w=(D==null?void 0:D.effect)||_.desc,j=D!=null&&D.image_url?`/manager-wars/icons/${D.image_url}`:null,T=_.icon||"⚡",R=document.createElement("div");R.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",R.innerHTML=`
      <div style="width:190px;border-radius:16px;border:3px solid ${z};background:${b};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 40px ${z}66">
        <div style="padding:10px;background:rgba(255,255,255,0.14);text-align:center">
      ...
      </div>`,document.body.appendChild(R),(S=R.querySelector("#pvp-gc-back"))==null||S.addEventListener("click",()=>R.remove()),(H=R.querySelector("#pvp-gc-use"))==null||H.addEventListener("click",async()=>{R.remove();const K=u["usedGc_"+n]||[];K.push(I),await x({["usedGc_"+n]:K})})}function Z(){var D;const I=u[n+"Score"],M=u[r+"Score"],F=I>M,G=I===M;e.style.overflow="hidden",e.innerHTML=`
    ...
    </div>`,(D=document.getElementById("pvp-home"))==null||D.addEventListener("click",()=>{$.removeChannel(v),le(e),d("home")})}async function U(){const I=(u["selected_"+n]||[]).map(G=>({...G,_line:G._role}));if(!I.length)return;const M=tt(I,u.modifiers[n]||{});I.forEach(G=>{const D=(u[n+"Team"][G._role]||[]).find(_=>_.cardId===G.cardId);D&&(D.used=!0)});const F=Array.isArray(u.log)?u.log:[];F.push({type:"attack",text:`⚔️ ${u[n+"Name"]} attaque (${M.total})`,players:I,total:M.total,side:n}),await x({pendingAttack:{...M,players:I,side:n},["selected_"+n]:[],modifiers:{...u.modifiers,[n]:{}},phase:r+"-defense",log:F})}async function ee(){const I=(u["selected_"+n]||[]).map(j=>({...j,_line:j._role})),M=it(I,u.modifiers[n]||{});I.forEach(j=>{const T=(u[n+"Team"][j._role]||[]).find(R=>R.cardId===j.cardId);T&&(T.used=!0)});const F=ot(u.pendingAttack.total,M.total,u.modifiers[n]||{}),G=u.pendingAttack.side,D=F==="attack",_=u[G+"Score"]+(D?1:0),b=G==="p1"?"p2":"p1",z=(u.round||0)+1,E=z>10,w=Array.isArray(u.log)?u.log:[];w.push({type:"defense",text:`🛡️ ${u[n+"Name"]} défend (${M.total})`,players:I,total:M.total,side:n}),w.push({type:D?"goal":"stop",text:D?`⚽ BUT de ${u[G+"Name"]} ! (${u.pendingAttack.total} vs ${M.total})`:`✋ Attaque stoppée (${u.pendingAttack.total} vs ${M.total})`}),await x({[G+"Score"]:_,["selected_"+n]:[],modifiers:{...u.modifiers,[n]:{}},pendingAttack:null,phase:E?"finished":b+"-attack",attacker:b,round:z,log:w}),E&&await $.from("matches").update({status:"finished"}).eq("id",i)}B()}const fo={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"};async function go(e,t){const{state:i,toast:s}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement du marché...</div>',await dt(e,t)}async function dt(e,t){const{state:i,toast:s}=t,{data:c}=await $.from("market_listings").select(`id, price, status, listed_at, seller_id,
  ...
  </div>`}const X={user:null,profile:null,page:"home",params:{}};function Be(e,t="info",i=3e3){const s=document.getElementById("toast");s&&(s.textContent=e,s.className=`show ${t}`,clearTimeout(s._t),s._t=setTimeout(()=>{s.className=""},i))}function bo(e,t,i=""){document.getElementById("modal-title").textContent=e,document.getElementById("modal-body").innerHTML=t,document.getElementById("modal-footer").innerHTML=i,document.getElementById("modal-overlay").classList.remove("hidden")}function Ze(){document.getElementById("modal-overlay").classList.add("hidden")}async function Fe(){if(!X.user)return;const{data:e}=await $.from("users").select("*").eq("id",X.user.id).single();e&&(X.profile=e)}const lt="mw_theme";function Pe(){return localStorage.getItem(lt)||"light"}function wo(e){var t;localStorage.setItem(lt,e),Qe(e),(t=X.profile)!=null&&t.id&&$.from("users").update({theme:e}).eq("id",X.profile.id).then(()=>{})}function Qe(e){document.documentElement.setAttribute("data-theme",e)}function De(e,t={}){X.page=e,X.params=t,ei()}async function ei(){const e=document.getElementById("page-content");if(!e)return;document.querySelectorAll(".bottom-nav a").forEach(s=>{s.classList.toggle("active",s.dataset.page===X.page)});const t=document.getElementById("nav-credits");t&&X.profile&&(t.textContent=`💰 ${(X.profile.credits||0).toLocaleString("fr")}`);const i={state:X,navigate:De,toast:Be,openModal:bo,closeModal:Ze,refreshProfile:Fe};switch(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽</div>',X.page){case"home":await kt(e,i);break;case"collection":await gi(e,i);break;case"decks":await Ve(e,i);break;case"boosters":await Ci(e,i);break;case"match":{(X.params&&X.params.matchMode||"vs_ai_easy")==="random"?await co(e,i):await Yi(e,i);break}case"market":await go(e,i);break;case"rankings":await ho(e,i);break;case"matches":await vo(e,i);break;default:await kt(e,i)}}function _o(){const e=document.getElementById("app"),t=X.profile;if(!t)return;const i="/manager-wars/icons/";e.innerHTML=`
    <nav class="top-nav">
  ...
  `,document.querySelectorAll(".bottom-nav a").forEach(s=>{s.addEventListener("click",c=>{c.preventDefault(),De(s.dataset.page)})}),document.getElementById("nav-logo").addEventListener("click",()=>De("home")),document.getElementById("nav-credits").addEventListener("click",()=>De("boosters")),document.getElementById("theme-toggle").addEventListener("click",()=>{const c=Pe()==="dark"?"light":"dark";wo(c),jt(c)}),jt(Pe())}function jt(e){const t=document.getElementById("theme-icon");t&&(t.textContent=e==="dark"?"☀️":"🌙")}async function $o(){Qe(Pe()),document.getElementById("modal-overlay").addEventListener("click",t=>{t.target===t.currentTarget&&Ze()}),document.getElementById("modal-close").addEventListener("click",Ze);const{data:{session:e}}=await $.auth.getSession();if(!e){Bt(),_t(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:Be});return}X.user=e.user,await Fe(),Bt();try{const{data:t}=await $.from("formation_links_overrides").select("formation, links"),i={};(t||[]).forEach(s=>{i[s.formation]=s.links}),ai(i)}catch(t){console.warn("Impossible de charger les overrides de formation:",t)}if(!X.profile){si(document.getElementById("app"),{state:X,navigate:async()=>{await Fe(),St()},toast:Be,refreshProfile:Fe});return}X.profile.theme&&X.profile.theme!==Pe()&&(localStorage.setItem(lt,X.profile.theme),Qe(X.profile.theme)),St(),$.auth.onAuthStateChange(async(t,i)=>{t==="SIGNED_OUT"&&(X.user=null,X.profile=null,document.getElementById("app").innerHTML="",_t(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:Be}))})}function He(){const e=document.getElementById("app");e&&(e.style.height=window.innerHeight+"px")}window.addEventListener("resize",He);window.addEventListener("orientationchange",()=>setTimeout(He,150));window.visualViewport&&window.visualViewport.addEventListener("resize",He);function St(){const e=document.getElementById("app");e.style.display="flex",e.style.flexDirection="column",He(),_o(),ei()}function Bt(){const e=document.getElementById("app-loader"),t=document.getElementById("app");t&&(t.style.display=""),e&&(e.classList.add("zoom-out"),setTimeout(()=>e.style.display="none",500))}$o();
