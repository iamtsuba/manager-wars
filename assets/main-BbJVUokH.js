...
import{s as w,l as Ot,m as Mi,F as ji,r as Se,j as ei,h as Ci,n as Pn,o as Gn,T as yi,k as Rn,p as Nn,b as On}from"./formation-links-CZYKSMms.js";const Hn="/";function Wi(e,{navigate:t,toast:i}){let n="login";const r=()=>{var a,s,l,c,d,x;const o=n==="login";e.innerHTML=`
  ...
  <div class="home-dark" id="home-dark">
    ...
    ${Se(e,{width:140})}
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),c=(s||[]).filter(S=>S.card_type==="player"&&S.player),d=(s||[]).filter(S=>S.card_type==="game_changer"),x=(s||[]).filter(S=>S.card_type==="formation"),y=(s||[]).filter(S=>S.card_type==="stadium"),{data:g}=await w.from("gc_definitions").select("name,gc_type,color,effect,image_url"),m={};(g||[]).forEach(S=>{m[S.name]=S});const{data:p}=await w.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),u={};(p||[]).forEach(S=>{u[S.id]=S}),y.forEach(S=>{S.stadium_def&&(u[S.stadium_id]=S.stadium_def)});const b=Object.keys(ji),v=Object.keys(Ce),h={};c.forEach(S=>{const D=S.player.id;h[D]=(h[D]||0)+1}),new Set(Object.keys(h).map(S=>String(S)));const $=new Set(x.map(S=>S.formation)),f=new Set(d.map(S=>S.gc_type));let M="player",k="Tous",P="",H=!1;function ae(){return[...c].sort((S,D)=>{const Y=ti.indexOf(S.player.job),te=ti.indexOf(D.player.job);return Y!==te?Y-te:(S.player.surname_real||"").localeCompare(D.player.surname_real||"")})}function re(){return[...l||[]].sort((S,D)=>{const Y=ti.indexOf(S.job),te=ti.indexOf(D.job);return Y!==te?Y-te:(S.surname_real||"").localeCompare(D.surname_real||"")})}function ee(){return ae().filter(S=>{const D=S.player,Y=k==="Tous"||D.job===k,te=!P||`${D.firstname} ${D.surname_real}`.toLowerCase().includes(P);return Y&&te})}function j(){return re().filter(S=>{const D=k==="Tous"||S.job===k,Y=!P||`${S.firstname} ${S.surname_real}`.toLowerCase().includes(P);return D&&Y})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${x.map((G,K)=>{const N=ee[G.id]||[],S=G.is_for_sale,D=N.length?N[N.length-1]:null,te=(d.rarity==="pepite"||d.rarity==="papyte")&&G.current_note!=null?` (☆${G.current_note})`:"";return`
            <div data-card-id="${G.id}" data-card-idx="${K}" class="exemplaire-row"
              ...
              <div style="display:flex;align-items:center;gap:10px">
                ...
                <div style="flex:1;min-width:0">
                  <div style="font-size:12px;font-weight:700">Exemplaire ${K+1}${te}${S?" 🏷️ En vente":""}</div>
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(ge=document.getElementById("close-detail"))==null||ge.addEventListener("click",s);let V=new Set;const ie=()=>{const G=V.size,K=document.getElementById("sell-action-panel");if(!K)return;K.style.display=G>0?"block":"none",document.getElementById("sell-selected-count").textContent=G;const N=document.getElementById("evolve-btn");N&&(N.textContent=`⬆️ Évoluer ${G>1?"(+"+G+")":""}`)};document.querySelectorAll(".expl-check").forEach(G=>{G.addEventListener("change",()=>{const K=G.dataset.id;G.checked?V.add(K):V.delete(K);const N=G.closest(".exemplaire-row");N&&(N.style.borderColor=G.checked?"#1A6B3C":"#eee"),ie()})}),document.querySelectorAll(".exemplaire-row").forEach(G=>{G.addEventListener("click",K=>{if(K.target.closest("button")||K.target.tagName==="INPUT")return;const N=G.querySelector(".expl-check");N&&!N.disabled&&(N.checked=!N.checked,N.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(G=>{G.addEventListener("click",K=>{K.stopPropagation();const N=document.querySelector(`.expl-hist[data-hist="${G.dataset.idx}"]`);N&&(N.style.display=N.style.display==="none"?"flex":"none")})}),(I=document.getElementById("evolve-btn"))==null||I.addEventListener("click",async()=>{if(y<=1)return;const G=[...V];if(!G.length)return;if(V.has(e.id)){const R=document.createElement("div");R.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",R.innerHTML=`
  ...
  `,document.body.appendChild(r);let o=!1;const a=document.getElementById("pack-cut-zone"),s=document.getElementById("pack-blade");let l=!1;const c=L=>L.touches&&L.touches[0]?L.touches[0].clientX:L.clientX;function d(L){o||(l=!0,s.style.opacity="1",x(L))}function x(L){if(!l||o)return;const V=a.getBoundingClientRect(),ie=c(L)-V.left,ue=Math.max(0,Math.min(1,ie/V.width));s.style.width=ue*V.width+"px",ue>=.82&&g()}function y(){o||(l=!1,s.style.transition="width .2s ease, opacity .2s ease",s.style.width="0",s.style.opacity="0",setTimeout(()=>{o||(s.style.transition="")},220))}function g(){var V;if(o)return;o=!0,l=!1,s.style.width="100%",s.style.opacity="1",(V=document.getElementById("cut-flash"))==null||V.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const L=document.getElementById("cut-hint");L&&(L.style.opacity="0"),a.classList.add("pack-cut"),setTimeout(()=>{s.style.opacity="0",document.getElementById("pack-phase").style.display="none",b(0)},620)}a.addEventListener("pointerdown",d),window.addEventListener("pointermove",x),window.addEventListener("pointerup",y),a.addEventListener("touchstart",d,{passive:!0}),window.addEventListener("touchmove",x,{passive:!0}),window.addEventListener("touchend",y);let m=0,p=!1;const u=new Set;function b(L){m=L,document.getElementById("reveal-phase").style.display="flex",v(),h(L,0),P()}function v(){const L=document.getElementById("card-dots");L&&(L.innerHTML=e.map((V,ie)=>`<div class="card-dot" data-i="${ie}" style="width:8px;height:8px;border-radius:50%;background:${ie===m?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),L.querySelectorAll(".card-dot").forEach(V=>V.addEventListener("click",()=>f(parseInt(V.dataset.i)))))}function h(L,V){var N,S;const ie=e[L],ue=document.getElementById("card-counter"),ge=document.getElementById("card-track");ue&&(ue.textContent=`Carte ${L+1} / ${e.length}`);const I=document.getElementById("reveal-btns");I&&(I.style.display=L===e.length-1?"flex":"none");const W=ie.card_type==="player"&&((N=ie.player)==null?void 0:N.rarity)==="legende",pe=!u.has(L);u.add(L);let G=0;if(ie.card_type==="player"&&ie.player){const D=ie.player,Y=D.job||"ATT";G=Number(Y==="GK"?D.note_g:Y==="DEF"?D.note_d:Y==="MIL"?D.note_m:D.note_a)||0}const K=D=>{ge.innerHTML=`
        <div id="current-card-wrap" style="position:relative;display:flex;flex-direction:column;align-items:center;gap:8px;${W?"filter:drop-shadow(0 0 20px #7a28b8)":""}">
          <div style="transform:scale(1.25);transform-origin:center">${Fo(ie)}</div>
      ...
      </div>`,document.body.appendChild(n),n.addEventListener("click",a=>{a.target===n&&n.remove()}),(i=document.getElementById("odds-close"))==null||i.addEventListener("click",()=>n.remove());return}Go()}function Go(){const e=document.createElement("div");e.style.cssText=`position:fixed;inset:0;background:rgba(0,0,0,0.6);display:flex;
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}function Ro(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText=`
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
  ...
  </div>`;const d=async()=>{await w.from("users").update({pending_boosters:a}).eq("id",t.user.id)};async function x(){var v;if(c>=l||!a.length){await w.from("users").update({pending_boosters:[],onboarding_done:!0}).eq("id",t.user.id),r&&await r(),n("Tous tes boosters sont ouverts ! Bon jeu 🎮","success",4e3),i("home");return}const m=a[0],{data:p}=await w.from("users").select("*").eq("id",t.user.id).single();p&&(t.profile=p);let u=[];try{if(m.type==="formation")u=await Tn(t.profile,0);else if(m.type==="game_changer")u=await En(t.profile,m.count||3,0);else if(s&&((v=s.rates)!=null&&v.length)){const h={...s,cost:0,cardCount:m.count||s.cardCount||5};u=await Ei(t.profile,h),m.guaranteeGK&&!t.profile.first_booster_opened&&(u.some(f=>f.player&&f.player.job==="GK")||await Oo(t.profile,u),await w.from("users").update({first_booster_opened:!0}).eq("id",t.profile.id))}else u=await $n(t.profile,m.count||5,0)}catch(h){n(h.message||"Erreur ouverture booster","error");return}a.shift(),c++,await d();const b=m.type==="formation"?{name:"Booster Formation",img:"/icons/booster-formation.png"}:m.type==="game_changer"?{name:"Booster Game Changer",img:"/icons/booster-gamechanger.png"}:{name:`Booster Joueurs (${c}/${l})`,img:(s==null?void 0:s.img)||"/icons/booster-players.png"};Ln(u,b,i,()=>{x()})}(g=document.getElementById("onboard-start"))==null||g.addEventListener("click",()=>x())}async function Oo(e,t){try{const{data:i}=await w.from("players").select("id,job,firstname,surname_real,country_code,club_id,rarity,note_g,note_d,note_m,note_a,skin,hair,hair_length,face,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0).eq("job","GK");if(!(i!=null&&i.length))return;const n=i[Math.floor(Math.random()*i.length)],r=t.findIndex(a=>a.player);if(r===-1)return;const o=t[r];await w.from("cards").update({player_id:n.id}).eq("id",o.id),t[r]={...o,player_id:n.id,player:n}}catch(i){console.warn("[Onboarding] ensureGK échec",i)}}const Gt={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},Ho={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function Lt(e,t,i,n,r){var o;e.innerHTML=`<div class="match-screen" style="display:flex;align-items:center;justify-content:center;min-height:100vh">
    ...
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff;position:relative">
      ...
      <div id="deck-top-bar" style="flex-shrink:0">
        <div style="padding:8px 16px;background:rgba(0,0,0,0.4);text-align:center">
          <div style="font-size:10px;opacity:.6;letter-spacing:2px;text-transform:uppercase">${Uo(i,t)}</div>
    ...
    <div style="animation:goalPop 0.4s ease-out;text-align:center">
      ...
      }`,document.head.appendChild(l)}setTimeout(()=>{a.style.opacity="0",a.style.transition="opacity 0.4s",setTimeout(()=>{a.remove(),r==null||r()},400)},1800)}function Yo(e,t,i){const n=document.getElementById("sub-anim-overlay");n&&n.remove();const r=document.createElement("div");r.id="sub-anim-overlay",r.style.cssText=`
    position:fixed;inset:0;z-index:3000;
    display:flex;flex-direction:column;align-items:center;justify-content:center;
  ...
  <div id="match-history-panel">
    ...
    </div>`,document.body.appendChild(g),(m=g.querySelector("#gc-back"))==null||m.addEventListener("click",()=>g.remove()),(p=g.querySelector("#gc-use"))==null||p.addEventListener("click",()=>{g.remove(),dr(e,t,i,n,r)})}function ni(e,t,i,n,r,o){const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let s=[];function l(){var c,d;a.innerHTML=`
    ...
    <div style="display:flex;flex-direction:column;gap:6px">
      ${n.map(r=>`
        <div class="player-boost-opt" data-card-id="${r.cardId}"
          ...
          <div style="width:32px;height:32px;background:${Ho[r.job]||"#888"};border-radius:6px;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:900;font-size:13px">${Ge(r,r._line||r.job)}</div>
    ...
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      ...
      ${ui(e.aiTeam,e.formation,null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.1))}
            ...
            </div>`}}catch(J){console.error("[Ranked] MMR update error:",J)}(ne=document.getElementById("pvp-end-overlay"))==null||ne.remove();const B=document.createElement("div");B.id="pvp-end-overlay",B.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;color:#fff;padding:24px;text-align:center;overflow-y:auto";const q=T?"🤝":A?"🏆":"😞",U=T?"MATCH NUL":A?"VICTOIRE !":"DÉFAITE",Q=T?"#fff":A?"#FFD700":"#ff6b6b";B.innerHTML=`
      ...
      ${F}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${d?"🏆 Retour à la Mini League":a?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(B),(Z=B.querySelector("#pvp-end-home"))==null||Z.addEventListener("click",()=>{B.remove(),et(e),d?g("mini-league",{openLeagueId:d}):g(a?"ranked":"home")})}const ee=w.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},_=>{var z;const E=_.new;try{if(E.status==="finished"||E.forfeit){if(M)return;M=!0,k&&(clearInterval(k),k=null),E.game_state&&(f=E.game_state),re(E);return}if(E.game_state){const A=f;f=E.game_state;const T=f._lastGC;if(T&&T.seq&&!ae.has(T.seq)&&(ae.add(T.seq),T.by!==p)){N(T.type,T.by,()=>W());return}const F=A[p+"Score"]||0,B=A[u+"Score"]||0,q=f[p+"Score"]||0,U=f[u+"Score"]||0,Q=q>F,ne=U>B;if((Q||ne)&&!H.has(f.round)){H.add(f.round);const Z=[...f.log||[]].reverse().find(de=>de.isGoal),J=((Z==null?void 0:Z.homePlayers)||[]).map(de=>({name:de.name,note:de.note,portrait:de.portrait,job:de.job}));!!((z=Z==null?void 0:Z.text)!=null&&z.includes("DERNIER CORNER"))?_e().then(()=>{Qe(J,q,U,Q,()=>W())}):Qe(J,q,U,Q,()=>W());return}W()}}catch(A){console.error("[PvP] crash:",A)}}).subscribe();async function j(_){Object.assign(f,_),f.lastActionAt=new Date().toISOString();const{error:E}=await w.from("matches").update({game_state:f}).eq("id",i);E&&m("Erreur de synchronisation","error");try{W()}catch(z){console.error("[PvP] renderPvpScreen crash:",z)}}async function L(){if(M)return;M=!0,k&&(clearInterval(k),k=null);const _=n?h.away_id:h.home_id,E=n?"p2":"p1",z=n?"p1":"p2",A={...f,[E+"Score"]:3,[z+"Score"]:0,phase:"finished"},T=A.p1Score||0,F=A.p2Score||0;try{await w.from("matches").update({game_state:A}).eq("id",i);const{data:B,error:q}=await w.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:_,p_home_score:T,p_away_score:F,p_forfeit:!0});if(q&&console.error("[PvP] finish_pvp_match (forfeit):",q),d)try{await w.rpc("check_and_finish_mini_league",{p_league_id:d})}catch(U){console.error("[PvP] check_and_finish_mini_league (forfeit):",U)}try{const{data:U}=await w.rpc("apply_match_rewards",{p_match_id:i});U!=null&&U.success&&!(U!=null&&U.skipped)&&typeof t.refreshProfile=="function"&&await t.refreshProfile()}catch(U){console.error("[PvP] apply_match_rewards (forfeit):",U)}}catch{}try{w.removeChannel(ee)}catch{}setTimeout(()=>{et(e),d?g("mini-league",{openLeagueId:d}):g("home")},800)}const V={BOOST_STAT:({value:_=1,count:E=1,roles:z=[]},A,T)=>{const F=A[p+"Team"],B=Object.entries(F).filter(([q])=>!z.length||z.includes(q)).flatMap(([q,U])=>U.filter(Q=>!Q.used).map(Q=>({...Q,_line:q})));if(!B.length){A.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),T(A);return}ie(B,E,`Choisir ${E} joueur(s) à booster (+${_})`,q=>{q.forEach(U=>{const Q=(F[U._line]||[]).find(ne=>ne.cardId===U.cardId);Q&&(Q.boost=(Q.boost||0)+_,A.log.push({text:`⚡ +${_} sur ${Q.name}`,type:"info"}))}),A[p+"Team"]=F,T(A)})},DEBUFF_STAT:({value:_=1,count:E=1,roles:z=[],target:A="ai"},T,F)=>{const B=A==="home"?p:u,q=T[B+"Team"],U=A==="home"?"allié":"adverse",Q=Object.entries(q).filter(([ne])=>!z.length||z.includes(ne)).flatMap(([ne,Z])=>Z.map(J=>({...J,_line:ne})));if(!Q.length){T.log.push({text:`🎯 Aucun joueur ${U}`,type:"info"}),F(T);return}ie(Q,E,`Choisir ${E} joueur(s) ${U}(s) (-${_})`,ne=>{ne.forEach(Z=>{const J=(q[Z._line]||[]).find(se=>se.cardId===Z.cardId);J&&(J.boost=(J.boost||0)-_,T.log.push({text:`🎯 -${_} sur ${J.name}`,type:"info"}))}),T[B+"Team"]=q,F(T)})},GRAY_PLAYER:({count:_=1,roles:E=[],target:z="ai"},A,T)=>{const F=z==="home"?p:u,B=A[F+"Team"],q=z==="home"?"allié":"adverse",U=Object.entries(B).filter(([Q])=>!E.length||E.includes(Q)).flatMap(([Q,ne])=>ne.filter(Z=>!Z.used).map(Z=>({...Z,_line:Q})));if(!U.length){A.log.push({text:`❌ Aucun joueur ${q}`,type:"info"}),T(A);return}ie(U,_,`Choisir ${_} joueur(s) ${q}(s) à exclure`,Q=>{const ne="usedCardIds_"+F,Z=new Set(A[ne]||[]);Q.forEach(J=>{const se=(B[J._line]||[]).find(de=>de.cardId===J.cardId);se&&(se.used=!0,Z.add(J.cardId),A.log.push({text:`❌ ${se.name} exclu !`,type:"info"}))}),A[ne]=[...Z],A[F+"Team"]=B,T(A)})},REVIVE_PLAYER:({count:_=1,roles:E=[]},z,A)=>{const T=z[p+"Team"],F=Object.entries(T).filter(([B])=>!E.length||E.includes(B)).flatMap(([B,q])=>q.filter(U=>U.used).map(U=>({...U,_line:B})));if(!F.length){z.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),A(z);return}ie(F,_,`Choisir ${_} joueur(s) à ressusciter`,B=>{B.forEach(q=>{const U=(T[q._line]||[]).find(Q=>Q.cardId===q.cardId);U&&(U.used=!1,z.log.push({text:`💫 ${U.name} ressuscité !`,type:"info"}))}),z[p+"Team"]=T,A(z)})},REMOVE_GOAL:({},_,E)=>{const z=u+"Score";_[z]>0?(_[z]--,_.log.push({text:"🚫 Dernier but annulé !",type:"info"})):_.log.push({text:"🚫 Aucun but à annuler",type:"info"}),E(_)},ADD_GOAL_DRAW:({},_,E)=>{_[p+"Score"]===_[u+"Score"]?(_[p+"Score"]++,_.log.push({text:"🎯 But bonus !",type:"info"})):_.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),E(_)},ADD_SUB:({value:_=1},E,z)=>{E.maxSubs=(E.maxSubs||3)+_,E.log.push({text:`🔄 +${_} remplacement(s)`,type:"info"}),z(E)},CUSTOM:({},_,E)=>E(_)};function ie(_,E,z,A){const T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let F=[];function B(){var U,Q;const q=_.map(ne=>{const Z=ne._line||ne.job||"MIL",J=F.find(de=>de.cardId===ne.cardId),se=Se({...ne,_evolution_bonus:0},{width:90,showStad:!0,role:Z,extraNote:ne.boost||0});return`<div class="pp-item" data-cid="${ne.cardId}"
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${Rt(_,f[p+"Formation"],ut,Q,ne?1300:zt(),ne?600:Pt(),Ue)}
      </div>
      ...
      <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
        ...
        <div id="mobile-play-area" style="flex:1;min-height:0;display:flex;overflow:hidden">
          <div id="match-field" style="flex:1;min-width:0;min-height:0;overflow:hidden">
            <div class="terrain-wrapper" style="width:100%;height:100%;overflow:hidden">
              ${Rt(_,f[p+"Formation"],ut,Q,zt(),Pt(),Ue)}
            </div>
          </div>
        </div>
        <div id="mobile-action-bar" style="position:absolute;left:0;right:0;bottom:0;z-index:20;background:rgba(0,0,0,0.55);padding:6px 8px 8px;display:flex;flex-direction:column;gap:6px;box-shadow:0 -4px 16px rgba(0,0,0,0.5)">
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;overflow-y:auto;background:#0a3d1e">
      ${Vo(f[u+"Team"],f[u+"Formation"],null,f[u+"Name"]||"Adversaire")}
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      </div>`,(Ne=Z.querySelector("#psub-close"))==null||Ne.addEventListener("click",()=>Z.remove()),(Ie=Z.querySelector("#pout-up"))==null||Ie.addEventListener("click",()=>{q>0&&(q--,J())}),(We=Z.querySelector("#pout-down"))==null||We.addEventListener("click",()=>{q<F.length-1&&(q++,J())}),(Ue=Z.querySelector("#pin-up"))==null||Ue.addEventListener("click",()=>{Q>0&&(Q--,J())}),(it=Z.querySelector("#pin-down"))==null||it.addEventListener("click",()=>{Q<B.length-1&&(Q++,J())});const Le=(qe,Xe,Ke,pt)=>{const nt=Z.querySelector("#"+qe);if(!nt)return;let St=0;nt.addEventListener("touchstart",ut=>{St=ut.touches[0].clientY},{passive:!0}),nt.addEventListener("touchend",ut=>{const _t=ut.changedTouches[0].clientY-St;if(Math.abs(_t)<30)return;const ot=Xe();_t<0&&ot<pt-1?(Ke(ot+1),J()):_t>0&&ot>0&&(Ke(ot-1),J())},{passive:!0})};Le("pin-panel",()=>Q,qe=>Q=qe,B.length),Le("pout-panel",()=>q,qe=>q=qe,F.length),(Ht=Z.querySelector("#psub-confirm"))==null||Ht.addEventListener("click",async qe=>{if(qe.preventDefault(),qe.stopPropagation(),ne)return;ne=!0;const Xe=F[q],Ke=B[Q];if(!Xe||!Ke)return;const pt=Xe._line,nt=(z[pt]||[]).findIndex(_t=>_t.cardId===Xe.cardId);if(nt===-1){m("Erreur : joueur introuvable","error"),Z.remove();return}const St={...Ke,_line:pt,position:Xe.position,used:!1,boost:0};z[pt].splice(nt,1,St);const ut=[...A,Ke.cardId];Z.remove(),te(Xe,Ke,async()=>{await j({[p+"Team"]:z,[u+"Team"]:f[u+"Team"],["usedSubIds_"+p]:ut})})})}document.body.appendChild(Z),J()}function te(_,E,z){const A={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const F=(U,Q,ne)=>`<div style="text-align:center">
      <div style="font-size:9px;color:${Q};letter-spacing:2px;text-transform:uppercase;font-weight:700;margin-bottom:6px">${ne}</div>
      <div style="width:70px;height:70px;border-radius:50%;background:${A[U.job]||"#555"};border:3px solid ${Q};position:relative;overflow:hidden;margin:0 auto">
    ...
    </div>`,(T=document.getElementById("pvp-home"))==null||T.addEventListener("click",()=>{try{w.removeChannel(ee)}catch{}et(e),d?g("mini-league",{openLeagueId:d}):g("home")})}W()}async function Ii(e,t,i=!1){var s,l;const{state:n,navigate:r,toast:o}=t,a=i&&((l=(s=t==null?void 0:t.state)==null?void 0:s.params)==null?void 0:l.rankedData)||null;await fi(e,t,i?"ranked":"random",async({deckId:c,formation:d,starters:x,subsRaw:y,gcCardsEnriched:g,gcDefs:m,stadiumDef:p})=>{dt(e);const u=async b=>{await fr(e,t,c,d,x,y,b||[],m,p,i,a)};if(!(g!=null&&g.length)){await u([]);return}pi(e,g,u)})}async function ur(e,t,i){return pr(e,t,i)}async function fr(e,t,i,n,r,o,a=[],s=[],l=null,c=!1,d=null){var re,ee;const{state:x,navigate:y,toast:g}=t,m=(d==null?void 0:d.mmr)??((re=x.profile)==null?void 0:re.mmr)??1e3,p=c?Ci(m):null,u=c?"linear-gradient(135deg, #1a0a2e 0%, #2d1060 50%, #1a0a2e 100%)":"linear-gradient(135deg, #0a1a2e 0%, #0d3d1e 100%)",b=c?(p==null?void 0:p.color)||"#D4A017":"#FFD700";let v=!1,h=null,$=null;e.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:20px;display:flex;flex-direction:column;gap:14px">
      ...
      </div>`,document.body.appendChild(t);const i=t.querySelector("#ml-pwd-input"),n=r=>{t.remove(),e(r)};setTimeout(()=>i==null?void 0:i.focus(),80),t.querySelector("#pwd-cancel").addEventListener("click",()=>n(null)),t.querySelector("#pwd-ok").addEventListener("click",()=>n(i.value.trim())),i.addEventListener("keydown",r=>{r.key==="Enter"&&n(i.value.trim())})})}async function Bn(e,t,i,n){var x;const{toast:r,state:o}=t,a=o.profile.id;let s=null;if(n==="private"&&(s=await $r(),s===null))return;const{data:l,error:c}=await w.rpc("join_mini_league",{p_league_id:i,p_user_id:a,p_password:s});if(c||!(l!=null&&l.success)){console.error("[MiniLeague] join_mini_league:",c||l),r((l==null?void 0:l.error)||"Erreur lors de l'inscription","error");return}if(typeof t.refreshProfile=="function")await t.refreshProfile();else{const{data:y}=await w.from("users").select("credits").eq("id",a).single();o.profile&&y&&(o.profile.credits=y.credits)}const d=document.getElementById("nav-credits");d&&(d.textContent=`💰 ${(((x=o.profile)==null?void 0:x.credits)||0).toLocaleString("fr")}`),r("✅ Inscrit ! Pot mis à jour.","success"),ct(e,t,i)}async function Er(e,t,i,n){const{toast:r,state:o}=t,a=o.profile.id;if(!await new Promise(y=>{const g=document.createElement("div");g.className="modal-overlay",g.style.zIndex="2100",g.innerHTML=`<div class="modal" style="max-width:360px">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
  ...
  </div>`;const h=a.status==="waiting"?"waiting":a.status==="active"?"active":"archived";if(($=document.getElementById("ml-back"))==null||$.addEventListener("click",()=>Ze(e,t,h)),(f=document.getElementById("ml-refresh"))==null||f.addEventListener("click",async j=>{const L=j.currentTarget;L.style.opacity="0.5",await ct(e,t,i)}),(M=document.getElementById("ml-start-btn"))==null||M.addEventListener("click",()=>Lr(e,t,a,x)),(k=document.getElementById("ml-next-day"))==null||k.addEventListener("click",()=>zr(e,t,i)),(P=document.getElementById("ml-join-now"))==null||P.addEventListener("click",()=>Bn(e,t,i,a.type)),(H=document.getElementById("ml-leave-btn"))==null||H.addEventListener("click",()=>Er(e,t,i,p)),(ae=document.getElementById("ml-delete-btn"))==null||ae.addEventListener("click",()=>Ai(e,t,i,a.name,"waiting")),(re=document.getElementById("ml-delete-now"))==null||re.addEventListener("click",()=>Ai(e,t,i,a.name,h)),(ee=document.getElementById("ml-archive-btn"))==null||ee.addEventListener("click",()=>Tr(e,t,i)),e.querySelectorAll("[data-play-match]").forEach(j=>{j.addEventListener("click",()=>{const L=g.find(V=>V.id===j.dataset.playMatch);L&&t.navigate("match-mini-league",{mlMatchId:L.id,leagueId:i})})}),a.status==="finished"&&v){const j=y.findIndex(L=>L.userId===o);j>=0&&j<3&&v.prize_amount>0&&!v.prize_claimed&&setTimeout(()=>Ir(e,t,a,v,j),400)}}function ln(e,t,i,n,r=!1){const o=x=>t.find(y=>y.id===x);if(e.is_bye){const x=o(e.home_id);return`<div style="padding:8px;border-radius:8px;background:rgba(255,255,255,0.03);margin-bottom:6px;font-size:12px;color:${st};text-align:center">🔵 ${(x==null?void 0:x.club_name)||(x==null?void 0:x.pseudo)||"?"} exempté(e)</div>`}const a=o(e.home_id),s=o(e.away_id),l=e.home_id===i||e.away_id===i,c=l&&e.status==="pending"&&n&&!r,d=e.status==="finished"?`${e.home_score} - ${e.away_score}`:"vs";return`<div style="display:flex;align-items:center;gap:8px;padding:10px;border-radius:8px;background:${l?"rgba(26,107,60,0.16)":"rgba(255,255,255,0.03)"};margin-bottom:6px;border:1px solid ${l?"rgba(74,222,128,0.35)":Tt}">
  ...
  </div>`}async function Lr(e,t,i,n){const{toast:r,state:o}=t,a=Ar(n.map(c=>c.id),i.mode),s=[];a.forEach((c,d)=>c.forEach(x=>s.push({league_id:i.id,matchday:d+1,home_id:x.home||x.bye,away_id:x.away||null,is_bye:!!x.bye,status:x.bye?"bye":"pending"})));const{error:l}=await w.from("mini_league_matches").insert(s);if(l){r("Erreur calendrier : "+l.message,"error");return}await w.from("mini_leagues").update({status:"active",current_day:1,total_days:a.length}).eq("id",i.id),r(`🚀 Lancée ! Pot : ${(i.pot||0).toLocaleString("fr")} cr.`,"success"),ct(e,t,i.id)}async function zr(e,t,i){const{data:n}=await w.from("mini_leagues").select("current_day,total_days,pot").eq("id",i).single(),r=(n.current_day||0)+1;if(r>(n.total_days||0)){ct(e,t,i);return}await w.from("mini_leagues").update({current_day:r}).eq("id",i),t.toast(`Journée ${r} commencée !`,"success"),ct(e,t,i)}async function Ir(e,t,i,n,r){var y,g;const{state:o,toast:a}=t,s=[Math.floor((i.pot||0)*.7),Math.floor((i.pot||0)*.2),Math.floor((i.pot||0)*.1)],l=["🥇","🥈","🥉"][r],c=n.prize_amount||s[r]||0,d=n.prize_claimed,x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",x.innerHTML=`
    ...
    <p style="margin-top:12px;font-size:13px;color:#888">Tu peux revoir ce tutoriel depuis les paramètres à tout moment.</p>`}];function Br(e,t,i){let n=0;const r=document.createElement("div");r.id="tutorial-overlay",r.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.65);z-index:9900;display:flex;align-items:center;justify-content:center;padding:16px";const o=()=>{var d,x,y;const s=t[n],l=n===t.length-1,c=Math.round((n+1)/t.length*100);r.innerHTML=`
      <div style="background:#fff;border-radius:20px;width:100%;max-width:420px;max-height:90vh;display:flex;flex-direction:column;overflow:hidden;box-shadow:0 16px 64px rgba(0,0,0,0.4)">
        ...
        <!-- Image optionnelle -->
        ${s.image_url?`<div style="padding:0 24px 8px;text-align:center"><img src="${jr}icons/${s.image_url}" style="max-height:160px;max-width:100%;border-radius:12px;object-fit:contain"></div>`:""}
        <!-- Contenu -->
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    </div>`,document.body.appendChild(r);const o=s=>{r.remove(),s&&t()};r.querySelector("#buy-cancel").addEventListener("click",()=>o(!1)),r.querySelector("#buy-ok").addEventListener("click",()=>o(!0)),r.addEventListener("click",s=>{s.target===r&&o(!1)})}async function Gr(e,t,i){const{toast:n}=i,{data:r}=await w.from("market_listings").select("card_id").eq("id",e).single();if(await w.from("market_listings").update({status:"cancelled"}).eq("id",e),r!=null&&r.card_id){const{count:o}=await w.from("market_listings").select("id",{count:"exact",head:!0}).eq("card_id",r.card_id).eq("status","active");o||await w.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",r.card_id)}n("Annonce retirée","success"),Ri(t,i)}async function Rr(e,t){var d,x,y,g;const{state:i,navigate:n}=t,r=((x=(d=t==null?void 0:t.state)==null?void 0:d.params)==null?void 0:x.tab)||"global";e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const[{data:o},{data:a}]=await Promise.all([w.from("users").select("id,pseudo,club_name,trophies_top1,trophies_top2,trophies_top3,wins,level").order("trophies_top1",{ascending:!1}).limit(100),w.from("users").select("id,pseudo,club_name,mmr,rank_tier,ranked_wins,ranked_losses,ranked_draws,placement_matches").gte("placement_matches",1).order("mmr",{ascending:!1}).limit(100)]);let s=r;function l(){var p,u;const m=document.getElementById("rankings-list");if(m){if(s==="global"){const b=o||[];m.innerHTML=b.length>0?b.map((v,h)=>`
  ...
  `,document.querySelectorAll(".bottom-nav a").forEach(r=>{r.addEventListener("click",o=>{o.preventDefault(),Vt(r.dataset.page)})}),document.getElementById("nav-logo").addEventListener("click",()=>Vt("home")),document.getElementById("nav-credits").addEventListener("click",()=>Vt("boosters")),(n=document.getElementById("journal-btn"))==null||n.addEventListener("click",()=>Kr())}async function Kr(){const{data:e}=await w.from("patch_notes").select("*").eq("is_published",!0).order("published_at",{ascending:!1}).limit(20),t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9000;display:flex;align-items:center;justify-content:center;padding:16px";const i=(e||[]).map(n=>{const r=new Date(n.published_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"long",year:"numeric"});return`<div style="padding:14px 0;border-bottom:1px solid #f0f0f0">
    ...
    </div>`,document.body.appendChild(t),t.querySelector("#journal-close").addEventListener("click",()=>t.remove()),t.addEventListener("click",n=>{n.target===t&&t.remove()})}function Vr(e,{onPlay:t}){var n;const i="/icons/";e.style.cssText="",e.innerHTML=`
  <style>
    #public-landing{
  ...
  </div>`,(n=e.querySelector("#pl-play-btn"))==null||n.addEventListener("click",t)}async function Yr(){document.documentElement.setAttribute("data-theme","light"),document.getElementById("modal-overlay").addEventListener("click",i=>{i.target===i.currentTarget&&Si()}),document.getElementById("modal-close").addEventListener("click",Si);const{data:{session:e}}=await w.auth.getSession();if(!e){pn(),Vr(document.getElementById("app"),{onPlay:()=>Wi(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:Ft})});return}we.user=e.user,await Kt(),pn();try{const{data:i}=await w.from("formation_links_overrides").select("formation, links"),n={};(i||[]).forEach(r=>{n[r.formation]=r.links}),Nn(n)}catch(i){console.warn("Impossible de charger les overrides de formation:",i)}if(!we.profile){Un(document.getElementById("app"),{state:we,navigate:async()=>{await Kt(),_i()},toast:Ft,refreshProfile:Kt});return}const t=Array.isArray(we.profile.pending_boosters)?we.profile.pending_boosters:[];if(!we.profile.onboarding_done&&t.length>0){No(document.getElementById("app"),{state:we,navigate:()=>_i(),toast:Ft,refreshProfile:Kt});return}_i(),setTimeout(()=>qr(we.profile,Vt,Ft),800),w.auth.onAuthStateChange(async(i,n)=>{i==="SIGNED_OUT"&&(we.user=null,we.profile=null,document.getElementById("app").innerHTML="",Wi(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:Ft}))})}function Wr(){return Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight)}function gi(){const e=document.getElementById("app");e&&(e.style.height=Wr()+"px")}window.addEventListener("resize",gi);window.addEventListener("orientationchange",()=>setTimeout(gi,150));window.visualViewport&&window.visualViewport.addEventListener("resize",gi);function _i(){const e=()=>{var i;(i=we.user)!=null&&i.id&&w.from("users").update({last_seen_at:new Date().toISOString()}).eq("id",we.user.id).then(()=>{})};e(),window._presencePingInterval&&clearInterval(window._presencePingInterval),window._presencePingInterval=setInterval(e,6e4);const t=document.getElementById("app");t.style.display="flex",t.style.flexDirection="column",gi(),Ur(),qn()}function pn(){const e=document.getElementById("app-loader"),t=document.getElementById("app");t&&(t.style.display=""),e&&(e.classList.add("zoom-out"),setTimeout(()=>e.style.display="none",500))}function Dn(e){var n;const t=document.getElementById("app-loader");if(t&&(t.style.display="none"),document.getElementById("boot-error"))return;const i=document.createElement("div");i.id="boot-error",i.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:99999;gap:16px;color:#fff;padding:24px;text-align:center",i.innerHTML=`
    <div style="font-size:42px">📡</div>
    <div style="font-size:18px;font-weight:900">Connexion impossible</div>
    <div style="font-size:13px;color:rgba(255,255,255,0.6);max-width:280px">${e||"Le chargement a échoué. Vérifie ta connexion et réessaie."}</div>
    <button id="boot-retry" style="margin-top:8px;padding:12px 30px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer">Réessayer</button>`,document.body.appendChild(i),(n=document.getElementById("boot-retry"))==null||n.addEventListener("click",()=>window.location.reload())}Yr().catch(e=>{console.error("Échec du démarrage:",e),Dn()});setTimeout(()=>{const e=document.getElementById("app-loader");e&&e.style.display!=="none"&&!e.classList.contains("zoom-out")&&!document.getElementById("boot-error")&&Dn("Le serveur met trop de temps à répondre.")},12e3);
