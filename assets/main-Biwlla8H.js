import{s as E,F as bi,h as Pt,j as Wi,l as at,i as So,k as Lo,b as Io}from"./formation-links-DJC25wD7.js";const Mo="/manager-wars/";function Ii(e,{navigate:t,toast:i}){let s="login";const c=()=>{var a,l,p,o,n,g;const d=s==="login";e.innerHTML=`
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),o=(l||[]).filter(W=>W.card_type==="player"&&W.player),n=(l||[]).filter(W=>W.card_type==="game_changer"),g=(l||[]).filter(W=>W.card_type==="formation"),v=(l||[]).filter(W=>W.card_type==="stadium"),{data:y}=await E.from("gc_definitions").select("name,gc_type,color,effect,image_url"),z={};(y||[]).forEach(W=>{z[W.name]=W});const{data:r}=await E.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),u={};(r||[]).forEach(W=>{u[W.id]=W}),v.forEach(W=>{W.stadium_def&&(u[W.stadium_id]=W.stadium_def)});const M=Object.keys(bi),L=Object.keys(Ke),N={};o.forEach(W=>{const ie=W.player.id;N[ie]=(N[ie]||0)+1}),new Set(Object.keys(N).map(W=>String(W)));const V=new Set(g.map(W=>W.formation)),C=new Set(n.map(W=>W.gc_type));let F="player",j="Tous",te="",le=!1;function de(){return[...o].sort((W,ie)=>{const K=Vt.indexOf(W.player.job),oe=Vt.indexOf(ie.player.job);return K!==oe?K-oe:(W.player.surname_encoded||"").localeCompare(ie.player.surname_encoded||"")})}function P(){return[...p||[]].sort((W,ie)=>{const K=Vt.indexOf(W.job),oe=Vt.indexOf(ie.job);return K!==oe?K-oe:(W.surname_encoded||"").localeCompare(ie.surname_encoded||"")})}function Y(){return de().filter(W=>{const ie=W.player,K=j==="Tous"||ie.job===j,oe=!te||`${ie.firstname} ${ie.surname_encoded}`.toLowerCase().includes(te);return K&&oe})}function H(){return P().filter(W=>{const ie=j==="Tous"||W.job===j,K=!te||`${W.firstname} ${W.surname_encoded}`.toLowerCase().includes(te);return ie&&K})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",ie=>{te=ie.target.value.toLowerCase(),ne()}),e.querySelectorAll(".filter-btn").forEach(ie=>{ie.addEventListener("click",()=>{j=ie.dataset.job,G(),ne()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{le=!le,G(),ne()})):(W.innerHTML=`
        <div style="display:flex;justify-content:flex-end">
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{le=!le,G(),ne()})))}function ne(){const W=document.getElementById("col-grid");W&&(F==="player"?_e(W):F==="formation"?Ae(W):F==="stadium"?xe(W):ze(W))}function fe(W,ie,K,oe,re){const Q=document.getElementById("col-grid"),ae=document.getElementById("col-big");if(!Q||!ae)return;var ke=0;function Le(){const Fe=window.innerWidth>=768,je=document.getElementById("col-big"),De=document.getElementById("col-grid");Fe&&je&&(je.style.minHeight="420px",je.style.flex="1 1 auto"),Fe&&De&&(De.style.height=Math.round(310*.76+16)+"px",De.style.flexShrink="0",De.style.overflowX="auto",De.style.overflowY="hidden",De.style.alignItems="center",De.style.padding="8px 16px"),ae.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+ie(W[ke])+"</div>";var I=ae.querySelector("[data-card-id],[data-form-id],[data-gc-id]");I&&I.addEventListener("click",function(){oe(W[ke])}),requestAnimationFrame(function(){var q=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!q||!ae)){var w=ae.clientHeight-8,k=ae.clientWidth-24,f=q.offsetHeight,m=q.offsetWidth;if(f>0&&m>0&&w>40){var x=Fe?2.2:1,b=Math.min(w/f,k/m,x);q.style.transform="scale("+b.toFixed(3)+")",q.style.transformOrigin="top center"}}}),Q.innerHTML=W.map(function(q,w){var k=w===ke,f="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+(k?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+w+'" style="'+f+'">'+K(q,k)+"</div>"}).join(""),Q.querySelectorAll(".col-mini-item").forEach(function(q){q.addEventListener("click",function(){ke=Number(q.dataset.idx),Le(),q.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}Le()}function ye(W){var ie=window.innerWidth>=768?.76:.54,K;if(!W||W._fake){var oe=W?W.player:null;if(!oe)return"";K=Fi(oe)}else K=si(W,"");return'<div style="display:inline-block;zoom:'+ie+';pointer-events:none;line-height:0">'+K+"</div>"}function Te(W,ie,K){ie=ie||100,K=K||140;var oe=Pt[W]||{},re={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},Q=Math.max(3,Math.round(ie*.06)),ae=Object.entries(oe).map(function(Le){var Fe=Le[0],je=Le[1],De=Fe.replace(/\d+$/,""),I=re[De]||"#888",q=Math.round(je.x*ie),w=Math.round(je.y*K);return'<circle cx="'+q+'" cy="'+w+'" r="'+Q+'" fill="'+I+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),ke=Math.max(1,Math.round(ie/50));return'<svg viewBox="0 0 '+ie+" "+K+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+ie+'" height="'+K+'" fill="#1A6B3C"/><rect x="'+Math.round(ie*.2)+'" y="'+Math.round(K*.02)+'" width="'+Math.round(ie*.6)+'" height="'+Math.round(K*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+ke+'"/><line x1="0" y1="'+Math.round(K*.5)+'" x2="'+ie+'" y2="'+Math.round(K*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+ke+'"/><ellipse cx="'+Math.round(ie*.5)+'" cy="'+Math.round(K*.5)+'" rx="'+Math.round(ie*.18)+'" ry="'+Math.round(K*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+ke+'"/><rect x="'+Math.round(ie*.2)+'" y="'+Math.round(K*.82)+'" width="'+Math.round(ie*.6)+'" height="'+Math.round(K*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+ke+'"/>'+ae+"</svg>"}function Ee(W,ie,K){var oe=K>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+K+"</div>":"",re=ie?'data-form-id="'+ie.id+'"':"",Q=W.length>7?14:W.length>5?16:19,ae=!!ie;return"<div "+re+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(ae?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(ae?"":"filter:grayscale(1);opacity:0.5")+'">'+oe+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(ae?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+Q+"px;font-weight:900;color:"+(ae?"#1A6B3C":"#aaa")+';line-height:1">'+W+'</div></div><div style="flex:1;overflow:hidden;background:'+(ae?"#1A6B3C":"#ccc")+'">'+Te(W,140,220)+"</div></div>"}function be(W,ie){var K=window.innerWidth>=768?.76:.54,oe=140,re=305,Q=Math.round(re*.22),ae=re-Q,ke=W.length>7?10:13,Le=Te(W,oe,ae),Fe=ie?"1.5px solid #2a7a40":"1px solid #ddd",je=ie?"":"filter:grayscale(1);opacity:0.45;",De=ie?"#1A6B3C":"#bbb",I="#fff";return'<div style="display:inline-block;zoom:'+K+';line-height:0;pointer-events:none"><div style="width:'+oe+"px;height:"+re+"px;border-radius:6px;border:"+Fe+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+je+'"><div style="height:'+Q+"px;background:"+De+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+ke+"px;font-weight:900;color:"+I+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(oe-4)+'px">'+W+'</span></div><div style="height:'+ae+'px;overflow:hidden;flex-shrink:0">'+Le+"</div></div></div>"}function _e(W){if(le){const ie=H();if(!ie.length){W.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const K=ie.map(oe=>o.find(re=>re.player.id===oe.id)||{_fake:!0,player:oe,id:"fake-"+oe.id});fe(K,oe=>oe._fake?Fi(oe.player):si(oe,""),oe=>oe._fake?ye({player:oe.player,id:"x",_fake:!0}):ye(oe),oe=>{oe._fake||Di(oe,o,N,t)})}else{const ie=Y();if(!ie.length){W.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const K={};ie.forEach(re=>{const Q=re.player.id;K[Q]||(K[Q]=[]),K[Q].push(re)});const oe=Object.values(K).map(re=>Xo(re));fe(oe,re=>{const Q=N[re.player.id]||1,ae=Q>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${Q}</div>`:"",Le=o.filter(Fe=>Fe.player.id===re.player.id&&Fe.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return si(re,ae+Le)},re=>ye(re),re=>Di(re,o,N,t))}}function Ae(W){const ie=le?M:[...V];if(!ie.length){W.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const K=ie.map(oe=>({formation:oe,card:g.find(re=>re.formation===oe)||null,owned:V.has(oe)}));fe(K,({formation:oe,card:re,owned:Q})=>Ee(oe,Q?re:null,Q?g.filter(ae=>ae.formation===oe).length:0),({formation:oe,owned:re})=>be(oe,re),({card:oe,owned:re})=>{re&&oe&&en(oe,g,t,d)})}function ze(W){const ie=Object.keys(z),K=le?ie.length?ie:L:[...C];if(!K.length){W.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const oe=K.map(re=>({type:re,gc:Ke[re]||{icon:"⚡",desc:""},def:z[re]||null,owned:C.has(re),card:n.find(Q=>Q.gc_type===re)||null}));fe(oe,({type:re,gc:Q,def:ae,owned:ke,card:Le})=>{var x;const Fe=ke?n.filter(b=>b.gc_type===re).length:0,je=Fe>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${Fe}</div>`:"",De=(ae==null?void 0:ae.gc_type)==="ultra_game_changer",I={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},q={purple:"#b06ce0",light_blue:"#00d4ef"},w=I[ae==null?void 0:ae.color]||I.purple,k=q[ae==null?void 0:ae.color]||q.purple,f=(ae==null?void 0:ae.effect)||Q.desc||"",m=ae!=null&&ae.image_url?`/manager-wars/icons/${ae.image_url}`:((x=ae==null?void 0:ae.club)==null?void 0:x.logo_url)||(ae!=null&&ae.country_code?`https://flagcdn.com/w80/${ae.country_code.toLowerCase()}.png`:null);return ke&&Le?`<div data-gc-id="${Le.id}" data-gc-type="${re}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${k};background:${w};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${k}66;cursor:pointer">
          ${je}
          ...
          <div style="height:150px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
            ${m?`<img src="${m}" style="max-width:120px;max-height:120px;object-fit:contain;border-radius:6px">`:`<span style="font-size:64px">${Q.icon}</span>`}
          </div>
        ...
        </div>`},({type:re,gc:Q,def:ae,owned:ke})=>{const Le=window.innerWidth>=768?.76:.54,Fe={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},je={purple:"#9b59b6",light_blue:"#00bcd4"},De=Fe[ae==null?void 0:ae.color]||Fe.purple,I=je[ae==null?void 0:ae.color]||je.purple,q=ae!=null&&ae.image_url?`/manager-wars/icons/${ae.image_url}`:null;return ke?`<div style="display:inline-block;zoom:${Le};line-height:0;pointer-events:none"><div style="width:140px;height:310px;border-radius:8px;background:${De};border:1px solid ${I};display:flex;flex-direction:column;overflow:hidden"><div style="height:56px;background:rgba(255,255,255,0.15);display:flex;align-items:center;justify-content:center"><span style="font-size:13px;font-weight:900;color:#fff;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:130px">${re}</span></div><div style="flex:1;display:flex;align-items:center;justify-content:center">${q?`<img src="${q}" style="max-width:130px;max-height:190px;object-fit:contain">`:`<span style="font-size:48px">${Q.icon}</span>`}</div><div style="height:54px;display:flex;align-items:center;justify-content:center;padding:0 4px"><span style="font-size:10px;color:rgba(255,255,255,0.7);text-align:center">${((ae==null?void 0:ae.effect)||Q.desc||"").slice(0,30)}</span></div></div></div>`:`<div style="display:inline-block;zoom:${Le};line-height:0;pointer-events:none"><div style="width:140px;height:310px;border-radius:8px;background:#eee;border:1px solid #ddd;display:flex;flex-direction:column;align-items:center;justify-content:center;filter:grayscale(1);opacity:0.45"><span style="font-size:36px">${Q.icon}</span><span style="font-size:12px;color:#aaa;margin-top:6px;text-align:center;padding:0 6px">${re}</span></div></div>`},({type:re,owned:Q,def:ae})=>{Q&&Qo(re,ae,d)})}function xe(W){const ie="#E87722",K="/manager-wars/";if(!v.length){W.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Stade.<br><small>Ouvre un booster Stade !</small></div>';return}const oe={};v.forEach(Q=>{const ae=Q.stadium_id||"?";(oe[ae]=oe[ae]||[]).push(Q)});const re=Object.entries(oe).map(([Q,ae])=>({sid:Q,def:u[Q]||null,count:ae.length,card:ae[0]}));fe(re,({def:Q,count:ae})=>{var I,q;const ke=(Q==null?void 0:Q.name)||"?",Le=((I=Q==null?void 0:Q.club)==null?void 0:I.encoded_name)||(Q==null?void 0:Q.country_code)||"—",Fe=Q!=null&&Q.image_url?`${K}icons/${Q.image_url}`:((q=Q==null?void 0:Q.club)==null?void 0:q.logo_url)||(Q!=null&&Q.country_code?`https://flagcdn.com/w80/${Q.country_code.toLowerCase()}.png`:null),je=Fe?`<img src="${Fe}" style="width:90px;height:90px;object-fit:contain;border-radius:4px" onerror="this.style.display='none'">`:'<div style="font-size:56px">🏟️</div>',De=ae>1?`<div style="position:absolute;top:8px;right:8px;background:#333;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${ae}</div>`:"";return`<div style="position:relative;width:140px;border-radius:12px;border:3px solid #c45a00;background:linear-gradient(160deg,${ie},#c45a00);display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${ie}66">
          ${De}
          ...
          <div style="height:140px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.08)">${je}</div>
        ...
        </div>`},({def:Q,count:ae})=>{var I,q;const ke=window.innerWidth>=768?.76:.54,Le=(Q==null?void 0:Q.name)||"?",Fe=((I=Q==null?void 0:Q.club)==null?void 0:I.encoded_name)||(Q==null?void 0:Q.country_code)||"—",je=Q!=null&&Q.image_url?`${K}icons/${Q.image_url}`:((q=Q==null?void 0:Q.club)==null?void 0:q.logo_url)||(Q!=null&&Q.country_code?`https://flagcdn.com/w80/${Q.country_code.toLowerCase()}.png`:null),De=je?`<img src="${je}" style="width:64px;height:64px;object-fit:contain;border-radius:4px" onerror="this.style.display='none'">`:'<span style="font-size:32px">🏟️</span>';return`<div style="display:inline-block;zoom:${ke};line-height:0;pointer-events:none">
    ...
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${g.map((K,oe)=>{const re=ne[K.id]||[],Q=K.is_for_sale,ae=re.length?re[re.length-1]:null,Le=(n.rarity==="pepite"||n.rarity==="papyte")&&K.current_note!=null?` (☆${K.current_note})`:"";return`
            <div data-card-id="${K.id}" data-card-idx="${oe}" class="exemplaire-row"
              style="border:2px solid #eee;border-radius:10px;padding:10px;cursor:pointer;transition:border-color .15s;${Q?"opacity:0.6":""}">
              <div style="display:flex;align-items:center;gap:10px">
                <input type="checkbox" class="expl-check" data-id="${K.id}" ${Q?"disabled":""} style="width:18px;height:18px;cursor:pointer;accent-color:#1A6B3C;flex-shrink:0">
                <div style="flex:1;min-width:0">
                  <div style="font-size:12px;font-weight:700">Exemplaire ${oe+1}${Le}${Q?" 🏷️ En vente":""}</div>
      ...
      <div style="width:140px;border-radius:12px;padding:6px;background:${j};flex-shrink:0">
        <div style="background:#f2e8d2;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
          ...
          <div style="background:#f2e8d2;padding:3px 6px;display:flex;align-items:center;justify-content:space-between;min-height:24px">
            ...
            ${(_e=n.clubs)!=null&&_e.logo_url?`<img src="${n.clubs.logo_url}" style="width:22px;height:16px;object-fit:contain">`:`<div style="background:#1a3a7a;color:#fff;border-radius:2px;padding:1px 3px;font-size:6px;font-weight:800">${(((Ae=n.clubs)==null?void 0:Ae.encoded_name)||"").slice(0,6)}</div>`}
          </div>
        </div>
      </div>
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",n.note_g],["DEF",n.note_d],["MIL",n.note_m],["ATT",n.note_a]].map(([K,oe])=>{const re=jt[K],Q=Number(oe)||0;return`<div style="display:flex;flex-direction:column;align-items:center;gap:2px">
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(ze=document.getElementById("close-detail"))==null||ze.addEventListener("click",l);let Te=new Set;const Ee=()=>{const K=Te.size,oe=document.getElementById("sell-action-panel");if(!oe)return;oe.style.display=K>0?"block":"none",document.getElementById("sell-selected-count").textContent=K;const re=document.getElementById("evolve-btn");re&&(re.textContent=`⬆️ Évoluer ${K>1?"(+"+K+")":""}`)};document.querySelectorAll(".expl-check").forEach(K=>{K.addEventListener("change",()=>{const oe=K.dataset.id;K.checked?Te.add(oe):Te.delete(oe);const re=K.closest(".exemplaire-row");re&&(re.style.borderColor=K.checked?"#1A6B3C":"#eee"),Ee()})}),document.querySelectorAll(".exemplaire-row").forEach(K=>{K.addEventListener("click",oe=>{if(oe.target.closest("button")||oe.target.tagName==="INPUT")return;const re=K.querySelector(".expl-check");re&&!re.disabled&&(re.checked=!re.checked,re.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(K=>{K.addEventListener("click",oe=>{oe.stopPropagation();const re=document.querySelector(`.expl-hist[data-hist="${K.dataset.idx}"]`);re&&(re.style.display=re.style.display==="none"?"flex":"none")})}),(xe=document.getElementById("evolve-btn"))==null||xe.addEventListener("click",async()=>{const K=[...Te];if(!K.length)return;const oe=playerCards.filter(Q=>K.includes(Q.id));if(!oe.length)return;const re=[];if(await Promise.all(oe.map(async Q=>{const ae=(Q.evolution_bonus||0)+1,{error:ke}=await E.from("cards").update({evolution_bonus:ae}).eq("id",Q.id);ke&&re.push(ke.message)})),re.length){d("Erreur : "+re[0],"error");return}d(`⬆️ ${K.length} carte${K.length>1?"s":""} évoluée${K.length>1?"s":""} ! (+1 par carte)`,"success",3e3),l(),p("collection")}),(W=document.getElementById("market-sell-btn"))==null||W.addEventListener("click",async()=>{var ke;const K=[...Te];if(!K.length){d("Sélectionne au moins un exemplaire","warning");return}const oe=parseInt((ke=document.getElementById("sell-market-price"))==null?void 0:ke.value);if(!oe||oe<1){d("Prix invalide","error");return}const{error:re}=await E.from("cards").update({is_for_sale:!0,sale_price:oe}).in("id",K);if(re){d(re.message,"error");return}const Q=K.map(Le=>({seller_id:c.profile.id,card_id:Le,price:oe,status:"active"})),{error:ae}=await E.from("market_listings").insert(Q);ae&&console.warn("[Market] insert listings:",ae.message),d(`${K.length} carte${K.length>1?"s":""} mise${K.length>1?"s":""} en vente à ${oe.toLocaleString("fr")} cr. chacune !`,"success"),l(),p("collection")}),(ie=document.getElementById("cancel-sell-btn"))==null||ie.addEventListener("click",async()=>{await E.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await E.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),d("Annonce retirée","success"),l(),p("collection")})}const Xt={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},vt={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function ti(e){const t=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function lo(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function co(e){var i;const t="https://fcnwclxlkytdfjotqkta.supabase.co";return(i=e==null?void 0:e.clubs)!=null&&i.logo_url?e.clubs.logo_url.startsWith("http")?e.clubs.logo_url:`${t}/storage/v1/object/public/assets/clubs/${e.clubs.logo_url}`:null}function tn(e,t=44,i=58){var y;const s=e?ti(e):null,c=e?co(e):null,d=lo(e==null?void 0:e.country_code),a=(e==null?void 0:e.job)||"MIL",l=vt[a]||"#555",p={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",o=Number(a==="GK"?e==null?void 0:e.note_g:a==="DEF"?e==null?void 0:e.note_d:a==="MIL"?e==null?void 0:e.note_m:e==null?void 0:e.note_a)||0,n=Math.round(i*.19),g=Math.round(i*.25),v=i-n-g;return e?`<div style="width:${t}px;height:${i}px;border-radius:5px;border:2px solid ${p};background:${l};position:relative;overflow:hidden;flex-shrink:0">
  ...
  `,document.body.appendChild(c);let d=!1;const a=document.getElementById("pack-cut-zone"),l=document.getElementById("pack-blade");let p=!1;const o=G=>G.touches&&G.touches[0]?G.touches[0].clientX:G.clientX;function n(G){d||(p=!0,l.style.opacity="1",g(G))}function g(G){if(!p||d)return;const ne=a.getBoundingClientRect(),fe=o(G)-ne.left,ye=Math.max(0,Math.min(1,fe/ne.width));l.style.width=ye*ne.width+"px",ye>=.82&&y()}function v(){d||(p=!1,l.style.transition="width .2s ease, opacity .2s ease",l.style.width="0",l.style.opacity="0",setTimeout(()=>{d||(l.style.transition="")},220))}function y(){var ne;if(d)return;d=!0,p=!1,l.style.width="100%",l.style.opacity="1",(ne=document.getElementById("cut-flash"))==null||ne.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const G=document.getElementById("cut-hint");G&&(G.style.opacity="0"),a.classList.add("pack-cut"),setTimeout(()=>{l.style.opacity="0",document.getElementById("pack-phase").style.display="none",M(0)},620)}a.addEventListener("pointerdown",n),window.addEventListener("pointermove",g),window.addEventListener("pointerup",v),a.addEventListener("touchstart",n,{passive:!0}),window.addEventListener("touchmove",g,{passive:!0}),window.addEventListener("touchend",v);let z=0,r=!1;const u=new Set;function M(G){z=G,document.getElementById("reveal-phase").style.display="flex",L(),N(G,0),te()}function L(){const G=document.getElementById("card-dots");G&&(G.innerHTML=e.map((ne,fe)=>`<div class="card-dot" data-i="${fe}" style="width:8px;height:8px;border-radius:50%;background:${fe===z?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),G.querySelectorAll(".card-dot").forEach(ne=>ne.addEventListener("click",()=>C(parseInt(ne.dataset.i)))))}function N(G,ne){var xe;const fe=e[G],ye=document.getElementById("card-counter"),Te=document.getElementById("card-track");ye&&(ye.textContent=`Carte ${G+1} / ${e.length}`);const Ee=document.getElementById("reveal-btns");Ee&&(Ee.style.display=G===e.length-1?"flex":"none");const be=fe.card_type==="player"&&((xe=fe.player)==null?void 0:xe.rarity)==="legende",_e=!u.has(G);u.add(G);let Ae=0;if(fe.card_type==="player"&&fe.player){const W=fe.player,ie=W.job||"ATT";Ae=Number(ie==="GK"?W.note_g:ie==="DEF"?W.note_d:ie==="MIL"?W.note_m:W.note_a)||0}const ze=W=>{Te.innerHTML=`
        ...
        </div>`;const ie=document.getElementById("current-card-wrap");ne!==0?(ie.style.transition="none",ie.style.transform=`translateX(${ne>0?100:-100}%)`,requestAnimationFrame(()=>{ie.style.transition="transform .28s cubic-bezier(.25,1,.5,1)",ie.style.transform="translateX(0)"})):ie.animate([{opacity:0,transform:"scale(.7)"},{opacity:1,transform:"scale(1)"}],{duration:300,easing:"cubic-bezier(.34,1.56,.64,1)"}),W||be?de():P(),L()};_e&&Ae>6&&fe.card_type==="player"&&fe.player?V(fe,()=>ze(!0)):ze(!1)}function V(G,ne){var Ae;r=!0;const fe=G.player,ye=`https://flagsapi.com/${fe.country_code}/flat/64.png`,Te=(Ae=fe.clubs)==null?void 0:Ae.logo_url,Ee=document.getElementById("walkout-overlay"),be=document.getElementById("walkout-stage");if(!Ee||!be){r=!1,ne();return}Ee.style.display="flex";const _e=()=>{const ze=be.firstElementChild;ze&&(ze.classList.remove("wo-in"),ze.classList.add("wo-out"))};be.innerHTML=`<img class="wo-in" src="${ye}" style="height:130px;border-radius:10px;box-shadow:0 10px 36px rgba(0,0,0,.6)" onerror="this.style.display='none'">`,navigator.vibrate&&navigator.vibrate(30),setTimeout(_e,2e3),setTimeout(()=>{var ze;be.innerHTML=Te?`<img class="wo-in" src="${Te}" style="max-height:160px;max-width:210px;object-fit:contain">`:`<div class="wo-in" style="font-size:34px;font-weight:900;color:#fff;text-align:center">${((ze=fe.clubs)==null?void 0:ze.encoded_name)||"CLUB"}</div>`,navigator.vibrate&&navigator.vibrate(30)},2450),setTimeout(_e,4450),setTimeout(()=>{Ee.style.display="none",be.innerHTML="",r=!1,navigator.vibrate&&navigator.vibrate([40,30,80]),ne()},4900)}function C(G){if(r||G<0||G>=e.length||G===z)return;const ne=G>z?1:-1;z=G,N(G,ne)}function F(){C(z+1)}function j(){C(z-1)}function te(){const G=document.getElementById("card-viewport");if(!G||G._swipeBound)return;G._swipeBound=!0;let ne=0,fe=0,ye=0,Te=!1;const Ee=xe=>xe.touches?xe.touches[0].clientX:xe.clientX,be=xe=>xe.touches?xe.touches[0].clientY:xe.clientY,_e=xe=>{Te=!0,ne=Ee(xe),fe=be(xe),ye=0},Ae=xe=>{if(!Te)return;ye=Ee(xe)-ne;const W=be(xe)-fe;if(Math.abs(ye)<Math.abs(W))return;const ie=document.getElementById("current-card-wrap");ie&&(ie.style.transition="none",ie.style.transform=`translateX(${ye*.6}px) rotate(${ye*.02}deg)`)},ze=()=>{if(!Te)return;Te=!1;const xe=document.getElementById("current-card-wrap"),W=55;ye<=-W&&z<e.length-1?F():ye>=W&&z>0?j():xe&&(xe.style.transition="transform .2s ease",xe.style.transform="translateX(0)")};G.addEventListener("pointerdown",_e),G.addEventListener("pointermove",Ae),G.addEventListener("pointerup",ze),G.addEventListener("pointercancel",ze),G.addEventListener("touchstart",_e,{passive:!0}),G.addEventListener("touchmove",Ae,{passive:!0}),G.addEventListener("touchend",ze),G.addEventListener("click",xe=>{if(Math.abs(ye)>8)return;const W=G.getBoundingClientRect();xe.clientX-W.left>W.width/2?F():j()})}let le=null;function de(){const G=document.getElementById("fireworks-canvas");if(!G)return;G.width=window.innerWidth,G.height=window.innerHeight;const ne=G.getContext("2d"),fe=[];function ye(){const Ee=Math.random()*G.width,be=Math.random()*G.height*.6,_e=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],Ae=_e[Math.floor(Math.random()*_e.length)];for(let ze=0;ze<60;ze++){const xe=Math.PI*2/60*ze,W=2+Math.random()*5;fe.push({x:Ee,y:be,vx:Math.cos(xe)*W,vy:Math.sin(xe)*W,alpha:1,color:Ae,size:2+Math.random()*3})}}ye(),le=setInterval(ye,600);function Te(){if(document.getElementById("fireworks-canvas")){ne.clearRect(0,0,G.width,G.height);for(let Ee=fe.length-1;Ee>=0;Ee--){const be=fe[Ee];if(be.x+=be.vx,be.y+=be.vy+.08,be.vy*=.98,be.alpha-=.018,be.alpha<=0){fe.splice(Ee,1);continue}ne.globalAlpha=be.alpha,ne.fillStyle=be.color,ne.beginPath(),ne.arc(be.x,be.y,be.size,0,Math.PI*2),ne.fill()}ne.globalAlpha=1,(le!==null||fe.length>0)&&requestAnimationFrame(Te)}}Te()}function P(){le!==null&&(clearInterval(le),le=null);const G=document.getElementById("fireworks-canvas");G&&G.getContext("2d").clearRect(0,0,G.width,G.height)}if(s){const G=document.getElementById("reveal-btns");G&&(G.innerHTML='<button class="btn btn-primary" id="reveal-next" style="flex:1">Continuer →</button>'),(H=document.getElementById("reveal-next"))==null||H.addEventListener("click",()=>{P(),c.remove(),s()})}else document.getElementById("reveal-collection").addEventListener("click",()=>{P(),c.remove(),i("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{P(),c.remove(),i("boosters")})}function yn(e){var t,i,s,c;if(e.card_type==="player"&&e.player)return un(e);if(e.card_type==="game_changer"){const d=e._gcDef,a=(d==null?void 0:d.gc_type)==="ultra_game_changer",l={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},p={purple:"#b06ce0",light_blue:"#00d4ef"},o=l[d==null?void 0:d.color]||l.purple,n=p[d==null?void 0:d.color]||p.purple,g=(d==null?void 0:d.name)||e.gc_type||"Game Changer",v=(d==null?void 0:d.effect)||((t=ui[e.gc_type])==null?void 0:t.desc)||"",y=d!=null&&d.image_url?`/manager-wars/icons/${d.image_url}`:null,z=((i=ui[e.gc_type])==null?void 0:i.icon)||"⚡";return`<div style="width:170px;height:240px;background:${o};border-radius:14px;border:3px solid ${n};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${n}88;flex-shrink:0">
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;height:${g}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ...
      ${c?`<img src="${c}" style="width:${g-4}px;height:${g-4}px;object-fit:contain">`:e!=null&&e.clubName?`<span style="font-size:${Math.max(4,g-8)}px;font-weight:700;color:#333">${(e.clubName||"").slice(0,3).toUpperCase()}</span>`:""}
        ...
        stroke="${de}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const y=48,z=64,r=13,u=16,M={normal:"#aaaaaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#111111",legende:"#7a28b8",légende:"#7a28b8"};for(const[V,C]of Object.entries(o)){const F=g(V);if(!F||!C)continue;const j=V.replace(/[0-9]/g,""),te=It[j]||"#555",le=a.includes(C.cardId),de=i==="attack"&&(["MIL","ATT"].includes(j)||le)&&!C.used||i==="defense"&&["GK","DEF","MIL"].includes(j)&&!C.used,P=s.includes(C.cardId);let Y;i==="attack"?Y=le?Math.max(1,Number(C.note_a)||0):j==="MIL"?Number(C.note_m)||0:Number(C.note_a)||0:i==="defense"?Y=j==="GK"?Number(C.note_g)||0:j==="MIL"?Number(C.note_m)||0:Number(C.note_d)||0:Y=Number(j==="GK"?C.note_g:j==="DEF"?C.note_d:j==="MIL"?C.note_m:C.note_a)||0,Y=Y+(C.boost||0),C.stadiumBonus&&(i==="attack"&&(j==="ATT"||j==="MIL")||i==="defense"&&(j==="GK"||j==="DEF"||j==="MIL")?Y+=10:i||(Y+=10));const H=(F.x-y/2).toFixed(1),G=(F.y-z/2).toFixed(1),ne=M[C==null?void 0:C.rarity]||M.normal;if(C.used){const Ae=typeof import.meta<"u"&&"/manager-wars/"||"/",xe=`${typeof window<"u"&&((N=window.location)==null?void 0:N.origin)||""}${Ae}icons/carte-dos.png`.replace(/([^:])\/\//g,"$1/");v+=`<rect x="${H}" y="${G}" width="${y}" height="${z}" rx="5" fill="#161616"/>`,v+=`<image href="${xe}" xlink:href="${xe}" x="${H}" y="${G}" width="${y}" height="${z}" preserveAspectRatio="xMidYMid slice"/>`,v+=`<rect x="${H}" y="${G}" width="${y}" height="${z}" rx="5" fill="none" stroke="${ne}" stroke-width="2" opacity="0.7"/>`,v+=`<rect x="${H}" y="${G}" width="${y}" height="${z}" rx="5" fill="rgba(0,0,0,0.01)" class="match-used-hit" data-card-id="${C.cardId}" data-role="${j}" style="cursor:pointer"/>`;continue}const fe=P?.45:1,ye=P?"#FFD700":ne,Te=P?3:(C==null?void 0:C.rarity)==="legende"||(C==null?void 0:C.rarity)==="légende"||(C==null?void 0:C.rarity)==="pepite"||(C==null?void 0:C.rarity)==="pépite"?2.5:2,Ee=z-r-u;v+=`<defs><clipPath id="cp-${V}"><rect x="${H}" y="${(F.y-z/2+r).toFixed(1)}" width="${y}" height="${Ee}"/></clipPath></defs>`,v+=`<rect x="${H}" y="${G}" width="${y}" height="${z}" rx="5" fill="${te}" opacity="${fe}"/>`;const be=Pe(C);be&&(v+=`<image href="${be}" xlink:href="${be}" x="${H}" y="${(F.y-z/2+r).toFixed(1)}" width="${y}" height="${Ee}" clip-path="url(#cp-${V})" preserveAspectRatio="xMidYMin slice"/>`),v+=`<rect x="${H}" y="${G}" width="${y}" height="${r}" rx="4" fill="rgba(255,255,255,0.92)"/>`,v+=`<text x="${F.x.toFixed(1)}" y="${(F.y-z/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(C.name||"").slice(0,9)}</text>`;const _e=(F.y+z/2-u).toFixed(1);v+=`<rect x="${H}" y="${_e}" width="${y}" height="${u}" fill="rgba(255,255,255,0.92)"/>`;{const Ae=Zt(C.country_code);Ae?v+=`<image href="${Ae}" xlink:href="${Ae}" x="${(F.x-20).toFixed(1)}" y="${(F.y+z/2-u+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:v+=`<text x="${(F.x-13).toFixed(1)}" y="${(F.y+z/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${vo(C.country_code)}</text>`;const ze=C.stadiumBonus?"#E87722":"#111";v+=`<text x="${F.x.toFixed(1)}" y="${(F.y+z/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="${ze}" font-family="Arial Black">${Y}</text>`;const xe=ho(C);xe?v+=`<image href="${xe}" xlink:href="${xe}" x="${(F.x+y/2-14).toFixed(1)}" y="${(F.y+z/2-u+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:C.clubName&&(v+=`<text x="${(F.x+14).toFixed(1)}" y="${(F.y+z/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(C.clubName||"").slice(0,3).toUpperCase()}</text>`),C.stadiumBonus&&(v+=`<rect x="${(F.x-y/2).toFixed(1)}" y="${(F.y-z/2).toFixed(1)}" width="${y}" height="${z}" rx="5" fill="none" stroke="#E87722" stroke-width="2" opacity="0.8"/>`)}v+=`<rect x="${H}" y="${G}" width="${y}" height="${z}" rx="5" fill="${P?"rgba(255,255,255,0.12)":"none"}" stroke="${ye}" stroke-width="${Te}" opacity="${fe}"/>`,de&&(v+=`<rect x="${H}" y="${G}" width="${y}" height="${z}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${P?"selected":""}" data-card-id="${C.cardId}" data-role="${j}" style="cursor:pointer"/>`)}const L=38;return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-L} ${-L} ${c+L*2} ${d+L*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${v}
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${r.map((N,V)=>{const C=V<r.length-1?at(N,r[V+1]):null,F=!C||C==="#ff3333"||C==="#cc2222",j=C==="#00ff88"?"+2":C==="#FFD700"?"+1":"";return`
          <div class="duel-card duel-card-${L}" data-idx="${V}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease, transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
            ${Ye({...N,note:Se(N,"MIL"),_line:"MIL"},58,78)}
          </div>
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    ${(()=>{const P=window.innerWidth>=700,Y=(_e,Ae,ze)=>{var je,De;const xe=(t.gcDefs||[]).find(I=>I.name===_e.gc_type),W={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[xe==null?void 0:xe.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",ie={purple:"#b06ce0",light_blue:"#00d4ef"}[xe==null?void 0:xe.color]||"#b06ce0",K=(xe==null?void 0:xe.name)||_e.gc_type,oe=(xe==null?void 0:xe.effect)||((je=Ke[_e.gc_type])==null?void 0:je.desc)||"",re=xe!=null&&xe.image_url?`/manager-wars/icons/${xe.image_url}`:null,Q=((De=Ke[_e.gc_type])==null?void 0:De.icon)||"⚡",ae=Math.round(ze*.22),ke=Math.round(ze*.22),Le=ze-ae-ke,Fe=K.length>12?7:9;return`<div class="gc-mini" data-gc-id="${_e.id}" data-gc-type="${_e.gc_type}"
          style="box-sizing:border-box;width:${Ae}px;height:${ze}px;border-radius:10px;border:2px solid ${ie};background:${W};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
          <div style="height:${ae}px;background:rgba(255,255,255,0.14);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 3px">
            <span style="font-size:${Fe}px;font-weight:900;color:#fff;text-align:center;line-height:1.1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${Ae-6}px">${K}</span>
          ...
          <div style="height:${Le}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
            ${re?`<img src="${re}" style="max-width:${Ae-10}px;max-height:${Le-6}px;object-fit:contain">`:`<span style="font-size:${Math.round(Le*.55)}px">${Q}</span>`}
          </div>
        ...
        </div>`},H=(_e,Ae)=>{var ze;return`<div id="boost-card"
  ...
  `,document.body.appendChild(s);let n=!1;const g=()=>{n||(n=!0,s.remove(),i())};s.addEventListener("click",g),setTimeout(g,2e3)}function Mt(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function di(e,t,i,s=null,c=null){var y,z;if(t.phase!=="attack"){Mt("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){Mt(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const d=Object.entries(t.homeTeam).flatMap(([r,u])=>(u||[]).filter(M=>M.used).map(M=>({...M,_line:M._line||r}))),a=t.homeSubs.filter(r=>!t.usedSubIds.includes(r.cardId));if(!d.length){Mt("Aucun joueur utilisé à remplacer");return}if(!a.length){Mt("Aucun remplaçant disponible");return}let l=Math.max(0,d.findIndex(r=>r.cardId===c));const p=((y=d[l])==null?void 0:y._line)||((z=d[l])==null?void 0:z.job);let o=s?Math.max(0,a.findIndex(r=>r.cardId===s)):Math.max(0,a.findIndex(r=>r.job===p)),n=!1;const g=document.createElement("div");g.id="sub-overlay",g.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function v(){var C,F,j,te,le,de;const r=d[l],u=a[o],M=Math.min(130,Math.round((window.innerWidth-90)/2)),L=Math.round(M*1.35),N=P=>`background:rgba(255,255,255,0.12);border:none;color:${P?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${P?"default":"pointer"};flex-shrink:0`;g.innerHTML=`
    ...
    </div>`,(C=g.querySelector("#sub-close"))==null||C.addEventListener("click",()=>g.remove()),(F=g.querySelector("#out-up"))==null||F.addEventListener("click",()=>{l>0&&(l--,v())}),(j=g.querySelector("#out-down"))==null||j.addEventListener("click",()=>{l<d.length-1&&(l++,v())}),(te=g.querySelector("#in-up"))==null||te.addEventListener("click",()=>{o>0&&(o--,v())}),(le=g.querySelector("#in-down"))==null||le.addEventListener("click",()=>{o<a.length-1&&(o++,v())});const V=(P,Y,H,G)=>{const ne=g.querySelector("#"+P);if(!ne)return;let fe=0;ne.addEventListener("touchstart",ye=>{fe=ye.touches[0].clientY},{passive:!0}),ne.addEventListener("touchend",ye=>{const Te=ye.changedTouches[0].clientY-fe;if(Math.abs(Te)<30)return;const Ee=Y();Te<0&&Ee<G-1?(H(Ee+1),v()):Te>0&&Ee>0&&(H(Ee-1),v())},{passive:!0})};V("in-panel",()=>o,P=>o=P,a.length),V("out-panel",()=>l,P=>l=P,d.length),(de=g.querySelector("#sub-confirm"))==null||de.addEventListener("click",P=>{if(P.preventDefault(),P.stopPropagation(),n)return;n=!0;const Y=d[l],H=a[o];if(!Y||!H)return;let G=null,ne=-1;for(const[ye,Te]of Object.entries(t.homeTeam)){const Ee=(Te||[]).findIndex(be=>be.cardId===Y.cardId);if(Ee!==-1){G=ye,ne=Ee;break}}if(ne===-1||!G){Mt("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),g.remove();return}const fe={...H,_line:G,_col:Y._col||0,used:!1,boost:0};t.homeTeam[G].splice(ne,1,fe),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(H.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:Y.name,firstname:Y.firstname,note:Se(Y,G),portrait:Pe(Y),job:Y.job,country_code:Y.country_code,rarity:Y.rarity,clubName:Y.clubName,clubLogo:Y.clubLogo},inPlayer:{name:H.name,firstname:H.firstname,note:Se(H,G),portrait:Pe(H),job:H.job,country_code:H.country_code,rarity:H.rarity,clubName:H.clubName,clubLogo:H.clubLogo},text:`🔄 ${H.firstname} ${H.name} remplace ${Y.firstname} ${Y.name}`}),g.remove(),Dn(Y,H,()=>Xe(e,t,i))})}document.body.appendChild(g),v()}function Gn(e,t,i,s,c){var z,r;const d=(s.gcDefs||[]).find(u=>u.name===t),a=Ke[t]||{icon:"⚡",desc:"Carte spéciale."},l={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[d==null?void 0:d.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",p={purple:"#b06ce0",light_blue:"#00d4ef"}[d==null?void 0:d.color]||"#b06ce0",o=(d==null?void 0:d.name)||t,n=(d==null?void 0:d.effect)||a.desc,g=d!=null&&d.image_url?`/manager-wars/icons/${d.image_url}`:null,v=a.icon||"⚡",y=document.createElement("div");y.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",y.innerHTML=`
    ...
    <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
      ${e.map(g=>{const v=g._line||g.job||"MIL",y={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[v]||"#555",z=Se(g,v)+(g.boost||0),r=l.find(u=>u.cardId===g.cardId);return`<div class="gc-pick-item" data-cid="${g.cardId}"
    ...
    <div style="display:flex;flex-direction:column;gap:6px">
      ${s.map(c=>`
        <div class="player-boost-opt" data-card-id="${c.cardId}"
          ...
          <div style="width:32px;height:32px;background:${It[c.job]||"#888"};border-radius:6px;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:900;font-size:13px">${Se(c,c._line||c.job)}</div>
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(d);let l=!1;const p=()=>{l||(l=!0,d.remove(),setTimeout(()=>c(),50))};d.addEventListener("click",p),setTimeout(p,3500)}async function qt(e,t,i){var n,g;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:s}=i,c=t.homeScore>t.aiScore,d=t.homeScore===t.aiScore,a=c?"victoire":d?"nul":"defaite",l=Ko(t.mode,a);t.matchId&&await E.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:c?s.profile.id:null,home_credits_reward:l,played_at:new Date().toISOString()}).eq("id",t.matchId);const p={credits:(s.profile.credits||0)+l,matches_played:(s.profile.matches_played||0)+1};c?p.wins=(s.profile.wins||0)+1:d?p.draws=(s.profile.draws||0)+1:p.losses=(s.profile.losses||0)+1,await E.from("users").update(p).eq("id",s.profile.id),await i.refreshProfile();const o=document.createElement("div");o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",o.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const z=async(V=!0)=>{v=!0,y&&(E.removeChannel(y),y=null),V&&await E.rpc("cancel_matchmaking",{p_user_id:o.profile.id}).catch(()=>{})};(N=document.getElementById("mm-cancel"))==null||N.addEventListener("click",async()=>{try{await z(!0)}catch{}We(e),n("home")});const r=async(V,C,F)=>{if(v)return;v=!0,y&&(E.removeChannel(y),y=null);const j=document.getElementById("mm-status");j&&(j.textContent="Adversaire trouvé !"),await new Promise(te=>setTimeout(te,600)),e.isConnected&&$o(e,t,V,F,a,l)},{data:u,error:M}=await E.rpc("try_matchmake",{p_user_id:o.profile.id,p_deck_id:i});if(M||!(u!=null&&u.success)){g("Erreur de matchmaking","error"),We(e),n("home");return}if(u.matched){r(u.match_id,u.opponent_id,!1);return}const L=document.getElementById("mm-status");L&&(L.textContent="En attente d'un autre joueur..."),y=E.channel("matchmaking-"+o.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${o.profile.id}`},V=>{const C=V.new;C.status==="matched"&&C.match_id&&r(C.match_id,C.matched_with,!0)}).subscribe()}async function Un(e,t,i){const{state:s,navigate:c,toast:d}=t;try{(E.getChannels?E.getChannels():[]).forEach(o=>{const n=o.topic||"";(n.includes("matchmaking")||n.includes("pvp-match")||n.includes("friend-invite"))&&E.removeChannel(o)})}catch{}const{data:a}=await E.from("matches").select("home_id, away_id, status").eq("id",i).single();if(!a){d("Match introuvable","error"),c("home");return}if(a.status==="finished"){d("Ce match est terminé","info"),c("home");return}const l=a.home_id===s.profile.id;$o(e,t,i,l,[],[])}async function $o(e,t,i,s,c=[],d=[]){const{state:a,navigate:l,toast:p}=t,o=s?"p1":"p2",n=s?"p2":"p1",g=(c||[]).map(I=>I.id),v=(c||[]).map(I=>({id:I.id,gc_type:I.gc_type,_gcDef:I._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:y}=await E.from("matches").select("*").eq("id",i).single();if(!y){p("Match introuvable","error"),l("home");return}async function z(){const[{data:I},{data:q},{data:w},{data:k}]=await Promise.all([E.rpc("get_deck_for_match",{p_deck_id:y.home_deck_id}),E.rpc("get_deck_for_match",{p_deck_id:y.away_deck_id}),E.from("users").select("id,pseudo,club_name").eq("id",y.home_id).single(),E.from("users").select("id,pseudo,club_name").eq("id",y.away_id).single()]),f=A=>({cardId:A.card_id,position:A.position,id:A.id,firstname:A.firstname,name:A.surname_encoded,country_code:A.country_code,club_id:A.club_id,job:A.job,job2:A.job2,note_g:Number(A.note_g)||0,note_d:Number(A.note_d)||0,note_m:Number(A.note_m)||0,note_a:Number(A.note_a)||0,rarity:A.rarity,skin:A.skin,hair:A.hair,hair_length:A.hair_length,clubName:A.club_encoded_name||null,clubLogo:A.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}),m=((I==null?void 0:I.starters)||[]).map(A=>f(A)),x=((q==null?void 0:q.starters)||[]).map(A=>f(A)),b=(I==null?void 0:I.formation)||"4-4-2",h=(q==null?void 0:q.formation)||"4-4-2";let _=yt(m,b),T=yt(x,h);const $=((I==null?void 0:I.subs)||[]).map(A=>f(A));return stadiumDef&&s&&(_=Ht(_,stadiumDef),ii($,stadiumDef)),{p1Team:_,p2Team:T,p1Subs:$,p2Subs:((q==null?void 0:q.subs)||[]).map(A=>f(A)),p1Formation:b,p2Formation:h,p1Name:(w==null?void 0:w.club_name)||(w==null?void 0:w.pseudo)||"Joueur 1",p2Name:(k==null?void 0:k.club_name)||(k==null?void 0:k.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:s?g:[],gc_p2:s?[]:g,gcCardsFull_p1:s?v:[],gcCardsFull_p2:s?[]:v,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:d||[],lastActionAt:new Date().toISOString()}}let r=y.game_state&&Object.keys(y.game_state).length?y.game_state:null;if(!r)if(s){r=await z();const{data:I}=await E.from("matches").select("game_state").eq("id",i).single();!(I!=null&&I.game_state)||!Object.keys(I.game_state).length?await E.from("matches").update({game_state:r,turn_user_id:y.home_id}).eq("id",i):r=I.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let I=0;I<30&&!r;I++){await new Promise(w=>setTimeout(w,400));const{data:q}=await E.from("matches").select("game_state").eq("id",i).single();q!=null&&q.game_state&&Object.keys(q.game_state).length&&(r=q.game_state)}if(!r){p("Erreur de synchronisation","error"),l("home");return}r.gc_p2=g,r.gcCardsFull_p2=v,await E.from("matches").update({game_state:r}).eq("id",i)}let u=!1,M=null,L=!1;const N=new Set,V=new Set;function C(I){var _,T;try{E.removeChannel(F)}catch{}const q=r[o+"Score"]||0,w=r[n+"Score"]||0;let k,f;I.winner_id?(k=I.winner_id===a.profile.id,f=!1):I.forfeit?(k=q>w,f=!1):(f=q===w,k=q>w),(_=document.getElementById("pvp-end-overlay"))==null||_.remove();const m=document.createElement("div");m.id="pvp-end-overlay",m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const x=f?"🤝":k?"🏆":"😞",b=f?"MATCH NUL":k?"VICTOIRE !":"DÉFAITE",h=f?"#fff":k?"#FFD700":"#ff6b6b";m.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(m),(T=m.querySelector("#pvp-end-home"))==null||T.addEventListener("click",()=>{m.remove(),We(e),l("home")})}const F=E.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},I=>{const q=I.new;try{if(q.status==="finished"||q.forfeit){if(u)return;u=!0,M&&(clearInterval(M),M=null),q.game_state&&(r=q.game_state),C(q);return}if(q.game_state){const w=r;r=q.game_state;const k=r._lastGC;if(k&&k.seq&&!V.has(k.seq)&&(V.add(k.seq),k.by!==o)){Ee(k.type,k.by,()=>G());return}const f=w[o+"Score"]||0,m=w[n+"Score"]||0,x=r[o+"Score"]||0,b=r[n+"Score"]||0,h=x>f,_=b>m;if((h||_)&&!N.has(r.round)){N.add(r.round);const T=[...r.log||[]].reverse().find(A=>A.isGoal),$=((T==null?void 0:T.homePlayers)||[]).map(A=>({name:A.name,note:A.note,portrait:A.portrait,job:A.job}));Te($,x,b,h,()=>G());return}G()}}catch(w){console.error("[PvP] crash:",w)}}).subscribe();async function j(I){Object.assign(r,I),r.lastActionAt=new Date().toISOString();const{error:q}=await E.from("matches").update({game_state:r}).eq("id",i);q&&p("Erreur de synchronisation","error");try{G()}catch(w){console.error("[PvP] renderPvpScreen crash:",w)}}async function te(){if(u)return;u=!0,M&&(clearInterval(M),M=null);const I=s?y.away_id:y.home_id,q=s?"p2":"p1",w=s?"p1":"p2",k={...r,[q+"Score"]:3,[w+"Score"]:0,phase:"finished"};try{await E.from("matches").update({status:"finished",forfeit:!0,winner_id:I,home_score:k.p1Score||0,away_score:k.p2Score||0,game_state:k}).eq("id",i)}catch{}try{E.removeChannel(F)}catch{}setTimeout(()=>{We(e),l("home")},800)}const le={BOOST_STAT:({value:I=1,count:q=1,roles:w=[]},k,f)=>{const m=k[o+"Team"],x=Object.entries(m).filter(([b])=>!w.length||w.includes(b)).flatMap(([b,h])=>h.filter(_=>!_.used).map(_=>({..._,_line:b})));if(!x.length){k.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),f(k);return}de(x,q,`Choisir ${q} joueur(s) à booster (+${I})`,b=>{b.forEach(h=>{const _=(m[h._line]||[]).find(T=>T.cardId===h.cardId);_&&(_.boost=(_.boost||0)+I,k.log.push({text:`⚡ +${I} sur ${_.name}`,type:"info"}))}),k[o+"Team"]=m,f(k)})},DEBUFF_STAT:({value:I=1,count:q=1,roles:w=[],target:k="ai"},f,m)=>{const x=k==="home"?o:n,b=f[x+"Team"],h=k==="home"?"allié":"adverse",_=Object.entries(b).filter(([T])=>!w.length||w.includes(T)).flatMap(([T,$])=>$.map(A=>({...A,_line:T})));if(!_.length){f.log.push({text:`🎯 Aucun joueur ${h}`,type:"info"}),m(f);return}de(_,q,`Choisir ${q} joueur(s) ${h}(s) (-${I})`,T=>{T.forEach($=>{const A=(b[$._line]||[]).find(S=>S.cardId===$.cardId);A&&(A.boost=(A.boost||0)-I,f.log.push({text:`🎯 -${I} sur ${A.name}`,type:"info"}))}),f[x+"Team"]=b,m(f)})},GRAY_PLAYER:({count:I=1,roles:q=[],target:w="ai"},k,f)=>{const m=w==="home"?o:n,x=k[m+"Team"],b=w==="home"?"allié":"adverse",h=Object.entries(x).filter(([_])=>!q.length||q.includes(_)).flatMap(([_,T])=>T.filter($=>!$.used).map($=>({...$,_line:_})));if(!h.length){k.log.push({text:`❌ Aucun joueur ${b}`,type:"info"}),f(k);return}de(h,I,`Choisir ${I} joueur(s) ${b}(s) à exclure`,_=>{const T="usedCardIds_"+m,$=new Set(k[T]||[]);_.forEach(A=>{const S=(x[A._line]||[]).find(B=>B.cardId===A.cardId);S&&(S.used=!0,$.add(A.cardId),k.log.push({text:`❌ ${S.name} exclu !`,type:"info"}))}),k[T]=[...$],k[m+"Team"]=x,f(k)})},REVIVE_PLAYER:({count:I=1,roles:q=[]},w,k)=>{const f=w[o+"Team"],m=Object.entries(f).filter(([x])=>!q.length||q.includes(x)).flatMap(([x,b])=>b.filter(h=>h.used).map(h=>({...h,_line:x})));if(!m.length){w.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),k(w);return}de(m,I,`Choisir ${I} joueur(s) à ressusciter`,x=>{x.forEach(b=>{const h=(f[b._line]||[]).find(_=>_.cardId===b.cardId);h&&(h.used=!1,w.log.push({text:`💫 ${h.name} ressuscité !`,type:"info"}))}),w[o+"Team"]=f,k(w)})},REMOVE_GOAL:({},I,q)=>{const w=n+"Score";I[w]>0?(I[w]--,I.log.push({text:"🚫 Dernier but annulé !",type:"info"})):I.log.push({text:"🚫 Aucun but à annuler",type:"info"}),q(I)},ADD_GOAL_DRAW:({},I,q)=>{I[o+"Score"]===I[n+"Score"]?(I[o+"Score"]++,I.log.push({text:"🎯 But bonus !",type:"info"})):I.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),q(I)},ADD_SUB:({value:I=1},q,w)=>{q.maxSubs=(q.maxSubs||3)+I,q.log.push({text:`🔄 +${I} remplacement(s)`,type:"info"}),w(q)},CUSTOM:({},I,q)=>q(I)};function de(I,q,w,k){const f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let m=[];function x(){var h,_;const b=I.map(T=>{const $={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[T._line]||"#555",A=Se(T,T._line)+(T.boost||0),B=m.find(R=>R.cardId===T.cardId)?"#FFD700":"rgba(255,255,255,0.25)",D=T.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${T.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${B};background:${$};overflow:hidden;cursor:pointer;${D}">
        ...
        <div style="height:${rt}px;background:rgba(255,255,255,0.14);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 3px">
          <span style="font-size:${$t}px;font-weight:900;color:#fff;text-align:center;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${we-6}px">${Ce}</span>
          <span style="font-size:6px;color:rgba(255,255,255,0.45)">⚡ GC</span>
        </div>
        <div style="height:${ct}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
          ${tt?`<img src="${tt}" style="max-width:${we-10}px;max-height:${ct-6}px;object-fit:contain">`:`<span style="font-size:${Math.round(ct*.55)}px">${it}</span>`}
        </div>
      ...
      </div>`}function ge(ce,we){return`<div id="pvp-boost-card"
        style="box-sizing:border-box;width:${ce}px;height:${we}px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:2px solid #87CEEB;border-radius:10px;cursor:pointer;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:${Math.round(we*.04)}px;text-align:center;flex-shrink:0">
        <div style="font-size:${Math.round(we*.2)}px">⚡</div>
        <div style="font-size:${Math.round(we*.09)}px;color:#000;font-weight:900">+${r.boostValue}</div>
      </div>`}const me=(ce,we)=>we?ge(130,175):se(ce,130,175),he=(ce,we)=>we?ge(68,95):se(ce,68,95),ve=T?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",Me=x?ke(o)?`<button id="pvp-action" style="${ve};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${h.length===0?"disabled":""}>⚔️ ATTAQUEZ <span id="pvp-timer"></span></button>`:`<button id="pvp-action" data-pass="1" style="${ve};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER <span id="pvp-timer"></span></button>`:b?`<button id="pvp-action" style="${ve};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${h.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="pvp-timer"></span></button>`:`<div style="font-size:11px;color:rgba(255,255,255,0.3);text-align:center;padding:4px">⏳ Tour de ${m}</div>`,Ie=x&&!ke(o)?'<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">Aucun attaquant — passez la main</div>':x||b?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${h.length}/3 sélectionné(s)</div>`:"",Re=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${T?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
    ...
    </div>`,qe=x?"attack":b?"defense":"idle",Oe=`<div style="overflow:hidden;min-width:0;flex:1;min-height:0;display:flex;flex-direction:column" id="match-field">
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${ft(I,r[o+"Formation"],qe,_,300,300,ee)}
      </div>
    </div>`;function Be(ce){if(ce.type==="duel"&&(ce.homeTotal!=null||ce.aiTotal!=null)){const we=(ce.homeTotal||0)>=(ce.aiTotal||0);return`<div style="background:rgba(255,255,255,0.05);border-radius:8px;padding:5px 6px;border:1px solid rgba(255,255,255,0.08)">
          ...
          <div style="display:flex;align-items:center;gap:3px;max-height:46px;overflow:hidden">
            <div style="flex:1;display:flex;gap:2px;justify-content:flex-end;flex-wrap:nowrap;overflow-x:auto;overflow-y:hidden">
              ${(ce.homePlayers||[]).map(Z=>dt(Z)).join("")}
            </div>
            <div style="text-align:center;padding:0 6px;flex-shrink:0">
              <div style="font-size:${we?20:14}px;font-weight:900;color:${we?"#FFD700":"rgba(255,255,255,0.4)"};line-height:1">${ce.homeTotal}</div>
              <div style="font-size:8px;color:rgba(255,255,255,0.3);margin:1px 0">VS</div>
              <div style="font-size:${we?14:20}px;font-weight:900;color:${we?"rgba(255,255,255,0.4)":"#ff6b6b"};line-height:1">${ce.aiTotal}</div>
            </div>
            <div style="flex:1;display:flex;gap:2px;justify-content:flex-start;flex-wrap:nowrap;overflow-x:auto;overflow-y:hidden">
              ${(ce.aiPlayers||[]).map(Z=>dt(Z)).join("")}
            </div>
          </div>
          ${ce.isGoal?`<div style="text-align:center;font-size:11px;color:#FFD700;font-weight:900;margin-top:3px">${ce.homeScored,"⚽ BUT !"}</div>`:""}
        </div>`}return`<div style="font-size:11px;color:${ce.type==="goal"?"#FFD700":"rgba(255,255,255,0.65)"};font-weight:${ce.type==="goal"?700:400};padding:3px 2px">${ce.text||""}</div>`}const Ve=(()=>{var we,Z;if(b&&((we=r.pendingAttack)!=null&&we.players)){const ue=r.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
          ...
          ${ot((ue.players||[]).map(X=>({...X,used:!1})),"#ff6b6b",ue.total)}
        </div>`}if(x&&((Z=r.pendingAttack)!=null&&Z.players)){const ue=r.pendingAttack;return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #00ff88">
          <div style="font-size:8px;color:#00ff88;letter-spacing:2px;margin-bottom:4px;text-transform:uppercase">⚔️ VOUS ATTAQUEZ</div>
          ${ot((ue.players||[]).map(X=>({...X,used:!1})),"#00ff88",ue.total)}
        </div>`}const ce=(r.log||[]).slice(-1)[0];return ce?'<div style="padding:2px 4px">'+Be(ce)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})(),Ne=`
      <div style="display:flex;align-items:center;padding:8px 10px;background:rgba(0,0,0,0.5);gap:6px;flex-shrink:0">
      ...
      </div>`;function He(){const ce=e.querySelector(".match-screen");if(!ce)return;const we=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);ce.style.height=we+"px",ce.style.minHeight=we+"px",ce.style.maxHeight=we+"px",ce.style.overflow="hidden";const Z=e.querySelector("#mobile-action-bar"),ue=e.querySelector("#mobile-play-area");Z&&ue&&(ue.style.paddingBottom=Z.offsetHeight+"px")}if(He(),setTimeout(He,120),setTimeout(He,400),L||(L=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",He),window.visualViewport.addEventListener("scroll",He)),window.addEventListener("resize",He)),function(){const we=e.querySelector(".terrain-wrapper svg");we&&(we.removeAttribute("width"),we.removeAttribute("height"),we.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",we.setAttribute("viewBox","-26 -26 352 352"),we.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),e.querySelectorAll(".match-slot-hit").forEach(ce=>{ce.addEventListener("click",()=>{if(!x&&!b)return;const we=ce.dataset.cardId,Z=ce.dataset.role,ue=(I[Z]||[]).find($e=>$e.cardId===we);if(!ue||ue.used)return;const X=ee.includes(we);if(x&&!["MIL","ATT"].includes(Z)&&!X)return;Array.isArray(r["selected_"+o])||(r["selected_"+o]=[]);const pe=r["selected_"+o],Ce=pe.findIndex($e=>$e.cardId===we);Ce>-1?pe.splice(Ce,1):pe.length<3&&pe.push({...ue,_role:Z}),G()})}),e.querySelectorAll(".match-used-hit").forEach(ce=>{ce.addEventListener("click",()=>Ae(ce.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(ce=>{ce.addEventListener("click",()=>Ae())}),(Ue=e.querySelector("#pvp-sub-open"))==null||Ue.addEventListener("click",()=>Ae()),e.querySelectorAll(".pvp-gc-mini").forEach(ce=>{ce.addEventListener("click",()=>be(ce.dataset.gcId,ce.dataset.gcType))}),(Je=e.querySelector("#pvp-boost-card"))==null||Je.addEventListener("click",()=>_e()),(Ze=e.querySelector("#pvp-action"))==null||Ze.addEventListener("click",ce=>{x?ce.currentTarget.dataset.pass==="1"||!ke(o)?ie():K():b&&oe()}),(Qe=e.querySelector("#pvp-quit"))==null||Qe.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&te()}),(nt=e.querySelector("#pvp-view-opp"))==null||nt.addEventListener("click",()=>xe()),(gt=e.querySelector("#pvp-toggle-history"))==null||gt.addEventListener("click",()=>W()),M&&(clearInterval(M),M=null),(x||b)&&!u){let ce=30,we=!1;const Z=()=>document.getElementById("pvp-timer"),ue=()=>{Z()&&(Z().textContent=ce+"s",Z().style.color=we?"#ff4444":"#fff")};ue(),M=setInterval(()=>{ce--,ce<0?we?(clearInterval(M),M=null,x&&!ke(o)?ie():te()):(we=!0,ce=15,ue()):ue()},1e3)}}function ne(){et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
      ...
      <div style="width:min(90vw,420px)">${ut(r[n+"Team"],r[n+"Formation"],null,[],300,300)}</div>
        ...
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${R.map((U,ee)=>{const se=ee<R.length-1?at(U,R[ee+1]):null,ge=!se||se==="#ff3333"||se==="#cc2222",me=se==="#00ff88"?"+2":se==="#FFD700"?"+1":"";return`<div class="duel-card duel-card-${J}" data-idx="${ee}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease,transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
              ${Ye({...U,note:Se(U,"MIL"),_line:"MIL"},58,78)}
            </div>
            ${ee<R.length-1?`<div class="duel-link duel-link-${J}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${ge?"rgba(255,255,255,0.12)":se};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${ge?"none":`0 0 8px ${se}`}">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    </div>`;const h=(R,O)=>e.querySelectorAll(R).forEach((J,U)=>{setTimeout(()=>{J.style.opacity="1",J.style.transform="translateY(0) scale(1)"},O+U*90)});h(".duel-card-me",150),h(".duel-card-opp",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((R,O)=>setTimeout(()=>{R.style.opacity="1"},O*70)),900),setTimeout(()=>{const R=e.querySelector("#pvp-vs");R&&(R.style.opacity="1",R.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(O=>O.style.opacity="1")},1250);function _(R,O,J){const U=document.getElementById(R);if(!U)return;const ee=performance.now(),se=ge=>{const me=Math.min(1,(ge-ee)/J);U.textContent=Math.round(O*(1-Math.pow(1-me,3))),me<1?requestAnimationFrame(se):U.textContent=O};requestAnimationFrame(se)}setTimeout(()=>{_("pvp-score-me",f,800),_("pvp-score-opp",m,800)},1500);const T=r.p1Team.MIL||[],$=r.p2Team.MIL||[],A=w(T)+k(T),S=w($)+k($),B=A>=S?"p1":"p2";let D=r.boostValue;D==null&&(D=oi(),r.boostValue=D,r.boostOwner=B,r.boostUsed=!1),fe=!0,setTimeout(()=>{const R=e.querySelector("#duel-row-"+(x?"me":"opp")),O=e.querySelector("#duel-row-"+(x?"opp":"me")),J=document.getElementById("pvp-score-me"),U=document.getElementById("pvp-score-opp"),ee=x?J:U,se=x?U:J;ee&&(ee.style.fontSize="80px",ee.style.color=x?"#FFD700":"#ff6b6b",ee.style.animation="duelPulse .5s ease"+(x?",duelGlow 1.5s ease infinite .5s":"")),se&&(se.style.opacity="0.25"),setTimeout(()=>{R&&(R.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",R.style.zIndex="5"),setTimeout(()=>{const ge=document.getElementById("duel-shock");ge&&(ge.style.animation="shockwave .5s ease-out forwards"),O&&(O.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var ve;const ge=document.getElementById("pvp-duel-finale");if(!ge)return;const me=r.boostOwner===o?'<div style="background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:3px solid #cdeffd;border-radius:18px;padding:20px 34px;text-align:center;animation:boostFlipIn .7s cubic-bezier(.34,1.56,.64,1) both;box-shadow:0 10px 36px rgba(135,206,235,.5)"><div style="font-size:10px;color:rgba(0,0,0,.6);letter-spacing:2px;text-transform:uppercase;margin-bottom:6px;font-weight:700">Carte Boost obtenue</div><div style="font-size:46px;line-height:1">⚡</div><div style="font-size:50px;font-weight:900;color:#063;line-height:1.1">+'+D+`</div><div style="font-size:10px;color:rgba(0,0,0,.55);margin-top:4px">Applicable sur n'importe quel joueur</div></div>`:"",he=o==="p1"?'<button id="pvp-start-match" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">▶ Commencer le match</button>':`<div style="font-size:14px;color:rgba(255,255,255,0.5);text-align:center;margin-top:8px;animation:fadeUp .4s ease both">⏳ En attente de l'adversaire...</div>`;ge.innerHTML='<div style="font-size:22px;font-weight:900;color:#fff;text-align:center;animation:fadeUp .4s ease both;text-shadow:0 2px 12px rgba(0,0,0,.5)">'+(x?"⚽ "+r[o+"Name"]+"<br>gagne le milieu et attaque !":"😔 "+r[n+"Name"]+"<br>gagne l'engagement et attaque !")+"</div>"+me+he,ge.style.transition="opacity .45s ease",ge.style.opacity="1",ge.style.pointerEvents="auto",(ve=document.getElementById("pvp-start-match"))==null||ve.addEventListener("click",async()=>{const Me=B;await j({phase:Me+"-attack",attacker:Me,round:1,boostValue:D,boostUsed:!1,boostOwner:Me})})},600)},700)},2800)}function Te(I,q,w,k,f){const m=document.createElement("div");m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:900;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;overflow:hidden;cursor:pointer";const x=Array.from({length:10},(T,$)=>`<div style="position:absolute;font-size:${16+Math.floor(Math.random()*24)}px;top:${5+Math.floor(Math.random()*65)}%;left:${3+Math.floor(Math.random()*94)}%;animation:fw${$%2===0?"A":"B"} ${.7+Math.random()*.7}s ease ${Math.random()*.9}s both;opacity:0">${["✨","🌟","⭐","💥","🎇","🎆","🔥","🌈"][$%8]}</div>`).join(""),b={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};m.innerHTML=`
      <style>
        @keyframes butPop{0%{transform:scale(0) rotate(-8deg);opacity:0}55%{transform:scale(1.25) rotate(2deg)}85%{transform:scale(0.92) rotate(-1deg)}100%{transform:scale(1);opacity:1}}
        @keyframes ballIn{0%{transform:translate(-70px,18px);opacity:0}65%{opacity:1}100%{transform:translate(26px,-8px);opacity:1}}
        @keyframes scoreIn{from{opacity:0;transform:translateY(-12px)}to{opacity:1;transform:translateY(0)}}
      ...
      </div>`,document.body.appendChild($),(B=$.querySelector("#pvp-gc-back"))==null||B.addEventListener("click",()=>$.remove()),(D=$.querySelector("#pvp-gc-use"))==null||D.addEventListener("click",()=>{$.remove(),P(I,q)})}function _e(){var k;const I=r[o+"Team"],q=Object.entries(I).flatMap(([f,m])=>(m||[]).filter(x=>!x.used).map(x=>({...x,_line:f})));if(!q.length)return;const w=document.createElement("div");w.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",w.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
        <div style="flex:1;font-size:14px;font-weight:700;color:#fff">⚡ Choisir un joueur pour +${r.boostValue}</div>
        <button id="bp-close" style="background:none;border:none;color:rgba(255,255,255,0.5);font-size:22px;cursor:pointer">✕</button>
      </div>
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${q.map(f=>{const m={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[f._line]||"#555",x=Se(f,f._line)+(f.boost||0);return`<div class="bp-item" data-cid="${f.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${m};overflow:hidden;cursor:pointer">
      ...
      </div>`,document.body.appendChild(w),(k=w.querySelector("#bp-close"))==null||k.addEventListener("click",()=>w.remove()),w.querySelectorAll(".bp-item").forEach(f=>{f.addEventListener("click",async()=>{const m=f.dataset.cid,x=q.find(h=>h.cardId===m);if(!x)return;const b=(I[x._line]||[]).find(h=>h.cardId===m);b&&(b.boost=(b.boost||0)+r.boostValue),w.remove(),await j({[o+"Team"]:I,boostUsed:!0})})})}function Ae(I=null){var S,B;if(!(r.phase===o+"-attack")){p("Remplacement uniquement avant votre attaque","warning");return}const w=r[o+"Team"],k=r["usedSubIds_"+o]||[],f=r.maxSubs||3;if(k.length>=f){p(`Maximum ${f} remplacements atteint`,"warning");return}const m=Object.entries(w).flatMap(([D,R])=>(R||[]).filter(O=>O.used).map(O=>({...O,_line:D}))),x=(r[o+"Subs"]||[]).filter(D=>!k.includes(D.cardId));if(!m.length){p("Aucun joueur utilisé à remplacer","warning");return}if(!x.length){p("Aucun remplaçant disponible","warning");return}let b=Math.max(0,m.findIndex(D=>D.cardId===I));const h=((S=m[b])==null?void 0:S._line)||((B=m[b])==null?void 0:B.job);let _=Math.max(0,x.findIndex(D=>D.job===h)),T=!1;const $=document.createElement("div");$.id="pvp-sub-overlay",$.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function A(){var se,ge,me,he,ve,Me;const D=m[b],R=x[_],O=Math.min(130,Math.round((window.innerWidth-90)/2)),J=Math.round(O*1.35),U=Ie=>`background:rgba(255,255,255,0.12);border:none;color:${Ie?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Ie?"default":"pointer"};flex-shrink:0`;$.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        <div id="pin-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
          ...
          <div>${R?Ye({...R,used:!1,boost:0},O,J):"<div>—</div>"}</div>
        ...
        <div id="pout-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
          ...
          <div>${D?Ye({...D,used:!1,boost:0},O,J):"<div>—</div>"}</div>
      ...
      </div>`,(se=$.querySelector("#psub-close"))==null||se.addEventListener("click",()=>$.remove()),(ge=$.querySelector("#pout-up"))==null||ge.addEventListener("click",()=>{b>0&&(b--,A())}),(me=$.querySelector("#pout-down"))==null||me.addEventListener("click",()=>{b<m.length-1&&(b++,A())}),(he=$.querySelector("#pin-up"))==null||he.addEventListener("click",()=>{_>0&&(_--,A())}),(ve=$.querySelector("#pin-down"))==null||ve.addEventListener("click",()=>{_<x.length-1&&(_++,A())});const ee=(Ie,Re,qe,Oe)=>{const Be=$.querySelector("#"+Ie);if(!Be)return;let Ve=0;Be.addEventListener("touchstart",Ne=>{Ve=Ne.touches[0].clientY},{passive:!0}),Be.addEventListener("touchend",Ne=>{const He=Ne.changedTouches[0].clientY-Ve;if(Math.abs(He)<30)return;const Ue=Re();He<0&&Ue<Oe-1?(qe(Ue+1),A()):He>0&&Ue>0&&(qe(Ue-1),A())},{passive:!0})};ee("pin-panel",()=>_,Ie=>_=Ie,x.length),ee("pout-panel",()=>b,Ie=>b=Ie,m.length),(Me=$.querySelector("#psub-confirm"))==null||Me.addEventListener("click",async Ie=>{if(Ie.preventDefault(),Ie.stopPropagation(),T)return;T=!0;const Re=m[b],qe=x[_];if(!Re||!qe)return;const Oe=Re._line,Be=(w[Oe]||[]).findIndex(He=>He.cardId===Re.cardId);if(Be===-1){p("Erreur : joueur introuvable","error"),$.remove();return}const Ve={...qe,_line:Oe,position:Re.position,used:!1,boost:0};w[Oe].splice(Be,1,Ve);const Ne=[...k,qe.cardId];$.remove(),ze(Re,qe,async()=>{await j({[o+"Team"]:w,[n+"Team"]:r[n+"Team"],["usedSubIds_"+o]:Ne})})})}document.body.appendChild($),A()}function ze(I,q,w){const k={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const m=(h,_,T)=>`<div style="text-align:center">
      ...
      </div>`,document.body.appendChild(q),(w=q.querySelector("#pvp-hist-close"))==null||w.addEventListener("click",()=>q.remove())}async function ie(){if(r.phase!==o+"-attack")return;const I=o==="p1"?"p2":"p1",q=(r.round||0)+1,w=[...r.log||[]];w.push({type:"info",text:`⏭️ ${r[o+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const k=Le(r),f=ke(I),m=k||!f?"finished":I+"-attack";await j({["selected_"+o]:[],modifiers:{...r.modifiers,[o]:{}},pendingAttack:null,phase:m,attacker:I,round:q,log:w}),m==="finished"&&await je(r)}async function K(){const I=r[o+"Team"],q=!["GK","DEF","MIL","ATT"].some(m=>(r[n+"Team"][m]||[]).some(x=>!x.used)),w=(r["selected_"+o]||[]).map(m=>{const x=(I[m._role]||[]).find(A=>A.cardId===m.cardId)||m,b=q&&["GK","DEF"].includes(m._role),h=I[m._role]||[],_=h.findIndex(A=>A.cardId===m.cardId),T=lt(h.length),$=_>=0?T[_]:x._col??1;return{...x,_line:m._role,_col:$,...b?{note_a:Math.max(1,Number(x.note_a)||0)}:{}}});if(!w.length)return;const k=Nt(w,r.modifiers[o]||{});Y(o,w.map(m=>m.cardId)),w.forEach(m=>{const x=(I[m._role]||[]).find(b=>b.cardId===m.cardId);x&&(x.used=!0)}),r["selected_"+o]=[],G();const f=[...r.log||[]];if(q){const m=(r[o+"Score"]||0)+1,x=w.map($=>({name:$.name,note:Se($,$._line||"ATT"),portrait:Pe($),job:$.job}));f.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:x,homeTotal:k.total,aiTotal:0});const b=(r.round||0)+1,h=o==="p1"?"p2":"p1",_={...r,[o+"Team"]:I,[o+"Score"]:m},T=Le(_);N.add(b),Te(x,m,r[n+"Score"]||0,!0,async()=>{await j({[o+"Team"]:I,[o+"Score"]:m,["selected_"+o]:[],modifiers:{...r.modifiers,[o]:{}},pendingAttack:null,phase:T?"finished":h+"-attack",attacker:h,round:b,log:f}),T&&await je({...r,[o+"Score"]:m})});return}f.push({type:"pending",text:`⚔️ ${r[o+"Name"]} attaque (${k.total})`}),await j({[o+"Team"]:I,[n+"Team"]:r[n+"Team"],pendingAttack:{...k,players:w,side:o},["selected_"+o]:[],modifiers:{...r.modifiers,[o]:{}},phase:n+"-defense",log:f})}async function oe(){const I=r[o+"Team"],q=(r["selected_"+o]||[]).map(D=>{const R=(I[D._role]||[]).find(se=>se.cardId===D.cardId)||D,O=I[D._role]||[],J=O.findIndex(se=>se.cardId===D.cardId),U=lt(O.length),ee=J>=0?U[J]:R._col??1;return{...R,_line:D._role,_col:ee}}),w=Rt(q,r.modifiers[o]||{});Y(o,q.map(D=>D.cardId)),q.forEach(D=>{const R=(I[D._role]||[]).find(O=>O.cardId===D.cardId);R&&(R.used=!0)}),r["selected_"+o]=[],G();const k=Ot(r.pendingAttack.total,w.total,r.modifiers[o]||{}),f=r.pendingAttack.side,m=k==="attack"||(k==null?void 0:k.goal),x=f==="p1"?"p2":"p1",b=(r.round||0)+1,h=(r.pendingAttack.players||[]).map(D=>({name:D.name,note:Se(D,D._line||"ATT"),portrait:Pe(D),job:D.job})),_=[...r.log||[]];_.push({type:"duel",isGoal:m,homeScored:m&&f===o,text:m?`⚽ BUT de ${r[f+"Name"]} ! (${r.pendingAttack.total} vs ${w.total})`:`✋ Attaque stoppée (${r.pendingAttack.total} vs ${w.total})`,homePlayers:h,aiPlayers:q.map(D=>({name:D.name,note:Se(D,D._line||"DEF"),portrait:Pe(D),job:D.job})),homeTotal:r.pendingAttack.total,aiTotal:w.total});const T=m?(r[f+"Score"]||0)+1:r[f+"Score"]||0,$={...r,[o+"Team"]:I,[f+"Score"]:T},A=Le($),S=A?"finished":x+"-attack",B=async()=>{await j({[o+"Team"]:I,[n+"Team"]:r[n+"Team"],[f+"Score"]:T,["selected_"+o]:[],modifiers:{...r.modifiers,[o]:{}},pendingAttack:null,phase:S,attacker:x,round:b,log:_}),(S==="finished"||A)&&await je({...r,[f+"Score"]:T})};if(m){const D=f===o,R=D?T:r[o+"Score"]||0,O=D?r[n+"Score"]||0:T;N.add(b),Te(h,R,O,D,B)}else await B()}function re(I){return["MIL","ATT"].some(q=>(I[q]||[]).some(w=>!w.used))}function Q(I){return["GK","DEF","MIL","ATT"].some(q=>(I[q]||[]).some(w=>!w.used))}function ae(I){return Q(I)&&!re(I)}function ke(I){const q=r[I+"Team"],w=r[(I==="p1"?"p2":"p1")+"Team"];return!!(re(q)||!Q(w)&&ae(q))}function Le(I){const q=["MIL","ATT"].some(_=>(I.p1Team[_]||[]).some(T=>!T.used)),w=["MIL","ATT"].some(_=>(I.p2Team[_]||[]).some(T=>!T.used)),k=Q(I.p1Team),f=Q(I.p2Team);return!q&&!(!f&&k)&&(!w&&!(!k&&f))}function Fe(I){const q=I.p1Score||0,w=I.p2Score||0;return q===w?null:q>w?y.home_id:y.away_id}async function je(I){try{const q=Fe(I),w=q?y.home_id===q?y.away_id:y.home_id:null;await E.from("matches").update({status:"finished",winner_id:q,home_score:I.p1Score||0,away_score:I.p2Score||0}).eq("id",i),q&&w&&updateEvolutiveCards(q,w).catch(()=>{})}catch(q){console.error("[PvP] finishMatch:",q)}}function De(){var f;const I=r[o+"Score"],q=r[n+"Score"],w=I>q,k=I===q;et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(b),(A=b.querySelector("#pvp-end-home"))==null||A.addEventListener("click",()=>{b.remove(),We(e),l("home")})}const te=E.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},w=>{const k=w.new;try{if(k.status==="finished"||k.forfeit){if(u)return;u=!0,N&&(clearInterval(N),N=null),k.game_state&&(r=k.game_state),j(k);return}if(k.game_state){const f=r;r=k.game_state;const m=r._lastGC;if(m&&m.seq&&!F.has(m.seq)&&(F.add(m.seq),m.by!==o)){_e(m.type,m.by,()=>fe());return}const x=f[o+"Score"]||0,b=f[n+"Score"]||0,h=r[o+"Score"]||0,_=r[n+"Score"]||0,T=h>x,$=_>b;if((T||$)&&!C.has(r.round)){C.add(r.round);const A=[...r.log||[]].reverse().find(B=>B.isGoal),S=((A==null?void 0:A.homePlayers)||[]).map(B=>({name:B.name,note:B.note,portrait:B.portrait,job:B.job}));be(S,h,_,T,()=>fe());return}fe()}}catch(f){console.error("[PvP] crash:",f)}}).subscribe();async function le(w){Object.assign(r,w),r.lastActionAt=new Date().toISOString();const{error:k}=await E.from("matches").update({game_state:r}).eq("id",i);k&&p("Erreur de synchronisation","error");try{fe()}catch(f){console.error("[PvP] renderPvpScreen crash:",f)}}async function de(){if(u)return;u=!0,N&&(clearInterval(N),N=null);const w=s?y.away_id:y.home_id,k=s?"p2":"p1",f=s?"p1":"p2",m={...r,[k+"Score"]:3,[f+"Score"]:0,phase:"finished"};try{await E.from("matches").update({status:"finished",forfeit:!0,winner_id:w,home_score:m.p1Score||0,away_score:m.p2Score||0,game_state:m}).eq("id",i)}catch{}try{E.removeChannel(te)}catch{}setTimeout(()=>{We(e),l("home")},800)}const P={BOOST_STAT:({value:w=1,count:k=1,roles:f=[]},m,x)=>{const b=m[o+"Team"],h=Object.entries(b).filter(([_])=>!f.length||f.includes(_)).flatMap(([_,T])=>T.filter($=>!$.used).map($=>({...$,_line:_})));if(!h.length){m.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),x(m);return}Y(h,k,`Choisir ${k} joueur(s) à booster (+${w})`,_=>{_.forEach(T=>{const $=(b[T._line]||[]).find(A=>A.cardId===T.cardId);$&&($.boost=($.boost||0)+w,m.log.push({text:`⚡ +${w} sur ${$.name}`,type:"info"}))}),m[o+"Team"]=b,x(m)})},DEBUFF_STAT:({value:w=1,count:k=1,roles:f=[],target:m="ai"},x,b)=>{const h=m==="home"?o:n,_=x[h+"Team"],T=m==="home"?"allié":"adverse",$=Object.entries(_).filter(([A])=>!f.length||f.includes(A)).flatMap(([A,S])=>S.map(B=>({...B,_line:A})));if(!$.length){x.log.push({text:`🎯 Aucun joueur ${T}`,type:"info"}),b(x);return}Y($,k,`Choisir ${k} joueur(s) ${T}(s) (-${w})`,A=>{A.forEach(S=>{const B=(_[S._line]||[]).find(D=>D.cardId===S.cardId);B&&(B.boost=(B.boost||0)-w,x.log.push({text:`🎯 -${w} sur ${B.name}`,type:"info"}))}),x[h+"Team"]=_,b(x)})},GRAY_PLAYER:({count:w=1,roles:k=[],target:f="ai"},m,x)=>{const b=f==="home"?o:n,h=m[b+"Team"],_=f==="home"?"allié":"adverse",T=Object.entries(h).filter(([$])=>!k.length||k.includes($)).flatMap(([$,A])=>A.filter(S=>!S.used).map(S=>({...S,_line:$})));if(!T.length){m.log.push({text:`❌ Aucun joueur ${_}`,type:"info"}),x(m);return}Y(T,w,`Choisir ${w} joueur(s) ${_}(s) à exclure`,$=>{const A="usedCardIds_"+b,S=new Set(m[A]||[]);$.forEach(B=>{const D=(h[B._line]||[]).find(R=>R.cardId===B.cardId);D&&(D.used=!0,S.add(B.cardId),m.log.push({text:`❌ ${D.name} exclu !`,type:"info"}))}),m[A]=[...S],m[b+"Team"]=h,x(m)})},REVIVE_PLAYER:({count:w=1,roles:k=[]},f,m)=>{const x=f[o+"Team"],b=Object.entries(x).filter(([h])=>!k.length||k.includes(h)).flatMap(([h,_])=>_.filter(T=>T.used).map(T=>({...T,_line:h})));if(!b.length){f.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),m(f);return}Y(b,w,`Choisir ${w} joueur(s) à ressusciter`,h=>{h.forEach(_=>{const T=(x[_._line]||[]).find($=>$.cardId===_.cardId);T&&(T.used=!1,f.log.push({text:`💫 ${T.name} ressuscité !`,type:"info"}))}),f[o+"Team"]=x,m(f)})},REMOVE_GOAL:({},w,k)=>{const f=n+"Score";w[f]>0?(w[f]--,w.log.push({text:"🚫 Dernier but annulé !",type:"info"})):w.log.push({text:"🚫 Aucun but à annuler",type:"info"}),k(w)},ADD_GOAL_DRAW:({},w,k)=>{w[o+"Score"]===w[n+"Score"]?(w[o+"Score"]++,w.log.push({text:"🎯 But bonus !",type:"info"})):w.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),k(w)},ADD_SUB:({value:w=1},k,f)=>{k.maxSubs=(k.maxSubs||3)+w,k.log.push({text:`🔄 +${w} remplacement(s)`,type:"info"}),f(k)},CUSTOM:({},w,k)=>k(w)};function Y(w,k,f,m){const x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let b=[];function h(){var T,$;const _=w.map(A=>{const S={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[A._line]||"#555",B=Se(A,A._line)+(A.boost||0),R=b.find(J=>J.cardId===A.cardId)?"#FFD700":"rgba(255,255,255,0.25)",O=A.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${A.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${R};background:${S};overflow:hidden;cursor:pointer;${O}">
        ...
        </div>`,(T=x.querySelector("#pp-close"))==null||T.addEventListener("click",()=>x.remove()),x.querySelectorAll(".pp-item").forEach(A=>{A.addEventListener("click",()=>{const S=A.dataset.cid,B=w.find(R=>R.cardId===S),D=b.findIndex(R=>R.cardId===S);B&&(D>-1?b.splice(D,1):b.length<k&&b.push(B),h())})}),($=x.querySelector("#pp-confirm"))==null||$.addEventListener("click",()=>{x.remove(),m(b)})}h(),document.body.appendChild(x)}async function H(w,k){const m=(r["gcCardsFull_"+o]||[]).find(_=>_.id===w),x=(m==null?void 0:m._gcDef)||(r.gcDefs||[]).find(_=>{var T;return _.name===k||((T=_.name)==null?void 0:T.toLowerCase().trim())===(k==null?void 0:k.toLowerCase().trim())}),b=[...r["usedGc_"+o]||[],w],h={type:k,by:o,seq:(r._gcAnimSeq||0)+1};F.add(h.seq),_e(k,o,async()=>{if(x!=null&&x.effect_type&&x.effect_type!=="CUSTOM"){const T=P[x.effect_type];if(T){const $={...r};T(x.effect_params||{},$,async A=>{A["usedGc_"+o]=b,A._lastGC=h,A._gcAnimSeq=h.seq,await le(A)});return}}const _={...r};switch(k){case"Remplacement+":_.maxSubs=(_.maxSubs||3)+1,_.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const T=n+"Score";_[T]>0&&(_[T]--,_.log.push({text:"🚫 But annulé",type:"info"}));break}}_["usedGc_"+o]=b,_._lastGC=h,_._gcAnimSeq=h.seq,await le(_)})}function G(w,k){const f="usedCardIds_"+w,m=new Set(r[f]||[]);k.forEach(x=>m.add(x)),r[f]=[...m]}function ne(){for(const w of["p1","p2"]){const k=new Set(r["usedCardIds_"+w]||[]),f=r[w+"Team"];if(f)for(const m of["GK","DEF","MIL","ATT"])(f[m]||[]).forEach(x=>{x.used=k.has(x.cardId)})}}function fe(){var Ze,Qe,nt,gt,ce,we;if(r.phase==="reveal")return ye();if(r.phase==="midfield")return Ee();if(r.phase==="finished")return q();const w=r[o+"Team"],k=r[n+"Team"];ne();const f=r[o+"Score"],m=r[n+"Score"],x=r[o+"Name"],b=r[n+"Name"],h=r.phase===o+"-attack",_=r.phase===o+"-defense",T=Array.isArray(r["selected_"+o])?r["selected_"+o]:[],$=T.map(Z=>Z.cardId),A=window.innerWidth>=700,S=r[o+"Subs"]||[],B=r["usedSubIds_"+o]||[],D=S.filter(Z=>!B.includes(Z.cardId)),R=r["gcCardsFull_"+o]||[],O=r["usedGc_"+o]||[],J=R.filter(Z=>!O.includes(Z.id)),U=r.boostOwner===o&&!r.boostUsed,ee=!["GK","DEF","MIL","ATT"].some(Z=>(k[Z]||[]).some(ue=>!ue.used)),se=[...w.MIL||[],...w.ATT||[]].filter(Z=>!Z.used),ge=h&&ee&&se.length===0?[...w.GK||[],...w.DEF||[]].filter(Z=>!Z.used).map(Z=>Z.cardId):[];function me(Z,ue,X){var kt,Ut;const pe=Z._gcDef,Ce={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[pe==null?void 0:pe.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",$e={purple:"#b06ce0",light_blue:"#00d4ef"}[pe==null?void 0:pe.color]||"#b06ce0",tt=(pe==null?void 0:pe.name)||Z.gc_type,it=(pe==null?void 0:pe.effect)||((kt=Ke[Z.gc_type])==null?void 0:kt.desc)||"",rt=pe!=null&&pe.image_url?`/manager-wars/icons/${pe.image_url}`:null,st=((Ut=Ke[Z.gc_type])==null?void 0:Ut.icon)||"⚡",ct=Math.round(X*.22),$t=Math.round(X*.22),pt=X-ct-$t,ht=tt.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${Z.id}" data-gc-type="${Z.gc_type}"
        style="box-sizing:border-box;width:${ue}px;height:${X}px;border-radius:10px;border:2px solid ${$e};background:${Ce};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
      ...
      </div>`}function he(Z,ue){return`<div id="pvp-boost-card"
        style="box-sizing:border-box;width:${Z}px;height:${ue}px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:2px solid #87CEEB;border-radius:10px;cursor:pointer;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:${Math.round(ue*.04)}px;text-align:center;flex-shrink:0">
      ...
      </div>`}const ve=(Z,ue)=>ue?he(130,175):me(Z,130,175),Me=(Z,ue)=>ue?he(68,95):me(Z,68,95),Ie=A?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",Re=h?Fe(o)?`<button id="pvp-action" style="${Ie};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${T.length===0?"disabled":""}>⚔️ ATTAQUEZ <span id="pvp-timer"></span></button>`:`<button id="pvp-action" data-pass="1" style="${Ie};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER <span id="pvp-timer"></span></button>`:_?`<button id="pvp-action" style="${Ie};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${T.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="pvp-timer"></span></button>`:`<div style="font-size:11px;color:rgba(255,255,255,0.3);text-align:center;padding:4px">⏳ Tour de ${b}</div>`,qe=h&&!Fe(o)?'<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">Aucun attaquant — passez la main</div>':h||_?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${T.length}/3 sélectionné(s)</div>`:"",Oe=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${A?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
      ${D.length===0?'<div style="font-size:7px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':D.map(Z=>`<div class="pvp-sub-btn" data-sub-id="${Z.cardId}" style="cursor:pointer;flex-shrink:0">${Ye(Z,A?76:44,A?100:58)}</div>`).join("")}
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${ft(w,r[o+"Formation"],Be,$,300,300,ge)}
      </div>
    </div>`;function Ne(Z){if(Z.type==="duel"&&(Z.homeTotal!=null||Z.aiTotal!=null)){const ue=(Z.homeTotal||0)>=(Z.aiTotal||0);return`<div style="background:rgba(255,255,255,0.05);border-radius:8px;padding:5px 6px;border:1px solid rgba(255,255,255,0.08)">
          ...
          <div style="display:flex;align-items:center;gap:3px;max-height:46px;overflow:hidden">
            <div style="flex:1;display:flex;gap:2px;justify-content:flex-end;flex-wrap:nowrap;overflow-x:auto;overflow-y:hidden">
              ${(Z.homePlayers||[]).map(X=>dt(X)).join("")}
            </div>
            <div style="text-align:center;padding:0 6px;flex-shrink:0">
              <div style="font-size:${ue?20:14}px;font-weight:900;color:${ue?"#FFD700":"rgba(255,255,255,0.4)"};line-height:1">${Z.homeTotal}</div>
              <div style="font-size:8px;color:rgba(255,255,255,0.3);margin:1px 0">VS</div>
              <div style="font-size:${ue?14:20}px;font-weight:900;color:${ue?"rgba(255,255,255,0.4)":"#ff6b6b"};line-height:1">${Z.aiTotal}</div>
            </div>
            <div style="flex:1;display:flex;gap:2px;justify-content:flex-start;flex-wrap:nowrap;overflow-x:auto;overflow-y:hidden">
              ${(Z.aiPlayers||[]).map(X=>dt(X)).join("")}
            </div>
          </div>
          ${Z.isGoal?`<div style="text-align:center;font-size:11px;color:#FFD700;font-weight:900;margin-top:3px">${Z.homeScored,"⚽ BUT !"}</div>`:""}
        </div>`}return`<div style="font-size:11px;color:${Z.type==="goal"?"#FFD700":"rgba(255,255,255,0.65)"};font-weight:${Z.type==="goal"?700:400};padding:3px 2px">${Z.text||""}</div>`}const He=(()=>{var ue,X;if(_&&((ue=r.pendingAttack)!=null&&ue.players)){const pe=r.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
          ...
          ${ot((pe.players||[]).map(Ce=>({...Ce,used:!1})),"#ff6b6b",pe.total)}
        </div>`}if(h&&((X=r.pendingAttack)!=null&&X.players)){const pe=r.pendingAttack;return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #00ff88">
          <div style="font-size:8px;color:#00ff88;letter-spacing:2px;margin-bottom:4px;text-transform:uppercase">⚔️ VOUS ATTAQUEZ</div>
          ${ot((pe.players||[]).map(Ce=>({...Ce,used:!1})),"#00ff88",pe.total)}
      ...
      </div>`;function Je(){const Z=e.querySelector(".match-screen");if(!Z)return;const ue=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);Z.style.height=ue+"px",Z.style.minHeight=ue+"px",Z.style.maxHeight=ue+"px",Z.style.overflow="hidden";const X=e.querySelector("#mobile-action-bar"),pe=e.querySelector("#mobile-play-area");X&&pe&&(pe.style.paddingBottom=X.offsetHeight+"px")}if(Je(),setTimeout(Je,120),setTimeout(Je,400),V||(V=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",Je),window.visualViewport.addEventListener("scroll",Je)),window.addEventListener("resize",Je)),function(){const ue=e.querySelector(".terrain-wrapper svg");ue&&(ue.removeAttribute("width"),ue.removeAttribute("height"),ue.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",ue.setAttribute("viewBox","-26 -26 352 352"),ue.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),e.querySelectorAll(".match-slot-hit").forEach(Z=>{Z.addEventListener("click",()=>{if(!h&&!_)return;const ue=Z.dataset.cardId,X=Z.dataset.role,pe=(w[X]||[]).find(it=>it.cardId===ue);if(!pe||pe.used)return;const Ce=ge.includes(ue);if(h&&!["MIL","ATT"].includes(X)&&!Ce)return;Array.isArray(r["selected_"+o])||(r["selected_"+o]=[]);const $e=r["selected_"+o],tt=$e.findIndex(it=>it.cardId===ue);tt>-1?$e.splice(tt,1):$e.length<3&&$e.push({...pe,_role:X}),fe()})}),e.querySelectorAll(".match-used-hit").forEach(Z=>{Z.addEventListener("click",()=>xe(Z.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(Z=>{Z.addEventListener("click",()=>xe())}),(Ze=e.querySelector("#pvp-sub-open"))==null||Ze.addEventListener("click",()=>xe()),e.querySelectorAll(".pvp-gc-mini").forEach(Z=>{Z.addEventListener("click",()=>Ae(Z.dataset.gcId,Z.dataset.gcType))}),(Qe=e.querySelector("#pvp-boost-card"))==null||Qe.addEventListener("click",()=>ze()),(nt=e.querySelector("#pvp-action"))==null||nt.addEventListener("click",Z=>{h?Z.currentTarget.dataset.pass==="1"||!Fe(o)?oe():re():_&&Q()}),(gt=e.querySelector("#pvp-quit"))==null||gt.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&de()}),(ce=e.querySelector("#pvp-view-opp"))==null||ce.addEventListener("click",()=>ie()),(we=e.querySelector("#pvp-toggle-history"))==null||we.addEventListener("click",()=>K()),N&&(clearInterval(N),N=null),(h||_)&&!u){let Z=30,ue=!1;const X=()=>document.getElementById("pvp-timer"),pe=()=>{X()&&(X().textContent=Z+"s",X().style.color=ue?"#ff4444":"#fff")};pe(),N=setInterval(()=>{Z--,Z<0?ue?(clearInterval(N),N=null,h&&!Fe(o)?oe():de()):(ue=!0,Z=15,pe()):pe()},1e3)}}function ye(){et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
      <div style="font-size:20px;font-weight:900;color:#ff6b6b">${r[n+"Name"]||"Adversaire"}</div>
      <div style="width:min(90vw,420px)">${ut(r[n+"Team"],r[n+"Formation"],null,[],300,300)}</div>
    </div>`,o==="p1"&&setTimeout(async()=>{await le({phase:"midfield"})},5e3)}let Te=!1;function Ee(){if(Te)return;const w=r[o+"Team"].MIL||[],k=r[n+"Team"].MIL||[];function f(J){return J.reduce((U,ee)=>U+Se(ee,"MIL"),0)}function m(J){let U=0;for(let ee=0;ee<J.length-1;ee++){const se=at(J[ee],J[ee+1]);se==="#00ff88"?U+=2:se==="#FFD700"&&(U+=1)}return U}const x=f(w)+m(w),b=f(k)+m(k),h=x>=b;function _(J,U,ee){return`<div id="duel-row-${ee}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
        <div style="font-size:11px;color:rgba(255,255,255,0.55);letter-spacing:2px;margin-bottom:10px;text-transform:uppercase;font-weight:700">${U}</div>
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${J.map((se,ge)=>{const me=ge<J.length-1?at(se,J[ge+1]):null,he=!me||me==="#ff3333"||me==="#cc2222",ve=me==="#00ff88"?"+2":me==="#FFD700"?"+1":"";return`<div class="duel-card duel-card-${ee}" data-idx="${ge}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease,transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
              ${Ye({...se,note:Se(se,"MIL"),_line:"MIL"},58,78)}
            </div>
            ${ge<J.length-1?`<div class="duel-link duel-link-${ee}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${he?"rgba(255,255,255,0.12)":me};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${he?"none":`0 0 8px ${me}`}">
        ...
        <div class="duel-score-line duel-score-line-${ee}" style="margin-top:10px;font-size:12px;color:rgba(255,255,255,0.55);opacity:0;transition:opacity .4s ease">
          Score: ${f(J)} + ${m(J)} liens = <b style="color:#fff">${f(J)+m(J)}</b>
        </div>
      </div>`}et(e),e.innerHTML=`
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      <style>
      ...
      <div id="duel-shock" style="position:absolute;left:50%;top:50%;width:120px;height:120px;border-radius:50%;border:6px solid #FFD700;opacity:0;pointer-events:none"></div>
      <div id="pvp-duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center,rgba(10,61,30,.4),rgba(10,61,30,.92))"></div>
    </div>`;const T=(J,U)=>e.querySelectorAll(J).forEach((ee,se)=>{setTimeout(()=>{ee.style.opacity="1",ee.style.transform="translateY(0) scale(1)"},U+se*90)});T(".duel-card-me",150),T(".duel-card-opp",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((J,U)=>setTimeout(()=>{J.style.opacity="1"},U*70)),900),setTimeout(()=>{const J=e.querySelector("#pvp-vs");J&&(J.style.opacity="1",J.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(U=>U.style.opacity="1")},1250);function $(J,U,ee){const se=document.getElementById(J);if(!se)return;const ge=performance.now(),me=he=>{const ve=Math.min(1,(he-ge)/ee);se.textContent=Math.round(U*(1-Math.pow(1-ve,3))),ve<1?requestAnimationFrame(me):se.textContent=U};requestAnimationFrame(me)}setTimeout(()=>{$("pvp-score-me",x,800),$("pvp-score-opp",b,800)},1500);const A=r.p1Team.MIL||[],S=r.p2Team.MIL||[],B=f(A)+m(A),D=f(S)+m(S),R=B>=D?"p1":"p2";let O=r.boostValue;O==null&&(O=oi(),r.boostValue=O,r.boostOwner=R,r.boostUsed=!1),Te=!0,setTimeout(()=>{const J=e.querySelector("#duel-row-"+(h?"me":"opp")),U=e.querySelector("#duel-row-"+(h?"opp":"me")),ee=document.getElementById("pvp-score-me"),se=document.getElementById("pvp-score-opp"),ge=h?ee:se,me=h?se:ee;ge&&(ge.style.fontSize="80px",ge.style.color=h?"#FFD700":"#ff6b6b",ge.style.animation="duelPulse .5s ease"+(h?",duelGlow 1.5s ease infinite .5s":"")),me&&(me.style.opacity="0.25"),setTimeout(()=>{J&&(J.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",J.style.zIndex="5"),setTimeout(()=>{const he=document.getElementById("duel-shock");he&&(he.style.animation="shockwave .5s ease-out forwards"),U&&(U.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var Ie;const he=document.getElementById("pvp-duel-finale");if(!he)return;const ve=r.boostOwner===o?'<div style="background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:3px solid #cdeffd;border-radius:18px;padding:20px 34px;text-align:center;animation:boostFlipIn .7s cubic-bezier(.34,1.56,.64,1) both;box-shadow:0 10px 36px rgba(135,206,235,.5)"><div style="font-size:10px;color:rgba(0,0,0,.6);letter-spacing:2px;text-transform:uppercase;margin-bottom:6px;font-weight:700">Carte Boost obtenue</div><div style="font-size:46px;line-height:1">⚡</div><div style="font-size:50px;font-weight:900;color:#063;line-height:1.1">+'+O+`</div><div style="font-size:10px;color:rgba(0,0,0,.55);margin-top:4px">Applicable sur n'importe quel joueur</div></div>`:"",Me=o==="p1"?'<button id="pvp-start-match" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">▶ Commencer le match</button>':`<div style="font-size:14px;color:rgba(255,255,255,0.5);text-align:center;margin-top:8px;animation:fadeUp .4s ease both">⏳ En attente de l'adversaire...</div>`;he.innerHTML='<div style="font-size:22px;font-weight:900;color:#fff;text-align:center;animation:fadeUp .4s ease both;text-shadow:0 2px 12px rgba(0,0,0,.5)">'+(h?"⚽ "+r[o+"Name"]+"<br>gagne le milieu et attaque !":"😔 "+r[n+"Name"]+"<br>gagne l'engagement et attaque !")+"</div>"+ve+Me,he.style.transition="opacity .45s ease",he.style.opacity="1",he.style.pointerEvents="auto",(Ie=document.getElementById("pvp-start-match"))==null||Ie.addEventListener("click",async()=>{const Re=R;await le({phase:Re+"-attack",attacker:Re,round:1,boostValue:O,boostUsed:!1,boostOwner:Re})})},600)},700)},2800)}function be(w,k,f,m,x){const b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:900;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;overflow:hidden;cursor:pointer";const h=Array.from({length:10},(A,S)=>`<div style="position:absolute;font-size:${16+Math.floor(Math.random()*24)}px;top:${5+Math.floor(Math.random()*65)}%;left:${3+Math.floor(Math.random()*94)}%;animation:fw${S%2===0?"A":"B"} ${.7+Math.random()*.7}s ease ${Math.random()*.9}s both;opacity:0">${["✨","🌟","⭐","💥","🎇","🎆","🔥","🌈"][S%8]}</div>`).join(""),_={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};b.innerHTML=`
      <style>
        @keyframes butPop{0%{transform:scale(0) rotate(-8deg);opacity:0}55%{transform:scale(1.25) rotate(2deg)}85%{transform:scale(0.92) rotate(-1deg)}100%{transform:scale(1);opacity:1}}
        @keyframes ballIn{0%{transform:translate(-70px,18px);opacity:0}65%{opacity:1}100%{transform:translate(26px,-8px);opacity:1}}
        @keyframes scoreIn{from{opacity:0;transform:translateY(-12px)}to{opacity:1;transform:translateY(0)}}
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(b);let T=!1;const $=()=>{T||(T=!0,b.remove(),setTimeout(()=>x(),50))};b.addEventListener("click",$),setTimeout($,3500)}function _e(w,k,f){var O,J;const m=(r.gcDefs||[]).find(U=>{var ee;return U.name===w||((ee=U.name)==null?void 0:ee.toLowerCase().trim())===(w==null?void 0:w.toLowerCase().trim())}),x={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[m==null?void 0:m.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",b={purple:"#b06ce0",light_blue:"#00d4ef"}[m==null?void 0:m.color]||"#b06ce0",h=(m==null?void 0:m.name)||w,_=(m==null?void 0:m.effect)||((O=Ke[w])==null?void 0:O.desc)||"",T=m!=null&&m.image_url?`/manager-wars/icons/${m.image_url}`:null,$=((J=Ke[w])==null?void 0:J.icon)||"⚡",S=k===o?"Vous":r[k+"Name"]||"Adversaire",B=document.createElement("div");B.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",B.innerHTML=`
      <style>
        @keyframes gcFlipIn{0%{transform:perspective(800px) rotateY(90deg) scale(.7);opacity:0}55%{transform:perspective(800px) rotateY(-12deg) scale(1.08);opacity:1}100%{transform:perspective(800px) rotateY(0) scale(1);opacity:1}}
      ...
      <div style="display:flex;gap:12px;width:190px">
        <button id="pvp-gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
        <button id="pvp-gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
      </div>`,document.body.appendChild(S),(R=S.querySelector("#pvp-gc-back"))==null||R.addEventListener("click",()=>S.remove()),(O=S.querySelector("#pvp-gc-use"))==null||O.addEventListener("click",()=>{S.remove(),H(w,k)})}function ze(){var m;const w=r[o+"Team"],k=Object.entries(w).flatMap(([x,b])=>(b||[]).filter(h=>!h.used).map(h=>({...h,_line:x})));if(!k.length)return;const f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",f.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
        <div style="flex:1;font-size:14px;font-weight:700;color:#fff">⚡ Choisir un joueur pour +${r.boostValue}</div>
        <button id="bp-close" style="background:none;border:none;color:rgba(255,255,255,0.5);font-size:22px;cursor:pointer">✕</button>
      </div>
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${k.map(x=>{const b={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[x._line]||"#555",h=Se(x,x._line)+(x.boost||0);return`<div class="bp-item" data-cid="${x.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${b};overflow:hidden;cursor:pointer">
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        <div id="pin-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
          ...
          <div>${J?Ye({...J,used:!1,boost:0},U,ee):"<div>—</div>"}</div>
      ...
      <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
        <button id="psub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
      </div>`,(me=S.querySelector("#psub-close"))==null||me.addEventListener("click",()=>S.remove()),(he=S.querySelector("#pout-up"))==null||he.addEventListener("click",()=>{_>0&&(_--,B())}),(ve=S.querySelector("#pout-down"))==null||ve.addEventListener("click",()=>{_<b.length-1&&(_++,B())}),(Me=S.querySelector("#pin-up"))==null||Me.addEventListener("click",()=>{$>0&&($--,B())}),(Ie=S.querySelector("#pin-down"))==null||Ie.addEventListener("click",()=>{$<h.length-1&&($++,B())});const ge=(qe,Oe,Be,Ve)=>{const Ne=S.querySelector("#"+qe);if(!Ne)return;let He=0;Ne.addEventListener("touchstart",Ue=>{He=Ue.touches[0].clientY},{passive:!0}),Ne.addEventListener("touchend",Ue=>{const Je=Ue.changedTouches[0].clientY-He;if(Math.abs(Je)<30)return;const Ze=Oe();Je<0&&Ze<Ve-1?(Be(Ze+1),B()):Je>0&&Ze>0&&(Be(Ze-1),B())},{passive:!0})};ge("pin-panel",()=>$,qe=>$=qe,h.length),ge("pout-panel",()=>_,qe=>_=qe,b.length),(Re=S.querySelector("#psub-confirm"))==null||Re.addEventListener("click",async qe=>{if(qe.preventDefault(),qe.stopPropagation(),A)return;A=!0;const Oe=b[_],Be=h[$];if(!Oe||!Be)return;const Ve=Oe._line,Ne=(f[Ve]||[]).findIndex(Je=>Je.cardId===Oe.cardId);if(Ne===-1){p("Erreur : joueur introuvable","error"),S.remove();return}const He={...Be,_line:Ve,position:Oe.position,used:!1,boost:0};f[Ve].splice(Ne,1,He);const Ue=[...m,Be.cardId];S.remove(),W(Oe,Be,async()=>{await le({[o+"Team"]:f,[n+"Team"]:r[n+"Team"],["usedSubIds_"+o]:Ue})})})}document.body.appendChild(S),B()}function W(w,k,f){const m={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const b=(T,$,A)=>`<div style="text-align:center">
      ...
      </div>`,document.body.appendChild(k),(m=k.querySelector("#pvp-hist-close"))==null||m.addEventListener("click",()=>k.remove())}async function oe(){if(r.phase!==o+"-attack")return;const w=o==="p1"?"p2":"p1",k=(r.round||0)+1,f=[...r.log||[]];f.push({type:"info",text:`⏭️ ${r[o+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const m=je(r),x=Fe(w),b=m||!x?"finished":w+"-attack";await le({["selected_"+o]:[],modifiers:{...r.modifiers,[o]:{}},pendingAttack:null,phase:b,attacker:w,round:k,log:f}),b==="finished"&&await I(r)}async function re(){const w=r[o+"Team"],k=!["GK","DEF","MIL","ATT"].some(b=>(r[n+"Team"][b]||[]).some(h=>!h.used)),f=(r["selected_"+o]||[]).map(b=>{const h=(w[b._role]||[]).find(B=>B.cardId===b.cardId)||b,_=k&&["GK","DEF"].includes(b._role),T=w[b._role]||[],$=T.findIndex(B=>B.cardId===b.cardId),A=lt(T.length),S=$>=0?A[$]:h._col??1;return{...h,_line:b._role,_col:S,..._?{note_a:Math.max(1,Number(h.note_a)||0)}:{}}});if(!f.length)return;const m=Nt(f,r.modifiers[o]||{});G(o,f.map(b=>b.cardId)),f.forEach(b=>{const h=(w[b._role]||[]).find(_=>_.cardId===b.cardId);h&&(h.used=!0)}),r["selected_"+o]=[],fe();const x=[...r.log||[]];if(k){const b=(r[o+"Score"]||0)+1,h=f.map(S=>({name:S.name,note:Se(S,S._line||"ATT"),portrait:Pe(S),job:S.job}));x.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:h,homeTotal:m.total,aiTotal:0});const _=(r.round||0)+1,T=o==="p1"?"p2":"p1",$={...r,[o+"Team"]:w,[o+"Score"]:b},A=je($);C.add(_),be(h,b,r[n+"Score"]||0,!0,async()=>{await le({[o+"Team"]:w,[o+"Score"]:b,["selected_"+o]:[],modifiers:{...r.modifiers,[o]:{}},pendingAttack:null,phase:A?"finished":T+"-attack",attacker:T,round:_,log:x}),A&&await I({...r,[o+"Score"]:b})});return}x.push({type:"pending",text:`⚔️ ${r[o+"Name"]} attaque (${m.total})`}),await le({[o+"Team"]:w,[n+"Team"]:r[n+"Team"],pendingAttack:{...m,players:f,side:o},["selected_"+o]:[],modifiers:{...r.modifiers,[o]:{}},phase:n+"-defense",log:x})}async function Q(){const w=r[o+"Team"],k=(r["selected_"+o]||[]).map(O=>{const J=(w[O._role]||[]).find(me=>me.cardId===O.cardId)||O,U=w[O._role]||[],ee=U.findIndex(me=>me.cardId===O.cardId),se=lt(U.length),ge=ee>=0?se[ee]:J._col??1;return{...J,_line:O._role,_col:ge}}),f=Rt(k,r.modifiers[o]||{});G(o,k.map(O=>O.cardId)),k.forEach(O=>{const J=(w[O._role]||[]).find(U=>U.cardId===O.cardId);J&&(J.used=!0)}),r["selected_"+o]=[],fe();const m=Ot(r.pendingAttack.total,f.total,r.modifiers[o]||{}),x=r.pendingAttack.side,b=m==="attack"||(m==null?void 0:m.goal),h=x==="p1"?"p2":"p1",_=(r.round||0)+1,T=(r.pendingAttack.players||[]).map(O=>({name:O.name,note:Se(O,O._line||"ATT"),portrait:Pe(O),job:O.job})),$=[...r.log||[]];$.push({type:"duel",isGoal:b,homeScored:b&&x===o,text:b?`⚽ BUT de ${r[x+"Name"]} ! (${r.pendingAttack.total} vs ${f.total})`:`✋ Attaque stoppée (${r.pendingAttack.total} vs ${f.total})`,homePlayers:T,aiPlayers:k.map(O=>({name:O.name,note:Se(O,O._line||"DEF"),portrait:Pe(O),job:O.job})),homeTotal:r.pendingAttack.total,aiTotal:f.total});const A=b?(r[x+"Score"]||0)+1:r[x+"Score"]||0,S={...r,[o+"Team"]:w,[x+"Score"]:A},B=je(S),D=B?"finished":h+"-attack",R=async()=>{await le({[o+"Team"]:w,[n+"Team"]:r[n+"Team"],[x+"Score"]:A,["selected_"+o]:[],modifiers:{...r.modifiers,[o]:{}},pendingAttack:null,phase:D,attacker:h,round:_,log:$}),(D==="finished"||B)&&await I({...r,[x+"Score"]:A})};if(b){const O=x===o,J=O?A:r[o+"Score"]||0,U=O?r[n+"Score"]||0:A;C.add(_),be(T,J,U,O,R)}else await R()}function ae(w){return["MIL","ATT"].some(k=>(w[k]||[]).some(f=>!f.used))}function ke(w){return["GK","DEF","MIL","ATT"].some(k=>(w[k]||[]).some(f=>!f.used))}function Le(w){return ke(w)&&!ae(w)}function Fe(w){const k=r[w+"Team"],f=r[(w==="p1"?"p2":"p1")+"Team"];return!!(ae(k)||!ke(f)&&Le(k))}function je(w){const k=["MIL","ATT"].some($=>(w.p1Team[$]||[]).some(A=>!A.used)),f=["MIL","ATT"].some($=>(w.p2Team[$]||[]).some(A=>!A.used)),m=ke(w.p1Team),x=ke(w.p2Team);return!k&&!(!x&&m)&&(!f&&!(!m&&x))}function De(w){const k=w.p1Score||0,f=w.p2Score||0;return k===f?null:k>f?y.home_id:y.away_id}async function I(w){try{const k=De(w),f=k?y.home_id===k?y.away_id:y.home_id:null;await E.from("matches").update({status:"finished",winner_id:k,home_score:w.p1Score||0,away_score:w.p2Score||0}).eq("id",i),k&&f&&ki(k,f).catch(()=>{})}catch(k){console.error("[PvP] finishMatch:",k)}}function q(){var x;const w=r[o+"Score"],k=r[n+"Score"],f=w>k,m=w===k;et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="background:linear-gradient(160deg,#1a0a2e,#3b1a6e);border-radius:20px;padding:32px 24px;max-width:320px;width:100%;text-align:center;color:#fff;box-shadow:0 8px 40px rgba(0,0,0,0.6)">
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">🏆 Retour à la Mini League</button>`,document.body.appendChild(_),(B=_.querySelector("#pvp-end-home"))==null||B.addEventListener("click",()=>{_.remove(),We(e),o?l("mini-league",{openLeagueId:o}):l("mini-league")})}const le=E.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},f=>{const m=f.new;try{if(m.status==="finished"||m.forfeit){if(M||(M=!0,V&&(clearInterval(V),V=null),m.game_state&&(u=m.game_state),u.phase==="finished"&&!m.forfeit&&document.getElementById("pvp-home")))return;te(m);return}if(m.game_state){const x=u;u=m.game_state;const b=u._lastGC;if(b&&b.seq&&!j.has(b.seq)&&(j.add(b.seq),b.by!==n)){Ae(b.type,b.by,()=>ye());return}const h=x[n+"Score"]||0,_=x[g+"Score"]||0,T=u[n+"Score"]||0,$=u[g+"Score"]||0,A=T>h,S=$>_;if((A||S)&&!F.has(u.round)){F.add(u.round);const B=[...u.log||[]].reverse().find(R=>R.isGoal),D=((B==null?void 0:B.homePlayers)||[]).map(R=>({name:R.name,note:R.note,portrait:R.portrait,job:R.job}));_e(D,T,$,A,()=>ye());return}ye()}}catch(x){console.error("[PvP] crash:",x)}}).subscribe();async function de(f){Object.assign(u,f),u.lastActionAt=new Date().toISOString();const{error:m}=await E.from("matches").update({game_state:u}).eq("id",i);m&&p("Erreur de synchronisation","error");try{ye()}catch(x){console.error("[PvP] renderPvpScreen crash:",x)}}async function P(){if(M)return;M=!0,V&&(clearInterval(V),V=null);const f=s?z.away_id:z.home_id,m=s?"p2":"p1",x=s?"p1":"p2",b={...u,[m+"Score"]:3,[x+"Score"]:0,phase:"finished"};try{await E.from("matches").update({status:"finished",forfeit:!0,winner_id:f,home_score:b.p1Score||0,away_score:b.p2Score||0,game_state:b}).eq("id",i)}catch{}try{E.removeChannel(le)}catch{}setTimeout(()=>{We(e),l("mini-league")},800)}const Y={BOOST_STAT:({value:f=1,count:m=1,roles:x=[]},b,h)=>{const _=b[n+"Team"],T=Object.entries(_).filter(([$])=>!x.length||x.includes($)).flatMap(([$,A])=>A.filter(S=>!S.used).map(S=>({...S,_line:$})));if(!T.length){b.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),h(b);return}H(T,m,`Choisir ${m} joueur(s) à booster (+${f})`,$=>{$.forEach(A=>{const S=(_[A._line]||[]).find(B=>B.cardId===A.cardId);S&&(S.boost=(S.boost||0)+f,b.log.push({text:`⚡ +${f} sur ${S.name}`,type:"info"}))}),b[n+"Team"]=_,h(b)})},DEBUFF_STAT:({value:f=1,count:m=1,roles:x=[],target:b="ai"},h,_)=>{const T=b==="home"?n:g,$=h[T+"Team"],A=b==="home"?"allié":"adverse",S=Object.entries($).filter(([B])=>!x.length||x.includes(B)).flatMap(([B,D])=>D.map(R=>({...R,_line:B})));if(!S.length){h.log.push({text:`🎯 Aucun joueur ${A}`,type:"info"}),_(h);return}H(S,m,`Choisir ${m} joueur(s) ${A}(s) (-${f})`,B=>{B.forEach(D=>{const R=($[D._line]||[]).find(O=>O.cardId===D.cardId);R&&(R.boost=(R.boost||0)-f,h.log.push({text:`🎯 -${f} sur ${R.name}`,type:"info"}))}),h[T+"Team"]=$,_(h)})},GRAY_PLAYER:({count:f=1,roles:m=[],target:x="ai"},b,h)=>{const _=x==="home"?n:g,T=b[_+"Team"],$=x==="home"?"allié":"adverse",A=Object.entries(T).filter(([S])=>!m.length||m.includes(S)).flatMap(([S,B])=>B.filter(D=>!D.used).map(D=>({...D,_line:S})));if(!A.length){b.log.push({text:`❌ Aucun joueur ${$}`,type:"info"}),h(b);return}H(A,f,`Choisir ${f} joueur(s) ${$}(s) à exclure`,S=>{const B="usedCardIds_"+_,D=new Set(b[B]||[]);S.forEach(R=>{const O=(T[R._line]||[]).find(J=>J.cardId===R.cardId);O&&(O.used=!0,D.add(R.cardId),b.log.push({text:`❌ ${O.name} exclu !`,type:"info"}))}),b[B]=[...D],b[_+"Team"]=T,h(b)})},REVIVE_PLAYER:({count:f=1,roles:m=[]},x,b)=>{const h=x[n+"Team"],_=Object.entries(h).filter(([T])=>!m.length||m.includes(T)).flatMap(([T,$])=>$.filter(A=>A.used).map(A=>({...A,_line:T})));if(!_.length){x.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),b(x);return}H(_,f,`Choisir ${f} joueur(s) à ressusciter`,T=>{T.forEach($=>{const A=(h[$._line]||[]).find(S=>S.cardId===$.cardId);A&&(A.used=!1,x.log.push({text:`💫 ${A.name} ressuscité !`,type:"info"}))}),x[n+"Team"]=h,b(x)})},REMOVE_GOAL:({},f,m)=>{const x=g+"Score";f[x]>0?(f[x]--,f.log.push({text:"🚫 Dernier but annulé !",type:"info"})):f.log.push({text:"🚫 Aucun but à annuler",type:"info"}),m(f)},ADD_GOAL_DRAW:({},f,m)=>{f[n+"Score"]===f[g+"Score"]?(f[n+"Score"]++,f.log.push({text:"🎯 But bonus !",type:"info"})):f.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),m(f)},ADD_SUB:({value:f=1},m,x)=>{m.maxSubs=(m.maxSubs||3)+f,m.log.push({text:`🔄 +${f} remplacement(s)`,type:"info"}),x(m)},CUSTOM:({},f,m)=>m(f)};function H(f,m,x,b){const h=document.createElement("div");h.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let _=[];function T(){var A,S;const $=f.map(B=>{const D={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[B._line]||"#555",R=Se(B,B._line)+(B.boost||0),J=_.find(ee=>ee.cardId===B.cardId)?"#FFD700":"rgba(255,255,255,0.25)",U=B.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${B.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${J};background:${D};overflow:hidden;cursor:pointer;${U}">
        ...
        </div>`,(A=h.querySelector("#pp-close"))==null||A.addEventListener("click",()=>h.remove()),h.querySelectorAll(".pp-item").forEach(B=>{B.addEventListener("click",()=>{const D=B.dataset.cid,R=f.find(J=>J.cardId===D),O=_.findIndex(J=>J.cardId===D);R&&(O>-1?_.splice(O,1):_.length<m&&_.push(R),T())})}),(S=h.querySelector("#pp-confirm"))==null||S.addEventListener("click",()=>{h.remove(),b(_)})}T(),document.body.appendChild(h)}async function G(f,m){const b=(u["gcCardsFull_"+n]||[]).find($=>$.id===f),h=(b==null?void 0:b._gcDef)||(u.gcDefs||[]).find($=>{var A;return $.name===m||((A=$.name)==null?void 0:A.toLowerCase().trim())===(m==null?void 0:m.toLowerCase().trim())}),_=[...u["usedGc_"+n]||[],f],T={type:m,by:n,seq:(u._gcAnimSeq||0)+1};j.add(T.seq),Ae(m,n,async()=>{if(h!=null&&h.effect_type&&h.effect_type!=="CUSTOM"){const A=Y[h.effect_type];if(A){const S={...u};A(h.effect_params||{},S,async B=>{B["usedGc_"+n]=_,B._lastGC=T,B._gcAnimSeq=T.seq,await de(B)});return}}const $={...u};switch(m){case"Remplacement+":$.maxSubs=($.maxSubs||3)+1,$.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const A=g+"Score";$[A]>0&&($[A]--,$.log.push({text:"🚫 But annulé",type:"info"}));break}}$["usedGc_"+n]=_,$._lastGC=T,$._gcAnimSeq=T.seq,await de($)})}function ne(f,m){const x="usedCardIds_"+f,b=new Set(u[x]||[]);m.forEach(h=>b.add(h)),u[x]=[...b]}function fe(){for(const f of["p1","p2"]){const m=new Set(u["usedCardIds_"+f]||[]),x=u[f+"Team"];if(x)for(const b of["GK","DEF","MIL","ATT"])(x[b]||[]).forEach(h=>{h.used=m.has(h.cardId)})}}function ye(){var nt,gt,ce,we,Z,ue;if(u.phase==="reveal")return Te();if(u.phase==="midfield")return be();if(u.phase==="finished")return w();const f=u[n+"Team"],m=u[g+"Team"];fe();const x=u[n+"Score"],b=u[g+"Score"],h=u[n+"Name"],_=u[g+"Name"],T=u.phase===n+"-attack",$=u.phase===n+"-defense",A=Array.isArray(u["selected_"+n])?u["selected_"+n]:[],S=A.map(X=>X.cardId),B=window.innerWidth>=700,D=u[n+"Subs"]||[],R=u["usedSubIds_"+n]||[],O=D.filter(X=>!R.includes(X.cardId)),J=u["gcCardsFull_"+n]||[],U=u["usedGc_"+n]||[],ee=J.filter(X=>!U.includes(X.id)),se=u.boostOwner===n&&!u.boostUsed,ge=!["GK","DEF","MIL","ATT"].some(X=>(m[X]||[]).some(pe=>!pe.used)),me=[...f.MIL||[],...f.ATT||[]].filter(X=>!X.used),he=T&&ge&&me.length===0?[...f.GK||[],...f.DEF||[]].filter(X=>!X.used).map(X=>X.cardId):[];function ve(X,pe,Ce){var Si,Li;const $e=X._gcDef,tt={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[$e==null?void 0:$e.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",it={purple:"#b06ce0",light_blue:"#00d4ef"}[$e==null?void 0:$e.color]||"#b06ce0",rt=($e==null?void 0:$e.name)||X.gc_type,st=($e==null?void 0:$e.effect)||((Si=Ke[X.gc_type])==null?void 0:Si.desc)||"",ct=$e!=null&&$e.image_url?`/manager-wars/icons/${$e.image_url}`:null,$t=((Li=Ke[X.gc_type])==null?void 0:Li.icon)||"⚡",pt=Math.round(Ce*.22),ht=Math.round(Ce*.22),kt=Ce-pt-ht,Ut=rt.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${X.id}" data-gc-type="${X.gc_type}"
        style="box-sizing:border-box;width:${pe}px;height:${Ce}px;border-radius:10px;border:2px solid ${it};background:${tt};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
      ...
      </div>`}function Me(X,pe){return`<div id="pvp-boost-card"
        style="box-sizing:border-box;width:${X}px;height:${pe}px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:2px solid #87CEEB;border-radius:10px;cursor:pointer;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:${Math.round(pe*.04)}px;text-align:center;flex-shrink:0">
      ...
      </div>`}const Ie=(X,pe)=>pe?Me(130,175):ve(X,130,175),Re=(X,pe)=>pe?Me(68,95):ve(X,68,95),qe=B?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",Oe=T?je(n)?`<button id="pvp-action" style="${qe};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${A.length===0?"disabled":""}>⚔️ ATTAQUEZ <span id="pvp-timer"></span></button>`:`<button id="pvp-action" data-pass="1" style="${qe};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER <span id="pvp-timer"></span></button>`:$?`<button id="pvp-action" style="${qe};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${A.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="pvp-timer"></span></button>`:`<div style="font-size:11px;color:rgba(255,255,255,0.3);text-align:center;padding:4px">⏳ Tour de ${_}</div>`,Be=T&&!je(n)?'<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">Aucun attaquant — passez la main</div>':T||$?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${A.length}/3 sélectionné(s)</div>`:"",Ve=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${B?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
      ${O.length===0?'<div style="font-size:7px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':O.map(X=>`<div class="pvp-sub-btn" data-sub-id="${X.cardId}" style="cursor:pointer;flex-shrink:0">${Ye(X,B?76:44,B?100:58)}</div>`).join("")}
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${ft(f,u[n+"Formation"],Ne,S,300,300,he)}
    ...
    </div>`;function Ue(X){if(X.type==="duel"&&(X.homeTotal!=null||X.aiTotal!=null)){const pe=(X.homeTotal||0)>=(X.aiTotal||0);return`<div style="background:rgba(255,255,255,0.05);border-radius:8px;padding:5px 6px;border:1px solid rgba(255,255,255,0.08)">
          ...
          <div style="display:flex;align-items:center;gap:3px;max-height:46px;overflow:hidden">
            <div style="flex:1;display:flex;gap:2px;justify-content:flex-end;flex-wrap:nowrap;overflow-x:auto;overflow-y:hidden">
              ${(X.homePlayers||[]).map(Ce=>dt(Ce)).join("")}
            </div>
            <div style="text-align:center;padding:0 6px;flex-shrink:0">
              <div style="font-size:${pe?20:14}px;font-weight:900;color:${pe?"#FFD700":"rgba(255,255,255,0.4)"};line-height:1">${X.homeTotal}</div>
              <div style="font-size:8px;color:rgba(255,255,255,0.3);margin:1px 0">VS</div>
              <div style="font-size:${pe?14:20}px;font-weight:900;color:${pe?"rgba(255,255,255,0.4)":"#ff6b6b"};line-height:1">${X.aiTotal}</div>
            </div>
            <div style="flex:1;display:flex;gap:2px;justify-content:flex-start;flex-wrap:nowrap;overflow-x:auto;overflow-y:hidden">
              ${(X.aiPlayers||[]).map(Ce=>dt(Ce)).join("")}
            </div>
          </div>
          ${X.isGoal?`<div style="text-align:center;font-size:11px;color:#FFD700;font-weight:900;margin-top:3px">${X.homeScored,"⚽ BUT !"}</div>`:""}
        </div>`}return`<div style="font-size:11px;color:${X.type==="goal"?"#FFD700":"rgba(255,255,255,0.65)"};font-weight:${X.type==="goal"?700:400};padding:3px 2px">${X.text||""}</div>`}const Je=(()=>{var pe,Ce;if($&&((pe=u.pendingAttack)!=null&&pe.players)){const $e=u.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
          ...
          ${ot(($e.players||[]).map(tt=>({...tt,used:!1})),"#ff6b6b",$e.total)}
        </div>`}if(T&&((Ce=u.pendingAttack)!=null&&Ce.players)){const $e=u.pendingAttack;return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #00ff88">
          <div style="font-size:8px;color:#00ff88;letter-spacing:2px;margin-bottom:4px;text-transform:uppercase">⚔️ VOUS ATTAQUEZ</div>
          ${ot(($e.players||[]).map(tt=>({...tt,used:!1})),"#00ff88",$e.total)}
        </div>`}const X=(u.log||[]).slice(-1)[0];return X?'<div style="padding:2px 4px">'+Ue(X)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})(),Ze=`
      <div style="display:flex;align-items:center;padding:8px 10px;background:rgba(0,0,0,0.5);gap:6px;flex-shrink:0">
        <button id="pvp-quit" style="width:34px;height:34px;border-radius:50%;background:rgba(220,50,50,0.7);border:none;color:#fff;font-size:16px;cursor:pointer;flex-shrink:0">✕</button>
        <div style="flex:1;display:flex;align-items:center;justify-content:center;gap:8px">
      ...
      </div>`;function Qe(){const X=e.querySelector(".match-screen");if(!X)return;const pe=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);X.style.height=pe+"px",X.style.minHeight=pe+"px",X.style.maxHeight=pe+"px",X.style.overflow="hidden";const Ce=e.querySelector("#mobile-action-bar"),$e=e.querySelector("#mobile-play-area");Ce&&$e&&($e.style.paddingBottom=Ce.offsetHeight+"px")}if(Qe(),setTimeout(Qe,120),setTimeout(Qe,400),C||(C=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",Qe),window.visualViewport.addEventListener("scroll",Qe)),window.addEventListener("resize",Qe)),function(){const pe=e.querySelector(".terrain-wrapper svg");pe&&(pe.removeAttribute("width"),pe.removeAttribute("height"),pe.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",pe.setAttribute("viewBox","-26 -26 352 352"),pe.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),e.querySelectorAll(".match-slot-hit").forEach(X=>{X.addEventListener("click",()=>{if(!T&&!$)return;const pe=X.dataset.cardId,Ce=X.dataset.role,$e=(f[Ce]||[]).find(st=>st.cardId===pe);if(!$e||$e.used)return;const tt=he.includes(pe);if(T&&!["MIL","ATT"].includes(Ce)&&!tt)return;Array.isArray(u["selected_"+n])||(u["selected_"+n]=[]);const it=u["selected_"+n],rt=it.findIndex(st=>st.cardId===pe);rt>-1?it.splice(rt,1):it.length<3&&it.push({...$e,_role:Ce}),ye()})}),e.querySelectorAll(".match-used-hit").forEach(X=>{X.addEventListener("click",()=>W(X.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(X=>{X.addEventListener("click",()=>W())}),(nt=e.querySelector("#pvp-sub-open"))==null||nt.addEventListener("click",()=>W()),e.querySelectorAll(".pvp-gc-mini").forEach(X=>{X.addEventListener("click",()=>ze(X.dataset.gcId,X.dataset.gcType))}),(gt=e.querySelector("#pvp-boost-card"))==null||gt.addEventListener("click",()=>xe()),(ce=e.querySelector("#pvp-action"))==null||ce.addEventListener("click",X=>{T?X.currentTarget.dataset.pass==="1"||!je(n)?re():Q():$&&ae()}),(we=e.querySelector("#pvp-quit"))==null||we.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&P()}),(Z=e.querySelector("#pvp-view-opp"))==null||Z.addEventListener("click",()=>K()),(ue=e.querySelector("#pvp-toggle-history"))==null||ue.addEventListener("click",()=>oe()),V&&(clearInterval(V),V=null),(T||$)&&!M){let X=30,pe=!1;const Ce=()=>document.getElementById("pvp-timer"),$e=()=>{Ce()&&(Ce().textContent=X+"s",Ce().style.color=pe?"#ff4444":"#fff")};$e(),V=setInterval(()=>{X--,X<0?pe?(clearInterval(V),V=null,T&&!je(n)?re():P()):(pe=!0,X=15,$e()):$e()},1e3)}}function Te(){et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
      ...
      <div style="width:min(90vw,420px)">${ut(u[g+"Team"],u[g+"Formation"],null,[],300,300)}</div>
    </div>`,n==="p1"&&setTimeout(async()=>{await de({phase:"midfield"})},5e3)}let Ee=!1;function be(){if(Ee)return;const f=u[n+"Team"].MIL||[],m=u[g+"Team"].MIL||[];function x(ee){return ee.reduce((se,ge)=>se+Se(ge,"MIL"),0)}function b(ee){let se=0;for(let ge=0;ge<ee.length-1;ge++){const me=at(ee[ge],ee[ge+1]);me==="#00ff88"?se+=2:me==="#FFD700"&&(se+=1)}return se}const h=x(f)+b(f),_=x(m)+b(m),T=h>=_;function $(ee,se,ge){return`<div id="duel-row-${ge}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
        ...
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${ee.map((me,he)=>{const ve=he<ee.length-1?at(me,ee[he+1]):null,Me=!ve||ve==="#ff3333"||ve==="#cc2222",Ie=ve==="#00ff88"?"+2":ve==="#FFD700"?"+1":"";return`<div class="duel-card duel-card-${ge}" data-idx="${he}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease,transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
              ${Ye({...me,note:Se(me,"MIL"),_line:"MIL"},58,78)}
            </div>
            ${he<ee.length-1?`<div class="duel-link duel-link-${ge}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${Me?"rgba(255,255,255,0.12)":ve};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${Me?"none":`0 0 8px ${ve}`}">
              ${Ie?`<span style="position:absolute;top:-13px;left:50%;transform:translateX(-50%);font-size:8px;font-weight:900;color:${ve}">${Ie}</span>`:""}
            </div>`:""}`}).join("")}
        </div>
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
        <button id="psub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
      </div>`,(ve=D.querySelector("#psub-close"))==null||ve.addEventListener("click",()=>D.remove()),(Me=D.querySelector("#pout-up"))==null||Me.addEventListener("click",()=>{$>0&&($--,R())}),(Ie=D.querySelector("#pout-down"))==null||Ie.addEventListener("click",()=>{$<_.length-1&&($++,R())}),(Re=D.querySelector("#pin-up"))==null||Re.addEventListener("click",()=>{S>0&&(S--,R())}),(qe=D.querySelector("#pin-down"))==null||qe.addEventListener("click",()=>{S<T.length-1&&(S++,R())});const he=(Be,Ve,Ne,He)=>{const Ue=D.querySelector("#"+Be);if(!Ue)return;let Je=0;Ue.addEventListener("touchstart",Ze=>{Je=Ze.touches[0].clientY},{passive:!0}),Ue.addEventListener("touchend",Ze=>{const Qe=Ze.changedTouches[0].clientY-Je;if(Math.abs(Qe)<30)return;const nt=Ve();Qe<0&&nt<He-1?(Ne(nt+1),R()):Qe>0&&nt>0&&(Ne(nt-1),R())},{passive:!0})};he("pin-panel",()=>S,Be=>S=Be,T.length),he("pout-panel",()=>$,Be=>$=Be,_.length),(Oe=D.querySelector("#psub-confirm"))==null||Oe.addEventListener("click",async Be=>{if(Be.preventDefault(),Be.stopPropagation(),B)return;B=!0;const Ve=_[$],Ne=T[S];if(!Ve||!Ne)return;const He=Ve._line,Ue=(x[He]||[]).findIndex(Qe=>Qe.cardId===Ve.cardId);if(Ue===-1){p("Erreur : joueur introuvable","error"),D.remove();return}const Je={...Ne,_line:He,position:Ve.position,used:!1,boost:0};x[He].splice(Ue,1,Je);const Ze=[...b,Ne.cardId];D.remove(),ie(Ve,Ne,async()=>{await de({[n+"Team"]:x,[g+"Team"]:u[g+"Team"],["usedSubIds_"+n]:Ze})})})}document.body.appendChild(D),R()}function ie(f,m,x){const b={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},h=document.createElement("div");h.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const _=(A,S,B)=>`<div style="text-align:center">
