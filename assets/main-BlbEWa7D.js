...
import{s as S,F as Ei,r as nt,j as Ot,l as Vi,m as rt,n as ji,h as Pt,o as vo,T as di,k as ho,p as wo,b as _o}from"./formation-links-1Ic4vV1e.js";const $o="/";function Ci(e,{navigate:t,toast:i}){let o="login";const d=()=>{var r,c,u,s,n,h;const a=o==="login";e.innerHTML=`
  ...
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
  ...
  </div>`;function H(){const O=document.getElementById("col-filters");O&&(F==="player"?(O.innerHTML=`
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Yt=1e3;function Wo(e,t,i,o){var _,g,l;const{state:d,toast:a,closeModal:r,navigate:c,refreshProfile:u}=i,s=e.formation,n={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function h(){const T=Ot[s]||{},f=Ei[s]||[],L=290,j=360,G=20;function F(N){const K=T[N];return K?{x:K.x*L,y:K.y*j}:null}let q=`<svg width="${L}" height="${j}" viewBox="0 0 ${L} ${j}" xmlns="http://www.w3.org/2000/svg">`;for(const[N,K]of f){const Q=F(N),re=F(K);!Q||!re||(q+=`<line x1="${Q.x}" y1="${Q.y}" x2="${re.x}" y2="${re.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const N of Object.keys(T)){const K=F(N);if(!K)continue;const Q=N.replace(/\d+/,""),re=n[Q]||"#555";q+=`<circle cx="${K.x}" cy="${K.y}" r="${G}" fill="${re}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,q+=`<text x="${K.x}" y="${K.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${Q}</text>`}return q+="</svg>",q}const m=t.filter(T=>T.formation===s);m.length;const y=!e.is_for_sale;o(`Formation ${s}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",n.note_g],["DEF",n.note_d],["MIL",n.note_m],["ATT",n.note_a]].map(([fe,me])=>{const ge=li[fe],O=fe==="GK"?"#fff":ge,X=fe===n.job||fe===n.job2,ae=(Number(me)||0)+(X&&F>0?F:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(_e=document.getElementById("close-detail"))==null||_e.addEventListener("click",c);let D=new Set;const oe=()=>{const fe=D.size,me=document.getElementById("sell-action-panel");if(!me)return;me.style.display=fe>0?"block":"none",document.getElementById("sell-selected-count").textContent=fe;const ge=document.getElementById("evolve-btn");ge&&(ge.textContent=`⬆️ Évoluer ${fe>1?"(+"+fe+")":""}`)};document.querySelectorAll(".expl-check").forEach(fe=>{fe.addEventListener("change",()=>{const me=fe.dataset.id;fe.checked?D.add(me):D.delete(me);const ge=fe.closest(".exemplaire-row");ge&&(ge.style.borderColor=fe.checked?"#1A6B3C":"#eee"),oe()})}),document.querySelectorAll(".exemplaire-row").forEach(fe=>{fe.addEventListener("click",me=>{if(me.target.closest("button")||me.target.tagName==="INPUT")return;const ge=fe.querySelector(".expl-check");ge&&!ge.disabled&&(ge.checked=!ge.checked,ge.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(fe=>{fe.addEventListener("click",me=>{me.stopPropagation();const ge=document.querySelector(`.expl-hist[data-hist="${fe.dataset.idx}"]`);ge&&(ge.style.display=ge.style.display==="none"?"flex":"none")})}),(he=document.getElementById("evolve-btn"))==null||he.addEventListener("click",async()=>{if(m<=1)return;const fe=[...D];if(!fe.length)return;if(D.has(e.id)){const pe=document.createElement("div");pe.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",pe.innerHTML=`
  ...
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
    ...
    <div class="page-body" style="padding-bottom:20px">
    ...
    </div>`;l("⚽ Choisir une formation",f),document.querySelectorAll("#modal-body [data-forma]").forEach(L=>{L.addEventListener("click",()=>{t.formation=L.dataset.forma;const j=Oi(t.formation),G={};j.forEach(F=>{t.slots[F]&&(G[F]=t.slots[F])}),t.slots=G,T(),ht(e,t,i)})})};e.querySelectorAll("#formation-mobile-btn, #formation-pc-btn").forEach(l=>l.addEventListener("click",y)),e.querySelectorAll("#add-stad-btn-pc, #add-stad-btn").forEach(l=>l.addEventListener("click",()=>Xo(t,e,i))),e.querySelectorAll("#save-deck").forEach(l=>l.addEventListener("click",()=>en(t,i))),e.querySelectorAll("[data-remove-sub]").forEach(l=>{l.addEventListener("click",()=>{t.subs=t.subs.filter(T=>T!==l.dataset.removeSub),ht(e,t,i)})}),e.querySelectorAll("#add-sub-btn").forEach(l=>l.addEventListener("click",()=>Zo(t,e,i)))}function Jo(e,t,i,o){var L,j;const d=window.innerWidth>=900,a=e.querySelector(d?"#deck-field-pc":"#deck-field-mobile");if(!a)return;const r=(L=t.stadiumCards)==null?void 0:L.find(G=>G.id===t.stadiumCardId),c=r&&((j=t.stadDefMap)==null?void 0:j[r.stadium_id])||null,u=Ot[t.formation]||{},s=Vi(t.formation)||[],n={};for(const G of i){const F=t.slots[G],q=F?t.playerCards.find(N=>N.id===F):null;q!=null&&q.player?n[G]={...q.player,_evolution_bonus:q.evolution_bonus||0,face:q.player.face||null}:n[G]=null}const h=window.innerWidth>=900,m=h?window.innerWidth-280:window.innerWidth-20,y=h?Math.min(m,860):m,_=Math.round(h?y*.82:y*.85),g=h?84:44;let l="";for(const[G,F]of s){const q=u[G],N=u[F];if(!q||!N)continue;const K=q.x*y,Q=Math.round(.03*_+q.y*.85*_),re=N.x*y,le=Math.round(.03*_+N.y*.85*_),te=n[G],H=n[F],D=rt(te,H);D==="#ff3333"||D==="#cc2222"?l+=`<line x1="${K.toFixed(1)}" y1="${Q.toFixed(1)}" x2="${re.toFixed(1)}" y2="${le.toFixed(1)}" stroke="${D}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(l+=`<line x1="${K.toFixed(1)}" y1="${Q.toFixed(1)}" x2="${re.toFixed(1)}" y2="${le.toFixed(1)}" stroke="${D}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,l+=`<line x1="${K.toFixed(1)}" y1="${Q.toFixed(1)}" x2="${re.toFixed(1)}" y2="${le.toFixed(1)}" stroke="${D}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let T="";const f=Math.round(g*657/507);for(const G of i){const F=u[G];if(!F)continue;const q=n[G],N=F.x*y;F.y*_;const K=Math.round(.03*_+F.y*(.85*_)),Q=Math.round(N-g/2),re=Math.round(K-f/2);if(q){const le=G.replace(/\d+/,""),te=c&&(c.club_id&&String(q.club_id)===String(c.club_id)||c.country_code&&q.country_code===c.country_code),H=nt({...q,_evolution_bonus:q._evolution_bonus||0},{width:g,showStad:!0,stadDef:c,role:le});T+=`<div style="position:absolute;left:${Q}px;top:${re}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${G}">
      ...
      </div>`}else{const le=G.replace(/\d+/,"");T+=`<div style="position:absolute;left:${Q}px;top:${re}px;width:${g}px;height:${f}px;
        border:2px dashed rgba(255,255,255,0.35);border-radius:6px;
        display:flex;flex-direction:column;align-items:center;justify-content:center;
        cursor:pointer;z-index:2;background:rgba(255,255,255,0.04)"
        class="deck-slot-hit" data-pos="${G}">
    ...
    <div style="position:relative;width:${y}px;height:${_}px;margin:0 auto">
      ...
      ${T}
    </div>`,a.querySelectorAll(".deck-slot-hit").forEach(G=>{G.addEventListener("click",()=>Qo(G.dataset.pos,t,e,o))})}function Xo(e,t,i){var r;const{openModal:o,closeModal:d}=i,a=e.stadiumCards||[];o("🏟️ Choisir un stade",`<div style="display:flex;flex-wrap:wrap;gap:10px;padding:8px">
    ...
    </div>`),(r=document.getElementById("stad-none"))==null||r.addEventListener("click",()=>{e.stadiumCardId=null,d(),ht(t,e,i)}),document.querySelectorAll(".stad-choice").forEach(c=>{c.addEventListener("click",()=>{e.stadiumCardId=c.dataset.stadId,d(),ht(t,e,i)})})}function Qo(e,t,i,o){var _,g,l,T,f;const{openModal:d,closeModal:a}=o,r=e.replace(/\d+/,""),c=(_=t.stadiumCards)==null?void 0:_.find(L=>L.id===t.stadiumCardId),u=c&&((g=t.stadDefMap)==null?void 0:g[c.stadium_id])||null,s=t.slots[e],n=s?t.playerCards.find(L=>L.id===s):null;(l=n==null?void 0:n.player)==null||l.id;const h=new Set;Object.entries(t.slots).forEach(([L,j])=>{var F;if(L===e||!j)return;const G=t.playerCards.find(q=>q.id===j);(F=G==null?void 0:G.player)!=null&&F.id&&h.add(G.player.id)}),t.subs.forEach(L=>{var G;const j=t.playerCards.find(F=>F.id===L);(G=j==null?void 0:j.player)!=null&&G.id&&h.add(j.player.id)});const m=new Set,y=t.playerCards.filter(L=>{const j=L.player;return!(j.job===r||j.job2===r)||h.has(j.id)||m.has(j.id)?!1:(m.add(j.id),!0)});y.sort((L,j)=>{const G=L.evolution_bonus||0,F=j.evolution_bonus||0,q=(r==="GK"?L.player.note_g:r==="DEF"?L.player.note_d:r==="MIL"?L.player.note_m:L.player.note_a)+(r===L.player.job||r===L.player.job2?G:0);return(r==="GK"?j.player.note_g:r==="DEF"?j.player.note_d:r==="MIL"?j.player.note_m:j.player.note_a)+(r===j.player.job||r===j.player.job2?F:0)-q}),d(`Choisir ${r} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      card:cards(id,card_type,formation,stadium_id,evolution_bonus,
        player:players(id,firstname,surname_real,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
          clubs(encoded_name,logo_url)))`).in("deck_id",r).order("slot_order"),u=[...new Set((c||[]).filter(m=>{var y,_;return((y=m.card)==null?void 0:y.card_type)==="stadium"&&((_=m.card)==null?void 0:_.stadium_id)}).map(m=>m.card.stadium_id))],s={};if(u.length){const{data:m}=await S.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",u);(m||[]).forEach(y=>{s[y.id]=y}),(c||[]).forEach(y=>{var _,g;((_=y.card)==null?void 0:_.card_type)==="stadium"&&((g=y.card)!=null&&g.stadium_id)&&(y.card._stadiumDef=s[y.card.stadium_id]||null)})}let n=0;function h(){var G,F,q,N,K,Q,re;const m=a[n],y=(c||[]).filter(le=>le.deck_id===m.id),_=y.filter(le=>{var te;return le.is_starter&&((te=le.card)==null?void 0:te.player)}).map(le=>wi(le.card,le.position)),g=y.find(le=>{var te;return((te=le.card)==null?void 0:te.card_type)==="formation"}),l=m.formation||((G=g==null?void 0:g.card)==null?void 0:G.formation)||"4-4-2";let T=_.length>=11?wt(_,l):null,f=((F=m.stadium_card)==null?void 0:F.stadium_def)||null;f&&T&&(T=St(T,f));const L=_.length>=11;Ze(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
  ...
  <div style="text-align:center;flex-shrink:0;width:38px">
    ...
    <div style="font-size:7px;color:rgba(255,255,255,0.5);margin-top:1px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${(e.name||"").slice(0,7)}</div>
          ...
          card:cards(id, card_type, formation, evolution_bonus,
            player:players(id,firstname,surname_real,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
              clubs(encoded_name,logo_url)))`).eq("deck_id",u).order("slot_order")]);s=q[0].data,h=q[0].error,n=q[1].data,m=q[1].error}catch(q){console.error("[Match] Exception chargement deck:",q),Tt(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>a("home"));return}if(h||m){console.error("[Match] Erreur Supabase:",h||m),Tt(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>a("home"));return}const y=(n||[]).filter(q=>{var N;return q.is_starter&&((N=q.card)==null?void 0:N.player)}).map(q=>wi(q.card,q.position)),_=(n||[]).filter(q=>{var N;return!q.is_starter&&((N=q.card)==null?void 0:N.player)}).map(q=>wi(q.card,q.position));if(y.length<11){Tt(e,"⚠️",`Deck incomplet (${y.length}/11).`,"Compléter",()=>a("decks"));return}const g=(n||[]).find(q=>{var N;return((N=q.card)==null?void 0:N.card_type)==="formation"}),l=(s==null?void 0:s.formation)||((F=g==null?void 0:g.card)==null?void 0:F.formation)||"4-4-2",{data:T,error:f}=await S.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",d.profile.id).eq("card_type","game_changer"),{data:L}=await S.from("gc_definitions").select("*").eq("is_active",!0),j=(T||[]).map(q=>({...q,_gcDef:(L==null?void 0:L.find(N=>N.name===q.gc_type||N.id===q.gc_definition_id))||null}));let G=null;if(s!=null&&s.stadium_card_id){const{data:q}=await S.from("cards").select("stadium_id").eq("id",s.stadium_card_id).single();if(q!=null&&q.stadium_id){const{data:N}=await S.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").eq("id",q.stadium_id).single();G=N||null}}o({deckId:u,formation:l,starters:y,subsRaw:_,gcCardsEnriched:j,gcDefs:L||[],stadiumDef:G})}async function yn(e,t){const{state:i}=t,d=(i.params||{}).matchMode||"vs_ai_easy",a=d.replace("vs_ai_",""),r=d;await ai(e,t,d,async({deckId:c,formation:u,starters:s,subsRaw:n,gcCardsEnriched:h,gcDefs:m,stadiumDef:y})=>{try{let _=wt(s,u);y&&(_=St(_,y),Ct(n,y));const g=await vn(u,a),l=g.lines||g,T=async f=>{try{const{data:L,error:j}=await S.from("matches").insert({home_id:i.profile.id,away_id:null,mode:r,home_deck_id:c,status:"in_progress"}).select().single();if(j){console.error("[MatchIA] Erreur création match:",j),Tt(e,"⚠️","Impossible de créer le match ("+j.message+").","Retour",()=>t.navigate("home"));return}const G=g.stadiumDef||null;G&&l&&(St(l,G),Ct(g.subs||[],G));const F={gcDefs:m||[],matchId:L==null?void 0:L.id,mode:r,difficulty:a,formation:u,homeTeam:_,aiTeam:l,homeSubs:n,subsUsed:0,maxSubs:Math.min(n.length,3),aiSubs:g.subs||[],aiSubsUsed:0,aiMaxSubs:Math.min((g.subs||[]).length,3),aiUsedSubIds:[],aiGcCards:g.gcCards||[],aiUsedGc:[],aiStadiumDef:G,homeScore:0,aiScore:0,gcCards:f,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};wn(e,F,t)}catch(L){console.error("[MatchIA] Exception launchMatch:",L),Tt(e,"⚠️","Erreur au lancement du match : "+L.message,"Retour",()=>t.navigate("home"))}};if(!h.length){T([]);return}ri(e,h,T)}catch(_){console.error("[MatchIA] Exception setup:",_),Tt(e,"⚠️","Erreur de préparation du match : "+_.message,"Retour",()=>t.navigate("home"))}})}async function vn(e,t){var l;const{data:i}=await S.from("players").select("id,firstname,surname_real,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(80);if(!i||i.length<11)return{lines:hn(e),subs:[],gcCards:[],stadiumDef:null};const o=jt[e]||jt["4-4-2"],d={GK:[],DEF:[],MIL:[],ATT:[]},a=new Set;function r(T,f,L){var j,G;return a.add(T.id),{cardId:"ai-"+T.id+"-"+L,id:T.id,firstname:T.firstname,name:T.surname_real,country_code:T.country_code,club_id:T.club_id,job:T.job,job2:T.job2,note_g:Number(T.note_g)||0,note_d:Number(T.note_d)||0,note_m:Number(T.note_m)||0,note_a:Number(T.note_a)||0,rarity:T.rarity,skin:T.skin,hair:T.hair,hair_length:T.hair_length,clubName:((j=T.clubs)==null?void 0:j.encoded_name)||null,clubLogo:((G=T.clubs)==null?void 0:G.logo_url)||null,boost:0,used:!1,_line:f}}for(const T of["GK","DEF","MIL","ATT"]){const f=i.filter(q=>q.job===T&&!a.has(q.id)),L=i.filter(q=>q.job!==T&&!a.has(q.id)),j=[...f,...L],G=[];for(let q=0;q<o[T];q++){const N=j[q];N&&G.push(r(N,T,q))}const F=dt(G.length);G.forEach((q,N)=>{q._col=F[N]}),d[T]=G}const u=i.filter(T=>!a.has(T.id)).slice(0,5).map((T,f)=>r(T,T.job,100+f)),h=Object.keys(Re).sort(()=>Math.random()-.5).slice(0,3).map((T,f)=>{var L,j;return{id:"ai-gc-"+f,gc_type:T,name:((L=Re[T])==null?void 0:L.name)||T,icon:((j=Re[T])==null?void 0:j.icon)||"⚡"}}),m=Object.values(d).flat(),y={};m.forEach(T=>{T.club_id&&(y[T.club_id]=(y[T.club_id]||0)+1)});const _=(l=Object.entries(y).sort((T,f)=>f[1]-T[1])[0])==null?void 0:l[0];let g=null;if(_){const{data:T}=await S.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",_).single();T&&(g={club_id:T.id,country_code:null,name:T.encoded_name+" Stadium",club:{encoded_name:T.encoded_name,logo_url:T.logo_url}})}return{lines:d,subs:u,gcCards:h,stadiumDef:g}}function hn(e){const t=jt[e]||jt["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},o=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let d=0;for(const a of["GK","DEF","MIL","ATT"]){const r=[];for(let u=0;u<t[a];u++){const s=3+Math.floor(Math.random()*5);r.push({cardId:"fake-"+d,id:"fake-"+d,firstname:"IA",name:o[d%o.length],country_code:"XX",club_id:null,job:a,job2:null,note_g:a==="GK"?s:2,note_d:a==="DEF"?s:2,note_m:a==="MIL"?s:2,note_a:a==="ATT"?s:2,rarity:"normal",boost:0,used:!1,_line:a}),d++}const c=dt(r.length);r.forEach((u,s)=>{u._col=c[s]}),i[a]=r}return i}function wn(e,t,i){var o;e.innerHTML=`
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
  </div>`,setTimeout(()=>_n(e,t,i),5e3)}function vt(e){var t,i;return{name:e.name,firstname:e.firstname||"",note_g:e.note_g||0,note_d:e.note_d||0,note_m:e.note_m||0,note_a:e.note_a||0,_evolution_bonus:0,stadiumBonus:e.stadiumBonus||!1,boost:e.boost||0,job:e.job,job2:e.job2||null,_line:e._line||e.job,_col:e._col,country_code:e.country_code,club_id:e.club_id,rarity:e.rarity,clubName:e.clubName||((t=e.clubs)==null?void 0:t.encoded_name)||null,clubLogo:e.clubLogo||((i=e.clubs)==null?void 0:i.logo_url)||null,face:e.face||null,portrait:it(e)}}function _n(e,t,i){const o=t.homeTeam.MIL||[],d=t.aiTeam.MIL||[],a=t.stadiumDef||null,r=t.aiStadiumDef||null;function c(f,L){const j=He(f,"MIL"),G=f.evolution_bonus||f._evolution_bonus||0,F=f.job==="MIL"||f.job2==="MIL"?G:0,q=f.stadiumBonus||L&&(L.club_id&&String(f.club_id)===String(L.club_id)||L.country_code&&f.country_code===L.country_code)?10:0;return j+F+q}function u(f,L){return f.reduce((j,G)=>j+c(G,L),0)}function s(f){let L=0;for(let j=0;j<f.length-1;j++){const G=rt(f[j],f[j+1]);G==="#00ff88"?L+=2:G==="#FFD700"&&(L+=1)}return L}const n=u(o,a)+s(o),h=u(d,r)+s(d),m=n>=h;function y(f,L,j,G,F){return`<div id="duel-row-${G}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0), opacity .5s ease;transform-origin:center">
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${f.map((q,N)=>{const K=N<f.length-1?rt(q,f[N+1]):null,Q=!K||K==="#ff3333"||K==="#cc2222",re=K==="#00ff88"?"+2":K==="#FFD700"?"+1":"";c(q,F),q.stadiumBonus||F&&(F.club_id&&String(q.club_id)===String(F.club_id)||F.country_code&&(q.country_code,F.country_code));const le=q.evolution_bonus||q._evolution_bonus||0;return`
          <div class="duel-card duel-card-${G}" data-idx="${N}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease, transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
            ${Je({...q,_evolution_bonus:le},58,78,F)}
          </div>
          ${N<f.length-1?`<div class="duel-link duel-link-${G}" data-idx="${N}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${Q?"rgba(255,255,255,0.12)":K};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${Q?"none":`0 0 8px ${K}`}">
            ${re?`<span style="position:absolute;top:-13px;left:50%;transform:translateX(-50%);font-size:8px;font-weight:900;color:${K}">${re}</span>`:""}
          </div>`:""}
          `}).join("")}
      </div>
      <div class="duel-score-line duel-score-line-${G}" style="margin-top:10px;font-size:12px;color:rgba(255,255,255,0.55);opacity:0;transition:opacity .4s ease">
        Score: ${u(f,F)} + ${s(f)} liens = <b style="color:#fff">${u(f,F)+s(f)}</b>
      </div>
    </div>`}e.innerHTML=`
  ...
  </div>`;const _=()=>{const f=(L,j)=>e.querySelectorAll(L).forEach((G,F)=>{setTimeout(()=>{G.style.opacity="1",G.style.transform="translateY(0) scale(1)"},j+F*90)});f(".duel-card-home",150),f(".duel-card-ai",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((L,j)=>{setTimeout(()=>{L.style.opacity="1"},j*70)}),900),setTimeout(()=>{const L=e.querySelector("#vs-label");L&&(L.style.opacity="1",L.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(j=>j.style.opacity="1")},1250),setTimeout(()=>{g("score-home",n,800),g("score-ai",h,800)},1500)};function g(f,L,j){const G=document.getElementById(f);if(!G)return;const F=performance.now(),q=N=>{const K=Math.min(1,(N-F)/j);G.textContent=Math.round(L*(1-Math.pow(1-K,3))),K<1?requestAnimationFrame(q):G.textContent=L};requestAnimationFrame(q)}requestAnimationFrame(_),t.attacker=m?"home":"ai";const l=m?ni():null;m&&(t.boostCard={value:l}),t.log.push({type:"duel",title:"Milieu de Terrain",homePlayers:o.map(f=>vt(f)),aiPlayers:d.map(f=>vt(f)),homeTotal:n,aiTotal:h,text:`Duel milieu : ${t.clubName} ${n} – ${h} IA → ${m?t.clubName+" attaque":"IA attaque"}`});const T=()=>{t.phase=t.attacker==="home"?"attack":"ai-attack",We(e,t,i),t.attacker==="ai"&&setTimeout(()=>ei(e,t,i),800)};setTimeout(()=>{const f=document.getElementById("score-home"),L=document.getElementById("score-ai"),j=document.getElementById(m?"duel-row-home":"duel-row-ai"),G=document.getElementById(m?"duel-row-ai":"duel-row-home"),F=m?f:L,q=m?L:f;F&&(F.style.fontSize="80px",F.style.color=m?"#FFD700":"#ff6b6b",F.style.animation="duelPulse .5s ease"+(m?", duelGlow 1.5s ease infinite .5s":"")),q&&(q.style.opacity="0.25"),setTimeout(()=>{j&&(j.style.transformOrigin="center",j.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",j.style.zIndex="5"),setTimeout(()=>{var K;const N=document.getElementById("duel-shock");if(N){const Q=(K=G||j)==null?void 0:K.getBoundingClientRect(),re=e.querySelector(".match-screen").getBoundingClientRect();Q&&(N.style.top=Q.top-re.top+Q.height/2+"px"),N.style.animation="shockwave .5s ease-out forwards"}G&&(G.style.transformOrigin="center",G.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var K;const N=document.getElementById("duel-finale");N&&(N.innerHTML=`
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(a);let c=!1;const u=()=>{c||(c=!0,a.remove(),setTimeout(()=>d(),50))};a.addEventListener("click",u),setTimeout(u,3500)}async function Ft(e,t,i){var n,h;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:o}=i,d=t.homeScore>t.aiScore,a=t.homeScore===t.aiScore,r=d?"victoire":a?"nul":"defaite",c=Fo(t.mode,r);t.matchId&&await S.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:d?o.profile.id:null,home_credits_reward:c,played_at:new Date().toISOString()}).eq("id",t.matchId);const u={credits:(o.profile.credits||0)+c,matches_played:(o.profile.matches_played||0)+1};d?u.wins=(o.profile.wins||0)+1:a?u.draws=(o.profile.draws||0)+1:u.losses=(o.profile.losses||0)+1,await S.from("users").update(u).eq("id",o.profile.id),await i.refreshProfile();const s=document.createElement("div");s.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",s.innerHTML=`
    ...
    </div>`,`<button class="btn btn-primary" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}function fi(e){var t,i;return{name:e.name,firstname:e.firstname||"",note_g:e.note_g||0,note_d:e.note_d||0,note_m:e.note_m||0,note_a:e.note_a||0,_evolution_bonus:0,stadiumBonus:e.stadiumBonus||!1,boost:e.boost||0,job:e.job,job2:e.job2||null,_line:e._line||e.job,_col:e._col,country_code:e.country_code,club_id:e.club_id,rarity:e.rarity,clubName:e.clubName||((t=e.clubs)==null?void 0:t.encoded_name)||null,clubLogo:e.clubLogo||((i=e.clubs)==null?void 0:i.logo_url)||null,face:e.face||null,portrait:it(e)}}async function _i(e,t,i=!1){var d,a,r,c;try{const u=(a=(d=t==null?void 0:t.state)==null?void 0:d.profile)==null?void 0:a.id;try{(S.getChannels?S.getChannels():[]).forEach(n=>{const h=n.topic||"";(h.includes("matchmaking")||h.includes("pvp-match")||h.includes("friend-invite"))&&S.removeChannel(n)})}catch(s){console.warn("[Random] cleanup canaux:",s)}u&&await S.rpc("cancel_matchmaking",{p_user_id:u}).catch(()=>{})}catch{}const o=i&&((c=(r=t==null?void 0:t.state)==null?void 0:r.params)==null?void 0:c.rankedData)||null;await ai(e,t,"random",({deckId:u,formation:s,starters:n,subsRaw:h,gcCardsEnriched:m,gcDefs:y,stadiumDef:_})=>{const g=l=>Bn(e,t,u,s,n,h,l,y||[],_,i,o);if(!m.length){g([]);return}ri(e,m,g)})}async function Bn(e,t,i,o,d,a,r=[],c=[],u=null,s=!1,n=null){var F,q;const{state:h,navigate:m,toast:y}=t;let _=!1,g=null;Ze(e);const l=(n==null?void 0:n.mmr)??((F=h.profile)==null?void 0:F.mmr)??1e3,T=s?Pt(l):null,f=s?"linear-gradient(180deg,#1a0030,#0a1628)":"linear-gradient(180deg,#0a1628,#1a0a2e)",L=s?(T==null?void 0:T.color)||"#D4A017":"#FFD700";e.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div id="mkt-content" style="padding:10px 16px;display:flex;flex-direction:column;gap:8px"></div>
  </div>`;let u="buy";const s=()=>{var g,l,T,f,L,j,G;return{name:(((g=document.getElementById("flt-name"))==null?void 0:g.value)||"").toLowerCase().trim(),club:(((l=document.getElementById("flt-club"))==null?void 0:l.value)||"").toLowerCase().trim(),country:(((T=document.getElementById("flt-country"))==null?void 0:T.value)||"").toLowerCase().trim(),job:((f=document.getElementById("flt-job"))==null?void 0:f.value)||"",rarity:((L=document.getElementById("flt-rarity"))==null?void 0:L.value)||"",note1:parseInt((j=document.getElementById("flt-note1"))==null?void 0:j.value)||0,note2:parseInt((G=document.getElementById("flt-note2"))==null?void 0:G.value)||0}};function n(g){const l=s();return g.filter(T=>{var K,Q,re;const f=(K=T.card)==null?void 0:K.player;if(!f)return!1;const L=`${f.firstname} ${f.surname_real}`.toLowerCase(),j=(((Q=f.clubs)==null?void 0:Q.encoded_name)||"").toLowerCase(),G=(f.country_code||"").toLowerCase(),F=((re=T.card)==null?void 0:re.evolution_bonus)||0,q=At(f,f.job,F),N=f.job2?At(f,f.job2,F):0;return!(l.name&&!L.includes(l.name)||l.club&&!j.includes(l.club)||l.country&&!G.includes(l.country)||l.job&&f.job!==l.job||l.rarity&&f.rarity!==l.rarity||l.note1&&q<l.note1||l.note2&&N<l.note2)})}function h(g){var N,K,Q,re;const l=(N=g.card)==null?void 0:N.player;if(!l)return"";const T=((K=g.card)==null?void 0:K.evolution_bonus)||0,f=At(l,l.job,T),L=l.job2?At(l,l.job2,T):0,j=(i.profile.credits||0)>=g.price,G=l.country_code?`https://flagsapi.com/${l.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,F=Jt[l.job]||"#bbb",q=l.job2?Jt[l.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden">
      <!-- Drapeau -->
      ...
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${F};display:flex;align-items:center;justify-content:center">
          <span style="font-size:14px;font-weight:900;color:${F};font-family:Arial Black,Arial">${f}</span>
        </div>
    ...
    </div>`}function m(g){var N,K,Q,re;const l=(N=g.card)==null?void 0:N.player;if(!l)return"";const T=((K=g.card)==null?void 0:K.evolution_bonus)||0,f=At(l,l.job,T),L=l.job2?At(l,l.job2,T):0,j=g.status==="sold",G=l.country_code?`https://flagsapi.com/${l.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,F=Jt[l.job]||"#bbb",q=l.job2?Jt[l.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${j?"opacity:0.7":""}">
      ${G?`<img src="${G}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      ${(Q=l.clubs)!=null&&Q.logo_url?`<img src="${l.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${F};display:flex;align-items:center;justify-content:center">
    ...
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center">
      ...
      `).join(""):'<div style="text-align:center;color:var(--gray-600);padding:40px">Aucun manager.</div>'}else{const T=r||[];_.innerHTML=T.length>0?T.map((f,L)=>{const j=Pt(f.mmr??1e3),G=(f.ranked_wins||0)+(f.ranked_losses||0)+(f.ranked_draws||0),F=G>0?Math.round((f.ranked_wins||0)/G*100):0,q=f.id===i.profile.id,N=(f.placement_matches||0)<10;return`
  ...
  <div class="page">
    ...
    </div>`,(F=document.getElementById("ranked-back"))==null||F.addEventListener("click",()=>o("home"));return}const u=c.mmr??1e3,s=c.mmr_deviation??350,n=c.mmr_volatility??.06,h=c.placement_matches??0,m=h<10,y=Math.max(0,10-h),_=Pt(u),g=vo(u),l=di.findIndex(Q=>Q.id===_.id),T=di[l+1]||null,f={bronze:"linear-gradient(160deg,#3d1c00,#7a3e00)",silver:"linear-gradient(160deg,#1a1a2e,#3a3a5e)",gold:"linear-gradient(160deg,#1a1200,#4a3500)",platinum:"linear-gradient(160deg,#001a20,#003040)",diamond:"linear-gradient(160deg,#001030,#1a2860)",master:"linear-gradient(160deg,#1a0030,#3d0070)"},L=(c.ranked_wins||0)+(c.ranked_losses||0)+(c.ranked_draws||0),j=L>0?Math.round((c.ranked_wins||0)/L*100):0,G=di.map(Q=>`
