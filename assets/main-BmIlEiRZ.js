...
import{s as w,l as Ht,m as Ci,F as Bi,r as Ae,j as ei,h as qi,n as On,o as Hn,T as yi,k as Un,p as Kn,b as Vn}from"./formation-links-CZYKSMms.js";const Yn="/";function Ji(e,{navigate:t,toast:i}){let n="login";const r=()=>{var a,s,l,c,d,x;const o=n==="login";e.innerHTML=`
    <div style="
  ...
  `;function s(){var m;const c=document.getElementById("logo-preview"),d=document.getElementById("logo-initials"),x=((m=document.getElementById("setup-club"))==null?void 0:m.value)||"MW",y=x.trim().split(" ").filter(Boolean),g=y.length>=2?(y[0][0]+y[1][0]).toUpperCase():x.slice(0,2).toUpperCase();c&&(c.style.background=a,c.style.borderColor=o),d&&(d.textContent=g,d.style.color=o)}document.getElementById("color1").addEventListener("input",c=>{o=c.target.value,document.getElementById("swatch1").style.background=o,s()}),document.getElementById("color2").addEventListener("input",c=>{a=c.target.value,document.getElementById("swatch2").style.background=a,s()});function l(c){document.querySelectorAll(".setup-step").forEach(d=>d.classList.remove("active")),document.getElementById(`step-${c}`).classList.add("active"),document.getElementById("step-num").textContent=c,document.getElementById("progress-fill").style.width=`${Math.round(c/3*100)}%`,c===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),d=document.getElementById("step1-error");if(d.textContent="",c.length<3){d.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:x}=await w.from("users").select("id").eq("pseudo",c).maybeSingle();if(x){d.textContent="Ce pseudo est déjà pris.";return}l(2)}),document.getElementById("step2-back").addEventListener("click",()=>l(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const c=document.getElementById("setup-club").value.trim(),d=document.getElementById("step2-error");if(d.textContent="",c.length<2){d.textContent="Nom trop court (min. 2 caractères).";return}const{data:x}=await w.from("users").select("id").eq("club_name",c).maybeSingle();if(x){d.textContent="Ce nom de club est déjà pris.";return}l(3)}),document.getElementById("step3-back").addEventListener("click",()=>l(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),d=document.getElementById("setup-club").value.trim(),x=document.getElementById("step3-error"),y=document.getElementById("step3-finish");x.textContent="",y.disabled=!0,y.textContent="Création en cours…";try{const{error:g}=await w.from("users").insert({id:t.user.id,pseudo:c,club_name:d,club_color1:o,club_color2:a,credits:1e4});if(g)throw g;await Xn(t.user.id),await r(),n(`Bienvenue ${c} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(g){x.textContent=g.message,y.disabled=!1,y.textContent="🚀 Créer mon profil !"}})}async function Xn(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await w.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const Jn="modulepreload",Qn=function(e){return"/"+e},Qi={},gn=function(t,i,n){let r=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),s=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));r=Promise.allSettled(i.map(l=>{if(l=Qn(l),l in Qi)return;Qi[l]=!0;const c=l.endsWith(".css"),d=c?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${l}"]${d}`))return;const x=document.createElement("link");if(x.rel=c?"stylesheet":Jn,c||(x.as="script"),x.crossOrigin="",x.href=l,s&&x.setAttribute("nonce",s),document.head.appendChild(x),c)return new Promise((y,g)=>{x.addEventListener("load",y),x.addEventListener("error",()=>g(new Error(`Unable to preload CSS for ${l}`)))})}))}function o(a){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=a,window.dispatchEvent(s),!s.defaultPrevented)throw a}return r.then(a=>{for(const s of a||[])s.status==="rejected"&&o(s.reason);return t().catch(o)})},Yt="#1A6B3C",Wt="#cc2222",Zn="#D4A017",Zi="#888";async function eo(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      ${y.map(({friendshipId:g,friend:m})=>to(m,g)).join("")}
    </div>`,c.querySelectorAll("[data-stats]").forEach(g=>{g.addEventListener("click",()=>no(e,g.dataset.stats,g.dataset.friendName))}),c.querySelectorAll("[data-match]").forEach(g=>{g.addEventListener("click",async()=>{const m=g.dataset.friendId,p=g.dataset.friendName;if(typeof n!="function"){t("Erreur navigation","error");return}const u=e.user.id,{data:b}=await w.from("friend_match_invites").select("id").eq("inviter_id",m).eq("invitee_id",u).eq("status","pending").order("created_at",{ascending:!1}).limit(1).maybeSingle(),v=!!b;console.log("[Friends] clic match",{fid:m,fname:p,isAccepting:v}),n("match",{matchMode:"friend",friendId:m,friendName:p,isAccepting:v})})})}function to(e,t){const i=e.club_name||e.pseudo||"?",n=e.pseudo||"",r=(n||i).slice(0,2).toUpperCase(),o=e.club_color2||Yt,a=e.club_color1||"#ffffff",s=e.last_seen_at?new Date(e.last_seen_at):null,l=s&&Date.now()-s.getTime()<3*60*1e3;return`
    ...
    </div>`}function io(e,t){const i=Di();i.innerHTML=`
    <div class="popup-box">
    ...
    </style>`}const oo="2026.07.19-1843";async function en(e,{state:t,navigate:i,toast:n}){var o,a;const r=t.profile;r&&(e.innerHTML=`
  <style>
    .home-dark {
      min-height: 100%;
  ...
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
        ...
        </div>`},null)}e.querySelectorAll(".col-tab-btn").forEach(A=>{A.addEventListener("click",()=>{M=A.dataset.tab,k="Tous",P="",H=!1,e.querySelectorAll(".col-tab-btn").forEach(D=>{const Y=D.dataset.tab===M;D.style.borderBottomColor=Y?"var(--green)":"transparent",D.style.color=Y?"var(--green)":"var(--gray-600)"}),L(),V()})}),L(),V()}function Eo(e,t,i){const n=Ce[e]||{icon:"⚡",desc:"Effet spécial."},r=(t==null?void 0:t.gc_type)==="ultra_game_changer",o={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},a={purple:"#b06ce0",light_blue:"#00d4ef"},s=o[t==null?void 0:t.color]||o.purple,l=a[t==null?void 0:t.color]||a.purple,c=(t==null?void 0:t.name)||e,d=(t==null?void 0:t.effect)||n.desc,x=t!=null&&t.image_url?`/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(ge=document.getElementById("close-detail"))==null||ge.addEventListener("click",s);let V=new Set;const ie=()=>{const G=V.size,K=document.getElementById("sell-action-panel");if(!K)return;K.style.display=G>0?"block":"none",document.getElementById("sell-selected-count").textContent=G;const N=document.getElementById("evolve-btn");N&&(N.textContent=`⬆️ Évoluer ${G>1?"(+"+G+")":""}`)};document.querySelectorAll(".expl-check").forEach(G=>{G.addEventListener("change",()=>{const K=G.dataset.id;G.checked?V.add(K):V.delete(K);const N=G.closest(".exemplaire-row");N&&(N.style.borderColor=G.checked?"#1A6B3C":"#eee"),ie()})}),document.querySelectorAll(".exemplaire-row").forEach(G=>{G.addEventListener("click",K=>{if(K.target.closest("button")||K.target.tagName==="INPUT")return;const N=G.querySelector(".expl-check");N&&!N.disabled&&(N.checked=!N.checked,N.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(G=>{G.addEventListener("click",K=>{K.stopPropagation();const N=document.querySelector(`.expl-hist[data-hist="${G.dataset.idx}"]`);N&&(N.style.display=N.style.display==="none"?"flex":"none")})}),(I=document.getElementById("evolve-btn"))==null||I.addEventListener("click",async()=>{if(y<=1)return;const G=[...V];if(!G.length)return;if(V.has(e.id)){const R=document.createElement("div");R.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",R.innerHTML=`
  ...
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
  ...
  </div>`;const d=window.innerWidth>=900,x=e.querySelector(".deck-pc-layout"),y=e.querySelector(".deck-mobile-layout");x&&(x.style.display=d?"block":"none"),y&&(y.style.display=d?"none":"block"),zo(e,t,r,i),e.querySelectorAll("#builder-back").forEach(u=>u.addEventListener("click",()=>n("decks")));const g=()=>{const{openModal:u,closeModal:b}=i,v=`<div style="display:flex;flex-wrap:wrap;gap:8px;padding:8px">
    ...
    <div style="position:relative;width:${g}px;height:${m}px;margin:0 auto">
      ...
      ${b}
    </div>`,o.querySelectorAll(".deck-slot-hit").forEach(f=>{f.addEventListener("click",()=>So(f.dataset.pos,t,e,n))})}function Io(e,t,i){var a;const{openModal:n,closeModal:r}=i,o=e.stadiumCards||[];n("🏟️ Choisir un stade",`<div style="display:flex;flex-wrap:wrap;gap:10px;padding:8px">
  ...
  `,document.body.appendChild(r);let o=!1;const a=document.getElementById("pack-cut-zone"),s=document.getElementById("pack-blade");let l=!1;const c=L=>L.touches&&L.touches[0]?L.touches[0].clientX:L.clientX;function d(L){o||(l=!0,s.style.opacity="1",x(L))}function x(L){if(!l||o)return;const V=a.getBoundingClientRect(),ie=c(L)-V.left,ue=Math.max(0,Math.min(1,ie/V.width));s.style.width=ue*V.width+"px",ue>=.82&&g()}function y(){o||(l=!1,s.style.transition="width .2s ease, opacity .2s ease",s.style.width="0",s.style.opacity="0",setTimeout(()=>{o||(s.style.transition="")},220))}function g(){var V;if(o)return;o=!0,l=!1,s.style.width="100%",s.style.opacity="1",(V=document.getElementById("cut-flash"))==null||V.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const L=document.getElementById("cut-hint");L&&(L.style.opacity="0"),a.classList.add("pack-cut"),setTimeout(()=>{s.style.opacity="0",document.getElementById("pack-phase").style.display="none",b(0)},620)}a.addEventListener("pointerdown",d),window.addEventListener("pointermove",x),window.addEventListener("pointerup",y),a.addEventListener("touchstart",d,{passive:!0}),window.addEventListener("touchmove",x,{passive:!0}),window.addEventListener("touchend",y);let m=0,p=!1;const u=new Set;function b(L){m=L,document.getElementById("reveal-phase").style.display="flex",v(),h(L,0),P()}function v(){const L=document.getElementById("card-dots");L&&(L.innerHTML=e.map((V,ie)=>`<div class="card-dot" data-i="${ie}" style="width:8px;height:8px;border-radius:50%;background:${ie===m?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),L.querySelectorAll(".card-dot").forEach(V=>V.addEventListener("click",()=>f(parseInt(V.dataset.i)))))}function h(L,V){var N,A;const ie=e[L],ue=document.getElementById("card-counter"),ge=document.getElementById("card-track");ue&&(ue.textContent=`Carte ${L+1} / ${e.length}`);const I=document.getElementById("reveal-btns");I&&(I.style.display=L===e.length-1?"flex":"none");const W=ie.card_type==="player"&&((N=ie.player)==null?void 0:N.rarity)==="legende",pe=!u.has(L);u.add(L);let G=0;if(ie.card_type==="player"&&ie.player){const D=ie.player,Y=D.job||"ATT";G=Number(Y==="GK"?D.note_g:Y==="DEF"?D.note_d:Y==="MIL"?D.note_m:D.note_a)||0}const K=D=>{ge.innerHTML=`
        <div id="current-card-wrap" style="position:relative;display:flex;flex-direction:column;align-items:center;gap:8px;${W?"filter:drop-shadow(0 0 20px #7a28b8)":""}">
          <div style="transform:scale(1.25);transform-origin:center">${Ro(ie)}</div>
  ...
  </div>`;const d=async()=>{await w.from("users").update({pending_boosters:a}).eq("id",t.user.id)};async function x(){var v;if(c>=l||!a.length){await w.from("users").update({pending_boosters:[],onboarding_done:!0}).eq("id",t.user.id),r&&await r(),n("Tous tes boosters sont ouverts ! Bon jeu 🎮","success",4e3),i("home");return}const m=a[0],{data:p}=await w.from("users").select("*").eq("id",t.user.id).single();p&&(t.profile=p);let u=[];try{if(m.type==="formation")u=await In(t.profile,0);else if(m.type==="game_changer")u=await zn(t.profile,m.count||3,0);else if(s&&((v=s.rates)!=null&&v.length)){const h={...s,cost:0,cardCount:m.count||s.cardCount||5};u=await Li(t.profile,h),m.guaranteeGK&&!t.profile.first_booster_opened&&(u.some(f=>f.player&&f.player.job==="GK")||await Ko(t.profile,u),await w.from("users").update({first_booster_opened:!0}).eq("id",t.profile.id))}else u=await Ln(t.profile,m.count||5,0)}catch(h){n(h.message||"Erreur ouverture booster","error");return}a.shift(),c++,await d();const b=m.type==="formation"?{name:"Booster Formation",img:"/icons/booster-formation.png"}:m.type==="game_changer"?{name:"Booster Game Changer",img:"/icons/booster-gamechanger.png"}:{name:`Booster Joueurs (${c}/${l})`,img:(s==null?void 0:s.img)||"/icons/booster-players.png"};Sn(u,b,i,()=>{x()})}(g=document.getElementById("onboard-start"))==null||g.addEventListener("click",()=>x())}async function Ko(e,t){try{const{data:i}=await w.from("players").select("id,job,firstname,surname_real,country_code,club_id,rarity,note_g,note_d,note_m,note_a,skin,hair,hair_length,face,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0).eq("job","GK");if(!(i!=null&&i.length))return;const n=i[Math.floor(Math.random()*i.length)],r=t.findIndex(a=>a.player);if(r===-1)return;const o=t[r];await w.from("cards").update({player_id:n.id}).eq("id",o.id),t[r]={...o,player_id:n.id,player:n}}catch(i){console.warn("[Onboarding] ensureGK échec",i)}}const Rt={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},Vo={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function Lt(e,t,i,n,r){var o;e.innerHTML=`<div class="match-screen" style="display:flex;align-items:center;justify-content:center;min-height:100vh">
    ...
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff;position:relative">
      ...
      <div id="deck-top-bar" style="flex-shrink:0">
        <div style="padding:8px 16px;background:rgba(0,0,0,0.4);text-align:center">
          <div style="font-size:10px;opacity:.6;letter-spacing:2px;text-transform:uppercase">${Yo(i,t)}</div>
  ...
  <div id="match-history-panel">
    ...
    </div>`,document.body.appendChild(g),(m=g.querySelector("#gc-back"))==null||m.addEventListener("click",()=>g.remove()),(p=g.querySelector("#gc-use"))==null||p.addEventListener("click",()=>{g.remove(),pr(e,t,i,n,r)})}function ni(e,t,i,n,r,o){const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let s=[];function l(){var c,d;a.innerHTML=`
    ...
    <div style="display:flex;flex-direction:column;gap:6px">
      ${n.map(r=>`
        <div class="player-boost-opt" data-card-id="${r.cardId}"
          ...
          <div style="width:32px;height:32px;background:${Vo[r.job]||"#888"};border-radius:6px;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:900;font-size:13px">${Ge(r,r._line||r.job)}</div>
          <div style="flex:1"><b>${r.firstname} ${r.name}</b><div style="font-size:11px;color:#888">${r._line||r.job}</div></div>
          <div style="color:#87CEEB;font-weight:700">+${t.boostCard.value}</div>
        </div>`).join("")}
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(r=>{r.addEventListener("click",()=>{const o=r.dataset.cardId;for(const a of["GK","DEF","MIL","ATT"]){const s=(t.homeTeam[a]||[]).find(l=>l.cardId===o);if(s){s.boost=(s.boost||0)+t.boostCard.value,t.log.push({text:`⚡ Boost +${t.boostCard.value} appliqué à ${s.name}`,type:"info"});break}}t.boostUsed=!0,i.closeModal(),$e(e,t,i)})})}async function ht(e,t,i){var d,x;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:n}=i,r=t.homeScore>t.aiScore,o=t.homeScore===t.aiScore,a=r?"victoire":o?"nul":"defaite",s=fo(t.mode,a);t.matchId&&await w.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:r?n.profile.id:null,home_credits_reward:s,played_at:new Date().toISOString()}).eq("id",t.matchId);const l={credits:(n.profile.credits||0)+s,matches_played:(n.profile.matches_played||0)+1};r?l.wins=(n.profile.wins||0)+1:o?l.draws=(n.profile.draws||0)+1:l.losses=(n.profile.losses||0)+1,await w.from("users").update(l).eq("id",n.profile.id),await i.refreshProfile();const c=document.createElement("div");c.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",c.innerHTML=`
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
    ...
    </div>`,ot=f[p+"Team"],Nn=(()=>{var fe,Se,we,jt,rt;if(q&&((fe=f.pendingAttack)!=null&&fe.players)){const De=f.pendingAttack;let Oe="";if(U.length>0){const Me=U.map(Pe=>{const ft=(ot[Pe._role]||[]).find(xi=>xi.cardId===Pe.cardId)||Pe,at=ot[Pe._role]||[],Ct=at.findIndex(xi=>xi.cardId===Pe.cardId),Bt=xt(at.length),gt=Ct>=0?Bt[Ct]:ft._col??1;return{...ft,_line:Pe._role,_col:gt}}),kt=Jt(Me,((Se=f.modifiers)==null?void 0:Se[p])||{},f[p+"Formation"]);Oe=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
      ...
      <div style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">${Nn}</div>
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;overflow-y:auto;background:#0a3d1e">
      ${Xo(f[u+"Team"],f[u+"Formation"],null,f[u+"Name"]||"Adversaire")}
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:20px;display:flex;flex-direction:column;gap:14px">
      ...
      </div>`,document.body.appendChild(t);const i=t.querySelector("#ml-pwd-input"),n=r=>{t.remove(),e(r)};setTimeout(()=>i==null?void 0:i.focus(),80),t.querySelector("#pwd-cancel").addEventListener("click",()=>n(null)),t.querySelector("#pwd-ok").addEventListener("click",()=>n(i.value.trim())),i.addEventListener("keydown",r=>{r.key==="Enter"&&n(i.value.trim())})})}async function Fn(e,t,i,n){var x;const{toast:r,state:o}=t,a=o.profile.id;let s=null;if(n==="private"&&(s=await Lr(),s===null))return;const{data:l,error:c}=await w.rpc("join_mini_league",{p_league_id:i,p_user_id:a,p_password:s});if(c||!(l!=null&&l.success)){console.error("[MiniLeague] join_mini_league:",c||l),r((l==null?void 0:l.error)||"Erreur lors de l'inscription","error");return}if(typeof t.refreshProfile=="function")await t.refreshProfile();else{const{data:y}=await w.from("users").select("credits").eq("id",a).single();o.profile&&y&&(o.profile.credits=y.credits)}const d=document.getElementById("nav-credits");d&&(d.textContent=`💰 ${(((x=o.profile)==null?void 0:x.credits)||0).toLocaleString("fr")}`),r("✅ Inscrit ! Pot mis à jour.","success"),ct(e,t,i)}async function zr(e,t,i,n){const{toast:r,state:o}=t,a=o.profile.id;if(!await new Promise(y=>{const g=document.createElement("div");g.className="modal-overlay",g.style.zIndex="2100",g.innerHTML=`<div class="modal" style="max-width:360px">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
  ...
  </div>`;const h=a.status==="waiting"?"waiting":a.status==="active"?"active":"archived";if(($=document.getElementById("ml-back"))==null||$.addEventListener("click",()=>Ze(e,t,h)),(f=document.getElementById("ml-refresh"))==null||f.addEventListener("click",async j=>{const L=j.currentTarget;L.style.opacity="0.5",await ct(e,t,i)}),(M=document.getElementById("ml-start-btn"))==null||M.addEventListener("click",()=>Sr(e,t,a,x)),(k=document.getElementById("ml-next-day"))==null||k.addEventListener("click",()=>Ar(e,t,i)),(P=document.getElementById("ml-join-now"))==null||P.addEventListener("click",()=>Fn(e,t,i,a.type)),(H=document.getElementById("ml-leave-btn"))==null||H.addEventListener("click",()=>zr(e,t,i,p)),(ae=document.getElementById("ml-delete-btn"))==null||ae.addEventListener("click",()=>Mi(e,t,i,a.name,"waiting")),(re=document.getElementById("ml-delete-now"))==null||re.addEventListener("click",()=>Mi(e,t,i,a.name,h)),(ee=document.getElementById("ml-archive-btn"))==null||ee.addEventListener("click",()=>Ir(e,t,i)),e.querySelectorAll("[data-play-match]").forEach(j=>{j.addEventListener("click",()=>{const L=g.find(V=>V.id===j.dataset.playMatch);L&&t.navigate("match-mini-league",{mlMatchId:L.id,leagueId:i})})}),a.status==="finished"&&v){const j=y.findIndex(L=>L.userId===o);j>=0&&j<3&&v.prize_amount>0&&!v.prize_claimed&&setTimeout(()=>Mr(e,t,a,v,j),400)}}function pn(e,t,i,n,r=!1){const o=x=>t.find(y=>y.id===x);if(e.is_bye){const x=o(e.home_id);return`<div style="padding:8px;border-radius:8px;background:rgba(255,255,255,0.03);margin-bottom:6px;font-size:12px;color:${st};text-align:center">🔵 ${(x==null?void 0:x.club_name)||(x==null?void 0:x.pseudo)||"?"} exempté(e)</div>`}const a=o(e.home_id),s=o(e.away_id),l=e.home_id===i||e.away_id===i,c=l&&e.status==="pending"&&n&&!r,d=e.status==="finished"?`${e.home_score} - ${e.away_score}`:"vs";return`<div style="display:flex;align-items:center;gap:8px;padding:10px;border-radius:8px;background:${l?"rgba(26,107,60,0.16)":"rgba(255,255,255,0.03)"};margin-bottom:6px;border:1px solid ${l?"rgba(74,222,128,0.35)":Tt}">
  ...
  </div>`}async function Sr(e,t,i,n){const{toast:r,state:o}=t,a=jr(n.map(c=>c.id),i.mode),s=[];a.forEach((c,d)=>c.forEach(x=>s.push({league_id:i.id,matchday:d+1,home_id:x.home||x.bye,away_id:x.away||null,is_bye:!!x.bye,status:x.bye?"bye":"pending"})));const{error:l}=await w.from("mini_league_matches").insert(s);if(l){r("Erreur calendrier : "+l.message,"error");return}await w.from("mini_leagues").update({status:"active",current_day:1,total_days:a.length}).eq("id",i.id),r(`🚀 Lancée ! Pot : ${(i.pot||0).toLocaleString("fr")} cr.`,"success"),ct(e,t,i.id)}async function Ar(e,t,i){const{data:n}=await w.from("mini_leagues").select("current_day,total_days,pot").eq("id",i).single(),r=(n.current_day||0)+1;if(r>(n.total_days||0)){ct(e,t,i);return}await w.from("mini_leagues").update({current_day:r}).eq("id",i),t.toast(`Journée ${r} commencée !`,"success"),ct(e,t,i)}async function Mr(e,t,i,n,r){var y,g;const{state:o,toast:a}=t,s=[Math.floor((i.pot||0)*.7),Math.floor((i.pot||0)*.2),Math.floor((i.pot||0)*.1)],l=["🥇","🥈","🥉"][r],c=n.prize_amount||s[r]||0,d=n.prize_claimed,x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",x.innerHTML=`
    ...
    <p style="margin-top:12px;font-size:13px;color:#888">Tu peux revoir ce tutoriel depuis les paramètres à tout moment.</p>`}];function Fr(e,t,i){let n=0;const r=document.createElement("div");r.id="tutorial-overlay",r.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.65);z-index:9900;display:flex;align-items:center;justify-content:center;padding:16px";const o=()=>{var d,x,y;const s=t[n],l=n===t.length-1,c=Math.round((n+1)/t.length*100);r.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    </div>`,document.body.appendChild(r);const o=s=>{r.remove(),s&&t()};r.querySelector("#buy-cancel").addEventListener("click",()=>o(!1)),r.querySelector("#buy-ok").addEventListener("click",()=>o(!0)),r.addEventListener("click",s=>{s.target===r&&o(!1)})}async function Or(e,t,i){const{toast:n}=i,{data:r}=await w.from("market_listings").select("card_id").eq("id",e).single();if(await w.from("market_listings").update({status:"cancelled"}).eq("id",e),r!=null&&r.card_id){const{count:o}=await w.from("market_listings").select("id",{count:"exact",head:!0}).eq("card_id",r.card_id).eq("status","active");o||await w.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",r.card_id)}n("Annonce retirée","success"),Oi(t,i)}async function Hr(e,t){var d,x,y,g;const{state:i,navigate:n}=t,r=((x=(d=t==null?void 0:t.state)==null?void 0:d.params)==null?void 0:x.tab)||"global";e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const[{data:o},{data:a}]=await Promise.all([w.from("users").select("id,pseudo,club_name,trophies_top1,trophies_top2,trophies_top3,wins,level").order("trophies_top1",{ascending:!1}).limit(100),w.from("users").select("id,pseudo,club_name,mmr,rank_tier,ranked_wins,ranked_losses,ranked_draws,placement_matches").gte("placement_matches",1).order("mmr",{ascending:!1}).limit(100)]);let s=r;function l(){var p,u;const m=document.getElementById("rankings-list");if(m){if(s==="global"){const b=o||[];m.innerHTML=b.length>0?b.map((v,h)=>`
  ...
  </div>`}Un(Vn);const be={user:null,profile:null,page:"home",params:{}};function Ft(e,t="info",i=3e3){const n=document.getElementById("toast");n&&(n.textContent=e,n.className=`show ${t}`,clearTimeout(n._t),n._t=setTimeout(()=>{n.className=""},i))}function Vr(e,t,i=""){document.getElementById("modal-title").textContent=e,document.getElementById("modal-body").innerHTML=t,document.getElementById("modal-footer").innerHTML=i,document.getElementById("modal-overlay").classList.remove("hidden")}function ji(){document.getElementById("modal-overlay").classList.add("hidden")}async function Vt(){if(!be.user)return;const{data:e}=await w.from("users").select("*").eq("id",be.user.id).single();e&&(be.profile=e)}const Pn="mw_theme";function Yr(){return localStorage.getItem(Pn)||"light"}function Wr(e){var t;localStorage.setItem(Pn,e),Xr(e),(t=be.profile)!=null&&t.id&&w.from("users").update({theme:e}).eq("id",be.profile.id).then(()=>{})}function Xr(e){document.documentElement.setAttribute("data-theme",e)}function Pt(e,t={}){be.page=e,be.params=t,Gn()}async function Gn(){var n,r,o,a;const e=document.getElementById("page-content");if(!e)return;document.querySelectorAll(".bottom-nav a").forEach(s=>{s.classList.toggle("active",s.dataset.page===be.page)});const t=document.getElementById("nav-credits");t&&be.profile&&(t.textContent=`💰 ${(be.profile.credits||0).toLocaleString("fr")}`);const i={state:be,navigate:Pt,toast:Ft,openModal:Vr,closeModal:ji,refreshProfile:Vt};switch(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽</div>',be.page){case"home":await en(e,i);break;case"settings":await $i(e,i);break;case"collection":await $o(e,i);break;case"decks":await Ei(e,i);break;case"boosters":await Do(e,i);break;case"ranked":await Ur(e,i);break;case"match":{const s=be.params&&be.params.matchMode||"vs_ai_easy";s==="random"?await Ai(e,i,!1):s==="ranked"?await Ai(e,i,!0):s==="friend"?await hr(e,i,(n=be.params)==null?void 0:n.friendId,(r=be.params)==null?void 0:r.friendName):s==="mini_league"||s==="mini-league"?await un(e,i,(o=be.params)==null?void 0:o.mlMatchId,(a=be.params)==null?void 0:a.leagueId):await Qo(e,i);break}case"market":await Gr(e,i);break;case"rankings":await Hr(e,i);break;case"matches":await Kr(e,i);break;case"friends":await eo(e,i);break;case"mini-league":await _r(e,i);break;case"match-mini-league":{const s=be.params||{};await un(e,i,s.mlMatchId,s.leagueId);break}default:await en(e,i)}}function Jr(){var n,r;const e=document.getElementById("app"),t=be.profile;if(!t)return;const i="/icons/";e.innerHTML=`
    <nav class="top-nav">
      <div class="logo" id="nav-logo" title="Manager Wars v2026.07.19-1843" style="cursor:pointer">
  ...
  </div>`,(n=e.querySelector("#pl-play-btn"))==null||n.addEventListener("click",t)}async function ea(){document.documentElement.setAttribute("data-theme","light"),document.getElementById("modal-overlay").addEventListener("click",i=>{i.target===i.currentTarget&&ji()}),document.getElementById("modal-close").addEventListener("click",ji);const{data:{session:e}}=await w.auth.getSession();if(!e){fn(),Zr(document.getElementById("app"),{onPlay:()=>Ji(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:Ft})});return}be.user=e.user,await Vt(),fn();try{const{data:i}=await w.from("formation_links_overrides").select("formation, links"),n={};(i||[]).forEach(r=>{n[r.formation]=r.links}),Kn(n)}catch(i){console.warn("Impossible de charger les overrides de formation:",i)}if(!be.profile){Wn(document.getElementById("app"),{state:be,navigate:async()=>{await Vt(),_i()},toast:Ft,refreshProfile:Vt});return}const t=Array.isArray(be.profile.pending_boosters)?be.profile.pending_boosters:[];if(!be.profile.onboarding_done&&t.length>0){Uo(document.getElementById("app"),{state:be,navigate:()=>_i(),toast:Ft,refreshProfile:Vt});return}_i(),setTimeout(()=>Pr(be.profile,Pt,Ft),800),w.auth.onAuthStateChange(async(i,n)=>{i==="SIGNED_OUT"&&(be.user=null,be.profile=null,document.getElementById("app").innerHTML="",Ji(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:Ft}))})}function ta(){return Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight)}function gi(){const e=document.getElementById("app");e&&(e.style.height=ta()+"px")}window.addEventListener("resize",gi);window.addEventListener("orientationchange",()=>setTimeout(gi,150));window.visualViewport&&window.visualViewport.addEventListener("resize",gi);function _i(){const e=()=>{var i;(i=be.user)!=null&&i.id&&w.from("users").update({last_seen_at:new Date().toISOString()}).eq("id",be.user.id).then(()=>{})};e(),window._presencePingInterval&&clearInterval(window._presencePingInterval),window._presencePingInterval=setInterval(e,6e4);const t=document.getElementById("app");t.style.display="flex",t.style.flexDirection="column",gi(),Jr(),Gn()}function fn(){const e=document.getElementById("app-loader"),t=document.getElementById("app");t&&(t.style.display=""),e&&(e.classList.add("zoom-out"),setTimeout(()=>e.style.display="none",500))}function Rn(e){var n;const t=document.getElementById("app-loader");if(t&&(t.style.display="none"),document.getElementById("boot-error"))return;const i=document.createElement("div");i.id="boot-error",i.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:99999;gap:16px;color:#fff;padding:24px;text-align:center",i.innerHTML=`
    <div style="font-size:42px">📡</div>
    <div style="font-size:18px;font-weight:900">Connexion impossible</div>
    <div style="font-size:13px;color:rgba(255,255,255,0.6);max-width:280px">${e||"Le chargement a échoué. Vérifie ta connexion et réessaie."}</div>
    <button id="boot-retry" style="margin-top:8px;padding:12px 30px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer">Réessayer</button>`,document.body.appendChild(i),(n=document.getElementById("boot-retry"))==null||n.addEventListener("click",()=>window.location.reload())}ea().catch(e=>{console.error("Échec du démarrage:",e),Rn()});setTimeout(()=>{const e=document.getElementById("app-loader");e&&e.style.display!=="none"&&!e.classList.contains("zoom-out")&&!document.getElementById("boot-error")&&Rn("Le serveur met trop de temps à répondre.")},12e3);
