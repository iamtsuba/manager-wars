import{s as k,l as X,F as Te,b as me,c as Je,d as ht}from"./formation-links-BrrCMBql.js";function De(e,{navigate:t,toast:i}){e.innerHTML=`
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),n=(l||[]).filter(A=>A.card_type==="player"&&A.player),c=(l||[]).filter(A=>A.card_type==="game_changer"),p=(l||[]).filter(A=>A.card_type==="formation"),f=Object.keys(Te),u=Object.keys(ne),x={};n.forEach(A=>{const z=A.player.id;x[z]=(x[z]||0)+1}),new Set(Object.keys(x).map(A=>String(A)));const m=new Set(p.map(A=>A.formation)),b=new Set(c.map(A=>A.gc_type));let $="player",y="Tous",g="",v=!1;function L(){return[...n].sort((A,z)=>{const B=se.indexOf(A.player.job),T=se.indexOf(z.player.job);return B!==T?B-T:(A.player.surname_encoded||"").localeCompare(z.player.surname_encoded||"")})}function h(){return[...r||[]].sort((A,z)=>{const B=se.indexOf(A.job),T=se.indexOf(z.job);return B!==T?B-T:(A.surname_encoded||"").localeCompare(z.surname_encoded||"")})}function w(){return L().filter(A=>{const z=A.player,B=y==="Tous"||z.job===y,T=!g||`${z.firstname} ${z.surname_encoded}`.toLowerCase().includes(g);return B&&T})}function _(){return h().filter(A=>{const z=y==="Tous"||A.job===y,B=!g||`${A.firstname} ${A.surname_encoded}`.toLowerCase().includes(g);return z&&B})}e.innerHTML=`
  <div class="page">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const ie=1e3;function At(e,t,i,a){var m,b,$;const{state:d,toast:o,closeModal:s,navigate:l,refreshProfile:r}=i,n=e.formation,c={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function p(){const y=me[n]||{},g=Te[n]||[],v=290,L=360,h=20;function w(I){const E=y[I];return E?{x:E.x*v,y:E.y*L}:null}let _=`<svg width="${v}" height="${L}" viewBox="0 0 ${v} ${L}" xmlns="http://www.w3.org/2000/svg">`;for(const[I,E]of g){const C=w(I),j=w(E);!C||!j||(_+=`<line x1="${C.x}" y1="${C.y}" x2="${j.x}" y2="${j.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const I of Object.keys(y)){const E=w(I);if(!E)continue;const C=I.replace(/\d+/,""),j=c[C]||"#555";_+=`<circle cx="${E.x}" cy="${E.y}" r="${h}" fill="${j}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,_+=`<text x="${E.x}" y="${E.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${C}</text>`}return _+="</svg>",_}const f=t.filter(y=>y.formation===n),u=f.length,x=!e.is_for_sale;a(`Formation ${n}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
  ...
  `,document.body.appendChild(a);let d=!1;document.getElementById("pack-visual").addEventListener("click",()=>{if(d)return;d=!0;const u=document.getElementById("pack-visual");u.classList.add("shaking"),setTimeout(()=>{u.classList.add("pack-open"),setTimeout(()=>{document.getElementById("pack-phase").style.display="none",s(0)},600)},500)});let o=0;function s(u){o=u;const x=document.getElementById("reveal-phase");x.style.display="flex",l(u)}function l(u){var h;const x=e[u],m=document.getElementById("card-counter"),b=document.getElementById("single-card-slot"),$=document.getElementById("card-tap-hint");m&&(m.textContent=`Carte ${u+1} / ${e.length}`),$&&($.textContent=u<e.length-1?"Appuie pour continuer →":"Appuie pour voir toutes tes cartes");const y=He(x),g=x.card_type==="player"&&((h=x.player)==null?void 0:h.rarity)==="legende";b.innerHTML=`
      ...
      </div>`,g&&c();const v=document.getElementById("current-card-wrap");let L=!1;v.addEventListener("click",()=>{if(L)return;L=!0;const w=u+1;w<e.length?(v.style.transition="all 0.25s ease",v.style.transform="translateX(-120%) rotate(-15deg)",v.style.opacity="0",setTimeout(()=>l(w),250)):r()})}function r(){p(),document.getElementById("reveal-phase").style.display="none";const u=document.getElementById("recap-phase");u.style.display="flex";const x=document.getElementById("recap-grid");x.innerHTML=e.map((m,b)=>`
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}const ei="simulation",dt="VOTRE_ZONE_ID";function ti(){switch(ei){case"propellerads":return ii();case"adsense":return oi();default:return ze()}}function ze(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:#111;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:14px";let i=5;t.innerHTML=`
    ...
    <div style="display:flex;flex-direction:column;height:100%;min-height:100%;background:linear-gradient(180deg,#0a1628,#1a0a2e);padding:16px;gap:14px">
      ...
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",s).order("slot_order");let r=0;function n(){var y,g,v,L,h;const c=o[r],p=(l||[]).filter(w=>w.deck_id===c.id),f=p.filter(w=>{var _;return w.is_starter&&((_=w.card)==null?void 0:_.player)}).map(w=>_e(w.card)),u=p.find(w=>{var _;return((_=w.card)==null?void 0:_.card_type)==="formation"}),x=c.formation||((y=u==null?void 0:u.card)==null?void 0:y.formation)||"4-4-2",m=f.length>=11?ct(f,x):null,b=f.length>=11;e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:calc(100dvh - 130px);overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div id="deck-swipe-zone" style="flex:1;min-height:0;overflow:hidden;position:relative;touch-action:pan-y;display:flex;align-items:center;justify-content:center">
        ${m?`<div style="width:min(98vw, calc(100dvh - 400px));overflow:hidden;flex-shrink:0">${ut(m,x,null,[],285,285)}</div>`:`<div style="display:flex;align-items:center;justify-content:center;height:100%;opacity:.4;flex-direction:column;gap:8px">
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;min-height:100%;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
  </div>`,setTimeout(()=>pi(e,t,i),5e3)}function pi(e,t,i){const a=t.homeTeam.MIL||[],d=t.aiTeam.MIL||[];function o(p){return p.reduce((f,u)=>f+F(u,"MIL"),0)}function s(p){let f=0;for(let u=0;u<p.length-1;u++){const x=X(p[u],p[u+1]);x==="#00ff88"?f+=2:x==="#FFD700"&&(f+=1)}return f}const l=o(a)+s(a),r=o(d)+s(d),n=l>=r;function c(p,f,u){return`<div style="text-align:center">
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${p.map((x,m)=>{const b=R(x),$=m<p.length-1?X(x,p[m+1]):null,y=$&&$!=="#ff3333"&&$!=="#cc2222";return`
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;min-height:100%;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
  ...
  </div>`,setTimeout(()=>{const p=document.getElementById("score-home"),f=document.getElementById("score-ai"),u=document.getElementById("midfield-result");if(p&&f&&(n?(p.style.fontSize="80px",p.style.color="#FFD700",f.style.opacity="0.25"):(f.style.fontSize="80px",f.style.color="#ff6b6b",p.style.opacity="0.25")),u){const x=ni();u.style.opacity="1",n&&t.clubName,u.innerHTML=`
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;height:${p}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ...
      ${d?`<img src="${d}" style="width:${p-4}px;height:${p-4}px;object-fit:contain">`:e!=null&&e.clubName?`<span style="font-size:${Math.max(4,p-8)}px;font-weight:700;color:#333">${(e.clubName||"").slice(0,3).toUpperCase()}</span>`:""}
        ...
        stroke="${_}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const f=48,u=64,x=13,m=16,b={normal:"#aaaaaa",pépite:"#D4A017",papyte:"#111111",légende:"#7a28b8"};for(const[y,g]of Object.entries(r)){const v=c(y);if(!v||!g)continue;const L=y.replace(/[0-9]/g,""),h=ee[L]||"#555",w=i==="attack"&&["MIL","ATT"].includes(L)&&!g.used||i==="defense"&&["GK","DEF","MIL"].includes(L)&&!g.used,_=a.includes(g.cardId);let I;i==="attack"?I=L==="MIL"?Number(g.note_m)||0:Number(g.note_a)||0:i==="defense"?I=L==="GK"?Number(g.note_g)||0:L==="MIL"?Number(g.note_m)||0:Number(g.note_d)||0:I=Number(L==="GK"?g.note_g:L==="DEF"?g.note_d:L==="MIL"?g.note_m:g.note_a)||0,I=I+(g.boost||0);const E=(v.x-f/2).toFixed(1),C=(v.y-u/2).toFixed(1),j=g.used?.25:1,G=b[g==null?void 0:g.rarity]||b.normal,q=_?"#ff3030":G,O=_?3:(g==null?void 0:g.rarity)==="légende"||(g==null?void 0:g.rarity)==="pépite"?2.5:2,A=u-x-m;p+=`<defs><clipPath id="cp-${y}"><rect x="${E}" y="${(v.y-u/2+x).toFixed(1)}" width="${f}" height="${A}"/></clipPath></defs>`,p+=`<rect x="${E}" y="${C}" width="${f}" height="${u}" rx="5" fill="${h}" opacity="${j}"/>`;const z=R(g);z&&!g.used&&(p+=`<image href="${z}" x="${E}" y="${(v.y-u/2+x).toFixed(1)}" width="${f}" height="${A}" clip-path="url(#cp-${y})" preserveAspectRatio="xMidYMin slice"/>`),p+=`<rect x="${E}" y="${C}" width="${f}" height="${x}" rx="4" fill="rgba(255,255,255,0.92)"/>`,p+=`<text x="${v.x.toFixed(1)}" y="${(v.y-u/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(g.name||"").slice(0,9)}</text>`;const B=(v.y+u/2-m).toFixed(1);if(p+=`<rect x="${E}" y="${B}" width="${f}" height="${m}" fill="rgba(255,255,255,0.92)"/>`,g.used)p+=`<text x="${v.x.toFixed(1)}" y="${(v.y+u/2-m/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="13" font-weight="900" fill="rgba(0,0,0,0.4)" font-family="Arial Black">–</text>`;else{const T=ke(g.country_code);T?p+=`<image href="${T}" x="${(v.x-20).toFixed(1)}" y="${(v.y+u/2-m+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:p+=`<text x="${(v.x-13).toFixed(1)}" y="${(v.y+u/2-m/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${ft(g.country_code)}</text>`,p+=`<text x="${v.x.toFixed(1)}" y="${(v.y+u/2-m/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${I}</text>`;const M=pt(g);M?p+=`<image href="${M}" x="${(v.x+f/2-14).toFixed(1)}" y="${(v.y+u/2-m+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:g.clubName&&(p+=`<text x="${(v.x+14).toFixed(1)}" y="${(v.y+u/2-m/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(g.clubName||"").slice(0,3).toUpperCase()}</text>`),g.boost&&(p+=`<rect x="${(v.x+f/2-12).toFixed(1)}" y="${(v.y-u/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,p+=`<text x="${(v.x+f/2-5).toFixed(1)}" y="${(v.y-u/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${g.boost}</text>`)}p+=`<rect x="${E}" y="${C}" width="${f}" height="${u}" rx="5" fill="${_?"rgba(255,255,255,0.12)":"none"}" stroke="${q}" stroke-width="${O}" opacity="${j}"/>`,w&&(p+=`<rect x="${E}" y="${C}" width="${f}" height="${u}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${_?"selected":""}" data-card-id="${g.cardId}" data-role="${L}" style="cursor:pointer"/>`)}const $=38;return`<svg viewBox="${-$} ${-$} ${d+$*2} ${o+$*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${p}
  ...
  `;function l(r){const n=document.getElementById("mkt-content"),c=r==="buy"?s:o;if(c.length===0){n.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      ${c.map(p=>{var b,$,y;const f=(b=p.card)==null?void 0:b.player;if(!f)return"";const u=f.job==="GK"?f.note_g:f.job==="DEF"?f.note_d:f.job==="MIL"?f.note_m:f.note_a,x=$i[f.rarity],m=(i.profile.credits||0)>=p.price;return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px">
          <div style="width:44px;height:44px;border-radius:8px;background:${Ii(f.job)};color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:18px;border:2px solid ${x};flex-shrink:0">${u}</div>
          <div style="flex:1;min-width:0">
  ...
  `,document.querySelectorAll(".bottom-nav a").forEach(a=>{a.addEventListener("click",d=>{d.preventDefault(),fe(a.dataset.page)})}),document.getElementById("nav-logo").addEventListener("click",()=>fe("home")),document.getElementById("nav-credits").addEventListener("click",()=>fe("boosters")),document.getElementById("theme-toggle").addEventListener("click",()=>{const d=xe()==="dark"?"light":"dark";zi(d),Ve(d)}),Ve(xe())}function Ve(e){const t=document.getElementById("theme-icon");t&&(t.textContent=e==="dark"?"☀️":"🌙")}async function Ci(){Ae(xe()),document.getElementById("modal-overlay").addEventListener("click",t=>{t.target===t.currentTarget&&Ie()}),document.getElementById("modal-close").addEventListener("click",Ie);const{data:{session:e}}=await k.auth.getSession();if(!e){We(),De(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:ce});return}S.user=e.user,await pe(),We();try{const{data:t}=await k.from("formation_links_overrides").select("formation, links"),i={};(t||[]).forEach(a=>{i[a.formation]=a.links}),ht(i)}catch(t){console.warn("Impossible de charger les overrides de formation:",t)}if(!S.profile){bt(document.getElementById("app"),{state:S,navigate:async()=>{await pe(),Ye()},toast:ce,refreshProfile:pe});return}S.profile.theme&&S.profile.theme!==xe()&&(localStorage.setItem(Fe,S.profile.theme),Ae(S.profile.theme)),Ye(),k.auth.onAuthStateChange(async(t,i)=>{t==="SIGNED_OUT"&&(S.user=null,S.profile=null,document.getElementById("app").innerHTML="",De(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:ce}))})}function Ye(){const e=document.getElementById("app");e.style.display="flex",e.style.flexDirection="column",Mi(),yt()}function We(){const e=document.getElementById("app-loader"),t=document.getElementById("app");t&&(t.style.display=""),e&&(e.classList.add("zoom-out"),setTimeout(()=>e.style.display="none",500))}Ci();
