...
import{s as z,F as _i,i as Ht,k as Zi,l as nt,m as ji,f as Pt,n as Lo,T as ci,j as Io,o as Mo,b as jo}from"./formation-links-5A-YoKwp.js";const Co="/";function Ci(e,{navigate:t,toast:i}){let n="login";const d=()=>{var r,c,f,a,o,l;const s=n==="login";e.innerHTML=`
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),a=(c||[]).filter(Y=>Y.card_type==="player"&&Y.player),o=(c||[]).filter(Y=>Y.card_type==="game_changer"),l=(c||[]).filter(Y=>Y.card_type==="formation"),b=(c||[]).filter(Y=>Y.card_type==="stadium"),{data:h}=await z.from("gc_definitions").select("name,gc_type,color,effect,image_url"),$={};(h||[]).forEach(Y=>{$[Y.name]=Y});const{data:p}=await z.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),u={};(p||[]).forEach(Y=>{u[Y.id]=Y}),b.forEach(Y=>{Y.stadium_def&&(u[Y.stadium_id]=Y.stadium_def)});const m=Object.keys(_i),A=Object.keys(Re),j={};a.forEach(Y=>{const oe=Y.player.id;j[oe]=(j[oe]||0)+1}),new Set(Object.keys(j).map(Y=>String(Y)));const F=new Set(l.map(Y=>Y.formation)),q=new Set(o.map(Y=>Y.gc_type));let B="player",C="Tous",P="",V=!1;function ee(){return[...a].sort((Y,oe)=>{const fe=Yt.indexOf(Y.player.job),U=Yt.indexOf(oe.player.job);return fe!==U?fe-U:(Y.player.surname_encoded||"").localeCompare(oe.player.surname_encoded||"")})}function se(){return[...f||[]].sort((Y,oe)=>{const fe=Yt.indexOf(Y.job),U=Yt.indexOf(oe.job);return fe!==U?fe-U:(Y.surname_encoded||"").localeCompare(oe.surname_encoded||"")})}function re(){return ee().filter(Y=>{const oe=Y.player,fe=C==="Tous"||oe.job===C,U=!P||`${oe.firstname} ${oe.surname_encoded}`.toLowerCase().includes(P);return fe&&U})}function Z(){return se().filter(Y=>{const oe=C==="Tous"||Y.job===C,fe=!P||`${Y.firstname} ${Y.surname_encoded}`.toLowerCase().includes(P);return oe&&fe})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
  ...
  </div>`;function O(){const Y=document.getElementById("col-filters");Y&&(B==="player"?(Y.innerHTML=`
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",oe=>{P=oe.target.value.toLowerCase(),D()}),e.querySelectorAll(".filter-btn").forEach(oe=>{oe.addEventListener("click",()=>{C=oe.dataset.job,O(),D()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{V=!V,O(),D()})):(Y.innerHTML=`
        <div style="display:flex;justify-content:flex-end">
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{V=!V,O(),D()})))}function D(){const Y=document.getElementById("col-grid");Y&&(B==="player"?Le(Y):B==="formation"?Fe(Y):B==="stadium"?we(Y):Ce(Y))}function ne(Y,oe,fe,U,te){const W=document.getElementById("col-grid"),de=document.getElementById("col-big");if(!W||!de)return;var ve=0;function Te(){const $e=window.innerWidth>=768,be=document.getElementById("col-big"),ke=document.getElementById("col-grid");$e&&be&&(be.style.minHeight="420px",be.style.flex="1 1 auto"),$e&&ke&&(ke.style.height=Math.round(310*.76+16)+"px",ke.style.flexShrink="0",ke.style.overflowX="auto",ke.style.overflowY="hidden",ke.style.alignItems="center",ke.style.padding="8px 16px"),de.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+oe(Y[ve])+"</div>";var qe=de.querySelector("[data-card-id],[data-form-id],[data-gc-id]");qe&&qe.addEventListener("click",function(){U(Y[ve])}),requestAnimationFrame(function(){var je=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!je||!de)){var w=de.clientHeight-8,T=de.clientWidth-24,g=je.offsetHeight,y=je.offsetWidth;if(g>0&&y>0&&w>40){var x=$e?2.2:1,_=Math.min(w/g,T/y,x);je.style.transform="scale("+_.toFixed(3)+")",je.style.transformOrigin="top center"}}}),W.innerHTML=Y.map(function(je,w){var T=w===ve,g="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+(T?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+w+'" style="'+g+'">'+fe(je,T)+"</div>"}).join(""),W.querySelectorAll(".col-mini-item").forEach(function(je){je.addEventListener("click",function(){ve=Number(je.dataset.idx),Te(),je.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}Te()}function ue(Y){var oe=window.innerWidth>=768?.76:.54,fe;if(!Y||Y._fake){var U=Y?Y.player:null;if(!U)return"";fe=Pi(U)}else fe=pi(Y,"");return'<div style="display:inline-block;zoom:'+oe+';pointer-events:none;line-height:0">'+fe+"</div>"}function ye(Y,oe,fe){oe=oe||100,fe=fe||140;var U=Ht[Y]||{},te={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},W=Math.max(3,Math.round(oe*.06)),de=Object.entries(U).map(function(Te){var $e=Te[0],be=Te[1],ke=$e.replace(/\d+$/,""),qe=te[ke]||"#888",je=Math.round(be.x*oe),w=Math.round(be.y*fe);return'<circle cx="'+je+'" cy="'+w+'" r="'+W+'" fill="'+qe+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),ve=Math.max(1,Math.round(oe/50));return'<svg viewBox="0 0 '+oe+" "+fe+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+oe+'" height="'+fe+'" fill="#1A6B3C"/><rect x="'+Math.round(oe*.2)+'" y="'+Math.round(fe*.02)+'" width="'+Math.round(oe*.6)+'" height="'+Math.round(fe*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+ve+'"/><line x1="0" y1="'+Math.round(fe*.5)+'" x2="'+oe+'" y2="'+Math.round(fe*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+ve+'"/><ellipse cx="'+Math.round(oe*.5)+'" cy="'+Math.round(fe*.5)+'" rx="'+Math.round(oe*.18)+'" ry="'+Math.round(fe*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+ve+'"/><rect x="'+Math.round(oe*.2)+'" y="'+Math.round(fe*.82)+'" width="'+Math.round(oe*.6)+'" height="'+Math.round(fe*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+ve+'"/>'+de+"</svg>"}function me(Y,oe,fe){var U=fe>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+fe+"</div>":"",te=oe?'data-form-id="'+oe.id+'"':"",W=Y.length>7?14:Y.length>5?16:19,de=!!oe;return"<div "+te+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(de?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(de?"":"filter:grayscale(1);opacity:0.5")+'">'+U+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(de?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+W+"px;font-weight:900;color:"+(de?"#1A6B3C":"#aaa")+';line-height:1">'+Y+'</div></div><div style="flex:1;overflow:hidden;background:'+(de?"#1A6B3C":"#ccc")+'">'+ye(Y,140,220)+"</div></div>"}function xe(Y,oe){var fe=window.innerWidth>=768?.76:.54,U=140,te=305,W=Math.round(te*.22),de=te-W,ve=Y.length>7?10:13,Te=ye(Y,U,de),$e=oe?"1.5px solid #2a7a40":"1px solid #ddd",be=oe?"":"filter:grayscale(1);opacity:0.45;",ke=oe?"#1A6B3C":"#bbb",qe="#fff";return'<div style="display:inline-block;zoom:'+fe+';line-height:0;pointer-events:none"><div style="width:'+U+"px;height:"+te+"px;border-radius:6px;border:"+$e+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+be+'"><div style="height:'+W+"px;background:"+ke+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+ve+"px;font-weight:900;color:"+qe+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(U-4)+'px">'+Y+'</span></div><div style="height:'+de+'px;overflow:hidden;flex-shrink:0">'+Te+"</div></div></div>"}function Le(Y){if(V){const oe=Z();if(!oe.length){Y.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const fe=oe.map(U=>a.find(te=>te.player.id===U.id)||{_fake:!0,player:U,id:"fake-"+U.id});ne(fe,U=>U._fake?Pi(U.player):pi(U,""),U=>U._fake?ue({player:U.player,id:"x",_fake:!0}):ue(U),U=>{U._fake||Ni(U,a,j,t)})}else{const oe=re();if(!oe.length){Y.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const fe={};oe.forEach(te=>{const W=te.player.id;fe[W]||(fe[W]=[]),fe[W].push(te)});const U=Object.values(fe).map(te=>Qo(te));ne(U,te=>{const W=j[te.player.id]||1,de=W>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${W}</div>`:"",Te=a.filter($e=>$e.player.id===te.player.id&&$e.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return pi(te,de+Te)},te=>ue(te),te=>Ni(te,a,j,t))}}function Fe(Y){const oe=V?m:[...F];if(!oe.length){Y.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const fe=oe.map(U=>({formation:U,card:l.find(te=>te.formation===U)||null,owned:F.has(U)}));ne(fe,({formation:U,card:te,owned:W})=>me(U,W?te:null,W?l.filter(de=>de.formation===U).length:0),({formation:U,owned:te})=>xe(U,te),({card:U,owned:te})=>{te&&U&&on(U,l,t,s)})}function Ce(Y){const oe=Object.keys($),fe=V?oe.length?oe:A:[...q];if(!fe.length){Y.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const U=fe.map(te=>({type:te,gc:Re[te]||{icon:"⚡",desc:""},def:$[te]||null,owned:q.has(te),card:o.find(W=>W.gc_type===te)||null}));ne(U,({type:te,gc:W,def:de,owned:ve,card:Te})=>{var x;const $e=ve?o.filter(_=>_.gc_type===te).length:0,be=$e>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${$e}</div>`:"",ke=(de==null?void 0:de.gc_type)==="ultra_game_changer",qe={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},je={purple:"#b06ce0",light_blue:"#00d4ef"},w=qe[de==null?void 0:de.color]||qe.purple,T=je[de==null?void 0:de.color]||je.purple,g=(de==null?void 0:de.effect)||W.desc||"",y=de!=null&&de.image_url?`/icons/${de.image_url}`:((x=de==null?void 0:de.club)==null?void 0:x.logo_url)||(de!=null&&de.country_code?`https://flagcdn.com/w80/${de.country_code.toLowerCase()}.png`:null);return ve&&Te?`<div data-gc-id="${Te.id}" data-gc-type="${te}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${T};background:${w};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${T}66;cursor:pointer">
          ${be}
          ...
          <div style="height:150px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
            ${y?`<img src="${y}" style="max-width:120px;max-height:120px;object-fit:contain;border-radius:6px">`:`<span style="font-size:64px">${W.icon}</span>`}
          </div>
          ...
          <div style="height:150px;display:flex;align-items:center;justify-content:center"><span style="font-size:64px">${W.icon}</span></div>
          <div style="padding:10px;background:rgba(0,0,0,0.05);text-align:center"><div style="font-size:11px;color:#aaa">Non possédée</div></div>
        </div>`},({type:te,gc:W,def:de,owned:ve})=>{const Te=window.innerWidth>=768?.76:.54,$e={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},be={purple:"#9b59b6",light_blue:"#00bcd4"},ke=$e[de==null?void 0:de.color]||$e.purple,qe=be[de==null?void 0:de.color]||be.purple,je=de!=null&&de.image_url?`/icons/${de.image_url}`:null;return ve?`<div style="display:inline-block;zoom:${Te};line-height:0;pointer-events:none"><div style="width:140px;height:310px;border-radius:8px;background:${ke};border:1px solid ${qe};display:flex;flex-direction:column;overflow:hidden"><div style="height:56px;background:rgba(255,255,255,0.15);display:flex;align-items:center;justify-content:center"><span style="font-size:13px;font-weight:900;color:#fff;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:130px">${te}</span></div><div style="flex:1;display:flex;align-items:center;justify-content:center">${je?`<img src="${je}" style="max-width:130px;max-height:190px;object-fit:contain">`:`<span style="font-size:48px">${W.icon}</span>`}</div><div style="height:54px;display:flex;align-items:center;justify-content:center;padding:0 4px"><span style="font-size:10px;color:rgba(255,255,255,0.7);text-align:center">${((de==null?void 0:de.effect)||W.desc||"").slice(0,30)}</span></div></div></div>`:`<div style="display:inline-block;zoom:${Te};line-height:0;pointer-events:none"><div style="width:140px;height:310px;border-radius:8px;background:#eee;border:1px solid #ddd;display:flex;flex-direction:column;align-items:center;justify-content:center;filter:grayscale(1);opacity:0.45"><span style="font-size:36px">${W.icon}</span><span style="font-size:12px;color:#aaa;margin-top:6px;text-align:center;padding:0 6px">${te}</span></div></div>`},({type:te,owned:W,def:de})=>{W&&tn(te,de,s)})}function we(Y){const oe="#E87722",fe="/";if(!b.length){Y.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Stade.<br><small>Ouvre un booster Stade !</small></div>';return}const U={};b.forEach(W=>{const de=W.stadium_id||"?";(U[de]=U[de]||[]).push(W)});const te=Object.entries(U).map(([W,de])=>({sid:W,def:u[W]||null,count:de.length,card:de[0]}));ne(te,({def:W,count:de})=>{var qe,je;const ve=(W==null?void 0:W.name)||"?",Te=((qe=W==null?void 0:W.club)==null?void 0:qe.encoded_name)||(W==null?void 0:W.country_code)||"—",$e=W!=null&&W.image_url?`${fe}icons/${W.image_url}`:((je=W==null?void 0:W.club)==null?void 0:je.logo_url)||(W!=null&&W.country_code?`https://flagcdn.com/w80/${W.country_code.toLowerCase()}.png`:null),be=$e?`<img src="${$e}" style="width:90px;height:90px;object-fit:contain;border-radius:4px" onerror="this.style.display='none'">`:'<div style="font-size:56px">🏟️</div>',ke=de>1?`<div style="position:absolute;top:8px;right:8px;background:#333;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${de}</div>`:"";return`<div style="position:relative;width:140px;border-radius:12px;border:3px solid #c45a00;background:linear-gradient(160deg,${oe},#c45a00);display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${oe}66">
          ${ke}
        ...
        </div>`},({def:W,count:de})=>{var qe,je;const ve=window.innerWidth>=768?.76:.54,Te=(W==null?void 0:W.name)||"?",$e=((qe=W==null?void 0:W.club)==null?void 0:qe.encoded_name)||(W==null?void 0:W.country_code)||"—",be=W!=null&&W.image_url?`${fe}icons/${W.image_url}`:((je=W==null?void 0:W.club)==null?void 0:je.logo_url)||(W!=null&&W.country_code?`https://flagcdn.com/w80/${W.country_code.toLowerCase()}.png`:null),ke=be?`<img src="${be}" style="width:64px;height:64px;object-fit:contain;border-radius:4px" onerror="this.style.display='none'">`:'<span style="font-size:32px">🏟️</span>';return`<div style="display:inline-block;zoom:${ve};line-height:0;pointer-events:none">
        ...
        </div>`},null)}e.querySelectorAll(".col-tab-btn").forEach(Y=>{Y.addEventListener("click",()=>{B=Y.dataset.tab,C="Tous",P="",V=!1,e.querySelectorAll(".col-tab-btn").forEach(oe=>{const fe=oe.dataset.tab===B;oe.style.borderBottomColor=fe?"var(--green)":"transparent",oe.style.color=fe?"var(--green)":"var(--gray-600)"}),O(),D()})}),O(),D()}function tn(e,t,i){const n=Re[e]||{icon:"⚡",desc:"Effet spécial."},d=(t==null?void 0:t.gc_type)==="ultra_game_changer",s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},r={purple:"#b06ce0",light_blue:"#00d4ef"},c=s[t==null?void 0:t.color]||s.purple,f=r[t==null?void 0:t.color]||r.purple,a=(t==null?void 0:t.name)||e,o=(t==null?void 0:t.effect)||n.desc,l=t!=null&&t.image_url?`/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Wt=1e3;function on(e,t,i,n){var $,p,u;const{state:d,toast:s,closeModal:r,navigate:c,refreshProfile:f}=i,a=e.formation,o={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function l(){const m=Ht[a]||{},A=_i[a]||[],j=290,F=360,q=20;function B(P){const V=m[P];return V?{x:V.x*j,y:V.y*F}:null}let C=`<svg width="${j}" height="${F}" viewBox="0 0 ${j} ${F}" xmlns="http://www.w3.org/2000/svg">`;for(const[P,V]of A){const ee=B(P),se=B(V);!ee||!se||(C+=`<line x1="${ee.x}" y1="${ee.y}" x2="${se.x}" y2="${se.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const P of Object.keys(m)){const V=B(P);if(!V)continue;const ee=P.replace(/\d+/,""),se=o[ee]||"#555";C+=`<circle cx="${V.x}" cy="${V.y}" r="${q}" fill="${se}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,C+=`<text x="${V.x}" y="${V.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${ee}</text>`}return C+="</svg>",C}const b=t.filter(m=>m.formation===a);b.length;const h=!e.is_for_sale;n(`Formation ${a}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),($=document.getElementById("direct-sell-form-btn"))==null||$.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${a} pour ${Wt.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const m=b.find(j=>!j.is_for_sale)||b[0];if(!m){s("Aucune carte à vendre","error");return}await z.from("market_listings").delete().eq("card_id",m.id),await z.from("transfer_history").delete().eq("card_id",m.id);const{error:A}=await z.from("cards").delete().eq("id",m.id);if(A){s(A.message,"error");return}await z.from("users").update({credits:(d.profile.credits||0)+Wt}).eq("id",d.profile.id),await f(),s(`+${Wt.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),r(),c("collection")}),(p=document.getElementById("market-sell-form-btn"))==null||p.addEventListener("click",async()=>{const m=parseInt(document.getElementById("sell-price-form").value);if(!m||m<1){s("Prix invalide","error");return}await z.from("cards").update({is_for_sale:!0,sale_price:m}).eq("id",e.id),await z.from("market_listings").insert({seller_id:d.profile.id,card_id:e.id,price:m}),s("Carte mise en vente sur le marché !","success"),r(),c("collection")}),(u=document.getElementById("cancel-sell-form-btn"))==null||u.addEventListener("click",async()=>{await z.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await z.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),s("Annonce retirée","success"),r(),c("collection")})}async function Ni(e,t,i,n){var Le,Fe,Ce,we,Y,oe,fe;const{state:d,toast:s,openModal:r,closeModal:c,navigate:f,refreshProfile:a}=n,o=e.player,l=t.filter(U=>U.player.id===o.id),b=l.length,h=e.evolution_bonus||0,$=Math.max((Number(o.note_g)||0)+(o.job==="GK"||o.job2==="GK"?h:0),(Number(o.note_d)||0)+(o.job==="DEF"||o.job2==="DEF"?h:0),(Number(o.note_m)||0)+(o.job==="MIL"||o.job2==="MIL"?h:0),(Number(o.note_a)||0)+(o.job==="ATT"||o.job2==="ATT"?h:0)),p=o.rarity||"normal",{data:u}=await z.from("sell_price_configs").select("*").eq("rarity",p).lte("note_min",$).gte("note_max",$).order("note_min",{ascending:!1}).limit(1);((Le=u==null?void 0:u[0])==null?void 0:Le.price)??Zo[p];const m=o.rarity!=="legende",A=po(o),j=((o.rarity==="pepite"||o.rarity==="papyte")&&e.current_note!=null?e.current_note:St(o,o.job))+h,F=o.rarity==="pepite"||o.rarity==="papyte",q=o.job2?(F?Ei(e,co(o.job2)):St(o,o.job2))+(St(o,o.job2)>0?h:0):null,B=Ft[o.job]||"#1A6B3C",C=o.job2?Ft[o.job2]:null,P=lo[o.rarity]||"#ccc",V=zi[o.country_code]||o.country_code||"",ee=e.evolution_bonus||0,re=j+ee,Z=q||0,O=Z>0?Z+ee:0,D=l.map(U=>U.id);let ne={};if(D.length){const{data:U}=await z.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",D).order("transferred_at",{ascending:!0});(U||[]).forEach(te=>{ne[te.card_id]||(ne[te.card_id]=[]),ne[te.card_id].push(te)})}const ue=U=>{const te=U.transferred_at?new Date(U.transferred_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"2-digit",year:"numeric"}):"",W=U.source==="booster"?"Booster":U.price?U.price.toLocaleString("fr"):"—";return`
    <div style="display:flex;justify-content:space-between;align-items:flex-start;background:#f9f9f9;border-radius:8px;padding:8px 12px;gap:8px">
      ...
      <div style="text-align:right;flex-shrink:0">
        <div style="font-size:13px;font-weight:700;color:${U.source==="booster"?"var(--gray-600)":"var(--yellow)"}">${W}</div>
    ...
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${l.map((U,te)=>{const W=ne[U.id]||[],de=U.is_for_sale,ve=W.length?W[W.length-1]:null,$e=(o.rarity==="pepite"||o.rarity==="papyte")&&U.current_note!=null?` (☆${U.current_note})`:"";return`
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;background:#f0fdf4;border-radius:10px;padding:10px 14px">
          <div>
            ...
            <div style="font-size:11px;color:#555;margin-top:2px">
              ${(e.evolution_bonus||0)>0?`+${e.evolution_bonus} appliqué${e.evolution_bonus>1?"s":""} · Note : ${re}${o.job2&&Z>0?` / ${O}`:""}`:`Note actuelle : ${re}${o.job2&&Z>0?` / ${O}`:""}`}
            </div>
          </div>
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",o.note_g],["DEF",o.note_d],["MIL",o.note_m],["ATT",o.note_a]].map(([U,te])=>{const W=Ft[U],de=U===o.job||U===o.job2,ve=(Number(te)||0)+(de&&ee>0?ee:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:2px">
                <svg width="38" height="37" viewBox="0 0 38 37" style="display:block">
                  <polygon points="19,2 24,13 36,13 26,21 30,33 19,26 8,33 12,21 2,13 14,13" fill="${W}" stroke="white" stroke-width="1.5"/>
    ...
    ${ye}
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(we=document.getElementById("close-detail"))==null||we.addEventListener("click",c);let me=new Set;const xe=()=>{const U=me.size,te=document.getElementById("sell-action-panel");if(!te)return;te.style.display=U>0?"block":"none",document.getElementById("sell-selected-count").textContent=U;const W=document.getElementById("evolve-btn");W&&(W.textContent=`⬆️ Évoluer ${U>1?"(+"+U+")":""}`)};document.querySelectorAll(".expl-check").forEach(U=>{U.addEventListener("change",()=>{const te=U.dataset.id;U.checked?me.add(te):me.delete(te);const W=U.closest(".exemplaire-row");W&&(W.style.borderColor=U.checked?"#1A6B3C":"#eee"),xe()})}),document.querySelectorAll(".exemplaire-row").forEach(U=>{U.addEventListener("click",te=>{if(te.target.closest("button")||te.target.tagName==="INPUT")return;const W=U.querySelector(".expl-check");W&&!W.disabled&&(W.checked=!W.checked,W.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(U=>{U.addEventListener("click",te=>{te.stopPropagation();const W=document.querySelector(`.expl-hist[data-hist="${U.dataset.idx}"]`);W&&(W.style.display=W.style.display==="none"?"flex":"none")})}),(Y=document.getElementById("evolve-btn"))==null||Y.addEventListener("click",async()=>{if(b<=1)return;const U=[...me];if(!U.length)return;if(me.has(e.id)){const be=document.createElement("div");be.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",be.innerHTML=`
        ...
        <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center;box-shadow:0 16px 48px rgba(0,0,0,0.3)">
          ...
          <div style="background:#f0fdf4;border-radius:10px;padding:12px;margin-bottom:16px;font-size:13px;color:#333">
            ...
            📈 Note : <strong>${j+(e.evolution_bonus||0)}</strong> → <strong style="color:#1A6B3C">${j+(e.evolution_bonus||0)+te.length*W}</strong>
            ${q&&q>0?`<br>📈 Note 2 : <strong>${q+(e.evolution_bonus||0)}</strong> → <strong style="color:#1A6B3C">${q+(e.evolution_bonus||0)+te.length*W}</strong>`:""}
            ${o.rarity==="legende"?`<br><span style="color:#7a28b8;font-size:11px;font-weight:700">✨ Légende : +${W} par copie sacrifiée</span>`:""}
          </div>
  ...
  <div style="height:100%;overflow-y:auto;padding-bottom:80px;background:var(--page-bg)">
    ...
    <div class="page-body">
      ...
      ${l.length>0?l.map(p=>{var q,B;const u=p.player,m=p.evolution_bonus||0,A=(r==="GK"?u.note_g:r==="DEF"?u.note_d:r==="MIL"?u.note_m:u.note_a)+(r===u.job||r===u.job2?m:0),j=ri(u),F={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[u.rarity];return`<div class="player-option" data-card-id="${p.id}"
          ...
          <!-- Note -->
          <div style="width:36px;height:36px;border-radius:8px;background:${_t[r]};color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:16px;border:2px solid ${F};flex-shrink:0">
            ${A}
          </div>
  ...
  `,document.body.appendChild(d);let s=!1;const r=document.getElementById("pack-cut-zone"),c=document.getElementById("pack-blade");let f=!1;const a=O=>O.touches&&O.touches[0]?O.touches[0].clientX:O.clientX;function o(O){s||(f=!0,c.style.opacity="1",l(O))}function l(O){if(!f||s)return;const D=r.getBoundingClientRect(),ne=a(O)-D.left,ue=Math.max(0,Math.min(1,ne/D.width));c.style.width=ue*D.width+"px",ue>=.82&&h()}function b(){s||(f=!1,c.style.transition="width .2s ease, opacity .2s ease",c.style.width="0",c.style.opacity="0",setTimeout(()=>{s||(c.style.transition="")},220))}function h(){var D;if(s)return;s=!0,f=!1,c.style.width="100%",c.style.opacity="1",(D=document.getElementById("cut-flash"))==null||D.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const O=document.getElementById("cut-hint");O&&(O.style.opacity="0"),r.classList.add("pack-cut"),setTimeout(()=>{c.style.opacity="0",document.getElementById("pack-phase").style.display="none",m(0)},620)}r.addEventListener("pointerdown",o),window.addEventListener("pointermove",l),window.addEventListener("pointerup",b),r.addEventListener("touchstart",o,{passive:!0}),window.addEventListener("touchmove",l,{passive:!0}),window.addEventListener("touchend",b);let $=0,p=!1;const u=new Set;function m(O){$=O,document.getElementById("reveal-phase").style.display="flex",A(),j(O,0),P()}function A(){const O=document.getElementById("card-dots");O&&(O.innerHTML=e.map((D,ne)=>`<div class="card-dot" data-i="${ne}" style="width:8px;height:8px;border-radius:50%;background:${ne===$?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),O.querySelectorAll(".card-dot").forEach(D=>D.addEventListener("click",()=>q(parseInt(D.dataset.i)))))}function j(O,D){var we;const ne=e[O],ue=document.getElementById("card-counter"),ye=document.getElementById("card-track");ue&&(ue.textContent=`Carte ${O+1} / ${e.length}`);const me=document.getElementById("reveal-btns");me&&(me.style.display=O===e.length-1?"flex":"none");const xe=ne.card_type==="player"&&((we=ne.player)==null?void 0:we.rarity)==="legende",Le=!u.has(O);u.add(O);let Fe=0;if(ne.card_type==="player"&&ne.player){const Y=ne.player,oe=Y.job||"ATT";Fe=Number(oe==="GK"?Y.note_g:oe==="DEF"?Y.note_d:oe==="MIL"?Y.note_m:Y.note_a)||0}const Ce=Y=>{ye.innerHTML=`
        ...
        </div>`;const oe=document.getElementById("current-card-wrap");D!==0?(oe.style.transition="none",oe.style.transform=`translateX(${D>0?100:-100}%)`,requestAnimationFrame(()=>{oe.style.transition="transform .28s cubic-bezier(.25,1,.5,1)",oe.style.transform="translateX(0)"})):oe.animate([{opacity:0,transform:"scale(.7)"},{opacity:1,transform:"scale(1)"}],{duration:300,easing:"cubic-bezier(.34,1.56,.64,1)"}),Y||xe?ee():se(),A()};Le&&Fe>6&&ne.card_type==="player"&&ne.player?F(ne,()=>Ce(!0)):Ce(!1)}function F(O,D){var Fe;p=!0;const ne=O.player,ue=`https://flagsapi.com/${ne.country_code}/flat/64.png`,ye=(Fe=ne.clubs)==null?void 0:Fe.logo_url,me=document.getElementById("walkout-overlay"),xe=document.getElementById("walkout-stage");if(!me||!xe){p=!1,D();return}me.style.display="flex";const Le=()=>{const Ce=xe.firstElementChild;Ce&&(Ce.classList.remove("wo-in"),Ce.classList.add("wo-out"))};xe.innerHTML=`<img class="wo-in" src="${ue}" style="height:130px;border-radius:10px;box-shadow:0 10px 36px rgba(0,0,0,.6)" onerror="this.style.display='none'">`,navigator.vibrate&&navigator.vibrate(30),setTimeout(Le,2e3),setTimeout(()=>{var Ce;xe.innerHTML=ye?`<img class="wo-in" src="${ye}" style="max-height:160px;max-width:210px;object-fit:contain">`:`<div class="wo-in" style="font-size:34px;font-weight:900;color:#fff;text-align:center">${((Ce=ne.clubs)==null?void 0:Ce.encoded_name)||"CLUB"}</div>`,navigator.vibrate&&navigator.vibrate(30)},2450),setTimeout(Le,4450),setTimeout(()=>{me.style.display="none",xe.innerHTML="",p=!1,navigator.vibrate&&navigator.vibrate([40,30,80]),D()},4900)}function q(O){if(p||O<0||O>=e.length||O===$)return;const D=O>$?1:-1;$=O,j(O,D)}function B(){q($+1)}function C(){q($-1)}function P(){const O=document.getElementById("card-viewport");if(!O||O._swipeBound)return;O._swipeBound=!0;let D=0,ne=0,ue=0,ye=!1;const me=we=>we.touches?we.touches[0].clientX:we.clientX,xe=we=>we.touches?we.touches[0].clientY:we.clientY,Le=we=>{ye=!0,D=me(we),ne=xe(we),ue=0},Fe=we=>{if(!ye)return;ue=me(we)-D;const Y=xe(we)-ne;if(Math.abs(ue)<Math.abs(Y))return;const oe=document.getElementById("current-card-wrap");oe&&(oe.style.transition="none",oe.style.transform=`translateX(${ue*.6}px) rotate(${ue*.02}deg)`)},Ce=()=>{if(!ye)return;ye=!1;const we=document.getElementById("current-card-wrap"),Y=55;ue<=-Y&&$<e.length-1?B():ue>=Y&&$>0?C():we&&(we.style.transition="transform .2s ease",we.style.transform="translateX(0)")};O.addEventListener("pointerdown",Le),O.addEventListener("pointermove",Fe),O.addEventListener("pointerup",Ce),O.addEventListener("pointercancel",Ce),O.addEventListener("touchstart",Le,{passive:!0}),O.addEventListener("touchmove",Fe,{passive:!0}),O.addEventListener("touchend",Ce),O.addEventListener("click",we=>{if(Math.abs(ue)>8)return;const Y=O.getBoundingClientRect();we.clientX-Y.left>Y.width/2?B():C()})}let V=null;function ee(){const O=document.getElementById("fireworks-canvas");if(!O)return;O.width=window.innerWidth,O.height=window.innerHeight;const D=O.getContext("2d"),ne=[];function ue(){const me=Math.random()*O.width,xe=Math.random()*O.height*.6,Le=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],Fe=Le[Math.floor(Math.random()*Le.length)];for(let Ce=0;Ce<60;Ce++){const we=Math.PI*2/60*Ce,Y=2+Math.random()*5;ne.push({x:me,y:xe,vx:Math.cos(we)*Y,vy:Math.sin(we)*Y,alpha:1,color:Fe,size:2+Math.random()*3})}}ue(),V=setInterval(ue,600);function ye(){if(document.getElementById("fireworks-canvas")){D.clearRect(0,0,O.width,O.height);for(let me=ne.length-1;me>=0;me--){const xe=ne[me];if(xe.x+=xe.vx,xe.y+=xe.vy+.08,xe.vy*=.98,xe.alpha-=.018,xe.alpha<=0){ne.splice(me,1);continue}D.globalAlpha=xe.alpha,D.fillStyle=xe.color,D.beginPath(),D.arc(xe.x,xe.y,xe.size,0,Math.PI*2),D.fill()}D.globalAlpha=1,(V!==null||ne.length>0)&&requestAnimationFrame(ye)}}ye()}function se(){V!==null&&(clearInterval(V),V=null);const O=document.getElementById("fireworks-canvas");O&&O.getContext("2d").clearRect(0,0,O.width,O.height)}if(n){const O=document.getElementById("reveal-btns");O&&(O.innerHTML='<button class="btn btn-primary" id="reveal-next" style="flex:1">Continuer →</button>'),(Z=document.getElementById("reveal-next"))==null||Z.addEventListener("click",()=>{se(),d.remove(),n()})}else document.getElementById("reveal-collection").addEventListener("click",()=>{se(),d.remove(),i("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{se(),d.remove(),i("boosters")})}function vn(e){var t,i,n,d;if(e.card_type==="player"&&e.player)return mn(e);if(e.card_type==="game_changer"){const s=e._gcDef,r=(s==null?void 0:s.gc_type)==="ultra_game_changer",c={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},f={purple:"#b06ce0",light_blue:"#00d4ef"},a=c[s==null?void 0:s.color]||c.purple,o=f[s==null?void 0:s.color]||f.purple,l=(s==null?void 0:s.name)||e.gc_type||"Game Changer",b=(s==null?void 0:s.effect)||((t=xi[e.gc_type])==null?void 0:t.desc)||"",h=s!=null&&s.image_url?`/icons/${s.image_url}`:null,$=((i=xi[e.gc_type])==null?void 0:i.icon)||"⚡";return`<div style="width:170px;height:240px;background:${a};border-radius:14px;border:3px solid ${o};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${o}88;flex-shrink:0">
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">
        ${s.map((re,Z)=>`<div style="width:7px;height:7px;border-radius:50%;background:${Z===o?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}
      </div>`:""}
  ...
  </div>`}function tt(e,t,i){if(!(e!=null&&e.length))return"";const n=e.slice(0,5);let d='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return n.forEach((s,r)=>{if(d+=We(s,40,52),r<n.length-1){const c=nt(s,n[r+1]);d+=`<div style="width:7px;height:3px;background:${c==="#ff3333"||c==="#cc2222"?"rgba(255,255,255,0.12)":c};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(d+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),d+="</div>",d}function ei(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function _o(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function ct(e,t,i,n,d=310,s=310,r=[]){var j;const c=Ht[t]||{},f=Zi(t)||_i[t]||[],a={},o=["ATT","MIL","DEF","GK"];for(const F of o)(e[F]||[]).forEach((B,C)=>{a[`${F}${C+1}`]=B});function l(F){const q=c[F];return q?{x:q.x*d,y:q.y*s}:null}let b="";for(const[F,q]of f){const B=l(F),C=l(q);if(!B||!C)continue;const P=a[F],V=a[q],ee=nt(P,V);ee==="#00ff88"||ee==="#FFD700"?(b+=`<line x1="${B.x.toFixed(1)}" y1="${B.y.toFixed(1)}" x2="${C.x.toFixed(1)}" y2="${C.y.toFixed(1)}"
        stroke="${ee}" stroke-width="10" stroke-linecap="round" opacity="0.22"/>`,b+=`<line x1="${B.x.toFixed(1)}" y1="${B.y.toFixed(1)}" x2="${C.x.toFixed(1)}" y2="${C.y.toFixed(1)}"
        stroke="${ee}" stroke-width="3.5" stroke-linecap="round" opacity="0.95"/>`):b+=`<line x1="${B.x.toFixed(1)}" y1="${B.y.toFixed(1)}" x2="${C.x.toFixed(1)}" y2="${C.y.toFixed(1)}"
        stroke="${ee}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const h=48,$=64,p=13,u=16,m={normal:"#aaaaaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#111111",legende:"#7a28b8",légende:"#7a28b8"};for(const[F,q]of Object.entries(a)){const B=l(F);if(!B||!q)continue;const C=F.replace(/[0-9]/g,""),P=jt[C]||"#555",V=r.includes(q.cardId),ee=i==="attack"&&(["MIL","ATT"].includes(C)||V)&&!q.used||i==="defense"&&["GK","DEF","MIL"].includes(C)&&!q.used,se=n.includes(q.cardId);let re;i==="attack"?re=V?Math.max(1,Number(q.note_a)||0):C==="MIL"?Number(q.note_m)||0:Number(q.note_a)||0:i==="defense"?re=C==="GK"?Number(q.note_g)||0:C==="MIL"?Number(q.note_m)||0:Number(q.note_d)||0:re=Number(C==="GK"?q.note_g:C==="DEF"?q.note_d:C==="MIL"?q.note_m:q.note_a)||0,re=re+(q.boost||0),q.stadiumBonus&&(i==="attack"&&(C==="ATT"||C==="MIL")||i==="defense"&&(C==="GK"||C==="DEF"||C==="MIL")?re+=10:i||(re+=10));const Z=(B.x-h/2).toFixed(1),O=(B.y-$/2).toFixed(1),D=m[q==null?void 0:q.rarity]||m.normal;if(q.used){const Fe=typeof import.meta<"u"&&"/"||"/",we=`${typeof window<"u"&&((j=window.location)==null?void 0:j.origin)||""}${Fe}icons/carte-dos.png`.replace(/([^:])\/\//g,"$1/");b+=`<rect x="${Z}" y="${O}" width="${h}" height="${$}" rx="5" fill="#161616"/>`,b+=`<image href="${we}" xlink:href="${we}" x="${Z}" y="${O}" width="${h}" height="${$}" preserveAspectRatio="xMidYMid slice"/>`,b+=`<rect x="${Z}" y="${O}" width="${h}" height="${$}" rx="5" fill="none" stroke="${D}" stroke-width="2" opacity="0.7"/>`,b+=`<rect x="${Z}" y="${O}" width="${h}" height="${$}" rx="5" fill="rgba(0,0,0,0.01)" class="match-used-hit" data-card-id="${q.cardId}" data-role="${C}" style="cursor:pointer"/>`;continue}const ne=se?.45:1,ue=se?"#FFD700":D,ye=se?3:(q==null?void 0:q.rarity)==="legende"||(q==null?void 0:q.rarity)==="légende"||(q==null?void 0:q.rarity)==="pepite"||(q==null?void 0:q.rarity)==="pépite"?2.5:2,me=$-p-u;b+=`<defs><clipPath id="cp-${F}"><rect x="${Z}" y="${(B.y-$/2+p).toFixed(1)}" width="${h}" height="${me}"/></clipPath></defs>`,b+=`<rect x="${Z}" y="${O}" width="${h}" height="${$}" rx="5" fill="${P}" opacity="${ne}"/>`;const xe=Pe(q);xe&&(b+=`<image href="${xe}" xlink:href="${xe}" x="${Z}" y="${(B.y-$/2+p).toFixed(1)}" width="${h}" height="${me}" clip-path="url(#cp-${F})" preserveAspectRatio="xMidYMin slice"/>`),b+=`<rect x="${Z}" y="${O}" width="${h}" height="${p}" rx="4" fill="rgba(255,255,255,0.92)"/>`,b+=`<text x="${B.x.toFixed(1)}" y="${(B.y-$/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(q.name||"").slice(0,9)}</text>`;const Le=(B.y+$/2-u).toFixed(1);b+=`<rect x="${Z}" y="${Le}" width="${h}" height="${u}" fill="rgba(255,255,255,0.92)"/>`;{const Fe=ei(q.country_code);Fe?b+=`<image href="${Fe}" xlink:href="${Fe}" x="${(B.x-20).toFixed(1)}" y="${(B.y+$/2-u+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:b+=`<text x="${(B.x-13).toFixed(1)}" y="${(B.y+$/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${_o(q.country_code)}</text>`;const Ce=q.stadiumBonus?"#E87722":"#111";b+=`<text x="${B.x.toFixed(1)}" y="${(B.y+$/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="${Ce}" font-family="Arial Black">${re}</text>`;const we=wo(q);we?b+=`<image href="${we}" xlink:href="${we}" x="${(B.x+h/2-14).toFixed(1)}" y="${(B.y+$/2-u+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:q.clubName&&(b+=`<text x="${(B.x+14).toFixed(1)}" y="${(B.y+$/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(q.clubName||"").slice(0,3).toUpperCase()}</text>`),q.stadiumBonus&&(b+=`<rect x="${(B.x-h/2).toFixed(1)}" y="${(B.y-$/2).toFixed(1)}" width="${h}" height="${$}" rx="5" fill="none" stroke="#E87722" stroke-width="2" opacity="0.8"/>`)}b+=`<rect x="${Z}" y="${O}" width="${h}" height="${$}" rx="5" fill="${se?"rgba(255,255,255,0.12)":"none"}" stroke="${ue}" stroke-width="${ye}" opacity="${ne}"/>`,ee&&(b+=`<rect x="${Z}" y="${O}" width="${h}" height="${$}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${se?"selected":""}" data-card-id="${q.cardId}" data-role="${C}" style="cursor:pointer"/>`)}const A=38;return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-A} ${-A} ${d+A*2} ${s+A*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${b}
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${A.map((C,P)=>{const V=P<A.length-1?nt(C,A[P+1]):null,ee=!V||V==="#ff3333"||V==="#cc2222",se=V==="#00ff88"?"+2":V==="#FFD700"?"+1":"";c(C,B),C.stadiumBonus||B&&(B.club_id&&String(C.club_id)===String(B.club_id)||B.country_code&&(C.country_code,B.country_code));const re=C.evolution_bonus||C._evolution_bonus||0;return`
          ...
          ${P<A.length-1?`<div class="duel-link duel-link-${q}" data-idx="${P}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${ee?"rgba(255,255,255,0.12)":V};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${ee?"none":`0 0 8px ${V}`}">
  ...
  <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div id="duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center, rgba(10,61,30,0.4), rgba(10,61,30,0.92))"></div>
  </div>`;const $=()=>{const A=(j,F)=>e.querySelectorAll(j).forEach((q,B)=>{setTimeout(()=>{q.style.opacity="1",q.style.transform="translateY(0) scale(1)"},F+B*90)});A(".duel-card-home",150),A(".duel-card-ai",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((j,F)=>{setTimeout(()=>{j.style.opacity="1"},F*70)}),900),setTimeout(()=>{const j=e.querySelector("#vs-label");j&&(j.style.opacity="1",j.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(F=>F.style.opacity="1")},1250),setTimeout(()=>{p("score-home",o,800),p("score-ai",l,800)},1500)};function p(A,j,F){const q=document.getElementById(A);if(!q)return;const B=performance.now(),C=P=>{const V=Math.min(1,(P-B)/F);q.textContent=Math.round(j*(1-Math.pow(1-V,3))),V<1?requestAnimationFrame(C):q.textContent=j};requestAnimationFrame(C)}requestAnimationFrame($),t.attacker=b?"home":"ai";const u=b?ai():null;b&&(t.boostCard={value:u}),t.log.push({type:"duel",title:"Milieu de Terrain",homePlayers:n.map(A=>({name:A.name,note:ze(A,"MIL"),portrait:Pe(A),job:A.job,country_code:A.country_code,rarity:A.rarity,clubName:A.clubName,clubLogo:A.clubLogo})),aiPlayers:d.map(A=>({name:A.name,note:ze(A,"MIL"),portrait:Pe(A),job:A.job,country_code:A.country_code,rarity:A.rarity,clubName:A.clubName,clubLogo:A.clubLogo})),homeTotal:o,aiTotal:l,text:`Duel milieu : ${t.clubName} ${o} – ${l} IA → ${b?t.clubName+" attaque":"IA attaque"}`});const m=()=>{t.phase=t.attacker==="home"?"attack":"ai-attack",Ye(e,t,i),t.attacker==="ai"&&setTimeout(()=>ti(e,t,i),800)};setTimeout(()=>{const A=document.getElementById("score-home"),j=document.getElementById("score-ai"),F=document.getElementById(b?"duel-row-home":"duel-row-ai"),q=document.getElementById(b?"duel-row-ai":"duel-row-home"),B=b?A:j,C=b?j:A;B&&(B.style.fontSize="80px",B.style.color=b?"#FFD700":"#ff6b6b",B.style.animation="duelPulse .5s ease"+(b?", duelGlow 1.5s ease infinite .5s":"")),C&&(C.style.opacity="0.25"),setTimeout(()=>{F&&(F.style.transformOrigin="center",F.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",F.style.zIndex="5"),setTimeout(()=>{var V;const P=document.getElementById("duel-shock");if(P){const ee=(V=q||F)==null?void 0:V.getBoundingClientRect(),se=e.querySelector(".match-screen").getBoundingClientRect();ee&&(P.style.top=ee.top-se.top+ee.height/2+"px"),P.style.animation="shockwave .5s ease-out forwards"}q&&(q.style.transformOrigin="center",q.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var V;const P=document.getElementById("duel-finale");P&&(P.innerHTML=`
          ...
          <button id="start-match-btn" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,0.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">
            ▶ Commencer le match
          </button>`,P.style.transition="opacity .45s ease",P.style.opacity="1",P.style.pointerEvents="auto",(V=document.getElementById("start-match-btn"))==null||V.addEventListener("click",m))},600)},700)},2800)}function jn(e){if(e.type==="duel"&&(e.homeTotal!=null||e.aiTotal!=null)){const t=(e.homeTotal||0)>=(e.aiTotal||0);return`
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    ${(()=>{const D=window.innerWidth>=700,ne=(Y,oe,fe)=>{var w,T;const U=(t.gcDefs||[]).find(g=>g.name===Y.gc_type),te={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[U==null?void 0:U.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",W={purple:"#b06ce0",light_blue:"#00d4ef"}[U==null?void 0:U.color]||"#b06ce0",de=(U==null?void 0:U.name)||Y.gc_type,ve=(U==null?void 0:U.effect)||((w=Re[Y.gc_type])==null?void 0:w.desc)||"",Te=U!=null&&U.image_url?`/icons/${U.image_url}`:null,$e=((T=Re[Y.gc_type])==null?void 0:T.icon)||"⚡",be=Math.round(fe*.22),ke=Math.round(fe*.22),qe=fe-be-ke,je=de.length>12?7:9;return`<div class="gc-mini" data-gc-id="${Y.id}" data-gc-type="${Y.gc_type}"
          style="box-sizing:border-box;width:${oe}px;height:${fe}px;border-radius:10px;border:2px solid ${W};background:${te};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
        ...
        </div>`},ue=(Y,oe)=>{var fe;return`<div id="boost-card"
          style="box-sizing:border-box;width:${Y}px;height:${oe}px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:2px solid #87CEEB;border-radius:10px;cursor:pointer;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:${Math.round(oe*.04)}px;text-align:center;flex-shrink:0">
  ...
  `,document.body.appendChild(n);let o=!1;const l=()=>{o||(o=!0,n.remove(),i())};n.addEventListener("click",l),setTimeout(l,2e3)}function qt(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function ui(e,t,i,n=null,d=null){var h,$;if(t.phase!=="attack"){qt("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){qt(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const s=Object.entries(t.homeTeam).flatMap(([p,u])=>(u||[]).filter(m=>m.used).map(m=>({...m,_line:m._line||p}))),r=t.homeSubs.filter(p=>!t.usedSubIds.includes(p.cardId));if(!s.length){qt("Aucun joueur utilisé à remplacer");return}if(!r.length){qt("Aucun remplaçant disponible");return}let c=Math.max(0,s.findIndex(p=>p.cardId===d));const f=((h=s[c])==null?void 0:h._line)||(($=s[c])==null?void 0:$.job);let a=n?Math.max(0,r.findIndex(p=>p.cardId===n)):Math.max(0,r.findIndex(p=>p.job===f)),o=!1;const l=document.createElement("div");l.id="sub-overlay",l.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function b(){var q,B,C,P,V,ee;const p=s[c],u=r[a],m=Math.min(130,Math.round((window.innerWidth-90)/2)),A=Math.round(m*1.35),j=se=>`background:rgba(255,255,255,0.12);border:none;color:${se?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${se?"default":"pointer"};flex-shrink:0`;l.innerHTML=`
    ...
    </div>`,(q=l.querySelector("#sub-close"))==null||q.addEventListener("click",()=>l.remove()),(B=l.querySelector("#out-up"))==null||B.addEventListener("click",()=>{c>0&&(c--,b())}),(C=l.querySelector("#out-down"))==null||C.addEventListener("click",()=>{c<s.length-1&&(c++,b())}),(P=l.querySelector("#in-up"))==null||P.addEventListener("click",()=>{a>0&&(a--,b())}),(V=l.querySelector("#in-down"))==null||V.addEventListener("click",()=>{a<r.length-1&&(a++,b())});const F=(se,re,Z,O)=>{const D=l.querySelector("#"+se);if(!D)return;let ne=0;D.addEventListener("touchstart",ue=>{ne=ue.touches[0].clientY},{passive:!0}),D.addEventListener("touchend",ue=>{const ye=ue.changedTouches[0].clientY-ne;if(Math.abs(ye)<30)return;const me=re();ye<0&&me<O-1?(Z(me+1),b()):ye>0&&me>0&&(Z(me-1),b())},{passive:!0})};F("in-panel",()=>a,se=>a=se,r.length),F("out-panel",()=>c,se=>c=se,s.length),(ee=l.querySelector("#sub-confirm"))==null||ee.addEventListener("click",se=>{if(se.preventDefault(),se.stopPropagation(),o)return;o=!0;const re=s[c],Z=r[a];if(!re||!Z)return;let O=null,D=-1;for(const[ue,ye]of Object.entries(t.homeTeam)){const me=(ye||[]).findIndex(xe=>xe.cardId===re.cardId);if(me!==-1){O=ue,D=me;break}}if(D===-1||!O){qt("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),l.remove();return}const ne={...Z,_line:O,_col:re._col||0,used:!1,boost:0};t.homeTeam[O].splice(D,1,ne),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(Z.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:re.name,firstname:re.firstname,note:ze(re,O),portrait:Pe(re),job:re.job,country_code:re.country_code,rarity:re.rarity,clubName:re.clubName,clubLogo:re.clubLogo},inPlayer:{name:Z.name,firstname:Z.firstname,note:ze(Z,O),portrait:Pe(Z),job:Z.job,country_code:Z.country_code,rarity:Z.rarity,clubName:Z.clubName,clubLogo:Z.clubLogo},text:`🔄 ${Z.firstname} ${Z.name} remplace ${re.firstname} ${re.name}`}),l.remove(),Pn(re,Z,()=>Ye(e,t,i))})}document.body.appendChild(l),b()}function Nn(e,t,i,n,d){var $,p;const s=(n.gcDefs||[]).find(u=>u.name===t),r=Re[t]||{icon:"⚡",desc:"Carte spéciale."},c={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[s==null?void 0:s.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",f={purple:"#b06ce0",light_blue:"#00d4ef"}[s==null?void 0:s.color]||"#b06ce0",a=(s==null?void 0:s.name)||t,o=(s==null?void 0:s.effect)||r.desc,l=s!=null&&s.image_url?`/icons/${s.image_url}`:null,b=r.icon||"⚡",h=document.createElement("div");h.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",h.innerHTML=`
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(s);let c=!1;const f=()=>{c||(c=!0,s.remove(),setTimeout(()=>d(),50))};s.addEventListener("click",f),setTimeout(f,3500)}async function Dt(e,t,i){var o,l;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:n}=i,d=t.homeScore>t.aiScore,s=t.homeScore===t.aiScore,r=d?"victoire":s?"nul":"defaite",c=Wo(t.mode,r);t.matchId&&await z.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:d?n.profile.id:null,home_credits_reward:c,played_at:new Date().toISOString()}).eq("id",t.matchId);const f={credits:(n.profile.credits||0)+c,matches_played:(n.profile.matches_played||0)+1};d?f.wins=(n.profile.wins||0)+1:s?f.draws=(n.profile.draws||0)+1:f.losses=(n.profile.losses||0)+1,await z.from("users").update(f).eq("id",n.profile.id),await i.refreshProfile();const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",a.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const F=async(P=!0)=>{$=!0,p&&(z.removeChannel(p),p=null),P&&await z.rpc("cancel_matchmaking",{p_user_id:l.profile.id}).catch(()=>{})};(C=document.getElementById("mm-cancel"))==null||C.addEventListener("click",async()=>{try{await F(!0)}catch{}Ve(e),b("home")});const q=async(P,V,ee)=>{if($)return;$=!0,p&&(z.removeChannel(p),p=null);const se=document.getElementById("mm-status");se&&(se.textContent="Adversaire trouvé !"),await new Promise(re=>setTimeout(re,600)),e.isConnected&&ko(e,t,P,ee,r,c,a,o)};if(a){const P=[{range:50,delay:0},{range:100,delay:15e3},{range:200,delay:3e4},{range:400,delay:45e3},{range:800,delay:6e4}];let V=0,ee=!1;const se=async()=>{if($||ee)return;const{range:re}=P[V],Z=document.getElementById("mm-range");Z&&(Z.textContent=`Plage MMR : ±${re}`);const{data:O,error:D}=await z.rpc("try_matchmake_ranked",{p_user_id:l.profile.id,p_deck_id:i,p_mmr:u,p_range:re});if($)return;if(D||!(O!=null&&O.success)){h("Erreur matchmaking ranked","error"),Ve(e),b("home");return}if(O.matched){ee=!0,q(O.match_id,O.opponent_id,!1);return}const ne=document.getElementById("mm-status");if(ne&&(ne.textContent="En attente d'un adversaire..."),p||(p=z.channel("matchmaking-"+l.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${l.profile.id}`},ue=>{const ye=ue.new;ye.status==="matched"&&ye.match_id&&!ee&&(ee=!0,q(ye.match_id,ye.matched_with,!0))}).subscribe()),V<P.length-1){const ue=P[V+1].delay-P[V].delay;V++,setTimeout(()=>{!$&&!ee&&se()},ue)}};await se()}else{const{data:P,error:V}=await z.rpc("try_matchmake",{p_user_id:l.profile.id,p_deck_id:i});if(V||!(P!=null&&P.success)){h("Erreur de matchmaking","error"),Ve(e),b("home");return}if(P.matched){q(P.match_id,P.opponent_id,!1);return}const ee=document.getElementById("mm-status");ee&&(ee.textContent="En attente d'un autre joueur..."),p=z.channel("matchmaking-"+l.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${l.profile.id}`},se=>{const re=se.new;re.status==="matched"&&re.match_id&&q(re.match_id,re.matched_with,!0)}).subscribe()}}async function Vn(e,t,i){const{state:n,navigate:d,toast:s}=t;try{(z.getChannels?z.getChannels():[]).forEach(a=>{const o=a.topic||"";(o.includes("matchmaking")||o.includes("pvp-match")||o.includes("friend-invite"))&&z.removeChannel(a)})}catch{}const{data:r}=await z.from("matches").select("home_id, away_id, status").eq("id",i).single();if(!r){s("Match introuvable","error"),d("home");return}if(r.status==="finished"){s("Ce match est terminé","info"),d("home");return}const c=r.home_id===n.profile.id;ko(e,t,i,c,[],[])}async function ko(e,t,i,n,d=[],s=[],r=!1,c=null){const{state:f,navigate:a,toast:o}=t,l=n?"p1":"p2",b=n?"p2":"p1",h=(d||[]).map(w=>w.id),$=(d||[]).map(w=>({id:w.id,gc_type:w.gc_type,_gcDef:w._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:p}=await z.from("matches").select("*").eq("id",i).single();if(!p){o("Match introuvable","error"),a("home");return}async function u(){const[{data:w},{data:T},{data:g},{data:y}]=await Promise.all([z.rpc("get_deck_for_match",{p_deck_id:p.home_deck_id}),z.rpc("get_deck_for_match",{p_deck_id:p.away_deck_id}),z.from("users").select("id,pseudo,club_name").eq("id",p.home_id).single(),z.from("users").select("id,pseudo,club_name").eq("id",p.away_id).single()]),x=I=>{const N=Number(I.evolution_bonus)||0;return{cardId:I.card_id,position:I.position,id:I.id,firstname:I.firstname,name:I.surname_encoded,country_code:I.country_code,club_id:I.club_id,job:I.job,job2:I.job2,note_g:(Number(I.note_g)||0)+(I.job==="GK"||I.job2==="GK"&&Number(I.note_g)>0?N:0),note_d:(Number(I.note_d)||0)+(I.job==="DEF"||I.job2==="DEF"&&Number(I.note_d)>0?N:0),note_m:(Number(I.note_m)||0)+(I.job==="MIL"||I.job2==="MIL"&&Number(I.note_m)>0?N:0),note_a:(Number(I.note_a)||0)+(I.job==="ATT"||I.job2==="ATT"&&Number(I.note_a)>0?N:0),evolution_bonus:N,rarity:I.rarity,skin:I.skin,hair:I.hair,hair_length:I.hair_length,clubName:I.club_encoded_name||null,clubLogo:I.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},_=((w==null?void 0:w.starters)||[]).map(I=>x(I)),v=((T==null?void 0:T.starters)||[]).map(I=>x(I)),k=(w==null?void 0:w.formation)||"4-4-2",L=(T==null?void 0:T.formation)||"4-4-2";let E=bt(_,k),M=bt(v,L);const S=((w==null?void 0:w.subs)||[]).map(I=>x(I));return stadiumDef&&n&&(E=Ct(E,stadiumDef),Ot(S,stadiumDef)),{p1Team:E,p2Team:M,p1Subs:S,p2Subs:((T==null?void 0:T.subs)||[]).map(I=>x(I)),p1Formation:k,p2Formation:L,p1Name:(g==null?void 0:g.club_name)||(g==null?void 0:g.pseudo)||"Joueur 1",p2Name:(y==null?void 0:y.club_name)||(y==null?void 0:y.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:n?h:[],gc_p2:n?[]:h,gcCardsFull_p1:n?$:[],gcCardsFull_p2:n?[]:$,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:s||[],lastActionAt:new Date().toISOString()}}let m=p.game_state&&Object.keys(p.game_state).length?p.game_state:null;if(!m)if(n){m=await u();const{data:w}=await z.from("matches").select("game_state").eq("id",i).single();!(w!=null&&w.game_state)||!Object.keys(w.game_state).length?await z.from("matches").update({game_state:m,turn_user_id:p.home_id}).eq("id",i):m=w.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let w=0;w<30&&!m;w++){await new Promise(g=>setTimeout(g,400));const{data:T}=await z.from("matches").select("game_state").eq("id",i).single();T!=null&&T.game_state&&Object.keys(T.game_state).length&&(m=T.game_state)}if(!m){o("Erreur de synchronisation","error"),a("home");return}m.gc_p2=h,m.gcCardsFull_p2=$,await z.from("matches").update({game_state:m}).eq("id",i)}let A=!1,j=null,F=!1;const q=new Set,B=new Set;async function C(w){var M,S;try{z.removeChannel(P)}catch{}const T=m[l+"Score"]||0,g=m[b+"Score"]||0;let y,x;w.winner_id?(y=w.winner_id===f.profile.id,x=!1):w.forfeit?(y=T>g,x=!1):(x=T===g,y=T>g);let _="";if(r&&n)try{const{data:I}=await z.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",p.home_id).single(),{data:N}=await z.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",p.away_id).single();if(I&&N){const Q=y?1:x?.5:0,R=1-Q,K=I.placement_matches<10,H=N.placement_matches<10,X=ji(I.mmr,I.mmr_deviation,I.mmr_volatility,N.mmr,N.mmr_deviation,Q===1?1:Q===0?0:.5,K),ae=ji(N.mmr,N.mmr_deviation,N.mmr_volatility,I.mmr,I.mmr_deviation,R===1?1:R===0?0:.5,H);await z.rpc("update_mmr_after_ranked",{p_match_id:i,p_winner_id:x?null:y?p.home_id:p.away_id,p_home_id:p.home_id,p_away_id:p.away_id,p_home_delta:X.delta,p_away_delta:ae.delta,p_home_new_rd:X.newRd,p_away_new_rd:ae.newRd,p_home_new_vol:X.newSigma,p_away_new_vol:ae.newSigma});const pe=X.delta,ce=X.newMmr,ge=Pt(ce),he=pe>=0?"+":"",Ie=pe>=0?"#4caf50":"#ff6b6b",Se=Pt(I.mmr);_=`
      ...
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(v),(S=v.querySelector("#pvp-end-home"))==null||S.addEventListener("click",()=>{v.remove(),Ve(e),a("home")})}const P=z.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},w=>{const T=w.new;try{if(T.status==="finished"||T.forfeit){if(A)return;A=!0,j&&(clearInterval(j),j=null),T.game_state&&(m=T.game_state),C(T);return}if(T.game_state){const g=m;m=T.game_state;const y=m._lastGC;if(y&&y.seq&&!B.has(y.seq)&&(B.add(y.seq),y.by!==l)){Le(y.type,y.by,()=>ne());return}const x=g[l+"Score"]||0,_=g[b+"Score"]||0,v=m[l+"Score"]||0,k=m[b+"Score"]||0,L=v>x,E=k>_;if((L||E)&&!q.has(m.round)){q.add(m.round);const M=[...m.log||[]].reverse().find(I=>I.isGoal),S=((M==null?void 0:M.homePlayers)||[]).map(I=>({name:I.name,note:I.note,portrait:I.portrait,job:I.job}));xe(S,v,k,L,()=>ne());return}ne()}}catch(g){console.error("[PvP] crash:",g)}}).subscribe();async function V(w){Object.assign(m,w),m.lastActionAt=new Date().toISOString();const{error:T}=await z.from("matches").update({game_state:m}).eq("id",i);T&&o("Erreur de synchronisation","error");try{ne()}catch(g){console.error("[PvP] renderPvpScreen crash:",g)}}async function ee(){if(A)return;A=!0,j&&(clearInterval(j),j=null);const w=n?p.away_id:p.home_id,T=n?"p2":"p1",g=n?"p1":"p2",y={...m,[T+"Score"]:3,[g+"Score"]:0,phase:"finished"};try{await z.from("matches").update({status:"finished",forfeit:!0,winner_id:w,home_score:y.p1Score||0,away_score:y.p2Score||0,game_state:y}).eq("id",i)}catch{}try{z.removeChannel(P)}catch{}setTimeout(()=>{Ve(e),a("home")},800)}const se={BOOST_STAT:({value:w=1,count:T=1,roles:g=[]},y,x)=>{const _=y[l+"Team"],v=Object.entries(_).filter(([k])=>!g.length||g.includes(k)).flatMap(([k,L])=>L.filter(E=>!E.used).map(E=>({...E,_line:k})));if(!v.length){y.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),x(y);return}re(v,T,`Choisir ${T} joueur(s) à booster (+${w})`,k=>{k.forEach(L=>{const E=(_[L._line]||[]).find(M=>M.cardId===L.cardId);E&&(E.boost=(E.boost||0)+w,y.log.push({text:`⚡ +${w} sur ${E.name}`,type:"info"}))}),y[l+"Team"]=_,x(y)})},DEBUFF_STAT:({value:w=1,count:T=1,roles:g=[],target:y="ai"},x,_)=>{const v=y==="home"?l:b,k=x[v+"Team"],L=y==="home"?"allié":"adverse",E=Object.entries(k).filter(([M])=>!g.length||g.includes(M)).flatMap(([M,S])=>S.map(I=>({...I,_line:M})));if(!E.length){x.log.push({text:`🎯 Aucun joueur ${L}`,type:"info"}),_(x);return}re(E,T,`Choisir ${T} joueur(s) ${L}(s) (-${w})`,M=>{M.forEach(S=>{const I=(k[S._line]||[]).find(N=>N.cardId===S.cardId);I&&(I.boost=(I.boost||0)-w,x.log.push({text:`🎯 -${w} sur ${I.name}`,type:"info"}))}),x[v+"Team"]=k,_(x)})},GRAY_PLAYER:({count:w=1,roles:T=[],target:g="ai"},y,x)=>{const _=g==="home"?l:b,v=y[_+"Team"],k=g==="home"?"allié":"adverse",L=Object.entries(v).filter(([E])=>!T.length||T.includes(E)).flatMap(([E,M])=>M.filter(S=>!S.used).map(S=>({...S,_line:E})));if(!L.length){y.log.push({text:`❌ Aucun joueur ${k}`,type:"info"}),x(y);return}re(L,w,`Choisir ${w} joueur(s) ${k}(s) à exclure`,E=>{const M="usedCardIds_"+_,S=new Set(y[M]||[]);E.forEach(I=>{const N=(v[I._line]||[]).find(Q=>Q.cardId===I.cardId);N&&(N.used=!0,S.add(I.cardId),y.log.push({text:`❌ ${N.name} exclu !`,type:"info"}))}),y[M]=[...S],y[_+"Team"]=v,x(y)})},REVIVE_PLAYER:({count:w=1,roles:T=[]},g,y)=>{const x=g[l+"Team"],_=Object.entries(x).filter(([v])=>!T.length||T.includes(v)).flatMap(([v,k])=>k.filter(L=>L.used).map(L=>({...L,_line:v})));if(!_.length){g.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),y(g);return}re(_,w,`Choisir ${w} joueur(s) à ressusciter`,v=>{v.forEach(k=>{const L=(x[k._line]||[]).find(E=>E.cardId===k.cardId);L&&(L.used=!1,g.log.push({text:`💫 ${L.name} ressuscité !`,type:"info"}))}),g[l+"Team"]=x,y(g)})},REMOVE_GOAL:({},w,T)=>{const g=b+"Score";w[g]>0?(w[g]--,w.log.push({text:"🚫 Dernier but annulé !",type:"info"})):w.log.push({text:"🚫 Aucun but à annuler",type:"info"}),T(w)},ADD_GOAL_DRAW:({},w,T)=>{w[l+"Score"]===w[b+"Score"]?(w[l+"Score"]++,w.log.push({text:"🎯 But bonus !",type:"info"})):w.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),T(w)},ADD_SUB:({value:w=1},T,g)=>{T.maxSubs=(T.maxSubs||3)+w,T.log.push({text:`🔄 +${w} remplacement(s)`,type:"info"}),g(T)},CUSTOM:({},w,T)=>T(w)};function re(w,T,g,y){const x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let _=[];function v(){var L,E;const k=w.map(M=>{const S={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[M._line]||"#555",I=ze(M,M._line)+(M.boost||0),Q=_.find(K=>K.cardId===M.cardId)?"#FFD700":"rgba(255,255,255,0.25)",R=M.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${M.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${Q};background:${S};overflow:hidden;cursor:pointer;${R}">
        ...
        </div>`,(L=x.querySelector("#pp-close"))==null||L.addEventListener("click",()=>x.remove()),x.querySelectorAll(".pp-item").forEach(M=>{M.addEventListener("click",()=>{const S=M.dataset.cid,I=w.find(Q=>Q.cardId===S),N=_.findIndex(Q=>Q.cardId===S);I&&(N>-1?_.splice(N,1):_.length<T&&_.push(I),v())})}),(E=x.querySelector("#pp-confirm"))==null||E.addEventListener("click",()=>{x.remove(),y(_)})}v(),document.body.appendChild(x)}async function Z(w,T){const y=(m["gcCardsFull_"+l]||[]).find(k=>k.id===w),x=(y==null?void 0:y._gcDef)||(m.gcDefs||[]).find(k=>{var L;return k.name===T||((L=k.name)==null?void 0:L.toLowerCase().trim())===(T==null?void 0:T.toLowerCase().trim())}),_=[...m["usedGc_"+l]||[],w],v={type:T,by:l,seq:(m._gcAnimSeq||0)+1};B.add(v.seq),Le(T,l,async()=>{if(x!=null&&x.effect_type&&x.effect_type!=="CUSTOM"){const L=se[x.effect_type];if(L){const E={...m};L(x.effect_params||{},E,async M=>{M["usedGc_"+l]=_,M._lastGC=v,M._gcAnimSeq=v.seq,await V(M)});return}}const k={...m};switch(T){case"Remplacement+":k.maxSubs=(k.maxSubs||3)+1,k.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const L=b+"Score";k[L]>0&&(k[L]--,k.log.push({text:"🚫 But annulé",type:"info"}));break}}k["usedGc_"+l]=_,k._lastGC=v,k._gcAnimSeq=v.seq,await V(k)})}function O(w,T){const g="usedCardIds_"+w,y=new Set(m[g]||[]);T.forEach(x=>y.add(x)),m[g]=[...y]}function D(){for(const w of["p1","p2"]){const T=new Set(m["usedCardIds_"+w]||[]),g=m[w+"Team"];if(g)for(const y of["GK","DEF","MIL","ATT"])(g[y]||[]).forEach(x=>{x.used=T.has(x.cardId)})}}function ne(){var Ke,Ze,it,ut,ft,gt;if(m.phase==="reveal")return ue();if(m.phase==="midfield")return me();if(m.phase==="finished")return je();const w=m[l+"Team"],T=m[b+"Team"];D();const g=m[l+"Score"],y=m[b+"Score"],x=m[l+"Name"],_=m[b+"Name"],v=m.phase===l+"-attack",k=m.phase===l+"-defense",L=Array.isArray(m["selected_"+l])?m["selected_"+l]:[],E=L.map(G=>G.cardId),M=window.innerWidth>=700,S=m[l+"Subs"]||[],I=m["usedSubIds_"+l]||[],N=S.filter(G=>!I.includes(G.cardId)),Q=m["gcCardsFull_"+l]||[],R=m["usedGc_"+l]||[],K=Q.filter(G=>!R.includes(G.id)),H=m.boostOwner===l&&!m.boostUsed,X=!["GK","DEF","MIL","ATT"].some(G=>(T[G]||[]).some(le=>!le.used)),ae=[...w.MIL||[],...w.ATT||[]].filter(G=>!G.used),pe=v&&X&&ae.length===0?[...w.GK||[],...w.DEF||[]].filter(G=>!G.used).map(G=>G.cardId):[];function ce(G,le,J){var lt,wt;const ie=G._gcDef,Ae={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ie==null?void 0:ie.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",_e={purple:"#b06ce0",light_blue:"#00d4ef"}[ie==null?void 0:ie.color]||"#b06ce0",Xe=(ie==null?void 0:ie.name)||G.gc_type,Qe=(ie==null?void 0:ie.effect)||((lt=Re[G.gc_type])==null?void 0:lt.desc)||"",ot=ie!=null&&ie.image_url?`/icons/${ie.image_url}`:null,dt=((wt=Re[G.gc_type])==null?void 0:wt.icon)||"⚡",mt=Math.round(J*.22),vt=Math.round(J*.22),rt=J-mt-vt,Tt=Xe.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${G.id}" data-gc-type="${G.gc_type}"
        style="box-sizing:border-box;width:${le}px;height:${J}px;border-radius:10px;border:2px solid ${_e};background:${Ae};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ...
        </div>`}if(v&&((J=m.pendingAttack)!=null&&J.players)){const ie=m.pendingAttack;return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #00ff88">
          <div style="font-size:8px;color:#00ff88;letter-spacing:2px;margin-bottom:4px;text-transform:uppercase">⚔️ VOUS ATTAQUEZ</div>
      ...
      </div>`;function Be(){const G=e.querySelector(".match-screen");if(!G)return;const le=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);G.style.height=le+"px",G.style.minHeight=le+"px",G.style.maxHeight=le+"px",G.style.overflow="hidden";const J=e.querySelector("#mobile-action-bar"),ie=e.querySelector("#mobile-play-area");J&&ie&&(ie.style.paddingBottom=J.offsetHeight+"px")}if(Be(),setTimeout(Be,120),setTimeout(Be,400),F||(F=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",Be),window.visualViewport.addEventListener("scroll",Be)),window.addEventListener("resize",Be)),function(){const le=e.querySelector(".terrain-wrapper svg");le&&(le.removeAttribute("width"),le.removeAttribute("height"),le.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",le.setAttribute("viewBox","-26 -26 352 352"),le.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),e.querySelectorAll(".match-slot-hit").forEach(G=>{G.addEventListener("click",()=>{if(!v&&!k)return;const le=G.dataset.cardId,J=G.dataset.role,ie=(w[J]||[]).find(Qe=>Qe.cardId===le);if(!ie||ie.used)return;const Ae=pe.includes(le);if(v&&!["MIL","ATT"].includes(J)&&!Ae)return;Array.isArray(m["selected_"+l])||(m["selected_"+l]=[]);const _e=m["selected_"+l],Xe=_e.findIndex(Qe=>Qe.cardId===le);Xe>-1?_e.splice(Xe,1):_e.length<3&&_e.push({...ie,_role:J}),ne()})}),e.querySelectorAll(".match-used-hit").forEach(G=>{G.addEventListener("click",()=>we(G.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(G=>{G.addEventListener("click",()=>we())}),(Ke=e.querySelector("#pvp-sub-open"))==null||Ke.addEventListener("click",()=>we()),e.querySelectorAll(".pvp-gc-mini").forEach(G=>{G.addEventListener("click",()=>Fe(G.dataset.gcId,G.dataset.gcType))}),(Ze=e.querySelector("#pvp-boost-card"))==null||Ze.addEventListener("click",()=>Ce()),(it=e.querySelector("#pvp-action"))==null||it.addEventListener("click",G=>{v?G.currentTarget.dataset.pass==="1"||!$e(l)?U():te():k&&W()}),(ut=e.querySelector("#pvp-quit"))==null||ut.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&ee()}),(ft=e.querySelector("#pvp-view-opp"))==null||ft.addEventListener("click",()=>oe()),(gt=e.querySelector("#pvp-toggle-history"))==null||gt.addEventListener("click",()=>fe()),j&&(clearInterval(j),j=null),(v||k)&&!A){let G=30,le=!1;const J=()=>document.getElementById("pvp-timer"),ie=()=>{J()&&(J().textContent=G+"s",J().style.color=le?"#ff4444":"#fff")};ie(),j=setInterval(()=>{G--,G<0?le?(clearInterval(j),j=null,v&&!$e(l)?U():ee()):(le=!0,G=15,ie()):ie()},1e3)}}function ue(){et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
    ...
    </div>`,l==="p1"&&setTimeout(async()=>{await V({phase:"midfield"})},5e3)}let ye=!1;function me(){if(ye)return;const w=m[l+"Team"].MIL||[],T=m[b+"Team"].MIL||[];function g(K){return K.reduce((H,X)=>H+ze(X,"MIL"),0)}function y(K){let H=0;for(let X=0;X<K.length-1;X++){const ae=nt(K[X],K[X+1]);ae==="#00ff88"?H+=2:ae==="#FFD700"&&(H+=1)}return H}const x=g(w)+y(w),_=g(T)+y(T),v=x>=_;function k(K,H,X){return`<div id="duel-row-${X}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      ...
      <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:20px;background:linear-gradient(180deg,#0a1628,#1a0a2e);padding:24px;text-align:center;color:#fff">
        ...
        ${!se&&ee?'<div style="font-size:13px;color:rgba(255,255,255,0.5);animation:lobbyPulse 1.5s ease-in-out infinite">En attente de ton ami...</div>':""}
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(_),(M=_.querySelector("#pvp-end-home"))==null||M.addEventListener("click",()=>{_.remove(),Ve(e),c("home")})}const P=z.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},w=>{const T=w.new;try{if(T.status==="finished"||T.forfeit){if(u)return;u=!0,j&&(clearInterval(j),j=null),T.game_state&&(p=T.game_state),C(T);return}if(T.game_state){const g=p;p=T.game_state;const y=p._lastGC;if(y&&y.seq&&!B.has(y.seq)&&(B.add(y.seq),y.by!==a)){Le(y.type,y.by,()=>ne());return}const x=g[a+"Score"]||0,_=g[o+"Score"]||0,v=p[a+"Score"]||0,k=p[o+"Score"]||0,L=v>x,E=k>_;if((L||E)&&!q.has(p.round)){q.add(p.round);const M=[...p.log||[]].reverse().find(I=>I.isGoal),S=((M==null?void 0:M.homePlayers)||[]).map(I=>({name:I.name,note:I.note,portrait:I.portrait,job:I.job}));xe(S,v,k,L,()=>ne());return}ne()}}catch(g){console.error("[PvP] crash:",g)}}).subscribe();async function V(w){Object.assign(p,w),p.lastActionAt=new Date().toISOString();const{error:T}=await z.from("matches").update({game_state:p}).eq("id",i);T&&f("Erreur de synchronisation","error");try{ne()}catch(g){console.error("[PvP] renderPvpScreen crash:",g)}}async function ee(){if(u)return;u=!0,j&&(clearInterval(j),j=null);const w=n?h.away_id:h.home_id,T=n?"p2":"p1",g=n?"p1":"p2",y={...p,[T+"Score"]:3,[g+"Score"]:0,phase:"finished"};try{await z.from("matches").update({status:"finished",forfeit:!0,winner_id:w,home_score:y.p1Score||0,away_score:y.p2Score||0,game_state:y}).eq("id",i)}catch{}try{z.removeChannel(P)}catch{}setTimeout(()=>{Ve(e),c("home")},800)}const se={BOOST_STAT:({value:w=1,count:T=1,roles:g=[]},y,x)=>{const _=y[a+"Team"],v=Object.entries(_).filter(([k])=>!g.length||g.includes(k)).flatMap(([k,L])=>L.filter(E=>!E.used).map(E=>({...E,_line:k})));if(!v.length){y.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),x(y);return}re(v,T,`Choisir ${T} joueur(s) à booster (+${w})`,k=>{k.forEach(L=>{const E=(_[L._line]||[]).find(M=>M.cardId===L.cardId);E&&(E.boost=(E.boost||0)+w,y.log.push({text:`⚡ +${w} sur ${E.name}`,type:"info"}))}),y[a+"Team"]=_,x(y)})},DEBUFF_STAT:({value:w=1,count:T=1,roles:g=[],target:y="ai"},x,_)=>{const v=y==="home"?a:o,k=x[v+"Team"],L=y==="home"?"allié":"adverse",E=Object.entries(k).filter(([M])=>!g.length||g.includes(M)).flatMap(([M,S])=>S.map(I=>({...I,_line:M})));if(!E.length){x.log.push({text:`🎯 Aucun joueur ${L}`,type:"info"}),_(x);return}re(E,T,`Choisir ${T} joueur(s) ${L}(s) (-${w})`,M=>{M.forEach(S=>{const I=(k[S._line]||[]).find(N=>N.cardId===S.cardId);I&&(I.boost=(I.boost||0)-w,x.log.push({text:`🎯 -${w} sur ${I.name}`,type:"info"}))}),x[v+"Team"]=k,_(x)})},GRAY_PLAYER:({count:w=1,roles:T=[],target:g="ai"},y,x)=>{const _=g==="home"?a:o,v=y[_+"Team"],k=g==="home"?"allié":"adverse",L=Object.entries(v).filter(([E])=>!T.length||T.includes(E)).flatMap(([E,M])=>M.filter(S=>!S.used).map(S=>({...S,_line:E})));if(!L.length){y.log.push({text:`❌ Aucun joueur ${k}`,type:"info"}),x(y);return}re(L,w,`Choisir ${w} joueur(s) ${k}(s) à exclure`,E=>{const M="usedCardIds_"+_,S=new Set(y[M]||[]);E.forEach(I=>{const N=(v[I._line]||[]).find(Q=>Q.cardId===I.cardId);N&&(N.used=!0,S.add(I.cardId),y.log.push({text:`❌ ${N.name} exclu !`,type:"info"}))}),y[M]=[...S],y[_+"Team"]=v,x(y)})},REVIVE_PLAYER:({count:w=1,roles:T=[]},g,y)=>{const x=g[a+"Team"],_=Object.entries(x).filter(([v])=>!T.length||T.includes(v)).flatMap(([v,k])=>k.filter(L=>L.used).map(L=>({...L,_line:v})));if(!_.length){g.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),y(g);return}re(_,w,`Choisir ${w} joueur(s) à ressusciter`,v=>{v.forEach(k=>{const L=(x[k._line]||[]).find(E=>E.cardId===k.cardId);L&&(L.used=!1,g.log.push({text:`💫 ${L.name} ressuscité !`,type:"info"}))}),g[a+"Team"]=x,y(g)})},REMOVE_GOAL:({},w,T)=>{const g=o+"Score";w[g]>0?(w[g]--,w.log.push({text:"🚫 Dernier but annulé !",type:"info"})):w.log.push({text:"🚫 Aucun but à annuler",type:"info"}),T(w)},ADD_GOAL_DRAW:({},w,T)=>{w[a+"Score"]===w[o+"Score"]?(w[a+"Score"]++,w.log.push({text:"🎯 But bonus !",type:"info"})):w.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),T(w)},ADD_SUB:({value:w=1},T,g)=>{T.maxSubs=(T.maxSubs||3)+w,T.log.push({text:`🔄 +${w} remplacement(s)`,type:"info"}),g(T)},CUSTOM:({},w,T)=>T(w)};function re(w,T,g,y){const x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let _=[];function v(){var L,E;const k=w.map(M=>{const S={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[M._line]||"#555",I=ze(M,M._line)+(M.boost||0),Q=_.find(K=>K.cardId===M.cardId)?"#FFD700":"rgba(255,255,255,0.25)",R=M.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${M.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${Q};background:${S};overflow:hidden;cursor:pointer;${R}">
        ...
        </div>`,(L=x.querySelector("#pp-close"))==null||L.addEventListener("click",()=>x.remove()),x.querySelectorAll(".pp-item").forEach(M=>{M.addEventListener("click",()=>{const S=M.dataset.cid,I=w.find(Q=>Q.cardId===S),N=_.findIndex(Q=>Q.cardId===S);I&&(N>-1?_.splice(N,1):_.length<T&&_.push(I),v())})}),(E=x.querySelector("#pp-confirm"))==null||E.addEventListener("click",()=>{x.remove(),y(_)})}v(),document.body.appendChild(x)}async function Z(w,T){const y=(p["gcCardsFull_"+a]||[]).find(k=>k.id===w),x=(y==null?void 0:y._gcDef)||(p.gcDefs||[]).find(k=>{var L;return k.name===T||((L=k.name)==null?void 0:L.toLowerCase().trim())===(T==null?void 0:T.toLowerCase().trim())}),_=[...p["usedGc_"+a]||[],w],v={type:T,by:a,seq:(p._gcAnimSeq||0)+1};B.add(v.seq),Le(T,a,async()=>{if(x!=null&&x.effect_type&&x.effect_type!=="CUSTOM"){const L=se[x.effect_type];if(L){const E={...p};L(x.effect_params||{},E,async M=>{M["usedGc_"+a]=_,M._lastGC=v,M._gcAnimSeq=v.seq,await V(M)});return}}const k={...p};switch(T){case"Remplacement+":k.maxSubs=(k.maxSubs||3)+1,k.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const L=o+"Score";k[L]>0&&(k[L]--,k.log.push({text:"🚫 But annulé",type:"info"}));break}}k["usedGc_"+a]=_,k._lastGC=v,k._gcAnimSeq=v.seq,await V(k)})}function O(w,T){const g="usedCardIds_"+w,y=new Set(p[g]||[]);T.forEach(x=>y.add(x)),p[g]=[...y]}function D(){for(const w of["p1","p2"]){const T=new Set(p["usedCardIds_"+w]||[]),g=p[w+"Team"];if(g)for(const y of["GK","DEF","MIL","ATT"])(g[y]||[]).forEach(x=>{x.used=T.has(x.cardId)})}}function ne(){var Ke,Ze,it,ut,ft,gt;if(p.phase==="reveal")return ue();if(p.phase==="midfield")return me();if(p.phase==="finished")return je();const w=p[a+"Team"],T=p[o+"Team"];D();const g=p[a+"Score"],y=p[o+"Score"],x=p[a+"Name"],_=p[o+"Name"],v=p.phase===a+"-attack",k=p.phase===a+"-defense",L=Array.isArray(p["selected_"+a])?p["selected_"+a]:[],E=L.map(G=>G.cardId),M=window.innerWidth>=700,S=p[a+"Subs"]||[],I=p["usedSubIds_"+a]||[],N=S.filter(G=>!I.includes(G.cardId)),Q=p["gcCardsFull_"+a]||[],R=p["usedGc_"+a]||[],K=Q.filter(G=>!R.includes(G.id)),H=p.boostOwner===a&&!p.boostUsed,X=!["GK","DEF","MIL","ATT"].some(G=>(T[G]||[]).some(le=>!le.used)),ae=[...w.MIL||[],...w.ATT||[]].filter(G=>!G.used),pe=v&&X&&ae.length===0?[...w.GK||[],...w.DEF||[]].filter(G=>!G.used).map(G=>G.cardId):[];function ce(G,le,J){var lt,wt;const ie=G._gcDef,Ae={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ie==null?void 0:ie.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",_e={purple:"#b06ce0",light_blue:"#00d4ef"}[ie==null?void 0:ie.color]||"#b06ce0",Xe=(ie==null?void 0:ie.name)||G.gc_type,Qe=(ie==null?void 0:ie.effect)||((lt=Re[G.gc_type])==null?void 0:lt.desc)||"",ot=ie!=null&&ie.image_url?`/icons/${ie.image_url}`:null,dt=((wt=Re[G.gc_type])==null?void 0:wt.icon)||"⚡",mt=Math.round(J*.22),vt=Math.round(J*.22),rt=J-mt-vt,Tt=Xe.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${G.id}" data-gc-type="${G.gc_type}"
        style="box-sizing:border-box;width:${le}px;height:${J}px;border-radius:10px;border:2px solid ${_e};background:${Ae};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
        <div style="height:${mt}px;background:rgba(255,255,255,0.14);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 3px">
          <span style="font-size:${Tt}px;font-weight:900;color:#fff;text-align:center;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${le-6}px">${Xe}</span>
          <span style="font-size:6px;color:rgba(255,255,255,0.45)">⚡ GC</span>
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${pt(w,p[a+"Formation"],Me,E,300,300,pe)}
          ...
          ${G.isGoal?`<div style="text-align:center;font-size:11px;color:#FFD700;font-weight:900;margin-top:3px">${G.homeScored,"⚽ BUT !"}</div>`:""}
        </div>`}return`<div style="font-size:11px;color:${G.type==="goal"?"#FFD700":"rgba(255,255,255,0.65)"};font-weight:${G.type==="goal"?700:400};padding:3px 2px">${G.text||""}</div>`}const Je=(()=>{var le,J;if(k&&((le=p.pendingAttack)!=null&&le.players)){const ie=p.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
          <div style="font-size:8px;color:#ff6b6b;letter-spacing:2px;margin-bottom:4px;text-transform:uppercase">⚔️ ${_} ATTAQUE — Défendez !</div>
          ${tt((ie.players||[]).map(Ae=>({...Ae,used:!1})),"#ff6b6b",ie.total)}
        </div>`}if(v&&((J=p.pendingAttack)!=null&&J.players)){const ie=p.pendingAttack;return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #00ff88">
          <div style="font-size:8px;color:#00ff88;letter-spacing:2px;margin-bottom:4px;text-transform:uppercase">⚔️ VOUS ATTAQUEZ</div>
          ${tt((ie.players||[]).map(Ae=>({...Ae,used:!1})),"#00ff88",ie.total)}
      ...
      </div>`;function Be(){const G=e.querySelector(".match-screen");if(!G)return;const le=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);G.style.height=le+"px",G.style.minHeight=le+"px",G.style.maxHeight=le+"px",G.style.overflow="hidden";const J=e.querySelector("#mobile-action-bar"),ie=e.querySelector("#mobile-play-area");J&&ie&&(ie.style.paddingBottom=J.offsetHeight+"px")}if(Be(),setTimeout(Be,120),setTimeout(Be,400),F||(F=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",Be),window.visualViewport.addEventListener("scroll",Be)),window.addEventListener("resize",Be)),function(){const le=e.querySelector(".terrain-wrapper svg");le&&(le.removeAttribute("width"),le.removeAttribute("height"),le.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",le.setAttribute("viewBox","-26 -26 352 352"),le.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),e.querySelectorAll(".match-slot-hit").forEach(G=>{G.addEventListener("click",()=>{if(!v&&!k)return;const le=G.dataset.cardId,J=G.dataset.role,ie=(w[J]||[]).find(Qe=>Qe.cardId===le);if(!ie||ie.used)return;const Ae=pe.includes(le);if(v&&!["MIL","ATT"].includes(J)&&!Ae)return;Array.isArray(p["selected_"+a])||(p["selected_"+a]=[]);const _e=p["selected_"+a],Xe=_e.findIndex(Qe=>Qe.cardId===le);Xe>-1?_e.splice(Xe,1):_e.length<3&&_e.push({...ie,_role:J}),ne()})}),e.querySelectorAll(".match-used-hit").forEach(G=>{G.addEventListener("click",()=>we(G.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(G=>{G.addEventListener("click",()=>we())}),(Ke=e.querySelector("#pvp-sub-open"))==null||Ke.addEventListener("click",()=>we()),e.querySelectorAll(".pvp-gc-mini").forEach(G=>{G.addEventListener("click",()=>Fe(G.dataset.gcId,G.dataset.gcType))}),(Ze=e.querySelector("#pvp-boost-card"))==null||Ze.addEventListener("click",()=>Ce()),(it=e.querySelector("#pvp-action"))==null||it.addEventListener("click",G=>{v?G.currentTarget.dataset.pass==="1"||!$e(a)?U():te():k&&W()}),(ut=e.querySelector("#pvp-quit"))==null||ut.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&ee()}),(ft=e.querySelector("#pvp-view-opp"))==null||ft.addEventListener("click",()=>oe()),(gt=e.querySelector("#pvp-toggle-history"))==null||gt.addEventListener("click",()=>fe()),j&&(clearInterval(j),j=null),(v||k)&&!u){let G=30,le=!1;const J=()=>document.getElementById("pvp-timer"),ie=()=>{J()&&(J().textContent=G+"s",J().style.color=le?"#ff4444":"#fff")};ie(),j=setInterval(()=>{G--,G<0?le?(clearInterval(j),j=null,v&&!$e(a)?U():ee()):(le=!0,G=15,ie()):ie()},1e3)}}function ue(){et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
    ...
    </div>`,a==="p1"&&setTimeout(async()=>{await V({phase:"midfield"})},5e3)}let ye=!1;function me(){if(ye)return;const w=p[a+"Team"].MIL||[],T=p[o+"Team"].MIL||[];function g(K){return K.reduce((H,X)=>H+ze(X,"MIL"),0)}function y(K){let H=0;for(let X=0;X<K.length-1;X++){const ae=nt(K[X],K[X+1]);ae==="#00ff88"?H+=2:ae==="#FFD700"&&(H+=1)}return H}const x=g(w)+y(w),_=g(T)+y(T),v=x>=_;function k(K,H,X){return`<div id="duel-row-${X}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
        <div style="font-size:11px;color:rgba(255,255,255,0.55);letter-spacing:2px;margin-bottom:10px;text-transform:uppercase;font-weight:700">${H}</div>
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${K.map((ae,pe)=>{const ce=pe<K.length-1?nt(ae,K[pe+1]):null,ge=!ce||ce==="#ff3333"||ce==="#cc2222",he=ce==="#00ff88"?"+2":ce==="#FFD700"?"+1":"";return`<div class="duel-card duel-card-${X}" data-idx="${pe}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease,transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
              ${We({...ae,note:ze(ae,"MIL"),_line:"MIL"},58,78)}
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div id="duel-shock" style="position:absolute;left:50%;top:50%;width:120px;height:120px;border-radius:50%;border:6px solid #FFD700;opacity:0;pointer-events:none"></div>
      <div id="pvp-duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center,rgba(10,61,30,.4),rgba(10,61,30,.92))"></div>
    </div>`;const L=(K,H)=>e.querySelectorAll(K).forEach((X,ae)=>{setTimeout(()=>{X.style.opacity="1",X.style.transform="translateY(0) scale(1)"},H+ae*90)});L(".duel-card-me",150),L(".duel-card-opp",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((K,H)=>setTimeout(()=>{K.style.opacity="1"},H*70)),900),setTimeout(()=>{const K=e.querySelector("#pvp-vs");K&&(K.style.opacity="1",K.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(H=>H.style.opacity="1")},1250);function E(K,H,X){const ae=document.getElementById(K);if(!ae)return;const pe=performance.now(),ce=ge=>{const he=Math.min(1,(ge-pe)/X);ae.textContent=Math.round(H*(1-Math.pow(1-he,3))),he<1?requestAnimationFrame(ce):ae.textContent=H};requestAnimationFrame(ce)}setTimeout(()=>{E("pvp-score-me",x,800),E("pvp-score-opp",_,800)},1500);const M=p.p1Team.MIL||[],S=p.p2Team.MIL||[],I=g(M)+y(M),N=g(S)+y(S),Q=I>=N?"p1":"p2";let R=p.boostValue;R==null&&(R=ai(),p.boostValue=R,p.boostOwner=Q,p.boostUsed=!1),ye=!0,setTimeout(()=>{const K=e.querySelector("#duel-row-"+(v?"me":"opp")),H=e.querySelector("#duel-row-"+(v?"opp":"me")),X=document.getElementById("pvp-score-me"),ae=document.getElementById("pvp-score-opp"),pe=v?X:ae,ce=v?ae:X;pe&&(pe.style.fontSize="80px",pe.style.color=v?"#FFD700":"#ff6b6b",pe.style.animation="duelPulse .5s ease"+(v?",duelGlow 1.5s ease infinite .5s":"")),ce&&(ce.style.opacity="0.25"),setTimeout(()=>{K&&(K.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",K.style.zIndex="5"),setTimeout(()=>{const ge=document.getElementById("duel-shock");ge&&(ge.style.animation="shockwave .5s ease-out forwards"),H&&(H.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var Se;const ge=document.getElementById("pvp-duel-finale");if(!ge)return;const he=p.boostOwner===a?'<div style="background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:3px solid #cdeffd;border-radius:18px;padding:20px 34px;text-align:center;animation:boostFlipIn .7s cubic-bezier(.34,1.56,.64,1) both;box-shadow:0 10px 36px rgba(135,206,235,.5)"><div style="font-size:10px;color:rgba(0,0,0,.6);letter-spacing:2px;text-transform:uppercase;margin-bottom:6px;font-weight:700">Carte Boost obtenue</div><div style="font-size:46px;line-height:1">⚡</div><div style="font-size:50px;font-weight:900;color:#063;line-height:1.1">+'+R+`</div><div style="font-size:10px;color:rgba(0,0,0,.55);margin-top:4px">Applicable sur n'importe quel joueur</div></div>`:"",Ie=a==="p1"?'<button id="pvp-start-match" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">▶ Commencer le match</button>':`<div style="font-size:14px;color:rgba(255,255,255,0.5);text-align:center;margin-top:8px;animation:fadeUp .4s ease both">⏳ En attente de l'adversaire...</div>`;ge.innerHTML='<div style="font-size:22px;font-weight:900;color:#fff;text-align:center;animation:fadeUp .4s ease both;text-shadow:0 2px 12px rgba(0,0,0,.5)">'+(v?"⚽ "+p[a+"Name"]+"<br>gagne le milieu et attaque !":"😔 "+p[o+"Name"]+"<br>gagne l'engagement et attaque !")+"</div>"+he+Ie,ge.style.transition="opacity .45s ease",ge.style.opacity="1",ge.style.pointerEvents="auto",(Se=document.getElementById("pvp-start-match"))==null||Se.addEventListener("click",async()=>{const Oe=Q;await V({phase:Oe+"-attack",attacker:Oe,round:1,boostValue:R,boostUsed:!1,boostOwner:Oe})})},600)},700)},2800)}function xe(w,T,g,y,x){const _=document.createElement("div");_.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:900;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;overflow:hidden;cursor:pointer";const v=Array.from({length:10},(M,S)=>`<div style="position:absolute;font-size:${16+Math.floor(Math.random()*24)}px;top:${5+Math.floor(Math.random()*65)}%;left:${3+Math.floor(Math.random()*94)}%;animation:fw${S%2===0?"A":"B"} ${.7+Math.random()*.7}s ease ${Math.random()*.9}s both;opacity:0">${["✨","🌟","⭐","💥","🎇","🎆","🔥","🌈"][S%8]}</div>`).join(""),k={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};_.innerHTML=`
      <style>
        @keyframes butPop{0%{transform:scale(0) rotate(-8deg);opacity:0}55%{transform:scale(1.25) rotate(2deg)}85%{transform:scale(0.92) rotate(-1deg)}100%{transform:scale(1);opacity:1}}
        @keyframes ballIn{0%{transform:translate(-70px,18px);opacity:0}65%{opacity:1}100%{transform:translate(26px,-8px);opacity:1}}
        @keyframes scoreIn{from{opacity:0;transform:translateY(-12px)}to{opacity:1;transform:translateY(0)}}
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(I);let N=!1;const Q=()=>{N||(N=!0,I.remove(),setTimeout(()=>g&&g(),50))};I.addEventListener("click",Q),setTimeout(Q,3e3)}function Fe(w,T){var I,N,Q,R;const y=(p["gcCardsFull_"+a]||[]).find(K=>K.id===w),x=y==null?void 0:y._gcDef,_={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[x==null?void 0:x.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",v={purple:"#b06ce0",light_blue:"#00d4ef"}[x==null?void 0:x.color]||"#b06ce0",k=(x==null?void 0:x.name)||T,L=(x==null?void 0:x.effect)||((I=Re[T])==null?void 0:I.desc)||"Carte spéciale.",E=x!=null&&x.image_url?`/icons/${x.image_url}`:null,M=((N=Re[T])==null?void 0:N.icon)||"⚡",S=document.createElement("div");S.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",S.innerHTML=`
      <div style="width:190px;border-radius:16px;border:3px solid ${v};background:${_};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 40px ${v}66">
        <div style="padding:10px;background:rgba(255,255,255,0.14);text-align:center">
          <div style="font-size:${k.length>14?11:14}px;font-weight:900;color:#fff;letter-spacing:.5px;text-transform:uppercase">${k}</div>
          <div style="font-size:8px;color:rgba(255,255,255,0.5);margin-top:2px">⚡ GAME CHANGER</div>
      ...
      <div style="display:flex;gap:12px;width:190px">
        <button id="pvp-gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
        <button id="pvp-gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
      </div>`,document.body.appendChild(S),(Q=S.querySelector("#pvp-gc-back"))==null||Q.addEventListener("click",()=>S.remove()),(R=S.querySelector("#pvp-gc-use"))==null||R.addEventListener("click",()=>{S.remove(),Z(w,T)})}function Ce(){var y;const w=p[a+"Team"],T=Object.entries(w).flatMap(([x,_])=>(_||[]).filter(v=>!v.used).map(v=>({...v,_line:x})));if(!T.length)return;const g=document.createElement("div");g.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",g.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
      ...
      </div>`,document.body.appendChild(g),(y=g.querySelector("#bp-close"))==null||y.addEventListener("click",()=>g.remove()),g.querySelectorAll(".bp-item").forEach(x=>{x.addEventListener("click",async()=>{const _=x.dataset.cid,v=T.find(L=>L.cardId===_);if(!v)return;const k=(w[v._line]||[]).find(L=>L.cardId===_);k&&(k.boost=(k.boost||0)+p.boostValue),g.remove(),await V({[a+"Team"]:w,boostUsed:!0})})})}function we(w=null){var N,Q;if(!(p.phase===a+"-attack")){f("Remplacement uniquement avant votre attaque","warning");return}const g=p[a+"Team"],y=p["usedSubIds_"+a]||[],x=p.maxSubs||3;if(y.length>=x){f(`Maximum ${x} remplacements atteint`,"warning");return}const _=Object.entries(g).flatMap(([R,K])=>(K||[]).filter(H=>H.used).map(H=>({...H,_line:R}))),v=(p[a+"Subs"]||[]).filter(R=>!y.includes(R.cardId));if(!_.length){f("Aucun joueur utilisé à remplacer","warning");return}if(!v.length){f("Aucun remplaçant disponible","warning");return}let k=Math.max(0,_.findIndex(R=>R.cardId===w));const L=((N=_[k])==null?void 0:N._line)||((Q=_[k])==null?void 0:Q.job);let E=Math.max(0,v.findIndex(R=>R.job===L)),M=!1;const S=document.createElement("div");S.id="pvp-sub-overlay",S.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function I(){var ce,ge,he,Ie,Se,Oe;const R=_[k],K=v[E],H=Math.min(130,Math.round((window.innerWidth-90)/2)),X=Math.round(H*1.35),ae=Ee=>`background:rgba(255,255,255,0.12);border:none;color:${Ee?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Ee?"default":"pointer"};flex-shrink:0`;S.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
        <div style="flex:1;font-size:15px;font-weight:900;color:#fff">🔄 Remplacement (${y.length}/${x})</div>
        <button id="psub-close" style="background:none;border:none;color:rgba(255,255,255,0.5);font-size:24px;cursor:pointer;padding:0">✕</button>
      </div>
      ...
      <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
        <button id="psub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
      </div>`,(ce=S.querySelector("#psub-close"))==null||ce.addEventListener("click",()=>S.remove()),(ge=S.querySelector("#pout-up"))==null||ge.addEventListener("click",()=>{k>0&&(k--,I())}),(he=S.querySelector("#pout-down"))==null||he.addEventListener("click",()=>{k<_.length-1&&(k++,I())}),(Ie=S.querySelector("#pin-up"))==null||Ie.addEventListener("click",()=>{E>0&&(E--,I())}),(Se=S.querySelector("#pin-down"))==null||Se.addEventListener("click",()=>{E<v.length-1&&(E++,I())});const pe=(Ee,Ge,Me,Ue)=>{const Ne=S.querySelector("#"+Ee);if(!Ne)return;let Je=0;Ne.addEventListener("touchstart",He=>{Je=He.touches[0].clientY},{passive:!0}),Ne.addEventListener("touchend",He=>{const Be=He.changedTouches[0].clientY-Je;if(Math.abs(Be)<30)return;const Ke=Ge();Be<0&&Ke<Ue-1?(Me(Ke+1),I()):Be>0&&Ke>0&&(Me(Ke-1),I())},{passive:!0})};pe("pin-panel",()=>E,Ee=>E=Ee,v.length),pe("pout-panel",()=>k,Ee=>k=Ee,_.length),(Oe=S.querySelector("#psub-confirm"))==null||Oe.addEventListener("click",async Ee=>{if(Ee.preventDefault(),Ee.stopPropagation(),M)return;M=!0;const Ge=_[k],Me=v[E];if(!Ge||!Me)return;const Ue=Ge._line,Ne=(g[Ue]||[]).findIndex(Be=>Be.cardId===Ge.cardId);if(Ne===-1){f("Erreur : joueur introuvable","error"),S.remove();return}const Je={...Me,_line:Ue,position:Ge.position,used:!1,boost:0};g[Ue].splice(Ne,1,Je);const He=[...y,Me.cardId];S.remove(),Y(Ge,Me,async()=>{await V({[a+"Team"]:g,[o+"Team"]:p[o+"Team"],["usedSubIds_"+a]:He})})})}document.body.appendChild(S),I()}function Y(w,T,g){const y={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const _=(L,E,M)=>`<div style="text-align:center">
      <div style="font-size:9px;color:${E};letter-spacing:2px;text-transform:uppercase;font-weight:700;margin-bottom:6px">${M}</div>
      <div style="width:70px;height:70px;border-radius:50%;background:${y[L.job]||"#555"};border:3px solid ${E};position:relative;overflow:hidden;margin:0 auto">
        ${Pe(L)?`<img src="${Pe(L)}" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover">`:""}
      </div>
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>${b.map((Z,O)=>{const D=$>0&&r.status==="finished"?O===0?Math.floor($*.7):O===1?Math.floor($*.2):O===2?Math.floor($*.1):0:0;return`<tr style="border-bottom:1px solid #f5f5f5;${Z.userId===s?"background:#f0fdf4;":""}">
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        ${Array.from({length:Math.max(0,r.status==="active"?r.current_day-1:r.current_day)},(Z,O)=>O+1).reverse().map(Z=>{const O=(f||[]).filter(D=>D.matchday===Z);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:#888;margin-bottom:6px">Journée ${Z}</div>${O.map(D=>Ki(D,l,s,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
    ...
    <div style="background:linear-gradient(160deg,#1a0a2e,#3b1a6e);border-radius:20px;padding:32px 24px;max-width:320px;width:100%;text-align:center;color:#fff;box-shadow:0 8px 40px rgba(0,0,0,0.6)">
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">🏆 Retour à la Mini League</button>`,document.body.appendChild(k),(I=k.querySelector("#pvp-end-home"))==null||I.addEventListener("click",()=>{k.remove(),Ve(e),a?c("mini-league",{openLeagueId:a}):c("mini-league")})}const V=z.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},g=>{const y=g.new;try{if(y.status==="finished"||y.forfeit){if(m||(m=!0,F&&(clearInterval(F),F=null),y.game_state&&(u=y.game_state),u.phase==="finished"&&!y.forfeit&&document.getElementById("pvp-home")))return;P(y);return}if(y.game_state){const x=u;u=y.game_state;const _=u._lastGC;if(_&&_.seq&&!C.has(_.seq)&&(C.add(_.seq),_.by!==o)){Fe(_.type,_.by,()=>ue());return}const v=x[o+"Score"]||0,k=x[l+"Score"]||0,L=u[o+"Score"]||0,E=u[l+"Score"]||0,M=L>v,S=E>k;if((M||S)&&!B.has(u.round)){B.add(u.round);const I=[...u.log||[]].reverse().find(Q=>Q.isGoal),N=((I==null?void 0:I.homePlayers)||[]).map(Q=>({name:Q.name,note:Q.note,portrait:Q.portrait,job:Q.job}));Le(N,L,E,M,()=>ue());return}ue()}}catch(x){console.error("[PvP] crash:",x)}}).subscribe();async function ee(g){Object.assign(u,g),u.lastActionAt=new Date().toISOString();const{error:y}=await z.from("matches").update({game_state:u}).eq("id",i);y&&f("Erreur de synchronisation","error");try{ue()}catch(x){console.error("[PvP] renderPvpScreen crash:",x)}}async function se(){if(m)return;m=!0,F&&(clearInterval(F),F=null);const g=n?$.away_id:$.home_id,y=n?"p2":"p1",x=n?"p1":"p2",_={...u,[y+"Score"]:3,[x+"Score"]:0,phase:"finished"};try{await z.from("matches").update({status:"finished",forfeit:!0,winner_id:g,home_score:_.p1Score||0,away_score:_.p2Score||0,game_state:_}).eq("id",i)}catch{}try{z.removeChannel(V)}catch{}setTimeout(()=>{Ve(e),c("mini-league")},800)}const re={BOOST_STAT:({value:g=1,count:y=1,roles:x=[]},_,v)=>{const k=_[o+"Team"],L=Object.entries(k).filter(([E])=>!x.length||x.includes(E)).flatMap(([E,M])=>M.filter(S=>!S.used).map(S=>({...S,_line:E})));if(!L.length){_.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),v(_);return}Z(L,y,`Choisir ${y} joueur(s) à booster (+${g})`,E=>{E.forEach(M=>{const S=(k[M._line]||[]).find(I=>I.cardId===M.cardId);S&&(S.boost=(S.boost||0)+g,_.log.push({text:`⚡ +${g} sur ${S.name}`,type:"info"}))}),_[o+"Team"]=k,v(_)})},DEBUFF_STAT:({value:g=1,count:y=1,roles:x=[],target:_="ai"},v,k)=>{const L=_==="home"?o:l,E=v[L+"Team"],M=_==="home"?"allié":"adverse",S=Object.entries(E).filter(([I])=>!x.length||x.includes(I)).flatMap(([I,N])=>N.map(Q=>({...Q,_line:I})));if(!S.length){v.log.push({text:`🎯 Aucun joueur ${M}`,type:"info"}),k(v);return}Z(S,y,`Choisir ${y} joueur(s) ${M}(s) (-${g})`,I=>{I.forEach(N=>{const Q=(E[N._line]||[]).find(R=>R.cardId===N.cardId);Q&&(Q.boost=(Q.boost||0)-g,v.log.push({text:`🎯 -${g} sur ${Q.name}`,type:"info"}))}),v[L+"Team"]=E,k(v)})},GRAY_PLAYER:({count:g=1,roles:y=[],target:x="ai"},_,v)=>{const k=x==="home"?o:l,L=_[k+"Team"],E=x==="home"?"allié":"adverse",M=Object.entries(L).filter(([S])=>!y.length||y.includes(S)).flatMap(([S,I])=>I.filter(N=>!N.used).map(N=>({...N,_line:S})));if(!M.length){_.log.push({text:`❌ Aucun joueur ${E}`,type:"info"}),v(_);return}Z(M,g,`Choisir ${g} joueur(s) ${E}(s) à exclure`,S=>{const I="usedCardIds_"+k,N=new Set(_[I]||[]);S.forEach(Q=>{const R=(L[Q._line]||[]).find(K=>K.cardId===Q.cardId);R&&(R.used=!0,N.add(Q.cardId),_.log.push({text:`❌ ${R.name} exclu !`,type:"info"}))}),_[I]=[...N],_[k+"Team"]=L,v(_)})},REVIVE_PLAYER:({count:g=1,roles:y=[]},x,_)=>{const v=x[o+"Team"],k=Object.entries(v).filter(([L])=>!y.length||y.includes(L)).flatMap(([L,E])=>E.filter(M=>M.used).map(M=>({...M,_line:L})));if(!k.length){x.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),_(x);return}Z(k,g,`Choisir ${g} joueur(s) à ressusciter`,L=>{L.forEach(E=>{const M=(v[E._line]||[]).find(S=>S.cardId===E.cardId);M&&(M.used=!1,x.log.push({text:`💫 ${M.name} ressuscité !`,type:"info"}))}),x[o+"Team"]=v,_(x)})},REMOVE_GOAL:({},g,y)=>{const x=l+"Score";g[x]>0?(g[x]--,g.log.push({text:"🚫 Dernier but annulé !",type:"info"})):g.log.push({text:"🚫 Aucun but à annuler",type:"info"}),y(g)},ADD_GOAL_DRAW:({},g,y)=>{g[o+"Score"]===g[l+"Score"]?(g[o+"Score"]++,g.log.push({text:"🎯 But bonus !",type:"info"})):g.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),y(g)},ADD_SUB:({value:g=1},y,x)=>{y.maxSubs=(y.maxSubs||3)+g,y.log.push({text:`🔄 +${g} remplacement(s)`,type:"info"}),x(y)},CUSTOM:({},g,y)=>y(g)};function Z(g,y,x,_){const v=document.createElement("div");v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let k=[];function L(){var M,S;const E=g.map(I=>{const N={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[I._line]||"#555",Q=ze(I,I._line)+(I.boost||0),K=k.find(X=>X.cardId===I.cardId)?"#FFD700":"rgba(255,255,255,0.25)",H=I.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${I.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${K};background:${N};overflow:hidden;cursor:pointer;${H}">
        ...
        </div>`,(M=v.querySelector("#pp-close"))==null||M.addEventListener("click",()=>v.remove()),v.querySelectorAll(".pp-item").forEach(I=>{I.addEventListener("click",()=>{const N=I.dataset.cid,Q=g.find(K=>K.cardId===N),R=k.findIndex(K=>K.cardId===N);Q&&(R>-1?k.splice(R,1):k.length<y&&k.push(Q),L())})}),(S=v.querySelector("#pp-confirm"))==null||S.addEventListener("click",()=>{v.remove(),_(k)})}L(),document.body.appendChild(v)}async function O(g,y){const _=(u["gcCardsFull_"+o]||[]).find(E=>E.id===g),v=(_==null?void 0:_._gcDef)||(u.gcDefs||[]).find(E=>{var M;return E.name===y||((M=E.name)==null?void 0:M.toLowerCase().trim())===(y==null?void 0:y.toLowerCase().trim())}),k=[...u["usedGc_"+o]||[],g],L={type:y,by:o,seq:(u._gcAnimSeq||0)+1};C.add(L.seq),Fe(y,o,async()=>{if(v!=null&&v.effect_type&&v.effect_type!=="CUSTOM"){const M=re[v.effect_type];if(M){const S={...u};M(v.effect_params||{},S,async I=>{I["usedGc_"+o]=k,I._lastGC=L,I._gcAnimSeq=L.seq,await ee(I)});return}}const E={...u};switch(y){case"Remplacement+":E.maxSubs=(E.maxSubs||3)+1,E.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const M=l+"Score";E[M]>0&&(E[M]--,E.log.push({text:"🚫 But annulé",type:"info"}));break}}E["usedGc_"+o]=k,E._lastGC=L,E._gcAnimSeq=L.seq,await ee(E)})}function D(g,y){const x="usedCardIds_"+g,_=new Set(u[x]||[]);y.forEach(v=>_.add(v)),u[x]=[..._]}function ne(){for(const g of["p1","p2"]){const y=new Set(u["usedCardIds_"+g]||[]),x=u[g+"Team"];if(x)for(const _ of["GK","DEF","MIL","ATT"])(x[_]||[]).forEach(v=>{v.used=y.has(v.cardId)})}}function ue(){var it,ut,ft,gt,G,le;if(u.phase==="reveal")return ye();if(u.phase==="midfield")return xe();if(u.phase==="finished")return w();const g=u[o+"Team"],y=u[l+"Team"];ne();const x=u[o+"Score"],_=u[l+"Score"],v=u[o+"Name"],k=u[l+"Name"],L=u.phase===o+"-attack",E=u.phase===o+"-defense",M=Array.isArray(u["selected_"+o])?u["selected_"+o]:[],S=M.map(J=>J.cardId),I=window.innerWidth>=700,N=u[o+"Subs"]||[],Q=u["usedSubIds_"+o]||[],R=N.filter(J=>!Q.includes(J.cardId)),K=u["gcCardsFull_"+o]||[],H=u["usedGc_"+o]||[],X=K.filter(J=>!H.includes(J.id)),ae=u.boostOwner===o&&!u.boostUsed,pe=!["GK","DEF","MIL","ATT"].some(J=>(y[J]||[]).some(ie=>!ie.used)),ce=[...g.MIL||[],...g.ATT||[]].filter(J=>!J.used),ge=L&&pe&&ce.length===0?[...g.GK||[],...g.DEF||[]].filter(J=>!J.used).map(J=>J.cardId):[];function he(J,ie,Ae){var Ii,Mi;const _e=J._gcDef,Xe={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[_e==null?void 0:_e.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",Qe={purple:"#b06ce0",light_blue:"#00d4ef"}[_e==null?void 0:_e.color]||"#b06ce0",ot=(_e==null?void 0:_e.name)||J.gc_type,dt=(_e==null?void 0:_e.effect)||((Ii=Re[J.gc_type])==null?void 0:Ii.desc)||"",mt=_e!=null&&_e.image_url?`/icons/${_e.image_url}`:null,vt=((Mi=Re[J.gc_type])==null?void 0:Mi.icon)||"⚡",rt=Math.round(Ae*.22),Tt=Math.round(Ae*.22),lt=Ae-rt-Tt,wt=ot.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${J.id}" data-gc-type="${J.gc_type}"
      ...
      </div>`}function Ie(J,ie){return`<div id="pvp-boost-card"
        style="box-sizing:border-box;width:${J}px;height:${ie}px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:2px solid #87CEEB;border-radius:10px;cursor:pointer;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:${Math.round(ie*.04)}px;text-align:center;flex-shrink:0">
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${pt(g,u[o+"Formation"],Ne,S,300,300,ge)}
    ...
    </div>`;function He(J){if(J.type==="duel"&&(J.homeTotal!=null||J.aiTotal!=null)){const ie=(J.homeTotal||0)>=(J.aiTotal||0);return`<div style="background:rgba(255,255,255,0.05);border-radius:8px;padding:5px 6px;border:1px solid rgba(255,255,255,0.08)">
          ...
          <div style="display:flex;align-items:center;gap:3px;max-height:46px;overflow:hidden">
            ...
            <div style="text-align:center;padding:0 6px;flex-shrink:0">
              <div style="font-size:${ie?20:14}px;font-weight:900;color:${ie?"#FFD700":"rgba(255,255,255,0.4)"};line-height:1">${J.homeTotal}</div>
              <div style="font-size:8px;color:rgba(255,255,255,0.3);margin:1px 0">VS</div>
              <div style="font-size:${ie?14:20}px;font-weight:900;color:${ie?"rgba(255,255,255,0.4)":"#ff6b6b"};line-height:1">${J.aiTotal}</div>
            </div>
            <div style="flex:1;display:flex;gap:2px;justify-content:flex-start;flex-wrap:nowrap;overflow-x:auto;overflow-y:hidden">
        ...
        </div>`}const J=(u.log||[]).slice(-1)[0];return J?'<div style="padding:2px 4px">'+He(J)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})(),Ke=`
      <div style="display:flex;align-items:center;padding:8px 10px;background:rgba(0,0,0,0.5);gap:6px;flex-shrink:0">
        <button id="pvp-quit" style="width:34px;height:34px;border-radius:50%;background:rgba(220,50,50,0.7);border:none;color:#fff;font-size:16px;cursor:pointer;flex-shrink:0">✕</button>
        <div style="flex:1;display:flex;align-items:center;justify-content:center;gap:8px">
      ...
      </div>`;function Ze(){const J=e.querySelector(".match-screen");if(!J)return;const ie=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);J.style.height=ie+"px",J.style.minHeight=ie+"px",J.style.maxHeight=ie+"px",J.style.overflow="hidden";const Ae=e.querySelector("#mobile-action-bar"),_e=e.querySelector("#mobile-play-area");Ae&&_e&&(_e.style.paddingBottom=Ae.offsetHeight+"px")}if(Ze(),setTimeout(Ze,120),setTimeout(Ze,400),q||(q=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",Ze),window.visualViewport.addEventListener("scroll",Ze)),window.addEventListener("resize",Ze)),function(){const ie=e.querySelector(".terrain-wrapper svg");ie&&(ie.removeAttribute("width"),ie.removeAttribute("height"),ie.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",ie.setAttribute("viewBox","-26 -26 352 352"),ie.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),e.querySelectorAll(".match-slot-hit").forEach(J=>{J.addEventListener("click",()=>{if(!L&&!E)return;const ie=J.dataset.cardId,Ae=J.dataset.role,_e=(g[Ae]||[]).find(dt=>dt.cardId===ie);if(!_e||_e.used)return;const Xe=ge.includes(ie);if(L&&!["MIL","ATT"].includes(Ae)&&!Xe)return;Array.isArray(u["selected_"+o])||(u["selected_"+o]=[]);const Qe=u["selected_"+o],ot=Qe.findIndex(dt=>dt.cardId===ie);ot>-1?Qe.splice(ot,1):Qe.length<3&&Qe.push({..._e,_role:Ae}),ue()})}),e.querySelectorAll(".match-used-hit").forEach(J=>{J.addEventListener("click",()=>Y(J.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(J=>{J.addEventListener("click",()=>Y())}),(it=e.querySelector("#pvp-sub-open"))==null||it.addEventListener("click",()=>Y()),e.querySelectorAll(".pvp-gc-mini").forEach(J=>{J.addEventListener("click",()=>Ce(J.dataset.gcId,J.dataset.gcType))}),(ut=e.querySelector("#pvp-boost-card"))==null||ut.addEventListener("click",()=>we()),(ft=e.querySelector("#pvp-action"))==null||ft.addEventListener("click",J=>{L?J.currentTarget.dataset.pass==="1"||!be(o)?te():W():E&&de()}),(gt=e.querySelector("#pvp-quit"))==null||gt.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&se()}),(G=e.querySelector("#pvp-view-opp"))==null||G.addEventListener("click",()=>fe()),(le=e.querySelector("#pvp-toggle-history"))==null||le.addEventListener("click",()=>U()),F&&(clearInterval(F),F=null),(L||E)&&!m){let J=30,ie=!1;const Ae=()=>document.getElementById("pvp-timer"),_e=()=>{Ae()&&(Ae().textContent=J+"s",Ae().style.color=ie?"#ff4444":"#fff")};_e(),F=setInterval(()=>{J--,J<0?ie?(clearInterval(F),F=null,L&&!be(o)?te():se()):(ie=!0,J=15,_e()):_e()},1e3)}}function ye(){et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
    ...
    </div>`,o==="p1"&&setTimeout(async()=>{await ee({phase:"midfield"})},5e3)}let me=!1;function xe(){if(me)return;const g=u[o+"Team"].MIL||[],y=u[l+"Team"].MIL||[];function x(X){return X.reduce((ae,pe)=>ae+ze(pe,"MIL"),0)}function _(X){let ae=0;for(let pe=0;pe<X.length-1;pe++){const ce=nt(X[pe],X[pe+1]);ce==="#00ff88"?ae+=2:ce==="#FFD700"&&(ae+=1)}return ae}const v=x(g)+_(g),k=x(y)+_(y),L=v>=k;function E(X,ae,pe){return`<div id="duel-row-${pe}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
        <button id="psub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
      </div>`,(he=N.querySelector("#psub-close"))==null||he.addEventListener("click",()=>N.remove()),(Ie=N.querySelector("#pout-up"))==null||Ie.addEventListener("click",()=>{E>0&&(E--,Q())}),(Se=N.querySelector("#pout-down"))==null||Se.addEventListener("click",()=>{E<k.length-1&&(E++,Q())}),(Oe=N.querySelector("#pin-up"))==null||Oe.addEventListener("click",()=>{S>0&&(S--,Q())}),(Ee=N.querySelector("#pin-down"))==null||Ee.addEventListener("click",()=>{S<L.length-1&&(S++,Q())});const ge=(Me,Ue,Ne,Je)=>{const He=N.querySelector("#"+Me);if(!He)return;let Be=0;He.addEventListener("touchstart",Ke=>{Be=Ke.touches[0].clientY},{passive:!0}),He.addEventListener("touchend",Ke=>{const Ze=Ke.changedTouches[0].clientY-Be;if(Math.abs(Ze)<30)return;const it=Ue();Ze<0&&it<Je-1?(Ne(it+1),Q()):Ze>0&&it>0&&(Ne(it-1),Q())},{passive:!0})};ge("pin-panel",()=>S,Me=>S=Me,L.length),ge("pout-panel",()=>E,Me=>E=Me,k.length),(Ge=N.querySelector("#psub-confirm"))==null||Ge.addEventListener("click",async Me=>{if(Me.preventDefault(),Me.stopPropagation(),I)return;I=!0;const Ue=k[E],Ne=L[S];if(!Ue||!Ne)return;const Je=Ue._line,He=(x[Je]||[]).findIndex(Ze=>Ze.cardId===Ue.cardId);if(He===-1){f("Erreur : joueur introuvable","error"),N.remove();return}const Be={...Ne,_line:Je,position:Ue.position,used:!1,boost:0};x[Je].splice(He,1,Be);const Ke=[..._,Ne.cardId];N.remove(),oe(Ue,Ne,async()=>{await ee({[o+"Team"]:x,[l+"Team"]:u[l+"Team"],["usedSubIds_"+o]:Ke})})})}document.body.appendChild(N),Q()}function oe(g,y,x){const _={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},v=document.createElement("div");v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const k=(M,S,I)=>`<div style="text-align:center">
    ...
    </div>`,(v=document.getElementById("pvp-home"))==null||v.addEventListener("click",()=>{try{z.removeChannel(V)}catch{}Ve(e),c("mini-league",a?{openLeagueId:a}:{})})}ue()}const ur="/",fr=[{emoji:"⚽",title:"Bienvenue dans Manager Wars !",color:"#1A6B3C",content:`<p>Tu es désormais un <strong>manager de football</strong> virtuel.</p>
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div id="mkt-content" style="padding:10px 16px;display:flex;flex-direction:column;gap:8px"></div>
  </div>`;let f="buy";const a=()=>{var p,u,m,A,j,F,q;return{name:(((p=document.getElementById("flt-name"))==null?void 0:p.value)||"").toLowerCase().trim(),club:(((u=document.getElementById("flt-club"))==null?void 0:u.value)||"").toLowerCase().trim(),country:(((m=document.getElementById("flt-country"))==null?void 0:m.value)||"").toLowerCase().trim(),job:((A=document.getElementById("flt-job"))==null?void 0:A.value)||"",rarity:((j=document.getElementById("flt-rarity"))==null?void 0:j.value)||"",note1:parseInt((F=document.getElementById("flt-note1"))==null?void 0:F.value)||0,note2:parseInt((q=document.getElementById("flt-note2"))==null?void 0:q.value)||0}};function o(p){const u=a();return p.filter(m=>{var V,ee,se;const A=(V=m.card)==null?void 0:V.player;if(!A)return!1;const j=`${A.firstname} ${A.surname_encoded}`.toLowerCase(),F=(((ee=A.clubs)==null?void 0:ee.encoded_name)||"").toLowerCase(),q=(A.country_code||"").toLowerCase(),B=((se=m.card)==null?void 0:se.evolution_bonus)||0,C=At(A,A.job,B),P=A.job2?At(A,A.job2,B):0;return!(u.name&&!j.includes(u.name)||u.club&&!F.includes(u.club)||u.country&&!q.includes(u.country)||u.job&&A.job!==u.job||u.rarity&&A.rarity!==u.rarity||u.note1&&C<u.note1||u.note2&&P<u.note2)})}function l(p){var q,B,C,P;const u=(q=p.card)==null?void 0:q.player;if(!u)return"";const m=((B=p.card)==null?void 0:B.evolution_bonus)||0,A=At(u,u.job,m),j=u.job2?At(u,u.job2,m):0,F=(i.profile.credits||0)>=p.price;return`<div class="card-panel" style="display:flex;align-items:center;gap:8px;padding:10px 12px;overflow:hidden;padding-left:6px">
      ${Yi(u.rarity)}
      ${Xt(A,u.job)}
      ${Xt(j,u.job2||null)}
      ${Wi(u.country_code)}
      ...
      <div style="text-align:right;flex-shrink:0">
        ...
        <button class="btn btn-primary btn-sm" data-buy="${p.id}" ${F?"":"disabled"} style="margin-top:4px;font-size:11px;padding:4px 10px">${F?"Acheter":"Trop cher"}</button>
      </div>
    </div>`}function b(p){var q,B,C,P;const u=(q=p.card)==null?void 0:q.player;if(!u)return"";const m=((B=p.card)==null?void 0:B.evolution_bonus)||0,A=At(u,u.job,m),j=u.job2?At(u,u.job2,m):0,F=p.status==="sold";return`<div class="card-panel" style="display:flex;align-items:center;gap:8px;padding:10px 12px;overflow:hidden;padding-left:6px;${F?"opacity:0.7":""}">
      ${Yi(u.rarity)}
      ${Xt(A,u.job)}
      ${Xt(j,u.job2||null)}
      ${Wi(u.country_code)}
    ...
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center">
      ...
      `).join(""):'<div style="text-align:center;color:var(--gray-600);padding:40px">Aucun manager.</div>'}else{const m=r||[];$.innerHTML=m.length>0?m.map((A,j)=>{const F=Pt(A.mmr??1e3),q=(A.ranked_wins||0)+(A.ranked_losses||0)+(A.ranked_draws||0),B=q>0?Math.round((A.ranked_wins||0)/q*100):0,C=A.id===i.profile.id,P=(A.placement_matches||0)<10;return`
          <div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${C?"border:2px solid var(--yellow)":""}">
            ...
            <div style="flex:1;min-width:0">
              ...
              <div style="font-size:11px;color:var(--gray-600)">${A.club_name} · ${F.label}</div>
            </div>
            <div style="text-align:right;flex-shrink:0">
  ...
  </div>`,f(),(b=document.getElementById("tab-global"))==null||b.addEventListener("click",()=>{c="global",f()}),(h=document.getElementById("tab-ranked"))==null||h.addEventListener("click",()=>{c="ranked",f()})}async function _r(e,t){var B,C,P;const{state:i,navigate:n,toast:d}=t,s=i.profile;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const[{data:r},{data:c}]=await Promise.all([z.from("ranked_seasons").select("*").eq("is_active",!0).maybeSingle(),z.from("users").select("id,pseudo,club_name,mmr,mmr_deviation,mmr_volatility,rank_tier,placement_matches,ranked_wins,ranked_losses,ranked_draws").eq("id",s.id).single()]);if(!c){d("Erreur chargement profil","error"),n("home");return}const f=c.mmr??1e3,a=c.mmr_deviation??350,o=c.mmr_volatility??.06,l=c.placement_matches??0,b=l<10,h=Math.max(0,10-l),$=Pt(f),p=Lo(f),u=ci.findIndex(V=>V.id===$.id),m=ci[u+1]||null,A={bronze:"linear-gradient(160deg,#3d1c00,#7a3e00)",silver:"linear-gradient(160deg,#1a1a2e,#3a3a5e)",gold:"linear-gradient(160deg,#1a1200,#4a3500)",platinum:"linear-gradient(160deg,#001a20,#003040)",diamond:"linear-gradient(160deg,#001030,#1a2860)",master:"linear-gradient(160deg,#1a0030,#3d0070)"},j=(c.ranked_wins||0)+(c.ranked_losses||0)+(c.ranked_draws||0),F=j>0?Math.round((c.ranked_wins||0)/j*100):0,q=ci.map(V=>`
    <div style="display:flex;flex-direction:column;align-items:center;gap:2px;opacity:${$.id===V.id?1:.35};
      transform:${$.id===V.id?"scale(1.15)":"scale(1)"};transition:all 0.3s">
      <div style="font-size:${$.id===V.id?"28px":"20px"}">${V.emoji}</div>
      <div style="font-size:9px;color:${V.color};font-weight:${$.id===V.id?"900":"400"};letter-spacing:0.5px">${V.label.toUpperCase()}</div>
    </div>
  `).join("");e.innerHTML=`
  ...
  </div>`,(B=document.getElementById("ranked-back"))==null||B.addEventListener("click",()=>n("home")),(C=document.getElementById("ranked-leaderboard-btn"))==null||C.addEventListener("click",()=>n("rankings",{tab:"ranked"})),(P=document.getElementById("ranked-play-btn"))==null||P.addEventListener("click",()=>{n("match",{matchMode:"ranked",rankedData:{mmr:f,rd:a,sigma:o,isPlacement:b}})})}async function $r(e,{state:t,navigate:i,toast:n}){const d=t.profile;if(!d)return;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:s}=await z.from("matches").select(`id,home_id,away_id,home_score,away_score,status,mode,winner_id,created_at,played_at,
      home:users!home_id(pseudo,club_name),
      away:users!away_id(pseudo,club_name)`).or(`home_id.eq.${d.id},away_id.eq.${d.id}`).order("created_at",{ascending:!1}).limit(50),r={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},c=(s||[]).filter(o=>o.status==="finished"),f=(s||[]).filter(o=>o.status==="in_progress");function a(o){const l=o.home_id===d.id;l?o.home_score:o.away_score,l?o.away_score:o.home_score;const b=o.winner_id===d.id,h=o.home_score===o.away_score&&o.status==="finished",$=o.status!=="finished"?"…":h?"N":b?"V":"D",p=o.status!=="finished"||h?"#888":b?"#1A6B3C":"#c0392b";let u=r[o.mode]||o.mode;o.away_id===null&&!u.startsWith("IA")&&(u="IA");const A=o.home_id===d.id?o.away:o.home;let j;o.away_id===null?j=u:A?j=`${A.club_name||A.pseudo} (${A.pseudo})`:j="Adversaire";let F="";o.status==="in_progress"&&Date.now()-new Date(o.created_at).getTime()>3600*1e3&&(F=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const q=new Date(o.created_at),B=q.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+q.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),C=o.status==="finished"?`${o.home_score} - ${o.away_score}`:`${o.home_score||0} - ${o.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
