import{s as k,F as Re,b as me,c as lt,l as ge,d as Ct}from"./formation-links-CDBKB_z4.js";function We(e,{navigate:t,toast:o}){e.innerHTML=`
  ...
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",A=>{L=A.target.value.toLowerCase(),S()}),e.querySelectorAll(".filter-btn").forEach(A=>{A.addEventListener("click",()=>{w=A.dataset.job,B(),S()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{m=!m,B(),S()})):(E.innerHTML=`
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const ue=1e3;function qt(e,t,o,r){var y,h,_;const{state:c,toast:i,closeModal:n,navigate:l,refreshProfile:a}=o,d=e.formation,s={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function p(){const b=me[d]||{},x=Re[d]||[],w=290,L=360,m=20;function v(I){const T=b[I];return T?{x:T.x*w,y:T.y*L}:null}let $=`<svg width="${w}" height="${L}" viewBox="0 0 ${w} ${L}" xmlns="http://www.w3.org/2000/svg">`;for(const[I,T]of x){const B=v(I),S=v(T);!B||!S||($+=`<line x1="${B.x}" y1="${B.y}" x2="${S.x}" y2="${S.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const I of Object.keys(b)){const T=v(I);if(!T)continue;const B=I.replace(/\d+/,""),S=s[B]||"#555";$+=`<circle cx="${T.x}" cy="${T.y}" r="${m}" fill="${S}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,$+=`<text x="${T.x}" y="${T.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${B}</text>`}return $+="</svg>",$}const u=t.filter(b=>b.formation===d),f=u.length,g=!e.is_for_sale;r(`Formation ${d}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
  ...
  <div class="page">
  ...
  </div>`,Pt(e,t,c,o),document.getElementById("builder-back").addEventListener("click",()=>r("decks")),document.getElementById("formation-select").addEventListener("change",d=>{t.formation=d.target.value;const s=it(t.formation),p={};s.forEach(u=>{t.slots[u]&&(p[u]=t.slots[u])}),t.slots=p,le(e,t,o)}),document.getElementById("save-deck").addEventListener("click",()=>Ut(t,o)),e.querySelectorAll("[data-remove-sub]").forEach(d=>{d.addEventListener("click",()=>{t.subs=t.subs.filter(s=>s!==d.dataset.removeSub),le(e,t,o)})}),(a=document.getElementById("add-sub-btn"))==null||a.addEventListener("click",()=>{Kt(t,e,o)})}function Pt(e,t,o,r){const c=e.querySelector("#deck-field");if(!c)return;const i=me[t.formation]||{},n=lt(t.formation)||[],l={};for(const _ of o){const b=t.slots[_],x=b?t.playerCards.find(w=>w.id===b):null;l[_]=x?x.player:null}const a=300,d=300,s=48,p=64,u=13,f=16,g=38;function y(_){const b=i[_];return b?{x:b.x*a,y:b.y*d}:null}let h="";for(const[_,b]of n){const x=y(_),w=y(b);if(!x||!w)continue;const L=l[_]?{...l[_],club_id:l[_].club_id,country_code:l[_].country_code,rarity:l[_].rarity}:null,m=l[b]?{...l[b],club_id:l[b].club_id,country_code:l[b].country_code,rarity:l[b].rarity}:null,v=ge(L,m);v==="#ff3333"||v==="#cc2222"?h+=`<line x1="${x.x.toFixed(1)}" y1="${x.y.toFixed(1)}" x2="${w.x.toFixed(1)}" y2="${w.y.toFixed(1)}" stroke="${v}" stroke-width="2.5" stroke-linecap="round" opacity="0.4"/>`:(h+=`<line x1="${x.x.toFixed(1)}" y1="${x.y.toFixed(1)}" x2="${w.x.toFixed(1)}" y2="${w.y.toFixed(1)}" stroke="${v}" stroke-width="8" stroke-linecap="round" opacity="0.2"/>`,h+=`<line x1="${x.x.toFixed(1)}" y1="${x.y.toFixed(1)}" x2="${w.x.toFixed(1)}" y2="${w.y.toFixed(1)}" stroke="${v}" stroke-width="2.5" stroke-linecap="round" opacity="0.9"/>`)}for(const _ of o){const b=y(_);if(!b)continue;const x=l[_],w=_.replace(/\d+/,""),L=te[w]||"#555",m=(b.x-s/2).toFixed(1),v=(b.y-p/2).toFixed(1),$={normal:"#aaa",pépite:"#D4A017",papyte:"#222",légende:"#7a28b8"}[x==null?void 0:x.rarity]||"#aaa";if(x){const I=Le(x),T=ht(x),B=mt(x.country_code),S=Number(w==="GK"?x.note_g:w==="DEF"?x.note_d:w==="MIL"?x.note_m:x.note_a)||0,N=p-u-f;h+=`<defs><clipPath id="dcp-${_}"><rect x="${m}" y="${(b.y-p/2+u).toFixed(1)}" width="${s}" height="${N}"/></clipPath></defs>`,h+=`<rect x="${m}" y="${v}" width="${s}" height="${p}" rx="5" fill="${L}"/>`,I&&(h+=`<image href="${I}" x="${m}" y="${(b.y-p/2+u).toFixed(1)}" width="${s}" height="${N}" clip-path="url(#dcp-${_})" preserveAspectRatio="xMidYMin slice"/>`),h+=`<rect x="${m}" y="${v}" width="${s}" height="${u}" fill="rgba(255,255,255,0.93)"/>`,h+=`<text x="${b.x.toFixed(1)}" y="${(b.y-p/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(x.surname_encoded||"").slice(0,9)}</text>`;const K=(b.y+p/2-f).toFixed(1);h+=`<rect x="${m}" y="${K}" width="${s}" height="${f}" fill="rgba(255,255,255,0.93)"/>`,B&&(h+=`<image href="${B}" x="${(b.x-21).toFixed(1)}" y="${(b.y+p/2-f+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`),h+=`<text x="${b.x.toFixed(1)}" y="${(b.y+p/2-f/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${S}</text>`,T&&(h+=`<image href="${T}" x="${(b.x+s/2-14).toFixed(1)}" y="${(b.y+p/2-f+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`),h+=`<rect x="${m}" y="${v}" width="${s}" height="${p}" rx="5" fill="none" stroke="${$}" stroke-width="2"/>`}else h+=`<rect x="${m}" y="${v}" width="${s}" height="${p}" rx="5" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="5,3"/>`,h+=`<text x="${b.x.toFixed(1)}" y="${b.y.toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="22" fill="rgba(255,255,255,0.35)">+</text>`,h+=`<text x="${b.x.toFixed(1)}" y="${(b.y+16).toFixed(1)}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.3)">${w}</text>`;h+=`<rect x="${m}" y="${v}" width="${s}" height="${p}" rx="5" fill="rgba(0,0,0,0.01)" class="deck-slot-hit" data-pos="${_}" style="cursor:pointer"/>`}c.innerHTML=`<svg viewBox="${-g} ${-g} ${a+g*2} ${d+g*2}" width="100%" style="display:block;max-width:440px;margin:0 auto">${h}</svg>`,c.querySelectorAll(".deck-slot-hit").forEach(_=>{_.addEventListener("click",()=>Ht(_.dataset.pos,t,e,r))})}function Ht(e,t,o,r){var u,f,g;const{openModal:c,closeModal:i}=r,n=e.replace(/\d+/,""),l=t.slots[e],a=l?t.playerCards.find(y=>y.id===l):null;(u=a==null?void 0:a.player)==null||u.id;const d=new Set;Object.entries(t.slots).forEach(([y,h])=>{var b;if(y===e||!h)return;const _=t.playerCards.find(x=>x.id===h);(b=_==null?void 0:_.player)!=null&&b.id&&d.add(_.player.id)}),t.subs.forEach(y=>{var _;const h=t.playerCards.find(b=>b.id===y);(_=h==null?void 0:h.player)!=null&&_.id&&d.add(h.player.id)});const s=new Set,p=t.playerCards.filter(y=>{const h=y.player;return!(h.job===n||h.job2===n)||d.has(h.id)||s.has(h.id)?!1:(s.add(h.id),!0)});p.sort((y,h)=>{const _=n==="GK"?y.player.note_g:n==="DEF"?y.player.note_d:n==="MIL"?y.player.note_m:y.player.note_a;return(n==="GK"?h.player.note_g:n==="DEF"?h.player.note_d:n==="MIL"?h.player.note_m:h.player.note_a)-_}),c(`Choisir ${n} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
    ...
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(f=document.getElementById("close-selector"))==null||f.addEventListener("click",i),(g=document.getElementById("remove-player"))==null||g.addEventListener("click",()=>{delete t.slots[e],i(),le(o,t,r)}),document.querySelectorAll(".player-option").forEach(y=>{y.addEventListener("click",()=>{t.slots[e]=y.dataset.cardId,i(),le(o,t,r)})})}function Kt(e,t,o){var a;const{openModal:r,closeModal:c}=o,i=new Set;Object.values(e.slots).filter(Boolean).forEach(d=>{var p;const s=e.playerCards.find(u=>u.id===d);(p=s==null?void 0:s.player)!=null&&p.id&&i.add(s.player.id)}),e.subs.forEach(d=>{var p;const s=e.playerCards.find(u=>u.id===d);(p=s==null?void 0:s.player)!=null&&p.id&&i.add(s.player.id)});const n=new Set,l=e.playerCards.filter(d=>{var s,p,u;return i.has((s=d.player)==null?void 0:s.id)||n.has((p=d.player)==null?void 0:p.id)?!1:(n.add((u=d.player)==null?void 0:u.id),!0)});r("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
  ...
  `,document.body.appendChild(r);let c=!1;document.getElementById("pack-visual").addEventListener("click",()=>{if(c)return;c=!0;const f=document.getElementById("pack-visual");f.classList.add("shaking"),setTimeout(()=>{f.classList.add("pack-open"),setTimeout(()=>{document.getElementById("pack-phase").style.display="none",n(0)},600)},500)});let i=0;function n(f){i=f;const g=document.getElementById("reveal-phase");g.style.display="flex",l(f)}function l(f){var m;const g=e[f],y=document.getElementById("card-counter"),h=document.getElementById("single-card-slot"),_=document.getElementById("card-tap-hint");y&&(y.textContent=`Carte ${f+1} / ${e.length}`),_&&(_.textContent=f<e.length-1?"Appuie pour continuer →":"Appuie pour voir toutes tes cartes");const b=nt(g),x=g.card_type==="player"&&((m=g.player)==null?void 0:m.rarity)==="legende";h.innerHTML=`
      ...
      </div>`,x&&s();const w=document.getElementById("current-card-wrap");let L=!1;w.addEventListener("click",()=>{if(L)return;L=!0;const v=f+1;v<e.length?(w.style.transition="all 0.25s ease",w.style.transform="translateX(-120%) rotate(-15deg)",w.style.opacity="0",setTimeout(()=>l(v),250)):a()})}function a(){p(),document.getElementById("reveal-phase").style.display="none";const f=document.getElementById("recap-phase");f.style.display="flex";const g=document.getElementById("recap-grid");g.innerHTML=e.map((y,h)=>`
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}const gi="simulation",wt="VOTRE_ZONE_ID";function xi(){switch(gi){case"propellerads":return yi();case"adsense":return mi();default:return qe()}}function qe(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:#111;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:14px";let o=5;t.innerHTML=`
    ...
    <div style="text-align:center;padding:40px;color:#fff">
    ...
    </div>`}function a(){var g,y,h;const s=e.querySelector("#gc-confirm-overlay");if(s&&s.remove(),n.length<3)return;const p=document.createElement("div");p.id="gc-confirm-overlay",p.style.cssText="position:absolute;inset:0;background:rgba(10,22,40,0.88);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:32px 24px;z-index:20;backdrop-filter:blur(4px)",p.innerHTML=`
      ...
      <button id="gc-reset" style="width:100%;max-width:320px;padding:14px;border-radius:14px;border:2px solid rgba(212,160,23,0.4);background:rgba(212,160,23,0.1);color:#D4A017;font-size:14px;font-weight:700;cursor:pointer">
        🔄 Changer de GC
      </button>`;const u=_=>{e.style.overflow="",e.style.height="",e.style.display="",e.style.flexDirection="",o(_)};(g=p.querySelector("#gc-launch"))==null||g.addEventListener("click",()=>u(n)),(y=p.querySelector("#gc-no-gc"))==null||y.addEventListener("click",()=>u([])),(h=p.querySelector("#gc-reset"))==null||h.addEventListener("click",()=>{n=[],d()});const f=e.querySelector("#gc-screen-wrap");f?f.appendChild(p):e.appendChild(p)}function d(){e.style.overflow="hidden";const s=e.clientHeight||e.getBoundingClientRect().height;s>50&&(e.style.height=s+"px"),e.innerHTML=`
    ...
    </div>`,e.querySelectorAll(".gc-select-card").forEach(p=>{p.addEventListener("click",()=>{const u=p.dataset.id,f=i.find(y=>y.id===u);if(!f)return;const g=n.findIndex(y=>y.gc_type===f.gc_type);g>-1?n.splice(g,1):n.length<3&&n.push(f),d(),n.length===3&&a()})}),n.length===3&&a()}d()}async function ki(e,t,o){const{state:r,navigate:c}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:i}=await k.from("decks").select("id,name,is_active,formation").eq("owner_id",r.profile.id).order("created_at",{ascending:!1});if(!i||i.length===0){$t(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>c("decks"));return}const n=i.map(s=>s.id),{data:l}=await k.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",n).order("slot_order");let a=0;function d(){var b,x,w,L,m;const s=i[a],p=(l||[]).filter(v=>v.deck_id===s.id),u=p.filter(v=>{var $;return v.is_starter&&(($=v.card)==null?void 0:$.player)}).map(v=>Be(v.card)),f=p.find(v=>{var $;return(($=v.card)==null?void 0:$.card_type)==="formation"}),g=s.formation||((b=f==null?void 0:f.card)==null?void 0:b.formation)||"4-4-2",y=u.length>=11?_t(u,g):null,h=u.length>=11;e.innerHTML=`
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(m=>{var v,$,I;if(m.type==="duel"){const T=m.isGoal,B=m.homeScored?"#FFD700":T?"#ff6b6b":"rgba(255,255,255,0.3)",S=m.homeScored?"⚽ BUT !":T?"⚽ BUT IA !":(v=m.homePlayers)!=null&&v.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${T?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${B};margin-bottom:4px">
  ...
  `,document.body.appendChild(r);let s=!1;const p=()=>{s||(s=!0,r.remove(),o())};r.addEventListener("click",p),setTimeout(p,2e3)}function fe(e,t="rgba(0,0,0,0.8)"){const o=document.createElement("div");o.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,o.textContent=e,document.body.appendChild(o),setTimeout(()=>o.remove(),2200)}function rt(e,t,o,r=null){if(t.phase!=="attack"){fe("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){fe(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const c=Object.values(t.homeTeam).flat().filter(p=>p.used),i=t.homeSubs.filter(p=>!t.usedSubIds.includes(p.cardId));if(!c.length){fe("Aucun joueur utilisé à remplacer");return}if(!i.length){fe("Aucun remplaçant disponible");return}let n=0,l=Math.max(0,i.findIndex(p=>p.cardId===r)),a=!1;const d=document.createElement("div");d.id="sub-overlay",d.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function s(){var _,b,x,w,L,m;const p=c[n],u=i[l],f=Math.min(130,Math.round((window.innerWidth-90)/2)),g=Math.round(f*1.35),y=v=>`background:rgba(255,255,255,0.12);border:none;color:${v?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${v?"default":"pointer"};flex-shrink:0`;d.innerHTML=`
    ...
    </div>`,(_=d.querySelector("#sub-close"))==null||_.addEventListener("click",()=>d.remove()),(b=d.querySelector("#out-up"))==null||b.addEventListener("click",()=>{n>0&&(n--,s())}),(x=d.querySelector("#out-down"))==null||x.addEventListener("click",()=>{n<c.length-1&&(n++,s())}),(w=d.querySelector("#in-up"))==null||w.addEventListener("click",()=>{l>0&&(l--,s())}),(L=d.querySelector("#in-down"))==null||L.addEventListener("click",()=>{l<i.length-1&&(l++,s())});const h=(v,$,I,T)=>{const B=d.querySelector("#"+v);if(!B)return;let S=0;B.addEventListener("touchstart",N=>{S=N.touches[0].clientY},{passive:!0}),B.addEventListener("touchend",N=>{const K=N.changedTouches[0].clientY-S;if(Math.abs(K)<30)return;const D=$();K<0&&D<T-1?(I(D+1),s()):K>0&&D>0&&(I(D-1),s())},{passive:!0})};h("in-panel",()=>l,v=>l=v,i.length),h("out-panel",()=>n,v=>n=v,c.length),(m=d.querySelector("#sub-confirm"))==null||m.addEventListener("click",v=>{if(v.preventDefault(),v.stopPropagation(),a)return;a=!0;const $=c[n],I=i[l];if(!$||!I)return;let T=null,B=-1;for(const[N,K]of Object.entries(t.homeTeam)){const D=(K||[]).findIndex(W=>W.cardId===$.cardId);if(D!==-1){T=N,B=D;break}}if(B===-1||!T){fe("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),d.remove();return}const S={...I,_line:T,_col:$._col||0,used:!1,boost:0};t.homeTeam[T].splice(B,1,S),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(I.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:$.name,firstname:$.firstname,note:U($,T),portrait:J($),job:$.job,country_code:$.country_code,rarity:$.rarity,clubName:$.clubName,clubLogo:$.clubLogo},inPlayer:{name:I.name,firstname:I.firstname,note:U(I,T),portrait:J(I),job:I.job,country_code:I.country_code,rarity:I.rarity,clubName:I.clubName,clubLogo:I.clubLogo},text:`🔄 ${I.firstname} ${I.name} remplace ${$.firstname} ${$.name}`}),d.remove(),Ci($,I,()=>V(e,t,o))})}document.body.appendChild(d),s()}function ji(e,t,o,r,c){var g,y;const i=(r.gcDefs||[]).find(h=>h.name===t),n=de[t]||{icon:"⚡",desc:"Carte spéciale."},l={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[i==null?void 0:i.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",a={purple:"#b06ce0",light_blue:"#00d4ef"}[i==null?void 0:i.color]||"#b06ce0",d=(i==null?void 0:i.name)||t,s=(i==null?void 0:i.effect)||n.desc,p=i!=null&&i.image_url?`/manager-wars/icons/${i.image_url}`:null,u=n.icon||"⚡",f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",f.innerHTML=`
  ...
  `;function l(a){const d=document.getElementById("mkt-content"),s=a==="buy"?n:i;if(s.length===0){d.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      ${s.map(p=>{var h,_,b;const u=(h=p.card)==null?void 0:h.player;if(!u)return"";const f=u.job==="GK"?u.note_g:u.job==="DEF"?u.note_d:u.job==="MIL"?u.note_m:u.note_a,g=Di[u.rarity],y=(o.profile.credits||0)>=p.price;return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px">
          ...
          <div style="text-align:right;flex-shrink:0">
            ...
            ${a==="buy"?`<button class="btn btn-primary btn-sm" data-buy="${p.id}" ${y?"":"disabled"} style="margin-top:4px">${y?"Acheter":"Trop cher"}</button>`:`<button class="btn btn-danger btn-sm" data-cancel="${p.id}" style="margin-top:4px">Retirer</button>`}
          </div>
        </div>`}).join("")}
    </div>`,d.querySelectorAll("[data-buy]").forEach(p=>{p.addEventListener("click",()=>Ri(p.dataset.buy,s,e,t))}),d.querySelectorAll("[data-cancel]").forEach(p=>{p.addEventListener("click",()=>Ni(p.dataset.cancel,e,t))})}l("buy"),e.querySelectorAll(".mkt-tab").forEach(a=>{a.addEventListener("click",()=>{e.querySelectorAll(".mkt-tab").forEach(d=>{const s=d===a;d.style.background=s?"var(--green)":"#fff",d.style.color=s?"#fff":"var(--gray-600)",d.style.borderColor=s?"var(--green)":"var(--gray-200)"}),l(a.dataset.tab)})})}async function Ri(e,t,o,r){var p;const{state:c,toast:i}=r,n=t.find(u=>u.id===e);if(!n)return;const l=n.price,a=c.profile.credits||0,d=(p=n.card)==null?void 0:p.player;if(a<l){i("Crédits insuffisants","error");return}if(!confirm(`Acheter ${d==null?void 0:d.firstname} ${d==null?void 0:d.surname_encoded} pour ${l.toLocaleString("fr")} crédits ?`))return;const s=document.querySelector(`[data-buy="${e}"]`);s&&(s.disabled=!0,s.textContent="⏳");try{const{data:u,error:f}=await k.rpc("buy_market_card",{p_listing_id:e,p_buyer_id:c.profile.id});if(f)throw new Error(f.message);if(!(u!=null&&u.success))throw new Error((u==null?void 0:u.error)||"Achat impossible");c.profile.credits=a-l;const g=document.querySelector("[data-credits]")||document.querySelector(".credits-display");g&&(g.textContent=(a-l).toLocaleString("fr")+" cr."),i(`✅ ${d==null?void 0:d.surname_encoded} ajouté à ta collection !`,"success"),Ue(o,r)}catch(u){i("❌ "+u.message,"error"),s&&(s.disabled=!1,s.textContent="Acheter")}}async function Ni(e,t,o){const{toast:r}=o,{data:c}=await k.from("market_listings").select("card_id").eq("id",e).single();await k.from("market_listings").update({status:"cancelled"}).eq("id",e),c&&await k.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",c.card_id),r("Annonce retirée","success"),Ue(t,o)}function qi(e){return{GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[e]||"#888"}async function Oi(e,{state:t,navigate:o,toast:r}){e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:c}=await k.from("users").select("id,pseudo,club_name,trophies_top1,trophies_top2,trophies_top3,wins,level").order("trophies_top1",{ascending:!1}).limit(100);e.innerHTML=`
  <div class="page">
    <div class="page-header">
  ...
  `}async function Pi(e,{state:t,navigate:o,toast:r}){const c=t.profile;if(!c)return;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:i}=await k.from("matches").select("id,home_id,away_id,home_score,away_score,status,mode,winner_id,created_at,played_at").or(`home_id.eq.${c.id},away_id.eq.${c.id}`).order("created_at",{ascending:!1}).limit(50),n={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},l=(i||[]).filter(s=>s.status==="finished"),a=(i||[]).filter(s=>s.status==="in_progress");function d(s){const p=s.home_id===c.id;p?s.home_score:s.away_score,p?s.away_score:s.home_score;const u=s.winner_id===c.id,f=s.home_score===s.away_score&&s.status==="finished",g=s.status!=="finished"?"…":f?"N":u?"V":"D",y=s.status!=="finished"||f?"#888":u?"#1A6B3C":"#c0392b";let h=n[s.mode]||s.mode;s.away_id===null&&!h.startsWith("IA")&&(h="IA");let _="";s.status==="in_progress"&&Date.now()-new Date(s.created_at).getTime()>3600*1e3&&(_=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const b=new Date(s.created_at),x=b.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+b.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),w=s.status==="finished"?`${s.home_score} - ${s.away_score}`:`${s.home_score||0} - ${s.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
