import{s as E,F as bi,h as Pt,j as Wi,l as at,i as So,k as Lo,b as Io}from"./formation-links-DJC25wD7.js";const Mo="/manager-wars/";function Ii(e,{navigate:t,toast:i}){let s="login";const c=()=>{var a,l,p,o,n,g;const d=s==="login";e.innerHTML=`
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),o=(l||[]).filter(J=>J.card_type==="player"&&J.player),n=(l||[]).filter(J=>J.card_type==="game_changer"),g=(l||[]).filter(J=>J.card_type==="formation"),v=(l||[]).filter(J=>J.card_type==="stadium"),{data:y}=await E.from("gc_definitions").select("name,gc_type,color,effect,image_url"),z={};(y||[]).forEach(J=>{z[J.name]=J});const{data:r}=await E.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),u={};(r||[]).forEach(J=>{u[J.id]=J}),v.forEach(J=>{J.stadium_def&&(u[J.stadium_id]=J.stadium_def)});const M=Object.keys(bi),L=Object.keys(Ke),N={};o.forEach(J=>{const oe=J.player.id;N[oe]=(N[oe]||0)+1}),new Set(Object.keys(N).map(J=>String(J)));const V=new Set(g.map(J=>J.formation)),C=new Set(n.map(J=>J.gc_type));let F="player",j="Tous",te="",de=!1;function le(){return[...o].sort((J,oe)=>{const K=Vt.indexOf(J.player.job),Y=Vt.indexOf(oe.player.job);return K!==Y?K-Y:(J.player.surname_encoded||"").localeCompare(oe.player.surname_encoded||"")})}function P(){return[...p||[]].sort((J,oe)=>{const K=Vt.indexOf(J.job),Y=Vt.indexOf(oe.job);return K!==Y?K-Y:(J.surname_encoded||"").localeCompare(oe.surname_encoded||"")})}function W(){return le().filter(J=>{const oe=J.player,K=j==="Tous"||oe.job===j,Y=!te||`${oe.firstname} ${oe.surname_encoded}`.toLowerCase().includes(te);return K&&Y})}function H(){return P().filter(J=>{const oe=j==="Tous"||J.job===j,K=!te||`${J.firstname} ${J.surname_encoded}`.toLowerCase().includes(te);return oe&&K})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
  ...
  </div>`;function G(){const J=document.getElementById("col-filters");J&&(F==="player"?(J.innerHTML=`
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",oe=>{te=oe.target.value.toLowerCase(),ne()}),e.querySelectorAll(".filter-btn").forEach(oe=>{oe.addEventListener("click",()=>{j=oe.dataset.job,G(),ne()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{de=!de,G(),ne()})):(J.innerHTML=`
        <div style="display:flex;justify-content:flex-end">
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{de=!de,G(),ne()})))}function ne(){const J=document.getElementById("col-grid");J&&(F==="player"?$e(J):F==="formation"?Se(J):F==="stadium"?xe(J):Le(J))}function fe(J,oe,K,Y,ae){const ie=document.getElementById("col-grid"),re=document.getElementById("col-big");if(!ie||!re)return;var Te=0;function we(){const Ae=window.innerWidth>=768,Ce=document.getElementById("col-big"),De=document.getElementById("col-grid");Ae&&Ce&&(Ce.style.minHeight="420px",Ce.style.flex="1 1 auto"),Ae&&De&&(De.style.height=Math.round(310*.76+16)+"px",De.style.flexShrink="0",De.style.overflowX="auto",De.style.overflowY="hidden",De.style.alignItems="center",De.style.padding="8px 16px"),re.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+oe(J[Te])+"</div>";var I=re.querySelector("[data-card-id],[data-form-id],[data-gc-id]");I&&I.addEventListener("click",function(){Y(J[Te])}),requestAnimationFrame(function(){var q=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!q||!re)){var w=re.clientHeight-8,k=re.clientWidth-24,f=q.offsetHeight,m=q.offsetWidth;if(f>0&&m>0&&w>40){var x=Ae?2.2:1,b=Math.min(w/f,k/m,x);q.style.transform="scale("+b.toFixed(3)+")",q.style.transformOrigin="top center"}}}),ie.innerHTML=J.map(function(q,w){var k=w===Te,f="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+(k?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+w+'" style="'+f+'">'+K(q,k)+"</div>"}).join(""),ie.querySelectorAll(".col-mini-item").forEach(function(q){q.addEventListener("click",function(){Te=Number(q.dataset.idx),we(),q.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}we()}function ye(J){var oe=window.innerWidth>=768?.76:.54,K;if(!J||J._fake){var Y=J?J.player:null;if(!Y)return"";K=Fi(Y)}else K=si(J,"");return'<div style="display:inline-block;zoom:'+oe+';pointer-events:none;line-height:0">'+K+"</div>"}function ze(J,oe,K){oe=oe||100,K=K||140;var Y=Pt[J]||{},ae={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},ie=Math.max(3,Math.round(oe*.06)),re=Object.entries(Y).map(function(we){var Ae=we[0],Ce=we[1],De=Ae.replace(/\d+$/,""),I=ae[De]||"#888",q=Math.round(Ce.x*oe),w=Math.round(Ce.y*K);return'<circle cx="'+q+'" cy="'+w+'" r="'+ie+'" fill="'+I+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),Te=Math.max(1,Math.round(oe/50));return'<svg viewBox="0 0 '+oe+" "+K+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+oe+'" height="'+K+'" fill="#1A6B3C"/><rect x="'+Math.round(oe*.2)+'" y="'+Math.round(K*.02)+'" width="'+Math.round(oe*.6)+'" height="'+Math.round(K*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+Te+'"/><line x1="0" y1="'+Math.round(K*.5)+'" x2="'+oe+'" y2="'+Math.round(K*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+Te+'"/><ellipse cx="'+Math.round(oe*.5)+'" cy="'+Math.round(K*.5)+'" rx="'+Math.round(oe*.18)+'" ry="'+Math.round(K*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+Te+'"/><rect x="'+Math.round(oe*.2)+'" y="'+Math.round(K*.82)+'" width="'+Math.round(oe*.6)+'" height="'+Math.round(K*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+Te+'"/>'+re+"</svg>"}function Ee(J,oe,K){var Y=K>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+K+"</div>":"",ae=oe?'data-form-id="'+oe.id+'"':"",ie=J.length>7?14:J.length>5?16:19,re=!!oe;return"<div "+ae+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(re?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(re?"":"filter:grayscale(1);opacity:0.5")+'">'+Y+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(re?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+ie+"px;font-weight:900;color:"+(re?"#1A6B3C":"#aaa")+';line-height:1">'+J+'</div></div><div style="flex:1;overflow:hidden;background:'+(re?"#1A6B3C":"#ccc")+'">'+ze(J,140,220)+"</div></div>"}function be(J,oe){var K=window.innerWidth>=768?.76:.54,Y=140,ae=305,ie=Math.round(ae*.22),re=ae-ie,Te=J.length>7?10:13,we=ze(J,Y,re),Ae=oe?"1.5px solid #2a7a40":"1px solid #ddd",Ce=oe?"":"filter:grayscale(1);opacity:0.45;",De=oe?"#1A6B3C":"#bbb",I="#fff";return'<div style="display:inline-block;zoom:'+K+';line-height:0;pointer-events:none"><div style="width:'+Y+"px;height:"+ae+"px;border-radius:6px;border:"+Ae+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+Ce+'"><div style="height:'+ie+"px;background:"+De+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+Te+"px;font-weight:900;color:"+I+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(Y-4)+'px">'+J+'</span></div><div style="height:'+re+'px;overflow:hidden;flex-shrink:0">'+we+"</div></div></div>"}function $e(J){if(de){const oe=H();if(!oe.length){J.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const K=oe.map(Y=>o.find(ae=>ae.player.id===Y.id)||{_fake:!0,player:Y,id:"fake-"+Y.id});fe(K,Y=>Y._fake?Fi(Y.player):si(Y,""),Y=>Y._fake?ye({player:Y.player,id:"x",_fake:!0}):ye(Y),Y=>{Y._fake||Di(Y,o,N,t)})}else{const oe=W();if(!oe.length){J.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const K={};oe.forEach(ae=>{const ie=ae.player.id;K[ie]||(K[ie]=[]),K[ie].push(ae)});const Y=Object.values(K).map(ae=>Xo(ae));fe(Y,ae=>{const ie=N[ae.player.id]||1,re=ie>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${ie}</div>`:"",we=o.filter(Ae=>Ae.player.id===ae.player.id&&Ae.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return si(ae,re+we)},ae=>ye(ae),ae=>Di(ae,o,N,t))}}function Se(J){const oe=de?M:[...V];if(!oe.length){J.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const K=oe.map(Y=>({formation:Y,card:g.find(ae=>ae.formation===Y)||null,owned:V.has(Y)}));fe(K,({formation:Y,card:ae,owned:ie})=>Ee(Y,ie?ae:null,ie?g.filter(re=>re.formation===Y).length:0),({formation:Y,owned:ae})=>be(Y,ae),({card:Y,owned:ae})=>{ae&&Y&&en(Y,g,t,d)})}function Le(J){const oe=Object.keys(z),K=de?oe.length?oe:L:[...C];if(!K.length){J.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const Y=K.map(ae=>({type:ae,gc:Ke[ae]||{icon:"⚡",desc:""},def:z[ae]||null,owned:C.has(ae),card:n.find(ie=>ie.gc_type===ae)||null}));fe(Y,({type:ae,gc:ie,def:re,owned:Te,card:we})=>{var x;const Ae=Te?n.filter(b=>b.gc_type===ae).length:0,Ce=Ae>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${Ae}</div>`:"",De=(re==null?void 0:re.gc_type)==="ultra_game_changer",I={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},q={purple:"#b06ce0",light_blue:"#00d4ef"},w=I[re==null?void 0:re.color]||I.purple,k=q[re==null?void 0:re.color]||q.purple,f=(re==null?void 0:re.effect)||ie.desc||"",m=re!=null&&re.image_url?`/manager-wars/icons/${re.image_url}`:((x=re==null?void 0:re.club)==null?void 0:x.logo_url)||(re!=null&&re.country_code?`https://flagcdn.com/w80/${re.country_code.toLowerCase()}.png`:null);return Te&&we?`<div data-gc-id="${we.id}" data-gc-type="${ae}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${k};background:${w};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${k}66;cursor:pointer">
          ${Ce}
          <div style="padding:10px 12px;background:rgba(255,255,255,0.14);text-align:center">
            <div style="font-size:${ae.length>14?10:13}px;font-weight:900;color:#fff;letter-spacing:.5px;text-transform:uppercase">${ae}</div>
          ...
          <div style="padding:10px 12px;background:rgba(0,0,0,0.05);text-align:center"><div style="font-size:13px;font-weight:900;color:#888;text-transform:uppercase">${ae}</div></div>
        ...
        </div>`},({type:ae,gc:ie,def:re,owned:Te})=>{const we=window.innerWidth>=768?.76:.54,Ae={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},Ce={purple:"#9b59b6",light_blue:"#00bcd4"},De=Ae[re==null?void 0:re.color]||Ae.purple,I=Ce[re==null?void 0:re.color]||Ce.purple,q=re!=null&&re.image_url?`/manager-wars/icons/${re.image_url}`:null;return Te?`<div style="display:inline-block;zoom:${we};line-height:0;pointer-events:none"><div style="width:140px;height:310px;border-radius:8px;background:${De};border:1px solid ${I};display:flex;flex-direction:column;overflow:hidden"><div style="height:56px;background:rgba(255,255,255,0.15);display:flex;align-items:center;justify-content:center"><span style="font-size:13px;font-weight:900;color:#fff;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:130px">${ae}</span></div><div style="flex:1;display:flex;align-items:center;justify-content:center">${q?`<img src="${q}" style="max-width:130px;max-height:190px;object-fit:contain">`:`<span style="font-size:48px">${ie.icon}</span>`}</div><div style="height:54px;display:flex;align-items:center;justify-content:center;padding:0 4px"><span style="font-size:10px;color:rgba(255,255,255,0.7);text-align:center">${((re==null?void 0:re.effect)||ie.desc||"").slice(0,30)}</span></div></div></div>`:`<div style="display:inline-block;zoom:${we};line-height:0;pointer-events:none"><div style="width:140px;height:310px;border-radius:8px;background:#eee;border:1px solid #ddd;display:flex;flex-direction:column;align-items:center;justify-content:center;filter:grayscale(1);opacity:0.45"><span style="font-size:36px">${ie.icon}</span><span style="font-size:12px;color:#aaa;margin-top:6px;text-align:center;padding:0 6px">${ae}</span></div></div>`},({type:ae,owned:ie,def:re})=>{ie&&Qo(ae,re,d)})}function xe(J){const oe="#E87722",K="/manager-wars/";if(!v.length){J.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Stade.<br><small>Ouvre un booster Stade !</small></div>';return}const Y={};v.forEach(ie=>{const re=ie.stadium_id||"?";(Y[re]=Y[re]||[]).push(ie)});const ae=Object.entries(Y).map(([ie,re])=>({sid:ie,def:u[ie]||null,count:re.length,card:re[0]}));fe(ae,({def:ie,count:re})=>{var I,q;const Te=(ie==null?void 0:ie.name)||"?",we=((I=ie==null?void 0:ie.club)==null?void 0:I.encoded_name)||(ie==null?void 0:ie.country_code)||"—",Ae=ie!=null&&ie.image_url?`${K}icons/${ie.image_url}`:((q=ie==null?void 0:ie.club)==null?void 0:q.logo_url)||(ie!=null&&ie.country_code?`https://flagcdn.com/w80/${ie.country_code.toLowerCase()}.png`:null),Ce=Ae?`<img src="${Ae}" style="width:90px;height:90px;object-fit:contain;border-radius:4px" onerror="this.style.display='none'">`:'<div style="font-size:56px">🏟️</div>',De=re>1?`<div style="position:absolute;top:8px;right:8px;background:#333;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${re}</div>`:"";return`<div style="position:relative;width:140px;border-radius:12px;border:3px solid #c45a00;background:linear-gradient(160deg,${oe},#c45a00);display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${oe}66">
          ${De}
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Kt=1e3;function en(e,t,i,s){var z,r,u;const{state:c,toast:d,closeModal:a,navigate:l,refreshProfile:p}=i,o=e.formation,n={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function g(){const M=Pt[o]||{},L=bi[o]||[],N=290,V=360,C=20;function F(te){const de=M[te];return de?{x:de.x*N,y:de.y*V}:null}let j=`<svg width="${N}" height="${V}" viewBox="0 0 ${N} ${V}" xmlns="http://www.w3.org/2000/svg">`;for(const[te,de]of L){const le=F(te),P=F(de);!le||!P||(j+=`<line x1="${le.x}" y1="${le.y}" x2="${P.x}" y2="${P.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const te of Object.keys(M)){const de=F(te);if(!de)continue;const le=te.replace(/\d+/,""),P=n[le]||"#555";j+=`<circle cx="${de.x}" cy="${de.y}" r="${C}" fill="${P}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,j+=`<text x="${de.x}" y="${de.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${le}</text>`}return j+="</svg>",j}const v=t.filter(M=>M.formation===o);v.length;const y=!e.is_for_sale;s(`Formation ${o}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    <div style="display:flex;justify-content:space-between;align-items:flex-start;background:#f9f9f9;border-radius:8px;padding:8px 12px;gap:8px">
      ...
      <div style="text-align:right;flex-shrink:0">
        <div style="font-size:13px;font-weight:700;color:${K.source==="booster"?"var(--gray-600)":"var(--yellow)"}">${ae}</div>
    ...
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${g.map((K,Y)=>{const ae=ne[K.id]||[],ie=K.is_for_sale,re=ae.length?ae[ae.length-1]:null,we=(n.rarity==="pepite"||n.rarity==="papyte")&&K.current_note!=null?` (☆${K.current_note})`:"";return`
            <div data-card-id="${K.id}" data-card-idx="${Y}" class="exemplaire-row"
              ...
              <div style="display:flex;align-items:center;gap:10px">
                ...
                <div style="flex:1;min-width:0">
                  <div style="font-size:12px;font-weight:700">Exemplaire ${Y+1}${we}${ie?" 🏷️ En vente":""}</div>
                ...
                <button class="expl-hist-toggle" data-idx="${Y}" style="font-size:11px;color:#999;border:none;background:none;cursor:pointer;padding:2px 6px;flex-shrink:0">
                  ${ae.length?`${ae.length} club${ae.length>1?"s":""}`:""} ▾
                </button>
              </div>
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;background:#f0fdf4;border-radius:10px;padding:10px 14px">
          <div>
            ...
            <div style="font-size:11px;color:#555;margin-top:2px">
              ${(e.evolution_bonus||0)>0?`+${e.evolution_bonus} appliqué${e.evolution_bonus>1?"s":""} · Note : ${P}${n.job2&&W>0?` / ${H}`:""}`:`Note actuelle : ${P}${n.job2&&W>0?` / ${H}`:""}`}
            </div>
          </div>
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",n.note_g],["DEF",n.note_d],["MIL",n.note_m],["ATT",n.note_a]].map(([K,Y])=>{const ae=jt[K],ie=K===n.job||K===n.job2,re=(Number(Y)||0)+(ie&&de>0?de:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:2px">
                <svg width="38" height="37" viewBox="0 0 38 37" style="display:block">
                  <polygon points="19,2 24,13 36,13 26,21 30,33 19,26 8,33 12,21 2,13 14,13" fill="${ae}" stroke="white" stroke-width="1.5"/>
    ...
    ${ye}
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(Le=document.getElementById("close-detail"))==null||Le.addEventListener("click",l);let ze=new Set;const Ee=()=>{const K=ze.size,Y=document.getElementById("sell-action-panel");if(!Y)return;Y.style.display=K>0?"block":"none",document.getElementById("sell-selected-count").textContent=K;const ae=document.getElementById("evolve-btn");ae&&(ae.textContent=`⬆️ Évoluer ${K>1?"(+"+K+")":""}`)};document.querySelectorAll(".expl-check").forEach(K=>{K.addEventListener("change",()=>{const Y=K.dataset.id;K.checked?ze.add(Y):ze.delete(Y);const ae=K.closest(".exemplaire-row");ae&&(ae.style.borderColor=K.checked?"#1A6B3C":"#eee"),Ee()})}),document.querySelectorAll(".exemplaire-row").forEach(K=>{K.addEventListener("click",Y=>{if(Y.target.closest("button")||Y.target.tagName==="INPUT")return;const ae=K.querySelector(".expl-check");ae&&!ae.disabled&&(ae.checked=!ae.checked,ae.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(K=>{K.addEventListener("click",Y=>{Y.stopPropagation();const ae=document.querySelector(`.expl-hist[data-hist="${K.dataset.idx}"]`);ae&&(ae.style.display=ae.style.display==="none"?"flex":"none")})}),(xe=document.getElementById("evolve-btn"))==null||xe.addEventListener("click",async()=>{const K=[...ze];if(!K.length)return;const Y=K.filter(we=>we!==e.id);if(Y.length,!(!Y.length&&K.length===1&&K[0]===e.id)){if(!Y.length){d("Sélectionne des copies à sacrifier","warning");return}}if(!await new Promise(we=>{const Ae=document.createElement("div");Ae.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",Ae.innerHTML=`
        <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center;box-shadow:0 16px 48px rgba(0,0,0,0.3)">
          ...
          <div style="background:#f0fdf4;border-radius:10px;padding:12px;margin-bottom:16px;font-size:13px;color:#333">
            ...
            📈 Note : <strong>${L+(e.evolution_bonus||0)}</strong> → <strong style="color:#1A6B3C">${L+(e.evolution_bonus||0)+Y.length}</strong>
            ${V&&V>0?`<br>📈 Note 2 : <strong>${V+(e.evolution_bonus||0)}</strong> → <strong style="color:#1A6B3C">${V+(e.evolution_bonus||0)+Y.length}</strong>`:""}
          </div>
  ...
  </div>`,on(e,t,c,i),document.getElementById("builder-back").addEventListener("click",()=>s("decks")),document.getElementById("formation-select").addEventListener("change",n=>{t.formation=n.target.value;const g=Pi(t.formation),v={};g.forEach(y=>{t.slots[y]&&(v[y]=t.slots[y])}),t.slots=v,St(e,t,i)}),(p=document.getElementById("stadium-select"))==null||p.addEventListener("change",n=>{t.stadiumCardId=n.target.value||null}),document.getElementById("save-deck").addEventListener("click",()=>an(t,i)),e.querySelectorAll("[data-remove-sub]").forEach(n=>{n.addEventListener("click",()=>{t.subs=t.subs.filter(g=>g!==n.dataset.removeSub),St(e,t,i)})}),(o=document.getElementById("add-sub-btn"))==null||o.addEventListener("click",()=>{rn(t,e,i)})}function on(e,t,i,s){var N,V;const c=e.querySelector("#deck-field");if(!c)return;const d=(N=t.stadiumCards)==null?void 0:N.find(C=>C.id===t.stadiumCardId),a=d&&((V=t.stadDefMap)==null?void 0:V[d.stadium_id])||null,l=Pt[t.formation]||{},p=Wi(t.formation)||[],o={};for(const C of i){const F=t.slots[C],j=F?t.playerCards.find(te=>te.id===F):null;o[C]=j?j.player:null}const n=300,g=300,v=48,y=64,z=13,r=16,u=38;function M(C){const F=l[C];return F?{x:F.x*n,y:F.y*g}:null}let L="";for(const[C,F]of p){const j=M(C),te=M(F);if(!j||!te)continue;const de=o[C]?{...o[C],club_id:o[C].club_id,country_code:o[C].country_code,rarity:o[C].rarity}:null,le=o[F]?{...o[F],club_id:o[F].club_id,country_code:o[F].country_code,rarity:o[F].rarity}:null,P=at(de,le);P==="#ff3333"||P==="#cc2222"?L+=`<line x1="${j.x.toFixed(1)}" y1="${j.y.toFixed(1)}" x2="${te.x.toFixed(1)}" y2="${te.y.toFixed(1)}" stroke="${P}" stroke-width="2.5" stroke-linecap="round" opacity="0.4"/>`:(L+=`<line x1="${j.x.toFixed(1)}" y1="${j.y.toFixed(1)}" x2="${te.x.toFixed(1)}" y2="${te.y.toFixed(1)}" stroke="${P}" stroke-width="8" stroke-linecap="round" opacity="0.2"/>`,L+=`<line x1="${j.x.toFixed(1)}" y1="${j.y.toFixed(1)}" x2="${te.x.toFixed(1)}" y2="${te.y.toFixed(1)}" stroke="${P}" stroke-width="2.5" stroke-linecap="round" opacity="0.9"/>`)}for(const C of i){const F=M(C);if(!F)continue;const j=o[C],te=C.replace(/\d+/,""),de=vt[te]||"#555",le=(F.x-v/2).toFixed(1),P=(F.y-y/2).toFixed(1),W={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[j==null?void 0:j.rarity]||"#aaa";if(j){const H=ti(j),G=co(j),ne=lo(j.country_code),fe=Number(te==="GK"?j.note_g:te==="DEF"?j.note_d:te==="MIL"?j.note_m:j.note_a)||0,ye=a&&(a.club_id&&String(j.club_id)===String(a.club_id)||a.country_code&&j.country_code===a.country_code),ze=ye?fe+10:fe,Ee=y-z-r;L+=`<defs><clipPath id="dcp-${C}"><rect x="${le}" y="${(F.y-y/2+z).toFixed(1)}" width="${v}" height="${Ee}"/></clipPath></defs>`,L+=`<rect x="${le}" y="${P}" width="${v}" height="${y}" rx="5" fill="${de}"/>`,H&&(L+=`<image href="${H}" x="${le}" y="${(F.y-y/2+z).toFixed(1)}" width="${v}" height="${Ee}" clip-path="url(#dcp-${C})" preserveAspectRatio="xMidYMin slice"/>`),L+=`<rect x="${le}" y="${P}" width="${v}" height="${z}" fill="rgba(255,255,255,0.93)"/>`,L+=`<text x="${F.x.toFixed(1)}" y="${(F.y-y/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(j.surname_encoded||"").slice(0,9)}</text>`;const be=(F.y+y/2-r).toFixed(1);L+=`<rect x="${le}" y="${be}" width="${v}" height="${r}" fill="rgba(255,255,255,0.93)"/>`,ne&&(L+=`<image href="${ne}" x="${(F.x-21).toFixed(1)}" y="${(F.y+y/2-r+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`),L+=`<text x="${F.x.toFixed(1)}" y="${(F.y+y/2-r/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${ze}</text>`,G&&(L+=`<image href="${G}" x="${(F.x+v/2-14).toFixed(1)}" y="${(F.y+y/2-r+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`),L+=`<rect x="${le}" y="${P}" width="${v}" height="${y}" rx="5" fill="none" stroke="${ye?"#E87722":W}" stroke-width="${ye?"2.5":"2"}"/>`,ye&&(L+=`<rect x="${(F.x+v/2-13).toFixed(1)}" y="${P}" width="13" height="9" rx="3" fill="#E87722"/>`,L+=`<text x="${(F.x+v/2-6.5).toFixed(1)}" y="${(F.y-y/2+4.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="900" fill="#fff" font-family="Arial">+10</text>`)}else L+=`<rect x="${le}" y="${P}" width="${v}" height="${y}" rx="5" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="5,3"/>`,L+=`<text x="${F.x.toFixed(1)}" y="${F.y.toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="22" fill="rgba(255,255,255,0.35)">+</text>`,L+=`<text x="${F.x.toFixed(1)}" y="${(F.y+16).toFixed(1)}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.3)">${te}</text>`;L+=`<rect x="${le}" y="${P}" width="${v}" height="${y}" rx="5" fill="rgba(0,0,0,0.01)" class="deck-slot-hit" data-pos="${C}" style="cursor:pointer"/>`}c.innerHTML=`<svg viewBox="${-u} ${-u} ${n+u*2} ${g+u*2}" width="100%" style="display:block;max-width:440px;margin:0 auto">${L}</svg>`,c.querySelectorAll(".deck-slot-hit").forEach(C=>{C.addEventListener("click",()=>nn(C.dataset.pos,t,e,s))})}function nn(e,t,i,s){var v,y,z;const{openModal:c,closeModal:d}=s,a=e.replace(/\d+/,""),l=t.slots[e],p=l?t.playerCards.find(r=>r.id===l):null;(v=p==null?void 0:p.player)==null||v.id;const o=new Set;Object.entries(t.slots).forEach(([r,u])=>{var L;if(r===e||!u)return;const M=t.playerCards.find(N=>N.id===u);(L=M==null?void 0:M.player)!=null&&L.id&&o.add(M.player.id)}),t.subs.forEach(r=>{var M;const u=t.playerCards.find(L=>L.id===r);(M=u==null?void 0:u.player)!=null&&M.id&&o.add(u.player.id)});const n=new Set,g=t.playerCards.filter(r=>{const u=r.player;return!(u.job===a||u.job2===a)||o.has(u.id)||n.has(u.id)?!1:(n.add(u.id),!0)});g.sort((r,u)=>{const M=a==="GK"?r.player.note_g:a==="DEF"?r.player.note_d:a==="MIL"?r.player.note_m:r.player.note_a;return(a==="GK"?u.player.note_g:a==="DEF"?u.player.note_d:a==="MIL"?u.player.note_m:u.player.note_a)-M}),c(`Choisir ${a} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      card:cards(id,card_type,formation,stadium_id,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order"),p=[...new Set((l||[]).filter(v=>{var y,z;return((y=v.card)==null?void 0:y.card_type)==="stadium"&&((z=v.card)==null?void 0:z.stadium_id)}).map(v=>v.card.stadium_id))],o={};if(p.length){const{data:v}=await E.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",p);(v||[]).forEach(y=>{o[y.id]=y}),(l||[]).forEach(y=>{var z,r;((z=y.card)==null?void 0:z.card_type)==="stadium"&&((r=y.card)!=null&&r.stadium_id)&&(y.card._stadiumDef=o[y.card.stadium_id]||null)})}let n=0;function g(){var C,F,j,te,de,le,P;const v=d[n],y=(l||[]).filter(W=>W.deck_id===v.id),z=y.filter(W=>{var H;return W.is_starter&&((H=W.card)==null?void 0:H.player)}).map(W=>gi(W.card,W.position)),r=y.find(W=>{var H;return((H=W.card)==null?void 0:H.card_type)==="formation"}),u=v.formation||((C=r==null?void 0:r.card)==null?void 0:C.formation)||"4-4-2";let M=z.length>=11?yt(z,u):null,L=((F=v.stadium_card)==null?void 0:F.stadium_def)||null;L&&M&&(M=Ht(M,L));const N=z.length>=11;et(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">
        ${d.map((W,H)=>`<div style="width:7px;height:7px;border-radius:50%;background:${H===n?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}
      </div>`:""}
    ...
    </div>`,function(){const H=e.querySelector(".deck-preview-wrap svg");H&&(H.removeAttribute("width"),H.removeAttribute("height"),H.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",H.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),(te=document.getElementById("prev-deck"))==null||te.addEventListener("click",()=>{n>0&&(n--,g())}),(de=document.getElementById("next-deck"))==null||de.addEventListener("click",()=>{n<d.length-1&&(n++,g())}),(le=document.getElementById("validate-deck"))==null||le.addEventListener("click",()=>{if(!N)return;const W=t.state.params||{};t.navigate("match",{...W,matchMode:W.matchMode||i,deckId:v.id})}),(P=document.getElementById("cancel-deck-select"))==null||P.addEventListener("click",()=>{We(e),c("home")});const V=document.getElementById("deck-swipe-zone");if(V){let W=0,H=0;V.addEventListener("touchstart",G=>{W=G.touches[0].clientX,H=G.touches[0].clientY},{passive:!0}),V.addEventListener("touchend",G=>{const ne=G.changedTouches[0].clientX-W,fe=G.changedTouches[0].clientY-H;Math.abs(ne)<40||Math.abs(ne)<Math.abs(fe)||(ne<0&&n<d.length-1?(n++,g()):ne>0&&n>0&&(n--,g()))},{passive:!0})}}g()}function ho(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";return e!=null&&e.clubLogo?e.clubLogo.startsWith("http")?e.clubLogo:`${t}/storage/v1/object/public/assets/clubs/${e.clubLogo}`:null}function Ye(e,t=44,i=58){if(!e)return`<div style="width:${t}px;height:${i}px;border:1.5px dashed rgba(255,255,255,0.2);border-radius:5px"></div>`;const s=typeof e.portrait=="string"&&e.portrait.startsWith("http")?e.portrait:Pe(e),c=ho(e),d=e._line||e.job||"MIL",a=It[d]||"#555",l={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",p=e.note!==void 0?Number(e.note)||0:(Number(Ie(e,d))||0)+(e.boost||0),o=vo(e==null?void 0:e.country_code),n=Math.round(i*.19),g=Math.round(i*.25),v=i-Math.round(i*.19)-Math.round(i*.25),y=e!=null&&e.used?.28:1;return`<div style="width:${t}px;height:${i}px;border-radius:5px;border:2px solid ${l};background:${a};position:relative;overflow:hidden;flex-shrink:0;opacity:${y}">
    <div style="position:absolute;top:0;left:0;right:0;height:${n}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:center;z-index:2">
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    ${(()=>{const P=window.innerWidth>=700,W=($e,Se,Le)=>{var Ce,De;const xe=(t.gcDefs||[]).find(I=>I.name===$e.gc_type),J={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[xe==null?void 0:xe.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",oe={purple:"#b06ce0",light_blue:"#00d4ef"}[xe==null?void 0:xe.color]||"#b06ce0",K=(xe==null?void 0:xe.name)||$e.gc_type,Y=(xe==null?void 0:xe.effect)||((Ce=Ke[$e.gc_type])==null?void 0:Ce.desc)||"",ae=xe!=null&&xe.image_url?`/manager-wars/icons/${xe.image_url}`:null,ie=((De=Ke[$e.gc_type])==null?void 0:De.icon)||"⚡",re=Math.round(Le*.22),Te=Math.round(Le*.22),we=Le-re-Te,Ae=K.length>12?7:9;return`<div class="gc-mini" data-gc-id="${$e.id}" data-gc-type="${$e.gc_type}"
          style="box-sizing:border-box;width:${Se}px;height:${Le}px;border-radius:10px;border:2px solid ${oe};background:${J};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
          ...
          <div style="height:${we}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
            ${ae?`<img src="${ae}" style="max-width:${Se-10}px;max-height:${we-6}px;object-fit:contain">`:`<span style="font-size:${Math.round(we*.55)}px">${ie}</span>`}
          </div>
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(P=>{var W,H,G;if(P.type==="duel"){const ne=P.isGoal,fe=P.homeScored?"#FFD700":ne?"#ff6b6b":"rgba(255,255,255,0.3)",ye=P.homeScored?"⚽ BUT !":ne?"⚽ BUT IA !":(W=P.homePlayers)!=null&&W.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${ne?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${fe};margin-bottom:4px">
  ...
  `,document.body.appendChild(s);let n=!1;const g=()=>{n||(n=!0,s.remove(),i())};s.addEventListener("click",g),setTimeout(g,2e3)}function Mt(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function di(e,t,i,s=null,c=null){var y,z;if(t.phase!=="attack"){Mt("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){Mt(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const d=Object.entries(t.homeTeam).flatMap(([r,u])=>(u||[]).filter(M=>M.used).map(M=>({...M,_line:M._line||r}))),a=t.homeSubs.filter(r=>!t.usedSubIds.includes(r.cardId));if(!d.length){Mt("Aucun joueur utilisé à remplacer");return}if(!a.length){Mt("Aucun remplaçant disponible");return}let l=Math.max(0,d.findIndex(r=>r.cardId===c));const p=((y=d[l])==null?void 0:y._line)||((z=d[l])==null?void 0:z.job);let o=s?Math.max(0,a.findIndex(r=>r.cardId===s)):Math.max(0,a.findIndex(r=>r.job===p)),n=!1;const g=document.createElement("div");g.id="sub-overlay",g.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function v(){var C,F,j,te,de,le;const r=d[l],u=a[o],M=Math.min(130,Math.round((window.innerWidth-90)/2)),L=Math.round(M*1.35),N=P=>`background:rgba(255,255,255,0.12);border:none;color:${P?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${P?"default":"pointer"};flex-shrink:0`;g.innerHTML=`
    ...
    </div>`,(C=g.querySelector("#sub-close"))==null||C.addEventListener("click",()=>g.remove()),(F=g.querySelector("#out-up"))==null||F.addEventListener("click",()=>{l>0&&(l--,v())}),(j=g.querySelector("#out-down"))==null||j.addEventListener("click",()=>{l<d.length-1&&(l++,v())}),(te=g.querySelector("#in-up"))==null||te.addEventListener("click",()=>{o>0&&(o--,v())}),(de=g.querySelector("#in-down"))==null||de.addEventListener("click",()=>{o<a.length-1&&(o++,v())});const V=(P,W,H,G)=>{const ne=g.querySelector("#"+P);if(!ne)return;let fe=0;ne.addEventListener("touchstart",ye=>{fe=ye.touches[0].clientY},{passive:!0}),ne.addEventListener("touchend",ye=>{const ze=ye.changedTouches[0].clientY-fe;if(Math.abs(ze)<30)return;const Ee=W();ze<0&&Ee<G-1?(H(Ee+1),v()):ze>0&&Ee>0&&(H(Ee-1),v())},{passive:!0})};V("in-panel",()=>o,P=>o=P,a.length),V("out-panel",()=>l,P=>l=P,d.length),(le=g.querySelector("#sub-confirm"))==null||le.addEventListener("click",P=>{if(P.preventDefault(),P.stopPropagation(),n)return;n=!0;const W=d[l],H=a[o];if(!W||!H)return;let G=null,ne=-1;for(const[ye,ze]of Object.entries(t.homeTeam)){const Ee=(ze||[]).findIndex(be=>be.cardId===W.cardId);if(Ee!==-1){G=ye,ne=Ee;break}}if(ne===-1||!G){Mt("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),g.remove();return}const fe={...H,_line:G,_col:W._col||0,used:!1,boost:0};t.homeTeam[G].splice(ne,1,fe),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(H.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:W.name,firstname:W.firstname,note:Ie(W,G),portrait:Pe(W),job:W.job,country_code:W.country_code,rarity:W.rarity,clubName:W.clubName,clubLogo:W.clubLogo},inPlayer:{name:H.name,firstname:H.firstname,note:Ie(H,G),portrait:Pe(H),job:H.job,country_code:H.country_code,rarity:H.rarity,clubName:H.clubName,clubLogo:H.clubLogo},text:`🔄 ${H.firstname} ${H.name} remplace ${W.firstname} ${W.name}`}),g.remove(),Dn(W,H,()=>Xe(e,t,i))})}document.body.appendChild(g),v()}function Gn(e,t,i,s,c){var z,r;const d=(s.gcDefs||[]).find(u=>u.name===t),a=Ke[t]||{icon:"⚡",desc:"Carte spéciale."},l={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[d==null?void 0:d.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",p={purple:"#b06ce0",light_blue:"#00d4ef"}[d==null?void 0:d.color]||"#b06ce0",o=(d==null?void 0:d.name)||t,n=(d==null?void 0:d.effect)||a.desc,g=d!=null&&d.image_url?`/manager-wars/icons/${d.image_url}`:null,v=a.icon||"⚡",y=document.createElement("div");y.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",y.innerHTML=`
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(d);let l=!1;const p=()=>{l||(l=!0,d.remove(),setTimeout(()=>c(),50))};d.addEventListener("click",p),setTimeout(p,3500)}async function qt(e,t,i){var n,g;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:s}=i,c=t.homeScore>t.aiScore,d=t.homeScore===t.aiScore,a=c?"victoire":d?"nul":"defaite",l=Ko(t.mode,a);t.matchId&&await E.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:c?s.profile.id:null,home_credits_reward:l,played_at:new Date().toISOString()}).eq("id",t.matchId);const p={credits:(s.profile.credits||0)+l,matches_played:(s.profile.matches_played||0)+1};c?p.wins=(s.profile.wins||0)+1:d?p.draws=(s.profile.draws||0)+1:p.losses=(s.profile.losses||0)+1,await E.from("users").update(p).eq("id",s.profile.id),await i.refreshProfile();const o=document.createElement("div");o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",o.innerHTML=`
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      ...
      <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:20px;background:linear-gradient(180deg,#0a1628,#1a0a2e);padding:24px;text-align:center;color:#fff">
        ...
        ${!P&&le?'<div style="font-size:13px;color:rgba(255,255,255,0.5);animation:lobbyPulse 1.5s ease-in-out infinite">En attente de ton ami...</div>':""}
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(b),(A=b.querySelector("#pvp-end-home"))==null||A.addEventListener("click",()=>{b.remove(),We(e),l("home")})}const te=E.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},w=>{const k=w.new;try{if(k.status==="finished"||k.forfeit){if(u)return;u=!0,N&&(clearInterval(N),N=null),k.game_state&&(r=k.game_state),j(k);return}if(k.game_state){const f=r;r=k.game_state;const m=r._lastGC;if(m&&m.seq&&!F.has(m.seq)&&(F.add(m.seq),m.by!==o)){$e(m.type,m.by,()=>fe());return}const x=f[o+"Score"]||0,b=f[n+"Score"]||0,h=r[o+"Score"]||0,_=r[n+"Score"]||0,T=h>x,$=_>b;if((T||$)&&!C.has(r.round)){C.add(r.round);const A=[...r.log||[]].reverse().find(B=>B.isGoal),S=((A==null?void 0:A.homePlayers)||[]).map(B=>({name:B.name,note:B.note,portrait:B.portrait,job:B.job}));be(S,h,_,T,()=>fe());return}fe()}}catch(f){console.error("[PvP] crash:",f)}}).subscribe();async function de(w){Object.assign(r,w),r.lastActionAt=new Date().toISOString();const{error:k}=await E.from("matches").update({game_state:r}).eq("id",i);k&&p("Erreur de synchronisation","error");try{fe()}catch(f){console.error("[PvP] renderPvpScreen crash:",f)}}async function le(){if(u)return;u=!0,N&&(clearInterval(N),N=null);const w=s?y.away_id:y.home_id,k=s?"p2":"p1",f=s?"p1":"p2",m={...r,[k+"Score"]:3,[f+"Score"]:0,phase:"finished"};try{await E.from("matches").update({status:"finished",forfeit:!0,winner_id:w,home_score:m.p1Score||0,away_score:m.p2Score||0,game_state:m}).eq("id",i)}catch{}try{E.removeChannel(te)}catch{}setTimeout(()=>{We(e),l("home")},800)}const P={BOOST_STAT:({value:w=1,count:k=1,roles:f=[]},m,x)=>{const b=m[o+"Team"],h=Object.entries(b).filter(([_])=>!f.length||f.includes(_)).flatMap(([_,T])=>T.filter($=>!$.used).map($=>({...$,_line:_})));if(!h.length){m.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),x(m);return}W(h,k,`Choisir ${k} joueur(s) à booster (+${w})`,_=>{_.forEach(T=>{const $=(b[T._line]||[]).find(A=>A.cardId===T.cardId);$&&($.boost=($.boost||0)+w,m.log.push({text:`⚡ +${w} sur ${$.name}`,type:"info"}))}),m[o+"Team"]=b,x(m)})},DEBUFF_STAT:({value:w=1,count:k=1,roles:f=[],target:m="ai"},x,b)=>{const h=m==="home"?o:n,_=x[h+"Team"],T=m==="home"?"allié":"adverse",$=Object.entries(_).filter(([A])=>!f.length||f.includes(A)).flatMap(([A,S])=>S.map(B=>({...B,_line:A})));if(!$.length){x.log.push({text:`🎯 Aucun joueur ${T}`,type:"info"}),b(x);return}W($,k,`Choisir ${k} joueur(s) ${T}(s) (-${w})`,A=>{A.forEach(S=>{const B=(_[S._line]||[]).find(D=>D.cardId===S.cardId);B&&(B.boost=(B.boost||0)-w,x.log.push({text:`🎯 -${w} sur ${B.name}`,type:"info"}))}),x[h+"Team"]=_,b(x)})},GRAY_PLAYER:({count:w=1,roles:k=[],target:f="ai"},m,x)=>{const b=f==="home"?o:n,h=m[b+"Team"],_=f==="home"?"allié":"adverse",T=Object.entries(h).filter(([$])=>!k.length||k.includes($)).flatMap(([$,A])=>A.filter(S=>!S.used).map(S=>({...S,_line:$})));if(!T.length){m.log.push({text:`❌ Aucun joueur ${_}`,type:"info"}),x(m);return}W(T,w,`Choisir ${w} joueur(s) ${_}(s) à exclure`,$=>{const A="usedCardIds_"+b,S=new Set(m[A]||[]);$.forEach(B=>{const D=(h[B._line]||[]).find(R=>R.cardId===B.cardId);D&&(D.used=!0,S.add(B.cardId),m.log.push({text:`❌ ${D.name} exclu !`,type:"info"}))}),m[A]=[...S],m[b+"Team"]=h,x(m)})},REVIVE_PLAYER:({count:w=1,roles:k=[]},f,m)=>{const x=f[o+"Team"],b=Object.entries(x).filter(([h])=>!k.length||k.includes(h)).flatMap(([h,_])=>_.filter(T=>T.used).map(T=>({...T,_line:h})));if(!b.length){f.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),m(f);return}W(b,w,`Choisir ${w} joueur(s) à ressusciter`,h=>{h.forEach(_=>{const T=(x[_._line]||[]).find($=>$.cardId===_.cardId);T&&(T.used=!1,f.log.push({text:`💫 ${T.name} ressuscité !`,type:"info"}))}),f[o+"Team"]=x,m(f)})},REMOVE_GOAL:({},w,k)=>{const f=n+"Score";w[f]>0?(w[f]--,w.log.push({text:"🚫 Dernier but annulé !",type:"info"})):w.log.push({text:"🚫 Aucun but à annuler",type:"info"}),k(w)},ADD_GOAL_DRAW:({},w,k)=>{w[o+"Score"]===w[n+"Score"]?(w[o+"Score"]++,w.log.push({text:"🎯 But bonus !",type:"info"})):w.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),k(w)},ADD_SUB:({value:w=1},k,f)=>{k.maxSubs=(k.maxSubs||3)+w,k.log.push({text:`🔄 +${w} remplacement(s)`,type:"info"}),f(k)},CUSTOM:({},w,k)=>k(w)};function W(w,k,f,m){const x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let b=[];function h(){var T,$;const _=w.map(A=>{const S={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[A._line]||"#555",B=Ie(A,A._line)+(A.boost||0),R=b.find(X=>X.cardId===A.cardId)?"#FFD700":"rgba(255,255,255,0.25)",O=A.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${A.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${R};background:${S};overflow:hidden;cursor:pointer;${O}">
        ...
        </div>`,(T=x.querySelector("#pp-close"))==null||T.addEventListener("click",()=>x.remove()),x.querySelectorAll(".pp-item").forEach(A=>{A.addEventListener("click",()=>{const S=A.dataset.cid,B=w.find(R=>R.cardId===S),D=b.findIndex(R=>R.cardId===S);B&&(D>-1?b.splice(D,1):b.length<k&&b.push(B),h())})}),($=x.querySelector("#pp-confirm"))==null||$.addEventListener("click",()=>{x.remove(),m(b)})}h(),document.body.appendChild(x)}async function H(w,k){const m=(r["gcCardsFull_"+o]||[]).find(_=>_.id===w),x=(m==null?void 0:m._gcDef)||(r.gcDefs||[]).find(_=>{var T;return _.name===k||((T=_.name)==null?void 0:T.toLowerCase().trim())===(k==null?void 0:k.toLowerCase().trim())}),b=[...r["usedGc_"+o]||[],w],h={type:k,by:o,seq:(r._gcAnimSeq||0)+1};F.add(h.seq),$e(k,o,async()=>{if(x!=null&&x.effect_type&&x.effect_type!=="CUSTOM"){const T=P[x.effect_type];if(T){const $={...r};T(x.effect_params||{},$,async A=>{A["usedGc_"+o]=b,A._lastGC=h,A._gcAnimSeq=h.seq,await de(A)});return}}const _={...r};switch(k){case"Remplacement+":_.maxSubs=(_.maxSubs||3)+1,_.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const T=n+"Score";_[T]>0&&(_[T]--,_.log.push({text:"🚫 But annulé",type:"info"}));break}}_["usedGc_"+o]=b,_._lastGC=h,_._gcAnimSeq=h.seq,await de(_)})}function G(w,k){const f="usedCardIds_"+w,m=new Set(r[f]||[]);k.forEach(x=>m.add(x)),r[f]=[...m]}function ne(){for(const w of["p1","p2"]){const k=new Set(r["usedCardIds_"+w]||[]),f=r[w+"Team"];if(f)for(const m of["GK","DEF","MIL","ATT"])(f[m]||[]).forEach(x=>{x.used=k.has(x.cardId)})}}function fe(){var Ze,Qe,nt,gt,ce,_e;if(r.phase==="reveal")return ye();if(r.phase==="midfield")return Ee();if(r.phase==="finished")return q();const w=r[o+"Team"],k=r[n+"Team"];ne();const f=r[o+"Score"],m=r[n+"Score"],x=r[o+"Name"],b=r[n+"Name"],h=r.phase===o+"-attack",_=r.phase===o+"-defense",T=Array.isArray(r["selected_"+o])?r["selected_"+o]:[],$=T.map(Q=>Q.cardId),A=window.innerWidth>=700,S=r[o+"Subs"]||[],B=r["usedSubIds_"+o]||[],D=S.filter(Q=>!B.includes(Q.cardId)),R=r["gcCardsFull_"+o]||[],O=r["usedGc_"+o]||[],X=R.filter(Q=>!O.includes(Q.id)),U=r.boostOwner===o&&!r.boostUsed,ee=!["GK","DEF","MIL","ATT"].some(Q=>(k[Q]||[]).some(ue=>!ue.used)),se=[...w.MIL||[],...w.ATT||[]].filter(Q=>!Q.used),ge=h&&ee&&se.length===0?[...w.GK||[],...w.DEF||[]].filter(Q=>!Q.used).map(Q=>Q.cardId):[];function me(Q,ue,Z){var kt,Ut;const pe=Q._gcDef,qe={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[pe==null?void 0:pe.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",ke={purple:"#b06ce0",light_blue:"#00d4ef"}[pe==null?void 0:pe.color]||"#b06ce0",tt=(pe==null?void 0:pe.name)||Q.gc_type,it=(pe==null?void 0:pe.effect)||((kt=Ke[Q.gc_type])==null?void 0:kt.desc)||"",rt=pe!=null&&pe.image_url?`/manager-wars/icons/${pe.image_url}`:null,st=((Ut=Ke[Q.gc_type])==null?void 0:Ut.icon)||"⚡",ct=Math.round(Z*.22),$t=Math.round(Z*.22),pt=Z-ct-$t,ht=tt.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${Q.id}" data-gc-type="${Q.gc_type}"
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    </div>`,n==="p1"&&setTimeout(async()=>{await le({phase:"midfield"})},5e3)}let Ee=!1;function be(){if(Ee)return;const f=u[n+"Team"].MIL||[],m=u[g+"Team"].MIL||[];function x(ee){return ee.reduce((se,ge)=>se+Ie(ge,"MIL"),0)}function b(ee){let se=0;for(let ge=0;ge<ee.length-1;ge++){const me=at(ee[ge],ee[ge+1]);me==="#00ff88"?se+=2:me==="#FFD700"&&(se+=1)}return se}const h=x(f)+b(f),_=x(m)+b(m),T=h>=_;function $(ee,se,ge){return`<div id="duel-row-${ge}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
