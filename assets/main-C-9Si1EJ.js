...
import{s as w,F as di,r as Se,j as Mt,l as Hi,m as vt,h as li,n as hn,o as vn,T as Vt,k as wn,p as _n,b as kn}from"./formation-links-TDtFd05S.js";const $n="/";function Si(e,{navigate:t,toast:i}){let n="login";const a=()=>{var r,s,l,c,d,u;const o=n==="login";e.innerHTML=`
    <div style="
  ...
  `;function s(){var f;const c=document.getElementById("logo-preview"),d=document.getElementById("logo-initials"),u=((f=document.getElementById("setup-club"))==null?void 0:f.value)||"MW",g=u.trim().split(" ").filter(Boolean),m=g.length>=2?(g[0][0]+g[1][0]).toUpperCase():u.slice(0,2).toUpperCase();c&&(c.style.background=r,c.style.borderColor=o),d&&(d.textContent=m,d.style.color=o)}document.getElementById("color1").addEventListener("input",c=>{o=c.target.value,document.getElementById("swatch1").style.background=o,s()}),document.getElementById("color2").addEventListener("input",c=>{r=c.target.value,document.getElementById("swatch2").style.background=r,s()});function l(c){document.querySelectorAll(".setup-step").forEach(d=>d.classList.remove("active")),document.getElementById(`step-${c}`).classList.add("active"),document.getElementById("step-num").textContent=c,document.getElementById("progress-fill").style.width=`${Math.round(c/3*100)}%`,c===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),d=document.getElementById("step1-error");if(d.textContent="",c.length<3){d.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:u}=await w.from("users").select("id").eq("pseudo",c).maybeSingle();if(u){d.textContent="Ce pseudo est déjà pris.";return}l(2)}),document.getElementById("step2-back").addEventListener("click",()=>l(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const c=document.getElementById("setup-club").value.trim(),d=document.getElementById("step2-error");if(d.textContent="",c.length<2){d.textContent="Nom trop court (min. 2 caractères).";return}const{data:u}=await w.from("users").select("id").eq("club_name",c).maybeSingle();if(u){d.textContent="Ce nom de club est déjà pris.";return}l(3)}),document.getElementById("step3-back").addEventListener("click",()=>l(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),d=document.getElementById("setup-club").value.trim(),u=document.getElementById("step3-error"),g=document.getElementById("step3-finish");u.textContent="",g.disabled=!0,g.textContent="Création en cours…";try{const{error:m}=await w.from("users").insert({id:t.user.id,pseudo:c,club_name:d,club_color1:o,club_color2:r,credits:1e4});if(m)throw m;await Ln(t.user.id),await a(),n(`Bienvenue ${c} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(m){u.textContent=m.message,g.disabled=!1,g.textContent="🚀 Créer mon profil !"}})}async function Ln(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await w.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const Tn="modulepreload",zn=function(e){return"/"+e},Ii={},Ui=function(t,i,n){let a=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const r=document.querySelector("meta[property=csp-nonce]"),s=(r==null?void 0:r.nonce)||(r==null?void 0:r.getAttribute("nonce"));a=Promise.allSettled(i.map(l=>{if(l=zn(l),l in Ii)return;Ii[l]=!0;const c=l.endsWith(".css"),d=c?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${l}"]${d}`))return;const u=document.createElement("link");if(u.rel=c?"stylesheet":Tn,c||(u.as="script"),u.crossOrigin="",u.href=l,s&&u.setAttribute("nonce",s),document.head.appendChild(u),c)return new Promise((g,m)=>{u.addEventListener("load",g),u.addEventListener("error",()=>m(new Error(`Unable to preload CSS for ${l}`)))})}))}function o(r){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=r,window.dispatchEvent(s),!s.defaultPrevented)throw r}return a.then(r=>{for(const s of r||[])s.status==="rejected"&&o(s.reason);return t().catch(o)})},Tt="#1A6B3C",zt="#cc2222",Sn="#D4A017",Mi="#888";async function In(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    </div>`}function An(e,t){const i=ci();i.innerHTML=`
    <div class="popup-box">
    ...
    ${pi()}`,document.body.appendChild(h),h.querySelector("#stats-close").addEventListener("click",()=>h.remove()),h.addEventListener("click",E=>{E.target===h&&h.remove()})}function ci(){const e=document.createElement("div");return e.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:8000;display:flex;align-items:center;justify-content:center;padding:20px",e}function pi(){return`
    <style>
      .popup-box {
    ...
    </style>`}const jn="2026.07.16-0047";async function Ai(e,{state:t,navigate:i,toast:n}){var o,r;const a=t.profile;a&&(e.innerHTML=`
  <style>
    .home-dark {
      min-height: 100%;
  ...
  <div class="home-dark" id="home-dark">
    ...
    ${Se(e,{width:140})}
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),c=(s||[]).filter(z=>z.card_type==="player"&&z.player),d=(s||[]).filter(z=>z.card_type==="game_changer"),u=(s||[]).filter(z=>z.card_type==="formation"),g=(s||[]).filter(z=>z.card_type==="stadium"),{data:m}=await w.from("gc_definitions").select("name,gc_type,color,effect,image_url"),f={};(m||[]).forEach(z=>{f[z.name]=z});const{data:p}=await w.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),x={};(p||[]).forEach(z=>{x[z.id]=z}),g.forEach(z=>{z.stadium_def&&(x[z.stadium_id]=z.stadium_def)});const b=Object.keys(di),v=Object.keys(Me),h={};c.forEach(z=>{const A=z.player.id;h[A]=(h[A]||0)+1}),new Set(Object.keys(h).map(z=>String(z)));const E=new Set(u.map(z=>z.formation)),y=new Set(d.map(z=>z.gc_type));let B="player",$="Tous",G="",N=!1;function te(){return[...c].sort((z,A)=>{const F=Ct.indexOf(z.player.job),V=Ct.indexOf(A.player.job);return F!==V?F-V:(z.player.surname_real||"").localeCompare(A.player.surname_real||"")})}function ie(){return[...l||[]].sort((z,A)=>{const F=Ct.indexOf(z.job),V=Ct.indexOf(A.job);return F!==V?F-V:(z.surname_real||"").localeCompare(A.surname_real||"")})}function Z(){return te().filter(z=>{const A=z.player,F=$==="Tous"||A.job===$,V=!G||`${A.firstname} ${A.surname_real}`.toLowerCase().includes(G);return F&&V})}function C(){return ie().filter(z=>{const A=$==="Tous"||z.job===$,F=!G||`${z.firstname} ${z.surname_real}`.toLowerCase().includes(G);return A&&F})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
  ...
  </div>`;function _(){const z=document.getElementById("col-filters");z&&(B==="player"?(z.innerHTML=`
          ...
          ${On.map(A=>`
            <button class="filter-btn" data-job="${A}"
              style="flex-shrink:0;padding:4px 12px;border-radius:20px;font-size:12px;font-weight:600;cursor:pointer;
                border:1.5px solid ${A===$?"var(--green)":"var(--gray-200)"};
                background:${A===$?"var(--green)":"#fff"};
                color:${A===$?"#fff":"var(--gray-600)"}">
              ${A}
            </button>`).join("")}
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",A=>{G=A.target.value.toLowerCase(),I()}),e.querySelectorAll(".filter-btn").forEach(A=>{A.addEventListener("click",()=>{$=A.dataset.job,_(),I()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{N=!N,_(),I()})):(z.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{N=!N,_(),I()})))}function I(){const z=document.getElementById("col-grid");z&&(B==="player"?Te(z):B==="formation"?K(z):B==="stadium"?R(z):Q(z))}function H(z,A,F,V,U){const j=document.getElementById("col-grid"),O=document.getElementById("col-big");if(!j||!O)return;var _e=0;function ke(){const ve=window.innerWidth>=768,me=document.getElementById("col-big"),$e=document.getElementById("col-grid");ve&&me&&(me.style.minHeight="420px",me.style.flex="1 1 auto"),ve&&$e&&($e.style.height=Math.round(310*.76+16)+"px",$e.style.flexShrink="0",$e.style.overflowX="auto",$e.style.overflowY="hidden",$e.style.alignItems="center",$e.style.padding="8px 16px"),O.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+A(z[_e])+"</div>";var Ee=O.querySelector("[data-card-id],[data-form-id],[data-gc-id]");Ee&&Ee.addEventListener("click",function(){V(z[_e])}),requestAnimationFrame(function(){var be=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!be||!O)){var Ce=O.clientHeight-8,qe=O.clientWidth-24,Ke=be.offsetHeight,k=be.offsetWidth;if(Ke>0&&k>0&&Ce>40){var L=ve?2.2:1.6,S=Math.min(Ce/Ke,qe/k,L);be.style.transform="scale("+S.toFixed(3)+")",be.style.transformOrigin="top center"}}}),j.innerHTML=z.map(function(be,Ce){var qe=Ce===_e,Ke="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+(qe?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+Ce+'" style="'+Ke+'">'+F(be,qe)+"</div>"}).join(""),j.querySelectorAll(".col-mini-item").forEach(function(be){be.addEventListener("click",function(){_e=Number(be.dataset.idx),ke(),be.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}ke()}function le(z){var A=window.innerWidth>=768?.76:.54,F;if(!z||z._fake){var V=z?z.player:null;if(!V)return"";F=Bi(V)}else F=Xt(z,"");return'<div style="display:inline-block;zoom:'+A+';pointer-events:none;line-height:0">'+F+"</div>"}function se(z,A,F){A=A||100,F=F||140;var V=Mt[z]||{},U={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},j=Math.max(3,Math.round(A*.06)),O=Object.entries(V).map(function(ke){var ve=ke[0],me=ke[1],$e=ve.replace(/\d+$/,""),Ee=U[$e]||"#888",be=Math.round(me.x*A),Ce=Math.round(me.y*F);return'<circle cx="'+be+'" cy="'+Ce+'" r="'+j+'" fill="'+Ee+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),_e=Math.max(1,Math.round(A/50));return'<svg viewBox="0 0 '+A+" "+F+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+A+'" height="'+F+'" fill="#1A6B3C"/><rect x="'+Math.round(A*.2)+'" y="'+Math.round(F*.02)+'" width="'+Math.round(A*.6)+'" height="'+Math.round(F*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+_e+'"/><line x1="0" y1="'+Math.round(F*.5)+'" x2="'+A+'" y2="'+Math.round(F*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+_e+'"/><ellipse cx="'+Math.round(A*.5)+'" cy="'+Math.round(F*.5)+'" rx="'+Math.round(A*.18)+'" ry="'+Math.round(F*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+_e+'"/><rect x="'+Math.round(A*.2)+'" y="'+Math.round(F*.82)+'" width="'+Math.round(A*.6)+'" height="'+Math.round(F*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+_e+'"/>'+O+"</svg>"}function pe(z,A,F){var V=F>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+F+"</div>":"",U=A?'data-form-id="'+A.id+'"':"",j=z.length>7?14:z.length>5?16:19,O=!!A;return"<div "+U+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(O?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(O?"":"filter:grayscale(1);opacity:0.5")+'">'+V+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(O?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+j+"px;font-weight:900;color:"+(O?"#1A6B3C":"#aaa")+';line-height:1">'+z+'</div></div><div style="flex:1;overflow:hidden;background:'+(O?"#1A6B3C":"#ccc")+'">'+se(z,140,220)+"</div></div>"}function de(z,A){var F=window.innerWidth>=768?.76:.54,V=140,U=305,j=Math.round(U*.22),O=U-j,_e=z.length>7?10:13,ke=se(z,V,O),ve=A?"1.5px solid #2a7a40":"1px solid #ddd",me=A?"":"filter:grayscale(1);opacity:0.45;",$e=A?"#1A6B3C":"#bbb",Ee="#fff";return'<div style="display:inline-block;zoom:'+F+';line-height:0;pointer-events:none"><div style="width:'+V+"px;height:"+U+"px;border-radius:6px;border:"+ve+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+me+'"><div style="height:'+j+"px;background:"+$e+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+_e+"px;font-weight:900;color:"+Ee+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(V-4)+'px">'+z+'</span></div><div style="height:'+O+'px;overflow:hidden;flex-shrink:0">'+ke+"</div></div></div>"}function Te(z){if(N){const A=C();if(!A.length){z.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const F=A.map(V=>c.find(U=>U.player.id===V.id)||{_fake:!0,player:V,id:"fake-"+V.id});H(F,V=>V._fake?Bi(V.player):Xt(V,""),V=>V._fake?le({player:V.player,id:"x",_fake:!0}):le(V),V=>{V._fake||qi(V,c,h,t)})}else{const A=Z();if(!A.length){z.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const F={};A.forEach(U=>{const j=U.player.id;F[j]||(F[j]=[]),F[j].push(U)});const V=Object.values(F).map(U=>Un(U));H(V,U=>{const j=h[U.player.id]||1,O=j>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${j}</div>`:"",ke=c.filter(ve=>ve.player.id===U.player.id&&ve.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Xt(U,O+ke)},U=>le(U),U=>qi(U,c,h,t))}}function K(z){const A=N?b:[...E];if(!A.length){z.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const F=A.map(V=>({formation:V,card:u.find(U=>U.formation===V)||null,owned:E.has(V)}));H(F,({formation:V,card:U,owned:j})=>pe(V,j?U:null,j?u.filter(O=>O.formation===V).length:0),({formation:V,owned:U})=>de(V,U),({card:V,owned:U})=>{U&&V&&Jn(V,u,t,o)})}function Q(z){const A=Object.keys(f),F=N?A.length?A:v:[...y];if(!F.length){z.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const V=F.map(U=>({type:U,gc:Me[U]||{icon:"⚡",desc:""},def:f[U]||null,owned:y.has(U),card:d.find(j=>j.gc_type===U)||null}));H(V,({type:U,gc:j,def:O,owned:_e,card:ke})=>{var L;const ve=_e?d.filter(S=>S.gc_type===U).length:0,me=ve>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${ve}</div>`:"",$e=(O==null?void 0:O.gc_type)==="ultra_game_changer",Ee={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},be={purple:"#b06ce0",light_blue:"#00d4ef"},Ce=Ee[O==null?void 0:O.color]||Ee.purple,qe=be[O==null?void 0:O.color]||be.purple,Ke=(O==null?void 0:O.effect)||j.desc||"",k=O!=null&&O.image_url?`/icons/${O.image_url}`:((L=O==null?void 0:O.club)==null?void 0:L.logo_url)||(O!=null&&O.country_code?`https://flagsapi.com/${O.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null);return _e&&ke?`<div data-gc-id="${ke.id}" data-gc-type="${U}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${qe};background:${Ce};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${qe}66;cursor:pointer">
          ${me}
        ...
        </div>`},null)}e.querySelectorAll(".col-tab-btn").forEach(z=>{z.addEventListener("click",()=>{B=z.dataset.tab,$="Tous",G="",N=!1,e.querySelectorAll(".col-tab-btn").forEach(A=>{const F=A.dataset.tab===B;A.style.borderBottomColor=F?"var(--green)":"transparent",A.style.color=F?"var(--green)":"var(--gray-600)"}),_(),I()})}),_(),I()}function Xn(e,t,i){const n=Me[e]||{icon:"⚡",desc:"Effet spécial."},a=(t==null?void 0:t.gc_type)==="ultra_game_changer",o={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},r={purple:"#b06ce0",light_blue:"#00d4ef"},s=o[t==null?void 0:t.color]||o.purple,l=r[t==null?void 0:t.color]||r.purple,c=(t==null?void 0:t.name)||e,d=(t==null?void 0:t.effect)||n.desc,u=t!=null&&t.image_url?`/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const jt=1e3;function Jn(e,t,i,n){var f,p,x;const{state:a,toast:o,closeModal:r,navigate:s,refreshProfile:l}=i,c=e.formation,d={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function u(){const b=Mt[c]||{},v=di[c]||[],h=290,E=360,y=20;function B(G){const N=b[G];return N?{x:N.x*h,y:N.y*E}:null}let $=`<svg width="${h}" height="${E}" viewBox="0 0 ${h} ${E}" xmlns="http://www.w3.org/2000/svg">`;for(const[G,N]of v){const te=B(G),ie=B(N);!te||!ie||($+=`<line x1="${te.x}" y1="${te.y}" x2="${ie.x}" y2="${ie.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const G of Object.keys(b)){const N=B(G);if(!N)continue;const te=G.replace(/\d+/,""),ie=d[te]||"#555";$+=`<circle cx="${N.x}" cy="${N.y}" r="${y}" fill="${ie}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,$+=`<text x="${N.x}" y="${N.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${te}</text>`}return $+="</svg>",$}const g=t.filter(b=>b.formation===c);g.length;const m=!e.is_for_sale;n(`Formation ${c}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(f=document.getElementById("direct-sell-form-btn"))==null||f.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${c} pour ${jt.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const b=g.find(h=>!h.is_for_sale)||g[0];if(!b){o("Aucune carte à vendre","error");return}await w.from("market_listings").delete().eq("card_id",b.id),await w.from("transfer_history").delete().eq("card_id",b.id);const{error:v}=await w.from("cards").delete().eq("id",b.id);if(v){o(v.message,"error");return}await w.from("users").update({credits:(a.profile.credits||0)+jt}).eq("id",a.profile.id),await l(),o(`+${jt.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),r(),s("collection")}),(p=document.getElementById("market-sell-form-btn"))==null||p.addEventListener("click",async()=>{const b=parseInt(document.getElementById("sell-price-form").value);if(!b||b<1){o("Prix invalide","error");return}await w.from("cards").update({is_for_sale:!0,sale_price:b}).eq("id",e.id),await w.from("market_listings").insert({seller_id:a.profile.id,card_id:e.id,price:b}),o("Carte mise en vente sur le marché !","success"),r(),s("collection")}),(x=document.getElementById("cancel-sell-form-btn"))==null||x.addEventListener("click",async()=>{await w.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await w.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),o("Annonce retirée","success"),r(),s("collection")})}async function qi(e,t,i,n){var le,se,pe,de,Te;const{state:a,toast:o,openModal:r,closeModal:s,navigate:l,refreshProfile:c}=n,d=e.player,u=t.filter(K=>K.player.id===d.id),g=u.length,m=e.evolution_bonus||0,f=Math.max((Number(d.note_g)||0)+(d.job==="GK"||d.job2==="GK"?m:0),(Number(d.note_d)||0)+(d.job==="DEF"||d.job2==="DEF"?m:0),(Number(d.note_m)||0)+(d.job==="MIL"||d.job2==="MIL"?m:0),(Number(d.note_a)||0)+(d.job==="ATT"||d.job2==="ATT"?m:0)),p=d.rarity||"normal",{data:x}=await w.from("sell_price_configs").select("*").eq("rarity",p).lte("note_min",f).gte("note_max",f).order("note_min",{ascending:!1}).limit(1);((le=x==null?void 0:x[0])==null?void 0:le.price)??Hn[p];const b=d.rarity!=="legende";Yn(d);const v=((d.rarity==="pepite"||d.rarity==="papyte")&&e.current_note!=null?e.current_note:Wt(d,d.job))+m,h=d.rarity==="pepite"||d.rarity==="papyte",E=d.job2?(h?Pn(e,Kn(d.job2)):Wt(d,d.job2))+(Wt(d,d.job2)>0?m:0):null;Yt[d.job],d.job2&&Yt[d.job2];const y=Nn[d.rarity]||"#ccc";Vn[d.country_code]||d.country_code;const B=e.evolution_bonus||0,G=v+B,N=E||0,te=N>0?N+B:0,ie=u.map(K=>K.id);let Z={};if(ie.length){const{data:K}=await w.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",ie).order("transferred_at",{ascending:!0});(K||[]).forEach(Q=>{Z[Q.card_id]||(Z[Q.card_id]=[]),Z[Q.card_id].push(Q)})}const C=K=>{const Q=K.transferred_at?new Date(K.transferred_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"2-digit",year:"numeric"}):"",R=K.source==="booster"?"Booster":K.price?K.price.toLocaleString("fr"):"—";return`
    ...
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${u.map((K,Q)=>{const R=Z[K.id]||[],z=K.is_for_sale,A=R.length?R[R.length-1]:null,V=(d.rarity==="pepite"||d.rarity==="papyte")&&K.current_note!=null?` (☆${K.current_note})`:"";return`
            <div data-card-id="${K.id}" data-card-idx="${Q}" class="exemplaire-row"
              ...
              <div style="display:flex;align-items:center;gap:10px">
                <input type="checkbox" class="expl-check" data-id="${K.id}" ${z?"disabled":""} style="width:18px;height:18px;cursor:pointer;accent-color:#1A6B3C;flex-shrink:0">
                <div style="flex:1;min-width:0">
                  <div style="font-size:12px;font-weight:700">Exemplaire ${Q+1}${V}${z?" 🏷️ En vente":""}</div>
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",d.note_g],["DEF",d.note_d],["MIL",d.note_m],["ATT",d.note_a]].map(([K,Q])=>{const R=Yt[K],z=K==="GK"?"#fff":R,A=K===d.job||K===d.job2,F=(Number(Q)||0)+(A&&B>0?B:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
                ...
                <span style="font-size:10px;font-weight:700;color:${z}">${K}</span>
              </div>`}).join("")}
          </div>
        </div>
        <div>
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(se=document.getElementById("close-detail"))==null||se.addEventListener("click",s);let I=new Set;const H=()=>{const K=I.size,Q=document.getElementById("sell-action-panel");if(!Q)return;Q.style.display=K>0?"block":"none",document.getElementById("sell-selected-count").textContent=K;const R=document.getElementById("evolve-btn");R&&(R.textContent=`⬆️ Évoluer ${K>1?"(+"+K+")":""}`)};document.querySelectorAll(".expl-check").forEach(K=>{K.addEventListener("change",()=>{const Q=K.dataset.id;K.checked?I.add(Q):I.delete(Q);const R=K.closest(".exemplaire-row");R&&(R.style.borderColor=K.checked?"#1A6B3C":"#eee"),H()})}),document.querySelectorAll(".exemplaire-row").forEach(K=>{K.addEventListener("click",Q=>{if(Q.target.closest("button")||Q.target.tagName==="INPUT")return;const R=K.querySelector(".expl-check");R&&!R.disabled&&(R.checked=!R.checked,R.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(K=>{K.addEventListener("click",Q=>{Q.stopPropagation();const R=document.querySelector(`.expl-hist[data-hist="${K.dataset.idx}"]`);R&&(R.style.display=R.style.display==="none"?"flex":"none")})}),(pe=document.getElementById("evolve-btn"))==null||pe.addEventListener("click",async()=>{if(g<=1)return;const K=[...I];if(!K.length)return;if(I.has(e.id)){const U=document.createElement("div");U.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",U.innerHTML=`
        ...
        <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center;box-shadow:0 16px 48px rgba(0,0,0,0.3)">
          ...
          <div style="background:#f0fdf4;border-radius:10px;padding:12px;margin-bottom:16px;font-size:13px;color:#333">
            ...
            📈 Note : <strong>${v+(e.evolution_bonus||0)}</strong> → <strong style="color:#1A6B3C">${v+(e.evolution_bonus||0)+Q.length*R}</strong>
            ${E&&E>0?`<br>📈 Note 2 : <strong>${E+(e.evolution_bonus||0)}</strong> → <strong style="color:#1A6B3C">${E+(e.evolution_bonus||0)+Q.length*R}</strong>`:""}
        ...
        </div>`,document.body.appendChild(j),j.querySelector("#sac-cancel").addEventListener("click",()=>{j.remove(),U(!1)}),j.querySelector("#sac-ok").addEventListener("click",()=>{j.remove(),U(!0)}),j.addEventListener("click",O=>{O.target===j&&(j.remove(),U(!1))})}))return;if(Q.length){await w.from("market_listings").delete().in("card_id",Q),await w.from("deck_cards").delete().in("card_id",Q),await w.from("transfer_history").delete().in("card_id",Q),await w.from("decks").update({stadium_card_id:null}).in("stadium_card_id",Q);const{error:U}=await w.from("cards").delete().in("id",Q);if(U){o("Erreur suppression : "+U.message,"error");return}}const A=(e.evolution_bonus||0)+Q.length*R,{error:F}=await w.from("cards").update({evolution_bonus:A}).eq("id",e.id);if(F){o("Erreur évolution : "+F.message,"error");return}const V=v+A;o(`⬆️ ${d.firstname} ${d.surname_real} : note ${v+e.evolution_bonus||v} → ${V}${Q.length?` · ${Q.length} copie${Q.length>1?"s":""} sacrifiée${Q.length>1?"s":""}`:""} !`,"success",4e3),s(),l("collection")}),(de=document.getElementById("market-sell-btn"))==null||de.addEventListener("click",async()=>{var F;const K=[...I];if(!K.length){o("Sélectionne au moins un exemplaire","warning");return}const Q=parseInt((F=document.getElementById("sell-market-price"))==null?void 0:F.value);if(!Q||Q<1){o("Prix invalide","error");return}const{error:R}=await w.from("cards").update({is_for_sale:!0,sale_price:Q}).in("id",K);if(R){o(R.message,"error");return}const z=K.map(V=>({seller_id:a.profile.id,card_id:V,price:Q,status:"active"})),{error:A}=await w.from("market_listings").insert(z);A&&console.warn("[Market] insert listings:",A.message),o(`${K.length} carte${K.length>1?"s":""} mise${K.length>1?"s":""} en vente à ${Q.toLocaleString("fr")} cr. chacune !`,"success"),s(),l("collection")}),(Te=document.getElementById("cancel-sell-btn"))==null||Te.addEventListener("click",async()=>{await w.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await w.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),o("Annonce retirée","success"),s(),l("collection")})}const Dt={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}};async function Zt(e,t){const{state:i,navigate:n,toast:a}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await w.from("decks").select("id,name,formation_card_id").eq("owner_id",i.profile.id).order("created_at",{ascending:!1});e.innerHTML=`
  ...
  </div>`,document.getElementById("new-deck-btn").addEventListener("click",async()=>{const r=prompt("Nom du deck :",`Deck ${((o==null?void 0:o.length)||0)+1}`);if(!r)return;const{data:s,error:l}=await w.from("decks").insert({owner_id:i.profile.id,name:r}).select().single();if(l){a(l.message,"error");return}a("Deck créé !","success"),Di(s.id,e,t)}),e.querySelectorAll("[data-open-deck]").forEach(r=>{r.addEventListener("click",()=>Di(r.dataset.openDeck,e,t))}),e.querySelectorAll("[data-rename]").forEach(r=>{r.addEventListener("click",async()=>{const s=prompt("Nouveau nom :",r.dataset.name);if(!s||s===r.dataset.name)return;const{error:l}=await w.from("decks").update({name:s}).eq("id",r.dataset.rename);if(l){a(l.message,"error");return}a("Deck renommé !","success"),Zt(e,t)})}),e.querySelectorAll("[data-delete]").forEach(r=>{r.addEventListener("click",async()=>{if(!confirm(`Supprimer le deck "${r.dataset.name}" ? Cette action est irréversible.`))return;await w.from("deck_cards").delete().eq("deck_id",r.dataset.delete);const{error:s}=await w.from("decks").delete().eq("id",r.dataset.delete);if(s){a(s.message,"error");return}a("Deck supprimé.","success"),Zt(e,t)})})}async function Di(e,t,i){const{state:n,toast:a}=i;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await w.from("decks").select("*").eq("id",e).single(),{data:r}=await w.from("cards").select(`id, card_type, formation, stadium_id, evolution_bonus,
      player:players(id, firstname, surname_real, country_code, club_id, job, job2,
  ...
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
    ...
    </div>`;x("⚽ Choisir une formation",v),document.querySelectorAll("#modal-body [data-forma]").forEach(h=>{h.addEventListener("click",()=>{t.formation=h.dataset.forma;const E=Fi(t.formation),y={};E.forEach(B=>{t.slots[B]&&(y[B]=t.slots[B])}),t.slots=y,b(),Ze(e,t,i)})})};e.querySelectorAll("#formation-mobile-btn, #formation-pc-btn").forEach(x=>x.addEventListener("click",m)),e.querySelectorAll("#add-stad-btn-pc, #add-stad-btn").forEach(x=>x.addEventListener("click",()=>Zn(t,e,i))),e.querySelectorAll("#save-deck").forEach(x=>x.addEventListener("click",()=>io(t,i))),e.querySelectorAll("[data-remove-sub]").forEach(x=>{x.addEventListener("click",()=>{t.subs=t.subs.filter(b=>b!==x.dataset.removeSub),Ze(e,t,i)})}),e.querySelectorAll("#add-sub-btn").forEach(x=>x.addEventListener("click",()=>to(t,e,i)))}function Qn(e,t,i,n){var h,E;const a=window.innerWidth>=900,o=e.querySelector(a?"#deck-field-pc":"#deck-field-mobile");if(!o)return;const r=(h=t.stadiumCards)==null?void 0:h.find(y=>y.id===t.stadiumCardId),s=r&&((E=t.stadDefMap)==null?void 0:E[r.stadium_id])||null,l=Mt[t.formation]||{},c=Hi(t.formation)||[],d={};for(const y of i){const B=t.slots[y],$=B?t.playerCards.find(G=>G.id===B):null;$!=null&&$.player?d[y]={...$.player,_evolution_bonus:$.evolution_bonus||0,face:$.player.face||null}:d[y]=null}const u=window.innerWidth>=900,g=u?window.innerWidth-280:window.innerWidth-20,m=u?Math.min(g,860):g,f=Math.round(u?m*.82:m*.85),p=u?84:44;let x="";for(const[y,B]of c){const $=l[y],G=l[B];if(!$||!G)continue;const N=$.x*m,te=Math.round(.03*f+$.y*.85*f),ie=G.x*m,Z=Math.round(.03*f+G.y*.85*f),C=d[y],_=d[B],I=vt(C,_);I==="#ff3333"||I==="#cc2222"?x+=`<line x1="${N.toFixed(1)}" y1="${te.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${Z.toFixed(1)}" stroke="${I}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(x+=`<line x1="${N.toFixed(1)}" y1="${te.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${Z.toFixed(1)}" stroke="${I}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,x+=`<line x1="${N.toFixed(1)}" y1="${te.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${Z.toFixed(1)}" stroke="${I}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let b="";const v=Math.round(p*657/507);for(const y of i){const B=l[y];if(!B)continue;const $=d[y],G=B.x*m;B.y*f;const N=Math.round(.03*f+B.y*(.85*f)),te=Math.round(G-p/2),ie=Math.round(N-v/2);if($){const Z=y.replace(/\d+/,""),C=s&&(s.club_id&&String($.club_id)===String(s.club_id)||s.country_code&&$.country_code===s.country_code),_=Se({...$,_evolution_bonus:$._evolution_bonus||0},{width:p,showStad:!0,stadDef:s,role:Z});b+=`<div style="position:absolute;left:${te}px;top:${ie}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${y}">
        <div style="position:relative">${C?'<div style="position:absolute;top:-18px;left:0;right:0;text-align:center;font-size:14px;z-index:5;line-height:1">🏟️</div>':""}${_}</div>
      </div>`}else{const Z=y.replace(/\d+/,"");b+=`<div style="position:absolute;left:${te}px;top:${ie}px;width:${p}px;height:${v}px;
    ...
    </div>`),(r=document.getElementById("stad-none"))==null||r.addEventListener("click",()=>{e.stadiumCardId=null,a(),Ze(t,e,i)}),document.querySelectorAll(".stad-choice").forEach(s=>{s.addEventListener("click",()=>{e.stadiumCardId=s.dataset.stadId,a(),Ze(t,e,i)})})}function eo(e,t,i,n){var f,p,x,b,v;const{openModal:a,closeModal:o}=n,r=e.replace(/\d+/,""),s=(f=t.stadiumCards)==null?void 0:f.find(h=>h.id===t.stadiumCardId),l=s&&((p=t.stadDefMap)==null?void 0:p[s.stadium_id])||null,c=t.slots[e],d=c?t.playerCards.find(h=>h.id===c):null;(x=d==null?void 0:d.player)==null||x.id;const u=new Set;Object.entries(t.slots).forEach(([h,E])=>{var B;if(h===e||!E)return;const y=t.playerCards.find($=>$.id===E);(B=y==null?void 0:y.player)!=null&&B.id&&u.add(y.player.id)}),t.subs.forEach(h=>{var y;const E=t.playerCards.find(B=>B.id===h);(y=E==null?void 0:E.player)!=null&&y.id&&u.add(E.player.id)});const g=new Set,m=t.playerCards.filter(h=>{const E=h.player;return!(E.job===r||E.job2===r)||u.has(E.id)||g.has(E.id)?!1:(g.add(E.id),!0)});m.sort((h,E)=>{const y=h.evolution_bonus||0,B=E.evolution_bonus||0,$=(r==="GK"?h.player.note_g:r==="DEF"?h.player.note_d:r==="MIL"?h.player.note_m:h.player.note_a)+(r===h.player.job||r===h.player.job2?y:0);return(r==="GK"?E.player.note_g:r==="DEF"?E.player.note_d:r==="MIL"?E.player.note_m:E.player.note_a)+(r===E.player.job||r===E.player.job2?B:0)-$}),a(`Choisir ${r} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${m.length>0?'<div style="display:flex;flex-wrap:wrap;gap:8px;justify-content:center">'+m.map(h=>{const E={...h.player,_evolution_bonus:h.evolution_bonus||0};return`<div class="player-option" data-card-id="${h.id}" style="cursor:pointer">
          ${Se(E,{width:100,showStad:!0,stadDef:l,role:r})}
  ...
  `,document.body.appendChild(a);let o=!1;const r=document.getElementById("pack-cut-zone"),s=document.getElementById("pack-blade");let l=!1;const c=_=>_.touches&&_.touches[0]?_.touches[0].clientX:_.clientX;function d(_){o||(l=!0,s.style.opacity="1",u(_))}function u(_){if(!l||o)return;const I=r.getBoundingClientRect(),H=c(_)-I.left,le=Math.max(0,Math.min(1,H/I.width));s.style.width=le*I.width+"px",le>=.82&&m()}function g(){o||(l=!1,s.style.transition="width .2s ease, opacity .2s ease",s.style.width="0",s.style.opacity="0",setTimeout(()=>{o||(s.style.transition="")},220))}function m(){var I;if(o)return;o=!0,l=!1,s.style.width="100%",s.style.opacity="1",(I=document.getElementById("cut-flash"))==null||I.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const _=document.getElementById("cut-hint");_&&(_.style.opacity="0"),r.classList.add("pack-cut"),setTimeout(()=>{s.style.opacity="0",document.getElementById("pack-phase").style.display="none",b(0)},620)}r.addEventListener("pointerdown",d),window.addEventListener("pointermove",u),window.addEventListener("pointerup",g),r.addEventListener("touchstart",d,{passive:!0}),window.addEventListener("touchmove",u,{passive:!0}),window.addEventListener("touchend",g);let f=0,p=!1;const x=new Set;function b(_){f=_,document.getElementById("reveal-phase").style.display="flex",v(),h(_,0),G()}function v(){const _=document.getElementById("card-dots");_&&(_.innerHTML=e.map((I,H)=>`<div class="card-dot" data-i="${H}" style="width:8px;height:8px;border-radius:50%;background:${H===f?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),_.querySelectorAll(".card-dot").forEach(I=>I.addEventListener("click",()=>y(parseInt(I.dataset.i)))))}function h(_,I){var R,z;const H=e[_],le=document.getElementById("card-counter"),se=document.getElementById("card-track");le&&(le.textContent=`Carte ${_+1} / ${e.length}`);const pe=document.getElementById("reveal-btns");pe&&(pe.style.display=_===e.length-1?"flex":"none");const de=H.card_type==="player"&&((R=H.player)==null?void 0:R.rarity)==="legende",Te=!x.has(_);x.add(_);let K=0;if(H.card_type==="player"&&H.player){const A=H.player,F=A.job||"ATT";K=Number(F==="GK"?A.note_g:F==="DEF"?A.note_d:F==="MIL"?A.note_m:A.note_a)||0}const Q=A=>{se.innerHTML=`
        ...
        ${Q}
      </div>`,z&&navigator.vibrate&&navigator.vibrate([50,30,100,30,200])},(j+O)*3),setTimeout(U,(j+O)*3+j),setTimeout(()=>{A.style.display="none",F.innerHTML="",p=!1,V&&!R&&V.pause(),navigator.vibrate&&navigator.vibrate([40,30,80]),I()},(j+O)*4)}function y(_){if(p||_<0||_>=e.length||_===f)return;const I=_>f?1:-1;f=_,h(_,I)}function B(){y(f+1)}function $(){y(f-1)}function G(){const _=document.getElementById("card-viewport");if(!_||_._swipeBound)return;_._swipeBound=!0;let I=0,H=0,le=0,se=!1;const pe=R=>R.touches?R.touches[0].clientX:R.clientX,de=R=>R.touches?R.touches[0].clientY:R.clientY,Te=R=>{se=!0,I=pe(R),H=de(R),le=0},K=R=>{if(!se)return;le=pe(R)-I;const z=de(R)-H;if(Math.abs(le)<Math.abs(z))return;const A=document.getElementById("current-card-wrap");A&&(A.style.transition="none",A.style.transform=`translateX(${le*.6}px) rotate(${le*.02}deg)`)},Q=()=>{if(!se)return;se=!1;const R=document.getElementById("current-card-wrap"),z=55;le<=-z&&f<e.length-1?B():le>=z&&f>0?$():R&&(R.style.transition="transform .2s ease",R.style.transform="translateX(0)")};_.addEventListener("pointerdown",Te),_.addEventListener("pointermove",K),_.addEventListener("pointerup",Q),_.addEventListener("pointercancel",Q),_.addEventListener("touchstart",Te,{passive:!0}),_.addEventListener("touchmove",K,{passive:!0}),_.addEventListener("touchend",Q),_.addEventListener("click",R=>{if(Math.abs(le)>8)return;const z=_.getBoundingClientRect();R.clientX-z.left>z.width/2?B():$()})}let N=null;function te(){const _=document.getElementById("fireworks-canvas");if(!_)return;_.width=window.innerWidth,_.height=window.innerHeight;const I=_.getContext("2d"),H=[];function le(){const pe=Math.random()*_.width,de=Math.random()*_.height*.6,Te=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],K=Te[Math.floor(Math.random()*Te.length)];for(let Q=0;Q<60;Q++){const R=Math.PI*2/60*Q,z=2+Math.random()*5;H.push({x:pe,y:de,vx:Math.cos(R)*z,vy:Math.sin(R)*z,alpha:1,color:K,size:2+Math.random()*3})}}le(),N=setInterval(le,600);function se(){if(document.getElementById("fireworks-canvas")){I.clearRect(0,0,_.width,_.height);for(let pe=H.length-1;pe>=0;pe--){const de=H[pe];if(de.x+=de.vx,de.y+=de.vy+.08,de.vy*=.98,de.alpha-=.018,de.alpha<=0){H.splice(pe,1);continue}I.globalAlpha=de.alpha,I.fillStyle=de.color,I.beginPath(),I.arc(de.x,de.y,de.size,0,Math.PI*2),I.fill()}I.globalAlpha=1,(N!==null||H.length>0)&&requestAnimationFrame(se)}}se()}function ie(){N!==null&&(clearInterval(N),N=null);const _=document.getElementById("fireworks-canvas");_&&_.getContext("2d").clearRect(0,0,_.width,_.height)}if(n){const _=document.getElementById("reveal-btns");_&&(_.innerHTML='<button class="btn btn-primary" id="reveal-next" style="flex:1">Continuer →</button>'),(C=document.getElementById("reveal-next"))==null||C.addEventListener("click",()=>{ie(),a.remove(),n()})}else document.getElementById("reveal-collection").addEventListener("click",()=>{ie(),a.remove(),i("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{ie(),a.remove(),i("boosters")})}function uo(e){var t,i,n,a;if(e.card_type==="player"&&e.player)return ao(e);if(e.card_type==="game_changer"){const o=e._gcDef,r=(o==null?void 0:o.gc_type)==="ultra_game_changer",s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},l={purple:"#b06ce0",light_blue:"#00d4ef"},c=s[o==null?void 0:o.color]||s.purple,d=l[o==null?void 0:o.color]||l.purple,u=(o==null?void 0:o.name)||e.gc_type||"Game Changer",g=(o==null?void 0:o.effect)||((t=ei[e.gc_type])==null?void 0:t.desc)||"",m=o!=null&&o.image_url?`/icons/${o.image_url}`:null,f=((i=ei[e.gc_type])==null?void 0:i.icon)||"⚡";return`<div style="width:170px;height:240px;background:${c};border-radius:14px;border:3px solid ${d};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${d}88;flex-shrink:0">
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      card:cards(id,card_type,formation,stadium_id,evolution_bonus,
        player:players(id,firstname,surname_real,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
          clubs(encoded_name,logo_url)))`).in("deck_id",r).order("slot_order"),l=[...new Set((s||[]).filter(g=>{var m,f;return((m=g.card)==null?void 0:m.card_type)==="stadium"&&((f=g.card)==null?void 0:f.stadium_id)}).map(g=>g.card.stadium_id))],c={};if(l.length){const{data:g}=await w.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",l);(g||[]).forEach(m=>{c[m.id]=m}),(s||[]).forEach(m=>{var f,p;((f=m.card)==null?void 0:f.card_type)==="stadium"&&((p=m.card)!=null&&p.stadium_id)&&(m.card._stadiumDef=c[m.card.stadium_id]||null)})}let d=0;function u(){var y,B,$,G,N,te,ie;const g=o[d],m=(s||[]).filter(Z=>Z.deck_id===g.id),f=m.filter(Z=>{var C;return Z.is_starter&&((C=Z.card)==null?void 0:C.player)}).map(Z=>ii(Z.card,Z.position)),p=m.find(Z=>{var C;return((C=Z.card)==null?void 0:C.card_type)==="formation"}),x=g.formation||((y=p==null?void 0:p.card)==null?void 0:y.formation)||"4-4-2";let b=f.length>=11?Gt(f,x):null,v=((B=g.stadium_card)==null?void 0:B.stadium_def)||null;v&&b&&(b=It(b,v));const h=f.length>=11;Ue(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff;position:relative">
      ...
      <!-- Pagination -->
      ${o.length>1?`<div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">${o.map((Z,C)=>`<div style="width:6px;height:6px;border-radius:50%;background:${C===d?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}</div>`:""}
    ...
    </div>`,requestAnimationFrame(()=>requestAnimationFrame(function Z(){const C=e.querySelector(".deck-preview-wrap"),_=e.querySelector("#deck-swipe-zone");if(!C||!_||!b)return;const I=_.clientWidth>=900,H=Math.max(200,_.clientHeight-(I?60:40)),se=Math.max(200,_.clientWidth-16),pe=I?Math.min(117,Math.max(52,Math.round(se*.22))):Math.max(44,Math.round(se*.168));if(H<220||se<220){requestAnimationFrame(Z);return}const de=I?null:Math.round(pe*.55);C.innerHTML=bt(b,x,null,[],se,H,[],de),C.style.cssText=`width:${se}px;height:${H}px;overflow:hidden;margin:${I?0:60}px auto 0`;const Te=C.querySelector("svg");Te&&(Te.style.cssText="display:block;width:100%;height:100%",Te.setAttribute("preserveAspectRatio",I?"xMidYMid meet":"none"))})),(G=document.getElementById("prev-deck"))==null||G.addEventListener("click",()=>{d>0&&(d--,u())}),(N=document.getElementById("next-deck"))==null||N.addEventListener("click",()=>{d<o.length-1&&(d++,u())}),(te=document.getElementById("validate-deck"))==null||te.addEventListener("click",()=>{if(!h)return;const Z=t.state.params||{};t.navigate("match",{...Z,matchMode:Z.matchMode||i,deckId:g.id})}),(ie=document.getElementById("cancel-deck-select"))==null||ie.addEventListener("click",()=>{Ne(e),a("home")});const E=document.getElementById("deck-swipe-zone");if(E){let Z=0,C=0;E.addEventListener("touchstart",_=>{Z=_.touches[0].clientX,C=_.touches[0].clientY},{passive:!0}),E.addEventListener("touchend",_=>{const I=_.changedTouches[0].clientX-Z,H=_.changedTouches[0].clientY-C;Math.abs(I)<40||Math.abs(I)<Math.abs(H)||(I<0&&d<o.length-1?(d++,u()):I>0&&d>0&&(d--,u()))},{passive:!0})}}u()}function Je(e,t=44,i=58,n=null){return Se(e,{width:t,showStad:!!n,stadDef:n,used:e==null?void 0:e.used})}function ut(e,t,i){if(!(e!=null&&e.length))return"";const n=e.slice(0,5);let a='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return n.forEach((o,r)=>{const s=o._line||o.job||"MIL";if(a+=Se(o,{width:40,role:s,extraNote:o.boost||0}),r<n.length-1){const l=vt(o,n[r+1]);a+=`<div style="width:7px;height:3px;background:${l==="#ff3333"||l==="#cc2222"?"rgba(255,255,255,0.12)":l};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(a+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),a+="</div>",a}function Pt(e,t,i,n,a=310,o=310,r=[],s=null){const l=Mt[t]||{},c=Hi(t)||di[t]||[],d={},u=["ATT","MIL","DEF","GK"];for(const b of u)(e[b]||[]).forEach((h,E)=>{d[`${b}${E+1}`]=h});function g(b){const v=l[b];return v?{x:v.x*a,y:v.y*o}:null}let m="";for(const[b,v]of c){const h=g(b),E=g(v);if(!h||!E)continue;const y=d[b],B=d[v],$=vt(y,B);$==="#00ff88"||$==="#FFD700"?(m+=`<line x1="${h.x.toFixed(1)}" y1="${h.y.toFixed(1)}" x2="${E.x.toFixed(1)}" y2="${E.y.toFixed(1)}"
        stroke="${$}" stroke-width="10" stroke-linecap="round" opacity="0.22"/>`,m+=`<line x1="${h.x.toFixed(1)}" y1="${h.y.toFixed(1)}" x2="${E.x.toFixed(1)}" y2="${E.y.toFixed(1)}"
        stroke="${$}" stroke-width="3.5" stroke-linecap="round" opacity="0.95"/>`):m+=`<line x1="${h.x.toFixed(1)}" y1="${h.y.toFixed(1)}" x2="${E.x.toFixed(1)}" y2="${E.y.toFixed(1)}"
        stroke="${$}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const f=typeof window<"u"&&window.innerWidth>=900?Math.min(Math.max(81,Math.round(a*.225)),117):Math.max(44,Math.round(a*.168)),p=Math.round(f*657/507);for(const[b,v]of Object.entries(d)){const h=g(b);if(!h||!v)continue;const E=b.replace(/[0-9]/g,""),y=r.includes(v.cardId),B=i==="attack"&&(["MIL","ATT"].includes(E)||y)&&!v.used||i==="defense"&&["GK","DEF","MIL"].includes(E)&&!v.used,$=n.includes(v.cardId);let G=v.boost||0,N=!1;v.stadiumBonus&&(i==="attack"&&(E==="ATT"||E==="MIL")||i==="defense"&&(E==="GK"||E==="DEF"||E==="MIL")?(G+=10,N=!0):i||(G+=10,N=!0));const te=Math.round(h.x-f/2),ie=Math.round(h.y-p/2);if(v.used){m+=`<image href="${`${typeof import.meta<"u"&&"/"||"/"}icons/carte-dos.png`}" x="${te}" y="${ie}" width="${f}" height="${p}" preserveAspectRatio="xMidYMid slice" class="match-used-hit" data-card-id="${v.cardId}" data-role="${E}" style="cursor:pointer"/>`;continue}const Z=Se({...v,_evolution_bonus:0,stadiumBonus:!1},{width:f,showStad:!1,stadDef:null,role:E,extraNote:G,_cardOffset:30,_forceStadColor:N}),C=$?`position:absolute;top:30px;left:0;width:${f}px;height:${p}px;outline:3px solid #FFD700;outline-offset:2px;border-radius:8px;pointer-events:none;`:"";m+=`<foreignObject x="${te-2}" y="${ie-30}" width="${f+8}" height="${p+60}" style="overflow:visible">
      <div xmlns="http://www.w3.org/1999/xhtml" style="position:relative">
        ${Z}
        ${$?`<div style="${C}"></div>`:""}
      </div>
    </foreignObject>`,B&&(m+=`<rect x="${te}" y="${ie}" width="${f}" height="${p}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${$?"selected":""}" data-card-id="${v.cardId}" data-role="${E}" style="cursor:pointer"/>`)}const x=s!==null?s:Math.round(Math.max(f*.7,80));return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-x} ${-x} ${a+x*2} ${o+x*2}" width="100%" style="display:block;width:100%;margin:0 auto">
    ${m}
  </svg>`}function bt(e,t,i,n,a=300,o=300,r=[],s=null){return`<div id="match-terrain-wrap" style="position:relative;padding:0 4px">
    ${Pt(e,t,i,n,a,o,r,s)}
  </div>`}async function At(e,t,i,n){var B;const{state:a,navigate:o,toast:r}=t;Ue(e);const s=a.params||{};if(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!s.deckId)return wo(e,t,i);const l=s.deckId;let c,d,u,g;try{const $=await Promise.all([w.from("decks").select("formation,name,stadium_card_id").eq("id",l).single(),w.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation, evolution_bonus,
            player:players(id,firstname,surname_real,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
              clubs(encoded_name,logo_url)))`).eq("deck_id",l).order("slot_order")]);c=$[0].data,u=$[0].error,d=$[1].data,g=$[1].error}catch($){console.error("[Match] Exception chargement deck:",$),at(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>o("home"));return}if(u||g){console.error("[Match] Erreur Supabase:",u||g),at(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>o("home"));return}const m=(d||[]).filter($=>{var G;return $.is_starter&&((G=$.card)==null?void 0:G.player)}).map($=>ii($.card,$.position)),f=(d||[]).filter($=>{var G;return!$.is_starter&&((G=$.card)==null?void 0:G.player)}).map($=>ii($.card,$.position));if(m.length<11){at(e,"⚠️",`Deck incomplet (${m.length}/11).`,"Compléter",()=>o("decks"));return}const p=(d||[]).find($=>{var G;return((G=$.card)==null?void 0:G.card_type)==="formation"}),x=(c==null?void 0:c.formation)||((B=p==null?void 0:p.card)==null?void 0:B.formation)||"4-4-2",{data:b,error:v}=await w.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",a.profile.id).eq("card_type","game_changer"),{data:h}=await w.from("gc_definitions").select("*").eq("is_active",!0),E=(b||[]).map($=>({...$,_gcDef:(h==null?void 0:h.find(G=>G.name===$.gc_type||G.id===$.gc_definition_id))||null}));let y=null;if(c!=null&&c.stadium_card_id){const{data:$}=await w.from("cards").select("stadium_id").eq("id",c.stadium_card_id).single();if($!=null&&$.stadium_id){const{data:G}=await w.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").eq("id",$.stadium_id).single();y=G||null}}n({deckId:l,formation:x,starters:m,subsRaw:f,gcCardsEnriched:E,gcDefs:h||[],stadiumDef:y})}function rt(){return Math.min(window.innerWidth-40,860)}function ft(){return Math.round(rt()*1.1)}function _o(e){var a,o;if(!e)return null;const t=e._line||e.job||"MIL",i=t==="GK"?e.note_g||0:t==="DEF"?e.note_d||0:t==="MIL"?e.note_m||0:e.note_a||0,n=e.stadiumBonus?10:0;return{name:e.name,firstname:e.firstname||"",note:i+(e.boost||0)+n,note_g:e.note_g||0,note_d:e.note_d||0,note_m:e.note_m||0,note_a:e.note_a||0,_evolution_bonus:0,stadiumBonus:e.stadiumBonus||!1,boost:e.boost||0,job:e.job,job2:e.job2||null,_line:e._line||e.job,_col:e._col,country_code:e.country_code,club_id:e.club_id,rarity:e.rarity,clubName:e.clubName||((a=e.clubs)==null?void 0:a.encoded_name)||null,clubLogo:e.clubLogo||((o=e.clubs)==null?void 0:o.logo_url)||null,face:e.face||null,portrait:St(e)}}function sn(e){var o,r,s;if(!e)return"";const t=l=>l?Se({...l,_evolution_bonus:0},{width:52,role:l._line||l.job,showStad:!!l.stadiumBonus,extraNote:l.boost||0}):"",n={goal:"⚽","goal-home":"⚽","goal-ai":"⚽",duel:"⚔️",midfield:"🎯",sub:"🔄",gc:"⚡",boost:"💥","defense-won":"🛡️","attack-won":"⚔️","defense-lost":"😓","attack-lost":"😓"}[e.type]||"📋";return`
    ...
    <div style="animation:goalPop 0.4s ease-out;text-align:center">
      ...
      }`,document.head.appendChild(l)}setTimeout(()=>{r.style.opacity="0",r.style.transition="opacity 0.4s",setTimeout(()=>{r.remove(),a==null||a()},400)},1800)}function ko(e,t,i){const n=document.getElementById("sub-anim-overlay");n&&n.remove();const a=document.createElement("div");a.id="sub-anim-overlay",a.style.cssText=`
    position:fixed;inset:0;z-index:3000;
    display:flex;flex-direction:column;align-items:center;justify-content:center;
    ...
    </div>`,document.body.appendChild(a),setTimeout(()=>{a.style.opacity="0",a.style.transition="opacity 0.4s",setTimeout(()=>{a.remove(),i==null||i()},400)},1600)}function $t(e,t="rgba(0,0,0,0.85)",i=2200){const n=document.getElementById("game-toast");n&&n.remove();const a=document.createElement("div");if(a.id="game-toast",a.style.cssText=`
    ...
    pointer-events:none;white-space:nowrap;max-width:90vw;text-align:center`,a.textContent=e,!document.getElementById("game-toast-style")){const o=document.createElement("style");o.id="game-toast-style",o.textContent=`
      ...
      }`,document.head.appendChild(o)}document.body.appendChild(a),setTimeout(()=>{a.style.opacity="0",a.style.transition="opacity 0.3s",setTimeout(()=>a.remove(),300)},i)}function dn(e,t){const i=Be(e,"MIL"),n=e.stadiumBonus||t&&(t.club_id&&String(e.club_id)===String(t.club_id)||t.country_code&&e.country_code===t.country_code)?10:0;return i+n}function Rt(e,t){return e.reduce((i,n)=>i+dn(n,t),0)}function Nt(e){let t=0;for(let i=0;i<e.length-1;i++){const n=vt(e[i],e[i+1]);n==="#00ff88"?t+=2:n==="#FFD700"&&(t+=1)}return t}function Gi(e,t,i,n,a){return`<div id="duel-row-${n}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0), opacity .5s ease;transform-origin:center">
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${e.map((o,r)=>{const s=r<e.length-1?vt(o,e[r+1]):null,l=!s||s==="#ff3333"||s==="#cc2222",c=s==="#00ff88"?"+2":s==="#FFD700"?"+1":"";return dn(o,a),o.stadiumBonus||a&&(a.club_id&&String(o.club_id)===String(a.club_id)||a.country_code&&(o.country_code,a.country_code)),`
    ...
    </div>`}async function $o(e,t){const{state:i}=t,a=(i.params||{}).matchMode||"vs_ai_easy",o=a.replace("vs_ai_",""),r=a;await At(e,t,a,async({deckId:s,formation:l,starters:c,subsRaw:d,gcCardsEnriched:u,gcDefs:g,stadiumDef:m})=>{try{let f=Gt(c,l);m&&(f=It(f,m),Ft(d,m));const p=await Eo(l,o),x=p.lines||p,b=async v=>{try{const{data:h,error:E}=await w.from("matches").insert({home_id:i.profile.id,away_id:null,mode:r,home_deck_id:s,status:"in_progress"}).select().single();if(E){console.error("[MatchIA] Erreur création match:",E),at(e,"⚠️","Impossible de créer le match ("+E.message+").","Retour",()=>t.navigate("home"));return}const y=p.stadiumDef||null;y&&x&&(It(x,y),Ft(p.subs||[],y));const B={gcDefs:g||[],matchId:h==null?void 0:h.id,mode:r,difficulty:o,formation:l,homeTeam:f,aiTeam:x,homeSubs:d,subsUsed:0,maxSubs:Math.min(d.length,3),aiSubs:p.subs||[],aiSubsUsed:0,aiMaxSubs:Math.min((p.subs||[]).length,3),aiUsedSubIds:[],aiGcCards:p.gcCards||[],aiUsedGc:[],aiStadiumDef:y,homeScore:0,aiScore:0,gcCards:v,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};To(e,B,t)}catch(h){console.error("[MatchIA] Exception launchMatch:",h),at(e,"⚠️","Erreur au lancement du match : "+h.message,"Retour",()=>t.navigate("home"))}};if(!u.length){b([]);return}ho(e,u,b)}catch(f){console.error("[MatchIA] Exception setup:",f),at(e,"⚠️","Erreur de préparation du match : "+f.message,"Retour",()=>t.navigate("home"))}})}async function Eo(e,t){var x;const{data:i}=await w.from("players").select("id,firstname,surname_real,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(80);if(!i||i.length<11)return{lines:Lo(e),subs:[],gcCards:[],stadiumDef:null};const n=xt[e]||xt["4-4-2"],a={GK:[],DEF:[],MIL:[],ATT:[]},o=new Set;function r(b,v,h){var E,y;return o.add(b.id),{cardId:"ai-"+b.id+"-"+h,id:b.id,firstname:b.firstname,name:b.surname_real,country_code:b.country_code,club_id:b.club_id,job:b.job,job2:b.job2,note_g:Number(b.note_g)||0,note_d:Number(b.note_d)||0,note_m:Number(b.note_m)||0,note_a:Number(b.note_a)||0,rarity:b.rarity,skin:b.skin,hair:b.hair,hair_length:b.hair_length,clubName:((E=b.clubs)==null?void 0:E.encoded_name)||null,clubLogo:((y=b.clubs)==null?void 0:y.logo_url)||null,boost:0,used:!1,_line:v}}for(const b of["GK","DEF","MIL","ATT"]){const v=i.filter($=>$.job===b&&!o.has($.id)),h=i.filter($=>$.job!==b&&!o.has($.id)),E=[...v,...h],y=[];for(let $=0;$<n[b];$++){const G=E[$];G&&y.push(r(G,b,$))}const B=yt(y.length);y.forEach(($,G)=>{$._col=B[G]}),a[b]=y}const l=i.filter(b=>!o.has(b.id)).slice(0,5).map((b,v)=>r(b,b.job,100+v)),u=Object.keys(Me).sort(()=>Math.random()-.5).slice(0,3).map((b,v)=>{var h,E;return{id:"ai-gc-"+v,gc_type:b,name:((h=Me[b])==null?void 0:h.name)||b,icon:((E=Me[b])==null?void 0:E.icon)||"⚡"}}),g=Object.values(a).flat(),m={};g.forEach(b=>{b.club_id&&(m[b.club_id]=(m[b.club_id]||0)+1)});const f=(x=Object.entries(m).sort((b,v)=>v[1]-b[1])[0])==null?void 0:x[0];let p=null;if(f){const{data:b}=await w.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",f).single();b&&(p={club_id:b.id,country_code:null,name:b.encoded_name+" Stadium",club:{encoded_name:b.encoded_name,logo_url:b.logo_url}})}return{lines:a,subs:l,gcCards:u,stadiumDef:p}}function Lo(e){const t=xt[e]||xt["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},n=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let a=0;for(const o of["GK","DEF","MIL","ATT"]){const r=[];for(let l=0;l<t[o];l++){const c=3+Math.floor(Math.random()*5);r.push({cardId:"fake-"+a,id:"fake-"+a,firstname:"IA",name:n[a%n.length],country_code:"XX",club_id:null,job:o,job2:null,note_g:o==="GK"?c:2,note_d:o==="DEF"?c:2,note_m:o==="MIL"?c:2,note_a:o==="ATT"?c:2,rarity:"normal",boost:0,used:!1,_line:o}),a++}const s=yt(r.length);r.forEach((l,c)=>{l._col=s[c]}),i[o]=r}return i}function To(e,t,i){var a;const n=`<div style="position:relative;width:22px;height:22px;flex-shrink:0;display:flex;align-items:center;justify-content:center">
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    ${(()=>{const _=window.innerWidth>=700,I=(R,z,A)=>{var be,Ce;const F=(t.gcDefs||[]).find(qe=>qe.name===R.gc_type),V={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[F==null?void 0:F.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",U={purple:"#b06ce0",light_blue:"#00d4ef"}[F==null?void 0:F.color]||"#b06ce0",j=(F==null?void 0:F.name)||R.gc_type,O=(F==null?void 0:F.effect)||((be=Me[R.gc_type])==null?void 0:be.desc)||"",_e=F!=null&&F.image_url?`/icons/${F.image_url}`:null,ke=((Ce=Me[R.gc_type])==null?void 0:Ce.icon)||"⚡",ve=Math.round(A*.22),me=Math.round(A*.22),$e=A-ve-me,Ee=j.length>12?7:9;return`<div class="gc-mini" data-gc-id="${R.id}" data-gc-type="${R.gc_type}"
          style="box-sizing:border-box;width:${z}px;height:${A}px;border-radius:10px;border:2px solid ${U};background:${V};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
          <div style="height:${ve}px;background:rgba(255,255,255,0.14);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 3px">
            <span style="font-size:${Ee}px;font-weight:900;color:#fff;text-align:center;line-height:1.1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${z-6}px">${j}</span>
          ...
          <div style="height:${$e}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
            ${_e?`<img src="${_e}" style="max-width:${z-10}px;max-height:${$e-6}px;object-fit:contain">`:`<span style="font-size:${Math.round($e*.55)}px">${ke}</span>`}
          </div>
        ...
        </div>`},H=(R,z)=>{var A;return`<div id="boost-card"
  ...
  <div id="match-history-panel">
  ...
  </div>`;function h(){const _=e.querySelector(".match-screen");if(!_)return;const I=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);_.style.bottom="auto",_.style.height=I+"px",_.style.minHeight=I+"px",_.style.maxHeight=I+"px",_.style.overflow="hidden";const H=e.querySelector("#mobile-action-bar"),le=e.querySelector("#mobile-play-area");H&&le&&(le.style.paddingBottom=H.offsetHeight+"px")}if(h(),setTimeout(h,120),setTimeout(h,400),setTimeout(h,1e3),t._vvBound||(t._vvBound=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",h),window.visualViewport.addEventListener("scroll",h)),window.addEventListener("resize",h)),function(){const I=e.querySelector("#match-field .terrain-wrapper svg")||e.querySelector(".terrain-wrapper svg");if(!I)return;const H=I.closest("#match-terrain-wrap");H&&(H.style.cssText="position:relative;width:100%;height:100%;padding:0"),I.removeAttribute("width"),I.removeAttribute("height"),I.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",I.setAttribute("preserveAspectRatio","xMidYMid meet")}(),t._resizeBound||(t._resizeBound=!0,window.addEventListener("resize",()=>{const _=e.querySelector(".terrain-wrapper svg");if(_){const I=_.closest("#match-terrain-wrap");I&&(I.style.cssText="position:relative;width:100%;height:100%;padding:0"),_.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0"}})),t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase==="attack"||t.phase==="defense"){let _=!1,I=30;const H=()=>document.getElementById("match-timer"),le=()=>{const se=H();if(!se)return;const pe=String(Math.floor(I/60)).padStart(2,"0"),de=String(I%60).padStart(2,"0");se.textContent=` ${pe}:${de}`,se.style.color=_?"#ff2222":"#ff9500",se.style.fontWeight="900"};le(),t._timerInt=setInterval(()=>{if(I--,I<0)if(!_)_=!0,I=15,le();else{clearInterval(t._timerInt),t._timerInt=null,t.homeScore=0,t.aiScore=3;const se=document.createElement("div");se.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;color:#fff;padding:24px;text-align:center",se.innerHTML='<div style="font-size:56px">⏱️</div><div style="font-size:24px;font-weight:900;color:#ff4444">MATCH PERDU PAR FORFAIT</div><div style="font-size:14px;color:rgba(255,255,255,0.6)">Temps écoulé</div>',document.body.appendChild(se),setTimeout(()=>{se.remove(),gt(e,t,i)},2500)}else le()},1e3)}(y=document.getElementById("match-quit"))==null||y.addEventListener("click",()=>{Ne(e),confirm("Abandonner ? Résultat : défaite 3-0")&&(t.homeScore=0,t.aiScore=3,gt(e,t,i))}),(B=document.getElementById("view-ai"))==null||B.addEventListener("click",()=>Go(t,i)),($=document.getElementById("toggle-history"))==null||$.addEventListener("click",()=>{var _;(_=document.getElementById("match-history-panel"))==null||_.classList.add("open")}),(G=document.getElementById("close-history"))==null||G.addEventListener("click",()=>{var _;(_=document.getElementById("match-history-panel"))==null||_.classList.remove("open")}),(N=document.getElementById("btn-action"))==null||N.addEventListener("click",()=>{t.selected.length!==0&&(u?Io(e,t,i):g&&Mo(e,t,i))}),(te=document.getElementById("btn-results"))==null||te.addEventListener("click",()=>gt(e,t,i)),(ie=document.getElementById("btn-pass"))==null||ie.addEventListener("click",()=>{t.log.push({text:"⏭️ Vous passez votre tour (plus d'attaquants)",type:"info"}),t.phase="ai-attack",Le(e,t,i),setTimeout(()=>xi(e,t,i),800)}),e.querySelectorAll(".match-slot-hit").forEach(_=>{_.addEventListener("click",()=>So(_,t,e,i))}),e.querySelectorAll(".match-used-hit").forEach(_=>{_.addEventListener("click",()=>Jt(e,t,i,null,_.dataset.cardId))}),e.querySelectorAll(".gc-mini").forEach(_=>{_.addEventListener("click",()=>Bo(_.dataset.gcId,_.dataset.gcType,e,t,i))}),(Z=document.getElementById("boost-card"))==null||Z.addEventListener("click",()=>Fo(e,t,i)),e.querySelectorAll(".sub-btn-col").forEach(_=>{_.addEventListener("click",()=>Jt(e,t,i,_.dataset.subId))}),(C=document.getElementById("sub-btn-main"))==null||C.addEventListener("click",()=>Jt(e,t,i))}function So(e,t,i,n){const a=e.dataset.cardId,o=e.dataset.role,r=t.selected.findIndex(s=>s.cardId===a);if(r!==-1)t.selected.splice(r,1);else{if(t.selected.length>=3){n.toast("Maximum 3 joueurs","error");return}const s=(t.homeTeam[o]||[]).find(l=>l.cardId===a);s&&t.selected.push({...s,_role:o,_line:o})}Le(i,t,n)}function mi(e,t,i){e.matchId&&w.from("matches").update({last_player_id:i}).eq("id",e.matchId).then(()=>{})}function Io(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),mi(t,i,i.state.profile.id);const n=t.selected.map(o=>{const r=(t.homeTeam[o._role]||[]).find(l=>l.cardId===o.cardId)||o,s=["GK","DEF"].includes(o._role);return{...r,_line:o._role,...s?{note_a:Math.max(1,Number(r.note_a)||0)}:{}}}),a=ui(n,t.modifiers.home);t.pendingAttack={...a,players:[...n],side:"home"},t.selected.forEach(o=>{const r=(t.homeTeam[o._role]||[]).find(s=>s.cardId===o.cardId);r&&(r.used=!0)}),t.log.push({text:`⚔️ Vous attaquez : ${a.total} (base ${a.base}${a.links?` +${a.links} liens`:""}) — ${t.selected.map(o=>o.name).join(", ")}`,type:"info"}),t.selected=[],t.modifiers.home={},t.phase="ai-defense",Le(e,t,i),setTimeout(()=>jo(e,t,i),1200)}function Mo(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),mi(t,i,i.state.profile.id);const n=t.stadiumDef||null,a=t.selected.map(l=>{const c=(t.homeTeam[l._role]||[]).find(u=>u.cardId===l.cardId)||l,d=c.stadiumBonus||n&&(n.club_id&&String(c.club_id)===String(n.club_id)||n.country_code&&c.country_code===n.country_code)||!1;return{...c,_line:l._role,stadiumBonus:d}}),o=fi(a,t.modifiers.home);t.selected.forEach(l=>{const c=(t.homeTeam[l._role]||[]).find(d=>d.cardId===l.cardId);c&&(c.used=!0)});const r=gi(t.pendingAttack.total,o.total,t.modifiers.home),s={type:"duel",title:"Défense",aiPlayers:(t.pendingAttack.players||[]).map(l=>Fe(l)),homePlayers:t.selected.map(l=>{const c=(t.homeTeam[l._role]||[]).find(d=>d.cardId===l.cardId)||l;return Fe(c)}),homeTotal:o.total,aiTotal:t.pendingAttack.total,isGoal:!1,homeScored:!1,text:""};if(r.shielded)s.text="🛡️ Bouclier ! But annulé.",t.log.push(s);else if(r.goal){t.aiScore++,s.isGoal=!0,s.homeScored=!1,s.text=`⚽ BUT IA ! (${t.pendingAttack.total} > ${o.total})`,t.log.push(s),t.selected=[],t.modifiers.home={},t.pendingAttack=null,Le(e,t,i),Qe(s.aiPlayers,t.homeScore,t.aiScore,!1,()=>{st(e,t,i,"home-attack")});return}else s.text=`🧤 Défense réussie ! (${o.total} ≥ ${t.pendingAttack.total})`,t.log.push(s);t.selected=[],t.modifiers.home={},t.pendingAttack=null,st(e,t,i,"home-attack")}function Ao(e){if(e.aiSubsUsed>=e.aiMaxSubs)return;const t=Object.values(e.aiTeam).flat().filter(l=>l.used);if(!t.length)return;const i=(e.aiSubs||[]).filter(l=>!e.aiUsedSubIds.includes(l.cardId));if(!i.length)return;const n=t[Math.floor(Math.random()*t.length)],a=i.find(l=>l.job===n.job)||i[0],o={...a,used:!1,_line:n._line,_col:n._col},r=e.aiTeam[n._line],s=r.findIndex(l=>l.cardId===n.cardId);s!==-1&&(r[s]=o),e.aiUsedSubIds.push(a.cardId),e.aiSubsUsed++,e.log.push({text:`🔄 IA : ${a.firstname} ${a.name} remplace ${n.firstname} ${n.name}`,type:"info"})}function Co(e){var n;if(!((n=e.aiGcCards)!=null&&n.length))return;const t=e.aiGcCards.filter(a=>!e.aiUsedGc.includes(a.id));if(!t.length||Math.random()>.3)return;const i=t[Math.floor(Math.random()*t.length)];switch(e.aiUsedGc.push(i.id),Me[i.gc_type],i.gc_type){case"Boost+2":{const a=Object.values(e.aiTeam).flat().filter(o=>!o.used);if(a.length){const o=a[Math.floor(Math.random()*a.length)];o.boost=(o.boost||0)+2}break}case"Boost+3":{const a=Object.values(e.aiTeam).flat().filter(o=>!o.used);if(a.length){const o=a[Math.floor(Math.random()*a.length)];o.boost=(o.boost||0)+3}break}case"Remplacement+":e.aiMaxSubs=(e.aiMaxSubs||3)+1;break;case"Bouclier":e.modifiers.ai.shield=!0;break;case"Nul+1":e.modifiers.ai.drawBonus=(e.modifiers.ai.drawBonus||0)+1;break}e.log.push({text:`⚡ IA joue ${i.icon||"⚡"} ${i.name}`,type:"gc"})}function xi(e,t,i){mi(t,i,null),Ao(t),Co(t);let n=[...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]].filter(d=>!d.used),a=!1;n.length||(n=[...t.aiTeam.GK||[],...t.aiTeam.DEF||[]].filter(d=>!d.used),a=!0);const o=Ji(n,"attack",t.difficulty);if(!o.length){oi(e,t,i);return}a&&o.forEach(d=>{d._line=d._line||d.job,d.note_a=Math.max(1,Number(d.note_a)||0)});const r=ui(o,t.modifiers.ai);t.pendingAttack={...r,players:o,side:"ai"},o.forEach(d=>{d.used=!0}),t.log.push({text:`🤖 IA attaque : ${r.total} (${o.map(d=>d.name).join(", ")})`,type:"info"}),t.modifiers.ai={};const s=[...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(d=>!d.used),c=(t.homeSubs||[]).filter(d=>!(t.usedSubIds||[]).includes(d.cardId)).length>0&&t.subsUsed<t.maxSubs;if(s.length===0&&!c){if(o.length===1&&(o[0]._line==="GK"||o[0].job==="GK")&&ni(t.homeTeam)&&t.homeScore===t.aiScore){t.aiScore++,Ot(e,t,i,"ai",o[0]);return}t.aiScore++;const u={type:"duel",isGoal:!0,homeScored:!1,aiPlayers:o.map(g=>Fe(g)),aiTotal:r.total,text:"⚽ BUT IA ! (aucun défenseur disponible)"};t.log.push(u),t.pendingAttack=null,Le(e,t,i),Qe(u.aiPlayers,t.homeScore,t.aiScore,!1,()=>{st(e,t,i,"home-attack")});return}t.phase="defense",Le(e,t,i)}function jo(e,t,i){var c,d;const n=[...t.aiTeam.GK||[],...t.aiTeam.DEF||[],...t.aiTeam.MIL||[]],a=Ji(n,"defense",t.difficulty);if(!["GK","DEF","MIL","ATT"].flatMap(u=>(t.aiTeam[u]||[]).filter(g=>!g.used)).length){const u=((c=t.pendingAttack)==null?void 0:c.players)||[];if(u.length===1&&(u[0]._line==="GK"||u[0].job==="GK")&&t.homeScore===t.aiScore){t.homeScore++,Ot(e,t,i,"home",u[0]);return}t.homeScore++;const m={type:"duel",isGoal:!0,homeScored:!0,homePlayers:u.map(f=>Fe(f)),homeTotal:((d=t.pendingAttack)==null?void 0:d.total)||0,aiTotal:0,text:"⚽ BUT ! L'IA n'a plus de joueurs — but automatique !"};t.log.push(m),t.modifiers.ai={},t.pendingAttack=null,Le(e,t,i),Qe(m.homePlayers,t.homeScore,t.aiScore,!0,()=>{st(e,t,i,"ai-attack")});return}const r=a.length>0?fi(a,t.modifiers.ai).total:0;a.forEach(u=>{u.used=!0});const s=gi(t.pendingAttack.total,r,t.modifiers.ai),l={type:"duel",title:"Attaque",homePlayers:(t.pendingAttack.players||[]).map(u=>Fe(u)),aiPlayers:a.map(u=>Fe(u)),homeTotal:t.pendingAttack.total,aiTotal:r,isGoal:!1,homeScored:!1,text:""};if(s.shielded)l.text="🛡️ Bouclier IA !",t.log.push(l);else if(s.goal){t.homeScore++,l.isGoal=!0,l.homeScored=!0,l.text=`⚽ BUT ! (${t.pendingAttack.total} > ${r})`,t.log.push(l),t.modifiers.ai={},t.pendingAttack=null,Le(e,t,i),Qe(l.homePlayers,t.homeScore,t.aiScore,!0,()=>{st(e,t,i,"ai-attack")});return}else l.text=`🧤 IA défend (${r} ≥ ${t.pendingAttack.total})`,t.log.push(l);t.modifiers.ai={},t.pendingAttack=null,st(e,t,i,"ai-attack")}function st(e,t,i,n){if(t.round++,!ln(e,t,i)){if(yi(t)){gt(e,t,i);return}if(n==="home-attack"){if(!["MIL","ATT","GK","DEF"].some(o=>(t.homeTeam[o]||[]).some(r=>!r.used))){oi(e,t,i);return}t.phase="attack",Le(e,t,i)}else{if(!["MIL","ATT","GK","DEF"].some(o=>(t.aiTeam[o]||[]).some(r=>!r.used))){oi(e,t,i);return}t.phase="ai-attack",Le(e,t,i),setTimeout(()=>xi(e,t,i),800)}}}function ni(e){return!["GK","DEF","MIL","ATT"].some(t=>(e[t]||[]).some(i=>!i.used))}function Pi(e){const t=(e.GK||[]).some(n=>!n.used),i=["DEF","MIL","ATT"].some(n=>(e[n]||[]).some(a=>!a.used));return t&&!i}function ln(e,t,i){if(t.homeScore!==t.aiScore)return!1;if(Pi(t.homeTeam)&&ni(t.aiTeam)){const n=(t.homeTeam.GK||[]).find(a=>!a.used);return n?(n.used=!0,t.homeScore++,Ot(e,t,i,"home",n),!0):!1}if(Pi(t.aiTeam)&&ni(t.homeTeam)){const n=(t.aiTeam.GK||[]).find(a=>!a.used);return n?(n.used=!0,t.aiScore++,Ot(e,t,i,"ai",n),!0):!1}return!1}function Ot(e,t,i,n,a){t.log.push({type:"duel",isGoal:!0,homeScored:n==="home",homePlayers:n==="home"?[Fe(a)]:[],aiPlayers:n==="ai"?[Fe(a)]:[],text:`⚽ DERNIER CORNER — Le gardien ${n==="home"?"":"adverse "}marque !`});const o=document.createElement("div");o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:950;display:flex;align-items:center;justify-content:center;overflow:hidden;text-align:center;padding:24px",document.body.appendChild(o);const r=(s,l)=>new Promise(c=>{o.innerHTML=`<div style="font-size:32px;font-weight:900;color:${l};letter-spacing:2px;animation:lcFade 1.4s ease both">${s}</div>
    <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(c,1400)});(async()=>(await r("⚽ DERNIER CORNER","#FFD700"),await r("🧤 LE GARDIEN MONTE !","#4fc3f7"),o.remove(),t.pendingAttack=null,Le(e,t,i),Qe([Fe(a)],t.homeScore,t.aiScore,n==="home",()=>{if(yi(t)){gt(e,t,i);return}st(e,t,i,n==="home"?"ai-attack":"home-attack")})))()}function yi(e){const t=["MIL","ATT","GK","DEF"].some(n=>(e.homeTeam[n]||[]).some(a=>!a.used)),i=["MIL","ATT","GK","DEF"].some(n=>(e.aiTeam[n]||[]).some(a=>!a.used));return!t&&!i}function oi(e,t,i){ln(e,t,i)||(yi(t)?gt(e,t,i):(t.phase="attack",Le(e,t,i)))}function Jt(e,t,i,n=null,a=null){var m,f;if(t.phase!=="attack"){$t("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){$t(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const o=Object.entries(t.homeTeam).flatMap(([p,x])=>(x||[]).filter(b=>b.used).map(b=>({...b,_line:b._line||p}))),r=t.homeSubs.filter(p=>!t.usedSubIds.includes(p.cardId));if(!o.length){$t("Aucun joueur utilisé à remplacer");return}if(!r.length){$t("Aucun remplaçant disponible");return}let s=Math.max(0,o.findIndex(p=>p.cardId===a));const l=((m=o[s])==null?void 0:m._line)||((f=o[s])==null?void 0:f.job);let c=n?Math.max(0,r.findIndex(p=>p.cardId===n)):Math.max(0,r.findIndex(p=>p.job===l)),d=!1;const u=document.createElement("div");u.id="sub-overlay",u.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function g(){var y,B,$,G,N,te;const p=o[s],x=r[c],b=Math.min(130,Math.round((window.innerWidth-90)/2)),v=Math.round(b*1.35),h=ie=>`background:rgba(255,255,255,0.12);border:none;color:${ie?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${ie?"default":"pointer"};flex-shrink:0`;u.innerHTML=`
    ...
    <div style="flex:1;display:flex;gap:0;overflow:hidden">
      ...
      <div id="in-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
        ...
        <div>${x?Je({...x,used:!1,boost:0},b,v):"<div>—</div>"}</div>
      ...
      <div id="out-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
        ...
        <div>${p?Je({...p,used:!1,boost:0},b,v):"<div>—</div>"}</div>
    ...
    </div>`,(y=u.querySelector("#sub-close"))==null||y.addEventListener("click",()=>u.remove()),(B=u.querySelector("#out-up"))==null||B.addEventListener("click",()=>{s>0&&(s--,g())}),($=u.querySelector("#out-down"))==null||$.addEventListener("click",()=>{s<o.length-1&&(s++,g())}),(G=u.querySelector("#in-up"))==null||G.addEventListener("click",()=>{c>0&&(c--,g())}),(N=u.querySelector("#in-down"))==null||N.addEventListener("click",()=>{c<r.length-1&&(c++,g())});const E=(ie,Z,C,_)=>{const I=u.querySelector("#"+ie);if(!I)return;let H=0;I.addEventListener("touchstart",le=>{H=le.touches[0].clientY},{passive:!0}),I.addEventListener("touchend",le=>{const se=le.changedTouches[0].clientY-H;if(Math.abs(se)<30)return;const pe=Z();se<0&&pe<_-1?(C(pe+1),g()):se>0&&pe>0&&(C(pe-1),g())},{passive:!0})};E("in-panel",()=>c,ie=>c=ie,r.length),E("out-panel",()=>s,ie=>s=ie,o.length),(te=u.querySelector("#sub-confirm"))==null||te.addEventListener("click",ie=>{if(ie.preventDefault(),ie.stopPropagation(),d)return;d=!0;const Z=o[s],C=r[c];if(!Z||!C)return;let _=null,I=-1;for(const[le,se]of Object.entries(t.homeTeam)){const pe=(se||[]).findIndex(de=>de.cardId===Z.cardId);if(pe!==-1){_=le,I=pe;break}}if(I===-1||!_){$t("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),u.remove();return}const H={...C,_line:_,_col:Z._col||0,used:!1,boost:0};t.homeTeam[_].splice(I,1,H),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(C.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:Z.name,firstname:Z.firstname,note:Be(Z,_),portrait:St(Z),job:Z.job,country_code:Z.country_code,rarity:Z.rarity,clubName:Z.clubName,clubLogo:Z.clubLogo},inPlayer:{name:C.name,firstname:C.firstname,note:Be(C,_),portrait:St(C),job:C.job,country_code:C.country_code,rarity:C.rarity,clubName:C.clubName,clubLogo:C.clubLogo},text:`🔄 ${C.firstname} ${C.name} remplace ${Z.firstname} ${Z.name}`}),u.remove(),ko(Z,C,()=>Le(e,t,i))})}document.body.appendChild(u),g()}function Bo(e,t,i,n,a){var f,p;const o=(n.gcDefs||[]).find(x=>x.name===t),r=Me[t]||{icon:"⚡",desc:"Carte spéciale."},s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[o==null?void 0:o.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",l={purple:"#b06ce0",light_blue:"#00d4ef"}[o==null?void 0:o.color]||"#b06ce0",c=(o==null?void 0:o.name)||t,d=(o==null?void 0:o.effect)||r.desc,u=o!=null&&o.image_url?`/icons/${o.image_url}`:null,g=r.icon||"⚡",m=document.createElement("div");m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",m.innerHTML=`
    ...
    </div>`,document.body.appendChild(m),(f=m.querySelector("#gc-back"))==null||f.addEventListener("click",()=>m.remove()),(p=m.querySelector("#gc-use"))==null||p.addEventListener("click",()=>{m.remove(),Do(e,t,i,n,a)})}function Bt(e,t,i,n,a,o){const r=document.createElement("div");r.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let s=[];function l(){var c,d;r.innerHTML=`
    ...
    </div>`,(c=r.querySelector("#gc-picker-close"))==null||c.addEventListener("click",()=>r.remove()),r.querySelectorAll(".gc-pick-item").forEach(u=>{u.addEventListener("click",()=>{const g=u.dataset.cid,m=e.find(p=>p.cardId===g);if(!m)return;const f=s.findIndex(p=>p.cardId===g);f>-1?s.splice(f,1):s.length<t&&s.push(m),l()})}),(d=r.querySelector("#gc-picker-confirm"))==null||d.addEventListener("click",()=>{r.remove(),o(s)})}l(),document.body.appendChild(r)}const qo={BOOST_STAT:({value:e=1,count:t=1,roles:i=[]},n,a,o)=>{const r=Object.entries(n.homeTeam).filter(([s])=>!i.length||i.includes(s)).flatMap(([s,l])=>l.filter(c=>!c.used).map(c=>({...c,_line:s})));return r.length?(Bt(r,t,`Choisir ${t} joueur(s) à booster (+${e})`,a,n,s=>{s.forEach(l=>{const c=(n.homeTeam[l._line]||[]).find(d=>d.cardId===l.cardId);c&&(c.boost=(c.boost||0)+e,n.log.push({text:`⚡ +${e} sur ${c.name}`,type:"info"}))}),Le(a,n,o)}),!0):(n.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),Le(a,n,o),!0)},DEBUFF_STAT:({value:e=1,count:t=1,roles:i=[],target:n="ai"},a,o,r)=>{const s=n==="home"?a.homeTeam:a.aiTeam,l=n==="ai"?"adverse":"allié",c=Object.entries(s).filter(([d])=>!i.length||i.includes(d)).flatMap(([d,u])=>u.filter(g=>!g.used).map(g=>({...g,_line:d})));return c.length?(Bt(c,t,`Choisir ${t} joueur(s) ${l}(s) à débuffer (-${e})`,o,a,d=>{d.forEach(u=>{const m=((n==="home"?a.homeTeam:a.aiTeam)[u._line]||[]).find(f=>f.cardId===u.cardId);m&&(m.boost=(m.boost||0)-e,a.log.push({text:`🎯 -${e} sur ${m.name}${n==="ai"?" (IA)":""}`,type:"info"}))}),Le(o,a,r)}),!0):(a.log.push({text:`🎯 Aucun joueur ${l} disponible`,type:"info"}),Le(o,a,r),!0)},GRAY_PLAYER:({count:e=1,roles:t=[],target:i="ai"},n,a,o)=>{const r=i==="home"?n.homeTeam:n.aiTeam,s=i==="ai"?"adverse":"allié",l=Object.entries(r).filter(([c])=>!t.length||t.includes(c)).flatMap(([c,d])=>d.filter(u=>!u.used).map(u=>({...u,_line:c})));return l.length?(Bt(l,e,`Choisir ${e} joueur(s) ${s}(s) à exclure`,a,n,c=>{c.forEach(d=>{const g=((i==="home"?n.homeTeam:n.aiTeam)[d._line]||[]).find(m=>m.cardId===d.cardId);g&&(g.used=!0,n.log.push({text:`❌ ${g.name}${i==="ai"?" (IA)":""} exclu !`,type:"info"}))}),Le(a,n,o)}),!0):(n.log.push({text:`❌ Aucun joueur ${s} à exclure`,type:"info"}),Le(a,n,o),!0)},REVIVE_PLAYER:({count:e=1,roles:t=[]},i,n,a)=>{const o=Object.entries(i.homeTeam).filter(([r])=>!t.length||t.includes(r)).flatMap(([r,s])=>s.filter(l=>l.used).map(l=>({...l,_line:r})));return o.length?(Bt(o,e,`Choisir ${e} joueur(s) à ressusciter`,n,i,r=>{r.forEach(s=>{const l=(i.homeTeam[s._line]||[]).find(c=>c.cardId===s.cardId);l&&(l.used=!1,i.log.push({text:`💫 ${l.name} ressuscité !`,type:"info"}))}),Le(n,i,a)}),!0):(i.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),Le(n,i,a),!0)},REMOVE_GOAL:({},e)=>(e.aiScore>0?(e.aiScore--,e.log.push({text:"🚫 Dernier but IA annulé !",type:"info"})):e.log.push({text:"🚫 Aucun but à annuler",type:"info"}),!1),ADD_GOAL_DRAW:({},e)=>(e.homeScore===e.aiScore?(e.homeScore++,e.log.push({text:"🎯 But bonus (match nul) !",type:"info"})):e.log.push({text:"🎯 But bonus : non applicable (pas de match nul)",type:"info"}),!1),ADD_SUB:({value:e=1},t)=>(t.maxSubs=(t.maxSubs||3)+e,t.log.push({text:`🔄 +${e} remplacement(s) débloqué(s)`,type:"info"}),!1),CUSTOM:()=>!1};function Do(e,t,i,n,a){n.usedGc.push(e);const o=n.gcDefs||[],r=o.find(l=>l.name===t)||o.find(l=>{var c;return((c=l.name)==null?void 0:c.toLowerCase().trim())===(t==null?void 0:t.toLowerCase().trim())});let s=!1;if(r!=null&&r.effect_type&&r.effect_type!=="CUSTOM"){const l=qo[r.effect_type];l?l(r.effect_params||{},n,i,a)||(s=!0):(a.toast(`Effet "${r.effect_type}" non implémenté`,"error"),s=!0)}else{switch(t){case"Double attaque":n.modifiers.home.doubleAttack=!0,n.log.push({text:"⚡ Double attaque activée !",type:"info"});break;case"Bouclier":n.modifiers.home.shield=!0,n.log.push({text:"🛡️ Bouclier activé !",type:"info"});break;case"Ressusciter":{const l=Object.entries(n.homeTeam).flatMap(([c,d])=>(d||[]).filter(u=>u.used).map(u=>({...u,_line:c})));l.length?(l[0].used=!1,n.log.push({text:`💫 ${l[0].name} ressuscité !`,type:"info"})):n.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"});break}case"Vol de note":n.modifiers.ai.stolenNote=(n.modifiers.ai.stolenNote||0)+1,n.log.push({text:"🎯 -1 à la prochaine attaque IA",type:"info"});break;case"Gel":{const l=[...n.aiTeam.ATT||[],...n.aiTeam.MIL||[]].filter(c=>!c.used);if(l.length){const c=l.sort((d,u)=>Be(u,"ATT")-Be(d,"ATT"))[0];c.used=!0,n.log.push({text:`❄️ ${c.name} (IA) gelé !`,type:"info"})}break}case"Remplacement+":n.maxSubs++,n.log.push({text:"🔄 +1 remplacement débloqué",type:"info"});break}s=!0}w.from("cards").delete().eq("id",e).then(()=>{}),s&&Le(i,n,a)}function Fo(e,t,i){const n=Object.values(t.homeTeam).flat().filter(a=>!a.used);if(!n.length){i.toast("Aucun joueur actif à booster","error");return}i.openModal("⚡ Utiliser le Boost",`<div style="margin-bottom:12px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border-radius:10px;padding:12px;text-align:center;color:#000">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(a=>{a.addEventListener("click",()=>{const o=a.dataset.cardId;for(const r of["GK","DEF","MIL","ATT"]){const s=(t.homeTeam[r]||[]).find(l=>l.cardId===o);if(s){s.boost=(s.boost||0)+t.boostCard.value,t.log.push({text:`⚡ Boost +${t.boostCard.value} appliqué à ${s.name}`,type:"info"});break}}t.boostUsed=!0,i.closeModal(),Le(e,t,i)})})}async function gt(e,t,i){var d,u;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:n}=i,a=t.homeScore>t.aiScore,o=t.homeScore===t.aiScore,r=a?"victoire":o?"nul":"defaite",s=Gn(t.mode,r);t.matchId&&await w.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:a?n.profile.id:null,home_credits_reward:s,played_at:new Date().toISOString()}).eq("id",t.matchId);const l={credits:(n.profile.credits||0)+s,matches_played:(n.profile.matches_played||0)+1};a?l.wins=(n.profile.wins||0)+1:o?l.draws=(n.profile.draws||0)+1:l.losses=(n.profile.losses||0)+1,await w.from("users").update(l).eq("id",n.profile.id),await i.refreshProfile();const c=document.createElement("div");c.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",c.innerHTML=`
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      ...
      ${Pt(e.aiTeam,e.formation,null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.1))}
    </div>`,`<button class="btn btn-primary" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}async function ht(e,t,i,n,a={}){return cn(e,t,i,n,a.myGC||[],a.gcDefs||[],a.isRanked||!1,a.rankedData||null,a.stadiumDef||null,a.onMatchEnd||null,a.mlLeagueId||null,a.mlMatchId||null)}async function Po(e,t,i){const{data:n}=await w.from("matches").select("home_id,away_id,mode,is_ranked").eq("id",i).single();if(!n){t.toast("Match introuvable","error"),t.navigate("home");return}const a=n.home_id===t.state.user.id;return cn(e,t,i,a,[],[],n.is_ranked||!1,null,null,null,null,null)}async function cn(e,t,i,n,a=[],o=[],r=!1,s=null,l=null,c=null,d=null,u=null){const{state:g,navigate:m,toast:f}=t,p=n?"p1":"p2",x=n?"p2":"p1",b=(a||[]).map(k=>k.id),v=(a||[]).map(k=>({id:k.id,gc_type:k.gc_type,_gcDef:k._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:h}=await w.from("matches").select("*").eq("id",i).single();if(!h){f("Match introuvable","error"),m("home");return}async function E(){var fe,xe;console.log("[PvP] buildGameState match:",{id:h.id,home_deck_id:h.home_deck_id,away_deck_id:h.away_deck_id,mode:h.mode,is_ranked:h.is_ranked});const[{data:k,error:L},{data:S,error:M},{data:T},{data:D}]=await Promise.all([w.rpc("get_deck_for_match",{p_deck_id:h.home_deck_id}),w.rpc("get_deck_for_match",{p_deck_id:h.away_deck_id}),w.from("users").select("id,pseudo,club_name").eq("id",h.home_id).single(),w.from("users").select("id,pseudo,club_name").eq("id",h.away_id).single()]);console.log("[PvP] get_deck_for_match p1:",L==null?void 0:L.message,"p2:",M==null?void 0:M.message,"p1D:",(fe=k==null?void 0:k.starters)==null?void 0:fe.length,"p2D:",(xe=S==null?void 0:S.starters)==null?void 0:xe.length);const q=ee=>{const he=Number(ee.evolution_bonus)||0;return{cardId:ee.card_id,position:ee.position,id:ee.id,firstname:ee.firstname,name:ee.surname_real,country_code:ee.country_code,club_id:ee.club_id,job:ee.job,job2:ee.job2,note_g:(Number(ee.note_g)||0)+(ee.job==="GK"||ee.job2==="GK"&&Number(ee.note_g)>0?he:0),note_d:(Number(ee.note_d)||0)+(ee.job==="DEF"||ee.job2==="DEF"&&Number(ee.note_d)>0?he:0),note_m:(Number(ee.note_m)||0)+(ee.job==="MIL"||ee.job2==="MIL"&&Number(ee.note_m)>0?he:0),note_a:(Number(ee.note_a)||0)+(ee.job==="ATT"||ee.job2==="ATT"&&Number(ee.note_a)>0?he:0),evolution_bonus:he,rarity:ee.rarity,skin:ee.skin,hair:ee.hair,hair_length:ee.hair_length,face:ee.face||null,clubName:ee.club_encoded_name||null,clubLogo:ee.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},P=((k==null?void 0:k.starters)||[]).map(ee=>q(ee)),Y=((S==null?void 0:S.starters)||[]).map(ee=>q(ee)),W=(k==null?void 0:k.formation)||"4-4-2",X=(S==null?void 0:S.formation)||"4-4-2";let re=Gt(P,W),J=Gt(Y,X);const ce=((k==null?void 0:k.subs)||[]).map(ee=>q(ee)),ae=((S==null?void 0:S.subs)||[]).map(ee=>q(ee)),oe=(k==null?void 0:k.stadium_def)||(n?l:null),ue=(S==null?void 0:S.stadium_def)||(n?null:l);return oe&&(re=It(re,oe),Ft(ce,oe)),ue&&(J=It(J,ue),Ft(ae,ue)),{p1Team:re,p2Team:J,p1Subs:ce,p2Subs:ae,p1Formation:W,p2Formation:X,p1Name:(T==null?void 0:T.club_name)||(T==null?void 0:T.pseudo)||"Joueur 1",p2Name:(D==null?void 0:D.club_name)||(D==null?void 0:D.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:n?b:[],gc_p2:n?[]:b,gcCardsFull_p1:n?v:[],gcCardsFull_p2:n?[]:v,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:o||[],lastActionAt:new Date().toISOString()}}let y=h.game_state&&Object.keys(h.game_state).length?h.game_state:null;if(console.log("[PvP] init - amIHome:",n,"gameState exists:",!!y,"match.status:",h.status,"home_id:",h.home_id,"away_id:",h.away_id,"myId:",g.profile.id),!y)if(n){y=await E(),console.log("[PvP] home - gameState built, p1Team keys:",Object.keys((y==null?void 0:y.p1Team)||{}));const{data:k}=await w.from("matches").select("game_state").eq("id",i).single();!(k!=null&&k.game_state)||!Object.keys(k.game_state).length?await w.from("matches").update({game_state:y,turn_user_id:h.home_id}).eq("id",i):y=k.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>',console.log("[PvP] away - waiting for home to write game_state...");for(let k=0;k<30&&!y;k++){await new Promise(S=>setTimeout(S,400));const{data:L}=await w.from("matches").select("game_state").eq("id",i).single();L!=null&&L.game_state&&Object.keys(L.game_state).length&&(y=L.game_state),k%5===0&&console.log("[PvP] away - poll",k,"game_state:",!!(L!=null&&L.game_state))}if(!y){f("Erreur de synchronisation","error"),m("home");return}y.gc_p2=b,y.gcCardsFull_p2=v,await w.from("matches").update({game_state:y}).eq("id",i)}let B=!1,$=null,G=!1;const N=new Set,te=new Set;async function ie(k){var X,re;try{w.removeChannel(Z)}catch{}const L=y[p+"Score"]||0,S=y[x+"Score"]||0;let M,T;k.winner_id?(M=k.winner_id===g.profile.id,T=!1):k.forfeit?(M=L>S,T=!1):(T=L===S,M=L>S);let D="";if(r&&n)try{const{data:J}=await w.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",h.home_id).single(),{data:ce}=await w.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",h.away_id).single();if(J&&ce){const ae=M?1:T?.5:0,oe=1-ae,ue=J.placement_matches<10,fe=ce.placement_matches<10,xe=computeGlicko2(J.mmr,J.mmr_deviation,J.mmr_volatility,ce.mmr,ce.mmr_deviation,ae===1?1:ae===0?0:.5,ue),ee=computeGlicko2(ce.mmr,ce.mmr_deviation,ce.mmr_volatility,J.mmr,J.mmr_deviation,oe===1?1:oe===0?0:.5,fe);await w.rpc("update_mmr_after_ranked",{p_match_id:i,p_winner_id:T?null:M?h.home_id:h.away_id,p_home_id:h.home_id,p_away_id:h.away_id,p_home_delta:xe.delta,p_away_delta:ee.delta,p_home_new_rd:xe.newRd,p_away_new_rd:ee.newRd,p_home_new_vol:xe.newSigma,p_away_new_vol:ee.newSigma});const he=xe.delta,ze=xe.newMmr,Ae=getTier(ze),Oe=he>=0?"+":"",Ge=he>=0?"#4caf50":"#ff6b6b",Pe=getTier(J.mmr);D=`
            <div style="background:rgba(255,255,255,0.08);border-radius:14px;padding:14px 20px;text-align:center;min-width:220px">
              ...
              ${Ae.id!==Pe.id?`<div style="font-size:20px;font-weight:900;color:${Ae.color}">
                    ${ze>J.mmr?"📈":"📉"} ${Pe.emoji} ${Pe.label} → ${Ae.emoji} ${Ae.label}
                  </div>
                  <div style="font-size:13px;color:rgba(255,255,255,0.5);margin-top:4px">${ze>J.mmr?"Promotion !":"Rétrogradation"}</div>`:`<div style="font-size:36px">${he>=0?"📈":"📉"}</div>
            ...
            </div>`}}catch(J){console.error("[Ranked] MMR update error:",J)}(X=document.getElementById("pvp-end-overlay"))==null||X.remove();const q=document.createElement("div");q.id="pvp-end-overlay",q.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;color:#fff;padding:24px;text-align:center;overflow-y:auto";const P=T?"🤝":M?"🏆":"😞",Y=T?"MATCH NUL":M?"VICTOIRE !":"DÉFAITE",W=T?"#fff":M?"#FFD700":"#ff6b6b";q.innerHTML=`
      ...
      ${D}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${r?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(q),(re=q.querySelector("#pvp-end-home"))==null||re.addEventListener("click",()=>{q.remove(),Ne(e),m(r?"ranked":"home")})}const Z=w.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},k=>{const L=k.new;try{if(L.status==="finished"||L.forfeit){if(B)return;B=!0,$&&(clearInterval($),$=null),L.game_state&&(y=L.game_state),ie(L);return}if(L.game_state){const S=y;y=L.game_state;const M=y._lastGC;if(M&&M.seq&&!te.has(M.seq)&&(te.add(M.seq),M.by!==p)){R(M.type,M.by,()=>de());return}const T=S[p+"Score"]||0,D=S[x+"Score"]||0,q=y[p+"Score"]||0,P=y[x+"Score"]||0,Y=q>T,W=P>D;if((Y||W)&&!N.has(y.round)){N.add(y.round);const X=[...y.log||[]].reverse().find(J=>J.isGoal),re=((X==null?void 0:X.homePlayers)||[]).map(J=>({name:J.name,note:J.note,portrait:J.portrait,job:J.job}));Qe(re,q,P,Y,()=>de());return}de()}}catch(S){console.error("[PvP] crash:",S)}}).subscribe();async function C(k){Object.assign(y,k),y.lastActionAt=new Date().toISOString();const{error:L}=await w.from("matches").update({game_state:y}).eq("id",i);L&&f("Erreur de synchronisation","error");try{de()}catch(S){console.error("[PvP] renderPvpScreen crash:",S)}}async function _(){if(B)return;B=!0,$&&(clearInterval($),$=null);const k=n?h.away_id:h.home_id,L=n?"p2":"p1",S=n?"p1":"p2",M={...y,[L+"Score"]:3,[S+"Score"]:0,phase:"finished"};try{await w.from("matches").update({status:"finished",forfeit:!0,winner_id:k,home_score:M.p1Score||0,away_score:M.p2Score||0,game_state:M}).eq("id",i)}catch{}try{w.removeChannel(Z)}catch{}setTimeout(()=>{Ne(e),m("home")},800)}const I={BOOST_STAT:({value:k=1,count:L=1,roles:S=[]},M,T)=>{const D=M[p+"Team"],q=Object.entries(D).filter(([P])=>!S.length||S.includes(P)).flatMap(([P,Y])=>Y.filter(W=>!W.used).map(W=>({...W,_line:P})));if(!q.length){M.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),T(M);return}H(q,L,`Choisir ${L} joueur(s) à booster (+${k})`,P=>{P.forEach(Y=>{const W=(D[Y._line]||[]).find(X=>X.cardId===Y.cardId);W&&(W.boost=(W.boost||0)+k,M.log.push({text:`⚡ +${k} sur ${W.name}`,type:"info"}))}),M[p+"Team"]=D,T(M)})},DEBUFF_STAT:({value:k=1,count:L=1,roles:S=[],target:M="ai"},T,D)=>{const q=M==="home"?p:x,P=T[q+"Team"],Y=M==="home"?"allié":"adverse",W=Object.entries(P).filter(([X])=>!S.length||S.includes(X)).flatMap(([X,re])=>re.map(J=>({...J,_line:X})));if(!W.length){T.log.push({text:`🎯 Aucun joueur ${Y}`,type:"info"}),D(T);return}H(W,L,`Choisir ${L} joueur(s) ${Y}(s) (-${k})`,X=>{X.forEach(re=>{const J=(P[re._line]||[]).find(ce=>ce.cardId===re.cardId);J&&(J.boost=(J.boost||0)-k,T.log.push({text:`🎯 -${k} sur ${J.name}`,type:"info"}))}),T[q+"Team"]=P,D(T)})},GRAY_PLAYER:({count:k=1,roles:L=[],target:S="ai"},M,T)=>{const D=S==="home"?p:x,q=M[D+"Team"],P=S==="home"?"allié":"adverse",Y=Object.entries(q).filter(([W])=>!L.length||L.includes(W)).flatMap(([W,X])=>X.filter(re=>!re.used).map(re=>({...re,_line:W})));if(!Y.length){M.log.push({text:`❌ Aucun joueur ${P}`,type:"info"}),T(M);return}H(Y,k,`Choisir ${k} joueur(s) ${P}(s) à exclure`,W=>{const X="usedCardIds_"+D,re=new Set(M[X]||[]);W.forEach(J=>{const ce=(q[J._line]||[]).find(ae=>ae.cardId===J.cardId);ce&&(ce.used=!0,re.add(J.cardId),M.log.push({text:`❌ ${ce.name} exclu !`,type:"info"}))}),M[X]=[...re],M[D+"Team"]=q,T(M)})},REVIVE_PLAYER:({count:k=1,roles:L=[]},S,M)=>{const T=S[p+"Team"],D=Object.entries(T).filter(([q])=>!L.length||L.includes(q)).flatMap(([q,P])=>P.filter(Y=>Y.used).map(Y=>({...Y,_line:q})));if(!D.length){S.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),M(S);return}H(D,k,`Choisir ${k} joueur(s) à ressusciter`,q=>{q.forEach(P=>{const Y=(T[P._line]||[]).find(W=>W.cardId===P.cardId);Y&&(Y.used=!1,S.log.push({text:`💫 ${Y.name} ressuscité !`,type:"info"}))}),S[p+"Team"]=T,M(S)})},REMOVE_GOAL:({},k,L)=>{const S=x+"Score";k[S]>0?(k[S]--,k.log.push({text:"🚫 Dernier but annulé !",type:"info"})):k.log.push({text:"🚫 Aucun but à annuler",type:"info"}),L(k)},ADD_GOAL_DRAW:({},k,L)=>{k[p+"Score"]===k[x+"Score"]?(k[p+"Score"]++,k.log.push({text:"🎯 But bonus !",type:"info"})):k.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),L(k)},ADD_SUB:({value:k=1},L,S)=>{L.maxSubs=(L.maxSubs||3)+k,L.log.push({text:`🔄 +${k} remplacement(s)`,type:"info"}),S(L)},CUSTOM:({},k,L)=>L(k)};function H(k,L,S,M){const T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let D=[];function q(){var Y,W;const P=k.map(X=>{const re={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[X._line]||"#555",J=Be(X,X._line)+(X.boost||0),ae=D.find(ue=>ue.cardId===X.cardId)?"#FFD700":"rgba(255,255,255,0.25)",oe=X.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${X.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${ae};background:${re};overflow:hidden;cursor:pointer;${oe}">
        ...
        </div>`,(Y=T.querySelector("#pp-close"))==null||Y.addEventListener("click",()=>T.remove()),T.querySelectorAll(".pp-item").forEach(X=>{X.addEventListener("click",()=>{const re=X.dataset.cid,J=k.find(ae=>ae.cardId===re),ce=D.findIndex(ae=>ae.cardId===re);J&&(ce>-1?D.splice(ce,1):D.length<L&&D.push(J),q())})}),(W=T.querySelector("#pp-confirm"))==null||W.addEventListener("click",()=>{T.remove(),M(D)})}q(),document.body.appendChild(T)}async function le(k,L){const M=(y["gcCardsFull_"+p]||[]).find(P=>P.id===k),T=(M==null?void 0:M._gcDef)||(y.gcDefs||[]).find(P=>{var Y;return P.name===L||((Y=P.name)==null?void 0:Y.toLowerCase().trim())===(L==null?void 0:L.toLowerCase().trim())}),D=[...y["usedGc_"+p]||[],k],q={type:L,by:p,seq:(y._gcAnimSeq||0)+1};te.add(q.seq),R(L,p,async()=>{if(T!=null&&T.effect_type&&T.effect_type!=="CUSTOM"){const Y=I[T.effect_type];if(Y){const W={...y};Y(T.effect_params||{},W,async X=>{X["usedGc_"+p]=D,X._lastGC=q,X._gcAnimSeq=q.seq,await C(X)});return}}const P={...y};switch(L){case"Remplacement+":P.maxSubs=(P.maxSubs||3)+1,P.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const Y=x+"Score";P[Y]>0&&(P[Y]--,P.log.push({text:"🚫 But annulé",type:"info"}));break}}P["usedGc_"+p]=D,P._lastGC=q,P._gcAnimSeq=q.seq,await C(P)})}function se(k,L){const S="usedCardIds_"+k,M=new Set(y[S]||[]);L.forEach(T=>M.add(T)),y[S]=[...M]}function pe(){for(const k of["p1","p2"]){const L=new Set(y["usedCardIds_"+k]||[]),S=y[k+"Team"];if(S)for(const M of["GK","DEF","MIL","ATT"])(S[M]||[]).forEach(T=>{T.used=L.has(T.cardId)})}}function de(){var Ye,nt,vi,wi,_i,ki;if(y.phase==="reveal")return Te();if(y.phase==="midfield")return Q();if(y.phase==="finished")return Ke();const k=y[p+"Team"],L=y[x+"Team"];pe();const S=y[p+"Score"],M=y[x+"Score"],T=y[p+"Name"],D=y[x+"Name"],q=y.phase===p+"-attack",P=y.phase===p+"-defense",Y=Array.isArray(y["selected_"+p])?y["selected_"+p]:[],W=Y.map(ne=>ne.cardId),X=window.innerWidth>=700,re=y[p+"Subs"]||[],J=y["usedSubIds_"+p]||[],ce=re.filter(ne=>!J.includes(ne.cardId)),ae=y["gcCardsFull_"+p]||[],oe=y["usedGc_"+p]||[],ue=ae.filter(ne=>!oe.includes(ne.id)),fe=y.boostOwner===p&&!y.boostUsed,xe=!["GK","DEF","MIL","ATT"].some(ne=>(L[ne]||[]).some(ge=>!ge.used)),ee=[...k.MIL||[],...k.ATT||[]].filter(ne=>!ne.used),he=q&&xe&&ee.length===0?[...k.GK||[],...k.DEF||[]].filter(ne=>!ne.used).map(ne=>ne.cardId):[];function ze(ne,ge,Ie){var Ti,zi;const ye=ne._gcDef,ot={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ye==null?void 0:ye.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",lt={purple:"#b06ce0",light_blue:"#00d4ef"}[ye==null?void 0:ye.color]||"#b06ce0",_t=(ye==null?void 0:ye.name)||ne.gc_type,kt=(ye==null?void 0:ye.effect)||((Ti=Me[ne.gc_type])==null?void 0:Ti.desc)||"",$i=ye!=null&&ye.image_url?`/icons/${ye.image_url}`:null,yn=((zi=Me[ne.gc_type])==null?void 0:zi.icon)||"⚡",Ei=Math.round(Ie*.22),Li=Math.round(Ie*.22),Kt=Ie-Ei-Li,bn=_t.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${ne.id}" data-gc-type="${ne.gc_type}"
        ...
        <div style="height:${Ei}px;background:rgba(255,255,255,0.14);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 3px">
          <span style="font-size:${bn}px;font-weight:900;color:#fff;text-align:center;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${ge-6}px">${_t}</span>
          <span style="font-size:6px;color:rgba(255,255,255,0.45)">⚡ GC</span>
        </div>
        <div style="height:${Kt}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
          ${$i?`<img src="${$i}" style="max-width:${ge-10}px;max-height:${Kt-6}px;object-fit:contain">`:`<span style="font-size:${Math.round(Kt*.55)}px">${yn}</span>`}
        </div>
        <div style="height:${Li}px;background:rgba(0,0,0,0.38);display:flex;align-items:center;justify-content:center;padding:0 3px">
      ...
      </div>`}const Oe=(ne,ge)=>ge?Ae(130,175):ze(ne,130,175),Ge=(ne,ge)=>ge?Ae(68,95):ze(ne,68,95),Pe=X?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",wt=q?Ee(p)?`<button id="pvp-action" style="${Pe};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${Y.length===0?"disabled":""}>⚔️ ATTAQUEZ <span id="pvp-timer"></span></button>`:`<button id="pvp-action" data-pass="1" style="${Pe};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER <span id="pvp-timer"></span></button>`:P?`<button id="pvp-action" style="${Pe};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${Y.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="pvp-timer"></span></button>`:`<div style="font-size:11px;color:rgba(255,255,255,0.3);text-align:center;padding:4px">⏳ Tour de ${D}</div>`,je=q&&!Ee(p)?'<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">Aucun attaquant — passez la main</div>':q||P?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${Y.length}/3 sélectionné(s)</div>`:"",He=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${X?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
    ...
    </div>`,Ve=(()=>{var ge,Ie;if(P&&((ge=y.pendingAttack)!=null&&ge.players)){const ye=y.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
        ...
        </div>`}if(q&&((Ie=y.pendingAttack)!=null&&Ie.players)){const ye=y.pendingAttack;return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #00ff88">
          <div style="font-size:8px;color:#00ff88;letter-spacing:2px;margin-bottom:4px;text-transform:uppercase">⚔️ VOUS ATTAQUEZ</div>
          ${ut((ye.players||[]).map(ot=>({...ot,used:!1})),"#00ff88",ye.total)}
        </div>`}const ne=(y.log||[]).slice(-1)[0];return ne?'<div style="padding:2px 4px">'+sn(ne)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})(),it=`
      <div style="display:flex;align-items:center;padding:8px 10px;background:rgba(0,0,0,0.5);gap:6px;flex-shrink:0">
      ...
      </button>`;Ue(e),e.style.overflow="hidden",X?e.innerHTML=`
      <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
        ${it}
        <div style="display:flex;flex:1;min-height:0;overflow:hidden">
          ${He}
      ...
      </div>`;function De(){const ne=e.querySelector(".match-screen");if(!ne)return;const ge=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);ne.style.height=ge+"px",ne.style.minHeight=ge+"px",ne.style.maxHeight=ge+"px",ne.style.overflow="hidden";const Ie=e.querySelector("#mobile-action-bar"),ye=e.querySelector("#mobile-play-area");Ie&&ye&&(ye.style.paddingBottom=Ie.offsetHeight+"px")}if(De(),setTimeout(De,120),setTimeout(De,400),G||(G=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",De),window.visualViewport.addEventListener("scroll",De)),window.addEventListener("resize",De)),function(){const ge=e.querySelector(".terrain-wrapper svg");ge&&(ge.removeAttribute("width"),ge.removeAttribute("height"),ge.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",ge.setAttribute("viewBox","-26 -26 352 352"),ge.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),e.querySelectorAll(".match-slot-hit").forEach(ne=>{ne.addEventListener("click",()=>{if(!q&&!P)return;const ge=ne.dataset.cardId,Ie=ne.dataset.role,ye=(k[Ie]||[]).find(kt=>kt.cardId===ge);if(!ye||ye.used)return;const ot=he.includes(ge);if(q&&!["MIL","ATT"].includes(Ie)&&!ot)return;Array.isArray(y["selected_"+p])||(y["selected_"+p]=[]);const lt=y["selected_"+p],_t=lt.findIndex(kt=>kt.cardId===ge);_t>-1?lt.splice(_t,1):lt.length<3&&lt.push({...ye,_role:Ie}),de()})}),e.querySelectorAll(".match-used-hit").forEach(ne=>{ne.addEventListener("click",()=>F(ne.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(ne=>{ne.addEventListener("click",()=>F())}),(Ye=e.querySelector("#pvp-sub-open"))==null||Ye.addEventListener("click",()=>F()),e.querySelectorAll(".pvp-gc-mini").forEach(ne=>{ne.addEventListener("click",()=>z(ne.dataset.gcId,ne.dataset.gcType))}),(nt=e.querySelector("#pvp-boost-card"))==null||nt.addEventListener("click",()=>A()),(vi=e.querySelector("#pvp-action"))==null||vi.addEventListener("click",ne=>{q?ne.currentTarget.dataset.pass==="1"||!Ee(p)?O():_e():P&&ke()}),(wi=e.querySelector("#pvp-quit"))==null||wi.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&_()}),(_i=e.querySelector("#pvp-view-opp"))==null||_i.addEventListener("click",()=>U()),(ki=e.querySelector("#pvp-toggle-history"))==null||ki.addEventListener("click",()=>j()),$&&(clearInterval($),$=null),(q||P)&&!B){let ne=30,ge=!1;const Ie=()=>document.getElementById("pvp-timer"),ye=()=>{Ie()&&(Ie().textContent=ne+"s",Ie().style.color=ge?"#ff4444":"#fff")};ye(),$=setInterval(()=>{ne--,ne<0?ge?(clearInterval($),$=null,q&&!Ee(p)?O():_()):(ge=!0,ne=15,ye()):ye()},1e3)}}function Te(){Ue(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="width:min(90vw,420px)">${Pt(y[x+"Team"],y[x+"Formation"],null,[],rt(),ft())}</div>
    </div>`,p==="p1"&&setTimeout(async()=>{await C({phase:"midfield"})},5e3)}let K=!1;function Q(){if(K)return;const k=y[p+"Team"].MIL||[],L=y[x+"Team"].MIL||[],S=milScore(k)+milLinks(k),M=milScore(L)+milLinks(L),T=S>=M;function D(ae,oe,ue){return`<div id="duel-row-${ue}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
        ...
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${ae.map((fe,xe)=>{const ee=xe<ae.length-1?vt(fe,ae[xe+1]):null,he=!ee||ee==="#ff3333"||ee==="#cc2222",ze=ee==="#00ff88"?"+2":ee==="#FFD700"?"+1":"";return`<div class="duel-card duel-card-${ue}" data-idx="${xe}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease,transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
              ${Se({...fe,_evolution_bonus:0},{width:window.innerWidth>=900?90:58,showStad:!0,stadDef:y.homeStadiumDef||y.stadiumDef||null,role:"MIL",extraNote:fe.boost||0})}
            </div>
            ${xe<ae.length-1?`<div class="duel-link duel-link-${ue}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${he?"rgba(255,255,255,0.12)":ee};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${he?"none":`0 0 8px ${ee}`}">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(J);let ce=!1;const ae=()=>{ce||(ce=!0,J.remove(),setTimeout(()=>S&&S(),50))};J.addEventListener("click",ae),setTimeout(ae,3e3)}function z(k,L){var J,ce,ae,oe;const M=(y["gcCardsFull_"+p]||[]).find(ue=>ue.id===k),T=M==null?void 0:M._gcDef,D={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[T==null?void 0:T.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",q={purple:"#b06ce0",light_blue:"#00d4ef"}[T==null?void 0:T.color]||"#b06ce0",P=(T==null?void 0:T.name)||L,Y=(T==null?void 0:T.effect)||((J=Me[L])==null?void 0:J.desc)||"Carte spéciale.",W=T!=null&&T.image_url?`/icons/${T.image_url}`:null,X=((ce=Me[L])==null?void 0:ce.icon)||"⚡",re=document.createElement("div");re.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",re.innerHTML=`
      ...
      </div>`,document.body.appendChild(S),(M=S.querySelector("#bp-close"))==null||M.addEventListener("click",()=>S.remove()),S.querySelectorAll(".bp-item").forEach(T=>{T.addEventListener("click",async()=>{const D=T.dataset.cid,q=L.find(Y=>Y.cardId===D);if(!q)return;const P=(k[q._line]||[]).find(Y=>Y.cardId===D);P&&(P.boost=(P.boost||0)+y.boostValue),S.remove(),await C({[p+"Team"]:k,boostUsed:!0})})})}function F(k=null){var ce,ae;if(!(y.phase===p+"-attack")){f("Remplacement uniquement avant votre attaque","warning");return}const S=y[p+"Team"],M=y["usedSubIds_"+p]||[],T=y.maxSubs||3;if(M.length>=T){f(`Maximum ${T} remplacements atteint`,"warning");return}const D=Object.entries(S).flatMap(([oe,ue])=>(ue||[]).filter(fe=>fe.used).map(fe=>({...fe,_line:oe}))),q=(y[p+"Subs"]||[]).filter(oe=>!M.includes(oe.cardId));if(!D.length){f("Aucun joueur utilisé à remplacer","warning");return}if(!q.length){f("Aucun remplaçant disponible","warning");return}let P=Math.max(0,D.findIndex(oe=>oe.cardId===k));const Y=((ce=D[P])==null?void 0:ce._line)||((ae=D[P])==null?void 0:ae.job);let W=Math.max(0,q.findIndex(oe=>oe.job===Y)),X=!1;const re=document.createElement("div");re.id="pvp-sub-overlay",re.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function J(){var ze,Ae,Oe,Ge,Pe,wt;const oe=D[P],ue=q[W],fe=Math.min(130,Math.round((window.innerWidth-90)/2)),xe=Math.round(fe*1.35),ee=je=>`background:rgba(255,255,255,0.12);border:none;color:${je?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${je?"default":"pointer"};flex-shrink:0`;re.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        <div id="pin-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
          ...
          <button id="pin-down" style="${ee(W>=q.length-1)}" ${W>=q.length-1?"disabled":""}>▼</button>
          <div style="font-size:10px;color:rgba(255,255,255,0.35)">${W+1}/${q.length}</div>
        </div>
        <div id="pout-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
          <div style="font-size:9px;color:#ff6b6b;letter-spacing:2px;text-transform:uppercase;font-weight:700">Joueur qui sort</div>
          <button id="pout-up" style="${ee(P===0)}" ${P===0?"disabled":""}>▲</button>
          <div>${oe?Je({...oe,used:!1,boost:0},fe,xe):"<div>—</div>"}</div>
          <button id="pout-down" style="${ee(P>=D.length-1)}" ${P>=D.length-1?"disabled":""}>▼</button>
          <div style="font-size:10px;color:rgba(255,255,255,0.35)">${P+1}/${D.length}</div>
        </div>
      </div>
      <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
        <button id="psub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
      </div>`,(ze=re.querySelector("#psub-close"))==null||ze.addEventListener("click",()=>re.remove()),(Ae=re.querySelector("#pout-up"))==null||Ae.addEventListener("click",()=>{P>0&&(P--,J())}),(Oe=re.querySelector("#pout-down"))==null||Oe.addEventListener("click",()=>{P<D.length-1&&(P++,J())}),(Ge=re.querySelector("#pin-up"))==null||Ge.addEventListener("click",()=>{W>0&&(W--,J())}),(Pe=re.querySelector("#pin-down"))==null||Pe.addEventListener("click",()=>{W<q.length-1&&(W++,J())});const he=(je,He,Re,tt)=>{const Ve=re.querySelector("#"+je);if(!Ve)return;let it=0;Ve.addEventListener("touchstart",De=>{it=De.touches[0].clientY},{passive:!0}),Ve.addEventListener("touchend",De=>{const Ye=De.changedTouches[0].clientY-it;if(Math.abs(Ye)<30)return;const nt=He();Ye<0&&nt<tt-1?(Re(nt+1),J()):Ye>0&&nt>0&&(Re(nt-1),J())},{passive:!0})};he("pin-panel",()=>W,je=>W=je,q.length),he("pout-panel",()=>P,je=>P=je,D.length),(wt=re.querySelector("#psub-confirm"))==null||wt.addEventListener("click",async je=>{if(je.preventDefault(),je.stopPropagation(),X)return;X=!0;const He=D[P],Re=q[W];if(!He||!Re)return;const tt=He._line,Ve=(S[tt]||[]).findIndex(Ye=>Ye.cardId===He.cardId);if(Ve===-1){f("Erreur : joueur introuvable","error"),re.remove();return}const it={...Re,_line:tt,position:He.position,used:!1,boost:0};S[tt].splice(Ve,1,it);const De=[...M,Re.cardId];re.remove(),V(He,Re,async()=>{await C({[p+"Team"]:S,[x+"Team"]:y[x+"Team"],["usedSubIds_"+p]:De})})})}document.body.appendChild(re),J()}function V(k,L,S){const M={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const D=(Y,W,X)=>`<div style="text-align:center">
      ...
      <div style="width:70px;height:70px;border-radius:50%;background:${M[Y.job]||"#555"};border:3px solid ${W};position:relative;overflow:hidden;margin:0 auto">
        ${St(Y)?`<img src="${St(Y)}" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover">`:""}
      </div>
      ...
      </div>`,document.body.appendChild(L),(S=L.querySelector("#pvp-hist-close"))==null||S.addEventListener("click",()=>L.remove())}async function O(){if(y.phase!==p+"-attack")return;const k=p==="p1"?"p2":"p1",L=(y.round||0)+1,S=[...y.log||[]];S.push({type:"info",text:`⏭️ ${y[p+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const M=be(y),T=Ee(k),D=M||!T?"finished":k+"-attack";await C({["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:D,attacker:k,round:L,log:S}),D==="finished"&&await qe(y)}async function _e(){const k=y[p+"Team"],L=!["GK","DEF","MIL","ATT"].some(D=>(y[x+"Team"][D]||[]).some(q=>!q.used)),S=(y["selected_"+p]||[]).map(D=>{const q=(k[D._role]||[]).find(ae=>ae.cardId===D.cardId)||D,P=L&&["GK","DEF"].includes(D._role),Y=k[D._role]||[],W=Y.findIndex(ae=>ae.cardId===D.cardId),X=yt(Y.length),re=W>=0?X[W]:q._col??1,J=y.stadiumDef||y.homeStadiumDef||null,ce=q.stadiumBonus||J&&(J.club_id&&String(q.club_id)===String(J.club_id)||J.country_code&&q.country_code===J.country_code)||!1;return{...q,_line:D._role,_col:re,stadiumBonus:ce,...P?{note_a:Math.max(1,Number(q.note_a)||0)}:{}}});if(!S.length)return;const M=ui(S,y.modifiers[p]||{});se(p,S.map(D=>D.cardId)),S.forEach(D=>{const q=(k[D._role]||[]).find(P=>P.cardId===D.cardId);q&&(q.used=!0)}),y["selected_"+p]=[],de();const T=[...y.log||[]];if(L){const D=(y[p+"Score"]||0)+1,q=S.map(re=>histPlayer(re));T.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:q,homeTotal:M.total,aiTotal:0});const P=(y.round||0)+1,Y=p==="p1"?"p2":"p1",W={...y,[p+"Team"]:k,[p+"Score"]:D},X=be(W);N.add(P),Qe(q,D,y[x+"Score"]||0,!0,async()=>{await C({[p+"Team"]:k,[p+"Score"]:D,["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:X?"finished":Y+"-attack",attacker:Y,round:P,log:T}),X&&await qe({...y,[p+"Score"]:D})});return}T.push({type:"pending",text:`⚔️ ${y[p+"Name"]} attaque (${M.total})`}),await C({[p+"Team"]:k,[x+"Team"]:y[x+"Team"],pendingAttack:{...M,players:S,side:p},["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},phase:x+"-defense",log:T})}async function ke(){const k=y[p+"Team"],L=(y["selected_"+p]||[]).map(oe=>{const ue=(k[oe._role]||[]).find(ze=>ze.cardId===oe.cardId)||oe,fe=k[oe._role]||[],xe=fe.findIndex(ze=>ze.cardId===oe.cardId),ee=yt(fe.length),he=xe>=0?ee[xe]:ue._col??1;return{...ue,_line:oe._role,_col:he}}),S=fi(L,y.modifiers[p]||{});se(p,L.map(oe=>oe.cardId)),L.forEach(oe=>{const ue=(k[oe._role]||[]).find(fe=>fe.cardId===oe.cardId);ue&&(ue.used=!0)}),y["selected_"+p]=[],de();const M=gi(y.pendingAttack.total,S.total,y.modifiers[p]||{}),T=y.pendingAttack.side,D=M==="attack"||(M==null?void 0:M.goal),q=T==="p1"?"p2":"p1",P=(y.round||0)+1,Y=(y.pendingAttack.players||[]).map(oe=>histPlayer(oe)),W=[...y.log||[]];W.push({type:"duel",isGoal:D,homeScored:D&&T===p,text:D?`⚽ BUT de ${y[T+"Name"]} ! (${y.pendingAttack.total} vs ${S.total})`:`✋ Attaque stoppée (${y.pendingAttack.total} vs ${S.total})`,homePlayers:Y,aiPlayers:L.map(oe=>histPlayer(oe)),homeTotal:y.pendingAttack.total,aiTotal:S.total});const X=D?(y[T+"Score"]||0)+1:y[T+"Score"]||0,re={...y,[p+"Team"]:k,[T+"Score"]:X},J=be(re),ce=J?"finished":q+"-attack",ae=async()=>{await C({[p+"Team"]:k,[x+"Team"]:y[x+"Team"],[T+"Score"]:X,["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:ce,attacker:q,round:P,log:W}),(ce==="finished"||J)&&await qe({...y,[T+"Score"]:X})};if(D){const oe=T===p,ue=oe?X:y[p+"Score"]||0,fe=oe?y[x+"Score"]||0:X;N.add(P),Qe(Y,ue,fe,oe,ae)}else await ae()}function ve(k){return["MIL","ATT"].some(L=>(k[L]||[]).some(S=>!S.used))}function me(k){return["GK","DEF","MIL","ATT"].some(L=>(k[L]||[]).some(S=>!S.used))}function $e(k){return me(k)&&!ve(k)}function Ee(k){const L=y[k+"Team"],S=y[(k==="p1"?"p2":"p1")+"Team"];return!!(ve(L)||!me(S)&&$e(L))}function be(k){const L=["MIL","ATT"].some(W=>(k.p1Team[W]||[]).some(X=>!X.used)),S=["MIL","ATT"].some(W=>(k.p2Team[W]||[]).some(X=>!X.used)),M=me(k.p1Team),T=me(k.p2Team);return!L&&!(!T&&M)&&(!S&&!(!M&&T))}function Ce(k){const L=k.p1Score||0,S=k.p2Score||0;return L===S?null:L>S?h.home_id:h.away_id}async function qe(k){try{const L=Ce(k),S=L?h.home_id===L?h.away_id:h.home_id:null;await w.from("matches").update({status:"finished",winner_id:L,home_score:k.p1Score||0,away_score:k.p2Score||0}).eq("id",i),L&&S&&updateEvolutiveCards(L,S).catch(()=>{})}catch(L){console.error("[PvP] finishMatch:",L)}}function Ke(){var T;const k=y[p+"Score"],L=y[x+"Score"],S=k>L,M=k===L;Ue(e),e.innerHTML=`
    ...
    </div>`,(T=document.getElementById("pvp-home"))==null||T.addEventListener("click",()=>{try{w.removeChannel(Z)}catch{}Ne(e),m("home")})}de()}async function ri(e,t,i=!1){var s,l;const{state:n,navigate:a,toast:o}=t,r=i&&((l=(s=t==null?void 0:t.state)==null?void 0:s.params)==null?void 0:l.rankedData)||null;await At(e,t,i?"ranked":"random",async({deckId:c,formation:d,starters:u,subsRaw:g,gcCardsEnriched:m,gcDefs:f,stadiumDef:p})=>{const x=m||[];Ue(e),await No(e,t,c,d,u,g,x,f,p,i,r)})}async function Ro(e,t,i){return Po(e,t,i)}async function No(e,t,i,n,a,o,r=[],s=[],l=null,c=!1,d=null){var ie,Z;const{state:u,navigate:g,toast:m}=t,f=(d==null?void 0:d.mmr)??((ie=u.profile)==null?void 0:ie.mmr)??1e3,p=c?li(f):null,x=c?"linear-gradient(135deg, #1a0a2e 0%, #2d1060 50%, #1a0a2e 100%)":"linear-gradient(135deg, #0a1a2e 0%, #0d3d1e 100%)",b=c?(p==null?void 0:p.color)||"#D4A017":"#FFD700";let v=!1,h=null,E=null;e.innerHTML=`
    ...
    </div>`;const y=async()=>{v=!0,h&&(h.unsubscribe(),h=null),E&&(clearInterval(E),E=null),await w.rpc("cancel_matchmaking",{p_user_id:u.profile.id}).catch(()=>{})};(Z=document.getElementById("cancel-mm"))==null||Z.addEventListener("click",async()=>{await y(),Ne(e),g("home")});const B=async(C,_)=>{v||(v=!0,h&&(h.unsubscribe(),h=null),E&&(clearInterval(E),E=null),await ht(e,t,C,_,{myGC:r,gcDefs:s,stadiumDef:l,isRanked:c,rankedData:d,onMatchEnd:c?Oo:null}))},$=u.user.id,{data:G,error:N}=c?await w.rpc("try_matchmake_ranked",{p_user_id:$,p_deck_id:i,p_mmr:f}):await w.rpc("try_matchmake",{p_user_id:$,p_deck_id:i});if(N||!(G!=null&&G.success)){console.error("[Matchmaking] try_matchmake error:",N||G),m("Erreur de matchmaking","error"),Ne(e),g("home");return}if(G.matched){await B(G.match_id,!1);return}const te=async()=>{if(v)return;const{data:C}=await w.from("matches").select("id, home_id, away_id").or(`home_id.eq.${$},away_id.eq.${$}`).eq("status","in_progress").order("created_at",{ascending:!1}).limit(1).maybeSingle();if(C){const _=C.home_id===$;await B(C.id,_)}};h=w.channel(`mm_${$}`).on("postgres_changes",{event:"INSERT",schema:"public",table:"matches",filter:`home_id=eq.${$}`},te).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`away_id=eq.${$}`},te).subscribe(),E=setInterval(te,3e3),setTimeout(()=>{v||y()},12e4)}async function Oo(e,t){const{state:i,toast:n}=t,{winnerId:a,homeId:o,awayId:r,homeScore:s,awayScore:l,matchId:c}=e,d=i.user.id,u=o===d,g=a===d,{data:m}=await w.from("users").select("mmr, mmr_rd, mmr_v").eq("id",d).single(),{data:f}=await w.from("users").select("mmr, mmr_rd, mmr_v").eq("id",u?r:o).single();if(m&&f){const p=hn(m.mmr,m.mmr_rd,m.mmr_v,f.mmr,f.mmr_rd,g?1:0);await w.from("users").update({mmr:p.mmr,mmr_rd:p.rd,mmr_v:p.v,mmr_wins:g?w.sql`mmr_wins + 1`:void 0,mmr_losses:g?void 0:w.sql`mmr_losses + 1`}).eq("id",d),n(g?`+MMR ↑ ${Math.round(p.mmr-m.mmr)}`:`-MMR ↓ ${Math.round(m.mmr-p.mmr)}`,g?"success":"error",4e3)}}const Ho=Object.freeze(Object.defineProperty({__proto__:null,renderMatchRandom:ri,resumePvpMatch:Ro},Symbol.toStringTag,{value:"Module"}));async function Uo(e,t){const{state:i,navigate:n,toast:a}=t,o=i.params||{},r=o.friendId||null,s=o.friendName||"Ami";await At(e,t,"friend",async({deckId:l,formation:c,starters:d,subsRaw:u,gcCardsEnriched:g,gcDefs:m,stadiumDef:f})=>{const p=g||[];Ue(e),r?await Ko(e,t,l,p,m,f,r,s):await Vo(e,t,l,p,m,f)})}async function Ko(e,t,i,n,a,o,r,s){var v;const{state:l,navigate:c,toast:d}=t,u=l.user.id,{data:g,error:m}=await w.from("friend_match_invites").insert({inviter_id:u,invitee_id:r,status:"pending",deck_id:i}).select("id").single();if(m||!g){console.error("[Friend] Erreur création invitation:",m),d("Impossible de créer l'invitation","error"),c("home");return}e.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="display:flex;background:#fff;border-bottom:1px solid #eee">
      ${x.map(v=>`<button class="ml-tab" data-tab="${v.key}" style="flex:1;padding:10px 4px;border:none;border-bottom:2px solid ${i===v.key?mt:"transparent"};background:none;font-size:12px;font-weight:${i===v.key?"900":"600"};color:${i===v.key?mt:"#888"};cursor:pointer">${v.label}${v.count?` (${v.count})`:""}</button>`).join("")}
    </div>
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:10px">
      ${i==="waiting"?Wo(g,p,a):i==="active"?Xo(m,a):Jo(f,a)}
    </div>
  </div>`,(b=document.getElementById("ml-create-btn"))==null||b.addEventListener("click",()=>Qo(e,t)),e.querySelectorAll(".ml-tab").forEach(v=>v.addEventListener("click",()=>dt(e,t,v.dataset.tab))),e.querySelectorAll("[data-league-id]").forEach(v=>v.addEventListener("click",()=>et(e,t,v.dataset.leagueId))),e.querySelectorAll("[data-join]").forEach(v=>v.addEventListener("click",h=>{h.stopPropagation(),pn(e,t,v.dataset.join,v.dataset.type)})),e.querySelectorAll("[data-delete]").forEach(v=>v.addEventListener("click",h=>{h.stopPropagation(),ai(e,t,v.dataset.delete,v.dataset.name,i)}))}function Ht(e,t,i=!1){var r,s;const n=e.creator_id===t,a=e.pot||0,o=((s=(r=e.mini_league_members)==null?void 0:r[0])==null?void 0:s.count)||0;return`<div data-league-id="${e.id}" style="background:#fff;border-radius:12px;padding:14px 16px;box-shadow:0 1px 6px rgba(0,0,0,0.08);cursor:pointer;margin-bottom:8px">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        ${u.map(C=>`
          <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid #f5f5f5">
            <div style="width:36px;height:36px;border-radius:50%;background:${C.club_color2||mt};display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:900;color:${C.club_color1||"#fff"}">${(C.pseudo||"?").slice(0,2).toUpperCase()}</div>
            <div style="flex:1"><div style="font-size:13px;font-weight:700">${C.club_name||C.pseudo}</div><div style="font-size:11px;color:#999">@${C.pseudo}</div></div>
            ${C.id===r.creator_id?'<span style="font-size:10px;color:#D4A017;font-weight:700">👑</span>':""}
          </div>`).join("")}
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>${g.map((C,_)=>{const I=f>0&&r.status==="finished"?_===0?Math.floor(f*.7):_===1?Math.floor(f*.2):_===2?Math.floor(f*.1):0:0;return`<tr style="border-bottom:1px solid #f5f5f5;${C.userId===o?"background:#f0fdf4;":""}">
              <td style="padding:7px 3px 7px 0;font-weight:700;color:${_===0?Xe:_<3?mt:"#555"}">${["🥇","🥈","🥉"][_]||_+1}</td>
              <td style="padding:7px 3px"><div style="font-weight:700">${C.clubName}</div><div style="font-size:10px;color:#999">@${C.pseudo}</div></td>
              <td style="text-align:center">${C.played}</td><td style="text-align:center">${C.won}-${C.drawn}-${C.lost}</td>
              <td style="text-align:center;color:${C.goalDiff>=0?mt:"#cc2222"}">${C.goalDiff>=0?"+":""}${C.goalDiff}</td>
              <td style="text-align:center;font-weight:900;font-size:14px">${C.points}</td>
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        ${Array.from({length:Math.max(0,r.status==="active"?r.current_day-1:r.current_day)},(C,_)=>_+1).reverse().map(C=>{const _=(l||[]).filter(I=>I.matchday===C);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:#888;margin-bottom:6px">Journée ${C}</div>${_.map(I=>Ri(I,u,o,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
  ...
  </div>`;const h=r.status==="waiting"?"waiting":r.status==="active"?"active":"archived";if((E=document.getElementById("ml-back"))==null||E.addEventListener("click",()=>dt(e,t,h)),(y=document.getElementById("ml-start-btn"))==null||y.addEventListener("click",()=>ir(e,t,r,u)),(B=document.getElementById("ml-next-day"))==null||B.addEventListener("click",()=>nr(e,t,i)),($=document.getElementById("ml-finish-btn"))==null||$.addEventListener("click",()=>un(e,t,i,f,g,u)),(G=document.getElementById("ml-join-now"))==null||G.addEventListener("click",()=>pn(e,t,i,r.type)),(N=document.getElementById("ml-leave-btn"))==null||N.addEventListener("click",()=>er(e,t,i,p)),(te=document.getElementById("ml-delete-btn"))==null||te.addEventListener("click",()=>ai(e,t,i,r.name,"waiting")),(ie=document.getElementById("ml-delete-now"))==null||ie.addEventListener("click",()=>ai(e,t,i,r.name,h)),(Z=document.getElementById("ml-archive-btn"))==null||Z.addEventListener("click",()=>tr(e,t,i)),e.querySelectorAll("[data-play-match]").forEach(C=>{C.addEventListener("click",()=>{const _=m.find(I=>I.id===C.dataset.playMatch);_&&t.navigate("match-mini-league",{mlMatchId:_.id,leagueId:i})})}),r.status==="finished"&&v){const C=g.findIndex(_=>_.userId===o);C>=0&&C<3&&v.prize_amount>0&&setTimeout(()=>or(e,t,r,v,C),400)}}function Ri(e,t,i,n,a=!1){const o=u=>t.find(g=>g.id===u);if(e.is_bye){const u=o(e.home_id);return`<div style="padding:8px;border-radius:8px;background:#f9f9f9;margin-bottom:6px;font-size:12px;color:#888;text-align:center">🔵 ${(u==null?void 0:u.club_name)||(u==null?void 0:u.pseudo)||"?"} exempté(e)</div>`}const r=o(e.home_id),s=o(e.away_id),l=e.home_id===i||e.away_id===i,c=l&&e.status==="pending"&&n&&!a,d=e.status==="finished"?`${e.home_score} - ${e.away_score}`:"vs";return`<div style="display:flex;align-items:center;gap:8px;padding:10px;border-radius:8px;background:${l?"#f0fdf4":"#f9f9f9"};margin-bottom:6px;border:1px solid ${l?"#86efac":"#f0f0f0"}">
    ...
    </div>`,(u=document.getElementById("start-league"))==null||u.addEventListener("click",async()=>{await w.from("mini_leagues").update({status:"in_progress"}).eq("id",i),await gn(e,t,i)}),(g=document.getElementById("join-league"))==null||g.addEventListener("click",async()=>{await At(e,t,"mini_league",async({deckId:f,gcCardsEnriched:p})=>{const x=p||[];await w.from("mini_league_participants").insert({league_id:i,user_id:r,deck_id:f,gc_cards:x.map(b=>b.id)}),o("Rejoint !","success"),await bi(e,t,i)})}),(m=document.getElementById("back-home"))==null||m.addEventListener("click",()=>a("home"))}async function gn(e,t,i){const{navigate:n,toast:a}=t,{data:o}=await w.from("mini_league_matches").select("id, match_id, home_id, away_id, status").eq("league_id",i).eq("status","pending").order("round",{ascending:!0}).limit(1),r=o==null?void 0:o[0];if(!r){await dr(e,t,i);return}const s=t.state.user.id,l=r.home_id===s;await ht(e,t,r.match_id,l,{mlLeagueId:i,mlMatchId:r.id,onMatchEnd:async c=>{await w.from("mini_league_matches").update({status:"finished",home_score:c.homeScore,away_score:c.awayScore,winner_id:c.winnerId}).eq("id",r.id),await gn(e,t,i)}})}async function sr(e,t,i,n){const{data:a}=await w.from("mini_league_matches").select("match_id, home_id, away_id").eq("id",i).single();if(!a){t.toast("Match introuvable","error"),t.navigate("home");return}const o=a.home_id===t.state.user.id;await ht(e,t,a.match_id,o,{mlLeagueId:n,mlMatchId:i,onMatchEnd:async r=>{await w.from("mini_league_matches").update({status:"finished",home_score:r.homeScore,away_score:r.awayScore,winner_id:r.winnerId}).eq("id",i),t.navigate("home")}})}async function dr(e,t,i){var d;const{navigate:n}=t,{data:a}=await w.from("mini_league_matches").select("home_id, away_id, home_score, away_score, winner_id, home:users!home_id(pseudo), away:users!away_id(pseudo)").eq("league_id",i),o={},r={};(a||[]).forEach(u=>{[u.home_id,u.away_id].forEach(g=>{o[g]||(o[g]=0),r[g]||(r[g]=0)}),u.winner_id?o[u.winner_id]=(o[u.winner_id]||0)+3:(o[u.home_id]=(o[u.home_id]||0)+1,o[u.away_id]=(o[u.away_id]||0)+1),r[u.home_id]=(r[u.home_id]||0)+(u.home_score||0),r[u.away_id]=(r[u.away_id]||0)+(u.away_score||0)});const s={};(a||[]).forEach(u=>{var g,m;s[u.home_id]=(g=u.home)==null?void 0:g.pseudo,s[u.away_id]=(m=u.away)==null?void 0:m.pseudo});const l=Object.keys(o).sort((u,g)=>o[g]-o[u]||r[g]-r[u]),c=["🥇","🥈","🥉","4️⃣"];e.innerHTML=`
    <div style="min-height:100%;background:linear-gradient(135deg,#0a1a2e,#0d3d1e);color:#fff;padding:24px;display:flex;flex-direction:column;gap:16px">
      <div style="text-align:center">
        <div style="font-size:36px">🏆</div>
    ...
    </div>`,(d=document.getElementById("back-home"))==null||d.addEventListener("click",()=>n("home"))}const lr="/",cr=[{emoji:"⚽",title:"Bienvenue dans Manager Wars !",color:"#1A6B3C",content:`<p>Tu es désormais un <strong>manager de football</strong> virtuel.</p>
    ...
    <p style="margin-top:12px;font-size:13px;color:#888">Tu peux revoir ce tutoriel depuis les paramètres à tout moment.</p>`}];function pr(e,t,i){let n=0;const a=document.createElement("div");a.id="tutorial-overlay",a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.65);z-index:9900;display:flex;align-items:center;justify-content:center;padding:16px";const o=()=>{var d,u,g;const s=t[n],l=n===t.length-1,c=Math.round((n+1)/t.length*100);a.innerHTML=`
      <div style="background:#fff;border-radius:20px;width:100%;max-width:420px;max-height:90vh;display:flex;flex-direction:column;overflow:hidden;box-shadow:0 16px 64px rgba(0,0,0,0.4)">
        ...
        <!-- Image optionnelle -->
        ${s.image_url?`<div style="padding:0 24px 8px;text-align:center"><img src="${lr}icons/${s.image_url}" style="max-height:160px;max-width:100%;border-radius:12px;object-fit:contain"></div>`:""}
        <!-- Contenu -->
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
  ...
  </div>`;let l="buy";const c=()=>{var p,x,b,v,h,E,y;return{name:(((p=document.getElementById("flt-name"))==null?void 0:p.value)||"").toLowerCase().trim(),club:(((x=document.getElementById("flt-club"))==null?void 0:x.value)||"").toLowerCase().trim(),country:(((b=document.getElementById("flt-country"))==null?void 0:b.value)||"").toLowerCase().trim(),job:((v=document.getElementById("flt-job"))==null?void 0:v.value)||"",rarity:((h=document.getElementById("flt-rarity"))==null?void 0:h.value)||"",note1:parseInt((E=document.getElementById("flt-note1"))==null?void 0:E.value)||0,note2:parseInt((y=document.getElementById("flt-note2"))==null?void 0:y.value)||0}};function d(p){const x=c();return p.filter(b=>{var N,te,ie;const v=(N=b.card)==null?void 0:N.player;if(!v)return!1;const h=`${v.firstname} ${v.surname_real}`.toLowerCase(),E=(((te=v.clubs)==null?void 0:te.encoded_name)||"").toLowerCase(),y=(v.country_code||"").toLowerCase(),B=((ie=b.card)==null?void 0:ie.evolution_bonus)||0,$=ct(v,v.job,B),G=v.job2?ct(v,v.job2,B):0;return!(x.name&&!h.includes(x.name)||x.club&&!E.includes(x.club)||x.country&&!y.includes(x.country)||x.job&&v.job!==x.job||x.rarity&&v.rarity!==x.rarity||x.note1&&$<x.note1||x.note2&&G<x.note2)})}function u(p){var G,N,te,ie;const x=(G=p.card)==null?void 0:G.player;if(!x)return"";const b=((N=p.card)==null?void 0:N.evolution_bonus)||0,v=ct(x,x.job,b),h=x.job2?ct(x,x.job2,b):0,E=(i.profile.credits||0)>=p.price,y=x.country_code?`https://flagsapi.com/${x.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,B=qt[x.job]||"#bbb",$=x.job2?qt[x.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden">
      <!-- Drapeau -->
    ...
    </div>`}function g(p){var G,N,te,ie;const x=(G=p.card)==null?void 0:G.player;if(!x)return"";const b=((N=p.card)==null?void 0:N.evolution_bonus)||0,v=ct(x,x.job,b),h=x.job2?ct(x,x.job2,b):0,E=p.status==="sold",y=x.country_code?`https://flagsapi.com/${x.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,B=qt[x.job]||"#bbb",$=x.job2?qt[x.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${E?"opacity:0.7":""}">
      ${y?`<img src="${y}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      ${(te=x.clubs)!=null&&te.logo_url?`<img src="${x.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${B};display:flex;align-items:center;justify-content:center">
    ...
    </div>`}function m(){const p=document.getElementById("mkt-content"),x=document.getElementById("mkt-filters");if(p){if(x.style.display=l==="buy"?"flex":"none",l==="buy"){const b=d(r);p.innerHTML=b.length?b.map(u).join(""):'<div style="text-align:center;color:#aaa;padding:40px">Aucune carte trouvée.</div>'}else{const b=s.filter(h=>h.status==="active").sort((h,E)=>new Date(E.listed_at)-new Date(h.listed_at)),v=s.filter(h=>h.status==="sold").sort((h,E)=>new Date(E.sold_at||E.listed_at)-new Date(h.sold_at||h.listed_at));p.innerHTML=(b.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:4px 0 6px;text-transform:uppercase;letter-spacing:1px">🟢 En vente (${b.length})</div>`+b.map(g).join(""):"")+(v.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:12px 0 6px;text-transform:uppercase;letter-spacing:1px">✅ Ventes réussies (${v.length})</div>`+v.map(g).join(""):"")+(!b.length&&!v.length?'<div style="text-align:center;color:#aaa;padding:40px">Aucune vente pour le moment.</div>':"")}p.querySelectorAll("[data-buy]").forEach(b=>b.addEventListener("click",()=>gr(b.dataset.buy,r,e,t))),p.querySelectorAll("[data-cancel]").forEach(b=>b.addEventListener("click",()=>xr(b.dataset.cancel,e,t)))}}e.querySelectorAll(".mkt-tab").forEach(p=>{p.addEventListener("click",()=>{l=p.dataset.tab,e.querySelectorAll(".mkt-tab").forEach(x=>{const b=x===p;x.style.background=b?"var(--green)":"#fff",x.style.color=b?"#fff":"var(--gray-600)",x.style.borderColor=b?"var(--green)":"var(--gray-200)"}),m()})});let f;["flt-name","flt-club","flt-country","flt-job","flt-rarity","flt-note1","flt-note2"].forEach(p=>{var x;(x=document.getElementById(p))==null||x.addEventListener("input",()=>{clearTimeout(f),f=setTimeout(m,250)})}),m()}async function gr(e,t,i,n){const{state:a,toast:o,refreshProfile:r}=n,s=t.find(d=>d.id===e);if(!s)return;const l=s.price;if((a.profile.credits||0)<l){o("Crédits insuffisants","error");return}mr(s,async()=>{const{error:d}=await w.rpc("buy_market_card",{p_listing_id:e,p_buyer_id:a.profile.id});if(d){o("Erreur achat : "+d.message,"error");return}await r();const u=document.getElementById("nav-credits");u&&(u.textContent=`💰 ${(a.profile.credits||0).toLocaleString("fr")}`),o("✅ Carte achetée !","success"),await hi(i,n)})}function mr(e,t){var r;const i=(r=e.card)==null?void 0:r.player,n=i?`${i.firstname} ${i.surname_real}`:"cette carte",a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",a.innerHTML=`
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center">
    ...
    </div>`,document.body.appendChild(a);const o=s=>{a.remove(),s&&t()};a.querySelector("#buy-cancel").addEventListener("click",()=>o(!1)),a.querySelector("#buy-ok").addEventListener("click",()=>o(!0)),a.addEventListener("click",s=>{s.target===a&&o(!1)})}async function xr(e,t,i){const{toast:n}=i,{data:a}=await w.from("market_listings").select("card_id").eq("id",e).single();if(await w.from("market_listings").update({status:"cancelled"}).eq("id",e),a!=null&&a.card_id){const{count:o}=await w.from("market_listings").select("id",{count:"exact",head:!0}).eq("card_id",a.card_id).eq("status","active");o||await w.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",a.card_id)}n("Annonce retirée","success"),hi(t,i)}async function yr(e,t){var d,u,g,m;const{state:i,navigate:n}=t,a=((u=(d=t==null?void 0:t.state)==null?void 0:d.params)==null?void 0:u.tab)||"global";e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const[{data:o},{data:r}]=await Promise.all([w.from("users").select("id,pseudo,club_name,trophies_top1,trophies_top2,trophies_top3,wins,level").order("trophies_top1",{ascending:!1}).limit(100),w.from("users").select("id,pseudo,club_name,mmr,rank_tier,ranked_wins,ranked_losses,ranked_draws,placement_matches").gte("placement_matches",1).order("mmr",{ascending:!1}).limit(100)]);let s=a;function l(){var p,x;const f=document.getElementById("rankings-list");if(f){if(s==="global"){const b=o||[];f.innerHTML=b.length>0?b.map((v,h)=>`
      ...
      `).join(""):'<div style="text-align:center;color:var(--gray-600);padding:40px">Aucun manager.</div>'}else{const b=r||[];f.innerHTML=b.length>0?b.map((v,h)=>{const E=li(v.mmr??1e3),y=(v.ranked_wins||0)+(v.ranked_losses||0)+(v.ranked_draws||0),B=y>0?Math.round((v.ranked_wins||0)/y*100):0,$=v.id===i.profile.id,G=(v.placement_matches||0)<10;return`
          <div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${$?"border:2px solid var(--yellow)":""}">
            ...
            <div style="text-align:right;flex-shrink:0">
              <div style="font-size:20px">${G?"❓":E.emoji}</div>
              <div style="font-size:11px;font-weight:700;color:${E.color}">${G?"Placement":E.label}</div>
  ...
  <div class="page">
    ...
    </div>`,(B=document.getElementById("ranked-back"))==null||B.addEventListener("click",()=>n("home"));return}const l=s.mmr??1e3,c=s.mmr_deviation??350,d=s.mmr_volatility??.06,u=s.placement_matches??0,g=u<10,m=Math.max(0,10-u),f=li(l),p=vn(l),x=Vt.findIndex(te=>te.id===f.id),b=Vt[x+1]||null,v={bronze:"linear-gradient(160deg,#3d1c00,#7a3e00)",silver:"linear-gradient(160deg,#1a1a2e,#3a3a5e)",gold:"linear-gradient(160deg,#1a1200,#4a3500)",platinum:"linear-gradient(160deg,#001a20,#003040)",diamond:"linear-gradient(160deg,#001030,#1a2860)",master:"linear-gradient(160deg,#1a0030,#3d0070)"},h=(s.ranked_wins||0)+(s.ranked_losses||0)+(s.ranked_draws||0),E=h>0?Math.round((s.ranked_wins||0)/h*100):0,y=Vt.map(te=>`
  ...
  </div>`}wn(kn);const we={user:null,profile:null,page:"home",params:{}};function pt(e,t="info",i=3e3){const n=document.getElementById("toast");n&&(n.textContent=e,n.className=`show ${t}`,clearTimeout(n._t),n._t=setTimeout(()=>{n.className=""},i))}function vr(e,t,i=""){document.getElementById("modal-title").textContent=e,document.getElementById("modal-body").innerHTML=t,document.getElementById("modal-footer").innerHTML=i,document.getElementById("modal-overlay").classList.remove("hidden")}function si(){document.getElementById("modal-overlay").classList.add("hidden")}async function Et(){if(!we.user)return;const{data:e}=await w.from("users").select("*").eq("id",we.user.id).single();e&&(we.profile=e)}function Lt(e,t={}){we.page=e,we.params=t,mn()}async function mn(){var n,a,o,r;const e=document.getElementById("page-content");if(!e)return;document.querySelectorAll(".bottom-nav a").forEach(s=>{s.classList.toggle("active",s.dataset.page===we.page)});const t=document.getElementById("nav-credits");t&&we.profile&&(t.textContent=`💰 ${(we.profile.credits||0).toLocaleString("fr")}`);const i={state:we,navigate:Lt,toast:pt,openModal:vr,closeModal:si,refreshProfile:Et};switch(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽</div>',we.page){case"home":await Ai(e,i);break;case"collection":await Wn(e,i);break;case"decks":await Zt(e,i);break;case"boosters":await so(e,i);break;case"ranked":await br(e,i);break;case"match":{const s=we.params&&we.params.matchMode||"vs_ai_easy";s==="random"?await ri(e,i,!1):s==="ranked"?await ri(e,i,!0):s==="friend"?await Uo(e,i,(n=we.params)==null?void 0:n.friendId,(a=we.params)==null?void 0:a.friendName):s==="mini-league"?await Ni(e,i,(o=we.params)==null?void 0:o.mlMatchId,(r=we.params)==null?void 0:r.leagueId):await $o(e,i);break}case"market":await fr(e,i);break;case"rankings":await yr(e,i);break;case"matches":await hr(e,i);break;case"friends":await In(e,i);break;case"mini-league":await Yo(e,i);break;case"match-mini-league":{const s=we.params||{};await Ni(e,i,s.mlMatchId,s.leagueId);break}default:await Ai(e,i)}}function wr(){var n;const e=document.getElementById("app"),t=we.profile;if(!t)return;const i="/icons/";e.innerHTML=`
    <nav class="top-nav">
  ...
  `,document.querySelectorAll(".bottom-nav a").forEach(a=>{a.addEventListener("click",o=>{o.preventDefault(),Lt(a.dataset.page)})}),document.getElementById("nav-logo").addEventListener("click",()=>Lt("home")),document.getElementById("nav-credits").addEventListener("click",()=>Lt("boosters")),(n=document.getElementById("journal-btn"))==null||n.addEventListener("click",()=>_r())}async function _r(){const{data:e}=await w.from("patch_notes").select("*").eq("is_published",!0).order("published_at",{ascending:!1}).limit(20),t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9000;display:flex;align-items:center;justify-content:center;padding:16px";const i=(e||[]).map(n=>{const a=new Date(n.published_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"long",year:"numeric"});return`<div style="padding:14px 0;border-bottom:1px solid #f0f0f0">
    ...
    </div>`,document.body.appendChild(t),t.querySelector("#journal-close").addEventListener("click",()=>t.remove()),t.addEventListener("click",n=>{n.target===t&&t.remove()})}async function kr(){document.documentElement.setAttribute("data-theme","light"),document.getElementById("modal-overlay").addEventListener("click",i=>{i.target===i.currentTarget&&si()}),document.getElementById("modal-close").addEventListener("click",si);const{data:{session:e}}=await w.auth.getSession();if(!e){Oi(),Si(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:pt});return}we.user=e.user,await Et(),Oi();try{const{data:i}=await w.from("formation_links_overrides").select("formation, links"),n={};(i||[]).forEach(a=>{n[a.formation]=a.links}),_n(n)}catch(i){console.warn("Impossible de charger les overrides de formation:",i)}if(!we.profile){En(document.getElementById("app"),{state:we,navigate:async()=>{await Et(),Qt()},toast:pt,refreshProfile:Et});return}const t=Array.isArray(we.profile.pending_boosters)?we.profile.pending_boosters:[];if(!we.profile.onboarding_done&&t.length>0){xo(document.getElementById("app"),{state:we,navigate:()=>Qt(),toast:pt,refreshProfile:Et});return}Qt(),setTimeout(()=>ur(we.profile,Lt,pt),800),w.auth.onAuthStateChange(async(i,n)=>{i==="SIGNED_OUT"&&(we.user=null,we.profile=null,document.getElementById("app").innerHTML="",Si(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:pt}))})}function $r(){return Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight)}function Ut(){const e=document.getElementById("app");e&&(e.style.height=$r()+"px")}window.addEventListener("resize",Ut);window.addEventListener("orientationchange",()=>setTimeout(Ut,150));window.visualViewport&&window.visualViewport.addEventListener("resize",Ut);function Qt(){const e=()=>{var i;(i=we.user)!=null&&i.id&&w.from("users").update({last_seen_at:new Date().toISOString()}).eq("id",we.user.id).then(()=>{})};e(),window._presencePingInterval&&clearInterval(window._presencePingInterval),window._presencePingInterval=setInterval(e,6e4);const t=document.getElementById("app");t.style.display="flex",t.style.flexDirection="column",Ut(),wr(),mn()}function Oi(){const e=document.getElementById("app-loader"),t=document.getElementById("app");t&&(t.style.display=""),e&&(e.classList.add("zoom-out"),setTimeout(()=>e.style.display="none",500))}function xn(e){var n;const t=document.getElementById("app-loader");if(t&&(t.style.display="none"),document.getElementById("boot-error"))return;const i=document.createElement("div");i.id="boot-error",i.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:99999;gap:16px;color:#fff;padding:24px;text-align:center",i.innerHTML=`
    <div style="font-size:42px">📡</div>
    <div style="font-size:18px;font-weight:900">Connexion impossible</div>
    <div style="font-size:13px;color:rgba(255,255,255,0.6);max-width:280px">${e||"Le chargement a échoué. Vérifie ta connexion et réessaie."}</div>
    <button id="boot-retry" style="margin-top:8px;padding:12px 30px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer">Réessayer</button>`,document.body.appendChild(i),(n=document.getElementById("boot-retry"))==null||n.addEventListener("click",()=>window.location.reload())}kr().catch(e=>{console.error("Échec du démarrage:",e),xn()});setTimeout(()=>{const e=document.getElementById("app-loader");e&&e.style.display!=="none"&&!e.classList.contains("zoom-out")&&!document.getElementById("boot-error")&&xn("Le serveur met trop de temps à répondre.")},12e3);
