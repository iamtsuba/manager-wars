import{s as k,l as de,F as Re,b as me,c as dt,d as zt}from"./formation-links-CDBKB_z4.js";function We(e,{navigate:t,toast:i}){e.innerHTML=`
  ...
  `,e.querySelectorAll(".auth-tab").forEach(r=>{r.addEventListener("click",()=>{e.querySelectorAll(".auth-tab").forEach(c=>c.classList.remove("active")),r.classList.add("active"),document.getElementById("tab-login").style.display=r.dataset.tab==="login"?"block":"none",document.getElementById("tab-register").style.display=r.dataset.tab==="register"?"block":"none"})}),document.getElementById("login-btn").addEventListener("click",async()=>{const r=document.getElementById("login-email").value.trim(),c=document.getElementById("login-password").value,o=document.getElementById("login-error");if(o.textContent="",!r||!c){o.textContent="Remplissez tous les champs.";return}const n=document.getElementById("login-btn");n.textContent="Connexion…",n.disabled=!0;const{error:l}=await k.auth.signInWithPassword({email:r,password:c});if(n.textContent="Se connecter",n.disabled=!1,l){o.textContent=l.message.includes("Invalid")?"Email ou mot de passe incorrect.":l.message;return}window.location.reload()}),document.getElementById("login-password").addEventListener("keydown",r=>{r.key==="Enter"&&document.getElementById("login-btn").click()}),document.getElementById("reg-btn").addEventListener("click",async()=>{const r=document.getElementById("reg-email").value.trim(),c=document.getElementById("reg-password").value,o=document.getElementById("reg-confirm").value,n=document.getElementById("reg-error");if(n.textContent="",!r||!c||!o){n.textContent="Remplissez tous les champs.";return}if(c.length<6){n.textContent="Mot de passe trop court (min. 6 caractères).";return}if(c!==o){n.textContent="Les mots de passe ne correspondent pas.";return}const l=document.getElementById("reg-btn");l.textContent="Création…",l.disabled=!0;const{error:a}=await k.auth.signUp({email:r,password:c});if(l.textContent="Créer mon compte",l.disabled=!1,a){n.textContent=a.message;return}i("Compte créé ! Connectez-vous.","success",4e3),document.querySelector('[data-tab="login"]').click(),document.getElementById("login-email").value=r})}function Ct(e,{state:t,navigate:i,toast:r,refreshProfile:c}){let o="#1A6B3C",n="#D4A017";e.innerHTML=`
    ...
    <div class="page-body">
      ...
      <div>
        ...
        <div class="card-panel" style="padding:0">
          ${o.map(a=>{const d=a.winner_id===c.id,s=a.home_score===a.away_score,p=s?"N":d?"V":"D",u=s?"#888":d?"#1A6B3C":"#c0392b",f={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat"}[a.mode]||a.mode,x=new Date(a.created_at).toLocaleDateString("fr",{month:"short",day:"numeric"});return`<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 14px;border-bottom:1px solid var(--gray-200)">
              <div>
  ...
  `,e.querySelectorAll("[data-nav]").forEach(a=>{a.addEventListener("click",d=>{d.preventDefault(),i(a.dataset.nav)})}),(n=document.getElementById("nav-rankings"))==null||n.addEventListener("click",()=>i("rankings")),(l=document.getElementById("nav-matches"))==null||l.addEventListener("click",()=>i("matches")),e.querySelectorAll("[data-action]").forEach(a=>{a.addEventListener("click",()=>{if(a.classList.add("tapped"),setTimeout(()=>a.classList.remove("tapped"),200),a.dataset.action==="match-ai"){Bt(i);return}r("Bientôt disponible","info")})}),document.getElementById("logout-btn").addEventListener("click",async()=>{await k.auth.signOut(),window.location.reload()})}function Bt(e){const t=[{mode:"vs_ai_easy",label:"Facile",sub:"Gain 500 cr.",icon:"🟢"},{mode:"vs_ai_medium",label:"Moyen",sub:"Gain 1 000 cr.",icon:"🟡"},{mode:"vs_ai_hard",label:"Difficile",sub:"Gain 1 500 cr.",icon:"🟠"},{mode:"vs_ai_club",label:"Club",sub:"Gain 2 500 cr.",icon:"🔴"}],i=document.createElement("div");i.className="modal-overlay",i.style.zIndex="2000",i.innerHTML=`
  ...
  `,document.body.appendChild(i);const r=()=>i.remove();document.getElementById("diff-cancel").addEventListener("click",r),i.addEventListener("click",c=>{c.target===i&&r()}),i.querySelectorAll("[data-mode]").forEach(c=>{c.addEventListener("click",()=>{r(),e("match",{matchMode:c.dataset.mode})})})}const le={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé pour ce match."},"Double attaque":{icon:"⚡",desc:"La note d'attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la note d'un joueur adverse."},Gel:{icon:"❄️",desc:"Bloque le meilleur attaquant IA."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function U(e,t){if(!e)return 0;switch(t){case"GK":return Number(e.note_g)||0;case"DEF":return Number(e.note_d)||0;case"MIL":return Number(e.note_m)||0;case"ATT":return Number(e.note_a)||0;default:return 0}}function lt(e){let t=0;for(let i=0;i<e.length-1;i++){const r=e[i],c=e[i+1];if(!r||!c)continue;const o=de(r,c);o==="#00ff88"?t+=2:o==="#FFD700"&&(t+=1)}return t}function ct(e,t={}){const i=e.reduce((o,n)=>{const l=n._line||n.job;return o+(Number(l==="MIL"?n.note_m:n.note_a)||0)+(Number(n.boost)||0)},0),r=lt(e);let c=i+r;return t.doubleAttack&&(c*=2),t.stolenNote&&(c-=t.stolenNote),{base:i,links:r,total:Math.max(0,c)}}function pt(e,t={}){const i=e.reduce((o,n)=>{const l=n._line||n.job;let a=0;return l==="GK"?a=Number(n.note_g)||0:l==="MIL"?a=Number(n.note_m)||0:a=Number(n.note_d)||0,o+a+(Number(n.boost)||0)},0),r=lt(e);let c=i+r;return t.stolenNote&&(c-=t.stolenNote),{base:i,links:r,total:Math.max(0,c)}}function ut(e,t,i={}){return i.shield?{goal:!1,shielded:!0}:{goal:e>t,shielded:!1}}function ft(e,t,i="easy"){const r=e.filter(n=>!n.used);if(!r.length)return[];const c=[...r].sort((n,l)=>{const a=t==="attack"?U(n,"ATT"):n._line==="GK"?U(n,"GK"):U(n,"DEF");return(t==="attack"?U(l,"ATT"):l._line==="GK"?U(l,"GK"):U(l,"DEF"))-a});let o=i==="easy"?1+Math.floor(Math.random()*2):i==="medium"?2+Math.floor(Math.random()*2):3;return c.slice(0,Math.min(o,c.length,3))}function St(e,t){const i={vs_ai_easy:{victoire:500,nul:250,defaite:50},vs_ai_medium:{victoire:1e3,nul:500,defaite:50},vs_ai_hard:{victoire:1500,nul:750,defaite:100},vs_ai_club:{victoire:2500,nul:1250,defaite:100}};return(i[e]||i.vs_ai_easy)[t]||0}const gt={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"},Ee={GK:"#111111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},be=["GK","DEF","MIL","ATT"],Ft=["Tous","GK","DEF","MIL","ATT"],Dt={normal:1e3,pepite:5e3,papyte:5e3,legende:1e4},qe={MA:"MAROC",FR:"FRANCE",AR:"ARGENTINE",PT:"PORTUGAL",BR:"BRESIL",ES:"ESPAGNE",DE:"ALLEMAGNE",GB:"ANGLETERRE",IT:"ITALIE",CM:"CAMEROUN",SN:"SENEGAL",NG:"NIGERIA",DK:"DANEMARK",NL:"PAYS-BAS",BE:"BELGIQUE",CI:"CÔTE D'IVOIRE",AL:"ALBANIE",HR:"CROATIE",RS:"SERBIE",TR:"TURQUIE"};function xt(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";if(!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function xe(e,t){return e&&Number(t==="GK"?e.note_g:t==="DEF"?e.note_d:t==="MIL"?e.note_m:e.note_a)||0}function Me(e,t=""){var p,u;const i=e.player;if(!i)return"";const r=i.job||"ATT",c=Ee[r],o=gt[i.rarity]||"#ccc",n=xe(i,r),l=i.job2?xe(i,i.job2):null,a=i.job2?Ee[i.job2]:null,d=xt(i),s=qe[i.country_code]||i.country_code||"";return`
  <div style="
  ...
  </div>`}function Qe(e){const t=e.job||"ATT",i=xe(e,t),r=qe[e.country_code]||e.country_code||"";return`
  <div style="
    width:140px;border-radius:12px;padding:6px;
  ...
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{h=!h,B(),S()})))}function S(){const E=document.getElementById("col-grid");E&&(y==="player"?P(E):y==="formation"?ie(E):oe(E))}function q(E,A,j,C,M){M=M||"#7a28b8";const F=document.getElementById("col-grid"),z=document.getElementById("col-big");if(!F||!z)return;var H=0;function N(){z.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+A(E[H])+"</div>";var O=z.querySelector("[data-card-id],[data-form-id],[data-gc-id]");O&&O.addEventListener("click",function(){C(E[H])}),requestAnimationFrame(function(){var G=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!G||!z)){var X=z.clientHeight-8,Q=z.clientWidth-24,Z=G.offsetHeight,ee=G.offsetWidth;if(Z>0&&ee>0&&X>40){var ne=Math.min(X/Z,Q/ee,1);G.style.transform="scale("+ne.toFixed(3)+")",G.style.transformOrigin="top center"}}}),F.innerHTML=E.map(function(G,X){return'<div class="col-mini-item" data-idx="'+X+'" style="flex-shrink:0;cursor:pointer;border-radius:8px;border:1.5px solid '+(X===H?M:"transparent")+';transition:border-color .15s;overflow:hidden">'+j(G,X===H)+"</div>"}).join(""),F.querySelectorAll(".col-mini-item").forEach(function(G){G.addEventListener("click",function(){H=Number(G.dataset.idx),N(),G.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}N()}function K(E){var A=.54,j=Math.round(140*A),C=Math.round(310*A),M;if(!E||E._fake){var F=E?E.player:null;if(!F)return"";M=Qe(F)}else M=Me(E,"");return'<div style="width:'+j+"px;height:"+C+'px;overflow:hidden;position:relative;flex-shrink:0"><div style="transform:scale('+A+');transform-origin:top left;position:absolute;top:0;left:0;pointer-events:none">'+M+"</div></div>"}function D(E,A,j){A=A||100,j=j||140;var C=me[E]||{},M={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},F=Math.max(3,Math.round(A*.06)),z=Object.entries(C).map(function(N){var O=N[0],G=N[1],X=O.replace(/\d+$/,""),Q=M[X]||"#888",Z=Math.round(G.x*A),ee=Math.round(G.y*j);return'<circle cx="'+Z+'" cy="'+ee+'" r="'+F+'" fill="'+Q+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),H=Math.max(1,Math.round(A/50));return'<svg viewBox="0 0 '+A+" "+j+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+A+'" height="'+j+'" fill="#1A6B3C"/><rect x="'+Math.round(A*.2)+'" y="'+Math.round(j*.02)+'" width="'+Math.round(A*.6)+'" height="'+Math.round(j*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+H+'"/><line x1="0" y1="'+Math.round(j*.5)+'" x2="'+A+'" y2="'+Math.round(j*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+H+'"/><ellipse cx="'+Math.round(A*.5)+'" cy="'+Math.round(j*.5)+'" rx="'+Math.round(A*.18)+'" ry="'+Math.round(j*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+H+'"/><rect x="'+Math.round(A*.2)+'" y="'+Math.round(j*.82)+'" width="'+Math.round(A*.6)+'" height="'+Math.round(j*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+H+'"/>'+z+"</svg>"}function W(E,A,j){var C=j>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+j+"</div>":"",M=A?'data-form-id="'+A.id+'"':"",F=E.length>7?14:E.length>5?16:19,z=!!A;return"<div "+M+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(z?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(z?"":"filter:grayscale(1);opacity:0.5")+'">'+C+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(z?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+F+"px;font-weight:900;color:"+(z?"#1A6B3C":"#aaa")+';line-height:1">'+E+'</div></div><div style="flex:1;overflow:hidden;background:'+(z?"#1A6B3C":"#ccc")+'">'+D(E,140,220)+"</div></div>"}function Y(E,A){var j=.54,C=Math.round(140*j),M=Math.round(305*j),F=Math.round(M*.22),z=M-F,H=E.length>7?5:7,N=D(E,C,z),O=A?"1.5px solid #2a7a40":"1px solid #ddd",G=A?"":"filter:grayscale(1);opacity:0.45;",X=A?"#1A6B3C":"#bbb",Q="#fff";return'<div style="width:'+C+"px;height:"+M+"px;border-radius:6px;border:"+O+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+G+'"><div style="height:'+F+"px;background:"+X+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+H+"px;font-weight:900;color:"+Q+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(C-4)+'px">'+E+'</span></div><div style="height:'+z+'px;overflow:hidden;flex-shrink:0">'+N+"</div></div>"}function P(E){if(h){const A=T();if(!A.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const j=A.map(C=>d.find(M=>M.player.id===C.id)||{_fake:!0,player:C,id:"fake-"+C.id});q(j,C=>C._fake?Qe(C.player):Me(C,""),C=>C._fake?K({player:C.player,id:"x",_fake:!0}):K(C),C=>{C._fake||Ze(C,d,m,t)},"#1A6B3C")}else{const A=I();if(!A.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const j={},C=[];A.forEach(M=>{j[M.player.id]||(j[M.player.id]=!0,C.push(M))}),q(C,M=>{const F=m[M.player.id]||1,z=F>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${F}</div>`:"",N=d.filter(O=>O.player.id===M.player.id&&O.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Me(M,z+N)},M=>K(M),M=>Ze(M,d,m,t),"#1A6B3C")}}function ie(E){const A=h?g:[..._];if(!A.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const j=A.map(C=>({formation:C,card:p.find(M=>M.formation===C)||null,owned:_.has(C)}));q(j,({formation:C,card:M,owned:F})=>W(C,F?M:null,F?p.filter(z=>z.formation===C).length:0),({formation:C,owned:M})=>Y(C,M),({card:C,owned:M})=>{M&&C&&qt(C,p,t,o)},"#1A6B3C")}function oe(E){const A=Object.keys(f),j=h?A.length?A:x:[...b];if(!j.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const C=j.map(M=>({type:M,gc:le[M]||{icon:"⚡",desc:""},def:f[M]||null,owned:b.has(M),card:s.find(F=>F.gc_type===M)||null}));q(C,({type:M,gc:F,def:z,owned:H,card:N})=>{const O=H?s.filter(Mt=>Mt.gc_type===M).length:0,G=O>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${O}</div>`:"",X=(z==null?void 0:z.gc_type)==="ultra_game_changer",Q={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},Z={purple:"#b06ce0",light_blue:"#00d4ef"},ee=Q[z==null?void 0:z.color]||Q.purple,ne=Z[z==null?void 0:z.color]||Z.purple,he=(z==null?void 0:z.effect)||F.desc||"",Ye=z!=null&&z.image_url?`/manager-wars/icons/${z.image_url}`:null;return H&&N?`<div data-gc-id="${N.id}" data-gc-type="${M}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${ne};background:${ee};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${ne}66;cursor:pointer">
          ${G}
        ...
        </div>`},({type:M,gc:F,def:z,owned:H})=>H?(()=>{const N=Math.round(75.60000000000001),O=Math.round(310*.54),G=Math.round(O*.65),X=Math.round(O*.18),Q={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},Z={purple:"#9b59b6",light_blue:"#00bcd4"},ee=Q[z==null?void 0:z.color]||Q.purple,ne=Z[z==null?void 0:z.color]||Z.purple,he=z!=null&&z.image_url?`/manager-wars/icons/${z.image_url}`:null;return`<div style="width:${N}px;height:${O}px;border-radius:8px;background:${ee};border:1px solid ${ne};display:flex;flex-direction:column;overflow:hidden"><div style="height:${X}px;background:rgba(255,255,255,0.15);display:flex;align-items:center;justify-content:center"><span style="font-size:7px;font-weight:900;color:#fff;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${N-6}px">${M}</span></div><div style="height:${G}px;display:flex;align-items:center;justify-content:center">${he?`<img src="${he}" style="max-width:${N-8}px;max-height:${G-4}px;object-fit:contain">`:`<span style="font-size:24px">${F.icon}</span>`}</div><div style="flex:1;display:flex;align-items:center;justify-content:center"><span style="font-size:7px;color:rgba(255,255,255,0.7);text-align:center;padding:0 2px">${((z==null?void 0:z.effect)||F.desc||"").slice(0,20)}</span></div></div>`})():(()=>{const N=Math.round(75.60000000000001),O=Math.round(310*.54);return`<div style="width:${N}px;height:${O}px;border-radius:8px;background:#eee;border:1px solid #ddd;display:flex;flex-direction:column;align-items:center;justify-content:center;filter:grayscale(1);opacity:0.45"><span style="font-size:22px">${F.icon}</span><span style="font-size:7px;color:#aaa;margin-top:4px;text-align:center;padding:0 4px">${M}</span></div>`})(),({type:M,owned:F})=>{F&&Rt(M,o)},"#7a28b8")}e.querySelectorAll(".col-tab-btn").forEach(E=>{E.addEventListener("click",()=>{y=E.dataset.tab,w="Tous",L="",h=!1,e.querySelectorAll(".col-tab-btn").forEach(A=>{const j=A.dataset.tab===y;A.style.borderBottomColor=j?"var(--green)":"transparent",A.style.color=j?"var(--green)":"var(--gray-600)"}),B(),S()})}),B(),S()}function Rt(e,t){const i=le[e]||{icon:"⚡",desc:"Effet spécial."};t("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(x=document.getElementById("direct-sell-form-btn"))==null||x.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${d} pour ${fe.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const b=u.find(w=>!w.is_for_sale)||u[0];if(!b){o("Aucune carte à vendre","error");return}const{error:y}=await k.from("cards").delete().eq("id",b.id);if(y){o(y.message,"error");return}await k.from("users").update({credits:(c.profile.credits||0)+fe}).eq("id",c.profile.id),await a(),o(`+${fe.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),n(),l("collection")}),(m=document.getElementById("market-sell-form-btn"))==null||m.addEventListener("click",async()=>{const b=parseInt(document.getElementById("sell-price-form").value);if(!b||b<1){o("Prix invalide","error");return}await k.from("cards").update({is_for_sale:!0,sale_price:b}).eq("id",e.id),await k.from("market_listings").insert({seller_id:c.profile.id,card_id:e.id,price:b}),o("Carte mise en vente sur le marché !","success"),n(),l("collection")}),(_=document.getElementById("cancel-sell-form-btn"))==null||_.addEventListener("click",async()=>{await k.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await k.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),o("Annonce retirée","success"),n(),l("collection")})}function Ze(e,t,i,r){var h,v,$,I,T,B;const{state:c,toast:o,openModal:n,closeModal:l,navigate:a,refreshProfile:d}=r,s=e.player,p=t.filter(S=>S.player.id===s.id),u=p.length,f=Dt[s.rarity]||1e3,g=s.rarity!=="legende",x=xt(s),m=xe(s,s.job),_=s.job2?xe(s,s.job2):null,b=Ee[s.job]||"#1A6B3C",y=s.job2?Ee[s.job2]:null,w=gt[s.rarity]||"#ccc",L=qe[s.country_code]||s.country_code||"";n(`${s.firstname} ${s.surname_encoded}`,`<div style="display:flex;gap:16px;flex-wrap:wrap;justify-content:center">
    ...
    </div>`:""}`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),($=document.getElementById("close-detail"))==null||$.addEventListener("click",l),(I=document.getElementById("direct-sell-btn"))==null||I.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte ${s.surname_encoded} pour ${f.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const S=p.find(K=>!K.is_for_sale)||p[0];if(!S){o("Aucune carte à vendre","error");return}const{error:q}=await k.from("cards").delete().eq("id",S.id);if(q){o(q.message,"error");return}await k.from("users").update({credits:(c.profile.credits||0)+f}).eq("id",c.profile.id),await d(),o(`+${f.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),l(),a("collection")}),(T=document.getElementById("market-sell-btn"))==null||T.addEventListener("click",async()=>{const S=parseInt(document.getElementById("sell-price").value);if(!S||S<1){o("Prix invalide","error");return}await k.from("cards").update({is_for_sale:!0,sale_price:S}).eq("id",e.id),await k.from("market_listings").insert({seller_id:c.profile.id,card_id:e.id,price:S}),o("Carte mise en vente sur le marché !","success"),l(),a("collection")}),(B=document.getElementById("cancel-sell-btn"))==null||B.addEventListener("click",async()=>{await k.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await k.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),o("Annonce retirée","success"),l(),a("collection")})}const Ae={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},te={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function Le(e){const t=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function yt(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function mt(e){var i;const t="https://fcnwclxlkytdfjotqkta.supabase.co";return(i=e==null?void 0:e.clubs)!=null&&i.logo_url?e.clubs.logo_url.startsWith("http")?e.clubs.logo_url:`${t}/storage/v1/object/public/assets/clubs/${e.clubs.logo_url}`:null}function Nt(e,t=44,i=58){var f;const r=e?Le(e):null,c=e?mt(e):null,o=yt(e==null?void 0:e.country_code),n=(e==null?void 0:e.job)||"MIL",l=te[n]||"#555",a={normal:"#aaa",pépite:"#D4A017",papyte:"#222",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",d=Number(n==="GK"?e==null?void 0:e.note_g:n==="DEF"?e==null?void 0:e.note_d:n==="MIL"?e==null?void 0:e.note_m:e==null?void 0:e.note_a)||0,s=Math.round(i*.19),p=Math.round(i*.25),u=i-s-p;return e?`<div style="width:${t}px;height:${i}px;border-radius:5px;border:2px solid ${a};background:${l};position:relative;overflow:hidden;flex-shrink:0">
  ...
  </div>`:`<div style="width:${t}px;height:${i}px;border:2px dashed rgba(255,255,255,0.3);border-radius:5px;display:flex;align-items:center;justify-content:center;font-size:20px;color:rgba(255,255,255,0.3)">+</div>`}async function Ce(e,t){const{state:i,navigate:r,toast:c}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await k.from("decks").select("id,name,formation_card_id").eq("owner_id",i.profile.id).order("created_at",{ascending:!1});e.innerHTML=`
  <div class="page">
  ...
  </div>`,document.getElementById("new-deck-btn").addEventListener("click",async()=>{const n=prompt("Nom du deck :",`Deck ${((o==null?void 0:o.length)||0)+1}`);if(!n)return;const{data:l,error:a}=await k.from("decks").insert({owner_id:i.profile.id,name:n}).select().single();if(a){c(a.message,"error");return}c("Deck créé !","success"),et(l.id,e,t)}),e.querySelectorAll("[data-open-deck]").forEach(n=>{n.addEventListener("click",()=>et(n.dataset.openDeck,e,t))}),e.querySelectorAll("[data-rename]").forEach(n=>{n.addEventListener("click",async()=>{const l=prompt("Nouveau nom :",n.dataset.name);if(!l||l===n.dataset.name)return;const{error:a}=await k.from("decks").update({name:l}).eq("id",n.dataset.rename);if(a){c(a.message,"error");return}c("Deck renommé !","success"),Ce(e,t)})}),e.querySelectorAll("[data-delete]").forEach(n=>{n.addEventListener("click",async()=>{if(!confirm(`Supprimer le deck "${n.dataset.name}" ? Cette action est irréversible.`))return;await k.from("deck_cards").delete().eq("deck_id",n.dataset.delete);const{error:l}=await k.from("decks").delete().eq("id",n.dataset.delete);if(l){c(l.message,"error");return}c("Deck supprimé.","success"),Ce(e,t)})})}async function et(e,t,i){const{state:r,toast:c}=i;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await k.from("decks").select("*").eq("id",e).single(),{data:n}=await k.from("cards").select(`id, card_type, formation,
      player:players(id, firstname, surname_encoded, country_code, club_id, job, job2,
        note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length,
        clubs(encoded_name, logo_url))`).eq("owner_id",r.profile.id),l=(n||[]).filter(f=>f.card_type==="player"&&f.player),a=(n||[]).filter(f=>f.card_type==="formation"),d=a.map(f=>f.formation).filter(Boolean),{data:s}=await k.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",e);let p=o.formation||"4-4-2";d.length>0&&!d.includes(p)&&(p=d[0]);const u={deckId:e,name:o.name,formation:p,formationCardId:o.formation_card_id,slots:{},subs:[],playerCards:l,formationCards:a,availableFormations:d};(s||[]).forEach(f=>{f.is_starter?u.slots[f.position]=f.card_id:u.subs.includes(f.card_id)||u.subs.push(f.card_id)}),ce(t,u,i)}function ce(e,t,i){var a;const{navigate:r}=i;Ae[t.formation];const c=tt(t.formation),o=c.filter(d=>t.slots[d]).length,n=t.availableFormations.length>0?t.availableFormations:Object.keys(Ae),l=t.subs.map(d=>t.playerCards.find(s=>s.id===d)).filter(Boolean);[...Object.values(t.slots),...t.subs],e.innerHTML=`
  <div class="page">
    ...
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(f=document.getElementById("close-selector"))==null||f.addEventListener("click",o),(g=document.getElementById("remove-player"))==null||g.addEventListener("click",()=>{delete t.slots[e],o(),ce(i,t,r)}),document.querySelectorAll(".player-option").forEach(x=>{x.addEventListener("click",()=>{t.slots[e]=x.dataset.cardId,o(),ce(i,t,r)})})}function Ht(e,t,i){var a;const{openModal:r,closeModal:c}=i,o=new Set;Object.values(e.slots).filter(Boolean).forEach(d=>{var p;const s=e.playerCards.find(u=>u.id===d);(p=s==null?void 0:s.player)!=null&&p.id&&o.add(s.player.id)}),e.subs.forEach(d=>{var p;const s=e.playerCards.find(u=>u.id===d);(p=s==null?void 0:s.player)!=null&&p.id&&o.add(s.player.id)});const n=new Set,l=e.playerCards.filter(d=>{var s,p,u;return o.has((s=d.player)==null?void 0:s.id)||n.has((p=d.player)==null?void 0:p.id)?!1:(n.add((u=d.player)==null?void 0:u.id),!0)});r("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
    ...
    </div>`,'<button class="btn btn-ghost" id="close-sub-selector">Fermer</button>'),(a=document.getElementById("close-sub-selector"))==null||a.addEventListener("click",c),document.querySelectorAll(".player-option").forEach(d=>{d.addEventListener("click",()=>{e.subs.push(d.dataset.cardId),c(),ce(t,e,i)})})}async function Kt(e,t){const{state:i,toast:r,navigate:c}=t,o=e.formationCards.find(a=>a.formation===e.formation),n=(o==null?void 0:o.id)||e.formationCardId;await k.from("decks").update({formation:e.formation,formation_card_id:n||null}).eq("id",e.deckId),await k.from("deck_cards").delete().eq("deck_id",e.deckId);const l=[];if(Object.entries(e.slots).forEach(([a,d],s)=>{l.push({deck_id:e.deckId,card_id:d,position:a,is_starter:!0,slot_order:s})}),e.subs.forEach((a,d)=>{l.push({deck_id:e.deckId,card_id:a,position:`SUB${d+1}`,is_starter:!1,slot_order:100+d})}),l.length>0){const{error:a}=await k.from("deck_cards").insert(l);if(a){r(a.message,"error");return}}r("Deck enregistré ✅","success"),c("decks")}function tt(e){const t=Ae[e]||Ae["4-4-2"],i=["GK1"];for(let r=1;r<=t.DEF;r++)i.push(`DEF${r}`);for(let r=1;r<=t.MIL;r++)i.push(`MIL${r}`);for(let r=1;r<=t.ATT;r++)i.push(`ATT${r}`);return i}const Ut="modulepreload",Vt=function(e){return"/manager-wars/"+e},it={},Yt=function(t,i,r){let c=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const n=document.querySelector("meta[property=csp-nonce]"),l=(n==null?void 0:n.nonce)||(n==null?void 0:n.getAttribute("nonce"));c=Promise.allSettled(i.map(a=>{if(a=Vt(a),a in it)return;it[a]=!0;const d=a.endsWith(".css"),s=d?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${a}"]${s}`))return;const p=document.createElement("link");if(p.rel=d?"stylesheet":Ut,d||(p.as="script"),p.crossOrigin="",p.href=a,l&&p.setAttribute("nonce",l),document.head.appendChild(p),d)return new Promise((u,f)=>{p.addEventListener("load",u),p.addEventListener("error",()=>f(new Error(`Unable to preload CSS for ${a}`)))})}))}function o(n){const l=new Event("vite:preloadError",{cancelable:!0});if(l.payload=n,window.dispatchEvent(l),!l.defaultPrevented)throw n}return c.then(n=>{for(const l of n||[])l.status==="rejected"&&o(l.reason);return t().catch(o)})};async function ht(){const{data:e}=await k.from("booster_configs").select("*").eq("is_active",!0).order("sort_order");if(!(e!=null&&e.length))return[];const{data:t}=await k.from("booster_drop_rates").select("*").in("booster_id",e.map(i=>i.id)).order("sort_order");return e.map(i=>({...i,rates:(t||[]).filter(r=>r.booster_id===i.id)}))}function Wt(e){if(!(e!=null&&e.length))return null;const t=e.reduce((r,c)=>r+Number(c.percentage),0);let i=Math.random()*t;for(const r of e)if(i-=Number(r.percentage),i<=0)return r;return e[e.length-1]}const Xt=Object.freeze(Object.defineProperty({__proto__:null,loadActiveBoosters:ht,rollDropRate:Wt},Symbol.toStringTag,{value:"Module"})),bt=()=>Object.keys(me),Jt=[{id:"players_std",img:"/manager-wars/icons/booster-players.png",name:"Players",sub:"5 cartes joueurs",cost:5e3,costLabel:"5 000 crédits",cardCount:5,type:"player"},{id:"players_pub",img:"/manager-wars/icons/booster-silver.png",name:"Players (pub)",sub:"3 cartes joueurs",cost:0,costLabel:"1 pub",cardCount:3,type:"player"},{id:"game_changer",img:"/manager-wars/icons/booster-gamechanger.png",name:"Game Changer",sub:"3 cartes spéciales",cost:1e4,costLabel:"10 000 crédits",cardCount:3,type:"game_changer"},{id:"formation",img:"/manager-wars/icons/booster-formation.png",name:"Formation",sub:"1 carte formation",cost:1e4,costLabel:"10 000 crédits",cardCount:1,type:"formation"}],je={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé."},"Double attaque":{icon:"⚡",desc:"La prochaine attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la prochaine action IA."},Gel:{icon:"❄️",desc:"Bloque le meilleur attaquant IA."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function Qt(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";if(!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}const Zt={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},ei={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"};function ti(e){var c;const t={};(e.rates||[]).forEach(o=>{t[o.card_type]=(t[o.card_type]||0)+Number(o.percentage||0)});const i=((c=Object.entries(t).sort((o,n)=>n[1]-o[1])[0])==null?void 0:c[0])||"player",r=e.image_url||"booster-players.png";return{id:e.id,img:"/manager-wars/icons/"+r,name:e.name,sub:`${e.card_count} carte(s)`,cost:e.price_type==="credits"&&e.price_credits||0,costLabel:e.price_type==="credits"?`${(e.price_credits||0).toLocaleString("fr")} crédits`:e.price_type==="pub"?"1 pub":"Gratuit",cardCount:e.card_count||5,type:i,isPub:e.price_type==="pub",rates:e.rates||[],_raw:e}}async function ii(e,{state:t,navigate:i,toast:r}){var n;const c=((n=t.profile)==null?void 0:n.credits)||0;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⏳ Chargement...</div>';let o=[];try{o=(await ht()).map(ti)}catch(l){console.warn("Erreur chargement boosters DB, fallback hardcodé",l)}o.length||(o=Jt.map(l=>({...l,rates:[],isPub:l.id==="players_pub"}))),e.innerHTML=`
  <div class="page">
    <div class="page-header">
      <h2>📦 Boosters</h2>
    ...
    <div class="page-body">
      <div class="booster-grid">
        ${o.map(l=>{const a=l.cost===0||c>=l.cost;return`<div class="booster-card ${a?"":"disabled"}" data-booster="${l.id}" style="position:relative">
  ...
  </div>`,e.querySelectorAll(".booster-card:not(.disabled)").forEach(l=>{l.addEventListener("click",async()=>{const a=o.find(d=>d.id===l.dataset.booster);if(a){l.style.opacity="0.5",l.style.pointerEvents="none";try{await oi(a,{state:t,toast:r,navigate:i,container:e})}catch(d){r(d.message,"error"),l.style.opacity="",l.style.pointerEvents=""}}})}),e.querySelectorAll(".booster-info-btn").forEach(l=>{l.addEventListener("click",a=>{a.stopPropagation();const d=o.find(s=>s.id===l.dataset.boosterId);pi(d)})})}async function oi(e,{state:t,toast:i,navigate:r,container:c}){if(e.cost>0&&t.profile.credits<e.cost){i("Crédits insuffisants","error");return}if(e.isPub)try{await gi()}catch(p){i(p.message||"Regardez la pub entièrement pour ouvrir le booster","error");return}const{data:o}=await k.from("cards").select("card_type, player_id, formation").eq("owner_id",t.profile.id),n=new Set((o||[]).filter(p=>p.card_type==="player").map(p=>p.player_id)),l=new Set((o||[]).filter(p=>p.card_type==="formation").map(p=>p.formation));let a=[];const d=e.type||"player";d==="player"?a=await si(t.profile,e.cardCount,e.cost):d==="game_changer"?a=await di(t.profile,e.cardCount,e.cost):d==="formation"?a=await li(t.profile,e.cost):a=await ai(t.profile,e),a.forEach(p=>{p.card_type==="player"&&p.player?p.isDuplicate=n.has(p.player.id):p.card_type==="formation"&&(p.isDuplicate=l.has(p.formation))});const{data:s}=await k.from("users").select("*").eq("id",t.profile.id).single();s&&(t.profile=s),ci(a,e,r)}function ni(){const e=Math.random()*100;return e<.5?"legende":e<2?"special":e<10?"normal_high":"normal_low"}function re(e){return Math.max(Number(e.note_g)||0,Number(e.note_d)||0,Number(e.note_m)||0,Number(e.note_a)||0)}function ri(e,t){let i;switch(t){case"legende":i=e.filter(r=>r.rarity==="legende"),i.length||(i=e.filter(r=>r.rarity==="pepite"||r.rarity==="papyte")),i.length||(i=e.filter(r=>re(r)>=6));break;case"special":i=e.filter(r=>r.rarity==="pepite"||r.rarity==="papyte"),i.length||(i=e.filter(r=>re(r)>=6));break;case"normal_high":i=e.filter(r=>r.rarity==="normal"&&re(r)>=6),i.length||(i=e.filter(r=>re(r)>=6));break;default:i=e.filter(r=>r.rarity==="normal"&&re(r)>=1&&re(r)<=5),i.length||(i=e.filter(r=>r.rarity==="normal"));break}return i.length||(i=e),i[Math.floor(Math.random()*i.length)]}async function ai(e,t){if(t.cost>0){const{error:c}=await k.from("users").update({credits:e.credits-t.cost}).eq("id",e.id);if(c)throw c}const{rollDropRate:i}=await Yt(async()=>{const{rollDropRate:c}=await Promise.resolve().then(()=>Xt);return{rollDropRate:c}},void 0),r=[];for(let c=0;c<(t.cardCount||5);c++){const o=i(t.rates);if(o){if(o.card_type==="player"){let n=k.from("players").select("id,job,firstname,surname_encoded,country_code,club_id,rarity,note_g,note_d,note_m,note_a,skin,hair,hair_length,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);o.rarity&&(n=n.eq("rarity",o.rarity));const{data:l}=await n;let a=l||[];if((o.note_min||o.note_max)&&(a=a.filter(p=>{const u=Math.max(Number(p.note_g)||0,Number(p.note_d)||0,Number(p.note_m)||0,Number(p.note_a)||0);return(!o.note_min||u>=o.note_min)&&(!o.note_max||u<=o.note_max)})),a.length||(o.note_min||o.note_max?(a=l||[],console.warn("[Booster] Aucun joueur avec note",o.note_min,"-",o.note_max,"— fallback rareté uniquement")):a=l||[]),!a.length)continue;const d=a[Math.floor(Math.random()*a.length)],{data:s}=await k.from("cards").insert({owner_id:e.id,player_id:d.id,card_type:"player"}).select().single();s&&r.push({...s,player:d})}else if(o.card_type==="game_changer"){const{data:n}=await k.from("gc_definitions").select("id,name").eq("is_active",!0).eq("gc_type","game_changer"),l=n!=null&&n.length?n:[{name:"Ressusciter"},{name:"Double attaque"},{name:"Bouclier"},{name:"Vol de note"},{name:"Gel"}],d=l[Math.floor(Math.random()*l.length)].name,{data:s}=await k.from("cards").insert({owner_id:e.id,card_type:"game_changer",gc_type:d}).select().single();s&&r.push(s)}else if(o.card_type==="formation"){const n=bt(),l=n[Math.floor(Math.random()*n.length)],{data:a}=await k.from("cards").insert({owner_id:e.id,card_type:"formation",formation:l}).select();a!=null&&a[0]&&r.push(a[0])}}}return r}async function si(e,t,i){if(i>0){const{error:d}=await k.from("users").update({credits:e.credits-i}).eq("id",e.id);if(d)throw d}const{data:r}=await k.from("players").select("id,job,firstname,surname_encoded,country_code,club_id,rarity,note_g,note_d,note_m,note_a,note_min,note_max,skin,hair,hair_length,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);if(!(r!=null&&r.length))throw new Error("Pas de joueurs en BDD — ajoutes-en via le panel admin !");const c=r.filter(d=>d.job==="GK"),o=r.filter(d=>d.job!=="GK"),n=!e.first_booster_opened&&c.length>0,l=[];for(let d=0;d<t;d++){const s=d===0&&n?c:d===0?o:r,p=ni(),u=ri(s,p);u&&l.push(u)}n&&await k.from("users").update({first_booster_opened:!0}).eq("id",e.id);const{data:a}=await k.from("cards").insert(l.map(d=>({owner_id:e.id,player_id:d.id,card_type:"player"}))).select();return l.map((d,s)=>({...a[s],player:d}))}async function di(e,t,i){const{error:r}=await k.from("users").update({credits:e.credits-i}).eq("id",e.id);if(r)throw r;const{data:c}=await k.from("gc_definitions").select("id,name,gc_type,color,effect,image_url").eq("is_active",!0),o=c!=null&&c.length?c:Object.keys(je).map(s=>({name:s,gc_type:"game_changer"})),n=Array.from({length:t},()=>{const s=o[Math.floor(Math.random()*o.length)];return{owner_id:e.id,card_type:"game_changer",gc_type:s.name,gc_definition_id:s.id||null}}),{data:l,error:a}=await k.from("cards").insert(n).select();return a&&console.error("[Booster GC] Erreur insert:",a.message,a),(l||[]).map(s=>{const p=c==null?void 0:c.find(u=>u.name===s.gc_type||u.id===s.gc_definition_id);return{...s,_gcDef:p||null}})}async function li(e,t){const{error:i}=await k.from("users").update({credits:e.credits-t}).eq("id",e.id);if(i)throw i;const r=bt(),c=r[Math.floor(Math.random()*r.length)],{data:o,error:n}=await k.from("cards").insert({owner_id:e.id,card_type:"formation",formation:c}).select();return n&&console.error("[Booster Formation] Erreur insert:",n.message,n),o||[]}function ci(e,t,i){var u;if(!e||e.length===0){const f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:3000;gap:16px;color:#fff;padding:24px;text-align:center",f.innerHTML=`
  ...
  `,document.body.appendChild(r);let c=!1;document.getElementById("pack-visual").addEventListener("click",()=>{if(c)return;c=!0;const f=document.getElementById("pack-visual");f.classList.add("shaking"),setTimeout(()=>{f.classList.add("pack-open"),setTimeout(()=>{document.getElementById("pack-phase").style.display="none",n(0)},600)},500)});let o=0;function n(f){o=f;const g=document.getElementById("reveal-phase");g.style.display="flex",l(f)}function l(f){var h;const g=e[f],x=document.getElementById("card-counter"),m=document.getElementById("single-card-slot"),_=document.getElementById("card-tap-hint");x&&(x.textContent=`Carte ${f+1} / ${e.length}`),_&&(_.textContent=f<e.length-1?"Appuie pour continuer →":"Appuie pour voir toutes tes cartes");const b=ot(g),y=g.card_type==="player"&&((h=g.player)==null?void 0:h.rarity)==="legende";m.innerHTML=`
    ...
    </div>`}if(e.card_type==="game_changer"){const o=e._gcDef,n=(o==null?void 0:o.gc_type)==="ultra_game_changer",l={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},a={purple:"#b06ce0",light_blue:"#00d4ef"},d=l[o==null?void 0:o.color]||l.purple,s=a[o==null?void 0:o.color]||a.purple,p=(o==null?void 0:o.name)||e.gc_type||"Game Changer",u=(o==null?void 0:o.effect)||((r=je[e.gc_type])==null?void 0:r.desc)||"",f=o!=null&&o.image_url?`/manager-wars/icons/${o.image_url}`:null,g=((c=je[e.gc_type])==null?void 0:c.icon)||"⚡";return`<div style="width:170px;height:240px;background:${d};border-radius:14px;border:3px solid ${s};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${s}88;flex-shrink:0">
      ...
      </div>`,document.body.appendChild(r),r.addEventListener("click",n=>{n.target===r&&r.remove()}),(i=document.getElementById("odds-close"))==null||i.addEventListener("click",()=>r.remove());return}ui()}function ui(){const e=document.createElement("div");e.style.cssText=`position:fixed;inset:0;background:rgba(0,0,0,0.6);display:flex;
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}const fi="simulation",vt="VOTRE_ZONE_ID";function gi(){switch(fi){case"propellerads":return xi();case"adsense":return yi();default:return Ne()}}function Ne(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:#111;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:14px";let i=5;t.innerHTML=`
      ...
      <button id="ad-skip" disabled style="margin-top:8px;padding:10px 24px;border-radius:20px;border:none;background:rgba(255,255,255,0.15);color:rgba(255,255,255,0.4);font-size:13px;cursor:default">
        Passer (5)
      </button>`,document.body.appendChild(t);const r=setInterval(()=>{i--;const c=document.getElementById("ad-cd"),o=document.getElementById("ad-skip");c&&(c.textContent=i),o&&(o.textContent=i>0?`Passer (${i})`:"✓ Continuer"),i<=0&&(clearInterval(r),o&&(o.disabled=!1,o.style.cssText=o.style.cssText.replace("rgba(255,255,255,0.15)","#1A6B3C").replace("rgba(255,255,255,0.4)","#fff").replace("default","pointer")),o==null||o.addEventListener("click",()=>{t.remove(),e(!0)},{once:!0}))},1e3)})}function xi(){return new Promise((e,t)=>{if(typeof window.propeller>"u")return console.warn("[Ad] PropellerAds non chargé → simulation"),e(Ne());window.propeller.push({zone_id:vt,onComplete:()=>e(!0),onSkip:()=>t(new Error("Publicité ignorée")),onError:()=>t(new Error("Erreur publicité"))})})}function yi(){return new Promise((e,t)=>{if(typeof window.adsbygoogle>"u")return console.warn("[Ad] AdSense non chargé → simulation"),e(Ne());try{(window.adsbygoogle=window.adsbygoogle||[]).push({google_ad_client:"ca-pub-VOTRE_PUBLISHER_ID",google_ad_slot:vt,google_ad_format:"rewarded",on_reward:()=>e(!0),on_dismiss:()=>t(new Error("Publicité fermée"))})}catch(i){t(i)}})}const pe={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},ue={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function wt(e,t,i,r,c){var o;e.innerHTML=`<div class="match-screen" style="display:flex;align-items:center;justify-content:center;min-height:100vh">
    <div style="text-align:center;padding:40px;color:#fff">
      <div style="font-size:48px;margin-bottom:16px">${t}</div>
      <p style="margin-bottom:16px">${i}</p>
      <button class="btn btn-primary" id="msg-btn">${r}</button>
    </div>
  </div>`,(o=document.getElementById("msg-btn"))==null||o.addEventListener("click",c)}function J(e){const t=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function Be(e){var i,r;const t=e.player;return{cardId:e.id,id:t.id,firstname:t.firstname,name:t.surname_encoded,country_code:t.country_code,club_id:t.club_id,job:t.job,job2:t.job2,note_g:Number(t.note_g)||0,note_d:Number(t.note_d)||0,note_m:Number(t.note_m)||0,note_a:Number(t.note_a)||0,rarity:t.rarity,skin:t.skin,hair:t.hair,hair_length:t.hair_length,clubName:((i=t.clubs)==null?void 0:i.encoded_name)||null,clubLogo:((r=t.clubs)==null?void 0:r.logo_url)||null,boost:0,used:!1,_line:null,_col:null}}function Oe(e){return e===1?[1]:e===2?[0,2]:e===3?[0,1,2]:e===4?[0,1,1,2]:e===5?[0,1,1,1,2]:[1]}function mi(){const e=Math.random()*100;return e<.1?4:e<5?3:e<20?2:1}function $t(e,t){const i=pe[t]||pe["4-4-2"],r={GK:[],DEF:[],MIL:[],ATT:[]},c=[...e];for(const o of["GK","DEF","MIL","ATT"]){const n=[];for(let a=0;a<i[o];a++){let d=c.findIndex(s=>s.job===o);if(d===-1&&(d=c.findIndex(s=>s.job2===o)),d===-1&&(d=0),c[d]){const s={...c[d],_line:o};n.push(s),c.splice(d,1)}}const l=Oe(n.length);n.forEach((a,d)=>{a._col=l[d]}),r[o]=n}return r}async function hi(e,t){var n,l;const{data:i}=await k.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(60);if(!i||i.length<11)return bi(e);const r=pe[e]||pe["4-4-2"],c={GK:[],DEF:[],MIL:[],ATT:[]},o=[...i];for(const a of["GK","DEF","MIL","ATT"]){const d=o.filter(g=>g.job===a),s=o.filter(g=>g.job!==a),p=[...d,...s],u=[];for(let g=0;g<r[a];g++){const x=p[g]||o[g];x&&u.push({cardId:"ai-"+x.id+"-"+g,id:x.id,firstname:x.firstname,name:x.surname_encoded,country_code:x.country_code,club_id:x.club_id,job:x.job,job2:x.job2,note_g:Number(x.note_g)||0,note_d:Number(x.note_d)||0,note_m:Number(x.note_m)||0,note_a:Number(x.note_a)||0,rarity:x.rarity,skin:x.skin,hair:x.hair,hair_length:x.hair_length,clubName:((n=x.clubs)==null?void 0:n.encoded_name)||null,clubLogo:((l=x.clubs)==null?void 0:l.logo_url)||null,boost:0,used:!1,_line:a})}const f=Oe(u.length);u.forEach((g,x)=>{g._col=f[x]}),c[a]=u}return c}function bi(e){const t=pe[e]||pe["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},r=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let c=0;for(const o of["GK","DEF","MIL","ATT"]){const n=[];for(let a=0;a<t[o];a++){const d=3+Math.floor(Math.random()*5);n.push({cardId:"fake-"+c,id:"fake-"+c,firstname:"IA",name:r[c%r.length],country_code:"XX",club_id:null,job:o,job2:null,note_g:o==="GK"?d:2,note_d:o==="DEF"?d:2,note_m:o==="MIL"?d:2,note_a:o==="ATT"?d:2,rarity:"normal",boost:0,used:!1,_line:o}),c++}const l=Oe(n.length);n.forEach((a,d)=>{a._col=l[d]}),i[o]=n}return i}function vi(e){let t=e.nextElementSibling;for(;t;)t.dataset.matchHidden||(t.dataset.matchHidden=t.style.display||"flex",t.style.display="none"),t=t.nextElementSibling}function Ie(e){let t=e.nextElementSibling;for(;t;)t.dataset.matchHidden&&(t.style.display=t.dataset.matchHidden==="none"?"":t.dataset.matchHidden,delete t.dataset.matchHidden),t=t.nextElementSibling}async function wi(e,t){var h;const{state:i,navigate:r,toast:c}=t;vi(e);const o=i.params||{};e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const n=o.matchMode||"vs_ai_easy",l=n.replace("vs_ai_",""),a=n;if(!o.deckId)return _i(e,t,n);const d=o.deckId,[{data:s},{data:p}]=await Promise.all([k.from("decks").select("formation,name").eq("id",d).single(),k.from("deck_cards").select(`position, is_starter, slot_order,
        card:cards(id, card_type, formation,
          player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
            note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
            clubs(encoded_name,logo_url)))`).eq("deck_id",d).order("slot_order")]),u=(p||[]).filter(v=>{var $;return v.is_starter&&(($=v.card)==null?void 0:$.player)}).map(v=>Be(v.card)),f=(p||[]).filter(v=>{var $;return!v.is_starter&&(($=v.card)==null?void 0:$.player)}).map(v=>Be(v.card));if(u.length<11){wt(e,"⚠️",`Deck incomplet (${u.length}/11).`,"Compléter",()=>r("decks"));return}const g=(p||[]).find(v=>{var $;return(($=v.card)==null?void 0:$.card_type)==="formation"}),x=(s==null?void 0:s.formation)||((h=g==null?void 0:g.card)==null?void 0:h.formation)||"4-4-2",{data:m}=await k.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",i.profile.id).eq("card_type","game_changer"),{data:_}=await k.from("gc_definitions").select("*").eq("is_active",!0),b=(m||[]).map(v=>({...v,_gcDef:(_==null?void 0:_.find($=>$.name===v.gc_type||$.id===v.gc_definition_id))||null})),y=$t(u,x),w=await hi(x),L=async v=>{const{data:$}=await k.from("matches").insert({home_id:i.profile.id,away_id:null,mode:a,home_deck_id:d,status:"in_progress"}).select().single(),I={gcDefs:_||[],matchId:$==null?void 0:$.id,mode:a,difficulty:l,formation:x,homeTeam:y,aiTeam:w,homeSubs:f,subsUsed:0,maxSubs:Math.min(f.length,3),homeScore:0,aiScore:0,gcCards:v,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};ki(e,I,t)};if(!b.length){L([]);return}$i(e,b,L)}function $i(e,t,i){const c=new Set,o=t.filter(s=>{const p=s.gc_type||s.id;return c.has(p)?!1:(c.add(p),!0)});let n=[];function l(s,p){const u=s._gcDef,f={purple:"linear-gradient(135deg,#3d0a7a,#7a28b8)",light_blue:"linear-gradient(135deg,#006080,#00bcd4)"},g={purple:"#9b59b6",light_blue:"#00bcd4"},x=f[u==null?void 0:u.color]||f.purple,m=g[u==null?void 0:u.color]||g.purple;return`<div class="gc-select-card" data-id="${s.id}"
      ...
      <button id="gc-reset" style="width:100%;max-width:320px;padding:14px;border-radius:14px;border:2px solid rgba(212,160,23,0.4);background:rgba(212,160,23,0.1);color:#D4A017;font-size:14px;font-weight:700;cursor:pointer">
        🔄 Changer de GC
      </button>`;const u=_=>{e.style.overflow="",e.style.height="",e.style.display="",e.style.flexDirection="",i(_)};(g=p.querySelector("#gc-launch"))==null||g.addEventListener("click",()=>u(n)),(x=p.querySelector("#gc-no-gc"))==null||x.addEventListener("click",()=>u([])),(m=p.querySelector("#gc-reset"))==null||m.addEventListener("click",()=>{n=[],d()});const f=e.querySelector("#gc-screen-wrap");f?f.appendChild(p):e.appendChild(p)}function d(){e.style.overflow="hidden";const s=e.clientHeight||e.getBoundingClientRect().height;s>50&&(e.style.height=s+"px"),e.innerHTML=`
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      <div style="flex:1;overflow-y:auto;display:flex;flex-wrap:wrap;gap:10px;justify-content:center;align-content:flex-start;padding:8px 16px 16px">
        ${o.map(p=>{const u=n.find(f=>f.gc_type===p.gc_type);return l(p,!!u)}).join("")}
      </div>
    </div>`,e.querySelectorAll(".gc-select-card").forEach(p=>{p.addEventListener("click",()=>{const u=p.dataset.id,f=o.find(x=>x.id===u);if(!f)return;const g=n.findIndex(x=>x.gc_type===f.gc_type);g>-1?n.splice(g,1):n.length<3&&n.push(f),d(),n.length===3&&a()})}),n.length===3&&a()}d()}async function _i(e,t,i){const{state:r,navigate:c}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await k.from("decks").select("id,name,is_active,formation").eq("owner_id",r.profile.id).order("created_at",{ascending:!1});if(!o||o.length===0){wt(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>c("decks"));return}const n=o.map(s=>s.id),{data:l}=await k.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",n).order("slot_order");let a=0;function d(){var b,y,w,L,h;const s=o[a],p=(l||[]).filter(v=>v.deck_id===s.id),u=p.filter(v=>{var $;return v.is_starter&&(($=v.card)==null?void 0:$.player)}).map(v=>Be(v.card)),f=p.find(v=>{var $;return(($=v.card)==null?void 0:$.card_type)==="formation"}),g=s.formation||((b=f==null?void 0:f.card)==null?void 0:b.formation)||"4-4-2",x=u.length>=11?$t(u,g):null,m=u.length>=11;e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:calc(100dvh - 130px);overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div style="display:flex;align-items:center;gap:8px;padding:8px;flex-shrink:0">
        ...
        <button id="next-deck" style="width:46px;height:46px;border-radius:50%;background:rgba(255,255,255,${a===o.length-1?"0.05":"0.15"});border:2px solid rgba(255,255,255,${a===o.length-1?"0.1":"0.3"});color:${a===o.length-1?"rgba(255,255,255,0.2)":"#fff"};font-size:20px;cursor:${a===o.length-1?"default":"pointer"};flex-shrink:0">▶</button>
      </div>
      ...
      <div id="deck-swipe-zone" style="flex:1;min-height:0;overflow:hidden;position:relative;touch-action:pan-y;display:flex;align-items:center;justify-content:center">
        ${x?`<div style="width:min(98vw, calc(100dvh - 400px));overflow:hidden;flex-shrink:0">${Et(x,g,null,[],285,285)}</div>`:`<div style="display:flex;align-items:center;justify-content:center;height:100%;opacity:.4;flex-direction:column;gap:8px">
      ...
      <div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">
        ${o.map((v,$)=>`<div style="width:7px;height:7px;border-radius:50%;background:${$===a?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}
      </div>`:""}
    ...
    </div>`,(y=document.getElementById("prev-deck"))==null||y.addEventListener("click",()=>{a>0&&(a--,d())}),(w=document.getElementById("next-deck"))==null||w.addEventListener("click",()=>{a<o.length-1&&(a++,d())}),(L=document.getElementById("validate-deck"))==null||L.addEventListener("click",()=>{m&&t.navigate("match",{matchMode:i,deckId:s.id})}),(h=document.getElementById("cancel-deck-select"))==null||h.addEventListener("click",()=>{Ie(e),c("home")});const _=document.getElementById("deck-swipe-zone");if(_){let v=0,$=0;_.addEventListener("touchstart",I=>{v=I.touches[0].clientX,$=I.touches[0].clientY},{passive:!0}),_.addEventListener("touchend",I=>{const T=I.changedTouches[0].clientX-v,B=I.changedTouches[0].clientY-$;Math.abs(T)<40||Math.abs(T)<Math.abs(B)||(T<0&&a<o.length-1?(a++,d()):T>0&&a>0&&(a--,d()))},{passive:!0})}}d()}function ki(e,t,i){e.innerHTML=`
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
  </div>`,setTimeout(()=>Ei(e,t,i),5e3)}function Ei(e,t,i){const r=t.homeTeam.MIL||[],c=t.aiTeam.MIL||[];function o(p){return p.reduce((u,f)=>u+U(f,"MIL"),0)}function n(p){let u=0;for(let f=0;f<p.length-1;f++){const g=de(p[f],p[f+1]);g==="#00ff88"?u+=2:g==="#FFD700"&&(u+=1)}return u}const l=o(r)+n(r),a=o(c)+n(c),d=l>=a;function s(p,u,f){return`<div style="text-align:center">
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${p.map((g,x)=>{const m=J(g),_=x<p.length-1?de(g,p[x+1]):null,b=_&&_!=="#ff3333"&&_!=="#cc2222";return`
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div id="midfield-result" style="opacity:0;text-align:center;transition:opacity 0.5s;color:#fff;max-width:320px"></div>
  </div>`,setTimeout(()=>{const p=document.getElementById("score-home"),u=document.getElementById("score-ai"),f=document.getElementById("midfield-result");if(p&&u&&(d?(p.style.fontSize="80px",p.style.color="#FFD700",u.style.opacity="0.25"):(u.style.fontSize="80px",u.style.color="#ff6b6b",p.style.opacity="0.25")),f){const g=mi();f.style.opacity="1",d&&t.clubName,f.innerHTML=`
      ...
      </div>`,(m=document.getElementById("start-match-btn"))==null||m.addEventListener("click",()=>{t.phase=t.attacker==="home"?"attack":"ai-attack",V(e,t,i),t.attacker==="ai"&&setTimeout(()=>Fe(e,t,i),800)})},100)},5e3)}function _t(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";return e!=null&&e.clubLogo?e.clubLogo.startsWith("http")?e.clubLogo:`${t}/storage/v1/object/public/assets/clubs/${e.clubLogo}`:null}function ae(e,t=44,i=58){if(!e)return`<div style="width:${t}px;height:${i}px;border:1.5px dashed rgba(255,255,255,0.2);border-radius:5px"></div>`;const r=typeof e.portrait=="string"&&e.portrait.startsWith("http")?e.portrait:J(e),c=_t(e),o=e._line||e.job||"MIL",n=ue[o]||"#555",l={normal:"#aaa",pépite:"#D4A017",papyte:"#222",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",a=e.note!==void 0?Number(e.note)||0:(Number(U(e,o))||0)+(e.boost||0),d=kt(e==null?void 0:e.country_code),s=Math.round(i*.19),p=Math.round(i*.25),u=i-Math.round(i*.19)-Math.round(i*.25),f=e!=null&&e.used?.28:1;return`<div style="width:${t}px;height:${i}px;border-radius:5px;border:2px solid ${l};background:${n};position:relative;overflow:hidden;flex-shrink:0;opacity:${f}">
    <div style="position:absolute;top:0;left:0;right:0;height:${s}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:center;z-index:2">
  ...
  </div>`}function ve(e,t,i){if(!(e!=null&&e.length))return"";const r=e.slice(0,5);let c='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return r.forEach((o,n)=>{if(c+=ae(o,40,52),n<r.length-1){const l=de(o,r[n+1]);c+=`<div style="width:7px;height:3px;background:${l==="#ff3333"||l==="#cc2222"?"rgba(255,255,255,0.12)":l};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(c+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),c+="</div>",c}function Se(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function kt(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function Pe(e,t,i,r,c=310,o=310){const n=me[t]||{},l=dt(t)||Re[t]||[],a={},d=["ATT","MIL","DEF","GK"];for(const b of d)(e[b]||[]).forEach((w,L)=>{a[`${b}${L+1}`]=w});function s(b){const y=n[b];return y?{x:y.x*c,y:y.y*o}:null}let p="";for(const[b,y]of l){const w=s(b),L=s(y);if(!w||!L)continue;const h=a[b],v=a[y],$=de(h,v);$==="#00ff88"||$==="#FFD700"?(p+=`<line x1="${w.x.toFixed(1)}" y1="${w.y.toFixed(1)}" x2="${L.x.toFixed(1)}" y2="${L.y.toFixed(1)}"
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;height:100%;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:100px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const v=t.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
          ...
          </div>`}const h=t.log[t.log.length-1];return h?'<div style="padding:2px 4px">'+Ai(h)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})()}
    </div>
    ...
    ${(()=>{const h=window.innerWidth>=700,v=(D,W,Y)=>{var N,O;const P=(t.gcDefs||[]).find(G=>G.name===D.gc_type),ie={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[P==null?void 0:P.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",oe={purple:"#b06ce0",light_blue:"#00d4ef"}[P==null?void 0:P.color]||"#b06ce0",E=(P==null?void 0:P.name)||D.gc_type,A=(P==null?void 0:P.effect)||((N=le[D.gc_type])==null?void 0:N.desc)||"",j=P!=null&&P.image_url?`/manager-wars/icons/${P.image_url}`:null,C=((O=le[D.gc_type])==null?void 0:O.icon)||"⚡",M=Math.round(Y*.22),F=Math.round(Y*.22),z=Y-M-F,H=E.length>12?7:9;return`<div class="gc-mini" data-gc-id="${D.id}" data-gc-type="${D.gc_type}"
          ...
          <div style="height:${z}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
            ${j?`<img src="${j}" style="max-width:${W-10}px;max-height:${z-6}px;object-fit:contain">`:`<span style="font-size:${Math.round(z*.55)}px">${C}</span>`}
          </div>
        ...
        <div class="terrain-wrapper" style="overflow:hidden;flex-shrink:0;display:flex;align-items:center;justify-content:center">
          ${Et(t.homeTeam,t.formation,t.phase,r,300,300)}
        </div>
      </div>`;return h?`
  ...
  </div>`,function(){const v=e.querySelector(".terrain-wrapper svg");if(!v)return;v.removeAttribute("width"),v.removeAttribute("height"),v.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",v.setAttribute("viewBox","-26 -26 352 352"),v.setAttribute("preserveAspectRatio","xMidYMid meet");const $=e.querySelector(".match-screen");if($){const I=e.clientHeight;I>50&&($.style.height=I+"px",$.style.overflow="hidden")}}(),t._resizeBound||(t._resizeBound=!0,window.addEventListener("resize",()=>{const h=e.querySelector(".terrain-wrapper svg");h&&(h.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0")})),t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase==="attack"||t.phase==="defense"){let h=!1,v=30;const $=()=>document.getElementById("match-timer"),I=()=>{const T=$();if(!T)return;const B=String(Math.floor(v/60)).padStart(2,"0"),S=String(v%60).padStart(2,"0");T.textContent=` ${B}:${S}`,T.style.color=h?"#ff2222":"#ff9500",T.style.fontWeight="900"};I(),t._timerInt=setInterval(()=>{if(v--,v<0)if(!h)h=!0,v=15,I();else{clearInterval(t._timerInt),t._timerInt=null,t.homeScore=0,t.aiScore=3;const T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;color:#fff;padding:24px;text-align:center",T.innerHTML='<div style="font-size:56px">⏱️</div><div style="font-size:24px;font-weight:900;color:#ff4444">MATCH PERDU PAR FORFAIT</div><div style="font-size:14px;color:rgba(255,255,255,0.6)">Temps écoulé</div>',document.body.appendChild(T),setTimeout(()=>{T.remove(),se(e,t,i)},2500)}else I()},1e3)}(g=document.getElementById("match-quit"))==null||g.addEventListener("click",()=>{Ie(e),confirm("Abandonner ? Résultat : défaite 3-0")&&(t.homeScore=0,t.aiScore=3,se(e,t,i))}),(x=document.getElementById("view-ai"))==null||x.addEventListener("click",()=>Si(t,i)),(m=document.getElementById("toggle-history"))==null||m.addEventListener("click",()=>{var h;(h=document.getElementById("match-history-panel"))==null||h.classList.add("open")}),(_=document.getElementById("close-history"))==null||_.addEventListener("click",()=>{var h;(h=document.getElementById("match-history-panel"))==null||h.classList.remove("open")}),(b=document.getElementById("btn-action"))==null||b.addEventListener("click",()=>{t.selected.length!==0&&(a?Ti(e,t,i):d&&Li(e,t,i))}),(y=document.getElementById("btn-results"))==null||y.addEventListener("click",()=>se(e,t,i)),e.querySelectorAll(".match-slot-hit").forEach(h=>{h.addEventListener("click",()=>Ii(h,t,e,i))}),e.querySelectorAll(".gc-mini").forEach(h=>{h.addEventListener("click",()=>Ci(h.dataset.gcId,h.dataset.gcType,e,t,i))}),(w=document.getElementById("boost-card"))==null||w.addEventListener("click",()=>Bi(e,t,i)),e.querySelectorAll(".sub-btn-col").forEach(h=>{h.addEventListener("click",()=>nt(e,t,i,h.dataset.subId))}),(L=document.getElementById("sub-btn-main"))==null||L.addEventListener("click",()=>nt(e,t,i))}function Ii(e,t,i,r){const c=e.dataset.cardId,o=e.dataset.role,n=t.selected.findIndex(l=>l.cardId===c);if(n!==-1)t.selected.splice(n,1);else{if(t.selected.length>=3){r.toast("Maximum 3 joueurs","error");return}const l=(t.homeTeam[o]||[]).find(a=>a.cardId===c);l&&t.selected.push({...l,_role:o,_line:o})}V(i,t,r)}function He(e,t,i){e.matchId&&k.from("matches").update({last_player_id:i}).eq("id",e.matchId).then(()=>{})}function Ti(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),He(t,i,i.state.profile.id);const r=t.selected.map(o=>({...o,_line:o._role})),c=ct(r,t.modifiers.home);t.pendingAttack={...c,players:[...t.selected],side:"home"},t.selected.forEach(o=>{const n=(t.homeTeam[o._role]||[]).find(l=>l.cardId===o.cardId);n&&(n.used=!0)}),t.log.push({text:`⚔️ Vous attaquez : ${c.total} (base ${c.base}${c.links?` +${c.links} liens`:""}) — ${t.selected.map(o=>o.name).join(", ")}`,type:"info"}),t.selected=[],t.modifiers.home={},t.phase="ai-defense",V(e,t,i),setTimeout(()=>Mi(e,t,i),1200)}function Li(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),He(t,i,i.state.profile.id);const r=t.selected.map(l=>({...l,_line:l._role})),c=pt(r,t.modifiers.home);t.selected.forEach(l=>{const a=(t.homeTeam[l._role]||[]).find(d=>d.cardId===l.cardId);a&&(a.used=!0)});const o=ut(t.pendingAttack.total,c.total,t.modifiers.home),n={type:"duel",title:"Défense",aiPlayers:(t.pendingAttack.players||[]).map(l=>({name:l.name,note:l._line==="MIL"?l.note_m:l.note_a,portrait:J(l),job:l.job,country_code:l.country_code,rarity:l.rarity,clubName:l.clubName,clubLogo:l.clubLogo})),homePlayers:t.selected.map(l=>{const a=(t.homeTeam[l._role]||[]).find(d=>d.cardId===l.cardId)||l;return{name:a.name,note:a._line==="GK"?a.note_g:a._line==="MIL"?a.note_m:a.note_d,portrait:J(a),job:a.job,country_code:a.country_code,rarity:a.rarity,clubName:a.clubName,clubLogo:a.clubLogo}}),homeTotal:c.total,aiTotal:t.pendingAttack.total,isGoal:!1,homeScored:!1,text:""};if(o.shielded)n.text="🛡️ Bouclier ! But annulé.",t.log.push(n);else if(o.goal){t.aiScore++,n.isGoal=!0,n.homeScored=!1,n.text=`⚽ BUT IA ! (${t.pendingAttack.total} > ${c.total})`,t.log.push(n),t.selected=[],t.modifiers.home={},t.pendingAttack=null,V(e,t,i),Ke(n.aiPlayers,t.homeScore,t.aiScore,!1,()=>{ye(e,t,i,"home-attack")});return}else n.text=`🧤 Défense réussie ! (${c.total} ≥ ${t.pendingAttack.total})`,t.log.push(n);t.selected=[],t.modifiers.home={},t.pendingAttack=null,ye(e,t,i,"home-attack")}function Fe(e,t,i){He(t,i,null);const r=[...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]],c=ft(r,"attack",t.difficulty);if(!c.length){It(e,t,i);return}const o=ct(c,t.modifiers.ai);t.pendingAttack={...o,players:c,side:"ai"},c.forEach(d=>{d.used=!0}),t.log.push({text:`🤖 IA attaque : ${o.total} (${c.map(d=>d.name).join(", ")})`,type:"info"}),t.modifiers.ai={};const n=[...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(d=>!d.used),a=(t.homeSubs||[]).filter(d=>!(t.usedSubIds||[]).includes(d.cardId)).length>0&&t.subsUsed<t.maxSubs;if(n.length===0&&!a){t.aiScore++;const d={type:"duel",isGoal:!0,homeScored:!1,aiPlayers:c.map(s=>({name:s.name,note:s._line==="MIL"?s.note_m:s.note_a,portrait:J(s),job:s.job,country_code:s.country_code,rarity:s.rarity,clubName:s.clubName,clubLogo:s.clubLogo})),aiTotal:o.total,text:"⚽ BUT IA ! (aucun défenseur disponible)"};t.log.push(d),t.pendingAttack=null,V(e,t,i),Ke(d.aiPlayers,t.homeScore,t.aiScore,!1,()=>{ye(e,t,i,"home-attack")});return}t.phase="defense",V(e,t,i)}function Mi(e,t,i){const r=[...t.aiTeam.GK||[],...t.aiTeam.DEF||[],...t.aiTeam.MIL||[]],c=ft(r,"defense",t.difficulty),o=c.length>0?pt(c,t.modifiers.ai).total:0;c.forEach(a=>{a.used=!0});const n=ut(t.pendingAttack.total,o,t.modifiers.ai),l={type:"duel",title:"Attaque",homePlayers:(t.pendingAttack.players||[]).map(a=>({name:a.name,note:a._line==="MIL"?a.note_m:a.note_a,portrait:J(a),job:a.job,country_code:a.country_code,rarity:a.rarity,clubName:a.clubName,clubLogo:a.clubLogo})),aiPlayers:c.map(a=>({name:a.name,note:a._line==="GK"?a.note_g:a._line==="MIL"?a.note_m:a.note_d,portrait:J(a),job:a.job,country_code:a.country_code,rarity:a.rarity,clubName:a.clubName,clubLogo:a.clubLogo})),homeTotal:t.pendingAttack.total,aiTotal:o,isGoal:!1,homeScored:!1,text:""};if(n.shielded)l.text="🛡️ Bouclier IA !",t.log.push(l);else if(n.goal){t.homeScore++,l.isGoal=!0,l.homeScored=!0,l.text=`⚽ BUT ! (${t.pendingAttack.total} > ${o})`,t.log.push(l),t.modifiers.ai={},t.pendingAttack=null,V(e,t,i),Ke(l.homePlayers,t.homeScore,t.aiScore,!0,()=>{ye(e,t,i,"ai-attack")});return}else l.text=`🧤 IA défend (${o} ≥ ${t.pendingAttack.total})`,t.log.push(l);t.modifiers.ai={},t.pendingAttack=null,ye(e,t,i,"ai-attack")}function ye(e,t,i,r){if(t.round++,At(t)){se(e,t,i);return}if(r==="home-attack"){if(![...t.homeTeam.MIL||[],...t.homeTeam.ATT||[]].filter(o=>!o.used).length){if(![...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(n=>!n.used).length){se(e,t,i);return}t.phase="ai-attack",V(e,t,i),setTimeout(()=>Fe(e,t,i),800);return}t.phase="attack",V(e,t,i)}else{if(![...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]].filter(o=>!o.used).length){It(e,t,i);return}t.phase="ai-attack",V(e,t,i),setTimeout(()=>Fe(e,t,i),800)}}function At(e){const t=["MIL","ATT","GK","DEF"].some(r=>(e.homeTeam[r]||[]).some(c=>!c.used)),i=["MIL","ATT","GK","DEF"].some(r=>(e.aiTeam[r]||[]).some(c=>!c.used));return!t&&!i}function It(e,t,i){At(t)?se(e,t,i):(t.phase="attack",V(e,t,i))}function zi(e,t,i){const r=document.createElement("div");r.style.cssText=`
    position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:800;
    display:flex;flex-direction:column;align-items:center;justify-content:center;gap:20px;
    animation:subFadeIn 0.2s ease;
  `;const c=J(e),o=J(t),n=ue[e.job]||"#555",l=ue[t.job]||"#555",a=e.job==="GK"?e.note_g:e.job==="DEF"?e.note_d:e.job==="MIL"?e.note_m:e.note_a,d=t.job==="GK"?t.note_g:t.job==="DEF"?t.note_d:t.job==="MIL"?t.note_m:t.note_a;r.innerHTML=`
    <style>
  ...
  `,document.body.appendChild(r);let s=!1;const p=()=>{s||(s=!0,r.remove(),i())};r.addEventListener("click",p),setTimeout(p,2e3)}function ge(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function nt(e,t,i,r=null){if(t.phase!=="attack"){ge("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){ge(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const c=Object.values(t.homeTeam).flat().filter(p=>p.used),o=t.homeSubs.filter(p=>!t.usedSubIds.includes(p.cardId));if(!c.length){ge("Aucun joueur utilisé à remplacer");return}if(!o.length){ge("Aucun remplaçant disponible");return}let n=0,l=Math.max(0,o.findIndex(p=>p.cardId===r)),a=!1;const d=document.createElement("div");d.id="sub-overlay",d.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function s(){var _,b,y,w,L,h;const p=c[n],u=o[l],f=Math.min(130,Math.round((window.innerWidth-90)/2)),g=Math.round(f*1.35),x=v=>`background:rgba(255,255,255,0.12);border:none;color:${v?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${v?"default":"pointer"};flex-shrink:0`;d.innerHTML=`
    ...
    </div>`,(_=d.querySelector("#sub-close"))==null||_.addEventListener("click",()=>d.remove()),(b=d.querySelector("#out-up"))==null||b.addEventListener("click",()=>{n>0&&(n--,s())}),(y=d.querySelector("#out-down"))==null||y.addEventListener("click",()=>{n<c.length-1&&(n++,s())}),(w=d.querySelector("#in-up"))==null||w.addEventListener("click",()=>{l>0&&(l--,s())}),(L=d.querySelector("#in-down"))==null||L.addEventListener("click",()=>{l<o.length-1&&(l++,s())});const m=(v,$,I,T)=>{const B=d.querySelector("#"+v);if(!B)return;let S=0;B.addEventListener("touchstart",q=>{S=q.touches[0].clientY},{passive:!0}),B.addEventListener("touchend",q=>{const K=q.changedTouches[0].clientY-S;if(Math.abs(K)<30)return;const D=$();K<0&&D<T-1?(I(D+1),s()):K>0&&D>0&&(I(D-1),s())},{passive:!0})};m("in-panel",()=>l,v=>l=v,o.length),m("out-panel",()=>n,v=>n=v,c.length),(h=d.querySelector("#sub-confirm"))==null||h.addEventListener("click",v=>{if(v.preventDefault(),v.stopPropagation(),a)return;a=!0;const $=c[n],I=o[l];if(!$||!I)return;let T=null,B=-1;for(const[q,K]of Object.entries(t.homeTeam)){const D=(K||[]).findIndex(W=>W.cardId===$.cardId);if(D!==-1){T=q,B=D;break}}if(B===-1||!T){ge("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),d.remove();return}const S={...I,_line:T,_col:$._col||0,used:!1,boost:0};t.homeTeam[T].splice(B,1,S),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(I.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:$.name,firstname:$.firstname,note:U($,T),portrait:J($),job:$.job,country_code:$.country_code,rarity:$.rarity,clubName:$.clubName,clubLogo:$.clubLogo},inPlayer:{name:I.name,firstname:I.firstname,note:U(I,T),portrait:J(I),job:I.job,country_code:I.country_code,rarity:I.rarity,clubName:I.clubName,clubLogo:I.clubLogo},text:`🔄 ${I.firstname} ${I.name} remplace ${$.firstname} ${$.name}`}),d.remove(),zi($,I,()=>V(e,t,i))})}document.body.appendChild(d),s()}function Ci(e,t,i,r,c){var g,x;const o=(r.gcDefs||[]).find(m=>m.name===t),n=le[t]||{icon:"⚡",desc:"Carte spéciale."},l={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[o==null?void 0:o.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",a={purple:"#b06ce0",light_blue:"#00d4ef"}[o==null?void 0:o.color]||"#b06ce0",d=(o==null?void 0:o.name)||t,s=(o==null?void 0:o.effect)||n.desc,p=o!=null&&o.image_url?`/manager-wars/icons/${o.image_url}`:null,u=n.icon||"⚡",f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",f.innerHTML=`
    ...
    </div>`,document.body.appendChild(f),(g=f.querySelector("#gc-back"))==null||g.addEventListener("click",()=>f.remove()),(x=f.querySelector("#gc-use"))==null||x.addEventListener("click",()=>{f.remove(),ji(e,t,i,r,c)})}function ze(e,t,i,r,c,o){const n=document.createElement("div");n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let l=[];function a(){var d,s;n.innerHTML=`
    ...
    </div>`,(d=n.querySelector("#gc-picker-close"))==null||d.addEventListener("click",()=>n.remove()),n.querySelectorAll(".gc-pick-item").forEach(p=>{p.addEventListener("click",()=>{const u=p.dataset.cid,f=e.find(x=>x.cardId===u);if(!f)return;const g=l.findIndex(x=>x.cardId===u);g>-1?l.splice(g,1):l.length<t&&l.push(f),a()})}),(s=n.querySelector("#gc-picker-confirm"))==null||s.addEventListener("click",()=>{n.remove(),o(l)})}a(),document.body.appendChild(n)}const Tt={BOOST_STAT({value:e=2,target:t="home",count:i=1,roles:r=null},c,o,n){const l=t==="home"?c.homeTeam:c.aiTeam,a=Object.entries(l).flatMap(([d,s])=>!r||r.includes(d)?(s||[]).filter(p=>!p.used).map(p=>({...p,_line:d})):[]);return a.length?(ze(a,i,e>0?`⚡ +${e} à ${i} joueur(s)`:`💀 -${Math.abs(e)} à ${i} joueur(s)`,o,c,d=>{d.forEach(s=>{for(const p of["GK","DEF","MIL","ATT"]){const u=(c.homeTeam[p]||[]).find(f=>f.cardId===s.cardId)||(c.aiTeam[p]||[]).find(f=>f.cardId===s.cardId);if(u){u.boost=(u.boost||0)+e;break}}}),c.log.push({text:`${e>0?"⚡":"💀"} ${Math.abs(e)>0?"+":""}${e} → ${d.map(s=>s.name).join(", ")}`,type:"info"}),V(o,c,n)}),!0):(n.toast("Aucun joueur disponible","error"),!0)},DEBUFF_STAT(e,t,i,r){return Tt.BOOST_STAT({...e,value:-Math.abs(e.value||2)},t,i,r)},GRAY_PLAYER({target:e="opponent",count:t=1,roles:i=null},r,c,o){const n=e==="opponent"?r.aiTeam:r.homeTeam,l=Object.entries(n).flatMap(([a,d])=>!i||i.includes(a)?(d||[]).filter(s=>!s.used).map(s=>({...s,_line:a})):[]);return l.length?(ze(l,t,`❄️ Griser ${t} joueur(s)`,c,r,a=>{a.forEach(d=>{for(const s of Object.keys(r.aiTeam).concat(Object.keys(r.homeTeam))){const u=((e==="opponent"?r.aiTeam[s]:r.homeTeam[s])||[]).find(f=>f.cardId===d.cardId);if(u){u.used=!0;break}}}),r.log.push({text:`❄️ ${a.map(d=>d.name).join(", ")} grisé(s) !`,type:"info"}),V(c,r,o)}),!0):(o.toast("Aucun joueur à griser","error"),!0)},REVIVE_PLAYER({count:e=1},t,i,r){const c=Object.entries(t.homeTeam).flatMap(([o,n])=>(n||[]).filter(l=>l.used).map(l=>({...l,_line:o})));return c.length?(ze(c,e,"💫 Ressusciter",i,t,o=>{o.forEach(n=>{for(const l of["GK","DEF","MIL","ATT"]){const a=(t.homeTeam[l]||[]).find(d=>d.cardId===n.cardId);if(a){a.used=!1;break}}}),t.log.push({text:`💫 ${o.map(n=>n.name).join(", ")} ressuscité(s) !`,type:"info"}),V(i,t,r)}),!0):(r.toast("Aucun joueur à ressusciter","error"),!0)},REMOVE_GOAL(e,t,i,r){return t.aiScore<=0?(r.toast("Aucun but adverse","error"),!1):(t.aiScore--,t.log.push({text:"🚫 But adverse retiré !",type:"info"}),!1)},ADD_GOAL_DRAW(e,t,i,r){return t.modifiers=t.modifiers||{},t.modifiers.home=t.modifiers.home||{},t.modifiers.home.addGoalOnDraw=!0,t.log.push({text:"⚽ +1 but si duel nul activé !",type:"info"}),!1},ADD_SUB({count:e=1},t,i,r){return t.maxSubs+=e,t.log.push({text:`🔄 +${e} remplacement(s) débloqué(s)`,type:"info"}),!1}};function ji(e,t,i,r,c){if(r.usedGc.includes(e))return;r.usedGc.push(e);const o=(r.gcDefs||[]).find(l=>l.name===t);let n=!1;if(o!=null&&o.effect_type&&o.effect_type!=="CUSTOM"){const l=Tt[o.effect_type];l?l(o.effect_params||{},r,i,c)||(n=!0):(c.toast(`Effet "${o.effect_type}" non implémenté`,"error"),n=!0)}else{switch(t){case"Double attaque":r.modifiers.home.doubleAttack=!0,r.log.push({text:"⚡ Double attaque activée !",type:"info"});break;case"Bouclier":r.modifiers.home.shield=!0,r.log.push({text:"🛡️ Bouclier activé !",type:"info"});break;case"Ressusciter":{const l=Object.entries(r.homeTeam).flatMap(([a,d])=>(d||[]).filter(s=>s.used).map(s=>({...s,_line:a})));l.length?(l[0].used=!1,r.log.push({text:`💫 ${l[0].name} ressuscité !`,type:"info"})):r.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"});break}case"Vol de note":r.modifiers.ai.stolenNote=(r.modifiers.ai.stolenNote||0)+1,r.log.push({text:"🎯 -1 à la prochaine attaque IA",type:"info"});break;case"Gel":{const l=[...r.aiTeam.ATT||[],...r.aiTeam.MIL||[]].filter(a=>!a.used);if(l.length){const a=l.sort((d,s)=>U(s,"ATT")-U(d,"ATT"))[0];a.used=!0,r.log.push({text:`❄️ ${a.name} (IA) gelé !`,type:"info"})}break}case"Remplacement+":r.maxSubs++,r.log.push({text:"🔄 +1 remplacement débloqué",type:"info"});break}n=!0}k.from("cards").delete().eq("id",e).then(()=>{}),n&&V(i,r,c)}function Bi(e,t,i){const r=Object.values(t.homeTeam).flat().filter(c=>!c.used);if(!r.length){i.toast("Aucun joueur actif à booster","error");return}i.openModal("⚡ Utiliser le Boost",`<div style="margin-bottom:12px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border-radius:10px;padding:12px;text-align:center;color:#000">
    ...
    <div style="display:flex;flex-direction:column;gap:6px">
      ${r.map(c=>`
        <div class="player-boost-opt" data-card-id="${c.cardId}"
          ...
          <div style="width:32px;height:32px;background:${ue[c.job]||"#888"};border-radius:6px;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:900;font-size:13px">${U(c,c._line||c.job)}</div>
          <div style="flex:1"><b>${c.firstname} ${c.name}</b><div style="font-size:11px;color:#888">${c._line||c.job}</div></div>
          <div style="color:#87CEEB;font-weight:700">+${t.boostCard.value}</div>
        </div>`).join("")}
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(c=>{c.addEventListener("click",()=>{const o=c.dataset.cardId;for(const n of["GK","DEF","MIL","ATT"]){const l=(t.homeTeam[n]||[]).find(a=>a.cardId===o);if(l){l.boost=(l.boost||0)+t.boostCard.value,t.log.push({text:`⚡ Boost +${t.boostCard.value} appliqué à ${l.name}`,type:"info"});break}}t.boostUsed=!0,i.closeModal(),V(e,t,i)})})}function Ke(e,t,i,r,c){const o=document.createElement("div");o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:900;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;overflow:hidden;cursor:pointer";const n=Array.from({length:10},(d,s)=>`
  ...
  <div style="display:flex;gap:10px;animation:scoreIn 0.4s ease 1s both;position:relative;z-index:1">
    ${e.map(d=>`
    <div style="text-align:center">
      <div style="width:50px;height:50px;border-radius:50%;background:${ue[d.job]||"#555"};border:2px solid #FFD700;position:relative;overflow:hidden;margin:0 auto">
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(o);let l=!1;const a=()=>{l||(l=!0,o.remove(),setTimeout(()=>c(),50))};o.addEventListener("click",a),setTimeout(a,3500)}async function se(e,t,i){var s,p;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:r}=i,c=t.homeScore>t.aiScore,o=t.homeScore===t.aiScore,n=c?"victoire":o?"nul":"defaite",l=St(t.mode,n);t.matchId&&await k.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:c?r.profile.id:null,home_credits_reward:l,played_at:new Date().toISOString()}).eq("id",t.matchId);const a={credits:(r.profile.credits||0)+l,matches_played:(r.profile.matches_played||0)+1};c?a.wins=(r.profile.wins||0)+1:o?a.draws=(r.profile.draws||0)+1:a.losses=(r.profile.losses||0)+1,await k.from("users").update(a).eq("id",r.profile.id),await i.refreshProfile();const d=document.createElement("div");d.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",d.innerHTML=`
    ...
    </div>`,document.body.appendChild(d),(s=document.getElementById("res-home"))==null||s.addEventListener("click",()=>{d.remove(),Ie(e),i.navigate("home")}),(p=document.getElementById("res-replay"))==null||p.addEventListener("click",()=>{d.remove(),Ie(e),i.navigate("match",{matchMode:t.mode})})}function Si(e,t){t.openModal("Équipe adverse (IA)",`<div style="background:#0a3d1e;padding:12px;border-radius:8px">
      ${Pe(e.aiTeam,e.formation,null,[],300,300)}
    </div>`,`<button class="btn btn-primary" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Fi={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"};async function Di(e,t){const{state:i,toast:r}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement du marché...</div>',await Ue(e,t)}async function Ue(e,t){const{state:i,toast:r}=t,{data:c}=await k.from("market_listings").select(`id, price, status, listed_at, seller_id,
      seller:users!seller_id(pseudo),
      card:cards(id, card_type,
        player:players(id, firstname, surname_encoded, country_code, job, job2,
          note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length, clubs(encoded_name, logo_url)))`).eq("status","active").order("listed_at",{ascending:!1}).limit(60),o=(c||[]).filter(a=>a.seller_id===i.profile.id),n=(c||[]).filter(a=>a.seller_id!==i.profile.id);e.innerHTML=`
  <div class="page">
    <div class="page-header">
  ...
  `;function l(a){const d=document.getElementById("mkt-content"),s=a==="buy"?n:o;if(s.length===0){d.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      ${s.map(p=>{var m,_,b;const u=(m=p.card)==null?void 0:m.player;if(!u)return"";const f=u.job==="GK"?u.note_g:u.job==="DEF"?u.note_d:u.job==="MIL"?u.note_m:u.note_a,g=Fi[u.rarity],x=(i.profile.credits||0)>=p.price;return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px">
    ...
    </div>`,d.querySelectorAll("[data-buy]").forEach(p=>{p.addEventListener("click",()=>Gi(p.dataset.buy,s,e,t))}),d.querySelectorAll("[data-cancel]").forEach(p=>{p.addEventListener("click",()=>Ri(p.dataset.cancel,e,t))})}l("buy"),e.querySelectorAll(".mkt-tab").forEach(a=>{a.addEventListener("click",()=>{e.querySelectorAll(".mkt-tab").forEach(d=>{const s=d===a;d.style.background=s?"var(--green)":"#fff",d.style.color=s?"#fff":"var(--gray-600)",d.style.borderColor=s?"var(--green)":"var(--gray-200)"}),l(a.dataset.tab)})})}async function Gi(e,t,i,r){var p;const{state:c,toast:o}=r,n=t.find(u=>u.id===e);if(!n)return;const l=n.price,a=c.profile.credits||0,d=(p=n.card)==null?void 0:p.player;if(a<l){o("Crédits insuffisants","error");return}if(!confirm(`Acheter ${d==null?void 0:d.firstname} ${d==null?void 0:d.surname_encoded} pour ${l.toLocaleString("fr")} crédits ?`))return;const s=document.querySelector(`[data-buy="${e}"]`);s&&(s.disabled=!0,s.textContent="⏳");try{const{error:u}=await k.from("cards").update({owner_id:c.profile.id}).eq("id",n.card.id);if(u)throw new Error("Transfert carte : "+u.message);const{error:f}=await k.from("market_listings").update({status:"sold",buyer_id:c.profile.id,sold_at:new Date().toISOString()}).eq("id",e);if(f)throw new Error("Mise à jour annonce : "+f.message);const{error:g}=await k.from("users").update({credits:a-l}).eq("id",c.profile.id);if(g)throw new Error("Débit acheteur : "+g.message);const{data:x,error:m}=await k.from("users").select("credits").eq("id",n.seller_id).single();!m&&x&&await k.from("users").update({credits:(x.credits||0)+l}).eq("id",n.seller_id),await k.from("notifications").insert({user_id:n.seller_id,type:"card_sold",message:`Ta carte ${(d==null?void 0:d.surname_encoded)||"?"} a été vendue pour ${l.toLocaleString("fr")} crédits !`,data:{card_id:n.card.id,price:l}}).then(()=>{}).catch(()=>{}),c.profile.credits=a-l;const _=document.querySelector("[data-credits]")||document.querySelector(".credits-display");_&&(_.textContent=(a-l).toLocaleString("fr")+" cr."),o(`✅ ${d==null?void 0:d.surname_encoded} ajouté à ta collection !`,"success"),Ue(i,r)}catch(u){o("❌ "+u.message,"error"),s&&(s.disabled=!1,s.textContent="Acheter")}}async function Ri(e,t,i){const{toast:r}=i,{data:c}=await k.from("market_listings").select("card_id").eq("id",e).single();await k.from("market_listings").update({status:"cancelled"}).eq("id",e),c&&await k.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",c.card_id),r("Annonce retirée","success"),Ue(t,i)}function qi(e){return{GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[e]||"#888"}async function Ni(e,{state:t,navigate:i,toast:r}){e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:c}=await k.from("users").select("id,pseudo,club_name,trophies_top1,trophies_top2,trophies_top3,wins,level").order("trophies_top1",{ascending:!1}).limit(100);e.innerHTML=`
  <div class="page">
    <div class="page-header">
  ...
  `}async function Oi(e,{state:t,navigate:i,toast:r}){const c=t.profile;if(!c)return;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await k.from("matches").select("id,home_id,away_id,home_score,away_score,status,mode,winner_id,created_at,played_at").or(`home_id.eq.${c.id},away_id.eq.${c.id}`).order("created_at",{ascending:!1}).limit(50),n={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},l=(o||[]).filter(s=>s.status==="finished"),a=(o||[]).filter(s=>s.status==="in_progress");function d(s){const p=s.home_id===c.id;p?s.home_score:s.away_score,p?s.away_score:s.home_score;const u=s.winner_id===c.id,f=s.home_score===s.away_score&&s.status==="finished",g=s.status!=="finished"?"…":f?"N":u?"V":"D",x=s.status!=="finished"||f?"#888":u?"#1A6B3C":"#c0392b";let m=n[s.mode]||s.mode;s.away_id===null&&!m.startsWith("IA")&&(m="IA");let _="";s.status==="in_progress"&&Date.now()-new Date(s.created_at).getTime()>3600*1e3&&(_=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const b=new Date(s.created_at),y=b.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+b.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),w=s.status==="finished"?`${s.home_score} - ${s.away_score}`:`${s.home_score||0} - ${s.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
  ...
  </div>`}const R={user:null,profile:null,page:"home",params:{}};function $e(e,t="info",i=3e3){const r=document.getElementById("toast");r&&(r.textContent=e,r.className=`show ${t}`,clearTimeout(r._t),r._t=setTimeout(()=>{r.className=""},i))}function Pi(e,t,i=""){document.getElementById("modal-title").textContent=e,document.getElementById("modal-body").innerHTML=t,document.getElementById("modal-footer").innerHTML=i,document.getElementById("modal-overlay").classList.remove("hidden")}function De(){document.getElementById("modal-overlay").classList.add("hidden")}async function _e(){if(!R.user)return;const{data:e}=await k.from("users").select("*").eq("id",R.user.id).single();e&&(R.profile=e)}const Ve="mw_theme";function Te(){return localStorage.getItem(Ve)||"light"}function Hi(e){var t;localStorage.setItem(Ve,e),Ge(e),(t=R.profile)!=null&&t.id&&k.from("users").update({theme:e}).eq("id",R.profile.id).then(()=>{})}function Ge(e){document.documentElement.setAttribute("data-theme",e)}function ke(e,t={}){R.page=e,R.params=t,Lt()}async function Lt(){const e=document.getElementById("page-content");if(!e)return;document.querySelectorAll(".bottom-nav a").forEach(r=>{r.classList.toggle("active",r.dataset.page===R.page)});const t=document.getElementById("nav-credits");t&&R.profile&&(t.textContent=`💰 ${(R.profile.credits||0).toLocaleString("fr")}`);const i={state:R,navigate:ke,toast:$e,openModal:Pi,closeModal:De,refreshProfile:_e};switch(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽</div>',R.page){case"home":await Je(e,i);break;case"collection":await Gt(e,i);break;case"decks":await Ce(e,i);break;case"boosters":await ii(e,i);break;case"match":await wi(e,i);break;case"market":await Di(e,i);break;case"rankings":await Ni(e,i);break;case"matches":await Oi(e,i);break;default:await Je(e,i)}}function Ki(){const e=document.getElementById("app"),t=R.profile;if(!t)return;const i="/manager-wars/icons/";e.innerHTML=`
    <nav class="top-nav">
  ...
  `,document.querySelectorAll(".bottom-nav a").forEach(r=>{r.addEventListener("click",c=>{c.preventDefault(),ke(r.dataset.page)})}),document.getElementById("nav-logo").addEventListener("click",()=>ke("home")),document.getElementById("nav-credits").addEventListener("click",()=>ke("boosters")),document.getElementById("theme-toggle").addEventListener("click",()=>{const c=Te()==="dark"?"light":"dark";Hi(c),rt(c)}),rt(Te())}function rt(e){const t=document.getElementById("theme-icon");t&&(t.textContent=e==="dark"?"☀️":"🌙")}async function Ui(){Ge(Te()),document.getElementById("modal-overlay").addEventListener("click",t=>{t.target===t.currentTarget&&De()}),document.getElementById("modal-close").addEventListener("click",De);const{data:{session:e}}=await k.auth.getSession();if(!e){st(),We(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:$e});return}R.user=e.user,await _e(),st();try{const{data:t}=await k.from("formation_links_overrides").select("formation, links"),i={};(t||[]).forEach(r=>{i[r.formation]=r.links}),zt(i)}catch(t){console.warn("Impossible de charger les overrides de formation:",t)}if(!R.profile){Ct(document.getElementById("app"),{state:R,navigate:async()=>{await _e(),at()},toast:$e,refreshProfile:_e});return}R.profile.theme&&R.profile.theme!==Te()&&(localStorage.setItem(Ve,R.profile.theme),Ge(R.profile.theme)),at(),k.auth.onAuthStateChange(async(t,i)=>{t==="SIGNED_OUT"&&(R.user=null,R.profile=null,document.getElementById("app").innerHTML="",We(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:$e}))})}function at(){const e=document.getElementById("app");e.style.display="flex",e.style.flexDirection="column",Ki(),Lt()}function st(){const e=document.getElementById("app-loader"),t=document.getElementById("app");t&&(t.style.display=""),e&&(e.classList.add("zoom-out"),setTimeout(()=>e.style.display="none",500))}Ui();
