...
import{s as A,F as _i,i as Ht,k as Zi,l as nt,m as ji,f as Pt,n as Lo,T as ci,j as Io,o as Mo,b as jo}from"./formation-links-5A-YoKwp.js";const Co="/";function Ci(e,{navigate:t,toast:i}){let n="login";const d=()=>{var r,c,f,a,o,l;const s=n==="login";e.innerHTML=`
    <div style="
  ...
  `;function c(){var $;const a=document.getElementById("logo-preview"),o=document.getElementById("logo-initials"),l=(($=document.getElementById("setup-club"))==null?void 0:$.value)||"MW",b=l.trim().split(" ").filter(Boolean),h=b.length>=2?(b[0][0]+b[1][0]).toUpperCase():l.slice(0,2).toUpperCase();a&&(a.style.background=r,a.style.borderColor=s),o&&(o.textContent=h,o.style.color=s)}document.getElementById("color1").addEventListener("input",a=>{s=a.target.value,document.getElementById("swatch1").style.background=s,c()}),document.getElementById("color2").addEventListener("input",a=>{r=a.target.value,document.getElementById("swatch2").style.background=r,c()});function f(a){document.querySelectorAll(".setup-step").forEach(o=>o.classList.remove("active")),document.getElementById(`step-${a}`).classList.add("active"),document.getElementById("step-num").textContent=a,document.getElementById("progress-fill").style.width=`${Math.round(a/3*100)}%`,a===3&&c()}document.getElementById("step1-next").addEventListener("click",async()=>{const a=document.getElementById("setup-pseudo").value.trim(),o=document.getElementById("step1-error");if(o.textContent="",a.length<3){o.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:l}=await A.from("users").select("id").eq("pseudo",a).maybeSingle();if(l){o.textContent="Ce pseudo est déjà pris.";return}f(2)}),document.getElementById("step2-back").addEventListener("click",()=>f(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const a=document.getElementById("setup-club").value.trim(),o=document.getElementById("step2-error");if(o.textContent="",a.length<2){o.textContent="Nom trop court (min. 2 caractères).";return}const{data:l}=await A.from("users").select("id").eq("club_name",a).maybeSingle();if(l){o.textContent="Ce nom de club est déjà pris.";return}f(3)}),document.getElementById("step3-back").addEventListener("click",()=>f(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const a=document.getElementById("setup-pseudo").value.trim(),o=document.getElementById("setup-club").value.trim(),l=document.getElementById("step3-error"),b=document.getElementById("step3-finish");l.textContent="",b.disabled=!0,b.textContent="Création en cours…";try{const{error:h}=await A.from("users").insert({id:t.user.id,pseudo:a,club_name:o,club_color1:s,club_color2:r,credits:1e4});if(h)throw h;await Bo(t.user.id),await d(),n(`Bienvenue ${a} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(h){l.textContent=h.message,b.disabled=!1,b.textContent="🚀 Créer mon profil !"}})}async function Bo(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await A.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const Fo="modulepreload",Do=function(e){return"/"+e},qi={},ni=function(t,i,n){let d=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const r=document.querySelector("meta[property=csp-nonce]"),c=(r==null?void 0:r.nonce)||(r==null?void 0:r.getAttribute("nonce"));d=Promise.allSettled(i.map(f=>{if(f=Do(f),f in qi)return;qi[f]=!0;const a=f.endsWith(".css"),o=a?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${f}"]${o}`))return;const l=document.createElement("link");if(l.rel=a?"stylesheet":Fo,a||(l.as="script"),l.crossOrigin="",l.href=f,c&&l.setAttribute("nonce",c),document.head.appendChild(l),a)return new Promise((b,h)=>{l.addEventListener("load",b),l.addEventListener("error",()=>h(new Error(`Unable to preload CSS for ${f}`)))})}))}function s(r){const c=new Event("vite:preloadError",{cancelable:!0});if(c.payload=r,window.dispatchEvent(c),!c.defaultPrevented)throw r}return d.then(r=>{for(const c of r||[])c.status==="rejected"&&s(c.reason);return t().catch(s)})},Nt="#1A6B3C",Rt="#cc2222",Go="#D4A017",Bi="#888";async function Po(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    </div>`,await Qi(i,n,t),document.getElementById("btn-add-friend").addEventListener("click",()=>Ro(i,n)),document.getElementById("btn-accept-friend").addEventListener("click",()=>eo(i,n,null,t))}async function Qi(e,t,i={}){const{navigate:n}=i,d=e.user.id,{data:s,error:r}=await A.from("friendships").select("id, requester_id, addressee_id").eq("status","accepted").or(`requester_id.eq.${d},addressee_id.eq.${d}`),{count:c}=await A.from("friendships").select("id",{count:"exact",head:!0}).eq("addressee_id",d).eq("status","pending"),f=document.getElementById("pending-badge");f&&(c>0?(f.style.display="flex",f.textContent=c):f.style.display="none");const a=document.getElementById("friends-list");if(!a)return;if(r){console.error("[Friends] Erreur:",r),a.innerHTML=`<div style="color:${Rt};text-align:center;padding:16px">Erreur chargement : ${r.message}</div>`;return}const o=(s||[]).map(h=>h.requester_id===d?h.addressee_id:h.requester_id);let l={};if(o.length){const{data:h}=await A.from("users").select("id, pseudo, club_name, last_seen_at, club_color1, club_color2").in("id",o);(h||[]).forEach($=>{l[$.id]=$})}const b=(s||[]).map(h=>({friendshipId:h.id,friend:l[h.requester_id===d?h.addressee_id:h.requester_id]||{pseudo:"?"}}));if(!b.length){a.innerHTML=`
    ...
    ${ki()}`,document.body.appendChild(i);const n=i.querySelector("#friend-pseudo-input"),d=i.querySelector("#add-friend-error"),s=()=>i.remove();n.focus(),i.querySelector("#add-cancel").addEventListener("click",s),i.addEventListener("click",r=>{r.target===i&&s()}),i.querySelector("#add-ok").addEventListener("click",async()=>{const r=n.value.trim();if(!r){d.textContent="Entre un pseudo";return}d.textContent="";const{data:c}=await A.from("users").select("id, pseudo").ilike("pseudo",r).single();if(!c){d.textContent="Utilisateur introuvable";return}if(c.id===e.user.id){d.textContent="Tu ne peux pas t'ajouter toi-même";return}const{data:f}=await A.from("friendships").select("id, status").or(`and(requester_id.eq.${e.user.id},addressee_id.eq.${c.id}),and(requester_id.eq.${c.id},addressee_id.eq.${e.user.id})`).single();if(f){const o=f.status==="accepted"?"Vous êtes déjà amis !":f.status==="pending"?"Demande déjà envoyée":"Une demande existe déjà";d.textContent=o;return}const{error:a}=await A.from("friendships").insert({requester_id:e.user.id,addressee_id:c.id,status:"pending"});if(a){d.textContent="Erreur : "+a.message;return}s(),t(`✅ Demande envoyée à ${c.pseudo} !`,"success")})}async function eo(e,t,i=null,n={}){const d=e.user.id,{data:s}=await A.from("friendships").select("id, requester_id").eq("addressee_id",d).eq("status","pending").order("created_at",{ascending:!1}),r=(s||[]).map(b=>b.requester_id);let c={};if(r.length){const{data:b}=await A.from("users").select("id, pseudo, club_name").in("id",r);(b||[]).forEach(h=>{c[h.id]=h})}const f=(s||[]).map(b=>({...b,requester:c[b.requester_id]||{pseudo:"?"}})),a=$i(),o=f||[];a.innerHTML=`
    <div class="popup-box">
    ...
    ${ki()}`,document.body.appendChild(a);const l=()=>a.remove();a.querySelector("#pending-close").addEventListener("click",l),a.addEventListener("click",b=>{b.target===a&&l()}),a.querySelectorAll("[data-accept]").forEach(b=>{b.addEventListener("click",async()=>{const{error:h}=await A.from("friendships").update({status:"accepted"}).eq("id",b.dataset.accept);if(h){t("Erreur : "+h.message,"error");return}b.closest("div[style]").remove(),t("✅ Ami accepté !","success"),Qi(e,t,n),i&&i()})}),a.querySelectorAll("[data-decline]").forEach(b=>{b.addEventListener("click",async()=>{await A.from("friendships").delete().eq("id",b.dataset.decline),b.closest("div[style]").remove(),t("Demande refusée","info"),i&&i()})})}async function Oo(e,t,i){const n=e.user.id,[d,s]=[n,t].sort(),r=n===d,{data:c}=await A.from("friend_match_stats").select("*").eq("player1_id",d).eq("player2_id",s).single(),f=e.profile.club_name||e.profile.pseudo||"Moi",a=c||{},o=r?a.wins_p1||0:a.wins_p2||0,l=r?a.wins_p2||0:a.wins_p1||0,b=a.draws||0,h=r?a.goals_p1||0:a.goals_p2||0,$=r?a.goals_p2||0:a.goals_p1||0,p=r?a.gc_used_p1||0:a.gc_used_p2||0,u=r?a.gc_used_p2||0:a.gc_used_p1||0,m=a.matches_total||0,T=(G,q,B,C=Nt,N=Rt)=>`
    ...
    </style>`}async function to({player1Id:e,player2Id:t,score1:i,score2:n,gc1:d,gc2:s}){const[r,c]=[e,t].sort(),f=e!==r,a=f?n:i,o=f?i:n,l=f?s:d,b=f?d:s,h=a>o?1:0,$=o>a?1:0,p=a===o?1:0,{data:u}=await A.from("friend_match_stats").select("*").eq("player1_id",r).eq("player2_id",c).single();u?await A.from("friend_match_stats").update({wins_p1:u.wins_p1+h,wins_p2:u.wins_p2+$,draws:u.draws+p,goals_p1:u.goals_p1+a,goals_p2:u.goals_p2+o,gc_used_p1:u.gc_used_p1+l,gc_used_p2:u.gc_used_p2+b,matches_total:u.matches_total+1}).eq("player1_id",r).eq("player2_id",c):await A.from("friend_match_stats").insert({player1_id:r,player2_id:c,wins_p1:h,wins_p2:$,draws:p,goals_p1:a,goals_p2:o,gc_used_p1:l,gc_used_p2:b,matches_total:1})}const Ho="2026.07.05-1317";async function Fi(e,{state:t,navigate:i,toast:n}){var s,r;const d=t.profile;d&&(e.innerHTML=`
  <style>
  ...
  `,e.querySelectorAll("[data-nav]").forEach(c=>{c.addEventListener("click",f=>{f.preventDefault(),i(c.dataset.nav)})}),(s=document.getElementById("nav-rankings"))==null||s.addEventListener("click",()=>i("rankings")),(r=document.getElementById("nav-matches"))==null||r.addEventListener("click",()=>i("matches")),e.querySelectorAll("[data-action]").forEach(c=>{c.addEventListener("click",()=>{c.classList.add("tapped"),setTimeout(()=>c.classList.remove("tapped"),200);const f=c.dataset.action;if(f==="match-ai"){Yo(i);return}if(f==="match-random"){i("match",{matchMode:"random"});return}if(f==="match-friend"){i("friends");return}if(f==="mini-league"){i("mini-league");return}if(f==="ranked"){i("ranked");return}n("Bientôt disponible","info")})}),window.innerWidth<768&&requestAnimationFrame(()=>{var G,q,B,C,N,J;const c=((G=window.visualViewport)==null?void 0:G.height)||window.innerHeight,f=((q=document.querySelector(".top-nav"))==null?void 0:q.offsetHeight)||56,a=((B=document.querySelector(".bottom-nav"))==null?void 0:B.offsetHeight)||60,o=((C=e.querySelector(".hero-compact"))==null?void 0:C.offsetHeight)||52,l=e.querySelector(".ranked-banner");e.querySelector(".play-grid");const b=((J=(N=e.querySelector("#logout-btn"))==null?void 0:N.closest("div"))==null?void 0:J.offsetHeight)||44,h=["friend-requests-banner","match-invite-banner","ongoing-match-banner"].reduce((Z,se)=>{var re;return Z+(((re=document.getElementById(se))==null?void 0:re.offsetHeight)||0)},0),$=14*5,p=c-f-a-o-b-h-$,u=Math.max(80,Math.round(p*.28)),m=Math.max(160,Math.round(p*.72)),T=Math.floor((m-10)/2);l&&(l.style.minHeight=l.style.maxHeight=u+"px"),e.querySelectorAll(".play-grid .play-card").forEach(Z=>{Z.style.minHeight=Z.style.height=T+"px"});const M=Math.round(T*.55);e.querySelectorAll(".play-card .play-icon").forEach(Z=>{Z.style.height=M+"px"})}),document.getElementById("logout-btn").addEventListener("click",async()=>{await A.auth.signOut(),window.location.reload()}),oo(t,n),Vo(t,n,i),io(t,n,i))}async function io(e,t,i){const n=document.getElementById("ongoing-match-banner");if(!n)return;const d=e.profile.id,{data:s}=await A.from("matches").select("id, home_id, away_id, status, mode").eq("status","active").or(`home_id.eq.${d},away_id.eq.${d}`).order("created_at",{ascending:!1});if(!(s!=null&&s.length)){n.innerHTML="";return}const r=s.map(f=>f.home_id===d?f.away_id:f.home_id).filter(Boolean);let c={};if(r.length){const{data:f}=await A.from("users").select("id, pseudo, club_name").in("id",r);(f||[]).forEach(a=>{c[a.id]=a.club_name||a.pseudo})}n.innerHTML=s.map(f=>{const a=f.home_id===d?f.away_id:f.home_id,o=c[a]||"Adversaire",l=f.mode==="mini_league";return`
    ...
    </div>`,(a=document.getElementById("match-inv-accept"))==null||a.addEventListener("click",()=>{n.innerHTML="",i("match",{matchMode:"friend",friendId:r,friendName:s})}),(o=document.getElementById("match-inv-decline"))==null||o.addEventListener("click",async()=>{await A.from("friend_match_invites").update({status:"declined"}).eq("id",d.id),n.innerHTML="",t("Invitation refusée","info")})}async function oo(e,t){const i=document.getElementById("friend-requests-banner");if(!i)return;const{data:n,error:d}=await A.from("friendships").select("id, requester:users!requester_id(pseudo, club_name)").eq("addressee_id",e.user.id).eq("status","pending");if(d||!(n!=null&&n.length)){i.innerHTML="";return}const s=n.length,r=n.slice(0,2).map(f=>{var a;return((a=f.requester)==null?void 0:a.pseudo)||"?"}).join(", "),c=s>2?` +${s-2}`:"";i.innerHTML=`
  ...
  </div>`}async function en(e,t){const{state:i,navigate:n,toast:d,openModal:s,closeModal:r}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:c}=await A.from("cards").select(`id, card_type, current_note, gc_type, formation, is_for_sale, sale_price, stadium_id, evolution_bonus,
      player:players(id, firstname, surname_encoded, country_code, club_id, job, job2,
        note_g, note_d, note_m, note_a, rarity, note_min, note_max, skin, hair, hair_length, sell_price,
        clubs(encoded_name, logo_url)),
      stadium_def:stadium_definitions(id, name, club_id, country_code, image_url,
        club:clubs(encoded_name, logo_url))`).eq("owner_id",i.profile.id),{data:f}=await A.from("players").select(`id, firstname, surname_encoded, country_code, club_id, job, job2,
      note_g, note_d, note_m, note_a, rarity, note_min, note_max, skin, hair, hair_length,
      clubs(encoded_name, logo_url)`).eq("is_active",!0),a=(c||[]).filter(V=>V.card_type==="player"&&V.player),o=(c||[]).filter(V=>V.card_type==="game_changer"),l=(c||[]).filter(V=>V.card_type==="formation"),b=(c||[]).filter(V=>V.card_type==="stadium"),{data:h}=await A.from("gc_definitions").select("name,gc_type,color,effect,image_url"),$={};(h||[]).forEach(V=>{$[V.name]=V});const{data:p}=await A.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),u={};(p||[]).forEach(V=>{u[V.id]=V}),b.forEach(V=>{V.stadium_def&&(u[V.stadium_id]=V.stadium_def)});const m=Object.keys(_i),T=Object.keys(Re),M={};a.forEach(V=>{const oe=V.player.id;M[oe]=(M[oe]||0)+1}),new Set(Object.keys(M).map(V=>String(V)));const G=new Set(l.map(V=>V.formation)),q=new Set(o.map(V=>V.gc_type));let B="player",C="Tous",N="",J=!1;function Z(){return[...a].sort((V,oe)=>{const fe=Yt.indexOf(V.player.job),K=Yt.indexOf(oe.player.job);return fe!==K?fe-K:(V.player.surname_encoded||"").localeCompare(oe.player.surname_encoded||"")})}function se(){return[...f||[]].sort((V,oe)=>{const fe=Yt.indexOf(V.job),K=Yt.indexOf(oe.job);return fe!==K?fe-K:(V.surname_encoded||"").localeCompare(oe.surname_encoded||"")})}function re(){return Z().filter(V=>{const oe=V.player,fe=C==="Tous"||oe.job===C,K=!N||`${oe.firstname} ${oe.surname_encoded}`.toLowerCase().includes(N);return fe&&K})}function Q(){return se().filter(V=>{const oe=C==="Tous"||V.job===C,fe=!N||`${V.firstname} ${V.surname_encoded}`.toLowerCase().includes(N);return oe&&fe})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
  ...
  </div>`;function U(){const V=document.getElementById("col-filters");V&&(B==="player"?(V.innerHTML=`
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",oe=>{N=oe.target.value.toLowerCase(),D()}),e.querySelectorAll(".filter-btn").forEach(oe=>{oe.addEventListener("click",()=>{C=oe.dataset.job,U(),D()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{J=!J,U(),D()})):(V.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{J=!J,U(),D()})))}function D(){const V=document.getElementById("col-grid");V&&(B==="player"?Le(V):B==="formation"?Fe(V):B==="stadium"?we(V):Ce(V))}function ne(V,oe,fe,K,te){const Y=document.getElementById("col-grid"),de=document.getElementById("col-big");if(!Y||!de)return;var ve=0;function Te(){const $e=window.innerWidth>=768,be=document.getElementById("col-big"),ke=document.getElementById("col-grid");$e&&be&&(be.style.minHeight="420px",be.style.flex="1 1 auto"),$e&&ke&&(ke.style.height=Math.round(310*.76+16)+"px",ke.style.flexShrink="0",ke.style.overflowX="auto",ke.style.overflowY="hidden",ke.style.alignItems="center",ke.style.padding="8px 16px"),de.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+oe(V[ve])+"</div>";var qe=de.querySelector("[data-card-id],[data-form-id],[data-gc-id]");qe&&qe.addEventListener("click",function(){K(V[ve])}),requestAnimationFrame(function(){var je=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!je||!de)){var w=de.clientHeight-8,z=de.clientWidth-24,g=je.offsetHeight,y=je.offsetWidth;if(g>0&&y>0&&w>40){var x=$e?2.2:1,_=Math.min(w/g,z/y,x);je.style.transform="scale("+_.toFixed(3)+")",je.style.transformOrigin="top center"}}}),Y.innerHTML=V.map(function(je,w){var z=w===ve,g="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+(z?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+w+'" style="'+g+'">'+fe(je,z)+"</div>"}).join(""),Y.querySelectorAll(".col-mini-item").forEach(function(je){je.addEventListener("click",function(){ve=Number(je.dataset.idx),Te(),je.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}Te()}function ue(V){var oe=window.innerWidth>=768?.76:.54,fe;if(!V||V._fake){var K=V?V.player:null;if(!K)return"";fe=Pi(K)}else fe=pi(V,"");return'<div style="display:inline-block;zoom:'+oe+';pointer-events:none;line-height:0">'+fe+"</div>"}function ye(V,oe,fe){oe=oe||100,fe=fe||140;var K=Ht[V]||{},te={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},Y=Math.max(3,Math.round(oe*.06)),de=Object.entries(K).map(function(Te){var $e=Te[0],be=Te[1],ke=$e.replace(/\d+$/,""),qe=te[ke]||"#888",je=Math.round(be.x*oe),w=Math.round(be.y*fe);return'<circle cx="'+je+'" cy="'+w+'" r="'+Y+'" fill="'+qe+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),ve=Math.max(1,Math.round(oe/50));return'<svg viewBox="0 0 '+oe+" "+fe+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+oe+'" height="'+fe+'" fill="#1A6B3C"/><rect x="'+Math.round(oe*.2)+'" y="'+Math.round(fe*.02)+'" width="'+Math.round(oe*.6)+'" height="'+Math.round(fe*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+ve+'"/><line x1="0" y1="'+Math.round(fe*.5)+'" x2="'+oe+'" y2="'+Math.round(fe*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+ve+'"/><ellipse cx="'+Math.round(oe*.5)+'" cy="'+Math.round(fe*.5)+'" rx="'+Math.round(oe*.18)+'" ry="'+Math.round(fe*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+ve+'"/><rect x="'+Math.round(oe*.2)+'" y="'+Math.round(fe*.82)+'" width="'+Math.round(oe*.6)+'" height="'+Math.round(fe*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+ve+'"/>'+de+"</svg>"}function me(V,oe,fe){var K=fe>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+fe+"</div>":"",te=oe?'data-form-id="'+oe.id+'"':"",Y=V.length>7?14:V.length>5?16:19,de=!!oe;return"<div "+te+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(de?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(de?"":"filter:grayscale(1);opacity:0.5")+'">'+K+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(de?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+Y+"px;font-weight:900;color:"+(de?"#1A6B3C":"#aaa")+';line-height:1">'+V+'</div></div><div style="flex:1;overflow:hidden;background:'+(de?"#1A6B3C":"#ccc")+'">'+ye(V,140,220)+"</div></div>"}function xe(V,oe){var fe=window.innerWidth>=768?.76:.54,K=140,te=305,Y=Math.round(te*.22),de=te-Y,ve=V.length>7?10:13,Te=ye(V,K,de),$e=oe?"1.5px solid #2a7a40":"1px solid #ddd",be=oe?"":"filter:grayscale(1);opacity:0.45;",ke=oe?"#1A6B3C":"#bbb",qe="#fff";return'<div style="display:inline-block;zoom:'+fe+';line-height:0;pointer-events:none"><div style="width:'+K+"px;height:"+te+"px;border-radius:6px;border:"+$e+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+be+'"><div style="height:'+Y+"px;background:"+ke+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+ve+"px;font-weight:900;color:"+qe+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(K-4)+'px">'+V+'</span></div><div style="height:'+de+'px;overflow:hidden;flex-shrink:0">'+Te+"</div></div></div>"}function Le(V){if(J){const oe=Q();if(!oe.length){V.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const fe=oe.map(K=>a.find(te=>te.player.id===K.id)||{_fake:!0,player:K,id:"fake-"+K.id});ne(fe,K=>K._fake?Pi(K.player):pi(K,""),K=>K._fake?ue({player:K.player,id:"x",_fake:!0}):ue(K),K=>{K._fake||Ni(K,a,M,t)})}else{const oe=re();if(!oe.length){V.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const fe={};oe.forEach(te=>{const Y=te.player.id;fe[Y]||(fe[Y]=[]),fe[Y].push(te)});const K=Object.values(fe).map(te=>Qo(te));ne(K,te=>{const Y=M[te.player.id]||1,de=Y>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${Y}</div>`:"",Te=a.filter($e=>$e.player.id===te.player.id&&$e.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return pi(te,de+Te)},te=>ue(te),te=>Ni(te,a,M,t))}}function Fe(V){const oe=J?m:[...G];if(!oe.length){V.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const fe=oe.map(K=>({formation:K,card:l.find(te=>te.formation===K)||null,owned:G.has(K)}));ne(fe,({formation:K,card:te,owned:Y})=>me(K,Y?te:null,Y?l.filter(de=>de.formation===K).length:0),({formation:K,owned:te})=>xe(K,te),({card:K,owned:te})=>{te&&K&&on(K,l,t,s)})}function Ce(V){const oe=Object.keys($),fe=J?oe.length?oe:T:[...q];if(!fe.length){V.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const K=fe.map(te=>({type:te,gc:Re[te]||{icon:"⚡",desc:""},def:$[te]||null,owned:q.has(te),card:o.find(Y=>Y.gc_type===te)||null}));ne(K,({type:te,gc:Y,def:de,owned:ve,card:Te})=>{var x;const $e=ve?o.filter(_=>_.gc_type===te).length:0,be=$e>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${$e}</div>`:"",ke=(de==null?void 0:de.gc_type)==="ultra_game_changer",qe={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},je={purple:"#b06ce0",light_blue:"#00d4ef"},w=qe[de==null?void 0:de.color]||qe.purple,z=je[de==null?void 0:de.color]||je.purple,g=(de==null?void 0:de.effect)||Y.desc||"",y=de!=null&&de.image_url?`/icons/${de.image_url}`:((x=de==null?void 0:de.club)==null?void 0:x.logo_url)||(de!=null&&de.country_code?`https://flagcdn.com/w80/${de.country_code.toLowerCase()}.png`:null);return ve&&Te?`<div data-gc-id="${Te.id}" data-gc-type="${te}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${z};background:${w};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${z}66;cursor:pointer">
          ${be}
        ...
        </div>`},null)}e.querySelectorAll(".col-tab-btn").forEach(V=>{V.addEventListener("click",()=>{B=V.dataset.tab,C="Tous",N="",J=!1,e.querySelectorAll(".col-tab-btn").forEach(oe=>{const fe=oe.dataset.tab===B;oe.style.borderBottomColor=fe?"var(--green)":"transparent",oe.style.color=fe?"var(--green)":"var(--gray-600)"}),U(),D()})}),U(),D()}function tn(e,t,i){const n=Re[e]||{icon:"⚡",desc:"Effet spécial."},d=(t==null?void 0:t.gc_type)==="ultra_game_changer",s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},r={purple:"#b06ce0",light_blue:"#00d4ef"},c=s[t==null?void 0:t.color]||s.purple,f=r[t==null?void 0:t.color]||r.purple,a=(t==null?void 0:t.name)||e,o=(t==null?void 0:t.effect)||n.desc,l=t!=null&&t.image_url?`/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Wt=1e3;function on(e,t,i,n){var $,p,u;const{state:d,toast:s,closeModal:r,navigate:c,refreshProfile:f}=i,a=e.formation,o={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function l(){const m=Ht[a]||{},T=_i[a]||[],M=290,G=360,q=20;function B(N){const J=m[N];return J?{x:J.x*M,y:J.y*G}:null}let C=`<svg width="${M}" height="${G}" viewBox="0 0 ${M} ${G}" xmlns="http://www.w3.org/2000/svg">`;for(const[N,J]of T){const Z=B(N),se=B(J);!Z||!se||(C+=`<line x1="${Z.x}" y1="${Z.y}" x2="${se.x}" y2="${se.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const N of Object.keys(m)){const J=B(N);if(!J)continue;const Z=N.replace(/\d+/,""),se=o[Z]||"#555";C+=`<circle cx="${J.x}" cy="${J.y}" r="${q}" fill="${se}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,C+=`<text x="${J.x}" y="${J.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${Z}</text>`}return C+="</svg>",C}const b=t.filter(m=>m.formation===a);b.length;const h=!e.is_for_sale;n(`Formation ${a}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),($=document.getElementById("direct-sell-form-btn"))==null||$.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${a} pour ${Wt.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const m=b.find(M=>!M.is_for_sale)||b[0];if(!m){s("Aucune carte à vendre","error");return}await A.from("market_listings").delete().eq("card_id",m.id),await A.from("transfer_history").delete().eq("card_id",m.id);const{error:T}=await A.from("cards").delete().eq("id",m.id);if(T){s(T.message,"error");return}await A.from("users").update({credits:(d.profile.credits||0)+Wt}).eq("id",d.profile.id),await f(),s(`+${Wt.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),r(),c("collection")}),(p=document.getElementById("market-sell-form-btn"))==null||p.addEventListener("click",async()=>{const m=parseInt(document.getElementById("sell-price-form").value);if(!m||m<1){s("Prix invalide","error");return}await A.from("cards").update({is_for_sale:!0,sale_price:m}).eq("id",e.id),await A.from("market_listings").insert({seller_id:d.profile.id,card_id:e.id,price:m}),s("Carte mise en vente sur le marché !","success"),r(),c("collection")}),(u=document.getElementById("cancel-sell-form-btn"))==null||u.addEventListener("click",async()=>{await A.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await A.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),s("Annonce retirée","success"),r(),c("collection")})}async function Ni(e,t,i,n){var Le,Fe,Ce,we,V,oe,fe;const{state:d,toast:s,openModal:r,closeModal:c,navigate:f,refreshProfile:a}=n,o=e.player,l=t.filter(K=>K.player.id===o.id),b=l.length,h=e.evolution_bonus||0,$=Math.max((Number(o.note_g)||0)+(o.job==="GK"||o.job2==="GK"?h:0),(Number(o.note_d)||0)+(o.job==="DEF"||o.job2==="DEF"?h:0),(Number(o.note_m)||0)+(o.job==="MIL"||o.job2==="MIL"?h:0),(Number(o.note_a)||0)+(o.job==="ATT"||o.job2==="ATT"?h:0)),p=o.rarity||"normal",{data:u}=await A.from("sell_price_configs").select("*").eq("rarity",p).lte("note_min",$).gte("note_max",$).order("note_min",{ascending:!1}).limit(1);((Le=u==null?void 0:u[0])==null?void 0:Le.price)??Zo[p];const m=o.rarity!=="legende",T=po(o),M=((o.rarity==="pepite"||o.rarity==="papyte")&&e.current_note!=null?e.current_note:St(o,o.job))+h,G=o.rarity==="pepite"||o.rarity==="papyte",q=o.job2?(G?Ei(e,co(o.job2)):St(o,o.job2))+(St(o,o.job2)>0?h:0):null,B=Ft[o.job]||"#1A6B3C",C=o.job2?Ft[o.job2]:null,N=lo[o.rarity]||"#ccc",J=zi[o.country_code]||o.country_code||"",Z=e.evolution_bonus||0,re=M+Z,Q=q||0,U=Q>0?Q+Z:0,D=l.map(K=>K.id);let ne={};if(D.length){const{data:K}=await A.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",D).order("transferred_at",{ascending:!0});(K||[]).forEach(te=>{ne[te.card_id]||(ne[te.card_id]=[]),ne[te.card_id].push(te)})}const ue=K=>{const te=K.transferred_at?new Date(K.transferred_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"2-digit",year:"numeric"}):"",Y=K.source==="booster"?"Booster":K.price?K.price.toLocaleString("fr"):"—";return`
    ...
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${l.map((K,te)=>{const Y=ne[K.id]||[],de=K.is_for_sale,ve=Y.length?Y[Y.length-1]:null,$e=(o.rarity==="pepite"||o.rarity==="papyte")&&K.current_note!=null?` (☆${K.current_note})`:"";return`
            <div data-card-id="${K.id}" data-card-idx="${te}" class="exemplaire-row"
              style="border:2px solid #eee;border-radius:10px;padding:10px;cursor:pointer;transition:border-color .15s;${de?"opacity:0.6":""}">
              <div style="display:flex;align-items:center;gap:10px">
                <input type="checkbox" class="expl-check" data-id="${K.id}" ${de?"disabled":""} style="width:18px;height:18px;cursor:pointer;accent-color:#1A6B3C;flex-shrink:0">
                <div style="flex:1;min-width:0">
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;background:#f0fdf4;border-radius:10px;padding:10px 14px">
          <div>
            ...
            <div style="font-size:11px;color:#555;margin-top:2px">
              ${(e.evolution_bonus||0)>0?`+${e.evolution_bonus} appliqué${e.evolution_bonus>1?"s":""} · Note : ${re}${o.job2&&Q>0?` / ${U}`:""}`:`Note actuelle : ${re}${o.job2&&Q>0?` / ${U}`:""}`}
            </div>
          </div>
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",o.note_g],["DEF",o.note_d],["MIL",o.note_m],["ATT",o.note_a]].map(([K,te])=>{const Y=Ft[K],de=K===o.job||K===o.job2,ve=(Number(te)||0)+(de&&Z>0?Z:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:2px">
    ...
    ${ye}
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(we=document.getElementById("close-detail"))==null||we.addEventListener("click",c);let me=new Set;const xe=()=>{const K=me.size,te=document.getElementById("sell-action-panel");if(!te)return;te.style.display=K>0?"block":"none",document.getElementById("sell-selected-count").textContent=K;const Y=document.getElementById("evolve-btn");Y&&(Y.textContent=`⬆️ Évoluer ${K>1?"(+"+K+")":""}`)};document.querySelectorAll(".expl-check").forEach(K=>{K.addEventListener("change",()=>{const te=K.dataset.id;K.checked?me.add(te):me.delete(te);const Y=K.closest(".exemplaire-row");Y&&(Y.style.borderColor=K.checked?"#1A6B3C":"#eee"),xe()})}),document.querySelectorAll(".exemplaire-row").forEach(K=>{K.addEventListener("click",te=>{if(te.target.closest("button")||te.target.tagName==="INPUT")return;const Y=K.querySelector(".expl-check");Y&&!Y.disabled&&(Y.checked=!Y.checked,Y.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(K=>{K.addEventListener("click",te=>{te.stopPropagation();const Y=document.querySelector(`.expl-hist[data-hist="${K.dataset.idx}"]`);Y&&(Y.style.display=Y.style.display==="none"?"flex":"none")})}),(V=document.getElementById("evolve-btn"))==null||V.addEventListener("click",async()=>{if(b<=1)return;const K=[...me];if(!K.length)return;if(me.has(e.id)){const be=document.createElement("div");be.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",be.innerHTML=`
        ...
        <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center;box-shadow:0 16px 48px rgba(0,0,0,0.3)">
          ...
          <div style="background:#f0fdf4;border-radius:10px;padding:12px;margin-bottom:16px;font-size:13px;color:#333">
            ...
            📈 Note : <strong>${M+(e.evolution_bonus||0)}</strong> → <strong style="color:#1A6B3C">${M+(e.evolution_bonus||0)+te.length*Y}</strong>
  ...
  </div>`,document.getElementById("new-deck-btn").addEventListener("click",async()=>{const r=prompt("Nom du deck :",`Deck ${((s==null?void 0:s.length)||0)+1}`);if(!r)return;const{data:c,error:f}=await A.from("decks").insert({owner_id:i.profile.id,name:r}).select().single();if(f){d(f.message,"error");return}d("Deck créé !","success"),Ri(c.id,e,t)}),e.querySelectorAll("[data-open-deck]").forEach(r=>{r.addEventListener("click",()=>Ri(r.dataset.openDeck,e,t))}),e.querySelectorAll("[data-rename]").forEach(r=>{r.addEventListener("click",async()=>{const c=prompt("Nouveau nom :",r.dataset.name);if(!c||c===r.dataset.name)return;const{error:f}=await A.from("decks").update({name:c}).eq("id",r.dataset.rename);if(f){d(f.message,"error");return}d("Deck renommé !","success"),mi(e,t)})}),e.querySelectorAll("[data-delete]").forEach(r=>{r.addEventListener("click",async()=>{if(!confirm(`Supprimer le deck "${r.dataset.name}" ? Cette action est irréversible.`))return;await A.from("deck_cards").delete().eq("deck_id",r.dataset.delete);const{error:c}=await A.from("decks").delete().eq("id",r.dataset.delete);if(c){d(c.message,"error");return}d("Deck supprimé.","success"),mi(e,t)})})}async function Ri(e,t,i){const{state:n,toast:d}=i;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:s}=await A.from("decks").select("*").eq("id",e).single(),{data:r}=await A.from("cards").select(`id, card_type, formation, stadium_id, evolution_bonus,
      player:players(id, firstname, surname_encoded, country_code, club_id, job, job2,
        note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length,
        clubs(encoded_name, logo_url))`).eq("owner_id",n.profile.id),{data:c}=await A.from("cards").select(`id, card_type, stadium_id,
      stadium_def:stadium_definitions(id, name, club_id, country_code, image_url,
        club:clubs(encoded_name, logo_url))`).eq("owner_id",n.profile.id).eq("card_type","stadium"),f=(r||[]).filter(m=>m.card_type==="player"&&m.player),a=(r||[]).filter(m=>m.card_type==="formation"),o=(c||[]).filter(m=>m.card_type==="stadium"),l=[...new Set(o.map(m=>m.stadium_id).filter(Boolean))];let b={};if(o.forEach(m=>{m.stadium_def&&m.stadium_id&&(b[m.stadium_id]=m.stadium_def)}),l.length&&Object.keys(b).length<l.length){const{data:m}=await A.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",l);(m||[]).forEach(T=>{b[T.id]=T})}const h=a.map(m=>m.formation).filter(Boolean),{data:$}=await A.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",e);let p=s.formation||"4-4-2";h.length>0&&!h.includes(p)&&(p=h[0]);const u={deckId:e,name:s.name,formation:p,formationCardId:s.formation_card_id,stadiumCardId:s.stadium_card_id||null,slots:{},subs:[],playerCards:f,formationCards:a,stadiumCards:o,stadDefMap:b,availableFormations:h};($||[]).forEach(m=>{m.is_starter?u.slots[m.position]=m.card_id:u.subs.includes(m.card_id)||u.subs.push(m.card_id)}),$t(t,u,i)}function $t(e,t,i){var a,o,l,b;const{navigate:n}=i;Qt[t.formation];const d=Oi(t.formation),s=d.filter(h=>t.slots[h]).length,r=t.availableFormations.length>0?t.availableFormations:Object.keys(Qt),c=(a=t.stadiumCards)==null?void 0:a.find(h=>h.id===t.stadiumCardId);c&&((o=t.stadDefMap)!=null&&o[c.stadium_id]);const f=t.subs.map(h=>t.playerCards.find($=>$.id===h)).filter(Boolean);[...Object.values(t.slots),...t.subs],e.innerHTML=`
  <div style="height:100%;overflow-y:auto;padding-bottom:80px;background:var(--page-bg)">
  ...
  </div>`,rn(e,t,d,i),document.getElementById("builder-back").addEventListener("click",()=>n("decks")),document.getElementById("formation-select").addEventListener("change",h=>{t.formation=h.target.value;const $=Oi(t.formation),p={};$.forEach(u=>{t.slots[u]&&(p[u]=t.slots[u])}),t.slots=p,$t(e,t,i)}),(l=document.getElementById("stadium-select"))==null||l.addEventListener("change",h=>{t.stadiumCardId=h.target.value||null,$t(e,t,i)}),document.getElementById("save-deck").addEventListener("click",()=>dn(t,i)),e.querySelectorAll("[data-remove-sub]").forEach(h=>{h.addEventListener("click",()=>{t.subs=t.subs.filter($=>$!==h.dataset.removeSub),$t(e,t,i)})}),(b=document.getElementById("add-sub-btn"))==null||b.addEventListener("click",()=>{sn(t,e,i)})}function rn(e,t,i,n){var M,G;const d=e.querySelector("#deck-field");if(!d)return;const s=(M=t.stadiumCards)==null?void 0:M.find(q=>q.id===t.stadiumCardId),r=s&&((G=t.stadDefMap)==null?void 0:G[s.stadium_id])||null,c=Ht[t.formation]||{},f=Zi(t.formation)||[],a={};for(const q of i){const B=t.slots[q],C=B?t.playerCards.find(N=>N.id===B):null;C!=null&&C.player?a[q]={...C.player,_evolution_bonus:C.evolution_bonus||0}:a[q]=null}const o=300,l=300,b=48,h=64,$=13,p=16,u=38;function m(q){const B=c[q];return B?{x:B.x*o,y:B.y*l}:null}let T="";for(const[q,B]of f){const C=m(q),N=m(B);if(!C||!N)continue;const J=a[q]?{...a[q],club_id:a[q].club_id,country_code:a[q].country_code,rarity:a[q].rarity}:null,Z=a[B]?{...a[B],club_id:a[B].club_id,country_code:a[B].country_code,rarity:a[B].rarity}:null,se=nt(J,Z);se==="#ff3333"||se==="#cc2222"?T+=`<line x1="${C.x.toFixed(1)}" y1="${C.y.toFixed(1)}" x2="${N.x.toFixed(1)}" y2="${N.y.toFixed(1)}" stroke="${se}" stroke-width="2.5" stroke-linecap="round" opacity="0.4"/>`:(T+=`<line x1="${C.x.toFixed(1)}" y1="${C.y.toFixed(1)}" x2="${N.x.toFixed(1)}" y2="${N.y.toFixed(1)}" stroke="${se}" stroke-width="8" stroke-linecap="round" opacity="0.2"/>`,T+=`<line x1="${C.x.toFixed(1)}" y1="${C.y.toFixed(1)}" x2="${N.x.toFixed(1)}" y2="${N.y.toFixed(1)}" stroke="${se}" stroke-width="2.5" stroke-linecap="round" opacity="0.9"/>`)}for(const q of i){const B=m(q);if(!B)continue;const C=a[q],N=q.replace(/\d+/,""),J=_t[N]||"#555",Z=(B.x-b/2).toFixed(1),se=(B.y-h/2).toFixed(1),re={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[C==null?void 0:C.rarity]||"#aaa";if(C){const Q=ri(C),U=fo(C),D=uo(C.country_code),ne=C._evolution_bonus||0,ue=(Number(N==="GK"?C.note_g:N==="DEF"?C.note_d:N==="MIL"?C.note_m:C.note_a)||0)+(N===C.job||N===C.job2?ne:0),ye=r&&(r.club_id&&String(C.club_id)===String(r.club_id)||r.country_code&&C.country_code===r.country_code),me=ye?ue+10:ue,xe=h-$-p;T+=`<defs><clipPath id="dcp-${q}"><rect x="${Z}" y="${(B.y-h/2+$).toFixed(1)}" width="${b}" height="${xe}"/></clipPath></defs>`,T+=`<rect x="${Z}" y="${se}" width="${b}" height="${h}" rx="5" fill="${J}"/>`,Q&&(T+=`<image href="${Q}" x="${Z}" y="${(B.y-h/2+$).toFixed(1)}" width="${b}" height="${xe}" clip-path="url(#dcp-${q})" preserveAspectRatio="xMidYMin slice"/>`),T+=`<rect x="${Z}" y="${se}" width="${b}" height="${$}" fill="rgba(255,255,255,0.93)"/>`,T+=`<text x="${B.x.toFixed(1)}" y="${(B.y-h/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(C.surname_encoded||"").slice(0,9)}</text>`;const Le=(B.y+h/2-p).toFixed(1);T+=`<rect x="${Z}" y="${Le}" width="${b}" height="${p}" fill="rgba(255,255,255,0.93)"/>`,D&&(T+=`<image href="${D}" x="${(B.x-21).toFixed(1)}" y="${(B.y+h/2-p+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`),T+=`<text x="${B.x.toFixed(1)}" y="${(B.y+h/2-p/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="${ye?"#E87722":"#111"}" font-family="Arial Black">${me}</text>`,U&&(T+=`<image href="${U}" x="${(B.x+b/2-14).toFixed(1)}" y="${(B.y+h/2-p+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`),T+=`<rect x="${Z}" y="${se}" width="${b}" height="${h}" rx="5" fill="none" stroke="${re}" stroke-width="2"/>`}else T+=`<rect x="${Z}" y="${se}" width="${b}" height="${h}" rx="5" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="5,3"/>`,T+=`<text x="${B.x.toFixed(1)}" y="${B.y.toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="22" fill="rgba(255,255,255,0.35)">+</text>`,T+=`<text x="${B.x.toFixed(1)}" y="${(B.y+16).toFixed(1)}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.3)">${N}</text>`;T+=`<rect x="${Z}" y="${se}" width="${b}" height="${h}" rx="5" fill="rgba(0,0,0,0.01)" class="deck-slot-hit" data-pos="${q}" style="cursor:pointer"/>`}d.innerHTML=`<svg viewBox="${-u} ${-u} ${o+u*2} ${l+u*2}" width="100%" style="display:block;max-width:440px;margin:0 auto">${T}</svg>`,d.querySelectorAll(".deck-slot-hit").forEach(q=>{q.addEventListener("click",()=>an(q.dataset.pos,t,e,n))})}function an(e,t,i,n){var b,h,$;const{openModal:d,closeModal:s}=n,r=e.replace(/\d+/,""),c=t.slots[e],f=c?t.playerCards.find(p=>p.id===c):null;(b=f==null?void 0:f.player)==null||b.id;const a=new Set;Object.entries(t.slots).forEach(([p,u])=>{var T;if(p===e||!u)return;const m=t.playerCards.find(M=>M.id===u);(T=m==null?void 0:m.player)!=null&&T.id&&a.add(m.player.id)}),t.subs.forEach(p=>{var m;const u=t.playerCards.find(T=>T.id===p);(m=u==null?void 0:u.player)!=null&&m.id&&a.add(u.player.id)});const o=new Set,l=t.playerCards.filter(p=>{const u=p.player;return!(u.job===r||u.job2===r)||a.has(u.id)||o.has(u.id)?!1:(o.add(u.id),!0)});l.sort((p,u)=>{const m=p.evolution_bonus||0,T=u.evolution_bonus||0,M=(r==="GK"?p.player.note_g:r==="DEF"?p.player.note_d:r==="MIL"?p.player.note_m:p.player.note_a)+(r===p.player.job||r===p.player.job2?m:0);return(r==="GK"?u.player.note_g:r==="DEF"?u.player.note_d:r==="MIL"?u.player.note_m:u.player.note_a)+(r===u.player.job||r===u.player.job2?T:0)-M}),d(`Choisir ${r} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${l.length>0?l.map(p=>{var q,B;const u=p.player,m=p.evolution_bonus||0,T=(r==="GK"?u.note_g:r==="DEF"?u.note_d:r==="MIL"?u.note_m:u.note_a)+(r===u.job||r===u.job2?m:0),M=ri(u),G={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[u.rarity];return`<div class="player-option" data-card-id="${p.id}"
          ...
          <div style="width:44px;height:44px;border-radius:8px;overflow:hidden;flex-shrink:0;background:#dde;border:2px solid ${_t[r]}">
            ${M?`<img src="${M}" style="width:100%;height:100%;object-fit:cover">`:`<div style="width:100%;height:100%;background:${_t[r]};display:flex;align-items:center;justify-content:center;color:#fff;font-size:10px;font-weight:700">${r}</div>`}
          </div>
          <!-- Infos -->
          <div style="flex:1;min-width:0">
          ...
          <!-- Note -->
          <div style="width:36px;height:36px;border-radius:8px;background:${_t[r]};color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:16px;border:2px solid ${G};flex-shrink:0">
            ${T}
          </div>
  ...
  </div>`}function xo(e){var d;const t={};(e.rates||[]).forEach(s=>{t[s.card_type]=(t[s.card_type]||0)+Number(s.percentage||0)});const i=((d=Object.entries(t).sort((s,r)=>r[1]-s[1])[0])==null?void 0:d[0])||"player",n=e.image_url||"booster-players.png";return{id:e.id,img:"/icons/"+n,name:e.name,sub:`${e.card_count} carte(s)`,cost:e.price_type==="credits"&&e.price_credits||0,costLabel:e.price_type==="credits"?`${(e.price_credits||0).toLocaleString("fr")} crédits`:e.price_type==="pub"?"1 pub":"Gratuit",cardCount:e.card_count||5,type:i,isPub:e.price_type==="pub",rates:e.rates||[],allow_duplicates:e.allow_duplicates!==!1,_boosterId:e.id,_raw:e}}async function xn(e,{state:t,navigate:i,toast:n}){var l,b,h;const d=((l=t.profile)==null?void 0:l.credits)||0;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⏳ Chargement...</div>';let s=[];try{s=(await go((b=t.user)==null?void 0:b.id)).map(xo)}catch($){console.warn("Erreur chargement boosters DB, fallback hardcodé",$)}s.length||(s=pn.map($=>({...$,rates:[],isPub:$.id==="players_pub"})));const r=await A.from("cards").select("player_id, card_type, formation, stadium_id, gc_type").eq("owner_id",t.profile.id).then($=>$.data||[]),c=new Set(r.filter($=>$.card_type==="stadium").map($=>$.stadium_id)),f=new Set(r.filter($=>$.card_type==="formation").map($=>$.formation)),a=new Set(r.filter($=>$.card_type==="game_changer").map($=>$.gc_type)),o={};for(const $ of s){if($.allow_duplicates!==!1||!((h=$.rates)!=null&&h.length))continue;const p=[...new Set(($.rates||[]).map(m=>m.card_type))];let u=!1;for(const m of p)if(m==="stadium"){const{data:T}=await A.from("stadium_definitions").select("id");if((T||[]).some(M=>!c.has(M.id))){u=!0;break}}else if(m==="game_changer"){const{data:T}=await A.from("gc_definitions").select("name").eq("is_active",!0);if((T||[]).some(M=>!a.has(M.name))){u=!0;break}}else if(m==="formation"){const{FORMATION_LINKS:T}=await ni(async()=>{const{FORMATION_LINKS:M}=await import("./formation-links-5A-YoKwp.js").then(G=>G.p);return{FORMATION_LINKS:M}},__vite__mapDeps([0,1]));if(Object.keys(T).some(M=>!f.has(M))){u=!0;break}}else{u=!0;break}u||(o[$.id]=!0)}e.innerHTML=`
  <div class="page">
    <div class="page-header">
      <h2>📦 Boosters</h2>
    ...
    <div class="page-body">
      ...
      <button style="margin-top:10px;padding:12px 28px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer" id="anim-close-err">Fermer</button>`,document.body.appendChild(h),(b=h.querySelector("#anim-close-err"))==null||b.addEventListener("click",()=>h.remove());return}f.forEach(h=>{h.card_type==="player"&&h.player?h.isDuplicate=r.has(h.player.id):h.card_type==="formation"&&(h.isDuplicate=c.has(h.formation))});const{data:o}=await A.from("users").select("*").eq("id",t.profile.id).single();o&&(t.profile=o),vo(f,e,n)}function bn(){const e=Math.random()*100;return e<.5?"legende":e<2?"special":e<10?"normal_high":"normal_low"}function xt(e){return Math.max(Number(e.note_g)||0,Number(e.note_d)||0,Number(e.note_m)||0,Number(e.note_a)||0)}function hn(e,t){let i;switch(t){case"legende":i=e.filter(n=>n.rarity==="legende"),i.length||(i=e.filter(n=>n.rarity==="pepite"||n.rarity==="papyte")),i.length||(i=e.filter(n=>xt(n)>=6));break;case"special":i=e.filter(n=>n.rarity==="pepite"||n.rarity==="papyte"),i.length||(i=e.filter(n=>xt(n)>=6));break;case"normal_high":i=e.filter(n=>n.rarity==="normal"&&xt(n)>=6),i.length||(i=e.filter(n=>xt(n)>=6));break;default:i=e.filter(n=>n.rarity==="normal"&&xt(n)>=1&&xt(n)<=5),i.length||(i=e.filter(n=>n.rarity==="normal"));break}return i.length||(i=e),i[Math.floor(Math.random()*i.length)]}async function yi(e,t){if(t.cost>0){const{error:b}=await A.from("users").update({credits:e.credits-t.cost}).eq("id",e.id);if(b)throw b}const i=t.allow_duplicates!==!1;let n=[];const{data:d,error:s}=await A.from("cards").select("player_id, card_type, formation, stadium_id, gc_type").eq("owner_id",e.id);if(s){const{data:b}=await A.from("cards").select("player_id, card_type, formation, gc_type").eq("owner_id",e.id);n=b||[]}else n=d||[];const r=new Set(n.filter(b=>b.card_type==="player").map(b=>b.player_id)),c=new Set(n.filter(b=>b.card_type==="formation").map(b=>b.formation)),f=new Set(n.filter(b=>b.card_type==="game_changer").map(b=>b.gc_type)),a=new Set(n.filter(b=>b.card_type==="stadium").map(b=>b.stadium_id).filter(Boolean)),o=new Set,l=[];for(let b=0;b<(t.cardCount||5);b++){const h=cn(t.rates);if(h){if(h.card_type==="player"){const $=C=>({légende:"legende",pépite:"pepite",pépites:"pepite"})[C]||C,p=h.rarity?$(h.rarity):null;let u=A.from("players").select("id,job,firstname,surname_encoded,country_code,club_id,rarity,note_g,note_d,note_m,note_a,skin,hair,hair_length,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);p&&(u=u.eq("rarity",p));const{data:m}=await u;let T=m||[];if((h.note_min||h.note_max)&&(T=T.filter(C=>{const N=Math.max(Number(C.note_g)||0,Number(C.note_d)||0,Number(C.note_m)||0,Number(C.note_a)||0);return(!h.note_min||N>=h.note_min)&&(!h.note_max||N<=h.note_max)})),T.length||(h.note_min||h.note_max?(T=m||[],console.warn("[Booster] Aucun joueur avec note",h.note_min,"-",h.note_max,"— fallback rareté uniquement")):T=m||[]),!T.length)continue;let M=T.filter(C=>!o.has(C.id));if(i)M.length||(M=T);else if(M=M.filter(C=>!r.has(C.id)),!M.length)continue;const G=M[Math.floor(Math.random()*M.length)];o.add(G.id);const q=r.has(G.id),{data:B}=await A.from("cards").insert({owner_id:e.id,player_id:G.id,card_type:"player"}).select().single();B&&(l.push({...B,player:G,isDuplicate:q}),A.rpc("record_transfer",{p_card_id:B.id,p_player_id:G.id,p_club_name:e.club_name||e.pseudo,p_manager_name:e.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{}))}else if(h.card_type==="game_changer"){const{data:$}=await A.from("gc_definitions").select("id,name").eq("is_active",!0).eq("gc_type","game_changer"),p=$!=null&&$.length?$:[{name:"Ressusciter"},{name:"Double attaque"},{name:"Bouclier"},{name:"Vol de note"},{name:"Gel"}],u=i?p:p.filter(G=>!f.has(G.name));if(!i&&!u.length)continue;const T=u[Math.floor(Math.random()*u.length)].name,{data:M}=await A.from("cards").insert({owner_id:e.id,card_type:"game_changer",gc_type:T}).select().single();M&&l.push(M)}else if(h.card_type==="formation"){const $=mo(),p=i?$:$.filter(M=>!c.has(M));if(!i&&!p.length)continue;const u=p[Math.floor(Math.random()*p.length)],m=c.has(u),{data:T}=await A.from("cards").insert({owner_id:e.id,card_type:"formation",formation:u}).select();T!=null&&T[0]&&l.push({...T[0],isDuplicate:m})}else if(h.card_type==="stadium"){const{data:$,error:p}=await A.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)");if(p){console.error("[Booster] stadium_definitions:",p.message);continue}if(!($!=null&&$.length)){console.warn("[Booster] Aucun stade en DB");continue}const u=i?$:$.filter(G=>!a.has(G.id));if(!i&&!u.length)continue;const m=u[Math.floor(Math.random()*u.length)],{data:T,error:M}=await A.from("cards").insert({owner_id:e.id,card_type:"stadium",stadium_id:m.id}).select("id,card_type,stadium_id").single();if(M){console.error("[Booster] insert stadium card:",M.message);continue}T&&l.push({...T,rarity:"normal",_stadiumDef:m})}}}return l}async function yo(e,t,i){if(i>0){const{error:a}=await A.from("users").update({credits:e.credits-i}).eq("id",e.id);if(a)throw a}const{data:n}=await A.from("players").select("id,job,firstname,surname_encoded,country_code,club_id,rarity,note_g,note_d,note_m,note_a,note_min,note_max,skin,hair,hair_length,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);if(!(n!=null&&n.length))throw new Error("Pas de joueurs en BDD — ajoutes-en via le panel admin !");const d=n.filter(a=>a.job==="GK"),s=n.filter(a=>a.job!=="GK"),r=!e.first_booster_opened&&d.length>0,c=[];for(let a=0;a<t;a++){const o=a===0&&r?d:a===0?s:n,l=bn(),b=hn(o,l);b&&c.push(b)}r&&await A.from("users").update({first_booster_opened:!0}).eq("id",e.id);const{data:f}=await A.from("cards").insert(c.map(a=>({owner_id:e.id,player_id:a.id,card_type:"player"}))).select();return(f||[]).forEach((a,o)=>{A.rpc("record_transfer",{p_card_id:a.id,p_player_id:c[o].id,p_club_name:e.club_name||e.pseudo,p_manager_name:e.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{})}),c.map((a,o)=>({...f[o],player:a}))}async function bo(e,t,i){const{error:n}=await A.from("users").update({credits:e.credits-i}).eq("id",e.id);if(n)throw n;const{data:d}=await A.from("gc_definitions").select("id,name,gc_type,color,effect,image_url").eq("is_active",!0),s=d!=null&&d.length?d:Object.keys(xi).map(o=>({name:o,gc_type:"game_changer"})),r=Array.from({length:t},()=>{const o=s[Math.floor(Math.random()*s.length)];return{owner_id:e.id,card_type:"game_changer",gc_type:o.name,gc_definition_id:o.id||null}}),{data:c,error:f}=await A.from("cards").insert(r).select();return f&&console.error("[Booster GC] Erreur insert:",f.message,f),(c||[]).map(o=>{const l=d==null?void 0:d.find(b=>b.name===o.gc_type||b.id===o.gc_definition_id);return{...o,_gcDef:l||null}})}async function ho(e,t){const{error:i}=await A.from("users").update({credits:e.credits-t}).eq("id",e.id);if(i)throw i;const{data:n}=await A.from("cards").select("formation").eq("owner_id",e.id).eq("card_type","formation"),d=new Set((n||[]).map(o=>o.formation)),s=mo(),r=s[Math.floor(Math.random()*s.length)],c=d.has(r),{data:f,error:a}=await A.from("cards").insert({owner_id:e.id,card_type:"formation",formation:r}).select();return a&&console.error("[Booster Formation] Erreur insert:",a.message,a),(f||[]).map(o=>({...o,isDuplicate:c}))}function vo(e,t,i,n=null){var re,Q;if(!e||e.length===0){const U=document.createElement("div");U.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:3000;gap:16px;color:#fff;padding:24px;text-align:center",U.innerHTML=`
      <div style="font-size:48px">😕</div>
      <div style="font-size:20px;font-weight:900">Aucune carte obtenue</div>
      <div style="font-size:13px;color:rgba(255,255,255,0.5)">Erreur lors du tirage (permissions DB ou colonne manquante)</div>
      <button style="margin-top:10px;padding:12px 28px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer" id="anim-close-err">Fermer</button>`,document.body.appendChild(U),(re=U.querySelector("#anim-close-err"))==null||re.addEventListener("click",()=>U.remove());return}e=[...e].sort((U,D)=>{const ne=U.player?xt(U.player):-1;return(D.player?xt(D.player):-1)-ne});const d=document.createElement("div");d.id="booster-anim-overlay",d.innerHTML=`
    <style>
  ...
  `,document.body.appendChild(d);let s=!1;const r=document.getElementById("pack-cut-zone"),c=document.getElementById("pack-blade");let f=!1;const a=U=>U.touches&&U.touches[0]?U.touches[0].clientX:U.clientX;function o(U){s||(f=!0,c.style.opacity="1",l(U))}function l(U){if(!f||s)return;const D=r.getBoundingClientRect(),ne=a(U)-D.left,ue=Math.max(0,Math.min(1,ne/D.width));c.style.width=ue*D.width+"px",ue>=.82&&h()}function b(){s||(f=!1,c.style.transition="width .2s ease, opacity .2s ease",c.style.width="0",c.style.opacity="0",setTimeout(()=>{s||(c.style.transition="")},220))}function h(){var D;if(s)return;s=!0,f=!1,c.style.width="100%",c.style.opacity="1",(D=document.getElementById("cut-flash"))==null||D.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const U=document.getElementById("cut-hint");U&&(U.style.opacity="0"),r.classList.add("pack-cut"),setTimeout(()=>{c.style.opacity="0",document.getElementById("pack-phase").style.display="none",m(0)},620)}r.addEventListener("pointerdown",o),window.addEventListener("pointermove",l),window.addEventListener("pointerup",b),r.addEventListener("touchstart",o,{passive:!0}),window.addEventListener("touchmove",l,{passive:!0}),window.addEventListener("touchend",b);let $=0,p=!1;const u=new Set;function m(U){$=U,document.getElementById("reveal-phase").style.display="flex",T(),M(U,0),N()}function T(){const U=document.getElementById("card-dots");U&&(U.innerHTML=e.map((D,ne)=>`<div class="card-dot" data-i="${ne}" style="width:8px;height:8px;border-radius:50%;background:${ne===$?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),U.querySelectorAll(".card-dot").forEach(D=>D.addEventListener("click",()=>q(parseInt(D.dataset.i)))))}function M(U,D){var we;const ne=e[U],ue=document.getElementById("card-counter"),ye=document.getElementById("card-track");ue&&(ue.textContent=`Carte ${U+1} / ${e.length}`);const me=document.getElementById("reveal-btns");me&&(me.style.display=U===e.length-1?"flex":"none");const xe=ne.card_type==="player"&&((we=ne.player)==null?void 0:we.rarity)==="legende",Le=!u.has(U);u.add(U);let Fe=0;if(ne.card_type==="player"&&ne.player){const V=ne.player,oe=V.job||"ATT";Fe=Number(oe==="GK"?V.note_g:oe==="DEF"?V.note_d:oe==="MIL"?V.note_m:V.note_a)||0}const Ce=V=>{ye.innerHTML=`
        ...
        </div>`;const oe=document.getElementById("current-card-wrap");D!==0?(oe.style.transition="none",oe.style.transform=`translateX(${D>0?100:-100}%)`,requestAnimationFrame(()=>{oe.style.transition="transform .28s cubic-bezier(.25,1,.5,1)",oe.style.transform="translateX(0)"})):oe.animate([{opacity:0,transform:"scale(.7)"},{opacity:1,transform:"scale(1)"}],{duration:300,easing:"cubic-bezier(.34,1.56,.64,1)"}),V||xe?Z():se(),T()};Le&&Fe>6&&ne.card_type==="player"&&ne.player?G(ne,()=>Ce(!0)):Ce(!1)}function G(U,D){var Fe;p=!0;const ne=U.player,ue=`https://flagsapi.com/${ne.country_code}/flat/64.png`,ye=(Fe=ne.clubs)==null?void 0:Fe.logo_url,me=document.getElementById("walkout-overlay"),xe=document.getElementById("walkout-stage");if(!me||!xe){p=!1,D();return}me.style.display="flex";const Le=()=>{const Ce=xe.firstElementChild;Ce&&(Ce.classList.remove("wo-in"),Ce.classList.add("wo-out"))};xe.innerHTML=`<img class="wo-in" src="${ue}" style="height:130px;border-radius:10px;box-shadow:0 10px 36px rgba(0,0,0,.6)" onerror="this.style.display='none'">`,navigator.vibrate&&navigator.vibrate(30),setTimeout(Le,2e3),setTimeout(()=>{var Ce;xe.innerHTML=ye?`<img class="wo-in" src="${ye}" style="max-height:160px;max-width:210px;object-fit:contain">`:`<div class="wo-in" style="font-size:34px;font-weight:900;color:#fff;text-align:center">${((Ce=ne.clubs)==null?void 0:Ce.encoded_name)||"CLUB"}</div>`,navigator.vibrate&&navigator.vibrate(30)},2450),setTimeout(Le,4450),setTimeout(()=>{me.style.display="none",xe.innerHTML="",p=!1,navigator.vibrate&&navigator.vibrate([40,30,80]),D()},4900)}function q(U){if(p||U<0||U>=e.length||U===$)return;const D=U>$?1:-1;$=U,M(U,D)}function B(){q($+1)}function C(){q($-1)}function N(){const U=document.getElementById("card-viewport");if(!U||U._swipeBound)return;U._swipeBound=!0;let D=0,ne=0,ue=0,ye=!1;const me=we=>we.touches?we.touches[0].clientX:we.clientX,xe=we=>we.touches?we.touches[0].clientY:we.clientY,Le=we=>{ye=!0,D=me(we),ne=xe(we),ue=0},Fe=we=>{if(!ye)return;ue=me(we)-D;const V=xe(we)-ne;if(Math.abs(ue)<Math.abs(V))return;const oe=document.getElementById("current-card-wrap");oe&&(oe.style.transition="none",oe.style.transform=`translateX(${ue*.6}px) rotate(${ue*.02}deg)`)},Ce=()=>{if(!ye)return;ye=!1;const we=document.getElementById("current-card-wrap"),V=55;ue<=-V&&$<e.length-1?B():ue>=V&&$>0?C():we&&(we.style.transition="transform .2s ease",we.style.transform="translateX(0)")};U.addEventListener("pointerdown",Le),U.addEventListener("pointermove",Fe),U.addEventListener("pointerup",Ce),U.addEventListener("pointercancel",Ce),U.addEventListener("touchstart",Le,{passive:!0}),U.addEventListener("touchmove",Fe,{passive:!0}),U.addEventListener("touchend",Ce),U.addEventListener("click",we=>{if(Math.abs(ue)>8)return;const V=U.getBoundingClientRect();we.clientX-V.left>V.width/2?B():C()})}let J=null;function Z(){const U=document.getElementById("fireworks-canvas");if(!U)return;U.width=window.innerWidth,U.height=window.innerHeight;const D=U.getContext("2d"),ne=[];function ue(){const me=Math.random()*U.width,xe=Math.random()*U.height*.6,Le=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],Fe=Le[Math.floor(Math.random()*Le.length)];for(let Ce=0;Ce<60;Ce++){const we=Math.PI*2/60*Ce,V=2+Math.random()*5;ne.push({x:me,y:xe,vx:Math.cos(we)*V,vy:Math.sin(we)*V,alpha:1,color:Fe,size:2+Math.random()*3})}}ue(),J=setInterval(ue,600);function ye(){if(document.getElementById("fireworks-canvas")){D.clearRect(0,0,U.width,U.height);for(let me=ne.length-1;me>=0;me--){const xe=ne[me];if(xe.x+=xe.vx,xe.y+=xe.vy+.08,xe.vy*=.98,xe.alpha-=.018,xe.alpha<=0){ne.splice(me,1);continue}D.globalAlpha=xe.alpha,D.fillStyle=xe.color,D.beginPath(),D.arc(xe.x,xe.y,xe.size,0,Math.PI*2),D.fill()}D.globalAlpha=1,(J!==null||ne.length>0)&&requestAnimationFrame(ye)}}ye()}function se(){J!==null&&(clearInterval(J),J=null);const U=document.getElementById("fireworks-canvas");U&&U.getContext("2d").clearRect(0,0,U.width,U.height)}if(n){const U=document.getElementById("reveal-btns");U&&(U.innerHTML='<button class="btn btn-primary" id="reveal-next" style="flex:1">Continuer →</button>'),(Q=document.getElementById("reveal-next"))==null||Q.addEventListener("click",()=>{se(),d.remove(),n()})}else document.getElementById("reveal-collection").addEventListener("click",()=>{se(),d.remove(),i("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{se(),d.remove(),i("boosters")})}function vn(e){var t,i,n,d;if(e.card_type==="player"&&e.player)return mn(e);if(e.card_type==="game_changer"){const s=e._gcDef,r=(s==null?void 0:s.gc_type)==="ultra_game_changer",c={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},f={purple:"#b06ce0",light_blue:"#00d4ef"},a=c[s==null?void 0:s.color]||c.purple,o=f[s==null?void 0:s.color]||f.purple,l=(s==null?void 0:s.name)||e.gc_type||"Game Changer",b=(s==null?void 0:s.effect)||((t=xi[e.gc_type])==null?void 0:t.desc)||"",h=s!=null&&s.image_url?`/icons/${s.image_url}`:null,$=((i=xi[e.gc_type])==null?void 0:i.icon)||"⚡";return`<div style="width:170px;height:240px;background:${a};border-radius:14px;border:3px solid ${o};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${o}88;flex-shrink:0">
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    </div>`,e.querySelectorAll(".gc-select-card").forEach($=>{$.addEventListener("click",()=>{const p=$.dataset.id,u=s.find(T=>T.id===p);if(!u)return;const m=r.findIndex(T=>T.gc_type===u.gc_type);m>-1?r.splice(m,1):r.length<3&&r.push(u),a()})}),(l=e.querySelector("#gc-launch"))==null||l.addEventListener("click",()=>{o&&f(r)}),(b=e.querySelector("#gc-no-gc"))==null||b.addEventListener("click",()=>f([])),(h=e.querySelector("#gc-reset"))==null||h.addEventListener("click",()=>{r.length&&(r=[],a())})}a()}function Tn(e){var n;const t=((n=e==null?void 0:e.state)==null?void 0:n.params)||{},i=t.matchMode||"vs_ai_easy";return i==="friend"?`Match vs ${t.friendName||"Ami"}`:i==="random"?"Match vs Random":`Match vs IA — ${i.replace("vs_ai_","").toUpperCase()}`}async function An(e,t,i){const{state:n,navigate:d}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:s}=await A.from("decks").select(`id,name,is_active,formation,stadium_card_id,
      stadium_card:cards!stadium_card_id(id,stadium_id,
        stadium_def:stadium_definitions(id,name,club_id,country_code,image_url,
          club:clubs(encoded_name,logo_url)))`).eq("owner_id",n.profile.id).order("created_at",{ascending:!1});if(!s||s.length===0){kt(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>d("decks"));return}const r=s.map(b=>b.id),{data:c}=await A.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,stadium_id,evolution_bonus,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",r).order("slot_order"),f=[...new Set((c||[]).filter(b=>{var h,$;return((h=b.card)==null?void 0:h.card_type)==="stadium"&&(($=b.card)==null?void 0:$.stadium_id)}).map(b=>b.card.stadium_id))],a={};if(f.length){const{data:b}=await A.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",f);(b||[]).forEach(h=>{a[h.id]=h}),(c||[]).forEach(h=>{var $,p;(($=h.card)==null?void 0:$.card_type)==="stadium"&&((p=h.card)!=null&&p.stadium_id)&&(h.card._stadiumDef=a[h.card.stadium_id]||null)})}let o=0;function l(){var q,B,C,N,J,Z,se;const b=s[o],h=(c||[]).filter(re=>re.deck_id===b.id),$=h.filter(re=>{var Q;return re.is_starter&&((Q=re.card)==null?void 0:Q.player)}).map(re=>bi(re.card,re.position)),p=h.find(re=>{var Q;return((Q=re.card)==null?void 0:Q.card_type)==="formation"}),u=b.formation||((q=p==null?void 0:p.card)==null?void 0:q.formation)||"4-4-2";let m=$.length>=11?bt($,u):null,T=((B=b.stadium_card)==null?void 0:B.stadium_def)||null;T&&m&&(m=Ct(m,T));const M=$.length>=11;et(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;height:${l}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ...
      <span style="font-size:${l-2}px;font-weight:900;color:${e!=null&&e.used?"#111":T};font-family:Arial Black,Arial">${e!=null&&e.used?"–":m}</span>
      ${s?`<img src="${s}" style="width:${l-4}px;height:${l-4}px;object-fit:contain">`:e!=null&&e.clubName?`<span style="font-size:${Math.max(4,l-8)}px;font-weight:700;color:#333">${(e.clubName||"").slice(0,3).toUpperCase()}</span>`:""}
        ...
        stroke="${Z}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const h=48,$=64,p=13,u=16,m={normal:"#aaaaaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#111111",legende:"#7a28b8",légende:"#7a28b8"};for(const[G,q]of Object.entries(a)){const B=l(G);if(!B||!q)continue;const C=G.replace(/[0-9]/g,""),N=jt[C]||"#555",J=r.includes(q.cardId),Z=i==="attack"&&(["MIL","ATT"].includes(C)||J)&&!q.used||i==="defense"&&["GK","DEF","MIL"].includes(C)&&!q.used,se=n.includes(q.cardId);let re;i==="attack"?re=J?Math.max(1,Number(q.note_a)||0):C==="MIL"?Number(q.note_m)||0:Number(q.note_a)||0:i==="defense"?re=C==="GK"?Number(q.note_g)||0:C==="MIL"?Number(q.note_m)||0:Number(q.note_d)||0:re=Number(C==="GK"?q.note_g:C==="DEF"?q.note_d:C==="MIL"?q.note_m:q.note_a)||0,re=re+(q.boost||0),q.stadiumBonus&&(i==="attack"&&(C==="ATT"||C==="MIL")||i==="defense"&&(C==="GK"||C==="DEF"||C==="MIL")?re+=10:i||(re+=10));const Q=(B.x-h/2).toFixed(1),U=(B.y-$/2).toFixed(1),D=m[q==null?void 0:q.rarity]||m.normal;if(q.used){const Fe=typeof import.meta<"u"&&"/"||"/",we=`${typeof window<"u"&&((M=window.location)==null?void 0:M.origin)||""}${Fe}icons/carte-dos.png`.replace(/([^:])\/\//g,"$1/");b+=`<rect x="${Q}" y="${U}" width="${h}" height="${$}" rx="5" fill="#161616"/>`,b+=`<image href="${we}" xlink:href="${we}" x="${Q}" y="${U}" width="${h}" height="${$}" preserveAspectRatio="xMidYMid slice"/>`,b+=`<rect x="${Q}" y="${U}" width="${h}" height="${$}" rx="5" fill="none" stroke="${D}" stroke-width="2" opacity="0.7"/>`,b+=`<rect x="${Q}" y="${U}" width="${h}" height="${$}" rx="5" fill="rgba(0,0,0,0.01)" class="match-used-hit" data-card-id="${q.cardId}" data-role="${C}" style="cursor:pointer"/>`;continue}const ne=se?.45:1,ue=se?"#FFD700":D,ye=se?3:(q==null?void 0:q.rarity)==="legende"||(q==null?void 0:q.rarity)==="légende"||(q==null?void 0:q.rarity)==="pepite"||(q==null?void 0:q.rarity)==="pépite"?2.5:2,me=$-p-u;b+=`<defs><clipPath id="cp-${G}"><rect x="${Q}" y="${(B.y-$/2+p).toFixed(1)}" width="${h}" height="${me}"/></clipPath></defs>`,b+=`<rect x="${Q}" y="${U}" width="${h}" height="${$}" rx="5" fill="${N}" opacity="${ne}"/>`;const xe=Pe(q);xe&&(b+=`<image href="${xe}" xlink:href="${xe}" x="${Q}" y="${(B.y-$/2+p).toFixed(1)}" width="${h}" height="${me}" clip-path="url(#cp-${G})" preserveAspectRatio="xMidYMin slice"/>`),b+=`<rect x="${Q}" y="${U}" width="${h}" height="${p}" rx="4" fill="rgba(255,255,255,0.92)"/>`,b+=`<text x="${B.x.toFixed(1)}" y="${(B.y-$/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(q.name||"").slice(0,9)}</text>`;const Le=(B.y+$/2-u).toFixed(1);b+=`<rect x="${Q}" y="${Le}" width="${h}" height="${u}" fill="rgba(255,255,255,0.92)"/>`;{const Fe=ei(q.country_code);Fe?b+=`<image href="${Fe}" xlink:href="${Fe}" x="${(B.x-20).toFixed(1)}" y="${(B.y+$/2-u+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:b+=`<text x="${(B.x-13).toFixed(1)}" y="${(B.y+$/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${_o(q.country_code)}</text>`;const Ce=q.stadiumBonus?"#E87722":"#111";b+=`<text x="${B.x.toFixed(1)}" y="${(B.y+$/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="${Ce}" font-family="Arial Black">${re}</text>`;const we=wo(q);we?b+=`<image href="${we}" xlink:href="${we}" x="${(B.x+h/2-14).toFixed(1)}" y="${(B.y+$/2-u+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:q.clubName&&(b+=`<text x="${(B.x+14).toFixed(1)}" y="${(B.y+$/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(q.clubName||"").slice(0,3).toUpperCase()}</text>`),q.stadiumBonus&&(b+=`<rect x="${(B.x-h/2).toFixed(1)}" y="${(B.y-$/2).toFixed(1)}" width="${h}" height="${$}" rx="5" fill="none" stroke="#E87722" stroke-width="2" opacity="0.8"/>`)}b+=`<rect x="${Q}" y="${U}" width="${h}" height="${$}" rx="5" fill="${se?"rgba(255,255,255,0.12)":"none"}" stroke="${ue}" stroke-width="${ye}" opacity="${ne}"/>`,Z&&(b+=`<rect x="${Q}" y="${U}" width="${h}" height="${$}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${se?"selected":""}" data-card-id="${q.cardId}" data-role="${C}" style="cursor:pointer"/>`)}const T=38;return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-T} ${-T} ${d+T*2} ${s+T*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${b}
  ...
  </div>`}async function di(e,t,i,n){var B;const{state:d,navigate:s,toast:r}=t;et(e);const c=d.params||{};if(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!c.deckId)return An(e,t,i);const f=c.deckId;let a,o,l,b;try{const C=await Promise.all([A.from("decks").select("formation,name,stadium_card_id").eq("id",f).single(),A.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation, evolution_bonus,
            player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
              clubs(encoded_name,logo_url)))`).eq("deck_id",f).order("slot_order")]);a=C[0].data,l=C[0].error,o=C[1].data,b=C[1].error}catch(C){console.error("[Match] Exception chargement deck:",C),kt(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>s("home"));return}if(l||b){console.error("[Match] Erreur Supabase:",l||b),kt(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>s("home"));return}const h=(o||[]).filter(C=>{var N;return C.is_starter&&((N=C.card)==null?void 0:N.player)}).map(C=>bi(C.card,C.position)),$=(o||[]).filter(C=>{var N;return!C.is_starter&&((N=C.card)==null?void 0:N.player)}).map(C=>bi(C.card,C.position));if(h.length<11){kt(e,"⚠️",`Deck incomplet (${h.length}/11).`,"Compléter",()=>s("decks"));return}const p=(o||[]).find(C=>{var N;return((N=C.card)==null?void 0:N.card_type)==="formation"}),u=(a==null?void 0:a.formation)||((B=p==null?void 0:p.card)==null?void 0:B.formation)||"4-4-2",{data:m,error:T}=await A.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",d.profile.id).eq("card_type","game_changer"),{data:M}=await A.from("gc_definitions").select("*").eq("is_active",!0),G=(m||[]).map(C=>({...C,_gcDef:(M==null?void 0:M.find(N=>N.name===C.gc_type||N.id===C.gc_definition_id))||null}));let q=null;if(a!=null&&a.stadium_card_id){const{data:C}=await A.from("cards").select("stadium_id").eq("id",a.stadium_card_id).single();if(C!=null&&C.stadium_id){const{data:N}=await A.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").eq("id",C.stadium_id).single();q=N||null}}n({deckId:f,formation:u,starters:h,subsRaw:$,gcCardsEnriched:G,gcDefs:M||[],stadiumDef:q})}async function zn(e,t){const{state:i}=t,d=(i.params||{}).matchMode||"vs_ai_easy",s=d.replace("vs_ai_",""),r=d;await di(e,t,d,async({deckId:c,formation:f,starters:a,subsRaw:o,gcCardsEnriched:l,gcDefs:b,stadiumDef:h})=>{try{let $=bt(a,f);h&&($=Ct($,h),Ot(o,h));const p=await Sn(f,s),u=p.lines||p,m=async T=>{try{const{data:M,error:G}=await A.from("matches").insert({home_id:i.profile.id,away_id:null,mode:r,home_deck_id:c,status:"in_progress"}).select().single();if(G){console.error("[MatchIA] Erreur création match:",G),kt(e,"⚠️","Impossible de créer le match ("+G.message+").","Retour",()=>t.navigate("home"));return}const q=p.stadiumDef||null;q&&u&&(Ct(u,q),Ot(p.subs||[],q));const B={gcDefs:b||[],matchId:M==null?void 0:M.id,mode:r,difficulty:s,formation:f,homeTeam:$,aiTeam:u,homeSubs:o,subsUsed:0,maxSubs:Math.min(o.length,3),aiSubs:p.subs||[],aiSubsUsed:0,aiMaxSubs:Math.min((p.subs||[]).length,3),aiUsedSubIds:[],aiGcCards:p.gcCards||[],aiUsedGc:[],aiStadiumDef:q,homeScore:0,aiScore:0,gcCards:T,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};In(e,B,t)}catch(M){console.error("[MatchIA] Exception launchMatch:",M),kt(e,"⚠️","Erreur au lancement du match : "+M.message,"Retour",()=>t.navigate("home"))}};if(!l.length){m([]);return}si(e,l,m)}catch($){console.error("[MatchIA] Exception setup:",$),kt(e,"⚠️","Erreur de préparation du match : "+$.message,"Retour",()=>t.navigate("home"))}})}async function Sn(e,t){var u;const{data:i}=await A.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(80);if(!i||i.length<11)return{lines:Ln(e),subs:[],gcCards:[],stadiumDef:null};const n=Mt[e]||Mt["4-4-2"],d={GK:[],DEF:[],MIL:[],ATT:[]},s=new Set;function r(m,T,M){var G,q;return s.add(m.id),{cardId:"ai-"+m.id+"-"+M,id:m.id,firstname:m.firstname,name:m.surname_encoded,country_code:m.country_code,club_id:m.club_id,job:m.job,job2:m.job2,note_g:Number(m.note_g)||0,note_d:Number(m.note_d)||0,note_m:Number(m.note_m)||0,note_a:Number(m.note_a)||0,rarity:m.rarity,skin:m.skin,hair:m.hair,hair_length:m.hair_length,clubName:((G=m.clubs)==null?void 0:G.encoded_name)||null,clubLogo:((q=m.clubs)==null?void 0:q.logo_url)||null,boost:0,used:!1,_line:T}}for(const m of["GK","DEF","MIL","ATT"]){const T=i.filter(C=>C.job===m&&!s.has(C.id)),M=i.filter(C=>C.job!==m&&!s.has(C.id)),G=[...T,...M],q=[];for(let C=0;C<n[m];C++){const N=G[C];N&&q.push(r(N,m,C))}const B=st(q.length);q.forEach((C,N)=>{C._col=B[N]}),d[m]=q}const f=i.filter(m=>!s.has(m.id)).slice(0,5).map((m,T)=>r(m,m.job,100+T)),l=Object.keys(Re).sort(()=>Math.random()-.5).slice(0,3).map((m,T)=>{var M,G;return{id:"ai-gc-"+T,gc_type:m,name:((M=Re[m])==null?void 0:M.name)||m,icon:((G=Re[m])==null?void 0:G.icon)||"⚡"}}),b=Object.values(d).flat(),h={};b.forEach(m=>{m.club_id&&(h[m.club_id]=(h[m.club_id]||0)+1)});const $=(u=Object.entries(h).sort((m,T)=>T[1]-m[1])[0])==null?void 0:u[0];let p=null;if($){const{data:m}=await A.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",$).single();m&&(p={club_id:m.id,country_code:null,name:m.encoded_name+" Stadium",club:{encoded_name:m.encoded_name,logo_url:m.logo_url}})}return{lines:d,subs:f,gcCards:l,stadiumDef:p}}function Ln(e){const t=Mt[e]||Mt["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},n=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let d=0;for(const s of["GK","DEF","MIL","ATT"]){const r=[];for(let f=0;f<t[s];f++){const a=3+Math.floor(Math.random()*5);r.push({cardId:"fake-"+d,id:"fake-"+d,firstname:"IA",name:n[d%n.length],country_code:"XX",club_id:null,job:s,job2:null,note_g:s==="GK"?a:2,note_d:s==="DEF"?a:2,note_m:s==="MIL"?a:2,note_a:s==="ATT"?a:2,rarity:"normal",boost:0,used:!1,_line:s}),d++}const c=st(r.length);r.forEach((f,a)=>{f._col=c[a]}),i[s]=r}return i}function In(e,t,i){var n;e.innerHTML=`
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
  </div>`,setTimeout(()=>Mn(e,t,i),5e3)}function Mn(e,t,i){const n=t.homeTeam.MIL||[],d=t.aiTeam.MIL||[],s=t.stadiumDef||null,r=t.aiStadiumDef||null;function c(T,M){const G=ze(T,"MIL"),q=T.evolution_bonus||T._evolution_bonus||0,B=T.job==="MIL"||T.job2==="MIL"?q:0,C=T.stadiumBonus||M&&(M.club_id&&String(T.club_id)===String(M.club_id)||M.country_code&&T.country_code===M.country_code)?10:0;return G+B+C}function f(T,M){return T.reduce((G,q)=>G+c(q,M),0)}function a(T){let M=0;for(let G=0;G<T.length-1;G++){const q=nt(T[G],T[G+1]);q==="#00ff88"?M+=2:q==="#FFD700"&&(M+=1)}return M}const o=f(n,s)+a(n),l=f(d,r)+a(d),b=o>=l;function h(T,M,G,q,B){return`<div id="duel-row-${q}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0), opacity .5s ease;transform-origin:center">
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${T.map((C,N)=>{const J=N<T.length-1?nt(C,T[N+1]):null,Z=!J||J==="#ff3333"||J==="#cc2222",se=J==="#00ff88"?"+2":J==="#FFD700"?"+1":"";c(C,B),C.stadiumBonus||B&&(B.club_id&&String(C.club_id)===String(B.club_id)||B.country_code&&(C.country_code,B.country_code));const re=C.evolution_bonus||C._evolution_bonus||0;return`
          <div class="duel-card duel-card-${q}" data-idx="${N}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease, transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
            ${We({...C,_evolution_bonus:re},58,78,B)}
          </div>
          ${N<T.length-1?`<div class="duel-link duel-link-${q}" data-idx="${N}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${Z?"rgba(255,255,255,0.12)":J};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${Z?"none":`0 0 8px ${J}`}">
  ...
  <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div id="duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center, rgba(10,61,30,0.4), rgba(10,61,30,0.92))"></div>
  </div>`;const $=()=>{const T=(M,G)=>e.querySelectorAll(M).forEach((q,B)=>{setTimeout(()=>{q.style.opacity="1",q.style.transform="translateY(0) scale(1)"},G+B*90)});T(".duel-card-home",150),T(".duel-card-ai",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((M,G)=>{setTimeout(()=>{M.style.opacity="1"},G*70)}),900),setTimeout(()=>{const M=e.querySelector("#vs-label");M&&(M.style.opacity="1",M.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(G=>G.style.opacity="1")},1250),setTimeout(()=>{p("score-home",o,800),p("score-ai",l,800)},1500)};function p(T,M,G){const q=document.getElementById(T);if(!q)return;const B=performance.now(),C=N=>{const J=Math.min(1,(N-B)/G);q.textContent=Math.round(M*(1-Math.pow(1-J,3))),J<1?requestAnimationFrame(C):q.textContent=M};requestAnimationFrame(C)}requestAnimationFrame($),t.attacker=b?"home":"ai";const u=b?ai():null;b&&(t.boostCard={value:u}),t.log.push({type:"duel",title:"Milieu de Terrain",homePlayers:n.map(T=>({name:T.name,note:ze(T,"MIL"),portrait:Pe(T),job:T.job,country_code:T.country_code,rarity:T.rarity,clubName:T.clubName,clubLogo:T.clubLogo})),aiPlayers:d.map(T=>({name:T.name,note:ze(T,"MIL"),portrait:Pe(T),job:T.job,country_code:T.country_code,rarity:T.rarity,clubName:T.clubName,clubLogo:T.clubLogo})),homeTotal:o,aiTotal:l,text:`Duel milieu : ${t.clubName} ${o} – ${l} IA → ${b?t.clubName+" attaque":"IA attaque"}`});const m=()=>{t.phase=t.attacker==="home"?"attack":"ai-attack",Ye(e,t,i),t.attacker==="ai"&&setTimeout(()=>ti(e,t,i),800)};setTimeout(()=>{const T=document.getElementById("score-home"),M=document.getElementById("score-ai"),G=document.getElementById(b?"duel-row-home":"duel-row-ai"),q=document.getElementById(b?"duel-row-ai":"duel-row-home"),B=b?T:M,C=b?M:T;B&&(B.style.fontSize="80px",B.style.color=b?"#FFD700":"#ff6b6b",B.style.animation="duelPulse .5s ease"+(b?", duelGlow 1.5s ease infinite .5s":"")),C&&(C.style.opacity="0.25"),setTimeout(()=>{G&&(G.style.transformOrigin="center",G.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",G.style.zIndex="5"),setTimeout(()=>{var J;const N=document.getElementById("duel-shock");if(N){const Z=(J=q||G)==null?void 0:J.getBoundingClientRect(),se=e.querySelector(".match-screen").getBoundingClientRect();Z&&(N.style.top=Z.top-se.top+Z.height/2+"px"),N.style.animation="shockwave .5s ease-out forwards"}q&&(q.style.transformOrigin="center",q.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var J;const N=document.getElementById("duel-finale");N&&(N.innerHTML=`
          ...
          <button id="start-match-btn" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,0.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">
            ▶ Commencer le match
          </button>`,N.style.transition="opacity .45s ease",N.style.opacity="1",N.style.pointerEvents="auto",(J=document.getElementById("start-match-btn"))==null||J.addEventListener("click",m))},600)},700)},2800)}function jn(e){if(e.type==="duel"&&(e.homeTotal!=null||e.aiTotal!=null)){const t=(e.homeTotal||0)>=(e.aiTotal||0);return`
    ...
    <div style="display:flex;align-items:center;gap:4px;${t?"flex-direction:row-reverse":""};background:rgba(255,255,255,0.04);border-radius:8px;padding:5px 8px;border:1px solid rgba(255,255,255,0.07)">
      ...
      ${at(e.inPlayer||{})}
    </div>`}return`<div style="font-size:11px;color:${e.type==="goal"?"#FFD700":"rgba(255,255,255,0.65)"};font-weight:${e.type==="goal"?700:400};padding:3px 2px">${e.text||""}</div>`}function Ye(e,t,i){var B,C,N,J,Z,se,re,Q,U;const n=t.selected.map(D=>D.cardId),d=t.usedSubIds||[],s=t.homeSubs.filter(D=>!d.includes(D.cardId)),c=Object.values(t.homeTeam).flat().filter(D=>D.used).length>0&&s.length>0&&t.subsUsed<t.maxSubs,f=!["GK","DEF","MIL","ATT"].some(D=>(t.aiTeam[D]||[]).some(ne=>!ne.used)),a=[...t.homeTeam.MIL||[],...t.homeTeam.ATT||[]].filter(D=>!D.used),o=t.phase==="attack"&&f&&a.length===0?[...t.homeTeam.GK||[],...t.homeTeam.DEF||[]].filter(D=>!D.used).map(D=>D.cardId):[];t.log[t.log.length-1];const l=t.phase==="ai-attack"||t.phase==="ai-defense",b=t.phase==="attack",h=t.phase==="defense",$=t.phase==="finished",u=(t.homeSubs||[]).filter(D=>!(t.usedSubIds||[]).includes(D.cardId)).length>0&&t.subsUsed<t.maxSubs,m=b&&a.length===0&&!u,T=t.gcCards.filter(D=>!t.usedGc.includes(D.id)),M=t.boostCard&&!t.boostUsed;e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column",e.innerHTML=`
  <style>
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const ne=t.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
          ...
          </div>`}const D=t.log[t.log.length-1];return D?'<div style="padding:2px 4px">'+jn(D)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})()}
    </div>
    ...
    ${(()=>{const D=window.innerWidth>=700,ne=(V,oe,fe)=>{var w,z;const K=(t.gcDefs||[]).find(g=>g.name===V.gc_type),te={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[K==null?void 0:K.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",Y={purple:"#b06ce0",light_blue:"#00d4ef"}[K==null?void 0:K.color]||"#b06ce0",de=(K==null?void 0:K.name)||V.gc_type,ve=(K==null?void 0:K.effect)||((w=Re[V.gc_type])==null?void 0:w.desc)||"",Te=K!=null&&K.image_url?`/icons/${K.image_url}`:null,$e=((z=Re[V.gc_type])==null?void 0:z.icon)||"⚡",be=Math.round(fe*.22),ke=Math.round(fe*.22),qe=fe-be-ke,je=de.length>12?7:9;return`<div class="gc-mini" data-gc-id="${V.id}" data-gc-type="${V.gc_type}"
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(D=>{var ne,ue,ye;if(D.type==="duel"){const me=D.isGoal,xe=D.homeScored?"#FFD700":me?"#ff6b6b":"rgba(255,255,255,0.3)",Le=D.homeScored?"⚽ BUT !":me?"⚽ BUT IA !":(ne=D.homePlayers)!=null&&ne.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${me?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${xe};margin-bottom:4px">
                <div style="font-size:9px;color:${xe};letter-spacing:1px;margin-bottom:5px;font-weight:700;text-transform:uppercase">${Le}</div>
                ${(ue=D.homePlayers)!=null&&ue.length?`<div style="margin-bottom:3px">${tt(D.homePlayers,"rgba(255,255,255,0.7)",D.homeTotal)}</div>`:""}
                ${(ye=D.aiPlayers)!=null&&ye.length?`<div style="opacity:0.7">${tt(D.aiPlayers,"#ff6b6b",D.aiTotal)}</div>`:""}
                ...
                <div style="display:flex;align-items:center;gap:8px">
                  ${D.outPlayer?We({...D.outPlayer,used:!0,_line:D.outPlayer.job,rarity:"normal"},38,50):""}
                  <span style="color:rgba(255,255,255,0.4);font-size:18px">→</span>
                  ${D.inPlayer?We({...D.inPlayer,_line:D.inPlayer.job,rarity:"normal"},38,50):""}
                </div>
  ...
  `,document.body.appendChild(n);let o=!1;const l=()=>{o||(o=!0,n.remove(),i())};n.addEventListener("click",l),setTimeout(l,2e3)}function qt(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function ui(e,t,i,n=null,d=null){var h,$;if(t.phase!=="attack"){qt("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){qt(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const s=Object.entries(t.homeTeam).flatMap(([p,u])=>(u||[]).filter(m=>m.used).map(m=>({...m,_line:m._line||p}))),r=t.homeSubs.filter(p=>!t.usedSubIds.includes(p.cardId));if(!s.length){qt("Aucun joueur utilisé à remplacer");return}if(!r.length){qt("Aucun remplaçant disponible");return}let c=Math.max(0,s.findIndex(p=>p.cardId===d));const f=((h=s[c])==null?void 0:h._line)||(($=s[c])==null?void 0:$.job);let a=n?Math.max(0,r.findIndex(p=>p.cardId===n)):Math.max(0,r.findIndex(p=>p.job===f)),o=!1;const l=document.createElement("div");l.id="sub-overlay",l.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function b(){var q,B,C,N,J,Z;const p=s[c],u=r[a],m=Math.min(130,Math.round((window.innerWidth-90)/2)),T=Math.round(m*1.35),M=se=>`background:rgba(255,255,255,0.12);border:none;color:${se?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${se?"default":"pointer"};flex-shrink:0`;l.innerHTML=`
    ...
    <div style="flex:1;display:flex;gap:0;overflow:hidden">
      ...
      <div id="in-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
        ...
        <div>${u?We({...u,used:!1,boost:0},m,T):"<div>—</div>"}</div>
      ...
      <div id="out-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
        ...
        <button id="out-up" style="${M(c===0)}" ${c===0?"disabled":""}>▲</button>
        <div>${p?We({...p,used:!1,boost:0},m,T):"<div>—</div>"}</div>
        <button id="out-down" style="${M(c>=s.length-1)}" ${c>=s.length-1?"disabled":""}>▼</button>
        <div style="font-size:10px;color:rgba(255,255,255,0.35)">${c+1}/${s.length}</div>
      </div>
    </div>
    <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
      <button id="sub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
    </div>`,(q=l.querySelector("#sub-close"))==null||q.addEventListener("click",()=>l.remove()),(B=l.querySelector("#out-up"))==null||B.addEventListener("click",()=>{c>0&&(c--,b())}),(C=l.querySelector("#out-down"))==null||C.addEventListener("click",()=>{c<s.length-1&&(c++,b())}),(N=l.querySelector("#in-up"))==null||N.addEventListener("click",()=>{a>0&&(a--,b())}),(J=l.querySelector("#in-down"))==null||J.addEventListener("click",()=>{a<r.length-1&&(a++,b())});const G=(se,re,Q,U)=>{const D=l.querySelector("#"+se);if(!D)return;let ne=0;D.addEventListener("touchstart",ue=>{ne=ue.touches[0].clientY},{passive:!0}),D.addEventListener("touchend",ue=>{const ye=ue.changedTouches[0].clientY-ne;if(Math.abs(ye)<30)return;const me=re();ye<0&&me<U-1?(Q(me+1),b()):ye>0&&me>0&&(Q(me-1),b())},{passive:!0})};G("in-panel",()=>a,se=>a=se,r.length),G("out-panel",()=>c,se=>c=se,s.length),(Z=l.querySelector("#sub-confirm"))==null||Z.addEventListener("click",se=>{if(se.preventDefault(),se.stopPropagation(),o)return;o=!0;const re=s[c],Q=r[a];if(!re||!Q)return;let U=null,D=-1;for(const[ue,ye]of Object.entries(t.homeTeam)){const me=(ye||[]).findIndex(xe=>xe.cardId===re.cardId);if(me!==-1){U=ue,D=me;break}}if(D===-1||!U){qt("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),l.remove();return}const ne={...Q,_line:U,_col:re._col||0,used:!1,boost:0};t.homeTeam[U].splice(D,1,ne),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(Q.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:re.name,firstname:re.firstname,note:ze(re,U),portrait:Pe(re),job:re.job,country_code:re.country_code,rarity:re.rarity,clubName:re.clubName,clubLogo:re.clubLogo},inPlayer:{name:Q.name,firstname:Q.firstname,note:ze(Q,U),portrait:Pe(Q),job:Q.job,country_code:Q.country_code,rarity:Q.rarity,clubName:Q.clubName,clubLogo:Q.clubLogo},text:`🔄 ${Q.firstname} ${Q.name} remplace ${re.firstname} ${re.name}`}),l.remove(),Pn(re,Q,()=>Ye(e,t,i))})}document.body.appendChild(l),b()}function Nn(e,t,i,n,d){var $,p;const s=(n.gcDefs||[]).find(u=>u.name===t),r=Re[t]||{icon:"⚡",desc:"Carte spéciale."},c={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[s==null?void 0:s.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",f={purple:"#b06ce0",light_blue:"#00d4ef"}[s==null?void 0:s.color]||"#b06ce0",a=(s==null?void 0:s.name)||t,o=(s==null?void 0:s.effect)||r.desc,l=s!=null&&s.image_url?`/icons/${s.image_url}`:null,b=r.icon||"⚡",h=document.createElement("div");h.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",h.innerHTML=`
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(s);let c=!1;const f=()=>{c||(c=!0,s.remove(),setTimeout(()=>d(),50))};s.addEventListener("click",f),setTimeout(f,3500)}async function Dt(e,t,i){var o,l;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:n}=i,d=t.homeScore>t.aiScore,s=t.homeScore===t.aiScore,r=d?"victoire":s?"nul":"defaite",c=Wo(t.mode,r);t.matchId&&await A.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:d?n.profile.id:null,home_credits_reward:c,played_at:new Date().toISOString()}).eq("id",t.matchId);const f={credits:(n.profile.credits||0)+c,matches_played:(n.profile.matches_played||0)+1};d?f.wins=(n.profile.wins||0)+1:s?f.draws=(n.profile.draws||0)+1:f.losses=(n.profile.losses||0)+1,await A.from("users").update(f).eq("id",n.profile.id),await i.refreshProfile();const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",a.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const G=async(N=!0)=>{$=!0,p&&(A.removeChannel(p),p=null),N&&await A.rpc("cancel_matchmaking",{p_user_id:l.profile.id}).catch(()=>{})};(C=document.getElementById("mm-cancel"))==null||C.addEventListener("click",async()=>{try{await G(!0)}catch{}Ve(e),b("home")});const q=async(N,J,Z)=>{if($)return;$=!0,p&&(A.removeChannel(p),p=null);const se=document.getElementById("mm-status");se&&(se.textContent="Adversaire trouvé !"),await new Promise(re=>setTimeout(re,600)),e.isConnected&&ko(e,t,N,Z,r,c,a,o)};if(a){const N=[{range:50,delay:0},{range:100,delay:15e3},{range:200,delay:3e4},{range:400,delay:45e3},{range:800,delay:6e4}];let J=0,Z=!1;const se=async()=>{if($||Z)return;const{range:re}=N[J],Q=document.getElementById("mm-range");Q&&(Q.textContent=`Plage MMR : ±${re}`);const{data:U,error:D}=await A.rpc("try_matchmake_ranked",{p_user_id:l.profile.id,p_deck_id:i,p_mmr:u,p_range:re});if(console.log("[Ranked] try_matchmake_ranked result:",U,"error:",JSON.stringify(D)),$)return;if(D||!(U!=null&&U.success)){h("Erreur matchmaking ranked","error"),Ve(e),b("home");return}if(U.matched){Z=!0,q(U.match_id,U.opponent_id,!1);return}const ne=document.getElementById("mm-status");if(ne&&(ne.textContent="En attente d'un adversaire..."),p||(p=A.channel("matchmaking-"+l.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${l.profile.id}`},ue=>{const ye=ue.new;ye.status==="matched"&&ye.match_id&&!Z&&(Z=!0,q(ye.match_id,ye.matched_with,!0))}).subscribe()),J<N.length-1){const ue=N[J+1].delay-N[J].delay;J++,setTimeout(()=>{!$&&!Z&&se()},ue)}};await se()}else{const{data:N,error:J}=await A.rpc("try_matchmake",{p_user_id:l.profile.id,p_deck_id:i});if(J||!(N!=null&&N.success)){h("Erreur de matchmaking","error"),Ve(e),b("home");return}if(N.matched){q(N.match_id,N.opponent_id,!1);return}const Z=document.getElementById("mm-status");Z&&(Z.textContent="En attente d'un autre joueur..."),p=A.channel("matchmaking-"+l.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${l.profile.id}`},se=>{const re=se.new;re.status==="matched"&&re.match_id&&q(re.match_id,re.matched_with,!0)}).subscribe()}}async function Vn(e,t,i){const{state:n,navigate:d,toast:s}=t;try{(A.getChannels?A.getChannels():[]).forEach(a=>{const o=a.topic||"";(o.includes("matchmaking")||o.includes("pvp-match")||o.includes("friend-invite"))&&A.removeChannel(a)})}catch{}const{data:r}=await A.from("matches").select("home_id, away_id, status").eq("id",i).single();if(!r){s("Match introuvable","error"),d("home");return}if(r.status==="finished"){s("Ce match est terminé","info"),d("home");return}const c=r.home_id===n.profile.id;ko(e,t,i,c,[],[])}async function ko(e,t,i,n,d=[],s=[],r=!1,c=null){const{state:f,navigate:a,toast:o}=t,l=n?"p1":"p2",b=n?"p2":"p1",h=(d||[]).map(w=>w.id),$=(d||[]).map(w=>({id:w.id,gc_type:w.gc_type,_gcDef:w._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:p}=await A.from("matches").select("*").eq("id",i).single();if(!p){o("Match introuvable","error"),a("home");return}async function u(){var ee,R;console.log("[PvP] buildGameState match:",{id:p.id,home_deck_id:p.home_deck_id,away_deck_id:p.away_deck_id,mode:p.mode,is_ranked:p.is_ranked});const[{data:w,error:z},{data:g,error:y},{data:x},{data:_}]=await Promise.all([A.rpc("get_deck_for_match",{p_deck_id:p.home_deck_id}),A.rpc("get_deck_for_match",{p_deck_id:p.away_deck_id}),A.from("users").select("id,pseudo,club_name").eq("id",p.home_id).single(),A.from("users").select("id,pseudo,club_name").eq("id",p.away_id).single()]);console.log("[PvP] get_deck_for_match p1:",z==null?void 0:z.message,"p2:",y==null?void 0:y.message,"p1D:",(ee=w==null?void 0:w.starters)==null?void 0:ee.length,"p2D:",(R=g==null?void 0:g.starters)==null?void 0:R.length);const v=F=>{const H=Number(F.evolution_bonus)||0;return{cardId:F.card_id,position:F.position,id:F.id,firstname:F.firstname,name:F.surname_encoded,country_code:F.country_code,club_id:F.club_id,job:F.job,job2:F.job2,note_g:(Number(F.note_g)||0)+(F.job==="GK"||F.job2==="GK"&&Number(F.note_g)>0?H:0),note_d:(Number(F.note_d)||0)+(F.job==="DEF"||F.job2==="DEF"&&Number(F.note_d)>0?H:0),note_m:(Number(F.note_m)||0)+(F.job==="MIL"||F.job2==="MIL"&&Number(F.note_m)>0?H:0),note_a:(Number(F.note_a)||0)+(F.job==="ATT"||F.job2==="ATT"&&Number(F.note_a)>0?H:0),evolution_bonus:H,rarity:F.rarity,skin:F.skin,hair:F.hair,hair_length:F.hair_length,clubName:F.club_encoded_name||null,clubLogo:F.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},k=((w==null?void 0:w.starters)||[]).map(F=>v(F)),L=((g==null?void 0:g.starters)||[]).map(F=>v(F)),E=(w==null?void 0:w.formation)||"4-4-2",I=(g==null?void 0:g.formation)||"4-4-2";let S=bt(k,E),j=bt(L,I);const O=((w==null?void 0:w.subs)||[]).map(F=>v(F));return stadiumDef&&n&&(S=Ct(S,stadiumDef),Ot(O,stadiumDef)),{p1Team:S,p2Team:j,p1Subs:O,p2Subs:((g==null?void 0:g.subs)||[]).map(F=>v(F)),p1Formation:E,p2Formation:I,p1Name:(x==null?void 0:x.club_name)||(x==null?void 0:x.pseudo)||"Joueur 1",p2Name:(_==null?void 0:_.club_name)||(_==null?void 0:_.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:n?h:[],gc_p2:n?[]:h,gcCardsFull_p1:n?$:[],gcCardsFull_p2:n?[]:$,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:s||[],lastActionAt:new Date().toISOString()}}let m=p.game_state&&Object.keys(p.game_state).length?p.game_state:null;if(!m)if(n){m=await u();const{data:w}=await A.from("matches").select("game_state").eq("id",i).single();!(w!=null&&w.game_state)||!Object.keys(w.game_state).length?await A.from("matches").update({game_state:m,turn_user_id:p.home_id}).eq("id",i):m=w.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let w=0;w<30&&!m;w++){await new Promise(g=>setTimeout(g,400));const{data:z}=await A.from("matches").select("game_state").eq("id",i).single();z!=null&&z.game_state&&Object.keys(z.game_state).length&&(m=z.game_state)}if(!m){o("Erreur de synchronisation","error"),a("home");return}m.gc_p2=h,m.gcCardsFull_p2=$,await A.from("matches").update({game_state:m}).eq("id",i)}let T=!1,M=null,G=!1;const q=new Set,B=new Set;async function C(w){var I,S;try{A.removeChannel(N)}catch{}const z=m[l+"Score"]||0,g=m[b+"Score"]||0;let y,x;w.winner_id?(y=w.winner_id===f.profile.id,x=!1):w.forfeit?(y=z>g,x=!1):(x=z===g,y=z>g);let _="";if(r&&n)try{const{data:j}=await A.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",p.home_id).single(),{data:O}=await A.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",p.away_id).single();if(j&&O){const ee=y?1:x?.5:0,R=1-ee,F=j.placement_matches<10,H=O.placement_matches<10,X=ji(j.mmr,j.mmr_deviation,j.mmr_volatility,O.mmr,O.mmr_deviation,ee===1?1:ee===0?0:.5,F),ae=ji(O.mmr,O.mmr_deviation,O.mmr_volatility,j.mmr,j.mmr_deviation,R===1?1:R===0?0:.5,H);await A.rpc("update_mmr_after_ranked",{p_match_id:i,p_winner_id:x?null:y?p.home_id:p.away_id,p_home_id:p.home_id,p_away_id:p.away_id,p_home_delta:X.delta,p_away_delta:ae.delta,p_home_new_rd:X.newRd,p_away_new_rd:ae.newRd,p_home_new_vol:X.newSigma,p_away_new_vol:ae.newSigma});const pe=X.delta,ce=X.newMmr,ge=Pt(ce),he=pe>=0?"+":"",Ie=pe>=0?"#4caf50":"#ff6b6b",Se=Pt(j.mmr);_=`
      ...
      <div style="font-size:18px">${m[l+"Name"]} ${z} – ${g} ${m[b+"Name"]}</div>
      ${w.forfeit?`<div style="font-size:13px;color:rgba(255,255,255,0.5)">${y?"L'adversaire a quitté":"Perdu par forfait"}</div>`:""}
      ${_}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(v),(S=v.querySelector("#pvp-end-home"))==null||S.addEventListener("click",()=>{v.remove(),Ve(e),a("home")})}const N=A.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},w=>{const z=w.new;try{if(z.status==="finished"||z.forfeit){if(T)return;T=!0,M&&(clearInterval(M),M=null),z.game_state&&(m=z.game_state),C(z);return}if(z.game_state){const g=m;m=z.game_state;const y=m._lastGC;if(y&&y.seq&&!B.has(y.seq)&&(B.add(y.seq),y.by!==l)){Le(y.type,y.by,()=>ne());return}const x=g[l+"Score"]||0,_=g[b+"Score"]||0,v=m[l+"Score"]||0,k=m[b+"Score"]||0,L=v>x,E=k>_;if((L||E)&&!q.has(m.round)){q.add(m.round);const I=[...m.log||[]].reverse().find(j=>j.isGoal),S=((I==null?void 0:I.homePlayers)||[]).map(j=>({name:j.name,note:j.note,portrait:j.portrait,job:j.job}));xe(S,v,k,L,()=>ne());return}ne()}}catch(g){console.error("[PvP] crash:",g)}}).subscribe();async function J(w){Object.assign(m,w),m.lastActionAt=new Date().toISOString();const{error:z}=await A.from("matches").update({game_state:m}).eq("id",i);z&&o("Erreur de synchronisation","error");try{ne()}catch(g){console.error("[PvP] renderPvpScreen crash:",g)}}async function Z(){if(T)return;T=!0,M&&(clearInterval(M),M=null);const w=n?p.away_id:p.home_id,z=n?"p2":"p1",g=n?"p1":"p2",y={...m,[z+"Score"]:3,[g+"Score"]:0,phase:"finished"};try{await A.from("matches").update({status:"finished",forfeit:!0,winner_id:w,home_score:y.p1Score||0,away_score:y.p2Score||0,game_state:y}).eq("id",i)}catch{}try{A.removeChannel(N)}catch{}setTimeout(()=>{Ve(e),a("home")},800)}const se={BOOST_STAT:({value:w=1,count:z=1,roles:g=[]},y,x)=>{const _=y[l+"Team"],v=Object.entries(_).filter(([k])=>!g.length||g.includes(k)).flatMap(([k,L])=>L.filter(E=>!E.used).map(E=>({...E,_line:k})));if(!v.length){y.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),x(y);return}re(v,z,`Choisir ${z} joueur(s) à booster (+${w})`,k=>{k.forEach(L=>{const E=(_[L._line]||[]).find(I=>I.cardId===L.cardId);E&&(E.boost=(E.boost||0)+w,y.log.push({text:`⚡ +${w} sur ${E.name}`,type:"info"}))}),y[l+"Team"]=_,x(y)})},DEBUFF_STAT:({value:w=1,count:z=1,roles:g=[],target:y="ai"},x,_)=>{const v=y==="home"?l:b,k=x[v+"Team"],L=y==="home"?"allié":"adverse",E=Object.entries(k).filter(([I])=>!g.length||g.includes(I)).flatMap(([I,S])=>S.map(j=>({...j,_line:I})));if(!E.length){x.log.push({text:`🎯 Aucun joueur ${L}`,type:"info"}),_(x);return}re(E,z,`Choisir ${z} joueur(s) ${L}(s) (-${w})`,I=>{I.forEach(S=>{const j=(k[S._line]||[]).find(O=>O.cardId===S.cardId);j&&(j.boost=(j.boost||0)-w,x.log.push({text:`🎯 -${w} sur ${j.name}`,type:"info"}))}),x[v+"Team"]=k,_(x)})},GRAY_PLAYER:({count:w=1,roles:z=[],target:g="ai"},y,x)=>{const _=g==="home"?l:b,v=y[_+"Team"],k=g==="home"?"allié":"adverse",L=Object.entries(v).filter(([E])=>!z.length||z.includes(E)).flatMap(([E,I])=>I.filter(S=>!S.used).map(S=>({...S,_line:E})));if(!L.length){y.log.push({text:`❌ Aucun joueur ${k}`,type:"info"}),x(y);return}re(L,w,`Choisir ${w} joueur(s) ${k}(s) à exclure`,E=>{const I="usedCardIds_"+_,S=new Set(y[I]||[]);E.forEach(j=>{const O=(v[j._line]||[]).find(ee=>ee.cardId===j.cardId);O&&(O.used=!0,S.add(j.cardId),y.log.push({text:`❌ ${O.name} exclu !`,type:"info"}))}),y[I]=[...S],y[_+"Team"]=v,x(y)})},REVIVE_PLAYER:({count:w=1,roles:z=[]},g,y)=>{const x=g[l+"Team"],_=Object.entries(x).filter(([v])=>!z.length||z.includes(v)).flatMap(([v,k])=>k.filter(L=>L.used).map(L=>({...L,_line:v})));if(!_.length){g.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),y(g);return}re(_,w,`Choisir ${w} joueur(s) à ressusciter`,v=>{v.forEach(k=>{const L=(x[k._line]||[]).find(E=>E.cardId===k.cardId);L&&(L.used=!1,g.log.push({text:`💫 ${L.name} ressuscité !`,type:"info"}))}),g[l+"Team"]=x,y(g)})},REMOVE_GOAL:({},w,z)=>{const g=b+"Score";w[g]>0?(w[g]--,w.log.push({text:"🚫 Dernier but annulé !",type:"info"})):w.log.push({text:"🚫 Aucun but à annuler",type:"info"}),z(w)},ADD_GOAL_DRAW:({},w,z)=>{w[l+"Score"]===w[b+"Score"]?(w[l+"Score"]++,w.log.push({text:"🎯 But bonus !",type:"info"})):w.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),z(w)},ADD_SUB:({value:w=1},z,g)=>{z.maxSubs=(z.maxSubs||3)+w,z.log.push({text:`🔄 +${w} remplacement(s)`,type:"info"}),g(z)},CUSTOM:({},w,z)=>z(w)};function re(w,z,g,y){const x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let _=[];function v(){var L,E;const k=w.map(I=>{const S={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[I._line]||"#555",j=ze(I,I._line)+(I.boost||0),ee=_.find(F=>F.cardId===I.cardId)?"#FFD700":"rgba(255,255,255,0.25)",R=I.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${I.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${ee};background:${S};overflow:hidden;cursor:pointer;${R}">
        ...
        <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
          ...
          <span style="font-size:12px;color:rgba(255,255,255,0.5)">${_.length}/${z}</span>
        ...
        <div style="padding:12px;background:rgba(0,0,0,0.4);flex-shrink:0">
          <button id="pp-confirm" ${_.length===0?'disabled style="opacity:0.4"':""} style="width:100%;padding:13px;border-radius:10px;border:none;background:#7a28b8;color:#fff;font-size:15px;font-weight:900;cursor:pointer">
            ✅ Confirmer (${_.length}/${z})
          </button>
        </div>`,(L=x.querySelector("#pp-close"))==null||L.addEventListener("click",()=>x.remove()),x.querySelectorAll(".pp-item").forEach(I=>{I.addEventListener("click",()=>{const S=I.dataset.cid,j=w.find(ee=>ee.cardId===S),O=_.findIndex(ee=>ee.cardId===S);j&&(O>-1?_.splice(O,1):_.length<z&&_.push(j),v())})}),(E=x.querySelector("#pp-confirm"))==null||E.addEventListener("click",()=>{x.remove(),y(_)})}v(),document.body.appendChild(x)}async function Q(w,z){const y=(m["gcCardsFull_"+l]||[]).find(k=>k.id===w),x=(y==null?void 0:y._gcDef)||(m.gcDefs||[]).find(k=>{var L;return k.name===z||((L=k.name)==null?void 0:L.toLowerCase().trim())===(z==null?void 0:z.toLowerCase().trim())}),_=[...m["usedGc_"+l]||[],w],v={type:z,by:l,seq:(m._gcAnimSeq||0)+1};B.add(v.seq),Le(z,l,async()=>{if(x!=null&&x.effect_type&&x.effect_type!=="CUSTOM"){const L=se[x.effect_type];if(L){const E={...m};L(x.effect_params||{},E,async I=>{I["usedGc_"+l]=_,I._lastGC=v,I._gcAnimSeq=v.seq,await J(I)});return}}const k={...m};switch(z){case"Remplacement+":k.maxSubs=(k.maxSubs||3)+1,k.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const L=b+"Score";k[L]>0&&(k[L]--,k.log.push({text:"🚫 But annulé",type:"info"}));break}}k["usedGc_"+l]=_,k._lastGC=v,k._gcAnimSeq=v.seq,await J(k)})}function U(w,z){const g="usedCardIds_"+w,y=new Set(m[g]||[]);z.forEach(x=>y.add(x)),m[g]=[...y]}function D(){for(const w of["p1","p2"]){const z=new Set(m["usedCardIds_"+w]||[]),g=m[w+"Team"];if(g)for(const y of["GK","DEF","MIL","ATT"])(g[y]||[]).forEach(x=>{x.used=z.has(x.cardId)})}}function ne(){var Ke,Ze,it,ut,ft,gt;if(m.phase==="reveal")return ue();if(m.phase==="midfield")return me();if(m.phase==="finished")return je();const w=m[l+"Team"],z=m[b+"Team"];D();const g=m[l+"Score"],y=m[b+"Score"],x=m[l+"Name"],_=m[b+"Name"],v=m.phase===l+"-attack",k=m.phase===l+"-defense",L=Array.isArray(m["selected_"+l])?m["selected_"+l]:[],E=L.map(P=>P.cardId),I=window.innerWidth>=700,S=m[l+"Subs"]||[],j=m["usedSubIds_"+l]||[],O=S.filter(P=>!j.includes(P.cardId)),ee=m["gcCardsFull_"+l]||[],R=m["usedGc_"+l]||[],F=ee.filter(P=>!R.includes(P.id)),H=m.boostOwner===l&&!m.boostUsed,X=!["GK","DEF","MIL","ATT"].some(P=>(z[P]||[]).some(le=>!le.used)),ae=[...w.MIL||[],...w.ATT||[]].filter(P=>!P.used),pe=v&&X&&ae.length===0?[...w.GK||[],...w.DEF||[]].filter(P=>!P.used).map(P=>P.cardId):[];function ce(P,le,W){var lt,wt;const ie=P._gcDef,Ae={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ie==null?void 0:ie.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",_e={purple:"#b06ce0",light_blue:"#00d4ef"}[ie==null?void 0:ie.color]||"#b06ce0",Xe=(ie==null?void 0:ie.name)||P.gc_type,Qe=(ie==null?void 0:ie.effect)||((lt=Re[P.gc_type])==null?void 0:lt.desc)||"",ot=ie!=null&&ie.image_url?`/icons/${ie.image_url}`:null,dt=((wt=Re[P.gc_type])==null?void 0:wt.icon)||"⚡",mt=Math.round(W*.22),vt=Math.round(W*.22),rt=W-mt-vt,Tt=Xe.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${P.id}" data-gc-type="${P.gc_type}"
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ...
        </div>`}const P=(m.log||[]).slice(-1)[0];return P?'<div style="padding:2px 4px">'+Ne(P)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})(),He=`
      <div style="display:flex;align-items:center;padding:8px 10px;background:rgba(0,0,0,0.5);gap:6px;flex-shrink:0">
      ...
      <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
        ...
        <div id="mobile-action-bar" style="position:absolute;left:0;right:0;bottom:0;z-index:20;background:rgba(0,0,0,0.55);padding:6px 8px 8px;display:flex;flex-direction:column;gap:6px;box-shadow:0 -4px 16px rgba(0,0,0,0.5)">
          <div style="display:flex;gap:6px;overflow-x:auto;align-items:flex-end;min-height:96px;padding-bottom:2px">
            ${F.map(P=>Ie(P,!1)).join("")}
            ${H?Ie(null,!0):""}
            <div id="pvp-sub-open" style="cursor:${v&&O.length>0?"pointer":"default"};flex-shrink:0;box-sizing:border-box;width:68px;height:95px;border-radius:10px;border:2px solid ${v&&O.length>0?"rgba(255,255,255,0.5)":"rgba(255,255,255,0.15)"};background:${v&&O.length>0?"rgba(60,60,60,0.9)":"rgba(40,40,40,0.5)"};display:flex;flex-direction:column;align-items:center;justify-content:center;gap:4px;opacity:${v&&O.length>0?1:.4}">
              <div style="display:flex;gap:6px;align-items:center">
                <div style="text-align:center">
                  <div style="font-size:7px;color:#00ff88;font-weight:700;letter-spacing:1px">IN</div>
      ...
      </div>`;function Be(){const P=e.querySelector(".match-screen");if(!P)return;const le=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);P.style.height=le+"px",P.style.minHeight=le+"px",P.style.maxHeight=le+"px",P.style.overflow="hidden";const W=e.querySelector("#mobile-action-bar"),ie=e.querySelector("#mobile-play-area");W&&ie&&(ie.style.paddingBottom=W.offsetHeight+"px")}if(Be(),setTimeout(Be,120),setTimeout(Be,400),G||(G=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",Be),window.visualViewport.addEventListener("scroll",Be)),window.addEventListener("resize",Be)),function(){const le=e.querySelector(".terrain-wrapper svg");le&&(le.removeAttribute("width"),le.removeAttribute("height"),le.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",le.setAttribute("viewBox","-26 -26 352 352"),le.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),e.querySelectorAll(".match-slot-hit").forEach(P=>{P.addEventListener("click",()=>{if(!v&&!k)return;const le=P.dataset.cardId,W=P.dataset.role,ie=(w[W]||[]).find(Qe=>Qe.cardId===le);if(!ie||ie.used)return;const Ae=pe.includes(le);if(v&&!["MIL","ATT"].includes(W)&&!Ae)return;Array.isArray(m["selected_"+l])||(m["selected_"+l]=[]);const _e=m["selected_"+l],Xe=_e.findIndex(Qe=>Qe.cardId===le);Xe>-1?_e.splice(Xe,1):_e.length<3&&_e.push({...ie,_role:W}),ne()})}),e.querySelectorAll(".match-used-hit").forEach(P=>{P.addEventListener("click",()=>we(P.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(P=>{P.addEventListener("click",()=>we())}),(Ke=e.querySelector("#pvp-sub-open"))==null||Ke.addEventListener("click",()=>we()),e.querySelectorAll(".pvp-gc-mini").forEach(P=>{P.addEventListener("click",()=>Fe(P.dataset.gcId,P.dataset.gcType))}),(Ze=e.querySelector("#pvp-boost-card"))==null||Ze.addEventListener("click",()=>Ce()),(it=e.querySelector("#pvp-action"))==null||it.addEventListener("click",P=>{v?P.currentTarget.dataset.pass==="1"||!$e(l)?K():te():k&&Y()}),(ut=e.querySelector("#pvp-quit"))==null||ut.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&Z()}),(ft=e.querySelector("#pvp-view-opp"))==null||ft.addEventListener("click",()=>oe()),(gt=e.querySelector("#pvp-toggle-history"))==null||gt.addEventListener("click",()=>fe()),M&&(clearInterval(M),M=null),(v||k)&&!T){let P=30,le=!1;const W=()=>document.getElementById("pvp-timer"),ie=()=>{W()&&(W().textContent=P+"s",W().style.color=le?"#ff4444":"#fff")};ie(),M=setInterval(()=>{P--,P<0?le?(clearInterval(M),M=null,v&&!$e(l)?K():Z()):(le=!0,P=15,ie()):ie()},1e3)}}function ue(){et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
      ...
      <div style="width:min(90vw,420px)">${ct(m[b+"Team"],m[b+"Formation"],null,[],300,300)}</div>
    </div>`,l==="p1"&&setTimeout(async()=>{await J({phase:"midfield"})},5e3)}let ye=!1;function me(){if(ye)return;const w=m[l+"Team"].MIL||[],z=m[b+"Team"].MIL||[];function g(F){return F.reduce((H,X)=>H+ze(X,"MIL"),0)}function y(F){let H=0;for(let X=0;X<F.length-1;X++){const ae=nt(F[X],F[X+1]);ae==="#00ff88"?H+=2:ae==="#FFD700"&&(H+=1)}return H}const x=g(w)+y(w),_=g(z)+y(z),v=x>=_;function k(F,H,X){return`<div id="duel-row-${X}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
        ...
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${F.map((ae,pe)=>{const ce=pe<F.length-1?nt(ae,F[pe+1]):null,ge=!ce||ce==="#ff3333"||ce==="#cc2222",he=ce==="#00ff88"?"+2":ce==="#FFD700"?"+1":"";return`<div class="duel-card duel-card-${X}" data-idx="${pe}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease,transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
              ${We({...ae,note:ze(ae,"MIL"),_line:"MIL"},58,78)}
            </div>
            ${pe<F.length-1?`<div class="duel-link duel-link-${X}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${ge?"rgba(255,255,255,0.12)":ce};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${ge?"none":`0 0 8px ${ce}`}">
        ...
        <div class="duel-score-line duel-score-line-${X}" style="margin-top:10px;font-size:12px;color:rgba(255,255,255,0.55);opacity:0;transition:opacity .4s ease">
          Score: ${g(F)} + ${y(F)} liens = <b style="color:#fff">${g(F)+y(F)}</b>
        </div>
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      ${k(z,m[b+"Name"]||"Adversaire","opp")}
      <div id="duel-shock" style="position:absolute;left:50%;top:50%;width:120px;height:120px;border-radius:50%;border:6px solid #FFD700;opacity:0;pointer-events:none"></div>
      <div id="pvp-duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center,rgba(10,61,30,.4),rgba(10,61,30,.92))"></div>
    </div>`;const L=(F,H)=>e.querySelectorAll(F).forEach((X,ae)=>{setTimeout(()=>{X.style.opacity="1",X.style.transform="translateY(0) scale(1)"},H+ae*90)});L(".duel-card-me",150),L(".duel-card-opp",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((F,H)=>setTimeout(()=>{F.style.opacity="1"},H*70)),900),setTimeout(()=>{const F=e.querySelector("#pvp-vs");F&&(F.style.opacity="1",F.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(H=>H.style.opacity="1")},1250);function E(F,H,X){const ae=document.getElementById(F);if(!ae)return;const pe=performance.now(),ce=ge=>{const he=Math.min(1,(ge-pe)/X);ae.textContent=Math.round(H*(1-Math.pow(1-he,3))),he<1?requestAnimationFrame(ce):ae.textContent=H};requestAnimationFrame(ce)}setTimeout(()=>{E("pvp-score-me",x,800),E("pvp-score-opp",_,800)},1500);const I=m.p1Team.MIL||[],S=m.p2Team.MIL||[],j=g(I)+y(I),O=g(S)+y(S),ee=j>=O?"p1":"p2";let R=m.boostValue;R==null&&(R=ai(),m.boostValue=R,m.boostOwner=ee,m.boostUsed=!1),ye=!0,setTimeout(()=>{const F=e.querySelector("#duel-row-"+(v?"me":"opp")),H=e.querySelector("#duel-row-"+(v?"opp":"me")),X=document.getElementById("pvp-score-me"),ae=document.getElementById("pvp-score-opp"),pe=v?X:ae,ce=v?ae:X;pe&&(pe.style.fontSize="80px",pe.style.color=v?"#FFD700":"#ff6b6b",pe.style.animation="duelPulse .5s ease"+(v?",duelGlow 1.5s ease infinite .5s":"")),ce&&(ce.style.opacity="0.25"),setTimeout(()=>{F&&(F.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",F.style.zIndex="5"),setTimeout(()=>{const ge=document.getElementById("duel-shock");ge&&(ge.style.animation="shockwave .5s ease-out forwards"),H&&(H.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var Se;const ge=document.getElementById("pvp-duel-finale");if(!ge)return;const he=m.boostOwner===l?'<div style="background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:3px solid #cdeffd;border-radius:18px;padding:20px 34px;text-align:center;animation:boostFlipIn .7s cubic-bezier(.34,1.56,.64,1) both;box-shadow:0 10px 36px rgba(135,206,235,.5)"><div style="font-size:10px;color:rgba(0,0,0,.6);letter-spacing:2px;text-transform:uppercase;margin-bottom:6px;font-weight:700">Carte Boost obtenue</div><div style="font-size:46px;line-height:1">⚡</div><div style="font-size:50px;font-weight:900;color:#063;line-height:1.1">+'+R+`</div><div style="font-size:10px;color:rgba(0,0,0,.55);margin-top:4px">Applicable sur n'importe quel joueur</div></div>`:"",Ie=l==="p1"?'<button id="pvp-start-match" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">▶ Commencer le match</button>':`<div style="font-size:14px;color:rgba(255,255,255,0.5);text-align:center;margin-top:8px;animation:fadeUp .4s ease both">⏳ En attente de l'adversaire...</div>`;ge.innerHTML='<div style="font-size:22px;font-weight:900;color:#fff;text-align:center;animation:fadeUp .4s ease both;text-shadow:0 2px 12px rgba(0,0,0,.5)">'+(v?"⚽ "+m[l+"Name"]+"<br>gagne le milieu et attaque !":"😔 "+m[b+"Name"]+"<br>gagne l'engagement et attaque !")+"</div>"+he+Ie,ge.style.transition="opacity .45s ease",ge.style.opacity="1",ge.style.pointerEvents="auto",(Se=document.getElementById("pvp-start-match"))==null||Se.addEventListener("click",async()=>{const Oe=ee;await J({phase:Oe+"-attack",attacker:Oe,round:1,boostValue:R,boostUsed:!1,boostOwner:Oe})})},600)},700)},2800)}function xe(w,z,g,y,x){const _=document.createElement("div");_.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:900;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;overflow:hidden;cursor:pointer";const v=Array.from({length:10},(I,S)=>`<div style="position:absolute;font-size:${16+Math.floor(Math.random()*24)}px;top:${5+Math.floor(Math.random()*65)}%;left:${3+Math.floor(Math.random()*94)}%;animation:fw${S%2===0?"A":"B"} ${.7+Math.random()*.7}s ease ${Math.random()*.9}s both;opacity:0">${["✨","🌟","⭐","💥","🎇","🎆","🔥","🌈"][S%8]}</div>`).join(""),k={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};_.innerHTML=`
      <style>
        @keyframes butPop{0%{transform:scale(0) rotate(-8deg);opacity:0}55%{transform:scale(1.25) rotate(2deg)}85%{transform:scale(0.92) rotate(-1deg)}100%{transform:scale(1);opacity:1}}
        @keyframes ballIn{0%{transform:translate(-70px,18px);opacity:0}65%{opacity:1}100%{transform:translate(26px,-8px);opacity:1}}
        @keyframes scoreIn{from{opacity:0;transform:translateY(-12px)}to{opacity:1;transform:translateY(0)}}
      ...
      ${w!=null&&w.length?`<div style="display:flex;gap:10px;animation:scoreIn .4s ease 1s both;position:relative;z-index:1">
        ${w.map(I=>`<div style="text-align:center">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(_);let L=!1;const E=()=>{L||(L=!0,_.remove(),setTimeout(()=>x(),50))};_.addEventListener("click",E),setTimeout(E,3500)}function Le(w,z,g){var R,F;const y=(m.gcDefs||[]).find(H=>{var X;return H.name===w||((X=H.name)==null?void 0:X.toLowerCase().trim())===(w==null?void 0:w.toLowerCase().trim())}),x={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[y==null?void 0:y.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",_={purple:"#b06ce0",light_blue:"#00d4ef"}[y==null?void 0:y.color]||"#b06ce0",v=(y==null?void 0:y.name)||w,k=(y==null?void 0:y.effect)||((R=Re[w])==null?void 0:R.desc)||"",L=y!=null&&y.image_url?`/icons/${y.image_url}`:null,E=((F=Re[w])==null?void 0:F.icon)||"⚡",S=z===l?"Vous":m[z+"Name"]||"Adversaire",j=document.createElement("div");j.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",j.innerHTML=`
      <style>
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(j);let O=!1;const ee=()=>{O||(O=!0,j.remove(),setTimeout(()=>g&&g(),50))};j.addEventListener("click",ee),setTimeout(ee,3e3)}function Fe(w,z){var j,O,ee,R;const y=(m["gcCardsFull_"+l]||[]).find(F=>F.id===w),x=y==null?void 0:y._gcDef,_={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[x==null?void 0:x.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",v={purple:"#b06ce0",light_blue:"#00d4ef"}[x==null?void 0:x.color]||"#b06ce0",k=(x==null?void 0:x.name)||z,L=(x==null?void 0:x.effect)||((j=Re[z])==null?void 0:j.desc)||"Carte spéciale.",E=x!=null&&x.image_url?`/icons/${x.image_url}`:null,I=((O=Re[z])==null?void 0:O.icon)||"⚡",S=document.createElement("div");S.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",S.innerHTML=`
      ...
      </div>`,document.body.appendChild(S),(ee=S.querySelector("#pvp-gc-back"))==null||ee.addEventListener("click",()=>S.remove()),(R=S.querySelector("#pvp-gc-use"))==null||R.addEventListener("click",()=>{S.remove(),Q(w,z)})}function Ce(){var y;const w=m[l+"Team"],z=Object.entries(w).flatMap(([x,_])=>(_||[]).filter(v=>!v.used).map(v=>({...v,_line:x})));if(!z.length)return;const g=document.createElement("div");g.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",g.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
        <div style="flex:1;font-size:14px;font-weight:700;color:#fff">⚡ Choisir un joueur pour +${m.boostValue}</div>
        <button id="bp-close" style="background:none;border:none;color:rgba(255,255,255,0.5);font-size:22px;cursor:pointer">✕</button>
      </div>
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${z.map(x=>{const _={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[x._line]||"#555",v=ze(x,x._line)+(x.boost||0);return`<div class="bp-item" data-cid="${x.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${_};overflow:hidden;cursor:pointer">
      ...
      </div>`,document.body.appendChild(g),(y=g.querySelector("#bp-close"))==null||y.addEventListener("click",()=>g.remove()),g.querySelectorAll(".bp-item").forEach(x=>{x.addEventListener("click",async()=>{const _=x.dataset.cid,v=z.find(L=>L.cardId===_);if(!v)return;const k=(w[v._line]||[]).find(L=>L.cardId===_);k&&(k.boost=(k.boost||0)+m.boostValue),g.remove(),await J({[l+"Team"]:w,boostUsed:!0})})})}function we(w=null){var O,ee;if(!(m.phase===l+"-attack")){o("Remplacement uniquement avant votre attaque","warning");return}const g=m[l+"Team"],y=m["usedSubIds_"+l]||[],x=m.maxSubs||3;if(y.length>=x){o(`Maximum ${x} remplacements atteint`,"warning");return}const _=Object.entries(g).flatMap(([R,F])=>(F||[]).filter(H=>H.used).map(H=>({...H,_line:R}))),v=(m[l+"Subs"]||[]).filter(R=>!y.includes(R.cardId));if(!_.length){o("Aucun joueur utilisé à remplacer","warning");return}if(!v.length){o("Aucun remplaçant disponible","warning");return}let k=Math.max(0,_.findIndex(R=>R.cardId===w));const L=((O=_[k])==null?void 0:O._line)||((ee=_[k])==null?void 0:ee.job);let E=Math.max(0,v.findIndex(R=>R.job===L)),I=!1;const S=document.createElement("div");S.id="pvp-sub-overlay",S.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function j(){var ce,ge,he,Ie,Se,Oe;const R=_[k],F=v[E],H=Math.min(130,Math.round((window.innerWidth-90)/2)),X=Math.round(H*1.35),ae=Ee=>`background:rgba(255,255,255,0.12);border:none;color:${Ee?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Ee?"default":"pointer"};flex-shrink:0`;S.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        <div id="pin-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
          ...
          <div>${F?We({...F,used:!1,boost:0},H,X):"<div>—</div>"}</div>
      ...
      </div>`,(ce=S.querySelector("#psub-close"))==null||ce.addEventListener("click",()=>S.remove()),(ge=S.querySelector("#pout-up"))==null||ge.addEventListener("click",()=>{k>0&&(k--,j())}),(he=S.querySelector("#pout-down"))==null||he.addEventListener("click",()=>{k<_.length-1&&(k++,j())}),(Ie=S.querySelector("#pin-up"))==null||Ie.addEventListener("click",()=>{E>0&&(E--,j())}),(Se=S.querySelector("#pin-down"))==null||Se.addEventListener("click",()=>{E<v.length-1&&(E++,j())});const pe=(Ee,Ge,Me,Ue)=>{const Ne=S.querySelector("#"+Ee);if(!Ne)return;let Je=0;Ne.addEventListener("touchstart",He=>{Je=He.touches[0].clientY},{passive:!0}),Ne.addEventListener("touchend",He=>{const Be=He.changedTouches[0].clientY-Je;if(Math.abs(Be)<30)return;const Ke=Ge();Be<0&&Ke<Ue-1?(Me(Ke+1),j()):Be>0&&Ke>0&&(Me(Ke-1),j())},{passive:!0})};pe("pin-panel",()=>E,Ee=>E=Ee,v.length),pe("pout-panel",()=>k,Ee=>k=Ee,_.length),(Oe=S.querySelector("#psub-confirm"))==null||Oe.addEventListener("click",async Ee=>{if(Ee.preventDefault(),Ee.stopPropagation(),I)return;I=!0;const Ge=_[k],Me=v[E];if(!Ge||!Me)return;const Ue=Ge._line,Ne=(g[Ue]||[]).findIndex(Be=>Be.cardId===Ge.cardId);if(Ne===-1){o("Erreur : joueur introuvable","error"),S.remove();return}const Je={...Me,_line:Ue,position:Ge.position,used:!1,boost:0};g[Ue].splice(Ne,1,Je);const He=[...y,Me.cardId];S.remove(),V(Ge,Me,async()=>{await J({[l+"Team"]:g,[b+"Team"]:m[b+"Team"],["usedSubIds_"+l]:He})})})}document.body.appendChild(S),j()}function V(w,z,g){const y={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const _=(L,E,I)=>`<div style="text-align:center">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:6px">Appuyer pour continuer</div>`,document.body.appendChild(x);let v=!1;const k=()=>{v||(v=!0,x.remove(),setTimeout(()=>g(),50))};x.addEventListener("click",k),setTimeout(k,2200)}function oe(){var z;const w=document.createElement("div");w.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:20px;overflow-y:auto",w.innerHTML=`
      <div style="font-size:12px;color:rgba(255,255,255,0.5);letter-spacing:2px;text-transform:uppercase">Équipe adverse</div>
      <div style="font-size:18px;font-weight:900;color:#ff6b6b">${m[b+"Name"]}</div>
      <div style="width:min(90vw,420px)">${ct(m[b+"Team"],m[b+"Formation"],null,[],300,300)}</div>
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(w),(z=w.querySelector("#pvp-opp-close"))==null||z.addEventListener("click",()=>w.remove())}function fe(){var g;const w=m.log||[],z=document.createElement("div");z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column",z.innerHTML=`
      ...
      </div>`,document.body.appendChild(z),(g=z.querySelector("#pvp-hist-close"))==null||g.addEventListener("click",()=>z.remove())}async function K(){if(m.phase!==l+"-attack")return;const w=l==="p1"?"p2":"p1",z=(m.round||0)+1,g=[...m.log||[]];g.push({type:"info",text:`⏭️ ${m[l+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const y=be(m),x=$e(w),_=y||!x?"finished":w+"-attack";await J({["selected_"+l]:[],modifiers:{...m.modifiers,[l]:{}},pendingAttack:null,phase:_,attacker:w,round:z,log:g}),_==="finished"&&await qe(m)}async function te(){const w=m[l+"Team"],z=!["GK","DEF","MIL","ATT"].some(_=>(m[b+"Team"][_]||[]).some(v=>!v.used)),g=(m["selected_"+l]||[]).map(_=>{const v=(w[_._role]||[]).find(j=>j.cardId===_.cardId)||_,k=z&&["GK","DEF"].includes(_._role),L=w[_._role]||[],E=L.findIndex(j=>j.cardId===_.cardId),I=st(L.length),S=E>=0?I[E]:v._col??1;return{...v,_line:_._role,_col:S,...k?{note_a:Math.max(1,Number(v.note_a)||0)}:{}}});if(!g.length)return;const y=Ut(g,m.modifiers[l]||{});U(l,g.map(_=>_.cardId)),g.forEach(_=>{const v=(w[_._role]||[]).find(k=>k.cardId===_.cardId);v&&(v.used=!0)}),m["selected_"+l]=[],ne();const x=[...m.log||[]];if(z){const _=(m[l+"Score"]||0)+1,v=g.map(S=>({name:S.name,note:ze(S,S._line||"ATT"),portrait:Pe(S),job:S.job}));x.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:v,homeTotal:y.total,aiTotal:0});const k=(m.round||0)+1,L=l==="p1"?"p2":"p1",E={...m,[l+"Team"]:w,[l+"Score"]:_},I=be(E);q.add(k),xe(v,_,m[b+"Score"]||0,!0,async()=>{await J({[l+"Team"]:w,[l+"Score"]:_,["selected_"+l]:[],modifiers:{...m.modifiers,[l]:{}},pendingAttack:null,phase:I?"finished":L+"-attack",attacker:L,round:k,log:x}),I&&await qe({...m,[l+"Score"]:_})});return}x.push({type:"pending",text:`⚔️ ${m[l+"Name"]} attaque (${y.total})`}),await J({[l+"Team"]:w,[b+"Team"]:m[b+"Team"],pendingAttack:{...y,players:g,side:l},["selected_"+l]:[],modifiers:{...m.modifiers,[l]:{}},phase:b+"-defense",log:x})}async function Y(){const w=m[l+"Team"],z=(m["selected_"+l]||[]).map(R=>{const F=(w[R._role]||[]).find(ce=>ce.cardId===R.cardId)||R,H=w[R._role]||[],X=H.findIndex(ce=>ce.cardId===R.cardId),ae=st(H.length),pe=X>=0?ae[X]:F._col??1;return{...F,_line:R._role,_col:pe}}),g=Kt(z,m.modifiers[l]||{});U(l,z.map(R=>R.cardId)),z.forEach(R=>{const F=(w[R._role]||[]).find(H=>H.cardId===R.cardId);F&&(F.used=!0)}),m["selected_"+l]=[],ne();const y=Vt(m.pendingAttack.total,g.total,m.modifiers[l]||{}),x=m.pendingAttack.side,_=y==="attack"||(y==null?void 0:y.goal),v=x==="p1"?"p2":"p1",k=(m.round||0)+1,L=(m.pendingAttack.players||[]).map(R=>({name:R.name,note:ze(R,R._line||"ATT"),portrait:Pe(R),job:R.job})),E=[...m.log||[]];E.push({type:"duel",isGoal:_,homeScored:_&&x===l,text:_?`⚽ BUT de ${m[x+"Name"]} ! (${m.pendingAttack.total} vs ${g.total})`:`✋ Attaque stoppée (${m.pendingAttack.total} vs ${g.total})`,homePlayers:L,aiPlayers:z.map(R=>({name:R.name,note:ze(R,R._line||"DEF"),portrait:Pe(R),job:R.job})),homeTotal:m.pendingAttack.total,aiTotal:g.total});const I=_?(m[x+"Score"]||0)+1:m[x+"Score"]||0,S={...m,[l+"Team"]:w,[x+"Score"]:I},j=be(S),O=j?"finished":v+"-attack",ee=async()=>{await J({[l+"Team"]:w,[b+"Team"]:m[b+"Team"],[x+"Score"]:I,["selected_"+l]:[],modifiers:{...m.modifiers,[l]:{}},pendingAttack:null,phase:O,attacker:v,round:k,log:E}),(O==="finished"||j)&&await qe({...m,[x+"Score"]:I})};if(_){const R=x===l,F=R?I:m[l+"Score"]||0,H=R?m[b+"Score"]||0:I;q.add(k),xe(L,F,H,R,ee)}else await ee()}function de(w){return["MIL","ATT"].some(z=>(w[z]||[]).some(g=>!g.used))}function ve(w){return["GK","DEF","MIL","ATT"].some(z=>(w[z]||[]).some(g=>!g.used))}function Te(w){return ve(w)&&!de(w)}function $e(w){const z=m[w+"Team"],g=m[(w==="p1"?"p2":"p1")+"Team"];return!!(de(z)||!ve(g)&&Te(z))}function be(w){const z=["MIL","ATT"].some(E=>(w.p1Team[E]||[]).some(I=>!I.used)),g=["MIL","ATT"].some(E=>(w.p2Team[E]||[]).some(I=>!I.used)),y=ve(w.p1Team),x=ve(w.p2Team);return!z&&!(!x&&y)&&(!g&&!(!y&&x))}function ke(w){const z=w.p1Score||0,g=w.p2Score||0;return z===g?null:z>g?p.home_id:p.away_id}async function qe(w){try{const z=ke(w),g=z?p.home_id===z?p.away_id:p.home_id:null;await A.from("matches").update({status:"finished",winner_id:z,home_score:w.p1Score||0,away_score:w.p2Score||0}).eq("id",i),z&&g&&updateEvolutiveCards(z,g).catch(()=>{})}catch(z){console.error("[PvP] finishMatch:",z)}}function je(){var x;const w=m[l+"Score"],z=m[b+"Score"],g=w>z,y=w===z;et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      ...
      </div>`,(U=document.getElementById("lobby-cancel"))==null||U.addEventListener("click",async()=>{$=!0,p&&(A.removeChannel(p),p=null),re&&await A.from("friend_match_invites").update({status:"declined"}).eq("id",re),Ve(e),l("home")})},m=async(Z,se)=>{$=!0,p&&(A.removeChannel(p),p=null),await new Promise(re=>setTimeout(re,600)),e.isConnected&&Xn(e,t,Z,se,r,c)},{data:T}=await A.from("friend_match_invites").select("*").eq("status","pending").or(`and(inviter_id.eq.${h},invitee_id.eq.${f}),and(inviter_id.eq.${f},invitee_id.eq.${h})`).order("created_at",{ascending:!1}).limit(1).maybeSingle();let M,G;if(T&&T.inviter_id===f)G=!1,M=T.id,await A.from("friend_match_invites").update({invitee_deck_id:i,invitee_ready:!0,status:"accepted"}).eq("id",M);else{G=!0;const{data:Z,error:se}=await A.from("friend_match_invites").insert({inviter_id:h,invitee_id:f,friend_id:f,inviter_deck_id:i,inviter_ready:!0,status:"pending"}).select().single();if(se||!Z){b("Erreur création invitation","error"),Ve(e),l("home");return}M=Z.id}if(u(!0,!G,M),!G){const Z={home_id:f,away_id:h,home_deck_id:T.inviter_deck_id,away_deck_id:i,status:"active",mode:"friend"},se=await A.from("matches").insert(Z).select().single(),re=se.data;if(!re){b("Erreur création match: "+(((J=se.error)==null?void 0:J.message)||""),"error"),Ve(e),l("home");return}await A.from("friend_match_invites").update({match_id:re.id,status:"matched"}).eq("id",M),m(re.id,!1);return}let q=!1;const B=Z=>{$||q||(Z.status==="matched"&&Z.match_id?(q=!0,clearInterval(C),clearInterval(N),m(Z.match_id,!0)):Z.status==="declined"?(clearInterval(C),clearInterval(N),b(`${a} a décliné l'invitation`,"warning"),Ve(e),l("home")):Z.invitee_ready&&u(!0,!0,M))},C=setInterval(()=>{if($){clearInterval(C);return}A.from("matchmaking_queue").delete().eq("user_id",h).then(()=>{},()=>{})},3e3),N=setInterval(async()=>{if($||q){clearInterval(N);return}const{data:Z}=await A.from("friend_match_invites").select("*").eq("id",M).maybeSingle();Z&&B(Z)},1200);p=A.channel("friend-invite-"+M).on("postgres_changes",{event:"UPDATE",schema:"public",table:"friend_match_invites",filter:`id=eq.${M}`},Z=>B(Z.new)).subscribe()}async function Xn(e,t,i,n,d=[],s=[]){const{state:r,navigate:c,toast:f}=t,a=n?"p1":"p2",o=n?"p2":"p1",l=(d||[]).map(w=>w.id),b=(d||[]).map(w=>({id:w.id,gc_type:w.gc_type,_gcDef:w._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:h}=await A.from("matches").select("*").eq("id",i).single();if(!h){f("Match introuvable","error"),c("home");return}async function $(){const[{data:w},{data:z},{data:g},{data:y}]=await Promise.all([A.rpc("get_deck_for_match",{p_deck_id:h.home_deck_id}),A.rpc("get_deck_for_match",{p_deck_id:h.away_deck_id}),A.from("users").select("id,pseudo,club_name").eq("id",h.home_id).single(),A.from("users").select("id,pseudo,club_name").eq("id",h.away_id).single()]),x=E=>{const I=Number(E.evolution_bonus)||0;return{cardId:E.card_id,position:E.position,id:E.id,firstname:E.firstname,name:E.surname_encoded,country_code:E.country_code,club_id:E.club_id,job:E.job,job2:E.job2,note_g:(Number(E.note_g)||0)+(E.job==="GK"||E.job2==="GK"&&Number(E.note_g)>0?I:0),note_d:(Number(E.note_d)||0)+(E.job==="DEF"||E.job2==="DEF"&&Number(E.note_d)>0?I:0),note_m:(Number(E.note_m)||0)+(E.job==="MIL"||E.job2==="MIL"&&Number(E.note_m)>0?I:0),note_a:(Number(E.note_a)||0)+(E.job==="ATT"||E.job2==="ATT"&&Number(E.note_a)>0?I:0),evolution_bonus:I,rarity:E.rarity,skin:E.skin,hair:E.hair,hair_length:E.hair_length,clubName:E.club_encoded_name||null,clubLogo:E.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},_=((w==null?void 0:w.starters)||[]).map(E=>x(E)),v=((z==null?void 0:z.starters)||[]).map(E=>x(E)),k=(w==null?void 0:w.formation)||"4-4-2",L=(z==null?void 0:z.formation)||"4-4-2";return{p1Team:(()=>{const E=bt(_,k);return stadiumDef?Ct(E,stadiumDef):E})(),p2Team:bt(v,L),p1Subs:(()=>{const E=((w==null?void 0:w.subs)||[]).map(I=>x(I));return stadiumDef&&Ot(E,stadiumDef),E})(),p2Subs:((z==null?void 0:z.subs)||[]).map(E=>x(E)),p1Formation:k,p2Formation:L,p1Name:(g==null?void 0:g.club_name)||(g==null?void 0:g.pseudo)||"Joueur 1",p2Name:(y==null?void 0:y.club_name)||(y==null?void 0:y.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:n?l:[],gc_p2:n?[]:l,gcCardsFull_p1:n?b:[],gcCardsFull_p2:n?[]:b,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:s||[],lastActionAt:new Date().toISOString()}}let p=h.game_state&&Object.keys(h.game_state).length?h.game_state:null;if(!p)if(n){p=await $();const{data:w}=await A.from("matches").select("game_state").eq("id",i).single();!(w!=null&&w.game_state)||!Object.keys(w.game_state).length?await A.from("matches").update({game_state:p,turn_user_id:h.home_id}).eq("id",i):p=w.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let w=0;w<60&&!p;w++){await new Promise(g=>setTimeout(g,400));const{data:z}=await A.from("matches").select("game_state").eq("id",i).single();z!=null&&z.game_state&&Object.keys(z.game_state).length&&(p=z.game_state)}if(!p){f("Erreur de synchronisation","error"),c("home");return}p.gc_p2=l,p.gcCardsFull_p2=b,await A.from("matches").update({game_state:p}).eq("id",i)}let u=!1,m=!1,T=!1,M=null,G=!1;const q=new Set,B=new Set;function C(w){var E,I;try{A.removeChannel(N)}catch{}const z=p[a+"Score"]||0,g=p[o+"Score"]||0;if(!T){T=!0;const S=w.winner_id||(z>g?r.profile.id:g>z?"opp":null),j=S===r.profile.id?"win":S?"loss":null;j&&ni(()=>Promise.resolve().then(()=>so),void 0).then(O=>O.applyOwnEvolution(r.profile.id,j)).catch(()=>{})}if(!m){const S=p.p1Score||0,j=p.p2Score||0,O=(p.usedGc_p1||[]).length,ee=(p.usedGc_p2||[]).length,R=w.winner_id||(S>j?h.home_id:j>S?h.away_id:null);(R?r.profile.id===R:r.profile.id<(n?h.away_id:h.home_id))&&(m=!0,to({player1Id:h.home_id,player2Id:h.away_id,score1:S,score2:j,gc1:O,gc2:ee}).catch(H=>console.warn("[FriendMatch] updateStats:",H)))}let y,x;w.winner_id?(y=w.winner_id===r.profile.id,x=!1):w.forfeit?(y=z>g,x=!1):(x=z===g,y=z>g),(E=document.getElementById("pvp-end-overlay"))==null||E.remove();const _=document.createElement("div");_.id="pvp-end-overlay",_.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const v=x?"🤝":y?"🏆":"😞",k=x?"MATCH NUL":y?"VICTOIRE !":"DÉFAITE",L=x?"#fff":y?"#FFD700":"#ff6b6b";_.innerHTML=`
      <div style="font-size:64px">${v}</div>
      <div style="font-size:26px;font-weight:900;color:${L}">${k}</div>
      <div style="font-size:18px">${p[a+"Name"]} ${z} – ${g} ${p[o+"Name"]}</div>
      ${w.forfeit?`<div style="font-size:13px;color:rgba(255,255,255,0.5)">${y?"L'adversaire a quitté":"Perdu par forfait"}</div>`:""}
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(_),(I=_.querySelector("#pvp-end-home"))==null||I.addEventListener("click",()=>{_.remove(),Ve(e),c("home")})}const N=A.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},w=>{const z=w.new;try{if(z.status==="finished"||z.forfeit){if(u)return;u=!0,M&&(clearInterval(M),M=null),z.game_state&&(p=z.game_state),C(z);return}if(z.game_state){const g=p;p=z.game_state;const y=p._lastGC;if(y&&y.seq&&!B.has(y.seq)&&(B.add(y.seq),y.by!==a)){Le(y.type,y.by,()=>ne());return}const x=g[a+"Score"]||0,_=g[o+"Score"]||0,v=p[a+"Score"]||0,k=p[o+"Score"]||0,L=v>x,E=k>_;if((L||E)&&!q.has(p.round)){q.add(p.round);const I=[...p.log||[]].reverse().find(j=>j.isGoal),S=((I==null?void 0:I.homePlayers)||[]).map(j=>({name:j.name,note:j.note,portrait:j.portrait,job:j.job}));xe(S,v,k,L,()=>ne());return}ne()}}catch(g){console.error("[PvP] crash:",g)}}).subscribe();async function J(w){Object.assign(p,w),p.lastActionAt=new Date().toISOString();const{error:z}=await A.from("matches").update({game_state:p}).eq("id",i);z&&f("Erreur de synchronisation","error");try{ne()}catch(g){console.error("[PvP] renderPvpScreen crash:",g)}}async function Z(){if(u)return;u=!0,M&&(clearInterval(M),M=null);const w=n?h.away_id:h.home_id,z=n?"p2":"p1",g=n?"p1":"p2",y={...p,[z+"Score"]:3,[g+"Score"]:0,phase:"finished"};try{await A.from("matches").update({status:"finished",forfeit:!0,winner_id:w,home_score:y.p1Score||0,away_score:y.p2Score||0,game_state:y}).eq("id",i)}catch{}try{A.removeChannel(N)}catch{}setTimeout(()=>{Ve(e),c("home")},800)}const se={BOOST_STAT:({value:w=1,count:z=1,roles:g=[]},y,x)=>{const _=y[a+"Team"],v=Object.entries(_).filter(([k])=>!g.length||g.includes(k)).flatMap(([k,L])=>L.filter(E=>!E.used).map(E=>({...E,_line:k})));if(!v.length){y.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),x(y);return}re(v,z,`Choisir ${z} joueur(s) à booster (+${w})`,k=>{k.forEach(L=>{const E=(_[L._line]||[]).find(I=>I.cardId===L.cardId);E&&(E.boost=(E.boost||0)+w,y.log.push({text:`⚡ +${w} sur ${E.name}`,type:"info"}))}),y[a+"Team"]=_,x(y)})},DEBUFF_STAT:({value:w=1,count:z=1,roles:g=[],target:y="ai"},x,_)=>{const v=y==="home"?a:o,k=x[v+"Team"],L=y==="home"?"allié":"adverse",E=Object.entries(k).filter(([I])=>!g.length||g.includes(I)).flatMap(([I,S])=>S.map(j=>({...j,_line:I})));if(!E.length){x.log.push({text:`🎯 Aucun joueur ${L}`,type:"info"}),_(x);return}re(E,z,`Choisir ${z} joueur(s) ${L}(s) (-${w})`,I=>{I.forEach(S=>{const j=(k[S._line]||[]).find(O=>O.cardId===S.cardId);j&&(j.boost=(j.boost||0)-w,x.log.push({text:`🎯 -${w} sur ${j.name}`,type:"info"}))}),x[v+"Team"]=k,_(x)})},GRAY_PLAYER:({count:w=1,roles:z=[],target:g="ai"},y,x)=>{const _=g==="home"?a:o,v=y[_+"Team"],k=g==="home"?"allié":"adverse",L=Object.entries(v).filter(([E])=>!z.length||z.includes(E)).flatMap(([E,I])=>I.filter(S=>!S.used).map(S=>({...S,_line:E})));if(!L.length){y.log.push({text:`❌ Aucun joueur ${k}`,type:"info"}),x(y);return}re(L,w,`Choisir ${w} joueur(s) ${k}(s) à exclure`,E=>{const I="usedCardIds_"+_,S=new Set(y[I]||[]);E.forEach(j=>{const O=(v[j._line]||[]).find(ee=>ee.cardId===j.cardId);O&&(O.used=!0,S.add(j.cardId),y.log.push({text:`❌ ${O.name} exclu !`,type:"info"}))}),y[I]=[...S],y[_+"Team"]=v,x(y)})},REVIVE_PLAYER:({count:w=1,roles:z=[]},g,y)=>{const x=g[a+"Team"],_=Object.entries(x).filter(([v])=>!z.length||z.includes(v)).flatMap(([v,k])=>k.filter(L=>L.used).map(L=>({...L,_line:v})));if(!_.length){g.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),y(g);return}re(_,w,`Choisir ${w} joueur(s) à ressusciter`,v=>{v.forEach(k=>{const L=(x[k._line]||[]).find(E=>E.cardId===k.cardId);L&&(L.used=!1,g.log.push({text:`💫 ${L.name} ressuscité !`,type:"info"}))}),g[a+"Team"]=x,y(g)})},REMOVE_GOAL:({},w,z)=>{const g=o+"Score";w[g]>0?(w[g]--,w.log.push({text:"🚫 Dernier but annulé !",type:"info"})):w.log.push({text:"🚫 Aucun but à annuler",type:"info"}),z(w)},ADD_GOAL_DRAW:({},w,z)=>{w[a+"Score"]===w[o+"Score"]?(w[a+"Score"]++,w.log.push({text:"🎯 But bonus !",type:"info"})):w.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),z(w)},ADD_SUB:({value:w=1},z,g)=>{z.maxSubs=(z.maxSubs||3)+w,z.log.push({text:`🔄 +${w} remplacement(s)`,type:"info"}),g(z)},CUSTOM:({},w,z)=>z(w)};function re(w,z,g,y){const x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let _=[];function v(){var L,E;const k=w.map(I=>{const S={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[I._line]||"#555",j=ze(I,I._line)+(I.boost||0),ee=_.find(F=>F.cardId===I.cardId)?"#FFD700":"rgba(255,255,255,0.25)",R=I.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${I.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${ee};background:${S};overflow:hidden;cursor:pointer;${R}">
          <div style="background:rgba(255,255,255,0.9);text-align:center;padding:2px;font-size:7px;font-weight:900;color:#111;overflow:hidden;white-space:nowrap;text-overflow:ellipsis">${I.name||"?"}</div>
          <div style="height:50px;display:flex;align-items:center;justify-content:center;font-size:22px;font-weight:900;color:#fff">${j}</div>
          <div style="background:rgba(255,255,255,0.9);text-align:center;padding:2px;font-size:8px;font-weight:700;color:#333">${I._line}</div>
        </div>`}).join("");x.innerHTML=`
        ...
        </div>`,(L=x.querySelector("#pp-close"))==null||L.addEventListener("click",()=>x.remove()),x.querySelectorAll(".pp-item").forEach(I=>{I.addEventListener("click",()=>{const S=I.dataset.cid,j=w.find(ee=>ee.cardId===S),O=_.findIndex(ee=>ee.cardId===S);j&&(O>-1?_.splice(O,1):_.length<z&&_.push(j),v())})}),(E=x.querySelector("#pp-confirm"))==null||E.addEventListener("click",()=>{x.remove(),y(_)})}v(),document.body.appendChild(x)}async function Q(w,z){const y=(p["gcCardsFull_"+a]||[]).find(k=>k.id===w),x=(y==null?void 0:y._gcDef)||(p.gcDefs||[]).find(k=>{var L;return k.name===z||((L=k.name)==null?void 0:L.toLowerCase().trim())===(z==null?void 0:z.toLowerCase().trim())}),_=[...p["usedGc_"+a]||[],w],v={type:z,by:a,seq:(p._gcAnimSeq||0)+1};B.add(v.seq),Le(z,a,async()=>{if(x!=null&&x.effect_type&&x.effect_type!=="CUSTOM"){const L=se[x.effect_type];if(L){const E={...p};L(x.effect_params||{},E,async I=>{I["usedGc_"+a]=_,I._lastGC=v,I._gcAnimSeq=v.seq,await J(I)});return}}const k={...p};switch(z){case"Remplacement+":k.maxSubs=(k.maxSubs||3)+1,k.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const L=o+"Score";k[L]>0&&(k[L]--,k.log.push({text:"🚫 But annulé",type:"info"}));break}}k["usedGc_"+a]=_,k._lastGC=v,k._gcAnimSeq=v.seq,await J(k)})}function U(w,z){const g="usedCardIds_"+w,y=new Set(p[g]||[]);z.forEach(x=>y.add(x)),p[g]=[...y]}function D(){for(const w of["p1","p2"]){const z=new Set(p["usedCardIds_"+w]||[]),g=p[w+"Team"];if(g)for(const y of["GK","DEF","MIL","ATT"])(g[y]||[]).forEach(x=>{x.used=z.has(x.cardId)})}}function ne(){var Ke,Ze,it,ut,ft,gt;if(p.phase==="reveal")return ue();if(p.phase==="midfield")return me();if(p.phase==="finished")return je();const w=p[a+"Team"],z=p[o+"Team"];D();const g=p[a+"Score"],y=p[o+"Score"],x=p[a+"Name"],_=p[o+"Name"],v=p.phase===a+"-attack",k=p.phase===a+"-defense",L=Array.isArray(p["selected_"+a])?p["selected_"+a]:[],E=L.map(P=>P.cardId),I=window.innerWidth>=700,S=p[a+"Subs"]||[],j=p["usedSubIds_"+a]||[],O=S.filter(P=>!j.includes(P.cardId)),ee=p["gcCardsFull_"+a]||[],R=p["usedGc_"+a]||[],F=ee.filter(P=>!R.includes(P.id)),H=p.boostOwner===a&&!p.boostUsed,X=!["GK","DEF","MIL","ATT"].some(P=>(z[P]||[]).some(le=>!le.used)),ae=[...w.MIL||[],...w.ATT||[]].filter(P=>!P.used),pe=v&&X&&ae.length===0?[...w.GK||[],...w.DEF||[]].filter(P=>!P.used).map(P=>P.cardId):[];function ce(P,le,W){var lt,wt;const ie=P._gcDef,Ae={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ie==null?void 0:ie.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",_e={purple:"#b06ce0",light_blue:"#00d4ef"}[ie==null?void 0:ie.color]||"#b06ce0",Xe=(ie==null?void 0:ie.name)||P.gc_type,Qe=(ie==null?void 0:ie.effect)||((lt=Re[P.gc_type])==null?void 0:lt.desc)||"",ot=ie!=null&&ie.image_url?`/icons/${ie.image_url}`:null,dt=((wt=Re[P.gc_type])==null?void 0:wt.icon)||"⚡",mt=Math.round(W*.22),vt=Math.round(W*.22),rt=W-mt-vt,Tt=Xe.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${P.id}" data-gc-type="${P.gc_type}"
        style="box-sizing:border-box;width:${le}px;height:${W}px;border-radius:10px;border:2px solid ${_e};background:${Ae};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
        <div style="height:${mt}px;background:rgba(255,255,255,0.14);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 3px">
          <span style="font-size:${Tt}px;font-weight:900;color:#fff;text-align:center;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${le-6}px">${Xe}</span>
          <span style="font-size:6px;color:rgba(255,255,255,0.45)">⚡ GC</span>
      ...
      </div>`;function Be(){const P=e.querySelector(".match-screen");if(!P)return;const le=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);P.style.height=le+"px",P.style.minHeight=le+"px",P.style.maxHeight=le+"px",P.style.overflow="hidden";const W=e.querySelector("#mobile-action-bar"),ie=e.querySelector("#mobile-play-area");W&&ie&&(ie.style.paddingBottom=W.offsetHeight+"px")}if(Be(),setTimeout(Be,120),setTimeout(Be,400),G||(G=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",Be),window.visualViewport.addEventListener("scroll",Be)),window.addEventListener("resize",Be)),function(){const le=e.querySelector(".terrain-wrapper svg");le&&(le.removeAttribute("width"),le.removeAttribute("height"),le.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",le.setAttribute("viewBox","-26 -26 352 352"),le.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),e.querySelectorAll(".match-slot-hit").forEach(P=>{P.addEventListener("click",()=>{if(!v&&!k)return;const le=P.dataset.cardId,W=P.dataset.role,ie=(w[W]||[]).find(Qe=>Qe.cardId===le);if(!ie||ie.used)return;const Ae=pe.includes(le);if(v&&!["MIL","ATT"].includes(W)&&!Ae)return;Array.isArray(p["selected_"+a])||(p["selected_"+a]=[]);const _e=p["selected_"+a],Xe=_e.findIndex(Qe=>Qe.cardId===le);Xe>-1?_e.splice(Xe,1):_e.length<3&&_e.push({...ie,_role:W}),ne()})}),e.querySelectorAll(".match-used-hit").forEach(P=>{P.addEventListener("click",()=>we(P.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(P=>{P.addEventListener("click",()=>we())}),(Ke=e.querySelector("#pvp-sub-open"))==null||Ke.addEventListener("click",()=>we()),e.querySelectorAll(".pvp-gc-mini").forEach(P=>{P.addEventListener("click",()=>Fe(P.dataset.gcId,P.dataset.gcType))}),(Ze=e.querySelector("#pvp-boost-card"))==null||Ze.addEventListener("click",()=>Ce()),(it=e.querySelector("#pvp-action"))==null||it.addEventListener("click",P=>{v?P.currentTarget.dataset.pass==="1"||!$e(a)?K():te():k&&Y()}),(ut=e.querySelector("#pvp-quit"))==null||ut.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&Z()}),(ft=e.querySelector("#pvp-view-opp"))==null||ft.addEventListener("click",()=>oe()),(gt=e.querySelector("#pvp-toggle-history"))==null||gt.addEventListener("click",()=>fe()),M&&(clearInterval(M),M=null),(v||k)&&!u){let P=30,le=!1;const W=()=>document.getElementById("pvp-timer"),ie=()=>{W()&&(W().textContent=P+"s",W().style.color=le?"#ff4444":"#fff")};ie(),M=setInterval(()=>{P--,P<0?le?(clearInterval(M),M=null,v&&!$e(a)?K():Z()):(le=!0,P=15,ie()):ie()},1e3)}}function ue(){et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
    ...
    </div>`,a==="p1"&&setTimeout(async()=>{await J({phase:"midfield"})},5e3)}let ye=!1;function me(){if(ye)return;const w=p[a+"Team"].MIL||[],z=p[o+"Team"].MIL||[];function g(F){return F.reduce((H,X)=>H+ze(X,"MIL"),0)}function y(F){let H=0;for(let X=0;X<F.length-1;X++){const ae=nt(F[X],F[X+1]);ae==="#00ff88"?H+=2:ae==="#FFD700"&&(H+=1)}return H}const x=g(w)+y(w),_=g(z)+y(z),v=x>=_;function k(F,H,X){return`<div id="duel-row-${X}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
        <div style="font-size:11px;color:rgba(255,255,255,0.55);letter-spacing:2px;margin-bottom:10px;text-transform:uppercase;font-weight:700">${H}</div>
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${F.map((ae,pe)=>{const ce=pe<F.length-1?nt(ae,F[pe+1]):null,ge=!ce||ce==="#ff3333"||ce==="#cc2222",he=ce==="#00ff88"?"+2":ce==="#FFD700"?"+1":"";return`<div class="duel-card duel-card-${X}" data-idx="${pe}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease,transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
              ${We({...ae,note:ze(ae,"MIL"),_line:"MIL"},58,78)}
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      ${k(z,p[o+"Name"]||"Adversaire","opp")}
      <div id="duel-shock" style="position:absolute;left:50%;top:50%;width:120px;height:120px;border-radius:50%;border:6px solid #FFD700;opacity:0;pointer-events:none"></div>
      <div id="pvp-duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center,rgba(10,61,30,.4),rgba(10,61,30,.92))"></div>
    </div>`;const L=(F,H)=>e.querySelectorAll(F).forEach((X,ae)=>{setTimeout(()=>{X.style.opacity="1",X.style.transform="translateY(0) scale(1)"},H+ae*90)});L(".duel-card-me",150),L(".duel-card-opp",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((F,H)=>setTimeout(()=>{F.style.opacity="1"},H*70)),900),setTimeout(()=>{const F=e.querySelector("#pvp-vs");F&&(F.style.opacity="1",F.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(H=>H.style.opacity="1")},1250);function E(F,H,X){const ae=document.getElementById(F);if(!ae)return;const pe=performance.now(),ce=ge=>{const he=Math.min(1,(ge-pe)/X);ae.textContent=Math.round(H*(1-Math.pow(1-he,3))),he<1?requestAnimationFrame(ce):ae.textContent=H};requestAnimationFrame(ce)}setTimeout(()=>{E("pvp-score-me",x,800),E("pvp-score-opp",_,800)},1500);const I=p.p1Team.MIL||[],S=p.p2Team.MIL||[],j=g(I)+y(I),O=g(S)+y(S),ee=j>=O?"p1":"p2";let R=p.boostValue;R==null&&(R=ai(),p.boostValue=R,p.boostOwner=ee,p.boostUsed=!1),ye=!0,setTimeout(()=>{const F=e.querySelector("#duel-row-"+(v?"me":"opp")),H=e.querySelector("#duel-row-"+(v?"opp":"me")),X=document.getElementById("pvp-score-me"),ae=document.getElementById("pvp-score-opp"),pe=v?X:ae,ce=v?ae:X;pe&&(pe.style.fontSize="80px",pe.style.color=v?"#FFD700":"#ff6b6b",pe.style.animation="duelPulse .5s ease"+(v?",duelGlow 1.5s ease infinite .5s":"")),ce&&(ce.style.opacity="0.25"),setTimeout(()=>{F&&(F.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",F.style.zIndex="5"),setTimeout(()=>{const ge=document.getElementById("duel-shock");ge&&(ge.style.animation="shockwave .5s ease-out forwards"),H&&(H.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var Se;const ge=document.getElementById("pvp-duel-finale");if(!ge)return;const he=p.boostOwner===a?'<div style="background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:3px solid #cdeffd;border-radius:18px;padding:20px 34px;text-align:center;animation:boostFlipIn .7s cubic-bezier(.34,1.56,.64,1) both;box-shadow:0 10px 36px rgba(135,206,235,.5)"><div style="font-size:10px;color:rgba(0,0,0,.6);letter-spacing:2px;text-transform:uppercase;margin-bottom:6px;font-weight:700">Carte Boost obtenue</div><div style="font-size:46px;line-height:1">⚡</div><div style="font-size:50px;font-weight:900;color:#063;line-height:1.1">+'+R+`</div><div style="font-size:10px;color:rgba(0,0,0,.55);margin-top:4px">Applicable sur n'importe quel joueur</div></div>`:"",Ie=a==="p1"?'<button id="pvp-start-match" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">▶ Commencer le match</button>':`<div style="font-size:14px;color:rgba(255,255,255,0.5);text-align:center;margin-top:8px;animation:fadeUp .4s ease both">⏳ En attente de l'adversaire...</div>`;ge.innerHTML='<div style="font-size:22px;font-weight:900;color:#fff;text-align:center;animation:fadeUp .4s ease both;text-shadow:0 2px 12px rgba(0,0,0,.5)">'+(v?"⚽ "+p[a+"Name"]+"<br>gagne le milieu et attaque !":"😔 "+p[o+"Name"]+"<br>gagne l'engagement et attaque !")+"</div>"+he+Ie,ge.style.transition="opacity .45s ease",ge.style.opacity="1",ge.style.pointerEvents="auto",(Se=document.getElementById("pvp-start-match"))==null||Se.addEventListener("click",async()=>{const Oe=ee;await J({phase:Oe+"-attack",attacker:Oe,round:1,boostValue:R,boostUsed:!1,boostOwner:Oe})})},600)},700)},2800)}function xe(w,z,g,y,x){const _=document.createElement("div");_.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:900;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;overflow:hidden;cursor:pointer";const v=Array.from({length:10},(I,S)=>`<div style="position:absolute;font-size:${16+Math.floor(Math.random()*24)}px;top:${5+Math.floor(Math.random()*65)}%;left:${3+Math.floor(Math.random()*94)}%;animation:fw${S%2===0?"A":"B"} ${.7+Math.random()*.7}s ease ${Math.random()*.9}s both;opacity:0">${["✨","🌟","⭐","💥","🎇","🎆","🔥","🌈"][S%8]}</div>`).join(""),k={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};_.innerHTML=`
      <style>
        @keyframes butPop{0%{transform:scale(0) rotate(-8deg);opacity:0}55%{transform:scale(1.25) rotate(2deg)}85%{transform:scale(0.92) rotate(-1deg)}100%{transform:scale(1);opacity:1}}
        @keyframes ballIn{0%{transform:translate(-70px,18px);opacity:0}65%{opacity:1}100%{transform:translate(26px,-8px);opacity:1}}
        @keyframes scoreIn{from{opacity:0;transform:translateY(-12px)}to{opacity:1;transform:translateY(0)}}
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(_);let L=!1;const E=()=>{L||(L=!0,_.remove(),setTimeout(()=>x(),50))};_.addEventListener("click",E),setTimeout(E,3500)}function Le(w,z,g){var R,F;const y=(p.gcDefs||[]).find(H=>{var X;return H.name===w||((X=H.name)==null?void 0:X.toLowerCase().trim())===(w==null?void 0:w.toLowerCase().trim())}),x={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[y==null?void 0:y.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",_={purple:"#b06ce0",light_blue:"#00d4ef"}[y==null?void 0:y.color]||"#b06ce0",v=(y==null?void 0:y.name)||w,k=(y==null?void 0:y.effect)||((R=Re[w])==null?void 0:R.desc)||"",L=y!=null&&y.image_url?`/icons/${y.image_url}`:null,E=((F=Re[w])==null?void 0:F.icon)||"⚡",S=z===a?"Vous":p[z+"Name"]||"Adversaire",j=document.createElement("div");j.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",j.innerHTML=`
      <style>
        @keyframes gcFlipIn{0%{transform:perspective(800px) rotateY(90deg) scale(.7);opacity:0}55%{transform:perspective(800px) rotateY(-12deg) scale(1.08);opacity:1}100%{transform:perspective(800px) rotateY(0) scale(1);opacity:1}}
        @keyframes gcGlow{0%,100%{box-shadow:0 0 30px ${_}66}50%{box-shadow:0 0 60px ${_}cc}}
        @keyframes gcLabel{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:translateY(0)}}
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(j);let O=!1;const ee=()=>{O||(O=!0,j.remove(),setTimeout(()=>g&&g(),50))};j.addEventListener("click",ee),setTimeout(ee,3e3)}function Fe(w,z){var j,O,ee,R;const y=(p["gcCardsFull_"+a]||[]).find(F=>F.id===w),x=y==null?void 0:y._gcDef,_={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[x==null?void 0:x.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",v={purple:"#b06ce0",light_blue:"#00d4ef"}[x==null?void 0:x.color]||"#b06ce0",k=(x==null?void 0:x.name)||z,L=(x==null?void 0:x.effect)||((j=Re[z])==null?void 0:j.desc)||"Carte spéciale.",E=x!=null&&x.image_url?`/icons/${x.image_url}`:null,I=((O=Re[z])==null?void 0:O.icon)||"⚡",S=document.createElement("div");S.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",S.innerHTML=`
      <div style="width:190px;border-radius:16px;border:3px solid ${v};background:${_};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 40px ${v}66">
        <div style="padding:10px;background:rgba(255,255,255,0.14);text-align:center">
          <div style="font-size:${k.length>14?11:14}px;font-weight:900;color:#fff;letter-spacing:.5px;text-transform:uppercase">${k}</div>
          <div style="font-size:8px;color:rgba(255,255,255,0.5);margin-top:2px">⚡ GAME CHANGER</div>
      ...
      <div style="display:flex;gap:12px;width:190px">
        <button id="pvp-gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
        <button id="pvp-gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
      </div>`,document.body.appendChild(S),(ee=S.querySelector("#pvp-gc-back"))==null||ee.addEventListener("click",()=>S.remove()),(R=S.querySelector("#pvp-gc-use"))==null||R.addEventListener("click",()=>{S.remove(),Q(w,z)})}function Ce(){var y;const w=p[a+"Team"],z=Object.entries(w).flatMap(([x,_])=>(_||[]).filter(v=>!v.used).map(v=>({...v,_line:x})));if(!z.length)return;const g=document.createElement("div");g.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",g.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
      ...
      </div>`,document.body.appendChild(g),(y=g.querySelector("#bp-close"))==null||y.addEventListener("click",()=>g.remove()),g.querySelectorAll(".bp-item").forEach(x=>{x.addEventListener("click",async()=>{const _=x.dataset.cid,v=z.find(L=>L.cardId===_);if(!v)return;const k=(w[v._line]||[]).find(L=>L.cardId===_);k&&(k.boost=(k.boost||0)+p.boostValue),g.remove(),await J({[a+"Team"]:w,boostUsed:!0})})})}function we(w=null){var O,ee;if(!(p.phase===a+"-attack")){f("Remplacement uniquement avant votre attaque","warning");return}const g=p[a+"Team"],y=p["usedSubIds_"+a]||[],x=p.maxSubs||3;if(y.length>=x){f(`Maximum ${x} remplacements atteint`,"warning");return}const _=Object.entries(g).flatMap(([R,F])=>(F||[]).filter(H=>H.used).map(H=>({...H,_line:R}))),v=(p[a+"Subs"]||[]).filter(R=>!y.includes(R.cardId));if(!_.length){f("Aucun joueur utilisé à remplacer","warning");return}if(!v.length){f("Aucun remplaçant disponible","warning");return}let k=Math.max(0,_.findIndex(R=>R.cardId===w));const L=((O=_[k])==null?void 0:O._line)||((ee=_[k])==null?void 0:ee.job);let E=Math.max(0,v.findIndex(R=>R.job===L)),I=!1;const S=document.createElement("div");S.id="pvp-sub-overlay",S.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function j(){var ce,ge,he,Ie,Se,Oe;const R=_[k],F=v[E],H=Math.min(130,Math.round((window.innerWidth-90)/2)),X=Math.round(H*1.35),ae=Ee=>`background:rgba(255,255,255,0.12);border:none;color:${Ee?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Ee?"default":"pointer"};flex-shrink:0`;S.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
        <div style="flex:1;font-size:15px;font-weight:900;color:#fff">🔄 Remplacement (${y.length}/${x})</div>
        <button id="psub-close" style="background:none;border:none;color:rgba(255,255,255,0.5);font-size:24px;cursor:pointer;padding:0">✕</button>
      </div>
      ...
      <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
        <button id="psub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
      </div>`,(ce=S.querySelector("#psub-close"))==null||ce.addEventListener("click",()=>S.remove()),(ge=S.querySelector("#pout-up"))==null||ge.addEventListener("click",()=>{k>0&&(k--,j())}),(he=S.querySelector("#pout-down"))==null||he.addEventListener("click",()=>{k<_.length-1&&(k++,j())}),(Ie=S.querySelector("#pin-up"))==null||Ie.addEventListener("click",()=>{E>0&&(E--,j())}),(Se=S.querySelector("#pin-down"))==null||Se.addEventListener("click",()=>{E<v.length-1&&(E++,j())});const pe=(Ee,Ge,Me,Ue)=>{const Ne=S.querySelector("#"+Ee);if(!Ne)return;let Je=0;Ne.addEventListener("touchstart",He=>{Je=He.touches[0].clientY},{passive:!0}),Ne.addEventListener("touchend",He=>{const Be=He.changedTouches[0].clientY-Je;if(Math.abs(Be)<30)return;const Ke=Ge();Be<0&&Ke<Ue-1?(Me(Ke+1),j()):Be>0&&Ke>0&&(Me(Ke-1),j())},{passive:!0})};pe("pin-panel",()=>E,Ee=>E=Ee,v.length),pe("pout-panel",()=>k,Ee=>k=Ee,_.length),(Oe=S.querySelector("#psub-confirm"))==null||Oe.addEventListener("click",async Ee=>{if(Ee.preventDefault(),Ee.stopPropagation(),I)return;I=!0;const Ge=_[k],Me=v[E];if(!Ge||!Me)return;const Ue=Ge._line,Ne=(g[Ue]||[]).findIndex(Be=>Be.cardId===Ge.cardId);if(Ne===-1){f("Erreur : joueur introuvable","error"),S.remove();return}const Je={...Me,_line:Ue,position:Ge.position,used:!1,boost:0};g[Ue].splice(Ne,1,Je);const He=[...y,Me.cardId];S.remove(),V(Ge,Me,async()=>{await J({[a+"Team"]:g,[o+"Team"]:p[o+"Team"],["usedSubIds_"+a]:He})})})}document.body.appendChild(S),j()}function V(w,z,g){const y={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const _=(L,E,I)=>`<div style="text-align:center">
      <div style="font-size:9px;color:${E};letter-spacing:2px;text-transform:uppercase;font-weight:700;margin-bottom:6px">${I}</div>
      <div style="width:70px;height:70px;border-radius:50%;background:${y[L.job]||"#555"};border:3px solid ${E};position:relative;overflow:hidden;margin:0 auto">
        ${Pe(L)?`<img src="${Pe(L)}" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover">`:""}
      </div>
      ...
      <div style="width:min(90vw,420px)">${ct(p[o+"Team"],p[o+"Formation"],null,[],300,300)}</div>
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(w),(z=w.querySelector("#pvp-opp-close"))==null||z.addEventListener("click",()=>w.remove())}function fe(){var y;const w=p.log||[],z=document.createElement("div");z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column";const g=x=>{var v,k,L;if(x.type==="duel"){const E=x.isGoal,I=x.homeScored&&a==="p1"||!x.homeScored&&E&&a==="p2",S=x.homeScored?"#FFD700":E?"#ff6b6b":"rgba(255,255,255,0.3)",j=E?I?"⚽ BUT !":"⚽ BUT adversaire !":(v=x.homePlayers)!=null&&v.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${E?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${S};margin-bottom:4px">
      ...
      </div>`,document.body.appendChild(z),(y=z.querySelector("#pvp-hist-close"))==null||y.addEventListener("click",()=>z.remove())}async function K(){if(p.phase!==a+"-attack")return;const w=a==="p1"?"p2":"p1",z=(p.round||0)+1,g=[...p.log||[]];g.push({type:"info",text:`⏭️ ${p[a+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const y=be(p),x=$e(w),_=y||!x?"finished":w+"-attack";await J({["selected_"+a]:[],modifiers:{...p.modifiers,[a]:{}},pendingAttack:null,phase:_,attacker:w,round:z,log:g}),_==="finished"&&await qe(p)}async function te(){const w=p[a+"Team"],z=!["GK","DEF","MIL","ATT"].some(_=>(p[o+"Team"][_]||[]).some(v=>!v.used)),g=(p["selected_"+a]||[]).map(_=>{const v=(w[_._role]||[]).find(j=>j.cardId===_.cardId)||_,k=z&&["GK","DEF"].includes(_._role),L=w[_._role]||[],E=L.findIndex(j=>j.cardId===_.cardId),I=st(L.length),S=E>=0?I[E]:v._col??1;return{...v,_line:_._role,_col:S,...k?{note_a:Math.max(1,Number(v.note_a)||0)}:{}}});if(!g.length)return;const y=Ut(g,p.modifiers[a]||{});U(a,g.map(_=>_.cardId)),g.forEach(_=>{const v=(w[_._role]||[]).find(k=>k.cardId===_.cardId);v&&(v.used=!0)}),p["selected_"+a]=[],ne();const x=[...p.log||[]];if(z){const _=(p[a+"Score"]||0)+1,v=g.map(S=>({name:S.name,note:ze(S,S._line||"ATT"),portrait:Pe(S),job:S.job}));x.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:v,homeTotal:y.total,aiTotal:0});const k=(p.round||0)+1,L=a==="p1"?"p2":"p1",E={...p,[a+"Team"]:w,[a+"Score"]:_},I=be(E);q.add(k),xe(v,_,p[o+"Score"]||0,!0,async()=>{await J({[a+"Team"]:w,[a+"Score"]:_,["selected_"+a]:[],modifiers:{...p.modifiers,[a]:{}},pendingAttack:null,phase:I?"finished":L+"-attack",attacker:L,round:k,log:x}),I&&await qe({...p,[a+"Score"]:_})});return}x.push({type:"pending",text:`⚔️ ${p[a+"Name"]} attaque (${y.total})`}),await J({[a+"Team"]:w,[o+"Team"]:p[o+"Team"],pendingAttack:{...y,players:g,side:a},["selected_"+a]:[],modifiers:{...p.modifiers,[a]:{}},phase:o+"-defense",log:x})}async function Y(){const w=p[a+"Team"],z=(p["selected_"+a]||[]).map(R=>{const F=(w[R._role]||[]).find(ce=>ce.cardId===R.cardId)||R,H=w[R._role]||[],X=H.findIndex(ce=>ce.cardId===R.cardId),ae=st(H.length),pe=X>=0?ae[X]:F._col??1;return{...F,_line:R._role,_col:pe}}),g=Kt(z,p.modifiers[a]||{});U(a,z.map(R=>R.cardId)),z.forEach(R=>{const F=(w[R._role]||[]).find(H=>H.cardId===R.cardId);F&&(F.used=!0)}),p["selected_"+a]=[],ne();const y=Vt(p.pendingAttack.total,g.total,p.modifiers[a]||{}),x=p.pendingAttack.side,_=y==="attack"||(y==null?void 0:y.goal),v=x==="p1"?"p2":"p1",k=(p.round||0)+1,L=(p.pendingAttack.players||[]).map(R=>({name:R.name,note:ze(R,R._line||"ATT"),portrait:Pe(R),job:R.job})),E=[...p.log||[]];E.push({type:"duel",isGoal:_,homeScored:_&&x===a,text:_?`⚽ BUT de ${p[x+"Name"]} ! (${p.pendingAttack.total} vs ${g.total})`:`✋ Attaque stoppée (${p.pendingAttack.total} vs ${g.total})`,homePlayers:L,aiPlayers:z.map(R=>({name:R.name,note:ze(R,R._line||"DEF"),portrait:Pe(R),job:R.job})),homeTotal:p.pendingAttack.total,aiTotal:g.total});const I=_?(p[x+"Score"]||0)+1:p[x+"Score"]||0,S={...p,[a+"Team"]:w,[x+"Score"]:I},j=be(S),O=j?"finished":v+"-attack",ee=async()=>{await J({[a+"Team"]:w,[o+"Team"]:p[o+"Team"],[x+"Score"]:I,["selected_"+a]:[],modifiers:{...p.modifiers,[a]:{}},pendingAttack:null,phase:O,attacker:v,round:k,log:E}),(O==="finished"||j)&&await qe({...p,[x+"Score"]:I})};if(_){const R=x===a,F=R?I:p[a+"Score"]||0,H=R?p[o+"Score"]||0:I;q.add(k),xe(L,F,H,R,ee)}else await ee()}function de(w){return["MIL","ATT"].some(z=>(w[z]||[]).some(g=>!g.used))}function ve(w){return["GK","DEF","MIL","ATT"].some(z=>(w[z]||[]).some(g=>!g.used))}function Te(w){return ve(w)&&!de(w)}function $e(w){const z=p[w+"Team"],g=p[(w==="p1"?"p2":"p1")+"Team"];return!!(de(z)||!ve(g)&&Te(z))}function be(w){const z=["MIL","ATT"].some(E=>(w.p1Team[E]||[]).some(I=>!I.used)),g=["MIL","ATT"].some(E=>(w.p2Team[E]||[]).some(I=>!I.used)),y=ve(w.p1Team),x=ve(w.p2Team);return!z&&!(!x&&y)&&(!g&&!(!y&&x))}function ke(w){const z=w.p1Score||0,g=w.p2Score||0;return z===g?null:z>g?h.home_id:h.away_id}async function qe(w){try{const z=ke(w),g=z?h.home_id===z?h.away_id:h.home_id:null;await A.from("matches").update({status:"finished",winner_id:z,home_score:w.p1Score||0,away_score:w.p2Score||0}).eq("id",i),z&&g&&Ai(z,g).catch(()=>{})}catch(z){console.error("[PvP] finishMatch:",z)}}function je(){var x;const w=p[a+"Score"],z=p[o+"Score"],g=w>z,y=w===z;et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      <div style="font-size:64px">${g?"🏆":y?"🤝":"😤"}</div>
      <div style="font-size:24px;font-weight:900;color:#fff">${g?"Victoire !":y?"Match nul":"Défaite"}</div>
      <div style="font-size:32px;font-weight:900;color:#FFD700">${w} - ${z}</div>
      <button id="pvp-home" style="padding:16px 40px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">🏠 Retour</button>
    </div>`,(x=document.getElementById("pvp-home"))==null||x.addEventListener("click",()=>{try{A.removeChannel(N)}catch{}Ve(e),c("home")})}ne()}const It="#1A6B3C",yt="#D4A017";async function Zn(e,t){var n;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const i=(n=t.state.params)==null?void 0:n.openLeagueId;if(i){t.state.params.openLeagueId=null,await ht(e,t,i);return}await Et(e,t)}async function Et(e,t,i="waiting"){var m;const{state:n}=t,d=n.profile.id,{data:s}=await A.from("mini_league_members").select("league_id").eq("user_id",d),r=(s||[]).map(T=>T.league_id),{data:c,error:f}=await A.from("mini_leagues").select("*, mini_league_members(count)").eq("status","waiting").eq("is_archived",!1).order("created_at",{ascending:!1}).limit(30),a=f?(await A.from("mini_leagues").select("*, mini_league_members(count)").eq("status","waiting").order("created_at",{ascending:!1}).limit(30)).data||[]:c||[],l=(r.length?await A.from("mini_leagues").select("*, mini_league_members(count)").in("id",r).order("created_at",{ascending:!1}):{data:[]}).data||[],b=l.filter(T=>T.status==="waiting"&&!T.is_archived),h=l.filter(T=>T.status==="active"&&!T.is_archived),$=l.filter(T=>T.is_archived||T.status==="finished"),p=a.filter(T=>!r.includes(T.id)),u=[{key:"waiting",label:"🟡 En attente",count:b.length+p.length},{key:"active",label:"🟢 En cours",count:h.length},{key:"archived",label:"📁 Archivées",count:$.length}];e.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:20px;display:flex;flex-direction:column;gap:14px">
      ...
      </div>`,document.body.appendChild(t);const i=t.querySelector("#ml-pwd-input"),n=d=>{t.remove(),e(d)};setTimeout(()=>i==null?void 0:i.focus(),80),t.querySelector("#pwd-cancel").addEventListener("click",()=>n(null)),t.querySelector("#pwd-ok").addEventListener("click",()=>n(i.value.trim())),i.addEventListener("keydown",d=>{d.key==="Enter"&&n(i.value.trim())}),t.addEventListener("click",d=>{d.target===t&&n(null)})})}async function Eo(e,t,i,n){const{toast:d,state:s}=t,r=s.profile.id,{data:c}=await A.from("mini_leagues").select("password,status,entry_fee,pot,max_players").eq("id",i).single();if(!c){d("Mini League introuvable","error");return}if(c.status!=="waiting"){d("Cette Mini League a déjà démarré","warning");return}const{count:f}=await A.from("mini_league_members").select("id",{count:"exact",head:!0}).eq("league_id",i);if(f>=c.max_players){d("La Mini League est complète","warning");return}if(n==="private"){const p=await or();if(p===null)return;if(c.password!==p){d("Mot de passe incorrect","error");return}}const a=c.entry_fee||100,{data:o}=await A.from("users").select("credits").eq("id",r).single();if(((o==null?void 0:o.credits)||0)<a){d(`Crédits insuffisants — il te faut ${a.toLocaleString("fr")} cr. (solde : ${((o==null?void 0:o.credits)||0).toLocaleString("fr")} cr.)`,"error");return}const{error:l}=await A.from("mini_league_members").insert({league_id:i,user_id:r});if(l){d("Erreur inscription : "+l.message,"error");return}const{error:b}=await A.from("users").update({credits:o.credits-a}).eq("id",r),{error:h}=await A.from("mini_leagues").update({pot:(c.pot||0)+a}).eq("id",i);(b||h)&&console.error("Erreur post-inscription:",b,h),s.profile&&(s.profile.credits=o.credits-a);const $=document.getElementById("nav-credits");$&&($.textContent=`💰 ${(s.profile.credits||0).toLocaleString("fr")}`),d(`✅ Inscrit ! −${a.toLocaleString("fr")} cr. · Pot : ${((c.pot||0)+a).toLocaleString("fr")} cr.`,"success"),ht(e,t,i)}async function nr(e,t,i,n){const{toast:d,state:s}=t,r=s.profile.id;if(!confirm(`Te désinscrire et récupérer ${n.toLocaleString("fr")} cr. ?`))return;const{data:c}=await A.from("mini_leagues").select("pot,status").eq("id",i).single();if(!c||c.status!=="waiting"){d("Impossible — la league a déjà démarré","error");return}const{data:f}=await A.from("users").select("credits").eq("id",r).single();await A.from("mini_leagues").update({pot:Math.max(0,(c.pot||0)-n)}).eq("id",i),await A.from("users").update({credits:((f==null?void 0:f.credits)||0)+n}).eq("id",r),await A.from("mini_league_members").delete().eq("league_id",i).eq("user_id",r),s.profile&&(s.profile.credits=((f==null?void 0:f.credits)||0)+n);const a=document.getElementById("nav-credits");a&&(a.textContent=`💰 ${(s.profile.credits||0).toLocaleString("fr")}`),d(`↩️ Désinscrit · +${n.toLocaleString("fr")} cr. remboursés`,"success"),Et(e,t,"waiting")}async function vi(e,t,i,n,d){const{toast:s}=t;if(!confirm(`Supprimer définitivement "${n}" et tous ses matchs/membres ? Action irréversible.`))return;await A.from("mini_league_matches").delete().eq("league_id",i),await A.from("mini_league_members").delete().eq("league_id",i);const{error:r}=await A.from("mini_leagues").delete().eq("id",i);if(r){s("Erreur suppression ("+r.message+")","error");return}s("Mini League supprimée avec succès","success"),Et(e,t,d)}async function rr(e,t,i){await A.from("mini_leagues").update({is_archived:!0}).eq("id",i),t.toast("Mini League archivée","success"),Et(e,t,"archived")}async function ht(e,t,i){var G,q,B,C,N,J,Z,se,re;const{state:n,toast:d}=t,s=n.profile.id,[{data:r},{data:c},{data:f}]=await Promise.all([A.from("mini_leagues").select("*").eq("id",i).single(),A.from("mini_league_members").select("*, user:users(id,pseudo,club_name,club_color1,club_color2)").eq("league_id",i),A.from("mini_league_matches").select("*").eq("league_id",i).order("matchday").order("created_at")]);if(!r){d("Introuvable","error"),await Et(e,t);return}const a=(c||[]).some(Q=>Q.user_id===s),o=r.creator_id===s,l=(c||[]).map(Q=>Q.user).filter(Boolean),b=Ao(l,f||[]),h=(f||[]).filter(Q=>Q.matchday===r.current_day),$=r.pot||0,p=r.entry_fee||100,u=h.length>0&&h.every(Q=>Q.status==="finished"||Q.status==="bye"),m=r.current_day>=r.total_days,T=(c||[]).find(Q=>Q.user_id===s);e.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>${b.map((Q,U)=>{const D=$>0&&r.status==="finished"?U===0?Math.floor($*.7):U===1?Math.floor($*.2):U===2?Math.floor($*.1):0:0;return`<tr style="border-bottom:1px solid #f5f5f5;${Q.userId===s?"background:#f0fdf4;":""}">
              ...
              ${$>0&&r.status==="finished"?`<td style="text-align:right;font-weight:700;color:${yt}">${D?D.toLocaleString("fr")+" cr.":"—"}</td>`:""}
            </tr>`}).join("")}</tbody>
        </table>
      </div>`:""}
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        ${Array.from({length:Math.max(0,r.status==="active"?r.current_day-1:r.current_day)},(Q,U)=>U+1).reverse().map(Q=>{const U=(f||[]).filter(D=>D.matchday===Q);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:#888;margin-bottom:6px">Journée ${Q}</div>${U.map(D=>Ki(D,l,s,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
  ...
  </div>`}async function ar(e,t,i,n){const{toast:d,state:s}=t,r=lr(n.map(a=>a.id),i.mode),c=[];r.forEach((a,o)=>a.forEach(l=>c.push({league_id:i.id,matchday:o+1,home_id:l.home||l.bye,away_id:l.away||null,is_bye:!!l.bye,status:l.bye?"bye":"pending"})));const{error:f}=await A.from("mini_league_matches").insert(c);if(f){d("Erreur calendrier : "+f.message,"error");return}await A.from("mini_leagues").update({status:"active",current_day:1,total_days:r.length}).eq("id",i.id),d(`🚀 Lancée ! Pot : ${(i.pot||0).toLocaleString("fr")} cr.`,"success"),ht(e,t,i.id)}async function sr(e,t,i){const{data:n}=await A.from("mini_leagues").select("current_day,total_days,pot").eq("id",i).single(),d=(n.current_day||0)+1;if(d>(n.total_days||0)){await To(e,t,i,n.pot||0,null,null);return}await A.from("mini_leagues").update({current_day:d}).eq("id",i),t.toast(`Journée ${d} commencée !`,"success"),ht(e,t,i)}async function To(e,t,i,n,d,s){const{toast:r,state:c}=t;let f=d,a=s;if(!f){const{data:l}=await A.from("mini_league_matches").select("*").eq("league_id",i),{data:b}=await A.from("mini_league_members").select("*, user:users(id,pseudo,club_name)").eq("league_id",i);a=(b||[]).map(h=>h.user).filter(Boolean),f=Ao(a,l||[])}const o=[Math.floor(n*.7),Math.floor(n*.2),Math.floor(n*.1)];await Promise.all(f.slice(0,3).map((l,b)=>o[b]?A.from("mini_league_members").update({prize_amount:o[b],prize_claimed:!1}).eq("league_id",i).eq("user_id",l.userId):Promise.resolve())),await A.from("mini_leagues").update({status:"finished"}).eq("id",i),r("🏆 Mini League terminée ! Les gagnants peuvent récupérer leurs crédits.","success"),ht(e,t,i)}async function dr(e,t,i,n,d){var b,h;const{state:s,toast:r}=t,c=[Math.floor((i.pot||0)*.7),Math.floor((i.pot||0)*.2),Math.floor((i.pot||0)*.1)],f=["🥇","🥈","🥉"][d],a=n.prize_amount||c[d]||0,o=n.prize_claimed,l=document.createElement("div");l.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",l.innerHTML=`
    <div style="background:linear-gradient(160deg,#1a0a2e,#3b1a6e);border-radius:20px;padding:32px 24px;max-width:320px;width:100%;text-align:center;color:#fff;box-shadow:0 8px 40px rgba(0,0,0,0.6)">
      ...
      <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:20px;background:linear-gradient(180deg,#0a1628,#1a0a2e);padding:24px;text-align:center;color:#fff">
        ...
        <button id="lobby-cancel" style="padding:10px 24px;border-radius:10px;border:1.5px solid rgba(255,255,255,0.2);background:transparent;color:rgba(255,255,255,0.6);font-size:13px;cursor:pointer">Annuler</button>
        <style>@keyframes lobbyPulse{0%,100%{opacity:1}50%{opacity:0.4}}</style>
      </div>`,(N=document.getElementById("lobby-cancel"))==null||N.addEventListener("click",()=>{$=!0,clearInterval(p),Ve(e),l("mini-league")})},q=async(B,C)=>{$=!0,clearInterval(p),await new Promise(N=>setTimeout(N,600)),e.isConnected&&pr(e,t,B,C,r,c)};if(G(),m){let B=u.match_id;if(!B){const{data:N,error:J}=await A.from("matches").insert({home_id:h,away_id:M,home_deck_id:i,away_deck_id:null,status:"active",mode:"mini_league"}).select().single();if(J||!N){b("Erreur création match","error"),Ve(e),l("mini-league");return}B=N.id,await A.from("mini_league_matches").update({match_id:B,status:"playing"}).eq("id",f)}const C=B;p=setInterval(async()=>{if($){clearInterval(p);return}const{data:N}=await A.from("matches").select("away_deck_id").eq("id",C).single();N!=null&&N.away_deck_id&&(clearInterval(p),q(C,!0))},1500)}else{let B=u.match_id;p=setInterval(async()=>{if($){clearInterval(p);return}const{data:C}=await A.from("mini_league_matches").select("match_id").eq("id",f).single();C!=null&&C.match_id&&(clearInterval(p),B=C.match_id,await A.from("matches").update({away_deck_id:i}).eq("id",B),q(B,!1))},1500)}}async function pr(e,t,i,n,d=[],s=[]){var z;const{state:r,navigate:c,toast:f}=t,a=((z=r.params)==null?void 0:z.leagueId)||null,o=n?"p1":"p2",l=n?"p2":"p1",b=(d||[]).map(g=>g.id),h=(d||[]).map(g=>({id:g.id,gc_type:g.gc_type,_gcDef:g._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:$}=await A.from("matches").select("*").eq("id",i).single();if(!$){f("Match introuvable","error"),c("mini-league");return}async function p(){const[{data:g},{data:y},{data:x},{data:_}]=await Promise.all([A.rpc("get_deck_for_match",{p_deck_id:$.home_deck_id}),A.rpc("get_deck_for_match",{p_deck_id:$.away_deck_id}),A.from("users").select("id,pseudo,club_name").eq("id",$.home_id).single(),A.from("users").select("id,pseudo,club_name").eq("id",$.away_id).single()]),v=S=>{const j=Number(S.evolution_bonus)||0;return{cardId:S.card_id,position:S.position,id:S.id,firstname:S.firstname,name:S.surname_encoded,country_code:S.country_code,club_id:S.club_id,job:S.job,job2:S.job2,note_g:(Number(S.note_g)||0)+(S.job==="GK"||S.job2==="GK"&&Number(S.note_g)>0?j:0),note_d:(Number(S.note_d)||0)+(S.job==="DEF"||S.job2==="DEF"&&Number(S.note_d)>0?j:0),note_m:(Number(S.note_m)||0)+(S.job==="MIL"||S.job2==="MIL"&&Number(S.note_m)>0?j:0),note_a:(Number(S.note_a)||0)+(S.job==="ATT"||S.job2==="ATT"&&Number(S.note_a)>0?j:0),evolution_bonus:j,rarity:S.rarity,skin:S.skin,hair:S.hair,hair_length:S.hair_length,clubName:S.club_encoded_name||null,clubLogo:S.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},k=((g==null?void 0:g.starters)||[]).map(S=>v(S)),L=((y==null?void 0:y.starters)||[]).map(S=>v(S)),E=(g==null?void 0:g.formation)||"4-4-2",I=(y==null?void 0:y.formation)||"4-4-2";return{p1Team:(()=>{const S=bt(k,E);return stadiumDef?Ct(S,stadiumDef):S})(),p2Team:bt(L,I),p1Subs:(()=>{const S=((g==null?void 0:g.subs)||[]).map(j=>v(j));return stadiumDef&&Ot(S,stadiumDef),S})(),p2Subs:((y==null?void 0:y.subs)||[]).map(S=>v(S)),p1Formation:E,p2Formation:I,p1Name:(x==null?void 0:x.club_name)||(x==null?void 0:x.pseudo)||"Joueur 1",p2Name:(_==null?void 0:_.club_name)||(_==null?void 0:_.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:n?b:[],gc_p2:n?[]:b,gcCardsFull_p1:n?h:[],gcCardsFull_p2:n?[]:h,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:s||[],lastActionAt:new Date().toISOString()}}let u=$.game_state&&Object.keys($.game_state).length?$.game_state:null;if(!u)if(n){u=await p();const{data:g}=await A.from("matches").select("game_state").eq("id",i).single();!(g!=null&&g.game_state)||!Object.keys(g.game_state).length?await A.from("matches").update({game_state:u,turn_user_id:$.home_id}).eq("id",i):u=g.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let g=0;g<60&&!u;g++){await new Promise(x=>setTimeout(x,400));const{data:y}=await A.from("matches").select("game_state").eq("id",i).single();y!=null&&y.game_state&&Object.keys(y.game_state).length&&(u=y.game_state)}if(!u){f("Erreur de synchronisation","error"),c("mini-league");return}u.gc_p2=b,u.gcCardsFull_p2=h,await A.from("matches").update({game_state:u}).eq("id",i)}let m=!1,T=!1,M=!1,G=null,q=!1;const B=new Set,C=new Set;function N(g){var S,j;try{A.removeChannel(J)}catch{}const y=u[o+"Score"]||0,x=u[l+"Score"]||0;if(!M){M=!0;const O=g.winner_id||(y>x?r.profile.id:x>y?"opp":null),ee=O===r.profile.id?"win":O?"loss":null;ee&&ni(()=>Promise.resolve().then(()=>so),void 0).then(R=>R.applyOwnEvolution(r.profile.id,ee)).catch(()=>{})}if(!T){const O=u.p1Score||0,ee=u.p2Score||0,R=(u.usedGc_p1||[]).length,F=(u.usedGc_p2||[]).length,H=g.winner_id||(O>ee?$.home_id:ee>O?$.away_id:null);(H?r.profile.id===H:r.profile.id<(n?$.away_id:$.home_id))&&(T=!0,to({player1Id:$.home_id,player2Id:$.away_id,score1:O,score2:ee,gc1:R,gc2:F}).catch(ae=>console.warn("[FriendMatch] updateStats:",ae)))}let _,v;g.winner_id?(_=g.winner_id===r.profile.id,v=!1):g.forfeit?(_=y>x,v=!1):(v=y===x,_=y>x),(S=document.getElementById("pvp-end-overlay"))==null||S.remove();const k=document.createElement("div");k.id="pvp-end-overlay",k.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const L=v?"🤝":_?"🏆":"😞",E=v?"MATCH NUL":_?"VICTOIRE !":"DÉFAITE",I=v?"#fff":_?"#FFD700":"#ff6b6b";k.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">🏆 Retour à la Mini League</button>`,document.body.appendChild(k),(j=k.querySelector("#pvp-end-home"))==null||j.addEventListener("click",()=>{k.remove(),Ve(e),a?c("mini-league",{openLeagueId:a}):c("mini-league")})}const J=A.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},g=>{const y=g.new;try{if(y.status==="finished"||y.forfeit){if(m||(m=!0,G&&(clearInterval(G),G=null),y.game_state&&(u=y.game_state),u.phase==="finished"&&!y.forfeit&&document.getElementById("pvp-home")))return;N(y);return}if(y.game_state){const x=u;u=y.game_state;const _=u._lastGC;if(_&&_.seq&&!C.has(_.seq)&&(C.add(_.seq),_.by!==o)){Fe(_.type,_.by,()=>ue());return}const v=x[o+"Score"]||0,k=x[l+"Score"]||0,L=u[o+"Score"]||0,E=u[l+"Score"]||0,I=L>v,S=E>k;if((I||S)&&!B.has(u.round)){B.add(u.round);const j=[...u.log||[]].reverse().find(ee=>ee.isGoal),O=((j==null?void 0:j.homePlayers)||[]).map(ee=>({name:ee.name,note:ee.note,portrait:ee.portrait,job:ee.job}));Le(O,L,E,I,()=>ue());return}ue()}}catch(x){console.error("[PvP] crash:",x)}}).subscribe();async function Z(g){Object.assign(u,g),u.lastActionAt=new Date().toISOString();const{error:y}=await A.from("matches").update({game_state:u}).eq("id",i);y&&f("Erreur de synchronisation","error");try{ue()}catch(x){console.error("[PvP] renderPvpScreen crash:",x)}}async function se(){if(m)return;m=!0,G&&(clearInterval(G),G=null);const g=n?$.away_id:$.home_id,y=n?"p2":"p1",x=n?"p1":"p2",_={...u,[y+"Score"]:3,[x+"Score"]:0,phase:"finished"};try{await A.from("matches").update({status:"finished",forfeit:!0,winner_id:g,home_score:_.p1Score||0,away_score:_.p2Score||0,game_state:_}).eq("id",i)}catch{}try{A.removeChannel(J)}catch{}setTimeout(()=>{Ve(e),c("mini-league")},800)}const re={BOOST_STAT:({value:g=1,count:y=1,roles:x=[]},_,v)=>{const k=_[o+"Team"],L=Object.entries(k).filter(([E])=>!x.length||x.includes(E)).flatMap(([E,I])=>I.filter(S=>!S.used).map(S=>({...S,_line:E})));if(!L.length){_.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),v(_);return}Q(L,y,`Choisir ${y} joueur(s) à booster (+${g})`,E=>{E.forEach(I=>{const S=(k[I._line]||[]).find(j=>j.cardId===I.cardId);S&&(S.boost=(S.boost||0)+g,_.log.push({text:`⚡ +${g} sur ${S.name}`,type:"info"}))}),_[o+"Team"]=k,v(_)})},DEBUFF_STAT:({value:g=1,count:y=1,roles:x=[],target:_="ai"},v,k)=>{const L=_==="home"?o:l,E=v[L+"Team"],I=_==="home"?"allié":"adverse",S=Object.entries(E).filter(([j])=>!x.length||x.includes(j)).flatMap(([j,O])=>O.map(ee=>({...ee,_line:j})));if(!S.length){v.log.push({text:`🎯 Aucun joueur ${I}`,type:"info"}),k(v);return}Q(S,y,`Choisir ${y} joueur(s) ${I}(s) (-${g})`,j=>{j.forEach(O=>{const ee=(E[O._line]||[]).find(R=>R.cardId===O.cardId);ee&&(ee.boost=(ee.boost||0)-g,v.log.push({text:`🎯 -${g} sur ${ee.name}`,type:"info"}))}),v[L+"Team"]=E,k(v)})},GRAY_PLAYER:({count:g=1,roles:y=[],target:x="ai"},_,v)=>{const k=x==="home"?o:l,L=_[k+"Team"],E=x==="home"?"allié":"adverse",I=Object.entries(L).filter(([S])=>!y.length||y.includes(S)).flatMap(([S,j])=>j.filter(O=>!O.used).map(O=>({...O,_line:S})));if(!I.length){_.log.push({text:`❌ Aucun joueur ${E}`,type:"info"}),v(_);return}Q(I,g,`Choisir ${g} joueur(s) ${E}(s) à exclure`,S=>{const j="usedCardIds_"+k,O=new Set(_[j]||[]);S.forEach(ee=>{const R=(L[ee._line]||[]).find(F=>F.cardId===ee.cardId);R&&(R.used=!0,O.add(ee.cardId),_.log.push({text:`❌ ${R.name} exclu !`,type:"info"}))}),_[j]=[...O],_[k+"Team"]=L,v(_)})},REVIVE_PLAYER:({count:g=1,roles:y=[]},x,_)=>{const v=x[o+"Team"],k=Object.entries(v).filter(([L])=>!y.length||y.includes(L)).flatMap(([L,E])=>E.filter(I=>I.used).map(I=>({...I,_line:L})));if(!k.length){x.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),_(x);return}Q(k,g,`Choisir ${g} joueur(s) à ressusciter`,L=>{L.forEach(E=>{const I=(v[E._line]||[]).find(S=>S.cardId===E.cardId);I&&(I.used=!1,x.log.push({text:`💫 ${I.name} ressuscité !`,type:"info"}))}),x[o+"Team"]=v,_(x)})},REMOVE_GOAL:({},g,y)=>{const x=l+"Score";g[x]>0?(g[x]--,g.log.push({text:"🚫 Dernier but annulé !",type:"info"})):g.log.push({text:"🚫 Aucun but à annuler",type:"info"}),y(g)},ADD_GOAL_DRAW:({},g,y)=>{g[o+"Score"]===g[l+"Score"]?(g[o+"Score"]++,g.log.push({text:"🎯 But bonus !",type:"info"})):g.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),y(g)},ADD_SUB:({value:g=1},y,x)=>{y.maxSubs=(y.maxSubs||3)+g,y.log.push({text:`🔄 +${g} remplacement(s)`,type:"info"}),x(y)},CUSTOM:({},g,y)=>y(g)};function Q(g,y,x,_){const v=document.createElement("div");v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let k=[];function L(){var I,S;const E=g.map(j=>{const O={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[j._line]||"#555",ee=ze(j,j._line)+(j.boost||0),F=k.find(X=>X.cardId===j.cardId)?"#FFD700":"rgba(255,255,255,0.25)",H=j.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${j.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${F};background:${O};overflow:hidden;cursor:pointer;${H}">
        ...
        </div>`,(I=v.querySelector("#pp-close"))==null||I.addEventListener("click",()=>v.remove()),v.querySelectorAll(".pp-item").forEach(j=>{j.addEventListener("click",()=>{const O=j.dataset.cid,ee=g.find(F=>F.cardId===O),R=k.findIndex(F=>F.cardId===O);ee&&(R>-1?k.splice(R,1):k.length<y&&k.push(ee),L())})}),(S=v.querySelector("#pp-confirm"))==null||S.addEventListener("click",()=>{v.remove(),_(k)})}L(),document.body.appendChild(v)}async function U(g,y){const _=(u["gcCardsFull_"+o]||[]).find(E=>E.id===g),v=(_==null?void 0:_._gcDef)||(u.gcDefs||[]).find(E=>{var I;return E.name===y||((I=E.name)==null?void 0:I.toLowerCase().trim())===(y==null?void 0:y.toLowerCase().trim())}),k=[...u["usedGc_"+o]||[],g],L={type:y,by:o,seq:(u._gcAnimSeq||0)+1};C.add(L.seq),Fe(y,o,async()=>{if(v!=null&&v.effect_type&&v.effect_type!=="CUSTOM"){const I=re[v.effect_type];if(I){const S={...u};I(v.effect_params||{},S,async j=>{j["usedGc_"+o]=k,j._lastGC=L,j._gcAnimSeq=L.seq,await Z(j)});return}}const E={...u};switch(y){case"Remplacement+":E.maxSubs=(E.maxSubs||3)+1,E.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const I=l+"Score";E[I]>0&&(E[I]--,E.log.push({text:"🚫 But annulé",type:"info"}));break}}E["usedGc_"+o]=k,E._lastGC=L,E._gcAnimSeq=L.seq,await Z(E)})}function D(g,y){const x="usedCardIds_"+g,_=new Set(u[x]||[]);y.forEach(v=>_.add(v)),u[x]=[..._]}function ne(){for(const g of["p1","p2"]){const y=new Set(u["usedCardIds_"+g]||[]),x=u[g+"Team"];if(x)for(const _ of["GK","DEF","MIL","ATT"])(x[_]||[]).forEach(v=>{v.used=y.has(v.cardId)})}}function ue(){var it,ut,ft,gt,P,le;if(u.phase==="reveal")return ye();if(u.phase==="midfield")return xe();if(u.phase==="finished")return w();const g=u[o+"Team"],y=u[l+"Team"];ne();const x=u[o+"Score"],_=u[l+"Score"],v=u[o+"Name"],k=u[l+"Name"],L=u.phase===o+"-attack",E=u.phase===o+"-defense",I=Array.isArray(u["selected_"+o])?u["selected_"+o]:[],S=I.map(W=>W.cardId),j=window.innerWidth>=700,O=u[o+"Subs"]||[],ee=u["usedSubIds_"+o]||[],R=O.filter(W=>!ee.includes(W.cardId)),F=u["gcCardsFull_"+o]||[],H=u["usedGc_"+o]||[],X=F.filter(W=>!H.includes(W.id)),ae=u.boostOwner===o&&!u.boostUsed,pe=!["GK","DEF","MIL","ATT"].some(W=>(y[W]||[]).some(ie=>!ie.used)),ce=[...g.MIL||[],...g.ATT||[]].filter(W=>!W.used),ge=L&&pe&&ce.length===0?[...g.GK||[],...g.DEF||[]].filter(W=>!W.used).map(W=>W.cardId):[];function he(W,ie,Ae){var Ii,Mi;const _e=W._gcDef,Xe={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[_e==null?void 0:_e.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",Qe={purple:"#b06ce0",light_blue:"#00d4ef"}[_e==null?void 0:_e.color]||"#b06ce0",ot=(_e==null?void 0:_e.name)||W.gc_type,dt=(_e==null?void 0:_e.effect)||((Ii=Re[W.gc_type])==null?void 0:Ii.desc)||"",mt=_e!=null&&_e.image_url?`/icons/${_e.image_url}`:null,vt=((Mi=Re[W.gc_type])==null?void 0:Mi.icon)||"⚡",rt=Math.round(Ae*.22),Tt=Math.round(Ae*.22),lt=Ae-rt-Tt,wt=ot.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${W.id}" data-gc-type="${W.gc_type}"
      ...
      </div>`}const Se=(W,ie)=>ie?Ie(130,175):he(W,130,175),Oe=(W,ie)=>ie?Ie(68,95):he(W,68,95),Ee=j?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",Ge=L?be(o)?`<button id="pvp-action" style="${Ee};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${I.length===0?"disabled":""}>⚔️ ATTAQUEZ <span id="pvp-timer"></span></button>`:`<button id="pvp-action" data-pass="1" style="${Ee};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER <span id="pvp-timer"></span></button>`:E?`<button id="pvp-action" style="${Ee};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${I.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="pvp-timer"></span></button>`:`<div style="font-size:11px;color:rgba(255,255,255,0.3);text-align:center;padding:4px">⏳ Tour de ${k}</div>`,Me=L&&!be(o)?'<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">Aucun attaquant — passez la main</div>':L||E?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${I.length}/3 sélectionné(s)</div>`:"",Ue=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${j?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
      ${R.length===0?'<div style="font-size:7px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':R.map(W=>`<div class="pvp-sub-btn" data-sub-id="${W.cardId}" style="cursor:pointer;flex-shrink:0">${We(W,j?76:44,j?100:58)}</div>`).join("")}
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(k);let I=!1;const S=()=>{I||(I=!0,k.remove(),setTimeout(()=>v(),50))};k.addEventListener("click",S),setTimeout(S,3500)}function Fe(g,y,x){var H,X;const _=(u.gcDefs||[]).find(ae=>{var pe;return ae.name===g||((pe=ae.name)==null?void 0:pe.toLowerCase().trim())===(g==null?void 0:g.toLowerCase().trim())}),v={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[_==null?void 0:_.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",k={purple:"#b06ce0",light_blue:"#00d4ef"}[_==null?void 0:_.color]||"#b06ce0",L=(_==null?void 0:_.name)||g,E=(_==null?void 0:_.effect)||((H=Re[g])==null?void 0:H.desc)||"",I=_!=null&&_.image_url?`/icons/${_.image_url}`:null,S=((X=Re[g])==null?void 0:X.icon)||"⚡",O=y===o?"Vous":u[y+"Name"]||"Adversaire",ee=document.createElement("div");ee.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",ee.innerHTML=`
      <style>
        @keyframes gcFlipIn{0%{transform:perspective(800px) rotateY(90deg) scale(.7);opacity:0}55%{transform:perspective(800px) rotateY(-12deg) scale(1.08);opacity:1}100%{transform:perspective(800px) rotateY(0) scale(1);opacity:1}}
        @keyframes gcGlow{0%,100%{box-shadow:0 0 30px ${k}66}50%{box-shadow:0 0 60px ${k}cc}}
        @keyframes gcLabel{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:translateY(0)}}
      </style>
      <div style="font-size:11px;color:${k};letter-spacing:3px;text-transform:uppercase;font-weight:700;animation:gcLabel .4s ease both">${O} joue une carte</div>
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(ee);let R=!1;const F=()=>{R||(R=!0,ee.remove(),setTimeout(()=>x&&x(),50))};ee.addEventListener("click",F),setTimeout(F,3e3)}function Ce(g,y){var ee,R,F,H;const _=(u["gcCardsFull_"+o]||[]).find(X=>X.id===g),v=_==null?void 0:_._gcDef,k={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[v==null?void 0:v.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",L={purple:"#b06ce0",light_blue:"#00d4ef"}[v==null?void 0:v.color]||"#b06ce0",E=(v==null?void 0:v.name)||y,I=(v==null?void 0:v.effect)||((ee=Re[y])==null?void 0:ee.desc)||"Carte spéciale.",S=v!=null&&v.image_url?`/icons/${v.image_url}`:null,j=((R=Re[y])==null?void 0:R.icon)||"⚡",O=document.createElement("div");O.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",O.innerHTML=`
      ...
      </div>`,document.body.appendChild(x),(_=x.querySelector("#bp-close"))==null||_.addEventListener("click",()=>x.remove()),x.querySelectorAll(".bp-item").forEach(v=>{v.addEventListener("click",async()=>{const k=v.dataset.cid,L=y.find(I=>I.cardId===k);if(!L)return;const E=(g[L._line]||[]).find(I=>I.cardId===k);E&&(E.boost=(E.boost||0)+u.boostValue),x.remove(),await Z({[o+"Team"]:g,boostUsed:!0})})})}function V(g=null){var R,F;if(!(u.phase===o+"-attack")){f("Remplacement uniquement avant votre attaque","warning");return}const x=u[o+"Team"],_=u["usedSubIds_"+o]||[],v=u.maxSubs||3;if(_.length>=v){f(`Maximum ${v} remplacements atteint`,"warning");return}const k=Object.entries(x).flatMap(([H,X])=>(X||[]).filter(ae=>ae.used).map(ae=>({...ae,_line:H}))),L=(u[o+"Subs"]||[]).filter(H=>!_.includes(H.cardId));if(!k.length){f("Aucun joueur utilisé à remplacer","warning");return}if(!L.length){f("Aucun remplaçant disponible","warning");return}let E=Math.max(0,k.findIndex(H=>H.cardId===g));const I=((R=k[E])==null?void 0:R._line)||((F=k[E])==null?void 0:F.job);let S=Math.max(0,L.findIndex(H=>H.job===I)),j=!1;const O=document.createElement("div");O.id="pvp-sub-overlay",O.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function ee(){var he,Ie,Se,Oe,Ee,Ge;const H=k[E],X=L[S],ae=Math.min(130,Math.round((window.innerWidth-90)/2)),pe=Math.round(ae*1.35),ce=Me=>`background:rgba(255,255,255,0.12);border:none;color:${Me?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Me?"default":"pointer"};flex-shrink:0`;O.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
        <button id="psub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
      </div>`,(he=O.querySelector("#psub-close"))==null||he.addEventListener("click",()=>O.remove()),(Ie=O.querySelector("#pout-up"))==null||Ie.addEventListener("click",()=>{E>0&&(E--,ee())}),(Se=O.querySelector("#pout-down"))==null||Se.addEventListener("click",()=>{E<k.length-1&&(E++,ee())}),(Oe=O.querySelector("#pin-up"))==null||Oe.addEventListener("click",()=>{S>0&&(S--,ee())}),(Ee=O.querySelector("#pin-down"))==null||Ee.addEventListener("click",()=>{S<L.length-1&&(S++,ee())});const ge=(Me,Ue,Ne,Je)=>{const He=O.querySelector("#"+Me);if(!He)return;let Be=0;He.addEventListener("touchstart",Ke=>{Be=Ke.touches[0].clientY},{passive:!0}),He.addEventListener("touchend",Ke=>{const Ze=Ke.changedTouches[0].clientY-Be;if(Math.abs(Ze)<30)return;const it=Ue();Ze<0&&it<Je-1?(Ne(it+1),ee()):Ze>0&&it>0&&(Ne(it-1),ee())},{passive:!0})};ge("pin-panel",()=>S,Me=>S=Me,L.length),ge("pout-panel",()=>E,Me=>E=Me,k.length),(Ge=O.querySelector("#psub-confirm"))==null||Ge.addEventListener("click",async Me=>{if(Me.preventDefault(),Me.stopPropagation(),j)return;j=!0;const Ue=k[E],Ne=L[S];if(!Ue||!Ne)return;const Je=Ue._line,He=(x[Je]||[]).findIndex(Ze=>Ze.cardId===Ue.cardId);if(He===-1){f("Erreur : joueur introuvable","error"),O.remove();return}const Be={...Ne,_line:Je,position:Ue.position,used:!1,boost:0};x[Je].splice(He,1,Be);const Ke=[..._,Ne.cardId];O.remove(),oe(Ue,Ne,async()=>{await Z({[o+"Team"]:x,[l+"Team"]:u[l+"Team"],["usedSubIds_"+o]:Ke})})})}document.body.appendChild(O),ee()}function oe(g,y,x){const _={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},v=document.createElement("div");v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const k=(I,S,j)=>`<div style="text-align:center">
      <div style="font-size:9px;color:${S};letter-spacing:2px;text-transform:uppercase;font-weight:700;margin-bottom:6px">${j}</div>
      <div style="width:70px;height:70px;border-radius:50%;background:${_[I.job]||"#555"};border:3px solid ${S};position:relative;overflow:hidden;margin:0 auto">
      ...
      <div style="width:min(90vw,420px)">${ct(u[l+"Team"],u[l+"Formation"],null,[],300,300)}</div>
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(g),(y=g.querySelector("#pvp-opp-close"))==null||y.addEventListener("click",()=>g.remove())}function K(){var _;const g=u.log||[],y=document.createElement("div");y.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column";const x=v=>{var L,E,I;if(v.type==="duel"){const S=v.isGoal,j=v.homeScored&&o==="p1"||!v.homeScored&&S&&o==="p2",O=v.homeScored?"#FFD700":S?"#ff6b6b":"rgba(255,255,255,0.3)",ee=S?j?"⚽ BUT !":"⚽ BUT adversaire !":(L=v.homePlayers)!=null&&L.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${S?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${O};margin-bottom:4px">
          <div style="font-size:9px;color:${O};letter-spacing:1px;margin-bottom:5px;font-weight:700;text-transform:uppercase">${ee}</div>
      ...
      </div>`,document.body.appendChild(y),(_=y.querySelector("#pvp-hist-close"))==null||_.addEventListener("click",()=>y.remove())}async function te(){if(u.phase!==o+"-attack")return;const g=o==="p1"?"p2":"p1",y=(u.round||0)+1,x=[...u.log||[]];x.push({type:"info",text:`⏭️ ${u[o+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const _=ke(u),v=be(g),k=_||!v?"finished":g+"-attack";await Z({["selected_"+o]:[],modifiers:{...u.modifiers,[o]:{}},pendingAttack:null,phase:k,attacker:g,round:y,log:x}),k==="finished"&&await je(u)}async function Y(){const g=u[o+"Team"],y=!["GK","DEF","MIL","ATT"].some(k=>(u[l+"Team"][k]||[]).some(L=>!L.used)),x=(u["selected_"+o]||[]).map(k=>{const L=(g[k._role]||[]).find(ee=>ee.cardId===k.cardId)||k,E=y&&["GK","DEF"].includes(k._role),I=g[k._role]||[],S=I.findIndex(ee=>ee.cardId===k.cardId),j=st(I.length),O=S>=0?j[S]:L._col??1;return{...L,_line:k._role,_col:O,...E?{note_a:Math.max(1,Number(L.note_a)||0)}:{}}});if(!x.length)return;const _=Ut(x,u.modifiers[o]||{});D(o,x.map(k=>k.cardId)),x.forEach(k=>{const L=(g[k._role]||[]).find(E=>E.cardId===k.cardId);L&&(L.used=!0)}),u["selected_"+o]=[],ue();const v=[...u.log||[]];if(y){const k=(u[o+"Score"]||0)+1,L=x.map(O=>({name:O.name,note:ze(O,O._line||"ATT"),portrait:Pe(O),job:O.job}));v.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:L,homeTotal:_.total,aiTotal:0});const E=(u.round||0)+1,I=o==="p1"?"p2":"p1",S={...u,[o+"Team"]:g,[o+"Score"]:k},j=ke(S);B.add(E),Le(L,k,u[l+"Score"]||0,!0,async()=>{await Z({[o+"Team"]:g,[o+"Score"]:k,["selected_"+o]:[],modifiers:{...u.modifiers,[o]:{}},pendingAttack:null,phase:j?"finished":I+"-attack",attacker:I,round:E,log:v}),j&&await je({...u,[o+"Score"]:k})});return}v.push({type:"pending",text:`⚔️ ${u[o+"Name"]} attaque (${_.total})`}),await Z({[o+"Team"]:g,[l+"Team"]:u[l+"Team"],pendingAttack:{..._,players:x,side:o},["selected_"+o]:[],modifiers:{...u.modifiers,[o]:{}},phase:l+"-defense",log:v})}async function de(){const g=u[o+"Team"],y=(u["selected_"+o]||[]).map(H=>{const X=(g[H._role]||[]).find(he=>he.cardId===H.cardId)||H,ae=g[H._role]||[],pe=ae.findIndex(he=>he.cardId===H.cardId),ce=st(ae.length),ge=pe>=0?ce[pe]:X._col??1;return{...X,_line:H._role,_col:ge}}),x=Kt(y,u.modifiers[o]||{});D(o,y.map(H=>H.cardId)),y.forEach(H=>{const X=(g[H._role]||[]).find(ae=>ae.cardId===H.cardId);X&&(X.used=!0)}),u["selected_"+o]=[],ue();const _=Vt(u.pendingAttack.total,x.total,u.modifiers[o]||{}),v=u.pendingAttack.side,k=_==="attack"||(_==null?void 0:_.goal),L=v==="p1"?"p2":"p1",E=(u.round||0)+1,I=(u.pendingAttack.players||[]).map(H=>({name:H.name,note:ze(H,H._line||"ATT"),portrait:Pe(H),job:H.job})),S=[...u.log||[]];S.push({type:"duel",isGoal:k,homeScored:k&&v===o,text:k?`⚽ BUT de ${u[v+"Name"]} ! (${u.pendingAttack.total} vs ${x.total})`:`✋ Attaque stoppée (${u.pendingAttack.total} vs ${x.total})`,homePlayers:I,aiPlayers:y.map(H=>({name:H.name,note:ze(H,H._line||"DEF"),portrait:Pe(H),job:H.job})),homeTotal:u.pendingAttack.total,aiTotal:x.total});const j=k?(u[v+"Score"]||0)+1:u[v+"Score"]||0,O={...u,[o+"Team"]:g,[v+"Score"]:j},ee=ke(O),R=ee?"finished":L+"-attack",F=async()=>{await Z({[o+"Team"]:g,[l+"Team"]:u[l+"Team"],[v+"Score"]:j,["selected_"+o]:[],modifiers:{...u.modifiers,[o]:{}},pendingAttack:null,phase:R,attacker:L,round:E,log:S}),(R==="finished"||ee)&&await je({...u,[v+"Score"]:j})};if(k){const H=v===o,X=H?j:u[o+"Score"]||0,ae=H?u[l+"Score"]||0:j;B.add(E),Le(I,X,ae,H,F)}else await F()}function ve(g){return["MIL","ATT"].some(y=>(g[y]||[]).some(x=>!x.used))}function Te(g){return["GK","DEF","MIL","ATT"].some(y=>(g[y]||[]).some(x=>!x.used))}function $e(g){return Te(g)&&!ve(g)}function be(g){const y=u[g+"Team"],x=u[(g==="p1"?"p2":"p1")+"Team"];return!!(ve(y)||!Te(x)&&$e(y))}function ke(g){const y=["MIL","ATT"].some(S=>(g.p1Team[S]||[]).some(j=>!j.used)),x=["MIL","ATT"].some(S=>(g.p2Team[S]||[]).some(j=>!j.used)),_=Te(g.p1Team),v=Te(g.p2Team);return!y&&!(!v&&_)&&(!x&&!(!_&&v))}function qe(g){const y=g.p1Score||0,x=g.p2Score||0;return y===x?null:y>x?$.home_id:$.away_id}async function je(g){try{const y=qe(g),x=y?$.home_id===y?$.away_id:$.home_id:null,_=g.p1Score||0,v=g.p2Score||0;await A.from("matches").update({status:"finished",winner_id:y,home_score:_,away_score:v}).eq("id",i);const{data:k}=await A.from("mini_league_matches").select("id, league_id, matchday").eq("match_id",i).single();if(k){await A.from("mini_league_matches").update({home_score:_,away_score:v,status:"finished"}).eq("id",k.id);const{data:L}=await A.from("mini_league_matches").select("id, status").eq("league_id",k.league_id).eq("matchday",k.matchday);if((L||[]).every(I=>I.status==="finished"||I.status==="bye")){const{data:I}=await A.from("mini_leagues").select("current_day,total_days").eq("id",k.league_id).single();if(I){const S=(I.current_day||0)+1,j=S>(I.total_days||0);await A.from("mini_leagues").update({current_day:j?I.total_days:S,status:j?"finished":"active"}).eq("id",k.league_id)}}}y&&x&&Ai(y,x).catch(()=>{})}catch(y){console.error("[ML] finishMatch:",y)}}function w(){var v;if(m&&document.getElementById("pvp-end-overlay"))return;m=!0;const g=u[o+"Score"],y=u[l+"Score"],x=g>y,_=g===y;et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
    ...
    </div>`,(v=document.getElementById("pvp-home"))==null||v.addEventListener("click",()=>{try{A.removeChannel(J)}catch{}Ve(e),c("mini-league",a?{openLeagueId:a}:{})})}ue()}const ur="/",fr=[{emoji:"⚽",title:"Bienvenue dans Manager Wars !",color:"#1A6B3C",content:`<p>Tu es désormais un <strong>manager de football</strong> virtuel.</p>
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div id="mkt-content" style="padding:10px 16px;display:flex;flex-direction:column;gap:8px"></div>
  </div>`;let f="buy";const a=()=>{var p,u,m,T,M,G,q;return{name:(((p=document.getElementById("flt-name"))==null?void 0:p.value)||"").toLowerCase().trim(),club:(((u=document.getElementById("flt-club"))==null?void 0:u.value)||"").toLowerCase().trim(),country:(((m=document.getElementById("flt-country"))==null?void 0:m.value)||"").toLowerCase().trim(),job:((T=document.getElementById("flt-job"))==null?void 0:T.value)||"",rarity:((M=document.getElementById("flt-rarity"))==null?void 0:M.value)||"",note1:parseInt((G=document.getElementById("flt-note1"))==null?void 0:G.value)||0,note2:parseInt((q=document.getElementById("flt-note2"))==null?void 0:q.value)||0}};function o(p){const u=a();return p.filter(m=>{var J,Z,se;const T=(J=m.card)==null?void 0:J.player;if(!T)return!1;const M=`${T.firstname} ${T.surname_encoded}`.toLowerCase(),G=(((Z=T.clubs)==null?void 0:Z.encoded_name)||"").toLowerCase(),q=(T.country_code||"").toLowerCase(),B=((se=m.card)==null?void 0:se.evolution_bonus)||0,C=At(T,T.job,B),N=T.job2?At(T,T.job2,B):0;return!(u.name&&!M.includes(u.name)||u.club&&!G.includes(u.club)||u.country&&!q.includes(u.country)||u.job&&T.job!==u.job||u.rarity&&T.rarity!==u.rarity||u.note1&&C<u.note1||u.note2&&N<u.note2)})}function l(p){var q,B,C,N;const u=(q=p.card)==null?void 0:q.player;if(!u)return"";const m=((B=p.card)==null?void 0:B.evolution_bonus)||0,T=At(u,u.job,m),M=u.job2?At(u,u.job2,m):0,G=(i.profile.credits||0)>=p.price;return`<div class="card-panel" style="display:flex;align-items:center;gap:8px;padding:10px 12px;overflow:hidden;padding-left:6px">
      ${Yi(u.rarity)}
      ${Xt(T,u.job)}
      ${Xt(M,u.job2||null)}
      ${Wi(u.country_code)}
      ...
      <div style="text-align:right;flex-shrink:0">
        ...
        <button class="btn btn-primary btn-sm" data-buy="${p.id}" ${G?"":"disabled"} style="margin-top:4px;font-size:11px;padding:4px 10px">${G?"Acheter":"Trop cher"}</button>
      </div>
    </div>`}function b(p){var q,B,C,N;const u=(q=p.card)==null?void 0:q.player;if(!u)return"";const m=((B=p.card)==null?void 0:B.evolution_bonus)||0,T=At(u,u.job,m),M=u.job2?At(u,u.job2,m):0,G=p.status==="sold";return`<div class="card-panel" style="display:flex;align-items:center;gap:8px;padding:10px 12px;overflow:hidden;padding-left:6px;${G?"opacity:0.7":""}">
      ${Yi(u.rarity)}
      ${Xt(T,u.job)}
      ${Xt(M,u.job2||null)}
      ${Wi(u.country_code)}
    ...
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center">
    ...
    </div>`,document.body.appendChild(d);const s=c=>{d.remove(),c&&t()};d.querySelector("#buy-cancel").addEventListener("click",()=>s(!1)),d.querySelector("#buy-ok").addEventListener("click",()=>s(!0)),d.addEventListener("click",c=>{c.target===d&&s(!1)})}async function vr(e,t,i){const{toast:n}=i,{data:d}=await A.from("market_listings").select("card_id").eq("id",e).single();if(await A.from("market_listings").update({status:"cancelled"}).eq("id",e),d!=null&&d.card_id){const{count:s}=await A.from("market_listings").select("id",{count:"exact",head:!0}).eq("card_id",d.card_id).eq("status","active");s||await A.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",d.card_id)}n("Annonce retirée","success"),Li(t,i)}async function wr(e,t){var o,l,b,h;const{state:i,navigate:n}=t,d=((l=(o=t==null?void 0:t.state)==null?void 0:o.params)==null?void 0:l.tab)||"global";e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const[{data:s},{data:r}]=await Promise.all([A.from("users").select("id,pseudo,club_name,trophies_top1,trophies_top2,trophies_top3,wins,level").order("trophies_top1",{ascending:!1}).limit(100),A.from("users").select("id,pseudo,club_name,mmr,rank_tier,ranked_wins,ranked_losses,ranked_draws,placement_matches").gte("placement_matches",1).order("mmr",{ascending:!1}).limit(100)]);let c=d;function f(){var p,u;const $=document.getElementById("rankings-list");if($){if(c==="global"){const m=s||[];$.innerHTML=m.length>0?m.map((T,M)=>`
      ...
      `).join(""):'<div style="text-align:center;color:var(--gray-600);padding:40px">Aucun manager.</div>'}else{const m=r||[];$.innerHTML=m.length>0?m.map((T,M)=>{const G=Pt(T.mmr??1e3),q=(T.ranked_wins||0)+(T.ranked_losses||0)+(T.ranked_draws||0),B=q>0?Math.round((T.ranked_wins||0)/q*100):0,C=T.id===i.profile.id,N=(T.placement_matches||0)<10;return`
          <div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${C?"border:2px solid var(--yellow)":""}">
            ...
            <div style="flex:1;min-width:0">
              ...
              <div style="font-size:11px;color:var(--gray-600)">${T.club_name} · ${G.label}</div>
            </div>
            <div style="text-align:right;flex-shrink:0">
  ...
  <div class="page">
  ...
  </div>`,f(),(b=document.getElementById("tab-global"))==null||b.addEventListener("click",()=>{c="global",f()}),(h=document.getElementById("tab-ranked"))==null||h.addEventListener("click",()=>{c="ranked",f()})}async function _r(e,t){var B,C,N,J;const{state:i,navigate:n,toast:d}=t,s=i.profile;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const[{data:r},{data:c}]=await Promise.all([A.from("ranked_seasons").select("*").eq("is_active",!0).maybeSingle(),A.from("users").select("id,pseudo,club_name,mmr,mmr_deviation,mmr_volatility,rank_tier,placement_matches,ranked_wins,ranked_losses,ranked_draws").eq("id",s.id).single()]);if(!c){d("Erreur chargement profil","error"),n("home");return}if(!r){e.innerHTML=`
    ...
    </div>`,(B=document.getElementById("ranked-back"))==null||B.addEventListener("click",()=>n("home"));return}const f=c.mmr??1e3,a=c.mmr_deviation??350,o=c.mmr_volatility??.06,l=c.placement_matches??0,b=l<10,h=Math.max(0,10-l),$=Pt(f),p=Lo(f),u=ci.findIndex(Z=>Z.id===$.id),m=ci[u+1]||null,T={bronze:"linear-gradient(160deg,#3d1c00,#7a3e00)",silver:"linear-gradient(160deg,#1a1a2e,#3a3a5e)",gold:"linear-gradient(160deg,#1a1200,#4a3500)",platinum:"linear-gradient(160deg,#001a20,#003040)",diamond:"linear-gradient(160deg,#001030,#1a2860)",master:"linear-gradient(160deg,#1a0030,#3d0070)"},M=(c.ranked_wins||0)+(c.ranked_losses||0)+(c.ranked_draws||0),G=M>0?Math.round((c.ranked_wins||0)/M*100):0,q=ci.map(Z=>`
  ...
  `).join("");e.innerHTML=`
  <div style="min-height:100%;background:${T[$.id]};padding:16px;overflow-y:auto;display:flex;flex-direction:column;gap:16px">
  ...
  </div>`,(C=document.getElementById("ranked-back"))==null||C.addEventListener("click",()=>n("home")),(N=document.getElementById("ranked-leaderboard-btn"))==null||N.addEventListener("click",()=>n("rankings",{tab:"ranked"})),(J=document.getElementById("ranked-play-btn"))==null||J.addEventListener("click",()=>{n("match",{matchMode:"ranked",rankedData:{mmr:f,rd:a,sigma:o,isPlacement:b}})})}async function $r(e,{state:t,navigate:i,toast:n}){const d=t.profile;if(!d)return;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:s}=await A.from("matches").select(`id,home_id,away_id,home_score,away_score,status,mode,winner_id,created_at,played_at,
      home:users!home_id(pseudo,club_name),
      away:users!away_id(pseudo,club_name)`).or(`home_id.eq.${d.id},away_id.eq.${d.id}`).order("created_at",{ascending:!1}).limit(50),r={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},c=(s||[]).filter(o=>o.status==="finished"),f=(s||[]).filter(o=>o.status==="in_progress");function a(o){const l=o.home_id===d.id;l?o.home_score:o.away_score,l?o.away_score:o.home_score;const b=o.winner_id===d.id,h=o.home_score===o.away_score&&o.status==="finished",$=o.status!=="finished"?"…":h?"N":b?"V":"D",p=o.status!=="finished"||h?"#888":b?"#1A6B3C":"#c0392b";let u=r[o.mode]||o.mode;o.away_id===null&&!u.startsWith("IA")&&(u="IA");const T=o.home_id===d.id?o.away:o.home;let M;o.away_id===null?M=u:T?M=`${T.club_name||T.pseudo} (${T.pseudo})`:M="Adversaire";let G="";o.status==="in_progress"&&Date.now()-new Date(o.created_at).getTime()>3600*1e3&&(G=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const q=new Date(o.created_at),B=q.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+q.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),C=o.status==="finished"?`${o.home_score} - ${o.away_score}`:`${o.home_score||0} - ${o.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
        <div style="font-size:13px;font-weight:600">${M}${G}</div>
  ...
  `,document.querySelectorAll(".bottom-nav a").forEach(d=>{d.addEventListener("click",s=>{s.preventDefault(),Gt(d.dataset.page)})}),document.getElementById("nav-logo").addEventListener("click",()=>Gt("home")),document.getElementById("nav-credits").addEventListener("click",()=>Gt("boosters")),(n=document.getElementById("journal-btn"))==null||n.addEventListener("click",()=>Tr())}async function Tr(){const{data:e}=await A.from("patch_notes").select("*").eq("is_published",!0).order("published_at",{ascending:!1}).limit(20),t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9000;display:flex;align-items:center;justify-content:center;padding:16px";const i=(e||[]).map(n=>{const d=new Date(n.published_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"long",year:"numeric"});return`<div style="padding:14px 0;border-bottom:1px solid #f0f0f0">
    ...
    </div>`,document.body.appendChild(t),t.querySelector("#journal-close").addEventListener("click",()=>t.remove()),t.addEventListener("click",n=>{n.target===t&&t.remove()})}async function Ar(){document.documentElement.setAttribute("data-theme","light"),document.getElementById("modal-overlay").addEventListener("click",i=>{i.target===i.currentTarget&&wi()}),document.getElementById("modal-close").addEventListener("click",wi);const{data:{session:e}}=await A.auth.getSession();if(!e){Xi(),Ci(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:zt});return}De.user=e.user,await Bt(),Xi();try{const{data:i}=await A.from("formation_links_overrides").select("formation, links"),n={};(i||[]).forEach(d=>{n[d.formation]=d.links}),Mo(n)}catch(i){console.warn("Impossible de charger les overrides de formation:",i)}if(!De.profile){qo(document.getElementById("app"),{state:De,navigate:async()=>{await Bt(),fi()},toast:zt,refreshProfile:Bt});return}const t=Array.isArray(De.profile.pending_boosters)?De.profile.pending_boosters:[];if(!De.profile.onboarding_done&&t.length>0){kn(document.getElementById("app"),{state:De,navigate:()=>fi(),toast:zt,refreshProfile:Bt});return}fi(),setTimeout(()=>mr(De.profile,Gt,zt),800),A.auth.onAuthStateChange(async(i,n)=>{i==="SIGNED_OUT"&&(De.user=null,De.profile=null,document.getElementById("app").innerHTML="",Ci(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:zt}))})}function zr(){return Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight)}function li(){const e=document.getElementById("app");e&&(e.style.height=zr()+"px")}window.addEventListener("resize",li);window.addEventListener("orientationchange",()=>setTimeout(li,150));window.visualViewport&&window.visualViewport.addEventListener("resize",li);function fi(){const e=()=>{var i;(i=De.user)!=null&&i.id&&A.from("users").update({last_seen_at:new Date().toISOString()}).eq("id",De.user.id).then(()=>{})};e(),window._presencePingInterval&&clearInterval(window._presencePingInterval),window._presencePingInterval=setInterval(e,6e4);const t=document.getElementById("app");t.style.display="flex",t.style.flexDirection="column",li(),Er(),zo()}function Xi(){const e=document.getElementById("app-loader"),t=document.getElementById("app");t&&(t.style.display=""),e&&(e.classList.add("zoom-out"),setTimeout(()=>e.style.display="none",500))}function So(e){var n;const t=document.getElementById("app-loader");if(t&&(t.style.display="none"),document.getElementById("boot-error"))return;const i=document.createElement("div");i.id="boot-error",i.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:99999;gap:16px;color:#fff;padding:24px;text-align:center",i.innerHTML=`
