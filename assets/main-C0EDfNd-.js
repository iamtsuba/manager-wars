...
import{s as w,F as si,r as Ae,j as St,l as Pi,m as yt,h as di,n as gn,o as mn,T as Yt,k as xn,p as yn,b as bn}from"./formation-links-D0diUT_D.js";const hn="/";function Li(e,{navigate:t,toast:i}){let n="login";const a=()=>{var r,s,d,l,c,u;const o=n==="login";e.innerHTML=`
  ...
  `;function s(){var f;const l=document.getElementById("logo-preview"),c=document.getElementById("logo-initials"),u=((f=document.getElementById("setup-club"))==null?void 0:f.value)||"MW",g=u.trim().split(" ").filter(Boolean),x=g.length>=2?(g[0][0]+g[1][0]).toUpperCase():u.slice(0,2).toUpperCase();l&&(l.style.background=r,l.style.borderColor=o),c&&(c.textContent=x,c.style.color=o)}document.getElementById("color1").addEventListener("input",l=>{o=l.target.value,document.getElementById("swatch1").style.background=o,s()}),document.getElementById("color2").addEventListener("input",l=>{r=l.target.value,document.getElementById("swatch2").style.background=r,s()});function d(l){document.querySelectorAll(".setup-step").forEach(c=>c.classList.remove("active")),document.getElementById(`step-${l}`).classList.add("active"),document.getElementById("step-num").textContent=l,document.getElementById("progress-fill").style.width=`${Math.round(l/3*100)}%`,l===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const l=document.getElementById("setup-pseudo").value.trim(),c=document.getElementById("step1-error");if(c.textContent="",l.length<3){c.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:u}=await w.from("users").select("id").eq("pseudo",l).maybeSingle();if(u){c.textContent="Ce pseudo est déjà pris.";return}d(2)}),document.getElementById("step2-back").addEventListener("click",()=>d(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const l=document.getElementById("setup-club").value.trim(),c=document.getElementById("step2-error");if(c.textContent="",l.length<2){c.textContent="Nom trop court (min. 2 caractères).";return}const{data:u}=await w.from("users").select("id").eq("club_name",l).maybeSingle();if(u){c.textContent="Ce nom de club est déjà pris.";return}d(3)}),document.getElementById("step3-back").addEventListener("click",()=>d(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const l=document.getElementById("setup-pseudo").value.trim(),c=document.getElementById("setup-club").value.trim(),u=document.getElementById("step3-error"),g=document.getElementById("step3-finish");u.textContent="",g.disabled=!0,g.textContent="Création en cours…";try{const{error:x}=await w.from("users").insert({id:t.user.id,pseudo:l,club_name:c,club_color1:o,club_color2:r,credits:1e4});if(x)throw x;await wn(t.user.id),await a(),n(`Bienvenue ${l} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(x){u.textContent=x.message,g.disabled=!1,g.textContent="🚀 Créer mon profil !"}})}async function wn(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await w.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const _n="modulepreload",kn=function(e){return"/"+e},Ti={},Ri=function(t,i,n){let a=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const r=document.querySelector("meta[property=csp-nonce]"),s=(r==null?void 0:r.nonce)||(r==null?void 0:r.getAttribute("nonce"));a=Promise.allSettled(i.map(d=>{if(d=kn(d),d in Ti)return;Ti[d]=!0;const l=d.endsWith(".css"),c=l?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${c}`))return;const u=document.createElement("link");if(u.rel=l?"stylesheet":_n,l||(u.as="script"),u.crossOrigin="",u.href=d,s&&u.setAttribute("nonce",s),document.head.appendChild(u),l)return new Promise((g,x)=>{u.addEventListener("load",g),u.addEventListener("error",()=>x(new Error(`Unable to preload CSS for ${d}`)))})}))}function o(r){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=r,window.dispatchEvent(s),!s.defaultPrevented)throw r}return a.then(r=>{for(const s of r||[])s.status==="rejected"&&o(s.reason);return t().catch(o)})},Et="#1A6B3C",Lt="#cc2222",$n="#D4A017",zi="#888";async function En(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    </style>`}const In="2026.07.12-1840";async function Ii(e,{state:t,navigate:i,toast:n}){var o,r;const a=t.profile;a&&(e.innerHTML=`
  <style>
    .home-dark {
      min-height: 100%;
  ...
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{N=!N,I(),k()})))}function k(){const T=document.getElementById("col-grid");T&&(C==="player"?Le(T):C==="formation"?K(T):C==="stadium"?V(T):Q(T))}function F(T,S,R,H,U){const j=document.getElementById("col-grid"),O=document.getElementById("col-big");if(!j||!O)return;var $e=0;function _e(){const we=window.innerWidth>=768,me=document.getElementById("col-big"),Ee=document.getElementById("col-grid");we&&me&&(me.style.minHeight="420px",me.style.flex="1 1 auto"),we&&Ee&&(Ee.style.height=Math.round(310*.76+16)+"px",Ee.style.flexShrink="0",Ee.style.overflowX="auto",Ee.style.overflowY="hidden",Ee.style.alignItems="center",Ee.style.padding="8px 16px"),O.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+S(T[$e])+"</div>";var ke=O.querySelector("[data-card-id],[data-form-id],[data-gc-id]");ke&&ke.addEventListener("click",function(){H(T[$e])}),requestAnimationFrame(function(){var he=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!he||!O)){var Ce=O.clientHeight-8,je=O.clientWidth-24,Fe=he.offsetHeight,_=he.offsetWidth;if(Fe>0&&_>0&&Ce>40){var L=we?2.2:1.6,A=Math.min(Ce/Fe,je/_,L);he.style.transform="scale("+A.toFixed(3)+")",he.style.transformOrigin="top center"}}}),j.innerHTML=T.map(function(he,Ce){var je=Ce===$e,Fe="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+(je?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+Ce+'" style="'+Fe+'">'+R(he,je)+"</div>"}).join(""),j.querySelectorAll(".col-mini-item").forEach(function(he){he.addEventListener("click",function(){$e=Number(he.dataset.idx),_e(),he.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}_e()}function se(T){var S=window.innerWidth>=768?.76:.54,R;if(!T||T._fake){var H=T?T.player:null;if(!H)return"";R=Mi(H)}else R=Jt(T,"");return'<div style="display:inline-block;zoom:'+S+';pointer-events:none;line-height:0">'+R+"</div>"}function ue(T,S,R){S=S||100,R=R||140;var H=St[T]||{},U={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},j=Math.max(3,Math.round(S*.06)),O=Object.entries(H).map(function(_e){var we=_e[0],me=_e[1],Ee=we.replace(/\d+$/,""),ke=U[Ee]||"#888",he=Math.round(me.x*S),Ce=Math.round(me.y*R);return'<circle cx="'+he+'" cy="'+Ce+'" r="'+j+'" fill="'+ke+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),$e=Math.max(1,Math.round(S/50));return'<svg viewBox="0 0 '+S+" "+R+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+S+'" height="'+R+'" fill="#1A6B3C"/><rect x="'+Math.round(S*.2)+'" y="'+Math.round(R*.02)+'" width="'+Math.round(S*.6)+'" height="'+Math.round(R*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+$e+'"/><line x1="0" y1="'+Math.round(R*.5)+'" x2="'+S+'" y2="'+Math.round(R*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+$e+'"/><ellipse cx="'+Math.round(S*.5)+'" cy="'+Math.round(R*.5)+'" rx="'+Math.round(S*.18)+'" ry="'+Math.round(R*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+$e+'"/><rect x="'+Math.round(S*.2)+'" y="'+Math.round(R*.82)+'" width="'+Math.round(S*.6)+'" height="'+Math.round(R*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+$e+'"/>'+O+"</svg>"}function ce(T,S,R){var H=R>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+R+"</div>":"",U=S?'data-form-id="'+S.id+'"':"",j=T.length>7?14:T.length>5?16:19,O=!!S;return"<div "+U+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(O?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(O?"":"filter:grayscale(1);opacity:0.5")+'">'+H+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(O?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+j+"px;font-weight:900;color:"+(O?"#1A6B3C":"#aaa")+';line-height:1">'+T+'</div></div><div style="flex:1;overflow:hidden;background:'+(O?"#1A6B3C":"#ccc")+'">'+ue(T,140,220)+"</div></div>"}function de(T,S){var R=window.innerWidth>=768?.76:.54,H=140,U=305,j=Math.round(U*.22),O=U-j,$e=T.length>7?10:13,_e=ue(T,H,O),we=S?"1.5px solid #2a7a40":"1px solid #ddd",me=S?"":"filter:grayscale(1);opacity:0.45;",Ee=S?"#1A6B3C":"#bbb",ke="#fff";return'<div style="display:inline-block;zoom:'+R+';line-height:0;pointer-events:none"><div style="width:'+H+"px;height:"+U+"px;border-radius:6px;border:"+we+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+me+'"><div style="height:'+j+"px;background:"+Ee+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+$e+"px;font-weight:900;color:"+ke+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(H-4)+'px">'+T+'</span></div><div style="height:'+O+'px;overflow:hidden;flex-shrink:0">'+_e+"</div></div></div>"}function Le(T){if(N){const S=q();if(!S.length){T.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const R=S.map(H=>l.find(U=>U.player.id===H.id)||{_fake:!0,player:H,id:"fake-"+H.id});F(R,H=>H._fake?Mi(H.player):Jt(H,""),H=>H._fake?se({player:H.player,id:"x",_fake:!0}):se(H),H=>{H._fake||Ci(H,l,h,t)})}else{const S=ee();if(!S.length){T.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const R={};S.forEach(U=>{const j=U.player.id;R[j]||(R[j]=[]),R[j].push(U)});const H=Object.values(R).map(U=>Pn(U));F(H,U=>{const j=h[U.player.id]||1,O=j>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${j}</div>`:"",_e=l.filter(we=>we.player.id===U.player.id&&we.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Jt(U,O+_e)},U=>se(U),U=>Ci(U,l,h,t))}}function K(T){const S=N?b:[...$];if(!S.length){T.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const R=S.map(H=>({formation:H,card:u.find(U=>U.formation===H)||null,owned:$.has(H)}));F(R,({formation:H,card:U,owned:j})=>ce(H,j?U:null,j?u.filter(O=>O.formation===H).length:0),({formation:H,owned:U})=>de(H,U),({card:H,owned:U})=>{U&&H&&Kn(H,u,t,o)})}function Q(T){const S=Object.keys(f),R=N?S.length?S:v:[...y];if(!R.length){T.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const H=R.map(U=>({type:U,gc:Se[U]||{icon:"⚡",desc:""},def:f[U]||null,owned:y.has(U),card:c.find(j=>j.gc_type===U)||null}));F(H,({type:U,gc:j,def:O,owned:$e,card:_e})=>{var L;const we=$e?c.filter(A=>A.gc_type===U).length:0,me=we>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${we}</div>`:"",Ee=(O==null?void 0:O.gc_type)==="ultra_game_changer",ke={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},he={purple:"#b06ce0",light_blue:"#00d4ef"},Ce=ke[O==null?void 0:O.color]||ke.purple,je=he[O==null?void 0:O.color]||he.purple,Fe=(O==null?void 0:O.effect)||j.desc||"",_=O!=null&&O.image_url?`/icons/${O.image_url}`:((L=O==null?void 0:O.club)==null?void 0:L.logo_url)||(O!=null&&O.country_code?`https://flagsapi.com/${O.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null);return $e&&_e?`<div data-gc-id="${_e.id}" data-gc-type="${U}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${je};background:${Ce};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${je}66;cursor:pointer">
          ${me}
        ...
        </div>`},({def:j,count:O})=>{var ke,he;const $e=window.innerWidth>=768?.76:.54,_e=(j==null?void 0:j.name)||"?",we=((ke=j==null?void 0:j.club)==null?void 0:ke.encoded_name)||(j==null?void 0:j.country_code)||"—",me=j!=null&&j.image_url?`${R}icons/${j.image_url}`:((he=j==null?void 0:j.club)==null?void 0:he.logo_url)||(j!=null&&j.country_code?`https://flagsapi.com/${j.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),Ee=me?`<img src="${me}" style="width:64px;height:64px;object-fit:contain;border-radius:4px" onerror="this.style.display='none'">`:'<span style="font-size:32px">🏟️</span>';return`<div style="display:inline-block;zoom:${$e};line-height:0;pointer-events:none">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(f=document.getElementById("direct-sell-form-btn"))==null||f.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${l} pour ${Bt.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const b=g.find(h=>!h.is_for_sale)||g[0];if(!b){o("Aucune carte à vendre","error");return}await w.from("market_listings").delete().eq("card_id",b.id),await w.from("transfer_history").delete().eq("card_id",b.id);const{error:v}=await w.from("cards").delete().eq("id",b.id);if(v){o(v.message,"error");return}await w.from("users").update({credits:(a.profile.credits||0)+Bt}).eq("id",a.profile.id),await d(),o(`+${Bt.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),r(),s("collection")}),(p=document.getElementById("market-sell-form-btn"))==null||p.addEventListener("click",async()=>{const b=parseInt(document.getElementById("sell-price-form").value);if(!b||b<1){o("Prix invalide","error");return}await w.from("cards").update({is_for_sale:!0,sale_price:b}).eq("id",e.id),await w.from("market_listings").insert({seller_id:a.profile.id,card_id:e.id,price:b}),o("Carte mise en vente sur le marché !","success"),r(),s("collection")}),(m=document.getElementById("cancel-sell-form-btn"))==null||m.addEventListener("click",async()=>{await w.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await w.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),o("Annonce retirée","success"),r(),s("collection")})}async function Ci(e,t,i,n){var se,ue,ce,de,Le;const{state:a,toast:o,openModal:r,closeModal:s,navigate:d,refreshProfile:l}=n,c=e.player,u=t.filter(K=>K.player.id===c.id),g=u.length,x=e.evolution_bonus||0,f=Math.max((Number(c.note_g)||0)+(c.job==="GK"||c.job2==="GK"?x:0),(Number(c.note_d)||0)+(c.job==="DEF"||c.job2==="DEF"?x:0),(Number(c.note_m)||0)+(c.job==="MIL"||c.job2==="MIL"?x:0),(Number(c.note_a)||0)+(c.job==="ATT"||c.job2==="ATT"?x:0)),p=c.rarity||"normal",{data:m}=await w.from("sell_price_configs").select("*").eq("rarity",p).lte("note_min",f).gte("note_max",f).order("note_min",{ascending:!1}).limit(1);((se=m==null?void 0:m[0])==null?void 0:se.price)??Gn[p];const b=c.rarity!=="legende";On(c);const v=((c.rarity==="pepite"||c.rarity==="papyte")&&e.current_note!=null?e.current_note:Xt(c,c.job))+x,h=c.rarity==="pepite"||c.rarity==="papyte",$=c.job2?(h?Bn(e,Rn(c.job2)):Xt(c,c.job2))+(Xt(c,c.job2)>0?x:0):null;Wt[c.job],c.job2&&Wt[c.job2];const y=Dn[c.rarity]||"#ccc";Nn[c.country_code]||c.country_code;const C=e.evolution_bonus||0,G=v+C,N=$||0,te=N>0?N+C:0,ne=u.map(K=>K.id);let ee={};if(ne.length){const{data:K}=await w.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",ne).order("transferred_at",{ascending:!0});(K||[]).forEach(Q=>{ee[Q.card_id]||(ee[Q.card_id]=[]),ee[Q.card_id].push(Q)})}const q=K=>{const Q=K.transferred_at?new Date(K.transferred_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"2-digit",year:"numeric"}):"",V=K.source==="booster"?"Booster":K.price?K.price.toLocaleString("fr"):"—";return`
    ...
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${u.map((K,Q)=>{const V=ee[K.id]||[],T=K.is_for_sale,S=V.length?V[V.length-1]:null,H=(c.rarity==="pepite"||c.rarity==="papyte")&&K.current_note!=null?` (☆${K.current_note})`:"";return`
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(ue=document.getElementById("close-detail"))==null||ue.addEventListener("click",s);let k=new Set;const F=()=>{const K=k.size,Q=document.getElementById("sell-action-panel");if(!Q)return;Q.style.display=K>0?"block":"none",document.getElementById("sell-selected-count").textContent=K;const V=document.getElementById("evolve-btn");V&&(V.textContent=`⬆️ Évoluer ${K>1?"(+"+K+")":""}`)};document.querySelectorAll(".expl-check").forEach(K=>{K.addEventListener("change",()=>{const Q=K.dataset.id;K.checked?k.add(Q):k.delete(Q);const V=K.closest(".exemplaire-row");V&&(V.style.borderColor=K.checked?"#1A6B3C":"#eee"),F()})}),document.querySelectorAll(".exemplaire-row").forEach(K=>{K.addEventListener("click",Q=>{if(Q.target.closest("button")||Q.target.tagName==="INPUT")return;const V=K.querySelector(".expl-check");V&&!V.disabled&&(V.checked=!V.checked,V.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(K=>{K.addEventListener("click",Q=>{Q.stopPropagation();const V=document.querySelector(`.expl-hist[data-hist="${K.dataset.idx}"]`);V&&(V.style.display=V.style.display==="none"?"flex":"none")})}),(ce=document.getElementById("evolve-btn"))==null||ce.addEventListener("click",async()=>{if(g<=1)return;const K=[...k];if(!K.length)return;if(k.has(e.id)){const U=document.createElement("div");U.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",U.innerHTML=`
  ...
  `,document.body.appendChild(a);let o=!1;const r=document.getElementById("pack-cut-zone"),s=document.getElementById("pack-blade");let d=!1;const l=I=>I.touches&&I.touches[0]?I.touches[0].clientX:I.clientX;function c(I){o||(d=!0,s.style.opacity="1",u(I))}function u(I){if(!d||o)return;const k=r.getBoundingClientRect(),F=l(I)-k.left,se=Math.max(0,Math.min(1,F/k.width));s.style.width=se*k.width+"px",se>=.82&&x()}function g(){o||(d=!1,s.style.transition="width .2s ease, opacity .2s ease",s.style.width="0",s.style.opacity="0",setTimeout(()=>{o||(s.style.transition="")},220))}function x(){var k;if(o)return;o=!0,d=!1,s.style.width="100%",s.style.opacity="1",(k=document.getElementById("cut-flash"))==null||k.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const I=document.getElementById("cut-hint");I&&(I.style.opacity="0"),r.classList.add("pack-cut"),setTimeout(()=>{s.style.opacity="0",document.getElementById("pack-phase").style.display="none",b(0)},620)}r.addEventListener("pointerdown",c),window.addEventListener("pointermove",u),window.addEventListener("pointerup",g),r.addEventListener("touchstart",c,{passive:!0}),window.addEventListener("touchmove",u,{passive:!0}),window.addEventListener("touchend",g);let f=0,p=!1;const m=new Set;function b(I){f=I,document.getElementById("reveal-phase").style.display="flex",v(),h(I,0),G()}function v(){const I=document.getElementById("card-dots");I&&(I.innerHTML=e.map((k,F)=>`<div class="card-dot" data-i="${F}" style="width:8px;height:8px;border-radius:50%;background:${F===f?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),I.querySelectorAll(".card-dot").forEach(k=>k.addEventListener("click",()=>y(parseInt(k.dataset.i)))))}function h(I,k){var V,T;const F=e[I],se=document.getElementById("card-counter"),ue=document.getElementById("card-track");se&&(se.textContent=`Carte ${I+1} / ${e.length}`);const ce=document.getElementById("reveal-btns");ce&&(ce.style.display=I===e.length-1?"flex":"none");const de=F.card_type==="player"&&((V=F.player)==null?void 0:V.rarity)==="legende",Le=!m.has(I);m.add(I);let K=0;if(F.card_type==="player"&&F.player){const S=F.player,R=S.job||"ATT";K=Number(R==="GK"?S.note_g:R==="DEF"?S.note_d:R==="MIL"?S.note_m:S.note_a)||0}const Q=S=>{ue.innerHTML=`
        ...
        </div>`;const R=document.getElementById("current-card-wrap");k!==0?(R.style.transition="none",R.style.transform=`translateX(${k>0?100:-100}%)`,requestAnimationFrame(()=>{R.style.transition="transform .28s cubic-bezier(.25,1,.5,1)",R.style.transform="translateX(0)"})):R.animate([{opacity:0,transform:"scale(.7)"},{opacity:1,transform:"scale(1)"}],{duration:300,easing:"cubic-bezier(.34,1.56,.64,1)"}),S||de?te():ne(),v()};Le&&(((T=F.player)==null?void 0:T.rarity)==="legende"||K>=8)&&F.card_type==="player"&&F.player?$(F,()=>Q(!0)):Q(!1)}function $(I,k){var we;p=!0;const F=I.player,se=`https://flagsapi.com/${F.country_code}/flat/64.png`,ue=(we=F.clubs)==null?void 0:we.logo_url,ce=F.face?"/"+F.face.replace(/^public\//,"").replace(/^\//,""):null,de=F.job||"ATT",Le=Number(de==="GK"?F.note_g:de==="DEF"?F.note_d:de==="MIL"?F.note_m:F.note_a)||0,K=I.evolution_bonus||0,Q=Le+K,V=F.rarity==="legende",T=Q>=18,S=document.getElementById("walkout-overlay"),R=document.getElementById("walkout-stage");if(!S||!R){p=!1,k();return}let H=null;V&&(H=new Audio("/sounds/Legendary.mp3"),H.volume=.8,H.play().catch(()=>{})),S.style.display="flex";const U=()=>{const me=R.firstElementChild;me&&(me.classList.remove("wo-in"),me.classList.add("wo-out"))},j=1800,O=400;R.innerHTML=`<img class="wo-in" src="${se}" style="height:130px;border-radius:10px;box-shadow:0 10px 36px rgba(0,0,0,.6)" onerror="this.style.display='none'">`,navigator.vibrate&&navigator.vibrate(30),setTimeout(U,j),setTimeout(()=>{var me;R.innerHTML=ue?`<img class="wo-in" src="${ue}" style="max-height:160px;max-width:210px;object-fit:contain">`:`<div class="wo-in" style="font-size:34px;font-weight:900;color:#fff;text-align:center">${((me=F.clubs)==null?void 0:me.encoded_name)||"CLUB"}</div>`,navigator.vibrate&&navigator.vibrate(30)},j+O),setTimeout(U,j*2+O),setTimeout(()=>{R.innerHTML=ce?`<img class="wo-in" src="${ce}" style="height:200px;border-radius:50%;box-shadow:0 0 40px rgba(255,255,255,0.3);object-fit:cover;object-position:top">`:'<div class="wo-in" style="font-size:80px">👤</div>',navigator.vibrate&&navigator.vibrate(30)},(j+O)*2),setTimeout(U,(j+O)*2+j);const _e=T?"#FFD700":{GK:"#c0c0c0",DEF:"#e03030",MIL:"#D4A017",ATT:"#3fbf5f"}[de]||"#fff";setTimeout(()=>{R.innerHTML=`<div class="wo-in" style="
        ...
        ${Q}
      </div>`,T&&navigator.vibrate&&navigator.vibrate([50,30,100,30,200])},(j+O)*3),setTimeout(U,(j+O)*3+j),setTimeout(()=>{S.style.display="none",R.innerHTML="",p=!1,H&&!V&&H.pause(),navigator.vibrate&&navigator.vibrate([40,30,80]),k()},(j+O)*4)}function y(I){if(p||I<0||I>=e.length||I===f)return;const k=I>f?1:-1;f=I,h(I,k)}function C(){y(f+1)}function E(){y(f-1)}function G(){const I=document.getElementById("card-viewport");if(!I||I._swipeBound)return;I._swipeBound=!0;let k=0,F=0,se=0,ue=!1;const ce=V=>V.touches?V.touches[0].clientX:V.clientX,de=V=>V.touches?V.touches[0].clientY:V.clientY,Le=V=>{ue=!0,k=ce(V),F=de(V),se=0},K=V=>{if(!ue)return;se=ce(V)-k;const T=de(V)-F;if(Math.abs(se)<Math.abs(T))return;const S=document.getElementById("current-card-wrap");S&&(S.style.transition="none",S.style.transform=`translateX(${se*.6}px) rotate(${se*.02}deg)`)},Q=()=>{if(!ue)return;ue=!1;const V=document.getElementById("current-card-wrap"),T=55;se<=-T&&f<e.length-1?C():se>=T&&f>0?E():V&&(V.style.transition="transform .2s ease",V.style.transform="translateX(0)")};I.addEventListener("pointerdown",Le),I.addEventListener("pointermove",K),I.addEventListener("pointerup",Q),I.addEventListener("pointercancel",Q),I.addEventListener("touchstart",Le,{passive:!0}),I.addEventListener("touchmove",K,{passive:!0}),I.addEventListener("touchend",Q),I.addEventListener("click",V=>{if(Math.abs(se)>8)return;const T=I.getBoundingClientRect();V.clientX-T.left>T.width/2?C():E()})}let N=null;function te(){const I=document.getElementById("fireworks-canvas");if(!I)return;I.width=window.innerWidth,I.height=window.innerHeight;const k=I.getContext("2d"),F=[];function se(){const ce=Math.random()*I.width,de=Math.random()*I.height*.6,Le=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],K=Le[Math.floor(Math.random()*Le.length)];for(let Q=0;Q<60;Q++){const V=Math.PI*2/60*Q,T=2+Math.random()*5;F.push({x:ce,y:de,vx:Math.cos(V)*T,vy:Math.sin(V)*T,alpha:1,color:K,size:2+Math.random()*3})}}se(),N=setInterval(se,600);function ue(){if(document.getElementById("fireworks-canvas")){k.clearRect(0,0,I.width,I.height);for(let ce=F.length-1;ce>=0;ce--){const de=F[ce];if(de.x+=de.vx,de.y+=de.vy+.08,de.vy*=.98,de.alpha-=.018,de.alpha<=0){F.splice(ce,1);continue}k.globalAlpha=de.alpha,k.fillStyle=de.color,k.beginPath(),k.arc(de.x,de.y,de.size,0,Math.PI*2),k.fill()}k.globalAlpha=1,(N!==null||F.length>0)&&requestAnimationFrame(ue)}}ue()}function ne(){N!==null&&(clearInterval(N),N=null);const I=document.getElementById("fireworks-canvas");I&&I.getContext("2d").clearRect(0,0,I.width,I.height)}if(n){const I=document.getElementById("reveal-btns");I&&(I.innerHTML='<button class="btn btn-primary" id="reveal-next" style="flex:1">Continuer →</button>'),(q=document.getElementById("reveal-next"))==null||q.addEventListener("click",()=>{ne(),a.remove(),n()})}else document.getElementById("reveal-collection").addEventListener("click",()=>{ne(),a.remove(),i("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{ne(),a.remove(),i("boosters")})}function ao(e){var t,i,n,a;if(e.card_type==="player"&&e.player)return to(e);if(e.card_type==="game_changer"){const o=e._gcDef,r=(o==null?void 0:o.gc_type)==="ultra_game_changer",s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},d={purple:"#b06ce0",light_blue:"#00d4ef"},l=s[o==null?void 0:o.color]||s.purple,c=d[o==null?void 0:o.color]||d.purple,u=(o==null?void 0:o.name)||e.gc_type||"Game Changer",g=(o==null?void 0:o.effect)||((t=ti[e.gc_type])==null?void 0:t.desc)||"",x=o!=null&&o.image_url?`/icons/${o.image_url}`:null,f=((i=ti[e.gc_type])==null?void 0:i.icon)||"⚡";return`<div style="width:170px;height:240px;background:${l};border-radius:14px;border:3px solid ${c};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${c}88;flex-shrink:0">
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    ${(()=>{const k=window.innerWidth>=700,F=(T,S,R)=>{var Ce,je;const H=(t.gcDefs||[]).find(Fe=>Fe.name===T.gc_type),U={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[H==null?void 0:H.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",j={purple:"#b06ce0",light_blue:"#00d4ef"}[H==null?void 0:H.color]||"#b06ce0",O=(H==null?void 0:H.name)||T.gc_type,$e=(H==null?void 0:H.effect)||((Ce=Se[T.gc_type])==null?void 0:Ce.desc)||"",_e=H!=null&&H.image_url?`/icons/${H.image_url}`:null,we=((je=Se[T.gc_type])==null?void 0:je.icon)||"⚡",me=Math.round(R*.22),Ee=Math.round(R*.22),ke=R-me-Ee,he=O.length>12?7:9;return`<div class="gc-mini" data-gc-id="${T.id}" data-gc-type="${T.gc_type}"
          style="box-sizing:border-box;width:${S}px;height:${R}px;border-radius:10px;border:2px solid ${j};background:${U};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
          ...
          </div>`},ue=(T,S)=>S?se(130,175):F(T,130,175),ce=(T,S)=>S?se(68,95):F(T,68,95),de=k?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",Le=f?`<button id="btn-results" style="${de};background:linear-gradient(135deg,#D4A017,#FFD700);border:none;color:#000">🏁 Résultats</button>`:u?`<div style="${de};background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);color:rgba(255,255,255,0.4)">⏳ Tour IA</div>`:b?`<button id="btn-pass" style="${de};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER (plus d'attaquants)</button>`:g?`<button id="btn-action" style="${de};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${t.selected.length===0?"disabled":""}> ⚔️ ATTAQUEZ <span id="match-timer" style="font-weight:900"></span></button>`:x?`<button id="btn-action" style="${de};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${t.selected.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="match-timer" style="font-weight:900"></span></button>`:`<div style="${de};background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1)"></div>`,K=g||x?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${t.selected.length}/3 sélectionné(s)</div>`:"",Q=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${k?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(k=>{var F,se,ue;if(k.type==="duel"){const ce=k.isGoal,de=k.homeScored?"#FFD700":ce?"#ff6b6b":"rgba(255,255,255,0.3)",Le=k.homeScored?"⚽ BUT !":ce?"⚽ BUT IA !":(F=k.homePlayers)!=null&&F.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${ce?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${de};margin-bottom:4px">
                <div style="font-size:9px;color:${de};letter-spacing:1px;margin-bottom:5px;font-weight:700;text-transform:uppercase">${Le}</div>
    ...
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      ...
      ${It(e.aiTeam,e.formation,null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.1))}
            ...
            </div>`}}catch(J){console.error("[Ranked] MMR update error:",J)}(X=document.getElementById("pvp-end-overlay"))==null||X.remove();const B=document.createElement("div");B.id="pvp-end-overlay",B.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;color:#fff;padding:24px;text-align:center;overflow-y:auto";const P=z?"🤝":M?"🏆":"😞",Y=z?"MATCH NUL":M?"VICTOIRE !":"DÉFAITE",W=z?"#fff":M?"#FFD700":"#ff6b6b";B.innerHTML=`
      ...
      ${D}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${r?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(B),(re=B.querySelector("#pvp-end-home"))==null||re.addEventListener("click",()=>{B.remove(),Ue(e),x(r?"ranked":"home")})}const ee=w.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},_=>{const L=_.new;try{if(L.status==="finished"||L.forfeit){if(C)return;C=!0,E&&(clearInterval(E),E=null),L.game_state&&(y=L.game_state),ne(L);return}if(L.game_state){const A=y;y=L.game_state;const M=y._lastGC;if(M&&M.seq&&!te.has(M.seq)&&(te.add(M.seq),M.by!==p)){V(M.type,M.by,()=>de());return}const z=A[p+"Score"]||0,D=A[m+"Score"]||0,B=y[p+"Score"]||0,P=y[m+"Score"]||0,Y=B>z,W=P>D;if((Y||W)&&!N.has(y.round)){N.add(y.round);const X=[...y.log||[]].reverse().find(J=>J.isGoal),re=((X==null?void 0:X.homePlayers)||[]).map(J=>({name:J.name,note:J.note,portrait:J.portrait,job:J.job}));st(re,B,P,Y,()=>de());return}de()}}catch(A){console.error("[PvP] crash:",A)}}).subscribe();async function q(_){Object.assign(y,_),y.lastActionAt=new Date().toISOString();const{error:L}=await w.from("matches").update({game_state:y}).eq("id",i);L&&f("Erreur de synchronisation","error");try{de()}catch(A){console.error("[PvP] renderPvpScreen crash:",A)}}async function I(){if(C)return;C=!0,E&&(clearInterval(E),E=null);const _=n?h.away_id:h.home_id,L=n?"p2":"p1",A=n?"p1":"p2",M={...y,[L+"Score"]:3,[A+"Score"]:0,phase:"finished"};try{await w.from("matches").update({status:"finished",forfeit:!0,winner_id:_,home_score:M.p1Score||0,away_score:M.p2Score||0,game_state:M}).eq("id",i)}catch{}try{w.removeChannel(ee)}catch{}setTimeout(()=>{Ue(e),x("home")},800)}const k={BOOST_STAT:({value:_=1,count:L=1,roles:A=[]},M,z)=>{const D=M[p+"Team"],B=Object.entries(D).filter(([P])=>!A.length||A.includes(P)).flatMap(([P,Y])=>Y.filter(W=>!W.used).map(W=>({...W,_line:P})));if(!B.length){M.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),z(M);return}F(B,L,`Choisir ${L} joueur(s) à booster (+${_})`,P=>{P.forEach(Y=>{const W=(D[Y._line]||[]).find(X=>X.cardId===Y.cardId);W&&(W.boost=(W.boost||0)+_,M.log.push({text:`⚡ +${_} sur ${W.name}`,type:"info"}))}),M[p+"Team"]=D,z(M)})},DEBUFF_STAT:({value:_=1,count:L=1,roles:A=[],target:M="ai"},z,D)=>{const B=M==="home"?p:m,P=z[B+"Team"],Y=M==="home"?"allié":"adverse",W=Object.entries(P).filter(([X])=>!A.length||A.includes(X)).flatMap(([X,re])=>re.map(J=>({...J,_line:X})));if(!W.length){z.log.push({text:`🎯 Aucun joueur ${Y}`,type:"info"}),D(z);return}F(W,L,`Choisir ${L} joueur(s) ${Y}(s) (-${_})`,X=>{X.forEach(re=>{const J=(P[re._line]||[]).find(le=>le.cardId===re.cardId);J&&(J.boost=(J.boost||0)-_,z.log.push({text:`🎯 -${_} sur ${J.name}`,type:"info"}))}),z[B+"Team"]=P,D(z)})},GRAY_PLAYER:({count:_=1,roles:L=[],target:A="ai"},M,z)=>{const D=A==="home"?p:m,B=M[D+"Team"],P=A==="home"?"allié":"adverse",Y=Object.entries(B).filter(([W])=>!L.length||L.includes(W)).flatMap(([W,X])=>X.filter(re=>!re.used).map(re=>({...re,_line:W})));if(!Y.length){M.log.push({text:`❌ Aucun joueur ${P}`,type:"info"}),z(M);return}F(Y,_,`Choisir ${_} joueur(s) ${P}(s) à exclure`,W=>{const X="usedCardIds_"+D,re=new Set(M[X]||[]);W.forEach(J=>{const le=(B[J._line]||[]).find(ae=>ae.cardId===J.cardId);le&&(le.used=!0,re.add(J.cardId),M.log.push({text:`❌ ${le.name} exclu !`,type:"info"}))}),M[X]=[...re],M[D+"Team"]=B,z(M)})},REVIVE_PLAYER:({count:_=1,roles:L=[]},A,M)=>{const z=A[p+"Team"],D=Object.entries(z).filter(([B])=>!L.length||L.includes(B)).flatMap(([B,P])=>P.filter(Y=>Y.used).map(Y=>({...Y,_line:B})));if(!D.length){A.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),M(A);return}F(D,_,`Choisir ${_} joueur(s) à ressusciter`,B=>{B.forEach(P=>{const Y=(z[P._line]||[]).find(W=>W.cardId===P.cardId);Y&&(Y.used=!1,A.log.push({text:`💫 ${Y.name} ressuscité !`,type:"info"}))}),A[p+"Team"]=z,M(A)})},REMOVE_GOAL:({},_,L)=>{const A=m+"Score";_[A]>0?(_[A]--,_.log.push({text:"🚫 Dernier but annulé !",type:"info"})):_.log.push({text:"🚫 Aucun but à annuler",type:"info"}),L(_)},ADD_GOAL_DRAW:({},_,L)=>{_[p+"Score"]===_[m+"Score"]?(_[p+"Score"]++,_.log.push({text:"🎯 But bonus !",type:"info"})):_.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),L(_)},ADD_SUB:({value:_=1},L,A)=>{L.maxSubs=(L.maxSubs||3)+_,L.log.push({text:`🔄 +${_} remplacement(s)`,type:"info"}),A(L)},CUSTOM:({},_,L)=>L(_)};function F(_,L,A,M){const z=document.createElement("div");z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let D=[];function B(){var Y,W;const P=_.map(X=>{const re={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[X._line]||"#555",J=qe(X,X._line)+(X.boost||0),ae=D.find(pe=>pe.cardId===X.cardId)?"#FFD700":"rgba(255,255,255,0.25)",oe=X.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${X.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${ae};background:${re};overflow:hidden;cursor:pointer;${oe}">
        ...
        </div>`,(Y=z.querySelector("#pp-close"))==null||Y.addEventListener("click",()=>z.remove()),z.querySelectorAll(".pp-item").forEach(X=>{X.addEventListener("click",()=>{const re=X.dataset.cid,J=_.find(ae=>ae.cardId===re),le=D.findIndex(ae=>ae.cardId===re);J&&(le>-1?D.splice(le,1):D.length<L&&D.push(J),B())})}),(W=z.querySelector("#pp-confirm"))==null||W.addEventListener("click",()=>{z.remove(),M(D)})}B(),document.body.appendChild(z)}async function se(_,L){const M=(y["gcCardsFull_"+p]||[]).find(P=>P.id===_),z=(M==null?void 0:M._gcDef)||(y.gcDefs||[]).find(P=>{var Y;return P.name===L||((Y=P.name)==null?void 0:Y.toLowerCase().trim())===(L==null?void 0:L.toLowerCase().trim())}),D=[...y["usedGc_"+p]||[],_],B={type:L,by:p,seq:(y._gcAnimSeq||0)+1};te.add(B.seq),V(L,p,async()=>{if(z!=null&&z.effect_type&&z.effect_type!=="CUSTOM"){const Y=k[z.effect_type];if(Y){const W={...y};Y(z.effect_params||{},W,async X=>{X["usedGc_"+p]=D,X._lastGC=B,X._gcAnimSeq=B.seq,await q(X)});return}}const P={...y};switch(L){case"Remplacement+":P.maxSubs=(P.maxSubs||3)+1,P.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const Y=m+"Score";P[Y]>0&&(P[Y]--,P.log.push({text:"🚫 But annulé",type:"info"}));break}}P["usedGc_"+p]=D,P._lastGC=B,P._gcAnimSeq=B.seq,await q(P)})}function ue(_,L){const A="usedCardIds_"+_,M=new Set(y[A]||[]);L.forEach(z=>M.add(z)),y[A]=[...M]}function ce(){for(const _ of["p1","p2"]){const L=new Set(y["usedCardIds_"+_]||[]),A=y[_+"Team"];if(A)for(const M of["GK","DEF","MIL","ATT"])(A[M]||[]).forEach(z=>{z.used=L.has(z.cardId)})}}function de(){var Ve,nt,yi,bi,hi,vi;if(y.phase==="reveal")return Le();if(y.phase==="midfield")return Q();if(y.phase==="finished")return Fe();const _=y[p+"Team"],L=y[m+"Team"];ce();const A=y[p+"Score"],M=y[m+"Score"],z=y[p+"Name"],D=y[m+"Name"],B=y.phase===p+"-attack",P=y.phase===p+"-defense",Y=Array.isArray(y["selected_"+p])?y["selected_"+p]:[],W=Y.map(ie=>ie.cardId),X=window.innerWidth>=700,re=y[p+"Subs"]||[],J=y["usedSubIds_"+p]||[],le=re.filter(ie=>!J.includes(ie.cardId)),ae=y["gcCardsFull_"+p]||[],oe=y["usedGc_"+p]||[],pe=ae.filter(ie=>!oe.includes(ie.id)),fe=y.boostOwner===p&&!y.boostUsed,xe=!["GK","DEF","MIL","ATT"].some(ie=>(L[ie]||[]).some(ge=>!ge.used)),Z=[..._.MIL||[],..._.ATT||[]].filter(ie=>!ie.used),be=B&&xe&&Z.length===0?[..._.GK||[],..._.DEF||[]].filter(ie=>!ie.used).map(ie=>ie.cardId):[];function ze(ie,ge,Ie){var $i,Ei;const ye=ie._gcDef,ot={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ye==null?void 0:ye.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",lt={purple:"#b06ce0",light_blue:"#00d4ef"}[ye==null?void 0:ye.color]||"#b06ce0",ht=(ye==null?void 0:ye.name)||ie.gc_type,vt=(ye==null?void 0:ye.effect)||(($i=Se[ie.gc_type])==null?void 0:$i.desc)||"",wi=ye!=null&&ye.image_url?`/icons/${ye.image_url}`:null,un=((Ei=Se[ie.gc_type])==null?void 0:Ei.icon)||"⚡",_i=Math.round(Ie*.22),ki=Math.round(Ie*.22),Vt=Ie-_i-ki,fn=ht.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${ie.id}" data-gc-type="${ie.gc_type}"
      ...
      </div>`}const Ne=(ie,ge)=>ge?Me(130,175):ze(ie,130,175),Ge=(ie,ge)=>ge?Me(68,95):ze(ie,68,95),Pe=X?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",bt=B?ke(p)?`<button id="pvp-action" style="${Pe};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${Y.length===0?"disabled":""}>⚔️ ATTAQUEZ <span id="pvp-timer"></span></button>`:`<button id="pvp-action" data-pass="1" style="${Pe};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER <span id="pvp-timer"></span></button>`:P?`<button id="pvp-action" style="${Pe};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${Y.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="pvp-timer"></span></button>`:`<div style="font-size:11px;color:rgba(255,255,255,0.3);text-align:center;padding:4px">⏳ Tour de ${D}</div>`,Be=B&&!ke(p)?'<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">Aucun attaquant — passez la main</div>':B||P?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${Y.length}/3 sélectionné(s)</div>`:"",Oe=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${X?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
      ${le.length===0?'<div style="font-size:7px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':le.map(ie=>`<div class="pvp-sub-btn" data-sub-id="${ie.cardId}" style="cursor:pointer;flex-shrink:0">${Je(ie,X?76:44,X?100:58)}</div>`).join("")}
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="width:min(90vw,420px)">${It(y[m+"Team"],y[m+"Formation"],null,[],We(),rt())}</div>
    </div>`,p==="p1"&&setTimeout(async()=>{await q({phase:"midfield"})},5e3)}let K=!1;function Q(){if(K)return;const _=y[p+"Team"].MIL||[],L=y[m+"Team"].MIL||[],A=milScore(_)+milLinks(_),M=milScore(L)+milLinks(L),z=A>=M;function D(ae,oe,pe){return`<div id="duel-row-${pe}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
        ...
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${ae.map((fe,xe)=>{const Z=xe<ae.length-1?yt(fe,ae[xe+1]):null,be=!Z||Z==="#ff3333"||Z==="#cc2222",ze=Z==="#00ff88"?"+2":Z==="#FFD700"?"+1":"";return`<div class="duel-card duel-card-${pe}" data-idx="${xe}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease,transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
            ...
            ${xe<ae.length-1?`<div class="duel-link duel-link-${pe}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${be?"rgba(255,255,255,0.12)":Z};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${be?"none":`0 0 8px ${Z}`}">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    </div>`;const B=(ae,oe)=>e.querySelectorAll(ae).forEach((pe,fe)=>{setTimeout(()=>{pe.style.opacity="1",pe.style.transform="translateY(0) scale(1)"},oe+fe*90)});B(".duel-card-me",150),B(".duel-card-opp",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((ae,oe)=>setTimeout(()=>{ae.style.opacity="1"},oe*70)),900),setTimeout(()=>{const ae=e.querySelector("#pvp-vs");ae&&(ae.style.opacity="1",ae.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(oe=>oe.style.opacity="1")},1250);function P(ae,oe,pe){const fe=document.getElementById(ae);if(!fe)return;const xe=performance.now(),Z=be=>{const ze=Math.min(1,(be-xe)/pe);fe.textContent=Math.round(oe*(1-Math.pow(1-ze,3))),ze<1?requestAnimationFrame(Z):fe.textContent=oe};requestAnimationFrame(Z)}setTimeout(()=>{P("pvp-score-me",A,800),P("pvp-score-opp",M,800)},1500);const Y=y.p1Team.MIL||[],W=y.p2Team.MIL||[],X=milScore(Y)+milLinks(Y),re=milScore(W)+milLinks(W),J=X>=re?"p1":"p2";let le=y.boostValue;le==null&&(le=tn(),y.boostValue=le,y.boostOwner=J,y.boostUsed=!1),K=!0,setTimeout(()=>{const ae=e.querySelector("#duel-row-"+(z?"me":"opp")),oe=e.querySelector("#duel-row-"+(z?"opp":"me")),pe=document.getElementById("pvp-score-me"),fe=document.getElementById("pvp-score-opp"),xe=z?pe:fe,Z=z?fe:pe;xe&&(xe.style.fontSize="80px",xe.style.color=z?"#FFD700":"#ff6b6b",xe.style.animation="duelPulse .5s ease"+(z?",duelGlow 1.5s ease infinite .5s":"")),Z&&(Z.style.opacity="0.25"),setTimeout(()=>{ae&&(ae.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",ae.style.zIndex="5"),setTimeout(()=>{const be=document.getElementById("duel-shock");be&&(be.style.animation="shockwave .5s ease-out forwards"),oe&&(oe.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var Ne;const be=document.getElementById("pvp-duel-finale");if(!be)return;const ze=y.boostOwner===p?'<div style="background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:3px solid #cdeffd;border-radius:18px;padding:20px 34px;text-align:center;animation:boostFlipIn .7s cubic-bezier(.34,1.56,.64,1) both;box-shadow:0 10px 36px rgba(135,206,235,.5)"><div style="font-size:10px;color:rgba(0,0,0,.6);letter-spacing:2px;text-transform:uppercase;margin-bottom:6px;font-weight:700">Carte Boost obtenue</div><div style="font-size:46px;line-height:1">⚡</div><div style="font-size:50px;font-weight:900;color:#063;line-height:1.1">+'+le+`</div><div style="font-size:10px;color:rgba(0,0,0,.55);margin-top:4px">Applicable sur n'importe quel joueur</div></div>`:"",Me=p==="p1"?'<button id="pvp-start-match" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">▶ Commencer le match</button>':`<div style="font-size:14px;color:rgba(255,255,255,0.5);text-align:center;margin-top:8px;animation:fadeUp .4s ease both">⏳ En attente de l'adversaire...</div>`;be.innerHTML='<div style="font-size:22px;font-weight:900;color:#fff;text-align:center;animation:fadeUp .4s ease both;text-shadow:0 2px 12px rgba(0,0,0,.5)">'+(z?"⚽ "+y[p+"Name"]+"<br>gagne le milieu et attaque !":"😔 "+y[m+"Name"]+"<br>gagne l'engagement et attaque !")+"</div>"+ze+Me,be.style.transition="opacity .45s ease",be.style.opacity="1",be.style.pointerEvents="auto",(Ne=document.getElementById("pvp-start-match"))==null||Ne.addEventListener("click",async()=>{const Ge=J;await q({phase:Ge+"-attack",attacker:Ge,round:1,boostValue:le,boostUsed:!1,boostOwner:Ge})})},600)},700)},2800)}function V(_,L,A){var oe,pe;const M=(y.gcDefs||[]).find(fe=>{var xe;return fe.name===_||((xe=fe.name)==null?void 0:xe.toLowerCase().trim())===(_==null?void 0:_.toLowerCase().trim())}),z={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[M==null?void 0:M.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",D={purple:"#b06ce0",light_blue:"#00d4ef"}[M==null?void 0:M.color]||"#b06ce0",B=(M==null?void 0:M.name)||_,P=(M==null?void 0:M.effect)||((oe=Se[_])==null?void 0:oe.desc)||"",Y=M!=null&&M.image_url?`/icons/${M.image_url}`:null,W=((pe=Se[_])==null?void 0:pe.icon)||"⚡",re=L===p?"Vous":y[L+"Name"]||"Adversaire",J=document.createElement("div");J.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",J.innerHTML=`
      <style>
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(J);let le=!1;const ae=()=>{le||(le=!0,J.remove(),setTimeout(()=>A&&A(),50))};J.addEventListener("click",ae),setTimeout(ae,3e3)}function T(_,L){var J,le,ae,oe;const M=(y["gcCardsFull_"+p]||[]).find(pe=>pe.id===_),z=M==null?void 0:M._gcDef,D={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[z==null?void 0:z.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",B={purple:"#b06ce0",light_blue:"#00d4ef"}[z==null?void 0:z.color]||"#b06ce0",P=(z==null?void 0:z.name)||L,Y=(z==null?void 0:z.effect)||((J=Se[L])==null?void 0:J.desc)||"Carte spéciale.",W=z!=null&&z.image_url?`/icons/${z.image_url}`:null,X=((le=Se[L])==null?void 0:le.icon)||"⚡",re=document.createElement("div");re.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",re.innerHTML=`
      ...
      </div>`,document.body.appendChild(A),(M=A.querySelector("#bp-close"))==null||M.addEventListener("click",()=>A.remove()),A.querySelectorAll(".bp-item").forEach(z=>{z.addEventListener("click",async()=>{const D=z.dataset.cid,B=L.find(Y=>Y.cardId===D);if(!B)return;const P=(_[B._line]||[]).find(Y=>Y.cardId===D);P&&(P.boost=(P.boost||0)+y.boostValue),A.remove(),await q({[p+"Team"]:_,boostUsed:!0})})})}function R(_=null){var le,ae;if(!(y.phase===p+"-attack")){f("Remplacement uniquement avant votre attaque","warning");return}const A=y[p+"Team"],M=y["usedSubIds_"+p]||[],z=y.maxSubs||3;if(M.length>=z){f(`Maximum ${z} remplacements atteint`,"warning");return}const D=Object.entries(A).flatMap(([oe,pe])=>(pe||[]).filter(fe=>fe.used).map(fe=>({...fe,_line:oe}))),B=(y[p+"Subs"]||[]).filter(oe=>!M.includes(oe.cardId));if(!D.length){f("Aucun joueur utilisé à remplacer","warning");return}if(!B.length){f("Aucun remplaçant disponible","warning");return}let P=Math.max(0,D.findIndex(oe=>oe.cardId===_));const Y=((le=D[P])==null?void 0:le._line)||((ae=D[P])==null?void 0:ae.job);let W=Math.max(0,B.findIndex(oe=>oe.job===Y)),X=!1;const re=document.createElement("div");re.id="pvp-sub-overlay",re.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function J(){var ze,Me,Ne,Ge,Pe,bt;const oe=D[P],pe=B[W],fe=Math.min(130,Math.round((window.innerWidth-90)/2)),xe=Math.round(fe*1.35),Z=Be=>`background:rgba(255,255,255,0.12);border:none;color:${Be?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Be?"default":"pointer"};flex-shrink:0`;re.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      </div>`,document.body.appendChild(L),(A=L.querySelector("#pvp-hist-close"))==null||A.addEventListener("click",()=>L.remove())}async function O(){if(y.phase!==p+"-attack")return;const _=p==="p1"?"p2":"p1",L=(y.round||0)+1,A=[...y.log||[]];A.push({type:"info",text:`⏭️ ${y[p+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const M=he(y),z=ke(_),D=M||!z?"finished":_+"-attack";await q({["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:D,attacker:_,round:L,log:A}),D==="finished"&&await je(y)}async function $e(){const _=y[p+"Team"],L=!["GK","DEF","MIL","ATT"].some(D=>(y[m+"Team"][D]||[]).some(B=>!B.used)),A=(y["selected_"+p]||[]).map(D=>{const B=(_[D._role]||[]).find(ae=>ae.cardId===D.cardId)||D,P=L&&["GK","DEF"].includes(D._role),Y=_[D._role]||[],W=Y.findIndex(ae=>ae.cardId===D.cardId),X=xt(Y.length),re=W>=0?X[W]:B._col??1,J=y.stadiumDef||y.homeStadiumDef||null,le=B.stadiumBonus||J&&(J.club_id&&String(B.club_id)===String(J.club_id)||J.country_code&&B.country_code===J.country_code)||!1;return{...B,_line:D._role,_col:re,stadiumBonus:le,...P?{note_a:Math.max(1,Number(B.note_a)||0)}:{}}});if(!A.length)return;const M=pi(A,y.modifiers[p]||{});ue(p,A.map(D=>D.cardId)),A.forEach(D=>{const B=(_[D._role]||[]).find(P=>P.cardId===D.cardId);B&&(B.used=!0)}),y["selected_"+p]=[],de();const z=[...y.log||[]];if(L){const D=(y[p+"Score"]||0)+1,B=A.map(re=>histPlayer(re));z.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:B,homeTotal:M.total,aiTotal:0});const P=(y.round||0)+1,Y=p==="p1"?"p2":"p1",W={...y,[p+"Team"]:_,[p+"Score"]:D},X=he(W);N.add(P),st(B,D,y[m+"Score"]||0,!0,async()=>{await q({[p+"Team"]:_,[p+"Score"]:D,["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:X?"finished":Y+"-attack",attacker:Y,round:P,log:z}),X&&await je({...y,[p+"Score"]:D})});return}z.push({type:"pending",text:`⚔️ ${y[p+"Name"]} attaque (${M.total})`}),await q({[p+"Team"]:_,[m+"Team"]:y[m+"Team"],pendingAttack:{...M,players:A,side:p},["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},phase:m+"-defense",log:z})}async function _e(){const _=y[p+"Team"],L=(y["selected_"+p]||[]).map(oe=>{const pe=(_[oe._role]||[]).find(ze=>ze.cardId===oe.cardId)||oe,fe=_[oe._role]||[],xe=fe.findIndex(ze=>ze.cardId===oe.cardId),Z=xt(fe.length),be=xe>=0?Z[xe]:pe._col??1;return{...pe,_line:oe._role,_col:be}}),A=ui(L,y.modifiers[p]||{});ue(p,L.map(oe=>oe.cardId)),L.forEach(oe=>{const pe=(_[oe._role]||[]).find(fe=>fe.cardId===oe.cardId);pe&&(pe.used=!0)}),y["selected_"+p]=[],de();const M=fi(y.pendingAttack.total,A.total,y.modifiers[p]||{}),z=y.pendingAttack.side,D=M==="attack"||(M==null?void 0:M.goal),B=z==="p1"?"p2":"p1",P=(y.round||0)+1,Y=(y.pendingAttack.players||[]).map(oe=>histPlayer(oe)),W=[...y.log||[]];W.push({type:"duel",isGoal:D,homeScored:D&&z===p,text:D?`⚽ BUT de ${y[z+"Name"]} ! (${y.pendingAttack.total} vs ${A.total})`:`✋ Attaque stoppée (${y.pendingAttack.total} vs ${A.total})`,homePlayers:Y,aiPlayers:L.map(oe=>histPlayer(oe)),homeTotal:y.pendingAttack.total,aiTotal:A.total});const X=D?(y[z+"Score"]||0)+1:y[z+"Score"]||0,re={...y,[p+"Team"]:_,[z+"Score"]:X},J=he(re),le=J?"finished":B+"-attack",ae=async()=>{await q({[p+"Team"]:_,[m+"Team"]:y[m+"Team"],[z+"Score"]:X,["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:le,attacker:B,round:P,log:W}),(le==="finished"||J)&&await je({...y,[z+"Score"]:X})};if(D){const oe=z===p,pe=oe?X:y[p+"Score"]||0,fe=oe?y[m+"Score"]||0:X;N.add(P),st(Y,pe,fe,oe,ae)}else await ae()}function we(_){return["MIL","ATT"].some(L=>(_[L]||[]).some(A=>!A.used))}function me(_){return["GK","DEF","MIL","ATT"].some(L=>(_[L]||[]).some(A=>!A.used))}function Ee(_){return me(_)&&!we(_)}function ke(_){const L=y[_+"Team"],A=y[(_==="p1"?"p2":"p1")+"Team"];return!!(we(L)||!me(A)&&Ee(L))}function he(_){const L=["MIL","ATT"].some(W=>(_.p1Team[W]||[]).some(X=>!X.used)),A=["MIL","ATT"].some(W=>(_.p2Team[W]||[]).some(X=>!X.used)),M=me(_.p1Team),z=me(_.p2Team);return!L&&!(!z&&M)&&(!A&&!(!M&&z))}function Ce(_){const L=_.p1Score||0,A=_.p2Score||0;return L===A?null:L>A?h.home_id:h.away_id}async function je(_){try{const L=Ce(_),A=L?h.home_id===L?h.away_id:h.home_id:null;await w.from("matches").update({status:"finished",winner_id:L,home_score:_.p1Score||0,away_score:_.p2Score||0}).eq("id",i),L&&A&&updateEvolutiveCards(L,A).catch(()=>{})}catch(L){console.error("[PvP] finishMatch:",L)}}function Fe(){var z;const _=y[p+"Score"],L=y[m+"Score"],A=_>L,M=_===L;He(e),e.innerHTML=`
