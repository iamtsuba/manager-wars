import{s as _,F as oi,h as zt,j as Mi,l as Ze,i as dn,k as ln,b as cn}from"./formation-links-CEzN0rcz.js";function xi(e,{navigate:t,toast:n}){e.innerHTML=`
  ...
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
  ...
  </div>`;function M(){const O=document.getElementById("col-filters");O&&(F==="player"?(O.innerHTML=`
        <input id="col-search" placeholder="🔍 Rechercher un joueur..." style="font-size:13px" value="${q}">
        <div style="display:flex;gap:6px;overflow-x:auto;padding-bottom:2px;align-items:center">
          ${Ln.map(H=>`
            <button class="filter-btn" data-job="${H}"
              style="flex-shrink:0;padding:4px 12px;border-radius:20px;font-size:12px;font-weight:600;cursor:pointer;
                border:1.5px solid ${H===K?"var(--green)":"var(--gray-200)"};
                background:${H===K?"var(--green)":"#fff"};
                color:${H===K?"#fff":"var(--gray-600)"}">
              ${H}
            </button>`).join("")}
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",H=>{q=H.target.value.toLowerCase(),Y()}),e.querySelectorAll(".filter-btn").forEach(H=>{H.addEventListener("click",()=>{K=H.dataset.job,M(),Y()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{j=!j,M(),Y()})):(O.innerHTML=`
        <div style="display:flex;justify-content:flex-end">
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{j=!j,M(),Y()})))}function Y(){const O=document.getElementById("col-grid");O&&(F==="player"?he(O):F==="formation"?xe(O):ge(O))}function ne(O,H,S,B,R){R=R||"#7a28b8";const ie=document.getElementById("col-grid"),ee=document.getElementById("col-big");if(!ie||!ee)return;var ke=0;function $e(){ee.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+H(O[ke])+"</div>";var Ee=ee.querySelector("[data-card-id],[data-form-id],[data-gc-id]");Ee&&Ee.addEventListener("click",function(){B(O[ke])}),requestAnimationFrame(function(){var _e=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!_e||!ee)){var Ae=ee.clientHeight-8,Te=ee.clientWidth-24,qe=_e.offsetHeight,Ne=_e.offsetWidth;if(qe>0&&Ne>0&&Ae>40){var He=Math.min(Ae/qe,Te/Ne,1);_e.style.transform="scale("+He.toFixed(3)+")",_e.style.transformOrigin="top center"}}}),ie.innerHTML=O.map(function(_e,Ae){return'<div class="col-mini-item" data-idx="'+Ae+'" style="flex-shrink:0;cursor:pointer;border-radius:8px;border:1.5px solid '+(Ae===ke?R:"transparent")+';transition:border-color .15s;overflow:hidden">'+S(_e,Ae===ke)+"</div>"}).join(""),ie.querySelectorAll(".col-mini-item").forEach(function(_e){_e.addEventListener("click",function(){ke=Number(_e.dataset.idx),$e(),_e.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}$e()}function D(O){var H=.54,S=Math.round(140*H),B=Math.round(310*H),R;if(!O||O._fake){var ie=O?O.player:null;if(!ie)return"";R=$i(ie)}else R=Vt(O,"");return'<div style="width:'+S+"px;height:"+B+'px;overflow:hidden;position:relative;flex-shrink:0"><div style="transform:scale('+H+');transform-origin:top left;position:absolute;top:0;left:0;pointer-events:none">'+R+"</div></div>"}function Q(O,H,S){H=H||100,S=S||140;var B=zt[O]||{},R={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},ie=Math.max(3,Math.round(H*.06)),ee=Object.entries(B).map(function($e){var Ee=$e[0],_e=$e[1],Ae=Ee.replace(/\d+$/,""),Te=R[Ae]||"#888",qe=Math.round(_e.x*H),Ne=Math.round(_e.y*S);return'<circle cx="'+qe+'" cy="'+Ne+'" r="'+ie+'" fill="'+Te+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),ke=Math.max(1,Math.round(H/50));return'<svg viewBox="0 0 '+H+" "+S+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+H+'" height="'+S+'" fill="#1A6B3C"/><rect x="'+Math.round(H*.2)+'" y="'+Math.round(S*.02)+'" width="'+Math.round(H*.6)+'" height="'+Math.round(S*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+ke+'"/><line x1="0" y1="'+Math.round(S*.5)+'" x2="'+H+'" y2="'+Math.round(S*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+ke+'"/><ellipse cx="'+Math.round(H*.5)+'" cy="'+Math.round(S*.5)+'" rx="'+Math.round(H*.18)+'" ry="'+Math.round(S*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+ke+'"/><rect x="'+Math.round(H*.2)+'" y="'+Math.round(S*.82)+'" width="'+Math.round(H*.6)+'" height="'+Math.round(S*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+ke+'"/>'+ee+"</svg>"}function ae(O,H,S){var B=S>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+S+"</div>":"",R=H?'data-form-id="'+H.id+'"':"",ie=O.length>7?14:O.length>5?16:19,ee=!!H;return"<div "+R+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(ee?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(ee?"":"filter:grayscale(1);opacity:0.5")+'">'+B+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(ee?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+ie+"px;font-weight:900;color:"+(ee?"#1A6B3C":"#aaa")+';line-height:1">'+O+'</div></div><div style="flex:1;overflow:hidden;background:'+(ee?"#1A6B3C":"#ccc")+'">'+Q(O,140,220)+"</div></div>"}function pe(O,H){var S=.54,B=Math.round(140*S),R=Math.round(305*S),ie=Math.round(R*.22),ee=R-ie,ke=O.length>7?5:7,$e=Q(O,B,ee),Ee=H?"1.5px solid #2a7a40":"1px solid #ddd",_e=H?"":"filter:grayscale(1);opacity:0.45;",Ae=H?"#1A6B3C":"#bbb",Te="#fff";return'<div style="width:'+B+"px;height:"+R+"px;border-radius:6px;border:"+Ee+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+_e+'"><div style="height:'+ie+"px;background:"+Ae+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+ke+"px;font-weight:900;color:"+Te+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(B-4)+'px">'+O+'</span></div><div style="height:'+ee+'px;overflow:hidden;flex-shrink:0">'+$e+"</div></div>"}function he(O){if(j){const H=re();if(!H.length){O.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const S=H.map(B=>i.find(R=>R.player.id===B.id)||{_fake:!0,player:B,id:"fake-"+B.id});ne(S,B=>B._fake?$i(B.player):Vt(B,""),B=>B._fake?D({player:B.player,id:"x",_fake:!0}):D(B),B=>{B._fake||ki(B,i,$,t)},"#1A6B3C")}else{const H=ce();if(!H.length){O.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const S={};H.forEach(R=>{const ie=R.player.id;S[ie]||(S[ie]=[]),S[ie].push(R)});const B=Object.values(S).map(R=>Mn(R));ne(B,R=>{const ie=$[R.player.id]||1,ee=ie>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${ie}</div>`:"",$e=i.filter(Ee=>Ee.player.id===R.player.id&&Ee.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Vt(R,ee+$e)},R=>D(R),R=>ki(R,i,$,t),"#1A6B3C")}}function xe(O){const H=j?k:[...L];if(!H.length){O.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const S=H.map(B=>({formation:B,card:u.find(R=>R.formation===B)||null,owned:L.has(B)}));ne(S,({formation:B,card:R,owned:ie})=>ae(B,ie?R:null,ie?u.filter(ee=>ee.formation===B).length:0),({formation:B,owned:R})=>pe(B,R),({card:B,owned:R})=>{R&&B&&jn(B,u,t,d)},"#1A6B3C")}function ge(O){const H=Object.keys(g),S=j?H.length?H:o:[...z];if(!S.length){O.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const B=S.map(R=>({type:R,gc:Ge[R]||{icon:"⚡",desc:""},def:g[R]||null,owned:z.has(R),card:r.find(ie=>ie.gc_type===R)||null}));ne(B,({type:R,gc:ie,def:ee,owned:ke,card:$e})=>{const Ee=ke?r.filter(m=>m.gc_type===R).length:0,_e=Ee>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${Ee}</div>`:"",Ae=(ee==null?void 0:ee.gc_type)==="ultra_game_changer",Te={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},qe={purple:"#b06ce0",light_blue:"#00d4ef"},Ne=Te[ee==null?void 0:ee.color]||Te.purple,He=qe[ee==null?void 0:ee.color]||qe.purple,T=(ee==null?void 0:ee.effect)||ie.desc||"",f=ee!=null&&ee.image_url?`/manager-wars/icons/${ee.image_url}`:null;return ke&&$e?`<div data-gc-id="${$e.id}" data-gc-type="${R}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${He};background:${Ne};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${He}66;cursor:pointer">
          ${_e}
          ...
          <div style="height:150px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
            ${f?`<img src="${f}" style="max-width:120px;max-height:120px;object-fit:contain;border-radius:6px">`:`<span style="font-size:64px">${ie.icon}</span>`}
          </div>
        ...
        </div>`},({type:R,gc:ie,def:ee,owned:ke})=>ke?(()=>{const $e=Math.round(75.60000000000001),Ee=Math.round(310*.54),_e=Math.round(Ee*.65),Ae=Math.round(Ee*.18),Te={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},qe={purple:"#9b59b6",light_blue:"#00bcd4"},Ne=Te[ee==null?void 0:ee.color]||Te.purple,He=qe[ee==null?void 0:ee.color]||qe.purple,T=ee!=null&&ee.image_url?`/manager-wars/icons/${ee.image_url}`:null;return`<div style="width:${$e}px;height:${Ee}px;border-radius:8px;background:${Ne};border:1px solid ${He};display:flex;flex-direction:column;overflow:hidden"><div style="height:${Ae}px;background:rgba(255,255,255,0.15);display:flex;align-items:center;justify-content:center"><span style="font-size:7px;font-weight:900;color:#fff;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${$e-6}px">${R}</span></div><div style="height:${_e}px;display:flex;align-items:center;justify-content:center">${T?`<img src="${T}" style="max-width:${$e-8}px;max-height:${_e-4}px;object-fit:contain">`:`<span style="font-size:24px">${ie.icon}</span>`}</div><div style="flex:1;display:flex;align-items:center;justify-content:center"><span style="font-size:7px;color:rgba(255,255,255,0.7);text-align:center;padding:0 2px">${((ee==null?void 0:ee.effect)||ie.desc||"").slice(0,20)}</span></div></div>`})():(()=>{const $e=Math.round(75.60000000000001),Ee=Math.round(310*.54);return`<div style="width:${$e}px;height:${Ee}px;border-radius:8px;background:#eee;border:1px solid #ddd;display:flex;flex-direction:column;align-items:center;justify-content:center;filter:grayscale(1);opacity:0.45"><span style="font-size:22px">${ie.icon}</span><span style="font-size:7px;color:#aaa;margin-top:4px;text-align:center;padding:0 4px">${R}</span></div>`})(),({type:R,owned:ie,def:ee})=>{ie&&Cn(R,ee,d)},"#7a28b8")}e.querySelectorAll(".col-tab-btn").forEach(O=>{O.addEventListener("click",()=>{F=O.dataset.tab,K="Tous",q="",j=!1,e.querySelectorAll(".col-tab-btn").forEach(H=>{const S=H.dataset.tab===F;H.style.borderBottomColor=S?"var(--green)":"transparent",H.style.color=S?"var(--green)":"var(--gray-600)"}),M(),Y()})}),M(),Y()}function Cn(e,t,n){const a=Ge[e]||{icon:"⚡",desc:"Effet spécial."},p=(t==null?void 0:t.gc_type)==="ultra_game_changer",d={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},s={purple:"#b06ce0",light_blue:"#00d4ef"},l=d[t==null?void 0:t.color]||d.purple,c=s[t==null?void 0:t.color]||s.purple,i=(t==null?void 0:t.name)||e,r=(t==null?void 0:t.effect)||a.desc,u=t!=null&&t.image_url?`/manager-wars/icons/${t.image_url}`:null;n("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    <div style="display:flex;justify-content:space-between;align-items:flex-start;background:#f9f9f9;border-radius:8px;padding:8px 12px;gap:8px">
      ...
      <div style="text-align:right;flex-shrink:0">
        <div style="font-size:13px;font-weight:700;color:${S.source==="booster"?"var(--gray-600)":"var(--yellow)"}">${R}</div>
    ...
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${u.map((S,B)=>{const R=M[S.id]||[],ie=S.is_for_sale,ee=R.length?R[R.length-1]:null,$e=(r.rarity==="pepite"||r.rarity==="papyte")&&S.current_note!=null?` (☆${S.current_note})`:"";return`
            <div data-card-id="${S.id}" data-card-idx="${B}" class="exemplaire-row"
              ...
              <div style="display:flex;align-items:center;gap:10px">
                ...
                <div style="flex:1;min-width:0">
                  <div style="font-size:12px;font-weight:700">Exemplaire ${B+1}${$e}${ie?" 🏷️ En vente":""}</div>
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",r.note_g],["DEF",r.note_d],["MIL",r.note_m],["ATT",r.note_a]].map(([S,B])=>{const R=Et[S],ie=Number(B)||0;return`<div style="display:flex;flex-direction:column;align-items:center;gap:2px">
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(xe=document.getElementById("close-detail"))==null||xe.addEventListener("click",l);let D=new Set;const Q=()=>{const S=D.size,B=document.getElementById("sell-action-panel");B&&(B.style.display=S>0?"block":"none",document.getElementById("sell-selected-count").textContent=S,document.getElementById("sell-direct-total").textContent=(S*$).toLocaleString("fr")+" cr.")};document.querySelectorAll(".expl-check").forEach(S=>{S.addEventListener("change",()=>{const B=S.dataset.id;S.checked?D.add(B):D.delete(B);const R=S.closest(".exemplaire-row");R&&(R.style.borderColor=S.checked?"#1A6B3C":"#eee"),Q()})}),document.querySelectorAll(".exemplaire-row").forEach(S=>{S.addEventListener("click",B=>{if(B.target.closest("button")||B.target.tagName==="INPUT")return;const R=S.querySelector(".expl-check");R&&!R.disabled&&(R.checked=!R.checked,R.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(S=>{S.addEventListener("click",B=>{B.stopPropagation();const R=document.querySelector(`.expl-hist[data-hist="${S.dataset.idx}"]`);R&&(R.style.display=R.style.display==="none"?"flex":"none")})}),(ge=document.getElementById("direct-sell-btn"))==null||ge.addEventListener("click",async()=>{const S=[...D];if(!S.length)return;const B=S.length*$;if(!confirm(`Vendre ${S.length} carte${S.length>1?"s":""} ${r.surname_encoded} pour ${B.toLocaleString("fr")} crédits ? Action irréversible.`))return;await _.from("market_listings").delete().in("card_id",S),await _.from("transfer_history").delete().in("card_id",S);const{error:R}=await _.from("cards").delete().in("id",S);if(R){d(R.message,"error");return}await _.from("users").update({credits:(p.profile.credits||0)+B}).eq("id",p.profile.id),await i();const ie=document.getElementById("nav-credits");ie&&(ie.textContent=`💰 ${(p.profile.credits||0).toLocaleString("fr")}`),d(`+${B.toLocaleString("fr")} crédits ! ${S.length} carte${S.length>1?"s":""} vendue${S.length>1?"s":""}.`,"success"),l(),c("collection")}),(O=document.getElementById("market-sell-btn"))==null||O.addEventListener("click",async()=>{var ke;const S=[...D];if(!S.length){d("Sélectionne au moins un exemplaire","warning");return}const B=parseInt((ke=document.getElementById("sell-market-price"))==null?void 0:ke.value);if(!B||B<1){d("Prix invalide","error");return}const{error:R}=await _.from("cards").update({is_for_sale:!0,sale_price:B}).in("id",S);if(R){d(R.message,"error");return}const ie=S.map($e=>({seller_id:p.profile.id,card_id:$e,price:B,status:"active"})),{error:ee}=await _.from("market_listings").insert(ie);ee&&console.warn("[Market] insert listings:",ee.message),d(`${S.length} carte${S.length>1?"s":""} mise${S.length>1?"s":""} en vente à ${B.toLocaleString("fr")} cr. chacune !`,"success"),l(),c("collection")}),(H=document.getElementById("cancel-sell-btn"))==null||H.addEventListener("click",async()=>{await _.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await _.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),d("Annonce retirée","success"),l(),c("collection")})}const Gt={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},rt={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function Ut(e){const t=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const n=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${n}.png`}function Hi(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function Ui(e){var n;const t="https://fcnwclxlkytdfjotqkta.supabase.co";return(n=e==null?void 0:e.clubs)!=null&&n.logo_url?e.clubs.logo_url.startsWith("http")?e.clubs.logo_url:`${t}/storage/v1/object/public/assets/clubs/${e.clubs.logo_url}`:null}function Bn(e,t=44,n=58){var g;const a=e?Ut(e):null,p=e?Ui(e):null,d=Hi(e==null?void 0:e.country_code),s=(e==null?void 0:e.job)||"MIL",l=rt[s]||"#555",c={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",i=Number(s==="GK"?e==null?void 0:e.note_g:s==="DEF"?e==null?void 0:e.note_d:s==="MIL"?e==null?void 0:e.note_m:e==null?void 0:e.note_a)||0,r=Math.round(n*.19),u=Math.round(n*.25),x=n-r-u;return e?`<div style="width:${t}px;height:${n}px;border-radius:5px;border:2px solid ${c};background:${l};position:relative;overflow:hidden;flex-shrink:0">
  ...
  </div>`,Fn(e,t,p,n),document.getElementById("builder-back").addEventListener("click",()=>a("decks")),document.getElementById("formation-select").addEventListener("change",i=>{t.formation=i.target.value;const r=Ai(t.formation),u={};r.forEach(x=>{t.slots[x]&&(u[x]=t.slots[x])}),t.slots=u,pt(e,t,n)}),document.getElementById("save-deck").addEventListener("click",()=>Gn(t,n)),e.querySelectorAll("[data-remove-sub]").forEach(i=>{i.addEventListener("click",()=>{t.subs=t.subs.filter(r=>r!==i.dataset.removeSub),pt(e,t,n)})}),(c=document.getElementById("add-sub-btn"))==null||c.addEventListener("click",()=>{Dn(t,e,n)})}function Fn(e,t,n,a){const p=e.querySelector("#deck-field");if(!p)return;const d=zt[t.formation]||{},s=Mi(t.formation)||[],l={};for(const L of n){const z=t.slots[L],F=z?t.playerCards.find(K=>K.id===z):null;l[L]=F?F.player:null}const c=300,i=300,r=48,u=64,x=13,g=16,k=38;function o(L){const z=d[L];return z?{x:z.x*c,y:z.y*i}:null}let $="";for(const[L,z]of s){const F=o(L),K=o(z);if(!F||!K)continue;const q=l[L]?{...l[L],club_id:l[L].club_id,country_code:l[L].country_code,rarity:l[L].rarity}:null,j=l[z]?{...l[z],club_id:l[z].club_id,country_code:l[z].country_code,rarity:l[z].rarity}:null,P=Ze(q,j);P==="#ff3333"||P==="#cc2222"?$+=`<line x1="${F.x.toFixed(1)}" y1="${F.y.toFixed(1)}" x2="${K.x.toFixed(1)}" y2="${K.y.toFixed(1)}" stroke="${P}" stroke-width="2.5" stroke-linecap="round" opacity="0.4"/>`:($+=`<line x1="${F.x.toFixed(1)}" y1="${F.y.toFixed(1)}" x2="${K.x.toFixed(1)}" y2="${K.y.toFixed(1)}" stroke="${P}" stroke-width="8" stroke-linecap="round" opacity="0.2"/>`,$+=`<line x1="${F.x.toFixed(1)}" y1="${F.y.toFixed(1)}" x2="${K.x.toFixed(1)}" y2="${K.y.toFixed(1)}" stroke="${P}" stroke-width="2.5" stroke-linecap="round" opacity="0.9"/>`)}for(const L of n){const z=o(L);if(!z)continue;const F=l[L],K=L.replace(/\d+/,""),q=rt[K]||"#555",j=(z.x-r/2).toFixed(1),P=(z.y-u/2).toFixed(1),te={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[F==null?void 0:F.rarity]||"#aaa";if(F){const ce=Ut(F),re=Ui(F),M=Hi(F.country_code),Y=Number(K==="GK"?F.note_g:K==="DEF"?F.note_d:K==="MIL"?F.note_m:F.note_a)||0,ne=u-x-g;$+=`<defs><clipPath id="dcp-${L}"><rect x="${j}" y="${(z.y-u/2+x).toFixed(1)}" width="${r}" height="${ne}"/></clipPath></defs>`,$+=`<rect x="${j}" y="${P}" width="${r}" height="${u}" rx="5" fill="${q}"/>`,ce&&($+=`<image href="${ce}" x="${j}" y="${(z.y-u/2+x).toFixed(1)}" width="${r}" height="${ne}" clip-path="url(#dcp-${L})" preserveAspectRatio="xMidYMin slice"/>`),$+=`<rect x="${j}" y="${P}" width="${r}" height="${x}" fill="rgba(255,255,255,0.93)"/>`,$+=`<text x="${z.x.toFixed(1)}" y="${(z.y-u/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(F.surname_encoded||"").slice(0,9)}</text>`;const D=(z.y+u/2-g).toFixed(1);$+=`<rect x="${j}" y="${D}" width="${r}" height="${g}" fill="rgba(255,255,255,0.93)"/>`,M&&($+=`<image href="${M}" x="${(z.x-21).toFixed(1)}" y="${(z.y+u/2-g+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`),$+=`<text x="${z.x.toFixed(1)}" y="${(z.y+u/2-g/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${Y}</text>`,re&&($+=`<image href="${re}" x="${(z.x+r/2-14).toFixed(1)}" y="${(z.y+u/2-g+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`),$+=`<rect x="${j}" y="${P}" width="${r}" height="${u}" rx="5" fill="none" stroke="${te}" stroke-width="2"/>`}else $+=`<rect x="${j}" y="${P}" width="${r}" height="${u}" rx="5" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="5,3"/>`,$+=`<text x="${z.x.toFixed(1)}" y="${z.y.toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="22" fill="rgba(255,255,255,0.35)">+</text>`,$+=`<text x="${z.x.toFixed(1)}" y="${(z.y+16).toFixed(1)}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.3)">${K}</text>`;$+=`<rect x="${j}" y="${P}" width="${r}" height="${u}" rx="5" fill="rgba(0,0,0,0.01)" class="deck-slot-hit" data-pos="${L}" style="cursor:pointer"/>`}p.innerHTML=`<svg viewBox="${-k} ${-k} ${c+k*2} ${i+k*2}" width="100%" style="display:block;max-width:440px;margin:0 auto">${$}</svg>`,p.querySelectorAll(".deck-slot-hit").forEach(L=>{L.addEventListener("click",()=>qn(L.dataset.pos,t,e,a))})}function qn(e,t,n,a){var x,g,k;const{openModal:p,closeModal:d}=a,s=e.replace(/\d+/,""),l=t.slots[e],c=l?t.playerCards.find(o=>o.id===l):null;(x=c==null?void 0:c.player)==null||x.id;const i=new Set;Object.entries(t.slots).forEach(([o,$])=>{var z;if(o===e||!$)return;const L=t.playerCards.find(F=>F.id===$);(z=L==null?void 0:L.player)!=null&&z.id&&i.add(L.player.id)}),t.subs.forEach(o=>{var L;const $=t.playerCards.find(z=>z.id===o);(L=$==null?void 0:$.player)!=null&&L.id&&i.add($.player.id)});const r=new Set,u=t.playerCards.filter(o=>{const $=o.player;return!($.job===s||$.job2===s)||i.has($.id)||r.has($.id)?!1:(r.add($.id),!0)});u.sort((o,$)=>{const L=s==="GK"?o.player.note_g:s==="DEF"?o.player.note_d:s==="MIL"?o.player.note_m:o.player.note_a;return(s==="GK"?$.player.note_g:s==="DEF"?$.player.note_d:s==="MIL"?$.player.note_m:$.player.note_a)-L}),p(`Choisir ${s} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${u.length>0?u.map(o=>{var K,q;const $=o.player,L=s==="GK"?$.note_g:s==="DEF"?$.note_d:s==="MIL"?$.note_m:$.note_a,z=Ut($),F={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[$.rarity];return`<div class="player-option" data-card-id="${o.id}"
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    </div>`,e.querySelectorAll(".gc-select-card").forEach(k=>{k.addEventListener("click",()=>{const o=k.dataset.id,$=d.find(z=>z.id===o);if(!$)return;const L=s.findIndex(z=>z.gc_type===$.gc_type);L>-1?s.splice(L,1):s.length<3&&s.push($),i()})}),(u=e.querySelector("#gc-launch"))==null||u.addEventListener("click",()=>{r&&c(s)}),(x=e.querySelector("#gc-no-gc"))==null||x.addEventListener("click",()=>c([])),(g=e.querySelector("#gc-reset"))==null||g.addEventListener("click",()=>{s.length&&(s=[],i())})}i()}function ro(e){var a;const t=((a=e==null?void 0:e.state)==null?void 0:a.params)||{},n=t.matchMode||"vs_ai_easy";return n==="friend"?`Match vs ${t.friendName||"Ami"}`:n==="random"?"Match vs Random":`Match vs IA — ${n.replace("vs_ai_","").toUpperCase()}`}async function ao(e,t,n){const{state:a,navigate:p}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:d}=await _.from("decks").select("id,name,is_active,formation").eq("owner_id",a.profile.id).order("created_at",{ascending:!1});if(!d||d.length===0){at(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>p("decks"));return}const s=d.map(r=>r.id),{data:l}=await _.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",s).order("slot_order");let c=0;function i(){var z,F,K,q,j;const r=d[c],u=(l||[]).filter(P=>P.deck_id===r.id),x=u.filter(P=>{var te;return P.is_starter&&((te=P.card)==null?void 0:te.player)}).map(P=>Zt(P.card,P.position)),g=u.find(P=>{var te;return((te=P.card)==null?void 0:te.card_type)==="formation"}),k=r.formation||((z=g==null?void 0:g.card)==null?void 0:z.formation)||"4-4-2",o=x.length>=11?gt(x,k):null,$=x.length>=11;Xe(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
  ...
  <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div style="display:flex;align-items:center;gap:4px;${t?"flex-direction:row-reverse":""};background:rgba(255,255,255,0.04);border-radius:8px;padding:5px 8px;border:1px solid rgba(255,255,255,0.07)">
      ...
      ${nt(e.inPlayer||{})}
    </div>`}return`<div style="font-size:11px;color:${e.type==="goal"?"#FFD700":"rgba(255,255,255,0.65)"};font-weight:${e.type==="goal"?700:400};padding:3px 2px">${e.text||""}</div>`}function Me(e,t,n){var F,K,q,j,P,te,ce,re;const a=t.selected.map(M=>M.cardId),p=t.usedSubIds||[],d=t.homeSubs.filter(M=>!p.includes(M.cardId)),l=Object.values(t.homeTeam).flat().filter(M=>M.used).length>0&&d.length>0&&t.subsUsed<t.maxSubs,c=!["GK","DEF","MIL","ATT"].some(M=>(t.aiTeam[M]||[]).some(Y=>!Y.used)),i=[...t.homeTeam.MIL||[],...t.homeTeam.ATT||[]].filter(M=>!M.used),r=t.phase==="attack"&&c&&i.length===0?[...t.homeTeam.GK||[],...t.homeTeam.DEF||[]].filter(M=>!M.used).map(M=>M.cardId):[];t.log[t.log.length-1];const u=t.phase==="ai-attack"||t.phase==="ai-defense",x=t.phase==="attack",g=t.phase==="defense",k=t.phase==="finished",o=t.gcCards.filter(M=>!t.usedGc.includes(M.id)),$=t.boostCard&&!t.boostUsed;e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column",e.innerHTML=`
  <style>
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    ${(()=>{const M=window.innerWidth>=700,Y=(O,H,S)=>{var Ne,He;const B=(t.gcDefs||[]).find(T=>T.name===O.gc_type),R={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[B==null?void 0:B.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",ie={purple:"#b06ce0",light_blue:"#00d4ef"}[B==null?void 0:B.color]||"#b06ce0",ee=(B==null?void 0:B.name)||O.gc_type,ke=(B==null?void 0:B.effect)||((Ne=Ge[O.gc_type])==null?void 0:Ne.desc)||"",$e=B!=null&&B.image_url?`/manager-wars/icons/${B.image_url}`:null,Ee=((He=Ge[O.gc_type])==null?void 0:He.icon)||"⚡",_e=Math.round(S*.22),Ae=Math.round(S*.22),Te=S-_e-Ae,qe=ee.length>12?7:9;return`<div class="gc-mini" data-gc-id="${O.id}" data-gc-type="${O.gc_type}"
          style="box-sizing:border-box;width:${H}px;height:${S}px;border-radius:10px;border:2px solid ${ie};background:${R};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
          <div style="height:${_e}px;background:rgba(255,255,255,0.14);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 3px">
            <span style="font-size:${qe}px;font-weight:900;color:#fff;text-align:center;line-height:1.1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${H-6}px">${ee}</span>
          ...
          <div style="height:${Te}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
            ${$e?`<img src="${$e}" style="max-width:${H-10}px;max-height:${Te-6}px;object-fit:contain">`:`<span style="font-size:${Math.round(Te*.55)}px">${Ee}</span>`}
          </div>
        ...
        </div>`},ne=(O,H)=>{var S;return`<div id="boost-card"
          style="box-sizing:border-box;width:${O}px;height:${H}px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:2px solid #87CEEB;border-radius:10px;cursor:pointer;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:${Math.round(H*.04)}px;text-align:center;flex-shrink:0">
            <div style="font-size:${Math.round(H*.2)}px">⚡</div>
            <div style="font-size:${Math.round(H*.09)}px;color:#000;font-weight:900">+${(S=t.boostCard)==null?void 0:S.value}</div>
          </div>`},D=(O,H)=>H?ne(130,175):Y(O,130,175),Q=(O,H)=>H?ne(68,95):Y(O,68,95),ae=M?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",pe=k?`<button id="btn-results" style="${ae};background:linear-gradient(135deg,#D4A017,#FFD700);border:none;color:#000">🏁 Résultats</button>`:u?`<div style="${ae};background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);color:rgba(255,255,255,0.4)">⏳ Tour IA</div>`:x?`<button id="btn-action" style="${ae};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${t.selected.length===0?"disabled":""}> ⚔️ ATTAQUEZ <span id="match-timer" style="font-weight:900"></span></button>`:g?`<button id="btn-action" style="${ae};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${t.selected.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="match-timer" style="font-weight:900"></span></button>`:`<div style="${ae};background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1)"></div>`,he=x||g?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${t.selected.length}/3 sélectionné(s)</div>`:"",xe=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${M?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
  ...
  `,document.body.appendChild(a);let r=!1;const u=()=>{r||(r=!0,a.remove(),n())};a.addEventListener("click",u),setTimeout(u,2e3)}function _t(e,t="rgba(0,0,0,0.8)"){const n=document.createElement("div");n.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,n.textContent=e,document.body.appendChild(n),setTimeout(()=>n.remove(),2200)}function Yt(e,t,n,a=null,p=null){var g,k;if(t.phase!=="attack"){_t("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){_t(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const d=Object.entries(t.homeTeam).flatMap(([o,$])=>($||[]).filter(L=>L.used).map(L=>({...L,_line:L._line||o}))),s=t.homeSubs.filter(o=>!t.usedSubIds.includes(o.cardId));if(!d.length){_t("Aucun joueur utilisé à remplacer");return}if(!s.length){_t("Aucun remplaçant disponible");return}let l=Math.max(0,d.findIndex(o=>o.cardId===p));const c=((g=d[l])==null?void 0:g._line)||((k=d[l])==null?void 0:k.job);let i=a?Math.max(0,s.findIndex(o=>o.cardId===a)):Math.max(0,s.findIndex(o=>o.job===c)),r=!1;const u=document.createElement("div");u.id="sub-overlay",u.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function x(){var q,j,P,te,ce,re;const o=d[l],$=s[i],L=Math.min(130,Math.round((window.innerWidth-90)/2)),z=Math.round(L*1.35),F=M=>`background:rgba(255,255,255,0.12);border:none;color:${M?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${M?"default":"pointer"};flex-shrink:0`;u.innerHTML=`
    ...
    </div>`,(q=u.querySelector("#sub-close"))==null||q.addEventListener("click",()=>u.remove()),(j=u.querySelector("#out-up"))==null||j.addEventListener("click",()=>{l>0&&(l--,x())}),(P=u.querySelector("#out-down"))==null||P.addEventListener("click",()=>{l<d.length-1&&(l++,x())}),(te=u.querySelector("#in-up"))==null||te.addEventListener("click",()=>{i>0&&(i--,x())}),(ce=u.querySelector("#in-down"))==null||ce.addEventListener("click",()=>{i<s.length-1&&(i++,x())});const K=(M,Y,ne,D)=>{const Q=u.querySelector("#"+M);if(!Q)return;let ae=0;Q.addEventListener("touchstart",pe=>{ae=pe.touches[0].clientY},{passive:!0}),Q.addEventListener("touchend",pe=>{const he=pe.changedTouches[0].clientY-ae;if(Math.abs(he)<30)return;const xe=Y();he<0&&xe<D-1?(ne(xe+1),x()):he>0&&xe>0&&(ne(xe-1),x())},{passive:!0})};K("in-panel",()=>i,M=>i=M,s.length),K("out-panel",()=>l,M=>l=M,d.length),(re=u.querySelector("#sub-confirm"))==null||re.addEventListener("click",M=>{if(M.preventDefault(),M.stopPropagation(),r)return;r=!0;const Y=d[l],ne=s[i];if(!Y||!ne)return;let D=null,Q=-1;for(const[pe,he]of Object.entries(t.homeTeam)){const xe=(he||[]).findIndex(ge=>ge.cardId===Y.cardId);if(xe!==-1){D=pe,Q=xe;break}}if(Q===-1||!D){_t("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),u.remove();return}const ae={...ne,_line:D,_col:Y._col||0,used:!1,boost:0};t.homeTeam[D].splice(Q,1,ae),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(ne.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:Y.name,firstname:Y.firstname,note:we(Y,D),portrait:ze(Y),job:Y.job,country_code:Y.country_code,rarity:Y.rarity,clubName:Y.clubName,clubLogo:Y.clubLogo},inPlayer:{name:ne.name,firstname:ne.firstname,note:we(ne,D),portrait:ze(ne),job:ne.job,country_code:ne.country_code,rarity:ne.rarity,clubName:ne.clubName,clubLogo:ne.clubLogo},text:`🔄 ${ne.firstname} ${ne.name} remplace ${Y.firstname} ${Y.name}`}),u.remove(),ho(Y,ne,()=>Me(e,t,n))})}document.body.appendChild(u),x()}function bo(e,t,n,a,p){var k,o;const d=(a.gcDefs||[]).find($=>$.name===t),s=Ge[t]||{icon:"⚡",desc:"Carte spéciale."},l={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[d==null?void 0:d.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",c={purple:"#b06ce0",light_blue:"#00d4ef"}[d==null?void 0:d.color]||"#b06ce0",i=(d==null?void 0:d.name)||t,r=(d==null?void 0:d.effect)||s.desc,u=d!=null&&d.image_url?`/manager-wars/icons/${d.image_url}`:null,x=s.icon||"⚡",g=document.createElement("div");g.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",g.innerHTML=`
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(d);let l=!1;const c=()=>{l||(l=!0,d.remove(),setTimeout(()=>p(),50))};d.addEventListener("click",c),setTimeout(c,3500)}async function At(e,t,n){var r,u;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:a}=n,p=t.homeScore>t.aiScore,d=t.homeScore===t.aiScore,s=p?"victoire":d?"nul":"defaite",l=Tn(t.mode,s);t.matchId&&await _.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:p?a.profile.id:null,home_credits_reward:l,played_at:new Date().toISOString()}).eq("id",t.matchId);const c={credits:(a.profile.credits||0)+l,matches_played:(a.profile.matches_played||0)+1};p?c.wins=(a.profile.wins||0)+1:d?c.draws=(a.profile.draws||0)+1:c.losses=(a.profile.losses||0)+1,await _.from("users").update(c).eq("id",a.profile.id),await n.refreshProfile();const i=document.createElement("div");i.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",i.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const g=async(F=!0)=>{u=!0,x&&(_.removeChannel(x),x=null),F&&await _.rpc("cancel_matchmaking",{p_user_id:c.profile.id}).catch(()=>{})};(z=document.getElementById("mm-cancel"))==null||z.addEventListener("click",async()=>{try{await g(!0)}catch{}Oe(e),i("home")});const k=async(F,K,q)=>{if(u)return;u=!0,x&&(_.removeChannel(x),x=null);const j=document.getElementById("mm-status");j&&(j.textContent="Adversaire trouvé !"),await new Promise(P=>setTimeout(P,600)),e.isConnected&&rn(e,t,F,q,s,l)},{data:o,error:$}=await _.rpc("try_matchmake",{p_user_id:c.profile.id,p_deck_id:n});if($||!(o!=null&&o.success)){r("Erreur de matchmaking","error"),Oe(e),i("home");return}if(o.matched){k(o.match_id,o.opponent_id,!1);return}const L=document.getElementById("mm-status");L&&(L.textContent="En attente d'un autre joueur..."),x=_.channel("matchmaking-"+c.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${c.profile.id}`},F=>{const K=F.new;K.status==="matched"&&K.match_id&&k(K.match_id,K.matched_with,!0)}).subscribe()}async function Eo(e,t,n){const{state:a,navigate:p,toast:d}=t;try{(_.getChannels?_.getChannels():[]).forEach(i=>{const r=i.topic||"";(r.includes("matchmaking")||r.includes("pvp-match")||r.includes("friend-invite"))&&_.removeChannel(i)})}catch{}const{data:s}=await _.from("matches").select("home_id, away_id, status").eq("id",n).single();if(!s){d("Match introuvable","error"),p("home");return}if(s.status==="finished"){d("Ce match est terminé","info"),p("home");return}const l=s.home_id===a.profile.id;rn(e,t,n,l,[],[])}async function rn(e,t,n,a,p=[],d=[]){const{state:s,navigate:l,toast:c}=t,i=a?"p1":"p2",r=a?"p2":"p1",u=(p||[]).map(T=>T.id),x=(p||[]).map(T=>({id:T.id,gc_type:T.gc_type,_gcDef:T._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:g}=await _.from("matches").select("*").eq("id",n).single();if(!g){c("Match introuvable","error"),l("home");return}async function k(){const[{data:T},{data:f},{data:m},{data:h}]=await Promise.all([_.rpc("get_deck_for_match",{p_deck_id:g.home_deck_id}),_.rpc("get_deck_for_match",{p_deck_id:g.away_deck_id}),_.from("users").select("id,pseudo,club_name").eq("id",g.home_id).single(),_.from("users").select("id,pseudo,club_name").eq("id",g.away_id).single()]),y=E=>({cardId:E.card_id,position:E.position,id:E.id,firstname:E.firstname,name:E.surname_encoded,country_code:E.country_code,club_id:E.club_id,job:E.job,job2:E.job2,note_g:Number(E.note_g)||0,note_d:Number(E.note_d)||0,note_m:Number(E.note_m)||0,note_a:Number(E.note_a)||0,rarity:E.rarity,skin:E.skin,hair:E.hair,hair_length:E.hair_length,clubName:E.club_encoded_name||null,clubLogo:E.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}),b=((T==null?void 0:T.starters)||[]).map(E=>y(E)),v=((f==null?void 0:f.starters)||[]).map(E=>y(E)),w=(T==null?void 0:T.formation)||"4-4-2",I=(f==null?void 0:f.formation)||"4-4-2";return{p1Team:gt(b,w),p2Team:gt(v,I),p1Subs:((T==null?void 0:T.subs)||[]).map(E=>y(E)),p2Subs:((f==null?void 0:f.subs)||[]).map(E=>y(E)),p1Formation:w,p2Formation:I,p1Name:(m==null?void 0:m.club_name)||(m==null?void 0:m.pseudo)||"Joueur 1",p2Name:(h==null?void 0:h.club_name)||(h==null?void 0:h.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:a?u:[],gc_p2:a?[]:u,gcCardsFull_p1:a?x:[],gcCardsFull_p2:a?[]:x,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:d||[],lastActionAt:new Date().toISOString()}}let o=g.game_state&&Object.keys(g.game_state).length?g.game_state:null;if(!o)if(a){o=await k();const{data:T}=await _.from("matches").select("game_state").eq("id",n).single();!(T!=null&&T.game_state)||!Object.keys(T.game_state).length?await _.from("matches").update({game_state:o,turn_user_id:g.home_id}).eq("id",n):o=T.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let T=0;T<30&&!o;T++){await new Promise(m=>setTimeout(m,400));const{data:f}=await _.from("matches").select("game_state").eq("id",n).single();f!=null&&f.game_state&&Object.keys(f.game_state).length&&(o=f.game_state)}if(!o){c("Erreur de synchronisation","error"),l("home");return}o.gc_p2=u,o.gcCardsFull_p2=x,await _.from("matches").update({game_state:o}).eq("id",n)}let $=!1,L=null,z=!1;const F=new Set,K=new Set;function q(T){var E,A;try{_.removeChannel(j)}catch{}const f=o[i+"Score"]||0,m=o[r+"Score"]||0;let h,y;T.winner_id?(h=T.winner_id===s.profile.id,y=!1):T.forfeit?(h=f>m,y=!1):(y=f===m,h=f>m),(E=document.getElementById("pvp-end-overlay"))==null||E.remove();const b=document.createElement("div");b.id="pvp-end-overlay",b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const v=y?"🤝":h?"🏆":"😞",w=y?"MATCH NUL":h?"VICTOIRE !":"DÉFAITE",I=y?"#fff":h?"#FFD700":"#ff6b6b";b.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(b),(A=b.querySelector("#pvp-end-home"))==null||A.addEventListener("click",()=>{b.remove(),Oe(e),l("home")})}const j=_.channel("pvp-match-"+n).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${n}`},T=>{const f=T.new;try{if(f.status==="finished"||f.forfeit){if($)return;$=!0,L&&(clearInterval(L),L=null),f.game_state&&(o=f.game_state),q(f);return}if(f.game_state){const m=o;o=f.game_state;const h=o._lastGC;if(h&&h.seq&&!K.has(h.seq)&&(K.add(h.seq),h.by!==i)){xe(h.type,h.by,()=>D());return}const y=m[i+"Score"]||0,b=m[r+"Score"]||0,v=o[i+"Score"]||0,w=o[r+"Score"]||0,I=v>y,E=w>b;if((I||E)&&!F.has(o.round)){F.add(o.round);const A=[...o.log||[]].reverse().find(N=>N.isGoal),C=((A==null?void 0:A.homePlayers)||[]).map(N=>({name:N.name,note:N.note,portrait:N.portrait,job:N.job}));he(C,v,w,I,()=>D());return}D()}}catch(m){console.error("[PvP] crash:",m)}}).subscribe();async function P(T){Object.assign(o,T),o.lastActionAt=new Date().toISOString();const{error:f}=await _.from("matches").update({game_state:o}).eq("id",n);f&&c("Erreur de synchronisation","error");try{D()}catch(m){console.error("[PvP] renderPvpScreen crash:",m)}}async function te(){if($)return;$=!0,L&&(clearInterval(L),L=null);const T=a?g.away_id:g.home_id,f=a?"p2":"p1",m=a?"p1":"p2",h={...o,[f+"Score"]:3,[m+"Score"]:0,phase:"finished"};try{await _.from("matches").update({status:"finished",forfeit:!0,winner_id:T,home_score:h.p1Score||0,away_score:h.p2Score||0,game_state:h}).eq("id",n)}catch{}try{_.removeChannel(j)}catch{}setTimeout(()=>{Oe(e),l("home")},800)}const ce={BOOST_STAT:({value:T=1,count:f=1,roles:m=[]},h,y)=>{const b=h[i+"Team"],v=Object.entries(b).filter(([w])=>!m.length||m.includes(w)).flatMap(([w,I])=>I.filter(E=>!E.used).map(E=>({...E,_line:w})));if(!v.length){h.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),y(h);return}re(v,f,`Choisir ${f} joueur(s) à booster (+${T})`,w=>{w.forEach(I=>{const E=(b[I._line]||[]).find(A=>A.cardId===I.cardId);E&&(E.boost=(E.boost||0)+T,h.log.push({text:`⚡ +${T} sur ${E.name}`,type:"info"}))}),h[i+"Team"]=b,y(h)})},DEBUFF_STAT:({value:T=1,count:f=1,roles:m=[],target:h="ai"},y,b)=>{const v=h==="home"?i:r,w=y[v+"Team"],I=h==="home"?"allié":"adverse",E=Object.entries(w).filter(([A])=>!m.length||m.includes(A)).flatMap(([A,C])=>C.filter(N=>!N.used).map(N=>({...N,_line:A})));if(!E.length){y.log.push({text:`🎯 Aucun joueur ${I}`,type:"info"}),b(y);return}re(E,f,`Choisir ${f} joueur(s) ${I}(s) (-${T})`,A=>{A.forEach(C=>{const N=(w[C._line]||[]).find(W=>W.cardId===C.cardId);N&&(N.boost=(N.boost||0)-T,y.log.push({text:`🎯 -${T} sur ${N.name}`,type:"info"}))}),y[v+"Team"]=w,b(y)})},GRAY_PLAYER:({count:T=1,roles:f=[],target:m="ai"},h,y)=>{const b=m==="home"?i:r,v=h[b+"Team"],w=m==="home"?"allié":"adverse",I=Object.entries(v).filter(([E])=>!f.length||f.includes(E)).flatMap(([E,A])=>A.filter(C=>!C.used).map(C=>({...C,_line:E})));if(!I.length){h.log.push({text:`❌ Aucun joueur ${w}`,type:"info"}),y(h);return}re(I,T,`Choisir ${T} joueur(s) ${w}(s) à exclure`,E=>{E.forEach(A=>{const C=(v[A._line]||[]).find(N=>N.cardId===A.cardId);C&&(C.used=!0,h.log.push({text:`❌ ${C.name} exclu !`,type:"info"}))}),h[b+"Team"]=v,y(h)})},REVIVE_PLAYER:({count:T=1,roles:f=[]},m,h)=>{const y=m[i+"Team"],b=Object.entries(y).filter(([v])=>!f.length||f.includes(v)).flatMap(([v,w])=>w.filter(I=>I.used).map(I=>({...I,_line:v})));if(!b.length){m.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),h(m);return}re(b,T,`Choisir ${T} joueur(s) à ressusciter`,v=>{v.forEach(w=>{const I=(y[w._line]||[]).find(E=>E.cardId===w.cardId);I&&(I.used=!1,m.log.push({text:`💫 ${I.name} ressuscité !`,type:"info"}))}),m[i+"Team"]=y,h(m)})},REMOVE_GOAL:({},T,f)=>{const m=r+"Score";T[m]>0?(T[m]--,T.log.push({text:"🚫 Dernier but annulé !",type:"info"})):T.log.push({text:"🚫 Aucun but à annuler",type:"info"}),f(T)},ADD_GOAL_DRAW:({},T,f)=>{T[i+"Score"]===T[r+"Score"]?(T[i+"Score"]++,T.log.push({text:"🎯 But bonus !",type:"info"})):T.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),f(T)},ADD_SUB:({value:T=1},f,m)=>{f.maxSubs=(f.maxSubs||3)+T,f.log.push({text:`🔄 +${T} remplacement(s)`,type:"info"}),m(f)},CUSTOM:({},T,f)=>f(T)};function re(T,f,m,h){const y=document.createElement("div");y.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let b=[];function v(){var I,E;const w=T.map(A=>{const C={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[A._line]||"#555",N=we(A,A._line)+(A.boost||0),le=b.find(U=>U.cardId===A.cardId)?"#FFD700":"rgba(255,255,255,0.25)",J=A.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${A.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${le};background:${C};overflow:hidden;cursor:pointer;${J}">
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="width:min(90vw,420px)">${st(o[r+"Team"],o[r+"Formation"],null,[],300,300)}</div>
    </div>`,i==="p1"&&setTimeout(async()=>{await P({phase:"midfield"})},5e3)}let ae=!1;function pe(){if(ae)return;const T=o[i+"Team"].MIL||[],f=o[r+"Team"].MIL||[];function m(U){return U.reduce((V,Z)=>V+we(Z,"MIL"),0)}function h(U){let V=0;for(let Z=0;Z<U.length-1;Z++){const de=Ze(U[Z],U[Z+1]);de==="#00ff88"?V+=2:de==="#FFD700"&&(V+=1)}return V}const y=m(T)+h(T),b=m(f)+h(f),v=y>=b;function w(U,V,Z){return`<div id="duel-row-${Z}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
        ...
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${U.map((de,ue)=>{const fe=ue<U.length-1?Ze(de,U[ue+1]):null,me=!fe||fe==="#ff3333"||fe==="#cc2222",be=fe==="#00ff88"?"+2":fe==="#FFD700"?"+1":"";return`<div class="duel-card duel-card-${Z}" data-idx="${ue}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease,transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
              ${Ve({...de,note:we(de,"MIL"),_line:"MIL"},58,78)}
            </div>
            ${ue<U.length-1?`<div class="duel-link duel-link-${Z}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${me?"rgba(255,255,255,0.12)":fe};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${me?"none":`0 0 8px ${fe}`}">
              ${be?`<span style="position:absolute;top:-13px;left:50%;transform:translateX(-50%);font-size:8px;font-weight:900;color:${fe}">${be}</span>`:""}
            </div>`:""}`}).join("")}
        </div>
        <div class="duel-score-line duel-score-line-${Z}" style="margin-top:10px;font-size:12px;color:rgba(255,255,255,0.55);opacity:0;transition:opacity .4s ease">
          Score: ${m(U)} + ${h(U)} liens = <b style="color:#fff">${m(U)+h(U)}</b>
        </div>
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(b);let I=!1;const E=()=>{I||(I=!0,b.remove(),setTimeout(()=>y(),50))};b.addEventListener("click",E),setTimeout(E,3500)}function xe(T,f,m){var J,U;const h=(o.gcDefs||[]).find(V=>{var Z;return V.name===T||((Z=V.name)==null?void 0:Z.toLowerCase().trim())===(T==null?void 0:T.toLowerCase().trim())}),y={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[h==null?void 0:h.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",b={purple:"#b06ce0",light_blue:"#00d4ef"}[h==null?void 0:h.color]||"#b06ce0",v=(h==null?void 0:h.name)||T,w=(h==null?void 0:h.effect)||((J=Ge[T])==null?void 0:J.desc)||"",I=h!=null&&h.image_url?`/manager-wars/icons/${h.image_url}`:null,E=((U=Ge[T])==null?void 0:U.icon)||"⚡",C=f===i?"Vous":o[f+"Name"]||"Adversaire",N=document.createElement("div");N.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",N.innerHTML=`
      <style>
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        <div id="pin-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
          ...
          <div>${U?Ve({...U,used:!1,boost:0},V,Z):"<div>—</div>"}</div>
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:6px">Appuyer pour continuer</div>`,document.body.appendChild(y);let v=!1;const w=()=>{v||(v=!0,y.remove(),setTimeout(()=>m(),50))};y.addEventListener("click",w),setTimeout(w,2200)}function B(){var f;const T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:20px;overflow-y:auto",T.innerHTML=`
      <div style="font-size:12px;color:rgba(255,255,255,0.5);letter-spacing:2px;text-transform:uppercase">Équipe adverse</div>
      <div style="font-size:18px;font-weight:900;color:#ff6b6b">${o[r+"Name"]}</div>
      <div style="width:min(90vw,420px)">${st(o[r+"Team"],o[r+"Formation"],null,[],300,300)}</div>
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(T),(f=T.querySelector("#pvp-opp-close"))==null||f.addEventListener("click",()=>T.remove())}function R(){var m;const T=o.log||[],f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column",f.innerHTML=`
      ...
      </div>`,document.body.appendChild(f),(m=f.querySelector("#pvp-hist-close"))==null||m.addEventListener("click",()=>f.remove())}async function ie(){if(o.phase!==i+"-attack")return;const T=i==="p1"?"p2":"p1",f=(o.round||0)+1,m=[...o.log||[]];m.push({type:"info",text:`⏭️ ${o[i+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const h=Te(o),y=Ae(T),b=h||!y?"finished":T+"-attack";await P({["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},pendingAttack:null,phase:b,attacker:T,round:f,log:m}),b==="finished"&&await Ne(o)}async function ee(){const T=o[i+"Team"],f=!["GK","DEF","MIL","ATT"].some(b=>(o[r+"Team"][b]||[]).some(v=>!v.used)),m=(o["selected_"+i]||[]).map(b=>{const v=(T[b._role]||[]).find(N=>N.cardId===b.cardId)||b,w=f&&["GK","DEF"].includes(b._role),I=T[b._role]||[],E=I.findIndex(N=>N.cardId===b.cardId),A=ot(I.length),C=E>=0?A[E]:v._col??1;return{...v,_line:b._role,_col:C,...w?{note_a:Math.max(1,Number(v.note_a)||0)}:{}}});if(!m.length)return;const h=Rt(m,o.modifiers[i]||{});Y(i,m.map(b=>b.cardId)),m.forEach(b=>{const v=(T[b._role]||[]).find(w=>w.cardId===b.cardId);v&&(v.used=!0)}),o["selected_"+i]=[],D();const y=[...o.log||[]];if(f){const b=(o[i+"Score"]||0)+1,v=m.map(C=>({name:C.name,note:we(C,C._line||"ATT"),portrait:ze(C),job:C.job}));y.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:v,homeTotal:h.total,aiTotal:0});const w=(o.round||0)+1,I=i==="p1"?"p2":"p1",E={...o,[i+"Team"]:T,[i+"Score"]:b},A=Te(E);F.add(w),he(v,b,o[r+"Score"]||0,!0,async()=>{await P({[i+"Team"]:T,[i+"Score"]:b,["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},pendingAttack:null,phase:A?"finished":I+"-attack",attacker:I,round:w,log:y}),A&&await Ne({...o,[i+"Score"]:b})});return}y.push({type:"pending",text:`⚔️ ${o[i+"Name"]} attaque (${h.total})`}),await P({[i+"Team"]:T,[r+"Team"]:o[r+"Team"],pendingAttack:{...h,players:m,side:i},["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},phase:r+"-defense",log:y})}async function ke(){const T=o[i+"Team"],f=(o["selected_"+i]||[]).map(J=>{const U=(T[J._role]||[]).find(fe=>fe.cardId===J.cardId)||J,V=T[J._role]||[],Z=V.findIndex(fe=>fe.cardId===J.cardId),de=ot(V.length),ue=Z>=0?de[Z]:U._col??1;return{...U,_line:J._role,_col:ue}}),m=Ot(f,o.modifiers[i]||{});Y(i,f.map(J=>J.cardId)),f.forEach(J=>{const U=(T[J._role]||[]).find(V=>V.cardId===J.cardId);U&&(U.used=!0)}),o["selected_"+i]=[],D();const h=Ht(o.pendingAttack.total,m.total,o.modifiers[i]||{}),y=o.pendingAttack.side,b=h==="attack"||(h==null?void 0:h.goal),v=y==="p1"?"p2":"p1",w=(o.round||0)+1,I=(o.pendingAttack.players||[]).map(J=>({name:J.name,note:we(J,J._line||"ATT"),portrait:ze(J),job:J.job})),E=[...o.log||[]];E.push({type:"duel",isGoal:b,homeScored:b&&y===i,text:b?`⚽ BUT de ${o[y+"Name"]} ! (${o.pendingAttack.total} vs ${m.total})`:`✋ Attaque stoppée (${o.pendingAttack.total} vs ${m.total})`,homePlayers:I,aiPlayers:f.map(J=>({name:J.name,note:we(J,J._line||"DEF"),portrait:ze(J),job:J.job})),homeTotal:o.pendingAttack.total,aiTotal:m.total});const A=b?(o[y+"Score"]||0)+1:o[y+"Score"]||0,C={...o,[i+"Team"]:T,[y+"Score"]:A},N=Te(C),W=N?"finished":v+"-attack",le=async()=>{await P({[i+"Team"]:T,[r+"Team"]:o[r+"Team"],[y+"Score"]:A,["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},pendingAttack:null,phase:W,attacker:v,round:w,log:E}),(W==="finished"||N)&&await Ne({...o,[y+"Score"]:A})};if(b){const J=y===i,U=J?A:o[i+"Score"]||0,V=J?o[r+"Score"]||0:A;F.add(w),he(I,U,V,J,le)}else await le()}function $e(T){return["MIL","ATT"].some(f=>(T[f]||[]).some(m=>!m.used))}function Ee(T){return["GK","DEF","MIL","ATT"].some(f=>(T[f]||[]).some(m=>!m.used))}function _e(T){return Ee(T)&&!$e(T)}function Ae(T){const f=o[T+"Team"],m=o[(T==="p1"?"p2":"p1")+"Team"];return!!($e(f)||!Ee(m)&&_e(f))}function Te(T){const f=["MIL","ATT"].some(E=>(T.p1Team[E]||[]).some(A=>!A.used)),m=["MIL","ATT"].some(E=>(T.p2Team[E]||[]).some(A=>!A.used)),h=Ee(T.p1Team),y=Ee(T.p2Team);return!f&&!(!y&&h)&&(!m&&!(!h&&y))}function qe(T){const f=T.p1Score||0,m=T.p2Score||0;return f===m?null:f>m?g.home_id:g.away_id}async function Ne(T){try{const f=qe(T),m=f?g.home_id===f?g.away_id:g.home_id:null;await _.from("matches").update({status:"finished",winner_id:f,home_score:T.p1Score||0,away_score:T.p2Score||0}).eq("id",n),f&&m&&Ni(f,m).catch(()=>{})}catch(f){console.error("[PvP] finishMatch:",f)}}function He(){var y;const T=o[i+"Score"],f=o[r+"Score"],m=T>f,h=T===f;Xe(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      ...
      </div>`,(D=document.getElementById("lobby-cancel"))==null||D.addEventListener("click",async()=>{k=!0,o&&(_.removeChannel(o),o=null),Y&&await _.from("friend_match_invites").update({status:"declined"}).eq("id",Y),Oe(e),u("home")})},L=async(re,M)=>{k=!0,o&&(_.removeChannel(o),o=null),await new Promise(Y=>setTimeout(Y,600)),e.isConnected&&Lo(e,t,re,M,s,l)},{data:z}=await _.from("friend_match_invites").select("*").eq("status","pending").or(`and(inviter_id.eq.${g},invitee_id.eq.${c}),and(inviter_id.eq.${c},invitee_id.eq.${g})`).order("created_at",{ascending:!1}).limit(1).maybeSingle();let F,K;if(z&&z.inviter_id===c)K=!1,F=z.id,await _.from("friend_match_invites").update({invitee_deck_id:n,invitee_ready:!0,status:"accepted"}).eq("id",F);else{K=!0;const{data:re,error:M}=await _.from("friend_match_invites").insert({inviter_id:g,invitee_id:c,friend_id:c,inviter_deck_id:n,inviter_ready:!0,status:"pending"}).select().single();if(M||!re){x("Erreur création invitation","error"),Oe(e),u("home");return}F=re.id}if($(!0,!K,F),!K){const re={home_id:c,away_id:g,home_deck_id:z.inviter_deck_id,away_deck_id:n,status:"active",mode:"friend"},M=await _.from("matches").insert(re).select().single(),Y=M.data;if(!Y){x("Erreur création match: "+(((ce=M.error)==null?void 0:ce.message)||""),"error"),Oe(e),u("home");return}await _.from("friend_match_invites").update({match_id:Y.id,status:"matched"}).eq("id",F),L(Y.id,!1);return}let q=!1;const j=re=>{k||q||(re.status==="matched"&&re.match_id?(q=!0,clearInterval(P),clearInterval(te),L(re.match_id,!0)):re.status==="declined"?(clearInterval(P),clearInterval(te),x(`${i} a décliné l'invitation`,"warning"),Oe(e),u("home")):re.invitee_ready&&$(!0,!0,F))},P=setInterval(()=>{if(k){clearInterval(P);return}_.from("matchmaking_queue").delete().eq("user_id",g).then(()=>{},()=>{})},3e3),te=setInterval(async()=>{if(k||q){clearInterval(te);return}const{data:re}=await _.from("friend_match_invites").select("*").eq("id",F).maybeSingle();re&&j(re)},1200);o=_.channel("friend-invite-"+F).on("postgres_changes",{event:"UPDATE",schema:"public",table:"friend_match_invites",filter:`id=eq.${F}`},re=>j(re.new)).subscribe()}async function Lo(e,t,n,a,p=[],d=[]){const{state:s,navigate:l,toast:c}=t,i=a?"p1":"p2",r=a?"p2":"p1",u=(p||[]).map(f=>f.id),x=(p||[]).map(f=>({id:f.id,gc_type:f.gc_type,_gcDef:f._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:g}=await _.from("matches").select("*").eq("id",n).single();if(!g){c("Match introuvable","error"),l("home");return}async function k(){const[{data:f},{data:m},{data:h},{data:y}]=await Promise.all([_.rpc("get_deck_for_match",{p_deck_id:g.home_deck_id}),_.rpc("get_deck_for_match",{p_deck_id:g.away_deck_id}),_.from("users").select("id,pseudo,club_name").eq("id",g.home_id).single(),_.from("users").select("id,pseudo,club_name").eq("id",g.away_id).single()]),b=A=>({cardId:A.card_id,position:A.position,id:A.id,firstname:A.firstname,name:A.surname_encoded,country_code:A.country_code,club_id:A.club_id,job:A.job,job2:A.job2,note_g:Number(A.note_g)||0,note_d:Number(A.note_d)||0,note_m:Number(A.note_m)||0,note_a:Number(A.note_a)||0,rarity:A.rarity,skin:A.skin,hair:A.hair,hair_length:A.hair_length,clubName:A.club_encoded_name||null,clubLogo:A.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}),v=((f==null?void 0:f.starters)||[]).map(A=>b(A)),w=((m==null?void 0:m.starters)||[]).map(A=>b(A)),I=(f==null?void 0:f.formation)||"4-4-2",E=(m==null?void 0:m.formation)||"4-4-2";return{p1Team:gt(v,I),p2Team:gt(w,E),p1Subs:((f==null?void 0:f.subs)||[]).map(A=>b(A)),p2Subs:((m==null?void 0:m.subs)||[]).map(A=>b(A)),p1Formation:I,p2Formation:E,p1Name:(h==null?void 0:h.club_name)||(h==null?void 0:h.pseudo)||"Joueur 1",p2Name:(y==null?void 0:y.club_name)||(y==null?void 0:y.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:a?u:[],gc_p2:a?[]:u,gcCardsFull_p1:a?x:[],gcCardsFull_p2:a?[]:x,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:d||[],lastActionAt:new Date().toISOString()}}let o=g.game_state&&Object.keys(g.game_state).length?g.game_state:null;if(!o)if(a){o=await k();const{data:f}=await _.from("matches").select("game_state").eq("id",n).single();!(f!=null&&f.game_state)||!Object.keys(f.game_state).length?await _.from("matches").update({game_state:o,turn_user_id:g.home_id}).eq("id",n):o=f.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let f=0;f<60&&!o;f++){await new Promise(h=>setTimeout(h,400));const{data:m}=await _.from("matches").select("game_state").eq("id",n).single();m!=null&&m.game_state&&Object.keys(m.game_state).length&&(o=m.game_state)}if(!o){c("Erreur de synchronisation","error"),l("home");return}o.gc_p2=u,o.gcCardsFull_p2=x,await _.from("matches").update({game_state:o}).eq("id",n)}let $=!1,L=!1,z=null,F=!1;const K=new Set,q=new Set;function j(f){var A,C;try{_.removeChannel(P)}catch{}const m=o[i+"Score"]||0,h=o[r+"Score"]||0;if(!L){const N=o.p1Score||0,W=o.p2Score||0,le=(o.usedGc_p1||[]).length,J=(o.usedGc_p2||[]).length,U=f.winner_id||(N>W?g.home_id:W>N?g.away_id:null);(U?s.profile.id===U:s.profile.id<(a?g.away_id:g.home_id))&&(L=!0,wn({player1Id:g.home_id,player2Id:g.away_id,score1:N,score2:W,gc1:le,gc2:J}).catch(Z=>console.warn("[FriendMatch] updateStats:",Z)))}let y,b;f.winner_id?(y=f.winner_id===s.profile.id,b=!1):f.forfeit?(y=m>h,b=!1):(b=m===h,y=m>h),(A=document.getElementById("pvp-end-overlay"))==null||A.remove();const v=document.createElement("div");v.id="pvp-end-overlay",v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const w=b?"🤝":y?"🏆":"😞",I=b?"MATCH NUL":y?"VICTOIRE !":"DÉFAITE",E=b?"#fff":y?"#FFD700":"#ff6b6b";v.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(v),(C=v.querySelector("#pvp-end-home"))==null||C.addEventListener("click",()=>{v.remove(),Oe(e),l("home")})}const P=_.channel("pvp-match-"+n).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${n}`},f=>{const m=f.new;try{if(m.status==="finished"||m.forfeit){if($)return;$=!0,z&&(clearInterval(z),z=null),m.game_state&&(o=m.game_state),j(m);return}if(m.game_state){const h=o;o=m.game_state;const y=o._lastGC;if(y&&y.seq&&!q.has(y.seq)&&(q.add(y.seq),y.by!==i)){ge(y.type,y.by,()=>Q());return}const b=h[i+"Score"]||0,v=h[r+"Score"]||0,w=o[i+"Score"]||0,I=o[r+"Score"]||0,E=w>b,A=I>v;if((E||A)&&!K.has(o.round)){K.add(o.round);const C=[...o.log||[]].reverse().find(W=>W.isGoal),N=((C==null?void 0:C.homePlayers)||[]).map(W=>({name:W.name,note:W.note,portrait:W.portrait,job:W.job}));xe(N,w,I,E,()=>Q());return}Q()}}catch(h){console.error("[PvP] crash:",h)}}).subscribe();async function te(f){Object.assign(o,f),o.lastActionAt=new Date().toISOString();const{error:m}=await _.from("matches").update({game_state:o}).eq("id",n);m&&c("Erreur de synchronisation","error");try{Q()}catch(h){console.error("[PvP] renderPvpScreen crash:",h)}}async function ce(){if($)return;$=!0,z&&(clearInterval(z),z=null);const f=a?g.away_id:g.home_id,m=a?"p2":"p1",h=a?"p1":"p2",y={...o,[m+"Score"]:3,[h+"Score"]:0,phase:"finished"};try{await _.from("matches").update({status:"finished",forfeit:!0,winner_id:f,home_score:y.p1Score||0,away_score:y.p2Score||0,game_state:y}).eq("id",n)}catch{}try{_.removeChannel(P)}catch{}setTimeout(()=>{Oe(e),l("home")},800)}const re={BOOST_STAT:({value:f=1,count:m=1,roles:h=[]},y,b)=>{const v=y[i+"Team"],w=Object.entries(v).filter(([I])=>!h.length||h.includes(I)).flatMap(([I,E])=>E.filter(A=>!A.used).map(A=>({...A,_line:I})));if(!w.length){y.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),b(y);return}M(w,m,`Choisir ${m} joueur(s) à booster (+${f})`,I=>{I.forEach(E=>{const A=(v[E._line]||[]).find(C=>C.cardId===E.cardId);A&&(A.boost=(A.boost||0)+f,y.log.push({text:`⚡ +${f} sur ${A.name}`,type:"info"}))}),y[i+"Team"]=v,b(y)})},DEBUFF_STAT:({value:f=1,count:m=1,roles:h=[],target:y="ai"},b,v)=>{const w=y==="home"?i:r,I=b[w+"Team"],E=y==="home"?"allié":"adverse",A=Object.entries(I).filter(([C])=>!h.length||h.includes(C)).flatMap(([C,N])=>N.filter(W=>!W.used).map(W=>({...W,_line:C})));if(!A.length){b.log.push({text:`🎯 Aucun joueur ${E}`,type:"info"}),v(b);return}M(A,m,`Choisir ${m} joueur(s) ${E}(s) (-${f})`,C=>{C.forEach(N=>{const W=(I[N._line]||[]).find(le=>le.cardId===N.cardId);W&&(W.boost=(W.boost||0)-f,b.log.push({text:`🎯 -${f} sur ${W.name}`,type:"info"}))}),b[w+"Team"]=I,v(b)})},GRAY_PLAYER:({count:f=1,roles:m=[],target:h="ai"},y,b)=>{const v=h==="home"?i:r,w=y[v+"Team"],I=h==="home"?"allié":"adverse",E=Object.entries(w).filter(([A])=>!m.length||m.includes(A)).flatMap(([A,C])=>C.filter(N=>!N.used).map(N=>({...N,_line:A})));if(!E.length){y.log.push({text:`❌ Aucun joueur ${I}`,type:"info"}),b(y);return}M(E,f,`Choisir ${f} joueur(s) ${I}(s) à exclure`,A=>{A.forEach(C=>{const N=(w[C._line]||[]).find(W=>W.cardId===C.cardId);N&&(N.used=!0,y.log.push({text:`❌ ${N.name} exclu !`,type:"info"}))}),y[v+"Team"]=w,b(y)})},REVIVE_PLAYER:({count:f=1,roles:m=[]},h,y)=>{const b=h[i+"Team"],v=Object.entries(b).filter(([w])=>!m.length||m.includes(w)).flatMap(([w,I])=>I.filter(E=>E.used).map(E=>({...E,_line:w})));if(!v.length){h.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),y(h);return}M(v,f,`Choisir ${f} joueur(s) à ressusciter`,w=>{w.forEach(I=>{const E=(b[I._line]||[]).find(A=>A.cardId===I.cardId);E&&(E.used=!1,h.log.push({text:`💫 ${E.name} ressuscité !`,type:"info"}))}),h[i+"Team"]=b,y(h)})},REMOVE_GOAL:({},f,m)=>{const h=r+"Score";f[h]>0?(f[h]--,f.log.push({text:"🚫 Dernier but annulé !",type:"info"})):f.log.push({text:"🚫 Aucun but à annuler",type:"info"}),m(f)},ADD_GOAL_DRAW:({},f,m)=>{f[i+"Score"]===f[r+"Score"]?(f[i+"Score"]++,f.log.push({text:"🎯 But bonus !",type:"info"})):f.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),m(f)},ADD_SUB:({value:f=1},m,h)=>{m.maxSubs=(m.maxSubs||3)+f,m.log.push({text:`🔄 +${f} remplacement(s)`,type:"info"}),h(m)},CUSTOM:({},f,m)=>m(f)};function M(f,m,h,y){const b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let v=[];function w(){var E,A;const I=f.map(C=>{const N={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[C._line]||"#555",W=we(C,C._line)+(C.boost||0),J=v.find(V=>V.cardId===C.cardId)?"#FFD700":"rgba(255,255,255,0.25)",U=C.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${C.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${J};background:${N};overflow:hidden;cursor:pointer;${U}">
        ...
        </div>`,(E=b.querySelector("#pp-close"))==null||E.addEventListener("click",()=>b.remove()),b.querySelectorAll(".pp-item").forEach(C=>{C.addEventListener("click",()=>{const N=C.dataset.cid,W=f.find(J=>J.cardId===N),le=v.findIndex(J=>J.cardId===N);W&&(le>-1?v.splice(le,1):v.length<m&&v.push(W),w())})}),(A=b.querySelector("#pp-confirm"))==null||A.addEventListener("click",()=>{b.remove(),y(v)})}w(),document.body.appendChild(b)}async function Y(f,m){const y=(o["gcCardsFull_"+i]||[]).find(I=>I.id===f),b=(y==null?void 0:y._gcDef)||(o.gcDefs||[]).find(I=>{var E;return I.name===m||((E=I.name)==null?void 0:E.toLowerCase().trim())===(m==null?void 0:m.toLowerCase().trim())}),v=[...o["usedGc_"+i]||[],f],w={type:m,by:i,seq:(o._gcAnimSeq||0)+1};q.add(w.seq),ge(m,i,async()=>{if(b!=null&&b.effect_type&&b.effect_type!=="CUSTOM"){const E=re[b.effect_type];if(E){const A={...o};E(b.effect_params||{},A,async C=>{C["usedGc_"+i]=v,C._lastGC=w,C._gcAnimSeq=w.seq,await te(C)});return}}const I={...o};switch(m){case"Remplacement+":I.maxSubs=(I.maxSubs||3)+1,I.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const E=r+"Score";I[E]>0&&(I[E]--,I.log.push({text:"🚫 But annulé",type:"info"}));break}}I["usedGc_"+i]=v,I._lastGC=w,I._gcAnimSeq=w.seq,await te(I)})}function ne(f,m){const h="usedCardIds_"+f,y=new Set(o[h]||[]);m.forEach(b=>y.add(b)),o[h]=[...y]}function D(){for(const f of["p1","p2"]){const m=new Set(o["usedCardIds_"+f]||[]),h=o[f+"Team"];if(h)for(const y of["GK","DEF","MIL","ATT"])(h[y]||[]).forEach(b=>{b.used=m.has(b.cardId)})}}function Q(){var Qe,mt,xt,yt,ht,X;if(o.phase==="reveal")return ae();if(o.phase==="midfield")return he();if(o.phase==="finished")return T();const f=o[i+"Team"],m=o[r+"Team"];D();const h=o[i+"Score"],y=o[r+"Score"],b=o[i+"Name"],v=o[r+"Name"],w=o.phase===i+"-attack",I=o.phase===i+"-defense",E=Array.isArray(o["selected_"+i])?o["selected_"+i]:[],A=E.map(G=>G.cardId),C=window.innerWidth>=700,N=o[i+"Subs"]||[],W=o["usedSubIds_"+i]||[],le=N.filter(G=>!W.includes(G.cardId)),J=o["gcCardsFull_"+i]||[],U=o["usedGc_"+i]||[],V=J.filter(G=>!U.includes(G.id)),Z=o.boostOwner===i&&!o.boostUsed,de=!["GK","DEF","MIL","ATT"].some(G=>(m[G]||[]).some(oe=>!oe.used)),ue=[...f.MIL||[],...f.ATT||[]].filter(G=>!G.used),fe=w&&de&&ue.length===0?[...f.GK||[],...f.DEF||[]].filter(G=>!G.used).map(G=>G.cardId):[];function me(G,oe,se){var wt,mi;const ye=G._gcDef,Ye={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ye==null?void 0:ye.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",We={purple:"#b06ce0",light_blue:"#00d4ef"}[ye==null?void 0:ye.color]||"#b06ce0",Je=(ye==null?void 0:ye.name)||G.gc_type,et=(ye==null?void 0:ye.effect)||((wt=Ge[G.gc_type])==null?void 0:wt.desc)||"",Mt=ye!=null&&ye.image_url?`/manager-wars/icons/${ye.image_url}`:null,St=((mi=Ge[G.gc_type])==null?void 0:mi.icon)||"⚡",bt=Math.round(se*.22),lt=Math.round(se*.22),vt=se-bt-lt,Ct=Je.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${G.id}" data-gc-type="${G.gc_type}"
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(v);let E=!1;const A=()=>{E||(E=!0,v.remove(),setTimeout(()=>b(),50))};v.addEventListener("click",A),setTimeout(A,3500)}function ge(f,m,h){var U,V;const y=(o.gcDefs||[]).find(Z=>{var de;return Z.name===f||((de=Z.name)==null?void 0:de.toLowerCase().trim())===(f==null?void 0:f.toLowerCase().trim())}),b={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[y==null?void 0:y.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",v={purple:"#b06ce0",light_blue:"#00d4ef"}[y==null?void 0:y.color]||"#b06ce0",w=(y==null?void 0:y.name)||f,I=(y==null?void 0:y.effect)||((U=Ge[f])==null?void 0:U.desc)||"",E=y!=null&&y.image_url?`/manager-wars/icons/${y.image_url}`:null,A=((V=Ge[f])==null?void 0:V.icon)||"⚡",N=m===i?"Vous":o[m+"Name"]||"Adversaire",W=document.createElement("div");W.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",W.innerHTML=`
      <style>
        @keyframes gcFlipIn{0%{transform:perspective(800px) rotateY(90deg) scale(.7);opacity:0}55%{transform:perspective(800px) rotateY(-12deg) scale(1.08);opacity:1}100%{transform:perspective(800px) rotateY(0) scale(1);opacity:1}}
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(W);let le=!1;const J=()=>{le||(le=!0,W.remove(),setTimeout(()=>h&&h(),50))};W.addEventListener("click",J),setTimeout(J,3e3)}function O(f,m){var W,le,J,U;const y=(o["gcCardsFull_"+i]||[]).find(V=>V.id===f),b=y==null?void 0:y._gcDef,v={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[b==null?void 0:b.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",w={purple:"#b06ce0",light_blue:"#00d4ef"}[b==null?void 0:b.color]||"#b06ce0",I=(b==null?void 0:b.name)||m,E=(b==null?void 0:b.effect)||((W=Ge[m])==null?void 0:W.desc)||"Carte spéciale.",A=b!=null&&b.image_url?`/manager-wars/icons/${b.image_url}`:null,C=((le=Ge[m])==null?void 0:le.icon)||"⚡",N=document.createElement("div");N.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",N.innerHTML=`
      ...
      </div>`,document.body.appendChild(h),(y=h.querySelector("#bp-close"))==null||y.addEventListener("click",()=>h.remove()),h.querySelectorAll(".bp-item").forEach(b=>{b.addEventListener("click",async()=>{const v=b.dataset.cid,w=m.find(E=>E.cardId===v);if(!w)return;const I=(f[w._line]||[]).find(E=>E.cardId===v);I&&(I.boost=(I.boost||0)+o.boostValue),h.remove(),await te({[i+"Team"]:f,boostUsed:!0})})})}function S(f=null){var le,J;if(!(o.phase===i+"-attack")){c("Remplacement uniquement avant votre attaque","warning");return}const h=o[i+"Team"],y=o["usedSubIds_"+i]||[],b=o.maxSubs||3;if(y.length>=b){c(`Maximum ${b} remplacements atteint`,"warning");return}const v=Object.entries(h).flatMap(([U,V])=>(V||[]).filter(Z=>Z.used).map(Z=>({...Z,_line:U}))),w=(o[i+"Subs"]||[]).filter(U=>!y.includes(U.cardId));if(!v.length){c("Aucun joueur utilisé à remplacer","warning");return}if(!w.length){c("Aucun remplaçant disponible","warning");return}let I=Math.max(0,v.findIndex(U=>U.cardId===f));const E=((le=v[I])==null?void 0:le._line)||((J=v[I])==null?void 0:J.job);let A=Math.max(0,w.findIndex(U=>U.job===E)),C=!1;const N=document.createElement("div");N.id="pvp-sub-overlay",N.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function W(){var me,be,Se,Pe,Fe,Ie;const U=v[I],V=w[A],Z=Math.min(130,Math.round((window.innerWidth-90)/2)),de=Math.round(Z*1.35),ue=Le=>`background:rgba(255,255,255,0.12);border:none;color:${Le?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Le?"default":"pointer"};flex-shrink:0`;N.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        ...
        <div id="pout-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
          ...
          <div>${U?Ve({...U,used:!1,boost:0},Z,de):"<div>—</div>"}</div>
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:6px">Appuyer pour continuer</div>`,document.body.appendChild(b);let w=!1;const I=()=>{w||(w=!0,b.remove(),setTimeout(()=>h(),50))};b.addEventListener("click",I),setTimeout(I,2200)}function R(){var m;const f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:20px;overflow-y:auto",f.innerHTML=`
      <div style="font-size:12px;color:rgba(255,255,255,0.5);letter-spacing:2px;text-transform:uppercase">Équipe adverse</div>
      <div style="font-size:18px;font-weight:900;color:#ff6b6b">${o[r+"Name"]}</div>
      <div style="width:min(90vw,420px)">${st(o[r+"Team"],o[r+"Formation"],null,[],300,300)}</div>
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(f),(m=f.querySelector("#pvp-opp-close"))==null||m.addEventListener("click",()=>f.remove())}function ie(){var h;const f=o.log||[],m=document.createElement("div");m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column",m.innerHTML=`
      <div style="display:flex;align-items:center;padding:14px 16px;border-bottom:1px solid rgba(255,255,255,0.1);flex-shrink:0">
        <div style="flex:1;font-size:14px;font-weight:700;color:#fff">📋 Historique</div>
        <button id="pvp-hist-close" style="background:none;border:none;color:rgba(255,255,255,0.6);font-size:20px;cursor:pointer">✕</button>
      </div>
      ...
      </div>`,document.body.appendChild(m),(h=m.querySelector("#pvp-hist-close"))==null||h.addEventListener("click",()=>m.remove())}async function ee(){if(o.phase!==i+"-attack")return;const f=i==="p1"?"p2":"p1",m=(o.round||0)+1,h=[...o.log||[]];h.push({type:"info",text:`⏭️ ${o[i+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const y=qe(o),b=Te(f),v=y||!b?"finished":f+"-attack";await te({["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},pendingAttack:null,phase:v,attacker:f,round:m,log:h}),v==="finished"&&await He(o)}async function ke(){const f=o[i+"Team"],m=!["GK","DEF","MIL","ATT"].some(v=>(o[r+"Team"][v]||[]).some(w=>!w.used)),h=(o["selected_"+i]||[]).map(v=>{const w=(f[v._role]||[]).find(W=>W.cardId===v.cardId)||v,I=m&&["GK","DEF"].includes(v._role),E=f[v._role]||[],A=E.findIndex(W=>W.cardId===v.cardId),C=ot(E.length),N=A>=0?C[A]:w._col??1;return{...w,_line:v._role,_col:N,...I?{note_a:Math.max(1,Number(w.note_a)||0)}:{}}});if(!h.length)return;const y=Rt(h,o.modifiers[i]||{});ne(i,h.map(v=>v.cardId)),h.forEach(v=>{const w=(f[v._role]||[]).find(I=>I.cardId===v.cardId);w&&(w.used=!0)}),o["selected_"+i]=[],Q();const b=[...o.log||[]];if(m){const v=(o[i+"Score"]||0)+1,w=h.map(N=>({name:N.name,note:we(N,N._line||"ATT"),portrait:ze(N),job:N.job}));b.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:w,homeTotal:y.total,aiTotal:0});const I=(o.round||0)+1,E=i==="p1"?"p2":"p1",A={...o,[i+"Team"]:f,[i+"Score"]:v},C=qe(A);K.add(I),xe(w,v,o[r+"Score"]||0,!0,async()=>{await te({[i+"Team"]:f,[i+"Score"]:v,["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},pendingAttack:null,phase:C?"finished":E+"-attack",attacker:E,round:I,log:b}),C&&await He({...o,[i+"Score"]:v})});return}b.push({type:"pending",text:`⚔️ ${o[i+"Name"]} attaque (${y.total})`}),await te({[i+"Team"]:f,[r+"Team"]:o[r+"Team"],pendingAttack:{...y,players:h,side:i},["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},phase:r+"-defense",log:b})}async function $e(){const f=o[i+"Team"],m=(o["selected_"+i]||[]).map(U=>{const V=(f[U._role]||[]).find(me=>me.cardId===U.cardId)||U,Z=f[U._role]||[],de=Z.findIndex(me=>me.cardId===U.cardId),ue=ot(Z.length),fe=de>=0?ue[de]:V._col??1;return{...V,_line:U._role,_col:fe}}),h=Ot(m,o.modifiers[i]||{});ne(i,m.map(U=>U.cardId)),m.forEach(U=>{const V=(f[U._role]||[]).find(Z=>Z.cardId===U.cardId);V&&(V.used=!0)}),o["selected_"+i]=[],Q();const y=Ht(o.pendingAttack.total,h.total,o.modifiers[i]||{}),b=o.pendingAttack.side,v=y==="attack"||(y==null?void 0:y.goal),w=b==="p1"?"p2":"p1",I=(o.round||0)+1,E=(o.pendingAttack.players||[]).map(U=>({name:U.name,note:we(U,U._line||"ATT"),portrait:ze(U),job:U.job})),A=[...o.log||[]];A.push({type:"duel",isGoal:v,homeScored:v&&b===i,text:v?`⚽ BUT de ${o[b+"Name"]} ! (${o.pendingAttack.total} vs ${h.total})`:`✋ Attaque stoppée (${o.pendingAttack.total} vs ${h.total})`,homePlayers:E,aiPlayers:m.map(U=>({name:U.name,note:we(U,U._line||"DEF"),portrait:ze(U),job:U.job})),homeTotal:o.pendingAttack.total,aiTotal:h.total});const C=v?(o[b+"Score"]||0)+1:o[b+"Score"]||0,N={...o,[i+"Team"]:f,[b+"Score"]:C},W=qe(N),le=W?"finished":w+"-attack",J=async()=>{await te({[i+"Team"]:f,[r+"Team"]:o[r+"Team"],[b+"Score"]:C,["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},pendingAttack:null,phase:le,attacker:w,round:I,log:A}),(le==="finished"||W)&&await He({...o,[b+"Score"]:C})};if(v){const U=b===i,V=U?C:o[i+"Score"]||0,Z=U?o[r+"Score"]||0:C;K.add(I),xe(E,V,Z,U,J)}else await J()}function Ee(f){return["MIL","ATT"].some(m=>(f[m]||[]).some(h=>!h.used))}function _e(f){return["GK","DEF","MIL","ATT"].some(m=>(f[m]||[]).some(h=>!h.used))}function Ae(f){return _e(f)&&!Ee(f)}function Te(f){const m=o[f+"Team"],h=o[(f==="p1"?"p2":"p1")+"Team"];return!!(Ee(m)||!_e(h)&&Ae(m))}function qe(f){const m=["MIL","ATT"].some(A=>(f.p1Team[A]||[]).some(C=>!C.used)),h=["MIL","ATT"].some(A=>(f.p2Team[A]||[]).some(C=>!C.used)),y=_e(f.p1Team),b=_e(f.p2Team);return!m&&!(!b&&y)&&(!h&&!(!y&&b))}function Ne(f){const m=f.p1Score||0,h=f.p2Score||0;return m===h?null:m>h?g.home_id:g.away_id}async function He(f){try{const m=Ne(f),h=m?g.home_id===m?g.away_id:g.home_id:null;await _.from("matches").update({status:"finished",winner_id:m,home_score:f.p1Score||0,away_score:f.p2Score||0}).eq("id",n),m&&h&&Ni(m,h).catch(()=>{})}catch(m){console.error("[PvP] finishMatch:",m)}}function T(){var b;const f=o[i+"Score"],m=o[r+"Score"],h=f>m,y=f===m;Xe(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      ...
      <button id="pvp-home" style="padding:16px 40px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">🏠 Retour</button>
    </div>`,(b=document.getElementById("pvp-home"))==null||b.addEventListener("click",()=>{try{_.removeChannel(P)}catch{}Oe(e),l("home")})}Q()}const zo={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"};async function Mo(e,t){const{state:n,toast:a}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement du marché...</div>',await fi(e,t)}async function fi(e,t){const{state:n,toast:a}=t,{data:p}=await _.from("market_listings").select(`id, price, status, listed_at, seller_id,
