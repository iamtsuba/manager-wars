...
import{s as E,F as hi,h as Pt,j as Yi,l as at,i as Ao,k as zo,b as So}from"./formation-links-Dnttc-Cp.js";const Lo="/";function Li(e,{navigate:t,toast:i}){let s="login";const l=()=>{var a,c,g,o,r,m;const d=s==="login";e.innerHTML=`
    <div style="
    ...
    </style>`,(a=document.getElementById("tab-login-btn"))==null||a.addEventListener("click",()=>{s="login",l()}),(c=document.getElementById("tab-reg-btn"))==null||c.addEventListener("click",()=>{s="register",l()}),d?((g=document.getElementById("login-password"))==null||g.addEventListener("keydown",b=>{var u;b.key==="Enter"&&((u=document.getElementById("login-btn"))==null||u.click())}),(o=document.getElementById("login-btn"))==null||o.addEventListener("click",async()=>{const b=document.getElementById("login-email").value.trim(),u=document.getElementById("login-password").value,k=document.getElementById("login-error");if(k.textContent="",!b||!u){k.textContent="Remplissez tous les champs.";return}const n=document.getElementById("login-btn");n.textContent="⏳ Connexion…",n.disabled=!0;const{error:p}=await E.auth.signInWithPassword({email:b,password:u});if(n.textContent="⚽ Se connecter",n.disabled=!1,p){k.textContent=p.message.includes("Invalid")?"Email ou mot de passe incorrect.":p.message;return}window.location.reload()})):((r=document.getElementById("reg-confirm"))==null||r.addEventListener("keydown",b=>{var u;b.key==="Enter"&&((u=document.getElementById("reg-btn"))==null||u.click())}),(m=document.getElementById("reg-btn"))==null||m.addEventListener("click",async()=>{const b=document.getElementById("reg-email").value.trim(),u=document.getElementById("reg-password").value,k=document.getElementById("reg-confirm").value,n=document.getElementById("reg-error");if(n.textContent="",!b||!u||!k){n.textContent="Remplissez tous les champs.";return}if(u.length<6){n.textContent="Mot de passe trop court (min. 6 caractères).";return}if(u!==k){n.textContent="Les mots de passe ne correspondent pas.";return}const p=document.getElementById("reg-btn");p.textContent="⏳ Création…",p.disabled=!0;const{error:I}=await E.auth.signUp({email:b,password:u});if(p.textContent="🚀 Créer mon compte",p.disabled=!1,I){n.textContent=I.message;return}i("Compte créé ! Connecte-toi pour commencer.","success",4e3),s="login",l(),setTimeout(()=>{const L=document.getElementById("login-email");L&&(L.value=b)},50)}))};l()}function Io(e,{state:t,navigate:i,toast:s,refreshProfile:l}){let d="#1A6B3C",a="#D4A017";e.innerHTML=`
  ...
  `;function c(){var k;const o=document.getElementById("logo-preview"),r=document.getElementById("logo-initials"),m=((k=document.getElementById("setup-club"))==null?void 0:k.value)||"MW",b=m.trim().split(" ").filter(Boolean),u=b.length>=2?(b[0][0]+b[1][0]).toUpperCase():m.slice(0,2).toUpperCase();o&&(o.style.background=a,o.style.borderColor=d),r&&(r.textContent=u,r.style.color=d)}document.getElementById("color1").addEventListener("input",o=>{d=o.target.value,document.getElementById("swatch1").style.background=d,c()}),document.getElementById("color2").addEventListener("input",o=>{a=o.target.value,document.getElementById("swatch2").style.background=a,c()});function g(o){document.querySelectorAll(".setup-step").forEach(r=>r.classList.remove("active")),document.getElementById(`step-${o}`).classList.add("active"),document.getElementById("step-num").textContent=o,document.getElementById("progress-fill").style.width=`${Math.round(o/3*100)}%`,o===3&&c()}document.getElementById("step1-next").addEventListener("click",async()=>{const o=document.getElementById("setup-pseudo").value.trim(),r=document.getElementById("step1-error");if(r.textContent="",o.length<3){r.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:m}=await E.from("users").select("id").eq("pseudo",o).maybeSingle();if(m){r.textContent="Ce pseudo est déjà pris.";return}g(2)}),document.getElementById("step2-back").addEventListener("click",()=>g(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const o=document.getElementById("setup-club").value.trim(),r=document.getElementById("step2-error");if(r.textContent="",o.length<2){r.textContent="Nom trop court (min. 2 caractères).";return}const{data:m}=await E.from("users").select("id").eq("club_name",o).maybeSingle();if(m){r.textContent="Ce nom de club est déjà pris.";return}g(3)}),document.getElementById("step3-back").addEventListener("click",()=>g(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const o=document.getElementById("setup-pseudo").value.trim(),r=document.getElementById("setup-club").value.trim(),m=document.getElementById("step3-error"),b=document.getElementById("step3-finish");m.textContent="",b.disabled=!0,b.textContent="Création en cours…";try{const{error:u}=await E.from("users").insert({id:t.user.id,pseudo:o,club_name:r,club_color1:d,club_color2:a,credits:1e4});if(u)throw u;await Mo(t.user.id),await l(),s(`Bienvenue ${o} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(u){m.textContent=u.message,b.disabled=!1,b.textContent="🚀 Créer mon profil !"}})}async function Mo(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await E.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const jo="modulepreload",Co=function(e){return"/"+e},Ii={},ti=function(t,i,s){let l=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),c=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));l=Promise.allSettled(i.map(g=>{if(g=Co(g),g in Ii)return;Ii[g]=!0;const o=g.endsWith(".css"),r=o?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${g}"]${r}`))return;const m=document.createElement("link");if(m.rel=o?"stylesheet":jo,o||(m.as="script"),m.crossOrigin="",m.href=g,c&&m.setAttribute("nonce",c),document.head.appendChild(m),o)return new Promise((b,u)=>{m.addEventListener("load",b),m.addEventListener("error",()=>u(new Error(`Unable to preload CSS for ${g}`)))})}))}function d(a){const c=new Event("vite:preloadError",{cancelable:!0});if(c.payload=a,window.dispatchEvent(c),!c.defaultPrevented)throw a}return l.then(a=>{for(const c of a||[])c.status==="rejected"&&d(c.reason);return t().catch(d)})},Dt="#1A6B3C",Gt="#cc2222",qo="#D4A017",Mi="#888";async function Bo(e,t){const{state:i,toast:s}=t;e.innerHTML=`
    ...
    </div>`,await Wi(i,s,t),document.getElementById("btn-add-friend").addEventListener("click",()=>Do(i,s)),document.getElementById("btn-accept-friend").addEventListener("click",()=>Ji(i,s,null,t))}async function Wi(e,t,i={}){const{navigate:s}=i,l=e.user.id,{data:d,error:a}=await E.from("friendships").select("id, requester_id, addressee_id").eq("status","accepted").or(`requester_id.eq.${l},addressee_id.eq.${l}`),{count:c}=await E.from("friendships").select("id",{count:"exact",head:!0}).eq("addressee_id",l).eq("status","pending"),g=document.getElementById("pending-badge");g&&(c>0?(g.style.display="flex",g.textContent=c):g.style.display="none");const o=document.getElementById("friends-list");if(!o)return;if(a){console.error("[Friends] Erreur:",a),o.innerHTML=`<div style="color:${Gt};text-align:center;padding:16px">Erreur chargement : ${a.message}</div>`;return}const r=(d||[]).map(u=>u.requester_id===l?u.addressee_id:u.requester_id);let m={};if(r.length){const{data:u}=await E.from("users").select("id, pseudo, club_name, last_seen_at, club_color1, club_color2").in("id",r);(u||[]).forEach(k=>{m[k.id]=k})}const b=(d||[]).map(u=>({friendshipId:u.id,friend:m[u.requester_id===l?u.addressee_id:u.requester_id]||{pseudo:"?"}}));if(!b.length){o.innerHTML=`
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      ${b.map(({friendshipId:u,friend:k})=>Fo(k,u)).join("")}
    </div>`,o.querySelectorAll("[data-stats]").forEach(u=>{u.addEventListener("click",()=>Go(e,u.dataset.stats,u.dataset.friendName))}),o.querySelectorAll("[data-match]").forEach(u=>{u.addEventListener("click",()=>{const k=u.dataset.friendId,n=u.dataset.friendName;console.log("[Friends] clic match",{fid:k,fname:n,hasNavigate:typeof s}),typeof s=="function"?s("match",{matchMode:"friend",friendId:k,friendName:n}):t("Erreur navigation","error")})})}function Fo(e,t){const i=e.club_name||e.pseudo||"?",s=e.pseudo||"",l=(s||i).slice(0,2).toUpperCase(),d=e.club_color2||Dt,a=e.club_color1||"#ffffff",c=e.last_seen_at?new Date(e.last_seen_at):null,g=c&&Date.now()-c.getTime()<3*60*1e3;return`
    ...
    <div class="popup-box">
      ...
      ${r.length?`<div style="display:flex;flex-direction:column;gap:8px;max-height:50vh;overflow-y:auto;margin-bottom:14px">
            ${r.map(b=>{var u,k,n;return`
              <div style="display:flex;align-items:center;gap:10px;background:#f9f9f9;border-radius:10px;padding:10px 12px">
                <div style="flex:1;font-size:14px;font-weight:700">${((u=b.requester)==null?void 0:u.club_name)||((k=b.requester)==null?void 0:k.pseudo)||"?"}
    ...
    ${wi()}`,document.body.appendChild(o);const m=()=>o.remove();o.querySelector("#pending-close").addEventListener("click",m),o.addEventListener("click",b=>{b.target===o&&m()}),o.querySelectorAll("[data-accept]").forEach(b=>{b.addEventListener("click",async()=>{const{error:u}=await E.from("friendships").update({status:"accepted"}).eq("id",b.dataset.accept);if(u){t("Erreur : "+u.message,"error");return}b.closest("div[style]").remove(),t("✅ Ami accepté !","success"),Wi(e,t,s),i&&i()})}),o.querySelectorAll("[data-decline]").forEach(b=>{b.addEventListener("click",async()=>{await E.from("friendships").delete().eq("id",b.dataset.decline),b.closest("div[style]").remove(),t("Demande refusée","info"),i&&i()})})}async function Go(e,t,i){const s=e.user.id,[l,d]=[s,t].sort(),a=s===l,{data:c}=await E.from("friend_match_stats").select("*").eq("player1_id",l).eq("player2_id",d).single(),g=e.profile.club_name||e.profile.pseudo||"Moi",o=c||{},r=a?o.wins_p1||0:o.wins_p2||0,m=a?o.wins_p2||0:o.wins_p1||0,b=o.draws||0,u=a?o.goals_p1||0:o.goals_p2||0,k=a?o.goals_p2||0:o.goals_p1||0,n=a?o.gc_used_p1||0:o.gc_used_p2||0,p=a?o.gc_used_p2||0:o.gc_used_p1||0,I=o.matches_total||0,L=(H,C,D,j=Dt,W=Gt)=>`
    <div style="display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:8px;padding:10px 0;border-bottom:1px solid #f0f0f0">
      ...
      <div style="text-align:left;font-size:18px;font-weight:900;color:${W}">${D}</div>
    ...
    <div class="popup-box" style="max-width:380px">
      ...
      ${I===0?`
        ...
        ${L("Défaites",m,r)}
        ${L("Buts marqués",u,k)}
        ${L("Buts encaissés",k,u,Gt,Dt)}
        ${L("GC utilisés ⚡",n,p,"#7a28b8","#7a28b8")}
  ...
  `,e.querySelectorAll("[data-nav]").forEach(c=>{c.addEventListener("click",g=>{g.preventDefault(),i(c.dataset.nav)})}),(d=document.getElementById("nav-rankings"))==null||d.addEventListener("click",()=>i("rankings")),(a=document.getElementById("nav-matches"))==null||a.addEventListener("click",()=>i("matches")),e.querySelectorAll("[data-action]").forEach(c=>{c.addEventListener("click",()=>{c.classList.add("tapped"),setTimeout(()=>c.classList.remove("tapped"),200);const g=c.dataset.action;if(g==="match-ai"){Ho(i);return}if(g==="match-random"){i("match",{matchMode:"random"});return}if(g==="match-friend"){i("friends");return}if(g==="mini-league"){i("mini-league");return}if(g==="ranked"){ctx.toast("Mode Ranked — Bientôt disponible !","info");return}s("Bientôt disponible","info")})}),window.innerWidth<768&&requestAnimationFrame(()=>{var H,C,D,j,W,de;const c=((H=window.visualViewport)==null?void 0:H.height)||window.innerHeight,g=((C=document.querySelector(".top-nav"))==null?void 0:C.offsetHeight)||56,o=((D=document.querySelector(".bottom-nav"))==null?void 0:D.offsetHeight)||60,r=((j=e.querySelector(".hero-compact"))==null?void 0:j.offsetHeight)||52,m=e.querySelector(".ranked-banner");e.querySelector(".play-grid");const b=((de=(W=e.querySelector("#logout-btn"))==null?void 0:W.closest("div"))==null?void 0:de.offsetHeight)||44,u=["friend-requests-banner","match-invite-banner","ongoing-match-banner"].reduce((re,P)=>{var X;return re+(((X=document.getElementById(P))==null?void 0:X.offsetHeight)||0)},0),k=14*5,n=c-g-o-r-b-u-k,p=Math.max(80,Math.round(n*.28)),I=Math.max(160,Math.round(n*.72)),L=Math.floor((I-10)/2);m&&(m.style.minHeight=m.style.maxHeight=p+"px"),e.querySelectorAll(".play-grid .play-card").forEach(re=>{re.style.minHeight=re.style.height=L+"px"});const B=Math.round(L*.55);e.querySelectorAll(".play-card .play-icon").forEach(re=>{re.style.height=B+"px"})}),document.getElementById("logout-btn").addEventListener("click",async()=>{await E.auth.signOut(),window.location.reload()}),Qi(t,s),Oo(t,s,i),Zi(t,s,i))}async function Zi(e,t,i){const s=document.getElementById("ongoing-match-banner");if(!s)return;const l=e.profile.id,{data:d}=await E.from("matches").select("id, home_id, away_id, status, mode").eq("status","active").or(`home_id.eq.${l},away_id.eq.${l}`).order("created_at",{ascending:!1});if(!(d!=null&&d.length)){s.innerHTML="";return}const a=d.map(g=>g.home_id===l?g.away_id:g.home_id).filter(Boolean);let c={};if(a.length){const{data:g}=await E.from("users").select("id, pseudo, club_name").in("id",a);(g||[]).forEach(o=>{c[o.id]=o.club_name||o.pseudo})}s.innerHTML=d.map(g=>{const o=g.home_id===l?g.away_id:g.home_id,r=c[o]||"Adversaire",m=g.mode==="mini_league";return`
    ...
    </div>`,document.body.appendChild(t),t.querySelector("#ab-cancel").addEventListener("click",()=>t.remove()),t.querySelector("#ab-ok").addEventListener("click",()=>{t.remove(),e()}),t.addEventListener("click",i=>{i.target===t&&t.remove()})}async function Oo(e,t,i){var c,g,o,r;const s=document.getElementById("match-invite-banner");if(!s)return;const{data:l}=await E.from("friend_match_invites").select("id, inviter_id, inviter:users!inviter_id(pseudo, club_name)").eq("invitee_id",e.user.id).eq("status","pending").order("created_at",{ascending:!1}).limit(1).maybeSingle();if(!l){s.innerHTML="";return}const d=((c=l.inviter)==null?void 0:c.club_name)||((g=l.inviter)==null?void 0:g.pseudo)||"?",a=l.inviter_id;s.innerHTML=`
    ...
    </div>`,(o=document.getElementById("match-inv-accept"))==null||o.addEventListener("click",()=>{s.innerHTML="",i("match",{matchMode:"friend",friendId:a,friendName:d})}),(r=document.getElementById("match-inv-decline"))==null||r.addEventListener("click",async()=>{await E.from("friend_match_invites").update({status:"declined"}).eq("id",l.id),s.innerHTML="",t("Invitation refusée","info")})}async function Qi(e,t){const i=document.getElementById("friend-requests-banner");if(!i)return;const{data:s,error:l}=await E.from("friendships").select("id, requester:users!requester_id(pseudo, club_name)").eq("addressee_id",e.user.id).eq("status","pending");if(l||!(s!=null&&s.length)){i.innerHTML="";return}const d=s.length,a=s.slice(0,2).map(g=>{var o;return((o=g.requester)==null?void 0:o.pseudo)||"?"}).join(", "),c=d>2?` +${d-2}`:"";i.innerHTML=`
  ...
  " data-card-id="${e.id}">
    ...
    <div style="background:#f2e8d2;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
      ...
      <div style="background:#f2e8d2;padding:3px 6px;display:flex;align-items:center;justify-content:space-between;min-height:26px;gap:4px">
        ...
        ${(k=i.clubs)!=null&&k.logo_url?`<img src="${i.clubs.logo_url}" style="width:22px;height:18px;object-fit:contain;flex-shrink:0">`:`<div style="background:#1a3a7a;color:#fff;border-radius:3px;padding:1px 4px;font-size:6px;font-weight:800;flex-shrink:0">${(((n=i.clubs)==null?void 0:n.encoded_name)||"").slice(0,6)}</div>`}
      </div>
    </div>
  </div>`}function Bi(e){const t=e.job||"ATT",i=zt(e,t),s=Ei[e.country_code]||e.country_code||"";return`
  <div style="
    width:140px;border-radius:12px;padding:6px;
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),o=(c||[]).filter(J=>J.card_type==="player"&&J.player),r=(c||[]).filter(J=>J.card_type==="game_changer"),m=(c||[]).filter(J=>J.card_type==="formation"),b=(c||[]).filter(J=>J.card_type==="stadium"),{data:u}=await E.from("gc_definitions").select("name,gc_type,color,effect,image_url"),k={};(u||[]).forEach(J=>{k[J.name]=J});const{data:n}=await E.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),p={};(n||[]).forEach(J=>{p[J.id]=J}),b.forEach(J=>{J.stadium_def&&(p[J.stadium_id]=J.stadium_def)});const I=Object.keys(hi),L=Object.keys(Ve),B={};o.forEach(J=>{const oe=J.player.id;B[oe]=(B[oe]||0)+1}),new Set(Object.keys(B).map(J=>String(J)));const H=new Set(m.map(J=>J.formation)),C=new Set(r.map(J=>J.gc_type));let D="player",j="Tous",W="",de=!1;function re(){return[...o].sort((J,oe)=>{const ge=Kt.indexOf(J.player.job),Y=Kt.indexOf(oe.player.job);return ge!==Y?ge-Y:(J.player.surname_encoded||"").localeCompare(oe.player.surname_encoded||"")})}function P(){return[...g||[]].sort((J,oe)=>{const ge=Kt.indexOf(J.job),Y=Kt.indexOf(oe.job);return ge!==Y?ge-Y:(J.surname_encoded||"").localeCompare(oe.surname_encoded||"")})}function X(){return re().filter(J=>{const oe=J.player,ge=j==="Tous"||oe.job===j,Y=!W||`${oe.firstname} ${oe.surname_encoded}`.toLowerCase().includes(W);return ge&&Y})}function U(){return P().filter(J=>{const oe=j==="Tous"||J.job===j,ge=!W||`${J.firstname} ${J.surname_encoded}`.toLowerCase().includes(W);return oe&&ge})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
  ...
  </div>`;function N(){const J=document.getElementById("col-filters");J&&(D==="player"?(J.innerHTML=`
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",oe=>{W=oe.target.value.toLowerCase(),ne()}),e.querySelectorAll(".filter-btn").forEach(oe=>{oe.addEventListener("click",()=>{j=oe.dataset.job,N(),ne()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{de=!de,N(),ne()})):(J.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{de=!de,N(),ne()})))}function ne(){const J=document.getElementById("col-grid");J&&(D==="player"?_e(J):D==="formation"?Ae(J):D==="stadium"?xe(J):Ie(J))}function pe(J,oe,ge,Y,Z){const K=document.getElementById("col-grid"),se=document.getElementById("col-big");if(!K||!se)return;var he=0;function Se(){const Me=window.innerWidth>=768,be=document.getElementById("col-big"),ze=document.getElementById("col-grid");Me&&be&&(be.style.minHeight="420px",be.style.flex="1 1 auto"),Me&&ze&&(ze.style.height=Math.round(310*.76+16)+"px",ze.style.flexShrink="0",ze.style.overflowX="auto",ze.style.overflowY="hidden",ze.style.alignItems="center",ze.style.padding="8px 16px"),se.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+oe(J[he])+"</div>";var M=se.querySelector("[data-card-id],[data-form-id],[data-gc-id]");M&&M.addEventListener("click",function(){Y(J[he])}),requestAnimationFrame(function(){var F=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!F||!se)){var w=se.clientHeight-8,T=se.clientWidth-24,f=F.offsetHeight,x=F.offsetWidth;if(f>0&&x>0&&w>40){var y=Me?2.2:1,h=Math.min(w/f,T/x,y);F.style.transform="scale("+h.toFixed(3)+")",F.style.transformOrigin="top center"}}}),K.innerHTML=J.map(function(F,w){var T=w===he,f="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+(T?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+w+'" style="'+f+'">'+ge(F,T)+"</div>"}).join(""),K.querySelectorAll(".col-mini-item").forEach(function(F){F.addEventListener("click",function(){he=Number(F.dataset.idx),Se(),F.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}Se()}function ye(J){var oe=window.innerWidth>=768?.76:.54,ge;if(!J||J._fake){var Y=J?J.player:null;if(!Y)return"";ge=Bi(Y)}else ge=di(J,"");return'<div style="display:inline-block;zoom:'+oe+';pointer-events:none;line-height:0">'+ge+"</div>"}function Le(J,oe,ge){oe=oe||100,ge=ge||140;var Y=Pt[J]||{},Z={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},K=Math.max(3,Math.round(oe*.06)),se=Object.entries(Y).map(function(Se){var Me=Se[0],be=Se[1],ze=Me.replace(/\d+$/,""),M=Z[ze]||"#888",F=Math.round(be.x*oe),w=Math.round(be.y*ge);return'<circle cx="'+F+'" cy="'+w+'" r="'+K+'" fill="'+M+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),he=Math.max(1,Math.round(oe/50));return'<svg viewBox="0 0 '+oe+" "+ge+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+oe+'" height="'+ge+'" fill="#1A6B3C"/><rect x="'+Math.round(oe*.2)+'" y="'+Math.round(ge*.02)+'" width="'+Math.round(oe*.6)+'" height="'+Math.round(ge*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+he+'"/><line x1="0" y1="'+Math.round(ge*.5)+'" x2="'+oe+'" y2="'+Math.round(ge*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+he+'"/><ellipse cx="'+Math.round(oe*.5)+'" cy="'+Math.round(ge*.5)+'" rx="'+Math.round(oe*.18)+'" ry="'+Math.round(ge*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+he+'"/><rect x="'+Math.round(oe*.2)+'" y="'+Math.round(ge*.82)+'" width="'+Math.round(oe*.6)+'" height="'+Math.round(ge*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+he+'"/>'+se+"</svg>"}function we(J,oe,ge){var Y=ge>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+ge+"</div>":"",Z=oe?'data-form-id="'+oe.id+'"':"",K=J.length>7?14:J.length>5?16:19,se=!!oe;return"<div "+Z+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(se?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(se?"":"filter:grayscale(1);opacity:0.5")+'">'+Y+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(se?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+K+"px;font-weight:900;color:"+(se?"#1A6B3C":"#aaa")+';line-height:1">'+J+'</div></div><div style="flex:1;overflow:hidden;background:'+(se?"#1A6B3C":"#ccc")+'">'+Le(J,140,220)+"</div></div>"}function ve(J,oe){var ge=window.innerWidth>=768?.76:.54,Y=140,Z=305,K=Math.round(Z*.22),se=Z-K,he=J.length>7?10:13,Se=Le(J,Y,se),Me=oe?"1.5px solid #2a7a40":"1px solid #ddd",be=oe?"":"filter:grayscale(1);opacity:0.45;",ze=oe?"#1A6B3C":"#bbb",M="#fff";return'<div style="display:inline-block;zoom:'+ge+';line-height:0;pointer-events:none"><div style="width:'+Y+"px;height:"+Z+"px;border-radius:6px;border:"+Me+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+be+'"><div style="height:'+K+"px;background:"+ze+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+he+"px;font-weight:900;color:"+M+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(Y-4)+'px">'+J+'</span></div><div style="height:'+se+'px;overflow:hidden;flex-shrink:0">'+Se+"</div></div></div>"}function _e(J){if(de){const oe=U();if(!oe.length){J.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const ge=oe.map(Y=>o.find(Z=>Z.player.id===Y.id)||{_fake:!0,player:Y,id:"fake-"+Y.id});pe(ge,Y=>Y._fake?Bi(Y.player):di(Y,""),Y=>Y._fake?ye({player:Y.player,id:"x",_fake:!0}):ye(Y),Y=>{Y._fake||Fi(Y,o,B,t)})}else{const oe=X();if(!oe.length){J.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const ge={};oe.forEach(Z=>{const K=Z.player.id;ge[K]||(ge[K]=[]),ge[K].push(Z)});const Y=Object.values(ge).map(Z=>Wo(Z));pe(Y,Z=>{const K=B[Z.player.id]||1,se=K>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${K}</div>`:"",Se=o.filter(Me=>Me.player.id===Z.player.id&&Me.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return di(Z,se+Se)},Z=>ye(Z),Z=>Fi(Z,o,B,t))}}function Ae(J){const oe=de?I:[...H];if(!oe.length){J.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const ge=oe.map(Y=>({formation:Y,card:m.find(Z=>Z.formation===Y)||null,owned:H.has(Y)}));pe(ge,({formation:Y,card:Z,owned:K})=>we(Y,K?Z:null,K?m.filter(se=>se.formation===Y).length:0),({formation:Y,owned:Z})=>ve(Y,Z),({card:Y,owned:Z})=>{Z&&Y&&Zo(Y,m,t,d)})}function Ie(J){const oe=Object.keys(k),ge=de?oe.length?oe:L:[...C];if(!ge.length){J.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const Y=ge.map(Z=>({type:Z,gc:Ve[Z]||{icon:"⚡",desc:""},def:k[Z]||null,owned:C.has(Z),card:r.find(K=>K.gc_type===Z)||null}));pe(Y,({type:Z,gc:K,def:se,owned:he,card:Se})=>{var y;const Me=he?r.filter(h=>h.gc_type===Z).length:0,be=Me>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${Me}</div>`:"",ze=(se==null?void 0:se.gc_type)==="ultra_game_changer",M={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},F={purple:"#b06ce0",light_blue:"#00d4ef"},w=M[se==null?void 0:se.color]||M.purple,T=F[se==null?void 0:se.color]||F.purple,f=(se==null?void 0:se.effect)||K.desc||"",x=se!=null&&se.image_url?`/icons/${se.image_url}`:((y=se==null?void 0:se.club)==null?void 0:y.logo_url)||(se!=null&&se.country_code?`https://flagcdn.com/w80/${se.country_code.toLowerCase()}.png`:null);return he&&Se?`<div data-gc-id="${Se.id}" data-gc-type="${Z}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${T};background:${w};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${T}66;cursor:pointer">
          ${be}
        ...
        </div>`},({type:Z,gc:K,def:se,owned:he})=>{const Se=window.innerWidth>=768?.76:.54,Me={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},be={purple:"#9b59b6",light_blue:"#00bcd4"},ze=Me[se==null?void 0:se.color]||Me.purple,M=be[se==null?void 0:se.color]||be.purple,F=se!=null&&se.image_url?`/icons/${se.image_url}`:null;return he?`<div style="display:inline-block;zoom:${Se};line-height:0;pointer-events:none"><div style="width:140px;height:310px;border-radius:8px;background:${ze};border:1px solid ${M};display:flex;flex-direction:column;overflow:hidden"><div style="height:56px;background:rgba(255,255,255,0.15);display:flex;align-items:center;justify-content:center"><span style="font-size:13px;font-weight:900;color:#fff;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:130px">${Z}</span></div><div style="flex:1;display:flex;align-items:center;justify-content:center">${F?`<img src="${F}" style="max-width:130px;max-height:190px;object-fit:contain">`:`<span style="font-size:48px">${K.icon}</span>`}</div><div style="height:54px;display:flex;align-items:center;justify-content:center;padding:0 4px"><span style="font-size:10px;color:rgba(255,255,255,0.7);text-align:center">${((se==null?void 0:se.effect)||K.desc||"").slice(0,30)}</span></div></div></div>`:`<div style="display:inline-block;zoom:${Se};line-height:0;pointer-events:none"><div style="width:140px;height:310px;border-radius:8px;background:#eee;border:1px solid #ddd;display:flex;flex-direction:column;align-items:center;justify-content:center;filter:grayscale(1);opacity:0.45"><span style="font-size:36px">${K.icon}</span><span style="font-size:12px;color:#aaa;margin-top:6px;text-align:center;padding:0 6px">${Z}</span></div></div>`},({type:Z,owned:K,def:se})=>{K&&Xo(Z,se,d)})}function xe(J){const oe="#E87722",ge="/";if(!b.length){J.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Stade.<br><small>Ouvre un booster Stade !</small></div>';return}const Y={};b.forEach(K=>{const se=K.stadium_id||"?";(Y[se]=Y[se]||[]).push(K)});const Z=Object.entries(Y).map(([K,se])=>({sid:K,def:p[K]||null,count:se.length,card:se[0]}));pe(Z,({def:K,count:se})=>{var M,F;const he=(K==null?void 0:K.name)||"?",Se=((M=K==null?void 0:K.club)==null?void 0:M.encoded_name)||(K==null?void 0:K.country_code)||"—",Me=K!=null&&K.image_url?`${ge}icons/${K.image_url}`:((F=K==null?void 0:K.club)==null?void 0:F.logo_url)||(K!=null&&K.country_code?`https://flagcdn.com/w80/${K.country_code.toLowerCase()}.png`:null),be=Me?`<img src="${Me}" style="width:90px;height:90px;object-fit:contain;border-radius:4px" onerror="this.style.display='none'">`:'<div style="font-size:56px">🏟️</div>',ze=se>1?`<div style="position:absolute;top:8px;right:8px;background:#333;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${se}</div>`:"";return`<div style="position:relative;width:140px;border-radius:12px;border:3px solid #c45a00;background:linear-gradient(160deg,${oe},#c45a00);display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${oe}66">
          ${ze}
        ...
        </div>`},({def:K,count:se})=>{var M,F;const he=window.innerWidth>=768?.76:.54,Se=(K==null?void 0:K.name)||"?",Me=((M=K==null?void 0:K.club)==null?void 0:M.encoded_name)||(K==null?void 0:K.country_code)||"—",be=K!=null&&K.image_url?`${ge}icons/${K.image_url}`:((F=K==null?void 0:K.club)==null?void 0:F.logo_url)||(K!=null&&K.country_code?`https://flagcdn.com/w80/${K.country_code.toLowerCase()}.png`:null),ze=be?`<img src="${be}" style="width:64px;height:64px;object-fit:contain;border-radius:4px" onerror="this.style.display='none'">`:'<span style="font-size:32px">🏟️</span>';return`<div style="display:inline-block;zoom:${he};line-height:0;pointer-events:none">
        ...
        </div>`},null)}e.querySelectorAll(".col-tab-btn").forEach(J=>{J.addEventListener("click",()=>{D=J.dataset.tab,j="Tous",W="",de=!1,e.querySelectorAll(".col-tab-btn").forEach(oe=>{const ge=oe.dataset.tab===D;oe.style.borderBottomColor=ge?"var(--green)":"transparent",oe.style.color=ge?"var(--green)":"var(--gray-600)"}),N(),ne()})}),N(),ne()}function Xo(e,t,i){const s=Ve[e]||{icon:"⚡",desc:"Effet spécial."},l=(t==null?void 0:t.gc_type)==="ultra_game_changer",d={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},a={purple:"#b06ce0",light_blue:"#00d4ef"},c=d[t==null?void 0:t.color]||d.purple,g=a[t==null?void 0:t.color]||a.purple,o=(t==null?void 0:t.name)||e,r=(t==null?void 0:t.effect)||s.desc,m=t!=null&&t.image_url?`/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Vt=1e3;function Zo(e,t,i,s){var k,n,p;const{state:l,toast:d,closeModal:a,navigate:c,refreshProfile:g}=i,o=e.formation,r={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function m(){const I=Pt[o]||{},L=hi[o]||[],B=290,H=360,C=20;function D(W){const de=I[W];return de?{x:de.x*B,y:de.y*H}:null}let j=`<svg width="${B}" height="${H}" viewBox="0 0 ${B} ${H}" xmlns="http://www.w3.org/2000/svg">`;for(const[W,de]of L){const re=D(W),P=D(de);!re||!P||(j+=`<line x1="${re.x}" y1="${re.y}" x2="${P.x}" y2="${P.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const W of Object.keys(I)){const de=D(W);if(!de)continue;const re=W.replace(/\d+/,""),P=r[re]||"#555";j+=`<circle cx="${de.x}" cy="${de.y}" r="${C}" fill="${P}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,j+=`<text x="${de.x}" y="${de.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${re}</text>`}return j+="</svg>",j}const b=t.filter(I=>I.formation===o);b.length;const u=!e.is_for_sale;s(`Formation ${o}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(k=document.getElementById("direct-sell-form-btn"))==null||k.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${o} pour ${Vt.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const I=b.find(B=>!B.is_for_sale)||b[0];if(!I){d("Aucune carte à vendre","error");return}await E.from("market_listings").delete().eq("card_id",I.id),await E.from("transfer_history").delete().eq("card_id",I.id);const{error:L}=await E.from("cards").delete().eq("id",I.id);if(L){d(L.message,"error");return}await E.from("users").update({credits:(l.profile.credits||0)+Vt}).eq("id",l.profile.id),await g(),d(`+${Vt.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),a(),c("collection")}),(n=document.getElementById("market-sell-form-btn"))==null||n.addEventListener("click",async()=>{const I=parseInt(document.getElementById("sell-price-form").value);if(!I||I<1){d("Prix invalide","error");return}await E.from("cards").update({is_for_sale:!0,sale_price:I}).eq("id",e.id),await E.from("market_listings").insert({seller_id:l.profile.id,card_id:e.id,price:I}),d("Carte mise en vente sur le marché !","success"),a(),c("collection")}),(p=document.getElementById("cancel-sell-form-btn"))==null||p.addEventListener("click",async()=>{await E.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await E.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),d("Annonce retirée","success"),a(),c("collection")})}async function Fi(e,t,i,s){var _e,Ae,Ie,xe,J,oe,ge;const{state:l,toast:d,openModal:a,closeModal:c,navigate:g,refreshProfile:o}=s,r=e.player,m=t.filter(Y=>Y.player.id===r.id),b=m.length,u=e.evolution_bonus||0,k=Math.max((Number(r.note_g)||0)+(r.job==="GK"||r.job2==="GK"?u:0),(Number(r.note_d)||0)+(r.job==="DEF"||r.job2==="DEF"?u:0),(Number(r.note_m)||0)+(r.job==="MIL"||r.job2==="MIL"?u:0),(Number(r.note_a)||0)+(r.job==="ATT"||r.job2==="ATT"?u:0)),n=r.rarity||"normal",{data:p}=await E.from("sell_price_configs").select("*").eq("rarity",n).lte("note_min",k).gte("note_max",k).order("note_min",{ascending:!1}).limit(1);((_e=p==null?void 0:p[0])==null?void 0:_e.price)??Yo[n];const I=r.rarity!=="legende",L=ao(r),B=((r.rarity==="pepite"||r.rarity==="papyte")&&e.current_note!=null?e.current_note:zt(r,r.job))+u,H=r.rarity==="pepite"||r.rarity==="papyte",C=r.job2?(H?_i(e,ro(r.job2)):zt(r,r.job2))+(zt(r,r.job2)>0?u:0):null,D=qt[r.job]||"#1A6B3C",j=r.job2?qt[r.job2]:null,W=no[r.rarity]||"#ccc",de=Ei[r.country_code]||r.country_code||"",re=e.evolution_bonus||0,X=B+re,U=C||0,N=U>0?U+re:0,ne=m.map(Y=>Y.id);let pe={};if(ne.length){const{data:Y}=await E.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",ne).order("transferred_at",{ascending:!0});(Y||[]).forEach(Z=>{pe[Z.card_id]||(pe[Z.card_id]=[]),pe[Z.card_id].push(Z)})}const ye=Y=>{const Z=Y.transferred_at?new Date(Y.transferred_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"2-digit",year:"numeric"}):"",K=Y.source==="booster"?"Booster":Y.price?Y.price.toLocaleString("fr"):"—";return`
    ...
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${m.map((Y,Z)=>{const K=pe[Y.id]||[],se=Y.is_for_sale,he=K.length?K[K.length-1]:null,Me=(r.rarity==="pepite"||r.rarity==="papyte")&&Y.current_note!=null?` (☆${Y.current_note})`:"";return`
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",r.note_g],["DEF",r.note_d],["MIL",r.note_m],["ATT",r.note_a]].map(([Y,Z])=>{const K=qt[Y],se=Y===r.job||Y===r.job2,he=(Number(Z)||0)+(se&&re>0?re:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:2px">
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(xe=document.getElementById("close-detail"))==null||xe.addEventListener("click",c);let we=new Set;const ve=()=>{const Y=we.size,Z=document.getElementById("sell-action-panel");if(!Z)return;Z.style.display=Y>0?"block":"none",document.getElementById("sell-selected-count").textContent=Y;const K=document.getElementById("evolve-btn");K&&(K.textContent=`⬆️ Évoluer ${Y>1?"(+"+Y+")":""}`)};document.querySelectorAll(".expl-check").forEach(Y=>{Y.addEventListener("change",()=>{const Z=Y.dataset.id;Y.checked?we.add(Z):we.delete(Z);const K=Y.closest(".exemplaire-row");K&&(K.style.borderColor=Y.checked?"#1A6B3C":"#eee"),ve()})}),document.querySelectorAll(".exemplaire-row").forEach(Y=>{Y.addEventListener("click",Z=>{if(Z.target.closest("button")||Z.target.tagName==="INPUT")return;const K=Y.querySelector(".expl-check");K&&!K.disabled&&(K.checked=!K.checked,K.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(Y=>{Y.addEventListener("click",Z=>{Z.stopPropagation();const K=document.querySelector(`.expl-hist[data-hist="${Y.dataset.idx}"]`);K&&(K.style.display=K.style.display==="none"?"flex":"none")})}),(J=document.getElementById("evolve-btn"))==null||J.addEventListener("click",async()=>{if(b<=1)return;const Y=[...we];if(!Y.length)return;if(we.has(e.id)){const be=document.createElement("div");be.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",be.innerHTML=`
    ...
    ${s?`<img src="${s}" style="position:absolute;top:${b}px;left:0;width:${t}px;height:${k}px;object-fit:cover;object-position:top center">`:""}
  ...
  </div>`,document.getElementById("new-deck-btn").addEventListener("click",async()=>{const a=prompt("Nom du deck :",`Deck ${((d==null?void 0:d.length)||0)+1}`);if(!a)return;const{data:c,error:g}=await E.from("decks").insert({owner_id:i.profile.id,name:a}).select().single();if(g){l(g.message,"error");return}l("Deck créé !","success"),Di(c.id,e,t)}),e.querySelectorAll("[data-open-deck]").forEach(a=>{a.addEventListener("click",()=>Di(a.dataset.openDeck,e,t))}),e.querySelectorAll("[data-rename]").forEach(a=>{a.addEventListener("click",async()=>{const c=prompt("Nouveau nom :",a.dataset.name);if(!c||c===a.dataset.name)return;const{error:g}=await E.from("decks").update({name:c}).eq("id",a.dataset.rename);if(g){l(g.message,"error");return}l("Deck renommé !","success"),ui(e,t)})}),e.querySelectorAll("[data-delete]").forEach(a=>{a.addEventListener("click",async()=>{if(!confirm(`Supprimer le deck "${a.dataset.name}" ? Cette action est irréversible.`))return;await E.from("deck_cards").delete().eq("deck_id",a.dataset.delete);const{error:c}=await E.from("decks").delete().eq("id",a.dataset.delete);if(c){l(c.message,"error");return}l("Deck supprimé.","success"),ui(e,t)})})}async function Di(e,t,i){const{state:s,toast:l}=i;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:d}=await E.from("decks").select("*").eq("id",e).single(),{data:a}=await E.from("cards").select(`id, card_type, formation, stadium_id, evolution_bonus,
      player:players(id, firstname, surname_encoded, country_code, club_id, job, job2,
        note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length,
        clubs(encoded_name, logo_url))`).eq("owner_id",s.profile.id),{data:c}=await E.from("cards").select(`id, card_type, stadium_id,
      stadium_def:stadium_definitions(id, name, club_id, country_code, image_url,
        club:clubs(encoded_name, logo_url))`).eq("owner_id",s.profile.id).eq("card_type","stadium"),g=(a||[]).filter(I=>I.card_type==="player"&&I.player),o=(a||[]).filter(I=>I.card_type==="formation"),r=(c||[]).filter(I=>I.card_type==="stadium"),m=[...new Set(r.map(I=>I.stadium_id).filter(Boolean))];let b={};if(r.forEach(I=>{I.stadium_def&&I.stadium_id&&(b[I.stadium_id]=I.stadium_def)}),m.length&&Object.keys(b).length<m.length){const{data:I}=await E.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",m);(I||[]).forEach(L=>{b[L.id]=L})}const u=o.map(I=>I.formation).filter(Boolean),{data:k}=await E.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",e);let n=d.formation||"4-4-2";u.length>0&&!u.includes(n)&&(n=u[0]);const p={deckId:e,name:d.name,formation:n,formationCardId:d.formation_card_id,stadiumCardId:d.stadium_card_id||null,slots:{},subs:[],playerCards:g,formationCards:o,stadiumCards:r,stadDefMap:b,availableFormations:u};(k||[]).forEach(I=>{I.is_starter?p.slots[I.position]=I.card_id:p.subs.includes(I.card_id)||p.subs.push(I.card_id)}),wt(t,p,i)}function wt(e,t,i){var o,r,m,b;const{navigate:s}=i;Xt[t.formation];const l=Gi(t.formation),d=l.filter(u=>t.slots[u]).length,a=t.availableFormations.length>0?t.availableFormations:Object.keys(Xt),c=(o=t.stadiumCards)==null?void 0:o.find(u=>u.id===t.stadiumCardId);c&&((r=t.stadDefMap)!=null&&r[c.stadium_id]);const g=t.subs.map(u=>t.playerCards.find(k=>k.id===u)).filter(Boolean);[...Object.values(t.slots),...t.subs],e.innerHTML=`
  <div style="height:100%;overflow-y:auto;padding-bottom:80px;background:var(--page-bg)">
    ...
    <div style="padding:10px 16px;background:#fff;border-bottom:1px solid var(--gray-200)">
      ...
      <select id="formation-select" style="width:100%;padding:7px;border-radius:6px;border:1.5px solid var(--gray-200)">
        ${a.map(u=>`<option value="${u}" ${u===t.formation?"selected":""}>${u}</option>`).join("")}
      </select>
    </div>
    ...
    <div style="padding:8px 16px;background:#fff;border-bottom:1px solid var(--gray-200);display:flex;align-items:center;gap:10px">
      ...
      ${t.stadiumCards.length>0?`
        <select id="stadium-select" style="padding:6px;border-radius:6px;border:1.5px solid #E87722;font-size:12px;max-width:180px;background:#fff">
          ...
          ${t.stadiumCards.map(u=>{var p;const k=t.stadDefMap[u.stadium_id],n=k?k.name+((p=k.club)!=null&&p.encoded_name?` (${k.club.encoded_name})`:k.country_code?` (${k.country_code})`:""):u.id.slice(0,8);return`<option value="${u.id}" ${t.stadiumCardId===u.id?"selected":""}>${n}</option>`}).join("")}
  ...
  </div>`,en(e,t,l,i),document.getElementById("builder-back").addEventListener("click",()=>s("decks")),document.getElementById("formation-select").addEventListener("change",u=>{t.formation=u.target.value;const k=Gi(t.formation),n={};k.forEach(p=>{t.slots[p]&&(n[p]=t.slots[p])}),t.slots=n,wt(e,t,i)}),(m=document.getElementById("stadium-select"))==null||m.addEventListener("change",u=>{t.stadiumCardId=u.target.value||null,wt(e,t,i)}),document.getElementById("save-deck").addEventListener("click",()=>nn(t,i)),e.querySelectorAll("[data-remove-sub]").forEach(u=>{u.addEventListener("click",()=>{t.subs=t.subs.filter(k=>k!==u.dataset.removeSub),wt(e,t,i)})}),(b=document.getElementById("add-sub-btn"))==null||b.addEventListener("click",()=>{on(t,e,i)})}function en(e,t,i,s){var B,H;const l=e.querySelector("#deck-field");if(!l)return;const d=(B=t.stadiumCards)==null?void 0:B.find(C=>C.id===t.stadiumCardId),a=d&&((H=t.stadDefMap)==null?void 0:H[d.stadium_id])||null,c=Pt[t.formation]||{},g=Yi(t.formation)||[],o={};for(const C of i){const D=t.slots[C],j=D?t.playerCards.find(W=>W.id===D):null;j!=null&&j.player?o[C]={...j.player,_evolution_bonus:j.evolution_bonus||0}:o[C]=null}const r=300,m=300,b=48,u=64,k=13,n=16,p=38;function I(C){const D=c[C];return D?{x:D.x*r,y:D.y*m}:null}let L="";for(const[C,D]of g){const j=I(C),W=I(D);if(!j||!W)continue;const de=o[C]?{...o[C],club_id:o[C].club_id,country_code:o[C].country_code,rarity:o[C].rarity}:null,re=o[D]?{...o[D],club_id:o[D].club_id,country_code:o[D].country_code,rarity:o[D].rarity}:null,P=at(de,re);P==="#ff3333"||P==="#cc2222"?L+=`<line x1="${j.x.toFixed(1)}" y1="${j.y.toFixed(1)}" x2="${W.x.toFixed(1)}" y2="${W.y.toFixed(1)}" stroke="${P}" stroke-width="2.5" stroke-linecap="round" opacity="0.4"/>`:(L+=`<line x1="${j.x.toFixed(1)}" y1="${j.y.toFixed(1)}" x2="${W.x.toFixed(1)}" y2="${W.y.toFixed(1)}" stroke="${P}" stroke-width="8" stroke-linecap="round" opacity="0.2"/>`,L+=`<line x1="${j.x.toFixed(1)}" y1="${j.y.toFixed(1)}" x2="${W.x.toFixed(1)}" y2="${W.y.toFixed(1)}" stroke="${P}" stroke-width="2.5" stroke-linecap="round" opacity="0.9"/>`)}for(const C of i){const D=I(C);if(!D)continue;const j=o[C],W=C.replace(/\d+/,""),de=vt[W]||"#555",re=(D.x-b/2).toFixed(1),P=(D.y-u/2).toFixed(1),X={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[j==null?void 0:j.rarity]||"#aaa";if(j){const U=ii(j),N=lo(j),ne=so(j.country_code),pe=j._evolution_bonus||0,ye=(Number(W==="GK"?j.note_g:W==="DEF"?j.note_d:W==="MIL"?j.note_m:j.note_a)||0)+(W===j.job||W===j.job2?pe:0),Le=a&&(a.club_id&&String(j.club_id)===String(a.club_id)||a.country_code&&j.country_code===a.country_code),we=Le?ye+10:ye,ve=u-k-n;L+=`<defs><clipPath id="dcp-${C}"><rect x="${re}" y="${(D.y-u/2+k).toFixed(1)}" width="${b}" height="${ve}"/></clipPath></defs>`,L+=`<rect x="${re}" y="${P}" width="${b}" height="${u}" rx="5" fill="${de}"/>`,U&&(L+=`<image href="${U}" x="${re}" y="${(D.y-u/2+k).toFixed(1)}" width="${b}" height="${ve}" clip-path="url(#dcp-${C})" preserveAspectRatio="xMidYMin slice"/>`),L+=`<rect x="${re}" y="${P}" width="${b}" height="${k}" fill="rgba(255,255,255,0.93)"/>`,L+=`<text x="${D.x.toFixed(1)}" y="${(D.y-u/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(j.surname_encoded||"").slice(0,9)}</text>`;const _e=(D.y+u/2-n).toFixed(1);L+=`<rect x="${re}" y="${_e}" width="${b}" height="${n}" fill="rgba(255,255,255,0.93)"/>`,ne&&(L+=`<image href="${ne}" x="${(D.x-21).toFixed(1)}" y="${(D.y+u/2-n+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`),L+=`<text x="${D.x.toFixed(1)}" y="${(D.y+u/2-n/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="${Le?"#E87722":"#111"}" font-family="Arial Black">${we}</text>`,N&&(L+=`<image href="${N}" x="${(D.x+b/2-14).toFixed(1)}" y="${(D.y+u/2-n+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`),L+=`<rect x="${re}" y="${P}" width="${b}" height="${u}" rx="5" fill="none" stroke="${X}" stroke-width="2"/>`}else L+=`<rect x="${re}" y="${P}" width="${b}" height="${u}" rx="5" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="5,3"/>`,L+=`<text x="${D.x.toFixed(1)}" y="${D.y.toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="22" fill="rgba(255,255,255,0.35)">+</text>`,L+=`<text x="${D.x.toFixed(1)}" y="${(D.y+16).toFixed(1)}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.3)">${W}</text>`;L+=`<rect x="${re}" y="${P}" width="${b}" height="${u}" rx="5" fill="rgba(0,0,0,0.01)" class="deck-slot-hit" data-pos="${C}" style="cursor:pointer"/>`}l.innerHTML=`<svg viewBox="${-p} ${-p} ${r+p*2} ${m+p*2}" width="100%" style="display:block;max-width:440px;margin:0 auto">${L}</svg>`,l.querySelectorAll(".deck-slot-hit").forEach(C=>{C.addEventListener("click",()=>tn(C.dataset.pos,t,e,s))})}function tn(e,t,i,s){var b,u,k;const{openModal:l,closeModal:d}=s,a=e.replace(/\d+/,""),c=t.slots[e],g=c?t.playerCards.find(n=>n.id===c):null;(b=g==null?void 0:g.player)==null||b.id;const o=new Set;Object.entries(t.slots).forEach(([n,p])=>{var L;if(n===e||!p)return;const I=t.playerCards.find(B=>B.id===p);(L=I==null?void 0:I.player)!=null&&L.id&&o.add(I.player.id)}),t.subs.forEach(n=>{var I;const p=t.playerCards.find(L=>L.id===n);(I=p==null?void 0:p.player)!=null&&I.id&&o.add(p.player.id)});const r=new Set,m=t.playerCards.filter(n=>{const p=n.player;return!(p.job===a||p.job2===a)||o.has(p.id)||r.has(p.id)?!1:(r.add(p.id),!0)});m.sort((n,p)=>{const I=n.evolution_bonus||0,L=p.evolution_bonus||0,B=(a==="GK"?n.player.note_g:a==="DEF"?n.player.note_d:a==="MIL"?n.player.note_m:n.player.note_a)+(a===n.player.job||a===n.player.job2?I:0);return(a==="GK"?p.player.note_g:a==="DEF"?p.player.note_d:a==="MIL"?p.player.note_m:p.player.note_a)+(a===p.player.job||a===p.player.job2?L:0)-B}),l(`Choisir ${a} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${m.length>0?m.map(n=>{var C,D;const p=n.player,I=n.evolution_bonus||0,L=(a==="GK"?p.note_g:a==="DEF"?p.note_d:a==="MIL"?p.note_m:p.note_a)+(a===p.job||a===p.job2?I:0),B=ii(p),H={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[p.rarity];return`<div class="player-option" data-card-id="${n.id}"
          ...
          <div style="flex:1;min-width:0">
            ...
            <div style="display:flex;align-items:center;gap:6px;margin-top:3px">
              ...
              ${(C=p.clubs)!=null&&C.logo_url?`<img src="${p.clubs.logo_url}" style="width:18px;height:18px;object-fit:contain">`:`<span style="font-size:10px;color:var(--gray-600)">${((D=p.clubs)==null?void 0:D.encoded_name)||"—"}</span>`}
    ...
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(u=document.getElementById("close-selector"))==null||u.addEventListener("click",d),(k=document.getElementById("remove-player"))==null||k.addEventListener("click",()=>{delete t.slots[e],d(),wt(i,t,s)}),document.querySelectorAll(".player-option").forEach(n=>{n.addEventListener("click",()=>{t.slots[e]=n.dataset.cardId,d(),wt(i,t,s)})})}function on(e,t,i){var g;const{openModal:s,closeModal:l}=i,d=new Set;Object.keys(e.slots).forEach(o=>{var b;const r=e.slots[o];if(!r)return;const m=e.playerCards.find(u=>u.id===r);(b=m==null?void 0:m.player)!=null&&b.id&&d.add(m.player.id)}),e.subs.forEach(o=>{var m;const r=e.playerCards.find(b=>b.id===o);(m=r==null?void 0:r.player)!=null&&m.id&&d.add(r.player.id)});const a=new Set,c=e.playerCards.filter(o=>{var r,m,b;return d.has((r=o.player)==null?void 0:r.id)||a.has((m=o.player)==null?void 0:m.id)?!1:(a.add((b=o.player)==null?void 0:b.id),!0)});s("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${c.length>0?c.map(o=>{var u;const r=o.player,m=ii(r),b=(r.job==="GK"?r.note_g:r.job==="DEF"?r.note_d:r.job==="MIL"?r.note_m:r.note_a)+(o.evolution_bonus||0);return`<div class="player-option" data-card-id="${o.id}"
          style="display:flex;align-items:center;gap:10px;padding:8px;border:1.5px solid var(--gray-200);border-radius:10px;cursor:pointer">
  ...
  </div>`}function uo(e){var l;const t={};(e.rates||[]).forEach(d=>{t[d.card_type]=(t[d.card_type]||0)+Number(d.percentage||0)});const i=((l=Object.entries(t).sort((d,a)=>a[1]-d[1])[0])==null?void 0:l[0])||"player",s=e.image_url||"booster-players.png";return{id:e.id,img:"/icons/"+s,name:e.name,sub:`${e.card_count} carte(s)`,cost:e.price_type==="credits"&&e.price_credits||0,costLabel:e.price_type==="credits"?`${(e.price_credits||0).toLocaleString("fr")} crédits`:e.price_type==="pub"?"1 pub":"Gratuit",cardCount:e.card_count||5,type:i,isPub:e.price_type==="pub",rates:e.rates||[],allow_duplicates:e.allow_duplicates!==!1,_raw:e}}async function pn(e,{state:t,navigate:i,toast:s}){var m,b;const l=((m=t.profile)==null?void 0:m.credits)||0;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⏳ Chargement...</div>';let d=[];try{d=(await co()).map(uo)}catch(u){console.warn("Erreur chargement boosters DB, fallback hardcodé",u)}d.length||(d=an.map(u=>({...u,rates:[],isPub:u.id==="players_pub"})));const a=await E.from("cards").select("player_id, card_type, formation, stadium_id, gc_type").eq("owner_id",t.profile.id).then(u=>u.data||[]),c=new Set(a.filter(u=>u.card_type==="stadium").map(u=>u.stadium_id)),g=new Set(a.filter(u=>u.card_type==="formation").map(u=>u.formation)),o=new Set(a.filter(u=>u.card_type==="game_changer").map(u=>u.gc_type)),r={};for(const u of d){if(u.allow_duplicates!==!1||!((b=u.rates)!=null&&b.length))continue;const k=[...new Set((u.rates||[]).map(p=>p.card_type))];let n=!1;for(const p of k)if(p==="stadium"){const{data:I}=await E.from("stadium_definitions").select("id");if((I||[]).some(L=>!c.has(L.id))){n=!0;break}}else if(p==="game_changer"){const{data:I}=await E.from("gc_definitions").select("name").eq("is_active",!0);if((I||[]).some(L=>!o.has(L.name))){n=!0;break}}else if(p==="formation"){const{FORMATION_LINKS:I}=await ti(async()=>{const{FORMATION_LINKS:L}=await import("./formation-links-Dnttc-Cp.js").then(B=>B.m);return{FORMATION_LINKS:L}},__vite__mapDeps([0,1]));if(Object.keys(I).some(L=>!g.has(L))){n=!0;break}}else{n=!0;break}n||(r[u.id]=!0)}e.innerHTML=`
  <div class="page">
    <div class="page-header">
      <h2>📦 Boosters</h2>
    ...
    <div class="page-body">
      <div class="booster-grid">
        ${d.map(u=>{const k=u.cost===0||l>=u.cost,n=r[u.id]===!0;return`<div class="booster-card ${!k||n?"disabled":""}" data-booster="${u.id}" style="position:relative">
            ...
            ${k?"":'<div style="font-size:10px;color:#c0392b;margin-top:4px">Crédits insuffisants</div>'}
  ...
  </div>`,e.querySelectorAll(".booster-card:not(.disabled)").forEach(u=>{u.addEventListener("click",async()=>{const k=d.find(n=>n.id===u.dataset.booster);if(k){u.style.opacity="0.5",u.style.pointerEvents="none";try{await un(k,{state:t,toast:s,navigate:i,container:e})}catch(n){s(n.message,"error"),u.style.opacity="",u.style.pointerEvents=""}}})}),e.querySelectorAll(".booster-info-btn").forEach(u=>{u.addEventListener("click",k=>{k.stopPropagation();const n=d.find(p=>p.id===u.dataset.boosterId);xn(n)})})}async function un(e,{state:t,toast:i,navigate:s,container:l}){var m,b;if(e.cost>0&&t.profile.credits<e.cost){i("Crédits insuffisants","error");return}e.isPub&&await bn();const{data:d}=await E.from("cards").select("card_type, player_id, formation").eq("owner_id",t.profile.id),a=new Set((d||[]).filter(u=>u.card_type==="player").map(u=>u.player_id)),c=new Set((d||[]).filter(u=>u.card_type==="formation").map(u=>u.formation));let g=[],o=null;try{if((m=e.rates)!=null&&m.length)g=await gi(t.profile,e);else{const u=e.type||"player";u==="player"?g=await fo(t.profile,e.cardCount,e.cost):u==="game_changer"?g=await go(t.profile,e.cardCount,e.cost):u==="formation"?g=await mo(t.profile,e.cost):g=await gi(t.profile,e)}}catch(u){o=u.message||String(u),console.error("[Booster] Erreur:",u)}if(!(g!=null&&g.length)){const u=document.createElement("div");u.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:3000;gap:16px;color:#fff;padding:24px;text-align:center",u.innerHTML=`
      <div style="font-size:48px">😕</div>
      <div style="font-size:20px;font-weight:900">Aucune carte obtenue</div>
      <div style="font-size:12px;color:rgba(255,255,255,0.6);max-width:320px;word-break:break-all;background:rgba(255,255,255,0.05);padding:10px;border-radius:8px;margin-top:6px">
        ${o||"Vérifie la console (F12) pour plus de détails"}
      </div>
      <button style="margin-top:10px;padding:12px 28px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer" id="anim-close-err">Fermer</button>`,document.body.appendChild(u),(b=u.querySelector("#anim-close-err"))==null||b.addEventListener("click",()=>u.remove());return}g.forEach(u=>{u.card_type==="player"&&u.player?u.isDuplicate=a.has(u.player.id):u.card_type==="formation"&&(u.isDuplicate=c.has(u.formation))});const{data:r}=await E.from("users").select("*").eq("id",t.profile.id).single();r&&(t.profile=r),xo(g,e,s)}function fn(){const e=Math.random()*100;return e<.5?"legende":e<2?"special":e<10?"normal_high":"normal_low"}function mt(e){return Math.max(Number(e.note_g)||0,Number(e.note_d)||0,Number(e.note_m)||0,Number(e.note_a)||0)}function gn(e,t){let i;switch(t){case"legende":i=e.filter(s=>s.rarity==="legende"),i.length||(i=e.filter(s=>s.rarity==="pepite"||s.rarity==="papyte")),i.length||(i=e.filter(s=>mt(s)>=6));break;case"special":i=e.filter(s=>s.rarity==="pepite"||s.rarity==="papyte"),i.length||(i=e.filter(s=>mt(s)>=6));break;case"normal_high":i=e.filter(s=>s.rarity==="normal"&&mt(s)>=6),i.length||(i=e.filter(s=>mt(s)>=6));break;default:i=e.filter(s=>s.rarity==="normal"&&mt(s)>=1&&mt(s)<=5),i.length||(i=e.filter(s=>s.rarity==="normal"));break}return i.length||(i=e),i[Math.floor(Math.random()*i.length)]}async function gi(e,t){if(t.cost>0){const{error:b}=await E.from("users").update({credits:e.credits-t.cost}).eq("id",e.id);if(b)throw b}const i=t.allow_duplicates!==!1;let s=[];const{data:l,error:d}=await E.from("cards").select("player_id, card_type, formation, stadium_id, gc_type").eq("owner_id",e.id);if(d){const{data:b}=await E.from("cards").select("player_id, card_type, formation, gc_type").eq("owner_id",e.id);s=b||[]}else s=l||[];const a=new Set(s.filter(b=>b.card_type==="player").map(b=>b.player_id)),c=new Set(s.filter(b=>b.card_type==="formation").map(b=>b.formation)),g=new Set(s.filter(b=>b.card_type==="game_changer").map(b=>b.gc_type)),o=new Set(s.filter(b=>b.card_type==="stadium").map(b=>b.stadium_id).filter(Boolean)),r=new Set,m=[];for(let b=0;b<(t.cardCount||5);b++){const u=rn(t.rates);if(u){if(u.card_type==="player"){const k=j=>({légende:"legende",pépite:"pepite",pépites:"pepite"})[j]||j,n=u.rarity?k(u.rarity):null;let p=E.from("players").select("id,job,firstname,surname_encoded,country_code,club_id,rarity,note_g,note_d,note_m,note_a,skin,hair,hair_length,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);n&&(p=p.eq("rarity",n));const{data:I}=await p;let L=I||[];if((u.note_min||u.note_max)&&(L=L.filter(j=>{const W=Math.max(Number(j.note_g)||0,Number(j.note_d)||0,Number(j.note_m)||0,Number(j.note_a)||0);return(!u.note_min||W>=u.note_min)&&(!u.note_max||W<=u.note_max)})),L.length||(u.note_min||u.note_max?(L=I||[],console.warn("[Booster] Aucun joueur avec note",u.note_min,"-",u.note_max,"— fallback rareté uniquement")):L=I||[]),!L.length)continue;let B=L.filter(j=>!r.has(j.id));if(i)B.length||(B=L);else if(B=B.filter(j=>!a.has(j.id)),!B.length)continue;const H=B[Math.floor(Math.random()*B.length)];r.add(H.id);const C=a.has(H.id),{data:D}=await E.from("cards").insert({owner_id:e.id,player_id:H.id,card_type:"player"}).select().single();D&&(m.push({...D,player:H,isDuplicate:C}),E.rpc("record_transfer",{p_card_id:D.id,p_player_id:H.id,p_club_name:e.club_name||e.pseudo,p_manager_name:e.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{}))}else if(u.card_type==="game_changer"){const{data:k}=await E.from("gc_definitions").select("id,name").eq("is_active",!0).eq("gc_type","game_changer"),n=k!=null&&k.length?k:[{name:"Ressusciter"},{name:"Double attaque"},{name:"Bouclier"},{name:"Vol de note"},{name:"Gel"}],p=i?n:n.filter(H=>!g.has(H.name));if(!i&&!p.length)continue;const L=p[Math.floor(Math.random()*p.length)].name,{data:B}=await E.from("cards").insert({owner_id:e.id,card_type:"game_changer",gc_type:L}).select().single();B&&m.push(B)}else if(u.card_type==="formation"){const k=po(),n=i?k:k.filter(B=>!c.has(B));if(!i&&!n.length)continue;const p=n[Math.floor(Math.random()*n.length)],I=c.has(p),{data:L}=await E.from("cards").insert({owner_id:e.id,card_type:"formation",formation:p}).select();L!=null&&L[0]&&m.push({...L[0],isDuplicate:I})}else if(u.card_type==="stadium"){const{data:k,error:n}=await E.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)");if(n){console.error("[Booster] stadium_definitions:",n.message);continue}if(!(k!=null&&k.length)){console.warn("[Booster] Aucun stade en DB");continue}const p=i?k:k.filter(H=>!o.has(H.id));if(!i&&!p.length)continue;const I=p[Math.floor(Math.random()*p.length)],{data:L,error:B}=await E.from("cards").insert({owner_id:e.id,card_type:"stadium",stadium_id:I.id}).select("id,card_type,stadium_id").single();if(B){console.error("[Booster] insert stadium card:",B.message);continue}L&&m.push({...L,rarity:"normal",_stadiumDef:I})}}}return m}async function fo(e,t,i){if(i>0){const{error:o}=await E.from("users").update({credits:e.credits-i}).eq("id",e.id);if(o)throw o}const{data:s}=await E.from("players").select("id,job,firstname,surname_encoded,country_code,club_id,rarity,note_g,note_d,note_m,note_a,note_min,note_max,skin,hair,hair_length,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);if(!(s!=null&&s.length))throw new Error("Pas de joueurs en BDD — ajoutes-en via le panel admin !");const l=s.filter(o=>o.job==="GK"),d=s.filter(o=>o.job!=="GK"),a=!e.first_booster_opened&&l.length>0,c=[];for(let o=0;o<t;o++){const r=o===0&&a?l:o===0?d:s,m=fn(),b=gn(r,m);b&&c.push(b)}a&&await E.from("users").update({first_booster_opened:!0}).eq("id",e.id);const{data:g}=await E.from("cards").insert(c.map(o=>({owner_id:e.id,player_id:o.id,card_type:"player"}))).select();return(g||[]).forEach((o,r)=>{E.rpc("record_transfer",{p_card_id:o.id,p_player_id:c[r].id,p_club_name:e.club_name||e.pseudo,p_manager_name:e.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{})}),c.map((o,r)=>({...g[r],player:o}))}async function go(e,t,i){const{error:s}=await E.from("users").update({credits:e.credits-i}).eq("id",e.id);if(s)throw s;const{data:l}=await E.from("gc_definitions").select("id,name,gc_type,color,effect,image_url").eq("is_active",!0),d=l!=null&&l.length?l:Object.keys(fi).map(r=>({name:r,gc_type:"game_changer"})),a=Array.from({length:t},()=>{const r=d[Math.floor(Math.random()*d.length)];return{owner_id:e.id,card_type:"game_changer",gc_type:r.name,gc_definition_id:r.id||null}}),{data:c,error:g}=await E.from("cards").insert(a).select();return g&&console.error("[Booster GC] Erreur insert:",g.message,g),(c||[]).map(r=>{const m=l==null?void 0:l.find(b=>b.name===r.gc_type||b.id===r.gc_definition_id);return{...r,_gcDef:m||null}})}async function mo(e,t){const{error:i}=await E.from("users").update({credits:e.credits-t}).eq("id",e.id);if(i)throw i;const{data:s}=await E.from("cards").select("formation").eq("owner_id",e.id).eq("card_type","formation"),l=new Set((s||[]).map(r=>r.formation)),d=po(),a=d[Math.floor(Math.random()*d.length)],c=l.has(a),{data:g,error:o}=await E.from("cards").insert({owner_id:e.id,card_type:"formation",formation:a}).select();return o&&console.error("[Booster Formation] Erreur insert:",o.message,o),(g||[]).map(r=>({...r,isDuplicate:c}))}function xo(e,t,i,s=null){var X,U;if(!e||e.length===0){const N=document.createElement("div");N.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:3000;gap:16px;color:#fff;padding:24px;text-align:center",N.innerHTML=`
      <div style="font-size:48px">😕</div>
      <div style="font-size:20px;font-weight:900">Aucune carte obtenue</div>
  ...
  `,document.body.appendChild(l);let d=!1;const a=document.getElementById("pack-cut-zone"),c=document.getElementById("pack-blade");let g=!1;const o=N=>N.touches&&N.touches[0]?N.touches[0].clientX:N.clientX;function r(N){d||(g=!0,c.style.opacity="1",m(N))}function m(N){if(!g||d)return;const ne=a.getBoundingClientRect(),pe=o(N)-ne.left,ye=Math.max(0,Math.min(1,pe/ne.width));c.style.width=ye*ne.width+"px",ye>=.82&&u()}function b(){d||(g=!1,c.style.transition="width .2s ease, opacity .2s ease",c.style.width="0",c.style.opacity="0",setTimeout(()=>{d||(c.style.transition="")},220))}function u(){var ne;if(d)return;d=!0,g=!1,c.style.width="100%",c.style.opacity="1",(ne=document.getElementById("cut-flash"))==null||ne.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const N=document.getElementById("cut-hint");N&&(N.style.opacity="0"),a.classList.add("pack-cut"),setTimeout(()=>{c.style.opacity="0",document.getElementById("pack-phase").style.display="none",I(0)},620)}a.addEventListener("pointerdown",r),window.addEventListener("pointermove",m),window.addEventListener("pointerup",b),a.addEventListener("touchstart",r,{passive:!0}),window.addEventListener("touchmove",m,{passive:!0}),window.addEventListener("touchend",b);let k=0,n=!1;const p=new Set;function I(N){k=N,document.getElementById("reveal-phase").style.display="flex",L(),B(N,0),W()}function L(){const N=document.getElementById("card-dots");N&&(N.innerHTML=e.map((ne,pe)=>`<div class="card-dot" data-i="${pe}" style="width:8px;height:8px;border-radius:50%;background:${pe===k?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),N.querySelectorAll(".card-dot").forEach(ne=>ne.addEventListener("click",()=>C(parseInt(ne.dataset.i)))))}function B(N,ne){var xe;const pe=e[N],ye=document.getElementById("card-counter"),Le=document.getElementById("card-track");ye&&(ye.textContent=`Carte ${N+1} / ${e.length}`);const we=document.getElementById("reveal-btns");we&&(we.style.display=N===e.length-1?"flex":"none");const ve=pe.card_type==="player"&&((xe=pe.player)==null?void 0:xe.rarity)==="legende",_e=!p.has(N);p.add(N);let Ae=0;if(pe.card_type==="player"&&pe.player){const J=pe.player,oe=J.job||"ATT";Ae=Number(oe==="GK"?J.note_g:oe==="DEF"?J.note_d:oe==="MIL"?J.note_m:J.note_a)||0}const Ie=J=>{Le.innerHTML=`
        ...
        </div>`;const oe=document.getElementById("current-card-wrap");ne!==0?(oe.style.transition="none",oe.style.transform=`translateX(${ne>0?100:-100}%)`,requestAnimationFrame(()=>{oe.style.transition="transform .28s cubic-bezier(.25,1,.5,1)",oe.style.transform="translateX(0)"})):oe.animate([{opacity:0,transform:"scale(.7)"},{opacity:1,transform:"scale(1)"}],{duration:300,easing:"cubic-bezier(.34,1.56,.64,1)"}),J||ve?re():P(),L()};_e&&Ae>6&&pe.card_type==="player"&&pe.player?H(pe,()=>Ie(!0)):Ie(!1)}function H(N,ne){var Ae;n=!0;const pe=N.player,ye=`https://flagsapi.com/${pe.country_code}/flat/64.png`,Le=(Ae=pe.clubs)==null?void 0:Ae.logo_url,we=document.getElementById("walkout-overlay"),ve=document.getElementById("walkout-stage");if(!we||!ve){n=!1,ne();return}we.style.display="flex";const _e=()=>{const Ie=ve.firstElementChild;Ie&&(Ie.classList.remove("wo-in"),Ie.classList.add("wo-out"))};ve.innerHTML=`<img class="wo-in" src="${ye}" style="height:130px;border-radius:10px;box-shadow:0 10px 36px rgba(0,0,0,.6)" onerror="this.style.display='none'">`,navigator.vibrate&&navigator.vibrate(30),setTimeout(_e,2e3),setTimeout(()=>{var Ie;ve.innerHTML=Le?`<img class="wo-in" src="${Le}" style="max-height:160px;max-width:210px;object-fit:contain">`:`<div class="wo-in" style="font-size:34px;font-weight:900;color:#fff;text-align:center">${((Ie=pe.clubs)==null?void 0:Ie.encoded_name)||"CLUB"}</div>`,navigator.vibrate&&navigator.vibrate(30)},2450),setTimeout(_e,4450),setTimeout(()=>{we.style.display="none",ve.innerHTML="",n=!1,navigator.vibrate&&navigator.vibrate([40,30,80]),ne()},4900)}function C(N){if(n||N<0||N>=e.length||N===k)return;const ne=N>k?1:-1;k=N,B(N,ne)}function D(){C(k+1)}function j(){C(k-1)}function W(){const N=document.getElementById("card-viewport");if(!N||N._swipeBound)return;N._swipeBound=!0;let ne=0,pe=0,ye=0,Le=!1;const we=xe=>xe.touches?xe.touches[0].clientX:xe.clientX,ve=xe=>xe.touches?xe.touches[0].clientY:xe.clientY,_e=xe=>{Le=!0,ne=we(xe),pe=ve(xe),ye=0},Ae=xe=>{if(!Le)return;ye=we(xe)-ne;const J=ve(xe)-pe;if(Math.abs(ye)<Math.abs(J))return;const oe=document.getElementById("current-card-wrap");oe&&(oe.style.transition="none",oe.style.transform=`translateX(${ye*.6}px) rotate(${ye*.02}deg)`)},Ie=()=>{if(!Le)return;Le=!1;const xe=document.getElementById("current-card-wrap"),J=55;ye<=-J&&k<e.length-1?D():ye>=J&&k>0?j():xe&&(xe.style.transition="transform .2s ease",xe.style.transform="translateX(0)")};N.addEventListener("pointerdown",_e),N.addEventListener("pointermove",Ae),N.addEventListener("pointerup",Ie),N.addEventListener("pointercancel",Ie),N.addEventListener("touchstart",_e,{passive:!0}),N.addEventListener("touchmove",Ae,{passive:!0}),N.addEventListener("touchend",Ie),N.addEventListener("click",xe=>{if(Math.abs(ye)>8)return;const J=N.getBoundingClientRect();xe.clientX-J.left>J.width/2?D():j()})}let de=null;function re(){const N=document.getElementById("fireworks-canvas");if(!N)return;N.width=window.innerWidth,N.height=window.innerHeight;const ne=N.getContext("2d"),pe=[];function ye(){const we=Math.random()*N.width,ve=Math.random()*N.height*.6,_e=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],Ae=_e[Math.floor(Math.random()*_e.length)];for(let Ie=0;Ie<60;Ie++){const xe=Math.PI*2/60*Ie,J=2+Math.random()*5;pe.push({x:we,y:ve,vx:Math.cos(xe)*J,vy:Math.sin(xe)*J,alpha:1,color:Ae,size:2+Math.random()*3})}}ye(),de=setInterval(ye,600);function Le(){if(document.getElementById("fireworks-canvas")){ne.clearRect(0,0,N.width,N.height);for(let we=pe.length-1;we>=0;we--){const ve=pe[we];if(ve.x+=ve.vx,ve.y+=ve.vy+.08,ve.vy*=.98,ve.alpha-=.018,ve.alpha<=0){pe.splice(we,1);continue}ne.globalAlpha=ve.alpha,ne.fillStyle=ve.color,ne.beginPath(),ne.arc(ve.x,ve.y,ve.size,0,Math.PI*2),ne.fill()}ne.globalAlpha=1,(de!==null||pe.length>0)&&requestAnimationFrame(Le)}}Le()}function P(){de!==null&&(clearInterval(de),de=null);const N=document.getElementById("fireworks-canvas");N&&N.getContext("2d").clearRect(0,0,N.width,N.height)}if(s){const N=document.getElementById("reveal-btns");N&&(N.innerHTML='<button class="btn btn-primary" id="reveal-next" style="flex:1">Continuer →</button>'),(U=document.getElementById("reveal-next"))==null||U.addEventListener("click",()=>{P(),l.remove(),s()})}else document.getElementById("reveal-collection").addEventListener("click",()=>{P(),l.remove(),i("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{P(),l.remove(),i("boosters")})}function mn(e){var t,i,s,l;if(e.card_type==="player"&&e.player)return cn(e);if(e.card_type==="game_changer"){const d=e._gcDef,a=(d==null?void 0:d.gc_type)==="ultra_game_changer",c={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},g={purple:"#b06ce0",light_blue:"#00d4ef"},o=c[d==null?void 0:d.color]||c.purple,r=g[d==null?void 0:d.color]||g.purple,m=(d==null?void 0:d.name)||e.gc_type||"Game Changer",b=(d==null?void 0:d.effect)||((t=fi[e.gc_type])==null?void 0:t.desc)||"",u=d!=null&&d.image_url?`/icons/${d.image_url}`:null,k=((i=fi[e.gc_type])==null?void 0:i.icon)||"⚡";return`<div style="width:170px;height:240px;background:${o};border-radius:14px;border:3px solid ${r};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${r}88;flex-shrink:0">
      ...
      <div style="flex:1;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06);margin:0">
        ${u?`<img src="${u}" style="max-width:120px;max-height:120px;width:auto;height:auto;object-fit:contain;border-radius:6px">`:`<span style="font-size:56px">${k}</span>`}
      </div>
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}function bn(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText=`
    ...
    `,document.body.appendChild(t);let i=5;const s=setInterval(()=>{i--;const l=document.getElementById("mw-ad-cd");l&&(l.textContent=i),i<=0&&(clearInterval(s),t.remove(),e(!0))},1e3)})}async function hn(e,{state:t,navigate:i,toast:s,refreshProfile:l}){var b,u;const{data:d}=await E.from("users").select("*").eq("id",t.user.id).single();d&&(t.profile=d);let a=Array.isArray((b=t.profile)==null?void 0:b.pending_boosters)?[...t.profile.pending_boosters]:[];if(!a.length){await E.from("users").update({onboarding_done:!0}).eq("id",t.user.id),i("home");return}let c=null;try{const n=(await co()).find(p=>(p.name||"").toLowerCase().includes("new player"));n&&(c=uo(n))}catch(k){console.warn('[Onboarding] Config "Booster (new player)" introuvable, fallback taux par défaut',k)}const g=a.length;let o=0;e.innerHTML=`
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
  ...
  </div>`;const r=async()=>{await E.from("users").update({pending_boosters:a}).eq("id",t.user.id)};async function m(){var L;if(o>=g||!a.length){await E.from("users").update({pending_boosters:[],onboarding_done:!0}).eq("id",t.user.id),l&&await l(),s("Tous tes boosters sont ouverts ! Bon jeu 🎮","success",4e3),i("home");return}const k=a[0],{data:n}=await E.from("users").select("*").eq("id",t.user.id).single();n&&(t.profile=n);let p=[];try{if(k.type==="formation")p=await mo(t.profile,0);else if(k.type==="game_changer")p=await go(t.profile,k.count||3,0);else if(c&&((L=c.rates)!=null&&L.length)){const B={...c,cost:0,cardCount:k.count||c.cardCount||5};p=await gi(t.profile,B),k.guaranteeGK&&!t.profile.first_booster_opened&&(p.some(C=>C.player&&C.player.job==="GK")||await vn(t.profile,p),await E.from("users").update({first_booster_opened:!0}).eq("id",t.profile.id))}else p=await fo(t.profile,k.count||5,0)}catch(B){s(B.message||"Erreur ouverture booster","error");return}a.shift(),o++,await r();const I=k.type==="formation"?{name:"Booster Formation",img:"/icons/booster-formation.png"}:k.type==="game_changer"?{name:"Booster Game Changer",img:"/icons/booster-gamechanger.png"}:{name:`Booster Joueurs (${o}/${g})`,img:(c==null?void 0:c.img)||"/icons/booster-players.png"};xo(p,I,i,()=>{m()})}(u=document.getElementById("onboard-start"))==null||u.addEventListener("click",()=>m())}async function vn(e,t){try{const{data:i}=await E.from("players").select("id,job,firstname,surname_encoded,country_code,club_id,rarity,note_g,note_d,note_m,note_a,skin,hair,hair_length,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0).eq("job","GK");if(!(i!=null&&i.length))return;const s=i[Math.floor(Math.random()*i.length)],l=t.findIndex(a=>a.player);if(l===-1)return;const d=t[l];await E.from("cards").update({player_id:s.id}).eq("id",d.id),t[l]={...d,player_id:s.id,player:s}}catch(i){console.warn("[Onboarding] ensureGK échec",i)}}const It={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},Mt={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function _t(e,t,i,s,l){var d;e.innerHTML=`<div class="match-screen" style="display:flex;align-items:center;justify-content:center;min-height:100vh">
    <div style="text-align:center;padding:40px;color:#fff">
      <div style="font-size:48px;margin-bottom:16px">${t}</div>
      <p style="margin-bottom:16px">${i}</p>
      <button class="btn btn-primary" id="msg-btn">${s}</button>
    </div>
  </div>`,(d=document.getElementById("msg-btn"))==null||d.addEventListener("click",l)}function Pe(e){const t=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function mi(e,t){var d,a;const i=e.player,s=e.evolution_bonus||0,l=i.job2&&Number(i[`note_${i.job2.toLowerCase()}`])||0;return{cardId:e.id,position:t||null,id:i.id,firstname:i.firstname,name:i.surname_encoded,country_code:i.country_code,club_id:i.club_id,job:i.job,job2:i.job2,note_g:(Number(i.note_g)||0)+(i.job==="GK"?s:0)+(i.job2==="GK"&&l>0?s:0),note_d:(Number(i.note_d)||0)+(i.job==="DEF"?s:0)+(i.job2==="DEF"&&l>0?s:0),note_m:(Number(i.note_m)||0)+(i.job==="MIL"?s:0)+(i.job2==="MIL"&&l>0?s:0),note_a:(Number(i.note_a)||0)+(i.job==="ATT"?s:0)+(i.job2==="ATT"&&l>0?s:0),evolution_bonus:s,rarity:i.rarity,skin:i.skin,hair:i.hair,hair_length:i.hair_length,clubName:((d=i.clubs)==null?void 0:d.encoded_name)||null,clubLogo:((a=i.clubs)==null?void 0:a.logo_url)||null,boost:0,used:!1,_line:null,_col:null}}function Ht(e,t){if(!t||!e)return e;const{club_id:i,country_code:s}=t;return Object.values(e).forEach(l=>{Array.isArray(l)&&l.forEach(d=>{const a=i&&String(d.club_id)===String(i),c=s&&String(d.country_code)===String(s);(a||c)&&(d.stadiumBonus=!0)})}),e}function oi(e,t){if(!t||!(e!=null&&e.length))return e;const{club_id:i,country_code:s}=t;return e.forEach(l=>{if(!l)return;const d=i&&String(l.club_id)===String(i),a=s&&String(l.country_code)===String(s);(d||a)&&(l.stadiumBonus=!0)}),e}function lt(e){return e===1?[1]:e===2?[0,2]:e===3?[0,1,2]:e===4?[0,1,1,2]:e===5?[0,1,1,1,2]:[1]}function ni(){const e=Math.random()*100;return e<10?3:e<30?2:1}function yt(e,t){const i=It[t]||It["4-4-2"],s={GK:[],DEF:[],MIL:[],ATT:[]};if(e.length&&e.every(a=>a.position)){for(const a of["GK","DEF","MIL","ATT"]){const c=e.filter(o=>o.position&&o.position.replace(/\d+$/,"")===a).sort((o,r)=>parseInt(o.position.replace(/\D+/g,""),10)-parseInt(r.position.replace(/\D+/g,""),10)).map(o=>({...o,_line:a})),g=lt(c.length);c.forEach((o,r)=>{o._col=g[r]}),s[a]=c}return s}const d=[...e];for(const a of["GK","DEF","MIL","ATT"]){const c=[];for(let o=0;o<i[a];o++){let r=d.findIndex(m=>m.job===a);if(r===-1&&(r=d.findIndex(m=>m.job2===a)),r===-1&&(r=0),d[r]){const m={...d[r],_line:a};c.push(m),d.splice(r,1)}}const g=lt(c.length);c.forEach((o,r)=>{o._col=g[r]}),s[a]=c}return s}function et(e){document.querySelectorAll(".top-nav, .bottom-nav").forEach(t=>{t.style.setProperty("display","none","important"),t.dataset.matchHidden="1"}),e&&e.style.setProperty("padding-bottom","0","important")}function We(e){document.querySelectorAll(".top-nav, .bottom-nav").forEach(t=>{t.style.removeProperty("display"),delete t.dataset.matchHidden}),e&&e.style.removeProperty("padding-bottom")}function ri(e,t,i){const l=new Set,d=t.filter(r=>{const m=r.gc_type||r.id;return l.has(m)?!1:(l.add(m),!0)});let a=[];function c(r,m){const b=r._gcDef,u={purple:"linear-gradient(135deg,#3d0a7a,#7a28b8)",light_blue:"linear-gradient(135deg,#006080,#00bcd4)"},k={purple:"#9b59b6",light_blue:"#00bcd4"},n=u[b==null?void 0:b.color]||u.purple,p=k[b==null?void 0:b.color]||k.purple;return`<div class="gc-select-card" data-id="${r.id}"
    ...
    </div>`}const g=r=>{e.style.overflow="",e.style.height="",e.style.display="",e.style.flexDirection="",i(r)};function o(){var m,b,u;e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column";const r=a.length>0;e.innerHTML=`
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      <div style="flex:1;overflow-y:auto;display:flex;flex-wrap:wrap;gap:10px;justify-content:center;align-content:flex-start;padding:8px 16px 16px">
        ${d.map(k=>{const n=a.find(p=>p.gc_type===k.gc_type);return c(k,!!n)}).join("")}
      </div>
    ...
    </div>`,e.querySelectorAll(".gc-select-card").forEach(k=>{k.addEventListener("click",()=>{const n=k.dataset.id,p=d.find(L=>L.id===n);if(!p)return;const I=a.findIndex(L=>L.gc_type===p.gc_type);I>-1?a.splice(I,1):a.length<3&&a.push(p),o()})}),(m=e.querySelector("#gc-launch"))==null||m.addEventListener("click",()=>{r&&g(a)}),(b=e.querySelector("#gc-no-gc"))==null||b.addEventListener("click",()=>g([])),(u=e.querySelector("#gc-reset"))==null||u.addEventListener("click",()=>{a.length&&(a=[],o())})}o()}function wn(e){var s;const t=((s=e==null?void 0:e.state)==null?void 0:s.params)||{},i=t.matchMode||"vs_ai_easy";return i==="friend"?`Match vs ${t.friendName||"Ami"}`:i==="random"?"Match vs Random":`Match vs IA — ${i.replace("vs_ai_","").toUpperCase()}`}async function _n(e,t,i){const{state:s,navigate:l}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:d}=await E.from("decks").select(`id,name,is_active,formation,stadium_card_id,
      stadium_card:cards!stadium_card_id(id,stadium_id,
        stadium_def:stadium_definitions(id,name,club_id,country_code,image_url,
          club:clubs(encoded_name,logo_url)))`).eq("owner_id",s.profile.id).order("created_at",{ascending:!1});if(!d||d.length===0){_t(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>l("decks"));return}const a=d.map(b=>b.id),{data:c}=await E.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,stadium_id,evolution_bonus,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order"),g=[...new Set((c||[]).filter(b=>{var u,k;return((u=b.card)==null?void 0:u.card_type)==="stadium"&&((k=b.card)==null?void 0:k.stadium_id)}).map(b=>b.card.stadium_id))],o={};if(g.length){const{data:b}=await E.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",g);(b||[]).forEach(u=>{o[u.id]=u}),(c||[]).forEach(u=>{var k,n;((k=u.card)==null?void 0:k.card_type)==="stadium"&&((n=u.card)!=null&&n.stadium_id)&&(u.card._stadiumDef=o[u.card.stadium_id]||null)})}let r=0;function m(){var C,D,j,W,de,re,P;const b=d[r],u=(c||[]).filter(X=>X.deck_id===b.id),k=u.filter(X=>{var U;return X.is_starter&&((U=X.card)==null?void 0:U.player)}).map(X=>mi(X.card,X.position)),n=u.find(X=>{var U;return((U=X.card)==null?void 0:U.card_type)==="formation"}),p=b.formation||((C=n==null?void 0:n.card)==null?void 0:C.formation)||"4-4-2";let I=k.length>=11?yt(k,p):null,L=((D=b.stadium_card)==null?void 0:D.stadium_def)||null;L&&I&&(I=Ht(I,L));const B=k.length>=11;et(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div style="display:flex;align-items:center;gap:8px;padding:8px;flex-shrink:0">
        ...
        <div style="flex:1;text-align:center">
          ...
          <div style="font-size:11px;opacity:.6;margin-top:2px">${p} · ${k.length}/11 ${b.is_active?"· ⭐ Actif":""}</div>
        </div>
      ...
      <div id="deck-swipe-zone" style="flex:1;min-height:0;overflow:hidden;position:relative;touch-action:pan-y;display:flex;align-items:center;justify-content:center;padding:4px">
        ${I?`<div class="deck-preview-wrap" style="aspect-ratio:1/1;max-width:100%;max-height:100%;width:auto;height:100%;overflow:hidden">${ft(I,p,null,[],285,285)}</div>`:`<div style="display:flex;align-items:center;justify-content:center;height:100%;opacity:.4;flex-direction:column;gap:8px">
              ...
              <div>Deck incomplet (${k.length}/11)</div>
             </div>`}
      </div>
    ...
    ${s&&!(e!=null&&e.used)?`<img src="${s}" style="position:absolute;top:${r}px;left:0;width:${t}px;height:${b}px;object-fit:cover;object-position:top center">`:""}
    <div style="position:absolute;bottom:0;left:0;right:0;height:${m}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ${Zt(e==null?void 0:e.country_code)?`<img src="${Zt(e.country_code)}" style="width:${m+2}px;height:${m-3}px;object-fit:cover;border-radius:1px">`:`<span style="font-size:${m-4}px">${o}</span>`}
  ...
  </div>`}function ot(e,t,i){if(!(e!=null&&e.length))return"";const s=e.slice(0,5);let l='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return s.forEach((d,a)=>{if(l+=Ye(d,40,52),a<s.length-1){const c=at(d,s[a+1]);l+=`<div style="width:7px;height:3px;background:${c==="#ff3333"||c==="#cc2222"?"rgba(255,255,255,0.12)":c};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(l+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),l+="</div>",l}function Zt(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function bo(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function ut(e,t,i,s,l=310,d=310,a=[]){var B;const c=Pt[t]||{},g=Yi(t)||hi[t]||[],o={},r=["ATT","MIL","DEF","GK"];for(const H of r)(e[H]||[]).forEach((D,j)=>{o[`${H}${j+1}`]=D});function m(H){const C=c[H];return C?{x:C.x*l,y:C.y*d}:null}let b="";for(const[H,C]of g){const D=m(H),j=m(C);if(!D||!j)continue;const W=o[H],de=o[C],re=at(W,de);re==="#00ff88"||re==="#FFD700"?(b+=`<line x1="${D.x.toFixed(1)}" y1="${D.y.toFixed(1)}" x2="${j.x.toFixed(1)}" y2="${j.y.toFixed(1)}"
        stroke="${re}" stroke-width="10" stroke-linecap="round" opacity="0.22"/>`,b+=`<line x1="${D.x.toFixed(1)}" y1="${D.y.toFixed(1)}" x2="${j.x.toFixed(1)}" y2="${j.y.toFixed(1)}"
        stroke="${re}" stroke-width="3.5" stroke-linecap="round" opacity="0.95"/>`):b+=`<line x1="${D.x.toFixed(1)}" y1="${D.y.toFixed(1)}" x2="${j.x.toFixed(1)}" y2="${j.y.toFixed(1)}"
        stroke="${re}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const u=48,k=64,n=13,p=16,I={normal:"#aaaaaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#111111",legende:"#7a28b8",légende:"#7a28b8"};for(const[H,C]of Object.entries(o)){const D=m(H);if(!D||!C)continue;const j=H.replace(/[0-9]/g,""),W=Mt[j]||"#555",de=a.includes(C.cardId),re=i==="attack"&&(["MIL","ATT"].includes(j)||de)&&!C.used||i==="defense"&&["GK","DEF","MIL"].includes(j)&&!C.used,P=s.includes(C.cardId);let X;i==="attack"?X=de?Math.max(1,Number(C.note_a)||0):j==="MIL"?Number(C.note_m)||0:Number(C.note_a)||0:i==="defense"?X=j==="GK"?Number(C.note_g)||0:j==="MIL"?Number(C.note_m)||0:Number(C.note_d)||0:X=Number(j==="GK"?C.note_g:j==="DEF"?C.note_d:j==="MIL"?C.note_m:C.note_a)||0,X=X+(C.boost||0),C.stadiumBonus&&(i==="attack"&&(j==="ATT"||j==="MIL")||i==="defense"&&(j==="GK"||j==="DEF"||j==="MIL")?X+=10:i||(X+=10));const U=(D.x-u/2).toFixed(1),N=(D.y-k/2).toFixed(1),ne=I[C==null?void 0:C.rarity]||I.normal;if(C.used){const Ae=typeof import.meta<"u"&&"/"||"/",xe=`${typeof window<"u"&&((B=window.location)==null?void 0:B.origin)||""}${Ae}icons/carte-dos.png`.replace(/([^:])\/\//g,"$1/");b+=`<rect x="${U}" y="${N}" width="${u}" height="${k}" rx="5" fill="#161616"/>`,b+=`<image href="${xe}" xlink:href="${xe}" x="${U}" y="${N}" width="${u}" height="${k}" preserveAspectRatio="xMidYMid slice"/>`,b+=`<rect x="${U}" y="${N}" width="${u}" height="${k}" rx="5" fill="none" stroke="${ne}" stroke-width="2" opacity="0.7"/>`,b+=`<rect x="${U}" y="${N}" width="${u}" height="${k}" rx="5" fill="rgba(0,0,0,0.01)" class="match-used-hit" data-card-id="${C.cardId}" data-role="${j}" style="cursor:pointer"/>`;continue}const pe=P?.45:1,ye=P?"#FFD700":ne,Le=P?3:(C==null?void 0:C.rarity)==="legende"||(C==null?void 0:C.rarity)==="légende"||(C==null?void 0:C.rarity)==="pepite"||(C==null?void 0:C.rarity)==="pépite"?2.5:2,we=k-n-p;b+=`<defs><clipPath id="cp-${H}"><rect x="${U}" y="${(D.y-k/2+n).toFixed(1)}" width="${u}" height="${we}"/></clipPath></defs>`,b+=`<rect x="${U}" y="${N}" width="${u}" height="${k}" rx="5" fill="${W}" opacity="${pe}"/>`;const ve=Pe(C);ve&&(b+=`<image href="${ve}" xlink:href="${ve}" x="${U}" y="${(D.y-k/2+n).toFixed(1)}" width="${u}" height="${we}" clip-path="url(#cp-${H})" preserveAspectRatio="xMidYMin slice"/>`),b+=`<rect x="${U}" y="${N}" width="${u}" height="${n}" rx="4" fill="rgba(255,255,255,0.92)"/>`,b+=`<text x="${D.x.toFixed(1)}" y="${(D.y-k/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(C.name||"").slice(0,9)}</text>`;const _e=(D.y+k/2-p).toFixed(1);b+=`<rect x="${U}" y="${_e}" width="${u}" height="${p}" fill="rgba(255,255,255,0.92)"/>`;{const Ae=Zt(C.country_code);Ae?b+=`<image href="${Ae}" xlink:href="${Ae}" x="${(D.x-20).toFixed(1)}" y="${(D.y+k/2-p+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:b+=`<text x="${(D.x-13).toFixed(1)}" y="${(D.y+k/2-p/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${bo(C.country_code)}</text>`;const Ie=C.stadiumBonus?"#E87722":"#111";b+=`<text x="${D.x.toFixed(1)}" y="${(D.y+k/2-p/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="${Ie}" font-family="Arial Black">${X}</text>`;const xe=yo(C);xe?b+=`<image href="${xe}" xlink:href="${xe}" x="${(D.x+u/2-14).toFixed(1)}" y="${(D.y+k/2-p+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:C.clubName&&(b+=`<text x="${(D.x+14).toFixed(1)}" y="${(D.y+k/2-p/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(C.clubName||"").slice(0,3).toUpperCase()}</text>`),C.stadiumBonus&&(b+=`<rect x="${(D.x-u/2).toFixed(1)}" y="${(D.y-k/2).toFixed(1)}" width="${u}" height="${k}" rx="5" fill="none" stroke="#E87722" stroke-width="2" opacity="0.8"/>`)}b+=`<rect x="${U}" y="${N}" width="${u}" height="${k}" rx="5" fill="${P?"rgba(255,255,255,0.12)":"none"}" stroke="${ye}" stroke-width="${Le}" opacity="${pe}"/>`,re&&(b+=`<rect x="${U}" y="${N}" width="${u}" height="${k}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${P?"selected":""}" data-card-id="${C.cardId}" data-role="${j}" style="cursor:pointer"/>`)}const L=38;return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-L} ${-L} ${l+L*2} ${d+L*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${b}
  ...
  </div>`}async function ai(e,t,i,s){var D;const{state:l,navigate:d,toast:a}=t;et(e);const c=l.params||{};if(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!c.deckId)return _n(e,t,i);const g=c.deckId;let o,r,m,b;try{const j=await Promise.all([E.from("decks").select("formation,name,stadium_card_id").eq("id",g).single(),E.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation, evolution_bonus,
            player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
              clubs(encoded_name,logo_url)))`).eq("deck_id",g).order("slot_order")]);o=j[0].data,m=j[0].error,r=j[1].data,b=j[1].error}catch(j){console.error("[Match] Exception chargement deck:",j),_t(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>d("home"));return}if(m||b){console.error("[Match] Erreur Supabase:",m||b),_t(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>d("home"));return}const u=(r||[]).filter(j=>{var W;return j.is_starter&&((W=j.card)==null?void 0:W.player)}).map(j=>mi(j.card,j.position)),k=(r||[]).filter(j=>{var W;return!j.is_starter&&((W=j.card)==null?void 0:W.player)}).map(j=>mi(j.card,j.position));if(u.length<11){_t(e,"⚠️",`Deck incomplet (${u.length}/11).`,"Compléter",()=>d("decks"));return}const n=(r||[]).find(j=>{var W;return((W=j.card)==null?void 0:W.card_type)==="formation"}),p=(o==null?void 0:o.formation)||((D=n==null?void 0:n.card)==null?void 0:D.formation)||"4-4-2",{data:I,error:L}=await E.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",l.profile.id).eq("card_type","game_changer"),{data:B}=await E.from("gc_definitions").select("*").eq("is_active",!0),H=(I||[]).map(j=>({...j,_gcDef:(B==null?void 0:B.find(W=>W.name===j.gc_type||W.id===j.gc_definition_id))||null}));let C=null;if(o!=null&&o.stadium_card_id){const{data:j}=await E.from("cards").select("stadium_id").eq("id",o.stadium_card_id).single();if(j!=null&&j.stadium_id){const{data:W}=await E.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").eq("id",j.stadium_id).single();C=W||null}}s({deckId:g,formation:p,starters:u,subsRaw:k,gcCardsEnriched:H,gcDefs:B||[],stadiumDef:C})}async function $n(e,t){const{state:i}=t,l=(i.params||{}).matchMode||"vs_ai_easy",d=l.replace("vs_ai_",""),a=l;await ai(e,t,l,async({deckId:c,formation:g,starters:o,subsRaw:r,gcCardsEnriched:m,gcDefs:b,stadiumDef:u})=>{try{let k=yt(o,g);u&&(k=Ht(k,u),oi(r,u));const n=await kn(g,d),p=async I=>{try{const{data:L,error:B}=await E.from("matches").insert({home_id:i.profile.id,away_id:null,mode:a,home_deck_id:c,status:"in_progress"}).select().single();if(B){console.error("[MatchIA] Erreur création match:",B),_t(e,"⚠️","Impossible de créer le match ("+B.message+").","Retour",()=>t.navigate("home"));return}const H={gcDefs:b||[],matchId:L==null?void 0:L.id,mode:a,difficulty:d,formation:g,homeTeam:k,aiTeam:n,homeSubs:r,subsUsed:0,maxSubs:Math.min(r.length,3),homeScore:0,aiScore:0,gcCards:I,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Tn(e,H,t)}catch(L){console.error("[MatchIA] Exception launchMatch:",L),_t(e,"⚠️","Erreur au lancement du match : "+L.message,"Retour",()=>t.navigate("home"))}};if(!m.length){p([]);return}ri(e,m,p)}catch(k){console.error("[MatchIA] Exception setup:",k),_t(e,"⚠️","Erreur de préparation du match : "+k.message,"Retour",()=>t.navigate("home"))}})}async function kn(e,t){var a,c;const{data:i}=await E.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(60);if(!i||i.length<11)return En(e);const s=It[e]||It["4-4-2"],l={GK:[],DEF:[],MIL:[],ATT:[]},d=[...i];for(const g of["GK","DEF","MIL","ATT"]){const o=d.filter(k=>k.job===g),r=d.filter(k=>k.job!==g),m=[...o,...r],b=[];for(let k=0;k<s[g];k++){const n=m[k]||d[k];n&&b.push({cardId:"ai-"+n.id+"-"+k,id:n.id,firstname:n.firstname,name:n.surname_encoded,country_code:n.country_code,club_id:n.club_id,job:n.job,job2:n.job2,note_g:Number(n.note_g)||0,note_d:Number(n.note_d)||0,note_m:Number(n.note_m)||0,note_a:Number(n.note_a)||0,rarity:n.rarity,skin:n.skin,hair:n.hair,hair_length:n.hair_length,clubName:((a=n.clubs)==null?void 0:a.encoded_name)||null,clubLogo:((c=n.clubs)==null?void 0:c.logo_url)||null,boost:0,used:!1,_line:g})}const u=lt(b.length);b.forEach((k,n)=>{k._col=u[n]}),l[g]=b}return l}function En(e){const t=It[e]||It["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},s=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let l=0;for(const d of["GK","DEF","MIL","ATT"]){const a=[];for(let g=0;g<t[d];g++){const o=3+Math.floor(Math.random()*5);a.push({cardId:"fake-"+l,id:"fake-"+l,firstname:"IA",name:s[l%s.length],country_code:"XX",club_id:null,job:d,job2:null,note_g:d==="GK"?o:2,note_d:d==="DEF"?o:2,note_m:d==="MIL"?o:2,note_a:d==="ATT"?o:2,rarity:"normal",boost:0,used:!1,_line:d}),l++}const c=lt(a.length);a.forEach((g,o)=>{g._col=c[o]}),i[d]=a}return i}function Tn(e,t,i){e.innerHTML=`
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
  </div>`,setTimeout(()=>An(e,t,i),5e3)}function An(e,t,i){const s=t.homeTeam.MIL||[],l=t.aiTeam.MIL||[];function d(n){return n.reduce((p,I)=>p+je(I,"MIL"),0)}function a(n){let p=0;for(let I=0;I<n.length-1;I++){const L=at(n[I],n[I+1]);L==="#00ff88"?p+=2:L==="#FFD700"&&(p+=1)}return p}const c=d(s)+a(s),g=d(l)+a(l),o=c>=g;function r(n,p,I,L){return`<div id="duel-row-${L}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0), opacity .5s ease;transform-origin:center">
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${n.map((B,H)=>{const C=H<n.length-1?at(B,n[H+1]):null,D=!C||C==="#ff3333"||C==="#cc2222",j=C==="#00ff88"?"+2":C==="#FFD700"?"+1":"";return`
          ...
          ${H<n.length-1?`<div class="duel-link duel-link-${L}" data-idx="${H}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${D?"rgba(255,255,255,0.12)":C};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${D?"none":`0 0 8px ${C}`}">
      ...
      <div class="duel-score-line duel-score-line-${L}" style="margin-top:10px;font-size:12px;color:rgba(255,255,255,0.55);opacity:0;transition:opacity .4s ease">
        Score: ${d(n)} + ${a(n)} liens = <b style="color:#fff">${d(n)+a(n)}</b>
      </div>
    </div>`}e.innerHTML=`
  <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div id="duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center, rgba(10,61,30,0.4), rgba(10,61,30,0.92))"></div>
  </div>`;const m=()=>{const n=(p,I)=>e.querySelectorAll(p).forEach((L,B)=>{setTimeout(()=>{L.style.opacity="1",L.style.transform="translateY(0) scale(1)"},I+B*90)});n(".duel-card-home",150),n(".duel-card-ai",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((p,I)=>{setTimeout(()=>{p.style.opacity="1"},I*70)}),900),setTimeout(()=>{const p=e.querySelector("#vs-label");p&&(p.style.opacity="1",p.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(I=>I.style.opacity="1")},1250),setTimeout(()=>{b("score-home",c,800),b("score-ai",g,800)},1500)};function b(n,p,I){const L=document.getElementById(n);if(!L)return;const B=performance.now(),H=C=>{const D=Math.min(1,(C-B)/I);L.textContent=Math.round(p*(1-Math.pow(1-D,3))),D<1?requestAnimationFrame(H):L.textContent=p};requestAnimationFrame(H)}requestAnimationFrame(m),t.attacker=o?"home":"ai";const u=o?ni():null;o&&(t.boostCard={value:u}),t.log.push({type:"duel",title:"Milieu de Terrain",homePlayers:s.map(n=>({name:n.name,note:je(n,"MIL"),portrait:Pe(n),job:n.job,country_code:n.country_code,rarity:n.rarity,clubName:n.clubName,clubLogo:n.clubLogo})),aiPlayers:l.map(n=>({name:n.name,note:je(n,"MIL"),portrait:Pe(n),job:n.job,country_code:n.country_code,rarity:n.rarity,clubName:n.clubName,clubLogo:n.clubLogo})),homeTotal:c,aiTotal:g,text:`Duel milieu : ${t.clubName} ${c} – ${g} IA → ${o?t.clubName+" attaque":"IA attaque"}`});const k=()=>{t.phase=t.attacker==="home"?"attack":"ai-attack",Xe(e,t,i),t.attacker==="ai"&&setTimeout(()=>xi(e,t,i),800)};setTimeout(()=>{const n=document.getElementById("score-home"),p=document.getElementById("score-ai"),I=document.getElementById(o?"duel-row-home":"duel-row-ai"),L=document.getElementById(o?"duel-row-ai":"duel-row-home"),B=o?n:p,H=o?p:n;B&&(B.style.fontSize="80px",B.style.color=o?"#FFD700":"#ff6b6b",B.style.animation="duelPulse .5s ease"+(o?", duelGlow 1.5s ease infinite .5s":"")),H&&(H.style.opacity="0.25"),setTimeout(()=>{I&&(I.style.transformOrigin="center",I.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",I.style.zIndex="5"),setTimeout(()=>{var D;const C=document.getElementById("duel-shock");if(C){const j=(D=L||I)==null?void 0:D.getBoundingClientRect(),W=e.querySelector(".match-screen").getBoundingClientRect();j&&(C.style.top=j.top-W.top+j.height/2+"px"),C.style.animation="shockwave .5s ease-out forwards"}L&&(L.style.transformOrigin="center",L.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var D;const C=document.getElementById("duel-finale");C&&(C.innerHTML=`
          ...
          <button id="start-match-btn" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,0.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">
            ▶ Commencer le match
          </button>`,C.style.transition="opacity .45s ease",C.style.opacity="1",C.style.pointerEvents="auto",(D=document.getElementById("start-match-btn"))==null||D.addEventListener("click",k))},600)},700)},2800)}function zn(e){if(e.type==="duel"&&(e.homeTotal!=null||e.aiTotal!=null)){const t=(e.homeTotal||0)>=(e.aiTotal||0);return`
    ...
    <div style="display:flex;align-items:center;gap:4px;${t?"flex-direction:row-reverse":""};background:rgba(255,255,255,0.04);border-radius:8px;padding:5px 8px;border:1px solid rgba(255,255,255,0.07)">
      ...
      ${dt(e.inPlayer||{})}
    </div>`}return`<div style="font-size:11px;color:${e.type==="goal"?"#FFD700":"rgba(255,255,255,0.65)"};font-weight:${e.type==="goal"?700:400};padding:3px 2px">${e.text||""}</div>`}function Xe(e,t,i){var B,H,C,D,j,W,de,re;const s=t.selected.map(P=>P.cardId),l=t.usedSubIds||[],d=t.homeSubs.filter(P=>!l.includes(P.cardId)),c=Object.values(t.homeTeam).flat().filter(P=>P.used).length>0&&d.length>0&&t.subsUsed<t.maxSubs,g=!["GK","DEF","MIL","ATT"].some(P=>(t.aiTeam[P]||[]).some(X=>!X.used)),o=[...t.homeTeam.MIL||[],...t.homeTeam.ATT||[]].filter(P=>!P.used),r=t.phase==="attack"&&g&&o.length===0?[...t.homeTeam.GK||[],...t.homeTeam.DEF||[]].filter(P=>!P.used).map(P=>P.cardId):[];t.log[t.log.length-1];const m=t.phase==="ai-attack"||t.phase==="ai-defense",b=t.phase==="attack",u=t.phase==="defense",k=t.phase==="finished",n=t.gcCards.filter(P=>!t.usedGc.includes(P.id)),p=t.boostCard&&!t.boostUsed;e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column",e.innerHTML=`
  <style>
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    ${(()=>{const P=window.innerWidth>=700,X=(_e,Ae,Ie)=>{var be,ze;const xe=(t.gcDefs||[]).find(M=>M.name===_e.gc_type),J={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[xe==null?void 0:xe.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",oe={purple:"#b06ce0",light_blue:"#00d4ef"}[xe==null?void 0:xe.color]||"#b06ce0",ge=(xe==null?void 0:xe.name)||_e.gc_type,Y=(xe==null?void 0:xe.effect)||((be=Ve[_e.gc_type])==null?void 0:be.desc)||"",Z=xe!=null&&xe.image_url?`/icons/${xe.image_url}`:null,K=((ze=Ve[_e.gc_type])==null?void 0:ze.icon)||"⚡",se=Math.round(Ie*.22),he=Math.round(Ie*.22),Se=Ie-se-he,Me=ge.length>12?7:9;return`<div class="gc-mini" data-gc-id="${_e.id}" data-gc-type="${_e.gc_type}"
          ...
          <div style="height:${Se}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
            ${Z?`<img src="${Z}" style="max-width:${Ae-10}px;max-height:${Se-6}px;object-fit:contain">`:`<span style="font-size:${Math.round(Se*.55)}px">${K}</span>`}
          </div>
        ...
        <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
          ${ft(t.homeTeam,t.formation,t.phase,s,300,300,r)}
        </div>
      </div>`;return P?`
  ...
  </div>`;function I(){const P=e.querySelector(".match-screen");if(!P)return;const X=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);P.style.bottom="auto",P.style.height=X+"px",P.style.minHeight=X+"px",P.style.maxHeight=X+"px",P.style.overflow="hidden";const U=e.querySelector("#mobile-action-bar"),N=e.querySelector("#mobile-play-area");U&&N&&(N.style.paddingBottom=U.offsetHeight+"px")}if(I(),setTimeout(I,120),setTimeout(I,400),setTimeout(I,1e3),t._vvBound||(t._vvBound=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",I),window.visualViewport.addEventListener("scroll",I)),window.addEventListener("resize",I)),function(){const X=e.querySelector(".terrain-wrapper svg");X&&(X.removeAttribute("width"),X.removeAttribute("height"),X.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",X.setAttribute("viewBox","-26 -26 352 352"),X.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),t._resizeBound||(t._resizeBound=!0,window.addEventListener("resize",()=>{const P=e.querySelector(".terrain-wrapper svg");P&&(P.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0")})),t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase==="attack"||t.phase==="defense"){let P=!1,X=30;const U=()=>document.getElementById("match-timer"),N=()=>{const ne=U();if(!ne)return;const pe=String(Math.floor(X/60)).padStart(2,"0"),ye=String(X%60).padStart(2,"0");ne.textContent=` ${pe}:${ye}`,ne.style.color=P?"#ff2222":"#ff9500",ne.style.fontWeight="900"};N(),t._timerInt=setInterval(()=>{if(X--,X<0)if(!P)P=!0,X=15,N();else{clearInterval(t._timerInt),t._timerInt=null,t.homeScore=0,t.aiScore=3;const ne=document.createElement("div");ne.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;color:#fff;padding:24px;text-align:center",ne.innerHTML='<div style="font-size:56px">⏱️</div><div style="font-size:24px;font-weight:900;color:#ff4444">MATCH PERDU PAR FORFAIT</div><div style="font-size:14px;color:rgba(255,255,255,0.6)">Temps écoulé</div>',document.body.appendChild(ne),setTimeout(()=>{ne.remove(),Bt(e,t,i)},2500)}else N()},1e3)}(B=document.getElementById("match-quit"))==null||B.addEventListener("click",()=>{We(e),confirm("Abandonner ? Résultat : défaite 3-0")&&(t.homeScore=0,t.aiScore=3,Bt(e,t,i))}),(H=document.getElementById("view-ai"))==null||H.addEventListener("click",()=>Dn(t,i)),(C=document.getElementById("toggle-history"))==null||C.addEventListener("click",()=>{var P;(P=document.getElementById("match-history-panel"))==null||P.classList.add("open")}),(D=document.getElementById("close-history"))==null||D.addEventListener("click",()=>{var P;(P=document.getElementById("match-history-panel"))==null||P.classList.remove("open")}),(j=document.getElementById("btn-action"))==null||j.addEventListener("click",()=>{t.selected.length!==0&&(b?Ln(e,t,i):u&&In(e,t,i))}),(W=document.getElementById("btn-results"))==null||W.addEventListener("click",()=>Bt(e,t,i)),e.querySelectorAll(".match-slot-hit").forEach(P=>{P.addEventListener("click",()=>Sn(P,t,e,i))}),e.querySelectorAll(".match-used-hit").forEach(P=>{P.addEventListener("click",()=>li(e,t,i,null,P.dataset.cardId))}),e.querySelectorAll(".gc-mini").forEach(P=>{P.addEventListener("click",()=>Cn(P.dataset.gcId,P.dataset.gcType,e,t,i))}),(de=document.getElementById("boost-card"))==null||de.addEventListener("click",()=>Fn(e,t,i)),e.querySelectorAll(".sub-btn-col").forEach(P=>{P.addEventListener("click",()=>li(e,t,i,P.dataset.subId))}),(re=document.getElementById("sub-btn-main"))==null||re.addEventListener("click",()=>li(e,t,i))}function Sn(e,t,i,s){const l=e.dataset.cardId,d=e.dataset.role,a=t.selected.findIndex(c=>c.cardId===l);if(a!==-1)t.selected.splice(a,1);else{if(t.selected.length>=3){s.toast("Maximum 3 joueurs","error");return}const c=(t.homeTeam[d]||[]).find(g=>g.cardId===l);c&&t.selected.push({...c,_role:d,_line:d})}Xe(i,t,s)}function Ti(e,t,i){e.matchId&&E.from("matches").update({last_player_id:i}).eq("id",e.matchId).then(()=>{})}function Ln(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),Ti(t,i,i.state.profile.id);const s=!["GK","DEF","MIL","ATT"].some(a=>(t.aiTeam[a]||[]).some(c=>!c.used)),l=t.selected.map(a=>{const c=(t.homeTeam[a._role]||[]).find(o=>o.cardId===a.cardId)||a,g=s&&["GK","DEF"].includes(a._role);return{...c,_line:a._role,...g?{note_a:Math.max(1,Number(c.note_a)||0)}:{}}}),d=Nt(l,t.modifiers.home);t.pendingAttack={...d,players:[...l],side:"home"},t.selected.forEach(a=>{const c=(t.homeTeam[a._role]||[]).find(g=>g.cardId===a.cardId);c&&(c.used=!0)}),t.log.push({text:`⚔️ Vous attaquez : ${d.total} (base ${d.base}${d.links?` +${d.links} liens`:""}) — ${t.selected.map(a=>a.name).join(", ")}`,type:"info"}),t.selected=[],t.modifiers.home={},t.phase="ai-defense",Xe(e,t,i),setTimeout(()=>Mn(e,t,i),1200)}function In(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),Ti(t,i,i.state.profile.id);const s=t.selected.map(c=>({...(t.homeTeam[c._role]||[]).find(o=>o.cardId===c.cardId)||c,_line:c._role})),l=Rt(s,t.modifiers.home);t.selected.forEach(c=>{const g=(t.homeTeam[c._role]||[]).find(o=>o.cardId===c.cardId);g&&(g.used=!0)});const d=Ot(t.pendingAttack.total,l.total,t.modifiers.home),a={type:"duel",title:"Défense",aiPlayers:(t.pendingAttack.players||[]).map(c=>({name:c.name,note:c._line==="MIL"?c.note_m:c.note_a,portrait:Pe(c),job:c.job,country_code:c.country_code,rarity:c.rarity,clubName:c.clubName,clubLogo:c.clubLogo})),homePlayers:t.selected.map(c=>{const g=(t.homeTeam[c._role]||[]).find(o=>o.cardId===c.cardId)||c;return{name:g.name,note:(g._line==="GK"?Number(g.note_g)||0:g._line==="MIL"?Number(g.note_m)||0:Number(g.note_d)||0)+(g.boost||0),portrait:Pe(g),job:g.job,country_code:g.country_code,rarity:g.rarity,clubName:g.clubName,clubLogo:g.clubLogo}}),homeTotal:l.total,aiTotal:t.pendingAttack.total,isGoal:!1,homeScored:!1,text:""};if(d.shielded)a.text="🛡️ Bouclier ! But annulé.",t.log.push(a);else if(d.goal){t.aiScore++,a.isGoal=!0,a.homeScored=!1,a.text=`⚽ BUT IA ! (${t.pendingAttack.total} > ${l.total})`,t.log.push(a),t.selected=[],t.modifiers.home={},t.pendingAttack=null,Xe(e,t,i),Qt(a.aiPlayers,t.homeScore,t.aiScore,!1,()=>{St(e,t,i,"home-attack")});return}else a.text=`🧤 Défense réussie ! (${l.total} ≥ ${t.pendingAttack.total})`,t.log.push(a);t.selected=[],t.modifiers.home={},t.pendingAttack=null,St(e,t,i,"home-attack")}function xi(e,t,i){Ti(t,i,null);const s=[...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]],l=to(s,"attack",t.difficulty);if(!l.length){Jt(e,t,i);return}const d=Nt(l,t.modifiers.ai);t.pendingAttack={...d,players:l,side:"ai"},l.forEach(o=>{o.used=!0}),t.log.push({text:`🤖 IA attaque : ${d.total} (${l.map(o=>o.name).join(", ")})`,type:"info"}),t.modifiers.ai={};const a=[...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(o=>!o.used),g=(t.homeSubs||[]).filter(o=>!(t.usedSubIds||[]).includes(o.cardId)).length>0&&t.subsUsed<t.maxSubs;if(a.length===0&&!g){t.aiScore++;const o={type:"duel",isGoal:!0,homeScored:!1,aiPlayers:l.map(r=>({name:r.name,note:r._line==="MIL"?r.note_m:r.note_a,portrait:Pe(r),job:r.job,country_code:r.country_code,rarity:r.rarity,clubName:r.clubName,clubLogo:r.clubLogo})),aiTotal:d.total,text:"⚽ BUT IA ! (aucun défenseur disponible)"};t.log.push(o),t.pendingAttack=null,Xe(e,t,i),Qt(o.aiPlayers,t.homeScore,t.aiScore,!1,()=>{St(e,t,i,"home-attack")});return}t.phase="defense",Xe(e,t,i)}function Mn(e,t,i){var o,r;const s=[...t.aiTeam.GK||[],...t.aiTeam.DEF||[],...t.aiTeam.MIL||[]],l=to(s,"defense",t.difficulty);if(!["GK","DEF","MIL","ATT"].flatMap(m=>(t.aiTeam[m]||[]).filter(b=>!b.used)).length){t.homeScore++;const b={type:"duel",isGoal:!0,homeScored:!0,homePlayers:(((o=t.pendingAttack)==null?void 0:o.players)||[]).map(u=>({name:u.name,note:je(u,u._line||u.job),portrait:Pe(u),job:u.job,country_code:u.country_code,rarity:u.rarity,clubName:u.clubName,clubLogo:u.clubLogo})),homeTotal:((r=t.pendingAttack)==null?void 0:r.total)||0,aiTotal:0,text:"⚽ BUT ! L'IA n'a plus de joueurs — but automatique !"};t.log.push(b),t.modifiers.ai={},t.pendingAttack=null,Xe(e,t,i),Qt(b.homePlayers,t.homeScore,t.aiScore,!0,()=>{St(e,t,i,"ai-attack")});return}const a=l.length>0?Rt(l,t.modifiers.ai).total:0;l.forEach(m=>{m.used=!0});const c=Ot(t.pendingAttack.total,a,t.modifiers.ai),g={type:"duel",title:"Attaque",homePlayers:(t.pendingAttack.players||[]).map(m=>({name:m.name,note:m._line==="MIL"?m.note_m:m.note_a,portrait:Pe(m),job:m.job,country_code:m.country_code,rarity:m.rarity,clubName:m.clubName,clubLogo:m.clubLogo})),aiPlayers:l.map(m=>({name:m.name,note:m._line==="GK"?m.note_g:m._line==="MIL"?m.note_m:m.note_d,portrait:Pe(m),job:m.job,country_code:m.country_code,rarity:m.rarity,clubName:m.clubName,clubLogo:m.clubLogo})),homeTotal:t.pendingAttack.total,aiTotal:a,isGoal:!1,homeScored:!1,text:""};if(c.shielded)g.text="🛡️ Bouclier IA !",t.log.push(g);else if(c.goal){t.homeScore++,g.isGoal=!0,g.homeScored=!0,g.text=`⚽ BUT ! (${t.pendingAttack.total} > ${a})`,t.log.push(g),t.modifiers.ai={},t.pendingAttack=null,Xe(e,t,i),Qt(g.homePlayers,t.homeScore,t.aiScore,!0,()=>{St(e,t,i,"ai-attack")});return}else g.text=`🧤 IA défend (${a} ≥ ${t.pendingAttack.total})`,t.log.push(g);t.modifiers.ai={},t.pendingAttack=null,St(e,t,i,"ai-attack")}function St(e,t,i,s){if(t.round++,ho(t)){Bt(e,t,i);return}if(s==="home-attack"){if(![...t.homeTeam.MIL||[],...t.homeTeam.ATT||[]].filter(d=>!d.used).length){if(![...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(c=>!c.used).length){Jt(e,t,i);return}if(![...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]].filter(c=>!c.used).length){Jt(e,t,i);return}setTimeout(()=>xi(e,t,i),100);return}t.phase="attack",Xe(e,t,i)}else{if(![...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]].filter(d=>!d.used).length){Jt(e,t,i);return}t.phase="ai-attack",Xe(e,t,i),setTimeout(()=>xi(e,t,i),800)}}function ho(e){const t=["MIL","ATT","GK","DEF"].some(s=>(e.homeTeam[s]||[]).some(l=>!l.used)),i=["MIL","ATT","GK","DEF"].some(s=>(e.aiTeam[s]||[]).some(l=>!l.used));return!t&&!i}function Jt(e,t,i){ho(t)?Bt(e,t,i):(t.phase="attack",Xe(e,t,i))}function jn(e,t,i){const s=document.createElement("div");s.style.cssText=`
    position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:800;
    display:flex;flex-direction:column;align-items:center;justify-content:center;gap:20px;
    animation:subFadeIn 0.2s ease;
  `;const l=Pe(e),d=Pe(t),a=Mt[e.job]||"#555",c=Mt[t.job]||"#555",g=e.job==="GK"?e.note_g:e.job==="DEF"?e.note_d:e.job==="MIL"?e.note_m:e.note_a,o=t.job==="GK"?t.note_g:t.job==="DEF"?t.note_d:t.job==="MIL"?t.note_m:t.note_a;s.innerHTML=`
    <style>
  ...
  `,document.body.appendChild(s);let r=!1;const m=()=>{r||(r=!0,s.remove(),i())};s.addEventListener("click",m),setTimeout(m,2e3)}function jt(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function li(e,t,i,s=null,l=null){var u,k;if(t.phase!=="attack"){jt("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){jt(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const d=Object.entries(t.homeTeam).flatMap(([n,p])=>(p||[]).filter(I=>I.used).map(I=>({...I,_line:I._line||n}))),a=t.homeSubs.filter(n=>!t.usedSubIds.includes(n.cardId));if(!d.length){jt("Aucun joueur utilisé à remplacer");return}if(!a.length){jt("Aucun remplaçant disponible");return}let c=Math.max(0,d.findIndex(n=>n.cardId===l));const g=((u=d[c])==null?void 0:u._line)||((k=d[c])==null?void 0:k.job);let o=s?Math.max(0,a.findIndex(n=>n.cardId===s)):Math.max(0,a.findIndex(n=>n.job===g)),r=!1;const m=document.createElement("div");m.id="sub-overlay",m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function b(){var C,D,j,W,de,re;const n=d[c],p=a[o],I=Math.min(130,Math.round((window.innerWidth-90)/2)),L=Math.round(I*1.35),B=P=>`background:rgba(255,255,255,0.12);border:none;color:${P?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${P?"default":"pointer"};flex-shrink:0`;m.innerHTML=`
    ...
    <div style="flex:1;display:flex;gap:0;overflow:hidden">
      ...
      <div id="in-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
        ...
        <button id="in-up" style="${B(o===0)}" ${o===0?"disabled":""}>▲</button>
        <div>${p?Ye({...p,used:!1,boost:0},I,L):"<div>—</div>"}</div>
        <button id="in-down" style="${B(o>=a.length-1)}" ${o>=a.length-1?"disabled":""}>▼</button>
    ...
    </div>`,(C=m.querySelector("#sub-close"))==null||C.addEventListener("click",()=>m.remove()),(D=m.querySelector("#out-up"))==null||D.addEventListener("click",()=>{c>0&&(c--,b())}),(j=m.querySelector("#out-down"))==null||j.addEventListener("click",()=>{c<d.length-1&&(c++,b())}),(W=m.querySelector("#in-up"))==null||W.addEventListener("click",()=>{o>0&&(o--,b())}),(de=m.querySelector("#in-down"))==null||de.addEventListener("click",()=>{o<a.length-1&&(o++,b())});const H=(P,X,U,N)=>{const ne=m.querySelector("#"+P);if(!ne)return;let pe=0;ne.addEventListener("touchstart",ye=>{pe=ye.touches[0].clientY},{passive:!0}),ne.addEventListener("touchend",ye=>{const Le=ye.changedTouches[0].clientY-pe;if(Math.abs(Le)<30)return;const we=X();Le<0&&we<N-1?(U(we+1),b()):Le>0&&we>0&&(U(we-1),b())},{passive:!0})};H("in-panel",()=>o,P=>o=P,a.length),H("out-panel",()=>c,P=>c=P,d.length),(re=m.querySelector("#sub-confirm"))==null||re.addEventListener("click",P=>{if(P.preventDefault(),P.stopPropagation(),r)return;r=!0;const X=d[c],U=a[o];if(!X||!U)return;let N=null,ne=-1;for(const[ye,Le]of Object.entries(t.homeTeam)){const we=(Le||[]).findIndex(ve=>ve.cardId===X.cardId);if(we!==-1){N=ye,ne=we;break}}if(ne===-1||!N){jt("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),m.remove();return}const pe={...U,_line:N,_col:X._col||0,used:!1,boost:0};t.homeTeam[N].splice(ne,1,pe),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(U.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:X.name,firstname:X.firstname,note:je(X,N),portrait:Pe(X),job:X.job,country_code:X.country_code,rarity:X.rarity,clubName:X.clubName,clubLogo:X.clubLogo},inPlayer:{name:U.name,firstname:U.firstname,note:je(U,N),portrait:Pe(U),job:U.job,country_code:U.country_code,rarity:U.rarity,clubName:U.clubName,clubLogo:U.clubLogo},text:`🔄 ${U.firstname} ${U.name} remplace ${X.firstname} ${X.name}`}),m.remove(),jn(X,U,()=>Xe(e,t,i))})}document.body.appendChild(m),b()}function Cn(e,t,i,s,l){var k,n;const d=(s.gcDefs||[]).find(p=>p.name===t),a=Ve[t]||{icon:"⚡",desc:"Carte spéciale."},c={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[d==null?void 0:d.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",g={purple:"#b06ce0",light_blue:"#00d4ef"}[d==null?void 0:d.color]||"#b06ce0",o=(d==null?void 0:d.name)||t,r=(d==null?void 0:d.effect)||a.desc,m=d!=null&&d.image_url?`/icons/${d.image_url}`:null,b=a.icon||"⚡",u=document.createElement("div");u.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",u.innerHTML=`
    ...
    </div>`,document.body.appendChild(u),(k=u.querySelector("#gc-back"))==null||k.addEventListener("click",()=>u.remove()),(n=u.querySelector("#gc-use"))==null||n.addEventListener("click",()=>{u.remove(),Bn(e,t,i,s,l)})}function Yt(e,t,i,s,l,d){const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let c=[];function g(){var o,r;a.innerHTML=`
    ...
    <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
      ${e.map(m=>{const b=m._line||m.job||"MIL",u={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[b]||"#555",k=je(m,b)+(m.boost||0),n=c.find(p=>p.cardId===m.cardId);return`<div class="gc-pick-item" data-cid="${m.cardId}"
    ...
    </div>`,(o=a.querySelector("#gc-picker-close"))==null||o.addEventListener("click",()=>a.remove()),a.querySelectorAll(".gc-pick-item").forEach(m=>{m.addEventListener("click",()=>{const b=m.dataset.cid,u=e.find(n=>n.cardId===b);if(!u)return;const k=c.findIndex(n=>n.cardId===b);k>-1?c.splice(k,1):c.length<t&&c.push(u),g()})}),(r=a.querySelector("#gc-picker-confirm"))==null||r.addEventListener("click",()=>{a.remove(),d(c)})}g(),document.body.appendChild(a)}const qn={BOOST_STAT:({value:e=1,count:t=1,roles:i=[]},s,l,d)=>{const a=Object.entries(s.homeTeam).filter(([c])=>!i.length||i.includes(c)).flatMap(([c,g])=>g.filter(o=>!o.used).map(o=>({...o,_line:c})));return a.length?(Yt(a,t,`Choisir ${t} joueur(s) à booster (+${e})`,l,s,c=>{c.forEach(g=>{const o=(s.homeTeam[g._line]||[]).find(r=>r.cardId===g.cardId);o&&(o.boost=(o.boost||0)+e,s.log.push({text:`⚡ +${e} sur ${o.name}`,type:"info"}))}),Xe(l,s,d)}),!0):(s.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),Xe(l,s,d),!0)},DEBUFF_STAT:({value:e=1,count:t=1,roles:i=[],target:s="ai"},l,d,a)=>{const c=s==="home"?l.homeTeam:l.aiTeam,g=s==="ai"?"adverse":"allié",o=Object.entries(c).filter(([r])=>!i.length||i.includes(r)).flatMap(([r,m])=>m.filter(b=>!b.used).map(b=>({...b,_line:r})));return o.length?(Yt(o,t,`Choisir ${t} joueur(s) ${g}(s) à débuffer (-${e})`,d,l,r=>{r.forEach(m=>{const u=((s==="home"?l.homeTeam:l.aiTeam)[m._line]||[]).find(k=>k.cardId===m.cardId);u&&(u.boost=(u.boost||0)-e,l.log.push({text:`🎯 -${e} sur ${u.name}${s==="ai"?" (IA)":""}`,type:"info"}))}),Xe(d,l,a)}),!0):(l.log.push({text:`🎯 Aucun joueur ${g} disponible`,type:"info"}),Xe(d,l,a),!0)},GRAY_PLAYER:({count:e=1,roles:t=[],target:i="ai"},s,l,d)=>{const a=i==="home"?s.homeTeam:s.aiTeam,c=i==="ai"?"adverse":"allié",g=Object.entries(a).filter(([o])=>!t.length||t.includes(o)).flatMap(([o,r])=>r.filter(m=>!m.used).map(m=>({...m,_line:o})));return g.length?(Yt(g,e,`Choisir ${e} joueur(s) ${c}(s) à exclure`,l,s,o=>{o.forEach(r=>{const b=((i==="home"?s.homeTeam:s.aiTeam)[r._line]||[]).find(u=>u.cardId===r.cardId);b&&(b.used=!0,s.log.push({text:`❌ ${b.name}${i==="ai"?" (IA)":""} exclu !`,type:"info"}))}),Xe(l,s,d)}),!0):(s.log.push({text:`❌ Aucun joueur ${c} à exclure`,type:"info"}),Xe(l,s,d),!0)},REVIVE_PLAYER:({count:e=1,roles:t=[]},i,s,l)=>{const d=Object.entries(i.homeTeam).filter(([a])=>!t.length||t.includes(a)).flatMap(([a,c])=>c.filter(g=>g.used).map(g=>({...g,_line:a})));return d.length?(Yt(d,e,`Choisir ${e} joueur(s) à ressusciter`,s,i,a=>{a.forEach(c=>{const g=(i.homeTeam[c._line]||[]).find(o=>o.cardId===c.cardId);g&&(g.used=!1,i.log.push({text:`💫 ${g.name} ressuscité !`,type:"info"}))}),Xe(s,i,l)}),!0):(i.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),Xe(s,i,l),!0)},REMOVE_GOAL:({},e)=>(e.aiScore>0?(e.aiScore--,e.log.push({text:"🚫 Dernier but IA annulé !",type:"info"})):e.log.push({text:"🚫 Aucun but à annuler",type:"info"}),!1),ADD_GOAL_DRAW:({},e)=>(e.homeScore===e.aiScore?(e.homeScore++,e.log.push({text:"🎯 But bonus (match nul) !",type:"info"})):e.log.push({text:"🎯 But bonus : non applicable (pas de match nul)",type:"info"}),!1),ADD_SUB:({value:e=1},t)=>(t.maxSubs=(t.maxSubs||3)+e,t.log.push({text:`🔄 +${e} remplacement(s) débloqué(s)`,type:"info"}),!1),CUSTOM:()=>!1};function Bn(e,t,i,s,l){s.usedGc.push(e);const d=s.gcDefs||[],a=d.find(g=>g.name===t)||d.find(g=>{var o;return((o=g.name)==null?void 0:o.toLowerCase().trim())===(t==null?void 0:t.toLowerCase().trim())});let c=!1;if(a!=null&&a.effect_type&&a.effect_type!=="CUSTOM"){const g=qn[a.effect_type];g?g(a.effect_params||{},s,i,l)||(c=!0):(l.toast(`Effet "${a.effect_type}" non implémenté`,"error"),c=!0)}else{switch(t){case"Double attaque":s.modifiers.home.doubleAttack=!0,s.log.push({text:"⚡ Double attaque activée !",type:"info"});break;case"Bouclier":s.modifiers.home.shield=!0,s.log.push({text:"🛡️ Bouclier activé !",type:"info"});break;case"Ressusciter":{const g=Object.entries(s.homeTeam).flatMap(([o,r])=>(r||[]).filter(m=>m.used).map(m=>({...m,_line:o})));g.length?(g[0].used=!1,s.log.push({text:`💫 ${g[0].name} ressuscité !`,type:"info"})):s.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"});break}case"Vol de note":s.modifiers.ai.stolenNote=(s.modifiers.ai.stolenNote||0)+1,s.log.push({text:"🎯 -1 à la prochaine attaque IA",type:"info"});break;case"Gel":{const g=[...s.aiTeam.ATT||[],...s.aiTeam.MIL||[]].filter(o=>!o.used);if(g.length){const o=g.sort((r,m)=>je(m,"ATT")-je(r,"ATT"))[0];o.used=!0,s.log.push({text:`❄️ ${o.name} (IA) gelé !`,type:"info"})}break}case"Remplacement+":s.maxSubs++,s.log.push({text:"🔄 +1 remplacement débloqué",type:"info"});break}c=!0}E.from("cards").delete().eq("id",e).then(()=>{}),c&&Xe(i,s,l)}function Fn(e,t,i){const s=Object.values(t.homeTeam).flat().filter(l=>!l.used);if(!s.length){i.toast("Aucun joueur actif à booster","error");return}i.openModal("⚡ Utiliser le Boost",`<div style="margin-bottom:12px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border-radius:10px;padding:12px;text-align:center;color:#000">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(l=>{l.addEventListener("click",()=>{const d=l.dataset.cardId;for(const a of["GK","DEF","MIL","ATT"]){const c=(t.homeTeam[a]||[]).find(g=>g.cardId===d);if(c){c.boost=(c.boost||0)+t.boostCard.value,t.log.push({text:`⚡ Boost +${t.boostCard.value} appliqué à ${c.name}`,type:"info"});break}}t.boostUsed=!0,i.closeModal(),Xe(e,t,i)})})}function Qt(e,t,i,s,l){const d=document.createElement("div");d.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:900;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;overflow:hidden;cursor:pointer";const a=Array.from({length:10},(o,r)=>`
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(d);let c=!1;const g=()=>{c||(c=!0,d.remove(),setTimeout(()=>l(),50))};d.addEventListener("click",g),setTimeout(g,3500)}async function Bt(e,t,i){var r,m;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:s}=i,l=t.homeScore>t.aiScore,d=t.homeScore===t.aiScore,a=l?"victoire":d?"nul":"defaite",c=Uo(t.mode,a);t.matchId&&await E.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:l?s.profile.id:null,home_credits_reward:c,played_at:new Date().toISOString()}).eq("id",t.matchId);const g={credits:(s.profile.credits||0)+c,matches_played:(s.profile.matches_played||0)+1};l?g.wins=(s.profile.wins||0)+1:d?g.draws=(s.profile.draws||0)+1:g.losses=(s.profile.losses||0)+1,await E.from("users").update(g).eq("id",s.profile.id),await i.refreshProfile();const o=document.createElement("div");o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",o.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const k=async(H=!0)=>{b=!0,u&&(E.removeChannel(u),u=null),H&&await E.rpc("cancel_matchmaking",{p_user_id:o.profile.id}).catch(()=>{})};(B=document.getElementById("mm-cancel"))==null||B.addEventListener("click",async()=>{try{await k(!0)}catch{}We(e),r("home")});const n=async(H,C,D)=>{if(b)return;b=!0,u&&(E.removeChannel(u),u=null);const j=document.getElementById("mm-status");j&&(j.textContent="Adversaire trouvé !"),await new Promise(W=>setTimeout(W,600)),e.isConnected&&wo(e,t,H,D,a,c)},{data:p,error:I}=await E.rpc("try_matchmake",{p_user_id:o.profile.id,p_deck_id:i});if(I||!(p!=null&&p.success)){m("Erreur de matchmaking","error"),We(e),r("home");return}if(p.matched){n(p.match_id,p.opponent_id,!1);return}const L=document.getElementById("mm-status");L&&(L.textContent="En attente d'un autre joueur..."),u=E.channel("matchmaking-"+o.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${o.profile.id}`},H=>{const C=H.new;C.status==="matched"&&C.match_id&&n(C.match_id,C.matched_with,!0)}).subscribe()}async function Pn(e,t,i){const{state:s,navigate:l,toast:d}=t;try{(E.getChannels?E.getChannels():[]).forEach(o=>{const r=o.topic||"";(r.includes("matchmaking")||r.includes("pvp-match")||r.includes("friend-invite"))&&E.removeChannel(o)})}catch{}const{data:a}=await E.from("matches").select("home_id, away_id, status").eq("id",i).single();if(!a){d("Match introuvable","error"),l("home");return}if(a.status==="finished"){d("Ce match est terminé","info"),l("home");return}const c=a.home_id===s.profile.id;wo(e,t,i,c,[],[])}async function wo(e,t,i,s,l=[],d=[]){const{state:a,navigate:c,toast:g}=t,o=s?"p1":"p2",r=s?"p2":"p1",m=(l||[]).map(M=>M.id),b=(l||[]).map(M=>({id:M.id,gc_type:M.gc_type,_gcDef:M._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:u}=await E.from("matches").select("*").eq("id",i).single();if(!u){g("Match introuvable","error"),c("home");return}async function k(){const[{data:M},{data:F},{data:w},{data:T}]=await Promise.all([E.rpc("get_deck_for_match",{p_deck_id:u.home_deck_id}),E.rpc("get_deck_for_match",{p_deck_id:u.away_deck_id}),E.from("users").select("id,pseudo,club_name").eq("id",u.home_id).single(),E.from("users").select("id,pseudo,club_name").eq("id",u.away_id).single()]),f=A=>{const S=Number(A.evolution_bonus)||0;return{cardId:A.card_id,position:A.position,id:A.id,firstname:A.firstname,name:A.surname_encoded,country_code:A.country_code,club_id:A.club_id,job:A.job,job2:A.job2,note_g:(Number(A.note_g)||0)+(A.job==="GK"||A.job2==="GK"&&Number(A.note_g)>0?S:0),note_d:(Number(A.note_d)||0)+(A.job==="DEF"||A.job2==="DEF"&&Number(A.note_d)>0?S:0),note_m:(Number(A.note_m)||0)+(A.job==="MIL"||A.job2==="MIL"&&Number(A.note_m)>0?S:0),note_a:(Number(A.note_a)||0)+(A.job==="ATT"||A.job2==="ATT"&&Number(A.note_a)>0?S:0),evolution_bonus:S,rarity:A.rarity,skin:A.skin,hair:A.hair,hair_length:A.hair_length,clubName:A.club_encoded_name||null,clubLogo:A.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},x=((M==null?void 0:M.starters)||[]).map(A=>f(A)),y=((F==null?void 0:F.starters)||[]).map(A=>f(A)),h=(M==null?void 0:M.formation)||"4-4-2",v=(F==null?void 0:F.formation)||"4-4-2";let _=yt(x,h),z=yt(y,v);const $=((M==null?void 0:M.subs)||[]).map(A=>f(A));return stadiumDef&&s&&(_=Ht(_,stadiumDef),oi($,stadiumDef)),{p1Team:_,p2Team:z,p1Subs:$,p2Subs:((F==null?void 0:F.subs)||[]).map(A=>f(A)),p1Formation:h,p2Formation:v,p1Name:(w==null?void 0:w.club_name)||(w==null?void 0:w.pseudo)||"Joueur 1",p2Name:(T==null?void 0:T.club_name)||(T==null?void 0:T.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:s?m:[],gc_p2:s?[]:m,gcCardsFull_p1:s?b:[],gcCardsFull_p2:s?[]:b,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:d||[],lastActionAt:new Date().toISOString()}}let n=u.game_state&&Object.keys(u.game_state).length?u.game_state:null;if(!n)if(s){n=await k();const{data:M}=await E.from("matches").select("game_state").eq("id",i).single();!(M!=null&&M.game_state)||!Object.keys(M.game_state).length?await E.from("matches").update({game_state:n,turn_user_id:u.home_id}).eq("id",i):n=M.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let M=0;M<30&&!n;M++){await new Promise(w=>setTimeout(w,400));const{data:F}=await E.from("matches").select("game_state").eq("id",i).single();F!=null&&F.game_state&&Object.keys(F.game_state).length&&(n=F.game_state)}if(!n){g("Erreur de synchronisation","error"),c("home");return}n.gc_p2=m,n.gcCardsFull_p2=b,await E.from("matches").update({game_state:n}).eq("id",i)}let p=!1,I=null,L=!1;const B=new Set,H=new Set;function C(M){var _,z;try{E.removeChannel(D)}catch{}const F=n[o+"Score"]||0,w=n[r+"Score"]||0;let T,f;M.winner_id?(T=M.winner_id===a.profile.id,f=!1):M.forfeit?(T=F>w,f=!1):(f=F===w,T=F>w),(_=document.getElementById("pvp-end-overlay"))==null||_.remove();const x=document.createElement("div");x.id="pvp-end-overlay",x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const y=f?"🤝":T?"🏆":"😞",h=f?"MATCH NUL":T?"VICTOIRE !":"DÉFAITE",v=f?"#fff":T?"#FFD700":"#ff6b6b";x.innerHTML=`
      <div style="font-size:64px">${y}</div>
      <div style="font-size:26px;font-weight:900;color:${v}">${h}</div>
      <div style="font-size:18px">${n[o+"Name"]} ${F} – ${w} ${n[r+"Name"]}</div>
      ${M.forfeit?`<div style="font-size:13px;color:rgba(255,255,255,0.5)">${T?"L'adversaire a quitté":"Perdu par forfait"}</div>`:""}
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(x),(z=x.querySelector("#pvp-end-home"))==null||z.addEventListener("click",()=>{x.remove(),We(e),c("home")})}const D=E.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},M=>{const F=M.new;try{if(F.status==="finished"||F.forfeit){if(p)return;p=!0,I&&(clearInterval(I),I=null),F.game_state&&(n=F.game_state),C(F);return}if(F.game_state){const w=n;n=F.game_state;const T=n._lastGC;if(T&&T.seq&&!H.has(T.seq)&&(H.add(T.seq),T.by!==o)){we(T.type,T.by,()=>N());return}const f=w[o+"Score"]||0,x=w[r+"Score"]||0,y=n[o+"Score"]||0,h=n[r+"Score"]||0,v=y>f,_=h>x;if((v||_)&&!B.has(n.round)){B.add(n.round);const z=[...n.log||[]].reverse().find(A=>A.isGoal),$=((z==null?void 0:z.homePlayers)||[]).map(A=>({name:A.name,note:A.note,portrait:A.portrait,job:A.job}));Le($,y,h,v,()=>N());return}N()}}catch(w){console.error("[PvP] crash:",w)}}).subscribe();async function j(M){Object.assign(n,M),n.lastActionAt=new Date().toISOString();const{error:F}=await E.from("matches").update({game_state:n}).eq("id",i);F&&g("Erreur de synchronisation","error");try{N()}catch(w){console.error("[PvP] renderPvpScreen crash:",w)}}async function W(){if(p)return;p=!0,I&&(clearInterval(I),I=null);const M=s?u.away_id:u.home_id,F=s?"p2":"p1",w=s?"p1":"p2",T={...n,[F+"Score"]:3,[w+"Score"]:0,phase:"finished"};try{await E.from("matches").update({status:"finished",forfeit:!0,winner_id:M,home_score:T.p1Score||0,away_score:T.p2Score||0,game_state:T}).eq("id",i)}catch{}try{E.removeChannel(D)}catch{}setTimeout(()=>{We(e),c("home")},800)}const de={BOOST_STAT:({value:M=1,count:F=1,roles:w=[]},T,f)=>{const x=T[o+"Team"],y=Object.entries(x).filter(([h])=>!w.length||w.includes(h)).flatMap(([h,v])=>v.filter(_=>!_.used).map(_=>({..._,_line:h})));if(!y.length){T.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),f(T);return}re(y,F,`Choisir ${F} joueur(s) à booster (+${M})`,h=>{h.forEach(v=>{const _=(x[v._line]||[]).find(z=>z.cardId===v.cardId);_&&(_.boost=(_.boost||0)+M,T.log.push({text:`⚡ +${M} sur ${_.name}`,type:"info"}))}),T[o+"Team"]=x,f(T)})},DEBUFF_STAT:({value:M=1,count:F=1,roles:w=[],target:T="ai"},f,x)=>{const y=T==="home"?o:r,h=f[y+"Team"],v=T==="home"?"allié":"adverse",_=Object.entries(h).filter(([z])=>!w.length||w.includes(z)).flatMap(([z,$])=>$.map(A=>({...A,_line:z})));if(!_.length){f.log.push({text:`🎯 Aucun joueur ${v}`,type:"info"}),x(f);return}re(_,F,`Choisir ${F} joueur(s) ${v}(s) (-${M})`,z=>{z.forEach($=>{const A=(h[$._line]||[]).find(S=>S.cardId===$.cardId);A&&(A.boost=(A.boost||0)-M,f.log.push({text:`🎯 -${M} sur ${A.name}`,type:"info"}))}),f[y+"Team"]=h,x(f)})},GRAY_PLAYER:({count:M=1,roles:F=[],target:w="ai"},T,f)=>{const x=w==="home"?o:r,y=T[x+"Team"],h=w==="home"?"allié":"adverse",v=Object.entries(y).filter(([_])=>!F.length||F.includes(_)).flatMap(([_,z])=>z.filter($=>!$.used).map($=>({...$,_line:_})));if(!v.length){T.log.push({text:`❌ Aucun joueur ${h}`,type:"info"}),f(T);return}re(v,M,`Choisir ${M} joueur(s) ${h}(s) à exclure`,_=>{const z="usedCardIds_"+x,$=new Set(T[z]||[]);_.forEach(A=>{const S=(y[A._line]||[]).find(q=>q.cardId===A.cardId);S&&(S.used=!0,$.add(A.cardId),T.log.push({text:`❌ ${S.name} exclu !`,type:"info"}))}),T[z]=[...$],T[x+"Team"]=y,f(T)})},REVIVE_PLAYER:({count:M=1,roles:F=[]},w,T)=>{const f=w[o+"Team"],x=Object.entries(f).filter(([y])=>!F.length||F.includes(y)).flatMap(([y,h])=>h.filter(v=>v.used).map(v=>({...v,_line:y})));if(!x.length){w.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),T(w);return}re(x,M,`Choisir ${M} joueur(s) à ressusciter`,y=>{y.forEach(h=>{const v=(f[h._line]||[]).find(_=>_.cardId===h.cardId);v&&(v.used=!1,w.log.push({text:`💫 ${v.name} ressuscité !`,type:"info"}))}),w[o+"Team"]=f,T(w)})},REMOVE_GOAL:({},M,F)=>{const w=r+"Score";M[w]>0?(M[w]--,M.log.push({text:"🚫 Dernier but annulé !",type:"info"})):M.log.push({text:"🚫 Aucun but à annuler",type:"info"}),F(M)},ADD_GOAL_DRAW:({},M,F)=>{M[o+"Score"]===M[r+"Score"]?(M[o+"Score"]++,M.log.push({text:"🎯 But bonus !",type:"info"})):M.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),F(M)},ADD_SUB:({value:M=1},F,w)=>{F.maxSubs=(F.maxSubs||3)+M,F.log.push({text:`🔄 +${M} remplacement(s)`,type:"info"}),w(F)},CUSTOM:({},M,F)=>F(M)};function re(M,F,w,T){const f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let x=[];function y(){var v,_;const h=M.map(z=>{const $={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[z._line]||"#555",A=je(z,z._line)+(z.boost||0),q=x.find(R=>R.cardId===z.cardId)?"#FFD700":"rgba(255,255,255,0.25)",G=z.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${z.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${q};background:${$};overflow:hidden;cursor:pointer;${G}">
        ...
        <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
          ...
          <span style="font-size:12px;color:rgba(255,255,255,0.5)">${x.length}/${F}</span>
        ...
        <div style="padding:12px;background:rgba(0,0,0,0.4);flex-shrink:0">
          <button id="pp-confirm" ${x.length===0?'disabled style="opacity:0.4"':""} style="width:100%;padding:13px;border-radius:10px;border:none;background:#7a28b8;color:#fff;font-size:15px;font-weight:900;cursor:pointer">
            ✅ Confirmer (${x.length}/${F})
          </button>
        </div>`,(v=f.querySelector("#pp-close"))==null||v.addEventListener("click",()=>f.remove()),f.querySelectorAll(".pp-item").forEach(z=>{z.addEventListener("click",()=>{const $=z.dataset.cid,A=M.find(q=>q.cardId===$),S=x.findIndex(q=>q.cardId===$);A&&(S>-1?x.splice(S,1):x.length<F&&x.push(A),y())})}),(_=f.querySelector("#pp-confirm"))==null||_.addEventListener("click",()=>{f.remove(),T(x)})}y(),document.body.appendChild(f)}async function P(M,F){const T=(n["gcCardsFull_"+o]||[]).find(h=>h.id===M),f=(T==null?void 0:T._gcDef)||(n.gcDefs||[]).find(h=>{var v;return h.name===F||((v=h.name)==null?void 0:v.toLowerCase().trim())===(F==null?void 0:F.toLowerCase().trim())}),x=[...n["usedGc_"+o]||[],M],y={type:F,by:o,seq:(n._gcAnimSeq||0)+1};H.add(y.seq),we(F,o,async()=>{if(f!=null&&f.effect_type&&f.effect_type!=="CUSTOM"){const v=de[f.effect_type];if(v){const _={...n};v(f.effect_params||{},_,async z=>{z["usedGc_"+o]=x,z._lastGC=y,z._gcAnimSeq=y.seq,await j(z)});return}}const h={...n};switch(F){case"Remplacement+":h.maxSubs=(h.maxSubs||3)+1,h.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const v=r+"Score";h[v]>0&&(h[v]--,h.log.push({text:"🚫 But annulé",type:"info"}));break}}h["usedGc_"+o]=x,h._lastGC=y,h._gcAnimSeq=y.seq,await j(h)})}function X(M,F){const w="usedCardIds_"+M,T=new Set(n[w]||[]);F.forEach(f=>T.add(f)),n[w]=[...T]}function U(){for(const M of["p1","p2"]){const F=new Set(n["usedCardIds_"+M]||[]),w=n[M+"Team"];if(w)for(const T of["GK","DEF","MIL","ATT"])(w[T]||[]).forEach(f=>{f.used=F.has(f.cardId)})}}function N(){var Ue,Je,Ze,Qe,nt,gt;if(n.phase==="reveal")return ne();if(n.phase==="midfield")return ye();if(n.phase==="finished")return ze();const M=n[o+"Team"],F=n[r+"Team"];U();const w=n[o+"Score"],T=n[r+"Score"],f=n[o+"Name"],x=n[r+"Name"],y=n.phase===o+"-attack",h=n.phase===o+"-defense",v=Array.isArray(n["selected_"+o])?n["selected_"+o]:[],_=v.map(le=>le.cardId),z=window.innerWidth>=700,$=n[o+"Subs"]||[],A=n["usedSubIds_"+o]||[],S=$.filter(le=>!A.includes(le.cardId)),q=n["gcCardsFull_"+o]||[],G=n["usedGc_"+o]||[],R=q.filter(le=>!G.includes(le.id)),O=n.boostOwner===o&&!n.boostUsed,Q=!["GK","DEF","MIL","ATT"].some(le=>(F[le]||[]).some(Ee=>!Ee.used)),V=[...M.MIL||[],...M.ATT||[]].filter(le=>!le.used),ie=y&&Q&&V.length===0?[...M.GK||[],...M.DEF||[]].filter(le=>!le.used).map(le=>le.cardId):[];function ae(le,Ee,te){var pt,ht;const ue=le._gcDef,ee={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ue==null?void 0:ue.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",ce={purple:"#b06ce0",light_blue:"#00d4ef"}[ue==null?void 0:ue.color]||"#b06ce0",Be=(ue==null?void 0:ue.name)||le.gc_type,Te=(ue==null?void 0:ue.effect)||((pt=Ve[le.gc_type])==null?void 0:pt.desc)||"",tt=ue!=null&&ue.image_url?`/icons/${ue.image_url}`:null,it=((ht=Ve[le.gc_type])==null?void 0:ht.icon)||"⚡",rt=Math.round(te*.22),st=Math.round(te*.22),ct=te-rt-st,kt=Be.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${le.id}" data-gc-type="${le.gc_type}"
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${ft(M,n[o+"Formation"],Fe,_,300,300,ie)}
      </div>
        ...
        </div>`}return`<div style="font-size:11px;color:${le.type==="goal"?"#FFD700":"rgba(255,255,255,0.65)"};font-weight:${le.type==="goal"?700:400};padding:3px 2px">${le.text||""}</div>`}const Ke=(()=>{var Ee,te;if(h&&((Ee=n.pendingAttack)!=null&&Ee.players)){const ue=n.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
      ...
      <div style="display:flex;align-items:center;padding:8px 10px;background:rgba(0,0,0,0.5);gap:6px;flex-shrink:0">
        ...
        <div style="flex:1;display:flex;align-items:center;justify-content:center;gap:8px">
          ...
          <span style="font-size:26px;font-weight:900;color:#FFD700;letter-spacing:2px">${w} – ${T}</span>
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="width:min(90vw,420px)">${ut(n[r+"Team"],n[r+"Formation"],null,[],300,300)}</div>
    </div>`,o==="p1"&&setTimeout(async()=>{await j({phase:"midfield"})},5e3)}let pe=!1;function ye(){if(pe)return;const M=n[o+"Team"].MIL||[],F=n[r+"Team"].MIL||[];function w(R){return R.reduce((O,Q)=>O+je(Q,"MIL"),0)}function T(R){let O=0;for(let Q=0;Q<R.length-1;Q++){const V=at(R[Q],R[Q+1]);V==="#00ff88"?O+=2:V==="#FFD700"&&(O+=1)}return O}const f=w(M)+T(M),x=w(F)+T(F),y=f>=x;function h(R,O,Q){return`<div id="duel-row-${Q}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
        ...
        <div class="duel-score-line duel-score-line-${Q}" style="margin-top:10px;font-size:12px;color:rgba(255,255,255,0.55);opacity:0;transition:opacity .4s ease">
          Score: ${w(R)} + ${T(R)} liens = <b style="color:#fff">${w(R)+T(R)}</b>
        </div>
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(x);let v=!1;const _=()=>{v||(v=!0,x.remove(),setTimeout(()=>f(),50))};x.addEventListener("click",_),setTimeout(_,3500)}function we(M,F,w){var G,R;const T=(n.gcDefs||[]).find(O=>{var Q;return O.name===M||((Q=O.name)==null?void 0:Q.toLowerCase().trim())===(M==null?void 0:M.toLowerCase().trim())}),f={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[T==null?void 0:T.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",x={purple:"#b06ce0",light_blue:"#00d4ef"}[T==null?void 0:T.color]||"#b06ce0",y=(T==null?void 0:T.name)||M,h=(T==null?void 0:T.effect)||((G=Ve[M])==null?void 0:G.desc)||"",v=T!=null&&T.image_url?`/icons/${T.image_url}`:null,_=((R=Ve[M])==null?void 0:R.icon)||"⚡",$=F===o?"Vous":n[F+"Name"]||"Adversaire",A=document.createElement("div");A.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",A.innerHTML=`
      <style>
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(A);let S=!1;const q=()=>{S||(S=!0,A.remove(),setTimeout(()=>w&&w(),50))};A.addEventListener("click",q),setTimeout(q,3e3)}function ve(M,F){var A,S,q,G;const T=(n["gcCardsFull_"+o]||[]).find(R=>R.id===M),f=T==null?void 0:T._gcDef,x={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[f==null?void 0:f.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",y={purple:"#b06ce0",light_blue:"#00d4ef"}[f==null?void 0:f.color]||"#b06ce0",h=(f==null?void 0:f.name)||F,v=(f==null?void 0:f.effect)||((A=Ve[F])==null?void 0:A.desc)||"Carte spéciale.",_=f!=null&&f.image_url?`/icons/${f.image_url}`:null,z=((S=Ve[F])==null?void 0:S.icon)||"⚡",$=document.createElement("div");$.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",$.innerHTML=`
      ...
      </div>`,document.body.appendChild($),(q=$.querySelector("#pvp-gc-back"))==null||q.addEventListener("click",()=>$.remove()),(G=$.querySelector("#pvp-gc-use"))==null||G.addEventListener("click",()=>{$.remove(),P(M,F)})}function _e(){var T;const M=n[o+"Team"],F=Object.entries(M).flatMap(([f,x])=>(x||[]).filter(y=>!y.used).map(y=>({...y,_line:f})));if(!F.length)return;const w=document.createElement("div");w.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",w.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
        <div style="flex:1;font-size:14px;font-weight:700;color:#fff">⚡ Choisir un joueur pour +${n.boostValue}</div>
        <button id="bp-close" style="background:none;border:none;color:rgba(255,255,255,0.5);font-size:22px;cursor:pointer">✕</button>
      </div>
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${F.map(f=>{const x={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[f._line]||"#555",y=je(f,f._line)+(f.boost||0);return`<div class="bp-item" data-cid="${f.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${x};overflow:hidden;cursor:pointer">
      ...
      </div>`,document.body.appendChild(w),(T=w.querySelector("#bp-close"))==null||T.addEventListener("click",()=>w.remove()),w.querySelectorAll(".bp-item").forEach(f=>{f.addEventListener("click",async()=>{const x=f.dataset.cid,y=F.find(v=>v.cardId===x);if(!y)return;const h=(M[y._line]||[]).find(v=>v.cardId===x);h&&(h.boost=(h.boost||0)+n.boostValue),w.remove(),await j({[o+"Team"]:M,boostUsed:!0})})})}function Ae(M=null){var S,q;if(!(n.phase===o+"-attack")){g("Remplacement uniquement avant votre attaque","warning");return}const w=n[o+"Team"],T=n["usedSubIds_"+o]||[],f=n.maxSubs||3;if(T.length>=f){g(`Maximum ${f} remplacements atteint`,"warning");return}const x=Object.entries(w).flatMap(([G,R])=>(R||[]).filter(O=>O.used).map(O=>({...O,_line:G}))),y=(n[o+"Subs"]||[]).filter(G=>!T.includes(G.cardId));if(!x.length){g("Aucun joueur utilisé à remplacer","warning");return}if(!y.length){g("Aucun remplaçant disponible","warning");return}let h=Math.max(0,x.findIndex(G=>G.cardId===M));const v=((S=x[h])==null?void 0:S._line)||((q=x[h])==null?void 0:q.job);let _=Math.max(0,y.findIndex(G=>G.job===v)),z=!1;const $=document.createElement("div");$.id="pvp-sub-overlay",$.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function A(){var ae,fe,me,$e,ke,qe;const G=x[h],R=y[_],O=Math.min(130,Math.round((window.innerWidth-90)/2)),Q=Math.round(O*1.35),V=Ce=>`background:rgba(255,255,255,0.12);border:none;color:${Ce?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Ce?"default":"pointer"};flex-shrink:0`;$.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:6px">Appuyer pour continuer</div>`,document.body.appendChild(f);let y=!1;const h=()=>{y||(y=!0,f.remove(),setTimeout(()=>w(),50))};f.addEventListener("click",h),setTimeout(h,2200)}function xe(){var F;const M=document.createElement("div");M.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:20px;overflow-y:auto",M.innerHTML=`
      <div style="font-size:12px;color:rgba(255,255,255,0.5);letter-spacing:2px;text-transform:uppercase">Équipe adverse</div>
      <div style="font-size:18px;font-weight:900;color:#ff6b6b">${n[r+"Name"]}</div>
      <div style="width:min(90vw,420px)">${ut(n[r+"Team"],n[r+"Formation"],null,[],300,300)}</div>
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(M),(F=M.querySelector("#pvp-opp-close"))==null||F.addEventListener("click",()=>M.remove())}function J(){var w;const M=n.log||[],F=document.createElement("div");F.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column",F.innerHTML=`
      ...
      </div>`,document.body.appendChild(F),(w=F.querySelector("#pvp-hist-close"))==null||w.addEventListener("click",()=>F.remove())}async function oe(){if(n.phase!==o+"-attack")return;const M=o==="p1"?"p2":"p1",F=(n.round||0)+1,w=[...n.log||[]];w.push({type:"info",text:`⏭️ ${n[o+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const T=Se(n),f=he(M),x=T||!f?"finished":M+"-attack";await j({["selected_"+o]:[],modifiers:{...n.modifiers,[o]:{}},pendingAttack:null,phase:x,attacker:M,round:F,log:w}),x==="finished"&&await be(n)}async function ge(){const M=n[o+"Team"],F=!["GK","DEF","MIL","ATT"].some(x=>(n[r+"Team"][x]||[]).some(y=>!y.used)),w=(n["selected_"+o]||[]).map(x=>{const y=(M[x._role]||[]).find(A=>A.cardId===x.cardId)||x,h=F&&["GK","DEF"].includes(x._role),v=M[x._role]||[],_=v.findIndex(A=>A.cardId===x.cardId),z=lt(v.length),$=_>=0?z[_]:y._col??1;return{...y,_line:x._role,_col:$,...h?{note_a:Math.max(1,Number(y.note_a)||0)}:{}}});if(!w.length)return;const T=Nt(w,n.modifiers[o]||{});X(o,w.map(x=>x.cardId)),w.forEach(x=>{const y=(M[x._role]||[]).find(h=>h.cardId===x.cardId);y&&(y.used=!0)}),n["selected_"+o]=[],N();const f=[...n.log||[]];if(F){const x=(n[o+"Score"]||0)+1,y=w.map($=>({name:$.name,note:je($,$._line||"ATT"),portrait:Pe($),job:$.job}));f.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:y,homeTotal:T.total,aiTotal:0});const h=(n.round||0)+1,v=o==="p1"?"p2":"p1",_={...n,[o+"Team"]:M,[o+"Score"]:x},z=Se(_);B.add(h),Le(y,x,n[r+"Score"]||0,!0,async()=>{await j({[o+"Team"]:M,[o+"Score"]:x,["selected_"+o]:[],modifiers:{...n.modifiers,[o]:{}},pendingAttack:null,phase:z?"finished":v+"-attack",attacker:v,round:h,log:f}),z&&await be({...n,[o+"Score"]:x})});return}f.push({type:"pending",text:`⚔️ ${n[o+"Name"]} attaque (${T.total})`}),await j({[o+"Team"]:M,[r+"Team"]:n[r+"Team"],pendingAttack:{...T,players:w,side:o},["selected_"+o]:[],modifiers:{...n.modifiers,[o]:{}},phase:r+"-defense",log:f})}async function Y(){const M=n[o+"Team"],F=(n["selected_"+o]||[]).map(G=>{const R=(M[G._role]||[]).find(ae=>ae.cardId===G.cardId)||G,O=M[G._role]||[],Q=O.findIndex(ae=>ae.cardId===G.cardId),V=lt(O.length),ie=Q>=0?V[Q]:R._col??1;return{...R,_line:G._role,_col:ie}}),w=Rt(F,n.modifiers[o]||{});X(o,F.map(G=>G.cardId)),F.forEach(G=>{const R=(M[G._role]||[]).find(O=>O.cardId===G.cardId);R&&(R.used=!0)}),n["selected_"+o]=[],N();const T=Ot(n.pendingAttack.total,w.total,n.modifiers[o]||{}),f=n.pendingAttack.side,x=T==="attack"||(T==null?void 0:T.goal),y=f==="p1"?"p2":"p1",h=(n.round||0)+1,v=(n.pendingAttack.players||[]).map(G=>({name:G.name,note:je(G,G._line||"ATT"),portrait:Pe(G),job:G.job})),_=[...n.log||[]];_.push({type:"duel",isGoal:x,homeScored:x&&f===o,text:x?`⚽ BUT de ${n[f+"Name"]} ! (${n.pendingAttack.total} vs ${w.total})`:`✋ Attaque stoppée (${n.pendingAttack.total} vs ${w.total})`,homePlayers:v,aiPlayers:F.map(G=>({name:G.name,note:je(G,G._line||"DEF"),portrait:Pe(G),job:G.job})),homeTotal:n.pendingAttack.total,aiTotal:w.total});const z=x?(n[f+"Score"]||0)+1:n[f+"Score"]||0,$={...n,[o+"Team"]:M,[f+"Score"]:z},A=Se($),S=A?"finished":y+"-attack",q=async()=>{await j({[o+"Team"]:M,[r+"Team"]:n[r+"Team"],[f+"Score"]:z,["selected_"+o]:[],modifiers:{...n.modifiers,[o]:{}},pendingAttack:null,phase:S,attacker:y,round:h,log:_}),(S==="finished"||A)&&await be({...n,[f+"Score"]:z})};if(x){const G=f===o,R=G?z:n[o+"Score"]||0,O=G?n[r+"Score"]||0:z;B.add(h),Le(v,R,O,G,q)}else await q()}function Z(M){return["MIL","ATT"].some(F=>(M[F]||[]).some(w=>!w.used))}function K(M){return["GK","DEF","MIL","ATT"].some(F=>(M[F]||[]).some(w=>!w.used))}function se(M){return K(M)&&!Z(M)}function he(M){const F=n[M+"Team"],w=n[(M==="p1"?"p2":"p1")+"Team"];return!!(Z(F)||!K(w)&&se(F))}function Se(M){const F=["MIL","ATT"].some(_=>(M.p1Team[_]||[]).some(z=>!z.used)),w=["MIL","ATT"].some(_=>(M.p2Team[_]||[]).some(z=>!z.used)),T=K(M.p1Team),f=K(M.p2Team);return!F&&!(!f&&T)&&(!w&&!(!T&&f))}function Me(M){const F=M.p1Score||0,w=M.p2Score||0;return F===w?null:F>w?u.home_id:u.away_id}async function be(M){try{const F=Me(M),w=F?u.home_id===F?u.away_id:u.home_id:null;await E.from("matches").update({status:"finished",winner_id:F,home_score:M.p1Score||0,away_score:M.p2Score||0}).eq("id",i),F&&w&&updateEvolutiveCards(F,w).catch(()=>{})}catch(F){console.error("[PvP] finishMatch:",F)}}function ze(){var f;const M=n[o+"Score"],F=n[r+"Score"],w=M>F,T=M===F;et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      <div style="font-size:64px">${w?"🏆":T?"🤝":"😤"}</div>
      <div style="font-size:24px;font-weight:900;color:#fff">${w?"Victoire !":T?"Match nul":"Défaite"}</div>
      <div style="font-size:32px;font-weight:900;color:#FFD700">${M} - ${F}</div>
      <button id="pvp-home" style="padding:16px 40px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">🏠 Retour</button>
    </div>`,(f=document.getElementById("pvp-home"))==null||f.addEventListener("click",()=>{try{E.removeChannel(D)}catch{}We(e),c("home")})}N()}const Nn=Object.freeze(Object.defineProperty({__proto__:null,renderMatchRandom:vo,resumePvpMatch:Pn},Symbol.toStringTag,{value:"Module"}));async function Rn(e,t,i,s){var l,d;try{const a=(d=(l=t==null?void 0:t.state)==null?void 0:l.profile)==null?void 0:d.id;try{(E.getChannels?E.getChannels():[]).forEach(g=>{const o=g.topic||"";(o.includes("matchmaking")||o.includes("pvp-match"))&&E.removeChannel(g)})}catch(c){console.warn("[FriendMatch] cleanup canaux:",c)}a&&(await E.rpc("cancel_matchmaking",{p_user_id:a}).catch(()=>{}),await E.from("matchmaking_queue").delete().eq("user_id",a).then(()=>{},()=>{}))}catch{}await ai(e,t,"random",({deckId:a,formation:c,starters:g,subsRaw:o,gcCardsEnriched:r,gcDefs:m,stadiumDef:b})=>{const u=k=>On(e,t,a,c,g,o,k,m||[],i,s);if(!r.length){u([]);return}ri(e,r,u)})}async function On(e,t,i,s,l,d,a=[],c=[],g,o){var de;const{state:r,navigate:m,toast:b}=t,u=r.profile.id;let k=!1,n=null;et(e);try{await E.rpc("cancel_matchmaking",{p_user_id:u})}catch{}try{await E.from("matchmaking_queue").delete().eq("user_id",u)}catch{}const p=(re,P,X,U)=>{var N;e.innerHTML=`
      ...
      </div>`,(N=document.getElementById("lobby-cancel"))==null||N.addEventListener("click",async()=>{k=!0,n&&(E.removeChannel(n),n=null),X&&await E.from("friend_match_invites").update({status:"declined"}).eq("id",X),We(e),m("home")})},I=async(re,P)=>{k=!0,n&&(E.removeChannel(n),n=null),await new Promise(X=>setTimeout(X,600)),e.isConnected&&Hn(e,t,re,P,a,c)},{data:L}=await E.from("friend_match_invites").select("*").eq("status","pending").or(`and(inviter_id.eq.${u},invitee_id.eq.${g}),and(inviter_id.eq.${g},invitee_id.eq.${u})`).order("created_at",{ascending:!1}).limit(1).maybeSingle();let B,H;if(L&&L.inviter_id===g)H=!1,B=L.id,await E.from("friend_match_invites").update({invitee_deck_id:i,invitee_ready:!0,status:"accepted"}).eq("id",B);else{H=!0;const{data:re,error:P}=await E.from("friend_match_invites").insert({inviter_id:u,invitee_id:g,friend_id:g,inviter_deck_id:i,inviter_ready:!0,status:"pending"}).select().single();if(P||!re){b("Erreur création invitation","error"),We(e),m("home");return}B=re.id}if(p(!0,!H,B),!H){const re={home_id:g,away_id:u,home_deck_id:L.inviter_deck_id,away_deck_id:i,status:"active",mode:"friend"},P=await E.from("matches").insert(re).select().single(),X=P.data;if(!X){b("Erreur création match: "+(((de=P.error)==null?void 0:de.message)||""),"error"),We(e),m("home");return}await E.from("friend_match_invites").update({match_id:X.id,status:"matched"}).eq("id",B),I(X.id,!1);return}let C=!1;const D=re=>{k||C||(re.status==="matched"&&re.match_id?(C=!0,clearInterval(j),clearInterval(W),I(re.match_id,!0)):re.status==="declined"?(clearInterval(j),clearInterval(W),b(`${o} a décliné l'invitation`,"warning"),We(e),m("home")):re.invitee_ready&&p(!0,!0,B))},j=setInterval(()=>{if(k){clearInterval(j);return}E.from("matchmaking_queue").delete().eq("user_id",u).then(()=>{},()=>{})},3e3),W=setInterval(async()=>{if(k||C){clearInterval(W);return}const{data:re}=await E.from("friend_match_invites").select("*").eq("id",B).maybeSingle();re&&D(re)},1200);n=E.channel("friend-invite-"+B).on("postgres_changes",{event:"UPDATE",schema:"public",table:"friend_match_invites",filter:`id=eq.${B}`},re=>D(re.new)).subscribe()}async function Hn(e,t,i,s,l=[],d=[]){const{state:a,navigate:c,toast:g}=t,o=s?"p1":"p2",r=s?"p2":"p1",m=(l||[]).map(w=>w.id),b=(l||[]).map(w=>({id:w.id,gc_type:w.gc_type,_gcDef:w._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:u}=await E.from("matches").select("*").eq("id",i).single();if(!u){g("Match introuvable","error"),c("home");return}async function k(){const[{data:w},{data:T},{data:f},{data:x}]=await Promise.all([E.rpc("get_deck_for_match",{p_deck_id:u.home_deck_id}),E.rpc("get_deck_for_match",{p_deck_id:u.away_deck_id}),E.from("users").select("id,pseudo,club_name").eq("id",u.home_id).single(),E.from("users").select("id,pseudo,club_name").eq("id",u.away_id).single()]),y=$=>{const A=Number($.evolution_bonus)||0;return{cardId:$.card_id,position:$.position,id:$.id,firstname:$.firstname,name:$.surname_encoded,country_code:$.country_code,club_id:$.club_id,job:$.job,job2:$.job2,note_g:(Number($.note_g)||0)+($.job==="GK"||$.job2==="GK"&&Number($.note_g)>0?A:0),note_d:(Number($.note_d)||0)+($.job==="DEF"||$.job2==="DEF"&&Number($.note_d)>0?A:0),note_m:(Number($.note_m)||0)+($.job==="MIL"||$.job2==="MIL"&&Number($.note_m)>0?A:0),note_a:(Number($.note_a)||0)+($.job==="ATT"||$.job2==="ATT"&&Number($.note_a)>0?A:0),evolution_bonus:A,rarity:$.rarity,skin:$.skin,hair:$.hair,hair_length:$.hair_length,clubName:$.club_encoded_name||null,clubLogo:$.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},h=((w==null?void 0:w.starters)||[]).map($=>y($)),v=((T==null?void 0:T.starters)||[]).map($=>y($)),_=(w==null?void 0:w.formation)||"4-4-2",z=(T==null?void 0:T.formation)||"4-4-2";return{p1Team:(()=>{const $=yt(h,_);return stadiumDef?Ht($,stadiumDef):$})(),p2Team:yt(v,z),p1Subs:(()=>{const $=((w==null?void 0:w.subs)||[]).map(A=>y(A));return stadiumDef&&oi($,stadiumDef),$})(),p2Subs:((T==null?void 0:T.subs)||[]).map($=>y($)),p1Formation:_,p2Formation:z,p1Name:(f==null?void 0:f.club_name)||(f==null?void 0:f.pseudo)||"Joueur 1",p2Name:(x==null?void 0:x.club_name)||(x==null?void 0:x.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:s?m:[],gc_p2:s?[]:m,gcCardsFull_p1:s?b:[],gcCardsFull_p2:s?[]:b,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:d||[],lastActionAt:new Date().toISOString()}}let n=u.game_state&&Object.keys(u.game_state).length?u.game_state:null;if(!n)if(s){n=await k();const{data:w}=await E.from("matches").select("game_state").eq("id",i).single();!(w!=null&&w.game_state)||!Object.keys(w.game_state).length?await E.from("matches").update({game_state:n,turn_user_id:u.home_id}).eq("id",i):n=w.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let w=0;w<60&&!n;w++){await new Promise(f=>setTimeout(f,400));const{data:T}=await E.from("matches").select("game_state").eq("id",i).single();T!=null&&T.game_state&&Object.keys(T.game_state).length&&(n=T.game_state)}if(!n){g("Erreur de synchronisation","error"),c("home");return}n.gc_p2=m,n.gcCardsFull_p2=b,await E.from("matches").update({game_state:n}).eq("id",i)}let p=!1,I=!1,L=!1,B=null,H=!1;const C=new Set,D=new Set;function j(w){var $,A;try{E.removeChannel(W)}catch{}const T=n[o+"Score"]||0,f=n[r+"Score"]||0;if(!L){L=!0;const S=w.winner_id||(T>f?a.profile.id:f>T?"opp":null),q=S===a.profile.id?"win":S?"loss":null;q&&ti(()=>Promise.resolve().then(()=>oo),void 0).then(G=>G.applyOwnEvolution(a.profile.id,q)).catch(()=>{})}if(!I){const S=n.p1Score||0,q=n.p2Score||0,G=(n.usedGc_p1||[]).length,R=(n.usedGc_p2||[]).length,O=w.winner_id||(S>q?u.home_id:q>S?u.away_id:null);(O?a.profile.id===O:a.profile.id<(s?u.away_id:u.home_id))&&(I=!0,Xi({player1Id:u.home_id,player2Id:u.away_id,score1:S,score2:q,gc1:G,gc2:R}).catch(V=>console.warn("[FriendMatch] updateStats:",V)))}let x,y;w.winner_id?(x=w.winner_id===a.profile.id,y=!1):w.forfeit?(x=T>f,y=!1):(y=T===f,x=T>f),($=document.getElementById("pvp-end-overlay"))==null||$.remove();const h=document.createElement("div");h.id="pvp-end-overlay",h.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const v=y?"🤝":x?"🏆":"😞",_=y?"MATCH NUL":x?"VICTOIRE !":"DÉFAITE",z=y?"#fff":x?"#FFD700":"#ff6b6b";h.innerHTML=`
      <div style="font-size:64px">${v}</div>
      <div style="font-size:26px;font-weight:900;color:${z}">${_}</div>
      <div style="font-size:18px">${n[o+"Name"]} ${T} – ${f} ${n[r+"Name"]}</div>
      ${w.forfeit?`<div style="font-size:13px;color:rgba(255,255,255,0.5)">${x?"L'adversaire a quitté":"Perdu par forfait"}</div>`:""}
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(h),(A=h.querySelector("#pvp-end-home"))==null||A.addEventListener("click",()=>{h.remove(),We(e),c("home")})}const W=E.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},w=>{const T=w.new;try{if(T.status==="finished"||T.forfeit){if(p)return;p=!0,B&&(clearInterval(B),B=null),T.game_state&&(n=T.game_state),j(T);return}if(T.game_state){const f=n;n=T.game_state;const x=n._lastGC;if(x&&x.seq&&!D.has(x.seq)&&(D.add(x.seq),x.by!==o)){_e(x.type,x.by,()=>pe());return}const y=f[o+"Score"]||0,h=f[r+"Score"]||0,v=n[o+"Score"]||0,_=n[r+"Score"]||0,z=v>y,$=_>h;if((z||$)&&!C.has(n.round)){C.add(n.round);const A=[...n.log||[]].reverse().find(q=>q.isGoal),S=((A==null?void 0:A.homePlayers)||[]).map(q=>({name:q.name,note:q.note,portrait:q.portrait,job:q.job}));ve(S,v,_,z,()=>pe());return}pe()}}catch(f){console.error("[PvP] crash:",f)}}).subscribe();async function de(w){Object.assign(n,w),n.lastActionAt=new Date().toISOString();const{error:T}=await E.from("matches").update({game_state:n}).eq("id",i);T&&g("Erreur de synchronisation","error");try{pe()}catch(f){console.error("[PvP] renderPvpScreen crash:",f)}}async function re(){if(p)return;p=!0,B&&(clearInterval(B),B=null);const w=s?u.away_id:u.home_id,T=s?"p2":"p1",f=s?"p1":"p2",x={...n,[T+"Score"]:3,[f+"Score"]:0,phase:"finished"};try{await E.from("matches").update({status:"finished",forfeit:!0,winner_id:w,home_score:x.p1Score||0,away_score:x.p2Score||0,game_state:x}).eq("id",i)}catch{}try{E.removeChannel(W)}catch{}setTimeout(()=>{We(e),c("home")},800)}const P={BOOST_STAT:({value:w=1,count:T=1,roles:f=[]},x,y)=>{const h=x[o+"Team"],v=Object.entries(h).filter(([_])=>!f.length||f.includes(_)).flatMap(([_,z])=>z.filter($=>!$.used).map($=>({...$,_line:_})));if(!v.length){x.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),y(x);return}X(v,T,`Choisir ${T} joueur(s) à booster (+${w})`,_=>{_.forEach(z=>{const $=(h[z._line]||[]).find(A=>A.cardId===z.cardId);$&&($.boost=($.boost||0)+w,x.log.push({text:`⚡ +${w} sur ${$.name}`,type:"info"}))}),x[o+"Team"]=h,y(x)})},DEBUFF_STAT:({value:w=1,count:T=1,roles:f=[],target:x="ai"},y,h)=>{const v=x==="home"?o:r,_=y[v+"Team"],z=x==="home"?"allié":"adverse",$=Object.entries(_).filter(([A])=>!f.length||f.includes(A)).flatMap(([A,S])=>S.map(q=>({...q,_line:A})));if(!$.length){y.log.push({text:`🎯 Aucun joueur ${z}`,type:"info"}),h(y);return}X($,T,`Choisir ${T} joueur(s) ${z}(s) (-${w})`,A=>{A.forEach(S=>{const q=(_[S._line]||[]).find(G=>G.cardId===S.cardId);q&&(q.boost=(q.boost||0)-w,y.log.push({text:`🎯 -${w} sur ${q.name}`,type:"info"}))}),y[v+"Team"]=_,h(y)})},GRAY_PLAYER:({count:w=1,roles:T=[],target:f="ai"},x,y)=>{const h=f==="home"?o:r,v=x[h+"Team"],_=f==="home"?"allié":"adverse",z=Object.entries(v).filter(([$])=>!T.length||T.includes($)).flatMap(([$,A])=>A.filter(S=>!S.used).map(S=>({...S,_line:$})));if(!z.length){x.log.push({text:`❌ Aucun joueur ${_}`,type:"info"}),y(x);return}X(z,w,`Choisir ${w} joueur(s) ${_}(s) à exclure`,$=>{const A="usedCardIds_"+h,S=new Set(x[A]||[]);$.forEach(q=>{const G=(v[q._line]||[]).find(R=>R.cardId===q.cardId);G&&(G.used=!0,S.add(q.cardId),x.log.push({text:`❌ ${G.name} exclu !`,type:"info"}))}),x[A]=[...S],x[h+"Team"]=v,y(x)})},REVIVE_PLAYER:({count:w=1,roles:T=[]},f,x)=>{const y=f[o+"Team"],h=Object.entries(y).filter(([v])=>!T.length||T.includes(v)).flatMap(([v,_])=>_.filter(z=>z.used).map(z=>({...z,_line:v})));if(!h.length){f.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),x(f);return}X(h,w,`Choisir ${w} joueur(s) à ressusciter`,v=>{v.forEach(_=>{const z=(y[_._line]||[]).find($=>$.cardId===_.cardId);z&&(z.used=!1,f.log.push({text:`💫 ${z.name} ressuscité !`,type:"info"}))}),f[o+"Team"]=y,x(f)})},REMOVE_GOAL:({},w,T)=>{const f=r+"Score";w[f]>0?(w[f]--,w.log.push({text:"🚫 Dernier but annulé !",type:"info"})):w.log.push({text:"🚫 Aucun but à annuler",type:"info"}),T(w)},ADD_GOAL_DRAW:({},w,T)=>{w[o+"Score"]===w[r+"Score"]?(w[o+"Score"]++,w.log.push({text:"🎯 But bonus !",type:"info"})):w.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),T(w)},ADD_SUB:({value:w=1},T,f)=>{T.maxSubs=(T.maxSubs||3)+w,T.log.push({text:`🔄 +${w} remplacement(s)`,type:"info"}),f(T)},CUSTOM:({},w,T)=>T(w)};function X(w,T,f,x){const y=document.createElement("div");y.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let h=[];function v(){var z,$;const _=w.map(A=>{const S={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[A._line]||"#555",q=je(A,A._line)+(A.boost||0),R=h.find(Q=>Q.cardId===A.cardId)?"#FFD700":"rgba(255,255,255,0.25)",O=A.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${A.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${R};background:${S};overflow:hidden;cursor:pointer;${O}">
        ...
        </div>`,(z=y.querySelector("#pp-close"))==null||z.addEventListener("click",()=>y.remove()),y.querySelectorAll(".pp-item").forEach(A=>{A.addEventListener("click",()=>{const S=A.dataset.cid,q=w.find(R=>R.cardId===S),G=h.findIndex(R=>R.cardId===S);q&&(G>-1?h.splice(G,1):h.length<T&&h.push(q),v())})}),($=y.querySelector("#pp-confirm"))==null||$.addEventListener("click",()=>{y.remove(),x(h)})}v(),document.body.appendChild(y)}async function U(w,T){const x=(n["gcCardsFull_"+o]||[]).find(_=>_.id===w),y=(x==null?void 0:x._gcDef)||(n.gcDefs||[]).find(_=>{var z;return _.name===T||((z=_.name)==null?void 0:z.toLowerCase().trim())===(T==null?void 0:T.toLowerCase().trim())}),h=[...n["usedGc_"+o]||[],w],v={type:T,by:o,seq:(n._gcAnimSeq||0)+1};D.add(v.seq),_e(T,o,async()=>{if(y!=null&&y.effect_type&&y.effect_type!=="CUSTOM"){const z=P[y.effect_type];if(z){const $={...n};z(y.effect_params||{},$,async A=>{A["usedGc_"+o]=h,A._lastGC=v,A._gcAnimSeq=v.seq,await de(A)});return}}const _={...n};switch(T){case"Remplacement+":_.maxSubs=(_.maxSubs||3)+1,_.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const z=r+"Score";_[z]>0&&(_[z]--,_.log.push({text:"🚫 But annulé",type:"info"}));break}}_["usedGc_"+o]=h,_._lastGC=v,_._gcAnimSeq=v.seq,await de(_)})}function N(w,T){const f="usedCardIds_"+w,x=new Set(n[f]||[]);T.forEach(y=>x.add(y)),n[f]=[...x]}function ne(){for(const w of["p1","p2"]){const T=new Set(n["usedCardIds_"+w]||[]),f=n[w+"Team"];if(f)for(const x of["GK","DEF","MIL","ATT"])(f[x]||[]).forEach(y=>{y.used=T.has(y.cardId)})}}function pe(){var Ze,Qe,nt,gt,le,Ee;if(n.phase==="reveal")return ye();if(n.phase==="midfield")return we();if(n.phase==="finished")return F();const w=n[o+"Team"],T=n[r+"Team"];ne();const f=n[o+"Score"],x=n[r+"Score"],y=n[o+"Name"],h=n[r+"Name"],v=n.phase===o+"-attack",_=n.phase===o+"-defense",z=Array.isArray(n["selected_"+o])?n["selected_"+o]:[],$=z.map(te=>te.cardId),A=window.innerWidth>=700,S=n[o+"Subs"]||[],q=n["usedSubIds_"+o]||[],G=S.filter(te=>!q.includes(te.cardId)),R=n["gcCardsFull_"+o]||[],O=n["usedGc_"+o]||[],Q=R.filter(te=>!O.includes(te.id)),V=n.boostOwner===o&&!n.boostUsed,ie=!["GK","DEF","MIL","ATT"].some(te=>(T[te]||[]).some(ue=>!ue.used)),ae=[...w.MIL||[],...w.ATT||[]].filter(te=>!te.used),fe=v&&ie&&ae.length===0?[...w.GK||[],...w.DEF||[]].filter(te=>!te.used).map(te=>te.cardId):[];function me(te,ue,ee){var Et,Ut;const ce=te._gcDef,Be={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ce==null?void 0:ce.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",Te={purple:"#b06ce0",light_blue:"#00d4ef"}[ce==null?void 0:ce.color]||"#b06ce0",tt=(ce==null?void 0:ce.name)||te.gc_type,it=(ce==null?void 0:ce.effect)||((Et=Ve[te.gc_type])==null?void 0:Et.desc)||"",rt=ce!=null&&ce.image_url?`/icons/${ce.image_url}`:null,st=((Ut=Ve[te.gc_type])==null?void 0:Ut.icon)||"⚡",ct=Math.round(ee*.22),kt=Math.round(ee*.22),pt=ee-ct-kt,ht=tt.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${te.id}" data-gc-type="${te.gc_type}"
      ...
      </div>`}const ke=(te,ue)=>ue?$e(130,175):me(te,130,175),qe=(te,ue)=>ue?$e(68,95):me(te,68,95),Ce=A?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",Re=v?Me(o)?`<button id="pvp-action" style="${Ce};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${z.length===0?"disabled":""}>⚔️ ATTAQUEZ <span id="pvp-timer"></span></button>`:`<button id="pvp-action" data-pass="1" style="${Ce};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER <span id="pvp-timer"></span></button>`:_?`<button id="pvp-action" style="${Ce};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${z.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="pvp-timer"></span></button>`:`<div style="font-size:11px;color:rgba(255,255,255,0.3);text-align:center;padding:4px">⏳ Tour de ${h}</div>`,Fe=v&&!Me(o)?'<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">Aucun attaquant — passez la main</div>':v||_?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${z.length}/3 sélectionné(s)</div>`:"",Oe=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${A?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
      ${G.length===0?'<div style="font-size:7px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':G.map(te=>`<div class="pvp-sub-btn" data-sub-id="${te.cardId}" style="cursor:pointer;flex-shrink:0">${Ye(te,A?76:44,A?100:58)}</div>`).join("")}
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ...
        </div>`}const te=(n.log||[]).slice(-1)[0];return te?'<div style="padding:2px 4px">'+Ne(te)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})(),Ue=`
      <div style="display:flex;align-items:center;padding:8px 10px;background:rgba(0,0,0,0.5);gap:6px;flex-shrink:0">
        <button id="pvp-quit" style="width:34px;height:34px;border-radius:50%;background:rgba(220,50,50,0.7);border:none;color:#fff;font-size:16px;cursor:pointer;flex-shrink:0">✕</button>
        <div style="flex:1;display:flex;align-items:center;justify-content:center;gap:8px">
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="width:min(90vw,420px)">${ut(n[r+"Team"],n[r+"Formation"],null,[],300,300)}</div>
        ...
        <div class="duel-score-line duel-score-line-${ie}" style="margin-top:10px;font-size:12px;color:rgba(255,255,255,0.55);opacity:0;transition:opacity .4s ease">
          Score: ${f(Q)} + ${x(Q)} liens = <b style="color:#fff">${f(Q)+x(Q)}</b>
        </div>
      </div>`}et(e),e.innerHTML=`
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      <style>
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(h);let z=!1;const $=()=>{z||(z=!0,h.remove(),setTimeout(()=>y(),50))};h.addEventListener("click",$),setTimeout($,3500)}function _e(w,T,f){var O,Q;const x=(n.gcDefs||[]).find(V=>{var ie;return V.name===w||((ie=V.name)==null?void 0:ie.toLowerCase().trim())===(w==null?void 0:w.toLowerCase().trim())}),y={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[x==null?void 0:x.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",h={purple:"#b06ce0",light_blue:"#00d4ef"}[x==null?void 0:x.color]||"#b06ce0",v=(x==null?void 0:x.name)||w,_=(x==null?void 0:x.effect)||((O=Ve[w])==null?void 0:O.desc)||"",z=x!=null&&x.image_url?`/icons/${x.image_url}`:null,$=((Q=Ve[w])==null?void 0:Q.icon)||"⚡",S=T===o?"Vous":n[T+"Name"]||"Adversaire",q=document.createElement("div");q.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",q.innerHTML=`
      <style>
        @keyframes gcFlipIn{0%{transform:perspective(800px) rotateY(90deg) scale(.7);opacity:0}55%{transform:perspective(800px) rotateY(-12deg) scale(1.08);opacity:1}100%{transform:perspective(800px) rotateY(0) scale(1);opacity:1}}
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(q);let G=!1;const R=()=>{G||(G=!0,q.remove(),setTimeout(()=>f&&f(),50))};q.addEventListener("click",R),setTimeout(R,3e3)}function Ae(w,T){var q,G,R,O;const x=(n["gcCardsFull_"+o]||[]).find(Q=>Q.id===w),y=x==null?void 0:x._gcDef,h={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[y==null?void 0:y.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",v={purple:"#b06ce0",light_blue:"#00d4ef"}[y==null?void 0:y.color]||"#b06ce0",_=(y==null?void 0:y.name)||T,z=(y==null?void 0:y.effect)||((q=Ve[T])==null?void 0:q.desc)||"Carte spéciale.",$=y!=null&&y.image_url?`/icons/${y.image_url}`:null,A=((G=Ve[T])==null?void 0:G.icon)||"⚡",S=document.createElement("div");S.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",S.innerHTML=`
      ...
      <div style="display:flex;gap:12px;width:190px">
        <button id="pvp-gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
        <button id="pvp-gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
      </div>`,document.body.appendChild(S),(R=S.querySelector("#pvp-gc-back"))==null||R.addEventListener("click",()=>S.remove()),(O=S.querySelector("#pvp-gc-use"))==null||O.addEventListener("click",()=>{S.remove(),U(w,T)})}function Ie(){var x;const w=n[o+"Team"],T=Object.entries(w).flatMap(([y,h])=>(h||[]).filter(v=>!v.used).map(v=>({...v,_line:y})));if(!T.length)return;const f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",f.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
        <div style="flex:1;font-size:14px;font-weight:700;color:#fff">⚡ Choisir un joueur pour +${n.boostValue}</div>
        <button id="bp-close" style="background:none;border:none;color:rgba(255,255,255,0.5);font-size:22px;cursor:pointer">✕</button>
      </div>
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${T.map(y=>{const h={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[y._line]||"#555",v=je(y,y._line)+(y.boost||0);return`<div class="bp-item" data-cid="${y.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${h};overflow:hidden;cursor:pointer">
      ...
      </div>`,document.body.appendChild(f),(x=f.querySelector("#bp-close"))==null||x.addEventListener("click",()=>f.remove()),f.querySelectorAll(".bp-item").forEach(y=>{y.addEventListener("click",async()=>{const h=y.dataset.cid,v=T.find(z=>z.cardId===h);if(!v)return;const _=(w[v._line]||[]).find(z=>z.cardId===h);_&&(_.boost=(_.boost||0)+n.boostValue),f.remove(),await de({[o+"Team"]:w,boostUsed:!0})})})}function xe(w=null){var G,R;if(!(n.phase===o+"-attack")){g("Remplacement uniquement avant votre attaque","warning");return}const f=n[o+"Team"],x=n["usedSubIds_"+o]||[],y=n.maxSubs||3;if(x.length>=y){g(`Maximum ${y} remplacements atteint`,"warning");return}const h=Object.entries(f).flatMap(([O,Q])=>(Q||[]).filter(V=>V.used).map(V=>({...V,_line:O}))),v=(n[o+"Subs"]||[]).filter(O=>!x.includes(O.cardId));if(!h.length){g("Aucun joueur utilisé à remplacer","warning");return}if(!v.length){g("Aucun remplaçant disponible","warning");return}let _=Math.max(0,h.findIndex(O=>O.cardId===w));const z=((G=h[_])==null?void 0:G._line)||((R=h[_])==null?void 0:R.job);let $=Math.max(0,v.findIndex(O=>O.job===z)),A=!1;const S=document.createElement("div");S.id="pvp-sub-overlay",S.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function q(){var me,$e,ke,qe,Ce,Re;const O=h[_],Q=v[$],V=Math.min(130,Math.round((window.innerWidth-90)/2)),ie=Math.round(V*1.35),ae=Fe=>`background:rgba(255,255,255,0.12);border:none;color:${Fe?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Fe?"default":"pointer"};flex-shrink:0`;S.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      </div>`,document.body.appendChild(T),(x=T.querySelector("#pvp-hist-close"))==null||x.addEventListener("click",()=>T.remove())}async function Y(){if(n.phase!==o+"-attack")return;const w=o==="p1"?"p2":"p1",T=(n.round||0)+1,f=[...n.log||[]];f.push({type:"info",text:`⏭️ ${n[o+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const x=be(n),y=Me(w),h=x||!y?"finished":w+"-attack";await de({["selected_"+o]:[],modifiers:{...n.modifiers,[o]:{}},pendingAttack:null,phase:h,attacker:w,round:T,log:f}),h==="finished"&&await M(n)}async function Z(){const w=n[o+"Team"],T=!["GK","DEF","MIL","ATT"].some(h=>(n[r+"Team"][h]||[]).some(v=>!v.used)),f=(n["selected_"+o]||[]).map(h=>{const v=(w[h._role]||[]).find(q=>q.cardId===h.cardId)||h,_=T&&["GK","DEF"].includes(h._role),z=w[h._role]||[],$=z.findIndex(q=>q.cardId===h.cardId),A=lt(z.length),S=$>=0?A[$]:v._col??1;return{...v,_line:h._role,_col:S,..._?{note_a:Math.max(1,Number(v.note_a)||0)}:{}}});if(!f.length)return;const x=Nt(f,n.modifiers[o]||{});N(o,f.map(h=>h.cardId)),f.forEach(h=>{const v=(w[h._role]||[]).find(_=>_.cardId===h.cardId);v&&(v.used=!0)}),n["selected_"+o]=[],pe();const y=[...n.log||[]];if(T){const h=(n[o+"Score"]||0)+1,v=f.map(S=>({name:S.name,note:je(S,S._line||"ATT"),portrait:Pe(S),job:S.job}));y.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:v,homeTotal:x.total,aiTotal:0});const _=(n.round||0)+1,z=o==="p1"?"p2":"p1",$={...n,[o+"Team"]:w,[o+"Score"]:h},A=be($);C.add(_),ve(v,h,n[r+"Score"]||0,!0,async()=>{await de({[o+"Team"]:w,[o+"Score"]:h,["selected_"+o]:[],modifiers:{...n.modifiers,[o]:{}},pendingAttack:null,phase:A?"finished":z+"-attack",attacker:z,round:_,log:y}),A&&await M({...n,[o+"Score"]:h})});return}y.push({type:"pending",text:`⚔️ ${n[o+"Name"]} attaque (${x.total})`}),await de({[o+"Team"]:w,[r+"Team"]:n[r+"Team"],pendingAttack:{...x,players:f,side:o},["selected_"+o]:[],modifiers:{...n.modifiers,[o]:{}},phase:r+"-defense",log:y})}async function K(){const w=n[o+"Team"],T=(n["selected_"+o]||[]).map(O=>{const Q=(w[O._role]||[]).find(me=>me.cardId===O.cardId)||O,V=w[O._role]||[],ie=V.findIndex(me=>me.cardId===O.cardId),ae=lt(V.length),fe=ie>=0?ae[ie]:Q._col??1;return{...Q,_line:O._role,_col:fe}}),f=Rt(T,n.modifiers[o]||{});N(o,T.map(O=>O.cardId)),T.forEach(O=>{const Q=(w[O._role]||[]).find(V=>V.cardId===O.cardId);Q&&(Q.used=!0)}),n["selected_"+o]=[],pe();const x=Ot(n.pendingAttack.total,f.total,n.modifiers[o]||{}),y=n.pendingAttack.side,h=x==="attack"||(x==null?void 0:x.goal),v=y==="p1"?"p2":"p1",_=(n.round||0)+1,z=(n.pendingAttack.players||[]).map(O=>({name:O.name,note:je(O,O._line||"ATT"),portrait:Pe(O),job:O.job})),$=[...n.log||[]];$.push({type:"duel",isGoal:h,homeScored:h&&y===o,text:h?`⚽ BUT de ${n[y+"Name"]} ! (${n.pendingAttack.total} vs ${f.total})`:`✋ Attaque stoppée (${n.pendingAttack.total} vs ${f.total})`,homePlayers:z,aiPlayers:T.map(O=>({name:O.name,note:je(O,O._line||"DEF"),portrait:Pe(O),job:O.job})),homeTotal:n.pendingAttack.total,aiTotal:f.total});const A=h?(n[y+"Score"]||0)+1:n[y+"Score"]||0,S={...n,[o+"Team"]:w,[y+"Score"]:A},q=be(S),G=q?"finished":v+"-attack",R=async()=>{await de({[o+"Team"]:w,[r+"Team"]:n[r+"Team"],[y+"Score"]:A,["selected_"+o]:[],modifiers:{...n.modifiers,[o]:{}},pendingAttack:null,phase:G,attacker:v,round:_,log:$}),(G==="finished"||q)&&await M({...n,[y+"Score"]:A})};if(h){const O=y===o,Q=O?A:n[o+"Score"]||0,V=O?n[r+"Score"]||0:A;C.add(_),ve(z,Q,V,O,R)}else await R()}function se(w){return["MIL","ATT"].some(T=>(w[T]||[]).some(f=>!f.used))}function he(w){return["GK","DEF","MIL","ATT"].some(T=>(w[T]||[]).some(f=>!f.used))}function Se(w){return he(w)&&!se(w)}function Me(w){const T=n[w+"Team"],f=n[(w==="p1"?"p2":"p1")+"Team"];return!!(se(T)||!he(f)&&Se(T))}function be(w){const T=["MIL","ATT"].some($=>(w.p1Team[$]||[]).some(A=>!A.used)),f=["MIL","ATT"].some($=>(w.p2Team[$]||[]).some(A=>!A.used)),x=he(w.p1Team),y=he(w.p2Team);return!T&&!(!y&&x)&&(!f&&!(!x&&y))}function ze(w){const T=w.p1Score||0,f=w.p2Score||0;return T===f?null:T>f?u.home_id:u.away_id}async function M(w){try{const T=ze(w),f=T?u.home_id===T?u.away_id:u.home_id:null;await E.from("matches").update({status:"finished",winner_id:T,home_score:w.p1Score||0,away_score:w.p2Score||0}).eq("id",i),T&&f&&ki(T,f).catch(()=>{})}catch(T){console.error("[PvP] finishMatch:",T)}}function F(){var y;const w=n[o+"Score"],T=n[r+"Score"],f=w>T,x=w===T;et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      ...
      <button id="pvp-home" style="padding:16px 40px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">🏠 Retour</button>
    </div>`,(y=document.getElementById("pvp-home"))==null||y.addEventListener("click",()=>{try{E.removeChannel(W)}catch{}We(e),c("home")})}pe()}const Lt="#1A6B3C",xt="#D4A017";async function Un(e,t){var s;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const i=(s=t.state.params)==null?void 0:s.openLeagueId;if(i){t.state.params.openLeagueId=null,await bt(e,t,i);return}await $t(e,t)}async function $t(e,t,i="waiting"){var I;const{state:s}=t,l=s.profile.id,{data:d}=await E.from("mini_league_members").select("league_id").eq("user_id",l),a=(d||[]).map(L=>L.league_id),{data:c,error:g}=await E.from("mini_leagues").select("*, mini_league_members(count)").eq("status","waiting").eq("is_archived",!1).order("created_at",{ascending:!1}).limit(30),o=g?(await E.from("mini_leagues").select("*, mini_league_members(count)").eq("status","waiting").order("created_at",{ascending:!1}).limit(30)).data||[]:c||[],m=(a.length?await E.from("mini_leagues").select("*, mini_league_members(count)").in("id",a).order("created_at",{ascending:!1}):{data:[]}).data||[],b=m.filter(L=>L.status==="waiting"&&!L.is_archived),u=m.filter(L=>L.status==="active"&&!L.is_archived),k=m.filter(L=>L.is_archived||L.status==="finished"),n=o.filter(L=>!a.includes(L.id)),p=[{key:"waiting",label:"🟡 En attente",count:b.length+n.length},{key:"active",label:"🟢 En cours",count:u.length},{key:"archived",label:"📁 Archivées",count:k.length}];e.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="display:flex;background:#fff;border-bottom:1px solid #eee">
      ${p.map(L=>`<button class="ml-tab" data-tab="${L.key}" style="flex:1;padding:10px 4px;border:none;border-bottom:2px solid ${i===L.key?Lt:"transparent"};background:none;font-size:12px;font-weight:${i===L.key?"900":"600"};color:${i===L.key?Lt:"#888"};cursor:pointer">${L.label}${L.count?` (${L.count})`:""}</button>`).join("")}
    </div>
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:10px">
      ${i==="waiting"?Kn(b,n,l):i==="active"?Vn(u,l):Yn(k,l)}
    </div>
  </div>`,(I=document.getElementById("ml-create-btn"))==null||I.addEventListener("click",()=>Wn(e,t)),e.querySelectorAll(".ml-tab").forEach(L=>L.addEventListener("click",()=>$t(e,t,L.dataset.tab))),e.querySelectorAll("[data-league-id]").forEach(L=>L.addEventListener("click",()=>bt(e,t,L.dataset.leagueId))),e.querySelectorAll("[data-join]").forEach(L=>L.addEventListener("click",B=>{B.stopPropagation(),_o(e,t,L.dataset.join,L.dataset.type)})),e.querySelectorAll("[data-delete]").forEach(L=>L.addEventListener("click",B=>{B.stopPropagation(),yi(e,t,L.dataset.delete,L.dataset.name,i)}))}function ei(e,t,i=!1){var a,c;const s=e.creator_id===t,l=e.pot||0,d=((c=(a=e.mini_league_members)==null?void 0:a[0])==null?void 0:c.count)||0;return`<div data-league-id="${e.id}" style="background:#fff;border-radius:12px;padding:14px 16px;box-shadow:0 1px 6px rgba(0,0,0,0.08);cursor:pointer;margin-bottom:8px">
    ...
    <div style="padding:20px;display:flex;flex-direction:column;gap:14px">
    ...
    </div></div>`,document.body.appendChild(i),i.querySelector("#ml-form-close").addEventListener("click",()=>i.remove()),i.addEventListener("click",s=>{s.target===i&&i.remove()}),i.querySelectorAll('input[name="ml-type"]').forEach(s=>s.addEventListener("change",()=>{document.getElementById("ml-pwd-block").style.display=s.value==="private"?"block":"none"})),i.querySelector("#ml-create-confirm").addEventListener("click",async()=>{var k,n,p,I;const{toast:s}=t,l=document.getElementById("ml-name").value.trim(),d=parseInt(document.getElementById("ml-max").value)||6,a=parseInt(document.getElementById("ml-fee").value)||500,c=((k=i.querySelector('input[name="ml-type"]:checked'))==null?void 0:k.value)||"public",g=((n=i.querySelector('input[name="ml-mode"]:checked'))==null?void 0:n.value)||"aller",o=((I=(p=document.getElementById("ml-pwd"))==null?void 0:p.value)==null?void 0:I.trim())||null;if(!l){s("Le nom est obligatoire","error");return}if(a<100){s("Mise minimum : 100 crédits","error");return}if(c==="private"&&!o){s("Mot de passe requis","error");return}const{data:r,error:m}=await E.from("mini_leagues").insert({name:l,creator_id:t.state.profile.id,type:c,password:o,mode:g,max_players:d,entry_fee:a}).select().single();if(m){s("Erreur : "+m.message,"error");return}const{data:b}=await E.from("users").select("credits").eq("id",t.state.profile.id).single();if(((b==null?void 0:b.credits)||0)<a){await E.from("mini_leagues").delete().eq("id",r.id),s(`Crédits insuffisants pour la mise (${a.toLocaleString("fr")} cr.)`,"error");return}await E.from("users").update({credits:b.credits-a}).eq("id",t.state.profile.id),await E.from("mini_leagues").update({pot:a}).eq("id",r.id),await E.from("mini_league_members").insert({league_id:r.id,user_id:t.state.profile.id}),t.state.profile&&(t.state.profile.credits=b.credits-a);const u=document.getElementById("nav-credits");u&&(u.textContent=`💰 ${(t.state.profile.credits||0).toLocaleString("fr")}`),i.remove(),s(`Mini League créée ! ${a.toLocaleString("fr")} cr. déduits.`,"success"),bt(e,t,r.id)})}function Jn(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9500;display:flex;align-items:center;justify-content:center;padding:20px",t.innerHTML=`
      ...
      </div>`,document.body.appendChild(t);const i=t.querySelector("#ml-pwd-input"),s=l=>{t.remove(),e(l)};setTimeout(()=>i==null?void 0:i.focus(),80),t.querySelector("#pwd-cancel").addEventListener("click",()=>s(null)),t.querySelector("#pwd-ok").addEventListener("click",()=>s(i.value.trim())),i.addEventListener("keydown",l=>{l.key==="Enter"&&s(i.value.trim())}),t.addEventListener("click",l=>{l.target===t&&s(null)})})}async function _o(e,t,i,s){const{toast:l,state:d}=t,a=d.profile.id,{data:c}=await E.from("mini_leagues").select("password,status,entry_fee,pot,max_players").eq("id",i).single();if(!c){l("Mini League introuvable","error");return}if(c.status!=="waiting"){l("Cette Mini League a déjà démarré","warning");return}const{count:g}=await E.from("mini_league_members").select("id",{count:"exact",head:!0}).eq("league_id",i);if(g>=c.max_players){l("La Mini League est complète","warning");return}if(s==="private"){const n=await Jn();if(n===null)return;if(c.password!==n){l("Mot de passe incorrect","error");return}}const o=c.entry_fee||100,{data:r}=await E.from("users").select("credits").eq("id",a).single();if(((r==null?void 0:r.credits)||0)<o){l(`Crédits insuffisants — il te faut ${o.toLocaleString("fr")} cr. (solde : ${((r==null?void 0:r.credits)||0).toLocaleString("fr")} cr.)`,"error");return}const{error:m}=await E.from("mini_league_members").insert({league_id:i,user_id:a});if(m){l("Erreur inscription : "+m.message,"error");return}const{error:b}=await E.from("users").update({credits:r.credits-o}).eq("id",a),{error:u}=await E.from("mini_leagues").update({pot:(c.pot||0)+o}).eq("id",i);(b||u)&&console.error("Erreur post-inscription:",b,u),d.profile&&(d.profile.credits=r.credits-o);const k=document.getElementById("nav-credits");k&&(k.textContent=`💰 ${(d.profile.credits||0).toLocaleString("fr")}`),l(`✅ Inscrit ! −${o.toLocaleString("fr")} cr. · Pot : ${((c.pot||0)+o).toLocaleString("fr")} cr.`,"success"),bt(e,t,i)}async function Xn(e,t,i,s){const{toast:l,state:d}=t,a=d.profile.id;if(!confirm(`Te désinscrire et récupérer ${s.toLocaleString("fr")} cr. ?`))return;const{data:c}=await E.from("mini_leagues").select("pot,status").eq("id",i).single();if(!c||c.status!=="waiting"){l("Impossible — la league a déjà démarré","error");return}const{data:g}=await E.from("users").select("credits").eq("id",a).single();await E.from("mini_leagues").update({pot:Math.max(0,(c.pot||0)-s)}).eq("id",i),await E.from("users").update({credits:((g==null?void 0:g.credits)||0)+s}).eq("id",a),await E.from("mini_league_members").delete().eq("league_id",i).eq("user_id",a),d.profile&&(d.profile.credits=((g==null?void 0:g.credits)||0)+s);const o=document.getElementById("nav-credits");o&&(o.textContent=`💰 ${(d.profile.credits||0).toLocaleString("fr")}`),l(`↩️ Désinscrit · +${s.toLocaleString("fr")} cr. remboursés`,"success"),$t(e,t,"waiting")}async function yi(e,t,i,s,l){const{toast:d}=t;if(!confirm(`Supprimer définitivement "${s}" et tous ses matchs/membres ? Action irréversible.`))return;await E.from("mini_league_matches").delete().eq("league_id",i),await E.from("mini_league_members").delete().eq("league_id",i);const{error:a}=await E.from("mini_leagues").delete().eq("id",i);if(a){d("Erreur suppression ("+a.message+")","error");return}d("Mini League supprimée avec succès","success"),$t(e,t,l)}async function Zn(e,t,i){await E.from("mini_leagues").update({is_archived:!0}).eq("id",i),t.toast("Mini League archivée","success"),$t(e,t,"archived")}async function bt(e,t,i){var H,C,D,j,W,de,re,P,X;const{state:s,toast:l}=t,d=s.profile.id,[{data:a},{data:c},{data:g}]=await Promise.all([E.from("mini_leagues").select("*").eq("id",i).single(),E.from("mini_league_members").select("*, user:users(id,pseudo,club_name,club_color1,club_color2)").eq("league_id",i),E.from("mini_league_matches").select("*").eq("league_id",i).order("matchday").order("created_at")]);if(!a){l("Introuvable","error"),await $t(e,t);return}const o=(c||[]).some(U=>U.user_id===d),r=a.creator_id===d,m=(c||[]).map(U=>U.user).filter(Boolean),b=ko(m,g||[]),u=(g||[]).filter(U=>U.matchday===a.current_day),k=a.pot||0,n=a.entry_fee||100,p=u.length>0&&u.every(U=>U.status==="finished"||U.status==="bye"),I=a.current_day>=a.total_days,L=(c||[]).find(U=>U.user_id===d);e.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    <div style="padding:12px 16px;background:#fff;border-bottom:1px solid #eee;display:flex;align-items:center;gap:10px">
      ...
      <div style="flex:1">
        ...
        <div style="font-size:11px;color:#888">${a.mode==="aller-retour"?"Aller-Retour":"Aller"} · max ${a.max_players} · 💰 ${n} cr./joueur</div>
      </div>
      <div style="text-align:right;flex-shrink:0">
        ...
        ${k>0?`<div style="font-size:12px;font-weight:900;color:${xt};margin-top:4px">🏆 ${k.toLocaleString("fr")} cr.</div>`:""}
      </div>
    </div>
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        ${r&&p&&!I?'<button id="ml-next-day" class="btn btn-primary" style="width:100%;margin-top:12px">➡️ Journée suivante</button>':""}
        ${r&&p&&I?'<button id="ml-finish-btn" class="btn btn-primary" style="width:100%;margin-top:12px;background:#7a28b8">🏆 Terminer et distribuer le pot</button>':""}
      </div>`:""}
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          <thead><tr style="font-size:10px;color:#888;text-transform:uppercase;border-bottom:2px solid #f0f0f0">
            ...
            ${k>0&&a.status==="finished"?`<th style="text-align:right;padding:5px 0;color:${xt}">💰</th>`:""}
          </tr></thead>
          <tbody>${b.map((U,N)=>{const ne=k>0&&a.status==="finished"?N===0?Math.floor(k*.7):N===1?Math.floor(k*.2):N===2?Math.floor(k*.1):0:0;return`<tr style="border-bottom:1px solid #f5f5f5;${U.userId===d?"background:#f0fdf4;":""}">
              ...
              ${k>0&&a.status==="finished"?`<td style="text-align:right;font-weight:700;color:${xt}">${ne?ne.toLocaleString("fr")+" cr.":"—"}</td>`:""}
            </tr>`}).join("")}</tbody>
        </table>
      </div>`:""}
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        ${Array.from({length:Math.max(0,a.status==="active"?a.current_day-1:a.current_day)},(U,N)=>N+1).reverse().map(U=>{const N=(g||[]).filter(ne=>ne.matchday===U);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:#888;margin-bottom:6px">Journée ${U}</div>${N.map(ne=>Ri(ne,m,d,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}

      ${r&&!a.is_archived&&a.status!=="waiting"?`
      <div style="display:flex;gap:8px">
  ...
  </div>`;const B=a.status==="waiting"?"waiting":a.status==="active"?"active":"archived";if((H=document.getElementById("ml-back"))==null||H.addEventListener("click",()=>$t(e,t,B)),(C=document.getElementById("ml-start-btn"))==null||C.addEventListener("click",()=>Qn(e,t,a,m)),(D=document.getElementById("ml-next-day"))==null||D.addEventListener("click",()=>er(e,t,i)),(j=document.getElementById("ml-finish-btn"))==null||j.addEventListener("click",()=>$o(e,t,i,k,b,m)),(W=document.getElementById("ml-join-now"))==null||W.addEventListener("click",()=>_o(e,t,i,a.type)),(de=document.getElementById("ml-leave-btn"))==null||de.addEventListener("click",()=>Xn(e,t,i,n)),(re=document.getElementById("ml-delete-btn"))==null||re.addEventListener("click",()=>yi(e,t,i,a.name,"waiting")),(P=document.getElementById("ml-delete-now"))==null||P.addEventListener("click",()=>yi(e,t,i,a.name,B)),(X=document.getElementById("ml-archive-btn"))==null||X.addEventListener("click",()=>Zn(e,t,i)),e.querySelectorAll("[data-play-match]").forEach(U=>{U.addEventListener("click",()=>{const N=u.find(ne=>ne.id===U.dataset.playMatch);N&&t.navigate("match-mini-league",{mlMatchId:N.id,leagueId:i})})}),a.status==="finished"&&L){const U=b.findIndex(N=>N.userId===d);U>=0&&U<3&&L.prize_amount>0&&setTimeout(()=>tr(e,t,a,L,U),400)}}function Ri(e,t,i,s,l=!1){const d=m=>t.find(b=>b.id===m);if(e.is_bye){const m=d(e.home_id);return`<div style="padding:8px;border-radius:8px;background:#f9f9f9;margin-bottom:6px;font-size:12px;color:#888;text-align:center">🔵 ${(m==null?void 0:m.club_name)||(m==null?void 0:m.pseudo)||"?"} exempté(e)</div>`}const a=d(e.home_id),c=d(e.away_id),g=e.home_id===i||e.away_id===i,o=g&&e.status==="pending"&&s&&!l,r=e.status==="finished"?`${e.home_score} - ${e.away_score}`:"vs";return`<div style="display:flex;align-items:center;gap:8px;padding:10px;border-radius:8px;background:${g?"#f0fdf4":"#f9f9f9"};margin-bottom:6px;border:1px solid ${g?"#86efac":"#f0f0f0"}">
  ...
  </div>`}async function Qn(e,t,i,s){const{toast:l,state:d}=t,a=ir(s.map(o=>o.id),i.mode),c=[];a.forEach((o,r)=>o.forEach(m=>c.push({league_id:i.id,matchday:r+1,home_id:m.home||m.bye,away_id:m.away||null,is_bye:!!m.bye,status:m.bye?"bye":"pending"})));const{error:g}=await E.from("mini_league_matches").insert(c);if(g){l("Erreur calendrier : "+g.message,"error");return}await E.from("mini_leagues").update({status:"active",current_day:1,total_days:a.length}).eq("id",i.id),l(`🚀 Lancée ! Pot : ${(i.pot||0).toLocaleString("fr")} cr.`,"success"),bt(e,t,i.id)}async function er(e,t,i){const{data:s}=await E.from("mini_leagues").select("current_day,total_days,pot").eq("id",i).single(),l=(s.current_day||0)+1;if(l>(s.total_days||0)){await $o(e,t,i,s.pot||0,null,null);return}await E.from("mini_leagues").update({current_day:l}).eq("id",i),t.toast(`Journée ${l} commencée !`,"success"),bt(e,t,i)}async function $o(e,t,i,s,l,d){const{toast:a,state:c}=t;let g=l,o=d;if(!g){const{data:m}=await E.from("mini_league_matches").select("*").eq("league_id",i),{data:b}=await E.from("mini_league_members").select("*, user:users(id,pseudo,club_name)").eq("league_id",i);o=(b||[]).map(u=>u.user).filter(Boolean),g=ko(o,m||[])}const r=[Math.floor(s*.7),Math.floor(s*.2),Math.floor(s*.1)];await Promise.all(g.slice(0,3).map((m,b)=>r[b]?E.from("mini_league_members").update({prize_amount:r[b],prize_claimed:!1}).eq("league_id",i).eq("user_id",m.userId):Promise.resolve())),await E.from("mini_leagues").update({status:"finished"}).eq("id",i),a("🏆 Mini League terminée ! Les gagnants peuvent récupérer leurs crédits.","success"),bt(e,t,i)}async function tr(e,t,i,s,l){var b,u;const{state:d,toast:a}=t,c=[Math.floor((i.pot||0)*.7),Math.floor((i.pot||0)*.2),Math.floor((i.pot||0)*.1)],g=["🥇","🥈","🥉"][l],o=s.prize_amount||c[l]||0,r=s.prize_claimed,m=document.createElement("div");m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",m.innerHTML=`
    <div style="background:linear-gradient(160deg,#1a0a2e,#3b1a6e);border-radius:20px;padding:32px 24px;max-width:320px;width:100%;text-align:center;color:#fff;box-shadow:0 8px 40px rgba(0,0,0,0.6)">
    ...
    </div>`,document.body.appendChild(m),(b=m.querySelector("#prize-close"))==null||b.addEventListener("click",()=>m.remove()),m.addEventListener("click",k=>{k.target===m&&m.remove()}),(u=m.querySelector("#claim-prize-btn"))==null||u.addEventListener("click",async()=>{const{data:k}=await E.from("users").select("credits").eq("id",d.profile.id).single();await E.from("users").update({credits:((k==null?void 0:k.credits)||0)+o}).eq("id",d.profile.id),await E.from("mini_league_members").update({prize_claimed:!0}).eq("league_id",i.id).eq("user_id",d.profile.id),d.profile&&(d.profile.credits=((k==null?void 0:k.credits)||0)+o);const n=document.getElementById("nav-credits");n&&(n.textContent=`💰 ${d.profile.credits.toLocaleString("fr")}`),a(`💰 +${o.toLocaleString("fr")} cr. ajoutés à ton solde !`,"success"),m.remove(),bt(e,t,i.id)})}function ir(e,t){const s=e.length%2===0?[...e]:[...e,null],l=s.length;let d=s.slice(1);const a=[];for(let c=0;c<l-1;c++){const g=[],o=[s[0],...d];for(let r=0;r<l/2;r++){const m=o[r],b=o[l-1-r];m===null?g.push({bye:b}):b===null?g.push({bye:m}):g.push({home:m,away:b})}a.push(g),d=[d[d.length-1],...d.slice(0,-1)]}return t==="aller-retour"?[...a,...a.map(c=>c.map(g=>g.bye?g:{home:g.away,away:g.home}))]:a}function ko(e,t){const i={};return e.forEach(s=>{i[s.id]={userId:s.id,pseudo:s.pseudo,clubName:s.club_name||s.pseudo,played:0,won:0,drawn:0,lost:0,goalsFor:0,goalsAgainst:0,goalDiff:0,points:0}}),t.filter(s=>s.status==="finished"&&!s.is_bye&&s.home_score!=null).forEach(s=>{const l=i[s.home_id],d=i[s.away_id];!l||!d||(l.played++,d.played++,l.goalsFor+=s.home_score,l.goalsAgainst+=s.away_score,d.goalsFor+=s.away_score,d.goalsAgainst+=s.home_score,s.home_score>s.away_score?(l.won++,l.points+=3,d.lost++):s.home_score<s.away_score?(d.won++,d.points+=3,l.lost++):(l.drawn++,l.points++,d.drawn++,d.points++),l.goalDiff=l.goalsFor-l.goalsAgainst,d.goalDiff=d.goalsFor-d.goalsAgainst)}),Object.values(i).sort((s,l)=>l.points-s.points||l.goalDiff-s.goalDiff||l.goalsFor-s.goalsFor)}async function Oi(e,t,i,s){var l,d;t.state.params={...t.state.params,matchMode:"mini-league",mlMatchId:i,leagueId:s};try{const a=(d=(l=t==null?void 0:t.state)==null?void 0:l.profile)==null?void 0:d.id;try{(E.getChannels?E.getChannels():[]).forEach(g=>{const o=g.topic||"";(o.includes("matchmaking")||o.includes("pvp-match"))&&E.removeChannel(g)})}catch(c){console.warn("[FriendMatch] cleanup canaux:",c)}a&&(await E.rpc("cancel_matchmaking",{p_user_id:a}).catch(()=>{}),await E.from("matchmaking_queue").delete().eq("user_id",a).then(()=>{},()=>{}))}catch{}await ai(e,t,"random",({deckId:a,formation:c,starters:g,subsRaw:o,gcCardsEnriched:r,gcDefs:m,stadiumDef:b})=>{const u=k=>or(e,t,a,c,g,o,k,m||[],i);if(!r.length){u([]);return}ri(e,r,u)})}async function or(e,t,i,s,l,d,a=[],c=[],g,o){const{state:r,navigate:m,toast:b}=t,u=r.profile.id;let k=!1,n=null;et(e);try{await E.rpc("cancel_matchmaking",{p_user_id:u})}catch{}try{await E.from("matchmaking_queue").delete().eq("user_id",u)}catch{}const{data:p}=await E.from("mini_league_matches").select("*, home:users!home_id(id,pseudo,club_name), away:users!away_id(id,pseudo,club_name)").eq("id",g).single();if(!p){b("Match introuvable","error"),We(e),m("mini-league");return}if(p.home_id!==u&&p.away_id!==u){b("Tu ne fais pas partie de ce match","error"),We(e),m("mini-league");return}const I=p.home_id===u,L=I?p.away:p.home,B=I?p.away_id:p.home_id,H=(D,j)=>{var W;e.innerHTML=`
      <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:20px;background:linear-gradient(180deg,#0a1628,#1a0a2e);padding:24px;text-align:center;color:#fff">
        ...
        <button id="lobby-cancel" style="padding:10px 24px;border-radius:10px;border:1.5px solid rgba(255,255,255,0.2);background:transparent;color:rgba(255,255,255,0.6);font-size:13px;cursor:pointer">Annuler</button>
        <style>@keyframes lobbyPulse{0%,100%{opacity:1}50%{opacity:0.4}}</style>
      </div>`,(W=document.getElementById("lobby-cancel"))==null||W.addEventListener("click",()=>{k=!0,clearInterval(n),We(e),m("mini-league")})},C=async(D,j)=>{k=!0,clearInterval(n),await new Promise(W=>setTimeout(W,600)),e.isConnected&&nr(e,t,D,j,a,c)};if(H(),I){let D=p.match_id;if(!D){const{data:W,error:de}=await E.from("matches").insert({home_id:u,away_id:B,home_deck_id:i,away_deck_id:null,status:"active",mode:"mini_league"}).select().single();if(de||!W){b("Erreur création match","error"),We(e),m("mini-league");return}D=W.id,await E.from("mini_league_matches").update({match_id:D,status:"playing"}).eq("id",g)}const j=D;n=setInterval(async()=>{if(k){clearInterval(n);return}const{data:W}=await E.from("matches").select("away_deck_id").eq("id",j).single();W!=null&&W.away_deck_id&&(clearInterval(n),C(j,!0))},1500)}else{let D=p.match_id;n=setInterval(async()=>{if(k){clearInterval(n);return}const{data:j}=await E.from("mini_league_matches").select("match_id").eq("id",g).single();j!=null&&j.match_id&&(clearInterval(n),D=j.match_id,await E.from("matches").update({away_deck_id:i}).eq("id",D),C(D,!1))},1500)}}async function nr(e,t,i,s,l=[],d=[]){var T;const{state:a,navigate:c,toast:g}=t,o=((T=a.params)==null?void 0:T.leagueId)||null,r=s?"p1":"p2",m=s?"p2":"p1",b=(l||[]).map(f=>f.id),u=(l||[]).map(f=>({id:f.id,gc_type:f.gc_type,_gcDef:f._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:k}=await E.from("matches").select("*").eq("id",i).single();if(!k){g("Match introuvable","error"),c("mini-league");return}async function n(){const[{data:f},{data:x},{data:y},{data:h}]=await Promise.all([E.rpc("get_deck_for_match",{p_deck_id:k.home_deck_id}),E.rpc("get_deck_for_match",{p_deck_id:k.away_deck_id}),E.from("users").select("id,pseudo,club_name").eq("id",k.home_id).single(),E.from("users").select("id,pseudo,club_name").eq("id",k.away_id).single()]),v=S=>{const q=Number(S.evolution_bonus)||0;return{cardId:S.card_id,position:S.position,id:S.id,firstname:S.firstname,name:S.surname_encoded,country_code:S.country_code,club_id:S.club_id,job:S.job,job2:S.job2,note_g:(Number(S.note_g)||0)+(S.job==="GK"||S.job2==="GK"&&Number(S.note_g)>0?q:0),note_d:(Number(S.note_d)||0)+(S.job==="DEF"||S.job2==="DEF"&&Number(S.note_d)>0?q:0),note_m:(Number(S.note_m)||0)+(S.job==="MIL"||S.job2==="MIL"&&Number(S.note_m)>0?q:0),note_a:(Number(S.note_a)||0)+(S.job==="ATT"||S.job2==="ATT"&&Number(S.note_a)>0?q:0),evolution_bonus:q,rarity:S.rarity,skin:S.skin,hair:S.hair,hair_length:S.hair_length,clubName:S.club_encoded_name||null,clubLogo:S.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},_=((f==null?void 0:f.starters)||[]).map(S=>v(S)),z=((x==null?void 0:x.starters)||[]).map(S=>v(S)),$=(f==null?void 0:f.formation)||"4-4-2",A=(x==null?void 0:x.formation)||"4-4-2";return{p1Team:(()=>{const S=yt(_,$);return stadiumDef?Ht(S,stadiumDef):S})(),p2Team:yt(z,A),p1Subs:(()=>{const S=((f==null?void 0:f.subs)||[]).map(q=>v(q));return stadiumDef&&oi(S,stadiumDef),S})(),p2Subs:((x==null?void 0:x.subs)||[]).map(S=>v(S)),p1Formation:$,p2Formation:A,p1Name:(y==null?void 0:y.club_name)||(y==null?void 0:y.pseudo)||"Joueur 1",p2Name:(h==null?void 0:h.club_name)||(h==null?void 0:h.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:s?b:[],gc_p2:s?[]:b,gcCardsFull_p1:s?u:[],gcCardsFull_p2:s?[]:u,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:d||[],lastActionAt:new Date().toISOString()}}let p=k.game_state&&Object.keys(k.game_state).length?k.game_state:null;if(!p)if(s){p=await n();const{data:f}=await E.from("matches").select("game_state").eq("id",i).single();!(f!=null&&f.game_state)||!Object.keys(f.game_state).length?await E.from("matches").update({game_state:p,turn_user_id:k.home_id}).eq("id",i):p=f.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let f=0;f<60&&!p;f++){await new Promise(y=>setTimeout(y,400));const{data:x}=await E.from("matches").select("game_state").eq("id",i).single();x!=null&&x.game_state&&Object.keys(x.game_state).length&&(p=x.game_state)}if(!p){g("Erreur de synchronisation","error"),c("mini-league");return}p.gc_p2=b,p.gcCardsFull_p2=u,await E.from("matches").update({game_state:p}).eq("id",i)}let I=!1,L=!1,B=!1,H=null,C=!1;const D=new Set,j=new Set;function W(f){var S,q;try{E.removeChannel(de)}catch{}const x=p[r+"Score"]||0,y=p[m+"Score"]||0;if(!B){B=!0;const G=f.winner_id||(x>y?a.profile.id:y>x?"opp":null),R=G===a.profile.id?"win":G?"loss":null;R&&ti(()=>Promise.resolve().then(()=>oo),void 0).then(O=>O.applyOwnEvolution(a.profile.id,R)).catch(()=>{})}if(!L){const G=p.p1Score||0,R=p.p2Score||0,O=(p.usedGc_p1||[]).length,Q=(p.usedGc_p2||[]).length,V=f.winner_id||(G>R?k.home_id:R>G?k.away_id:null);(V?a.profile.id===V:a.profile.id<(s?k.away_id:k.home_id))&&(L=!0,Xi({player1Id:k.home_id,player2Id:k.away_id,score1:G,score2:R,gc1:O,gc2:Q}).catch(ae=>console.warn("[FriendMatch] updateStats:",ae)))}let h,v;f.winner_id?(h=f.winner_id===a.profile.id,v=!1):f.forfeit?(h=x>y,v=!1):(v=x===y,h=x>y),(S=document.getElementById("pvp-end-overlay"))==null||S.remove();const _=document.createElement("div");_.id="pvp-end-overlay",_.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const z=v?"🤝":h?"🏆":"😞",$=v?"MATCH NUL":h?"VICTOIRE !":"DÉFAITE",A=v?"#fff":h?"#FFD700":"#ff6b6b";_.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">🏆 Retour à la Mini League</button>`,document.body.appendChild(_),(q=_.querySelector("#pvp-end-home"))==null||q.addEventListener("click",()=>{_.remove(),We(e),o?c("mini-league",{openLeagueId:o}):c("mini-league")})}const de=E.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},f=>{const x=f.new;try{if(x.status==="finished"||x.forfeit){if(I||(I=!0,H&&(clearInterval(H),H=null),x.game_state&&(p=x.game_state),p.phase==="finished"&&!x.forfeit&&document.getElementById("pvp-home")))return;W(x);return}if(x.game_state){const y=p;p=x.game_state;const h=p._lastGC;if(h&&h.seq&&!j.has(h.seq)&&(j.add(h.seq),h.by!==r)){Ae(h.type,h.by,()=>ye());return}const v=y[r+"Score"]||0,_=y[m+"Score"]||0,z=p[r+"Score"]||0,$=p[m+"Score"]||0,A=z>v,S=$>_;if((A||S)&&!D.has(p.round)){D.add(p.round);const q=[...p.log||[]].reverse().find(R=>R.isGoal),G=((q==null?void 0:q.homePlayers)||[]).map(R=>({name:R.name,note:R.note,portrait:R.portrait,job:R.job}));_e(G,z,$,A,()=>ye());return}ye()}}catch(y){console.error("[PvP] crash:",y)}}).subscribe();async function re(f){Object.assign(p,f),p.lastActionAt=new Date().toISOString();const{error:x}=await E.from("matches").update({game_state:p}).eq("id",i);x&&g("Erreur de synchronisation","error");try{ye()}catch(y){console.error("[PvP] renderPvpScreen crash:",y)}}async function P(){if(I)return;I=!0,H&&(clearInterval(H),H=null);const f=s?k.away_id:k.home_id,x=s?"p2":"p1",y=s?"p1":"p2",h={...p,[x+"Score"]:3,[y+"Score"]:0,phase:"finished"};try{await E.from("matches").update({status:"finished",forfeit:!0,winner_id:f,home_score:h.p1Score||0,away_score:h.p2Score||0,game_state:h}).eq("id",i)}catch{}try{E.removeChannel(de)}catch{}setTimeout(()=>{We(e),c("mini-league")},800)}const X={BOOST_STAT:({value:f=1,count:x=1,roles:y=[]},h,v)=>{const _=h[r+"Team"],z=Object.entries(_).filter(([$])=>!y.length||y.includes($)).flatMap(([$,A])=>A.filter(S=>!S.used).map(S=>({...S,_line:$})));if(!z.length){h.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),v(h);return}U(z,x,`Choisir ${x} joueur(s) à booster (+${f})`,$=>{$.forEach(A=>{const S=(_[A._line]||[]).find(q=>q.cardId===A.cardId);S&&(S.boost=(S.boost||0)+f,h.log.push({text:`⚡ +${f} sur ${S.name}`,type:"info"}))}),h[r+"Team"]=_,v(h)})},DEBUFF_STAT:({value:f=1,count:x=1,roles:y=[],target:h="ai"},v,_)=>{const z=h==="home"?r:m,$=v[z+"Team"],A=h==="home"?"allié":"adverse",S=Object.entries($).filter(([q])=>!y.length||y.includes(q)).flatMap(([q,G])=>G.map(R=>({...R,_line:q})));if(!S.length){v.log.push({text:`🎯 Aucun joueur ${A}`,type:"info"}),_(v);return}U(S,x,`Choisir ${x} joueur(s) ${A}(s) (-${f})`,q=>{q.forEach(G=>{const R=($[G._line]||[]).find(O=>O.cardId===G.cardId);R&&(R.boost=(R.boost||0)-f,v.log.push({text:`🎯 -${f} sur ${R.name}`,type:"info"}))}),v[z+"Team"]=$,_(v)})},GRAY_PLAYER:({count:f=1,roles:x=[],target:y="ai"},h,v)=>{const _=y==="home"?r:m,z=h[_+"Team"],$=y==="home"?"allié":"adverse",A=Object.entries(z).filter(([S])=>!x.length||x.includes(S)).flatMap(([S,q])=>q.filter(G=>!G.used).map(G=>({...G,_line:S})));if(!A.length){h.log.push({text:`❌ Aucun joueur ${$}`,type:"info"}),v(h);return}U(A,f,`Choisir ${f} joueur(s) ${$}(s) à exclure`,S=>{const q="usedCardIds_"+_,G=new Set(h[q]||[]);S.forEach(R=>{const O=(z[R._line]||[]).find(Q=>Q.cardId===R.cardId);O&&(O.used=!0,G.add(R.cardId),h.log.push({text:`❌ ${O.name} exclu !`,type:"info"}))}),h[q]=[...G],h[_+"Team"]=z,v(h)})},REVIVE_PLAYER:({count:f=1,roles:x=[]},y,h)=>{const v=y[r+"Team"],_=Object.entries(v).filter(([z])=>!x.length||x.includes(z)).flatMap(([z,$])=>$.filter(A=>A.used).map(A=>({...A,_line:z})));if(!_.length){y.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),h(y);return}U(_,f,`Choisir ${f} joueur(s) à ressusciter`,z=>{z.forEach($=>{const A=(v[$._line]||[]).find(S=>S.cardId===$.cardId);A&&(A.used=!1,y.log.push({text:`💫 ${A.name} ressuscité !`,type:"info"}))}),y[r+"Team"]=v,h(y)})},REMOVE_GOAL:({},f,x)=>{const y=m+"Score";f[y]>0?(f[y]--,f.log.push({text:"🚫 Dernier but annulé !",type:"info"})):f.log.push({text:"🚫 Aucun but à annuler",type:"info"}),x(f)},ADD_GOAL_DRAW:({},f,x)=>{f[r+"Score"]===f[m+"Score"]?(f[r+"Score"]++,f.log.push({text:"🎯 But bonus !",type:"info"})):f.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),x(f)},ADD_SUB:({value:f=1},x,y)=>{x.maxSubs=(x.maxSubs||3)+f,x.log.push({text:`🔄 +${f} remplacement(s)`,type:"info"}),y(x)},CUSTOM:({},f,x)=>x(f)};function U(f,x,y,h){const v=document.createElement("div");v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let _=[];function z(){var A,S;const $=f.map(q=>{const G={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[q._line]||"#555",R=je(q,q._line)+(q.boost||0),Q=_.find(ie=>ie.cardId===q.cardId)?"#FFD700":"rgba(255,255,255,0.25)",V=q.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${q.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${Q};background:${G};overflow:hidden;cursor:pointer;${V}">
        ...
        </div>`,(A=v.querySelector("#pp-close"))==null||A.addEventListener("click",()=>v.remove()),v.querySelectorAll(".pp-item").forEach(q=>{q.addEventListener("click",()=>{const G=q.dataset.cid,R=f.find(Q=>Q.cardId===G),O=_.findIndex(Q=>Q.cardId===G);R&&(O>-1?_.splice(O,1):_.length<x&&_.push(R),z())})}),(S=v.querySelector("#pp-confirm"))==null||S.addEventListener("click",()=>{v.remove(),h(_)})}z(),document.body.appendChild(v)}async function N(f,x){const h=(p["gcCardsFull_"+r]||[]).find($=>$.id===f),v=(h==null?void 0:h._gcDef)||(p.gcDefs||[]).find($=>{var A;return $.name===x||((A=$.name)==null?void 0:A.toLowerCase().trim())===(x==null?void 0:x.toLowerCase().trim())}),_=[...p["usedGc_"+r]||[],f],z={type:x,by:r,seq:(p._gcAnimSeq||0)+1};j.add(z.seq),Ae(x,r,async()=>{if(v!=null&&v.effect_type&&v.effect_type!=="CUSTOM"){const A=X[v.effect_type];if(A){const S={...p};A(v.effect_params||{},S,async q=>{q["usedGc_"+r]=_,q._lastGC=z,q._gcAnimSeq=z.seq,await re(q)});return}}const $={...p};switch(x){case"Remplacement+":$.maxSubs=($.maxSubs||3)+1,$.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const A=m+"Score";$[A]>0&&($[A]--,$.log.push({text:"🚫 But annulé",type:"info"}));break}}$["usedGc_"+r]=_,$._lastGC=z,$._gcAnimSeq=z.seq,await re($)})}function ne(f,x){const y="usedCardIds_"+f,h=new Set(p[y]||[]);x.forEach(v=>h.add(v)),p[y]=[...h]}function pe(){for(const f of["p1","p2"]){const x=new Set(p["usedCardIds_"+f]||[]),y=p[f+"Team"];if(y)for(const h of["GK","DEF","MIL","ATT"])(y[h]||[]).forEach(v=>{v.used=x.has(v.cardId)})}}function ye(){var nt,gt,le,Ee,te,ue;if(p.phase==="reveal")return Le();if(p.phase==="midfield")return ve();if(p.phase==="finished")return w();const f=p[r+"Team"],x=p[m+"Team"];pe();const y=p[r+"Score"],h=p[m+"Score"],v=p[r+"Name"],_=p[m+"Name"],z=p.phase===r+"-attack",$=p.phase===r+"-defense",A=Array.isArray(p["selected_"+r])?p["selected_"+r]:[],S=A.map(ee=>ee.cardId),q=window.innerWidth>=700,G=p[r+"Subs"]||[],R=p["usedSubIds_"+r]||[],O=G.filter(ee=>!R.includes(ee.cardId)),Q=p["gcCardsFull_"+r]||[],V=p["usedGc_"+r]||[],ie=Q.filter(ee=>!V.includes(ee.id)),ae=p.boostOwner===r&&!p.boostUsed,fe=!["GK","DEF","MIL","ATT"].some(ee=>(x[ee]||[]).some(ce=>!ce.used)),me=[...f.MIL||[],...f.ATT||[]].filter(ee=>!ee.used),$e=z&&fe&&me.length===0?[...f.GK||[],...f.DEF||[]].filter(ee=>!ee.used).map(ee=>ee.cardId):[];function ke(ee,ce,Be){var zi,Si;const Te=ee._gcDef,tt={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[Te==null?void 0:Te.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",it={purple:"#b06ce0",light_blue:"#00d4ef"}[Te==null?void 0:Te.color]||"#b06ce0",rt=(Te==null?void 0:Te.name)||ee.gc_type,st=(Te==null?void 0:Te.effect)||((zi=Ve[ee.gc_type])==null?void 0:zi.desc)||"",ct=Te!=null&&Te.image_url?`/icons/${Te.image_url}`:null,kt=((Si=Ve[ee.gc_type])==null?void 0:Si.icon)||"⚡",pt=Math.round(Be*.22),ht=Math.round(Be*.22),Et=Be-pt-ht,Ut=rt.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${ee.id}" data-gc-type="${ee.gc_type}"
      ...
      </div>`}const Ce=(ee,ce)=>ce?qe(130,175):ke(ee,130,175),Re=(ee,ce)=>ce?qe(68,95):ke(ee,68,95),Fe=q?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",Oe=z?be(r)?`<button id="pvp-action" style="${Fe};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${A.length===0?"disabled":""}>⚔️ ATTAQUEZ <span id="pvp-timer"></span></button>`:`<button id="pvp-action" data-pass="1" style="${Fe};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER <span id="pvp-timer"></span></button>`:$?`<button id="pvp-action" style="${Fe};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${A.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="pvp-timer"></span></button>`:`<div style="font-size:11px;color:rgba(255,255,255,0.3);text-align:center;padding:4px">⏳ Tour de ${_}</div>`,De=z&&!be(r)?'<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">Aucun attaquant — passez la main</div>':z||$?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${A.length}/3 sélectionné(s)</div>`:"",Ke=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${q?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${ft(f,p[r+"Formation"],Ne,S,300,300,$e)}
      </div>
    ...
    </div>`,r==="p1"&&setTimeout(async()=>{await re({phase:"midfield"})},5e3)}let we=!1;function ve(){if(we)return;const f=p[r+"Team"].MIL||[],x=p[m+"Team"].MIL||[];function y(ie){return ie.reduce((ae,fe)=>ae+je(fe,"MIL"),0)}function h(ie){let ae=0;for(let fe=0;fe<ie.length-1;fe++){const me=at(ie[fe],ie[fe+1]);me==="#00ff88"?ae+=2:me==="#FFD700"&&(ae+=1)}return ae}const v=y(f)+h(f),_=y(x)+h(x),z=v>=_;function $(ie,ae,fe){return`<div id="duel-row-${fe}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(_);let A=!1;const S=()=>{A||(A=!0,_.remove(),setTimeout(()=>v(),50))};_.addEventListener("click",S),setTimeout(S,3500)}function Ae(f,x,y){var V,ie;const h=(p.gcDefs||[]).find(ae=>{var fe;return ae.name===f||((fe=ae.name)==null?void 0:fe.toLowerCase().trim())===(f==null?void 0:f.toLowerCase().trim())}),v={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[h==null?void 0:h.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",_={purple:"#b06ce0",light_blue:"#00d4ef"}[h==null?void 0:h.color]||"#b06ce0",z=(h==null?void 0:h.name)||f,$=(h==null?void 0:h.effect)||((V=Ve[f])==null?void 0:V.desc)||"",A=h!=null&&h.image_url?`/icons/${h.image_url}`:null,S=((ie=Ve[f])==null?void 0:ie.icon)||"⚡",G=x===r?"Vous":p[x+"Name"]||"Adversaire",R=document.createElement("div");R.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",R.innerHTML=`
      <style>
        @keyframes gcFlipIn{0%{transform:perspective(800px) rotateY(90deg) scale(.7);opacity:0}55%{transform:perspective(800px) rotateY(-12deg) scale(1.08);opacity:1}100%{transform:perspective(800px) rotateY(0) scale(1);opacity:1}}
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(R);let O=!1;const Q=()=>{O||(O=!0,R.remove(),setTimeout(()=>y&&y(),50))};R.addEventListener("click",Q),setTimeout(Q,3e3)}function Ie(f,x){var R,O,Q,V;const h=(p["gcCardsFull_"+r]||[]).find(ie=>ie.id===f),v=h==null?void 0:h._gcDef,_={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[v==null?void 0:v.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",z={purple:"#b06ce0",light_blue:"#00d4ef"}[v==null?void 0:v.color]||"#b06ce0",$=(v==null?void 0:v.name)||x,A=(v==null?void 0:v.effect)||((R=Ve[x])==null?void 0:R.desc)||"Carte spéciale.",S=v!=null&&v.image_url?`/icons/${v.image_url}`:null,q=((O=Ve[x])==null?void 0:O.icon)||"⚡",G=document.createElement("div");G.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",G.innerHTML=`
      ...
      </div>`,document.body.appendChild(y),(h=y.querySelector("#bp-close"))==null||h.addEventListener("click",()=>y.remove()),y.querySelectorAll(".bp-item").forEach(v=>{v.addEventListener("click",async()=>{const _=v.dataset.cid,z=x.find(A=>A.cardId===_);if(!z)return;const $=(f[z._line]||[]).find(A=>A.cardId===_);$&&($.boost=($.boost||0)+p.boostValue),y.remove(),await re({[r+"Team"]:f,boostUsed:!0})})})}function J(f=null){var O,Q;if(!(p.phase===r+"-attack")){g("Remplacement uniquement avant votre attaque","warning");return}const y=p[r+"Team"],h=p["usedSubIds_"+r]||[],v=p.maxSubs||3;if(h.length>=v){g(`Maximum ${v} remplacements atteint`,"warning");return}const _=Object.entries(y).flatMap(([V,ie])=>(ie||[]).filter(ae=>ae.used).map(ae=>({...ae,_line:V}))),z=(p[r+"Subs"]||[]).filter(V=>!h.includes(V.cardId));if(!_.length){g("Aucun joueur utilisé à remplacer","warning");return}if(!z.length){g("Aucun remplaçant disponible","warning");return}let $=Math.max(0,_.findIndex(V=>V.cardId===f));const A=((O=_[$])==null?void 0:O._line)||((Q=_[$])==null?void 0:Q.job);let S=Math.max(0,z.findIndex(V=>V.job===A)),q=!1;const G=document.createElement("div");G.id="pvp-sub-overlay",G.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function R(){var ke,qe,Ce,Re,Fe,Oe;const V=_[$],ie=z[S],ae=Math.min(130,Math.round((window.innerWidth-90)/2)),fe=Math.round(ae*1.35),me=De=>`background:rgba(255,255,255,0.12);border:none;color:${De?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${De?"default":"pointer"};flex-shrink:0`;G.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      </div>`,document.body.appendChild(x),(h=x.querySelector("#pvp-hist-close"))==null||h.addEventListener("click",()=>x.remove())}async function Z(){if(p.phase!==r+"-attack")return;const f=r==="p1"?"p2":"p1",x=(p.round||0)+1,y=[...p.log||[]];y.push({type:"info",text:`⏭️ ${p[r+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const h=ze(p),v=be(f),_=h||!v?"finished":f+"-attack";await re({["selected_"+r]:[],modifiers:{...p.modifiers,[r]:{}},pendingAttack:null,phase:_,attacker:f,round:x,log:y}),_==="finished"&&await F(p)}async function K(){const f=p[r+"Team"],x=!["GK","DEF","MIL","ATT"].some(_=>(p[m+"Team"][_]||[]).some(z=>!z.used)),y=(p["selected_"+r]||[]).map(_=>{const z=(f[_._role]||[]).find(R=>R.cardId===_.cardId)||_,$=x&&["GK","DEF"].includes(_._role),A=f[_._role]||[],S=A.findIndex(R=>R.cardId===_.cardId),q=lt(A.length),G=S>=0?q[S]:z._col??1;return{...z,_line:_._role,_col:G,...$?{note_a:Math.max(1,Number(z.note_a)||0)}:{}}});if(!y.length)return;const h=Nt(y,p.modifiers[r]||{});ne(r,y.map(_=>_.cardId)),y.forEach(_=>{const z=(f[_._role]||[]).find($=>$.cardId===_.cardId);z&&(z.used=!0)}),p["selected_"+r]=[],ye();const v=[...p.log||[]];if(x){const _=(p[r+"Score"]||0)+1,z=y.map(G=>({name:G.name,note:je(G,G._line||"ATT"),portrait:Pe(G),job:G.job}));v.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:z,homeTotal:h.total,aiTotal:0});const $=(p.round||0)+1,A=r==="p1"?"p2":"p1",S={...p,[r+"Team"]:f,[r+"Score"]:_},q=ze(S);D.add($),_e(z,_,p[m+"Score"]||0,!0,async()=>{await re({[r+"Team"]:f,[r+"Score"]:_,["selected_"+r]:[],modifiers:{...p.modifiers,[r]:{}},pendingAttack:null,phase:q?"finished":A+"-attack",attacker:A,round:$,log:v}),q&&await F({...p,[r+"Score"]:_})});return}v.push({type:"pending",text:`⚔️ ${p[r+"Name"]} attaque (${h.total})`}),await re({[r+"Team"]:f,[m+"Team"]:p[m+"Team"],pendingAttack:{...h,players:y,side:r},["selected_"+r]:[],modifiers:{...p.modifiers,[r]:{}},phase:m+"-defense",log:v})}async function se(){const f=p[r+"Team"],x=(p["selected_"+r]||[]).map(V=>{const ie=(f[V._role]||[]).find(ke=>ke.cardId===V.cardId)||V,ae=f[V._role]||[],fe=ae.findIndex(ke=>ke.cardId===V.cardId),me=lt(ae.length),$e=fe>=0?me[fe]:ie._col??1;return{...ie,_line:V._role,_col:$e}}),y=Rt(x,p.modifiers[r]||{});ne(r,x.map(V=>V.cardId)),x.forEach(V=>{const ie=(f[V._role]||[]).find(ae=>ae.cardId===V.cardId);ie&&(ie.used=!0)}),p["selected_"+r]=[],ye();const h=Ot(p.pendingAttack.total,y.total,p.modifiers[r]||{}),v=p.pendingAttack.side,_=h==="attack"||(h==null?void 0:h.goal),z=v==="p1"?"p2":"p1",$=(p.round||0)+1,A=(p.pendingAttack.players||[]).map(V=>({name:V.name,note:je(V,V._line||"ATT"),portrait:Pe(V),job:V.job})),S=[...p.log||[]];S.push({type:"duel",isGoal:_,homeScored:_&&v===r,text:_?`⚽ BUT de ${p[v+"Name"]} ! (${p.pendingAttack.total} vs ${y.total})`:`✋ Attaque stoppée (${p.pendingAttack.total} vs ${y.total})`,homePlayers:A,aiPlayers:x.map(V=>({name:V.name,note:je(V,V._line||"DEF"),portrait:Pe(V),job:V.job})),homeTotal:p.pendingAttack.total,aiTotal:y.total});const q=_?(p[v+"Score"]||0)+1:p[v+"Score"]||0,G={...p,[r+"Team"]:f,[v+"Score"]:q},R=ze(G),O=R?"finished":z+"-attack",Q=async()=>{await re({[r+"Team"]:f,[m+"Team"]:p[m+"Team"],[v+"Score"]:q,["selected_"+r]:[],modifiers:{...p.modifiers,[r]:{}},pendingAttack:null,phase:O,attacker:z,round:$,log:S}),(O==="finished"||R)&&await F({...p,[v+"Score"]:q})};if(_){const V=v===r,ie=V?q:p[r+"Score"]||0,ae=V?p[m+"Score"]||0:q;D.add($),_e(A,ie,ae,V,Q)}else await Q()}function he(f){return["MIL","ATT"].some(x=>(f[x]||[]).some(y=>!y.used))}function Se(f){return["GK","DEF","MIL","ATT"].some(x=>(f[x]||[]).some(y=>!y.used))}function Me(f){return Se(f)&&!he(f)}function be(f){const x=p[f+"Team"],y=p[(f==="p1"?"p2":"p1")+"Team"];return!!(he(x)||!Se(y)&&Me(x))}function ze(f){const x=["MIL","ATT"].some(S=>(f.p1Team[S]||[]).some(q=>!q.used)),y=["MIL","ATT"].some(S=>(f.p2Team[S]||[]).some(q=>!q.used)),h=Se(f.p1Team),v=Se(f.p2Team);return!x&&!(!v&&h)&&(!y&&!(!h&&v))}function M(f){const x=f.p1Score||0,y=f.p2Score||0;return x===y?null:x>y?k.home_id:k.away_id}async function F(f){try{const x=M(f),y=x?k.home_id===x?k.away_id:k.home_id:null,h=f.p1Score||0,v=f.p2Score||0;await E.from("matches").update({status:"finished",winner_id:x,home_score:h,away_score:v}).eq("id",i);const{data:_}=await E.from("mini_league_matches").select("id, league_id, matchday").eq("match_id",i).single();if(_){await E.from("mini_league_matches").update({home_score:h,away_score:v,status:"finished"}).eq("id",_.id);const{data:z}=await E.from("mini_league_matches").select("id, status").eq("league_id",_.league_id).eq("matchday",_.matchday);if((z||[]).every(A=>A.status==="finished"||A.status==="bye")){const{data:A}=await E.from("mini_leagues").select("current_day,total_days").eq("id",_.league_id).single();if(A){const S=(A.current_day||0)+1,q=S>(A.total_days||0);await E.from("mini_leagues").update({current_day:q?A.total_days:S,status:q?"finished":"active"}).eq("id",_.league_id)}}}x&&y&&ki(x,y).catch(()=>{})}catch(x){console.error("[ML] finishMatch:",x)}}function w(){var v;if(I&&document.getElementById("pvp-end-overlay"))return;I=!0;const f=p[r+"Score"],x=p[m+"Score"],y=f>x,h=f===x;et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
    ...
    </div>`,(v=document.getElementById("pvp-home"))==null||v.addEventListener("click",()=>{try{E.removeChannel(de)}catch{}We(e),c("mini-league",o?{openLeagueId:o}:{})})}ye()}const rr="/",ar=[{emoji:"⚽",title:"Bienvenue dans Manager Wars !",color:"#1A6B3C",content:`<p>Tu es désormais un <strong>manager de football</strong> virtuel.</p>
    ...
    <p style="margin-top:12px;font-size:13px;color:#888">Tu peux revoir ce tutoriel depuis les paramètres à tout moment.</p>`}];function sr(e,t,i){let s=0;const l=document.createElement("div");l.id="tutorial-overlay",l.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.65);z-index:9900;display:flex;align-items:center;justify-content:center;padding:16px";const d=()=>{var r,m,b;const c=t[s],g=s===t.length-1,o=Math.round((s+1)/t.length*100);l.innerHTML=`
    ...
    `,l.querySelectorAll("ul,ol").forEach(u=>{u.style.paddingLeft="20px",u.style.marginTop="6px",u.style.marginBottom="6px"}),l.querySelectorAll("li").forEach(u=>{u.style.marginBottom="4px"}),l.querySelectorAll("p").forEach(u=>{u.style.marginBottom="8px"}),(r=l.querySelector("#tuto-prev"))==null||r.addEventListener("click",()=>{s--,d()}),(m=l.querySelector("#tuto-next"))==null||m.addEventListener("click",()=>{g?a():(s++,d())}),(b=l.querySelector("#tuto-skip"))==null||b.addEventListener("click",()=>{confirm("Passer le tutoriel ? Tu pourras le revoir plus tard depuis les paramètres.")&&a()})},a=async()=>{l.remove(),e!=null&&e.id&&await E.from("users").update({tutorial_done:!0}).eq("id",e.id),i==null||i()};document.body.appendChild(l),d()}async function dr(e,t,i){if(!e||e.tutorial_done)return;let s=[];const{data:l,error:d}=await E.rpc("get_tutorial_steps");if(!d&&(l==null?void 0:l.length)>0)s=l,console.log(`[Tutorial] RPC OK → ${s.length} étapes`);else{const{data:c,error:g}=await E.from("tutorial_steps").select("*").eq("is_active",!0).order("step_order");!g&&(c==null?void 0:c.length)>0?(s=c,console.log(`[Tutorial] Direct OK → ${s.length} étapes`)):(console.warn(`[Tutorial] Aucune étape DB (RPC: ${d==null?void 0:d.message}, Direct: ${g==null?void 0:g.message})`),i&&i("[Tutorial] DB vide ou inaccessible — tuto local utilisé","warning",5e3))}const a=s.length>0?s.map(c=>({emoji:c.emoji,title:c.title,color:c.color,content:c.content,image_url:c.image_url||null})):ar;sr(e,a,()=>t("boosters"))}const lr={GK:"#111111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function Tt(e,t,i=0){return t?(Number(t==="GK"?e.note_g:t==="DEF"?e.note_d:t==="MIL"?e.note_m:e.note_a)||0)+(t===e.job||t===e.job2?i:0):0}function Hi(e){return`<div style="width:6px;height:100%;background:${{normal:"#e0e0e0",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[e]||"#e0e0e0"};border-radius:4px 0 0 4px;flex-shrink:0;align-self:stretch;min-height:52px"></div>`}function Wt(e,t){const s=t?lr[t]||"#bbb":"#d0d0d0",l=e>0?e:t||"—";return`<div style="width:32px;height:32px;border-radius:6px;background:${s};display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:13px;font-weight:900;color:#fff;text-shadow:0 1px 2px rgba(0,0,0,0.4)">${l}</div>`}function Ui(e){const i=Zt(e);return i?`<div style="width:32px;height:32px;border-radius:6px;overflow:hidden;flex-shrink:0;display:flex;align-items:center;justify-content:center;background:#f0f0f0"><img src="${i}" style="width:100%;height:100%;object-fit:cover"></div>`:'<div style="width:32px;height:32px;border-radius:6px;background:#eee;display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:16px">🌍</div>'}function Ki(e){return e?`<div style="width:32px;height:32px;border-radius:6px;overflow:hidden;flex-shrink:0;display:flex;align-items:center;justify-content:center;background:#f0f0f0"><img src="${e}" style="width:28px;height:28px;object-fit:contain" onerror="this.parentElement.innerHTML='🏟️'"></div>`:'<div style="width:32px;height:32px;border-radius:6px;background:#f0f0f0;display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:16px">🏟️</div>'}async function cr(e,t){e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',await Ai(e,t)}async function Ai(e,t){const{state:i,toast:s}=t,{data:l}=await E.from("market_listings").select(`id, price, status, listed_at, seller_id,
      ...
      card:cards(id, card_type, current_note, evolution_bonus,
        player:players(id, firstname, surname_encoded, country_code, job, job2,
          ...
          clubs(encoded_name, logo_url)))`).eq("seller_id",i.profile.id).in("status",["active","sold"]).order("listed_at",{ascending:!1}).limit(100),a=(l||[]).filter(n=>n.seller_id!==i.profile.id),c=d||[];e.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    <!-- Header -->
    ...
    <div id="mkt-content" style="padding:10px 16px;display:flex;flex-direction:column;gap:8px"></div>
  </div>`;let g="buy";const o=()=>{var n,p,I,L,B,H,C;return{name:(((n=document.getElementById("flt-name"))==null?void 0:n.value)||"").toLowerCase().trim(),club:(((p=document.getElementById("flt-club"))==null?void 0:p.value)||"").toLowerCase().trim(),country:(((I=document.getElementById("flt-country"))==null?void 0:I.value)||"").toLowerCase().trim(),job:((L=document.getElementById("flt-job"))==null?void 0:L.value)||"",rarity:((B=document.getElementById("flt-rarity"))==null?void 0:B.value)||"",note1:parseInt((H=document.getElementById("flt-note1"))==null?void 0:H.value)||0,note2:parseInt((C=document.getElementById("flt-note2"))==null?void 0:C.value)||0}};function r(n){const p=o();return n.filter(I=>{var de,re,P;const L=(de=I.card)==null?void 0:de.player;if(!L)return!1;const B=`${L.firstname} ${L.surname_encoded}`.toLowerCase(),H=(((re=L.clubs)==null?void 0:re.encoded_name)||"").toLowerCase(),C=(L.country_code||"").toLowerCase(),D=((P=I.card)==null?void 0:P.evolution_bonus)||0,j=Tt(L,L.job,D),W=L.job2?Tt(L,L.job2,D):0;return!(p.name&&!B.includes(p.name)||p.club&&!H.includes(p.club)||p.country&&!C.includes(p.country)||p.job&&L.job!==p.job||p.rarity&&L.rarity!==p.rarity||p.note1&&j<p.note1||p.note2&&W<p.note2)})}function m(n){var C,D,j,W;const p=(C=n.card)==null?void 0:C.player;if(!p)return"";const I=((D=n.card)==null?void 0:D.evolution_bonus)||0,L=Tt(p,p.job,I),B=p.job2?Tt(p,p.job2,I):0,H=(i.profile.credits||0)>=n.price;return`<div class="card-panel" style="display:flex;align-items:center;gap:8px;padding:10px 12px;overflow:hidden;padding-left:6px">
      ${Hi(p.rarity)}
      ${Wt(L,p.job)}
      ${Wt(B,p.job2||null)}
      ${Ui(p.country_code)}
      ...
      <div style="text-align:right;flex-shrink:0">
        ...
        <button class="btn btn-primary btn-sm" data-buy="${n.id}" ${H?"":"disabled"} style="margin-top:4px;font-size:11px;padding:4px 10px">${H?"Acheter":"Trop cher"}</button>
      </div>
    </div>`}function b(n){var C,D,j,W;const p=(C=n.card)==null?void 0:C.player;if(!p)return"";const I=((D=n.card)==null?void 0:D.evolution_bonus)||0,L=Tt(p,p.job,I),B=p.job2?Tt(p,p.job2,I):0,H=n.status==="sold";return`<div class="card-panel" style="display:flex;align-items:center;gap:8px;padding:10px 12px;overflow:hidden;padding-left:6px;${H?"opacity:0.7":""}">
      ${Hi(p.rarity)}
      ${Wt(L,p.job)}
      ${Wt(B,p.job2||null)}
      ${Ui(p.country_code)}
      ...
      <div style="flex:1;min-width:0">
        ...
        <div style="font-size:10px;color:${H?"#22c55e":"#999"};margin-top:1px">
          ${H?`✅ Vendu à ${((W=n.buyer)==null?void 0:W.pseudo)||"—"} · ${n.sold_at?new Date(n.sold_at).toLocaleDateString("fr"):""}`:`🟢 En vente depuis le ${new Date(n.listed_at).toLocaleDateString("fr")}`}
        </div>
      </div>
      <div style="text-align:right;flex-shrink:0">
        <div style="font-size:14px;font-weight:900;color:#D4A017">${n.price.toLocaleString("fr")}</div>
        ${H?'<span style="font-size:10px;font-weight:700;color:#fff;background:#22c55e;padding:3px 8px;border-radius:10px;display:inline-block;margin-top:4px">VENDU</span>':`<button class="btn btn-danger btn-sm" data-cancel="${n.id}" style="margin-top:4px;font-size:11px;padding:4px 10px">Retirer</button>`}
      </div>
    </div>`}function u(){const n=document.getElementById("mkt-content"),p=document.getElementById("mkt-filters");if(n){if(p.style.display=g==="buy"?"flex":"none",g==="buy"){const I=r(a);n.innerHTML=I.length?I.map(m).join(""):'<div style="text-align:center;color:#aaa;padding:40px">Aucune carte trouvée.</div>'}else{const I=c.filter(B=>B.status==="active").sort((B,H)=>new Date(H.listed_at)-new Date(B.listed_at)),L=c.filter(B=>B.status==="sold").sort((B,H)=>new Date(H.sold_at||H.listed_at)-new Date(B.sold_at||B.listed_at));n.innerHTML=(I.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:4px 0 6px;text-transform:uppercase;letter-spacing:1px">🟢 En vente (${I.length})</div>`+I.map(b).join(""):"")+(L.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:12px 0 6px;text-transform:uppercase;letter-spacing:1px">✅ Ventes réussies (${L.length})</div>`+L.map(b).join(""):"")+(!I.length&&!L.length?'<div style="text-align:center;color:#aaa;padding:40px">Aucune vente pour le moment.</div>':"")}n.querySelectorAll("[data-buy]").forEach(I=>I.addEventListener("click",()=>pr(I.dataset.buy,a,e,t))),n.querySelectorAll("[data-cancel]").forEach(I=>I.addEventListener("click",()=>fr(I.dataset.cancel,e,t)))}}e.querySelectorAll(".mkt-tab").forEach(n=>{n.addEventListener("click",()=>{g=n.dataset.tab,e.querySelectorAll(".mkt-tab").forEach(p=>{const I=p===n;p.style.background=I?"var(--green)":"#fff",p.style.color=I?"#fff":"var(--gray-600)",p.style.borderColor=I?"var(--green)":"var(--gray-200)"}),u()})});let k;["flt-name","flt-club","flt-country","flt-job","flt-rarity","flt-note1","flt-note2"].forEach(n=>{var p;(p=document.getElementById(n))==null||p.addEventListener("input",()=>{clearTimeout(k),k=setTimeout(u,250)})}),u()}async function pr(e,t,i,s){const{state:l,toast:d,refreshProfile:a}=s,c=t.find(r=>r.id===e);if(!c)return;const g=c.price;if((l.profile.credits||0)<g){d("Crédits insuffisants","error");return}ur(c,async()=>{const{error:r}=await E.rpc("buy_market_card",{p_listing_id:e,p_buyer_id:l.profile.id});if(r){d("Erreur achat : "+r.message,"error");return}await a();const m=document.getElementById("nav-credits");m&&(m.textContent=`💰 ${(l.profile.credits||0).toLocaleString("fr")}`),d("✅ Carte achetée !","success"),await Ai(i,s)})}function ur(e,t){var a;const i=(a=e.card)==null?void 0:a.player,s=i?`${i.firstname} ${i.surname_encoded}`:"cette carte",l=document.createElement("div");l.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",l.innerHTML=`
  ...
  `,document.querySelectorAll(".bottom-nav a").forEach(l=>{l.addEventListener("click",d=>{d.preventDefault(),Ft(l.dataset.page)})}),document.getElementById("nav-logo").addEventListener("click",()=>Ft("home")),document.getElementById("nav-credits").addEventListener("click",()=>Ft("boosters")),(s=document.getElementById("journal-btn"))==null||s.addEventListener("click",()=>br())}async function br(){const{data:e}=await E.from("patch_notes").select("*").eq("is_published",!0).order("published_at",{ascending:!1}).limit(20),t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9000;display:flex;align-items:center;justify-content:center;padding:16px";const i=(e||[]).map(s=>{const l=new Date(s.published_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"long",year:"numeric"});return`<div style="padding:14px 0;border-bottom:1px solid #f0f0f0">
    ...
    </div>`,document.body.appendChild(t),t.querySelector("#journal-close").addEventListener("click",()=>t.remove()),t.addEventListener("click",s=>{s.target===t&&t.remove()})}async function hr(){document.documentElement.setAttribute("data-theme","light"),document.getElementById("modal-overlay").addEventListener("click",i=>{i.target===i.currentTarget&&bi()}),document.getElementById("modal-close").addEventListener("click",bi);const{data:{session:e}}=await E.auth.getSession();if(!e){Vi(),Li(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:At});return}Ge.user=e.user,await Ct(),Vi();try{const{data:i}=await E.from("formation_links_overrides").select("formation, links"),s={};(i||[]).forEach(l=>{s[l.formation]=l.links}),zo(s)}catch(i){console.warn("Impossible de charger les overrides de formation:",i)}if(!Ge.profile){Io(document.getElementById("app"),{state:Ge,navigate:async()=>{await Ct(),ci()},toast:At,refreshProfile:Ct});return}const t=Array.isArray(Ge.profile.pending_boosters)?Ge.profile.pending_boosters:[];if(!Ge.profile.onboarding_done&&t.length>0){hn(document.getElementById("app"),{state:Ge,navigate:()=>ci(),toast:At,refreshProfile:Ct});return}ci(),setTimeout(()=>dr(Ge.profile,Ft,At),800),E.auth.onAuthStateChange(async(i,s)=>{i==="SIGNED_OUT"&&(Ge.user=null,Ge.profile=null,document.getElementById("app").innerHTML="",Li(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:At}))})}function vr(){return Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight)}function si(){const e=document.getElementById("app");e&&(e.style.height=vr()+"px")}window.addEventListener("resize",si);window.addEventListener("orientationchange",()=>setTimeout(si,150));window.visualViewport&&window.visualViewport.addEventListener("resize",si);function ci(){const e=()=>{var i;(i=Ge.user)!=null&&i.id&&E.from("users").update({last_seen_at:new Date().toISOString()}).eq("id",Ge.user.id).then(()=>{})};e(),window._presencePingInterval&&clearInterval(window._presencePingInterval),window._presencePingInterval=setInterval(e,6e4);const t=document.getElementById("app");t.style.display="flex",t.style.flexDirection="column",si(),yr(),Eo()}function Vi(){const e=document.getElementById("app-loader"),t=document.getElementById("app");t&&(t.style.display=""),e&&(e.classList.add("zoom-out"),setTimeout(()=>e.style.display="none",500))}function To(e){var s;const t=document.getElementById("app-loader");if(t&&(t.style.display="none"),document.getElementById("boot-error"))return;const i=document.createElement("div");i.id="boot-error",i.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:99999;gap:16px;color:#fff;padding:24px;text-align:center",i.innerHTML=`
