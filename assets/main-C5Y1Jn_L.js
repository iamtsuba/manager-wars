import{s as $,l as U,F as te,b as $e,c as me,d as ot}from"./formation-links-BXiJqmnj.js";function Ae(e,{navigate:t,toast:i}){e.innerHTML=`
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),l=(d||[]).filter(E=>E.card_type==="player"&&E.player),c=(d||[]).filter(E=>E.card_type==="game_changer"),u=(d||[]).filter(E=>E.card_type==="formation"),p=Object.keys(te),g=Object.keys(ce),y={};l.forEach(E=>{const A=E.player.id;y[A]=(y[A]||0)+1});const w=new Set(Object.keys(y).map(E=>String(E))),h=new Set(u.map(E=>E.formation)),k=new Set(c.map(E=>E.gc_type));let v="player",m="Tous",x="",f=!1;function b(){return[...l].sort((E,A)=>{const T=ne.indexOf(E.player.job),j=ne.indexOf(A.player.job);return T!==j?T-j:(E.player.surname_encoded||"").localeCompare(A.player.surname_encoded||"")})}function _(){return[...s||[]].sort((E,A)=>{const T=ne.indexOf(E.job),j=ne.indexOf(A.job);return T!==j?T-j:(E.surname_encoded||"").localeCompare(A.surname_encoded||"")})}function I(){return b().filter(E=>{const A=E.player,T=m==="Tous"||A.job===m,j=!x||`${A.firstname} ${A.surname_encoded}`.toLowerCase().includes(x);return T&&j})}function L(){return _().filter(E=>{const A=m==="Tous"||E.job===m,T=!x||`${E.firstname} ${E.surname_encoded}`.toLowerCase().includes(x);return A&&T})}e.innerHTML=`
  <div class="page">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Z=1e3;function ut(e,t,i,o){var w,h,k;const{state:r,toast:n,closeModal:a,navigate:d,refreshProfile:s}=i,l=e.formation,c={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function u(){const v=$e[l]||{},m=te[l]||[],x=290,f=360,b=20;function _(L){const M=v[L];return M?{x:M.x*x,y:M.y*f}:null}let I=`<svg width="${x}" height="${f}" viewBox="0 0 ${x} ${f}" xmlns="http://www.w3.org/2000/svg">`;for(const[L,M]of m){const z=_(L),C=_(M);!z||!C||(I+=`<line x1="${z.x}" y1="${z.y}" x2="${C.x}" y2="${C.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const L of Object.keys(v)){const M=_(L);if(!M)continue;const z=L.replace(/\d+/,""),C=c[z]||"#555";I+=`<circle cx="${M.x}" cy="${M.y}" r="${b}" fill="${C}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,I+=`<text x="${M.x}" y="${M.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${z}</text>`}return I+="</svg>",I}const p=t.filter(v=>v.formation===l),g=p.length,y=!e.is_for_sale;o(`Formation ${l}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
  ...
  <div class="page">
    ...
    <div class="page-body">
      <button class="btn btn-primary" id="save-deck" style="width:100%" ${n<11?"disabled":""}>
        ...
        stroke="${L}" stroke-width="${w}" stroke-linecap="round" opacity="${z}"/>`}}for(const m of i){const x=h(m);if(!x)continue;const f=s[m],b=m.replace(/\d+/,""),I={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[b]||"#555",L=f?Number(b==="GK"?f.note_g:b==="DEF"?f.note_d:b==="MIL"?f.note_m:f.note_a)||0:null,M=f?(f.surname_encoded||"").slice(0,8).toUpperCase():"";if(f){const z=ke(f);z&&(k+=`
          <defs>
            <clipPath id="clip-${m}">
    ...
    <svg viewBox="0 0 ${p} ${g}" width="100%" style="display:block;max-width:380px;margin:0 auto">
      ${v}
      ${k}
    </svg>`,r.querySelectorAll(".deck-slot-hit").forEach(m=>{m.addEventListener("click",()=>xt(m.dataset.pos,t,e,o))})}function xt(e,t,i,o){var p,g,y;const{openModal:r,closeModal:n}=o,a=e.replace(/\d+/,""),d=t.slots[e],s=d?t.playerCards.find(w=>w.id===d):null;(p=s==null?void 0:s.player)==null||p.id;const l=new Set;Object.entries(t.slots).forEach(([w,h])=>{var v;if(w===e||!h)return;const k=t.playerCards.find(m=>m.id===h);(v=k==null?void 0:k.player)!=null&&v.id&&l.add(k.player.id)}),t.subs.forEach(w=>{var k;const h=t.playerCards.find(v=>v.id===w);(k=h==null?void 0:h.player)!=null&&k.id&&l.add(h.player.id)});const c=new Set,u=t.playerCards.filter(w=>{const h=w.player;return!(h.job===a||h.job2===a)||l.has(h.id)||c.has(h.id)?!1:(c.add(h.id),!0)});u.sort((w,h)=>{const k=a==="GK"?w.player.note_g:a==="DEF"?w.player.note_d:a==="MIL"?w.player.note_m:w.player.note_a;return(a==="GK"?h.player.note_g:a==="DEF"?h.player.note_d:a==="MIL"?h.player.note_m:h.player.note_a)-k}),r(`Choisir ${a} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${u.length>0?u.map(w=>{var x,f;const h=w.player,k=a==="GK"?h.note_g:a==="DEF"?h.note_d:a==="MIL"?h.note_m:h.note_a,v=ke(h),m={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[h.rarity];return`<div class="player-option" data-card-id="${w.id}"
          ...
          <div style="flex:1;min-width:0">
            ...
            <div style="display:flex;align-items:center;gap:6px;margin-top:3px">
              ...
              ${(x=h.clubs)!=null&&x.logo_url?`<img src="${h.clubs.logo_url}" style="width:18px;height:18px;object-fit:contain">`:`<span style="font-size:10px;color:var(--gray-600)">${((f=h.clubs)==null?void 0:f.encoded_name)||"—"}</span>`}
  ...
  `,document.body.appendChild(o);let r=!1;document.getElementById("pack-visual").addEventListener("click",()=>{if(r)return;r=!0;const p=document.getElementById("pack-visual");p.classList.add("shaking"),setTimeout(()=>{p.classList.add("pack-open"),setTimeout(()=>{document.getElementById("pack-phase").style.display="none",a(0)},600)},500)});let n=0;function a(p){n=p;const g=document.getElementById("reveal-phase");g.style.display="flex",d(p)}function d(p){var f;const g=e[p],y=document.getElementById("card-counter"),w=document.getElementById("single-card-slot"),h=document.getElementById("card-tap-hint");y&&(y.textContent=`Carte ${p+1} / ${e.length}`),h&&(h.textContent=p<e.length-1?"Appuie pour continuer →":"Appuie pour voir toutes tes cartes");const k=je(g),v=g.card_type==="player"&&((f=g.player)==null?void 0:f.rarity)==="legende";w.innerHTML=`
      ...
      </div>`,v&&c();const m=document.getElementById("current-card-wrap");let x=!1;m.addEventListener("click",()=>{if(x)return;x=!0;const b=p+1;b<e.length?(m.style.transition="all 0.25s ease",m.style.transform="translateX(-120%) rotate(-15deg)",m.style.opacity="0",setTimeout(()=>d(b),250)):s()})}function s(){u(),document.getElementById("reveal-phase").style.display="none";const p=document.getElementById("recap-phase");p.style.display="flex";const g=document.getElementById("recap-grid");g.innerHTML=e.map((y,w)=>`
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}function zt(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:#000;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:16px";let i=5;t.innerHTML=`<div style="font-size:48px">📺</div>
    ...
    <div style="text-align:center;padding:40px;color:#fff">
      ...
      <button class="btn btn-primary" id="msg-btn">${o}</button>
        ...
        card:cards(id, card_type, formation,
          player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
            note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
            clubs(encoded_name,logo_url)))`).eq("deck_id",l).order("slot_order")]),p=(u||[]).filter(b=>{var _;return b.is_starter&&((_=b.card)==null?void 0:_.player)}).map(b=>ye(b.card)),g=(u||[]).filter(b=>{var _;return!b.is_starter&&((_=b.card)==null?void 0:_.player)}).map(b=>ye(b.card));if(p.length<11){Ve(e,"⚠️",`Deck incomplet (${p.length}/11).`,"Compléter",()=>o("decks"));return}const y=(u||[]).find(b=>{var _;return((_=b.card)==null?void 0:_.card_type)==="formation"}),w=(c==null?void 0:c.formation)||((f=y==null?void 0:y.card)==null?void 0:f.formation)||"4-4-2",{data:h}=await $.from("cards").select("id,gc_type").eq("owner_id",i.profile.id).eq("card_type","game_changer"),k=Ye(p,w),v=await Bt(w),{data:m}=await $.from("matches").insert({home_id:i.profile.id,away_id:null,mode:s,home_deck_id:l,status:"in_progress"}).select().single(),x={matchId:m==null?void 0:m.id,mode:s,difficulty:d,formation:w,homeTeam:k,aiTeam:v,homeSubs:g,subsUsed:0,maxSubs:Math.min(g.length,3),homeScore:0,aiScore:0,gcCards:h||[],usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Dt(e,x,t)}async function Ft(e,t,i){const{state:o,navigate:r}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:n}=await $.from("decks").select("id,name,is_active,formation").eq("owner_id",o.profile.id).order("created_at",{ascending:!1});if(!n||n.length===0){Ve(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>r("decks"));return}const a=n.map(c=>c.id),{data:d}=await $.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order");let s=0;function l(){var v,m,x,f,b;const c=n[s],u=(d||[]).filter(_=>_.deck_id===c.id),p=u.filter(_=>{var I;return _.is_starter&&((I=_.card)==null?void 0:I.player)}).map(_=>ye(_.card)),g=u.find(_=>{var I;return((I=_.card)==null?void 0:I.card_type)==="formation"}),y=c.formation||((v=g==null?void 0:g.card)==null?void 0:v.formation)||"4-4-2",w=p.length>=11?Ye(p,y):null,h=p.length>=11;e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:calc(100dvh - 130px);overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${u.map((y,w)=>{const h=G(y),k=w<u.length-1?U(y,u[w+1]):null,v=k&&k!=="#ff3333"&&k!=="#cc2222";return`
  ...
  `;function d(s){const l=document.getElementById("mkt-content"),c=s==="buy"?a:n;if(c.length===0){l.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      ${c.map(u=>{var h,k,v;const p=(h=u.card)==null?void 0:h.player;if(!p)return"";const g=p.job==="GK"?p.note_g:p.job==="DEF"?p.note_d:p.job==="MIL"?p.note_m:p.note_a,y=Vt[p.rarity],w=(i.profile.credits||0)>=u.price;return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px">
