import{s as k,F as it,b as Ce,c as Gt,l as ge,d as di}from"./formation-links-Bt44rMnM.js";function kt(e,{navigate:t,toast:i}){e.innerHTML=`
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),n=(d||[]).filter($=>$.card_type==="player"&&$.player),r=(d||[]).filter($=>$.card_type==="game_changer"),u=(d||[]).filter($=>$.card_type==="formation"),{data:m}=await k.from("gc_definitions").select("name,gc_type,color,effect,image_url"),f={};(m||[]).forEach($=>{f[$.name]=$});const _=Object.keys(it),p=Object.keys(de),y={};n.forEach($=>{const b=$.player.id;y[b]=(y[b]||0)+1}),new Set(Object.keys(y).map($=>String($)));const E=new Set(u.map($=>$.formation)),h=new Set(r.map($=>$.gc_type));let g="player",v="Tous",M="",I=!1;function x(){return[...n].sort(($,b)=>{const j=Be.indexOf($.player.job),A=Be.indexOf(b.player.job);return j!==A?j-A:($.player.surname_encoded||"").localeCompare(b.player.surname_encoded||"")})}function T(){return[...l||[]].sort(($,b)=>{const j=Be.indexOf($.job),A=Be.indexOf(b.job);return j!==A?j-A:($.surname_encoded||"").localeCompare(b.surname_encoded||"")})}function G(){return x().filter($=>{const b=$.player,j=v==="Tous"||b.job===v,A=!M||`${b.firstname} ${b.surname_encoded}`.toLowerCase().includes(M);return j&&A})}function R(){return T().filter($=>{const b=v==="Tous"||$.job===v,j=!M||`${$.firstname} ${$.surname_encoded}`.toLowerCase().includes(M);return b&&j})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",b=>{M=b.target.value.toLowerCase(),H()}),e.querySelectorAll(".filter-btn").forEach(b=>{b.addEventListener("click",()=>{v=b.dataset.job,P(),H()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{I=!I,P(),H()})):($.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{I=!I,P(),H()})))}function H(){const $=document.getElementById("col-grid");$&&(g==="player"?B($):g==="formation"?N($):S($))}function W($,b,j,A,w){w=w||"#7a28b8";const D=document.getElementById("col-grid"),L=document.getElementById("col-big");if(!D||!L)return;var q=0;function F(){L.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+b($[q])+"</div>";var U=L.querySelector("[data-card-id],[data-form-id],[data-gc-id]");U&&U.addEventListener("click",function(){A($[q])}),requestAnimationFrame(function(){var K=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!K||!L)){var X=L.clientHeight-8,te=L.clientWidth-24,re=K.offsetHeight,ae=K.offsetWidth;if(re>0&&ae>0&&X>40){var se=Math.min(X/re,te/ae,1);K.style.transform="scale("+se.toFixed(3)+")",K.style.transformOrigin="top center"}}}),D.innerHTML=$.map(function(K,X){return'<div class="col-mini-item" data-idx="'+X+'" style="flex-shrink:0;cursor:pointer;border-radius:8px;border:1.5px solid '+(X===q?w:"transparent")+';transition:border-color .15s;overflow:hidden">'+j(K,X===q)+"</div>"}).join(""),D.querySelectorAll(".col-mini-item").forEach(function(K){K.addEventListener("click",function(){q=Number(K.dataset.idx),F(),K.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}F()}function V($){var b=.54,j=Math.round(140*b),A=Math.round(310*b),w;if(!$||$._fake){var D=$?$.player:null;if(!D)return"";w=Tt(D)}else w=Ye($,"");return'<div style="width:'+j+"px;height:"+A+'px;overflow:hidden;position:relative;flex-shrink:0"><div style="transform:scale('+b+');transform-origin:top left;position:absolute;top:0;left:0;pointer-events:none">'+w+"</div></div>"}function ee($,b,j){b=b||100,j=j||140;var A=Ce[$]||{},w={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},D=Math.max(3,Math.round(b*.06)),L=Object.entries(A).map(function(F){var U=F[0],K=F[1],X=U.replace(/\d+$/,""),te=w[X]||"#888",re=Math.round(K.x*b),ae=Math.round(K.y*j);return'<circle cx="'+re+'" cy="'+ae+'" r="'+D+'" fill="'+te+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),q=Math.max(1,Math.round(b/50));return'<svg viewBox="0 0 '+b+" "+j+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+b+'" height="'+j+'" fill="#1A6B3C"/><rect x="'+Math.round(b*.2)+'" y="'+Math.round(j*.02)+'" width="'+Math.round(b*.6)+'" height="'+Math.round(j*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+q+'"/><line x1="0" y1="'+Math.round(j*.5)+'" x2="'+b+'" y2="'+Math.round(j*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+q+'"/><ellipse cx="'+Math.round(b*.5)+'" cy="'+Math.round(j*.5)+'" rx="'+Math.round(b*.18)+'" ry="'+Math.round(j*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+q+'"/><rect x="'+Math.round(b*.2)+'" y="'+Math.round(j*.82)+'" width="'+Math.round(b*.6)+'" height="'+Math.round(j*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+q+'"/>'+L+"</svg>"}function C($,b,j){var A=j>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+j+"</div>":"",w=b?'data-form-id="'+b.id+'"':"",D=$.length>7?14:$.length>5?16:19,L=!!b;return"<div "+w+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(L?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(L?"":"filter:grayscale(1);opacity:0.5")+'">'+A+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(L?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+D+"px;font-weight:900;color:"+(L?"#1A6B3C":"#aaa")+';line-height:1">'+$+'</div></div><div style="flex:1;overflow:hidden;background:'+(L?"#1A6B3C":"#ccc")+'">'+ee($,140,220)+"</div></div>"}function z($,b){var j=.54,A=Math.round(140*j),w=Math.round(305*j),D=Math.round(w*.22),L=w-D,q=$.length>7?5:7,F=ee($,A,L),U=b?"1.5px solid #2a7a40":"1px solid #ddd",K=b?"":"filter:grayscale(1);opacity:0.45;",X=b?"#1A6B3C":"#bbb",te="#fff";return'<div style="width:'+A+"px;height:"+w+"px;border-radius:6px;border:"+U+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+K+'"><div style="height:'+D+"px;background:"+X+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+q+"px;font-weight:900;color:"+te+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(A-4)+'px">'+$+'</span></div><div style="height:'+L+'px;overflow:hidden;flex-shrink:0">'+F+"</div></div>"}function B($){if(I){const b=R();if(!b.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const j=b.map(A=>n.find(w=>w.player.id===A.id)||{_fake:!0,player:A,id:"fake-"+A.id});W(j,A=>A._fake?Tt(A.player):Ye(A,""),A=>A._fake?V({player:A.player,id:"x",_fake:!0}):V(A),A=>{A._fake||Lt(A,n,y,t)},"#1A6B3C")}else{const b=G();if(!b.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const j={},A=[];b.forEach(w=>{j[w.player.id]||(j[w.player.id]=!0,A.push(w))}),W(A,w=>{const D=y[w.player.id]||1,L=D>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${D}</div>`:"",F=n.filter(U=>U.player.id===w.player.id&&U.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Ye(w,L+F)},w=>V(w),w=>Lt(w,n,y,t),"#1A6B3C")}}function N($){const b=I?_:[...E];if(!b.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const j=b.map(A=>({formation:A,card:u.find(w=>w.formation===A)||null,owned:E.has(A)}));W(j,({formation:A,card:w,owned:D})=>C(A,D?w:null,D?u.filter(L=>L.formation===A).length:0),({formation:A,owned:w})=>z(A,w),({card:A,owned:w})=>{w&&A&&hi(A,u,t,o)},"#1A6B3C")}function S($){const b=Object.keys(f),j=I?b.length?b:p:[...h];if(!j.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const A=j.map(w=>({type:w,gc:de[w]||{icon:"⚡",desc:""},def:f[w]||null,owned:h.has(w),card:r.find(D=>D.gc_type===w)||null}));W(A,({type:w,gc:D,def:L,owned:q,card:F})=>{const U=q?r.filter(Se=>Se.gc_type===w).length:0,K=U>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${U}</div>`:"",X=(L==null?void 0:L.gc_type)==="ultra_game_changer",te={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},re={purple:"#b06ce0",light_blue:"#00d4ef"},ae=te[L==null?void 0:L.color]||te.purple,se=re[L==null?void 0:L.color]||re.purple,me=(L==null?void 0:L.effect)||D.desc||"",xe=L!=null&&L.image_url?`/manager-wars/icons/${L.image_url}`:null;return q&&F?`<div data-gc-id="${F.id}" data-gc-type="${w}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${se};background:${ae};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${se}66;cursor:pointer">
          ${K}
        ...
        </div>`},({type:w,gc:D,def:L,owned:q})=>q?(()=>{const F=Math.round(75.60000000000001),U=Math.round(310*.54),K=Math.round(U*.65),X=Math.round(U*.18),te={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},re={purple:"#9b59b6",light_blue:"#00bcd4"},ae=te[L==null?void 0:L.color]||te.purple,se=re[L==null?void 0:L.color]||re.purple,me=L!=null&&L.image_url?`/manager-wars/icons/${L.image_url}`:null;return`<div style="width:${F}px;height:${U}px;border-radius:8px;background:${ae};border:1px solid ${se};display:flex;flex-direction:column;overflow:hidden"><div style="height:${X}px;background:rgba(255,255,255,0.15);display:flex;align-items:center;justify-content:center"><span style="font-size:7px;font-weight:900;color:#fff;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${F-6}px">${w}</span></div><div style="height:${K}px;display:flex;align-items:center;justify-content:center">${me?`<img src="${me}" style="max-width:${F-8}px;max-height:${K-4}px;object-fit:contain">`:`<span style="font-size:24px">${D.icon}</span>`}</div><div style="flex:1;display:flex;align-items:center;justify-content:center"><span style="font-size:7px;color:rgba(255,255,255,0.7);text-align:center;padding:0 2px">${((L==null?void 0:L.effect)||D.desc||"").slice(0,20)}</span></div></div>`})():(()=>{const F=Math.round(75.60000000000001),U=Math.round(310*.54);return`<div style="width:${F}px;height:${U}px;border-radius:8px;background:#eee;border:1px solid #ddd;display:flex;flex-direction:column;align-items:center;justify-content:center;filter:grayscale(1);opacity:0.45"><span style="font-size:22px">${D.icon}</span><span style="font-size:7px;color:#aaa;margin-top:4px;text-align:center;padding:0 4px">${w}</span></div>`})(),({type:w,owned:D,def:L})=>{D&&yi(w,L,o)},"#7a28b8")}e.querySelectorAll(".col-tab-btn").forEach($=>{$.addEventListener("click",()=>{g=$.dataset.tab,v="Tous",M="",I=!1,e.querySelectorAll(".col-tab-btn").forEach(b=>{const j=b.dataset.tab===g;b.style.borderBottomColor=j?"var(--green)":"transparent",b.style.color=j?"var(--green)":"var(--gray-600)"}),P(),H()})}),P(),H()}function yi(e,t,i){const s=de[e]||{icon:"⚡",desc:"Effet spécial."},c=(t==null?void 0:t.gc_type)==="ultra_game_changer",o={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},a={purple:"#b06ce0",light_blue:"#00d4ef"},d=o[t==null?void 0:t.color]||o.purple,l=a[t==null?void 0:t.color]||a.purple,n=(t==null?void 0:t.name)||e,r=(t==null?void 0:t.effect)||s.desc,u=t!=null&&t.image_url?`/manager-wars/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Ee=1e3;function hi(e,t,i,s){var p,y,E;const{state:c,toast:o,closeModal:a,navigate:d,refreshProfile:l}=i,n=e.formation,r={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function u(){const h=Ce[n]||{},g=it[n]||[],v=290,M=360,I=20;function x(G){const R=h[G];return R?{x:R.x*v,y:R.y*M}:null}let T=`<svg width="${v}" height="${M}" viewBox="0 0 ${v} ${M}" xmlns="http://www.w3.org/2000/svg">`;for(const[G,R]of g){const P=x(G),H=x(R);!P||!H||(T+=`<line x1="${P.x}" y1="${P.y}" x2="${H.x}" y2="${H.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const G of Object.keys(h)){const R=x(G);if(!R)continue;const P=G.replace(/\d+/,""),H=r[P]||"#555";T+=`<circle cx="${R.x}" cy="${R.y}" r="${I}" fill="${H}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,T+=`<text x="${R.x}" y="${R.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${P}</text>`}return T+="</svg>",T}const m=t.filter(h=>h.formation===n),f=m.length,_=!e.is_for_sale;s(`Formation ${n}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(p=document.getElementById("direct-sell-form-btn"))==null||p.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${n} pour ${Ee.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const h=m.find(v=>!v.is_for_sale)||m[0];if(!h){o("Aucune carte à vendre","error");return}const{error:g}=await k.from("cards").delete().eq("id",h.id);if(g){o(g.message,"error");return}await k.from("users").update({credits:(c.profile.credits||0)+Ee}).eq("id",c.profile.id),await l(),o(`+${Ee.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),a(),d("collection")}),(y=document.getElementById("market-sell-form-btn"))==null||y.addEventListener("click",async()=>{const h=parseInt(document.getElementById("sell-price-form").value);if(!h||h<1){o("Prix invalide","error");return}await k.from("cards").update({is_for_sale:!0,sale_price:h}).eq("id",e.id),await k.from("market_listings").insert({seller_id:c.profile.id,card_id:e.id,price:h}),o("Carte mise en vente sur le marché !","success"),a(),d("collection")}),(E=document.getElementById("cancel-sell-form-btn"))==null||E.addEventListener("click",async()=>{await k.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await k.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),o("Annonce retirée","success"),a(),d("collection")})}async function Lt(e,t,i,s){var T,G,R,P,H,W;const{state:c,toast:o,openModal:a,closeModal:d,navigate:l,refreshProfile:n}=s,r=e.player,u=t.filter(V=>V.player.id===r.id),m=u.length,f=mi[r.rarity]||1e3,_=r.rarity!=="legende",p=Pt(),y=Te(r,r.job),E=r.job2?Te(r,r.job2):null,h=Re[r.job]||"#1A6B3C",g=r.job2?Re[r.job2]:null,v=Nt[r.rarity]||"#ccc",M=at[r.country_code]||r.country_code||"",{data:I}=await k.from("transfer_history").select("club_name, manager_name, source, price, transferred_at").eq("player_id",r.id).order("transferred_at",{ascending:!0}),x=I&&I.length?`
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
  ...
  </div>`,bi(e,t,c,i),document.getElementById("builder-back").addEventListener("click",()=>s("decks")),document.getElementById("formation-select").addEventListener("change",n=>{t.formation=n.target.value;const r=zt(t.formation),u={};r.forEach(m=>{t.slots[m]&&(u[m]=t.slots[m])}),t.slots=u,_e(e,t,i)}),document.getElementById("save-deck").addEventListener("click",()=>$i(t,i)),e.querySelectorAll("[data-remove-sub]").forEach(n=>{n.addEventListener("click",()=>{t.subs=t.subs.filter(r=>r!==n.dataset.removeSub),_e(e,t,i)})}),(l=document.getElementById("add-sub-btn"))==null||l.addEventListener("click",()=>{_i(t,e,i)})}function bi(e,t,i,s){const c=e.querySelector("#deck-field");if(!c)return;const o=Ce[t.formation]||{},a=Gt(t.formation)||[],d={};for(const E of i){const h=t.slots[E],g=h?t.playerCards.find(v=>v.id===h):null;d[E]=g?g.player:null}const l=300,n=300,r=48,u=64,m=13,f=16,_=38;function p(E){const h=o[E];return h?{x:h.x*l,y:h.y*n}:null}let y="";for(const[E,h]of a){const g=p(E),v=p(h);if(!g||!v)continue;const M=d[E]?{...d[E],club_id:d[E].club_id,country_code:d[E].country_code,rarity:d[E].rarity}:null,I=d[h]?{...d[h],club_id:d[h].club_id,country_code:d[h].country_code,rarity:d[h].rarity}:null,x=ge(M,I);x==="#ff3333"||x==="#cc2222"?y+=`<line x1="${g.x.toFixed(1)}" y1="${g.y.toFixed(1)}" x2="${v.x.toFixed(1)}" y2="${v.y.toFixed(1)}" stroke="${x}" stroke-width="2.5" stroke-linecap="round" opacity="0.4"/>`:(y+=`<line x1="${g.x.toFixed(1)}" y1="${g.y.toFixed(1)}" x2="${v.x.toFixed(1)}" y2="${v.y.toFixed(1)}" stroke="${x}" stroke-width="8" stroke-linecap="round" opacity="0.2"/>`,y+=`<line x1="${g.x.toFixed(1)}" y1="${g.y.toFixed(1)}" x2="${v.x.toFixed(1)}" y2="${v.y.toFixed(1)}" stroke="${x}" stroke-width="2.5" stroke-linecap="round" opacity="0.9"/>`)}for(const E of i){const h=p(E);if(!h)continue;const g=d[E],v=E.replace(/\d+/,""),M=ue[v]||"#555",I=(h.x-r/2).toFixed(1),x=(h.y-u/2).toFixed(1),T={normal:"#aaa",pépite:"#D4A017",papyte:"#222",légende:"#7a28b8"}[g==null?void 0:g.rarity]||"#aaa";if(g){const G=Ue(g),R=Ht(g),P=Ot(g.country_code),H=Number(v==="GK"?g.note_g:v==="DEF"?g.note_d:v==="MIL"?g.note_m:g.note_a)||0,W=u-m-f;y+=`<defs><clipPath id="dcp-${E}"><rect x="${I}" y="${(h.y-u/2+m).toFixed(1)}" width="${r}" height="${W}"/></clipPath></defs>`,y+=`<rect x="${I}" y="${x}" width="${r}" height="${u}" rx="5" fill="${M}"/>`,G&&(y+=`<image href="${G}" x="${I}" y="${(h.y-u/2+m).toFixed(1)}" width="${r}" height="${W}" clip-path="url(#dcp-${E})" preserveAspectRatio="xMidYMin slice"/>`),y+=`<rect x="${I}" y="${x}" width="${r}" height="${m}" fill="rgba(255,255,255,0.93)"/>`,y+=`<text x="${h.x.toFixed(1)}" y="${(h.y-u/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(g.surname_encoded||"").slice(0,9)}</text>`;const V=(h.y+u/2-f).toFixed(1);y+=`<rect x="${I}" y="${V}" width="${r}" height="${f}" fill="rgba(255,255,255,0.93)"/>`,P&&(y+=`<image href="${P}" x="${(h.x-21).toFixed(1)}" y="${(h.y+u/2-f+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`),y+=`<text x="${h.x.toFixed(1)}" y="${(h.y+u/2-f/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${H}</text>`,R&&(y+=`<image href="${R}" x="${(h.x+r/2-14).toFixed(1)}" y="${(h.y+u/2-f+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`),y+=`<rect x="${I}" y="${x}" width="${r}" height="${u}" rx="5" fill="none" stroke="${T}" stroke-width="2"/>`}else y+=`<rect x="${I}" y="${x}" width="${r}" height="${u}" rx="5" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="5,3"/>`,y+=`<text x="${h.x.toFixed(1)}" y="${h.y.toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="22" fill="rgba(255,255,255,0.35)">+</text>`,y+=`<text x="${h.x.toFixed(1)}" y="${(h.y+16).toFixed(1)}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.3)">${v}</text>`;y+=`<rect x="${I}" y="${x}" width="${r}" height="${u}" rx="5" fill="rgba(0,0,0,0.01)" class="deck-slot-hit" data-pos="${E}" style="cursor:pointer"/>`}c.innerHTML=`<svg viewBox="${-_} ${-_} ${l+_*2} ${n+_*2}" width="100%" style="display:block;max-width:440px;margin:0 auto">${y}</svg>`,c.querySelectorAll(".deck-slot-hit").forEach(E=>{E.addEventListener("click",()=>wi(E.dataset.pos,t,e,s))})}function wi(e,t,i,s){var m,f,_;const{openModal:c,closeModal:o}=s,a=e.replace(/\d+/,""),d=t.slots[e],l=d?t.playerCards.find(p=>p.id===d):null;(m=l==null?void 0:l.player)==null||m.id;const n=new Set;Object.entries(t.slots).forEach(([p,y])=>{var h;if(p===e||!y)return;const E=t.playerCards.find(g=>g.id===y);(h=E==null?void 0:E.player)!=null&&h.id&&n.add(E.player.id)}),t.subs.forEach(p=>{var E;const y=t.playerCards.find(h=>h.id===p);(E=y==null?void 0:y.player)!=null&&E.id&&n.add(y.player.id)});const r=new Set,u=t.playerCards.filter(p=>{const y=p.player;return!(y.job===a||y.job2===a)||n.has(y.id)||r.has(y.id)?!1:(r.add(y.id),!0)});u.sort((p,y)=>{const E=a==="GK"?p.player.note_g:a==="DEF"?p.player.note_d:a==="MIL"?p.player.note_m:p.player.note_a;return(a==="GK"?y.player.note_g:a==="DEF"?y.player.note_d:a==="MIL"?y.player.note_m:y.player.note_a)-E}),c(`Choisir ${a} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${u.length>0?u.map(p=>{var v,M;const y=p.player,E=a==="GK"?y.note_g:a==="DEF"?y.note_d:a==="MIL"?y.note_m:y.note_a,h=Ue(y),g={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[y.rarity];return`<div class="player-option" data-card-id="${p.id}"
          ...
          <div style="flex:1;min-width:0">
            ...
            <div style="display:flex;align-items:center;gap:6px;margin-top:3px">
              ...
              ${(v=y.clubs)!=null&&v.logo_url?`<img src="${y.clubs.logo_url}" style="width:18px;height:18px;object-fit:contain">`:`<span style="font-size:10px;color:var(--gray-600)">${((M=y.clubs)==null?void 0:M.encoded_name)||"—"}</span>`}
  ...
  `,document.body.appendChild(s);let c=!1;document.getElementById("pack-visual").addEventListener("click",()=>{if(c)return;c=!0;const f=document.getElementById("pack-visual");f.classList.add("shaking"),setTimeout(()=>{f.classList.add("pack-open"),setTimeout(()=>{document.getElementById("pack-phase").style.display="none",a(0)},600)},500)});let o=0;function a(f){o=f;const _=document.getElementById("reveal-phase");_.style.display="flex",d(f)}function d(f){var I;const _=e[f],p=document.getElementById("card-counter"),y=document.getElementById("single-card-slot"),E=document.getElementById("card-tap-hint");p&&(p.textContent=`Carte ${f+1} / ${e.length}`),E&&(E.textContent=f<e.length-1?"Appuie pour continuer →":"Appuie pour voir toutes tes cartes");const h=St(_),g=_.card_type==="player"&&((I=_.player)==null?void 0:I.rarity)==="legende";y.innerHTML=`
      <div id="current-card-wrap" class="single-card-reveal" style="cursor:pointer;display:flex;flex-direction:column;align-items:center;gap:6px;${g?"filter:drop-shadow(0 0 20px #7a28b8)":""}">
        ${h}
        ${_.isDuplicate?'<div style="font-size:11px;font-weight:700;color:#fff;background:#cc2222;border-radius:8px;padding:2px 10px">Doublon</div>':""}
      </div>`,g&&r();const v=document.getElementById("current-card-wrap");let M=!1;v.addEventListener("click",()=>{if(M)return;M=!0;const x=f+1;x<e.length?(v.style.transition="all 0.25s ease",v.style.transform="translateX(-120%) rotate(-15deg)",v.style.opacity="0",setTimeout(()=>d(x),250)):l()})}function l(){u(),document.getElementById("reveal-phase").style.display="none";const f=document.getElementById("recap-phase");f.style.display="flex";const _=document.getElementById("recap-grid");_.innerHTML=e.map((p,y)=>`
      ...
      <div style="flex:1;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06);margin:0">
        ${f?`<img src="${f}" style="max-width:120px;max-height:120px;width:auto;height:auto;object-fit:contain;border-radius:6px">`:`<span style="font-size:56px">${_}</span>`}
      </div>
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}const Ui="simulation",Vt="VOTRE_ZONE_ID";function Ki(){switch(Ui){case"propellerads":return Vi();case"adsense":return Yi();default:return st()}}function st(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:#111;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:14px";let i=5;t.innerHTML=`
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      <div style="flex:1;overflow-y:auto;display:flex;flex-wrap:wrap;gap:10px;justify-content:center;align-content:flex-start;padding:8px 16px 16px">
        ${o.map(_=>{const p=a.find(y=>y.gc_type===_.gc_type);return d(_,!!p)}).join("")}
      </div>
    ...
    </div>`,e.querySelectorAll(".gc-select-card").forEach(_=>{_.addEventListener("click",()=>{const p=_.dataset.id,y=o.find(h=>h.id===p);if(!y)return;const E=a.findIndex(h=>h.gc_type===y.gc_type);E>-1?a.splice(E,1):a.length<3&&a.push(y),n()})}),(u=e.querySelector("#gc-launch"))==null||u.addEventListener("click",()=>{r&&l(a)}),(m=e.querySelector("#gc-no-gc"))==null||m.addEventListener("click",()=>l([])),(f=e.querySelector("#gc-reset"))==null||f.addEventListener("click",()=>{a.length&&(a=[],n())})}n()}async function Wi(e,t,i){const{state:s,navigate:c}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await k.from("decks").select("id,name,is_active,formation").eq("owner_id",s.profile.id).order("created_at",{ascending:!1});if(!o||o.length===0){fe(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>c("decks"));return}const a=o.map(r=>r.id),{data:d}=await k.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order");let l=0;function n(){var h,g,v,M,I;const r=o[l],u=(d||[]).filter(x=>x.deck_id===r.id),m=u.filter(x=>{var T;return x.is_starter&&((T=x.card)==null?void 0:T.player)}).map(x=>Je(x.card,x.position)),f=u.find(x=>{var T;return((T=x.card)==null?void 0:T.card_type)==="formation"}),_=r.formation||((h=f==null?void 0:f.card)==null?void 0:h.formation)||"4-4-2",p=m.length>=11?Oe(m,_):null,y=m.length>=11;Wt(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div style="display:flex;align-items:center;gap:8px;padding:8px;flex-shrink:0">
        ...
        <div style="flex:1;text-align:center">
          ...
          <div style="font-size:11px;opacity:.6;margin-top:2px">${_} · ${m.length}/11 ${r.is_active?"· ⭐ Actif":""}</div>
        </div>
      ...
      <div id="deck-swipe-zone" style="flex:1;min-height:0;overflow:hidden;position:relative;touch-action:pan-y;display:flex;align-items:center;justify-content:center;padding:4px">
        ${p?`<div class="deck-preview-wrap" style="aspect-ratio:1/1;max-width:100%;max-height:100%;width:auto;height:100%;overflow:hidden">${Me(p,_,null,[],285,285)}</div>`:`<div style="display:flex;align-items:center;justify-content:center;height:100%;opacity:.4;flex-direction:column;gap:8px">
  ...
  </div>`}function Ie(e,t,i){if(!(e!=null&&e.length))return"";const s=e.slice(0,5);let c='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return s.forEach((o,a)=>{if(c+=pe(o,40,52),a<s.length-1){const d=ge(o,s[a+1]);c+=`<div style="width:7px;height:3px;background:${d==="#ff3333"||d==="#cc2222"?"rgba(255,255,255,0.12)":d};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(c+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),c+="</div>",c}function Ze(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function Zt(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function Le(e,t,i,s,c=310,o=310){const a=Ce[t]||{},d=Gt(t)||it[t]||[],l={},n=["ATT","MIL","DEF","GK"];for(const h of n)(e[h]||[]).forEach((v,M)=>{l[`${h}${M+1}`]=v});function r(h){const g=a[h];return g?{x:g.x*c,y:g.y*o}:null}let u="";for(const[h,g]of d){const v=r(h),M=r(g);if(!v||!M)continue;const I=l[h],x=l[g],T=ge(I,x);T==="#00ff88"||T==="#FFD700"?(u+=`<line x1="${v.x.toFixed(1)}" y1="${v.y.toFixed(1)}" x2="${M.x.toFixed(1)}" y2="${M.y.toFixed(1)}"
        stroke="${T}" stroke-width="10" stroke-linecap="round" opacity="0.22"/>`,u+=`<line x1="${v.x.toFixed(1)}" y1="${v.y.toFixed(1)}" x2="${M.x.toFixed(1)}" y2="${M.y.toFixed(1)}"
        stroke="${T}" stroke-width="3.5" stroke-linecap="round" opacity="0.95"/>`):u+=`<line x1="${v.x.toFixed(1)}" y1="${v.y.toFixed(1)}" x2="${M.x.toFixed(1)}" y2="${M.y.toFixed(1)}"
        stroke="${T}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const m=48,f=64,_=13,p=16,y={normal:"#aaaaaa",pépite:"#D4A017",papyte:"#111111",légende:"#7a28b8"};for(const[h,g]of Object.entries(l)){const v=r(h);if(!v||!g)continue;const M=h.replace(/[0-9]/g,""),I=ke[M]||"#555",x=i==="attack"&&["MIL","ATT"].includes(M)&&!g.used||i==="defense"&&["GK","DEF","MIL"].includes(M)&&!g.used,T=s.includes(g.cardId);let G;i==="attack"?G=M==="MIL"?Number(g.note_m)||0:Number(g.note_a)||0:i==="defense"?G=M==="GK"?Number(g.note_g)||0:M==="MIL"?Number(g.note_m)||0:Number(g.note_d)||0:G=Number(M==="GK"?g.note_g:M==="DEF"?g.note_d:M==="MIL"?g.note_m:g.note_a)||0,G=G+(g.boost||0);const R=(v.x-m/2).toFixed(1),P=(v.y-f/2).toFixed(1),H=g.used?.25:1,W=y[g==null?void 0:g.rarity]||y.normal,V=T?"#ff3030":W,ee=T?3:(g==null?void 0:g.rarity)==="légende"||(g==null?void 0:g.rarity)==="pépite"?2.5:2,C=f-_-p;u+=`<defs><clipPath id="cp-${h}"><rect x="${R}" y="${(v.y-f/2+_).toFixed(1)}" width="${m}" height="${C}"/></clipPath></defs>`,u+=`<rect x="${R}" y="${P}" width="${m}" height="${f}" rx="5" fill="${I}" opacity="${H}"/>`;const z=ne(g);z&&!g.used&&(u+=`<image href="${z}" x="${R}" y="${(v.y-f/2+_).toFixed(1)}" width="${m}" height="${C}" clip-path="url(#cp-${h})" preserveAspectRatio="xMidYMin slice"/>`),u+=`<rect x="${R}" y="${P}" width="${m}" height="${_}" rx="4" fill="rgba(255,255,255,0.92)"/>`,u+=`<text x="${v.x.toFixed(1)}" y="${(v.y-f/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(g.name||"").slice(0,9)}</text>`;const B=(v.y+f/2-p).toFixed(1);if(u+=`<rect x="${R}" y="${B}" width="${m}" height="${p}" fill="rgba(255,255,255,0.92)"/>`,g.used)u+=`<text x="${v.x.toFixed(1)}" y="${(v.y+f/2-p/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="13" font-weight="900" fill="rgba(0,0,0,0.4)" font-family="Arial Black">–</text>`;else{const N=Ze(g.country_code);N?u+=`<image href="${N}" x="${(v.x-20).toFixed(1)}" y="${(v.y+f/2-p+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:u+=`<text x="${(v.x-13).toFixed(1)}" y="${(v.y+f/2-p/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${Zt(g.country_code)}</text>`,u+=`<text x="${v.x.toFixed(1)}" y="${(v.y+f/2-p/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${G}</text>`;const S=Jt(g);S?u+=`<image href="${S}" x="${(v.x+m/2-14).toFixed(1)}" y="${(v.y+f/2-p+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:g.clubName&&(u+=`<text x="${(v.x+14).toFixed(1)}" y="${(v.y+f/2-p/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(g.clubName||"").slice(0,3).toUpperCase()}</text>`),g.boost&&(u+=`<rect x="${(v.x+m/2-12).toFixed(1)}" y="${(v.y-f/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,u+=`<text x="${(v.x+m/2-5).toFixed(1)}" y="${(v.y-f/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${g.boost}</text>`)}u+=`<rect x="${R}" y="${P}" width="${m}" height="${f}" rx="5" fill="${T?"rgba(255,255,255,0.12)":"none"}" stroke="${V}" stroke-width="${ee}" opacity="${H}"/>`,x&&(u+=`<rect x="${R}" y="${P}" width="${m}" height="${f}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${T?"selected":""}" data-card-id="${g.cardId}" data-role="${M}" style="cursor:pointer"/>`)}const E=38;return`<svg viewBox="${-E} ${-E} ${c+E*2} ${o+E*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${u}
  ...
  </div>`}async function Qt(e,t,i,s){var M;const{state:c,navigate:o,toast:a}=t;Wt(e);const d=c.params||{};if(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!d.deckId)return Wi(e,t,i);const l=d.deckId;let n,r,u,m;try{const I=await Promise.all([k.from("decks").select("formation,name").eq("id",l).single(),k.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation,
            player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
              clubs(encoded_name,logo_url)))`).eq("deck_id",l).order("slot_order")]);n=I[0].data,u=I[0].error,r=I[1].data,m=I[1].error}catch(I){console.error("[Match] Exception chargement deck:",I),fe(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>o("home"));return}if(u||m){console.error("[Match] Erreur Supabase:",u||m),fe(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>o("home"));return}const f=(r||[]).filter(I=>{var x;return I.is_starter&&((x=I.card)==null?void 0:x.player)}).map(I=>Je(I.card,I.position)),_=(r||[]).filter(I=>{var x;return!I.is_starter&&((x=I.card)==null?void 0:x.player)}).map(I=>Je(I.card,I.position));if(f.length<11){fe(e,"⚠️",`Deck incomplet (${f.length}/11).`,"Compléter",()=>o("decks"));return}const p=(r||[]).find(I=>{var x;return((x=I.card)==null?void 0:x.card_type)==="formation"}),y=(n==null?void 0:n.formation)||((M=p==null?void 0:p.card)==null?void 0:M.formation)||"4-4-2",{data:E,error:h}=await k.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",c.profile.id).eq("card_type","game_changer"),{data:g}=await k.from("gc_definitions").select("*").eq("is_active",!0),v=(E||[]).map(I=>({...I,_gcDef:(g==null?void 0:g.find(x=>x.name===I.gc_type||x.id===I.gc_definition_id))||null}));s({deckId:l,formation:y,starters:f,subsRaw:_,gcCardsEnriched:v,gcDefs:g||[]})}async function Xi(e,t){const{state:i}=t,c=(i.params||{}).matchMode||"vs_ai_easy",o=c.replace("vs_ai_",""),a=c;await Qt(e,t,c,async({deckId:d,formation:l,starters:n,subsRaw:r,gcCardsEnriched:u,gcDefs:m})=>{try{const f=Oe(n,l),_=await Ji(l,o),p=async y=>{try{const{data:E,error:h}=await k.from("matches").insert({home_id:i.profile.id,away_id:null,mode:a,home_deck_id:d,status:"in_progress"}).select().single();if(h){console.error("[MatchIA] Erreur création match:",h),fe(e,"⚠️","Impossible de créer le match ("+h.message+").","Retour",()=>t.navigate("home"));return}const g={gcDefs:m||[],matchId:E==null?void 0:E.id,mode:a,difficulty:o,formation:l,homeTeam:f,aiTeam:_,homeSubs:r,subsUsed:0,maxSubs:Math.min(r.length,3),homeScore:0,aiScore:0,gcCards:y,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Qi(e,g,t)}catch(E){console.error("[MatchIA] Exception launchMatch:",E),fe(e,"⚠️","Erreur au lancement du match : "+E.message,"Retour",()=>t.navigate("home"))}};if(!u.length){p([]);return}Xt(e,u,p)}catch(f){console.error("[MatchIA] Exception setup:",f),fe(e,"⚠️","Erreur de préparation du match : "+f.message,"Retour",()=>t.navigate("home"))}})}async function Ji(e,t){var a,d;const{data:i}=await k.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(60);if(!i||i.length<11)return Zi(e);const s=$e[e]||$e["4-4-2"],c={GK:[],DEF:[],MIL:[],ATT:[]},o=[...i];for(const l of["GK","DEF","MIL","ATT"]){const n=o.filter(_=>_.job===l),r=o.filter(_=>_.job!==l),u=[...n,...r],m=[];for(let _=0;_<s[l];_++){const p=u[_]||o[_];p&&m.push({cardId:"ai-"+p.id+"-"+_,id:p.id,firstname:p.firstname,name:p.surname_encoded,country_code:p.country_code,club_id:p.club_id,job:p.job,job2:p.job2,note_g:Number(p.note_g)||0,note_d:Number(p.note_d)||0,note_m:Number(p.note_m)||0,note_a:Number(p.note_a)||0,rarity:p.rarity,skin:p.skin,hair:p.hair,hair_length:p.hair_length,clubName:((a=p.clubs)==null?void 0:a.encoded_name)||null,clubLogo:((d=p.clubs)==null?void 0:d.logo_url)||null,boost:0,used:!1,_line:l})}const f=Pe(m.length);m.forEach((_,p)=>{_._col=f[p]}),c[l]=m}return c}function Zi(e){const t=$e[e]||$e["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},s=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let c=0;for(const o of["GK","DEF","MIL","ATT"]){const a=[];for(let l=0;l<t[o];l++){const n=3+Math.floor(Math.random()*5);a.push({cardId:"fake-"+c,id:"fake-"+c,firstname:"IA",name:s[c%s.length],country_code:"XX",club_id:null,job:o,job2:null,note_g:o==="GK"?n:2,note_d:o==="DEF"?n:2,note_m:o==="MIL"?n:2,note_a:o==="ATT"?n:2,rarity:"normal",boost:0,used:!1,_line:o}),c++}const d=Pe(a.length);a.forEach((l,n)=>{l._col=d[n]}),i[o]=a}return i}function Qi(e,t,i){e.innerHTML=`
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
  </div>`,setTimeout(()=>eo(e,t,i),5e3)}function eo(e,t,i){const s=t.homeTeam.MIL||[],c=t.aiTeam.MIL||[];function o(p){return p.reduce((y,E)=>y+ie(E,"MIL"),0)}function a(p){let y=0;for(let E=0;E<p.length-1;E++){const h=ge(p[E],p[E+1]);h==="#00ff88"?y+=2:h==="#FFD700"&&(y+=1)}return y}const d=o(s)+a(s),l=o(c)+a(c),n=d>=l;function r(p,y,E,h){return`<div id="duel-row-${h}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0), opacity .5s ease;transform-origin:center">
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${p.map((g,v)=>{const M=v<p.length-1?ge(g,p[v+1]):null,I=!M||M==="#ff3333"||M==="#cc2222",x=M==="#00ff88"?"+2":M==="#FFD700"?"+1":"";return`
          ...
          ${v<p.length-1?`<div class="duel-link duel-link-${h}" data-idx="${v}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${I?"rgba(255,255,255,0.12)":M};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${I?"none":`0 0 8px ${M}`}">
            ${x?`<span style="position:absolute;top:-13px;left:50%;transform:translateX(-50%);font-size:8px;font-weight:900;color:${M}">${x}</span>`:""}
          </div>`:""}
          `}).join("")}
      </div>
  ...
  <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div id="duel-finale" style="display:flex;flex-direction:column;align-items:center;gap:14px;margin-top:6px"></div>
  </div>`;const u=()=>{const p=(y,E)=>e.querySelectorAll(y).forEach((h,g)=>{setTimeout(()=>{h.style.opacity="1",h.style.transform="translateY(0) scale(1)"},E+g*90)});p(".duel-card-home",150),p(".duel-card-ai",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((y,E)=>{setTimeout(()=>{y.style.opacity="1"},E*70)}),900),setTimeout(()=>{const y=e.querySelector("#vs-label");y&&(y.style.opacity="1",y.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(E=>E.style.opacity="1")},1250),setTimeout(()=>{m("score-home",d,800),m("score-ai",l,800)},1500)};function m(p,y,E){const h=document.getElementById(p);if(!h)return;const g=performance.now(),v=M=>{const I=Math.min(1,(M-g)/E);h.textContent=Math.round(y*(1-Math.pow(1-I,3))),I<1?requestAnimationFrame(v):h.textContent=y};requestAnimationFrame(v)}requestAnimationFrame(u),t.attacker=n?"home":"ai";const f=n?Yt():null;n&&(t.boostCard={value:f}),t.log.push({type:"duel",title:"Milieu de Terrain",homePlayers:s.map(p=>({name:p.name,note:ie(p,"MIL"),portrait:ne(p),job:p.job,country_code:p.country_code,rarity:p.rarity,clubName:p.clubName,clubLogo:p.clubLogo})),aiPlayers:c.map(p=>({name:p.name,note:ie(p,"MIL"),portrait:ne(p),job:p.job,country_code:p.country_code,rarity:p.rarity,clubName:p.clubName,clubLogo:p.clubLogo})),homeTotal:d,aiTotal:l,text:`Duel milieu : ${t.clubName} ${d} – ${l} IA → ${n?t.clubName+" attaque":"IA attaque"}`});const _=()=>{t.phase=t.attacker==="home"?"attack":"ai-attack",oe(e,t,i),t.attacker==="ai"&&setTimeout(()=>Qe(e,t,i),800)};setTimeout(()=>{const p=document.getElementById("score-home"),y=document.getElementById("score-ai"),E=document.getElementById(n?"duel-row-home":"duel-row-ai"),h=document.getElementById(n?"duel-row-ai":"duel-row-home"),g=n?p:y,v=n?y:p;g&&(g.style.fontSize="80px",g.style.color=n?"#FFD700":"#ff6b6b",g.style.animation="duelPulse .5s ease"+(n?", duelGlow 1.5s ease infinite .5s":"")),v&&(v.style.opacity="0.25"),setTimeout(()=>{if(E&&h){const M=E.getBoundingClientRect(),I=h.getBoundingClientRect(),x=Math.round(I.top-M.top);E.style.setProperty("--crush-dist",x+"px"),E.style.animation="crushSlam .55s cubic-bezier(.4,0,.6,1) forwards",E.style.zIndex="5",setTimeout(()=>{const T=document.getElementById("duel-shock");T&&(T.style.top=I.top-E.getBoundingClientRect().top+"px",T.style.animation="shockwave .5s ease-out forwards"),h.style.animation="crushSquash .45s ease-in forwards",navigator.vibrate&&navigator.vibrate(60)},360)}setTimeout(()=>{var I;const M=document.getElementById("duel-finale");h&&(h.style.display="none"),p&&(p.parentElement.style.display="none"),M&&(M.innerHTML=`
            <div style="font-size:18px;font-weight:900;color:#fff;animation:fadeUp .4s ease both">
              ${n?`⚽ ${t.clubName} attaque !`:"😔 L'IA gagne l'engagement"}
            </div>
            ${n?`
            <div style="background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:3px solid #cdeffd;border-radius:16px;padding:18px 30px;text-align:center;animation:boostFlipIn .7s cubic-bezier(.34,1.56,.64,1) both;box-shadow:0 8px 30px rgba(135,206,235,0.4)">
              ...
              <div style="font-size:44px;font-weight:900;color:#063;line-height:1">+${f}</div>
            ...
            <button id="start-match-btn" style="margin-top:4px;padding:16px 44px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:17px;font-weight:900;cursor:pointer;box-shadow:0 4px 20px rgba(0,0,0,0.3);animation:fadeUp .4s ease both;animation-delay:.5s;opacity:0">
              ▶ Commencer le match
            </button>`,(I=document.getElementById("start-match-btn"))==null||I.addEventListener("click",_))},700)},700)},2800)}function to(e){if(e.type==="duel"){const t=e.homeTotal>=e.aiTotal;return`
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:100px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const T=t.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
            ...
            ${Ie((T.players||[]).map(G=>({...G,used:!1})),"#ff6b6b",T.total)}
            ...
            ${Ie((T.players||[]).map(G=>({...G,used:!1})),"#00ff88",T.total)}
          </div>`}const x=t.log[t.log.length-1];return x?'<div style="padding:2px 4px">'+to(x)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})()}
    </div>
    ...
    ${(()=>{const x=window.innerWidth>=700,T=(z,B,N)=>{var K,X;const S=(t.gcDefs||[]).find(te=>te.name===z.gc_type),$={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[S==null?void 0:S.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",b={purple:"#b06ce0",light_blue:"#00d4ef"}[S==null?void 0:S.color]||"#b06ce0",j=(S==null?void 0:S.name)||z.gc_type,A=(S==null?void 0:S.effect)||((K=de[z.gc_type])==null?void 0:K.desc)||"",w=S!=null&&S.image_url?`/manager-wars/icons/${S.image_url}`:null,D=((X=de[z.gc_type])==null?void 0:X.icon)||"⚡",L=Math.round(N*.22),q=Math.round(N*.22),F=N-L-q,U=j.length>12?7:9;return`<div class="gc-mini" data-gc-id="${z.id}" data-gc-type="${z.gc_type}"
          style="box-sizing:border-box;width:${B}px;height:${N}px;border-radius:10px;border:2px solid ${b};background:${$};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
        ...
        </div>`},G=(z,B)=>{var N;return`<div id="boost-card"
  ...
  `,document.body.appendChild(s);let r=!1;const u=()=>{r||(r=!0,s.remove(),i())};s.addEventListener("click",u),setTimeout(u,2e3)}function Ae(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function jt(e,t,i,s=null){if(t.phase!=="attack"){Ae("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){Ae(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const c=Object.values(t.homeTeam).flat().filter(u=>u.used),o=t.homeSubs.filter(u=>!t.usedSubIds.includes(u.cardId));if(!c.length){Ae("Aucun joueur utilisé à remplacer");return}if(!o.length){Ae("Aucun remplaçant disponible");return}let a=0,d=Math.max(0,o.findIndex(u=>u.cardId===s)),l=!1;const n=document.createElement("div");n.id="sub-overlay",n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function r(){var E,h,g,v,M,I;const u=c[a],m=o[d],f=Math.min(130,Math.round((window.innerWidth-90)/2)),_=Math.round(f*1.35),p=x=>`background:rgba(255,255,255,0.12);border:none;color:${x?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${x?"default":"pointer"};flex-shrink:0`;n.innerHTML=`
    ...
    <div style="flex:1;display:flex;gap:0;overflow:hidden">
      ...
      <div id="in-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
        ...
        <div>${m?pe({...m,used:!1,boost:0},f,_):"<div>—</div>"}</div>
      ...
      <div id="out-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
        ...
        <div>${u?pe({...u,used:!1,boost:0},f,_):"<div>—</div>"}</div>
    ...
    </div>`,(E=n.querySelector("#sub-close"))==null||E.addEventListener("click",()=>n.remove()),(h=n.querySelector("#out-up"))==null||h.addEventListener("click",()=>{a>0&&(a--,r())}),(g=n.querySelector("#out-down"))==null||g.addEventListener("click",()=>{a<c.length-1&&(a++,r())}),(v=n.querySelector("#in-up"))==null||v.addEventListener("click",()=>{d>0&&(d--,r())}),(M=n.querySelector("#in-down"))==null||M.addEventListener("click",()=>{d<o.length-1&&(d++,r())});const y=(x,T,G,R)=>{const P=n.querySelector("#"+x);if(!P)return;let H=0;P.addEventListener("touchstart",W=>{H=W.touches[0].clientY},{passive:!0}),P.addEventListener("touchend",W=>{const V=W.changedTouches[0].clientY-H;if(Math.abs(V)<30)return;const ee=T();V<0&&ee<R-1?(G(ee+1),r()):V>0&&ee>0&&(G(ee-1),r())},{passive:!0})};y("in-panel",()=>d,x=>d=x,o.length),y("out-panel",()=>a,x=>a=x,c.length),(I=n.querySelector("#sub-confirm"))==null||I.addEventListener("click",x=>{if(x.preventDefault(),x.stopPropagation(),l)return;l=!0;const T=c[a],G=o[d];if(!T||!G)return;let R=null,P=-1;for(const[W,V]of Object.entries(t.homeTeam)){const ee=(V||[]).findIndex(C=>C.cardId===T.cardId);if(ee!==-1){R=W,P=ee;break}}if(P===-1||!R){Ae("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),n.remove();return}const H={...G,_line:R,_col:T._col||0,used:!1,boost:0};t.homeTeam[R].splice(P,1,H),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(G.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:T.name,firstname:T.firstname,note:ie(T,R),portrait:ne(T),job:T.job,country_code:T.country_code,rarity:T.rarity,clubName:T.clubName,clubLogo:T.clubLogo},inPlayer:{name:G.name,firstname:G.firstname,note:ie(G,R),portrait:ne(G),job:G.job,country_code:G.country_code,rarity:G.rarity,clubName:G.clubName,clubLogo:G.clubLogo},text:`🔄 ${G.firstname} ${G.name} remplace ${T.firstname} ${T.name}`}),n.remove(),ao(T,G,()=>oe(e,t,i))})}document.body.appendChild(n),r()}function so(e,t,i,s,c){var _,p;const o=(s.gcDefs||[]).find(y=>y.name===t),a=de[t]||{icon:"⚡",desc:"Carte spéciale."},d={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[o==null?void 0:o.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",l={purple:"#b06ce0",light_blue:"#00d4ef"}[o==null?void 0:o.color]||"#b06ce0",n=(o==null?void 0:o.name)||t,r=(o==null?void 0:o.effect)||a.desc,u=o!=null&&o.image_url?`/manager-wars/icons/${o.image_url}`:null,m=a.icon||"⚡",f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",f.innerHTML=`
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(o);let d=!1;const l=()=>{d||(d=!0,o.remove(),setTimeout(()=>c(),50))};o.addEventListener("click",l),setTimeout(l,3500)}async function we(e,t,i){var r,u;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:s}=i,c=t.homeScore>t.aiScore,o=t.homeScore===t.aiScore,a=c?"victoire":o?"nul":"defaite",d=fi(t.mode,a);t.matchId&&await k.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:c?s.profile.id:null,home_credits_reward:d,played_at:new Date().toISOString()}).eq("id",t.matchId);const l={credits:(s.profile.credits||0)+d,matches_played:(s.profile.matches_played||0)+1};c?l.wins=(s.profile.wins||0)+1:o?l.draws=(s.profile.draws||0)+1:l.losses=(s.profile.losses||0)+1,await k.from("users").update(l).eq("id",s.profile.id),await i.refreshProfile();const n=document.createElement("div");n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",n.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const f=()=>document.getElementById("mm-status"),_=async(g=!0)=>{u=!0,m&&(k.removeChannel(m),m=null),g&&await k.rpc("cancel_matchmaking",{p_user_id:l.profile.id}).catch(()=>{})};(h=document.getElementById("mm-cancel"))==null||h.addEventListener("click",async()=>{try{await _(!0)}catch(g){console.warn("[Matchmaking] cleanup error:",g)}le(e),n("home")});const p=async(g,v,M)=>{u||(u=!0,m&&(k.removeChannel(m),m=null),f()&&(f().textContent="Adversaire trouvé !"),await new Promise(I=>setTimeout(I,600)),e.isConnected!==!1&&go(e,t,g,M,a,d))},{data:y,error:E}=await k.rpc("try_matchmake",{p_user_id:l.profile.id,p_deck_id:i});if(E||!(y!=null&&y.success)){r("Erreur de matchmaking","error"),le(e),n("home");return}if(y.matched){p(y.match_id,y.opponent_id,!1);return}f()&&(f().textContent="En attente d'un autre joueur..."),m=k.channel("matchmaking-"+l.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${l.profile.id}`},g=>{const v=g.new;v.status==="matched"&&v.match_id&&p(v.match_id,v.matched_with,!0)}).subscribe()}async function go(e,t,i,s,c=[],o=[]){const{state:a,navigate:d,toast:l}=t,n=s?"p1":"p2",r=s?"p2":"p1",u=(c||[]).map(C=>C.id),m=(c||[]).map(C=>({id:C.id,gc_type:C.gc_type,_gcDef:C._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation du match...</div>';const{data:f}=await k.from("matches").select("*").eq("id",i).single();if(!f){l("Match introuvable","error"),d("home");return}async function _(){const[{data:C,error:z},{data:B,error:N},{data:S},{data:$}]=await Promise.all([k.rpc("get_deck_for_match",{p_deck_id:f.home_deck_id}),k.rpc("get_deck_for_match",{p_deck_id:f.away_deck_id}),k.from("users").select("id,pseudo,club_name").eq("id",f.home_id).single(),k.from("users").select("id,pseudo,club_name").eq("id",f.away_id).single()]);z&&console.error("[PvP] get_deck_for_match p1 error:",z),N&&console.error("[PvP] get_deck_for_match p2 error:",N);function b(F,U){return{cardId:F.card_id,position:U||F.position,id:F.id,firstname:F.firstname,name:F.surname_encoded,country_code:F.country_code,club_id:F.club_id,job:F.job,job2:F.job2,note_g:Number(F.note_g)||0,note_d:Number(F.note_d)||0,note_m:Number(F.note_m)||0,note_a:Number(F.note_a)||0,rarity:F.rarity,skin:F.skin,hair:F.hair,hair_length:F.hair_length,clubName:F.club_encoded_name||null,clubLogo:F.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}}const j=((C==null?void 0:C.starters)||[]).map(F=>b(F,F.position)),A=((B==null?void 0:B.starters)||[]).map(F=>b(F,F.position)),w=((C==null?void 0:C.subs)||[]).map(F=>b(F,F.position)),D=((B==null?void 0:B.subs)||[]).map(F=>b(F,F.position)),L=(C==null?void 0:C.formation)||"4-4-2",q=(B==null?void 0:B.formation)||"4-4-2";return console.log("[PvP] p1Formation:",L,"p1 starters:",j.length,"positions:",j.map(F=>F.position)),console.log("[PvP] p2Formation:",q,"p2 starters:",A.length,"positions:",A.map(F=>F.position)),{p1Team:Oe(j,L),p2Team:Oe(A,q),p1Subs:w,p2Subs:D,p1Formation:L,p2Formation:q,p1Name:(S==null?void 0:S.club_name)||(S==null?void 0:S.pseudo)||"Joueur 1",p2Name:($==null?void 0:$.club_name)||($==null?void 0:$.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:s?u:[],gc_p2:s?[]:u,gcCardsFull_p1:s?m:[],gcCardsFull_p2:s?[]:m,gcReady_p1:!0,gcReady_p2:!0,usedGc_p1:[],usedGc_p2:[],lastActionAt:new Date().toISOString()}}let p=f.game_state&&Object.keys(f.game_state).length?f.game_state:null;if(!p)if(s){p=await _();const{data:C}=await k.from("matches").select("game_state").eq("id",i).single();!(C!=null&&C.game_state)||!Object.keys(C.game_state).length?await k.from("matches").update({game_state:p,turn_user_id:f.home_id}).eq("id",i):p=C.game_state}else{e.innerHTML=`<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation avec l'adversaire...</div>`;for(let C=0;C<30&&!p;C++){await new Promise(B=>setTimeout(B,400));const{data:z}=await k.from("matches").select("game_state").eq("id",i).single();z!=null&&z.game_state&&Object.keys(z.game_state).length&&(p=z.game_state)}if(!p){l("Erreur de synchronisation","error"),d("home");return}p.gc_p2=u,p.gcCardsFull_p2=m,await k.from("matches").update({game_state:p}).eq("id",i)}let y=!1;function E(C){var j;try{k.removeChannel(h)}catch{}const z=a.profile.id,B=C.winner_id===z,N=!!C.forfeit,S=p[n+"Score"]??0,$=p[r+"Score"]??0,b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center",b.innerHTML=`
      <div style="font-size:64px">${B?"🏆":"😞"}</div>
      <div style="font-size:26px;font-weight:900;color:${B?"#FFD700":"#ff6b6b"}">${B?"VICTOIRE !":"DÉFAITE"}</div>
      <div style="font-size:18px;color:#fff">${p[n+"Name"]} ${S} – ${$} ${p[r+"Name"]}</div>
      ${N?`<div style="font-size:13px;color:rgba(255,255,255,0.5)">${B?"L'adversaire a quitté la partie":"Match perdu par forfait"}</div>`:""}
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(b),(j=b.querySelector("#pvp-end-home"))==null||j.addEventListener("click",()=>{b.remove(),le(e),d("home")})}const h=k.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},C=>{const z=C.new;try{if(z.status==="finished"||z.forfeit){if(y)return;y=!0,p._timerInt&&(clearInterval(p._timerInt),p._timerInt=null),z.game_state&&(p=z.game_state),E(z);return}z.game_state&&(p=z.game_state,M())}catch(B){console.error("[PvP] Realtime render crash:",B,"gameState:",p)}}).subscribe();async function g(C){Object.assign(p,C),p.lastActionAt=new Date().toISOString();const{error:z}=await k.from("matches").update({game_state:p}).eq("id",i);z&&(console.error("[PvP] pushState DB error:",z.message,z),l("Erreur de synchronisation : "+z.message,"error"));try{M()}catch(B){console.error("[PvP] renderPvpScreen crash après pushState:",B),l("Erreur d'affichage : "+B.message,"error")}}async function v(){if(y)return;y=!0,p._timerInt&&(clearInterval(p._timerInt),p._timerInt=null);const C=s?f.away_id:f.home_id;try{await k.from("matches").update({status:"finished",forfeit:!0,winner_id:C}).eq("id",i)}catch(z){console.warn("[PvP] forfeit DB error:",z)}try{k.removeChannel(h)}catch{}le(e),d("home")}function M(){var gt,mt,xt,yt,ht;const C=p[n+"Team"];p[r+"Team"];const z=p[n+"Score"],B=p[r+"Score"],N=p[n+"Name"],S=p[r+"Name"];if(p.phase==="gc-select")return I();if(p.phase==="reveal")return x();if(p.phase==="midfield")return T();if(p.phase==="finished")return W();const $=p.phase===n+"-attack",b=p.phase===n+"-defense",j=p.phase===r+"-attack"||p.phase===r+"-defense",A=Array.isArray(p["selected_"+n])?p["selected_"+n]:[],w=A.map(O=>O.cardId);e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column";const D=window.innerWidth>=700,L=p[n+"Subs"]||[],q=p["usedSubIds_"+n]||[],F=L.filter(O=>!q.includes(O.cardId)),U=p["gcCardsFull_"+n]||[],K=p["usedGc_"+n]||[],X=U.filter(O=>!K.includes(O.id)),te=p.boostOwner===n&&!p.boostUsed,re=(O,J,Q)=>{var _t,$t;const Y=O._gcDef,ce={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[Y==null?void 0:Y.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",je={purple:"#b06ce0",light_blue:"#00d4ef"}[Y==null?void 0:Y.color]||"#b06ce0",ye=(Y==null?void 0:Y.name)||O.gc_type,ri=(Y==null?void 0:Y.effect)||((_t=de[O.gc_type])==null?void 0:_t.desc)||"",vt=Y!=null&&Y.image_url?`/manager-wars/icons/${Y.image_url}`:null,ai=(($t=de[O.gc_type])==null?void 0:$t.icon)||"⚡",bt=Math.round(Q*.22),wt=Math.round(Q*.22),Ve=Q-bt-wt,si=ye.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${O.id}" data-gc-type="${O.gc_type}"
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 14px;background:rgba(0,0,0,0.25);flex-shrink:0">
        ...
        <div style="flex:1;text-align:center;color:#fff;font-size:14px;font-weight:700">
          ${N} <span style="color:#FFD700;font-size:18px">${z}</span> – <span style="color:#FFD700;font-size:18px">${B}</span> ${S}
        </div>
      ...
      </div>`;const O=e.querySelector(".terrain-wrapper");O&&(O.innerHTML=Me(C,p[n+"Formation"],$?"attack":b?"defense":"idle",w,300,300))}if(e.querySelectorAll(".match-slot-hit").forEach(O=>{O.addEventListener("click",()=>{if(!$&&!b)return;const J=O.dataset.cardId,Q=O.dataset.role,Y=(C[Q]||[]).find(ye=>ye.cardId===J);if(!Y||Y.used)return;Array.isArray(p["selected_"+n])||(p["selected_"+n]=[]);const ce=p["selected_"+n],je=ce.findIndex(ye=>ye.cardId===J);je>-1?ce.splice(je,1):ce.length<3&&ce.push({...Y,_role:Q}),M()})}),(gt=e.querySelector("#pvp-boost-card"))==null||gt.addEventListener("click",()=>{G()}),e.querySelectorAll(".pvp-gc-mini").forEach(O=>{O.addEventListener("click",()=>{H(O.dataset.gcId,O.dataset.gcType)})}),(mt=e.querySelector("#pvp-action"))==null||mt.addEventListener("click",()=>{$?V():b&&ee()}),(xt=e.querySelector("#pvp-quit"))==null||xt.addEventListener("click",()=>{confirm("Quitter le match ? Vous perdrez par forfait.")&&v()}),(yt=e.querySelector("#pvp-view-opp"))==null||yt.addEventListener("click",()=>{P()}),(ht=e.querySelector("#pvp-toggle-history"))==null||ht.addEventListener("click",()=>{R()}),function(){const J=e.querySelector(".terrain-wrapper svg");J&&(J.removeAttribute("width"),J.removeAttribute("height"),J.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",J.setAttribute("viewBox","-26 -26 352 352"),J.setAttribute("preserveAspectRatio","xMidYMid meet"));const Q=e.querySelector(".match-screen");if(Q){const Y=e.clientHeight;Y>50&&(Q.style.height=Y+"px",Q.style.maxHeight=Y+"px",Q.style.overflow="hidden")}}(),p._timerInt&&(clearInterval(p._timerInt),p._timerInt=null),($||b)&&!y){let O=30,J=!1;const Q=()=>document.getElementById("pvp-timer"),Y=()=>{Q()&&(Q().textContent=O+"s",Q().style.color=J?"#ff4444":"#fff")};Y(),p._timerInt=setInterval(()=>{O--,O<0?J?(clearInterval(p._timerInt),p._timerInt=null,v()):(J=!0,O=15,Y()):Y()},1e3)}}function I(){var b;const C=p["gcReady_"+n],z=p["gcReady_"+r];Array.isArray(p["gc_"+n])||(p["gc_"+n]=[]);let B=p["gc_"+n];const N=3;function S(j,A){const w=j._gcDef,D={purple:"linear-gradient(135deg,#3d0a7a,#7a28b8)",light_blue:"linear-gradient(135deg,#006080,#00bcd4)"},L={purple:"#9b59b6",light_blue:"#00bcd4"},q=D[w==null?void 0:w.color]||D.purple,F=L[w==null?void 0:w.color]||L.purple;return`<div class="pvp-gc-card" data-id="${j.id}"
        style="width:100px;border-radius:10px;border:3px solid ${A?"#FFD700":F};background:${q};
          display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0;position:relative;
          box-shadow:${A?"0 0 18px #FFD700":"0 2px 8px rgba(0,0,0,0.4)"};
          transform:${A?"scale(1.06)":"scale(1)"};transition:all 0.15s">
        <div style="padding:5px 6px;background:rgba(255,255,255,0.12);text-align:center;min-height:32px;display:flex;align-items:center;justify-content:center">
          <span style="font-size:${((w==null?void 0:w.name)||j.gc_type).length>12?8:10}px;font-weight:900;color:#fff;line-height:1.2;text-align:center">${(w==null?void 0:w.name)||j.gc_type}</span>
        </div>
        <div style="height:70px;display:flex;align-items:center;justify-content:center;padding:4px">
    ...
    </div>`,C)return;const $=n==="p1"?f.home_id:f.away_id;k.from("cards").select("id,gc_type").eq("owner_id",$).eq("card_type","game_changer").then(async({data:j})=>{const{data:A}=await k.from("gc_definitions").select("*").eq("is_active",!0),w=new Set,D=(j||[]).filter(q=>w.has(q.gc_type)?!1:(w.add(q.gc_type),!0)).map(q=>({...q,_gcDef:(A==null?void 0:A.find(F=>F.name===q.gc_type))||null})),L=document.getElementById("pvp-gc-grid");if(L){if(!D.length){L.innerHTML='<div style="color:rgba(255,255,255,0.4);font-size:13px;text-align:center;margin-top:30px">Aucune carte Game Changer disponible.</div>';return}L.innerHTML=D.map(q=>S(q,B.includes(q.id))).join(""),L.querySelectorAll(".pvp-gc-card").forEach(q=>{q.addEventListener("click",()=>{const F=q.dataset.id,U=B.indexOf(F);U>-1?B.splice(U,1):B.length<N&&B.push(F),M()})})}}),(b=e.querySelector("#pvp-gc-validate"))==null||b.addEventListener("click",async()=>{const j=n==="p1"?f.home_id:f.away_id,{data:A}=await k.from("cards").select("id,gc_type").eq("owner_id",j).eq("card_type","game_changer").in("id",B),{data:w}=await k.from("gc_definitions").select("*").eq("is_active",!0),D=(A||[]).map(L=>({...L,_gcDef:(w==null?void 0:w.find(q=>q.name===L.gc_type))||null}));await g({["gcReady_"+n]:!0,["gcCardsFull_"+n]:D}),p["gcReady_"+r]&&await g({phase:"reveal"})})}function x(){e.style.overflow="hidden",e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
      ...
      <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
    </div>`,n==="p1"&&setTimeout(async()=>{await g({phase:"midfield"})},5e3)}function T(){const C=p[n+"Team"].MIL||[],z=p[r+"Team"].MIL||[];function B(A){return A.reduce((w,D)=>w+ie(D,"MIL"),0)}function N(A){let w=0;for(let D=0;D<A.length-1;D++){const L=ge(A[D],A[D+1]);L==="#00ff88"?w+=2:L==="#FFD700"&&(w+=1)}return w}const S=B(C)+N(C),$=B(z)+N(z),b=S>=$;function j(A,w,D){return`<div style="text-align:center">
        <div style="font-size:10px;color:rgba(255,255,255,0.5);letter-spacing:2px;margin-bottom:8px;text-transform:uppercase">${w}</div>
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${A.map((L,q)=>{const F=ne(L),U=q<A.length-1?ge(L,A[q+1]):null,K=U&&U!=="#ff3333"&&U!=="#cc2222";return`
            <div style="width:52px;height:52px;border-radius:8px;background:${D};position:relative;flex-shrink:0;overflow:hidden;border:2px solid rgba(255,255,255,0.3)">
              ...
              <div style="position:relative;z-index:1;font-size:15px;font-weight:900;color:#fff;text-shadow:0 1px 3px #000;text-align:center;padding-top:4px">${ie(L,"MIL")}</div>
              <div style="position:relative;z-index:1;font-size:6px;color:#fff;text-align:center;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;padding:0 2px">${L.name}</div>
            </div>
        ...
        <div style="margin-top:6px;font-size:11px;color:rgba(255,255,255,0.5)">
          Score: ${B(A)} + ${N(A)} liens = <b style="color:#fff">${B(A)+N(A)}</b>
        </div>
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    </div>`,setTimeout(()=>{const A=document.getElementById("pvp-score-me"),w=document.getElementById("pvp-score-opp"),D=document.getElementById("pvp-midfield-result");A&&w&&(b?(A.style.fontSize="80px",A.style.color="#FFD700",w.style.opacity="0.25"):(w.style.fontSize="80px",w.style.color="#ff6b6b",A.style.opacity="0.25"));const L=p.p1Team.MIL||[],q=p.p2Team.MIL||[],F=B(L)+N(L),U=B(q)+N(q),K=F>=U?"p1":"p2";p[K+"Name"];let X=p.boostValue;X==null&&(X=Yt(),p.boostValue=X,p.boostOwner=K,p.boostUsed=!1),D&&(D.style.opacity="1",D.innerHTML=`<div style="font-size:20px;font-weight:900;margin-bottom:10px">
      ...
      </div>`,document.body.appendChild(B),(N=B.querySelector("#boost-picker-close"))==null||N.addEventListener("click",()=>B.remove()),B.querySelectorAll(".boost-pick-item").forEach(S=>{S.addEventListener("click",async()=>{const $=S.dataset.cid,b=z.find(w=>w.cardId===$);if(!b)return;const j=p[n+"Team"],A=(j[b._line]||[]).find(w=>w.cardId===$);A&&(A.boost=(A.boost||0)+p.boostValue),B.remove(),await g({[n+"Team"]:j,boostUsed:!0})})})}function R(){var B;const C=Array.isArray(p.log)?p.log:[],z=document.createElement("div");z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column",z.innerHTML=`
      <div style="display:flex;align-items:center;padding:14px 16px;border-bottom:1px solid rgba(255,255,255,0.1);flex-shrink:0">
        <div style="flex:1;font-size:14px;font-weight:700;color:#fff">📋 Historique du match</div>
        <button id="pvp-hist-close" style="background:none;border:none;color:rgba(255,255,255,0.6);font-size:20px;cursor:pointer">✕</button>
      </div>
      <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
        ${C.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...C].reverse().map(N=>`<div style="padding:8px 10px;border-radius:8px;background:rgba(255,255,255,0.04);border-left:3px solid ${N.type==="goal"?"#FFD700":N.type==="defense"?"#00ff88":"rgba(255,255,255,0.5)"}">
      ...
      <div style="width:min(90vw,420px)">${Le(p[r+"Team"],p[r+"Formation"],null,[],300,300)}</div>
      <button id="pvp-view-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(C),(z=C.querySelector("#pvp-view-opp-close"))==null||z.addEventListener("click",()=>C.remove())}function H(C,z){var F,U;const N=(p["gcCardsFull_"+n]||[]).find(K=>K.id===C),S=N==null?void 0:N._gcDef,$=de[z]||{icon:"⚡",desc:"Carte spéciale."},b={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[S==null?void 0:S.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",j={purple:"#b06ce0",light_blue:"#00d4ef"}[S==null?void 0:S.color]||"#b06ce0",A=(S==null?void 0:S.name)||z,w=(S==null?void 0:S.effect)||$.desc,D=S!=null&&S.image_url?`/manager-wars/icons/${S.image_url}`:null,L=$.icon||"⚡",q=document.createElement("div");q.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",q.innerHTML=`
      <div style="width:190px;border-radius:16px;border:3px solid ${j};background:${b};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 40px ${j}66">
        ...
        <div style="height:160px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
          ${D?`<img src="${D}" style="max-width:150px;max-height:150px;object-fit:contain">`:`<span style="font-size:72px">${L}</span>`}
        </div>
        <div style="padding:10px;background:rgba(0,0,0,0.38);text-align:center">
      ...
      </div>`,document.body.appendChild(q),(F=q.querySelector("#pvp-gc-back"))==null||F.addEventListener("click",()=>q.remove()),(U=q.querySelector("#pvp-gc-use"))==null||U.addEventListener("click",async()=>{q.remove();const K=p["usedGc_"+n]||[];K.push(C),await g({["usedGc_"+n]:K})})}function W(){var S;const C=p[n+"Score"],z=p[r+"Score"],B=C>z,N=C===z;e.style.overflow="hidden",e.innerHTML=`
    ...
    </div>`,(S=document.getElementById("pvp-home"))==null||S.addEventListener("click",()=>{k.removeChannel(h),le(e),d("home")})}async function V(){const C=(p["selected_"+n]||[]).map(N=>({...N,_line:N._role}));if(!C.length)return;const z=ot(C,p.modifiers[n]||{});C.forEach(N=>{const S=(p[n+"Team"][N._role]||[]).find($=>$.cardId===N.cardId);S&&(S.used=!0)});const B=Array.isArray(p.log)?p.log:[];B.push({type:"attack",text:`⚔️ ${p[n+"Name"]} attaque (${z.total})`,players:C,total:z.total,side:n}),await g({pendingAttack:{...z,players:C,side:n},["selected_"+n]:[],modifiers:{...p.modifiers,[n]:{}},phase:r+"-defense",log:B})}async function ee(){const C=(p["selected_"+n]||[]).map(D=>({...D,_line:D._role})),z=nt(C,p.modifiers[n]||{});C.forEach(D=>{const L=(p[n+"Team"][D._role]||[]).find(q=>q.cardId===D.cardId);L&&(L.used=!0)});const B=rt(p.pendingAttack.total,z.total,p.modifiers[n]||{}),N=p.pendingAttack.side,S=B==="attack",$=p[N+"Score"]+(S?1:0),b=N==="p1"?"p2":"p1",j=(p.round||0)+1,A=j>10,w=Array.isArray(p.log)?p.log:[];w.push({type:"defense",text:`🛡️ ${p[n+"Name"]} défend (${z.total})`,players:C,total:z.total,side:n}),w.push({type:S?"goal":"stop",text:S?`⚽ BUT de ${p[N+"Name"]} ! (${p.pendingAttack.total} vs ${z.total})`:`✋ Attaque stoppée (${p.pendingAttack.total} vs ${z.total})`}),await g({[N+"Score"]:$,["selected_"+n]:[],modifiers:{...p.modifiers,[n]:{}},pendingAttack:null,phase:A?"finished":b+"-attack",attacker:b,round:j,log:w}),A&&await k.from("matches").update({status:"finished"}).eq("id",i)}M()}const mo={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"};async function xo(e,t){const{state:i,toast:s}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement du marché...</div>',await ct(e,t)}async function ct(e,t){const{state:i,toast:s}=t,{data:c}=await k.from("market_listings").select(`id, price, status, listed_at, seller_id,
  ...
  `;function l(n){const r=document.getElementById("mkt-content"),u=n==="buy"?a:d;if(u.length===0){r.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      </div>`;return}const m=n==="mine"?[...u].sort((f,_)=>f.status!==_.status?f.status==="active"?-1:1:new Date(_.listed_at)-new Date(f.listed_at)):u;r.innerHTML=`<div style="display:flex;flex-direction:column;gap:10px">
      ${m.map(f=>{var g,v,M,I;const _=(g=f.card)==null?void 0:g.player;if(!_)return"";const p=_.job==="GK"?_.note_g:_.job==="DEF"?_.note_d:_.job==="MIL"?_.note_m:_.note_a,y=mo[_.rarity],E=(i.profile.credits||0)>=f.price,h=f.status==="sold";return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${h?"opacity:0.65":""}">
          <div style="width:44px;height:44px;border-radius:8px;background:${vo(_.job)};color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:18px;border:2px solid ${y};flex-shrink:0">${p}</div>
          <div style="flex:1;min-width:0">
            ...
            ${n==="buy"?`<div style="font-size:11px;color:var(--gray-600)">Vendeur : ${((M=f.seller)==null?void 0:M.pseudo)||"—"}</div>`:h?`<div style="font-size:11px;color:var(--green)">✅ Vendu à ${((I=f.buyer)==null?void 0:I.pseudo)||"—"} · ${f.sold_at?new Date(f.sold_at).toLocaleDateString("fr"):""}</div>`:`<div style="font-size:11px;color:var(--gray-600)">🟢 En vente depuis le ${new Date(f.listed_at).toLocaleDateString("fr")}</div>`}
          </div>
