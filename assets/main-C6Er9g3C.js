...
import{t as Zi,s as v,u as Uo,n as ut,g as Bt,p as nt,v as si,o as Ut,r as Ae,k as At,w as Gi,x as Ft,y as Ii,F as zi,z as Ko,T as Di,q as Tn,C as In,d as zn}from"./special-cards-Bn7RhUuk.js";const An="/";async function ko(t,{navigate:e,toast:i}){let o="login";const r=await Zi("signup_access_code_required"),n=()=>{var d,c,s,l,p,x,g,b;const a=o==="login";if(t.innerHTML=`
    <div style="
    ...
    </style>`,(d=document.getElementById("tab-login-btn"))==null||d.addEventListener("click",()=>{o="login",n()}),(c=document.getElementById("tab-reg-btn"))==null||c.addEventListener("click",()=>{o="register",n()}),a){(s=document.getElementById("login-password"))==null||s.addEventListener("keydown",u=>{var w;u.key==="Enter"&&((w=document.getElementById("login-btn"))==null||w.click())});let m=null,y=!1;r&&((l=document.getElementById("login-access-code"))==null||l.addEventListener("input",u=>{const w=u.target.value,h=document.getElementById("login-access-code-status"),$=document.getElementById("login-btn");if(clearTimeout(m),y=!1,$&&($.disabled=!0,$.style.opacity="0.45",$.style.cursor="not-allowed"),!w){h&&(h.textContent="");return}h&&(h.textContent="⏳ Vérification…",h.style.color="rgba(255,255,255,0.4)"),m=setTimeout(async()=>{const{data:f,error:T}=await v.rpc("check_signup_password",{input_password:w});if(T){h&&(h.textContent="Erreur de vérification.",h.style.color="#f87171");return}y=!!f,h&&(h.textContent=y?"✅ Code valide":"❌ Code incorrect",h.style.color=y?"#4ade80":"#f87171"),$&&($.disabled=!y,$.style.opacity=y?"1":"0.45",$.style.cursor=y?"pointer":"not-allowed")},400)})),(p=document.getElementById("login-btn"))==null||p.addEventListener("click",async()=>{var E;const u=document.getElementById("login-email").value.trim(),w=document.getElementById("login-password").value,h=((E=document.getElementById("login-access-code"))==null?void 0:E.value)||"",$=document.getElementById("login-error");if($.textContent="",!u||!w){$.textContent="Remplissez tous les champs.";return}if(r){const{data:I}=await v.rpc("check_signup_password",{input_password:h});if(!I){$.textContent="Code d'accès incorrect.";return}}const f=document.getElementById("login-btn");f.textContent="⏳ Connexion…",f.disabled=!0;const{error:T}=await v.auth.signInWithPassword({email:u,password:w});if(f.textContent="⚽ Se connecter",f.disabled=!1,T){$.textContent=T.message.includes("Invalid")?"Email ou mot de passe incorrect.":T.message;return}window.location.reload()})}else{(x=document.getElementById("reg-confirm"))==null||x.addEventListener("keydown",u=>{var w;u.key==="Enter"&&((w=document.getElementById("reg-btn"))==null||w.click())});let m=null,y=!1;r&&((g=document.getElementById("reg-access-code"))==null||g.addEventListener("input",u=>{const w=u.target.value,h=document.getElementById("access-code-status"),$=document.getElementById("reg-btn");if(clearTimeout(m),y=!1,$&&($.disabled=!0,$.style.opacity="0.45",$.style.cursor="not-allowed"),!w){h&&(h.textContent="");return}h&&(h.textContent="⏳ Vérification…",h.style.color="rgba(255,255,255,0.4)"),m=setTimeout(async()=>{const{data:f,error:T}=await v.rpc("check_signup_password",{input_password:w});if(T){h&&(h.textContent="Erreur de vérification.",h.style.color="#f87171");return}y=!!f,h&&(h.textContent=y?"✅ Code valide":"❌ Code incorrect",h.style.color=y?"#4ade80":"#f87171"),$&&($.disabled=!y,$.style.opacity=y?"1":"0.45",$.style.cursor=y?"pointer":"not-allowed")},400)})),(b=document.getElementById("reg-btn"))==null||b.addEventListener("click",async()=>{var B;const u=document.getElementById("reg-email").value.trim(),w=document.getElementById("reg-password").value,h=document.getElementById("reg-confirm").value,$=((B=document.getElementById("reg-access-code"))==null?void 0:B.value)||"",f=document.getElementById("reg-error");if(f.textContent="",!u||!w||!h){f.textContent="Remplissez tous les champs.";return}if(!u.includes("@")||!u.includes(".")){f.textContent="Adresse email invalide.";return}if(w.length<6){f.textContent="Mot de passe trop court (min. 6 caractères).";return}if(w!==h){f.textContent="Les mots de passe ne correspondent pas.";return}if(r){const{data:V}=await v.rpc("check_signup_password",{input_password:$});if(!V){f.textContent="Code d'accès incorrect.";return}}const T=document.getElementById("reg-btn");T.textContent="⏳ Création…",T.disabled=!0;const{data:E,error:I}=await v.auth.signUp({email:u,password:w});if(T.textContent="🚀 Créer mon compte",T.disabled=!1,I){I.message.includes("already registered")||I.message.includes("already exists")||I.message.includes("User already")?f.textContent="Un compte existe déjà avec cette adresse email.":f.textContent=I.message;return}if(E!=null&&E.user&&Array.isArray(E.user.identities)&&E.user.identities.length===0){f.textContent="Un compte existe déjà avec cette adresse email.";return}i("Compte créé ! Connecte-toi pour commencer.","success",4e3),o="login",n(),setTimeout(()=>{const V=document.getElementById("login-email");V&&(V.value=u)},50)})}};n()}function Mn(t,{state:e,navigate:i,toast:o,refreshProfile:r}){let n="#1A6B3C",a="#D4A017";t.innerHTML=`
  ...
  `;function d(){var b;const s=document.getElementById("logo-preview"),l=document.getElementById("logo-initials"),p=((b=document.getElementById("setup-club"))==null?void 0:b.value)||"MW",x=p.trim().split(" ").filter(Boolean),g=x.length>=2?(x[0][0]+x[1][0]).toUpperCase():p.slice(0,2).toUpperCase();s&&(s.style.background=a,s.style.borderColor=n),l&&(l.textContent=g,l.style.color=n)}document.getElementById("color1").addEventListener("input",s=>{n=s.target.value,document.getElementById("swatch1").style.background=n,d()}),document.getElementById("color2").addEventListener("input",s=>{a=s.target.value,document.getElementById("swatch2").style.background=a,d()});function c(s){document.querySelectorAll(".setup-step").forEach(l=>l.classList.remove("active")),document.getElementById(`step-${s}`).classList.add("active"),document.getElementById("step-num").textContent=s,document.getElementById("progress-fill").style.width=`${Math.round(s/3*100)}%`,s===3&&d()}document.getElementById("step1-next").addEventListener("click",async()=>{const s=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("step1-error");if(l.textContent="",s.length<3){l.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:p}=await v.from("users").select("id").eq("pseudo",s).maybeSingle();if(p){l.textContent="Ce pseudo est déjà pris.";return}c(2)}),document.getElementById("step2-back").addEventListener("click",()=>c(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const s=document.getElementById("setup-club").value.trim(),l=document.getElementById("step2-error");if(l.textContent="",s.length<2){l.textContent="Nom trop court (min. 2 caractères).";return}const{data:p}=await v.from("users").select("id").eq("club_name",s).maybeSingle();if(p){l.textContent="Ce nom de club est déjà pris.";return}c(3)}),document.getElementById("step3-back").addEventListener("click",()=>c(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const s=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("setup-club").value.trim(),p=document.getElementById("step3-error"),x=document.getElementById("step3-finish");p.textContent="",x.disabled=!0,x.textContent="Création en cours…";try{const{error:g}=await v.from("users").insert({id:e.user.id,pseudo:s,club_name:l,club_color1:n,club_color2:a,credits:15e3});if(g)throw g;await Cn(e.user.id),await r(),o(`Bienvenue ${s} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(g){p.textContent=g.message,x.disabled=!1,x.textContent="🚀 Créer mon profil !"}})}async function Cn(t){const e=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1},{type:"stadium",count:1}];try{await v.from("users").update({pending_boosters:e,onboarding_done:!1,first_booster_opened:!1}).eq("id",t)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const jn="modulepreload",qn=function(t){return"/"+t},$o={},Vo=function(e,i,o){let r=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),d=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));r=Promise.allSettled(i.map(c=>{if(c=qn(c),c in $o)return;$o[c]=!0;const s=c.endsWith(".css"),l=s?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${c}"]${l}`))return;const p=document.createElement("link");if(p.rel=s?"stylesheet":jn,s||(p.as="script"),p.crossOrigin="",p.href=c,d&&p.setAttribute("nonce",d),document.head.appendChild(p),s)return new Promise((x,g)=>{p.addEventListener("load",x),p.addEventListener("error",()=>g(new Error(`Unable to preload CSS for ${c}`)))})}))}function n(a){const d=new Event("vite:preloadError",{cancelable:!0});if(d.payload=a,window.dispatchEvent(d),!d.defaultPrevented)throw a}return r.then(a=>{for(const d of a||[])d.status==="rejected"&&n(d.reason);return e().catch(n)})},Yo="mw_sound_volume";function eo(){const t=localStorage.getItem(Yo);if(t===null)return 100;const e=parseInt(t,10);return Number.isFinite(e)?Math.max(0,Math.min(100,e)):100}function Bn(t){localStorage.setItem(Yo,String(Math.max(0,Math.min(100,Math.round(t)))))}function to(){return eo()===0}function wi(t){return Math.max(0,Math.min(1,t*(eo()/100)))}function io(t,e=1){if(to())return null;try{const i=new Audio(t);return i.volume=wi(e),i.play().catch(()=>{}),i}catch{return null}}let vt=null,Oi=null,Eo=.3;function Wo(t,e=.3){if(Eo=e,vt&&Oi===t&&!vt.paused){vt.volume=wi(Eo);return}if(Ot(),!to())try{const i=new Audio(t);i.loop=!0,i.volume=wi(e),i.play().catch(()=>{}),vt=i,Oi=t}catch{}}function Ot(){if(vt)try{vt.pause(),vt.currentTime=0}catch{}vt=null,Oi=null}let ti=null;function Xo(t,e=.6){if(Ve(),!to())try{const i=new Audio(t);i.volume=wi(e),i.play().catch(()=>{}),ti=i}catch{}}function Ve(){if(ti)try{ti.pause(),ti.currentTime=0}catch{}ti=null}async function Jo(t=null){const e=new Date().toISOString().slice(0,10),{data:i}=await v.from("booster_configs").select("*").eq("is_active",!0).order("sort_order");if(!(i!=null&&i.length))return[];const o=i.filter(a=>!(a.available_from&&e<a.available_from||a.available_until&&e>a.available_until));if(!o.length)return[];let r=o;if(t){const a=o.filter(d=>d.max_per_user!=null);if(a.length){const{data:d}=await v.from("booster_claims").select("booster_id").eq("user_id",t).in("booster_id",a.map(s=>s.id)),c={};(d||[]).forEach(s=>{c[s.booster_id]=(c[s.booster_id]||0)+1}),r=o.filter(s=>s.max_per_user==null?!0:(c[s.id]||0)<s.max_per_user)}}if(!r.length)return[];const{data:n}=await v.from("booster_drop_rates").select("*").in("booster_id",r.map(a=>a.id)).order("sort_order");return r.map(a=>({...a,rates:(n||[]).filter(d=>d.booster_id===a.id)}))}async function Dn(t,e){const{data:i}=await v.from("booster_configs").select("max_per_user").eq("id",e).single();i!=null&&i.max_per_user&&await v.from("booster_claims").insert({user_id:t,booster_id:e})}function Fn(t){if(!(t!=null&&t.length))return null;const e=t.reduce((o,r)=>o+Number(r.percentage),0);let i=Math.random()*e;for(const o of t)if(i-=Number(o.percentage),i<=0)return o;return t[t.length-1]}const Qo=()=>Object.keys(ut),Pn=[{id:"players_std",img:"/icons/booster-players.png",name:"Players",sub:"5 cartes joueurs",cost:5e3,costLabel:"5 000 crédits",cardCount:5,type:"player"},{id:"players_pub",img:"/icons/booster-silver.png",name:"Players (pub)",sub:"3 cartes joueurs",cost:0,costLabel:"1 pub",cardCount:3,type:"player"},{id:"game_changer",img:"/icons/booster-gamechanger.png",name:"Game Changer",sub:"3 cartes spéciales",cost:1e4,costLabel:"10 000 crédits",cardCount:3,type:"game_changer"},{id:"formation",img:"/icons/booster-formation.png",name:"Formation",sub:"1 carte formation",cost:1e4,costLabel:"10 000 crédits",cardCount:1,type:"formation"}],Hi={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé."},"Double attaque":{icon:"⚡",desc:"La prochaine attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la prochaine action IA."},Gel:{icon:"❄️",desc:"Bloque le meilleur attaquant IA."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function Rn(t){const e=t.player;if(!e)return"";const i=t.evolution_bonus||0;return Ae({...e,_evolution_bonus:i},{width:140})}function oo(t){var r;const e={};(t.rates||[]).forEach(n=>{e[n.card_type]=(e[n.card_type]||0)+Number(n.percentage||0)});const i=((r=Object.entries(e).sort((n,a)=>a[1]-n[1])[0])==null?void 0:r[0])||"player",o=t.image_url||"booster-players.png";return{id:t.id,img:"/icons/"+o,name:t.name,sub:`${t.card_count} carte(s)`,cost:t.price_type==="credits"&&t.price_credits||0,costLabel:t.price_type==="credits"?`${(t.price_credits||0).toLocaleString("fr")} crédits`:t.price_type==="pub"?"1 pub":"Gratuit",cardCount:t.card_count||5,type:i,isPub:t.price_type==="pub",rates:t.rates||[],allow_duplicates:t.allow_duplicates!==!1,_boosterId:t.id,_raw:t}}async function Nn(t,e,i,o){if(t.reward_type==="credits"){const r=(e.credits||0)+(t.credits_amount||0),{error:n}=await v.from("users").update({credits:r}).eq("id",e.id);if(n)throw n;return e.credits=r,{type:"credits",amount:t.credits_amount}}if(t.reward_type==="card"){const{data:r}=await v.from("players").select("firstname, surname_real, rarity").eq("id",t.player_id).single(),{error:n}=await v.from("cards").insert({owner_id:e.id,player_id:t.player_id,card_type:"player"});if(n)throw n;return{type:"card",player:r}}if(t.reward_type==="booster"){const{data:r}=await v.from("booster_configs").select("*").eq("id",t.booster_config_id).single(),{data:n}=await v.from("booster_drop_rates").select("*").eq("booster_id",t.booster_config_id);if(!r)throw new Error("Booster introuvable (peut-être supprimé depuis).");const a=oo({...r,rates:n});let d=[];return a.type==="formation"?d=await ao(e,0):a.type==="game_changer"?d=await ro(e,a.cardCount,0):a.type==="player"?d=await no(e,a.cardCount,0):d=await _i(e,{...a,cost:0}),{type:"booster",name:r.name,cards:d,boosterUI:a}}throw new Error("Type de récompense inconnu.")}async function Gn(t,{state:e,navigate:i,toast:o}){var p,x,g;Yt(i,e.profile,"boosters","/icons/",o);const r=((p=e.profile)==null?void 0:p.credits)||0;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⏳ Chargement...</div>';let n=[];try{n=(await Jo((x=e.user)==null?void 0:x.id)).map(oo)}catch(b){console.warn("Erreur chargement boosters DB, fallback hardcodé",b)}n.length||(n=Pn.map(b=>({...b,rates:[],isPub:b.id==="players_pub"})));const a=await v.from("cards").select("player_id, card_type, formation, stadium_id, gc_type").eq("owner_id",e.profile.id).then(b=>b.data||[]),d=new Set(a.filter(b=>b.card_type==="stadium").map(b=>b.stadium_id)),c=new Set(a.filter(b=>b.card_type==="formation").map(b=>b.formation)),s=new Set(a.filter(b=>b.card_type==="game_changer").map(b=>b.gc_type)),l={};for(const b of n){if(b.allow_duplicates!==!1||!((g=b.rates)!=null&&g.length))continue;const m=[...new Set((b.rates||[]).map(u=>u.card_type))];let y=!1;for(const u of m)if(u==="stadium"){const{data:w}=await v.from("stadium_definitions").select("id");if((w||[]).some(h=>!d.has(h.id))){y=!0;break}}else if(u==="game_changer"){const{data:w}=await v.from("gc_definitions").select("name").eq("is_active",!0);if((w||[]).some(h=>!s.has(h.name))){y=!0;break}}else if(u==="formation"){const{FORMATION_LINKS:w}=await Vo(async()=>{const{FORMATION_LINKS:h}=await import("./special-cards-Bn7RhUuk.js").then($=>$.G);return{FORMATION_LINKS:h}},__vite__mapDeps([0,1]));if(Object.keys(w).some(h=>!c.has(h))){y=!0;break}}else{y=!0;break}y||(l[b.id]=!0)}t.innerHTML=`
  <div class="page">
    ...
    <div class="page-body">
      <div class="booster-grid">
        ${n.map(b=>{const m=b.cost===0||r>=b.cost,y=l[b.id]===!0;return`<div class="booster-card ${!m||y?"disabled":""}" data-booster="${b.id}" style="position:relative">
            <button class="booster-info-btn" data-booster-id="${b.id}"
              style="position:absolute;top:6px;right:6px;width:20px;height:20px;border-radius:50%;
              background:rgba(0,0,0,0.15);border:none;cursor:pointer;font-size:11px;font-weight:700;
              color:var(--gray-600);display:flex;align-items:center;justify-content:center;z-index:2"
              onclick="event.stopPropagation()">ℹ</button>
            <div class="icon"><img src="${b.img}" alt="${b.name}" style="height:64px;width:auto;display:block;margin:0 auto" onerror="this.src='/icons/booster-players.png'"></div>
  ...
  </div>`,t.querySelectorAll(".booster-card:not(.disabled)").forEach(b=>{b.addEventListener("click",async()=>{const m=n.find(y=>y.id===b.dataset.booster);if(m){b.style.opacity="0.5",b.style.pointerEvents="none";try{await On(m,{state:e,toast:o,navigate:i,container:t})}catch(y){o(y.message,"error"),b.style.opacity="",b.style.pointerEvents=""}}})}),t.querySelectorAll(".booster-info-btn").forEach(b=>{b.addEventListener("click",m=>{m.stopPropagation();const y=n.find(u=>u.id===b.dataset.boosterId);Yn(y)})})}async function On(t,{state:e,toast:i,navigate:o,container:r}){var p,x;if(t.cost>0&&e.profile.credits<t.cost){i("Crédits insuffisants","error");return}if(t.isPub){if(!await Zi("pub_mode")){Uo();return}await Xn()}const{data:n}=await v.from("cards").select("card_type, player_id, formation").eq("owner_id",e.profile.id),a=new Set((n||[]).filter(g=>g.card_type==="player").map(g=>g.player_id)),d=new Set((n||[]).filter(g=>g.card_type==="formation").map(g=>g.formation));let c=[],s=null;try{if((p=t.rates)!=null&&p.length)c=await _i(e.profile,t);else{const g=t.type||"player";g==="player"?c=await no(e.profile,t.cardCount,t.cost):g==="game_changer"?c=await ro(e.profile,t.cardCount,t.cost):g==="formation"?c=await ao(e.profile,t.cost):c=await _i(e.profile,t)}}catch(g){s=g.message||String(g),console.error("[Booster] Erreur:",g)}if(c!=null&&c.length&&t._boosterId&&await Dn(e.user.id,t._boosterId),!(c!=null&&c.length)){const g=document.createElement("div");g.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:3000;gap:16px;color:#fff;padding:24px;text-align:center",g.innerHTML=`
      <div style="font-size:48px">😕</div>
      <div style="font-size:20px;font-weight:900">Aucune carte obtenue</div>
      <div style="font-size:12px;color:rgba(255,255,255,0.6);max-width:320px;word-break:break-all;background:rgba(255,255,255,0.05);padding:10px;border-radius:8px;margin-top:6px">
        ${s||"Vérifie la console (F12) pour plus de détails"}
      </div>
      <button style="margin-top:10px;padding:12px 28px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer" id="anim-close-err">Fermer</button>`,document.body.appendChild(g),(x=g.querySelector("#anim-close-err"))==null||x.addEventListener("click",()=>g.remove());return}c.forEach(g=>{g.card_type==="player"&&g.player?g.isDuplicate=a.has(g.player.id):g.card_type==="formation"&&(g.isDuplicate=d.has(g.formation))});const{data:l}=await v.from("users").select("*").eq("id",e.profile.id).single();l&&(e.profile=l),so(c,t,o)}function Hn(){const t=Math.random()*100;return t<.5?"legende":t<2?"special":t<10?"normal_high":"normal_low"}function ht(t){return Math.max(Number(t.note_g)||0,Number(t.note_d)||0,Number(t.note_m)||0,Number(t.note_a)||0)}function Un(t,e){let i;switch(e){case"legende":i=t.filter(o=>o.rarity==="legende"),i.length||(i=t.filter(o=>o.rarity==="pepite"||o.rarity==="papyte")),i.length||(i=t.filter(o=>ht(o)>=6));break;case"special":i=t.filter(o=>o.rarity==="pepite"||o.rarity==="papyte"),i.length||(i=t.filter(o=>ht(o)>=6));break;case"normal_high":i=t.filter(o=>o.rarity==="normal"&&ht(o)>=6),i.length||(i=t.filter(o=>ht(o)>=6));break;default:i=t.filter(o=>o.rarity==="normal"&&ht(o)>=1&&ht(o)<=5),i.length||(i=t.filter(o=>o.rarity==="normal"));break}return i.length||(i=t),i[Math.floor(Math.random()*i.length)]}async function _i(t,e){if(e.cost>0){const{error:x}=await v.from("users").update({credits:t.credits-e.cost}).eq("id",t.id);if(x)throw x;t.credits-=e.cost,Wt(t.credits)}const i=e.allow_duplicates!==!1;let o=[];const{data:r,error:n}=await v.from("cards").select("player_id, card_type, formation, stadium_id, gc_type").eq("owner_id",t.id);if(n){const{data:x}=await v.from("cards").select("player_id, card_type, formation, gc_type").eq("owner_id",t.id);o=x||[]}else o=r||[];const a=new Set(o.filter(x=>x.card_type==="player").map(x=>x.player_id)),d=new Set(o.filter(x=>x.card_type==="formation").map(x=>x.formation)),c=new Set(o.filter(x=>x.card_type==="game_changer").map(x=>x.gc_type)),s=new Set(o.filter(x=>x.card_type==="stadium").map(x=>x.stadium_id).filter(Boolean)),l=new Set,p=[];for(let x=0;x<(e.cardCount||5);x++){const g=Fn(e.rates);if(g){if(g.card_type==="player"){const b=E=>({légende:"legende",pépite:"pepite",pépites:"pepite"})[E]||E,m=g.rarity?b(g.rarity):null;let y=v.from("players").select("id,job,firstname,surname_real,country_code,club_id,rarity,note_g,note_d,note_m,note_a,skin,hair,hair_length,face,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);m&&(y=y.eq("rarity",m));const{data:u}=await y;let w=u||[];if((g.note_min||g.note_max)&&(w=w.filter(E=>{const I=Math.max(Number(E.note_g)||0,Number(E.note_d)||0,Number(E.note_m)||0,Number(E.note_a)||0);return(!g.note_min||I>=g.note_min)&&(!g.note_max||I<=g.note_max)})),w.length||(g.note_min||g.note_max?(w=u||[],console.warn("[Booster] Aucun joueur avec note",g.note_min,"-",g.note_max,"— fallback rareté uniquement")):w=u||[]),!w.length)continue;let h=w.filter(E=>!l.has(E.id));if(i)h.length||(h=w);else if(h=h.filter(E=>!a.has(E.id)),!h.length)continue;const $=h[Math.floor(Math.random()*h.length)];l.add($.id);const f=a.has($.id),{data:T}=await v.from("cards").insert({owner_id:t.id,player_id:$.id,card_type:"player"}).select().single();T&&(p.push({...T,player:$,isDuplicate:f}),v.rpc("record_transfer",{p_card_id:T.id,p_player_id:$.id,p_club_name:t.club_name||t.pseudo,p_manager_name:t.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{}))}else if(g.card_type==="game_changer"){const{data:b}=await v.from("gc_definitions").select("id,name,color,effect,image_url,gc_type").eq("is_active",!0).eq("gc_type","game_changer"),m=b!=null&&b.length?b:[{name:"Ressusciter"},{name:"Double attaque"},{name:"Bouclier"},{name:"Vol de note"},{name:"Gel"}],y=i?m:m.filter($=>!c.has($.name));if(!i&&!y.length)continue;const u=y[Math.floor(Math.random()*y.length)],w=u.name,{data:h}=await v.from("cards").insert({owner_id:t.id,card_type:"game_changer",gc_type:w,gc_definition_id:u.id||null}).select().single();h&&p.push({...h,_gcDef:u})}else if(g.card_type==="formation"){const b=Qo(),m=i?b:b.filter(h=>!d.has(h));if(!i&&!m.length)continue;const y=m[Math.floor(Math.random()*m.length)],u=d.has(y),{data:w}=await v.from("cards").insert({owner_id:t.id,card_type:"formation",formation:y}).select();w!=null&&w[0]&&p.push({...w[0],isDuplicate:u})}else if(g.card_type==="stadium"){const{data:b,error:m}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)");if(m){console.error("[Booster] stadium_definitions:",m.message);continue}if(!(b!=null&&b.length)){console.warn("[Booster] Aucun stade en DB");continue}const y=i?b:b.filter($=>!s.has($.id));if(!i&&!y.length)continue;const u=y[Math.floor(Math.random()*y.length)],{data:w,error:h}=await v.from("cards").insert({owner_id:t.id,card_type:"stadium",stadium_id:u.id}).select("id,card_type,stadium_id").single();if(h){console.error("[Booster] insert stadium card:",h.message);continue}w&&p.push({...w,rarity:"normal",_stadiumDef:u})}}}return p}async function no(t,e,i){if(i>0){const{error:s}=await v.from("users").update({credits:t.credits-i}).eq("id",t.id);if(s)throw s;t.credits-=i,Wt(t.credits)}const{data:o}=await v.from("players").select("id,job,firstname,surname_real,country_code,club_id,rarity,note_g,note_d,note_m,note_a,note_min,note_max,skin,hair,hair_length,face,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);if(!(o!=null&&o.length))throw new Error("Pas de joueurs en BDD — ajoutes-en via le panel admin !");const r=o.filter(s=>s.job==="GK"),n=o.filter(s=>s.job!=="GK"),a=!t.first_booster_opened&&r.length>0,d=[];for(let s=0;s<e;s++){const l=s===0&&a?r:s===0?n:o,p=Hn(),x=Un(l,p);x&&d.push(x)}a&&await v.from("users").update({first_booster_opened:!0}).eq("id",t.id);const{data:c}=await v.from("cards").insert(d.map(s=>({owner_id:t.id,player_id:s.id,card_type:"player"}))).select();return(c||[]).forEach((s,l)=>{v.rpc("record_transfer",{p_card_id:s.id,p_player_id:d[l].id,p_club_name:t.club_name||t.pseudo,p_manager_name:t.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{})}),d.map((s,l)=>({...c[l],player:s}))}async function ro(t,e,i){const{error:o}=await v.from("users").update({credits:t.credits-i}).eq("id",t.id);if(o)throw o;t.credits-=i,Wt(t.credits);const{data:r}=await v.from("gc_definitions").select("id,name,gc_type,color,effect,image_url").eq("is_active",!0),n=r!=null&&r.length?r:Object.keys(Hi).map(p=>({name:p,gc_type:"game_changer"})),a=Array.from({length:e},()=>n[Math.floor(Math.random()*n.length)]),d=a.map(p=>({owner_id:t.id,card_type:"game_changer",gc_type:p.name,gc_definition_id:p.id||null})),{data:c,error:s}=await v.from("cards").insert(d).select();return s&&console.error("[Booster GC] Erreur insert:",s.message,s),(c||[]).map((p,x)=>({...p,_gcDef:a[x]||null}))}async function ao(t,e){const{error:i}=await v.from("users").update({credits:t.credits-e}).eq("id",t.id);if(i)throw i;t.credits-=e,Wt(t.credits);const{data:o}=await v.from("cards").select("formation").eq("owner_id",t.id).eq("card_type","formation"),r=new Set((o||[]).map(l=>l.formation)),n=Qo(),a=n[Math.floor(Math.random()*n.length)],d=r.has(a),{data:c,error:s}=await v.from("cards").insert({owner_id:t.id,card_type:"formation",formation:a}).select();return s&&console.error("[Booster Formation] Erreur insert:",s.message,s),(c||[]).map(l=>({...l,isDuplicate:d}))}async function Kn(t,e){const{data:i}=await v.from("cards").select("stadium_id").eq("owner_id",t.id).eq("card_type","stadium"),o=new Set((i||[]).map(s=>s.stadium_id).filter(Boolean)),{data:r}=await v.from("stadium_definitions").select("*");if(!(r!=null&&r.length))throw new Error("Aucun stade configuré en base.");const n=r[Math.floor(Math.random()*r.length)],a=o.has(n.id),{data:d,error:c}=await v.from("cards").insert({owner_id:t.id,card_type:"stadium",stadium_id:n.id}).select();return c&&console.error("[Booster Stade] Erreur insert:",c.message,c),(d||[]).map(s=>({...s,isDuplicate:a,_stadiumDef:n}))}function so(t,e,i,o=null){var K,L;if(!t||t.length===0){const _=document.createElement("div");_.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:3000;gap:16px;color:#fff;padding:24px;text-align:center",_.innerHTML=`
      <div style="font-size:48px">😕</div>
      <div style="font-size:20px;font-weight:900">Aucune carte obtenue</div>
  ...
  `,document.body.appendChild(t),t.addEventListener("click",e=>{e.target===t&&t.remove()}),document.getElementById("odds-close").addEventListener("click",()=>t.remove())}function Xn(){return new Promise(t=>{const e=document.createElement("div");e.style.cssText=`
    ...
    `,document.body.appendChild(e);try{(window.adsbygoogle=window.adsbygoogle||[]).push({})}catch(r){console.warn("[AdSense]",r)}let i=5;const o=setInterval(()=>{i--;const r=document.getElementById("mw-ad-cd");r&&(r.textContent=i),i<=0&&(clearInterval(o),e.remove(),t(!0))},1e3)})}async function Jn(t,{state:e,navigate:i,toast:o,refreshProfile:r}){var x,g;const{data:n}=await v.from("users").select("*").eq("id",e.user.id).single();n&&(e.profile=n);let a=Array.isArray((x=e.profile)==null?void 0:x.pending_boosters)?[...e.profile.pending_boosters]:[];if(!a.length){await v.from("users").update({onboarding_done:!0}).eq("id",e.user.id),i("home");return}let d=null;try{const m=(await Jo()).find(y=>(y.name||"").toLowerCase().includes("new player"));m&&(d=oo(m))}catch(b){console.warn('[Onboarding] Config "Booster (new player)" introuvable, fallback taux par défaut',b)}const c=a.length;let s=0;t.innerHTML=`
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
  ...
  </div>`;const l=async()=>{await v.from("users").update({pending_boosters:a}).eq("id",e.user.id)};async function p(){var w;if(s>=c||!a.length){await v.from("users").update({pending_boosters:[],onboarding_done:!0}).eq("id",e.user.id),r&&await r(),o("Tous tes boosters sont ouverts ! Bon jeu 🎮","success",4e3),i("home");return}const b=a[0],{data:m}=await v.from("users").select("*").eq("id",e.user.id).single();m&&(e.profile=m);let y=[];try{if(b.type==="formation")y=await ao(e.profile,0);else if(b.type==="game_changer")y=await ro(e.profile,b.count||3,0);else if(b.type==="stadium")y=await Kn(e.profile,0);else if(d&&((w=d.rates)!=null&&w.length)){const h={...d,cost:0,cardCount:b.count||d.cardCount||5};y=await _i(e.profile,h),b.guaranteeGK&&!e.profile.first_booster_opened&&(y.some(f=>f.player&&f.player.job==="GK")||await Qn(e.profile,y),await v.from("users").update({first_booster_opened:!0}).eq("id",e.profile.id))}else y=await no(e.profile,b.count||5,0)}catch(h){o(h.message||"Erreur ouverture booster","error");return}a.shift(),s++,await l();const u=b.type==="formation"?{name:"Booster Formation",img:"/icons/booster-formation.png"}:b.type==="game_changer"?{name:"Booster Game Changer",img:"/icons/booster-gamechanger.png"}:b.type==="stadium"?{name:"Booster Stade",img:"/icons/booster-stade.png"}:{name:`Booster Joueurs (${s}/${c})`,img:(d==null?void 0:d.img)||"/icons/booster-players.png"};so(y,u,i,()=>{p()})}(g=document.getElementById("onboard-start"))==null||g.addEventListener("click",()=>p())}async function Qn(t,e){try{const{data:i}=await v.from("players").select("id,job,firstname,surname_real,country_code,club_id,rarity,note_g,note_d,note_m,note_a,skin,hair,hair_length,face,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0).eq("job","GK");if(!(i!=null&&i.length))return;const o=i[Math.floor(Math.random()*i.length)],r=e.findIndex(a=>a.player);if(r===-1)return;const n=e[r];await v.from("cards").update({player_id:o.id}).eq("id",n.id),e[r]={...n,player_id:o.id,player:o}}catch(i){console.warn("[Onboarding] ensureGK échec",i)}}const Zn=[{key:"home2",route:"home2",label:"ACCUEIL",icon:"nav-home.png"},{key:"cards",route:"collection",label:"CARTES",icon:"nav-collection.png"},{key:"decks",route:"decks",label:"EQUIPES",icon:"nav-decks.png"},{key:"boosters",route:"boosters",label:"BOUTIQUE",icon:"nav-boosters.png"},{key:"market",route:"market",label:"MERCATO",icon:"nav-market.png"}];function Yt(t,e,i,o,r){if(!document.getElementById("home2-chrome-style")){const p=document.createElement("style");p.id="home2-chrome-style",p.textContent=`
    ...
    `;try{(window.adsbygoogle=window.adsbygoogle||[]).push({})}catch(p){console.warn("[AdSense]",p)}let s=e;const l=setInterval(()=>{s--;const p=a.querySelector("#credits-ad-cd");p&&(p.textContent=s),s<=0&&(clearInterval(l),c())},1e3)}function c(){var p,x;const s=a.querySelector("#credits-ad-status"),l=a.querySelector("#credits-ad-next-zone");n<t?(s&&(s.textContent="Publicité terminée ✅"),l&&(l.innerHTML=`
        ...
        </button>`),(x=a.querySelector("#credits-ad-claim-btn"))==null||x.addEventListener("click",async()=>{const g=a.querySelector("#credits-ad-claim-btn");g.disabled=!0,g.textContent="⏳...";const b=(o.credits||0)+i,{error:m}=await v.from("users").update({credits:b}).eq("id",o.id);if(m){r(m.message,"error"),g.disabled=!1;return}o.credits=b,Wt(b),r(`+${i.toLocaleString("fr")} crédits ✅`,"success"),a.remove()}))}d()}async function Zo(t,e,i){const o=document.createElement("div");o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.85);display:flex;align-items:center;justify-content:center;z-index:9999;padding:16px",document.body.appendChild(o);async function r(){var c;const{data:n}=await v.from("pending_rewards").select(`*,
        player:players(id, firstname, surname_real, country_code, club_id, job, job2,
          note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length, face,
          clubs(encoded_name, logo_url)),
        booster:booster_configs(name, image_url)`).eq("user_id",t.profile.id).eq("claimed",!1).order("created_at",{ascending:!0});if(!(n!=null&&n.length)){o.remove();const s=document.getElementById("pending-rewards-banner");s&&s.remove();return}const a=s=>{var l;return s.reward_type==="credits"?'<div style="width:56px;height:56px;border-radius:12px;background:rgba(212,160,23,0.15);display:flex;align-items:center;justify-content:center;font-size:28px;flex-shrink:0">💰</div>':s.reward_type==="card"&&s.player?`<div style="flex-shrink:0">${Ae(s.player,{width:56})}</div>`:s.reward_type==="booster"?`<img src="/icons/${((l=s.booster)==null?void 0:l.image_url)||"booster-players.png"}" style="width:56px;height:56px;object-fit:contain;border-radius:12px;background:rgba(255,255,255,0.05);flex-shrink:0">`:'<div style="width:56px;height:56px;border-radius:12px;background:rgba(255,255,255,0.05);display:flex;align-items:center;justify-content:center;font-size:28px;flex-shrink:0">❓</div>'},d=s=>{var l,p,x;return s.reward_type==="credits"?`${(s.credits_amount||0).toLocaleString("fr")} crédits`:s.reward_type==="card"?`${((l=s.player)==null?void 0:l.firstname)||""} ${((p=s.player)==null?void 0:p.surname_real)||""}`.trim()||"Carte joueur":s.reward_type==="booster"?((x=s.booster)==null?void 0:x.name)||"Booster":"Récompense"};o.innerHTML=`
    ...
    </div>`,document.body.appendChild(e),e.querySelector("#journal-close-v2").addEventListener("click",()=>e.remove()),e.addEventListener("click",o=>{o.target===e&&e.remove()})}async function Fi(t,{state:e,navigate:i,toast:o,openModal:r,closeModal:n}){var E,I,B,V,W,K;const a=e.profile;if(!a)return;const d="/icons/";Yt(i,a,"home2",d,o);const c=a.mmr??1e3,s=At(c),l=or(c,s),p=Gi(c),x=isFinite(s.max)?s.max+1:null,[{list:g,iAmInTop:b,myPosition:m},y,u,{count:w}]=await Promise.all([nr(a.id),rr(5),ar(),v.from("pending_rewards").select("*",{count:"exact",head:!0}).eq("user_id",a.id).eq("claimed",!1)]),h=(L,_)=>`
  ...
  <div class="home-dark" id="home-dark">
      ...
      <div class="home2-dash">
          ...
          <div class="rank-card">
            <div class="rank-top-row">
              ...
              <div class="rank-right">
                <div class="rank-rp-label">${c.toLocaleString("fr")}${x?" / "+x.toLocaleString("fr"):""} RP</div>
        ...
        <div class="home2-col-right">
          <div class="news-widget">
            ...
            ${y.length?y.map($).join(""):'<div style="font-size:11px;color:var(--tile-fg-dim);padding:8px 0">Aucune actualité pour le moment.</div>'}
          </div>
        </div>
  ...
  </div>`,document.body.appendChild(i);const o=()=>i.remove();document.getElementById("solo-cancel").addEventListener("click",o),i.addEventListener("click",s=>{s.target===i&&o()});const[{data:r},{data:n}]=await Promise.all([v.from("solo_levels").select("*").eq("is_active",!0).order("level_number"),v.from("user_solo_progress").select("unlocked_level").eq("user_id",e.profile.id).maybeSingle()]),a=(n==null?void 0:n.unlocked_level)||1,d=r||[],c=document.getElementById("solo-levels-list");if(c){if(!d.length){c.innerHTML='<div style="text-align:center;color:#999;padding:20px">Aucun niveau configuré.</div>';return}c.innerHTML=d.map(s=>{const l=s.level_number>a,p=s.level_number===a,x=l?"#f0f0f0":p?"#eefaf2":"#f7f7f7",g=l?"#ddd":p?"#bfe8cf":"#e0e0e0",b=l?"#999":"#12401f";return`
      <div class="solo-level-card" data-level="${s.level_number}" data-locked="${l}"
        style="cursor:${l?"not-allowed":"pointer"};display:flex;align-items:center;gap:14px;padding:14px 16px;border-radius:14px;background:${x};border:1px solid ${g};opacity:${l?.6:1};transition:transform .12s ease">
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      ${x.map(({friendshipId:g,friend:b})=>mr(b,g)).join("")}
    </div>`,s.querySelectorAll("[data-stats]").forEach(g=>{g.addEventListener("click",()=>xr(t,g.dataset.stats,g.dataset.friendName))}),s.querySelectorAll("[data-match]").forEach(g=>{g.addEventListener("click",async()=>{const b=g.dataset.friendId,m=g.dataset.friendName;if(typeof o!="function"){e("Erreur navigation","error");return}const y=t.user.id,{data:u}=await v.from("friend_match_invites").select("id").eq("inviter_id",b).eq("invitee_id",y).eq("status","pending").order("created_at",{ascending:!1}).limit(1).maybeSingle(),w=!!u;console.log("[Friends] clic match",{fid:b,fname:m,isAccepting:w}),mt(),o("match",{matchMode:"friend",friendId:b,friendName:m,isAccepting:w})})})}function mr(t,e){const i=t.club_name||t.pseudo||"?",o=t.pseudo||"",r=(o||i).slice(0,2).toUpperCase(),n=t.club_color2||di,a=t.club_color1||"#ffffff",d=t.last_seen_at?new Date(t.last_seen_at):null,c=d&&Date.now()-d.getTime()<3*60*1e3;return`
    ...
    </div>`}function gr(t,e){const i=lo();i.innerHTML=`
    <div class="popup-box">
    ...
    <div class="popup-box">
      ...
      ${l.length?`<div style="display:flex;flex-direction:column;gap:8px;max-height:50vh;overflow-y:auto;margin-bottom:14px">
            ${l.map(x=>{var g,b,m;return`
              <div style="display:flex;align-items:center;gap:10px;background:#f9f9f9;border-radius:10px;padding:10px 12px">
                <div style="flex:1;font-size:14px;font-weight:700">${((g=x.requester)==null?void 0:g.club_name)||((b=x.requester)==null?void 0:b.pseudo)||"?"}
    ...
    ${co()}`,document.body.appendChild(s);const p=()=>s.remove();s.querySelector("#pending-close").addEventListener("click",p),s.addEventListener("click",x=>{x.target===s&&p()}),s.querySelectorAll("[data-accept]").forEach(x=>{x.addEventListener("click",async()=>{const{error:g}=await v.from("friendships").update({status:"accepted"}).eq("id",x.dataset.accept);if(g){e("Erreur : "+g.message,"error");return}x.closest("div[style]").remove(),e("✅ Ami accepté !","success"),nn(t,e,o),i&&i()})}),s.querySelectorAll("[data-decline]").forEach(x=>{x.addEventListener("click",async()=>{await v.from("friendships").delete().eq("id",x.dataset.decline),x.closest("div[style]").remove(),e("Demande refusée","info"),i&&i()})})}async function xr(t,e,i){const o=t.user.id,[r,n]=[o,e].sort(),a=o===r,{data:d}=await v.from("friend_match_stats").select("*").eq("player1_id",r).eq("player2_id",n).single(),c=t.profile.club_name||t.profile.pseudo||"Moi",s=d||{},l=a?s.wins_p1||0:s.wins_p2||0,p=a?s.wins_p2||0:s.wins_p1||0,x=s.draws||0,g=a?s.goals_p1||0:s.goals_p2||0,b=a?s.goals_p2||0:s.goals_p1||0,m=a?s.gc_used_p1||0:s.gc_used_p2||0,y=a?s.gc_used_p2||0:s.gc_used_p1||0,u=s.matches_total||0,w=($,f,T,E=di,I=li)=>`
    <div style="display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:8px;padding:10px 0;border-bottom:1px solid #f0f0f0">
      <div style="text-align:right;font-size:18px;font-weight:900;color:${E}">${f}</div>
  ...
  <div class="game-page">
    ...
    <p style="margin-top:12px;font-size:13px;color:#888">Tu peux revoir ce tutoriel depuis les paramètres à tout moment.</p>`}];function sn(t,e,i){let o=0;const r=document.createElement("div");r.id="tutorial-overlay",r.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.65);z-index:9900;display:flex;align-items:center;justify-content:center;padding:16px";const n=()=>{var l,p,x;const d=e[o],c=o===e.length-1,s=Math.round((o+1)/e.length*100);r.innerHTML=`
    ...
    `,r.querySelectorAll("ul,ol").forEach(g=>{g.style.paddingLeft="20px",g.style.marginTop="6px",g.style.marginBottom="6px"}),r.querySelectorAll("li").forEach(g=>{g.style.marginBottom="4px"}),r.querySelectorAll("p").forEach(g=>{g.style.marginBottom="8px"}),(l=r.querySelector("#tuto-prev"))==null||l.addEventListener("click",()=>{o--,n()}),(p=r.querySelector("#tuto-next"))==null||p.addEventListener("click",()=>{c?a():(o++,n())}),(x=r.querySelector("#tuto-skip"))==null||x.addEventListener("click",()=>{confirm("Passer le tutoriel ? Tu pourras le revoir plus tard depuis les paramètres.")&&a()})},a=async()=>{r.remove(),t!=null&&t.id&&await v.from("users").update({tutorial_done:!0}).eq("id",t.id),i==null||i()};document.body.appendChild(r),n()}async function dn(t,e,i){let o=[];const{data:r,error:n}=await v.rpc("get_tutorial_steps");if(!n&&(r==null?void 0:r.length)>0)o=r;else{const{data:d,error:c}=await v.from("tutorial_steps").select("*").eq("is_active",!0).order("step_order");!c&&(d==null?void 0:d.length)>0?o=d:i&&i("[Tutorial] DB vide ou inaccessible — tuto local utilisé","warning",5e3)}const a=o.length>0?o.map(d=>({emoji:d.emoji,title:d.title,color:d.color,content:d.content,image_url:d.image_url||null})):an;sn(t,a,()=>e("settings"))}async function To(t,e,i){if(!t||t.tutorial_done)return;let o=[];const{data:r,error:n}=await v.rpc("get_tutorial_steps");if(!n&&(r==null?void 0:r.length)>0)o=r,console.log(`[Tutorial] RPC OK → ${o.length} étapes`);else{const{data:d,error:c}=await v.from("tutorial_steps").select("*").eq("is_active",!0).order("step_order");!c&&(d==null?void 0:d.length)>0?(o=d,console.log(`[Tutorial] Direct OK → ${o.length} étapes`)):(console.warn(`[Tutorial] Aucune étape DB (RPC: ${n==null?void 0:n.message}, Direct: ${c==null?void 0:c.message})`),i&&i("[Tutorial] DB vide ou inaccessible — tuto local utilisé","warning",5e3))}const a=o.length>0?o.map(d=>({emoji:d.emoji,title:d.title,color:d.color,content:d.content,image_url:d.image_url||null})):an;sn(t,a,()=>e("boosters"))}async function ln(t,e){var y,u,w;const{state:i,navigate:o,toast:r}=e,n=ai(),a=eo(),d=(y=i==null?void 0:i.profile)==null?void 0:y.is_admin,c="2026.08.01-0932";t.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    ${Ae(t,{width:140})}
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),s=(d||[]).filter(j=>j.card_type==="player"&&j.player),l=(d||[]).filter(j=>j.card_type==="game_changer"),p=(d||[]).filter(j=>j.card_type==="formation"),x=(d||[]).filter(j=>j.card_type==="stadium"),{data:g}=await v.from("gc_definitions").select("name,gc_type,color,effect,image_url"),b={};(g||[]).forEach(j=>{b[j.name]=j});const{data:m}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),y={};(m||[]).forEach(j=>{y[j.id]=j}),x.forEach(j=>{j.stadium_def&&(y[j.stadium_id]=j.stadium_def)});const u=Object.keys(zi),w=Object.keys(Oe),h={};s.forEach(j=>{const Y=j.player.id;h[Y]=(h[Y]||0)+1}),new Set(Object.keys(h).map(j=>String(j)));const $=new Set(p.map(j=>j.formation)),f=new Set(l.map(j=>j.gc_type));let T="player",E="Tous",I="",B=!1,V=!1;function W(){return[...s].sort((j,Y)=>{const re=mi.indexOf(j.player.job),te=mi.indexOf(Y.player.job);return re!==te?re-te:(j.player.surname_real||"").localeCompare(Y.player.surname_real||"")})}function K(){return[...c||[]].sort((j,Y)=>{const re=mi.indexOf(j.job),te=mi.indexOf(Y.job);return re!==te?re-te:(j.surname_real||"").localeCompare(Y.surname_real||"")})}function L(){return W().filter(j=>{const Y=j.player,re=E==="Tous"||Y.job===E,te=!I||`${Y.firstname} ${Y.surname_real}`.toLowerCase().includes(I);return re&&te})}function _(){return K().filter(j=>{const Y=E==="Tous"||j.job===E,re=!I||`${j.firstname} ${j.surname_real}`.toLowerCase().includes(I);return Y&&re})}t.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:0 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none;align-items:center"></div>
          ...
          ${wr.map(Y=>`
            <button class="filter-btn" data-job="${Y}"
              style="flex-shrink:0;padding:4px 12px;border-radius:20px;font-size:12px;font-weight:600;cursor:pointer;
                border:1.5px solid ${Y===E?"var(--green)":"var(--tile-border)"};
                background:${Y===E?"var(--green)":"#fff"};
                color:${Y===E?"#fff":"#555"}">
              ${Y}
            </button>`).join("")}
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",Y=>{I=Y.target.value.toLowerCase(),G()}),t.querySelectorAll(".filter-btn").forEach(Y=>{Y.addEventListener("click",()=>{E=Y.dataset.job,N(),G()})}),document.getElementById("dupes-only-btn").addEventListener("click",()=>{V=!V,N(),G()}),document.getElementById("show-all-btn").addEventListener("click",()=>{B=!B,N(),G()})):(j.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{B=!B,N(),G()})))}function G(){const j=document.getElementById("col-grid");j&&(T==="player"?ue(j):T==="formation"?ge(j):T==="stadium"?de(j):xe(j))}function le(j,Y,re,te,X,P=0){const J=document.getElementById("col-grid"),F=document.getElementById("col-big");if(!J||!F)return;var H=0;function ae(){var De;const ce=window.innerWidth>=768,be=document.getElementById("col-big"),ke=document.getElementById("col-gap"),Ee=((De=document.getElementById("col-grid"))==null?void 0:De.parentElement)||null,Se=be?be.closest(".page"):null;if(be&&Ee&&Se){let Me=0;Array.from(Se.children).forEach(function(z){z!==be&&z!==Ee&&z!==ke&&(Me+=z.offsetHeight)});const Ce=Math.max(0,Se.clientHeight-Me),He=Math.round(Ce*(P/100)),k=Math.max(0,Ce-He),S=ce?50/71:45/63,A=Math.round(k*S),C=Math.max(0,k-A);ke&&(ke.style.height=He+"px"),be.style.flex="none",be.style.height=A+"px",be.style.minHeight="0",Ee.style.flex="none",Ee.style.height=C+"px",Ee.style.minHeight="0",Ee.style.display="flex",Ee.style.overflow="hidden"}const ye=document.getElementById("col-grid");ye&&(ye.style.height="100%",ye.style.flexShrink="0",ye.style.overflowX="auto",ye.style.overflowY="hidden",ye.style.alignItems="center",ye.style.width="100%"),F.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:center center">'+Y(j[H])+"</div>";var Le=F.querySelector("[data-card-id],[data-form-id],[data-gc-id]");Le&&Le.addEventListener("click",function(){te(j[H])}),requestAnimationFrame(function(){var Me=document.getElementById("big-card-inner");if(!(!Me||!F)){var Ce=F.clientHeight,He=F.clientWidth-16,k=Me.offsetHeight,S=Me.offsetWidth;if(k>0&&S>0&&Ce>40){var A=Math.min(Ce/k,He/S);Me.style.transform="scale("+A.toFixed(3)+")",Me.style.transformOrigin="center center"}}}),J.innerHTML=j.map(function(Me,Ce){var He=Ce===H,k="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+(He?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+Ce+'" style="'+k+'">'+re(Me,He)+"</div>"}).join(""),J.querySelectorAll(".col-mini-item").forEach(function(Me){Me.addEventListener("click",function(){H=Number(Me.dataset.idx),ae(),Me.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})}),requestAnimationFrame(function(){var Me=ye?ye.clientHeight:0,Ce=J.querySelector(".col-mini-item > div");if(!(!Me||!Ce)){var He=Ce.style.zoom;Ce.style.zoom="1";var k=Ce.offsetHeight;if(Ce.style.zoom=He,!(k<=0)){var S=Me/k;J.querySelectorAll(".col-mini-item > div").forEach(function(A){A.style.zoom=S.toFixed(4)})}}})}ae()}function pe(j,Y){var re=window.innerWidth>=768?.76:.54,te;if(!j||j._fake){var X=j?j.player:null;if(!X)return"";te=Ao(X)}else te=Pi(j,"");var P=j&&!j._fake?h[j.player&&j.player.id]||1:0,J=P>0?'<div style="position:absolute;top:6px;right:6px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:11px;font-weight:900;padding:2px 7px;z-index:3;box-shadow:0 1px 4px rgba(0,0,0,0.4)">×'+P+"</div>":"";return'<div style="display:inline-block;position:relative;zoom:'+re+';pointer-events:none;line-height:0">'+J+te+"</div>"}function M(j,Y,re){var te=re>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+re+"</div>":"",X=!!Y,P=si(j,ut[j],{width:160});return X||(P='<div style="filter:grayscale(1);opacity:0.5">'+P+"</div>"),"<div "+(Y?'data-form-id="'+Y.id+'"':"")+' style="position:relative;cursor:pointer">'+te+P+"</div>"}function O(j,Y){var re=window.innerWidth>=768?.76:.54,te=si(j,ut[j],{width:140});return Y||(te='<div style="filter:grayscale(1);opacity:0.45">'+te+"</div>"),'<div style="display:inline-block;zoom:'+re+';line-height:0;pointer-events:none">'+te+"</div>"}function ue(j){if(B){const Y=_();if(!Y.length){j.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucun joueur.</div>';return}const re=Y.map(te=>s.find(X=>X.player.id===te.id)||{_fake:!0,player:te,id:"fake-"+te.id});le(re,te=>te._fake?Ao(te.player):Pi(te,""),te=>te._fake?pe({player:te.player,id:"x",_fake:!0}):pe(te),te=>{te._fake||Mo(te,s,h,e)})}else{const Y=L();if(!Y.length){j.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const re={};Y.forEach(X=>{const P=X.player.id;re[P]||(re[P]=[]),re[P].push(X)});const te=Object.values(re).map(X=>kr(X)).filter(X=>!V||(h[X.player.id]||1)>1);if(V&&!te.length){j.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte en plusieurs exemplaires.</div>';return}le(te,X=>{const P=h[X.player.id]||1,J=P>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${P}</div>`:"",H=s.filter(ae=>ae.player.id===X.player.id&&ae.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Pi(X,J+H)},X=>pe(X),X=>Mo(X,s,h,e))}}function ge(j){const Y=B?u:[...$];if(!Y.length){j.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const re=Y.map(te=>({formation:te,card:p.find(X=>X.formation===te)||null,owned:$.has(te)}));le(re,({formation:te,card:X,owned:P})=>M(te,P?X:null,P?p.filter(J=>J.formation===te).length:0),({formation:te,owned:X})=>O(te,X),({card:te,owned:X})=>{X&&te&&Sr(te,p,e,n)},"#1A6B3C",5)}function xe(j){const Y=Object.keys(b),re=B?Y.length?Y:w:[...f];if(!re.length){j.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const te=re.map(X=>({type:X,gc:Oe[X]||{icon:"⚡",desc:""},def:b[X]||null,owned:f.has(X),card:l.find(P=>P.gc_type===X)||null}));le(te,({type:X,gc:P,def:J,owned:F,card:H})=>{var ye;const ae=(J==null?void 0:J.name)||X,ce=F?l.filter(Le=>Le.gc_type===X).length:0,be=ce>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${ce}</div>`:"",ke=(J==null?void 0:J.effect)||P.desc||"",Ee=J!=null&&J.image_url?`/icons/${J.image_url}`:((ye=J==null?void 0:J.club)==null?void 0:ye.logo_url)||(J!=null&&J.country_code?`https://flagsapi.com/${J.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null);let Se=nt(ae,Ee,P.icon,ke,{width:160,onClick:F});return F||(Se=`<div style="filter:grayscale(1);opacity:0.5">${Se}</div>`),`<div ${F&&H?`data-gc-id="${H.id}" data-gc-type="${X}"`:""} style="position:relative">${be}${Se}</div>`},({type:X,gc:P,def:J,owned:F})=>{const H=window.innerWidth>=768?.76:.54,ae=(J==null?void 0:J.name)||X,ce=(J==null?void 0:J.effect)||P.desc||"",be=J!=null&&J.image_url?`/icons/${J.image_url}`:null;let ke=nt(ae,be,P.icon,ce,{width:140});return F||(ke=`<div style="filter:grayscale(1);opacity:0.45">${ke}</div>`),`<div style="display:inline-block;zoom:${H};line-height:0;pointer-events:none">${ke}</div>`},({type:X,owned:P,def:J})=>{P&&Lr(X,J,n)},"#7a28b8",5)}function de(j){const Y="#4FC3F7",re="/";if(!x.length){j.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Stade.<br><small>Ouvre un booster Stade !</small></div>';return}const te={};x.forEach(P=>{const J=P.stadium_id||"?";(te[J]=te[J]||[]).push(P)});const X=Object.entries(te).map(([P,J])=>({sid:P,def:y[P]||null,count:J.length,card:J[0]}));le(X,({def:P,count:J})=>{var ke,Ee;const F=(P==null?void 0:P.name)||"?",H=((ke=P==null?void 0:P.club)==null?void 0:ke.encoded_name)||(P==null?void 0:P.country_code)||"—",ae=P!=null&&P.image_url?`${re}icons/${P.image_url}`:((Ee=P==null?void 0:P.club)==null?void 0:Ee.logo_url)||(P!=null&&P.country_code?`https://flagsapi.com/${P.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),ce=J>1?`<div style="position:absolute;top:8px;right:8px;background:#333;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${J}</div>`:"",be=`${H}<br>+10 ⭐ joueurs alliés`;return`<div style="position:relative">${ce}${Ut(F,ae,be,{width:160})}</div>`},({def:P})=>{var be,ke;const J=window.innerWidth>=768?.76:.54,F=(P==null?void 0:P.name)||"?",H=((be=P==null?void 0:P.club)==null?void 0:be.encoded_name)||(P==null?void 0:P.country_code)||"—",ae=P!=null&&P.image_url?`${re}icons/${P.image_url}`:((ke=P==null?void 0:P.club)==null?void 0:ke.logo_url)||(P!=null&&P.country_code?`https://flagsapi.com/${P.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),ce=`${H}<br>+10 ⭐`;return`<div style="display:inline-block;zoom:${J};line-height:0;pointer-events:none">${Ut(F,ae,ce,{width:140})}</div>`},null,Y,5)}t.querySelectorAll(".col-tab-btn").forEach(j=>{j.addEventListener("click",()=>{T=j.dataset.tab,E="Tous",I="",B=!1,t.querySelectorAll(".col-tab-btn").forEach(Y=>{const re=Y.dataset.tab===T;Y.style.borderBottomColor=re?"var(--green)":"transparent",Y.style.color=re?"var(--green)":"var(--tile-fg-dim)"}),N(),G()})}),N(),G()}function Lr(t,e,i){const o=Oe[t]||{icon:"⚡",desc:"Effet spécial."},r=(e==null?void 0:e.name)||t,n=(e==null?void 0:e.effect)||o.desc,a=e!=null&&e.image_url?`/icons/${e.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
      ${nt(r,a,o.icon,n,{width:200})}
      <div style="background:#fff3cd;border-radius:10px;padding:10px 14px;width:100%">
        <div style="font-size:12px;color:#856404">⚠️ Cette carte est à <b>usage unique</b>. Une fois jouée en match, elle est définitivement supprimée de ta collection.</div>
      </div>
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const xi=1e3;function Sr(t,e,i,o){var b,m,y;const{state:r,toast:n,closeModal:a,navigate:d,refreshProfile:c}=i,s=t.formation,l={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function p(){const u=ut[s]||{},w=zi[s]||[],h=290,$=360,f=20;function T(I){const B=u[I];return B?{x:B.x*h,y:B.y*$}:null}let E=`<svg width="${h}" height="${$}" viewBox="0 0 ${h} ${$}" xmlns="http://www.w3.org/2000/svg">`;for(const[I,B]of w){const V=T(I),W=T(B);!V||!W||(E+=`<line x1="${V.x}" y1="${V.y}" x2="${W.x}" y2="${W.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const I of Object.keys(u)){const B=T(I);if(!B)continue;const V=I.replace(/\d+/,""),W=l[V]||"#555";E+=`<circle cx="${B.x}" cy="${B.y}" r="${f}" fill="${W}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,E+=`<text x="${B.x}" y="${B.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${V}</text>`}return E+="</svg>",E}const x=e.filter(u=>u.formation===s);x.length;const g=!t.is_for_sale;o(`Formation ${s}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(b=document.getElementById("direct-sell-form-btn"))==null||b.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${s} pour ${xi.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const u=x.find(h=>!h.is_for_sale)||x[0];if(!u){n("Aucune carte à vendre","error");return}await v.from("market_listings").delete().eq("card_id",u.id),await v.from("transfer_history").delete().eq("card_id",u.id);const{error:w}=await v.from("cards").delete().eq("id",u.id);if(w){n(w.message,"error");return}await v.from("users").update({credits:(r.profile.credits||0)+xi}).eq("id",r.profile.id),await c(),n(`+${xi.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),a(),d("collection")}),(m=document.getElementById("market-sell-form-btn"))==null||m.addEventListener("click",async()=>{const u=parseInt(document.getElementById("sell-price-form").value);if(!u||u<1){n("Prix invalide","error");return}await v.from("cards").update({is_for_sale:!0,sale_price:u}).eq("id",t.id),await v.from("market_listings").insert({seller_id:r.profile.id,card_id:t.id,price:u}),n("Carte mise en vente sur le marché !","success"),a(),d("collection")}),(y=document.getElementById("cancel-sell-form-btn"))==null||y.addEventListener("click",async()=>{await v.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",t.id),await v.from("market_listings").update({status:"cancelled"}).eq("card_id",t.id).eq("status","active"),n("Annonce retirée","success"),a(),d("collection")})}async function Mo(t,e,i,o){var ue,ge,xe,de,j,Y,re,te,X,P,J;const{state:r,toast:n,openModal:a,closeModal:d,navigate:c,refreshProfile:s}=o,l=t.player,p=e.filter(F=>F.player.id===l.id),x=p.length,g=t.evolution_bonus||0,b=Math.max((Number(l.note_g)||0)+(l.job==="GK"||l.job2==="GK"?g:0),(Number(l.note_d)||0)+(l.job==="DEF"||l.job2==="DEF"?g:0),(Number(l.note_m)||0)+(l.job==="MIL"||l.job2==="MIL"?g:0),(Number(l.note_a)||0)+(l.job==="ATT"||l.job2==="ATT"?g:0)),m=l.rarity||"normal",{data:y}=await v.from("sell_price_configs").select("*").eq("rarity",m).lte("note_min",b).gte("note_max",b).order("note_min",{ascending:!1}).limit(1),u=((ue=y==null?void 0:y[0])==null?void 0:ue.price)??_r[m]??1e3,w=((ge=y==null?void 0:y[0])==null?void 0:ge.price_min)??null,h=((xe=y==null?void 0:y[0])==null?void 0:xe.price_max)??null;Bt(l);const $=gi(l,l.job)+g,f=l.job2?gi(l,l.job2)+(gi(l,l.job2)>0?g:0):null;ei[l.job],l.job2&&ei[l.job2];const T=vr[l.rarity]||"#ccc",E=$r[l.country_code]||l.country_code||"",I=t.evolution_bonus||0,V=$+I,W=f||0,K=W>0?W+I:0,L=p.map(F=>F.id);let _={};if(L.length){const{data:F}=await v.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",L).order("transferred_at",{ascending:!0});(F||[]).forEach(H=>{_[H.card_id]||(_[H.card_id]=[]),_[H.card_id].push(H)})}const N=L.length?`
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;background:#f0fdf4;border-radius:10px;padding:10px 14px">
          <div>
            ...
            <div style="font-size:11px;color:#555;margin-top:2px">
              ${(t.evolution_bonus||0)>0?`+${t.evolution_bonus} appliqué${t.evolution_bonus>1?"s":""} · Note : ${V}${l.job2&&W>0?` / ${K}`:""}`:`Note actuelle : ${V}${l.job2&&W>0?` / ${K}`:""}`}
            </div>
          </div>
      ...
      <div style="flex:1;min-width:220px;display:flex;flex-direction:column;gap:12px">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px 16px;background:#f8f8f8;border-radius:10px;padding:10px 12px">
          <div>
            ...
            <div style="font-weight:700;font-size:13px;color:#333">${E||"—"}</div>
          </div>
          <div>
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(j=document.getElementById("close-detail"))==null||j.addEventListener("click",d);let G=new Set;const le=l.rarity==="pepite"?1.3:l.rarity==="papyte"?.7:1;function pe(){let F=0;return document.querySelectorAll(".expl-check:checked").forEach(H=>{if(H.dataset.id===t.id)return;const ce=Number(H.dataset.evo)||0,be=Number(H.dataset.note)||0;F+=be+ce}),Math.round(F*le)}const M=()=>{const F=G.size,H=document.getElementById("sell-action-panel");if(!H)return;H.style.display=F>0?"block":"none",document.getElementById("sell-selected-count").textContent=F;const ae=document.getElementById("evolve-btn");if(ae){const ce=pe();ae.textContent=ce>0?`⬆️ Évoluer (+${ce})`:"⬆️ Évoluer"}};document.querySelectorAll(".expl-check").forEach(F=>{F.addEventListener("change",()=>{const H=F.dataset.id;F.checked?G.add(H):G.delete(H);const ae=F.closest(".exemplaire-row");if(ae){const ce=ae.querySelector(".expl-sel-overlay"),be=ae.querySelector(".expl-sel-check");ce&&(ce.style.display=F.checked?"block":"none"),be&&(be.style.display=F.checked?"flex":"none"),ae.style.transform=F.checked?"scale(1.05)":"scale(1)"}M()})}),document.querySelectorAll(".exemplaire-row").forEach(F=>{F.addEventListener("click",H=>{if(H.target.tagName==="INPUT")return;const ae=F.querySelector(".expl-check");ae&&!ae.disabled&&(ae.checked=!ae.checked,ae.dispatchEvent(new Event("change")))})}),(Y=document.getElementById("evolve-btn"))==null||Y.addEventListener("click",async()=>{if(x<=1)return;const F=[...G];if(!F.length)return;if(G.has(t.id)){const ye=document.createElement("div");ye.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",ye.innerHTML=`
        ...
        <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center;box-shadow:0 16px 48px rgba(0,0,0,0.3)">
          ...
          <div style="background:#f0fdf4;border-radius:10px;padding:12px;margin-bottom:16px;font-size:13px;color:#333">
            ...
            📊 Note finale : <strong>${$}</strong> → <strong style="color:#1A6B3C">${$+ce}</strong>
            ${f&&f>0?`<br>📊 Note 2 finale : <strong>${f}</strong> → <strong style="color:#1A6B3C">${f+ce}</strong>`:""}
          </div>
        ...
        </div>`,document.body.appendChild(Le),Le.querySelector("#sac-cancel").addEventListener("click",()=>{Le.remove(),ye(!1)}),Le.querySelector("#sac-ok").addEventListener("click",()=>{Le.remove(),ye(!0)}),Le.addEventListener("click",De=>{De.target===Le&&(Le.remove(),ye(!1))})}))return;if(H.length){await v.from("market_listings").delete().in("card_id",H),await v.from("deck_cards").delete().in("card_id",H),await v.from("transfer_history").delete().in("card_id",H),await v.from("decks").update({stadium_card_id:null}).in("stadium_card_id",H);const{error:ye}=await v.from("cards").delete().in("id",H);if(ye){n("Erreur suppression : "+ye.message,"error");return}}const Ee=(t.evolution_bonus||0)+ce,{error:Se}=await v.from("cards").update({evolution_bonus:Ee}).eq("id",t.id);if(Se){n("Erreur évolution : "+Se.message,"error");return}n(`⬆️ ${l.firstname} ${l.surname_real} : note ${$} → ${$+Ee} (+${ce}) !`,"success",4e3),d(),c("collection")});async function O(F){const{data:H}=await v.from("sell_price_configs").select("price_min, price_max").eq("rarity",l.rarity).lte("note_min",$).gte("note_max",$).order("note_max",{ascending:!0}).limit(1).maybeSingle();return H?F<H.price_min||F>H.price_max?{ok:!1,min:H.price_min,max:H.price_max}:{ok:!0}:{ok:!0}}(re=document.getElementById("market-sell-btn"))==null||re.addEventListener("click",async()=>{var Ee;const F=[...G];if(!F.length){n("Sélectionne au moins un exemplaire","warning");return}const H=parseInt((Ee=document.getElementById("sell-market-price"))==null?void 0:Ee.value);if(!H||H<1){n("Prix invalide","error");return}const ae=await O(H);if(!ae.ok){n(`Prix hors grille : entre ${ae.min.toLocaleString("fr")} et ${ae.max.toLocaleString("fr")} cr. pour cette rareté/note`,"error");return}const{error:ce}=await v.from("cards").update({is_for_sale:!0,sale_price:H}).in("id",F);if(ce){n(ce.message,"error");return}const be=F.map(Se=>({seller_id:r.profile.id,card_id:Se,price:H,status:"active"})),{error:ke}=await v.from("market_listings").insert(be);ke&&console.warn("[Market] insert listings:",ke.message),n(`${F.length} carte${F.length>1?"s":""} mise${F.length>1?"s":""} en vente à ${H.toLocaleString("fr")} cr. chacune !`,"success"),d(),c("collection")}),(te=document.getElementById("single-sell-btn"))==null||te.addEventListener("click",async()=>{var be;const F=parseInt((be=document.getElementById("single-sell-price"))==null?void 0:be.value);if(!F||F<1){n("Prix invalide","error");return}const H=await O(F);if(!H.ok){n(`Prix hors grille : entre ${H.min.toLocaleString("fr")} et ${H.max.toLocaleString("fr")} cr. pour cette rareté/note`,"error");return}const{error:ae}=await v.from("cards").update({is_for_sale:!0,sale_price:F}).eq("id",t.id);if(ae){n(ae.message,"error");return}const{error:ce}=await v.from("market_listings").insert({seller_id:r.profile.id,card_id:t.id,price:F,status:"active"});ce&&console.warn("[Market] insert listing:",ce.message),n(`Carte mise en vente à ${F.toLocaleString("fr")} cr. !`,"success"),d(),c("collection")}),(X=document.getElementById("single-direct-sell-btn"))==null||X.addEventListener("click",async()=>{const F=w??u;if(!confirm(`Vendre cette carte immédiatement pour ${F.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;await v.from("market_listings").delete().eq("card_id",t.id),await v.from("transfer_history").delete().eq("card_id",t.id);const{error:H}=await v.from("cards").delete().eq("id",t.id);if(H){n(H.message,"error");return}await v.from("users").update({credits:(r.profile.credits||0)+F}).eq("id",r.profile.id),await s(),n(`+${F.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),d(),c("collection")}),(P=document.getElementById("market-direct-sell-btn"))==null||P.addEventListener("click",async()=>{const F=[...G];if(!F.length){n("Sélectionne au moins un exemplaire","warning");return}const ae=(w??u)*F.length;if(!confirm(`Vendre ${F.length} carte${F.length>1?"s":""} immédiatement pour ${ae.toLocaleString("fr")} crédits au total ? Cette action est irréversible.`))return;await v.from("market_listings").delete().in("card_id",F),await v.from("transfer_history").delete().in("card_id",F);const{error:ce}=await v.from("cards").delete().in("id",F);if(ce){n(ce.message,"error");return}await v.from("users").update({credits:(r.profile.credits||0)+ae}).eq("id",r.profile.id),await s(),n(`+${ae.toLocaleString("fr")} crédits ! ${F.length} carte${F.length>1?"s":""} vendue${F.length>1?"s":""}.`,"success"),d(),c("collection")}),(J=document.getElementById("cancel-sell-btn"))==null||J.addEventListener("click",async()=>{await v.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",t.id),await v.from("market_listings").update({status:"cancelled"}).eq("card_id",t.id).eq("status","active"),n("Annonce retirée","success"),d(),c("collection")})}function Co(t,e=""){return new Promise(i=>{const o=document.createElement("div");o.className="modal-overlay",o.style.zIndex="2100",o.innerHTML=`<div class="modal" style="max-width:360px">
  ...
  </div>`,document.getElementById("new-deck-btn").addEventListener("click",async()=>{const c=await Co("Nom du deck",`Deck ${((n==null?void 0:n.length)||0)+1}`);if(!c)return;const{data:s,error:l}=await v.from("decks").insert({owner_id:i.profile.id,name:c}).select().single();if(l){r(l.message,"error");return}r("Deck créé !","success"),jo(s.id,t,e)}),t.querySelectorAll("[data-open-deck]").forEach(c=>{c.addEventListener("click",()=>jo(c.dataset.openDeck,t,e))}),t.querySelectorAll("[data-rename]").forEach(c=>{c.addEventListener("click",async()=>{const s=await Co("Nouveau nom",c.dataset.name);if(!s||s===c.dataset.name)return;const{error:l}=await v.from("decks").update({name:s}).eq("id",c.dataset.rename);if(l){r(l.message,"error");return}r("Deck renommé !","success"),Ui(t,e)})}),t.querySelectorAll("[data-delete]").forEach(c=>{c.addEventListener("click",async()=>{if(!await Tr(`Supprimer le deck "${c.dataset.name}" ? Cette action est irréversible.`,!0))return;await v.from("deck_cards").delete().eq("deck_id",c.dataset.delete);const{error:s}=await v.from("decks").delete().eq("id",c.dataset.delete);if(s){r(s.message,"error");return}r("Deck supprimé.","success"),Ui(t,e)})})}async function jo(t,e,i){const{state:o,toast:r}=i;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:n}=await v.from("decks").select("*").eq("id",t).single(),{data:a}=await v.from("cards").select(`id, card_type, formation, stadium_id, evolution_bonus,
      player:players(id, firstname, surname_real, country_code, club_id, job, job2,
        note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length, face,
        clubs(encoded_name, logo_url))`).eq("owner_id",o.profile.id),{data:d}=await v.from("cards").select(`id, card_type, stadium_id,
      stadium_def:stadium_definitions(id, name, club_id, country_code, image_url,
        club:clubs(encoded_name, logo_url))`).eq("owner_id",o.profile.id).eq("card_type","stadium"),c=(a||[]).filter(u=>u.card_type==="player"&&u.player),s=(a||[]).filter(u=>u.card_type==="formation"),l=(d||[]).filter(u=>u.card_type==="stadium"),p=[...new Set(l.map(u=>u.stadium_id).filter(Boolean))];let x={};if(l.forEach(u=>{u.stadium_def&&u.stadium_id&&(x[u.stadium_id]=u.stadium_def)}),p.length&&Object.keys(x).length<p.length){const{data:u}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",p);(u||[]).forEach(w=>{x[w.id]=w})}const g=s.map(u=>u.formation).filter(Boolean),{data:b}=await v.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",t);let m=n.formation||"4-4-2";g.length>0&&!g.includes(m)&&(m=g[0]);const y={deckId:t,name:n.name,formation:m,formationCardId:n.formation_card_id,stadiumCardId:n.stadium_card_id||null,slots:{},subs:[],playerCards:c,formationCards:s,stadiumCards:l,stadDefMap:x,availableFormations:g};(b||[]).forEach(u=>{u.is_starter?y.slots[u.position]=u.card_id:y.subs.includes(u.card_id)||y.subs.push(u.card_id)}),Lt(e,y,i,!0)}function Lt(t,e,i,o=!1){var m,y;const{navigate:r}=i;o||Cr(e,i),ki[e.formation];const n=qo(e.formation),a=n.filter(u=>e.slots[u]).length,d=e.availableFormations.length>0?e.availableFormations:Object.keys(ki),c=(m=e.stadiumCards)==null?void 0:m.find(u=>u.id===e.stadiumCardId),s=c&&((y=e.stadDefMap)==null?void 0:y[c.stadium_id])||null,l=e.subs.map(u=>e.playerCards.find(w=>w.id===u)).filter(Boolean);l.length!==e.subs.length&&(e.subs=l.map(u=>u.id)),[...Object.values(e.slots),...e.subs],t.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
  ...
  </div>`;const p=window.innerWidth>=900,x=t.querySelector(".deck-pc-layout"),g=t.querySelector(".deck-mobile-layout");x&&(x.style.display=p?"block":"none"),g&&(g.style.display=p?"none":"block"),t.querySelectorAll("#save-deck").forEach(u=>{u.closest(".page-body").style.display=p?"none":"block"}),Ir(t,e,n,i),t.querySelectorAll("#builder-back").forEach(u=>u.addEventListener("click",()=>r("decks")));const b=()=>{const{openModal:u,closeModal:w}=i,$=`<div style="display:flex;flex-wrap:wrap;gap:12px;padding:8px;justify-content:center">
      ${[...new Set(d)].map(f=>{const T=f===e.formation;return`<div data-forma="${f}" style="cursor:pointer;border-radius:10px;${T?"box-shadow:0 0 0 3px #1A6B3C":""}">
          ${si(f,ut[f],{width:90})}
        </div>`}).join("")}
    </div>`;u("⚽ Choisir une formation",$),document.querySelectorAll("#modal-body [data-forma]").forEach(f=>{f.addEventListener("click",()=>{e.formation=f.dataset.forma;const T=qo(e.formation),E={};T.forEach(I=>{e.slots[I]&&(E[I]=e.slots[I])}),e.slots=E,w(),Lt(t,e,i)})})};t.querySelectorAll("#formation-mobile-btn, #formation-pc-btn").forEach(u=>u.addEventListener("click",b)),t.querySelectorAll("#add-stad-btn-pc, #add-stad-btn").forEach(u=>u.addEventListener("click",()=>zr(e,t,i))),t.querySelectorAll("#save-deck, #save-deck-pc").forEach(u=>u.addEventListener("click",()=>jr(e,i))),t.querySelectorAll("[data-remove-sub]").forEach(u=>{u.addEventListener("click",()=>{e.subs=e.subs.filter(w=>w!==u.dataset.removeSub),Lt(t,e,i)})}),t.querySelectorAll("#add-sub-btn").forEach(u=>u.addEventListener("click",()=>Mr(e,t,i)))}function Ir(t,e,i,o){var h,$,f;const r=window.innerWidth>=900,n=t.querySelector(r?"#deck-field-pc":"#deck-field-mobile");if(!n)return;const a=(h=e.stadiumCards)==null?void 0:h.find(T=>T.id===e.stadiumCardId),d=a&&(($=e.stadDefMap)==null?void 0:$[a.stadium_id])||null,c=ut[e.formation]||{},s=Ii(e.formation)||[],l={};for(const T of i){const E=e.slots[T],I=E?e.playerCards.find(B=>B.id===E):null;I!=null&&I.player?l[T]={...I.player,_evolution_bonus:I.evolution_bonus||0,face:I.player.face||null}:l[T]=null}const p=window.innerWidth>=900,x=p?window.innerWidth-280:window.innerWidth-20,g=p?Math.min(x,860):x,b=Math.round(p?g*.82:g*.85),m=p?84:44;let y="";for(const[T,E]of s){const I=c[T],B=c[E];if(!I||!B)continue;const V=I.x*g,W=Math.round(.03*b+I.y*.85*b),K=B.x*g,L=Math.round(.03*b+B.y*.85*b),_=l[T],N=l[E],G=Ft(_,N);G==="#ff3333"||G==="#cc2222"?y+=`<line x1="${V.toFixed(1)}" y1="${W.toFixed(1)}" x2="${K.toFixed(1)}" y2="${L.toFixed(1)}" stroke="${G}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(y+=`<line x1="${V.toFixed(1)}" y1="${W.toFixed(1)}" x2="${K.toFixed(1)}" y2="${L.toFixed(1)}" stroke="${G}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,y+=`<line x1="${V.toFixed(1)}" y1="${W.toFixed(1)}" x2="${K.toFixed(1)}" y2="${L.toFixed(1)}" stroke="${G}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let u="";const w=Math.round(m*657/507);for(const T of i){const E=c[T];if(!E)continue;const I=l[T],B=E.x*g;E.y*b;const V=Math.round(.03*b+E.y*(.85*b)),W=Math.round(B-m/2),K=Math.round(V-w/2);if(I){const L=T.replace(/\d+/,""),_=d&&(d.club_id&&String(I.club_id)===String(d.club_id)||d.country_code&&I.country_code===d.country_code),N=Ae({...I,_evolution_bonus:I._evolution_bonus||0},{width:m,showStad:!0,stadDef:d,role:L});_&&((f=d.club)!=null&&f.logo_url||d.image_url),u+=`<div style="position:absolute;left:${W}px;top:${K}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${T}">
        <div style="position:relative">${N}</div>
      </div>`}else{const L=T.replace(/\d+/,"");u+=`<div style="position:absolute;left:${W}px;top:${K}px;width:${m}px;height:${w}px;
    ...
    <div style="position:relative;width:${g}px;height:${b}px;margin:0 auto">
      <svg style="position:absolute;inset:0;width:100%;height:100%;pointer-events:none" viewBox="0 0 ${g} ${b}">${y}</svg>
      ${u}
      ...
      ${a.map(c=>{var g;const s=t.stadDefMap[c.stadium_id],l=((g=s==null?void 0:s.club)==null?void 0:g.logo_url)||(s==null?void 0:s.image_url)||(s!=null&&s.country_code?`https://flagsapi.com/${s.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),p=t.stadiumCardId===c.id,x=Ut((s==null?void 0:s.name)||"Stade",l,"+10⭐",{width:90});return`<div class="stad-choice" data-stad-id="${c.id}" style="cursor:pointer;position:relative;border-radius:8px;${p?"box-shadow:0 0 0 3px #4fc3f7":""}">
          ${x}
        </div>`}).join("")}
    </div>`),(d=document.getElementById("stad-none"))==null||d.addEventListener("click",()=>{t.stadiumCardId=null,r(),Lt(e,t,i)}),document.querySelectorAll(".stad-choice").forEach(c=>{c.addEventListener("click",()=>{t.stadiumCardId=c.dataset.stadId,r(),Lt(e,t,i)})})}function Ar(t,e,i,o){var b,m,y,u,w;const{openModal:r,closeModal:n}=o,a=t.replace(/\d+/,""),d=(b=e.stadiumCards)==null?void 0:b.find(h=>h.id===e.stadiumCardId),c=d&&((m=e.stadDefMap)==null?void 0:m[d.stadium_id])||null,s=e.slots[t],l=s?e.playerCards.find(h=>h.id===s):null;(y=l==null?void 0:l.player)==null||y.id;const p=new Set;Object.entries(e.slots).forEach(([h,$])=>{var T;if(h===t||!$)return;const f=e.playerCards.find(E=>E.id===$);(T=f==null?void 0:f.player)!=null&&T.id&&p.add(f.player.id)}),e.subs.forEach(h=>{var f;const $=e.playerCards.find(T=>T.id===h);(f=$==null?void 0:$.player)!=null&&f.id&&p.add($.player.id)});const x=new Set,g=e.playerCards.filter(h=>{const $=h.player;return!($.job===a||$.job2===a)||p.has($.id)||x.has($.id)?!1:(x.add($.id),!0)});g.sort((h,$)=>{const f=h.evolution_bonus||0,T=$.evolution_bonus||0,E=(a==="GK"?h.player.note_g:a==="DEF"?h.player.note_d:a==="MIL"?h.player.note_m:h.player.note_a)+(a===h.player.job||a===h.player.job2?f:0);return(a==="GK"?$.player.note_g:a==="DEF"?$.player.note_d:a==="MIL"?$.player.note_m:$.player.note_a)+(a===$.player.job||a===$.player.job2?T:0)-E}),r(`Choisir ${a} — ${t}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${e.slots[t]?`
      ...
      ${g.length>0?'<div style="display:flex;flex-wrap:wrap;gap:8px;justify-content:center">'+g.map(h=>{const $={...h.player,_evolution_bonus:h.evolution_bonus||0};return`<div class="player-option" data-card-id="${h.id}" style="cursor:pointer">
          ${Ae($,{width:100,showStad:!0,stadDef:c,role:a})}
    ...
    </div>`}const c=l=>{t.style.overflow="",t.style.height="",t.style.display="",t.style.flexDirection="",i(l)};function s(){var p,x,g;t.style.overflow="hidden",t.style.height="100%",t.style.display="flex",t.style.flexDirection="column";const l=a.length>0;t.innerHTML=`
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      <div style="flex:1;overflow-y:auto;display:flex;flex-wrap:wrap;gap:10px;justify-content:center;align-content:flex-start;padding:8px 16px 16px">
        ${n.map(b=>{const m=a.find(y=>y.gc_type===b.gc_type);return d(b,!!m)}).join("")}
      </div>
    ...
    </div>`,t.querySelectorAll(".gc-select-card").forEach(b=>{b.addEventListener("click",()=>{const m=b.dataset.id,y=n.find(w=>w.id===m);if(!y)return;const u=a.findIndex(w=>w.gc_type===y.gc_type);u>-1?a.splice(u,1):a.length<3&&a.push(y),s()})}),(p=t.querySelector("#gc-launch"))==null||p.addEventListener("click",()=>{l&&c(a)}),(x=t.querySelector("#gc-no-gc"))==null||x.addEventListener("click",()=>c([])),(g=t.querySelector("#gc-reset"))==null||g.addEventListener("click",()=>{a.length&&(a=[],s())})}s()}function Br(t,e){var r;const i=((r=e==null?void 0:e.state)==null?void 0:r.params)||{},o=t||i.matchMode||"vs_ai_easy";return o==="friend"?`Match vs ${i.friendName||"Ami"}`:o==="random"?"Match vs Random":o==="ranked"?"Match Classé":o==="mini_league"||o==="mini-league"?"🏆 Match de Mini League":`Match vs IA — ${o.replace("vs_ai_","").toUpperCase()}`}async function Dr(t,e,i){const{state:o,navigate:r}=e;t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:n}=await v.from("decks").select(`id,name,is_active,formation,stadium_card_id,
      stadium_card:cards!stadium_card_id(id,stadium_id,
        stadium_def:stadium_definitions(id,name,club_id,country_code,image_url,
          club:clubs(encoded_name,logo_url)))`).eq("owner_id",o.profile.id).order("created_at",{ascending:!1});if(!n||n.length===0){Ct(t,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>r("decks"));return}const a=n.map(x=>x.id),{data:d}=await v.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,stadium_id,evolution_bonus,
        player:players(id,firstname,surname_real,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order"),c=[...new Set((d||[]).filter(x=>{var g,b;return((g=x.card)==null?void 0:g.card_type)==="stadium"&&((b=x.card)==null?void 0:b.stadium_id)}).map(x=>x.card.stadium_id))],s={};if(c.length){const{data:x}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",c);(x||[]).forEach(g=>{s[g.id]=g}),(d||[]).forEach(g=>{var b,m;((b=g.card)==null?void 0:b.card_type)==="stadium"&&((m=g.card)!=null&&m.stadium_id)&&(g.card._stadiumDef=s[g.card.stadium_id]||null)})}let l=0;function p(){var f,T,E,I,B,V,W;const x=n[l],g=(d||[]).filter(K=>K.deck_id===x.id),b=g.filter(K=>{var L;return K.is_starter&&((L=K.card)==null?void 0:L.player)}).map(K=>Vi(K.card,K.position)),m=g.find(K=>{var L;return((L=K.card)==null?void 0:L.card_type)==="formation"}),y=x.formation||((f=m==null?void 0:m.card)==null?void 0:f.formation)||"4-4-2";let u=b.length>=11?Ei(b,y):null,w=((T=x.stadium_card)==null?void 0:T.stadium_def)||null;w&&u&&(u=ui(u,w));const h=b.length>=11;ct(t),t.style.height="100%",t.style.overflow="hidden",t.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff;position:relative">
      ...
      <div id="deck-top-bar" style="flex-shrink:0">
        ...
        <div style="display:flex;align-items:center;gap:8px;padding:6px 8px">
          ...
          <div style="flex:1;text-align:center">
            ...
            <div style="font-size:11px;opacity:.6">${y} · ${b.length}/11${x.is_active?" · ⭐":""}</div>
          </div>
      ...
      <div id="deck-swipe-zone" style="flex:1;min-height:0;overflow-y:auto;overflow-x:hidden;position:relative;touch-action:pan-y;display:flex;align-items:center;justify-content:center">
        ${u?'<div class="deck-preview-wrap" style="overflow:visible;width:100%;min-height:100%;display:flex;align-items:center;justify-content:center"></div>':`<div style="opacity:.4;text-align:center"><div style="font-size:32px">⚠️</div><div>Deck incomplet (${b.length}/11)</div></div>`}
      </div>
    ...
    </div>`,requestAnimationFrame(()=>requestAnimationFrame(function K(){const L=t.querySelector(".deck-preview-wrap"),_=t.querySelector("#deck-swipe-zone");if(!L||!_||!u)return;const N=_.clientWidth>=900,G=Math.max(200,_.clientHeight-(N?60:40)),pe=Math.max(200,_.clientWidth-16),M=N?Math.min(117,Math.max(52,Math.round(pe*.22))):Math.max(44,Math.round(pe*.168));if(G<220||pe<220){requestAnimationFrame(K);return}const O=N?null:Math.round(M*.55);L.innerHTML=Kt(u,y,null,[],pe,G,[],O),L.style.cssText=`width:${pe}px;height:${G}px;overflow:hidden;margin:${N?0:60}px auto 0`;const ue=L.querySelector("svg");ue&&(ue.style.cssText="display:block;width:100%;height:100%",ue.setAttribute("preserveAspectRatio",N?"xMidYMid meet":"none"))})),(I=document.getElementById("prev-deck"))==null||I.addEventListener("click",()=>{l>0&&(l--,p())}),(B=document.getElementById("next-deck"))==null||B.addEventListener("click",()=>{l<n.length-1&&(l++,p())}),(V=document.getElementById("validate-deck"))==null||V.addEventListener("click",()=>{if(!h)return;const K=e.state.params||{};e.navigate("match",{...K,matchMode:K.matchMode||i,deckId:x.id})}),(W=document.getElementById("cancel-deck-select"))==null||W.addEventListener("click",()=>{Ye(t),r("home")});const $=document.getElementById("deck-swipe-zone");if($){let K=0,L=0;$.addEventListener("touchstart",_=>{K=_.touches[0].clientX,L=_.touches[0].clientY},{passive:!0}),$.addEventListener("touchend",_=>{const N=_.changedTouches[0].clientX-K,G=_.changedTouches[0].clientY-L;Math.abs(N)<40||Math.abs(N)<Math.abs(G)||(N<0&&l<n.length-1?(l++,p()):N>0&&l>0&&(l--,p()))},{passive:!0})}}p()}function Et(t,e=44,i=58,o=null){return Ae(t,{width:e,showStad:!!o,stadDef:o,used:t==null?void 0:t.used})}function et(t,e,i,o,r){if(!(t!=null&&t.length))return"";const n=t.slice(0,5);let a='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return n.forEach((d,c)=>{const s=d._line||d.job||"MIL";let l=d.boost||0;if(d.stadiumBonus&&(o==="attack"&&(s==="ATT"||s==="MIL")||o==="defense"&&(s==="GK"||s==="DEF"||s==="MIL")?l+=10:o||(l+=10)),a+=Ae({...d,_evolution_bonus:0,evolution_bonus:0},{width:40,role:s,extraNote:l}),c<n.length-1){const p=n[c+1],x=cn(d,p,r)?Ft(d,p):null;a+=`<div style="width:7px;height:3px;background:${!x||x==="#ff3333"||x==="#cc2222"?"rgba(255,255,255,0.12)":x};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(a+=`<div style="margin-left:6px;background:${e};color:${e==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),a+="</div>",a}function Mi(t,e,i,o,r=310,n=310,a=[],d=null){const c=ut[e]||{},s=Ii(e)||zi[e]||[],l={},p=["ATT","MIL","DEF","GK"];for(const u of p)(t[u]||[]).forEach((h,$)=>{l[`${u}${$+1}`]=h});function x(u){const w=c[u];return w?{x:w.x*r,y:w.y*n}:null}let g="";for(const[u,w]of s){const h=x(u),$=x(w);if(!h||!$)continue;const f=l[u],T=l[w],E=Ft(f,T);E==="#00ff88"||E==="#FFD700"?(g+=`<line x1="${h.x.toFixed(1)}" y1="${h.y.toFixed(1)}" x2="${$.x.toFixed(1)}" y2="${$.y.toFixed(1)}"
        stroke="${E}" stroke-width="10" stroke-linecap="round" opacity="0.22"/>`,g+=`<line x1="${h.x.toFixed(1)}" y1="${h.y.toFixed(1)}" x2="${$.x.toFixed(1)}" y2="${$.y.toFixed(1)}"
        stroke="${E}" stroke-width="3.5" stroke-linecap="round" opacity="0.95"/>`):g+=`<line x1="${h.x.toFixed(1)}" y1="${h.y.toFixed(1)}" x2="${$.x.toFixed(1)}" y2="${$.y.toFixed(1)}"
        stroke="${E}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const b=typeof window<"u"&&window.innerWidth>=900?Math.min(Math.max(81,Math.round(r*.225)),117):Math.max(44,Math.round(r*.168)),m=Math.round(b*657/507);for(const[u,w]of Object.entries(l)){const h=x(u);if(!h||!w)continue;const $=u.replace(/[0-9]/g,""),f=a.includes(w.cardId),T=i==="attack"&&(["MIL","ATT"].includes($)||f)&&!w.used||i==="defense"&&["GK","DEF","MIL"].includes($)&&!w.used,E=o.includes(w.cardId);let I=w.boost||0,B=!1;w.stadiumBonus&&(i==="attack"&&($==="ATT"||$==="MIL")||i==="defense"&&($==="GK"||$==="DEF"||$==="MIL")?(I+=10,B=!0):i||(I+=10,B=!0));const V=Math.round(h.x-b/2),W=Math.round(h.y-m/2);if(w.used){g+=`<image href="${`${typeof import.meta<"u"&&"/"||"/"}icons/carte-dos.png`}" x="${V}" y="${W}" width="${b}" height="${m}" preserveAspectRatio="xMidYMid slice" class="match-used-hit" data-card-id="${w.cardId}" data-role="${$}" style="cursor:pointer"/>`;continue}const K=Ae({...w,_evolution_bonus:0,stadiumBonus:!1},{width:b,showStad:!1,stadDef:null,role:$,extraNote:I,_cardOffset:30,_forceStadColor:B}),L=E?`position:absolute;top:30px;left:0;width:${b}px;height:${m}px;outline:3px solid #FFD700;outline-offset:2px;border-radius:8px;pointer-events:none;`:"";g+=`<foreignObject x="${V-2}" y="${W-30}" width="${b+8}" height="${m+60}" style="overflow:visible">
      <div xmlns="http://www.w3.org/1999/xhtml" style="position:relative">
        ${K}
        ${E?`<div style="${L}"></div>`:""}
      </div>
    </foreignObject>`,T&&(g+=`<rect x="${V}" y="${W}" width="${b}" height="${m}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${E?"selected":""}" data-card-id="${w.cardId}" data-role="${$}" style="cursor:pointer"/>`)}const y=d!==null?d:Math.round(Math.max(b*.7,80));return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-y} ${-y} ${r+y*2} ${n+y*2}" width="100%" style="display:block;width:100%;margin:0 auto">
    ${g}
  </svg>`}function Kt(t,e,i,o,r=300,n=300,a=[],d=null){return`<div id="match-terrain-wrap" style="position:relative;padding:0 4px">
    ${Mi(t,e,i,o,r,n,a,d)}
  </div>`}async function Ci(t,e,i,o){var T;const{state:r,navigate:n,toast:a}=e;ct(t);const d=r.params||{};if(t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!d.deckId)return Dr(t,e,i);const c=d.deckId;let s,l,p,x;try{const E=await Promise.all([v.from("decks").select("formation,name,stadium_card_id").eq("id",c).single(),v.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation, evolution_bonus,
            player:players(id,firstname,surname_real,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
              clubs(encoded_name,logo_url)))`).eq("deck_id",c).order("slot_order")]);s=E[0].data,p=E[0].error,l=E[1].data,x=E[1].error}catch(E){console.error("[Match] Exception chargement deck:",E),Ct(t,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>n("home"));return}if(p||x){console.error("[Match] Erreur Supabase:",p||x),Ct(t,"⚠️","Erreur lors du chargement du deck.","Retour",()=>n("home"));return}const g=(l||[]).filter(E=>{var I;return E.is_starter&&((I=E.card)==null?void 0:I.player)}).map(E=>Vi(E.card,E.position)),b=(l||[]).filter(E=>{var I;return!E.is_starter&&((I=E.card)==null?void 0:I.player)}).map(E=>Vi(E.card,E.position));if(g.length<11){Ct(t,"⚠️",`Deck incomplet (${g.length}/11).`,"Compléter",()=>n("decks"));return}const m=(l||[]).find(E=>{var I;return((I=E.card)==null?void 0:I.card_type)==="formation"}),y=(s==null?void 0:s.formation)||((T=m==null?void 0:m.card)==null?void 0:T.formation)||"4-4-2",{data:u,error:w}=await v.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",r.profile.id).eq("card_type","game_changer"),{data:h}=await v.from("gc_definitions").select("*").eq("is_active",!0),$=(u||[]).map(E=>({...E,_gcDef:(h==null?void 0:h.find(I=>I.name===E.gc_type||I.id===E.gc_definition_id))||null}));let f=null;if(s!=null&&s.stadium_card_id){const{data:E}=await v.from("cards").select("stadium_id").eq("id",s.stadium_card_id).single();if(E!=null&&E.stadium_id){const{data:I}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").eq("id",E.stadium_id).single();f=I||null}}o({deckId:c,formation:y,starters:g,subsRaw:b,gcCardsEnriched:$,gcDefs:h||[],stadiumDef:f})}function jt(){return Math.min(window.innerWidth-40,860)}function Ht(){return Math.round(jt()*1.1)}function gn(t){var r,n;if(!t)return null;const e=t._line||t.job||"MIL",i=e==="GK"?t.note_g||0:e==="DEF"?t.note_d||0:e==="MIL"?t.note_m||0:t.note_a||0,o=t.stadiumBonus?10:0;return{name:t.name,firstname:t.firstname||"",note:i+(t.boost||0)+o,note_g:t.note_g||0,note_d:t.note_d||0,note_m:t.note_m||0,note_a:t.note_a||0,_evolution_bonus:0,stadiumBonus:t.stadiumBonus||!1,boost:t.boost||0,job:t.job,job2:t.job2||null,_line:t._line||t.job,_col:t._col,country_code:t.country_code,club_id:t.club_id,rarity:t.rarity,clubName:t.clubName||((r=t.clubs)==null?void 0:r.encoded_name)||null,clubLogo:t.clubLogo||((n=t.clubs)==null?void 0:n.logo_url)||null,face:t.face||null,portrait:Bt(t)}}function Fr(t,e,i,o="Adversaire"){Wo("/sounds/match-opening.mp3",.3);const r=jt(),n=Ht();return`
    ...
    </div>`}function tt(t,e,i,o,r){io("/sounds/goal.mp3",.7);const n=document.getElementById("goal-anim-overlay");n&&n.remove();const a=document.createElement("div");a.id="goal-anim-overlay",a.style.cssText=`
    ...
    </div>`}async function Rr(t,e){const{state:i}=e,o=i.params||{},r=o.matchMode||"vs_ai_easy",n=r==="solo",a=r==="ranked_ai",d=o.soloLevel||1,c=r.replace("vs_ai_",""),s=r;let l=null;if(n){const{data:b}=await v.from("solo_levels").select("*").eq("level_number",d).maybeSingle();l=b||{level_number:d,target_note_avg:10,nb_liens_jaune:2,nb_liens_vert:1,nb_joueurs_stade:2,reward_credits:500}}const p=o.rankedData||null,x=o.presetSetup||null;async function g({deckId:b,formation:m,starters:y,subsRaw:u,gcCardsEnriched:w,gcDefs:h,stadiumDef:$}){try{let f=Ei(y,m);$&&(f=ui(f,$),$i(u,$));let T=null;if(a){const V=Gr(f),{jaune:W,vert:K}=Or(f,m),L=.9+Math.random()*.2;T={target_note_avg:Math.max(1,Math.round(V*L/16)),nb_liens_jaune:W,nb_liens_vert:K,nb_joueurs_stade:$?3:0}}const E=n?await Do(m,l):a?await Do(m,T):await Nr(m,c),I=E.lines||E,B=async V=>{try{const W=s==="vs_ai_club"||s==="solo"||s==="ranked_ai"?"club":s,{data:K,error:L}=await v.from("matches").insert({home_id:i.profile.id,away_id:null,mode:W,home_deck_id:b,status:"in_progress"}).select().single();if(L){console.error("[MatchIA] Erreur création match:",L),Ct(t,"⚠️","Impossible de créer le match ("+L.message+").","Retour",()=>e.navigate("home"));return}const _=E.stadiumDef||null;_&&I&&(ui(I,_),$i(E.subs||[],_));const N={gcDefs:h||[],matchId:K==null?void 0:K.id,mode:s,difficulty:c,formation:m,isSolo:n,soloLevel:d,soloLevelConfig:l,isRankedAI:a,rankedAIData:p,homeTeam:f,aiTeam:I,homeSubs:u,subsUsed:0,maxSubs:Math.min(u.length,3),aiSubs:E.subs||[],aiSubsUsed:0,aiMaxSubs:Math.min((E.subs||[]).length,3),aiUsedSubIds:[],aiGcCards:E.gcCards||[],aiUsedGc:[],aiStadiumDef:_,homeScore:0,aiScore:0,gcCards:V,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Hr(t,N,e)}catch(W){console.error("[MatchIA] Exception launchMatch:",W),Ct(t,"⚠️","Erreur au lancement du match : "+W.message,"Retour",()=>e.navigate("home"))}};if(a&&x){B(x.gcCardsEnriched||[]);return}if(!w.length){B([]);return}Ai(t,w,B)}catch(f){console.error("[MatchIA] Exception setup:",f),Ct(t,"⚠️","Erreur de préparation du match : "+f.message,"Retour",()=>e.navigate("home"))}}x?await g(x):await Ci(t,e,r,g)}async function Nr(t,e){var y;const{data:i}=await v.from("players").select("id,firstname,surname_real,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(80);if(!i||i.length<11)return{lines:yn(t),subs:[],gcCards:[],stadiumDef:null};const o=St[t]||St["4-4-2"],r={GK:[],DEF:[],MIL:[],ATT:[]},n=new Set;function a(u,w,h){var $,f;return n.add(u.id),{cardId:"ai-"+u.id+"-"+h,id:u.id,firstname:u.firstname,name:u.surname_real,country_code:u.country_code,club_id:u.club_id,job:u.job,job2:u.job2,note_g:Number(u.note_g)||0,note_d:Number(u.note_d)||0,note_m:Number(u.note_m)||0,note_a:Number(u.note_a)||0,rarity:u.rarity,skin:u.skin,hair:u.hair,hair_length:u.hair_length,face:u.face,clubName:(($=u.clubs)==null?void 0:$.encoded_name)||null,clubLogo:((f=u.clubs)==null?void 0:f.logo_url)||null,boost:0,used:!1,_line:w}}for(const u of["GK","DEF","MIL","ATT"]){const w=i.filter(E=>E.job===u&&!n.has(E.id)),h=i.filter(E=>E.job!==u&&!n.has(E.id)),$=[...w,...h],f=[];for(let E=0;E<o[u];E++){const I=$[E];I&&f.push(a(I,u,E))}const T=lt(f.length);f.forEach((E,I)=>{E._col=T[I]}),r[u]=f}const c=i.filter(u=>!n.has(u.id)).slice(0,5).map((u,w)=>a(u,u.job,100+w)),{data:s}=await v.from("gc_definitions").select("*").eq("is_active",!0),p=[...s||[]].sort(()=>Math.random()-.5).slice(0,3).map((u,w)=>({id:"ai-gc-"+w,gc_type:u.name,name:u.name,icon:"⚡",effect_type:u.effect_type,effect_params:u.effect_params||{}})),x=Object.values(r).flat(),g={};x.forEach(u=>{u.club_id&&(g[u.club_id]=(g[u.club_id]||0)+1)});const b=(y=Object.entries(g).sort((u,w)=>w[1]-u[1])[0])==null?void 0:y[0];let m=null;if(b){const{data:u}=await v.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",b).single();u&&(m={club_id:u.id,country_code:null,name:u.encoded_name+" Stadium",club:{encoded_name:u.encoded_name,logo_url:u.logo_url}})}return{lines:r,subs:c,gcCards:p,stadiumDef:m}}function Gr(t){let e=0;for(const i of["GK","DEF","MIL","ATT"])(t[i]||[]).forEach(o=>{const r=o._line||i,n=Number(r==="GK"?o.note_g:r==="DEF"?o.note_d:r==="MIL"?o.note_m:o.note_a)||0;e+=n+(o.boost||0)});return e}function Or(t,e){const i={};for(const a of["GK","DEF","MIL","ATT"])(t[a]||[]).forEach((d,c)=>{i[`${a}${c+1}`]=d});const o=Ii(e)||zi[e]||[];let r=0,n=0;for(const[a,d]of o){const c=i[a],s=i[d];if(!c||!s)continue;const l=Ft(c,s);l==="#00ff88"?n++:l==="#FFD700"&&r++}return{jaune:r,vert:n}}function Bo(t){const e=t.job||"ATT";return Number(e==="GK"?t.note_g:e==="DEF"?t.note_d:e==="MIL"?t.note_m:t.note_a)||0}function bi(t,e,i){if(i<=0||!t.length)return[];const o=[...t].sort((r,n)=>Math.abs(Bo(r)-e)-Math.abs(Bo(n)-e));return o.slice(0,Math.min(i,o.length))}async function Do(t,e){var K,L;const i=Number(e==null?void 0:e.target_note_avg)||10,o=Number(e==null?void 0:e.nb_liens_jaune)||0,r=Number(e==null?void 0:e.nb_liens_vert)||0,n=Number(e==null?void 0:e.nb_joueurs_stade)||0,{data:a}=await v.from("players").select("id,firstname,surname_real,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,clubs(encoded_name,logo_url,country_code)").eq("is_active",!0).limit(300);if(!a||a.length<16)return{lines:yn(t),subs:[],gcCards:[],stadiumDef:null};const d={};a.forEach(_=>{_.club_id&&(d[_.club_id]=(d[_.club_id]||0)+1)});const c=((K=Object.entries(d).sort((_,N)=>N[1]-_[1])[0])==null?void 0:K[0])||null,s=c?a.filter(_=>_.club_id===c):[],l=((L=s[0])==null?void 0:L.country_code)||null,p=l?a.filter(_=>_.country_code===l&&_.club_id!==c):[],x=Math.min(s.length,Math.max(2,r+1),16),g=Math.min(p.length,Math.max(0,o),16-x),b=[],m=new Set;bi(s,i,x).forEach(_=>{b.push(_),m.add(_.id)}),bi(p,i,g).forEach(_=>{b.push(_),m.add(_.id)});const y=Math.max(0,16-b.length),u=a.filter(_=>!m.has(_.id));bi(u,i,y).forEach(_=>{b.push(_),m.add(_.id)});const w=St[t]||St["4-4-2"],h={GK:[],DEF:[],MIL:[],ATT:[]},$=new Set;function f(_,N,G){var le,pe;return $.add(_.id),{cardId:"ai-"+_.id+"-"+G,id:_.id,firstname:_.firstname,name:_.surname_real,country_code:_.country_code,club_id:_.club_id,job:_.job,job2:_.job2,note_g:Number(_.note_g)||0,note_d:Number(_.note_d)||0,note_m:Number(_.note_m)||0,note_a:Number(_.note_a)||0,rarity:_.rarity,skin:_.skin,hair:_.hair,hair_length:_.hair_length,face:_.face,clubName:((le=_.clubs)==null?void 0:le.encoded_name)||null,clubLogo:((pe=_.clubs)==null?void 0:pe.logo_url)||null,boost:0,used:!1,_line:N}}for(const _ of["GK","DEF","MIL","ATT"]){const N=b.filter(O=>O.job===_&&!$.has(O.id)),G=b.filter(O=>O.job!==_&&!$.has(O.id)),le=[...N,...G],pe=[];for(let O=0;O<w[_];O++){const ue=le[O];ue&&pe.push(f(ue,_,O))}const M=lt(pe.length);pe.forEach((O,ue)=>{O._col=M[ue]}),h[_]=pe}let T=b.filter(_=>!$.has(_.id));if(T.length<5){const _=a.filter(N=>!$.has(N.id)&&!T.some(G=>G.id===N.id));T=T.concat(bi(_,i,5-T.length))}const E=T.slice(0,5).map((_,N)=>f(_,_.job,100+N)),{data:I}=await v.from("gc_definitions").select("*").eq("is_active",!0),V=[...I||[]].sort(()=>Math.random()-.5).slice(0,3).map((_,N)=>({id:"ai-gc-"+N,gc_type:_.name,name:_.name,icon:"⚡",effect_type:_.effect_type,effect_params:_.effect_params||{}}));let W=null;if(n>0&&c){const{data:_}=await v.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",c).single();_&&(W={club_id:_.id,country_code:null,name:_.encoded_name+" Stadium",club:{encoded_name:_.encoded_name,logo_url:_.logo_url}})}return{lines:h,subs:E,gcCards:V,stadiumDef:W}}function yn(t){const e=St[t]||St["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},o=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let r=0;for(const n of["GK","DEF","MIL","ATT"]){const a=[];for(let c=0;c<e[n];c++){const s=3+Math.floor(Math.random()*5);a.push({cardId:"fake-"+r,id:"fake-"+r,firstname:"IA",name:o[r%o.length],country_code:"XX",club_id:null,job:n,job2:null,note_g:n==="GK"?s:2,note_d:n==="DEF"?s:2,note_m:n==="MIL"?s:2,note_a:n==="ATT"?s:2,rarity:"normal",boost:0,used:!1,_line:n}),r++}const d=lt(a.length);a.forEach((c,s)=>{c._col=d[s]}),i[n]=a}return i}function Hr(t,e,i){var r;Wo("/sounds/match-opening.mp3",.3);const o=`<div style="position:relative;width:22px;height:22px;flex-shrink:0;display:flex;align-items:center;justify-content:center">
  ...
  <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:clamp(6px,1.5vh,20px);padding:clamp(8px,1.5vw,24px);background:#0a3d1e">
    ...
    <div id="duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center, rgba(10,61,30,0.4), rgba(10,61,30,0.92))"></div>
  </div>`;const l=()=>{const b=(m,y)=>t.querySelectorAll(m).forEach((u,w)=>{setTimeout(()=>{u.style.opacity="1",u.style.transform="translateY(0) scale(1)"},y+w*90)});b(".duel-card-home",150),b(".duel-card-ai",500),setTimeout(()=>t.querySelectorAll(".duel-link").forEach((m,y)=>{setTimeout(()=>{m.style.opacity="1"},y*70)}),900),setTimeout(()=>{const m=t.querySelector("#vs-label");m&&(m.style.opacity="1",m.style.animation="vsFlash .5s ease"),t.querySelectorAll(".duel-score-line").forEach(y=>y.style.opacity="1")},1250),setTimeout(()=>{p("score-home",d,800),p("score-ai",c,800)},1500)};function p(b,m,y){const u=document.getElementById(b);if(!u)return;const w=performance.now(),h=$=>{const f=Math.min(1,($-w)/y);u.textContent=Math.round(m*(1-Math.pow(1-f,3))),f<1?requestAnimationFrame(h):u.textContent=m};requestAnimationFrame(h)}requestAnimationFrame(l),e.attacker=s?"home":"ai";const x=s?mn():null;s&&(e.boostCard={value:x}),e.log.push({type:"duel",title:"Milieu de Terrain",homePlayers:o.map(b=>We(b)),aiPlayers:r.map(b=>We(b)),homeTotal:d,aiTotal:c,text:`Duel milieu : ${e.clubName} ${d} – ${c} IA → ${s?e.clubName+" attaque":"IA attaque"}`});const g=()=>{e.phase=e.attacker==="home"?"attack":"ai-attack",Ie(t,e,i),e.attacker==="ai"&&setTimeout(()=>fo(t,e,i),800)};setTimeout(()=>{const b=document.getElementById("score-home"),m=document.getElementById("score-ai"),y=document.getElementById(s?"duel-row-home":"duel-row-ai"),u=document.getElementById(s?"duel-row-ai":"duel-row-home"),w=s?b:m,h=s?m:b;w&&(w.style.fontSize="80px",w.style.color=s?"#FFD700":"#ff6b6b",w.style.animation="duelPulse .5s ease"+(s?", duelGlow 1.5s ease infinite .5s":"")),h&&(h.style.opacity="0.25"),setTimeout(()=>{y&&(y.style.transformOrigin="center",y.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",y.style.zIndex="5"),setTimeout(()=>{var f;const $=document.getElementById("duel-shock");if($){const T=(f=u||y)==null?void 0:f.getBoundingClientRect(),E=t.querySelector(".match-screen").getBoundingClientRect();T&&($.style.top=T.top-E.top+T.height/2+"px"),$.style.animation="shockwave .5s ease-out forwards"}u&&(u.style.transformOrigin="center",u.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var f;const $=document.getElementById("duel-finale");$&&($.innerHTML=`
          ...
          <button id="start-match-btn" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,0.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">
            ▶ Commencer le match
          </button>`,$.style.transition="opacity .45s ease",$.style.opacity="1",$.style.pointerEvents="auto",(f=document.getElementById("start-match-btn"))==null||f.addEventListener("click",g))},600)},700)},2800)}function Ie(t,e,i){var V,W,K,L,_,N,G,le,pe;const o=e.selected.map(M=>M.cardId),r=e.usedSubIds||[],n=e.homeSubs.filter(M=>!r.includes(M.cardId)),d=Object.values(e.homeTeam).flat().filter(M=>M.used).length>0&&n.length>0&&e.subsUsed<e.maxSubs,c=[...e.homeTeam.MIL||[],...e.homeTeam.ATT||[]].filter(M=>!M.used),s=[...e.aiTeam.MIL||[],...e.aiTeam.ATT||[]].filter(M=>!M.used),l=Vt(e.aiTeam),p=c.length===0&&s.length===0&&!l,x=(e.homeTeam.DEF||[]).filter(M=>!M.used),g=(e.homeTeam.GK||[]).filter(M=>!M.used);let b=[];e.phase==="attack"&&c.length===0&&(l?(b=x.map(M=>M.cardId),x.length===0&&(b=b.concat(g.map(M=>M.cardId)))):p&&(b=[...x,...g].map(M=>M.cardId))),e.log[e.log.length-1];const m=e.phase==="ai-attack"||e.phase==="ai-defense",y=e.phase==="attack",u=e.phase==="defense",w=e.phase==="finished",$=(e.homeSubs||[]).filter(M=>!(e.usedSubIds||[]).includes(M.cardId)).length>0&&e.subsUsed<e.maxSubs,f=y&&c.length===0&&b.length===0&&!$,T=e.gcCards.filter(M=>!e.usedGc.includes(M.id)),E=e.boostCard&&!e.boostUsed;t.style.overflow="hidden",t.style.height="100%",t.style.display="flex",t.style.flexDirection="column",t.innerHTML=`
  <style>
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">
      ${(()=>{if(e.phase==="defense"&&e.pendingAttack){const O=e.pendingAttack;let ue="";if(e.selected.length>0){const ge=e.selected.map(de=>({...(e.homeTeam[de._role]||[]).find(Y=>Y.cardId===de.cardId)||de,_line:de._role})),xe=pi(ge,e.modifiers.home,e.formation);ue=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
    ...
    ${(()=>{const M=window.innerWidth>=700,O=(X,P,J)=>{var ke,Ee;const F=(e.gcDefs||[]).find(Se=>Se.name===X.gc_type),H=(F==null?void 0:F.name)||X.gc_type,ae=(F==null?void 0:F.effect)||((ke=Oe[X.gc_type])==null?void 0:ke.desc)||"",ce=F!=null&&F.image_url?`/icons/${F.image_url}`:null,be=((Ee=Oe[X.gc_type])==null?void 0:Ee.icon)||"⚡";return`<div class="gc-mini" data-gc-id="${X.id}" data-gc-type="${X.gc_type}" style="flex-shrink:0;cursor:pointer">
        ...
        <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
          ${Kt(e.homeTeam,e.formation,e.phase,o,M?1300:jt(),M?600:Ht(),b)}
        </div>
      </div>`;return M?`
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;display:flex;gap:0;overflow:hidden">
      ...
      <div id="out-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
        ...
        <div>${m?Et({...m,used:!1,boost:0},u,w):"<div>—</div>"}</div>
    ...
    </div>`,(f=p.querySelector("#sub-close"))==null||f.addEventListener("click",()=>p.remove()),(T=p.querySelector("#out-up"))==null||T.addEventListener("click",()=>{d>0&&(d--,x())}),(E=p.querySelector("#out-down"))==null||E.addEventListener("click",()=>{d<n.length-1&&(d++,x())}),(I=p.querySelector("#in-up"))==null||I.addEventListener("click",()=>{s>0&&(s--,x())}),(B=p.querySelector("#in-down"))==null||B.addEventListener("click",()=>{s<a.length-1&&(s++,x())});const $=(W,K,L,_)=>{const N=p.querySelector("#"+W);if(!N)return;let G=0;N.addEventListener("touchstart",le=>{G=le.touches[0].clientY},{passive:!0}),N.addEventListener("touchend",le=>{const pe=le.changedTouches[0].clientY-G;if(Math.abs(pe)<30)return;const M=K();pe<0&&M<_-1?(L(M+1),x()):pe>0&&M>0&&(L(M-1),x())},{passive:!0})};$("in-panel",()=>s,W=>s=W,a.length),$("out-panel",()=>d,W=>d=W,n.length),(V=p.querySelector("#sub-confirm"))==null||V.addEventListener("click",W=>{if(W.preventDefault(),W.stopPropagation(),l)return;l=!0;const K=n[d],L=a[s];if(!K||!L)return;let _=null,N=-1;for(const[le,pe]of Object.entries(e.homeTeam)){const M=(pe||[]).findIndex(O=>O.cardId===K.cardId);if(M!==-1){_=le,N=M;break}}if(N===-1||!_){It("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),p.remove();return}const G={...L,_line:_,_col:K._col||0,used:!1,boost:0};e.homeTeam[_].splice(N,1,G),e.usedSubIds||(e.usedSubIds=[]),e.usedSubIds.push(L.cardId),e.subsUsed++,e.selected=[],e.log.push({type:"sub",subSide:"home",clubName:e.clubName,outPlayer:{name:K.name,firstname:K.firstname,note:Re(K,_),portrait:Bt(K),job:K.job,country_code:K.country_code,rarity:K.rarity,clubName:K.clubName,clubLogo:K.clubLogo},inPlayer:{name:L.name,firstname:L.firstname,note:Re(L,_),portrait:Bt(L),job:L.job,country_code:L.country_code,rarity:L.rarity,clubName:L.clubName,clubLogo:L.clubLogo},text:`🔄 ${L.firstname} ${L.name} remplace ${K.firstname} ${K.name}`}),p.remove(),Pr(K,L,()=>Ie(t,e,i))})}document.body.appendChild(p),x()}function Zr(t,e,i,o,r){var x,g;const n=(o.gcDefs||[]).find(b=>b.name===e),a=Oe[e]||{icon:"⚡",desc:"Carte spéciale."},d=(n==null?void 0:n.name)||e,c=(n==null?void 0:n.effect)||a.desc,s=n!=null&&n.image_url?`/icons/${n.image_url}`:null,l=a.icon||"⚡",p=document.createElement("div");p.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",p.innerHTML=`
    ${nt(d,s,l,c,{width:190})}
    <!-- Boutons -->
    <div style="display:flex;gap:12px;width:190px">
      <button id="gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
      <button id="gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
    </div>`,document.body.appendChild(p),(x=p.querySelector("#gc-back"))==null||x.addEventListener("click",()=>p.remove()),(g=p.querySelector("#gc-use"))==null||g.addEventListener("click",()=>{p.remove(),ta(t,e,i,o,r)})}function yi(t,e,i,o,r,n){const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let d=[];function c(){var s,l;a.innerHTML=`
    ...
    <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start;justify-content:center">
      ${t.map(p=>{const x=p._line||p.job||"MIL",g=d.find(m=>m.cardId===p.cardId),b=Ae({...p,_evolution_bonus:0},{width:90,showStad:!0,role:x,extraNote:p.boost||0});return`<div class="gc-pick-item" data-cid="${p.cardId}"
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(r=>{r.addEventListener("click",()=>{const n=r.dataset.cardId;for(const a of["GK","DEF","MIL","ATT"]){const d=(e.homeTeam[a]||[]).find(c=>c.cardId===n);if(d){d.boost=(d.boost||0)+e.boostCard.value,e.log.push({text:`⚡ Boost +${e.boostCard.value} appliqué à ${d.name}`,type:"info"});break}}e.boostUsed=!0,i.closeModal(),Ie(t,e,i)})})}async function kt(t,e,i){var x,g,b,m;Ot(),Ve(),e._timerInt&&(clearInterval(e._timerInt),e._timerInt=null),e.phase="finished";const{state:o}=i,r=e.homeScore>e.aiScore,n=e.homeScore===e.aiScore,a=r?"victoire":n?"nul":"defaite",d=e.isSolo&&((x=e.soloLevelConfig)!=null&&x.reward_credits)&&r?Number(e.soloLevelConfig.reward_credits):hr(e.mode,a);if(e.isSolo&&r)try{const{data:y}=await v.from("user_solo_progress").select("unlocked_level").eq("user_id",o.profile.id).maybeSingle(),u=(y==null?void 0:y.unlocked_level)||1;e.soloLevel>=u&&await v.from("user_solo_progress").upsert({user_id:o.profile.id,unlocked_level:e.soloLevel+1,updated_at:new Date().toISOString()})}catch(y){console.warn("[Solo] Erreur mise à jour progression:",y.message)}const c="00000000-0000-0000-0000-000000000000";let s=null;if(e.isRankedAI)try{const{data:y}=await v.from("users").select("mmr, mmr_deviation, mmr_volatility, placement_matches").eq("id",o.profile.id).single();if(y){const u=y.mmr??1e3,w=y.mmr_deviation??350,h=y.mmr_volatility??.06,$=r?1:n?.5:0,f=(y.placement_matches??0)<10,T=Ko(u,w,h,u,350,$,f),E=n?null:r?o.profile.id:c;await v.rpc("update_mmr_after_ranked",{p_match_id:e.matchId,p_winner_id:E,p_home_id:o.profile.id,p_away_id:c,p_home_delta:T.delta,p_away_delta:0,p_home_new_rd:T.newRd,p_away_new_rd:350,p_home_new_vol:T.newSigma,p_away_new_vol:.06}),s=T.delta}}catch(y){console.warn("[RankedAI] Erreur mise à jour MMR:",y.message)}e.matchId&&await v.from("matches").update({status:"finished",home_score:e.homeScore,away_score:e.aiScore,winner_id:r?o.profile.id:null,home_credits_reward:d,played_at:new Date().toISOString()}).eq("id",e.matchId);const l={credits:(o.profile.credits||0)+d,matches_played:(o.profile.matches_played||0)+1};r?l.wins=(o.profile.wins||0)+1:n?l.draws=(o.profile.draws||0)+1:l.losses=(o.profile.losses||0)+1,await v.from("users").update(l).eq("id",o.profile.id),await i.refreshProfile();const p=document.createElement("div");p.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",p.innerHTML=`
    <div style="text-align:center;padding:40px;color:#fff;max-width:380px;width:100%">
    ...
    </div>`,document.body.appendChild(p),(g=document.getElementById("res-home"))==null||g.addEventListener("click",()=>{p.remove(),Ye(t),i.navigate("home")}),(b=document.getElementById("res-replay"))==null||b.addEventListener("click",async()=>{if(p.remove(),Ye(t),e.isRankedAI){const{data:y}=await v.from("users").select("mmr, mmr_deviation, mmr_volatility, placement_matches").eq("id",i.state.profile.id).single();mt(),i.navigate("match",{matchMode:"ranked",rankedData:{mmr:(y==null?void 0:y.mmr)??1e3,rd:(y==null?void 0:y.mmr_deviation)??350,sigma:(y==null?void 0:y.mmr_volatility)??.06,isPlacement:((y==null?void 0:y.placement_matches)??0)<10}});return}i.navigate("match",e.isSolo?{matchMode:e.mode,soloLevel:e.soloLevel}:{matchMode:e.mode})}),(m=document.getElementById("res-next-level"))==null||m.addEventListener("click",()=>{p.remove(),Ye(t),i.navigate("match",{matchMode:"solo",soloLevel:e.soloLevel+1})})}function oa(t,e){e.openModal("Équipe adverse (IA)",`<div style="background:#0a3d1e;padding:12px;border-radius:8px">
      ${Mi(t.aiTeam,t.formation,null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.1))}
    </div>`,`<button class="btn btn-primary" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const na={pepite:{win:2,loss:-1,boosterStart:"min"},papyte:{win:1,loss:-2,boosterStart:"max"}};function ra(t){const e=t.job||"ATT";return Number(e==="GK"?t.note_g:e==="DEF"?t.note_d:e==="MIL"?t.note_m:t.note_a)||0}async function aa(t,e){!t||!e||await Promise.all([Po(t,"win"),Po(e,"loss")])}async function Po(t,e){const{data:i}=await v.from("cards").select("id, current_note, player:players(rarity, job, note_g, note_d, note_m, note_a, note_min, note_max)").eq("owner_id",t).eq("card_type","player");if(!(i!=null&&i.length))return;const o=i.filter(r=>{var n,a;return((n=r.player)==null?void 0:n.rarity)==="pepite"||((a=r.player)==null?void 0:a.rarity)==="papyte"});o.length&&await Promise.all(o.map(r=>{const n=na[r.player.rarity],a=e==="win"?n.win:n.loss,d=r.player.note_min??1,c=r.player.note_max??10,s=ra(r.player),l=r.current_note??s,p=Math.min(c,Math.max(d,l+a));return v.from("cards").update({current_note:p}).eq("id",r.id)}))}const Gt=t=>gn(t);async function Dt(t,e,i,o,r={}){return vn(t,e,i,o,r.myGC||[],r.gcDefs||[],r.isRanked||!1,r.rankedData||null,r.stadiumDef||null,r.onMatchEnd||null,r.mlLeagueId||null,r.mlMatchId||null)}async function sa(t,e,i){const{data:o}=await v.from("matches").select("home_id,away_id,mode,is_ranked").eq("id",i).single();if(!o){e.toast("Match introuvable","error"),e.navigate("home");return}const r=o.home_id===e.state.user.id;let n=null,a=null;if(o.mode==="mini_league"){const{data:d}=await v.from("mini_league_matches").select("id, league_id").eq("match_id",i).maybeSingle();d&&(n=d.league_id,a=d.id)}return vn(t,e,i,r,[],[],o.is_ranked||!1,null,null,null,n,a)}async function vn(t,e,i,o,r=[],n=[],a=!1,d=null,c=null,s=null,l=null,p=null){const{state:x,navigate:g,toast:b}=e,m=o?"p1":"p2",y=o?"p2":"p1",u=(r||[]).map(k=>k.id),w=(r||[]).map(k=>({id:k.id,gc_type:k.gc_type,_gcDef:k._gcDef||null}));t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:h}=await v.from("matches").select("*").eq("id",i).single();if(!h){b("Match introuvable","error"),g("home");return}async function $(){var _e,$e;console.log("[PvP] buildGameState match:",{id:h.id,home_deck_id:h.home_deck_id,away_deck_id:h.away_deck_id,mode:h.mode,is_ranked:h.is_ranked});const[{data:k,error:S},{data:A,error:C},{data:z},{data:R}]=await Promise.all([v.rpc("get_deck_for_match",{p_deck_id:h.home_deck_id}),v.rpc("get_deck_for_match",{p_deck_id:h.away_deck_id}),v.from("users").select("id,pseudo,club_name").eq("id",h.home_id).single(),v.from("users").select("id,pseudo,club_name").eq("id",h.away_id).single()]);console.log("[PvP] get_deck_for_match p1:",S==null?void 0:S.message,"p2:",C==null?void 0:C.message,"p1D:",(_e=k==null?void 0:k.starters)==null?void 0:_e.length,"p2D:",($e=A==null?void 0:A.starters)==null?void 0:$e.length);const q=ne=>{const ze=Number(ne.evolution_bonus)||0;return{cardId:ne.card_id,position:ne.position,id:ne.id,firstname:ne.firstname,name:ne.surname_real,country_code:ne.country_code,club_id:ne.club_id,job:ne.job,job2:ne.job2,note_g:(Number(ne.note_g)||0)+(ne.job==="GK"||ne.job2==="GK"&&Number(ne.note_g)>0?ze:0),note_d:(Number(ne.note_d)||0)+(ne.job==="DEF"||ne.job2==="DEF"&&Number(ne.note_d)>0?ze:0),note_m:(Number(ne.note_m)||0)+(ne.job==="MIL"||ne.job2==="MIL"&&Number(ne.note_m)>0?ze:0),note_a:(Number(ne.note_a)||0)+(ne.job==="ATT"||ne.job2==="ATT"&&Number(ne.note_a)>0?ze:0),evolution_bonus:ze,rarity:ne.rarity,skin:ne.skin,hair:ne.hair,hair_length:ne.hair_length,face:ne.face||null,clubName:ne.club_encoded_name||null,clubLogo:ne.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},D=((k==null?void 0:k.starters)||[]).map(ne=>q(ne)),U=((A==null?void 0:A.starters)||[]).map(ne=>q(ne)),Z=(k==null?void 0:k.formation)||"4-4-2",oe=(A==null?void 0:A.formation)||"4-4-2";let ie=Ei(D,Z),ee=Ei(U,oe);const se=((k==null?void 0:k.subs)||[]).map(ne=>q(ne)),fe=((A==null?void 0:A.subs)||[]).map(ne=>q(ne)),we=(k==null?void 0:k.stadium_def)||(o?c:null),me=(A==null?void 0:A.stadium_def)||(o?null:c);return we&&(ie=ui(ie,we),$i(se,we)),me&&(ee=ui(ee,me),$i(fe,me)),{p1Team:ie,p2Team:ee,p1Subs:se,p2Subs:fe,p1Formation:Z,p2Formation:oe,p1Name:(z==null?void 0:z.club_name)||(z==null?void 0:z.pseudo)||"Joueur 1",p2Name:(R==null?void 0:R.club_name)||(R==null?void 0:R.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:o?u:[],gc_p2:o?[]:u,gcCardsFull_p1:o?w:[],gcCardsFull_p2:o?[]:w,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:n||[],lastActionAt:new Date().toISOString()}}let f=h.game_state&&Object.keys(h.game_state).length?h.game_state:null;if(console.log("[PvP] init - amIHome:",o,"gameState exists:",!!f,"match.status:",h.status,"home_id:",h.home_id,"away_id:",h.away_id,"myId:",x.profile.id),!f)if(o){f=await $(),console.log("[PvP] home - gameState built, p1Team keys:",Object.keys((f==null?void 0:f.p1Team)||{}));const{data:k}=await v.from("matches").select("game_state").eq("id",i).single();!(k!=null&&k.game_state)||!Object.keys(k.game_state).length?await v.from("matches").update({game_state:f,turn_user_id:h.home_id}).eq("id",i):f=k.game_state}else{t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>',console.log("[PvP] away - waiting for home to write game_state...");for(let k=0;k<30&&!f;k++){await new Promise(A=>setTimeout(A,400));const{data:S}=await v.from("matches").select("game_state").eq("id",i).single();S!=null&&S.game_state&&Object.keys(S.game_state).length&&(f=S.game_state),k%5===0&&console.log("[PvP] away - poll",k,"game_state:",!!(S!=null&&S.game_state))}if(!f){b("Erreur de synchronisation","error"),g("home");return}f.gc_p2=u,f.gcCardsFull_p2=w,await v.from("matches").update({game_state:f}).eq("id",i)}let T=!1,E=null,I=!1;const B=new Set,V=new Set;async function W(k){var oe,ie;Ot();try{v.removeChannel(K)}catch{}const S=f[m+"Score"]||0,A=f[y+"Score"]||0;let C,z;k.winner_id?(C=k.winner_id===x.profile.id,z=!1):k.forfeit?(C=S>A,z=!1):(z=S===A,C=S>A);let R="";if(a&&o)try{const{data:ee}=await v.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",h.home_id).single(),{data:se}=await v.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",h.away_id).single();if(ee&&se){const fe=C?1:z?.5:0,we=1-fe,me=ee.placement_matches<10,_e=se.placement_matches<10,$e=computeGlicko2(ee.mmr,ee.mmr_deviation,ee.mmr_volatility,se.mmr,se.mmr_deviation,fe===1?1:fe===0?0:.5,me),ne=computeGlicko2(se.mmr,se.mmr_deviation,se.mmr_volatility,ee.mmr,ee.mmr_deviation,we===1?1:we===0?0:.5,_e);await v.rpc("update_mmr_after_ranked",{p_match_id:i,p_winner_id:z?null:C?h.home_id:h.away_id,p_home_id:h.home_id,p_away_id:h.away_id,p_home_delta:$e.delta,p_away_delta:ne.delta,p_home_new_rd:$e.newRd,p_away_new_rd:ne.newRd,p_home_new_vol:$e.newSigma,p_away_new_vol:ne.newSigma});const ze=$e.delta,Ne=$e.newMmr,je=getTier(Ne),Xe=ze>=0?"+":"",Ue=ze>=0?"#4caf50":"#ff6b6b",rt=getTier(ee.mmr);R=`
      ...
      <div style="font-size:18px">${f[m+"Name"]} ${S} – ${A} ${f[y+"Name"]}</div>
      ${k.forfeit?`<div style="font-size:13px;color:rgba(255,255,255,0.5)">${C?"L'adversaire a quitté":"Perdu par forfait"}</div>`:""}
      ${R}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${l?"🏆 Retour à la Mini League":a?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(q),(ie=q.querySelector("#pvp-end-home"))==null||ie.addEventListener("click",()=>{q.remove(),Ye(t),l?g("mini-league",{openLeagueId:l}):g(a?"ranked":"home")})}const K=v.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},k=>{var A;const S=k.new;try{if(S.status==="finished"||S.forfeit){if(T)return;T=!0,E&&(clearInterval(E),E=null),Ve(),S.game_state&&(f=S.game_state),W(S);return}if(S.game_state){const C=f;f=S.game_state;const z=f._lastGC;if(z&&z.seq&&!V.has(z.seq)&&(V.add(z.seq),z.by!==m)){de(z.type,z.by,()=>O());return}const R=C[m+"Score"]||0,q=C[y+"Score"]||0,D=f[m+"Score"]||0,U=f[y+"Score"]||0,Z=D>R,oe=U>q;if((Z||oe)&&!B.has(f.round)){B.add(f.round);const ie=[...f.log||[]].reverse().find(fe=>fe.isGoal),ee=((ie==null?void 0:ie.homePlayers)||[]).map(fe=>({name:fe.name,note:fe.note,portrait:fe.portrait,job:fe.job}));!!((A=ie==null?void 0:ie.text)!=null&&A.includes("DERNIER CORNER"))?ae().then(()=>{tt(ee,D,U,Z,()=>O())}):tt(ee,D,U,Z,()=>O());return}O()}}catch(C){console.error("[PvP] crash:",C)}}).subscribe();async function L(k){Object.assign(f,k),f.lastActionAt=new Date().toISOString();const{error:S}=await v.from("matches").update({game_state:f}).eq("id",i);S&&b("Erreur de synchronisation","error");try{O()}catch(A){console.error("[PvP] renderPvpScreen crash:",A)}}async function _(){if(T)return;T=!0,Ot(),Ve(),E&&(clearInterval(E),E=null);const k=o?h.away_id:h.home_id,S=o?"p2":"p1",A=o?"p1":"p2",C={...f,[S+"Score"]:3,[A+"Score"]:0,phase:"finished"},z=C.p1Score||0,R=C.p2Score||0;try{await v.from("matches").update({game_state:C}).eq("id",i);const{data:q,error:D}=await v.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:k,p_home_score:z,p_away_score:R,p_forfeit:!0});if(D&&console.error("[PvP] finish_pvp_match (forfeit):",D),l)try{await v.rpc("check_and_finish_mini_league",{p_league_id:l})}catch(U){console.error("[PvP] check_and_finish_mini_league (forfeit):",U)}try{const{data:U}=await v.rpc("apply_match_rewards",{p_match_id:i});U!=null&&U.success&&!(U!=null&&U.skipped)&&typeof e.refreshProfile=="function"&&await e.refreshProfile()}catch(U){console.error("[PvP] apply_match_rewards (forfeit):",U)}}catch{}try{v.removeChannel(K)}catch{}setTimeout(()=>{Ye(t),l?g("mini-league",{openLeagueId:l}):g("home")},800)}const N={BOOST_STAT:({value:k=1,count:S=1,roles:A=[]},C,z)=>{const R=C[m+"Team"],q=Object.entries(R).filter(([D])=>!A.length||A.includes(D)).flatMap(([D,U])=>U.filter(Z=>!Z.used).map(Z=>({...Z,_line:D})));if(!q.length){C.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),z(C);return}G(q,S,`Choisir ${S} joueur(s) à booster (+${k})`,D=>{D.forEach(U=>{const Z=(R[U._line]||[]).find(oe=>oe.cardId===U.cardId);Z&&(Z.boost=(Z.boost||0)+k,C.log.push({text:`⚡ +${k} sur ${Z.name}`,type:"info"}))}),C[m+"Team"]=R,z(C)})},DEBUFF_STAT:({value:k=1,count:S=1,roles:A=[],target:C="ai"},z,R)=>{const q=C==="home"?m:y,D=z[q+"Team"],U=C==="home"?"allié":"adverse",Z=Object.entries(D).filter(([oe])=>!A.length||A.includes(oe)).flatMap(([oe,ie])=>ie.map(ee=>({...ee,_line:oe})));if(!Z.length){z.log.push({text:`🎯 Aucun joueur ${U}`,type:"info"}),R(z);return}G(Z,S,`Choisir ${S} joueur(s) ${U}(s) (-${k})`,oe=>{oe.forEach(ie=>{const ee=(D[ie._line]||[]).find(se=>se.cardId===ie.cardId);ee&&(ee.boost=(ee.boost||0)-k,z.log.push({text:`🎯 -${k} sur ${ee.name}`,type:"info"}))}),z[q+"Team"]=D,R(z)})},GRAY_PLAYER:({count:k=1,roles:S=[],target:A="ai"},C,z)=>{const R=A==="home"?m:y,q=C[R+"Team"],D=A==="home"?"allié":"adverse",U=Object.entries(q).filter(([Z])=>!S.length||S.includes(Z)).flatMap(([Z,oe])=>oe.filter(ie=>!ie.used).map(ie=>({...ie,_line:Z})));if(!U.length){C.log.push({text:`❌ Aucun joueur ${D}`,type:"info"}),z(C);return}G(U,k,`Choisir ${k} joueur(s) ${D}(s) à exclure`,Z=>{const oe="usedCardIds_"+R,ie=new Set(C[oe]||[]);Z.forEach(ee=>{const se=(q[ee._line]||[]).find(fe=>fe.cardId===ee.cardId);se&&(se.used=!0,ie.add(ee.cardId),C.log.push({text:`❌ ${se.name} exclu !`,type:"info"}))}),C[oe]=[...ie],C[R+"Team"]=q,z(C)})},REVIVE_PLAYER:({count:k=1,roles:S=[]},A,C)=>{const z=A[m+"Team"],R=Object.entries(z).filter(([q])=>!S.length||S.includes(q)).flatMap(([q,D])=>D.filter(U=>U.used).map(U=>({...U,_line:q})));if(!R.length){A.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),C(A);return}G(R,k,`Choisir ${k} joueur(s) à ressusciter`,q=>{q.forEach(D=>{const U=(z[D._line]||[]).find(Z=>Z.cardId===D.cardId);U&&(U.used=!1,A.log.push({text:`💫 ${U.name} ressuscité !`,type:"info"}))}),A[m+"Team"]=z,C(A)})},REMOVE_GOAL:({},k,S)=>{const A=y+"Score";k[A]>0?(k[A]--,k.log.push({text:"🚫 Dernier but annulé !",type:"info"})):k.log.push({text:"🚫 Aucun but à annuler",type:"info"}),S(k)},ADD_GOAL_DRAW:({},k,S)=>{k[m+"Score"]===k[y+"Score"]?(k[m+"Score"]++,k.log.push({text:"🎯 But bonus !",type:"info"})):k.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),S(k)},ADD_SUB:({value:k=1},S,A)=>{S.maxSubs=(S.maxSubs||3)+k,S.log.push({text:`🔄 +${k} remplacement(s)`,type:"info"}),A(S)},CUSTOM:({},k,S)=>S(k)};function G(k,S,A,C){const z=document.createElement("div");z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let R=[];function q(){var U,Z;const D=k.map(oe=>{const ie=oe._line||oe.job||"MIL",ee=R.find(fe=>fe.cardId===oe.cardId),se=Ae({...oe,_evolution_bonus:0},{width:90,showStad:!0,role:ie,extraNote:oe.boost||0});return`<div class="pp-item" data-cid="${oe.cardId}"
        ...
        </div>`,(U=z.querySelector("#pp-close"))==null||U.addEventListener("click",()=>z.remove()),z.querySelectorAll(".pp-item").forEach(oe=>{oe.addEventListener("click",()=>{const ie=oe.dataset.cid,ee=k.find(fe=>fe.cardId===ie),se=R.findIndex(fe=>fe.cardId===ie);ee&&(se>-1?R.splice(se,1):R.length<S&&R.push(ee),q())})}),(Z=z.querySelector("#pp-confirm"))==null||Z.addEventListener("click",()=>{z.remove(),C(R)})}q(),document.body.appendChild(z)}async function le(k,S){const C=(f["gcCardsFull_"+m]||[]).find(D=>D.id===k),z=(C==null?void 0:C._gcDef)||(f.gcDefs||[]).find(D=>{var U;return D.name===S||((U=D.name)==null?void 0:U.toLowerCase().trim())===(S==null?void 0:S.toLowerCase().trim())}),R=[...f["usedGc_"+m]||[],k],q={type:S,by:m,seq:(f._gcAnimSeq||0)+1};V.add(q.seq),de(S,m,async()=>{if(z!=null&&z.effect_type&&z.effect_type!=="CUSTOM"){const U=N[z.effect_type];if(U){const Z={...f};U(z.effect_params||{},Z,async oe=>{oe["usedGc_"+m]=R,oe._lastGC=q,oe._gcAnimSeq=q.seq,await L(oe)});return}}const D={...f};switch(S){case"Remplacement+":D.maxSubs=(D.maxSubs||3)+1,D.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const U=y+"Score";D[U]>0&&(D[U]--,D.log.push({text:"🚫 But annulé",type:"info"}));break}}D["usedGc_"+m]=R,D._lastGC=q,D._gcAnimSeq=q.seq,await L(D)})}function pe(k,S){const A="usedCardIds_"+k,C=new Set(f[A]||[]);S.forEach(z=>C.add(z)),f[A]=[...C]}function M(){for(const k of["p1","p2"]){const S=new Set(f["usedCardIds_"+k]||[]),A=f[k+"Team"];if(A)for(const C of["GK","DEF","MIL","ATT"])(A[C]||[]).forEach(z=>{z.used=S.has(z.cardId)})}}function O(){var bo,yo,ho,vo,wo,_o;if(f.phase==="reveal")return ue();if(f.phase==="midfield")return xe();if(f.phase==="finished")return He();const k=f[m+"Team"],S=f[y+"Team"];M();const A=f[m+"Score"],C=f[y+"Score"],z=f[m+"Name"],R=f[y+"Name"],q=f.phase===m+"-attack",D=f.phase===m+"-defense",U=Array.isArray(f["selected_"+m])?f["selected_"+m]:[],Z=U.map(Q=>Q.cardId),oe=window.innerWidth>=700,ie=f[m+"Subs"]||[],ee=f["usedSubIds_"+m]||[],se=ie.filter(Q=>!ee.includes(Q.cardId)),fe=f["gcCardsFull_"+m]||[],we=f["usedGc_"+m]||[],me=fe.filter(Q=>!we.includes(Q.id)),_e=f.boostOwner===m&&!f.boostUsed,$e=[...k.MIL||[],...k.ATT||[]].filter(Q=>!Q.used),ne=[...S.MIL||[],...S.ATT||[]].filter(Q=>!Q.used),ze=!Se(S),Ne=$e.length===0&&ne.length===0&&!ze,je=(k.DEF||[]).filter(Q=>!Q.used),Xe=(k.GK||[]).filter(Q=>!Q.used);let Ue=[];q&&$e.length===0&&(ze?(Ue=je.map(Q=>Q.cardId),je.length===0&&(Ue=Ue.concat(Xe.map(Q=>Q.cardId)))):Ne&&(Ue=[...je,...Xe].map(Q=>Q.cardId)));function rt(Q,ve,qe){var Be,bt;const Te=Q._gcDef,Nt=(Te==null?void 0:Te.name)||Q.gc_type,dt=(Te==null?void 0:Te.effect)||((Be=Oe[Q.gc_type])==null?void 0:Be.desc)||"",Pe=Te!=null&&Te.image_url?`/icons/${Te.image_url}`:null,Ge=((bt=Oe[Q.gc_type])==null?void 0:bt.icon)||"⚡";return`<div class="pvp-gc-mini" data-gc-id="${Q.id}" data-gc-type="${Q.gc_type}" style="flex-shrink:0;cursor:pointer">
        ...
        <div style="font-size:${Math.round(ve*.09)}px;color:#000;font-weight:900">+${f.boostValue}</div>
      </div>`}const Fe=(Q,ve)=>ve?Xt(95,162):rt(Q,95),Je=(Q,ve)=>ve?Xt(68,116):rt(Q,68),Ke=oe?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",gt=q?Le(m)?`<button id="pvp-action" style="${Ke};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${U.length===0?"disabled":""}>⚔️ ATTAQUEZ <span id="pvp-timer"></span></button>`:`<button id="pvp-action" data-pass="1" style="${Ke};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER <span id="pvp-timer"></span></button>`:D?`<button id="pvp-action" style="${Ke};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${U.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="pvp-timer"></span></button>`:`<div style="font-size:11px;color:rgba(255,255,255,0.3);text-align:center;padding:4px">⏳ Tour de ${R}</div>`,at=q&&!Le(m)?'<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">Aucun attaquant — passez la main</div>':q||D?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${U.length}/3 sélectionné(s)</div>`:"",Pt=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${oe?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${Kt(k,f[m+"Formation"],xt,Z,oe?1300:jt(),oe?600:Ht(),Ue)}
      </div>
    </div>`,st=f[m+"Team"],Sn=(()=>{var ve,qe,Te,Nt,dt;if(D&&((ve=f.pendingAttack)!=null&&ve.players)){const Pe=f.pendingAttack;let Ge="";if(U.length>0){const Be=U.map(Qe=>{const Jt=(st[Qe._role]||[]).find(Bi=>Bi.cardId===Qe.cardId)||Qe,Qt=st[Qe._role]||[],fi=Qt.findIndex(Bi=>Bi.cardId===Qe.cardId),qi=lt(Qt.length),Zt=fi>=0?qi[fi]:Jt._col??1;return{...Jt,_line:Qe._role,_col:Zt}}),bt=pi(Be,((qe=f.modifiers)==null?void 0:qe[m])||{},f[m+"Formation"]);Ge=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
            ...
            <div style="display:flex;justify-content:center">${et(Be.map(Qe=>({...Qe,used:!1})),"#3a7bd5",bt.total,"defense",f[m+"Formation"])}</div>
          </div>`}return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b;text-align:center">
          ...
          <div style="display:flex;justify-content:center">${et((Pe.players||[]).map(Be=>({...Be,used:!1})),"#ff6b6b",Pe.total,"attack",f[y+"Formation"])}</div>
          ${Ge}
        </div>`}if(q&&((Te=f.pendingAttack)!=null&&Te.players)){const Pe=f.pendingAttack;return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #00ff88;text-align:center">
          <div style="font-size:8px;color:#00ff88;letter-spacing:2px;margin-bottom:4px;text-transform:uppercase">⚔️ VOUS ATTAQUEZ</div>
          <div style="display:flex;justify-content:center">${et((Pe.players||[]).map(Ge=>({...Ge,used:!1})),"#00ff88",Pe.total,"attack",f[m+"Formation"])}</div>
        </div>`}if(q&&!((Nt=f.pendingAttack)!=null&&Nt.players)&&U.length>0){const Pe=U.map(Be=>{const bt=(st[Be._role]||[]).find(Zt=>Zt.cardId===Be.cardId)||Be,Qe=["GK","DEF"].includes(Be._role),Jt=st[Be._role]||[],Qt=Jt.findIndex(Zt=>Zt.cardId===Be.cardId),fi=lt(Jt.length),qi=Qt>=0?fi[Qt]:bt._col??1;return{...bt,_line:Be._role,_col:qi,...Qe?{note_a:Math.max(1,Number(bt.note_a)||0)}:{}}}),Ge=ci(Pe,((dt=f.modifiers)==null?void 0:dt[m])||{},f[m+"Formation"]);return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #FFD700;text-align:center">
          ...
          <div style="display:flex;justify-content:center">${et(Pe.map(Be=>({...Be,used:!1})),"#FFD700",Ge.total,"attack",f[m+"Formation"])}</div>
        </div>`}const Q=(f.log||[]).slice(-1)[0];return Q?'<div style="padding:2px 4px">'+xn(Q)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3);text-align:center">⏳ Match en cours...</div>'})(),xo=`
      <div style="display:flex;align-items:center;padding:8px 10px;background:rgba(0,0,0,0.5);gap:6px;flex-shrink:0">
      ...
      <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
        ...
        <div id="mobile-play-area" style="flex:1;min-height:0;display:flex;overflow:hidden">
          <div id="match-field" style="flex:1;min-width:0;min-height:0;overflow:hidden">
            <div class="terrain-wrapper" style="width:100%;height:100%;overflow:hidden">
              ${Kt(k,f[m+"Formation"],xt,Z,jt(),Ht(),Ue)}
            </div>
          </div>
        </div>
        <div id="mobile-action-bar" style="position:absolute;left:0;right:0;bottom:0;z-index:20;background:rgba(0,0,0,0.55);padding:6px 8px 8px;display:flex;flex-direction:column;gap:6px;box-shadow:0 -4px 16px rgba(0,0,0,0.5)">
      ...
      </div>`;function Rt(){const Q=t.querySelector(".match-screen");if(!Q)return;const ve=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);Q.style.height=ve+"px",Q.style.minHeight=ve+"px",Q.style.maxHeight=ve+"px",Q.style.overflow="hidden";const qe=t.querySelector("#mobile-action-bar"),Te=t.querySelector("#mobile-play-area");qe&&Te&&(Te.style.paddingBottom=qe.offsetHeight+"px")}if(Rt(),setTimeout(Rt,120),setTimeout(Rt,400),I||(I=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",Rt),window.visualViewport.addEventListener("scroll",Rt)),window.addEventListener("resize",Rt)),function(){const ve=t.querySelector("#match-field .terrain-wrapper svg")||t.querySelector(".terrain-wrapper svg");if(!ve)return;const qe=ve.closest("#match-terrain-wrap");qe&&(qe.style.cssText="position:relative;width:100%;height:100%;padding:0"),ve.removeAttribute("width"),ve.removeAttribute("height"),ve.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",ve.setAttribute("preserveAspectRatio","xMidYMid meet")}(),f._pvpResizeBound||(f._pvpResizeBound=!0,window.addEventListener("resize",()=>{const Q=t.querySelector(".terrain-wrapper svg");if(Q){const ve=Q.closest("#match-terrain-wrap");ve&&(ve.style.cssText="position:relative;width:100%;height:100%;padding:0"),Q.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0"}})),t.querySelectorAll(".match-slot-hit").forEach(Q=>{Q.addEventListener("click",()=>{if(!q&&!D)return;const ve=Q.dataset.cardId,qe=Q.dataset.role,Te=(k[qe]||[]).find(Ge=>Ge.cardId===ve);if(!Te||Te.used)return;const Nt=Ue.includes(ve);if(q&&!["MIL","ATT"].includes(qe)&&!Nt)return;Array.isArray(f["selected_"+m])||(f["selected_"+m]=[]);const dt=f["selected_"+m],Pe=dt.findIndex(Ge=>Ge.cardId===ve);Pe>-1?dt.splice(Pe,1):dt.length<3&&dt.push({...Te,_role:qe}),O()})}),t.querySelectorAll(".match-used-hit").forEach(Q=>{Q.addEventListener("click",()=>re(Q.dataset.cardId))}),t.querySelectorAll(".pvp-sub-btn").forEach(Q=>{Q.addEventListener("click",()=>re())}),(bo=t.querySelector("#pvp-sub-open"))==null||bo.addEventListener("click",()=>re()),t.querySelectorAll(".pvp-gc-mini").forEach(Q=>{q?Q.addEventListener("click",()=>j(Q.dataset.gcId,Q.dataset.gcType)):(Q.style.opacity="0.35",Q.style.cursor="default",Q.addEventListener("click",()=>It("⚡ Les Game Changers ne sont utilisables qu'en attaque","rgba(180,100,0,0.9)")))}),(yo=t.querySelector("#pvp-boost-card"))==null||yo.addEventListener("click",()=>Y()),(ho=t.querySelector("#pvp-action"))==null||ho.addEventListener("click",Q=>{q?Q.currentTarget.dataset.pass==="1"||!Le(m)?J():be():D&&ke()}),(vo=t.querySelector("#pvp-quit"))==null||vo.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&_()}),(wo=t.querySelector("#pvp-view-opp"))==null||wo.addEventListener("click",()=>X()),(_o=t.querySelector("#pvp-toggle-history"))==null||_o.addEventListener("click",()=>P()),E&&(clearInterval(E),E=null),Ve(),(q||D)&&!T){let Q=30,ve=!1;const qe=()=>document.getElementById("pvp-timer"),Te=()=>{qe()&&(qe().textContent=Q+"s",qe().style.color=ve?"#ff4444":"#fff")};Te(),E=setInterval(()=>{Q--,Q<0?ve?(clearInterval(E),E=null,Ve(),q&&!Le(m)?J():_()):(ve=!0,Q=15,Te(),Xo("/sounds/timer-urgent.mp3",.6)):Te()},1e3)}}function ue(){ct(t),t.innerHTML=`
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(ie);let ee=!1;const se=()=>{ee||(ee=!0,ie.remove(),setTimeout(()=>A&&A(),50))};ie.addEventListener("click",se),setTimeout(se,3e3)}function j(k,S){var oe,ie,ee,se;const C=(f["gcCardsFull_"+m]||[]).find(fe=>fe.id===k),z=C==null?void 0:C._gcDef,R=(z==null?void 0:z.name)||S,q=(z==null?void 0:z.effect)||((oe=Oe[S])==null?void 0:oe.desc)||"Carte spéciale.",D=z!=null&&z.image_url?`/icons/${z.image_url}`:null,U=((ie=Oe[S])==null?void 0:ie.icon)||"⚡",Z=document.createElement("div");Z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",Z.innerHTML=`
      ${nt(R,D,U,q,{width:190})}
      <div style="display:flex;gap:12px;width:190px">
        <button id="pvp-gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
        <button id="pvp-gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
      </div>`,document.body.appendChild(Z),(ee=Z.querySelector("#pvp-gc-back"))==null||ee.addEventListener("click",()=>Z.remove()),(se=Z.querySelector("#pvp-gc-use"))==null||se.addEventListener("click",()=>{Z.remove(),le(k,S)})}function Y(){var C;const k=f[m+"Team"],S=Object.entries(k).flatMap(([z,R])=>(R||[]).filter(q=>!q.used).map(q=>({...q,_line:z})));if(!S.length)return;const A=document.createElement("div");A.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",A.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
      ...
      </div>`,document.body.appendChild(A),(C=A.querySelector("#bp-close"))==null||C.addEventListener("click",()=>A.remove()),A.querySelectorAll(".bp-item").forEach(z=>{z.addEventListener("click",async()=>{const R=z.dataset.cid,q=S.find(U=>U.cardId===R);if(!q)return;const D=(k[q._line]||[]).find(U=>U.cardId===R);D&&(D.boost=(D.boost||0)+f.boostValue),A.remove(),await L({[m+"Team"]:k,boostUsed:!0})})})}function re(k=null){var se,fe;if(!(f.phase===m+"-attack")){b("Remplacement uniquement avant votre attaque","warning");return}const A=f[m+"Team"],C=f["usedSubIds_"+m]||[],z=f.maxSubs||3;if(C.length>=z){b(`Maximum ${z} remplacements atteint`,"warning");return}const R=Object.entries(A).flatMap(([we,me])=>(me||[]).filter(_e=>_e.used).map(_e=>({..._e,_line:we}))),q=(f[m+"Subs"]||[]).filter(we=>!C.includes(we.cardId));if(!R.length){b("Aucun joueur utilisé à remplacer","warning");return}if(!q.length){b("Aucun remplaçant disponible","warning");return}let D=Math.max(0,R.findIndex(we=>we.cardId===k));const U=((se=R[D])==null?void 0:se._line)||((fe=R[D])==null?void 0:fe.job);let Z=Math.max(0,q.findIndex(we=>we.job===U)),oe=!1;const ie=document.createElement("div");ie.id="pvp-sub-overlay",ie.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function ee(){var Ne,je,Xe,Ue,rt,Xt;const we=R[D],me=q[Z],_e=Math.min(130,Math.round((window.innerWidth-90)/2)),$e=Math.round(_e*1.35),ne=Fe=>`background:rgba(255,255,255,0.12);border:none;color:${Fe?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Fe?"default":"pointer"};flex-shrink:0`;ie.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="width:min(90vw,420px)">${Mi(f[y+"Team"],f[y+"Formation"],null,[],jt(),Ht())}</div>
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(k),(S=k.querySelector("#pvp-opp-close"))==null||S.addEventListener("click",()=>k.remove())}function P(){var A;const k=f.log||[],S=document.createElement("div");S.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column",S.innerHTML=`
      ...
      </div>`,document.body.appendChild(S),(A=S.querySelector("#pvp-hist-close"))==null||A.addEventListener("click",()=>S.remove())}async function J(){if(f.phase!==m+"-attack")return;const k=m==="p1"?"p2":"p1",S=(f.round||0)+1,A=[...f.log||[]];A.push({type:"info",text:`⏭️ ${f[m+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const C=De(f),z=Le(k),R=C||!z?"finished":k+"-attack";await L({["selected_"+m]:[],modifiers:{...f.modifiers,[m]:{}},pendingAttack:null,phase:R,attacker:k,round:S,log:A}),R==="finished"&&await Ce(f)}function F(k){const S=(k.GK||[]).some(C=>!C.used),A=["DEF","MIL","ATT"].some(C=>(k[C]||[]).some(z=>!z.used));return S&&!A}function H(k){return(k.p1Score||0)!==(k.p2Score||0)?null:F(k.p1Team)&&!Se(k.p2Team)?{side:"p1",gk:(k.p1Team.GK||[]).find(S=>!S.used)}:F(k.p2Team)&&!Se(k.p1Team)?{side:"p2",gk:(k.p2Team.GK||[]).find(S=>!S.used)}:null}async function ae(){const k=document.createElement("div");k.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:950;display:flex;align-items:center;justify-content:center;overflow:hidden;text-align:center;padding:24px",document.body.appendChild(k);const S=(A,C)=>new Promise(z=>{k.innerHTML=`<div style="font-size:32px;font-weight:900;color:${C};letter-spacing:2px;animation:lcFade 1.4s ease both">${A}</div>
      <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(z,1400)});await S("⚽ DERNIER CORNER","#FFD700"),await S("🧤 LE GARDIEN MONTE !","#4fc3f7"),k.remove()}async function ce(k,S){const{side:A,gk:C}=k;if(!C)return;const z=A+"Score",R=(S[z]||0)+1;C.used=!0;const q=[...S.log||[]];q.push({type:"duel",isGoal:!0,homeScored:A===m,homePlayers:A==="p1"?[Gt(C)]:[],aiPlayers:A==="p2"?[Gt(C)]:[],text:`⚽ DERNIER CORNER — Le gardien de ${S[A+"Name"]} marque !`});const D=(S.round||0)+1,U=A==="p1"?"p2":"p1",Z={...S,[z]:R},oe=De(Z);B.add(D),await ae();const ie=A===m?R:S[m+"Score"]||0,ee=A===m?S[y+"Score"]||0:R;tt([Gt(C)],ie,ee,A===m,async()=>{await L({[z]:R,log:q,round:D,pendingAttack:null,phase:oe?"finished":U+"-attack",attacker:U,["selected_"+m]:[],modifiers:{...f.modifiers,[m]:{}}}),oe&&await Ce({...f,[z]:R})})}async function be(){const k=f[m+"Team"],S=!["GK","DEF","MIL","ATT"].some(R=>(f[y+"Team"][R]||[]).some(q=>!q.used)),A=(f["selected_"+m]||[]).map(R=>{const q=(k[R._role]||[]).find(fe=>fe.cardId===R.cardId)||R,D=["GK","DEF"].includes(R._role),U=k[R._role]||[],Z=U.findIndex(fe=>fe.cardId===R.cardId),oe=lt(U.length),ie=Z>=0?oe[Z]:q._col??1,ee=f.stadiumDef||f.homeStadiumDef||null,se=q.stadiumBonus||ee&&(ee.club_id&&String(q.club_id)===String(ee.club_id)||ee.country_code&&q.country_code===ee.country_code)||!1;return{...q,_line:R._role,_col:ie,stadiumBonus:se,...D?{note_a:Math.max(1,Number(q.note_a)||0)}:{}}});if(!A.length)return;const C=ci(A,f.modifiers[m]||{},f[m+"Formation"]);pe(m,A.map(R=>R.cardId)),A.forEach(R=>{const q=(k[R._role]||[]).find(D=>D.cardId===R.cardId);q&&(q.used=!0)}),f["selected_"+m]=[],O();const z=[...f.log||[]];if(S){if(A.length===1&&(A[0]._line==="GK"||A[0].job==="GK")&&(f.p1Score||0)===(f.p2Score||0)){await ce({side:m,gk:A[0]},{...f,[m+"Team"]:k,log:z});return}const q=(f[m+"Score"]||0)+1,D=A.map(ee=>Gt(ee));z.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:D,homeTotal:C.total,aiTotal:0});const U=(f.round||0)+1,Z=m==="p1"?"p2":"p1",oe={...f,[m+"Team"]:k,[m+"Score"]:q},ie=De(oe);B.add(U),tt(D,q,f[y+"Score"]||0,!0,async()=>{await L({[m+"Team"]:k,[m+"Score"]:q,["selected_"+m]:[],modifiers:{...f.modifiers,[m]:{}},pendingAttack:null,phase:ie?"finished":Z+"-attack",attacker:Z,round:U,log:z}),ie&&await Ce({...f,[m+"Score"]:q})});return}z.push({type:"pending",text:`⚔️ ${f[m+"Name"]} attaque (${C.total})`}),await L({[m+"Team"]:k,[y+"Team"]:f[y+"Team"],pendingAttack:{...C,players:A,side:m},["selected_"+m]:[],modifiers:{...f.modifiers,[m]:{}},phase:y+"-defense",log:z})}async function ke(){const k=f[m+"Team"],S=(f["selected_"+m]||[]).map(me=>{const _e=(k[me._role]||[]).find(je=>je.cardId===me.cardId)||me,$e=k[me._role]||[],ne=$e.findIndex(je=>je.cardId===me.cardId),ze=lt($e.length),Ne=ne>=0?ze[ne]:_e._col??1;return{..._e,_line:me._role,_col:Ne}}),A=pi(S,f.modifiers[m]||{},f[m+"Formation"]);pe(m,S.map(me=>me.cardId)),S.forEach(me=>{const _e=(k[me._role]||[]).find($e=>$e.cardId===me.cardId);_e&&(_e.used=!0)}),f["selected_"+m]=[],O();const C=po(f.pendingAttack.total,A.total,f.modifiers[m]||{}),z=f.pendingAttack.side,R=C==="attack"||(C==null?void 0:C.goal),q=z==="p1"?"p2":"p1",D=(f.round||0)+1,U=(f.pendingAttack.players||[]).map(me=>Gt(me)),Z=[...f.log||[]];Z.push({type:"duel",isGoal:R,homeScored:R&&z===m,text:R?`⚽ BUT de ${f[z+"Name"]} ! (${f.pendingAttack.total} vs ${A.total})`:`✋ Attaque stoppée (${f.pendingAttack.total} vs ${A.total})`,homePlayers:U,aiPlayers:S.map(me=>Gt(me)),homeTotal:f.pendingAttack.total,aiTotal:A.total});const oe=R?(f[z+"Score"]||0)+1:f[z+"Score"]||0,ie={...f,[m+"Team"]:k,[z+"Score"]:oe},ee=De(ie),se=ee?"finished":q+"-attack",fe=!ee&&!R?H(ie):null,we=async()=>{if(fe){await ce(fe,ie);return}await L({[m+"Team"]:k,[y+"Team"]:f[y+"Team"],[z+"Score"]:oe,["selected_"+m]:[],modifiers:{...f.modifiers,[m]:{}},pendingAttack:null,phase:se,attacker:q,round:D,log:Z}),(se==="finished"||ee)&&await Ce({...f,[z+"Score"]:oe})};if(R){const me=z===m,_e=me?oe:f[m+"Score"]||0,$e=me?f[y+"Score"]||0:oe;B.add(D),tt(U,_e,$e,me,we)}else await we()}function Ee(k){return["MIL","ATT"].some(S=>(k[S]||[]).some(A=>!A.used))}function Se(k){return["GK","DEF","MIL","ATT"].some(S=>(k[S]||[]).some(A=>!A.used))}function ye(k){return Se(k)&&!Ee(k)}function Le(k){const S=f[k+"Team"],A=f[(k==="p1"?"p2":"p1")+"Team"];return!!(Ee(S)||ye(S)&&!Se(A)||ye(S)&&!Ee(A)&&Se(A))}function De(k){return!Se(k.p1Team)&&!Se(k.p2Team)}function Me(k){const S=k.p1Score||0,A=k.p2Score||0;return S===A?null:S>A?h.home_id:h.away_id}async function Ce(k){Ot(),Ve();try{const S=Me(k),A=S?h.home_id===S?h.away_id:h.home_id:null,C=k.p1Score||0,z=k.p2Score||0,{data:R,error:q}=await v.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:S,p_home_score:C,p_away_score:z,p_forfeit:!1});if(q&&console.error("[PvP] finish_pvp_match:",q),l)try{await v.rpc("check_and_finish_mini_league",{p_league_id:l})}catch(D){console.error("[PvP] check_and_finish_mini_league:",D)}S&&A&&aa(S,A).catch(()=>{});try{const{data:D}=await v.rpc("apply_match_rewards",{p_match_id:i});D!=null&&D.success&&!(D!=null&&D.skipped)&&typeof e.refreshProfile=="function"&&await e.refreshProfile()}catch(D){console.error("[PvP] apply_match_rewards:",D)}if(typeof s=="function")try{await s({homeScore:C,awayScore:z,winnerId:S,homeId:h.home_id,awayId:h.away_id,matchId:i})}catch(D){console.error("[PvP] onMatchEnd:",D)}}catch(S){console.error("[PvP] finishMatch:",S)}}function He(){var z;const k=f[m+"Score"],S=f[y+"Score"],A=k>S,C=k===S;ct(t),t.innerHTML=`
    ...
    </div>`,(z=document.getElementById("pvp-home"))==null||z.addEventListener("click",()=>{try{v.removeChannel(K)}catch{}Ye(t),l?g("mini-league",{openLeagueId:l}):g("home")})}O()}async function Xi(t,e,i=!1){var d,c;const{state:o,navigate:r,toast:n}=e,a=i&&((c=(d=e==null?void 0:e.state)==null?void 0:d.params)==null?void 0:c.rankedData)||null;await Ci(t,e,i?"ranked":"random",async({deckId:s,formation:l,starters:p,subsRaw:x,gcCardsEnriched:g,gcDefs:b,stadiumDef:m})=>{ct(t);const y=async u=>{await la(t,e,s,l,p,x,u||[],b,m,i,a)};if(!(g!=null&&g.length)){await y([]);return}Ai(t,g,y)})}async function da(t,e,i){return sa(t,e,i)}async function la(t,e,i,o,r,n,a=[],d=[],c=null,s=!1,l=null){var W,K;const{state:p,navigate:x,toast:g}=e,b=(l==null?void 0:l.mmr)??((W=p.profile)==null?void 0:W.mmr)??1e3,m=s?At(b):null,y=s?"linear-gradient(135deg, #1a0a2e 0%, #2d1060 50%, #1a0a2e 100%)":"linear-gradient(135deg, #0a1a2e 0%, #0d3d1e 100%)",u=s?(m==null?void 0:m.color)||"#D4A017":"#FFD700";let w=!1,h=null,$=null;t.innerHTML=`
    <div style="min-height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;background:${y};color:#fff;padding:32px;text-align:center;gap:20px">
      ...
      ${s?`<div style="font-size:13px;color:${(m==null?void 0:m.color)||"#D4A017"}">${(m==null?void 0:m.name)||""} · ${Math.round(b)} MMR</div>`:""}
    ...
    </div>`;const f=async()=>{w=!0,h&&(h.unsubscribe(),h=null),$&&(clearInterval($),$=null);try{await v.rpc("cancel_matchmaking",{p_user_id:p.profile.id})}catch(L){console.error("[Matchmaking] cancel error:",L)}};(K=document.getElementById("cancel-mm"))==null||K.addEventListener("click",async()=>{await f(),Ye(t),x("home")});const T=async(L,_)=>{w||(w=!0,h&&(h.unsubscribe(),h=null),$&&(clearInterval($),$=null),await Dt(t,e,L,_,{myGC:a,gcDefs:d,stadiumDef:c,isRanked:s,rankedData:l,onMatchEnd:s?ca:null}))},E=p.user.id,{data:I,error:B}=s?await v.rpc("try_matchmake_ranked",{p_user_id:E,p_deck_id:i,p_mmr:b,p_range:200}):await v.rpc("try_matchmake",{p_user_id:E,p_deck_id:i});if(B||!(I!=null&&I.success)){console.error("[Matchmaking] try_matchmake error:",B||I),g("Erreur de matchmaking","error"),Ye(t),x("home");return}if(I.matched){await T(I.match_id,!1);return}const V=async()=>{if(w)return;const{data:L}=await v.from("matchmaking_queue").select("status, match_id").eq("user_id",E).maybeSingle();(L==null?void 0:L.status)==="matched"&&L.match_id&&await T(L.match_id,!0)};h=v.channel(`mm_${E}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${E}`},L=>{const _=L.new;_.status==="matched"&&_.match_id&&T(_.match_id,!0)}).subscribe(),$=setInterval(V,3e3),s?setTimeout(async()=>{if(!w){w=!0,h&&(h.unsubscribe(),h=null),$&&(clearInterval($),$=null);try{await v.rpc("cancel_matchmaking",{p_user_id:p.profile.id})}catch(L){console.error("[Matchmaking] cancel error:",L)}g("Aucun adversaire trouvé — match contre une IA calibrée sur votre niveau","info",4e3),x("match",{matchMode:"ranked_ai",rankedData:l,presetSetup:{deckId:i,formation:o,starters:r,subsRaw:n,gcCardsEnriched:a,gcDefs:d,stadiumDef:c}})}},2e4):setTimeout(()=>{w||f()},12e4)}async function ca(t,e){const{state:i,toast:o}=e,{winnerId:r,homeId:n,awayId:a,homeScore:d,awayScore:c,matchId:s}=t,l=i.user.id,p=n===l,x=r===l,{data:g}=await v.from("users").select("mmr, mmr_rd, mmr_v").eq("id",l).single(),{data:b}=await v.from("users").select("mmr, mmr_rd, mmr_v").eq("id",p?a:n).single();if(g&&b){const m=Ko(g.mmr,g.mmr_rd,g.mmr_v,b.mmr,b.mmr_rd,x?1:0);await v.from("users").update({mmr:m.mmr,mmr_rd:m.rd,mmr_v:m.v,mmr_wins:x?v.sql`mmr_wins + 1`:void 0,mmr_losses:x?void 0:v.sql`mmr_losses + 1`}).eq("id",l),o(x?`+MMR ↑ ${Math.round(m.mmr-g.mmr)}`:`-MMR ↓ ${Math.round(g.mmr-m.mmr)}`,x?"success":"error",4e3)}}const pa=Object.freeze(Object.defineProperty({__proto__:null,renderMatchRandom:Xi,resumePvpMatch:da},Symbol.toStringTag,{value:"Module"}));async function ua(t,e){const{state:i,navigate:o,toast:r}=e,n=i.params||{},a=n.friendId||null,d=n.friendName||"Ami",c=!!n.isAccepting;await Ci(t,e,"friend",async({deckId:s,formation:l,starters:p,subsRaw:x,gcCardsEnriched:g,gcDefs:b,stadiumDef:m})=>{ct(t);const y=async u=>{const w=u||[];c?await ma(t,e,s,w,b,m):await fa(t,e,s,w,b,m,a,d)};if(!(g!=null&&g.length)){await y([]);return}Ai(t,g,y)})}async function fa(t,e,i,o,r,n,a,d){var w;const{state:c,navigate:s,toast:l}=e,p=c.user.id,{data:x,error:g}=await v.from("friend_match_invites").insert({inviter_id:p,invitee_id:a,friend_id:a,status:"pending",inviter_deck_id:i,expires_at:new Date(Date.now()+2*60*1e3).toISOString()}).select("id").single();if(g||!x){console.error("[Friend] Erreur création invitation:",g),l("Impossible de créer l'invitation","error"),s("home");return}t.innerHTML=`
    ...
    </div>`;let b=!1,m=null;const y=async()=>{m&&(m.unsubscribe(),m=null)};(w=document.getElementById("cancel-friend"))==null||w.addEventListener("click",async()=>{await y(),await v.from("friend_match_invites").update({status:"declined"}).eq("id",x.id),Ye(t),s("home")}),m=v.channel(`friend_invite_${x.id}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"friend_match_invites",filter:`id=eq.${x.id}`},async({new:h})=>{b||h.status!=="accepted"||!h.match_id||(b=!0,await y(),await Dt(t,e,h.match_id,!0,{myGC:o,gcDefs:r,stadiumDef:n}))}).subscribe();const u=setInterval(async()=>{if(b)return;const{data:h}=await v.from("friend_match_invites").select("status, match_id").eq("id",x.id).single();(h==null?void 0:h.status)==="accepted"&&h.match_id&&(b=!0,clearInterval(u),await y(),await Dt(t,e,h.match_id,!0,{myGC:o,gcDefs:r,stadiumDef:n}))},3e3);setTimeout(async()=>{b||(clearInterval(u),await y(),l("Invitation expirée","info"),s("home"))},12e4)}async function ma(t,e,i,o,r,n){const{state:a,navigate:d,toast:c}=e,s=a.user.id,{data:l}=await v.from("friend_match_invites").select("id, inviter_id").eq("invitee_id",s).eq("status","pending").order("created_at",{ascending:!1}).limit(1).maybeSingle();if(!l){c("Aucune invitation en attente","error"),d("home");return}const{data:p,error:x}=await v.rpc("accept_friend_invite",{p_invite_id:l.id,p_invitee_deck_id:i});if(x||!(p!=null&&p.success)){console.error("[Friend] Erreur accept_friend_invite:",x||p),c((p==null?void 0:p.error)||"Impossible de rejoindre le match","error"),d("home");return}await Dt(t,e,p.match_id,!1,{myGC:o,gcDefs:r,stadiumDef:n})}const wn="#1A6B3C",pt="#D4A017",ii="var(--tile-bg)",oi="var(--tile-border)",Mt="var(--divider)",Ze="var(--tile-fg-on-page)",zt="var(--tile-fg-dim)",it="var(--tile-fg-faint)",ri="var(--nav-fg,#fff)",_n="rgba(255,255,255,0.62)",ga="rgba(255,255,255,0.4)";async function xa(t,e){var o;t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const i=(o=e.state.params)==null?void 0:o.openLeagueId;if(i){e.state.params.openLeagueId=null,await ft(t,e,i);return}await ot(t,e)}async function ot(t,e,i="waiting"){var h,$;const{state:o}=e,r=o.profile.id,{data:n}=await v.from("mini_league_members").select("league_id, prize_amount, prize_claimed").eq("user_id",r),a=(n||[]).map(f=>f.league_id),d={};(n||[]).forEach(f=>{d[f.league_id]={amount:f.prize_amount,claimed:f.prize_claimed}});const{data:c,error:s}=await v.from("mini_leagues").select("*, mini_league_members(count)").eq("status","waiting").eq("is_archived",!1).order("created_at",{ascending:!1}).limit(30),l=s?(await v.from("mini_leagues").select("*, mini_league_members(count)").eq("status","waiting").order("created_at",{ascending:!1}).limit(30)).data||[]:c||[],x=(a.length?await v.from("mini_leagues").select("*, mini_league_members(count)").in("id",a).order("created_at",{ascending:!1}):{data:[]}).data||[],g=x.filter(f=>f.status==="waiting"&&!f.is_archived),b=x.filter(f=>f.status==="active"&&!f.is_archived),m=x.filter(f=>f.is_archived||f.status==="finished");let y=m;if(m.length){const{data:f}=await v.from("mini_league_matches").select("league_id").in("league_id",m.map(E=>E.id)).or(`home_id.eq.${r},away_id.eq.${r}`).not("status","eq","bye"),T=new Set((f||[]).map(E=>E.league_id));y=m.filter(E=>T.has(E.id))}const u=l.filter(f=>!a.includes(f.id)),w=[{key:"waiting",label:"🟡 En attente",count:g.length+u.length},{key:"active",label:"🟢 En cours",count:b.length},{key:"archived",label:"📁 Archivées",count:y.length}];t.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="display:flex;background:var(--nav-bg,#0d1a0f);border-bottom:1px solid ${Mt}">
      ${w.map(f=>`<button class="ml-tab" data-tab="${f.key}" style="flex:1;padding:11px 4px;border:none;border-bottom:2px solid ${i===f.key?wn:"transparent"};background:none;font-size:12px;font-weight:${i===f.key?"900":"600"};color:${i===f.key?"#4ade80":ga};cursor:pointer">${f.label}${f.count?` (${f.count})`:""}</button>`).join("")}
    </div>
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:10px">
      ${i==="waiting"?ba(g,u,r):i==="active"?ya(b,r):ha(y,r,d)}
    </div>
  </div>`,(h=document.getElementById("ml-create-btn"))==null||h.addEventListener("click",()=>va(t,e)),($=document.getElementById("ml-refresh-list"))==null||$.addEventListener("click",()=>ot(t,e,i)),t.querySelectorAll(".ml-tab").forEach(f=>f.addEventListener("click",()=>ot(t,e,f.dataset.tab))),t.querySelectorAll("[data-league-id]").forEach(f=>f.addEventListener("click",()=>ft(t,e,f.dataset.leagueId))),t.querySelectorAll("[data-join]").forEach(f=>f.addEventListener("click",T=>{T.stopPropagation(),kn(t,e,f.dataset.join,f.dataset.type)})),t.querySelectorAll("[data-delete]").forEach(f=>f.addEventListener("click",T=>{T.stopPropagation(),Ji(t,e,f.dataset.delete,f.dataset.name,i)})),t.querySelectorAll("[data-claim-league]").forEach(f=>f.addEventListener("click",async T=>{T.stopPropagation(),f.disabled=!0,f.textContent="...";const{data:E,error:I}=await v.rpc("claim_mini_league_prize",{p_league_id:f.dataset.claimLeague,p_user_id:o.profile.id});if(I||!(E!=null&&E.success)){e.toast((E==null?void 0:E.error)||"Erreur lors de la récupération","error"),ot(t,e,i);return}if(!E.already_claimed){o.profile.credits=(o.profile.credits||0)+E.prize;const B=document.getElementById("nav-credits");B&&(B.textContent=`💰 ${o.profile.credits.toLocaleString("fr")}`),e.toast(`💰 +${E.prize.toLocaleString("fr")} cr. ajoutés !`,"success")}ot(t,e,i)}))}function Ti(t,e,i=!1,o=null){var c,s;const r=t.creator_id===e,n=t.pot||0,a=((s=(c=t.mini_league_members)==null?void 0:c[0])==null?void 0:s.count)||0,d=o&&o.amount>0&&!o.claimed;return`<div data-league-id="${t.id}" style="background:${ii};border:1px solid ${d?"rgba(212,160,23,0.5)":oi};border-radius:12px;padding:14px 16px;cursor:pointer;margin-bottom:8px">
    ...
    </div></div>`,document.body.appendChild(i),i.querySelector("#ml-form-close").addEventListener("click",()=>i.remove()),i.querySelectorAll('input[name="ml-type"]').forEach(o=>o.addEventListener("change",()=>{document.getElementById("ml-pwd-block").style.display=o.value==="private"?"block":"none"})),i.querySelector("#ml-create-confirm").addEventListener("click",async()=>{var b,m,y,u;const{toast:o}=e,r=document.getElementById("ml-name").value.trim(),n=parseInt(document.getElementById("ml-max").value)||6,a=parseInt(document.getElementById("ml-fee").value)||500,d=((b=i.querySelector('input[name="ml-type"]:checked'))==null?void 0:b.value)||"public",c=((m=i.querySelector('input[name="ml-mode"]:checked'))==null?void 0:m.value)||"aller",s=((u=(y=document.getElementById("ml-pwd"))==null?void 0:y.value)==null?void 0:u.trim())||null;if(!r){o("Le nom est obligatoire","error");return}if(a<100){o("Mise minimum : 100 crédits","error");return}if(d==="private"&&!s){o("Mot de passe requis","error");return}const{data:l,error:p}=await v.from("mini_leagues").insert({name:r,creator_id:e.state.profile.id,type:d,password:s,mode:c,max_players:n,entry_fee:a}).select().single();if(p){o("Erreur : "+p.message,"error");return}const{data:x}=await v.from("users").select("credits").eq("id",e.state.profile.id).single();if(((x==null?void 0:x.credits)||0)<a){await v.from("mini_leagues").delete().eq("id",l.id),o(`Crédits insuffisants pour la mise (${a.toLocaleString("fr")} cr.)`,"error");return}await v.from("users").update({credits:x.credits-a}).eq("id",e.state.profile.id),await v.from("mini_leagues").update({pot:a}).eq("id",l.id),await v.from("mini_league_members").insert({league_id:l.id,user_id:e.state.profile.id}),e.state.profile&&(e.state.profile.credits=x.credits-a);const g=document.getElementById("nav-credits");g&&(g.textContent=`💰 ${(e.state.profile.credits||0).toLocaleString("fr")}`),i.remove(),o(`Mini League créée ! ${a.toLocaleString("fr")} cr. déduits.`,"success"),ft(t,e,l.id)})}function wa(){return new Promise(t=>{const e=document.createElement("div");e.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9500;display:flex;align-items:center;justify-content:center;padding:20px",e.innerHTML=`
    ...
    </div>`,document.body.appendChild(g),g.querySelector("#lv-ok").onclick=()=>{g.remove(),x(!0)},g.querySelector("#lv-cancel").onclick=()=>{g.remove(),x(!1)}}))return;const{data:c,error:s}=await v.rpc("leave_mini_league",{p_league_id:i,p_user_id:a});if(s||!(c!=null&&c.success)){r((c==null?void 0:c.error)||"Erreur lors de la désinscription","error");return}const l=c.refund||o;n.profile&&(n.profile.credits=(n.profile.credits||0)+l);const p=document.getElementById("nav-credits");p&&(p.textContent=`💰 ${(n.profile.credits||0).toLocaleString("fr")}`),r(`↩️ Désinscrit · +${l.toLocaleString("fr")} cr. remboursés`,"success"),ot(t,e,"waiting")}async function Ji(t,e,i,o,r){const{toast:n}=e;if(!confirm(`Supprimer définitivement "${o}" et tous ses matchs/membres ? Action irréversible.`))return;await v.from("mini_league_matches").delete().eq("league_id",i),await v.from("mini_league_members").delete().eq("league_id",i);const{error:a}=await v.from("mini_leagues").delete().eq("id",i);if(a){n("Erreur suppression ("+a.message+")","error");return}n("Mini League supprimée avec succès","success"),ot(t,e,r)}async function ka(t,e,i){await v.from("mini_leagues").update({is_archived:!0}).eq("id",i),e.toast("Mini League archivée","success"),ot(t,e,"archived")}async function ft(t,e,i){var $,f,T,E,I,B,V,W,K;const{state:o,toast:r}=e,n=o.profile.id,[{data:a},{data:d},{data:c}]=await Promise.all([v.from("mini_leagues").select("*").eq("id",i).single(),v.from("mini_league_members").select("*, user:users(id,pseudo,club_name,club_color1,club_color2)").eq("league_id",i),v.from("mini_league_matches").select("*").eq("league_id",i).order("matchday").order("created_at")]);if(!a){r("Introuvable","error"),await ot(t,e);return}const s=(d||[]).some(L=>L.user_id===n),l=a.creator_id===n,p=(d||[]).map(L=>L.user).filter(Boolean),x=Ta(p,c||[]),g=(c||[]).filter(L=>L.matchday===a.current_day),b=a.pot||0,m=a.entry_fee||100,y=g.length>0&&g.every(L=>L.status==="finished"||L.status==="bye"),u=a.current_day>=a.total_days,w=(d||[]).find(L=>L.user_id===n);t.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    <div style="padding:14px 16px;background:var(--nav-bg,#0d1a0f);border-bottom:1px solid ${Mt};display:flex;align-items:center;gap:10px">
      ...
      <div style="text-align:right;flex-shrink:0">
        ...
        ${b>0?`<div style="font-size:12px;font-weight:900;color:${pt};margin-top:4px">🏆 ${b.toLocaleString("fr")} cr.</div>`:""}
      </div>
    </div>
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:${ii};border:1px solid ${oi};border-radius:12px;padding:16px">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          <thead><tr style="font-size:10px;color:${it};text-transform:uppercase;border-bottom:2px solid ${Mt}">
            ...
            ${b>0&&a.status==="finished"?`<th style="text-align:right;padding:5px 0;color:${pt}">💰</th>`:""}
          </tr></thead>
          <tbody>${x.map((L,_)=>{const N=b>0&&a.status==="finished"?_===0?Math.floor(b*.7):_===1?Math.floor(b*.2):_===2?Math.floor(b*.1):0:0;return`<tr style="border-bottom:1px solid ${Mt};${L.userId===n?"background:rgba(74,222,128,0.08);":""}">
              ...
              ${b>0&&a.status==="finished"?`<td style="text-align:right;font-weight:700;color:${pt}">${N?N.toLocaleString("fr")+" cr.":"—"}</td>`:""}
            </tr>`}).join("")}</tbody>
        </table>
      </div>`:""}
      ...
      <div style="background:${ii};border:1px solid ${oi};border-radius:12px;padding:16px">
        ...
        ${Array.from({length:Math.max(0,a.status==="active"?a.current_day-1:a.current_day)},(L,_)=>_+1).reverse().map(L=>{const _=(c||[]).filter(N=>N.matchday===L);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:${it};margin-bottom:6px">Journée ${L}</div>${_.map(N=>Ro(N,p,n,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
    ...
    </div>`,document.body.appendChild(p),(x=p.querySelector("#prize-close"))==null||x.addEventListener("click",()=>p.remove()),(g=p.querySelector("#claim-prize-btn"))==null||g.addEventListener("click",async b=>{const m=b.currentTarget;m.disabled=!0,m.textContent="...";const{data:y,error:u}=await v.rpc("claim_mini_league_prize",{p_league_id:i.id,p_user_id:n.profile.id});if(u||!(y!=null&&y.success)){console.error("[MiniLeague] claim_mini_league_prize:",u||y),a((y==null?void 0:y.error)||"Erreur lors de la récupération","error"),m.disabled=!1,m.textContent=`💰 Récupérer ${s.toLocaleString("fr")} cr.`;return}if(y.already_claimed)a("Déjà récupéré précédemment","info");else{const w=(n.profile.credits||0)+y.prize;n.profile&&(n.profile.credits=w);const h=document.getElementById("nav-credits");h&&(h.textContent=`💰 ${w.toLocaleString("fr")}`),a(`💰 +${y.prize.toLocaleString("fr")} cr. ajoutés à ton solde !`,"success")}p.remove(),ft(t,e,i.id)})}function Sa(t,e){const o=t.length%2===0?[...t]:[...t,null],r=o.length;let n=o.slice(1);const a=[];for(let d=0;d<r-1;d++){const c=[],s=[o[0],...n];for(let l=0;l<r/2;l++){const p=s[l],x=s[r-1-l];p===null?c.push({bye:x}):x===null?c.push({bye:p}):c.push({home:p,away:x})}a.push(c),n=[n[n.length-1],...n.slice(0,-1)]}return e==="aller-retour"?[...a,...a.map(d=>d.map(c=>c.bye?c:{home:c.away,away:c.home}))]:a}function Ta(t,e){const i={};return t.forEach(o=>{i[o.id]={userId:o.id,pseudo:o.pseudo,clubName:o.club_name||o.pseudo,played:0,won:0,drawn:0,lost:0,goalsFor:0,goalsAgainst:0,goalDiff:0,points:0}}),e.filter(o=>o.status==="finished"&&!o.is_bye&&o.home_score!=null).forEach(o=>{const r=i[o.home_id],n=i[o.away_id];!r||!n||(r.played++,n.played++,r.goalsFor+=o.home_score,r.goalsAgainst+=o.away_score,n.goalsFor+=o.away_score,n.goalsAgainst+=o.home_score,o.home_score>o.away_score?(r.won++,r.points+=3,n.lost++):o.home_score<o.away_score?(n.won++,n.points+=3,r.lost++):(r.drawn++,r.points++,n.drawn++,n.points++),r.goalDiff=r.goalsFor-r.goalsAgainst,n.goalDiff=n.goalsFor-n.goalsAgainst)}),Object.values(i).sort((o,r)=>r.points-o.points||r.goalDiff-o.goalDiff||r.goalsFor-o.goalsFor)}async function No(t,e){const{state:i,navigate:o,toast:r}=e,n=i.params||{},a=n.leagueId||null,d=n.mlMatchId||null,c=i.user.id;if(!d||!a){r("Match introuvable","error"),o("mini-league");return}const{data:s,error:l}=await v.from("mini_league_matches").select("id, league_id, home_id, away_id, status, match_id").eq("id",d).single();if(l||!s){r("Match introuvable","error"),o("mini-league",{openLeagueId:a});return}if(s.home_id!==c&&s.away_id!==c){r("Vous ne faites pas partie de ce match","error"),o("mini-league",{openLeagueId:a});return}const p=s.home_id===c;if(s.match_id){await Dt(t,e,s.match_id,p,{mlLeagueId:a,mlMatchId:d});return}await Ci(t,e,"mini_league",async({deckId:x,gcCardsEnriched:g,gcDefs:b,stadiumDef:m})=>{ct(t);const y=async u=>{const{data:w,error:h}=await v.rpc("start_mini_league_match",{p_ml_match_id:d,p_user_id:c,p_deck_id:x});if(h||!(w!=null&&w.success)){console.error("[MiniLeague] start_mini_league_match error:",h||w),r((w==null?void 0:w.error)||"Impossible de lancer le match","error"),o("mini-league",{openLeagueId:a});return}if(w.matched){await Dt(t,e,w.match_id,p,{mlLeagueId:a,mlMatchId:d,myGC:u||[],gcDefs:b,stadiumDef:m});return}await Ia(t,e,{mlMatchId:d,leagueId:a,amIHome:p,chosenGC:u,gcDefs:b,stadiumDef:m})};if(!(g!=null&&g.length)){await y([]);return}Ai(t,g,y)})}function Ia(t,e,{mlMatchId:i,leagueId:o,amIHome:r,chosenGC:n,gcDefs:a,stadiumDef:d}){return new Promise(c=>{var m;const{navigate:s}=e;t.innerHTML=`
    <div style="min-height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;background:linear-gradient(135deg,#0a1a2e,#0d3d1e);color:#fff;padding:32px;text-align:center;gap:20px">
      <div style="font-size:36px">🏆</div>
    ...
    </div>`;let l=!1;const p=async y=>{if(!l){if(l=!0,clearInterval(b),g)try{g.unsubscribe()}catch{}y&&await Dt(t,e,y,r,{mlLeagueId:o,mlMatchId:i,myGC:n||[],gcDefs:a,stadiumDef:d}),c()}};(m=document.getElementById("wait-back"))==null||m.addEventListener("click",()=>{if(l=!0,clearInterval(b),g)try{g.unsubscribe()}catch{}s("mini-league",{openLeagueId:o}),c()});const x=async()=>{if(l)return;const{data:y}=await v.from("mini_league_matches").select("match_id").eq("id",i).single();y!=null&&y.match_id&&p(y.match_id)},g=v.channel(`ml_match_${i}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"mini_league_matches",filter:`id=eq.${i}`},y=>{var u;(u=y.new)!=null&&u.match_id&&p(y.new.match_id)}).subscribe(),b=setInterval(x,3e3);x()})}const Go={GK:"#111111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function hi(t,e,i=0){return e?(Number(e==="GK"?t.note_g:e==="DEF"?t.note_d:e==="MIL"?t.note_m:t.note_a)||0)+(e===t.job||e===t.job2?i:0):0}async function za(t,e){t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',await go(t,e)}async function go(t,e){const{state:i,toast:o,navigate:r}=e;Yt(r,i.profile,"market","/icons/",o);const{data:n}=await v.from("market_listings").select(`id, price, status, listed_at, seller_id,
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div id="mkt-content" style="padding:10px 16px;display:flex;flex-direction:column;gap:8px"></div>
  </div>`;let s="buy";const l=()=>{var y,u,w,h,$,f,T;return{name:(((y=document.getElementById("flt-name"))==null?void 0:y.value)||"").toLowerCase().trim(),club:(((u=document.getElementById("flt-club"))==null?void 0:u.value)||"").toLowerCase().trim(),country:(((w=document.getElementById("flt-country"))==null?void 0:w.value)||"").toLowerCase().trim(),job:((h=document.getElementById("flt-job"))==null?void 0:h.value)||"",rarity:(($=document.getElementById("flt-rarity"))==null?void 0:$.value)||"",note1:parseInt((f=document.getElementById("flt-note1"))==null?void 0:f.value)||0,note2:parseInt((T=document.getElementById("flt-note2"))==null?void 0:T.value)||0}};function p(y){const u=l();return y.filter(w=>{var V,W,K;const h=(V=w.card)==null?void 0:V.player;if(!h)return!1;const $=`${h.firstname} ${h.surname_real}`.toLowerCase(),f=(((W=h.clubs)==null?void 0:W.encoded_name)||"").toLowerCase(),T=(h.country_code||"").toLowerCase(),E=((K=w.card)==null?void 0:K.evolution_bonus)||0,I=hi(h,h.job,E),B=h.job2?hi(h,h.job2,E):0;return!(u.name&&!$.includes(u.name)||u.club&&!f.includes(u.club)||u.country&&!T.includes(u.country)||u.job&&h.job!==u.job||u.rarity&&h.rarity!==u.rarity||u.note1&&I<u.note1||u.note2&&B<u.note2)})}function x(y){var f,T,E;const u=(f=y.card)==null?void 0:f.player;if(!u)return"";const w=((T=y.card)==null?void 0:T.evolution_bonus)||0,h=(i.profile.credits||0)>=y.price;return`<div class="mkt-buy-tile">
      ${Ae({...u,_evolution_bonus:w},{width:140})}
      <div class="mkt-price">${y.price.toLocaleString("fr")} cr.</div>
      <div class="mkt-seller">Vendeur : ${((E=y.seller)==null?void 0:E.pseudo)||"—"}</div>
    ...
    </div>`}function g(y){var W,K,L,_;const u=(W=y.card)==null?void 0:W.player;if(!u)return"";const w=((K=y.card)==null?void 0:K.evolution_bonus)||0,h=hi(u,u.job,w),$=u.job2?hi(u,u.job2,w):0,f=y.status==="sold",T=u.country_code?`https://flagsapi.com/${u.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,E=Go[u.job]||"#bbb",I=u.job2?Go[u.job2]||"#bbb":null,B=u.job==="GK"?"#fff":E,V=u.job2==="GK"?"#fff":I;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${f?"opacity:0.7":""}">
      ...
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${E};display:flex;align-items:center;justify-content:center">
      ...
      <div style="flex:1;min-width:0">
        ...
        <div style="font-size:10px;color:${f?"#22c55e":"#999"};margin-top:1px">
          ${f?`✅ Vendu à ${((_=y.buyer)==null?void 0:_.pseudo)||"—"} · ${y.sold_at?new Date(y.sold_at).toLocaleDateString("fr"):""}`:`🟢 En vente depuis le ${new Date(y.listed_at).toLocaleDateString("fr")}`}
        </div>
      </div>
      <div style="text-align:right;flex-shrink:0">
        <div style="font-size:14px;font-weight:900;color:#D4A017">${y.price.toLocaleString("fr")}</div>
        ${f?'<span style="font-size:10px;font-weight:700;color:#fff;background:#22c55e;padding:3px 8px;border-radius:10px;display:inline-block;margin-top:4px">VENDU</span>':`<button class="btn btn-danger btn-sm" data-cancel="${y.id}" style="margin-top:4px;font-size:11px;padding:4px 10px">Retirer</button>`}
      </div>
    </div>`}function b(){const y=document.getElementById("mkt-content"),u=document.getElementById("mkt-filters");if(y){if(u.style.display=s==="buy"?"flex":"none",s==="buy"){const w=p(d);y.innerHTML=w.length?`<div class="mkt-buy-grid">${w.map(x).join("")}</div>`:'<div style="text-align:center;color:#aaa;padding:40px">Aucune carte trouvée.</div>'}else{const w=c.filter($=>$.status==="active").sort(($,f)=>new Date(f.listed_at)-new Date($.listed_at)),h=c.filter($=>$.status==="sold").sort(($,f)=>new Date(f.sold_at||f.listed_at)-new Date($.sold_at||$.listed_at));y.innerHTML=(w.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:4px 0 6px;text-transform:uppercase;letter-spacing:1px">🟢 En vente (${w.length})</div>`+w.map(g).join(""):"")+(h.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:12px 0 6px;text-transform:uppercase;letter-spacing:1px">✅ Ventes réussies (${h.length})</div>`+h.map(g).join(""):"")+(!w.length&&!h.length?'<div style="text-align:center;color:#aaa;padding:40px">Aucune vente pour le moment.</div>':"")}y.querySelectorAll("[data-buy]").forEach(w=>w.addEventListener("click",()=>Aa(w.dataset.buy,d,t,e))),y.querySelectorAll("[data-cancel]").forEach(w=>w.addEventListener("click",()=>Ca(w.dataset.cancel,t,e)))}}t.querySelectorAll(".mkt-tab").forEach(y=>{y.addEventListener("click",()=>{s=y.dataset.tab,t.querySelectorAll(".mkt-tab").forEach(u=>{const w=u===y;u.style.background=w?"var(--green)":"var(--tile-bg)",u.style.color=w?"#fff":"var(--tile-fg-dim)",u.style.borderColor=w?"var(--green)":"var(--tile-border)"}),b()})});let m;["flt-name","flt-club","flt-country","flt-job","flt-rarity","flt-note1","flt-note2"].forEach(y=>{var u;(u=document.getElementById(y))==null||u.addEventListener("input",()=>{clearTimeout(m),m=setTimeout(b,250)})}),b()}async function Aa(t,e,i,o){const{state:r,toast:n,refreshProfile:a}=o,d=e.find(l=>l.id===t);if(!d)return;const c=d.price;if((r.profile.credits||0)<c){n("Crédits insuffisants","error");return}Ma(d,async()=>{const{error:l}=await v.rpc("buy_market_card",{p_listing_id:t,p_buyer_id:r.profile.id});if(l){n("Erreur achat : "+l.message,"error");return}await a();const p=document.getElementById("nav-credits");p&&(p.textContent=`💰 ${(r.profile.credits||0).toLocaleString("fr")}`),n("✅ Carte achetée !","success"),await go(i,o)})}function Ma(t,e){var a;const i=(a=t.card)==null?void 0:a.player,o=i?`${i.firstname} ${i.surname_real}`:"cette carte",r=document.createElement("div");r.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",r.innerHTML=`
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center">
      ...
      <div style="display:flex;gap:10px">
        ...
        <button id="buy-ok" style="flex:1;padding:12px;border-radius:10px;border:none;background:var(--green);color:#fff;font-size:14px;font-weight:900;cursor:pointer">Confirmer</button>
    ...
    </div>`,document.body.appendChild(r);const n=d=>{r.remove(),d&&e()};r.querySelector("#buy-cancel").addEventListener("click",()=>n(!1)),r.querySelector("#buy-ok").addEventListener("click",()=>n(!0)),r.addEventListener("click",d=>{d.target===r&&n(!1)})}async function Ca(t,e,i){const{toast:o}=i,{data:r}=await v.from("market_listings").select("card_id").eq("id",t).single();if(await v.from("market_listings").update({status:"cancelled"}).eq("id",t),r!=null&&r.card_id){const{count:n}=await v.from("market_listings").select("id",{count:"exact",head:!0}).eq("card_id",r.card_id).eq("status","active");n||await v.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",r.card_id)}o("Annonce retirée","success"),go(e,i)}async function ja(t,e){var g,b,m,y,u;const{state:i,navigate:o}=e,r=((b=(g=e==null?void 0:e.state)==null?void 0:g.params)==null?void 0:b.tab)||"ranked";t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:var(--tile-fg-dim)">⚽ Chargement...</div>';const[{data:n},{data:a},{data:d}]=await Promise.all([v.from("users").select("id,pseudo,club_name,mmr,rank_tier,ranked_wins,ranked_losses,ranked_draws,placement_matches").order("mmr",{ascending:!1}).limit(100),v.rpc("get_mini_league_leaderboard"),v.rpc("get_random_leaderboard")]);let c=r;const s=w=>w<3?["#D4A017","#a0a0a0","#cd7f32"][w]:"var(--green)",l=w=>w<3?"#000":"#fff";function p(){var $,f,T;const w=document.getElementById("rankings-legend");w&&(w.innerHTML=c==="random"?'<div style="font-size:11px;color:var(--tile-fg-dim);text-align:center;margin-bottom:10px">📊 = score de classement fiable (tient compte du nombre de matchs joués, pas seulement du %)</div>':"");const h=document.getElementById("rankings-list");if(h){if(c==="ranked"){const E=n||[];h.innerHTML=E.length>0?E.map((I,B)=>{const V=At(I.mmr??1e3),W=(I.ranked_wins||0)+(I.ranked_losses||0)+(I.ranked_draws||0),K=W>0?Math.round((I.ranked_wins||0)/W*100):0,L=I.id===i.profile.id,_=(I.placement_matches||0)<10;return`
          ...
          </div>`}).join(""):'<div style="text-align:center;color:var(--tile-fg-dim);padding:40px">Aucun joueur classé.</div>'}else if(c==="mini-league"){const E=a||[];h.innerHTML=E.length>0?E.map((I,B)=>`
      ...
      `).join(""):`<div style="text-align:center;color:var(--tile-fg-dim);padding:40px">Aucun résultat de Mini League pour l'instant.</div>`}else{const E=d||[];h.innerHTML=E.length>0?E.map((I,B)=>`
        <div style="display:flex;align-items:center;gap:12px;padding:12px;background:var(--tile-bg,rgba(255,255,255,0.05));border-radius:12px;border:1px solid var(--tile-border,rgba(255,255,255,0.1));${I.user_id===i.profile.id?"border:2px solid var(--yellow)":""}">
          <div style="width:32px;height:32px;border-radius:50%;background:${s(B)};color:${l(B)};display:flex;align-items:center;justify-content:center;font-weight:900;flex-shrink:0;font-size:${B<3?"16":"13"}px">${B<3?["🥇","🥈","🥉"][B]:B+1}</div>
          <div style="flex:1;min-width:0">
            <div style="font-weight:700;color:var(--tile-fg-on-page);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${I.pseudo}</div>
  ...
  <div class="page">
  ...
  </div>`,p(),(m=document.getElementById("tab-ranked"))==null||m.addEventListener("click",()=>{c="ranked",p()}),(y=document.getElementById("tab-mini-league"))==null||y.addEventListener("click",()=>{c="mini-league",p()}),(u=document.getElementById("tab-random"))==null||u.addEventListener("click",()=>{c="random",p()})}async function qa(t,e){var T,E,I,B;const{state:i,navigate:o,toast:r}=e,n=i.profile;t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const[{data:a},{data:d}]=await Promise.all([v.from("ranked_seasons").select("*").eq("is_active",!0).maybeSingle(),v.from("users").select("id,pseudo,club_name,mmr,mmr_deviation,mmr_volatility,rank_tier,placement_matches,ranked_wins,ranked_losses,ranked_draws").eq("id",n.id).single()]);if(!d){r("Erreur chargement profil","error"),o("home");return}if(!a){const V=d.mmr??1e3,W=At(V);Gi(V);const K=Math.round(1e3+(V-1e3)*.5),L=At(K),{data:_}=await v.from("users").select("id, pseudo, club_name, mmr, rank_tier").order("mmr",{ascending:!1}).limit(100),N=(_||[]).findIndex(G=>G.id===d.id)+1;t.innerHTML=`
    <div style="min-height:100%;background:linear-gradient(160deg,#1a1a1a,#2a2a2a);padding:16px;overflow-y:auto;display:flex;flex-direction:column;gap:16px">
      ...
      <div style="background:rgba(0,0,0,0.3);border-radius:16px;padding:18px;text-align:center;border:2px solid ${W.color}40">
        ...
        <div style="font-size:12px;color:rgba(255,255,255,0.4);margin-top:4px">MMR ${V} ${N?`· #${N} au classement général`:""}</div>
      </div>
      ...
      <div style="background:rgba(0,0,0,0.3);border-radius:16px;padding:18px;border:1.5px dashed rgba(255,255,255,0.2)">
        ...
        <div style="display:flex;align-items:center;justify-content:center;gap:24px">
          <div style="text-align:center;opacity:.6">
            ...
            <div style="font-size:12px;font-weight:700;color:${W.color}">${V}</div>
          </div>
    ...
    </div>`,(T=document.getElementById("ranked-back"))==null||T.addEventListener("click",()=>o("home"));return}const c=d.mmr??1e3,s=d.mmr_deviation??350,l=d.mmr_volatility??.06,p=d.placement_matches??0,x=p<10,g=Math.max(0,10-p),b=At(c),m=Gi(c),y=Di.findIndex(V=>V.id===b.id),u=Di[y+1]||null,w={bronze:"linear-gradient(160deg,#3d1c00,#7a3e00)",silver:"linear-gradient(160deg,#1a1a2e,#3a3a5e)",gold:"linear-gradient(160deg,#1a1200,#4a3500)",platinum:"linear-gradient(160deg,#001a20,#003040)",diamond:"linear-gradient(160deg,#001030,#1a2860)",master:"linear-gradient(160deg,#1a0030,#3d0070)"},h=(d.ranked_wins||0)+(d.ranked_losses||0)+(d.ranked_draws||0),$=h>0?Math.round((d.ranked_wins||0)/h*100):0,f=Di.map(V=>`
    <div style="display:flex;flex-direction:column;align-items:center;gap:2px;opacity:${b.id===V.id?1:.35};
      transform:${b.id===V.id?"scale(1.15)":"scale(1)"};transition:all 0.3s">
      <div style="font-size:${b.id===V.id?"28px":"20px"}">${V.emoji}</div>
      <div style="font-size:9px;color:${V.color};font-weight:${b.id===V.id?"900":"400"};letter-spacing:0.5px">${V.label.toUpperCase()}</div>
    </div>
  `).join("");t.innerHTML=`
  <div style="min-height:100%;background:${w[b.id]};padding:16px;overflow-y:auto;display:flex;flex-direction:column;gap:16px">
    ...
    <!-- Bandeau tier -->
    <div style="background:rgba(0,0,0,0.35);border-radius:20px;padding:20px 16px;text-align:center;border:2px solid ${b.color}40">
      <div style="font-size:52px;margin-bottom:4px">${b.emoji}</div>
      <div style="font-size:22px;font-weight:900;color:${b.color};letter-spacing:3px;text-transform:uppercase">${b.label}</div>
    </div>
    ...
    <div style="background:rgba(0,0,0,0.3);border-radius:12px;padding:12px 16px">
      ...
      <div style="background:rgba(255,255,255,0.1);border-radius:6px;height:10px;overflow:hidden">
        <div style="height:100%;width:${m}%;background:linear-gradient(90deg,${b.color},${b.color}aa);border-radius:6px;transition:width 0.8s ease"></div>
      </div>
    ...
    <div style="display:flex;flex-direction:column;gap:10px;margin-top:auto;padding-top:8px">
      <button id="ranked-play-btn" style="width:100%;padding:18px;border-radius:16px;border:none;
        ...
        box-shadow:0 4px 20px ${b.color}60;text-transform:uppercase">
        ⚽ Jouer en Ranked
      </button>
  ...
  </div>`,(E=document.getElementById("ranked-back"))==null||E.addEventListener("click",()=>o("home")),(I=document.getElementById("ranked-leaderboard-btn"))==null||I.addEventListener("click",()=>o("rankings",{tab:"ranked"})),(B=document.getElementById("ranked-play-btn"))==null||B.addEventListener("click",()=>{mt(),o("match",{matchMode:"ranked",rankedData:{mmr:c,rd:s,sigma:l,isPlacement:x}})})}async function Ba(t,{state:e,navigate:i,toast:o}){const r=e.profile;if(!r)return;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:n}=await v.from("matches").select(`id,home_id,away_id,home_score,away_score,status,mode,winner_id,created_at,played_at,
      home:users!home_id(pseudo,club_name),
      away:users!away_id(pseudo,club_name)`).or(`home_id.eq.${r.id},away_id.eq.${r.id}`).order("created_at",{ascending:!1}).limit(50),a={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},d=(n||[]).filter(l=>l.status==="finished"),c=(n||[]).filter(l=>l.status==="in_progress");function s(l){const p=l.home_id===r.id;p?l.home_score:l.away_score,p?l.away_score:l.home_score;const x=l.winner_id===r.id,g=l.home_score===l.away_score&&l.status==="finished",b=l.status!=="finished"?"…":g?"N":x?"V":"D",m=l.status!=="finished"||g?"#888":x?"#1A6B3C":"#c0392b";let y=a[l.mode]||l.mode;l.away_id===null&&!y.startsWith("IA")&&(y="IA");const w=l.home_id===r.id?l.away:l.home;let h;l.away_id===null?h=y:w?h=`${w.club_name||w.pseudo} (${w.pseudo})`:h="Adversaire";let $="";l.status==="in_progress"&&Date.now()-new Date(l.created_at).getTime()>3600*1e3&&($=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const f=new Date(l.created_at),T=f.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+f.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),E=l.status==="finished"?`${l.home_score} - ${l.away_score}`:`${l.home_score||0} - ${l.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
      ...
      <div style="display:flex;align-items:center;gap:8px">
        ...
        <span style="background:${m};color:#fff;width:22px;height:22px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:900">${b}</span>
      </div>
    </div>`}t.innerHTML=`
  <div class="page">
    <div class="page-header">
  ...
  `,document.querySelectorAll(".bottom-nav a").forEach(n=>{n.addEventListener("click",a=>{a.preventDefault(),$t(n.dataset.page)})}),document.getElementById("nav-logo").addEventListener("click",()=>$t("home")),document.getElementById("nav-credits").addEventListener("click",()=>$t("boosters")),(o=document.getElementById("journal-btn"))==null||o.addEventListener("click",()=>Pa()),(r=document.getElementById("settings-btn"))==null||r.addEventListener("click",()=>$t("settings"))}async function Pa(){const{data:t}=await v.from("patch_notes").select("*").eq("is_published",!0).order("published_at",{ascending:!1}).limit(20),e=document.createElement("div");e.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9000;display:flex;align-items:center;justify-content:center;padding:16px";const i=(t||[]).map(o=>{const r=new Date(o.published_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"long",year:"numeric"});return`<div style="padding:14px 0;border-bottom:1px solid #f0f0f0">
      ${o.image_url?`<img src="${o.image_url}" style="width:100%;max-height:160px;object-fit:cover;border-radius:8px;margin-bottom:10px">`:""}
      <div style="font-size:12px;color:#999;margin-bottom:4px">${r}</div>
      <div style="font-size:15px;font-weight:900;margin-bottom:6px">${o.title}</div>
      <div style="font-size:13px;color:#444;line-height:1.6;white-space:pre-line">${o.description}</div>
  ...
  </div>`,(o=t.querySelector("#pl-play-btn"))==null||o.addEventListener("click",e)}async function Na(){vi(ai()),document.getElementById("modal-overlay").addEventListener("click",o=>{o.target===o.currentTarget&&Qi()}),document.getElementById("modal-close").addEventListener("click",Qi);const{data:{session:t}}=await v.auth.getSession();if(!t){Ho(),Ra(document.getElementById("app"),{onPlay:()=>ko(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:yt})});return}he.user=t.user,await ni(),vi(ai()),Ho();try{const{data:o}=await v.from("formation_links_overrides").select("formation, links"),r={};(o||[]).forEach(n=>{r[n.formation]=n.links}),In(r)}catch(o){console.warn("Impossible de charger les overrides de formation:",o)}if(!he.profile){Mn(document.getElementById("app"),{state:he,navigate:async()=>{await ni(),vi(ai()),Ni()},toast:yt,refreshProfile:ni});return}const e=Array.isArray(he.profile.pending_boosters)?he.profile.pending_boosters:[];if(!he.profile.onboarding_done&&e.length>0){Jn(document.getElementById("app"),{state:he,toast:yt,refreshProfile:ni,navigate:()=>{Ni(),setTimeout(()=>To(he.profile,$t,yt),800)}});return}Ni(),new URLSearchParams(location.search).get("tutorial")==="1"?(history.replaceState({},"",location.pathname),setTimeout(()=>dn(he.profile,$t,yt),800)):setTimeout(()=>To(he.profile,$t,yt),800),v.auth.onAuthStateChange(async(o,r)=>{o==="SIGNED_OUT"&&(he.user=null,he.profile=null,document.getElementById("app").innerHTML="",ko(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:yt}))})}function Ga(){return Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight)}function ji(){const t=document.getElementById("app");t&&(t.style.height=Ga()+"px")}window.addEventListener("resize",ji);window.addEventListener("orientationchange",()=>setTimeout(ji,150));window.visualViewport&&window.visualViewport.addEventListener("resize",ji);function Ni(){const t=()=>{var i;(i=he.user)!=null&&i.id&&v.from("users").update({last_seen_at:new Date().toISOString()}).eq("id",he.user.id).then(()=>{})};t(),window._presencePingInterval&&clearInterval(window._presencePingInterval),window._presencePingInterval=setInterval(t,6e4);const e=document.getElementById("app");e.style.display="flex",e.style.flexDirection="column",ji(),Fa(),En()}function Ho(){const t=document.getElementById("app-loader"),e=document.getElementById("app");e&&(e.style.display=""),t&&(t.classList.add("zoom-out"),setTimeout(()=>t.style.display="none",500))}function Ln(t){var o;const e=document.getElementById("app-loader");if(e&&(e.style.display="none"),document.getElementById("boot-error"))return;const i=document.createElement("div");i.id="boot-error",i.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:99999;gap:16px;color:#fff;padding:24px;text-align:center",i.innerHTML=`
