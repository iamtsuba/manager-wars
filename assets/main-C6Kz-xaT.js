...
import{s as _,q as Ut,t as Ri,F as Oi,o as tt,n as ti,g as At,r as Ie,m as zt,u as zi,j as Hi,v as ao,w as so,T as Ei,p as lo,x as co,d as po}from"./special-cards-DZxbxWIu.js";const uo="/";function ln(e,{navigate:t,toast:i}){let n="login";const r=()=>{var a,s,d,c,l,m,b;const o=n==="login";if(e.innerHTML=`
    <div style="
  ...
  `;function s(){var g;const c=document.getElementById("logo-preview"),l=document.getElementById("logo-initials"),m=((g=document.getElementById("setup-club"))==null?void 0:g.value)||"MW",b=m.trim().split(" ").filter(Boolean),y=b.length>=2?(b[0][0]+b[1][0]).toUpperCase():m.slice(0,2).toUpperCase();c&&(c.style.background=a,c.style.borderColor=o),l&&(l.textContent=y,l.style.color=o)}document.getElementById("color1").addEventListener("input",c=>{o=c.target.value,document.getElementById("swatch1").style.background=o,s()}),document.getElementById("color2").addEventListener("input",c=>{a=c.target.value,document.getElementById("swatch2").style.background=a,s()});function d(c){document.querySelectorAll(".setup-step").forEach(l=>l.classList.remove("active")),document.getElementById(`step-${c}`).classList.add("active"),document.getElementById("step-num").textContent=c,document.getElementById("progress-fill").style.width=`${Math.round(c/3*100)}%`,c===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("step1-error");if(l.textContent="",c.length<3){l.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:m}=await _.from("users").select("id").eq("pseudo",c).maybeSingle();if(m){l.textContent="Ce pseudo est déjà pris.";return}d(2)}),document.getElementById("step2-back").addEventListener("click",()=>d(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const c=document.getElementById("setup-club").value.trim(),l=document.getElementById("step2-error");if(l.textContent="",c.length<2){l.textContent="Nom trop court (min. 2 caractères).";return}const{data:m}=await _.from("users").select("id").eq("club_name",c).maybeSingle();if(m){l.textContent="Ce nom de club est déjà pris.";return}d(3)}),document.getElementById("step3-back").addEventListener("click",()=>d(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("setup-club").value.trim(),m=document.getElementById("step3-error"),b=document.getElementById("step3-finish");m.textContent="",b.disabled=!0,b.textContent="Création en cours…";try{const{error:y}=await _.from("users").insert({id:t.user.id,pseudo:c,club_name:l,club_color1:o,club_color2:a,credits:1e4});if(y)throw y;await mo(t.user.id),await r(),n(`Bienvenue ${c} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(y){m.textContent=y.message,b.disabled=!1,b.textContent="🚀 Créer mon profil !"}})}async function mo(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await _.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const go="modulepreload",yo=function(e){return"/"+e},cn={},Ln=function(t,i,n){let r=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),s=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));r=Promise.allSettled(i.map(d=>{if(d=yo(d),d in cn)return;cn[d]=!0;const c=d.endsWith(".css"),l=c?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${l}`))return;const m=document.createElement("link");if(m.rel=c?"stylesheet":go,c||(m.as="script"),m.crossOrigin="",m.href=d,s&&m.setAttribute("nonce",s),document.head.appendChild(m),c)return new Promise((b,y)=>{m.addEventListener("load",b),m.addEventListener("error",()=>y(new Error(`Unable to preload CSS for ${d}`)))})}))}function o(a){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=a,window.dispatchEvent(s),!s.defaultPrevented)throw a}return r.then(a=>{for(const s of a||[])s.status==="rejected"&&o(s.reason);return t().catch(o)})},ii="#1A6B3C",ni="#cc2222",xo="#D4A017",pn="#888";async function bo(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    </style>`}const Sn="mw_sound_volume";function Vi(){const e=localStorage.getItem(Sn);if(e===null)return 100;const t=parseInt(e,10);return Number.isFinite(t)?Math.max(0,Math.min(100,t)):100}function wo(e){localStorage.setItem(Sn,String(Math.max(0,Math.min(100,Math.round(e)))))}function Yi(){return Vi()===0}function ui(e){return Math.max(0,Math.min(1,e*(Vi()/100)))}function Wi(e,t=1){if(Yi())return null;try{const i=new Audio(e);return i.volume=ui(t),i.play().catch(()=>{}),i}catch{return null}}let gt=null,Mi=null,un=.3;function An(e,t=.3){if(un=t,gt&&Mi===e&&!gt.paused){gt.volume=ui(un);return}if(Gt(),!Yi())try{const i=new Audio(e);i.loop=!0,i.volume=ui(t),i.play().catch(()=>{}),gt=i,Mi=e}catch{}}function Gt(){if(gt)try{gt.pause(),gt.currentTime=0}catch{}gt=null,Mi=null}let Xt=null;function zn(e,t=.6){if(Ue(),!Yi())try{const i=new Audio(e);i.volume=ui(t),i.play().catch(()=>{}),Xt=i}catch{}}function Ue(){if(Xt)try{Xt.pause(),Xt.currentTime=0}catch{}Xt=null}const ko="2026.07.22-1533";async function fn(e,{state:t,navigate:i,toast:n}){var o,a;const r=t.profile;r&&(e.innerHTML=`
  <style>
    .home-dark {
      min-height: 100%;
  ...
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:0 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none;align-items:center"></div>
  ...
  </div>`,function(N){const V=document.getElementById(N);if(!V)return;V.addEventListener("wheel",le=>{Math.abs(le.deltaY)<=Math.abs(le.deltaX)||(le.preventDefault(),V.scrollLeft+=le.deltaY)},{passive:!1});let H=!1,re=0,B=0,R=!1,ue=!1;V.addEventListener("mousedown",le=>{H=!0,R=!1,re=le.pageX,B=V.scrollLeft}),window.addEventListener("mouseup",()=>{H=!1,R&&(V.style.cursor="",ue=!0),R=!1}),window.addEventListener("mousemove",le=>{if(!H)return;const he=le.pageX-re;!R&&Math.abs(he)<6||(R=!0,V.style.cursor="grabbing",le.preventDefault(),V.scrollLeft=B-he)}),V.addEventListener("click",le=>{ue&&(le.stopPropagation(),le.preventDefault(),ue=!1)},!0)}("col-grid");function I(){const $=document.getElementById("col-filters");$&&(z==="player"?($.innerHTML=`
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",N=>{S=N.target.value.toLowerCase(),K()}),e.querySelectorAll(".filter-btn").forEach(N=>{N.addEventListener("click",()=>{k=N.dataset.job,I(),K()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{q=!q,I(),K()})):($.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{q=!q,I(),K()})))}function K(){const $=document.getElementById("col-grid");$&&(z==="player"?Y($):z==="formation"?ce($):z==="stadium"?O($):G($))}function Q($,N,V,H,re,B=0){const R=document.getElementById("col-grid"),ue=document.getElementById("col-big");if(!R||!ue)return;var le=0;function he(){var ct;const Se=window.innerWidth>=768,ve=document.getElementById("col-big"),Te=document.getElementById("col-gap"),Ae=((ct=document.getElementById("col-grid"))==null?void 0:ct.parentElement)||null,Be=ve?ve.closest(".page"):null;if(ve&&Ae&&Be){let $e=0;Array.from(Be.children).forEach(function(C){C!==ve&&C!==Ae&&C!==Te&&($e+=C.offsetHeight)});const Ce=Math.max(0,Be.clientHeight-$e),qe=Math.round(Ce*(B/100)),Ge=Math.max(0,Ce-qe),w=Se?50/71:45/63,L=Math.round(Ge*w),A=Math.max(0,Ge-L);Te&&(Te.style.height=qe+"px"),ve.style.flex="none",ve.style.height=L+"px",ve.style.minHeight="0",Ae.style.flex="none",Ae.style.height=A+"px",Ae.style.minHeight="0",Ae.style.display="flex",Ae.style.overflow="hidden"}const ke=document.getElementById("col-grid");ke&&(ke.style.height="100%",ke.style.flexShrink="0",ke.style.overflowX="auto",ke.style.overflowY="hidden",ke.style.alignItems="center",ke.style.width="100%"),ue.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:center center">'+N($[le])+"</div>";var wt=ue.querySelector("[data-card-id],[data-form-id],[data-gc-id]");wt&&wt.addEventListener("click",function(){H($[le])}),requestAnimationFrame(function(){var $e=document.getElementById("big-card-inner");if(!(!$e||!ue)){var Ce=ue.clientHeight,qe=ue.clientWidth-16,Ge=$e.offsetHeight,w=$e.offsetWidth;if(Ge>0&&w>0&&Ce>40){var L=Math.min(Ce/Ge,qe/w);$e.style.transform="scale("+L.toFixed(3)+")",$e.style.transformOrigin="center center"}}}),R.innerHTML=$.map(function($e,Ce){var qe=Ce===le,Ge="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+(qe?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+Ce+'" style="'+Ge+'">'+V($e,qe)+"</div>"}).join(""),R.querySelectorAll(".col-mini-item").forEach(function($e){$e.addEventListener("click",function(){le=Number($e.dataset.idx),he(),$e.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})}),requestAnimationFrame(function(){var $e=ke?ke.clientHeight:0,Ce=R.querySelector(".col-mini-item > div");if(!(!$e||!Ce)){var qe=Ce.style.zoom;Ce.style.zoom="1";var Ge=Ce.offsetHeight;if(Ce.style.zoom=qe,!(Ge<=0)){var w=$e/Ge;R.querySelectorAll(".col-mini-item > div").forEach(function(L){L.style.zoom=w.toFixed(4)})}}})}he()}function pe($){var N=window.innerWidth>=768?.76:.54,V;if(!$||$._fake){var H=$?$.player:null;if(!H)return"";V=xn(H)}else V=Ii($,"");return'<div style="display:inline-block;zoom:'+N+';pointer-events:none;line-height:0">'+V+"</div>"}function fe($,N,V){var H=V>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+V+"</div>":"",re=!!N,B=zi($,zt[$],{width:160});return re||(B='<div style="filter:grayscale(1);opacity:0.5">'+B+"</div>"),"<div "+(N?'data-form-id="'+N.id+'"':"")+' style="position:relative;cursor:pointer">'+H+B+"</div>"}function M($,N){var V=window.innerWidth>=768?.76:.54,H=zi($,zt[$],{width:140});return N||(H='<div style="filter:grayscale(1);opacity:0.45">'+H+"</div>"),'<div style="display:inline-block;zoom:'+V+';line-height:0;pointer-events:none">'+H+"</div>"}function Y($){if(q){const N=j();if(!N.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucun joueur.</div>';return}const V=N.map(H=>c.find(re=>re.player.id===H.id)||{_fake:!0,player:H,id:"fake-"+H.id});Q(V,H=>H._fake?xn(H.player):Ii(H,""),H=>H._fake?pe({player:H.player,id:"x",_fake:!0}):pe(H),H=>{H._fake||bn(H,c,v,t)})}else{const N=Z();if(!N.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const V={};N.forEach(re=>{const B=re.player.id;V[B]||(V[B]=[]),V[B].push(re)});const H=Object.values(V).map(re=>qo(re));Q(H,re=>{const B=v[re.player.id]||1,R=B>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${B}</div>`:"",le=c.filter(he=>he.player.id===re.player.id&&he.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Ii(re,R+le)},re=>pe(re),re=>bn(re,c,v,t))}}function ce($){const N=q?x:[...E];if(!N.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const V=N.map(H=>({formation:H,card:m.find(re=>re.formation===H)||null,owned:E.has(H)}));Q(V,({formation:H,card:re,owned:B})=>fe(H,B?re:null,B?m.filter(R=>R.formation===H).length:0),({formation:H,owned:re})=>M(H,re),({card:H,owned:re})=>{re&&H&&No(H,m,t,o)},"#1A6B3C",5)}function G($){const N=Object.keys(g),V=q?N.length?N:h:[...f];if(!V.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const H=V.map(re=>({type:re,gc:je[re]||{icon:"⚡",desc:""},def:g[re]||null,owned:f.has(re),card:l.find(B=>B.gc_type===re)||null}));Q(H,({type:re,gc:B,def:R,owned:ue,card:le})=>{var ke;const he=(R==null?void 0:R.name)||re,Se=ue?l.filter(wt=>wt.gc_type===re).length:0,ve=Se>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${Se}</div>`:"",Te=(R==null?void 0:R.effect)||B.desc||"",Ae=R!=null&&R.image_url?`/icons/${R.image_url}`:((ke=R==null?void 0:R.club)==null?void 0:ke.logo_url)||(R!=null&&R.country_code?`https://flagsapi.com/${R.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null);let Be=tt(he,Ae,B.icon,Te,{width:160,onClick:ue});return ue||(Be=`<div style="filter:grayscale(1);opacity:0.5">${Be}</div>`),`<div ${ue&&le?`data-gc-id="${le.id}" data-gc-type="${re}"`:""} style="position:relative">${ve}${Be}</div>`},({type:re,gc:B,def:R,owned:ue})=>{const le=window.innerWidth>=768?.76:.54,he=(R==null?void 0:R.name)||re,Se=(R==null?void 0:R.effect)||B.desc||"",ve=R!=null&&R.image_url?`/icons/${R.image_url}`:null;let Te=tt(he,ve,B.icon,Se,{width:140});return ue||(Te=`<div style="filter:grayscale(1);opacity:0.45">${Te}</div>`),`<div style="display:inline-block;zoom:${le};line-height:0;pointer-events:none">${Te}</div>`},({type:re,owned:B,def:R})=>{B&&Go(re,R,o)},"#7a28b8",5)}function O($){const N="#4FC3F7",V="/";if(!b.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Stade.<br><small>Ouvre un booster Stade !</small></div>';return}const H={};b.forEach(B=>{const R=B.stadium_id||"?";(H[R]=H[R]||[]).push(B)});const re=Object.entries(H).map(([B,R])=>({sid:B,def:u[B]||null,count:R.length,card:R[0]}));Q(re,({def:B,count:R})=>{var Te,Ae;const ue=(B==null?void 0:B.name)||"?",le=((Te=B==null?void 0:B.club)==null?void 0:Te.encoded_name)||(B==null?void 0:B.country_code)||"—",he=B!=null&&B.image_url?`${V}icons/${B.image_url}`:((Ae=B==null?void 0:B.club)==null?void 0:Ae.logo_url)||(B!=null&&B.country_code?`https://flagsapi.com/${B.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),Se=R>1?`<div style="position:absolute;top:8px;right:8px;background:#333;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${R}</div>`:"",ve=`${le}<br>+10 ⭐ joueurs alliés`;return`<div style="position:relative">${Se}${ti(ue,he,ve,{width:160})}</div>`},({def:B})=>{var ve,Te;const R=window.innerWidth>=768?.76:.54,ue=(B==null?void 0:B.name)||"?",le=((ve=B==null?void 0:B.club)==null?void 0:ve.encoded_name)||(B==null?void 0:B.country_code)||"—",he=B!=null&&B.image_url?`${V}icons/${B.image_url}`:((Te=B==null?void 0:B.club)==null?void 0:Te.logo_url)||(B!=null&&B.country_code?`https://flagsapi.com/${B.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),Se=`${le}<br>+10 ⭐`;return`<div style="display:inline-block;zoom:${R};line-height:0;pointer-events:none">${ti(ue,he,Se,{width:140})}</div>`},null,N,5)}e.querySelectorAll(".col-tab-btn").forEach($=>{$.addEventListener("click",()=>{z=$.dataset.tab,k="Tous",S="",q=!1,e.querySelectorAll(".col-tab-btn").forEach(N=>{const V=N.dataset.tab===z;N.style.borderBottomColor=V?"var(--green)":"transparent",N.style.color=V?"var(--green)":"var(--tile-fg-dim)"}),I(),K()})}),I(),K()}function Go(e,t,i){const n=je[e]||{icon:"⚡",desc:"Effet spécial."},r=(t==null?void 0:t.name)||e,o=(t==null?void 0:t.effect)||n.desc,a=t!=null&&t.image_url?`/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
      ${tt(r,a,n.icon,o,{width:200})}
      <div style="background:#fff3cd;border-radius:10px;padding:10px 14px;width:100%">
        <div style="font-size:12px;color:#856404">⚠️ Cette carte est à <b>usage unique</b>. Une fois jouée en match, elle est définitivement supprimée de ta collection.</div>
      </div>
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const li=1e3;function No(e,t,i,n){var g,p,u;const{state:r,toast:o,closeModal:a,navigate:s,refreshProfile:d}=i,c=e.formation,l={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function m(){const x=zt[c]||{},h=Oi[c]||[],v=290,E=360,f=20;function z(S){const q=x[S];return q?{x:q.x*v,y:q.y*E}:null}let k=`<svg width="${v}" height="${E}" viewBox="0 0 ${v} ${E}" xmlns="http://www.w3.org/2000/svg">`;for(const[S,q]of h){const ie=z(S),ne=z(q);!ie||!ne||(k+=`<line x1="${ie.x}" y1="${ie.y}" x2="${ne.x}" y2="${ne.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const S of Object.keys(x)){const q=z(S);if(!q)continue;const ie=S.replace(/\d+/,""),ne=l[ie]||"#555";k+=`<circle cx="${q.x}" cy="${q.y}" r="${f}" fill="${ne}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,k+=`<text x="${q.x}" y="${q.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${ie}</text>`}return k+="</svg>",k}const b=t.filter(x=>x.formation===c);b.length;const y=!e.is_for_sale;n(`Formation ${c}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(g=document.getElementById("direct-sell-form-btn"))==null||g.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${c} pour ${li.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const x=b.find(v=>!v.is_for_sale)||b[0];if(!x){o("Aucune carte à vendre","error");return}await _.from("market_listings").delete().eq("card_id",x.id),await _.from("transfer_history").delete().eq("card_id",x.id);const{error:h}=await _.from("cards").delete().eq("id",x.id);if(h){o(h.message,"error");return}await _.from("users").update({credits:(r.profile.credits||0)+li}).eq("id",r.profile.id),await d(),o(`+${li.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),a(),s("collection")}),(p=document.getElementById("market-sell-form-btn"))==null||p.addEventListener("click",async()=>{const x=parseInt(document.getElementById("sell-price-form").value);if(!x||x<1){o("Prix invalide","error");return}await _.from("cards").update({is_for_sale:!0,sale_price:x}).eq("id",e.id),await _.from("market_listings").insert({seller_id:r.profile.id,card_id:e.id,price:x}),o("Carte mise en vente sur le marché !","success"),a(),s("collection")}),(u=document.getElementById("cancel-sell-form-btn"))==null||u.addEventListener("click",async()=>{await _.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await _.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),o("Annonce retirée","success"),a(),s("collection")})}async function bn(e,t,i,n){var pe,fe,M,Y,ce;const{state:r,toast:o,openModal:a,closeModal:s,navigate:d,refreshProfile:c}=n,l=e.player,m=t.filter(G=>G.player.id===l.id),b=m.length,y=e.evolution_bonus||0,g=Math.max((Number(l.note_g)||0)+(l.job==="GK"||l.job2==="GK"?y:0),(Number(l.note_d)||0)+(l.job==="DEF"||l.job2==="DEF"?y:0),(Number(l.note_m)||0)+(l.job==="MIL"||l.job2==="MIL"?y:0),(Number(l.note_a)||0)+(l.job==="ATT"||l.job2==="ATT"?y:0)),p=l.rarity||"normal",{data:u}=await _.from("sell_price_configs").select("*").eq("rarity",p).lte("note_min",g).gte("note_max",g).order("note_min",{ascending:!1}).limit(1);((pe=u==null?void 0:u[0])==null?void 0:pe.price)??Bo[p];const x=l.rarity!=="legende";At(l);const h=((l.rarity==="pepite"||l.rarity==="papyte")&&e.current_note!=null?e.current_note:Ti(l,l.job))+y,v=l.rarity==="pepite"||l.rarity==="papyte",E=l.job2?(v?zo(e,Do(l.job2)):Ti(l,l.job2))+(Ti(l,l.job2)>0?y:0):null;Li[l.job],l.job2&&Li[l.job2];const f=Co[l.rarity]||"#ccc";Fo[l.country_code]||l.country_code;const z=e.evolution_bonus||0,S=h+z,q=E||0,ie=q>0?q+z:0,ne=m.map(G=>G.id);let Z={};if(ne.length){const{data:G}=await _.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",ne).order("transferred_at",{ascending:!0});(G||[]).forEach(O=>{Z[O.card_id]||(Z[O.card_id]=[]),Z[O.card_id].push(O)})}const j=G=>{const O=G.transferred_at?new Date(G.transferred_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"2-digit",year:"numeric"}):"",$=G.source==="booster"?"Booster":G.price?G.price.toLocaleString("fr"):"—";return`
    ...
    <div style="margin-top:16px;border-top:1px solid var(--tile-border);padding-top:14px">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${m.map((G,O)=>{const $=Z[G.id]||[],N=G.is_for_sale,V=$.length?$[$.length-1]:null,re=(l.rarity==="pepite"||l.rarity==="papyte")&&G.current_note!=null?` (☆${G.current_note})`:"";return`
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;background:#f0fdf4;border-radius:10px;padding:10px 14px">
          <div>
            ...
            <div style="font-size:11px;color:#555;margin-top:2px">
              ${(e.evolution_bonus||0)>0?`+${e.evolution_bonus} appliqué${e.evolution_bonus>1?"s":""} · Note : ${S}${l.job2&&q>0?` / ${ie}`:""}`:`Note actuelle : ${S}${l.job2&&q>0?` / ${ie}`:""}`}
            </div>
          </div>
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(fe=document.getElementById("close-detail"))==null||fe.addEventListener("click",s);let K=new Set;const Q=()=>{const G=K.size,O=document.getElementById("sell-action-panel");if(!O)return;O.style.display=G>0?"block":"none",document.getElementById("sell-selected-count").textContent=G;const $=document.getElementById("evolve-btn");$&&($.textContent=`⬆️ Évoluer ${G>1?"(+"+G+")":""}`)};document.querySelectorAll(".expl-check").forEach(G=>{G.addEventListener("change",()=>{const O=G.dataset.id;G.checked?K.add(O):K.delete(O);const $=G.closest(".exemplaire-row");$&&($.style.borderColor=G.checked?"#1A6B3C":"#eee"),Q()})}),document.querySelectorAll(".exemplaire-row").forEach(G=>{G.addEventListener("click",O=>{if(O.target.closest("button")||O.target.tagName==="INPUT")return;const $=G.querySelector(".expl-check");$&&!$.disabled&&($.checked=!$.checked,$.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(G=>{G.addEventListener("click",O=>{O.stopPropagation();const $=document.querySelector(`.expl-hist[data-hist="${G.dataset.idx}"]`);$&&($.style.display=$.style.display==="none"?"flex":"none")})}),(M=document.getElementById("evolve-btn"))==null||M.addEventListener("click",async()=>{if(b<=1)return;const G=[...K];if(!G.length)return;if(K.has(e.id)){const B=document.createElement("div");B.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",B.innerHTML=`
        ...
        <div style="background:var(--tile-bg);border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center;box-shadow:0 16px 48px rgba(0,0,0,0.3)">
          ...
          <div style="background:#f0fdf4;border-radius:10px;padding:12px;margin-bottom:16px;font-size:13px;color:#333">
            ...
            📈 Note : <strong>${h+(e.evolution_bonus||0)}</strong> → <strong style="color:#1A6B3C">${h+(e.evolution_bonus||0)+O.length*$}</strong>
  ...
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
    ...
    </div>`;u("⚽ Choisir une formation",h),document.querySelectorAll("#modal-body [data-forma]").forEach(v=>{v.addEventListener("click",()=>{t.formation=v.dataset.forma;const E=_n(t.formation),f={};E.forEach(z=>{t.slots[z]&&(f[z]=t.slots[z])}),t.slots=f,x(),_t(e,t,i)})})};e.querySelectorAll("#formation-mobile-btn, #formation-pc-btn").forEach(u=>u.addEventListener("click",y)),e.querySelectorAll("#add-stad-btn-pc, #add-stad-btn").forEach(u=>u.addEventListener("click",()=>Ho(t,e,i))),e.querySelectorAll("#save-deck").forEach(u=>u.addEventListener("click",()=>Vo(t,i))),e.querySelectorAll("[data-remove-sub]").forEach(u=>{u.addEventListener("click",()=>{t.subs=t.subs.filter(x=>x!==u.dataset.removeSub),_t(e,t,i)})}),e.querySelectorAll("#add-sub-btn").forEach(u=>u.addEventListener("click",()=>Ko(t,e,i)))}function Oo(e,t,i,n){var v,E,f;const r=window.innerWidth>=900,o=e.querySelector(r?"#deck-field-pc":"#deck-field-mobile");if(!o)return;const a=(v=t.stadiumCards)==null?void 0:v.find(z=>z.id===t.stadiumCardId),s=a&&((E=t.stadDefMap)==null?void 0:E[a.stadium_id])||null,d=zt[t.formation]||{},c=Ri(t.formation)||[],l={};for(const z of i){const k=t.slots[z],S=k?t.playerCards.find(q=>q.id===k):null;S!=null&&S.player?l[z]={...S.player,_evolution_bonus:S.evolution_bonus||0,face:S.player.face||null}:l[z]=null}const m=window.innerWidth>=900,b=m?window.innerWidth-280:window.innerWidth-20,y=m?Math.min(b,860):b,g=Math.round(m?y*.82:y*.85),p=m?84:44;let u="";for(const[z,k]of c){const S=d[z],q=d[k];if(!S||!q)continue;const ie=S.x*y,ne=Math.round(.03*g+S.y*.85*g),Z=q.x*y,j=Math.round(.03*g+q.y*.85*g),I=l[z],K=l[k],Q=Ut(I,K);Q==="#ff3333"||Q==="#cc2222"?u+=`<line x1="${ie.toFixed(1)}" y1="${ne.toFixed(1)}" x2="${Z.toFixed(1)}" y2="${j.toFixed(1)}" stroke="${Q}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(u+=`<line x1="${ie.toFixed(1)}" y1="${ne.toFixed(1)}" x2="${Z.toFixed(1)}" y2="${j.toFixed(1)}" stroke="${Q}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,u+=`<line x1="${ie.toFixed(1)}" y1="${ne.toFixed(1)}" x2="${Z.toFixed(1)}" y2="${j.toFixed(1)}" stroke="${Q}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let x="";const h=Math.round(p*657/507);for(const z of i){const k=d[z];if(!k)continue;const S=l[z],q=k.x*y;k.y*g;const ie=Math.round(.03*g+k.y*(.85*g)),ne=Math.round(q-p/2),Z=Math.round(ie-h/2);if(S){const j=z.replace(/\d+/,""),I=s&&(s.club_id&&String(S.club_id)===String(s.club_id)||s.country_code&&S.country_code===s.country_code),K=Ie({...S,_evolution_bonus:S._evolution_bonus||0},{width:p,showStad:!0,stadDef:s,role:j});I&&((f=s.club)!=null&&f.logo_url||s.image_url),x+=`<div style="position:absolute;left:${ne}px;top:${Z}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${z}">
        <div style="position:relative">${K}</div>
      </div>`}else{const j=z.replace(/\d+/,"");x+=`<div style="position:absolute;left:${ne}px;top:${Z}px;width:${p}px;height:${h}px;
  ...
  `,document.body.appendChild(r);let o=!1;const a=document.getElementById("pack-cut-zone"),s=document.getElementById("pack-blade");let d=!1;const c=I=>I.touches&&I.touches[0]?I.touches[0].clientX:I.clientX;function l(I){o||(d=!0,s.style.opacity="1",m(I))}function m(I){if(!d||o)return;const K=a.getBoundingClientRect(),Q=c(I)-K.left,pe=Math.max(0,Math.min(1,Q/K.width));s.style.width=pe*K.width+"px",pe>=.82&&y()}function b(){o||(d=!1,s.style.transition="width .2s ease, opacity .2s ease",s.style.width="0",s.style.opacity="0",setTimeout(()=>{o||(s.style.transition="")},220))}function y(){var K;if(o)return;o=!0,d=!1,s.style.width="100%",s.style.opacity="1",(K=document.getElementById("cut-flash"))==null||K.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const I=document.getElementById("cut-hint");I&&(I.style.opacity="0"),a.classList.add("pack-cut"),setTimeout(()=>{s.style.opacity="0",document.getElementById("pack-phase").style.display="none",x(0)},620)}a.addEventListener("pointerdown",l),window.addEventListener("pointermove",m),window.addEventListener("pointerup",b),a.addEventListener("touchstart",l,{passive:!0}),window.addEventListener("touchmove",m,{passive:!0}),window.addEventListener("touchend",b);let g=0,p=!1;const u=new Set;function x(I){g=I,document.getElementById("reveal-phase").style.display="flex",h(),v(I,0),S()}function h(){const I=document.getElementById("card-dots");I&&(I.innerHTML=e.map((K,Q)=>`<div class="card-dot" data-i="${Q}" style="width:8px;height:8px;border-radius:50%;background:${Q===g?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),I.querySelectorAll(".card-dot").forEach(K=>K.addEventListener("click",()=>f(parseInt(K.dataset.i)))))}function v(I,K){var $,N;const Q=e[I],pe=document.getElementById("card-counter"),fe=document.getElementById("card-track");pe&&(pe.textContent=`Carte ${I+1} / ${e.length}`);const M=document.getElementById("reveal-btns");M&&(M.style.display=I===e.length-1?"flex":"none");const Y=Q.card_type==="player"&&(($=Q.player)==null?void 0:$.rarity)==="legende",ce=!u.has(I);u.add(I);let G=0;if(Q.card_type==="player"&&Q.player){const V=Q.player,H=V.job||"ATT";G=Number(H==="GK"?V.note_g:H==="DEF"?V.note_d:H==="MIL"?V.note_m:V.note_a)||0}const O=V=>{fe.innerHTML=`
        <div id="current-card-wrap" style="position:relative;display:flex;flex-direction:column;align-items:center;gap:8px;${Y?"filter:drop-shadow(0 0 20px #7a28b8)":""}">
          <div style="transform:scale(1.25);transform-origin:center">${ir(Q)}</div>
        ...
        </div>`;const H=document.getElementById("current-card-wrap");K!==0?(H.style.transition="none",H.style.transform=`translateX(${K>0?100:-100}%)`,requestAnimationFrame(()=>{H.style.transition="transform .28s cubic-bezier(.25,1,.5,1)",H.style.transform="translateX(0)"})):H.animate([{opacity:0,transform:"scale(.7)"},{opacity:1,transform:"scale(1)"}],{duration:300,easing:"cubic-bezier(.34,1.56,.64,1)"}),V||Y?ie():ne(),h()};ce&&(((N=Q.player)==null?void 0:N.rarity)==="legende"||G>=8)&&Q.card_type==="player"&&Q.player?E(Q,()=>O(!0)):O(!1)}function E(I,K){var Se;p=!0;const Q=I.player,pe=`https://flagsapi.com/${Q.country_code}/flat/64.png`,fe=(Se=Q.clubs)==null?void 0:Se.logo_url,M=At(Q),Y=Q.job||"ATT",ce=Number(Y==="GK"?Q.note_g:Y==="DEF"?Q.note_d:Y==="MIL"?Q.note_m:Q.note_a)||0,G=I.evolution_bonus||0,O=ce+G,$=O>=18&&O<=20,N=O>=18,V=document.getElementById("walkout-overlay"),H=document.getElementById("walkout-stage");if(!V||!H){p=!1,K();return}let re=null;$&&(re=Wi("/sounds/Legendary.mp3",.8)),V.style.display="flex";const B=()=>{const ve=H.firstElementChild;ve&&(ve.classList.remove("wo-in"),ve.classList.add("wo-out"))},R=1800,ue=400;H.innerHTML=`<img class="wo-in" src="${pe}" style="height:130px;border-radius:10px;box-shadow:0 10px 36px rgba(0,0,0,.6)" onerror="this.style.display='none'">`,navigator.vibrate&&navigator.vibrate(30),setTimeout(B,R),setTimeout(()=>{var ve;H.innerHTML=fe?`<img class="wo-in" src="${fe}" style="max-height:160px;max-width:210px;object-fit:contain">`:`<div class="wo-in" style="font-size:34px;font-weight:900;color:#fff;text-align:center">${((ve=Q.clubs)==null?void 0:ve.encoded_name)||"CLUB"}</div>`,navigator.vibrate&&navigator.vibrate(30)},R+ue),setTimeout(B,R*2+ue),setTimeout(()=>{H.innerHTML=M?`<img class="wo-in" src="${M}" style="height:200px;border-radius:50%;box-shadow:0 0 40px rgba(255,255,255,0.3);object-fit:cover;object-position:top">`:'<div class="wo-in" style="font-size:80px">👤</div>',navigator.vibrate&&navigator.vibrate(30)},(R+ue)*2),setTimeout(B,(R+ue)*2+R);const he=N?"#FFD700":{GK:"#c0c0c0",DEF:"#e03030",MIL:"#D4A017",ATT:"#3fbf5f"}[Y]||"#fff";setTimeout(()=>{H.innerHTML=`<div class="wo-in" style="
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      card:cards(id,card_type,formation,stadium_id,evolution_bonus,
        player:players(id,firstname,surname_real,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order"),d=[...new Set((s||[]).filter(b=>{var y,g;return((y=b.card)==null?void 0:y.card_type)==="stadium"&&((g=b.card)==null?void 0:g.stadium_id)}).map(b=>b.card.stadium_id))],c={};if(d.length){const{data:b}=await _.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",d);(b||[]).forEach(y=>{c[y.id]=y}),(s||[]).forEach(y=>{var g,p;((g=y.card)==null?void 0:g.card_type)==="stadium"&&((p=y.card)!=null&&p.stadium_id)&&(y.card._stadiumDef=c[y.card.stadium_id]||null)})}let l=0;function m(){var f,z,k,S,q,ie,ne;const b=o[l],y=(s||[]).filter(Z=>Z.deck_id===b.id),g=y.filter(Z=>{var j;return Z.is_starter&&((j=Z.card)==null?void 0:j.player)}).map(Z=>qi(Z.card,Z.position)),p=y.find(Z=>{var j;return((j=Z.card)==null?void 0:j.card_type)==="formation"}),u=b.formation||((f=p==null?void 0:p.card)==null?void 0:f.formation)||"4-4-2";let x=g.length>=11?gi(g,u):null,h=((z=b.stadium_card)==null?void 0:z.stadium_def)||null;h&&x&&(x=ai(x,h));const v=g.length>=11;st(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff;position:relative">
      ...
      <div id="deck-bottom-bar" style="flex-shrink:0;padding:10px 14px 14px;display:flex;flex-direction:column;gap:8px;background:rgba(0,0,0,0.2)">
        ...
        stroke="${k}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const g=typeof window<"u"&&window.innerWidth>=900?Math.min(Math.max(81,Math.round(r*.225)),117):Math.max(44,Math.round(r*.168)),p=Math.round(g*657/507);for(const[x,h]of Object.entries(l)){const v=b(x);if(!v||!h)continue;const E=x.replace(/[0-9]/g,""),f=a.includes(h.cardId),z=i==="attack"&&(["MIL","ATT"].includes(E)||f)&&!h.used||i==="defense"&&["GK","DEF","MIL"].includes(E)&&!h.used,k=n.includes(h.cardId);let S=h.boost||0,q=!1;h.stadiumBonus&&(i==="attack"&&(E==="ATT"||E==="MIL")||i==="defense"&&(E==="GK"||E==="DEF"||E==="MIL")?(S+=10,q=!0):i||(S+=10,q=!0));const ie=Math.round(v.x-g/2),ne=Math.round(v.y-p/2);if(h.used){y+=`<image href="${`${typeof import.meta<"u"&&"/"||"/"}icons/carte-dos.png`}" x="${ie}" y="${ne}" width="${g}" height="${p}" preserveAspectRatio="xMidYMid slice" class="match-used-hit" data-card-id="${h.cardId}" data-role="${E}" style="cursor:pointer"/>`;continue}const Z=Ie({...h,_evolution_bonus:0,stadiumBonus:!1},{width:g,showStad:!1,stadDef:null,role:E,extraNote:S,_cardOffset:30,_forceStadColor:q}),j=k?`position:absolute;top:30px;left:0;width:${g}px;height:${p}px;outline:3px solid #FFD700;outline-offset:2px;border-radius:8px;pointer-events:none;`:"";y+=`<foreignObject x="${ie-2}" y="${ne-30}" width="${g+8}" height="${p+60}" style="overflow:visible">
      <div xmlns="http://www.w3.org/1999/xhtml" style="position:relative">
        ${Z}
        ${k?`<div style="${j}"></div>`:""}
      </div>
    </foreignObject>`,z&&(y+=`<rect x="${ie}" y="${ne}" width="${g}" height="${p}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${k?"selected":""}" data-card-id="${h.cardId}" data-role="${E}" style="cursor:pointer"/>`)}const u=s!==null?s:Math.round(Math.max(g*.7,80));return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-u} ${-u} ${r+u*2} ${o+u*2}" width="100%" style="display:block;width:100%;margin:0 auto">
    ${y}
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const Y=t.pendingAttack;let ce="";if(t.selected.length>0){const G=t.selected.map($=>({...(t.homeTeam[$._role]||[]).find(V=>V.cardId===$.cardId)||$,_line:$._role})),O=ri(G,t.modifiers.home,t.formation);ce=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
            ...
            <div style="display:flex;justify-content:center">${Je(Y.map(G=>({...G,used:!1})),"#FFD700",ce.total,"attack",t.formation)}</div>
    ...
    ${(()=>{const M=window.innerWidth>=700,Y=(B,R,ue)=>{var Ae,Be;const le=(t.gcDefs||[]).find(ke=>ke.name===B.gc_type),he=(le==null?void 0:le.name)||B.gc_type,Se=(le==null?void 0:le.effect)||((Ae=je[B.gc_type])==null?void 0:Ae.desc)||"",ve=le!=null&&le.image_url?`/icons/${le.image_url}`:null,Te=((Be=je[B.gc_type])==null?void 0:Be.icon)||"⚡";return`<div class="gc-mini" data-gc-id="${B.id}" data-gc-type="${B.gc_type}" style="flex-shrink:0;cursor:pointer">
          ${tt(he,ve,Te,Se,{width:R})}
        </div>`},ce=(B,R)=>{var ue;return`<div id="boost-card"
  ...
  <div id="match-history-panel">
    ...
    </div>`,(f=m.querySelector("#sub-close"))==null||f.addEventListener("click",()=>m.remove()),(z=m.querySelector("#out-up"))==null||z.addEventListener("click",()=>{s>0&&(s--,b())}),(k=m.querySelector("#out-down"))==null||k.addEventListener("click",()=>{s<o.length-1&&(s++,b())}),(S=m.querySelector("#in-up"))==null||S.addEventListener("click",()=>{c>0&&(c--,b())}),(q=m.querySelector("#in-down"))==null||q.addEventListener("click",()=>{c<a.length-1&&(c++,b())});const E=(ne,Z,j,I)=>{const K=m.querySelector("#"+ne);if(!K)return;let Q=0;K.addEventListener("touchstart",pe=>{Q=pe.touches[0].clientY},{passive:!0}),K.addEventListener("touchend",pe=>{const fe=pe.changedTouches[0].clientY-Q;if(Math.abs(fe)<30)return;const M=Z();fe<0&&M<I-1?(j(M+1),b()):fe>0&&M>0&&(j(M-1),b())},{passive:!0})};E("in-panel",()=>c,ne=>c=ne,a.length),E("out-panel",()=>s,ne=>s=ne,o.length),(ie=m.querySelector("#sub-confirm"))==null||ie.addEventListener("click",ne=>{if(ne.preventDefault(),ne.stopPropagation(),l)return;l=!0;const Z=o[s],j=a[c];if(!Z||!j)return;let I=null,K=-1;for(const[pe,fe]of Object.entries(t.homeTeam)){const M=(fe||[]).findIndex(Y=>Y.cardId===Z.cardId);if(M!==-1){I=pe,K=M;break}}if(K===-1||!I){$t("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),m.remove();return}const Q={...j,_line:I,_col:Z._col||0,used:!1,boost:0};t.homeTeam[I].splice(K,1,Q),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(j.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:Z.name,firstname:Z.firstname,note:Pe(Z,I),portrait:At(Z),job:Z.job,country_code:Z.country_code,rarity:Z.rarity,clubName:Z.clubName,clubLogo:Z.clubLogo},inPlayer:{name:j.name,firstname:j.firstname,note:Pe(j,I),portrait:At(j),job:j.job,country_code:j.country_code,rarity:j.rarity,clubName:j.clubName,clubLogo:j.clubLogo},text:`🔄 ${j.firstname} ${j.name} remplace ${Z.firstname} ${Z.name}`}),m.remove(),ur(Z,j,()=>we(e,t,i))})}document.body.appendChild(m),b()}function $r(e,t,i,n,r){var b,y;const o=(n.gcDefs||[]).find(g=>g.name===t),a=je[t]||{icon:"⚡",desc:"Carte spéciale."},s=(o==null?void 0:o.name)||t,d=(o==null?void 0:o.effect)||a.desc,c=o!=null&&o.image_url?`/icons/${o.image_url}`:null,l=a.icon||"⚡",m=document.createElement("div");m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",m.innerHTML=`
    ${tt(s,c,l,d,{width:190})}
    <!-- Boutons -->
    <div style="display:flex;gap:12px;width:190px">
      <button id="gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
      <button id="gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
    </div>`,document.body.appendChild(m),(b=m.querySelector("#gc-back"))==null||b.addEventListener("click",()=>m.remove()),(y=m.querySelector("#gc-use"))==null||y.addEventListener("click",()=>{m.remove(),Lr(e,t,i,n,r)})}function ci(e,t,i,n,r,o){const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let s=[];function d(){var c,l;a.innerHTML=`
    ...
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      ...
      ${hi(e.aiTeam,e.formation,null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.1))}
            ...
            <div style="background:rgba(255,255,255,0.08);border-radius:14px;padding:14px 20px;text-align:center;min-width:220px">
              ...
              ${Le.id!==nt.id?`<div style="font-size:20px;font-weight:900;color:${Le.color}">
                    ${Ne>J.mmr?"📈":"📉"} ${nt.emoji} ${nt.label} → ${Le.emoji} ${Le.label}
                  </div>
                  <div style="font-size:13px;color:rgba(255,255,255,0.5);margin-top:4px">${Ne>J.mmr?"Promotion !":"Rétrogradation"}</div>`:`<div style="font-size:36px">${Ee>=0?"📈":"📉"}</div>
            ...
            </div>`}}catch(J){console.error("[Ranked] MMR update error:",J)}(te=document.getElementById("pvp-end-overlay"))==null||te.remove();const D=document.createElement("div");D.id="pvp-end-overlay",D.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;color:#fff;padding:24px;text-align:center;overflow-y:auto";const F=T?"🤝":C?"🏆":"😞",U=T?"MATCH NUL":C?"VICTOIRE !":"DÉFAITE",X=T?"#fff":C?"#FFD700":"#ff6b6b";D.innerHTML=`
      <div style="font-size:64px">${F}</div>
      <div style="font-size:26px;font-weight:900;color:${X}">${U}</div>
      ...
      ${P}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${l?"🏆 Retour à la Mini League":a?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(D),(ee=D.querySelector("#pvp-end-home"))==null||ee.addEventListener("click",()=>{D.remove(),it(e),l?y("mini-league",{openLeagueId:l}):y(a?"ranked":"home")})}const Z=_.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},w=>{var A;const L=w.new;try{if(L.status==="finished"||L.forfeit){if(z)return;z=!0,k&&(clearInterval(k),k=null),Ue(),L.game_state&&(f=L.game_state),ne(L);return}if(L.game_state){const C=f;f=L.game_state;const T=f._lastGC;if(T&&T.seq&&!ie.has(T.seq)&&(ie.add(T.seq),T.by!==p)){$(T.type,T.by,()=>Y());return}const P=C[p+"Score"]||0,D=C[u+"Score"]||0,F=f[p+"Score"]||0,U=f[u+"Score"]||0,X=F>P,te=U>D;if((X||te)&&!q.has(f.round)){q.add(f.round);const ee=[...f.log||[]].reverse().find(se=>se.isGoal),J=((ee==null?void 0:ee.homePlayers)||[]).map(se=>({name:se.name,note:se.note,portrait:se.portrait,job:se.job}));!!((A=ee==null?void 0:ee.text)!=null&&A.includes("DERNIER CORNER"))?Se().then(()=>{Qe(J,F,U,X,()=>Y())}):Qe(J,F,U,X,()=>Y());return}Y()}}catch(C){console.error("[PvP] crash:",C)}}).subscribe();async function j(w){Object.assign(f,w),f.lastActionAt=new Date().toISOString();const{error:L}=await _.from("matches").update({game_state:f}).eq("id",i);L&&g("Erreur de synchronisation","error");try{Y()}catch(A){console.error("[PvP] renderPvpScreen crash:",A)}}async function I(){if(z)return;z=!0,Gt(),Ue(),k&&(clearInterval(k),k=null);const w=n?v.away_id:v.home_id,L=n?"p2":"p1",A=n?"p1":"p2",C={...f,[L+"Score"]:3,[A+"Score"]:0,phase:"finished"},T=C.p1Score||0,P=C.p2Score||0;try{await _.from("matches").update({game_state:C}).eq("id",i);const{data:D,error:F}=await _.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:w,p_home_score:T,p_away_score:P,p_forfeit:!0});if(F&&console.error("[PvP] finish_pvp_match (forfeit):",F),l)try{await _.rpc("check_and_finish_mini_league",{p_league_id:l})}catch(U){console.error("[PvP] check_and_finish_mini_league (forfeit):",U)}try{const{data:U}=await _.rpc("apply_match_rewards",{p_match_id:i});U!=null&&U.success&&!(U!=null&&U.skipped)&&typeof t.refreshProfile=="function"&&await t.refreshProfile()}catch(U){console.error("[PvP] apply_match_rewards (forfeit):",U)}}catch{}try{_.removeChannel(Z)}catch{}setTimeout(()=>{it(e),l?y("mini-league",{openLeagueId:l}):y("home")},800)}const K={BOOST_STAT:({value:w=1,count:L=1,roles:A=[]},C,T)=>{const P=C[p+"Team"],D=Object.entries(P).filter(([F])=>!A.length||A.includes(F)).flatMap(([F,U])=>U.filter(X=>!X.used).map(X=>({...X,_line:F})));if(!D.length){C.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),T(C);return}Q(D,L,`Choisir ${L} joueur(s) à booster (+${w})`,F=>{F.forEach(U=>{const X=(P[U._line]||[]).find(te=>te.cardId===U.cardId);X&&(X.boost=(X.boost||0)+w,C.log.push({text:`⚡ +${w} sur ${X.name}`,type:"info"}))}),C[p+"Team"]=P,T(C)})},DEBUFF_STAT:({value:w=1,count:L=1,roles:A=[],target:C="ai"},T,P)=>{const D=C==="home"?p:u,F=T[D+"Team"],U=C==="home"?"allié":"adverse",X=Object.entries(F).filter(([te])=>!A.length||A.includes(te)).flatMap(([te,ee])=>ee.map(J=>({...J,_line:te})));if(!X.length){T.log.push({text:`🎯 Aucun joueur ${U}`,type:"info"}),P(T);return}Q(X,L,`Choisir ${L} joueur(s) ${U}(s) (-${w})`,te=>{te.forEach(ee=>{const J=(F[ee._line]||[]).find(ae=>ae.cardId===ee.cardId);J&&(J.boost=(J.boost||0)-w,T.log.push({text:`🎯 -${w} sur ${J.name}`,type:"info"}))}),T[D+"Team"]=F,P(T)})},GRAY_PLAYER:({count:w=1,roles:L=[],target:A="ai"},C,T)=>{const P=A==="home"?p:u,D=C[P+"Team"],F=A==="home"?"allié":"adverse",U=Object.entries(D).filter(([X])=>!L.length||L.includes(X)).flatMap(([X,te])=>te.filter(ee=>!ee.used).map(ee=>({...ee,_line:X})));if(!U.length){C.log.push({text:`❌ Aucun joueur ${F}`,type:"info"}),T(C);return}Q(U,w,`Choisir ${w} joueur(s) ${F}(s) à exclure`,X=>{const te="usedCardIds_"+P,ee=new Set(C[te]||[]);X.forEach(J=>{const ae=(D[J._line]||[]).find(se=>se.cardId===J.cardId);ae&&(ae.used=!0,ee.add(J.cardId),C.log.push({text:`❌ ${ae.name} exclu !`,type:"info"}))}),C[te]=[...ee],C[P+"Team"]=D,T(C)})},REVIVE_PLAYER:({count:w=1,roles:L=[]},A,C)=>{const T=A[p+"Team"],P=Object.entries(T).filter(([D])=>!L.length||L.includes(D)).flatMap(([D,F])=>F.filter(U=>U.used).map(U=>({...U,_line:D})));if(!P.length){A.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),C(A);return}Q(P,w,`Choisir ${w} joueur(s) à ressusciter`,D=>{D.forEach(F=>{const U=(T[F._line]||[]).find(X=>X.cardId===F.cardId);U&&(U.used=!1,A.log.push({text:`💫 ${U.name} ressuscité !`,type:"info"}))}),A[p+"Team"]=T,C(A)})},REMOVE_GOAL:({},w,L)=>{const A=u+"Score";w[A]>0?(w[A]--,w.log.push({text:"🚫 Dernier but annulé !",type:"info"})):w.log.push({text:"🚫 Aucun but à annuler",type:"info"}),L(w)},ADD_GOAL_DRAW:({},w,L)=>{w[p+"Score"]===w[u+"Score"]?(w[p+"Score"]++,w.log.push({text:"🎯 But bonus !",type:"info"})):w.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),L(w)},ADD_SUB:({value:w=1},L,A)=>{L.maxSubs=(L.maxSubs||3)+w,L.log.push({text:`🔄 +${w} remplacement(s)`,type:"info"}),A(L)},CUSTOM:({},w,L)=>L(w)};function Q(w,L,A,C){const T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let P=[];function D(){var U,X;const F=w.map(te=>{const ee=te._line||te.job||"MIL",J=P.find(se=>se.cardId===te.cardId),ae=Ie({...te,_evolution_bonus:0},{width:90,showStad:!0,role:ee,extraNote:te.boost||0});return`<div class="pp-item" data-cid="${te.cardId}"
          style="position:relative;border-radius:8px;${J?"outline:3px solid #FFD700;outline-offset:2px;":""}cursor:pointer;flex-shrink:0;${te.used?"opacity:0.3;pointer-events:none":""}">
          ${ae}
        ...
        </div>`,(U=T.querySelector("#pp-close"))==null||U.addEventListener("click",()=>T.remove()),T.querySelectorAll(".pp-item").forEach(te=>{te.addEventListener("click",()=>{const ee=te.dataset.cid,J=w.find(se=>se.cardId===ee),ae=P.findIndex(se=>se.cardId===ee);J&&(ae>-1?P.splice(ae,1):P.length<L&&P.push(J),D())})}),(X=T.querySelector("#pp-confirm"))==null||X.addEventListener("click",()=>{T.remove(),C(P)})}D(),document.body.appendChild(T)}async function pe(w,L){const C=(f["gcCardsFull_"+p]||[]).find(F=>F.id===w),T=(C==null?void 0:C._gcDef)||(f.gcDefs||[]).find(F=>{var U;return F.name===L||((U=F.name)==null?void 0:U.toLowerCase().trim())===(L==null?void 0:L.toLowerCase().trim())}),P=[...f["usedGc_"+p]||[],w],D={type:L,by:p,seq:(f._gcAnimSeq||0)+1};ie.add(D.seq),$(L,p,async()=>{if(T!=null&&T.effect_type&&T.effect_type!=="CUSTOM"){const U=K[T.effect_type];if(U){const X={...f};U(T.effect_params||{},X,async te=>{te["usedGc_"+p]=P,te._lastGC=D,te._gcAnimSeq=D.seq,await j(te)});return}}const F={...f};switch(L){case"Remplacement+":F.maxSubs=(F.maxSubs||3)+1,F.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const U=u+"Score";F[U]>0&&(F[U]--,F.log.push({text:"🚫 But annulé",type:"info"}));break}}F["usedGc_"+p]=P,F._lastGC=D,F._gcAnimSeq=D.seq,await j(F)})}function fe(w,L){const A="usedCardIds_"+w,C=new Set(f[A]||[]);L.forEach(T=>C.add(T)),f[A]=[...C]}function M(){for(const w of["p1","p2"]){const L=new Set(f["usedCardIds_"+w]||[]),A=f[w+"Team"];if(A)for(const C of["GK","DEF","MIL","ATT"])(A[C]||[]).forEach(T=>{T.used=L.has(T.cardId)})}}function Y(){var nn,on,rn,an,sn,dn;if(f.phase==="reveal")return ce();if(f.phase==="midfield")return O();if(f.phase==="finished")return Ge();const w=f[p+"Team"],L=f[u+"Team"];M();const A=f[p+"Score"],C=f[u+"Score"],T=f[p+"Name"],P=f[u+"Name"],D=f.phase===p+"-attack",F=f.phase===p+"-defense",U=Array.isArray(f["selected_"+p])?f["selected_"+p]:[],X=U.map(W=>W.cardId),te=window.innerWidth>=700,ee=f[p+"Subs"]||[],J=f["usedSubIds_"+p]||[],ae=ee.filter(W=>!J.includes(W.cardId)),se=f["gcCardsFull_"+p]||[],ge=f["usedGc_"+p]||[],de=se.filter(W=>!ge.includes(W.id)),ye=f.boostOwner===p&&!f.boostUsed,be=[...w.MIL||[],...w.ATT||[]].filter(W=>!W.used),oe=[...L.MIL||[],...L.ATT||[]].filter(W=>!W.used),Ee=!ke(L),Ne=be.length===0&&oe.length===0&&!Ee,Le=(w.DEF||[]).filter(W=>!W.used),Ve=(w.GK||[]).filter(W=>!W.used);let Oe=[];D&&be.length===0&&(Ee?(Oe=Le.map(W=>W.cardId),Le.length===0&&(Oe=Oe.concat(Ve.map(W=>W.cardId)))):Ne&&(Oe=[...Le,...Ve].map(W=>W.cardId)));function nt(W,me,ze){var Me,ft;const _e=W._gcDef,Bt=(_e==null?void 0:_e.name)||W.gc_type,at=(_e==null?void 0:_e.effect)||((Me=je[W.gc_type])==null?void 0:Me.desc)||"",Fe=_e!=null&&_e.image_url?`/icons/${_e.image_url}`:null,Re=((ft=je[W.gc_type])==null?void 0:ft.icon)||"⚡";return`<div class="pvp-gc-mini" data-gc-id="${W.id}" data-gc-type="${W.gc_type}" style="flex-shrink:0;cursor:pointer">
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${Ot(w,f[p+"Formation"],ut,X,te?1300:It(),te?600:Nt(),Oe)}
      </div>
    </div>`,rt=f[p+"Team"],ro=(()=>{var me,ze,_e,Bt,at;if(F&&((me=f.pendingAttack)!=null&&me.players)){const Fe=f.pendingAttack;let Re="";if(U.length>0){const Me=U.map(We=>{const Vt=(rt[We._role]||[]).find($i=>$i.cardId===We.cardId)||We,Yt=rt[We._role]||[],si=Yt.findIndex($i=>$i.cardId===We.cardId),ki=yt(Yt.length),Wt=si>=0?ki[si]:Vt._col??1;return{...Vt,_line:We._role,_col:Wt}}),ft=ri(Me,((ze=f.modifiers)==null?void 0:ze[p])||{},f[p+"Formation"]);Re=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
        ...
        </div>`}if(D&&!((Bt=f.pendingAttack)!=null&&Bt.players)&&U.length>0){const Fe=U.map(Me=>{const ft=(rt[Me._role]||[]).find(Wt=>Wt.cardId===Me.cardId)||Me,We=["GK","DEF"].includes(Me._role),Vt=rt[Me._role]||[],Yt=Vt.findIndex(Wt=>Wt.cardId===Me.cardId),si=yt(Vt.length),ki=Yt>=0?si[Yt]:ft._col??1;return{...ft,_line:Me._role,_col:ki,...We?{note_a:Math.max(1,Number(ft.note_a)||0)}:{}}}),Re=oi(Fe,((at=f.modifiers)==null?void 0:at[p])||{},f[p+"Formation"]);return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #FFD700;text-align:center">
        ...
        </div>`}const W=(f.log||[]).slice(-1)[0];return W?'<div style="padding:2px 4px">'+Yn(W)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3);text-align:center">⏳ Match en cours...</div>'})(),tn=`
      <div style="display:flex;align-items:center;padding:8px 10px;background:rgba(0,0,0,0.5);gap:6px;flex-shrink:0">
      ...
      </div>`;function jt(){const W=e.querySelector(".match-screen");if(!W)return;const me=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);W.style.height=me+"px",W.style.minHeight=me+"px",W.style.maxHeight=me+"px",W.style.overflow="hidden";const ze=e.querySelector("#mobile-action-bar"),_e=e.querySelector("#mobile-play-area");ze&&_e&&(_e.style.paddingBottom=ze.offsetHeight+"px")}if(jt(),setTimeout(jt,120),setTimeout(jt,400),S||(S=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",jt),window.visualViewport.addEventListener("scroll",jt)),window.addEventListener("resize",jt)),function(){const me=e.querySelector("#match-field .terrain-wrapper svg")||e.querySelector(".terrain-wrapper svg");if(!me)return;const ze=me.closest("#match-terrain-wrap");ze&&(ze.style.cssText="position:relative;width:100%;height:100%;padding:0"),me.removeAttribute("width"),me.removeAttribute("height"),me.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",me.setAttribute("preserveAspectRatio","xMidYMid meet")}(),f._pvpResizeBound||(f._pvpResizeBound=!0,window.addEventListener("resize",()=>{const W=e.querySelector(".terrain-wrapper svg");if(W){const me=W.closest("#match-terrain-wrap");me&&(me.style.cssText="position:relative;width:100%;height:100%;padding:0"),W.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0"}})),e.querySelectorAll(".match-slot-hit").forEach(W=>{W.addEventListener("click",()=>{if(!D&&!F)return;const me=W.dataset.cardId,ze=W.dataset.role,_e=(w[ze]||[]).find(Re=>Re.cardId===me);if(!_e||_e.used)return;const Bt=Oe.includes(me);if(D&&!["MIL","ATT"].includes(ze)&&!Bt)return;Array.isArray(f["selected_"+p])||(f["selected_"+p]=[]);const at=f["selected_"+p],Fe=at.findIndex(Re=>Re.cardId===me);Fe>-1?at.splice(Fe,1):at.length<3&&at.push({..._e,_role:ze}),Y()})}),e.querySelectorAll(".match-used-hit").forEach(W=>{W.addEventListener("click",()=>H(W.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(W=>{W.addEventListener("click",()=>H())}),(nn=e.querySelector("#pvp-sub-open"))==null||nn.addEventListener("click",()=>H()),e.querySelectorAll(".pvp-gc-mini").forEach(W=>{D?W.addEventListener("click",()=>N(W.dataset.gcId,W.dataset.gcType)):(W.style.opacity="0.35",W.style.cursor="default",W.addEventListener("click",()=>$t("⚡ Les Game Changers ne sont utilisables qu'en attaque","rgba(180,100,0,0.9)")))}),(on=e.querySelector("#pvp-boost-card"))==null||on.addEventListener("click",()=>V()),(rn=e.querySelector("#pvp-action"))==null||rn.addEventListener("click",W=>{D?W.currentTarget.dataset.pass==="1"||!ct(p)?ue():Te():F&&Ae()}),(an=e.querySelector("#pvp-quit"))==null||an.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&I()}),(sn=e.querySelector("#pvp-view-opp"))==null||sn.addEventListener("click",()=>B()),(dn=e.querySelector("#pvp-toggle-history"))==null||dn.addEventListener("click",()=>R()),k&&(clearInterval(k),k=null),Ue(),(D||F)&&!z){let W=30,me=!1;const ze=()=>document.getElementById("pvp-timer"),_e=()=>{ze()&&(ze().textContent=W+"s",ze().style.color=me?"#ff4444":"#fff")};_e(),k=setInterval(()=>{W--,W<0?me?(clearInterval(k),k=null,Ue(),D&&!ct(p)?ue():I()):(me=!0,W=15,_e(),zn("/sounds/timer-urgent.mp3",.6)):_e()},1e3)}}function ce(){st(e),e.innerHTML=`
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(ee);let J=!1;const ae=()=>{J||(J=!0,ee.remove(),setTimeout(()=>A&&A(),50))};ee.addEventListener("click",ae),setTimeout(ae,3e3)}function N(w,L){var te,ee,J,ae;const C=(f["gcCardsFull_"+p]||[]).find(se=>se.id===w),T=C==null?void 0:C._gcDef,P=(T==null?void 0:T.name)||L,D=(T==null?void 0:T.effect)||((te=je[L])==null?void 0:te.desc)||"Carte spéciale.",F=T!=null&&T.image_url?`/icons/${T.image_url}`:null,U=((ee=je[L])==null?void 0:ee.icon)||"⚡",X=document.createElement("div");X.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",X.innerHTML=`
      ${tt(P,F,U,D,{width:190})}
      <div style="display:flex;gap:12px;width:190px">
        <button id="pvp-gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
        <button id="pvp-gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
      </div>`,document.body.appendChild(X),(J=X.querySelector("#pvp-gc-back"))==null||J.addEventListener("click",()=>X.remove()),(ae=X.querySelector("#pvp-gc-use"))==null||ae.addEventListener("click",()=>{X.remove(),pe(w,L)})}function V(){var C;const w=f[p+"Team"],L=Object.entries(w).flatMap(([T,P])=>(P||[]).filter(D=>!D.used).map(D=>({...D,_line:T})));if(!L.length)return;const A=document.createElement("div");A.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",A.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
      ...
      </div>`,document.body.appendChild(A),(C=A.querySelector("#bp-close"))==null||C.addEventListener("click",()=>A.remove()),A.querySelectorAll(".bp-item").forEach(T=>{T.addEventListener("click",async()=>{const P=T.dataset.cid,D=L.find(U=>U.cardId===P);if(!D)return;const F=(w[D._line]||[]).find(U=>U.cardId===P);F&&(F.boost=(F.boost||0)+f.boostValue),A.remove(),await j({[p+"Team"]:w,boostUsed:!0})})})}function H(w=null){var ae,se;if(!(f.phase===p+"-attack")){g("Remplacement uniquement avant votre attaque","warning");return}const A=f[p+"Team"],C=f["usedSubIds_"+p]||[],T=f.maxSubs||3;if(C.length>=T){g(`Maximum ${T} remplacements atteint`,"warning");return}const P=Object.entries(A).flatMap(([ge,de])=>(de||[]).filter(ye=>ye.used).map(ye=>({...ye,_line:ge}))),D=(f[p+"Subs"]||[]).filter(ge=>!C.includes(ge.cardId));if(!P.length){g("Aucun joueur utilisé à remplacer","warning");return}if(!D.length){g("Aucun remplaçant disponible","warning");return}let F=Math.max(0,P.findIndex(ge=>ge.cardId===w));const U=((ae=P[F])==null?void 0:ae._line)||((se=P[F])==null?void 0:se.job);let X=Math.max(0,D.findIndex(ge=>ge.job===U)),te=!1;const ee=document.createElement("div");ee.id="pvp-sub-overlay",ee.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function J(){var Ne,Le,Ve,Oe,nt,Kt;const ge=P[F],de=D[X],ye=Math.min(130,Math.round((window.innerWidth-90)/2)),be=Math.round(ye*1.35),oe=De=>`background:rgba(255,255,255,0.12);border:none;color:${De?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${De?"default":"pointer"};flex-shrink:0`;ee.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      </div>`,(Ne=ee.querySelector("#psub-close"))==null||Ne.addEventListener("click",()=>ee.remove()),(Le=ee.querySelector("#pout-up"))==null||Le.addEventListener("click",()=>{F>0&&(F--,J())}),(Ve=ee.querySelector("#pout-down"))==null||Ve.addEventListener("click",()=>{F<P.length-1&&(F++,J())}),(Oe=ee.querySelector("#pin-up"))==null||Oe.addEventListener("click",()=>{X>0&&(X--,J())}),(nt=ee.querySelector("#pin-down"))==null||nt.addEventListener("click",()=>{X<D.length-1&&(X++,J())});const Ee=(De,Ye,He,pt)=>{const ot=ee.querySelector("#"+De);if(!ot)return;let Ct=0;ot.addEventListener("touchstart",ut=>{Ct=ut.touches[0].clientY},{passive:!0}),ot.addEventListener("touchend",ut=>{const kt=ut.changedTouches[0].clientY-Ct;if(Math.abs(kt)<30)return;const rt=Ye();kt<0&&rt<pt-1?(He(rt+1),J()):kt>0&&rt>0&&(He(rt-1),J())},{passive:!0})};Ee("pin-panel",()=>X,De=>X=De,D.length),Ee("pout-panel",()=>F,De=>F=De,P.length),(Kt=ee.querySelector("#psub-confirm"))==null||Kt.addEventListener("click",async De=>{if(De.preventDefault(),De.stopPropagation(),te)return;te=!0;const Ye=P[F],He=D[X];if(!Ye||!He)return;const pt=Ye._line,ot=(A[pt]||[]).findIndex(kt=>kt.cardId===Ye.cardId);if(ot===-1){g("Erreur : joueur introuvable","error"),ee.remove();return}const Ct={...He,_line:pt,position:Ye.position,used:!1,boost:0};A[pt].splice(ot,1,Ct);const ut=[...C,He.cardId];ee.remove(),re(Ye,He,async()=>{await j({[p+"Team"]:A,[u+"Team"]:f[u+"Team"],["usedSubIds_"+p]:ut})})})}document.body.appendChild(ee),J()}function re(w,L,A){const C={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const P=(U,X,te)=>`<div style="text-align:center">
      <div style="font-size:9px;color:${X};letter-spacing:2px;text-transform:uppercase;font-weight:700;margin-bottom:6px">${te}</div>
      <div style="width:70px;height:70px;border-radius:50%;background:${C[U.job]||"#555"};border:3px solid ${X};position:relative;overflow:hidden;margin:0 auto">
      ...
      </div>`,document.body.appendChild(L),(A=L.querySelector("#pvp-hist-close"))==null||A.addEventListener("click",()=>L.remove())}async function ue(){if(f.phase!==p+"-attack")return;const w=p==="p1"?"p2":"p1",L=(f.round||0)+1,A=[...f.log||[]];A.push({type:"info",text:`⏭️ ${f[p+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const C=$e(f),T=ct(w),P=C||!T?"finished":w+"-attack";await j({["selected_"+p]:[],modifiers:{...f.modifiers,[p]:{}},pendingAttack:null,phase:P,attacker:w,round:L,log:A}),P==="finished"&&await qe(f)}function le(w){const L=(w.GK||[]).some(C=>!C.used),A=["DEF","MIL","ATT"].some(C=>(w[C]||[]).some(T=>!T.used));return L&&!A}function he(w){return(w.p1Score||0)!==(w.p2Score||0)?null:le(w.p1Team)&&!ke(w.p2Team)?{side:"p1",gk:(w.p1Team.GK||[]).find(L=>!L.used)}:le(w.p2Team)&&!ke(w.p1Team)?{side:"p2",gk:(w.p2Team.GK||[]).find(L=>!L.used)}:null}async function Se(){const w=document.createElement("div");w.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:950;display:flex;align-items:center;justify-content:center;overflow:hidden;text-align:center;padding:24px",document.body.appendChild(w);const L=(A,C)=>new Promise(T=>{w.innerHTML=`<div style="font-size:32px;font-weight:900;color:${C};letter-spacing:2px;animation:lcFade 1.4s ease both">${A}</div>
      <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(T,1400)});await L("⚽ DERNIER CORNER","#FFD700"),await L("🧤 LE GARDIEN MONTE !","#4fc3f7"),w.remove()}async function ve(w,L){const{side:A,gk:C}=w;if(!C)return;const T=A+"Score",P=(L[T]||0)+1;C.used=!0;const D=[...L.log||[]];D.push({type:"duel",isGoal:!0,homeScored:A===p,homePlayers:A==="p1"?[qt(C)]:[],aiPlayers:A==="p2"?[qt(C)]:[],text:`⚽ DERNIER CORNER — Le gardien de ${L[A+"Name"]} marque !`});const F=(L.round||0)+1,U=A==="p1"?"p2":"p1",X={...L,[T]:P},te=$e(X);q.add(F),await Se();const ee=A===p?P:L[p+"Score"]||0,J=A===p?L[u+"Score"]||0:P;Qe([qt(C)],ee,J,A===p,async()=>{await j({[T]:P,log:D,round:F,pendingAttack:null,phase:te?"finished":U+"-attack",attacker:U,["selected_"+p]:[],modifiers:{...f.modifiers,[p]:{}}}),te&&await qe({...f,[T]:P})})}async function Te(){const w=f[p+"Team"],L=!["GK","DEF","MIL","ATT"].some(P=>(f[u+"Team"][P]||[]).some(D=>!D.used)),A=(f["selected_"+p]||[]).map(P=>{const D=(w[P._role]||[]).find(se=>se.cardId===P.cardId)||P,F=["GK","DEF"].includes(P._role),U=w[P._role]||[],X=U.findIndex(se=>se.cardId===P.cardId),te=yt(U.length),ee=X>=0?te[X]:D._col??1,J=f.stadiumDef||f.homeStadiumDef||null,ae=D.stadiumBonus||J&&(J.club_id&&String(D.club_id)===String(J.club_id)||J.country_code&&D.country_code===J.country_code)||!1;return{...D,_line:P._role,_col:ee,stadiumBonus:ae,...F?{note_a:Math.max(1,Number(D.note_a)||0)}:{}}});if(!A.length)return;const C=oi(A,f.modifiers[p]||{},f[p+"Formation"]);fe(p,A.map(P=>P.cardId)),A.forEach(P=>{const D=(w[P._role]||[]).find(F=>F.cardId===P.cardId);D&&(D.used=!0)}),f["selected_"+p]=[],Y();const T=[...f.log||[]];if(L){if(A.length===1&&(A[0]._line==="GK"||A[0].job==="GK")&&(f.p1Score||0)===(f.p2Score||0)){await ve({side:p,gk:A[0]},{...f,[p+"Team"]:w,log:T});return}const D=(f[p+"Score"]||0)+1,F=A.map(J=>qt(J));T.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:F,homeTotal:C.total,aiTotal:0});const U=(f.round||0)+1,X=p==="p1"?"p2":"p1",te={...f,[p+"Team"]:w,[p+"Score"]:D},ee=$e(te);q.add(U),Qe(F,D,f[u+"Score"]||0,!0,async()=>{await j({[p+"Team"]:w,[p+"Score"]:D,["selected_"+p]:[],modifiers:{...f.modifiers,[p]:{}},pendingAttack:null,phase:ee?"finished":X+"-attack",attacker:X,round:U,log:T}),ee&&await qe({...f,[p+"Score"]:D})});return}T.push({type:"pending",text:`⚔️ ${f[p+"Name"]} attaque (${C.total})`}),await j({[p+"Team"]:w,[u+"Team"]:f[u+"Team"],pendingAttack:{...C,players:A,side:p},["selected_"+p]:[],modifiers:{...f.modifiers,[p]:{}},phase:u+"-defense",log:T})}async function Ae(){const w=f[p+"Team"],L=(f["selected_"+p]||[]).map(de=>{const ye=(w[de._role]||[]).find(Le=>Le.cardId===de.cardId)||de,be=w[de._role]||[],oe=be.findIndex(Le=>Le.cardId===de.cardId),Ee=yt(be.length),Ne=oe>=0?Ee[oe]:ye._col??1;return{...ye,_line:de._role,_col:Ne}}),A=ri(L,f.modifiers[p]||{},f[p+"Formation"]);fe(p,L.map(de=>de.cardId)),L.forEach(de=>{const ye=(w[de._role]||[]).find(be=>be.cardId===de.cardId);ye&&(ye.used=!0)}),f["selected_"+p]=[],Y();const C=Xi(f.pendingAttack.total,A.total,f.modifiers[p]||{}),T=f.pendingAttack.side,P=C==="attack"||(C==null?void 0:C.goal),D=T==="p1"?"p2":"p1",F=(f.round||0)+1,U=(f.pendingAttack.players||[]).map(de=>qt(de)),X=[...f.log||[]];X.push({type:"duel",isGoal:P,homeScored:P&&T===p,text:P?`⚽ BUT de ${f[T+"Name"]} ! (${f.pendingAttack.total} vs ${A.total})`:`✋ Attaque stoppée (${f.pendingAttack.total} vs ${A.total})`,homePlayers:U,aiPlayers:L.map(de=>qt(de)),homeTotal:f.pendingAttack.total,aiTotal:A.total});const te=P?(f[T+"Score"]||0)+1:f[T+"Score"]||0,ee={...f,[p+"Team"]:w,[T+"Score"]:te},J=$e(ee),ae=J?"finished":D+"-attack",se=!J&&!P?he(ee):null,ge=async()=>{if(se){await ve(se,ee);return}await j({[p+"Team"]:w,[u+"Team"]:f[u+"Team"],[T+"Score"]:te,["selected_"+p]:[],modifiers:{...f.modifiers,[p]:{}},pendingAttack:null,phase:ae,attacker:D,round:F,log:X}),(ae==="finished"||J)&&await qe({...f,[T+"Score"]:te})};if(P){const de=T===p,ye=de?te:f[p+"Score"]||0,be=de?f[u+"Score"]||0:te;q.add(F),Qe(U,ye,be,de,ge)}else await ge()}function Be(w){return["MIL","ATT"].some(L=>(w[L]||[]).some(A=>!A.used))}function ke(w){return["GK","DEF","MIL","ATT"].some(L=>(w[L]||[]).some(A=>!A.used))}function wt(w){return ke(w)&&!Be(w)}function ct(w){const L=f[w+"Team"],A=f[(w==="p1"?"p2":"p1")+"Team"];return!!(Be(L)||wt(L)&&!ke(A)||wt(L)&&!Be(A)&&ke(A))}function $e(w){return!ke(w.p1Team)&&!ke(w.p2Team)}function Ce(w){const L=w.p1Score||0,A=w.p2Score||0;return L===A?null:L>A?v.home_id:v.away_id}async function qe(w){Gt(),Ue();try{const L=Ce(w),A=L?v.home_id===L?v.away_id:v.home_id:null,C=w.p1Score||0,T=w.p2Score||0,{data:P,error:D}=await _.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:L,p_home_score:C,p_away_score:T,p_forfeit:!1});if(D&&console.error("[PvP] finish_pvp_match:",D),l)try{await _.rpc("check_and_finish_mini_league",{p_league_id:l})}catch(F){console.error("[PvP] check_and_finish_mini_league:",F)}L&&A&&Mo(L,A).catch(()=>{});try{const{data:F}=await _.rpc("apply_match_rewards",{p_match_id:i});F!=null&&F.success&&!(F!=null&&F.skipped)&&typeof t.refreshProfile=="function"&&await t.refreshProfile()}catch(F){console.error("[PvP] apply_match_rewards:",F)}if(typeof c=="function")try{await c({homeScore:C,awayScore:T,winnerId:L,homeId:v.home_id,awayId:v.away_id,matchId:i})}catch(F){console.error("[PvP] onMatchEnd:",F)}}catch(L){console.error("[PvP] finishMatch:",L)}}function Ge(){var T;const w=f[p+"Score"],L=f[u+"Score"],A=w>L,C=w===L;st(e),e.innerHTML=`
    ...
    </div>`,(T=document.getElementById("pvp-home"))==null||T.addEventListener("click",()=>{try{_.removeChannel(Z)}catch{}it(e),l?y("mini-league",{openLeagueId:l}):y("home")})}Y()}async function Pi(e,t,i=!1){var s,d;const{state:n,navigate:r,toast:o}=t,a=i&&((d=(s=t==null?void 0:t.state)==null?void 0:s.params)==null?void 0:d.rankedData)||null;await _i(e,t,i?"ranked":"random",async({deckId:c,formation:l,starters:m,subsRaw:b,gcCardsEnriched:y,gcDefs:g,stadiumDef:p})=>{st(e);const u=async x=>{await zr(e,t,c,l,m,b,x||[],g,p,i,a)};if(!(y!=null&&y.length)){await u([]);return}vi(e,y,u)})}async function Ar(e,t,i){return Sr(e,t,i)}async function zr(e,t,i,n,r,o,a=[],s=[],d=null,c=!1,l=null){var ne,Z;const{state:m,navigate:b,toast:y}=t,g=(l==null?void 0:l.mmr)??((ne=m.profile)==null?void 0:ne.mmr)??1e3,p=c?Hi(g):null,u=c?"linear-gradient(135deg, #1a0a2e 0%, #2d1060 50%, #1a0a2e 100%)":"linear-gradient(135deg, #0a1a2e 0%, #0d3d1e 100%)",x=c?(p==null?void 0:p.color)||"#D4A017":"#FFD700";let h=!1,v=null,E=null;e.innerHTML=`
    ...
    </div>`;const f=async()=>{h=!0,v&&(v.unsubscribe(),v=null),E&&(clearInterval(E),E=null);try{await _.rpc("cancel_matchmaking",{p_user_id:m.profile.id})}catch(j){console.error("[Matchmaking] cancel error:",j)}};(Z=document.getElementById("cancel-mm"))==null||Z.addEventListener("click",async()=>{await f(),it(e),b("home")});const z=async(j,I)=>{h||(h=!0,v&&(v.unsubscribe(),v=null),E&&(clearInterval(E),E=null),await Mt(e,t,j,I,{myGC:a,gcDefs:s,stadiumDef:d,isRanked:c,rankedData:l,onMatchEnd:c?Mr:null}))},k=m.user.id,{data:S,error:q}=c?await _.rpc("try_matchmake_ranked",{p_user_id:k,p_deck_id:i,p_mmr:g}):await _.rpc("try_matchmake",{p_user_id:k,p_deck_id:i});if(q||!(S!=null&&S.success)){console.error("[Matchmaking] try_matchmake error:",q||S),y("Erreur de matchmaking","error"),it(e),b("home");return}if(S.matched){await z(S.match_id,!1);return}const ie=async()=>{if(h)return;const{data:j}=await _.from("matchmaking_queue").select("status, match_id").eq("user_id",k).maybeSingle();(j==null?void 0:j.status)==="matched"&&j.match_id&&await z(j.match_id,!0)};v=_.channel(`mm_${k}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${k}`},j=>{const I=j.new;I.status==="matched"&&I.match_id&&z(I.match_id,!0)}).subscribe(),E=setInterval(ie,3e3),setTimeout(()=>{h||f()},12e4)}async function Mr(e,t){const{state:i,toast:n}=t,{winnerId:r,homeId:o,awayId:a,homeScore:s,awayScore:d,matchId:c}=e,l=i.user.id,m=o===l,b=r===l,{data:y}=await _.from("users").select("mmr, mmr_rd, mmr_v").eq("id",l).single(),{data:g}=await _.from("users").select("mmr, mmr_rd, mmr_v").eq("id",m?a:o).single();if(y&&g){const p=ao(y.mmr,y.mmr_rd,y.mmr_v,g.mmr,g.mmr_rd,b?1:0);await _.from("users").update({mmr:p.mmr,mmr_rd:p.rd,mmr_v:p.v,mmr_wins:b?_.sql`mmr_wins + 1`:void 0,mmr_losses:b?void 0:_.sql`mmr_losses + 1`}).eq("id",l),n(b?`+MMR ↑ ${Math.round(p.mmr-y.mmr)}`:`-MMR ↓ ${Math.round(y.mmr-p.mmr)}`,b?"success":"error",4e3)}}const Cr=Object.freeze(Object.defineProperty({__proto__:null,renderMatchRandom:Pi,resumePvpMatch:Ar},Symbol.toStringTag,{value:"Module"}));async function jr(e,t){const{state:i,navigate:n,toast:r}=t,o=i.params||{},a=o.friendId||null,s=o.friendName||"Ami",d=!!o.isAccepting;await _i(e,t,"friend",async({deckId:c,formation:l,starters:m,subsRaw:b,gcCardsEnriched:y,gcDefs:g,stadiumDef:p})=>{st(e);const u=async x=>{const h=x||[];d?await qr(e,t,c,h,g,p):await Br(e,t,c,h,g,p,a,s)};if(!(y!=null&&y.length)){await u([]);return}vi(e,y,u)})}async function Br(e,t,i,n,r,o,a,s){var h;const{state:d,navigate:c,toast:l}=t,m=d.user.id,{data:b,error:y}=await _.from("friend_match_invites").insert({inviter_id:m,invitee_id:a,friend_id:a,status:"pending",inviter_deck_id:i,expires_at:new Date(Date.now()+2*60*1e3).toISOString()}).select("id").single();if(y||!b){console.error("[Friend] Erreur création invitation:",y),l("Impossible de créer l'invitation","error"),c("home");return}e.innerHTML=`
    ...
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:${Jt};border:1px solid ${Qt};border-radius:12px;padding:16px">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>${b.map((j,I)=>{const K=g>0&&a.status==="finished"?I===0?Math.floor(g*.7):I===1?Math.floor(g*.2):I===2?Math.floor(g*.1):0:0;return`<tr style="border-bottom:1px solid ${Lt};${j.userId===o?"background:rgba(74,222,128,0.08);":""}">
      ...
      <div style="background:${Jt};border:1px solid ${Qt};border-radius:12px;padding:16px">
        ...
        ${Array.from({length:Math.max(0,a.status==="active"?a.current_day-1:a.current_day)},(j,I)=>I+1).reverse().map(j=>{const I=(d||[]).filter(K=>K.matchday===j);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:${Ze};margin-bottom:6px">Journée ${j}</div>${I.map(K=>kn(K,m,o,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
  ...
  </div>`;let d="buy";const c=()=>{var p,u,x,h,v,E,f;return{name:(((p=document.getElementById("flt-name"))==null?void 0:p.value)||"").toLowerCase().trim(),club:(((u=document.getElementById("flt-club"))==null?void 0:u.value)||"").toLowerCase().trim(),country:(((x=document.getElementById("flt-country"))==null?void 0:x.value)||"").toLowerCase().trim(),job:((h=document.getElementById("flt-job"))==null?void 0:h.value)||"",rarity:((v=document.getElementById("flt-rarity"))==null?void 0:v.value)||"",note1:parseInt((E=document.getElementById("flt-note1"))==null?void 0:E.value)||0,note2:parseInt((f=document.getElementById("flt-note2"))==null?void 0:f.value)||0}};function l(p){const u=c();return p.filter(x=>{var q,ie,ne;const h=(q=x.card)==null?void 0:q.player;if(!h)return!1;const v=`${h.firstname} ${h.surname_real}`.toLowerCase(),E=(((ie=h.clubs)==null?void 0:ie.encoded_name)||"").toLowerCase(),f=(h.country_code||"").toLowerCase(),z=((ne=x.card)==null?void 0:ne.evolution_bonus)||0,k=Dt(h,h.job,z),S=h.job2?Dt(h,h.job2,z):0;return!(u.name&&!v.includes(u.name)||u.club&&!E.includes(u.club)||u.country&&!f.includes(u.country)||u.job&&h.job!==u.job||u.rarity&&h.rarity!==u.rarity||u.note1&&k<u.note1||u.note2&&S<u.note2)})}function m(p){var S,q,ie,ne;const u=(S=p.card)==null?void 0:S.player;if(!u)return"";const x=((q=p.card)==null?void 0:q.evolution_bonus)||0,h=Dt(u,u.job,x),v=u.job2?Dt(u,u.job2,x):0,E=(i.profile.credits||0)>=p.price,f=u.country_code?`https://flagsapi.com/${u.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,z=pi[u.job]||"#bbb",k=u.job2?pi[u.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden">
      <!-- Drapeau -->
    ...
    </div>`}function b(p){var S,q,ie,ne;const u=(S=p.card)==null?void 0:S.player;if(!u)return"";const x=((q=p.card)==null?void 0:q.evolution_bonus)||0,h=Dt(u,u.job,x),v=u.job2?Dt(u,u.job2,x):0,E=p.status==="sold",f=u.country_code?`https://flagsapi.com/${u.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,z=pi[u.job]||"#bbb",k=u.job2?pi[u.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${E?"opacity:0.7":""}">
      ${f?`<img src="${f}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      ${(ie=u.clubs)!=null&&ie.logo_url?`<img src="${u.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${z};display:flex;align-items:center;justify-content:center">
        ...
        <button id="buy-ok" style="flex:1;padding:12px;border-radius:10px;border:none;background:var(--green);color:#fff;font-size:14px;font-weight:900;cursor:pointer">Confirmer</button>
          ...
          <div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${j?"border:2px solid var(--yellow)":""}">
            ...
            <div style="flex:1;min-width:0">
              ...
              <div style="font-size:11px;color:var(--tile-fg-dim)">${S.club_name} · ${ie.label}</div>
            </div>
            <div style="text-align:right;flex-shrink:0">
  ...
  <div class="page">
    ...
    </div>`,(z=document.getElementById("ranked-back"))==null||z.addEventListener("click",()=>n("home"));return}const d=s.mmr??1e3,c=s.mmr_deviation??350,l=s.mmr_volatility??.06,m=s.placement_matches??0,b=m<10,y=Math.max(0,10-m),g=Hi(d),p=so(d),u=Ei.findIndex(ie=>ie.id===g.id),x=Ei[u+1]||null,h={bronze:"linear-gradient(160deg,#3d1c00,#7a3e00)",silver:"linear-gradient(160deg,#1a1a2e,#3a3a5e)",gold:"linear-gradient(160deg,#1a1200,#4a3500)",platinum:"linear-gradient(160deg,#001a20,#003040)",diamond:"linear-gradient(160deg,#001030,#1a2860)",master:"linear-gradient(160deg,#1a0030,#3d0070)"},v=(s.ranked_wins||0)+(s.ranked_losses||0)+(s.ranked_draws||0),E=v>0?Math.round((s.ranked_wins||0)/v*100):0,f=Ei.map(ie=>`
    <div style="display:flex;flex-direction:column;align-items:center;gap:2px;opacity:${g.id===ie.id?1:.35};
      transform:${g.id===ie.id?"scale(1.15)":"scale(1)"};transition:all 0.3s">
      <div style="font-size:${g.id===ie.id?"28px":"20px"}">${ie.emoji}</div>
      <div style="font-size:9px;color:${ie.color};font-weight:${g.id===ie.id?"900":"400"};letter-spacing:0.5px">${ie.label.toUpperCase()}</div>
    </div>
  `).join("");e.innerHTML=`
  ...
  <div class="page">
    ...
    <nav class="top-nav">
      <div class="logo" id="nav-logo" title="Manager Wars v2026.07.22-1533" style="cursor:pointer">
