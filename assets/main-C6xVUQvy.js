import{s as I,F as Nt,h as yt,j as yi,l as Je,i as Oi,k as Pi,b as Hi}from"./formation-links-CEzN0rcz.js";function ai(e,{navigate:t,toast:i}){e.innerHTML=`
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;height:${u}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ...
      ${c?`<img src="${c}" style="width:${u-4}px;height:${u-4}px;object-fit:contain">`:e!=null&&e.clubName?`<span style="font-size:${Math.max(4,u-8)}px;font-weight:700;color:#333">${(e.clubName||"").slice(0,3).toUpperCase()}</span>`:""}
  ...
  </div>`}function et(e,t,i){if(!(e!=null&&e.length))return"";const n=e.slice(0,5);let c='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return n.forEach((s,a)=>{if(c+=Be(s,40,52),a<n.length-1){const l=Je(s,n[a+1]);c+=`<div style="width:7px;height:3px;background:${l==="#ff3333"||l==="#cc2222"?"rgba(255,255,255,0.12)":l};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(c+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),c+="</div>",c}function Dt(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function Bi(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function mt(e,t,i,n,c=310,s=310,a=[]){var M;const l=yt[t]||{},p=yi(t)||Nt[t]||[],o={},r=["ATT","MIL","DEF","GK"];for(const G of r)(e[G]||[]).forEach((v,F)=>{o[`${G}${F+1}`]=v});function u(G){const k=l[G];return k?{x:k.x*c,y:k.y*s}:null}let f="";for(const[G,k]of p){const v=u(G),F=u(k);if(!v||!F)continue;const Z=o[G],ne=o[k],ee=Je(Z,ne);ee==="#00ff88"||ee==="#FFD700"?(f+=`<line x1="${v.x.toFixed(1)}" y1="${v.y.toFixed(1)}" x2="${F.x.toFixed(1)}" y2="${F.y.toFixed(1)}"
        ...
        stroke="${ee}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const g=48,y=64,d=13,x=16,$={normal:"#aaaaaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#111111",legende:"#7a28b8",légende:"#7a28b8"};for(const[G,k]of Object.entries(o)){const v=u(G);if(!v||!k)continue;const F=G.replace(/[0-9]/g,""),Z=rt[F]||"#555",ne=a.includes(k.cardId),ee=i==="attack"&&(["MIL","ATT"].includes(F)||ne)&&!k.used||i==="defense"&&["GK","DEF","MIL"].includes(F)&&!k.used,w=n.includes(k.cardId);let q;i==="attack"?q=ne?Math.max(1,Number(k.note_a)||0):F==="MIL"?Number(k.note_m)||0:Number(k.note_a)||0:i==="defense"?q=F==="GK"?Number(k.note_g)||0:F==="MIL"?Number(k.note_m)||0:Number(k.note_d)||0:q=Number(F==="GK"?k.note_g:F==="DEF"?k.note_d:F==="MIL"?k.note_m:k.note_a)||0,q=q+(k.boost||0);const V=(v.x-g/2).toFixed(1),h=(v.y-y/2).toFixed(1),U=$[k==null?void 0:k.rarity]||$.normal;if(k.used){const S=typeof import.meta<"u"&&"/manager-wars/"||"/",C=`${typeof window<"u"&&((M=window.location)==null?void 0:M.origin)||""}${S}icons/carte-dos.png`.replace(/([^:])\/\//g,"$1/");f+=`<rect x="${V}" y="${h}" width="${g}" height="${y}" rx="5" fill="#161616"/>`,f+=`<image href="${C}" xlink:href="${C}" x="${V}" y="${h}" width="${g}" height="${y}" preserveAspectRatio="xMidYMid slice"/>`,f+=`<rect x="${V}" y="${h}" width="${g}" height="${y}" rx="5" fill="none" stroke="${U}" stroke-width="2" opacity="0.7"/>`,f+=`<rect x="${V}" y="${h}" width="${g}" height="${y}" rx="5" fill="rgba(0,0,0,0.01)" class="match-used-hit" data-card-id="${k.cardId}" data-role="${F}" style="cursor:pointer"/>`;continue}const W=w?.45:1,re=w?"#FFD700":U,ce=w?3:(k==null?void 0:k.rarity)==="legende"||(k==null?void 0:k.rarity)==="légende"||(k==null?void 0:k.rarity)==="pepite"||(k==null?void 0:k.rarity)==="pépite"?2.5:2,se=y-d-x;f+=`<defs><clipPath id="cp-${G}"><rect x="${V}" y="${(v.y-y/2+d).toFixed(1)}" width="${g}" height="${se}"/></clipPath></defs>`,f+=`<rect x="${V}" y="${h}" width="${g}" height="${y}" rx="5" fill="${Z}" opacity="${W}"/>`;const ae=Te(k);ae&&(f+=`<image href="${ae}" xlink:href="${ae}" x="${V}" y="${(v.y-y/2+d).toFixed(1)}" width="${g}" height="${se}" clip-path="url(#cp-${G})" preserveAspectRatio="xMidYMin slice"/>`),f+=`<rect x="${V}" y="${h}" width="${g}" height="${d}" rx="4" fill="rgba(255,255,255,0.92)"/>`,f+=`<text x="${v.x.toFixed(1)}" y="${(v.y-y/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(k.name||"").slice(0,9)}</text>`;const z=(v.y+y/2-x).toFixed(1);f+=`<rect x="${V}" y="${z}" width="${g}" height="${x}" fill="rgba(255,255,255,0.92)"/>`;{const S=Dt(k.country_code);S?f+=`<image href="${S}" xlink:href="${S}" x="${(v.x-20).toFixed(1)}" y="${(v.y+y/2-x+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:f+=`<text x="${(v.x-13).toFixed(1)}" y="${(v.y+y/2-x/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${Bi(k.country_code)}</text>`,f+=`<text x="${v.x.toFixed(1)}" y="${(v.y+y/2-x/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${q}</text>`;const R=ji(k);R?f+=`<image href="${R}" xlink:href="${R}" x="${(v.x+g/2-14).toFixed(1)}" y="${(v.y+y/2-x+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:k.clubName&&(f+=`<text x="${(v.x+14).toFixed(1)}" y="${(v.y+y/2-x/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(k.clubName||"").slice(0,3).toUpperCase()}</text>`),k.boost&&(f+=`<rect x="${(v.x+g/2-12).toFixed(1)}" y="${(v.y-y/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,f+=`<text x="${(v.x+g/2-5).toFixed(1)}" y="${(v.y-y/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${k.boost}</text>`)}f+=`<rect x="${V}" y="${h}" width="${g}" height="${y}" rx="5" fill="${w?"rgba(255,255,255,0.12)":"none"}" stroke="${re}" stroke-width="${ce}" opacity="${W}"/>`,ee&&(f+=`<rect x="${V}" y="${h}" width="${g}" height="${y}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${w?"selected":""}" data-card-id="${k.cardId}" data-role="${F}" style="cursor:pointer"/>`)}const _=38;return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-_} ${-_} ${c+_*2} ${s+_*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${f}
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${d.map((M,G)=>{const k=G<d.length-1?Je(M,d[G+1]):null,v=!k||k==="#ff3333"||k==="#cc2222",F=k==="#00ff88"?"+2":k==="#FFD700"?"+1":"";return`
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(s);let l=!1;const p=()=>{l||(l=!0,s.remove(),setTimeout(()=>c(),50))};s.addEventListener("click",p),setTimeout(p,3500)}async function ft(e,t,i){var r,u;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:n}=i,c=t.homeScore>t.aiScore,s=t.homeScore===t.aiScore,a=c?"victoire":s?"nul":"defaite",l=Wi(t.mode,a);t.matchId&&await I.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:c?n.profile.id:null,home_credits_reward:l,played_at:new Date().toISOString()}).eq("id",t.matchId);const p={credits:(n.profile.credits||0)+l,matches_played:(n.profile.matches_played||0)+1};c?p.wins=(n.profile.wins||0)+1:s?p.draws=(n.profile.draws||0)+1:p.losses=(n.profile.losses||0)+1,await I.from("users").update(p).eq("id",n.profile.id),await i.refreshProfile();const o=document.createElement("div");o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",o.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const g=async(M=!0)=>{u=!0,f&&(I.removeChannel(f),f=null),M&&await I.rpc("cancel_matchmaking",{p_user_id:p.profile.id}).catch(()=>{})};(_=document.getElementById("mm-cancel"))==null||_.addEventListener("click",async()=>{try{await g(!0)}catch{}Re(e),o("home")});const y=async(M,G,k)=>{if(u)return;u=!0,f&&(I.removeChannel(f),f=null);const v=document.getElementById("mm-status");v&&(v.textContent="Adversaire trouvé !"),await new Promise(F=>setTimeout(F,600)),e.isConnected&&Uo(e,t,M,k,a,l)},{data:d,error:x}=await I.rpc("try_matchmake",{p_user_id:p.profile.id,p_deck_id:i});if(x||!(d!=null&&d.success)){r("Erreur de matchmaking","error"),Re(e),o("home");return}if(d.matched){y(d.match_id,d.opponent_id,!1);return}const $=document.getElementById("mm-status");$&&($.textContent="En attente d'un autre joueur..."),f=I.channel("matchmaking-"+p.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${p.profile.id}`},M=>{const G=M.new;G.status==="matched"&&G.match_id&&y(G.match_id,G.matched_with,!0)}).subscribe()}async function Uo(e,t,i,n,c=[],s=[]){const{state:a,navigate:l,toast:p}=t,o=n?"p1":"p2",r=n?"p2":"p1",u=(c||[]).map(m=>m.id),f=(c||[]).map(m=>({id:m.id,gc_type:m.gc_type,_gcDef:m._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:g}=await I.from("matches").select("*").eq("id",i).single();if(!g){p("Match introuvable","error"),l("home");return}async function y(){const[{data:m},{data:b},{data:T},{data:A}]=await Promise.all([I.rpc("get_deck_for_match",{p_deck_id:g.home_deck_id}),I.rpc("get_deck_for_match",{p_deck_id:g.away_deck_id}),I.from("users").select("id,pseudo,club_name").eq("id",g.home_id).single(),I.from("users").select("id,pseudo,club_name").eq("id",g.away_id).single()]),E=B=>({cardId:B.card_id,position:B.position,id:B.id,firstname:B.firstname,name:B.surname_encoded,country_code:B.country_code,club_id:B.club_id,job:B.job,job2:B.job2,note_g:Number(B.note_g)||0,note_d:Number(B.note_d)||0,note_m:Number(B.note_m)||0,note_a:Number(B.note_a)||0,rarity:B.rarity,skin:B.skin,hair:B.hair,hair_length:B.hair_length,clubName:B.club_encoded_name||null,clubLogo:B.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}),L=((m==null?void 0:m.starters)||[]).map(B=>E(B)),j=((b==null?void 0:b.starters)||[]).map(B=>E(B)),D=(m==null?void 0:m.formation)||"4-4-2",P=(b==null?void 0:b.formation)||"4-4-2";return{p1Team:kt(L,D),p2Team:kt(j,P),p1Subs:((m==null?void 0:m.subs)||[]).map(B=>E(B)),p2Subs:((b==null?void 0:b.subs)||[]).map(B=>E(B)),p1Formation:D,p2Formation:P,p1Name:(T==null?void 0:T.club_name)||(T==null?void 0:T.pseudo)||"Joueur 1",p2Name:(A==null?void 0:A.club_name)||(A==null?void 0:A.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:n?u:[],gc_p2:n?[]:u,gcCardsFull_p1:n?f:[],gcCardsFull_p2:n?[]:f,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:s||[],lastActionAt:new Date().toISOString()}}let d=g.game_state&&Object.keys(g.game_state).length?g.game_state:null;if(!d)if(n){d=await y();const{data:m}=await I.from("matches").select("game_state").eq("id",i).single();!(m!=null&&m.game_state)||!Object.keys(m.game_state).length?await I.from("matches").update({game_state:d,turn_user_id:g.home_id}).eq("id",i):d=m.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let m=0;m<30&&!d;m++){await new Promise(T=>setTimeout(T,400));const{data:b}=await I.from("matches").select("game_state").eq("id",i).single();b!=null&&b.game_state&&Object.keys(b.game_state).length&&(d=b.game_state)}if(!d){p("Erreur de synchronisation","error"),l("home");return}d.gc_p2=u,d.gcCardsFull_p2=f,await I.from("matches").update({game_state:d}).eq("id",i)}let x=!1,$=null,_=!1;const M=new Set,G=new Set;function k(m){var B,H;try{I.removeChannel(v)}catch{}const b=d[o+"Score"]||0,T=d[r+"Score"]||0;let A,E;m.winner_id?(A=m.winner_id===a.profile.id,E=!1):m.forfeit?(A=b>T,E=!1):(E=b===T,A=b>T),(B=document.getElementById("pvp-end-overlay"))==null||B.remove();const L=document.createElement("div");L.id="pvp-end-overlay",L.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const j=E?"🤝":A?"🏆":"😞",D=E?"MATCH NUL":A?"VICTOIRE !":"DÉFAITE",P=E?"#fff":A?"#FFD700":"#ff6b6b";L.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(L),(H=L.querySelector("#pvp-end-home"))==null||H.addEventListener("click",()=>{L.remove(),Re(e),l("home")})}const v=I.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},m=>{const b=m.new;try{if(b.status==="finished"||b.forfeit){if(x)return;x=!0,$&&(clearInterval($),$=null),b.game_state&&(d=b.game_state),k(b);return}if(b.game_state){const T=d;d=b.game_state;const A=d._lastGC;if(A&&A.seq&&!G.has(A.seq)&&(G.add(A.seq),A.by!==o)){se(A.type,A.by,()=>h());return}const E=T[o+"Score"]||0,L=T[r+"Score"]||0,j=d[o+"Score"]||0,D=d[r+"Score"]||0,P=j>E,B=D>L;if((P||B)&&!M.has(d.round)){M.add(d.round);const H=[...d.log||[]].reverse().find(Q=>Q.isGoal),K=((H==null?void 0:H.homePlayers)||[]).map(Q=>({name:Q.name,note:Q.note,portrait:Q.portrait,job:Q.job}));ce(K,j,D,P,()=>h());return}h()}}catch(T){console.error("[PvP] crash:",T)}}).subscribe();async function F(m){Object.assign(d,m),d.lastActionAt=new Date().toISOString();const{error:b}=await I.from("matches").update({game_state:d}).eq("id",i);b&&p("Erreur de synchronisation","error");try{h()}catch(T){console.error("[PvP] renderPvpScreen crash:",T)}}async function Z(){if(x)return;x=!0,$&&(clearInterval($),$=null);const m=n?g.away_id:g.home_id,b=n?"p2":"p1",T=n?"p1":"p2",A={...d,[b+"Score"]:3,[T+"Score"]:0,phase:"finished"};try{await I.from("matches").update({status:"finished",forfeit:!0,winner_id:m,home_score:A.p1Score||0,away_score:A.p2Score||0,game_state:A}).eq("id",i)}catch{}try{I.removeChannel(v)}catch{}setTimeout(()=>{Re(e),l("home")},800)}const ne={BOOST_STAT:({value:m=1,count:b=1,roles:T=[]},A,E)=>{const L=A[o+"Team"],j=Object.entries(L).filter(([D])=>!T.length||T.includes(D)).flatMap(([D,P])=>P.filter(B=>!B.used).map(B=>({...B,_line:D})));if(!j.length){A.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),E(A);return}ee(j,b,`Choisir ${b} joueur(s) à booster (+${m})`,D=>{D.forEach(P=>{const B=(L[P._line]||[]).find(H=>H.cardId===P.cardId);B&&(B.boost=(B.boost||0)+m,A.log.push({text:`⚡ +${m} sur ${B.name}`,type:"info"}))}),A[o+"Team"]=L,E(A)})},DEBUFF_STAT:({value:m=1,count:b=1,roles:T=[],target:A="ai"},E,L)=>{const j=A==="home"?o:r,D=E[j+"Team"],P=A==="home"?"allié":"adverse",B=Object.entries(D).filter(([H])=>!T.length||T.includes(H)).flatMap(([H,K])=>K.filter(Q=>!Q.used).map(Q=>({...Q,_line:H})));if(!B.length){E.log.push({text:`🎯 Aucun joueur ${P}`,type:"info"}),L(E);return}ee(B,b,`Choisir ${b} joueur(s) ${P}(s) (-${m})`,H=>{H.forEach(K=>{const Q=(D[K._line]||[]).find(ue=>ue.cardId===K.cardId);Q&&(Q.boost=(Q.boost||0)-m,E.log.push({text:`🎯 -${m} sur ${Q.name}`,type:"info"}))}),E[j+"Team"]=D,L(E)})},GRAY_PLAYER:({count:m=1,roles:b=[],target:T="ai"},A,E)=>{const L=T==="home"?o:r,j=A[L+"Team"],D=T==="home"?"allié":"adverse",P=Object.entries(j).filter(([B])=>!b.length||b.includes(B)).flatMap(([B,H])=>H.filter(K=>!K.used).map(K=>({...K,_line:B})));if(!P.length){A.log.push({text:`❌ Aucun joueur ${D}`,type:"info"}),E(A);return}ee(P,m,`Choisir ${m} joueur(s) ${D}(s) à exclure`,B=>{B.forEach(H=>{const K=(j[H._line]||[]).find(Q=>Q.cardId===H.cardId);K&&(K.used=!0,A.log.push({text:`❌ ${K.name} exclu !`,type:"info"}))}),A[L+"Team"]=j,E(A)})},REVIVE_PLAYER:({count:m=1,roles:b=[]},T,A)=>{const E=T[o+"Team"],L=Object.entries(E).filter(([j])=>!b.length||b.includes(j)).flatMap(([j,D])=>D.filter(P=>P.used).map(P=>({...P,_line:j})));if(!L.length){T.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),A(T);return}ee(L,m,`Choisir ${m} joueur(s) à ressusciter`,j=>{j.forEach(D=>{const P=(E[D._line]||[]).find(B=>B.cardId===D.cardId);P&&(P.used=!1,T.log.push({text:`💫 ${P.name} ressuscité !`,type:"info"}))}),T[o+"Team"]=E,A(T)})},REMOVE_GOAL:({},m,b)=>{const T=r+"Score";m[T]>0?(m[T]--,m.log.push({text:"🚫 Dernier but annulé !",type:"info"})):m.log.push({text:"🚫 Aucun but à annuler",type:"info"}),b(m)},ADD_GOAL_DRAW:({},m,b)=>{m[o+"Score"]===m[r+"Score"]?(m[o+"Score"]++,m.log.push({text:"🎯 But bonus !",type:"info"})):m.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),b(m)},ADD_SUB:({value:m=1},b,T)=>{b.maxSubs=(b.maxSubs||3)+m,b.log.push({text:`🔄 +${m} remplacement(s)`,type:"info"}),T(b)},CUSTOM:({},m,b)=>b(m)};function ee(m,b,T,A){const E=document.createElement("div");E.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let L=[];function j(){var P,B;const D=m.map(H=>{const K={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[H._line]||"#555",Q=me(H,H._line)+(H.boost||0),we=L.find(J=>J.cardId===H.cardId)?"#FFD700":"rgba(255,255,255,0.25)",X=H.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${H.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${we};background:${K};overflow:hidden;cursor:pointer;${X}">
        ...
        <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
          ...
          <span style="font-size:12px;color:rgba(255,255,255,0.5)">${L.length}/${b}</span>
    ...
    </div>`,o==="p1"&&setTimeout(async()=>{await F({phase:"midfield"})},5e3)}let W=!1;function re(){if(W)return;const m=d[o+"Team"].MIL||[],b=d[r+"Team"].MIL||[];function T(J){return J.reduce((oe,de)=>oe+me(de,"MIL"),0)}function A(J){let oe=0;for(let de=0;de<J.length-1;de++){const ye=Je(J[de],J[de+1]);ye==="#00ff88"?oe+=2:ye==="#FFD700"&&(oe+=1)}return oe}const E=T(m)+A(m),L=T(b)+A(b),j=E>=L;function D(J,oe,de){return`<div id="duel-row-${de}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      </div>`,document.body.appendChild(K),(we=K.querySelector("#pvp-gc-back"))==null||we.addEventListener("click",()=>K.remove()),(X=K.querySelector("#pvp-gc-use"))==null||X.addEventListener("click",()=>{K.remove(),w(m,b)})}function z(){var A;const m=d[o+"Team"],b=Object.entries(m).flatMap(([E,L])=>(L||[]).filter(j=>!j.used).map(j=>({...j,_line:E})));if(!b.length)return;const T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",T.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
        <div style="flex:1;font-size:14px;font-weight:700;color:#fff">⚡ Choisir un joueur pour +${d.boostValue}</div>
        <button id="bp-close" style="background:none;border:none;color:rgba(255,255,255,0.5);font-size:22px;cursor:pointer">✕</button>
      </div>
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${b.map(E=>{const L={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[E._line]||"#555",j=me(E,E._line)+(E.boost||0);return`<div class="bp-item" data-cid="${E.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${L};overflow:hidden;cursor:pointer">
      ...
      </div>`,(he=K.querySelector("#psub-close"))==null||he.addEventListener("click",()=>K.remove()),(Ae=K.querySelector("#pout-up"))==null||Ae.addEventListener("click",()=>{D>0&&(D--,Q())}),(ze=K.querySelector("#pout-down"))==null||ze.addEventListener("click",()=>{D<L.length-1&&(D++,Q())}),(He=K.querySelector("#pin-up"))==null||He.addEventListener("click",()=>{B>0&&(B--,Q())}),(De=K.querySelector("#pin-down"))==null||De.addEventListener("click",()=>{B<j.length-1&&(B++,Q())});const ke=(Me,qe,Fe,Ue)=>{const Ne=K.querySelector("#"+Me);if(!Ne)return;let Qe=0;Ne.addEventListener("touchstart",Oe=>{Qe=Oe.touches[0].clientY},{passive:!0}),Ne.addEventListener("touchend",Oe=>{const Se=Oe.changedTouches[0].clientY-Qe;if(Math.abs(Se)<30)return;const Ke=qe();Se<0&&Ke<Ue-1?(Fe(Ke+1),Q()):Se>0&&Ke>0&&(Fe(Ke-1),Q())},{passive:!0})};ke("pin-panel",()=>B,Me=>B=Me,j.length),ke("pout-panel",()=>D,Me=>D=Me,L.length),(Ge=K.querySelector("#psub-confirm"))==null||Ge.addEventListener("click",async Me=>{if(Me.preventDefault(),Me.stopPropagation(),H)return;H=!0;const qe=L[D],Fe=j[B];if(!qe||!Fe)return;const Ue=qe._line,Ne=(T[Ue]||[]).findIndex(Se=>Se.cardId===qe.cardId);if(Ne===-1){p("Erreur : joueur introuvable","error"),K.remove();return}const Qe={...Fe,_line:Ue,position:qe.position,used:!1,boost:0};T[Ue].splice(Ne,1,Qe);const Oe=[...A,Fe.cardId];K.remove(),R(qe,Fe,async()=>{await F({[o+"Team"]:T,[r+"Team"]:d[r+"Team"],["usedSubIds_"+o]:Oe})})})}document.body.appendChild(K),Q()}function R(m,b,T){const A={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},E=document.createElement("div");E.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const L=(P,B,H)=>`<div style="text-align:center">
