...
import{s as v,l as Ht,m as Bi,F as qi,r as Ce,j as ii,h as Di,n as Kn,o as Vn,T as bi,k as Yn,p as Wn,b as Xn}from"./formation-links-Bb-Ce4un.js";const Jn="/";function Qi(e,{navigate:t,toast:i}){let n="login";const r=()=>{var a,s,d,c,l,g;const o=n==="login";e.innerHTML=`
    <div style="
    ...
    </style>`,(a=document.getElementById("tab-login-btn"))==null||a.addEventListener("click",()=>{n="login",r()}),(s=document.getElementById("tab-reg-btn"))==null||s.addEventListener("click",()=>{n="register",r()}),o?((d=document.getElementById("login-password"))==null||d.addEventListener("keydown",y=>{var m;y.key==="Enter"&&((m=document.getElementById("login-btn"))==null||m.click())}),(c=document.getElementById("login-btn"))==null||c.addEventListener("click",async()=>{const y=document.getElementById("login-email").value.trim(),m=document.getElementById("login-password").value,x=document.getElementById("login-error");if(x.textContent="",!y||!m){x.textContent="Remplissez tous les champs.";return}const p=document.getElementById("login-btn");p.textContent="⏳ Connexion…",p.disabled=!0;const{error:u}=await v.auth.signInWithPassword({email:y,password:m});if(p.textContent="⚽ Se connecter",p.disabled=!1,u){x.textContent=u.message.includes("Invalid")?"Email ou mot de passe incorrect.":u.message;return}window.location.reload()})):((l=document.getElementById("reg-confirm"))==null||l.addEventListener("keydown",y=>{var m;y.key==="Enter"&&((m=document.getElementById("reg-btn"))==null||m.click())}),(g=document.getElementById("reg-btn"))==null||g.addEventListener("click",async()=>{const y=document.getElementById("reg-email").value.trim(),m=document.getElementById("reg-password").value,x=document.getElementById("reg-confirm").value,p=document.getElementById("reg-error");if(p.textContent="",!y||!m||!x){p.textContent="Remplissez tous les champs.";return}if(m.length<6){p.textContent="Mot de passe trop court (min. 6 caractères).";return}if(m!==x){p.textContent="Les mots de passe ne correspondent pas.";return}const u=document.getElementById("reg-btn");u.textContent="⏳ Création…",u.disabled=!0;const{error:b}=await v.auth.signUp({email:y,password:m});if(u.textContent="🚀 Créer mon compte",u.disabled=!1,b){p.textContent=b.message;return}i("Compte créé ! Connecte-toi pour commencer.","success",4e3),n="login",r(),setTimeout(()=>{const w=document.getElementById("login-email");w&&(w.value=y)},50)}))};r()}function Qn(e,{state:t,navigate:i,toast:n,refreshProfile:r}){let o="#1A6B3C",a="#D4A017";e.innerHTML=`
  ...
  `;function s(){var x;const c=document.getElementById("logo-preview"),l=document.getElementById("logo-initials"),g=((x=document.getElementById("setup-club"))==null?void 0:x.value)||"MW",y=g.trim().split(" ").filter(Boolean),m=y.length>=2?(y[0][0]+y[1][0]).toUpperCase():g.slice(0,2).toUpperCase();c&&(c.style.background=a,c.style.borderColor=o),l&&(l.textContent=m,l.style.color=o)}document.getElementById("color1").addEventListener("input",c=>{o=c.target.value,document.getElementById("swatch1").style.background=o,s()}),document.getElementById("color2").addEventListener("input",c=>{a=c.target.value,document.getElementById("swatch2").style.background=a,s()});function d(c){document.querySelectorAll(".setup-step").forEach(l=>l.classList.remove("active")),document.getElementById(`step-${c}`).classList.add("active"),document.getElementById("step-num").textContent=c,document.getElementById("progress-fill").style.width=`${Math.round(c/3*100)}%`,c===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("step1-error");if(l.textContent="",c.length<3){l.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:g}=await v.from("users").select("id").eq("pseudo",c).maybeSingle();if(g){l.textContent="Ce pseudo est déjà pris.";return}d(2)}),document.getElementById("step2-back").addEventListener("click",()=>d(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const c=document.getElementById("setup-club").value.trim(),l=document.getElementById("step2-error");if(l.textContent="",c.length<2){l.textContent="Nom trop court (min. 2 caractères).";return}const{data:g}=await v.from("users").select("id").eq("club_name",c).maybeSingle();if(g){l.textContent="Ce nom de club est déjà pris.";return}d(3)}),document.getElementById("step3-back").addEventListener("click",()=>d(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("setup-club").value.trim(),g=document.getElementById("step3-error"),y=document.getElementById("step3-finish");g.textContent="",y.disabled=!0,y.textContent="Création en cours…";try{const{error:m}=await v.from("users").insert({id:t.user.id,pseudo:c,club_name:l,club_color1:o,club_color2:a,credits:1e4});if(m)throw m;await Zn(t.user.id),await r(),n(`Bienvenue ${c} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(m){g.textContent=m.message,y.disabled=!1,y.textContent="🚀 Créer mon profil !"}})}async function Zn(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await v.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const eo="modulepreload",to=function(e){return"/"+e},Zi={},mn=function(t,i,n){let r=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),s=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));r=Promise.allSettled(i.map(d=>{if(d=to(d),d in Zi)return;Zi[d]=!0;const c=d.endsWith(".css"),l=c?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${l}`))return;const g=document.createElement("link");if(g.rel=c?"stylesheet":eo,c||(g.as="script"),g.crossOrigin="",g.href=d,s&&g.setAttribute("nonce",s),document.head.appendChild(g),c)return new Promise((y,m)=>{g.addEventListener("load",y),g.addEventListener("error",()=>m(new Error(`Unable to preload CSS for ${d}`)))})}))}function o(a){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=a,window.dispatchEvent(s),!s.defaultPrevented)throw a}return r.then(a=>{for(const s of a||[])s.status==="rejected"&&o(s.reason);return t().catch(o)})},Xt="#1A6B3C",Jt="#cc2222",io="#D4A017",en="#888";async function no(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      ${y.map(({friendshipId:m,friend:x})=>oo(x,m)).join("")}
    </div>`,c.querySelectorAll("[data-stats]").forEach(m=>{m.addEventListener("click",()=>ao(e,m.dataset.stats,m.dataset.friendName))}),c.querySelectorAll("[data-match]").forEach(m=>{m.addEventListener("click",async()=>{const x=m.dataset.friendId,p=m.dataset.friendName;if(typeof n!="function"){t("Erreur navigation","error");return}const u=e.user.id,{data:b}=await v.from("friend_match_invites").select("id").eq("inviter_id",x).eq("invitee_id",u).eq("status","pending").order("created_at",{ascending:!1}).limit(1).maybeSingle(),w=!!b;console.log("[Friends] clic match",{fid:x,fname:p,isAccepting:w}),n("match",{matchMode:"friend",friendId:x,friendName:p,isAccepting:w})})})}function oo(e,t){const i=e.club_name||e.pseudo||"?",n=e.pseudo||"",r=(n||i).slice(0,2).toUpperCase(),o=e.club_color2||Xt,a=e.club_color1||"#ffffff",s=e.last_seen_at?new Date(e.last_seen_at):null,d=s&&Date.now()-s.getTime()<3*60*1e3;return`
    ...
    <div class="popup-box">
      ...
      ${l.length?`<div style="display:flex;flex-direction:column;gap:8px;max-height:50vh;overflow-y:auto;margin-bottom:14px">
            ${l.map(y=>{var m,x,p;return`
              <div style="display:flex;align-items:center;gap:10px;background:#f9f9f9;border-radius:10px;padding:10px 12px">
                <div style="flex:1;font-size:14px;font-weight:700">${((m=y.requester)==null?void 0:m.club_name)||((x=y.requester)==null?void 0:x.pseudo)||"?"}
    ...
    ${Pi()}`,document.body.appendChild(c);const g=()=>c.remove();c.querySelector("#pending-close").addEventListener("click",g),c.addEventListener("click",y=>{y.target===c&&g()}),c.querySelectorAll("[data-accept]").forEach(y=>{y.addEventListener("click",async()=>{const{error:m}=await v.from("friendships").update({status:"accepted"}).eq("id",y.dataset.accept);if(m){t("Erreur : "+m.message,"error");return}y.closest("div[style]").remove(),t("✅ Ami accepté !","success"),xn(e,t,n),i&&i()})}),c.querySelectorAll("[data-decline]").forEach(y=>{y.addEventListener("click",async()=>{await v.from("friendships").delete().eq("id",y.dataset.decline),y.closest("div[style]").remove(),t("Demande refusée","info"),i&&i()})})}async function ao(e,t,i){const n=e.user.id,[r,o]=[n,t].sort(),a=n===r,{data:s}=await v.from("friend_match_stats").select("*").eq("player1_id",r).eq("player2_id",o).single(),d=e.profile.club_name||e.profile.pseudo||"Moi",c=s||{},l=a?c.wins_p1||0:c.wins_p2||0,g=a?c.wins_p2||0:c.wins_p1||0,y=c.draws||0,m=a?c.goals_p1||0:c.goals_p2||0,x=a?c.goals_p2||0:c.goals_p1||0,p=a?c.gc_used_p1||0:c.gc_used_p2||0,u=a?c.gc_used_p2||0:c.gc_used_p1||0,b=c.matches_total||0,w=($,f,C,k=Xt,I=Jt)=>`
    ...
    </style>`}const so="2026.07.20-2340";async function tn(e,{state:t,navigate:i,toast:n}){var o,a;const r=t.profile;r&&(e.innerHTML=`
  <style>
    .home-dark {
      min-height: 100%;
  ...
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:0 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none;align-items:center"></div>
        ...
        </div>`},null,F,5)}e.querySelectorAll(".col-tab-btn").forEach(M=>{M.addEventListener("click",()=>{C=M.dataset.tab,k="Tous",I="",D=!1,e.querySelectorAll(".col-tab-btn").forEach(F=>{const O=F.dataset.tab===C;F.style.borderBottomColor=O?"var(--green)":"transparent",F.style.color=O?"var(--green)":"var(--tile-fg-dim)"}),z(),Y()})}),z(),Y()}function zo(e,t,i){const n=qe[e]||{icon:"⚡",desc:"Effet spécial."},r=(t==null?void 0:t.gc_type)==="ultra_game_changer",o={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},a={purple:"#b06ce0",light_blue:"#00d4ef"},s=o[t==null?void 0:t.color]||o.purple,d=a[t==null?void 0:t.color]||a.purple,c=(t==null?void 0:t.name)||e,l=(t==null?void 0:t.effect)||n.desc,g=t!=null&&t.image_url?`/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const oi=1e3;function Io(e,t,i,n){var x,p,u;const{state:r,toast:o,closeModal:a,navigate:s,refreshProfile:d}=i,c=e.formation,l={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function g(){const b=ii[c]||{},w=qi[c]||[],h=290,$=360,f=20;function C(I){const D=b[I];return D?{x:D.x*h,y:D.y*$}:null}let k=`<svg width="${h}" height="${$}" viewBox="0 0 ${h} ${$}" xmlns="http://www.w3.org/2000/svg">`;for(const[I,D]of w){const ie=C(I),ne=C(D);!ie||!ne||(k+=`<line x1="${ie.x}" y1="${ie.y}" x2="${ne.x}" y2="${ne.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const I of Object.keys(b)){const D=C(I);if(!D)continue;const ie=I.replace(/\d+/,""),ne=l[ie]||"#555";k+=`<circle cx="${D.x}" cy="${D.y}" r="${f}" fill="${ne}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,k+=`<text x="${D.x}" y="${D.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${ie}</text>`}return k+="</svg>",k}const y=t.filter(b=>b.formation===c);y.length;const m=!e.is_for_sale;n(`Formation ${c}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    <div style="margin-top:16px;border-top:1px solid var(--tile-border);padding-top:14px">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${g.map((N,V)=>{const U=Z[N.id]||[],M=N.is_for_sale,F=U.length?U[U.length-1]:null,Q=(l.rarity==="pepite"||l.rarity==="papyte")&&N.current_note!=null?` (☆${N.current_note})`:"";return`
      ...
      <!-- Carte visuelle -->
      ${Ce({...l,_evolution_bonus:m},{width:160})}
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(me=document.getElementById("close-detail"))==null||me.addEventListener("click",s);let Y=new Set;const oe=()=>{const N=Y.size,V=document.getElementById("sell-action-panel");if(!V)return;V.style.display=N>0?"block":"none",document.getElementById("sell-selected-count").textContent=N;const U=document.getElementById("evolve-btn");U&&(U.textContent=`⬆️ Évoluer ${N>1?"(+"+N+")":""}`)};document.querySelectorAll(".expl-check").forEach(N=>{N.addEventListener("change",()=>{const V=N.dataset.id;N.checked?Y.add(V):Y.delete(V);const U=N.closest(".exemplaire-row");U&&(U.style.borderColor=N.checked?"#1A6B3C":"#eee"),oe()})}),document.querySelectorAll(".exemplaire-row").forEach(N=>{N.addEventListener("click",V=>{if(V.target.closest("button")||V.target.tagName==="INPUT")return;const U=N.querySelector(".expl-check");U&&!U.disabled&&(U.checked=!U.checked,U.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(N=>{N.addEventListener("click",V=>{V.stopPropagation();const U=document.querySelector(`.expl-hist[data-hist="${N.dataset.idx}"]`);U&&(U.style.display=U.style.display==="none"?"flex":"none")})}),(A=document.getElementById("evolve-btn"))==null||A.addEventListener("click",async()=>{if(y<=1)return;const N=[...Y];if(!N.length)return;if(Y.has(e.id)){const H=document.createElement("div");H.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",H.innerHTML=`
  ...
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
        ...
        <div style="display:flex;gap:10px;align-items:flex-start">
          ...
          <div style="flex:1;min-width:0">
            ...
            <div style="display:flex;gap:4px;align-items:center;flex-wrap:nowrap;overflow-x:auto;scrollbar-width:none;-ms-overflow-style:none" id="subs-list" class="no-scrollbar">
              ${c.map(u=>{const b={...u.player,_evolution_bonus:u.evolution_bonus||0};return`<div style="position:relative;flex-shrink:0;overflow:visible">
                  ${Ce({...b,_evolution_bonus:b._evolution_bonus||0},{width:58,showStad:!0,stadDef:d})}
                  <button data-remove-sub="${u.id}"
              ...
              ${t.subs.length<5?'<div id="add-sub-btn" style="width:37px;height:47px;border:2px dashed rgba(255,255,255,0.3);border-radius:5px;display:flex;align-items:center;justify-content:center;font-size:16px;color:rgba(255,255,255,0.4);cursor:pointer;flex-shrink:0">+</div>':""}
            </div>
          </div>
          <!-- Formation mobile -->
  ...
  </div>`;const l=window.innerWidth>=900,g=e.querySelector(".deck-pc-layout"),y=e.querySelector(".deck-mobile-layout");g&&(g.style.display=l?"block":"none"),y&&(y.style.display=l?"none":"block"),So(e,t,r,i),e.querySelectorAll("#builder-back").forEach(u=>u.addEventListener("click",()=>n("decks")));const m=()=>{const{openModal:u,closeModal:b}=i,w=`<div style="display:flex;flex-wrap:wrap;gap:8px;padding:8px">
    ...
    <div style="position:relative;width:${m}px;height:${x}px;margin:0 auto">
      <svg style="position:absolute;inset:0;width:100%;height:100%;pointer-events:none" viewBox="0 0 ${m} ${x}">${u}</svg>
      ${b}
    ...
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(b=document.getElementById("close-selector"))==null||b.addEventListener("click",o),(w=document.getElementById("remove-player"))==null||w.addEventListener("click",()=>{delete t.slots[e],o(),wt(i,t,n)}),document.querySelectorAll(".player-option").forEach(h=>{h.addEventListener("click",()=>{t.slots[e]=h.dataset.cardId,o(),wt(i,t,n)})})}function Co(e,t,i){var l,g,y;const{openModal:n,closeModal:r}=i,o=(l=e.stadiumCards)==null?void 0:l.find(m=>m.id===e.stadiumCardId),a=o&&((g=e.stadDefMap)==null?void 0:g[o.stadium_id])||null,s=new Set;Object.keys(e.slots).forEach(m=>{var u;const x=e.slots[m];if(!x)return;const p=e.playerCards.find(b=>b.id===x);(u=p==null?void 0:p.player)!=null&&u.id&&s.add(p.player.id)}),e.subs.forEach(m=>{var p;const x=e.playerCards.find(u=>u.id===m);(p=x==null?void 0:x.player)!=null&&p.id&&s.add(x.player.id)});const d=new Set,c=e.playerCards.filter(m=>{var x,p,u;return s.has((x=m.player)==null?void 0:x.id)||d.has((p=m.player)==null?void 0:p.id)?!1:(d.add((u=m.player)==null?void 0:u.id),!0)});n("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${c.length>0?'<div style="display:flex;flex-wrap:wrap;gap:8px;justify-content:center">'+c.map(m=>{const x={...m.player,_evolution_bonus:m.evolution_bonus||0};return`<div class="player-option" data-card-id="${m.id}" style="cursor:pointer">
  ...
  `,document.body.appendChild(r);let o=!1;const a=document.getElementById("pack-cut-zone"),s=document.getElementById("pack-blade");let d=!1;const c=z=>z.touches&&z.touches[0]?z.touches[0].clientX:z.clientX;function l(z){o||(d=!0,s.style.opacity="1",g(z))}function g(z){if(!d||o)return;const Y=a.getBoundingClientRect(),oe=c(z)-Y.left,fe=Math.max(0,Math.min(1,oe/Y.width));s.style.width=fe*Y.width+"px",fe>=.82&&m()}function y(){o||(d=!1,s.style.transition="width .2s ease, opacity .2s ease",s.style.width="0",s.style.opacity="0",setTimeout(()=>{o||(s.style.transition="")},220))}function m(){var Y;if(o)return;o=!0,d=!1,s.style.width="100%",s.style.opacity="1",(Y=document.getElementById("cut-flash"))==null||Y.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const z=document.getElementById("cut-hint");z&&(z.style.opacity="0"),a.classList.add("pack-cut"),setTimeout(()=>{s.style.opacity="0",document.getElementById("pack-phase").style.display="none",b(0)},620)}a.addEventListener("pointerdown",l),window.addEventListener("pointermove",g),window.addEventListener("pointerup",y),a.addEventListener("touchstart",l,{passive:!0}),window.addEventListener("touchmove",g,{passive:!0}),window.addEventListener("touchend",y);let x=0,p=!1;const u=new Set;function b(z){x=z,document.getElementById("reveal-phase").style.display="flex",w(),h(z,0),I()}function w(){const z=document.getElementById("card-dots");z&&(z.innerHTML=e.map((Y,oe)=>`<div class="card-dot" data-i="${oe}" style="width:8px;height:8px;border-radius:50%;background:${oe===x?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),z.querySelectorAll(".card-dot").forEach(Y=>Y.addEventListener("click",()=>f(parseInt(Y.dataset.i)))))}function h(z,Y){var U,M;const oe=e[z],fe=document.getElementById("card-counter"),me=document.getElementById("card-track");fe&&(fe.textContent=`Carte ${z+1} / ${e.length}`);const A=document.getElementById("reveal-btns");A&&(A.style.display=z===e.length-1?"flex":"none");const W=oe.card_type==="player"&&((U=oe.player)==null?void 0:U.rarity)==="legende",ue=!u.has(z);u.add(z);let N=0;if(oe.card_type==="player"&&oe.player){const F=oe.player,O=F.job||"ATT";N=Number(O==="GK"?F.note_g:O==="DEF"?F.note_d:O==="MIL"?F.note_m:F.note_a)||0}const V=F=>{me.innerHTML=`
        ...
        ${V}
      </div>`,M&&navigator.vibrate&&navigator.vibrate([50,30,100,30,200])},(B+R)*3),setTimeout(H,(B+R)*3+B),setTimeout(()=>{F.style.display="none",O.innerHTML="",p=!1,Q&&!U&&Q.pause(),navigator.vibrate&&navigator.vibrate([40,30,80]),Y()},(B+R)*4)}function f(z){if(p||z<0||z>=e.length||z===x)return;const Y=z>x?1:-1;x=z,h(z,Y)}function C(){f(x+1)}function k(){f(x-1)}function I(){const z=document.getElementById("card-viewport");if(!z||z._swipeBound)return;z._swipeBound=!0;let Y=0,oe=0,fe=0,me=!1;const A=U=>U.touches?U.touches[0].clientX:U.clientX,W=U=>U.touches?U.touches[0].clientY:U.clientY,ue=U=>{me=!0,Y=A(U),oe=W(U),fe=0},N=U=>{if(!me)return;fe=A(U)-Y;const M=W(U)-oe;if(Math.abs(fe)<Math.abs(M))return;const F=document.getElementById("current-card-wrap");F&&(F.style.transition="none",F.style.transform=`translateX(${fe*.6}px) rotate(${fe*.02}deg)`)},V=()=>{if(!me)return;me=!1;const U=document.getElementById("current-card-wrap"),M=55;fe<=-M&&x<e.length-1?C():fe>=M&&x>0?k():U&&(U.style.transition="transform .2s ease",U.style.transform="translateX(0)")};z.addEventListener("pointerdown",ue),z.addEventListener("pointermove",N),z.addEventListener("pointerup",V),z.addEventListener("pointercancel",V),z.addEventListener("touchstart",ue,{passive:!0}),z.addEventListener("touchmove",N,{passive:!0}),z.addEventListener("touchend",V),z.addEventListener("click",U=>{if(Math.abs(fe)>8)return;const M=z.getBoundingClientRect();U.clientX-M.left>M.width/2?C():k()})}let D=null;function ie(){const z=document.getElementById("fireworks-canvas");if(!z)return;z.width=window.innerWidth,z.height=window.innerHeight;const Y=z.getContext("2d"),oe=[];function fe(){const A=Math.random()*z.width,W=Math.random()*z.height*.6,ue=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],N=ue[Math.floor(Math.random()*ue.length)];for(let V=0;V<60;V++){const U=Math.PI*2/60*V,M=2+Math.random()*5;oe.push({x:A,y:W,vx:Math.cos(U)*M,vy:Math.sin(U)*M,alpha:1,color:N,size:2+Math.random()*3})}}fe(),D=setInterval(fe,600);function me(){if(document.getElementById("fireworks-canvas")){Y.clearRect(0,0,z.width,z.height);for(let A=oe.length-1;A>=0;A--){const W=oe[A];if(W.x+=W.vx,W.y+=W.vy+.08,W.vy*=.98,W.alpha-=.018,W.alpha<=0){oe.splice(A,1);continue}Y.globalAlpha=W.alpha,Y.fillStyle=W.color,Y.beginPath(),Y.arc(W.x,W.y,W.size,0,Math.PI*2),Y.fill()}Y.globalAlpha=1,(D!==null||oe.length>0)&&requestAnimationFrame(me)}}me()}function ne(){D!==null&&(clearInterval(D),D=null);const z=document.getElementById("fireworks-canvas");z&&z.getContext("2d").clearRect(0,0,z.width,z.height)}if(n){const z=document.getElementById("reveal-btns");z&&(z.innerHTML='<button class="btn btn-primary" id="reveal-next" style="flex:1">Continuer →</button>'),(j=document.getElementById("reveal-next"))==null||j.addEventListener("click",()=>{ne(),r.remove(),n()})}else document.getElementById("reveal-collection").addEventListener("click",()=>{ne(),r.remove(),i("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{ne(),r.remove(),i("boosters")})}function Ho(e){var t,i,n,r;if(e.card_type==="player"&&e.player)return Po(e);if(e.card_type==="game_changer"){const o=e._gcDef,a=(o==null?void 0:o.gc_type)==="ultra_game_changer",s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},d={purple:"#b06ce0",light_blue:"#00d4ef"},c=s[o==null?void 0:o.color]||s.purple,l=d[o==null?void 0:o.color]||d.purple,g=(o==null?void 0:o.name)||e.gc_type||"Game Changer",y=(o==null?void 0:o.effect)||((t=Ti[e.gc_type])==null?void 0:t.desc)||"",m=o!=null&&o.image_url?`/icons/${o.image_url}`:null,x=((i=Ti[e.gc_type])==null?void 0:i.icon)||"⚡";return`<div style="width:170px;height:240px;background:${c};border-radius:14px;border:3px solid ${l};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${l}88;flex-shrink:0">
      ...
      <div style="flex:1;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06);margin:0">
        ${m?`<img src="${m}" style="max-width:120px;max-height:120px;width:auto;height:auto;object-fit:contain;border-radius:6px">`:`<span style="font-size:56px">${x}</span>`}
      </div>
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    </div>`}const d=l=>{e.style.overflow="",e.style.height="",e.style.display="",e.style.flexDirection="",i(l)};function c(){var g,y,m;e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column";const l=a.length>0;e.innerHTML=`
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      card:cards(id,card_type,formation,stadium_id,evolution_bonus,
        player:players(id,firstname,surname_real,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order"),d=[...new Set((s||[]).filter(y=>{var m,x;return((m=y.card)==null?void 0:m.card_type)==="stadium"&&((x=y.card)==null?void 0:x.stadium_id)}).map(y=>y.card.stadium_id))],c={};if(d.length){const{data:y}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",d);(y||[]).forEach(m=>{c[m.id]=m}),(s||[]).forEach(m=>{var x,p;((x=m.card)==null?void 0:x.card_type)==="stadium"&&((p=m.card)!=null&&p.stadium_id)&&(m.card._stadiumDef=c[m.card.stadium_id]||null)})}let l=0;function g(){var f,C,k,I,D,ie,ne;const y=o[l],m=(s||[]).filter(Z=>Z.deck_id===y.id),x=m.filter(Z=>{var j;return Z.is_starter&&((j=Z.card)==null?void 0:j.player)}).map(Z=>Ii(Z.card,Z.position)),p=m.find(Z=>{var j;return((j=Z.card)==null?void 0:j.card_type)==="formation"}),u=y.formation||((f=p==null?void 0:p.card)==null?void 0:f.formation)||"4-4-2";let b=x.length>=11?li(x,u):null,w=((C=y.stadium_card)==null?void 0:C.stadium_def)||null;w&&b&&(b=ti(b,w));const h=x.length>=11;dt(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
  ...
  <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:clamp(6px,1.5vh,20px);padding:clamp(8px,1.5vw,24px);background:#0a3d1e">
    ...
    <div id="duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center, rgba(10,61,30,0.4), rgba(10,61,30,0.92))"></div>
          ...
          <button id="start-match-btn" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,0.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">
            ▶ Commencer le match
          </button>`,$.style.transition="opacity .45s ease",$.style.opacity="1",$.style.pointerEvents="auto",(f=document.getElementById("start-match-btn"))==null||f.addEventListener("click",m))},600)},700)},2800)}function ze(e,t,i){var ie,ne,Z,j,z,Y,oe,fe,me;const n=t.selected.map(A=>A.cardId),r=t.usedSubIds||[],o=t.homeSubs.filter(A=>!r.includes(A.cardId)),s=Object.values(t.homeTeam).flat().filter(A=>A.used).length>0&&o.length>0&&t.subsUsed<t.maxSubs,d=[...t.homeTeam.MIL||[],...t.homeTeam.ATT||[]].filter(A=>!A.used),c=[...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]].filter(A=>!A.used),l=Ot(t.aiTeam),g=d.length===0&&c.length===0&&!l,y=(t.homeTeam.DEF||[]).filter(A=>!A.used),m=(t.homeTeam.GK||[]).filter(A=>!A.used);let x=[];t.phase==="attack"&&d.length===0&&(l?(x=y.map(A=>A.cardId),y.length===0&&(x=x.concat(m.map(A=>A.cardId)))):g&&(x=[...y,...m].map(A=>A.cardId))),t.log[t.log.length-1];const p=t.phase==="ai-attack"||t.phase==="ai-defense",u=t.phase==="attack",b=t.phase==="defense",w=t.phase==="finished",$=(t.homeSubs||[]).filter(A=>!(t.usedSubIds||[]).includes(A.cardId)).length>0&&t.subsUsed<t.maxSubs,f=u&&d.length===0&&x.length===0&&!$,C=t.gcCards.filter(A=>!t.usedGc.includes(A.id)),k=t.boostCard&&!t.boostUsed;e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column",e.innerHTML=`
  <style>
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    ${(()=>{const A=window.innerWidth>=700,W=(H,B,R)=>{var Ee,Le;const le=(t.gcDefs||[]).find(Fe=>Fe.name===H.gc_type),pe={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[le==null?void 0:le.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",be={purple:"#b06ce0",light_blue:"#00d4ef"}[le==null?void 0:le.color]||"#b06ce0",we=(le==null?void 0:le.name)||H.gc_type,Te=(le==null?void 0:le.effect)||((Ee=qe[H.gc_type])==null?void 0:Ee.desc)||"",Ae=le!=null&&le.image_url?`/icons/${le.image_url}`:null,ke=((Le=qe[H.gc_type])==null?void 0:Le.icon)||"⚡",$e=Math.round(R*.22),je=Math.round(R*.22),De=R-$e-je,Ue=we.length>12?7:9;return`<div class="gc-mini" data-gc-id="${H.id}" data-gc-type="${H.gc_type}"
        ...
        <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
          ${Nt(t.homeTeam,t.formation,t.phase,n,A?1300:zt(),A?600:Gt(),x)}
        </div>
      </div>`;return A?`
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(A=>{var W,ue,N;if(A.type==="duel"){const V=A.isGoal,U=A.homeScored?"#FFD700":V?"#ff6b6b":"rgba(255,255,255,0.3)",M=A.homeScored?"⚽ BUT !":V?"⚽ BUT IA !":(W=A.homePlayers)!=null&&W.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${V?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${U};margin-bottom:4px">
                <div style="font-size:9px;color:${U};letter-spacing:1px;margin-bottom:5px;font-weight:700;text-transform:uppercase">${M}</div>
                ${(ue=A.homePlayers)!=null&&ue.length?`<div style="margin-bottom:3px">${Qe(A.homePlayers,"rgba(255,255,255,0.7)",A.homeTotal,void 0,t.formation)}</div>`:""}
                ${(N=A.aiPlayers)!=null&&N.length?`<div style="opacity:0.7">${Qe(A.aiPlayers,"#ff6b6b",A.aiTotal,void 0,t.formation)}</div>`:""}
                ...
                <div style="display:flex;align-items:center;gap:8px">
                  ${A.outPlayer?vt({...A.outPlayer,used:!0,_line:A.outPlayer.job,rarity:"normal"},38,50):""}
                  <span style="color:rgba(255,255,255,0.4);font-size:18px">→</span>
                  ${A.inPlayer?vt({...A.inPlayer,_line:A.inPlayer.job,rarity:"normal"},38,50):""}
                </div>
    ...
    </div>`,(f=g.querySelector("#sub-close"))==null||f.addEventListener("click",()=>g.remove()),(C=g.querySelector("#out-up"))==null||C.addEventListener("click",()=>{s>0&&(s--,y())}),(k=g.querySelector("#out-down"))==null||k.addEventListener("click",()=>{s<o.length-1&&(s++,y())}),(I=g.querySelector("#in-up"))==null||I.addEventListener("click",()=>{c>0&&(c--,y())}),(D=g.querySelector("#in-down"))==null||D.addEventListener("click",()=>{c<a.length-1&&(c++,y())});const $=(ne,Z,j,z)=>{const Y=g.querySelector("#"+ne);if(!Y)return;let oe=0;Y.addEventListener("touchstart",fe=>{oe=fe.touches[0].clientY},{passive:!0}),Y.addEventListener("touchend",fe=>{const me=fe.changedTouches[0].clientY-oe;if(Math.abs(me)<30)return;const A=Z();me<0&&A<z-1?(j(A+1),y()):me>0&&A>0&&(j(A-1),y())},{passive:!0})};$("in-panel",()=>c,ne=>c=ne,a.length),$("out-panel",()=>s,ne=>s=ne,o.length),(ie=g.querySelector("#sub-confirm"))==null||ie.addEventListener("click",ne=>{if(ne.preventDefault(),ne.stopPropagation(),l)return;l=!0;const Z=o[s],j=a[c];if(!Z||!j)return;let z=null,Y=-1;for(const[fe,me]of Object.entries(t.homeTeam)){const A=(me||[]).findIndex(W=>W.cardId===Z.cardId);if(A!==-1){z=fe,Y=A;break}}if(Y===-1||!z){$t("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),g.remove();return}const oe={...j,_line:z,_col:Z._col||0,used:!1,boost:0};t.homeTeam[z].splice(Y,1,oe),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(j.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:Z.name,firstname:Z.firstname,note:Ne(Z,z),portrait:ei(Z),job:Z.job,country_code:Z.country_code,rarity:Z.rarity,clubName:Z.clubName,clubLogo:Z.clubLogo},inPlayer:{name:j.name,firstname:j.firstname,note:Ne(j,z),portrait:ei(j),job:j.job,country_code:j.country_code,rarity:j.rarity,clubName:j.clubName,clubLogo:j.clubLogo},text:`🔄 ${j.firstname} ${j.name} remplace ${Z.firstname} ${Z.name}`}),g.remove(),er(Z,j,()=>ze(e,t,i))})}document.body.appendChild(g),y()}function ur(e,t,i,n,r){var x,p;const o=(n.gcDefs||[]).find(u=>u.name===t),a=qe[t]||{icon:"⚡",desc:"Carte spéciale."},s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[o==null?void 0:o.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",d={purple:"#b06ce0",light_blue:"#00d4ef"}[o==null?void 0:o.color]||"#b06ce0",c=(o==null?void 0:o.name)||t,l=(o==null?void 0:o.effect)||a.desc,g=o!=null&&o.image_url?`/icons/${o.image_url}`:null,y=a.icon||"⚡",m=document.createElement("div");m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",m.innerHTML=`
    ...
    </div>`,document.body.appendChild(m),(x=m.querySelector("#gc-back"))==null||x.addEventListener("click",()=>m.remove()),(p=m.querySelector("#gc-use"))==null||p.addEventListener("click",()=>{m.remove(),gr(e,t,i,n,r)})}function ri(e,t,i,n,r,o){const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let s=[];function d(){var c,l;a.innerHTML=`
    ...
    <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start;justify-content:center">
      ${e.map(g=>{const y=g._line||g.job||"MIL",m=s.find(p=>p.cardId===g.cardId),x=Ce({...g,_evolution_bonus:0},{width:90,showStad:!0,role:y,extraNote:g.boost||0});return`<div class="gc-pick-item" data-cid="${g.cardId}"
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(r=>{r.addEventListener("click",()=>{const o=r.dataset.cardId;for(const a of["GK","DEF","MIL","ATT"]){const s=(t.homeTeam[a]||[]).find(d=>d.cardId===o);if(s){s.boost=(s.boost||0)+t.boostCard.value,t.log.push({text:`⚡ Boost +${t.boostCard.value} appliqué à ${s.name}`,type:"info"});break}}t.boostUsed=!0,i.closeModal(),ze(e,t,i)})})}async function ht(e,t,i){var l,g;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:n}=i,r=t.homeScore>t.aiScore,o=t.homeScore===t.aiScore,a=r?"victoire":o?"nul":"defaite",s=xo(t.mode,a);t.matchId&&await v.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:r?n.profile.id:null,home_credits_reward:s,played_at:new Date().toISOString()}).eq("id",t.matchId);const d={credits:(n.profile.credits||0)+s,matches_played:(n.profile.matches_played||0)+1};r?d.wins=(n.profile.wins||0)+1:o?d.draws=(n.profile.draws||0)+1:d.losses=(n.profile.losses||0)+1,await v.from("users").update(d).eq("id",n.profile.id),await i.refreshProfile();const c=document.createElement("div");c.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",c.innerHTML=`
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      ...
      ${gi(e.aiTeam,e.formation,null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.1))}
            ...
            <div style="background:rgba(255,255,255,0.08);border-radius:14px;padding:14px 20px;text-align:center;min-width:220px">
              ...
              ${Se.id!==nt.id?`<div style="font-size:20px;font-weight:900;color:${Se.color}">
                    ${Oe>J.mmr?"📈":"📉"} ${nt.emoji} ${nt.label} → ${Se.emoji} ${Se.label}
                  </div>
            ...
            </div>`}}catch(J){console.error("[Ranked] MMR update error:",J)}(re=document.getElementById("pvp-end-overlay"))==null||re.remove();const q=document.createElement("div");q.id="pvp-end-overlay",q.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;color:#fff;padding:24px;text-align:center;overflow-y:auto";const P=L?"🤝":S?"🏆":"😞",K=L?"MATCH NUL":S?"VICTOIRE !":"DÉFAITE",ee=L?"#fff":S?"#FFD700":"#ff6b6b";q.innerHTML=`
      ...
      ${G}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${l?"🏆 Retour à la Mini League":a?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(q),(te=q.querySelector("#pvp-end-home"))==null||te.addEventListener("click",()=>{q.remove(),it(e),l?m("mini-league",{openLeagueId:l}):m(a?"ranked":"home")})}const Z=v.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},_=>{var T;const E=_.new;try{if(E.status==="finished"||E.forfeit){if(C)return;C=!0,k&&(clearInterval(k),k=null),E.game_state&&(f=E.game_state),ne(E);return}if(E.game_state){const S=f;f=E.game_state;const L=f._lastGC;if(L&&L.seq&&!ie.has(L.seq)&&(ie.add(L.seq),L.by!==p)){U(L.type,L.by,()=>W());return}const G=S[p+"Score"]||0,q=S[u+"Score"]||0,P=f[p+"Score"]||0,K=f[u+"Score"]||0,ee=P>G,re=K>q;if((ee||re)&&!D.has(f.round)){D.add(f.round);const te=[...f.log||[]].reverse().find(de=>de.isGoal),J=((te==null?void 0:te.homePlayers)||[]).map(de=>({name:de.name,note:de.note,portrait:de.portrait,job:de.job}));!!((T=te==null?void 0:te.text)!=null&&T.includes("DERNIER CORNER"))?be().then(()=>{Ze(J,P,K,ee,()=>W())}):Ze(J,P,K,ee,()=>W());return}W()}}catch(S){console.error("[PvP] crash:",S)}}).subscribe();async function j(_){Object.assign(f,_),f.lastActionAt=new Date().toISOString();const{error:E}=await v.from("matches").update({game_state:f}).eq("id",i);E&&x("Erreur de synchronisation","error");try{W()}catch(T){console.error("[PvP] renderPvpScreen crash:",T)}}async function z(){if(C)return;C=!0,k&&(clearInterval(k),k=null);const _=n?h.away_id:h.home_id,E=n?"p2":"p1",T=n?"p1":"p2",S={...f,[E+"Score"]:3,[T+"Score"]:0,phase:"finished"},L=S.p1Score||0,G=S.p2Score||0;try{await v.from("matches").update({game_state:S}).eq("id",i);const{data:q,error:P}=await v.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:_,p_home_score:L,p_away_score:G,p_forfeit:!0});if(P&&console.error("[PvP] finish_pvp_match (forfeit):",P),l)try{await v.rpc("check_and_finish_mini_league",{p_league_id:l})}catch(K){console.error("[PvP] check_and_finish_mini_league (forfeit):",K)}try{const{data:K}=await v.rpc("apply_match_rewards",{p_match_id:i});K!=null&&K.success&&!(K!=null&&K.skipped)&&typeof t.refreshProfile=="function"&&await t.refreshProfile()}catch(K){console.error("[PvP] apply_match_rewards (forfeit):",K)}}catch{}try{v.removeChannel(Z)}catch{}setTimeout(()=>{it(e),l?m("mini-league",{openLeagueId:l}):m("home")},800)}const Y={BOOST_STAT:({value:_=1,count:E=1,roles:T=[]},S,L)=>{const G=S[p+"Team"],q=Object.entries(G).filter(([P])=>!T.length||T.includes(P)).flatMap(([P,K])=>K.filter(ee=>!ee.used).map(ee=>({...ee,_line:P})));if(!q.length){S.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),L(S);return}oe(q,E,`Choisir ${E} joueur(s) à booster (+${_})`,P=>{P.forEach(K=>{const ee=(G[K._line]||[]).find(re=>re.cardId===K.cardId);ee&&(ee.boost=(ee.boost||0)+_,S.log.push({text:`⚡ +${_} sur ${ee.name}`,type:"info"}))}),S[p+"Team"]=G,L(S)})},DEBUFF_STAT:({value:_=1,count:E=1,roles:T=[],target:S="ai"},L,G)=>{const q=S==="home"?p:u,P=L[q+"Team"],K=S==="home"?"allié":"adverse",ee=Object.entries(P).filter(([re])=>!T.length||T.includes(re)).flatMap(([re,te])=>te.map(J=>({...J,_line:re})));if(!ee.length){L.log.push({text:`🎯 Aucun joueur ${K}`,type:"info"}),G(L);return}oe(ee,E,`Choisir ${E} joueur(s) ${K}(s) (-${_})`,re=>{re.forEach(te=>{const J=(P[te._line]||[]).find(se=>se.cardId===te.cardId);J&&(J.boost=(J.boost||0)-_,L.log.push({text:`🎯 -${_} sur ${J.name}`,type:"info"}))}),L[q+"Team"]=P,G(L)})},GRAY_PLAYER:({count:_=1,roles:E=[],target:T="ai"},S,L)=>{const G=T==="home"?p:u,q=S[G+"Team"],P=T==="home"?"allié":"adverse",K=Object.entries(q).filter(([ee])=>!E.length||E.includes(ee)).flatMap(([ee,re])=>re.filter(te=>!te.used).map(te=>({...te,_line:ee})));if(!K.length){S.log.push({text:`❌ Aucun joueur ${P}`,type:"info"}),L(S);return}oe(K,_,`Choisir ${_} joueur(s) ${P}(s) à exclure`,ee=>{const re="usedCardIds_"+G,te=new Set(S[re]||[]);ee.forEach(J=>{const se=(q[J._line]||[]).find(de=>de.cardId===J.cardId);se&&(se.used=!0,te.add(J.cardId),S.log.push({text:`❌ ${se.name} exclu !`,type:"info"}))}),S[re]=[...te],S[G+"Team"]=q,L(S)})},REVIVE_PLAYER:({count:_=1,roles:E=[]},T,S)=>{const L=T[p+"Team"],G=Object.entries(L).filter(([q])=>!E.length||E.includes(q)).flatMap(([q,P])=>P.filter(K=>K.used).map(K=>({...K,_line:q})));if(!G.length){T.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),S(T);return}oe(G,_,`Choisir ${_} joueur(s) à ressusciter`,q=>{q.forEach(P=>{const K=(L[P._line]||[]).find(ee=>ee.cardId===P.cardId);K&&(K.used=!1,T.log.push({text:`💫 ${K.name} ressuscité !`,type:"info"}))}),T[p+"Team"]=L,S(T)})},REMOVE_GOAL:({},_,E)=>{const T=u+"Score";_[T]>0?(_[T]--,_.log.push({text:"🚫 Dernier but annulé !",type:"info"})):_.log.push({text:"🚫 Aucun but à annuler",type:"info"}),E(_)},ADD_GOAL_DRAW:({},_,E)=>{_[p+"Score"]===_[u+"Score"]?(_[p+"Score"]++,_.log.push({text:"🎯 But bonus !",type:"info"})):_.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),E(_)},ADD_SUB:({value:_=1},E,T)=>{E.maxSubs=(E.maxSubs||3)+_,E.log.push({text:`🔄 +${_} remplacement(s)`,type:"info"}),T(E)},CUSTOM:({},_,E)=>E(_)};function oe(_,E,T,S){const L=document.createElement("div");L.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let G=[];function q(){var K,ee;const P=_.map(re=>{const te=re._line||re.job||"MIL",J=G.find(de=>de.cardId===re.cardId),se=Ce({...re,_evolution_bonus:0},{width:90,showStad:!0,role:te,extraNote:re.boost||0});return`<div class="pp-item" data-cid="${re.cardId}"
        ...
        </div>`,(K=L.querySelector("#pp-close"))==null||K.addEventListener("click",()=>L.remove()),L.querySelectorAll(".pp-item").forEach(re=>{re.addEventListener("click",()=>{const te=re.dataset.cid,J=_.find(de=>de.cardId===te),se=G.findIndex(de=>de.cardId===te);J&&(se>-1?G.splice(se,1):G.length<E&&G.push(J),q())})}),(ee=L.querySelector("#pp-confirm"))==null||ee.addEventListener("click",()=>{L.remove(),S(G)})}q(),document.body.appendChild(L)}async function fe(_,E){const S=(f["gcCardsFull_"+p]||[]).find(P=>P.id===_),L=(S==null?void 0:S._gcDef)||(f.gcDefs||[]).find(P=>{var K;return P.name===E||((K=P.name)==null?void 0:K.toLowerCase().trim())===(E==null?void 0:E.toLowerCase().trim())}),G=[...f["usedGc_"+p]||[],_],q={type:E,by:p,seq:(f._gcAnimSeq||0)+1};ie.add(q.seq),U(E,p,async()=>{if(L!=null&&L.effect_type&&L.effect_type!=="CUSTOM"){const K=Y[L.effect_type];if(K){const ee={...f};K(L.effect_params||{},ee,async re=>{re["usedGc_"+p]=G,re._lastGC=q,re._gcAnimSeq=q.seq,await j(re)});return}}const P={...f};switch(E){case"Remplacement+":P.maxSubs=(P.maxSubs||3)+1,P.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const K=u+"Score";P[K]>0&&(P[K]--,P.log.push({text:"🚫 But annulé",type:"info"}));break}}P["usedGc_"+p]=G,P._lastGC=q,P._gcAnimSeq=q.seq,await j(P)})}function me(_,E){const T="usedCardIds_"+_,S=new Set(f[T]||[]);E.forEach(L=>S.add(L)),f[T]=[...S]}function A(){for(const _ of["p1","p2"]){const E=new Set(f["usedCardIds_"+_]||[]),T=f[_+"Team"];if(T)for(const S of["GK","DEF","MIL","ATT"])(T[S]||[]).forEach(L=>{L.used=E.has(L.cardId)})}}function W(){var Ki,Vi,Yi,Wi,Xi,Ji;if(f.phase==="reveal")return ue();if(f.phase==="midfield")return V();if(f.phase==="finished")return Fe();const _=f[p+"Team"],E=f[u+"Team"];A();const T=f[p+"Score"],S=f[u+"Score"],L=f[p+"Name"],G=f[u+"Name"],q=f.phase===p+"-attack",P=f.phase===p+"-defense",K=Array.isArray(f["selected_"+p])?f["selected_"+p]:[],ee=K.map(X=>X.cardId),re=window.innerWidth>=700,te=f[p+"Subs"]||[],J=f["usedSubIds_"+p]||[],se=te.filter(X=>!J.includes(X.cardId)),de=f["gcCardsFull_"+p]||[],xe=f["usedGc_"+p]||[],ce=de.filter(X=>!xe.includes(X.id)),ye=f.boostOwner===p&&!f.boostUsed,he=[..._.MIL||[],..._.ATT||[]].filter(X=>!X.used),ae=[...E.MIL||[],...E.ATT||[]].filter(X=>!X.used),Ie=!$e(E),Oe=he.length===0&&ae.length===0&&!Ie,Se=(_.DEF||[]).filter(X=>!X.used),We=(_.GK||[]).filter(X=>!X.used);let Ke=[];q&&he.length===0&&(Ie?(Ke=Se.map(X=>X.cardId),Se.length===0&&(Ke=Ke.concat(We.map(X=>X.cardId)))):Oe&&(Ke=[...Se,...We].map(X=>X.cardId)));function nt(X,ge,Me){var Bt,gt;const _e=X._gcDef,jt={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[_e==null?void 0:_e.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",at={purple:"#b06ce0",light_blue:"#00d4ef"}[_e==null?void 0:_e.color]||"#b06ce0",Ge=(_e==null?void 0:_e.name)||X.gc_type,He=(_e==null?void 0:_e.effect)||((Bt=qe[X.gc_type])==null?void 0:Bt.desc)||"",Be=_e!=null&&_e.image_url?`/icons/${_e.image_url}`:null,kt=((gt=qe[X.gc_type])==null?void 0:gt.icon)||"⚡",Re=Math.round(Me*.22),ft=Math.round(Me*.22),st=Me-Re-ft,Ct=Ge.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${X.id}" data-gc-type="${X.gc_type}"
      ...
      <div style="display:flex;align-items:center;padding:8px 10px;background:rgba(0,0,0,0.5);gap:6px;flex-shrink:0">
        ...
        <div style="flex:1;display:flex;align-items:center;justify-content:center;gap:8px">
          ...
          <span style="font-size:26px;font-weight:900;color:#FFD700;letter-spacing:2px">${T} – ${S}</span>
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      </div>`,document.body.appendChild(T),(S=T.querySelector("#bp-close"))==null||S.addEventListener("click",()=>T.remove()),T.querySelectorAll(".bp-item").forEach(L=>{L.addEventListener("click",async()=>{const G=L.dataset.cid,q=E.find(K=>K.cardId===G);if(!q)return;const P=(_[q._line]||[]).find(K=>K.cardId===G);P&&(P.boost=(P.boost||0)+f.boostValue),T.remove(),await j({[p+"Team"]:_,boostUsed:!0})})})}function O(_=null){var se,de;if(!(f.phase===p+"-attack")){x("Remplacement uniquement avant votre attaque","warning");return}const T=f[p+"Team"],S=f["usedSubIds_"+p]||[],L=f.maxSubs||3;if(S.length>=L){x(`Maximum ${L} remplacements atteint`,"warning");return}const G=Object.entries(T).flatMap(([xe,ce])=>(ce||[]).filter(ye=>ye.used).map(ye=>({...ye,_line:xe}))),q=(f[p+"Subs"]||[]).filter(xe=>!S.includes(xe.cardId));if(!G.length){x("Aucun joueur utilisé à remplacer","warning");return}if(!q.length){x("Aucun remplaçant disponible","warning");return}let P=Math.max(0,G.findIndex(xe=>xe.cardId===_));const K=((se=G[P])==null?void 0:se._line)||((de=G[P])==null?void 0:de.job);let ee=Math.max(0,q.findIndex(xe=>xe.job===K)),re=!1;const te=document.createElement("div");te.id="pvp-sub-overlay",te.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function J(){var Oe,Se,We,Ke,nt,Ut;const xe=G[P],ce=q[ee],ye=Math.min(130,Math.round((window.innerWidth-90)/2)),he=Math.round(ye*1.35),ae=Pe=>`background:rgba(255,255,255,0.12);border:none;color:${Pe?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Pe?"default":"pointer"};flex-shrink:0`;te.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      </div>`,(Oe=te.querySelector("#psub-close"))==null||Oe.addEventListener("click",()=>te.remove()),(Se=te.querySelector("#pout-up"))==null||Se.addEventListener("click",()=>{P>0&&(P--,J())}),(We=te.querySelector("#pout-down"))==null||We.addEventListener("click",()=>{P<G.length-1&&(P++,J())}),(Ke=te.querySelector("#pin-up"))==null||Ke.addEventListener("click",()=>{ee>0&&(ee--,J())}),(nt=te.querySelector("#pin-down"))==null||nt.addEventListener("click",()=>{ee<q.length-1&&(ee++,J())});const Ie=(Pe,Xe,Ve,pt)=>{const ot=te.querySelector("#"+Pe);if(!ot)return;let St=0;ot.addEventListener("touchstart",ut=>{St=ut.touches[0].clientY},{passive:!0}),ot.addEventListener("touchend",ut=>{const _t=ut.changedTouches[0].clientY-St;if(Math.abs(_t)<30)return;const rt=Xe();_t<0&&rt<pt-1?(Ve(rt+1),J()):_t>0&&rt>0&&(Ve(rt-1),J())},{passive:!0})};Ie("pin-panel",()=>ee,Pe=>ee=Pe,q.length),Ie("pout-panel",()=>P,Pe=>P=Pe,G.length),(Ut=te.querySelector("#psub-confirm"))==null||Ut.addEventListener("click",async Pe=>{if(Pe.preventDefault(),Pe.stopPropagation(),re)return;re=!0;const Xe=G[P],Ve=q[ee];if(!Xe||!Ve)return;const pt=Xe._line,ot=(T[pt]||[]).findIndex(_t=>_t.cardId===Xe.cardId);if(ot===-1){x("Erreur : joueur introuvable","error"),te.remove();return}const St={...Ve,_line:pt,position:Xe.position,used:!1,boost:0};T[pt].splice(ot,1,St);const ut=[...S,Ve.cardId];te.remove(),Q(Xe,Ve,async()=>{await j({[p+"Team"]:T,[u+"Team"]:f[u+"Team"],["usedSubIds_"+p]:ut})})})}document.body.appendChild(te),J()}function Q(_,E,T){const S={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},L=document.createElement("div");L.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const G=(K,ee,re)=>`<div style="text-align:center">
      <div style="font-size:9px;color:${ee};letter-spacing:2px;text-transform:uppercase;font-weight:700;margin-bottom:6px">${re}</div>
      <div style="width:70px;height:70px;border-radius:50%;background:${S[K.job]||"#555"};border:3px solid ${ee};position:relative;overflow:hidden;margin:0 auto">
      ...
      </div>`,document.body.appendChild(E),(T=E.querySelector("#pvp-hist-close"))==null||T.addEventListener("click",()=>E.remove())}async function R(){if(f.phase!==p+"-attack")return;const _=p==="p1"?"p2":"p1",E=(f.round||0)+1,T=[...f.log||[]];T.push({type:"info",text:`⏭️ ${f[p+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const S=Ue(f),L=De(_),G=S||!L?"finished":_+"-attack";await j({["selected_"+p]:[],modifiers:{...f.modifiers,[p]:{}},pendingAttack:null,phase:G,attacker:_,round:E,log:T}),G==="finished"&&await Le(f)}function le(_){const E=(_.GK||[]).some(S=>!S.used),T=["DEF","MIL","ATT"].some(S=>(_[S]||[]).some(L=>!L.used));return E&&!T}function pe(_){return(_.p1Score||0)!==(_.p2Score||0)?null:le(_.p1Team)&&!$e(_.p2Team)?{side:"p1",gk:(_.p1Team.GK||[]).find(E=>!E.used)}:le(_.p2Team)&&!$e(_.p1Team)?{side:"p2",gk:(_.p2Team.GK||[]).find(E=>!E.used)}:null}async function be(){const _=document.createElement("div");_.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:950;display:flex;align-items:center;justify-content:center;overflow:hidden;text-align:center;padding:24px",document.body.appendChild(_);const E=(T,S)=>new Promise(L=>{_.innerHTML=`<div style="font-size:32px;font-weight:900;color:${S};letter-spacing:2px;animation:lcFade 1.4s ease both">${T}</div>
      <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(L,1400)});await E("⚽ DERNIER CORNER","#FFD700"),await E("🧤 LE GARDIEN MONTE !","#4fc3f7"),_.remove()}async function we(_,E){const{side:T,gk:S}=_;if(!S)return;const L=T+"Score",G=(E[L]||0)+1;S.used=!0;const q=[...E.log||[]];q.push({type:"duel",isGoal:!0,homeScored:T===p,homePlayers:T==="p1"?[qt(S)]:[],aiPlayers:T==="p2"?[qt(S)]:[],text:`⚽ DERNIER CORNER — Le gardien de ${E[T+"Name"]} marque !`});const P=(E.round||0)+1,K=T==="p1"?"p2":"p1",ee={...E,[L]:G},re=Ue(ee);D.add(P),await be();const te=T===p?G:E[p+"Score"]||0,J=T===p?E[u+"Score"]||0:G;Ze([qt(S)],te,J,T===p,async()=>{await j({[L]:G,log:q,round:P,pendingAttack:null,phase:re?"finished":K+"-attack",attacker:K,["selected_"+p]:[],modifiers:{...f.modifiers,[p]:{}}}),re&&await Le({...f,[L]:G})})}async function Te(){const _=f[p+"Team"],E=!["GK","DEF","MIL","ATT"].some(G=>(f[u+"Team"][G]||[]).some(q=>!q.used)),T=(f["selected_"+p]||[]).map(G=>{const q=(_[G._role]||[]).find(de=>de.cardId===G.cardId)||G,P=["GK","DEF"].includes(G._role),K=_[G._role]||[],ee=K.findIndex(de=>de.cardId===G.cardId),re=xt(K.length),te=ee>=0?re[ee]:q._col??1,J=f.stadiumDef||f.homeStadiumDef||null,se=q.stadiumBonus||J&&(J.club_id&&String(q.club_id)===String(J.club_id)||J.country_code&&q.country_code===J.country_code)||!1;return{...q,_line:G._role,_col:te,stadiumBonus:se,...P?{note_a:Math.max(1,Number(q.note_a)||0)}:{}}});if(!T.length)return;const S=Qt(T,f.modifiers[p]||{},f[p+"Formation"]);me(p,T.map(G=>G.cardId)),T.forEach(G=>{const q=(_[G._role]||[]).find(P=>P.cardId===G.cardId);q&&(q.used=!0)}),f["selected_"+p]=[],W();const L=[...f.log||[]];if(E){if(T.length===1&&(T[0]._line==="GK"||T[0].job==="GK")&&(f.p1Score||0)===(f.p2Score||0)){await we({side:p,gk:T[0]},{...f,[p+"Team"]:_,log:L});return}const q=(f[p+"Score"]||0)+1,P=T.map(J=>qt(J));L.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:P,homeTotal:S.total,aiTotal:0});const K=(f.round||0)+1,ee=p==="p1"?"p2":"p1",re={...f,[p+"Team"]:_,[p+"Score"]:q},te=Ue(re);D.add(K),Ze(P,q,f[u+"Score"]||0,!0,async()=>{await j({[p+"Team"]:_,[p+"Score"]:q,["selected_"+p]:[],modifiers:{...f.modifiers,[p]:{}},pendingAttack:null,phase:te?"finished":ee+"-attack",attacker:ee,round:K,log:L}),te&&await Le({...f,[p+"Score"]:q})});return}L.push({type:"pending",text:`⚔️ ${f[p+"Name"]} attaque (${S.total})`}),await j({[p+"Team"]:_,[u+"Team"]:f[u+"Team"],pendingAttack:{...S,players:T,side:p},["selected_"+p]:[],modifiers:{...f.modifiers,[p]:{}},phase:u+"-defense",log:L})}async function Ae(){const _=f[p+"Team"],E=(f["selected_"+p]||[]).map(ce=>{const ye=(_[ce._role]||[]).find(Se=>Se.cardId===ce.cardId)||ce,he=_[ce._role]||[],ae=he.findIndex(Se=>Se.cardId===ce.cardId),Ie=xt(he.length),Oe=ae>=0?Ie[ae]:ye._col??1;return{...ye,_line:ce._role,_col:Oe}}),T=Zt(E,f.modifiers[p]||{},f[p+"Formation"]);me(p,E.map(ce=>ce.cardId)),E.forEach(ce=>{const ye=(_[ce._role]||[]).find(he=>he.cardId===ce.cardId);ye&&(ye.used=!0)}),f["selected_"+p]=[],W();const S=Gi(f.pendingAttack.total,T.total,f.modifiers[p]||{}),L=f.pendingAttack.side,G=S==="attack"||(S==null?void 0:S.goal),q=L==="p1"?"p2":"p1",P=(f.round||0)+1,K=(f.pendingAttack.players||[]).map(ce=>qt(ce)),ee=[...f.log||[]];ee.push({type:"duel",isGoal:G,homeScored:G&&L===p,text:G?`⚽ BUT de ${f[L+"Name"]} ! (${f.pendingAttack.total} vs ${T.total})`:`✋ Attaque stoppée (${f.pendingAttack.total} vs ${T.total})`,homePlayers:K,aiPlayers:E.map(ce=>qt(ce)),homeTotal:f.pendingAttack.total,aiTotal:T.total});const re=G?(f[L+"Score"]||0)+1:f[L+"Score"]||0,te={...f,[p+"Team"]:_,[L+"Score"]:re},J=Ue(te),se=J?"finished":q+"-attack",de=!J&&!G?pe(te):null,xe=async()=>{if(de){await we(de,te);return}await j({[p+"Team"]:_,[u+"Team"]:f[u+"Team"],[L+"Score"]:re,["selected_"+p]:[],modifiers:{...f.modifiers,[p]:{}},pendingAttack:null,phase:se,attacker:q,round:P,log:ee}),(se==="finished"||J)&&await Le({...f,[L+"Score"]:re})};if(G){const ce=L===p,ye=ce?re:f[p+"Score"]||0,he=ce?f[u+"Score"]||0:re;D.add(P),Ze(K,ye,he,ce,xe)}else await xe()}function ke(_){return["MIL","ATT"].some(E=>(_[E]||[]).some(T=>!T.used))}function $e(_){return["GK","DEF","MIL","ATT"].some(E=>(_[E]||[]).some(T=>!T.used))}function je(_){return $e(_)&&!ke(_)}function De(_){const E=f[_+"Team"],T=f[(_==="p1"?"p2":"p1")+"Team"];return!!(ke(E)||je(E)&&!$e(T)||je(E)&&!ke(T)&&$e(T))}function Ue(_){return!$e(_.p1Team)&&!$e(_.p2Team)}function Ee(_){const E=_.p1Score||0,T=_.p2Score||0;return E===T?null:E>T?h.home_id:h.away_id}async function Le(_){try{const E=Ee(_),T=E?h.home_id===E?h.away_id:h.home_id:null,S=_.p1Score||0,L=_.p2Score||0,{data:G,error:q}=await v.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:E,p_home_score:S,p_away_score:L,p_forfeit:!1});if(q&&console.error("[PvP] finish_pvp_match:",q),l)try{await v.rpc("check_and_finish_mini_league",{p_league_id:l})}catch(P){console.error("[PvP] check_and_finish_mini_league:",P)}E&&T&&ho(E,T).catch(()=>{});try{const{data:P}=await v.rpc("apply_match_rewards",{p_match_id:i});P!=null&&P.success&&!(P!=null&&P.skipped)&&typeof t.refreshProfile=="function"&&await t.refreshProfile()}catch(P){console.error("[PvP] apply_match_rewards:",P)}if(typeof c=="function")try{await c({homeScore:S,awayScore:L,winnerId:E,homeId:h.home_id,awayId:h.away_id,matchId:i})}catch(P){console.error("[PvP] onMatchEnd:",P)}}catch(E){console.error("[PvP] finishMatch:",E)}}function Fe(){var L;const _=f[p+"Score"],E=f[u+"Score"],T=_>E,S=_===E;dt(e),e.innerHTML=`
    ...
    </div>`,(L=document.getElementById("pvp-home"))==null||L.addEventListener("click",()=>{try{v.removeChannel(Z)}catch{}it(e),l?m("mini-league",{openLeagueId:l}):m("home")})}W()}async function Mi(e,t,i=!1){var s,d;const{state:n,navigate:r,toast:o}=t,a=i&&((d=(s=t==null?void 0:t.state)==null?void 0:s.params)==null?void 0:d.rankedData)||null;await mi(e,t,i?"ranked":"random",async({deckId:c,formation:l,starters:g,subsRaw:y,gcCardsEnriched:m,gcDefs:x,stadiumDef:p})=>{dt(e);const u=async b=>{await hr(e,t,c,l,g,y,b||[],x,p,i,a)};if(!(m!=null&&m.length)){await u([]);return}fi(e,m,u)})}async function br(e,t,i){return yr(e,t,i)}async function hr(e,t,i,n,r,o,a=[],s=[],d=null,c=!1,l=null){var ne,Z;const{state:g,navigate:y,toast:m}=t,x=(l==null?void 0:l.mmr)??((ne=g.profile)==null?void 0:ne.mmr)??1e3,p=c?Di(x):null,u=c?"linear-gradient(135deg, #1a0a2e 0%, #2d1060 50%, #1a0a2e 100%)":"linear-gradient(135deg, #0a1a2e 0%, #0d3d1e 100%)",b=c?(p==null?void 0:p.color)||"#D4A017":"#FFD700";let w=!1,h=null,$=null;e.innerHTML=`
    ...
    </div>`;const f=async()=>{w=!0,h&&(h.unsubscribe(),h=null),$&&(clearInterval($),$=null);try{await v.rpc("cancel_matchmaking",{p_user_id:g.profile.id})}catch(j){console.error("[Matchmaking] cancel error:",j)}};(Z=document.getElementById("cancel-mm"))==null||Z.addEventListener("click",async()=>{await f(),it(e),y("home")});const C=async(j,z)=>{w||(w=!0,h&&(h.unsubscribe(),h=null),$&&(clearInterval($),$=null),await At(e,t,j,z,{myGC:a,gcDefs:s,stadiumDef:d,isRanked:c,rankedData:l,onMatchEnd:c?vr:null}))},k=g.user.id,{data:I,error:D}=c?await v.rpc("try_matchmake_ranked",{p_user_id:k,p_deck_id:i,p_mmr:x}):await v.rpc("try_matchmake",{p_user_id:k,p_deck_id:i});if(D||!(I!=null&&I.success)){console.error("[Matchmaking] try_matchmake error:",D||I),m("Erreur de matchmaking","error"),it(e),y("home");return}if(I.matched){await C(I.match_id,!1);return}const ie=async()=>{if(w)return;const{data:j}=await v.from("matchmaking_queue").select("status, match_id").eq("user_id",k).maybeSingle();(j==null?void 0:j.status)==="matched"&&j.match_id&&await C(j.match_id,!0)};h=v.channel(`mm_${k}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${k}`},j=>{const z=j.new;z.status==="matched"&&z.match_id&&C(z.match_id,!0)}).subscribe(),$=setInterval(ie,3e3),setTimeout(()=>{w||f()},12e4)}async function vr(e,t){const{state:i,toast:n}=t,{winnerId:r,homeId:o,awayId:a,homeScore:s,awayScore:d,matchId:c}=e,l=i.user.id,g=o===l,y=r===l,{data:m}=await v.from("users").select("mmr, mmr_rd, mmr_v").eq("id",l).single(),{data:x}=await v.from("users").select("mmr, mmr_rd, mmr_v").eq("id",g?a:o).single();if(m&&x){const p=Kn(m.mmr,m.mmr_rd,m.mmr_v,x.mmr,x.mmr_rd,y?1:0);await v.from("users").update({mmr:p.mmr,mmr_rd:p.rd,mmr_v:p.v,mmr_wins:y?v.sql`mmr_wins + 1`:void 0,mmr_losses:y?void 0:v.sql`mmr_losses + 1`}).eq("id",l),n(y?`+MMR ↑ ${Math.round(p.mmr-m.mmr)}`:`-MMR ↓ ${Math.round(m.mmr-p.mmr)}`,y?"success":"error",4e3)}}const wr=Object.freeze(Object.defineProperty({__proto__:null,renderMatchRandom:Mi,resumePvpMatch:br},Symbol.toStringTag,{value:"Module"}));async function _r(e,t){const{state:i,navigate:n,toast:r}=t,o=i.params||{},a=o.friendId||null,s=o.friendName||"Ami",d=!!o.isAccepting;await mi(e,t,"friend",async({deckId:c,formation:l,starters:g,subsRaw:y,gcCardsEnriched:m,gcDefs:x,stadiumDef:p})=>{dt(e);const u=async b=>{const w=b||[];d?await $r(e,t,c,w,x,p):await kr(e,t,c,w,x,p,a,s)};if(!(m!=null&&m.length)){await u([]);return}fi(e,m,u)})}async function kr(e,t,i,n,r,o,a,s){var w;const{state:d,navigate:c,toast:l}=t,g=d.user.id,{data:y,error:m}=await v.from("friend_match_invites").insert({inviter_id:g,invitee_id:a,friend_id:a,status:"pending",inviter_deck_id:i,expires_at:new Date(Date.now()+2*60*1e3).toISOString()}).select("id").single();if(m||!y){console.error("[Friend] Erreur création invitation:",m),l("Impossible de créer l'invitation","error"),c("home");return}e.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    </div>`,document.body.appendChild(m),m.querySelector("#lv-ok").onclick=()=>{m.remove(),y(!0)},m.querySelector("#lv-cancel").onclick=()=>{m.remove(),y(!1)}}))return;const{data:d,error:c}=await v.rpc("leave_mini_league",{p_league_id:i,p_user_id:a});if(c||!(d!=null&&d.success)){r((d==null?void 0:d.error)||"Erreur lors de la désinscription","error");return}const l=d.refund||n;o.profile&&(o.profile.credits=(o.profile.credits||0)+l);const g=document.getElementById("nav-credits");g&&(g.textContent=`💰 ${(o.profile.credits||0).toLocaleString("fr")}`),r(`↩️ Désinscrit · +${l.toLocaleString("fr")} cr. remboursés`,"success"),tt(e,t,"waiting")}async function ji(e,t,i,n,r){const{toast:o}=t;if(!confirm(`Supprimer définitivement "${n}" et tous ses matchs/membres ? Action irréversible.`))return;await v.from("mini_league_matches").delete().eq("league_id",i),await v.from("mini_league_members").delete().eq("league_id",i);const{error:a}=await v.from("mini_leagues").delete().eq("id",i);if(a){o("Erreur suppression ("+a.message+")","error");return}o("Mini League supprimée avec succès","success"),tt(e,t,r)}async function jr(e,t,i){await v.from("mini_leagues").update({is_archived:!0}).eq("id",i),t.toast("Mini League archivée","success"),tt(e,t,"archived")}async function ct(e,t,i){var $,f,C,k,I,D,ie,ne,Z;const{state:n,toast:r}=t,o=n.profile.id,[{data:a},{data:s},{data:d}]=await Promise.all([v.from("mini_leagues").select("*").eq("id",i).single(),v.from("mini_league_members").select("*, user:users(id,pseudo,club_name,club_color1,club_color2)").eq("league_id",i),v.from("mini_league_matches").select("*").eq("league_id",i).order("matchday").order("created_at")]);if(!a){r("Introuvable","error"),await tt(e,t);return}const c=(s||[]).some(j=>j.user_id===o),l=a.creator_id===o,g=(s||[]).map(j=>j.user).filter(Boolean),y=Fr(g,d||[]),m=(d||[]).filter(j=>j.matchday===a.current_day),x=a.pot||0,p=a.entry_fee||100,u=m.length>0&&m.every(j=>j.status==="finished"||j.status==="bye"),b=a.current_day>=a.total_days,w=(s||[]).find(j=>j.user_id===o);e.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:${Kt};border:1px solid ${Vt};border-radius:12px;padding:16px">
        ...
        ${Array.from({length:Math.max(0,a.status==="active"?a.current_day-1:a.current_day)},(j,z)=>z+1).reverse().map(j=>{const z=(d||[]).filter(Y=>Y.matchday===j);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:${et};margin-bottom:6px">Journée ${j}</div>${z.map(Y=>un(Y,g,o,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
    ...
    </div>`;let l=!1;const g=async u=>{if(!l){if(l=!0,clearInterval(x),m)try{m.unsubscribe()}catch{}u&&await At(e,t,u,r,{mlLeagueId:n,mlMatchId:i,myGC:o||[],gcDefs:a,stadiumDef:s}),d()}};(p=document.getElementById("wait-back"))==null||p.addEventListener("click",()=>{if(l=!0,clearInterval(x),m)try{m.unsubscribe()}catch{}c("mini-league",{openLeagueId:n}),d()});const y=async()=>{if(l)return;const{data:u}=await v.from("mini_league_matches").select("match_id").eq("id",i).single();u!=null&&u.match_id&&g(u.match_id)},m=v.channel(`ml_match_${i}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"mini_league_matches",filter:`id=eq.${i}`},u=>{var b;(b=u.new)!=null&&b.match_id&&g(u.new.match_id)}).subscribe(),x=setInterval(y,3e3);y()})}const Gr="/",Rr=[{emoji:"⚽",title:"Bienvenue dans Manager Wars !",color:"#1A6B3C",content:`<p>Tu es désormais un <strong>manager de football</strong> virtuel.</p>
    ...
    <p style="margin-top:12px;font-size:13px;color:#888">Tu peux revoir ce tutoriel depuis les paramètres à tout moment.</p>`}];function Nr(e,t,i){let n=0;const r=document.createElement("div");r.id="tutorial-overlay",r.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.65);z-index:9900;display:flex;align-items:center;justify-content:center;padding:16px";const o=()=>{var l,g,y;const s=t[n],d=n===t.length-1,c=Math.round((n+1)/t.length*100);r.innerHTML=`
  ...
  <div style="min-height:100%;background:${w[x.id]};padding:16px;overflow-y:auto;display:flex;flex-direction:column;gap:16px">
    ...
    <div style="display:flex;flex-direction:column;gap:10px;margin-top:auto;padding-top:8px">
      ...
      away:users!away_id(pseudo,club_name)`).or(`home_id.eq.${r.id},away_id.eq.${r.id}`).order("created_at",{ascending:!1}).limit(50),a={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},s=(o||[]).filter(l=>l.status==="finished"),d=(o||[]).filter(l=>l.status==="in_progress");function c(l){const g=l.home_id===r.id;g?l.home_score:l.away_score,g?l.away_score:l.home_score;const y=l.winner_id===r.id,m=l.home_score===l.away_score&&l.status==="finished",x=l.status!=="finished"?"…":m?"N":y?"V":"D",p=l.status!=="finished"||m?"#888":y?"#1A6B3C":"#c0392b";let u=a[l.mode]||l.mode;l.away_id===null&&!u.startsWith("IA")&&(u="IA");const w=l.home_id===r.id?l.away:l.home;let h;l.away_id===null?h=u:w?h=`${w.club_name||w.pseudo} (${w.pseudo})`:h="Adversaire";let $="";l.status==="in_progress"&&Date.now()-new Date(l.created_at).getTime()>3600*1e3&&($=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const f=new Date(l.created_at),C=f.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+f.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),k=l.status==="finished"?`${l.home_score} - ${l.away_score}`:`${l.home_score||0} - ${l.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
  ...
  <div class="page">
    ...
    <nav class="top-nav">
      <div class="logo" id="nav-logo" title="Manager Wars v2026.07.20-2340" style="cursor:pointer">
