...
import{s as _,q as Ut,t as Oi,F as Hi,o as tt,n as ii,g as At,r as Ie,m as zt,u as Mi,j as Ui,v as so,w as lo,T as Ti,p as co,x as po,d as uo}from"./special-cards-DZxbxWIu.js";const fo="/";function cn(e,{navigate:t,toast:i}){let n="login";const r=()=>{var a,s,d,c,l,m,b;const o=n==="login";if(e.innerHTML=`
    <div style="
    ...
    ">
      ...
      <div style="margin-bottom:28px;text-align:center;animation:authFadeIn .5s ease">
        <img src="${fo}icons/logo-withname.png" alt="Manager Wars" style="height:72px;width:auto;filter:drop-shadow(0 4px 16px rgba(212,160,23,0.4))">
  ...
  `;function s(){var g;const c=document.getElementById("logo-preview"),l=document.getElementById("logo-initials"),m=((g=document.getElementById("setup-club"))==null?void 0:g.value)||"MW",b=m.trim().split(" ").filter(Boolean),y=b.length>=2?(b[0][0]+b[1][0]).toUpperCase():m.slice(0,2).toUpperCase();c&&(c.style.background=a,c.style.borderColor=o),l&&(l.textContent=y,l.style.color=o)}document.getElementById("color1").addEventListener("input",c=>{o=c.target.value,document.getElementById("swatch1").style.background=o,s()}),document.getElementById("color2").addEventListener("input",c=>{a=c.target.value,document.getElementById("swatch2").style.background=a,s()});function d(c){document.querySelectorAll(".setup-step").forEach(l=>l.classList.remove("active")),document.getElementById(`step-${c}`).classList.add("active"),document.getElementById("step-num").textContent=c,document.getElementById("progress-fill").style.width=`${Math.round(c/3*100)}%`,c===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("step1-error");if(l.textContent="",c.length<3){l.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:m}=await _.from("users").select("id").eq("pseudo",c).maybeSingle();if(m){l.textContent="Ce pseudo est déjà pris.";return}d(2)}),document.getElementById("step2-back").addEventListener("click",()=>d(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const c=document.getElementById("setup-club").value.trim(),l=document.getElementById("step2-error");if(l.textContent="",c.length<2){l.textContent="Nom trop court (min. 2 caractères).";return}const{data:m}=await _.from("users").select("id").eq("club_name",c).maybeSingle();if(m){l.textContent="Ce nom de club est déjà pris.";return}d(3)}),document.getElementById("step3-back").addEventListener("click",()=>d(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("setup-club").value.trim(),m=document.getElementById("step3-error"),b=document.getElementById("step3-finish");m.textContent="",b.disabled=!0,b.textContent="Création en cours…";try{const{error:y}=await _.from("users").insert({id:t.user.id,pseudo:c,club_name:l,club_color1:o,club_color2:a,credits:1e4});if(y)throw y;await go(t.user.id),await r(),n(`Bienvenue ${c} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(y){m.textContent=y.message,b.disabled=!1,b.textContent="🚀 Créer mon profil !"}})}async function go(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await _.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const yo="modulepreload",xo=function(e){return"/"+e},pn={},Tn=function(t,i,n){let r=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),s=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));r=Promise.allSettled(i.map(d=>{if(d=xo(d),d in pn)return;pn[d]=!0;const c=d.endsWith(".css"),l=c?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${l}`))return;const m=document.createElement("link");if(m.rel=c?"stylesheet":yo,c||(m.as="script"),m.crossOrigin="",m.href=d,s&&m.setAttribute("nonce",s),document.head.appendChild(m),c)return new Promise((b,y)=>{m.addEventListener("load",b),m.addEventListener("error",()=>y(new Error(`Unable to preload CSS for ${d}`)))})}))}function o(a){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=a,window.dispatchEvent(s),!s.defaultPrevented)throw a}return r.then(a=>{for(const s of a||[])s.status==="rejected"&&o(s.reason);return t().catch(o)})},ni="#1A6B3C",oi="#cc2222",bo="#D4A017",un="#888";async function vo(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    </div>`,await In(i,n,t),document.getElementById("btn-add-friend").addEventListener("click",()=>_o(i,n)),document.getElementById("btn-accept-friend").addEventListener("click",()=>Sn(i,n,null,t))}async function In(e,t,i={}){const{navigate:n}=i,r=e.user.id,{data:o,error:a}=await _.from("friendships").select("id, requester_id, addressee_id").eq("status","accepted").or(`requester_id.eq.${r},addressee_id.eq.${r}`),{count:s}=await _.from("friendships").select("id",{count:"exact",head:!0}).eq("addressee_id",r).eq("status","pending"),d=document.getElementById("pending-badge");d&&(s>0?(d.style.display="flex",d.textContent=s):d.style.display="none");const c=document.getElementById("friends-list");if(!c)return;if(a){console.error("[Friends] Erreur:",a),c.innerHTML=`<div style="color:${oi};text-align:center;padding:16px">Erreur chargement : ${a.message}</div>`;return}const l=(o||[]).map(y=>y.requester_id===r?y.addressee_id:y.requester_id);let m={};if(l.length){const{data:y}=await _.from("users").select("id, pseudo, club_name, last_seen_at, club_color1, club_color2").in("id",l);(y||[]).forEach(g=>{m[g.id]=g})}const b=(o||[]).map(y=>({friendshipId:y.id,friend:m[y.requester_id===r?y.addressee_id:y.requester_id]||{pseudo:"?"}}));if(!b.length){c.innerHTML=`
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      ${b.map(({friendshipId:y,friend:g})=>ho(g,y)).join("")}
    </div>`,c.querySelectorAll("[data-stats]").forEach(y=>{y.addEventListener("click",()=>wo(e,y.dataset.stats,y.dataset.friendName))}),c.querySelectorAll("[data-match]").forEach(y=>{y.addEventListener("click",async()=>{const g=y.dataset.friendId,p=y.dataset.friendName;if(typeof n!="function"){t("Erreur navigation","error");return}const u=e.user.id,{data:x}=await _.from("friend_match_invites").select("id").eq("inviter_id",g).eq("invitee_id",u).eq("status","pending").order("created_at",{ascending:!1}).limit(1).maybeSingle(),h=!!x;console.log("[Friends] clic match",{fid:g,fname:p,isAccepting:h}),n("match",{matchMode:"friend",friendId:g,friendName:p,isAccepting:h})})})}function ho(e,t){const i=e.club_name||e.pseudo||"?",n=e.pseudo||"",r=(n||i).slice(0,2).toUpperCase(),o=e.club_color2||ni,a=e.club_color1||"#ffffff",s=e.last_seen_at?new Date(e.last_seen_at):null,d=s&&Date.now()-s.getTime()<3*60*1e3;return`
    <div style="display:flex;align-items:center;gap:12px;background:var(--tile-bg);border-radius:12px;padding:12px 14px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
      ...
      <div style="display:flex;gap:8px;flex-shrink:0">
        ...
        <button data-stats="${e.id}" data-friend-name="${i}" title="Voir les stats"
          style="width:38px;height:38px;border-radius:50%;border:2px solid ${bo};background:var(--tile-bg);font-size:16px;cursor:pointer;display:flex;align-items:center;justify-content:center">📊</button>
      </div>
    </div>`}function _o(e,t){const i=Ki();i.innerHTML=`
    <div class="popup-box">
    ...
    ${Vi()}`,document.body.appendChild(c);const m=()=>c.remove();c.querySelector("#pending-close").addEventListener("click",m),c.addEventListener("click",b=>{b.target===c&&m()}),c.querySelectorAll("[data-accept]").forEach(b=>{b.addEventListener("click",async()=>{const{error:y}=await _.from("friendships").update({status:"accepted"}).eq("id",b.dataset.accept);if(y){t("Erreur : "+y.message,"error");return}b.closest("div[style]").remove(),t("✅ Ami accepté !","success"),In(e,t,n),i&&i()})}),c.querySelectorAll("[data-decline]").forEach(b=>{b.addEventListener("click",async()=>{await _.from("friendships").delete().eq("id",b.dataset.decline),b.closest("div[style]").remove(),t("Demande refusée","info"),i&&i()})})}async function wo(e,t,i){const n=e.user.id,[r,o]=[n,t].sort(),a=n===r,{data:s}=await _.from("friend_match_stats").select("*").eq("player1_id",r).eq("player2_id",o).single(),d=e.profile.club_name||e.profile.pseudo||"Moi",c=s||{},l=a?c.wins_p1||0:c.wins_p2||0,m=a?c.wins_p2||0:c.wins_p1||0,b=c.draws||0,y=a?c.goals_p1||0:c.goals_p2||0,g=a?c.goals_p2||0:c.goals_p1||0,p=a?c.gc_used_p1||0:c.gc_used_p2||0,u=a?c.gc_used_p2||0:c.gc_used_p1||0,x=c.matches_total||0,h=($,f,z,k=ni,S=oi)=>`
    ...
    </style>`}const An="mw_sound_volume";function Yi(){const e=localStorage.getItem(An);if(e===null)return 100;const t=parseInt(e,10);return Number.isFinite(t)?Math.max(0,Math.min(100,t)):100}function ko(e){localStorage.setItem(An,String(Math.max(0,Math.min(100,Math.round(e)))))}function Wi(){return Yi()===0}function mi(e){return Math.max(0,Math.min(1,e*(Yi()/100)))}function Xi(e,t=1){if(Wi())return null;try{const i=new Audio(e);return i.volume=mi(t),i.play().catch(()=>{}),i}catch{return null}}let gt=null,Ci=null,fn=.3;function zn(e,t=.3){if(fn=t,gt&&Ci===e&&!gt.paused){gt.volume=mi(fn);return}if(Gt(),!Wi())try{const i=new Audio(e);i.loop=!0,i.volume=mi(t),i.play().catch(()=>{}),gt=i,Ci=e}catch{}}function Gt(){if(gt)try{gt.pause(),gt.currentTime=0}catch{}gt=null,Ci=null}let Jt=null;function Mn(e,t=.6){if(Ue(),!Wi())try{const i=new Audio(e);i.volume=mi(t),i.play().catch(()=>{}),Jt=i}catch{}}function Ue(){if(Jt)try{Jt.pause(),Jt.currentTime=0}catch{}Jt=null}const $o="2026.07.22-1704";async function mn(e,{state:t,navigate:i,toast:n}){var o,a;const r=t.profile;r&&(e.innerHTML=`
  <style>
    .home-dark {
      min-height: 100%;
  ...
  <div class="home-dark" id="home-dark">
    ...
    </div>`}).join(""),n.querySelectorAll("[data-resume]").forEach(d=>{d.addEventListener("click",async()=>{const c=document.getElementById("page-content")||document.getElementById("app");if(d.dataset.mini==="1"){const{data:l}=await _.from("mini_league_matches").select("id, league_id").eq("match_id",d.dataset.resume).single();l?i("match-mini-league",{mlMatchId:l.id,leagueId:l.league_id}):i("mini-league")}else{const{resumePvpMatch:l}=await Tn(async()=>{const{resumePvpMatch:m}=await Promise.resolve().then(()=>Cr);return{resumePvpMatch:m}},void 0);l(c,{state:e,navigate:i,toast:t,openModal:null,closeModal:null,refreshProfile:null},d.dataset.resume)}})}),n.querySelectorAll("[data-abandon]").forEach(d=>{d.addEventListener("click",()=>{To(async()=>{await Lo(d.dataset.abandon,d.dataset.opp,r),t("Match abandonné (défaite 3-0)","info"),Cn(e,t,i)})})})}async function Lo(e,t,i){Gt();const{data:n}=await _.from("matches").select("home_id, away_id, game_state, mode").eq("id",e).single();if(!n)return;const r=n.home_id===i,o=r?0:3,a=r?3:0,s=n.game_state||{};s.p1Score=o,s.p2Score=a,s.phase="finished",s.forfeit=!0,await _.from("matches").update({status:"finished",forfeit:!0,winner_id:t,home_score:o,away_score:a,game_state:s}).eq("id",e),n.mode==="mini_league"&&await _.from("mini_league_matches").update({status:"finished",home_score:o,away_score:a}).eq("match_id",e)}function To(e){const t=document.createElement("div");t.className="modal-overlay",t.innerHTML=`<div class="modal" style="max-width:340px">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
  ...
  </div>`,e.querySelector("#settings-back").addEventListener("click",()=>i("home")),e.querySelectorAll("[data-theme-choice]").forEach(d=>{d.addEventListener("click",()=>{ca(d.dataset.themeChoice),Bn(e,t)})});const o=e.querySelector("#volume-slider"),a=e.querySelector("#volume-label");let s=null;o.addEventListener("input",()=>{ko(Number(o.value)),a.textContent=`${o.value}%`,s&&(s.volume=Math.max(0,Math.min(1,Number(o.value)/100)))}),e.querySelector("#volume-test").addEventListener("click",()=>{s=Xi("/sounds/match-opening.mp3",1)}),e.querySelector("#settings-logout").addEventListener("click",async()=>{await _.auth.signOut(),window.location.reload()})}const je={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé pour ce match."},"Double attaque":{icon:"⚡",desc:"La note d'attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la note d'un joueur adverse."},Gel:{icon:"❄️",desc:"Bloque le meilleur attaquant IA."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function Pe(e,t){if(!e)return 0;let i=0;switch(t){case"GK":i=Number(e.note_g)||0;break;case"DEF":i=Number(e.note_d)||0;break;case"MIL":i=Number(e.note_m)||0;break;case"ATT":i=Number(e.note_a)||0;break;default:i=0}return i+(e.boost||0)}const gn=["ATT","MIL","DEF","GK"];function qn(e,t,i){if(!e||!t)return!1;if(i&&e.position&&t.position)return Oi(i).some(([l,m])=>l===e.position&&m===t.position||l===t.position&&m===e.position);const n=e._col!=null&&t._col!=null&&e._col===t._col,r=e._col!=null&&t._col!=null&&Math.abs(e._col-t._col)===1,o=gn.indexOf(e._line||e.job),a=gn.indexOf(t._line||t.job),s=Math.abs(o-a)===1;return(e._line||e.job)===(t._line||t.job)&&r||n&&s}function Dn(e,t){let i=0;const n=e.length;for(let r=0;r<n;r++)for(let o=r+1;o<n;o++){const a=e[r],s=e[o];if(!a||!s||!qn(a,s,t))continue;const d=Ut(a,s);d==="#00ff88"?i+=10:d==="#FFD700"&&(i+=5)}return i}function ri(e,t={},i){let n=0,r=0;e.forEach(d=>{const c=d._line||d.job,l=Number(c==="MIL"?d.note_m:d.note_a)||0,m=d.stadiumBonus&&(c==="MIL"||c==="ATT")?10:0;n+=t.doubleAttack?l*2:l,r+=(d.boost||0)+m});const o=n+r,a=Dn(e,i);let s=o+a;return t.stolenNote&&(s-=t.stolenNote),{base:o,links:a,total:Math.max(0,s)}}function ai(e,t={},i){const n=e.reduce((a,s)=>{const d=s._line||s.job;let c=0;d==="GK"?c=Number(s.note_g)||0:d==="MIL"?c=Number(s.note_m)||0:c=Number(s.note_d)||0;const l=s.stadiumBonus&&(d==="GK"||d==="DEF"||d==="MIL")?10:0;return a+c+(s.boost||0)+l},0),r=Dn(e,i);let o=n+r;return t.stolenNote&&(o-=t.stolenNote),{base:n,links:r,total:Math.max(0,o)}}function Ji(e,t,i={}){return i.shield?{goal:!1,shielded:!0}:{goal:e>t,shielded:!1}}function Fn(e,t,i="easy"){const n=e.filter(a=>!a.used);if(!n.length)return[];const r=[...n].sort((a,s)=>{const d=t==="attack"?Pe(a,"ATT"):a._line==="GK"?Pe(a,"GK"):Pe(a,"DEF");return(t==="attack"?Pe(s,"ATT"):s._line==="GK"?Pe(s,"GK"):Pe(s,"DEF"))-d});let o=i==="easy"?1+Math.floor(Math.random()*2):i==="medium"?2+Math.floor(Math.random()*2):3;return r.slice(0,Math.min(o,r.length,3))}function Ao(e,t){const i={vs_ai_easy:{victoire:500,nul:250,defaite:50},vs_ai_medium:{victoire:1e3,nul:500,defaite:50},vs_ai_hard:{victoire:1500,nul:750,defaite:100},vs_ai_club:{victoire:2500,nul:1250,defaite:100}};return(i[e]||i.vs_ai_easy)[t]||0}const zo={pepite:{win:2,loss:-1,boosterStart:"min"},papyte:{win:1,loss:-2,boosterStart:"max"}};function Mo(e,t){const i=e.player;if(!i)return 0;const n=i.rarity;if(n!=="pepite"&&n!=="papyte")return Number(i[t])||0;const r=Pn(i),o=Number(i[t])||0;if(o<=0)return 0;const a=i.note_min??1,s=i.note_max??10,c=(e.current_note??r)-r;return Math.min(s,Math.max(a,o+c))}function Pn(e){const t=e.job||"ATT";return Number(t==="GK"?e.note_g:t==="DEF"?e.note_d:t==="MIL"?e.note_m:e.note_a)||0}async function Co(e,t){!e||!t||await Promise.all([yn(e,"win"),yn(t,"loss")])}async function yn(e,t){const{data:i}=await _.from("cards").select("id, current_note, player:players(rarity, job, note_g, note_d, note_m, note_a, note_min, note_max)").eq("owner_id",e).eq("card_type","player");if(!(i!=null&&i.length))return;const n=i.filter(r=>{var o,a;return((o=r.player)==null?void 0:o.rarity)==="pepite"||((a=r.player)==null?void 0:a.rarity)==="papyte"});n.length&&await Promise.all(n.map(r=>{const o=zo[r.player.rarity],a=t==="win"?o.win:o.loss,s=r.player.note_min??1,d=r.player.note_max??10,c=Pn(r.player),l=r.current_note??c,m=Math.min(d,Math.max(s,l+a));return _.from("cards").update({current_note:m}).eq("id",r.id)}))}const Ii={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"},li={GK:"#111111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},ci=["GK","DEF","MIL","ATT"],jo=["Tous","GK","DEF","MIL","ATT"],Bo={normal:1e3,pepite:5e3,papyte:5e3,legende:1e4};function xn(e){const t=e.player;if(!t)return null;const i=t.rarity;return(i==="pepite"||i==="papyte")&&e.current_note!=null?e.current_note:Math.max(Number(t.note_g)||0,Number(t.note_d)||0,Number(t.note_m)||0,Number(t.note_a)||0)}function qo(e){return e.length?e.reduce((t,i)=>xn(i)>xn(t)?i:t,e[0]):e[0]}function Do(e){return e==="GK"?"note_g":e==="DEF"?"note_d":e==="MIL"?"note_m":"note_a"}const Fo={MA:"MAROC",FR:"FRANCE",AR:"ARGENTINE",PT:"PORTUGAL",BR:"BRESIL",ES:"ESPAGNE",DE:"ALLEMAGNE",GB:"ANGLETERRE",IT:"ITALIE",CM:"CAMEROUN",SN:"SENEGAL",NG:"NIGERIA",DK:"DANEMARK",NL:"PAYS-BAS",BE:"BELGIQUE",CI:"CÔTE D'IVOIRE",AL:"ALBANIE",HR:"CROATIE",RS:"SERBIE",TR:"TURQUIE"};function Xt(e,t){return e&&Number(t==="GK"?e.note_g:t==="DEF"?e.note_d:t==="MIL"?e.note_m:e.note_a)||0}function Si(e,t=""){const i=e.player;if(!i)return"";const n=e.evolution_bonus||0,r={...i,_evolution_bonus:n},o=t?`<div style="position:absolute;top:6px;right:6px;z-index:10;background:#0a3d1e;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 7px">${t}</div>`:"";return`<div style="position:relative;display:inline-block;cursor:pointer" data-card-id="${e.id}">
    ${o}
    ${Ie(r,{width:140})}
    ...
    ${Ie(e,{width:140})}
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),c=(s||[]).filter(E=>E.card_type==="player"&&E.player),l=(s||[]).filter(E=>E.card_type==="game_changer"),m=(s||[]).filter(E=>E.card_type==="formation"),b=(s||[]).filter(E=>E.card_type==="stadium"),{data:y}=await _.from("gc_definitions").select("name,gc_type,color,effect,image_url"),g={};(y||[]).forEach(E=>{g[E.name]=E});const{data:p}=await _.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),u={};(p||[]).forEach(E=>{u[E.id]=E}),b.forEach(E=>{E.stadium_def&&(u[E.stadium_id]=E.stadium_def)});const x=Object.keys(Hi),h=Object.keys(je),v={};c.forEach(E=>{const G=E.player.id;v[G]=(v[G]||0)+1}),new Set(Object.keys(v).map(E=>String(E)));const $=new Set(m.map(E=>E.formation)),f=new Set(l.map(E=>E.gc_type));let z="player",k="Tous",S="",q=!1;function ne(){return[...c].sort((E,G)=>{const R=ci.indexOf(E.player.job),N=ci.indexOf(G.player.job);return R!==N?R-N:(E.player.surname_real||"").localeCompare(G.player.surname_real||"")})}function oe(){return[...d||[]].sort((E,G)=>{const R=ci.indexOf(E.job),N=ci.indexOf(G.job);return R!==N?R-N:(E.surname_real||"").localeCompare(G.surname_real||"")})}function ee(){return ne().filter(E=>{const G=E.player,R=k==="Tous"||G.job===k,N=!S||`${G.firstname} ${G.surname_real}`.toLowerCase().includes(S);return R&&N})}function j(){return oe().filter(E=>{const G=k==="Tous"||E.job===k,R=!S||`${E.firstname} ${E.surname_real}`.toLowerCase().includes(S);return G&&R})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:0 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none;align-items:center"></div>
  ...
  </div>`,function(G){const R=document.getElementById(G);if(!R)return;R.addEventListener("wheel",de=>{Math.abs(de.deltaY)<=Math.abs(de.deltaX)||(de.preventDefault(),R.scrollLeft+=de.deltaY)},{passive:!1});let N=!1,W=0,B=0,O=!1,pe=!1;R.addEventListener("mousedown",de=>{N=!0,O=!1,W=de.pageX,B=R.scrollLeft}),window.addEventListener("mouseup",()=>{N=!1,O&&(R.style.cursor="",pe=!0),O=!1}),window.addEventListener("mousemove",de=>{if(!N)return;const he=de.pageX-W;!O&&Math.abs(he)<6||(O=!0,R.style.cursor="grabbing",de.preventDefault(),R.scrollLeft=B-he)}),R.addEventListener("click",de=>{pe&&(de.stopPropagation(),de.preventDefault(),pe=!1)},!0)}("col-grid");function T(){const E=document.getElementById("col-filters");E&&(z==="player"?(E.innerHTML=`
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",G=>{S=G.target.value.toLowerCase(),K()}),e.querySelectorAll(".filter-btn").forEach(G=>{G.addEventListener("click",()=>{k=G.dataset.job,T(),K()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{q=!q,T(),K()})):(E.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{q=!q,T(),K()})))}function K(){const E=document.getElementById("col-grid");E&&(z==="player"?Y(E):z==="formation"?ce(E):z==="stadium"?V(E):U(E))}function Z(E,G,R,N,W,B=0){const O=document.getElementById("col-grid"),pe=document.getElementById("col-big");if(!O||!pe)return;var de=0;function he(){var ct;const Se=window.innerWidth>=768,ve=document.getElementById("col-big"),Te=document.getElementById("col-gap"),Ae=((ct=document.getElementById("col-grid"))==null?void 0:ct.parentElement)||null,Be=ve?ve.closest(".page"):null;if(ve&&Ae&&Be){let $e=0;Array.from(Be.children).forEach(function(C){C!==ve&&C!==Ae&&C!==Te&&($e+=C.offsetHeight)});const Ce=Math.max(0,Be.clientHeight-$e),qe=Math.round(Ce*(B/100)),Ge=Math.max(0,Ce-qe),w=Se?50/71:45/63,L=Math.round(Ge*w),A=Math.max(0,Ge-L);Te&&(Te.style.height=qe+"px"),ve.style.flex="none",ve.style.height=L+"px",ve.style.minHeight="0",Ae.style.flex="none",Ae.style.height=A+"px",Ae.style.minHeight="0",Ae.style.display="flex",Ae.style.overflow="hidden"}const ke=document.getElementById("col-grid");ke&&(ke.style.height="100%",ke.style.flexShrink="0",ke.style.overflowX="auto",ke.style.overflowY="hidden",ke.style.alignItems="center",ke.style.width="100%"),pe.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:center center">'+G(E[de])+"</div>";var wt=pe.querySelector("[data-card-id],[data-form-id],[data-gc-id]");wt&&wt.addEventListener("click",function(){N(E[de])}),requestAnimationFrame(function(){var $e=document.getElementById("big-card-inner");if(!(!$e||!pe)){var Ce=pe.clientHeight,qe=pe.clientWidth-16,Ge=$e.offsetHeight,w=$e.offsetWidth;if(Ge>0&&w>0&&Ce>40){var L=Math.min(Ce/Ge,qe/w);$e.style.transform="scale("+L.toFixed(3)+")",$e.style.transformOrigin="center center"}}}),O.innerHTML=E.map(function($e,Ce){var qe=Ce===de,Ge="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+(qe?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+Ce+'" style="'+Ge+'">'+R($e,qe)+"</div>"}).join(""),O.querySelectorAll(".col-mini-item").forEach(function($e){$e.addEventListener("click",function(){de=Number($e.dataset.idx),he(),$e.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})}),requestAnimationFrame(function(){var $e=ke?ke.clientHeight:0,Ce=O.querySelector(".col-mini-item > div");if(!(!$e||!Ce)){var qe=Ce.style.zoom;Ce.style.zoom="1";var Ge=Ce.offsetHeight;if(Ce.style.zoom=qe,!(Ge<=0)){var w=$e/Ge;O.querySelectorAll(".col-mini-item > div").forEach(function(L){L.style.zoom=w.toFixed(4)})}}})}he()}function ue(E){var G=window.innerWidth>=768?.76:.54,R;if(!E||E._fake){var N=E?E.player:null;if(!N)return"";R=bn(N)}else R=Si(E,"");return'<div style="display:inline-block;zoom:'+G+';pointer-events:none;line-height:0">'+R+"</div>"}function fe(E,G,R){var N=R>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+R+"</div>":"",W=!!G,B=Mi(E,zt[E],{width:160});return W||(B='<div style="filter:grayscale(1);opacity:0.5">'+B+"</div>"),"<div "+(G?'data-form-id="'+G.id+'"':"")+' style="position:relative;cursor:pointer">'+N+B+"</div>"}function M(E,G){var R=window.innerWidth>=768?.76:.54,N=Mi(E,zt[E],{width:140});return G||(N='<div style="filter:grayscale(1);opacity:0.45">'+N+"</div>"),'<div style="display:inline-block;zoom:'+R+';line-height:0;pointer-events:none">'+N+"</div>"}function Y(E){if(q){const G=j();if(!G.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucun joueur.</div>';return}const R=G.map(N=>c.find(W=>W.player.id===N.id)||{_fake:!0,player:N,id:"fake-"+N.id});Z(R,N=>N._fake?bn(N.player):Si(N,""),N=>N._fake?ue({player:N.player,id:"x",_fake:!0}):ue(N),N=>{N._fake||vn(N,c,v,t)})}else{const G=ee();if(!G.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const R={};G.forEach(W=>{const B=W.player.id;R[B]||(R[B]=[]),R[B].push(W)});const N=Object.values(R).map(W=>qo(W));Z(N,W=>{const B=v[W.player.id]||1,O=B>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${B}</div>`:"",de=c.filter(he=>he.player.id===W.player.id&&he.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Si(W,O+de)},W=>ue(W),W=>vn(W,c,v,t))}}function ce(E){const G=q?x:[...$];if(!G.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const R=G.map(N=>({formation:N,card:m.find(W=>W.formation===N)||null,owned:$.has(N)}));Z(R,({formation:N,card:W,owned:B})=>fe(N,B?W:null,B?m.filter(O=>O.formation===N).length:0),({formation:N,owned:W})=>M(N,W),({card:N,owned:W})=>{W&&N&&No(N,m,t,o)},"#1A6B3C",5)}function U(E){const G=Object.keys(g),R=q?G.length?G:h:[...f];if(!R.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const N=R.map(W=>({type:W,gc:je[W]||{icon:"⚡",desc:""},def:g[W]||null,owned:f.has(W),card:l.find(B=>B.gc_type===W)||null}));Z(N,({type:W,gc:B,def:O,owned:pe,card:de})=>{var ke;const he=(O==null?void 0:O.name)||W,Se=pe?l.filter(wt=>wt.gc_type===W).length:0,ve=Se>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${Se}</div>`:"",Te=(O==null?void 0:O.effect)||B.desc||"",Ae=O!=null&&O.image_url?`/icons/${O.image_url}`:((ke=O==null?void 0:O.club)==null?void 0:ke.logo_url)||(O!=null&&O.country_code?`https://flagsapi.com/${O.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null);let Be=tt(he,Ae,B.icon,Te,{width:160,onClick:pe});return pe||(Be=`<div style="filter:grayscale(1);opacity:0.5">${Be}</div>`),`<div ${pe&&de?`data-gc-id="${de.id}" data-gc-type="${W}"`:""} style="position:relative">${ve}${Be}</div>`},({type:W,gc:B,def:O,owned:pe})=>{const de=window.innerWidth>=768?.76:.54,he=(O==null?void 0:O.name)||W,Se=(O==null?void 0:O.effect)||B.desc||"",ve=O!=null&&O.image_url?`/icons/${O.image_url}`:null;let Te=tt(he,ve,B.icon,Se,{width:140});return pe||(Te=`<div style="filter:grayscale(1);opacity:0.45">${Te}</div>`),`<div style="display:inline-block;zoom:${de};line-height:0;pointer-events:none">${Te}</div>`},({type:W,owned:B,def:O})=>{B&&Go(W,O,o)},"#7a28b8",5)}function V(E){const G="#4FC3F7",R="/";if(!b.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Stade.<br><small>Ouvre un booster Stade !</small></div>';return}const N={};b.forEach(B=>{const O=B.stadium_id||"?";(N[O]=N[O]||[]).push(B)});const W=Object.entries(N).map(([B,O])=>({sid:B,def:u[B]||null,count:O.length,card:O[0]}));Z(W,({def:B,count:O})=>{var Te,Ae;const pe=(B==null?void 0:B.name)||"?",de=((Te=B==null?void 0:B.club)==null?void 0:Te.encoded_name)||(B==null?void 0:B.country_code)||"—",he=B!=null&&B.image_url?`${R}icons/${B.image_url}`:((Ae=B==null?void 0:B.club)==null?void 0:Ae.logo_url)||(B!=null&&B.country_code?`https://flagsapi.com/${B.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),Se=O>1?`<div style="position:absolute;top:8px;right:8px;background:#333;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${O}</div>`:"",ve=`${de}<br>+10 ⭐ joueurs alliés`;return`<div style="position:relative">${Se}${ii(pe,he,ve,{width:160})}</div>`},({def:B})=>{var ve,Te;const O=window.innerWidth>=768?.76:.54,pe=(B==null?void 0:B.name)||"?",de=((ve=B==null?void 0:B.club)==null?void 0:ve.encoded_name)||(B==null?void 0:B.country_code)||"—",he=B!=null&&B.image_url?`${R}icons/${B.image_url}`:((Te=B==null?void 0:B.club)==null?void 0:Te.logo_url)||(B!=null&&B.country_code?`https://flagsapi.com/${B.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),Se=`${de}<br>+10 ⭐`;return`<div style="display:inline-block;zoom:${O};line-height:0;pointer-events:none">${ii(pe,he,Se,{width:140})}</div>`},null,G,5)}e.querySelectorAll(".col-tab-btn").forEach(E=>{E.addEventListener("click",()=>{z=E.dataset.tab,k="Tous",S="",q=!1,e.querySelectorAll(".col-tab-btn").forEach(G=>{const R=G.dataset.tab===z;G.style.borderBottomColor=R?"var(--green)":"transparent",G.style.color=R?"var(--green)":"var(--tile-fg-dim)"}),T(),K()})}),T(),K()}function Go(e,t,i){const n=je[e]||{icon:"⚡",desc:"Effet spécial."},r=(t==null?void 0:t.name)||e,o=(t==null?void 0:t.effect)||n.desc,a=t!=null&&t.image_url?`/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
      ${tt(r,a,n.icon,o,{width:200})}
      <div style="background:#fff3cd;border-radius:10px;padding:10px 14px;width:100%">
        <div style="font-size:12px;color:#856404">⚠️ Cette carte est à <b>usage unique</b>. Une fois jouée en match, elle est définitivement supprimée de ta collection.</div>
      </div>
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const pi=1e3;function No(e,t,i,n){var g,p,u;const{state:r,toast:o,closeModal:a,navigate:s,refreshProfile:d}=i,c=e.formation,l={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function m(){const x=zt[c]||{},h=Hi[c]||[],v=290,$=360,f=20;function z(S){const q=x[S];return q?{x:q.x*v,y:q.y*$}:null}let k=`<svg width="${v}" height="${$}" viewBox="0 0 ${v} ${$}" xmlns="http://www.w3.org/2000/svg">`;for(const[S,q]of h){const ne=z(S),oe=z(q);!ne||!oe||(k+=`<line x1="${ne.x}" y1="${ne.y}" x2="${oe.x}" y2="${oe.y}"
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(g=document.getElementById("direct-sell-form-btn"))==null||g.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${c} pour ${pi.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const x=b.find(v=>!v.is_for_sale)||b[0];if(!x){o("Aucune carte à vendre","error");return}await _.from("market_listings").delete().eq("card_id",x.id),await _.from("transfer_history").delete().eq("card_id",x.id);const{error:h}=await _.from("cards").delete().eq("id",x.id);if(h){o(h.message,"error");return}await _.from("users").update({credits:(r.profile.credits||0)+pi}).eq("id",r.profile.id),await d(),o(`+${pi.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),a(),s("collection")}),(p=document.getElementById("market-sell-form-btn"))==null||p.addEventListener("click",async()=>{const x=parseInt(document.getElementById("sell-price-form").value);if(!x||x<1){o("Prix invalide","error");return}await _.from("cards").update({is_for_sale:!0,sale_price:x}).eq("id",e.id),await _.from("market_listings").insert({seller_id:r.profile.id,card_id:e.id,price:x}),o("Carte mise en vente sur le marché !","success"),a(),s("collection")}),(u=document.getElementById("cancel-sell-form-btn"))==null||u.addEventListener("click",async()=>{await _.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await _.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),o("Annonce retirée","success"),a(),s("collection")})}async function vn(e,t,i,n){var ue,fe,M,Y,ce;const{state:r,toast:o,openModal:a,closeModal:s,navigate:d,refreshProfile:c}=n,l=e.player,m=t.filter(U=>U.player.id===l.id),b=m.length,y=e.evolution_bonus||0,g=Math.max((Number(l.note_g)||0)+(l.job==="GK"||l.job2==="GK"?y:0),(Number(l.note_d)||0)+(l.job==="DEF"||l.job2==="DEF"?y:0),(Number(l.note_m)||0)+(l.job==="MIL"||l.job2==="MIL"?y:0),(Number(l.note_a)||0)+(l.job==="ATT"||l.job2==="ATT"?y:0)),p=l.rarity||"normal",{data:u}=await _.from("sell_price_configs").select("*").eq("rarity",p).lte("note_min",g).gte("note_max",g).order("note_min",{ascending:!1}).limit(1);((ue=u==null?void 0:u[0])==null?void 0:ue.price)??Bo[p];const x=l.rarity!=="legende";At(l);const h=((l.rarity==="pepite"||l.rarity==="papyte")&&e.current_note!=null?e.current_note:Xt(l,l.job))+y,v=l.rarity==="pepite"||l.rarity==="papyte",$=l.job2?(v?Mo(e,Do(l.job2)):Xt(l,l.job2))+(Xt(l,l.job2)>0?y:0):null;li[l.job],l.job2&&li[l.job2];const f=Ii[l.rarity]||"#ccc";Fo[l.country_code]||l.country_code;const z=e.evolution_bonus||0,S=h+z,q=$||0,ne=q>0?q+z:0,oe=m.map(U=>U.id);let ee={};if(oe.length){const{data:U}=await _.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",oe).order("transferred_at",{ascending:!0});(U||[]).forEach(V=>{ee[V.card_id]||(ee[V.card_id]=[]),ee[V.card_id].push(V)})}const j=oe.length?`
    <div style="margin-top:16px;border-top:1px solid var(--tile-border);padding-top:14px">
      ...
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px">
        ${m.map((U,V)=>{const E=ee[U.id]||[],G=U.is_for_sale,R=E.length?E[E.length-1]:null,N=U.evolution_bonus||0,W=Xt(l,l.job)+N;l.rarity==="pepite"||l.rarity;const B=li[l.job]||"#1A6B3C",O=Ii[l.rarity]||"#ccc",pe=R?R.source==="booster"?"Booster":R.price?R.price.toLocaleString("fr")+" cr.":"—":"—",de=R?new Date(R.transferred_at).toLocaleDateString("fr",{day:"2-digit",month:"2-digit",year:"numeric"}):"";return R&&R.club_name,`
            <div class="exemplaire-row" data-card-id="${U.id}" data-card-idx="${V}"
              ...
              <input type="checkbox" class="expl-check"
                data-id="${U.id}" data-evo="${N}" data-note="${Xt(l,l.job)}"
                ${G?"disabled":""}
              ...
              <div class="expl-mini-card" style="
                border-radius:8px;border:2.5px solid ${O};background:${B};
                ...
                <!-- Note + evo badge -->
                <div style="font-size:14px;font-weight:900;color:#fff;z-index:0">${W}</div>
                ${N>0?`<div style="position:absolute;bottom:20px;right:3px;background:#D4A017;color:#000;font-size:7px;font-weight:900;border-radius:3px;padding:1px 3px;z-index:2">+${N}</div>`:""}
                <!-- Label en vente -->
                ${G?'<div style="position:absolute;top:0;right:0;background:#e67e22;color:#fff;font-size:6px;font-weight:900;padding:2px 4px;border-radius:0 6px 0 4px;z-index:2">VENTE</div>':""}
              </div>
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",l.note_g],["DEF",l.note_d],["MIL",l.note_m],["ATT",l.note_a]].map(([U,V])=>{const E=li[U],G=U==="GK"?"#fff":E,R=U===l.job||U===l.job2,N=(Number(V)||0)+(R&&z>0?z:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
    ...
    ${j}
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(fe=document.getElementById("close-detail"))==null||fe.addEventListener("click",s);let T=new Set;function K(){let U=0;return document.querySelectorAll(".expl-check:checked").forEach(V=>{if(V.dataset.id===e.id)return;const G=Number(V.dataset.evo)||0,R=Number(V.dataset.note)||0;U+=R+G}),U}const Z=()=>{const U=T.size,V=document.getElementById("sell-action-panel");if(!V)return;V.style.display=U>0?"block":"none",document.getElementById("sell-selected-count").textContent=U;const E=document.getElementById("evolve-btn");if(E){const G=K();E.textContent=G>0?`⬆️ Évoluer (+${G})`:"⬆️ Évoluer"}};document.querySelectorAll(".expl-check").forEach(U=>{U.addEventListener("change",()=>{const V=U.dataset.id;U.checked?T.add(V):T.delete(V);const E=U.closest(".exemplaire-row");if(E){const G=E.querySelector(".expl-mini-card"),R=E.querySelector(".expl-sel-overlay");G&&(G.style.border=U.checked?"2.5px solid #1A6B3C":`2.5px solid ${Ii[l.rarity]||"#ccc"}`),R&&(R.style.display=U.checked?"block":"none"),E.style.transform=U.checked?"scale(1.04)":"scale(1)"}Z()})}),document.querySelectorAll(".exemplaire-row").forEach(U=>{U.addEventListener("click",V=>{if(V.target.tagName==="INPUT")return;const E=U.querySelector(".expl-check");E&&!E.disabled&&(E.checked=!E.checked,E.dispatchEvent(new Event("change")))})}),(M=document.getElementById("evolve-btn"))==null||M.addEventListener("click",async()=>{if(b<=1)return;const U=[...T];if(!U.length)return;if(T.has(e.id)){const W=document.createElement("div");W.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",W.innerHTML=`
        ...
        <div style="background:var(--tile-bg);border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center;box-shadow:0 16px 48px rgba(0,0,0,0.3)">
          ...
          <div style="background:#f0fdf4;border-radius:10px;padding:12px;margin-bottom:16px;font-size:13px;color:#333">
            ...
            ⬆️ Bonus gagné : <strong style="color:#1A6B3C">+${E}</strong> <span style="font-size:11px;color:#888">(Σ note + évolution de chaque copie)</span><br>
            📈 Évolution : <strong>${e.evolution_bonus||0}</strong> → <strong style="color:#1A6B3C">${(e.evolution_bonus||0)+E}</strong><br>
            📊 Note finale : <strong>${h}</strong> → <strong style="color:#1A6B3C">${h+E}</strong>
            ${$&&$>0?`<br>📊 Note 2 finale : <strong>${$}</strong> → <strong style="color:#1A6B3C">${$+E}</strong>`:""}
          </div>
        ...
        </div>`,document.body.appendChild(B),B.querySelector("#sac-cancel").addEventListener("click",()=>{B.remove(),W(!1)}),B.querySelector("#sac-ok").addEventListener("click",()=>{B.remove(),W(!0)}),B.addEventListener("click",O=>{O.target===B&&(B.remove(),W(!1))})}))return;if(V.length){await _.from("market_listings").delete().in("card_id",V),await _.from("deck_cards").delete().in("card_id",V),await _.from("transfer_history").delete().in("card_id",V),await _.from("decks").update({stadium_card_id:null}).in("stadium_card_id",V);const{error:W}=await _.from("cards").delete().in("id",V);if(W){o("Erreur suppression : "+W.message,"error");return}}const R=(e.evolution_bonus||0)+E,{error:N}=await _.from("cards").update({evolution_bonus:R}).eq("id",e.id);if(N){o("Erreur évolution : "+N.message,"error");return}o(`⬆️ ${l.firstname} ${l.surname_real} : note ${h} → ${h+R} (+${E}) !`,"success",4e3),s(),d("collection")}),(Y=document.getElementById("market-sell-btn"))==null||Y.addEventListener("click",async()=>{var N;const U=[...T];if(!U.length){o("Sélectionne au moins un exemplaire","warning");return}const V=parseInt((N=document.getElementById("sell-market-price"))==null?void 0:N.value);if(!V||V<1){o("Prix invalide","error");return}const{error:E}=await _.from("cards").update({is_for_sale:!0,sale_price:V}).in("id",U);if(E){o(E.message,"error");return}const G=U.map(W=>({seller_id:r.profile.id,card_id:W,price:V,status:"active"})),{error:R}=await _.from("market_listings").insert(G);R&&console.warn("[Market] insert listings:",R.message),o(`${U.length} carte${U.length>1?"s":""} mise${U.length>1?"s":""} en vente à ${V.toLocaleString("fr")} cr. chacune !`,"success"),s(),d("collection")}),(ce=document.getElementById("cancel-sell-btn"))==null||ce.addEventListener("click",async()=>{await _.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await _.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),o("Annonce retirée","success"),s(),d("collection")})}function hn(e,t=""){return new Promise(i=>{const n=document.createElement("div");n.className="modal-overlay",n.style.zIndex="2100",n.innerHTML=`<div class="modal" style="max-width:360px">
    ...
    </div>`,document.body.appendChild(n);const r=o=>{n.remove(),i(o)};n.querySelector("#cm-ok").addEventListener("click",()=>r(!0)),n.querySelector("#cm-cancel").addEventListener("click",()=>r(!1)),n.addEventListener("click",o=>{o.target===n&&r(!1)})})}const gi={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}};async function ji(e,t){const{state:i,navigate:n,toast:r}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await _.from("decks").select("id,name,formation_card_id").eq("owner_id",i.profile.id).order("created_at",{ascending:!1});e.innerHTML=`
  ...
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
        ...
        <div style="width:130px;flex-shrink:0;background:rgba(0,0,0,0.3);display:flex;flex-direction:column;align-items:center;padding:12px 8px;gap:12px;border-left:1px solid rgba(255,255,255,0.1)">
          ...
          <div style="width:100%;text-align:center">
            ...
            <div id="add-stad-btn-pc" style="cursor:pointer;margin:0 auto;width:fit-content">
              ${s?(()=>{var h;const u=t.stadDefMap[s.stadium_id],x=((h=u==null?void 0:u.club)==null?void 0:h.logo_url)||(u==null?void 0:u.image_url)||null;return ii((u==null?void 0:u.name)||"Stade",x,"+10 ⭐",{width:100})})():`<div style="width:100px;height:171px;border:2px dashed rgba(79,195,247,0.4);border-radius:8px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px">
        ...
        <div style="display:flex;gap:10px;align-items:flex-start">
          ...
          <div style="flex-shrink:0;text-align:center">
            ...
            <div id="add-stad-btn" style="cursor:pointer">
              ${s?(()=>{var h;const u=t.stadDefMap[s.stadium_id],x=((h=u==null?void 0:u.club)==null?void 0:h.logo_url)||(u==null?void 0:u.image_url)||null;return ii((u==null?void 0:u.name)||"Stade",x,"+10⭐",{width:66})})():`<div style="width:66px;height:86px;border:2px dashed rgba(79,195,247,0.5);border-radius:6px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px">
    ...
    </div>`;u("⚽ Choisir une formation",h),document.querySelectorAll("#modal-body [data-forma]").forEach(v=>{v.addEventListener("click",()=>{t.formation=v.dataset.forma;const $=wn(t.formation),f={};$.forEach(z=>{t.slots[z]&&(f[z]=t.slots[z])}),t.slots=f,x(),_t(e,t,i)})})};e.querySelectorAll("#formation-mobile-btn, #formation-pc-btn").forEach(u=>u.addEventListener("click",y)),e.querySelectorAll("#add-stad-btn-pc, #add-stad-btn").forEach(u=>u.addEventListener("click",()=>Ho(t,e,i))),e.querySelectorAll("#save-deck").forEach(u=>u.addEventListener("click",()=>Vo(t,i))),e.querySelectorAll("[data-remove-sub]").forEach(u=>{u.addEventListener("click",()=>{t.subs=t.subs.filter(x=>x!==u.dataset.removeSub),_t(e,t,i)})}),e.querySelectorAll("#add-sub-btn").forEach(u=>u.addEventListener("click",()=>Ko(t,e,i)))}function Oo(e,t,i,n){var v,$,f;const r=window.innerWidth>=900,o=e.querySelector(r?"#deck-field-pc":"#deck-field-mobile");if(!o)return;const a=(v=t.stadiumCards)==null?void 0:v.find(z=>z.id===t.stadiumCardId),s=a&&(($=t.stadDefMap)==null?void 0:$[a.stadium_id])||null,d=zt[t.formation]||{},c=Oi(t.formation)||[],l={};for(const z of i){const k=t.slots[z],S=k?t.playerCards.find(q=>q.id===k):null;S!=null&&S.player?l[z]={...S.player,_evolution_bonus:S.evolution_bonus||0,face:S.player.face||null}:l[z]=null}const m=window.innerWidth>=900,b=m?window.innerWidth-280:window.innerWidth-20,y=m?Math.min(b,860):b,g=Math.round(m?y*.82:y*.85),p=m?84:44;let u="";for(const[z,k]of c){const S=d[z],q=d[k];if(!S||!q)continue;const ne=S.x*y,oe=Math.round(.03*g+S.y*.85*g),ee=q.x*y,j=Math.round(.03*g+q.y*.85*g),T=l[z],K=l[k],Z=Ut(T,K);Z==="#ff3333"||Z==="#cc2222"?u+=`<line x1="${ne.toFixed(1)}" y1="${oe.toFixed(1)}" x2="${ee.toFixed(1)}" y2="${j.toFixed(1)}" stroke="${Z}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(u+=`<line x1="${ne.toFixed(1)}" y1="${oe.toFixed(1)}" x2="${ee.toFixed(1)}" y2="${j.toFixed(1)}" stroke="${Z}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,u+=`<line x1="${ne.toFixed(1)}" y1="${oe.toFixed(1)}" x2="${ee.toFixed(1)}" y2="${j.toFixed(1)}" stroke="${Z}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let x="";const h=Math.round(p*657/507);for(const z of i){const k=d[z];if(!k)continue;const S=l[z],q=k.x*y;k.y*g;const ne=Math.round(.03*g+k.y*(.85*g)),oe=Math.round(q-p/2),ee=Math.round(ne-h/2);if(S){const j=z.replace(/\d+/,""),T=s&&(s.club_id&&String(S.club_id)===String(s.club_id)||s.country_code&&S.country_code===s.country_code),K=Ie({...S,_evolution_bonus:S._evolution_bonus||0},{width:p,showStad:!0,stadDef:s,role:j});T&&((f=s.club)!=null&&f.logo_url||s.image_url),x+=`<div style="position:absolute;left:${oe}px;top:${ee}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${z}">
  ...
  `,document.body.appendChild(r);let o=!1;const a=document.getElementById("pack-cut-zone"),s=document.getElementById("pack-blade");let d=!1;const c=T=>T.touches&&T.touches[0]?T.touches[0].clientX:T.clientX;function l(T){o||(d=!0,s.style.opacity="1",m(T))}function m(T){if(!d||o)return;const K=a.getBoundingClientRect(),Z=c(T)-K.left,ue=Math.max(0,Math.min(1,Z/K.width));s.style.width=ue*K.width+"px",ue>=.82&&y()}function b(){o||(d=!1,s.style.transition="width .2s ease, opacity .2s ease",s.style.width="0",s.style.opacity="0",setTimeout(()=>{o||(s.style.transition="")},220))}function y(){var K;if(o)return;o=!0,d=!1,s.style.width="100%",s.style.opacity="1",(K=document.getElementById("cut-flash"))==null||K.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const T=document.getElementById("cut-hint");T&&(T.style.opacity="0"),a.classList.add("pack-cut"),setTimeout(()=>{s.style.opacity="0",document.getElementById("pack-phase").style.display="none",x(0)},620)}a.addEventListener("pointerdown",l),window.addEventListener("pointermove",m),window.addEventListener("pointerup",b),a.addEventListener("touchstart",l,{passive:!0}),window.addEventListener("touchmove",m,{passive:!0}),window.addEventListener("touchend",b);let g=0,p=!1;const u=new Set;function x(T){g=T,document.getElementById("reveal-phase").style.display="flex",h(),v(T,0),S()}function h(){const T=document.getElementById("card-dots");T&&(T.innerHTML=e.map((K,Z)=>`<div class="card-dot" data-i="${Z}" style="width:8px;height:8px;border-radius:50%;background:${Z===g?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),T.querySelectorAll(".card-dot").forEach(K=>K.addEventListener("click",()=>f(parseInt(K.dataset.i)))))}function v(T,K){var E,G;const Z=e[T],ue=document.getElementById("card-counter"),fe=document.getElementById("card-track");ue&&(ue.textContent=`Carte ${T+1} / ${e.length}`);const M=document.getElementById("reveal-btns");M&&(M.style.display=T===e.length-1?"flex":"none");const Y=Z.card_type==="player"&&((E=Z.player)==null?void 0:E.rarity)==="legende",ce=!u.has(T);u.add(T);let U=0;if(Z.card_type==="player"&&Z.player){const R=Z.player,N=R.job||"ATT";U=Number(N==="GK"?R.note_g:N==="DEF"?R.note_d:N==="MIL"?R.note_m:R.note_a)||0}const V=R=>{fe.innerHTML=`
        ...
        </div>`;const N=document.getElementById("current-card-wrap");K!==0?(N.style.transition="none",N.style.transform=`translateX(${K>0?100:-100}%)`,requestAnimationFrame(()=>{N.style.transition="transform .28s cubic-bezier(.25,1,.5,1)",N.style.transform="translateX(0)"})):N.animate([{opacity:0,transform:"scale(.7)"},{opacity:1,transform:"scale(1)"}],{duration:300,easing:"cubic-bezier(.34,1.56,.64,1)"}),R||Y?ne():oe(),h()};ce&&(((G=Z.player)==null?void 0:G.rarity)==="legende"||U>=8)&&Z.card_type==="player"&&Z.player?$(Z,()=>V(!0)):V(!1)}function $(T,K){var Se;p=!0;const Z=T.player,ue=`https://flagsapi.com/${Z.country_code}/flat/64.png`,fe=(Se=Z.clubs)==null?void 0:Se.logo_url,M=At(Z),Y=Z.job||"ATT",ce=Number(Y==="GK"?Z.note_g:Y==="DEF"?Z.note_d:Y==="MIL"?Z.note_m:Z.note_a)||0,U=T.evolution_bonus||0,V=ce+U,E=V>=18&&V<=20,G=V>=18,R=document.getElementById("walkout-overlay"),N=document.getElementById("walkout-stage");if(!R||!N){p=!1,K();return}let W=null;E&&(W=Xi("/sounds/Legendary.mp3",.8)),R.style.display="flex";const B=()=>{const ve=N.firstElementChild;ve&&(ve.classList.remove("wo-in"),ve.classList.add("wo-out"))},O=1800,pe=400;N.innerHTML=`<img class="wo-in" src="${ue}" style="height:130px;border-radius:10px;box-shadow:0 10px 36px rgba(0,0,0,.6)" onerror="this.style.display='none'">`,navigator.vibrate&&navigator.vibrate(30),setTimeout(B,O),setTimeout(()=>{var ve;N.innerHTML=fe?`<img class="wo-in" src="${fe}" style="max-height:160px;max-width:210px;object-fit:contain">`:`<div class="wo-in" style="font-size:34px;font-weight:900;color:#fff;text-align:center">${((ve=Z.clubs)==null?void 0:ve.encoded_name)||"CLUB"}</div>`,navigator.vibrate&&navigator.vibrate(30)},O+pe),setTimeout(B,O*2+pe),setTimeout(()=>{N.innerHTML=M?`<img class="wo-in" src="${M}" style="height:200px;border-radius:50%;box-shadow:0 0 40px rgba(255,255,255,0.3);object-fit:cover;object-position:top">`:'<div class="wo-in" style="font-size:80px">👤</div>',navigator.vibrate&&navigator.vibrate(30)},(O+pe)*2),setTimeout(B,(O+pe)*2+O);const he=G?"#FFD700":{GK:"#c0c0c0",DEF:"#e03030",MIL:"#D4A017",ATT:"#3fbf5f"}[Y]||"#fff";setTimeout(()=>{N.innerHTML=`<div class="wo-in" style="
        ...
        ${V}
      </div>`,G&&navigator.vibrate&&navigator.vibrate([50,30,100,30,200])},(O+pe)*3),setTimeout(B,(O+pe)*3+O),setTimeout(()=>{R.style.display="none",N.innerHTML="",p=!1,W&&!E&&W.pause(),navigator.vibrate&&navigator.vibrate([40,30,80]),K()},(O+pe)*4)}function f(T){if(p||T<0||T>=e.length||T===g)return;const K=T>g?1:-1;g=T,v(T,K)}function z(){f(g+1)}function k(){f(g-1)}function S(){const T=document.getElementById("card-viewport");if(!T||T._swipeBound)return;T._swipeBound=!0;let K=0,Z=0,ue=0,fe=!1;const M=E=>E.touches?E.touches[0].clientX:E.clientX,Y=E=>E.touches?E.touches[0].clientY:E.clientY,ce=E=>{fe=!0,K=M(E),Z=Y(E),ue=0},U=E=>{if(!fe)return;ue=M(E)-K;const G=Y(E)-Z;if(Math.abs(ue)<Math.abs(G))return;const R=document.getElementById("current-card-wrap");R&&(R.style.transition="none",R.style.transform=`translateX(${ue*.6}px) rotate(${ue*.02}deg)`)},V=()=>{if(!fe)return;fe=!1;const E=document.getElementById("current-card-wrap"),G=55;ue<=-G&&g<e.length-1?z():ue>=G&&g>0?k():E&&(E.style.transition="transform .2s ease",E.style.transform="translateX(0)")};T.addEventListener("pointerdown",ce),T.addEventListener("pointermove",U),T.addEventListener("pointerup",V),T.addEventListener("pointercancel",V),T.addEventListener("touchstart",ce,{passive:!0}),T.addEventListener("touchmove",U,{passive:!0}),T.addEventListener("touchend",V),T.addEventListener("click",E=>{if(Math.abs(ue)>8)return;const G=T.getBoundingClientRect();E.clientX-G.left>G.width/2?z():k()})}let q=null;function ne(){const T=document.getElementById("fireworks-canvas");if(!T)return;T.width=window.innerWidth,T.height=window.innerHeight;const K=T.getContext("2d"),Z=[];function ue(){const M=Math.random()*T.width,Y=Math.random()*T.height*.6,ce=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],U=ce[Math.floor(Math.random()*ce.length)];for(let V=0;V<60;V++){const E=Math.PI*2/60*V,G=2+Math.random()*5;Z.push({x:M,y:Y,vx:Math.cos(E)*G,vy:Math.sin(E)*G,alpha:1,color:U,size:2+Math.random()*3})}}ue(),q=setInterval(ue,600);function fe(){if(document.getElementById("fireworks-canvas")){K.clearRect(0,0,T.width,T.height);for(let M=Z.length-1;M>=0;M--){const Y=Z[M];if(Y.x+=Y.vx,Y.y+=Y.vy+.08,Y.vy*=.98,Y.alpha-=.018,Y.alpha<=0){Z.splice(M,1);continue}K.globalAlpha=Y.alpha,K.fillStyle=Y.color,K.beginPath(),K.arc(Y.x,Y.y,Y.size,0,Math.PI*2),K.fill()}K.globalAlpha=1,(q!==null||Z.length>0)&&requestAnimationFrame(fe)}}fe()}function oe(){q!==null&&(clearInterval(q),q=null);const T=document.getElementById("fireworks-canvas");T&&T.getContext("2d").clearRect(0,0,T.width,T.height)}if(n){const T=document.getElementById("reveal-btns");T&&(T.innerHTML='<button class="btn btn-primary" id="reveal-next" style="flex:1">Continuer →</button>'),(j=document.getElementById("reveal-next"))==null||j.addEventListener("click",()=>{oe(),r.remove(),n()})}else document.getElementById("reveal-collection").addEventListener("click",()=>{oe(),r.remove(),i("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{oe(),r.remove(),i("boosters")})}function ir(e){var t,i,n,r;if(e.card_type==="player"&&e.player)return Jo(e);if(e.card_type==="game_changer"){const o=e._gcDef,a=(o==null?void 0:o.name)||e.gc_type||"Game Changer",s=(o==null?void 0:o.effect)||((t=Bi[e.gc_type])==null?void 0:t.desc)||"",d=o!=null&&o.image_url?`/icons/${o.image_url}`:null,c=((i=Bi[e.gc_type])==null?void 0:i.icon)||"⚡";return tt(a,d,c,s,{width:170})}if(e.card_type==="formation")return Mi(e.formation,zt[e.formation],{width:160});if(e.card_type==="stadium"){const o=e._stadiumDef,a=(o==null?void 0:o.name)||"Stade",s=((n=o==null?void 0:o.club)==null?void 0:n.encoded_name)||(o==null?void 0:o.country_code)||"—",d=o!=null&&o.image_url?`/icons/${o.image_url}`:((r=o==null?void 0:o.club)==null?void 0:r.logo_url)||(o!=null&&o.country_code?`https://flagsapi.com/${o.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),c=`${s}<br>+10 ⭐ joueurs alliés`;return ii(a,d,c,{width:170})}return'<div style="width:140px;height:200px;background:#333;border-radius:12px"></div>'}function nr(e){var t,i;if((t=e==null?void 0:e.rates)!=null&&t.length){const n=document.createElement("div");n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.6);display:flex;align-items:center;justify-content:center;z-index:4000;padding:16px";const r={normal:"#ccc",pepite:"#D4A017",pépite:"#D4A017",papyte:"#909090",legende:"#7a28b8",légende:"#7a28b8"},o={player:"Joueur",formation:"Formation",game_changer:"Game Changer",game_helper:"Game Helper"};n.innerHTML=`
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff;position:relative">
      ...
      <div id="deck-bottom-bar" style="flex-shrink:0;padding:10px 14px 14px;display:flex;flex-direction:column;gap:8px;background:rgba(0,0,0,0.2)">
        ...
        stroke="${k}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const g=typeof window<"u"&&window.innerWidth>=900?Math.min(Math.max(81,Math.round(r*.225)),117):Math.max(44,Math.round(r*.168)),p=Math.round(g*657/507);for(const[x,h]of Object.entries(l)){const v=b(x);if(!v||!h)continue;const $=x.replace(/[0-9]/g,""),f=a.includes(h.cardId),z=i==="attack"&&(["MIL","ATT"].includes($)||f)&&!h.used||i==="defense"&&["GK","DEF","MIL"].includes($)&&!h.used,k=n.includes(h.cardId);let S=h.boost||0,q=!1;h.stadiumBonus&&(i==="attack"&&($==="ATT"||$==="MIL")||i==="defense"&&($==="GK"||$==="DEF"||$==="MIL")?(S+=10,q=!0):i||(S+=10,q=!0));const ne=Math.round(v.x-g/2),oe=Math.round(v.y-p/2);if(h.used){y+=`<image href="${`${typeof import.meta<"u"&&"/"||"/"}icons/carte-dos.png`}" x="${ne}" y="${oe}" width="${g}" height="${p}" preserveAspectRatio="xMidYMid slice" class="match-used-hit" data-card-id="${h.cardId}" data-role="${$}" style="cursor:pointer"/>`;continue}const ee=Ie({...h,_evolution_bonus:0,stadiumBonus:!1},{width:g,showStad:!1,stadDef:null,role:$,extraNote:S,_cardOffset:30,_forceStadColor:q}),j=k?`position:absolute;top:30px;left:0;width:${g}px;height:${p}px;outline:3px solid #FFD700;outline-offset:2px;border-radius:8px;pointer-events:none;`:"";y+=`<foreignObject x="${ne-2}" y="${oe-30}" width="${g+8}" height="${p+60}" style="overflow:visible">
      <div xmlns="http://www.w3.org/1999/xhtml" style="position:relative">
        ${ee}
        ${k?`<div style="${j}"></div>`:""}
      </div>
    </foreignObject>`,z&&(y+=`<rect x="${ne}" y="${oe}" width="${g}" height="${p}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${k?"selected":""}" data-card-id="${h.cardId}" data-role="${$}" style="cursor:pointer"/>`)}const u=s!==null?s:Math.round(Math.max(g*.7,80));return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-u} ${-u} ${r+u*2} ${o+u*2}" width="100%" style="display:block;width:100%;margin:0 auto">
    ${y}
  </svg>`}function Ot(e,t,i,n,r=300,o=300,a=[],s=null){return`<div id="match-terrain-wrap" style="position:relative;padding:0 4px">
    ${wi(e,t,i,n,r,o,a,s)}
  </div>`}async function ki(e,t,i,n){var z;const{state:r,navigate:o,toast:a}=t;st(e);const s=r.params||{};if(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!s.deckId)return cr(e,t,i);const d=s.deckId;let c,l,m,b;try{const k=await Promise.all([_.from("decks").select("formation,name,stadium_card_id").eq("id",d).single(),_.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation, evolution_bonus,
            player:players(id,firstname,surname_real,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
              clubs(encoded_name,logo_url)))`).eq("deck_id",d).order("slot_order")]);c=k[0].data,m=k[0].error,l=k[1].data,b=k[1].error}catch(k){console.error("[Match] Exception chargement deck:",k),Tt(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>o("home"));return}if(m||b){console.error("[Match] Erreur Supabase:",m||b),Tt(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>o("home"));return}const y=(l||[]).filter(k=>{var S;return k.is_starter&&((S=k.card)==null?void 0:S.player)}).map(k=>Di(k.card,k.position)),g=(l||[]).filter(k=>{var S;return!k.is_starter&&((S=k.card)==null?void 0:S.player)}).map(k=>Di(k.card,k.position));if(y.length<11){Tt(e,"⚠️",`Deck incomplet (${y.length}/11).`,"Compléter",()=>o("decks"));return}const p=(l||[]).find(k=>{var S;return((S=k.card)==null?void 0:S.card_type)==="formation"}),u=(c==null?void 0:c.formation)||((z=p==null?void 0:p.card)==null?void 0:z.formation)||"4-4-2",{data:x,error:h}=await _.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",r.profile.id).eq("card_type","game_changer"),{data:v}=await _.from("gc_definitions").select("*").eq("is_active",!0),$=(x||[]).map(k=>({...k,_gcDef:(v==null?void 0:v.find(S=>S.name===k.gc_type||S.id===k.gc_definition_id))||null}));let f=null;if(c!=null&&c.stadium_card_id){const{data:k}=await _.from("cards").select("stadium_id").eq("id",c.stadium_card_id).single();if(k!=null&&k.stadium_id){const{data:S}=await _.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").eq("id",k.stadium_id).single();f=S||null}}n({deckId:d,formation:u,starters:y,subsRaw:g,gcCardsEnriched:$,gcDefs:v||[],stadiumDef:f})}function It(){return Math.min(window.innerWidth-40,860)}function Nt(){return Math.round(It()*1.1)}function Yn(e){var r,o;if(!e)return null;const t=e._line||e.job||"MIL",i=t==="GK"?e.note_g||0:t==="DEF"?e.note_d||0:t==="MIL"?e.note_m||0:e.note_a||0,n=e.stadiumBonus?10:0;return{name:e.name,firstname:e.firstname||"",note:i+(e.boost||0)+n,note_g:e.note_g||0,note_d:e.note_d||0,note_m:e.note_m||0,note_a:e.note_a||0,_evolution_bonus:0,stadiumBonus:e.stadiumBonus||!1,boost:e.boost||0,job:e.job,job2:e.job2||null,_line:e._line||e.job,_col:e._col,country_code:e.country_code,club_id:e.club_id,rarity:e.rarity,clubName:e.clubName||((r=e.clubs)==null?void 0:r.encoded_name)||null,clubLogo:e.clubLogo||((o=e.clubs)==null?void 0:o.logo_url)||null,face:e.face||null,portrait:At(e)}}function pr(e,t,i,n="Adversaire"){zn("/sounds/match-opening.mp3",.3);const r=It(),o=Nt();return`
    ...
    </div>`}function Wn(e){var o,a,s;if(!e)return"";const t=d=>d?Ie({...d,_evolution_bonus:0},{width:52,role:d._line||d.job,showStad:!!d.stadiumBonus,extraNote:d.boost||0}):"",n={goal:"⚽","goal-home":"⚽","goal-ai":"⚽",duel:"⚔️",midfield:"🎯",sub:"🔄",gc:"⚡",boost:"💥","defense-won":"🛡️","attack-won":"⚔️","defense-lost":"😓","attack-lost":"😓"}[e.type]||"📋";return`
    ...
    pointer-events:none;white-space:nowrap;max-width:90vw;text-align:center`,r.textContent=e,!document.getElementById("game-toast-style")){const o=document.createElement("style");o.id="game-toast-style",o.textContent=`
      ...
      }`,document.head.appendChild(o)}document.body.appendChild(r),setTimeout(()=>{r.style.opacity="0",r.style.transition="opacity 0.3s",setTimeout(()=>r.remove(),300)},i)}function Xn(e,t){const i=Pe(e,"MIL"),n=e.stadiumBonus||t&&(t.club_id&&String(e.club_id)===String(t.club_id)||t.country_code&&e.country_code===t.country_code)?10:0;return i+n}function xt(e,t){return e.reduce((i,n)=>i+Xn(n,t),0)}function bt(e){let t=0;for(let i=0;i<e.length-1;i++){const n=Ut(e[i],e[i+1]);n==="#00ff88"?t+=10:n==="#FFD700"&&(t+=5)}return t}function bi(e,t,i,n,r){return`<div id="duel-row-${n}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0), opacity .5s ease;transform-origin:center">
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${e.map((o,a)=>{const s=a<e.length-1?Ut(o,e[a+1]):null,d=!s||s==="#ff3333"||s==="#cc2222",c=s==="#00ff88"?"+10":s==="#FFD700"?"+5":"";return Xn(o,r),o.stadiumBonus||r&&(r.club_id&&String(o.club_id)===String(r.club_id)||r.country_code&&(o.country_code,r.country_code)),`
    ...
    </div>`}async function fr(e,t){const{state:i}=t,r=(i.params||{}).matchMode||"vs_ai_easy",o=r.replace("vs_ai_",""),a=r;await ki(e,t,r,async({deckId:s,formation:d,starters:c,subsRaw:l,gcCardsEnriched:m,gcDefs:b,stadiumDef:y})=>{try{let g=xi(c,d);y&&(g=si(g,y),yi(l,y));const p=await mr(d,o),u=p.lines||p,x=async h=>{try{const v=a==="vs_ai_club"?"club":a,{data:$,error:f}=await _.from("matches").insert({home_id:i.profile.id,away_id:null,mode:v,home_deck_id:s,status:"in_progress"}).select().single();if(f){console.error("[MatchIA] Erreur création match:",f),Tt(e,"⚠️","Impossible de créer le match ("+f.message+").","Retour",()=>t.navigate("home"));return}const z=p.stadiumDef||null;z&&u&&(si(u,z),yi(p.subs||[],z));const k={gcDefs:b||[],matchId:$==null?void 0:$.id,mode:a,difficulty:o,formation:d,homeTeam:g,aiTeam:u,homeSubs:l,subsUsed:0,maxSubs:Math.min(l.length,3),aiSubs:p.subs||[],aiSubsUsed:0,aiMaxSubs:Math.min((p.subs||[]).length,3),aiUsedSubIds:[],aiGcCards:p.gcCards||[],aiUsedGc:[],aiStadiumDef:z,homeScore:0,aiScore:0,gcCards:h,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};yr(e,k,t)}catch(v){console.error("[MatchIA] Exception launchMatch:",v),Tt(e,"⚠️","Erreur au lancement du match : "+v.message,"Retour",()=>t.navigate("home"))}};if(!m.length){x([]);return}_i(e,m,x)}catch(g){console.error("[MatchIA] Exception setup:",g),Tt(e,"⚠️","Erreur de préparation du match : "+g.message,"Retour",()=>t.navigate("home"))}})}async function mr(e,t){var u;const{data:i}=await _.from("players").select("id,firstname,surname_real,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(80);if(!i||i.length<11)return{lines:gr(e),subs:[],gcCards:[],stadiumDef:null};const n=Rt[e]||Rt["4-4-2"],r={GK:[],DEF:[],MIL:[],ATT:[]},o=new Set;function a(x,h,v){var $,f;return o.add(x.id),{cardId:"ai-"+x.id+"-"+v,id:x.id,firstname:x.firstname,name:x.surname_real,country_code:x.country_code,club_id:x.club_id,job:x.job,job2:x.job2,note_g:Number(x.note_g)||0,note_d:Number(x.note_d)||0,note_m:Number(x.note_m)||0,note_a:Number(x.note_a)||0,rarity:x.rarity,skin:x.skin,hair:x.hair,hair_length:x.hair_length,clubName:(($=x.clubs)==null?void 0:$.encoded_name)||null,clubLogo:((f=x.clubs)==null?void 0:f.logo_url)||null,boost:0,used:!1,_line:h}}for(const x of["GK","DEF","MIL","ATT"]){const h=i.filter(k=>k.job===x&&!o.has(k.id)),v=i.filter(k=>k.job!==x&&!o.has(k.id)),$=[...h,...v],f=[];for(let k=0;k<n[x];k++){const S=$[k];S&&f.push(a(S,x,k))}const z=yt(f.length);f.forEach((k,S)=>{k._col=z[S]}),r[x]=f}const d=i.filter(x=>!o.has(x.id)).slice(0,5).map((x,h)=>a(x,x.job,100+h)),m=Object.keys(je).sort(()=>Math.random()-.5).slice(0,3).map((x,h)=>{var v,$;return{id:"ai-gc-"+h,gc_type:x,name:((v=je[x])==null?void 0:v.name)||x,icon:(($=je[x])==null?void 0:$.icon)||"⚡"}}),b=Object.values(r).flat(),y={};b.forEach(x=>{x.club_id&&(y[x.club_id]=(y[x.club_id]||0)+1)});const g=(u=Object.entries(y).sort((x,h)=>h[1]-x[1])[0])==null?void 0:u[0];let p=null;if(g){const{data:x}=await _.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",g).single();x&&(p={club_id:x.id,country_code:null,name:x.encoded_name+" Stadium",club:{encoded_name:x.encoded_name,logo_url:x.logo_url}})}return{lines:r,subs:d,gcCards:m,stadiumDef:p}}function gr(e){const t=Rt[e]||Rt["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},n=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let r=0;for(const o of["GK","DEF","MIL","ATT"]){const a=[];for(let d=0;d<t[o];d++){const c=3+Math.floor(Math.random()*5);a.push({cardId:"fake-"+r,id:"fake-"+r,firstname:"IA",name:n[r%n.length],country_code:"XX",club_id:null,job:o,job2:null,note_g:o==="GK"?c:2,note_d:o==="DEF"?c:2,note_m:o==="MIL"?c:2,note_a:o==="ATT"?c:2,rarity:"normal",boost:0,used:!1,_line:o}),r++}const s=yt(a.length);a.forEach((d,c)=>{d._col=s[c]}),i[o]=a}return i}function yr(e,t,i){var r;zn("/sounds/match-opening.mp3",.3);const n=`<div style="position:relative;width:22px;height:22px;flex-shrink:0;display:flex;align-items:center;justify-content:center">
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const Y=t.pendingAttack;let ce="";if(t.selected.length>0){const U=t.selected.map(E=>({...(t.homeTeam[E._role]||[]).find(R=>R.cardId===E.cardId)||E,_line:E._role})),V=ai(U,t.modifiers.home,t.formation);ce=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
          ...
          </div>`}if(t.phase==="attack"&&t.selected.length>0){const Y=t.selected.map(U=>{const V=(t.homeTeam[U._role]||[]).find(G=>G.cardId===U.cardId)||U,E=["GK","DEF"].includes(U._role);return{...V,_line:U._role,...E?{note_a:Math.max(1,Number(V.note_a)||0)}:{}}}),ce=ri(Y,t.modifiers.home,t.formation);return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #FFD700;text-align:center">
          ...
          </div>`}const M=t.log[t.log.length-1];return M?'<div style="padding:2px 4px">'+Wn(M)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})()}
    </div>
    ...
    ${(()=>{const M=window.innerWidth>=700,Y=(B,O,pe)=>{var Ae,Be;const de=(t.gcDefs||[]).find(ke=>ke.name===B.gc_type),he=(de==null?void 0:de.name)||B.gc_type,Se=(de==null?void 0:de.effect)||((Ae=je[B.gc_type])==null?void 0:Ae.desc)||"",ve=de!=null&&de.image_url?`/icons/${de.image_url}`:null,Te=((Be=je[B.gc_type])==null?void 0:Be.icon)||"⚡";return`<div class="gc-mini" data-gc-id="${B.id}" data-gc-type="${B.gc_type}" style="flex-shrink:0;cursor:pointer">
          ${tt(he,ve,Te,Se,{width:O})}
        </div>`},ce=(B,O)=>{var pe;return`<div id="boost-card"
          style="box-sizing:border-box;width:${B}px;height:${O}px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:2px solid #87CEEB;border-radius:10px;cursor:pointer;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:${Math.round(O*.04)}px;text-align:center;flex-shrink:0">
            <div style="font-size:${Math.round(O*.2)}px">⚡</div>
            <div style="font-size:${Math.round(O*.09)}px;color:#000;font-weight:900">+${(pe=t.boostCard)==null?void 0:pe.value}</div>
          </div>`},U=(B,O)=>O?ce(95,162):Y(B,95),V=(B,O)=>O?ce(68,116):Y(B,68),E=M?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",G=h?`<button id="btn-results" style="${E};background:linear-gradient(135deg,#D4A017,#FFD700);border:none;color:#000">🏁 Résultats</button>`:p?`<div style="${E};background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);color:rgba(255,255,255,0.4)">⏳ Tour IA</div>`:f?`<button id="btn-pass" style="${E};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER (plus d'attaquants)</button>`:u?`<button id="btn-action" style="${E};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${t.selected.length===0?"disabled":""}> ⚔️ ATTAQUEZ <span id="match-timer" style="font-weight:900"></span></button>`:x?`<button id="btn-action" style="${E};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${t.selected.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="match-timer" style="font-weight:900"></span></button>`:`<div style="${E};background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1)"></div>`,R=u||x?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${t.selected.length}/3 sélectionné(s)</div>`:"",N=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${M?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(M=>{var Y,ce,U;if(M.type==="duel"){const V=M.isGoal,E=M.homeScored?"#FFD700":V?"#ff6b6b":"rgba(255,255,255,0.3)",G=M.homeScored?"⚽ BUT !":V?"⚽ BUT IA !":(Y=M.homePlayers)!=null&&Y.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${V?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${E};margin-bottom:4px">
                <div style="font-size:9px;color:${E};letter-spacing:1px;margin-bottom:5px;font-weight:700;text-transform:uppercase">${G}</div>
    ...
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      ...
      ${wi(e.aiTeam,e.formation,null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.1))}
            ...
            </div>`}}catch(Q){console.error("[Ranked] MMR update error:",Q)}(ie=document.getElementById("pvp-end-overlay"))==null||ie.remove();const D=document.createElement("div");D.id="pvp-end-overlay",D.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;color:#fff;padding:24px;text-align:center;overflow-y:auto";const F=I?"🤝":C?"🏆":"😞",H=I?"MATCH NUL":C?"VICTOIRE !":"DÉFAITE",J=I?"#fff":C?"#FFD700":"#ff6b6b";D.innerHTML=`
      ...
      ${P}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${l?"🏆 Retour à la Mini League":a?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(D),(te=D.querySelector("#pvp-end-home"))==null||te.addEventListener("click",()=>{D.remove(),it(e),l?y("mini-league",{openLeagueId:l}):y(a?"ranked":"home")})}const ee=_.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},w=>{var A;const L=w.new;try{if(L.status==="finished"||L.forfeit){if(z)return;z=!0,k&&(clearInterval(k),k=null),Ue(),L.game_state&&(f=L.game_state),oe(L);return}if(L.game_state){const C=f;f=L.game_state;const I=f._lastGC;if(I&&I.seq&&!ne.has(I.seq)&&(ne.add(I.seq),I.by!==p)){E(I.type,I.by,()=>Y());return}const P=C[p+"Score"]||0,D=C[u+"Score"]||0,F=f[p+"Score"]||0,H=f[u+"Score"]||0,J=F>P,ie=H>D;if((J||ie)&&!q.has(f.round)){q.add(f.round);const te=[...f.log||[]].reverse().find(se=>se.isGoal),Q=((te==null?void 0:te.homePlayers)||[]).map(se=>({name:se.name,note:se.note,portrait:se.portrait,job:se.job}));!!((A=te==null?void 0:te.text)!=null&&A.includes("DERNIER CORNER"))?Se().then(()=>{Qe(Q,F,H,J,()=>Y())}):Qe(Q,F,H,J,()=>Y());return}Y()}}catch(C){console.error("[PvP] crash:",C)}}).subscribe();async function j(w){Object.assign(f,w),f.lastActionAt=new Date().toISOString();const{error:L}=await _.from("matches").update({game_state:f}).eq("id",i);L&&g("Erreur de synchronisation","error");try{Y()}catch(A){console.error("[PvP] renderPvpScreen crash:",A)}}async function T(){if(z)return;z=!0,Gt(),Ue(),k&&(clearInterval(k),k=null);const w=n?v.away_id:v.home_id,L=n?"p2":"p1",A=n?"p1":"p2",C={...f,[L+"Score"]:3,[A+"Score"]:0,phase:"finished"},I=C.p1Score||0,P=C.p2Score||0;try{await _.from("matches").update({game_state:C}).eq("id",i);const{data:D,error:F}=await _.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:w,p_home_score:I,p_away_score:P,p_forfeit:!0});if(F&&console.error("[PvP] finish_pvp_match (forfeit):",F),l)try{await _.rpc("check_and_finish_mini_league",{p_league_id:l})}catch(H){console.error("[PvP] check_and_finish_mini_league (forfeit):",H)}try{const{data:H}=await _.rpc("apply_match_rewards",{p_match_id:i});H!=null&&H.success&&!(H!=null&&H.skipped)&&typeof t.refreshProfile=="function"&&await t.refreshProfile()}catch(H){console.error("[PvP] apply_match_rewards (forfeit):",H)}}catch{}try{_.removeChannel(ee)}catch{}setTimeout(()=>{it(e),l?y("mini-league",{openLeagueId:l}):y("home")},800)}const K={BOOST_STAT:({value:w=1,count:L=1,roles:A=[]},C,I)=>{const P=C[p+"Team"],D=Object.entries(P).filter(([F])=>!A.length||A.includes(F)).flatMap(([F,H])=>H.filter(J=>!J.used).map(J=>({...J,_line:F})));if(!D.length){C.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),I(C);return}Z(D,L,`Choisir ${L} joueur(s) à booster (+${w})`,F=>{F.forEach(H=>{const J=(P[H._line]||[]).find(ie=>ie.cardId===H.cardId);J&&(J.boost=(J.boost||0)+w,C.log.push({text:`⚡ +${w} sur ${J.name}`,type:"info"}))}),C[p+"Team"]=P,I(C)})},DEBUFF_STAT:({value:w=1,count:L=1,roles:A=[],target:C="ai"},I,P)=>{const D=C==="home"?p:u,F=I[D+"Team"],H=C==="home"?"allié":"adverse",J=Object.entries(F).filter(([ie])=>!A.length||A.includes(ie)).flatMap(([ie,te])=>te.map(Q=>({...Q,_line:ie})));if(!J.length){I.log.push({text:`🎯 Aucun joueur ${H}`,type:"info"}),P(I);return}Z(J,L,`Choisir ${L} joueur(s) ${H}(s) (-${w})`,ie=>{ie.forEach(te=>{const Q=(F[te._line]||[]).find(ae=>ae.cardId===te.cardId);Q&&(Q.boost=(Q.boost||0)-w,I.log.push({text:`🎯 -${w} sur ${Q.name}`,type:"info"}))}),I[D+"Team"]=F,P(I)})},GRAY_PLAYER:({count:w=1,roles:L=[],target:A="ai"},C,I)=>{const P=A==="home"?p:u,D=C[P+"Team"],F=A==="home"?"allié":"adverse",H=Object.entries(D).filter(([J])=>!L.length||L.includes(J)).flatMap(([J,ie])=>ie.filter(te=>!te.used).map(te=>({...te,_line:J})));if(!H.length){C.log.push({text:`❌ Aucun joueur ${F}`,type:"info"}),I(C);return}Z(H,w,`Choisir ${w} joueur(s) ${F}(s) à exclure`,J=>{const ie="usedCardIds_"+P,te=new Set(C[ie]||[]);J.forEach(Q=>{const ae=(D[Q._line]||[]).find(se=>se.cardId===Q.cardId);ae&&(ae.used=!0,te.add(Q.cardId),C.log.push({text:`❌ ${ae.name} exclu !`,type:"info"}))}),C[ie]=[...te],C[P+"Team"]=D,I(C)})},REVIVE_PLAYER:({count:w=1,roles:L=[]},A,C)=>{const I=A[p+"Team"],P=Object.entries(I).filter(([D])=>!L.length||L.includes(D)).flatMap(([D,F])=>F.filter(H=>H.used).map(H=>({...H,_line:D})));if(!P.length){A.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),C(A);return}Z(P,w,`Choisir ${w} joueur(s) à ressusciter`,D=>{D.forEach(F=>{const H=(I[F._line]||[]).find(J=>J.cardId===F.cardId);H&&(H.used=!1,A.log.push({text:`💫 ${H.name} ressuscité !`,type:"info"}))}),A[p+"Team"]=I,C(A)})},REMOVE_GOAL:({},w,L)=>{const A=u+"Score";w[A]>0?(w[A]--,w.log.push({text:"🚫 Dernier but annulé !",type:"info"})):w.log.push({text:"🚫 Aucun but à annuler",type:"info"}),L(w)},ADD_GOAL_DRAW:({},w,L)=>{w[p+"Score"]===w[u+"Score"]?(w[p+"Score"]++,w.log.push({text:"🎯 But bonus !",type:"info"})):w.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),L(w)},ADD_SUB:({value:w=1},L,A)=>{L.maxSubs=(L.maxSubs||3)+w,L.log.push({text:`🔄 +${w} remplacement(s)`,type:"info"}),A(L)},CUSTOM:({},w,L)=>L(w)};function Z(w,L,A,C){const I=document.createElement("div");I.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let P=[];function D(){var H,J;const F=w.map(ie=>{const te=ie._line||ie.job||"MIL",Q=P.find(se=>se.cardId===ie.cardId),ae=Ie({...ie,_evolution_bonus:0},{width:90,showStad:!0,role:te,extraNote:ie.boost||0});return`<div class="pp-item" data-cid="${ie.cardId}"
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
    ...
    </div>`,rt=f[p+"Team"],ao=(()=>{var me,ze,_e,Bt,at;if(F&&((me=f.pendingAttack)!=null&&me.players)){const Fe=f.pendingAttack;let Re="";if(H.length>0){const Me=H.map(We=>{const Vt=(rt[We._role]||[]).find(Li=>Li.cardId===We.cardId)||We,Yt=rt[We._role]||[],di=Yt.findIndex(Li=>Li.cardId===We.cardId),Ei=yt(Yt.length),Wt=di>=0?Ei[di]:Vt._col??1;return{...Vt,_line:We._role,_col:Wt}}),ft=ai(Me,((ze=f.modifiers)==null?void 0:ze[p])||{},f[p+"Formation"]);Re=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
        ...
        </div>`}if(D&&!((Bt=f.pendingAttack)!=null&&Bt.players)&&H.length>0){const Fe=H.map(Me=>{const ft=(rt[Me._role]||[]).find(Wt=>Wt.cardId===Me.cardId)||Me,We=["GK","DEF"].includes(Me._role),Vt=rt[Me._role]||[],Yt=Vt.findIndex(Wt=>Wt.cardId===Me.cardId),di=yt(Vt.length),Ei=Yt>=0?di[Yt]:ft._col??1;return{...ft,_line:Me._role,_col:Ei,...We?{note_a:Math.max(1,Number(ft.note_a)||0)}:{}}}),Re=ri(Fe,((at=f.modifiers)==null?void 0:at[p])||{},f[p+"Formation"]);return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #FFD700;text-align:center">
        ...
        </div>`}const X=(f.log||[]).slice(-1)[0];return X?'<div style="padding:2px 4px">'+Wn(X)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3);text-align:center">⏳ Match en cours...</div>'})(),nn=`
      <div style="display:flex;align-items:center;padding:8px 10px;background:rgba(0,0,0,0.5);gap:6px;flex-shrink:0">
      ...
      <div style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">${ao}</div>
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="width:min(90vw,420px)">${wi(f[u+"Team"],f[u+"Formation"],null,[],It(),Nt())}</div>
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(w),(L=w.querySelector("#pvp-opp-close"))==null||L.addEventListener("click",()=>w.remove())}function O(){var A;const w=f.log||[],L=document.createElement("div");L.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column",L.innerHTML=`
      ...
      <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(I,1400)});await L("⚽ DERNIER CORNER","#FFD700"),await L("🧤 LE GARDIEN MONTE !","#4fc3f7"),w.remove()}async function ve(w,L){const{side:A,gk:C}=w;if(!C)return;const I=A+"Score",P=(L[I]||0)+1;C.used=!0;const D=[...L.log||[]];D.push({type:"duel",isGoal:!0,homeScored:A===p,homePlayers:A==="p1"?[qt(C)]:[],aiPlayers:A==="p2"?[qt(C)]:[],text:`⚽ DERNIER CORNER — Le gardien de ${L[A+"Name"]} marque !`});const F=(L.round||0)+1,H=A==="p1"?"p2":"p1",J={...L,[I]:P},ie=$e(J);q.add(F),await Se();const te=A===p?P:L[p+"Score"]||0,Q=A===p?L[u+"Score"]||0:P;Qe([qt(C)],te,Q,A===p,async()=>{await j({[I]:P,log:D,round:F,pendingAttack:null,phase:ie?"finished":H+"-attack",attacker:H,["selected_"+p]:[],modifiers:{...f.modifiers,[p]:{}}}),ie&&await qe({...f,[I]:P})})}async function Te(){const w=f[p+"Team"],L=!["GK","DEF","MIL","ATT"].some(P=>(f[u+"Team"][P]||[]).some(D=>!D.used)),A=(f["selected_"+p]||[]).map(P=>{const D=(w[P._role]||[]).find(se=>se.cardId===P.cardId)||P,F=["GK","DEF"].includes(P._role),H=w[P._role]||[],J=H.findIndex(se=>se.cardId===P.cardId),ie=yt(H.length),te=J>=0?ie[J]:D._col??1,Q=f.stadiumDef||f.homeStadiumDef||null,ae=D.stadiumBonus||Q&&(Q.club_id&&String(D.club_id)===String(Q.club_id)||Q.country_code&&D.country_code===Q.country_code)||!1;return{...D,_line:P._role,_col:te,stadiumBonus:ae,...F?{note_a:Math.max(1,Number(D.note_a)||0)}:{}}});if(!A.length)return;const C=ri(A,f.modifiers[p]||{},f[p+"Formation"]);fe(p,A.map(P=>P.cardId)),A.forEach(P=>{const D=(w[P._role]||[]).find(F=>F.cardId===P.cardId);D&&(D.used=!0)}),f["selected_"+p]=[],Y();const I=[...f.log||[]];if(L){if(A.length===1&&(A[0]._line==="GK"||A[0].job==="GK")&&(f.p1Score||0)===(f.p2Score||0)){await ve({side:p,gk:A[0]},{...f,[p+"Team"]:w,log:I});return}const D=(f[p+"Score"]||0)+1,F=A.map(Q=>qt(Q));I.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:F,homeTotal:C.total,aiTotal:0});const H=(f.round||0)+1,J=p==="p1"?"p2":"p1",ie={...f,[p+"Team"]:w,[p+"Score"]:D},te=$e(ie);q.add(H),Qe(F,D,f[u+"Score"]||0,!0,async()=>{await j({[p+"Team"]:w,[p+"Score"]:D,["selected_"+p]:[],modifiers:{...f.modifiers,[p]:{}},pendingAttack:null,phase:te?"finished":J+"-attack",attacker:J,round:H,log:I}),te&&await qe({...f,[p+"Score"]:D})});return}I.push({type:"pending",text:`⚔️ ${f[p+"Name"]} attaque (${C.total})`}),await j({[p+"Team"]:w,[u+"Team"]:f[u+"Team"],pendingAttack:{...C,players:A,side:p},["selected_"+p]:[],modifiers:{...f.modifiers,[p]:{}},phase:u+"-defense",log:I})}async function Ae(){const w=f[p+"Team"],L=(f["selected_"+p]||[]).map(le=>{const ye=(w[le._role]||[]).find(Le=>Le.cardId===le.cardId)||le,be=w[le._role]||[],re=be.findIndex(Le=>Le.cardId===le.cardId),Ee=yt(be.length),Ne=re>=0?Ee[re]:ye._col??1;return{...ye,_line:le._role,_col:Ne}}),A=ai(L,f.modifiers[p]||{},f[p+"Formation"]);fe(p,L.map(le=>le.cardId)),L.forEach(le=>{const ye=(w[le._role]||[]).find(be=>be.cardId===le.cardId);ye&&(ye.used=!0)}),f["selected_"+p]=[],Y();const C=Ji(f.pendingAttack.total,A.total,f.modifiers[p]||{}),I=f.pendingAttack.side,P=C==="attack"||(C==null?void 0:C.goal),D=I==="p1"?"p2":"p1",F=(f.round||0)+1,H=(f.pendingAttack.players||[]).map(le=>qt(le)),J=[...f.log||[]];J.push({type:"duel",isGoal:P,homeScored:P&&I===p,text:P?`⚽ BUT de ${f[I+"Name"]} ! (${f.pendingAttack.total} vs ${A.total})`:`✋ Attaque stoppée (${f.pendingAttack.total} vs ${A.total})`,homePlayers:H,aiPlayers:L.map(le=>qt(le)),homeTotal:f.pendingAttack.total,aiTotal:A.total});const ie=P?(f[I+"Score"]||0)+1:f[I+"Score"]||0,te={...f,[p+"Team"]:w,[I+"Score"]:ie},Q=$e(te),ae=Q?"finished":D+"-attack",se=!Q&&!P?he(te):null,ge=async()=>{if(se){await ve(se,te);return}await j({[p+"Team"]:w,[u+"Team"]:f[u+"Team"],[I+"Score"]:ie,["selected_"+p]:[],modifiers:{...f.modifiers,[p]:{}},pendingAttack:null,phase:ae,attacker:D,round:F,log:J}),(ae==="finished"||Q)&&await qe({...f,[I+"Score"]:ie})};if(P){const le=I===p,ye=le?ie:f[p+"Score"]||0,be=le?f[u+"Score"]||0:ie;q.add(F),Qe(H,ye,be,le,ge)}else await ge()}function Be(w){return["MIL","ATT"].some(L=>(w[L]||[]).some(A=>!A.used))}function ke(w){return["GK","DEF","MIL","ATT"].some(L=>(w[L]||[]).some(A=>!A.used))}function wt(w){return ke(w)&&!Be(w)}function ct(w){const L=f[w+"Team"],A=f[(w==="p1"?"p2":"p1")+"Team"];return!!(Be(L)||wt(L)&&!ke(A)||wt(L)&&!Be(A)&&ke(A))}function $e(w){return!ke(w.p1Team)&&!ke(w.p2Team)}function Ce(w){const L=w.p1Score||0,A=w.p2Score||0;return L===A?null:L>A?v.home_id:v.away_id}async function qe(w){Gt(),Ue();try{const L=Ce(w),A=L?v.home_id===L?v.away_id:v.home_id:null,C=w.p1Score||0,I=w.p2Score||0,{data:P,error:D}=await _.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:L,p_home_score:C,p_away_score:I,p_forfeit:!1});if(D&&console.error("[PvP] finish_pvp_match:",D),l)try{await _.rpc("check_and_finish_mini_league",{p_league_id:l})}catch(F){console.error("[PvP] check_and_finish_mini_league:",F)}L&&A&&Co(L,A).catch(()=>{});try{const{data:F}=await _.rpc("apply_match_rewards",{p_match_id:i});F!=null&&F.success&&!(F!=null&&F.skipped)&&typeof t.refreshProfile=="function"&&await t.refreshProfile()}catch(F){console.error("[PvP] apply_match_rewards:",F)}if(typeof c=="function")try{await c({homeScore:C,awayScore:I,winnerId:L,homeId:v.home_id,awayId:v.away_id,matchId:i})}catch(F){console.error("[PvP] onMatchEnd:",F)}}catch(L){console.error("[PvP] finishMatch:",L)}}function Ge(){var I;const w=f[p+"Score"],L=f[u+"Score"],A=w>L,C=w===L;st(e),e.innerHTML=`
    ...
    </div>`,(I=document.getElementById("pvp-home"))==null||I.addEventListener("click",()=>{try{_.removeChannel(ee)}catch{}it(e),l?y("mini-league",{openLeagueId:l}):y("home")})}Y()}async function Gi(e,t,i=!1){var s,d;const{state:n,navigate:r,toast:o}=t,a=i&&((d=(s=t==null?void 0:t.state)==null?void 0:s.params)==null?void 0:d.rankedData)||null;await ki(e,t,i?"ranked":"random",async({deckId:c,formation:l,starters:m,subsRaw:b,gcCardsEnriched:y,gcDefs:g,stadiumDef:p})=>{st(e);const u=async x=>{await zr(e,t,c,l,m,b,x||[],g,p,i,a)};if(!(y!=null&&y.length)){await u([]);return}_i(e,y,u)})}async function Ar(e,t,i){return Sr(e,t,i)}async function zr(e,t,i,n,r,o,a=[],s=[],d=null,c=!1,l=null){var oe,ee;const{state:m,navigate:b,toast:y}=t,g=(l==null?void 0:l.mmr)??((oe=m.profile)==null?void 0:oe.mmr)??1e3,p=c?Ui(g):null,u=c?"linear-gradient(135deg, #1a0a2e 0%, #2d1060 50%, #1a0a2e 100%)":"linear-gradient(135deg, #0a1a2e 0%, #0d3d1e 100%)",x=c?(p==null?void 0:p.color)||"#D4A017":"#FFD700";let h=!1,v=null,$=null;e.innerHTML=`
    ...
    </div>`;const f=async()=>{h=!0,v&&(v.unsubscribe(),v=null),$&&(clearInterval($),$=null);try{await _.rpc("cancel_matchmaking",{p_user_id:m.profile.id})}catch(j){console.error("[Matchmaking] cancel error:",j)}};(ee=document.getElementById("cancel-mm"))==null||ee.addEventListener("click",async()=>{await f(),it(e),b("home")});const z=async(j,T)=>{h||(h=!0,v&&(v.unsubscribe(),v=null),$&&(clearInterval($),$=null),await Mt(e,t,j,T,{myGC:a,gcDefs:s,stadiumDef:d,isRanked:c,rankedData:l,onMatchEnd:c?Mr:null}))},k=m.user.id,{data:S,error:q}=c?await _.rpc("try_matchmake_ranked",{p_user_id:k,p_deck_id:i,p_mmr:g}):await _.rpc("try_matchmake",{p_user_id:k,p_deck_id:i});if(q||!(S!=null&&S.success)){console.error("[Matchmaking] try_matchmake error:",q||S),y("Erreur de matchmaking","error"),it(e),b("home");return}if(S.matched){await z(S.match_id,!1);return}const ne=async()=>{if(h)return;const{data:j}=await _.from("matchmaking_queue").select("status, match_id").eq("user_id",k).maybeSingle();(j==null?void 0:j.status)==="matched"&&j.match_id&&await z(j.match_id,!0)};v=_.channel(`mm_${k}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${k}`},j=>{const T=j.new;T.status==="matched"&&T.match_id&&z(T.match_id,!0)}).subscribe(),$=setInterval(ne,3e3),setTimeout(()=>{h||f()},12e4)}async function Mr(e,t){const{state:i,toast:n}=t,{winnerId:r,homeId:o,awayId:a,homeScore:s,awayScore:d,matchId:c}=e,l=i.user.id,m=o===l,b=r===l,{data:y}=await _.from("users").select("mmr, mmr_rd, mmr_v").eq("id",l).single(),{data:g}=await _.from("users").select("mmr, mmr_rd, mmr_v").eq("id",m?a:o).single();if(y&&g){const p=so(y.mmr,y.mmr_rd,y.mmr_v,g.mmr,g.mmr_rd,b?1:0);await _.from("users").update({mmr:p.mmr,mmr_rd:p.rd,mmr_v:p.v,mmr_wins:b?_.sql`mmr_wins + 1`:void 0,mmr_losses:b?void 0:_.sql`mmr_losses + 1`}).eq("id",l),n(b?`+MMR ↑ ${Math.round(p.mmr-y.mmr)}`:`-MMR ↓ ${Math.round(y.mmr-p.mmr)}`,b?"success":"error",4e3)}}const Cr=Object.freeze(Object.defineProperty({__proto__:null,renderMatchRandom:Gi,resumePvpMatch:Ar},Symbol.toStringTag,{value:"Module"}));async function jr(e,t){const{state:i,navigate:n,toast:r}=t,o=i.params||{},a=o.friendId||null,s=o.friendName||"Ami",d=!!o.isAccepting;await ki(e,t,"friend",async({deckId:c,formation:l,starters:m,subsRaw:b,gcCardsEnriched:y,gcDefs:g,stadiumDef:p})=>{st(e);const u=async x=>{const h=x||[];d?await qr(e,t,c,h,g,p):await Br(e,t,c,h,g,p,a,s)};if(!(y!=null&&y.length)){await u([]);return}_i(e,y,u)})}async function Br(e,t,i,n,r,o,a,s){var h;const{state:d,navigate:c,toast:l}=t,m=d.user.id,{data:b,error:y}=await _.from("friend_match_invites").insert({inviter_id:m,invitee_id:a,friend_id:a,status:"pending",inviter_deck_id:i,expires_at:new Date(Date.now()+2*60*1e3).toISOString()}).select("id").single();if(y||!b){console.error("[Friend] Erreur création invitation:",y),l("Impossible de créer l'invitation","error"),c("home");return}e.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    <div style="padding:14px 16px;background:var(--nav-bg,#0d1a0f);border-bottom:1px solid ${Lt};display:flex;align-items:center;justify-content:space-between">
      <div>
        ...
        <div style="font-size:12px;color:${eo}">Championnats 3 à 8 joueurs</div>
      </div>
      <div style="display:flex;align-items:center;gap:8px">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    <div style="padding:14px 16px;background:var(--nav-bg,#0d1a0f);border-bottom:1px solid ${Lt};display:flex;align-items:center;gap:10px">
      ...
      <div style="flex:1">
        ...
        <div style="font-size:11px;color:${eo}">${a.mode==="aller-retour"?"Aller-Retour":"Aller"} · max ${a.max_players} · 💰 ${p} cr./joueur</div>
      </div>
    ...
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:${Qt};border:1px solid ${Zt};border-radius:12px;padding:16px">
        ...
        ${Array.from({length:Math.max(0,a.status==="active"?a.current_day-1:a.current_day)},(j,T)=>T+1).reverse().map(j=>{const T=(d||[]).filter(K=>K.matchday===j);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:${Ze};margin-bottom:6px">Journée ${j}</div>${T.map(K=>$n(K,m,o,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
  ...
  </div>`;let d="buy";const c=()=>{var p,u,x,h,v,$,f;return{name:(((p=document.getElementById("flt-name"))==null?void 0:p.value)||"").toLowerCase().trim(),club:(((u=document.getElementById("flt-club"))==null?void 0:u.value)||"").toLowerCase().trim(),country:(((x=document.getElementById("flt-country"))==null?void 0:x.value)||"").toLowerCase().trim(),job:((h=document.getElementById("flt-job"))==null?void 0:h.value)||"",rarity:((v=document.getElementById("flt-rarity"))==null?void 0:v.value)||"",note1:parseInt(($=document.getElementById("flt-note1"))==null?void 0:$.value)||0,note2:parseInt((f=document.getElementById("flt-note2"))==null?void 0:f.value)||0}};function l(p){const u=c();return p.filter(x=>{var q,ne,oe;const h=(q=x.card)==null?void 0:q.player;if(!h)return!1;const v=`${h.firstname} ${h.surname_real}`.toLowerCase(),$=(((ne=h.clubs)==null?void 0:ne.encoded_name)||"").toLowerCase(),f=(h.country_code||"").toLowerCase(),z=((oe=x.card)==null?void 0:oe.evolution_bonus)||0,k=Dt(h,h.job,z),S=h.job2?Dt(h,h.job2,z):0;return!(u.name&&!v.includes(u.name)||u.club&&!$.includes(u.club)||u.country&&!f.includes(u.country)||u.job&&h.job!==u.job||u.rarity&&h.rarity!==u.rarity||u.note1&&k<u.note1||u.note2&&S<u.note2)})}function m(p){var S,q,ne,oe;const u=(S=p.card)==null?void 0:S.player;if(!u)return"";const x=((q=p.card)==null?void 0:q.evolution_bonus)||0,h=Dt(u,u.job,x),v=u.job2?Dt(u,u.job2,x):0,$=(i.profile.credits||0)>=p.price,f=u.country_code?`https://flagsapi.com/${u.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,z=fi[u.job]||"#bbb",k=u.job2?fi[u.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden">
      <!-- Drapeau -->
    ...
    </div>`}function b(p){var S,q,ne,oe;const u=(S=p.card)==null?void 0:S.player;if(!u)return"";const x=((q=p.card)==null?void 0:q.evolution_bonus)||0,h=Dt(u,u.job,x),v=u.job2?Dt(u,u.job2,x):0,$=p.status==="sold",f=u.country_code?`https://flagsapi.com/${u.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,z=fi[u.job]||"#bbb",k=u.job2?fi[u.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${$?"opacity:0.7":""}">
      ${f?`<img src="${f}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      ${(ne=u.clubs)!=null&&ne.logo_url?`<img src="${u.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${z};display:flex;align-items:center;justify-content:center">
  ...
  <div class="page">
    ...
    </div>`,(z=document.getElementById("ranked-back"))==null||z.addEventListener("click",()=>n("home"));return}const d=s.mmr??1e3,c=s.mmr_deviation??350,l=s.mmr_volatility??.06,m=s.placement_matches??0,b=m<10,y=Math.max(0,10-m),g=Ui(d),p=lo(d),u=Ti.findIndex(ne=>ne.id===g.id),x=Ti[u+1]||null,h={bronze:"linear-gradient(160deg,#3d1c00,#7a3e00)",silver:"linear-gradient(160deg,#1a1a2e,#3a3a5e)",gold:"linear-gradient(160deg,#1a1200,#4a3500)",platinum:"linear-gradient(160deg,#001a20,#003040)",diamond:"linear-gradient(160deg,#001030,#1a2860)",master:"linear-gradient(160deg,#1a0030,#3d0070)"},v=(s.ranked_wins||0)+(s.ranked_losses||0)+(s.ranked_draws||0),$=v>0?Math.round((s.ranked_wins||0)/v*100):0,f=Ti.map(ne=>`
  ...
  </div>`}co(uo);const xe={user:null,profile:null,page:"home",params:{}};function Ft(e,t="info",i=3e3){const n=document.getElementById("toast");n&&(n.textContent=e,n.className=`show ${t}`,clearTimeout(n._t),n._t=setTimeout(()=>{n.className=""},i))}function la(e,t,i=""){document.getElementById("modal-title").textContent=e,document.getElementById("modal-body").innerHTML=t,document.getElementById("modal-footer").innerHTML=i,document.getElementById("modal-overlay").classList.remove("hidden")}function Ri(){document.getElementById("modal-overlay").classList.add("hidden")}async function ei(){if(!xe.user)return;const{data:e}=await _.from("users").select("*").eq("id",xe.user.id).single();e&&(xe.profile=e)}const io="mw_theme";function no(){return localStorage.getItem(io)||"dark"}function ca(e){var t;localStorage.setItem(io,e),pa(e),(t=xe.profile)!=null&&t.id&&_.from("users").update({theme:e}).eq("id",xe.profile.id).then(()=>{})}function pa(e){document.documentElement.setAttribute("data-theme",e)}function Pt(e,t={}){xe.page=e,xe.params=t,oo()}async function oo(){var n,r,o,a;const e=document.getElementById("page-content");if(!e)return;document.querySelectorAll(".bottom-nav a").forEach(s=>{s.classList.toggle("active",s.dataset.page===xe.page)});const t=document.getElementById("nav-credits");t&&xe.profile&&(t.textContent=`💰 ${(xe.profile.credits||0).toLocaleString("fr")}`);const i={state:xe,navigate:Pt,toast:Ft,openModal:la,closeModal:Ri,refreshProfile:ei};switch(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽</div>',xe.page){case"home":await mn(e,i);break;case"settings":await Bn(e,i);break;case"collection":await Po(e,i);break;case"decks":await ji(e,i);break;case"boosters":await Qo(e,i);break;case"ranked":await sa(e,i);break;case"match":{const s=xe.params&&xe.params.matchMode||"vs_ai_easy";s==="random"?await Gi(e,i,!1):s==="ranked"?await Gi(e,i,!0):s==="friend"?await jr(e,i,(n=xe.params)==null?void 0:n.friendId,(r=xe.params)==null?void 0:r.friendName):s==="mini_league"||s==="mini-league"?await En(e,i,(o=xe.params)==null?void 0:o.mlMatchId,(a=xe.params)==null?void 0:a.leagueId):await fr(e,i);break}case"market":await ia(e,i);break;case"rankings":await aa(e,i);break;case"matches":await da(e,i);break;case"friends":await vo(e,i);break;case"mini-league":await Fr(e,i);break;case"match-mini-league":{const s=xe.params||{};await En(e,i,s.mlMatchId,s.leagueId);break}default:await mn(e,i)}}function ua(){var n,r;const e=document.getElementById("app"),t=xe.profile;if(!t)return;const i="/icons/";e.innerHTML=`
    <nav class="top-nav">
      <div class="logo" id="nav-logo" title="Manager Wars v2026.07.22-1704" style="cursor:pointer">
  ...
  </div>`,(n=e.querySelector("#pl-play-btn"))==null||n.addEventListener("click",t)}async function ga(){document.documentElement.setAttribute("data-theme",no()),document.getElementById("modal-overlay").addEventListener("click",i=>{i.target===i.currentTarget&&Ri()}),document.getElementById("modal-close").addEventListener("click",Ri);const{data:{session:e}}=await _.auth.getSession();if(!e){Ln(),ma(document.getElementById("app"),{onPlay:()=>cn(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:Ft})});return}xe.user=e.user,await ei(),Ln();try{const{data:i}=await _.from("formation_links_overrides").select("formation, links"),n={};(i||[]).forEach(r=>{n[r.formation]=r.links}),po(n)}catch(i){console.warn("Impossible de charger les overrides de formation:",i)}if(!xe.profile){mo(document.getElementById("app"),{state:xe,navigate:async()=>{await ei(),zi()},toast:Ft,refreshProfile:ei});return}const t=Array.isArray(xe.profile.pending_boosters)?xe.profile.pending_boosters:[];if(!xe.profile.onboarding_done&&t.length>0){ar(document.getElementById("app"),{state:xe,navigate:()=>zi(),toast:Ft,refreshProfile:ei});return}zi(),setTimeout(()=>ta(xe.profile,Pt,Ft),800),_.auth.onAuthStateChange(async(i,n)=>{i==="SIGNED_OUT"&&(xe.user=null,xe.profile=null,document.getElementById("app").innerHTML="",cn(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:Ft}))})}function ya(){return Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight)}function $i(){const e=document.getElementById("app");e&&(e.style.height=ya()+"px")}window.addEventListener("resize",$i);window.addEventListener("orientationchange",()=>setTimeout($i,150));window.visualViewport&&window.visualViewport.addEventListener("resize",$i);function zi(){const e=()=>{var i;(i=xe.user)!=null&&i.id&&_.from("users").update({last_seen_at:new Date().toISOString()}).eq("id",xe.user.id).then(()=>{})};e(),window._presencePingInterval&&clearInterval(window._presencePingInterval),window._presencePingInterval=setInterval(e,6e4);const t=document.getElementById("app");t.style.display="flex",t.style.flexDirection="column",$i(),ua(),oo()}function Ln(){const e=document.getElementById("app-loader"),t=document.getElementById("app");t&&(t.style.display=""),e&&(e.classList.add("zoom-out"),setTimeout(()=>e.style.display="none",500))}function ro(e){var n;const t=document.getElementById("app-loader");if(t&&(t.style.display="none"),document.getElementById("boot-error"))return;const i=document.createElement("div");i.id="boot-error",i.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:99999;gap:16px;color:#fff;padding:24px;text-align:center",i.innerHTML=`
    <div style="font-size:42px">📡</div>
    <div style="font-size:18px;font-weight:900">Connexion impossible</div>
    <div style="font-size:13px;color:rgba(255,255,255,0.6);max-width:280px">${e||"Le chargement a échoué. Vérifie ta connexion et réessaie."}</div>
    <button id="boot-retry" style="margin-top:8px;padding:12px 30px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer">Réessayer</button>`,document.body.appendChild(i),(n=document.getElementById("boot-retry"))==null||n.addEventListener("click",()=>window.location.reload())}ga().catch(e=>{console.error("Échec du démarrage:",e),ro()});setTimeout(()=>{const e=document.getElementById("app-loader");e&&e.style.display!=="none"&&!e.classList.contains("zoom-out")&&!document.getElementById("boot-error")&&ro("Le serveur met trop de temps à répondre.")},12e3);
