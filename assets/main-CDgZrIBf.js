import{s as E,F as yi,h as Pt,j as Yi,l as at,i as zn,k as Sn,b as Ln}from"./formation-links-DJC25wD7.js";const In="/manager-wars/";function Li(e,{navigate:t,toast:i}){let d="login";const c=()=>{var s,l,p,n,o,g;const a=d==="login";e.innerHTML=`
    <div style="
  ...
  `;function l(){var T;const n=document.getElementById("logo-preview"),o=document.getElementById("logo-initials"),g=((T=document.getElementById("setup-club"))==null?void 0:T.value)||"MW",v=g.trim().split(" ").filter(Boolean),y=v.length>=2?(v[0][0]+v[1][0]).toUpperCase():g.slice(0,2).toUpperCase();n&&(n.style.background=s,n.style.borderColor=a),o&&(o.textContent=y,o.style.color=a)}document.getElementById("color1").addEventListener("input",n=>{a=n.target.value,document.getElementById("swatch1").style.background=a,l()}),document.getElementById("color2").addEventListener("input",n=>{s=n.target.value,document.getElementById("swatch2").style.background=s,l()});function p(n){document.querySelectorAll(".setup-step").forEach(o=>o.classList.remove("active")),document.getElementById(`step-${n}`).classList.add("active"),document.getElementById("step-num").textContent=n,document.getElementById("progress-fill").style.width=`${Math.round(n/3*100)}%`,n===3&&l()}document.getElementById("step1-next").addEventListener("click",async()=>{const n=document.getElementById("setup-pseudo").value.trim(),o=document.getElementById("step1-error");if(o.textContent="",n.length<3){o.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:g}=await E.from("users").select("id").eq("pseudo",n).maybeSingle();if(g){o.textContent="Ce pseudo est déjà pris.";return}p(2)}),document.getElementById("step2-back").addEventListener("click",()=>p(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const n=document.getElementById("setup-club").value.trim(),o=document.getElementById("step2-error");if(o.textContent="",n.length<2){o.textContent="Nom trop court (min. 2 caractères).";return}const{data:g}=await E.from("users").select("id").eq("club_name",n).maybeSingle();if(g){o.textContent="Ce nom de club est déjà pris.";return}p(3)}),document.getElementById("step3-back").addEventListener("click",()=>p(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const n=document.getElementById("setup-pseudo").value.trim(),o=document.getElementById("setup-club").value.trim(),g=document.getElementById("step3-error"),v=document.getElementById("step3-finish");g.textContent="",v.disabled=!0,v.textContent="Création en cours…";try{const{error:y}=await E.from("users").insert({id:t.user.id,pseudo:n,club_name:o,club_color1:a,club_color2:s,credits:1e4});if(y)throw y;await Cn(t.user.id),await c(),d(`Bienvenue ${n} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(y){g.textContent=y.message,v.disabled=!1,v.textContent="🚀 Créer mon profil !"}})}async function Cn(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await E.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const jn="modulepreload",qn=function(e){return"/manager-wars/"+e},Ii={},bi=function(t,i,d){let c=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const s=document.querySelector("meta[property=csp-nonce]"),l=(s==null?void 0:s.nonce)||(s==null?void 0:s.getAttribute("nonce"));c=Promise.allSettled(i.map(p=>{if(p=qn(p),p in Ii)return;Ii[p]=!0;const n=p.endsWith(".css"),o=n?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${p}"]${o}`))return;const g=document.createElement("link");if(g.rel=n?"stylesheet":jn,n||(g.as="script"),g.crossOrigin="",g.href=p,l&&g.setAttribute("nonce",l),document.head.appendChild(g),n)return new Promise((v,y)=>{g.addEventListener("load",v),g.addEventListener("error",()=>y(new Error(`Unable to preload CSS for ${p}`)))})}))}function a(s){const l=new Event("vite:preloadError",{cancelable:!0});if(l.payload=s,window.dispatchEvent(l),!l.defaultPrevented)throw s}return c.then(s=>{for(const l of s||[])l.status==="rejected"&&a(l.reason);return t().catch(a)})},Bt="#1A6B3C",Dt="#cc2222",Fn="#D4A017",Mi="#888";async function Bn(e,t){const{state:i,toast:d}=t;e.innerHTML=`
    ...
    </div>`,await Wi(i,d,t),document.getElementById("btn-add-friend").addEventListener("click",()=>Gn(i,d)),document.getElementById("btn-accept-friend").addEventListener("click",()=>Ji(i,d,null,t))}async function Wi(e,t,i={}){const{navigate:d}=i,c=e.user.id,{data:a,error:s}=await E.from("friendships").select("id, requester_id, addressee_id").eq("status","accepted").or(`requester_id.eq.${c},addressee_id.eq.${c}`),{count:l}=await E.from("friendships").select("id",{count:"exact",head:!0}).eq("addressee_id",c).eq("status","pending"),p=document.getElementById("pending-badge");p&&(l>0?(p.style.display="flex",p.textContent=l):p.style.display="none");const n=document.getElementById("friends-list");if(!n)return;if(s){console.error("[Friends] Erreur:",s),n.innerHTML=`<div style="color:${Dt};text-align:center;padding:16px">Erreur chargement : ${s.message}</div>`;return}const o=(a||[]).map(y=>y.requester_id===c?y.addressee_id:y.requester_id);let g={};if(o.length){const{data:y}=await E.from("users").select("id, pseudo, club_name, last_seen_at, club_color1, club_color2").in("id",o);(y||[]).forEach(T=>{g[T.id]=T})}const v=(a||[]).map(y=>({friendshipId:y.id,friend:g[y.requester_id===c?y.addressee_id:y.requester_id]||{pseudo:"?"}}));if(!v.length){n.innerHTML=`
    ...
    <div style="display:flex;align-items:center;gap:12px;background:#fff;border-radius:12px;padding:12px 14px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
      ...
      <div style="flex:1;min-width:0">
        ...
        <div style="font-size:11px;color:#888;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">@${d}</div>
    ...
    </div>`}function Gn(e,t){const i=hi();i.innerHTML=`
    <div class="popup-box">
    ...
    ${vi()}`,document.body.appendChild(i);const d=i.querySelector("#friend-pseudo-input"),c=i.querySelector("#add-friend-error"),a=()=>i.remove();d.focus(),i.querySelector("#add-cancel").addEventListener("click",a),i.addEventListener("click",s=>{s.target===i&&a()}),i.querySelector("#add-ok").addEventListener("click",async()=>{const s=d.value.trim();if(!s){c.textContent="Entre un pseudo";return}c.textContent="";const{data:l}=await E.from("users").select("id, pseudo").ilike("pseudo",s).single();if(!l){c.textContent="Utilisateur introuvable";return}if(l.id===e.user.id){c.textContent="Tu ne peux pas t'ajouter toi-même";return}const{data:p}=await E.from("friendships").select("id, status").or(`and(requester_id.eq.${e.user.id},addressee_id.eq.${l.id}),and(requester_id.eq.${l.id},addressee_id.eq.${e.user.id})`).single();if(p){const o=p.status==="accepted"?"Vous êtes déjà amis !":p.status==="pending"?"Demande déjà envoyée":"Une demande existe déjà";c.textContent=o;return}const{error:n}=await E.from("friendships").insert({requester_id:e.user.id,addressee_id:l.id,status:"pending"});if(n){c.textContent="Erreur : "+n.message;return}a(),t(`✅ Demande envoyée à ${l.pseudo} !`,"success")})}async function Ji(e,t,i=null,d={}){const c=e.user.id,{data:a}=await E.from("friendships").select("id, requester_id").eq("addressee_id",c).eq("status","pending").order("created_at",{ascending:!1}),s=(a||[]).map(v=>v.requester_id);let l={};if(s.length){const{data:v}=await E.from("users").select("id, pseudo, club_name").in("id",s);(v||[]).forEach(y=>{l[y.id]=y})}const p=(a||[]).map(v=>({...v,requester:l[v.requester_id]||{pseudo:"?"}})),n=hi(),o=p||[];n.innerHTML=`
    <div class="popup-box">
    ...
    ${vi()}`,document.body.appendChild(n);const g=()=>n.remove();n.querySelector("#pending-close").addEventListener("click",g),n.addEventListener("click",v=>{v.target===n&&g()}),n.querySelectorAll("[data-accept]").forEach(v=>{v.addEventListener("click",async()=>{const{error:y}=await E.from("friendships").update({status:"accepted"}).eq("id",v.dataset.accept);if(y){t("Erreur : "+y.message,"error");return}v.closest("div[style]").remove(),t("✅ Ami accepté !","success"),Wi(e,t,d),i&&i()})}),n.querySelectorAll("[data-decline]").forEach(v=>{v.addEventListener("click",async()=>{await E.from("friendships").delete().eq("id",v.dataset.decline),v.closest("div[style]").remove(),t("Demande refusée","info"),i&&i()})})}async function Pn(e,t,i){const d=e.user.id,[c,a]=[d,t].sort(),s=d===c,{data:l}=await E.from("friend_match_stats").select("*").eq("player1_id",c).eq("player2_id",a).single(),p=e.profile.club_name||e.profile.pseudo||"Moi",n=l||{},o=s?n.wins_p1||0:n.wins_p2||0,g=s?n.wins_p2||0:n.wins_p1||0,v=n.draws||0,y=s?n.goals_p1||0:n.goals_p2||0,T=s?n.goals_p2||0:n.goals_p1||0,r=s?n.gc_used_p1||0:n.gc_used_p2||0,u=s?n.gc_used_p2||0:n.gc_used_p1||0,M=n.matches_total||0,L=(K,C,q,D=Bt,X=Dt)=>`
    ...
    ${vi()}`,document.body.appendChild(N),N.querySelector("#stats-close").addEventListener("click",()=>N.remove()),N.addEventListener("click",K=>{K.target===N&&N.remove()})}function hi(){const e=document.createElement("div");return e.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:8000;display:flex;align-items:center;justify-content:center;padding:20px",e}function vi(){return`
    <style>
      .popup-box {
    ...
    </style>`}async function Xi({player1Id:e,player2Id:t,score1:i,score2:d,gc1:c,gc2:a}){const[s,l]=[e,t].sort(),p=e!==s,n=p?d:i,o=p?i:d,g=p?a:c,v=p?c:a,y=n>o?1:0,T=o>n?1:0,r=n===o?1:0,{data:u}=await E.from("friend_match_stats").select("*").eq("player1_id",s).eq("player2_id",l).single();u?await E.from("friend_match_stats").update({wins_p1:u.wins_p1+y,wins_p2:u.wins_p2+T,draws:u.draws+r,goals_p1:u.goals_p1+n,goals_p2:u.goals_p2+o,gc_used_p1:u.gc_used_p1+g,gc_used_p2:u.gc_used_p2+v,matches_total:u.matches_total+1}).eq("player1_id",s).eq("player2_id",l):await E.from("friend_match_stats").insert({player1_id:s,player2_id:l,wins_p1:y,wins_p2:T,draws:r,goals_p1:n,goals_p2:o,gc_used_p1:g,gc_used_p2:v,matches_total:1})}const Nn="2026.07.03-2133";async function Ci(e,{state:t,navigate:i,toast:d}){var a,s;const c=t.profile;c&&(e.innerHTML=`
  <style>
  ...
  `,e.querySelectorAll("[data-nav]").forEach(l=>{l.addEventListener("click",p=>{p.preventDefault(),i(l.dataset.nav)})}),(a=document.getElementById("nav-rankings"))==null||a.addEventListener("click",()=>i("rankings")),(s=document.getElementById("nav-matches"))==null||s.addEventListener("click",()=>i("matches")),e.querySelectorAll("[data-action]").forEach(l=>{l.addEventListener("click",()=>{l.classList.add("tapped"),setTimeout(()=>l.classList.remove("tapped"),200);const p=l.dataset.action;if(p==="match-ai"){Un(i);return}if(p==="match-random"){i("match",{matchMode:"random"});return}if(p==="match-friend"){i("friends");return}if(p==="mini-league"){i("mini-league");return}if(p==="ranked"){ctx.toast("Mode Ranked — Bientôt disponible !","info");return}d("Bientôt disponible","info")})}),window.innerWidth<768&&requestAnimationFrame(()=>{var K,C,q,D,X,se;const l=((K=window.visualViewport)==null?void 0:K.height)||window.innerHeight,p=((C=document.querySelector(".top-nav"))==null?void 0:C.offsetHeight)||56,n=((q=document.querySelector(".bottom-nav"))==null?void 0:q.offsetHeight)||60,o=((D=e.querySelector(".hero-compact"))==null?void 0:D.offsetHeight)||52,g=e.querySelector(".ranked-banner");e.querySelector(".play-grid");const v=((se=(X=e.querySelector("#logout-btn"))==null?void 0:X.closest("div"))==null?void 0:se.offsetHeight)||44,y=["friend-requests-banner","match-invite-banner","ongoing-match-banner"].reduce((oe,P)=>{var J;return oe+(((J=document.getElementById(P))==null?void 0:J.offsetHeight)||0)},0),T=14*5,r=l-p-n-o-v-y-T,u=Math.max(80,Math.round(r*.28)),M=Math.max(160,Math.round(r*.72)),L=Math.floor((M-10)/2);g&&(g.style.minHeight=g.style.maxHeight=u+"px"),e.querySelectorAll(".play-grid .play-card").forEach(oe=>{oe.style.minHeight=oe.style.height=L+"px"});const N=Math.round(L*.55);e.querySelectorAll(".play-card .play-icon").forEach(oe=>{oe.style.height=N+"px"})}),document.getElementById("logout-btn").addEventListener("click",async()=>{await E.auth.signOut(),window.location.reload()}),Qi(t,d),Hn(t,d,i),Zi(t,d,i))}async function Zi(e,t,i){const d=document.getElementById("ongoing-match-banner");if(!d)return;const c=e.profile.id,{data:a}=await E.from("matches").select("id, home_id, away_id, status, mode").eq("status","active").or(`home_id.eq.${c},away_id.eq.${c}`).order("created_at",{ascending:!1});if(!(a!=null&&a.length)){d.innerHTML="";return}const s=a.map(p=>p.home_id===c?p.away_id:p.home_id).filter(Boolean);let l={};if(s.length){const{data:p}=await E.from("users").select("id, pseudo, club_name").in("id",s);(p||[]).forEach(n=>{l[n.id]=n.club_name||n.pseudo})}d.innerHTML=a.map(p=>{const n=p.home_id===c?p.away_id:p.home_id,o=l[n]||"Adversaire",g=p.mode==="mini_league";return`
      ...
      </div>`}).join(""),d.querySelectorAll("[data-resume]").forEach(p=>{p.addEventListener("click",async()=>{const n=document.getElementById("page-content")||document.getElementById("app");if(p.dataset.mini==="1"){const{data:o}=await E.from("mini_league_matches").select("id, league_id").eq("match_id",p.dataset.resume).single();o?i("match-mini-league",{mlMatchId:o.id,leagueId:o.league_id}):i("mini-league")}else{const{resumePvpMatch:o}=await bi(async()=>{const{resumePvpMatch:g}=await Promise.resolve().then(()=>Uo);return{resumePvpMatch:g}},void 0);o(n,{state:e,navigate:i,toast:t,openModal:null,closeModal:null,refreshProfile:null},p.dataset.resume)}})}),d.querySelectorAll("[data-abandon]").forEach(p=>{p.addEventListener("click",()=>{On(async()=>{await Rn(p.dataset.abandon,p.dataset.opp,c),t("Match abandonné (défaite 3-0)","info"),Zi(e,t,i)})})})}async function Rn(e,t,i){const{data:d}=await E.from("matches").select("home_id, away_id, game_state").eq("id",e).single();if(!d)return;const c=d.home_id===i,a=c?0:3,s=c?3:0,l=d.game_state||{};l.p1Score=a,l.p2Score=s,l.phase="finished",l.forfeit=!0,await E.from("matches").update({status:"finished",forfeit:!0,winner_id:t,home_score:a,away_score:s,game_state:l}).eq("id",e)}function On(e){const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",t.innerHTML=`
    ...
    </div>`,document.body.appendChild(t),t.querySelector("#ab-cancel").addEventListener("click",()=>t.remove()),t.querySelector("#ab-ok").addEventListener("click",()=>{t.remove(),e()}),t.addEventListener("click",i=>{i.target===t&&t.remove()})}async function Hn(e,t,i){var l,p,n,o;const d=document.getElementById("match-invite-banner");if(!d)return;const{data:c}=await E.from("friend_match_invites").select("id, inviter_id, inviter:users!inviter_id(pseudo, club_name)").eq("invitee_id",e.user.id).eq("status","pending").order("created_at",{ascending:!1}).limit(1).maybeSingle();if(!c){d.innerHTML="";return}const a=((l=c.inviter)==null?void 0:l.club_name)||((p=c.inviter)==null?void 0:p.pseudo)||"?",s=c.inviter_id;d.innerHTML=`
    ...
    </div>`,(n=document.getElementById("match-inv-accept"))==null||n.addEventListener("click",()=>{d.innerHTML="",i("match",{matchMode:"friend",friendId:s,friendName:a})}),(o=document.getElementById("match-inv-decline"))==null||o.addEventListener("click",async()=>{await E.from("friend_match_invites").update({status:"declined"}).eq("id",c.id),d.innerHTML="",t("Invitation refusée","info")})}async function Qi(e,t){const i=document.getElementById("friend-requests-banner");if(!i)return;const{data:d,error:c}=await E.from("friendships").select("id, requester:users!requester_id(pseudo, club_name)").eq("addressee_id",e.user.id).eq("status","pending");if(c||!(d!=null&&d.length)){i.innerHTML="";return}const a=d.length,s=d.slice(0,2).map(p=>{var n;return((n=p.requester)==null?void 0:n.pseudo)||"?"}).join(", "),l=a>2?` +${a-2}`:"";i.innerHTML=`
  ...
  `,document.body.appendChild(i);const d=()=>i.remove();document.getElementById("diff-cancel").addEventListener("click",d),i.addEventListener("click",c=>{c.target===i&&d()}),i.querySelectorAll("[data-mode]").forEach(c=>{c.addEventListener("click",()=>{d(),e("match",{matchMode:c.dataset.mode})})})}const Ke={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé pour ce match."},"Double attaque":{icon:"⚡",desc:"La note d'attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la note d'un joueur adverse."},Gel:{icon:"❄️",desc:"Bloque le meilleur attaquant IA."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function ze(e,t){if(!e)return 0;switch(t){case"GK":return Number(e.note_g)||0;case"DEF":return Number(e.note_d)||0;case"MIL":return Number(e.note_m)||0;case"ATT":return Number(e.note_a)||0;default:return 0}}const ji=["ATT","MIL","DEF","GK"];function en(e){let t=0;const i=e.length;for(let d=0;d<i;d++)for(let c=d+1;c<i;c++){const a=e[d],s=e[c];if(!a||!s)continue;const l=a._col!=null&&s._col!=null&&a._col===s._col,p=a._col!=null&&s._col!=null&&Math.abs(a._col-s._col)===1,n=ji.indexOf(a._line||a.job),o=ji.indexOf(s._line||s.job),g=Math.abs(n-o)===1;if(!((a._line||a.job)===(s._line||s.job)&&p||l&&g))continue;const T=a.country_code&&s.country_code&&a.country_code===s.country_code,r=a.club_id&&s.club_id&&a.club_id===s.club_id;T&&r?t+=2:(T||r)&&(t+=1)}return t}function Nt(e,t={}){const i=e.reduce((a,s)=>{const l=s._line||s.job;return a+(Number(l==="MIL"?s.note_m:s.note_a)||0)+(s.boost||0)},0),d=en(e);let c=i+d;return t.doubleAttack&&(c*=2),t.stolenNote&&(c-=t.stolenNote),{base:i,links:d,total:Math.max(0,c)}}function Rt(e,t={}){const i=e.reduce((a,s)=>{const l=s._line||s.job;let p=0;return l==="GK"?p=Number(s.note_g)||0:l==="MIL"?p=Number(s.note_m)||0:p=Number(s.note_d)||0,a+p+(s.boost||0)},0),d=en(e);let c=i+d;return t.stolenNote&&(c-=t.stolenNote),{base:i,links:d,total:Math.max(0,c)}}function Ot(e,t,i={}){return i.shield?{goal:!1,shielded:!0}:{goal:e>t,shielded:!1}}function tn(e,t,i="easy"){const d=e.filter(s=>!s.used);if(!d.length)return[];const c=[...d].sort((s,l)=>{const p=t==="attack"?ze(s,"ATT"):s._line==="GK"?ze(s,"GK"):ze(s,"DEF");return(t==="attack"?ze(l,"ATT"):l._line==="GK"?ze(l,"GK"):ze(l,"DEF"))-p});let a=i==="easy"?1+Math.floor(Math.random()*2):i==="medium"?2+Math.floor(Math.random()*2):3;return c.slice(0,Math.min(a,c.length,3))}function Vn(e,t){const i={vs_ai_easy:{victoire:500,nul:250,defaite:50},vs_ai_medium:{victoire:1e3,nul:500,defaite:50},vs_ai_hard:{victoire:1500,nul:750,defaite:100},vs_ai_club:{victoire:2500,nul:1250,defaite:100}};return(i[e]||i.vs_ai_easy)[t]||0}const nn={pepite:{win:2,loss:-1,boosterStart:"min"},papyte:{win:1,loss:-2,boosterStart:"max"}};function wi(e,t){const i=e.player;if(!i)return 0;const d=i.rarity;if(d!=="pepite"&&d!=="papyte")return Number(i[t])||0;const c=_i(i),a=Number(i[t])||0;if(a<=0)return 0;const s=i.note_min??1,l=i.note_max??10,n=(e.current_note??c)-c;return Math.min(l,Math.max(s,a+n))}function _i(e){const t=e.job||"ATT";return Number(t==="GK"?e.note_g:t==="DEF"?e.note_d:t==="MIL"?e.note_m:e.note_a)||0}async function $i(e,t){!e||!t||await Promise.all([li(e,"win"),li(t,"loss")])}async function li(e,t){const{data:i}=await E.from("cards").select("id, current_note, player:players(rarity, job, note_g, note_d, note_m, note_a, note_min, note_max)").eq("owner_id",e).eq("card_type","player");if(!(i!=null&&i.length))return;const d=i.filter(c=>{var a,s;return((a=c.player)==null?void 0:a.rarity)==="pepite"||((s=c.player)==null?void 0:s.rarity)==="papyte"});d.length&&await Promise.all(d.map(c=>{const a=nn[c.player.rarity],s=t==="win"?a.win:a.loss,l=c.player.note_min??1,p=c.player.note_max??10,n=_i(c.player),o=c.current_note??n,g=Math.min(p,Math.max(l,o+s));return E.from("cards").update({current_note:g}).eq("id",c.id)}))}async function Kn(e,t){return li(e,t)}const on=Object.freeze(Object.defineProperty({__proto__:null,EVOLUTIVE_RULES:nn,applyOwnEvolution:Kn,currentSecondaryNote:wi,getBaseMainNote:_i,updateEvolutiveCards:$i},Symbol.toStringTag,{value:"Module"})),rn={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"},jt={GK:"#111111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},Vt=["GK","DEF","MIL","ATT"],Yn=["Tous","GK","DEF","MIL","ATT"],Wn={normal:1e3,pepite:5e3,papyte:5e3,legende:1e4};function qi(e){const t=e.player;if(!t)return null;const i=t.rarity;return(i==="pepite"||i==="papyte")&&e.current_note!=null?e.current_note:Math.max(Number(t.note_g)||0,Number(t.note_d)||0,Number(t.note_m)||0,Number(t.note_a)||0)}function Jn(e){return e.length?e.reduce((t,i)=>qi(i)>qi(t)?i:t,e[0]):e[0]}function an(e){return e==="GK"?"note_g":e==="DEF"?"note_d":e==="MIL"?"note_m":"note_a"}const ki={MA:"MAROC",FR:"FRANCE",AR:"ARGENTINE",PT:"PORTUGAL",BR:"BRESIL",ES:"ESPAGNE",DE:"ALLEMAGNE",GB:"ANGLETERRE",IT:"ITALIE",CM:"CAMEROUN",SN:"SENEGAL",NG:"NIGERIA",DK:"DANEMARK",NL:"PAYS-BAS",BE:"BELGIQUE",CI:"CÔTE D'IVOIRE",AL:"ALBANIE",HR:"CROATIE",RS:"SERBIE",TR:"TURQUIE"};function sn(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";if(!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function Gt(e,t){return e&&Number(t==="GK"?e.note_g:t==="DEF"?e.note_d:t==="MIL"?e.note_m:e.note_a)||0}function ai(e,t=""){var v,y;const i=e.player;if(!i)return"";const d=i.job||"ATT",c=jt[d],a=rn[i.rarity]||"#ccc",s=i.rarity==="pepite"||i.rarity==="papyte",l=s&&e.current_note!=null?e.current_note:Gt(i,d),p=i.job2?s?wi(e,an(i.job2)):Gt(i,i.job2):null,n=i.job2?jt[i.job2]:null,o=sn(i),g=ki[i.country_code]||i.country_code||"";return`
  <div style="
  ...
  </div>`}function Fi(e){const t=e.job||"ATT",i=Gt(e,t),d=ki[e.country_code]||e.country_code||"";return`
  <div style="
    width:140px;border-radius:12px;padding:6px;
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
  ...
  </div>`}async function Xn(e,t){const{state:i,navigate:d,toast:c,openModal:a,closeModal:s}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:l}=await E.from("cards").select(`id, card_type, current_note, gc_type, formation, is_for_sale, sale_price, stadium_id,
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),n=(l||[]).filter(R=>R.card_type==="player"&&R.player),o=(l||[]).filter(R=>R.card_type==="game_changer"),g=(l||[]).filter(R=>R.card_type==="formation"),v=(l||[]).filter(R=>R.card_type==="stadium"),{data:y}=await E.from("gc_definitions").select("name,gc_type,color,effect,image_url"),T={};(y||[]).forEach(R=>{T[R.name]=R});const{data:r}=await E.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),u={};(r||[]).forEach(R=>{u[R.id]=R}),v.forEach(R=>{R.stadium_def&&(u[R.stadium_id]=R.stadium_def)});const M=Object.keys(yi),L=Object.keys(Ke),N={};n.forEach(R=>{const Z=R.player.id;N[Z]=(N[Z]||0)+1}),new Set(Object.keys(N).map(R=>String(R)));const K=new Set(g.map(R=>R.formation)),C=new Set(o.map(R=>R.gc_type));let q="player",D="Tous",X="",se=!1;function oe(){return[...n].sort((R,Z)=>{const ge=Vt.indexOf(R.player.job),me=Vt.indexOf(Z.player.job);return ge!==me?ge-me:(R.player.surname_encoded||"").localeCompare(Z.player.surname_encoded||"")})}function P(){return[...p||[]].sort((R,Z)=>{const ge=Vt.indexOf(R.job),me=Vt.indexOf(Z.job);return ge!==me?ge-me:(R.surname_encoded||"").localeCompare(Z.surname_encoded||"")})}function J(){return oe().filter(R=>{const Z=R.player,ge=D==="Tous"||Z.job===D,me=!X||`${Z.firstname} ${Z.surname_encoded}`.toLowerCase().includes(X);return ge&&me})}function U(){return P().filter(R=>{const Z=D==="Tous"||R.job===D,ge=!X||`${R.firstname} ${R.surname_encoded}`.toLowerCase().includes(X);return Z&&ge})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
        ...
        <div style="display:flex;justify-content:flex-end">
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{se=!se,G(),ie()})))}function ie(){const R=document.getElementById("col-grid");R&&(q==="player"?$e(R):q==="formation"?Ae(R):q==="stadium"?ae(R):te(R))}function ue(R,Z,ge,me,xe){const re=document.getElementById("col-grid"),ce=document.getElementById("col-big");if(!re||!ce)return;var Se=0;function qe(){const Be=window.innerWidth>=768,Ce=document.getElementById("col-big"),De=document.getElementById("col-grid");Be&&Ce&&(Ce.style.minHeight="420px",Ce.style.flex="1 1 auto"),Be&&De&&(De.style.height=Math.round(310*.76+16)+"px",De.style.flexShrink="0",De.style.overflowX="auto",De.style.overflowY="hidden",De.style.alignItems="center",De.style.padding="8px 16px"),ce.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+Z(R[Se])+"</div>";var I=ce.querySelector("[data-card-id],[data-form-id],[data-gc-id]");I&&I.addEventListener("click",function(){me(R[Se])}),requestAnimationFrame(function(){var j=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!j||!ce)){var w=ce.clientHeight-8,k=ce.clientWidth-24,f=j.offsetHeight,m=j.offsetWidth;if(f>0&&m>0&&w>40){var x=Be?2.2:1,b=Math.min(w/f,k/m,x);j.style.transform="scale("+b.toFixed(3)+")",j.style.transformOrigin="top center"}}}),re.innerHTML=R.map(function(j,w){var k=w===Se,f="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+(k?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+w+'" style="'+f+'">'+ge(j,k)+"</div>"}).join(""),re.querySelectorAll(".col-mini-item").forEach(function(j){j.addEventListener("click",function(){Se=Number(j.dataset.idx),qe(),j.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}qe()}function be(R){var Z=window.innerWidth>=768?.76:.54,ge;if(!R||R._fake){var me=R?R.player:null;if(!me)return"";ge=Fi(me)}else ge=ai(R,"");return'<div style="display:inline-block;zoom:'+Z+';pointer-events:none;line-height:0">'+ge+"</div>"}function Te(R,Z,ge){Z=Z||100,ge=ge||140;var me=Pt[R]||{},xe={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},re=Math.max(3,Math.round(Z*.06)),ce=Object.entries(me).map(function(qe){var Be=qe[0],Ce=qe[1],De=Be.replace(/\d+$/,""),I=xe[De]||"#888",j=Math.round(Ce.x*Z),w=Math.round(Ce.y*ge);return'<circle cx="'+j+'" cy="'+w+'" r="'+re+'" fill="'+I+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),Se=Math.max(1,Math.round(Z/50));return'<svg viewBox="0 0 '+Z+" "+ge+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+Z+'" height="'+ge+'" fill="#1A6B3C"/><rect x="'+Math.round(Z*.2)+'" y="'+Math.round(ge*.02)+'" width="'+Math.round(Z*.6)+'" height="'+Math.round(ge*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+Se+'"/><line x1="0" y1="'+Math.round(ge*.5)+'" x2="'+Z+'" y2="'+Math.round(ge*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+Se+'"/><ellipse cx="'+Math.round(Z*.5)+'" cy="'+Math.round(ge*.5)+'" rx="'+Math.round(Z*.18)+'" ry="'+Math.round(ge*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+Se+'"/><rect x="'+Math.round(Z*.2)+'" y="'+Math.round(ge*.82)+'" width="'+Math.round(Z*.6)+'" height="'+Math.round(ge*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+Se+'"/>'+ce+"</svg>"}function Ee(R,Z,ge){var me=ge>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+ge+"</div>":"",xe=Z?'data-form-id="'+Z.id+'"':"",re=R.length>7?14:R.length>5?16:19,ce=!!Z;return"<div "+xe+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(ce?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(ce?"":"filter:grayscale(1);opacity:0.5")+'">'+me+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(ce?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+re+"px;font-weight:900;color:"+(ce?"#1A6B3C":"#aaa")+';line-height:1">'+R+'</div></div><div style="flex:1;overflow:hidden;background:'+(ce?"#1A6B3C":"#ccc")+'">'+Te(R,140,220)+"</div></div>"}function he(R,Z){var ge=window.innerWidth>=768?.76:.54,me=140,xe=305,re=Math.round(xe*.22),ce=xe-re,Se=R.length>7?10:13,qe=Te(R,me,ce),Be=Z?"1.5px solid #2a7a40":"1px solid #ddd",Ce=Z?"":"filter:grayscale(1);opacity:0.45;",De=Z?"#1A6B3C":"#bbb",I="#fff";return'<div style="display:inline-block;zoom:'+ge+';line-height:0;pointer-events:none"><div style="width:'+me+"px;height:"+xe+"px;border-radius:6px;border:"+Be+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+Ce+'"><div style="height:'+re+"px;background:"+De+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+Se+"px;font-weight:900;color:"+I+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(me-4)+'px">'+R+'</span></div><div style="height:'+ce+'px;overflow:hidden;flex-shrink:0">'+qe+"</div></div></div>"}function $e(R){if(se){const Z=U();if(!Z.length){R.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const ge=Z.map(me=>n.find(xe=>xe.player.id===me.id)||{_fake:!0,player:me,id:"fake-"+me.id});ue(ge,me=>me._fake?Fi(me.player):ai(me,""),me=>me._fake?be({player:me.player,id:"x",_fake:!0}):be(me),me=>{me._fake||Bi(me,n,N,t)})}else{const Z=J();if(!Z.length){R.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const ge={};Z.forEach(xe=>{const re=xe.player.id;ge[re]||(ge[re]=[]),ge[re].push(xe)});const me=Object.values(ge).map(xe=>Jn(xe));ue(me,xe=>{const re=N[xe.player.id]||1,ce=re>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${re}</div>`:"",qe=n.filter(Be=>Be.player.id===xe.player.id&&Be.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return ai(xe,ce+qe)},xe=>be(xe),xe=>Bi(xe,n,N,t))}}function Ae(R){const Z=se?M:[...K];if(!Z.length){R.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const ge=Z.map(me=>({formation:me,card:g.find(xe=>xe.formation===me)||null,owned:K.has(me)}));ue(ge,({formation:me,card:xe,owned:re})=>Ee(me,re?xe:null,re?g.filter(ce=>ce.formation===me).length:0),({formation:me,owned:xe})=>he(me,xe),({card:me,owned:xe})=>{xe&&me&&Qn(me,g,t,a)})}function te(R){const Z=Object.keys(T),ge=se?Z.length?Z:L:[...C];if(!ge.length){R.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const me=ge.map(xe=>({type:xe,gc:Ke[xe]||{icon:"⚡",desc:""},def:T[xe]||null,owned:C.has(xe),card:o.find(re=>re.gc_type===xe)||null}));ue(me,({type:xe,gc:re,def:ce,owned:Se,card:qe})=>{var x;const Be=Se?o.filter(b=>b.gc_type===xe).length:0,Ce=Be>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${Be}</div>`:"",De=(ce==null?void 0:ce.gc_type)==="ultra_game_changer",I={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},j={purple:"#b06ce0",light_blue:"#00d4ef"},w=I[ce==null?void 0:ce.color]||I.purple,k=j[ce==null?void 0:ce.color]||j.purple,f=(ce==null?void 0:ce.effect)||re.desc||"",m=ce!=null&&ce.image_url?`/manager-wars/icons/${ce.image_url}`:((x=ce==null?void 0:ce.club)==null?void 0:x.logo_url)||(ce!=null&&ce.country_code?`https://flagcdn.com/w80/${ce.country_code.toLowerCase()}.png`:null);return Se&&qe?`<div data-gc-id="${qe.id}" data-gc-type="${xe}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${k};background:${w};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${k}66;cursor:pointer">
          ${Ce}
          ...
          <div style="height:150px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
            ${m?`<img src="${m}" style="max-width:120px;max-height:120px;object-fit:contain;border-radius:6px">`:`<span style="font-size:64px">${re.icon}</span>`}
          </div>
        ...
        </div>`},({type:xe,gc:re,def:ce,owned:Se})=>{const qe=window.innerWidth>=768?.76:.54,Be={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},Ce={purple:"#9b59b6",light_blue:"#00bcd4"},De=Be[ce==null?void 0:ce.color]||Be.purple,I=Ce[ce==null?void 0:ce.color]||Ce.purple,j=ce!=null&&ce.image_url?`/manager-wars/icons/${ce.image_url}`:null;return Se?`<div style="display:inline-block;zoom:${qe};line-height:0;pointer-events:none"><div style="width:140px;height:310px;border-radius:8px;background:${De};border:1px solid ${I};display:flex;flex-direction:column;overflow:hidden"><div style="height:56px;background:rgba(255,255,255,0.15);display:flex;align-items:center;justify-content:center"><span style="font-size:13px;font-weight:900;color:#fff;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:130px">${xe}</span></div><div style="flex:1;display:flex;align-items:center;justify-content:center">${j?`<img src="${j}" style="max-width:130px;max-height:190px;object-fit:contain">`:`<span style="font-size:48px">${re.icon}</span>`}</div><div style="height:54px;display:flex;align-items:center;justify-content:center;padding:0 4px"><span style="font-size:10px;color:rgba(255,255,255,0.7);text-align:center">${((ce==null?void 0:ce.effect)||re.desc||"").slice(0,30)}</span></div></div></div>`:`<div style="display:inline-block;zoom:${qe};line-height:0;pointer-events:none"><div style="width:140px;height:310px;border-radius:8px;background:#eee;border:1px solid #ddd;display:flex;flex-direction:column;align-items:center;justify-content:center;filter:grayscale(1);opacity:0.45"><span style="font-size:36px">${re.icon}</span><span style="font-size:12px;color:#aaa;margin-top:6px;text-align:center;padding:0 6px">${xe}</span></div></div>`},({type:xe,owned:re,def:ce})=>{re&&Zn(xe,ce,a)})}function ae(R){const Z="#E87722",ge="/manager-wars/";if(!v.length){R.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Stade.<br><small>Ouvre un booster Stade !</small></div>';return}const me={};v.forEach(re=>{const ce=re.stadium_id||"?";(me[ce]=me[ce]||[]).push(re)});const xe=Object.entries(me).map(([re,ce])=>({sid:re,def:u[re]||null,count:ce.length,card:ce[0]}));ue(xe,({def:re,count:ce})=>{var I,j;const Se=(re==null?void 0:re.name)||"?",qe=((I=re==null?void 0:re.club)==null?void 0:I.encoded_name)||(re==null?void 0:re.country_code)||"—",Be=re!=null&&re.image_url?`${ge}icons/${re.image_url}`:((j=re==null?void 0:re.club)==null?void 0:j.logo_url)||(re!=null&&re.country_code?`https://flagcdn.com/w80/${re.country_code.toLowerCase()}.png`:null),Ce=Be?`<img src="${Be}" style="width:90px;height:90px;object-fit:contain;border-radius:4px" onerror="this.style.display='none'">`:'<div style="font-size:56px">🏟️</div>',De=ce>1?`<div style="position:absolute;top:8px;right:8px;background:#333;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${ce}</div>`:"";return`<div style="position:relative;width:140px;border-radius:12px;border:3px solid #c45a00;background:linear-gradient(160deg,${Z},#c45a00);display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${Z}66">
          ${De}
        ...
        </div>`},({def:re,count:ce})=>{var I,j;const Se=window.innerWidth>=768?.76:.54,qe=(re==null?void 0:re.name)||"?",Be=((I=re==null?void 0:re.club)==null?void 0:I.encoded_name)||(re==null?void 0:re.country_code)||"—",Ce=re!=null&&re.image_url?`${ge}icons/${re.image_url}`:((j=re==null?void 0:re.club)==null?void 0:j.logo_url)||(re!=null&&re.country_code?`https://flagcdn.com/w80/${re.country_code.toLowerCase()}.png`:null),De=Ce?`<img src="${Ce}" style="width:64px;height:64px;object-fit:contain;border-radius:4px" onerror="this.style.display='none'">`:'<span style="font-size:32px">🏟️</span>';return`<div style="display:inline-block;zoom:${Se};line-height:0;pointer-events:none">
        ...
        </div>`},null)}e.querySelectorAll(".col-tab-btn").forEach(R=>{R.addEventListener("click",()=>{q=R.dataset.tab,D="Tous",X="",se=!1,e.querySelectorAll(".col-tab-btn").forEach(Z=>{const ge=Z.dataset.tab===q;Z.style.borderBottomColor=ge?"var(--green)":"transparent",Z.style.color=ge?"var(--green)":"var(--gray-600)"}),G(),ie()})}),G(),ie()}function Zn(e,t,i){const d=Ke[e]||{icon:"⚡",desc:"Effet spécial."},c=(t==null?void 0:t.gc_type)==="ultra_game_changer",a={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},s={purple:"#b06ce0",light_blue:"#00d4ef"},l=a[t==null?void 0:t.color]||a.purple,p=s[t==null?void 0:t.color]||s.purple,n=(t==null?void 0:t.name)||e,o=(t==null?void 0:t.effect)||d.desc,g=t!=null&&t.image_url?`/manager-wars/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
      <div style="background:${l};border-radius:16px;border:2px solid ${p};
        ...
        <div style="height:160px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
          ${g?`<img src="${g}" style="max-width:150px;max-height:150px;object-fit:contain">`:`<span style="font-size:64px">${d.icon}</span>`}
        </div>
      </div>
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Kt=1e3;function Qn(e,t,i,d){var T,r,u;const{state:c,toast:a,closeModal:s,navigate:l,refreshProfile:p}=i,n=e.formation,o={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function g(){const M=Pt[n]||{},L=yi[n]||[],N=290,K=360,C=20;function q(X){const se=M[X];return se?{x:se.x*N,y:se.y*K}:null}let D=`<svg width="${N}" height="${K}" viewBox="0 0 ${N} ${K}" xmlns="http://www.w3.org/2000/svg">`;for(const[X,se]of L){const oe=q(X),P=q(se);!oe||!P||(D+=`<line x1="${oe.x}" y1="${oe.y}" x2="${P.x}" y2="${P.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const X of Object.keys(M)){const se=q(X);if(!se)continue;const oe=X.replace(/\d+/,""),P=o[oe]||"#555";D+=`<circle cx="${se.x}" cy="${se.y}" r="${C}" fill="${P}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,D+=`<text x="${se.x}" y="${se.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${oe}</text>`}return D+="</svg>",D}const v=t.filter(M=>M.formation===n);v.length;const y=!e.is_for_sale;d(`Formation ${n}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(T=document.getElementById("direct-sell-form-btn"))==null||T.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${n} pour ${Kt.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const M=v.find(N=>!N.is_for_sale)||v[0];if(!M){a("Aucune carte à vendre","error");return}await E.from("market_listings").delete().eq("card_id",M.id),await E.from("transfer_history").delete().eq("card_id",M.id);const{error:L}=await E.from("cards").delete().eq("id",M.id);if(L){a(L.message,"error");return}await E.from("users").update({credits:(c.profile.credits||0)+Kt}).eq("id",c.profile.id),await p(),a(`+${Kt.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),s(),l("collection")}),(r=document.getElementById("market-sell-form-btn"))==null||r.addEventListener("click",async()=>{const M=parseInt(document.getElementById("sell-price-form").value);if(!M||M<1){a("Prix invalide","error");return}await E.from("cards").update({is_for_sale:!0,sale_price:M}).eq("id",e.id),await E.from("market_listings").insert({seller_id:c.profile.id,card_id:e.id,price:M}),a("Carte mise en vente sur le marché !","success"),s(),l("collection")}),(u=document.getElementById("cancel-sell-form-btn"))==null||u.addEventListener("click",async()=>{await E.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await E.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),a("Annonce retirée","success"),s(),l("collection")})}async function Bi(e,t,i,d){var ue,be,Te,Ee,he,$e,Ae;const{state:c,toast:a,openModal:s,closeModal:l,navigate:p,refreshProfile:n}=d,o=e.player,g=t.filter(te=>te.player.id===o.id),v=g.length,y=Math.max(Number(o.note_g)||0,Number(o.note_d)||0,Number(o.note_m)||0,Number(o.note_a)||0),T=o.rarity||"normal",{data:r}=await E.from("sell_price_configs").select("*").eq("rarity",T).lte("note_min",y).gte("note_max",y).order("note_min",{ascending:!1}).limit(1),u=((ue=r==null?void 0:r[0])==null?void 0:ue.price)??Wn[T]??1e3,M=o.rarity!=="legende",L=sn(o),N=(o.rarity==="pepite"||o.rarity==="papyte")&&e.current_note!=null?e.current_note:Gt(o,o.job),K=o.rarity==="pepite"||o.rarity==="papyte",C=o.job2?K?wi(e,an(o.job2)):Gt(o,o.job2):null,q=jt[o.job]||"#1A6B3C",D=o.job2?jt[o.job2]:null,X=rn[o.rarity]||"#ccc",se=ki[o.country_code]||o.country_code||"",oe=g.map(te=>te.id);let P={};if(oe.length){const{data:te}=await E.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",oe).order("transferred_at",{ascending:!0});(te||[]).forEach(ae=>{P[ae.card_id]||(P[ae.card_id]=[]),P[ae.card_id].push(ae)})}const J=te=>{const ae=te.transferred_at?new Date(te.transferred_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"2-digit",year:"numeric"}):"",R=te.source==="booster"?"Booster":te.price?te.price.toLocaleString("fr"):"—";return`
    <div style="display:flex;justify-content:space-between;align-items:flex-start;background:#f9f9f9;border-radius:8px;padding:8px 12px;gap:8px">
      ...
      <div style="text-align:right;flex-shrink:0">
        ...
        <div style="font-size:11px;color:var(--gray-600)">${ae}</div>
      </div>
    </div>`},U=oe.length?`
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      <div style="font-size:13px;font-weight:700;margin-bottom:10px">🏟️ Mes exemplaires ${v>1?`(${v})`:""}</div>
      <div style="display:flex;flex-direction:column;gap:10px">
        ${g.map((te,ae)=>{const R=P[te.id]||[],Z=te.is_for_sale,ge=R.length?R[R.length-1]:null,xe=(o.rarity==="pepite"||o.rarity==="papyte")&&te.current_note!=null?` (☆${te.current_note})`:"";return`
            <div data-card-id="${te.id}" data-card-idx="${ae}" class="exemplaire-row"
              ...
              <div style="display:flex;align-items:center;gap:10px">
                ...
                <div style="flex:1;min-width:0">
                  <div style="font-size:12px;font-weight:700">Exemplaire ${ae+1}${xe}${Z?" 🏷️ En vente":""}</div>
                ...
                <button class="expl-hist-toggle" data-idx="${ae}" style="font-size:11px;color:#999;border:none;background:none;cursor:pointer;padding:2px 6px;flex-shrink:0">
                  ${R.length?`${R.length} club${R.length>1?"s":""}`:""} ▾
                </button>
              </div>
              <div class="expl-hist" data-hist="${ae}" style="display:none;margin-top:8px;display:flex;flex-direction:column;gap:4px">
                ${R.map(J).join("")}
              </div>
            </div>`}).join("")}
      </div>
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",o.note_g],["DEF",o.note_d],["MIL",o.note_m],["ATT",o.note_a]].map(([te,ae])=>{const R=jt[te],Z=Number(ae)||0;return`<div style="display:flex;flex-direction:column;align-items:center;gap:2px">
    ...
    ${U}
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(Ee=document.getElementById("close-detail"))==null||Ee.addEventListener("click",l);let G=new Set;const ie=()=>{const te=G.size,ae=document.getElementById("sell-action-panel");ae&&(ae.style.display=te>0?"block":"none",document.getElementById("sell-selected-count").textContent=te,document.getElementById("sell-direct-total").textContent=(te*u).toLocaleString("fr")+" cr.")};document.querySelectorAll(".expl-check").forEach(te=>{te.addEventListener("change",()=>{const ae=te.dataset.id;te.checked?G.add(ae):G.delete(ae);const R=te.closest(".exemplaire-row");R&&(R.style.borderColor=te.checked?"#1A6B3C":"#eee"),ie()})}),document.querySelectorAll(".exemplaire-row").forEach(te=>{te.addEventListener("click",ae=>{if(ae.target.closest("button")||ae.target.tagName==="INPUT")return;const R=te.querySelector(".expl-check");R&&!R.disabled&&(R.checked=!R.checked,R.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(te=>{te.addEventListener("click",ae=>{ae.stopPropagation();const R=document.querySelector(`.expl-hist[data-hist="${te.dataset.idx}"]`);R&&(R.style.display=R.style.display==="none"?"flex":"none")})}),(he=document.getElementById("direct-sell-btn"))==null||he.addEventListener("click",async()=>{const te=[...G];if(!te.length)return;const ae=te.length*u;if(!confirm(`Vendre ${te.length} carte${te.length>1?"s":""} ${o.surname_encoded} pour ${ae.toLocaleString("fr")} crédits ? Action irréversible.`))return;await E.from("market_listings").delete().in("card_id",te),await E.from("transfer_history").delete().in("card_id",te);const{error:R}=await E.from("cards").delete().in("id",te);if(R){a(R.message,"error");return}await E.from("users").update({credits:(c.profile.credits||0)+ae}).eq("id",c.profile.id),await n();const Z=document.getElementById("nav-credits");Z&&(Z.textContent=`💰 ${(c.profile.credits||0).toLocaleString("fr")}`),a(`+${ae.toLocaleString("fr")} crédits ! ${te.length} carte${te.length>1?"s":""} vendue${te.length>1?"s":""}.`,"success"),l(),p("collection")}),($e=document.getElementById("market-sell-btn"))==null||$e.addEventListener("click",async()=>{var me;const te=[...G];if(!te.length){a("Sélectionne au moins un exemplaire","warning");return}const ae=parseInt((me=document.getElementById("sell-market-price"))==null?void 0:me.value);if(!ae||ae<1){a("Prix invalide","error");return}const{error:R}=await E.from("cards").update({is_for_sale:!0,sale_price:ae}).in("id",te);if(R){a(R.message,"error");return}const Z=te.map(xe=>({seller_id:c.profile.id,card_id:xe,price:ae,status:"active"})),{error:ge}=await E.from("market_listings").insert(Z);ge&&console.warn("[Market] insert listings:",ge.message),a(`${te.length} carte${te.length>1?"s":""} mise${te.length>1?"s":""} en vente à ${ae.toLocaleString("fr")} cr. chacune !`,"success"),l(),p("collection")}),(Ae=document.getElementById("cancel-sell-btn"))==null||Ae.addEventListener("click",async()=>{await E.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await E.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),a("Annonce retirée","success"),l(),p("collection")})}const Xt={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},vt={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function ti(e){const t=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function dn(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function ln(e){var i;const t="https://fcnwclxlkytdfjotqkta.supabase.co";return(i=e==null?void 0:e.clubs)!=null&&i.logo_url?e.clubs.logo_url.startsWith("http")?e.clubs.logo_url:`${t}/storage/v1/object/public/assets/clubs/${e.clubs.logo_url}`:null}function eo(e,t=44,i=58){var y;const d=e?ti(e):null,c=e?ln(e):null,a=dn(e==null?void 0:e.country_code),s=(e==null?void 0:e.job)||"MIL",l=vt[s]||"#555",p={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",n=Number(s==="GK"?e==null?void 0:e.note_g:s==="DEF"?e==null?void 0:e.note_d:s==="MIL"?e==null?void 0:e.note_m:e==null?void 0:e.note_a)||0,o=Math.round(i*.19),g=Math.round(i*.25),v=i-o-g;return e?`<div style="width:${t}px;height:${i}px;border-radius:5px;border:2px solid ${p};background:${l};position:relative;overflow:hidden;flex-shrink:0">
    ...
    ${d?`<img src="${d}" style="position:absolute;top:${o}px;left:0;width:${t}px;height:${v}px;object-fit:cover;object-position:top center">`:""}
    <div style="position:absolute;bottom:0;left:0;right:0;height:${g}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ${a?`<img src="${a}" style="width:${g+2}px;height:${g-3}px;object-fit:cover;border-radius:1px">`:`<span style="font-size:${g-4}px">🌍</span>`}
  ...
  </div>`:`<div style="width:${t}px;height:${i}px;border:2px dashed rgba(255,255,255,0.3);border-radius:5px;display:flex;align-items:center;justify-content:center;font-size:20px;color:rgba(255,255,255,0.3)">+</div>`}async function ci(e,t){const{state:i,navigate:d,toast:c}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:a}=await E.from("decks").select("id,name,formation_card_id").eq("owner_id",i.profile.id).order("created_at",{ascending:!1});e.innerHTML=`
  ...
  </div>`,document.getElementById("new-deck-btn").addEventListener("click",async()=>{const s=prompt("Nom du deck :",`Deck ${((a==null?void 0:a.length)||0)+1}`);if(!s)return;const{data:l,error:p}=await E.from("decks").insert({owner_id:i.profile.id,name:s}).select().single();if(p){c(p.message,"error");return}c("Deck créé !","success"),Di(l.id,e,t)}),e.querySelectorAll("[data-open-deck]").forEach(s=>{s.addEventListener("click",()=>Di(s.dataset.openDeck,e,t))}),e.querySelectorAll("[data-rename]").forEach(s=>{s.addEventListener("click",async()=>{const l=prompt("Nouveau nom :",s.dataset.name);if(!l||l===s.dataset.name)return;const{error:p}=await E.from("decks").update({name:l}).eq("id",s.dataset.rename);if(p){c(p.message,"error");return}c("Deck renommé !","success"),ci(e,t)})}),e.querySelectorAll("[data-delete]").forEach(s=>{s.addEventListener("click",async()=>{if(!confirm(`Supprimer le deck "${s.dataset.name}" ? Cette action est irréversible.`))return;await E.from("deck_cards").delete().eq("deck_id",s.dataset.delete);const{error:l}=await E.from("decks").delete().eq("id",s.dataset.delete);if(l){c(l.message,"error");return}c("Deck supprimé.","success"),ci(e,t)})})}async function Di(e,t,i){const{state:d,toast:c}=i;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:a}=await E.from("decks").select("*").eq("id",e).single(),{data:s}=await E.from("cards").select(`id, card_type, formation, stadium_id,
      player:players(id, firstname, surname_encoded, country_code, club_id, job, job2,
        note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length,
        clubs(encoded_name, logo_url)),
      stadium_def:stadium_definitions(id, name, club_id, country_code, image_url,
        club:clubs(encoded_name, logo_url))`).eq("owner_id",d.profile.id),l=(s||[]).filter(u=>u.card_type==="player"&&u.player),p=(s||[]).filter(u=>u.card_type==="formation"),n=(s||[]).filter(u=>u.card_type==="stadium"),o=[...new Set(n.map(u=>u.stadium_id).filter(Boolean))];let g={};if(n.forEach(u=>{u.stadium_def&&u.stadium_id&&(g[u.stadium_id]=u.stadium_def)}),o.length&&Object.keys(g).length<o.length){const{data:u}=await E.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",o);(u||[]).forEach(M=>{g[M.id]=M})}const v=p.map(u=>u.formation).filter(Boolean),{data:y}=await E.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",e);let T=a.formation||"4-4-2";v.length>0&&!v.includes(T)&&(T=v[0]);const r={deckId:e,name:a.name,formation:T,formationCardId:a.formation_card_id,stadiumCardId:a.stadium_card_id||null,slots:{},subs:[],playerCards:l,formationCards:p,stadiumCards:n,stadDefMap:g,availableFormations:v};(y||[]).forEach(u=>{u.is_starter?r.slots[u.position]=u.card_id:r.subs.includes(u.card_id)||r.subs.push(u.card_id)}),St(t,r,i)}function St(e,t,i){var p,n;const{navigate:d}=i;Xt[t.formation];const c=Gi(t.formation),a=c.filter(o=>t.slots[o]).length,s=t.availableFormations.length>0?t.availableFormations:Object.keys(Xt),l=t.subs.map(o=>t.playerCards.find(g=>g.id===o)).filter(Boolean);[...Object.values(t.slots),...t.subs],e.innerHTML=`
  <div style="height:100%;overflow-y:auto;padding-bottom:80px;background:var(--page-bg)">
  ...
  </div>`,to(e,t,c,i),document.getElementById("builder-back").addEventListener("click",()=>d("decks")),document.getElementById("formation-select").addEventListener("change",o=>{t.formation=o.target.value;const g=Gi(t.formation),v={};g.forEach(y=>{t.slots[y]&&(v[y]=t.slots[y])}),t.slots=v,St(e,t,i)}),(p=document.getElementById("stadium-select"))==null||p.addEventListener("change",o=>{t.stadiumCardId=o.target.value||null}),document.getElementById("save-deck").addEventListener("click",()=>oo(t,i)),e.querySelectorAll("[data-remove-sub]").forEach(o=>{o.addEventListener("click",()=>{t.subs=t.subs.filter(g=>g!==o.dataset.removeSub),St(e,t,i)})}),(n=document.getElementById("add-sub-btn"))==null||n.addEventListener("click",()=>{no(t,e,i)})}function to(e,t,i,d){var N,K;const c=e.querySelector("#deck-field");if(!c)return;const a=(N=t.stadiumCards)==null?void 0:N.find(C=>C.id===t.stadiumCardId),s=a&&((K=t.stadDefMap)==null?void 0:K[a.stadium_id])||null,l=Pt[t.formation]||{},p=Yi(t.formation)||[],n={};for(const C of i){const q=t.slots[C],D=q?t.playerCards.find(X=>X.id===q):null;n[C]=D?D.player:null}const o=300,g=300,v=48,y=64,T=13,r=16,u=38;function M(C){const q=l[C];return q?{x:q.x*o,y:q.y*g}:null}let L="";for(const[C,q]of p){const D=M(C),X=M(q);if(!D||!X)continue;const se=n[C]?{...n[C],club_id:n[C].club_id,country_code:n[C].country_code,rarity:n[C].rarity}:null,oe=n[q]?{...n[q],club_id:n[q].club_id,country_code:n[q].country_code,rarity:n[q].rarity}:null,P=at(se,oe);P==="#ff3333"||P==="#cc2222"?L+=`<line x1="${D.x.toFixed(1)}" y1="${D.y.toFixed(1)}" x2="${X.x.toFixed(1)}" y2="${X.y.toFixed(1)}" stroke="${P}" stroke-width="2.5" stroke-linecap="round" opacity="0.4"/>`:(L+=`<line x1="${D.x.toFixed(1)}" y1="${D.y.toFixed(1)}" x2="${X.x.toFixed(1)}" y2="${X.y.toFixed(1)}" stroke="${P}" stroke-width="8" stroke-linecap="round" opacity="0.2"/>`,L+=`<line x1="${D.x.toFixed(1)}" y1="${D.y.toFixed(1)}" x2="${X.x.toFixed(1)}" y2="${X.y.toFixed(1)}" stroke="${P}" stroke-width="2.5" stroke-linecap="round" opacity="0.9"/>`)}for(const C of i){const q=M(C);if(!q)continue;const D=n[C],X=C.replace(/\d+/,""),se=vt[X]||"#555",oe=(q.x-v/2).toFixed(1),P=(q.y-y/2).toFixed(1),J={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[D==null?void 0:D.rarity]||"#aaa";if(D){const U=ti(D),G=ln(D),ie=dn(D.country_code),ue=Number(X==="GK"?D.note_g:X==="DEF"?D.note_d:X==="MIL"?D.note_m:D.note_a)||0,be=s&&(s.club_id&&String(D.club_id)===String(s.club_id)||s.country_code&&D.country_code===s.country_code),Te=be?ue+10:ue,Ee=y-T-r;L+=`<defs><clipPath id="dcp-${C}"><rect x="${oe}" y="${(q.y-y/2+T).toFixed(1)}" width="${v}" height="${Ee}"/></clipPath></defs>`,L+=`<rect x="${oe}" y="${P}" width="${v}" height="${y}" rx="5" fill="${se}"/>`,U&&(L+=`<image href="${U}" x="${oe}" y="${(q.y-y/2+T).toFixed(1)}" width="${v}" height="${Ee}" clip-path="url(#dcp-${C})" preserveAspectRatio="xMidYMin slice"/>`),L+=`<rect x="${oe}" y="${P}" width="${v}" height="${T}" fill="rgba(255,255,255,0.93)"/>`,L+=`<text x="${q.x.toFixed(1)}" y="${(q.y-y/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(D.surname_encoded||"").slice(0,9)}</text>`;const he=(q.y+y/2-r).toFixed(1);L+=`<rect x="${oe}" y="${he}" width="${v}" height="${r}" fill="rgba(255,255,255,0.93)"/>`,ie&&(L+=`<image href="${ie}" x="${(q.x-21).toFixed(1)}" y="${(q.y+y/2-r+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`),L+=`<text x="${q.x.toFixed(1)}" y="${(q.y+y/2-r/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${Te}</text>`,G&&(L+=`<image href="${G}" x="${(q.x+v/2-14).toFixed(1)}" y="${(q.y+y/2-r+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`),L+=`<rect x="${oe}" y="${P}" width="${v}" height="${y}" rx="5" fill="none" stroke="${be?"#E87722":J}" stroke-width="${be?"2.5":"2"}"/>`,be&&(L+=`<rect x="${(q.x+v/2-13).toFixed(1)}" y="${P}" width="13" height="9" rx="3" fill="#E87722"/>`,L+=`<text x="${(q.x+v/2-6.5).toFixed(1)}" y="${(q.y-y/2+4.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="900" fill="#fff" font-family="Arial">+10</text>`)}else L+=`<rect x="${oe}" y="${P}" width="${v}" height="${y}" rx="5" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="5,3"/>`,L+=`<text x="${q.x.toFixed(1)}" y="${q.y.toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="22" fill="rgba(255,255,255,0.35)">+</text>`,L+=`<text x="${q.x.toFixed(1)}" y="${(q.y+16).toFixed(1)}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.3)">${X}</text>`;L+=`<rect x="${oe}" y="${P}" width="${v}" height="${y}" rx="5" fill="rgba(0,0,0,0.01)" class="deck-slot-hit" data-pos="${C}" style="cursor:pointer"/>`}c.innerHTML=`<svg viewBox="${-u} ${-u} ${o+u*2} ${g+u*2}" width="100%" style="display:block;max-width:440px;margin:0 auto">${L}</svg>`,c.querySelectorAll(".deck-slot-hit").forEach(C=>{C.addEventListener("click",()=>io(C.dataset.pos,t,e,d))})}function io(e,t,i,d){var v,y,T;const{openModal:c,closeModal:a}=d,s=e.replace(/\d+/,""),l=t.slots[e],p=l?t.playerCards.find(r=>r.id===l):null;(v=p==null?void 0:p.player)==null||v.id;const n=new Set;Object.entries(t.slots).forEach(([r,u])=>{var L;if(r===e||!u)return;const M=t.playerCards.find(N=>N.id===u);(L=M==null?void 0:M.player)!=null&&L.id&&n.add(M.player.id)}),t.subs.forEach(r=>{var M;const u=t.playerCards.find(L=>L.id===r);(M=u==null?void 0:u.player)!=null&&M.id&&n.add(u.player.id)});const o=new Set,g=t.playerCards.filter(r=>{const u=r.player;return!(u.job===s||u.job2===s)||n.has(u.id)||o.has(u.id)?!1:(o.add(u.id),!0)});g.sort((r,u)=>{const M=s==="GK"?r.player.note_g:s==="DEF"?r.player.note_d:s==="MIL"?r.player.note_m:r.player.note_a;return(s==="GK"?u.player.note_g:s==="DEF"?u.player.note_d:s==="MIL"?u.player.note_m:u.player.note_a)-M}),c(`Choisir ${s} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${g.length>0?g.map(r=>{var K,C;const u=r.player,M=s==="GK"?u.note_g:s==="DEF"?u.note_d:s==="MIL"?u.note_m:u.note_a,L=ti(u),N={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[u.rarity];return`<div class="player-option" data-card-id="${r.id}"
    ...
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(y=document.getElementById("close-selector"))==null||y.addEventListener("click",a),(T=document.getElementById("remove-player"))==null||T.addEventListener("click",()=>{delete t.slots[e],a(),St(i,t,d)}),document.querySelectorAll(".player-option").forEach(r=>{r.addEventListener("click",()=>{t.slots[e]=r.dataset.cardId,a(),St(i,t,d)})})}function no(e,t,i){var p;const{openModal:d,closeModal:c}=i,a=new Set;Object.values(e.slots).filter(Boolean).forEach(n=>{var g;const o=e.playerCards.find(v=>v.id===n);(g=o==null?void 0:o.player)!=null&&g.id&&a.add(o.player.id)}),e.subs.forEach(n=>{var g;const o=e.playerCards.find(v=>v.id===n);(g=o==null?void 0:o.player)!=null&&g.id&&a.add(o.player.id)});const s=new Set,l=e.playerCards.filter(n=>{var o,g,v;return a.has((o=n.player)==null?void 0:o.id)||s.has((g=n.player)==null?void 0:g.id)?!1:(s.add((v=n.player)==null?void 0:v.id),!0)});d("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
  ...
  </div>`}function un(e){var c;const t={};(e.rates||[]).forEach(a=>{t[a.card_type]=(t[a.card_type]||0)+Number(a.percentage||0)});const i=((c=Object.entries(t).sort((a,s)=>s[1]-a[1])[0])==null?void 0:c[0])||"player",d=e.image_url||"booster-players.png";return{id:e.id,img:"/manager-wars/icons/"+d,name:e.name,sub:`${e.card_count} carte(s)`,cost:e.price_type==="credits"&&e.price_credits||0,costLabel:e.price_type==="credits"?`${(e.price_credits||0).toLocaleString("fr")} crédits`:e.price_type==="pub"?"1 pub":"Gratuit",cardCount:e.card_count||5,type:i,isPub:e.price_type==="pub",rates:e.rates||[],_raw:e}}async function uo(e,{state:t,navigate:i,toast:d}){var s;const c=((s=t.profile)==null?void 0:s.credits)||0;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⏳ Chargement...</div>';let a=[];try{a=(await cn()).map(un)}catch(l){console.warn("Erreur chargement boosters DB, fallback hardcodé",l)}a.length||(a=ao.map(l=>({...l,rates:[],isPub:l.id==="players_pub"}))),e.innerHTML=`
  <div class="page">
    <div class="page-header">
      <h2>📦 Boosters</h2>
  ...
  </div>`,e.querySelectorAll(".booster-card:not(.disabled)").forEach(l=>{l.addEventListener("click",async()=>{const p=a.find(n=>n.id===l.dataset.booster);if(p){l.style.opacity="0.5",l.style.pointerEvents="none";try{await fo(p,{state:t,toast:d,navigate:i,container:e})}catch(n){d(n.message,"error"),l.style.opacity="",l.style.pointerEvents=""}}})}),e.querySelectorAll(".booster-info-btn").forEach(l=>{l.addEventListener("click",p=>{p.stopPropagation();const n=a.find(o=>o.id===l.dataset.boosterId);yo(n)})})}async function fo(e,{state:t,toast:i,navigate:d,container:c}){var g,v;if(e.cost>0&&t.profile.credits<e.cost){i("Crédits insuffisants","error");return}if(e.isPub)try{await vo()}catch(y){i(y.message||"Regardez la pub entièrement pour ouvrir le booster","error");return}const{data:a}=await E.from("cards").select("card_type, player_id, formation").eq("owner_id",t.profile.id),s=new Set((a||[]).filter(y=>y.card_type==="player").map(y=>y.player_id)),l=new Set((a||[]).filter(y=>y.card_type==="formation").map(y=>y.formation));let p=[],n=null;try{if((g=e.rates)!=null&&g.length)p=await ui(t.profile,e);else{const y=e.type||"player";y==="player"?p=await fn(t.profile,e.cardCount,e.cost):y==="game_changer"?p=await gn(t.profile,e.cardCount,e.cost):y==="formation"?p=await mn(t.profile,e.cost):p=await ui(t.profile,e)}}catch(y){n=y.message||String(y),console.error("[Booster] Erreur:",y)}if(!(p!=null&&p.length)){const y=document.createElement("div");y.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:3000;gap:16px;color:#fff;padding:24px;text-align:center",y.innerHTML=`
      <div style="font-size:48px">😕</div>
      <div style="font-size:20px;font-weight:900">Aucune carte obtenue</div>
      <div style="font-size:12px;color:rgba(255,255,255,0.6);max-width:320px;word-break:break-all;background:rgba(255,255,255,0.05);padding:10px;border-radius:8px;margin-top:6px">
        ${n||"Vérifie la console (F12) pour plus de détails"}
      </div>
      <button style="margin-top:10px;padding:12px 28px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer" id="anim-close-err">Fermer</button>`,document.body.appendChild(y),(v=y.querySelector("#anim-close-err"))==null||v.addEventListener("click",()=>y.remove());return}p.forEach(y=>{y.card_type==="player"&&y.player?y.isDuplicate=s.has(y.player.id):y.card_type==="formation"&&(y.isDuplicate=l.has(y.formation))});const{data:o}=await E.from("users").select("*").eq("id",t.profile.id).single();o&&(t.profile=o),xn(p,e,d)}function go(){const e=Math.random()*100;return e<.5?"legende":e<2?"special":e<10?"normal_high":"normal_low"}function xt(e){return Math.max(Number(e.note_g)||0,Number(e.note_d)||0,Number(e.note_m)||0,Number(e.note_a)||0)}function mo(e,t){let i;switch(t){case"legende":i=e.filter(d=>d.rarity==="legende"),i.length||(i=e.filter(d=>d.rarity==="pepite"||d.rarity==="papyte")),i.length||(i=e.filter(d=>xt(d)>=6));break;case"special":i=e.filter(d=>d.rarity==="pepite"||d.rarity==="papyte"),i.length||(i=e.filter(d=>xt(d)>=6));break;case"normal_high":i=e.filter(d=>d.rarity==="normal"&&xt(d)>=6),i.length||(i=e.filter(d=>xt(d)>=6));break;default:i=e.filter(d=>d.rarity==="normal"&&xt(d)>=1&&xt(d)<=5),i.length||(i=e.filter(d=>d.rarity==="normal"));break}return i.length||(i=e),i[Math.floor(Math.random()*i.length)]}async function ui(e,t){if(t.cost>0){const{error:l}=await E.from("users").update({credits:e.credits-t.cost}).eq("id",e.id);if(l)throw l}const{data:i}=await E.from("cards").select("player_id, card_type, formation").eq("owner_id",e.id),d=new Set((i||[]).filter(l=>l.card_type==="player").map(l=>l.player_id)),c=new Set((i||[]).filter(l=>l.card_type==="formation").map(l=>l.formation)),a=new Set,s=[];for(let l=0;l<(t.cardCount||5);l++){const p=ro(t.rates);if(p){if(p.card_type==="player"){const n=L=>({légende:"legende",pépite:"pepite",pépites:"pepite"})[L]||L,o=p.rarity?n(p.rarity):null;let g=E.from("players").select("id,job,firstname,surname_encoded,country_code,club_id,rarity,note_g,note_d,note_m,note_a,skin,hair,hair_length,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);o&&(g=g.eq("rarity",o));const{data:v}=await g;let y=v||[];if((p.note_min||p.note_max)&&(y=y.filter(L=>{const N=Math.max(Number(L.note_g)||0,Number(L.note_d)||0,Number(L.note_m)||0,Number(L.note_a)||0);return(!p.note_min||N>=p.note_min)&&(!p.note_max||N<=p.note_max)})),y.length||(p.note_min||p.note_max?(y=v||[],console.warn("[Booster] Aucun joueur avec note",p.note_min,"-",p.note_max,"— fallback rareté uniquement")):y=v||[]),!y.length)continue;let T=y.filter(L=>!a.has(L.id));T.length||(T=y);const r=T[Math.floor(Math.random()*T.length)];a.add(r.id);const u=d.has(r.id),{data:M}=await E.from("cards").insert({owner_id:e.id,player_id:r.id,card_type:"player"}).select().single();M&&(s.push({...M,player:r,isDuplicate:u}),E.rpc("record_transfer",{p_card_id:M.id,p_player_id:r.id,p_club_name:e.club_name||e.pseudo,p_manager_name:e.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{}))}else if(p.card_type==="game_changer"){const{data:n}=await E.from("gc_definitions").select("id,name").eq("is_active",!0).eq("gc_type","game_changer"),o=n!=null&&n.length?n:[{name:"Ressusciter"},{name:"Double attaque"},{name:"Bouclier"},{name:"Vol de note"},{name:"Gel"}],v=o[Math.floor(Math.random()*o.length)].name,{data:y}=await E.from("cards").insert({owner_id:e.id,card_type:"game_changer",gc_type:v}).select().single();y&&s.push(y)}else if(p.card_type==="formation"){const n=pn(),o=n[Math.floor(Math.random()*n.length)],g=c.has(o),{data:v}=await E.from("cards").insert({owner_id:e.id,card_type:"formation",formation:o}).select();v!=null&&v[0]&&s.push({...v[0],isDuplicate:g})}else if(p.card_type==="stadium"){const{data:n,error:o}=await E.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)");if(o){console.error("[Booster] stadium_definitions:",o.message);continue}if(!(n!=null&&n.length)){console.warn("[Booster] Aucun stade en DB");continue}const g=n[Math.floor(Math.random()*n.length)],{data:v,error:y}=await E.from("cards").insert({owner_id:e.id,card_type:"stadium",stadium_id:g.id}).select("id,card_type,stadium_id").single();if(y){console.error("[Booster] insert stadium card:",y.message);continue}v&&s.push({...v,rarity:"normal",_stadiumDef:g})}}}return s}async function fn(e,t,i){if(i>0){const{error:n}=await E.from("users").update({credits:e.credits-i}).eq("id",e.id);if(n)throw n}const{data:d}=await E.from("players").select("id,job,firstname,surname_encoded,country_code,club_id,rarity,note_g,note_d,note_m,note_a,note_min,note_max,skin,hair,hair_length,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);if(!(d!=null&&d.length))throw new Error("Pas de joueurs en BDD — ajoutes-en via le panel admin !");const c=d.filter(n=>n.job==="GK"),a=d.filter(n=>n.job!=="GK"),s=!e.first_booster_opened&&c.length>0,l=[];for(let n=0;n<t;n++){const o=n===0&&s?c:n===0?a:d,g=go(),v=mo(o,g);v&&l.push(v)}s&&await E.from("users").update({first_booster_opened:!0}).eq("id",e.id);const{data:p}=await E.from("cards").insert(l.map(n=>({owner_id:e.id,player_id:n.id,card_type:"player"}))).select();return(p||[]).forEach((n,o)=>{E.rpc("record_transfer",{p_card_id:n.id,p_player_id:l[o].id,p_club_name:e.club_name||e.pseudo,p_manager_name:e.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{})}),l.map((n,o)=>({...p[o],player:n}))}async function gn(e,t,i){const{error:d}=await E.from("users").update({credits:e.credits-i}).eq("id",e.id);if(d)throw d;const{data:c}=await E.from("gc_definitions").select("id,name,gc_type,color,effect,image_url").eq("is_active",!0),a=c!=null&&c.length?c:Object.keys(pi).map(o=>({name:o,gc_type:"game_changer"})),s=Array.from({length:t},()=>{const o=a[Math.floor(Math.random()*a.length)];return{owner_id:e.id,card_type:"game_changer",gc_type:o.name,gc_definition_id:o.id||null}}),{data:l,error:p}=await E.from("cards").insert(s).select();return p&&console.error("[Booster GC] Erreur insert:",p.message,p),(l||[]).map(o=>{const g=c==null?void 0:c.find(v=>v.name===o.gc_type||v.id===o.gc_definition_id);return{...o,_gcDef:g||null}})}async function mn(e,t){const{error:i}=await E.from("users").update({credits:e.credits-t}).eq("id",e.id);if(i)throw i;const{data:d}=await E.from("cards").select("formation").eq("owner_id",e.id).eq("card_type","formation"),c=new Set((d||[]).map(o=>o.formation)),a=pn(),s=a[Math.floor(Math.random()*a.length)],l=c.has(s),{data:p,error:n}=await E.from("cards").insert({owner_id:e.id,card_type:"formation",formation:s}).select();return n&&console.error("[Booster Formation] Erreur insert:",n.message,n),(p||[]).map(o=>({...o,isDuplicate:l}))}function xn(e,t,i,d=null){var J,U;if(!e||e.length===0){const G=document.createElement("div");G.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:3000;gap:16px;color:#fff;padding:24px;text-align:center",G.innerHTML=`
      <div style="font-size:48px">😕</div>
      <div style="font-size:20px;font-weight:900">Aucune carte obtenue</div>
      <div style="font-size:13px;color:rgba(255,255,255,0.5)">Erreur lors du tirage (permissions DB ou colonne manquante)</div>
      <button style="margin-top:10px;padding:12px 28px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer" id="anim-close-err">Fermer</button>`,document.body.appendChild(G),(J=G.querySelector("#anim-close-err"))==null||J.addEventListener("click",()=>G.remove());return}e=[...e].sort((G,ie)=>{const ue=G.player?xt(G.player):-1;return(ie.player?xt(ie.player):-1)-ue});const c=document.createElement("div");c.id="booster-anim-overlay",c.innerHTML=`
    <style>
  ...
  `,document.body.appendChild(c);let a=!1;const s=document.getElementById("pack-cut-zone"),l=document.getElementById("pack-blade");let p=!1;const n=G=>G.touches&&G.touches[0]?G.touches[0].clientX:G.clientX;function o(G){a||(p=!0,l.style.opacity="1",g(G))}function g(G){if(!p||a)return;const ie=s.getBoundingClientRect(),ue=n(G)-ie.left,be=Math.max(0,Math.min(1,ue/ie.width));l.style.width=be*ie.width+"px",be>=.82&&y()}function v(){a||(p=!1,l.style.transition="width .2s ease, opacity .2s ease",l.style.width="0",l.style.opacity="0",setTimeout(()=>{a||(l.style.transition="")},220))}function y(){var ie;if(a)return;a=!0,p=!1,l.style.width="100%",l.style.opacity="1",(ie=document.getElementById("cut-flash"))==null||ie.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const G=document.getElementById("cut-hint");G&&(G.style.opacity="0"),s.classList.add("pack-cut"),setTimeout(()=>{l.style.opacity="0",document.getElementById("pack-phase").style.display="none",M(0)},620)}s.addEventListener("pointerdown",o),window.addEventListener("pointermove",g),window.addEventListener("pointerup",v),s.addEventListener("touchstart",o,{passive:!0}),window.addEventListener("touchmove",g,{passive:!0}),window.addEventListener("touchend",v);let T=0,r=!1;const u=new Set;function M(G){T=G,document.getElementById("reveal-phase").style.display="flex",L(),N(G,0),X()}function L(){const G=document.getElementById("card-dots");G&&(G.innerHTML=e.map((ie,ue)=>`<div class="card-dot" data-i="${ue}" style="width:8px;height:8px;border-radius:50%;background:${ue===T?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),G.querySelectorAll(".card-dot").forEach(ie=>ie.addEventListener("click",()=>C(parseInt(ie.dataset.i)))))}function N(G,ie){var ae;const ue=e[G],be=document.getElementById("card-counter"),Te=document.getElementById("card-track");be&&(be.textContent=`Carte ${G+1} / ${e.length}`);const Ee=document.getElementById("reveal-btns");Ee&&(Ee.style.display=G===e.length-1?"flex":"none");const he=ue.card_type==="player"&&((ae=ue.player)==null?void 0:ae.rarity)==="legende",$e=!u.has(G);u.add(G);let Ae=0;if(ue.card_type==="player"&&ue.player){const R=ue.player,Z=R.job||"ATT";Ae=Number(Z==="GK"?R.note_g:Z==="DEF"?R.note_d:Z==="MIL"?R.note_m:R.note_a)||0}const te=R=>{Te.innerHTML=`
        <div id="current-card-wrap" style="position:relative;display:flex;flex-direction:column;align-items:center;gap:8px;${he?"filter:drop-shadow(0 0 20px #7a28b8)":""}">
          <div style="transform:scale(1.25);transform-origin:center">${xo(ue)}</div>
        ...
        </div>`;const Z=document.getElementById("current-card-wrap");ie!==0?(Z.style.transition="none",Z.style.transform=`translateX(${ie>0?100:-100}%)`,requestAnimationFrame(()=>{Z.style.transition="transform .28s cubic-bezier(.25,1,.5,1)",Z.style.transform="translateX(0)"})):Z.animate([{opacity:0,transform:"scale(.7)"},{opacity:1,transform:"scale(1)"}],{duration:300,easing:"cubic-bezier(.34,1.56,.64,1)"}),R||he?oe():P(),L()};$e&&Ae>6&&ue.card_type==="player"&&ue.player?K(ue,()=>te(!0)):te(!1)}function K(G,ie){var Ae;r=!0;const ue=G.player,be=`https://flagsapi.com/${ue.country_code}/flat/64.png`,Te=(Ae=ue.clubs)==null?void 0:Ae.logo_url,Ee=document.getElementById("walkout-overlay"),he=document.getElementById("walkout-stage");if(!Ee||!he){r=!1,ie();return}Ee.style.display="flex";const $e=()=>{const te=he.firstElementChild;te&&(te.classList.remove("wo-in"),te.classList.add("wo-out"))};he.innerHTML=`<img class="wo-in" src="${be}" style="height:130px;border-radius:10px;box-shadow:0 10px 36px rgba(0,0,0,.6)" onerror="this.style.display='none'">`,navigator.vibrate&&navigator.vibrate(30),setTimeout($e,2e3),setTimeout(()=>{var te;he.innerHTML=Te?`<img class="wo-in" src="${Te}" style="max-height:160px;max-width:210px;object-fit:contain">`:`<div class="wo-in" style="font-size:34px;font-weight:900;color:#fff;text-align:center">${((te=ue.clubs)==null?void 0:te.encoded_name)||"CLUB"}</div>`,navigator.vibrate&&navigator.vibrate(30)},2450),setTimeout($e,4450),setTimeout(()=>{Ee.style.display="none",he.innerHTML="",r=!1,navigator.vibrate&&navigator.vibrate([40,30,80]),ie()},4900)}function C(G){if(r||G<0||G>=e.length||G===T)return;const ie=G>T?1:-1;T=G,N(G,ie)}function q(){C(T+1)}function D(){C(T-1)}function X(){const G=document.getElementById("card-viewport");if(!G||G._swipeBound)return;G._swipeBound=!0;let ie=0,ue=0,be=0,Te=!1;const Ee=ae=>ae.touches?ae.touches[0].clientX:ae.clientX,he=ae=>ae.touches?ae.touches[0].clientY:ae.clientY,$e=ae=>{Te=!0,ie=Ee(ae),ue=he(ae),be=0},Ae=ae=>{if(!Te)return;be=Ee(ae)-ie;const R=he(ae)-ue;if(Math.abs(be)<Math.abs(R))return;const Z=document.getElementById("current-card-wrap");Z&&(Z.style.transition="none",Z.style.transform=`translateX(${be*.6}px) rotate(${be*.02}deg)`)},te=()=>{if(!Te)return;Te=!1;const ae=document.getElementById("current-card-wrap"),R=55;be<=-R&&T<e.length-1?q():be>=R&&T>0?D():ae&&(ae.style.transition="transform .2s ease",ae.style.transform="translateX(0)")};G.addEventListener("pointerdown",$e),G.addEventListener("pointermove",Ae),G.addEventListener("pointerup",te),G.addEventListener("pointercancel",te),G.addEventListener("touchstart",$e,{passive:!0}),G.addEventListener("touchmove",Ae,{passive:!0}),G.addEventListener("touchend",te),G.addEventListener("click",ae=>{if(Math.abs(be)>8)return;const R=G.getBoundingClientRect();ae.clientX-R.left>R.width/2?q():D()})}let se=null;function oe(){const G=document.getElementById("fireworks-canvas");if(!G)return;G.width=window.innerWidth,G.height=window.innerHeight;const ie=G.getContext("2d"),ue=[];function be(){const Ee=Math.random()*G.width,he=Math.random()*G.height*.6,$e=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],Ae=$e[Math.floor(Math.random()*$e.length)];for(let te=0;te<60;te++){const ae=Math.PI*2/60*te,R=2+Math.random()*5;ue.push({x:Ee,y:he,vx:Math.cos(ae)*R,vy:Math.sin(ae)*R,alpha:1,color:Ae,size:2+Math.random()*3})}}be(),se=setInterval(be,600);function Te(){if(document.getElementById("fireworks-canvas")){ie.clearRect(0,0,G.width,G.height);for(let Ee=ue.length-1;Ee>=0;Ee--){const he=ue[Ee];if(he.x+=he.vx,he.y+=he.vy+.08,he.vy*=.98,he.alpha-=.018,he.alpha<=0){ue.splice(Ee,1);continue}ie.globalAlpha=he.alpha,ie.fillStyle=he.color,ie.beginPath(),ie.arc(he.x,he.y,he.size,0,Math.PI*2),ie.fill()}ie.globalAlpha=1,(se!==null||ue.length>0)&&requestAnimationFrame(Te)}}Te()}function P(){se!==null&&(clearInterval(se),se=null);const G=document.getElementById("fireworks-canvas");G&&G.getContext("2d").clearRect(0,0,G.width,G.height)}if(d){const G=document.getElementById("reveal-btns");G&&(G.innerHTML='<button class="btn btn-primary" id="reveal-next" style="flex:1">Continuer →</button>'),(U=document.getElementById("reveal-next"))==null||U.addEventListener("click",()=>{P(),c.remove(),d()})}else document.getElementById("reveal-collection").addEventListener("click",()=>{P(),c.remove(),i("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{P(),c.remove(),i("boosters")})}function xo(e){var t,i,d,c;if(e.card_type==="player"&&e.player)return po(e);if(e.card_type==="game_changer"){const a=e._gcDef,s=(a==null?void 0:a.gc_type)==="ultra_game_changer",l={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},p={purple:"#b06ce0",light_blue:"#00d4ef"},n=l[a==null?void 0:a.color]||l.purple,o=p[a==null?void 0:a.color]||p.purple,g=(a==null?void 0:a.name)||e.gc_type||"Game Changer",v=(a==null?void 0:a.effect)||((t=pi[e.gc_type])==null?void 0:t.desc)||"",y=a!=null&&a.image_url?`/manager-wars/icons/${a.image_url}`:null,T=((i=pi[e.gc_type])==null?void 0:i.icon)||"⚡";return`<div style="width:170px;height:240px;background:${n};border-radius:14px;border:3px solid ${o};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${o}88;flex-shrink:0">
    ...
    </div>`;if(e.card_type==="stadium"){const a=e._stadiumDef,s=(a==null?void 0:a.name)||"Stade",l=((d=a==null?void 0:a.club)==null?void 0:d.encoded_name)||(a==null?void 0:a.country_code)||"—",p=a!=null&&a.image_url?`/manager-wars/icons/${a.image_url}`:((c=a==null?void 0:a.club)==null?void 0:c.logo_url)||(a!=null&&a.country_code?`https://flagcdn.com/64x48/${a.country_code.toLowerCase()}.png`:null);return`<div style="width:160px;height:230px;background:linear-gradient(160deg,#E87722,#c45a00);border-radius:14px;border:3px solid #c45a00;display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px rgba(232,119,34,0.6)">
      ...
      </div>`,document.body.appendChild(d),d.addEventListener("click",s=>{s.target===d&&d.remove()}),(i=document.getElementById("odds-close"))==null||i.addEventListener("click",()=>d.remove());return}bo()}function bo(){const e=document.createElement("div");e.style.cssText=`position:fixed;inset:0;background:rgba(0,0,0,0.6);display:flex;
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}const ho="simulation",yn="VOTRE_ZONE_ID";function vo(){switch(ho){case"propellerads":return wo();case"adsense":return _o();default:return Ei()}}function Ei(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:#111;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:14px";let i=5;t.innerHTML=`
      ...
      <button id="ad-skip" disabled style="margin-top:8px;padding:10px 24px;border-radius:20px;border:none;background:rgba(255,255,255,0.15);color:rgba(255,255,255,0.4);font-size:13px;cursor:default">
        Passer (5)
      </button>`,document.body.appendChild(t);const d=setInterval(()=>{i--;const c=document.getElementById("ad-cd"),a=document.getElementById("ad-skip");c&&(c.textContent=i),a&&(a.textContent=i>0?`Passer (${i})`:"✓ Continuer"),i<=0&&(clearInterval(d),a&&(a.disabled=!1,a.style.cssText=a.style.cssText.replace("rgba(255,255,255,0.15)","#1A6B3C").replace("rgba(255,255,255,0.4)","#fff").replace("default","pointer")),a==null||a.addEventListener("click",()=>{t.remove(),e(!0)},{once:!0}))},1e3)})}function wo(){return new Promise((e,t)=>{if(typeof window.propeller>"u")return console.warn("[Ad] PropellerAds non chargé → simulation"),e(Ei());window.propeller.push({zone_id:yn,onComplete:()=>e(!0),onSkip:()=>t(new Error("Publicité ignorée")),onError:()=>t(new Error("Erreur publicité"))})})}function _o(){return new Promise((e,t)=>{if(typeof window.adsbygoogle>"u")return console.warn("[Ad] AdSense non chargé → simulation"),e(Ei());try{(window.adsbygoogle=window.adsbygoogle||[]).push({google_ad_client:"ca-pub-VOTRE_PUBLISHER_ID",google_ad_slot:yn,google_ad_format:"rewarded",on_reward:()=>e(!0),on_dismiss:()=>t(new Error("Publicité fermée"))})}catch(i){t(i)}})}async function $o(e,{state:t,navigate:i,toast:d,refreshProfile:c}){var v,y;const{data:a}=await E.from("users").select("*").eq("id",t.user.id).single();a&&(t.profile=a);let s=Array.isArray((v=t.profile)==null?void 0:v.pending_boosters)?[...t.profile.pending_boosters]:[];if(!s.length){await E.from("users").update({onboarding_done:!0}).eq("id",t.user.id),i("home");return}let l=null;try{const r=(await cn()).find(u=>(u.name||"").toLowerCase().includes("new player"));r&&(l=un(r))}catch(T){console.warn('[Onboarding] Config "Booster (new player)" introuvable, fallback taux par défaut',T)}const p=s.length;let n=0;e.innerHTML=`
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
  ...
  </div>`;const o=async()=>{await E.from("users").update({pending_boosters:s}).eq("id",t.user.id)};async function g(){var L;if(n>=p||!s.length){await E.from("users").update({pending_boosters:[],onboarding_done:!0}).eq("id",t.user.id),c&&await c(),d("Tous tes boosters sont ouverts ! Bon jeu 🎮","success",4e3),i("home");return}const T=s[0],{data:r}=await E.from("users").select("*").eq("id",t.user.id).single();r&&(t.profile=r);let u=[];try{if(T.type==="formation")u=await mn(t.profile,0);else if(T.type==="game_changer")u=await gn(t.profile,T.count||3,0);else if(l&&((L=l.rates)!=null&&L.length)){const N={...l,cost:0,cardCount:T.count||l.cardCount||5};u=await ui(t.profile,N),T.guaranteeGK&&!t.profile.first_booster_opened&&(u.some(C=>C.player&&C.player.job==="GK")||await ko(t.profile,u),await E.from("users").update({first_booster_opened:!0}).eq("id",t.profile.id))}else u=await fn(t.profile,T.count||5,0)}catch(N){d(N.message||"Erreur ouverture booster","error");return}s.shift(),n++,await o();const M=T.type==="formation"?{name:"Booster Formation",img:"/manager-wars/icons/booster-formation.png"}:T.type==="game_changer"?{name:"Booster Game Changer",img:"/manager-wars/icons/booster-gamechanger.png"}:{name:`Booster Joueurs (${n}/${p})`,img:(l==null?void 0:l.img)||"/manager-wars/icons/booster-players.png"};xn(u,M,i,()=>{g()})}(y=document.getElementById("onboard-start"))==null||y.addEventListener("click",()=>g())}async function ko(e,t){try{const{data:i}=await E.from("players").select("id,job,firstname,surname_encoded,country_code,club_id,rarity,note_g,note_d,note_m,note_a,skin,hair,hair_length,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0).eq("job","GK");if(!(i!=null&&i.length))return;const d=i[Math.floor(Math.random()*i.length)],c=t.findIndex(s=>s.player);if(c===-1)return;const a=t[c];await E.from("cards").update({player_id:d.id}).eq("id",a.id),t[c]={...a,player_id:d.id,player:d}}catch(i){console.warn("[Onboarding] ensureGK échec",i)}}const Lt={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},It={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function wt(e,t,i,d,c){var a;e.innerHTML=`<div class="match-screen" style="display:flex;align-items:center;justify-content:center;min-height:100vh">
    <div style="text-align:center;padding:40px;color:#fff">
      <div style="font-size:48px;margin-bottom:16px">${t}</div>
      <p style="margin-bottom:16px">${i}</p>
      <button class="btn btn-primary" id="msg-btn">${d}</button>
    </div>
  </div>`,(a=document.getElementById("msg-btn"))==null||a.addEventListener("click",c)}function Pe(e){const t=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function fi(e,t){var d,c;const i=e.player;return{cardId:e.id,position:t||null,id:i.id,firstname:i.firstname,name:i.surname_encoded,country_code:i.country_code,club_id:i.club_id,job:i.job,job2:i.job2,note_g:Number(i.note_g)||0,note_d:Number(i.note_d)||0,note_m:Number(i.note_m)||0,note_a:Number(i.note_a)||0,rarity:i.rarity,skin:i.skin,hair:i.hair,hair_length:i.hair_length,clubName:((d=i.clubs)==null?void 0:d.encoded_name)||null,clubLogo:((c=i.clubs)==null?void 0:c.logo_url)||null,boost:0,used:!1,_line:null,_col:null}}function Ht(e,t){if(!t||!e)return e;const{club_id:i,country_code:d}=t;return Object.values(e).forEach(c=>{Array.isArray(c)&&c.forEach(a=>{const s=i&&String(a.club_id)===String(i),l=d&&String(a.country_code)===String(d);(s||l)&&(a.note_g=(Number(a.note_g)||0)+10,a.note_d=(Number(a.note_d)||0)+10,a.note_m=(Number(a.note_m)||0)+10,a.note_a=(Number(a.note_a)||0)+10,a.boost=(a.boost||0)+10,a.stadiumBonus=!0)})}),e}function ct(e){return e===1?[1]:e===2?[0,2]:e===3?[0,1,2]:e===4?[0,1,1,2]:e===5?[0,1,1,1,2]:[1]}function ii(){const e=Math.random()*100;return e<10?3:e<30?2:1}function lt(e,t){const i=Lt[t]||Lt["4-4-2"],d={GK:[],DEF:[],MIL:[],ATT:[]};if(e.length&&e.every(s=>s.position)){for(const s of["GK","DEF","MIL","ATT"]){const l=e.filter(n=>n.position&&n.position.replace(/\d+$/,"")===s).sort((n,o)=>parseInt(n.position.replace(/\D+/g,""),10)-parseInt(o.position.replace(/\D+/g,""),10)).map(n=>({...n,_line:s})),p=ct(l.length);l.forEach((n,o)=>{n._col=p[o]}),d[s]=l}return d}const a=[...e];for(const s of["GK","DEF","MIL","ATT"]){const l=[];for(let n=0;n<i[s];n++){let o=a.findIndex(g=>g.job===s);if(o===-1&&(o=a.findIndex(g=>g.job2===s)),o===-1&&(o=0),a[o]){const g={...a[o],_line:s};l.push(g),a.splice(o,1)}}const p=ct(l.length);l.forEach((n,o)=>{n._col=p[o]}),d[s]=l}return d}function et(e){document.querySelectorAll(".top-nav, .bottom-nav").forEach(t=>{t.style.setProperty("display","none","important"),t.dataset.matchHidden="1"}),e&&e.style.setProperty("padding-bottom","0","important")}function We(e){document.querySelectorAll(".top-nav, .bottom-nav").forEach(t=>{t.style.removeProperty("display"),delete t.dataset.matchHidden}),e&&e.style.removeProperty("padding-bottom")}function ni(e,t,i){const c=new Set,a=t.filter(o=>{const g=o.gc_type||o.id;return c.has(g)?!1:(c.add(g),!0)});let s=[];function l(o,g){const v=o._gcDef,y={purple:"linear-gradient(135deg,#3d0a7a,#7a28b8)",light_blue:"linear-gradient(135deg,#006080,#00bcd4)"},T={purple:"#9b59b6",light_blue:"#00bcd4"},r=y[v==null?void 0:v.color]||y.purple,u=T[v==null?void 0:v.color]||T.purple;return`<div class="gc-select-card" data-id="${o.id}"
    ...
    </div>`}const p=o=>{e.style.overflow="",e.style.height="",e.style.display="",e.style.flexDirection="",i(o)};function n(){var g,v,y;e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column";const o=s.length>0;e.innerHTML=`
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      <div style="flex:1;overflow-y:auto;display:flex;flex-wrap:wrap;gap:10px;justify-content:center;align-content:flex-start;padding:8px 16px 16px">
        ${a.map(T=>{const r=s.find(u=>u.gc_type===T.gc_type);return l(T,!!r)}).join("")}
      </div>
    ...
    </div>`,e.querySelectorAll(".gc-select-card").forEach(T=>{T.addEventListener("click",()=>{const r=T.dataset.id,u=a.find(L=>L.id===r);if(!u)return;const M=s.findIndex(L=>L.gc_type===u.gc_type);M>-1?s.splice(M,1):s.length<3&&s.push(u),n()})}),(g=e.querySelector("#gc-launch"))==null||g.addEventListener("click",()=>{o&&p(s)}),(v=e.querySelector("#gc-no-gc"))==null||v.addEventListener("click",()=>p([])),(y=e.querySelector("#gc-reset"))==null||y.addEventListener("click",()=>{s.length&&(s=[],n())})}n()}function Eo(e){var d;const t=((d=e==null?void 0:e.state)==null?void 0:d.params)||{},i=t.matchMode||"vs_ai_easy";return i==="friend"?`Match vs ${t.friendName||"Ami"}`:i==="random"?"Match vs Random":`Match vs IA — ${i.replace("vs_ai_","").toUpperCase()}`}async function Ao(e,t,i){const{state:d,navigate:c}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:a}=await E.from("decks").select(`id,name,is_active,formation,stadium_card_id,
      stadium_card:cards!stadium_card_id(id,stadium_id,
        stadium_def:stadium_definitions(id,name,club_id,country_code,image_url,
          club:clubs(encoded_name,logo_url)))`).eq("owner_id",d.profile.id).order("created_at",{ascending:!1});if(!a||a.length===0){wt(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>c("decks"));return}const s=a.map(v=>v.id),{data:l}=await E.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,stadium_id,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",s).order("slot_order"),p=[...new Set((l||[]).filter(v=>{var y,T;return((y=v.card)==null?void 0:y.card_type)==="stadium"&&((T=v.card)==null?void 0:T.stadium_id)}).map(v=>v.card.stadium_id))],n={};if(p.length){const{data:v}=await E.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",p);(v||[]).forEach(y=>{n[y.id]=y}),(l||[]).forEach(y=>{var T,r;((T=y.card)==null?void 0:T.card_type)==="stadium"&&((r=y.card)!=null&&r.stadium_id)&&(y.card._stadiumDef=n[y.card.stadium_id]||null)})}let o=0;function g(){var C,q,D,X,se,oe,P;const v=a[o],y=(l||[]).filter(J=>J.deck_id===v.id),T=y.filter(J=>{var U;return J.is_starter&&((U=J.card)==null?void 0:U.player)}).map(J=>fi(J.card,J.position)),r=y.find(J=>{var U;return((U=J.card)==null?void 0:U.card_type)==="formation"}),u=v.formation||((C=r==null?void 0:r.card)==null?void 0:C.formation)||"4-4-2";let M=T.length>=11?lt(T,u):null,L=((q=v.stadium_card)==null?void 0:q.stadium_def)||null;L&&M&&(M=Ht(M,L));const N=T.length>=11;et(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div id="deck-swipe-zone" style="flex:1;min-height:0;overflow:hidden;position:relative;touch-action:pan-y;display:flex;align-items:center;justify-content:center;padding:4px">
        ${M?`<div class="deck-preview-wrap" style="aspect-ratio:1/1;max-width:100%;max-height:100%;width:auto;height:100%;overflow:hidden">${gt(M,u,null,[],285,285)}</div>`:`<div style="display:flex;align-items:center;justify-content:center;height:100%;opacity:.4;flex-direction:column;gap:8px">
      ...
      <div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">
        ${a.map((J,U)=>`<div style="width:7px;height:7px;border-radius:50%;background:${U===o?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}
      </div>`:""}
    ...
    </div>`,function(){const U=e.querySelector(".deck-preview-wrap svg");U&&(U.removeAttribute("width"),U.removeAttribute("height"),U.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",U.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),(X=document.getElementById("prev-deck"))==null||X.addEventListener("click",()=>{o>0&&(o--,g())}),(se=document.getElementById("next-deck"))==null||se.addEventListener("click",()=>{o<a.length-1&&(o++,g())}),(oe=document.getElementById("validate-deck"))==null||oe.addEventListener("click",()=>{if(!N)return;const J=t.state.params||{};t.navigate("match",{...J,matchMode:J.matchMode||i,deckId:v.id})}),(P=document.getElementById("cancel-deck-select"))==null||P.addEventListener("click",()=>{We(e),c("home")});const K=document.getElementById("deck-swipe-zone");if(K){let J=0,U=0;K.addEventListener("touchstart",G=>{J=G.touches[0].clientX,U=G.touches[0].clientY},{passive:!0}),K.addEventListener("touchend",G=>{const ie=G.changedTouches[0].clientX-J,ue=G.changedTouches[0].clientY-U;Math.abs(ie)<40||Math.abs(ie)<Math.abs(ue)||(ie<0&&o<a.length-1?(o++,g()):ie>0&&o>0&&(o--,g()))},{passive:!0})}}g()}function bn(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";return e!=null&&e.clubLogo?e.clubLogo.startsWith("http")?e.clubLogo:`${t}/storage/v1/object/public/assets/clubs/${e.clubLogo}`:null}function Ye(e,t=44,i=58){if(!e)return`<div style="width:${t}px;height:${i}px;border:1.5px dashed rgba(255,255,255,0.2);border-radius:5px"></div>`;const d=typeof e.portrait=="string"&&e.portrait.startsWith("http")?e.portrait:Pe(e),c=bn(e),a=e._line||e.job||"MIL",s=It[a]||"#555",l={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",p=e.note!==void 0?Number(e.note)||0:(Number(ze(e,a))||0)+(e.boost||0),n=hn(e==null?void 0:e.country_code),o=Math.round(i*.19),g=Math.round(i*.25),v=i-Math.round(i*.19)-Math.round(i*.25),y=e!=null&&e.used?.28:1;return`<div style="width:${t}px;height:${i}px;border-radius:5px;border:2px solid ${l};background:${s};position:relative;overflow:hidden;flex-shrink:0;opacity:${y}">
    <div style="position:absolute;top:0;left:0;right:0;height:${o}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:center;z-index:2">
      <span style="font-size:${Math.max(5,Math.round(t/9))}px;font-weight:900;color:#111;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${t-4}px">${((e==null?void 0:e.name)||"").slice(0,9)}</span>
    </div>
    ${d&&!(e!=null&&e.used)?`<img src="${d}" style="position:absolute;top:${o}px;left:0;width:${t}px;height:${v}px;object-fit:cover;object-position:top center">`:""}
    <div style="position:absolute;bottom:0;left:0;right:0;height:${g}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ...
      ${c?`<img src="${c}" style="width:${g-4}px;height:${g-4}px;object-fit:contain">`:e!=null&&e.clubName?`<span style="font-size:${Math.max(4,g-8)}px;font-weight:700;color:#333">${(e.clubName||"").slice(0,3).toUpperCase()}</span>`:""}
  ...
  </div>`}function nt(e,t,i){if(!(e!=null&&e.length))return"";const d=e.slice(0,5);let c='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return d.forEach((a,s)=>{if(c+=Ye(a,40,52),s<d.length-1){const l=at(a,d[s+1]);c+=`<div style="width:7px;height:3px;background:${l==="#ff3333"||l==="#cc2222"?"rgba(255,255,255,0.12)":l};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(c+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),c+="</div>",c}function Zt(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function hn(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function ft(e,t,i,d,c=310,a=310,s=[]){var N;const l=Pt[t]||{},p=Yi(t)||yi[t]||[],n={},o=["ATT","MIL","DEF","GK"];for(const K of o)(e[K]||[]).forEach((q,D)=>{n[`${K}${D+1}`]=q});function g(K){const C=l[K];return C?{x:C.x*c,y:C.y*a}:null}let v="";for(const[K,C]of p){const q=g(K),D=g(C);if(!q||!D)continue;const X=n[K],se=n[C],oe=at(X,se);oe==="#00ff88"||oe==="#FFD700"?(v+=`<line x1="${q.x.toFixed(1)}" y1="${q.y.toFixed(1)}" x2="${D.x.toFixed(1)}" y2="${D.y.toFixed(1)}"
        ...
        stroke="${oe}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const y=48,T=64,r=13,u=16,M={normal:"#aaaaaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#111111",legende:"#7a28b8",légende:"#7a28b8"};for(const[K,C]of Object.entries(n)){const q=g(K);if(!q||!C)continue;const D=K.replace(/[0-9]/g,""),X=It[D]||"#555",se=s.includes(C.cardId),oe=i==="attack"&&(["MIL","ATT"].includes(D)||se)&&!C.used||i==="defense"&&["GK","DEF","MIL"].includes(D)&&!C.used,P=d.includes(C.cardId);let J;i==="attack"?J=se?Math.max(1,Number(C.note_a)||0):D==="MIL"?Number(C.note_m)||0:Number(C.note_a)||0:i==="defense"?J=D==="GK"?Number(C.note_g)||0:D==="MIL"?Number(C.note_m)||0:Number(C.note_d)||0:J=Number(D==="GK"?C.note_g:D==="DEF"?C.note_d:D==="MIL"?C.note_m:C.note_a)||0,J=J+(C.boost||0);const U=(q.x-y/2).toFixed(1),G=(q.y-T/2).toFixed(1),ie=M[C==null?void 0:C.rarity]||M.normal;if(C.used){const Ae=typeof import.meta<"u"&&"/manager-wars/"||"/",ae=`${typeof window<"u"&&((N=window.location)==null?void 0:N.origin)||""}${Ae}icons/carte-dos.png`.replace(/([^:])\/\//g,"$1/");v+=`<rect x="${U}" y="${G}" width="${y}" height="${T}" rx="5" fill="#161616"/>`,v+=`<image href="${ae}" xlink:href="${ae}" x="${U}" y="${G}" width="${y}" height="${T}" preserveAspectRatio="xMidYMid slice"/>`,v+=`<rect x="${U}" y="${G}" width="${y}" height="${T}" rx="5" fill="none" stroke="${ie}" stroke-width="2" opacity="0.7"/>`,v+=`<rect x="${U}" y="${G}" width="${y}" height="${T}" rx="5" fill="rgba(0,0,0,0.01)" class="match-used-hit" data-card-id="${C.cardId}" data-role="${D}" style="cursor:pointer"/>`;continue}const ue=P?.45:1,be=P?"#FFD700":ie,Te=P?3:(C==null?void 0:C.rarity)==="legende"||(C==null?void 0:C.rarity)==="légende"||(C==null?void 0:C.rarity)==="pepite"||(C==null?void 0:C.rarity)==="pépite"?2.5:2,Ee=T-r-u;v+=`<defs><clipPath id="cp-${K}"><rect x="${U}" y="${(q.y-T/2+r).toFixed(1)}" width="${y}" height="${Ee}"/></clipPath></defs>`,v+=`<rect x="${U}" y="${G}" width="${y}" height="${T}" rx="5" fill="${X}" opacity="${ue}"/>`;const he=Pe(C);he&&(v+=`<image href="${he}" xlink:href="${he}" x="${U}" y="${(q.y-T/2+r).toFixed(1)}" width="${y}" height="${Ee}" clip-path="url(#cp-${K})" preserveAspectRatio="xMidYMin slice"/>`),v+=`<rect x="${U}" y="${G}" width="${y}" height="${r}" rx="4" fill="rgba(255,255,255,0.92)"/>`,v+=`<text x="${q.x.toFixed(1)}" y="${(q.y-T/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(C.name||"").slice(0,9)}</text>`;const $e=(q.y+T/2-u).toFixed(1);v+=`<rect x="${U}" y="${$e}" width="${y}" height="${u}" fill="rgba(255,255,255,0.92)"/>`;{const Ae=Zt(C.country_code);Ae?v+=`<image href="${Ae}" xlink:href="${Ae}" x="${(q.x-20).toFixed(1)}" y="${(q.y+T/2-u+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:v+=`<text x="${(q.x-13).toFixed(1)}" y="${(q.y+T/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${hn(C.country_code)}</text>`,v+=`<text x="${q.x.toFixed(1)}" y="${(q.y+T/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${J}</text>`;const te=bn(C);te?v+=`<image href="${te}" xlink:href="${te}" x="${(q.x+y/2-14).toFixed(1)}" y="${(q.y+T/2-u+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:C.clubName&&(v+=`<text x="${(q.x+14).toFixed(1)}" y="${(q.y+T/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(C.clubName||"").slice(0,3).toUpperCase()}</text>`),C.boost&&(v+=`<rect x="${(q.x+y/2-12).toFixed(1)}" y="${(q.y-T/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,v+=`<text x="${(q.x+y/2-5).toFixed(1)}" y="${(q.y-T/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${C.boost}</text>`)}v+=`<rect x="${U}" y="${G}" width="${y}" height="${T}" rx="5" fill="${P?"rgba(255,255,255,0.12)":"none"}" stroke="${be}" stroke-width="${Te}" opacity="${ue}"/>`,oe&&(v+=`<rect x="${U}" y="${G}" width="${y}" height="${T}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${P?"selected":""}" data-card-id="${C.cardId}" data-role="${D}" style="cursor:pointer"/>`)}const L=38;return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-L} ${-L} ${c+L*2} ${a+L*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${v}
  </svg>`}function gt(e,t,i,d,c=300,a=300,s=[]){return`<div id="match-terrain-wrap" style="position:relative;padding:0 4px">
    ${ft(e,t,i,d,c,a,s)}
  ...
  </div>`}async function oi(e,t,i,d){var q;const{state:c,navigate:a,toast:s}=t;et(e);const l=c.params||{};if(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!l.deckId)return Ao(e,t,i);const p=l.deckId;let n,o,g,v;try{const D=await Promise.all([E.from("decks").select("formation,name,stadium_card_id").eq("id",p).single(),E.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation,
            player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
              clubs(encoded_name,logo_url)))`).eq("deck_id",p).order("slot_order")]);n=D[0].data,g=D[0].error,o=D[1].data,v=D[1].error}catch(D){console.error("[Match] Exception chargement deck:",D),wt(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>a("home"));return}if(g||v){console.error("[Match] Erreur Supabase:",g||v),wt(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>a("home"));return}const y=(o||[]).filter(D=>{var X;return D.is_starter&&((X=D.card)==null?void 0:X.player)}).map(D=>fi(D.card,D.position)),T=(o||[]).filter(D=>{var X;return!D.is_starter&&((X=D.card)==null?void 0:X.player)}).map(D=>fi(D.card,D.position));if(y.length<11){wt(e,"⚠️",`Deck incomplet (${y.length}/11).`,"Compléter",()=>a("decks"));return}const r=(o||[]).find(D=>{var X;return((X=D.card)==null?void 0:X.card_type)==="formation"}),u=(n==null?void 0:n.formation)||((q=r==null?void 0:r.card)==null?void 0:q.formation)||"4-4-2",{data:M,error:L}=await E.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",c.profile.id).eq("card_type","game_changer"),{data:N}=await E.from("gc_definitions").select("*").eq("is_active",!0),K=(M||[]).map(D=>({...D,_gcDef:(N==null?void 0:N.find(X=>X.name===D.gc_type||X.id===D.gc_definition_id))||null}));let C=null;if(n!=null&&n.stadium_card_id){const{data:D}=await E.from("cards").select("stadium_id").eq("id",n.stadium_card_id).single();if(D!=null&&D.stadium_id){const{data:X}=await E.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").eq("id",D.stadium_id).single();C=X||null}}d({deckId:p,formation:u,starters:y,subsRaw:T,gcCardsEnriched:K,gcDefs:N||[],stadiumDef:C})}async function To(e,t){const{state:i}=t,c=(i.params||{}).matchMode||"vs_ai_easy",a=c.replace("vs_ai_",""),s=c;await oi(e,t,c,async({deckId:l,formation:p,starters:n,subsRaw:o,gcCardsEnriched:g,gcDefs:v,stadiumDef:y})=>{try{let T=lt(n,p);y&&(T=Ht(T,y));const r=await zo(p,a),u=async M=>{try{const{data:L,error:N}=await E.from("matches").insert({home_id:i.profile.id,away_id:null,mode:s,home_deck_id:l,status:"in_progress"}).select().single();if(N){console.error("[MatchIA] Erreur création match:",N),wt(e,"⚠️","Impossible de créer le match ("+N.message+").","Retour",()=>t.navigate("home"));return}const K={gcDefs:v||[],matchId:L==null?void 0:L.id,mode:s,difficulty:a,formation:p,homeTeam:T,aiTeam:r,homeSubs:o,subsUsed:0,maxSubs:Math.min(o.length,3),homeScore:0,aiScore:0,gcCards:M,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Lo(e,K,t)}catch(L){console.error("[MatchIA] Exception launchMatch:",L),wt(e,"⚠️","Erreur au lancement du match : "+L.message,"Retour",()=>t.navigate("home"))}};if(!g.length){u([]);return}ni(e,g,u)}catch(T){console.error("[MatchIA] Exception setup:",T),wt(e,"⚠️","Erreur de préparation du match : "+T.message,"Retour",()=>t.navigate("home"))}})}async function zo(e,t){var s,l;const{data:i}=await E.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(60);if(!i||i.length<11)return So(e);const d=Lt[e]||Lt["4-4-2"],c={GK:[],DEF:[],MIL:[],ATT:[]},a=[...i];for(const p of["GK","DEF","MIL","ATT"]){const n=a.filter(T=>T.job===p),o=a.filter(T=>T.job!==p),g=[...n,...o],v=[];for(let T=0;T<d[p];T++){const r=g[T]||a[T];r&&v.push({cardId:"ai-"+r.id+"-"+T,id:r.id,firstname:r.firstname,name:r.surname_encoded,country_code:r.country_code,club_id:r.club_id,job:r.job,job2:r.job2,note_g:Number(r.note_g)||0,note_d:Number(r.note_d)||0,note_m:Number(r.note_m)||0,note_a:Number(r.note_a)||0,rarity:r.rarity,skin:r.skin,hair:r.hair,hair_length:r.hair_length,clubName:((s=r.clubs)==null?void 0:s.encoded_name)||null,clubLogo:((l=r.clubs)==null?void 0:l.logo_url)||null,boost:0,used:!1,_line:p})}const y=ct(v.length);v.forEach((T,r)=>{T._col=y[r]}),c[p]=v}return c}function So(e){const t=Lt[e]||Lt["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},d=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let c=0;for(const a of["GK","DEF","MIL","ATT"]){const s=[];for(let p=0;p<t[a];p++){const n=3+Math.floor(Math.random()*5);s.push({cardId:"fake-"+c,id:"fake-"+c,firstname:"IA",name:d[c%d.length],country_code:"XX",club_id:null,job:a,job2:null,note_g:a==="GK"?n:2,note_d:a==="DEF"?n:2,note_m:a==="MIL"?n:2,note_a:a==="ATT"?n:2,rarity:"normal",boost:0,used:!1,_line:a}),c++}const l=ct(s.length);s.forEach((p,n)=>{p._col=l[n]}),i[a]=s}return i}function Lo(e,t,i){e.innerHTML=`
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
  </div>`,setTimeout(()=>Io(e,t,i),5e3)}function Io(e,t,i){const d=t.homeTeam.MIL||[],c=t.aiTeam.MIL||[];function a(r){return r.reduce((u,M)=>u+ze(M,"MIL"),0)}function s(r){let u=0;for(let M=0;M<r.length-1;M++){const L=at(r[M],r[M+1]);L==="#00ff88"?u+=2:L==="#FFD700"&&(u+=1)}return u}const l=a(d)+s(d),p=a(c)+s(c),n=l>=p;function o(r,u,M,L){return`<div id="duel-row-${L}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0), opacity .5s ease;transform-origin:center">
      ...
      <div class="duel-score-line duel-score-line-${L}" style="margin-top:10px;font-size:12px;color:rgba(255,255,255,0.55);opacity:0;transition:opacity .4s ease">
        Score: ${a(r)} + ${s(r)} liens = <b style="color:#fff">${a(r)+s(r)}</b>
      </div>
    </div>`}e.innerHTML=`
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const J=t.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
            ...
            ${nt((J.players||[]).map(U=>({...U,used:!1})),"#ff6b6b",J.total)}
            ...
            ${nt((J.players||[]).map(U=>({...U,used:!1})),"#00ff88",J.total)}
    ...
    ${(()=>{const P=window.innerWidth>=700,J=($e,Ae,te)=>{var Ce,De;const ae=(t.gcDefs||[]).find(I=>I.name===$e.gc_type),R={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ae==null?void 0:ae.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",Z={purple:"#b06ce0",light_blue:"#00d4ef"}[ae==null?void 0:ae.color]||"#b06ce0",ge=(ae==null?void 0:ae.name)||$e.gc_type,me=(ae==null?void 0:ae.effect)||((Ce=Ke[$e.gc_type])==null?void 0:Ce.desc)||"",xe=ae!=null&&ae.image_url?`/manager-wars/icons/${ae.image_url}`:null,re=((De=Ke[$e.gc_type])==null?void 0:De.icon)||"⚡",ce=Math.round(te*.22),Se=Math.round(te*.22),qe=te-ce-Se,Be=ge.length>12?7:9;return`<div class="gc-mini" data-gc-id="${$e.id}" data-gc-type="${$e.gc_type}"
          ...
          <div style="height:${qe}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
            ${xe?`<img src="${xe}" style="max-width:${Ae-10}px;max-height:${qe-6}px;object-fit:contain">`:`<span style="font-size:${Math.round(qe*.55)}px">${re}</span>`}
          </div>
          ...
          </div>`},G=($e,Ae)=>Ae?U(130,175):J($e,130,175),ie=($e,Ae)=>Ae?U(68,95):J($e,68,95),ue=P?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",be=T?`<button id="btn-results" style="${ue};background:linear-gradient(135deg,#D4A017,#FFD700);border:none;color:#000">🏁 Résultats</button>`:g?`<div style="${ue};background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);color:rgba(255,255,255,0.4)">⏳ Tour IA</div>`:v?`<button id="btn-action" style="${ue};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${t.selected.length===0?"disabled":""}> ⚔️ ATTAQUEZ <span id="match-timer" style="font-weight:900"></span></button>`:y?`<button id="btn-action" style="${ue};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${t.selected.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="match-timer" style="font-weight:900"></span></button>`:`<div style="${ue};background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1)"></div>`,Te=v||y?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${t.selected.length}/3 sélectionné(s)</div>`:"",Ee=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${P?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
        ${a.length===0?'<div style="font-size:7px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':a.map($e=>`<div class="sub-btn-col" data-sub-id="${$e.cardId}" style="cursor:pointer;flex-shrink:0">${Ye($e,76,100)}</div>`).join("")}
        ...
        <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
          ${gt(t.homeTeam,t.formation,t.phase,d,300,300,o)}
        </div>
      </div>`;return P?`
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(P=>{var J,U,G;if(P.type==="duel"){const ie=P.isGoal,ue=P.homeScored?"#FFD700":ie?"#ff6b6b":"rgba(255,255,255,0.3)",be=P.homeScored?"⚽ BUT !":ie?"⚽ BUT IA !":(J=P.homePlayers)!=null&&J.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${ie?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${ue};margin-bottom:4px">
  ...
  </div>`;function M(){const P=e.querySelector(".match-screen");if(!P)return;const J=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);P.style.bottom="auto",P.style.height=J+"px",P.style.minHeight=J+"px",P.style.maxHeight=J+"px",P.style.overflow="hidden";const U=e.querySelector("#mobile-action-bar"),G=e.querySelector("#mobile-play-area");U&&G&&(G.style.paddingBottom=U.offsetHeight+"px")}if(M(),setTimeout(M,120),setTimeout(M,400),setTimeout(M,1e3),t._vvBound||(t._vvBound=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",M),window.visualViewport.addEventListener("scroll",M)),window.addEventListener("resize",M)),function(){const J=e.querySelector(".terrain-wrapper svg");J&&(J.removeAttribute("width"),J.removeAttribute("height"),J.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",J.setAttribute("viewBox","-26 -26 352 352"),J.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),t._resizeBound||(t._resizeBound=!0,window.addEventListener("resize",()=>{const P=e.querySelector(".terrain-wrapper svg");P&&(P.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0")})),t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase==="attack"||t.phase==="defense"){let P=!1,J=30;const U=()=>document.getElementById("match-timer"),G=()=>{const ie=U();if(!ie)return;const ue=String(Math.floor(J/60)).padStart(2,"0"),be=String(J%60).padStart(2,"0");ie.textContent=` ${ue}:${be}`,ie.style.color=P?"#ff2222":"#ff9500",ie.style.fontWeight="900"};G(),t._timerInt=setInterval(()=>{if(J--,J<0)if(!P)P=!0,J=15,G();else{clearInterval(t._timerInt),t._timerInt=null,t.homeScore=0,t.aiScore=3;const ie=document.createElement("div");ie.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;color:#fff;padding:24px;text-align:center",ie.innerHTML='<div style="font-size:56px">⏱️</div><div style="font-size:24px;font-weight:900;color:#ff4444">MATCH PERDU PAR FORFAIT</div><div style="font-size:14px;color:rgba(255,255,255,0.6)">Temps écoulé</div>',document.body.appendChild(ie),setTimeout(()=>{ie.remove(),qt(e,t,i)},2500)}else G()},1e3)}(N=document.getElementById("match-quit"))==null||N.addEventListener("click",()=>{We(e),confirm("Abandonner ? Résultat : défaite 3-0")&&(t.homeScore=0,t.aiScore=3,qt(e,t,i))}),(K=document.getElementById("view-ai"))==null||K.addEventListener("click",()=>Ro(t,i)),(C=document.getElementById("toggle-history"))==null||C.addEventListener("click",()=>{var P;(P=document.getElementById("match-history-panel"))==null||P.classList.add("open")}),(q=document.getElementById("close-history"))==null||q.addEventListener("click",()=>{var P;(P=document.getElementById("match-history-panel"))==null||P.classList.remove("open")}),(D=document.getElementById("btn-action"))==null||D.addEventListener("click",()=>{t.selected.length!==0&&(v?jo(e,t,i):y&&qo(e,t,i))}),(X=document.getElementById("btn-results"))==null||X.addEventListener("click",()=>qt(e,t,i)),e.querySelectorAll(".match-slot-hit").forEach(P=>{P.addEventListener("click",()=>Co(P,t,e,i))}),e.querySelectorAll(".match-used-hit").forEach(P=>{P.addEventListener("click",()=>si(e,t,i,null,P.dataset.cardId))}),e.querySelectorAll(".gc-mini").forEach(P=>{P.addEventListener("click",()=>Do(P.dataset.gcId,P.dataset.gcType,e,t,i))}),(se=document.getElementById("boost-card"))==null||se.addEventListener("click",()=>No(e,t,i)),e.querySelectorAll(".sub-btn-col").forEach(P=>{P.addEventListener("click",()=>si(e,t,i,P.dataset.subId))}),(oe=document.getElementById("sub-btn-main"))==null||oe.addEventListener("click",()=>si(e,t,i))}function Co(e,t,i,d){const c=e.dataset.cardId,a=e.dataset.role,s=t.selected.findIndex(l=>l.cardId===c);if(s!==-1)t.selected.splice(s,1);else{if(t.selected.length>=3){d.toast("Maximum 3 joueurs","error");return}const l=(t.homeTeam[a]||[]).find(p=>p.cardId===c);l&&t.selected.push({...l,_role:a,_line:a})}Xe(i,t,d)}function Ai(e,t,i){e.matchId&&E.from("matches").update({last_player_id:i}).eq("id",e.matchId).then(()=>{})}function jo(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),Ai(t,i,i.state.profile.id);const d=!["GK","DEF","MIL","ATT"].some(s=>(t.aiTeam[s]||[]).some(l=>!l.used)),c=t.selected.map(s=>{const l=(t.homeTeam[s._role]||[]).find(n=>n.cardId===s.cardId)||s,p=d&&["GK","DEF"].includes(s._role);return{...l,_line:s._role,...p?{note_a:Math.max(1,Number(l.note_a)||0)}:{}}}),a=Nt(c,t.modifiers.home);t.pendingAttack={...a,players:[...c],side:"home"},t.selected.forEach(s=>{const l=(t.homeTeam[s._role]||[]).find(p=>p.cardId===s.cardId);l&&(l.used=!0)}),t.log.push({text:`⚔️ Vous attaquez : ${a.total} (base ${a.base}${a.links?` +${a.links} liens`:""}) — ${t.selected.map(s=>s.name).join(", ")}`,type:"info"}),t.selected=[],t.modifiers.home={},t.phase="ai-defense",Xe(e,t,i),setTimeout(()=>Fo(e,t,i),1200)}function qo(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),Ai(t,i,i.state.profile.id);const d=t.selected.map(l=>({...(t.homeTeam[l._role]||[]).find(n=>n.cardId===l.cardId)||l,_line:l._role})),c=Rt(d,t.modifiers.home);t.selected.forEach(l=>{const p=(t.homeTeam[l._role]||[]).find(n=>n.cardId===l.cardId);p&&(p.used=!0)});const a=Ot(t.pendingAttack.total,c.total,t.modifiers.home),s={type:"duel",title:"Défense",aiPlayers:(t.pendingAttack.players||[]).map(l=>({name:l.name,note:l._line==="MIL"?l.note_m:l.note_a,portrait:Pe(l),job:l.job,country_code:l.country_code,rarity:l.rarity,clubName:l.clubName,clubLogo:l.clubLogo})),homePlayers:t.selected.map(l=>{const p=(t.homeTeam[l._role]||[]).find(n=>n.cardId===l.cardId)||l;return{name:p.name,note:(p._line==="GK"?Number(p.note_g)||0:p._line==="MIL"?Number(p.note_m)||0:Number(p.note_d)||0)+(p.boost||0),portrait:Pe(p),job:p.job,country_code:p.country_code,rarity:p.rarity,clubName:p.clubName,clubLogo:p.clubLogo}}),homeTotal:c.total,aiTotal:t.pendingAttack.total,isGoal:!1,homeScored:!1,text:""};if(a.shielded)s.text="🛡️ Bouclier ! But annulé.",t.log.push(s);else if(a.goal){t.aiScore++,s.isGoal=!0,s.homeScored=!1,s.text=`⚽ BUT IA ! (${t.pendingAttack.total} > ${c.total})`,t.log.push(s),t.selected=[],t.modifiers.home={},t.pendingAttack=null,Xe(e,t,i),Qt(s.aiPlayers,t.homeScore,t.aiScore,!1,()=>{Tt(e,t,i,"home-attack")});return}else s.text=`🧤 Défense réussie ! (${c.total} ≥ ${t.pendingAttack.total})`,t.log.push(s);t.selected=[],t.modifiers.home={},t.pendingAttack=null,Tt(e,t,i,"home-attack")}function gi(e,t,i){Ai(t,i,null);const d=[...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]],c=tn(d,"attack",t.difficulty);if(!c.length){Jt(e,t,i);return}const a=Nt(c,t.modifiers.ai);t.pendingAttack={...a,players:c,side:"ai"},c.forEach(n=>{n.used=!0}),t.log.push({text:`🤖 IA attaque : ${a.total} (${c.map(n=>n.name).join(", ")})`,type:"info"}),t.modifiers.ai={};const s=[...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(n=>!n.used),p=(t.homeSubs||[]).filter(n=>!(t.usedSubIds||[]).includes(n.cardId)).length>0&&t.subsUsed<t.maxSubs;if(s.length===0&&!p){t.aiScore++;const n={type:"duel",isGoal:!0,homeScored:!1,aiPlayers:c.map(o=>({name:o.name,note:o._line==="MIL"?o.note_m:o.note_a,portrait:Pe(o),job:o.job,country_code:o.country_code,rarity:o.rarity,clubName:o.clubName,clubLogo:o.clubLogo})),aiTotal:a.total,text:"⚽ BUT IA ! (aucun défenseur disponible)"};t.log.push(n),t.pendingAttack=null,Xe(e,t,i),Qt(n.aiPlayers,t.homeScore,t.aiScore,!1,()=>{Tt(e,t,i,"home-attack")});return}t.phase="defense",Xe(e,t,i)}function Fo(e,t,i){var n,o;const d=[...t.aiTeam.GK||[],...t.aiTeam.DEF||[],...t.aiTeam.MIL||[]],c=tn(d,"defense",t.difficulty);if(!["GK","DEF","MIL","ATT"].flatMap(g=>(t.aiTeam[g]||[]).filter(v=>!v.used)).length){t.homeScore++;const v={type:"duel",isGoal:!0,homeScored:!0,homePlayers:(((n=t.pendingAttack)==null?void 0:n.players)||[]).map(y=>({name:y.name,note:ze(y,y._line||y.job),portrait:Pe(y),job:y.job,country_code:y.country_code,rarity:y.rarity,clubName:y.clubName,clubLogo:y.clubLogo})),homeTotal:((o=t.pendingAttack)==null?void 0:o.total)||0,aiTotal:0,text:"⚽ BUT ! L'IA n'a plus de joueurs — but automatique !"};t.log.push(v),t.modifiers.ai={},t.pendingAttack=null,Xe(e,t,i),Qt(v.homePlayers,t.homeScore,t.aiScore,!0,()=>{Tt(e,t,i,"ai-attack")});return}const s=c.length>0?Rt(c,t.modifiers.ai).total:0;c.forEach(g=>{g.used=!0});const l=Ot(t.pendingAttack.total,s,t.modifiers.ai),p={type:"duel",title:"Attaque",homePlayers:(t.pendingAttack.players||[]).map(g=>({name:g.name,note:g._line==="MIL"?g.note_m:g.note_a,portrait:Pe(g),job:g.job,country_code:g.country_code,rarity:g.rarity,clubName:g.clubName,clubLogo:g.clubLogo})),aiPlayers:c.map(g=>({name:g.name,note:g._line==="GK"?g.note_g:g._line==="MIL"?g.note_m:g.note_d,portrait:Pe(g),job:g.job,country_code:g.country_code,rarity:g.rarity,clubName:g.clubName,clubLogo:g.clubLogo})),homeTotal:t.pendingAttack.total,aiTotal:s,isGoal:!1,homeScored:!1,text:""};if(l.shielded)p.text="🛡️ Bouclier IA !",t.log.push(p);else if(l.goal){t.homeScore++,p.isGoal=!0,p.homeScored=!0,p.text=`⚽ BUT ! (${t.pendingAttack.total} > ${s})`,t.log.push(p),t.modifiers.ai={},t.pendingAttack=null,Xe(e,t,i),Qt(p.homePlayers,t.homeScore,t.aiScore,!0,()=>{Tt(e,t,i,"ai-attack")});return}else p.text=`🧤 IA défend (${s} ≥ ${t.pendingAttack.total})`,t.log.push(p);t.modifiers.ai={},t.pendingAttack=null,Tt(e,t,i,"ai-attack")}function Tt(e,t,i,d){if(t.round++,vn(t)){qt(e,t,i);return}if(d==="home-attack"){if(![...t.homeTeam.MIL||[],...t.homeTeam.ATT||[]].filter(a=>!a.used).length){if(![...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(l=>!l.used).length){Jt(e,t,i);return}if(![...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]].filter(l=>!l.used).length){Jt(e,t,i);return}setTimeout(()=>gi(e,t,i),100);return}t.phase="attack",Xe(e,t,i)}else{if(![...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]].filter(a=>!a.used).length){Jt(e,t,i);return}t.phase="ai-attack",Xe(e,t,i),setTimeout(()=>gi(e,t,i),800)}}function vn(e){const t=["MIL","ATT","GK","DEF"].some(d=>(e.homeTeam[d]||[]).some(c=>!c.used)),i=["MIL","ATT","GK","DEF"].some(d=>(e.aiTeam[d]||[]).some(c=>!c.used));return!t&&!i}function Jt(e,t,i){vn(t)?qt(e,t,i):(t.phase="attack",Xe(e,t,i))}function Bo(e,t,i){const d=document.createElement("div");d.style.cssText=`
    position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:800;
    display:flex;flex-direction:column;align-items:center;justify-content:center;gap:20px;
    animation:subFadeIn 0.2s ease;
  `;const c=Pe(e),a=Pe(t),s=It[e.job]||"#555",l=It[t.job]||"#555",p=e.job==="GK"?e.note_g:e.job==="DEF"?e.note_d:e.job==="MIL"?e.note_m:e.note_a,n=t.job==="GK"?t.note_g:t.job==="DEF"?t.note_d:t.job==="MIL"?t.note_m:t.note_a;d.innerHTML=`
    <style>
  ...
  `,document.body.appendChild(d);let o=!1;const g=()=>{o||(o=!0,d.remove(),i())};d.addEventListener("click",g),setTimeout(g,2e3)}function Mt(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function si(e,t,i,d=null,c=null){var y,T;if(t.phase!=="attack"){Mt("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){Mt(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const a=Object.entries(t.homeTeam).flatMap(([r,u])=>(u||[]).filter(M=>M.used).map(M=>({...M,_line:M._line||r}))),s=t.homeSubs.filter(r=>!t.usedSubIds.includes(r.cardId));if(!a.length){Mt("Aucun joueur utilisé à remplacer");return}if(!s.length){Mt("Aucun remplaçant disponible");return}let l=Math.max(0,a.findIndex(r=>r.cardId===c));const p=((y=a[l])==null?void 0:y._line)||((T=a[l])==null?void 0:T.job);let n=d?Math.max(0,s.findIndex(r=>r.cardId===d)):Math.max(0,s.findIndex(r=>r.job===p)),o=!1;const g=document.createElement("div");g.id="sub-overlay",g.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function v(){var C,q,D,X,se,oe;const r=a[l],u=s[n],M=Math.min(130,Math.round((window.innerWidth-90)/2)),L=Math.round(M*1.35),N=P=>`background:rgba(255,255,255,0.12);border:none;color:${P?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${P?"default":"pointer"};flex-shrink:0`;g.innerHTML=`
    ...
    <div style="flex:1;display:flex;gap:0;overflow:hidden">
      ...
      <div id="in-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
        ...
        <button id="in-down" style="${N(n>=s.length-1)}" ${n>=s.length-1?"disabled":""}>▼</button>
      ...
      <div id="out-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
        ...
        <button id="out-down" style="${N(l>=a.length-1)}" ${l>=a.length-1?"disabled":""}>▼</button>
        <div style="font-size:10px;color:rgba(255,255,255,0.35)">${l+1}/${a.length}</div>
      </div>
    </div>
    <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
      <button id="sub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
    </div>`,(C=g.querySelector("#sub-close"))==null||C.addEventListener("click",()=>g.remove()),(q=g.querySelector("#out-up"))==null||q.addEventListener("click",()=>{l>0&&(l--,v())}),(D=g.querySelector("#out-down"))==null||D.addEventListener("click",()=>{l<a.length-1&&(l++,v())}),(X=g.querySelector("#in-up"))==null||X.addEventListener("click",()=>{n>0&&(n--,v())}),(se=g.querySelector("#in-down"))==null||se.addEventListener("click",()=>{n<s.length-1&&(n++,v())});const K=(P,J,U,G)=>{const ie=g.querySelector("#"+P);if(!ie)return;let ue=0;ie.addEventListener("touchstart",be=>{ue=be.touches[0].clientY},{passive:!0}),ie.addEventListener("touchend",be=>{const Te=be.changedTouches[0].clientY-ue;if(Math.abs(Te)<30)return;const Ee=J();Te<0&&Ee<G-1?(U(Ee+1),v()):Te>0&&Ee>0&&(U(Ee-1),v())},{passive:!0})};K("in-panel",()=>n,P=>n=P,s.length),K("out-panel",()=>l,P=>l=P,a.length),(oe=g.querySelector("#sub-confirm"))==null||oe.addEventListener("click",P=>{if(P.preventDefault(),P.stopPropagation(),o)return;o=!0;const J=a[l],U=s[n];if(!J||!U)return;let G=null,ie=-1;for(const[be,Te]of Object.entries(t.homeTeam)){const Ee=(Te||[]).findIndex(he=>he.cardId===J.cardId);if(Ee!==-1){G=be,ie=Ee;break}}if(ie===-1||!G){Mt("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),g.remove();return}const ue={...U,_line:G,_col:J._col||0,used:!1,boost:0};t.homeTeam[G].splice(ie,1,ue),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(U.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:J.name,firstname:J.firstname,note:ze(J,G),portrait:Pe(J),job:J.job,country_code:J.country_code,rarity:J.rarity,clubName:J.clubName,clubLogo:J.clubLogo},inPlayer:{name:U.name,firstname:U.firstname,note:ze(U,G),portrait:Pe(U),job:U.job,country_code:U.country_code,rarity:U.rarity,clubName:U.clubName,clubLogo:U.clubLogo},text:`🔄 ${U.firstname} ${U.name} remplace ${J.firstname} ${J.name}`}),g.remove(),Bo(J,U,()=>Xe(e,t,i))})}document.body.appendChild(g),v()}function Do(e,t,i,d,c){var T,r;const a=(d.gcDefs||[]).find(u=>u.name===t),s=Ke[t]||{icon:"⚡",desc:"Carte spéciale."},l={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[a==null?void 0:a.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",p={purple:"#b06ce0",light_blue:"#00d4ef"}[a==null?void 0:a.color]||"#b06ce0",n=(a==null?void 0:a.name)||t,o=(a==null?void 0:a.effect)||s.desc,g=a!=null&&a.image_url?`/manager-wars/icons/${a.image_url}`:null,v=s.icon||"⚡",y=document.createElement("div");y.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",y.innerHTML=`
    ...
    </div>`,document.body.appendChild(y),(T=y.querySelector("#gc-back"))==null||T.addEventListener("click",()=>y.remove()),(r=y.querySelector("#gc-use"))==null||r.addEventListener("click",()=>{y.remove(),Po(e,t,i,d,c)})}function Yt(e,t,i,d,c,a){const s=document.createElement("div");s.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let l=[];function p(){var n,o;s.innerHTML=`
    ...
    </div>`,(n=s.querySelector("#gc-picker-close"))==null||n.addEventListener("click",()=>s.remove()),s.querySelectorAll(".gc-pick-item").forEach(g=>{g.addEventListener("click",()=>{const v=g.dataset.cid,y=e.find(r=>r.cardId===v);if(!y)return;const T=l.findIndex(r=>r.cardId===v);T>-1?l.splice(T,1):l.length<t&&l.push(y),p()})}),(o=s.querySelector("#gc-picker-confirm"))==null||o.addEventListener("click",()=>{s.remove(),a(l)})}p(),document.body.appendChild(s)}const Go={BOOST_STAT:({value:e=1,count:t=1,roles:i=[]},d,c,a)=>{const s=Object.entries(d.homeTeam).filter(([l])=>!i.length||i.includes(l)).flatMap(([l,p])=>p.filter(n=>!n.used).map(n=>({...n,_line:l})));return s.length?(Yt(s,t,`Choisir ${t} joueur(s) à booster (+${e})`,c,d,l=>{l.forEach(p=>{const n=(d.homeTeam[p._line]||[]).find(o=>o.cardId===p.cardId);n&&(n.boost=(n.boost||0)+e,d.log.push({text:`⚡ +${e} sur ${n.name}`,type:"info"}))}),Xe(c,d,a)}),!0):(d.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),Xe(c,d,a),!0)},DEBUFF_STAT:({value:e=1,count:t=1,roles:i=[],target:d="ai"},c,a,s)=>{const l=d==="home"?c.homeTeam:c.aiTeam,p=d==="ai"?"adverse":"allié",n=Object.entries(l).filter(([o])=>!i.length||i.includes(o)).flatMap(([o,g])=>g.filter(v=>!v.used).map(v=>({...v,_line:o})));return n.length?(Yt(n,t,`Choisir ${t} joueur(s) ${p}(s) à débuffer (-${e})`,a,c,o=>{o.forEach(g=>{const y=((d==="home"?c.homeTeam:c.aiTeam)[g._line]||[]).find(T=>T.cardId===g.cardId);y&&(y.boost=(y.boost||0)-e,c.log.push({text:`🎯 -${e} sur ${y.name}${d==="ai"?" (IA)":""}`,type:"info"}))}),Xe(a,c,s)}),!0):(c.log.push({text:`🎯 Aucun joueur ${p} disponible`,type:"info"}),Xe(a,c,s),!0)},GRAY_PLAYER:({count:e=1,roles:t=[],target:i="ai"},d,c,a)=>{const s=i==="home"?d.homeTeam:d.aiTeam,l=i==="ai"?"adverse":"allié",p=Object.entries(s).filter(([n])=>!t.length||t.includes(n)).flatMap(([n,o])=>o.filter(g=>!g.used).map(g=>({...g,_line:n})));return p.length?(Yt(p,e,`Choisir ${e} joueur(s) ${l}(s) à exclure`,c,d,n=>{n.forEach(o=>{const v=((i==="home"?d.homeTeam:d.aiTeam)[o._line]||[]).find(y=>y.cardId===o.cardId);v&&(v.used=!0,d.log.push({text:`❌ ${v.name}${i==="ai"?" (IA)":""} exclu !`,type:"info"}))}),Xe(c,d,a)}),!0):(d.log.push({text:`❌ Aucun joueur ${l} à exclure`,type:"info"}),Xe(c,d,a),!0)},REVIVE_PLAYER:({count:e=1,roles:t=[]},i,d,c)=>{const a=Object.entries(i.homeTeam).filter(([s])=>!t.length||t.includes(s)).flatMap(([s,l])=>l.filter(p=>p.used).map(p=>({...p,_line:s})));return a.length?(Yt(a,e,`Choisir ${e} joueur(s) à ressusciter`,d,i,s=>{s.forEach(l=>{const p=(i.homeTeam[l._line]||[]).find(n=>n.cardId===l.cardId);p&&(p.used=!1,i.log.push({text:`💫 ${p.name} ressuscité !`,type:"info"}))}),Xe(d,i,c)}),!0):(i.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),Xe(d,i,c),!0)},REMOVE_GOAL:({},e)=>(e.aiScore>0?(e.aiScore--,e.log.push({text:"🚫 Dernier but IA annulé !",type:"info"})):e.log.push({text:"🚫 Aucun but à annuler",type:"info"}),!1),ADD_GOAL_DRAW:({},e)=>(e.homeScore===e.aiScore?(e.homeScore++,e.log.push({text:"🎯 But bonus (match nul) !",type:"info"})):e.log.push({text:"🎯 But bonus : non applicable (pas de match nul)",type:"info"}),!1),ADD_SUB:({value:e=1},t)=>(t.maxSubs=(t.maxSubs||3)+e,t.log.push({text:`🔄 +${e} remplacement(s) débloqué(s)`,type:"info"}),!1),CUSTOM:()=>!1};function Po(e,t,i,d,c){d.usedGc.push(e);const a=d.gcDefs||[],s=a.find(p=>p.name===t)||a.find(p=>{var n;return((n=p.name)==null?void 0:n.toLowerCase().trim())===(t==null?void 0:t.toLowerCase().trim())});let l=!1;if(s!=null&&s.effect_type&&s.effect_type!=="CUSTOM"){const p=Go[s.effect_type];p?p(s.effect_params||{},d,i,c)||(l=!0):(c.toast(`Effet "${s.effect_type}" non implémenté`,"error"),l=!0)}else{switch(t){case"Double attaque":d.modifiers.home.doubleAttack=!0,d.log.push({text:"⚡ Double attaque activée !",type:"info"});break;case"Bouclier":d.modifiers.home.shield=!0,d.log.push({text:"🛡️ Bouclier activé !",type:"info"});break;case"Ressusciter":{const p=Object.entries(d.homeTeam).flatMap(([n,o])=>(o||[]).filter(g=>g.used).map(g=>({...g,_line:n})));p.length?(p[0].used=!1,d.log.push({text:`💫 ${p[0].name} ressuscité !`,type:"info"})):d.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"});break}case"Vol de note":d.modifiers.ai.stolenNote=(d.modifiers.ai.stolenNote||0)+1,d.log.push({text:"🎯 -1 à la prochaine attaque IA",type:"info"});break;case"Gel":{const p=[...d.aiTeam.ATT||[],...d.aiTeam.MIL||[]].filter(n=>!n.used);if(p.length){const n=p.sort((o,g)=>ze(g,"ATT")-ze(o,"ATT"))[0];n.used=!0,d.log.push({text:`❄️ ${n.name} (IA) gelé !`,type:"info"})}break}case"Remplacement+":d.maxSubs++,d.log.push({text:"🔄 +1 remplacement débloqué",type:"info"});break}l=!0}E.from("cards").delete().eq("id",e).then(()=>{}),l&&Xe(i,d,c)}function No(e,t,i){const d=Object.values(t.homeTeam).flat().filter(c=>!c.used);if(!d.length){i.toast("Aucun joueur actif à booster","error");return}i.openModal("⚡ Utiliser le Boost",`<div style="margin-bottom:12px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border-radius:10px;padding:12px;text-align:center;color:#000">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(c=>{c.addEventListener("click",()=>{const a=c.dataset.cardId;for(const s of["GK","DEF","MIL","ATT"]){const l=(t.homeTeam[s]||[]).find(p=>p.cardId===a);if(l){l.boost=(l.boost||0)+t.boostCard.value,t.log.push({text:`⚡ Boost +${t.boostCard.value} appliqué à ${l.name}`,type:"info"});break}}t.boostUsed=!0,i.closeModal(),Xe(e,t,i)})})}function Qt(e,t,i,d,c){const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:900;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;overflow:hidden;cursor:pointer";const s=Array.from({length:10},(n,o)=>`
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(a);let l=!1;const p=()=>{l||(l=!0,a.remove(),setTimeout(()=>c(),50))};a.addEventListener("click",p),setTimeout(p,3500)}async function qt(e,t,i){var o,g;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:d}=i,c=t.homeScore>t.aiScore,a=t.homeScore===t.aiScore,s=c?"victoire":a?"nul":"defaite",l=Vn(t.mode,s);t.matchId&&await E.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:c?d.profile.id:null,home_credits_reward:l,played_at:new Date().toISOString()}).eq("id",t.matchId);const p={credits:(d.profile.credits||0)+l,matches_played:(d.profile.matches_played||0)+1};c?p.wins=(d.profile.wins||0)+1:a?p.draws=(d.profile.draws||0)+1:p.losses=(d.profile.losses||0)+1,await E.from("users").update(p).eq("id",d.profile.id),await i.refreshProfile();const n=document.createElement("div");n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",n.innerHTML=`
    ...
    </div>`,`<button class="btn btn-primary" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}async function wn(e,t){var i,d;try{const c=(d=(i=t==null?void 0:t.state)==null?void 0:i.profile)==null?void 0:d.id;try{(E.getChannels?E.getChannels():[]).forEach(s=>{const l=s.topic||"";(l.includes("matchmaking")||l.includes("pvp-match")||l.includes("friend-invite"))&&E.removeChannel(s)})}catch(a){console.warn("[Random] cleanup canaux:",a)}c&&await E.rpc("cancel_matchmaking",{p_user_id:c}).catch(()=>{})}catch{}await oi(e,t,"random",({deckId:c,formation:a,starters:s,subsRaw:l,gcCardsEnriched:p,gcDefs:n,stadiumDef:o})=>{const g=v=>Oo(e,t,c,a,s,l,v,n||[],o);if(!p.length){g([]);return}ni(e,p,g)})}async function Oo(e,t,i,d,c,a,s=[],l=[],p=null){var N;const{state:n,navigate:o,toast:g}=t;let v=!1,y=null;et(e),e.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const T=async(K=!0)=>{v=!0,y&&(E.removeChannel(y),y=null),K&&await E.rpc("cancel_matchmaking",{p_user_id:n.profile.id}).catch(()=>{})};(N=document.getElementById("mm-cancel"))==null||N.addEventListener("click",async()=>{try{await T(!0)}catch{}We(e),o("home")});const r=async(K,C,q)=>{if(v)return;v=!0,y&&(E.removeChannel(y),y=null);const D=document.getElementById("mm-status");D&&(D.textContent="Adversaire trouvé !"),await new Promise(X=>setTimeout(X,600)),e.isConnected&&_n(e,t,K,q,s,l)},{data:u,error:M}=await E.rpc("try_matchmake",{p_user_id:n.profile.id,p_deck_id:i});if(M||!(u!=null&&u.success)){g("Erreur de matchmaking","error"),We(e),o("home");return}if(u.matched){r(u.match_id,u.opponent_id,!1);return}const L=document.getElementById("mm-status");L&&(L.textContent="En attente d'un autre joueur..."),y=E.channel("matchmaking-"+n.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${n.profile.id}`},K=>{const C=K.new;C.status==="matched"&&C.match_id&&r(C.match_id,C.matched_with,!0)}).subscribe()}async function Ho(e,t,i){const{state:d,navigate:c,toast:a}=t;try{(E.getChannels?E.getChannels():[]).forEach(n=>{const o=n.topic||"";(o.includes("matchmaking")||o.includes("pvp-match")||o.includes("friend-invite"))&&E.removeChannel(n)})}catch{}const{data:s}=await E.from("matches").select("home_id, away_id, status").eq("id",i).single();if(!s){a("Match introuvable","error"),c("home");return}if(s.status==="finished"){a("Ce match est terminé","info"),c("home");return}const l=s.home_id===d.profile.id;_n(e,t,i,l,[],[])}async function _n(e,t,i,d,c=[],a=[]){const{state:s,navigate:l,toast:p}=t,n=d?"p1":"p2",o=d?"p2":"p1",g=(c||[]).map(I=>I.id),v=(c||[]).map(I=>({id:I.id,gc_type:I.gc_type,_gcDef:I._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:y}=await E.from("matches").select("*").eq("id",i).single();if(!y){p("Match introuvable","error"),l("home");return}async function T(){const[{data:I},{data:j},{data:w},{data:k}]=await Promise.all([E.rpc("get_deck_for_match",{p_deck_id:y.home_deck_id}),E.rpc("get_deck_for_match",{p_deck_id:y.away_deck_id}),E.from("users").select("id,pseudo,club_name").eq("id",y.home_id).single(),E.from("users").select("id,pseudo,club_name").eq("id",y.away_id).single()]),f=_=>({cardId:_.card_id,position:_.position,id:_.id,firstname:_.firstname,name:_.surname_encoded,country_code:_.country_code,club_id:_.club_id,job:_.job,job2:_.job2,note_g:Number(_.note_g)||0,note_d:Number(_.note_d)||0,note_m:Number(_.note_m)||0,note_a:Number(_.note_a)||0,rarity:_.rarity,skin:_.skin,hair:_.hair,hair_length:_.hair_length,clubName:_.club_encoded_name||null,clubLogo:_.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}),m=((I==null?void 0:I.starters)||[]).map(_=>f(_)),x=((j==null?void 0:j.starters)||[]).map(_=>f(_)),b=(I==null?void 0:I.formation)||"4-4-2",h=(j==null?void 0:j.formation)||"4-4-2";let $=lt(m,b),A=lt(x,h);return stadiumDef&&d&&($=Ht($,stadiumDef)),{p1Team:$,p2Team:A,p1Subs:((I==null?void 0:I.subs)||[]).map(_=>f(_)),p2Subs:((j==null?void 0:j.subs)||[]).map(_=>f(_)),p1Formation:b,p2Formation:h,p1Name:(w==null?void 0:w.club_name)||(w==null?void 0:w.pseudo)||"Joueur 1",p2Name:(k==null?void 0:k.club_name)||(k==null?void 0:k.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:d?g:[],gc_p2:d?[]:g,gcCardsFull_p1:d?v:[],gcCardsFull_p2:d?[]:v,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:a||[],lastActionAt:new Date().toISOString()}}let r=y.game_state&&Object.keys(y.game_state).length?y.game_state:null;if(!r)if(d){r=await T();const{data:I}=await E.from("matches").select("game_state").eq("id",i).single();!(I!=null&&I.game_state)||!Object.keys(I.game_state).length?await E.from("matches").update({game_state:r,turn_user_id:y.home_id}).eq("id",i):r=I.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let I=0;I<30&&!r;I++){await new Promise(w=>setTimeout(w,400));const{data:j}=await E.from("matches").select("game_state").eq("id",i).single();j!=null&&j.game_state&&Object.keys(j.game_state).length&&(r=j.game_state)}if(!r){p("Erreur de synchronisation","error"),l("home");return}r.gc_p2=g,r.gcCardsFull_p2=v,await E.from("matches").update({game_state:r}).eq("id",i)}let u=!1,M=null,L=!1;const N=new Set,K=new Set;function C(I){var $,A;try{E.removeChannel(q)}catch{}const j=r[n+"Score"]||0,w=r[o+"Score"]||0;let k,f;I.winner_id?(k=I.winner_id===s.profile.id,f=!1):I.forfeit?(k=j>w,f=!1):(f=j===w,k=j>w),($=document.getElementById("pvp-end-overlay"))==null||$.remove();const m=document.createElement("div");m.id="pvp-end-overlay",m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const x=f?"🤝":k?"🏆":"😞",b=f?"MATCH NUL":k?"VICTOIRE !":"DÉFAITE",h=f?"#fff":k?"#FFD700":"#ff6b6b";m.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(m),(A=m.querySelector("#pvp-end-home"))==null||A.addEventListener("click",()=>{m.remove(),We(e),l("home")})}const q=E.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},I=>{const j=I.new;try{if(j.status==="finished"||j.forfeit){if(u)return;u=!0,M&&(clearInterval(M),M=null),j.game_state&&(r=j.game_state),C(j);return}if(j.game_state){const w=r;r=j.game_state;const k=r._lastGC;if(k&&k.seq&&!K.has(k.seq)&&(K.add(k.seq),k.by!==n)){Ee(k.type,k.by,()=>G());return}const f=w[n+"Score"]||0,m=w[o+"Score"]||0,x=r[n+"Score"]||0,b=r[o+"Score"]||0,h=x>f,$=b>m;if((h||$)&&!N.has(r.round)){N.add(r.round);const A=[...r.log||[]].reverse().find(z=>z.isGoal),_=((A==null?void 0:A.homePlayers)||[]).map(z=>({name:z.name,note:z.note,portrait:z.portrait,job:z.job}));Te(_,x,b,h,()=>G());return}G()}}catch(w){console.error("[PvP] crash:",w)}}).subscribe();async function D(I){Object.assign(r,I),r.lastActionAt=new Date().toISOString();const{error:j}=await E.from("matches").update({game_state:r}).eq("id",i);j&&p("Erreur de synchronisation","error");try{G()}catch(w){console.error("[PvP] renderPvpScreen crash:",w)}}async function X(){if(u)return;u=!0,M&&(clearInterval(M),M=null);const I=d?y.away_id:y.home_id,j=d?"p2":"p1",w=d?"p1":"p2",k={...r,[j+"Score"]:3,[w+"Score"]:0,phase:"finished"};try{await E.from("matches").update({status:"finished",forfeit:!0,winner_id:I,home_score:k.p1Score||0,away_score:k.p2Score||0,game_state:k}).eq("id",i)}catch{}try{E.removeChannel(q)}catch{}setTimeout(()=>{We(e),l("home")},800)}const se={BOOST_STAT:({value:I=1,count:j=1,roles:w=[]},k,f)=>{const m=k[n+"Team"],x=Object.entries(m).filter(([b])=>!w.length||w.includes(b)).flatMap(([b,h])=>h.filter($=>!$.used).map($=>({...$,_line:b})));if(!x.length){k.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),f(k);return}oe(x,j,`Choisir ${j} joueur(s) à booster (+${I})`,b=>{b.forEach(h=>{const $=(m[h._line]||[]).find(A=>A.cardId===h.cardId);$&&($.boost=($.boost||0)+I,k.log.push({text:`⚡ +${I} sur ${$.name}`,type:"info"}))}),k[n+"Team"]=m,f(k)})},DEBUFF_STAT:({value:I=1,count:j=1,roles:w=[],target:k="ai"},f,m)=>{const x=k==="home"?n:o,b=f[x+"Team"],h=k==="home"?"allié":"adverse",$=Object.entries(b).filter(([A])=>!w.length||w.includes(A)).flatMap(([A,_])=>_.map(z=>({...z,_line:A})));if(!$.length){f.log.push({text:`🎯 Aucun joueur ${h}`,type:"info"}),m(f);return}oe($,j,`Choisir ${j} joueur(s) ${h}(s) (-${I})`,A=>{A.forEach(_=>{const z=(b[_._line]||[]).find(S=>S.cardId===_.cardId);z&&(z.boost=(z.boost||0)-I,f.log.push({text:`🎯 -${I} sur ${z.name}`,type:"info"}))}),f[x+"Team"]=b,m(f)})},GRAY_PLAYER:({count:I=1,roles:j=[],target:w="ai"},k,f)=>{const m=w==="home"?n:o,x=k[m+"Team"],b=w==="home"?"allié":"adverse",h=Object.entries(x).filter(([$])=>!j.length||j.includes($)).flatMap(([$,A])=>A.filter(_=>!_.used).map(_=>({..._,_line:$})));if(!h.length){k.log.push({text:`❌ Aucun joueur ${b}`,type:"info"}),f(k);return}oe(h,I,`Choisir ${I} joueur(s) ${b}(s) à exclure`,$=>{const A="usedCardIds_"+m,_=new Set(k[A]||[]);$.forEach(z=>{const S=(x[z._line]||[]).find(F=>F.cardId===z.cardId);S&&(S.used=!0,_.add(z.cardId),k.log.push({text:`❌ ${S.name} exclu !`,type:"info"}))}),k[A]=[..._],k[m+"Team"]=x,f(k)})},REVIVE_PLAYER:({count:I=1,roles:j=[]},w,k)=>{const f=w[n+"Team"],m=Object.entries(f).filter(([x])=>!j.length||j.includes(x)).flatMap(([x,b])=>b.filter(h=>h.used).map(h=>({...h,_line:x})));if(!m.length){w.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),k(w);return}oe(m,I,`Choisir ${I} joueur(s) à ressusciter`,x=>{x.forEach(b=>{const h=(f[b._line]||[]).find($=>$.cardId===b.cardId);h&&(h.used=!1,w.log.push({text:`💫 ${h.name} ressuscité !`,type:"info"}))}),w[n+"Team"]=f,k(w)})},REMOVE_GOAL:({},I,j)=>{const w=o+"Score";I[w]>0?(I[w]--,I.log.push({text:"🚫 Dernier but annulé !",type:"info"})):I.log.push({text:"🚫 Aucun but à annuler",type:"info"}),j(I)},ADD_GOAL_DRAW:({},I,j)=>{I[n+"Score"]===I[o+"Score"]?(I[n+"Score"]++,I.log.push({text:"🎯 But bonus !",type:"info"})):I.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),j(I)},ADD_SUB:({value:I=1},j,w)=>{j.maxSubs=(j.maxSubs||3)+I,j.log.push({text:`🔄 +${I} remplacement(s)`,type:"info"}),w(j)},CUSTOM:({},I,j)=>j(I)};function oe(I,j,w,k){const f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let m=[];function x(){var h,$;const b=I.map(A=>{const _={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[A._line]||"#555",z=ze(A,A._line)+(A.boost||0),F=m.find(O=>O.cardId===A.cardId)?"#FFD700":"rgba(255,255,255,0.25)",B=A.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${A.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${F};background:${_};overflow:hidden;cursor:pointer;${B}">
        ...
        </div>`,(h=f.querySelector("#pp-close"))==null||h.addEventListener("click",()=>f.remove()),f.querySelectorAll(".pp-item").forEach(A=>{A.addEventListener("click",()=>{const _=A.dataset.cid,z=I.find(F=>F.cardId===_),S=m.findIndex(F=>F.cardId===_);z&&(S>-1?m.splice(S,1):m.length<j&&m.push(z),x())})}),($=f.querySelector("#pp-confirm"))==null||$.addEventListener("click",()=>{f.remove(),k(m)})}x(),document.body.appendChild(f)}async function P(I,j){const k=(r["gcCardsFull_"+n]||[]).find(b=>b.id===I),f=(k==null?void 0:k._gcDef)||(r.gcDefs||[]).find(b=>{var h;return b.name===j||((h=b.name)==null?void 0:h.toLowerCase().trim())===(j==null?void 0:j.toLowerCase().trim())}),m=[...r["usedGc_"+n]||[],I],x={type:j,by:n,seq:(r._gcAnimSeq||0)+1};K.add(x.seq),Ee(j,n,async()=>{if(f!=null&&f.effect_type&&f.effect_type!=="CUSTOM"){const h=se[f.effect_type];if(h){const $={...r};h(f.effect_params||{},$,async A=>{A["usedGc_"+n]=m,A._lastGC=x,A._gcAnimSeq=x.seq,await D(A)});return}}const b={...r};switch(j){case"Remplacement+":b.maxSubs=(b.maxSubs||3)+1,b.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const h=o+"Score";b[h]>0&&(b[h]--,b.log.push({text:"🚫 But annulé",type:"info"}));break}}b["usedGc_"+n]=m,b._lastGC=x,b._gcAnimSeq=x.seq,await D(b)})}function J(I,j){const w="usedCardIds_"+I,k=new Set(r[w]||[]);j.forEach(f=>k.add(f)),r[w]=[...k]}function U(){for(const I of["p1","p2"]){const j=new Set(r["usedCardIds_"+I]||[]),w=r[I+"Team"];if(w)for(const k of["GK","DEF","MIL","ATT"])(w[k]||[]).forEach(f=>{f.used=j.has(f.cardId)})}}function G(){var Ue,Je,Ze,Qe,ot,mt;if(r.phase==="reveal")return ie();if(r.phase==="midfield")return be();if(r.phase==="finished")return De();const I=r[n+"Team"],j=r[o+"Team"];U();const w=r[n+"Score"],k=r[o+"Score"],f=r[n+"Name"],m=r[o+"Name"],x=r.phase===n+"-attack",b=r.phase===n+"-defense",h=Array.isArray(r["selected_"+n])?r["selected_"+n]:[],$=h.map(de=>de.cardId),A=window.innerWidth>=700,_=r[n+"Subs"]||[],z=r["usedSubIds_"+n]||[],S=_.filter(de=>!z.includes(de.cardId)),F=r["gcCardsFull_"+n]||[],B=r["usedGc_"+n]||[],O=F.filter(de=>!B.includes(de.id)),H=r.boostOwner===n&&!r.boostUsed,Y=!["GK","DEF","MIL","ATT"].some(de=>(j[de]||[]).some(_e=>!_e.used)),V=[...I.MIL||[],...I.ATT||[]].filter(de=>!de.used),ee=x&&Y&&V.length===0?[...I.GK||[],...I.DEF||[]].filter(de=>!de.used).map(de=>de.cardId):[];function ne(de,_e,Q){var ut,ht;const pe=de._gcDef,W={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[pe==null?void 0:pe.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",le={purple:"#b06ce0",light_blue:"#00d4ef"}[pe==null?void 0:pe.color]||"#b06ce0",Me=(pe==null?void 0:pe.name)||de.gc_type,ke=(pe==null?void 0:pe.effect)||((ut=Ke[de.gc_type])==null?void 0:ut.desc)||"",tt=pe!=null&&pe.image_url?`/manager-wars/icons/${pe.image_url}`:null,it=((ht=Ke[de.gc_type])==null?void 0:ht.icon)||"⚡",rt=Math.round(Q*.22),st=Math.round(Q*.22),pt=Q-rt-st,$t=Me.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${de.id}" data-gc-type="${de.gc_type}"
        style="box-sizing:border-box;width:${_e}px;height:${Q}px;border-radius:10px;border:2px solid ${le};background:${W};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
        ...
        <div style="height:${pt}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
          ${tt?`<img src="${tt}" style="max-width:${_e-10}px;max-height:${pt-6}px;object-fit:contain">`:`<span style="font-size:${Math.round(pt*.55)}px">${it}</span>`}
        </div>
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${gt(I,r[n+"Formation"],je,$,300,300,ee)}
      </div>
          ...
          ${nt((pe.players||[]).map(W=>({...W,used:!1})),"#ff6b6b",pe.total)}
          ...
          <div style="font-size:8px;color:#00ff88;letter-spacing:2px;margin-bottom:4px;text-transform:uppercase">⚔️ VOUS ATTAQUEZ</div>
          ${nt((pe.players||[]).map(W=>({...W,used:!1})),"#00ff88",pe.total)}
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="width:min(90vw,420px)">${ft(r[o+"Team"],r[o+"Formation"],null,[],300,300)}</div>
        ...
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${O.map((V,ee)=>{const ne=ee<O.length-1?at(V,O[ee+1]):null,fe=!ne||ne==="#ff3333"||ne==="#cc2222",ye=ne==="#00ff88"?"+2":ne==="#FFD700"?"+1":"";return`<div class="duel-card duel-card-${Y}" data-idx="${ee}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease,transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
              ${Ye({...V,note:ze(V,"MIL"),_line:"MIL"},58,78)}
            </div>
            ${ee<O.length-1?`<div class="duel-link duel-link-${Y}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${fe?"rgba(255,255,255,0.12)":ne};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${fe?"none":`0 0 8px ${ne}`}">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        <div id="pin-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
          ...
          <div>${O?Ye({...O,used:!1,boost:0},H,Y):"<div>—</div>"}</div>
        ...
        <div id="pout-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
          ...
          <div>${B?Ye({...B,used:!1,boost:0},H,Y):"<div>—</div>"}</div>
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
    ...
    </div>`,(f=document.getElementById("pvp-home"))==null||f.addEventListener("click",()=>{try{E.removeChannel(q)}catch{}We(e),l("home")})}G()}const Uo=Object.freeze(Object.defineProperty({__proto__:null,renderMatchRandom:wn,resumePvpMatch:Ho},Symbol.toStringTag,{value:"Module"}));async function Vo(e,t,i,d){var c,a;try{const s=(a=(c=t==null?void 0:t.state)==null?void 0:c.profile)==null?void 0:a.id;try{(E.getChannels?E.getChannels():[]).forEach(p=>{const n=p.topic||"";(n.includes("matchmaking")||n.includes("pvp-match"))&&E.removeChannel(p)})}catch(l){console.warn("[FriendMatch] cleanup canaux:",l)}s&&(await E.rpc("cancel_matchmaking",{p_user_id:s}).catch(()=>{}),await E.from("matchmaking_queue").delete().eq("user_id",s).then(()=>{},()=>{}))}catch{}await oi(e,t,"random",({deckId:s,formation:l,starters:p,subsRaw:n,gcCardsEnriched:o,gcDefs:g,stadiumDef:v})=>{const y=T=>Ko(e,t,s,l,p,n,T,g||[],i,d);if(!o.length){y([]);return}ni(e,o,y)})}async function Ko(e,t,i,d,c,a,s=[],l=[],p,n){var se;const{state:o,navigate:g,toast:v}=t,y=o.profile.id;let T=!1,r=null;et(e);try{await E.rpc("cancel_matchmaking",{p_user_id:y})}catch{}try{await E.from("matchmaking_queue").delete().eq("user_id",y)}catch{}const u=(oe,P,J,U)=>{var G;e.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(b),(z=b.querySelector("#pvp-end-home"))==null||z.addEventListener("click",()=>{b.remove(),We(e),l("home")})}const X=E.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},w=>{const k=w.new;try{if(k.status==="finished"||k.forfeit){if(u)return;u=!0,N&&(clearInterval(N),N=null),k.game_state&&(r=k.game_state),D(k);return}if(k.game_state){const f=r;r=k.game_state;const m=r._lastGC;if(m&&m.seq&&!q.has(m.seq)&&(q.add(m.seq),m.by!==n)){$e(m.type,m.by,()=>ue());return}const x=f[n+"Score"]||0,b=f[o+"Score"]||0,h=r[n+"Score"]||0,$=r[o+"Score"]||0,A=h>x,_=$>b;if((A||_)&&!C.has(r.round)){C.add(r.round);const z=[...r.log||[]].reverse().find(F=>F.isGoal),S=((z==null?void 0:z.homePlayers)||[]).map(F=>({name:F.name,note:F.note,portrait:F.portrait,job:F.job}));he(S,h,$,A,()=>ue());return}ue()}}catch(f){console.error("[PvP] crash:",f)}}).subscribe();async function se(w){Object.assign(r,w),r.lastActionAt=new Date().toISOString();const{error:k}=await E.from("matches").update({game_state:r}).eq("id",i);k&&p("Erreur de synchronisation","error");try{ue()}catch(f){console.error("[PvP] renderPvpScreen crash:",f)}}async function oe(){if(u)return;u=!0,N&&(clearInterval(N),N=null);const w=d?y.away_id:y.home_id,k=d?"p2":"p1",f=d?"p1":"p2",m={...r,[k+"Score"]:3,[f+"Score"]:0,phase:"finished"};try{await E.from("matches").update({status:"finished",forfeit:!0,winner_id:w,home_score:m.p1Score||0,away_score:m.p2Score||0,game_state:m}).eq("id",i)}catch{}try{E.removeChannel(X)}catch{}setTimeout(()=>{We(e),l("home")},800)}const P={BOOST_STAT:({value:w=1,count:k=1,roles:f=[]},m,x)=>{const b=m[n+"Team"],h=Object.entries(b).filter(([$])=>!f.length||f.includes($)).flatMap(([$,A])=>A.filter(_=>!_.used).map(_=>({..._,_line:$})));if(!h.length){m.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),x(m);return}J(h,k,`Choisir ${k} joueur(s) à booster (+${w})`,$=>{$.forEach(A=>{const _=(b[A._line]||[]).find(z=>z.cardId===A.cardId);_&&(_.boost=(_.boost||0)+w,m.log.push({text:`⚡ +${w} sur ${_.name}`,type:"info"}))}),m[n+"Team"]=b,x(m)})},DEBUFF_STAT:({value:w=1,count:k=1,roles:f=[],target:m="ai"},x,b)=>{const h=m==="home"?n:o,$=x[h+"Team"],A=m==="home"?"allié":"adverse",_=Object.entries($).filter(([z])=>!f.length||f.includes(z)).flatMap(([z,S])=>S.map(F=>({...F,_line:z})));if(!_.length){x.log.push({text:`🎯 Aucun joueur ${A}`,type:"info"}),b(x);return}J(_,k,`Choisir ${k} joueur(s) ${A}(s) (-${w})`,z=>{z.forEach(S=>{const F=($[S._line]||[]).find(B=>B.cardId===S.cardId);F&&(F.boost=(F.boost||0)-w,x.log.push({text:`🎯 -${w} sur ${F.name}`,type:"info"}))}),x[h+"Team"]=$,b(x)})},GRAY_PLAYER:({count:w=1,roles:k=[],target:f="ai"},m,x)=>{const b=f==="home"?n:o,h=m[b+"Team"],$=f==="home"?"allié":"adverse",A=Object.entries(h).filter(([_])=>!k.length||k.includes(_)).flatMap(([_,z])=>z.filter(S=>!S.used).map(S=>({...S,_line:_})));if(!A.length){m.log.push({text:`❌ Aucun joueur ${$}`,type:"info"}),x(m);return}J(A,w,`Choisir ${w} joueur(s) ${$}(s) à exclure`,_=>{const z="usedCardIds_"+b,S=new Set(m[z]||[]);_.forEach(F=>{const B=(h[F._line]||[]).find(O=>O.cardId===F.cardId);B&&(B.used=!0,S.add(F.cardId),m.log.push({text:`❌ ${B.name} exclu !`,type:"info"}))}),m[z]=[...S],m[b+"Team"]=h,x(m)})},REVIVE_PLAYER:({count:w=1,roles:k=[]},f,m)=>{const x=f[n+"Team"],b=Object.entries(x).filter(([h])=>!k.length||k.includes(h)).flatMap(([h,$])=>$.filter(A=>A.used).map(A=>({...A,_line:h})));if(!b.length){f.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),m(f);return}J(b,w,`Choisir ${w} joueur(s) à ressusciter`,h=>{h.forEach($=>{const A=(x[$._line]||[]).find(_=>_.cardId===$.cardId);A&&(A.used=!1,f.log.push({text:`💫 ${A.name} ressuscité !`,type:"info"}))}),f[n+"Team"]=x,m(f)})},REMOVE_GOAL:({},w,k)=>{const f=o+"Score";w[f]>0?(w[f]--,w.log.push({text:"🚫 Dernier but annulé !",type:"info"})):w.log.push({text:"🚫 Aucun but à annuler",type:"info"}),k(w)},ADD_GOAL_DRAW:({},w,k)=>{w[n+"Score"]===w[o+"Score"]?(w[n+"Score"]++,w.log.push({text:"🎯 But bonus !",type:"info"})):w.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),k(w)},ADD_SUB:({value:w=1},k,f)=>{k.maxSubs=(k.maxSubs||3)+w,k.log.push({text:`🔄 +${w} remplacement(s)`,type:"info"}),f(k)},CUSTOM:({},w,k)=>k(w)};function J(w,k,f,m){const x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let b=[];function h(){var A,_;const $=w.map(z=>{const S={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[z._line]||"#555",F=ze(z,z._line)+(z.boost||0),O=b.find(Y=>Y.cardId===z.cardId)?"#FFD700":"rgba(255,255,255,0.25)",H=z.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${z.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${O};background:${S};overflow:hidden;cursor:pointer;${H}">
        ...
        </div>`,(A=x.querySelector("#pp-close"))==null||A.addEventListener("click",()=>x.remove()),x.querySelectorAll(".pp-item").forEach(z=>{z.addEventListener("click",()=>{const S=z.dataset.cid,F=w.find(O=>O.cardId===S),B=b.findIndex(O=>O.cardId===S);F&&(B>-1?b.splice(B,1):b.length<k&&b.push(F),h())})}),(_=x.querySelector("#pp-confirm"))==null||_.addEventListener("click",()=>{x.remove(),m(b)})}h(),document.body.appendChild(x)}async function U(w,k){const m=(r["gcCardsFull_"+n]||[]).find($=>$.id===w),x=(m==null?void 0:m._gcDef)||(r.gcDefs||[]).find($=>{var A;return $.name===k||((A=$.name)==null?void 0:A.toLowerCase().trim())===(k==null?void 0:k.toLowerCase().trim())}),b=[...r["usedGc_"+n]||[],w],h={type:k,by:n,seq:(r._gcAnimSeq||0)+1};q.add(h.seq),$e(k,n,async()=>{if(x!=null&&x.effect_type&&x.effect_type!=="CUSTOM"){const A=P[x.effect_type];if(A){const _={...r};A(x.effect_params||{},_,async z=>{z["usedGc_"+n]=b,z._lastGC=h,z._gcAnimSeq=h.seq,await se(z)});return}}const $={...r};switch(k){case"Remplacement+":$.maxSubs=($.maxSubs||3)+1,$.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const A=o+"Score";$[A]>0&&($[A]--,$.log.push({text:"🚫 But annulé",type:"info"}));break}}$["usedGc_"+n]=b,$._lastGC=h,$._gcAnimSeq=h.seq,await se($)})}function G(w,k){const f="usedCardIds_"+w,m=new Set(r[f]||[]);k.forEach(x=>m.add(x)),r[f]=[...m]}function ie(){for(const w of["p1","p2"]){const k=new Set(r["usedCardIds_"+w]||[]),f=r[w+"Team"];if(f)for(const m of["GK","DEF","MIL","ATT"])(f[m]||[]).forEach(x=>{x.used=k.has(x.cardId)})}}function ue(){var Ze,Qe,ot,mt,de,_e;if(r.phase==="reveal")return be();if(r.phase==="midfield")return Ee();if(r.phase==="finished")return j();const w=r[n+"Team"],k=r[o+"Team"];ie();const f=r[n+"Score"],m=r[o+"Score"],x=r[n+"Name"],b=r[o+"Name"],h=r.phase===n+"-attack",$=r.phase===n+"-defense",A=Array.isArray(r["selected_"+n])?r["selected_"+n]:[],_=A.map(Q=>Q.cardId),z=window.innerWidth>=700,S=r[n+"Subs"]||[],F=r["usedSubIds_"+n]||[],B=S.filter(Q=>!F.includes(Q.cardId)),O=r["gcCardsFull_"+n]||[],H=r["usedGc_"+n]||[],Y=O.filter(Q=>!H.includes(Q.id)),V=r.boostOwner===n&&!r.boostUsed,ee=!["GK","DEF","MIL","ATT"].some(Q=>(k[Q]||[]).some(pe=>!pe.used)),ne=[...w.MIL||[],...w.ATT||[]].filter(Q=>!Q.used),fe=h&&ee&&ne.length===0?[...w.GK||[],...w.DEF||[]].filter(Q=>!Q.used).map(Q=>Q.cardId):[];function ye(Q,pe,W){var kt,Ut;const le=Q._gcDef,Me={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[le==null?void 0:le.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",ke={purple:"#b06ce0",light_blue:"#00d4ef"}[le==null?void 0:le.color]||"#b06ce0",tt=(le==null?void 0:le.name)||Q.gc_type,it=(le==null?void 0:le.effect)||((kt=Ke[Q.gc_type])==null?void 0:kt.desc)||"",rt=le!=null&&le.image_url?`/manager-wars/icons/${le.image_url}`:null,st=((Ut=Ke[Q.gc_type])==null?void 0:Ut.icon)||"⚡",pt=Math.round(W*.22),$t=Math.round(W*.22),ut=W-pt-$t,ht=tt.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${Q.id}" data-gc-type="${Q.gc_type}"
        style="box-sizing:border-box;width:${pe}px;height:${W}px;border-radius:10px;border:2px solid ${ke};background:${Me};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
        ...
        <div style="height:${ut}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
          ${rt?`<img src="${rt}" style="max-width:${pe-10}px;max-height:${ut-6}px;object-fit:contain">`:`<span style="font-size:${Math.round(ut*.55)}px">${st}</span>`}
        </div>
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${gt(w,r[n+"Formation"],Fe,_,300,300,fe)}
      </div>
        ...
        </div>`}if(h&&((W=r.pendingAttack)!=null&&W.players)){const le=r.pendingAttack;return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #00ff88">
          <div style="font-size:8px;color:#00ff88;letter-spacing:2px;margin-bottom:4px;text-transform:uppercase">⚔️ VOUS ATTAQUEZ</div>
      ...
      </div>`;function Je(){const Q=e.querySelector(".match-screen");if(!Q)return;const pe=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);Q.style.height=pe+"px",Q.style.minHeight=pe+"px",Q.style.maxHeight=pe+"px",Q.style.overflow="hidden";const W=e.querySelector("#mobile-action-bar"),le=e.querySelector("#mobile-play-area");W&&le&&(le.style.paddingBottom=W.offsetHeight+"px")}if(Je(),setTimeout(Je,120),setTimeout(Je,400),K||(K=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",Je),window.visualViewport.addEventListener("scroll",Je)),window.addEventListener("resize",Je)),function(){const pe=e.querySelector(".terrain-wrapper svg");pe&&(pe.removeAttribute("width"),pe.removeAttribute("height"),pe.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",pe.setAttribute("viewBox","-26 -26 352 352"),pe.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),e.querySelectorAll(".match-slot-hit").forEach(Q=>{Q.addEventListener("click",()=>{if(!h&&!$)return;const pe=Q.dataset.cardId,W=Q.dataset.role,le=(w[W]||[]).find(it=>it.cardId===pe);if(!le||le.used)return;const Me=fe.includes(pe);if(h&&!["MIL","ATT"].includes(W)&&!Me)return;Array.isArray(r["selected_"+n])||(r["selected_"+n]=[]);const ke=r["selected_"+n],tt=ke.findIndex(it=>it.cardId===pe);tt>-1?ke.splice(tt,1):ke.length<3&&ke.push({...le,_role:W}),ue()})}),e.querySelectorAll(".match-used-hit").forEach(Q=>{Q.addEventListener("click",()=>ae(Q.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(Q=>{Q.addEventListener("click",()=>ae())}),(Ze=e.querySelector("#pvp-sub-open"))==null||Ze.addEventListener("click",()=>ae()),e.querySelectorAll(".pvp-gc-mini").forEach(Q=>{Q.addEventListener("click",()=>Ae(Q.dataset.gcId,Q.dataset.gcType))}),(Qe=e.querySelector("#pvp-boost-card"))==null||Qe.addEventListener("click",()=>te()),(ot=e.querySelector("#pvp-action"))==null||ot.addEventListener("click",Q=>{h?Q.currentTarget.dataset.pass==="1"||!Be(n)?me():xe():$&&re()}),(mt=e.querySelector("#pvp-quit"))==null||mt.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&oe()}),(de=e.querySelector("#pvp-view-opp"))==null||de.addEventListener("click",()=>Z()),(_e=e.querySelector("#pvp-toggle-history"))==null||_e.addEventListener("click",()=>ge()),N&&(clearInterval(N),N=null),(h||$)&&!u){let Q=30,pe=!1;const W=()=>document.getElementById("pvp-timer"),le=()=>{W()&&(W().textContent=Q+"s",W().style.color=pe?"#ff4444":"#fff")};le(),N=setInterval(()=>{Q--,Q<0?pe?(clearInterval(N),N=null,h&&!Be(n)?me():oe()):(pe=!0,Q=15,le()):le()},1e3)}}function be(){et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
      <div style="font-size:20px;font-weight:900;color:#ff6b6b">${r[o+"Name"]||"Adversaire"}</div>
      <div style="width:min(90vw,420px)">${ft(r[o+"Team"],r[o+"Formation"],null,[],300,300)}</div>
    </div>`,n==="p1"&&setTimeout(async()=>{await se({phase:"midfield"})},5e3)}let Te=!1;function Ee(){if(Te)return;const w=r[n+"Team"].MIL||[],k=r[o+"Team"].MIL||[];function f(Y){return Y.reduce((V,ee)=>V+ze(ee,"MIL"),0)}function m(Y){let V=0;for(let ee=0;ee<Y.length-1;ee++){const ne=at(Y[ee],Y[ee+1]);ne==="#00ff88"?V+=2:ne==="#FFD700"&&(V+=1)}return V}const x=f(w)+m(w),b=f(k)+m(k),h=x>=b;function $(Y,V,ee){return`<div id="duel-row-${ee}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
        ...
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${Y.map((ne,fe)=>{const ye=fe<Y.length-1?at(ne,Y[fe+1]):null,ve=!ye||ye==="#ff3333"||ye==="#cc2222",we=ye==="#00ff88"?"+2":ye==="#FFD700"?"+1":"";return`<div class="duel-card duel-card-${ee}" data-idx="${fe}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease,transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
              ${Ye({...ne,note:ze(ne,"MIL"),_line:"MIL"},58,78)}
            </div>
            ${fe<Y.length-1?`<div class="duel-link duel-link-${ee}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${ve?"rgba(255,255,255,0.12)":ye};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${ve?"none":`0 0 8px ${ye}`}">
        ...
        <div class="duel-score-line duel-score-line-${ee}" style="margin-top:10px;font-size:12px;color:rgba(255,255,255,0.55);opacity:0;transition:opacity .4s ease">
          Score: ${f(Y)} + ${m(Y)} liens = <b style="color:#fff">${f(Y)+m(Y)}</b>
        </div>
      </div>`}et(e),e.innerHTML=`
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      <style>
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(b);let A=!1;const _=()=>{A||(A=!0,b.remove(),setTimeout(()=>x(),50))};b.addEventListener("click",_),setTimeout(_,3500)}function $e(w,k,f){var H,Y;const m=(r.gcDefs||[]).find(V=>{var ee;return V.name===w||((ee=V.name)==null?void 0:ee.toLowerCase().trim())===(w==null?void 0:w.toLowerCase().trim())}),x={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[m==null?void 0:m.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",b={purple:"#b06ce0",light_blue:"#00d4ef"}[m==null?void 0:m.color]||"#b06ce0",h=(m==null?void 0:m.name)||w,$=(m==null?void 0:m.effect)||((H=Ke[w])==null?void 0:H.desc)||"",A=m!=null&&m.image_url?`/manager-wars/icons/${m.image_url}`:null,_=((Y=Ke[w])==null?void 0:Y.icon)||"⚡",S=k===n?"Vous":r[k+"Name"]||"Adversaire",F=document.createElement("div");F.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",F.innerHTML=`
      <style>
        @keyframes gcFlipIn{0%{transform:perspective(800px) rotateY(90deg) scale(.7);opacity:0}55%{transform:perspective(800px) rotateY(-12deg) scale(1.08);opacity:1}100%{transform:perspective(800px) rotateY(0) scale(1);opacity:1}}
      ...
      </div>`,document.body.appendChild(f),(m=f.querySelector("#bp-close"))==null||m.addEventListener("click",()=>f.remove()),f.querySelectorAll(".bp-item").forEach(x=>{x.addEventListener("click",async()=>{const b=x.dataset.cid,h=k.find(A=>A.cardId===b);if(!h)return;const $=(w[h._line]||[]).find(A=>A.cardId===b);$&&($.boost=($.boost||0)+r.boostValue),f.remove(),await se({[n+"Team"]:w,boostUsed:!0})})})}function ae(w=null){var B,O;if(!(r.phase===n+"-attack")){p("Remplacement uniquement avant votre attaque","warning");return}const f=r[n+"Team"],m=r["usedSubIds_"+n]||[],x=r.maxSubs||3;if(m.length>=x){p(`Maximum ${x} remplacements atteint`,"warning");return}const b=Object.entries(f).flatMap(([H,Y])=>(Y||[]).filter(V=>V.used).map(V=>({...V,_line:H}))),h=(r[n+"Subs"]||[]).filter(H=>!m.includes(H.cardId));if(!b.length){p("Aucun joueur utilisé à remplacer","warning");return}if(!h.length){p("Aucun remplaçant disponible","warning");return}let $=Math.max(0,b.findIndex(H=>H.cardId===w));const A=((B=b[$])==null?void 0:B._line)||((O=b[$])==null?void 0:O.job);let _=Math.max(0,h.findIndex(H=>H.job===A)),z=!1;const S=document.createElement("div");S.id="pvp-sub-overlay",S.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function F(){var ye,ve,we,Ie,Le,Re;const H=b[$],Y=h[_],V=Math.min(130,Math.round((window.innerWidth-90)/2)),ee=Math.round(V*1.35),ne=je=>`background:rgba(255,255,255,0.12);border:none;color:${je?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${je?"default":"pointer"};flex-shrink:0`;S.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        <div id="pin-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
          ...
          <div>${Y?Ye({...Y,used:!1,boost:0},V,ee):"<div>—</div>"}</div>
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      ...
      <button id="pvp-home" style="padding:16px 40px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">🏠 Retour</button>
    </div>`,(x=document.getElementById("pvp-home"))==null||x.addEventListener("click",()=>{try{E.removeChannel(X)}catch{}We(e),l("home")})}ue()}const zt="#1A6B3C",yt="#D4A017";async function Wo(e,t){var d;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const i=(d=t.state.params)==null?void 0:d.openLeagueId;if(i){t.state.params.openLeagueId=null,await bt(e,t,i);return}await _t(e,t)}async function _t(e,t,i="waiting"){var M;const{state:d}=t,c=d.profile.id,{data:a}=await E.from("mini_league_members").select("league_id").eq("user_id",c),s=(a||[]).map(L=>L.league_id),{data:l,error:p}=await E.from("mini_leagues").select("*, mini_league_members(count)").eq("status","waiting").eq("is_archived",!1).order("created_at",{ascending:!1}).limit(30),n=p?(await E.from("mini_leagues").select("*, mini_league_members(count)").eq("status","waiting").order("created_at",{ascending:!1}).limit(30)).data||[]:l||[],g=(s.length?await E.from("mini_leagues").select("*, mini_league_members(count)").in("id",s).order("created_at",{ascending:!1}):{data:[]}).data||[],v=g.filter(L=>L.status==="waiting"&&!L.is_archived),y=g.filter(L=>L.status==="active"&&!L.is_archived),T=g.filter(L=>L.is_archived||L.status==="finished"),r=n.filter(L=>!s.includes(L.id)),u=[{key:"waiting",label:"🟡 En attente",count:v.length+r.length},{key:"active",label:"🟢 En cours",count:y.length},{key:"archived",label:"📁 Archivées",count:T.length}];e.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
  ...
  </div>`,(M=document.getElementById("ml-create-btn"))==null||M.addEventListener("click",()=>Qo(e,t)),e.querySelectorAll(".ml-tab").forEach(L=>L.addEventListener("click",()=>_t(e,t,L.dataset.tab))),e.querySelectorAll("[data-league-id]").forEach(L=>L.addEventListener("click",()=>bt(e,t,L.dataset.leagueId))),e.querySelectorAll("[data-join]").forEach(L=>L.addEventListener("click",N=>{N.stopPropagation(),$n(e,t,L.dataset.join,L.dataset.type)})),e.querySelectorAll("[data-delete]").forEach(L=>L.addEventListener("click",N=>{N.stopPropagation(),mi(e,t,L.dataset.delete,L.dataset.name,i)}))}function ei(e,t,i=!1){var s,l;const d=e.creator_id===t,c=e.pot||0,a=((l=(s=e.mini_league_members)==null?void 0:s[0])==null?void 0:l.count)||0;return`<div data-league-id="${e.id}" style="background:#fff;border-radius:12px;padding:14px 16px;box-shadow:0 1px 6px rgba(0,0,0,0.08);cursor:pointer;margin-bottom:8px">
    <div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:6px">
      <div style="font-size:15px;font-weight:900;flex:1;margin-right:8px">${e.name}</div>
      ${d?`<button data-delete="${e.id}" data-name="${e.name}" style="background:none;border:none;font-size:16px;cursor:pointer;color:#cc2222;flex-shrink:0;padding:0">🗑️</button>`:""}
    </div>
    <div style="display:flex;flex-wrap:wrap;gap:8px;font-size:11px;color:#666;margin-bottom:${i?"10px":"0"}">
      ...
      ${c>0?`<span style="color:${yt};font-weight:700">🏆 ${c.toLocaleString("fr")} cr.</span>`:""}
  ...
  </div>`}function Jo(e,t,i){const d=[];return e.length&&(d.push('<div style="font-size:11px;font-weight:700;color:#555;text-transform:uppercase;letter-spacing:1px">Mes leagues en attente</div>'),d.push(...e.map(c=>ei(c,i,!1)))),t.length&&(d.push('<div style="font-size:11px;font-weight:700;color:#555;text-transform:uppercase;letter-spacing:1px;margin-top:4px">Rejoindre</div>'),d.push(...t.map(c=>ei(c,i,!0)))),d.length?d.join(""):'<div style="text-align:center;padding:40px;color:#aaa">🏆<br>Aucune mini league.<br>Crée la première !</div>'}function Xo(e,t){return e.length?e.map(i=>ei(i,t)).join(""):'<div style="text-align:center;padding:40px;color:#aaa">Aucune mini league en cours.</div>'}function Zo(e,t){return e.length?e.map(i=>ei(i,t)).join(""):'<div style="text-align:center;padding:40px;color:#aaa">Aucune mini league archivée.</div>'}function Qo(e,t){const i=document.createElement("div");i.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9000;display:flex;align-items:center;justify-content:center;padding:16px",i.innerHTML=`<div style="background:#fff;border-radius:16px;width:100%;max-width:400px;max-height:90vh;overflow-y:auto">
    ...
    <div style="padding:20px;display:flex;flex-direction:column;gap:14px">
    ...
    </div></div>`,document.body.appendChild(i),i.querySelector("#ml-form-close").addEventListener("click",()=>i.remove()),i.addEventListener("click",d=>{d.target===i&&i.remove()}),i.querySelectorAll('input[name="ml-type"]').forEach(d=>d.addEventListener("change",()=>{document.getElementById("ml-pwd-block").style.display=d.value==="private"?"block":"none"})),i.querySelector("#ml-create-confirm").addEventListener("click",async()=>{var T,r,u,M;const{toast:d}=t,c=document.getElementById("ml-name").value.trim(),a=parseInt(document.getElementById("ml-max").value)||6,s=parseInt(document.getElementById("ml-fee").value)||500,l=((T=i.querySelector('input[name="ml-type"]:checked'))==null?void 0:T.value)||"public",p=((r=i.querySelector('input[name="ml-mode"]:checked'))==null?void 0:r.value)||"aller",n=((M=(u=document.getElementById("ml-pwd"))==null?void 0:u.value)==null?void 0:M.trim())||null;if(!c){d("Le nom est obligatoire","error");return}if(s<100){d("Mise minimum : 100 crédits","error");return}if(l==="private"&&!n){d("Mot de passe requis","error");return}const{data:o,error:g}=await E.from("mini_leagues").insert({name:c,creator_id:t.state.profile.id,type:l,password:n,mode:p,max_players:a,entry_fee:s}).select().single();if(g){d("Erreur : "+g.message,"error");return}const{data:v}=await E.from("users").select("credits").eq("id",t.state.profile.id).single();if(((v==null?void 0:v.credits)||0)<s){await E.from("mini_leagues").delete().eq("id",o.id),d(`Crédits insuffisants pour la mise (${s.toLocaleString("fr")} cr.)`,"error");return}await E.from("users").update({credits:v.credits-s}).eq("id",t.state.profile.id),await E.from("mini_leagues").update({pot:s}).eq("id",o.id),await E.from("mini_league_members").insert({league_id:o.id,user_id:t.state.profile.id}),t.state.profile&&(t.state.profile.credits=v.credits-s);const y=document.getElementById("nav-credits");y&&(y.textContent=`💰 ${(t.state.profile.credits||0).toLocaleString("fr")}`),i.remove(),d(`Mini League créée ! ${s.toLocaleString("fr")} cr. déduits.`,"success"),bt(e,t,o.id)})}function er(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9500;display:flex;align-items:center;justify-content:center;padding:20px",t.innerHTML=`
      ...
      </div>`,document.body.appendChild(t);const i=t.querySelector("#ml-pwd-input"),d=c=>{t.remove(),e(c)};setTimeout(()=>i==null?void 0:i.focus(),80),t.querySelector("#pwd-cancel").addEventListener("click",()=>d(null)),t.querySelector("#pwd-ok").addEventListener("click",()=>d(i.value.trim())),i.addEventListener("keydown",c=>{c.key==="Enter"&&d(i.value.trim())}),t.addEventListener("click",c=>{c.target===t&&d(null)})})}async function $n(e,t,i,d){const{toast:c,state:a}=t,s=a.profile.id,{data:l}=await E.from("mini_leagues").select("password,status,entry_fee,pot,max_players").eq("id",i).single();if(!l){c("Mini League introuvable","error");return}if(l.status!=="waiting"){c("Cette Mini League a déjà démarré","warning");return}const{count:p}=await E.from("mini_league_members").select("id",{count:"exact",head:!0}).eq("league_id",i);if(p>=l.max_players){c("La Mini League est complète","warning");return}if(d==="private"){const r=await er();if(r===null)return;if(l.password!==r){c("Mot de passe incorrect","error");return}}const n=l.entry_fee||100,{data:o}=await E.from("users").select("credits").eq("id",s).single();if(((o==null?void 0:o.credits)||0)<n){c(`Crédits insuffisants — il te faut ${n.toLocaleString("fr")} cr. (solde : ${((o==null?void 0:o.credits)||0).toLocaleString("fr")} cr.)`,"error");return}const{error:g}=await E.from("mini_league_members").insert({league_id:i,user_id:s});if(g){c("Erreur inscription : "+g.message,"error");return}const{error:v}=await E.from("users").update({credits:o.credits-n}).eq("id",s),{error:y}=await E.from("mini_leagues").update({pot:(l.pot||0)+n}).eq("id",i);(v||y)&&console.error("Erreur post-inscription:",v,y),a.profile&&(a.profile.credits=o.credits-n);const T=document.getElementById("nav-credits");T&&(T.textContent=`💰 ${(a.profile.credits||0).toLocaleString("fr")}`),c(`✅ Inscrit ! −${n.toLocaleString("fr")} cr. · Pot : ${((l.pot||0)+n).toLocaleString("fr")} cr.`,"success"),bt(e,t,i)}async function tr(e,t,i,d){const{toast:c,state:a}=t,s=a.profile.id;if(!confirm(`Te désinscrire et récupérer ${d.toLocaleString("fr")} cr. ?`))return;const{data:l}=await E.from("mini_leagues").select("pot,status").eq("id",i).single();if(!l||l.status!=="waiting"){c("Impossible — la league a déjà démarré","error");return}const{data:p}=await E.from("users").select("credits").eq("id",s).single();await E.from("mini_leagues").update({pot:Math.max(0,(l.pot||0)-d)}).eq("id",i),await E.from("users").update({credits:((p==null?void 0:p.credits)||0)+d}).eq("id",s),await E.from("mini_league_members").delete().eq("league_id",i).eq("user_id",s),a.profile&&(a.profile.credits=((p==null?void 0:p.credits)||0)+d);const n=document.getElementById("nav-credits");n&&(n.textContent=`💰 ${(a.profile.credits||0).toLocaleString("fr")}`),c(`↩️ Désinscrit · +${d.toLocaleString("fr")} cr. remboursés`,"success"),_t(e,t,"waiting")}async function mi(e,t,i,d,c){const{toast:a}=t;if(!confirm(`Supprimer définitivement "${d}" et tous ses matchs/membres ? Action irréversible.`))return;await E.from("mini_league_matches").delete().eq("league_id",i),await E.from("mini_league_members").delete().eq("league_id",i);const{error:s}=await E.from("mini_leagues").delete().eq("id",i);if(s){a("Erreur suppression ("+s.message+")","error");return}a("Mini League supprimée avec succès","success"),_t(e,t,c)}async function ir(e,t,i){await E.from("mini_leagues").update({is_archived:!0}).eq("id",i),t.toast("Mini League archivée","success"),_t(e,t,"archived")}async function bt(e,t,i){var K,C,q,D,X,se,oe,P,J;const{state:d,toast:c}=t,a=d.profile.id,[{data:s},{data:l},{data:p}]=await Promise.all([E.from("mini_leagues").select("*").eq("id",i).single(),E.from("mini_league_members").select("*, user:users(id,pseudo,club_name,club_color1,club_color2)").eq("league_id",i),E.from("mini_league_matches").select("*").eq("league_id",i).order("matchday").order("created_at")]);if(!s){c("Introuvable","error"),await _t(e,t);return}const n=(l||[]).some(U=>U.user_id===a),o=s.creator_id===a,g=(l||[]).map(U=>U.user).filter(Boolean),v=En(g,p||[]),y=(p||[]).filter(U=>U.matchday===s.current_day),T=s.pot||0,r=s.entry_fee||100,u=y.length>0&&y.every(U=>U.status==="finished"||U.status==="bye"),M=s.current_day>=s.total_days,L=(l||[]).find(U=>U.user_id===a);e.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    <div style="padding:12px 16px;background:#fff;border-bottom:1px solid #eee;display:flex;align-items:center;gap:10px">
      ...
      <div style="text-align:right;flex-shrink:0">
        ...
        ${T>0?`<div style="font-size:12px;font-weight:900;color:${yt};margin-top:4px">🏆 ${T.toLocaleString("fr")} cr.</div>`:""}
      </div>
    </div>
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          <thead><tr style="font-size:10px;color:#888;text-transform:uppercase;border-bottom:2px solid #f0f0f0">
            ...
            ${T>0&&s.status==="finished"?`<th style="text-align:right;padding:5px 0;color:${yt}">💰</th>`:""}
          </tr></thead>
          <tbody>${v.map((U,G)=>{const ie=T>0&&s.status==="finished"?G===0?Math.floor(T*.7):G===1?Math.floor(T*.2):G===2?Math.floor(T*.1):0:0;return`<tr style="border-bottom:1px solid #f5f5f5;${U.userId===a?"background:#f0fdf4;":""}">
              <td style="padding:7px 3px 7px 0;font-weight:700;color:${G===0?yt:G<3?zt:"#555"}">${["🥇","🥈","🥉"][G]||G+1}</td>
              ...
              ${T>0&&s.status==="finished"?`<td style="text-align:right;font-weight:700;color:${yt}">${ie?ie.toLocaleString("fr")+" cr.":"—"}</td>`:""}
            </tr>`}).join("")}</tbody>
        </table>
      </div>`:""}
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        ${Array.from({length:Math.max(0,s.status==="active"?s.current_day-1:s.current_day)},(U,G)=>G+1).reverse().map(U=>{const G=(p||[]).filter(ie=>ie.matchday===U);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:#888;margin-bottom:6px">Journée ${U}</div>${G.map(ie=>Ri(ie,g,a,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
  ...
  </div>`;const N=s.status==="waiting"?"waiting":s.status==="active"?"active":"archived";if((K=document.getElementById("ml-back"))==null||K.addEventListener("click",()=>_t(e,t,N)),(C=document.getElementById("ml-start-btn"))==null||C.addEventListener("click",()=>nr(e,t,s,g)),(q=document.getElementById("ml-next-day"))==null||q.addEventListener("click",()=>or(e,t,i)),(D=document.getElementById("ml-finish-btn"))==null||D.addEventListener("click",()=>kn(e,t,i,T,v,g)),(X=document.getElementById("ml-join-now"))==null||X.addEventListener("click",()=>$n(e,t,i,s.type)),(se=document.getElementById("ml-leave-btn"))==null||se.addEventListener("click",()=>tr(e,t,i,r)),(oe=document.getElementById("ml-delete-btn"))==null||oe.addEventListener("click",()=>mi(e,t,i,s.name,"waiting")),(P=document.getElementById("ml-delete-now"))==null||P.addEventListener("click",()=>mi(e,t,i,s.name,N)),(J=document.getElementById("ml-archive-btn"))==null||J.addEventListener("click",()=>ir(e,t,i)),e.querySelectorAll("[data-play-match]").forEach(U=>{U.addEventListener("click",()=>{const G=y.find(ie=>ie.id===U.dataset.playMatch);G&&t.navigate("match-mini-league",{mlMatchId:G.id,leagueId:i})})}),s.status==="finished"&&L){const U=v.findIndex(G=>G.userId===a);U>=0&&U<3&&L.prize_amount>0&&setTimeout(()=>rr(e,t,s,L,U),400)}}function Ri(e,t,i,d,c=!1){const a=g=>t.find(v=>v.id===g);if(e.is_bye){const g=a(e.home_id);return`<div style="padding:8px;border-radius:8px;background:#f9f9f9;margin-bottom:6px;font-size:12px;color:#888;text-align:center">🔵 ${(g==null?void 0:g.club_name)||(g==null?void 0:g.pseudo)||"?"} exempté(e)</div>`}const s=a(e.home_id),l=a(e.away_id),p=e.home_id===i||e.away_id===i,n=p&&e.status==="pending"&&d&&!c,o=e.status==="finished"?`${e.home_score} - ${e.away_score}`:"vs";return`<div style="display:flex;align-items:center;gap:8px;padding:10px;border-radius:8px;background:${p?"#f0fdf4":"#f9f9f9"};margin-bottom:6px;border:1px solid ${p?"#86efac":"#f0f0f0"}">
  ...
  </div>`}async function nr(e,t,i,d){const{toast:c,state:a}=t,s=ar(d.map(n=>n.id),i.mode),l=[];s.forEach((n,o)=>n.forEach(g=>l.push({league_id:i.id,matchday:o+1,home_id:g.home||g.bye,away_id:g.away||null,is_bye:!!g.bye,status:g.bye?"bye":"pending"})));const{error:p}=await E.from("mini_league_matches").insert(l);if(p){c("Erreur calendrier : "+p.message,"error");return}await E.from("mini_leagues").update({status:"active",current_day:1,total_days:s.length}).eq("id",i.id),c(`🚀 Lancée ! Pot : ${(i.pot||0).toLocaleString("fr")} cr.`,"success"),bt(e,t,i.id)}async function or(e,t,i){const{data:d}=await E.from("mini_leagues").select("current_day,total_days,pot").eq("id",i).single(),c=(d.current_day||0)+1;if(c>(d.total_days||0)){await kn(e,t,i,d.pot||0,null,null);return}await E.from("mini_leagues").update({current_day:c}).eq("id",i),t.toast(`Journée ${c} commencée !`,"success"),bt(e,t,i)}async function kn(e,t,i,d,c,a){const{toast:s,state:l}=t;let p=c,n=a;if(!p){const{data:g}=await E.from("mini_league_matches").select("*").eq("league_id",i),{data:v}=await E.from("mini_league_members").select("*, user:users(id,pseudo,club_name)").eq("league_id",i);n=(v||[]).map(y=>y.user).filter(Boolean),p=En(n,g||[])}const o=[Math.floor(d*.7),Math.floor(d*.2),Math.floor(d*.1)];await Promise.all(p.slice(0,3).map((g,v)=>o[v]?E.from("mini_league_members").update({prize_amount:o[v],prize_claimed:!1}).eq("league_id",i).eq("user_id",g.userId):Promise.resolve())),await E.from("mini_leagues").update({status:"finished"}).eq("id",i),s("🏆 Mini League terminée ! Les gagnants peuvent récupérer leurs crédits.","success"),bt(e,t,i)}async function rr(e,t,i,d,c){var v,y;const{state:a,toast:s}=t,l=[Math.floor((i.pot||0)*.7),Math.floor((i.pot||0)*.2),Math.floor((i.pot||0)*.1)],p=["🥇","🥈","🥉"][c],n=d.prize_amount||l[c]||0,o=d.prize_claimed,g=document.createElement("div");g.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",g.innerHTML=`
    <div style="background:linear-gradient(160deg,#1a0a2e,#3b1a6e);border-radius:20px;padding:32px 24px;max-width:320px;width:100%;text-align:center;color:#fff;box-shadow:0 8px 40px rgba(0,0,0,0.6)">
      ...
      <div style="background:rgba(212,160,23,0.2);border:2px solid ${yt};border-radius:14px;padding:16px;margin-bottom:24px">
        ...
        <div style="font-size:32px;font-weight:900;color:${yt}">${n.toLocaleString("fr")} cr.</div>
      ...
      ${o?'<div style="font-size:13px;color:#86efac;margin-bottom:16px">✅ Crédits déjà récupérés</div>':`<button id="claim-prize-btn" style="width:100%;padding:14px;border-radius:12px;border:none;background:${yt};color:#111;font-size:16px;font-weight:900;cursor:pointer;margin-bottom:12px">💰 Récupérer ${n.toLocaleString("fr")} cr.</button>`}
      <button id="prize-close" style="background:rgba(255,255,255,0.1);border:none;color:rgba(255,255,255,0.7);padding:10px 24px;border-radius:10px;font-size:14px;cursor:pointer">Fermer</button>
    </div>`,document.body.appendChild(g),(v=g.querySelector("#prize-close"))==null||v.addEventListener("click",()=>g.remove()),g.addEventListener("click",T=>{T.target===g&&g.remove()}),(y=g.querySelector("#claim-prize-btn"))==null||y.addEventListener("click",async()=>{const{data:T}=await E.from("users").select("credits").eq("id",a.profile.id).single();await E.from("users").update({credits:((T==null?void 0:T.credits)||0)+n}).eq("id",a.profile.id),await E.from("mini_league_members").update({prize_claimed:!0}).eq("league_id",i.id).eq("user_id",a.profile.id),a.profile&&(a.profile.credits=((T==null?void 0:T.credits)||0)+n);const r=document.getElementById("nav-credits");r&&(r.textContent=`💰 ${a.profile.credits.toLocaleString("fr")}`),s(`💰 +${n.toLocaleString("fr")} cr. ajoutés à ton solde !`,"success"),g.remove(),bt(e,t,i.id)})}function ar(e,t){const d=e.length%2===0?[...e]:[...e,null],c=d.length;let a=d.slice(1);const s=[];for(let l=0;l<c-1;l++){const p=[],n=[d[0],...a];for(let o=0;o<c/2;o++){const g=n[o],v=n[c-1-o];g===null?p.push({bye:v}):v===null?p.push({bye:g}):p.push({home:g,away:v})}s.push(p),a=[a[a.length-1],...a.slice(0,-1)]}return t==="aller-retour"?[...s,...s.map(l=>l.map(p=>p.bye?p:{home:p.away,away:p.home}))]:s}function En(e,t){const i={};return e.forEach(d=>{i[d.id]={userId:d.id,pseudo:d.pseudo,clubName:d.club_name||d.pseudo,played:0,won:0,drawn:0,lost:0,goalsFor:0,goalsAgainst:0,goalDiff:0,points:0}}),t.filter(d=>d.status==="finished"&&!d.is_bye&&d.home_score!=null).forEach(d=>{const c=i[d.home_id],a=i[d.away_id];!c||!a||(c.played++,a.played++,c.goalsFor+=d.home_score,c.goalsAgainst+=d.away_score,a.goalsFor+=d.away_score,a.goalsAgainst+=d.home_score,d.home_score>d.away_score?(c.won++,c.points+=3,a.lost++):d.home_score<d.away_score?(a.won++,a.points+=3,c.lost++):(c.drawn++,c.points++,a.drawn++,a.points++),c.goalDiff=c.goalsFor-c.goalsAgainst,a.goalDiff=a.goalsFor-a.goalsAgainst)}),Object.values(i).sort((d,c)=>c.points-d.points||c.goalDiff-d.goalDiff||c.goalsFor-d.goalsFor)}async function Oi(e,t,i,d){var c,a;t.state.params={...t.state.params,matchMode:"mini-league",mlMatchId:i,leagueId:d};try{const s=(a=(c=t==null?void 0:t.state)==null?void 0:c.profile)==null?void 0:a.id;try{(E.getChannels?E.getChannels():[]).forEach(p=>{const n=p.topic||"";(n.includes("matchmaking")||n.includes("pvp-match"))&&E.removeChannel(p)})}catch(l){console.warn("[FriendMatch] cleanup canaux:",l)}s&&(await E.rpc("cancel_matchmaking",{p_user_id:s}).catch(()=>{}),await E.from("matchmaking_queue").delete().eq("user_id",s).then(()=>{},()=>{}))}catch{}await oi(e,t,"random",({deckId:s,formation:l,starters:p,subsRaw:n,gcCardsEnriched:o,gcDefs:g,stadiumDef:v})=>{const y=T=>sr(e,t,s,l,p,n,T,g||[],i);if(!o.length){y([]);return}ni(e,o,y)})}async function sr(e,t,i,d,c,a,s=[],l=[],p,n){const{state:o,navigate:g,toast:v}=t,y=o.profile.id;let T=!1,r=null;et(e);try{await E.rpc("cancel_matchmaking",{p_user_id:y})}catch{}try{await E.from("matchmaking_queue").delete().eq("user_id",y)}catch{}const{data:u}=await E.from("mini_league_matches").select("*, home:users!home_id(id,pseudo,club_name), away:users!away_id(id,pseudo,club_name)").eq("id",p).single();if(!u){v("Match introuvable","error"),We(e),g("mini-league");return}if(u.home_id!==y&&u.away_id!==y){v("Tu ne fais pas partie de ce match","error"),We(e),g("mini-league");return}const M=u.home_id===y,L=M?u.away:u.home,N=M?u.away_id:u.home_id,K=(q,D)=>{var X;e.innerHTML=`
      <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:20px;background:linear-gradient(180deg,#0a1628,#1a0a2e);padding:24px;text-align:center;color:#fff">
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">🏆 Retour à la Mini League</button>`,document.body.appendChild($),(F=$.querySelector("#pvp-end-home"))==null||F.addEventListener("click",()=>{$.remove(),We(e),n?l("mini-league",{openLeagueId:n}):l("mini-league")})}const se=E.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},f=>{const m=f.new;try{if(m.status==="finished"||m.forfeit){if(M||(M=!0,K&&(clearInterval(K),K=null),m.game_state&&(u=m.game_state),u.phase==="finished"&&!m.forfeit&&document.getElementById("pvp-home")))return;X(m);return}if(m.game_state){const x=u;u=m.game_state;const b=u._lastGC;if(b&&b.seq&&!D.has(b.seq)&&(D.add(b.seq),b.by!==o)){Ae(b.type,b.by,()=>be());return}const h=x[o+"Score"]||0,$=x[g+"Score"]||0,A=u[o+"Score"]||0,_=u[g+"Score"]||0,z=A>h,S=_>$;if((z||S)&&!q.has(u.round)){q.add(u.round);const F=[...u.log||[]].reverse().find(O=>O.isGoal),B=((F==null?void 0:F.homePlayers)||[]).map(O=>({name:O.name,note:O.note,portrait:O.portrait,job:O.job}));$e(B,A,_,z,()=>be());return}be()}}catch(x){console.error("[PvP] crash:",x)}}).subscribe();async function oe(f){Object.assign(u,f),u.lastActionAt=new Date().toISOString();const{error:m}=await E.from("matches").update({game_state:u}).eq("id",i);m&&p("Erreur de synchronisation","error");try{be()}catch(x){console.error("[PvP] renderPvpScreen crash:",x)}}async function P(){if(M)return;M=!0,K&&(clearInterval(K),K=null);const f=d?T.away_id:T.home_id,m=d?"p2":"p1",x=d?"p1":"p2",b={...u,[m+"Score"]:3,[x+"Score"]:0,phase:"finished"};try{await E.from("matches").update({status:"finished",forfeit:!0,winner_id:f,home_score:b.p1Score||0,away_score:b.p2Score||0,game_state:b}).eq("id",i)}catch{}try{E.removeChannel(se)}catch{}setTimeout(()=>{We(e),l("mini-league")},800)}const J={BOOST_STAT:({value:f=1,count:m=1,roles:x=[]},b,h)=>{const $=b[o+"Team"],A=Object.entries($).filter(([_])=>!x.length||x.includes(_)).flatMap(([_,z])=>z.filter(S=>!S.used).map(S=>({...S,_line:_})));if(!A.length){b.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),h(b);return}U(A,m,`Choisir ${m} joueur(s) à booster (+${f})`,_=>{_.forEach(z=>{const S=($[z._line]||[]).find(F=>F.cardId===z.cardId);S&&(S.boost=(S.boost||0)+f,b.log.push({text:`⚡ +${f} sur ${S.name}`,type:"info"}))}),b[o+"Team"]=$,h(b)})},DEBUFF_STAT:({value:f=1,count:m=1,roles:x=[],target:b="ai"},h,$)=>{const A=b==="home"?o:g,_=h[A+"Team"],z=b==="home"?"allié":"adverse",S=Object.entries(_).filter(([F])=>!x.length||x.includes(F)).flatMap(([F,B])=>B.map(O=>({...O,_line:F})));if(!S.length){h.log.push({text:`🎯 Aucun joueur ${z}`,type:"info"}),$(h);return}U(S,m,`Choisir ${m} joueur(s) ${z}(s) (-${f})`,F=>{F.forEach(B=>{const O=(_[B._line]||[]).find(H=>H.cardId===B.cardId);O&&(O.boost=(O.boost||0)-f,h.log.push({text:`🎯 -${f} sur ${O.name}`,type:"info"}))}),h[A+"Team"]=_,$(h)})},GRAY_PLAYER:({count:f=1,roles:m=[],target:x="ai"},b,h)=>{const $=x==="home"?o:g,A=b[$+"Team"],_=x==="home"?"allié":"adverse",z=Object.entries(A).filter(([S])=>!m.length||m.includes(S)).flatMap(([S,F])=>F.filter(B=>!B.used).map(B=>({...B,_line:S})));if(!z.length){b.log.push({text:`❌ Aucun joueur ${_}`,type:"info"}),h(b);return}U(z,f,`Choisir ${f} joueur(s) ${_}(s) à exclure`,S=>{const F="usedCardIds_"+$,B=new Set(b[F]||[]);S.forEach(O=>{const H=(A[O._line]||[]).find(Y=>Y.cardId===O.cardId);H&&(H.used=!0,B.add(O.cardId),b.log.push({text:`❌ ${H.name} exclu !`,type:"info"}))}),b[F]=[...B],b[$+"Team"]=A,h(b)})},REVIVE_PLAYER:({count:f=1,roles:m=[]},x,b)=>{const h=x[o+"Team"],$=Object.entries(h).filter(([A])=>!m.length||m.includes(A)).flatMap(([A,_])=>_.filter(z=>z.used).map(z=>({...z,_line:A})));if(!$.length){x.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),b(x);return}U($,f,`Choisir ${f} joueur(s) à ressusciter`,A=>{A.forEach(_=>{const z=(h[_._line]||[]).find(S=>S.cardId===_.cardId);z&&(z.used=!1,x.log.push({text:`💫 ${z.name} ressuscité !`,type:"info"}))}),x[o+"Team"]=h,b(x)})},REMOVE_GOAL:({},f,m)=>{const x=g+"Score";f[x]>0?(f[x]--,f.log.push({text:"🚫 Dernier but annulé !",type:"info"})):f.log.push({text:"🚫 Aucun but à annuler",type:"info"}),m(f)},ADD_GOAL_DRAW:({},f,m)=>{f[o+"Score"]===f[g+"Score"]?(f[o+"Score"]++,f.log.push({text:"🎯 But bonus !",type:"info"})):f.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),m(f)},ADD_SUB:({value:f=1},m,x)=>{m.maxSubs=(m.maxSubs||3)+f,m.log.push({text:`🔄 +${f} remplacement(s)`,type:"info"}),x(m)},CUSTOM:({},f,m)=>m(f)};function U(f,m,x,b){const h=document.createElement("div");h.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let $=[];function A(){var z,S;const _=f.map(F=>{const B={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[F._line]||"#555",O=ze(F,F._line)+(F.boost||0),Y=$.find(ee=>ee.cardId===F.cardId)?"#FFD700":"rgba(255,255,255,0.25)",V=F.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${F.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${Y};background:${B};overflow:hidden;cursor:pointer;${V}">
        ...
        </div>`,(z=h.querySelector("#pp-close"))==null||z.addEventListener("click",()=>h.remove()),h.querySelectorAll(".pp-item").forEach(F=>{F.addEventListener("click",()=>{const B=F.dataset.cid,O=f.find(Y=>Y.cardId===B),H=$.findIndex(Y=>Y.cardId===B);O&&(H>-1?$.splice(H,1):$.length<m&&$.push(O),A())})}),(S=h.querySelector("#pp-confirm"))==null||S.addEventListener("click",()=>{h.remove(),b($)})}A(),document.body.appendChild(h)}async function G(f,m){const b=(u["gcCardsFull_"+o]||[]).find(_=>_.id===f),h=(b==null?void 0:b._gcDef)||(u.gcDefs||[]).find(_=>{var z;return _.name===m||((z=_.name)==null?void 0:z.toLowerCase().trim())===(m==null?void 0:m.toLowerCase().trim())}),$=[...u["usedGc_"+o]||[],f],A={type:m,by:o,seq:(u._gcAnimSeq||0)+1};D.add(A.seq),Ae(m,o,async()=>{if(h!=null&&h.effect_type&&h.effect_type!=="CUSTOM"){const z=J[h.effect_type];if(z){const S={...u};z(h.effect_params||{},S,async F=>{F["usedGc_"+o]=$,F._lastGC=A,F._gcAnimSeq=A.seq,await oe(F)});return}}const _={...u};switch(m){case"Remplacement+":_.maxSubs=(_.maxSubs||3)+1,_.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const z=g+"Score";_[z]>0&&(_[z]--,_.log.push({text:"🚫 But annulé",type:"info"}));break}}_["usedGc_"+o]=$,_._lastGC=A,_._gcAnimSeq=A.seq,await oe(_)})}function ie(f,m){const x="usedCardIds_"+f,b=new Set(u[x]||[]);m.forEach(h=>b.add(h)),u[x]=[...b]}function ue(){for(const f of["p1","p2"]){const m=new Set(u["usedCardIds_"+f]||[]),x=u[f+"Team"];if(x)for(const b of["GK","DEF","MIL","ATT"])(x[b]||[]).forEach(h=>{h.used=m.has(h.cardId)})}}function be(){var ot,mt,de,_e,Q,pe;if(u.phase==="reveal")return Te();if(u.phase==="midfield")return he();if(u.phase==="finished")return w();const f=u[o+"Team"],m=u[g+"Team"];ue();const x=u[o+"Score"],b=u[g+"Score"],h=u[o+"Name"],$=u[g+"Name"],A=u.phase===o+"-attack",_=u.phase===o+"-defense",z=Array.isArray(u["selected_"+o])?u["selected_"+o]:[],S=z.map(W=>W.cardId),F=window.innerWidth>=700,B=u[o+"Subs"]||[],O=u["usedSubIds_"+o]||[],H=B.filter(W=>!O.includes(W.cardId)),Y=u["gcCardsFull_"+o]||[],V=u["usedGc_"+o]||[],ee=Y.filter(W=>!V.includes(W.id)),ne=u.boostOwner===o&&!u.boostUsed,fe=!["GK","DEF","MIL","ATT"].some(W=>(m[W]||[]).some(le=>!le.used)),ye=[...f.MIL||[],...f.ATT||[]].filter(W=>!W.used),ve=A&&fe&&ye.length===0?[...f.GK||[],...f.DEF||[]].filter(W=>!W.used).map(W=>W.cardId):[];function we(W,le,Me){var zi,Si;const ke=W._gcDef,tt={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ke==null?void 0:ke.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",it={purple:"#b06ce0",light_blue:"#00d4ef"}[ke==null?void 0:ke.color]||"#b06ce0",rt=(ke==null?void 0:ke.name)||W.gc_type,st=(ke==null?void 0:ke.effect)||((zi=Ke[W.gc_type])==null?void 0:zi.desc)||"",pt=ke!=null&&ke.image_url?`/manager-wars/icons/${ke.image_url}`:null,$t=((Si=Ke[W.gc_type])==null?void 0:Si.icon)||"⚡",ut=Math.round(Me*.22),ht=Math.round(Me*.22),kt=Me-ut-ht,Ut=rt.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${W.id}" data-gc-type="${W.gc_type}"
        ...
        <div style="height:${ut}px;background:rgba(255,255,255,0.14);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 3px">
        ...
        <div style="height:${kt}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
          ${pt?`<img src="${pt}" style="max-width:${le-10}px;max-height:${kt-6}px;object-fit:contain">`:`<span style="font-size:${Math.round(kt*.55)}px">${$t}</span>`}
        </div>
      ...
      </div>`}function Ie(W,le){return`<div id="pvp-boost-card"
        style="box-sizing:border-box;width:${W}px;height:${le}px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:2px solid #87CEEB;border-radius:10px;cursor:pointer;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:${Math.round(le*.04)}px;text-align:center;flex-shrink:0">
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${gt(f,u[o+"Formation"],Ne,S,300,300,ve)}
      </div>
    </div>`;function Ue(W){if(W.type==="duel"&&(W.homeTotal!=null||W.aiTotal!=null)){const le=(W.homeTotal||0)>=(W.aiTotal||0);return`<div style="background:rgba(255,255,255,0.05);border-radius:8px;padding:5px 6px;border:1px solid rgba(255,255,255,0.08)">
        ...
        </div>`}const W=(u.log||[]).slice(-1)[0];return W?'<div style="padding:2px 4px">'+Ue(W)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})(),Ze=`
      <div style="display:flex;align-items:center;padding:8px 10px;background:rgba(0,0,0,0.5);gap:6px;flex-shrink:0">
        <button id="pvp-quit" style="width:34px;height:34px;border-radius:50%;background:rgba(220,50,50,0.7);border:none;color:#fff;font-size:16px;cursor:pointer;flex-shrink:0">✕</button>
        <div style="flex:1;display:flex;align-items:center;justify-content:center;gap:8px">
      ...
      </div>`;function Qe(){const W=e.querySelector(".match-screen");if(!W)return;const le=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);W.style.height=le+"px",W.style.minHeight=le+"px",W.style.maxHeight=le+"px",W.style.overflow="hidden";const Me=e.querySelector("#mobile-action-bar"),ke=e.querySelector("#mobile-play-area");Me&&ke&&(ke.style.paddingBottom=Me.offsetHeight+"px")}if(Qe(),setTimeout(Qe,120),setTimeout(Qe,400),C||(C=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",Qe),window.visualViewport.addEventListener("scroll",Qe)),window.addEventListener("resize",Qe)),function(){const le=e.querySelector(".terrain-wrapper svg");le&&(le.removeAttribute("width"),le.removeAttribute("height"),le.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",le.setAttribute("viewBox","-26 -26 352 352"),le.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),e.querySelectorAll(".match-slot-hit").forEach(W=>{W.addEventListener("click",()=>{if(!A&&!_)return;const le=W.dataset.cardId,Me=W.dataset.role,ke=(f[Me]||[]).find(st=>st.cardId===le);if(!ke||ke.used)return;const tt=ve.includes(le);if(A&&!["MIL","ATT"].includes(Me)&&!tt)return;Array.isArray(u["selected_"+o])||(u["selected_"+o]=[]);const it=u["selected_"+o],rt=it.findIndex(st=>st.cardId===le);rt>-1?it.splice(rt,1):it.length<3&&it.push({...ke,_role:Me}),be()})}),e.querySelectorAll(".match-used-hit").forEach(W=>{W.addEventListener("click",()=>R(W.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(W=>{W.addEventListener("click",()=>R())}),(ot=e.querySelector("#pvp-sub-open"))==null||ot.addEventListener("click",()=>R()),e.querySelectorAll(".pvp-gc-mini").forEach(W=>{W.addEventListener("click",()=>te(W.dataset.gcId,W.dataset.gcType))}),(mt=e.querySelector("#pvp-boost-card"))==null||mt.addEventListener("click",()=>ae()),(de=e.querySelector("#pvp-action"))==null||de.addEventListener("click",W=>{A?W.currentTarget.dataset.pass==="1"||!Ce(o)?xe():re():_&&ce()}),(_e=e.querySelector("#pvp-quit"))==null||_e.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&P()}),(Q=e.querySelector("#pvp-view-opp"))==null||Q.addEventListener("click",()=>ge()),(pe=e.querySelector("#pvp-toggle-history"))==null||pe.addEventListener("click",()=>me()),K&&(clearInterval(K),K=null),(A||_)&&!M){let W=30,le=!1;const Me=()=>document.getElementById("pvp-timer"),ke=()=>{Me()&&(Me().textContent=W+"s",Me().style.color=le?"#ff4444":"#fff")};ke(),K=setInterval(()=>{W--,W<0?le?(clearInterval(K),K=null,A&&!Ce(o)?xe():P()):(le=!0,W=15,ke()):ke()},1e3)}}function Te(){et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
      <div style="font-size:20px;font-weight:900;color:#ff6b6b">${u[g+"Name"]||"Adversaire"}</div>
      <div style="width:min(90vw,420px)">${ft(u[g+"Team"],u[g+"Formation"],null,[],300,300)}</div>
    ...
    <p style="margin-top:12px;font-size:13px;color:#888">Tu peux revoir ce tutoriel depuis les paramètres à tout moment.</p>`}];function pr(e,t,i){let d=0;const c=document.createElement("div");c.id="tutorial-overlay",c.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.65);z-index:9900;display:flex;align-items:center;justify-content:center;padding:16px";const a=()=>{var o,g,v;const l=t[d],p=d===t.length-1,n=Math.round((d+1)/t.length*100);c.innerHTML=`
      <div style="background:#fff;border-radius:20px;width:100%;max-width:420px;max-height:90vh;display:flex;flex-direction:column;overflow:hidden;box-shadow:0 16px 64px rgba(0,0,0,0.4)">
        ...
        <div style="padding:24px 24px 0;text-align:center">
          ...
          <div style="font-size:11px;color:#aaa">${d+1} / ${t.length}</div>
        </div>
        <!-- Image optionnelle -->
        ...
        <div style="padding:16px 24px;border-top:1px solid #f0f0f0;display:flex;gap:10px;align-items:center">
          ${d>0?'<button id="tuto-prev" style="padding:10px 18px;border-radius:10px;border:1.5px solid #ddd;background:#fff;font-size:13px;font-weight:700;cursor:pointer;color:#555;flex-shrink:0">‹ Précédent</button>':'<div style="flex:1"></div>'}
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    </div>`,document.body.appendChild(c);const a=l=>{c.remove(),l&&t()};c.querySelector("#buy-cancel").addEventListener("click",()=>a(!1)),c.querySelector("#buy-ok").addEventListener("click",()=>a(!0)),c.addEventListener("click",l=>{l.target===c&&a(!1)})}async function yr(e,t,i){const{toast:d}=i,{data:c}=await E.from("market_listings").select("card_id").eq("id",e).single();if(await E.from("market_listings").update({status:"cancelled"}).eq("id",e),c!=null&&c.card_id){const{count:a}=await E.from("market_listings").select("id",{count:"exact",head:!0}).eq("card_id",c.card_id).eq("status","active");a||await E.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",c.card_id)}d("Annonce retirée","success"),Ti(t,i)}async function br(e,{state:t,navigate:i,toast:d}){e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:c}=await E.from("users").select("id,pseudo,club_name,trophies_top1,trophies_top2,trophies_top3,wins,level").order("trophies_top1",{ascending:!1}).limit(100);e.innerHTML=`
  <div class="page">
    <div class="page-header">
  ...
  `}async function hr(e,{state:t,navigate:i,toast:d}){const c=t.profile;if(!c)return;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:a}=await E.from("matches").select(`id,home_id,away_id,home_score,away_score,status,mode,winner_id,created_at,played_at,
      home:users!home_id(pseudo,club_name),
      away:users!away_id(pseudo,club_name)`).or(`home_id.eq.${c.id},away_id.eq.${c.id}`).order("created_at",{ascending:!1}).limit(50),s={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},l=(a||[]).filter(o=>o.status==="finished"),p=(a||[]).filter(o=>o.status==="in_progress");function n(o){const g=o.home_id===c.id;g?o.home_score:o.away_score,g?o.away_score:o.home_score;const v=o.winner_id===c.id,y=o.home_score===o.away_score&&o.status==="finished",T=o.status!=="finished"?"…":y?"N":v?"V":"D",r=o.status!=="finished"||y?"#888":v?"#1A6B3C":"#c0392b";let u=s[o.mode]||o.mode;o.away_id===null&&!u.startsWith("IA")&&(u="IA");const L=o.home_id===c.id?o.away:o.home;let N;o.away_id===null?N=u:L?N=`${L.club_name||L.pseudo} (${L.pseudo})`:N="Adversaire";let K="";o.status==="in_progress"&&Date.now()-new Date(o.created_at).getTime()>3600*1e3&&(K=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const C=new Date(o.created_at),q=C.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+C.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),D=o.status==="finished"?`${o.home_score} - ${o.away_score}`:`${o.home_score||0} - ${o.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
  ...
  </div>`}zn(Ln);const Ge={user:null,profile:null,page:"home",params:{}};function At(e,t="info",i=3e3){const d=document.getElementById("toast");d&&(d.textContent=e,d.className=`show ${t}`,clearTimeout(d._t),d._t=setTimeout(()=>{d.className=""},i))}function vr(e,t,i=""){document.getElementById("modal-title").textContent=e,document.getElementById("modal-body").innerHTML=t,document.getElementById("modal-footer").innerHTML=i,document.getElementById("modal-overlay").classList.remove("hidden")}function xi(){document.getElementById("modal-overlay").classList.add("hidden")}async function Ct(){if(!Ge.user)return;const{data:e}=await E.from("users").select("*").eq("id",Ge.user.id).single();e&&(Ge.profile=e)}function Ft(e,t={}){Ge.page=e,Ge.params=t,An()}async function An(){var d,c,a,s;const e=document.getElementById("page-content");if(!e)return;document.querySelectorAll(".bottom-nav a").forEach(l=>{l.classList.toggle("active",l.dataset.page===Ge.page)});const t=document.getElementById("nav-credits");t&&Ge.profile&&(t.textContent=`💰 ${(Ge.profile.credits||0).toLocaleString("fr")}`);const i={state:Ge,navigate:Ft,toast:At,openModal:vr,closeModal:xi,refreshProfile:Ct};switch(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽</div>',Ge.page){case"home":await Ci(e,i);break;case"collection":await Xn(e,i);break;case"decks":await ci(e,i);break;case"boosters":await uo(e,i);break;case"match":{const l=Ge.params&&Ge.params.matchMode||"vs_ai_easy";l==="random"?await wn(e,i):l==="friend"?await Vo(e,i,(d=Ge.params)==null?void 0:d.friendId,(c=Ge.params)==null?void 0:c.friendName):l==="mini-league"?await Oi(e,i,(a=Ge.params)==null?void 0:a.mlMatchId,(s=Ge.params)==null?void 0:s.leagueId):await To(e,i);break}case"market":await gr(e,i);break;case"rankings":await br(e,i);break;case"matches":await hr(e,i);break;case"friends":await Bn(e,i);break;case"mini-league":await Wo(e,i);break;case"match-mini-league":{const l=Ge.params||{};await Oi(e,i,l.mlMatchId,l.leagueId);break}default:await Ci(e,i)}}function wr(){var d;const e=document.getElementById("app"),t=Ge.profile;if(!t)return;const i="/manager-wars/icons/";e.innerHTML=`
    <nav class="top-nav">
  ...
  `,document.querySelectorAll(".bottom-nav a").forEach(c=>{c.addEventListener("click",a=>{a.preventDefault(),Ft(c.dataset.page)})}),document.getElementById("nav-logo").addEventListener("click",()=>Ft("home")),document.getElementById("nav-credits").addEventListener("click",()=>Ft("boosters")),(d=document.getElementById("journal-btn"))==null||d.addEventListener("click",()=>_r())}async function _r(){const{data:e}=await E.from("patch_notes").select("*").eq("is_published",!0).order("published_at",{ascending:!1}).limit(20),t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9000;display:flex;align-items:center;justify-content:center;padding:16px";const i=(e||[]).map(d=>{const c=new Date(d.published_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"long",year:"numeric"});return`<div style="padding:14px 0;border-bottom:1px solid #f0f0f0">
      ${d.image_url?`<img src="${d.image_url}" style="width:100%;max-height:160px;object-fit:cover;border-radius:8px;margin-bottom:10px">`:""}
      <div style="font-size:12px;color:#999;margin-bottom:4px">${c}</div>
      <div style="font-size:15px;font-weight:900;margin-bottom:6px">${d.title}</div>
      <div style="font-size:13px;color:#444;line-height:1.6;white-space:pre-line">${d.description}</div>
    ...
    </div>`,document.body.appendChild(t),t.querySelector("#journal-close").addEventListener("click",()=>t.remove()),t.addEventListener("click",d=>{d.target===t&&t.remove()})}async function $r(){document.documentElement.setAttribute("data-theme","light"),document.getElementById("modal-overlay").addEventListener("click",i=>{i.target===i.currentTarget&&xi()}),document.getElementById("modal-close").addEventListener("click",xi);const{data:{session:e}}=await E.auth.getSession();if(!e){Ki(),Li(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:At});return}Ge.user=e.user,await Ct(),Ki();try{const{data:i}=await E.from("formation_links_overrides").select("formation, links"),d={};(i||[]).forEach(c=>{d[c.formation]=c.links}),Sn(d)}catch(i){console.warn("Impossible de charger les overrides de formation:",i)}if(!Ge.profile){Mn(document.getElementById("app"),{state:Ge,navigate:async()=>{await Ct(),di()},toast:At,refreshProfile:Ct});return}const t=Array.isArray(Ge.profile.pending_boosters)?Ge.profile.pending_boosters:[];if(!Ge.profile.onboarding_done&&t.length>0){$o(document.getElementById("app"),{state:Ge,navigate:()=>di(),toast:At,refreshProfile:Ct});return}di(),setTimeout(()=>ur(Ge.profile,Ft,At),800),E.auth.onAuthStateChange(async(i,d)=>{i==="SIGNED_OUT"&&(Ge.user=null,Ge.profile=null,document.getElementById("app").innerHTML="",Li(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:At}))})}function kr(){return Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight)}function ri(){const e=document.getElementById("app");e&&(e.style.height=kr()+"px")}window.addEventListener("resize",ri);window.addEventListener("orientationchange",()=>setTimeout(ri,150));window.visualViewport&&window.visualViewport.addEventListener("resize",ri);function di(){const e=()=>{var i;(i=Ge.user)!=null&&i.id&&E.from("users").update({last_seen_at:new Date().toISOString()}).eq("id",Ge.user.id).then(()=>{})};e(),window._presencePingInterval&&clearInterval(window._presencePingInterval),window._presencePingInterval=setInterval(e,6e4);const t=document.getElementById("app");t.style.display="flex",t.style.flexDirection="column",ri(),wr(),An()}function Ki(){const e=document.getElementById("app-loader"),t=document.getElementById("app");t&&(t.style.display=""),e&&(e.classList.add("zoom-out"),setTimeout(()=>e.style.display="none",500))}function Tn(e){var d;const t=document.getElementById("app-loader");if(t&&(t.style.display="none"),document.getElementById("boot-error"))return;const i=document.createElement("div");i.id="boot-error",i.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:99999;gap:16px;color:#fff;padding:24px;text-align:center",i.innerHTML=`
    <div style="font-size:42px">📡</div>
    <div style="font-size:18px;font-weight:900">Connexion impossible</div>
    <div style="font-size:13px;color:rgba(255,255,255,0.6);max-width:280px">${e||"Le chargement a échoué. Vérifie ta connexion et réessaie."}</div>
    <button id="boot-retry" style="margin-top:8px;padding:12px 30px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer">Réessayer</button>`,document.body.appendChild(i),(d=document.getElementById("boot-retry"))==null||d.addEventListener("click",()=>window.location.reload())}$r().catch(e=>{console.error("Échec du démarrage:",e),Tn()});setTimeout(()=>{const e=document.getElementById("app-loader");e&&e.style.display!=="none"&&!e.classList.contains("zoom-out")&&!document.getElementById("boot-error")&&Tn("Le serveur met trop de temps à répondre.")},12e3);
