...
import{s as T,F as $i,i as Ut,k as Vi,l as nt,m as Ii,f as Nt,n as vo,T as li,j as wo,o as _o,b as $o}from"./formation-links-BZe0pGr4.js";const ko="/";function Mi(e,{navigate:t,toast:i}){let n="login";const d=()=>{var r,l,u,a,o,x;const s=n==="login";e.innerHTML=`
  ...
  `;function l(){var _;const a=document.getElementById("logo-preview"),o=document.getElementById("logo-initials"),x=((_=document.getElementById("setup-club"))==null?void 0:_.value)||"MW",p=x.trim().split(" ").filter(Boolean),h=p.length>=2?(p[0][0]+p[1][0]).toUpperCase():x.slice(0,2).toUpperCase();a&&(a.style.background=r,a.style.borderColor=s),o&&(o.textContent=h,o.style.color=s)}document.getElementById("color1").addEventListener("input",a=>{s=a.target.value,document.getElementById("swatch1").style.background=s,l()}),document.getElementById("color2").addEventListener("input",a=>{r=a.target.value,document.getElementById("swatch2").style.background=r,l()});function u(a){document.querySelectorAll(".setup-step").forEach(o=>o.classList.remove("active")),document.getElementById(`step-${a}`).classList.add("active"),document.getElementById("step-num").textContent=a,document.getElementById("progress-fill").style.width=`${Math.round(a/3*100)}%`,a===3&&l()}document.getElementById("step1-next").addEventListener("click",async()=>{const a=document.getElementById("setup-pseudo").value.trim(),o=document.getElementById("step1-error");if(o.textContent="",a.length<3){o.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:x}=await T.from("users").select("id").eq("pseudo",a).maybeSingle();if(x){o.textContent="Ce pseudo est déjà pris.";return}u(2)}),document.getElementById("step2-back").addEventListener("click",()=>u(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const a=document.getElementById("setup-club").value.trim(),o=document.getElementById("step2-error");if(o.textContent="",a.length<2){o.textContent="Nom trop court (min. 2 caractères).";return}const{data:x}=await T.from("users").select("id").eq("club_name",a).maybeSingle();if(x){o.textContent="Ce nom de club est déjà pris.";return}u(3)}),document.getElementById("step3-back").addEventListener("click",()=>u(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const a=document.getElementById("setup-pseudo").value.trim(),o=document.getElementById("setup-club").value.trim(),x=document.getElementById("step3-error"),p=document.getElementById("step3-finish");x.textContent="",p.disabled=!0,p.textContent="Création en cours…";try{const{error:h}=await T.from("users").insert({id:t.user.id,pseudo:a,club_name:o,club_color1:s,club_color2:r,credits:1e4});if(h)throw h;await To(t.user.id),await d(),n(`Bienvenue ${a} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(h){x.textContent=h.message,p.disabled=!1,p.textContent="🚀 Créer mon profil !"}})}async function To(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await T.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const zo="modulepreload",So=function(e){return"/"+e},ji={},ni=function(t,i,n){let d=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const r=document.querySelector("meta[property=csp-nonce]"),l=(r==null?void 0:r.nonce)||(r==null?void 0:r.getAttribute("nonce"));d=Promise.allSettled(i.map(u=>{if(u=So(u),u in ji)return;ji[u]=!0;const a=u.endsWith(".css"),o=a?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${u}"]${o}`))return;const x=document.createElement("link");if(x.rel=a?"stylesheet":zo,a||(x.as="script"),x.crossOrigin="",x.href=u,l&&x.setAttribute("nonce",l),document.head.appendChild(x),a)return new Promise((p,h)=>{x.addEventListener("load",p),x.addEventListener("error",()=>h(new Error(`Unable to preload CSS for ${u}`)))})}))}function s(r){const l=new Event("vite:preloadError",{cancelable:!0});if(l.payload=r,window.dispatchEvent(l),!l.defaultPrevented)throw r}return d.then(r=>{for(const l of r||[])l.status==="rejected"&&s(l.reason);return t().catch(s)})},Rt="#1A6B3C",Ot="#cc2222",Ao="#D4A017",Ci="#888";async function Lo(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    <div style="display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:8px;padding:10px 0;border-bottom:1px solid #f0f0f0">
      ...
      <div style="text-align:left;font-size:18px;font-weight:900;color:${R}">${B}</div>
  ...
  ${B?`<img src="${B}"
    ...
    width:${N}px;height:${P}px;object-fit:cover;object-position:top center;z-index:2"
  ...
  <div style="position:absolute;left:${ce}px;top:${se}px;width:${Ee}px;height:${ve}px;z-index:3;
    ...
    ${ht(e,{width:140})}
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),a=(l||[]).filter(H=>H.card_type==="player"&&H.player),o=(l||[]).filter(H=>H.card_type==="game_changer"),x=(l||[]).filter(H=>H.card_type==="formation"),p=(l||[]).filter(H=>H.card_type==="stadium"),{data:h}=await T.from("gc_definitions").select("name,gc_type,color,effect,image_url"),_={};(h||[]).forEach(H=>{_[H.name]=H});const{data:m}=await T.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),c={};(m||[]).forEach(H=>{c[H.id]=H}),p.forEach(H=>{H.stadium_def&&(c[H.stadium_id]=H.stadium_def)});const A=Object.keys($i),g=Object.keys(Oe),I={};a.forEach(H=>{const Z=H.player.id;I[Z]=(I[Z]||0)+1}),new Set(Object.keys(I).map(H=>String(H)));const M=new Set(x.map(H=>H.formation)),C=new Set(o.map(H=>H.gc_type));let B="player",q="Tous",R="",V=!1;function K(){return[...a].sort((H,Z)=>{const ge=Wt.indexOf(H.player.job),de=Wt.indexOf(Z.player.job);return ge!==de?ge-de:(H.player.surname_encoded||"").localeCompare(Z.player.surname_encoded||"")})}function ne(){return[...u||[]].sort((H,Z)=>{const ge=Wt.indexOf(H.job),de=Wt.indexOf(Z.job);return ge!==de?ge-de:(H.surname_encoded||"").localeCompare(Z.surname_encoded||"")})}function ie(){return K().filter(H=>{const Z=H.player,ge=q==="Tous"||Z.job===q,de=!R||`${Z.firstname} ${Z.surname_encoded}`.toLowerCase().includes(R);return ge&&de})}function Q(){return ne().filter(H=>{const Z=q==="Tous"||H.job===q,ge=!R||`${H.firstname} ${H.surname_encoded}`.toLowerCase().includes(R);return Z&&ge})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
  ...
  </div>`;function N(){const H=document.getElementById("col-filters");H&&(B==="player"?(H.innerHTML=`
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",Z=>{R=Z.target.value.toLowerCase(),P()}),e.querySelectorAll(".filter-btn").forEach(Z=>{Z.addEventListener("click",()=>{q=Z.dataset.job,N(),P()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{V=!V,N(),P()})):(H.innerHTML=`
        ...
        </div>`},null)}e.querySelectorAll(".col-tab-btn").forEach(H=>{H.addEventListener("click",()=>{B=H.dataset.tab,q="Tous",R="",V=!1,e.querySelectorAll(".col-tab-btn").forEach(Z=>{const ge=Z.dataset.tab===B;Z.style.borderBottomColor=ge?"var(--green)":"transparent",Z.style.color=ge?"var(--green)":"var(--gray-600)"}),N(),P()})}),N(),P()}function en(e,t,i){const n=Oe[e]||{icon:"⚡",desc:"Effet spécial."},d=(t==null?void 0:t.gc_type)==="ultra_game_changer",s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},r={purple:"#b06ce0",light_blue:"#00d4ef"},l=s[t==null?void 0:t.color]||s.purple,u=r[t==null?void 0:t.color]||r.purple,a=(t==null?void 0:t.name)||e,o=(t==null?void 0:t.effect)||n.desc,x=t!=null&&t.image_url?`/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Jt=1e3;function tn(e,t,i,n){var _,m,c;const{state:d,toast:s,closeModal:r,navigate:l,refreshProfile:u}=i,a=e.formation,o={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function x(){const A=Ut[a]||{},g=$i[a]||[],I=290,M=360,C=20;function B(R){const V=A[R];return V?{x:V.x*I,y:V.y*M}:null}let q=`<svg width="${I}" height="${M}" viewBox="0 0 ${I} ${M}" xmlns="http://www.w3.org/2000/svg">`;for(const[R,V]of g){const K=B(R),ne=B(V);!K||!ne||(q+=`<line x1="${K.x}" y1="${K.y}" x2="${ne.x}" y2="${ne.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const R of Object.keys(A)){const V=B(R);if(!V)continue;const K=R.replace(/\d+/,""),ne=o[K]||"#555";q+=`<circle cx="${V.x}" cy="${V.y}" r="${C}" fill="${ne}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,q+=`<text x="${V.x}" y="${V.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${K}</text>`}return q+="</svg>",q}const p=t.filter(A=>A.formation===a);p.length;const h=!e.is_for_sale;n(`Formation ${a}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(_=document.getElementById("direct-sell-form-btn"))==null||_.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${a} pour ${Jt.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const A=p.find(I=>!I.is_for_sale)||p[0];if(!A){s("Aucune carte à vendre","error");return}await T.from("market_listings").delete().eq("card_id",A.id),await T.from("transfer_history").delete().eq("card_id",A.id);const{error:g}=await T.from("cards").delete().eq("id",A.id);if(g){s(g.message,"error");return}await T.from("users").update({credits:(d.profile.credits||0)+Jt}).eq("id",d.profile.id),await u(),s(`+${Jt.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),r(),l("collection")}),(m=document.getElementById("market-sell-form-btn"))==null||m.addEventListener("click",async()=>{const A=parseInt(document.getElementById("sell-price-form").value);if(!A||A<1){s("Prix invalide","error");return}await T.from("cards").update({is_for_sale:!0,sale_price:A}).eq("id",e.id),await T.from("market_listings").insert({seller_id:d.profile.id,card_id:e.id,price:A}),s("Carte mise en vente sur le marché !","success"),r(),l("collection")}),(c=document.getElementById("cancel-sell-form-btn"))==null||c.addEventListener("click",async()=>{await T.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await T.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),s("Annonce retirée","success"),r(),l("collection")})}async function Ni(e,t,i,n){var xe,_e,we,ve,Ee;const{state:d,toast:s,openModal:r,closeModal:l,navigate:u,refreshProfile:a}=n,o=e.player,x=t.filter(se=>se.player.id===o.id),p=x.length,h=e.evolution_bonus||0,_=Math.max((Number(o.note_g)||0)+(o.job==="GK"||o.job2==="GK"?h:0),(Number(o.note_d)||0)+(o.job==="DEF"||o.job2==="DEF"?h:0),(Number(o.note_m)||0)+(o.job==="MIL"||o.job2==="MIL"?h:0),(Number(o.note_a)||0)+(o.job==="ATT"||o.job2==="ATT"?h:0)),m=o.rarity||"normal",{data:c}=await T.from("sell_price_configs").select("*").eq("rarity",m).lte("note_min",_).gte("note_max",_).order("note_min",{ascending:!1}).limit(1);((xe=c==null?void 0:c[0])==null?void 0:xe.price)??Yo[m];const A=o.rarity!=="legende";Qo(o);const g=((o.rarity==="pepite"||o.rarity==="papyte")&&e.current_note!=null?e.current_note:pi(o,o.job))+h,I=o.rarity==="pepite"||o.rarity==="papyte",M=o.job2?(I?io(e,Jo(o.job2)):pi(o,o.job2))+(pi(o,o.job2)>0?h:0):null;ci[o.job],o.job2&&ci[o.job2];const C=Ko[o.rarity]||"#ccc";Xo[o.country_code]||o.country_code;const B=e.evolution_bonus||0,R=g+B,V=M||0,K=V>0?V+B:0,ne=x.map(se=>se.id);let ie={};if(ne.length){const{data:se}=await T.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",ne).order("transferred_at",{ascending:!0});(se||[]).forEach(pe=>{ie[pe.card_id]||(ie[pe.card_id]=[]),ie[pe.card_id].push(pe)})}const Q=se=>{const pe=se.transferred_at?new Date(se.transferred_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"2-digit",year:"numeric"}):"",ce=se.source==="booster"?"Booster":se.price?se.price.toLocaleString("fr"):"—";return`
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",o.note_g],["DEF",o.note_d],["MIL",o.note_m],["ATT",o.note_a]].map(([se,pe])=>{const ce=ci[se],H=se==="GK"?"#fff":ce,Z=se===o.job||se===o.job2,ge=(Number(pe)||0)+(Z&&B>0?B:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(_e=document.getElementById("close-detail"))==null||_e.addEventListener("click",l);let P=new Set;const re=()=>{const se=P.size,pe=document.getElementById("sell-action-panel");if(!pe)return;pe.style.display=se>0?"block":"none",document.getElementById("sell-selected-count").textContent=se;const ce=document.getElementById("evolve-btn");ce&&(ce.textContent=`⬆️ Évoluer ${se>1?"(+"+se+")":""}`)};document.querySelectorAll(".expl-check").forEach(se=>{se.addEventListener("change",()=>{const pe=se.dataset.id;se.checked?P.add(pe):P.delete(pe);const ce=se.closest(".exemplaire-row");ce&&(ce.style.borderColor=se.checked?"#1A6B3C":"#eee"),re()})}),document.querySelectorAll(".exemplaire-row").forEach(se=>{se.addEventListener("click",pe=>{if(pe.target.closest("button")||pe.target.tagName==="INPUT")return;const ce=se.querySelector(".expl-check");ce&&!ce.disabled&&(ce.checked=!ce.checked,ce.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(se=>{se.addEventListener("click",pe=>{pe.stopPropagation();const ce=document.querySelector(`.expl-hist[data-hist="${se.dataset.idx}"]`);ce&&(ce.style.display=ce.style.display==="none"?"flex":"none")})}),(we=document.getElementById("evolve-btn"))==null||we.addEventListener("click",async()=>{if(p<=1)return;const se=[...P];if(!se.length)return;if(P.has(e.id)){const me=document.createElement("div");me.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",me.innerHTML=`
        ...
        <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center;box-shadow:0 16px 48px rgba(0,0,0,0.3)">
          ...
          <div style="background:#f0fdf4;border-radius:10px;padding:12px;margin-bottom:16px;font-size:13px;color:#333">
            ...
            ${M&&M>0?`<br>📈 Note 2 : <strong>${M+(e.evolution_bonus||0)}</strong> → <strong style="color:#1A6B3C">${M+(e.evolution_bonus||0)+pe.length*ce}</strong>`:""}
  ...
  <div style="height:100%;overflow-y:auto;padding-bottom:80px;background:var(--page-bg)">
    ...
    <div class="formation-mobile-only" style="padding:10px 16px;background:#fff;border-bottom:1px solid var(--gray-200)">
      ...
      <select id="formation-select" style="width:100%;padding:7px;border-radius:6px;border:1.5px solid var(--gray-200)">
        ${r.map(M=>`<option value="${M}" ${M===t.formation?"selected":""}>${M}</option>`).join("")}
      </select>
    </div>
      ...
      <div style="display:flex;gap:0;min-height:600px">
        ...
        <div style="width:130px;flex-shrink:0;background:rgba(0,0,0,0.3);display:flex;flex-direction:column;align-items:center;padding:12px 8px;gap:10px;border-right:1px solid rgba(255,255,255,0.1)">
          ...
          <div style="display:flex;flex-direction:column;gap:6px;align-items:center" id="subs-list">
            ${a.map(M=>{const C={...M.player,_evolution_bonus:M.evolution_bonus||0};return`<div style="position:relative;flex-shrink:0;overflow:visible">
                ${ht({...C,_evolution_bonus:C._evolution_bonus||0},{width:60,showStad:!0,stadDef:u})}
        ...
        <div style="width:130px;flex-shrink:0;background:rgba(0,0,0,0.3);display:flex;flex-direction:column;align-items:center;padding:12px 8px;gap:12px;border-left:1px solid rgba(255,255,255,0.1)">
          ...
          <div style="width:100%;text-align:center">
            ...
            <div id="add-stad-btn" style="cursor:pointer;margin:0 auto;width:fit-content">
              ${l?(()=>{var B;const M=t.stadDefMap[l.stadium_id],C=((B=M==null?void 0:M.club)==null?void 0:B.logo_url)||null;return`<div style="width:100px;height:130px;border-radius:8px;background:linear-gradient(135deg,#1a3a6b,#0a1a3a);border:2px solid #4fc3f7;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px">
                  ${C?`<img src="${C}" style="width:48px;height:48px;object-fit:contain">`:'<span style="font-size:36px">🏟️</span>'}
        ...
        <div style="display:flex;gap:10px;align-items:flex-start">
          ...
          <div style="flex:1;min-width:0">
            ...
            <div style="display:flex;gap:4px;align-items:flex-end;flex-wrap:nowrap;overflow-x:auto" id="subs-list">
              ${a.map(M=>{const C={...M.player,_evolution_bonus:M.evolution_bonus||0};return`<div style="position:relative;flex-shrink:0;overflow:visible">
                  ${ht({...C,_evolution_bonus:C._evolution_bonus||0},{width:44,showStad:!0,stadDef:u})}
                  <button data-remove-sub="${M.id}"
          ...
          <div style="flex-shrink:0;text-align:center">
            ...
            <div id="add-stad-btn" style="cursor:pointer">
              ${l?(()=>{var B;const M=t.stadDefMap[l.stadium_id],C=((B=M==null?void 0:M.club)==null?void 0:B.logo_url)||null;return`<div style="width:50px;height:65px;border-radius:6px;background:linear-gradient(135deg,#1a3a6b,#0a1a3a);border:2px solid #4fc3f7;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px">
                  ${C?`<img src="${C}" style="width:26px;height:26px;object-fit:contain">`:'<span style="font-size:18px">🏟️</span>'}
                  <span style="font-size:7px;font-weight:700;color:#E87722;text-align:center;padding:0 2px">${((M==null?void 0:M.name)||"Stade").slice(0,10)}</span>
  ...
  </div>`;const o=window.innerWidth>=768,x=e.querySelector(".deck-pc-layout"),p=e.querySelector(".deck-mobile-layout"),h=e.querySelector(".formation-mobile-only");x&&(x.style.display=o?"block":"none"),p&&(p.style.display=o?"none":"block"),h&&(h.style.display=o?"none":"block"),on(e,t,d,i),document.getElementById("builder-back").addEventListener("click",()=>n("decks")),document.getElementById("formation-select").addEventListener("change",M=>{t.formation=M.target.value;const C=Oi(t.formation),B={};C.forEach(q=>{t.slots[q]&&(B[q]=t.slots[q])}),t.slots=B,at(e,t,i)}),(c=document.getElementById("formation-pc-btn"))==null||c.addEventListener("click",()=>{const{openModal:M,closeModal:C}=i;M("⚽ Choisir une formation",`<div style="display:flex;flex-wrap:wrap;gap:8px;padding:8px">
        ${r.map(B=>`
          <div class="forma-choice" data-forma="${B}" style="cursor:pointer;
            padding:10px 16px;border-radius:8px;
            background:${B===t.formation?"#1A6B3C":"#f0f0f0"};
            color:${B===t.formation?"#fff":"#111"};font-weight:900;font-size:16px;
            border:2px solid ${B===t.formation?"#1A6B3C":"#ddd"}">
            ${B}
          </div>`).join("")}
      </div>`),document.querySelectorAll(".forma-choice").forEach(B=>{B.addEventListener("click",()=>{t.formation=B.dataset.forma,C(),at(e,t,i)})})}),(A=document.getElementById("formation-pc-btn"))==null||A.addEventListener("click",()=>{const{openModal:M,closeModal:C}=i;M("Choisir une formation",`<div style="display:flex;flex-wrap:wrap;gap:8px;padding:8px">
        ${r.map(B=>`
          <div class="forma-choice" data-forma="${B}" style="cursor:pointer;padding:10px 16px;border-radius:8px;background:${B===t.formation?"#1A6B3C":"#f0f0f0"};color:${B===t.formation?"#fff":"#111"};font-weight:900;font-size:16px;border:2px solid ${B===t.formation?"#1A6B3C":"#ddd"}">${B}</div>`).join("")}
      </div>`),document.querySelectorAll(".forma-choice").forEach(B=>{B.addEventListener("click",()=>{t.formation=B.dataset.forma,C(),at(e,t,i)})})}),(g=document.getElementById("add-stad-btn"))==null||g.addEventListener("click",()=>{nn(t,e,i)}),document.getElementById("save-deck").addEventListener("click",()=>sn(t,i)),e.querySelectorAll("[data-remove-sub]").forEach(M=>{M.addEventListener("click",()=>{t.subs=t.subs.filter(C=>C!==M.dataset.removeSub),at(e,t,i)})}),(I=document.getElementById("add-sub-btn"))==null||I.addEventListener("click",()=>{an(t,e,i)})}function on(e,t,i,n){var I,M;const d=window.innerWidth>=768,s=e.querySelector(d?"#deck-field-pc":"#deck-field-mobile");if(!s)return;const r=(I=t.stadiumCards)==null?void 0:I.find(C=>C.id===t.stadiumCardId),l=r&&((M=t.stadDefMap)==null?void 0:M[r.stadium_id])||null,u=Ut[t.formation]||{},a=Vi(t.formation)||[],o={};for(const C of i){const B=t.slots[C],q=B?t.playerCards.find(R=>R.id===B):null;q!=null&&q.player?o[C]={...q.player,_evolution_bonus:q.evolution_bonus||0,face:q.player.face||null}:o[C]=null}const x=window.innerWidth>=768,p=x?window.innerWidth-280:window.innerWidth-20,h=x?Math.min(p,860):p,_=Math.round(x?h*.82:h*1.35),m=x?84:44;let c="";for(const[C,B]of a){const q=u[C],R=u[B];if(!q||!R)continue;const V=q.x*h,K=Math.round(.03*_+q.y*.85*_),ne=R.x*h,ie=Math.round(.03*_+R.y*.85*_),Q=o[C],N=o[B],P=nt(Q,N);P==="#ff3333"||P==="#cc2222"?c+=`<line x1="${V.toFixed(1)}" y1="${K.toFixed(1)}" x2="${ne.toFixed(1)}" y2="${ie.toFixed(1)}" stroke="${P}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(c+=`<line x1="${V.toFixed(1)}" y1="${K.toFixed(1)}" x2="${ne.toFixed(1)}" y2="${ie.toFixed(1)}" stroke="${P}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,c+=`<line x1="${V.toFixed(1)}" y1="${K.toFixed(1)}" x2="${ne.toFixed(1)}" y2="${ie.toFixed(1)}" stroke="${P}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let A="";const g=Math.round(m*657/507);for(const C of i){const B=u[C];if(!B)continue;const q=o[C],R=B.x*h;B.y*_;const V=Math.round(.03*_+B.y*(.85*_)),K=Math.round(R-m/2),ne=Math.round(V-g/2);if(q){const ie=C.replace(/\d+/,""),Q=l&&(l.club_id&&String(q.club_id)===String(l.club_id)||l.country_code&&q.country_code===l.country_code),N=ht({...q,_evolution_bonus:q._evolution_bonus||0},{width:m,showStad:!0,stadDef:l,role:ie});A+=`<div style="position:absolute;left:${K}px;top:${ne}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${C}">
        <div style="position:relative">${Q?'<div style="position:absolute;top:-30px;left:0;right:0;text-align:center;font-size:20px;z-index:5;line-height:1">🏟️</div>':""}${N}</div>
      </div>`}else{const ie=C.replace(/\d+/,"");A+=`<div style="position:absolute;left:${K}px;top:${ne}px;width:${m}px;height:${g}px;
    ...
    <div style="position:relative;width:${h}px;height:${_}px;margin:0 auto">
      ...
      ${x.length>0?x.map(m=>{var C,B;const c=m.player,A=m.evolution_bonus||0,g=(r==="GK"?c.note_g:r==="DEF"?c.note_d:r==="MIL"?c.note_m:c.note_a)+(r===c.job||r===c.job2?A:0),I=no(c),M={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[c.rarity];return`<div class="player-option" data-card-id="${m.id}"
          ...
          <!-- Note -->
          <div style="width:36px;height:36px;border-radius:8px;background:${Dt[r]};color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:16px;border:2px solid ${M};flex-shrink:0">
            ${g}
          </div>
  ...
  `,document.body.appendChild(d);let s=!1;const r=document.getElementById("pack-cut-zone"),l=document.getElementById("pack-blade");let u=!1;const a=N=>N.touches&&N.touches[0]?N.touches[0].clientX:N.clientX;function o(N){s||(u=!0,l.style.opacity="1",x(N))}function x(N){if(!u||s)return;const P=r.getBoundingClientRect(),re=a(N)-P.left,xe=Math.max(0,Math.min(1,re/P.width));l.style.width=xe*P.width+"px",xe>=.82&&h()}function p(){s||(u=!1,l.style.transition="width .2s ease, opacity .2s ease",l.style.width="0",l.style.opacity="0",setTimeout(()=>{s||(l.style.transition="")},220))}function h(){var P;if(s)return;s=!0,u=!1,l.style.width="100%",l.style.opacity="1",(P=document.getElementById("cut-flash"))==null||P.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const N=document.getElementById("cut-hint");N&&(N.style.opacity="0"),r.classList.add("pack-cut"),setTimeout(()=>{l.style.opacity="0",document.getElementById("pack-phase").style.display="none",A(0)},620)}r.addEventListener("pointerdown",o),window.addEventListener("pointermove",x),window.addEventListener("pointerup",p),r.addEventListener("touchstart",o,{passive:!0}),window.addEventListener("touchmove",x,{passive:!0}),window.addEventListener("touchend",p);let _=0,m=!1;const c=new Set;function A(N){_=N,document.getElementById("reveal-phase").style.display="flex",g(),I(N,0),R()}function g(){const N=document.getElementById("card-dots");N&&(N.innerHTML=e.map((P,re)=>`<div class="card-dot" data-i="${re}" style="width:8px;height:8px;border-radius:50%;background:${re===_?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),N.querySelectorAll(".card-dot").forEach(P=>P.addEventListener("click",()=>C(parseInt(P.dataset.i)))))}function I(N,P){var ce;const re=e[N],xe=document.getElementById("card-counter"),_e=document.getElementById("card-track");xe&&(xe.textContent=`Carte ${N+1} / ${e.length}`);const we=document.getElementById("reveal-btns");we&&(we.style.display=N===e.length-1?"flex":"none");const ve=re.card_type==="player"&&((ce=re.player)==null?void 0:ce.rarity)==="legende",Ee=!c.has(N);c.add(N);let se=0;if(re.card_type==="player"&&re.player){const H=re.player,Z=H.job||"ATT";se=Number(Z==="GK"?H.note_g:Z==="DEF"?H.note_d:Z==="MIL"?H.note_m:H.note_a)||0}const pe=H=>{_e.innerHTML=`
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      card:cards(id,card_type,formation,stadium_id,evolution_bonus,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
          clubs(encoded_name,logo_url)))`).in("deck_id",r).order("slot_order"),u=[...new Set((l||[]).filter(p=>{var h,_;return((h=p.card)==null?void 0:h.card_type)==="stadium"&&((_=p.card)==null?void 0:_.stadium_id)}).map(p=>p.card.stadium_id))],a={};if(u.length){const{data:p}=await T.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",u);(p||[]).forEach(h=>{a[h.id]=h}),(l||[]).forEach(h=>{var _,m;((_=h.card)==null?void 0:_.card_type)==="stadium"&&((m=h.card)!=null&&m.stadium_id)&&(h.card._stadiumDef=a[h.card.stadium_id]||null)})}let o=0;function x(){var C,B,q,R,V,K,ne;const p=s[o],h=(l||[]).filter(ie=>ie.deck_id===p.id),_=h.filter(ie=>{var Q;return ie.is_starter&&((Q=ie.card)==null?void 0:Q.player)}).map(ie=>hi(ie.card,ie.position)),m=h.find(ie=>{var Q;return((Q=ie.card)==null?void 0:Q.card_type)==="formation"}),c=p.formation||((C=m==null?void 0:m.card)==null?void 0:C.formation)||"4-4-2";let A=_.length>=11?vt(_,c):null,g=((B=p.stadium_card)==null?void 0:B.stadium_def)||null;g&&A&&(A=Tt(A,g));const I=_.length>=11;et(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    ...
    </div>`,function(){const Q=e.querySelector(".deck-preview-wrap svg");Q&&(Q.removeAttribute("width"),Q.removeAttribute("height"),Q.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",Q.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),(R=document.getElementById("prev-deck"))==null||R.addEventListener("click",()=>{o>0&&(o--,x())}),(V=document.getElementById("next-deck"))==null||V.addEventListener("click",()=>{o<s.length-1&&(o++,x())}),(K=document.getElementById("validate-deck"))==null||K.addEventListener("click",()=>{if(!I)return;const ie=t.state.params||{};t.navigate("match",{...ie,matchMode:ie.matchMode||i,deckId:p.id})}),(ne=document.getElementById("cancel-deck-select"))==null||ne.addEventListener("click",()=>{Ve(e),d("home")});const M=document.getElementById("deck-swipe-zone");if(M){let ie=0,Q=0;M.addEventListener("touchstart",N=>{ie=N.touches[0].clientX,Q=N.touches[0].clientY},{passive:!0}),M.addEventListener("touchend",N=>{const P=N.changedTouches[0].clientX-ie,re=N.changedTouches[0].clientY-Q;Math.abs(P)<40||Math.abs(P)<Math.abs(re)||(P<0&&o<s.length-1?(o++,x()):P>0&&o>0&&(o--,x()))},{passive:!0})}}x()}function kn(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";return e!=null&&e.clubLogo?e.clubLogo.startsWith("http")?e.clubLogo:`${t}/storage/v1/object/public/assets/clubs/${e.clubLogo}`:null}function Xe(e,t=44,i=58,n=null){return ht(e,{width:t,showStad:!!n,stadDef:n,used:e==null?void 0:e.used})}function it(e,t,i){if(!(e!=null&&e.length))return"";const n=e.slice(0,5);let d='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return n.forEach((s,r)=>{if(d+=Xe(s,40,52),r<n.length-1){const l=nt(s,n[r+1]);d+=`<div style="width:7px;height:3px;background:${l==="#ff3333"||l==="#cc2222"?"rgba(255,255,255,0.12)":l};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(d+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),d+="</div>",d}function En(e){return!e||e.length<2?null:`https://flagsapi.com/${e.slice(0,2).toUpperCase()}/flat/64.png`}function Tn(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function ct(e,t,i,n,d=310,s=310,r=[]){var I;const l=Ut[t]||{},u=Vi(t)||$i[t]||[],a={},o=["ATT","MIL","DEF","GK"];for(const M of o)(e[M]||[]).forEach((B,q)=>{a[`${M}${q+1}`]=B});function x(M){const C=l[M];return C?{x:C.x*d,y:C.y*s}:null}let p="";for(const[M,C]of u){const B=x(M),q=x(C);if(!B||!q)continue;const R=a[M],V=a[C],K=nt(R,V);K==="#00ff88"||K==="#FFD700"?(p+=`<line x1="${B.x.toFixed(1)}" y1="${B.y.toFixed(1)}" x2="${q.x.toFixed(1)}" y2="${q.y.toFixed(1)}"
        stroke="${K}" stroke-width="10" stroke-linecap="round" opacity="0.22"/>`,p+=`<line x1="${B.x.toFixed(1)}" y1="${B.y.toFixed(1)}" x2="${q.x.toFixed(1)}" y2="${q.y.toFixed(1)}"
        stroke="${K}" stroke-width="3.5" stroke-linecap="round" opacity="0.95"/>`):p+=`<line x1="${B.x.toFixed(1)}" y1="${B.y.toFixed(1)}" x2="${q.x.toFixed(1)}" y2="${q.y.toFixed(1)}"
        stroke="${K}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const h=48,_=64,m=13,c=16,A={normal:"#aaaaaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#111111",legende:"#7a28b8",légende:"#7a28b8"};for(const[M,C]of Object.entries(a)){const B=x(M);if(!B||!C)continue;const q=M.replace(/[0-9]/g,""),R=Ht[q]||"#555",V=r.includes(C.cardId),K=i==="attack"&&(["MIL","ATT"].includes(q)||V)&&!C.used||i==="defense"&&["GK","DEF","MIL"].includes(q)&&!C.used,ne=n.includes(C.cardId);let ie;i==="attack"?ie=V?Math.max(1,Number(C.note_a)||0):q==="MIL"?Number(C.note_m)||0:Number(C.note_a)||0:i==="defense"?ie=q==="GK"?Number(C.note_g)||0:q==="MIL"?Number(C.note_m)||0:Number(C.note_d)||0:ie=Number(q==="GK"?C.note_g:q==="DEF"?C.note_d:q==="MIL"?C.note_m:C.note_a)||0,ie=ie+(C.boost||0),C.stadiumBonus&&(i==="attack"&&(q==="ATT"||q==="MIL")||i==="defense"&&(q==="GK"||q==="DEF"||q==="MIL")?ie+=10:i||(ie+=10));const Q=(B.x-h/2).toFixed(1),N=(B.y-_/2).toFixed(1),P=A[C==null?void 0:C.rarity]||A.normal;if(C.used){const se=typeof import.meta<"u"&&"/"||"/",ce=`${typeof window<"u"&&((I=window.location)==null?void 0:I.origin)||""}${se}icons/carte-dos.png`.replace(/([^:])\/\//g,"$1/");p+=`<rect x="${Q}" y="${N}" width="${h}" height="${_}" rx="5" fill="#161616"/>`,p+=`<image href="${ce}" xlink:href="${ce}" x="${Q}" y="${N}" width="${h}" height="${_}" preserveAspectRatio="xMidYMid slice"/>`,p+=`<rect x="${Q}" y="${N}" width="${h}" height="${_}" rx="5" fill="none" stroke="${P}" stroke-width="2" opacity="0.7"/>`,p+=`<rect x="${Q}" y="${N}" width="${h}" height="${_}" rx="5" fill="rgba(0,0,0,0.01)" class="match-used-hit" data-card-id="${C.cardId}" data-role="${q}" style="cursor:pointer"/>`;continue}const re=ne?.45:1,xe=ne?"#FFD700":P,_e=ne?3:(C==null?void 0:C.rarity)==="legende"||(C==null?void 0:C.rarity)==="légende"||(C==null?void 0:C.rarity)==="pepite"||(C==null?void 0:C.rarity)==="pépite"?2.5:2,we=_-m-c;p+=`<defs><clipPath id="cp-${M}"><rect x="${Q}" y="${(B.y-_/2+m).toFixed(1)}" width="${h}" height="${we}"/></clipPath></defs>`,p+=`<rect x="${Q}" y="${N}" width="${h}" height="${_}" rx="5" fill="${R}" opacity="${re}"/>`;const ve=He(C);ve&&(p+=`<image href="${ve}" xlink:href="${ve}" x="${Q}" y="${(B.y-_/2+m).toFixed(1)}" width="${h}" height="${we}" clip-path="url(#cp-${M})" preserveAspectRatio="xMidYMin slice"/>`),p+=`<rect x="${Q}" y="${N}" width="${h}" height="${m}" rx="4" fill="rgba(255,255,255,0.92)"/>`,p+=`<text x="${B.x.toFixed(1)}" y="${(B.y-_/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(C.name||"").slice(0,9)}</text>`;const Ee=(B.y+_/2-c).toFixed(1);p+=`<rect x="${Q}" y="${Ee}" width="${h}" height="${c}" fill="rgba(255,255,255,0.92)"/>`;{const se=En(C.country_code);se?p+=`<image href="${se}" xlink:href="${se}" x="${(B.x-20).toFixed(1)}" y="${(B.y+_/2-c+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:p+=`<text x="${(B.x-13).toFixed(1)}" y="${(B.y+_/2-c/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${Tn(C.country_code)}</text>`;const pe=C.stadiumBonus?"#E87722":"#111";p+=`<text x="${B.x.toFixed(1)}" y="${(B.y+_/2-c/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="${pe}" font-family="Arial Black">${ie}</text>`;const ce=kn(C);ce?p+=`<image href="${ce}" xlink:href="${ce}" x="${(B.x+h/2-14).toFixed(1)}" y="${(B.y+_/2-c+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:C.clubName&&(p+=`<text x="${(B.x+14).toFixed(1)}" y="${(B.y+_/2-c/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(C.clubName||"").slice(0,3).toUpperCase()}</text>`),C.stadiumBonus&&(p+=`<rect x="${(B.x-h/2).toFixed(1)}" y="${(B.y-_/2).toFixed(1)}" width="${h}" height="${_}" rx="5" fill="none" stroke="#E87722" stroke-width="2" opacity="0.8"/>`)}p+=`<rect x="${Q}" y="${N}" width="${h}" height="${_}" rx="5" fill="${ne?"rgba(255,255,255,0.12)":"none"}" stroke="${xe}" stroke-width="${_e}" opacity="${re}"/>`,K&&(p+=`<rect x="${Q}" y="${N}" width="${h}" height="${_}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${ne?"selected":""}" data-card-id="${C.cardId}" data-role="${q}" style="cursor:pointer"/>`)}const g=38;return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-g} ${-g} ${d+g*2} ${s+g*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${p}
  ...
  </div>`}async function si(e,t,i,n){var B;const{state:d,navigate:s,toast:r}=t;et(e);const l=d.params||{};if(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!l.deckId)return $n(e,t,i);const u=l.deckId;let a,o,x,p;try{const q=await Promise.all([T.from("decks").select("formation,name,stadium_card_id").eq("id",u).single(),T.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation, evolution_bonus,
            player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
              clubs(encoded_name,logo_url)))`).eq("deck_id",u).order("slot_order")]);a=q[0].data,x=q[0].error,o=q[1].data,p=q[1].error}catch(q){console.error("[Match] Exception chargement deck:",q),Et(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>s("home"));return}if(x||p){console.error("[Match] Erreur Supabase:",x||p),Et(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>s("home"));return}const h=(o||[]).filter(q=>{var R;return q.is_starter&&((R=q.card)==null?void 0:R.player)}).map(q=>hi(q.card,q.position)),_=(o||[]).filter(q=>{var R;return!q.is_starter&&((R=q.card)==null?void 0:R.player)}).map(q=>hi(q.card,q.position));if(h.length<11){Et(e,"⚠️",`Deck incomplet (${h.length}/11).`,"Compléter",()=>s("decks"));return}const m=(o||[]).find(q=>{var R;return((R=q.card)==null?void 0:R.card_type)==="formation"}),c=(a==null?void 0:a.formation)||((B=m==null?void 0:m.card)==null?void 0:B.formation)||"4-4-2",{data:A,error:g}=await T.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",d.profile.id).eq("card_type","game_changer"),{data:I}=await T.from("gc_definitions").select("*").eq("is_active",!0),M=(A||[]).map(q=>({...q,_gcDef:(I==null?void 0:I.find(R=>R.name===q.gc_type||R.id===q.gc_definition_id))||null}));let C=null;if(a!=null&&a.stadium_card_id){const{data:q}=await T.from("cards").select("stadium_id").eq("id",a.stadium_card_id).single();if(q!=null&&q.stadium_id){const{data:R}=await T.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").eq("id",q.stadium_id).single();C=R||null}}n({deckId:u,formation:c,starters:h,subsRaw:_,gcCardsEnriched:M,gcDefs:I||[],stadiumDef:C})}async function zn(e,t){const{state:i}=t,d=(i.params||{}).matchMode||"vs_ai_easy",s=d.replace("vs_ai_",""),r=d;await si(e,t,d,async({deckId:l,formation:u,starters:a,subsRaw:o,gcCardsEnriched:x,gcDefs:p,stadiumDef:h})=>{try{let _=vt(a,u);h&&(_=Tt(_,h),jt(o,h));const m=await Sn(u,s),c=m.lines||m,A=async g=>{try{const{data:I,error:M}=await T.from("matches").insert({home_id:i.profile.id,away_id:null,mode:r,home_deck_id:l,status:"in_progress"}).select().single();if(M){console.error("[MatchIA] Erreur création match:",M),Et(e,"⚠️","Impossible de créer le match ("+M.message+").","Retour",()=>t.navigate("home"));return}const C=m.stadiumDef||null;C&&c&&(Tt(c,C),jt(m.subs||[],C));const B={gcDefs:p||[],matchId:I==null?void 0:I.id,mode:r,difficulty:s,formation:u,homeTeam:_,aiTeam:c,homeSubs:o,subsUsed:0,maxSubs:Math.min(o.length,3),aiSubs:m.subs||[],aiSubsUsed:0,aiMaxSubs:Math.min((m.subs||[]).length,3),aiUsedSubIds:[],aiGcCards:m.gcCards||[],aiUsedGc:[],aiStadiumDef:C,homeScore:0,aiScore:0,gcCards:g,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Ln(e,B,t)}catch(I){console.error("[MatchIA] Exception launchMatch:",I),Et(e,"⚠️","Erreur au lancement du match : "+I.message,"Retour",()=>t.navigate("home"))}};if(!x.length){A([]);return}ai(e,x,A)}catch(_){console.error("[MatchIA] Exception setup:",_),Et(e,"⚠️","Erreur de préparation du match : "+_.message,"Retour",()=>t.navigate("home"))}})}async function Sn(e,t){var c;const{data:i}=await T.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(80);if(!i||i.length<11)return{lines:An(e),subs:[],gcCards:[],stadiumDef:null};const n=Mt[e]||Mt["4-4-2"],d={GK:[],DEF:[],MIL:[],ATT:[]},s=new Set;function r(A,g,I){var M,C;return s.add(A.id),{cardId:"ai-"+A.id+"-"+I,id:A.id,firstname:A.firstname,name:A.surname_encoded,country_code:A.country_code,club_id:A.club_id,job:A.job,job2:A.job2,note_g:Number(A.note_g)||0,note_d:Number(A.note_d)||0,note_m:Number(A.note_m)||0,note_a:Number(A.note_a)||0,rarity:A.rarity,skin:A.skin,hair:A.hair,hair_length:A.hair_length,clubName:((M=A.clubs)==null?void 0:M.encoded_name)||null,clubLogo:((C=A.clubs)==null?void 0:C.logo_url)||null,boost:0,used:!1,_line:g}}for(const A of["GK","DEF","MIL","ATT"]){const g=i.filter(q=>q.job===A&&!s.has(q.id)),I=i.filter(q=>q.job!==A&&!s.has(q.id)),M=[...g,...I],C=[];for(let q=0;q<n[A];q++){const R=M[q];R&&C.push(r(R,A,q))}const B=dt(C.length);C.forEach((q,R)=>{q._col=B[R]}),d[A]=C}const u=i.filter(A=>!s.has(A.id)).slice(0,5).map((A,g)=>r(A,A.job,100+g)),x=Object.keys(Oe).sort(()=>Math.random()-.5).slice(0,3).map((A,g)=>{var I,M;return{id:"ai-gc-"+g,gc_type:A,name:((I=Oe[A])==null?void 0:I.name)||A,icon:((M=Oe[A])==null?void 0:M.icon)||"⚡"}}),p=Object.values(d).flat(),h={};p.forEach(A=>{A.club_id&&(h[A.club_id]=(h[A.club_id]||0)+1)});const _=(c=Object.entries(h).sort((A,g)=>g[1]-A[1])[0])==null?void 0:c[0];let m=null;if(_){const{data:A}=await T.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",_).single();A&&(m={club_id:A.id,country_code:null,name:A.encoded_name+" Stadium",club:{encoded_name:A.encoded_name,logo_url:A.logo_url}})}return{lines:d,subs:u,gcCards:x,stadiumDef:m}}function An(e){const t=Mt[e]||Mt["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},n=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let d=0;for(const s of["GK","DEF","MIL","ATT"]){const r=[];for(let u=0;u<t[s];u++){const a=3+Math.floor(Math.random()*5);r.push({cardId:"fake-"+d,id:"fake-"+d,firstname:"IA",name:n[d%n.length],country_code:"XX",club_id:null,job:s,job2:null,note_g:s==="GK"?a:2,note_d:s==="DEF"?a:2,note_m:s==="MIL"?a:2,note_a:s==="ATT"?a:2,rarity:"normal",boost:0,used:!1,_line:s}),d++}const l=dt(r.length);r.forEach((u,a)=>{u._col=l[a]}),i[s]=r}return i}function Ln(e,t,i){var n;e.innerHTML=`
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
  </div>`,setTimeout(()=>In(e,t,i),5e3)}function In(e,t,i){const n=t.homeTeam.MIL||[],d=t.aiTeam.MIL||[],s=t.stadiumDef||null,r=t.aiStadiumDef||null;function l(g,I){const M=Se(g,"MIL"),C=g.evolution_bonus||g._evolution_bonus||0,B=g.job==="MIL"||g.job2==="MIL"?C:0,q=g.stadiumBonus||I&&(I.club_id&&String(g.club_id)===String(I.club_id)||I.country_code&&g.country_code===I.country_code)?10:0;return M+B+q}function u(g,I){return g.reduce((M,C)=>M+l(C,I),0)}function a(g){let I=0;for(let M=0;M<g.length-1;M++){const C=nt(g[M],g[M+1]);C==="#00ff88"?I+=2:C==="#FFD700"&&(I+=1)}return I}const o=u(n,s)+a(n),x=u(d,r)+a(d),p=o>=x;function h(g,I,M,C,B){return`<div id="duel-row-${C}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0), opacity .5s ease;transform-origin:center">
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${g.map((q,R)=>{const V=R<g.length-1?nt(q,g[R+1]):null,K=!V||V==="#ff3333"||V==="#cc2222",ne=V==="#00ff88"?"+2":V==="#FFD700"?"+1":"";l(q,B),q.stadiumBonus||B&&(B.club_id&&String(q.club_id)===String(B.club_id)||B.country_code&&(q.country_code,B.country_code));const ie=q.evolution_bonus||q._evolution_bonus||0;return`
          <div class="duel-card duel-card-${C}" data-idx="${R}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease, transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
            ${Xe({...q,_evolution_bonus:ie},58,78,B)}
          </div>
          ${R<g.length-1?`<div class="duel-link duel-link-${C}" data-idx="${R}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${K?"rgba(255,255,255,0.12)":V};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${K?"none":`0 0 8px ${V}`}">
      ...
      <div class="duel-score-line duel-score-line-${C}" style="margin-top:10px;font-size:12px;color:rgba(255,255,255,0.55);opacity:0;transition:opacity .4s ease">
        Score: ${u(g,B)} + ${a(g)} liens = <b style="color:#fff">${u(g,B)+a(g)}</b>
      </div>
    </div>`}e.innerHTML=`
  ...
  </div>`;const _=()=>{const g=(I,M)=>e.querySelectorAll(I).forEach((C,B)=>{setTimeout(()=>{C.style.opacity="1",C.style.transform="translateY(0) scale(1)"},M+B*90)});g(".duel-card-home",150),g(".duel-card-ai",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((I,M)=>{setTimeout(()=>{I.style.opacity="1"},M*70)}),900),setTimeout(()=>{const I=e.querySelector("#vs-label");I&&(I.style.opacity="1",I.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(M=>M.style.opacity="1")},1250),setTimeout(()=>{m("score-home",o,800),m("score-ai",x,800)},1500)};function m(g,I,M){const C=document.getElementById(g);if(!C)return;const B=performance.now(),q=R=>{const V=Math.min(1,(R-B)/M);C.textContent=Math.round(I*(1-Math.pow(1-V,3))),V<1?requestAnimationFrame(q):C.textContent=I};requestAnimationFrame(q)}requestAnimationFrame(_),t.attacker=p?"home":"ai";const c=p?ri():null;p&&(t.boostCard={value:c}),t.log.push({type:"duel",title:"Milieu de Terrain",homePlayers:n.map(g=>({name:g.name,note:Se(g,"MIL"),portrait:He(g),job:g.job,country_code:g.country_code,rarity:g.rarity,clubName:g.clubName,clubLogo:g.clubLogo})),aiPlayers:d.map(g=>({name:g.name,note:Se(g,"MIL"),portrait:He(g),job:g.job,country_code:g.country_code,rarity:g.rarity,clubName:g.clubName,clubLogo:g.clubLogo})),homeTotal:o,aiTotal:x,text:`Duel milieu : ${t.clubName} ${o} – ${x} IA → ${p?t.clubName+" attaque":"IA attaque"}`});const A=()=>{t.phase=t.attacker==="home"?"attack":"ai-attack",Je(e,t,i),t.attacker==="ai"&&setTimeout(()=>ti(e,t,i),800)};setTimeout(()=>{const g=document.getElementById("score-home"),I=document.getElementById("score-ai"),M=document.getElementById(p?"duel-row-home":"duel-row-ai"),C=document.getElementById(p?"duel-row-ai":"duel-row-home"),B=p?g:I,q=p?I:g;B&&(B.style.fontSize="80px",B.style.color=p?"#FFD700":"#ff6b6b",B.style.animation="duelPulse .5s ease"+(p?", duelGlow 1.5s ease infinite .5s":"")),q&&(q.style.opacity="0.25"),setTimeout(()=>{M&&(M.style.transformOrigin="center",M.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",M.style.zIndex="5"),setTimeout(()=>{var V;const R=document.getElementById("duel-shock");if(R){const K=(V=C||M)==null?void 0:V.getBoundingClientRect(),ne=e.querySelector(".match-screen").getBoundingClientRect();K&&(R.style.top=K.top-ne.top+K.height/2+"px"),R.style.animation="shockwave .5s ease-out forwards"}C&&(C.style.transformOrigin="center",C.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var V;const R=document.getElementById("duel-finale");R&&(R.innerHTML=`
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    ${(()=>{const P=window.innerWidth>=700,re=(H,Z,ge)=>{var F,$;const de=(t.gcDefs||[]).find(f=>f.name===H.gc_type),me={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[de==null?void 0:de.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",oe={purple:"#b06ce0",light_blue:"#00d4ef"}[de==null?void 0:de.color]||"#b06ce0",ue=(de==null?void 0:de.name)||H.gc_type,Le=(de==null?void 0:de.effect)||((F=Oe[H.gc_type])==null?void 0:F.desc)||"",Ie=de!=null&&de.image_url?`/icons/${de.image_url}`:null,Ce=(($=Oe[H.gc_type])==null?void 0:$.icon)||"⚡",Te=Math.round(ge*.22),qe=Math.round(ge*.22),Pe=ge-Te-qe,ze=ue.length>12?7:9;return`<div class="gc-mini" data-gc-id="${H.id}" data-gc-type="${H.gc_type}"
  ...
  `,document.body.appendChild(n);let o=!1;const x=()=>{o||(o=!0,n.remove(),i())};n.addEventListener("click",x),setTimeout(x,2e3)}function qt(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function fi(e,t,i,n=null,d=null){var h,_;if(t.phase!=="attack"){qt("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){qt(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const s=Object.entries(t.homeTeam).flatMap(([m,c])=>(c||[]).filter(A=>A.used).map(A=>({...A,_line:A._line||m}))),r=t.homeSubs.filter(m=>!t.usedSubIds.includes(m.cardId));if(!s.length){qt("Aucun joueur utilisé à remplacer");return}if(!r.length){qt("Aucun remplaçant disponible");return}let l=Math.max(0,s.findIndex(m=>m.cardId===d));const u=((h=s[l])==null?void 0:h._line)||((_=s[l])==null?void 0:_.job);let a=n?Math.max(0,r.findIndex(m=>m.cardId===n)):Math.max(0,r.findIndex(m=>m.job===u)),o=!1;const x=document.createElement("div");x.id="sub-overlay",x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function p(){var C,B,q,R,V,K;const m=s[l],c=r[a],A=Math.min(130,Math.round((window.innerWidth-90)/2)),g=Math.round(A*1.35),I=ne=>`background:rgba(255,255,255,0.12);border:none;color:${ne?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${ne?"default":"pointer"};flex-shrink:0`;x.innerHTML=`
    ...
    </div>`,(C=x.querySelector("#sub-close"))==null||C.addEventListener("click",()=>x.remove()),(B=x.querySelector("#out-up"))==null||B.addEventListener("click",()=>{l>0&&(l--,p())}),(q=x.querySelector("#out-down"))==null||q.addEventListener("click",()=>{l<s.length-1&&(l++,p())}),(R=x.querySelector("#in-up"))==null||R.addEventListener("click",()=>{a>0&&(a--,p())}),(V=x.querySelector("#in-down"))==null||V.addEventListener("click",()=>{a<r.length-1&&(a++,p())});const M=(ne,ie,Q,N)=>{const P=x.querySelector("#"+ne);if(!P)return;let re=0;P.addEventListener("touchstart",xe=>{re=xe.touches[0].clientY},{passive:!0}),P.addEventListener("touchend",xe=>{const _e=xe.changedTouches[0].clientY-re;if(Math.abs(_e)<30)return;const we=ie();_e<0&&we<N-1?(Q(we+1),p()):_e>0&&we>0&&(Q(we-1),p())},{passive:!0})};M("in-panel",()=>a,ne=>a=ne,r.length),M("out-panel",()=>l,ne=>l=ne,s.length),(K=x.querySelector("#sub-confirm"))==null||K.addEventListener("click",ne=>{if(ne.preventDefault(),ne.stopPropagation(),o)return;o=!0;const ie=s[l],Q=r[a];if(!ie||!Q)return;let N=null,P=-1;for(const[xe,_e]of Object.entries(t.homeTeam)){const we=(_e||[]).findIndex(ve=>ve.cardId===ie.cardId);if(we!==-1){N=xe,P=we;break}}if(P===-1||!N){qt("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),x.remove();return}const re={...Q,_line:N,_col:ie._col||0,used:!1,boost:0};t.homeTeam[N].splice(P,1,re),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(Q.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:ie.name,firstname:ie.firstname,note:Se(ie,N),portrait:He(ie),job:ie.job,country_code:ie.country_code,rarity:ie.rarity,clubName:ie.clubName,clubLogo:ie.clubLogo},inPlayer:{name:Q.name,firstname:Q.firstname,note:Se(Q,N),portrait:He(Q),job:Q.job,country_code:Q.country_code,rarity:Q.rarity,clubName:Q.clubName,clubLogo:Q.clubLogo},text:`🔄 ${Q.firstname} ${Q.name} remplace ${ie.firstname} ${ie.name}`}),x.remove(),Pn(ie,Q,()=>Je(e,t,i))})}document.body.appendChild(x),p()}function Gn(e,t,i,n,d){var _,m;const s=(n.gcDefs||[]).find(c=>c.name===t),r=Oe[t]||{icon:"⚡",desc:"Carte spéciale."},l={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[s==null?void 0:s.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",u={purple:"#b06ce0",light_blue:"#00d4ef"}[s==null?void 0:s.color]||"#b06ce0",a=(s==null?void 0:s.name)||t,o=(s==null?void 0:s.effect)||r.desc,x=s!=null&&s.image_url?`/icons/${s.image_url}`:null,p=r.icon||"⚡",h=document.createElement("div");h.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",h.innerHTML=`
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(s);let l=!1;const u=()=>{l||(l=!0,s.remove(),setTimeout(()=>d(),50))};s.addEventListener("click",u),setTimeout(u,3500)}async function Pt(e,t,i){var o,x;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:n}=i,d=t.homeScore>t.aiScore,s=t.homeScore===t.aiScore,r=d?"victoire":s?"nul":"defaite",l=Ho(t.mode,r);t.matchId&&await T.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:d?n.profile.id:null,home_credits_reward:l,played_at:new Date().toISOString()}).eq("id",t.matchId);const u={credits:(n.profile.credits||0)+l,matches_played:(n.profile.matches_played||0)+1};d?u.wins=(n.profile.wins||0)+1:s?u.draws=(n.profile.draws||0)+1:u.losses=(n.profile.losses||0)+1,await T.from("users").update(u).eq("id",n.profile.id),await i.refreshProfile();const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",a.innerHTML=`
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      ...
      </div>`,(N=document.getElementById("lobby-cancel"))==null||N.addEventListener("click",async()=>{_=!0,m&&(T.removeChannel(m),m=null),ie&&await T.from("friend_match_invites").update({status:"declined"}).eq("id",ie),Ve(e),x("home")})},A=async(K,ne)=>{_=!0,m&&(T.removeChannel(m),m=null),await new Promise(ie=>setTimeout(ie,600)),e.isConnected&&Jn(e,t,K,ne,r,l)},{data:g}=await T.from("friend_match_invites").select("*").eq("status","pending").or(`and(inviter_id.eq.${h},invitee_id.eq.${u}),and(inviter_id.eq.${u},invitee_id.eq.${h})`).order("created_at",{ascending:!1}).limit(1).maybeSingle();let I,M;if(g&&g.inviter_id===u)M=!1,I=g.id,await T.from("friend_match_invites").update({invitee_deck_id:i,invitee_ready:!0,status:"accepted"}).eq("id",I);else{M=!0;const{data:K,error:ne}=await T.from("friend_match_invites").insert({inviter_id:h,invitee_id:u,friend_id:u,inviter_deck_id:i,inviter_ready:!0,status:"pending"}).select().single();if(ne||!K){p("Erreur création invitation","error"),Ve(e),x("home");return}I=K.id}if(c(!0,!M,I),!M){const K={home_id:u,away_id:h,home_deck_id:g.inviter_deck_id,away_deck_id:i,status:"active",mode:"friend"},ne=await T.from("matches").insert(K).select().single(),ie=ne.data;if(!ie){p("Erreur création match: "+(((V=ne.error)==null?void 0:V.message)||""),"error"),Ve(e),x("home");return}await T.from("friend_match_invites").update({match_id:ie.id,status:"matched"}).eq("id",I),A(ie.id,!1);return}let C=!1;const B=K=>{_||C||(K.status==="matched"&&K.match_id?(C=!0,clearInterval(q),clearInterval(R),A(K.match_id,!0)):K.status==="declined"?(clearInterval(q),clearInterval(R),p(`${a} a décliné l'invitation`,"warning"),Ve(e),x("home")):K.invitee_ready&&c(!0,!0,I))},q=setInterval(()=>{if(_){clearInterval(q);return}T.from("matchmaking_queue").delete().eq("user_id",h).then(()=>{},()=>{})},3e3),R=setInterval(async()=>{if(_||C){clearInterval(R);return}const{data:K}=await T.from("friend_match_invites").select("*").eq("id",I).maybeSingle();K&&B(K)},1200);m=T.channel("friend-invite-"+I).on("postgres_changes",{event:"UPDATE",schema:"public",table:"friend_match_invites",filter:`id=eq.${I}`},K=>B(K.new)).subscribe()}async function Jn(e,t,i,n,d=[],s=[]){const{state:r,navigate:l,toast:u}=t,a=n?"p1":"p2",o=n?"p2":"p1",x=(d||[]).map(F=>F.id),p=(d||[]).map(F=>({id:F.id,gc_type:F.gc_type,_gcDef:F._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:h}=await T.from("matches").select("*").eq("id",i).single();if(!h){u("Match introuvable","error"),l("home");return}async function _(){const[{data:F},{data:$},{data:f},{data:y}]=await Promise.all([T.rpc("get_deck_for_match",{p_deck_id:h.home_deck_id}),T.rpc("get_deck_for_match",{p_deck_id:h.away_deck_id}),T.from("users").select("id,pseudo,club_name").eq("id",h.home_id).single(),T.from("users").select("id,pseudo,club_name").eq("id",h.away_id).single()]),b=E=>{const L=Number(E.evolution_bonus)||0;return{cardId:E.card_id,position:E.position,id:E.id,firstname:E.firstname,name:E.surname_encoded,country_code:E.country_code,club_id:E.club_id,job:E.job,job2:E.job2,note_g:(Number(E.note_g)||0)+(E.job==="GK"||E.job2==="GK"&&Number(E.note_g)>0?L:0),note_d:(Number(E.note_d)||0)+(E.job==="DEF"||E.job2==="DEF"&&Number(E.note_d)>0?L:0),note_m:(Number(E.note_m)||0)+(E.job==="MIL"||E.job2==="MIL"&&Number(E.note_m)>0?L:0),note_a:(Number(E.note_a)||0)+(E.job==="ATT"||E.job2==="ATT"&&Number(E.note_a)>0?L:0),evolution_bonus:L,rarity:E.rarity,skin:E.skin,hair:E.hair,hair_length:E.hair_length,clubName:E.club_encoded_name||null,clubLogo:E.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},v=((F==null?void 0:F.starters)||[]).map(E=>b(E)),w=(($==null?void 0:$.starters)||[]).map(E=>b(E)),k=(F==null?void 0:F.formation)||"4-4-2",z=($==null?void 0:$.formation)||"4-4-2";return{p1Team:(()=>{const E=vt(v,k);return stadiumDef?Tt(E,stadiumDef):E})(),p2Team:vt(w,z),p1Subs:(()=>{const E=((F==null?void 0:F.subs)||[]).map(L=>b(L));return stadiumDef&&jt(E,stadiumDef),E})(),p2Subs:(($==null?void 0:$.subs)||[]).map(E=>b(E)),p1Formation:k,p2Formation:z,p1Name:(f==null?void 0:f.club_name)||(f==null?void 0:f.pseudo)||"Joueur 1",p2Name:(y==null?void 0:y.club_name)||(y==null?void 0:y.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:n?x:[],gc_p2:n?[]:x,gcCardsFull_p1:n?p:[],gcCardsFull_p2:n?[]:p,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:s||[],lastActionAt:new Date().toISOString()}}let m=h.game_state&&Object.keys(h.game_state).length?h.game_state:null;if(!m)if(n){m=await _();const{data:F}=await T.from("matches").select("game_state").eq("id",i).single();!(F!=null&&F.game_state)||!Object.keys(F.game_state).length?await T.from("matches").update({game_state:m,turn_user_id:h.home_id}).eq("id",i):m=F.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let F=0;F<60&&!m;F++){await new Promise(f=>setTimeout(f,400));const{data:$}=await T.from("matches").select("game_state").eq("id",i).single();$!=null&&$.game_state&&Object.keys($.game_state).length&&(m=$.game_state)}if(!m){u("Erreur de synchronisation","error"),l("home");return}m.gc_p2=x,m.gcCardsFull_p2=p,await T.from("matches").update({game_state:m}).eq("id",i)}let c=!1,A=!1,g=!1,I=null,M=!1;const C=new Set,B=new Set;function q(F){var E,L;try{T.removeChannel(R)}catch{}const $=m[a+"Score"]||0,f=m[o+"Score"]||0;if(!g){g=!0;const S=F.winner_id||($>f?r.profile.id:f>$?"opp":null),j=S===r.profile.id?"win":S?"loss":null;j&&ni(()=>Promise.resolve().then(()=>oo),void 0).then(D=>D.applyOwnEvolution(r.profile.id,j)).catch(()=>{})}if(!A){const S=m.p1Score||0,j=m.p2Score||0,D=(m.usedGc_p1||[]).length,Y=(m.usedGc_p2||[]).length,J=F.winner_id||(S>j?h.home_id:j>S?h.away_id:null);(J?r.profile.id===J:r.profile.id<(n?h.away_id:h.home_id))&&(A=!0,Ji({player1Id:h.home_id,player2Id:h.away_id,score1:S,score2:j,gc1:D,gc2:Y}).catch(G=>console.warn("[FriendMatch] updateStats:",G)))}let y,b;F.winner_id?(y=F.winner_id===r.profile.id,b=!1):F.forfeit?(y=$>f,b=!1):(b=$===f,y=$>f),(E=document.getElementById("pvp-end-overlay"))==null||E.remove();const v=document.createElement("div");v.id="pvp-end-overlay",v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const w=b?"🤝":y?"🏆":"😞",k=b?"MATCH NUL":y?"VICTOIRE !":"DÉFAITE",z=b?"#fff":y?"#FFD700":"#ff6b6b";v.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(v),(L=v.querySelector("#pvp-end-home"))==null||L.addEventListener("click",()=>{v.remove(),Ve(e),l("home")})}const R=T.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},F=>{const $=F.new;try{if($.status==="finished"||$.forfeit){if(c)return;c=!0,I&&(clearInterval(I),I=null),$.game_state&&(m=$.game_state),q($);return}if($.game_state){const f=m;m=$.game_state;const y=m._lastGC;if(y&&y.seq&&!B.has(y.seq)&&(B.add(y.seq),y.by!==a)){Ee(y.type,y.by,()=>re());return}const b=f[a+"Score"]||0,v=f[o+"Score"]||0,w=m[a+"Score"]||0,k=m[o+"Score"]||0,z=w>b,E=k>v;if((z||E)&&!C.has(m.round)){C.add(m.round);const L=[...m.log||[]].reverse().find(j=>j.isGoal),S=((L==null?void 0:L.homePlayers)||[]).map(j=>({name:j.name,note:j.note,portrait:j.portrait,job:j.job}));ve(S,w,k,z,()=>re());return}re()}}catch(f){console.error("[PvP] crash:",f)}}).subscribe();async function V(F){Object.assign(m,F),m.lastActionAt=new Date().toISOString();const{error:$}=await T.from("matches").update({game_state:m}).eq("id",i);$&&u("Erreur de synchronisation","error");try{re()}catch(f){console.error("[PvP] renderPvpScreen crash:",f)}}async function K(){if(c)return;c=!0,I&&(clearInterval(I),I=null);const F=n?h.away_id:h.home_id,$=n?"p2":"p1",f=n?"p1":"p2",y={...m,[$+"Score"]:3,[f+"Score"]:0,phase:"finished"};try{await T.from("matches").update({status:"finished",forfeit:!0,winner_id:F,home_score:y.p1Score||0,away_score:y.p2Score||0,game_state:y}).eq("id",i)}catch{}try{T.removeChannel(R)}catch{}setTimeout(()=>{Ve(e),l("home")},800)}const ne={BOOST_STAT:({value:F=1,count:$=1,roles:f=[]},y,b)=>{const v=y[a+"Team"],w=Object.entries(v).filter(([k])=>!f.length||f.includes(k)).flatMap(([k,z])=>z.filter(E=>!E.used).map(E=>({...E,_line:k})));if(!w.length){y.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),b(y);return}ie(w,$,`Choisir ${$} joueur(s) à booster (+${F})`,k=>{k.forEach(z=>{const E=(v[z._line]||[]).find(L=>L.cardId===z.cardId);E&&(E.boost=(E.boost||0)+F,y.log.push({text:`⚡ +${F} sur ${E.name}`,type:"info"}))}),y[a+"Team"]=v,b(y)})},DEBUFF_STAT:({value:F=1,count:$=1,roles:f=[],target:y="ai"},b,v)=>{const w=y==="home"?a:o,k=b[w+"Team"],z=y==="home"?"allié":"adverse",E=Object.entries(k).filter(([L])=>!f.length||f.includes(L)).flatMap(([L,S])=>S.map(j=>({...j,_line:L})));if(!E.length){b.log.push({text:`🎯 Aucun joueur ${z}`,type:"info"}),v(b);return}ie(E,$,`Choisir ${$} joueur(s) ${z}(s) (-${F})`,L=>{L.forEach(S=>{const j=(k[S._line]||[]).find(D=>D.cardId===S.cardId);j&&(j.boost=(j.boost||0)-F,b.log.push({text:`🎯 -${F} sur ${j.name}`,type:"info"}))}),b[w+"Team"]=k,v(b)})},GRAY_PLAYER:({count:F=1,roles:$=[],target:f="ai"},y,b)=>{const v=f==="home"?a:o,w=y[v+"Team"],k=f==="home"?"allié":"adverse",z=Object.entries(w).filter(([E])=>!$.length||$.includes(E)).flatMap(([E,L])=>L.filter(S=>!S.used).map(S=>({...S,_line:E})));if(!z.length){y.log.push({text:`❌ Aucun joueur ${k}`,type:"info"}),b(y);return}ie(z,F,`Choisir ${F} joueur(s) ${k}(s) à exclure`,E=>{const L="usedCardIds_"+v,S=new Set(y[L]||[]);E.forEach(j=>{const D=(w[j._line]||[]).find(Y=>Y.cardId===j.cardId);D&&(D.used=!0,S.add(j.cardId),y.log.push({text:`❌ ${D.name} exclu !`,type:"info"}))}),y[L]=[...S],y[v+"Team"]=w,b(y)})},REVIVE_PLAYER:({count:F=1,roles:$=[]},f,y)=>{const b=f[a+"Team"],v=Object.entries(b).filter(([w])=>!$.length||$.includes(w)).flatMap(([w,k])=>k.filter(z=>z.used).map(z=>({...z,_line:w})));if(!v.length){f.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),y(f);return}ie(v,F,`Choisir ${F} joueur(s) à ressusciter`,w=>{w.forEach(k=>{const z=(b[k._line]||[]).find(E=>E.cardId===k.cardId);z&&(z.used=!1,f.log.push({text:`💫 ${z.name} ressuscité !`,type:"info"}))}),f[a+"Team"]=b,y(f)})},REMOVE_GOAL:({},F,$)=>{const f=o+"Score";F[f]>0?(F[f]--,F.log.push({text:"🚫 Dernier but annulé !",type:"info"})):F.log.push({text:"🚫 Aucun but à annuler",type:"info"}),$(F)},ADD_GOAL_DRAW:({},F,$)=>{F[a+"Score"]===F[o+"Score"]?(F[a+"Score"]++,F.log.push({text:"🎯 But bonus !",type:"info"})):F.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),$(F)},ADD_SUB:({value:F=1},$,f)=>{$.maxSubs=($.maxSubs||3)+F,$.log.push({text:`🔄 +${F} remplacement(s)`,type:"info"}),f($)},CUSTOM:({},F,$)=>$(F)};function ie(F,$,f,y){const b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let v=[];function w(){var z,E;const k=F.map(L=>{const S={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[L._line]||"#555",j=Se(L,L._line)+(L.boost||0),Y=v.find(U=>U.cardId===L.cardId)?"#FFD700":"rgba(255,255,255,0.25)",J=L.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${L.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${Y};background:${S};overflow:hidden;cursor:pointer;${J}">
        ...
        </div>`,(z=b.querySelector("#pp-close"))==null||z.addEventListener("click",()=>b.remove()),b.querySelectorAll(".pp-item").forEach(L=>{L.addEventListener("click",()=>{const S=L.dataset.cid,j=F.find(Y=>Y.cardId===S),D=v.findIndex(Y=>Y.cardId===S);j&&(D>-1?v.splice(D,1):v.length<$&&v.push(j),w())})}),(E=b.querySelector("#pp-confirm"))==null||E.addEventListener("click",()=>{b.remove(),y(v)})}w(),document.body.appendChild(b)}async function Q(F,$){const y=(m["gcCardsFull_"+a]||[]).find(k=>k.id===F),b=(y==null?void 0:y._gcDef)||(m.gcDefs||[]).find(k=>{var z;return k.name===$||((z=k.name)==null?void 0:z.toLowerCase().trim())===($==null?void 0:$.toLowerCase().trim())}),v=[...m["usedGc_"+a]||[],F],w={type:$,by:a,seq:(m._gcAnimSeq||0)+1};B.add(w.seq),Ee($,a,async()=>{if(b!=null&&b.effect_type&&b.effect_type!=="CUSTOM"){const z=ne[b.effect_type];if(z){const E={...m};z(b.effect_params||{},E,async L=>{L["usedGc_"+a]=v,L._lastGC=w,L._gcAnimSeq=w.seq,await V(L)});return}}const k={...m};switch($){case"Remplacement+":k.maxSubs=(k.maxSubs||3)+1,k.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const z=o+"Score";k[z]>0&&(k[z]--,k.log.push({text:"🚫 But annulé",type:"info"}));break}}k["usedGc_"+a]=v,k._lastGC=w,k._gcAnimSeq=w.seq,await V(k)})}function N(F,$){const f="usedCardIds_"+F,y=new Set(m[f]||[]);$.forEach(b=>y.add(b)),m[f]=[...y]}function P(){for(const F of["p1","p2"]){const $=new Set(m["usedCardIds_"+F]||[]),f=m[F+"Team"];if(f)for(const y of["GK","DEF","MIL","ATT"])(f[y]||[]).forEach(b=>{b.used=$.has(b.cardId)})}}function re(){var Ge,We,ot,ut,ft,gt;if(m.phase==="reveal")return xe();if(m.phase==="midfield")return we();if(m.phase==="finished")return ze();const F=m[a+"Team"],$=m[o+"Team"];P();const f=m[a+"Score"],y=m[o+"Score"],b=m[a+"Name"],v=m[o+"Name"],w=m.phase===a+"-attack",k=m.phase===a+"-defense",z=Array.isArray(m["selected_"+a])?m["selected_"+a]:[],E=z.map(le=>le.cardId),L=window.innerWidth>=700,S=m[a+"Subs"]||[],j=m["usedSubIds_"+a]||[],D=S.filter(le=>!j.includes(le.cardId)),Y=m["gcCardsFull_"+a]||[],J=m["usedGc_"+a]||[],U=Y.filter(le=>!J.includes(le.id)),G=m.boostOwner===a&&!m.boostUsed,W=!["GK","DEF","MIL","ATT"].some(le=>($[le]||[]).some(X=>!X.used)),te=[...F.MIL||[],...F.ATT||[]].filter(le=>!le.used),ee=w&&W&&te.length===0?[...F.GK||[],...F.DEF||[]].filter(le=>!le.used).map(le=>le.cardId):[];function fe(le,X,O){var xt,kt;const ae=le._gcDef,he={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ae==null?void 0:ae.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",$e={purple:"#b06ce0",light_blue:"#00d4ef"}[ae==null?void 0:ae.color]||"#b06ce0",Qe=(ae==null?void 0:ae.name)||le.gc_type,Ze=(ae==null?void 0:ae.effect)||((xt=Oe[le.gc_type])==null?void 0:xt.desc)||"",tt=ae!=null&&ae.image_url?`/icons/${ae.image_url}`:null,rt=((kt=Oe[le.gc_type])==null?void 0:kt.icon)||"⚡",_t=Math.round(O*.22),$t=Math.round(O*.22),lt=O-_t-$t,mt=Qe.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${le.id}" data-gc-type="${le.gc_type}"
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${pt(F,m[a+"Formation"],Ae,E,300,300,ee)}
      </div>
      ...
      </div>`;function Re(){const le=e.querySelector(".match-screen");if(!le)return;const X=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);le.style.height=X+"px",le.style.minHeight=X+"px",le.style.maxHeight=X+"px",le.style.overflow="hidden";const O=e.querySelector("#mobile-action-bar"),ae=e.querySelector("#mobile-play-area");O&&ae&&(ae.style.paddingBottom=O.offsetHeight+"px")}if(Re(),setTimeout(Re,120),setTimeout(Re,400),M||(M=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",Re),window.visualViewport.addEventListener("scroll",Re)),window.addEventListener("resize",Re)),function(){const X=e.querySelector(".terrain-wrapper svg");X&&(X.removeAttribute("width"),X.removeAttribute("height"),X.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",X.setAttribute("viewBox","-26 -26 352 352"),X.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),e.querySelectorAll(".match-slot-hit").forEach(le=>{le.addEventListener("click",()=>{if(!w&&!k)return;const X=le.dataset.cardId,O=le.dataset.role,ae=(F[O]||[]).find(Ze=>Ze.cardId===X);if(!ae||ae.used)return;const he=ee.includes(X);if(w&&!["MIL","ATT"].includes(O)&&!he)return;Array.isArray(m["selected_"+a])||(m["selected_"+a]=[]);const $e=m["selected_"+a],Qe=$e.findIndex(Ze=>Ze.cardId===X);Qe>-1?$e.splice(Qe,1):$e.length<3&&$e.push({...ae,_role:O}),re()})}),e.querySelectorAll(".match-used-hit").forEach(le=>{le.addEventListener("click",()=>ce(le.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(le=>{le.addEventListener("click",()=>ce())}),(Ge=e.querySelector("#pvp-sub-open"))==null||Ge.addEventListener("click",()=>ce()),e.querySelectorAll(".pvp-gc-mini").forEach(le=>{le.addEventListener("click",()=>se(le.dataset.gcId,le.dataset.gcType))}),(We=e.querySelector("#pvp-boost-card"))==null||We.addEventListener("click",()=>pe()),(ot=e.querySelector("#pvp-action"))==null||ot.addEventListener("click",le=>{w?le.currentTarget.dataset.pass==="1"||!Ce(a)?de():me():k&&oe()}),(ut=e.querySelector("#pvp-quit"))==null||ut.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&K()}),(ft=e.querySelector("#pvp-view-opp"))==null||ft.addEventListener("click",()=>Z()),(gt=e.querySelector("#pvp-toggle-history"))==null||gt.addEventListener("click",()=>ge()),I&&(clearInterval(I),I=null),(w||k)&&!c){let le=30,X=!1;const O=()=>document.getElementById("pvp-timer"),ae=()=>{O()&&(O().textContent=le+"s",O().style.color=X?"#ff4444":"#fff")};ae(),I=setInterval(()=>{le--,le<0?X?(clearInterval(I),I=null,w&&!Ce(a)?de():K()):(X=!0,le=15,ae()):ae()},1e3)}}function xe(){et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
    ...
    </div>`,a==="p1"&&setTimeout(async()=>{await V({phase:"midfield"})},5e3)}let _e=!1;function we(){if(_e)return;const F=m[a+"Team"].MIL||[],$=m[o+"Team"].MIL||[];function f(U){return U.reduce((G,W)=>G+Se(W,"MIL"),0)}function y(U){let G=0;for(let W=0;W<U.length-1;W++){const te=nt(U[W],U[W+1]);te==="#00ff88"?G+=2:te==="#FFD700"&&(G+=1)}return G}const b=f(F)+y(F),v=f($)+y($),w=b>=v;function k(U,G,W){return`<div id="duel-row-${W}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(v);let z=!1;const E=()=>{z||(z=!0,v.remove(),setTimeout(()=>b(),50))};v.addEventListener("click",E),setTimeout(E,3500)}function Ee(F,$,f){var J,U;const y=(m.gcDefs||[]).find(G=>{var W;return G.name===F||((W=G.name)==null?void 0:W.toLowerCase().trim())===(F==null?void 0:F.toLowerCase().trim())}),b={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[y==null?void 0:y.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",v={purple:"#b06ce0",light_blue:"#00d4ef"}[y==null?void 0:y.color]||"#b06ce0",w=(y==null?void 0:y.name)||F,k=(y==null?void 0:y.effect)||((J=Oe[F])==null?void 0:J.desc)||"",z=y!=null&&y.image_url?`/icons/${y.image_url}`:null,E=((U=Oe[F])==null?void 0:U.icon)||"⚡",S=$===a?"Vous":m[$+"Name"]||"Adversaire",j=document.createElement("div");j.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",j.innerHTML=`
      <style>
        @keyframes gcFlipIn{0%{transform:perspective(800px) rotateY(90deg) scale(.7);opacity:0}55%{transform:perspective(800px) rotateY(-12deg) scale(1.08);opacity:1}100%{transform:perspective(800px) rotateY(0) scale(1);opacity:1}}
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        ${Array.from({length:Math.max(0,r.status==="active"?r.current_day-1:r.current_day)},(Q,N)=>N+1).reverse().map(Q=>{const N=(u||[]).filter(P=>P.matchday===Q);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:#888;margin-bottom:6px">Journée ${Q}</div>${N.map(P=>Hi(P,x,s,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
    ...
    <div style="background:linear-gradient(160deg,#1a0a2e,#3b1a6e);border-radius:20px;padding:32px 24px;max-width:320px;width:100%;text-align:center;color:#fff;box-shadow:0 8px 40px rgba(0,0,0,0.6)">
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">🏆 Retour à la Mini League</button>`,document.body.appendChild(k),(j=k.querySelector("#pvp-end-home"))==null||j.addEventListener("click",()=>{k.remove(),Ve(e),a?l("mini-league",{openLeagueId:a}):l("mini-league")})}const V=T.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},f=>{const y=f.new;try{if(y.status==="finished"||y.forfeit){if(A||(A=!0,M&&(clearInterval(M),M=null),y.game_state&&(c=y.game_state),c.phase==="finished"&&!y.forfeit&&document.getElementById("pvp-home")))return;R(y);return}if(y.game_state){const b=c;c=y.game_state;const v=c._lastGC;if(v&&v.seq&&!q.has(v.seq)&&(q.add(v.seq),v.by!==o)){se(v.type,v.by,()=>xe());return}const w=b[o+"Score"]||0,k=b[x+"Score"]||0,z=c[o+"Score"]||0,E=c[x+"Score"]||0,L=z>w,S=E>k;if((L||S)&&!B.has(c.round)){B.add(c.round);const j=[...c.log||[]].reverse().find(Y=>Y.isGoal),D=((j==null?void 0:j.homePlayers)||[]).map(Y=>({name:Y.name,note:Y.note,portrait:Y.portrait,job:Y.job}));Ee(D,z,E,L,()=>xe());return}xe()}}catch(b){console.error("[PvP] crash:",b)}}).subscribe();async function K(f){Object.assign(c,f),c.lastActionAt=new Date().toISOString();const{error:y}=await T.from("matches").update({game_state:c}).eq("id",i);y&&u("Erreur de synchronisation","error");try{xe()}catch(b){console.error("[PvP] renderPvpScreen crash:",b)}}async function ne(){if(A)return;A=!0,M&&(clearInterval(M),M=null);const f=n?_.away_id:_.home_id,y=n?"p2":"p1",b=n?"p1":"p2",v={...c,[y+"Score"]:3,[b+"Score"]:0,phase:"finished"};try{await T.from("matches").update({status:"finished",forfeit:!0,winner_id:f,home_score:v.p1Score||0,away_score:v.p2Score||0,game_state:v}).eq("id",i)}catch{}try{T.removeChannel(V)}catch{}setTimeout(()=>{Ve(e),l("mini-league")},800)}const ie={BOOST_STAT:({value:f=1,count:y=1,roles:b=[]},v,w)=>{const k=v[o+"Team"],z=Object.entries(k).filter(([E])=>!b.length||b.includes(E)).flatMap(([E,L])=>L.filter(S=>!S.used).map(S=>({...S,_line:E})));if(!z.length){v.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),w(v);return}Q(z,y,`Choisir ${y} joueur(s) à booster (+${f})`,E=>{E.forEach(L=>{const S=(k[L._line]||[]).find(j=>j.cardId===L.cardId);S&&(S.boost=(S.boost||0)+f,v.log.push({text:`⚡ +${f} sur ${S.name}`,type:"info"}))}),v[o+"Team"]=k,w(v)})},DEBUFF_STAT:({value:f=1,count:y=1,roles:b=[],target:v="ai"},w,k)=>{const z=v==="home"?o:x,E=w[z+"Team"],L=v==="home"?"allié":"adverse",S=Object.entries(E).filter(([j])=>!b.length||b.includes(j)).flatMap(([j,D])=>D.map(Y=>({...Y,_line:j})));if(!S.length){w.log.push({text:`🎯 Aucun joueur ${L}`,type:"info"}),k(w);return}Q(S,y,`Choisir ${y} joueur(s) ${L}(s) (-${f})`,j=>{j.forEach(D=>{const Y=(E[D._line]||[]).find(J=>J.cardId===D.cardId);Y&&(Y.boost=(Y.boost||0)-f,w.log.push({text:`🎯 -${f} sur ${Y.name}`,type:"info"}))}),w[z+"Team"]=E,k(w)})},GRAY_PLAYER:({count:f=1,roles:y=[],target:b="ai"},v,w)=>{const k=b==="home"?o:x,z=v[k+"Team"],E=b==="home"?"allié":"adverse",L=Object.entries(z).filter(([S])=>!y.length||y.includes(S)).flatMap(([S,j])=>j.filter(D=>!D.used).map(D=>({...D,_line:S})));if(!L.length){v.log.push({text:`❌ Aucun joueur ${E}`,type:"info"}),w(v);return}Q(L,f,`Choisir ${f} joueur(s) ${E}(s) à exclure`,S=>{const j="usedCardIds_"+k,D=new Set(v[j]||[]);S.forEach(Y=>{const J=(z[Y._line]||[]).find(U=>U.cardId===Y.cardId);J&&(J.used=!0,D.add(Y.cardId),v.log.push({text:`❌ ${J.name} exclu !`,type:"info"}))}),v[j]=[...D],v[k+"Team"]=z,w(v)})},REVIVE_PLAYER:({count:f=1,roles:y=[]},b,v)=>{const w=b[o+"Team"],k=Object.entries(w).filter(([z])=>!y.length||y.includes(z)).flatMap(([z,E])=>E.filter(L=>L.used).map(L=>({...L,_line:z})));if(!k.length){b.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),v(b);return}Q(k,f,`Choisir ${f} joueur(s) à ressusciter`,z=>{z.forEach(E=>{const L=(w[E._line]||[]).find(S=>S.cardId===E.cardId);L&&(L.used=!1,b.log.push({text:`💫 ${L.name} ressuscité !`,type:"info"}))}),b[o+"Team"]=w,v(b)})},REMOVE_GOAL:({},f,y)=>{const b=x+"Score";f[b]>0?(f[b]--,f.log.push({text:"🚫 Dernier but annulé !",type:"info"})):f.log.push({text:"🚫 Aucun but à annuler",type:"info"}),y(f)},ADD_GOAL_DRAW:({},f,y)=>{f[o+"Score"]===f[x+"Score"]?(f[o+"Score"]++,f.log.push({text:"🎯 But bonus !",type:"info"})):f.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),y(f)},ADD_SUB:({value:f=1},y,b)=>{y.maxSubs=(y.maxSubs||3)+f,y.log.push({text:`🔄 +${f} remplacement(s)`,type:"info"}),b(y)},CUSTOM:({},f,y)=>y(f)};function Q(f,y,b,v){const w=document.createElement("div");w.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let k=[];function z(){var L,S;const E=f.map(j=>{const D={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[j._line]||"#555",Y=Se(j,j._line)+(j.boost||0),U=k.find(W=>W.cardId===j.cardId)?"#FFD700":"rgba(255,255,255,0.25)",G=j.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${j.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${U};background:${D};overflow:hidden;cursor:pointer;${G}">
        ...
        </div>`,(L=w.querySelector("#pp-close"))==null||L.addEventListener("click",()=>w.remove()),w.querySelectorAll(".pp-item").forEach(j=>{j.addEventListener("click",()=>{const D=j.dataset.cid,Y=f.find(U=>U.cardId===D),J=k.findIndex(U=>U.cardId===D);Y&&(J>-1?k.splice(J,1):k.length<y&&k.push(Y),z())})}),(S=w.querySelector("#pp-confirm"))==null||S.addEventListener("click",()=>{w.remove(),v(k)})}z(),document.body.appendChild(w)}async function N(f,y){const v=(c["gcCardsFull_"+o]||[]).find(E=>E.id===f),w=(v==null?void 0:v._gcDef)||(c.gcDefs||[]).find(E=>{var L;return E.name===y||((L=E.name)==null?void 0:L.toLowerCase().trim())===(y==null?void 0:y.toLowerCase().trim())}),k=[...c["usedGc_"+o]||[],f],z={type:y,by:o,seq:(c._gcAnimSeq||0)+1};q.add(z.seq),se(y,o,async()=>{if(w!=null&&w.effect_type&&w.effect_type!=="CUSTOM"){const L=ie[w.effect_type];if(L){const S={...c};L(w.effect_params||{},S,async j=>{j["usedGc_"+o]=k,j._lastGC=z,j._gcAnimSeq=z.seq,await K(j)});return}}const E={...c};switch(y){case"Remplacement+":E.maxSubs=(E.maxSubs||3)+1,E.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const L=x+"Score";E[L]>0&&(E[L]--,E.log.push({text:"🚫 But annulé",type:"info"}));break}}E["usedGc_"+o]=k,E._lastGC=z,E._gcAnimSeq=z.seq,await K(E)})}function P(f,y){const b="usedCardIds_"+f,v=new Set(c[b]||[]);y.forEach(w=>v.add(w)),c[b]=[...v]}function re(){for(const f of["p1","p2"]){const y=new Set(c["usedCardIds_"+f]||[]),b=c[f+"Team"];if(b)for(const v of["GK","DEF","MIL","ATT"])(b[v]||[]).forEach(w=>{w.used=y.has(w.cardId)})}}function xe(){var ot,ut,ft,gt,le,X;if(c.phase==="reveal")return _e();if(c.phase==="midfield")return ve();if(c.phase==="finished")return F();const f=c[o+"Team"],y=c[x+"Team"];re();const b=c[o+"Score"],v=c[x+"Score"],w=c[o+"Name"],k=c[x+"Name"],z=c.phase===o+"-attack",E=c.phase===o+"-defense",L=Array.isArray(c["selected_"+o])?c["selected_"+o]:[],S=L.map(O=>O.cardId),j=window.innerWidth>=700,D=c[o+"Subs"]||[],Y=c["usedSubIds_"+o]||[],J=D.filter(O=>!Y.includes(O.cardId)),U=c["gcCardsFull_"+o]||[],G=c["usedGc_"+o]||[],W=U.filter(O=>!G.includes(O.id)),te=c.boostOwner===o&&!c.boostUsed,ee=!["GK","DEF","MIL","ATT"].some(O=>(y[O]||[]).some(ae=>!ae.used)),fe=[...f.MIL||[],...f.ATT||[]].filter(O=>!O.used),ye=z&&ee&&fe.length===0?[...f.GK||[],...f.DEF||[]].filter(O=>!O.used).map(O=>O.cardId):[];function be(O,ae,he){var Ct,Li;const $e=O._gcDef,Qe={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[$e==null?void 0:$e.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",Ze={purple:"#b06ce0",light_blue:"#00d4ef"}[$e==null?void 0:$e.color]||"#b06ce0",tt=($e==null?void 0:$e.name)||O.gc_type,rt=($e==null?void 0:$e.effect)||((Ct=Oe[O.gc_type])==null?void 0:Ct.desc)||"",_t=$e!=null&&$e.image_url?`/icons/${$e.image_url}`:null,$t=((Li=Oe[O.gc_type])==null?void 0:Li.icon)||"⚡",lt=Math.round(he*.22),mt=Math.round(he*.22),xt=he-lt-mt,kt=tt.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${O.id}" data-gc-type="${O.gc_type}"
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div id="mkt-content" style="padding:10px 16px;display:flex;flex-direction:column;gap:8px"></div>
  </div>`;let u="buy";const a=()=>{var m,c,A,g,I,M,C;return{name:(((m=document.getElementById("flt-name"))==null?void 0:m.value)||"").toLowerCase().trim(),club:(((c=document.getElementById("flt-club"))==null?void 0:c.value)||"").toLowerCase().trim(),country:(((A=document.getElementById("flt-country"))==null?void 0:A.value)||"").toLowerCase().trim(),job:((g=document.getElementById("flt-job"))==null?void 0:g.value)||"",rarity:((I=document.getElementById("flt-rarity"))==null?void 0:I.value)||"",note1:parseInt((M=document.getElementById("flt-note1"))==null?void 0:M.value)||0,note2:parseInt((C=document.getElementById("flt-note2"))==null?void 0:C.value)||0}};function o(m){const c=a();return m.filter(A=>{var V,K,ne;const g=(V=A.card)==null?void 0:V.player;if(!g)return!1;const I=`${g.firstname} ${g.surname_encoded}`.toLowerCase(),M=(((K=g.clubs)==null?void 0:K.encoded_name)||"").toLowerCase(),C=(g.country_code||"").toLowerCase(),B=((ne=A.card)==null?void 0:ne.evolution_bonus)||0,q=St(g,g.job,B),R=g.job2?St(g,g.job2,B):0;return!(c.name&&!I.includes(c.name)||c.club&&!M.includes(c.club)||c.country&&!C.includes(c.country)||c.job&&g.job!==c.job||c.rarity&&g.rarity!==c.rarity||c.note1&&q<c.note1||c.note2&&R<c.note2)})}function x(m){var R,V,K,ne;const c=(R=m.card)==null?void 0:R.player;if(!c)return"";const A=((V=m.card)==null?void 0:V.evolution_bonus)||0,g=St(c,c.job,A),I=c.job2?St(c,c.job2,A):0,M=(i.profile.credits||0)>=m.price,C=c.country_code?`https://flagsapi.com/${c.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,B=Qt[c.job]||"#bbb",q=c.job2?Qt[c.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden">
      <!-- Drapeau -->
      ...
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${B};display:flex;align-items:center;justify-content:center">
          <span style="font-size:14px;font-weight:900;color:${B};font-family:Arial Black,Arial">${g}</span>
        </div>
      ...
      <div style="text-align:right;flex-shrink:0">
        ...
        <button class="btn btn-primary btn-sm" data-buy="${m.id}" ${M?"":"disabled"} style="margin-top:4px;font-size:11px;padding:4px 10px">${M?"Acheter":"Trop cher"}</button>
      </div>
    </div>`}function p(m){var R,V,K,ne;const c=(R=m.card)==null?void 0:R.player;if(!c)return"";const A=((V=m.card)==null?void 0:V.evolution_bonus)||0,g=St(c,c.job,A),I=c.job2?St(c,c.job2,A):0,M=m.status==="sold",C=c.country_code?`https://flagsapi.com/${c.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,B=Qt[c.job]||"#bbb",q=c.job2?Qt[c.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${M?"opacity:0.7":""}">
      ${C?`<img src="${C}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      ${(K=c.clubs)!=null&&K.logo_url?`<img src="${c.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${B};display:flex;align-items:center;justify-content:center">
    ...
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center">
      ...
      `).join(""):'<div style="text-align:center;color:var(--gray-600);padding:40px">Aucun manager.</div>'}else{const A=r||[];_.innerHTML=A.length>0?A.map((g,I)=>{const M=Nt(g.mmr??1e3),C=(g.ranked_wins||0)+(g.ranked_losses||0)+(g.ranked_draws||0),B=C>0?Math.round((g.ranked_wins||0)/C*100):0,q=g.id===i.profile.id,R=(g.placement_matches||0)<10;return`
          <div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${q?"border:2px solid var(--yellow)":""}">
            ...
            <div style="flex:1;min-width:0">
              ...
              <div style="font-size:11px;color:var(--gray-600)">${g.club_name} · ${M.label}</div>
            </div>
            <div style="text-align:right;flex-shrink:0">
              <div style="font-size:20px">${R?"❓":M.emoji}</div>
              <div style="font-size:11px;font-weight:700;color:${M.color}">${R?"Placement":M.label}</div>
              ${R?"":`<div style="font-size:10px;color:var(--gray-600)">${B}% WR</div>`}
            </div>
  ...
  <div style="min-height:100%;background:${g[_.id]};padding:16px;overflow-y:auto;display:flex;flex-direction:column;gap:16px">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:8px">
      ...
      <div style="background:rgba(0,0,0,0.3);border-radius:12px;padding:10px;text-align:center">
        <div style="font-size:20px;font-weight:900;color:#D4A017">${M}%</div>
