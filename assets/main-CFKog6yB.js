import{s as $,l as U,F as ie,b as we,c as ye,d as rt}from"./formation-links-CDBKB_z4.js";function ze(t,{navigate:e,toast:i}){t.innerHTML=`
    ...
    <div class="page-body">
      ...
      <div>
        ...
        <div class="card-panel" style="padding:0">
          ${n.map(r=>{const d=r.winner_id===a.id,c=r.home_score===r.away_score,p=c?"N":d?"V":"D",f=c?"#888":d?"#1A6B3C":"#c0392b",u={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat"}[r.mode]||r.mode,w=new Date(r.created_at).toLocaleDateString("fr",{month:"short",day:"numeric"});return`<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 14px;border-bottom:1px solid var(--gray-200)">
              <div>
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
  ...
  </div>`}async function ut(t,e){const{state:i,navigate:o,toast:a,openModal:n,closeModal:s}=e;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:l}=await $.from("cards").select(`id, card_type, current_note, gc_type, formation, is_for_sale, sale_price,
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),d=(l||[]).filter(I=>I.card_type==="player"&&I.player),c=(l||[]).filter(I=>I.card_type==="game_changer"),p=(l||[]).filter(I=>I.card_type==="formation"),f=Object.keys(ie),u=Object.keys(oe),m={};d.forEach(I=>{const L=I.player.id;m[L]=(m[L]||0)+1});const w=new Set(Object.keys(m).map(I=>String(I))),h=new Set(p.map(I=>I.formation)),E=new Set(c.map(I=>I.gc_type));let b="player",g="Tous",x="",v=!1;function y(){return[...d].sort((I,L)=>{const M=ae.indexOf(I.player.job),B=ae.indexOf(L.player.job);return M!==B?M-B:(I.player.surname_encoded||"").localeCompare(L.player.surname_encoded||"")})}function k(){return[...r||[]].sort((I,L)=>{const M=ae.indexOf(I.job),B=ae.indexOf(L.job);return M!==B?M-B:(I.surname_encoded||"").localeCompare(L.surname_encoded||"")})}function _(){return y().filter(I=>{const L=I.player,M=g==="Tous"||L.job===g,B=!x||`${L.firstname} ${L.surname_encoded}`.toLowerCase().includes(x);return M&&B})}function T(){return k().filter(I=>{const L=g==="Tous"||I.job===g,M=!x||`${I.firstname} ${I.surname_encoded}`.toLowerCase().includes(x);return L&&M})}t.innerHTML=`
  <div class="page">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const ee=1e3;function yt(t,e,i,o){var w,h,E;const{state:a,toast:n,closeModal:s,navigate:l,refreshProfile:r}=i,d=t.formation,c={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function p(){const b=we[d]||{},g=ie[d]||[],x=290,v=360,y=20;function k(T){const A=b[T];return A?{x:A.x*x,y:A.y*v}:null}let _=`<svg width="${x}" height="${v}" viewBox="0 0 ${x} ${v}" xmlns="http://www.w3.org/2000/svg">`;for(const[T,A]of g){const z=k(T),C=k(A);!z||!C||(_+=`<line x1="${z.x}" y1="${z.y}" x2="${C.x}" y2="${C.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const T of Object.keys(b)){const A=k(T);if(!A)continue;const z=T.replace(/\d+/,""),C=c[z]||"#555";_+=`<circle cx="${A.x}" cy="${A.y}" r="${y}" fill="${C}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,_+=`<text x="${A.x}" y="${A.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${z}</text>`}return _+="</svg>",_}const f=e.filter(b=>b.formation===d),u=f.length,m=!t.is_for_sale;o(`Formation ${d}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(w=document.getElementById("direct-sell-form-btn"))==null||w.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${d} pour ${ee.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const b=f.find(x=>!x.is_for_sale)||f[0];if(!b){n("Aucune carte à vendre","error");return}const{error:g}=await $.from("cards").delete().eq("id",b.id);if(g){n(g.message,"error");return}await $.from("users").update({credits:(a.profile.credits||0)+ee}).eq("id",a.profile.id),await r(),n(`+${ee.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),s(),l("collection")}),(h=document.getElementById("market-sell-form-btn"))==null||h.addEventListener("click",async()=>{const b=parseInt(document.getElementById("sell-price-form").value);if(!b||b<1){n("Prix invalide","error");return}await $.from("cards").update({is_for_sale:!0,sale_price:b}).eq("id",t.id),await $.from("market_listings").insert({seller_id:a.profile.id,card_id:t.id,price:b}),n("Carte mise en vente sur le marché !","success"),s(),l("collection")}),(E=document.getElementById("cancel-sell-form-btn"))==null||E.addEventListener("click",async()=>{await $.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",t.id),await $.from("market_listings").update({status:"cancelled"}).eq("card_id",t.id).eq("status","active"),n("Annonce retirée","success"),s(),l("collection")})}function xt(t,e,i,o){var y,k,_,T,A,z;const{state:a,toast:n,openModal:s,closeModal:l,navigate:r,refreshProfile:d}=o,c=t.player,p=e.filter(C=>C.player.id===c.id),f=p.length,u=pt[c.rarity]||1e3,m=c.rarity!=="legende",w=We(c),h=ne(c,c.job),E=c.job2?ne(c,c.job2):null,b=fe[c.job]||"#1A6B3C",g=c.job2?fe[c.job2]:null,x=Ye[c.rarity]||"#ccc",v=ke[c.country_code]||c.country_code||"";s(`${c.firstname} ${c.surname_encoded}`,`<div style="display:flex;gap:16px;flex-wrap:wrap;justify-content:center">
      ...
      <div style="width:140px;border-radius:12px;padding:6px;background:${x};flex-shrink:0">
        <div style="background:#f2e8d2;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
          ...
          <div style="background:#f2e8d2;padding:3px 6px;display:flex;align-items:center;justify-content:space-between;min-height:24px">
            ...
            ${(y=c.clubs)!=null&&y.logo_url?`<img src="${c.clubs.logo_url}" style="width:22px;height:16px;object-fit:contain">`:`<div style="background:#1a3a7a;color:#fff;border-radius:2px;padding:1px 3px;font-size:6px;font-weight:800">${(((k=c.clubs)==null?void 0:k.encoded_name)||"").slice(0,6)}</div>`}
          </div>
        </div>
      </div>
  ...
  </div>`,document.getElementById("new-deck-btn").addEventListener("click",async()=>{const s=prompt("Nom du deck :",`Deck ${((n==null?void 0:n.length)||0)+1}`);if(!s)return;const{data:l,error:r}=await $.from("decks").insert({owner_id:i.profile.id,name:s}).select().single();if(r){a(r.message,"error");return}a("Deck créé !","success"),Se(l.id,t,e)}),t.querySelectorAll("[data-open-deck]").forEach(s=>{s.addEventListener("click",()=>Se(s.dataset.openDeck,t,e))}),t.querySelectorAll("[data-rename]").forEach(s=>{s.addEventListener("click",async()=>{const l=prompt("Nouveau nom :",s.dataset.name);if(!l||l===s.dataset.name)return;const{error:r}=await $.from("decks").update({name:l}).eq("id",s.dataset.rename);if(r){a(r.message,"error");return}a("Deck renommé !","success"),xe(t,e)})}),t.querySelectorAll("[data-delete]").forEach(s=>{s.addEventListener("click",async()=>{if(!confirm(`Supprimer le deck "${s.dataset.name}" ? Cette action est irréversible.`))return;await $.from("deck_cards").delete().eq("deck_id",s.dataset.delete);const{error:l}=await $.from("decks").delete().eq("id",s.dataset.delete);if(l){a(l.message,"error");return}a("Deck supprimé.","success"),xe(t,e)})})}async function Se(t,e,i){const{state:o,toast:a}=i;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:n}=await $.from("decks").select("*").eq("id",t).single(),{data:s}=await $.from("cards").select(`id, card_type, formation,
      player:players(id, firstname, surname_encoded, country_code, club_id, job, job2,
        note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length,
        clubs(encoded_name, logo_url))`).eq("owner_id",o.profile.id),l=(s||[]).filter(u=>u.card_type==="player"&&u.player),r=(s||[]).filter(u=>u.card_type==="formation"),d=r.map(u=>u.formation).filter(Boolean),{data:c}=await $.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",t);let p=n.formation||"4-4-2";d.length>0&&!d.includes(p)&&(p=d[0]);const f={deckId:t,name:n.name,formation:p,formationCardId:n.formation_card_id,slots:{},subs:[],playerCards:l,formationCards:r,availableFormations:d};(c||[]).forEach(u=>{u.is_starter?f.slots[u.position]=u.card_id:f.subs.includes(u.card_id)||f.subs.push(u.card_id)}),X(e,f,i)}function X(t,e,i){var r;const{navigate:o}=i;ue[e.formation];const a=Fe(e.formation),n=a.filter(d=>e.slots[d]).length,s=e.availableFormations.length>0?e.availableFormations:Object.keys(ue),l=e.subs.map(d=>e.playerCards.find(c=>c.id===d)).filter(Boolean);[...Object.values(e.slots),...e.subs],t.innerHTML=`
  <div class="page">
  ...
  </div>`,mt(t,e,a,i),document.getElementById("builder-back").addEventListener("click",()=>o("decks")),document.getElementById("formation-select").addEventListener("change",d=>{e.formation=d.target.value;const c=Fe(e.formation),p={};c.forEach(f=>{e.slots[f]&&(p[f]=e.slots[f])}),e.slots=p,X(t,e,i)}),document.getElementById("save-deck").addEventListener("click",()=>ht(e,i)),t.querySelectorAll("[data-remove-sub]").forEach(d=>{d.addEventListener("click",()=>{e.subs=e.subs.filter(c=>c!==d.dataset.removeSub),X(t,e,i)})}),(r=document.getElementById("add-sub-btn"))==null||r.addEventListener("click",()=>{bt(e,t,i)})}function mt(t,e,i,o){const a=document.getElementById("deck-field");if(!a)return;const n=e.formation,s=we[n]||{},l=ye?ye(n)||ie[n]||[]:ie[n]||[],r={};i.forEach(g=>{const x=e.slots[g],v=x?e.playerCards.find(y=>y.id===x):null;r[g]=(v==null?void 0:v.player)||null});const d=i.filter(g=>g.startsWith("MIL"));let c=0,p=0;d.forEach(g=>{const x=r[g];x&&(c+=Number(x.note_m)||0)}),l.forEach(([g,x])=>{if(!g.startsWith("MIL")||!x.startsWith("MIL"))return;U(r[g],r[x])!=="#ff3333"&&p++});const f=320,u=320,m=28,w=4;function h(g){const x=s[g];return x?{x:x.x*f,y:x.y*u}:null}let E="";for(const[g,x]of l){const v=h(g),y=h(x);if(!v||!y)continue;const k=r[g],_=r[x],T=U(k,_);if(T==="#00ff88"||T==="#FFD700")E+=`<line x1="${v.x}" y1="${v.y}" x2="${y.x}" y2="${y.y}"
        stroke="${T}" stroke-width="${w*3}" stroke-linecap="round" opacity="0.25"/>`,E+=`<line x1="${v.x}" y1="${v.y}" x2="${y.x}" y2="${y.y}"
        stroke="${T}" stroke-width="${w}" stroke-linecap="round" opacity="0.95"/>`;else{const z=T==="#ff3333"||T==="#cc2222"?.75:.9;E+=`<line x1="${v.x}" y1="${v.y}" x2="${y.x}" y2="${y.y}"
        stroke="${T}" stroke-width="${w}" stroke-linecap="round" opacity="${z}"/>`}}for(const g of i){const x=h(g);if(!x)continue;const v=r[g],y=g.replace(/\d+/,""),_={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[y]||"#555",T=v?Number(y==="GK"?v.note_g:y==="DEF"?v.note_d:y==="MIL"?v.note_m:v.note_a)||0:null,A=v?(v.surname_encoded||"").slice(0,8).toUpperCase():"";if(v){const z=_e(v);z&&(E+=`
          <defs>
            <clipPath id="clip-${g}">
    ...
    <svg viewBox="0 0 ${f} ${u}" width="100%" style="display:block;max-width:380px;margin:0 auto">
      ${b}
      ${E}
    </svg>`,a.querySelectorAll(".deck-slot-hit").forEach(g=>{g.addEventListener("click",()=>vt(g.dataset.pos,e,t,o))})}function vt(t,e,i,o){var f,u,m;const{openModal:a,closeModal:n}=o,s=t.replace(/\d+/,""),l=e.slots[t],r=l?e.playerCards.find(w=>w.id===l):null;(f=r==null?void 0:r.player)==null||f.id;const d=new Set;Object.entries(e.slots).forEach(([w,h])=>{var b;if(w===t||!h)return;const E=e.playerCards.find(g=>g.id===h);(b=E==null?void 0:E.player)!=null&&b.id&&d.add(E.player.id)}),e.subs.forEach(w=>{var E;const h=e.playerCards.find(b=>b.id===w);(E=h==null?void 0:h.player)!=null&&E.id&&d.add(h.player.id)});const c=new Set,p=e.playerCards.filter(w=>{const h=w.player;return!(h.job===s||h.job2===s)||d.has(h.id)||c.has(h.id)?!1:(c.add(h.id),!0)});p.sort((w,h)=>{const E=s==="GK"?w.player.note_g:s==="DEF"?w.player.note_d:s==="MIL"?w.player.note_m:w.player.note_a;return(s==="GK"?h.player.note_g:s==="DEF"?h.player.note_d:s==="MIL"?h.player.note_m:h.player.note_a)-E}),a(`Choisir ${s} — ${t}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${e.slots[t]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${p.length>0?p.map(w=>{var x,v;const h=w.player,E=s==="GK"?h.note_g:s==="DEF"?h.note_d:s==="MIL"?h.note_m:h.note_a,b=_e(h),g={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[h.rarity];return`<div class="player-option" data-card-id="${w.id}"
    ...
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(u=document.getElementById("close-selector"))==null||u.addEventListener("click",n),(m=document.getElementById("remove-player"))==null||m.addEventListener("click",()=>{delete e.slots[t],n(),X(i,e,o)}),document.querySelectorAll(".player-option").forEach(w=>{w.addEventListener("click",()=>{e.slots[t]=w.dataset.cardId,n(),X(i,e,o)})})}function bt(t,e,i){var r;const{openModal:o,closeModal:a}=i,n=new Set;Object.values(t.slots).filter(Boolean).forEach(d=>{var p;const c=t.playerCards.find(f=>f.id===d);(p=c==null?void 0:c.player)!=null&&p.id&&n.add(c.player.id)}),t.subs.forEach(d=>{var p;const c=t.playerCards.find(f=>f.id===d);(p=c==null?void 0:c.player)!=null&&p.id&&n.add(c.player.id)});const s=new Set,l=t.playerCards.filter(d=>{var c,p,f;return n.has((c=d.player)==null?void 0:c.id)||s.has((p=d.player)==null?void 0:p.id)?!1:(s.add((f=d.player)==null?void 0:f.id),!0)});o("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
    ...
    </div>`,'<button class="btn btn-ghost" id="close-sub-selector">Fermer</button>'),(r=document.getElementById("close-sub-selector"))==null||r.addEventListener("click",a),document.querySelectorAll(".player-option").forEach(d=>{d.addEventListener("click",()=>{t.subs.push(d.dataset.cardId),a(),X(e,t,i)})})}async function ht(t,e){const{state:i,toast:o,navigate:a}=e,n=t.formationCards.find(r=>r.formation===t.formation),s=(n==null?void 0:n.id)||t.formationCardId;await $.from("decks").update({formation:t.formation,formation_card_id:s||null}).eq("id",t.deckId),await $.from("deck_cards").delete().eq("deck_id",t.deckId);const l=[];if(Object.entries(t.slots).forEach(([r,d],c)=>{l.push({deck_id:t.deckId,card_id:d,position:r,is_starter:!0,slot_order:c})}),t.subs.forEach((r,d)=>{l.push({deck_id:t.deckId,card_id:r,position:`SUB${d+1}`,is_starter:!1,slot_order:100+d})}),l.length>0){const{error:r}=await $.from("deck_cards").insert(l);if(r){o(r.message,"error");return}}o("Deck enregistré ✅","success"),a("decks")}function Fe(t){const e=ue[t]||ue["4-4-2"],i=["GK1"];for(let o=1;o<=e.DEF;o++)i.push(`DEF${o}`);for(let o=1;o<=e.MIL;o++)i.push(`MIL${o}`);for(let o=1;o<=e.ATT;o++)i.push(`ATT${o}`);return i}const De=[{id:"players_std",img:"/manager-wars/icons/booster-players.png",name:"Players",sub:"5 cartes joueurs",cost:5e3,costLabel:"5 000 crédits",cardCount:5,type:"player"},{id:"players_pub",img:"/manager-wars/icons/booster-silver.png",name:"Players (pub)",sub:"3 cartes joueurs",cost:0,costLabel:"1 pub",cardCount:3,type:"player"},{id:"game_changer",img:"/manager-wars/icons/booster-gamechanger.png",name:"Game Changer",sub:"3 cartes spéciales",cost:1e4,costLabel:"10 000 crédits",cardCount:3,type:"game_changer"},{id:"formation",img:"/manager-wars/icons/booster-formation.png",name:"Formation",sub:"1 carte formation",cost:1e4,costLabel:"10 000 crédits",cardCount:1,type:"formation"}],Je={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé."},"Double attaque":{icon:"⚡",desc:"La prochaine attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la prochaine action IA."},Gel:{icon:"❄️",desc:"Bloque le meilleur attaquant IA."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function $t(t){const e="https://fcnwclxlkytdfjotqkta.supabase.co";if(!(t!=null&&t.skin)||!(t!=null&&t.hair))return null;const i=t.hair==="chauve"?`${t.skin}-chauve-rase`:`${t.skin}-${t.hair}-${t.hair_length}`;return`${e}/storage/v1/object/public/assets/tetes/${i}.png`}const wt={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},kt={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"};async function _t(t,{state:e,navigate:i,toast:o}){var n;const a=((n=e.profile)==null?void 0:n.credits)||0;t.innerHTML=`
  <div class="page">
    <div class="page-header">
      <h2>📦 Boosters</h2>
    ...
    <div class="page-body">
      <div class="booster-grid">
        ${De.map(s=>{const l=a>=s.cost||s.cost===0,r=s.id==="players_std"||s.id==="players_pub";return`<div class="booster-card ${l?"":"disabled"}" data-booster="${s.id}" style="position:relative">
  ...
  `,document.body.appendChild(o);let a=!1;document.getElementById("pack-visual").addEventListener("click",()=>{if(a)return;a=!0;const f=document.getElementById("pack-visual");f.classList.add("shaking"),setTimeout(()=>{f.classList.add("pack-open"),setTimeout(()=>{document.getElementById("pack-phase").style.display="none",s(0)},600)},500)});let n=0;function s(f){n=f;const u=document.getElementById("reveal-phase");u.style.display="flex",l(f)}function l(f){var v;const u=t[f],m=document.getElementById("card-counter"),w=document.getElementById("single-card-slot"),h=document.getElementById("card-tap-hint");m&&(m.textContent=`Carte ${f+1} / ${t.length}`),h&&(h.textContent=f<t.length-1?"Appuie pour continuer →":"Appuie pour voir toutes tes cartes");const E=Ge(u),b=u.card_type==="player"&&((v=u.player)==null?void 0:v.rarity)==="legende";w.innerHTML=`
      ...
      </div>`,b&&c();const g=document.getElementById("current-card-wrap");let x=!1;g.addEventListener("click",()=>{if(x)return;x=!0;const y=f+1;y<t.length?(g.style.transition="all 0.25s ease",g.style.transform="translateX(-120%) rotate(-15deg)",g.style.opacity="0",setTimeout(()=>l(y),250)):r()})}function r(){p(),document.getElementById("reveal-phase").style.display="none";const f=document.getElementById("recap-phase");f.style.display="flex";const u=document.getElementById("recap-grid");u.innerHTML=t.map((m,w)=>`
  ...
  `,document.body.appendChild(t),t.addEventListener("click",e=>{e.target===t&&t.remove()}),document.getElementById("odds-close").addEventListener("click",()=>t.remove())}function jt(){return new Promise(t=>{const e=document.createElement("div");e.style.cssText="position:fixed;inset:0;background:#000;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:16px";let i=5;e.innerHTML=`<div style="font-size:48px">📺</div>
  ...
  </div>`,(n=document.getElementById("msg-btn"))==null||n.addEventListener("click",a)}function G(t){const e=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!e||!(t!=null&&t.skin)||!(t!=null&&t.hair))return null;const i=t.hair==="chauve"?`${t.skin}-chauve-rase`:`${t.skin}-${t.hair}-${t.hair_length}`;return`${e}/storage/v1/object/public/assets/tetes/${i}.png`}function me(t){var i,o;const e=t.player;return{cardId:t.id,id:e.id,firstname:e.firstname,name:e.surname_encoded,country_code:e.country_code,club_id:e.club_id,job:e.job,job2:e.job2,note_g:Number(e.note_g)||0,note_d:Number(e.note_d)||0,note_m:Number(e.note_m)||0,note_a:Number(e.note_a)||0,rarity:e.rarity,skin:e.skin,hair:e.hair,hair_length:e.hair_length,clubName:((i=e.clubs)==null?void 0:i.encoded_name)||null,clubLogo:((o=e.clubs)==null?void 0:o.logo_url)||null,boost:0,used:!1,_line:null,_col:null}}function Ee(t){return t===1?[1]:t===2?[0,2]:t===3?[0,1,2]:t===4?[0,1,1,2]:t===5?[0,1,1,1,2]:[1]}function Bt(){const t=Math.random()*100;return t<.1?4:t<5?3:t<20?2:1}function Qe(t,e){const i=Q[e]||Q["4-4-2"],o={GK:[],DEF:[],MIL:[],ATT:[]},a=[...t];for(const n of["GK","DEF","MIL","ATT"]){const s=[];for(let r=0;r<i[n];r++){let d=a.findIndex(c=>c.job===n);if(d===-1&&(d=a.findIndex(c=>c.job2===n)),d===-1&&(d=0),a[d]){const c={...a[d],_line:n};s.push(c),a.splice(d,1)}}const l=Ee(s.length);s.forEach((r,d)=>{r._col=l[d]}),o[n]=s}return o}async function St(t,e){const{data:i}=await $.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length").eq("is_active",!0).limit(60);if(!i||i.length<11)return Ft(t);const o=Q[t]||Q["4-4-2"],a={GK:[],DEF:[],MIL:[],ATT:[]},n=[...i];for(const s of["GK","DEF","MIL","ATT"]){const l=n.filter(f=>f.job===s),r=n.filter(f=>f.job!==s),d=[...l,...r],c=[];for(let f=0;f<o[s];f++){const u=d[f]||n[f];u&&c.push({cardId:"ai-"+u.id+"-"+f,id:u.id,firstname:u.firstname,name:u.surname_encoded,country_code:u.country_code,club_id:u.club_id,job:u.job,job2:u.job2,note_g:Number(u.note_g)||0,note_d:Number(u.note_d)||0,note_m:Number(u.note_m)||0,note_a:Number(u.note_a)||0,rarity:u.rarity,skin:u.skin,hair:u.hair,hair_length:u.hair_length,boost:0,used:!1,_line:s})}const p=Ee(c.length);c.forEach((f,u)=>{f._col=p[u]}),a[s]=c}return a}function Ft(t){const e=Q[t]||Q["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},o=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let a=0;for(const n of["GK","DEF","MIL","ATT"]){const s=[];for(let r=0;r<e[n];r++){const d=3+Math.floor(Math.random()*5);s.push({cardId:"fake-"+a,id:"fake-"+a,firstname:"IA",name:o[a%o.length],country_code:"XX",club_id:null,job:n,job2:null,note_g:n==="GK"?d:2,note_d:n==="DEF"?d:2,note_m:n==="MIL"?d:2,note_a:n==="ATT"?d:2,rarity:"normal",boost:0,used:!1,_line:n}),a++}const l=Ee(s.length);s.forEach((r,d)=>{r._col=l[d]}),i[n]=s}return i}async function Dt(t,e){var v;const{state:i,navigate:o,toast:a}=e,n=i.params||{};t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const s=n.matchMode||"vs_ai_easy",l=s.replace("vs_ai_",""),r=s;if(!n.deckId)return Gt(t,e,s);const d=n.deckId,[{data:c},{data:p}]=await Promise.all([$.from("decks").select("formation,name").eq("id",d).single(),$.from("deck_cards").select(`position, is_starter, slot_order,
        card:cards(id, card_type, formation,
          player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
            note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
            clubs(encoded_name,logo_url)))`).eq("deck_id",d).order("slot_order")]),f=(p||[]).filter(y=>{var k;return y.is_starter&&((k=y.card)==null?void 0:k.player)}).map(y=>me(y.card)),u=(p||[]).filter(y=>{var k;return!y.is_starter&&((k=y.card)==null?void 0:k.player)}).map(y=>me(y.card));if(f.length<11){Xe(t,"⚠️",`Deck incomplet (${f.length}/11).`,"Compléter",()=>o("decks"));return}const m=(p||[]).find(y=>{var k;return((k=y.card)==null?void 0:k.card_type)==="formation"}),w=(c==null?void 0:c.formation)||((v=m==null?void 0:m.card)==null?void 0:v.formation)||"4-4-2",{data:h}=await $.from("cards").select("id,gc_type").eq("owner_id",i.profile.id).eq("card_type","game_changer"),E=Qe(f,w),b=await St(w),{data:g}=await $.from("matches").insert({home_id:i.profile.id,away_id:null,mode:r,home_deck_id:d,status:"in_progress"}).select().single(),x={matchId:g==null?void 0:g.id,mode:r,difficulty:l,formation:w,homeTeam:E,aiTeam:b,homeSubs:u,subsUsed:0,maxSubs:Math.min(u.length,3),homeScore:0,aiScore:0,gcCards:h||[],usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Nt(t,x,e)}async function Gt(t,e,i){const{state:o,navigate:a}=e;t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:n}=await $.from("decks").select("id,name,is_active,formation").eq("owner_id",o.profile.id).order("created_at",{ascending:!1});if(!n||n.length===0){Xe(t,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>a("decks"));return}const s=n.map(c=>c.id),{data:l}=await $.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",s).order("slot_order");let r=0;function d(){var b,g,x,v,y;const c=n[r],p=(l||[]).filter(k=>k.deck_id===c.id),f=p.filter(k=>{var _;return k.is_starter&&((_=k.card)==null?void 0:_.player)}).map(k=>me(k.card)),u=p.find(k=>{var _;return((_=k.card)==null?void 0:_.card_type)==="formation"}),m=c.formation||((b=u==null?void 0:u.card)==null?void 0:b.formation)||"4-4-2",w=f.length>=11?Qe(f,m):null,h=f.length>=11;t.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:calc(100dvh - 130px);overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div id="deck-swipe-zone" style="flex:1;min-height:0;overflow:hidden;position:relative;touch-action:pan-y;display:flex;align-items:center;justify-content:center">
        ${w?`<div style="width:min(98vw, calc(100dvh - 400px));overflow:hidden;flex-shrink:0">${tt(w,m,null,[],285,285)}</div>`:`<div style="display:flex;align-items:center;justify-content:center;height:100%;opacity:.4;flex-direction:column;gap:8px">
      ...
      <div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">
        ${n.map((k,_)=>`<div style="width:7px;height:7px;border-radius:50%;background:${_===r?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}
      </div>`:""}
    ...
    </div>`,(g=document.getElementById("prev-deck"))==null||g.addEventListener("click",()=>{r>0&&(r--,d())}),(x=document.getElementById("next-deck"))==null||x.addEventListener("click",()=>{r<n.length-1&&(r++,d())}),(v=document.getElementById("validate-deck"))==null||v.addEventListener("click",()=>{h&&e.navigate("match",{matchMode:i,deckId:c.id})}),(y=document.getElementById("cancel-deck-select"))==null||y.addEventListener("click",()=>a("home"));const E=document.getElementById("deck-swipe-zone");if(E){let k=0,_=0;E.addEventListener("touchstart",T=>{k=T.touches[0].clientX,_=T.touches[0].clientY},{passive:!0}),E.addEventListener("touchend",T=>{const A=T.changedTouches[0].clientX-k,z=T.changedTouches[0].clientY-_;Math.abs(A)<40||Math.abs(A)<Math.abs(z)||(A<0&&r<n.length-1?(r++,d()):A>0&&r>0&&(r--,d()))},{passive:!0})}}d()}function Nt(t,e,i){t.innerHTML=`
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;min-height:100vh;gap:12px;padding:12px 16px;background:#0a3d1e">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${p.map((m,w)=>{const h=G(m),E=w<p.length-1?U(m,p[w+1]):null,b=E&&E!=="#ff3333"&&E!=="#cc2222";return`
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;min-height:100vh;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;height:${p}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ...
      ${a?`<img src="${a}" style="width:${p-4}px;height:${p-4}px;object-fit:contain">`:t!=null&&t.clubName?`<span style="font-size:${Math.max(4,p-8)}px;font-weight:700;color:#333">${(t.clubName||"").slice(0,3).toUpperCase()}</span>`:""}
        ...
        stroke="${_}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const f=48,u=64,m=13,w=16,h={normal:"#aaaaaa",pépite:"#D4A017",papyte:"#111111",légende:"#7a28b8"};for(const[b,g]of Object.entries(r)){const x=c(b);if(!x||!g)continue;const v=b.replace(/[0-9]/g,""),y=Z[v]||"#555",k=i==="attack"&&["MIL","ATT"].includes(v)&&!g.used||i==="defense"&&["GK","DEF","MIL"].includes(v)&&!g.used,_=o.includes(g.cardId);let T;i==="attack"?T=v==="MIL"?Number(g.note_m)||0:Number(g.note_a)||0:i==="defense"?T=v==="GK"?Number(g.note_g)||0:v==="MIL"?Number(g.note_m)||0:Number(g.note_d)||0:T=Number(v==="GK"?g.note_g:v==="DEF"?g.note_d:v==="MIL"?g.note_m:g.note_a)||0,T=T+(g.boost||0);const A=(x.x-f/2).toFixed(1),z=(x.y-u/2).toFixed(1),C=g.used?.25:1,R=h[g==null?void 0:g.rarity]||h.normal,q=_?"#ff3030":R,I=_?3:(g==null?void 0:g.rarity)==="légende"||(g==null?void 0:g.rarity)==="pépite"?2.5:2,L=u-m-w;p+=`<defs><clipPath id="cp-${b}"><rect x="${A}" y="${(x.y-u/2+m).toFixed(1)}" width="${f}" height="${L}"/></clipPath></defs>`,p+=`<rect x="${A}" y="${z}" width="${f}" height="${u}" rx="5" fill="${y}" opacity="${C}"/>`;const M=G(g);M&&!g.used&&(p+=`<image href="${M}" x="${A}" y="${(x.y-u/2+m).toFixed(1)}" width="${f}" height="${L}" clip-path="url(#cp-${b})" preserveAspectRatio="xMidYMin slice"/>`),p+=`<rect x="${A}" y="${z}" width="${f}" height="${m}" rx="4" fill="rgba(255,255,255,0.92)"/>`,p+=`<text x="${x.x.toFixed(1)}" y="${(x.y-u/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(g.name||"").slice(0,9)}</text>`;const B=(x.y+u/2-w).toFixed(1);if(p+=`<rect x="${A}" y="${B}" width="${f}" height="${w}" fill="rgba(255,255,255,0.92)"/>`,g.used)p+=`<text x="${x.x.toFixed(1)}" y="${(x.y+u/2-w/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="13" font-weight="900" fill="rgba(0,0,0,0.4)" font-family="Arial Black">–</text>`;else{const S=ve(g.country_code);S?p+=`<image href="${S}" x="${(x.x-20).toFixed(1)}" y="${(x.y+u/2-w+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:p+=`<text x="${(x.x-13).toFixed(1)}" y="${(x.y+u/2-w/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${et(g.country_code)}</text>`,p+=`<text x="${x.x.toFixed(1)}" y="${(x.y+u/2-w/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${T}</text>`;const F=Ze(g);F?p+=`<image href="${F}" x="${(x.x+f/2-14).toFixed(1)}" y="${(x.y+u/2-w+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:g.clubName&&(p+=`<text x="${(x.x+14).toFixed(1)}" y="${(x.y+u/2-w/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(g.clubName||"").slice(0,3).toUpperCase()}</text>`),g.boost&&(p+=`<rect x="${(x.x+f/2-12).toFixed(1)}" y="${(x.y-u/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,p+=`<text x="${(x.x+f/2-5).toFixed(1)}" y="${(x.y-u/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${g.boost}</text>`)}p+=`<rect x="${A}" y="${z}" width="${f}" height="${u}" rx="5" fill="${_?"rgba(255,255,255,0.12)":"none"}" stroke="${q}" stroke-width="${I}" opacity="${C}"/>`,k&&(p+=`<rect x="${A}" y="${z}" width="${f}" height="${u}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${_?"selected":""}" data-card-id="${g.cardId}" data-role="${v}" style="cursor:pointer"/>`)}const E=38;return`<svg viewBox="${-E} ${-E} ${a+E*2} ${n+E*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${p}
  ...
  <div style="text-align:center;flex-shrink:0;width:38px">
    ...
    <div style="display:flex;align-items:center;gap:4px;${e?"flex-direction:row-reverse":""};background:rgba(255,255,255,0.04);border-radius:8px;padding:5px 8px;border:1px solid rgba(255,255,255,0.07)">
      ...
      ${de(t.inPlayer||{})}
    </div>`}return`<div style="font-size:11px;color:${t.type==="goal"?"#FFD700":"rgba(255,255,255,0.65)"};font-weight:${t.type==="goal"?700:400};padding:3px 2px">${t.text||""}</div>`}function N(t,e,i){var m,w,h,E,b,g,x,v;const o=e.selected.map(y=>y.cardId),a=e.usedSubIds||[],n=e.homeSubs.filter(y=>!a.includes(y.cardId));Object.values(e.homeTeam).flat().filter(y=>y.used).length>0&&n.length>0&&e.subsUsed<e.maxSubs,e.log[e.log.length-1];const l=e.phase==="ai-attack"||e.phase==="ai-defense",r=e.phase==="attack",d=e.phase==="defense",c=e.phase==="finished",p=e.gcCards.filter(y=>!e.usedGc.includes(y.id)),f=e.boostCard&&!e.boostUsed;if(t.innerHTML=`
  <style>
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;height:calc(100dvh - 130px);max-height:calc(100dvh - 130px);overflow:hidden;background:#0a3d1e;position:relative">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:100px">
      ${(()=>{if(e.phase==="defense"&&e.pendingAttack){const k=e.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
            ...
            ${se((k.players||[]).map(_=>({..._,used:!1})),"#ff6b6b",k.total)}
            ...
            ${se((k.players||[]).map(_=>({..._,used:!1})),"#00ff88",k.total)}
    ...
    <div style="display:flex;align-items:flex-end;padding:6px 8px;background:rgba(0,0,0,0.35);gap:8px;flex-shrink:0;min-height:80px">
      ...
      <div style="flex:1;display:grid;grid-template-columns:repeat(4,1fr);gap:3px;align-content:start">
        ${p.map(y=>{var k;return`
          <div class="gc-mini" data-gc-id="${y.id}" data-gc-type="${y.gc_type}"
            ...
            <div style="font-size:16px">${((k=oe[y.gc_type])==null?void 0:k.icon)||"⚡"}</div>
      ...
      <div style="flex-shrink:0;display:flex;flex-direction:column;align-items:center;gap:3px">
        ...
        ${r||d?`<div style="font-size:9px;color:rgba(255,255,255,0.4)">${e.selected.length}/3 sélectionné(s)</div>`:""}
      </div>
    </div>
  </div>
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${e.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...e.log].reverse().map(y=>{var k,_,T;if(y.type==="duel"){const A=y.isGoal,z=y.homeScored?"#FFD700":A?"#ff6b6b":"rgba(255,255,255,0.3)",C=y.homeScored?"⚽ BUT !":A?"⚽ BUT IA !":(k=y.homePlayers)!=null&&k.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${A?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${z};margin-bottom:4px">
  ...
  </div>`,e._timerInt&&(clearInterval(e._timerInt),e._timerInt=null),e.phase==="attack"||e.phase==="defense"){let y=!1,k=30;const _=()=>document.getElementById("match-timer"),T=()=>{const A=_();if(!A)return;const z=String(Math.floor(k/60)).padStart(2,"0"),C=String(k%60).padStart(2,"0");A.textContent=` ${z}:${C}`,A.style.color=y?"#ff2222":"#ff9500",A.style.fontWeight="900"};T(),e._timerInt=setInterval(()=>{if(k--,k<0)if(!y)y=!0,k=15,T();else{clearInterval(e._timerInt),e._timerInt=null,e.homeScore=0,e.aiScore=3;const A=document.createElement("div");A.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;color:#fff;padding:24px;text-align:center",A.innerHTML='<div style="font-size:56px">⏱️</div><div style="font-size:24px;font-weight:900;color:#ff4444">MATCH PERDU PAR FORFAIT</div><div style="font-size:14px;color:rgba(255,255,255,0.6)">Temps écoulé</div>',document.body.appendChild(A),setTimeout(()=>{A.remove(),J(t,e,i)},2500)}else T()},1e3)}(m=document.getElementById("match-quit"))==null||m.addEventListener("click",()=>{confirm("Abandonner ? Résultat : défaite 3-0")&&(e.homeScore=0,e.aiScore=3,J(t,e,i))}),(w=document.getElementById("view-ai"))==null||w.addEventListener("click",()=>Jt(e,i)),(h=document.getElementById("toggle-history"))==null||h.addEventListener("click",()=>{var y;(y=document.getElementById("match-history-panel"))==null||y.classList.add("open")}),(E=document.getElementById("close-history"))==null||E.addEventListener("click",()=>{var y;(y=document.getElementById("match-history-panel"))==null||y.classList.remove("open")}),(b=document.getElementById("btn-action"))==null||b.addEventListener("click",()=>{e.selected.length!==0&&(r?Pt(t,e,i):d&&Kt(t,e,i))}),(g=document.getElementById("btn-results"))==null||g.addEventListener("click",()=>J(t,e,i)),t.querySelectorAll(".match-slot-hit").forEach(y=>{y.addEventListener("click",()=>Ot(y,e,t,i))}),t.querySelectorAll(".gc-mini").forEach(y=>{y.addEventListener("click",()=>Vt(y.dataset.gcId,y.dataset.gcType,t,e,i))}),(x=document.getElementById("boost-card"))==null||x.addEventListener("click",()=>Wt(t,e,i)),t.querySelectorAll(".sub-btn-col").forEach(y=>{y.addEventListener("click",()=>Ne(t,e,i,y.dataset.subId))}),(v=document.getElementById("sub-btn-main"))==null||v.addEventListener("click",()=>Ne(t,e,i))}function Ot(t,e,i,o){const a=t.dataset.cardId,n=t.dataset.role,s=e.selected.findIndex(l=>l.cardId===a);if(s!==-1)e.selected.splice(s,1);else{if(e.selected.length>=3){o.toast("Maximum 3 joueurs","error");return}const l=(e.homeTeam[n]||[]).find(r=>r.cardId===a);l&&e.selected.push({...l,_role:n,_line:n})}N(i,e,o)}function Te(t,e,i){t.matchId&&$.from("matches").update({last_player_id:i}).eq("id",t.matchId).then(()=>{})}function Pt(t,e,i){e._timerInt&&(clearInterval(e._timerInt),e._timerInt=null),Te(e,i,i.state.profile.id);const o=e.selected.map(n=>({...n,_line:n._role})),a=Ke(o,e.modifiers.home);e.pendingAttack={...a,players:[...e.selected],side:"home"},e.selected.forEach(n=>{const s=(e.homeTeam[n._role]||[]).find(l=>l.cardId===n.cardId);s&&(s.used=!0)}),e.log.push({text:`⚔️ Vous attaquez : ${a.total} (base ${a.base}${a.links?` +${a.links} liens`:""}) — ${e.selected.map(n=>n.name).join(", ")}`,type:"info"}),e.selected=[],e.modifiers.home={},e.phase="ai-defense",N(t,e,i),setTimeout(()=>Ht(t,e,i),1200)}function Kt(t,e,i){e._timerInt&&(clearInterval(e._timerInt),e._timerInt=null),Te(e,i,i.state.profile.id);const o=e.selected.map(l=>({...l,_line:l._role})),a=He(o,e.modifiers.home);e.selected.forEach(l=>{const r=(e.homeTeam[l._role]||[]).find(d=>d.cardId===l.cardId);r&&(r.used=!0)});const n=Ue(e.pendingAttack.total,a.total,e.modifiers.home),s={type:"duel",title:"Défense",aiPlayers:(e.pendingAttack.players||[]).map(l=>({name:l.name,note:l._line==="MIL"?l.note_m:l.note_a,portrait:G(l),job:l.job,country_code:l.country_code,rarity:l.rarity,clubName:l.clubName,clubLogo:l.clubLogo})),homePlayers:e.selected.map(l=>{const r=(e.homeTeam[l._role]||[]).find(d=>d.cardId===l.cardId)||l;return{name:r.name,note:r._line==="GK"?r.note_g:r._line==="MIL"?r.note_m:r.note_d,portrait:G(r),job:r.job,country_code:r.country_code,rarity:r.rarity,clubName:r.clubName,clubLogo:r.clubLogo}}),homeTotal:a.total,aiTotal:e.pendingAttack.total,isGoal:!1,homeScored:!1,text:""};if(n.shielded)s.text="🛡️ Bouclier ! But annulé.",e.log.push(s);else if(n.goal){e.aiScore++,s.isGoal=!0,s.homeScored=!1,s.text=`⚽ BUT IA ! (${e.pendingAttack.total} > ${a.total})`,e.log.push(s),e.selected=[],e.modifiers.home={},e.pendingAttack=null,N(t,e,i),Ae(s.aiPlayers,e.homeScore,e.aiScore,!1,()=>{re(t,e,i,"home-attack")});return}else s.text=`🧤 Défense réussie ! (${a.total} ≥ ${e.pendingAttack.total})`,e.log.push(s);e.selected=[],e.modifiers.home={},e.pendingAttack=null,re(t,e,i,"home-attack")}function be(t,e,i){Te(e,i,null);const o=[...e.aiTeam.MIL||[],...e.aiTeam.ATT||[]],a=Ve(o,"attack",e.difficulty);if(!a.length){ot(t,e,i);return}const n=Ke(a,e.modifiers.ai);e.pendingAttack={...n,players:a,side:"ai"},a.forEach(d=>{d.used=!0}),e.log.push({text:`🤖 IA attaque : ${n.total} (${a.map(d=>d.name).join(", ")})`,type:"info"}),e.modifiers.ai={};const s=[...e.homeTeam.GK||[],...e.homeTeam.DEF||[],...e.homeTeam.MIL||[]].filter(d=>!d.used),r=(e.homeSubs||[]).filter(d=>!(e.usedSubIds||[]).includes(d.cardId)).length>0&&e.subsUsed<e.maxSubs;if(s.length===0&&!r){e.aiScore++;const d={type:"duel",isGoal:!0,homeScored:!1,aiPlayers:a.map(c=>({name:c.name,note:c._line==="MIL"?c.note_m:c.note_a,portrait:G(c),job:c.job,country_code:c.country_code,rarity:c.rarity,clubName:c.clubName,clubLogo:c.clubLogo})),aiTotal:n.total,text:"⚽ BUT IA ! (aucun défenseur disponible)"};e.log.push(d),e.pendingAttack=null,N(t,e,i),Ae(d.aiPlayers,e.homeScore,e.aiScore,!1,()=>{re(t,e,i,"home-attack")});return}e.phase="defense",N(t,e,i)}function Ht(t,e,i){const o=[...e.aiTeam.GK||[],...e.aiTeam.DEF||[],...e.aiTeam.MIL||[]],a=Ve(o,"defense",e.difficulty),n=a.length>0?He(a,e.modifiers.ai).total:0;a.forEach(r=>{r.used=!0});const s=Ue(e.pendingAttack.total,n,e.modifiers.ai),l={type:"duel",title:"Attaque",homePlayers:(e.pendingAttack.players||[]).map(r=>({name:r.name,note:r._line==="MIL"?r.note_m:r.note_a,portrait:G(r),job:r.job,country_code:r.country_code,rarity:r.rarity,clubName:r.clubName,clubLogo:r.clubLogo})),aiPlayers:a.map(r=>({name:r.name,note:r._line==="GK"?r.note_g:r._line==="MIL"?r.note_m:r.note_d,portrait:G(r),job:r.job,country_code:r.country_code,rarity:r.rarity,clubName:r.clubName,clubLogo:r.clubLogo})),homeTotal:e.pendingAttack.total,aiTotal:n,isGoal:!1,homeScored:!1,text:""};if(s.shielded)l.text="🛡️ Bouclier IA !",e.log.push(l);else if(s.goal){e.homeScore++,l.isGoal=!0,l.homeScored=!0,l.text=`⚽ BUT ! (${e.pendingAttack.total} > ${n})`,e.log.push(l),e.modifiers.ai={},e.pendingAttack=null,N(t,e,i),Ae(l.homePlayers,e.homeScore,e.aiScore,!0,()=>{re(t,e,i,"ai-attack")});return}else l.text=`🧤 IA défend (${n} ≥ ${e.pendingAttack.total})`,e.log.push(l);e.modifiers.ai={},e.pendingAttack=null,re(t,e,i,"ai-attack")}function re(t,e,i,o){if(e.round++,it(e)){J(t,e,i);return}if(o==="home-attack"){if(![...e.homeTeam.MIL||[],...e.homeTeam.ATT||[]].filter(n=>!n.used).length){if(![...e.homeTeam.GK||[],...e.homeTeam.DEF||[],...e.homeTeam.MIL||[]].filter(s=>!s.used).length){J(t,e,i);return}e.phase="ai-attack",N(t,e,i),setTimeout(()=>be(t,e,i),800);return}e.phase="attack",N(t,e,i)}else{if(![...e.aiTeam.MIL||[],...e.aiTeam.ATT||[]].filter(n=>!n.used).length){ot(t,e,i);return}e.phase="ai-attack",N(t,e,i),setTimeout(()=>be(t,e,i),800)}}function it(t){const e=["MIL","ATT","GK","DEF"].some(o=>(t.homeTeam[o]||[]).some(a=>!a.used)),i=["MIL","ATT","GK","DEF"].some(o=>(t.aiTeam[o]||[]).some(a=>!a.used));return!e&&!i}function ot(t,e,i){it(e)?J(t,e,i):(e.phase="attack",N(t,e,i))}function Ut(t,e,i){const o=document.createElement("div");o.style.cssText=`
    position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:800;
    display:flex;flex-direction:column;align-items:center;justify-content:center;gap:20px;
    animation:subFadeIn 0.2s ease;
  `;const a=G(t),n=G(e),s=Z[t.job]||"#555",l=Z[e.job]||"#555",r=t.job==="GK"?t.note_g:t.job==="DEF"?t.note_d:t.job==="MIL"?t.note_m:t.note_a,d=e.job==="GK"?e.note_g:e.job==="DEF"?e.note_d:e.job==="MIL"?e.note_m:e.note_a;o.innerHTML=`
    <style>
  ...
  `,document.body.appendChild(o);let c=!1;const p=()=>{c||(c=!0,o.remove(),i())};o.addEventListener("click",p),setTimeout(p,2e3)}function te(t,e="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${e};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=t,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function Ne(t,e,i,o=null){if(e.phase!=="attack"){te("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(e.usedSubIds||(e.usedSubIds=[]),e.subsUsed>=e.maxSubs){te(`Maximum ${e.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const a=Object.values(e.homeTeam).flat().filter(p=>p.used),n=e.homeSubs.filter(p=>!e.usedSubIds.includes(p.cardId));if(!a.length){te("Aucun joueur utilisé à remplacer");return}if(!n.length){te("Aucun remplaçant disponible");return}let s=0,l=Math.max(0,n.findIndex(p=>p.cardId===o)),r=!1;const d=document.createElement("div");d.id="sub-overlay",d.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function c(){var E,b,g,x,v,y;const p=a[s],f=n[l],u=Math.min(130,Math.round((window.innerWidth-90)/2)),m=Math.round(u*1.35),w=k=>`background:rgba(255,255,255,0.12);border:none;color:${k?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${k?"default":"pointer"};flex-shrink:0`;d.innerHTML=`
    ...
    <div style="flex:1;display:flex;gap:0;overflow:hidden">
      ...
      <div id="in-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
        ...
        <button id="in-down" style="${w(l>=n.length-1)}" ${l>=n.length-1?"disabled":""}>▼</button>
      ...
      <div id="out-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
        ...
        <button id="out-down" style="${w(s>=a.length-1)}" ${s>=a.length-1?"disabled":""}>▼</button>
        <div style="font-size:10px;color:rgba(255,255,255,0.35)">${s+1}/${a.length}</div>
      </div>
    </div>
    <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
      <button id="sub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
    </div>`,(E=d.querySelector("#sub-close"))==null||E.addEventListener("click",()=>d.remove()),(b=d.querySelector("#out-up"))==null||b.addEventListener("click",()=>{s>0&&(s--,c())}),(g=d.querySelector("#out-down"))==null||g.addEventListener("click",()=>{s<a.length-1&&(s++,c())}),(x=d.querySelector("#in-up"))==null||x.addEventListener("click",()=>{l>0&&(l--,c())}),(v=d.querySelector("#in-down"))==null||v.addEventListener("click",()=>{l<n.length-1&&(l++,c())});const h=(k,_,T,A)=>{const z=d.querySelector("#"+k);if(!z)return;let C=0;z.addEventListener("touchstart",R=>{C=R.touches[0].clientY},{passive:!0}),z.addEventListener("touchend",R=>{const q=R.changedTouches[0].clientY-C;if(Math.abs(q)<30)return;const I=_();q<0&&I<A-1?(T(I+1),c()):q>0&&I>0&&(T(I-1),c())},{passive:!0})};h("in-panel",()=>l,k=>l=k,n.length),h("out-panel",()=>s,k=>s=k,a.length),(y=d.querySelector("#sub-confirm"))==null||y.addEventListener("click",k=>{if(k.preventDefault(),k.stopPropagation(),r)return;r=!0;const _=a[s],T=n[l];if(!_||!T)return;let A=null,z=-1;for(const[R,q]of Object.entries(e.homeTeam)){const I=(q||[]).findIndex(L=>L.cardId===_.cardId);if(I!==-1){A=R,z=I;break}}if(z===-1||!A){te("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),d.remove();return}const C={...T,_line:A,_col:_._col||0,used:!1,boost:0};e.homeTeam[A].splice(z,1,C),e.usedSubIds||(e.usedSubIds=[]),e.usedSubIds.push(T.cardId),e.subsUsed++,e.selected=[],e.log.push({type:"sub",subSide:"home",clubName:e.clubName,outPlayer:{name:_.name,firstname:_.firstname,note:D(_,A),portrait:G(_),job:_.job,country_code:_.country_code,rarity:_.rarity,clubName:_.clubName,clubLogo:_.clubLogo},inPlayer:{name:T.name,firstname:T.firstname,note:D(T,A),portrait:G(T),job:T.job,country_code:T.country_code,rarity:T.rarity,clubName:T.clubName,clubLogo:T.clubLogo},text:`🔄 ${T.firstname} ${T.name} remplace ${_.firstname} ${_.name}`}),d.remove(),Ut(_,T,()=>N(t,e,i))})}document.body.appendChild(d),c()}function Vt(t,e,i,o,a){var l,r;const n=oe[e]||{icon:"⚡",desc:"Carte spéciale."},s=document.createElement("div");s.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;align-items:center;justify-content:center;padding:24px",s.innerHTML=`
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(n);let l=!1;const r=()=>{l||(l=!0,n.remove(),setTimeout(()=>a(),50))};n.addEventListener("click",r),setTimeout(r,3500)}async function J(t,e,i){var c,p;e._timerInt&&(clearInterval(e._timerInt),e._timerInt=null),e.phase="finished";const{state:o}=i,a=e.homeScore>e.aiScore,n=e.homeScore===e.aiScore,s=a?"victoire":n?"nul":"defaite",l=lt(e.mode,s);e.matchId&&await $.from("matches").update({status:"finished",home_score:e.homeScore,away_score:e.aiScore,winner_id:a?o.profile.id:null,home_credits_reward:l,played_at:new Date().toISOString()}).eq("id",e.matchId);const r={credits:(o.profile.credits||0)+l,matches_played:(o.profile.matches_played||0)+1};a?r.wins=(o.profile.wins||0)+1:n?r.draws=(o.profile.draws||0)+1:r.losses=(o.profile.losses||0)+1,await $.from("users").update(r).eq("id",o.profile.id),await i.refreshProfile();const d=document.createElement("div");d.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",d.innerHTML=`
    ...
    </div>`,`<button class="btn btn-primary" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Xt={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"};async function Qt(t,e){const{state:i,toast:o}=e;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement du marché...</div>',await Le(t,e)}async function Le(t,e){const{state:i,toast:o}=e,{data:a}=await $.from("market_listings").select(`id, price, status, listed_at, seller_id,
  ...
  `;function l(r){const d=document.getElementById("mkt-content"),c=r==="buy"?s:n;if(c.length===0){d.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      ${c.map(p=>{var h,E,b;const f=(h=p.card)==null?void 0:h.player;if(!f)return"";const u=f.job==="GK"?f.note_g:f.job==="DEF"?f.note_d:f.job==="MIL"?f.note_m:f.note_a,m=Xt[f.rarity],w=(i.profile.credits||0)>=p.price;return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px">
