import{s as k,F as it,b as Ce,c as Gt,l as ge,d as di}from"./formation-links-Bt44rMnM.js";function kt(e,{navigate:t,toast:i}){e.innerHTML=`
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),n=(d||[]).filter(_=>_.card_type==="player"&&_.player),r=(d||[]).filter(_=>_.card_type==="game_changer"),u=(d||[]).filter(_=>_.card_type==="formation"),{data:m}=await k.from("gc_definitions").select("name,gc_type,color,effect,image_url"),f={};(m||[]).forEach(_=>{f[_.name]=_});const $=Object.keys(it),p=Object.keys(de),y={};n.forEach(_=>{const b=_.player.id;y[b]=(y[b]||0)+1}),new Set(Object.keys(y).map(_=>String(_)));const E=new Set(u.map(_=>_.formation)),h=new Set(r.map(_=>_.gc_type));let g="player",v="Tous",L="",I=!1;function x(){return[...n].sort((_,b)=>{const j=Be.indexOf(_.player.job),A=Be.indexOf(b.player.job);return j!==A?j-A:(_.player.surname_encoded||"").localeCompare(b.player.surname_encoded||"")})}function T(){return[...l||[]].sort((_,b)=>{const j=Be.indexOf(_.job),A=Be.indexOf(b.job);return j!==A?j-A:(_.surname_encoded||"").localeCompare(b.surname_encoded||"")})}function G(){return x().filter(_=>{const b=_.player,j=v==="Tous"||b.job===v,A=!L||`${b.firstname} ${b.surname_encoded}`.toLowerCase().includes(L);return j&&A})}function N(){return T().filter(_=>{const b=v==="Tous"||_.job===v,j=!L||`${_.firstname} ${_.surname_encoded}`.toLowerCase().includes(L);return b&&j})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Ee=1e3;function hi(e,t,i,s){var p,y,E;const{state:c,toast:o,closeModal:a,navigate:d,refreshProfile:l}=i,n=e.formation,r={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function u(){const h=Ce[n]||{},g=it[n]||[],v=290,L=360,I=20;function x(G){const N=h[G];return N?{x:N.x*v,y:N.y*L}:null}let T=`<svg width="${v}" height="${L}" viewBox="0 0 ${v} ${L}" xmlns="http://www.w3.org/2000/svg">`;for(const[G,N]of g){const P=x(G),H=x(N);!P||!H||(T+=`<line x1="${P.x}" y1="${P.y}" x2="${H.x}" y2="${H.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const G of Object.keys(h)){const N=x(G);if(!N)continue;const P=G.replace(/\d+/,""),H=r[P]||"#555";T+=`<circle cx="${N.x}" cy="${N.y}" r="${I}" fill="${H}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,T+=`<text x="${N.x}" y="${N.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${P}</text>`}return T+="</svg>",T}const m=t.filter(h=>h.formation===n),f=m.length,$=!e.is_for_sale;s(`Formation ${n}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}const Ui="simulation",Vt="VOTRE_ZONE_ID";function Ki(){switch(Ui){case"propellerads":return Vi();case"adsense":return Yi();default:return st()}}function st(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:#111;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:14px";let i=5;t.innerHTML=`
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order");let l=0;function n(){var h,g,v,L,I;const r=o[l],u=(d||[]).filter(x=>x.deck_id===r.id),m=u.filter(x=>{var T;return x.is_starter&&((T=x.card)==null?void 0:T.player)}).map(x=>Je(x.card,x.position)),f=u.find(x=>{var T;return((T=x.card)==null?void 0:T.card_type)==="formation"}),$=r.formation||((h=f==null?void 0:f.card)==null?void 0:h.formation)||"4-4-2",p=m.length>=11?Oe(m,$):null,y=m.length>=11;Wt(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div id="deck-swipe-zone" style="flex:1;min-height:0;overflow:hidden;position:relative;touch-action:pan-y;display:flex;align-items:center;justify-content:center;padding:4px">
        ${p?`<div class="deck-preview-wrap" style="aspect-ratio:1/1;max-width:100%;max-height:100%;width:auto;height:100%;overflow:hidden">${Me(p,$,null,[],285,285)}</div>`:`<div style="display:flex;align-items:center;justify-content:center;height:100%;opacity:.4;flex-direction:column;gap:8px">
  ...
  <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div id="duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center, rgba(10,61,30,0.4), rgba(10,61,30,0.92))"></div>
          ...
          <button id="start-match-btn" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,0.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">
            ▶ Commencer le match
          </button>`,L.style.transition="opacity .45s ease",L.style.opacity="1",L.style.pointerEvents="auto",(I=document.getElementById("start-match-btn"))==null||I.addEventListener("click",$))},600)},700)},2800)}function to(e){if(e.type==="duel"){const t=e.homeTotal>=e.aiTotal;return`
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    ${(()=>{const x=window.innerWidth>=700,T=(z,B,R)=>{var K,X;const S=(t.gcDefs||[]).find(te=>te.name===z.gc_type),_={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[S==null?void 0:S.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",b={purple:"#b06ce0",light_blue:"#00d4ef"}[S==null?void 0:S.color]||"#b06ce0",j=(S==null?void 0:S.name)||z.gc_type,A=(S==null?void 0:S.effect)||((K=de[z.gc_type])==null?void 0:K.desc)||"",w=S!=null&&S.image_url?`/manager-wars/icons/${S.image_url}`:null,D=((X=de[z.gc_type])==null?void 0:X.icon)||"⚡",M=Math.round(R*.22),q=Math.round(R*.22),F=R-M-q,U=j.length>12?7:9;return`<div class="gc-mini" data-gc-id="${z.id}" data-gc-type="${z.gc_type}"
        ...
        </div>`},G=(z,B)=>{var R;return`<div id="boost-card"
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(x=>{var T,G,N;if(x.type==="duel"){const P=x.isGoal,H=x.homeScored?"#FFD700":P?"#ff6b6b":"rgba(255,255,255,0.3)",W=x.homeScored?"⚽ BUT !":P?"⚽ BUT IA !":(T=x.homePlayers)!=null&&T.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${P?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${H};margin-bottom:4px">
                ...
                ${(N=x.aiPlayers)!=null&&N.length?`<div style="opacity:0.7">${Ie(x.aiPlayers,"#ff6b6b",x.aiTotal)}</div>`:""}
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(o);let d=!1;const l=()=>{d||(d=!0,o.remove(),setTimeout(()=>c(),50))};o.addEventListener("click",l),setTimeout(l,3500)}async function we(e,t,i){var r,u;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:s}=i,c=t.homeScore>t.aiScore,o=t.homeScore===t.aiScore,a=c?"victoire":o?"nul":"defaite",d=fi(t.mode,a);t.matchId&&await k.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:c?s.profile.id:null,home_credits_reward:d,played_at:new Date().toISOString()}).eq("id",t.matchId);const l={credits:(s.profile.credits||0)+d,matches_played:(s.profile.matches_played||0)+1};c?l.wins=(s.profile.wins||0)+1:o?l.draws=(s.profile.draws||0)+1:l.losses=(s.profile.losses||0)+1,await k.from("users").update(l).eq("id",s.profile.id),await i.refreshProfile();const n=document.createElement("div");n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",n.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const f=()=>document.getElementById("mm-status"),$=async(g=!0)=>{u=!0,m&&(k.removeChannel(m),m=null),g&&await k.rpc("cancel_matchmaking",{p_user_id:l.profile.id}).catch(()=>{})};(h=document.getElementById("mm-cancel"))==null||h.addEventListener("click",async()=>{try{await $(!0)}catch(g){console.warn("[Matchmaking] cleanup error:",g)}le(e),n("home")});const p=async(g,v,L)=>{u||(u=!0,m&&(k.removeChannel(m),m=null),f()&&(f().textContent="Adversaire trouvé !"),await new Promise(I=>setTimeout(I,600)),e.isConnected!==!1&&go(e,t,g,L,a,d))},{data:y,error:E}=await k.rpc("try_matchmake",{p_user_id:l.profile.id,p_deck_id:i});if(E||!(y!=null&&y.success)){r("Erreur de matchmaking","error"),le(e),n("home");return}if(y.matched){p(y.match_id,y.opponent_id,!1);return}f()&&(f().textContent="En attente d'un autre joueur..."),m=k.channel("matchmaking-"+l.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${l.profile.id}`},g=>{const v=g.new;v.status==="matched"&&v.match_id&&p(v.match_id,v.matched_with,!0)}).subscribe()}async function go(e,t,i,s,c=[],o=[]){const{state:a,navigate:d,toast:l}=t,n=s?"p1":"p2",r=s?"p2":"p1",u=(c||[]).map(C=>C.id),m=(c||[]).map(C=>({id:C.id,gc_type:C.gc_type,_gcDef:C._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation du match...</div>';const{data:f}=await k.from("matches").select("*").eq("id",i).single();if(!f){l("Match introuvable","error"),d("home");return}async function $(){const[{data:C,error:z},{data:B,error:R},{data:S},{data:_}]=await Promise.all([k.rpc("get_deck_for_match",{p_deck_id:f.home_deck_id}),k.rpc("get_deck_for_match",{p_deck_id:f.away_deck_id}),k.from("users").select("id,pseudo,club_name").eq("id",f.home_id).single(),k.from("users").select("id,pseudo,club_name").eq("id",f.away_id).single()]);z&&console.error("[PvP] get_deck_for_match p1 error:",z),R&&console.error("[PvP] get_deck_for_match p2 error:",R);function b(F,U){return{cardId:F.card_id,position:U||F.position,id:F.id,firstname:F.firstname,name:F.surname_encoded,country_code:F.country_code,club_id:F.club_id,job:F.job,job2:F.job2,note_g:Number(F.note_g)||0,note_d:Number(F.note_d)||0,note_m:Number(F.note_m)||0,note_a:Number(F.note_a)||0,rarity:F.rarity,skin:F.skin,hair:F.hair,hair_length:F.hair_length,clubName:F.club_encoded_name||null,clubLogo:F.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}}const j=((C==null?void 0:C.starters)||[]).map(F=>b(F,F.position)),A=((B==null?void 0:B.starters)||[]).map(F=>b(F,F.position)),w=((C==null?void 0:C.subs)||[]).map(F=>b(F,F.position)),D=((B==null?void 0:B.subs)||[]).map(F=>b(F,F.position)),M=(C==null?void 0:C.formation)||"4-4-2",q=(B==null?void 0:B.formation)||"4-4-2";return console.log("[PvP] p1Formation:",M,"p1 starters:",j.length,"positions:",j.map(F=>F.position)),console.log("[PvP] p2Formation:",q,"p2 starters:",A.length,"positions:",A.map(F=>F.position)),{p1Team:Oe(j,M),p2Team:Oe(A,q),p1Subs:w,p2Subs:D,p1Formation:M,p2Formation:q,p1Name:(S==null?void 0:S.club_name)||(S==null?void 0:S.pseudo)||"Joueur 1",p2Name:(_==null?void 0:_.club_name)||(_==null?void 0:_.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:s?u:[],gc_p2:s?[]:u,gcCardsFull_p1:s?m:[],gcCardsFull_p2:s?[]:m,gcReady_p1:!0,gcReady_p2:!0,usedGc_p1:[],usedGc_p2:[],lastActionAt:new Date().toISOString()}}let p=f.game_state&&Object.keys(f.game_state).length?f.game_state:null;if(!p)if(s){p=await $();const{data:C}=await k.from("matches").select("game_state").eq("id",i).single();!(C!=null&&C.game_state)||!Object.keys(C.game_state).length?await k.from("matches").update({game_state:p,turn_user_id:f.home_id}).eq("id",i):p=C.game_state}else{e.innerHTML=`<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation avec l'adversaire...</div>`;for(let C=0;C<30&&!p;C++){await new Promise(B=>setTimeout(B,400));const{data:z}=await k.from("matches").select("game_state").eq("id",i).single();z!=null&&z.game_state&&Object.keys(z.game_state).length&&(p=z.game_state)}if(!p){l("Erreur de synchronisation","error"),d("home");return}p.gc_p2=u,p.gcCardsFull_p2=m,await k.from("matches").update({game_state:p}).eq("id",i)}let y=!1;function E(C){var j;try{k.removeChannel(h)}catch{}const z=a.profile.id,B=C.winner_id===z,R=!!C.forfeit,S=p[n+"Score"]??0,_=p[r+"Score"]??0,b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center",b.innerHTML=`
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 14px;background:rgba(0,0,0,0.25);flex-shrink:0">
        ...
        <div style="flex:1;text-align:center;color:#fff;font-size:14px;font-weight:700">
          ${R} <span style="color:#FFD700;font-size:18px">${z}</span> – <span style="color:#FFD700;font-size:18px">${B}</span> ${S}
        </div>
      ...
      <div style="text-align:center;padding:4px;background:rgba(0,0,0,0.15);font-size:11px;color:${j?"rgba(255,255,255,0.4)":"#FFD700"};font-weight:700;flex-shrink:0">
        ${j?`⏳ Tour de ${S}`:_?"⚔️ À vous d'attaquer !":b?"🛡️ À vous de défendre !":""}
      </div>
      <!-- Zone actions -->
    ...
    </div>`,C)return;const _=n==="p1"?f.home_id:f.away_id;k.from("cards").select("id,gc_type").eq("owner_id",_).eq("card_type","game_changer").then(async({data:j})=>{const{data:A}=await k.from("gc_definitions").select("*").eq("is_active",!0),w=new Set,D=(j||[]).filter(q=>w.has(q.gc_type)?!1:(w.add(q.gc_type),!0)).map(q=>({...q,_gcDef:(A==null?void 0:A.find(F=>F.name===q.gc_type))||null})),M=document.getElementById("pvp-gc-grid");if(M){if(!D.length){M.innerHTML='<div style="color:rgba(255,255,255,0.4);font-size:13px;text-align:center;margin-top:30px">Aucune carte Game Changer disponible.</div>';return}M.innerHTML=D.map(q=>S(q,B.includes(q.id))).join(""),M.querySelectorAll(".pvp-gc-card").forEach(q=>{q.addEventListener("click",()=>{const F=q.dataset.id,U=B.indexOf(F);U>-1?B.splice(U,1):B.length<R&&B.push(F),L()})})}}),(b=e.querySelector("#pvp-gc-validate"))==null||b.addEventListener("click",async()=>{const j=n==="p1"?f.home_id:f.away_id,{data:A}=await k.from("cards").select("id,gc_type").eq("owner_id",j).eq("card_type","game_changer").in("id",B),{data:w}=await k.from("gc_definitions").select("*").eq("is_active",!0),D=(A||[]).map(M=>({...M,_gcDef:(w==null?void 0:w.find(q=>q.name===M.gc_type))||null}));await g({["gcReady_"+n]:!0,["gcCardsFull_"+n]:D}),p["gcReady_"+r]&&await g({phase:"reveal"})})}function x(){e.style.overflow="hidden",e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    </div>`,setTimeout(()=>{const A=document.getElementById("pvp-score-me"),w=document.getElementById("pvp-score-opp"),D=document.getElementById("pvp-midfield-result");A&&w&&(b?(A.style.fontSize="80px",A.style.color="#FFD700",w.style.opacity="0.25"):(w.style.fontSize="80px",w.style.color="#ff6b6b",A.style.opacity="0.25"));const M=p.p1Team.MIL||[],q=p.p2Team.MIL||[],F=B(M)+R(M),U=B(q)+R(q),K=F>=U?"p1":"p2";p[K+"Name"];let X=p.boostValue;X==null&&(X=Yt(),p.boostValue=X,p.boostOwner=K,p.boostUsed=!1),D&&(D.style.opacity="1",D.innerHTML=`<div style="font-size:20px;font-weight:900;margin-bottom:10px">
      ...
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${z.map(S=>{const _={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[S._line]||"#555",b=ie(S,S._line)+(S.boost||0);return`<div class="boost-pick-item" data-cid="${S.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${_};overflow:hidden;cursor:pointer">
      ...
      <div style="width:min(90vw,420px)">${Le(p[r+"Team"],p[r+"Formation"],null,[],300,300)}</div>
      <button id="pvp-view-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(C),(z=C.querySelector("#pvp-view-opp-close"))==null||z.addEventListener("click",()=>C.remove())}function H(C,z){var F,U;const R=(p["gcCardsFull_"+n]||[]).find(K=>K.id===C),S=R==null?void 0:R._gcDef,_=de[z]||{icon:"⚡",desc:"Carte spéciale."},b={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[S==null?void 0:S.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",j={purple:"#b06ce0",light_blue:"#00d4ef"}[S==null?void 0:S.color]||"#b06ce0",A=(S==null?void 0:S.name)||z,w=(S==null?void 0:S.effect)||_.desc,D=S!=null&&S.image_url?`/manager-wars/icons/${S.image_url}`:null,M=_.icon||"⚡",q=document.createElement("div");q.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",q.innerHTML=`
      ...
      </div>`,document.body.appendChild(q),(F=q.querySelector("#pvp-gc-back"))==null||F.addEventListener("click",()=>q.remove()),(U=q.querySelector("#pvp-gc-use"))==null||U.addEventListener("click",async()=>{q.remove();const K=p["usedGc_"+n]||[];K.push(C),await g({["usedGc_"+n]:K})})}function W(){var S;const C=p[n+"Score"],z=p[r+"Score"],B=C>z,R=C===z;e.style.overflow="hidden",e.innerHTML=`
    ...
    </div>`,(S=document.getElementById("pvp-home"))==null||S.addEventListener("click",()=>{k.removeChannel(h),le(e),d("home")})}async function V(){const C=(p["selected_"+n]||[]).map(R=>({...(p[n+"Team"][R._role]||[]).find(_=>_.cardId===R.cardId)||R,_line:R._role}));if(!C.length)return;const z=ot(C,p.modifiers[n]||{});C.forEach(R=>{const S=(p[n+"Team"][R._role]||[]).find(_=>_.cardId===R.cardId);S&&(S.used=!0)});const B=Array.isArray(p.log)?p.log:[];B.push({type:"attack",text:`⚔️ ${p[n+"Name"]} attaque (${z.total})`,players:C,total:z.total,side:n}),await g({pendingAttack:{...z,players:C,side:n},["selected_"+n]:[],modifiers:{...p.modifiers,[n]:{}},phase:r+"-defense",log:B})}async function ee(){const C=(p["selected_"+n]||[]).map(D=>({...(p[n+"Team"][D._role]||[]).find(q=>q.cardId===D.cardId)||D,_line:D._role})),z=nt(C,p.modifiers[n]||{});C.forEach(D=>{const M=(p[n+"Team"][D._role]||[]).find(q=>q.cardId===D.cardId);M&&(M.used=!0)});const B=rt(p.pendingAttack.total,z.total,p.modifiers[n]||{}),R=p.pendingAttack.side,S=B==="attack",_=p[R+"Score"]+(S?1:0),b=R==="p1"?"p2":"p1",j=(p.round||0)+1,A=j>10,w=Array.isArray(p.log)?p.log:[];w.push({type:"defense",text:`🛡️ ${p[n+"Name"]} défend (${z.total})`,players:C,total:z.total,side:n}),w.push({type:S?"goal":"stop",text:S?`⚽ BUT de ${p[R+"Name"]} ! (${p.pendingAttack.total} vs ${z.total})`:`✋ Attaque stoppée (${p.pendingAttack.total} vs ${z.total})`}),await g({[R+"Score"]:_,["selected_"+n]:[],modifiers:{...p.modifiers,[n]:{}},pendingAttack:null,phase:A?"finished":b+"-attack",attacker:b,round:j,log:w}),A&&await k.from("matches").update({status:"finished"}).eq("id",i)}L()}const mo={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"};async function xo(e,t){const{state:i,toast:s}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement du marché...</div>',await ct(e,t)}async function ct(e,t){const{state:i,toast:s}=t,{data:c}=await k.from("market_listings").select(`id, price, status, listed_at, seller_id,
  ...
  `;function l(n){const r=document.getElementById("mkt-content"),u=n==="buy"?a:d;if(u.length===0){r.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      ${m.map(f=>{var g,v,L,I;const $=(g=f.card)==null?void 0:g.player;if(!$)return"";const p=$.job==="GK"?$.note_g:$.job==="DEF"?$.note_d:$.job==="MIL"?$.note_m:$.note_a,y=mo[$.rarity],E=(i.profile.credits||0)>=f.price,h=f.status==="sold";return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${h?"opacity:0.65":""}">
          <div style="width:44px;height:44px;border-radius:8px;background:${vo($.job)};color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:18px;border:2px solid ${y};flex-shrink:0">${p}</div>
          <div style="flex:1;min-width:0">
