...
import{s as w,F as mi,r as Me,j as Bt,l as Yi,m as qt,h as xi,n as En,o as Ln,T as ei,k as Tn,p as zn,b as In}from"./formation-links-EITwj297.js";const Mn="/";function ji(e,{navigate:t,toast:i}){let n="login";const a=()=>{var r,s,l,c,d,u;const o=n==="login";e.innerHTML=`
    <div style="
  ...
  `;function s(){var f;const c=document.getElementById("logo-preview"),d=document.getElementById("logo-initials"),u=((f=document.getElementById("setup-club"))==null?void 0:f.value)||"MW",g=u.trim().split(" ").filter(Boolean),m=g.length>=2?(g[0][0]+g[1][0]).toUpperCase():u.slice(0,2).toUpperCase();c&&(c.style.background=r,c.style.borderColor=o),d&&(d.textContent=m,d.style.color=o)}document.getElementById("color1").addEventListener("input",c=>{o=c.target.value,document.getElementById("swatch1").style.background=o,s()}),document.getElementById("color2").addEventListener("input",c=>{r=c.target.value,document.getElementById("swatch2").style.background=r,s()});function l(c){document.querySelectorAll(".setup-step").forEach(d=>d.classList.remove("active")),document.getElementById(`step-${c}`).classList.add("active"),document.getElementById("step-num").textContent=c,document.getElementById("progress-fill").style.width=`${Math.round(c/3*100)}%`,c===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),d=document.getElementById("step1-error");if(d.textContent="",c.length<3){d.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:u}=await w.from("users").select("id").eq("pseudo",c).maybeSingle();if(u){d.textContent="Ce pseudo est déjà pris.";return}l(2)}),document.getElementById("step2-back").addEventListener("click",()=>l(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const c=document.getElementById("setup-club").value.trim(),d=document.getElementById("step2-error");if(d.textContent="",c.length<2){d.textContent="Nom trop court (min. 2 caractères).";return}const{data:u}=await w.from("users").select("id").eq("club_name",c).maybeSingle();if(u){d.textContent="Ce nom de club est déjà pris.";return}l(3)}),document.getElementById("step3-back").addEventListener("click",()=>l(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),d=document.getElementById("setup-club").value.trim(),u=document.getElementById("step3-error"),g=document.getElementById("step3-finish");u.textContent="",g.disabled=!0,g.textContent="Création en cours…";try{const{error:m}=await w.from("users").insert({id:t.user.id,pseudo:c,club_name:d,club_color1:o,club_color2:r,credits:1e4});if(m)throw m;await An(t.user.id),await a(),n(`Bienvenue ${c} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(m){u.textContent=m.message,g.disabled=!1,g.textContent="🚀 Créer mon profil !"}})}async function An(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await w.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const jn="modulepreload",Cn=function(e){return"/"+e},Ci={},Wi=function(t,i,n){let a=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const r=document.querySelector("meta[property=csp-nonce]"),s=(r==null?void 0:r.nonce)||(r==null?void 0:r.getAttribute("nonce"));a=Promise.allSettled(i.map(l=>{if(l=Cn(l),l in Ci)return;Ci[l]=!0;const c=l.endsWith(".css"),d=c?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${l}"]${d}`))return;const u=document.createElement("link");if(u.rel=c?"stylesheet":jn,c||(u.as="script"),u.crossOrigin="",u.href=l,s&&u.setAttribute("nonce",s),document.head.appendChild(u),c)return new Promise((g,m)=>{u.addEventListener("load",g),u.addEventListener("error",()=>m(new Error(`Unable to preload CSS for ${l}`)))})}))}function o(r){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=r,window.dispatchEvent(s),!s.defaultPrevented)throw r}return a.then(r=>{for(const s of r||[])s.status==="rejected"&&o(s.reason);return t().catch(o)})},St="#1A6B3C",At="#cc2222",Bn="#D4A017",Bi="#888";async function qn(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    </div>`}function Fn(e,t){const i=yi();i.innerHTML=`
    <div class="popup-box">
            ...
            ${d.map(g=>{var m,f,p;return`
              <div style="display:flex;align-items:center;gap:10px;background:#f9f9f9;border-radius:10px;padding:10px 12px">
                ...
                <button data-accept="${g.id}" title="Accepter"
                  style="width:34px;height:34px;border-radius:50%;border:none;background:${St};color:#fff;font-size:18px;cursor:pointer">✓</button>
    ...
    ${bi()}`,document.body.appendChild(c);const u=()=>c.remove();c.querySelector("#pending-close").addEventListener("click",u),c.addEventListener("click",g=>{g.target===c&&u()}),c.querySelectorAll("[data-accept]").forEach(g=>{g.addEventListener("click",async()=>{const{error:m}=await w.from("friendships").update({status:"accepted"}).eq("id",g.dataset.accept);if(m){t("Erreur : "+m.message,"error");return}g.closest("div[style]").remove(),t("✅ Ami accepté !","success"),Xi(e,t,n),i&&i()})}),c.querySelectorAll("[data-decline]").forEach(g=>{g.addEventListener("click",async()=>{await w.from("friendships").delete().eq("id",g.dataset.decline),g.closest("div[style]").remove(),t("Demande refusée","info"),i&&i()})})}async function Gn(e,t,i){const n=e.user.id,[a,o]=[n,t].sort(),r=n===a,{data:s}=await w.from("friend_match_stats").select("*").eq("player1_id",a).eq("player2_id",o).single(),l=e.profile.club_name||e.profile.pseudo||"Moi",c=s||{},d=r?c.wins_p1||0:c.wins_p2||0,u=r?c.wins_p2||0:c.wins_p1||0,g=c.draws||0,m=r?c.goals_p1||0:c.goals_p2||0,f=r?c.goals_p2||0:c.goals_p1||0,p=r?c.gc_used_p1||0:c.gc_used_p2||0,x=r?c.gc_used_p2||0:c.gc_used_p1||0,b=c.matches_total||0,v=($,y,B,E=St,G=At)=>`
    ...
    </style>`}const Pn="2026.07.17-2133";async function qi(e,{state:t,navigate:i,toast:n}){var o,r;const a=t.profile;a&&(e.innerHTML=`
  <style>
    .home-dark {
      min-height: 100%;
  ...
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
      ...
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
        ...
        <div style="display:flex;justify-content:flex-end">
          ...
          <div style="height:150px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
            ${rt?`<img src="${rt}" style="max-width:120px;max-height:120px;object-fit:contain;border-radius:6px">`:`<span style="font-size:64px">${q.icon}</span>`}
          </div>
        ...
        </div>`},({def:q,count:O})=>{var Le,ge;const we=window.innerWidth>=768?.76:.54,_e=(q==null?void 0:q.name)||"?",be=((Le=q==null?void 0:q.club)==null?void 0:Le.encoded_name)||(q==null?void 0:q.country_code)||"—",he=q!=null&&q.image_url?`${F}icons/${q.image_url}`:((ge=q==null?void 0:q.club)==null?void 0:ge.logo_url)||(q!=null&&q.country_code?`https://flagsapi.com/${q.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),$e=he?`<img src="${he}" style="width:64px;height:64px;object-fit:contain;border-radius:4px" onerror="this.style.display='none'">`:'<span style="font-size:32px">🏟️</span>';return`<div style="display:inline-block;zoom:${we};line-height:0;pointer-events:none">
        ...
        </div>`},null)}e.querySelectorAll(".col-tab-btn").forEach(I=>{I.addEventListener("click",()=>{B=I.dataset.tab,E="Tous",G="",N=!1,e.querySelectorAll(".col-tab-btn").forEach(C=>{const F=C.dataset.tab===B;C.style.borderBottomColor=F?"var(--green)":"transparent",C.style.color=F?"var(--green)":"var(--gray-600)"}),_(),S()})}),_(),S()}function no(e,t,i){const n=Se[e]||{icon:"⚡",desc:"Effet spécial."},a=(t==null?void 0:t.gc_type)==="ultra_game_changer",o={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},r={purple:"#b06ce0",light_blue:"#00d4ef"},s=o[t==null?void 0:t.color]||o.purple,l=r[t==null?void 0:t.color]||r.purple,c=(t==null?void 0:t.name)||e,d=(t==null?void 0:t.effect)||n.desc,u=t!=null&&t.image_url?`/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Pt=1e3;function oo(e,t,i,n){var f,p,x;const{state:a,toast:o,closeModal:r,navigate:s,refreshProfile:l}=i,c=e.formation,d={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function u(){const b=Bt[c]||{},v=mi[c]||[],h=290,$=360,y=20;function B(G){const N=b[G];return N?{x:N.x*h,y:N.y*$}:null}let E=`<svg width="${h}" height="${$}" viewBox="0 0 ${h} ${$}" xmlns="http://www.w3.org/2000/svg">`;for(const[G,N]of v){const ne=B(G),ie=B(N);!ne||!ie||(E+=`<line x1="${ne.x}" y1="${ne.y}" x2="${ie.x}" y2="${ie.y}"
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",d.note_g],["DEF",d.note_d],["MIL",d.note_m],["ATT",d.note_a]].map(([Y,J])=>{const P=ti[Y],I=Y==="GK"?"#fff":P,C=Y===d.job||Y===d.job2,F=(Number(J)||0)+(C&&B>0?B:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(ae=document.getElementById("close-detail"))==null||ae.addEventListener("click",s);let S=new Set;const U=()=>{const Y=S.size,J=document.getElementById("sell-action-panel");if(!J)return;J.style.display=Y>0?"block":"none",document.getElementById("sell-selected-count").textContent=Y;const P=document.getElementById("evolve-btn");P&&(P.textContent=`⬆️ Évoluer ${Y>1?"(+"+Y+")":""}`)};document.querySelectorAll(".expl-check").forEach(Y=>{Y.addEventListener("change",()=>{const J=Y.dataset.id;Y.checked?S.add(J):S.delete(J);const P=Y.closest(".exemplaire-row");P&&(P.style.borderColor=Y.checked?"#1A6B3C":"#eee"),U()})}),document.querySelectorAll(".exemplaire-row").forEach(Y=>{Y.addEventListener("click",J=>{if(J.target.closest("button")||J.target.tagName==="INPUT")return;const P=Y.querySelector(".expl-check");P&&!P.disabled&&(P.checked=!P.checked,P.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(Y=>{Y.addEventListener("click",J=>{J.stopPropagation();const P=document.querySelector(`.expl-hist[data-hist="${Y.dataset.idx}"]`);P&&(P.style.display=P.style.display==="none"?"flex":"none")})}),(se=document.getElementById("evolve-btn"))==null||se.addEventListener("click",async()=>{if(g<=1)return;const Y=[...S];if(!Y.length)return;if(S.has(e.id)){const K=document.createElement("div");K.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",K.innerHTML=`
  ...
  </div>`,document.getElementById("new-deck-btn").addEventListener("click",async()=>{const r=prompt("Nom du deck :",`Deck ${((o==null?void 0:o.length)||0)+1}`);if(!r)return;const{data:s,error:l}=await w.from("decks").insert({owner_id:i.profile.id,name:r}).select().single();if(l){a(l.message,"error");return}a("Deck créé !","success"),Ni(s.id,e,t)}),e.querySelectorAll("[data-open-deck]").forEach(r=>{r.addEventListener("click",()=>Ni(r.dataset.openDeck,e,t))}),e.querySelectorAll("[data-rename]").forEach(r=>{r.addEventListener("click",async()=>{const s=prompt("Nouveau nom :",r.dataset.name);if(!s||s===r.dataset.name)return;const{error:l}=await w.from("decks").update({name:s}).eq("id",r.dataset.rename);if(l){a(l.message,"error");return}a("Deck renommé !","success"),ai(e,t)})}),e.querySelectorAll("[data-delete]").forEach(r=>{r.addEventListener("click",async()=>{if(!confirm(`Supprimer le deck "${r.dataset.name}" ? Cette action est irréversible.`))return;await w.from("deck_cards").delete().eq("deck_id",r.dataset.delete);const{error:s}=await w.from("decks").delete().eq("id",r.dataset.delete);if(s){a(s.message,"error");return}a("Deck supprimé.","success"),ai(e,t)})})}async function Ni(e,t,i){const{state:n,toast:a}=i;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await w.from("decks").select("*").eq("id",e).single(),{data:r}=await w.from("cards").select(`id, card_type, formation, stadium_id, evolution_bonus,
      player:players(id, firstname, surname_real, country_code, club_id, job, job2,
  ...
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
  ...
  </div>`;const d=window.innerWidth>=900,u=e.querySelector(".deck-pc-layout"),g=e.querySelector(".deck-mobile-layout");u&&(u.style.display=d?"block":"none"),g&&(g.style.display=d?"none":"block"),ro(e,t,a,i),e.querySelectorAll("#builder-back").forEach(x=>x.addEventListener("click",()=>n("decks")));const m=()=>{const{openModal:x,closeModal:b}=i,v=`<div style="display:flex;flex-wrap:wrap;gap:8px;padding:8px">
    ...
    </div>`;x("⚽ Choisir une formation",v),document.querySelectorAll("#modal-body [data-forma]").forEach(h=>{h.addEventListener("click",()=>{t.formation=h.dataset.forma;const $=Oi(t.formation),y={};$.forEach(B=>{t.slots[B]&&(y[B]=t.slots[B])}),t.slots=y,b(),nt(e,t,i)})})};e.querySelectorAll("#formation-mobile-btn, #formation-pc-btn").forEach(x=>x.addEventListener("click",m)),e.querySelectorAll("#add-stad-btn-pc, #add-stad-btn").forEach(x=>x.addEventListener("click",()=>ao(t,e,i))),e.querySelectorAll("#save-deck").forEach(x=>x.addEventListener("click",()=>co(t,i))),e.querySelectorAll("[data-remove-sub]").forEach(x=>{x.addEventListener("click",()=>{t.subs=t.subs.filter(b=>b!==x.dataset.removeSub),nt(e,t,i)})}),e.querySelectorAll("#add-sub-btn").forEach(x=>x.addEventListener("click",()=>lo(t,e,i)))}function ro(e,t,i,n){var h,$;const a=window.innerWidth>=900,o=e.querySelector(a?"#deck-field-pc":"#deck-field-mobile");if(!o)return;const r=(h=t.stadiumCards)==null?void 0:h.find(y=>y.id===t.stadiumCardId),s=r&&(($=t.stadDefMap)==null?void 0:$[r.stadium_id])||null,l=Bt[t.formation]||{},c=Yi(t.formation)||[],d={};for(const y of i){const B=t.slots[y],E=B?t.playerCards.find(G=>G.id===B):null;E!=null&&E.player?d[y]={...E.player,_evolution_bonus:E.evolution_bonus||0,face:E.player.face||null}:d[y]=null}const u=window.innerWidth>=900,g=u?window.innerWidth-280:window.innerWidth-20,m=u?Math.min(g,860):g,f=Math.round(u?m*.82:m*.85),p=u?84:44;let x="";for(const[y,B]of c){const E=l[y],G=l[B];if(!E||!G)continue;const N=E.x*m,ne=Math.round(.03*f+E.y*.85*f),ie=G.x*m,Q=Math.round(.03*f+G.y*.85*f),j=d[y],_=d[B],S=qt(j,_);S==="#ff3333"||S==="#cc2222"?x+=`<line x1="${N.toFixed(1)}" y1="${ne.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${Q.toFixed(1)}" stroke="${S}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(x+=`<line x1="${N.toFixed(1)}" y1="${ne.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${Q.toFixed(1)}" stroke="${S}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,x+=`<line x1="${N.toFixed(1)}" y1="${ne.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${Q.toFixed(1)}" stroke="${S}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let b="";const v=Math.round(p*657/507);for(const y of i){const B=l[y];if(!B)continue;const E=d[y],G=B.x*m;B.y*f;const N=Math.round(.03*f+B.y*(.85*f)),ne=Math.round(G-p/2),ie=Math.round(N-v/2);if(E){const Q=y.replace(/\d+/,""),j=s&&(s.club_id&&String(E.club_id)===String(s.club_id)||s.country_code&&E.country_code===s.country_code),_=Me({...E,_evolution_bonus:E._evolution_bonus||0},{width:p,showStad:!0,stadDef:s,role:Q});b+=`<div style="position:absolute;left:${ne}px;top:${ie}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${y}">
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}function _o(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText=`
    ...
    `,document.body.appendChild(t);let i=5;const n=setInterval(()=>{i--;const a=document.getElementById("mw-ad-cd");a&&(a.textContent=i),i<=0&&(clearInterval(n),t.remove(),e(!0))},1e3)})}async function ko(e,{state:t,navigate:i,toast:n,refreshProfile:a}){var g,m;const{data:o}=await w.from("users").select("*").eq("id",t.user.id).single();o&&(t.profile=o);let r=Array.isArray((g=t.profile)==null?void 0:g.pending_boosters)?[...t.profile.pending_boosters]:[];if(!r.length){await w.from("users").update({onboarding_done:!0}).eq("id",t.user.id),i("home");return}let s=null;try{const p=(await on()).find(x=>(x.name||"").toLowerCase().includes("new player"));p&&(s=an(p))}catch(f){console.warn('[Onboarding] Config "Booster (new player)" introuvable, fallback taux par défaut',f)}const l=r.length;let c=0;e.innerHTML=`
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    ${m}
  </svg>`}function $t(e,t,i,n,a=300,o=300,r=[],s=null){return`<div id="match-terrain-wrap" style="position:relative;padding:0 4px">
    ${Jt(e,t,i,n,a,o,r,s)}
    ...
    </div>`}function fn(e){var o,r,s;if(!e)return"";const t=l=>l?Me({...l,_evolution_bonus:0},{width:52,role:l._line||l.job,showStad:!!l.stadiumBonus,extraNote:l.boost||0}):"",n={goal:"⚽","goal-home":"⚽","goal-ai":"⚽",duel:"⚔️",midfield:"🎯",sub:"🔄",gc:"⚡",boost:"💥","defense-won":"🛡️","attack-won":"⚔️","defense-lost":"😓","attack-lost":"😓"}[e.type]||"📋";return`
    ...
    <div style="animation:goalPop 0.4s ease-out;text-align:center">
      ...
      }`,document.head.appendChild(l)}setTimeout(()=>{r.style.opacity="0",r.style.transition="opacity 0.4s",setTimeout(()=>{r.remove(),a==null||a()},400)},1800)}function Io(e,t,i){const n=document.getElementById("sub-anim-overlay");n&&n.remove();const a=document.createElement("div");a.id="sub-anim-overlay",a.style.cssText=`
    position:fixed;inset:0;z-index:3000;
    display:flex;flex-direction:column;align-items:center;justify-content:center;
    ...
    </div>`,document.body.appendChild(a),setTimeout(()=>{a.style.opacity="0",a.style.transition="opacity 0.4s",setTimeout(()=>{a.remove(),i==null||i()},400)},1600)}function yt(e,t="rgba(0,0,0,0.85)",i=2200){const n=document.getElementById("game-toast");n&&n.remove();const a=document.createElement("div");if(a.id="game-toast",a.style.cssText=`
    ...
    pointer-events:none;white-space:nowrap;max-width:90vw;text-align:center`,a.textContent=e,!document.getElementById("game-toast-style")){const o=document.createElement("style");o.id="game-toast-style",o.textContent=`
      ...
      }`,document.head.appendChild(o)}document.body.appendChild(a),setTimeout(()=>{a.style.opacity="0",a.style.transition="opacity 0.3s",setTimeout(()=>a.remove(),300)},i)}function gn(e,t){const i=De(e,"MIL"),n=e.stadiumBonus||t&&(t.club_id&&String(e.club_id)===String(t.club_id)||t.country_code&&e.country_code===t.country_code)?10:0;return i+n}function Ze(e,t){return e.reduce((i,n)=>i+gn(n,t),0)}function et(e){let t=0;for(let i=0;i<e.length-1;i++){const n=qt(e[i],e[i+1]);n==="#00ff88"?t+=10:n==="#FFD700"&&(t+=5)}return t}function Kt(e,t,i,n,a){return`<div id="duel-row-${n}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0), opacity .5s ease;transform-origin:center">
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${e.map((o,r)=>{const s=r<e.length-1?qt(o,e[r+1]):null,l=!s||s==="#ff3333"||s==="#cc2222",c=s==="#00ff88"?"+10":s==="#FFD700"?"+5":"";return gn(o,a),o.stadiumBonus||a&&(a.club_id&&String(o.club_id)===String(a.club_id)||a.country_code&&(o.country_code,a.country_code)),`
      ...
      <div class="duel-score-line duel-score-line-${n}" style="margin-top:10px;font-size:12px;color:rgba(255,255,255,0.55);opacity:0;transition:opacity .4s ease">
        Score: ${Ze(e,a)} + ${et(e)} liens = <b style="color:#fff">${Ze(e,a)+et(e)}</b>
      </div>
    </div>`}async function Mo(e,t){const{state:i}=t,a=(i.params||{}).matchMode||"vs_ai_easy",o=a.replace("vs_ai_",""),r=a;await Dt(e,t,a,async({deckId:s,formation:l,starters:c,subsRaw:d,gcCardsEnriched:u,gcDefs:g,stadiumDef:m})=>{try{let f=Ut(c,l);m&&(f=Ct(f,m),Ht(d,m));const p=await So(l,o),x=p.lines||p,b=async v=>{try{const{data:h,error:$}=await w.from("matches").insert({home_id:i.profile.id,away_id:null,mode:r,home_deck_id:s,status:"in_progress"}).select().single();if($){console.error("[MatchIA] Erreur création match:",$),lt(e,"⚠️","Impossible de créer le match ("+$.message+").","Retour",()=>t.navigate("home"));return}const y=p.stadiumDef||null;y&&x&&(Ct(x,y),Ht(p.subs||[],y));const B={gcDefs:g||[],matchId:h==null?void 0:h.id,mode:r,difficulty:o,formation:l,homeTeam:f,aiTeam:x,homeSubs:d,subsUsed:0,maxSubs:Math.min(d.length,3),aiSubs:p.subs||[],aiSubsUsed:0,aiMaxSubs:Math.min((p.subs||[]).length,3),aiUsedSubIds:[],aiGcCards:p.gcCards||[],aiUsedGc:[],aiStadiumDef:y,homeScore:0,aiScore:0,gcCards:v,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};jo(e,B,t)}catch(h){console.error("[MatchIA] Exception launchMatch:",h),lt(e,"⚠️","Erreur au lancement du match : "+h.message,"Retour",()=>t.navigate("home"))}};if(!u.length){b([]);return}vi(e,u,b)}catch(f){console.error("[MatchIA] Exception setup:",f),lt(e,"⚠️","Erreur de préparation du match : "+f.message,"Retour",()=>t.navigate("home"))}})}async function So(e,t){var x;const{data:i}=await w.from("players").select("id,firstname,surname_real,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(80);if(!i||i.length<11)return{lines:Ao(e),subs:[],gcCards:[],stadiumDef:null};const n=_t[e]||_t["4-4-2"],a={GK:[],DEF:[],MIL:[],ATT:[]},o=new Set;function r(b,v,h){var $,y;return o.add(b.id),{cardId:"ai-"+b.id+"-"+h,id:b.id,firstname:b.firstname,name:b.surname_real,country_code:b.country_code,club_id:b.club_id,job:b.job,job2:b.job2,note_g:Number(b.note_g)||0,note_d:Number(b.note_d)||0,note_m:Number(b.note_m)||0,note_a:Number(b.note_a)||0,rarity:b.rarity,skin:b.skin,hair:b.hair,hair_length:b.hair_length,clubName:(($=b.clubs)==null?void 0:$.encoded_name)||null,clubLogo:((y=b.clubs)==null?void 0:y.logo_url)||null,boost:0,used:!1,_line:v}}for(const b of["GK","DEF","MIL","ATT"]){const v=i.filter(E=>E.job===b&&!o.has(E.id)),h=i.filter(E=>E.job!==b&&!o.has(E.id)),$=[...v,...h],y=[];for(let E=0;E<n[b];E++){const G=$[E];G&&y.push(r(G,b,E))}const B=kt(y.length);y.forEach((E,G)=>{E._col=B[G]}),a[b]=y}const l=i.filter(b=>!o.has(b.id)).slice(0,5).map((b,v)=>r(b,b.job,100+v)),u=Object.keys(Se).sort(()=>Math.random()-.5).slice(0,3).map((b,v)=>{var h,$;return{id:"ai-gc-"+v,gc_type:b,name:((h=Se[b])==null?void 0:h.name)||b,icon:(($=Se[b])==null?void 0:$.icon)||"⚡"}}),g=Object.values(a).flat(),m={};g.forEach(b=>{b.club_id&&(m[b.club_id]=(m[b.club_id]||0)+1)});const f=(x=Object.entries(m).sort((b,v)=>v[1]-b[1])[0])==null?void 0:x[0];let p=null;if(f){const{data:b}=await w.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",f).single();b&&(p={club_id:b.id,country_code:null,name:b.encoded_name+" Stadium",club:{encoded_name:b.encoded_name,logo_url:b.logo_url}})}return{lines:a,subs:l,gcCards:u,stadiumDef:p}}function Ao(e){const t=_t[e]||_t["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},n=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let a=0;for(const o of["GK","DEF","MIL","ATT"]){const r=[];for(let l=0;l<t[o];l++){const c=3+Math.floor(Math.random()*5);r.push({cardId:"fake-"+a,id:"fake-"+a,firstname:"IA",name:n[a%n.length],country_code:"XX",club_id:null,job:o,job2:null,note_g:o==="GK"?c:2,note_d:o==="DEF"?c:2,note_m:o==="MIL"?c:2,note_a:o==="ATT"?c:2,rarity:"normal",boost:0,used:!1,_line:o}),a++}const s=kt(r.length);r.forEach((l,c)=>{l._col=s[c]}),i[o]=r}return i}function jo(e,t,i){var a;const n=`<div style="position:relative;width:22px;height:22px;flex-shrink:0;display:flex;align-items:center;justify-content:center">
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const S=t.pendingAttack;let U="";if(t.selected.length>0){const re=t.selected.map(se=>({...(t.homeTeam[se._role]||[]).find(ke=>ke.cardId===se.cardId)||se,_line:se._role})),ae=Xt(re,t.modifiers.home);U=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
              ...
              <div style="display:flex;justify-content:center">${Qe(re.map(se=>({...se,used:!1})),"#3a7bd5",ae.total)}</div>
            ...
            <div style="display:flex;justify-content:center">${Qe((S.players||[]).map(re=>({...re,used:!1})),"#ff6b6b",S.total)}</div>
            ${U}
            ...
            <div style="display:flex;justify-content:center">${Qe((S.players||[]).map(U=>({...U,used:!1})),"#00ff88",S.total)}</div>
          </div>`}if(t.phase==="attack"&&t.selected.length>0){const S=t.selected.map(re=>{const ae=(t.homeTeam[re._role]||[]).find(de=>de.cardId===re.cardId)||re,se=["GK","DEF"].includes(re._role);return{...ae,_line:re._role,...se?{note_a:Math.max(1,Number(ae.note_a)||0)}:{}}}),U=Wt(S,t.modifiers.home);return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #FFD700;text-align:center">
            ...
            <div style="display:flex;justify-content:center">${Qe(S.map(re=>({...re,used:!1})),"#FFD700",U.total)}</div>
    ...
    ${(()=>{const _=window.innerWidth>=700,S=(P,I,C)=>{var ge,je;const F=(t.gcDefs||[]).find(Ae=>Ae.name===P.gc_type),W={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[F==null?void 0:F.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",K={purple:"#b06ce0",light_blue:"#00d4ef"}[F==null?void 0:F.color]||"#b06ce0",q=(F==null?void 0:F.name)||P.gc_type,O=(F==null?void 0:F.effect)||((ge=Se[P.gc_type])==null?void 0:ge.desc)||"",we=F!=null&&F.image_url?`/icons/${F.image_url}`:null,_e=((je=Se[P.gc_type])==null?void 0:je.icon)||"⚡",be=Math.round(C*.22),he=Math.round(C*.22),$e=C-be-he,Le=q.length>12?7:9;return`<div class="gc-mini" data-gc-id="${P.id}" data-gc-type="${P.gc_type}"
        ...
        <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
          ${$t(t.homeTeam,t.formation,t.phase,n,_?1300:ct(),_?600:ht(),c)}
        </div>
      </div>`;return _?`
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(_=>{var S,U,re;if(_.type==="duel"){const ae=_.isGoal,se=_.homeScored?"#FFD700":ae?"#ff6b6b":"rgba(255,255,255,0.3)",de=_.homeScored?"⚽ BUT !":ae?"⚽ BUT IA !":(S=_.homePlayers)!=null&&S.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${ae?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${se};margin-bottom:4px">
                ...
                ${(U=_.homePlayers)!=null&&U.length?`<div style="margin-bottom:3px">${Qe(_.homePlayers,"rgba(255,255,255,0.7)",_.homeTotal)}</div>`:""}
                ${(re=_.aiPlayers)!=null&&re.length?`<div style="opacity:0.7">${Qe(_.aiPlayers,"#ff6b6b",_.aiTotal)}</div>`:""}
                ...
                <div style="display:flex;align-items:center;gap:8px">
                  ${_.outPlayer?it({..._.outPlayer,used:!0,_line:_.outPlayer.job,rarity:"normal"},38,50):""}
                  <span style="color:rgba(255,255,255,0.4);font-size:18px">→</span>
                  ${_.inPlayer?it({..._.inPlayer,_line:_.inPlayer.job,rarity:"normal"},38,50):""}
                </div>
  ...
  </div>`;function h(){const _=e.querySelector(".match-screen");if(!_)return;const S=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);_.style.bottom="auto",_.style.height=S+"px",_.style.minHeight=S+"px",_.style.maxHeight=S+"px",_.style.overflow="hidden";const U=e.querySelector("#mobile-action-bar"),re=e.querySelector("#mobile-play-area");U&&re&&(re.style.paddingBottom=U.offsetHeight+"px")}if(h(),setTimeout(h,120),setTimeout(h,400),setTimeout(h,1e3),t._vvBound||(t._vvBound=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",h),window.visualViewport.addEventListener("scroll",h)),window.addEventListener("resize",h)),function(){const S=e.querySelector("#match-field .terrain-wrapper svg")||e.querySelector(".terrain-wrapper svg");if(!S)return;const U=S.closest("#match-terrain-wrap");U&&(U.style.cssText="position:relative;width:100%;height:100%;padding:0"),S.removeAttribute("width"),S.removeAttribute("height"),S.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",S.setAttribute("preserveAspectRatio","xMidYMid meet")}(),t._resizeBound||(t._resizeBound=!0,window.addEventListener("resize",()=>{const _=e.querySelector(".terrain-wrapper svg");if(_){const S=_.closest("#match-terrain-wrap");S&&(S.style.cssText="position:relative;width:100%;height:100%;padding:0"),_.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0"}})),t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase==="attack"||t.phase==="defense"){let _=!1,S=30;const U=()=>document.getElementById("match-timer"),re=()=>{const ae=U();if(!ae)return;const se=String(Math.floor(S/60)).padStart(2,"0"),de=String(S%60).padStart(2,"0");ae.textContent=` ${se}:${de}`,ae.style.color=_?"#ff2222":"#ff9500",ae.style.fontWeight="900"};re(),t._timerInt=setInterval(()=>{if(S--,S<0)if(!_)_=!0,S=15,re();else{clearInterval(t._timerInt),t._timerInt=null,t.homeScore=0,t.aiScore=3;const ae=document.createElement("div");ae.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;color:#fff;padding:24px;text-align:center",ae.innerHTML='<div style="font-size:56px">⏱️</div><div style="font-size:24px;font-weight:900;color:#ff4444">MATCH PERDU PAR FORFAIT</div><div style="font-size:14px;color:rgba(255,255,255,0.6)">Temps écoulé</div>',document.body.appendChild(ae),setTimeout(()=>{ae.remove(),vt(e,t,i)},2500)}else re()},1e3)}(y=document.getElementById("match-quit"))==null||y.addEventListener("click",()=>{Re(e),confirm("Abandonner ? Résultat : défaite 3-0")&&(t.homeScore=0,t.aiScore=3,vt(e,t,i))}),(B=document.getElementById("view-ai"))==null||B.addEventListener("click",()=>Uo(t,i)),(E=document.getElementById("toggle-history"))==null||E.addEventListener("click",()=>{var _;(_=document.getElementById("match-history-panel"))==null||_.classList.add("open")}),(G=document.getElementById("close-history"))==null||G.addEventListener("click",()=>{var _;(_=document.getElementById("match-history-panel"))==null||_.classList.remove("open")}),(N=document.getElementById("btn-action"))==null||N.addEventListener("click",()=>{t.selected.length!==0&&(u?qo(e,t,i):g&&Do(e,t,i))}),(ne=document.getElementById("btn-results"))==null||ne.addEventListener("click",()=>vt(e,t,i)),(ie=document.getElementById("btn-pass"))==null||ie.addEventListener("click",()=>{t.log.push({text:"⏭️ Vous passez votre tour (plus d'attaquants)",type:"info"}),t.phase="ai-attack",Ee(e,t,i),setTimeout(()=>_i(e,t,i),800)}),e.querySelectorAll(".match-slot-hit").forEach(_=>{_.addEventListener("click",()=>Bo(_,t,e,i))}),e.querySelectorAll(".match-used-hit").forEach(_=>{_.addEventListener("click",()=>oi(e,t,i,null,_.dataset.cardId))}),e.querySelectorAll(".gc-mini").forEach(_=>{_.addEventListener("click",()=>Ro(_.dataset.gcId,_.dataset.gcType,e,t,i))}),(Q=document.getElementById("boost-card"))==null||Q.addEventListener("click",()=>Ho(e,t,i)),e.querySelectorAll(".sub-btn-col").forEach(_=>{_.addEventListener("click",()=>oi(e,t,i,_.dataset.subId))}),(j=document.getElementById("sub-btn-main"))==null||j.addEventListener("click",()=>oi(e,t,i))}function Bo(e,t,i,n){const a=e.dataset.cardId,o=e.dataset.role,r=t.selected.findIndex(s=>s.cardId===a);if(r!==-1)t.selected.splice(r,1);else{if(t.selected.length>=3){n.toast("Maximum 3 joueurs","error");return}const s=(t.homeTeam[o]||[]).find(l=>l.cardId===a);s&&t.selected.push({...s,_role:o,_line:o})}Ee(i,t,n)}function wi(e,t,i){e.matchId&&w.from("matches").update({last_player_id:i}).eq("id",e.matchId).then(()=>{})}function qo(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),wi(t,i,i.state.profile.id);const n=t.selected.map(o=>{const r=(t.homeTeam[o._role]||[]).find(l=>l.cardId===o.cardId)||o,s=["GK","DEF"].includes(o._role);return{...r,_line:o._role,...s?{note_a:Math.max(1,Number(r.note_a)||0)}:{}}}),a=Wt(n,t.modifiers.home);t.pendingAttack={...a,players:[...n],side:"home"},t.selected.forEach(o=>{const r=(t.homeTeam[o._role]||[]).find(s=>s.cardId===o.cardId);r&&(r.used=!0)}),t.log.push({text:`⚔️ Vous attaquez : ${a.total} (base ${a.base}${a.links?` +${a.links} liens`:""}) — ${t.selected.map(o=>o.name).join(", ")}`,type:"info"}),t.selected=[],t.modifiers.home={},t.phase="ai-defense",Ee(e,t,i),setTimeout(()=>Po(e,t,i),1200)}function Do(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),wi(t,i,i.state.profile.id);const n=t.stadiumDef||null,a=t.selected.map(l=>{const c=(t.homeTeam[l._role]||[]).find(u=>u.cardId===l.cardId)||l,d=c.stadiumBonus||n&&(n.club_id&&String(c.club_id)===String(n.club_id)||n.country_code&&c.country_code===n.country_code)||!1;return{...c,_line:l._role,stadiumBonus:d}}),o=Xt(a,t.modifiers.home);t.selected.forEach(l=>{const c=(t.homeTeam[l._role]||[]).find(d=>d.cardId===l.cardId);c&&(c.used=!0)});const r=hi(t.pendingAttack.total,o.total,t.modifiers.home),s={type:"duel",title:"Défense",aiPlayers:(t.pendingAttack.players||[]).map(l=>Pe(l)),homePlayers:t.selected.map(l=>{const c=(t.homeTeam[l._role]||[]).find(d=>d.cardId===l.cardId)||l;return Pe(c)}),homeTotal:o.total,aiTotal:t.pendingAttack.total,isGoal:!1,homeScored:!1,text:""};if(r.shielded)s.text="🛡️ Bouclier ! But annulé.",t.log.push(s);else if(r.goal){t.aiScore++,s.isGoal=!0,s.homeScored=!1,s.text=`⚽ BUT IA ! (${t.pendingAttack.total} > ${o.total})`,t.log.push(s),t.selected=[],t.modifiers.home={},t.pendingAttack=null,Ee(e,t,i),Ve(s.aiPlayers,t.homeScore,t.aiScore,!1,()=>{pt(e,t,i,"home-attack")});return}else s.text=`🧤 Défense réussie ! (${o.total} ≥ ${t.pendingAttack.total})`,t.log.push(s);t.selected=[],t.modifiers.home={},t.pendingAttack=null,pt(e,t,i,"home-attack")}function Fo(e){if(e.aiSubsUsed>=e.aiMaxSubs)return;const t=Object.values(e.aiTeam).flat().filter(l=>l.used);if(!t.length)return;const i=(e.aiSubs||[]).filter(l=>!e.aiUsedSubIds.includes(l.cardId));if(!i.length)return;const n=t[Math.floor(Math.random()*t.length)],a=i.find(l=>l.job===n.job)||i[0],o={...a,used:!1,_line:n._line,_col:n._col},r=e.aiTeam[n._line],s=r.findIndex(l=>l.cardId===n.cardId);s!==-1&&(r[s]=o),e.aiUsedSubIds.push(a.cardId),e.aiSubsUsed++,e.log.push({text:`🔄 IA : ${a.firstname} ${a.name} remplace ${n.firstname} ${n.name}`,type:"info"})}function Go(e){var n;if(!((n=e.aiGcCards)!=null&&n.length))return;const t=e.aiGcCards.filter(a=>!e.aiUsedGc.includes(a.id));if(!t.length||Math.random()>.3)return;const i=t[Math.floor(Math.random()*t.length)];switch(e.aiUsedGc.push(i.id),Se[i.gc_type],i.gc_type){case"Boost+2":{const a=Object.values(e.aiTeam).flat().filter(o=>!o.used);if(a.length){const o=a[Math.floor(Math.random()*a.length)];o.boost=(o.boost||0)+2}break}case"Boost+3":{const a=Object.values(e.aiTeam).flat().filter(o=>!o.used);if(a.length){const o=a[Math.floor(Math.random()*a.length)];o.boost=(o.boost||0)+3}break}case"Remplacement+":e.aiMaxSubs=(e.aiMaxSubs||3)+1;break;case"Bouclier":e.modifiers.ai.shield=!0;break;case"Nul+1":e.modifiers.ai.drawBonus=(e.modifiers.ai.drawBonus||0)+1;break}e.log.push({text:`⚡ IA joue ${i.icon||"⚡"} ${i.name}`,type:"gc"})}function _i(e,t,i){wi(t,i,null),Fo(t),Go(t);let n=[...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]].filter(d=>!d.used),a=!1;n.length||(n=[...t.aiTeam.GK||[],...t.aiTeam.DEF||[]].filter(d=>!d.used),a=!0);const o=tn(n,"attack",t.difficulty);if(!o.length){pi(e,t,i);return}a&&o.forEach(d=>{d._line=d._line||d.job,d.note_a=Math.max(1,Number(d.note_a)||0)});const r=Wt(o,t.modifiers.ai);t.pendingAttack={...r,players:o,side:"ai"},o.forEach(d=>{d.used=!0}),t.log.push({text:`🤖 IA attaque : ${r.total} (${o.map(d=>d.name).join(", ")})`,type:"info"}),t.modifiers.ai={};const s=[...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(d=>!d.used),c=(t.homeSubs||[]).filter(d=>!(t.usedSubIds||[]).includes(d.cardId)).length>0&&t.subsUsed<t.maxSubs;if(s.length===0&&!c){if(o.length===1&&(o[0]._line==="GK"||o[0].job==="GK")&&ci(t.homeTeam)&&t.homeScore===t.aiScore){t.aiScore++,Vt(e,t,i,"ai",o[0]);return}t.aiScore++;const u={type:"duel",isGoal:!0,homeScored:!1,aiPlayers:o.map(g=>Pe(g)),aiTotal:r.total,text:"⚽ BUT IA ! (aucun défenseur disponible)"};t.log.push(u),t.pendingAttack=null,Ee(e,t,i),Ve(u.aiPlayers,t.homeScore,t.aiScore,!1,()=>{pt(e,t,i,"home-attack")});return}t.phase="defense",Ee(e,t,i)}function Po(e,t,i){var c,d;const n=[...t.aiTeam.GK||[],...t.aiTeam.DEF||[],...t.aiTeam.MIL||[]],a=tn(n,"defense",t.difficulty);if(!["GK","DEF","MIL","ATT"].flatMap(u=>(t.aiTeam[u]||[]).filter(g=>!g.used)).length){const u=((c=t.pendingAttack)==null?void 0:c.players)||[];if(u.length===1&&(u[0]._line==="GK"||u[0].job==="GK")&&t.homeScore===t.aiScore){t.homeScore++,Vt(e,t,i,"home",u[0]);return}t.homeScore++;const m={type:"duel",isGoal:!0,homeScored:!0,homePlayers:u.map(f=>Pe(f)),homeTotal:((d=t.pendingAttack)==null?void 0:d.total)||0,aiTotal:0,text:"⚽ BUT ! L'IA n'a plus de joueurs — but automatique !"};t.log.push(m),t.modifiers.ai={},t.pendingAttack=null,Ee(e,t,i),Ve(m.homePlayers,t.homeScore,t.aiScore,!0,()=>{pt(e,t,i,"ai-attack")});return}const r=a.length>0?Xt(a,t.modifiers.ai).total:0;a.forEach(u=>{u.used=!0});const s=hi(t.pendingAttack.total,r,t.modifiers.ai),l={type:"duel",title:"Attaque",homePlayers:(t.pendingAttack.players||[]).map(u=>Pe(u)),aiPlayers:a.map(u=>Pe(u)),homeTotal:t.pendingAttack.total,aiTotal:r,isGoal:!1,homeScored:!1,text:""};if(s.shielded)l.text="🛡️ Bouclier IA !",t.log.push(l);else if(s.goal){t.homeScore++,l.isGoal=!0,l.homeScored=!0,l.text=`⚽ BUT ! (${t.pendingAttack.total} > ${r})`,t.log.push(l),t.modifiers.ai={},t.pendingAttack=null,Ee(e,t,i),Ve(l.homePlayers,t.homeScore,t.aiScore,!0,()=>{pt(e,t,i,"ai-attack")});return}else l.text=`🧤 IA défend (${r} ≥ ${t.pendingAttack.total})`,t.log.push(l);t.modifiers.ai={},t.pendingAttack=null,pt(e,t,i,"ai-attack")}function pt(e,t,i,n){if(t.round++,!mn(e,t,i)){if(ki(t)){vt(e,t,i);return}if(n==="home-attack"){if(!["MIL","ATT","GK","DEF"].some(o=>(t.homeTeam[o]||[]).some(r=>!r.used))){pi(e,t,i);return}t.phase="attack",Ee(e,t,i)}else{if(!["MIL","ATT","GK","DEF"].some(o=>(t.aiTeam[o]||[]).some(r=>!r.used))){pi(e,t,i);return}t.phase="ai-attack",Ee(e,t,i),setTimeout(()=>_i(e,t,i),800)}}}function ci(e){return!["GK","DEF","MIL","ATT"].some(t=>(e[t]||[]).some(i=>!i.used))}function Hi(e){const t=(e.GK||[]).some(n=>!n.used),i=["DEF","MIL","ATT"].some(n=>(e[n]||[]).some(a=>!a.used));return t&&!i}function mn(e,t,i){if(t.homeScore!==t.aiScore)return!1;if(Hi(t.homeTeam)&&ci(t.aiTeam)){const n=(t.homeTeam.GK||[]).find(a=>!a.used);return n?(n.used=!0,t.homeScore++,Vt(e,t,i,"home",n),!0):!1}if(Hi(t.aiTeam)&&ci(t.homeTeam)){const n=(t.aiTeam.GK||[]).find(a=>!a.used);return n?(n.used=!0,t.aiScore++,Vt(e,t,i,"ai",n),!0):!1}return!1}function Vt(e,t,i,n,a){t.log.push({type:"duel",isGoal:!0,homeScored:n==="home",homePlayers:n==="home"?[Pe(a)]:[],aiPlayers:n==="ai"?[Pe(a)]:[],text:`⚽ DERNIER CORNER — Le gardien ${n==="home"?"":"adverse "}marque !`});const o=document.createElement("div");o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:950;display:flex;align-items:center;justify-content:center;overflow:hidden;text-align:center;padding:24px",document.body.appendChild(o);const r=(s,l)=>new Promise(c=>{o.innerHTML=`<div style="font-size:32px;font-weight:900;color:${l};letter-spacing:2px;animation:lcFade 1.4s ease both">${s}</div>
    <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(c,1400)});(async()=>(await r("⚽ DERNIER CORNER","#FFD700"),await r("🧤 LE GARDIEN MONTE !","#4fc3f7"),o.remove(),t.pendingAttack=null,Ee(e,t,i),Ve([Pe(a)],t.homeScore,t.aiScore,n==="home",()=>{if(ki(t)){vt(e,t,i);return}pt(e,t,i,n==="home"?"ai-attack":"home-attack")})))()}function ki(e){const t=["MIL","ATT","GK","DEF"].some(n=>(e.homeTeam[n]||[]).some(a=>!a.used)),i=["MIL","ATT","GK","DEF"].some(n=>(e.aiTeam[n]||[]).some(a=>!a.used));return!t&&!i}function pi(e,t,i){mn(e,t,i)||(ki(t)?vt(e,t,i):(t.phase="attack",Ee(e,t,i)))}function oi(e,t,i,n=null,a=null){var m,f;if(t.phase!=="attack"){yt("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){yt(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const o=Object.entries(t.homeTeam).flatMap(([p,x])=>(x||[]).filter(b=>b.used).map(b=>({...b,_line:b._line||p}))),r=t.homeSubs.filter(p=>!t.usedSubIds.includes(p.cardId));if(!o.length){yt("Aucun joueur utilisé à remplacer");return}if(!r.length){yt("Aucun remplaçant disponible");return}let s=Math.max(0,o.findIndex(p=>p.cardId===a));const l=((m=o[s])==null?void 0:m._line)||((f=o[s])==null?void 0:f.job);let c=n?Math.max(0,r.findIndex(p=>p.cardId===n)):Math.max(0,r.findIndex(p=>p.job===l)),d=!1;const u=document.createElement("div");u.id="sub-overlay",u.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function g(){var y,B,E,G,N,ne;const p=o[s],x=r[c],b=Math.min(130,Math.round((window.innerWidth-90)/2)),v=Math.round(b*1.35),h=ie=>`background:rgba(255,255,255,0.12);border:none;color:${ie?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${ie?"default":"pointer"};flex-shrink:0`;u.innerHTML=`
    ...
    <div style="flex:1;display:flex;gap:0;overflow:hidden">
      ...
      <div id="in-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
        ...
        <div>${x?it({...x,used:!1,boost:0},b,v):"<div>—</div>"}</div>
      ...
      <div id="out-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
        ...
        <div>${p?it({...p,used:!1,boost:0},b,v):"<div>—</div>"}</div>
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(a=>{a.addEventListener("click",()=>{const o=a.dataset.cardId;for(const r of["GK","DEF","MIL","ATT"]){const s=(t.homeTeam[r]||[]).find(l=>l.cardId===o);if(s){s.boost=(s.boost||0)+t.boostCard.value,t.log.push({text:`⚡ Boost +${t.boostCard.value} appliqué à ${s.name}`,type:"info"});break}}t.boostUsed=!0,i.closeModal(),Ee(e,t,i)})})}async function vt(e,t,i){var d,u;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:n}=i,a=t.homeScore>t.aiScore,o=t.homeScore===t.aiScore,r=a?"victoire":o?"nul":"defaite",s=Un(t.mode,r);t.matchId&&await w.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:a?n.profile.id:null,home_credits_reward:s,played_at:new Date().toISOString()}).eq("id",t.matchId);const l={credits:(n.profile.credits||0)+s,matches_played:(n.profile.matches_played||0)+1};a?l.wins=(n.profile.wins||0)+1:o?l.draws=(n.profile.draws||0)+1:l.losses=(n.profile.losses||0)+1,await w.from("users").update(l).eq("id",n.profile.id),await i.refreshProfile();const c=document.createElement("div");c.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",c.innerHTML=`
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      ...
      ${Jt(e.aiTeam,e.formation,null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.1))}
            ...
            </div>`}}catch(H){console.error("[Ranked] MMR update error:",H)}(oe=document.getElementById("pvp-end-overlay"))==null||oe.remove();const D=document.createElement("div");D.id="pvp-end-overlay",D.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;color:#fff;padding:24px;text-align:center;overflow-y:auto";const R=T?"🤝":M?"🏆":"😞",X=T?"MATCH NUL":M?"VICTOIRE !":"DÉFAITE",V=T?"#fff":M?"#FFD700":"#ff6b6b";D.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${r?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(D),(Z=D.querySelector("#pvp-end-home"))==null||Z.addEventListener("click",()=>{D.remove(),Re(e),m(r?"ranked":"home")})}const Q=w.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},k=>{const L=k.new;try{if(L.status==="finished"||L.forfeit){if(B)return;B=!0,E&&(clearInterval(E),E=null),L.game_state&&(y=L.game_state),ie(L);return}if(L.game_state){const z=y;y=L.game_state;const M=y._lastGC;if(M&&M.seq&&!ne.has(M.seq)&&(ne.add(M.seq),M.by!==p)){P(M.type,M.by,()=>de());return}const T=z[p+"Score"]||0,A=z[x+"Score"]||0,D=y[p+"Score"]||0,R=y[x+"Score"]||0,X=D>T,V=R>A;if((X||V)&&!N.has(y.round)){N.add(y.round);const oe=[...y.log||[]].reverse().find(H=>H.isGoal),Z=((oe==null?void 0:oe.homePlayers)||[]).map(H=>({name:H.name,note:H.note,portrait:H.portrait,job:H.job}));Ve(Z,D,R,X,()=>de());return}de()}}catch(z){console.error("[PvP] crash:",z)}}).subscribe();async function j(k){Object.assign(y,k),y.lastActionAt=new Date().toISOString();const{error:L}=await w.from("matches").update({game_state:y}).eq("id",i);L&&f("Erreur de synchronisation","error");try{de()}catch(z){console.error("[PvP] renderPvpScreen crash:",z)}}async function _(){if(B)return;B=!0,E&&(clearInterval(E),E=null);const k=n?h.away_id:h.home_id,L=n?"p2":"p1",z=n?"p1":"p2",M={...y,[L+"Score"]:3,[z+"Score"]:0,phase:"finished"};try{await w.from("matches").update({status:"finished",forfeit:!0,winner_id:k,home_score:M.p1Score||0,away_score:M.p2Score||0,game_state:M}).eq("id",i)}catch{}try{w.removeChannel(Q)}catch{}setTimeout(()=>{Re(e),m("home")},800)}const S={BOOST_STAT:({value:k=1,count:L=1,roles:z=[]},M,T)=>{const A=M[p+"Team"],D=Object.entries(A).filter(([R])=>!z.length||z.includes(R)).flatMap(([R,X])=>X.filter(V=>!V.used).map(V=>({...V,_line:R})));if(!D.length){M.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),T(M);return}U(D,L,`Choisir ${L} joueur(s) à booster (+${k})`,R=>{R.forEach(X=>{const V=(A[X._line]||[]).find(oe=>oe.cardId===X.cardId);V&&(V.boost=(V.boost||0)+k,M.log.push({text:`⚡ +${k} sur ${V.name}`,type:"info"}))}),M[p+"Team"]=A,T(M)})},DEBUFF_STAT:({value:k=1,count:L=1,roles:z=[],target:M="ai"},T,A)=>{const D=M==="home"?p:x,R=T[D+"Team"],X=M==="home"?"allié":"adverse",V=Object.entries(R).filter(([oe])=>!z.length||z.includes(oe)).flatMap(([oe,Z])=>Z.map(H=>({...H,_line:oe})));if(!V.length){T.log.push({text:`🎯 Aucun joueur ${X}`,type:"info"}),A(T);return}U(V,L,`Choisir ${L} joueur(s) ${X}(s) (-${k})`,oe=>{oe.forEach(Z=>{const H=(R[Z._line]||[]).find(le=>le.cardId===Z.cardId);H&&(H.boost=(H.boost||0)-k,T.log.push({text:`🎯 -${k} sur ${H.name}`,type:"info"}))}),T[D+"Team"]=R,A(T)})},GRAY_PLAYER:({count:k=1,roles:L=[],target:z="ai"},M,T)=>{const A=z==="home"?p:x,D=M[A+"Team"],R=z==="home"?"allié":"adverse",X=Object.entries(D).filter(([V])=>!L.length||L.includes(V)).flatMap(([V,oe])=>oe.filter(Z=>!Z.used).map(Z=>({...Z,_line:V})));if(!X.length){M.log.push({text:`❌ Aucun joueur ${R}`,type:"info"}),T(M);return}U(X,k,`Choisir ${k} joueur(s) ${R}(s) à exclure`,V=>{const oe="usedCardIds_"+A,Z=new Set(M[oe]||[]);V.forEach(H=>{const le=(D[H._line]||[]).find(pe=>pe.cardId===H.cardId);le&&(le.used=!0,Z.add(H.cardId),M.log.push({text:`❌ ${le.name} exclu !`,type:"info"}))}),M[oe]=[...Z],M[A+"Team"]=D,T(M)})},REVIVE_PLAYER:({count:k=1,roles:L=[]},z,M)=>{const T=z[p+"Team"],A=Object.entries(T).filter(([D])=>!L.length||L.includes(D)).flatMap(([D,R])=>R.filter(X=>X.used).map(X=>({...X,_line:D})));if(!A.length){z.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),M(z);return}U(A,k,`Choisir ${k} joueur(s) à ressusciter`,D=>{D.forEach(R=>{const X=(T[R._line]||[]).find(V=>V.cardId===R.cardId);X&&(X.used=!1,z.log.push({text:`💫 ${X.name} ressuscité !`,type:"info"}))}),z[p+"Team"]=T,M(z)})},REMOVE_GOAL:({},k,L)=>{const z=x+"Score";k[z]>0?(k[z]--,k.log.push({text:"🚫 Dernier but annulé !",type:"info"})):k.log.push({text:"🚫 Aucun but à annuler",type:"info"}),L(k)},ADD_GOAL_DRAW:({},k,L)=>{k[p+"Score"]===k[x+"Score"]?(k[p+"Score"]++,k.log.push({text:"🎯 But bonus !",type:"info"})):k.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),L(k)},ADD_SUB:({value:k=1},L,z)=>{L.maxSubs=(L.maxSubs||3)+k,L.log.push({text:`🔄 +${k} remplacement(s)`,type:"info"}),z(L)},CUSTOM:({},k,L)=>L(k)};function U(k,L,z,M){const T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let A=[];function D(){var X,V;const R=k.map(oe=>{const Z=oe._line||oe.job||"MIL",H=A.find(pe=>pe.cardId===oe.cardId),le=Me({...oe,_evolution_bonus:0},{width:90,showStad:!0,role:Z,extraNote:oe.boost||0});return`<div class="pp-item" data-cid="${oe.cardId}"
        ...
        </div>`,(X=T.querySelector("#pp-close"))==null||X.addEventListener("click",()=>T.remove()),T.querySelectorAll(".pp-item").forEach(oe=>{oe.addEventListener("click",()=>{const Z=oe.dataset.cid,H=k.find(pe=>pe.cardId===Z),le=A.findIndex(pe=>pe.cardId===Z);H&&(le>-1?A.splice(le,1):A.length<L&&A.push(H),D())})}),(V=T.querySelector("#pp-confirm"))==null||V.addEventListener("click",()=>{T.remove(),M(A)})}D(),document.body.appendChild(T)}async function re(k,L){const M=(y["gcCardsFull_"+p]||[]).find(R=>R.id===k),T=(M==null?void 0:M._gcDef)||(y.gcDefs||[]).find(R=>{var X;return R.name===L||((X=R.name)==null?void 0:X.toLowerCase().trim())===(L==null?void 0:L.toLowerCase().trim())}),A=[...y["usedGc_"+p]||[],k],D={type:L,by:p,seq:(y._gcAnimSeq||0)+1};ne.add(D.seq),P(L,p,async()=>{if(T!=null&&T.effect_type&&T.effect_type!=="CUSTOM"){const X=S[T.effect_type];if(X){const V={...y};X(T.effect_params||{},V,async oe=>{oe["usedGc_"+p]=A,oe._lastGC=D,oe._gcAnimSeq=D.seq,await j(oe)});return}}const R={...y};switch(L){case"Remplacement+":R.maxSubs=(R.maxSubs||3)+1,R.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const X=x+"Score";R[X]>0&&(R[X]--,R.log.push({text:"🚫 But annulé",type:"info"}));break}}R["usedGc_"+p]=A,R._lastGC=D,R._gcAnimSeq=D.seq,await j(R)})}function ae(k,L){const z="usedCardIds_"+k,M=new Set(y[z]||[]);L.forEach(T=>M.add(T)),y[z]=[...M]}function se(){for(const k of["p1","p2"]){const L=new Set(y["usedCardIds_"+k]||[]),z=y[k+"Team"];if(z)for(const M of["GK","DEF","MIL","ATT"])(z[M]||[]).forEach(T=>{T.used=L.has(T.cardId)})}}function de(){var at,We,Xe,st,Li,Ti;if(y.phase==="reveal")return ke();if(y.phase==="midfield")return J();if(y.phase==="finished")return Lt();const k=y[p+"Team"];y[x+"Team"],se();const L=y[p+"Score"],z=y[x+"Score"],M=y[p+"Name"],T=y[x+"Name"],A=y.phase===p+"-attack",D=y.phase===p+"-defense",R=Array.isArray(y["selected_"+p])?y["selected_"+p]:[],X=R.map(ee=>ee.cardId),V=window.innerWidth>=700,oe=y[p+"Subs"]||[],Z=y["usedSubIds_"+p]||[],H=oe.filter(ee=>!Z.includes(ee.cardId)),le=y["gcCardsFull_"+p]||[],pe=y["usedGc_"+p]||[],ue=le.filter(ee=>!pe.includes(ee.id)),ce=y.boostOwner===p&&!y.boostUsed,me=[...k.MIL||[],...k.ATT||[]].filter(ee=>!ee.used),xe=A&&me.length===0?[...k.GK||[],...k.DEF||[]].filter(ee=>!ee.used).map(ee=>ee.cardId):[];function te(ee,fe,ze){var Si,Ai;const ye=ee._gcDef,dt={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ye==null?void 0:ye.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",gt={purple:"#b06ce0",light_blue:"#00d4ef"}[ye==null?void 0:ye.color]||"#b06ce0",Tt=(ye==null?void 0:ye.name)||ee.gc_type,zt=(ye==null?void 0:ye.effect)||((Si=Se[ee.gc_type])==null?void 0:Si.desc)||"",zi=ye!=null&&ye.image_url?`/icons/${ye.image_url}`:null,kn=((Ai=Se[ee.gc_type])==null?void 0:Ai.icon)||"⚡",Ii=Math.round(ze*.22),Mi=Math.round(ze*.22),Zt=ze-Ii-Mi,$n=Tt.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${ee.id}" data-gc-type="${ee.gc_type}"
        style="box-sizing:border-box;width:${fe}px;height:${ze}px;border-radius:10px;border:2px solid ${gt};background:${dt};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
        <div style="height:${Ii}px;background:rgba(255,255,255,0.14);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 3px">
          <span style="font-size:${$n}px;font-weight:900;color:#fff;text-align:center;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${fe-6}px">${Tt}</span>
          <span style="font-size:6px;color:rgba(255,255,255,0.45)">⚡ GC</span>
        </div>
        <div style="height:${Zt}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
          ${zi?`<img src="${zi}" style="max-width:${fe-10}px;max-height:${Zt-6}px;object-fit:contain">`:`<span style="font-size:${Math.round(Zt*.55)}px">${kn}</span>`}
        </div>
        <div style="height:${Mi}px;background:rgba(0,0,0,0.38);display:flex;align-items:center;justify-content:center;padding:0 3px">
          <span style="font-size:6px;color:rgba(255,255,255,0.9);text-align:center;line-height:1.25">${zt.slice(0,38)}</span>
        </div>
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${$t(k,y[p+"Formation"],Ce,X,V?1300:ct(),V?600:ht(),xe)}
      </div>
          ...
          <div style="display:flex;justify-content:center">${Qe((ye.players||[]).map(dt=>({...dt,used:!1})),"#ff6b6b",ye.total)}</div>
          ...
          <div style="font-size:8px;color:#00ff88;letter-spacing:2px;margin-bottom:4px;text-transform:uppercase">⚔️ VOUS ATTAQUEZ</div>
          <div style="display:flex;justify-content:center">${Qe((ye.players||[]).map(dt=>({...dt,used:!1})),"#00ff88",ye.total)}</div>
        </div>`}const ee=(y.log||[]).slice(-1)[0];return ee?'<div style="padding:2px 4px">'+fn(ee)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3);text-align:center">⏳ Match en cours...</div>'})(),Ye=`
      <div style="display:flex;align-items:center;padding:8px 10px;background:rgba(0,0,0,0.5);gap:6px;flex-shrink:0">
      ...
      <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
        ...
        <div id="mobile-play-area" style="flex:1;min-height:0;display:flex;overflow:hidden">
          <div id="match-field" style="flex:1;min-width:0;min-height:0;overflow:hidden">
            <div class="terrain-wrapper" style="width:100%;height:100%;overflow:hidden">
              ${$t(k,y[p+"Formation"],Ce,X,ct(),ht(),xe)}
            </div>
          </div>
        </div>
        <div id="mobile-action-bar" style="position:absolute;left:0;right:0;bottom:0;z-index:20;background:rgba(0,0,0,0.55);padding:6px 8px 8px;display:flex;flex-direction:column;gap:6px;box-shadow:0 -4px 16px rgba(0,0,0,0.5)">
      ...
      </div>`;function qe(){const ee=e.querySelector(".match-screen");if(!ee)return;const fe=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);ee.style.height=fe+"px",ee.style.minHeight=fe+"px",ee.style.maxHeight=fe+"px",ee.style.overflow="hidden";const ze=e.querySelector("#mobile-action-bar"),ye=e.querySelector("#mobile-play-area");ze&&ye&&(ye.style.paddingBottom=ze.offsetHeight+"px")}if(qe(),setTimeout(qe,120),setTimeout(qe,400),G||(G=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",qe),window.visualViewport.addEventListener("scroll",qe)),window.addEventListener("resize",qe)),function(){const fe=e.querySelector("#match-field .terrain-wrapper svg")||e.querySelector(".terrain-wrapper svg");if(!fe)return;const ze=fe.closest("#match-terrain-wrap");ze&&(ze.style.cssText="position:relative;width:100%;height:100%;padding:0"),fe.removeAttribute("width"),fe.removeAttribute("height"),fe.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",fe.setAttribute("preserveAspectRatio","xMidYMid meet")}(),y._pvpResizeBound||(y._pvpResizeBound=!0,window.addEventListener("resize",()=>{const ee=e.querySelector(".terrain-wrapper svg");if(ee){const fe=ee.closest("#match-terrain-wrap");fe&&(fe.style.cssText="position:relative;width:100%;height:100%;padding:0"),ee.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0"}})),e.querySelectorAll(".match-slot-hit").forEach(ee=>{ee.addEventListener("click",()=>{if(!A&&!D)return;const fe=ee.dataset.cardId,ze=ee.dataset.role,ye=(k[ze]||[]).find(zt=>zt.cardId===fe);if(!ye||ye.used)return;const dt=xe.includes(fe);if(A&&!["MIL","ATT"].includes(ze)&&!dt)return;Array.isArray(y["selected_"+p])||(y["selected_"+p]=[]);const gt=y["selected_"+p],Tt=gt.findIndex(zt=>zt.cardId===fe);Tt>-1?gt.splice(Tt,1):gt.length<3&&gt.push({...ye,_role:ze}),de()})}),e.querySelectorAll(".match-used-hit").forEach(ee=>{ee.addEventListener("click",()=>F(ee.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(ee=>{ee.addEventListener("click",()=>F())}),(at=e.querySelector("#pvp-sub-open"))==null||at.addEventListener("click",()=>F()),e.querySelectorAll(".pvp-gc-mini").forEach(ee=>{A?ee.addEventListener("click",()=>I(ee.dataset.gcId,ee.dataset.gcType)):(ee.style.opacity="0.35",ee.style.cursor="default",ee.addEventListener("click",()=>yt("⚡ Les Game Changers ne sont utilisables qu'en attaque","rgba(180,100,0,0.9)")))}),(We=e.querySelector("#pvp-boost-card"))==null||We.addEventListener("click",()=>C()),(Xe=e.querySelector("#pvp-action"))==null||Xe.addEventListener("click",ee=>{A?ee.currentTarget.dataset.pass==="1"||!Ae(p)?O():he():D&&$e()}),(st=e.querySelector("#pvp-quit"))==null||st.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&_()}),(Li=e.querySelector("#pvp-view-opp"))==null||Li.addEventListener("click",()=>K()),(Ti=e.querySelector("#pvp-toggle-history"))==null||Ti.addEventListener("click",()=>q()),E&&(clearInterval(E),E=null),(A||D)&&!B){let ee=30,fe=!1;const ze=()=>document.getElementById("pvp-timer"),ye=()=>{ze()&&(ze().textContent=ee+"s",ze().style.color=fe?"#ff4444":"#fff")};ye(),E=setInterval(()=>{ee--,ee<0?fe?(clearInterval(E),E=null,A&&!Ae(p)?O():_()):(fe=!0,ee=15,ye()):ye()},1e3)}}function ke(){Ke(e),e.innerHTML=`
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        <div id="pin-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
          ...
          <div>${ce?it({...ce,used:!1,boost:0},me,xe):"<div>—</div>"}</div>
        ...
        <div id="pout-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
          ...
          <div>${ue?it({...ue,used:!1,boost:0},me,xe):"<div>—</div>"}</div>
          <button id="pout-down" style="${te(R>=A.length-1)}" ${R>=A.length-1?"disabled":""}>▼</button>
          <div style="font-size:10px;color:rgba(255,255,255,0.35)">${R+1}/${A.length}</div>
        </div>
      </div>
      <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
        <button id="psub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
      </div>`,(Be=Z.querySelector("#psub-close"))==null||Be.addEventListener("click",()=>Z.remove()),(Ie=Z.querySelector("#pout-up"))==null||Ie.addEventListener("click",()=>{R>0&&(R--,H())}),(Ge=Z.querySelector("#pout-down"))==null||Ge.addEventListener("click",()=>{R<A.length-1&&(R++,H())}),(ft=Z.querySelector("#pin-up"))==null||ft.addEventListener("click",()=>{V>0&&(V--,H())}),(Oe=Z.querySelector("#pin-down"))==null||Oe.addEventListener("click",()=>{V<D.length-1&&(V++,H())});const Te=(Ce,He,Ue,Ye)=>{const qe=Z.querySelector("#"+Ce);if(!qe)return;let at=0;qe.addEventListener("touchstart",We=>{at=We.touches[0].clientY},{passive:!0}),qe.addEventListener("touchend",We=>{const Xe=We.changedTouches[0].clientY-at;if(Math.abs(Xe)<30)return;const st=He();Xe<0&&st<Ye-1?(Ue(st+1),H()):Xe>0&&st>0&&(Ue(st-1),H())},{passive:!0})};Te("pin-panel",()=>V,Ce=>V=Ce,D.length),Te("pout-panel",()=>R,Ce=>R=Ce,A.length),(Ft=Z.querySelector("#psub-confirm"))==null||Ft.addEventListener("click",async Ce=>{if(Ce.preventDefault(),Ce.stopPropagation(),oe)return;oe=!0;const He=A[R],Ue=D[V];if(!He||!Ue)return;const Ye=He._line,qe=(z[Ye]||[]).findIndex(Xe=>Xe.cardId===He.cardId);if(qe===-1){f("Erreur : joueur introuvable","error"),Z.remove();return}const at={...Ue,_line:Ye,position:He.position,used:!1,boost:0};z[Ye].splice(qe,1,at);const We=[...M,Ue.cardId];Z.remove(),W(He,Ue,async()=>{await j({[p+"Team"]:z,[x+"Team"]:y[x+"Team"],["usedSubIds_"+p]:We})})})}document.body.appendChild(Z),H()}function W(k,L,z){const M={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const A=(X,V,oe)=>`<div style="text-align:center">
      ...
      <div style="width:min(90vw,420px)">${Jt(y[x+"Team"],y[x+"Formation"],null,[],ct(),ht())}</div>
      ...
      </div>`,document.body.appendChild(L),(z=L.querySelector("#pvp-hist-close"))==null||z.addEventListener("click",()=>L.remove())}async function O(){if(y.phase!==p+"-attack")return;const k=p==="p1"?"p2":"p1",L=(y.round||0)+1,z=[...y.log||[]];z.push({type:"info",text:`⏭️ ${y[p+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const M=Fe(y),T=Ae(k),A=M||!T?"finished":k+"-attack";await j({["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:A,attacker:k,round:L,log:z}),A==="finished"&&await Ne(y)}function we(k){const L=(k.GK||[]).some(M=>!M.used),z=["DEF","MIL","ATT"].some(M=>(k[M]||[]).some(T=>!T.used));return L&&!z}function _e(k){return(k.p1Score||0)!==(k.p2Score||0)?null:we(k.p1Team)&&!ge(k.p2Team)?{side:"p1",gk:(k.p1Team.GK||[]).find(L=>!L.used)}:we(k.p2Team)&&!ge(k.p1Team)?{side:"p2",gk:(k.p2Team.GK||[]).find(L=>!L.used)}:null}async function be(k,L){const{side:z,gk:M}=k;if(!M)return;const T=z+"Score",A=(L[T]||0)+1;M.used=!0;const D=[...L.log||[]];D.push({type:"duel",isGoal:!0,homeScored:z===p,homePlayers:z==="p1"?[mt(M)]:[],aiPlayers:z==="p2"?[mt(M)]:[],text:`⚽ DERNIER CORNER — Le gardien de ${L[z+"Name"]} marque !`});const R=(L.round||0)+1,X=z==="p1"?"p2":"p1",V={...L,[T]:A},oe=Fe(V);N.add(R);const Z=document.createElement("div");Z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:950;display:flex;align-items:center;justify-content:center;overflow:hidden;text-align:center;padding:24px",document.body.appendChild(Z);const H=(ue,ce)=>new Promise(me=>{Z.innerHTML=`<div style="font-size:32px;font-weight:900;color:${ce};letter-spacing:2px;animation:lcFade 1.4s ease both">${ue}</div>
      <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(me,1400)});await H("⚽ DERNIER CORNER","#FFD700"),await H("🧤 LE GARDIEN MONTE !","#4fc3f7"),Z.remove();const le=z===p?A:L[p+"Score"]||0,pe=z===p?L[x+"Score"]||0:A;Ve([mt(M)],le,pe,z===p,async()=>{await j({[T]:A,log:D,round:R,pendingAttack:null,phase:oe?"finished":X+"-attack",attacker:X,["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}}}),oe&&await Ne({...y,[T]:A})})}async function he(){const k=y[p+"Team"],L=!["GK","DEF","MIL","ATT"].some(A=>(y[x+"Team"][A]||[]).some(D=>!D.used)),z=(y["selected_"+p]||[]).map(A=>{const D=(k[A._role]||[]).find(pe=>pe.cardId===A.cardId)||A,R=["GK","DEF"].includes(A._role),X=k[A._role]||[],V=X.findIndex(pe=>pe.cardId===A.cardId),oe=kt(X.length),Z=V>=0?oe[V]:D._col??1,H=y.stadiumDef||y.homeStadiumDef||null,le=D.stadiumBonus||H&&(H.club_id&&String(D.club_id)===String(H.club_id)||H.country_code&&D.country_code===H.country_code)||!1;return{...D,_line:A._role,_col:Z,stadiumBonus:le,...R?{note_a:Math.max(1,Number(D.note_a)||0)}:{}}});if(!z.length)return;const M=Wt(z,y.modifiers[p]||{});ae(p,z.map(A=>A.cardId)),z.forEach(A=>{const D=(k[A._role]||[]).find(R=>R.cardId===A.cardId);D&&(D.used=!0)}),y["selected_"+p]=[],de();const T=[...y.log||[]];if(L){if(z.length===1&&(z[0]._line==="GK"||z[0].job==="GK")&&(y.p1Score||0)===(y.p2Score||0)){await be({side:p,gk:z[0]},{...y,[p+"Team"]:k,log:T});return}const D=(y[p+"Score"]||0)+1,R=z.map(H=>mt(H));T.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:R,homeTotal:M.total,aiTotal:0});const X=(y.round||0)+1,V=p==="p1"?"p2":"p1",oe={...y,[p+"Team"]:k,[p+"Score"]:D},Z=Fe(oe);N.add(X),Ve(R,D,y[x+"Score"]||0,!0,async()=>{await j({[p+"Team"]:k,[p+"Score"]:D,["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:Z?"finished":V+"-attack",attacker:V,round:X,log:T}),Z&&await Ne({...y,[p+"Score"]:D})});return}T.push({type:"pending",text:`⚔️ ${y[p+"Name"]} attaque (${M.total})`}),await j({[p+"Team"]:k,[x+"Team"]:y[x+"Team"],pendingAttack:{...M,players:z,side:p},["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},phase:x+"-defense",log:T})}async function $e(){const k=y[p+"Team"],L=(y["selected_"+p]||[]).map(ce=>{const me=(k[ce._role]||[]).find(Ie=>Ie.cardId===ce.cardId)||ce,xe=k[ce._role]||[],te=xe.findIndex(Ie=>Ie.cardId===ce.cardId),Te=kt(xe.length),Be=te>=0?Te[te]:me._col??1;return{...me,_line:ce._role,_col:Be}}),z=Xt(L,y.modifiers[p]||{});ae(p,L.map(ce=>ce.cardId)),L.forEach(ce=>{const me=(k[ce._role]||[]).find(xe=>xe.cardId===ce.cardId);me&&(me.used=!0)}),y["selected_"+p]=[],de();const M=hi(y.pendingAttack.total,z.total,y.modifiers[p]||{}),T=y.pendingAttack.side,A=M==="attack"||(M==null?void 0:M.goal),D=T==="p1"?"p2":"p1",R=(y.round||0)+1,X=(y.pendingAttack.players||[]).map(ce=>mt(ce)),V=[...y.log||[]];V.push({type:"duel",isGoal:A,homeScored:A&&T===p,text:A?`⚽ BUT de ${y[T+"Name"]} ! (${y.pendingAttack.total} vs ${z.total})`:`✋ Attaque stoppée (${y.pendingAttack.total} vs ${z.total})`,homePlayers:X,aiPlayers:L.map(ce=>mt(ce)),homeTotal:y.pendingAttack.total,aiTotal:z.total});const oe=A?(y[T+"Score"]||0)+1:y[T+"Score"]||0,Z={...y,[p+"Team"]:k,[T+"Score"]:oe},H=Fe(Z),le=H?"finished":D+"-attack",pe=!H&&!A?_e(Z):null,ue=async()=>{if(pe){await be(pe,Z);return}await j({[p+"Team"]:k,[x+"Team"]:y[x+"Team"],[T+"Score"]:oe,["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:le,attacker:D,round:R,log:V}),(le==="finished"||H)&&await Ne({...y,[T+"Score"]:oe})};if(A){const ce=T===p,me=ce?oe:y[p+"Score"]||0,xe=ce?y[x+"Score"]||0:oe;N.add(R),Ve(X,me,xe,ce,ue)}else await ue()}function Le(k){return["MIL","ATT"].some(L=>(k[L]||[]).some(z=>!z.used))}function ge(k){return["GK","DEF","MIL","ATT"].some(L=>(k[L]||[]).some(z=>!z.used))}function je(k){return ge(k)&&!Le(k)}function Ae(k){const L=y[k+"Team"];return!!(Le(L)||je(L))}function Fe(k){return!ge(k.p1Team)&&!ge(k.p2Team)}function rt(k){const L=k.p1Score||0,z=k.p2Score||0;return L===z?null:L>z?h.home_id:h.away_id}async function Ne(k){try{const L=rt(k),z=L?h.home_id===L?h.away_id:h.home_id:null;await w.from("matches").update({status:"finished",winner_id:L,home_score:k.p1Score||0,away_score:k.p2Score||0}).eq("id",i),L&&z&&Yn(L,z).catch(()=>{})}catch(L){console.error("[PvP] finishMatch:",L)}}function Lt(){var T;const k=y[p+"Score"],L=y[x+"Score"],z=k>L,M=k===L;Ke(e),e.innerHTML=`
    ...
    </div>`,(T=document.getElementById("pvp-home"))==null||T.addEventListener("click",()=>{try{w.removeChannel(Q)}catch{}Re(e),m("home")})}de()}async function ui(e,t,i=!1){var s,l;const{state:n,navigate:a,toast:o}=t,r=i&&((l=(s=t==null?void 0:t.state)==null?void 0:s.params)==null?void 0:l.rankedData)||null;await Dt(e,t,i?"ranked":"random",async({deckId:c,formation:d,starters:u,subsRaw:g,gcCardsEnriched:m,gcDefs:f,stadiumDef:p})=>{Ke(e);const x=async b=>{await Yo(e,t,c,d,u,g,b||[],f,p,i,r)};if(!(m!=null&&m.length)){await x([]);return}vi(e,m,x)})}async function Vo(e,t,i){return Ko(e,t,i)}async function Yo(e,t,i,n,a,o,r=[],s=[],l=null,c=!1,d=null){var ie,Q;const{state:u,navigate:g,toast:m}=t,f=(d==null?void 0:d.mmr)??((ie=u.profile)==null?void 0:ie.mmr)??1e3,p=c?xi(f):null,x=c?"linear-gradient(135deg, #1a0a2e 0%, #2d1060 50%, #1a0a2e 100%)":"linear-gradient(135deg, #0a1a2e 0%, #0d3d1e 100%)",b=c?(p==null?void 0:p.color)||"#D4A017":"#FFD700";let v=!1,h=null,$=null;e.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="display:flex;background:#fff;border-bottom:1px solid #eee">
      ${x.map(v=>`<button class="ml-tab" data-tab="${v.key}" style="flex:1;padding:10px 4px;border:none;border-bottom:2px solid ${i===v.key?wt:"transparent"};background:none;font-size:12px;font-weight:${i===v.key?"900":"600"};color:${i===v.key?wt:"#888"};cursor:pointer">${v.label}${v.count?` (${v.count})`:""}</button>`).join("")}
    </div>
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:10px">
      ${i==="waiting"?tr(g,p,a):i==="active"?ir(m,a):nr(f,a)}
    </div>
  </div>`,(b=document.getElementById("ml-create-btn"))==null||b.addEventListener("click",()=>or(e,t)),e.querySelectorAll(".ml-tab").forEach(v=>v.addEventListener("click",()=>ut(e,t,v.dataset.tab))),e.querySelectorAll("[data-league-id]").forEach(v=>v.addEventListener("click",()=>ot(e,t,v.dataset.leagueId))),e.querySelectorAll("[data-join]").forEach(v=>v.addEventListener("click",h=>{h.stopPropagation(),yn(e,t,v.dataset.join,v.dataset.type)})),e.querySelectorAll("[data-delete]").forEach(v=>v.addEventListener("click",h=>{h.stopPropagation(),fi(e,t,v.dataset.delete,v.dataset.name,i)}))}function Yt(e,t,i=!1){var r,s;const n=e.creator_id===t,a=e.pot||0,o=((s=(r=e.mini_league_members)==null?void 0:r[0])==null?void 0:s.count)||0;return`<div data-league-id="${e.id}" style="background:#fff;border-radius:12px;padding:14px 16px;box-shadow:0 1px 6px rgba(0,0,0,0.08);cursor:pointer;margin-bottom:8px">
    ...
    <div style="display:flex;flex-wrap:wrap;gap:8px;font-size:11px;color:#666;margin-bottom:${i?"10px":"0"}">
      ...
      ${a>0?`<span style="color:${tt};font-weight:700">🏆 ${a.toLocaleString("fr")} cr.</span>`:""}
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    <div style="padding:12px 16px;background:#fff;border-bottom:1px solid #eee;display:flex;align-items:center;gap:10px">
      ...
      <div style="text-align:right;flex-shrink:0">
        ...
        ${f>0?`<div style="font-size:12px;font-weight:900;color:${tt};margin-top:4px">🏆 ${f.toLocaleString("fr")} cr.</div>`:""}
      </div>
    </div>
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        ${u.map(j=>`
          <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid #f5f5f5">
            <div style="width:36px;height:36px;border-radius:50%;background:${j.club_color2||wt};display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:900;color:${j.club_color1||"#fff"}">${(j.pseudo||"?").slice(0,2).toUpperCase()}</div>
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          <thead><tr style="font-size:10px;color:#888;text-transform:uppercase;border-bottom:2px solid #f0f0f0">
            ...
            ${f>0&&r.status==="finished"?`<th style="text-align:right;padding:5px 0;color:${tt}">💰</th>`:""}
          </tr></thead>
          <tbody>${g.map((j,_)=>{const S=f>0&&r.status==="finished"?_===0?Math.floor(f*.7):_===1?Math.floor(f*.2):_===2?Math.floor(f*.1):0:0;return`<tr style="border-bottom:1px solid #f5f5f5;${j.userId===o?"background:#f0fdf4;":""}">
              <td style="padding:7px 3px 7px 0;font-weight:700;color:${_===0?tt:_<3?wt:"#555"}">${["🥇","🥈","🥉"][_]||_+1}</td>
              ...
              <td style="text-align:center;color:${j.goalDiff>=0?wt:"#cc2222"}">${j.goalDiff>=0?"+":""}${j.goalDiff}</td>
              <td style="text-align:center;font-weight:900;font-size:14px">${j.points}</td>
              ${f>0&&r.status==="finished"?`<td style="text-align:right;font-weight:700;color:${tt}">${S?S.toLocaleString("fr")+" cr.":"—"}</td>`:""}
            </tr>`}).join("")}</tbody>
        </table>
      </div>`:""}
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        ${Array.from({length:Math.max(0,r.status==="active"?r.current_day-1:r.current_day)},(j,_)=>_+1).reverse().map(j=>{const _=(l||[]).filter(S=>S.matchday===j);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:#888;margin-bottom:6px">Journée ${j}</div>${_.map(S=>Ui(S,u,o,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
    ...
    <div style="font-size:13px;font-weight:900;min-width:50px;text-align:center;color:${e.status==="finished"?wt:"#999"}">${d}</div>
  ...
  </div>`}async function dr(e,t,i,n){const{toast:a,state:o}=t,r=pr(n.map(c=>c.id),i.mode),s=[];r.forEach((c,d)=>c.forEach(u=>s.push({league_id:i.id,matchday:d+1,home_id:u.home||u.bye,away_id:u.away||null,is_bye:!!u.bye,status:u.bye?"bye":"pending"})));const{error:l}=await w.from("mini_league_matches").insert(s);if(l){a("Erreur calendrier : "+l.message,"error");return}await w.from("mini_leagues").update({status:"active",current_day:1,total_days:r.length}).eq("id",i.id),a(`🚀 Lancée ! Pot : ${(i.pot||0).toLocaleString("fr")} cr.`,"success"),ot(e,t,i.id)}async function lr(e,t,i){const{data:n}=await w.from("mini_leagues").select("current_day,total_days,pot").eq("id",i).single(),a=(n.current_day||0)+1;if(a>(n.total_days||0)){await bn(e,t,i,n.pot||0,null,null);return}await w.from("mini_leagues").update({current_day:a}).eq("id",i),t.toast(`Journée ${a} commencée !`,"success"),ot(e,t,i)}async function bn(e,t,i,n,a,o){const{toast:r,state:s}=t;let l=a,c=o;if(!l){const{data:u}=await w.from("mini_league_matches").select("*").eq("league_id",i),{data:g}=await w.from("mini_league_members").select("*, user:users(id,pseudo,club_name)").eq("league_id",i);c=(g||[]).map(m=>m.user).filter(Boolean),l=hn(c,u||[])}const d=[Math.floor(n*.7),Math.floor(n*.2),Math.floor(n*.1)];await Promise.all(l.slice(0,3).map((u,g)=>d[g]?w.from("mini_league_members").update({prize_amount:d[g],prize_claimed:!1}).eq("league_id",i).eq("user_id",u.userId):Promise.resolve())),await w.from("mini_leagues").update({status:"finished"}).eq("id",i),r("🏆 Mini League terminée ! Les gagnants peuvent récupérer leurs crédits.","success"),ot(e,t,i)}async function cr(e,t,i,n,a){var g,m;const{state:o,toast:r}=t,s=[Math.floor((i.pot||0)*.7),Math.floor((i.pot||0)*.2),Math.floor((i.pot||0)*.1)],l=["🥇","🥈","🥉"][a],c=n.prize_amount||s[a]||0,d=n.prize_claimed,u=document.createElement("div");u.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",u.innerHTML=`
    <div style="background:linear-gradient(160deg,#1a0a2e,#3b1a6e);border-radius:20px;padding:32px 24px;max-width:320px;width:100%;text-align:center;color:#fff;box-shadow:0 8px 40px rgba(0,0,0,0.6)">
      ...
      <div style="background:rgba(212,160,23,0.2);border:2px solid ${tt};border-radius:14px;padding:16px;margin-bottom:24px">
        <div style="font-size:12px;color:rgba(255,255,255,0.6);margin-bottom:4px">${d?"Déjà récupéré":"Tes gains"}</div>
        <div style="font-size:32px;font-weight:900;color:${tt}">${c.toLocaleString("fr")} cr.</div>
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    </div>`}function g(p){var G,N,ne,ie;const x=(G=p.card)==null?void 0:G.player;if(!x)return"";const b=((N=p.card)==null?void 0:N.evolution_bonus)||0,v=xt(x,x.job,b),h=x.job2?xt(x,x.job2,b):0,$=p.status==="sold",y=x.country_code?`https://flagsapi.com/${x.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,B=Nt[x.job]||"#bbb",E=x.job2?Nt[x.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${$?"opacity:0.7":""}">
      ${y?`<img src="${y}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      ${(ne=x.clubs)!=null&&ne.logo_url?`<img src="${x.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${B};display:flex;align-items:center;justify-content:center">
    ...
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center">
      ...
      `).join(""):'<div style="text-align:center;color:var(--gray-600);padding:40px">Aucun manager.</div>'}else{const b=r||[];f.innerHTML=b.length>0?b.map((v,h)=>{const $=xi(v.mmr??1e3),y=(v.ranked_wins||0)+(v.ranked_losses||0)+(v.ranked_draws||0),B=y>0?Math.round((v.ranked_wins||0)/y*100):0,E=v.id===i.profile.id,G=(v.placement_matches||0)<10;return`
  ...
  </div>`}Tn(In);const ve={user:null,profile:null,page:"home",params:{}};function bt(e,t="info",i=3e3){const n=document.getElementById("toast");n&&(n.textContent=e,n.className=`show ${t}`,clearTimeout(n._t),n._t=setTimeout(()=>{n.className=""},i))}function Lr(e,t,i=""){document.getElementById("modal-title").textContent=e,document.getElementById("modal-body").innerHTML=t,document.getElementById("modal-footer").innerHTML=i,document.getElementById("modal-overlay").classList.remove("hidden")}function gi(){document.getElementById("modal-overlay").classList.add("hidden")}async function It(){if(!ve.user)return;const{data:e}=await w.from("users").select("*").eq("id",ve.user.id).single();e&&(ve.profile=e)}function Mt(e,t={}){ve.page=e,ve.params=t,wn()}async function wn(){var n,a,o,r;const e=document.getElementById("page-content");if(!e)return;document.querySelectorAll(".bottom-nav a").forEach(s=>{s.classList.toggle("active",s.dataset.page===ve.page)});const t=document.getElementById("nav-credits");t&&ve.profile&&(t.textContent=`💰 ${(ve.profile.credits||0).toLocaleString("fr")}`);const i={state:ve,navigate:Mt,toast:bt,openModal:Lr,closeModal:gi,refreshProfile:It};switch(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽</div>',ve.page){case"home":await qi(e,i);break;case"collection":await io(e,i);break;case"decks":await ai(e,i);break;case"boosters":await mo(e,i);break;case"ranked":await $r(e,i);break;case"match":{const s=ve.params&&ve.params.matchMode||"vs_ai_easy";s==="random"?await ui(e,i,!1):s==="ranked"?await ui(e,i,!0):s==="friend"?await Jo(e,i,(n=ve.params)==null?void 0:n.friendId,(a=ve.params)==null?void 0:a.friendName):s==="mini-league"?await Ki(e,i,(o=ve.params)==null?void 0:o.mlMatchId,(r=ve.params)==null?void 0:r.leagueId):await Mo(e,i);break}case"market":await hr(e,i);break;case"rankings":await kr(e,i);break;case"matches":await Er(e,i);break;case"friends":await qn(e,i);break;case"mini-league":await er(e,i);break;case"match-mini-league":{const s=ve.params||{};await Ki(e,i,s.mlMatchId,s.leagueId);break}default:await qi(e,i)}}function Tr(){var n;const e=document.getElementById("app"),t=ve.profile;if(!t)return;const i="/icons/";e.innerHTML=`
    <nav class="top-nav">
  ...
  `,document.querySelectorAll(".bottom-nav a").forEach(a=>{a.addEventListener("click",o=>{o.preventDefault(),Mt(a.dataset.page)})}),document.getElementById("nav-logo").addEventListener("click",()=>Mt("home")),document.getElementById("nav-credits").addEventListener("click",()=>Mt("boosters")),(n=document.getElementById("journal-btn"))==null||n.addEventListener("click",()=>zr())}async function zr(){const{data:e}=await w.from("patch_notes").select("*").eq("is_published",!0).order("published_at",{ascending:!1}).limit(20),t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9000;display:flex;align-items:center;justify-content:center;padding:16px";const i=(e||[]).map(n=>{const a=new Date(n.published_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"long",year:"numeric"});return`<div style="padding:14px 0;border-bottom:1px solid #f0f0f0">
    ...
    </div>`,document.body.appendChild(t),t.querySelector("#journal-close").addEventListener("click",()=>t.remove()),t.addEventListener("click",n=>{n.target===t&&t.remove()})}async function Ir(){document.documentElement.setAttribute("data-theme","light"),document.getElementById("modal-overlay").addEventListener("click",i=>{i.target===i.currentTarget&&gi()}),document.getElementById("modal-close").addEventListener("click",gi);const{data:{session:e}}=await w.auth.getSession();if(!e){Vi(),ji(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:bt});return}ve.user=e.user,await It(),Vi();try{const{data:i}=await w.from("formation_links_overrides").select("formation, links"),n={};(i||[]).forEach(a=>{n[a.formation]=a.links}),zn(n)}catch(i){console.warn("Impossible de charger les overrides de formation:",i)}if(!ve.profile){Sn(document.getElementById("app"),{state:ve,navigate:async()=>{await It(),ri()},toast:bt,refreshProfile:It});return}const t=Array.isArray(ve.profile.pending_boosters)?ve.profile.pending_boosters:[];if(!ve.profile.onboarding_done&&t.length>0){ko(document.getElementById("app"),{state:ve,navigate:()=>ri(),toast:bt,refreshProfile:It});return}ri(),setTimeout(()=>br(ve.profile,Mt,bt),800),w.auth.onAuthStateChange(async(i,n)=>{i==="SIGNED_OUT"&&(ve.user=null,ve.profile=null,document.getElementById("app").innerHTML="",ji(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:bt}))})}function Mr(){return Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight)}function Qt(){const e=document.getElementById("app");e&&(e.style.height=Mr()+"px")}window.addEventListener("resize",Qt);window.addEventListener("orientationchange",()=>setTimeout(Qt,150));window.visualViewport&&window.visualViewport.addEventListener("resize",Qt);function ri(){const e=()=>{var i;(i=ve.user)!=null&&i.id&&w.from("users").update({last_seen_at:new Date().toISOString()}).eq("id",ve.user.id).then(()=>{})};e(),window._presencePingInterval&&clearInterval(window._presencePingInterval),window._presencePingInterval=setInterval(e,6e4);const t=document.getElementById("app");t.style.display="flex",t.style.flexDirection="column",Qt(),Tr(),wn()}function Vi(){const e=document.getElementById("app-loader"),t=document.getElementById("app");t&&(t.style.display=""),e&&(e.classList.add("zoom-out"),setTimeout(()=>e.style.display="none",500))}function _n(e){var n;const t=document.getElementById("app-loader");if(t&&(t.style.display="none"),document.getElementById("boot-error"))return;const i=document.createElement("div");i.id="boot-error",i.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:99999;gap:16px;color:#fff;padding:24px;text-align:center",i.innerHTML=`
    <div style="font-size:42px">📡</div>
    <div style="font-size:18px;font-weight:900">Connexion impossible</div>
    <div style="font-size:13px;color:rgba(255,255,255,0.6);max-width:280px">${e||"Le chargement a échoué. Vérifie ta connexion et réessaie."}</div>
    <button id="boot-retry" style="margin-top:8px;padding:12px 30px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer">Réessayer</button>`,document.body.appendChild(i),(n=document.getElementById("boot-retry"))==null||n.addEventListener("click",()=>window.location.reload())}Ir().catch(e=>{console.error("Échec du démarrage:",e),_n()});setTimeout(()=>{const e=document.getElementById("app-loader");e&&e.style.display!=="none"&&!e.classList.contains("zoom-out")&&!document.getElementById("boot-error")&&_n("Le serveur met trop de temps à répondre.")},12e3);
