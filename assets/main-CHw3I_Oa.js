import{s as $,F as et,b as Me,c as Ft,l as ge,d as ai}from"./formation-links-CDBKB_z4.js";function _t(e,{navigate:t,toast:i}){e.innerHTML=`
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
  ...
  </div>`}async function gi(e,t){const{state:i,navigate:s,toast:c,openModal:o,closeModal:a}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:d}=await $.from("cards").select(`id, card_type, current_note, gc_type, formation, is_for_sale, sale_price,
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),n=(d||[]).filter(_=>_.card_type==="player"&&_.player),r=(d||[]).filter(_=>_.card_type==="game_changer"),p=(d||[]).filter(_=>_.card_type==="formation"),{data:g}=await $.from("gc_definitions").select("name,gc_type,color,effect,image_url"),f={};(g||[]).forEach(_=>{f[_.name]=_});const y=Object.keys(et),u=Object.keys(de),h={};n.forEach(_=>{const b=_.player.id;h[b]=(h[b]||0)+1}),new Set(Object.keys(h).map(_=>String(_)));const L=new Set(p.map(_=>_.formation)),v=new Set(r.map(_=>_.gc_type));let x="player",k="Tous",B="",m=!1;function E(){return[...n].sort((_,b)=>{const C=je.indexOf(_.player.job),A=je.indexOf(b.player.job);return C!==A?C-A:(_.player.surname_encoded||"").localeCompare(b.player.surname_encoded||"")})}function M(){return[...l||[]].sort((_,b)=>{const C=je.indexOf(_.job),A=je.indexOf(b.job);return C!==A?C-A:(_.surname_encoded||"").localeCompare(b.surname_encoded||"")})}function G(){return E().filter(_=>{const b=_.player,C=k==="Tous"||b.job===k,A=!B||`${b.firstname} ${b.surname_encoded}`.toLowerCase().includes(B);return C&&A})}function D(){return M().filter(_=>{const b=k==="Tous"||_.job===k,C=!B||`${_.firstname} ${_.surname_encoded}`.toLowerCase().includes(B);return b&&C})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",b=>{B=b.target.value.toLowerCase(),K()}),e.querySelectorAll(".filter-btn").forEach(b=>{b.addEventListener("click",()=>{k=b.dataset.job,P(),K()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{m=!m,P(),K()})):(_.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{m=!m,P(),K()})))}function K(){const _=document.getElementById("col-grid");_&&(x==="player"?F(_):x==="formation"?R(_):q(_))}function Y(_,b,C,A,w){w=w||"#7a28b8";const j=document.getElementById("col-grid"),T=document.getElementById("col-big");if(!j||!T)return;var N=0;function S(){T.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+b(_[N])+"</div>";var H=T.querySelector("[data-card-id],[data-form-id],[data-gc-id]");H&&H.addEventListener("click",function(){A(_[N])}),requestAnimationFrame(function(){var U=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!U||!T)){var J=T.clientHeight-8,ie=T.clientWidth-24,re=U.offsetHeight,ae=U.offsetWidth;if(re>0&&ae>0&&J>40){var se=Math.min(J/re,ie/ae,1);U.style.transform="scale("+se.toFixed(3)+")",U.style.transformOrigin="top center"}}}),j.innerHTML=_.map(function(U,J){return'<div class="col-mini-item" data-idx="'+J+'" style="flex-shrink:0;cursor:pointer;border-radius:8px;border:1.5px solid '+(J===N?w:"transparent")+';transition:border-color .15s;overflow:hidden">'+C(U,J===N)+"</div>"}).join(""),j.querySelectorAll(".col-mini-item").forEach(function(U){U.addEventListener("click",function(){N=Number(U.dataset.idx),S(),U.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}S()}function V(_){var b=.54,C=Math.round(140*b),A=Math.round(310*b),w;if(!_||_._fake){var j=_?_.player:null;if(!j)return"";w=At(j)}else w=Ue(_,"");return'<div style="width:'+C+"px;height:"+A+'px;overflow:hidden;position:relative;flex-shrink:0"><div style="transform:scale('+b+');transform-origin:top left;position:absolute;top:0;left:0;pointer-events:none">'+w+"</div></div>"}function ee(_,b,C){b=b||100,C=C||140;var A=Me[_]||{},w={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},j=Math.max(3,Math.round(b*.06)),T=Object.entries(A).map(function(S){var H=S[0],U=S[1],J=H.replace(/\d+$/,""),ie=w[J]||"#888",re=Math.round(U.x*b),ae=Math.round(U.y*C);return'<circle cx="'+re+'" cy="'+ae+'" r="'+j+'" fill="'+ie+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),N=Math.max(1,Math.round(b/50));return'<svg viewBox="0 0 '+b+" "+C+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+b+'" height="'+C+'" fill="#1A6B3C"/><rect x="'+Math.round(b*.2)+'" y="'+Math.round(C*.02)+'" width="'+Math.round(b*.6)+'" height="'+Math.round(C*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+N+'"/><line x1="0" y1="'+Math.round(C*.5)+'" x2="'+b+'" y2="'+Math.round(C*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+N+'"/><ellipse cx="'+Math.round(b*.5)+'" cy="'+Math.round(C*.5)+'" rx="'+Math.round(b*.18)+'" ry="'+Math.round(C*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+N+'"/><rect x="'+Math.round(b*.2)+'" y="'+Math.round(C*.82)+'" width="'+Math.round(b*.6)+'" height="'+Math.round(C*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+N+'"/>'+T+"</svg>"}function I(_,b,C){var A=C>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+C+"</div>":"",w=b?'data-form-id="'+b.id+'"':"",j=_.length>7?14:_.length>5?16:19,T=!!b;return"<div "+w+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(T?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(T?"":"filter:grayscale(1);opacity:0.5")+'">'+A+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(T?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+j+"px;font-weight:900;color:"+(T?"#1A6B3C":"#aaa")+';line-height:1">'+_+'</div></div><div style="flex:1;overflow:hidden;background:'+(T?"#1A6B3C":"#ccc")+'">'+ee(_,140,220)+"</div></div>"}function z(_,b){var C=.54,A=Math.round(140*C),w=Math.round(305*C),j=Math.round(w*.22),T=w-j,N=_.length>7?5:7,S=ee(_,A,T),H=b?"1.5px solid #2a7a40":"1px solid #ddd",U=b?"":"filter:grayscale(1);opacity:0.45;",J=b?"#1A6B3C":"#bbb",ie="#fff";return'<div style="width:'+A+"px;height:"+w+"px;border-radius:6px;border:"+H+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+U+'"><div style="height:'+j+"px;background:"+J+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+N+"px;font-weight:900;color:"+ie+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(A-4)+'px">'+_+'</span></div><div style="height:'+T+'px;overflow:hidden;flex-shrink:0">'+S+"</div></div>"}function F(_){if(m){const b=D();if(!b.length){_.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const C=b.map(A=>n.find(w=>w.player.id===A.id)||{_fake:!0,player:A,id:"fake-"+A.id});Y(C,A=>A._fake?At(A.player):Ue(A,""),A=>A._fake?V({player:A.player,id:"x",_fake:!0}):V(A),A=>{A._fake||It(A,n,h,t)},"#1A6B3C")}else{const b=G();if(!b.length){_.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const C={},A=[];b.forEach(w=>{C[w.player.id]||(C[w.player.id]=!0,A.push(w))}),Y(A,w=>{const j=h[w.player.id]||1,T=j>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${j}</div>`:"",S=n.filter(H=>H.player.id===w.player.id&&H.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Ue(w,T+S)},w=>V(w),w=>It(w,n,h,t),"#1A6B3C")}}function R(_){const b=m?y:[...L];if(!b.length){_.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const C=b.map(A=>({formation:A,card:p.find(w=>w.formation===A)||null,owned:L.has(A)}));Y(C,({formation:A,card:w,owned:j})=>I(A,j?w:null,j?p.filter(T=>T.formation===A).length:0),({formation:A,owned:w})=>z(A,w),({card:A,owned:w})=>{w&&A&&mi(A,p,t,o)},"#1A6B3C")}function q(_){const b=Object.keys(f),C=m?b.length?b:u:[...v];if(!C.length){_.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const A=C.map(w=>({type:w,gc:de[w]||{icon:"⚡",desc:""},def:f[w]||null,owned:v.has(w),card:r.find(j=>j.gc_type===w)||null}));Y(A,({type:w,gc:j,def:T,owned:N,card:S})=>{const H=N?r.filter(ze=>ze.gc_type===w).length:0,U=H>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${H}</div>`:"",J=(T==null?void 0:T.gc_type)==="ultra_game_changer",ie={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},re={purple:"#b06ce0",light_blue:"#00d4ef"},ae=ie[T==null?void 0:T.color]||ie.purple,se=re[T==null?void 0:T.color]||re.purple,xe=(T==null?void 0:T.effect)||j.desc||"",me=T!=null&&T.image_url?`/manager-wars/icons/${T.image_url}`:null;return N&&S?`<div data-gc-id="${S.id}" data-gc-type="${w}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${se};background:${ae};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${se}66;cursor:pointer">
          ${U}
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const $e=1e3;function mi(e,t,i,s){var u,h,L;const{state:c,toast:o,closeModal:a,navigate:d,refreshProfile:l}=i,n=e.formation,r={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function p(){const v=Me[n]||{},x=et[n]||[],k=290,B=360,m=20;function E(G){const D=v[G];return D?{x:D.x*k,y:D.y*B}:null}let M=`<svg width="${k}" height="${B}" viewBox="0 0 ${k} ${B}" xmlns="http://www.w3.org/2000/svg">`;for(const[G,D]of x){const P=E(G),K=E(D);!P||!K||(M+=`<line x1="${P.x}" y1="${P.y}" x2="${K.x}" y2="${K.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const G of Object.keys(v)){const D=E(G);if(!D)continue;const P=G.replace(/\d+/,""),K=r[P]||"#555";M+=`<circle cx="${D.x}" cy="${D.y}" r="${m}" fill="${K}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,M+=`<text x="${D.x}" y="${D.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${P}</text>`}return M+="</svg>",M}const g=t.filter(v=>v.formation===n),f=g.length,y=!e.is_for_sale;s(`Formation ${n}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
      ...
      <div style="width:140px;border-radius:12px;padding:6px;background:${k};flex-shrink:0">
        <div style="background:#f2e8d2;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
          ...
          <div style="background:#f2e8d2;padding:3px 6px;display:flex;align-items:center;justify-content:space-between;min-height:24px">
            ...
            ${(M=r.clubs)!=null&&M.logo_url?`<img src="${r.clubs.logo_url}" style="width:22px;height:16px;object-fit:contain">`:`<div style="background:#1a3a7a;color:#fff;border-radius:2px;padding:1px 3px;font-size:6px;font-weight:800">${(((G=r.clubs)==null?void 0:G.encoded_name)||"").slice(0,6)}</div>`}
          </div>
        </div>
      </div>
    ...
    </div>`:""}`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(D=document.getElementById("close-detail"))==null||D.addEventListener("click",d),(P=document.getElementById("direct-sell-btn"))==null||P.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte ${r.surname_encoded} pour ${f.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const V=p.find(I=>!I.is_for_sale)||p[0];if(!V){o("Aucune carte à vendre","error");return}const{error:ee}=await $.from("cards").delete().eq("id",V.id);if(ee){o(ee.message,"error");return}await $.from("users").update({credits:(c.profile.credits||0)+f}).eq("id",c.profile.id),await n(),o(`+${f.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),d(),l("collection")}),(K=document.getElementById("market-sell-btn"))==null||K.addEventListener("click",async()=>{const V=parseInt(document.getElementById("sell-price").value);if(!V||V<1){o("Prix invalide","error");return}await $.from("cards").update({is_for_sale:!0,sale_price:V}).eq("id",e.id),await $.from("market_listings").insert({seller_id:c.profile.id,card_id:e.id,price:V}),o("Carte mise en vente sur le marché !","success"),d(),l("collection")}),(Y=document.getElementById("cancel-sell-btn"))==null||Y.addEventListener("click",async()=>{await $.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await $.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),o("Annonce retirée","success"),d(),l("collection")})}const qe={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},pe={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function Oe(e){const t=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function Nt(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function Pt(e){var i;const t="https://fcnwclxlkytdfjotqkta.supabase.co";return(i=e==null?void 0:e.clubs)!=null&&i.logo_url?e.clubs.logo_url.startsWith("http")?e.clubs.logo_url:`${t}/storage/v1/object/public/assets/clubs/${e.clubs.logo_url}`:null}function yi(e,t=44,i=58){var f;const s=e?Oe(e):null,c=e?Pt(e):null,o=Nt(e==null?void 0:e.country_code),a=(e==null?void 0:e.job)||"MIL",d=pe[a]||"#555",l={normal:"#aaa",pépite:"#D4A017",papyte:"#222",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",n=Number(a==="GK"?e==null?void 0:e.note_g:a==="DEF"?e==null?void 0:e.note_d:a==="MIL"?e==null?void 0:e.note_m:e==null?void 0:e.note_a)||0,r=Math.round(i*.19),p=Math.round(i*.25),g=i-r-p;return e?`<div style="width:${t}px;height:${i}px;border-radius:5px;border:2px solid ${l};background:${d};position:relative;overflow:hidden;flex-shrink:0">
  ...
  </div>`,hi(e,t,c,i),document.getElementById("builder-back").addEventListener("click",()=>s("decks")),document.getElementById("formation-select").addEventListener("change",n=>{t.formation=n.target.value;const r=Lt(t.formation),p={};r.forEach(g=>{t.slots[g]&&(p[g]=t.slots[g])}),t.slots=p,be(e,t,i)}),document.getElementById("save-deck").addEventListener("click",()=>wi(t,i)),e.querySelectorAll("[data-remove-sub]").forEach(n=>{n.addEventListener("click",()=>{t.subs=t.subs.filter(r=>r!==n.dataset.removeSub),be(e,t,i)})}),(l=document.getElementById("add-sub-btn"))==null||l.addEventListener("click",()=>{bi(t,e,i)})}function hi(e,t,i,s){const c=e.querySelector("#deck-field");if(!c)return;const o=Me[t.formation]||{},a=Ft(t.formation)||[],d={};for(const L of i){const v=t.slots[L],x=v?t.playerCards.find(k=>k.id===v):null;d[L]=x?x.player:null}const l=300,n=300,r=48,p=64,g=13,f=16,y=38;function u(L){const v=o[L];return v?{x:v.x*l,y:v.y*n}:null}let h="";for(const[L,v]of a){const x=u(L),k=u(v);if(!x||!k)continue;const B=d[L]?{...d[L],club_id:d[L].club_id,country_code:d[L].country_code,rarity:d[L].rarity}:null,m=d[v]?{...d[v],club_id:d[v].club_id,country_code:d[v].country_code,rarity:d[v].rarity}:null,E=ge(B,m);E==="#ff3333"||E==="#cc2222"?h+=`<line x1="${x.x.toFixed(1)}" y1="${x.y.toFixed(1)}" x2="${k.x.toFixed(1)}" y2="${k.y.toFixed(1)}" stroke="${E}" stroke-width="2.5" stroke-linecap="round" opacity="0.4"/>`:(h+=`<line x1="${x.x.toFixed(1)}" y1="${x.y.toFixed(1)}" x2="${k.x.toFixed(1)}" y2="${k.y.toFixed(1)}" stroke="${E}" stroke-width="8" stroke-linecap="round" opacity="0.2"/>`,h+=`<line x1="${x.x.toFixed(1)}" y1="${x.y.toFixed(1)}" x2="${k.x.toFixed(1)}" y2="${k.y.toFixed(1)}" stroke="${E}" stroke-width="2.5" stroke-linecap="round" opacity="0.9"/>`)}for(const L of i){const v=u(L);if(!v)continue;const x=d[L],k=L.replace(/\d+/,""),B=pe[k]||"#555",m=(v.x-r/2).toFixed(1),E=(v.y-p/2).toFixed(1),M={normal:"#aaa",pépite:"#D4A017",papyte:"#222",légende:"#7a28b8"}[x==null?void 0:x.rarity]||"#aaa";if(x){const G=Oe(x),D=Pt(x),P=Nt(x.country_code),K=Number(k==="GK"?x.note_g:k==="DEF"?x.note_d:k==="MIL"?x.note_m:x.note_a)||0,Y=p-g-f;h+=`<defs><clipPath id="dcp-${L}"><rect x="${m}" y="${(v.y-p/2+g).toFixed(1)}" width="${r}" height="${Y}"/></clipPath></defs>`,h+=`<rect x="${m}" y="${E}" width="${r}" height="${p}" rx="5" fill="${B}"/>`,G&&(h+=`<image href="${G}" x="${m}" y="${(v.y-p/2+g).toFixed(1)}" width="${r}" height="${Y}" clip-path="url(#dcp-${L})" preserveAspectRatio="xMidYMin slice"/>`),h+=`<rect x="${m}" y="${E}" width="${r}" height="${g}" fill="rgba(255,255,255,0.93)"/>`,h+=`<text x="${v.x.toFixed(1)}" y="${(v.y-p/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(x.surname_encoded||"").slice(0,9)}</text>`;const V=(v.y+p/2-f).toFixed(1);h+=`<rect x="${m}" y="${V}" width="${r}" height="${f}" fill="rgba(255,255,255,0.93)"/>`,P&&(h+=`<image href="${P}" x="${(v.x-21).toFixed(1)}" y="${(v.y+p/2-f+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`),h+=`<text x="${v.x.toFixed(1)}" y="${(v.y+p/2-f/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${K}</text>`,D&&(h+=`<image href="${D}" x="${(v.x+r/2-14).toFixed(1)}" y="${(v.y+p/2-f+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`),h+=`<rect x="${m}" y="${E}" width="${r}" height="${p}" rx="5" fill="none" stroke="${M}" stroke-width="2"/>`}else h+=`<rect x="${m}" y="${E}" width="${r}" height="${p}" rx="5" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="5,3"/>`,h+=`<text x="${v.x.toFixed(1)}" y="${v.y.toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="22" fill="rgba(255,255,255,0.35)">+</text>`,h+=`<text x="${v.x.toFixed(1)}" y="${(v.y+16).toFixed(1)}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.3)">${k}</text>`;h+=`<rect x="${m}" y="${E}" width="${r}" height="${p}" rx="5" fill="rgba(0,0,0,0.01)" class="deck-slot-hit" data-pos="${L}" style="cursor:pointer"/>`}c.innerHTML=`<svg viewBox="${-y} ${-y} ${l+y*2} ${n+y*2}" width="100%" style="display:block;max-width:440px;margin:0 auto">${h}</svg>`,c.querySelectorAll(".deck-slot-hit").forEach(L=>{L.addEventListener("click",()=>vi(L.dataset.pos,t,e,s))})}function vi(e,t,i,s){var g,f,y;const{openModal:c,closeModal:o}=s,a=e.replace(/\d+/,""),d=t.slots[e],l=d?t.playerCards.find(u=>u.id===d):null;(g=l==null?void 0:l.player)==null||g.id;const n=new Set;Object.entries(t.slots).forEach(([u,h])=>{var v;if(u===e||!h)return;const L=t.playerCards.find(x=>x.id===h);(v=L==null?void 0:L.player)!=null&&v.id&&n.add(L.player.id)}),t.subs.forEach(u=>{var L;const h=t.playerCards.find(v=>v.id===u);(L=h==null?void 0:h.player)!=null&&L.id&&n.add(h.player.id)});const r=new Set,p=t.playerCards.filter(u=>{const h=u.player;return!(h.job===a||h.job2===a)||n.has(h.id)||r.has(h.id)?!1:(r.add(h.id),!0)});p.sort((u,h)=>{const L=a==="GK"?u.player.note_g:a==="DEF"?u.player.note_d:a==="MIL"?u.player.note_m:u.player.note_a;return(a==="GK"?h.player.note_g:a==="DEF"?h.player.note_d:a==="MIL"?h.player.note_m:h.player.note_a)-L}),c(`Choisir ${a} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}const Oi="simulation",Kt="VOTRE_ZONE_ID";function Hi(){switch(Oi){case"propellerads":return Ki();case"adsense":return Ui();default:return rt()}}function rt(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:#111;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:14px";let i=5;t.innerHTML=`
    ...
    </div>`,e.querySelectorAll(".gc-select-card").forEach(y=>{y.addEventListener("click",()=>{const u=y.dataset.id,h=o.find(v=>v.id===u);if(!h)return;const L=a.findIndex(v=>v.gc_type===h.gc_type);L>-1?a.splice(L,1):a.length<3&&a.push(h),n()})}),(p=e.querySelector("#gc-launch"))==null||p.addEventListener("click",()=>{r&&l(a)}),(g=e.querySelector("#gc-no-gc"))==null||g.addEventListener("click",()=>l([])),(f=e.querySelector("#gc-reset"))==null||f.addEventListener("click",()=>{a.length&&(a=[],n())})}n()}async function Vi(e,t,i){const{state:s,navigate:c}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await $.from("decks").select("id,name,is_active,formation").eq("owner_id",s.profile.id).order("created_at",{ascending:!1});if(!o||o.length===0){ue(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>c("decks"));return}const a=o.map(r=>r.id),{data:d}=await $.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order");let l=0;function n(){var v,x,k,B,m;const r=o[l],p=(d||[]).filter(E=>E.deck_id===r.id),g=p.filter(E=>{var M;return E.is_starter&&((M=E.card)==null?void 0:M.player)}).map(E=>Ye(E.card,E.position)),f=p.find(E=>{var M;return((M=E.card)==null?void 0:M.card_type)==="formation"}),y=r.formation||((v=f==null?void 0:f.card)==null?void 0:v.formation)||"4-4-2",u=g.length>=11?Ne(g,y):null,h=g.length>=11;Vt(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">
        ${o.map((E,M)=>`<div style="width:7px;height:7px;border-radius:50%;background:${M===l?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}
      </div>`:""}
    ...
    </div>`,function(){const M=e.querySelector(".deck-preview-wrap svg");M&&(M.removeAttribute("width"),M.removeAttribute("height"),M.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",M.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),(x=document.getElementById("prev-deck"))==null||x.addEventListener("click",()=>{l>0&&(l--,n())}),(k=document.getElementById("next-deck"))==null||k.addEventListener("click",()=>{l<o.length-1&&(l++,n())}),(B=document.getElementById("validate-deck"))==null||B.addEventListener("click",()=>{h&&t.navigate("match",{matchMode:i,deckId:r.id})}),(m=document.getElementById("cancel-deck-select"))==null||m.addEventListener("click",()=>{le(e),c("home")});const L=document.getElementById("deck-swipe-zone");if(L){let E=0,M=0;L.addEventListener("touchstart",G=>{E=G.touches[0].clientX,M=G.touches[0].clientY},{passive:!0}),L.addEventListener("touchend",G=>{const D=G.changedTouches[0].clientX-E,P=G.changedTouches[0].clientY-M;Math.abs(D)<40||Math.abs(D)<Math.abs(P)||(D<0&&l<o.length-1?(l++,n()):D>0&&l>0&&(l--,n()))},{passive:!0})}}n()}function Yt(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";return e!=null&&e.clubLogo?e.clubLogo.startsWith("http")?e.clubLogo:`${t}/storage/v1/object/public/assets/clubs/${e.clubLogo}`:null}function fe(e,t=44,i=58){if(!e)return`<div style="width:${t}px;height:${i}px;border:1.5px dashed rgba(255,255,255,0.2);border-radius:5px"></div>`;const s=typeof e.portrait=="string"&&e.portrait.startsWith("http")?e.portrait:oe(e),c=Yt(e),o=e._line||e.job||"MIL",a=_e[o]||"#555",d={normal:"#aaa",pépite:"#D4A017",papyte:"#222",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",l=e.note!==void 0?Number(e.note)||0:(Number(te(e,o))||0)+(e.boost||0),n=Jt(e==null?void 0:e.country_code),r=Math.round(i*.19),p=Math.round(i*.25),g=i-Math.round(i*.19)-Math.round(i*.25),f=e!=null&&e.used?.28:1;return`<div style="width:${t}px;height:${i}px;border-radius:5px;border:2px solid ${d};background:${a};position:relative;overflow:hidden;flex-shrink:0;opacity:${f}">
    <div style="position:absolute;top:0;left:0;right:0;height:${r}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:center;z-index:2">
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;height:${p}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ${Je(e==null?void 0:e.country_code)?`<img src="${Je(e.country_code)}" style="width:${p+2}px;height:${p-3}px;object-fit:cover;border-radius:1px">`:`<span style="font-size:${p-4}px">${n}</span>`}
      ...
      ${c?`<img src="${c}" style="width:${p-4}px;height:${p-4}px;object-fit:contain">`:e!=null&&e.clubName?`<span style="font-size:${Math.max(4,p-8)}px;font-weight:700;color:#333">${(e.clubName||"").slice(0,3).toUpperCase()}</span>`:""}
        ...
        stroke="${M}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const g=48,f=64,y=13,u=16,h={normal:"#aaaaaa",pépite:"#D4A017",papyte:"#111111",légende:"#7a28b8"};for(const[v,x]of Object.entries(l)){const k=r(v);if(!k||!x)continue;const B=v.replace(/[0-9]/g,""),m=_e[B]||"#555",E=i==="attack"&&["MIL","ATT"].includes(B)&&!x.used||i==="defense"&&["GK","DEF","MIL"].includes(B)&&!x.used,M=s.includes(x.cardId);let G;i==="attack"?G=B==="MIL"?Number(x.note_m)||0:Number(x.note_a)||0:i==="defense"?G=B==="GK"?Number(x.note_g)||0:B==="MIL"?Number(x.note_m)||0:Number(x.note_d)||0:G=Number(B==="GK"?x.note_g:B==="DEF"?x.note_d:B==="MIL"?x.note_m:x.note_a)||0,G=G+(x.boost||0);const D=(k.x-g/2).toFixed(1),P=(k.y-f/2).toFixed(1),K=x.used?.25:1,Y=h[x==null?void 0:x.rarity]||h.normal,V=M?"#ff3030":Y,ee=M?3:(x==null?void 0:x.rarity)==="légende"||(x==null?void 0:x.rarity)==="pépite"?2.5:2,I=f-y-u;p+=`<defs><clipPath id="cp-${v}"><rect x="${D}" y="${(k.y-f/2+y).toFixed(1)}" width="${g}" height="${I}"/></clipPath></defs>`,p+=`<rect x="${D}" y="${P}" width="${g}" height="${f}" rx="5" fill="${m}" opacity="${K}"/>`;const z=oe(x);z&&!x.used&&(p+=`<image href="${z}" x="${D}" y="${(k.y-f/2+y).toFixed(1)}" width="${g}" height="${I}" clip-path="url(#cp-${v})" preserveAspectRatio="xMidYMin slice"/>`),p+=`<rect x="${D}" y="${P}" width="${g}" height="${y}" rx="4" fill="rgba(255,255,255,0.92)"/>`,p+=`<text x="${k.x.toFixed(1)}" y="${(k.y-f/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(x.name||"").slice(0,9)}</text>`;const F=(k.y+f/2-u).toFixed(1);if(p+=`<rect x="${D}" y="${F}" width="${g}" height="${u}" fill="rgba(255,255,255,0.92)"/>`,x.used)p+=`<text x="${k.x.toFixed(1)}" y="${(k.y+f/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="13" font-weight="900" fill="rgba(0,0,0,0.4)" font-family="Arial Black">–</text>`;else{const R=Je(x.country_code);R?p+=`<image href="${R}" x="${(k.x-20).toFixed(1)}" y="${(k.y+f/2-u+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:p+=`<text x="${(k.x-13).toFixed(1)}" y="${(k.y+f/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${Jt(x.country_code)}</text>`,p+=`<text x="${k.x.toFixed(1)}" y="${(k.y+f/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${G}</text>`;const q=Yt(x);q?p+=`<image href="${q}" x="${(k.x+g/2-14).toFixed(1)}" y="${(k.y+f/2-u+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:x.clubName&&(p+=`<text x="${(k.x+14).toFixed(1)}" y="${(k.y+f/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(x.clubName||"").slice(0,3).toUpperCase()}</text>`),x.boost&&(p+=`<rect x="${(k.x+g/2-12).toFixed(1)}" y="${(k.y-f/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,p+=`<text x="${(k.x+g/2-5).toFixed(1)}" y="${(k.y-f/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${x.boost}</text>`)}p+=`<rect x="${D}" y="${P}" width="${g}" height="${f}" rx="5" fill="${M?"rgba(255,255,255,0.12)":"none"}" stroke="${V}" stroke-width="${ee}" opacity="${K}"/>`,E&&(p+=`<rect x="${D}" y="${P}" width="${g}" height="${f}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${M?"selected":""}" data-card-id="${x.cardId}" data-role="${B}" style="cursor:pointer"/>`)}const L=38;return`<svg viewBox="${-L} ${-L} ${c+L*2} ${o+L*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${p}
  ...
  </div>`,setTimeout(()=>Zi(e,t,i),5e3)}function Zi(e,t,i){const s=t.homeTeam.MIL||[],c=t.aiTeam.MIL||[];function o(p){return p.reduce((g,f)=>g+te(f,"MIL"),0)}function a(p){let g=0;for(let f=0;f<p.length-1;f++){const y=ge(p[f],p[f+1]);y==="#00ff88"?g+=2:y==="#FFD700"&&(g+=1)}return g}const d=o(s)+a(s),l=o(c)+a(c),n=d>=l;function r(p,g,f){return`<div style="text-align:center">
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div id="midfield-result" style="opacity:0;text-align:center;transition:opacity 0.5s;color:#fff;max-width:320px"></div>
  </div>`,setTimeout(()=>{const p=document.getElementById("score-home"),g=document.getElementById("score-ai"),f=document.getElementById("midfield-result");if(p&&g&&(n?(p.style.fontSize="80px",p.style.color="#FFD700",g.style.opacity="0.25"):(g.style.fontSize="80px",g.style.color="#ff6b6b",p.style.opacity="0.25")),f){const y=Ut();f.style.opacity="1",n&&t.clubName,f.innerHTML=`
      ...
      </div>`,(h=document.getElementById("start-match-btn"))==null||h.addEventListener("click",()=>{t.phase=t.attacker==="home"?"attack":"ai-attack",ne(e,t,i),t.attacker==="ai"&&setTimeout(()=>Xe(e,t,i),800)})},100)},5e3)}function Qi(e){if(e.type==="duel"){const t=e.homeTotal>=e.aiTotal;return`
  ...
  <div class="match-screen" style="position:fixed;inset:0;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;height:100%;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:100px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const E=t.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
            ...
            ${Ee((E.players||[]).map(M=>({...M,used:!1})),"#ff6b6b",E.total)}
            ...
            ${Ee((E.players||[]).map(M=>({...M,used:!1})),"#00ff88",E.total)}
    ...
    ${(()=>{const m=window.innerWidth>=700,E=(I,z,F)=>{var H,U;const R=(t.gcDefs||[]).find(J=>J.name===I.gc_type),q={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[R==null?void 0:R.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",_={purple:"#b06ce0",light_blue:"#00d4ef"}[R==null?void 0:R.color]||"#b06ce0",b=(R==null?void 0:R.name)||I.gc_type,C=(R==null?void 0:R.effect)||((H=de[I.gc_type])==null?void 0:H.desc)||"",A=R!=null&&R.image_url?`/manager-wars/icons/${R.image_url}`:null,w=((U=de[I.gc_type])==null?void 0:U.icon)||"⚡",j=Math.round(F*.22),T=Math.round(F*.22),N=F-j-T,S=b.length>12?7:9;return`<div class="gc-mini" data-gc-id="${I.id}" data-gc-type="${I.gc_type}"
          style="box-sizing:border-box;width:${z}px;height:${F}px;border-radius:10px;border:2px solid ${_};background:${q};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
          ...
          <div style="height:${N}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
            ${A?`<img src="${A}" style="max-width:${z-10}px;max-height:${N-6}px;object-fit:contain">`:`<span style="font-size:${Math.round(N*.55)}px">${w}</span>`}
          </div>
          ...
          </div>`},G=(I,z)=>z?M(110,150):E(I,110,150),D=(I,z)=>z?M(68,95):E(I,68,95),P=m?"padding:14px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:10px 8px;border-radius:12px;font-size:13px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",K=r?`<button id="btn-results" style="${P};background:linear-gradient(135deg,#D4A017,#FFD700);border:none;color:#000">🏁 Résultats</button>`:d?`<div style="${P};background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);color:rgba(255,255,255,0.4)">⏳ Tour IA</div>`:l?`<button id="btn-action" style="${P};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${t.selected.length===0?"disabled":""}> ⚔️ ATTAQUEZ <span id="match-timer" style="font-weight:900"></span></button>`:n?`<button id="btn-action" style="${P};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${t.selected.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="match-timer" style="font-weight:900"></span></button>`:`<div style="${P};background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1)"></div>`,Y=l||n?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${t.selected.length}/3 sélectionné(s)</div>`:"",V=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${m?56:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
        ...
        <div style="display:flex;flex:0.8;min-height:0;overflow:hidden">
          ...
          <div style="width:140px;flex-shrink:0;display:flex;flex-direction:column;padding:10px 8px;background:rgba(0,0,0,0.2)">
            ...
            <div style="width:100%;flex-shrink:0;padding-top:8px">${K}${Y}</div>
          </div>
        </div>`:`
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(m=>{var E,M,G;if(m.type==="duel"){const D=m.isGoal,P=m.homeScored?"#FFD700":D?"#ff6b6b":"rgba(255,255,255,0.3)",K=m.homeScored?"⚽ BUT !":D?"⚽ BUT IA !":(E=m.homePlayers)!=null&&E.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${D?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${P};margin-bottom:4px">
                <div style="font-size:9px;color:${P};letter-spacing:1px;margin-bottom:5px;font-weight:700;text-transform:uppercase">${K}</div>
  ...
  </div>`,function(){const E=e.querySelector("#dbg-bar");if(!E)return;const M=window.visualViewport,G=e.querySelector(".match-screen"),D=e.querySelector("#mobile-action-bar"),P=e.querySelector("#btn-action, #btn-results");setTimeout(()=>{const K=G?G.getBoundingClientRect():null,Y=D?D.getBoundingClientRect():null;E.textContent=`iW${window.innerWidth} iH${window.innerHeight} vvH${M?Math.round(M.height):"-"} PC${window.innerWidth>=700?1:0} ph:${t.phase} ms${K?Math.round(K.height):"-"} bar${Y?Math.round(Y.bottom):"-"}/${Y?Math.round(Y.top):"-"} btn${P?1:0}`},60)}(),function(){const E=e.querySelector(".match-screen");if(E){const D=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);E.style.height=D+"px",E.style.maxHeight=D+"px",E.style.overflow="hidden"}const M=e.querySelector("#mobile-action-bar"),G=e.querySelector("#mobile-play-area");M&&G&&requestAnimationFrame(()=>{G.style.paddingBottom=M.offsetHeight+"px"})}(),function(){const E=e.querySelector(".terrain-wrapper svg");E&&(E.removeAttribute("width"),E.removeAttribute("height"),E.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",E.setAttribute("viewBox","-26 -26 352 352"),E.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),t._resizeBound||(t._resizeBound=!0,window.addEventListener("resize",()=>{const m=e.querySelector(".terrain-wrapper svg");m&&(m.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0")})),t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase==="attack"||t.phase==="defense"){let m=!1,E=30;const M=()=>document.getElementById("match-timer"),G=()=>{const D=M();if(!D)return;const P=String(Math.floor(E/60)).padStart(2,"0"),K=String(E%60).padStart(2,"0");D.textContent=` ${P}:${K}`,D.style.color=m?"#ff2222":"#ff9500",D.style.fontWeight="900"};G(),t._timerInt=setInterval(()=>{if(E--,E<0)if(!m)m=!0,E=15,G();else{clearInterval(t._timerInt),t._timerInt=null,t.homeScore=0,t.aiScore=3;const D=document.createElement("div");D.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;color:#fff;padding:24px;text-align:center",D.innerHTML='<div style="font-size:56px">⏱️</div><div style="font-size:24px;font-weight:900;color:#ff4444">MATCH PERDU PAR FORFAIT</div><div style="font-size:14px;color:rgba(255,255,255,0.6)">Temps écoulé</div>',document.body.appendChild(D),setTimeout(()=>{D.remove(),ve(e,t,i)},2500)}else G()},1e3)}(y=document.getElementById("match-quit"))==null||y.addEventListener("click",()=>{le(e),confirm("Abandonner ? Résultat : défaite 3-0")&&(t.homeScore=0,t.aiScore=3,ve(e,t,i))}),(u=document.getElementById("view-ai"))==null||u.addEventListener("click",()=>lo(t,i)),(h=document.getElementById("toggle-history"))==null||h.addEventListener("click",()=>{var m;(m=document.getElementById("match-history-panel"))==null||m.classList.add("open")}),(L=document.getElementById("close-history"))==null||L.addEventListener("click",()=>{var m;(m=document.getElementById("match-history-panel"))==null||m.classList.remove("open")}),(v=document.getElementById("btn-action"))==null||v.addEventListener("click",()=>{t.selected.length!==0&&(l?to(e,t,i):n&&io(e,t,i))}),(x=document.getElementById("btn-results"))==null||x.addEventListener("click",()=>ve(e,t,i)),e.querySelectorAll(".match-slot-hit").forEach(m=>{m.addEventListener("click",()=>eo(m,t,e,i))}),e.querySelectorAll(".gc-mini").forEach(m=>{m.addEventListener("click",()=>ro(m.dataset.gcId,m.dataset.gcType,e,t,i))}),(k=document.getElementById("boost-card"))==null||k.addEventListener("click",()=>so(e,t,i)),e.querySelectorAll(".sub-btn-col").forEach(m=>{m.addEventListener("click",()=>Ct(e,t,i,m.dataset.subId))}),(B=document.getElementById("sub-btn-main"))==null||B.addEventListener("click",()=>Ct(e,t,i))}function eo(e,t,i,s){const c=e.dataset.cardId,o=e.dataset.role,a=t.selected.findIndex(d=>d.cardId===c);if(a!==-1)t.selected.splice(a,1);else{if(t.selected.length>=3){s.toast("Maximum 3 joueurs","error");return}const d=(t.homeTeam[o]||[]).find(l=>l.cardId===c);d&&t.selected.push({...d,_role:o,_line:o})}ne(i,t,s)}function at(e,t,i){e.matchId&&$.from("matches").update({last_player_id:i}).eq("id",e.matchId).then(()=>{})}function to(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),at(t,i,i.state.profile.id);const s=t.selected.map(o=>({...o,_line:o._role})),c=tt(s,t.modifiers.home);t.pendingAttack={...c,players:[...t.selected],side:"home"},t.selected.forEach(o=>{const a=(t.homeTeam[o._role]||[]).find(d=>d.cardId===o.cardId);a&&(a.used=!0)}),t.log.push({text:`⚔️ Vous attaquez : ${c.total} (base ${c.base}${c.links?` +${c.links} liens`:""}) — ${t.selected.map(o=>o.name).join(", ")}`,type:"info"}),t.selected=[],t.modifiers.home={},t.phase="ai-defense",ne(e,t,i),setTimeout(()=>oo(e,t,i),1200)}function io(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),at(t,i,i.state.profile.id);const s=t.selected.map(d=>({...d,_line:d._role})),c=it(s,t.modifiers.home);t.selected.forEach(d=>{const l=(t.homeTeam[d._role]||[]).find(n=>n.cardId===d.cardId);l&&(l.used=!0)});const o=ot(t.pendingAttack.total,c.total,t.modifiers.home),a={type:"duel",title:"Défense",aiPlayers:(t.pendingAttack.players||[]).map(d=>({name:d.name,note:d._line==="MIL"?d.note_m:d.note_a,portrait:oe(d),job:d.job,country_code:d.country_code,rarity:d.rarity,clubName:d.clubName,clubLogo:d.clubLogo})),homePlayers:t.selected.map(d=>{const l=(t.homeTeam[d._role]||[]).find(n=>n.cardId===d.cardId)||d;return{name:l.name,note:l._line==="GK"?l.note_g:l._line==="MIL"?l.note_m:l.note_d,portrait:oe(l),job:l.job,country_code:l.country_code,rarity:l.rarity,clubName:l.clubName,clubLogo:l.clubLogo}}),homeTotal:c.total,aiTotal:t.pendingAttack.total,isGoal:!1,homeScored:!1,text:""};if(o.shielded)a.text="🛡️ Bouclier ! But annulé.",t.log.push(a);else if(o.goal){t.aiScore++,a.isGoal=!0,a.homeScored=!1,a.text=`⚽ BUT IA ! (${t.pendingAttack.total} > ${c.total})`,t.log.push(a),t.selected=[],t.modifiers.home={},t.pendingAttack=null,ne(e,t,i),st(a.aiPlayers,t.homeScore,t.aiScore,!1,()=>{Le(e,t,i,"home-attack")});return}else a.text=`🧤 Défense réussie ! (${c.total} ≥ ${t.pendingAttack.total})`,t.log.push(a);t.selected=[],t.modifiers.home={},t.pendingAttack=null,Le(e,t,i,"home-attack")}function Xe(e,t,i){at(t,i,null);const s=[...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]],c=Gt(s,"attack",t.difficulty);if(!c.length){Qt(e,t,i);return}const o=tt(c,t.modifiers.ai);t.pendingAttack={...o,players:c,side:"ai"},c.forEach(n=>{n.used=!0}),t.log.push({text:`🤖 IA attaque : ${o.total} (${c.map(n=>n.name).join(", ")})`,type:"info"}),t.modifiers.ai={};const a=[...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(n=>!n.used),l=(t.homeSubs||[]).filter(n=>!(t.usedSubIds||[]).includes(n.cardId)).length>0&&t.subsUsed<t.maxSubs;if(a.length===0&&!l){t.aiScore++;const n={type:"duel",isGoal:!0,homeScored:!1,aiPlayers:c.map(r=>({name:r.name,note:r._line==="MIL"?r.note_m:r.note_a,portrait:oe(r),job:r.job,country_code:r.country_code,rarity:r.rarity,clubName:r.clubName,clubLogo:r.clubLogo})),aiTotal:o.total,text:"⚽ BUT IA ! (aucun défenseur disponible)"};t.log.push(n),t.pendingAttack=null,ne(e,t,i),st(n.aiPlayers,t.homeScore,t.aiScore,!1,()=>{Le(e,t,i,"home-attack")});return}t.phase="defense",ne(e,t,i)}function oo(e,t,i){const s=[...t.aiTeam.GK||[],...t.aiTeam.DEF||[],...t.aiTeam.MIL||[]],c=Gt(s,"defense",t.difficulty),o=c.length>0?it(c,t.modifiers.ai).total:0;c.forEach(l=>{l.used=!0});const a=ot(t.pendingAttack.total,o,t.modifiers.ai),d={type:"duel",title:"Attaque",homePlayers:(t.pendingAttack.players||[]).map(l=>({name:l.name,note:l._line==="MIL"?l.note_m:l.note_a,portrait:oe(l),job:l.job,country_code:l.country_code,rarity:l.rarity,clubName:l.clubName,clubLogo:l.clubLogo})),aiPlayers:c.map(l=>({name:l.name,note:l._line==="GK"?l.note_g:l._line==="MIL"?l.note_m:l.note_d,portrait:oe(l),job:l.job,country_code:l.country_code,rarity:l.rarity,clubName:l.clubName,clubLogo:l.clubLogo})),homeTotal:t.pendingAttack.total,aiTotal:o,isGoal:!1,homeScored:!1,text:""};if(a.shielded)d.text="🛡️ Bouclier IA !",t.log.push(d);else if(a.goal){t.homeScore++,d.isGoal=!0,d.homeScored=!0,d.text=`⚽ BUT ! (${t.pendingAttack.total} > ${o})`,t.log.push(d),t.modifiers.ai={},t.pendingAttack=null,ne(e,t,i),st(d.homePlayers,t.homeScore,t.aiScore,!0,()=>{Le(e,t,i,"ai-attack")});return}else d.text=`🧤 IA défend (${o} ≥ ${t.pendingAttack.total})`,t.log.push(d);t.modifiers.ai={},t.pendingAttack=null,Le(e,t,i,"ai-attack")}function Le(e,t,i,s){if(t.round++,Zt(t)){ve(e,t,i);return}if(s==="home-attack"){if(![...t.homeTeam.MIL||[],...t.homeTeam.ATT||[]].filter(o=>!o.used).length){if(![...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(a=>!a.used).length){ve(e,t,i);return}t.phase="ai-attack",ne(e,t,i),setTimeout(()=>Xe(e,t,i),800);return}t.phase="attack",ne(e,t,i)}else{if(![...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]].filter(o=>!o.used).length){Qt(e,t,i);return}t.phase="ai-attack",ne(e,t,i),setTimeout(()=>Xe(e,t,i),800)}}function Zt(e){const t=["MIL","ATT","GK","DEF"].some(s=>(e.homeTeam[s]||[]).some(c=>!c.used)),i=["MIL","ATT","GK","DEF"].some(s=>(e.aiTeam[s]||[]).some(c=>!c.used));return!t&&!i}function Qt(e,t,i){Zt(t)?ve(e,t,i):(t.phase="attack",ne(e,t,i))}function no(e,t,i){const s=document.createElement("div");s.style.cssText=`
  ...
  `,document.body.appendChild(s);let r=!1;const p=()=>{r||(r=!0,s.remove(),i())};s.addEventListener("click",p),setTimeout(p,2e3)}function ke(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function Ct(e,t,i,s=null){if(t.phase!=="attack"){ke("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){ke(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const c=Object.values(t.homeTeam).flat().filter(p=>p.used),o=t.homeSubs.filter(p=>!t.usedSubIds.includes(p.cardId));if(!c.length){ke("Aucun joueur utilisé à remplacer");return}if(!o.length){ke("Aucun remplaçant disponible");return}let a=0,d=Math.max(0,o.findIndex(p=>p.cardId===s)),l=!1;const n=document.createElement("div");n.id="sub-overlay",n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function r(){var L,v,x,k,B,m;const p=c[a],g=o[d],f=Math.min(130,Math.round((window.innerWidth-90)/2)),y=Math.round(f*1.35),u=E=>`background:rgba(255,255,255,0.12);border:none;color:${E?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${E?"default":"pointer"};flex-shrink:0`;n.innerHTML=`
    ...
    </div>`,(L=n.querySelector("#sub-close"))==null||L.addEventListener("click",()=>n.remove()),(v=n.querySelector("#out-up"))==null||v.addEventListener("click",()=>{a>0&&(a--,r())}),(x=n.querySelector("#out-down"))==null||x.addEventListener("click",()=>{a<c.length-1&&(a++,r())}),(k=n.querySelector("#in-up"))==null||k.addEventListener("click",()=>{d>0&&(d--,r())}),(B=n.querySelector("#in-down"))==null||B.addEventListener("click",()=>{d<o.length-1&&(d++,r())});const h=(E,M,G,D)=>{const P=n.querySelector("#"+E);if(!P)return;let K=0;P.addEventListener("touchstart",Y=>{K=Y.touches[0].clientY},{passive:!0}),P.addEventListener("touchend",Y=>{const V=Y.changedTouches[0].clientY-K;if(Math.abs(V)<30)return;const ee=M();V<0&&ee<D-1?(G(ee+1),r()):V>0&&ee>0&&(G(ee-1),r())},{passive:!0})};h("in-panel",()=>d,E=>d=E,o.length),h("out-panel",()=>a,E=>a=E,c.length),(m=n.querySelector("#sub-confirm"))==null||m.addEventListener("click",E=>{if(E.preventDefault(),E.stopPropagation(),l)return;l=!0;const M=c[a],G=o[d];if(!M||!G)return;let D=null,P=-1;for(const[Y,V]of Object.entries(t.homeTeam)){const ee=(V||[]).findIndex(I=>I.cardId===M.cardId);if(ee!==-1){D=Y,P=ee;break}}if(P===-1||!D){ke("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),n.remove();return}const K={...G,_line:D,_col:M._col||0,used:!1,boost:0};t.homeTeam[D].splice(P,1,K),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(G.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:M.name,firstname:M.firstname,note:te(M,D),portrait:oe(M),job:M.job,country_code:M.country_code,rarity:M.rarity,clubName:M.clubName,clubLogo:M.clubLogo},inPlayer:{name:G.name,firstname:G.firstname,note:te(G,D),portrait:oe(G),job:G.job,country_code:G.country_code,rarity:G.rarity,clubName:G.clubName,clubLogo:G.clubLogo},text:`🔄 ${G.firstname} ${G.name} remplace ${M.firstname} ${M.name}`}),n.remove(),no(M,G,()=>ne(e,t,i))})}document.body.appendChild(n),r()}function ro(e,t,i,s,c){var y,u;const o=(s.gcDefs||[]).find(h=>h.name===t),a=de[t]||{icon:"⚡",desc:"Carte spéciale."},d={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[o==null?void 0:o.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",l={purple:"#b06ce0",light_blue:"#00d4ef"}[o==null?void 0:o.color]||"#b06ce0",n=(o==null?void 0:o.name)||t,r=(o==null?void 0:o.effect)||a.desc,p=o!=null&&o.image_url?`/manager-wars/icons/${o.image_url}`:null,g=a.icon||"⚡",f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",f.innerHTML=`
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(c=>{c.addEventListener("click",()=>{const o=c.dataset.cardId;for(const a of["GK","DEF","MIL","ATT"]){const d=(t.homeTeam[a]||[]).find(l=>l.cardId===o);if(d){d.boost=(d.boost||0)+t.boostCard.value,t.log.push({text:`⚡ Boost +${t.boostCard.value} appliqué à ${d.name}`,type:"info"});break}}t.boostUsed=!0,i.closeModal(),ne(e,t,i)})})}function st(e,t,i,s,c){const o=document.createElement("div");o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:900;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;overflow:hidden;cursor:pointer";const a=Array.from({length:10},(n,r)=>`
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(o);let d=!1;const l=()=>{d||(d=!0,o.remove(),setTimeout(()=>c(),50))};o.addEventListener("click",l),setTimeout(l,3500)}async function ve(e,t,i){var r,p;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:s}=i,c=t.homeScore>t.aiScore,o=t.homeScore===t.aiScore,a=c?"victoire":o?"nul":"defaite",d=pi(t.mode,a);t.matchId&&await $.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:c?s.profile.id:null,home_credits_reward:d,played_at:new Date().toISOString()}).eq("id",t.matchId);const l={credits:(s.profile.credits||0)+d,matches_played:(s.profile.matches_played||0)+1};c?l.wins=(s.profile.wins||0)+1:o?l.draws=(s.profile.draws||0)+1:l.losses=(s.profile.losses||0)+1,await $.from("users").update(l).eq("id",s.profile.id),await i.refreshProfile();const n=document.createElement("div");n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",n.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const f=()=>document.getElementById("mm-status"),y=async(x=!0)=>{p=!0,g&&($.removeChannel(g),g=null),x&&await $.rpc("cancel_matchmaking",{p_user_id:l.profile.id}).catch(()=>{})};(v=document.getElementById("mm-cancel"))==null||v.addEventListener("click",async()=>{try{await y(!0)}catch(x){console.warn("[Matchmaking] cleanup error:",x)}le(e),n("home")});const u=async(x,k,B)=>{p||(p=!0,g&&($.removeChannel(g),g=null),f()&&(f().textContent="Adversaire trouvé !"),await new Promise(m=>setTimeout(m,600)),e.isConnected!==!1&&uo(e,t,x,B,a,d))},{data:h,error:L}=await $.rpc("try_matchmake",{p_user_id:l.profile.id,p_deck_id:i});if(L||!(h!=null&&h.success)){r("Erreur de matchmaking","error"),le(e),n("home");return}if(h.matched){u(h.match_id,h.opponent_id,!1);return}f()&&(f().textContent="En attente d'un autre joueur..."),g=$.channel("matchmaking-"+l.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${l.profile.id}`},x=>{const k=x.new;k.status==="matched"&&k.match_id&&u(k.match_id,k.matched_with,!0)}).subscribe()}async function uo(e,t,i,s,c=[],o=[]){const{state:a,navigate:d,toast:l}=t,n=s?"p1":"p2",r=s?"p2":"p1",p=(c||[]).map(I=>I.id),g=(c||[]).map(I=>({id:I.id,gc_type:I.gc_type,_gcDef:I._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation du match...</div>';const{data:f}=await $.from("matches").select("*").eq("id",i).single();if(!f){l("Match introuvable","error"),d("home");return}async function y(){const[{data:I,error:z},{data:F,error:R},{data:q},{data:_}]=await Promise.all([$.rpc("get_deck_for_match",{p_deck_id:f.home_deck_id}),$.rpc("get_deck_for_match",{p_deck_id:f.away_deck_id}),$.from("users").select("id,pseudo,club_name").eq("id",f.home_id).single(),$.from("users").select("id,pseudo,club_name").eq("id",f.away_id).single()]);z&&console.error("[PvP] get_deck_for_match p1 error:",z),R&&console.error("[PvP] get_deck_for_match p2 error:",R);function b(S,H){return{cardId:S.card_id,position:H||S.position,id:S.id,firstname:S.firstname,name:S.surname_encoded,country_code:S.country_code,club_id:S.club_id,job:S.job,job2:S.job2,note_g:Number(S.note_g)||0,note_d:Number(S.note_d)||0,note_m:Number(S.note_m)||0,note_a:Number(S.note_a)||0,rarity:S.rarity,skin:S.skin,hair:S.hair,hair_length:S.hair_length,clubName:S.club_encoded_name||null,clubLogo:S.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}}const C=((I==null?void 0:I.starters)||[]).map(S=>b(S,S.position)),A=((F==null?void 0:F.starters)||[]).map(S=>b(S,S.position)),w=((I==null?void 0:I.subs)||[]).map(S=>b(S,S.position)),j=((F==null?void 0:F.subs)||[]).map(S=>b(S,S.position)),T=(I==null?void 0:I.formation)||"4-4-2",N=(F==null?void 0:F.formation)||"4-4-2";return console.log("[PvP] p1Formation:",T,"p1 starters:",C.length,"positions:",C.map(S=>S.position)),console.log("[PvP] p2Formation:",N,"p2 starters:",A.length,"positions:",A.map(S=>S.position)),{p1Team:Ne(C,T),p2Team:Ne(A,N),p1Subs:w,p2Subs:j,p1Formation:T,p2Formation:N,p1Name:(q==null?void 0:q.club_name)||(q==null?void 0:q.pseudo)||"Joueur 1",p2Name:(_==null?void 0:_.club_name)||(_==null?void 0:_.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:s?p:[],gc_p2:s?[]:p,gcCardsFull_p1:s?g:[],gcCardsFull_p2:s?[]:g,gcReady_p1:!0,gcReady_p2:!0,usedGc_p1:[],usedGc_p2:[],lastActionAt:new Date().toISOString()}}let u=f.game_state&&Object.keys(f.game_state).length?f.game_state:null;if(!u)if(s){u=await y();const{data:I}=await $.from("matches").select("game_state").eq("id",i).single();!(I!=null&&I.game_state)||!Object.keys(I.game_state).length?await $.from("matches").update({game_state:u,turn_user_id:f.home_id}).eq("id",i):u=I.game_state}else{e.innerHTML=`<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation avec l'adversaire...</div>`;for(let I=0;I<30&&!u;I++){await new Promise(F=>setTimeout(F,400));const{data:z}=await $.from("matches").select("game_state").eq("id",i).single();z!=null&&z.game_state&&Object.keys(z.game_state).length&&(u=z.game_state)}if(!u){l("Erreur de synchronisation","error"),d("home");return}u.gc_p2=p,u.gcCardsFull_p2=g,await $.from("matches").update({game_state:u}).eq("id",i)}let h=!1;function L(I){var C;try{$.removeChannel(v)}catch{}const z=a.profile.id,F=I.winner_id===z,R=!!I.forfeit,q=u[n+"Score"]??0,_=u[r+"Score"]??0,b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center",b.innerHTML=`
      ...
      <div class="terrain-wrapper" style="width:100%;height:100%;overflow:hidden">
        ...
        </div>`;const X=Array.isArray(u.log)?u.log:[],Q=X[X.length-1];return Q?`<div style="padding:7px 10px;border-left:3px solid ${Q.type==="goal"?"#FFD700":Q.type==="stop"?"#00ff88":Q.type==="attack"?"#ff6b6b":"rgba(255,255,255,0.6)"};font-size:12px;color:#fff">${Q.text||""}</div>`:'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})(),ii=(Array.isArray(u.log)?u.log:[]).length,pt=`
      <!-- Score -->
      <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 14px;background:rgba(0,0,0,0.25);flex-shrink:0">
        ...
        <div style="flex:1;text-align:center;color:#fff;font-size:14px;font-weight:700">
          ${R} <span style="color:#FFD700;font-size:18px">${z}</span> – <span style="color:#FFD700;font-size:18px">${F}</span> ${q}
        </div>
      ...
      <div style="text-align:center;padding:4px;background:rgba(0,0,0,0.15);font-size:11px;color:${C?"rgba(255,255,255,0.4)":"#FFD700"};font-weight:700;flex-shrink:0">
        ${C?`⏳ Tour de ${q}`:_?"⚔️ À vous d'attaquer !":b?"🛡️ À vous de défendre !":""}
      </div>
      <!-- Zone actions -->
      ...
      </div>`;const O=e.querySelector(".terrain-wrapper");O&&(O.innerHTML=Te(I,u[n+"Formation"],_?"attack":b?"defense":"idle",w,300,300))}if(e.querySelectorAll(".match-slot-hit").forEach(O=>{O.addEventListener("click",()=>{if(!_&&!b)return;const X=O.dataset.cardId,Q=O.dataset.role,W=(I[Q]||[]).find(ye=>ye.cardId===X);if(!W||W.used)return;Array.isArray(u["selected_"+n])||(u["selected_"+n]=[]);const ce=u["selected_"+n],Ce=ce.findIndex(ye=>ye.cardId===X);Ce>-1?ce.splice(Ce,1):ce.length<3&&ce.push({...W,_role:Q}),B()})}),(ut=e.querySelector("#pvp-boost-card"))==null||ut.addEventListener("click",()=>{G()}),e.querySelectorAll(".pvp-gc-mini").forEach(O=>{O.addEventListener("click",()=>{K(O.dataset.gcId,O.dataset.gcType)})}),(ft=e.querySelector("#pvp-action"))==null||ft.addEventListener("click",()=>{_?V():b&&ee()}),(gt=e.querySelector("#pvp-quit"))==null||gt.addEventListener("click",()=>{confirm("Quitter le match ? Vous perdrez par forfait.")&&k()}),(xt=e.querySelector("#pvp-view-opp"))==null||xt.addEventListener("click",()=>{P()}),(mt=e.querySelector("#pvp-toggle-history"))==null||mt.addEventListener("click",()=>{D()}),function(){const X=e.querySelector(".terrain-wrapper svg");X&&(X.removeAttribute("width"),X.removeAttribute("height"),X.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",X.setAttribute("viewBox","-26 -26 352 352"),X.setAttribute("preserveAspectRatio","xMidYMid meet"));const Q=e.querySelector(".match-screen");if(Q){const W=e.clientHeight;W>50&&(Q.style.height=W+"px",Q.style.maxHeight=W+"px",Q.style.overflow="hidden")}}(),u._timerInt&&(clearInterval(u._timerInt),u._timerInt=null),(_||b)&&!h){let O=30,X=!1;const Q=()=>document.getElementById("pvp-timer"),W=()=>{Q()&&(Q().textContent=O+"s",Q().style.color=X?"#ff4444":"#fff")};W(),u._timerInt=setInterval(()=>{O--,O<0?X?(clearInterval(u._timerInt),u._timerInt=null,k()):(X=!0,O=15,W()):W()},1e3)}}function m(){var b;const I=u["gcReady_"+n],z=u["gcReady_"+r];Array.isArray(u["gc_"+n])||(u["gc_"+n]=[]);let F=u["gc_"+n];const R=3;function q(C,A){const w=C._gcDef,j={purple:"linear-gradient(135deg,#3d0a7a,#7a28b8)",light_blue:"linear-gradient(135deg,#006080,#00bcd4)"},T={purple:"#9b59b6",light_blue:"#00bcd4"},N=j[w==null?void 0:w.color]||j.purple,S=T[w==null?void 0:w.color]||T.purple;return`<div class="pvp-gc-card" data-id="${C.id}"
        style="width:100px;border-radius:10px;border:3px solid ${A?"#FFD700":S};background:${N};
          display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0;position:relative;
    ...
    <div id="pvp-gc-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      <div style="text-align:center;padding:12px 16px 8px;flex-shrink:0">
        ...
        <div style="font-size:20px;font-weight:900;color:#fff">Choisir ses Game Changers</div>
        <div style="font-size:13px;color:rgba(255,255,255,0.5);margin-top:3px">
          Sélectionne <b style="color:#FFD700">${R}</b> cartes · ${F.length}/${R}
        </div>
      </div>
      ${I?`
    ...
    </div>`,I)return;const _=n==="p1"?f.home_id:f.away_id;$.from("cards").select("id,gc_type").eq("owner_id",_).eq("card_type","game_changer").then(async({data:C})=>{const{data:A}=await $.from("gc_definitions").select("*").eq("is_active",!0),w=new Set,j=(C||[]).filter(N=>w.has(N.gc_type)?!1:(w.add(N.gc_type),!0)).map(N=>({...N,_gcDef:(A==null?void 0:A.find(S=>S.name===N.gc_type))||null})),T=document.getElementById("pvp-gc-grid");if(T){if(!j.length){T.innerHTML='<div style="color:rgba(255,255,255,0.4);font-size:13px;text-align:center;margin-top:30px">Aucune carte Game Changer disponible.</div>';return}T.innerHTML=j.map(N=>q(N,F.includes(N.id))).join(""),T.querySelectorAll(".pvp-gc-card").forEach(N=>{N.addEventListener("click",()=>{const S=N.dataset.id,H=F.indexOf(S);H>-1?F.splice(H,1):F.length<R&&F.push(S),B()})})}}),(b=e.querySelector("#pvp-gc-validate"))==null||b.addEventListener("click",async()=>{const C=n==="p1"?f.home_id:f.away_id,{data:A}=await $.from("cards").select("id,gc_type").eq("owner_id",C).eq("card_type","game_changer").in("id",F),{data:w}=await $.from("gc_definitions").select("*").eq("is_active",!0),j=(A||[]).map(T=>({...T,_gcDef:(w==null?void 0:w.find(N=>N.name===T.gc_type))||null}));await x({["gcReady_"+n]:!0,["gcCardsFull_"+n]:j}),u["gcReady_"+r]&&await x({phase:"reveal"})})}function E(){e.style.overflow="hidden",e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
      ...
      <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
    </div>`,n==="p1"&&setTimeout(async()=>{await x({phase:"midfield"})},5e3)}function M(){const I=u[n+"Team"].MIL||[],z=u[r+"Team"].MIL||[];function F(A){return A.reduce((w,j)=>w+te(j,"MIL"),0)}function R(A){let w=0;for(let j=0;j<A.length-1;j++){const T=ge(A[j],A[j+1]);T==="#00ff88"?w+=2:T==="#FFD700"&&(w+=1)}return w}const q=F(I)+R(I),_=F(z)+R(z),b=q>=_;function C(A,w,j){return`<div style="text-align:center">
        <div style="font-size:10px;color:rgba(255,255,255,0.5);letter-spacing:2px;margin-bottom:8px;text-transform:uppercase">${w}</div>
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${A.map((T,N)=>{const S=oe(T),H=N<A.length-1?ge(T,A[N+1]):null,U=H&&H!=="#ff3333"&&H!=="#cc2222";return`
        ...
        <div style="margin-top:6px;font-size:11px;color:rgba(255,255,255,0.5)">
          Score: ${F(A)} + ${R(A)} liens = <b style="color:#fff">${F(A)+R(A)}</b>
        </div>
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div id="pvp-midfield-result" style="opacity:0;text-align:center;transition:opacity 0.5s;color:#fff;max-width:320px"></div>
    </div>`,setTimeout(()=>{const A=document.getElementById("pvp-score-me"),w=document.getElementById("pvp-score-opp"),j=document.getElementById("pvp-midfield-result");A&&w&&(b?(A.style.fontSize="80px",A.style.color="#FFD700",w.style.opacity="0.25"):(w.style.fontSize="80px",w.style.color="#ff6b6b",A.style.opacity="0.25"));const T=u.p1Team.MIL||[],N=u.p2Team.MIL||[],S=F(T)+R(T),H=F(N)+R(N),U=S>=H?"p1":"p2";u[U+"Name"];let J=u.boostValue;J==null&&(J=Ut(),u.boostValue=J,u.boostOwner=U,u.boostUsed=!1),j&&(j.style.opacity="1",j.innerHTML=`<div style="font-size:20px;font-weight:900;margin-bottom:10px">
        ...
        <div style="background:rgba(135,206,235,0.15);border:2px solid #87CEEB;border-radius:14px;padding:14px 24px;display:inline-block;margin-top:4px" id="pvp-boost-display">
          ...
          <div style="font-size:10px;color:rgba(135,206,235,0.7)">Applicable sur n'importe quel joueur</div>
        </div>`:""}`),setTimeout(async()=>{if(n!=="p1")return;const ie=U;await x({phase:ie+"-attack",attacker:ie,round:1,boostValue:u.boostValue,boostUsed:!1,boostOwner:ie})},1800)},600)}function G(){var R;const I=u[n+"Team"],z=Object.entries(I).flatMap(([q,_])=>(_||[]).filter(b=>!b.used).map(b=>({...b,_line:q})));if(!z.length)return;const F=document.createElement("div");F.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",F.innerHTML=`
      ...
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${z.map(q=>{const _={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[q._line]||"#555",b=te(q,q._line)+(q.boost||0);return`<div class="boost-pick-item" data-cid="${q.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${_};overflow:hidden;cursor:pointer">
      ...
      </div>`,document.body.appendChild(F),(R=F.querySelector("#boost-picker-close"))==null||R.addEventListener("click",()=>F.remove()),F.querySelectorAll(".boost-pick-item").forEach(q=>{q.addEventListener("click",async()=>{const _=q.dataset.cid,b=z.find(w=>w.cardId===_);if(!b)return;const C=u[n+"Team"],A=(C[b._line]||[]).find(w=>w.cardId===_);A&&(A.boost=(A.boost||0)+u.boostValue),F.remove(),await x({[n+"Team"]:C,boostUsed:!0})})})}function D(){var F;const I=Array.isArray(u.log)?u.log:[],z=document.createElement("div");z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column",z.innerHTML=`
      <div style="display:flex;align-items:center;padding:14px 16px;border-bottom:1px solid rgba(255,255,255,0.1);flex-shrink:0">
        <div style="flex:1;font-size:14px;font-weight:700;color:#fff">📋 Historique du match</div>
        <button id="pvp-hist-close" style="background:none;border:none;color:rgba(255,255,255,0.6);font-size:20px;cursor:pointer">✕</button>
      </div>
      <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
        ${I.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...I].reverse().map(R=>`<div style="padding:8px 10px;border-radius:8px;background:rgba(255,255,255,0.04);border-left:3px solid ${R.type==="goal"?"#FFD700":R.type==="defense"?"#00ff88":"rgba(255,255,255,0.5)"}">
      ...
      <div style="width:min(90vw,420px)">${Ie(u[r+"Team"],u[r+"Formation"],null,[],300,300)}</div>
      <button id="pvp-view-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(I),(z=I.querySelector("#pvp-view-opp-close"))==null||z.addEventListener("click",()=>I.remove())}function K(I,z){var S,H;const R=(u["gcCardsFull_"+n]||[]).find(U=>U.id===I),q=R==null?void 0:R._gcDef,_=de[z]||{icon:"⚡",desc:"Carte spéciale."},b={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[q==null?void 0:q.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",C={purple:"#b06ce0",light_blue:"#00d4ef"}[q==null?void 0:q.color]||"#b06ce0",A=(q==null?void 0:q.name)||z,w=(q==null?void 0:q.effect)||_.desc,j=q!=null&&q.image_url?`/manager-wars/icons/${q.image_url}`:null,T=_.icon||"⚡",N=document.createElement("div");N.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",N.innerHTML=`
      ...
      </div>`,document.body.appendChild(N),(S=N.querySelector("#pvp-gc-back"))==null||S.addEventListener("click",()=>N.remove()),(H=N.querySelector("#pvp-gc-use"))==null||H.addEventListener("click",async()=>{N.remove();const U=u["usedGc_"+n]||[];U.push(I),await x({["usedGc_"+n]:U})})}function Y(){var q;const I=u[n+"Score"],z=u[r+"Score"],F=I>z,R=I===z;e.style.overflow="hidden",e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      <div style="font-size:64px">${F?"🏆":R?"🤝":"😤"}</div>
      <div style="font-size:24px;font-weight:900;color:#fff">${F?"Victoire !":R?"Match nul":"Défaite"}</div>
    ...
    </div>`,(q=document.getElementById("pvp-home"))==null||q.addEventListener("click",()=>{$.removeChannel(v),le(e),d("home")})}async function V(){const I=(u["selected_"+n]||[]).map(R=>({...R,_line:R._role}));if(!I.length)return;const z=tt(I,u.modifiers[n]||{});I.forEach(R=>{const q=(u[n+"Team"][R._role]||[]).find(_=>_.cardId===R.cardId);q&&(q.used=!0)});const F=Array.isArray(u.log)?u.log:[];F.push({type:"attack",text:`⚔️ ${u[n+"Name"]} attaque (${z.total})`,players:I,total:z.total,side:n}),await x({pendingAttack:{...z,players:I,side:n},["selected_"+n]:[],modifiers:{...u.modifiers,[n]:{}},phase:r+"-defense",log:F})}async function ee(){const I=(u["selected_"+n]||[]).map(j=>({...j,_line:j._role})),z=it(I,u.modifiers[n]||{});I.forEach(j=>{const T=(u[n+"Team"][j._role]||[]).find(N=>N.cardId===j.cardId);T&&(T.used=!0)});const F=ot(u.pendingAttack.total,z.total,u.modifiers[n]||{}),R=u.pendingAttack.side,q=F==="attack",_=u[R+"Score"]+(q?1:0),b=R==="p1"?"p2":"p1",C=(u.round||0)+1,A=C>10,w=Array.isArray(u.log)?u.log:[];w.push({type:"defense",text:`🛡️ ${u[n+"Name"]} défend (${z.total})`,players:I,total:z.total,side:n}),w.push({type:q?"goal":"stop",text:q?`⚽ BUT de ${u[R+"Name"]} ! (${u.pendingAttack.total} vs ${z.total})`:`✋ Attaque stoppée (${u.pendingAttack.total} vs ${z.total})`}),await x({[R+"Score"]:_,["selected_"+n]:[],modifiers:{...u.modifiers,[n]:{}},pendingAttack:null,phase:A?"finished":b+"-attack",attacker:b,round:C,log:w}),A&&await $.from("matches").update({status:"finished"}).eq("id",i)}B()}const fo={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"};async function go(e,t){const{state:i,toast:s}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement du marché...</div>',await dt(e,t)}async function dt(e,t){const{state:i,toast:s}=t,{data:c}=await $.from("market_listings").select(`id, price, status, listed_at, seller_id,
  ...
  `;function l(n){const r=document.getElementById("mkt-content"),p=n==="buy"?a:d;if(p.length===0){r.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      ${g.map(f=>{var x,k,B,m;const y=(x=f.card)==null?void 0:x.player;if(!y)return"";const u=y.job==="GK"?y.note_g:y.job==="DEF"?y.note_d:y.job==="MIL"?y.note_m:y.note_a,h=fo[y.rarity],L=(i.profile.credits||0)>=f.price,v=f.status==="sold";return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${v?"opacity:0.65":""}">
          <div style="width:44px;height:44px;border-radius:8px;background:${yo(y.job)};color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:18px;border:2px solid ${h};flex-shrink:0">${u}</div>
          <div style="flex:1;min-width:0">
    ...
    </div>`,r.querySelectorAll("[data-buy]").forEach(f=>{f.addEventListener("click",()=>xo(f.dataset.buy,p,e,t))}),r.querySelectorAll("[data-cancel]").forEach(f=>{f.addEventListener("click",()=>mo(f.dataset.cancel,e,t))})}l("buy"),e.querySelectorAll(".mkt-tab").forEach(n=>{n.addEventListener("click",()=>{e.querySelectorAll(".mkt-tab").forEach(r=>{const p=r===n;r.style.background=p?"var(--green)":"#fff",r.style.color=p?"#fff":"var(--gray-600)",r.style.borderColor=p?"var(--green)":"var(--gray-200)"}),l(n.dataset.tab)})})}async function xo(e,t,i,s){var p;const{state:c,toast:o}=s,a=t.find(g=>g.id===e);if(!a)return;const d=a.price,l=c.profile.credits||0,n=(p=a.card)==null?void 0:p.player;if(l<d){o("Crédits insuffisants","error");return}if(!confirm(`Acheter ${n==null?void 0:n.firstname} ${n==null?void 0:n.surname_encoded} pour ${d.toLocaleString("fr")} crédits ?`))return;const r=document.querySelector(`[data-buy="${e}"]`);r&&(r.disabled=!0,r.textContent="⏳");try{const{data:g,error:f}=await $.rpc("buy_market_card",{p_listing_id:e,p_buyer_id:c.profile.id});if(f)throw new Error(f.message);if(!(g!=null&&g.success))throw new Error((g==null?void 0:g.error)||"Achat impossible");c.profile.credits=l-d;const y=document.querySelector("[data-credits]")||document.querySelector(".credits-display");y&&(y.textContent=(l-d).toLocaleString("fr")+" cr."),o(`✅ ${n==null?void 0:n.surname_encoded} ajouté à ta collection !`,"success"),dt(i,s)}catch(g){o("❌ "+g.message,"error"),r&&(r.disabled=!1,r.textContent="Acheter")}}async function mo(e,t,i){const{toast:s}=i,{data:c}=await $.from("market_listings").select("card_id").eq("id",e).single();await $.from("market_listings").update({status:"cancelled"}).eq("id",e),c&&await $.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",c.card_id),s("Annonce retirée","success"),dt(t,i)}function yo(e){return{GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[e]||"#888"}async function ho(e,{state:t,navigate:i,toast:s}){e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:c}=await $.from("users").select("id,pseudo,club_name,trophies_top1,trophies_top2,trophies_top3,wins,level").order("trophies_top1",{ascending:!1}).limit(100);e.innerHTML=`
  <div class="page">
    <div class="page-header">
  ...
  `}async function vo(e,{state:t,navigate:i,toast:s}){const c=t.profile;if(!c)return;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await $.from("matches").select(`id,home_id,away_id,home_score,away_score,status,mode,winner_id,created_at,played_at,
  ...
  </div>`}const Z={user:null,profile:null,page:"home",params:{}};function Be(e,t="info",i=3e3){const s=document.getElementById("toast");s&&(s.textContent=e,s.className=`show ${t}`,clearTimeout(s._t),s._t=setTimeout(()=>{s.className=""},i))}function bo(e,t,i=""){document.getElementById("modal-title").textContent=e,document.getElementById("modal-body").innerHTML=t,document.getElementById("modal-footer").innerHTML=i,document.getElementById("modal-overlay").classList.remove("hidden")}function Ze(){document.getElementById("modal-overlay").classList.add("hidden")}async function Fe(){if(!Z.user)return;const{data:e}=await $.from("users").select("*").eq("id",Z.user.id).single();e&&(Z.profile=e)}const lt="mw_theme";function Pe(){return localStorage.getItem(lt)||"light"}function wo(e){var t;localStorage.setItem(lt,e),Qe(e),(t=Z.profile)!=null&&t.id&&$.from("users").update({theme:e}).eq("id",Z.profile.id).then(()=>{})}function Qe(e){document.documentElement.setAttribute("data-theme",e)}function De(e,t={}){Z.page=e,Z.params=t,ei()}async function ei(){const e=document.getElementById("page-content");if(!e)return;document.querySelectorAll(".bottom-nav a").forEach(s=>{s.classList.toggle("active",s.dataset.page===Z.page)});const t=document.getElementById("nav-credits");t&&Z.profile&&(t.textContent=`💰 ${(Z.profile.credits||0).toLocaleString("fr")}`);const i={state:Z,navigate:De,toast:Be,openModal:bo,closeModal:Ze,refreshProfile:Fe};switch(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽</div>',Z.page){case"home":await kt(e,i);break;case"collection":await gi(e,i);break;case"decks":await Ve(e,i);break;case"boosters":await Ci(e,i);break;case"match":{(Z.params&&Z.params.matchMode||"vs_ai_easy")==="random"?await co(e,i):await Wi(e,i);break}case"market":await go(e,i);break;case"rankings":await ho(e,i);break;case"matches":await vo(e,i);break;default:await kt(e,i)}}function _o(){const e=document.getElementById("app"),t=Z.profile;if(!t)return;const i="/manager-wars/icons/";e.innerHTML=`
    <nav class="top-nav">
