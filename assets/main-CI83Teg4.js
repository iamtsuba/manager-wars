...
import{s as w,F as si,r as Ie,j as It,l as Gi,m as bt,h as di,n as mn,o as xn,T as Yt,k as yn,p as bn,b as hn}from"./formation-links-BZii7Dlg.js";const vn="/";function Li(e,{navigate:t,toast:i}){let n="login";const a=()=>{var r,s,d,l,c,u;const o=n==="login";e.innerHTML=`
  ...
  `;function s(){var f;const l=document.getElementById("logo-preview"),c=document.getElementById("logo-initials"),u=((f=document.getElementById("setup-club"))==null?void 0:f.value)||"MW",g=u.trim().split(" ").filter(Boolean),m=g.length>=2?(g[0][0]+g[1][0]).toUpperCase():u.slice(0,2).toUpperCase();l&&(l.style.background=r,l.style.borderColor=o),c&&(c.textContent=m,c.style.color=o)}document.getElementById("color1").addEventListener("input",l=>{o=l.target.value,document.getElementById("swatch1").style.background=o,s()}),document.getElementById("color2").addEventListener("input",l=>{r=l.target.value,document.getElementById("swatch2").style.background=r,s()});function d(l){document.querySelectorAll(".setup-step").forEach(c=>c.classList.remove("active")),document.getElementById(`step-${l}`).classList.add("active"),document.getElementById("step-num").textContent=l,document.getElementById("progress-fill").style.width=`${Math.round(l/3*100)}%`,l===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const l=document.getElementById("setup-pseudo").value.trim(),c=document.getElementById("step1-error");if(c.textContent="",l.length<3){c.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:u}=await w.from("users").select("id").eq("pseudo",l).maybeSingle();if(u){c.textContent="Ce pseudo est déjà pris.";return}d(2)}),document.getElementById("step2-back").addEventListener("click",()=>d(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const l=document.getElementById("setup-club").value.trim(),c=document.getElementById("step2-error");if(c.textContent="",l.length<2){c.textContent="Nom trop court (min. 2 caractères).";return}const{data:u}=await w.from("users").select("id").eq("club_name",l).maybeSingle();if(u){c.textContent="Ce nom de club est déjà pris.";return}d(3)}),document.getElementById("step3-back").addEventListener("click",()=>d(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const l=document.getElementById("setup-pseudo").value.trim(),c=document.getElementById("setup-club").value.trim(),u=document.getElementById("step3-error"),g=document.getElementById("step3-finish");u.textContent="",g.disabled=!0,g.textContent="Création en cours…";try{const{error:m}=await w.from("users").insert({id:t.user.id,pseudo:l,club_name:c,club_color1:o,club_color2:r,credits:1e4});if(m)throw m;await _n(t.user.id),await a(),n(`Bienvenue ${l} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(m){u.textContent=m.message,g.disabled=!1,g.textContent="🚀 Créer mon profil !"}})}async function _n(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await w.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const kn="modulepreload",$n=function(e){return"/"+e},Ti={},Ri=function(t,i,n){let a=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const r=document.querySelector("meta[property=csp-nonce]"),s=(r==null?void 0:r.nonce)||(r==null?void 0:r.getAttribute("nonce"));a=Promise.allSettled(i.map(d=>{if(d=$n(d),d in Ti)return;Ti[d]=!0;const l=d.endsWith(".css"),c=l?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${c}`))return;const u=document.createElement("link");if(u.rel=l?"stylesheet":kn,l||(u.as="script"),u.crossOrigin="",u.href=d,s&&u.setAttribute("nonce",s),document.head.appendChild(u),l)return new Promise((g,m)=>{u.addEventListener("load",g),u.addEventListener("error",()=>m(new Error(`Unable to preload CSS for ${d}`)))})}))}function o(r){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=r,window.dispatchEvent(s),!s.defaultPrevented)throw r}return a.then(r=>{for(const s of r||[])s.status==="rejected"&&o(s.reason);return t().catch(o)})},Lt="#1A6B3C",Tt="#cc2222",En="#D4A017",zi="#888";async function Ln(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    </style>`}const In="2026.07.14-2238";async function Ai(e,{state:t,navigate:i,toast:n}){var o,r;const a=t.profile;a&&(e.innerHTML=`
  <style>
    .home-dark {
      min-height: 100%;
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(pe=document.getElementById("close-detail"))==null||pe.addEventListener("click",s);let $=new Set;const P=()=>{const K=$.size,Q=document.getElementById("sell-action-panel");if(!Q)return;Q.style.display=K>0?"block":"none",document.getElementById("sell-selected-count").textContent=K;const V=document.getElementById("evolve-btn");V&&(V.textContent=`⬆️ Évoluer ${K>1?"(+"+K+")":""}`)};document.querySelectorAll(".expl-check").forEach(K=>{K.addEventListener("change",()=>{const Q=K.dataset.id;K.checked?$.add(Q):$.delete(Q);const V=K.closest(".exemplaire-row");V&&(V.style.borderColor=K.checked?"#1A6B3C":"#eee"),P()})}),document.querySelectorAll(".exemplaire-row").forEach(K=>{K.addEventListener("click",Q=>{if(Q.target.closest("button")||Q.target.tagName==="INPUT")return;const V=K.querySelector(".expl-check");V&&!V.disabled&&(V.checked=!V.checked,V.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(K=>{K.addEventListener("click",Q=>{Q.stopPropagation();const V=document.querySelector(`.expl-hist[data-hist="${K.dataset.idx}"]`);V&&(V.style.display=V.style.display==="none"?"flex":"none")})}),(le=document.getElementById("evolve-btn"))==null||le.addEventListener("click",async()=>{if(g<=1)return;const K=[...$];if(!K.length)return;if($.has(e.id)){const U=document.createElement("div");U.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",U.innerHTML=`
  ...
  `,document.body.appendChild(a);let o=!1;const r=document.getElementById("pack-cut-zone"),s=document.getElementById("pack-blade");let d=!1;const l=A=>A.touches&&A.touches[0]?A.touches[0].clientX:A.clientX;function c(A){o||(d=!0,s.style.opacity="1",u(A))}function u(A){if(!d||o)return;const $=r.getBoundingClientRect(),P=l(A)-$.left,ae=Math.max(0,Math.min(1,P/$.width));s.style.width=ae*$.width+"px",ae>=.82&&m()}function g(){o||(d=!1,s.style.transition="width .2s ease, opacity .2s ease",s.style.width="0",s.style.opacity="0",setTimeout(()=>{o||(s.style.transition="")},220))}function m(){var $;if(o)return;o=!0,d=!1,s.style.width="100%",s.style.opacity="1",($=document.getElementById("cut-flash"))==null||$.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const A=document.getElementById("cut-hint");A&&(A.style.opacity="0"),r.classList.add("pack-cut"),setTimeout(()=>{s.style.opacity="0",document.getElementById("pack-phase").style.display="none",b(0)},620)}r.addEventListener("pointerdown",c),window.addEventListener("pointermove",u),window.addEventListener("pointerup",g),r.addEventListener("touchstart",c,{passive:!0}),window.addEventListener("touchmove",u,{passive:!0}),window.addEventListener("touchend",g);let f=0,p=!1;const x=new Set;function b(A){f=A,document.getElementById("reveal-phase").style.display="flex",h(),v(A,0),F()}function h(){const A=document.getElementById("card-dots");A&&(A.innerHTML=e.map(($,P)=>`<div class="card-dot" data-i="${P}" style="width:8px;height:8px;border-radius:50%;background:${P===f?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),A.querySelectorAll(".card-dot").forEach($=>$.addEventListener("click",()=>y(parseInt($.dataset.i)))))}function v(A,$){var V,T;const P=e[A],ae=document.getElementById("card-counter"),pe=document.getElementById("card-track");ae&&(ae.textContent=`Carte ${A+1} / ${e.length}`);const le=document.getElementById("reveal-btns");le&&(le.style.display=A===e.length-1?"flex":"none");const se=P.card_type==="player"&&((V=P.player)==null?void 0:V.rarity)==="legende",ke=!x.has(A);x.add(A);let K=0;if(P.card_type==="player"&&P.player){const M=P.player,R=M.job||"ATT";K=Number(R==="GK"?M.note_g:R==="DEF"?M.note_d:R==="MIL"?M.note_m:M.note_a)||0}const Q=M=>{pe.innerHTML=`
        ...
        ${Q}
      </div>`,T&&navigator.vibrate&&navigator.vibrate([50,30,100,30,200])},(j+O)*3),setTimeout(U,(j+O)*3+j),setTimeout(()=>{M.style.display="none",R.innerHTML="",p=!1,H&&!V&&H.pause(),navigator.vibrate&&navigator.vibrate([40,30,80]),$()},(j+O)*4)}function y(A){if(p||A<0||A>=e.length||A===f)return;const $=A>f?1:-1;f=A,v(A,$)}function C(){y(f+1)}function E(){y(f-1)}function F(){const A=document.getElementById("card-viewport");if(!A||A._swipeBound)return;A._swipeBound=!0;let $=0,P=0,ae=0,pe=!1;const le=V=>V.touches?V.touches[0].clientX:V.clientX,se=V=>V.touches?V.touches[0].clientY:V.clientY,ke=V=>{pe=!0,$=le(V),P=se(V),ae=0},K=V=>{if(!pe)return;ae=le(V)-$;const T=se(V)-P;if(Math.abs(ae)<Math.abs(T))return;const M=document.getElementById("current-card-wrap");M&&(M.style.transition="none",M.style.transform=`translateX(${ae*.6}px) rotate(${ae*.02}deg)`)},Q=()=>{if(!pe)return;pe=!1;const V=document.getElementById("current-card-wrap"),T=55;ae<=-T&&f<e.length-1?C():ae>=T&&f>0?E():V&&(V.style.transition="transform .2s ease",V.style.transform="translateX(0)")};A.addEventListener("pointerdown",ke),A.addEventListener("pointermove",K),A.addEventListener("pointerup",Q),A.addEventListener("pointercancel",Q),A.addEventListener("touchstart",ke,{passive:!0}),A.addEventListener("touchmove",K,{passive:!0}),A.addEventListener("touchend",Q),A.addEventListener("click",V=>{if(Math.abs(ae)>8)return;const T=A.getBoundingClientRect();V.clientX-T.left>T.width/2?C():E()})}let N=null;function te(){const A=document.getElementById("fireworks-canvas");if(!A)return;A.width=window.innerWidth,A.height=window.innerHeight;const $=A.getContext("2d"),P=[];function ae(){const le=Math.random()*A.width,se=Math.random()*A.height*.6,ke=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],K=ke[Math.floor(Math.random()*ke.length)];for(let Q=0;Q<60;Q++){const V=Math.PI*2/60*Q,T=2+Math.random()*5;P.push({x:le,y:se,vx:Math.cos(V)*T,vy:Math.sin(V)*T,alpha:1,color:K,size:2+Math.random()*3})}}ae(),N=setInterval(ae,600);function pe(){if(document.getElementById("fireworks-canvas")){$.clearRect(0,0,A.width,A.height);for(let le=P.length-1;le>=0;le--){const se=P[le];if(se.x+=se.vx,se.y+=se.vy+.08,se.vy*=.98,se.alpha-=.018,se.alpha<=0){P.splice(le,1);continue}$.globalAlpha=se.alpha,$.fillStyle=se.color,$.beginPath(),$.arc(se.x,se.y,se.size,0,Math.PI*2),$.fill()}$.globalAlpha=1,(N!==null||P.length>0)&&requestAnimationFrame(pe)}}pe()}function ne(){N!==null&&(clearInterval(N),N=null);const A=document.getElementById("fireworks-canvas");A&&A.getContext("2d").clearRect(0,0,A.width,A.height)}if(n){const A=document.getElementById("reveal-btns");A&&(A.innerHTML='<button class="btn btn-primary" id="reveal-next" style="flex:1">Continuer →</button>'),(q=document.getElementById("reveal-next"))==null||q.addEventListener("click",()=>{ne(),a.remove(),n()})}else document.getElementById("reveal-collection").addEventListener("click",()=>{ne(),a.remove(),i("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{ne(),a.remove(),i("boosters")})}function so(e){var t,i,n,a;if(e.card_type==="player"&&e.player)return io(e);if(e.card_type==="game_changer"){const o=e._gcDef,r=(o==null?void 0:o.gc_type)==="ultra_game_changer",s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},d={purple:"#b06ce0",light_blue:"#00d4ef"},l=s[o==null?void 0:o.color]||s.purple,c=d[o==null?void 0:o.color]||d.purple,u=(o==null?void 0:o.name)||e.gc_type||"Game Changer",g=(o==null?void 0:o.effect)||((t=ti[e.gc_type])==null?void 0:t.desc)||"",m=o!=null&&o.image_url?`/icons/${o.image_url}`:null,f=((i=ti[e.gc_type])==null?void 0:i.icon)||"⚡";return`<div style="width:170px;height:240px;background:${l};border-radius:14px;border:3px solid ${c};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${c}88;flex-shrink:0">
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${e.map((o,r)=>{const s=r<e.length-1?bt(o,e[r+1]):null,d=!s||s==="#ff3333"||s==="#cc2222",l=s==="#00ff88"?"+2":s==="#FFD700"?"+1":"";return on(o,a),o.stadiumBonus||a&&(a.club_id&&String(o.club_id)===String(a.club_id)||a.country_code&&(o.country_code,a.country_code)),`
          <div class="duel-card duel-card-${n}" data-idx="${r}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease, transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
            ${Ie({...o,_evolution_bonus:0},{width:window.innerWidth>=900?Math.min(150,Math.max(90,Math.round(window.innerWidth*.22/3))):58,showStad:!0,stadDef:a,role:"MIL",extraNote:o.boost||0})}
          </div>
  ...
  </div>`,requestAnimationFrame(()=>requestAnimationFrame(()=>{const o=e.querySelector(".opponent-preview-wrap"),r=e.querySelector("#opponent-swipe-zone");if(!o||!r)return;const s=r.clientWidth>=900,d=Math.max(200,r.clientHeight-(s?20:40)),l=Math.max(200,r.clientWidth-(s?20:16)),c=s?null:Math.round(Math.max(44,Math.round(l*.168))*.55);o.innerHTML=yt(t.aiTeam,t.formation,null,[],l,d,[],c),o.style.cssText=`width:${l}px;height:${d}px;overflow:visible;flex-shrink:0`;const u=o.querySelector("svg");u&&(u.style.cssText="display:block;width:100%;height:100%",u.setAttribute("preserveAspectRatio",s?"xMidYMid meet":"none"))})),setTimeout(()=>$o(e,t,i),5e3)}const Je=e=>bo(e);function $o(e,t,i){const n=t.homeTeam.MIL||[],a=t.aiTeam.MIL||[],o=t.stadiumDef||null,r=t.aiStadiumDef||null,s=Nt(n,o)+Ot(n),d=Nt(a,r)+Ot(a),l=s>=d;e.innerHTML=`
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const P=t.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
            ...
            ${pt((P.players||[]).map(ae=>({...ae,used:!1})),"#ff6b6b",P.total)}
            ...
            ${pt((P.players||[]).map(ae=>({...ae,used:!1})),"#00ff88",P.total)}
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map($=>{var P,ae,pe;if($.type==="duel"){const le=$.isGoal,se=$.homeScored?"#FFD700":le?"#ff6b6b":"rgba(255,255,255,0.3)",ke=$.homeScored?"⚽ BUT !":le?"⚽ BUT IA !":(P=$.homePlayers)!=null&&P.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${le?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${se};margin-bottom:4px">
