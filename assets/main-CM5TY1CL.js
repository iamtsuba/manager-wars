import{s as $,l as V,F as X,b as ve,c as ue,d as et}from"./formation-links-BD9zicgB.js";function _e(t,{navigate:e,toast:i}){t.innerHTML=`
    ...
    <div class="page-body">
      ...
      <div>
        ...
        <div class="card-panel" style="padding:0">
          ${n.map(l=>{const r=l.winner_id===a.id,d=l.home_score===l.away_score,c=d?"N":r?"V":"D",u=d?"#888":r?"#1A6B3C":"#c0392b",p={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat"}[l.mode]||l.mode,y=new Date(l.created_at).toLocaleDateString("fr",{month:"short",day:"numeric"});return`<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 14px;border-bottom:1px solid var(--gray-200)">
              <div>
  ...
  </div>`}function st(t){const e=t.job||"ATT",i=Q(t,e),o=be[t.country_code]||t.country_code||"";return`
  <div style="
    width:140px;border-radius:12px;padding:6px;
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
  ...
  </div>`}async function dt(t,e){const{state:i,navigate:o,toast:a,openModal:n,closeModal:s}=e;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:l}=await $.from("cards").select(`id, card_type, current_note, gc_type, formation, is_for_sale, sale_price,
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),d=(l||[]).filter(E=>E.card_type==="player"&&E.player),c=(l||[]).filter(E=>E.card_type==="game_changer"),u=(l||[]).filter(E=>E.card_type==="formation"),p=Object.keys(X),f=Object.keys(se),y={};d.forEach(E=>{const A=E.player.id;y[A]=(y[A]||0)+1});const m=new Set(Object.keys(y).map(E=>String(E))),g=new Set(u.map(E=>E.formation)),v=new Set(c.map(E=>E.gc_type));let h="player",k="Tous",b="",w=!1;function x(){return[...d].sort((E,A)=>{const T=te.indexOf(E.player.job),C=te.indexOf(A.player.job);return T!==C?T-C:(E.player.surname_encoded||"").localeCompare(A.player.surname_encoded||"")})}function _(){return[...r||[]].sort((E,A)=>{const T=te.indexOf(E.job),C=te.indexOf(A.job);return T!==C?T-C:(E.surname_encoded||"").localeCompare(A.surname_encoded||"")})}function I(){return x().filter(E=>{const A=E.player,T=k==="Tous"||A.job===k,C=!b||`${A.firstname} ${A.surname_encoded}`.toLowerCase().includes(b);return T&&C})}function L(){return _().filter(E=>{const A=k==="Tous"||E.job===k,T=!b||`${E.firstname} ${E.surname_encoded}`.toLowerCase().includes(b);return A&&T})}t.innerHTML=`
  <div class="page">
    ...
    <div class="page-body">
      <div style="display:flex;flex-wrap:wrap;gap:10px;justify-content:flex-start" id="col-grid"></div>
  ...
  </div>`;function M(){const E=document.getElementById("col-filters");E&&(h==="player"?(E.innerHTML=`
          ...
          ${rt.map(A=>`
            <button class="filter-btn" data-job="${A}"
              style="flex-shrink:0;padding:4px 12px;border-radius:20px;font-size:12px;font-weight:600;cursor:pointer;
                border:1.5px solid ${A===k?"var(--green)":"var(--gray-200)"};
                background:${A===k?"var(--green)":"#fff"};
                color:${A===k?"#fff":"var(--gray-600)"}">
              ${A}
            </button>`).join("")}
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",A=>{b=A.target.value.toLowerCase(),z()}),t.querySelectorAll(".filter-btn").forEach(A=>{A.addEventListener("click",()=>{k=A.dataset.job,M(),z()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{w=!w,M(),z()})):(E.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{w=!w,M(),z()})))}function z(){const E=document.getElementById("col-grid");E&&(h==="player"?B(E):h==="formation"?ee(E):fe(E))}function B(E){if(w){const A=L();if(!A.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}E.innerHTML=A.map(T=>{if(m.has(String(T.id))){const F=d.find(P=>P.player.id===T.id),G=y[T.id]||1,q=G>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${G}</div>`:"",K=d.filter(P=>P.player.id===T.id&&P.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Ae(F,q+K)}return st(T)}).join("")}else{const A=I();if(!A.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const T={},C=[];A.forEach(F=>{const G=F.player.id;T[G]||(T[G]=!0,C.push(F))}),E.innerHTML=C.map(F=>{const G=y[F.player.id]||1,q=G>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${G}</div>`:"",K=d.filter(P=>P.player.id===F.player.id&&P.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Ae(F,q+K)}).join("")}E.querySelectorAll("[data-card-id]").forEach(A=>{A.addEventListener("click",()=>{const T=d.find(C=>C.id===A.dataset.cardId);T&&pt(T,d,y,e)})})}function ee(E){const A=w?p:[...g];if(!A.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}E.innerHTML=A.map(T=>{if(g.has(T)){const F=u.find(O=>O.formation===T),G=u.filter(O=>O.formation===T).length,q=G>1?`<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${G}</div>`:"";return`
        ...
        <div style="
          ...
          <div style="font-weight:900;font-size:18px">${T}</div>
        </div>`}).join(""),E.querySelectorAll("[data-form-id]").forEach(T=>{T.addEventListener("click",()=>{const C=u.find(F=>F.id===T.dataset.formId);C&&ct(C,u,e,n)})})}function fe(E){const A=w?f:[...v];if(!A.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}E.innerHTML=A.map(T=>{const C=v.has(T),F=se[T]||{icon:"⚡"};if(C){const G=c.find(K=>K.gc_type===T),q=c.filter(K=>K.gc_type===T).length,O=q>1?`<div style="position:absolute;top:4px;right:4px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${q}</div>`:"";return`
        ...
        <div style="
          ...
          <div style="font-weight:700;font-size:13px">${T}</div>
        </div>`}).join(""),E.querySelectorAll("[data-gc-id]").forEach(T=>{T.addEventListener("click",()=>lt(T.dataset.gcType,n))})}t.querySelectorAll(".col-tab-btn").forEach(E=>{E.addEventListener("click",()=>{h=E.dataset.tab,k="Tous",b="",w=!1,t.querySelectorAll(".col-tab-btn").forEach(A=>{const T=A.dataset.tab===h;A.style.borderBottomColor=T?"var(--green)":"transparent",A.style.color=T?"var(--green)":"var(--gray-600)"}),M(),z()})}),M(),z()}function lt(t,e){const i=se[t]||{icon:"⚡",desc:"Effet spécial."};e("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const W=1e3;function ct(t,e,i,o){var m,g,v;const{state:a,toast:n,closeModal:s,navigate:l,refreshProfile:r}=i,d=t.formation,c={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function u(){const h=ve[d]||{},k=X[d]||[],b=290,w=360,x=20;function _(L){const M=h[L];return M?{x:M.x*b,y:M.y*w}:null}let I=`<svg width="${b}" height="${w}" viewBox="0 0 ${b} ${w}" xmlns="http://www.w3.org/2000/svg">`;for(const[L,M]of k){const z=_(L),B=_(M);!z||!B||(I+=`<line x1="${z.x}" y1="${z.y}" x2="${B.x}" y2="${B.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const L of Object.keys(h)){const M=_(L);if(!M)continue;const z=L.replace(/\d+/,""),B=c[z]||"#555";I+=`<circle cx="${M.x}" cy="${M.y}" r="${x}" fill="${B}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,I+=`<text x="${M.x}" y="${M.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${z}</text>`}return I+="</svg>",I}const p=e.filter(h=>h.formation===d),f=p.length,y=!t.is_for_sale;o(`Formation ${d}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      ...
      <div style="background:#f9f9f9;border-radius:10px;padding:12px;display:flex;justify-content:space-between;align-items:center">
        <div>
          ...
          <div style="font-size:18px;font-weight:900;color:var(--yellow)">${W.toLocaleString("fr")} crédits</div>
    ...
    <div style="margin-top:12px;border-top:1px solid var(--gray-200);padding-top:12px">
      ...
      <div style="display:flex;gap:8px">
        <input type="number" id="sell-price-form" min="1" placeholder="Prix en crédits" value="${W}"
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(m=document.getElementById("direct-sell-form-btn"))==null||m.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${d} pour ${W.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const h=p.find(b=>!b.is_for_sale)||p[0];if(!h){n("Aucune carte à vendre","error");return}const{error:k}=await $.from("cards").delete().eq("id",h.id);if(k){n(k.message,"error");return}await $.from("users").update({credits:(a.profile.credits||0)+W}).eq("id",a.profile.id),await r(),n(`+${W.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),s(),l("collection")}),(g=document.getElementById("market-sell-form-btn"))==null||g.addEventListener("click",async()=>{const h=parseInt(document.getElementById("sell-price-form").value);if(!h||h<1){n("Prix invalide","error");return}await $.from("cards").update({is_for_sale:!0,sale_price:h}).eq("id",t.id),await $.from("market_listings").insert({seller_id:a.profile.id,card_id:t.id,price:h}),n("Carte mise en vente sur le marché !","success"),s(),l("collection")}),(v=document.getElementById("cancel-sell-form-btn"))==null||v.addEventListener("click",async()=>{await $.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",t.id),await $.from("market_listings").update({status:"cancelled"}).eq("card_id",t.id).eq("status","active"),n("Annonce retirée","success"),s(),l("collection")})}function pt(t,e,i,o){var x,_,I,L,M,z;const{state:a,toast:n,openModal:s,closeModal:l,navigate:r,refreshProfile:d}=o,c=t.player,u=e.filter(B=>B.player.id===c.id),p=u.length,f=at[c.rarity]||1e3,y=c.rarity!=="legende",m=Ke(c),g=Q(c,c.job),v=c.job2?Q(c,c.job2):null,h=de[c.job]||"#1A6B3C",k=c.job2?de[c.job2]:null,b=Oe[c.rarity]||"#ccc",w=be[c.country_code]||c.country_code||"";s(`${c.firstname} ${c.surname_encoded}`,`<div style="display:flex;gap:16px;flex-wrap:wrap;justify-content:center">
      ...
      <div style="width:140px;border-radius:12px;padding:6px;background:${b};flex-shrink:0">
        <div style="background:#f2e8d2;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
          ...
          <div style="position:relative;height:80px;background:#f2e8d2;display:flex;flex-direction:column;align-items:center">
            ...
            ${v!==null?`
            <svg width="32" height="31" viewBox="0 0 32 31" style="position:absolute;top:50px;z-index:2;display:block">
              <polygon points="16,2 19.5,11 30,11 22,17.5 25,27 16,21.5 7,27 10,17.5 2,11 12.5,11" fill="${k}" stroke="white" stroke-width="1.5"/>
          ...
          <div style="background:#f2e8d2;padding:3px 6px;display:flex;align-items:center;justify-content:space-between;min-height:24px">
            ...
            ${(x=c.clubs)!=null&&x.logo_url?`<img src="${c.clubs.logo_url}" style="width:22px;height:16px;object-fit:contain">`:`<div style="background:#1a3a7a;color:#fff;border-radius:2px;padding:1px 3px;font-size:6px;font-weight:800">${(((_=c.clubs)==null?void 0:_.encoded_name)||"").slice(0,6)}</div>`}
          </div>
        </div>
      </div>
    ...
    </div>`:""}`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(I=document.getElementById("close-detail"))==null||I.addEventListener("click",l),(L=document.getElementById("direct-sell-btn"))==null||L.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte ${c.surname_encoded} pour ${f.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const B=u.find(fe=>!fe.is_for_sale)||u[0];if(!B){n("Aucune carte à vendre","error");return}const{error:ee}=await $.from("cards").delete().eq("id",B.id);if(ee){n(ee.message,"error");return}await $.from("users").update({credits:(a.profile.credits||0)+f}).eq("id",a.profile.id),await d(),n(`+${f.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),l(),r("collection")}),(M=document.getElementById("market-sell-btn"))==null||M.addEventListener("click",async()=>{const B=parseInt(document.getElementById("sell-price").value);if(!B||B<1){n("Prix invalide","error");return}await $.from("cards").update({is_for_sale:!0,sale_price:B}).eq("id",t.id),await $.from("market_listings").insert({seller_id:a.profile.id,card_id:t.id,price:B}),n("Carte mise en vente sur le marché !","success"),l(),r("collection")}),(z=document.getElementById("cancel-sell-btn"))==null||z.addEventListener("click",async()=>{await $.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",t.id),await $.from("market_listings").update({status:"cancelled"}).eq("card_id",t.id).eq("status","active"),n("Annonce retirée","success"),l(),r("collection")})}const le={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},U={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function he(t){const e=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!e||!(t!=null&&t.skin)||!(t!=null&&t.hair))return null;const i=t.hair==="chauve"?`${t.skin}-chauve-rase`:`${t.skin}-${t.hair}-${t.hair_length}`;return`${e}/storage/v1/object/public/assets/tetes/${i}.png`}async function Pe(t,e){const{state:i,navigate:o,toast:a}=e;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:n}=await $.from("decks").select("id,name,formation_card_id,is_active").eq("owner_id",i.profile.id).order("created_at",{ascending:!1});t.innerHTML=`
  <div class="page">
    <div class="page-header">
      <h2>Mes decks</h2>
  ...
  </div>`,document.getElementById("new-deck-btn").addEventListener("click",async()=>{const s=prompt("Nom du deck :",`Deck ${((n==null?void 0:n.length)||0)+1}`);if(!s)return;const{data:l,error:r}=await $.from("decks").insert({owner_id:i.profile.id,name:s,is_active:!(n!=null&&n.length)}).select().single();if(r){a(r.message,"error");return}a("Deck créé !","success"),Te(l.id,t,e)}),t.querySelectorAll("[data-edit]").forEach(s=>{s.addEventListener("click",()=>Te(s.dataset.edit,t,e))}),t.querySelectorAll("[data-activate]").forEach(s=>{s.addEventListener("click",async()=>{await $.from("decks").update({is_active:!1}).eq("owner_id",i.profile.id),await $.from("decks").update({is_active:!0}).eq("id",s.dataset.activate),a("Deck activé !","success"),Pe(t,e)})})}async function Te(t,e,i){const{state:o,toast:a}=i;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:n}=await $.from("decks").select("*").eq("id",t).single(),{data:s}=await $.from("cards").select(`id, card_type, formation,
      player:players(id, firstname, surname_encoded, country_code, club_id, job, job2,
        note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length,
        clubs(encoded_name, logo_url))`).eq("owner_id",o.profile.id),l=(s||[]).filter(f=>f.card_type==="player"&&f.player),r=(s||[]).filter(f=>f.card_type==="formation"),d=r.map(f=>f.formation).filter(Boolean),{data:c}=await $.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",t);let u=n.formation||"4-4-2";d.length>0&&!d.includes(u)&&(u=d[0]);const p={deckId:t,name:n.name,formation:u,formationCardId:n.formation_card_id,slots:{},subs:[],playerCards:l,formationCards:r,availableFormations:d};(c||[]).forEach(f=>{f.is_starter?p.slots[f.position]=f.card_id:p.subs.includes(f.card_id)||p.subs.push(f.card_id)}),Y(e,p,i)}function Y(t,e,i){var r;const{navigate:o}=i;le[e.formation];const a=Le(e.formation),n=a.filter(d=>e.slots[d]).length,s=e.availableFormations.length>0?e.availableFormations:Object.keys(le),l=e.subs.map(d=>e.playerCards.find(c=>c.id===d)).filter(Boolean);[...Object.values(e.slots),...e.subs],t.innerHTML=`
  <div class="page">
    ...
    <div style="padding:12px 16px;background:#fff;border-top:1px solid var(--gray-200)">
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap" id="subs-list">
        ${l.map(d=>{const c=d.player;return`<div style="display:flex;align-items:center;gap:6px;background:#f5f5f5;border-radius:8px;padding:4px 8px;font-size:12px">
            <span style="background:${U[c.job]};color:#fff;border-radius:4px;padding:1px 5px;font-size:10px;font-weight:700">${c.job}</span>
  ...
  </div>`,ft(t,e,a,i),document.getElementById("builder-back").addEventListener("click",()=>o("decks")),document.getElementById("formation-select").addEventListener("change",d=>{e.formation=d.target.value;const c=Le(e.formation),u={};c.forEach(p=>{e.slots[p]&&(u[p]=e.slots[p])}),e.slots=u,Y(t,e,i)}),document.getElementById("save-deck").addEventListener("click",()=>yt(e,i)),t.querySelectorAll("[data-remove-sub]").forEach(d=>{d.addEventListener("click",()=>{e.subs=e.subs.filter(c=>c!==d.dataset.removeSub),Y(t,e,i)})}),(r=document.getElementById("add-sub-btn"))==null||r.addEventListener("click",()=>{gt(e,t,i)})}function ft(t,e,i,o){const a=document.getElementById("deck-field");if(!a)return;const n=e.formation,s=ve[n]||{},l=ue?ue(n)||X[n]||[]:X[n]||[],r={};i.forEach(k=>{const b=e.slots[k],w=b?e.playerCards.find(x=>x.id===b):null;r[k]=(w==null?void 0:w.player)||null});const d=i.filter(k=>k.startsWith("MIL"));let c=0,u=0;d.forEach(k=>{const b=r[k];b&&(c+=Number(b.note_m)||0)}),l.forEach(([k,b])=>{if(!k.startsWith("MIL")||!b.startsWith("MIL"))return;V(r[k],r[b])!=="#ff3333"&&u++});const p=320,f=320,y=28,m=4;function g(k){const b=s[k];return b?{x:b.x*p,y:b.y*f}:null}let v="";for(const[k,b]of l){const w=g(k),x=g(b);if(!w||!x)continue;const _=r[k],I=r[b],L=V(_,I);if(L==="#00ff88"||L==="#FFD700")v+=`<line x1="${w.x}" y1="${w.y}" x2="${x.x}" y2="${x.y}"
        stroke="${L}" stroke-width="${m*3}" stroke-linecap="round" opacity="0.25"/>`,v+=`<line x1="${w.x}" y1="${w.y}" x2="${x.x}" y2="${x.y}"
        stroke="${L}" stroke-width="${m}" stroke-linecap="round" opacity="0.95"/>`;else{const z=L==="#ff3333"||L==="#cc2222"?.75:.9;v+=`<line x1="${w.x}" y1="${w.y}" x2="${x.x}" y2="${x.y}"
        stroke="${L}" stroke-width="${m}" stroke-linecap="round" opacity="${z}"/>`}}for(const k of i){const b=g(k);if(!b)continue;const w=r[k],x=k.replace(/\d+/,""),I={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[x]||"#555",L=w?Number(x==="GK"?w.note_g:x==="DEF"?w.note_d:x==="MIL"?w.note_m:w.note_a)||0:null,M=w?(w.surname_encoded||"").slice(0,8).toUpperCase():"";if(w){const z=he(w);z&&(v+=`
          <defs>
            <clipPath id="clip-${k}">
              <circle cx="${b.x}" cy="${b.y}" r="${y}"/>
            </clipPath>
          </defs>
          <image href="${z}" x="${b.x-y}" y="${b.y-y}" width="${y*2}" height="${y*2}"
            clip-path="url(#clip-${k})" preserveAspectRatio="xMidYMid slice" opacity="0.85"/>`),v+=`
        <circle cx="${b.x}" cy="${b.y}" r="${y}" fill="${z?"transparent":I}"
          stroke="${I}" stroke-width="2" />
        <circle cx="${b.x}" cy="${b.y}" r="${y}" fill="${I}" opacity="${z?"0.5":"1"}"
          stroke="rgba(255,255,255,0.6)" stroke-width="2"/>
        ${z?`<image href="${z}" x="${b.x-y}" y="${b.y-y}" width="${y*2}" height="${y*2}"
          clip-path="url(#clip-${k})" preserveAspectRatio="xMidYMid slice" opacity="0.9"/>`:""}
        <text x="${b.x}" y="${b.y-3}" text-anchor="middle" font-size="13" font-weight="900"
          fill="white" font-family="Arial Black,Arial" style="text-shadow:0 1px 2px #000">${L}</text>
        <text x="${b.x}" y="${b.y+11}" text-anchor="middle" font-size="6.5" fill="rgba(255,255,255,0.9)"
          font-family="Arial">${M}</text>
        <rect x="${b.x-y}" y="${b.y-y}" width="${y*2}" height="${y*2}"
          fill="transparent" class="deck-slot-hit" data-pos="${k}" style="cursor:pointer"/>`}else v+=`
        <circle cx="${b.x}" cy="${b.y}" r="${y}" fill="rgba(255,255,255,0.08)"
          stroke="rgba(255,255,255,0.3)" stroke-width="2" stroke-dasharray="4,3"/>
        <text x="${b.x}" y="${b.y+6}" text-anchor="middle" font-size="16" fill="rgba(255,255,255,0.4)"
          font-family="Arial">+</text>
        <rect x="${b.x-y}" y="${b.y-y}" width="${y*2}" height="${y*2}"
          fill="transparent" class="deck-slot-hit" data-pos="${k}" style="cursor:pointer"/>`}const h="";a.innerHTML=`
    <!-- Badge MIL -->
    ${d.length>0?`
    ...
    <svg viewBox="0 0 ${p} ${f}" width="100%" style="display:block;max-width:380px;margin:0 auto">
      ${h}
      ${v}
    </svg>`,a.querySelectorAll(".deck-slot-hit").forEach(k=>{k.addEventListener("click",()=>ut(k.dataset.pos,e,t,o))})}function ut(t,e,i,o){var p,f,y;const{openModal:a,closeModal:n}=o,s=t.replace(/\d+/,""),l=e.slots[t],r=l?e.playerCards.find(m=>m.id===l):null;(p=r==null?void 0:r.player)==null||p.id;const d=new Set;Object.entries(e.slots).forEach(([m,g])=>{var h;if(m===t||!g)return;const v=e.playerCards.find(k=>k.id===g);(h=v==null?void 0:v.player)!=null&&h.id&&d.add(v.player.id)}),e.subs.forEach(m=>{var v;const g=e.playerCards.find(h=>h.id===m);(v=g==null?void 0:g.player)!=null&&v.id&&d.add(g.player.id)});const c=new Set,u=e.playerCards.filter(m=>{const g=m.player;return!(g.job===s||g.job2===s)||d.has(g.id)||c.has(g.id)?!1:(c.add(g.id),!0)});u.sort((m,g)=>{const v=s==="GK"?m.player.note_g:s==="DEF"?m.player.note_d:s==="MIL"?m.player.note_m:m.player.note_a;return(s==="GK"?g.player.note_g:s==="DEF"?g.player.note_d:s==="MIL"?g.player.note_m:g.player.note_a)-v}),a(`Choisir ${s} — ${t}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${e.slots[t]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${u.length>0?u.map(m=>{var b,w;const g=m.player,v=s==="GK"?g.note_g:s==="DEF"?g.note_d:s==="MIL"?g.note_m:g.note_a,h=he(g),k={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[g.rarity];return`<div class="player-option" data-card-id="${m.id}"
          ...
          <div style="width:44px;height:44px;border-radius:8px;overflow:hidden;flex-shrink:0;background:#dde;border:2px solid ${U[s]}">
            ${h?`<img src="${h}" style="width:100%;height:100%;object-fit:cover">`:`<div style="width:100%;height:100%;background:${U[s]};display:flex;align-items:center;justify-content:center;color:#fff;font-size:10px;font-weight:700">${s}</div>`}
          </div>
          <!-- Infos -->
          <div style="flex:1;min-width:0">
            ...
            <div style="display:flex;align-items:center;gap:6px;margin-top:3px">
              ...
              ${(b=g.clubs)!=null&&b.logo_url?`<img src="${g.clubs.logo_url}" style="width:18px;height:18px;object-fit:contain">`:`<span style="font-size:10px;color:var(--gray-600)">${((w=g.clubs)==null?void 0:w.encoded_name)||"—"}</span>`}
              <span style="font-size:10px;color:var(--gray-600)">${g.country_code}</span>
            </div>
          </div>
          <!-- Note -->
          <div style="width:36px;height:36px;border-radius:8px;background:${U[s]};color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:16px;border:2px solid ${k};flex-shrink:0">
            ${v}
          </div>
        </div>`}).join(""):'<div style="text-align:center;color:var(--gray-600);padding:20px">Aucun joueur pour ce poste.<br><small>Ouvre des boosters !</small></div>'}
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(f=document.getElementById("close-selector"))==null||f.addEventListener("click",n),(y=document.getElementById("remove-player"))==null||y.addEventListener("click",()=>{delete e.slots[t],n(),Y(i,e,o)}),document.querySelectorAll(".player-option").forEach(m=>{m.addEventListener("click",()=>{e.slots[t]=m.dataset.cardId,n(),Y(i,e,o)})})}function gt(t,e,i){var r;const{openModal:o,closeModal:a}=i,n=new Set;Object.values(t.slots).filter(Boolean).forEach(d=>{var u;const c=t.playerCards.find(p=>p.id===d);(u=c==null?void 0:c.player)!=null&&u.id&&n.add(c.player.id)}),t.subs.forEach(d=>{var u;const c=t.playerCards.find(p=>p.id===d);(u=c==null?void 0:c.player)!=null&&u.id&&n.add(c.player.id)});const s=new Set,l=t.playerCards.filter(d=>{var c,u,p;return n.has((c=d.player)==null?void 0:c.id)||s.has((u=d.player)==null?void 0:u.id)?!1:(s.add((p=d.player)==null?void 0:p.id),!0)});o("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${l.length>0?l.map(d=>{var f;const c=d.player,u=he(c),p=c.job==="GK"?c.note_g:c.job==="DEF"?c.note_d:c.job==="MIL"?c.note_m:c.note_a;return`<div class="player-option" data-card-id="${d.id}"
          style="display:flex;align-items:center;gap:10px;padding:8px;border:1.5px solid var(--gray-200);border-radius:10px;cursor:pointer">
    ...
    </div>`,'<button class="btn btn-ghost" id="close-sub-selector">Fermer</button>'),(r=document.getElementById("close-sub-selector"))==null||r.addEventListener("click",a),document.querySelectorAll(".player-option").forEach(d=>{d.addEventListener("click",()=>{t.subs.push(d.dataset.cardId),a(),Y(e,t,i)})})}async function yt(t,e){const{state:i,toast:o,navigate:a}=e,n=t.formationCards.find(d=>d.formation===t.formation),s=(n==null?void 0:n.id)||t.formationCardId,{error:l}=await $.from("decks").update({formation:t.formation,formation_card_id:s||null}).eq("id",t.deckId);if(l){o(l.message,"error");return}await $.from("deck_cards").delete().eq("deck_id",t.deckId);const r=[];if(Object.entries(t.slots).forEach(([d,c],u)=>{r.push({deck_id:t.deckId,card_id:c,position:d,is_starter:!0,slot_order:u})}),t.subs.forEach((d,c)=>{r.push({deck_id:t.deckId,card_id:d,position:`SUB${c+1}`,is_starter:!1,slot_order:100+c})}),r.length>0){const{error:d}=await $.from("deck_cards").insert(r);if(d){o(d.message,"error");return}}o("Deck enregistré ✅","success"),a("decks")}function Le(t){const e=le[t]||le["4-4-2"],i=["GK1"];for(let o=1;o<=e.DEF;o++)i.push(`DEF${o}`);for(let o=1;o<=e.MIL;o++)i.push(`MIL${o}`);for(let o=1;o<=e.ATT;o++)i.push(`ATT${o}`);return i}const Me=[{id:"players_std",img:"/manager-wars/icons/booster-players.png",name:"Players",sub:"5 cartes joueurs",cost:5e3,costLabel:"5 000 crédits",cardCount:5,type:"player"},{id:"players_pub",img:"/manager-wars/icons/booster-silver.png",name:"Players (pub)",sub:"3 cartes joueurs",cost:0,costLabel:"1 pub",cardCount:3,type:"player"},{id:"game_changer",img:"/manager-wars/icons/booster-gamechanger.png",name:"Game Changer",sub:"3 cartes spéciales",cost:1e4,costLabel:"10 000 crédits",cardCount:3,type:"game_changer"},{id:"formation",img:"/manager-wars/icons/booster-formation.png",name:"Formation",sub:"1 carte formation",cost:1e4,costLabel:"10 000 crédits",cardCount:1,type:"formation"}],He={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé."},"Double attaque":{icon:"⚡",desc:"La prochaine attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la prochaine action IA."},Gel:{icon:"❄️",desc:"Bloque le meilleur attaquant IA."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function xt(t){const e="https://fcnwclxlkytdfjotqkta.supabase.co";if(!(t!=null&&t.skin)||!(t!=null&&t.hair))return null;const i=t.hair==="chauve"?`${t.skin}-chauve-rase`:`${t.skin}-${t.hair}-${t.hair_length}`;return`${e}/storage/v1/object/public/assets/tetes/${i}.png`}const mt={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},vt={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"};async function bt(t,{state:e,navigate:i,toast:o}){var n;const a=((n=e.profile)==null?void 0:n.credits)||0;t.innerHTML=`
  <div class="page">
    <div class="page-header">
      <h2>📦 Boosters</h2>
    ...
    <div class="page-body">
      <div class="booster-grid">
        ${Me.map(s=>{const l=a>=s.cost||s.cost===0,r=s.id==="players_std"||s.id==="players_pub";return`<div class="booster-card ${l?"":"disabled"}" data-booster="${s.id}" style="position:relative">
  ...
  `,document.body.appendChild(o);let a=!1;document.getElementById("pack-visual").addEventListener("click",()=>{if(a)return;a=!0;const p=document.getElementById("pack-visual");p.classList.add("shaking"),setTimeout(()=>{p.classList.add("pack-open"),setTimeout(()=>{document.getElementById("pack-phase").style.display="none",s(0)},600)},500)});let n=0;function s(p){n=p;const f=document.getElementById("reveal-phase");f.style.display="flex",l(p)}function l(p){var w;const f=t[p],y=document.getElementById("card-counter"),m=document.getElementById("single-card-slot"),g=document.getElementById("card-tap-hint");y&&(y.textContent=`Carte ${p+1} / ${t.length}`),g&&(g.textContent=p<t.length-1?"Appuie pour continuer →":"Appuie pour voir toutes tes cartes");const v=ze(f),h=f.card_type==="player"&&((w=f.player)==null?void 0:w.rarity)==="legende";m.innerHTML=`
      ...
      </div>`,h&&c();const k=document.getElementById("current-card-wrap");let b=!1;k.addEventListener("click",()=>{if(b)return;b=!0;const x=p+1;x<t.length?(k.style.transition="all 0.25s ease",k.style.transform="translateX(-120%) rotate(-15deg)",k.style.opacity="0",setTimeout(()=>l(x),250)):r()})}function r(){u(),document.getElementById("reveal-phase").style.display="none";const p=document.getElementById("recap-phase");p.style.display="flex";const f=document.getElementById("recap-grid");f.innerHTML=t.map((y,m)=>`
  ...
  `,document.body.appendChild(t),t.addEventListener("click",e=>{e.target===t&&t.remove()}),document.getElementById("odds-close").addEventListener("click",()=>t.remove())}function Tt(){return new Promise(t=>{const e=document.createElement("div");e.style.cssText="position:fixed;inset:0;background:#000;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:16px";let i=5;e.innerHTML=`<div style="font-size:48px">📺</div>
      ...
      <div style="font-size:36px;font-weight:900" id="ad-cd">5</div>`,document.body.appendChild(e);const o=setInterval(()=>{i--;const a=document.getElementById("ad-cd");a&&(a.textContent=i),i<=0&&(clearInterval(o),e.remove(),t(!0))},1e3)})}const J={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},N={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function Ue(t,e,i,o,a){var n;t.innerHTML=`<div class="match-screen" style="display:flex;align-items:center;justify-content:center;min-height:100vh">
    <div style="text-align:center;padding:40px;color:#fff">
      <div style="font-size:48px;margin-bottom:16px">${e}</div>
      <p style="margin-bottom:16px">${i}</p>
      <button class="btn btn-primary" id="msg-btn">${o}</button>
    </div>
  </div>`,(n=document.getElementById("msg-btn"))==null||n.addEventListener("click",a)}function S(t){const e=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!e||!(t!=null&&t.skin)||!(t!=null&&t.hair))return null;const i=t.hair==="chauve"?`${t.skin}-chauve-rase`:`${t.skin}-${t.hair}-${t.hair_length}`;return`${e}/storage/v1/object/public/assets/tetes/${i}.png`}function ge(t){const e=t.player;return{cardId:t.id,id:e.id,firstname:e.firstname,name:e.surname_encoded,country_code:e.country_code,club_id:e.club_id,job:e.job,job2:e.job2,note_g:Number(e.note_g)||0,note_d:Number(e.note_d)||0,note_m:Number(e.note_m)||0,note_a:Number(e.note_a)||0,rarity:e.rarity,skin:e.skin,hair:e.hair,hair_length:e.hair_length,boost:0,used:!1,_line:null,_col:null}}function we(t){return t===1?[1]:t===2?[0,2]:t===3?[0,1,2]:t===4?[0,1,1,2]:t===5?[0,1,1,1,2]:[1]}function Lt(){const t=Math.random()*100;return t<.1?4:t<5?3:t<20?2:1}function Ve(t,e){const i=J[e]||J["4-4-2"],o={GK:[],DEF:[],MIL:[],ATT:[]},a=[...t];for(const n of["GK","DEF","MIL","ATT"]){const s=[];for(let r=0;r<i[n];r++){let d=a.findIndex(c=>c.job===n);if(d===-1&&(d=a.findIndex(c=>c.job2===n)),d===-1&&(d=0),a[d]){const c={...a[d],_line:n};s.push(c),a.splice(d,1)}}const l=we(s.length);s.forEach((r,d)=>{r._col=l[d]}),o[n]=s}return o}async function Mt(t,e){const{data:i}=await $.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length").eq("is_active",!0).limit(60);if(!i||i.length<11)return zt(t);const o=J[t]||J["4-4-2"],a={GK:[],DEF:[],MIL:[],ATT:[]},n=[...i];for(const s of["GK","DEF","MIL","ATT"]){const l=n.filter(p=>p.job===s),r=n.filter(p=>p.job!==s),d=[...l,...r],c=[];for(let p=0;p<o[s];p++){const f=d[p]||n[p];f&&c.push({cardId:"ai-"+f.id+"-"+p,id:f.id,firstname:f.firstname,name:f.surname_encoded,country_code:f.country_code,club_id:f.club_id,job:f.job,job2:f.job2,note_g:Number(f.note_g)||0,note_d:Number(f.note_d)||0,note_m:Number(f.note_m)||0,note_a:Number(f.note_a)||0,rarity:f.rarity,skin:f.skin,hair:f.hair,hair_length:f.hair_length,boost:0,used:!1,_line:s})}const u=we(c.length);c.forEach((p,f)=>{p._col=u[f]}),a[s]=c}return a}function zt(t){const e=J[t]||J["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},o=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let a=0;for(const n of["GK","DEF","MIL","ATT"]){const s=[];for(let r=0;r<e[n];r++){const d=3+Math.floor(Math.random()*5);s.push({cardId:"fake-"+a,id:"fake-"+a,firstname:"IA",name:o[a%o.length],country_code:"XX",club_id:null,job:n,job2:null,note_g:n==="GK"?d:2,note_d:n==="DEF"?d:2,note_m:n==="MIL"?d:2,note_a:n==="ATT"?d:2,rarity:"normal",boost:0,used:!1,_line:n}),a++}const l=we(s.length);s.forEach((r,d)=>{r._col=l[d]}),i[n]=s}return i}async function jt(t,e){var w;const{state:i,navigate:o,toast:a}=e,n=i.params||{};t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const s=n.matchMode||"vs_ai_easy",l=s.replace("vs_ai_",""),r=s;if(!n.deckId)return Ct(t,e,s);const d=n.deckId,[{data:c},{data:u}]=await Promise.all([$.from("decks").select("formation,name").eq("id",d).single(),$.from("deck_cards").select(`position, is_starter, slot_order,
        card:cards(id, card_type, formation,
          player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
            note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
            clubs(encoded_name,logo_url)))`).eq("deck_id",d).order("slot_order")]),p=(u||[]).filter(x=>{var _;return x.is_starter&&((_=x.card)==null?void 0:_.player)}).map(x=>ge(x.card)),f=(u||[]).filter(x=>{var _;return!x.is_starter&&((_=x.card)==null?void 0:_.player)}).map(x=>ge(x.card));if(p.length<11){Ue(t,"⚠️",`Deck incomplet (${p.length}/11).`,"Compléter",()=>o("decks"));return}const y=(u||[]).find(x=>{var _;return((_=x.card)==null?void 0:_.card_type)==="formation"}),m=(c==null?void 0:c.formation)||((w=y==null?void 0:y.card)==null?void 0:w.formation)||"4-4-2",{data:g}=await $.from("cards").select("id,gc_type").eq("owner_id",i.profile.id).eq("card_type","game_changer"),v=Ve(p,m),h=await Mt(m),{data:k}=await $.from("matches").insert({home_id:i.profile.id,away_id:null,mode:r,home_deck_id:d,status:"in_progress"}).select().single(),b={matchId:k==null?void 0:k.id,mode:r,difficulty:l,formation:m,homeTeam:v,aiTeam:h,homeSubs:f,subsUsed:0,maxSubs:Math.min(f.length,3),homeScore:0,aiScore:0,gcCards:g||[],usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Bt(t,b,e)}async function Ct(t,e,i){const{state:o,navigate:a}=e;t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:n}=await $.from("decks").select("id,name,is_active,formation").eq("owner_id",o.profile.id).order("created_at",{ascending:!1});if(!n||n.length===0){Ue(t,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>a("decks"));return}const s=n.map(c=>c.id),{data:l}=await $.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length))`).in("deck_id",s).order("slot_order");let r=0;function d(){var h,k,b,w,x;const c=n[r],u=(l||[]).filter(_=>_.deck_id===c.id),p=u.filter(_=>{var I;return _.is_starter&&((I=_.card)==null?void 0:I.player)}).map(_=>ge(_.card)),f=u.find(_=>{var I;return((I=_.card)==null?void 0:I.card_type)==="formation"}),y=c.formation||((h=f==null?void 0:f.card)==null?void 0:h.formation)||"4-4-2",m=p.length>=11?Ve(p,y):null,g=p.length>=11;t.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:calc(100vh - 130px);overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div id="deck-swipe-zone" style="flex:1;min-height:0;overflow:hidden;position:relative;touch-action:pan-y;display:flex;align-items:center;justify-content:center">
        ${m?`<div style="width:min(88vw, calc(100vh - 430px));overflow:hidden;flex-shrink:0">${Je(m,y,null,[],240,240)}</div>`:`<div style="display:flex;align-items:center;justify-content:center;height:100%;opacity:.4;flex-direction:column;gap:8px">
      ...
      <div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">
        ${n.map((_,I)=>`<div style="width:7px;height:7px;border-radius:50%;background:${I===r?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}
      </div>`:""}
    ...
    </div>`,(k=document.getElementById("prev-deck"))==null||k.addEventListener("click",()=>{r>0&&(r--,d())}),(b=document.getElementById("next-deck"))==null||b.addEventListener("click",()=>{r<n.length-1&&(r++,d())}),(w=document.getElementById("validate-deck"))==null||w.addEventListener("click",()=>{g&&e.navigate("match",{matchMode:i,deckId:c.id})}),(x=document.getElementById("cancel-deck-select"))==null||x.addEventListener("click",()=>a("home"));const v=document.getElementById("deck-swipe-zone");if(v){let _=0,I=0;v.addEventListener("touchstart",L=>{_=L.touches[0].clientX,I=L.touches[0].clientY},{passive:!0}),v.addEventListener("touchend",L=>{const M=L.changedTouches[0].clientX-_,z=L.changedTouches[0].clientY-I;Math.abs(M)<40||Math.abs(M)<Math.abs(z)||(M<0&&r<n.length-1?(r++,d()):M>0&&r>0&&(r--,d()))},{passive:!0})}}d()}function Bt(t,e,i){const o=e.homeTeam.MIL||[],a=e.aiTeam.MIL||[];function n(u){return u.reduce((p,f)=>p+D(f,"MIL"),0)}function s(u){let p=0;for(let f=0;f<u.length-1;f++){const y=V(u[f],u[f+1]);y!=="#ff3333"&&y!=="#cc2222"&&p++}return p}const l=n(o)+s(o),r=n(a)+s(a),d=l>=r;function c(u,p,f){return`<div style="text-align:center">
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${u.map((y,m)=>{const g=S(y),v=m<u.length-1?V(y,u[m+1]):null,h=v&&v!=="#ff3333"&&v!=="#cc2222";return`
          ...
          ${v?`<div style="width:14px;height:4px;border-radius:2px;background:${v};flex-shrink:0;opacity:${h?.9:.3}"></div>`:""}
          `}).join("")}
      </div>
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:100vh;gap:24px;padding:24px;background:#0a3d1e">
    ...
    <div id="midfield-result" style="opacity:0;text-align:center;transition:opacity 0.5s;color:#fff;max-width:320px"></div>
  </div>`,setTimeout(()=>{const u=document.getElementById("score-home"),p=document.getElementById("score-ai"),f=document.getElementById("midfield-result");if(u&&p&&(d?(u.style.fontSize="80px",u.style.color="#FFD700",p.style.opacity="0.25"):(p.style.fontSize="80px",p.style.color="#ff6b6b",u.style.opacity="0.25")),f){const y=Lt();f.style.opacity="1",d&&e.clubName,f.innerHTML=`
      ...
      `,d&&(e.boostCard={value:y})}e.attacker=d?"home":"ai",e.log.push({type:"duel",title:"Milieu de Terrain",homePlayers:o.map(y=>({name:y.name,note:D(y,"MIL"),portrait:S(y),job:y.job})),aiPlayers:a.map(y=>({name:y.name,note:D(y,"MIL"),portrait:S(y),job:y.job})),homeTotal:l,aiTotal:r,text:`Duel milieu : ${e.clubName} ${l} – ${r} IA → ${d?e.clubName+" attaque":"IA attaque"}`}),setTimeout(()=>{e.phase=e.attacker==="home"?"attack":"ai-attack",R(t,e,i),e.attacker==="ai"&&setTimeout(()=>ye(t,e,i),1e3)},2800)},1200)}function Ye(t,e,i,o,a=310,n=310){const s=ve[e]||{},l=ue(e)||X[e]||[],r=26,d={},c=["ATT","MIL","DEF","GK"];for(const y of c)(t[y]||[]).forEach((g,v)=>{d[`${y}${v+1}`]=g});function u(y){const m=s[y];return m?{x:m.x*a,y:m.y*n}:null}let p="";for(const[y,m]of l){const g=u(y),v=u(m);if(!g||!v)continue;const h=d[y],k=d[m],b=V(h,k);b==="#00ff88"||b==="#FFD700"?(p+=`<line x1="${g.x.toFixed(1)}" y1="${g.y.toFixed(1)}" x2="${v.x.toFixed(1)}" y2="${v.y.toFixed(1)}"
        stroke="${b}" stroke-width="10" stroke-linecap="round" opacity="0.22"/>`,p+=`<line x1="${g.x.toFixed(1)}" y1="${g.y.toFixed(1)}" x2="${v.x.toFixed(1)}" y2="${v.y.toFixed(1)}"
        stroke="${b}" stroke-width="3.5" stroke-linecap="round" opacity="0.95"/>`):p+=`<line x1="${g.x.toFixed(1)}" y1="${g.y.toFixed(1)}" x2="${v.x.toFixed(1)}" y2="${v.y.toFixed(1)}"
        stroke="${b}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}for(const[y,m]of Object.entries(d)){const g=u(y);if(!g)continue;const v=y.replace(/[0-9]/g,""),h=N[v]||"#555",k=i==="attack"&&["MIL","ATT"].includes(v)&&!m.used||i==="defense"&&["GK","DEF","MIL"].includes(v)&&!m.used,b=o.includes(m.cardId);let w;i==="attack"?w=v==="MIL"?Number(m.note_m)||0:Number(m.note_a)||0:i==="defense"?w=v==="GK"?Number(m.note_g)||0:v==="MIL"?Number(m.note_m)||0:Number(m.note_d)||0:w=Number(v==="GK"?m.note_g:v==="DEF"?m.note_d:v==="MIL"?m.note_m:m.note_a)||0,w=w+(m.boost||0);const x=b?"#FFD700":k?"rgba(255,255,255,0.7)":"rgba(255,255,255,0.25)",_=b?3:2,I=m.used?.2:1,L=S(m);L&&(p+=`<defs><clipPath id="mc-${y}"><circle cx="${g.x}" cy="${g.y}" r="${r}"/></clipPath></defs>`),p+=`<circle cx="${g.x}" cy="${g.y}" r="${r}" fill="${h}" opacity="${I}"
      stroke="${x}" stroke-width="${_}"/>`,L&&!m.used&&(p+=`<image href="${L}" x="${g.x-r}" y="${g.y-r}" width="${r*2}" height="${r*2}"
        clip-path="url(#mc-${y})" preserveAspectRatio="xMidYMid slice" opacity="0.8"/>`,p+=`<circle cx="${g.x}" cy="${g.y}" r="${r}" fill="${h}" opacity="0.3"
        stroke="${x}" stroke-width="${_}"/>`),m.boost&&(p+=`<rect x="${g.x+r-10}" y="${g.y-r}" width="14" height="10" rx="3" fill="#87CEEB"/>
    ...
    <text x="${g.x}" y="${g.y+11}" text-anchor="middle" font-size="6" fill="rgba(255,255,255,${m.used?.2:.8})"
      font-family="Arial">${(m.name||"").slice(0,8)}</text>`,k&&(p+=`<circle cx="${g.x}" cy="${g.y}" r="${r}" fill="rgba(255,255,255,0.08)"
        class="match-slot-hit ${b?"selected":""}" data-card-id="${m.cardId}" data-role="${v}"
        style="cursor:pointer"/>`)}const f=r+4;return`<svg viewBox="${-f} ${-f} ${a+f*2} ${n+f*2}" width="100%" style="display:block;width:100%;max-width:380px;margin:0 auto">
    ${p}
  ...
  <div style="text-align:center;flex-shrink:0;width:38px">
    ...
    <div style="display:flex;align-items:center;gap:4px;${e?"flex-direction:row-reverse":""};background:rgba(255,255,255,0.04);border-radius:8px;padding:5px 8px;border:1px solid rgba(255,255,255,0.07)">
      ...
      ${ie(t.inPlayer||{})}
    </div>`}return`<div style="font-size:11px;color:${t.type==="goal"?"#FFD700":"rgba(255,255,255,0.65)"};font-weight:${t.type==="goal"?700:400};padding:3px 2px">${t.text||""}</div>`}function R(t,e,i){var y,m,g,v,h,k,b,w;const o=e.selected.map(x=>x.cardId),a=e.usedSubIds||[],n=e.homeSubs.filter(x=>!a.includes(x.cardId)),l=Object.values(e.homeTeam).flat().filter(x=>x.used).length>0&&n.length>0&&e.subsUsed<e.maxSubs;e.log[e.log.length-1];const r=e.phase==="ai-attack"||e.phase==="ai-defense",d=e.phase==="attack",c=e.phase==="defense",u=e.phase==="finished",p=e.gcCards.filter(x=>!e.usedGc.includes(x.id)),f=e.boostCard&&!e.boostUsed;t.innerHTML=`
  <style>
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;position:relative">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden">
      ${(()=>{if(e.phase==="defense"&&e.pendingAttack){const _=e.pendingAttack;return`<div style="padding:6px 8px;background:rgba(180,30,30,0.25);border-left:3px solid #ff6b6b">
            ...
            <div style="display:flex;align-items:center;gap:6px">
              <div style="display:flex;gap:5px;flex:1">
                ${(_.players||[]).map(I=>{const L=I._line==="MIL"?I.note_m:I.note_a,M=S(I);return'<div style="text-align:center"><div style="width:36px;height:36px;border-radius:6px;background:'+(N[I.job]||"#555")+';position:relative;overflow:hidden;border:1.5px solid rgba(255,255,255,0.3)">'+(M?'<img src="'+M+'" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover">':"")+'<div style="position:absolute;bottom:0;left:0;right:0;background:rgba(0,0,0,0.7);font-size:9px;color:#fff;font-weight:900;text-align:center">'+L+'</div></div><div style="font-size:6px;color:rgba(255,255,255,0.5)">'+(I.name||"").slice(0,6)+"</div></div>"}).join("")}
              </div>
              <div style="background:#ff6b6b;color:#fff;border-radius:8px;padding:4px 10px;font-size:20px;font-weight:900;flex-shrink:0">${_.total}</div>
            </div>
          </div>`}if(e.phase==="ai-defense"&&e.pendingAttack){const _=e.pendingAttack;return`<div style="padding:6px 8px;background:rgba(26,107,60,0.25);border-left:3px solid #00ff88">
            <div style="font-size:9px;color:#00ff88;letter-spacing:2px;margin-bottom:5px;text-transform:uppercase">⚔️ VOUS ATTAQUEZ</div>
            <div style="display:flex;align-items:center;gap:6px">
              <div style="display:flex;gap:5px;flex:1">
                ${(_.players||[]).map(I=>{const L=I._line==="MIL"?I.note_m:I.note_a,M=S(I);return'<div style="text-align:center"><div style="width:36px;height:36px;border-radius:6px;background:'+(N[I.job]||"#555")+';position:relative;overflow:hidden;border:1.5px solid rgba(255,255,255,0.3)">'+(M?'<img src="'+M+'" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover">':"")+'<div style="position:absolute;bottom:0;left:0;right:0;background:rgba(0,0,0,0.7);font-size:9px;color:#fff;font-weight:900;text-align:center">'+L+'</div></div><div style="font-size:6px;color:rgba(255,255,255,0.5)">'+(I.name||"").slice(0,6)+"</div></div>"}).join("")}
              </div>
              <div style="background:#00ff88;color:#000;border-radius:8px;padding:4px 10px;font-size:20px;font-weight:900;flex-shrink:0">${_.total}</div>
            </div>
          </div>`}const x=e.log[e.log.length-1];return x?'<div style="padding:2px 4px">'+St(x)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})()}
    </div>
    ...
    <div style="display:flex;flex:1;min-height:0;overflow:hidden">
      ...
      <div style="display:flex;flex-direction:column;gap:5px;padding:6px 3px;width:42px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
        ${n.length===0?'<div style="font-size:8px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':n.map(x=>{const _=S(x),I=N[x.job]||"#555",L=x.job==="GK"?x.note_g:x.job==="DEF"?x.note_d:x.job==="MIL"?x.note_m:x.note_a;return`
              <div class="sub-btn-col" data-sub-id="${x.cardId}" title="${x.firstname} ${x.name}"
                style="width:36px;height:36px;border-radius:50%;background:${I};border:2px solid rgba(255,255,255,0.4);cursor:pointer;position:relative;overflow:visible;flex-shrink:0">
                <div style="position:absolute;inset:0;border-radius:50%;overflow:hidden">
                  ${_?`<img src="${_}" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;opacity:0.85">`:""}
                </div>
      ...
      <div style="flex:1;display:grid;grid-template-columns:repeat(4,1fr);gap:3px;align-content:start">
        ${p.map(x=>{var _;return`
          <div class="gc-mini" data-gc-id="${x.id}" data-gc-type="${x.gc_type}"
            ...
            <div style="font-size:16px">${((_=se[x.gc_type])==null?void 0:_.icon)||"⚡"}</div>
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${e.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...e.log].reverse().map((x,_)=>`
  ...
  </div>`,(y=document.getElementById("match-quit"))==null||y.addEventListener("click",()=>{confirm("Abandonner ? Résultat : défaite 3-0")&&(e.homeScore=0,e.aiScore=3,Z(t,e,i))}),(m=document.getElementById("view-ai"))==null||m.addEventListener("click",()=>Kt(e,i)),(g=document.getElementById("toggle-history"))==null||g.addEventListener("click",()=>{var x;(x=document.getElementById("match-history-panel"))==null||x.classList.add("open")}),(v=document.getElementById("close-history"))==null||v.addEventListener("click",()=>{var x;(x=document.getElementById("match-history-panel"))==null||x.classList.remove("open")}),(h=document.getElementById("btn-action"))==null||h.addEventListener("click",()=>{e.selected.length!==0&&(d?Dt(t,e,i):c&&Gt(t,e,i))}),(k=document.getElementById("btn-results"))==null||k.addEventListener("click",()=>Z(t,e,i)),t.querySelectorAll(".match-slot-hit").forEach(x=>{x.addEventListener("click",()=>Ft(x,e,t,i))}),t.querySelectorAll(".gc-mini").forEach(x=>{x.addEventListener("click",()=>qt(x.dataset.gcId,x.dataset.gcType,t,e,i))}),(b=document.getElementById("boost-card"))==null||b.addEventListener("click",()=>Ot(t,e,i)),t.querySelectorAll(".sub-btn-col").forEach(x=>{x.addEventListener("click",()=>Ce(t,e,i,x.dataset.subId))}),(w=document.getElementById("sub-btn-main"))==null||w.addEventListener("click",()=>Ce(t,e,i))}function Ft(t,e,i,o){const a=t.dataset.cardId,n=t.dataset.role,s=e.selected.findIndex(l=>l.cardId===a);if(s!==-1)e.selected.splice(s,1);else{if(e.selected.length>=3){o.toast("Maximum 3 joueurs","error");return}const l=(e.homeTeam[n]||[]).find(r=>r.cardId===a);l&&e.selected.push({...l,_role:n,_line:n})}R(i,e,o)}function Dt(t,e,i){const o=e.selected.map(n=>({...n,_line:n._role})),a=Ge(o,e.modifiers.home);e.pendingAttack={...a,players:[...e.selected],side:"home"},e.selected.forEach(n=>{const s=(e.homeTeam[n._role]||[]).find(l=>l.cardId===n.cardId);s&&(s.used=!0)}),e.log.push({text:`⚔️ Vous attaquez : ${a.total} (base ${a.base}${a.links?` +${a.links} liens`:""}) — ${e.selected.map(n=>n.name).join(", ")}`,type:"info"}),e.selected=[],e.modifiers.home={},e.phase="ai-defense",R(t,e,i),setTimeout(()=>Rt(t,e,i),1200)}function Gt(t,e,i){const o=e.selected.map(l=>({...l,_line:l._role})),a=Re(o,e.modifiers.home);e.selected.forEach(l=>{const r=(e.homeTeam[l._role]||[]).find(d=>d.cardId===l.cardId);r&&(r.used=!0)});const n=Ne(e.pendingAttack.total,a.total,e.modifiers.home),s={type:"duel",title:"Défense",aiPlayers:(e.pendingAttack.players||[]).map(l=>({name:l.name,note:l._line==="MIL"?l.note_m:l.note_a,portrait:S(l),job:l.job})),homePlayers:e.selected.map(l=>{const r=(e.homeTeam[l._role]||[]).find(d=>d.cardId===l.cardId)||l;return{name:r.name,note:r._line==="GK"?r.note_g:r._line==="MIL"?r.note_m:r.note_d,portrait:S(r),job:r.job}}),homeTotal:a.total,aiTotal:e.pendingAttack.total,isGoal:!1,homeScored:!1,text:""};if(n.shielded)s.text="🛡️ Bouclier ! But annulé.",e.log.push(s);else if(n.goal){e.aiScore++,s.isGoal=!0,s.homeScored=!1,s.text=`⚽ BUT IA ! (${e.pendingAttack.total} > ${a.total})`,e.log.push(s),e.selected=[],e.modifiers.home={},e.pendingAttack=null,R(t,e,i),Qe(s.aiPlayers,e.homeScore,e.aiScore,!1,()=>{ce(t,e,i,"home-attack")});return}else s.text=`🧤 Défense réussie ! (${a.total} ≥ ${e.pendingAttack.total})`,e.log.push(s);e.selected=[],e.modifiers.home={},e.pendingAttack=null,ce(t,e,i,"home-attack")}function ye(t,e,i){const o=[...e.aiTeam.MIL||[],...e.aiTeam.ATT||[]],a=qe(o,"attack",e.difficulty);if(!a.length){Xe(t,e,i);return}const n=Ge(a,e.modifiers.ai);e.pendingAttack={...n,players:a,side:"ai"},a.forEach(s=>{s.used=!0}),e.log.push({text:`🤖 IA attaque : ${n.total} (${a.map(s=>s.name).join(", ")})`,type:"info"}),e.modifiers.ai={},e.phase="defense",R(t,e,i)}function Rt(t,e,i){const o=[...e.aiTeam.GK||[],...e.aiTeam.DEF||[],...e.aiTeam.MIL||[]],a=qe(o,"defense",e.difficulty),n=a.length>0?Re(a,e.modifiers.ai).total:0;a.forEach(r=>{r.used=!0});const s=Ne(e.pendingAttack.total,n,e.modifiers.ai),l={type:"duel",title:"Attaque",homePlayers:(e.pendingAttack.players||[]).map(r=>({name:r.name,note:r._line==="MIL"?r.note_m:r.note_a,portrait:S(r),job:r.job})),aiPlayers:a.map(r=>({name:r.name,note:r._line==="GK"?r.note_g:r._line==="MIL"?r.note_m:r.note_d,portrait:S(r),job:r.job})),homeTotal:e.pendingAttack.total,aiTotal:n,isGoal:!1,homeScored:!1,text:""};if(s.shielded)l.text="🛡️ Bouclier IA !",e.log.push(l);else if(s.goal){e.homeScore++,l.isGoal=!0,l.homeScored=!0,l.text=`⚽ BUT ! (${e.pendingAttack.total} > ${n})`,e.log.push(l),e.modifiers.ai={},e.pendingAttack=null,R(t,e,i),Qe(l.homePlayers,e.homeScore,e.aiScore,!0,()=>{ce(t,e,i,"ai-attack")});return}else l.text=`🧤 IA défend (${n} ≥ ${e.pendingAttack.total})`,e.log.push(l);e.modifiers.ai={},e.pendingAttack=null,ce(t,e,i,"ai-attack")}function ce(t,e,i,o){if(e.round++,We(e)){Z(t,e,i);return}if(o==="home-attack"){if(![...e.homeTeam.MIL||[],...e.homeTeam.ATT||[]].filter(n=>!n.used).length){if(![...e.homeTeam.GK||[],...e.homeTeam.DEF||[],...e.homeTeam.MIL||[]].filter(s=>!s.used).length){Z(t,e,i);return}e.phase="ai-attack",R(t,e,i),setTimeout(()=>ye(t,e,i),800);return}e.phase="attack",R(t,e,i)}else{if(![...e.aiTeam.MIL||[],...e.aiTeam.ATT||[]].filter(n=>!n.used).length){Xe(t,e,i);return}e.phase="ai-attack",R(t,e,i),setTimeout(()=>ye(t,e,i),800)}}function We(t){const e=["MIL","ATT","GK","DEF"].some(o=>(t.homeTeam[o]||[]).some(a=>!a.used)),i=["MIL","ATT","GK","DEF"].some(o=>(t.aiTeam[o]||[]).some(a=>!a.used));return!e&&!i}function Xe(t,e,i){We(e)?Z(t,e,i):(e.phase="attack",R(t,e,i))}function Nt(t,e,i){const o=document.createElement("div");o.style.cssText=`
  ...
  <div style="width:72px;overflow:hidden;border-radius:8px;border:2px solid ${o};background:rgba(0,0,0,0.5);flex-shrink:0">
  ...
  </div>`}function Ce(t,e,i,o=null){if(e.phase!=="attack"){oe("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(e.usedSubIds||(e.usedSubIds=[]),e.subsUsed>=e.maxSubs){oe(`Maximum ${e.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const a=Object.values(e.homeTeam).flat().filter(u=>u.used),n=e.homeSubs.filter(u=>!e.usedSubIds.includes(u.cardId));if(!a.length){oe("Aucun joueur utilisé à remplacer");return}if(!n.length){oe("Aucun remplaçant disponible");return}let s=null,l=null,r=o||null;const d=document.createElement("div");d.id="sub-overlay",d.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:700;display:flex;flex-direction:column;overflow:hidden";function c(){var u,p;d.innerHTML=`
    ...
    <div style="flex:1;overflow-y:auto;padding:14px">
      ...
      <div style="display:flex;flex-wrap:wrap;gap:10px;margin-bottom:20px">
        ${a.map(f=>`
        <div class="grayed-pick" data-id="${f.cardId}" data-line="${f._line}"
          style="cursor:pointer;border-radius:10px;border:2px solid ${s===f.cardId?"#ff6b6b":"rgba(255,255,255,0.1)"};overflow:hidden;transition:border-color 0.15s">
          ${je(f)}
        </div>`).join("")}
      </div>
      ...
      <div style="display:flex;flex-wrap:wrap;gap:10px">
        ${n.map(f=>`
        <div class="sub-pick" data-id="${f.cardId}"
          style="cursor:pointer;border-radius:10px;border:2px solid ${r===f.cardId?"#00ff88":"rgba(255,255,255,0.1)"};overflow:hidden;transition:border-color 0.15s">
          ${je(f)}
        </div>`).join("")}
      </div>
    </div>
    <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
      <button id="sub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;
        background:${s&&r?"#1A6B3C":"rgba(255,255,255,0.08)"};
        color:${s&&r?"#fff":"rgba(255,255,255,0.25)"};
        font-size:15px;font-weight:900;cursor:${s&&r?"pointer":"default"}">
        ${s&&r?"✅ Confirmer le remplacement":"Sélectionnez un joueur sortant et entrant"}
      </button>
    </div>`,(u=document.getElementById("sub-close"))==null||u.addEventListener("click",()=>d.remove()),d.querySelectorAll(".grayed-pick").forEach(f=>{f.addEventListener("click",()=>{s=f.dataset.id,l=f.dataset.line,c()})}),d.querySelectorAll(".sub-pick").forEach(f=>{f.addEventListener("click",()=>{r=f.dataset.id,c()})}),(p=document.getElementById("sub-confirm"))==null||p.addEventListener("click",()=>{if(!s||!r)return;const f=l,y=e.homeTeam[f]||[],m=y.findIndex(h=>h.cardId===s),g=e.homeSubs.find(h=>h.cardId===r),v=m!==-1?{...y[m]}:null;if(m!==-1&&g){const h={...g,_line:f,_col:y[m]._col,used:!1,boost:0};e.homeTeam[f].splice(m,1,h),e.usedSubIds.push(r),e.subsUsed++,e.log.push({type:"sub",subSide:"home",outPlayer:{name:v.name,firstname:v.firstname,note:D(v,f),portrait:S(v),job:v.job},inPlayer:{name:g.name,firstname:g.firstname,note:D(g,f),portrait:S(g),job:g.job},text:`🔄 ${g.firstname} ${g.name} remplace ${v.firstname} ${v.name}`}),d.remove(),Nt(v,g,()=>R(t,e,i))}})}c(),document.body.appendChild(d)}function qt(t,e,i,o,a){if(!o.usedGc.includes(t)){switch(o.usedGc.push(t),e){case"Double attaque":o.modifiers.home.doubleAttack=!0,o.log.push({text:"⚡ Double attaque activée !",type:"info"});break;case"Bouclier":o.modifiers.home.shield=!0,o.log.push({text:"🛡️ Bouclier activé !",type:"info"});break;case"Ressusciter":{let n=!1;for(const s of["ATT","MIL","DEF","GK"]){const l=(o.homeTeam[s]||[]).find(r=>r.used);if(l){l.used=!1,n=!0;break}}o.log.push({text:n?"💫 Joueur ressuscité !":"💫 Aucun joueur à ressusciter",type:"info"});break}case"Vol de note":o.modifiers.ai.stolenNote=(o.modifiers.ai.stolenNote||0)+1,o.log.push({text:"🎯 -1 à la prochaine attaque IA",type:"info"});break;case"Gel":{const n=[...o.aiTeam.ATT||[],...o.aiTeam.MIL||[]].filter(s=>!s.used);if(n.length){const s=n.sort((l,r)=>D(r,"ATT")-D(l,"ATT"))[0];s.used=!0,o.log.push({text:`❄️ ${s.name} (IA) gelé !`,type:"info"})}break}case"Remplacement+":o.maxSubs++,o.log.push({text:"🔄 +1 remplacement débloqué",type:"info"});break}$.from("cards").delete().eq("id",t).then(()=>{}),R(i,o,a)}}function Ot(t,e,i){const o=Object.values(e.homeTeam).flat().filter(a=>!a.used);if(!o.length){i.toast("Aucun joueur actif à booster","error");return}i.openModal("⚡ Utiliser le Boost",`<div style="margin-bottom:12px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border-radius:10px;padding:12px;text-align:center;color:#000">
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(n);const l=()=>{n.remove(),a()};n.addEventListener("click",l),setTimeout(l,3500)}async function Z(t,e,i){var c,u;e.phase="finished";const{state:o}=i,a=e.homeScore>e.aiScore,n=e.homeScore===e.aiScore,s=a?"victoire":n?"nul":"defaite",l=nt(e.mode,s);e.matchId&&await $.from("matches").update({status:"finished",home_score:e.homeScore,away_score:e.aiScore,winner_id:a?o.profile.id:null,home_credits_reward:l,played_at:new Date().toISOString()}).eq("id",e.matchId);const r={credits:(o.profile.credits||0)+l,matches_played:(o.profile.matches_played||0)+1};a?r.wins=(o.profile.wins||0)+1:n?r.draws=(o.profile.draws||0)+1:r.losses=(o.profile.losses||0)+1,await $.from("users").update(r).eq("id",o.profile.id),await i.refreshProfile();const d=document.createElement("div");d.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",d.innerHTML=`
    ...
    </div>`,`<button class="btn btn-primary" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Pt={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"};async function Ht(t,e){const{state:i,toast:o}=e;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement du marché...</div>',await ke(t,e)}async function ke(t,e){const{state:i,toast:o}=e,{data:a}=await $.from("market_listings").select(`id, price, status, listed_at, seller_id,
  ...
  `;function l(r){const d=document.getElementById("mkt-content"),c=r==="buy"?s:n;if(c.length===0){d.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      ${c.map(u=>{var g,v,h;const p=(g=u.card)==null?void 0:g.player;if(!p)return"";const f=p.job==="GK"?p.note_g:p.job==="DEF"?p.note_d:p.job==="MIL"?p.note_m:p.note_a,y=Pt[p.rarity],m=(i.profile.credits||0)>=u.price;return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px">
          <div style="width:44px;height:44px;border-radius:8px;background:${Yt(p.job)};color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:18px;border:2px solid ${y};flex-shrink:0">${f}</div>
          <div style="flex:1;min-width:0">
          ...
          <div style="text-align:right;flex-shrink:0">
            ...
            ${r==="buy"?`<button class="btn btn-primary btn-sm" data-buy="${u.id}" ${m?"":"disabled"} style="margin-top:4px">${m?"Acheter":"Trop cher"}</button>`:`<button class="btn btn-danger btn-sm" data-cancel="${u.id}" style="margin-top:4px">Retirer</button>`}
          </div>
        </div>`}).join("")}
    </div>`,d.querySelectorAll("[data-buy]").forEach(u=>{u.addEventListener("click",()=>Ut(u.dataset.buy,c,t,e))}),d.querySelectorAll("[data-cancel]").forEach(u=>{u.addEventListener("click",()=>Vt(u.dataset.cancel,t,e))})}l("buy"),t.querySelectorAll(".mkt-tab").forEach(r=>{r.addEventListener("click",()=>{t.querySelectorAll(".mkt-tab").forEach(d=>{const c=d===r;d.style.background=c?"var(--green)":"#fff",d.style.color=c?"#fff":"var(--gray-600)",d.style.borderColor=c?"var(--green)":"var(--gray-200)"}),l(r.dataset.tab)})})}async function Ut(t,e,i,o){const{state:a,toast:n,refreshProfile:s}=o,l=e.find(r=>r.id===t);if(l){if((a.profile.credits||0)<l.price){n("Crédits insuffisants","error");return}if(confirm(`Acheter ${l.card.player.firstname} ${l.card.player.surname_encoded} pour ${l.price.toLocaleString("fr")} crédits ?`))try{const{error:r}=await $.from("cards").update({owner_id:a.profile.id,is_for_sale:!1,sale_price:null}).eq("id",l.card.id);if(r)throw r;await $.from("market_listings").update({status:"sold",buyer_id:a.profile.id,sold_at:new Date().toISOString()}).eq("id",t),await $.from("users").update({credits:(a.profile.credits||0)-l.price}).eq("id",a.profile.id);const{data:d}=await $.from("users").select("credits").eq("id",l.seller_id).single();d&&await $.from("users").update({credits:(d.credits||0)+l.price}).eq("id",l.seller_id),await $.from("notifications").insert({user_id:l.seller_id,type:"card_sold",message:`Ta carte ${l.card.player.surname_encoded} a été vendue pour ${l.price} crédits !`,data:{card_id:l.card.id,price:l.price}}),await s(),n("Carte achetée ! ✅","success"),ke(i,o)}catch(r){n("Erreur : "+r.message,"error")}}}async function Vt(t,e,i){const{toast:o}=i,{data:a}=await $.from("market_listings").select("card_id").eq("id",t).single();await $.from("market_listings").update({status:"cancelled"}).eq("id",t),a&&await $.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",a.card_id),o("Annonce retirée","success"),ke(e,i)}function Yt(t){return{GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[t]||"#888"}async function Jt(t,{state:e,navigate:i,toast:o}){t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:a}=await $.from("users").select("id,pseudo,club_name,trophies_top1,trophies_top2,trophies_top3,wins,level").order("trophies_top1",{ascending:!1}).limit(100);t.innerHTML=`
  <div class="page">
    <div class="page-header">
  ...
  `}const j={user:null,profile:null,page:"home",params:{}};function ne(t,e="info",i=3e3){const o=document.getElementById("toast");o&&(o.textContent=t,o.className=`show ${e}`,clearTimeout(o._t),o._t=setTimeout(()=>{o.className=""},i))}function Wt(t,e,i=""){document.getElementById("modal-title").textContent=t,document.getElementById("modal-body").innerHTML=e,document.getElementById("modal-footer").innerHTML=i,document.getElementById("modal-overlay").classList.remove("hidden")}function xe(){document.getElementById("modal-overlay").classList.add("hidden")}async function re(){if(!j.user)return;const{data:t}=await $.from("users").select("*").eq("id",j.user.id).single();t&&(j.profile=t)}const $e="mw_theme";function pe(){return localStorage.getItem($e)||"light"}function Xt(t){var e;localStorage.setItem($e,t),me(t),(e=j.profile)!=null&&e.id&&$.from("users").update({theme:t}).eq("id",j.profile.id).then(()=>{})}function me(t){document.documentElement.setAttribute("data-theme",t)}function ae(t,e={}){j.page=t,j.params=e,Ze()}async function Ze(){const t=document.getElementById("page-content");if(!t)return;document.querySelectorAll(".bottom-nav a").forEach(o=>{o.classList.toggle("active",o.dataset.page===j.page)});const e=document.getElementById("nav-credits");e&&j.profile&&(e.textContent=`💰 ${(j.profile.credits||0).toLocaleString("fr")}`);const i={state:j,navigate:ae,toast:ne,openModal:Wt,closeModal:xe,refreshProfile:re};switch(t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽</div>',j.page){case"home":await Ie(t,i);break;case"collection":await dt(t,i);break;case"decks":await Pe(t,i);break;case"boosters":await bt(t,i);break;case"match":await jt(t,i);break;case"market":await Ht(t,i);break;case"rankings":await Jt(t,i);break;default:await Ie(t,i)}}function Qt(){const t=document.getElementById("app"),e=j.profile;if(!e)return;const i="/manager-wars/icons/";t.innerHTML=`
    <nav class="top-nav">
  ...
  `,document.querySelectorAll(".bottom-nav a").forEach(o=>{o.addEventListener("click",a=>{a.preventDefault(),ae(o.dataset.page)})}),document.getElementById("nav-logo").addEventListener("click",()=>ae("home")),document.getElementById("nav-credits").addEventListener("click",()=>ae("boosters")),document.getElementById("theme-toggle").addEventListener("click",()=>{const a=pe()==="dark"?"light":"dark";Xt(a),Be(a)}),Be(pe())}function Be(t){const e=document.getElementById("theme-icon");e&&(e.textContent=t==="dark"?"☀️":"🌙")}async function Zt(){me(pe()),document.getElementById("modal-overlay").addEventListener("click",e=>{e.target===e.currentTarget&&xe()}),document.getElementById("modal-close").addEventListener("click",xe);const{data:{session:t}}=await $.auth.getSession();if(!t){Fe(),_e(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:ne});return}j.user=t.user,await re(),Fe();try{const{data:e}=await $.from("formation_links_overrides").select("formation, links"),i={};(e||[]).forEach(o=>{i[o.formation]=o.links}),et(i)}catch(e){console.warn("Impossible de charger les overrides de formation:",e)}if(!j.profile){tt(document.getElementById("app"),{state:j,navigate:async()=>{await re(),Se()},toast:ne,refreshProfile:re});return}j.profile.theme&&j.profile.theme!==pe()&&(localStorage.setItem($e,j.profile.theme),me(j.profile.theme)),Se(),$.auth.onAuthStateChange(async(e,i)=>{e==="SIGNED_OUT"&&(j.user=null,j.profile=null,document.getElementById("app").innerHTML="",_e(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:ne}))})}function Se(){const t=document.getElementById("app");t.style.display="flex",t.style.flexDirection="column",Qt(),Ze()}function Fe(){const t=document.getElementById("app-loader"),e=document.getElementById("app");e&&(e.style.display=""),t&&(t.classList.add("zoom-out"),setTimeout(()=>t.style.display="none",500))}Zt();
