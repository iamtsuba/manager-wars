...
import{s as v,j as vi,q as zn,t as Ht,u as Oi,F as Hi,o as tt,n as ei,g as Mt,r as Se,m as Ct,v as Mi,w as go,T as Ti,p as xo,x as bo,d as yo}from"./special-cards-lEGt-tGs.js";const ho="/";function pn(t,{navigate:e,toast:i}){let n="login";const r=()=>{var a,s,d,l,c,u,b;const o=n==="login";if(t.innerHTML=`
    <div style="
  ...
  `;function s(){var m;const l=document.getElementById("logo-preview"),c=document.getElementById("logo-initials"),u=((m=document.getElementById("setup-club"))==null?void 0:m.value)||"MW",b=u.trim().split(" ").filter(Boolean),x=b.length>=2?(b[0][0]+b[1][0]).toUpperCase():u.slice(0,2).toUpperCase();l&&(l.style.background=a,l.style.borderColor=o),c&&(c.textContent=x,c.style.color=o)}document.getElementById("color1").addEventListener("input",l=>{o=l.target.value,document.getElementById("swatch1").style.background=o,s()}),document.getElementById("color2").addEventListener("input",l=>{a=l.target.value,document.getElementById("swatch2").style.background=a,s()});function d(l){document.querySelectorAll(".setup-step").forEach(c=>c.classList.remove("active")),document.getElementById(`step-${l}`).classList.add("active"),document.getElementById("step-num").textContent=l,document.getElementById("progress-fill").style.width=`${Math.round(l/3*100)}%`,l===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const l=document.getElementById("setup-pseudo").value.trim(),c=document.getElementById("step1-error");if(c.textContent="",l.length<3){c.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:u}=await v.from("users").select("id").eq("pseudo",l).maybeSingle();if(u){c.textContent="Ce pseudo est déjà pris.";return}d(2)}),document.getElementById("step2-back").addEventListener("click",()=>d(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const l=document.getElementById("setup-club").value.trim(),c=document.getElementById("step2-error");if(c.textContent="",l.length<2){c.textContent="Nom trop court (min. 2 caractères).";return}const{data:u}=await v.from("users").select("id").eq("club_name",l).maybeSingle();if(u){c.textContent="Ce nom de club est déjà pris.";return}d(3)}),document.getElementById("step3-back").addEventListener("click",()=>d(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const l=document.getElementById("setup-pseudo").value.trim(),c=document.getElementById("setup-club").value.trim(),u=document.getElementById("step3-error"),b=document.getElementById("step3-finish");u.textContent="",b.disabled=!0,b.textContent="Création en cours…";try{const{error:x}=await v.from("users").insert({id:e.user.id,pseudo:l,club_name:c,club_color1:o,club_color2:a,credits:1e4});if(x)throw x;await wo(e.user.id),await r(),n(`Bienvenue ${l} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(x){u.textContent=x.message,b.disabled=!1,b.textContent="🚀 Créer mon profil !"}})}async function wo(t){const e=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await v.from("users").update({pending_boosters:e,onboarding_done:!1,first_booster_opened:!1}).eq("id",t)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const _o="modulepreload",ko=function(t){return"/"+t},un={},Ui=function(e,i,n){let r=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),s=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));r=Promise.allSettled(i.map(d=>{if(d=ko(d),d in un)return;un[d]=!0;const l=d.endsWith(".css"),c=l?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${c}`))return;const u=document.createElement("link");if(u.rel=l?"stylesheet":_o,l||(u.as="script"),u.crossOrigin="",u.href=d,s&&u.setAttribute("nonce",s),document.head.appendChild(u),l)return new Promise((b,x)=>{u.addEventListener("load",b),u.addEventListener("error",()=>x(new Error(`Unable to preload CSS for ${d}`)))})}))}function o(a){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=a,window.dispatchEvent(s),!s.defaultPrevented)throw a}return r.then(a=>{for(const s of a||[])s.status==="rejected"&&o(s.reason);return e().catch(o)})},ti="#1A6B3C",ii="#cc2222",$o="#D4A017",fn="#888";async function Eo(t,e){const{state:i,toast:n}=e;t.innerHTML=`
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      ${b.map(({friendshipId:x,friend:m})=>Lo(m,x)).join("")}
    </div>`,l.querySelectorAll("[data-stats]").forEach(x=>{x.addEventListener("click",()=>Io(t,x.dataset.stats,x.dataset.friendName))}),l.querySelectorAll("[data-match]").forEach(x=>{x.addEventListener("click",async()=>{const m=x.dataset.friendId,p=x.dataset.friendName;if(typeof n!="function"){e("Erreur navigation","error");return}const f=t.user.id,{data:y}=await v.from("friend_match_invites").select("id").eq("inviter_id",m).eq("invitee_id",f).eq("status","pending").order("created_at",{ascending:!1}).limit(1).maybeSingle(),w=!!y;console.log("[Friends] clic match",{fid:m,fname:p,isAccepting:w}),n("match",{matchMode:"friend",friendId:m,friendName:p,isAccepting:w})})})}function Lo(t,e){const i=t.club_name||t.pseudo||"?",n=t.pseudo||"",r=(n||i).slice(0,2).toUpperCase(),o=t.club_color2||ti,a=t.club_color1||"#ffffff",s=t.last_seen_at?new Date(t.last_seen_at):null,d=s&&Date.now()-s.getTime()<3*60*1e3;return`
    ...
    ${Yi()}`,document.body.appendChild(l);const u=()=>l.remove();l.querySelector("#pending-close").addEventListener("click",u),l.addEventListener("click",b=>{b.target===l&&u()}),l.querySelectorAll("[data-accept]").forEach(b=>{b.addEventListener("click",async()=>{const{error:x}=await v.from("friendships").update({status:"accepted"}).eq("id",b.dataset.accept);if(x){e("Erreur : "+x.message,"error");return}b.closest("div[style]").remove(),e("✅ Ami accepté !","success"),An(t,e,n),i&&i()})}),l.querySelectorAll("[data-decline]").forEach(b=>{b.addEventListener("click",async()=>{await v.from("friendships").delete().eq("id",b.dataset.decline),b.closest("div[style]").remove(),e("Demande refusée","info"),i&&i()})})}async function Io(t,e,i){const n=t.user.id,[r,o]=[n,e].sort(),a=n===r,{data:s}=await v.from("friend_match_stats").select("*").eq("player1_id",r).eq("player2_id",o).single(),d=t.profile.club_name||t.profile.pseudo||"Moi",l=s||{},c=a?l.wins_p1||0:l.wins_p2||0,u=a?l.wins_p2||0:l.wins_p1||0,b=l.draws||0,x=a?l.goals_p1||0:l.goals_p2||0,m=a?l.goals_p2||0:l.goals_p1||0,p=a?l.gc_used_p1||0:l.gc_used_p2||0,f=a?l.gc_used_p2||0:l.gc_used_p1||0,y=l.matches_total||0,w=(L,g,A,E=ti,S=ii)=>`
    <div style="display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:8px;padding:10px 0;border-bottom:1px solid #f0f0f0">
      <div style="text-align:right;font-size:18px;font-weight:900;color:${E}">${g}</div>
  ...
  <div class="home-dark" id="home-dark">
    ...
    `,document.head.appendChild(l)}const r=qo.map(l=>`
    ...
    </div>`,document.body.appendChild(e),e.querySelector("#journal-close-v2").addEventListener("click",()=>e.remove()),e.addEventListener("click",n=>{n.target===e&&e.remove()})}async function Ho(t,{state:e,navigate:i,toast:n}){var g,A,E,S,q,W,ee;const r=e.profile;if(!r)return;const o="/icons/";Dn(i,r,"home2",o);const a=r.mmr??1e3,s=vi(a),d=Po(a,s),l=zn(a),c=isFinite(s.max)?s.max+1:null,[{list:u,iAmInTop:b,myPosition:x},m,p]=await Promise.all([No(r.id),Go(5),Ro()]),f=(G,I)=>`
  ...
  <div class="home-dark" id="home-dark">
      ...
      <div class="home2-dash">
        ...
        <div class="home2-col-right">
          <div class="news-widget">
            ...
            ${m.length?m.map(y).join(""):'<div style="font-size:11px;color:var(--tile-fg-dim);padding:8px 0">Aucune actualité pour le moment.</div>'}
          </div>
        </div>
  ...
  </div>`,document.body.appendChild(o),o.querySelector("#claim-later-btn").addEventListener("click",()=>o.remove()),o.querySelector("#claim-all-btn").addEventListener("click",async a=>{const s=a.currentTarget;s.disabled=!0,s.textContent="...";let d=0;for(const l of n){const{data:c}=await v.rpc("claim_mini_league_prize",{p_league_id:l.league_id,p_user_id:i});c!=null&&c.success&&!c.already_claimed&&(d+=c.prize||0)}if(d>0){t.profile.credits=(t.profile.credits||0)+d;const l=document.getElementById("nav-credits");l&&(l.textContent=`💰 ${t.profile.credits.toLocaleString("fr")}`),e(`💰 +${d.toLocaleString("fr")} cr. récupérés !`,"success")}o.remove()})}async function Fn(t,e,i){const n=document.getElementById("ongoing-match-banner");if(!n)return;const r=t.profile.id,{data:o}=await v.from("matches").select("id, home_id, away_id, status, mode").eq("status","active").or(`home_id.eq.${r},away_id.eq.${r}`).order("created_at",{ascending:!1});if(!(o!=null&&o.length)){n.innerHTML="";return}const a=o.map(d=>d.home_id===r?d.away_id:d.home_id).filter(Boolean);let s={};if(a.length){const{data:d}=await v.from("users").select("id, pseudo, club_name").in("id",a);(d||[]).forEach(l=>{s[l.id]=l.club_name||l.pseudo})}n.innerHTML=o.map(d=>{const l=d.home_id===r?d.away_id:d.home_id,c=s[l]||"Adversaire",u=d.mode==="mini_league";return`<div style="display:flex;align-items:center;gap:10px;background:linear-gradient(135deg,rgba(10,61,30,0.8),rgba(26,107,60,0.6));color:#fff;border-radius:12px;padding:12px 16px;border:1px solid rgba(26,107,60,0.4);box-shadow:0 3px 12px rgba(26,107,60,0.3)">
      <div style="background:rgba(255,255,255,0.15);border-radius:50%;width:36px;height:36px;display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0">⚽</div>
      <div style="flex:1;min-width:0">
        <div style="font-size:13px;font-weight:900">${u?"🏆 Mini League":d.mode==="friend"?"Match ami":"Match"} en cours</div>
        <div style="font-size:11px;opacity:0.8;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">vs ${c}</div>
  ...
  </div>`,document.body.appendChild(e),e.querySelector("#ab-cancel").addEventListener("click",()=>e.remove()),e.querySelector("#ab-ok").addEventListener("click",()=>{e.remove(),t()}),e.addEventListener("click",i=>{i.target===e&&e.remove()})}async function Yo(t,e,i){var s,d,l,c;const n=document.getElementById("match-invite-banner");if(!n)return;const{data:r}=await v.from("friend_match_invites").select("id, inviter_id, inviter:users!inviter_id(pseudo, club_name)").eq("invitee_id",t.user.id).eq("status","pending").order("created_at",{ascending:!1}).limit(1).maybeSingle();if(!r){n.innerHTML="";return}const o=((s=r.inviter)==null?void 0:s.club_name)||((d=r.inviter)==null?void 0:d.pseudo)||"?",a=r.inviter_id;n.innerHTML=`<div id="match-invite-btn" style="display:flex;align-items:center;gap:10px;background:linear-gradient(135deg,rgba(26,10,46,0.8),rgba(74,26,138,0.6));color:#fff;border-radius:12px;padding:12px 16px;border:1px solid rgba(122,40,184,0.4);cursor:pointer;box-shadow:0 3px 12px rgba(74,10,138,0.3)">
    <div style="background:rgba(255,255,255,0.15);border-radius:50%;width:36px;height:36px;display:flex;align-items:center;justify-content:center;font-size:20px;flex-shrink:0">⚽</div>
    <div style="flex:1"><div style="font-size:13px;font-weight:900">${o} t'invite à jouer !</div><div style="font-size:11px;opacity:0.75">Accepter la partie ?</div></div>
    <div style="display:flex;gap:8px">
      <button id="match-inv-accept" style="padding:6px 12px;border-radius:8px;border:none;background:#22c55e;color:#fff;font-size:12px;font-weight:900;cursor:pointer">Jouer !</button>
    ...
    <div class="modal-body" style="padding:16px">
  ...
  </div>`,document.body.appendChild(i);const n=()=>i.remove();document.getElementById("solo-cancel").addEventListener("click",n),i.addEventListener("click",l=>{l.target===i&&n()});const[{data:r},{data:o}]=await Promise.all([v.from("solo_levels").select("*").eq("is_active",!0).order("level_number"),v.from("user_solo_progress").select("unlocked_level").eq("user_id",e.profile.id).maybeSingle()]),a=(o==null?void 0:o.unlocked_level)||1,s=r||[],d=document.getElementById("solo-levels-list");if(d){if(!s.length){d.innerHTML='<div style="text-align:center;color:#999;padding:20px">Aucun niveau configuré.</div>';return}d.innerHTML=s.map(l=>{const c=l.level_number>a,u=l.level_number===a,b=c?"#f0f0f0":u?"#eefaf2":"#f7f7f7",x=c?"#ddd":u?"#bfe8cf":"#e0e0e0",m=c?"#999":"#12401f";return`
      <div class="solo-level-card" data-level="${l.level_number}" data-locked="${c}"
        ...
        <div style="flex:1;min-width:0">
          <div style="font-weight:800;font-size:15px;color:${m}">Niveau ${l.level_number}</div>
        ...
        ${c?"":`<div style="font-weight:900;font-size:12.5px;color:${m};background:rgba(255,255,255,0.7);padding:6px 11px;border-radius:999px;flex-shrink:0;white-space:nowrap">+${l.reward_credits} cr.</div>`}
      </div>`}).join(""),d.querySelectorAll(".solo-level-card").forEach(l=>{l.dataset.locked!=="true"&&(l.addEventListener("mouseenter",()=>{l.style.transform="translateY(-1px)"}),l.addEventListener("mouseleave",()=>{l.style.transform=""}),l.addEventListener("click",()=>{n(),t("match",{matchMode:"solo",soloLevel:Number(l.dataset.level)})}))})}}async function Wo(t,{state:e,navigate:i,toast:n}){const r=e.profile;if(!r)return;const o="/icons/";Dn(i,r,"game",o),t.innerHTML=`
  <style>
    .game-page {
      min-height: 100%;
      background: var(--page-bg);
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    ${Se(t,{width:140})}
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),l=(s||[]).filter($=>$.card_type==="player"&&$.player),c=(s||[]).filter($=>$.card_type==="game_changer"),u=(s||[]).filter($=>$.card_type==="formation"),b=(s||[]).filter($=>$.card_type==="stadium"),{data:x}=await v.from("gc_definitions").select("name,gc_type,color,effect,image_url"),m={};(x||[]).forEach($=>{m[$.name]=$});const{data:p}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),f={};(p||[]).forEach($=>{f[$.id]=$}),b.forEach($=>{$.stadium_def&&(f[$.stadium_id]=$.stadium_def)});const y=Object.keys(Hi),w=Object.keys(ze),h={};l.forEach($=>{const C=$.player.id;h[C]=(h[C]||0)+1}),new Set(Object.keys(h).map($=>String($)));const L=new Set(u.map($=>$.formation)),g=new Set(c.map($=>$.gc_type));let A="player",E="Tous",S="",q=!1;function W(){return[...l].sort(($,C)=>{const O=si.indexOf($.player.job),P=si.indexOf(C.player.job);return O!==P?O-P:($.player.surname_real||"").localeCompare(C.player.surname_real||"")})}function ee(){return[...d||[]].sort(($,C)=>{const O=si.indexOf($.job),P=si.indexOf(C.job);return O!==P?O-P:($.surname_real||"").localeCompare(C.surname_real||"")})}function G(){return W().filter($=>{const C=$.player,O=E==="Tous"||C.job===E,P=!S||`${C.firstname} ${C.surname_real}`.toLowerCase().includes(S);return O&&P})}function I(){return ee().filter($=>{const C=E==="Tous"||$.job===E,O=!S||`${$.firstname} ${$.surname_real}`.toLowerCase().includes(S);return C&&O})}t.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(m=document.getElementById("direct-sell-form-btn"))==null||m.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${l} pour ${li.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const y=b.find(h=>!h.is_for_sale)||b[0];if(!y){o("Aucune carte à vendre","error");return}await v.from("market_listings").delete().eq("card_id",y.id),await v.from("transfer_history").delete().eq("card_id",y.id);const{error:w}=await v.from("cards").delete().eq("id",y.id);if(w){o(w.message,"error");return}await v.from("users").update({credits:(r.profile.credits||0)+li}).eq("id",r.profile.id),await d(),o(`+${li.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),a(),s("collection")}),(p=document.getElementById("market-sell-form-btn"))==null||p.addEventListener("click",async()=>{const y=parseInt(document.getElementById("sell-price-form").value);if(!y||y<1){o("Prix invalide","error");return}await v.from("cards").update({is_for_sale:!0,sale_price:y}).eq("id",t.id),await v.from("market_listings").insert({seller_id:r.profile.id,card_id:t.id,price:y}),o("Carte mise en vente sur le marché !","success"),a(),s("collection")}),(f=document.getElementById("cancel-sell-form-btn"))==null||f.addEventListener("click",async()=>{await v.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",t.id),await v.from("market_listings").update({status:"cancelled"}).eq("card_id",t.id).eq("status","active"),o("Annonce retirée","success"),a(),s("collection")})}async function hn(t,e,i,n){var de,j,U,le,fe,ue;const{state:r,toast:o,openModal:a,closeModal:s,navigate:d,refreshProfile:l}=n,c=t.player,u=e.filter($=>$.player.id===c.id),b=u.length,x=t.evolution_bonus||0,m=Math.max((Number(c.note_g)||0)+(c.job==="GK"||c.job2==="GK"?x:0),(Number(c.note_d)||0)+(c.job==="DEF"||c.job2==="DEF"?x:0),(Number(c.note_m)||0)+(c.job==="MIL"||c.job2==="MIL"?x:0),(Number(c.note_a)||0)+(c.job==="ATT"||c.job2==="ATT"?x:0)),p=c.rarity||"normal",{data:f}=await v.from("sell_price_configs").select("*").eq("rarity",p).lte("note_min",m).gte("note_max",m).order("note_min",{ascending:!1}).limit(1);((de=f==null?void 0:f[0])==null?void 0:de.price)??Zo[p];const y=c.rarity!=="legende";Mt(c);const w=di(c,c.job)+x,h=c.job2?di(c,c.job2)+(di(c,c.job2)>0?x:0):null;Ii[c.job],c.job2&&Ii[c.job2];const L=Jo[c.rarity]||"#ccc";tr[c.country_code]||c.country_code;const g=t.evolution_bonus||0,E=w+g,S=h||0,q=S>0?S+g:0,W=u.map($=>$.id);let ee={};if(W.length){const{data:$}=await v.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",W).order("transferred_at",{ascending:!0});($||[]).forEach(C=>{ee[C.card_id]||(ee[C.card_id]=[]),ee[C.card_id].push(C)})}const G=W.length?`
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",c.note_g],["DEF",c.note_d],["MIL",c.note_m],["ATT",c.note_a]].map(([$,C])=>{const O=Ii[$],P=$==="GK"?"#fff":O,X=$===c.job||$===c.job2,D=(Number(C)||0)+(X&&g>0?g:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(j=document.getElementById("close-detail"))==null||j.addEventListener("click",s);let I=new Set;const _=c.rarity==="pepite"?1.3:c.rarity==="papyte"?.7:1;function H(){let $=0;return document.querySelectorAll(".expl-check:checked").forEach(C=>{if(C.dataset.id===t.id)return;const P=Number(C.dataset.evo)||0,X=Number(C.dataset.note)||0;$+=X+P}),Math.round($*_)}const Y=()=>{const $=I.size,C=document.getElementById("sell-action-panel");if(!C)return;C.style.display=$>0?"block":"none",document.getElementById("sell-selected-count").textContent=$;const O=document.getElementById("evolve-btn");if(O){const P=H();O.textContent=P>0?`⬆️ Évoluer (+${P})`:"⬆️ Évoluer"}};document.querySelectorAll(".expl-check").forEach($=>{$.addEventListener("change",()=>{const C=$.dataset.id;$.checked?I.add(C):I.delete(C);const O=$.closest(".exemplaire-row");if(O){const P=O.querySelector(".expl-sel-overlay"),X=O.querySelector(".expl-sel-check");P&&(P.style.display=$.checked?"block":"none"),X&&(X.style.display=$.checked?"flex":"none"),O.style.transform=$.checked?"scale(1.05)":"scale(1)"}Y()})}),document.querySelectorAll(".exemplaire-row").forEach($=>{$.addEventListener("click",C=>{if(C.target.tagName==="INPUT")return;const O=$.querySelector(".expl-check");O&&!O.disabled&&(O.checked=!O.checked,O.dispatchEvent(new Event("change")))})}),(U=document.getElementById("evolve-btn"))==null||U.addEventListener("click",async()=>{if(b<=1)return;const $=[...I];if(!$.length)return;if(I.has(t.id)){const ne=document.createElement("div");ne.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",ne.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    </div>`;f("⚽ Choisir une formation",w),document.querySelectorAll("#modal-body [data-forma]").forEach(h=>{h.addEventListener("click",()=>{e.formation=h.dataset.forma;const L=_n(e.formation),g={};L.forEach(A=>{e.slots[A]&&(g[A]=e.slots[A])}),e.slots=g,y(),wt(t,e,i)})})};t.querySelectorAll("#formation-mobile-btn, #formation-pc-btn").forEach(f=>f.addEventListener("click",x)),t.querySelectorAll("#add-stad-btn-pc, #add-stad-btn").forEach(f=>f.addEventListener("click",()=>sr(e,t,i))),t.querySelectorAll("#save-deck").forEach(f=>f.addEventListener("click",()=>cr(e,i))),t.querySelectorAll("[data-remove-sub]").forEach(f=>{f.addEventListener("click",()=>{e.subs=e.subs.filter(y=>y!==f.dataset.removeSub),wt(t,e,i)})}),t.querySelectorAll("#add-sub-btn").forEach(f=>f.addEventListener("click",()=>lr(e,t,i)))}function ar(t,e,i,n){var h,L,g;const r=window.innerWidth>=900,o=t.querySelector(r?"#deck-field-pc":"#deck-field-mobile");if(!o)return;const a=(h=e.stadiumCards)==null?void 0:h.find(A=>A.id===e.stadiumCardId),s=a&&((L=e.stadDefMap)==null?void 0:L[a.stadium_id])||null,d=Ct[e.formation]||{},l=Oi(e.formation)||[],c={};for(const A of i){const E=e.slots[A],S=E?e.playerCards.find(q=>q.id===E):null;S!=null&&S.player?c[A]={...S.player,_evolution_bonus:S.evolution_bonus||0,face:S.player.face||null}:c[A]=null}const u=window.innerWidth>=900,b=u?window.innerWidth-280:window.innerWidth-20,x=u?Math.min(b,860):b,m=Math.round(u?x*.82:x*.85),p=u?84:44;let f="";for(const[A,E]of l){const S=d[A],q=d[E];if(!S||!q)continue;const W=S.x*x,ee=Math.round(.03*m+S.y*.85*m),G=q.x*x,I=Math.round(.03*m+q.y*.85*m),_=c[A],H=c[E],Y=Ht(_,H);Y==="#ff3333"||Y==="#cc2222"?f+=`<line x1="${W.toFixed(1)}" y1="${ee.toFixed(1)}" x2="${G.toFixed(1)}" y2="${I.toFixed(1)}" stroke="${Y}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(f+=`<line x1="${W.toFixed(1)}" y1="${ee.toFixed(1)}" x2="${G.toFixed(1)}" y2="${I.toFixed(1)}" stroke="${Y}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,f+=`<line x1="${W.toFixed(1)}" y1="${ee.toFixed(1)}" x2="${G.toFixed(1)}" y2="${I.toFixed(1)}" stroke="${Y}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let y="";const w=Math.round(p*657/507);for(const A of i){const E=d[A];if(!E)continue;const S=c[A],q=E.x*x;E.y*m;const W=Math.round(.03*m+E.y*(.85*m)),ee=Math.round(q-p/2),G=Math.round(W-w/2);if(S){const I=A.replace(/\d+/,""),_=s&&(s.club_id&&String(S.club_id)===String(s.club_id)||s.country_code&&S.country_code===s.country_code),H=Se({...S,_evolution_bonus:S._evolution_bonus||0},{width:p,showStad:!0,stadDef:s,role:I});_&&((g=s.club)!=null&&g.logo_url||s.image_url),y+=`<div style="position:absolute;left:${ee}px;top:${G}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${A}">
    ...
    <div style="position:relative;width:${x}px;height:${m}px;margin:0 auto">
      <svg style="position:absolute;inset:0;width:100%;height:100%;pointer-events:none" viewBox="0 0 ${x} ${m}">${f}</svg>
      ${y}
      ...
      <div id="stad-none" style="cursor:pointer;width:70px;text-align:center">
        <div style="width:65px;height:85px;border:2px dashed #ccc;border-radius:8px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:4px;${t.stadiumCardId?"":"border-color:#E87722"}">
          <span style="font-size:22px">🚫</span>
          <span style="font-size:9px;color:#666">Aucun</span>
        </div>
      </div>
      ${o.map(s=>{var y,w;const d=t.stadDefMap[s.stadium_id],l=((y=d==null?void 0:d.club)==null?void 0:y.logo_url)||null,c=d!=null&&d.image_url?"/icons/"+d.image_url:null,u=t.stadiumCardId===s.id,b=d!=null&&d.country_code&&!l?`https://flagsapi.com/${d.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,x=(d==null?void 0:d.name)||"Stade",m=x.match(/^(Stade\s+(?:de\s+)?(?:\w+)?)(.*)?$/i),p=m?m[1].trim():x.slice(0,8),f=((w=m==null?void 0:m[2])==null?void 0:w.trim())||"";return`<div class="stad-choice" data-stad-id="${s.id}" style="cursor:pointer;width:80px;text-align:center">
          <div style="width:75px;height:95px;border-radius:8px;background:linear-gradient(135deg,#1a3a6b,#0a1a3a);border:2px solid ${u?"#4fc3f7":"#444"};display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px;overflow:hidden;position:relative">
            ...
            ${l?`<img src="${l}" style="width:36px;height:36px;object-fit:contain;position:relative;z-index:1">`:b?`<img src="${b}" style="width:40px;height:28px;object-fit:cover;border-radius:3px;position:relative;z-index:1">`:'<span style="font-size:24px;position:relative;z-index:1">🏟️</span>'}
    ...
    </div>`),(a=document.getElementById("stad-none"))==null||a.addEventListener("click",()=>{t.stadiumCardId=null,r(),wt(e,t,i)}),document.querySelectorAll(".stad-choice").forEach(s=>{s.addEventListener("click",()=>{t.stadiumCardId=s.dataset.stadId,r(),wt(e,t,i)})})}function dr(t,e,i,n){var m,p,f,y,w;const{openModal:r,closeModal:o}=n,a=t.replace(/\d+/,""),s=(m=e.stadiumCards)==null?void 0:m.find(h=>h.id===e.stadiumCardId),d=s&&((p=e.stadDefMap)==null?void 0:p[s.stadium_id])||null,l=e.slots[t],c=l?e.playerCards.find(h=>h.id===l):null;(f=c==null?void 0:c.player)==null||f.id;const u=new Set;Object.entries(e.slots).forEach(([h,L])=>{var A;if(h===t||!L)return;const g=e.playerCards.find(E=>E.id===L);(A=g==null?void 0:g.player)!=null&&A.id&&u.add(g.player.id)}),e.subs.forEach(h=>{var g;const L=e.playerCards.find(A=>A.id===h);(g=L==null?void 0:L.player)!=null&&g.id&&u.add(L.player.id)});const b=new Set,x=e.playerCards.filter(h=>{const L=h.player;return!(L.job===a||L.job2===a)||u.has(L.id)||b.has(L.id)?!1:(b.add(L.id),!0)});x.sort((h,L)=>{const g=h.evolution_bonus||0,A=L.evolution_bonus||0,E=(a==="GK"?h.player.note_g:a==="DEF"?h.player.note_d:a==="MIL"?h.player.note_m:h.player.note_a)+(a===h.player.job||a===h.player.job2?g:0);return(a==="GK"?L.player.note_g:a==="DEF"?L.player.note_d:a==="MIL"?L.player.note_m:L.player.note_a)+(a===L.player.job||a===L.player.job2?A:0)-E}),r(`Choisir ${a} — ${t}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${e.slots[t]?`
  ...
  <div class="page">
    ...
    <div class="page-body">
      <div class="booster-grid">
        ${o.map(m=>{const p=m.cost===0||r>=m.cost,f=c[m.id]===!0;return`<div class="booster-card ${!p||f?"disabled":""}" data-booster="${m.id}" style="position:relative">
  ...
  </div>`,t.querySelectorAll(".booster-card:not(.disabled)").forEach(m=>{m.addEventListener("click",async()=>{const p=o.find(f=>f.id===m.dataset.booster);if(p){m.style.opacity="0.5",m.style.pointerEvents="none";try{await xr(p,{state:e,toast:n,navigate:i,container:t})}catch(f){n(f.message,"error"),m.style.opacity="",m.style.pointerEvents=""}}})}),t.querySelectorAll(".booster-info-btn").forEach(m=>{m.addEventListener("click",p=>{p.stopPropagation();const f=o.find(y=>y.id===m.dataset.boosterId);vr(f)})})}async function xr(t,{state:e,toast:i,navigate:n,container:r}){var u,b;if(t.cost>0&&e.profile.credits<t.cost){i("Crédits insuffisants","error");return}t.isPub&&await _r();const{data:o}=await v.from("cards").select("card_type, player_id, formation").eq("owner_id",e.profile.id),a=new Set((o||[]).filter(x=>x.card_type==="player").map(x=>x.player_id)),s=new Set((o||[]).filter(x=>x.card_type==="formation").map(x=>x.formation));let d=[],l=null;try{if((u=t.rates)!=null&&u.length)d=await qi(e.profile,t);else{const x=t.type||"player";x==="player"?d=await Yn(e.profile,t.cardCount,t.cost):x==="game_changer"?d=await Wn(e.profile,t.cardCount,t.cost):x==="formation"?d=await Xn(e.profile,t.cost):d=await qi(e.profile,t)}}catch(x){l=x.message||String(x),console.error("[Booster] Erreur:",x)}if(d!=null&&d.length&&t._boosterId&&await pr(e.user.id,t._boosterId),!(d!=null&&d.length)){const x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:3000;gap:16px;color:#fff;padding:24px;text-align:center",x.innerHTML=`
      <div style="font-size:48px">😕</div>
      <div style="font-size:20px;font-weight:900">Aucune carte obtenue</div>
      <div style="font-size:12px;color:rgba(255,255,255,0.6);max-width:320px;word-break:break-all;background:rgba(255,255,255,0.05);padding:10px;border-radius:8px;margin-top:6px">
        ${l||"Vérifie la console (F12) pour plus de détails"}
      </div>
      <button style="margin-top:10px;padding:12px 28px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer" id="anim-close-err">Fermer</button>`,document.body.appendChild(x),(b=x.querySelector("#anim-close-err"))==null||b.addEventListener("click",()=>x.remove());return}d.forEach(x=>{x.card_type==="player"&&x.player?x.isDuplicate=a.has(x.player.id):x.card_type==="formation"&&(x.isDuplicate=s.has(x.formation))});const{data:c}=await v.from("users").select("*").eq("id",e.profile.id).single();c&&(e.profile=c),Jn(d,t,n)}function br(){const t=Math.random()*100;return t<.5?"legende":t<2?"special":t<10?"normal_high":"normal_low"}function gt(t){return Math.max(Number(t.note_g)||0,Number(t.note_d)||0,Number(t.note_m)||0,Number(t.note_a)||0)}function yr(t,e){let i;switch(e){case"legende":i=t.filter(n=>n.rarity==="legende"),i.length||(i=t.filter(n=>n.rarity==="pepite"||n.rarity==="papyte")),i.length||(i=t.filter(n=>gt(n)>=6));break;case"special":i=t.filter(n=>n.rarity==="pepite"||n.rarity==="papyte"),i.length||(i=t.filter(n=>gt(n)>=6));break;case"normal_high":i=t.filter(n=>n.rarity==="normal"&&gt(n)>=6),i.length||(i=t.filter(n=>gt(n)>=6));break;default:i=t.filter(n=>n.rarity==="normal"&&gt(n)>=1&&gt(n)<=5),i.length||(i=t.filter(n=>n.rarity==="normal"));break}return i.length||(i=t),i[Math.floor(Math.random()*i.length)]}async function qi(t,e){if(e.cost>0){const{error:b}=await v.from("users").update({credits:t.credits-e.cost}).eq("id",t.id);if(b)throw b}const i=e.allow_duplicates!==!1;let n=[];const{data:r,error:o}=await v.from("cards").select("player_id, card_type, formation, stadium_id, gc_type").eq("owner_id",t.id);if(o){const{data:b}=await v.from("cards").select("player_id, card_type, formation, gc_type").eq("owner_id",t.id);n=b||[]}else n=r||[];const a=new Set(n.filter(b=>b.card_type==="player").map(b=>b.player_id)),s=new Set(n.filter(b=>b.card_type==="formation").map(b=>b.formation)),d=new Set(n.filter(b=>b.card_type==="game_changer").map(b=>b.gc_type)),l=new Set(n.filter(b=>b.card_type==="stadium").map(b=>b.stadium_id).filter(Boolean)),c=new Set,u=[];for(let b=0;b<(e.cardCount||5);b++){const x=ur(e.rates);if(x){if(x.card_type==="player"){const m=E=>({légende:"legende",pépite:"pepite",pépites:"pepite"})[E]||E,p=x.rarity?m(x.rarity):null;let f=v.from("players").select("id,job,firstname,surname_real,country_code,club_id,rarity,note_g,note_d,note_m,note_a,skin,hair,hair_length,face,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);p&&(f=f.eq("rarity",p));const{data:y}=await f;let w=y||[];if((x.note_min||x.note_max)&&(w=w.filter(E=>{const S=Math.max(Number(E.note_g)||0,Number(E.note_d)||0,Number(E.note_m)||0,Number(E.note_a)||0);return(!x.note_min||S>=x.note_min)&&(!x.note_max||S<=x.note_max)})),w.length||(x.note_min||x.note_max?(w=y||[],console.warn("[Booster] Aucun joueur avec note",x.note_min,"-",x.note_max,"— fallback rareté uniquement")):w=y||[]),!w.length)continue;let h=w.filter(E=>!c.has(E.id));if(i)h.length||(h=w);else if(h=h.filter(E=>!a.has(E.id)),!h.length)continue;const L=h[Math.floor(Math.random()*h.length)];c.add(L.id);const g=a.has(L.id),{data:A}=await v.from("cards").insert({owner_id:t.id,player_id:L.id,card_type:"player"}).select().single();A&&(u.push({...A,player:L,isDuplicate:g}),v.rpc("record_transfer",{p_card_id:A.id,p_player_id:L.id,p_club_name:t.club_name||t.pseudo,p_manager_name:t.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{}))}else if(x.card_type==="game_changer"){const{data:m}=await v.from("gc_definitions").select("id,name,color,effect,image_url,gc_type").eq("is_active",!0).eq("gc_type","game_changer"),p=m!=null&&m.length?m:[{name:"Ressusciter"},{name:"Double attaque"},{name:"Bouclier"},{name:"Vol de note"},{name:"Gel"}],f=i?p:p.filter(L=>!d.has(L.name));if(!i&&!f.length)continue;const y=f[Math.floor(Math.random()*f.length)],w=y.name,{data:h}=await v.from("cards").insert({owner_id:t.id,card_type:"game_changer",gc_type:w,gc_definition_id:y.id||null}).select().single();h&&u.push({...h,_gcDef:y})}else if(x.card_type==="formation"){const m=Kn(),p=i?m:m.filter(h=>!s.has(h));if(!i&&!p.length)continue;const f=p[Math.floor(Math.random()*p.length)],y=s.has(f),{data:w}=await v.from("cards").insert({owner_id:t.id,card_type:"formation",formation:f}).select();w!=null&&w[0]&&u.push({...w[0],isDuplicate:y})}else if(x.card_type==="stadium"){const{data:m,error:p}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)");if(p){console.error("[Booster] stadium_definitions:",p.message);continue}if(!(m!=null&&m.length)){console.warn("[Booster] Aucun stade en DB");continue}const f=i?m:m.filter(L=>!l.has(L.id));if(!i&&!f.length)continue;const y=f[Math.floor(Math.random()*f.length)],{data:w,error:h}=await v.from("cards").insert({owner_id:t.id,card_type:"stadium",stadium_id:y.id}).select("id,card_type,stadium_id").single();if(h){console.error("[Booster] insert stadium card:",h.message);continue}w&&u.push({...w,rarity:"normal",_stadiumDef:y})}}}return u}async function Yn(t,e,i){if(i>0){const{error:l}=await v.from("users").update({credits:t.credits-i}).eq("id",t.id);if(l)throw l}const{data:n}=await v.from("players").select("id,job,firstname,surname_real,country_code,club_id,rarity,note_g,note_d,note_m,note_a,note_min,note_max,skin,hair,hair_length,face,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);if(!(n!=null&&n.length))throw new Error("Pas de joueurs en BDD — ajoutes-en via le panel admin !");const r=n.filter(l=>l.job==="GK"),o=n.filter(l=>l.job!=="GK"),a=!t.first_booster_opened&&r.length>0,s=[];for(let l=0;l<e;l++){const c=l===0&&a?r:l===0?o:n,u=br(),b=yr(c,u);b&&s.push(b)}a&&await v.from("users").update({first_booster_opened:!0}).eq("id",t.id);const{data:d}=await v.from("cards").insert(s.map(l=>({owner_id:t.id,player_id:l.id,card_type:"player"}))).select();return(d||[]).forEach((l,c)=>{v.rpc("record_transfer",{p_card_id:l.id,p_player_id:s[c].id,p_club_name:t.club_name||t.pseudo,p_manager_name:t.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{})}),s.map((l,c)=>({...d[c],player:l}))}async function Wn(t,e,i){const{error:n}=await v.from("users").update({credits:t.credits-i}).eq("id",t.id);if(n)throw n;const{data:r}=await v.from("gc_definitions").select("id,name,gc_type,color,effect,image_url").eq("is_active",!0),o=r!=null&&r.length?r:Object.keys(Bi).map(u=>({name:u,gc_type:"game_changer"})),a=Array.from({length:e},()=>o[Math.floor(Math.random()*o.length)]),s=a.map(u=>({owner_id:t.id,card_type:"game_changer",gc_type:u.name,gc_definition_id:u.id||null})),{data:d,error:l}=await v.from("cards").insert(s).select();return l&&console.error("[Booster GC] Erreur insert:",l.message,l),(d||[]).map((u,b)=>({...u,_gcDef:a[b]||null}))}async function Xn(t,e){const{error:i}=await v.from("users").update({credits:t.credits-e}).eq("id",t.id);if(i)throw i;const{data:n}=await v.from("cards").select("formation").eq("owner_id",t.id).eq("card_type","formation"),r=new Set((n||[]).map(c=>c.formation)),o=Kn(),a=o[Math.floor(Math.random()*o.length)],s=r.has(a),{data:d,error:l}=await v.from("cards").insert({owner_id:t.id,card_type:"formation",formation:a}).select();return l&&console.error("[Booster Formation] Erreur insert:",l.message,l),(d||[]).map(c=>({...c,isDuplicate:s}))}function Jn(t,e,i,n=null){var G,I;if(!t||t.length===0){const _=document.createElement("div");_.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:3000;gap:16px;color:#fff;padding:24px;text-align:center",_.innerHTML=`
      <div style="font-size:48px">😕</div>
      <div style="font-size:20px;font-weight:900">Aucune carte obtenue</div>
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    </div>`}const d=c=>{t.style.overflow="",t.style.height="",t.style.display="",t.style.flexDirection="",i(c)};function l(){var u,b,x;t.style.overflow="hidden",t.style.height="100%",t.style.display="flex",t.style.flexDirection="column";const c=a.length>0;t.innerHTML=`
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      <div style="flex:1;overflow-y:auto;display:flex;flex-wrap:wrap;gap:10px;justify-content:center;align-content:flex-start;padding:8px 16px 16px">
        ${o.map(m=>{const p=a.find(f=>f.gc_type===m.gc_type);return s(m,!!p)}).join("")}
      </div>
    ...
    </div>`,t.querySelectorAll(".gc-select-card").forEach(m=>{m.addEventListener("click",()=>{const p=m.dataset.id,f=o.find(w=>w.id===p);if(!f)return;const y=a.findIndex(w=>w.gc_type===f.gc_type);y>-1?a.splice(y,1):a.length<3&&a.push(f),l()})}),(u=t.querySelector("#gc-launch"))==null||u.addEventListener("click",()=>{c&&d(a)}),(b=t.querySelector("#gc-no-gc"))==null||b.addEventListener("click",()=>d([])),(x=t.querySelector("#gc-reset"))==null||x.addEventListener("click",()=>{a.length&&(a=[],l())})}l()}function Lr(t,e){var r;const i=((r=e==null?void 0:e.state)==null?void 0:r.params)||{},n=t||i.matchMode||"vs_ai_easy";return n==="friend"?`Match vs ${i.friendName||"Ami"}`:n==="random"?"Match vs Random":n==="ranked"?"Match Classé":n==="mini_league"||n==="mini-league"?"🏆 Match de Mini League":`Match vs IA — ${n.replace("vs_ai_","").toUpperCase()}`}async function Tr(t,e,i){const{state:n,navigate:r}=e;t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await v.from("decks").select(`id,name,is_active,formation,stadium_card_id,
      ...
      card:cards(id,card_type,formation,stadium_id,evolution_bonus,
        player:players(id,firstname,surname_real,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order"),d=[...new Set((s||[]).filter(b=>{var x,m;return((x=b.card)==null?void 0:x.card_type)==="stadium"&&((m=b.card)==null?void 0:m.stadium_id)}).map(b=>b.card.stadium_id))],l={};if(d.length){const{data:b}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",d);(b||[]).forEach(x=>{l[x.id]=x}),(s||[]).forEach(x=>{var m,p;((m=x.card)==null?void 0:m.card_type)==="stadium"&&((p=x.card)!=null&&p.stadium_id)&&(x.card._stadiumDef=l[x.card.stadium_id]||null)})}let c=0;function u(){var g,A,E,S,q,W,ee;const b=o[c],x=(s||[]).filter(G=>G.deck_id===b.id),m=x.filter(G=>{var I;return G.is_starter&&((I=G.card)==null?void 0:I.player)}).map(G=>Di(G.card,G.position)),p=x.find(G=>{var I;return((I=G.card)==null?void 0:I.card_type)==="formation"}),f=b.formation||((g=p==null?void 0:p.card)==null?void 0:g.formation)||"4-4-2";let y=m.length>=11?xi(m,f):null,w=((A=b.stadium_card)==null?void 0:A.stadium_def)||null;w&&y&&(y=ri(y,w));const h=m.length>=11;dt(t),t.style.height="100%",t.style.overflow="hidden",t.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff;position:relative">
      ...
      <div id="deck-top-bar" style="flex-shrink:0">
        ...
        <div style="display:flex;align-items:center;gap:8px;padding:6px 8px">
          ...
          <div style="flex:1;text-align:center">
            ...
            <div style="font-size:11px;opacity:.6">${f} · ${m.length}/11${b.is_active?" · ⭐":""}</div>
          </div>
      ...
      <div id="deck-bottom-bar" style="flex-shrink:0;padding:10px 14px 14px;display:flex;flex-direction:column;gap:8px;background:rgba(0,0,0,0.2)">
        ...
        stroke="${E}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const m=typeof window<"u"&&window.innerWidth>=900?Math.min(Math.max(81,Math.round(r*.225)),117):Math.max(44,Math.round(r*.168)),p=Math.round(m*657/507);for(const[y,w]of Object.entries(c)){const h=b(y);if(!h||!w)continue;const L=y.replace(/[0-9]/g,""),g=a.includes(w.cardId),A=i==="attack"&&(["MIL","ATT"].includes(L)||g)&&!w.used||i==="defense"&&["GK","DEF","MIL"].includes(L)&&!w.used,E=n.includes(w.cardId);let S=w.boost||0,q=!1;w.stadiumBonus&&(i==="attack"&&(L==="ATT"||L==="MIL")||i==="defense"&&(L==="GK"||L==="DEF"||L==="MIL")?(S+=10,q=!0):i||(S+=10,q=!0));const W=Math.round(h.x-m/2),ee=Math.round(h.y-p/2);if(w.used){x+=`<image href="${`${typeof import.meta<"u"&&"/"||"/"}icons/carte-dos.png`}" x="${W}" y="${ee}" width="${m}" height="${p}" preserveAspectRatio="xMidYMid slice" class="match-used-hit" data-card-id="${w.cardId}" data-role="${L}" style="cursor:pointer"/>`;continue}const G=Se({...w,_evolution_bonus:0,stadiumBonus:!1},{width:m,showStad:!1,stadDef:null,role:L,extraNote:S,_cardOffset:30,_forceStadColor:q}),I=E?`position:absolute;top:30px;left:0;width:${m}px;height:${p}px;outline:3px solid #FFD700;outline-offset:2px;border-radius:8px;pointer-events:none;`:"";x+=`<foreignObject x="${W-2}" y="${ee-30}" width="${m+8}" height="${p+60}" style="overflow:visible">
      <div xmlns="http://www.w3.org/1999/xhtml" style="position:relative">
        ${G}
        ${E?`<div style="${I}"></div>`:""}
      </div>
    </foreignObject>`,A&&(x+=`<rect x="${W}" y="${ee}" width="${m}" height="${p}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${E?"selected":""}" data-card-id="${w.cardId}" data-role="${L}" style="cursor:pointer"/>`)}const f=s!==null?s:Math.round(Math.max(m*.7,80));return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-f} ${-f} ${r+f*2} ${o+f*2}" width="100%" style="display:block;width:100%;margin:0 auto">
    ${x}
  </svg>`}function Rt(t,e,i,n,r=300,o=300,a=[],s=null){return`<div id="match-terrain-wrap" style="position:relative;padding:0 4px">
    ${_i(t,e,i,n,r,o,a,s)}
  </div>`}async function ki(t,e,i,n){var A;const{state:r,navigate:o,toast:a}=e;dt(t);const s=r.params||{};if(t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!s.deckId)return Tr(t,e,i);const d=s.deckId;let l,c,u,b;try{const E=await Promise.all([v.from("decks").select("formation,name,stadium_card_id").eq("id",d).single(),v.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation, evolution_bonus,
            player:players(id,firstname,surname_real,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
              clubs(encoded_name,logo_url)))`).eq("deck_id",d).order("slot_order")]);l=E[0].data,u=E[0].error,c=E[1].data,b=E[1].error}catch(E){console.error("[Match] Exception chargement deck:",E),St(t,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>o("home"));return}if(u||b){console.error("[Match] Erreur Supabase:",u||b),St(t,"⚠️","Erreur lors du chargement du deck.","Retour",()=>o("home"));return}const x=(c||[]).filter(E=>{var S;return E.is_starter&&((S=E.card)==null?void 0:S.player)}).map(E=>Di(E.card,E.position)),m=(c||[]).filter(E=>{var S;return!E.is_starter&&((S=E.card)==null?void 0:S.player)}).map(E=>Di(E.card,E.position));if(x.length<11){St(t,"⚠️",`Deck incomplet (${x.length}/11).`,"Compléter",()=>o("decks"));return}const p=(c||[]).find(E=>{var S;return((S=E.card)==null?void 0:S.card_type)==="formation"}),f=(l==null?void 0:l.formation)||((A=p==null?void 0:p.card)==null?void 0:A.formation)||"4-4-2",{data:y,error:w}=await v.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",r.profile.id).eq("card_type","game_changer"),{data:h}=await v.from("gc_definitions").select("*").eq("is_active",!0),L=(y||[]).map(E=>({...E,_gcDef:(h==null?void 0:h.find(S=>S.name===E.gc_type||S.id===E.gc_definition_id))||null}));let g=null;if(l!=null&&l.stadium_card_id){const{data:E}=await v.from("cards").select("stadium_id").eq("id",l.stadium_card_id).single();if(E!=null&&E.stadium_id){const{data:S}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").eq("id",E.stadium_id).single();g=S||null}}n({deckId:d,formation:f,starters:x,subsRaw:m,gcCardsEnriched:L,gcDefs:h||[],stadiumDef:g})}function zt(){return Math.min(window.innerWidth-40,860)}function Gt(){return Math.round(zt()*1.1)}function Zn(t){var r,o;if(!t)return null;const e=t._line||t.job||"MIL",i=e==="GK"?t.note_g||0:e==="DEF"?t.note_d||0:e==="MIL"?t.note_m||0:t.note_a||0,n=t.stadiumBonus?10:0;return{name:t.name,firstname:t.firstname||"",note:i+(t.boost||0)+n,note_g:t.note_g||0,note_d:t.note_d||0,note_m:t.note_m||0,note_a:t.note_a||0,_evolution_bonus:0,stadiumBonus:t.stadiumBonus||!1,boost:t.boost||0,job:t.job,job2:t.job2||null,_line:t._line||t.job,_col:t._col,country_code:t.country_code,club_id:t.club_id,rarity:t.rarity,clubName:t.clubName||((r=t.clubs)==null?void 0:r.encoded_name)||null,clubLogo:t.clubLogo||((o=t.clubs)==null?void 0:o.logo_url)||null,face:t.face||null,portrait:Mt(t)}}function Ir(t,e,i,n="Adversaire"){Cn("/sounds/match-opening.mp3",.3);const r=zt(),o=Gt();return`
    ...
    </div>`}function eo(t){var o,a,s;if(!t)return"";const e=d=>d?Se({...d,_evolution_bonus:0},{width:52,role:d._line||d.job,showStad:!!d.stadiumBonus,extraNote:d.boost||0}):"",n={goal:"⚽","goal-home":"⚽","goal-ai":"⚽",duel:"⚔️",midfield:"🎯",sub:"🔄",gc:"⚡",boost:"💥","defense-won":"🛡️","attack-won":"⚔️","defense-lost":"😓","attack-lost":"😓"}[t.type]||"📋";return`
    ...
    <div style="animation:goalPop 0.4s ease-out;text-align:center">
      ...
      }`,document.head.appendChild(d)}setTimeout(()=>{a.style.opacity="0",a.style.transition="opacity 0.4s",setTimeout(()=>{a.remove(),r==null||r()},400)},1800)}function Sr(t,e,i){const n=document.getElementById("sub-anim-overlay");n&&n.remove();const r=document.createElement("div");r.id="sub-anim-overlay",r.style.cssText=`
    position:fixed;inset:0;z-index:3000;
    display:flex;flex-direction:column;align-items:center;justify-content:center;
    ...
    pointer-events:none;white-space:nowrap;max-width:90vw;text-align:center`,r.textContent=t,!document.getElementById("game-toast-style")){const o=document.createElement("style");o.id="game-toast-style",o.textContent=`
      ...
      }`,document.head.appendChild(o)}document.body.appendChild(r),setTimeout(()=>{r.style.opacity="0",r.style.transition="opacity 0.3s",setTimeout(()=>r.remove(),300)},i)}function to(t,e){const i=Pe(t,"MIL"),n=t.stadiumBonus||e&&(e.club_id&&String(t.club_id)===String(e.club_id)||e.country_code&&t.country_code===e.country_code)?10:0;return i+n}function bt(t,e){return t.reduce((i,n)=>i+to(n,e),0)}function yt(t){let e=0;for(let i=0;i<t.length-1;i++){const n=Ht(t[i],t[i+1]);n==="#00ff88"?e+=10:n==="#FFD700"&&(e+=5)}return e}function bi(t,e,i,n,r){return`<div id="duel-row-${n}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0), opacity .5s ease;transform-origin:center">
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${t.map((o,a)=>{const s=a<t.length-1?Ht(o,t[a+1]):null,d=!s||s==="#ff3333"||s==="#cc2222",l=s==="#00ff88"?"+10":s==="#FFD700"?"+5":"";return to(o,r),o.stadiumBonus||r&&(r.club_id&&String(o.club_id)===String(r.club_id)||r.country_code&&(o.country_code,r.country_code)),`
    ...
    </div>`}async function zr(t,e){const{state:i}=e,n=i.params||{},r=n.matchMode||"vs_ai_easy",o=r==="solo",a=n.soloLevel||1,s=r.replace("vs_ai_",""),d=r;let l=null;if(o){const{data:c}=await v.from("solo_levels").select("*").eq("level_number",a).maybeSingle();l=c||{level_number:a,target_note_avg:10,nb_liens_jaune:2,nb_liens_vert:1,nb_joueurs_stade:2,reward_credits:500}}await ki(t,e,r,async({deckId:c,formation:u,starters:b,subsRaw:x,gcCardsEnriched:m,gcDefs:p,stadiumDef:f})=>{try{let y=xi(b,u);f&&(y=ri(y,f),gi(x,f));const w=o?await Mr(u,l):await Ar(u,s),h=w.lines||w,L=async g=>{try{const A=d==="vs_ai_club"||d==="solo"?"club":d,{data:E,error:S}=await v.from("matches").insert({home_id:i.profile.id,away_id:null,mode:A,home_deck_id:c,status:"in_progress"}).select().single();if(S){console.error("[MatchIA] Erreur création match:",S),St(t,"⚠️","Impossible de créer le match ("+S.message+").","Retour",()=>e.navigate("home"));return}const q=w.stadiumDef||null;q&&h&&(ri(h,q),gi(w.subs||[],q));const W={gcDefs:p||[],matchId:E==null?void 0:E.id,mode:d,difficulty:s,formation:u,isSolo:o,soloLevel:a,soloLevelConfig:l,homeTeam:y,aiTeam:h,homeSubs:x,subsUsed:0,maxSubs:Math.min(x.length,3),aiSubs:w.subs||[],aiSubsUsed:0,aiMaxSubs:Math.min((w.subs||[]).length,3),aiUsedSubIds:[],aiGcCards:w.gcCards||[],aiUsedGc:[],aiStadiumDef:q,homeScore:0,aiScore:0,gcCards:g,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Cr(t,W,e)}catch(A){console.error("[MatchIA] Exception launchMatch:",A),St(t,"⚠️","Erreur au lancement du match : "+A.message,"Retour",()=>e.navigate("home"))}};if(!m.length){L([]);return}wi(t,m,L)}catch(y){console.error("[MatchIA] Exception setup:",y),St(t,"⚠️","Erreur de préparation du match : "+y.message,"Retour",()=>e.navigate("home"))}})}async function Ar(t,e){var f;const{data:i}=await v.from("players").select("id,firstname,surname_real,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(80);if(!i||i.length<11)return{lines:io(t),subs:[],gcCards:[],stadiumDef:null};const n=_t[t]||_t["4-4-2"],r={GK:[],DEF:[],MIL:[],ATT:[]},o=new Set;function a(y,w,h){var L,g;return o.add(y.id),{cardId:"ai-"+y.id+"-"+h,id:y.id,firstname:y.firstname,name:y.surname_real,country_code:y.country_code,club_id:y.club_id,job:y.job,job2:y.job2,note_g:Number(y.note_g)||0,note_d:Number(y.note_d)||0,note_m:Number(y.note_m)||0,note_a:Number(y.note_a)||0,rarity:y.rarity,skin:y.skin,hair:y.hair,hair_length:y.hair_length,clubName:((L=y.clubs)==null?void 0:L.encoded_name)||null,clubLogo:((g=y.clubs)==null?void 0:g.logo_url)||null,boost:0,used:!1,_line:w}}for(const y of["GK","DEF","MIL","ATT"]){const w=i.filter(E=>E.job===y&&!o.has(E.id)),h=i.filter(E=>E.job!==y&&!o.has(E.id)),L=[...w,...h],g=[];for(let E=0;E<n[y];E++){const S=L[E];S&&g.push(a(S,y,E))}const A=st(g.length);g.forEach((E,S)=>{E._col=A[S]}),r[y]=g}const d=i.filter(y=>!o.has(y.id)).slice(0,5).map((y,w)=>a(y,y.job,100+w)),u=Object.keys(ze).sort(()=>Math.random()-.5).slice(0,3).map((y,w)=>{var h,L;return{id:"ai-gc-"+w,gc_type:y,name:((h=ze[y])==null?void 0:h.name)||y,icon:((L=ze[y])==null?void 0:L.icon)||"⚡"}}),b=Object.values(r).flat(),x={};b.forEach(y=>{y.club_id&&(x[y.club_id]=(x[y.club_id]||0)+1)});const m=(f=Object.entries(x).sort((y,w)=>w[1]-y[1])[0])==null?void 0:f[0];let p=null;if(m){const{data:y}=await v.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",m).single();y&&(p={club_id:y.id,country_code:null,name:y.encoded_name+" Stadium",club:{encoded_name:y.encoded_name,logo_url:y.logo_url}})}return{lines:r,subs:d,gcCards:u,stadiumDef:p}}function kn(t){const e=t.job||"ATT";return Number(e==="GK"?t.note_g:e==="DEF"?t.note_d:e==="MIL"?t.note_m:t.note_a)||0}function ci(t,e,i){if(i<=0||!t.length)return[];const n=[...t].sort((r,o)=>Math.abs(kn(r)-e)-Math.abs(kn(o)-e));return n.slice(0,Math.min(i,n.length))}async function Mr(t,e){var G,I;const i=Number(e==null?void 0:e.target_note_avg)||10,n=Number(e==null?void 0:e.nb_liens_jaune)||0,r=Number(e==null?void 0:e.nb_liens_vert)||0,o=Number(e==null?void 0:e.nb_joueurs_stade)||0,{data:a}=await v.from("players").select("id,firstname,surname_real,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,clubs(encoded_name,logo_url,country_code)").eq("is_active",!0).limit(300);if(!a||a.length<16)return{lines:io(t),subs:[],gcCards:[],stadiumDef:null};const s={};a.forEach(_=>{_.club_id&&(s[_.club_id]=(s[_.club_id]||0)+1)});const d=((G=Object.entries(s).sort((_,H)=>H[1]-_[1])[0])==null?void 0:G[0])||null,l=d?a.filter(_=>_.club_id===d):[],c=((I=l[0])==null?void 0:I.country_code)||null,u=c?a.filter(_=>_.country_code===c&&_.club_id!==d):[],b=Math.min(l.length,Math.max(2,r+1),16),x=Math.min(u.length,Math.max(0,n),16-b),m=[],p=new Set;ci(l,i,b).forEach(_=>{m.push(_),p.add(_.id)}),ci(u,i,x).forEach(_=>{m.push(_),p.add(_.id)});const f=Math.max(0,16-m.length),y=a.filter(_=>!p.has(_.id));ci(y,i,f).forEach(_=>{m.push(_),p.add(_.id)});const w=_t[t]||_t["4-4-2"],h={GK:[],DEF:[],MIL:[],ATT:[]},L=new Set;function g(_,H,Y){var ae,de;return L.add(_.id),{cardId:"ai-"+_.id+"-"+Y,id:_.id,firstname:_.firstname,name:_.surname_real,country_code:_.country_code,club_id:_.club_id,job:_.job,job2:_.job2,note_g:Number(_.note_g)||0,note_d:Number(_.note_d)||0,note_m:Number(_.note_m)||0,note_a:Number(_.note_a)||0,rarity:_.rarity,skin:_.skin,hair:_.hair,hair_length:_.hair_length,clubName:((ae=_.clubs)==null?void 0:ae.encoded_name)||null,clubLogo:((de=_.clubs)==null?void 0:de.logo_url)||null,boost:0,used:!1,_line:H}}for(const _ of["GK","DEF","MIL","ATT"]){const H=m.filter(U=>U.job===_&&!L.has(U.id)),Y=m.filter(U=>U.job!==_&&!L.has(U.id)),ae=[...H,...Y],de=[];for(let U=0;U<w[_];U++){const le=ae[U];le&&de.push(g(le,_,U))}const j=st(de.length);de.forEach((U,le)=>{U._col=j[le]}),h[_]=de}let A=m.filter(_=>!L.has(_.id));if(A.length<5){const _=a.filter(H=>!L.has(H.id)&&!A.some(Y=>Y.id===H.id));A=A.concat(ci(_,i,5-A.length))}const E=A.slice(0,5).map((_,H)=>g(_,_.job,100+H)),W=Object.keys(ze).sort(()=>Math.random()-.5).slice(0,3).map((_,H)=>{var Y,ae;return{id:"ai-gc-"+H,gc_type:_,name:((Y=ze[_])==null?void 0:Y.name)||_,icon:((ae=ze[_])==null?void 0:ae.icon)||"⚡"}});let ee=null;if(o>0&&d){const{data:_}=await v.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",d).single();_&&(ee={club_id:_.id,country_code:null,name:_.encoded_name+" Stadium",club:{encoded_name:_.encoded_name,logo_url:_.logo_url}})}return{lines:h,subs:E,gcCards:W,stadiumDef:ee}}function io(t){const e=_t[t]||_t["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},n=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let r=0;for(const o of["GK","DEF","MIL","ATT"]){const a=[];for(let d=0;d<e[o];d++){const l=3+Math.floor(Math.random()*5);a.push({cardId:"fake-"+r,id:"fake-"+r,firstname:"IA",name:n[r%n.length],country_code:"XX",club_id:null,job:o,job2:null,note_g:o==="GK"?l:2,note_d:o==="DEF"?l:2,note_m:o==="MIL"?l:2,note_a:o==="ATT"?l:2,rarity:"normal",boost:0,used:!1,_line:o}),r++}const s=st(a.length);a.forEach((d,l)=>{d._col=s[l]}),i[o]=a}return i}function Cr(t,e,i){var r;Cn("/sounds/match-opening.mp3",.3);const n=`<div style="position:relative;width:22px;height:22px;flex-shrink:0;display:flex;align-items:center;justify-content:center">
  ...
  </div>`,requestAnimationFrame(()=>requestAnimationFrame(()=>{const o=t.querySelector(".opponent-preview-wrap"),a=t.querySelector("#opponent-swipe-zone");if(!o||!a)return;const s=a.clientWidth>=900,d=Math.max(200,a.clientHeight-(s?8:40)),l=Math.max(200,a.clientWidth-(s?8:16)),c=s?null:Math.round(Math.max(44,Math.round(l*.168))*.55);o.innerHTML=Rt(e.aiTeam,e.formation,null,[],l,d,[],c),o.style.cssText=`width:${l}px;height:${d}px;overflow:hidden;flex-shrink:0`;const u=o.querySelector("svg");u&&(u.style.cssText="display:block;width:100%;height:100%",u.setAttribute("preserveAspectRatio",s?"xMidYMid meet":"none"))})),setTimeout(()=>jr(t,e,i),5e3)}const Ke=t=>Zn(t);function jr(t,e,i){const n=e.homeTeam.MIL||[],r=e.aiTeam.MIL||[],o=e.stadiumDef||null,a=e.aiStadiumDef||null,s=bt(n,o)+yt(n),d=bt(r,a)+yt(r),l=s>=d;t.innerHTML=`
  <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:clamp(6px,1.5vh,20px);padding:clamp(8px,1.5vw,24px);background:#0a3d1e">
    ...
    <div id="duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center, rgba(10,61,30,0.4), rgba(10,61,30,0.92))"></div>
  </div>`;const c=()=>{const m=(p,f)=>t.querySelectorAll(p).forEach((y,w)=>{setTimeout(()=>{y.style.opacity="1",y.style.transform="translateY(0) scale(1)"},f+w*90)});m(".duel-card-home",150),m(".duel-card-ai",500),setTimeout(()=>t.querySelectorAll(".duel-link").forEach((p,f)=>{setTimeout(()=>{p.style.opacity="1"},f*70)}),900),setTimeout(()=>{const p=t.querySelector("#vs-label");p&&(p.style.opacity="1",p.style.animation="vsFlash .5s ease"),t.querySelectorAll(".duel-score-line").forEach(f=>f.style.opacity="1")},1250),setTimeout(()=>{u("score-home",s,800),u("score-ai",d,800)},1500)};function u(m,p,f){const y=document.getElementById(m);if(!y)return;const w=performance.now(),h=L=>{const g=Math.min(1,(L-w)/f);y.textContent=Math.round(p*(1-Math.pow(1-g,3))),g<1?requestAnimationFrame(h):y.textContent=p};requestAnimationFrame(h)}requestAnimationFrame(c),e.attacker=l?"home":"ai";const b=l?Qn():null;l&&(e.boostCard={value:b}),e.log.push({type:"duel",title:"Milieu de Terrain",homePlayers:n.map(m=>Ke(m)),aiPlayers:r.map(m=>Ke(m)),homeTotal:s,aiTotal:d,text:`Duel milieu : ${e.clubName} ${s} – ${d} IA → ${l?e.clubName+" attaque":"IA attaque"}`});const x=()=>{e.phase=e.attacker==="home"?"attack":"ai-attack",ke(t,e,i),e.attacker==="ai"&&setTimeout(()=>en(t,e,i),800)};setTimeout(()=>{const m=document.getElementById("score-home"),p=document.getElementById("score-ai"),f=document.getElementById(l?"duel-row-home":"duel-row-ai"),y=document.getElementById(l?"duel-row-ai":"duel-row-home"),w=l?m:p,h=l?p:m;w&&(w.style.fontSize="80px",w.style.color=l?"#FFD700":"#ff6b6b",w.style.animation="duelPulse .5s ease"+(l?", duelGlow 1.5s ease infinite .5s":"")),h&&(h.style.opacity="0.25"),setTimeout(()=>{f&&(f.style.transformOrigin="center",f.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",f.style.zIndex="5"),setTimeout(()=>{var g;const L=document.getElementById("duel-shock");if(L){const A=(g=y||f)==null?void 0:g.getBoundingClientRect(),E=t.querySelector(".match-screen").getBoundingClientRect();A&&(L.style.top=A.top-E.top+A.height/2+"px"),L.style.animation="shockwave .5s ease-out forwards"}y&&(y.style.transformOrigin="center",y.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var g;const L=document.getElementById("duel-finale");L&&(L.innerHTML=`
          ...
          <button id="start-match-btn" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,0.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">
            ▶ Commencer le match
          </button>`,L.style.transition="opacity .45s ease",L.style.opacity="1",L.style.pointerEvents="auto",(g=document.getElementById("start-match-btn"))==null||g.addEventListener("click",x))},600)},700)},2800)}function ke(t,e,i){var W,ee,G,I,_,H,Y,ae,de;const n=e.selected.map(j=>j.cardId),r=e.usedSubIds||[],o=e.homeSubs.filter(j=>!r.includes(j.cardId)),s=Object.values(e.homeTeam).flat().filter(j=>j.used).length>0&&o.length>0&&e.subsUsed<e.maxSubs,d=[...e.homeTeam.MIL||[],...e.homeTeam.ATT||[]].filter(j=>!j.used),l=[...e.aiTeam.MIL||[],...e.aiTeam.ATT||[]].filter(j=>!j.used),c=Ot(e.aiTeam),u=d.length===0&&l.length===0&&!c,b=(e.homeTeam.DEF||[]).filter(j=>!j.used),x=(e.homeTeam.GK||[]).filter(j=>!j.used);let m=[];e.phase==="attack"&&d.length===0&&(c?(m=b.map(j=>j.cardId),b.length===0&&(m=m.concat(x.map(j=>j.cardId)))):u&&(m=[...b,...x].map(j=>j.cardId))),e.log[e.log.length-1];const p=e.phase==="ai-attack"||e.phase==="ai-defense",f=e.phase==="attack",y=e.phase==="defense",w=e.phase==="finished",L=(e.homeSubs||[]).filter(j=>!(e.usedSubIds||[]).includes(j.cardId)).length>0&&e.subsUsed<e.maxSubs,g=f&&d.length===0&&m.length===0&&!L,A=e.gcCards.filter(j=>!e.usedGc.includes(j.id)),E=e.boostCard&&!e.boostUsed;t.style.overflow="hidden",t.style.height="100%",t.style.display="flex",t.style.flexDirection="column",t.innerHTML=`
  <style>
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    ${(()=>{const j=window.innerWidth>=700,U=(D,K,pe)=>{var Ae,Be;const ne=(e.gcDefs||[]).find($e=>$e.name===D.gc_type),me=(ne==null?void 0:ne.name)||D.gc_type,we=(ne==null?void 0:ne.effect)||((Ae=ze[D.gc_type])==null?void 0:Ae.desc)||"",he=ne!=null&&ne.image_url?`/icons/${ne.image_url}`:null,Te=((Be=ze[D.gc_type])==null?void 0:Be.icon)||"⚡";return`<div class="gc-mini" data-gc-id="${D.id}" data-gc-type="${D.gc_type}" style="flex-shrink:0;cursor:pointer">
        ...
        <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
          ${Rt(e.homeTeam,e.formation,e.phase,n,j?1300:zt(),j?600:Gt(),m)}
        </div>
      </div>`;return j?`
  ...
  <div id="match-history-panel">
    ...
    </div>`,(g=u.querySelector("#sub-close"))==null||g.addEventListener("click",()=>u.remove()),(A=u.querySelector("#out-up"))==null||A.addEventListener("click",()=>{s>0&&(s--,b())}),(E=u.querySelector("#out-down"))==null||E.addEventListener("click",()=>{s<o.length-1&&(s++,b())}),(S=u.querySelector("#in-up"))==null||S.addEventListener("click",()=>{l>0&&(l--,b())}),(q=u.querySelector("#in-down"))==null||q.addEventListener("click",()=>{l<a.length-1&&(l++,b())});const L=(ee,G,I,_)=>{const H=u.querySelector("#"+ee);if(!H)return;let Y=0;H.addEventListener("touchstart",ae=>{Y=ae.touches[0].clientY},{passive:!0}),H.addEventListener("touchend",ae=>{const de=ae.changedTouches[0].clientY-Y;if(Math.abs(de)<30)return;const j=G();de<0&&j<_-1?(I(j+1),b()):de>0&&j>0&&(I(j-1),b())},{passive:!0})};L("in-panel",()=>l,ee=>l=ee,a.length),L("out-panel",()=>s,ee=>s=ee,o.length),(W=u.querySelector("#sub-confirm"))==null||W.addEventListener("click",ee=>{if(ee.preventDefault(),ee.stopPropagation(),c)return;c=!0;const G=o[s],I=a[l];if(!G||!I)return;let _=null,H=-1;for(const[ae,de]of Object.entries(e.homeTeam)){const j=(de||[]).findIndex(U=>U.cardId===G.cardId);if(j!==-1){_=ae,H=j;break}}if(H===-1||!_){Et("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),u.remove();return}const Y={...I,_line:_,_col:G._col||0,used:!1,boost:0};e.homeTeam[_].splice(H,1,Y),e.usedSubIds||(e.usedSubIds=[]),e.usedSubIds.push(I.cardId),e.subsUsed++,e.selected=[],e.log.push({type:"sub",subSide:"home",clubName:e.clubName,outPlayer:{name:G.name,firstname:G.firstname,note:Pe(G,_),portrait:Mt(G),job:G.job,country_code:G.country_code,rarity:G.rarity,clubName:G.clubName,clubLogo:G.clubLogo},inPlayer:{name:I.name,firstname:I.firstname,note:Pe(I,_),portrait:Mt(I),job:I.job,country_code:I.country_code,rarity:I.rarity,clubName:I.clubName,clubLogo:I.clubLogo},text:`🔄 ${I.firstname} ${I.name} remplace ${G.firstname} ${G.name}`}),u.remove(),Sr(G,I,()=>ke(t,e,i))})}document.body.appendChild(u),b()}function Gr(t,e,i,n,r){var b,x;const o=(n.gcDefs||[]).find(m=>m.name===e),a=ze[e]||{icon:"⚡",desc:"Carte spéciale."},s=(o==null?void 0:o.name)||e,d=(o==null?void 0:o.effect)||a.desc,l=o!=null&&o.image_url?`/icons/${o.image_url}`:null,c=a.icon||"⚡",u=document.createElement("div");u.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",u.innerHTML=`
    ${tt(s,l,c,d,{width:190})}
    <!-- Boutons -->
    <div style="display:flex;gap:12px;width:190px">
      <button id="gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
      <button id="gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
    </div>`,document.body.appendChild(u),(b=u.querySelector("#gc-back"))==null||b.addEventListener("click",()=>u.remove()),(x=u.querySelector("#gc-use"))==null||x.addEventListener("click",()=>{u.remove(),Or(t,e,i,n,r)})}function pi(t,e,i,n,r,o){const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let s=[];function d(){var l,c;a.innerHTML=`
    ...
    <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start;justify-content:center">
      ${t.map(u=>{const b=u._line||u.job||"MIL",x=s.find(p=>p.cardId===u.cardId),m=Se({...u,_evolution_bonus:0},{width:90,showStad:!0,role:b,extraNote:u.boost||0});return`<div class="gc-pick-item" data-cid="${u.cardId}"
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(r=>{r.addEventListener("click",()=>{const o=r.dataset.cardId;for(const a of["GK","DEF","MIL","ATT"]){const s=(e.homeTeam[a]||[]).find(d=>d.cardId===o);if(s){s.boost=(s.boost||0)+e.boostCard.value,e.log.push({text:`⚡ Boost +${e.boostCard.value} appliqué à ${s.name}`,type:"info"});break}}e.boostUsed=!0,i.closeModal(),ke(t,e,i)})})}async function ht(t,e,i){var c,u,b;It(),Ue(),e._timerInt&&(clearInterval(e._timerInt),e._timerInt=null),e.phase="finished";const{state:n}=i,r=e.homeScore>e.aiScore,o=e.homeScore===e.aiScore,a=r?"victoire":o?"nul":"defaite",s=e.isSolo&&((c=e.soloLevelConfig)!=null&&c.reward_credits)&&r?Number(e.soloLevelConfig.reward_credits):Xo(e.mode,a);if(e.isSolo&&r)try{const{data:x}=await v.from("user_solo_progress").select("unlocked_level").eq("user_id",n.profile.id).maybeSingle(),m=(x==null?void 0:x.unlocked_level)||1;e.soloLevel>=m&&await v.from("user_solo_progress").upsert({user_id:n.profile.id,unlocked_level:e.soloLevel+1,updated_at:new Date().toISOString()})}catch(x){console.warn("[Solo] Erreur mise à jour progression:",x.message)}e.matchId&&await v.from("matches").update({status:"finished",home_score:e.homeScore,away_score:e.aiScore,winner_id:r?n.profile.id:null,home_credits_reward:s,played_at:new Date().toISOString()}).eq("id",e.matchId);const d={credits:(n.profile.credits||0)+s,matches_played:(n.profile.matches_played||0)+1};r?d.wins=(n.profile.wins||0)+1:o?d.draws=(n.profile.draws||0)+1:d.losses=(n.profile.losses||0)+1,await v.from("users").update(d).eq("id",n.profile.id),await i.refreshProfile();const l=document.createElement("div");l.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",l.innerHTML=`
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      ...
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${c?"🏆 Retour à la Mini League":a?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(F),(te=F.querySelector("#pvp-end-home"))==null||te.addEventListener("click",()=>{F.remove(),it(t),c?x("mini-league",{openLeagueId:c}):x(a?"ranked":"home")})}const G=v.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},k=>{var M;const T=k.new;try{if(T.status==="finished"||T.forfeit){if(A)return;A=!0,E&&(clearInterval(E),E=null),Ue(),T.game_state&&(g=T.game_state),ee(T);return}if(T.game_state){const B=g;g=T.game_state;const z=g._lastGC;if(z&&z.seq&&!W.has(z.seq)&&(W.add(z.seq),z.by!==p)){$(z.type,z.by,()=>U());return}const R=B[p+"Score"]||0,F=B[f+"Score"]||0,N=g[p+"Score"]||0,V=g[f+"Score"]||0,Q=N>R,ie=V>F;if((Q||ie)&&!q.has(g.round)){q.add(g.round);const te=[...g.log||[]].reverse().find(se=>se.isGoal),Z=((te==null?void 0:te.homePlayers)||[]).map(se=>({name:se.name,note:se.note,portrait:se.portrait,job:se.job}));!!((M=te==null?void 0:te.text)!=null&&M.includes("DERNIER CORNER"))?we().then(()=>{Qe(Z,N,V,Q,()=>U())}):Qe(Z,N,V,Q,()=>U());return}U()}}catch(B){console.error("[PvP] crash:",B)}}).subscribe();async function I(k){Object.assign(g,k),g.lastActionAt=new Date().toISOString();const{error:T}=await v.from("matches").update({game_state:g}).eq("id",i);T&&m("Erreur de synchronisation","error");try{U()}catch(M){console.error("[PvP] renderPvpScreen crash:",M)}}async function _(){if(A)return;A=!0,It(),Ue(),E&&(clearInterval(E),E=null);const k=n?h.away_id:h.home_id,T=n?"p2":"p1",M=n?"p1":"p2",B={...g,[T+"Score"]:3,[M+"Score"]:0,phase:"finished"},z=B.p1Score||0,R=B.p2Score||0;try{await v.from("matches").update({game_state:B}).eq("id",i);const{data:F,error:N}=await v.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:k,p_home_score:z,p_away_score:R,p_forfeit:!0});if(N&&console.error("[PvP] finish_pvp_match (forfeit):",N),c)try{await v.rpc("check_and_finish_mini_league",{p_league_id:c})}catch(V){console.error("[PvP] check_and_finish_mini_league (forfeit):",V)}try{const{data:V}=await v.rpc("apply_match_rewards",{p_match_id:i});V!=null&&V.success&&!(V!=null&&V.skipped)&&typeof e.refreshProfile=="function"&&await e.refreshProfile()}catch(V){console.error("[PvP] apply_match_rewards (forfeit):",V)}}catch{}try{v.removeChannel(G)}catch{}setTimeout(()=>{it(t),c?x("mini-league",{openLeagueId:c}):x("home")},800)}const H={BOOST_STAT:({value:k=1,count:T=1,roles:M=[]},B,z)=>{const R=B[p+"Team"],F=Object.entries(R).filter(([N])=>!M.length||M.includes(N)).flatMap(([N,V])=>V.filter(Q=>!Q.used).map(Q=>({...Q,_line:N})));if(!F.length){B.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),z(B);return}Y(F,T,`Choisir ${T} joueur(s) à booster (+${k})`,N=>{N.forEach(V=>{const Q=(R[V._line]||[]).find(ie=>ie.cardId===V.cardId);Q&&(Q.boost=(Q.boost||0)+k,B.log.push({text:`⚡ +${k} sur ${Q.name}`,type:"info"}))}),B[p+"Team"]=R,z(B)})},DEBUFF_STAT:({value:k=1,count:T=1,roles:M=[],target:B="ai"},z,R)=>{const F=B==="home"?p:f,N=z[F+"Team"],V=B==="home"?"allié":"adverse",Q=Object.entries(N).filter(([ie])=>!M.length||M.includes(ie)).flatMap(([ie,te])=>te.map(Z=>({...Z,_line:ie})));if(!Q.length){z.log.push({text:`🎯 Aucun joueur ${V}`,type:"info"}),R(z);return}Y(Q,T,`Choisir ${T} joueur(s) ${V}(s) (-${k})`,ie=>{ie.forEach(te=>{const Z=(N[te._line]||[]).find(re=>re.cardId===te.cardId);Z&&(Z.boost=(Z.boost||0)-k,z.log.push({text:`🎯 -${k} sur ${Z.name}`,type:"info"}))}),z[F+"Team"]=N,R(z)})},GRAY_PLAYER:({count:k=1,roles:T=[],target:M="ai"},B,z)=>{const R=M==="home"?p:f,F=B[R+"Team"],N=M==="home"?"allié":"adverse",V=Object.entries(F).filter(([Q])=>!T.length||T.includes(Q)).flatMap(([Q,ie])=>ie.filter(te=>!te.used).map(te=>({...te,_line:Q})));if(!V.length){B.log.push({text:`❌ Aucun joueur ${N}`,type:"info"}),z(B);return}Y(V,k,`Choisir ${k} joueur(s) ${N}(s) à exclure`,Q=>{const ie="usedCardIds_"+R,te=new Set(B[ie]||[]);Q.forEach(Z=>{const re=(F[Z._line]||[]).find(se=>se.cardId===Z.cardId);re&&(re.used=!0,te.add(Z.cardId),B.log.push({text:`❌ ${re.name} exclu !`,type:"info"}))}),B[ie]=[...te],B[R+"Team"]=F,z(B)})},REVIVE_PLAYER:({count:k=1,roles:T=[]},M,B)=>{const z=M[p+"Team"],R=Object.entries(z).filter(([F])=>!T.length||T.includes(F)).flatMap(([F,N])=>N.filter(V=>V.used).map(V=>({...V,_line:F})));if(!R.length){M.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),B(M);return}Y(R,k,`Choisir ${k} joueur(s) à ressusciter`,F=>{F.forEach(N=>{const V=(z[N._line]||[]).find(Q=>Q.cardId===N.cardId);V&&(V.used=!1,M.log.push({text:`💫 ${V.name} ressuscité !`,type:"info"}))}),M[p+"Team"]=z,B(M)})},REMOVE_GOAL:({},k,T)=>{const M=f+"Score";k[M]>0?(k[M]--,k.log.push({text:"🚫 Dernier but annulé !",type:"info"})):k.log.push({text:"🚫 Aucun but à annuler",type:"info"}),T(k)},ADD_GOAL_DRAW:({},k,T)=>{k[p+"Score"]===k[f+"Score"]?(k[p+"Score"]++,k.log.push({text:"🎯 But bonus !",type:"info"})):k.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),T(k)},ADD_SUB:({value:k=1},T,M)=>{T.maxSubs=(T.maxSubs||3)+k,T.log.push({text:`🔄 +${k} remplacement(s)`,type:"info"}),M(T)},CUSTOM:({},k,T)=>T(k)};function Y(k,T,M,B){const z=document.createElement("div");z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let R=[];function F(){var V,Q;const N=k.map(ie=>{const te=ie._line||ie.job||"MIL",Z=R.find(se=>se.cardId===ie.cardId),re=Se({...ie,_evolution_bonus:0},{width:90,showStad:!0,role:te,extraNote:ie.boost||0});return`<div class="pp-item" data-cid="${ie.cardId}"
        ...
        </div>`,(V=z.querySelector("#pp-close"))==null||V.addEventListener("click",()=>z.remove()),z.querySelectorAll(".pp-item").forEach(ie=>{ie.addEventListener("click",()=>{const te=ie.dataset.cid,Z=k.find(se=>se.cardId===te),re=R.findIndex(se=>se.cardId===te);Z&&(re>-1?R.splice(re,1):R.length<T&&R.push(Z),F())})}),(Q=z.querySelector("#pp-confirm"))==null||Q.addEventListener("click",()=>{z.remove(),B(R)})}F(),document.body.appendChild(z)}async function ae(k,T){const B=(g["gcCardsFull_"+p]||[]).find(N=>N.id===k),z=(B==null?void 0:B._gcDef)||(g.gcDefs||[]).find(N=>{var V;return N.name===T||((V=N.name)==null?void 0:V.toLowerCase().trim())===(T==null?void 0:T.toLowerCase().trim())}),R=[...g["usedGc_"+p]||[],k],F={type:T,by:p,seq:(g._gcAnimSeq||0)+1};W.add(F.seq),$(T,p,async()=>{if(z!=null&&z.effect_type&&z.effect_type!=="CUSTOM"){const V=H[z.effect_type];if(V){const Q={...g};V(z.effect_params||{},Q,async ie=>{ie["usedGc_"+p]=R,ie._lastGC=F,ie._gcAnimSeq=F.seq,await I(ie)});return}}const N={...g};switch(T){case"Remplacement+":N.maxSubs=(N.maxSubs||3)+1,N.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const V=f+"Score";N[V]>0&&(N[V]--,N.log.push({text:"🚫 But annulé",type:"info"}));break}}N["usedGc_"+p]=R,N._lastGC=F,N._gcAnimSeq=F.seq,await I(N)})}function de(k,T){const M="usedCardIds_"+k,B=new Set(g[M]||[]);T.forEach(z=>B.add(z)),g[M]=[...B]}function j(){for(const k of["p1","p2"]){const T=new Set(g["usedCardIds_"+k]||[]),M=g[k+"Team"];if(M)for(const B of["GK","DEF","MIL","ATT"])(M[B]||[]).forEach(z=>{z.used=T.has(z.cardId)})}}function U(){var rn,an,sn,dn,ln,cn;if(g.phase==="reveal")return le();if(g.phase==="midfield")return ue();if(g.phase==="finished")return Ne();const k=g[p+"Team"],T=g[f+"Team"];j();const M=g[p+"Score"],B=g[f+"Score"],z=g[p+"Name"],R=g[f+"Name"],F=g.phase===p+"-attack",N=g.phase===p+"-defense",V=Array.isArray(g["selected_"+p])?g["selected_"+p]:[],Q=V.map(J=>J.cardId),ie=window.innerWidth>=700,te=g[p+"Subs"]||[],Z=g["usedSubIds_"+p]||[],re=te.filter(J=>!Z.includes(J.cardId)),se=g["gcCardsFull_"+p]||[],xe=g["usedGc_"+p]||[],ce=se.filter(J=>!xe.includes(J.id)),be=g.boostOwner===p&&!g.boostUsed,ve=[...k.MIL||[],...k.ATT||[]].filter(J=>!J.used),oe=[...T.MIL||[],...T.ATT||[]].filter(J=>!J.used),Le=!$e(T),Ge=ve.length===0&&oe.length===0&&!Le,Ie=(k.DEF||[]).filter(J=>!J.used),Ve=(k.GK||[]).filter(J=>!J.used);let Oe=[];F&&ve.length===0&&(Le?(Oe=Ie.map(J=>J.cardId),Ie.length===0&&(Oe=Oe.concat(Ve.map(J=>J.cardId)))):Ge&&(Oe=[...Ie,...Ve].map(J=>J.cardId)));function nt(J,ge,Me){var Ce,mt;const _e=J._gcDef,Dt=(_e==null?void 0:_e.name)||J.gc_type,at=(_e==null?void 0:_e.effect)||((Ce=ze[J.gc_type])==null?void 0:Ce.desc)||"",Fe=_e!=null&&_e.image_url?`/icons/${_e.image_url}`:null,Re=((mt=ze[J.gc_type])==null?void 0:mt.icon)||"⚡";return`<div class="pvp-gc-mini" data-gc-id="${J.id}" data-gc-type="${J.gc_type}" style="flex-shrink:0;cursor:pointer">
        ${tt(Dt,Fe,Re,at,{width:ge})}
      </div>`}function Ut(J,ge){return`<div id="pvp-boost-card"
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${Rt(k,g[p+"Formation"],ft,Q,ie?1300:zt(),ie?600:Gt(),Oe)}
      </div>
    </div>`,rt=g[p+"Team"],mo=(()=>{var ge,Me,_e,Dt,at;if(N&&((ge=g.pendingAttack)!=null&&ge.players)){const Fe=g.pendingAttack;let Re="";if(V.length>0){const Ce=V.map(We=>{const Kt=(rt[We._role]||[]).find(Li=>Li.cardId===We.cardId)||We,Vt=rt[We._role]||[],ai=Vt.findIndex(Li=>Li.cardId===We.cardId),Ei=st(Vt.length),Yt=ai>=0?Ei[ai]:Kt._col??1;return{...Kt,_line:We._role,_col:Yt}}),mt=oi(Ce,((Me=g.modifiers)==null?void 0:Me[p])||{},g[p+"Formation"]);Re=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
            ...
            <div style="display:flex;justify-content:center">${Je(Ce.map(We=>({...We,used:!1})),"#3a7bd5",mt.total,"defense",g[p+"Formation"])}</div>
          </div>`}return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b;text-align:center">
          ...
          <div style="display:flex;justify-content:center">${Je((Fe.players||[]).map(Ce=>({...Ce,used:!1})),"#ff6b6b",Fe.total,"attack",g[f+"Formation"])}</div>
          ${Re}
        </div>`}if(F&&((_e=g.pendingAttack)!=null&&_e.players)){const Fe=g.pendingAttack;return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #00ff88;text-align:center">
          <div style="font-size:8px;color:#00ff88;letter-spacing:2px;margin-bottom:4px;text-transform:uppercase">⚔️ VOUS ATTAQUEZ</div>
          <div style="display:flex;justify-content:center">${Je((Fe.players||[]).map(Re=>({...Re,used:!1})),"#00ff88",Fe.total,"attack",g[p+"Formation"])}</div>
        </div>`}if(F&&!((Dt=g.pendingAttack)!=null&&Dt.players)&&V.length>0){const Fe=V.map(Ce=>{const mt=(rt[Ce._role]||[]).find(Yt=>Yt.cardId===Ce.cardId)||Ce,We=["GK","DEF"].includes(Ce._role),Kt=rt[Ce._role]||[],Vt=Kt.findIndex(Yt=>Yt.cardId===Ce.cardId),ai=st(Kt.length),Ei=Vt>=0?ai[Vt]:mt._col??1;return{...mt,_line:Ce._role,_col:Ei,...We?{note_a:Math.max(1,Number(mt.note_a)||0)}:{}}}),Re=ni(Fe,((at=g.modifiers)==null?void 0:at[p])||{},g[p+"Formation"]);return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #FFD700;text-align:center">
          ...
          <div style="display:flex;justify-content:center">${Je(Fe.map(Ce=>({...Ce,used:!1})),"#FFD700",Re.total,"attack",g[p+"Formation"])}</div>
        </div>`}const J=(g.log||[]).slice(-1)[0];return J?'<div style="padding:2px 4px">'+eo(J)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3);text-align:center">⏳ Match en cours...</div>'})(),on=`
      <div style="display:flex;align-items:center;padding:8px 10px;background:rgba(0,0,0,0.5);gap:6px;flex-shrink:0">
      ...
      <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
        ...
        <div id="mobile-play-area" style="flex:1;min-height:0;display:flex;overflow:hidden">
          <div id="match-field" style="flex:1;min-width:0;min-height:0;overflow:hidden">
            <div class="terrain-wrapper" style="width:100%;height:100%;overflow:hidden">
              ${Rt(k,g[p+"Formation"],ft,Q,zt(),Gt(),Oe)}
            </div>
          </div>
        </div>
        <div id="mobile-action-bar" style="position:absolute;left:0;right:0;bottom:0;z-index:20;background:rgba(0,0,0,0.55);padding:6px 8px 8px;display:flex;flex-direction:column;gap:6px;box-shadow:0 -4px 16px rgba(0,0,0,0.5)">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      </div>`,document.body.appendChild(M),(B=M.querySelector("#bp-close"))==null||B.addEventListener("click",()=>M.remove()),M.querySelectorAll(".bp-item").forEach(z=>{z.addEventListener("click",async()=>{const R=z.dataset.cid,F=T.find(V=>V.cardId===R);if(!F)return;const N=(k[F._line]||[]).find(V=>V.cardId===R);N&&(N.boost=(N.boost||0)+g.boostValue),M.remove(),await I({[p+"Team"]:k,boostUsed:!0})})})}function P(k=null){var re,se;if(!(g.phase===p+"-attack")){m("Remplacement uniquement avant votre attaque","warning");return}const M=g[p+"Team"],B=g["usedSubIds_"+p]||[],z=g.maxSubs||3;if(B.length>=z){m(`Maximum ${z} remplacements atteint`,"warning");return}const R=Object.entries(M).flatMap(([xe,ce])=>(ce||[]).filter(be=>be.used).map(be=>({...be,_line:xe}))),F=(g[p+"Subs"]||[]).filter(xe=>!B.includes(xe.cardId));if(!R.length){m("Aucun joueur utilisé à remplacer","warning");return}if(!F.length){m("Aucun remplaçant disponible","warning");return}let N=Math.max(0,R.findIndex(xe=>xe.cardId===k));const V=((re=R[N])==null?void 0:re._line)||((se=R[N])==null?void 0:se.job);let Q=Math.max(0,F.findIndex(xe=>xe.job===V)),ie=!1;const te=document.createElement("div");te.id="pvp-sub-overlay",te.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function Z(){var Ge,Ie,Ve,Oe,nt,Ut;const xe=R[N],ce=F[Q],be=Math.min(130,Math.round((window.innerWidth-90)/2)),ve=Math.round(be*1.35),oe=De=>`background:rgba(255,255,255,0.12);border:none;color:${De?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${De?"default":"pointer"};flex-shrink:0`;te.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="font-size:18px;font-weight:900;color:#ff6b6b">${g[f+"Name"]}</div>
      <div style="width:min(90vw,420px)">${_i(g[f+"Team"],g[f+"Formation"],null,[],zt(),Gt())}</div>
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(k),(T=k.querySelector("#pvp-opp-close"))==null||T.addEventListener("click",()=>k.remove())}function K(){var M;const k=g.log||[],T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column",T.innerHTML=`
      ...
      </div>`,document.body.appendChild(T),(M=T.querySelector("#pvp-hist-close"))==null||M.addEventListener("click",()=>T.remove())}async function pe(){if(g.phase!==p+"-attack")return;const k=p==="p1"?"p2":"p1",T=(g.round||0)+1,M=[...g.log||[]];M.push({type:"info",text:`⏭️ ${g[p+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const B=Ee(g),z=pt(k),R=B||!z?"finished":k+"-attack";await I({["selected_"+p]:[],modifiers:{...g.modifiers,[p]:{}},pendingAttack:null,phase:R,attacker:k,round:T,log:M}),R==="finished"&&await qe(g)}function ne(k){const T=(k.GK||[]).some(B=>!B.used),M=["DEF","MIL","ATT"].some(B=>(k[B]||[]).some(z=>!z.used));return T&&!M}function me(k){return(k.p1Score||0)!==(k.p2Score||0)?null:ne(k.p1Team)&&!$e(k.p2Team)?{side:"p1",gk:(k.p1Team.GK||[]).find(T=>!T.used)}:ne(k.p2Team)&&!$e(k.p1Team)?{side:"p2",gk:(k.p2Team.GK||[]).find(T=>!T.used)}:null}async function we(){const k=document.createElement("div");k.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:950;display:flex;align-items:center;justify-content:center;overflow:hidden;text-align:center;padding:24px",document.body.appendChild(k);const T=(M,B)=>new Promise(z=>{k.innerHTML=`<div style="font-size:32px;font-weight:900;color:${B};letter-spacing:2px;animation:lcFade 1.4s ease both">${M}</div>
      <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(z,1400)});await T("⚽ DERNIER CORNER","#FFD700"),await T("🧤 LE GARDIEN MONTE !","#4fc3f7"),k.remove()}async function he(k,T){const{side:M,gk:B}=k;if(!B)return;const z=M+"Score",R=(T[z]||0)+1;B.used=!0;const F=[...T.log||[]];F.push({type:"duel",isGoal:!0,homeScored:M===p,homePlayers:M==="p1"?[Ft(B)]:[],aiPlayers:M==="p2"?[Ft(B)]:[],text:`⚽ DERNIER CORNER — Le gardien de ${T[M+"Name"]} marque !`});const N=(T.round||0)+1,V=M==="p1"?"p2":"p1",Q={...T,[z]:R},ie=Ee(Q);q.add(N),await we();const te=M===p?R:T[p+"Score"]||0,Z=M===p?T[f+"Score"]||0:R;Qe([Ft(B)],te,Z,M===p,async()=>{await I({[z]:R,log:F,round:N,pendingAttack:null,phase:ie?"finished":V+"-attack",attacker:V,["selected_"+p]:[],modifiers:{...g.modifiers,[p]:{}}}),ie&&await qe({...g,[z]:R})})}async function Te(){const k=g[p+"Team"],T=!["GK","DEF","MIL","ATT"].some(R=>(g[f+"Team"][R]||[]).some(F=>!F.used)),M=(g["selected_"+p]||[]).map(R=>{const F=(k[R._role]||[]).find(se=>se.cardId===R.cardId)||R,N=["GK","DEF"].includes(R._role),V=k[R._role]||[],Q=V.findIndex(se=>se.cardId===R.cardId),ie=st(V.length),te=Q>=0?ie[Q]:F._col??1,Z=g.stadiumDef||g.homeStadiumDef||null,re=F.stadiumBonus||Z&&(Z.club_id&&String(F.club_id)===String(Z.club_id)||Z.country_code&&F.country_code===Z.country_code)||!1;return{...F,_line:R._role,_col:te,stadiumBonus:re,...N?{note_a:Math.max(1,Number(F.note_a)||0)}:{}}});if(!M.length)return;const B=ni(M,g.modifiers[p]||{},g[p+"Formation"]);de(p,M.map(R=>R.cardId)),M.forEach(R=>{const F=(k[R._role]||[]).find(N=>N.cardId===R.cardId);F&&(F.used=!0)}),g["selected_"+p]=[],U();const z=[...g.log||[]];if(T){if(M.length===1&&(M[0]._line==="GK"||M[0].job==="GK")&&(g.p1Score||0)===(g.p2Score||0)){await he({side:p,gk:M[0]},{...g,[p+"Team"]:k,log:z});return}const F=(g[p+"Score"]||0)+1,N=M.map(Z=>Ft(Z));z.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:N,homeTotal:B.total,aiTotal:0});const V=(g.round||0)+1,Q=p==="p1"?"p2":"p1",ie={...g,[p+"Team"]:k,[p+"Score"]:F},te=Ee(ie);q.add(V),Qe(N,F,g[f+"Score"]||0,!0,async()=>{await I({[p+"Team"]:k,[p+"Score"]:F,["selected_"+p]:[],modifiers:{...g.modifiers,[p]:{}},pendingAttack:null,phase:te?"finished":Q+"-attack",attacker:Q,round:V,log:z}),te&&await qe({...g,[p+"Score"]:F})});return}z.push({type:"pending",text:`⚔️ ${g[p+"Name"]} attaque (${B.total})`}),await I({[p+"Team"]:k,[f+"Team"]:g[f+"Team"],pendingAttack:{...B,players:M,side:p},["selected_"+p]:[],modifiers:{...g.modifiers,[p]:{}},phase:f+"-defense",log:z})}async function Ae(){const k=g[p+"Team"],T=(g["selected_"+p]||[]).map(ce=>{const be=(k[ce._role]||[]).find(Ie=>Ie.cardId===ce.cardId)||ce,ve=k[ce._role]||[],oe=ve.findIndex(Ie=>Ie.cardId===ce.cardId),Le=st(ve.length),Ge=oe>=0?Le[oe]:be._col??1;return{...be,_line:ce._role,_col:Ge}}),M=oi(T,g.modifiers[p]||{},g[p+"Formation"]);de(p,T.map(ce=>ce.cardId)),T.forEach(ce=>{const be=(k[ce._role]||[]).find(ve=>ve.cardId===ce.cardId);be&&(be.used=!0)}),g["selected_"+p]=[],U();const B=Qi(g.pendingAttack.total,M.total,g.modifiers[p]||{}),z=g.pendingAttack.side,R=B==="attack"||(B==null?void 0:B.goal),F=z==="p1"?"p2":"p1",N=(g.round||0)+1,V=(g.pendingAttack.players||[]).map(ce=>Ft(ce)),Q=[...g.log||[]];Q.push({type:"duel",isGoal:R,homeScored:R&&z===p,text:R?`⚽ BUT de ${g[z+"Name"]} ! (${g.pendingAttack.total} vs ${M.total})`:`✋ Attaque stoppée (${g.pendingAttack.total} vs ${M.total})`,homePlayers:V,aiPlayers:T.map(ce=>Ft(ce)),homeTotal:g.pendingAttack.total,aiTotal:M.total});const ie=R?(g[z+"Score"]||0)+1:g[z+"Score"]||0,te={...g,[p+"Team"]:k,[z+"Score"]:ie},Z=Ee(te),re=Z?"finished":F+"-attack",se=!Z&&!R?me(te):null,xe=async()=>{if(se){await he(se,te);return}await I({[p+"Team"]:k,[f+"Team"]:g[f+"Team"],[z+"Score"]:ie,["selected_"+p]:[],modifiers:{...g.modifiers,[p]:{}},pendingAttack:null,phase:re,attacker:F,round:N,log:Q}),(re==="finished"||Z)&&await qe({...g,[z+"Score"]:ie})};if(R){const ce=z===p,be=ce?ie:g[p+"Score"]||0,ve=ce?g[f+"Score"]||0:ie;q.add(N),Qe(V,be,ve,ce,xe)}else await xe()}function Be(k){return["MIL","ATT"].some(T=>(k[T]||[]).some(M=>!M.used))}function $e(k){return["GK","DEF","MIL","ATT"].some(T=>(k[T]||[]).some(M=>!M.used))}function kt(k){return $e(k)&&!Be(k)}function pt(k){const T=g[k+"Team"],M=g[(k==="p1"?"p2":"p1")+"Team"];return!!(Be(T)||kt(T)&&!$e(M)||kt(T)&&!Be(M)&&$e(M))}function Ee(k){return!$e(k.p1Team)&&!$e(k.p2Team)}function je(k){const T=k.p1Score||0,M=k.p2Score||0;return T===M?null:T>M?h.home_id:h.away_id}async function qe(k){It(),Ue();try{const T=je(k),M=T?h.home_id===T?h.away_id:h.home_id:null,B=k.p1Score||0,z=k.p2Score||0,{data:R,error:F}=await v.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:T,p_home_score:B,p_away_score:z,p_forfeit:!1});if(F&&console.error("[PvP] finish_pvp_match:",F),c)try{await v.rpc("check_and_finish_mini_league",{p_league_id:c})}catch(N){console.error("[PvP] check_and_finish_mini_league:",N)}T&&M&&Yr(T,M).catch(()=>{});try{const{data:N}=await v.rpc("apply_match_rewards",{p_match_id:i});N!=null&&N.success&&!(N!=null&&N.skipped)&&typeof e.refreshProfile=="function"&&await e.refreshProfile()}catch(N){console.error("[PvP] apply_match_rewards:",N)}if(typeof l=="function")try{await l({homeScore:B,awayScore:z,winnerId:T,homeId:h.home_id,awayId:h.away_id,matchId:i})}catch(N){console.error("[PvP] onMatchEnd:",N)}}catch(T){console.error("[PvP] finishMatch:",T)}}function Ne(){var z;const k=g[p+"Score"],T=g[f+"Score"],M=k>T,B=k===T;dt(t),t.innerHTML=`
    ...
    </div>`,(z=document.getElementById("pvp-home"))==null||z.addEventListener("click",()=>{try{v.removeChannel(G)}catch{}it(t),c?x("mini-league",{openLeagueId:c}):x("home")})}U()}async function Ni(t,e,i=!1){var s,d;const{state:n,navigate:r,toast:o}=e,a=i&&((d=(s=e==null?void 0:e.state)==null?void 0:s.params)==null?void 0:d.rankedData)||null;await ki(t,e,i?"ranked":"random",async({deckId:l,formation:c,starters:u,subsRaw:b,gcCardsEnriched:x,gcDefs:m,stadiumDef:p})=>{dt(t);const f=async y=>{await Jr(t,e,l,c,u,b,y||[],m,p,i,a)};if(!(x!=null&&x.length)){await f([]);return}wi(t,x,f)})}async function Xr(t,e,i){return Wr(t,e,i)}async function Jr(t,e,i,n,r,o,a=[],s=[],d=null,l=!1,c=null){var ee,G;const{state:u,navigate:b,toast:x}=e,m=(c==null?void 0:c.mmr)??((ee=u.profile)==null?void 0:ee.mmr)??1e3,p=l?vi(m):null,f=l?"linear-gradient(135deg, #1a0a2e 0%, #2d1060 50%, #1a0a2e 100%)":"linear-gradient(135deg, #0a1a2e 0%, #0d3d1e 100%)",y=l?(p==null?void 0:p.color)||"#D4A017":"#FFD700";let w=!1,h=null,L=null;t.innerHTML=`
    ...
    </div>`;const g=async()=>{w=!0,h&&(h.unsubscribe(),h=null),L&&(clearInterval(L),L=null);try{await v.rpc("cancel_matchmaking",{p_user_id:u.profile.id})}catch(I){console.error("[Matchmaking] cancel error:",I)}};(G=document.getElementById("cancel-mm"))==null||G.addEventListener("click",async()=>{await g(),it(t),b("home")});const A=async(I,_)=>{w||(w=!0,h&&(h.unsubscribe(),h=null),L&&(clearInterval(L),L=null),await jt(t,e,I,_,{myGC:a,gcDefs:s,stadiumDef:d,isRanked:l,rankedData:c,onMatchEnd:l?Qr:null}))},E=u.user.id,{data:S,error:q}=l?await v.rpc("try_matchmake_ranked",{p_user_id:E,p_deck_id:i,p_mmr:m}):await v.rpc("try_matchmake",{p_user_id:E,p_deck_id:i});if(q||!(S!=null&&S.success)){console.error("[Matchmaking] try_matchmake error:",q||S),x("Erreur de matchmaking","error"),it(t),b("home");return}if(S.matched){await A(S.match_id,!1);return}const W=async()=>{if(w)return;const{data:I}=await v.from("matchmaking_queue").select("status, match_id").eq("user_id",E).maybeSingle();(I==null?void 0:I.status)==="matched"&&I.match_id&&await A(I.match_id,!0)};h=v.channel(`mm_${E}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${E}`},I=>{const _=I.new;_.status==="matched"&&_.match_id&&A(_.match_id,!0)}).subscribe(),L=setInterval(W,3e3),setTimeout(()=>{w||g()},12e4)}async function Qr(t,e){const{state:i,toast:n}=e,{winnerId:r,homeId:o,awayId:a,homeScore:s,awayScore:d,matchId:l}=t,c=i.user.id,u=o===c,b=r===c,{data:x}=await v.from("users").select("mmr, mmr_rd, mmr_v").eq("id",c).single(),{data:m}=await v.from("users").select("mmr, mmr_rd, mmr_v").eq("id",u?a:o).single();if(x&&m){const p=go(x.mmr,x.mmr_rd,x.mmr_v,m.mmr,m.mmr_rd,b?1:0);await v.from("users").update({mmr:p.mmr,mmr_rd:p.rd,mmr_v:p.v,mmr_wins:b?v.sql`mmr_wins + 1`:void 0,mmr_losses:b?void 0:v.sql`mmr_losses + 1`}).eq("id",c),n(b?`+MMR ↑ ${Math.round(p.mmr-x.mmr)}`:`-MMR ↓ ${Math.round(x.mmr-p.mmr)}`,b?"success":"error",4e3)}}const ro=Object.freeze(Object.defineProperty({__proto__:null,renderMatchRandom:Ni,resumePvpMatch:Xr},Symbol.toStringTag,{value:"Module"}));async function Zr(t,e){const{state:i,navigate:n,toast:r}=e,o=i.params||{},a=o.friendId||null,s=o.friendName||"Ami",d=!!o.isAccepting;await ki(t,e,"friend",async({deckId:l,formation:c,starters:u,subsRaw:b,gcCardsEnriched:x,gcDefs:m,stadiumDef:p})=>{dt(t);const f=async y=>{const w=y||[];d?await ta(t,e,l,w,m,p):await ea(t,e,l,w,m,p,a,s)};if(!(x!=null&&x.length)){await f([]);return}wi(t,x,f)})}async function ea(t,e,i,n,r,o,a,s){var w;const{state:d,navigate:l,toast:c}=e,u=d.user.id,{data:b,error:x}=await v.from("friend_match_invites").insert({inviter_id:u,invitee_id:a,friend_id:a,status:"pending",inviter_deck_id:i,expires_at:new Date(Date.now()+2*60*1e3).toISOString()}).select("id").single();if(x||!b){console.error("[Friend] Erreur création invitation:",x),c("Impossible de créer l'invitation","error"),l("home");return}t.innerHTML=`
    ...
    </div>`;let m=!1,p=null;const f=async()=>{p&&(p.unsubscribe(),p=null)};(w=document.getElementById("cancel-friend"))==null||w.addEventListener("click",async()=>{await f(),await v.from("friend_match_invites").update({status:"declined"}).eq("id",b.id),it(t),l("home")}),p=v.channel(`friend_invite_${b.id}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"friend_match_invites",filter:`id=eq.${b.id}`},async({new:h})=>{m||h.status!=="accepted"||!h.match_id||(m=!0,await f(),await jt(t,e,h.match_id,!0,{myGC:n,gcDefs:r,stadiumDef:o}))}).subscribe();const y=setInterval(async()=>{if(m)return;const{data:h}=await v.from("friend_match_invites").select("status, match_id").eq("id",b.id).single();(h==null?void 0:h.status)==="accepted"&&h.match_id&&(m=!0,clearInterval(y),await f(),await jt(t,e,h.match_id,!0,{myGC:n,gcDefs:r,stadiumDef:o}))},3e3);setTimeout(async()=>{m||(clearInterval(y),await f(),c("Invitation expirée","info"),l("home"))},12e4)}async function ta(t,e,i,n,r,o){const{state:a,navigate:s,toast:d}=e,l=a.user.id,{data:c}=await v.from("friend_match_invites").select("id, inviter_id").eq("invitee_id",l).eq("status","pending").order("created_at",{ascending:!1}).limit(1).maybeSingle();if(!c){d("Aucune invitation en attente","error"),s("home");return}const{data:u,error:b}=await v.rpc("accept_friend_invite",{p_invite_id:c.id,p_invitee_deck_id:i});if(b||!(u!=null&&u.success)){console.error("[Friend] Erreur accept_friend_invite:",b||u),d((u==null?void 0:u.error)||"Impossible de rejoindre le match","error"),s("home");return}await jt(t,e,u.match_id,!1,{myGC:n,gcDefs:r,stadiumDef:o})}const ao="#1A6B3C",lt="#D4A017",Xt="var(--tile-bg)",Jt="var(--tile-border)",Tt="var(--divider)",Xe="var(--tile-fg-on-page)",Lt="var(--tile-fg-dim)",Ze="var(--tile-fg-faint)",Zt="var(--nav-fg,#fff)",so="rgba(255,255,255,0.62)",ia="rgba(255,255,255,0.4)";async function na(t,e){var n;t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const i=(n=e.state.params)==null?void 0:n.openLeagueId;if(i){e.state.params.openLeagueId=null,await ct(t,e,i);return}await et(t,e)}async function et(t,e,i="waiting"){var h,L;const{state:n}=e,r=n.profile.id,{data:o}=await v.from("mini_league_members").select("league_id, prize_amount, prize_claimed").eq("user_id",r),a=(o||[]).map(g=>g.league_id),s={};(o||[]).forEach(g=>{s[g.league_id]={amount:g.prize_amount,claimed:g.prize_claimed}});const{data:d,error:l}=await v.from("mini_leagues").select("*, mini_league_members(count)").eq("status","waiting").eq("is_archived",!1).order("created_at",{ascending:!1}).limit(30),c=l?(await v.from("mini_leagues").select("*, mini_league_members(count)").eq("status","waiting").order("created_at",{ascending:!1}).limit(30)).data||[]:d||[],b=(a.length?await v.from("mini_leagues").select("*, mini_league_members(count)").in("id",a).order("created_at",{ascending:!1}):{data:[]}).data||[],x=b.filter(g=>g.status==="waiting"&&!g.is_archived),m=b.filter(g=>g.status==="active"&&!g.is_archived),p=b.filter(g=>g.is_archived||g.status==="finished");let f=p;if(p.length){const{data:g}=await v.from("mini_league_matches").select("league_id").in("league_id",p.map(E=>E.id)).or(`home_id.eq.${r},away_id.eq.${r}`).not("status","eq","bye"),A=new Set((g||[]).map(E=>E.league_id));f=p.filter(E=>A.has(E.id))}const y=c.filter(g=>!a.includes(g.id)),w=[{key:"waiting",label:"🟡 En attente",count:x.length+y.length},{key:"active",label:"🟢 En cours",count:m.length},{key:"archived",label:"📁 Archivées",count:f.length}];t.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="display:flex;background:var(--nav-bg,#0d1a0f);border-bottom:1px solid ${Tt}">
      ${w.map(g=>`<button class="ml-tab" data-tab="${g.key}" style="flex:1;padding:11px 4px;border:none;border-bottom:2px solid ${i===g.key?ao:"transparent"};background:none;font-size:12px;font-weight:${i===g.key?"900":"600"};color:${i===g.key?"#4ade80":ia};cursor:pointer">${g.label}${g.count?` (${g.count})`:""}</button>`).join("")}
    </div>
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:10px">
      ${i==="waiting"?oa(x,y,r):i==="active"?ra(m,r):aa(f,r,s)}
    </div>
  </div>`,(h=document.getElementById("ml-create-btn"))==null||h.addEventListener("click",()=>sa(t,e)),(L=document.getElementById("ml-refresh-list"))==null||L.addEventListener("click",()=>et(t,e,i)),t.querySelectorAll(".ml-tab").forEach(g=>g.addEventListener("click",()=>et(t,e,g.dataset.tab))),t.querySelectorAll("[data-league-id]").forEach(g=>g.addEventListener("click",()=>ct(t,e,g.dataset.leagueId))),t.querySelectorAll("[data-join]").forEach(g=>g.addEventListener("click",A=>{A.stopPropagation(),lo(t,e,g.dataset.join,g.dataset.type)})),t.querySelectorAll("[data-delete]").forEach(g=>g.addEventListener("click",A=>{A.stopPropagation(),Gi(t,e,g.dataset.delete,g.dataset.name,i)})),t.querySelectorAll("[data-claim-league]").forEach(g=>g.addEventListener("click",async A=>{A.stopPropagation(),g.disabled=!0,g.textContent="...";const{data:E,error:S}=await v.rpc("claim_mini_league_prize",{p_league_id:g.dataset.claimLeague,p_user_id:n.profile.id});if(S||!(E!=null&&E.success)){e.toast((E==null?void 0:E.error)||"Erreur lors de la récupération","error"),et(t,e,i);return}if(!E.already_claimed){n.profile.credits=(n.profile.credits||0)+E.prize;const q=document.getElementById("nav-credits");q&&(q.textContent=`💰 ${n.profile.credits.toLocaleString("fr")}`),e.toast(`💰 +${E.prize.toLocaleString("fr")} cr. ajoutés !`,"success")}et(t,e,i)}))}function hi(t,e,i=!1,n=null){var d,l;const r=t.creator_id===e,o=t.pot||0,a=((l=(d=t.mini_league_members)==null?void 0:d[0])==null?void 0:l.count)||0,s=n&&n.amount>0&&!n.claimed;return`<div data-league-id="${t.id}" style="background:${Xt};border:1px solid ${s?"rgba(212,160,23,0.5)":Jt};border-radius:12px;padding:14px 16px;cursor:pointer;margin-bottom:8px">
    ...
    </div></div>`,document.body.appendChild(i),i.querySelector("#ml-form-close").addEventListener("click",()=>i.remove()),i.querySelectorAll('input[name="ml-type"]').forEach(n=>n.addEventListener("change",()=>{document.getElementById("ml-pwd-block").style.display=n.value==="private"?"block":"none"})),i.querySelector("#ml-create-confirm").addEventListener("click",async()=>{var m,p,f,y;const{toast:n}=e,r=document.getElementById("ml-name").value.trim(),o=parseInt(document.getElementById("ml-max").value)||6,a=parseInt(document.getElementById("ml-fee").value)||500,s=((m=i.querySelector('input[name="ml-type"]:checked'))==null?void 0:m.value)||"public",d=((p=i.querySelector('input[name="ml-mode"]:checked'))==null?void 0:p.value)||"aller",l=((y=(f=document.getElementById("ml-pwd"))==null?void 0:f.value)==null?void 0:y.trim())||null;if(!r){n("Le nom est obligatoire","error");return}if(a<100){n("Mise minimum : 100 crédits","error");return}if(s==="private"&&!l){n("Mot de passe requis","error");return}const{data:c,error:u}=await v.from("mini_leagues").insert({name:r,creator_id:e.state.profile.id,type:s,password:l,mode:d,max_players:o,entry_fee:a}).select().single();if(u){n("Erreur : "+u.message,"error");return}const{data:b}=await v.from("users").select("credits").eq("id",e.state.profile.id).single();if(((b==null?void 0:b.credits)||0)<a){await v.from("mini_leagues").delete().eq("id",c.id),n(`Crédits insuffisants pour la mise (${a.toLocaleString("fr")} cr.)`,"error");return}await v.from("users").update({credits:b.credits-a}).eq("id",e.state.profile.id),await v.from("mini_leagues").update({pot:a}).eq("id",c.id),await v.from("mini_league_members").insert({league_id:c.id,user_id:e.state.profile.id}),e.state.profile&&(e.state.profile.credits=b.credits-a);const x=document.getElementById("nav-credits");x&&(x.textContent=`💰 ${(e.state.profile.credits||0).toLocaleString("fr")}`),i.remove(),n(`Mini League créée ! ${a.toLocaleString("fr")} cr. déduits.`,"success"),ct(t,e,c.id)})}function da(){return new Promise(t=>{const e=document.createElement("div");e.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9500;display:flex;align-items:center;justify-content:center;padding:20px",e.innerHTML=`
    ...
    </div>`,document.body.appendChild(x),x.querySelector("#lv-ok").onclick=()=>{x.remove(),b(!0)},x.querySelector("#lv-cancel").onclick=()=>{x.remove(),b(!1)}}))return;const{data:d,error:l}=await v.rpc("leave_mini_league",{p_league_id:i,p_user_id:a});if(l||!(d!=null&&d.success)){r((d==null?void 0:d.error)||"Erreur lors de la désinscription","error");return}const c=d.refund||n;o.profile&&(o.profile.credits=(o.profile.credits||0)+c);const u=document.getElementById("nav-credits");u&&(u.textContent=`💰 ${(o.profile.credits||0).toLocaleString("fr")}`),r(`↩️ Désinscrit · +${c.toLocaleString("fr")} cr. remboursés`,"success"),et(t,e,"waiting")}async function Gi(t,e,i,n,r){const{toast:o}=e;if(!confirm(`Supprimer définitivement "${n}" et tous ses matchs/membres ? Action irréversible.`))return;await v.from("mini_league_matches").delete().eq("league_id",i),await v.from("mini_league_members").delete().eq("league_id",i);const{error:a}=await v.from("mini_leagues").delete().eq("id",i);if(a){o("Erreur suppression ("+a.message+")","error");return}o("Mini League supprimée avec succès","success"),et(t,e,r)}async function ca(t,e,i){await v.from("mini_leagues").update({is_archived:!0}).eq("id",i),e.toast("Mini League archivée","success"),et(t,e,"archived")}async function ct(t,e,i){var L,g,A,E,S,q,W,ee,G;const{state:n,toast:r}=e,o=n.profile.id,[{data:a},{data:s},{data:d}]=await Promise.all([v.from("mini_leagues").select("*").eq("id",i).single(),v.from("mini_league_members").select("*, user:users(id,pseudo,club_name,club_color1,club_color2)").eq("league_id",i),v.from("mini_league_matches").select("*").eq("league_id",i).order("matchday").order("created_at")]);if(!a){r("Introuvable","error"),await et(t,e);return}const l=(s||[]).some(I=>I.user_id===o),c=a.creator_id===o,u=(s||[]).map(I=>I.user).filter(Boolean),b=ga(u,d||[]),x=(d||[]).filter(I=>I.matchday===a.current_day),m=a.pot||0,p=a.entry_fee||100,f=x.length>0&&x.every(I=>I.status==="finished"||I.status==="bye"),y=a.current_day>=a.total_days,w=(s||[]).find(I=>I.user_id===o);t.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:${Xt};border:1px solid ${Jt};border-radius:12px;padding:16px">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>${b.map((I,_)=>{const H=m>0&&a.status==="finished"?_===0?Math.floor(m*.7):_===1?Math.floor(m*.2):_===2?Math.floor(m*.1):0:0;return`<tr style="border-bottom:1px solid ${Tt};${I.userId===o?"background:rgba(74,222,128,0.08);":""}">
              ...
              ${m>0&&a.status==="finished"?`<td style="text-align:right;font-weight:700;color:${lt}">${H?H.toLocaleString("fr")+" cr.":"—"}</td>`:""}
            </tr>`}).join("")}</tbody>
        </table>
      </div>`:""}
      ...
      <div style="background:${Xt};border:1px solid ${Jt};border-radius:12px;padding:16px">
        ...
        ${Array.from({length:Math.max(0,a.status==="active"?a.current_day-1:a.current_day)},(I,_)=>_+1).reverse().map(I=>{const _=(d||[]).filter(H=>H.matchday===I);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:${Ze};margin-bottom:6px">Journée ${I}</div>${_.map(H=>Ln(H,u,o,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
    ...
    </div>`;let c=!1;const u=async f=>{if(!c){if(c=!0,clearInterval(m),x)try{x.unsubscribe()}catch{}f&&await jt(t,e,f,r,{mlLeagueId:n,mlMatchId:i,myGC:o||[],gcDefs:a,stadiumDef:s}),d()}};(p=document.getElementById("wait-back"))==null||p.addEventListener("click",()=>{if(c=!0,clearInterval(m),x)try{x.unsubscribe()}catch{}l("mini-league",{openLeagueId:n}),d()});const b=async()=>{if(c)return;const{data:f}=await v.from("mini_league_matches").select("match_id").eq("id",i).single();f!=null&&f.match_id&&u(f.match_id)},x=v.channel(`ml_match_${i}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"mini_league_matches",filter:`id=eq.${i}`},f=>{var y;(y=f.new)!=null&&y.match_id&&u(f.new.match_id)}).subscribe(),m=setInterval(b,3e3);b()})}const ba="/",ya=[{emoji:"⚽",title:"Bienvenue dans Manager Wars !",color:"#1A6B3C",content:`<p>Tu es désormais un <strong>manager de football</strong> virtuel.</p>
    ...
    <p style="margin-top:12px;font-size:13px;color:#888">Tu peux revoir ce tutoriel depuis les paramètres à tout moment.</p>`}];function ha(t,e,i){let n=0;const r=document.createElement("div");r.id="tutorial-overlay",r.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.65);z-index:9900;display:flex;align-items:center;justify-content:center;padding:16px";const o=()=>{var c,u,b;const s=e[n],d=n===e.length-1,l=Math.round((n+1)/e.length*100);r.innerHTML=`
    ...
    `,r.querySelectorAll("ul,ol").forEach(x=>{x.style.paddingLeft="20px",x.style.marginTop="6px",x.style.marginBottom="6px"}),r.querySelectorAll("li").forEach(x=>{x.style.marginBottom="4px"}),r.querySelectorAll("p").forEach(x=>{x.style.marginBottom="8px"}),(c=r.querySelector("#tuto-prev"))==null||c.addEventListener("click",()=>{n--,o()}),(u=r.querySelector("#tuto-next"))==null||u.addEventListener("click",()=>{d?a():(n++,o())}),(b=r.querySelector("#tuto-skip"))==null||b.addEventListener("click",()=>{confirm("Passer le tutoriel ? Tu pourras le revoir plus tard depuis les paramètres.")&&a()})},a=async()=>{r.remove(),t!=null&&t.id&&await v.from("users").update({tutorial_done:!0}).eq("id",t.id),i==null||i()};document.body.appendChild(r),o()}async function va(t,e,i){if(!t||t.tutorial_done)return;let n=[];const{data:r,error:o}=await v.rpc("get_tutorial_steps");if(!o&&(r==null?void 0:r.length)>0)n=r,console.log(`[Tutorial] RPC OK → ${n.length} étapes`);else{const{data:s,error:d}=await v.from("tutorial_steps").select("*").eq("is_active",!0).order("step_order");!d&&(s==null?void 0:s.length)>0?(n=s,console.log(`[Tutorial] Direct OK → ${n.length} étapes`)):(console.warn(`[Tutorial] Aucune étape DB (RPC: ${o==null?void 0:o.message}, Direct: ${d==null?void 0:d.message})`),i&&i("[Tutorial] DB vide ou inaccessible — tuto local utilisé","warning",5e3))}const a=n.length>0?n.map(s=>({emoji:s.emoji,title:s.title,color:s.color,content:s.content,image_url:s.image_url||null})):ya;ha(t,a,()=>e("boosters"))}const In={GK:"#111111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function ui(t,e,i=0){return e?(Number(e==="GK"?t.note_g:e==="DEF"?t.note_d:e==="MIL"?t.note_m:t.note_a)||0)+(e===t.job||e===t.job2?i:0):0}async function wa(t,e){t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',await nn(t,e)}async function nn(t,e){const{state:i,toast:n}=e,{data:r}=await v.from("market_listings").select(`id, price, status, listed_at, seller_id,
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
  ...
  </div>`;let d="buy";const l=()=>{var p,f,y,w,h,L,g;return{name:(((p=document.getElementById("flt-name"))==null?void 0:p.value)||"").toLowerCase().trim(),club:(((f=document.getElementById("flt-club"))==null?void 0:f.value)||"").toLowerCase().trim(),country:(((y=document.getElementById("flt-country"))==null?void 0:y.value)||"").toLowerCase().trim(),job:((w=document.getElementById("flt-job"))==null?void 0:w.value)||"",rarity:((h=document.getElementById("flt-rarity"))==null?void 0:h.value)||"",note1:parseInt((L=document.getElementById("flt-note1"))==null?void 0:L.value)||0,note2:parseInt((g=document.getElementById("flt-note2"))==null?void 0:g.value)||0}};function c(p){const f=l();return p.filter(y=>{var q,W,ee;const w=(q=y.card)==null?void 0:q.player;if(!w)return!1;const h=`${w.firstname} ${w.surname_real}`.toLowerCase(),L=(((W=w.clubs)==null?void 0:W.encoded_name)||"").toLowerCase(),g=(w.country_code||"").toLowerCase(),A=((ee=y.card)==null?void 0:ee.evolution_bonus)||0,E=ui(w,w.job,A),S=w.job2?ui(w,w.job2,A):0;return!(f.name&&!h.includes(f.name)||f.club&&!L.includes(f.club)||f.country&&!g.includes(f.country)||f.job&&w.job!==f.job||f.rarity&&w.rarity!==f.rarity||f.note1&&E<f.note1||f.note2&&S<f.note2)})}function u(p){var L,g,A;const f=(L=p.card)==null?void 0:L.player;if(!f)return"";const y=((g=p.card)==null?void 0:g.evolution_bonus)||0,w=(i.profile.credits||0)>=p.price;return`<div class="mkt-buy-tile">
    ...
    </div>`}function b(p){var S,q,W,ee;const f=(S=p.card)==null?void 0:S.player;if(!f)return"";const y=((q=p.card)==null?void 0:q.evolution_bonus)||0,w=ui(f,f.job,y),h=f.job2?ui(f,f.job2,y):0,L=p.status==="sold",g=f.country_code?`https://flagsapi.com/${f.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,A=In[f.job]||"#bbb",E=f.job2?In[f.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${L?"opacity:0.7":""}">
  ...
  <div class="page">
    ...
    </div>`,(A=document.getElementById("ranked-back"))==null||A.addEventListener("click",()=>n("home"));return}const d=s.mmr??1e3,l=s.mmr_deviation??350,c=s.mmr_volatility??.06,u=s.placement_matches??0,b=u<10,x=Math.max(0,10-u),m=vi(d),p=zn(d),f=Ti.findIndex(W=>W.id===m.id),y=Ti[f+1]||null,w={bronze:"linear-gradient(160deg,#3d1c00,#7a3e00)",silver:"linear-gradient(160deg,#1a1a2e,#3a3a5e)",gold:"linear-gradient(160deg,#1a1200,#4a3500)",platinum:"linear-gradient(160deg,#001a20,#003040)",diamond:"linear-gradient(160deg,#001030,#1a2860)",master:"linear-gradient(160deg,#1a0030,#3d0070)"},h=(s.ranked_wins||0)+(s.ranked_losses||0)+(s.ranked_draws||0),L=h>0?Math.round((s.ranked_wins||0)/h*100):0,g=Ti.map(W=>`
    <div style="display:flex;flex-direction:column;align-items:center;gap:2px;opacity:${m.id===W.id?1:.35};
      transform:${m.id===W.id?"scale(1.15)":"scale(1)"};transition:all 0.3s">
      <div style="font-size:${m.id===W.id?"28px":"20px"}">${W.emoji}</div>
      <div style="font-size:9px;color:${W.color};font-weight:${m.id===W.id?"900":"400"};letter-spacing:0.5px">${W.label.toUpperCase()}</div>
    </div>
  `).join("");t.innerHTML=`
  <div style="min-height:100%;background:${w[m.id]};padding:16px;overflow-y:auto;display:flex;flex-direction:column;gap:16px">
    ...
    <div style="display:flex;flex-direction:column;gap:10px;margin-top:auto;padding-top:8px">
      ...
      away:users!away_id(pseudo,club_name)`).or(`home_id.eq.${r.id},away_id.eq.${r.id}`).order("created_at",{ascending:!1}).limit(50),a={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},s=(o||[]).filter(c=>c.status==="finished"),d=(o||[]).filter(c=>c.status==="in_progress");function l(c){const u=c.home_id===r.id;u?c.home_score:c.away_score,u?c.away_score:c.home_score;const b=c.winner_id===r.id,x=c.home_score===c.away_score&&c.status==="finished",m=c.status!=="finished"?"…":x?"N":b?"V":"D",p=c.status!=="finished"||x?"#888":b?"#1A6B3C":"#c0392b";let f=a[c.mode]||c.mode;c.away_id===null&&!f.startsWith("IA")&&(f="IA");const w=c.home_id===r.id?c.away:c.home;let h;c.away_id===null?h=f:w?h=`${w.club_name||w.pseudo} (${w.pseudo})`:h="Adversaire";let L="";c.status==="in_progress"&&Date.now()-new Date(c.created_at).getTime()>3600*1e3&&(L=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const g=new Date(c.created_at),A=g.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+g.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),E=c.status==="finished"?`${c.home_score} - ${c.away_score}`:`${c.home_score||0} - ${c.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
