import{s as E,F as xi,h as Pt,j as Yi,l as at,i as zn,k as Sn,b as Ln}from"./formation-links-DJC25wD7.js";const In="/manager-wars/";function Li(e,{navigate:t,toast:i}){let s="login";const c=()=>{var a,l,u,o,n,f;const d=s==="login";e.innerHTML=`
    <div style="
    ...
    </style>`,(a=document.getElementById("tab-login-btn"))==null||a.addEventListener("click",()=>{s="login",c()}),(l=document.getElementById("tab-reg-btn"))==null||l.addEventListener("click",()=>{s="register",c()}),d?((u=document.getElementById("login-password"))==null||u.addEventListener("keydown",v=>{var h;v.key==="Enter"&&((h=document.getElementById("login-btn"))==null||h.click())}),(o=document.getElementById("login-btn"))==null||o.addEventListener("click",async()=>{const v=document.getElementById("login-email").value.trim(),h=document.getElementById("login-password").value,T=document.getElementById("login-error");if(T.textContent="",!v||!h){T.textContent="Remplissez tous les champs.";return}const r=document.getElementById("login-btn");r.textContent="⏳ Connexion…",r.disabled=!0;const{error:p}=await E.auth.signInWithPassword({email:v,password:h});if(r.textContent="⚽ Se connecter",r.disabled=!1,p){T.textContent=p.message.includes("Invalid")?"Email ou mot de passe incorrect.":p.message;return}window.location.reload()})):((n=document.getElementById("reg-confirm"))==null||n.addEventListener("keydown",v=>{var h;v.key==="Enter"&&((h=document.getElementById("reg-btn"))==null||h.click())}),(f=document.getElementById("reg-btn"))==null||f.addEventListener("click",async()=>{const v=document.getElementById("reg-email").value.trim(),h=document.getElementById("reg-password").value,T=document.getElementById("reg-confirm").value,r=document.getElementById("reg-error");if(r.textContent="",!v||!h||!T){r.textContent="Remplissez tous les champs.";return}if(h.length<6){r.textContent="Mot de passe trop court (min. 6 caractères).";return}if(h!==T){r.textContent="Les mots de passe ne correspondent pas.";return}const p=document.getElementById("reg-btn");p.textContent="⏳ Création…",p.disabled=!0;const{error:I}=await E.auth.signUp({email:v,password:h});if(p.textContent="🚀 Créer mon compte",p.disabled=!1,I){r.textContent=I.message;return}i("Compte créé ! Connecte-toi pour commencer.","success",4e3),s="login",c(),setTimeout(()=>{const z=document.getElementById("login-email");z&&(z.value=v)},50)}))};c()}function Mn(e,{state:t,navigate:i,toast:s,refreshProfile:c}){let d="#1A6B3C",a="#D4A017";e.innerHTML=`
  ...
  `;function l(){var T;const o=document.getElementById("logo-preview"),n=document.getElementById("logo-initials"),f=((T=document.getElementById("setup-club"))==null?void 0:T.value)||"MW",v=f.trim().split(" ").filter(Boolean),h=v.length>=2?(v[0][0]+v[1][0]).toUpperCase():f.slice(0,2).toUpperCase();o&&(o.style.background=a,o.style.borderColor=d),n&&(n.textContent=h,n.style.color=d)}document.getElementById("color1").addEventListener("input",o=>{d=o.target.value,document.getElementById("swatch1").style.background=d,l()}),document.getElementById("color2").addEventListener("input",o=>{a=o.target.value,document.getElementById("swatch2").style.background=a,l()});function u(o){document.querySelectorAll(".setup-step").forEach(n=>n.classList.remove("active")),document.getElementById(`step-${o}`).classList.add("active"),document.getElementById("step-num").textContent=o,document.getElementById("progress-fill").style.width=`${Math.round(o/3*100)}%`,o===3&&l()}document.getElementById("step1-next").addEventListener("click",async()=>{const o=document.getElementById("setup-pseudo").value.trim(),n=document.getElementById("step1-error");if(n.textContent="",o.length<3){n.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:f}=await E.from("users").select("id").eq("pseudo",o).maybeSingle();if(f){n.textContent="Ce pseudo est déjà pris.";return}u(2)}),document.getElementById("step2-back").addEventListener("click",()=>u(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const o=document.getElementById("setup-club").value.trim(),n=document.getElementById("step2-error");if(n.textContent="",o.length<2){n.textContent="Nom trop court (min. 2 caractères).";return}const{data:f}=await E.from("users").select("id").eq("club_name",o).maybeSingle();if(f){n.textContent="Ce nom de club est déjà pris.";return}u(3)}),document.getElementById("step3-back").addEventListener("click",()=>u(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const o=document.getElementById("setup-pseudo").value.trim(),n=document.getElementById("setup-club").value.trim(),f=document.getElementById("step3-error"),v=document.getElementById("step3-finish");f.textContent="",v.disabled=!0,v.textContent="Création en cours…";try{const{error:h}=await E.from("users").insert({id:t.user.id,pseudo:o,club_name:n,club_color1:d,club_color2:a,credits:1e4});if(h)throw h;await Cn(t.user.id),await c(),s(`Bienvenue ${o} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(h){f.textContent=h.message,v.disabled=!1,v.textContent="🚀 Créer mon profil !"}})}async function Cn(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await E.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const jn="modulepreload",qn=function(e){return"/manager-wars/"+e},Ii={},yi=function(t,i,s){let c=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),l=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));c=Promise.allSettled(i.map(u=>{if(u=qn(u),u in Ii)return;Ii[u]=!0;const o=u.endsWith(".css"),n=o?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${u}"]${n}`))return;const f=document.createElement("link");if(f.rel=o?"stylesheet":jn,o||(f.as="script"),f.crossOrigin="",f.href=u,l&&f.setAttribute("nonce",l),document.head.appendChild(f),o)return new Promise((v,h)=>{f.addEventListener("load",v),f.addEventListener("error",()=>h(new Error(`Unable to preload CSS for ${u}`)))})}))}function d(a){const l=new Event("vite:preloadError",{cancelable:!0});if(l.payload=a,window.dispatchEvent(l),!l.defaultPrevented)throw a}return c.then(a=>{for(const l of a||[])l.status==="rejected"&&d(l.reason);return t().catch(d)})},Ft="#1A6B3C",Dt="#cc2222",Bn="#D4A017",Mi="#888";async function Fn(e,t){const{state:i,toast:s}=t;e.innerHTML=`
    ...
    </div>`,await Wi(i,s,t),document.getElementById("btn-add-friend").addEventListener("click",()=>Gn(i,s)),document.getElementById("btn-accept-friend").addEventListener("click",()=>Ji(i,s,null,t))}async function Wi(e,t,i={}){const{navigate:s}=i,c=e.user.id,{data:d,error:a}=await E.from("friendships").select("id, requester_id, addressee_id").eq("status","accepted").or(`requester_id.eq.${c},addressee_id.eq.${c}`),{count:l}=await E.from("friendships").select("id",{count:"exact",head:!0}).eq("addressee_id",c).eq("status","pending"),u=document.getElementById("pending-badge");u&&(l>0?(u.style.display="flex",u.textContent=l):u.style.display="none");const o=document.getElementById("friends-list");if(!o)return;if(a){console.error("[Friends] Erreur:",a),o.innerHTML=`<div style="color:${Dt};text-align:center;padding:16px">Erreur chargement : ${a.message}</div>`;return}const n=(d||[]).map(h=>h.requester_id===c?h.addressee_id:h.requester_id);let f={};if(n.length){const{data:h}=await E.from("users").select("id, pseudo, club_name, last_seen_at, club_color1, club_color2").in("id",n);(h||[]).forEach(T=>{f[T.id]=T})}const v=(d||[]).map(h=>({friendshipId:h.id,friend:f[h.requester_id===c?h.addressee_id:h.requester_id]||{pseudo:"?"}}));if(!v.length){o.innerHTML=`
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      ${v.map(({friendshipId:h,friend:T})=>Dn(T,h)).join("")}
    </div>`,o.querySelectorAll("[data-stats]").forEach(h=>{h.addEventListener("click",()=>Pn(e,h.dataset.stats,h.dataset.friendName))}),o.querySelectorAll("[data-match]").forEach(h=>{h.addEventListener("click",()=>{const T=h.dataset.friendId,r=h.dataset.friendName;console.log("[Friends] clic match",{fid:T,fname:r,hasNavigate:typeof s}),typeof s=="function"?s("match",{matchMode:"friend",friendId:T,friendName:r}):t("Erreur navigation","error")})})}function Dn(e,t){const i=e.club_name||e.pseudo||"?",s=e.pseudo||"",c=(s||i).slice(0,2).toUpperCase(),d=e.club_color2||Ft,a=e.club_color1||"#ffffff",l=e.last_seen_at?new Date(e.last_seen_at):null,u=l&&Date.now()-l.getTime()<3*60*1e3;return`
    ...
    <div class="popup-box">
      ...
      ${n.length?`<div style="display:flex;flex-direction:column;gap:8px;max-height:50vh;overflow-y:auto;margin-bottom:14px">
            ${n.map(v=>{var h,T,r;return`
              <div style="display:flex;align-items:center;gap:10px;background:#f9f9f9;border-radius:10px;padding:10px 12px">
                <div style="flex:1;font-size:14px;font-weight:700">${((h=v.requester)==null?void 0:h.club_name)||((T=v.requester)==null?void 0:T.pseudo)||"?"}
    ...
    ${hi()}`,document.body.appendChild(o);const f=()=>o.remove();o.querySelector("#pending-close").addEventListener("click",f),o.addEventListener("click",v=>{v.target===o&&f()}),o.querySelectorAll("[data-accept]").forEach(v=>{v.addEventListener("click",async()=>{const{error:h}=await E.from("friendships").update({status:"accepted"}).eq("id",v.dataset.accept);if(h){t("Erreur : "+h.message,"error");return}v.closest("div[style]").remove(),t("✅ Ami accepté !","success"),Wi(e,t,s),i&&i()})}),o.querySelectorAll("[data-decline]").forEach(v=>{v.addEventListener("click",async()=>{await E.from("friendships").delete().eq("id",v.dataset.decline),v.closest("div[style]").remove(),t("Demande refusée","info"),i&&i()})})}async function Pn(e,t,i){const s=e.user.id,[c,d]=[s,t].sort(),a=s===c,{data:l}=await E.from("friend_match_stats").select("*").eq("player1_id",c).eq("player2_id",d).single(),u=e.profile.club_name||e.profile.pseudo||"Moi",o=l||{},n=a?o.wins_p1||0:o.wins_p2||0,f=a?o.wins_p2||0:o.wins_p1||0,v=o.draws||0,h=a?o.goals_p1||0:o.goals_p2||0,T=a?o.goals_p2||0:o.goals_p1||0,r=a?o.gc_used_p1||0:o.gc_used_p2||0,p=a?o.gc_used_p2||0:o.gc_used_p1||0,I=o.matches_total||0,z=(U,D,N,G=Ft,ie=Dt)=>`
    ...
    </style>`}async function Xi({player1Id:e,player2Id:t,score1:i,score2:s,gc1:c,gc2:d}){const[a,l]=[e,t].sort(),u=e!==a,o=u?s:i,n=u?i:s,f=u?d:c,v=u?c:d,h=o>n?1:0,T=n>o?1:0,r=o===n?1:0,{data:p}=await E.from("friend_match_stats").select("*").eq("player1_id",a).eq("player2_id",l).single();p?await E.from("friend_match_stats").update({wins_p1:p.wins_p1+h,wins_p2:p.wins_p2+T,draws:p.draws+r,goals_p1:p.goals_p1+o,goals_p2:p.goals_p2+n,gc_used_p1:p.gc_used_p1+f,gc_used_p2:p.gc_used_p2+v,matches_total:p.matches_total+1}).eq("player1_id",a).eq("player2_id",l):await E.from("friend_match_stats").insert({player1_id:a,player2_id:l,wins_p1:h,wins_p2:T,draws:r,goals_p1:o,goals_p2:n,gc_used_p1:f,gc_used_p2:v,matches_total:1})}const Nn="2026.07.03-1704";async function Ci(e,{state:t,navigate:i,toast:s}){var d,a;const c=t.profile;c&&(e.innerHTML=`
  <style>
  ...
  `,e.querySelectorAll("[data-nav]").forEach(l=>{l.addEventListener("click",u=>{u.preventDefault(),i(l.dataset.nav)})}),(d=document.getElementById("nav-rankings"))==null||d.addEventListener("click",()=>i("rankings")),(a=document.getElementById("nav-matches"))==null||a.addEventListener("click",()=>i("matches")),e.querySelectorAll("[data-action]").forEach(l=>{l.addEventListener("click",()=>{l.classList.add("tapped"),setTimeout(()=>l.classList.remove("tapped"),200);const u=l.dataset.action;if(u==="match-ai"){Un(i);return}if(u==="match-random"){i("match",{matchMode:"random"});return}if(u==="match-friend"){i("friends");return}if(u==="mini-league"){i("mini-league");return}if(u==="ranked"){ctx.toast("Mode Ranked — Bientôt disponible !","info");return}s("Bientôt disponible","info")})}),window.innerWidth<768&&requestAnimationFrame(()=>{var U,D,N,G,ie,se;const l=((U=window.visualViewport)==null?void 0:U.height)||window.innerHeight,u=((D=document.querySelector(".top-nav"))==null?void 0:D.offsetHeight)||56,o=((N=document.querySelector(".bottom-nav"))==null?void 0:N.offsetHeight)||60,n=((G=e.querySelector(".hero-compact"))==null?void 0:G.offsetHeight)||52,f=e.querySelector(".ranked-banner");e.querySelector(".play-grid");const v=((se=(ie=e.querySelector("#logout-btn"))==null?void 0:ie.closest("div"))==null?void 0:se.offsetHeight)||44,h=["friend-requests-banner","match-invite-banner","ongoing-match-banner"].reduce((de,P)=>{var Y;return de+(((Y=document.getElementById(P))==null?void 0:Y.offsetHeight)||0)},0),T=14*5,r=l-u-o-n-v-h-T,p=Math.max(80,Math.round(r*.28)),I=Math.max(160,Math.round(r*.72)),z=Math.floor((I-10)/2);f&&(f.style.minHeight=f.style.maxHeight=p+"px"),e.querySelectorAll(".play-grid .play-card").forEach(de=>{de.style.minHeight=de.style.height=z+"px"});const C=Math.round(z*.55);e.querySelectorAll(".play-card .play-icon").forEach(de=>{de.style.height=C+"px"})}),document.getElementById("logout-btn").addEventListener("click",async()=>{await E.auth.signOut(),window.location.reload()}),Qi(t,s),Hn(t,s,i),Zi(t,s,i))}async function Zi(e,t,i){const s=document.getElementById("ongoing-match-banner");if(!s)return;const c=e.profile.id,{data:d}=await E.from("matches").select("id, home_id, away_id, status, mode").eq("status","active").or(`home_id.eq.${c},away_id.eq.${c}`).order("created_at",{ascending:!1});if(!(d!=null&&d.length)){s.innerHTML="";return}const a=d.map(u=>u.home_id===c?u.away_id:u.home_id).filter(Boolean);let l={};if(a.length){const{data:u}=await E.from("users").select("id, pseudo, club_name").in("id",a);(u||[]).forEach(o=>{l[o.id]=o.club_name||o.pseudo})}s.innerHTML=d.map(u=>{const o=u.home_id===c?u.away_id:u.home_id,n=l[o]||"Adversaire",f=u.mode==="mini_league";return`
  ...
  " data-card-id="${e.id}">
    ...
    <div style="background:#f2e8d2;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
      ...
      <div style="background:#f2e8d2;padding:3px 6px;display:flex;align-items:center;justify-content:space-between;min-height:26px;gap:4px">
        ...
        ${(v=i.clubs)!=null&&v.logo_url?`<img src="${i.clubs.logo_url}" style="width:22px;height:18px;object-fit:contain;flex-shrink:0">`:`<div style="background:#1a3a7a;color:#fff;border-radius:3px;padding:1px 4px;font-size:6px;font-weight:800;flex-shrink:0">${(((h=i.clubs)==null?void 0:h.encoded_name)||"").slice(0,6)}</div>`}
      </div>
    </div>
  ...
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
        ...
        </div>`},null)}e.querySelectorAll(".col-tab-btn").forEach(O=>{O.addEventListener("click",()=>{N=O.dataset.tab,G="Tous",ie="",se=!1,e.querySelectorAll(".col-tab-btn").forEach(Z=>{const ue=Z.dataset.tab===N;Z.style.borderBottomColor=ue?"var(--green)":"transparent",Z.style.color=ue?"var(--green)":"var(--gray-600)"}),F(),oe()})}),F(),oe()}function Zn(e,t,i){const s=Ke[e]||{icon:"⚡",desc:"Effet spécial."},c=(t==null?void 0:t.gc_type)==="ultra_game_changer",d={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},a={purple:"#b06ce0",light_blue:"#00d4ef"},l=d[t==null?void 0:t.color]||d.purple,u=a[t==null?void 0:t.color]||a.purple,o=(t==null?void 0:t.name)||e,n=(t==null?void 0:t.effect)||s.desc,f=t!=null&&t.image_url?`/manager-wars/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
      <div style="background:${l};border-radius:16px;border:2px solid ${u};
        ...
        <div style="height:160px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
          ${f?`<img src="${f}" style="max-width:150px;max-height:150px;object-fit:contain">`:`<span style="font-size:64px">${s.icon}</span>`}
        </div>
      </div>
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Vt=1e3;function Qn(e,t,i,s){var T,r,p;const{state:c,toast:d,closeModal:a,navigate:l,refreshProfile:u}=i,o=e.formation,n={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function f(){const I=Pt[o]||{},z=xi[o]||[],C=290,U=360,D=20;function N(ie){const se=I[ie];return se?{x:se.x*C,y:se.y*U}:null}let G=`<svg width="${C}" height="${U}" viewBox="0 0 ${C} ${U}" xmlns="http://www.w3.org/2000/svg">`;for(const[ie,se]of z){const de=N(ie),P=N(se);!de||!P||(G+=`<line x1="${de.x}" y1="${de.y}" x2="${P.x}" y2="${P.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const ie of Object.keys(I)){const se=N(ie);if(!se)continue;const de=ie.replace(/\d+/,""),P=n[de]||"#555";G+=`<circle cx="${se.x}" cy="${se.y}" r="${D}" fill="${P}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,G+=`<text x="${se.x}" y="${se.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${de}</text>`}return G+="</svg>",G}const v=t.filter(I=>I.formation===o);v.length;const h=!e.is_for_sale;s(`Formation ${o}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(Ae=document.getElementById("close-detail"))==null||Ae.addEventListener("click",l);let F=new Set;const oe=()=>{const te=F.size,re=document.getElementById("sell-action-panel");re&&(re.style.display=te>0?"block":"none",document.getElementById("sell-selected-count").textContent=te,document.getElementById("sell-direct-total").textContent=(te*p).toLocaleString("fr")+" cr.")};document.querySelectorAll(".expl-check").forEach(te=>{te.addEventListener("change",()=>{const re=te.dataset.id;te.checked?F.add(re):F.delete(re);const O=te.closest(".exemplaire-row");O&&(O.style.borderColor=te.checked?"#1A6B3C":"#eee"),oe()})}),document.querySelectorAll(".exemplaire-row").forEach(te=>{te.addEventListener("click",re=>{if(re.target.closest("button")||re.target.tagName==="INPUT")return;const O=te.querySelector(".expl-check");O&&!O.disabled&&(O.checked=!O.checked,O.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(te=>{te.addEventListener("click",re=>{re.stopPropagation();const O=document.querySelector(`.expl-hist[data-hist="${te.dataset.idx}"]`);O&&(O.style.display=O.style.display==="none"?"flex":"none")})}),(we=document.getElementById("direct-sell-btn"))==null||we.addEventListener("click",async()=>{const te=[...F];if(!te.length)return;const re=te.length*p;if(!confirm(`Vendre ${te.length} carte${te.length>1?"s":""} ${n.surname_encoded} pour ${re.toLocaleString("fr")} crédits ? Action irréversible.`))return;await E.from("market_listings").delete().in("card_id",te),await E.from("transfer_history").delete().in("card_id",te);const{error:O}=await E.from("cards").delete().in("id",te);if(O){d(O.message,"error");return}await E.from("users").update({credits:(c.profile.credits||0)+re}).eq("id",c.profile.id),await o();const Z=document.getElementById("nav-credits");Z&&(Z.textContent=`💰 ${(c.profile.credits||0).toLocaleString("fr")}`),d(`+${re.toLocaleString("fr")} crédits ! ${te.length} carte${te.length>1?"s":""} vendue${te.length>1?"s":""}.`,"success"),l(),u("collection")}),($e=document.getElementById("market-sell-btn"))==null||$e.addEventListener("click",async()=>{var me;const te=[...F];if(!te.length){d("Sélectionne au moins un exemplaire","warning");return}const re=parseInt((me=document.getElementById("sell-market-price"))==null?void 0:me.value);if(!re||re<1){d("Prix invalide","error");return}const{error:O}=await E.from("cards").update({is_for_sale:!0,sale_price:re}).in("id",te);if(O){d(O.message,"error");return}const Z=te.map(X=>({seller_id:c.profile.id,card_id:X,price:re,status:"active"})),{error:ue}=await E.from("market_listings").insert(Z);ue&&console.warn("[Market] insert listings:",ue.message),d(`${te.length} carte${te.length>1?"s":""} mise${te.length>1?"s":""} en vente à ${re.toLocaleString("fr")} cr. chacune !`,"success"),l(),u("collection")}),(Ee=document.getElementById("cancel-sell-btn"))==null||Ee.addEventListener("click",async()=>{await E.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await E.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),d("Annonce retirée","success"),l(),u("collection")})}const Jt={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},vt={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function ei(e){const t=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function dn(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function ln(e){var i;const t="https://fcnwclxlkytdfjotqkta.supabase.co";return(i=e==null?void 0:e.clubs)!=null&&i.logo_url?e.clubs.logo_url.startsWith("http")?e.clubs.logo_url:`${t}/storage/v1/object/public/assets/clubs/${e.clubs.logo_url}`:null}function eo(e,t=44,i=58){var h;const s=e?ei(e):null,c=e?ln(e):null,d=dn(e==null?void 0:e.country_code),a=(e==null?void 0:e.job)||"MIL",l=vt[a]||"#555",u={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",o=Number(a==="GK"?e==null?void 0:e.note_g:a==="DEF"?e==null?void 0:e.note_d:a==="MIL"?e==null?void 0:e.note_m:e==null?void 0:e.note_a)||0,n=Math.round(i*.19),f=Math.round(i*.25),v=i-n-f;return e?`<div style="width:${t}px;height:${i}px;border-radius:5px;border:2px solid ${u};background:${l};position:relative;overflow:hidden;flex-shrink:0">
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;height:${f}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ...
      ${c?`<img src="${c}" style="width:${f-4}px;height:${f-4}px;object-fit:contain">`:(h=e==null?void 0:e.clubs)!=null&&h.encoded_name?`<span style="font-size:${Math.max(4,f-8)}px;font-weight:700;color:#333">${(e.clubs.encoded_name||"").slice(0,3).toUpperCase()}</span>`:"<span></span>"}
    </div>
  ...
  </div>`,document.getElementById("new-deck-btn").addEventListener("click",async()=>{const a=prompt("Nom du deck :",`Deck ${((d==null?void 0:d.length)||0)+1}`);if(!a)return;const{data:l,error:u}=await E.from("decks").insert({owner_id:i.profile.id,name:a}).select().single();if(u){c(u.message,"error");return}c("Deck créé !","success"),Di(l.id,e,t)}),e.querySelectorAll("[data-open-deck]").forEach(a=>{a.addEventListener("click",()=>Di(a.dataset.openDeck,e,t))}),e.querySelectorAll("[data-rename]").forEach(a=>{a.addEventListener("click",async()=>{const l=prompt("Nouveau nom :",a.dataset.name);if(!l||l===a.dataset.name)return;const{error:u}=await E.from("decks").update({name:l}).eq("id",a.dataset.rename);if(u){c(u.message,"error");return}c("Deck renommé !","success"),li(e,t)})}),e.querySelectorAll("[data-delete]").forEach(a=>{a.addEventListener("click",async()=>{if(!confirm(`Supprimer le deck "${a.dataset.name}" ? Cette action est irréversible.`))return;await E.from("deck_cards").delete().eq("deck_id",a.dataset.delete);const{error:l}=await E.from("decks").delete().eq("id",a.dataset.delete);if(l){c(l.message,"error");return}c("Deck supprimé.","success"),li(e,t)})})}async function Di(e,t,i){const{state:s,toast:c}=i;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:d}=await E.from("decks").select("*").eq("id",e).single(),{data:a}=await E.from("cards").select(`id, card_type, formation,
      player:players(id, firstname, surname_encoded, country_code, club_id, job, job2,
        note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length,
        clubs(encoded_name, logo_url))`).eq("owner_id",s.profile.id),l=(a||[]).filter(p=>p.card_type==="player"&&p.player),u=(a||[]).filter(p=>p.card_type==="formation"),o=(a||[]).filter(p=>p.card_type==="stadium"),n=[...new Set(o.map(p=>p.stadium_id).filter(Boolean))];let f={};if(n.length){const{data:p}=await E.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",n);(p||[]).forEach(I=>{f[I.id]=I})}const v=u.map(p=>p.formation).filter(Boolean),{data:h}=await E.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",e);let T=d.formation||"4-4-2";v.length>0&&!v.includes(T)&&(T=v[0]);const r={deckId:e,name:d.name,formation:T,formationCardId:d.formation_card_id,stadiumCardId:d.stadium_card_id||null,slots:{},subs:[],playerCards:l,formationCards:u,stadiumCards:o,stadDefMap:f,availableFormations:v};(h||[]).forEach(p=>{p.is_starter?r.slots[p.position]=p.card_id:r.subs.includes(p.card_id)||r.subs.push(p.card_id)}),St(t,r,i)}function St(e,t,i){var u,o;const{navigate:s}=i;Jt[t.formation];const c=Gi(t.formation),d=c.filter(n=>t.slots[n]).length,a=t.availableFormations.length>0?t.availableFormations:Object.keys(Jt),l=t.subs.map(n=>t.playerCards.find(f=>f.id===n)).filter(Boolean);[...Object.values(t.slots),...t.subs],e.innerHTML=`
  <div style="height:100%;overflow-y:auto;padding-bottom:80px;background:var(--page-bg)">
    ...
    <div style="padding:8px 16px;background:#fff;border-bottom:1px solid var(--gray-200);display:flex;align-items:center;gap:10px">
      ...
      ${t.stadiumCards.length>0?`
        <select id="stadium-select" style="padding:6px;border-radius:6px;border:1.5px solid #E87722;font-size:12px;max-width:180px;background:#fff">
          ...
          ${t.stadiumCards.map(n=>{var h;const f=t.stadDefMap[n.stadium_id],v=f?f.name+((h=f.club)!=null&&h.encoded_name?` (${f.club.encoded_name})`:f.country_code?` (${f.country_code})`:""):n.id.slice(0,8);return`<option value="${n.id}" ${t.stadiumCardId===n.id?"selected":""}>${v}</option>`}).join("")}
    ...
    <div style="padding:10px 12px;background:rgba(0,0,0,0.25);border-top:1px solid rgba(255,255,255,0.1)">
      ...
      <div style="display:flex;gap:8px;align-items:flex-end;overflow-x:auto;padding-bottom:4px" id="subs-list">
        ${l.map(n=>{const f=n.player;return`<div style="position:relative;flex-shrink:0">
            ${eo(f,44,58)}
  ...
  </div>`,to(e,t,c,i),document.getElementById("builder-back").addEventListener("click",()=>s("decks")),document.getElementById("formation-select").addEventListener("change",n=>{t.formation=n.target.value;const f=Gi(t.formation),v={};f.forEach(h=>{t.slots[h]&&(v[h]=t.slots[h])}),t.slots=v,St(e,t,i)}),(u=document.getElementById("stadium-select"))==null||u.addEventListener("change",n=>{t.stadiumCardId=n.target.value||null}),document.getElementById("save-deck").addEventListener("click",()=>oo(t,i)),e.querySelectorAll("[data-remove-sub]").forEach(n=>{n.addEventListener("click",()=>{t.subs=t.subs.filter(f=>f!==n.dataset.removeSub),St(e,t,i)})}),(o=document.getElementById("add-sub-btn"))==null||o.addEventListener("click",()=>{no(t,e,i)})}function to(e,t,i,s){const c=e.querySelector("#deck-field");if(!c)return;const d=Pt[t.formation]||{},a=Yi(t.formation)||[],l={};for(const I of i){const z=t.slots[I],C=z?t.playerCards.find(U=>U.id===z):null;l[I]=C?C.player:null}const u=300,o=300,n=48,f=64,v=13,h=16,T=38;function r(I){const z=d[I];return z?{x:z.x*u,y:z.y*o}:null}let p="";for(const[I,z]of a){const C=r(I),U=r(z);if(!C||!U)continue;const D=l[I]?{...l[I],club_id:l[I].club_id,country_code:l[I].country_code,rarity:l[I].rarity}:null,N=l[z]?{...l[z],club_id:l[z].club_id,country_code:l[z].country_code,rarity:l[z].rarity}:null,G=at(D,N);G==="#ff3333"||G==="#cc2222"?p+=`<line x1="${C.x.toFixed(1)}" y1="${C.y.toFixed(1)}" x2="${U.x.toFixed(1)}" y2="${U.y.toFixed(1)}" stroke="${G}" stroke-width="2.5" stroke-linecap="round" opacity="0.4"/>`:(p+=`<line x1="${C.x.toFixed(1)}" y1="${C.y.toFixed(1)}" x2="${U.x.toFixed(1)}" y2="${U.y.toFixed(1)}" stroke="${G}" stroke-width="8" stroke-linecap="round" opacity="0.2"/>`,p+=`<line x1="${C.x.toFixed(1)}" y1="${C.y.toFixed(1)}" x2="${U.x.toFixed(1)}" y2="${U.y.toFixed(1)}" stroke="${G}" stroke-width="2.5" stroke-linecap="round" opacity="0.9"/>`)}for(const I of i){const z=r(I);if(!z)continue;const C=l[I],U=I.replace(/\d+/,""),D=vt[U]||"#555",N=(z.x-n/2).toFixed(1),G=(z.y-f/2).toFixed(1),ie={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[C==null?void 0:C.rarity]||"#aaa";if(C){const se=ei(C),de=ln(C),P=dn(C.country_code),Y=Number(U==="GK"?C.note_g:U==="DEF"?C.note_d:U==="MIL"?C.note_m:C.note_a)||0,V=f-v-h;p+=`<defs><clipPath id="dcp-${I}"><rect x="${N}" y="${(z.y-f/2+v).toFixed(1)}" width="${n}" height="${V}"/></clipPath></defs>`,p+=`<rect x="${N}" y="${G}" width="${n}" height="${f}" rx="5" fill="${D}"/>`,se&&(p+=`<image href="${se}" x="${N}" y="${(z.y-f/2+v).toFixed(1)}" width="${n}" height="${V}" clip-path="url(#dcp-${I})" preserveAspectRatio="xMidYMin slice"/>`),p+=`<rect x="${N}" y="${G}" width="${n}" height="${v}" fill="rgba(255,255,255,0.93)"/>`,p+=`<text x="${z.x.toFixed(1)}" y="${(z.y-f/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(C.surname_encoded||"").slice(0,9)}</text>`;const F=(z.y+f/2-h).toFixed(1);p+=`<rect x="${N}" y="${F}" width="${n}" height="${h}" fill="rgba(255,255,255,0.93)"/>`,P&&(p+=`<image href="${P}" x="${(z.x-21).toFixed(1)}" y="${(z.y+f/2-h+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`),p+=`<text x="${z.x.toFixed(1)}" y="${(z.y+f/2-h/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${Y}</text>`,de&&(p+=`<image href="${de}" x="${(z.x+n/2-14).toFixed(1)}" y="${(z.y+f/2-h+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`),p+=`<rect x="${N}" y="${G}" width="${n}" height="${f}" rx="5" fill="none" stroke="${ie}" stroke-width="2"/>`}else p+=`<rect x="${N}" y="${G}" width="${n}" height="${f}" rx="5" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="5,3"/>`,p+=`<text x="${z.x.toFixed(1)}" y="${z.y.toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="22" fill="rgba(255,255,255,0.35)">+</text>`,p+=`<text x="${z.x.toFixed(1)}" y="${(z.y+16).toFixed(1)}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.3)">${U}</text>`;p+=`<rect x="${N}" y="${G}" width="${n}" height="${f}" rx="5" fill="rgba(0,0,0,0.01)" class="deck-slot-hit" data-pos="${I}" style="cursor:pointer"/>`}c.innerHTML=`<svg viewBox="${-T} ${-T} ${u+T*2} ${o+T*2}" width="100%" style="display:block;max-width:440px;margin:0 auto">${p}</svg>`,c.querySelectorAll(".deck-slot-hit").forEach(I=>{I.addEventListener("click",()=>io(I.dataset.pos,t,e,s))})}function io(e,t,i,s){var v,h,T;const{openModal:c,closeModal:d}=s,a=e.replace(/\d+/,""),l=t.slots[e],u=l?t.playerCards.find(r=>r.id===l):null;(v=u==null?void 0:u.player)==null||v.id;const o=new Set;Object.entries(t.slots).forEach(([r,p])=>{var z;if(r===e||!p)return;const I=t.playerCards.find(C=>C.id===p);(z=I==null?void 0:I.player)!=null&&z.id&&o.add(I.player.id)}),t.subs.forEach(r=>{var I;const p=t.playerCards.find(z=>z.id===r);(I=p==null?void 0:p.player)!=null&&I.id&&o.add(p.player.id)});const n=new Set,f=t.playerCards.filter(r=>{const p=r.player;return!(p.job===a||p.job2===a)||o.has(p.id)||n.has(p.id)?!1:(n.add(p.id),!0)});f.sort((r,p)=>{const I=a==="GK"?r.player.note_g:a==="DEF"?r.player.note_d:a==="MIL"?r.player.note_m:r.player.note_a;return(a==="GK"?p.player.note_g:a==="DEF"?p.player.note_d:a==="MIL"?p.player.note_m:p.player.note_a)-I}),c(`Choisir ${a} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${f.length>0?f.map(r=>{var U,D;const p=r.player,I=a==="GK"?p.note_g:a==="DEF"?p.note_d:a==="MIL"?p.note_m:p.note_a,z=ei(p),C={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[p.rarity];return`<div class="player-option" data-card-id="${r.id}"
    ...
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(h=document.getElementById("close-selector"))==null||h.addEventListener("click",d),(T=document.getElementById("remove-player"))==null||T.addEventListener("click",()=>{delete t.slots[e],d(),St(i,t,s)}),document.querySelectorAll(".player-option").forEach(r=>{r.addEventListener("click",()=>{t.slots[e]=r.dataset.cardId,d(),St(i,t,s)})})}function no(e,t,i){var u;const{openModal:s,closeModal:c}=i,d=new Set;Object.values(e.slots).filter(Boolean).forEach(o=>{var f;const n=e.playerCards.find(v=>v.id===o);(f=n==null?void 0:n.player)!=null&&f.id&&d.add(n.player.id)}),e.subs.forEach(o=>{var f;const n=e.playerCards.find(v=>v.id===o);(f=n==null?void 0:n.player)!=null&&f.id&&d.add(n.player.id)});const a=new Set,l=e.playerCards.filter(o=>{var n,f,v;return d.has((n=o.player)==null?void 0:n.id)||a.has((f=o.player)==null?void 0:f.id)?!1:(a.add((v=o.player)==null?void 0:v.id),!0)});s("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${l.length>0?l.map(o=>{var h;const n=o.player,f=ei(n),v=n.job==="GK"?n.note_g:n.job==="DEF"?n.note_d:n.job==="MIL"?n.note_m:n.note_a;return`<div class="player-option" data-card-id="${o.id}"
          style="display:flex;align-items:center;gap:10px;padding:8px;border:1.5px solid var(--gray-200);border-radius:10px;cursor:pointer">
  ...
  <div class="page">
    ...
    <div class="page-body">
      <div class="booster-grid">
        ${d.map(l=>{const u=l.cost===0||c>=l.cost;return`<div class="booster-card ${u?"":"disabled"}" data-booster="${l.id}" style="position:relative">
  ...
  </div>`,e.querySelectorAll(".booster-card:not(.disabled)").forEach(l=>{l.addEventListener("click",async()=>{const u=d.find(o=>o.id===l.dataset.booster);if(u){l.style.opacity="0.5",l.style.pointerEvents="none";try{await fo(u,{state:t,toast:s,navigate:i,container:e})}catch(o){s(o.message,"error"),l.style.opacity="",l.style.pointerEvents=""}}})}),e.querySelectorAll(".booster-info-btn").forEach(l=>{l.addEventListener("click",u=>{u.stopPropagation();const o=d.find(n=>n.id===l.dataset.boosterId);yo(o)})})}async function fo(e,{state:t,toast:i,navigate:s,container:c}){var n;if(e.cost>0&&t.profile.credits<e.cost){i("Crédits insuffisants","error");return}if(e.isPub)try{await vo()}catch(f){i(f.message||"Regardez la pub entièrement pour ouvrir le booster","error");return}const{data:d}=await E.from("cards").select("card_type, player_id, formation").eq("owner_id",t.profile.id),a=new Set((d||[]).filter(f=>f.card_type==="player").map(f=>f.player_id)),l=new Set((d||[]).filter(f=>f.card_type==="formation").map(f=>f.formation));let u=[];if((n=e.rates)!=null&&n.length)u=await pi(t.profile,e);else{const f=e.type||"player";f==="player"?u=await fn(t.profile,e.cardCount,e.cost):f==="game_changer"?u=await gn(t.profile,e.cardCount,e.cost):f==="formation"?u=await mn(t.profile,e.cost):u=await pi(t.profile,e)}u.forEach(f=>{f.card_type==="player"&&f.player?f.isDuplicate=a.has(f.player.id):f.card_type==="formation"&&(f.isDuplicate=l.has(f.formation))});const{data:o}=await E.from("users").select("*").eq("id",t.profile.id).single();o&&(t.profile=o),xn(u,e,s)}function go(){const e=Math.random()*100;return e<.5?"legende":e<2?"special":e<10?"normal_high":"normal_low"}function mt(e){return Math.max(Number(e.note_g)||0,Number(e.note_d)||0,Number(e.note_m)||0,Number(e.note_a)||0)}function mo(e,t){let i;switch(t){case"legende":i=e.filter(s=>s.rarity==="legende"),i.length||(i=e.filter(s=>s.rarity==="pepite"||s.rarity==="papyte")),i.length||(i=e.filter(s=>mt(s)>=6));break;case"special":i=e.filter(s=>s.rarity==="pepite"||s.rarity==="papyte"),i.length||(i=e.filter(s=>mt(s)>=6));break;case"normal_high":i=e.filter(s=>s.rarity==="normal"&&mt(s)>=6),i.length||(i=e.filter(s=>mt(s)>=6));break;default:i=e.filter(s=>s.rarity==="normal"&&mt(s)>=1&&mt(s)<=5),i.length||(i=e.filter(s=>s.rarity==="normal"));break}return i.length||(i=e),i[Math.floor(Math.random()*i.length)]}async function pi(e,t){if(t.cost>0){const{error:l}=await E.from("users").update({credits:e.credits-t.cost}).eq("id",e.id);if(l)throw l}const{data:i}=await E.from("cards").select("player_id, card_type, formation").eq("owner_id",e.id),s=new Set((i||[]).filter(l=>l.card_type==="player").map(l=>l.player_id)),c=new Set((i||[]).filter(l=>l.card_type==="formation").map(l=>l.formation)),d=new Set,a=[];for(let l=0;l<(t.cardCount||5);l++){const u=ro(t.rates);if(u){if(u.card_type==="player"){const o=z=>({légende:"legende",pépite:"pepite",pépites:"pepite"})[z]||z,n=u.rarity?o(u.rarity):null;let f=E.from("players").select("id,job,firstname,surname_encoded,country_code,club_id,rarity,note_g,note_d,note_m,note_a,skin,hair,hair_length,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);n&&(f=f.eq("rarity",n));const{data:v}=await f;let h=v||[];if((u.note_min||u.note_max)&&(h=h.filter(z=>{const C=Math.max(Number(z.note_g)||0,Number(z.note_d)||0,Number(z.note_m)||0,Number(z.note_a)||0);return(!u.note_min||C>=u.note_min)&&(!u.note_max||C<=u.note_max)})),h.length||(u.note_min||u.note_max?(h=v||[],console.warn("[Booster] Aucun joueur avec note",u.note_min,"-",u.note_max,"— fallback rareté uniquement")):h=v||[]),!h.length)continue;let T=h.filter(z=>!d.has(z.id));T.length||(T=h);const r=T[Math.floor(Math.random()*T.length)];d.add(r.id);const p=s.has(r.id),{data:I}=await E.from("cards").insert({owner_id:e.id,player_id:r.id,card_type:"player"}).select().single();I&&(a.push({...I,player:r,isDuplicate:p}),E.rpc("record_transfer",{p_card_id:I.id,p_player_id:r.id,p_club_name:e.club_name||e.pseudo,p_manager_name:e.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{}))}else if(u.card_type==="game_changer"){const{data:o}=await E.from("gc_definitions").select("id,name").eq("is_active",!0).eq("gc_type","game_changer"),n=o!=null&&o.length?o:[{name:"Ressusciter"},{name:"Double attaque"},{name:"Bouclier"},{name:"Vol de note"},{name:"Gel"}],v=n[Math.floor(Math.random()*n.length)].name,{data:h}=await E.from("cards").insert({owner_id:e.id,card_type:"game_changer",gc_type:v}).select().single();h&&a.push(h)}else if(u.card_type==="formation"){const o=pn(),n=o[Math.floor(Math.random()*o.length)],f=c.has(n),{data:v}=await E.from("cards").insert({owner_id:e.id,card_type:"formation",formation:n}).select();v!=null&&v[0]&&a.push({...v[0],isDuplicate:f})}else if(u.card_type==="stadium"){const{data:o,error:n}=await E.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)");if(n){console.error("[Booster] stadium_definitions:",n.message);continue}if(!(o!=null&&o.length)){console.warn("[Booster] Aucun stade en DB");continue}const f=o[Math.floor(Math.random()*o.length)],{data:v,error:h}=await E.from("cards").insert({owner_id:e.id,card_type:"stadium",stadium_id:f.id,rarity:u.rarity||"normal"}).select("id,card_type,stadium_id,rarity").single();if(h){console.error("[Booster] insert stadium card:",h.message);continue}v&&a.push({...v,_stadiumDef:f})}}}return a}async function fn(e,t,i){if(i>0){const{error:o}=await E.from("users").update({credits:e.credits-i}).eq("id",e.id);if(o)throw o}const{data:s}=await E.from("players").select("id,job,firstname,surname_encoded,country_code,club_id,rarity,note_g,note_d,note_m,note_a,note_min,note_max,skin,hair,hair_length,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);if(!(s!=null&&s.length))throw new Error("Pas de joueurs en BDD — ajoutes-en via le panel admin !");const c=s.filter(o=>o.job==="GK"),d=s.filter(o=>o.job!=="GK"),a=!e.first_booster_opened&&c.length>0,l=[];for(let o=0;o<t;o++){const n=o===0&&a?c:o===0?d:s,f=go(),v=mo(n,f);v&&l.push(v)}a&&await E.from("users").update({first_booster_opened:!0}).eq("id",e.id);const{data:u}=await E.from("cards").insert(l.map(o=>({owner_id:e.id,player_id:o.id,card_type:"player"}))).select();return(u||[]).forEach((o,n)=>{E.rpc("record_transfer",{p_card_id:o.id,p_player_id:l[n].id,p_club_name:e.club_name||e.pseudo,p_manager_name:e.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{})}),l.map((o,n)=>({...u[n],player:o}))}async function gn(e,t,i){const{error:s}=await E.from("users").update({credits:e.credits-i}).eq("id",e.id);if(s)throw s;const{data:c}=await E.from("gc_definitions").select("id,name,gc_type,color,effect,image_url").eq("is_active",!0),d=c!=null&&c.length?c:Object.keys(ci).map(n=>({name:n,gc_type:"game_changer"})),a=Array.from({length:t},()=>{const n=d[Math.floor(Math.random()*d.length)];return{owner_id:e.id,card_type:"game_changer",gc_type:n.name,gc_definition_id:n.id||null}}),{data:l,error:u}=await E.from("cards").insert(a).select();return u&&console.error("[Booster GC] Erreur insert:",u.message,u),(l||[]).map(n=>{const f=c==null?void 0:c.find(v=>v.name===n.gc_type||v.id===n.gc_definition_id);return{...n,_gcDef:f||null}})}async function mn(e,t){const{error:i}=await E.from("users").update({credits:e.credits-t}).eq("id",e.id);if(i)throw i;const{data:s}=await E.from("cards").select("formation").eq("owner_id",e.id).eq("card_type","formation"),c=new Set((s||[]).map(n=>n.formation)),d=pn(),a=d[Math.floor(Math.random()*d.length)],l=c.has(a),{data:u,error:o}=await E.from("cards").insert({owner_id:e.id,card_type:"formation",formation:a}).select();return o&&console.error("[Booster Formation] Erreur insert:",o.message,o),(u||[]).map(n=>({...n,isDuplicate:l}))}function xn(e,t,i,s=null){var Y,V;if(!e||e.length===0){const F=document.createElement("div");F.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:3000;gap:16px;color:#fff;padding:24px;text-align:center",F.innerHTML=`
  ...
  `,document.body.appendChild(c);let d=!1;const a=document.getElementById("pack-cut-zone"),l=document.getElementById("pack-blade");let u=!1;const o=F=>F.touches&&F.touches[0]?F.touches[0].clientX:F.clientX;function n(F){d||(u=!0,l.style.opacity="1",f(F))}function f(F){if(!u||d)return;const oe=a.getBoundingClientRect(),pe=o(F)-oe.left,ye=Math.max(0,Math.min(1,pe/oe.width));l.style.width=ye*oe.width+"px",ye>=.82&&h()}function v(){d||(u=!1,l.style.transition="width .2s ease, opacity .2s ease",l.style.width="0",l.style.opacity="0",setTimeout(()=>{d||(l.style.transition="")},220))}function h(){var oe;if(d)return;d=!0,u=!1,l.style.width="100%",l.style.opacity="1",(oe=document.getElementById("cut-flash"))==null||oe.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const F=document.getElementById("cut-hint");F&&(F.style.opacity="0"),a.classList.add("pack-cut"),setTimeout(()=>{l.style.opacity="0",document.getElementById("pack-phase").style.display="none",I(0)},620)}a.addEventListener("pointerdown",n),window.addEventListener("pointermove",f),window.addEventListener("pointerup",v),a.addEventListener("touchstart",n,{passive:!0}),window.addEventListener("touchmove",f,{passive:!0}),window.addEventListener("touchend",v);let T=0,r=!1;const p=new Set;function I(F){T=F,document.getElementById("reveal-phase").style.display="flex",z(),C(F,0),ie()}function z(){const F=document.getElementById("card-dots");F&&(F.innerHTML=e.map((oe,pe)=>`<div class="card-dot" data-i="${pe}" style="width:8px;height:8px;border-radius:50%;background:${pe===T?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),F.querySelectorAll(".card-dot").forEach(oe=>oe.addEventListener("click",()=>D(parseInt(oe.dataset.i)))))}function C(F,oe){var re;const pe=e[F],ye=document.getElementById("card-counter"),ze=document.getElementById("card-track");ye&&(ye.textContent=`Carte ${F+1} / ${e.length}`);const Ae=document.getElementById("reveal-btns");Ae&&(Ae.style.display=F===e.length-1?"flex":"none");const we=pe.card_type==="player"&&((re=pe.player)==null?void 0:re.rarity)==="legende",$e=!p.has(F);p.add(F);let Ee=0;if(pe.card_type==="player"&&pe.player){const O=pe.player,Z=O.job||"ATT";Ee=Number(Z==="GK"?O.note_g:Z==="DEF"?O.note_d:Z==="MIL"?O.note_m:O.note_a)||0}const te=O=>{ze.innerHTML=`
      ...
      <div style="flex:1;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06);margin:0">
        ${h?`<img src="${h}" style="max-width:120px;max-height:120px;width:auto;height:auto;object-fit:contain;border-radius:6px">`:`<span style="font-size:56px">${T}</span>`}
      </div>
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    </div>`}const u=n=>{e.style.overflow="",e.style.height="",e.style.display="",e.style.flexDirection="",i(n)};function o(){var f,v,h;e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column";const n=a.length>0;e.innerHTML=`
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      <div style="flex:1;overflow-y:auto;display:flex;flex-wrap:wrap;gap:10px;justify-content:center;align-content:flex-start;padding:8px 16px 16px">
        ${d.map(T=>{const r=a.find(p=>p.gc_type===T.gc_type);return l(T,!!r)}).join("")}
      </div>
    ...
    </div>`,e.querySelectorAll(".gc-select-card").forEach(T=>{T.addEventListener("click",()=>{const r=T.dataset.id,p=d.find(z=>z.id===r);if(!p)return;const I=a.findIndex(z=>z.gc_type===p.gc_type);I>-1?a.splice(I,1):a.length<3&&a.push(p),o()})}),(f=e.querySelector("#gc-launch"))==null||f.addEventListener("click",()=>{n&&u(a)}),(v=e.querySelector("#gc-no-gc"))==null||v.addEventListener("click",()=>u([])),(h=e.querySelector("#gc-reset"))==null||h.addEventListener("click",()=>{a.length&&(a=[],o())})}o()}function Eo(e){var s;const t=((s=e==null?void 0:e.state)==null?void 0:s.params)||{},i=t.matchMode||"vs_ai_easy";return i==="friend"?`Match vs ${t.friendName||"Ami"}`:i==="random"?"Match vs Random":`Match vs IA — ${i.replace("vs_ai_","").toUpperCase()}`}async function Ao(e,t,i){const{state:s,navigate:c}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:d}=await E.from("decks").select("id,name,is_active,formation").eq("owner_id",s.profile.id).order("created_at",{ascending:!1});if(!d||d.length===0){wt(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>c("decks"));return}const a=d.map(v=>v.id),{data:l}=await E.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,stadium_id,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order"),u=[...new Set((l||[]).filter(v=>{var h,T;return((h=v.card)==null?void 0:h.card_type)==="stadium"&&((T=v.card)==null?void 0:T.stadium_id)}).map(v=>v.card.stadium_id))],o={};if(u.length){const{data:v}=await E.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",u);(v||[]).forEach(h=>{o[h.id]=h}),(l||[]).forEach(h=>{var T,r;((T=h.card)==null?void 0:T.card_type)==="stadium"&&((r=h.card)!=null&&r.stadium_id)&&(h.card._stadiumDef=o[h.card.stadium_id]||null)})}let n=0;function f(){var D,N,G,ie,se,de,P;const v=d[n],h=(l||[]).filter(Y=>Y.deck_id===v.id),T=h.filter(Y=>{var V;return Y.is_starter&&((V=Y.card)==null?void 0:V.player)}).map(Y=>ui(Y.card,Y.position)),r=h.find(Y=>{var V;return((V=Y.card)==null?void 0:V.card_type)==="formation"}),p=v.formation||((D=r==null?void 0:r.card)==null?void 0:D.formation)||"4-4-2";let I=T.length>=11?yt(T,p):null,z=null;if(v.stadium_card_id&&I){const Y=h.find(V=>{var F;return((F=V.card)==null?void 0:F.id)===v.stadium_card_id});(N=Y==null?void 0:Y.card)!=null&&N.stadium_id&&(z=Y.card._stadiumDef||null),z&&(I=Ei(I,z))}const C=T.length>=11;et(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div style="display:flex;align-items:center;gap:8px;padding:8px;flex-shrink:0">
        ...
        <div style="flex:1;text-align:center">
          ...
          <div style="font-size:11px;opacity:.6;margin-top:2px">${p} · ${T.length}/11 ${v.is_active?"· ⭐ Actif":""}</div>
        </div>
    ...
    </div>`,function(){const V=e.querySelector(".deck-preview-wrap svg");V&&(V.removeAttribute("width"),V.removeAttribute("height"),V.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",V.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),(ie=document.getElementById("prev-deck"))==null||ie.addEventListener("click",()=>{n>0&&(n--,f())}),(se=document.getElementById("next-deck"))==null||se.addEventListener("click",()=>{n<d.length-1&&(n++,f())}),(de=document.getElementById("validate-deck"))==null||de.addEventListener("click",()=>{if(!C)return;const Y=t.state.params||{};t.navigate("match",{...Y,matchMode:Y.matchMode||i,deckId:v.id})}),(P=document.getElementById("cancel-deck-select"))==null||P.addEventListener("click",()=>{We(e),c("home")});const U=document.getElementById("deck-swipe-zone");if(U){let Y=0,V=0;U.addEventListener("touchstart",F=>{Y=F.touches[0].clientX,V=F.touches[0].clientY},{passive:!0}),U.addEventListener("touchend",F=>{const oe=F.changedTouches[0].clientX-Y,pe=F.changedTouches[0].clientY-V;Math.abs(oe)<40||Math.abs(oe)<Math.abs(pe)||(oe<0&&n<d.length-1?(n++,f()):oe>0&&n>0&&(n--,f()))},{passive:!0})}}f()}function bn(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";return e!=null&&e.clubLogo?e.clubLogo.startsWith("http")?e.clubLogo:`${t}/storage/v1/object/public/assets/clubs/${e.clubLogo}`:null}function Ye(e,t=44,i=58){if(!e)return`<div style="width:${t}px;height:${i}px;border:1.5px dashed rgba(255,255,255,0.2);border-radius:5px"></div>`;const s=typeof e.portrait=="string"&&e.portrait.startsWith("http")?e.portrait:Pe(e),c=bn(e),d=e._line||e.job||"MIL",a=It[d]||"#555",l={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",u=e.note!==void 0?Number(e.note)||0:(Number(Te(e,d))||0)+(e.boost||0),o=hn(e==null?void 0:e.country_code),n=Math.round(i*.19),f=Math.round(i*.25),v=i-Math.round(i*.19)-Math.round(i*.25),h=e!=null&&e.used?.28:1;return`<div style="width:${t}px;height:${i}px;border-radius:5px;border:2px solid ${l};background:${a};position:relative;overflow:hidden;flex-shrink:0;opacity:${h}">
    <div style="position:absolute;top:0;left:0;right:0;height:${n}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:center;z-index:2">
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;height:${f}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ...
      ${c?`<img src="${c}" style="width:${f-4}px;height:${f-4}px;object-fit:contain">`:e!=null&&e.clubName?`<span style="font-size:${Math.max(4,f-8)}px;font-weight:700;color:#333">${(e.clubName||"").slice(0,3).toUpperCase()}</span>`:""}
        ...
        stroke="${de}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const h=48,T=64,r=13,p=16,I={normal:"#aaaaaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#111111",legende:"#7a28b8",légende:"#7a28b8"};for(const[U,D]of Object.entries(o)){const N=f(U);if(!N||!D)continue;const G=U.replace(/[0-9]/g,""),ie=It[G]||"#555",se=a.includes(D.cardId),de=i==="attack"&&(["MIL","ATT"].includes(G)||se)&&!D.used||i==="defense"&&["GK","DEF","MIL"].includes(G)&&!D.used,P=s.includes(D.cardId);let Y;i==="attack"?Y=se?Math.max(1,Number(D.note_a)||0):G==="MIL"?Number(D.note_m)||0:Number(D.note_a)||0:i==="defense"?Y=G==="GK"?Number(D.note_g)||0:G==="MIL"?Number(D.note_m)||0:Number(D.note_d)||0:Y=Number(G==="GK"?D.note_g:G==="DEF"?D.note_d:G==="MIL"?D.note_m:D.note_a)||0,Y=Y+(D.boost||0);const V=(N.x-h/2).toFixed(1),F=(N.y-T/2).toFixed(1),oe=I[D==null?void 0:D.rarity]||I.normal;if(D.used){const Ee=typeof import.meta<"u"&&"/manager-wars/"||"/",re=`${typeof window<"u"&&((C=window.location)==null?void 0:C.origin)||""}${Ee}icons/carte-dos.png`.replace(/([^:])\/\//g,"$1/");v+=`<rect x="${V}" y="${F}" width="${h}" height="${T}" rx="5" fill="#161616"/>`,v+=`<image href="${re}" xlink:href="${re}" x="${V}" y="${F}" width="${h}" height="${T}" preserveAspectRatio="xMidYMid slice"/>`,v+=`<rect x="${V}" y="${F}" width="${h}" height="${T}" rx="5" fill="none" stroke="${oe}" stroke-width="2" opacity="0.7"/>`,v+=`<rect x="${V}" y="${F}" width="${h}" height="${T}" rx="5" fill="rgba(0,0,0,0.01)" class="match-used-hit" data-card-id="${D.cardId}" data-role="${G}" style="cursor:pointer"/>`;continue}const pe=P?.45:1,ye=P?"#FFD700":oe,ze=P?3:(D==null?void 0:D.rarity)==="legende"||(D==null?void 0:D.rarity)==="légende"||(D==null?void 0:D.rarity)==="pepite"||(D==null?void 0:D.rarity)==="pépite"?2.5:2,Ae=T-r-p;v+=`<defs><clipPath id="cp-${U}"><rect x="${V}" y="${(N.y-T/2+r).toFixed(1)}" width="${h}" height="${Ae}"/></clipPath></defs>`,v+=`<rect x="${V}" y="${F}" width="${h}" height="${T}" rx="5" fill="${ie}" opacity="${pe}"/>`;const we=Pe(D);we&&(v+=`<image href="${we}" xlink:href="${we}" x="${V}" y="${(N.y-T/2+r).toFixed(1)}" width="${h}" height="${Ae}" clip-path="url(#cp-${U})" preserveAspectRatio="xMidYMin slice"/>`),v+=`<rect x="${V}" y="${F}" width="${h}" height="${r}" rx="4" fill="rgba(255,255,255,0.92)"/>`,v+=`<text x="${N.x.toFixed(1)}" y="${(N.y-T/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(D.name||"").slice(0,9)}</text>`;const $e=(N.y+T/2-p).toFixed(1);v+=`<rect x="${V}" y="${$e}" width="${h}" height="${p}" fill="rgba(255,255,255,0.92)"/>`;{const Ee=Xt(D.country_code);Ee?v+=`<image href="${Ee}" xlink:href="${Ee}" x="${(N.x-20).toFixed(1)}" y="${(N.y+T/2-p+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:v+=`<text x="${(N.x-13).toFixed(1)}" y="${(N.y+T/2-p/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${hn(D.country_code)}</text>`,v+=`<text x="${N.x.toFixed(1)}" y="${(N.y+T/2-p/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${Y}</text>`;const te=bn(D);te?v+=`<image href="${te}" xlink:href="${te}" x="${(N.x+h/2-14).toFixed(1)}" y="${(N.y+T/2-p+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:D.clubName&&(v+=`<text x="${(N.x+14).toFixed(1)}" y="${(N.y+T/2-p/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(D.clubName||"").slice(0,3).toUpperCase()}</text>`),D.boost&&(v+=`<rect x="${(N.x+h/2-12).toFixed(1)}" y="${(N.y-T/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,v+=`<text x="${(N.x+h/2-5).toFixed(1)}" y="${(N.y-T/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${D.boost}</text>`)}v+=`<rect x="${V}" y="${F}" width="${h}" height="${T}" rx="5" fill="${P?"rgba(255,255,255,0.12)":"none"}" stroke="${ye}" stroke-width="${ze}" opacity="${pe}"/>`,de&&(v+=`<rect x="${V}" y="${F}" width="${h}" height="${T}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${P?"selected":""}" data-card-id="${D.cardId}" data-role="${G}" style="cursor:pointer"/>`)}const z=38;return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-z} ${-z} ${c+z*2} ${d+z*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${v}
  ...
  </div>`}async function ni(e,t,i,s){var N;const{state:c,navigate:d,toast:a}=t;et(e);const l=c.params||{};if(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!l.deckId)return Ao(e,t,i);const u=l.deckId;let o,n,f,v;try{const G=await Promise.all([E.from("decks").select("formation,name").eq("id",u).single(),E.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation,
            player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
              clubs(encoded_name,logo_url)))`).eq("deck_id",u).order("slot_order")]);o=G[0].data,f=G[0].error,n=G[1].data,v=G[1].error}catch(G){console.error("[Match] Exception chargement deck:",G),wt(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>d("home"));return}if(f||v){console.error("[Match] Erreur Supabase:",f||v),wt(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>d("home"));return}const h=(n||[]).filter(G=>{var ie;return G.is_starter&&((ie=G.card)==null?void 0:ie.player)}).map(G=>ui(G.card,G.position)),T=(n||[]).filter(G=>{var ie;return!G.is_starter&&((ie=G.card)==null?void 0:ie.player)}).map(G=>ui(G.card,G.position));if(h.length<11){wt(e,"⚠️",`Deck incomplet (${h.length}/11).`,"Compléter",()=>d("decks"));return}const r=(n||[]).find(G=>{var ie;return((ie=G.card)==null?void 0:ie.card_type)==="formation"}),p=(o==null?void 0:o.formation)||((N=r==null?void 0:r.card)==null?void 0:N.formation)||"4-4-2",{data:I,error:z}=await E.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",c.profile.id).eq("card_type","game_changer"),{data:C}=await E.from("gc_definitions").select("*").eq("is_active",!0),U=(I||[]).map(G=>({...G,_gcDef:(C==null?void 0:C.find(ie=>ie.name===G.gc_type||ie.id===G.gc_definition_id))||null}));let D=null;if(deck!=null&&deck.stadium_card_id){const{data:G}=await E.from("cards").select("stadium_id").eq("id",deck.stadium_card_id).single();if(G!=null&&G.stadium_id){const{data:ie}=await E.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").eq("id",G.stadium_id).single();D=ie||null}}s({deckId:u,formation:p,starters:h,subsRaw:T,gcCardsEnriched:U,gcDefs:C||[],stadiumDef:D})}async function To(e,t){const{state:i}=t,c=(i.params||{}).matchMode||"vs_ai_easy",d=c.replace("vs_ai_",""),a=c;await ni(e,t,c,async({deckId:l,formation:u,starters:o,subsRaw:n,gcCardsEnriched:f,gcDefs:v,stadiumDef:h})=>{try{let T=yt(o,u);h&&(T=Ei(T,h));const r=await zo(u,d),p=async I=>{try{const{data:z,error:C}=await E.from("matches").insert({home_id:i.profile.id,away_id:null,mode:a,home_deck_id:l,status:"in_progress"}).select().single();if(C){console.error("[MatchIA] Erreur création match:",C),wt(e,"⚠️","Impossible de créer le match ("+C.message+").","Retour",()=>t.navigate("home"));return}const U={gcDefs:v||[],matchId:z==null?void 0:z.id,mode:a,difficulty:d,formation:u,homeTeam:T,aiTeam:r,homeSubs:n,subsUsed:0,maxSubs:Math.min(n.length,3),homeScore:0,aiScore:0,gcCards:I,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Lo(e,U,t)}catch(z){console.error("[MatchIA] Exception launchMatch:",z),wt(e,"⚠️","Erreur au lancement du match : "+z.message,"Retour",()=>t.navigate("home"))}};if(!f.length){p([]);return}ii(e,f,p)}catch(T){console.error("[MatchIA] Exception setup:",T),wt(e,"⚠️","Erreur de préparation du match : "+T.message,"Retour",()=>t.navigate("home"))}})}async function zo(e,t){var a,l;const{data:i}=await E.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(60);if(!i||i.length<11)return So(e);const s=Lt[e]||Lt["4-4-2"],c={GK:[],DEF:[],MIL:[],ATT:[]},d=[...i];for(const u of["GK","DEF","MIL","ATT"]){const o=d.filter(T=>T.job===u),n=d.filter(T=>T.job!==u),f=[...o,...n],v=[];for(let T=0;T<s[u];T++){const r=f[T]||d[T];r&&v.push({cardId:"ai-"+r.id+"-"+T,id:r.id,firstname:r.firstname,name:r.surname_encoded,country_code:r.country_code,club_id:r.club_id,job:r.job,job2:r.job2,note_g:Number(r.note_g)||0,note_d:Number(r.note_d)||0,note_m:Number(r.note_m)||0,note_a:Number(r.note_a)||0,rarity:r.rarity,skin:r.skin,hair:r.hair,hair_length:r.hair_length,clubName:((a=r.clubs)==null?void 0:a.encoded_name)||null,clubLogo:((l=r.clubs)==null?void 0:l.logo_url)||null,boost:0,used:!1,_line:u})}const h=lt(v.length);v.forEach((T,r)=>{T._col=h[r]}),c[u]=v}return c}function So(e){const t=Lt[e]||Lt["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},s=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let c=0;for(const d of["GK","DEF","MIL","ATT"]){const a=[];for(let u=0;u<t[d];u++){const o=3+Math.floor(Math.random()*5);a.push({cardId:"fake-"+c,id:"fake-"+c,firstname:"IA",name:s[c%s.length],country_code:"XX",club_id:null,job:d,job2:null,note_g:d==="GK"?o:2,note_d:d==="DEF"?o:2,note_m:d==="MIL"?o:2,note_a:d==="ATT"?o:2,rarity:"normal",boost:0,used:!1,_line:d}),c++}const l=lt(a.length);a.forEach((u,o)=>{u._col=l[o]}),i[d]=a}return i}function Lo(e,t,i){e.innerHTML=`
  ...
  <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div id="duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center, rgba(10,61,30,0.4), rgba(10,61,30,0.92))"></div>
          ...
          <div style="background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:3px solid #cdeffd;border-radius:18px;padding:20px 34px;text-align:center;animation:boostFlipIn .7s cubic-bezier(.34,1.56,.64,1) both;box-shadow:0 10px 36px rgba(135,206,235,0.5)">
            ...
            <div style="font-size:50px;font-weight:900;color:#063;line-height:1.1">+${h}</div>
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    ${(()=>{const P=window.innerWidth>=700,Y=($e,Ee,te)=>{var qe,De;const re=(t.gcDefs||[]).find(M=>M.name===$e.gc_type),O={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[re==null?void 0:re.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",Z={purple:"#b06ce0",light_blue:"#00d4ef"}[re==null?void 0:re.color]||"#b06ce0",ue=(re==null?void 0:re.name)||$e.gc_type,me=(re==null?void 0:re.effect)||((qe=Ke[$e.gc_type])==null?void 0:qe.desc)||"",X=re!=null&&re.image_url?`/manager-wars/icons/${re.image_url}`:null,be=((De=Ke[$e.gc_type])==null?void 0:De.icon)||"⚡",ge=Math.round(te*.22),Se=Math.round(te*.22),Ce=te-ge-Se,Fe=ue.length>12?7:9;return`<div class="gc-mini" data-gc-id="${$e.id}" data-gc-type="${$e.gc_type}"
          ...
          </div>`},F=($e,Ee)=>Ee?V(130,175):Y($e,130,175),oe=($e,Ee)=>Ee?V(68,95):Y($e,68,95),pe=P?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",ye=T?`<button id="btn-results" style="${pe};background:linear-gradient(135deg,#D4A017,#FFD700);border:none;color:#000">🏁 Résultats</button>`:f?`<div style="${pe};background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);color:rgba(255,255,255,0.4)">⏳ Tour IA</div>`:v?`<button id="btn-action" style="${pe};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${t.selected.length===0?"disabled":""}> ⚔️ ATTAQUEZ <span id="match-timer" style="font-weight:900"></span></button>`:h?`<button id="btn-action" style="${pe};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${t.selected.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="match-timer" style="font-weight:900"></span></button>`:`<div style="${pe};background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1)"></div>`,ze=v||h?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${t.selected.length}/3 sélectionné(s)</div>`:"",Ae=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${P?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
  ...
  `,document.body.appendChild(s);let n=!1;const f=()=>{n||(n=!0,s.remove(),i())};s.addEventListener("click",f),setTimeout(f,2e3)}function Mt(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function ai(e,t,i,s=null,c=null){var h,T;if(t.phase!=="attack"){Mt("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){Mt(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const d=Object.entries(t.homeTeam).flatMap(([r,p])=>(p||[]).filter(I=>I.used).map(I=>({...I,_line:I._line||r}))),a=t.homeSubs.filter(r=>!t.usedSubIds.includes(r.cardId));if(!d.length){Mt("Aucun joueur utilisé à remplacer");return}if(!a.length){Mt("Aucun remplaçant disponible");return}let l=Math.max(0,d.findIndex(r=>r.cardId===c));const u=((h=d[l])==null?void 0:h._line)||((T=d[l])==null?void 0:T.job);let o=s?Math.max(0,a.findIndex(r=>r.cardId===s)):Math.max(0,a.findIndex(r=>r.job===u)),n=!1;const f=document.createElement("div");f.id="sub-overlay",f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function v(){var D,N,G,ie,se,de;const r=d[l],p=a[o],I=Math.min(130,Math.round((window.innerWidth-90)/2)),z=Math.round(I*1.35),C=P=>`background:rgba(255,255,255,0.12);border:none;color:${P?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${P?"default":"pointer"};flex-shrink:0`;f.innerHTML=`
    ...
    </div>`,(D=f.querySelector("#sub-close"))==null||D.addEventListener("click",()=>f.remove()),(N=f.querySelector("#out-up"))==null||N.addEventListener("click",()=>{l>0&&(l--,v())}),(G=f.querySelector("#out-down"))==null||G.addEventListener("click",()=>{l<d.length-1&&(l++,v())}),(ie=f.querySelector("#in-up"))==null||ie.addEventListener("click",()=>{o>0&&(o--,v())}),(se=f.querySelector("#in-down"))==null||se.addEventListener("click",()=>{o<a.length-1&&(o++,v())});const U=(P,Y,V,F)=>{const oe=f.querySelector("#"+P);if(!oe)return;let pe=0;oe.addEventListener("touchstart",ye=>{pe=ye.touches[0].clientY},{passive:!0}),oe.addEventListener("touchend",ye=>{const ze=ye.changedTouches[0].clientY-pe;if(Math.abs(ze)<30)return;const Ae=Y();ze<0&&Ae<F-1?(V(Ae+1),v()):ze>0&&Ae>0&&(V(Ae-1),v())},{passive:!0})};U("in-panel",()=>o,P=>o=P,a.length),U("out-panel",()=>l,P=>l=P,d.length),(de=f.querySelector("#sub-confirm"))==null||de.addEventListener("click",P=>{if(P.preventDefault(),P.stopPropagation(),n)return;n=!0;const Y=d[l],V=a[o];if(!Y||!V)return;let F=null,oe=-1;for(const[ye,ze]of Object.entries(t.homeTeam)){const Ae=(ze||[]).findIndex(we=>we.cardId===Y.cardId);if(Ae!==-1){F=ye,oe=Ae;break}}if(oe===-1||!F){Mt("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),f.remove();return}const pe={...V,_line:F,_col:Y._col||0,used:!1,boost:0};t.homeTeam[F].splice(oe,1,pe),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(V.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:Y.name,firstname:Y.firstname,note:Te(Y,F),portrait:Pe(Y),job:Y.job,country_code:Y.country_code,rarity:Y.rarity,clubName:Y.clubName,clubLogo:Y.clubLogo},inPlayer:{name:V.name,firstname:V.firstname,note:Te(V,F),portrait:Pe(V),job:V.job,country_code:V.country_code,rarity:V.rarity,clubName:V.clubName,clubLogo:V.clubLogo},text:`🔄 ${V.firstname} ${V.name} remplace ${Y.firstname} ${Y.name}`}),f.remove(),Fo(Y,V,()=>Xe(e,t,i))})}document.body.appendChild(f),v()}function Do(e,t,i,s,c){var T,r;const d=(s.gcDefs||[]).find(p=>p.name===t),a=Ke[t]||{icon:"⚡",desc:"Carte spéciale."},l={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[d==null?void 0:d.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",u={purple:"#b06ce0",light_blue:"#00d4ef"}[d==null?void 0:d.color]||"#b06ce0",o=(d==null?void 0:d.name)||t,n=(d==null?void 0:d.effect)||a.desc,f=d!=null&&d.image_url?`/manager-wars/icons/${d.image_url}`:null,v=a.icon||"⚡",h=document.createElement("div");h.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",h.innerHTML=`
    ...
    </div>`,document.body.appendChild(h),(T=h.querySelector("#gc-back"))==null||T.addEventListener("click",()=>h.remove()),(r=h.querySelector("#gc-use"))==null||r.addEventListener("click",()=>{h.remove(),Po(e,t,i,s,c)})}function Kt(e,t,i,s,c,d){const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let l=[];function u(){var o,n;a.innerHTML=`
    ...
    <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
      ${e.map(f=>{const v=f._line||f.job||"MIL",h={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[v]||"#555",T=Te(f,v)+(f.boost||0),r=l.find(p=>p.cardId===f.cardId);return`<div class="gc-pick-item" data-cid="${f.cardId}"
          style="width:80px;border-radius:8px;border:2.5px solid ${r?"#FFD700":"rgba(255,255,255,0.25)"};background:${h};overflow:hidden;cursor:pointer;flex-shrink:0;${f.used?"opacity:0.3;pointer-events:none":""}">
    ...
    </div>`,(o=a.querySelector("#gc-picker-close"))==null||o.addEventListener("click",()=>a.remove()),a.querySelectorAll(".gc-pick-item").forEach(f=>{f.addEventListener("click",()=>{const v=f.dataset.cid,h=e.find(r=>r.cardId===v);if(!h)return;const T=l.findIndex(r=>r.cardId===v);T>-1?l.splice(T,1):l.length<t&&l.push(h),u()})}),(n=a.querySelector("#gc-picker-confirm"))==null||n.addEventListener("click",()=>{a.remove(),d(l)})}u(),document.body.appendChild(a)}const Go={BOOST_STAT:({value:e=1,count:t=1,roles:i=[]},s,c,d)=>{const a=Object.entries(s.homeTeam).filter(([l])=>!i.length||i.includes(l)).flatMap(([l,u])=>u.filter(o=>!o.used).map(o=>({...o,_line:l})));return a.length?(Kt(a,t,`Choisir ${t} joueur(s) à booster (+${e})`,c,s,l=>{l.forEach(u=>{const o=(s.homeTeam[u._line]||[]).find(n=>n.cardId===u.cardId);o&&(o.boost=(o.boost||0)+e,s.log.push({text:`⚡ +${e} sur ${o.name}`,type:"info"}))}),Xe(c,s,d)}),!0):(s.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),Xe(c,s,d),!0)},DEBUFF_STAT:({value:e=1,count:t=1,roles:i=[],target:s="ai"},c,d,a)=>{const l=s==="home"?c.homeTeam:c.aiTeam,u=s==="ai"?"adverse":"allié",o=Object.entries(l).filter(([n])=>!i.length||i.includes(n)).flatMap(([n,f])=>f.filter(v=>!v.used).map(v=>({...v,_line:n})));return o.length?(Kt(o,t,`Choisir ${t} joueur(s) ${u}(s) à débuffer (-${e})`,d,c,n=>{n.forEach(f=>{const h=((s==="home"?c.homeTeam:c.aiTeam)[f._line]||[]).find(T=>T.cardId===f.cardId);h&&(h.boost=(h.boost||0)-e,c.log.push({text:`🎯 -${e} sur ${h.name}${s==="ai"?" (IA)":""}`,type:"info"}))}),Xe(d,c,a)}),!0):(c.log.push({text:`🎯 Aucun joueur ${u} disponible`,type:"info"}),Xe(d,c,a),!0)},GRAY_PLAYER:({count:e=1,roles:t=[],target:i="ai"},s,c,d)=>{const a=i==="home"?s.homeTeam:s.aiTeam,l=i==="ai"?"adverse":"allié",u=Object.entries(a).filter(([o])=>!t.length||t.includes(o)).flatMap(([o,n])=>n.filter(f=>!f.used).map(f=>({...f,_line:o})));return u.length?(Kt(u,e,`Choisir ${e} joueur(s) ${l}(s) à exclure`,c,s,o=>{o.forEach(n=>{const v=((i==="home"?s.homeTeam:s.aiTeam)[n._line]||[]).find(h=>h.cardId===n.cardId);v&&(v.used=!0,s.log.push({text:`❌ ${v.name}${i==="ai"?" (IA)":""} exclu !`,type:"info"}))}),Xe(c,s,d)}),!0):(s.log.push({text:`❌ Aucun joueur ${l} à exclure`,type:"info"}),Xe(c,s,d),!0)},REVIVE_PLAYER:({count:e=1,roles:t=[]},i,s,c)=>{const d=Object.entries(i.homeTeam).filter(([a])=>!t.length||t.includes(a)).flatMap(([a,l])=>l.filter(u=>u.used).map(u=>({...u,_line:a})));return d.length?(Kt(d,e,`Choisir ${e} joueur(s) à ressusciter`,s,i,a=>{a.forEach(l=>{const u=(i.homeTeam[l._line]||[]).find(o=>o.cardId===l.cardId);u&&(u.used=!1,i.log.push({text:`💫 ${u.name} ressuscité !`,type:"info"}))}),Xe(s,i,c)}),!0):(i.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),Xe(s,i,c),!0)},REMOVE_GOAL:({},e)=>(e.aiScore>0?(e.aiScore--,e.log.push({text:"🚫 Dernier but IA annulé !",type:"info"})):e.log.push({text:"🚫 Aucun but à annuler",type:"info"}),!1),ADD_GOAL_DRAW:({},e)=>(e.homeScore===e.aiScore?(e.homeScore++,e.log.push({text:"🎯 But bonus (match nul) !",type:"info"})):e.log.push({text:"🎯 But bonus : non applicable (pas de match nul)",type:"info"}),!1),ADD_SUB:({value:e=1},t)=>(t.maxSubs=(t.maxSubs||3)+e,t.log.push({text:`🔄 +${e} remplacement(s) débloqué(s)`,type:"info"}),!1),CUSTOM:()=>!1};function Po(e,t,i,s,c){s.usedGc.push(e);const d=s.gcDefs||[],a=d.find(u=>u.name===t)||d.find(u=>{var o;return((o=u.name)==null?void 0:o.toLowerCase().trim())===(t==null?void 0:t.toLowerCase().trim())});let l=!1;if(a!=null&&a.effect_type&&a.effect_type!=="CUSTOM"){const u=Go[a.effect_type];u?u(a.effect_params||{},s,i,c)||(l=!0):(c.toast(`Effet "${a.effect_type}" non implémenté`,"error"),l=!0)}else{switch(t){case"Double attaque":s.modifiers.home.doubleAttack=!0,s.log.push({text:"⚡ Double attaque activée !",type:"info"});break;case"Bouclier":s.modifiers.home.shield=!0,s.log.push({text:"🛡️ Bouclier activé !",type:"info"});break;case"Ressusciter":{const u=Object.entries(s.homeTeam).flatMap(([o,n])=>(n||[]).filter(f=>f.used).map(f=>({...f,_line:o})));u.length?(u[0].used=!1,s.log.push({text:`💫 ${u[0].name} ressuscité !`,type:"info"})):s.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"});break}case"Vol de note":s.modifiers.ai.stolenNote=(s.modifiers.ai.stolenNote||0)+1,s.log.push({text:"🎯 -1 à la prochaine attaque IA",type:"info"});break;case"Gel":{const u=[...s.aiTeam.ATT||[],...s.aiTeam.MIL||[]].filter(o=>!o.used);if(u.length){const o=u.sort((n,f)=>Te(f,"ATT")-Te(n,"ATT"))[0];o.used=!0,s.log.push({text:`❄️ ${o.name} (IA) gelé !`,type:"info"})}break}case"Remplacement+":s.maxSubs++,s.log.push({text:"🔄 +1 remplacement débloqué",type:"info"});break}l=!0}E.from("cards").delete().eq("id",e).then(()=>{}),l&&Xe(i,s,c)}function No(e,t,i){const s=Object.values(t.homeTeam).flat().filter(c=>!c.used);if(!s.length){i.toast("Aucun joueur actif à booster","error");return}i.openModal("⚡ Utiliser le Boost",`<div style="margin-bottom:12px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border-radius:10px;padding:12px;text-align:center;color:#000">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(c=>{c.addEventListener("click",()=>{const d=c.dataset.cardId;for(const a of["GK","DEF","MIL","ATT"]){const l=(t.homeTeam[a]||[]).find(u=>u.cardId===d);if(l){l.boost=(l.boost||0)+t.boostCard.value,t.log.push({text:`⚡ Boost +${t.boostCard.value} appliqué à ${l.name}`,type:"info"});break}}t.boostUsed=!0,i.closeModal(),Xe(e,t,i)})})}function Zt(e,t,i,s,c){const d=document.createElement("div");d.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:900;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;overflow:hidden;cursor:pointer";const a=Array.from({length:10},(o,n)=>`
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(d);let l=!1;const u=()=>{l||(l=!0,d.remove(),setTimeout(()=>c(),50))};d.addEventListener("click",u),setTimeout(u,3500)}async function qt(e,t,i){var n,f;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:s}=i,c=t.homeScore>t.aiScore,d=t.homeScore===t.aiScore,a=c?"victoire":d?"nul":"defaite",l=Vn(t.mode,a);t.matchId&&await E.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:c?s.profile.id:null,home_credits_reward:l,played_at:new Date().toISOString()}).eq("id",t.matchId);const u={credits:(s.profile.credits||0)+l,matches_played:(s.profile.matches_played||0)+1};c?u.wins=(s.profile.wins||0)+1:d?u.draws=(s.profile.draws||0)+1:u.losses=(s.profile.losses||0)+1,await E.from("users").update(u).eq("id",s.profile.id),await i.refreshProfile();const o=document.createElement("div");o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",o.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const T=async(U=!0)=>{v=!0,h&&(E.removeChannel(h),h=null),U&&await E.rpc("cancel_matchmaking",{p_user_id:o.profile.id}).catch(()=>{})};(C=document.getElementById("mm-cancel"))==null||C.addEventListener("click",async()=>{try{await T(!0)}catch{}We(e),n("home")});const r=async(U,D,N)=>{if(v)return;v=!0,h&&(E.removeChannel(h),h=null);const G=document.getElementById("mm-status");G&&(G.textContent="Adversaire trouvé !"),await new Promise(ie=>setTimeout(ie,600)),e.isConnected&&_n(e,t,U,N,a,l)},{data:p,error:I}=await E.rpc("try_matchmake",{p_user_id:o.profile.id,p_deck_id:i});if(I||!(p!=null&&p.success)){f("Erreur de matchmaking","error"),We(e),n("home");return}if(p.matched){r(p.match_id,p.opponent_id,!1);return}const z=document.getElementById("mm-status");z&&(z.textContent="En attente d'un autre joueur..."),h=E.channel("matchmaking-"+o.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${o.profile.id}`},U=>{const D=U.new;D.status==="matched"&&D.match_id&&r(D.match_id,D.matched_with,!0)}).subscribe()}async function Ho(e,t,i){const{state:s,navigate:c,toast:d}=t;try{(E.getChannels?E.getChannels():[]).forEach(o=>{const n=o.topic||"";(n.includes("matchmaking")||n.includes("pvp-match")||n.includes("friend-invite"))&&E.removeChannel(o)})}catch{}const{data:a}=await E.from("matches").select("home_id, away_id, status").eq("id",i).single();if(!a){d("Match introuvable","error"),c("home");return}if(a.status==="finished"){d("Ce match est terminé","info"),c("home");return}const l=a.home_id===s.profile.id;_n(e,t,i,l,[],[])}async function _n(e,t,i,s,c=[],d=[]){const{state:a,navigate:l,toast:u}=t,o=s?"p1":"p2",n=s?"p2":"p1",f=(c||[]).map(M=>M.id),v=(c||[]).map(M=>({id:M.id,gc_type:M.gc_type,_gcDef:M._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:h}=await E.from("matches").select("*").eq("id",i).single();if(!h){u("Match introuvable","error"),l("home");return}async function T(){const[{data:M},{data:q},{data:w},{data:k}]=await Promise.all([E.rpc("get_deck_for_match",{p_deck_id:h.home_deck_id}),E.rpc("get_deck_for_match",{p_deck_id:h.away_deck_id}),E.from("users").select("id,pseudo,club_name").eq("id",h.home_id).single(),E.from("users").select("id,pseudo,club_name").eq("id",h.away_id).single()]),g=_=>({cardId:_.card_id,position:_.position,id:_.id,firstname:_.firstname,name:_.surname_encoded,country_code:_.country_code,club_id:_.club_id,job:_.job,job2:_.job2,note_g:Number(_.note_g)||0,note_d:Number(_.note_d)||0,note_m:Number(_.note_m)||0,note_a:Number(_.note_a)||0,rarity:_.rarity,skin:_.skin,hair:_.hair,hair_length:_.hair_length,clubName:_.club_encoded_name||null,clubLogo:_.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}),m=((M==null?void 0:M.starters)||[]).map(_=>g(_)),x=((q==null?void 0:q.starters)||[]).map(_=>g(_)),y=(M==null?void 0:M.formation)||"4-4-2",b=(q==null?void 0:q.formation)||"4-4-2";let $=yt(m,y),A=yt(x,b);return stadiumDef&&s&&($=Ei($,stadiumDef)),{p1Team:$,p2Team:A,p1Subs:((M==null?void 0:M.subs)||[]).map(_=>g(_)),p2Subs:((q==null?void 0:q.subs)||[]).map(_=>g(_)),p1Formation:y,p2Formation:b,p1Name:(w==null?void 0:w.club_name)||(w==null?void 0:w.pseudo)||"Joueur 1",p2Name:(k==null?void 0:k.club_name)||(k==null?void 0:k.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:s?f:[],gc_p2:s?[]:f,gcCardsFull_p1:s?v:[],gcCardsFull_p2:s?[]:v,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:d||[],lastActionAt:new Date().toISOString()}}let r=h.game_state&&Object.keys(h.game_state).length?h.game_state:null;if(!r)if(s){r=await T();const{data:M}=await E.from("matches").select("game_state").eq("id",i).single();!(M!=null&&M.game_state)||!Object.keys(M.game_state).length?await E.from("matches").update({game_state:r,turn_user_id:h.home_id}).eq("id",i):r=M.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let M=0;M<30&&!r;M++){await new Promise(w=>setTimeout(w,400));const{data:q}=await E.from("matches").select("game_state").eq("id",i).single();q!=null&&q.game_state&&Object.keys(q.game_state).length&&(r=q.game_state)}if(!r){u("Erreur de synchronisation","error"),l("home");return}r.gc_p2=f,r.gcCardsFull_p2=v,await E.from("matches").update({game_state:r}).eq("id",i)}let p=!1,I=null,z=!1;const C=new Set,U=new Set;function D(M){var $,A;try{E.removeChannel(N)}catch{}const q=r[o+"Score"]||0,w=r[n+"Score"]||0;let k,g;M.winner_id?(k=M.winner_id===a.profile.id,g=!1):M.forfeit?(k=q>w,g=!1):(g=q===w,k=q>w),($=document.getElementById("pvp-end-overlay"))==null||$.remove();const m=document.createElement("div");m.id="pvp-end-overlay",m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const x=g?"🤝":k?"🏆":"😞",y=g?"MATCH NUL":k?"VICTOIRE !":"DÉFAITE",b=g?"#fff":k?"#FFD700":"#ff6b6b";m.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(m),(A=m.querySelector("#pvp-end-home"))==null||A.addEventListener("click",()=>{m.remove(),We(e),l("home")})}const N=E.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},M=>{const q=M.new;try{if(q.status==="finished"||q.forfeit){if(p)return;p=!0,I&&(clearInterval(I),I=null),q.game_state&&(r=q.game_state),D(q);return}if(q.game_state){const w=r;r=q.game_state;const k=r._lastGC;if(k&&k.seq&&!U.has(k.seq)&&(U.add(k.seq),k.by!==o)){Ae(k.type,k.by,()=>F());return}const g=w[o+"Score"]||0,m=w[n+"Score"]||0,x=r[o+"Score"]||0,y=r[n+"Score"]||0,b=x>g,$=y>m;if((b||$)&&!C.has(r.round)){C.add(r.round);const A=[...r.log||[]].reverse().find(S=>S.isGoal),_=((A==null?void 0:A.homePlayers)||[]).map(S=>({name:S.name,note:S.note,portrait:S.portrait,job:S.job}));ze(_,x,y,b,()=>F());return}F()}}catch(w){console.error("[PvP] crash:",w)}}).subscribe();async function G(M){Object.assign(r,M),r.lastActionAt=new Date().toISOString();const{error:q}=await E.from("matches").update({game_state:r}).eq("id",i);q&&u("Erreur de synchronisation","error");try{F()}catch(w){console.error("[PvP] renderPvpScreen crash:",w)}}async function ie(){if(p)return;p=!0,I&&(clearInterval(I),I=null);const M=s?h.away_id:h.home_id,q=s?"p2":"p1",w=s?"p1":"p2",k={...r,[q+"Score"]:3,[w+"Score"]:0,phase:"finished"};try{await E.from("matches").update({status:"finished",forfeit:!0,winner_id:M,home_score:k.p1Score||0,away_score:k.p2Score||0,game_state:k}).eq("id",i)}catch{}try{E.removeChannel(N)}catch{}setTimeout(()=>{We(e),l("home")},800)}const se={BOOST_STAT:({value:M=1,count:q=1,roles:w=[]},k,g)=>{const m=k[o+"Team"],x=Object.entries(m).filter(([y])=>!w.length||w.includes(y)).flatMap(([y,b])=>b.filter($=>!$.used).map($=>({...$,_line:y})));if(!x.length){k.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),g(k);return}de(x,q,`Choisir ${q} joueur(s) à booster (+${M})`,y=>{y.forEach(b=>{const $=(m[b._line]||[]).find(A=>A.cardId===b.cardId);$&&($.boost=($.boost||0)+M,k.log.push({text:`⚡ +${M} sur ${$.name}`,type:"info"}))}),k[o+"Team"]=m,g(k)})},DEBUFF_STAT:({value:M=1,count:q=1,roles:w=[],target:k="ai"},g,m)=>{const x=k==="home"?o:n,y=g[x+"Team"],b=k==="home"?"allié":"adverse",$=Object.entries(y).filter(([A])=>!w.length||w.includes(A)).flatMap(([A,_])=>_.map(S=>({...S,_line:A})));if(!$.length){g.log.push({text:`🎯 Aucun joueur ${b}`,type:"info"}),m(g);return}de($,q,`Choisir ${q} joueur(s) ${b}(s) (-${M})`,A=>{A.forEach(_=>{const S=(y[_._line]||[]).find(L=>L.cardId===_.cardId);S&&(S.boost=(S.boost||0)-M,g.log.push({text:`🎯 -${M} sur ${S.name}`,type:"info"}))}),g[x+"Team"]=y,m(g)})},GRAY_PLAYER:({count:M=1,roles:q=[],target:w="ai"},k,g)=>{const m=w==="home"?o:n,x=k[m+"Team"],y=w==="home"?"allié":"adverse",b=Object.entries(x).filter(([$])=>!q.length||q.includes($)).flatMap(([$,A])=>A.filter(_=>!_.used).map(_=>({..._,_line:$})));if(!b.length){k.log.push({text:`❌ Aucun joueur ${y}`,type:"info"}),g(k);return}de(b,M,`Choisir ${M} joueur(s) ${y}(s) à exclure`,$=>{const A="usedCardIds_"+m,_=new Set(k[A]||[]);$.forEach(S=>{const L=(x[S._line]||[]).find(j=>j.cardId===S.cardId);L&&(L.used=!0,_.add(S.cardId),k.log.push({text:`❌ ${L.name} exclu !`,type:"info"}))}),k[A]=[..._],k[m+"Team"]=x,g(k)})},REVIVE_PLAYER:({count:M=1,roles:q=[]},w,k)=>{const g=w[o+"Team"],m=Object.entries(g).filter(([x])=>!q.length||q.includes(x)).flatMap(([x,y])=>y.filter(b=>b.used).map(b=>({...b,_line:x})));if(!m.length){w.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),k(w);return}de(m,M,`Choisir ${M} joueur(s) à ressusciter`,x=>{x.forEach(y=>{const b=(g[y._line]||[]).find($=>$.cardId===y.cardId);b&&(b.used=!1,w.log.push({text:`💫 ${b.name} ressuscité !`,type:"info"}))}),w[o+"Team"]=g,k(w)})},REMOVE_GOAL:({},M,q)=>{const w=n+"Score";M[w]>0?(M[w]--,M.log.push({text:"🚫 Dernier but annulé !",type:"info"})):M.log.push({text:"🚫 Aucun but à annuler",type:"info"}),q(M)},ADD_GOAL_DRAW:({},M,q)=>{M[o+"Score"]===M[n+"Score"]?(M[o+"Score"]++,M.log.push({text:"🎯 But bonus !",type:"info"})):M.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),q(M)},ADD_SUB:({value:M=1},q,w)=>{q.maxSubs=(q.maxSubs||3)+M,q.log.push({text:`🔄 +${M} remplacement(s)`,type:"info"}),w(q)},CUSTOM:({},M,q)=>q(M)};function de(M,q,w,k){const g=document.createElement("div");g.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let m=[];function x(){var b,$;const y=M.map(A=>{const _={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[A._line]||"#555",S=Te(A,A._line)+(A.boost||0),j=m.find(R=>R.cardId===A.cardId)?"#FFD700":"rgba(255,255,255,0.25)",B=A.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${A.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${j};background:${_};overflow:hidden;cursor:pointer;${B}">
        ...
        </div>`,(b=g.querySelector("#pp-close"))==null||b.addEventListener("click",()=>g.remove()),g.querySelectorAll(".pp-item").forEach(A=>{A.addEventListener("click",()=>{const _=A.dataset.cid,S=M.find(j=>j.cardId===_),L=m.findIndex(j=>j.cardId===_);S&&(L>-1?m.splice(L,1):m.length<q&&m.push(S),x())})}),($=g.querySelector("#pp-confirm"))==null||$.addEventListener("click",()=>{g.remove(),k(m)})}x(),document.body.appendChild(g)}async function P(M,q){const k=(r["gcCardsFull_"+o]||[]).find(y=>y.id===M),g=(k==null?void 0:k._gcDef)||(r.gcDefs||[]).find(y=>{var b;return y.name===q||((b=y.name)==null?void 0:b.toLowerCase().trim())===(q==null?void 0:q.toLowerCase().trim())}),m=[...r["usedGc_"+o]||[],M],x={type:q,by:o,seq:(r._gcAnimSeq||0)+1};U.add(x.seq),Ae(q,o,async()=>{if(g!=null&&g.effect_type&&g.effect_type!=="CUSTOM"){const b=se[g.effect_type];if(b){const $={...r};b(g.effect_params||{},$,async A=>{A["usedGc_"+o]=m,A._lastGC=x,A._gcAnimSeq=x.seq,await G(A)});return}}const y={...r};switch(q){case"Remplacement+":y.maxSubs=(y.maxSubs||3)+1,y.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const b=n+"Score";y[b]>0&&(y[b]--,y.log.push({text:"🚫 But annulé",type:"info"}));break}}y["usedGc_"+o]=m,y._lastGC=x,y._gcAnimSeq=x.seq,await G(y)})}function Y(M,q){const w="usedCardIds_"+M,k=new Set(r[w]||[]);q.forEach(g=>k.add(g)),r[w]=[...k]}function V(){for(const M of["p1","p2"]){const q=new Set(r["usedCardIds_"+M]||[]),w=r[M+"Team"];if(w)for(const k of["GK","DEF","MIL","ATT"])(w[k]||[]).forEach(g=>{g.used=q.has(g.cardId)})}}function F(){var Ue,Je,Ze,Qe,ot,gt;if(r.phase==="reveal")return oe();if(r.phase==="midfield")return ye();if(r.phase==="finished")return De();const M=r[o+"Team"],q=r[n+"Team"];V();const w=r[o+"Score"],k=r[n+"Score"],g=r[o+"Name"],m=r[n+"Name"],x=r.phase===o+"-attack",y=r.phase===o+"-defense",b=Array.isArray(r["selected_"+o])?r["selected_"+o]:[],$=b.map(ae=>ae.cardId),A=window.innerWidth>=700,_=r[o+"Subs"]||[],S=r["usedSubIds_"+o]||[],L=_.filter(ae=>!S.includes(ae.cardId)),j=r["gcCardsFull_"+o]||[],B=r["usedGc_"+o]||[],R=j.filter(ae=>!B.includes(ae.id)),H=r.boostOwner===o&&!r.boostUsed,W=!["GK","DEF","MIL","ATT"].some(ae=>(q[ae]||[]).some(_e=>!_e.used)),K=[...M.MIL||[],...M.ATT||[]].filter(ae=>!ae.used),ee=x&&W&&K.length===0?[...M.GK||[],...M.DEF||[]].filter(ae=>!ae.used).map(ae=>ae.cardId):[];function ne(ae,_e,Q){var pt,ht;const ce=ae._gcDef,J={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ce==null?void 0:ce.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",le={purple:"#b06ce0",light_blue:"#00d4ef"}[ce==null?void 0:ce.color]||"#b06ce0",Me=(ce==null?void 0:ce.name)||ae.gc_type,ke=(ce==null?void 0:ce.effect)||((pt=Ke[ae.gc_type])==null?void 0:pt.desc)||"",tt=ce!=null&&ce.image_url?`/manager-wars/icons/${ce.image_url}`:null,it=((ht=Ke[ae.gc_type])==null?void 0:ht.icon)||"⚡",rt=Math.round(Q*.22),st=Math.round(Q*.22),ct=Q-rt-st,$t=Me.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${ae.id}" data-gc-type="${ae.gc_type}"
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      ${M!=null&&M.length?`<div style="display:flex;gap:10px;animation:scoreIn .4s ease 1s both;position:relative;z-index:1">
        ${M.map(A=>`<div style="text-align:center">
          <div style="width:50px;height:50px;border-radius:50%;background:${y[A.job]||"#555"};border:2px solid #FFD700;position:relative;overflow:hidden;margin:0 auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(m);let b=!1;const $=()=>{b||(b=!0,m.remove(),setTimeout(()=>g(),50))};m.addEventListener("click",$),setTimeout($,3500)}function Ae(M,q,w){var B,R;const k=(r.gcDefs||[]).find(H=>{var W;return H.name===M||((W=H.name)==null?void 0:W.toLowerCase().trim())===(M==null?void 0:M.toLowerCase().trim())}),g={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[k==null?void 0:k.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",m={purple:"#b06ce0",light_blue:"#00d4ef"}[k==null?void 0:k.color]||"#b06ce0",x=(k==null?void 0:k.name)||M,y=(k==null?void 0:k.effect)||((B=Ke[M])==null?void 0:B.desc)||"",b=k!=null&&k.image_url?`/manager-wars/icons/${k.image_url}`:null,$=((R=Ke[M])==null?void 0:R.icon)||"⚡",_=q===o?"Vous":r[q+"Name"]||"Adversaire",S=document.createElement("div");S.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",S.innerHTML=`
      <style>
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(S);let L=!1;const j=()=>{L||(L=!0,S.remove(),setTimeout(()=>w&&w(),50))};S.addEventListener("click",j),setTimeout(j,3e3)}function we(M,q){var S,L,j,B;const k=(r["gcCardsFull_"+o]||[]).find(R=>R.id===M),g=k==null?void 0:k._gcDef,m={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[g==null?void 0:g.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",x={purple:"#b06ce0",light_blue:"#00d4ef"}[g==null?void 0:g.color]||"#b06ce0",y=(g==null?void 0:g.name)||q,b=(g==null?void 0:g.effect)||((S=Ke[q])==null?void 0:S.desc)||"Carte spéciale.",$=g!=null&&g.image_url?`/manager-wars/icons/${g.image_url}`:null,A=((L=Ke[q])==null?void 0:L.icon)||"⚡",_=document.createElement("div");_.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",_.innerHTML=`
      ...
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${q.map(g=>{const m={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[g._line]||"#555",x=Te(g,g._line)+(g.boost||0);return`<div class="bp-item" data-cid="${g.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${m};overflow:hidden;cursor:pointer">
      ...
      </div>`,document.body.appendChild(w),(k=w.querySelector("#bp-close"))==null||k.addEventListener("click",()=>w.remove()),w.querySelectorAll(".bp-item").forEach(g=>{g.addEventListener("click",async()=>{const m=g.dataset.cid,x=q.find(b=>b.cardId===m);if(!x)return;const y=(M[x._line]||[]).find(b=>b.cardId===m);y&&(y.boost=(y.boost||0)+r.boostValue),w.remove(),await G({[o+"Team"]:M,boostUsed:!0})})})}function Ee(M=null){var L,j;if(!(r.phase===o+"-attack")){u("Remplacement uniquement avant votre attaque","warning");return}const w=r[o+"Team"],k=r["usedSubIds_"+o]||[],g=r.maxSubs||3;if(k.length>=g){u(`Maximum ${g} remplacements atteint`,"warning");return}const m=Object.entries(w).flatMap(([B,R])=>(R||[]).filter(H=>H.used).map(H=>({...H,_line:B}))),x=(r[o+"Subs"]||[]).filter(B=>!k.includes(B.cardId));if(!m.length){u("Aucun joueur utilisé à remplacer","warning");return}if(!x.length){u("Aucun remplaçant disponible","warning");return}let y=Math.max(0,m.findIndex(B=>B.cardId===M));const b=((L=m[y])==null?void 0:L._line)||((j=m[y])==null?void 0:j.job);let $=Math.max(0,x.findIndex(B=>B.job===b)),A=!1;const _=document.createElement("div");_.id="pvp-sub-overlay",_.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function S(){var ne,fe,xe,he,ve,Ie;const B=m[y],R=x[$],H=Math.min(130,Math.round((window.innerWidth-90)/2)),W=Math.round(H*1.35),K=Le=>`background:rgba(255,255,255,0.12);border:none;color:${Le?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Le?"default":"pointer"};flex-shrink:0`;_.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      </div>`,(ne=_.querySelector("#psub-close"))==null||ne.addEventListener("click",()=>_.remove()),(fe=_.querySelector("#pout-up"))==null||fe.addEventListener("click",()=>{y>0&&(y--,S())}),(xe=_.querySelector("#pout-down"))==null||xe.addEventListener("click",()=>{y<m.length-1&&(y++,S())}),(he=_.querySelector("#pin-up"))==null||he.addEventListener("click",()=>{$>0&&($--,S())}),(ve=_.querySelector("#pin-down"))==null||ve.addEventListener("click",()=>{$<x.length-1&&($++,S())});const ee=(Le,Re,je,Oe)=>{const Be=_.querySelector("#"+Le);if(!Be)return;let Ve=0;Be.addEventListener("touchstart",Ne=>{Ve=Ne.touches[0].clientY},{passive:!0}),Be.addEventListener("touchend",Ne=>{const He=Ne.changedTouches[0].clientY-Ve;if(Math.abs(He)<30)return;const Ue=Re();He<0&&Ue<Oe-1?(je(Ue+1),S()):He>0&&Ue>0&&(je(Ue-1),S())},{passive:!0})};ee("pin-panel",()=>$,Le=>$=Le,x.length),ee("pout-panel",()=>y,Le=>y=Le,m.length),(Ie=_.querySelector("#psub-confirm"))==null||Ie.addEventListener("click",async Le=>{if(Le.preventDefault(),Le.stopPropagation(),A)return;A=!0;const Re=m[y],je=x[$];if(!Re||!je)return;const Oe=Re._line,Be=(w[Oe]||[]).findIndex(He=>He.cardId===Re.cardId);if(Be===-1){u("Erreur : joueur introuvable","error"),_.remove();return}const Ve={...je,_line:Oe,position:Re.position,used:!1,boost:0};w[Oe].splice(Be,1,Ve);const Ne=[...k,je.cardId];_.remove(),te(Re,je,async()=>{await G({[o+"Team"]:w,[n+"Team"]:r[n+"Team"],["usedSubIds_"+o]:Ne})})})}document.body.appendChild(_),S()}function te(M,q,w){const k={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},g=document.createElement("div");g.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const m=(b,$,A)=>`<div style="text-align:center">
      ...
      </div>`,document.body.appendChild(q),(w=q.querySelector("#pvp-hist-close"))==null||w.addEventListener("click",()=>q.remove())}async function Z(){if(r.phase!==o+"-attack")return;const M=o==="p1"?"p2":"p1",q=(r.round||0)+1,w=[...r.log||[]];w.push({type:"info",text:`⏭️ ${r[o+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const k=Ce(r),g=Se(M),m=k||!g?"finished":M+"-attack";await G({["selected_"+o]:[],modifiers:{...r.modifiers,[o]:{}},pendingAttack:null,phase:m,attacker:M,round:q,log:w}),m==="finished"&&await qe(r)}async function ue(){const M=r[o+"Team"],q=!["GK","DEF","MIL","ATT"].some(m=>(r[n+"Team"][m]||[]).some(x=>!x.used)),w=(r["selected_"+o]||[]).map(m=>{const x=(M[m._role]||[]).find(S=>S.cardId===m.cardId)||m,y=q&&["GK","DEF"].includes(m._role),b=M[m._role]||[],$=b.findIndex(S=>S.cardId===m.cardId),A=lt(b.length),_=$>=0?A[$]:x._col??1;return{...x,_line:m._role,_col:_,...y?{note_a:Math.max(1,Number(x.note_a)||0)}:{}}});if(!w.length)return;const k=Nt(w,r.modifiers[o]||{});Y(o,w.map(m=>m.cardId)),w.forEach(m=>{const x=(M[m._role]||[]).find(y=>y.cardId===m.cardId);x&&(x.used=!0)}),r["selected_"+o]=[],F();const g=[...r.log||[]];if(q){const m=(r[o+"Score"]||0)+1,x=w.map(_=>({name:_.name,note:Te(_,_._line||"ATT"),portrait:Pe(_),job:_.job}));g.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:x,homeTotal:k.total,aiTotal:0});const y=(r.round||0)+1,b=o==="p1"?"p2":"p1",$={...r,[o+"Team"]:M,[o+"Score"]:m},A=Ce($);C.add(y),ze(x,m,r[n+"Score"]||0,!0,async()=>{await G({[o+"Team"]:M,[o+"Score"]:m,["selected_"+o]:[],modifiers:{...r.modifiers,[o]:{}},pendingAttack:null,phase:A?"finished":b+"-attack",attacker:b,round:y,log:g}),A&&await qe({...r,[o+"Score"]:m})});return}g.push({type:"pending",text:`⚔️ ${r[o+"Name"]} attaque (${k.total})`}),await G({[o+"Team"]:M,[n+"Team"]:r[n+"Team"],pendingAttack:{...k,players:w,side:o},["selected_"+o]:[],modifiers:{...r.modifiers,[o]:{}},phase:n+"-defense",log:g})}async function me(){const M=r[o+"Team"],q=(r["selected_"+o]||[]).map(B=>{const R=(M[B._role]||[]).find(ne=>ne.cardId===B.cardId)||B,H=M[B._role]||[],W=H.findIndex(ne=>ne.cardId===B.cardId),K=lt(H.length),ee=W>=0?K[W]:R._col??1;return{...R,_line:B._role,_col:ee}}),w=Rt(q,r.modifiers[o]||{});Y(o,q.map(B=>B.cardId)),q.forEach(B=>{const R=(M[B._role]||[]).find(H=>H.cardId===B.cardId);R&&(R.used=!0)}),r["selected_"+o]=[],F();const k=Ot(r.pendingAttack.total,w.total,r.modifiers[o]||{}),g=r.pendingAttack.side,m=k==="attack"||(k==null?void 0:k.goal),x=g==="p1"?"p2":"p1",y=(r.round||0)+1,b=(r.pendingAttack.players||[]).map(B=>({name:B.name,note:Te(B,B._line||"ATT"),portrait:Pe(B),job:B.job})),$=[...r.log||[]];$.push({type:"duel",isGoal:m,homeScored:m&&g===o,text:m?`⚽ BUT de ${r[g+"Name"]} ! (${r.pendingAttack.total} vs ${w.total})`:`✋ Attaque stoppée (${r.pendingAttack.total} vs ${w.total})`,homePlayers:b,aiPlayers:q.map(B=>({name:B.name,note:Te(B,B._line||"DEF"),portrait:Pe(B),job:B.job})),homeTotal:r.pendingAttack.total,aiTotal:w.total});const A=m?(r[g+"Score"]||0)+1:r[g+"Score"]||0,_={...r,[o+"Team"]:M,[g+"Score"]:A},S=Ce(_),L=S?"finished":x+"-attack",j=async()=>{await G({[o+"Team"]:M,[n+"Team"]:r[n+"Team"],[g+"Score"]:A,["selected_"+o]:[],modifiers:{...r.modifiers,[o]:{}},pendingAttack:null,phase:L,attacker:x,round:y,log:$}),(L==="finished"||S)&&await qe({...r,[g+"Score"]:A})};if(m){const B=g===o,R=B?A:r[o+"Score"]||0,H=B?r[n+"Score"]||0:A;C.add(y),ze(b,R,H,B,j)}else await j()}function X(M){return["MIL","ATT"].some(q=>(M[q]||[]).some(w=>!w.used))}function be(M){return["GK","DEF","MIL","ATT"].some(q=>(M[q]||[]).some(w=>!w.used))}function ge(M){return be(M)&&!X(M)}function Se(M){const q=r[M+"Team"],w=r[(M==="p1"?"p2":"p1")+"Team"];return!!(X(q)||!be(w)&&ge(q))}function Ce(M){const q=["MIL","ATT"].some($=>(M.p1Team[$]||[]).some(A=>!A.used)),w=["MIL","ATT"].some($=>(M.p2Team[$]||[]).some(A=>!A.used)),k=be(M.p1Team),g=be(M.p2Team);return!q&&!(!g&&k)&&(!w&&!(!k&&g))}function Fe(M){const q=M.p1Score||0,w=M.p2Score||0;return q===w?null:q>w?h.home_id:h.away_id}async function qe(M){try{const q=Fe(M),w=q?h.home_id===q?h.away_id:h.home_id:null;await E.from("matches").update({status:"finished",winner_id:q,home_score:M.p1Score||0,away_score:M.p2Score||0}).eq("id",i),q&&w&&updateEvolutiveCards(q,w).catch(()=>{})}catch(q){console.error("[PvP] finishMatch:",q)}}function De(){var g;const M=r[o+"Score"],q=r[n+"Score"],w=M>q,k=M===q;et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
    ...
    </div>`,(g=document.getElementById("pvp-home"))==null||g.addEventListener("click",()=>{try{E.removeChannel(N)}catch{}We(e),l("home")})}F()}const Uo=Object.freeze(Object.defineProperty({__proto__:null,renderMatchRandom:wn,resumePvpMatch:Ho},Symbol.toStringTag,{value:"Module"}));async function Vo(e,t,i,s){var c,d;try{const a=(d=(c=t==null?void 0:t.state)==null?void 0:c.profile)==null?void 0:d.id;try{(E.getChannels?E.getChannels():[]).forEach(u=>{const o=u.topic||"";(o.includes("matchmaking")||o.includes("pvp-match"))&&E.removeChannel(u)})}catch(l){console.warn("[FriendMatch] cleanup canaux:",l)}a&&(await E.rpc("cancel_matchmaking",{p_user_id:a}).catch(()=>{}),await E.from("matchmaking_queue").delete().eq("user_id",a).then(()=>{},()=>{}))}catch{}await ni(e,t,"random",({deckId:a,formation:l,starters:u,subsRaw:o,gcCardsEnriched:n,gcDefs:f,stadiumDef:v})=>{const h=T=>Ko(e,t,a,l,u,o,T,f||[],i,s);if(!n.length){h([]);return}ii(e,n,h)})}async function Ko(e,t,i,s,c,d,a=[],l=[],u,o){var se;const{state:n,navigate:f,toast:v}=t,h=n.profile.id;let T=!1,r=null;et(e);try{await E.rpc("cancel_matchmaking",{p_user_id:h})}catch{}try{await E.from("matchmaking_queue").delete().eq("user_id",h)}catch{}const p=(de,P,Y,V)=>{var F;e.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(y),(S=y.querySelector("#pvp-end-home"))==null||S.addEventListener("click",()=>{y.remove(),We(e),l("home")})}const ie=E.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},w=>{const k=w.new;try{if(k.status==="finished"||k.forfeit){if(p)return;p=!0,C&&(clearInterval(C),C=null),k.game_state&&(r=k.game_state),G(k);return}if(k.game_state){const g=r;r=k.game_state;const m=r._lastGC;if(m&&m.seq&&!N.has(m.seq)&&(N.add(m.seq),m.by!==o)){$e(m.type,m.by,()=>pe());return}const x=g[o+"Score"]||0,y=g[n+"Score"]||0,b=r[o+"Score"]||0,$=r[n+"Score"]||0,A=b>x,_=$>y;if((A||_)&&!D.has(r.round)){D.add(r.round);const S=[...r.log||[]].reverse().find(j=>j.isGoal),L=((S==null?void 0:S.homePlayers)||[]).map(j=>({name:j.name,note:j.note,portrait:j.portrait,job:j.job}));we(L,b,$,A,()=>pe());return}pe()}}catch(g){console.error("[PvP] crash:",g)}}).subscribe();async function se(w){Object.assign(r,w),r.lastActionAt=new Date().toISOString();const{error:k}=await E.from("matches").update({game_state:r}).eq("id",i);k&&u("Erreur de synchronisation","error");try{pe()}catch(g){console.error("[PvP] renderPvpScreen crash:",g)}}async function de(){if(p)return;p=!0,C&&(clearInterval(C),C=null);const w=s?h.away_id:h.home_id,k=s?"p2":"p1",g=s?"p1":"p2",m={...r,[k+"Score"]:3,[g+"Score"]:0,phase:"finished"};try{await E.from("matches").update({status:"finished",forfeit:!0,winner_id:w,home_score:m.p1Score||0,away_score:m.p2Score||0,game_state:m}).eq("id",i)}catch{}try{E.removeChannel(ie)}catch{}setTimeout(()=>{We(e),l("home")},800)}const P={BOOST_STAT:({value:w=1,count:k=1,roles:g=[]},m,x)=>{const y=m[o+"Team"],b=Object.entries(y).filter(([$])=>!g.length||g.includes($)).flatMap(([$,A])=>A.filter(_=>!_.used).map(_=>({..._,_line:$})));if(!b.length){m.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),x(m);return}Y(b,k,`Choisir ${k} joueur(s) à booster (+${w})`,$=>{$.forEach(A=>{const _=(y[A._line]||[]).find(S=>S.cardId===A.cardId);_&&(_.boost=(_.boost||0)+w,m.log.push({text:`⚡ +${w} sur ${_.name}`,type:"info"}))}),m[o+"Team"]=y,x(m)})},DEBUFF_STAT:({value:w=1,count:k=1,roles:g=[],target:m="ai"},x,y)=>{const b=m==="home"?o:n,$=x[b+"Team"],A=m==="home"?"allié":"adverse",_=Object.entries($).filter(([S])=>!g.length||g.includes(S)).flatMap(([S,L])=>L.map(j=>({...j,_line:S})));if(!_.length){x.log.push({text:`🎯 Aucun joueur ${A}`,type:"info"}),y(x);return}Y(_,k,`Choisir ${k} joueur(s) ${A}(s) (-${w})`,S=>{S.forEach(L=>{const j=($[L._line]||[]).find(B=>B.cardId===L.cardId);j&&(j.boost=(j.boost||0)-w,x.log.push({text:`🎯 -${w} sur ${j.name}`,type:"info"}))}),x[b+"Team"]=$,y(x)})},GRAY_PLAYER:({count:w=1,roles:k=[],target:g="ai"},m,x)=>{const y=g==="home"?o:n,b=m[y+"Team"],$=g==="home"?"allié":"adverse",A=Object.entries(b).filter(([_])=>!k.length||k.includes(_)).flatMap(([_,S])=>S.filter(L=>!L.used).map(L=>({...L,_line:_})));if(!A.length){m.log.push({text:`❌ Aucun joueur ${$}`,type:"info"}),x(m);return}Y(A,w,`Choisir ${w} joueur(s) ${$}(s) à exclure`,_=>{const S="usedCardIds_"+y,L=new Set(m[S]||[]);_.forEach(j=>{const B=(b[j._line]||[]).find(R=>R.cardId===j.cardId);B&&(B.used=!0,L.add(j.cardId),m.log.push({text:`❌ ${B.name} exclu !`,type:"info"}))}),m[S]=[...L],m[y+"Team"]=b,x(m)})},REVIVE_PLAYER:({count:w=1,roles:k=[]},g,m)=>{const x=g[o+"Team"],y=Object.entries(x).filter(([b])=>!k.length||k.includes(b)).flatMap(([b,$])=>$.filter(A=>A.used).map(A=>({...A,_line:b})));if(!y.length){g.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),m(g);return}Y(y,w,`Choisir ${w} joueur(s) à ressusciter`,b=>{b.forEach($=>{const A=(x[$._line]||[]).find(_=>_.cardId===$.cardId);A&&(A.used=!1,g.log.push({text:`💫 ${A.name} ressuscité !`,type:"info"}))}),g[o+"Team"]=x,m(g)})},REMOVE_GOAL:({},w,k)=>{const g=n+"Score";w[g]>0?(w[g]--,w.log.push({text:"🚫 Dernier but annulé !",type:"info"})):w.log.push({text:"🚫 Aucun but à annuler",type:"info"}),k(w)},ADD_GOAL_DRAW:({},w,k)=>{w[o+"Score"]===w[n+"Score"]?(w[o+"Score"]++,w.log.push({text:"🎯 But bonus !",type:"info"})):w.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),k(w)},ADD_SUB:({value:w=1},k,g)=>{k.maxSubs=(k.maxSubs||3)+w,k.log.push({text:`🔄 +${w} remplacement(s)`,type:"info"}),g(k)},CUSTOM:({},w,k)=>k(w)};function Y(w,k,g,m){const x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let y=[];function b(){var A,_;const $=w.map(S=>{const L={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[S._line]||"#555",j=Te(S,S._line)+(S.boost||0),R=y.find(W=>W.cardId===S.cardId)?"#FFD700":"rgba(255,255,255,0.25)",H=S.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${S.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${R};background:${L};overflow:hidden;cursor:pointer;${H}">
        ...
        </div>`,(A=x.querySelector("#pp-close"))==null||A.addEventListener("click",()=>x.remove()),x.querySelectorAll(".pp-item").forEach(S=>{S.addEventListener("click",()=>{const L=S.dataset.cid,j=w.find(R=>R.cardId===L),B=y.findIndex(R=>R.cardId===L);j&&(B>-1?y.splice(B,1):y.length<k&&y.push(j),b())})}),(_=x.querySelector("#pp-confirm"))==null||_.addEventListener("click",()=>{x.remove(),m(y)})}b(),document.body.appendChild(x)}async function V(w,k){const m=(r["gcCardsFull_"+o]||[]).find($=>$.id===w),x=(m==null?void 0:m._gcDef)||(r.gcDefs||[]).find($=>{var A;return $.name===k||((A=$.name)==null?void 0:A.toLowerCase().trim())===(k==null?void 0:k.toLowerCase().trim())}),y=[...r["usedGc_"+o]||[],w],b={type:k,by:o,seq:(r._gcAnimSeq||0)+1};N.add(b.seq),$e(k,o,async()=>{if(x!=null&&x.effect_type&&x.effect_type!=="CUSTOM"){const A=P[x.effect_type];if(A){const _={...r};A(x.effect_params||{},_,async S=>{S["usedGc_"+o]=y,S._lastGC=b,S._gcAnimSeq=b.seq,await se(S)});return}}const $={...r};switch(k){case"Remplacement+":$.maxSubs=($.maxSubs||3)+1,$.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const A=n+"Score";$[A]>0&&($[A]--,$.log.push({text:"🚫 But annulé",type:"info"}));break}}$["usedGc_"+o]=y,$._lastGC=b,$._gcAnimSeq=b.seq,await se($)})}function F(w,k){const g="usedCardIds_"+w,m=new Set(r[g]||[]);k.forEach(x=>m.add(x)),r[g]=[...m]}function oe(){for(const w of["p1","p2"]){const k=new Set(r["usedCardIds_"+w]||[]),g=r[w+"Team"];if(g)for(const m of["GK","DEF","MIL","ATT"])(g[m]||[]).forEach(x=>{x.used=k.has(x.cardId)})}}function pe(){var Ze,Qe,ot,gt,ae,_e;if(r.phase==="reveal")return ye();if(r.phase==="midfield")return Ae();if(r.phase==="finished")return q();const w=r[o+"Team"],k=r[n+"Team"];oe();const g=r[o+"Score"],m=r[n+"Score"],x=r[o+"Name"],y=r[n+"Name"],b=r.phase===o+"-attack",$=r.phase===o+"-defense",A=Array.isArray(r["selected_"+o])?r["selected_"+o]:[],_=A.map(Q=>Q.cardId),S=window.innerWidth>=700,L=r[o+"Subs"]||[],j=r["usedSubIds_"+o]||[],B=L.filter(Q=>!j.includes(Q.cardId)),R=r["gcCardsFull_"+o]||[],H=r["usedGc_"+o]||[],W=R.filter(Q=>!H.includes(Q.id)),K=r.boostOwner===o&&!r.boostUsed,ee=!["GK","DEF","MIL","ATT"].some(Q=>(k[Q]||[]).some(ce=>!ce.used)),ne=[...w.MIL||[],...w.ATT||[]].filter(Q=>!Q.used),fe=b&&ee&&ne.length===0?[...w.GK||[],...w.DEF||[]].filter(Q=>!Q.used).map(Q=>Q.cardId):[];function xe(Q,ce,J){var kt,Ht;const le=Q._gcDef,Me={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[le==null?void 0:le.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",ke={purple:"#b06ce0",light_blue:"#00d4ef"}[le==null?void 0:le.color]||"#b06ce0",tt=(le==null?void 0:le.name)||Q.gc_type,it=(le==null?void 0:le.effect)||((kt=Ke[Q.gc_type])==null?void 0:kt.desc)||"",rt=le!=null&&le.image_url?`/manager-wars/icons/${le.image_url}`:null,st=((Ht=Ke[Q.gc_type])==null?void 0:Ht.icon)||"⚡",ct=Math.round(J*.22),$t=Math.round(J*.22),pt=J-ct-$t,ht=tt.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${Q.id}" data-gc-type="${Q.gc_type}"
      ...
      </div>`}const ve=(Q,ce)=>ce?he(130,175):xe(Q,130,175),Ie=(Q,ce)=>ce?he(68,95):xe(Q,68,95),Le=S?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",Re=b?Fe(o)?`<button id="pvp-action" style="${Le};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${A.length===0?"disabled":""}>⚔️ ATTAQUEZ <span id="pvp-timer"></span></button>`:`<button id="pvp-action" data-pass="1" style="${Le};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER <span id="pvp-timer"></span></button>`:$?`<button id="pvp-action" style="${Le};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${A.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="pvp-timer"></span></button>`:`<div style="font-size:11px;color:rgba(255,255,255,0.3);text-align:center;padding:4px">⏳ Tour de ${y}</div>`,je=b&&!Fe(o)?'<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">Aucun attaquant — passez la main</div>':b||$?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${A.length}/3 sélectionné(s)</div>`:"",Oe=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${S?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
    ...
    </div>`,Be=b?"attack":$?"defense":"idle",Ve=`<div style="overflow:hidden;min-width:0;flex:1;min-height:0;display:flex;flex-direction:column" id="match-field">
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
      ...
      <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
        ...
        <div id="mobile-action-bar" style="position:absolute;left:0;right:0;bottom:0;z-index:20;background:rgba(0,0,0,0.55);padding:6px 8px 8px;display:flex;flex-direction:column;gap:6px;box-shadow:0 -4px 16px rgba(0,0,0,0.5)">
          <div style="display:flex;gap:6px;overflow-x:auto;align-items:flex-end;min-height:96px;padding-bottom:2px">
            ${W.map(Q=>Ie(Q,!1)).join("")}
            ${K?Ie(null,!0):""}
            <div id="pvp-sub-open" style="cursor:${b&&B.length>0?"pointer":"default"};flex-shrink:0;box-sizing:border-box;width:68px;height:95px;border-radius:10px;border:2px solid ${b&&B.length>0?"rgba(255,255,255,0.5)":"rgba(255,255,255,0.15)"};background:${b&&B.length>0?"rgba(60,60,60,0.9)":"rgba(40,40,40,0.5)"};display:flex;flex-direction:column;align-items:center;justify-content:center;gap:4px;opacity:${b&&B.length>0?1:.4}">
              <div style="display:flex;gap:6px;align-items:center">
                <div style="text-align:center">
                  <div style="font-size:7px;color:#00ff88;font-weight:700;letter-spacing:1px">IN</div>
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(y);let A=!1;const _=()=>{A||(A=!0,y.remove(),setTimeout(()=>x(),50))};y.addEventListener("click",_),setTimeout(_,3500)}function $e(w,k,g){var H,W;const m=(r.gcDefs||[]).find(K=>{var ee;return K.name===w||((ee=K.name)==null?void 0:ee.toLowerCase().trim())===(w==null?void 0:w.toLowerCase().trim())}),x={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[m==null?void 0:m.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",y={purple:"#b06ce0",light_blue:"#00d4ef"}[m==null?void 0:m.color]||"#b06ce0",b=(m==null?void 0:m.name)||w,$=(m==null?void 0:m.effect)||((H=Ke[w])==null?void 0:H.desc)||"",A=m!=null&&m.image_url?`/manager-wars/icons/${m.image_url}`:null,_=((W=Ke[w])==null?void 0:W.icon)||"⚡",L=k===o?"Vous":r[k+"Name"]||"Adversaire",j=document.createElement("div");j.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",j.innerHTML=`
      <style>
        @keyframes gcFlipIn{0%{transform:perspective(800px) rotateY(90deg) scale(.7);opacity:0}55%{transform:perspective(800px) rotateY(-12deg) scale(1.08);opacity:1}100%{transform:perspective(800px) rotateY(0) scale(1);opacity:1}}
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(j);let B=!1;const R=()=>{B||(B=!0,j.remove(),setTimeout(()=>g&&g(),50))};j.addEventListener("click",R),setTimeout(R,3e3)}function Ee(w,k){var j,B,R,H;const m=(r["gcCardsFull_"+o]||[]).find(W=>W.id===w),x=m==null?void 0:m._gcDef,y={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[x==null?void 0:x.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",b={purple:"#b06ce0",light_blue:"#00d4ef"}[x==null?void 0:x.color]||"#b06ce0",$=(x==null?void 0:x.name)||k,A=(x==null?void 0:x.effect)||((j=Ke[k])==null?void 0:j.desc)||"Carte spéciale.",_=x!=null&&x.image_url?`/manager-wars/icons/${x.image_url}`:null,S=((B=Ke[k])==null?void 0:B.icon)||"⚡",L=document.createElement("div");L.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",L.innerHTML=`
      ...
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${k.map(x=>{const y={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[x._line]||"#555",b=Te(x,x._line)+(x.boost||0);return`<div class="bp-item" data-cid="${x.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${y};overflow:hidden;cursor:pointer">
      ...
      </div>`,document.body.appendChild(g),(m=g.querySelector("#bp-close"))==null||m.addEventListener("click",()=>g.remove()),g.querySelectorAll(".bp-item").forEach(x=>{x.addEventListener("click",async()=>{const y=x.dataset.cid,b=k.find(A=>A.cardId===y);if(!b)return;const $=(w[b._line]||[]).find(A=>A.cardId===y);$&&($.boost=($.boost||0)+r.boostValue),g.remove(),await se({[o+"Team"]:w,boostUsed:!0})})})}function re(w=null){var B,R;if(!(r.phase===o+"-attack")){u("Remplacement uniquement avant votre attaque","warning");return}const g=r[o+"Team"],m=r["usedSubIds_"+o]||[],x=r.maxSubs||3;if(m.length>=x){u(`Maximum ${x} remplacements atteint`,"warning");return}const y=Object.entries(g).flatMap(([H,W])=>(W||[]).filter(K=>K.used).map(K=>({...K,_line:H}))),b=(r[o+"Subs"]||[]).filter(H=>!m.includes(H.cardId));if(!y.length){u("Aucun joueur utilisé à remplacer","warning");return}if(!b.length){u("Aucun remplaçant disponible","warning");return}let $=Math.max(0,y.findIndex(H=>H.cardId===w));const A=((B=y[$])==null?void 0:B._line)||((R=y[$])==null?void 0:R.job);let _=Math.max(0,b.findIndex(H=>H.job===A)),S=!1;const L=document.createElement("div");L.id="pvp-sub-overlay",L.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function j(){var xe,he,ve,Ie,Le,Re;const H=y[$],W=b[_],K=Math.min(130,Math.round((window.innerWidth-90)/2)),ee=Math.round(K*1.35),ne=je=>`background:rgba(255,255,255,0.12);border:none;color:${je?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${je?"default":"pointer"};flex-shrink:0`;L.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
        <button id="psub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
      </div>`,(xe=L.querySelector("#psub-close"))==null||xe.addEventListener("click",()=>L.remove()),(he=L.querySelector("#pout-up"))==null||he.addEventListener("click",()=>{$>0&&($--,j())}),(ve=L.querySelector("#pout-down"))==null||ve.addEventListener("click",()=>{$<y.length-1&&($++,j())}),(Ie=L.querySelector("#pin-up"))==null||Ie.addEventListener("click",()=>{_>0&&(_--,j())}),(Le=L.querySelector("#pin-down"))==null||Le.addEventListener("click",()=>{_<b.length-1&&(_++,j())});const fe=(je,Oe,Be,Ve)=>{const Ne=L.querySelector("#"+je);if(!Ne)return;let He=0;Ne.addEventListener("touchstart",Ue=>{He=Ue.touches[0].clientY},{passive:!0}),Ne.addEventListener("touchend",Ue=>{const Je=Ue.changedTouches[0].clientY-He;if(Math.abs(Je)<30)return;const Ze=Oe();Je<0&&Ze<Ve-1?(Be(Ze+1),j()):Je>0&&Ze>0&&(Be(Ze-1),j())},{passive:!0})};fe("pin-panel",()=>_,je=>_=je,b.length),fe("pout-panel",()=>$,je=>$=je,y.length),(Re=L.querySelector("#psub-confirm"))==null||Re.addEventListener("click",async je=>{if(je.preventDefault(),je.stopPropagation(),S)return;S=!0;const Oe=y[$],Be=b[_];if(!Oe||!Be)return;const Ve=Oe._line,Ne=(g[Ve]||[]).findIndex(Je=>Je.cardId===Oe.cardId);if(Ne===-1){u("Erreur : joueur introuvable","error"),L.remove();return}const He={...Be,_line:Ve,position:Oe.position,used:!1,boost:0};g[Ve].splice(Ne,1,He);const Ue=[...m,Be.cardId];L.remove(),O(Oe,Be,async()=>{await se({[o+"Team"]:g,[n+"Team"]:r[n+"Team"],["usedSubIds_"+o]:Ue})})})}document.body.appendChild(L),j()}function O(w,k,g){const m={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const y=(A,_,S)=>`<div style="text-align:center">
      ...
      </div>`,document.body.appendChild(k),(m=k.querySelector("#pvp-hist-close"))==null||m.addEventListener("click",()=>k.remove())}async function me(){if(r.phase!==o+"-attack")return;const w=o==="p1"?"p2":"p1",k=(r.round||0)+1,g=[...r.log||[]];g.push({type:"info",text:`⏭️ ${r[o+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const m=qe(r),x=Fe(w),y=m||!x?"finished":w+"-attack";await se({["selected_"+o]:[],modifiers:{...r.modifiers,[o]:{}},pendingAttack:null,phase:y,attacker:w,round:k,log:g}),y==="finished"&&await M(r)}async function X(){const w=r[o+"Team"],k=!["GK","DEF","MIL","ATT"].some(y=>(r[n+"Team"][y]||[]).some(b=>!b.used)),g=(r["selected_"+o]||[]).map(y=>{const b=(w[y._role]||[]).find(j=>j.cardId===y.cardId)||y,$=k&&["GK","DEF"].includes(y._role),A=w[y._role]||[],_=A.findIndex(j=>j.cardId===y.cardId),S=lt(A.length),L=_>=0?S[_]:b._col??1;return{...b,_line:y._role,_col:L,...$?{note_a:Math.max(1,Number(b.note_a)||0)}:{}}});if(!g.length)return;const m=Nt(g,r.modifiers[o]||{});F(o,g.map(y=>y.cardId)),g.forEach(y=>{const b=(w[y._role]||[]).find($=>$.cardId===y.cardId);b&&(b.used=!0)}),r["selected_"+o]=[],pe();const x=[...r.log||[]];if(k){const y=(r[o+"Score"]||0)+1,b=g.map(L=>({name:L.name,note:Te(L,L._line||"ATT"),portrait:Pe(L),job:L.job}));x.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:b,homeTotal:m.total,aiTotal:0});const $=(r.round||0)+1,A=o==="p1"?"p2":"p1",_={...r,[o+"Team"]:w,[o+"Score"]:y},S=qe(_);D.add($),we(b,y,r[n+"Score"]||0,!0,async()=>{await se({[o+"Team"]:w,[o+"Score"]:y,["selected_"+o]:[],modifiers:{...r.modifiers,[o]:{}},pendingAttack:null,phase:S?"finished":A+"-attack",attacker:A,round:$,log:x}),S&&await M({...r,[o+"Score"]:y})});return}x.push({type:"pending",text:`⚔️ ${r[o+"Name"]} attaque (${m.total})`}),await se({[o+"Team"]:w,[n+"Team"]:r[n+"Team"],pendingAttack:{...m,players:g,side:o},["selected_"+o]:[],modifiers:{...r.modifiers,[o]:{}},phase:n+"-defense",log:x})}async function be(){const w=r[o+"Team"],k=(r["selected_"+o]||[]).map(H=>{const W=(w[H._role]||[]).find(xe=>xe.cardId===H.cardId)||H,K=w[H._role]||[],ee=K.findIndex(xe=>xe.cardId===H.cardId),ne=lt(K.length),fe=ee>=0?ne[ee]:W._col??1;return{...W,_line:H._role,_col:fe}}),g=Rt(k,r.modifiers[o]||{});F(o,k.map(H=>H.cardId)),k.forEach(H=>{const W=(w[H._role]||[]).find(K=>K.cardId===H.cardId);W&&(W.used=!0)}),r["selected_"+o]=[],pe();const m=Ot(r.pendingAttack.total,g.total,r.modifiers[o]||{}),x=r.pendingAttack.side,y=m==="attack"||(m==null?void 0:m.goal),b=x==="p1"?"p2":"p1",$=(r.round||0)+1,A=(r.pendingAttack.players||[]).map(H=>({name:H.name,note:Te(H,H._line||"ATT"),portrait:Pe(H),job:H.job})),_=[...r.log||[]];_.push({type:"duel",isGoal:y,homeScored:y&&x===o,text:y?`⚽ BUT de ${r[x+"Name"]} ! (${r.pendingAttack.total} vs ${g.total})`:`✋ Attaque stoppée (${r.pendingAttack.total} vs ${g.total})`,homePlayers:A,aiPlayers:k.map(H=>({name:H.name,note:Te(H,H._line||"DEF"),portrait:Pe(H),job:H.job})),homeTotal:r.pendingAttack.total,aiTotal:g.total});const S=y?(r[x+"Score"]||0)+1:r[x+"Score"]||0,L={...r,[o+"Team"]:w,[x+"Score"]:S},j=qe(L),B=j?"finished":b+"-attack",R=async()=>{await se({[o+"Team"]:w,[n+"Team"]:r[n+"Team"],[x+"Score"]:S,["selected_"+o]:[],modifiers:{...r.modifiers,[o]:{}},pendingAttack:null,phase:B,attacker:b,round:$,log:_}),(B==="finished"||j)&&await M({...r,[x+"Score"]:S})};if(y){const H=x===o,W=H?S:r[o+"Score"]||0,K=H?r[n+"Score"]||0:S;D.add($),we(A,W,K,H,R)}else await R()}function ge(w){return["MIL","ATT"].some(k=>(w[k]||[]).some(g=>!g.used))}function Se(w){return["GK","DEF","MIL","ATT"].some(k=>(w[k]||[]).some(g=>!g.used))}function Ce(w){return Se(w)&&!ge(w)}function Fe(w){const k=r[w+"Team"],g=r[(w==="p1"?"p2":"p1")+"Team"];return!!(ge(k)||!Se(g)&&Ce(k))}function qe(w){const k=["MIL","ATT"].some(_=>(w.p1Team[_]||[]).some(S=>!S.used)),g=["MIL","ATT"].some(_=>(w.p2Team[_]||[]).some(S=>!S.used)),m=Se(w.p1Team),x=Se(w.p2Team);return!k&&!(!x&&m)&&(!g&&!(!m&&x))}function De(w){const k=w.p1Score||0,g=w.p2Score||0;return k===g?null:k>g?h.home_id:h.away_id}async function M(w){try{const k=De(w),g=k?h.home_id===k?h.away_id:h.home_id:null;await E.from("matches").update({status:"finished",winner_id:k,home_score:w.p1Score||0,away_score:w.p2Score||0}).eq("id",i),k&&g&&_i(k,g).catch(()=>{})}catch(k){console.error("[PvP] finishMatch:",k)}}function q(){var x;const w=r[o+"Score"],k=r[n+"Score"],g=w>k,m=w===k;et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:10px">
      ${i==="waiting"?Jo(v,r,c):i==="active"?Xo(h,c):Zo(T,c)}
    </div>
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        <div style="font-size:14px;font-weight:900;margin-bottom:4px">👥 Joueurs (${f.length}/${a.max_players})</div>
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        ${Array.from({length:Math.max(0,a.status==="active"?a.current_day-1:a.current_day)},(V,F)=>F+1).reverse().map(V=>{const F=(u||[]).filter(oe=>oe.matchday===V);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:#888;margin-bottom:6px">Journée ${V}</div>${F.map(oe=>Ri(oe,f,d,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
  ...
  </div>`;const C=a.status==="waiting"?"waiting":a.status==="active"?"active":"archived";if((U=document.getElementById("ml-back"))==null||U.addEventListener("click",()=>_t(e,t,C)),(D=document.getElementById("ml-start-btn"))==null||D.addEventListener("click",()=>nr(e,t,a,f)),(N=document.getElementById("ml-next-day"))==null||N.addEventListener("click",()=>or(e,t,i)),(G=document.getElementById("ml-finish-btn"))==null||G.addEventListener("click",()=>kn(e,t,i,T,v,f)),(ie=document.getElementById("ml-join-now"))==null||ie.addEventListener("click",()=>$n(e,t,i,a.type)),(se=document.getElementById("ml-leave-btn"))==null||se.addEventListener("click",()=>tr(e,t,i,r)),(de=document.getElementById("ml-delete-btn"))==null||de.addEventListener("click",()=>gi(e,t,i,a.name,"waiting")),(P=document.getElementById("ml-delete-now"))==null||P.addEventListener("click",()=>gi(e,t,i,a.name,C)),(Y=document.getElementById("ml-archive-btn"))==null||Y.addEventListener("click",()=>ir(e,t,i)),e.querySelectorAll("[data-play-match]").forEach(V=>{V.addEventListener("click",()=>{const F=h.find(oe=>oe.id===V.dataset.playMatch);F&&t.navigate("match-mini-league",{mlMatchId:F.id,leagueId:i})})}),a.status==="finished"&&z){const V=v.findIndex(F=>F.userId===d);V>=0&&V<3&&z.prize_amount>0&&setTimeout(()=>rr(e,t,a,z,V),400)}}function Ri(e,t,i,s,c=!1){const d=f=>t.find(v=>v.id===f);if(e.is_bye){const f=d(e.home_id);return`<div style="padding:8px;border-radius:8px;background:#f9f9f9;margin-bottom:6px;font-size:12px;color:#888;text-align:center">🔵 ${(f==null?void 0:f.club_name)||(f==null?void 0:f.pseudo)||"?"} exempté(e)</div>`}const a=d(e.home_id),l=d(e.away_id),u=e.home_id===i||e.away_id===i,o=u&&e.status==="pending"&&s&&!c,n=e.status==="finished"?`${e.home_score} - ${e.away_score}`:"vs";return`<div style="display:flex;align-items:center;gap:8px;padding:10px;border-radius:8px;background:${u?"#f0fdf4":"#f9f9f9"};margin-bottom:6px;border:1px solid ${u?"#86efac":"#f0f0f0"}">
  ...
  </div>`}async function nr(e,t,i,s){const{toast:c,state:d}=t,a=ar(s.map(o=>o.id),i.mode),l=[];a.forEach((o,n)=>o.forEach(f=>l.push({league_id:i.id,matchday:n+1,home_id:f.home||f.bye,away_id:f.away||null,is_bye:!!f.bye,status:f.bye?"bye":"pending"})));const{error:u}=await E.from("mini_league_matches").insert(l);if(u){c("Erreur calendrier : "+u.message,"error");return}await E.from("mini_leagues").update({status:"active",current_day:1,total_days:a.length}).eq("id",i.id),c(`🚀 Lancée ! Pot : ${(i.pot||0).toLocaleString("fr")} cr.`,"success"),bt(e,t,i.id)}async function or(e,t,i){const{data:s}=await E.from("mini_leagues").select("current_day,total_days,pot").eq("id",i).single(),c=(s.current_day||0)+1;if(c>(s.total_days||0)){await kn(e,t,i,s.pot||0,null,null);return}await E.from("mini_leagues").update({current_day:c}).eq("id",i),t.toast(`Journée ${c} commencée !`,"success"),bt(e,t,i)}async function kn(e,t,i,s,c,d){const{toast:a,state:l}=t;let u=c,o=d;if(!u){const{data:f}=await E.from("mini_league_matches").select("*").eq("league_id",i),{data:v}=await E.from("mini_league_members").select("*, user:users(id,pseudo,club_name)").eq("league_id",i);o=(v||[]).map(h=>h.user).filter(Boolean),u=En(o,f||[])}const n=[Math.floor(s*.7),Math.floor(s*.2),Math.floor(s*.1)];await Promise.all(u.slice(0,3).map((f,v)=>n[v]?E.from("mini_league_members").update({prize_amount:n[v],prize_claimed:!1}).eq("league_id",i).eq("user_id",f.userId):Promise.resolve())),await E.from("mini_leagues").update({status:"finished"}).eq("id",i),a("🏆 Mini League terminée ! Les gagnants peuvent récupérer leurs crédits.","success"),bt(e,t,i)}async function rr(e,t,i,s,c){var v,h;const{state:d,toast:a}=t,l=[Math.floor((i.pot||0)*.7),Math.floor((i.pot||0)*.2),Math.floor((i.pot||0)*.1)],u=["🥇","🥈","🥉"][c],o=s.prize_amount||l[c]||0,n=s.prize_claimed,f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",f.innerHTML=`
    <div style="background:linear-gradient(160deg,#1a0a2e,#3b1a6e);border-radius:20px;padding:32px 24px;max-width:320px;width:100%;text-align:center;color:#fff;box-shadow:0 8px 40px rgba(0,0,0,0.6)">
    ...
    </div>`,document.body.appendChild(f),(v=f.querySelector("#prize-close"))==null||v.addEventListener("click",()=>f.remove()),f.addEventListener("click",T=>{T.target===f&&f.remove()}),(h=f.querySelector("#claim-prize-btn"))==null||h.addEventListener("click",async()=>{const{data:T}=await E.from("users").select("credits").eq("id",d.profile.id).single();await E.from("users").update({credits:((T==null?void 0:T.credits)||0)+o}).eq("id",d.profile.id),await E.from("mini_league_members").update({prize_claimed:!0}).eq("league_id",i.id).eq("user_id",d.profile.id),d.profile&&(d.profile.credits=((T==null?void 0:T.credits)||0)+o);const r=document.getElementById("nav-credits");r&&(r.textContent=`💰 ${d.profile.credits.toLocaleString("fr")}`),a(`💰 +${o.toLocaleString("fr")} cr. ajoutés à ton solde !`,"success"),f.remove(),bt(e,t,i.id)})}function ar(e,t){const s=e.length%2===0?[...e]:[...e,null],c=s.length;let d=s.slice(1);const a=[];for(let l=0;l<c-1;l++){const u=[],o=[s[0],...d];for(let n=0;n<c/2;n++){const f=o[n],v=o[c-1-n];f===null?u.push({bye:v}):v===null?u.push({bye:f}):u.push({home:f,away:v})}a.push(u),d=[d[d.length-1],...d.slice(0,-1)]}return t==="aller-retour"?[...a,...a.map(l=>l.map(u=>u.bye?u:{home:u.away,away:u.home}))]:a}function En(e,t){const i={};return e.forEach(s=>{i[s.id]={userId:s.id,pseudo:s.pseudo,clubName:s.club_name||s.pseudo,played:0,won:0,drawn:0,lost:0,goalsFor:0,goalsAgainst:0,goalDiff:0,points:0}}),t.filter(s=>s.status==="finished"&&!s.is_bye&&s.home_score!=null).forEach(s=>{const c=i[s.home_id],d=i[s.away_id];!c||!d||(c.played++,d.played++,c.goalsFor+=s.home_score,c.goalsAgainst+=s.away_score,d.goalsFor+=s.away_score,d.goalsAgainst+=s.home_score,s.home_score>s.away_score?(c.won++,c.points+=3,d.lost++):s.home_score<s.away_score?(d.won++,d.points+=3,c.lost++):(c.drawn++,c.points++,d.drawn++,d.points++),c.goalDiff=c.goalsFor-c.goalsAgainst,d.goalDiff=d.goalsFor-d.goalsAgainst)}),Object.values(i).sort((s,c)=>c.points-s.points||c.goalDiff-s.goalDiff||c.goalsFor-s.goalsFor)}async function Oi(e,t,i,s){var c,d;t.state.params={...t.state.params,matchMode:"mini-league",mlMatchId:i,leagueId:s};try{const a=(d=(c=t==null?void 0:t.state)==null?void 0:c.profile)==null?void 0:d.id;try{(E.getChannels?E.getChannels():[]).forEach(u=>{const o=u.topic||"";(o.includes("matchmaking")||o.includes("pvp-match"))&&E.removeChannel(u)})}catch(l){console.warn("[FriendMatch] cleanup canaux:",l)}a&&(await E.rpc("cancel_matchmaking",{p_user_id:a}).catch(()=>{}),await E.from("matchmaking_queue").delete().eq("user_id",a).then(()=>{},()=>{}))}catch{}await ni(e,t,"random",({deckId:a,formation:l,starters:u,subsRaw:o,gcCardsEnriched:n,gcDefs:f,stadiumDef:v})=>{const h=T=>sr(e,t,a,l,u,o,T,f||[],i);if(!n.length){h([]);return}ii(e,n,h)})}async function sr(e,t,i,s,c,d,a=[],l=[],u,o){const{state:n,navigate:f,toast:v}=t,h=n.profile.id;let T=!1,r=null;et(e);try{await E.rpc("cancel_matchmaking",{p_user_id:h})}catch{}try{await E.from("matchmaking_queue").delete().eq("user_id",h)}catch{}const{data:p}=await E.from("mini_league_matches").select("*, home:users!home_id(id,pseudo,club_name), away:users!away_id(id,pseudo,club_name)").eq("id",u).single();if(!p){v("Match introuvable","error"),We(e),f("mini-league");return}if(p.home_id!==h&&p.away_id!==h){v("Tu ne fais pas partie de ce match","error"),We(e),f("mini-league");return}const I=p.home_id===h,z=I?p.away:p.home,C=I?p.away_id:p.home_id,U=(N,G)=>{var ie;e.innerHTML=`
      <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:20px;background:linear-gradient(180deg,#0a1628,#1a0a2e);padding:24px;text-align:center;color:#fff">
        ...
        <button id="lobby-cancel" style="padding:10px 24px;border-radius:10px;border:1.5px solid rgba(255,255,255,0.2);background:transparent;color:rgba(255,255,255,0.6);font-size:13px;cursor:pointer">Annuler</button>
        <style>@keyframes lobbyPulse{0%,100%{opacity:1}50%{opacity:0.4}}</style>
      </div>`,(ie=document.getElementById("lobby-cancel"))==null||ie.addEventListener("click",()=>{T=!0,clearInterval(r),We(e),f("mini-league")})},D=async(N,G)=>{T=!0,clearInterval(r),await new Promise(ie=>setTimeout(ie,600)),e.isConnected&&dr(e,t,N,G,a,l)};if(U(),I){let N=p.match_id;if(!N){const{data:ie,error:se}=await E.from("matches").insert({home_id:h,away_id:C,home_deck_id:i,away_deck_id:null,status:"active",mode:"mini_league"}).select().single();if(se||!ie){v("Erreur création match","error"),We(e),f("mini-league");return}N=ie.id,await E.from("mini_league_matches").update({match_id:N,status:"playing"}).eq("id",u)}const G=N;r=setInterval(async()=>{if(T){clearInterval(r);return}const{data:ie}=await E.from("matches").select("away_deck_id").eq("id",G).single();ie!=null&&ie.away_deck_id&&(clearInterval(r),D(G,!0))},1500)}else{let N=p.match_id;r=setInterval(async()=>{if(T){clearInterval(r);return}const{data:G}=await E.from("mini_league_matches").select("match_id").eq("id",u).single();G!=null&&G.match_id&&(clearInterval(r),N=G.match_id,await E.from("matches").update({away_deck_id:i}).eq("id",N),D(N,!1))},1500)}}async function dr(e,t,i,s,c=[],d=[]){var k;const{state:a,navigate:l,toast:u}=t,o=((k=a.params)==null?void 0:k.leagueId)||null,n=s?"p1":"p2",f=s?"p2":"p1",v=(c||[]).map(g=>g.id),h=(c||[]).map(g=>({id:g.id,gc_type:g.gc_type,_gcDef:g._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:T}=await E.from("matches").select("*").eq("id",i).single();if(!T){u("Match introuvable","error"),l("mini-league");return}async function r(){const[{data:g},{data:m},{data:x},{data:y}]=await Promise.all([E.rpc("get_deck_for_match",{p_deck_id:T.home_deck_id}),E.rpc("get_deck_for_match",{p_deck_id:T.away_deck_id}),E.from("users").select("id,pseudo,club_name").eq("id",T.home_id).single(),E.from("users").select("id,pseudo,club_name").eq("id",T.away_id).single()]),b=L=>({cardId:L.card_id,position:L.position,id:L.id,firstname:L.firstname,name:L.surname_encoded,country_code:L.country_code,club_id:L.club_id,job:L.job,job2:L.job2,note_g:Number(L.note_g)||0,note_d:Number(L.note_d)||0,note_m:Number(L.note_m)||0,note_a:Number(L.note_a)||0,rarity:L.rarity,skin:L.skin,hair:L.hair,hair_length:L.hair_length,clubName:L.club_encoded_name||null,clubLogo:L.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}),$=((g==null?void 0:g.starters)||[]).map(L=>b(L)),A=((m==null?void 0:m.starters)||[]).map(L=>b(L)),_=(g==null?void 0:g.formation)||"4-4-2",S=(m==null?void 0:m.formation)||"4-4-2";return{p1Team:yt($,_),p2Team:yt(A,S),p1Subs:((g==null?void 0:g.subs)||[]).map(L=>b(L)),p2Subs:((m==null?void 0:m.subs)||[]).map(L=>b(L)),p1Formation:_,p2Formation:S,p1Name:(x==null?void 0:x.club_name)||(x==null?void 0:x.pseudo)||"Joueur 1",p2Name:(y==null?void 0:y.club_name)||(y==null?void 0:y.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:s?v:[],gc_p2:s?[]:v,gcCardsFull_p1:s?h:[],gcCardsFull_p2:s?[]:h,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:d||[],lastActionAt:new Date().toISOString()}}let p=T.game_state&&Object.keys(T.game_state).length?T.game_state:null;if(!p)if(s){p=await r();const{data:g}=await E.from("matches").select("game_state").eq("id",i).single();!(g!=null&&g.game_state)||!Object.keys(g.game_state).length?await E.from("matches").update({game_state:p,turn_user_id:T.home_id}).eq("id",i):p=g.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let g=0;g<60&&!p;g++){await new Promise(x=>setTimeout(x,400));const{data:m}=await E.from("matches").select("game_state").eq("id",i).single();m!=null&&m.game_state&&Object.keys(m.game_state).length&&(p=m.game_state)}if(!p){u("Erreur de synchronisation","error"),l("mini-league");return}p.gc_p2=v,p.gcCardsFull_p2=h,await E.from("matches").update({game_state:p}).eq("id",i)}let I=!1,z=!1,C=!1,U=null,D=!1;const N=new Set,G=new Set;function ie(g){var L,j;try{E.removeChannel(se)}catch{}const m=p[n+"Score"]||0,x=p[f+"Score"]||0;if(!C){C=!0;const B=g.winner_id||(m>x?a.profile.id:x>m?"opp":null),R=B===a.profile.id?"win":B?"loss":null;R&&yi(()=>Promise.resolve().then(()=>on),void 0).then(H=>H.applyOwnEvolution(a.profile.id,R)).catch(()=>{})}if(!z){const B=p.p1Score||0,R=p.p2Score||0,H=(p.usedGc_p1||[]).length,W=(p.usedGc_p2||[]).length,K=g.winner_id||(B>R?T.home_id:R>B?T.away_id:null);(K?a.profile.id===K:a.profile.id<(s?T.away_id:T.home_id))&&(z=!0,Xi({player1Id:T.home_id,player2Id:T.away_id,score1:B,score2:R,gc1:H,gc2:W}).catch(ne=>console.warn("[FriendMatch] updateStats:",ne)))}let y,b;g.winner_id?(y=g.winner_id===a.profile.id,b=!1):g.forfeit?(y=m>x,b=!1):(b=m===x,y=m>x),(L=document.getElementById("pvp-end-overlay"))==null||L.remove();const $=document.createElement("div");$.id="pvp-end-overlay",$.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const A=b?"🤝":y?"🏆":"😞",_=b?"MATCH NUL":y?"VICTOIRE !":"DÉFAITE",S=b?"#fff":y?"#FFD700":"#ff6b6b";$.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">🏆 Retour à la Mini League</button>`,document.body.appendChild($),(j=$.querySelector("#pvp-end-home"))==null||j.addEventListener("click",()=>{$.remove(),We(e),o?l("mini-league",{openLeagueId:o}):l("mini-league")})}const se=E.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},g=>{const m=g.new;try{if(m.status==="finished"||m.forfeit){if(I||(I=!0,U&&(clearInterval(U),U=null),m.game_state&&(p=m.game_state),p.phase==="finished"&&!m.forfeit&&document.getElementById("pvp-home")))return;ie(m);return}if(m.game_state){const x=p;p=m.game_state;const y=p._lastGC;if(y&&y.seq&&!G.has(y.seq)&&(G.add(y.seq),y.by!==n)){Ee(y.type,y.by,()=>ye());return}const b=x[n+"Score"]||0,$=x[f+"Score"]||0,A=p[n+"Score"]||0,_=p[f+"Score"]||0,S=A>b,L=_>$;if((S||L)&&!N.has(p.round)){N.add(p.round);const j=[...p.log||[]].reverse().find(R=>R.isGoal),B=((j==null?void 0:j.homePlayers)||[]).map(R=>({name:R.name,note:R.note,portrait:R.portrait,job:R.job}));$e(B,A,_,S,()=>ye());return}ye()}}catch(x){console.error("[PvP] crash:",x)}}).subscribe();async function de(g){Object.assign(p,g),p.lastActionAt=new Date().toISOString();const{error:m}=await E.from("matches").update({game_state:p}).eq("id",i);m&&u("Erreur de synchronisation","error");try{ye()}catch(x){console.error("[PvP] renderPvpScreen crash:",x)}}async function P(){if(I)return;I=!0,U&&(clearInterval(U),U=null);const g=s?T.away_id:T.home_id,m=s?"p2":"p1",x=s?"p1":"p2",y={...p,[m+"Score"]:3,[x+"Score"]:0,phase:"finished"};try{await E.from("matches").update({status:"finished",forfeit:!0,winner_id:g,home_score:y.p1Score||0,away_score:y.p2Score||0,game_state:y}).eq("id",i)}catch{}try{E.removeChannel(se)}catch{}setTimeout(()=>{We(e),l("mini-league")},800)}const Y={BOOST_STAT:({value:g=1,count:m=1,roles:x=[]},y,b)=>{const $=y[n+"Team"],A=Object.entries($).filter(([_])=>!x.length||x.includes(_)).flatMap(([_,S])=>S.filter(L=>!L.used).map(L=>({...L,_line:_})));if(!A.length){y.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),b(y);return}V(A,m,`Choisir ${m} joueur(s) à booster (+${g})`,_=>{_.forEach(S=>{const L=($[S._line]||[]).find(j=>j.cardId===S.cardId);L&&(L.boost=(L.boost||0)+g,y.log.push({text:`⚡ +${g} sur ${L.name}`,type:"info"}))}),y[n+"Team"]=$,b(y)})},DEBUFF_STAT:({value:g=1,count:m=1,roles:x=[],target:y="ai"},b,$)=>{const A=y==="home"?n:f,_=b[A+"Team"],S=y==="home"?"allié":"adverse",L=Object.entries(_).filter(([j])=>!x.length||x.includes(j)).flatMap(([j,B])=>B.map(R=>({...R,_line:j})));if(!L.length){b.log.push({text:`🎯 Aucun joueur ${S}`,type:"info"}),$(b);return}V(L,m,`Choisir ${m} joueur(s) ${S}(s) (-${g})`,j=>{j.forEach(B=>{const R=(_[B._line]||[]).find(H=>H.cardId===B.cardId);R&&(R.boost=(R.boost||0)-g,b.log.push({text:`🎯 -${g} sur ${R.name}`,type:"info"}))}),b[A+"Team"]=_,$(b)})},GRAY_PLAYER:({count:g=1,roles:m=[],target:x="ai"},y,b)=>{const $=x==="home"?n:f,A=y[$+"Team"],_=x==="home"?"allié":"adverse",S=Object.entries(A).filter(([L])=>!m.length||m.includes(L)).flatMap(([L,j])=>j.filter(B=>!B.used).map(B=>({...B,_line:L})));if(!S.length){y.log.push({text:`❌ Aucun joueur ${_}`,type:"info"}),b(y);return}V(S,g,`Choisir ${g} joueur(s) ${_}(s) à exclure`,L=>{const j="usedCardIds_"+$,B=new Set(y[j]||[]);L.forEach(R=>{const H=(A[R._line]||[]).find(W=>W.cardId===R.cardId);H&&(H.used=!0,B.add(R.cardId),y.log.push({text:`❌ ${H.name} exclu !`,type:"info"}))}),y[j]=[...B],y[$+"Team"]=A,b(y)})},REVIVE_PLAYER:({count:g=1,roles:m=[]},x,y)=>{const b=x[n+"Team"],$=Object.entries(b).filter(([A])=>!m.length||m.includes(A)).flatMap(([A,_])=>_.filter(S=>S.used).map(S=>({...S,_line:A})));if(!$.length){x.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),y(x);return}V($,g,`Choisir ${g} joueur(s) à ressusciter`,A=>{A.forEach(_=>{const S=(b[_._line]||[]).find(L=>L.cardId===_.cardId);S&&(S.used=!1,x.log.push({text:`💫 ${S.name} ressuscité !`,type:"info"}))}),x[n+"Team"]=b,y(x)})},REMOVE_GOAL:({},g,m)=>{const x=f+"Score";g[x]>0?(g[x]--,g.log.push({text:"🚫 Dernier but annulé !",type:"info"})):g.log.push({text:"🚫 Aucun but à annuler",type:"info"}),m(g)},ADD_GOAL_DRAW:({},g,m)=>{g[n+"Score"]===g[f+"Score"]?(g[n+"Score"]++,g.log.push({text:"🎯 But bonus !",type:"info"})):g.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),m(g)},ADD_SUB:({value:g=1},m,x)=>{m.maxSubs=(m.maxSubs||3)+g,m.log.push({text:`🔄 +${g} remplacement(s)`,type:"info"}),x(m)},CUSTOM:({},g,m)=>m(g)};function V(g,m,x,y){const b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let $=[];function A(){var S,L;const _=g.map(j=>{const B={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[j._line]||"#555",R=Te(j,j._line)+(j.boost||0),W=$.find(ee=>ee.cardId===j.cardId)?"#FFD700":"rgba(255,255,255,0.25)",K=j.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${j.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${W};background:${B};overflow:hidden;cursor:pointer;${K}">
        ...
        </div>`,(S=b.querySelector("#pp-close"))==null||S.addEventListener("click",()=>b.remove()),b.querySelectorAll(".pp-item").forEach(j=>{j.addEventListener("click",()=>{const B=j.dataset.cid,R=g.find(W=>W.cardId===B),H=$.findIndex(W=>W.cardId===B);R&&(H>-1?$.splice(H,1):$.length<m&&$.push(R),A())})}),(L=b.querySelector("#pp-confirm"))==null||L.addEventListener("click",()=>{b.remove(),y($)})}A(),document.body.appendChild(b)}async function F(g,m){const y=(p["gcCardsFull_"+n]||[]).find(_=>_.id===g),b=(y==null?void 0:y._gcDef)||(p.gcDefs||[]).find(_=>{var S;return _.name===m||((S=_.name)==null?void 0:S.toLowerCase().trim())===(m==null?void 0:m.toLowerCase().trim())}),$=[...p["usedGc_"+n]||[],g],A={type:m,by:n,seq:(p._gcAnimSeq||0)+1};G.add(A.seq),Ee(m,n,async()=>{if(b!=null&&b.effect_type&&b.effect_type!=="CUSTOM"){const S=Y[b.effect_type];if(S){const L={...p};S(b.effect_params||{},L,async j=>{j["usedGc_"+n]=$,j._lastGC=A,j._gcAnimSeq=A.seq,await de(j)});return}}const _={...p};switch(m){case"Remplacement+":_.maxSubs=(_.maxSubs||3)+1,_.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const S=f+"Score";_[S]>0&&(_[S]--,_.log.push({text:"🚫 But annulé",type:"info"}));break}}_["usedGc_"+n]=$,_._lastGC=A,_._gcAnimSeq=A.seq,await de(_)})}function oe(g,m){const x="usedCardIds_"+g,y=new Set(p[x]||[]);m.forEach(b=>y.add(b)),p[x]=[...y]}function pe(){for(const g of["p1","p2"]){const m=new Set(p["usedCardIds_"+g]||[]),x=p[g+"Team"];if(x)for(const y of["GK","DEF","MIL","ATT"])(x[y]||[]).forEach(b=>{b.used=m.has(b.cardId)})}}function ye(){var ot,gt,ae,_e,Q,ce;if(p.phase==="reveal")return ze();if(p.phase==="midfield")return we();if(p.phase==="finished")return w();const g=p[n+"Team"],m=p[f+"Team"];pe();const x=p[n+"Score"],y=p[f+"Score"],b=p[n+"Name"],$=p[f+"Name"],A=p.phase===n+"-attack",_=p.phase===n+"-defense",S=Array.isArray(p["selected_"+n])?p["selected_"+n]:[],L=S.map(J=>J.cardId),j=window.innerWidth>=700,B=p[n+"Subs"]||[],R=p["usedSubIds_"+n]||[],H=B.filter(J=>!R.includes(J.cardId)),W=p["gcCardsFull_"+n]||[],K=p["usedGc_"+n]||[],ee=W.filter(J=>!K.includes(J.id)),ne=p.boostOwner===n&&!p.boostUsed,fe=!["GK","DEF","MIL","ATT"].some(J=>(m[J]||[]).some(le=>!le.used)),xe=[...g.MIL||[],...g.ATT||[]].filter(J=>!J.used),he=A&&fe&&xe.length===0?[...g.GK||[],...g.DEF||[]].filter(J=>!J.used).map(J=>J.cardId):[];function ve(J,le,Me){var zi,Si;const ke=J._gcDef,tt={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ke==null?void 0:ke.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",it={purple:"#b06ce0",light_blue:"#00d4ef"}[ke==null?void 0:ke.color]||"#b06ce0",rt=(ke==null?void 0:ke.name)||J.gc_type,st=(ke==null?void 0:ke.effect)||((zi=Ke[J.gc_type])==null?void 0:zi.desc)||"",ct=ke!=null&&ke.image_url?`/manager-wars/icons/${ke.image_url}`:null,$t=((Si=Ke[J.gc_type])==null?void 0:Si.icon)||"⚡",pt=Math.round(Me*.22),ht=Math.round(Me*.22),kt=Me-pt-ht,Ht=rt.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${J.id}" data-gc-type="${J.gc_type}"
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ...
        </div>`}if(A&&((Me=p.pendingAttack)!=null&&Me.players)){const ke=p.pendingAttack;return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #00ff88">
          <div style="font-size:8px;color:#00ff88;letter-spacing:2px;margin-bottom:4px;text-transform:uppercase">⚔️ VOUS ATTAQUEZ</div>
    ...
    </div>`,n==="p1"&&setTimeout(async()=>{await de({phase:"midfield"})},5e3)}let Ae=!1;function we(){if(Ae)return;const g=p[n+"Team"].MIL||[],m=p[f+"Team"].MIL||[];function x(ee){return ee.reduce((ne,fe)=>ne+Te(fe,"MIL"),0)}function y(ee){let ne=0;for(let fe=0;fe<ee.length-1;fe++){const xe=at(ee[fe],ee[fe+1]);xe==="#00ff88"?ne+=2:xe==="#FFD700"&&(ne+=1)}return ne}const b=x(g)+y(g),$=x(m)+y(m),A=b>=$;function _(ee,ne,fe){return`<div id="duel-row-${fe}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="display:flex;gap:12px;width:190px">
        <button id="pvp-gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
        <button id="pvp-gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
      </div>`,document.body.appendChild(B),(W=B.querySelector("#pvp-gc-back"))==null||W.addEventListener("click",()=>B.remove()),(K=B.querySelector("#pvp-gc-use"))==null||K.addEventListener("click",()=>{B.remove(),F(g,m)})}function re(){var y;const g=p[n+"Team"],m=Object.entries(g).flatMap(([b,$])=>($||[]).filter(A=>!A.used).map(A=>({...A,_line:b})));if(!m.length)return;const x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",x.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
        <div style="flex:1;font-size:14px;font-weight:700;color:#fff">⚡ Choisir un joueur pour +${p.boostValue}</div>
        <button id="bp-close" style="background:none;border:none;color:rgba(255,255,255,0.5);font-size:22px;cursor:pointer">✕</button>
      </div>
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${m.map(b=>{const $={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[b._line]||"#555",A=Te(b,b._line)+(b.boost||0);return`<div class="bp-item" data-cid="${b.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${$};overflow:hidden;cursor:pointer">
      ...
      </div>`,document.body.appendChild(x),(y=x.querySelector("#bp-close"))==null||y.addEventListener("click",()=>x.remove()),x.querySelectorAll(".bp-item").forEach(b=>{b.addEventListener("click",async()=>{const $=b.dataset.cid,A=m.find(S=>S.cardId===$);if(!A)return;const _=(g[A._line]||[]).find(S=>S.cardId===$);_&&(_.boost=(_.boost||0)+p.boostValue),x.remove(),await de({[n+"Team"]:g,boostUsed:!0})})})}function O(g=null){var H,W;if(!(p.phase===n+"-attack")){u("Remplacement uniquement avant votre attaque","warning");return}const x=p[n+"Team"],y=p["usedSubIds_"+n]||[],b=p.maxSubs||3;if(y.length>=b){u(`Maximum ${b} remplacements atteint`,"warning");return}const $=Object.entries(x).flatMap(([K,ee])=>(ee||[]).filter(ne=>ne.used).map(ne=>({...ne,_line:K}))),A=(p[n+"Subs"]||[]).filter(K=>!y.includes(K.cardId));if(!$.length){u("Aucun joueur utilisé à remplacer","warning");return}if(!A.length){u("Aucun remplaçant disponible","warning");return}let _=Math.max(0,$.findIndex(K=>K.cardId===g));const S=((H=$[_])==null?void 0:H._line)||((W=$[_])==null?void 0:W.job);let L=Math.max(0,A.findIndex(K=>K.job===S)),j=!1;const B=document.createElement("div");B.id="pvp-sub-overlay",B.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function R(){var ve,Ie,Le,Re,je,Oe;const K=$[_],ee=A[L],ne=Math.min(130,Math.round((window.innerWidth-90)/2)),fe=Math.round(ne*1.35),xe=Be=>`background:rgba(255,255,255,0.12);border:none;color:${Be?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Be?"default":"pointer"};flex-shrink:0`;B.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:6px">Appuyer pour continuer</div>`,document.body.appendChild(b);let A=!1;const _=()=>{A||(A=!0,b.remove(),setTimeout(()=>x(),50))};b.addEventListener("click",_),setTimeout(_,2200)}function ue(){var m;const g=document.createElement("div");g.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:20px;overflow-y:auto",g.innerHTML=`
      <div style="font-size:12px;color:rgba(255,255,255,0.5);letter-spacing:2px;text-transform:uppercase">Équipe adverse</div>
      <div style="font-size:18px;font-weight:900;color:#ff6b6b">${p[f+"Name"]}</div>
      <div style="width:min(90vw,420px)">${ut(p[f+"Team"],p[f+"Formation"],null,[],300,300)}</div>
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(g),(m=g.querySelector("#pvp-opp-close"))==null||m.addEventListener("click",()=>g.remove())}function me(){var y;const g=p.log||[],m=document.createElement("div");m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column";const x=b=>{var A,_,S;if(b.type==="duel"){const L=b.isGoal,j=b.homeScored&&n==="p1"||!b.homeScored&&L&&n==="p2",B=b.homeScored?"#FFD700":L?"#ff6b6b":"rgba(255,255,255,0.3)",R=L?j?"⚽ BUT !":"⚽ BUT adversaire !":(A=b.homePlayers)!=null&&A.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${L?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${B};margin-bottom:4px">
      ...
      </div>`,document.body.appendChild(m),(y=m.querySelector("#pvp-hist-close"))==null||y.addEventListener("click",()=>m.remove())}async function X(){if(p.phase!==n+"-attack")return;const g=n==="p1"?"p2":"p1",m=(p.round||0)+1,x=[...p.log||[]];x.push({type:"info",text:`⏭️ ${p[n+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const y=De(p),b=qe(g),$=y||!b?"finished":g+"-attack";await de({["selected_"+n]:[],modifiers:{...p.modifiers,[n]:{}},pendingAttack:null,phase:$,attacker:g,round:m,log:x}),$==="finished"&&await q(p)}async function be(){const g=p[n+"Team"],m=!["GK","DEF","MIL","ATT"].some($=>(p[f+"Team"][$]||[]).some(A=>!A.used)),x=(p["selected_"+n]||[]).map($=>{const A=(g[$._role]||[]).find(R=>R.cardId===$.cardId)||$,_=m&&["GK","DEF"].includes($._role),S=g[$._role]||[],L=S.findIndex(R=>R.cardId===$.cardId),j=lt(S.length),B=L>=0?j[L]:A._col??1;return{...A,_line:$._role,_col:B,..._?{note_a:Math.max(1,Number(A.note_a)||0)}:{}}});if(!x.length)return;const y=Nt(x,p.modifiers[n]||{});oe(n,x.map($=>$.cardId)),x.forEach($=>{const A=(g[$._role]||[]).find(_=>_.cardId===$.cardId);A&&(A.used=!0)}),p["selected_"+n]=[],ye();const b=[...p.log||[]];if(m){const $=(p[n+"Score"]||0)+1,A=x.map(B=>({name:B.name,note:Te(B,B._line||"ATT"),portrait:Pe(B),job:B.job}));b.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:A,homeTotal:y.total,aiTotal:0});const _=(p.round||0)+1,S=n==="p1"?"p2":"p1",L={...p,[n+"Team"]:g,[n+"Score"]:$},j=De(L);N.add(_),$e(A,$,p[f+"Score"]||0,!0,async()=>{await de({[n+"Team"]:g,[n+"Score"]:$,["selected_"+n]:[],modifiers:{...p.modifiers,[n]:{}},pendingAttack:null,phase:j?"finished":S+"-attack",attacker:S,round:_,log:b}),j&&await q({...p,[n+"Score"]:$})});return}b.push({type:"pending",text:`⚔️ ${p[n+"Name"]} attaque (${y.total})`}),await de({[n+"Team"]:g,[f+"Team"]:p[f+"Team"],pendingAttack:{...y,players:x,side:n},["selected_"+n]:[],modifiers:{...p.modifiers,[n]:{}},phase:f+"-defense",log:b})}async function ge(){const g=p[n+"Team"],m=(p["selected_"+n]||[]).map(K=>{const ee=(g[K._role]||[]).find(ve=>ve.cardId===K.cardId)||K,ne=g[K._role]||[],fe=ne.findIndex(ve=>ve.cardId===K.cardId),xe=lt(ne.length),he=fe>=0?xe[fe]:ee._col??1;return{...ee,_line:K._role,_col:he}}),x=Rt(m,p.modifiers[n]||{});oe(n,m.map(K=>K.cardId)),m.forEach(K=>{const ee=(g[K._role]||[]).find(ne=>ne.cardId===K.cardId);ee&&(ee.used=!0)}),p["selected_"+n]=[],ye();const y=Ot(p.pendingAttack.total,x.total,p.modifiers[n]||{}),b=p.pendingAttack.side,$=y==="attack"||(y==null?void 0:y.goal),A=b==="p1"?"p2":"p1",_=(p.round||0)+1,S=(p.pendingAttack.players||[]).map(K=>({name:K.name,note:Te(K,K._line||"ATT"),portrait:Pe(K),job:K.job})),L=[...p.log||[]];L.push({type:"duel",isGoal:$,homeScored:$&&b===n,text:$?`⚽ BUT de ${p[b+"Name"]} ! (${p.pendingAttack.total} vs ${x.total})`:`✋ Attaque stoppée (${p.pendingAttack.total} vs ${x.total})`,homePlayers:S,aiPlayers:m.map(K=>({name:K.name,note:Te(K,K._line||"DEF"),portrait:Pe(K),job:K.job})),homeTotal:p.pendingAttack.total,aiTotal:x.total});const j=$?(p[b+"Score"]||0)+1:p[b+"Score"]||0,B={...p,[n+"Team"]:g,[b+"Score"]:j},R=De(B),H=R?"finished":A+"-attack",W=async()=>{await de({[n+"Team"]:g,[f+"Team"]:p[f+"Team"],[b+"Score"]:j,["selected_"+n]:[],modifiers:{...p.modifiers,[n]:{}},pendingAttack:null,phase:H,attacker:A,round:_,log:L}),(H==="finished"||R)&&await q({...p,[b+"Score"]:j})};if($){const K=b===n,ee=K?j:p[n+"Score"]||0,ne=K?p[f+"Score"]||0:j;N.add(_),$e(S,ee,ne,K,W)}else await W()}function Se(g){return["MIL","ATT"].some(m=>(g[m]||[]).some(x=>!x.used))}function Ce(g){return["GK","DEF","MIL","ATT"].some(m=>(g[m]||[]).some(x=>!x.used))}function Fe(g){return Ce(g)&&!Se(g)}function qe(g){const m=p[g+"Team"],x=p[(g==="p1"?"p2":"p1")+"Team"];return!!(Se(m)||!Ce(x)&&Fe(m))}function De(g){const m=["MIL","ATT"].some(L=>(g.p1Team[L]||[]).some(j=>!j.used)),x=["MIL","ATT"].some(L=>(g.p2Team[L]||[]).some(j=>!j.used)),y=Ce(g.p1Team),b=Ce(g.p2Team);return!m&&!(!b&&y)&&(!x&&!(!y&&b))}function M(g){const m=g.p1Score||0,x=g.p2Score||0;return m===x?null:m>x?T.home_id:T.away_id}async function q(g){try{const m=M(g),x=m?T.home_id===m?T.away_id:T.home_id:null,y=g.p1Score||0,b=g.p2Score||0;await E.from("matches").update({status:"finished",winner_id:m,home_score:y,away_score:b}).eq("id",i);const{data:$}=await E.from("mini_league_matches").select("id, league_id, matchday").eq("match_id",i).single();if($){await E.from("mini_league_matches").update({home_score:y,away_score:b,status:"finished"}).eq("id",$.id);const{data:A}=await E.from("mini_league_matches").select("id, status").eq("league_id",$.league_id).eq("matchday",$.matchday);if((A||[]).every(S=>S.status==="finished"||S.status==="bye")){const{data:S}=await E.from("mini_leagues").select("current_day,total_days").eq("id",$.league_id).single();if(S){const L=(S.current_day||0)+1,j=L>(S.total_days||0);await E.from("mini_leagues").update({current_day:j?S.total_days:L,status:j?"finished":"active"}).eq("id",$.league_id)}}}m&&x&&_i(m,x).catch(()=>{})}catch(m){console.error("[ML] finishMatch:",m)}}function w(){var b;if(I&&document.getElementById("pvp-end-overlay"))return;I=!0;const g=p[n+"Score"],m=p[f+"Score"],x=g>m,y=g===m;et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
    ...
    </div>`,(b=document.getElementById("pvp-home"))==null||b.addEventListener("click",()=>{try{E.removeChannel(se)}catch{}We(e),l("mini-league",o?{openLeagueId:o}:{})})}ye()}const lr="/manager-wars/",cr=[{emoji:"⚽",title:"Bienvenue dans Manager Wars !",color:"#1A6B3C",content:`<p>Tu es désormais un <strong>manager de football</strong> virtuel.</p>
    ...
    <p style="margin-top:12px;font-size:13px;color:#888">Tu peux revoir ce tutoriel depuis les paramètres à tout moment.</p>`}];function pr(e,t,i){let s=0;const c=document.createElement("div");c.id="tutorial-overlay",c.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.65);z-index:9900;display:flex;align-items:center;justify-content:center;padding:16px";const d=()=>{var n,f,v;const l=t[s],u=s===t.length-1,o=Math.round((s+1)/t.length*100);c.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    </div>`}function v(r){var U,D,N;const p=(U=r.card)==null?void 0:U.player;if(!p)return"";const I=Et(p,p.job),z=p.job2?Et(p,p.job2):0,C=r.status==="sold";return`<div class="card-panel" style="display:flex;align-items:center;gap:8px;padding:10px 12px;overflow:hidden;padding-left:6px;${C?"opacity:0.7":""}">
      ${Hi(p.rarity)}
      ${Yt(I,p.job)}
      ${Yt(z,p.job2||null)}
      ${Ui(p.country_code)}
    ...
    </div>`}function h(){const r=document.getElementById("mkt-content"),p=document.getElementById("mkt-filters");if(r){if(p.style.display=u==="buy"?"flex":"none",u==="buy"){const I=n(a);r.innerHTML=I.length?I.map(f).join(""):'<div style="text-align:center;color:#aaa;padding:40px">Aucune carte trouvée.</div>'}else{const I=l.filter(C=>C.status==="active").sort((C,U)=>new Date(U.listed_at)-new Date(C.listed_at)),z=l.filter(C=>C.status==="sold").sort((C,U)=>new Date(U.sold_at||U.listed_at)-new Date(C.sold_at||C.listed_at));r.innerHTML=(I.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:4px 0 6px;text-transform:uppercase;letter-spacing:1px">🟢 En vente (${I.length})</div>`+I.map(v).join(""):"")+(z.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:12px 0 6px;text-transform:uppercase;letter-spacing:1px">✅ Ventes réussies (${z.length})</div>`+z.map(v).join(""):"")+(!I.length&&!z.length?'<div style="text-align:center;color:#aaa;padding:40px">Aucune vente pour le moment.</div>':"")}r.querySelectorAll("[data-buy]").forEach(I=>I.addEventListener("click",()=>mr(I.dataset.buy,a,e,t))),r.querySelectorAll("[data-cancel]").forEach(I=>I.addEventListener("click",()=>yr(I.dataset.cancel,e,t)))}}e.querySelectorAll(".mkt-tab").forEach(r=>{r.addEventListener("click",()=>{u=r.dataset.tab,e.querySelectorAll(".mkt-tab").forEach(p=>{const I=p===r;p.style.background=I?"var(--green)":"#fff",p.style.color=I?"#fff":"var(--gray-600)",p.style.borderColor=I?"var(--green)":"var(--gray-200)"}),h()})});let T;["flt-name","flt-club","flt-country","flt-job","flt-rarity","flt-note1","flt-note2"].forEach(r=>{var p;(p=document.getElementById(r))==null||p.addEventListener("input",()=>{clearTimeout(T),T=setTimeout(h,250)})}),h()}async function mr(e,t,i,s){const{state:c,toast:d,refreshProfile:a}=s,l=t.find(n=>n.id===e);if(!l)return;const u=l.price;if((c.profile.credits||0)<u){d("Crédits insuffisants","error");return}xr(l,async()=>{const{error:n}=await E.rpc("buy_market_card",{p_listing_id:e,p_buyer_id:c.profile.id});if(n){d("Erreur achat : "+n.message,"error");return}await a();const f=document.getElementById("nav-credits");f&&(f.textContent=`💰 ${(c.profile.credits||0).toLocaleString("fr")}`),d("✅ Carte achetée !","success"),await Ti(i,s)})}function xr(e,t){var a;const i=(a=e.card)==null?void 0:a.player,s=i?`${i.firstname} ${i.surname_encoded}`:"cette carte",c=document.createElement("div");c.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",c.innerHTML=`
  ...
  `}async function hr(e,{state:t,navigate:i,toast:s}){const c=t.profile;if(!c)return;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:d}=await E.from("matches").select(`id,home_id,away_id,home_score,away_score,status,mode,winner_id,created_at,played_at,
      ...
      away:users!away_id(pseudo,club_name)`).or(`home_id.eq.${c.id},away_id.eq.${c.id}`).order("created_at",{ascending:!1}).limit(50),a={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},l=(d||[]).filter(n=>n.status==="finished"),u=(d||[]).filter(n=>n.status==="in_progress");function o(n){const f=n.home_id===c.id;f?n.home_score:n.away_score,f?n.away_score:n.home_score;const v=n.winner_id===c.id,h=n.home_score===n.away_score&&n.status==="finished",T=n.status!=="finished"?"…":h?"N":v?"V":"D",r=n.status!=="finished"||h?"#888":v?"#1A6B3C":"#c0392b";let p=a[n.mode]||n.mode;n.away_id===null&&!p.startsWith("IA")&&(p="IA");const z=n.home_id===c.id?n.away:n.home;let C;n.away_id===null?C=p:z?C=`${z.club_name||z.pseudo} (${z.pseudo})`:C="Adversaire";let U="";n.status==="in_progress"&&Date.now()-new Date(n.created_at).getTime()>3600*1e3&&(U=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const D=new Date(n.created_at),N=D.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+D.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),G=n.status==="finished"?`${n.home_score} - ${n.away_score}`:`${n.home_score||0} - ${n.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
