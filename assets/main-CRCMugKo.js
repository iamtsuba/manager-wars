...
import{s as T,F as ki,i as Ot,k as eo,l as nt,m as ji,f as Pt,n as Lo,T as ci,j as Io,o as Mo,b as jo}from"./formation-links-BZe0pGr4.js";const Co="/";function Ci(e,{navigate:t,toast:i}){let n="login";const d=()=>{var r,l,m,a,o,y;const s=n==="login";e.innerHTML=`
    <div style="
  ...
  `;function l(){var _;const a=document.getElementById("logo-preview"),o=document.getElementById("logo-initials"),y=((_=document.getElementById("setup-club"))==null?void 0:_.value)||"MW",p=y.trim().split(" ").filter(Boolean),x=p.length>=2?(p[0][0]+p[1][0]).toUpperCase():y.slice(0,2).toUpperCase();a&&(a.style.background=r,a.style.borderColor=s),o&&(o.textContent=x,o.style.color=s)}document.getElementById("color1").addEventListener("input",a=>{s=a.target.value,document.getElementById("swatch1").style.background=s,l()}),document.getElementById("color2").addEventListener("input",a=>{r=a.target.value,document.getElementById("swatch2").style.background=r,l()});function m(a){document.querySelectorAll(".setup-step").forEach(o=>o.classList.remove("active")),document.getElementById(`step-${a}`).classList.add("active"),document.getElementById("step-num").textContent=a,document.getElementById("progress-fill").style.width=`${Math.round(a/3*100)}%`,a===3&&l()}document.getElementById("step1-next").addEventListener("click",async()=>{const a=document.getElementById("setup-pseudo").value.trim(),o=document.getElementById("step1-error");if(o.textContent="",a.length<3){o.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:y}=await T.from("users").select("id").eq("pseudo",a).maybeSingle();if(y){o.textContent="Ce pseudo est déjà pris.";return}m(2)}),document.getElementById("step2-back").addEventListener("click",()=>m(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const a=document.getElementById("setup-club").value.trim(),o=document.getElementById("step2-error");if(o.textContent="",a.length<2){o.textContent="Nom trop court (min. 2 caractères).";return}const{data:y}=await T.from("users").select("id").eq("club_name",a).maybeSingle();if(y){o.textContent="Ce nom de club est déjà pris.";return}m(3)}),document.getElementById("step3-back").addEventListener("click",()=>m(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const a=document.getElementById("setup-pseudo").value.trim(),o=document.getElementById("setup-club").value.trim(),y=document.getElementById("step3-error"),p=document.getElementById("step3-finish");y.textContent="",p.disabled=!0,p.textContent="Création en cours…";try{const{error:x}=await T.from("users").insert({id:t.user.id,pseudo:a,club_name:o,club_color1:s,club_color2:r,credits:1e4});if(x)throw x;await Bo(t.user.id),await d(),n(`Bienvenue ${a} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(x){y.textContent=x.message,p.disabled=!1,p.textContent="🚀 Créer mon profil !"}})}async function Bo(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await T.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const Fo="modulepreload",Do=function(e){return"/"+e},qi={},ni=function(t,i,n){let d=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const r=document.querySelector("meta[property=csp-nonce]"),l=(r==null?void 0:r.nonce)||(r==null?void 0:r.getAttribute("nonce"));d=Promise.allSettled(i.map(m=>{if(m=Do(m),m in qi)return;qi[m]=!0;const a=m.endsWith(".css"),o=a?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${m}"]${o}`))return;const y=document.createElement("link");if(y.rel=a?"stylesheet":Fo,a||(y.as="script"),y.crossOrigin="",y.href=m,l&&y.setAttribute("nonce",l),document.head.appendChild(y),a)return new Promise((p,x)=>{y.addEventListener("load",p),y.addEventListener("error",()=>x(new Error(`Unable to preload CSS for ${m}`)))})}))}function s(r){const l=new Event("vite:preloadError",{cancelable:!0});if(l.payload=r,window.dispatchEvent(l),!l.defaultPrevented)throw r}return d.then(r=>{for(const l of r||[])l.status==="rejected"&&s(l.reason);return t().catch(s)})},Nt="#1A6B3C",Rt="#cc2222",Go="#D4A017",Bi="#888";async function Po(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    </div>`,await to(i,n,t),document.getElementById("btn-add-friend").addEventListener("click",()=>Ro(i,n)),document.getElementById("btn-accept-friend").addEventListener("click",()=>io(i,n,null,t))}async function to(e,t,i={}){const{navigate:n}=i,d=e.user.id,{data:s,error:r}=await T.from("friendships").select("id, requester_id, addressee_id").eq("status","accepted").or(`requester_id.eq.${d},addressee_id.eq.${d}`),{count:l}=await T.from("friendships").select("id",{count:"exact",head:!0}).eq("addressee_id",d).eq("status","pending"),m=document.getElementById("pending-badge");m&&(l>0?(m.style.display="flex",m.textContent=l):m.style.display="none");const a=document.getElementById("friends-list");if(!a)return;if(r){console.error("[Friends] Erreur:",r),a.innerHTML=`<div style="color:${Rt};text-align:center;padding:16px">Erreur chargement : ${r.message}</div>`;return}const o=(s||[]).map(x=>x.requester_id===d?x.addressee_id:x.requester_id);let y={};if(o.length){const{data:x}=await T.from("users").select("id, pseudo, club_name, last_seen_at, club_color1, club_color2").in("id",o);(x||[]).forEach(_=>{y[_.id]=_})}const p=(s||[]).map(x=>({friendshipId:x.id,friend:y[x.requester_id===d?x.addressee_id:x.requester_id]||{pseudo:"?"}}));if(!p.length){a.innerHTML=`
  ...
  `,document.body.appendChild(i);const n=()=>i.remove();document.getElementById("diff-cancel").addEventListener("click",n),i.addEventListener("click",d=>{d.target===i&&n()}),i.querySelectorAll("[data-mode]").forEach(d=>{d.addEventListener("click",()=>{n(),e("match",{matchMode:d.dataset.mode})})})}const Vt="/",Di={GK:Vt+"cards/card-GK.png",DEF:Vt+"cards/card-DEF.png",MIL:Vt+"cards/card-MIL.png",ATT:Vt+"cards/card-ATT.png"},Wo={GK:"#aaaaaa",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function Jo(e){return!e||e.length<2?null:`https://flagcdn.com/64x48/${e.slice(0,2).toLowerCase()}.png`}function Xo(e){var n;const t="https://fcnwclxlkytdfjotqkta.supabase.co",i=((n=e==null?void 0:e.clubs)==null?void 0:n.logo_url)||(e==null?void 0:e.clubLogo);return i?i.startsWith("http")?i:`${t}/storage/v1/object/public/assets/clubs/${i}`:null}function Gi(e,t,i=0){return t?(Number(e[`note_${t.toLowerCase()}`])||0)+(t===e.job||t===e.job2?i:0):0}function Zo(e,t){return t?t.club_id&&String(e.club_id)===String(t.club_id)||t.country_code&&e.country_code===t.country_code:!1}function Ai(e,t={}){var se;const{width:i=160,showStad:n=!1,stadDef:d=null,used:s=!1,extraNote:r=0}=t;if(!e)return`<div style="width:${i}px;height:${Math.round(i*657/507)}px;border-radius:8px;background:#111;opacity:0.3"></div>`;const l=i/507,m=Math.round(i*657/507),a=de=>Math.round(de*l)+"px",o=de=>Math.round(de*l),y=e._line||e.job||"MIL",p=Wo[y]||"#D4A017",x=Di[y]||Di.MIL,_=e._evolution_bonus??e.evolution_bonus??0,f=n&&(e.stadiumBonus||Zo(e,d))?10:0,c=Gi(e,y,_)+r+f,z=f>0?"#E87722":"#fff",u=e.job2&&e.job2!==y?e.job2:null,I=u?Gi(e,u,_)+r+f:null,F=Jo(e.country_code),C=Xo(e),q=(e.firstname||"").toUpperCase(),j=(e.surname_encoded||e.name||"").toUpperCase(),N=s?"opacity:0.35;":"",X=j.length>10?34:j.length>7?40:46,K=o(30),re=o(170),oe=o(450),Z=o(179),R=o(307),G=o(170),ne=o(450),ge=o(179),_e=o(150),we=o(209),ve=o(450),Ee=o(179);return`<div style="position:relative;width:${i}px;height:${m}px;flex-shrink:0;${N}user-select:none">

  <img src="${x}" style="position:absolute;inset:0;width:100%;height:100%;object-fit:fill" draggable="false">

  <!-- Prénom -->
  <div style="position:absolute;top:${a(38)};left:0;right:0;text-align:center;z-index:2;padding:0 ${a(50)}">
    <span style="font-size:${a(17)};font-weight:700;color:${p};letter-spacing:${a(2)};line-height:1;text-shadow:0 1px 3px #000">${q}</span>
  </div>

  <!-- Nom -->
  <div style="position:absolute;top:${a(68)};left:0;right:0;text-align:center;z-index:2;padding:0 ${a(14)}">
    <span style="font-size:${a(X)};font-weight:900;color:#fff;line-height:1;text-shadow:0 2px 8px #000;font-family:Arial Black,Arial;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;display:block">${j}</span>
  </div>

  <!-- Note principale : centrée dans l'octogone du template -->
  <div style="position:absolute;
    left:${_e}px;top:${ve}px;
    width:${we}px;height:${Ee}px;
    z-index:3;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:0">
    <span style="font-size:${a(I!==null?54:66)};font-weight:900;color:${z};font-family:Arial Black,Arial;line-height:1;text-shadow:0 2px 6px rgba(0,0,0,0.8)">${c}</span>
    ${I!==null?`<span style="font-size:${a(24)};font-weight:900;color:#bb2020;font-family:Arial Black,Arial;line-height:1.1">${I}</span>`:""}
  </div>

  <!-- Drapeau : octogone gauche décalé vers centre -->
  <div style="position:absolute;left:${K}px;top:${oe}px;width:${re}px;height:${Z}px;z-index:3;
    clip-path:polygon(30% 0%,70% 0%,100% 30%,100% 70%,70% 100%,30% 100%,0% 70%,0% 30%);
    background:${p};display:flex;align-items:center;justify-content:center;overflow:hidden">
    ${F?`<img src="${F}" style="width:100%;height:100%;object-fit:cover">`:`<span style="font-size:${a(22)}">🌍</span>`}
  </div>

  <!-- Logo club : octogone droit décalé vers centre -->
  <div style="position:absolute;left:${R}px;top:${ne}px;width:${G}px;height:${ge}px;z-index:3;
    clip-path:polygon(30% 0%,70% 0%,100% 30%,100% 70%,70% 100%,30% 100%,0% 70%,0% 30%);
    background:${p};display:flex;align-items:center;justify-content:center;overflow:hidden">
    ${C?`<img src="${C}" style="width:100%;height:100%;object-fit:contain">`:`<span style="font-size:${a(13)};font-weight:900;color:#fff">${(((se=e.clubs)==null?void 0:se.encoded_name)||e.clubName||"").slice(0,3).toUpperCase()}</span>`}
  </div>

</div>`}const He={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé pour ce match."},"Double attaque":{icon:"⚡",desc:"La note d'attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la note d'un joueur adverse."},Gel:{icon:"❄️",desc:"Bloque le meilleur attaquant IA."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function Te(e,t){if(!e)return 0;switch(t){case"GK":return Number(e.note_g)||0;case"DEF":return Number(e.note_d)||0;case"MIL":return Number(e.note_m)||0;case"ATT":return Number(e.note_a)||0;default:return 0}}const Pi=["ATT","MIL","DEF","GK"];function ao(e){let t=0;const i=e.length;for(let n=0;n<i;n++)for(let d=n+1;d<i;d++){const s=e[n],r=e[d];if(!s||!r)continue;const l=s._col!=null&&r._col!=null&&s._col===r._col,m=s._col!=null&&r._col!=null&&Math.abs(s._col-r._col)===1,a=Pi.indexOf(s._line||s.job),o=Pi.indexOf(r._line||r.job),y=Math.abs(a-o)===1;if(!((s._line||s.job)===(r._line||r.job)&&m||l&&y))continue;const _=s.country_code&&r.country_code&&s.country_code===r.country_code,f=s.club_id&&r.club_id&&s.club_id===r.club_id;_&&f?t+=2:(_||f)&&(t+=1)}return t}function Ht(e,t={}){const i=e.reduce((s,r)=>{const l=r._line||r.job,m=r.stadiumBonus&&(l==="MIL"||l==="ATT")?10:0;return s+(Number(l==="MIL"?r.note_m:r.note_a)||0)+(r.boost||0)+m},0),n=ao(e);let d=i+n;return t.doubleAttack&&(d*=2),t.stolenNote&&(d-=t.stolenNote),{base:i,links:n,total:Math.max(0,d)}}function Ut(e,t={}){const i=e.reduce((s,r)=>{const l=r._line||r.job;let m=0;l==="GK"?m=Number(r.note_g)||0:l==="MIL"?m=Number(r.note_m)||0:m=Number(r.note_d)||0;const a=r.stadiumBonus&&(l==="GK"||l==="DEF"||l==="MIL")?10:0;return s+m+(r.boost||0)+a},0),n=ao(e);let d=i+n;return t.stolenNote&&(d-=t.stolenNote),{base:i,links:n,total:Math.max(0,d)}}function Kt(e,t,i={}){return i.shield?{goal:!1,shielded:!0}:{goal:e>t,shielded:!1}}function so(e,t,i="easy"){const n=e.filter(r=>!r.used);if(!n.length)return[];const d=[...n].sort((r,l)=>{const m=t==="attack"?Te(r,"ATT"):r._line==="GK"?Te(r,"GK"):Te(r,"DEF");return(t==="attack"?Te(l,"ATT"):l._line==="GK"?Te(l,"GK"):Te(l,"DEF"))-m});let s=i==="easy"?1+Math.floor(Math.random()*2):i==="medium"?2+Math.floor(Math.random()*2):3;return d.slice(0,Math.min(s,d.length,3))}function Qo(e,t){const i={vs_ai_easy:{victoire:500,nul:250,defaite:50},vs_ai_medium:{victoire:1e3,nul:500,defaite:50},vs_ai_hard:{victoire:1500,nul:750,defaite:100},vs_ai_club:{victoire:2500,nul:1250,defaite:100}};return(i[e]||i.vs_ai_easy)[t]||0}const lo={pepite:{win:2,loss:-1,boosterStart:"min"},papyte:{win:1,loss:-2,boosterStart:"max"}};function co(e,t){const i=e.player;if(!i)return 0;const n=i.rarity;if(n!=="pepite"&&n!=="papyte")return Number(i[t])||0;const d=Si(i),s=Number(i[t])||0;if(s<=0)return 0;const r=i.note_min??1,l=i.note_max??10,a=(e.current_note??d)-d;return Math.min(l,Math.max(r,s+a))}function Si(e){const t=e.job||"ATT";return Number(t==="GK"?e.note_g:t==="DEF"?e.note_d:t==="MIL"?e.note_m:e.note_a)||0}async function zi(e,t){!e||!t||await Promise.all([xi(e,"win"),xi(t,"loss")])}async function xi(e,t){const{data:i}=await T.from("cards").select("id, current_note, player:players(rarity, job, note_g, note_d, note_m, note_a, note_min, note_max)").eq("owner_id",e).eq("card_type","player");if(!(i!=null&&i.length))return;const n=i.filter(d=>{var s,r;return((s=d.player)==null?void 0:s.rarity)==="pepite"||((r=d.player)==null?void 0:r.rarity)==="papyte"});n.length&&await Promise.all(n.map(d=>{const s=lo[d.player.rarity],r=t==="win"?s.win:s.loss,l=d.player.note_min??1,m=d.player.note_max??10,a=Si(d.player),o=d.current_note??a,y=Math.min(m,Math.max(l,o+r));return T.from("cards").update({current_note:y}).eq("id",d.id)}))}async function en(e,t){return xi(e,t)}const po=Object.freeze(Object.defineProperty({__proto__:null,EVOLUTIVE_RULES:lo,applyOwnEvolution:en,currentSecondaryNote:co,getBaseMainNote:Si,updateEvolutiveCards:zi},Symbol.toStringTag,{value:"Module"})),tn={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"},pi={GK:"#111111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},Yt=["GK","DEF","MIL","ATT"],on=["Tous","GK","DEF","MIL","ATT"],nn={normal:1e3,pepite:5e3,papyte:5e3,legende:1e4};function Ni(e){const t=e.player;if(!t)return null;const i=t.rarity;return(i==="pepite"||i==="papyte")&&e.current_note!=null?e.current_note:Math.max(Number(t.note_g)||0,Number(t.note_d)||0,Number(t.note_m)||0,Number(t.note_a)||0)}function rn(e){return e.length?e.reduce((t,i)=>Ni(i)>Ni(t)?i:t,e[0]):e[0]}function an(e){return e==="GK"?"note_g":e==="DEF"?"note_d":e==="MIL"?"note_m":"note_a"}const sn={MA:"MAROC",FR:"FRANCE",AR:"ARGENTINE",PT:"PORTUGAL",BR:"BRESIL",ES:"ESPAGNE",DE:"ALLEMAGNE",GB:"ANGLETERRE",IT:"ITALIE",CM:"CAMEROUN",SN:"SENEGAL",NG:"NIGERIA",DK:"DANEMARK",NL:"PAYS-BAS",BE:"BELGIQUE",CI:"CÔTE D'IVOIRE",AL:"ALBANIE",HR:"CROATIE",RS:"SERBIE",TR:"TURQUIE"};function dn(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";if(!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function ui(e,t){return e&&Number(t==="GK"?e.note_g:t==="DEF"?e.note_d:t==="MIL"?e.note_m:e.note_a)||0}function fi(e,t=""){const i=e.player;if(!i)return"";const n=e.evolution_bonus||0,d={...i,_evolution_bonus:n},s=t?`<div style="position:absolute;top:6px;right:6px;z-index:10;background:#0a3d1e;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 7px">${t}</div>`:"";return`<div style="position:relative;display:inline-block;cursor:pointer" data-card-id="${e.id}">
    ${s}
    ${Ai(d,{width:140})}
  </div>`}function Ri(e){return`<div style="display:inline-block;filter:grayscale(1);opacity:0.4">
    ${Ai(e,{width:140})}
  </div>`}async function ln(e,t){const{state:i,navigate:n,toast:d,openModal:s,closeModal:r}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:l}=await T.from("cards").select(`id, card_type, current_note, gc_type, formation, is_for_sale, sale_price, stadium_id, evolution_bonus,
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),a=(l||[]).filter(U=>U.card_type==="player"&&U.player),o=(l||[]).filter(U=>U.card_type==="game_changer"),y=(l||[]).filter(U=>U.card_type==="formation"),p=(l||[]).filter(U=>U.card_type==="stadium"),{data:x}=await T.from("gc_definitions").select("name,gc_type,color,effect,image_url"),_={};(x||[]).forEach(U=>{_[U.name]=U});const{data:f}=await T.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),c={};(f||[]).forEach(U=>{c[U.id]=U}),p.forEach(U=>{U.stadium_def&&(c[U.stadium_id]=U.stadium_def)});const z=Object.keys(ki),u=Object.keys(He),I={};a.forEach(U=>{const ee=U.player.id;I[ee]=(I[ee]||0)+1}),new Set(Object.keys(I).map(U=>String(U)));const F=new Set(y.map(U=>U.formation)),C=new Set(o.map(U=>U.gc_type));let q="player",j="Tous",N="",X=!1;function K(){return[...a].sort((U,ee)=>{const xe=Yt.indexOf(U.player.job),le=Yt.indexOf(ee.player.job);return xe!==le?xe-le:(U.player.surname_encoded||"").localeCompare(ee.player.surname_encoded||"")})}function re(){return[...m||[]].sort((U,ee)=>{const xe=Yt.indexOf(U.job),le=Yt.indexOf(ee.job);return xe!==le?xe-le:(U.surname_encoded||"").localeCompare(ee.surname_encoded||"")})}function oe(){return K().filter(U=>{const ee=U.player,xe=j==="Tous"||ee.job===j,le=!N||`${ee.firstname} ${ee.surname_encoded}`.toLowerCase().includes(N);return xe&&le})}function Z(){return re().filter(U=>{const ee=j==="Tous"||U.job===j,xe=!N||`${U.firstname} ${U.surname_encoded}`.toLowerCase().includes(N);return ee&&xe})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",ee=>{N=ee.target.value.toLowerCase(),G()}),e.querySelectorAll(".filter-btn").forEach(ee=>{ee.addEventListener("click",()=>{j=ee.dataset.job,R(),G()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{X=!X,R(),G()})):(U.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{X=!X,R(),G()})))}function G(){const U=document.getElementById("col-grid");U&&(q==="player"?Ee(U):q==="formation"?se(U):q==="stadium"?ue(U):de(U))}function ne(U,ee,xe,le,me){const ie=document.getElementById("col-grid"),pe=document.getElementById("col-big");if(!ie||!pe)return;var Le=0;function Ie(){const Ce=window.innerWidth>=768,Ae=document.getElementById("col-big"),qe=document.getElementById("col-grid");Ce&&Ae&&(Ae.style.minHeight="420px",Ae.style.flex="1 1 auto"),Ce&&qe&&(qe.style.height=Math.round(310*.76+16)+"px",qe.style.flexShrink="0",qe.style.overflowX="auto",qe.style.overflowY="hidden",qe.style.alignItems="center",qe.style.padding="8px 16px"),pe.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+ee(U[Le])+"</div>";var Ge=pe.querySelector("[data-card-id],[data-form-id],[data-gc-id]");Ge&&Ge.addEventListener("click",function(){le(U[Le])}),requestAnimationFrame(function(){var Se=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!Se||!pe)){var B=pe.clientHeight-8,$=pe.clientWidth-24,g=Se.offsetHeight,b=Se.offsetWidth;if(g>0&&b>0&&B>40){var h=Ce?2.2:1,v=Math.min(B/g,$/b,h);Se.style.transform="scale("+v.toFixed(3)+")",Se.style.transformOrigin="top center"}}}),ie.innerHTML=U.map(function(Se,B){var $=B===Le,g="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+($?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+B+'" style="'+g+'">'+xe(Se,$)+"</div>"}).join(""),ie.querySelectorAll(".col-mini-item").forEach(function(Se){Se.addEventListener("click",function(){Le=Number(Se.dataset.idx),Ie(),Se.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}Ie()}function ge(U){var ee=window.innerWidth>=768?.76:.54,xe;if(!U||U._fake){var le=U?U.player:null;if(!le)return"";xe=Ri(le)}else xe=fi(U,"");return'<div style="display:inline-block;zoom:'+ee+';pointer-events:none;line-height:0">'+xe+"</div>"}function _e(U,ee,xe){ee=ee||100,xe=xe||140;var le=Ot[U]||{},me={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},ie=Math.max(3,Math.round(ee*.06)),pe=Object.entries(le).map(function(Ie){var Ce=Ie[0],Ae=Ie[1],qe=Ce.replace(/\d+$/,""),Ge=me[qe]||"#888",Se=Math.round(Ae.x*ee),B=Math.round(Ae.y*xe);return'<circle cx="'+Se+'" cy="'+B+'" r="'+ie+'" fill="'+Ge+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),Le=Math.max(1,Math.round(ee/50));return'<svg viewBox="0 0 '+ee+" "+xe+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+ee+'" height="'+xe+'" fill="#1A6B3C"/><rect x="'+Math.round(ee*.2)+'" y="'+Math.round(xe*.02)+'" width="'+Math.round(ee*.6)+'" height="'+Math.round(xe*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+Le+'"/><line x1="0" y1="'+Math.round(xe*.5)+'" x2="'+ee+'" y2="'+Math.round(xe*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+Le+'"/><ellipse cx="'+Math.round(ee*.5)+'" cy="'+Math.round(xe*.5)+'" rx="'+Math.round(ee*.18)+'" ry="'+Math.round(xe*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+Le+'"/><rect x="'+Math.round(ee*.2)+'" y="'+Math.round(xe*.82)+'" width="'+Math.round(ee*.6)+'" height="'+Math.round(xe*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+Le+'"/>'+pe+"</svg>"}function we(U,ee,xe){var le=xe>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+xe+"</div>":"",me=ee?'data-form-id="'+ee.id+'"':"",ie=U.length>7?14:U.length>5?16:19,pe=!!ee;return"<div "+me+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(pe?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(pe?"":"filter:grayscale(1);opacity:0.5")+'">'+le+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(pe?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+ie+"px;font-weight:900;color:"+(pe?"#1A6B3C":"#aaa")+';line-height:1">'+U+'</div></div><div style="flex:1;overflow:hidden;background:'+(pe?"#1A6B3C":"#ccc")+'">'+_e(U,140,220)+"</div></div>"}function ve(U,ee){var xe=window.innerWidth>=768?.76:.54,le=140,me=305,ie=Math.round(me*.22),pe=me-ie,Le=U.length>7?10:13,Ie=_e(U,le,pe),Ce=ee?"1.5px solid #2a7a40":"1px solid #ddd",Ae=ee?"":"filter:grayscale(1);opacity:0.45;",qe=ee?"#1A6B3C":"#bbb",Ge="#fff";return'<div style="display:inline-block;zoom:'+xe+';line-height:0;pointer-events:none"><div style="width:'+le+"px;height:"+me+"px;border-radius:6px;border:"+Ce+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+Ae+'"><div style="height:'+ie+"px;background:"+qe+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+Le+"px;font-weight:900;color:"+Ge+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(le-4)+'px">'+U+'</span></div><div style="height:'+pe+'px;overflow:hidden;flex-shrink:0">'+Ie+"</div></div></div>"}function Ee(U){if(X){const ee=Z();if(!ee.length){U.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const xe=ee.map(le=>a.find(me=>me.player.id===le.id)||{_fake:!0,player:le,id:"fake-"+le.id});ne(xe,le=>le._fake?Ri(le.player):fi(le,""),le=>le._fake?ge({player:le.player,id:"x",_fake:!0}):ge(le),le=>{le._fake||Oi(le,a,I,t)})}else{const ee=oe();if(!ee.length){U.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const xe={};ee.forEach(me=>{const ie=me.player.id;xe[ie]||(xe[ie]=[]),xe[ie].push(me)});const le=Object.values(xe).map(me=>rn(me));ne(le,me=>{const ie=I[me.player.id]||1,pe=ie>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${ie}</div>`:"",Ie=a.filter(Ce=>Ce.player.id===me.player.id&&Ce.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return fi(me,pe+Ie)},me=>ge(me),me=>Oi(me,a,I,t))}}function se(U){const ee=X?z:[...F];if(!ee.length){U.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const xe=ee.map(le=>({formation:le,card:y.find(me=>me.formation===le)||null,owned:F.has(le)}));ne(xe,({formation:le,card:me,owned:ie})=>we(le,ie?me:null,ie?y.filter(pe=>pe.formation===le).length:0),({formation:le,owned:me})=>ve(le,me),({card:le,owned:me})=>{me&&le&&pn(le,y,t,s)})}function de(U){const ee=Object.keys(_),xe=X?ee.length?ee:u:[...C];if(!xe.length){U.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const le=xe.map(me=>({type:me,gc:He[me]||{icon:"⚡",desc:""},def:_[me]||null,owned:C.has(me),card:o.find(ie=>ie.gc_type===me)||null}));ne(le,({type:me,gc:ie,def:pe,owned:Le,card:Ie})=>{var h;const Ce=Le?o.filter(v=>v.gc_type===me).length:0,Ae=Ce>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${Ce}</div>`:"",qe=(pe==null?void 0:pe.gc_type)==="ultra_game_changer",Ge={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},Se={purple:"#b06ce0",light_blue:"#00d4ef"},B=Ge[pe==null?void 0:pe.color]||Ge.purple,$=Se[pe==null?void 0:pe.color]||Se.purple,g=(pe==null?void 0:pe.effect)||ie.desc||"",b=pe!=null&&pe.image_url?`/icons/${pe.image_url}`:((h=pe==null?void 0:pe.club)==null?void 0:h.logo_url)||(pe!=null&&pe.country_code?`https://flagcdn.com/w80/${pe.country_code.toLowerCase()}.png`:null);return Le&&Ie?`<div data-gc-id="${Ie.id}" data-gc-type="${me}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${$};background:${B};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${$}66;cursor:pointer">
          ${Ae}
        ...
        </div>`},({type:me,gc:ie,def:pe,owned:Le})=>{const Ie=window.innerWidth>=768?.76:.54,Ce={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},Ae={purple:"#9b59b6",light_blue:"#00bcd4"},qe=Ce[pe==null?void 0:pe.color]||Ce.purple,Ge=Ae[pe==null?void 0:pe.color]||Ae.purple,Se=pe!=null&&pe.image_url?`/icons/${pe.image_url}`:null;return Le?`<div style="display:inline-block;zoom:${Ie};line-height:0;pointer-events:none"><div style="width:140px;height:310px;border-radius:8px;background:${qe};border:1px solid ${Ge};display:flex;flex-direction:column;overflow:hidden"><div style="height:56px;background:rgba(255,255,255,0.15);display:flex;align-items:center;justify-content:center"><span style="font-size:13px;font-weight:900;color:#fff;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:130px">${me}</span></div><div style="flex:1;display:flex;align-items:center;justify-content:center">${Se?`<img src="${Se}" style="max-width:130px;max-height:190px;object-fit:contain">`:`<span style="font-size:48px">${ie.icon}</span>`}</div><div style="height:54px;display:flex;align-items:center;justify-content:center;padding:0 4px"><span style="font-size:10px;color:rgba(255,255,255,0.7);text-align:center">${((pe==null?void 0:pe.effect)||ie.desc||"").slice(0,30)}</span></div></div></div>`:`<div style="display:inline-block;zoom:${Ie};line-height:0;pointer-events:none"><div style="width:140px;height:310px;border-radius:8px;background:#eee;border:1px solid #ddd;display:flex;flex-direction:column;align-items:center;justify-content:center;filter:grayscale(1);opacity:0.45"><span style="font-size:36px">${ie.icon}</span><span style="font-size:12px;color:#aaa;margin-top:6px;text-align:center;padding:0 6px">${me}</span></div></div>`},({type:me,owned:ie,def:pe})=>{ie&&cn(me,pe,s)})}function ue(U){const ee="#E87722",xe="/";if(!p.length){U.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Stade.<br><small>Ouvre un booster Stade !</small></div>';return}const le={};p.forEach(ie=>{const pe=ie.stadium_id||"?";(le[pe]=le[pe]||[]).push(ie)});const me=Object.entries(le).map(([ie,pe])=>({sid:ie,def:c[ie]||null,count:pe.length,card:pe[0]}));ne(me,({def:ie,count:pe})=>{var Ge,Se;const Le=(ie==null?void 0:ie.name)||"?",Ie=((Ge=ie==null?void 0:ie.club)==null?void 0:Ge.encoded_name)||(ie==null?void 0:ie.country_code)||"—",Ce=ie!=null&&ie.image_url?`${xe}icons/${ie.image_url}`:((Se=ie==null?void 0:ie.club)==null?void 0:Se.logo_url)||(ie!=null&&ie.country_code?`https://flagcdn.com/w80/${ie.country_code.toLowerCase()}.png`:null),Ae=Ce?`<img src="${Ce}" style="width:90px;height:90px;object-fit:contain;border-radius:4px" onerror="this.style.display='none'">`:'<div style="font-size:56px">🏟️</div>',qe=pe>1?`<div style="position:absolute;top:8px;right:8px;background:#333;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${pe}</div>`:"";return`<div style="position:relative;width:140px;border-radius:12px;border:3px solid #c45a00;background:linear-gradient(160deg,${ee},#c45a00);display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${ee}66">
          ${qe}
        ...
        </div>`},({def:ie,count:pe})=>{var Ge,Se;const Le=window.innerWidth>=768?.76:.54,Ie=(ie==null?void 0:ie.name)||"?",Ce=((Ge=ie==null?void 0:ie.club)==null?void 0:Ge.encoded_name)||(ie==null?void 0:ie.country_code)||"—",Ae=ie!=null&&ie.image_url?`${xe}icons/${ie.image_url}`:((Se=ie==null?void 0:ie.club)==null?void 0:Se.logo_url)||(ie!=null&&ie.country_code?`https://flagcdn.com/w80/${ie.country_code.toLowerCase()}.png`:null),qe=Ae?`<img src="${Ae}" style="width:64px;height:64px;object-fit:contain;border-radius:4px" onerror="this.style.display='none'">`:'<span style="font-size:32px">🏟️</span>';return`<div style="display:inline-block;zoom:${Le};line-height:0;pointer-events:none">
        ...
        </div>`},null)}e.querySelectorAll(".col-tab-btn").forEach(U=>{U.addEventListener("click",()=>{q=U.dataset.tab,j="Tous",N="",X=!1,e.querySelectorAll(".col-tab-btn").forEach(ee=>{const xe=ee.dataset.tab===q;ee.style.borderBottomColor=xe?"var(--green)":"transparent",ee.style.color=xe?"var(--green)":"var(--gray-600)"}),R(),G()})}),R(),G()}function cn(e,t,i){const n=He[e]||{icon:"⚡",desc:"Effet spécial."},d=(t==null?void 0:t.gc_type)==="ultra_game_changer",s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},r={purple:"#b06ce0",light_blue:"#00d4ef"},l=s[t==null?void 0:t.color]||s.purple,m=r[t==null?void 0:t.color]||r.purple,a=(t==null?void 0:t.name)||e,o=(t==null?void 0:t.effect)||n.desc,y=t!=null&&t.image_url?`/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Wt=1e3;function pn(e,t,i,n){var _,f,c;const{state:d,toast:s,closeModal:r,navigate:l,refreshProfile:m}=i,a=e.formation,o={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function y(){const z=Ot[a]||{},u=ki[a]||[],I=290,F=360,C=20;function q(N){const X=z[N];return X?{x:X.x*I,y:X.y*F}:null}let j=`<svg width="${I}" height="${F}" viewBox="0 0 ${I} ${F}" xmlns="http://www.w3.org/2000/svg">`;for(const[N,X]of u){const K=q(N),re=q(X);!K||!re||(j+=`<line x1="${K.x}" y1="${K.y}" x2="${re.x}" y2="${re.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const N of Object.keys(z)){const X=q(N);if(!X)continue;const K=N.replace(/\d+/,""),re=o[K]||"#555";j+=`<circle cx="${X.x}" cy="${X.y}" r="${C}" fill="${re}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,j+=`<text x="${X.x}" y="${X.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${K}</text>`}return j+="</svg>",j}const p=t.filter(z=>z.formation===a);p.length;const x=!e.is_for_sale;n(`Formation ${a}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(_=document.getElementById("direct-sell-form-btn"))==null||_.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${a} pour ${Wt.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const z=p.find(I=>!I.is_for_sale)||p[0];if(!z){s("Aucune carte à vendre","error");return}await T.from("market_listings").delete().eq("card_id",z.id),await T.from("transfer_history").delete().eq("card_id",z.id);const{error:u}=await T.from("cards").delete().eq("id",z.id);if(u){s(u.message,"error");return}await T.from("users").update({credits:(d.profile.credits||0)+Wt}).eq("id",d.profile.id),await m(),s(`+${Wt.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),r(),l("collection")}),(f=document.getElementById("market-sell-form-btn"))==null||f.addEventListener("click",async()=>{const z=parseInt(document.getElementById("sell-price-form").value);if(!z||z<1){s("Prix invalide","error");return}await T.from("cards").update({is_for_sale:!0,sale_price:z}).eq("id",e.id),await T.from("market_listings").insert({seller_id:d.profile.id,card_id:e.id,price:z}),s("Carte mise en vente sur le marché !","success"),r(),l("collection")}),(c=document.getElementById("cancel-sell-form-btn"))==null||c.addEventListener("click",async()=>{await T.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await T.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),s("Annonce retirée","success"),r(),l("collection")})}async function Oi(e,t,i,n){var ge,_e,we,ve,Ee;const{state:d,toast:s,openModal:r,closeModal:l,navigate:m,refreshProfile:a}=n,o=e.player,y=t.filter(se=>se.player.id===o.id),p=y.length,x=e.evolution_bonus||0,_=Math.max((Number(o.note_g)||0)+(o.job==="GK"||o.job2==="GK"?x:0),(Number(o.note_d)||0)+(o.job==="DEF"||o.job2==="DEF"?x:0),(Number(o.note_m)||0)+(o.job==="MIL"||o.job2==="MIL"?x:0),(Number(o.note_a)||0)+(o.job==="ATT"||o.job2==="ATT"?x:0)),f=o.rarity||"normal",{data:c}=await T.from("sell_price_configs").select("*").eq("rarity",f).lte("note_min",_).gte("note_max",_).order("note_min",{ascending:!1}).limit(1);((ge=c==null?void 0:c[0])==null?void 0:ge.price)??nn[f];const z=o.rarity!=="legende";dn(o);const u=((o.rarity==="pepite"||o.rarity==="papyte")&&e.current_note!=null?e.current_note:ui(o,o.job))+x,I=o.rarity==="pepite"||o.rarity==="papyte",F=o.job2?(I?co(e,an(o.job2)):ui(o,o.job2))+(ui(o,o.job2)>0?x:0):null;pi[o.job],o.job2&&pi[o.job2];const C=tn[o.rarity]||"#ccc";sn[o.country_code]||o.country_code;const q=e.evolution_bonus||0,N=u+q,X=F||0,K=X>0?X+q:0,re=y.map(se=>se.id);let oe={};if(re.length){const{data:se}=await T.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",re).order("transferred_at",{ascending:!0});(se||[]).forEach(de=>{oe[de.card_id]||(oe[de.card_id]=[]),oe[de.card_id].push(de)})}const Z=se=>{const de=se.transferred_at?new Date(se.transferred_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"2-digit",year:"numeric"}):"",ue=se.source==="booster"?"Booster":se.price?se.price.toLocaleString("fr"):"—";return`
    <div style="display:flex;justify-content:space-between;align-items:flex-start;background:#f9f9f9;border-radius:8px;padding:8px 12px;gap:8px">
      ...
      <div style="text-align:right;flex-shrink:0">
        ...
        <div style="font-size:11px;color:var(--gray-600)">${de}</div>
      </div>
    </div>`},R=re.length?`
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      <div style="font-size:13px;font-weight:700;margin-bottom:10px">🏟️ Mes exemplaires ${p>1?`(${p})`:""}</div>
      <div style="display:flex;flex-direction:column;gap:10px">
        ${y.map((se,de)=>{const ue=oe[se.id]||[],U=se.is_for_sale,ee=ue.length?ue[ue.length-1]:null,le=(o.rarity==="pepite"||o.rarity==="papyte")&&se.current_note!=null?` (☆${se.current_note})`:"";return`
            <div data-card-id="${se.id}" data-card-idx="${de}" class="exemplaire-row"
              ...
              <div style="display:flex;align-items:center;gap:10px">
                ...
                <div style="flex:1;min-width:0">
                  <div style="font-size:12px;font-weight:700">Exemplaire ${de+1}${le}${U?" 🏷️ En vente":""}</div>
                ...
                <button class="expl-hist-toggle" data-idx="${de}" style="font-size:11px;color:#999;border:none;background:none;cursor:pointer;padding:2px 6px;flex-shrink:0">
                  ${ue.length?`${ue.length} club${ue.length>1?"s":""}`:""} ▾
                </button>
              </div>
              <div class="expl-hist" data-hist="${de}" style="display:none;margin-top:8px;display:flex;flex-direction:column;gap:4px">
                ${ue.map(Z).join("")}
              </div>
            </div>`}).join("")}
      </div>
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",o.note_g],["DEF",o.note_d],["MIL",o.note_m],["ATT",o.note_a]].map(([se,de])=>{const ue=pi[se],U=se===o.job||se===o.job2,ee=(Number(de)||0)+(U&&q>0?q:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:2px">
    ...
    ${R}
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(_e=document.getElementById("close-detail"))==null||_e.addEventListener("click",l);let G=new Set;const ne=()=>{const se=G.size,de=document.getElementById("sell-action-panel");if(!de)return;de.style.display=se>0?"block":"none",document.getElementById("sell-selected-count").textContent=se;const ue=document.getElementById("evolve-btn");ue&&(ue.textContent=`⬆️ Évoluer ${se>1?"(+"+se+")":""}`)};document.querySelectorAll(".expl-check").forEach(se=>{se.addEventListener("change",()=>{const de=se.dataset.id;se.checked?G.add(de):G.delete(de);const ue=se.closest(".exemplaire-row");ue&&(ue.style.borderColor=se.checked?"#1A6B3C":"#eee"),ne()})}),document.querySelectorAll(".exemplaire-row").forEach(se=>{se.addEventListener("click",de=>{if(de.target.closest("button")||de.target.tagName==="INPUT")return;const ue=se.querySelector(".expl-check");ue&&!ue.disabled&&(ue.checked=!ue.checked,ue.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(se=>{se.addEventListener("click",de=>{de.stopPropagation();const ue=document.querySelector(`.expl-hist[data-hist="${se.dataset.idx}"]`);ue&&(ue.style.display=ue.style.display==="none"?"flex":"none")})}),(we=document.getElementById("evolve-btn"))==null||we.addEventListener("click",async()=>{if(p<=1)return;const se=[...G];if(!se.length)return;if(G.has(e.id)){const me=document.createElement("div");me.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",me.innerHTML=`
        ...
        <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center;box-shadow:0 16px 48px rgba(0,0,0,0.3)">
          ...
          <div style="background:#f0fdf4;border-radius:10px;padding:12px;margin-bottom:16px;font-size:13px;color:#333">
            🗑️ <strong>${de.length}</strong> copie${de.length>1?"s":""} sacrifiée${de.length>1?"s":""}<br>
            📈 Note : <strong>${u+(e.evolution_bonus||0)}</strong> → <strong style="color:#1A6B3C">${u+(e.evolution_bonus||0)+de.length*ue}</strong>
            ${F&&F>0?`<br>📈 Note 2 : <strong>${F+(e.evolution_bonus||0)}</strong> → <strong style="color:#1A6B3C">${F+(e.evolution_bonus||0)+de.length*ue}</strong>`:""}
  ...
  </div>`,fn(e,t,d,i),document.getElementById("builder-back").addEventListener("click",()=>n("decks")),document.getElementById("formation-select").addEventListener("change",x=>{t.formation=x.target.value;const _=Ui(t.formation),f={};_.forEach(c=>{t.slots[c]&&(f[c]=t.slots[c])}),t.slots=f,kt(e,t,i)}),(y=document.getElementById("stadium-select"))==null||y.addEventListener("change",x=>{t.stadiumCardId=x.target.value||null,kt(e,t,i)}),document.getElementById("save-deck").addEventListener("click",()=>xn(t,i)),e.querySelectorAll("[data-remove-sub]").forEach(x=>{x.addEventListener("click",()=>{t.subs=t.subs.filter(_=>_!==x.dataset.removeSub),kt(e,t,i)})}),(p=document.getElementById("add-sub-btn"))==null||p.addEventListener("click",()=>{mn(t,e,i)})}function fn(e,t,i,n){var I,F;const d=e.querySelector("#deck-field");if(!d)return;const s=(I=t.stadiumCards)==null?void 0:I.find(C=>C.id===t.stadiumCardId),r=s&&((F=t.stadDefMap)==null?void 0:F[s.stadium_id])||null,l=Ot[t.formation]||{},m=eo(t.formation)||[],a={};for(const C of i){const q=t.slots[C],j=q?t.playerCards.find(N=>N.id===q):null;j!=null&&j.player?a[C]={...j.player,_evolution_bonus:j.evolution_bonus||0}:a[C]=null}const o=300,y=300,p=48,x=64,_=13,f=16,c=38;function z(C){const q=l[C];return q?{x:q.x*o,y:q.y*y}:null}let u="";for(const[C,q]of m){const j=z(C),N=z(q);if(!j||!N)continue;const X=a[C]?{...a[C],club_id:a[C].club_id,country_code:a[C].country_code,rarity:a[C].rarity}:null,K=a[q]?{...a[q],club_id:a[q].club_id,country_code:a[q].country_code,rarity:a[q].rarity}:null,re=nt(X,K);re==="#ff3333"||re==="#cc2222"?u+=`<line x1="${j.x.toFixed(1)}" y1="${j.y.toFixed(1)}" x2="${N.x.toFixed(1)}" y2="${N.y.toFixed(1)}" stroke="${re}" stroke-width="2.5" stroke-linecap="round" opacity="0.4"/>`:(u+=`<line x1="${j.x.toFixed(1)}" y1="${j.y.toFixed(1)}" x2="${N.x.toFixed(1)}" y2="${N.y.toFixed(1)}" stroke="${re}" stroke-width="8" stroke-linecap="round" opacity="0.2"/>`,u+=`<line x1="${j.x.toFixed(1)}" y1="${j.y.toFixed(1)}" x2="${N.x.toFixed(1)}" y2="${N.y.toFixed(1)}" stroke="${re}" stroke-width="2.5" stroke-linecap="round" opacity="0.9"/>`)}for(const C of i){const q=z(C);if(!q)continue;const j=a[C],N=C.replace(/\d+/,""),X=$t[N]||"#555",K=(q.x-p/2).toFixed(1),re=(q.y-x/2).toFixed(1),oe={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[j==null?void 0:j.rarity]||"#aaa";if(j){const Z=ri(j),R=fo(j),G=uo(j.country_code),ne=j._evolution_bonus||0,ge=(Number(N==="GK"?j.note_g:N==="DEF"?j.note_d:N==="MIL"?j.note_m:j.note_a)||0)+(N===j.job||N===j.job2?ne:0),_e=r&&(r.club_id&&String(j.club_id)===String(r.club_id)||r.country_code&&j.country_code===r.country_code),we=_e?ge+10:ge,ve=x-_-f;u+=`<defs><clipPath id="dcp-${C}"><rect x="${K}" y="${(q.y-x/2+_).toFixed(1)}" width="${p}" height="${ve}"/></clipPath></defs>`,u+=`<rect x="${K}" y="${re}" width="${p}" height="${x}" rx="5" fill="${X}"/>`,Z&&(u+=`<image href="${Z}" x="${K}" y="${(q.y-x/2+_).toFixed(1)}" width="${p}" height="${ve}" clip-path="url(#dcp-${C})" preserveAspectRatio="xMidYMin slice"/>`),u+=`<rect x="${K}" y="${re}" width="${p}" height="${_}" fill="rgba(255,255,255,0.93)"/>`,u+=`<text x="${q.x.toFixed(1)}" y="${(q.y-x/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(j.surname_encoded||"").slice(0,9)}</text>`;const Ee=(q.y+x/2-f).toFixed(1);u+=`<rect x="${K}" y="${Ee}" width="${p}" height="${f}" fill="rgba(255,255,255,0.93)"/>`,G&&(u+=`<image href="${G}" x="${(q.x-21).toFixed(1)}" y="${(q.y+x/2-f+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`),u+=`<text x="${q.x.toFixed(1)}" y="${(q.y+x/2-f/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="${_e?"#E87722":"#111"}" font-family="Arial Black">${we}</text>`,R&&(u+=`<image href="${R}" x="${(q.x+p/2-14).toFixed(1)}" y="${(q.y+x/2-f+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`),u+=`<rect x="${K}" y="${re}" width="${p}" height="${x}" rx="5" fill="none" stroke="${oe}" stroke-width="2"/>`}else u+=`<rect x="${K}" y="${re}" width="${p}" height="${x}" rx="5" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="5,3"/>`,u+=`<text x="${q.x.toFixed(1)}" y="${q.y.toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="22" fill="rgba(255,255,255,0.35)">+</text>`,u+=`<text x="${q.x.toFixed(1)}" y="${(q.y+16).toFixed(1)}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.3)">${N}</text>`;u+=`<rect x="${K}" y="${re}" width="${p}" height="${x}" rx="5" fill="rgba(0,0,0,0.01)" class="deck-slot-hit" data-pos="${C}" style="cursor:pointer"/>`}d.innerHTML=`<svg viewBox="${-c} ${-c} ${o+c*2} ${y+c*2}" width="100%" style="display:block;max-width:440px;margin:0 auto">${u}</svg>`,d.querySelectorAll(".deck-slot-hit").forEach(C=>{C.addEventListener("click",()=>gn(C.dataset.pos,t,e,n))})}function gn(e,t,i,n){var p,x,_;const{openModal:d,closeModal:s}=n,r=e.replace(/\d+/,""),l=t.slots[e],m=l?t.playerCards.find(f=>f.id===l):null;(p=m==null?void 0:m.player)==null||p.id;const a=new Set;Object.entries(t.slots).forEach(([f,c])=>{var u;if(f===e||!c)return;const z=t.playerCards.find(I=>I.id===c);(u=z==null?void 0:z.player)!=null&&u.id&&a.add(z.player.id)}),t.subs.forEach(f=>{var z;const c=t.playerCards.find(u=>u.id===f);(z=c==null?void 0:c.player)!=null&&z.id&&a.add(c.player.id)});const o=new Set,y=t.playerCards.filter(f=>{const c=f.player;return!(c.job===r||c.job2===r)||a.has(c.id)||o.has(c.id)?!1:(o.add(c.id),!0)});y.sort((f,c)=>{const z=f.evolution_bonus||0,u=c.evolution_bonus||0,I=(r==="GK"?f.player.note_g:r==="DEF"?f.player.note_d:r==="MIL"?f.player.note_m:f.player.note_a)+(r===f.player.job||r===f.player.job2?z:0);return(r==="GK"?c.player.note_g:r==="DEF"?c.player.note_d:r==="MIL"?c.player.note_m:c.player.note_a)+(r===c.player.job||r===c.player.job2?u:0)-I}),d(`Choisir ${r} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${y.length>0?y.map(f=>{var C,q;const c=f.player,z=f.evolution_bonus||0,u=(r==="GK"?c.note_g:r==="DEF"?c.note_d:r==="MIL"?c.note_m:c.note_a)+(r===c.job||r===c.job2?z:0),I=ri(c),F={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[c.rarity];return`<div class="player-option" data-card-id="${f.id}"
  ...
  </div>`}function xo(e){var d;const t={};(e.rates||[]).forEach(s=>{t[s.card_type]=(t[s.card_type]||0)+Number(s.percentage||0)});const i=((d=Object.entries(t).sort((s,r)=>r[1]-s[1])[0])==null?void 0:d[0])||"player",n=e.image_url||"booster-players.png";return{id:e.id,img:"/icons/"+n,name:e.name,sub:`${e.card_count} carte(s)`,cost:e.price_type==="credits"&&e.price_credits||0,costLabel:e.price_type==="credits"?`${(e.price_credits||0).toLocaleString("fr")} crédits`:e.price_type==="pub"?"1 pub":"Gratuit",cardCount:e.card_count||5,type:i,isPub:e.price_type==="pub",rates:e.rates||[],allow_duplicates:e.allow_duplicates!==!1,_boosterId:e.id,_raw:e}}async function kn(e,{state:t,navigate:i,toast:n}){var y,p,x;const d=((y=t.profile)==null?void 0:y.credits)||0;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⏳ Chargement...</div>';let s=[];try{s=(await go((p=t.user)==null?void 0:p.id)).map(xo)}catch(_){console.warn("Erreur chargement boosters DB, fallback hardcodé",_)}s.length||(s=hn.map(_=>({..._,rates:[],isPub:_.id==="players_pub"})));const r=await T.from("cards").select("player_id, card_type, formation, stadium_id, gc_type").eq("owner_id",t.profile.id).then(_=>_.data||[]),l=new Set(r.filter(_=>_.card_type==="stadium").map(_=>_.stadium_id)),m=new Set(r.filter(_=>_.card_type==="formation").map(_=>_.formation)),a=new Set(r.filter(_=>_.card_type==="game_changer").map(_=>_.gc_type)),o={};for(const _ of s){if(_.allow_duplicates!==!1||!((x=_.rates)!=null&&x.length))continue;const f=[...new Set((_.rates||[]).map(z=>z.card_type))];let c=!1;for(const z of f)if(z==="stadium"){const{data:u}=await T.from("stadium_definitions").select("id");if((u||[]).some(I=>!l.has(I.id))){c=!0;break}}else if(z==="game_changer"){const{data:u}=await T.from("gc_definitions").select("name").eq("is_active",!0);if((u||[]).some(I=>!a.has(I.name))){c=!0;break}}else if(z==="formation"){const{FORMATION_LINKS:u}=await ni(async()=>{const{FORMATION_LINKS:I}=await import("./formation-links-BZe0pGr4.js").then(F=>F.p);return{FORMATION_LINKS:I}},__vite__mapDeps([0,1]));if(Object.keys(u).some(I=>!m.has(I))){c=!0;break}}else{c=!0;break}c||(o[_.id]=!0)}e.innerHTML=`
  <div class="page">
    <div class="page-header">
      <h2>📦 Boosters</h2>
  ...
  `,document.body.appendChild(d);let s=!1;const r=document.getElementById("pack-cut-zone"),l=document.getElementById("pack-blade");let m=!1;const a=R=>R.touches&&R.touches[0]?R.touches[0].clientX:R.clientX;function o(R){s||(m=!0,l.style.opacity="1",y(R))}function y(R){if(!m||s)return;const G=r.getBoundingClientRect(),ne=a(R)-G.left,ge=Math.max(0,Math.min(1,ne/G.width));l.style.width=ge*G.width+"px",ge>=.82&&x()}function p(){s||(m=!1,l.style.transition="width .2s ease, opacity .2s ease",l.style.width="0",l.style.opacity="0",setTimeout(()=>{s||(l.style.transition="")},220))}function x(){var G;if(s)return;s=!0,m=!1,l.style.width="100%",l.style.opacity="1",(G=document.getElementById("cut-flash"))==null||G.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const R=document.getElementById("cut-hint");R&&(R.style.opacity="0"),r.classList.add("pack-cut"),setTimeout(()=>{l.style.opacity="0",document.getElementById("pack-phase").style.display="none",z(0)},620)}r.addEventListener("pointerdown",o),window.addEventListener("pointermove",y),window.addEventListener("pointerup",p),r.addEventListener("touchstart",o,{passive:!0}),window.addEventListener("touchmove",y,{passive:!0}),window.addEventListener("touchend",p);let _=0,f=!1;const c=new Set;function z(R){_=R,document.getElementById("reveal-phase").style.display="flex",u(),I(R,0),N()}function u(){const R=document.getElementById("card-dots");R&&(R.innerHTML=e.map((G,ne)=>`<div class="card-dot" data-i="${ne}" style="width:8px;height:8px;border-radius:50%;background:${ne===_?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),R.querySelectorAll(".card-dot").forEach(G=>G.addEventListener("click",()=>C(parseInt(G.dataset.i)))))}function I(R,G){var ue;const ne=e[R],ge=document.getElementById("card-counter"),_e=document.getElementById("card-track");ge&&(ge.textContent=`Carte ${R+1} / ${e.length}`);const we=document.getElementById("reveal-btns");we&&(we.style.display=R===e.length-1?"flex":"none");const ve=ne.card_type==="player"&&((ue=ne.player)==null?void 0:ue.rarity)==="legende",Ee=!c.has(R);c.add(R);let se=0;if(ne.card_type==="player"&&ne.player){const U=ne.player,ee=U.job||"ATT";se=Number(ee==="GK"?U.note_g:ee==="DEF"?U.note_d:ee==="MIL"?U.note_m:U.note_a)||0}const de=U=>{_e.innerHTML=`
        <div id="current-card-wrap" style="position:relative;display:flex;flex-direction:column;align-items:center;gap:8px;${ve?"filter:drop-shadow(0 0 20px #7a28b8)":""}">
          <div style="transform:scale(1.25);transform-origin:center">${Sn(ne)}</div>
        ...
        </div>`;const ee=document.getElementById("current-card-wrap");G!==0?(ee.style.transition="none",ee.style.transform=`translateX(${G>0?100:-100}%)`,requestAnimationFrame(()=>{ee.style.transition="transform .28s cubic-bezier(.25,1,.5,1)",ee.style.transform="translateX(0)"})):ee.animate([{opacity:0,transform:"scale(.7)"},{opacity:1,transform:"scale(1)"}],{duration:300,easing:"cubic-bezier(.34,1.56,.64,1)"}),U||ve?K():re(),u()};Ee&&se>6&&ne.card_type==="player"&&ne.player?F(ne,()=>de(!0)):de(!1)}function F(R,G){var se;f=!0;const ne=R.player,ge=`https://flagsapi.com/${ne.country_code}/flat/64.png`,_e=(se=ne.clubs)==null?void 0:se.logo_url,we=document.getElementById("walkout-overlay"),ve=document.getElementById("walkout-stage");if(!we||!ve){f=!1,G();return}we.style.display="flex";const Ee=()=>{const de=ve.firstElementChild;de&&(de.classList.remove("wo-in"),de.classList.add("wo-out"))};ve.innerHTML=`<img class="wo-in" src="${ge}" style="height:130px;border-radius:10px;box-shadow:0 10px 36px rgba(0,0,0,.6)" onerror="this.style.display='none'">`,navigator.vibrate&&navigator.vibrate(30),setTimeout(Ee,2e3),setTimeout(()=>{var de;ve.innerHTML=_e?`<img class="wo-in" src="${_e}" style="max-height:160px;max-width:210px;object-fit:contain">`:`<div class="wo-in" style="font-size:34px;font-weight:900;color:#fff;text-align:center">${((de=ne.clubs)==null?void 0:de.encoded_name)||"CLUB"}</div>`,navigator.vibrate&&navigator.vibrate(30)},2450),setTimeout(Ee,4450),setTimeout(()=>{we.style.display="none",ve.innerHTML="",f=!1,navigator.vibrate&&navigator.vibrate([40,30,80]),G()},4900)}function C(R){if(f||R<0||R>=e.length||R===_)return;const G=R>_?1:-1;_=R,I(R,G)}function q(){C(_+1)}function j(){C(_-1)}function N(){const R=document.getElementById("card-viewport");if(!R||R._swipeBound)return;R._swipeBound=!0;let G=0,ne=0,ge=0,_e=!1;const we=ue=>ue.touches?ue.touches[0].clientX:ue.clientX,ve=ue=>ue.touches?ue.touches[0].clientY:ue.clientY,Ee=ue=>{_e=!0,G=we(ue),ne=ve(ue),ge=0},se=ue=>{if(!_e)return;ge=we(ue)-G;const U=ve(ue)-ne;if(Math.abs(ge)<Math.abs(U))return;const ee=document.getElementById("current-card-wrap");ee&&(ee.style.transition="none",ee.style.transform=`translateX(${ge*.6}px) rotate(${ge*.02}deg)`)},de=()=>{if(!_e)return;_e=!1;const ue=document.getElementById("current-card-wrap"),U=55;ge<=-U&&_<e.length-1?q():ge>=U&&_>0?j():ue&&(ue.style.transition="transform .2s ease",ue.style.transform="translateX(0)")};R.addEventListener("pointerdown",Ee),R.addEventListener("pointermove",se),R.addEventListener("pointerup",de),R.addEventListener("pointercancel",de),R.addEventListener("touchstart",Ee,{passive:!0}),R.addEventListener("touchmove",se,{passive:!0}),R.addEventListener("touchend",de),R.addEventListener("click",ue=>{if(Math.abs(ge)>8)return;const U=R.getBoundingClientRect();ue.clientX-U.left>U.width/2?q():j()})}let X=null;function K(){const R=document.getElementById("fireworks-canvas");if(!R)return;R.width=window.innerWidth,R.height=window.innerHeight;const G=R.getContext("2d"),ne=[];function ge(){const we=Math.random()*R.width,ve=Math.random()*R.height*.6,Ee=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],se=Ee[Math.floor(Math.random()*Ee.length)];for(let de=0;de<60;de++){const ue=Math.PI*2/60*de,U=2+Math.random()*5;ne.push({x:we,y:ve,vx:Math.cos(ue)*U,vy:Math.sin(ue)*U,alpha:1,color:se,size:2+Math.random()*3})}}ge(),X=setInterval(ge,600);function _e(){if(document.getElementById("fireworks-canvas")){G.clearRect(0,0,R.width,R.height);for(let we=ne.length-1;we>=0;we--){const ve=ne[we];if(ve.x+=ve.vx,ve.y+=ve.vy+.08,ve.vy*=.98,ve.alpha-=.018,ve.alpha<=0){ne.splice(we,1);continue}G.globalAlpha=ve.alpha,G.fillStyle=ve.color,G.beginPath(),G.arc(ve.x,ve.y,ve.size,0,Math.PI*2),G.fill()}G.globalAlpha=1,(X!==null||ne.length>0)&&requestAnimationFrame(_e)}}_e()}function re(){X!==null&&(clearInterval(X),X=null);const R=document.getElementById("fireworks-canvas");R&&R.getContext("2d").clearRect(0,0,R.width,R.height)}if(n){const R=document.getElementById("reveal-btns");R&&(R.innerHTML='<button class="btn btn-primary" id="reveal-next" style="flex:1">Continuer →</button>'),(Z=document.getElementById("reveal-next"))==null||Z.addEventListener("click",()=>{re(),d.remove(),n()})}else document.getElementById("reveal-collection").addEventListener("click",()=>{re(),d.remove(),i("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{re(),d.remove(),i("boosters")})}function Sn(e){var t,i,n,d;if(e.card_type==="player"&&e.player)return $n(e);if(e.card_type==="game_changer"){const s=e._gcDef,r=(s==null?void 0:s.gc_type)==="ultra_game_changer",l={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},m={purple:"#b06ce0",light_blue:"#00d4ef"},a=l[s==null?void 0:s.color]||l.purple,o=m[s==null?void 0:s.color]||m.purple,y=(s==null?void 0:s.name)||e.gc_type||"Game Changer",p=(s==null?void 0:s.effect)||((t=bi[e.gc_type])==null?void 0:t.desc)||"",x=s!=null&&s.image_url?`/icons/${s.image_url}`:null,_=((i=bi[e.gc_type])==null?void 0:i.icon)||"⚡";return`<div style="width:170px;height:240px;background:${a};border-radius:14px;border:3px solid ${o};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${o}88;flex-shrink:0">
      ...
      </div>`,document.body.appendChild(n),n.addEventListener("click",r=>{r.target===n&&n.remove()}),(i=document.getElementById("odds-close"))==null||i.addEventListener("click",()=>n.remove());return}Ln()}function Ln(){const e=document.createElement("div");e.style.cssText=`position:fixed;inset:0;background:rgba(0,0,0,0.6);display:flex;
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}function In(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText=`
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      card:cards(id,card_type,formation,stadium_id,evolution_bonus,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",r).order("slot_order"),m=[...new Set((l||[]).filter(p=>{var x,_;return((x=p.card)==null?void 0:x.card_type)==="stadium"&&((_=p.card)==null?void 0:_.stadium_id)}).map(p=>p.card.stadium_id))],a={};if(m.length){const{data:p}=await T.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",m);(p||[]).forEach(x=>{a[x.id]=x}),(l||[]).forEach(x=>{var _,f;((_=x.card)==null?void 0:_.card_type)==="stadium"&&((f=x.card)!=null&&f.stadium_id)&&(x.card._stadiumDef=a[x.card.stadium_id]||null)})}let o=0;function y(){var C,q,j,N,X,K,re;const p=s[o],x=(l||[]).filter(oe=>oe.deck_id===p.id),_=x.filter(oe=>{var Z;return oe.is_starter&&((Z=oe.card)==null?void 0:Z.player)}).map(oe=>vi(oe.card,oe.position)),f=x.find(oe=>{var Z;return((Z=oe.card)==null?void 0:Z.card_type)==="formation"}),c=p.formation||((C=f==null?void 0:f.card)==null?void 0:C.formation)||"4-4-2";let z=_.length>=11?bt(_,c):null,u=((q=p.stadium_card)==null?void 0:q.stadium_def)||null;u&&z&&(z=Tt(z,u));const I=_.length>=11;et(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">
        ${s.map((oe,Z)=>`<div style="width:7px;height:7px;border-radius:50%;background:${Z===o?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}
      </div>`:""}
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;height:${y}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ...
      ${s?`<img src="${s}" style="width:${y-4}px;height:${y-4}px;object-fit:contain">`:e!=null&&e.clubName?`<span style="font-size:${Math.max(4,y-8)}px;font-weight:700;color:#333">${(e.clubName||"").slice(0,3).toUpperCase()}</span>`:""}
        ...
        stroke="${K}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const x=48,_=64,f=13,c=16,z={normal:"#aaaaaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#111111",legende:"#7a28b8",légende:"#7a28b8"};for(const[F,C]of Object.entries(a)){const q=y(F);if(!q||!C)continue;const j=F.replace(/[0-9]/g,""),N=jt[j]||"#555",X=r.includes(C.cardId),K=i==="attack"&&(["MIL","ATT"].includes(j)||X)&&!C.used||i==="defense"&&["GK","DEF","MIL"].includes(j)&&!C.used,re=n.includes(C.cardId);let oe;i==="attack"?oe=X?Math.max(1,Number(C.note_a)||0):j==="MIL"?Number(C.note_m)||0:Number(C.note_a)||0:i==="defense"?oe=j==="GK"?Number(C.note_g)||0:j==="MIL"?Number(C.note_m)||0:Number(C.note_d)||0:oe=Number(j==="GK"?C.note_g:j==="DEF"?C.note_d:j==="MIL"?C.note_m:C.note_a)||0,oe=oe+(C.boost||0),C.stadiumBonus&&(i==="attack"&&(j==="ATT"||j==="MIL")||i==="defense"&&(j==="GK"||j==="DEF"||j==="MIL")?oe+=10:i||(oe+=10));const Z=(q.x-x/2).toFixed(1),R=(q.y-_/2).toFixed(1),G=z[C==null?void 0:C.rarity]||z.normal;if(C.used){const se=typeof import.meta<"u"&&"/"||"/",ue=`${typeof window<"u"&&((I=window.location)==null?void 0:I.origin)||""}${se}icons/carte-dos.png`.replace(/([^:])\/\//g,"$1/");p+=`<rect x="${Z}" y="${R}" width="${x}" height="${_}" rx="5" fill="#161616"/>`,p+=`<image href="${ue}" xlink:href="${ue}" x="${Z}" y="${R}" width="${x}" height="${_}" preserveAspectRatio="xMidYMid slice"/>`,p+=`<rect x="${Z}" y="${R}" width="${x}" height="${_}" rx="5" fill="none" stroke="${G}" stroke-width="2" opacity="0.7"/>`,p+=`<rect x="${Z}" y="${R}" width="${x}" height="${_}" rx="5" fill="rgba(0,0,0,0.01)" class="match-used-hit" data-card-id="${C.cardId}" data-role="${j}" style="cursor:pointer"/>`;continue}const ne=re?.45:1,ge=re?"#FFD700":G,_e=re?3:(C==null?void 0:C.rarity)==="legende"||(C==null?void 0:C.rarity)==="légende"||(C==null?void 0:C.rarity)==="pepite"||(C==null?void 0:C.rarity)==="pépite"?2.5:2,we=_-f-c;p+=`<defs><clipPath id="cp-${F}"><rect x="${Z}" y="${(q.y-_/2+f).toFixed(1)}" width="${x}" height="${we}"/></clipPath></defs>`,p+=`<rect x="${Z}" y="${R}" width="${x}" height="${_}" rx="5" fill="${N}" opacity="${ne}"/>`;const ve=Ne(C);ve&&(p+=`<image href="${ve}" xlink:href="${ve}" x="${Z}" y="${(q.y-_/2+f).toFixed(1)}" width="${x}" height="${we}" clip-path="url(#cp-${F})" preserveAspectRatio="xMidYMin slice"/>`),p+=`<rect x="${Z}" y="${R}" width="${x}" height="${f}" rx="4" fill="rgba(255,255,255,0.92)"/>`,p+=`<text x="${q.x.toFixed(1)}" y="${(q.y-_/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(C.name||"").slice(0,9)}</text>`;const Ee=(q.y+_/2-c).toFixed(1);p+=`<rect x="${Z}" y="${Ee}" width="${x}" height="${c}" fill="rgba(255,255,255,0.92)"/>`;{const se=ei(C.country_code);se?p+=`<image href="${se}" xlink:href="${se}" x="${(q.x-20).toFixed(1)}" y="${(q.y+_/2-c+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:p+=`<text x="${(q.x-13).toFixed(1)}" y="${(q.y+_/2-c/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${_o(C.country_code)}</text>`;const de=C.stadiumBonus?"#E87722":"#111";p+=`<text x="${q.x.toFixed(1)}" y="${(q.y+_/2-c/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="${de}" font-family="Arial Black">${oe}</text>`;const ue=wo(C);ue?p+=`<image href="${ue}" xlink:href="${ue}" x="${(q.x+x/2-14).toFixed(1)}" y="${(q.y+_/2-c+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:C.clubName&&(p+=`<text x="${(q.x+14).toFixed(1)}" y="${(q.y+_/2-c/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(C.clubName||"").slice(0,3).toUpperCase()}</text>`),C.stadiumBonus&&(p+=`<rect x="${(q.x-x/2).toFixed(1)}" y="${(q.y-_/2).toFixed(1)}" width="${x}" height="${_}" rx="5" fill="none" stroke="#E87722" stroke-width="2" opacity="0.8"/>`)}p+=`<rect x="${Z}" y="${R}" width="${x}" height="${_}" rx="5" fill="${re?"rgba(255,255,255,0.12)":"none"}" stroke="${ge}" stroke-width="${_e}" opacity="${ne}"/>`,K&&(p+=`<rect x="${Z}" y="${R}" width="${x}" height="${_}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${re?"selected":""}" data-card-id="${C.cardId}" data-role="${j}" style="cursor:pointer"/>`)}const u=38;return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-u} ${-u} ${d+u*2} ${s+u*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${p}
  ...
  </div>`}async function di(e,t,i,n){var q;const{state:d,navigate:s,toast:r}=t;et(e);const l=d.params||{};if(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!l.deckId)return qn(e,t,i);const m=l.deckId;let a,o,y,p;try{const j=await Promise.all([T.from("decks").select("formation,name,stadium_card_id").eq("id",m).single(),T.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation, evolution_bonus,
            player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
              clubs(encoded_name,logo_url)))`).eq("deck_id",m).order("slot_order")]);a=j[0].data,y=j[0].error,o=j[1].data,p=j[1].error}catch(j){console.error("[Match] Exception chargement deck:",j),Et(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>s("home"));return}if(y||p){console.error("[Match] Erreur Supabase:",y||p),Et(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>s("home"));return}const x=(o||[]).filter(j=>{var N;return j.is_starter&&((N=j.card)==null?void 0:N.player)}).map(j=>vi(j.card,j.position)),_=(o||[]).filter(j=>{var N;return!j.is_starter&&((N=j.card)==null?void 0:N.player)}).map(j=>vi(j.card,j.position));if(x.length<11){Et(e,"⚠️",`Deck incomplet (${x.length}/11).`,"Compléter",()=>s("decks"));return}const f=(o||[]).find(j=>{var N;return((N=j.card)==null?void 0:N.card_type)==="formation"}),c=(a==null?void 0:a.formation)||((q=f==null?void 0:f.card)==null?void 0:q.formation)||"4-4-2",{data:z,error:u}=await T.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",d.profile.id).eq("card_type","game_changer"),{data:I}=await T.from("gc_definitions").select("*").eq("is_active",!0),F=(z||[]).map(j=>({...j,_gcDef:(I==null?void 0:I.find(N=>N.name===j.gc_type||N.id===j.gc_definition_id))||null}));let C=null;if(a!=null&&a.stadium_card_id){const{data:j}=await T.from("cards").select("stadium_id").eq("id",a.stadium_card_id).single();if(j!=null&&j.stadium_id){const{data:N}=await T.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").eq("id",j.stadium_id).single();C=N||null}}n({deckId:m,formation:c,starters:x,subsRaw:_,gcCardsEnriched:F,gcDefs:I||[],stadiumDef:C})}async function Bn(e,t){const{state:i}=t,d=(i.params||{}).matchMode||"vs_ai_easy",s=d.replace("vs_ai_",""),r=d;await di(e,t,d,async({deckId:l,formation:m,starters:a,subsRaw:o,gcCardsEnriched:y,gcDefs:p,stadiumDef:x})=>{try{let _=bt(a,m);x&&(_=Tt(_,x),Ct(o,x));const f=await Fn(m,s),c=f.lines||f,z=async u=>{try{const{data:I,error:F}=await T.from("matches").insert({home_id:i.profile.id,away_id:null,mode:r,home_deck_id:l,status:"in_progress"}).select().single();if(F){console.error("[MatchIA] Erreur création match:",F),Et(e,"⚠️","Impossible de créer le match ("+F.message+").","Retour",()=>t.navigate("home"));return}const C=f.stadiumDef||null;C&&c&&(Tt(c,C),Ct(f.subs||[],C));const q={gcDefs:p||[],matchId:I==null?void 0:I.id,mode:r,difficulty:s,formation:m,homeTeam:_,aiTeam:c,homeSubs:o,subsUsed:0,maxSubs:Math.min(o.length,3),aiSubs:f.subs||[],aiSubsUsed:0,aiMaxSubs:Math.min((f.subs||[]).length,3),aiUsedSubIds:[],aiGcCards:f.gcCards||[],aiUsedGc:[],aiStadiumDef:C,homeScore:0,aiScore:0,gcCards:u,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Gn(e,q,t)}catch(I){console.error("[MatchIA] Exception launchMatch:",I),Et(e,"⚠️","Erreur au lancement du match : "+I.message,"Retour",()=>t.navigate("home"))}};if(!y.length){z([]);return}si(e,y,z)}catch(_){console.error("[MatchIA] Exception setup:",_),Et(e,"⚠️","Erreur de préparation du match : "+_.message,"Retour",()=>t.navigate("home"))}})}async function Fn(e,t){var c;const{data:i}=await T.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(80);if(!i||i.length<11)return{lines:Dn(e),subs:[],gcCards:[],stadiumDef:null};const n=Mt[e]||Mt["4-4-2"],d={GK:[],DEF:[],MIL:[],ATT:[]},s=new Set;function r(z,u,I){var F,C;return s.add(z.id),{cardId:"ai-"+z.id+"-"+I,id:z.id,firstname:z.firstname,name:z.surname_encoded,country_code:z.country_code,club_id:z.club_id,job:z.job,job2:z.job2,note_g:Number(z.note_g)||0,note_d:Number(z.note_d)||0,note_m:Number(z.note_m)||0,note_a:Number(z.note_a)||0,rarity:z.rarity,skin:z.skin,hair:z.hair,hair_length:z.hair_length,clubName:((F=z.clubs)==null?void 0:F.encoded_name)||null,clubLogo:((C=z.clubs)==null?void 0:C.logo_url)||null,boost:0,used:!1,_line:u}}for(const z of["GK","DEF","MIL","ATT"]){const u=i.filter(j=>j.job===z&&!s.has(j.id)),I=i.filter(j=>j.job!==z&&!s.has(j.id)),F=[...u,...I],C=[];for(let j=0;j<n[z];j++){const N=F[j];N&&C.push(r(N,z,j))}const q=st(C.length);C.forEach((j,N)=>{j._col=q[N]}),d[z]=C}const m=i.filter(z=>!s.has(z.id)).slice(0,5).map((z,u)=>r(z,z.job,100+u)),y=Object.keys(He).sort(()=>Math.random()-.5).slice(0,3).map((z,u)=>{var I,F;return{id:"ai-gc-"+u,gc_type:z,name:((I=He[z])==null?void 0:I.name)||z,icon:((F=He[z])==null?void 0:F.icon)||"⚡"}}),p=Object.values(d).flat(),x={};p.forEach(z=>{z.club_id&&(x[z.club_id]=(x[z.club_id]||0)+1)});const _=(c=Object.entries(x).sort((z,u)=>u[1]-z[1])[0])==null?void 0:c[0];let f=null;if(_){const{data:z}=await T.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",_).single();z&&(f={club_id:z.id,country_code:null,name:z.encoded_name+" Stadium",club:{encoded_name:z.encoded_name,logo_url:z.logo_url}})}return{lines:d,subs:m,gcCards:y,stadiumDef:f}}function Dn(e){const t=Mt[e]||Mt["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},n=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let d=0;for(const s of["GK","DEF","MIL","ATT"]){const r=[];for(let m=0;m<t[s];m++){const a=3+Math.floor(Math.random()*5);r.push({cardId:"fake-"+d,id:"fake-"+d,firstname:"IA",name:n[d%n.length],country_code:"XX",club_id:null,job:s,job2:null,note_g:s==="GK"?a:2,note_d:s==="DEF"?a:2,note_m:s==="MIL"?a:2,note_a:s==="ATT"?a:2,rarity:"normal",boost:0,used:!1,_line:s}),d++}const l=st(r.length);r.forEach((m,a)=>{m._col=l[a]}),i[s]=r}return i}function Gn(e,t,i){var n;e.innerHTML=`
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
  </div>`,setTimeout(()=>Pn(e,t,i),5e3)}function Pn(e,t,i){const n=t.homeTeam.MIL||[],d=t.aiTeam.MIL||[],s=t.stadiumDef||null,r=t.aiStadiumDef||null;function l(u,I){const F=Te(u,"MIL"),C=u.evolution_bonus||u._evolution_bonus||0,q=u.job==="MIL"||u.job2==="MIL"?C:0,j=u.stadiumBonus||I&&(I.club_id&&String(u.club_id)===String(I.club_id)||I.country_code&&u.country_code===I.country_code)?10:0;return F+q+j}function m(u,I){return u.reduce((F,C)=>F+l(C,I),0)}function a(u){let I=0;for(let F=0;F<u.length-1;F++){const C=nt(u[F],u[F+1]);C==="#00ff88"?I+=2:C==="#FFD700"&&(I+=1)}return I}const o=m(n,s)+a(n),y=m(d,r)+a(d),p=o>=y;function x(u,I,F,C,q){return`<div id="duel-row-${C}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0), opacity .5s ease;transform-origin:center">
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${u.map((j,N)=>{const X=N<u.length-1?nt(j,u[N+1]):null,K=!X||X==="#ff3333"||X==="#cc2222",re=X==="#00ff88"?"+2":X==="#FFD700"?"+1":"";l(j,q),j.stadiumBonus||q&&(q.club_id&&String(j.club_id)===String(q.club_id)||q.country_code&&(j.country_code,q.country_code));const oe=j.evolution_bonus||j._evolution_bonus||0;return`
          ...
          ${N<u.length-1?`<div class="duel-link duel-link-${C}" data-idx="${N}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${K?"rgba(255,255,255,0.12)":X};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${K?"none":`0 0 8px ${X}`}">
  ...
  <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div id="duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center, rgba(10,61,30,0.4), rgba(10,61,30,0.92))"></div>
  </div>`;const _=()=>{const u=(I,F)=>e.querySelectorAll(I).forEach((C,q)=>{setTimeout(()=>{C.style.opacity="1",C.style.transform="translateY(0) scale(1)"},F+q*90)});u(".duel-card-home",150),u(".duel-card-ai",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((I,F)=>{setTimeout(()=>{I.style.opacity="1"},F*70)}),900),setTimeout(()=>{const I=e.querySelector("#vs-label");I&&(I.style.opacity="1",I.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(F=>F.style.opacity="1")},1250),setTimeout(()=>{f("score-home",o,800),f("score-ai",y,800)},1500)};function f(u,I,F){const C=document.getElementById(u);if(!C)return;const q=performance.now(),j=N=>{const X=Math.min(1,(N-q)/F);C.textContent=Math.round(I*(1-Math.pow(1-X,3))),X<1?requestAnimationFrame(j):C.textContent=I};requestAnimationFrame(j)}requestAnimationFrame(_),t.attacker=p?"home":"ai";const c=p?ai():null;p&&(t.boostCard={value:c}),t.log.push({type:"duel",title:"Milieu de Terrain",homePlayers:n.map(u=>({name:u.name,note:Te(u,"MIL"),portrait:Ne(u),job:u.job,country_code:u.country_code,rarity:u.rarity,clubName:u.clubName,clubLogo:u.clubLogo})),aiPlayers:d.map(u=>({name:u.name,note:Te(u,"MIL"),portrait:Ne(u),job:u.job,country_code:u.country_code,rarity:u.rarity,clubName:u.clubName,clubLogo:u.clubLogo})),homeTotal:o,aiTotal:y,text:`Duel milieu : ${t.clubName} ${o} – ${y} IA → ${p?t.clubName+" attaque":"IA attaque"}`});const z=()=>{t.phase=t.attacker==="home"?"attack":"ai-attack",Je(e,t,i),t.attacker==="ai"&&setTimeout(()=>ti(e,t,i),800)};setTimeout(()=>{const u=document.getElementById("score-home"),I=document.getElementById("score-ai"),F=document.getElementById(p?"duel-row-home":"duel-row-ai"),C=document.getElementById(p?"duel-row-ai":"duel-row-home"),q=p?u:I,j=p?I:u;q&&(q.style.fontSize="80px",q.style.color=p?"#FFD700":"#ff6b6b",q.style.animation="duelPulse .5s ease"+(p?", duelGlow 1.5s ease infinite .5s":"")),j&&(j.style.opacity="0.25"),setTimeout(()=>{F&&(F.style.transformOrigin="center",F.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",F.style.zIndex="5"),setTimeout(()=>{var X;const N=document.getElementById("duel-shock");if(N){const K=(X=C||F)==null?void 0:X.getBoundingClientRect(),re=e.querySelector(".match-screen").getBoundingClientRect();K&&(N.style.top=K.top-re.top+K.height/2+"px"),N.style.animation="shockwave .5s ease-out forwards"}C&&(C.style.transformOrigin="center",C.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var X;const N=document.getElementById("duel-finale");N&&(N.innerHTML=`
          ...
          <button id="start-match-btn" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,0.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">
            ▶ Commencer le match
          </button>`,N.style.transition="opacity .45s ease",N.style.opacity="1",N.style.pointerEvents="auto",(X=document.getElementById("start-match-btn"))==null||X.addEventListener("click",z))},600)},700)},2800)}function Nn(e){if(e.type==="duel"&&(e.homeTotal!=null||e.aiTotal!=null)){const t=(e.homeTotal||0)>=(e.aiTotal||0);return`
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const ne=t.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
          ...
          </div>`}const G=t.log[t.log.length-1];return G?'<div style="padding:2px 4px">'+Nn(G)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})()}
    </div>
    ...
    ${(()=>{const G=window.innerWidth>=700,ne=(U,ee,xe)=>{var B,$;const le=(t.gcDefs||[]).find(g=>g.name===U.gc_type),me={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[le==null?void 0:le.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",ie={purple:"#b06ce0",light_blue:"#00d4ef"}[le==null?void 0:le.color]||"#b06ce0",pe=(le==null?void 0:le.name)||U.gc_type,Le=(le==null?void 0:le.effect)||((B=He[U.gc_type])==null?void 0:B.desc)||"",Ie=le!=null&&le.image_url?`/icons/${le.image_url}`:null,Ce=(($=He[U.gc_type])==null?void 0:$.icon)||"⚡",Ae=Math.round(xe*.22),qe=Math.round(xe*.22),Ge=xe-Ae-qe,Se=pe.length>12?7:9;return`<div class="gc-mini" data-gc-id="${U.id}" data-gc-type="${U.gc_type}"
          ...
          <div style="height:${Ae}px;background:rgba(255,255,255,0.14);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 3px">
            <span style="font-size:${Se}px;font-weight:900;color:#fff;text-align:center;line-height:1.1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${ee-6}px">${pe}</span>
          ...
          </div>`},_e=(U,ee)=>ee?ge(130,175):ne(U,130,175),we=(U,ee)=>ee?ge(68,95):ne(U,68,95),ve=G?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",Ee=_?`<button id="btn-results" style="${ve};background:linear-gradient(135deg,#D4A017,#FFD700);border:none;color:#000">🏁 Résultats</button>`:y?`<div style="${ve};background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);color:rgba(255,255,255,0.4)">⏳ Tour IA</div>`:z?`<button id="btn-pass" style="${ve};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER (plus d'attaquants)</button>`:p?`<button id="btn-action" style="${ve};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${t.selected.length===0?"disabled":""}> ⚔️ ATTAQUEZ <span id="match-timer" style="font-weight:900"></span></button>`:x?`<button id="btn-action" style="${ve};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${t.selected.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="match-timer" style="font-weight:900"></span></button>`:`<div style="${ve};background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1)"></div>`,se=p||x?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${t.selected.length}/3 sélectionné(s)</div>`:"",de=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${G?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(G=>{var ne,ge,_e;if(G.type==="duel"){const we=G.isGoal,ve=G.homeScored?"#FFD700":we?"#ff6b6b":"rgba(255,255,255,0.3)",Ee=G.homeScored?"⚽ BUT !":we?"⚽ BUT IA !":(ne=G.homePlayers)!=null&&ne.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${we?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${ve};margin-bottom:4px">
  ...
  </div>`;function F(){const G=e.querySelector(".match-screen");if(!G)return;const ne=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);G.style.bottom="auto",G.style.height=ne+"px",G.style.minHeight=ne+"px",G.style.maxHeight=ne+"px",G.style.overflow="hidden";const ge=e.querySelector("#mobile-action-bar"),_e=e.querySelector("#mobile-play-area");ge&&_e&&(_e.style.paddingBottom=ge.offsetHeight+"px")}if(F(),setTimeout(F,120),setTimeout(F,400),setTimeout(F,1e3),t._vvBound||(t._vvBound=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",F),window.visualViewport.addEventListener("scroll",F)),window.addEventListener("resize",F)),function(){const ne=e.querySelector(".terrain-wrapper svg");ne&&(ne.removeAttribute("width"),ne.removeAttribute("height"),ne.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",ne.setAttribute("viewBox","-26 -26 352 352"),ne.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),t._resizeBound||(t._resizeBound=!0,window.addEventListener("resize",()=>{const G=e.querySelector(".terrain-wrapper svg");G&&(G.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0")})),t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase==="attack"||t.phase==="defense"){let G=!1,ne=30;const ge=()=>document.getElementById("match-timer"),_e=()=>{const we=ge();if(!we)return;const ve=String(Math.floor(ne/60)).padStart(2,"0"),Ee=String(ne%60).padStart(2,"0");we.textContent=` ${ve}:${Ee}`,we.style.color=G?"#ff2222":"#ff9500",we.style.fontWeight="900"};_e(),t._timerInt=setInterval(()=>{if(ne--,ne<0)if(!G)G=!0,ne=15,_e();else{clearInterval(t._timerInt),t._timerInt=null,t.homeScore=0,t.aiScore=3;const we=document.createElement("div");we.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;color:#fff;padding:24px;text-align:center",we.innerHTML='<div style="font-size:56px">⏱️</div><div style="font-size:24px;font-weight:900;color:#ff4444">MATCH PERDU PAR FORFAIT</div><div style="font-size:14px;color:rgba(255,255,255,0.6)">Temps écoulé</div>',document.body.appendChild(we),setTimeout(()=>{we.remove(),Dt(e,t,i)},2500)}else _e()},1e3)}(q=document.getElementById("match-quit"))==null||q.addEventListener("click",()=>{Ve(e),confirm("Abandonner ? Résultat : défaite 3-0")&&(t.homeScore=0,t.aiScore=3,Dt(e,t,i))}),(j=document.getElementById("view-ai"))==null||j.addEventListener("click",()=>Qn(t,i)),(N=document.getElementById("toggle-history"))==null||N.addEventListener("click",()=>{var G;(G=document.getElementById("match-history-panel"))==null||G.classList.add("open")}),(X=document.getElementById("close-history"))==null||X.addEventListener("click",()=>{var G;(G=document.getElementById("match-history-panel"))==null||G.classList.remove("open")}),(K=document.getElementById("btn-action"))==null||K.addEventListener("click",()=>{t.selected.length!==0&&(p?On(e,t,i):x&&Hn(e,t,i))}),(re=document.getElementById("btn-results"))==null||re.addEventListener("click",()=>Dt(e,t,i)),(oe=document.getElementById("btn-pass"))==null||oe.addEventListener("click",()=>{t.log.push({text:"⏭️ Vous passez votre tour (plus d'attaquants)",type:"info"}),t.phase="ai-attack",Je(e,t,i),setTimeout(()=>ti(e,t,i),800)}),e.querySelectorAll(".match-slot-hit").forEach(G=>{G.addEventListener("click",()=>Rn(G,t,e,i))}),e.querySelectorAll(".match-used-hit").forEach(G=>{G.addEventListener("click",()=>gi(e,t,i,null,G.dataset.cardId))}),e.querySelectorAll(".gc-mini").forEach(G=>{G.addEventListener("click",()=>Wn(G.dataset.gcId,G.dataset.gcType,e,t,i))}),(Z=document.getElementById("boost-card"))==null||Z.addEventListener("click",()=>Zn(e,t,i)),e.querySelectorAll(".sub-btn-col").forEach(G=>{G.addEventListener("click",()=>gi(e,t,i,G.dataset.subId))}),(R=document.getElementById("sub-btn-main"))==null||R.addEventListener("click",()=>gi(e,t,i))}function Rn(e,t,i,n){const d=e.dataset.cardId,s=e.dataset.role,r=t.selected.findIndex(l=>l.cardId===d);if(r!==-1)t.selected.splice(r,1);else{if(t.selected.length>=3){n.toast("Maximum 3 joueurs","error");return}const l=(t.homeTeam[s]||[]).find(m=>m.cardId===d);l&&t.selected.push({...l,_role:s,_line:s})}Je(i,t,n)}function Li(e,t,i){e.matchId&&T.from("matches").update({last_player_id:i}).eq("id",e.matchId).then(()=>{})}function On(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),Li(t,i,i.state.profile.id);const n=!["GK","DEF","MIL","ATT"].some(r=>(t.aiTeam[r]||[]).some(l=>!l.used)),d=t.selected.map(r=>{const l=(t.homeTeam[r._role]||[]).find(a=>a.cardId===r.cardId)||r,m=n&&["GK","DEF"].includes(r._role);return{...l,_line:r._role,...m?{note_a:Math.max(1,Number(l.note_a)||0)}:{}}}),s=Ht(d,t.modifiers.home);t.pendingAttack={...s,players:[...d],side:"home"},t.selected.forEach(r=>{const l=(t.homeTeam[r._role]||[]).find(m=>m.cardId===r.cardId);l&&(l.used=!0)}),t.log.push({text:`⚔️ Vous attaquez : ${s.total} (base ${s.base}${s.links?` +${s.links} liens`:""}) — ${t.selected.map(r=>r.name).join(", ")}`,type:"info"}),t.selected=[],t.modifiers.home={},t.phase="ai-defense",Je(e,t,i),setTimeout(()=>Vn(e,t,i),1200)}function Hn(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),Li(t,i,i.state.profile.id);const n=t.selected.map(l=>({...(t.homeTeam[l._role]||[]).find(a=>a.cardId===l.cardId)||l,_line:l._role})),d=Ut(n,t.modifiers.home);t.selected.forEach(l=>{const m=(t.homeTeam[l._role]||[]).find(a=>a.cardId===l.cardId);m&&(m.used=!0)});const s=Kt(t.pendingAttack.total,d.total,t.modifiers.home),r={type:"duel",title:"Défense",aiPlayers:(t.pendingAttack.players||[]).map(l=>({name:l.name,note:l._line==="MIL"?l.note_m:l.note_a,portrait:Ne(l),job:l.job,country_code:l.country_code,rarity:l.rarity,clubName:l.clubName,clubLogo:l.clubLogo})),homePlayers:t.selected.map(l=>{const m=(t.homeTeam[l._role]||[]).find(a=>a.cardId===l.cardId)||l;return{name:m.name,note:(m._line==="GK"?Number(m.note_g)||0:m._line==="MIL"?Number(m.note_m)||0:Number(m.note_d)||0)+(m.boost||0),portrait:Ne(m),job:m.job,country_code:m.country_code,rarity:m.rarity,clubName:m.clubName,clubLogo:m.clubLogo}}),homeTotal:d.total,aiTotal:t.pendingAttack.total,isGoal:!1,homeScored:!1,text:""};if(s.shielded)r.text="🛡️ Bouclier ! But annulé.",t.log.push(r);else if(s.goal){t.aiScore++,r.isGoal=!0,r.homeScored=!1,r.text=`⚽ BUT IA ! (${t.pendingAttack.total} > ${d.total})`,t.log.push(r),t.selected=[],t.modifiers.home={},t.pendingAttack=null,Je(e,t,i),ii(r.aiPlayers,t.homeScore,t.aiScore,!1,()=>{Lt(e,t,i,"home-attack")});return}else r.text=`🧤 Défense réussie ! (${d.total} ≥ ${t.pendingAttack.total})`,t.log.push(r);t.selected=[],t.modifiers.home={},t.pendingAttack=null,Lt(e,t,i,"home-attack")}function Un(e){if(e.aiSubsUsed>=e.aiMaxSubs)return;const t=Object.values(e.aiTeam).flat().filter(m=>m.used);if(!t.length)return;const i=(e.aiSubs||[]).filter(m=>!e.aiUsedSubIds.includes(m.cardId));if(!i.length)return;const n=t[Math.floor(Math.random()*t.length)],d=i.find(m=>m.job===n.job)||i[0],s={...d,used:!1,_line:n._line,_col:n._col},r=e.aiTeam[n._line],l=r.findIndex(m=>m.cardId===n.cardId);l!==-1&&(r[l]=s),e.aiUsedSubIds.push(d.cardId),e.aiSubsUsed++,e.log.push({text:`🔄 IA : ${d.firstname} ${d.name} remplace ${n.firstname} ${n.name}`,type:"info"})}function Kn(e){var n;if(!((n=e.aiGcCards)!=null&&n.length))return;const t=e.aiGcCards.filter(d=>!e.aiUsedGc.includes(d.id));if(!t.length||Math.random()>.3)return;const i=t[Math.floor(Math.random()*t.length)];switch(e.aiUsedGc.push(i.id),He[i.gc_type],i.gc_type){case"Boost+2":{const d=Object.values(e.aiTeam).flat().filter(s=>!s.used);if(d.length){const s=d[Math.floor(Math.random()*d.length)];s.boost=(s.boost||0)+2}break}case"Boost+3":{const d=Object.values(e.aiTeam).flat().filter(s=>!s.used);if(d.length){const s=d[Math.floor(Math.random()*d.length)];s.boost=(s.boost||0)+3}break}case"Remplacement+":e.aiMaxSubs=(e.aiMaxSubs||3)+1;break;case"Bouclier":e.modifiers.ai.shield=!0;break;case"Nul+1":e.modifiers.ai.drawBonus=(e.modifiers.ai.drawBonus||0)+1;break}e.log.push({text:`⚡ IA joue ${i.icon||"⚡"} ${i.name}`,type:"gc"})}function ti(e,t,i){Li(t,i,null),Un(t),Kn(t);const n=[...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]],d=so(n,"attack",t.difficulty);if(!d.length){Zt(e,t,i);return}const s=Ht(d,t.modifiers.ai);t.pendingAttack={...s,players:d,side:"ai"},d.forEach(a=>{a.used=!0}),t.log.push({text:`🤖 IA attaque : ${s.total} (${d.map(a=>a.name).join(", ")})`,type:"info"}),t.modifiers.ai={};const r=[...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(a=>!a.used),m=(t.homeSubs||[]).filter(a=>!(t.usedSubIds||[]).includes(a.cardId)).length>0&&t.subsUsed<t.maxSubs;if(r.length===0&&!m){t.aiScore++;const a={type:"duel",isGoal:!0,homeScored:!1,aiPlayers:d.map(o=>({name:o.name,note:o._line==="MIL"?o.note_m:o.note_a,portrait:Ne(o),job:o.job,country_code:o.country_code,rarity:o.rarity,clubName:o.clubName,clubLogo:o.clubLogo})),aiTotal:s.total,text:"⚽ BUT IA ! (aucun défenseur disponible)"};t.log.push(a),t.pendingAttack=null,Je(e,t,i),ii(a.aiPlayers,t.homeScore,t.aiScore,!1,()=>{Lt(e,t,i,"home-attack")});return}t.phase="defense",Je(e,t,i)}function Vn(e,t,i){var a,o;const n=[...t.aiTeam.GK||[],...t.aiTeam.DEF||[],...t.aiTeam.MIL||[]],d=so(n,"defense",t.difficulty);if(!["GK","DEF","MIL","ATT"].flatMap(y=>(t.aiTeam[y]||[]).filter(p=>!p.used)).length){t.homeScore++;const p={type:"duel",isGoal:!0,homeScored:!0,homePlayers:(((a=t.pendingAttack)==null?void 0:a.players)||[]).map(x=>({name:x.name,note:Te(x,x._line||x.job),portrait:Ne(x),job:x.job,country_code:x.country_code,rarity:x.rarity,clubName:x.clubName,clubLogo:x.clubLogo})),homeTotal:((o=t.pendingAttack)==null?void 0:o.total)||0,aiTotal:0,text:"⚽ BUT ! L'IA n'a plus de joueurs — but automatique !"};t.log.push(p),t.modifiers.ai={},t.pendingAttack=null,Je(e,t,i),ii(p.homePlayers,t.homeScore,t.aiScore,!0,()=>{Lt(e,t,i,"ai-attack")});return}const r=d.length>0?Ut(d,t.modifiers.ai).total:0;d.forEach(y=>{y.used=!0});const l=Kt(t.pendingAttack.total,r,t.modifiers.ai),m={type:"duel",title:"Attaque",homePlayers:(t.pendingAttack.players||[]).map(y=>({name:y.name,note:y._line==="MIL"?y.note_m:y.note_a,portrait:Ne(y),job:y.job,country_code:y.country_code,rarity:y.rarity,clubName:y.clubName,clubLogo:y.clubLogo})),aiPlayers:d.map(y=>({name:y.name,note:y._line==="GK"?y.note_g:y._line==="MIL"?y.note_m:y.note_d,portrait:Ne(y),job:y.job,country_code:y.country_code,rarity:y.rarity,clubName:y.clubName,clubLogo:y.clubLogo})),homeTotal:t.pendingAttack.total,aiTotal:r,isGoal:!1,homeScored:!1,text:""};if(l.shielded)m.text="🛡️ Bouclier IA !",t.log.push(m);else if(l.goal){t.homeScore++,m.isGoal=!0,m.homeScored=!0,m.text=`⚽ BUT ! (${t.pendingAttack.total} > ${r})`,t.log.push(m),t.modifiers.ai={},t.pendingAttack=null,Je(e,t,i),ii(m.homePlayers,t.homeScore,t.aiScore,!0,()=>{Lt(e,t,i,"ai-attack")});return}else m.text=`🧤 IA défend (${r} ≥ ${t.pendingAttack.total})`,t.log.push(m);t.modifiers.ai={},t.pendingAttack=null,Lt(e,t,i,"ai-attack")}function Lt(e,t,i,n){if(t.round++,$o(t)){Dt(e,t,i);return}if(n==="home-attack"){if(![...t.homeTeam.MIL||[],...t.homeTeam.ATT||[]].filter(s=>!s.used).length){if(![...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(l=>!l.used).length){Zt(e,t,i);return}if(![...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]].filter(l=>!l.used).length){Zt(e,t,i);return}setTimeout(()=>ti(e,t,i),100);return}t.phase="attack",Je(e,t,i)}else{if(![...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]].filter(s=>!s.used).length){Zt(e,t,i);return}t.phase="ai-attack",Je(e,t,i),setTimeout(()=>ti(e,t,i),800)}}function $o(e){const t=["MIL","ATT","GK","DEF"].some(n=>(e.homeTeam[n]||[]).some(d=>!d.used)),i=["MIL","ATT","GK","DEF"].some(n=>(e.aiTeam[n]||[]).some(d=>!d.used));return!t&&!i}function Zt(e,t,i){$o(t)?Dt(e,t,i):(t.phase="attack",Je(e,t,i))}function Yn(e,t,i){const n=document.createElement("div");n.style.cssText=`
  ...
  `,document.body.appendChild(n);let o=!1;const y=()=>{o||(o=!0,n.remove(),i())};n.addEventListener("click",y),setTimeout(y,2e3)}function Bt(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function gi(e,t,i,n=null,d=null){var x,_;if(t.phase!=="attack"){Bt("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){Bt(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const s=Object.entries(t.homeTeam).flatMap(([f,c])=>(c||[]).filter(z=>z.used).map(z=>({...z,_line:z._line||f}))),r=t.homeSubs.filter(f=>!t.usedSubIds.includes(f.cardId));if(!s.length){Bt("Aucun joueur utilisé à remplacer");return}if(!r.length){Bt("Aucun remplaçant disponible");return}let l=Math.max(0,s.findIndex(f=>f.cardId===d));const m=((x=s[l])==null?void 0:x._line)||((_=s[l])==null?void 0:_.job);let a=n?Math.max(0,r.findIndex(f=>f.cardId===n)):Math.max(0,r.findIndex(f=>f.job===m)),o=!1;const y=document.createElement("div");y.id="sub-overlay",y.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function p(){var C,q,j,N,X,K;const f=s[l],c=r[a],z=Math.min(130,Math.round((window.innerWidth-90)/2)),u=Math.round(z*1.35),I=re=>`background:rgba(255,255,255,0.12);border:none;color:${re?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${re?"default":"pointer"};flex-shrink:0`;y.innerHTML=`
    ...
    </div>`,(C=y.querySelector("#sub-close"))==null||C.addEventListener("click",()=>y.remove()),(q=y.querySelector("#out-up"))==null||q.addEventListener("click",()=>{l>0&&(l--,p())}),(j=y.querySelector("#out-down"))==null||j.addEventListener("click",()=>{l<s.length-1&&(l++,p())}),(N=y.querySelector("#in-up"))==null||N.addEventListener("click",()=>{a>0&&(a--,p())}),(X=y.querySelector("#in-down"))==null||X.addEventListener("click",()=>{a<r.length-1&&(a++,p())});const F=(re,oe,Z,R)=>{const G=y.querySelector("#"+re);if(!G)return;let ne=0;G.addEventListener("touchstart",ge=>{ne=ge.touches[0].clientY},{passive:!0}),G.addEventListener("touchend",ge=>{const _e=ge.changedTouches[0].clientY-ne;if(Math.abs(_e)<30)return;const we=oe();_e<0&&we<R-1?(Z(we+1),p()):_e>0&&we>0&&(Z(we-1),p())},{passive:!0})};F("in-panel",()=>a,re=>a=re,r.length),F("out-panel",()=>l,re=>l=re,s.length),(K=y.querySelector("#sub-confirm"))==null||K.addEventListener("click",re=>{if(re.preventDefault(),re.stopPropagation(),o)return;o=!0;const oe=s[l],Z=r[a];if(!oe||!Z)return;let R=null,G=-1;for(const[ge,_e]of Object.entries(t.homeTeam)){const we=(_e||[]).findIndex(ve=>ve.cardId===oe.cardId);if(we!==-1){R=ge,G=we;break}}if(G===-1||!R){Bt("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),y.remove();return}const ne={...Z,_line:R,_col:oe._col||0,used:!1,boost:0};t.homeTeam[R].splice(G,1,ne),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(Z.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:oe.name,firstname:oe.firstname,note:Te(oe,R),portrait:Ne(oe),job:oe.job,country_code:oe.country_code,rarity:oe.rarity,clubName:oe.clubName,clubLogo:oe.clubLogo},inPlayer:{name:Z.name,firstname:Z.firstname,note:Te(Z,R),portrait:Ne(Z),job:Z.job,country_code:Z.country_code,rarity:Z.rarity,clubName:Z.clubName,clubLogo:Z.clubLogo},text:`🔄 ${Z.firstname} ${Z.name} remplace ${oe.firstname} ${oe.name}`}),y.remove(),Yn(oe,Z,()=>Je(e,t,i))})}document.body.appendChild(y),p()}function Wn(e,t,i,n,d){var _,f;const s=(n.gcDefs||[]).find(c=>c.name===t),r=He[t]||{icon:"⚡",desc:"Carte spéciale."},l={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[s==null?void 0:s.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",m={purple:"#b06ce0",light_blue:"#00d4ef"}[s==null?void 0:s.color]||"#b06ce0",a=(s==null?void 0:s.name)||t,o=(s==null?void 0:s.effect)||r.desc,y=s!=null&&s.image_url?`/icons/${s.image_url}`:null,p=r.icon||"⚡",x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",x.innerHTML=`
    ...
    </div>`,document.body.appendChild(x),(_=x.querySelector("#gc-back"))==null||_.addEventListener("click",()=>x.remove()),(f=x.querySelector("#gc-use"))==null||f.addEventListener("click",()=>{x.remove(),Xn(e,t,i,n,d)})}function Jt(e,t,i,n,d,s){const r=document.createElement("div");r.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let l=[];function m(){var a,o;r.innerHTML=`
    ...
    <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
      ${e.map(y=>{const p=y._line||y.job||"MIL",x={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[p]||"#555",_=Te(y,p)+(y.boost||0),f=l.find(c=>c.cardId===y.cardId);return`<div class="gc-pick-item" data-cid="${y.cardId}"
    ...
    <div style="display:flex;flex-direction:column;gap:6px">
      ${n.map(d=>`
        <div class="player-boost-opt" data-card-id="${d.cardId}"
          ...
          <div style="width:32px;height:32px;background:${jt[d.job]||"#888"};border-radius:6px;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:900;font-size:13px">${Te(d,d._line||d.job)}</div>
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(s);let l=!1;const m=()=>{l||(l=!0,s.remove(),setTimeout(()=>d(),50))};s.addEventListener("click",m),setTimeout(m,3500)}async function Dt(e,t,i){var o,y;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:n}=i,d=t.homeScore>t.aiScore,s=t.homeScore===t.aiScore,r=d?"victoire":s?"nul":"defaite",l=Qo(t.mode,r);t.matchId&&await T.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:d?n.profile.id:null,home_credits_reward:l,played_at:new Date().toISOString()}).eq("id",t.matchId);const m={credits:(n.profile.credits||0)+l,matches_played:(n.profile.matches_played||0)+1};d?m.wins=(n.profile.wins||0)+1:s?m.draws=(n.profile.draws||0)+1:m.losses=(n.profile.losses||0)+1,await T.from("users").update(m).eq("id",n.profile.id),await i.refreshProfile();const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",a.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const F=async(N=!0)=>{_=!0,f&&(T.removeChannel(f),f=null),N&&await T.rpc("cancel_matchmaking",{p_user_id:y.profile.id}).catch(()=>{})};(j=document.getElementById("mm-cancel"))==null||j.addEventListener("click",async()=>{try{await F(!0)}catch{}Ve(e),p("home")});const C=async(N,X,K)=>{if(_)return;_=!0,f&&(T.removeChannel(f),f=null);const re=document.getElementById("mm-status");re&&(re.textContent="Adversaire trouvé !"),await new Promise(oe=>setTimeout(oe,600)),e.isConnected&&ko(e,t,N,K,r,l,a,o,m)};if(a){const N=[{range:50,delay:0},{range:100,delay:15e3},{range:200,delay:3e4},{range:400,delay:45e3},{range:800,delay:6e4}];let X=0,K=!1;const re=async()=>{if(_||K)return;const{range:oe}=N[X],Z=document.getElementById("mm-range");Z&&(Z.textContent=`Plage MMR : ±${oe}`);const{data:R,error:G}=await T.rpc("try_matchmake_ranked",{p_user_id:y.profile.id,p_deck_id:i,p_mmr:c,p_range:oe});if(console.log("[Ranked] try_matchmake_ranked result:",R,"error:",JSON.stringify(G)),_)return;if(G||!(R!=null&&R.success)){x("Erreur matchmaking ranked","error"),Ve(e),p("home");return}if(R.matched){K=!0,C(R.match_id,R.opponent_id,!1);return}const ne=document.getElementById("mm-status");if(ne&&(ne.textContent="En attente d'un adversaire..."),f||(f=T.channel("matchmaking-"+y.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${y.profile.id}`},ge=>{const _e=ge.new;_e.status==="matched"&&_e.match_id&&!K&&(K=!0,C(_e.match_id,_e.matched_with,!0))}).subscribe()),X<N.length-1){const ge=N[X+1].delay-N[X].delay;X++,setTimeout(()=>{!_&&!K&&re()},ge)}};await re()}else{const{data:N,error:X}=await T.rpc("try_matchmake",{p_user_id:y.profile.id,p_deck_id:i});if(X||!(N!=null&&N.success)){x("Erreur de matchmaking","error"),Ve(e),p("home");return}if(N.matched){C(N.match_id,N.opponent_id,!1);return}const K=document.getElementById("mm-status");K&&(K.textContent="En attente d'un autre joueur..."),f=T.channel("matchmaking-"+y.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${y.profile.id}`},re=>{const oe=re.new;oe.status==="matched"&&oe.match_id&&C(oe.match_id,oe.matched_with,!0)}).subscribe()}}async function tr(e,t,i){const{state:n,navigate:d,toast:s}=t;try{(T.getChannels?T.getChannels():[]).forEach(a=>{const o=a.topic||"";(o.includes("matchmaking")||o.includes("pvp-match")||o.includes("friend-invite"))&&T.removeChannel(a)})}catch{}const{data:r}=await T.from("matches").select("home_id, away_id, status").eq("id",i).single();if(!r){s("Match introuvable","error"),d("home");return}if(r.status==="finished"){s("Ce match est terminé","info"),d("home");return}const l=r.home_id===n.profile.id;ko(e,t,i,l,[],[])}async function ko(e,t,i,n,d=[],s=[],r=!1,l=null,m=null){const{state:a,navigate:o,toast:y}=t,p=n?"p1":"p2",x=n?"p2":"p1",_=(d||[]).map($=>$.id),f=(d||[]).map($=>({id:$.id,gc_type:$.gc_type,_gcDef:$._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:c}=await T.from("matches").select("*").eq("id",i).single();if(!c){y("Match introuvable","error"),o("home");return}async function z(){var Y,te;console.log("[PvP] buildGameState match:",{id:c.id,home_deck_id:c.home_deck_id,away_deck_id:c.away_deck_id,mode:c.mode,is_ranked:c.is_ranked});const[{data:$,error:g},{data:b,error:h},{data:v},{data:w}]=await Promise.all([T.rpc("get_deck_for_match",{p_deck_id:c.home_deck_id}),T.rpc("get_deck_for_match",{p_deck_id:c.away_deck_id}),T.from("users").select("id,pseudo,club_name").eq("id",c.home_id).single(),T.from("users").select("id,pseudo,club_name").eq("id",c.away_id).single()]);console.log("[PvP] get_deck_for_match p1:",g==null?void 0:g.message,"p2:",h==null?void 0:h.message,"p1D:",(Y=$==null?void 0:$.starters)==null?void 0:Y.length,"p2D:",(te=b==null?void 0:b.starters)==null?void 0:te.length);const k=Q=>{const fe=Number(Q.evolution_bonus)||0;return{cardId:Q.card_id,position:Q.position,id:Q.id,firstname:Q.firstname,name:Q.surname_encoded,country_code:Q.country_code,club_id:Q.club_id,job:Q.job,job2:Q.job2,note_g:(Number(Q.note_g)||0)+(Q.job==="GK"||Q.job2==="GK"&&Number(Q.note_g)>0?fe:0),note_d:(Number(Q.note_d)||0)+(Q.job==="DEF"||Q.job2==="DEF"&&Number(Q.note_d)>0?fe:0),note_m:(Number(Q.note_m)||0)+(Q.job==="MIL"||Q.job2==="MIL"&&Number(Q.note_m)>0?fe:0),note_a:(Number(Q.note_a)||0)+(Q.job==="ATT"||Q.job2==="ATT"&&Number(Q.note_a)>0?fe:0),evolution_bonus:fe,rarity:Q.rarity,skin:Q.skin,hair:Q.hair,hair_length:Q.hair_length,clubName:Q.club_encoded_name||null,clubLogo:Q.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},A=(($==null?void 0:$.starters)||[]).map(Q=>k(Q)),E=((b==null?void 0:b.starters)||[]).map(Q=>k(Q)),L=($==null?void 0:$.formation)||"4-4-2",S=(b==null?void 0:b.formation)||"4-4-2";let M=bt(A,L),D=bt(E,S);const V=(($==null?void 0:$.subs)||[]).map(Q=>k(Q)),W=((b==null?void 0:b.subs)||[]).map(Q=>k(Q)),H=($==null?void 0:$.stadium_def)||(n?m:null),P=(b==null?void 0:b.stadium_def)||(n?null:m);return H&&(M=Tt(M,H),Ct(V,H)),P&&(D=Tt(D,P),Ct(W,P)),{p1Team:M,p2Team:D,p1Subs:V,p2Subs:W,p1Formation:L,p2Formation:S,p1Name:(v==null?void 0:v.club_name)||(v==null?void 0:v.pseudo)||"Joueur 1",p2Name:(w==null?void 0:w.club_name)||(w==null?void 0:w.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:n?_:[],gc_p2:n?[]:_,gcCardsFull_p1:n?f:[],gcCardsFull_p2:n?[]:f,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:s||[],lastActionAt:new Date().toISOString()}}let u=c.game_state&&Object.keys(c.game_state).length?c.game_state:null;if(console.log("[PvP] init - amIHome:",n,"gameState exists:",!!u,"match.status:",c.status,"home_id:",c.home_id,"away_id:",c.away_id,"myId:",a.profile.id),!u)if(n){u=await z(),console.log("[PvP] home - gameState built, p1Team keys:",Object.keys((u==null?void 0:u.p1Team)||{}));const{data:$}=await T.from("matches").select("game_state").eq("id",i).single();!($!=null&&$.game_state)||!Object.keys($.game_state).length?await T.from("matches").update({game_state:u,turn_user_id:c.home_id}).eq("id",i):u=$.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>',console.log("[PvP] away - waiting for home to write game_state...");for(let $=0;$<30&&!u;$++){await new Promise(b=>setTimeout(b,400));const{data:g}=await T.from("matches").select("game_state").eq("id",i).single();g!=null&&g.game_state&&Object.keys(g.game_state).length&&(u=g.game_state),$%5===0&&console.log("[PvP] away - poll",$,"game_state:",!!(g!=null&&g.game_state))}if(!u){y("Erreur de synchronisation","error"),o("home");return}u.gc_p2=_,u.gcCardsFull_p2=f,await T.from("matches").update({game_state:u}).eq("id",i)}let I=!1,F=null,C=!1;const q=new Set,j=new Set;async function N($){var S,M;try{T.removeChannel(X)}catch{}const g=u[p+"Score"]||0,b=u[x+"Score"]||0;let h,v;$.winner_id?(h=$.winner_id===a.profile.id,v=!1):$.forfeit?(h=g>b,v=!1):(v=g===b,h=g>b);let w="";if(r&&n)try{const{data:D}=await T.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",c.home_id).single(),{data:V}=await T.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",c.away_id).single();if(D&&V){const W=h?1:v?.5:0,H=1-W,P=D.placement_matches<10,Y=V.placement_matches<10,te=ji(D.mmr,D.mmr_deviation,D.mmr_volatility,V.mmr,V.mmr_deviation,W===1?1:W===0?0:.5,P),Q=ji(V.mmr,V.mmr_deviation,V.mmr_volatility,D.mmr,D.mmr_deviation,H===1?1:H===0?0:.5,Y);await T.rpc("update_mmr_after_ranked",{p_match_id:i,p_winner_id:v?null:h?c.home_id:c.away_id,p_home_id:c.home_id,p_away_id:c.away_id,p_home_delta:te.delta,p_away_delta:Q.delta,p_home_new_rd:te.newRd,p_away_new_rd:Q.newRd,p_home_new_vol:te.newSigma,p_away_new_vol:Q.newSigma});const fe=te.delta,ye=te.newMmr,be=Pt(ye),ke=fe>=0?"+":"",Be=fe>=0?"#4caf50":"#ff6b6b",Fe=Pt(D.mmr);w=`
      ...
      ${w}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${r?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(k),(M=k.querySelector("#pvp-end-home"))==null||M.addEventListener("click",()=>{k.remove(),Ve(e),o(r?"ranked":"home")})}const X=T.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},$=>{const g=$.new;try{if(g.status==="finished"||g.forfeit){if(I)return;I=!0,F&&(clearInterval(F),F=null),g.game_state&&(u=g.game_state),N(g);return}if(g.game_state){const b=u;u=g.game_state;const h=u._lastGC;if(h&&h.seq&&!j.has(h.seq)&&(j.add(h.seq),h.by!==p)){se(h.type,h.by,()=>ge());return}const v=b[p+"Score"]||0,w=b[x+"Score"]||0,k=u[p+"Score"]||0,A=u[x+"Score"]||0,E=k>v,L=A>w;if((E||L)&&!q.has(u.round)){q.add(u.round);const S=[...u.log||[]].reverse().find(D=>D.isGoal),M=((S==null?void 0:S.homePlayers)||[]).map(D=>({name:D.name,note:D.note,portrait:D.portrait,job:D.job}));Ee(M,k,A,E,()=>ge());return}ge()}}catch(b){console.error("[PvP] crash:",b)}}).subscribe();async function K($){Object.assign(u,$),u.lastActionAt=new Date().toISOString();const{error:g}=await T.from("matches").update({game_state:u}).eq("id",i);g&&y("Erreur de synchronisation","error");try{ge()}catch(b){console.error("[PvP] renderPvpScreen crash:",b)}}async function re(){if(I)return;I=!0,F&&(clearInterval(F),F=null);const $=n?c.away_id:c.home_id,g=n?"p2":"p1",b=n?"p1":"p2",h={...u,[g+"Score"]:3,[b+"Score"]:0,phase:"finished"};try{await T.from("matches").update({status:"finished",forfeit:!0,winner_id:$,home_score:h.p1Score||0,away_score:h.p2Score||0,game_state:h}).eq("id",i)}catch{}try{T.removeChannel(X)}catch{}setTimeout(()=>{Ve(e),o("home")},800)}const oe={BOOST_STAT:({value:$=1,count:g=1,roles:b=[]},h,v)=>{const w=h[p+"Team"],k=Object.entries(w).filter(([A])=>!b.length||b.includes(A)).flatMap(([A,E])=>E.filter(L=>!L.used).map(L=>({...L,_line:A})));if(!k.length){h.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),v(h);return}Z(k,g,`Choisir ${g} joueur(s) à booster (+${$})`,A=>{A.forEach(E=>{const L=(w[E._line]||[]).find(S=>S.cardId===E.cardId);L&&(L.boost=(L.boost||0)+$,h.log.push({text:`⚡ +${$} sur ${L.name}`,type:"info"}))}),h[p+"Team"]=w,v(h)})},DEBUFF_STAT:({value:$=1,count:g=1,roles:b=[],target:h="ai"},v,w)=>{const k=h==="home"?p:x,A=v[k+"Team"],E=h==="home"?"allié":"adverse",L=Object.entries(A).filter(([S])=>!b.length||b.includes(S)).flatMap(([S,M])=>M.map(D=>({...D,_line:S})));if(!L.length){v.log.push({text:`🎯 Aucun joueur ${E}`,type:"info"}),w(v);return}Z(L,g,`Choisir ${g} joueur(s) ${E}(s) (-${$})`,S=>{S.forEach(M=>{const D=(A[M._line]||[]).find(V=>V.cardId===M.cardId);D&&(D.boost=(D.boost||0)-$,v.log.push({text:`🎯 -${$} sur ${D.name}`,type:"info"}))}),v[k+"Team"]=A,w(v)})},GRAY_PLAYER:({count:$=1,roles:g=[],target:b="ai"},h,v)=>{const w=b==="home"?p:x,k=h[w+"Team"],A=b==="home"?"allié":"adverse",E=Object.entries(k).filter(([L])=>!g.length||g.includes(L)).flatMap(([L,S])=>S.filter(M=>!M.used).map(M=>({...M,_line:L})));if(!E.length){h.log.push({text:`❌ Aucun joueur ${A}`,type:"info"}),v(h);return}Z(E,$,`Choisir ${$} joueur(s) ${A}(s) à exclure`,L=>{const S="usedCardIds_"+w,M=new Set(h[S]||[]);L.forEach(D=>{const V=(k[D._line]||[]).find(W=>W.cardId===D.cardId);V&&(V.used=!0,M.add(D.cardId),h.log.push({text:`❌ ${V.name} exclu !`,type:"info"}))}),h[S]=[...M],h[w+"Team"]=k,v(h)})},REVIVE_PLAYER:({count:$=1,roles:g=[]},b,h)=>{const v=b[p+"Team"],w=Object.entries(v).filter(([k])=>!g.length||g.includes(k)).flatMap(([k,A])=>A.filter(E=>E.used).map(E=>({...E,_line:k})));if(!w.length){b.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),h(b);return}Z(w,$,`Choisir ${$} joueur(s) à ressusciter`,k=>{k.forEach(A=>{const E=(v[A._line]||[]).find(L=>L.cardId===A.cardId);E&&(E.used=!1,b.log.push({text:`💫 ${E.name} ressuscité !`,type:"info"}))}),b[p+"Team"]=v,h(b)})},REMOVE_GOAL:({},$,g)=>{const b=x+"Score";$[b]>0?($[b]--,$.log.push({text:"🚫 Dernier but annulé !",type:"info"})):$.log.push({text:"🚫 Aucun but à annuler",type:"info"}),g($)},ADD_GOAL_DRAW:({},$,g)=>{$[p+"Score"]===$[x+"Score"]?($[p+"Score"]++,$.log.push({text:"🎯 But bonus !",type:"info"})):$.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),g($)},ADD_SUB:({value:$=1},g,b)=>{g.maxSubs=(g.maxSubs||3)+$,g.log.push({text:`🔄 +${$} remplacement(s)`,type:"info"}),b(g)},CUSTOM:({},$,g)=>g($)};function Z($,g,b,h){const v=document.createElement("div");v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let w=[];function k(){var E,L;const A=$.map(S=>{const M={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[S._line]||"#555",D=Te(S,S._line)+(S.boost||0),W=w.find(P=>P.cardId===S.cardId)?"#FFD700":"rgba(255,255,255,0.25)",H=S.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${S.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${W};background:${M};overflow:hidden;cursor:pointer;${H}">
        ...
        </div>`,(E=v.querySelector("#pp-close"))==null||E.addEventListener("click",()=>v.remove()),v.querySelectorAll(".pp-item").forEach(S=>{S.addEventListener("click",()=>{const M=S.dataset.cid,D=$.find(W=>W.cardId===M),V=w.findIndex(W=>W.cardId===M);D&&(V>-1?w.splice(V,1):w.length<g&&w.push(D),k())})}),(L=v.querySelector("#pp-confirm"))==null||L.addEventListener("click",()=>{v.remove(),h(w)})}k(),document.body.appendChild(v)}async function R($,g){const h=(u["gcCardsFull_"+p]||[]).find(A=>A.id===$),v=(h==null?void 0:h._gcDef)||(u.gcDefs||[]).find(A=>{var E;return A.name===g||((E=A.name)==null?void 0:E.toLowerCase().trim())===(g==null?void 0:g.toLowerCase().trim())}),w=[...u["usedGc_"+p]||[],$],k={type:g,by:p,seq:(u._gcAnimSeq||0)+1};j.add(k.seq),se(g,p,async()=>{if(v!=null&&v.effect_type&&v.effect_type!=="CUSTOM"){const E=oe[v.effect_type];if(E){const L={...u};E(v.effect_params||{},L,async S=>{S["usedGc_"+p]=w,S._lastGC=k,S._gcAnimSeq=k.seq,await K(S)});return}}const A={...u};switch(g){case"Remplacement+":A.maxSubs=(A.maxSubs||3)+1,A.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const E=x+"Score";A[E]>0&&(A[E]--,A.log.push({text:"🚫 But annulé",type:"info"}));break}}A["usedGc_"+p]=w,A._lastGC=k,A._gcAnimSeq=k.seq,await K(A)})}function G($,g){const b="usedCardIds_"+$,h=new Set(u[b]||[]);g.forEach(v=>h.add(v)),u[b]=[...h]}function ne(){for(const $ of["p1","p2"]){const g=new Set(u["usedCardIds_"+$]||[]),b=u[$+"Team"];if(b)for(const h of["GK","DEF","MIL","ATT"])(b[h]||[]).forEach(v=>{v.used=g.has(v.cardId)})}}function ge(){var We,ot,pt,ut,ft,ce;if(u.phase==="reveal")return _e();if(u.phase==="midfield")return ve();if(u.phase==="finished")return B();const $=u[p+"Team"],g=u[x+"Team"];ne();const b=u[p+"Score"],h=u[x+"Score"],v=u[p+"Name"],w=u[x+"Name"],k=u.phase===p+"-attack",A=u.phase===p+"-defense",E=Array.isArray(u["selected_"+p])?u["selected_"+p]:[],L=E.map(J=>J.cardId),S=window.innerWidth>=700,M=u[p+"Subs"]||[],D=u["usedSubIds_"+p]||[],V=M.filter(J=>!D.includes(J.cardId)),W=u["gcCardsFull_"+p]||[],H=u["usedGc_"+p]||[],P=W.filter(J=>!H.includes(J.id)),Y=u.boostOwner===p&&!u.boostUsed,te=!["GK","DEF","MIL","ATT"].some(J=>(g[J]||[]).some(O=>!O.used)),Q=[...$.MIL||[],...$.ATT||[]].filter(J=>!J.used),fe=k&&te&&Q.length===0?[...$.GK||[],...$.DEF||[]].filter(J=>!J.used).map(J=>J.cardId):[];function ye(J,O,ae){var _t,qt;const he=J._gcDef,$e={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[he==null?void 0:he.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",Ze={purple:"#b06ce0",light_blue:"#00d4ef"}[he==null?void 0:he.color]||"#b06ce0",Qe=(he==null?void 0:he.name)||J.gc_type,tt=(he==null?void 0:he.effect)||((_t=He[J.gc_type])==null?void 0:_t.desc)||"",rt=he!=null&&he.image_url?`/icons/${he.image_url}`:null,vt=((qt=He[J.gc_type])==null?void 0:qt.icon)||"⚡",wt=Math.round(ae*.22),dt=Math.round(ae*.22),gt=ae-wt-dt,mt=Qe.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${J.id}" data-gc-type="${J.gc_type}"
      ...
      </div>`}function be(J,O){return`<div id="pvp-boost-card"
        style="box-sizing:border-box;width:${J}px;height:${O}px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:2px solid #87CEEB;border-radius:10px;cursor:pointer;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:${Math.round(O*.04)}px;text-align:center;flex-shrink:0">
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${ct($,u[p+"Formation"],Re,L,300,300,fe)}
    ...
    </div>`;function Ye(J){if(J.type==="duel"&&(J.homeTotal!=null||J.aiTotal!=null)){const O=(J.homeTotal||0)>=(J.aiTotal||0);return`<div style="background:rgba(255,255,255,0.05);border-radius:8px;padding:5px 6px;border:1px solid rgba(255,255,255,0.08)">
          ...
          <div style="display:flex;align-items:center;gap:3px;max-height:46px;overflow:hidden">
            ...
            <div style="text-align:center;padding:0 6px;flex-shrink:0">
              <div style="font-size:${O?20:14}px;font-weight:900;color:${O?"#FFD700":"rgba(255,255,255,0.4)"};line-height:1">${J.homeTotal}</div>
              <div style="font-size:8px;color:rgba(255,255,255,0.3);margin:1px 0">VS</div>
              <div style="font-size:${O?14:20}px;font-weight:900;color:${O?"rgba(255,255,255,0.4)":"#ff6b6b"};line-height:1">${J.aiTotal}</div>
            </div>
            <div style="flex:1;display:flex;gap:2px;justify-content:flex-start;flex-wrap:nowrap;overflow-x:auto;overflow-y:hidden">
        ...
        </div>`}const J=(u.log||[]).slice(-1)[0];return J?'<div style="padding:2px 4px">'+Ye(J)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})(),Oe=`
      <div style="display:flex;align-items:center;padding:8px 10px;background:rgba(0,0,0,0.5);gap:6px;flex-shrink:0">
      ...
      </div>`;function Pe(){const J=e.querySelector(".match-screen");if(!J)return;const O=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);J.style.height=O+"px",J.style.minHeight=O+"px",J.style.maxHeight=O+"px",J.style.overflow="hidden";const ae=e.querySelector("#mobile-action-bar"),he=e.querySelector("#mobile-play-area");ae&&he&&(he.style.paddingBottom=ae.offsetHeight+"px")}if(Pe(),setTimeout(Pe,120),setTimeout(Pe,400),C||(C=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",Pe),window.visualViewport.addEventListener("scroll",Pe)),window.addEventListener("resize",Pe)),function(){const O=e.querySelector(".terrain-wrapper svg");O&&(O.removeAttribute("width"),O.removeAttribute("height"),O.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",O.setAttribute("viewBox","-26 -26 352 352"),O.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),e.querySelectorAll(".match-slot-hit").forEach(J=>{J.addEventListener("click",()=>{if(!k&&!A)return;const O=J.dataset.cardId,ae=J.dataset.role,he=($[ae]||[]).find(tt=>tt.cardId===O);if(!he||he.used)return;const $e=fe.includes(O);if(k&&!["MIL","ATT"].includes(ae)&&!$e)return;Array.isArray(u["selected_"+p])||(u["selected_"+p]=[]);const Ze=u["selected_"+p],Qe=Ze.findIndex(tt=>tt.cardId===O);Qe>-1?Ze.splice(Qe,1):Ze.length<3&&Ze.push({...he,_role:ae}),ge()})}),e.querySelectorAll(".match-used-hit").forEach(J=>{J.addEventListener("click",()=>U(J.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(J=>{J.addEventListener("click",()=>U())}),(We=e.querySelector("#pvp-sub-open"))==null||We.addEventListener("click",()=>U()),e.querySelectorAll(".pvp-gc-mini").forEach(J=>{J.addEventListener("click",()=>de(J.dataset.gcId,J.dataset.gcType))}),(ot=e.querySelector("#pvp-boost-card"))==null||ot.addEventListener("click",()=>ue()),(pt=e.querySelector("#pvp-action"))==null||pt.addEventListener("click",J=>{k?J.currentTarget.dataset.pass==="1"||!Ae(p)?me():ie():A&&pe()}),(ut=e.querySelector("#pvp-quit"))==null||ut.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&re()}),(ft=e.querySelector("#pvp-view-opp"))==null||ft.addEventListener("click",()=>xe()),(ce=e.querySelector("#pvp-toggle-history"))==null||ce.addEventListener("click",()=>le()),F&&(clearInterval(F),F=null),(k||A)&&!I){let J=30,O=!1;const ae=()=>document.getElementById("pvp-timer"),he=()=>{ae()&&(ae().textContent=J+"s",ae().style.color=O?"#ff4444":"#fff")};he(),F=setInterval(()=>{J--,J<0?O?(clearInterval(F),F=null,k&&!Ae(p)?me():re()):(O=!0,J=15,he()):he()},1e3)}}function _e(){et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
      ...
      <div style="width:min(90vw,420px)">${lt(u[x+"Team"],u[x+"Formation"],null,[],300,300)}</div>
    </div>`,p==="p1"&&setTimeout(async()=>{await K({phase:"midfield"})},5e3)}let we=!1;function ve(){if(we)return;const $=u[p+"Team"].MIL||[],g=u[x+"Team"].MIL||[];function b(P){return P.reduce((Y,te)=>Y+Te(te,"MIL"),0)}function h(P){let Y=0;for(let te=0;te<P.length-1;te++){const Q=nt(P[te],P[te+1]);Q==="#00ff88"?Y+=2:Q==="#FFD700"&&(Y+=1)}return Y}const v=b($)+h($),w=b(g)+h(g),k=v>=w;function A(P,Y,te){return`<div id="duel-row-${te}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
        ...
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${P.map((Q,fe)=>{const ye=fe<P.length-1?nt(Q,P[fe+1]):null,be=!ye||ye==="#ff3333"||ye==="#cc2222",ke=ye==="#00ff88"?"+2":ye==="#FFD700"?"+1":"";return`<div class="duel-card duel-card-${te}" data-idx="${fe}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease,transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
              ${Xe({...Q,note:Te(Q,"MIL"),_line:"MIL"},58,78)}
            </div>
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${g.map(v=>{const w={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[v._line]||"#555",k=Te(v,v._line)+(v.boost||0);return`<div class="bp-item" data-cid="${v.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${w};overflow:hidden;cursor:pointer">
      ...
      <div style="width:min(90vw,420px)">${lt(u[x+"Team"],u[x+"Formation"],null,[],300,300)}</div>
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild($),(g=$.querySelector("#pvp-opp-close"))==null||g.addEventListener("click",()=>$.remove())}function le(){var b;const $=u.log||[],g=document.createElement("div");g.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column",g.innerHTML=`
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(v),(L=v.querySelector("#pvp-end-home"))==null||L.addEventListener("click",()=>{v.remove(),Ve(e),l("home")})}const N=T.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},B=>{const $=B.new;try{if($.status==="finished"||$.forfeit){if(c)return;c=!0,I&&(clearInterval(I),I=null),$.game_state&&(f=$.game_state),j($);return}if($.game_state){const g=f;f=$.game_state;const b=f._lastGC;if(b&&b.seq&&!q.has(b.seq)&&(q.add(b.seq),b.by!==a)){Ee(b.type,b.by,()=>ne());return}const h=g[a+"Score"]||0,v=g[o+"Score"]||0,w=f[a+"Score"]||0,k=f[o+"Score"]||0,A=w>h,E=k>v;if((A||E)&&!C.has(f.round)){C.add(f.round);const L=[...f.log||[]].reverse().find(M=>M.isGoal),S=((L==null?void 0:L.homePlayers)||[]).map(M=>({name:M.name,note:M.note,portrait:M.portrait,job:M.job}));ve(S,w,k,A,()=>ne());return}ne()}}catch(g){console.error("[PvP] crash:",g)}}).subscribe();async function X(B){Object.assign(f,B),f.lastActionAt=new Date().toISOString();const{error:$}=await T.from("matches").update({game_state:f}).eq("id",i);$&&m("Erreur de synchronisation","error");try{ne()}catch(g){console.error("[PvP] renderPvpScreen crash:",g)}}async function K(){if(c)return;c=!0,I&&(clearInterval(I),I=null);const B=n?x.away_id:x.home_id,$=n?"p2":"p1",g=n?"p1":"p2",b={...f,[$+"Score"]:3,[g+"Score"]:0,phase:"finished"};try{await T.from("matches").update({status:"finished",forfeit:!0,winner_id:B,home_score:b.p1Score||0,away_score:b.p2Score||0,game_state:b}).eq("id",i)}catch{}try{T.removeChannel(N)}catch{}setTimeout(()=>{Ve(e),l("home")},800)}const re={BOOST_STAT:({value:B=1,count:$=1,roles:g=[]},b,h)=>{const v=b[a+"Team"],w=Object.entries(v).filter(([k])=>!g.length||g.includes(k)).flatMap(([k,A])=>A.filter(E=>!E.used).map(E=>({...E,_line:k})));if(!w.length){b.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),h(b);return}oe(w,$,`Choisir ${$} joueur(s) à booster (+${B})`,k=>{k.forEach(A=>{const E=(v[A._line]||[]).find(L=>L.cardId===A.cardId);E&&(E.boost=(E.boost||0)+B,b.log.push({text:`⚡ +${B} sur ${E.name}`,type:"info"}))}),b[a+"Team"]=v,h(b)})},DEBUFF_STAT:({value:B=1,count:$=1,roles:g=[],target:b="ai"},h,v)=>{const w=b==="home"?a:o,k=h[w+"Team"],A=b==="home"?"allié":"adverse",E=Object.entries(k).filter(([L])=>!g.length||g.includes(L)).flatMap(([L,S])=>S.map(M=>({...M,_line:L})));if(!E.length){h.log.push({text:`🎯 Aucun joueur ${A}`,type:"info"}),v(h);return}oe(E,$,`Choisir ${$} joueur(s) ${A}(s) (-${B})`,L=>{L.forEach(S=>{const M=(k[S._line]||[]).find(D=>D.cardId===S.cardId);M&&(M.boost=(M.boost||0)-B,h.log.push({text:`🎯 -${B} sur ${M.name}`,type:"info"}))}),h[w+"Team"]=k,v(h)})},GRAY_PLAYER:({count:B=1,roles:$=[],target:g="ai"},b,h)=>{const v=g==="home"?a:o,w=b[v+"Team"],k=g==="home"?"allié":"adverse",A=Object.entries(w).filter(([E])=>!$.length||$.includes(E)).flatMap(([E,L])=>L.filter(S=>!S.used).map(S=>({...S,_line:E})));if(!A.length){b.log.push({text:`❌ Aucun joueur ${k}`,type:"info"}),h(b);return}oe(A,B,`Choisir ${B} joueur(s) ${k}(s) à exclure`,E=>{const L="usedCardIds_"+v,S=new Set(b[L]||[]);E.forEach(M=>{const D=(w[M._line]||[]).find(V=>V.cardId===M.cardId);D&&(D.used=!0,S.add(M.cardId),b.log.push({text:`❌ ${D.name} exclu !`,type:"info"}))}),b[L]=[...S],b[v+"Team"]=w,h(b)})},REVIVE_PLAYER:({count:B=1,roles:$=[]},g,b)=>{const h=g[a+"Team"],v=Object.entries(h).filter(([w])=>!$.length||$.includes(w)).flatMap(([w,k])=>k.filter(A=>A.used).map(A=>({...A,_line:w})));if(!v.length){g.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),b(g);return}oe(v,B,`Choisir ${B} joueur(s) à ressusciter`,w=>{w.forEach(k=>{const A=(h[k._line]||[]).find(E=>E.cardId===k.cardId);A&&(A.used=!1,g.log.push({text:`💫 ${A.name} ressuscité !`,type:"info"}))}),g[a+"Team"]=h,b(g)})},REMOVE_GOAL:({},B,$)=>{const g=o+"Score";B[g]>0?(B[g]--,B.log.push({text:"🚫 Dernier but annulé !",type:"info"})):B.log.push({text:"🚫 Aucun but à annuler",type:"info"}),$(B)},ADD_GOAL_DRAW:({},B,$)=>{B[a+"Score"]===B[o+"Score"]?(B[a+"Score"]++,B.log.push({text:"🎯 But bonus !",type:"info"})):B.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),$(B)},ADD_SUB:({value:B=1},$,g)=>{$.maxSubs=($.maxSubs||3)+B,$.log.push({text:`🔄 +${B} remplacement(s)`,type:"info"}),g($)},CUSTOM:({},B,$)=>$(B)};function oe(B,$,g,b){const h=document.createElement("div");h.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let v=[];function w(){var A,E;const k=B.map(L=>{const S={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[L._line]||"#555",M=Te(L,L._line)+(L.boost||0),V=v.find(H=>H.cardId===L.cardId)?"#FFD700":"rgba(255,255,255,0.25)",W=L.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${L.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${V};background:${S};overflow:hidden;cursor:pointer;${W}">
        ...
        </div>`,(A=h.querySelector("#pp-close"))==null||A.addEventListener("click",()=>h.remove()),h.querySelectorAll(".pp-item").forEach(L=>{L.addEventListener("click",()=>{const S=L.dataset.cid,M=B.find(V=>V.cardId===S),D=v.findIndex(V=>V.cardId===S);M&&(D>-1?v.splice(D,1):v.length<$&&v.push(M),w())})}),(E=h.querySelector("#pp-confirm"))==null||E.addEventListener("click",()=>{h.remove(),b(v)})}w(),document.body.appendChild(h)}async function Z(B,$){const b=(f["gcCardsFull_"+a]||[]).find(k=>k.id===B),h=(b==null?void 0:b._gcDef)||(f.gcDefs||[]).find(k=>{var A;return k.name===$||((A=k.name)==null?void 0:A.toLowerCase().trim())===($==null?void 0:$.toLowerCase().trim())}),v=[...f["usedGc_"+a]||[],B],w={type:$,by:a,seq:(f._gcAnimSeq||0)+1};q.add(w.seq),Ee($,a,async()=>{if(h!=null&&h.effect_type&&h.effect_type!=="CUSTOM"){const A=re[h.effect_type];if(A){const E={...f};A(h.effect_params||{},E,async L=>{L["usedGc_"+a]=v,L._lastGC=w,L._gcAnimSeq=w.seq,await X(L)});return}}const k={...f};switch($){case"Remplacement+":k.maxSubs=(k.maxSubs||3)+1,k.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const A=o+"Score";k[A]>0&&(k[A]--,k.log.push({text:"🚫 But annulé",type:"info"}));break}}k["usedGc_"+a]=v,k._lastGC=w,k._gcAnimSeq=w.seq,await X(k)})}function R(B,$){const g="usedCardIds_"+B,b=new Set(f[g]||[]);$.forEach(h=>b.add(h)),f[g]=[...b]}function G(){for(const B of["p1","p2"]){const $=new Set(f["usedCardIds_"+B]||[]),g=f[B+"Team"];if(g)for(const b of["GK","DEF","MIL","ATT"])(g[b]||[]).forEach(h=>{h.used=$.has(h.cardId)})}}function ne(){var Pe,We,ot,pt,ut,ft;if(f.phase==="reveal")return ge();if(f.phase==="midfield")return we();if(f.phase==="finished")return Se();const B=f[a+"Team"],$=f[o+"Team"];G();const g=f[a+"Score"],b=f[o+"Score"],h=f[a+"Name"],v=f[o+"Name"],w=f.phase===a+"-attack",k=f.phase===a+"-defense",A=Array.isArray(f["selected_"+a])?f["selected_"+a]:[],E=A.map(ce=>ce.cardId),L=window.innerWidth>=700,S=f[a+"Subs"]||[],M=f["usedSubIds_"+a]||[],D=S.filter(ce=>!M.includes(ce.cardId)),V=f["gcCardsFull_"+a]||[],W=f["usedGc_"+a]||[],H=V.filter(ce=>!W.includes(ce.id)),P=f.boostOwner===a&&!f.boostUsed,Y=!["GK","DEF","MIL","ATT"].some(ce=>($[ce]||[]).some(J=>!J.used)),te=[...B.MIL||[],...B.ATT||[]].filter(ce=>!ce.used),Q=w&&Y&&te.length===0?[...B.GK||[],...B.DEF||[]].filter(ce=>!ce.used).map(ce=>ce.cardId):[];function fe(ce,J,O){var mt,_t;const ae=ce._gcDef,he={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ae==null?void 0:ae.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",$e={purple:"#b06ce0",light_blue:"#00d4ef"}[ae==null?void 0:ae.color]||"#b06ce0",Ze=(ae==null?void 0:ae.name)||ce.gc_type,Qe=(ae==null?void 0:ae.effect)||((mt=He[ce.gc_type])==null?void 0:mt.desc)||"",tt=ae!=null&&ae.image_url?`/icons/${ae.image_url}`:null,rt=((_t=He[ce.gc_type])==null?void 0:_t.icon)||"⚡",vt=Math.round(O*.22),wt=Math.round(O*.22),dt=O-vt-wt,gt=Ze.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${ce.id}" data-gc-type="${ce.gc_type}"
        style="box-sizing:border-box;width:${J}px;height:${O}px;border-radius:10px;border:2px solid ${$e};background:${he};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
        <div style="height:${vt}px;background:rgba(255,255,255,0.14);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 3px">
          <span style="font-size:${gt}px;font-weight:900;color:#fff;text-align:center;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${J-6}px">${Ze}</span>
          <span style="font-size:6px;color:rgba(255,255,255,0.45)">⚡ GC</span>
        </div>
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ...
        </div>`}return`<div style="font-size:11px;color:${ce.type==="goal"?"#FFD700":"rgba(255,255,255,0.65)"};font-weight:${ce.type==="goal"?700:400};padding:3px 2px">${ce.text||""}</div>`}const Ye=(()=>{var J,O;if(k&&((J=f.pendingAttack)!=null&&J.players)){const ae=f.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
      ...
      </div>`;function Oe(){const ce=e.querySelector(".match-screen");if(!ce)return;const J=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);ce.style.height=J+"px",ce.style.minHeight=J+"px",ce.style.maxHeight=J+"px",ce.style.overflow="hidden";const O=e.querySelector("#mobile-action-bar"),ae=e.querySelector("#mobile-play-area");O&&ae&&(ae.style.paddingBottom=O.offsetHeight+"px")}if(Oe(),setTimeout(Oe,120),setTimeout(Oe,400),F||(F=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",Oe),window.visualViewport.addEventListener("scroll",Oe)),window.addEventListener("resize",Oe)),function(){const J=e.querySelector(".terrain-wrapper svg");J&&(J.removeAttribute("width"),J.removeAttribute("height"),J.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",J.setAttribute("viewBox","-26 -26 352 352"),J.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),e.querySelectorAll(".match-slot-hit").forEach(ce=>{ce.addEventListener("click",()=>{if(!w&&!k)return;const J=ce.dataset.cardId,O=ce.dataset.role,ae=(B[O]||[]).find(Qe=>Qe.cardId===J);if(!ae||ae.used)return;const he=Q.includes(J);if(w&&!["MIL","ATT"].includes(O)&&!he)return;Array.isArray(f["selected_"+a])||(f["selected_"+a]=[]);const $e=f["selected_"+a],Ze=$e.findIndex(Qe=>Qe.cardId===J);Ze>-1?$e.splice(Ze,1):$e.length<3&&$e.push({...ae,_role:O}),ne()})}),e.querySelectorAll(".match-used-hit").forEach(ce=>{ce.addEventListener("click",()=>ue(ce.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(ce=>{ce.addEventListener("click",()=>ue())}),(Pe=e.querySelector("#pvp-sub-open"))==null||Pe.addEventListener("click",()=>ue()),e.querySelectorAll(".pvp-gc-mini").forEach(ce=>{ce.addEventListener("click",()=>se(ce.dataset.gcId,ce.dataset.gcType))}),(We=e.querySelector("#pvp-boost-card"))==null||We.addEventListener("click",()=>de()),(ot=e.querySelector("#pvp-action"))==null||ot.addEventListener("click",ce=>{w?ce.currentTarget.dataset.pass==="1"||!Ce(a)?le():me():k&&ie()}),(pt=e.querySelector("#pvp-quit"))==null||pt.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&K()}),(ut=e.querySelector("#pvp-view-opp"))==null||ut.addEventListener("click",()=>ee()),(ft=e.querySelector("#pvp-toggle-history"))==null||ft.addEventListener("click",()=>xe()),I&&(clearInterval(I),I=null),(w||k)&&!c){let ce=30,J=!1;const O=()=>document.getElementById("pvp-timer"),ae=()=>{O()&&(O().textContent=ce+"s",O().style.color=J?"#ff4444":"#fff")};ae(),I=setInterval(()=>{ce--,ce<0?J?(clearInterval(I),I=null,w&&!Ce(a)?le():K()):(J=!0,ce=15,ae()):ae()},1e3)}}function ge(){et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
      ...
      <div style="width:min(90vw,420px)">${lt(f[o+"Team"],f[o+"Formation"],null,[],300,300)}</div>
    </div>`,a==="p1"&&setTimeout(async()=>{await X({phase:"midfield"})},5e3)}let _e=!1;function we(){if(_e)return;const B=f[a+"Team"].MIL||[],$=f[o+"Team"].MIL||[];function g(H){return H.reduce((P,Y)=>P+Te(Y,"MIL"),0)}function b(H){let P=0;for(let Y=0;Y<H.length-1;Y++){const te=nt(H[Y],H[Y+1]);te==="#00ff88"?P+=2:te==="#FFD700"&&(P+=1)}return P}const h=g(B)+b(B),v=g($)+b($),w=h>=v;function k(H,P,Y){return`<div id="duel-row-${Y}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
        ...
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${H.map((te,Q)=>{const fe=Q<H.length-1?nt(te,H[Q+1]):null,ye=!fe||fe==="#ff3333"||fe==="#cc2222",be=fe==="#00ff88"?"+2":fe==="#FFD700"?"+1":"";return`<div class="duel-card duel-card-${Y}" data-idx="${Q}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease,transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
              ${Xe({...te,note:Te(te,"MIL"),_line:"MIL"},58,78)}
            </div>
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="display:flex;gap:12px;width:190px">
        <button id="pvp-gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
        <button id="pvp-gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
      </div>`,document.body.appendChild(S),(V=S.querySelector("#pvp-gc-back"))==null||V.addEventListener("click",()=>S.remove()),(W=S.querySelector("#pvp-gc-use"))==null||W.addEventListener("click",()=>{S.remove(),Z(B,$)})}function de(){var b;const B=f[a+"Team"],$=Object.entries(B).flatMap(([h,v])=>(v||[]).filter(w=>!w.used).map(w=>({...w,_line:h})));if(!$.length)return;const g=document.createElement("div");g.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",g.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
        <div style="flex:1;font-size:14px;font-weight:700;color:#fff">⚡ Choisir un joueur pour +${f.boostValue}</div>
        <button id="bp-close" style="background:none;border:none;color:rgba(255,255,255,0.5);font-size:22px;cursor:pointer">✕</button>
      </div>
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${$.map(h=>{const v={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[h._line]||"#555",w=Te(h,h._line)+(h.boost||0);return`<div class="bp-item" data-cid="${h.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${v};overflow:hidden;cursor:pointer">
      ...
      </div>`,document.body.appendChild($),(b=$.querySelector("#pvp-hist-close"))==null||b.addEventListener("click",()=>$.remove())}async function le(){if(f.phase!==a+"-attack")return;const B=a==="p1"?"p2":"p1",$=(f.round||0)+1,g=[...f.log||[]];g.push({type:"info",text:`⏭️ ${f[a+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const b=Ae(f),h=Ce(B),v=b||!h?"finished":B+"-attack";await X({["selected_"+a]:[],modifiers:{...f.modifiers,[a]:{}},pendingAttack:null,phase:v,attacker:B,round:$,log:g}),v==="finished"&&await Ge(f)}async function me(){const B=f[a+"Team"],$=!["GK","DEF","MIL","ATT"].some(v=>(f[o+"Team"][v]||[]).some(w=>!w.used)),g=(f["selected_"+a]||[]).map(v=>{const w=(B[v._role]||[]).find(M=>M.cardId===v.cardId)||v,k=$&&["GK","DEF"].includes(v._role),A=B[v._role]||[],E=A.findIndex(M=>M.cardId===v.cardId),L=st(A.length),S=E>=0?L[E]:w._col??1;return{...w,_line:v._role,_col:S,...k?{note_a:Math.max(1,Number(w.note_a)||0)}:{}}});if(!g.length)return;const b=Ht(g,f.modifiers[a]||{});R(a,g.map(v=>v.cardId)),g.forEach(v=>{const w=(B[v._role]||[]).find(k=>k.cardId===v.cardId);w&&(w.used=!0)}),f["selected_"+a]=[],ne();const h=[...f.log||[]];if($){const v=(f[a+"Score"]||0)+1,w=g.map(S=>({name:S.name,note:Te(S,S._line||"ATT"),portrait:Ne(S),job:S.job}));h.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:w,homeTotal:b.total,aiTotal:0});const k=(f.round||0)+1,A=a==="p1"?"p2":"p1",E={...f,[a+"Team"]:B,[a+"Score"]:v},L=Ae(E);C.add(k),ve(w,v,f[o+"Score"]||0,!0,async()=>{await X({[a+"Team"]:B,[a+"Score"]:v,["selected_"+a]:[],modifiers:{...f.modifiers,[a]:{}},pendingAttack:null,phase:L?"finished":A+"-attack",attacker:A,round:k,log:h}),L&&await Ge({...f,[a+"Score"]:v})});return}h.push({type:"pending",text:`⚔️ ${f[a+"Name"]} attaque (${b.total})`}),await X({[a+"Team"]:B,[o+"Team"]:f[o+"Team"],pendingAttack:{...b,players:g,side:a},["selected_"+a]:[],modifiers:{...f.modifiers,[a]:{}},phase:o+"-defense",log:h})}async function ie(){const B=f[a+"Team"],$=(f["selected_"+a]||[]).map(W=>{const H=(B[W._role]||[]).find(fe=>fe.cardId===W.cardId)||W,P=B[W._role]||[],Y=P.findIndex(fe=>fe.cardId===W.cardId),te=st(P.length),Q=Y>=0?te[Y]:H._col??1;return{...H,_line:W._role,_col:Q}}),g=Ut($,f.modifiers[a]||{});R(a,$.map(W=>W.cardId)),$.forEach(W=>{const H=(B[W._role]||[]).find(P=>P.cardId===W.cardId);H&&(H.used=!0)}),f["selected_"+a]=[],ne();const b=Kt(f.pendingAttack.total,g.total,f.modifiers[a]||{}),h=f.pendingAttack.side,v=b==="attack"||(b==null?void 0:b.goal),w=h==="p1"?"p2":"p1",k=(f.round||0)+1,A=(f.pendingAttack.players||[]).map(W=>({name:W.name,note:Te(W,W._line||"ATT"),portrait:Ne(W),job:W.job})),E=[...f.log||[]];E.push({type:"duel",isGoal:v,homeScored:v&&h===a,text:v?`⚽ BUT de ${f[h+"Name"]} ! (${f.pendingAttack.total} vs ${g.total})`:`✋ Attaque stoppée (${f.pendingAttack.total} vs ${g.total})`,homePlayers:A,aiPlayers:$.map(W=>({name:W.name,note:Te(W,W._line||"DEF"),portrait:Ne(W),job:W.job})),homeTotal:f.pendingAttack.total,aiTotal:g.total});const L=v?(f[h+"Score"]||0)+1:f[h+"Score"]||0,S={...f,[a+"Team"]:B,[h+"Score"]:L},M=Ae(S),D=M?"finished":w+"-attack",V=async()=>{await X({[a+"Team"]:B,[o+"Team"]:f[o+"Team"],[h+"Score"]:L,["selected_"+a]:[],modifiers:{...f.modifiers,[a]:{}},pendingAttack:null,phase:D,attacker:w,round:k,log:E}),(D==="finished"||M)&&await Ge({...f,[h+"Score"]:L})};if(v){const W=h===a,H=W?L:f[a+"Score"]||0,P=W?f[o+"Score"]||0:L;C.add(k),ve(A,H,P,W,V)}else await V()}function pe(B){return["MIL","ATT"].some($=>(B[$]||[]).some(g=>!g.used))}function Le(B){return["GK","DEF","MIL","ATT"].some($=>(B[$]||[]).some(g=>!g.used))}function Ie(B){return Le(B)&&!pe(B)}function Ce(B){const $=f[B+"Team"],g=f[(B==="p1"?"p2":"p1")+"Team"];return!!(pe($)||!Le(g)&&Ie($))}function Ae(B){const $=["MIL","ATT"].some(E=>(B.p1Team[E]||[]).some(L=>!L.used)),g=["MIL","ATT"].some(E=>(B.p2Team[E]||[]).some(L=>!L.used)),b=Le(B.p1Team),h=Le(B.p2Team);return!$&&!(!h&&b)&&(!g&&!(!b&&h))}function qe(B){const $=B.p1Score||0,g=B.p2Score||0;return $===g?null:$>g?x.home_id:x.away_id}async function Ge(B){try{const $=qe(B),g=$?x.home_id===$?x.away_id:x.home_id:null;await T.from("matches").update({status:"finished",winner_id:$,home_score:B.p1Score||0,away_score:B.p2Score||0}).eq("id",i),$&&g&&zi($,g).catch(()=>{})}catch($){console.error("[PvP] finishMatch:",$)}}function Se(){var h;const B=f[a+"Score"],$=f[o+"Score"],g=B>$,b=B===$;et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:10px">
      ${i==="waiting"?sr(p,f,d):i==="active"?dr(x,d):lr(_,d)}
    </div>
  </div>`,(z=document.getElementById("ml-create-btn"))==null||z.addEventListener("click",()=>cr(e,t)),e.querySelectorAll(".ml-tab").forEach(u=>u.addEventListener("click",()=>At(e,t,u.dataset.tab))),e.querySelectorAll("[data-league-id]").forEach(u=>u.addEventListener("click",()=>ht(e,t,u.dataset.leagueId))),e.querySelectorAll("[data-join]").forEach(u=>u.addEventListener("click",I=>{I.stopPropagation(),Eo(e,t,u.dataset.join,u.dataset.type)})),e.querySelectorAll("[data-delete]").forEach(u=>u.addEventListener("click",I=>{I.stopPropagation(),_i(e,t,u.dataset.delete,u.dataset.name,i)}))}function oi(e,t,i=!1){var r,l;const n=e.creator_id===t,d=e.pot||0,s=((l=(r=e.mini_league_members)==null?void 0:r[0])==null?void 0:l.count)||0;return`<div data-league-id="${e.id}" style="background:#fff;border-radius:12px;padding:14px 16px;box-shadow:0 1px 6px rgba(0,0,0,0.08);cursor:pointer;margin-bottom:8px">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        ${Array.from({length:Math.max(0,r.status==="active"?r.current_day-1:r.current_day)},(Z,R)=>R+1).reverse().map(Z=>{const R=(m||[]).filter(G=>G.matchday===Z);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:#888;margin-bottom:6px">Journée ${Z}</div>${R.map(G=>Yi(G,y,s,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
    ...
    <div style="background:linear-gradient(160deg,#1a0a2e,#3b1a6e);border-radius:20px;padding:32px 24px;max-width:320px;width:100%;text-align:center;color:#fff;box-shadow:0 8px 40px rgba(0,0,0,0.6)">
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">🏆 Retour à la Mini League</button>`,document.body.appendChild(k),(M=k.querySelector("#pvp-end-home"))==null||M.addEventListener("click",()=>{k.remove(),Ve(e),a?l("mini-league",{openLeagueId:a}):l("mini-league")})}const X=T.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},g=>{const b=g.new;try{if(b.status==="finished"||b.forfeit){if(z||(z=!0,F&&(clearInterval(F),F=null),b.game_state&&(c=b.game_state),c.phase==="finished"&&!b.forfeit&&document.getElementById("pvp-home")))return;N(b);return}if(b.game_state){const h=c;c=b.game_state;const v=c._lastGC;if(v&&v.seq&&!j.has(v.seq)&&(j.add(v.seq),v.by!==o)){se(v.type,v.by,()=>ge());return}const w=h[o+"Score"]||0,k=h[y+"Score"]||0,A=c[o+"Score"]||0,E=c[y+"Score"]||0,L=A>w,S=E>k;if((L||S)&&!q.has(c.round)){q.add(c.round);const M=[...c.log||[]].reverse().find(V=>V.isGoal),D=((M==null?void 0:M.homePlayers)||[]).map(V=>({name:V.name,note:V.note,portrait:V.portrait,job:V.job}));Ee(D,A,E,L,()=>ge());return}ge()}}catch(h){console.error("[PvP] crash:",h)}}).subscribe();async function K(g){Object.assign(c,g),c.lastActionAt=new Date().toISOString();const{error:b}=await T.from("matches").update({game_state:c}).eq("id",i);b&&m("Erreur de synchronisation","error");try{ge()}catch(h){console.error("[PvP] renderPvpScreen crash:",h)}}async function re(){if(z)return;z=!0,F&&(clearInterval(F),F=null);const g=n?_.away_id:_.home_id,b=n?"p2":"p1",h=n?"p1":"p2",v={...c,[b+"Score"]:3,[h+"Score"]:0,phase:"finished"};try{await T.from("matches").update({status:"finished",forfeit:!0,winner_id:g,home_score:v.p1Score||0,away_score:v.p2Score||0,game_state:v}).eq("id",i)}catch{}try{T.removeChannel(X)}catch{}setTimeout(()=>{Ve(e),l("mini-league")},800)}const oe={BOOST_STAT:({value:g=1,count:b=1,roles:h=[]},v,w)=>{const k=v[o+"Team"],A=Object.entries(k).filter(([E])=>!h.length||h.includes(E)).flatMap(([E,L])=>L.filter(S=>!S.used).map(S=>({...S,_line:E})));if(!A.length){v.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),w(v);return}Z(A,b,`Choisir ${b} joueur(s) à booster (+${g})`,E=>{E.forEach(L=>{const S=(k[L._line]||[]).find(M=>M.cardId===L.cardId);S&&(S.boost=(S.boost||0)+g,v.log.push({text:`⚡ +${g} sur ${S.name}`,type:"info"}))}),v[o+"Team"]=k,w(v)})},DEBUFF_STAT:({value:g=1,count:b=1,roles:h=[],target:v="ai"},w,k)=>{const A=v==="home"?o:y,E=w[A+"Team"],L=v==="home"?"allié":"adverse",S=Object.entries(E).filter(([M])=>!h.length||h.includes(M)).flatMap(([M,D])=>D.map(V=>({...V,_line:M})));if(!S.length){w.log.push({text:`🎯 Aucun joueur ${L}`,type:"info"}),k(w);return}Z(S,b,`Choisir ${b} joueur(s) ${L}(s) (-${g})`,M=>{M.forEach(D=>{const V=(E[D._line]||[]).find(W=>W.cardId===D.cardId);V&&(V.boost=(V.boost||0)-g,w.log.push({text:`🎯 -${g} sur ${V.name}`,type:"info"}))}),w[A+"Team"]=E,k(w)})},GRAY_PLAYER:({count:g=1,roles:b=[],target:h="ai"},v,w)=>{const k=h==="home"?o:y,A=v[k+"Team"],E=h==="home"?"allié":"adverse",L=Object.entries(A).filter(([S])=>!b.length||b.includes(S)).flatMap(([S,M])=>M.filter(D=>!D.used).map(D=>({...D,_line:S})));if(!L.length){v.log.push({text:`❌ Aucun joueur ${E}`,type:"info"}),w(v);return}Z(L,g,`Choisir ${g} joueur(s) ${E}(s) à exclure`,S=>{const M="usedCardIds_"+k,D=new Set(v[M]||[]);S.forEach(V=>{const W=(A[V._line]||[]).find(H=>H.cardId===V.cardId);W&&(W.used=!0,D.add(V.cardId),v.log.push({text:`❌ ${W.name} exclu !`,type:"info"}))}),v[M]=[...D],v[k+"Team"]=A,w(v)})},REVIVE_PLAYER:({count:g=1,roles:b=[]},h,v)=>{const w=h[o+"Team"],k=Object.entries(w).filter(([A])=>!b.length||b.includes(A)).flatMap(([A,E])=>E.filter(L=>L.used).map(L=>({...L,_line:A})));if(!k.length){h.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),v(h);return}Z(k,g,`Choisir ${g} joueur(s) à ressusciter`,A=>{A.forEach(E=>{const L=(w[E._line]||[]).find(S=>S.cardId===E.cardId);L&&(L.used=!1,h.log.push({text:`💫 ${L.name} ressuscité !`,type:"info"}))}),h[o+"Team"]=w,v(h)})},REMOVE_GOAL:({},g,b)=>{const h=y+"Score";g[h]>0?(g[h]--,g.log.push({text:"🚫 Dernier but annulé !",type:"info"})):g.log.push({text:"🚫 Aucun but à annuler",type:"info"}),b(g)},ADD_GOAL_DRAW:({},g,b)=>{g[o+"Score"]===g[y+"Score"]?(g[o+"Score"]++,g.log.push({text:"🎯 But bonus !",type:"info"})):g.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),b(g)},ADD_SUB:({value:g=1},b,h)=>{b.maxSubs=(b.maxSubs||3)+g,b.log.push({text:`🔄 +${g} remplacement(s)`,type:"info"}),h(b)},CUSTOM:({},g,b)=>b(g)};function Z(g,b,h,v){const w=document.createElement("div");w.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let k=[];function A(){var L,S;const E=g.map(M=>{const D={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[M._line]||"#555",V=Te(M,M._line)+(M.boost||0),H=k.find(Y=>Y.cardId===M.cardId)?"#FFD700":"rgba(255,255,255,0.25)",P=M.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${M.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${H};background:${D};overflow:hidden;cursor:pointer;${P}">
      ...
      <div style="width:min(90vw,420px)">${lt(c[y+"Team"],c[y+"Formation"],null,[],300,300)}</div>
    </div>`,o==="p1"&&setTimeout(async()=>{await K({phase:"midfield"})},5e3)}let we=!1;function ve(){if(we)return;const g=c[o+"Team"].MIL||[],b=c[y+"Team"].MIL||[];function h(Y){return Y.reduce((te,Q)=>te+Te(Q,"MIL"),0)}function v(Y){let te=0;for(let Q=0;Q<Y.length-1;Q++){const fe=nt(Y[Q],Y[Q+1]);fe==="#00ff88"?te+=2:fe==="#FFD700"&&(te+=1)}return te}const w=h(g)+v(g),k=h(b)+v(b),A=w>=k;function E(Y,te,Q){return`<div id="duel-row-${Q}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
        ...
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${Y.map((fe,ye)=>{const be=ye<Y.length-1?nt(fe,Y[ye+1]):null,ke=!be||be==="#ff3333"||be==="#cc2222",Be=be==="#00ff88"?"+2":be==="#FFD700"?"+1":"";return`<div class="duel-card duel-card-${Q}" data-idx="${ye}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease,transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
              ${Xe({...fe,note:Te(fe,"MIL"),_line:"MIL"},58,78)}
            </div>
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${b.map(w=>{const k={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[w._line]||"#555",A=Te(w,w._line)+(w.boost||0);return`<div class="bp-item" data-cid="${w.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${k};overflow:hidden;cursor:pointer">
    ...
    </div>`,(w=document.getElementById("pvp-home"))==null||w.addEventListener("click",()=>{try{T.removeChannel(X)}catch{}Ve(e),l("mini-league",a?{openLeagueId:a}:{})})}ge()}const vr="/",wr=[{emoji:"⚽",title:"Bienvenue dans Manager Wars !",color:"#1A6B3C",content:`<p>Tu es désormais un <strong>manager de football</strong> virtuel.</p>
    ...
    <p style="margin-top:12px;font-size:13px;color:#888">Tu peux revoir ce tutoriel depuis les paramètres à tout moment.</p>`}];function _r(e,t,i){let n=0;const d=document.createElement("div");d.id="tutorial-overlay",d.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.65);z-index:9900;display:flex;align-items:center;justify-content:center;padding:16px";const s=()=>{var o,y,p;const l=t[n],m=n===t.length-1,a=Math.round((n+1)/t.length*100);d.innerHTML=`
      <div style="background:#fff;border-radius:20px;width:100%;max-width:420px;max-height:90vh;display:flex;flex-direction:column;overflow:hidden;box-shadow:0 16px 64px rgba(0,0,0,0.4)">
        ...
        <!-- Image optionnelle -->
        ${l.image_url?`<div style="padding:0 24px 8px;text-align:center"><img src="${vr}icons/${l.image_url}" style="max-height:160px;max-width:100%;border-radius:12px;object-fit:contain"></div>`:""}
        <!-- Contenu -->
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
  ...
  </div>`;let m="buy";const a=()=>{var f,c,z,u,I,F,C;return{name:(((f=document.getElementById("flt-name"))==null?void 0:f.value)||"").toLowerCase().trim(),club:(((c=document.getElementById("flt-club"))==null?void 0:c.value)||"").toLowerCase().trim(),country:(((z=document.getElementById("flt-country"))==null?void 0:z.value)||"").toLowerCase().trim(),job:((u=document.getElementById("flt-job"))==null?void 0:u.value)||"",rarity:((I=document.getElementById("flt-rarity"))==null?void 0:I.value)||"",note1:parseInt((F=document.getElementById("flt-note1"))==null?void 0:F.value)||0,note2:parseInt((C=document.getElementById("flt-note2"))==null?void 0:C.value)||0}};function o(f){const c=a();return f.filter(z=>{var X,K,re;const u=(X=z.card)==null?void 0:X.player;if(!u)return!1;const I=`${u.firstname} ${u.surname_encoded}`.toLowerCase(),F=(((K=u.clubs)==null?void 0:K.encoded_name)||"").toLowerCase(),C=(u.country_code||"").toLowerCase(),q=((re=z.card)==null?void 0:re.evolution_bonus)||0,j=St(u,u.job,q),N=u.job2?St(u,u.job2,q):0;return!(c.name&&!I.includes(c.name)||c.club&&!F.includes(c.club)||c.country&&!C.includes(c.country)||c.job&&u.job!==c.job||c.rarity&&u.rarity!==c.rarity||c.note1&&j<c.note1||c.note2&&N<c.note2)})}function y(f){var C,q,j,N;const c=(C=f.card)==null?void 0:C.player;if(!c)return"";const z=((q=f.card)==null?void 0:q.evolution_bonus)||0,u=St(c,c.job,z),I=c.job2?St(c,c.job2,z):0,F=(i.profile.credits||0)>=f.price;return`<div class="card-panel" style="display:flex;align-items:center;gap:8px;padding:10px 12px;overflow:hidden;padding-left:6px">
      ${Ji(c.rarity)}
      ${Xt(u,c.job)}
      ${Xt(I,c.job2||null)}
      ${Xi(c.country_code)}
      ${Zi((j=c.clubs)==null?void 0:j.logo_url)}
      <div style="flex:1;min-width:0">
    ...
    </div>`}function p(f){var C,q,j,N;const c=(C=f.card)==null?void 0:C.player;if(!c)return"";const z=((q=f.card)==null?void 0:q.evolution_bonus)||0,u=St(c,c.job,z),I=c.job2?St(c,c.job2,z):0,F=f.status==="sold";return`<div class="card-panel" style="display:flex;align-items:center;gap:8px;padding:10px 12px;overflow:hidden;padding-left:6px;${F?"opacity:0.7":""}">
      ${Ji(c.rarity)}
      ${Xt(u,c.job)}
      ${Xt(I,c.job2||null)}
      ${Xi(c.country_code)}
    ...
    </div>`}function x(){const f=document.getElementById("mkt-content"),c=document.getElementById("mkt-filters");if(f){if(c.style.display=m==="buy"?"flex":"none",m==="buy"){const z=o(r);f.innerHTML=z.length?z.map(y).join(""):'<div style="text-align:center;color:#aaa;padding:40px">Aucune carte trouvée.</div>'}else{const z=l.filter(I=>I.status==="active").sort((I,F)=>new Date(F.listed_at)-new Date(I.listed_at)),u=l.filter(I=>I.status==="sold").sort((I,F)=>new Date(F.sold_at||F.listed_at)-new Date(I.sold_at||I.listed_at));f.innerHTML=(z.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:4px 0 6px;text-transform:uppercase;letter-spacing:1px">🟢 En vente (${z.length})</div>`+z.map(p).join(""):"")+(u.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:12px 0 6px;text-transform:uppercase;letter-spacing:1px">✅ Ventes réussies (${u.length})</div>`+u.map(p).join(""):"")+(!z.length&&!u.length?'<div style="text-align:center;color:#aaa;padding:40px">Aucune vente pour le moment.</div>':"")}f.querySelectorAll("[data-buy]").forEach(z=>z.addEventListener("click",()=>Tr(z.dataset.buy,r,e,t))),f.querySelectorAll("[data-cancel]").forEach(z=>z.addEventListener("click",()=>Sr(z.dataset.cancel,e,t)))}}e.querySelectorAll(".mkt-tab").forEach(f=>{f.addEventListener("click",()=>{m=f.dataset.tab,e.querySelectorAll(".mkt-tab").forEach(c=>{const z=c===f;c.style.background=z?"var(--green)":"#fff",c.style.color=z?"#fff":"var(--gray-600)",c.style.borderColor=z?"var(--green)":"var(--gray-200)"}),x()})});let _;["flt-name","flt-club","flt-country","flt-job","flt-rarity","flt-note1","flt-note2"].forEach(f=>{var c;(c=document.getElementById(f))==null||c.addEventListener("input",()=>{clearTimeout(_),_=setTimeout(x,250)})}),x()}async function Tr(e,t,i,n){const{state:d,toast:s,refreshProfile:r}=n,l=t.find(o=>o.id===e);if(!l)return;const m=l.price;if((d.profile.credits||0)<m){s("Crédits insuffisants","error");return}Ar(l,async()=>{const{error:o}=await T.rpc("buy_market_card",{p_listing_id:e,p_buyer_id:d.profile.id});if(o){s("Erreur achat : "+o.message,"error");return}await r();const y=document.getElementById("nav-credits");y&&(y.textContent=`💰 ${(d.profile.credits||0).toLocaleString("fr")}`),s("✅ Carte achetée !","success"),await Ii(i,n)})}function Ar(e,t){var r;const i=(r=e.card)==null?void 0:r.player,n=i?`${i.firstname} ${i.surname_encoded}`:"cette carte",d=document.createElement("div");d.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",d.innerHTML=`
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center">
      ...
      `).join(""):'<div style="text-align:center;color:var(--gray-600);padding:40px">Aucun manager.</div>'}else{const z=r||[];_.innerHTML=z.length>0?z.map((u,I)=>{const F=Pt(u.mmr??1e3),C=(u.ranked_wins||0)+(u.ranked_losses||0)+(u.ranked_draws||0),q=C>0?Math.round((u.ranked_wins||0)/C*100):0,j=u.id===i.profile.id,N=(u.placement_matches||0)<10;return`
  ...
  </div>`}Io(jo);const De={user:null,profile:null,page:"home",params:{}};function zt(e,t="info",i=3e3){const n=document.getElementById("toast");n&&(n.textContent=e,n.className=`show ${t}`,clearTimeout(n._t),n._t=setTimeout(()=>{n.className=""},i))}function Mr(e,t,i=""){document.getElementById("modal-title").textContent=e,document.getElementById("modal-body").innerHTML=t,document.getElementById("modal-footer").innerHTML=i,document.getElementById("modal-overlay").classList.remove("hidden")}function $i(){document.getElementById("modal-overlay").classList.add("hidden")}async function Ft(){if(!De.user)return;const{data:e}=await T.from("users").select("*").eq("id",De.user.id).single();e&&(De.profile=e)}function Gt(e,t={}){De.page=e,De.params=t,So()}async function So(){var n,d,s,r;const e=document.getElementById("page-content");if(!e)return;document.querySelectorAll(".bottom-nav a").forEach(l=>{l.classList.toggle("active",l.dataset.page===De.page)});const t=document.getElementById("nav-credits");t&&De.profile&&(t.textContent=`💰 ${(De.profile.credits||0).toLocaleString("fr")}`);const i={state:De,navigate:Gt,toast:zt,openModal:Mr,closeModal:$i,refreshProfile:Ft};switch(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽</div>',De.page){case"home":await Fi(e,i);break;case"collection":await ln(e,i);break;case"decks":await yi(e,i);break;case"boosters":await kn(e,i);break;case"ranked":await Lr(e,i);break;case"match":{const l=De.params&&De.params.matchMode||"vs_ai_easy";l==="random"?await wi(e,i,!1):l==="ranked"?await wi(e,i,!0):l==="friend"?await or(e,i,(n=De.params)==null?void 0:n.friendId,(d=De.params)==null?void 0:d.friendName):l==="mini-league"?await Wi(e,i,(s=De.params)==null?void 0:s.mlMatchId,(r=De.params)==null?void 0:r.leagueId):await Bn(e,i);break}case"market":await Er(e,i);break;case"rankings":await zr(e,i);break;case"matches":await Ir(e,i);break;case"friends":await Po(e,i);break;case"mini-league":await ar(e,i);break;case"match-mini-league":{const l=De.params||{};await Wi(e,i,l.mlMatchId,l.leagueId);break}default:await Fi(e,i)}}function jr(){var n;const e=document.getElementById("app"),t=De.profile;if(!t)return;const i="/icons/";e.innerHTML=`
    <nav class="top-nav">
  ...
  `,document.querySelectorAll(".bottom-nav a").forEach(d=>{d.addEventListener("click",s=>{s.preventDefault(),Gt(d.dataset.page)})}),document.getElementById("nav-logo").addEventListener("click",()=>Gt("home")),document.getElementById("nav-credits").addEventListener("click",()=>Gt("boosters")),(n=document.getElementById("journal-btn"))==null||n.addEventListener("click",()=>Cr())}async function Cr(){const{data:e}=await T.from("patch_notes").select("*").eq("is_published",!0).order("published_at",{ascending:!1}).limit(20),t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9000;display:flex;align-items:center;justify-content:center;padding:16px";const i=(e||[]).map(n=>{const d=new Date(n.published_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"long",year:"numeric"});return`<div style="padding:14px 0;border-bottom:1px solid #f0f0f0">
    ...
    </div>`,document.body.appendChild(t),t.querySelector("#journal-close").addEventListener("click",()=>t.remove()),t.addEventListener("click",n=>{n.target===t&&t.remove()})}async function qr(){document.documentElement.setAttribute("data-theme","light"),document.getElementById("modal-overlay").addEventListener("click",i=>{i.target===i.currentTarget&&$i()}),document.getElementById("modal-close").addEventListener("click",$i);const{data:{session:e}}=await T.auth.getSession();if(!e){Qi(),Ci(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:zt});return}De.user=e.user,await Ft(),Qi();try{const{data:i}=await T.from("formation_links_overrides").select("formation, links"),n={};(i||[]).forEach(d=>{n[d.formation]=d.links}),Mo(n)}catch(i){console.warn("Impossible de charger les overrides de formation:",i)}if(!De.profile){qo(document.getElementById("app"),{state:De,navigate:async()=>{await Ft(),mi()},toast:zt,refreshProfile:Ft});return}const t=Array.isArray(De.profile.pending_boosters)?De.profile.pending_boosters:[];if(!De.profile.onboarding_done&&t.length>0){Mn(document.getElementById("app"),{state:De,navigate:()=>mi(),toast:zt,refreshProfile:Ft});return}mi(),setTimeout(()=>$r(De.profile,Gt,zt),800),T.auth.onAuthStateChange(async(i,n)=>{i==="SIGNED_OUT"&&(De.user=null,De.profile=null,document.getElementById("app").innerHTML="",Ci(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:zt}))})}function Br(){return Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight)}function li(){const e=document.getElementById("app");e&&(e.style.height=Br()+"px")}window.addEventListener("resize",li);window.addEventListener("orientationchange",()=>setTimeout(li,150));window.visualViewport&&window.visualViewport.addEventListener("resize",li);function mi(){const e=()=>{var i;(i=De.user)!=null&&i.id&&T.from("users").update({last_seen_at:new Date().toISOString()}).eq("id",De.user.id).then(()=>{})};e(),window._presencePingInterval&&clearInterval(window._presencePingInterval),window._presencePingInterval=setInterval(e,6e4);const t=document.getElementById("app");t.style.display="flex",t.style.flexDirection="column",li(),jr(),So()}function Qi(){const e=document.getElementById("app-loader"),t=document.getElementById("app");t&&(t.style.display=""),e&&(e.classList.add("zoom-out"),setTimeout(()=>e.style.display="none",500))}function zo(e){var n;const t=document.getElementById("app-loader");if(t&&(t.style.display="none"),document.getElementById("boot-error"))return;const i=document.createElement("div");i.id="boot-error",i.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:99999;gap:16px;color:#fff;padding:24px;text-align:center",i.innerHTML=`
    <div style="font-size:42px">📡</div>
    <div style="font-size:18px;font-weight:900">Connexion impossible</div>
    <div style="font-size:13px;color:rgba(255,255,255,0.6);max-width:280px">${e||"Le chargement a échoué. Vérifie ta connexion et réessaie."}</div>
    <button id="boot-retry" style="margin-top:8px;padding:12px 30px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer">Réessayer</button>`,document.body.appendChild(i),(n=document.getElementById("boot-retry"))==null||n.addEventListener("click",()=>window.location.reload())}qr().catch(e=>{console.error("Échec du démarrage:",e),zo()});setTimeout(()=>{const e=document.getElementById("app-loader");e&&e.style.display!=="none"&&!e.classList.contains("zoom-out")&&!document.getElementById("boot-error")&&zo("Le serveur met trop de temps à répondre.")},12e3);
