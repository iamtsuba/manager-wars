  ...
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
        ...
        <div style="display:flex;justify-content:flex-end">
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{oe=!oe,G(),re()})))}function re(){const X=document.getElementById("col-grid");X&&(P==="player"?we(X):P==="formation"?ze(X):P==="stadium"?xe(X):Ie(X))}function pe(X,ne,ge,J,Z){const V=document.getElementById("col-grid"),ae=document.getElementById("col-big");if(!V||!ae)return;var he=0;function Le(){const be=window.innerWidth>=768,$e=document.getElementById("col-big"),De=document.getElementById("col-grid");be&&$e&&($e.style.minHeight="420px",$e.style.flex="1 1 auto"),be&&De&&(De.style.height=Math.round(310*.76+16)+"px",De.style.flexShrink="0",De.style.overflowX="auto",De.style.overflowY="hidden",De.style.alignItems="center",De.style.padding="8px 16px"),ae.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+ne(X[he])+"</div>";var I=ae.querySelector("[data-card-id],[data-form-id],[data-gc-id]");I&&I.addEventListener("click",function(){J(X[he])}),requestAnimationFrame(function(){var C=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!C||!ae)){var w=ae.clientHeight-8,k=ae.clientWidth-24,f=C.offsetHeight,m=C.offsetWidth;if(f>0&&m>0&&w>40){var x=be?2.2:1,b=Math.min(w/f,k/m,x);C.style.transform="scale("+b.toFixed(3)+")",C.style.transformOrigin="top center"}}}),V.innerHTML=X.map(function(C,w){var k=w===he,f="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+(k?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+w+'" style="'+f+'">'+ge(C,k)+"</div>"}).join(""),V.querySelectorAll(".col-mini-item").forEach(function(C){C.addEventListener("click",function(){he=Number(C.dataset.idx),Le(),C.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}Le()}function ye(X){var ne=window.innerWidth>=768?.76:.54,ge;if(!X||X._fake){var J=X?X.player:null;if(!J)return"";ge=Fi(J)}else ge=si(X,"");return'<div style="display:inline-block;zoom:'+ne+';pointer-events:none;line-height:0">'+ge+"</div>"}function Se(X,ne,ge){ne=ne||100,ge=ge||140;var J=Pt[X]||{},Z={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},V=Math.max(3,Math.round(ne*.06)),ae=Object.entries(J).map(function(Le){var be=Le[0],$e=Le[1],De=be.replace(/\d+$/,""),I=Z[De]||"#888",C=Math.round($e.x*ne),w=Math.round($e.y*ge);return'<circle cx="'+C+'" cy="'+w+'" r="'+V+'" fill="'+I+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),he=Math.max(1,Math.round(ne/50));return'<svg viewBox="0 0 '+ne+" "+ge+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+ne+'" height="'+ge+'" fill="#1A6B3C"/><rect x="'+Math.round(ne*.2)+'" y="'+Math.round(ge*.02)+'" width="'+Math.round(ne*.6)+'" height="'+Math.round(ge*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+he+'"/><line x1="0" y1="'+Math.round(ge*.5)+'" x2="'+ne+'" y2="'+Math.round(ge*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+he+'"/><ellipse cx="'+Math.round(ne*.5)+'" cy="'+Math.round(ge*.5)+'" rx="'+Math.round(ne*.18)+'" ry="'+Math.round(ge*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+he+'"/><rect x="'+Math.round(ne*.2)+'" y="'+Math.round(ge*.82)+'" width="'+Math.round(ne*.6)+'" height="'+Math.round(ge*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+he+'"/>'+ae+"</svg>"}function ve(X,ne,ge){var J=ge>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+ge+"</div>":"",Z=ne?'data-form-id="'+ne.id+'"':"",V=X.length>7?14:X.length>5?16:19,ae=!!ne;return"<div "+Z+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(ae?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(ae?"":"filter:grayscale(1);opacity:0.5")+'">'+J+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(ae?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+V+"px;font-weight:900;color:"+(ae?"#1A6B3C":"#aaa")+';line-height:1">'+X+'</div></div><div style="flex:1;overflow:hidden;background:'+(ae?"#1A6B3C":"#ccc")+'">'+Se(X,140,220)+"</div></div>"}function _e(X,ne){var ge=window.innerWidth>=768?.76:.54,J=140,Z=305,V=Math.round(Z*.22),ae=Z-V,he=X.length>7?10:13,Le=Se(X,J,ae),be=ne?"1.5px solid #2a7a40":"1px solid #ddd",$e=ne?"":"filter:grayscale(1);opacity:0.45;",De=ne?"#1A6B3C":"#bbb",I="#fff";return'<div style="display:inline-block;zoom:'+ge+';line-height:0;pointer-events:none"><div style="width:'+J+"px;height:"+Z+"px;border-radius:6px;border:"+be+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+$e+'"><div style="height:'+V+"px;background:"+De+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+he+"px;font-weight:900;color:"+I+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(J-4)+'px">'+X+'</span></div><div style="height:'+ae+'px;overflow:hidden;flex-shrink:0">'+Le+"</div></div></div>"}function we(X){if(oe){const ne=H();if(!ne.length){X.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const ge=ne.map(J=>o.find(Z=>Z.player.id===J.id)||{_fake:!0,player:J,id:"fake-"+J.id});pe(ge,J=>J._fake?Fi(J.player):si(J,""),J=>J._fake?ye({player:J.player,id:"x",_fake:!0}):ye(J),J=>{J._fake||Di(J,o,D,t)})}else{const ne=Y();if(!ne.length){X.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const ge={};ne.forEach(Z=>{const V=Z.player.id;ge[V]||(ge[V]=[]),ge[V].push(Z)});const J=Object.values(ge).map(Z=>Xo(Z));pe(J,Z=>{const V=D[Z.player.id]||1,ae=V>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${V}</div>`:"",Le=o.filter(be=>be.player.id===Z.player.id&&be.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return si(Z,ae+Le)},Z=>ye(Z),Z=>Di(Z,o,D,t))}}function ze(X){const ne=oe?M:[...W];if(!ne.length){X.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const ge=ne.map(J=>({formation:J,card:g.find(Z=>Z.formation===J)||null,owned:W.has(J)}));pe(ge,({formation:J,card:Z,owned:V})=>ve(J,V?Z:null,V?g.filter(ae=>ae.formation===J).length:0),({formation:J,owned:Z})=>_e(J,Z),({card:J,owned:Z})=>{Z&&J&&en(J,g,t,d)})}function Ie(X){const ne=Object.keys(z),ge=oe?ne.length?ne:L:[...F];if(!ge.length){X.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const J=ge.map(Z=>({type:Z,gc:Ve[Z]||{icon:"⚡",desc:""},def:z[Z]||null,owned:F.has(Z),card:n.find(V=>V.gc_type===Z)||null}));pe(J,({type:Z,gc:V,def:ae,owned:he,card:Le})=>{var x;const be=he?n.filter(b=>b.gc_type===Z).length:0,$e=be>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${be}</div>`:"",De=(ae==null?void 0:ae.gc_type)==="ultra_game_changer",I={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},C={purple:"#b06ce0",light_blue:"#00d4ef"},w=I[ae==null?void 0:ae.color]||I.purple,k=C[ae==null?void 0:ae.color]||C.purple,f=(ae==null?void 0:ae.effect)||V.desc||"",m=ae!=null&&ae.image_url?`/manager-wars/icons/${ae.image_url}`:((x=ae==null?void 0:ae.club)==null?void 0:x.logo_url)||(ae!=null&&ae.country_code?`https://flagcdn.com/w80/${ae.country_code.toLowerCase()}.png`:null);return he&&Le?`<div data-gc-id="${Le.id}" data-gc-type="${Z}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${k};background:${w};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${k}66;cursor:pointer">
          ${$e}
          ...
          <div style="height:140px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.08)">${$e}</div>
        ...
        </div>`},({def:V,count:ae})=>{var I,C;const he=window.innerWidth>=768?.76:.54,Le=(V==null?void 0:V.name)||"?",be=((I=V==null?void 0:V.club)==null?void 0:I.encoded_name)||(V==null?void 0:V.country_code)||"—",$e=V!=null&&V.image_url?`${ge}icons/${V.image_url}`:((C=V==null?void 0:V.club)==null?void 0:C.logo_url)||(V!=null&&V.country_code?`https://flagcdn.com/w80/${V.country_code.toLowerCase()}.png`:null),De=$e?`<img src="${$e}" style="width:64px;height:64px;object-fit:contain;border-radius:4px" onerror="this.style.display='none'">`:'<span style="font-size:32px">🏟️</span>';return`<div style="display:inline-block;zoom:${he};line-height:0;pointer-events:none">
    ...
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${g.map((J,Z)=>{const V=pe[J.id]||[],ae=J.is_for_sale,he=V.length?V[V.length-1]:null,be=(n.rarity==="pepite"||n.rarity==="papyte")&&J.current_note!=null?` (☆${J.current_note})`:"";return`
            <div data-card-id="${J.id}" data-card-idx="${Z}" class="exemplaire-row"
              ...
              <div style="display:flex;align-items:center;gap:10px">
                ...
                <div style="flex:1;min-width:0">
                  <div style="font-size:12px;font-weight:700">Exemplaire ${Z+1}${be}${ae?" 🏷️ En vente":""}</div>
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",n.note_g],["DEF",n.note_d],["MIL",n.note_m],["ATT",n.note_a]].map(([J,Z])=>{const V=qt[J],ae=J===n.job||J===n.job2,he=(Number(Z)||0)+(ae&&de>0?de:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:2px">
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(xe=document.getElementById("close-detail"))==null||xe.addEventListener("click",l);let ve=new Set;const _e=()=>{const J=ve.size,Z=document.getElementById("sell-action-panel");if(!Z)return;Z.style.display=J>0?"block":"none",document.getElementById("sell-selected-count").textContent=J;const V=document.getElementById("evolve-btn");V&&(V.textContent=`⬆️ Évoluer ${J>1?"(+"+J+")":""}`)};document.querySelectorAll(".expl-check").forEach(J=>{J.addEventListener("change",()=>{const Z=J.dataset.id;J.checked?ve.add(Z):ve.delete(Z);const V=J.closest(".exemplaire-row");V&&(V.style.borderColor=J.checked?"#1A6B3C":"#eee"),_e()})}),document.querySelectorAll(".exemplaire-row").forEach(J=>{J.addEventListener("click",Z=>{if(Z.target.closest("button")||Z.target.tagName==="INPUT")return;const V=J.querySelector(".expl-check");V&&!V.disabled&&(V.checked=!V.checked,V.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(J=>{J.addEventListener("click",Z=>{Z.stopPropagation();const V=document.querySelector(`.expl-hist[data-hist="${J.dataset.idx}"]`);V&&(V.style.display=V.style.display==="none"?"flex":"none")})}),(X=document.getElementById("evolve-btn"))==null||X.addEventListener("click",async()=>{if(h<=1)return;const J=[...ve];if(!J.length)return;if(ve.has(e.id)){const be=document.createElement("div");be.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",be.innerHTML=`
        ...
        </div>`,document.body.appendChild(be),be.querySelector("#err-close").addEventListener("click",()=>be.remove()),be.addEventListener("click",$e=>{$e.target===be&&be.remove()});return}const Z=J.filter(be=>be!==e.id);if(Z.length,!(!Z.length&&J.length===1&&J[0]===e.id)){if(!Z.length){d("Sélectionne des copies à sacrifier","warning");return}}if(!await new Promise(be=>{const $e=document.createElement("div");$e.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",$e.innerHTML=`
  ...
  </div>`,on(e,t,c,i),document.getElementById("builder-back").addEventListener("click",()=>s("decks")),document.getElementById("formation-select").addEventListener("change",n=>{t.formation=n.target.value;const g=Pi(t.formation),h={};g.forEach(y=>{t.slots[y]&&(h[y]=t.slots[y])}),t.slots=h,Lt(e,t,i)}),(p=document.getElementById("stadium-select"))==null||p.addEventListener("change",n=>{t.stadiumCardId=n.target.value||null}),document.getElementById("save-deck").addEventListener("click",()=>an(t,i)),e.querySelectorAll("[data-remove-sub]").forEach(n=>{n.addEventListener("click",()=>{t.subs=t.subs.filter(g=>g!==n.dataset.removeSub),Lt(e,t,i)})}),(o=document.getElementById("add-sub-btn"))==null||o.addEventListener("click",()=>{rn(t,e,i)})}function on(e,t,i,s){var D,W,F;const c=e.querySelector("#deck-field");if(!c)return;const d=(D=t.stadiumCards)==null?void 0:D.find(P=>P.id===t.stadiumCardId),a=d&&((W=t.stadDefMap)==null?void 0:W[d.stadium_id])||null,l=Pt[t.formation]||{},p=Wi(t.formation)||[],o={};for(const P of i){const j=t.slots[P],U=j?t.playerCards.find(oe=>oe.id===j):null;o[P]=U?U.player:null}const n=300,g=300,h=48,y=64,z=13,r=16,u=38;function M(P){const j=l[P];return j?{x:j.x*n,y:j.y*g}:null}let L="";for(const[P,j]of p){const U=M(P),oe=M(j);if(!U||!oe)continue;const de=o[P]?{...o[P],club_id:o[P].club_id,country_code:o[P].country_code,rarity:o[P].rarity}:null,N=o[j]?{...o[j],club_id:o[j].club_id,country_code:o[j].country_code,rarity:o[j].rarity}:null,Y=at(de,N);Y==="#ff3333"||Y==="#cc2222"?L+=`<line x1="${U.x.toFixed(1)}" y1="${U.y.toFixed(1)}" x2="${oe.x.toFixed(1)}" y2="${oe.y.toFixed(1)}" stroke="${Y}" stroke-width="2.5" stroke-linecap="round" opacity="0.4"/>`:(L+=`<line x1="${U.x.toFixed(1)}" y1="${U.y.toFixed(1)}" x2="${oe.x.toFixed(1)}" y2="${oe.y.toFixed(1)}" stroke="${Y}" stroke-width="8" stroke-linecap="round" opacity="0.2"/>`,L+=`<line x1="${U.x.toFixed(1)}" y1="${U.y.toFixed(1)}" x2="${oe.x.toFixed(1)}" y2="${oe.y.toFixed(1)}" stroke="${Y}" stroke-width="2.5" stroke-linecap="round" opacity="0.9"/>`)}for(const P of i){const j=M(P);if(!j)continue;const U=o[P],oe=P.replace(/\d+/,""),de=vt[oe]||"#555",N=(j.x-h/2).toFixed(1),Y=(j.y-y/2).toFixed(1),H={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[U==null?void 0:U.rarity]||"#aaa";if(U){const G=ti(U),re=co(U),pe=lo(U.country_code),ye=((F=U.card)==null?void 0:F.evolution_bonus)||U.evolution_bonus||0,Se=(Number(oe==="GK"?U.note_g:oe==="DEF"?U.note_d:oe==="MIL"?U.note_m:U.note_a)||0)+(oe===U.job||oe===U.job2?ye:0),ve=a&&(a.club_id&&String(U.club_id)===String(a.club_id)||a.country_code&&U.country_code===a.country_code),_e=ve?Se+10:Se,we=y-z-r;L+=`<defs><clipPath id="dcp-${P}"><rect x="${N}" y="${(j.y-y/2+z).toFixed(1)}" width="${h}" height="${we}"/></clipPath></defs>`,L+=`<rect x="${N}" y="${Y}" width="${h}" height="${y}" rx="5" fill="${de}"/>`,G&&(L+=`<image href="${G}" x="${N}" y="${(j.y-y/2+z).toFixed(1)}" width="${h}" height="${we}" clip-path="url(#dcp-${P})" preserveAspectRatio="xMidYMin slice"/>`),L+=`<rect x="${N}" y="${Y}" width="${h}" height="${z}" fill="rgba(255,255,255,0.93)"/>`,L+=`<text x="${j.x.toFixed(1)}" y="${(j.y-y/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(U.surname_encoded||"").slice(0,9)}</text>`;const ze=(j.y+y/2-r).toFixed(1);L+=`<rect x="${N}" y="${ze}" width="${h}" height="${r}" fill="rgba(255,255,255,0.93)"/>`,pe&&(L+=`<image href="${pe}" x="${(j.x-21).toFixed(1)}" y="${(j.y+y/2-r+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`),L+=`<text x="${j.x.toFixed(1)}" y="${(j.y+y/2-r/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${_e}</text>`,re&&(L+=`<image href="${re}" x="${(j.x+h/2-14).toFixed(1)}" y="${(j.y+y/2-r+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`),L+=`<rect x="${N}" y="${Y}" width="${h}" height="${y}" rx="5" fill="none" stroke="${ve?"#E87722":H}" stroke-width="${ve?"2.5":"2"}"/>`,ve&&(L+=`<rect x="${(j.x+h/2-13).toFixed(1)}" y="${Y}" width="13" height="9" rx="3" fill="#E87722"/>`,L+=`<text x="${(j.x+h/2-6.5).toFixed(1)}" y="${(j.y-y/2+4.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="900" fill="#fff" font-family="Arial">+10</text>`)}else L+=`<rect x="${N}" y="${Y}" width="${h}" height="${y}" rx="5" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="5,3"/>`,L+=`<text x="${j.x.toFixed(1)}" y="${j.y.toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="22" fill="rgba(255,255,255,0.35)">+</text>`,L+=`<text x="${j.x.toFixed(1)}" y="${(j.y+16).toFixed(1)}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.3)">${oe}</text>`;L+=`<rect x="${N}" y="${Y}" width="${h}" height="${y}" rx="5" fill="rgba(0,0,0,0.01)" class="deck-slot-hit" data-pos="${P}" style="cursor:pointer"/>`}c.innerHTML=`<svg viewBox="${-u} ${-u} ${n+u*2} ${g+u*2}" width="100%" style="display:block;max-width:440px;margin:0 auto">${L}</svg>`,c.querySelectorAll(".deck-slot-hit").forEach(P=>{P.addEventListener("click",()=>nn(P.dataset.pos,t,e,s))})}function nn(e,t,i,s){var h,y,z;const{openModal:c,closeModal:d}=s,a=e.replace(/\d+/,""),l=t.slots[e],p=l?t.playerCards.find(r=>r.id===l):null;(h=p==null?void 0:p.player)==null||h.id;const o=new Set;Object.entries(t.slots).forEach(([r,u])=>{var L;if(r===e||!u)return;const M=t.playerCards.find(D=>D.id===u);(L=M==null?void 0:M.player)!=null&&L.id&&o.add(M.player.id)}),t.subs.forEach(r=>{var M;const u=t.playerCards.find(L=>L.id===r);(M=u==null?void 0:u.player)!=null&&M.id&&o.add(u.player.id)});const n=new Set,g=t.playerCards.filter(r=>{const u=r.player;return!(u.job===a||u.job2===a)||o.has(u.id)||n.has(u.id)?!1:(n.add(u.id),!0)});g.sort((r,u)=>{card!=null&&card.evolution_bonus;const M=r.evolution_bonus||0,L=u.evolution_bonus||0,D=(a==="GK"?r.player.note_g:a==="DEF"?r.player.note_d:a==="MIL"?r.player.note_m:r.player.note_a)+(a===r.player.job||a===r.player.job2?M:0);return(a==="GK"?u.player.note_g:a==="DEF"?u.player.note_d:a==="MIL"?u.player.note_m:u.player.note_a)+(a===u.player.job||a===u.player.job2?L:0)-D}),c(`Choisir ${a} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      card:cards(id,card_type,formation,stadium_id,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order"),p=[...new Set((l||[]).filter(h=>{var y,z;return((y=h.card)==null?void 0:y.card_type)==="stadium"&&((z=h.card)==null?void 0:z.stadium_id)}).map(h=>h.card.stadium_id))],o={};if(p.length){const{data:h}=await E.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",p);(h||[]).forEach(y=>{o[y.id]=y}),(l||[]).forEach(y=>{var z,r;((z=y.card)==null?void 0:z.card_type)==="stadium"&&((r=y.card)!=null&&r.stadium_id)&&(y.card._stadiumDef=o[y.card.stadium_id]||null)})}let n=0;function g(){var F,P,j,U,oe,de,N;const h=d[n],y=(l||[]).filter(Y=>Y.deck_id===h.id),z=y.filter(Y=>{var H;return Y.is_starter&&((H=Y.card)==null?void 0:H.player)}).map(Y=>gi(Y.card,Y.position)),r=y.find(Y=>{var H;return((H=Y.card)==null?void 0:H.card_type)==="formation"}),u=h.formation||((F=r==null?void 0:r.card)==null?void 0:F.formation)||"4-4-2";let M=z.length>=11?yt(z,u):null,L=((P=h.stadium_card)==null?void 0:P.stadium_def)||null;L&&M&&(M=Ht(M,L));const D=z.length>=11;et(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    ${(()=>{const N=window.innerWidth>=700,Y=(we,ze,Ie)=>{var $e,De;const xe=(t.gcDefs||[]).find(I=>I.name===we.gc_type),X={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[xe==null?void 0:xe.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",ne={purple:"#b06ce0",light_blue:"#00d4ef"}[xe==null?void 0:xe.color]||"#b06ce0",ge=(xe==null?void 0:xe.name)||we.gc_type,J=(xe==null?void 0:xe.effect)||(($e=Ve[we.gc_type])==null?void 0:$e.desc)||"",Z=xe!=null&&xe.image_url?`/manager-wars/icons/${xe.image_url}`:null,V=((De=Ve[we.gc_type])==null?void 0:De.icon)||"⚡",ae=Math.round(Ie*.22),he=Math.round(Ie*.22),Le=Ie-ae-he,be=ge.length>12?7:9;return`<div class="gc-mini" data-gc-id="${we.id}" data-gc-type="${we.gc_type}"
          ...
          <div style="height:${ae}px;background:rgba(255,255,255,0.14);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 3px">
            <span style="font-size:${be}px;font-weight:900;color:#fff;text-align:center;line-height:1.1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${ze-6}px">${ge}</span>
        ...
        </div>`},H=(we,ze)=>{var Ie;return`<div id="boost-card"
          style="box-sizing:border-box;width:${we}px;height:${ze}px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:2px solid #87CEEB;border-radius:10px;cursor:pointer;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:${Math.round(ze*.04)}px;text-align:center;flex-shrink:0">
  ...
  `,document.body.appendChild(s);let n=!1;const g=()=>{n||(n=!0,s.remove(),i())};s.addEventListener("click",g),setTimeout(g,2e3)}function jt(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function di(e,t,i,s=null,c=null){var y,z;if(t.phase!=="attack"){jt("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){jt(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const d=Object.entries(t.homeTeam).flatMap(([r,u])=>(u||[]).filter(M=>M.used).map(M=>({...M,_line:M._line||r}))),a=t.homeSubs.filter(r=>!t.usedSubIds.includes(r.cardId));if(!d.length){jt("Aucun joueur utilisé à remplacer");return}if(!a.length){jt("Aucun remplaçant disponible");return}let l=Math.max(0,d.findIndex(r=>r.cardId===c));const p=((y=d[l])==null?void 0:y._line)||((z=d[l])==null?void 0:z.job);let o=s?Math.max(0,a.findIndex(r=>r.cardId===s)):Math.max(0,a.findIndex(r=>r.job===p)),n=!1;const g=document.createElement("div");g.id="sub-overlay",g.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function h(){var F,P,j,U,oe,de;const r=d[l],u=a[o],M=Math.min(130,Math.round((window.innerWidth-90)/2)),L=Math.round(M*1.35),D=N=>`background:rgba(255,255,255,0.12);border:none;color:${N?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${N?"default":"pointer"};flex-shrink:0`;g.innerHTML=`
    ...
    <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
      ${e.map(g=>{const h=g._line||g.job||"MIL",y={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[h]||"#555",z=Me(g,h)+(g.boost||0),r=l.find(u=>u.cardId===g.cardId);return`<div class="gc-pick-item" data-cid="${g.cardId}"
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(d);let l=!1;const p=()=>{l||(l=!0,d.remove(),setTimeout(()=>c(),50))};d.addEventListener("click",p),setTimeout(p,3500)}async function Bt(e,t,i){var n,g;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:s}=i,c=t.homeScore>t.aiScore,d=t.homeScore===t.aiScore,a=c?"victoire":d?"nul":"defaite",l=Vo(t.mode,a);t.matchId&&await E.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:c?s.profile.id:null,home_credits_reward:l,played_at:new Date().toISOString()}).eq("id",t.matchId);const p={credits:(s.profile.credits||0)+l,matches_played:(s.profile.matches_played||0)+1};c?p.wins=(s.profile.wins||0)+1:d?p.draws=(s.profile.draws||0)+1:p.losses=(s.profile.losses||0)+1,await E.from("users").update(p).eq("id",s.profile.id),await i.refreshProfile();const o=document.createElement("div");o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",o.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const z=async(W=!0)=>{h=!0,y&&(E.removeChannel(y),y=null),W&&await E.rpc("cancel_matchmaking",{p_user_id:o.profile.id}).catch(()=>{})};(D=document.getElementById("mm-cancel"))==null||D.addEventListener("click",async()=>{try{await z(!0)}catch{}We(e),n("home")});const r=async(W,F,P)=>{if(h)return;h=!0,y&&(E.removeChannel(y),y=null);const j=document.getElementById("mm-status");j&&(j.textContent="Adversaire trouvé !"),await new Promise(U=>setTimeout(U,600)),e.isConnected&&$o(e,t,W,P,a,l)},{data:u,error:M}=await E.rpc("try_matchmake",{p_user_id:o.profile.id,p_deck_id:i});if(M||!(u!=null&&u.success)){g("Erreur de matchmaking","error"),We(e),n("home");return}if(u.matched){r(u.match_id,u.opponent_id,!1);return}const L=document.getElementById("mm-status");L&&(L.textContent="En attente d'un autre joueur..."),y=E.channel("matchmaking-"+o.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${o.profile.id}`},W=>{const F=W.new;F.status==="matched"&&F.match_id&&r(F.match_id,F.matched_with,!0)}).subscribe()}async function Un(e,t,i){const{state:s,navigate:c,toast:d}=t;try{(E.getChannels?E.getChannels():[]).forEach(o=>{const n=o.topic||"";(n.includes("matchmaking")||n.includes("pvp-match")||n.includes("friend-invite"))&&E.removeChannel(o)})}catch{}const{data:a}=await E.from("matches").select("home_id, away_id, status").eq("id",i).single();if(!a){d("Match introuvable","error"),c("home");return}if(a.status==="finished"){d("Ce match est terminé","info"),c("home");return}const l=a.home_id===s.profile.id;$o(e,t,i,l,[],[])}async function $o(e,t,i,s,c=[],d=[]){const{state:a,navigate:l,toast:p}=t,o=s?"p1":"p2",n=s?"p2":"p1",g=(c||[]).map(I=>I.id),h=(c||[]).map(I=>({id:I.id,gc_type:I.gc_type,_gcDef:I._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:y}=await E.from("matches").select("*").eq("id",i).single();if(!y){p("Match introuvable","error"),l("home");return}async function z(){const[{data:I},{data:C},{data:w},{data:k}]=await Promise.all([E.rpc("get_deck_for_match",{p_deck_id:y.home_deck_id}),E.rpc("get_deck_for_match",{p_deck_id:y.away_deck_id}),E.from("users").select("id,pseudo,club_name").eq("id",y.home_id).single(),E.from("users").select("id,pseudo,club_name").eq("id",y.away_id).single()]),f=A=>({cardId:A.card_id,position:A.position,id:A.id,firstname:A.firstname,name:A.surname_encoded,country_code:A.country_code,club_id:A.club_id,job:A.job,job2:A.job2,note_g:Number(A.note_g)||0,note_d:Number(A.note_d)||0,note_m:Number(A.note_m)||0,note_a:Number(A.note_a)||0,rarity:A.rarity,skin:A.skin,hair:A.hair,hair_length:A.hair_length,clubName:A.club_encoded_name||null,clubLogo:A.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}),m=((I==null?void 0:I.starters)||[]).map(A=>f(A)),x=((C==null?void 0:C.starters)||[]).map(A=>f(A)),b=(I==null?void 0:I.formation)||"4-4-2",v=(C==null?void 0:C.formation)||"4-4-2";let _=yt(m,b),T=yt(x,v);const $=((I==null?void 0:I.subs)||[]).map(A=>f(A));return stadiumDef&&s&&(_=Ht(_,stadiumDef),ii($,stadiumDef)),{p1Team:_,p2Team:T,p1Subs:$,p2Subs:((C==null?void 0:C.subs)||[]).map(A=>f(A)),p1Formation:b,p2Formation:v,p1Name:(w==null?void 0:w.club_name)||(w==null?void 0:w.pseudo)||"Joueur 1",p2Name:(k==null?void 0:k.club_name)||(k==null?void 0:k.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:s?g:[],gc_p2:s?[]:g,gcCardsFull_p1:s?h:[],gcCardsFull_p2:s?[]:h,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:d||[],lastActionAt:new Date().toISOString()}}let r=y.game_state&&Object.keys(y.game_state).length?y.game_state:null;if(!r)if(s){r=await z();const{data:I}=await E.from("matches").select("game_state").eq("id",i).single();!(I!=null&&I.game_state)||!Object.keys(I.game_state).length?await E.from("matches").update({game_state:r,turn_user_id:y.home_id}).eq("id",i):r=I.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let I=0;I<30&&!r;I++){await new Promise(w=>setTimeout(w,400));const{data:C}=await E.from("matches").select("game_state").eq("id",i).single();C!=null&&C.game_state&&Object.keys(C.game_state).length&&(r=C.game_state)}if(!r){p("Erreur de synchronisation","error"),l("home");return}r.gc_p2=g,r.gcCardsFull_p2=h,await E.from("matches").update({game_state:r}).eq("id",i)}let u=!1,M=null,L=!1;const D=new Set,W=new Set;function F(I){var _,T;try{E.removeChannel(P)}catch{}const C=r[o+"Score"]||0,w=r[n+"Score"]||0;let k,f;I.winner_id?(k=I.winner_id===a.profile.id,f=!1):I.forfeit?(k=C>w,f=!1):(f=C===w,k=C>w),(_=document.getElementById("pvp-end-overlay"))==null||_.remove();const m=document.createElement("div");m.id="pvp-end-overlay",m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const x=f?"🤝":k?"🏆":"😞",b=f?"MATCH NUL":k?"VICTOIRE !":"DÉFAITE",v=f?"#fff":k?"#FFD700":"#ff6b6b";m.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(m),(T=m.querySelector("#pvp-end-home"))==null||T.addEventListener("click",()=>{m.remove(),We(e),l("home")})}const P=E.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},I=>{const C=I.new;try{if(C.status==="finished"||C.forfeit){if(u)return;u=!0,M&&(clearInterval(M),M=null),C.game_state&&(r=C.game_state),F(C);return}if(C.game_state){const w=r;r=C.game_state;const k=r._lastGC;if(k&&k.seq&&!W.has(k.seq)&&(W.add(k.seq),k.by!==o)){ve(k.type,k.by,()=>G());return}const f=w[o+"Score"]||0,m=w[n+"Score"]||0,x=r[o+"Score"]||0,b=r[n+"Score"]||0,v=x>f,_=b>m;if((v||_)&&!D.has(r.round)){D.add(r.round);const T=[...r.log||[]].reverse().find(A=>A.isGoal),$=((T==null?void 0:T.homePlayers)||[]).map(A=>({name:A.name,note:A.note,portrait:A.portrait,job:A.job}));Se($,x,b,v,()=>G());return}G()}}catch(w){console.error("[PvP] crash:",w)}}).subscribe();async function j(I){Object.assign(r,I),r.lastActionAt=new Date().toISOString();const{error:C}=await E.from("matches").update({game_state:r}).eq("id",i);C&&p("Erreur de synchronisation","error");try{G()}catch(w){console.error("[PvP] renderPvpScreen crash:",w)}}async function U(){if(u)return;u=!0,M&&(clearInterval(M),M=null);const I=s?y.away_id:y.home_id,C=s?"p2":"p1",w=s?"p1":"p2",k={...r,[C+"Score"]:3,[w+"Score"]:0,phase:"finished"};try{await E.from("matches").update({status:"finished",forfeit:!0,winner_id:I,home_score:k.p1Score||0,away_score:k.p2Score||0,game_state:k}).eq("id",i)}catch{}try{E.removeChannel(P)}catch{}setTimeout(()=>{We(e),l("home")},800)}const oe={BOOST_STAT:({value:I=1,count:C=1,roles:w=[]},k,f)=>{const m=k[o+"Team"],x=Object.entries(m).filter(([b])=>!w.length||w.includes(b)).flatMap(([b,v])=>v.filter(_=>!_.used).map(_=>({..._,_line:b})));if(!x.length){k.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),f(k);return}de(x,C,`Choisir ${C} joueur(s) à booster (+${I})`,b=>{b.forEach(v=>{const _=(m[v._line]||[]).find(T=>T.cardId===v.cardId);_&&(_.boost=(_.boost||0)+I,k.log.push({text:`⚡ +${I} sur ${_.name}`,type:"info"}))}),k[o+"Team"]=m,f(k)})},DEBUFF_STAT:({value:I=1,count:C=1,roles:w=[],target:k="ai"},f,m)=>{const x=k==="home"?o:n,b=f[x+"Team"],v=k==="home"?"allié":"adverse",_=Object.entries(b).filter(([T])=>!w.length||w.includes(T)).flatMap(([T,$])=>$.map(A=>({...A,_line:T})));if(!_.length){f.log.push({text:`🎯 Aucun joueur ${v}`,type:"info"}),m(f);return}de(_,C,`Choisir ${C} joueur(s) ${v}(s) (-${I})`,T=>{T.forEach($=>{const A=(b[$._line]||[]).find(S=>S.cardId===$.cardId);A&&(A.boost=(A.boost||0)-I,f.log.push({text:`🎯 -${I} sur ${A.name}`,type:"info"}))}),f[x+"Team"]=b,m(f)})},GRAY_PLAYER:({count:I=1,roles:C=[],target:w="ai"},k,f)=>{const m=w==="home"?o:n,x=k[m+"Team"],b=w==="home"?"allié":"adverse",v=Object.entries(x).filter(([_])=>!C.length||C.includes(_)).flatMap(([_,T])=>T.filter($=>!$.used).map($=>({...$,_line:_})));if(!v.length){k.log.push({text:`❌ Aucun joueur ${b}`,type:"info"}),f(k);return}de(v,I,`Choisir ${I} joueur(s) ${b}(s) à exclure`,_=>{const T="usedCardIds_"+m,$=new Set(k[T]||[]);_.forEach(A=>{const S=(x[A._line]||[]).find(q=>q.cardId===A.cardId);S&&(S.used=!0,$.add(A.cardId),k.log.push({text:`❌ ${S.name} exclu !`,type:"info"}))}),k[T]=[...$],k[m+"Team"]=x,f(k)})},REVIVE_PLAYER:({count:I=1,roles:C=[]},w,k)=>{const f=w[o+"Team"],m=Object.entries(f).filter(([x])=>!C.length||C.includes(x)).flatMap(([x,b])=>b.filter(v=>v.used).map(v=>({...v,_line:x})));if(!m.length){w.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),k(w);return}de(m,I,`Choisir ${I} joueur(s) à ressusciter`,x=>{x.forEach(b=>{const v=(f[b._line]||[]).find(_=>_.cardId===b.cardId);v&&(v.used=!1,w.log.push({text:`💫 ${v.name} ressuscité !`,type:"info"}))}),w[o+"Team"]=f,k(w)})},REMOVE_GOAL:({},I,C)=>{const w=n+"Score";I[w]>0?(I[w]--,I.log.push({text:"🚫 Dernier but annulé !",type:"info"})):I.log.push({text:"🚫 Aucun but à annuler",type:"info"}),C(I)},ADD_GOAL_DRAW:({},I,C)=>{I[o+"Score"]===I[n+"Score"]?(I[o+"Score"]++,I.log.push({text:"🎯 But bonus !",type:"info"})):I.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),C(I)},ADD_SUB:({value:I=1},C,w)=>{C.maxSubs=(C.maxSubs||3)+I,C.log.push({text:`🔄 +${I} remplacement(s)`,type:"info"}),w(C)},CUSTOM:({},I,C)=>C(I)};function de(I,C,w,k){const f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let m=[];function x(){var v,_;const b=I.map(T=>{const $={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[T._line]||"#555",A=Me(T,T._line)+(T.boost||0),q=m.find(R=>R.cardId===T.cardId)?"#FFD700":"rgba(255,255,255,0.25)",B=T.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${T.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${q};background:${$};overflow:hidden;cursor:pointer;${B}">
        ...
        </div>`,(v=f.querySelector("#pp-close"))==null||v.addEventListener("click",()=>f.remove()),f.querySelectorAll(".pp-item").forEach(T=>{T.addEventListener("click",()=>{const $=T.dataset.cid,A=I.find(q=>q.cardId===$),S=m.findIndex(q=>q.cardId===$);A&&(S>-1?m.splice(S,1):m.length<C&&m.push(A),x())})}),(_=f.querySelector("#pp-confirm"))==null||_.addEventListener("click",()=>{f.remove(),k(m)})}x(),document.body.appendChild(f)}async function N(I,C){const k=(r["gcCardsFull_"+o]||[]).find(b=>b.id===I),f=(k==null?void 0:k._gcDef)||(r.gcDefs||[]).find(b=>{var v;return b.name===C||((v=b.name)==null?void 0:v.toLowerCase().trim())===(C==null?void 0:C.toLowerCase().trim())}),m=[...r["usedGc_"+o]||[],I],x={type:C,by:o,seq:(r._gcAnimSeq||0)+1};W.add(x.seq),ve(C,o,async()=>{if(f!=null&&f.effect_type&&f.effect_type!=="CUSTOM"){const v=oe[f.effect_type];if(v){const _={...r};v(f.effect_params||{},_,async T=>{T["usedGc_"+o]=m,T._lastGC=x,T._gcAnimSeq=x.seq,await j(T)});return}}const b={...r};switch(C){case"Remplacement+":b.maxSubs=(b.maxSubs||3)+1,b.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const v=n+"Score";b[v]>0&&(b[v]--,b.log.push({text:"🚫 But annulé",type:"info"}));break}}b["usedGc_"+o]=m,b._lastGC=x,b._gcAnimSeq=x.seq,await j(b)})}function Y(I,C){const w="usedCardIds_"+I,k=new Set(r[w]||[]);C.forEach(f=>k.add(f)),r[w]=[...k]}function H(){for(const I of["p1","p2"]){const C=new Set(r["usedCardIds_"+I]||[]),w=r[I+"Team"];if(w)for(const k of["GK","DEF","MIL","ATT"])(w[k]||[]).forEach(f=>{f.used=C.has(f.cardId)})}}function G(){var Ue,Je,Ze,Qe,nt,gt;if(r.phase==="reveal")return re();if(r.phase==="midfield")return ye();if(r.phase==="finished")return De();const I=r[o+"Team"],C=r[n+"Team"];H();const w=r[o+"Score"],k=r[n+"Score"],f=r[o+"Name"],m=r[n+"Name"],x=r.phase===o+"-attack",b=r.phase===o+"-defense",v=Array.isArray(r["selected_"+o])?r["selected_"+o]:[],_=v.map(le=>le.cardId),T=window.innerWidth>=700,$=r[o+"Subs"]||[],A=r["usedSubIds_"+o]||[],S=$.filter(le=>!A.includes(le.cardId)),q=r["gcCardsFull_"+o]||[],B=r["usedGc_"+o]||[],R=q.filter(le=>!B.includes(le.id)),O=r.boostOwner===o&&!r.boostUsed,Q=!["GK","DEF","MIL","ATT"].some(le=>(C[le]||[]).some(Te=>!Te.used)),K=[...I.MIL||[],...I.ATT||[]].filter(le=>!le.used),ie=x&&Q&&K.length===0?[...I.GK||[],...I.DEF||[]].filter(le=>!le.used).map(le=>le.cardId):[];function se(le,Te,te){var pt,ht;const ue=le._gcDef,ee={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ue==null?void 0:ue.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",ce={purple:"#b06ce0",light_blue:"#00d4ef"}[ue==null?void 0:ue.color]||"#b06ce0",qe=(ue==null?void 0:ue.name)||le.gc_type,Ae=(ue==null?void 0:ue.effect)||((pt=Ve[le.gc_type])==null?void 0:pt.desc)||"",tt=ue!=null&&ue.image_url?`/manager-wars/icons/${ue.image_url}`:null,it=((ht=Ve[le.gc_type])==null?void 0:ht.icon)||"⚡",rt=Math.round(te*.22),st=Math.round(te*.22),ct=te-rt-st,$t=qe.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${le.id}" data-gc-type="${le.gc_type}"
        style="box-sizing:border-box;width:${Te}px;height:${te}px;border-radius:10px;border:2px solid ${ce};background:${ee};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
        <div style="height:${rt}px;background:rgba(255,255,255,0.14);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 3px">
          <span style="font-size:${$t}px;font-weight:900;color:#fff;text-align:center;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${Te-6}px">${qe}</span>
          <span style="font-size:6px;color:rgba(255,255,255,0.45)">⚡ GC</span>
        </div>
        <div style="height:${ct}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
          ${tt?`<img src="${tt}" style="max-width:${Te-10}px;max-height:${ct-6}px;object-fit:contain">`:`<span style="font-size:${Math.round(ct*.55)}px">${it}</span>`}
        </div>
      ...
      </div>`}function fe(le,Te){return`<div id="pvp-boost-card"
        style="box-sizing:border-box;width:${le}px;height:${Te}px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:2px solid #87CEEB;border-radius:10px;cursor:pointer;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:${Math.round(Te*.04)}px;text-align:center;flex-shrink:0">
        <div style="font-size:${Math.round(Te*.2)}px">⚡</div>
        <div style="font-size:${Math.round(Te*.09)}px;color:#000;font-weight:900">+${r.boostValue}</div>
      </div>`}const me=(le,Te)=>Te?fe(130,175):se(le,130,175),ke=(le,Te)=>Te?fe(68,95):se(le,68,95),Ee=T?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",Ce=x?he(o)?`<button id="pvp-action" style="${Ee};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${v.length===0?"disabled":""}>⚔️ ATTAQUEZ <span id="pvp-timer"></span></button>`:`<button id="pvp-action" data-pass="1" style="${Ee};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER <span id="pvp-timer"></span></button>`:b?`<button id="pvp-action" style="${Ee};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${v.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="pvp-timer"></span></button>`:`<div style="font-size:11px;color:rgba(255,255,255,0.3);text-align:center;padding:4px">⏳ Tour de ${m}</div>`,je=x&&!he(o)?'<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">Aucun attaquant — passez la main</div>':x||b?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${v.length}/3 sélectionné(s)</div>`:"",Re=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${T?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${ft(I,r[o+"Formation"],Be,_,300,300,ie)}
    ...
    </div>`;function Fe(le){if(le.type==="duel"&&(le.homeTotal!=null||le.aiTotal!=null)){const Te=(le.homeTotal||0)>=(le.aiTotal||0);return`<div style="background:rgba(255,255,255,0.05);border-radius:8px;padding:5px 6px;border:1px solid rgba(255,255,255,0.08)">
          ...
          <div style="display:flex;align-items:center;gap:3px;max-height:46px;overflow:hidden">
            ...
            <div style="text-align:center;padding:0 6px;flex-shrink:0">
              <div style="font-size:${Te?20:14}px;font-weight:900;color:${Te?"#FFD700":"rgba(255,255,255,0.4)"};line-height:1">${le.homeTotal}</div>
              <div style="font-size:8px;color:rgba(255,255,255,0.3);margin:1px 0">VS</div>
              <div style="font-size:${Te?14:20}px;font-weight:900;color:${Te?"rgba(255,255,255,0.4)":"#ff6b6b"};line-height:1">${le.aiTotal}</div>
            </div>
            <div style="flex:1;display:flex;gap:2px;justify-content:flex-start;flex-wrap:nowrap;overflow-x:auto;overflow-y:hidden">
        ...
        </div>`}return`<div style="font-size:11px;color:${le.type==="goal"?"#FFD700":"rgba(255,255,255,0.65)"};font-weight:${le.type==="goal"?700:400};padding:3px 2px">${le.text||""}</div>`}const Ke=(()=>{var Te,te;if(b&&((Te=r.pendingAttack)!=null&&Te.players)){const ue=r.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
      ...
      </div>`;function He(){const le=e.querySelector(".match-screen");if(!le)return;const Te=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);le.style.height=Te+"px",le.style.minHeight=Te+"px",le.style.maxHeight=Te+"px",le.style.overflow="hidden";const te=e.querySelector("#mobile-action-bar"),ue=e.querySelector("#mobile-play-area");te&&ue&&(ue.style.paddingBottom=te.offsetHeight+"px")}if(He(),setTimeout(He,120),setTimeout(He,400),L||(L=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",He),window.visualViewport.addEventListener("scroll",He)),window.addEventListener("resize",He)),function(){const Te=e.querySelector(".terrain-wrapper svg");Te&&(Te.removeAttribute("width"),Te.removeAttribute("height"),Te.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",Te.setAttribute("viewBox","-26 -26 352 352"),Te.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),e.querySelectorAll(".match-slot-hit").forEach(le=>{le.addEventListener("click",()=>{if(!x&&!b)return;const Te=le.dataset.cardId,te=le.dataset.role,ue=(I[te]||[]).find(Ae=>Ae.cardId===Te);if(!ue||ue.used)return;const ee=ie.includes(Te);if(x&&!["MIL","ATT"].includes(te)&&!ee)return;Array.isArray(r["selected_"+o])||(r["selected_"+o]=[]);const ce=r["selected_"+o],qe=ce.findIndex(Ae=>Ae.cardId===Te);qe>-1?ce.splice(qe,1):ce.length<3&&ce.push({...ue,_role:te}),G()})}),e.querySelectorAll(".match-used-hit").forEach(le=>{le.addEventListener("click",()=>ze(le.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(le=>{le.addEventListener("click",()=>ze())}),(Ue=e.querySelector("#pvp-sub-open"))==null||Ue.addEventListener("click",()=>ze()),e.querySelectorAll(".pvp-gc-mini").forEach(le=>{le.addEventListener("click",()=>_e(le.dataset.gcId,le.dataset.gcType))}),(Je=e.querySelector("#pvp-boost-card"))==null||Je.addEventListener("click",()=>we()),(Ze=e.querySelector("#pvp-action"))==null||Ze.addEventListener("click",le=>{x?le.currentTarget.dataset.pass==="1"||!he(o)?ne():ge():b&&J()}),(Qe=e.querySelector("#pvp-quit"))==null||Qe.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&U()}),(nt=e.querySelector("#pvp-view-opp"))==null||nt.addEventListener("click",()=>xe()),(gt=e.querySelector("#pvp-toggle-history"))==null||gt.addEventListener("click",()=>X()),M&&(clearInterval(M),M=null),(x||b)&&!u){let le=30,Te=!1;const te=()=>document.getElementById("pvp-timer"),ue=()=>{te()&&(te().textContent=le+"s",te().style.color=Te?"#ff4444":"#fff")};ue(),M=setInterval(()=>{le--,le<0?Te?(clearInterval(M),M=null,x&&!he(o)?ne():U()):(Te=!0,le=15,ue()):ue()},1e3)}}function re(){et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(m);let v=!1;const _=()=>{v||(v=!0,m.remove(),setTimeout(()=>f(),50))};m.addEventListener("click",_),setTimeout(_,3500)}function ve(I,C,w){var B,R;const k=(r.gcDefs||[]).find(O=>{var Q;return O.name===I||((Q=O.name)==null?void 0:Q.toLowerCase().trim())===(I==null?void 0:I.toLowerCase().trim())}),f={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[k==null?void 0:k.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",m={purple:"#b06ce0",light_blue:"#00d4ef"}[k==null?void 0:k.color]||"#b06ce0",x=(k==null?void 0:k.name)||I,b=(k==null?void 0:k.effect)||((B=Ve[I])==null?void 0:B.desc)||"",v=k!=null&&k.image_url?`/manager-wars/icons/${k.image_url}`:null,_=((R=Ve[I])==null?void 0:R.icon)||"⚡",$=C===o?"Vous":r[C+"Name"]||"Adversaire",A=document.createElement("div");A.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",A.innerHTML=`
      <style>
      ...
      </div>`,document.body.appendChild(w),(k=w.querySelector("#bp-close"))==null||k.addEventListener("click",()=>w.remove()),w.querySelectorAll(".bp-item").forEach(f=>{f.addEventListener("click",async()=>{const m=f.dataset.cid,x=C.find(v=>v.cardId===m);if(!x)return;const b=(I[x._line]||[]).find(v=>v.cardId===m);b&&(b.boost=(b.boost||0)+r.boostValue),w.remove(),await j({[o+"Team"]:I,boostUsed:!0})})})}function ze(I=null){var S,q;if(!(r.phase===o+"-attack")){p("Remplacement uniquement avant votre attaque","warning");return}const w=r[o+"Team"],k=r["usedSubIds_"+o]||[],f=r.maxSubs||3;if(k.length>=f){p(`Maximum ${f} remplacements atteint`,"warning");return}const m=Object.entries(w).flatMap(([B,R])=>(R||[]).filter(O=>O.used).map(O=>({...O,_line:B}))),x=(r[o+"Subs"]||[]).filter(B=>!k.includes(B.cardId));if(!m.length){p("Aucun joueur utilisé à remplacer","warning");return}if(!x.length){p("Aucun remplaçant disponible","warning");return}let b=Math.max(0,m.findIndex(B=>B.cardId===I));const v=((S=m[b])==null?void 0:S._line)||((q=m[b])==null?void 0:q.job);let _=Math.max(0,x.findIndex(B=>B.job===v)),T=!1;const $=document.createElement("div");$.id="pvp-sub-overlay",$.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function A(){var se,fe,me,ke,Ee,Ce;const B=m[b],R=x[_],O=Math.min(130,Math.round((window.innerWidth-90)/2)),Q=Math.round(O*1.35),K=je=>`background:rgba(255,255,255,0.12);border:none;color:${je?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${je?"default":"pointer"};flex-shrink:0`;$.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(b),(A=b.querySelector("#pvp-end-home"))==null||A.addEventListener("click",()=>{b.remove(),We(e),l("home")})}const U=E.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},w=>{const k=w.new;try{if(k.status==="finished"||k.forfeit){if(u)return;u=!0,D&&(clearInterval(D),D=null),k.game_state&&(r=k.game_state),j(k);return}if(k.game_state){const f=r;r=k.game_state;const m=r._lastGC;if(m&&m.seq&&!P.has(m.seq)&&(P.add(m.seq),m.by!==o)){we(m.type,m.by,()=>pe());return}const x=f[o+"Score"]||0,b=f[n+"Score"]||0,v=r[o+"Score"]||0,_=r[n+"Score"]||0,T=v>x,$=_>b;if((T||$)&&!F.has(r.round)){F.add(r.round);const A=[...r.log||[]].reverse().find(q=>q.isGoal),S=((A==null?void 0:A.homePlayers)||[]).map(q=>({name:q.name,note:q.note,portrait:q.portrait,job:q.job}));_e(S,v,_,T,()=>pe());return}pe()}}catch(f){console.error("[PvP] crash:",f)}}).subscribe();async function oe(w){Object.assign(r,w),r.lastActionAt=new Date().toISOString();const{error:k}=await E.from("matches").update({game_state:r}).eq("id",i);k&&p("Erreur de synchronisation","error");try{pe()}catch(f){console.error("[PvP] renderPvpScreen crash:",f)}}async function de(){if(u)return;u=!0,D&&(clearInterval(D),D=null);const w=s?y.away_id:y.home_id,k=s?"p2":"p1",f=s?"p1":"p2",m={...r,[k+"Score"]:3,[f+"Score"]:0,phase:"finished"};try{await E.from("matches").update({status:"finished",forfeit:!0,winner_id:w,home_score:m.p1Score||0,away_score:m.p2Score||0,game_state:m}).eq("id",i)}catch{}try{E.removeChannel(U)}catch{}setTimeout(()=>{We(e),l("home")},800)}const N={BOOST_STAT:({value:w=1,count:k=1,roles:f=[]},m,x)=>{const b=m[o+"Team"],v=Object.entries(b).filter(([_])=>!f.length||f.includes(_)).flatMap(([_,T])=>T.filter($=>!$.used).map($=>({...$,_line:_})));if(!v.length){m.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),x(m);return}Y(v,k,`Choisir ${k} joueur(s) à booster (+${w})`,_=>{_.forEach(T=>{const $=(b[T._line]||[]).find(A=>A.cardId===T.cardId);$&&($.boost=($.boost||0)+w,m.log.push({text:`⚡ +${w} sur ${$.name}`,type:"info"}))}),m[o+"Team"]=b,x(m)})},DEBUFF_STAT:({value:w=1,count:k=1,roles:f=[],target:m="ai"},x,b)=>{const v=m==="home"?o:n,_=x[v+"Team"],T=m==="home"?"allié":"adverse",$=Object.entries(_).filter(([A])=>!f.length||f.includes(A)).flatMap(([A,S])=>S.map(q=>({...q,_line:A})));if(!$.length){x.log.push({text:`🎯 Aucun joueur ${T}`,type:"info"}),b(x);return}Y($,k,`Choisir ${k} joueur(s) ${T}(s) (-${w})`,A=>{A.forEach(S=>{const q=(_[S._line]||[]).find(B=>B.cardId===S.cardId);q&&(q.boost=(q.boost||0)-w,x.log.push({text:`🎯 -${w} sur ${q.name}`,type:"info"}))}),x[v+"Team"]=_,b(x)})},GRAY_PLAYER:({count:w=1,roles:k=[],target:f="ai"},m,x)=>{const b=f==="home"?o:n,v=m[b+"Team"],_=f==="home"?"allié":"adverse",T=Object.entries(v).filter(([$])=>!k.length||k.includes($)).flatMap(([$,A])=>A.filter(S=>!S.used).map(S=>({...S,_line:$})));if(!T.length){m.log.push({text:`❌ Aucun joueur ${_}`,type:"info"}),x(m);return}Y(T,w,`Choisir ${w} joueur(s) ${_}(s) à exclure`,$=>{const A="usedCardIds_"+b,S=new Set(m[A]||[]);$.forEach(q=>{const B=(v[q._line]||[]).find(R=>R.cardId===q.cardId);B&&(B.used=!0,S.add(q.cardId),m.log.push({text:`❌ ${B.name} exclu !`,type:"info"}))}),m[A]=[...S],m[b+"Team"]=v,x(m)})},REVIVE_PLAYER:({count:w=1,roles:k=[]},f,m)=>{const x=f[o+"Team"],b=Object.entries(x).filter(([v])=>!k.length||k.includes(v)).flatMap(([v,_])=>_.filter(T=>T.used).map(T=>({...T,_line:v})));if(!b.length){f.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),m(f);return}Y(b,w,`Choisir ${w} joueur(s) à ressusciter`,v=>{v.forEach(_=>{const T=(x[_._line]||[]).find($=>$.cardId===_.cardId);T&&(T.used=!1,f.log.push({text:`💫 ${T.name} ressuscité !`,type:"info"}))}),f[o+"Team"]=x,m(f)})},REMOVE_GOAL:({},w,k)=>{const f=n+"Score";w[f]>0?(w[f]--,w.log.push({text:"🚫 Dernier but annulé !",type:"info"})):w.log.push({text:"🚫 Aucun but à annuler",type:"info"}),k(w)},ADD_GOAL_DRAW:({},w,k)=>{w[o+"Score"]===w[n+"Score"]?(w[o+"Score"]++,w.log.push({text:"🎯 But bonus !",type:"info"})):w.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),k(w)},ADD_SUB:({value:w=1},k,f)=>{k.maxSubs=(k.maxSubs||3)+w,k.log.push({text:`🔄 +${w} remplacement(s)`,type:"info"}),f(k)},CUSTOM:({},w,k)=>k(w)};function Y(w,k,f,m){const x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let b=[];function v(){var T,$;const _=w.map(A=>{const S={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[A._line]||"#555",q=Me(A,A._line)+(A.boost||0),R=b.find(Q=>Q.cardId===A.cardId)?"#FFD700":"rgba(255,255,255,0.25)",O=A.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${A.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${R};background:${S};overflow:hidden;cursor:pointer;${O}">
        ...
        </div>`,(T=x.querySelector("#pp-close"))==null||T.addEventListener("click",()=>x.remove()),x.querySelectorAll(".pp-item").forEach(A=>{A.addEventListener("click",()=>{const S=A.dataset.cid,q=w.find(R=>R.cardId===S),B=b.findIndex(R=>R.cardId===S);q&&(B>-1?b.splice(B,1):b.length<k&&b.push(q),v())})}),($=x.querySelector("#pp-confirm"))==null||$.addEventListener("click",()=>{x.remove(),m(b)})}v(),document.body.appendChild(x)}async function H(w,k){const m=(r["gcCardsFull_"+o]||[]).find(_=>_.id===w),x=(m==null?void 0:m._gcDef)||(r.gcDefs||[]).find(_=>{var T;return _.name===k||((T=_.name)==null?void 0:T.toLowerCase().trim())===(k==null?void 0:k.toLowerCase().trim())}),b=[...r["usedGc_"+o]||[],w],v={type:k,by:o,seq:(r._gcAnimSeq||0)+1};P.add(v.seq),we(k,o,async()=>{if(x!=null&&x.effect_type&&x.effect_type!=="CUSTOM"){const T=N[x.effect_type];if(T){const $={...r};T(x.effect_params||{},$,async A=>{A["usedGc_"+o]=b,A._lastGC=v,A._gcAnimSeq=v.seq,await oe(A)});return}}const _={...r};switch(k){case"Remplacement+":_.maxSubs=(_.maxSubs||3)+1,_.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const T=n+"Score";_[T]>0&&(_[T]--,_.log.push({text:"🚫 But annulé",type:"info"}));break}}_["usedGc_"+o]=b,_._lastGC=v,_._gcAnimSeq=v.seq,await oe(_)})}function G(w,k){const f="usedCardIds_"+w,m=new Set(r[f]||[]);k.forEach(x=>m.add(x)),r[f]=[...m]}function re(){for(const w of["p1","p2"]){const k=new Set(r["usedCardIds_"+w]||[]),f=r[w+"Team"];if(f)for(const m of["GK","DEF","MIL","ATT"])(f[m]||[]).forEach(x=>{x.used=k.has(x.cardId)})}}function pe(){var Ze,Qe,nt,gt,le,Te;if(r.phase==="reveal")return ye();if(r.phase==="midfield")return ve();if(r.phase==="finished")return C();const w=r[o+"Team"],k=r[n+"Team"];re();const f=r[o+"Score"],m=r[n+"Score"],x=r[o+"Name"],b=r[n+"Name"],v=r.phase===o+"-attack",_=r.phase===o+"-defense",T=Array.isArray(r["selected_"+o])?r["selected_"+o]:[],$=T.map(te=>te.cardId),A=window.innerWidth>=700,S=r[o+"Subs"]||[],q=r["usedSubIds_"+o]||[],B=S.filter(te=>!q.includes(te.cardId)),R=r["gcCardsFull_"+o]||[],O=r["usedGc_"+o]||[],Q=R.filter(te=>!O.includes(te.id)),K=r.boostOwner===o&&!r.boostUsed,ie=!["GK","DEF","MIL","ATT"].some(te=>(k[te]||[]).some(ue=>!ue.used)),se=[...w.MIL||[],...w.ATT||[]].filter(te=>!te.used),fe=v&&ie&&se.length===0?[...w.GK||[],...w.DEF||[]].filter(te=>!te.used).map(te=>te.cardId):[];function me(te,ue,ee){var kt,Ut;const ce=te._gcDef,qe={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ce==null?void 0:ce.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",Ae={purple:"#b06ce0",light_blue:"#00d4ef"}[ce==null?void 0:ce.color]||"#b06ce0",tt=(ce==null?void 0:ce.name)||te.gc_type,it=(ce==null?void 0:ce.effect)||((kt=Ve[te.gc_type])==null?void 0:kt.desc)||"",rt=ce!=null&&ce.image_url?`/manager-wars/icons/${ce.image_url}`:null,st=((Ut=Ve[te.gc_type])==null?void 0:Ut.icon)||"⚡",ct=Math.round(ee*.22),$t=Math.round(ee*.22),pt=ee-ct-$t,ht=tt.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${te.id}" data-gc-type="${te.gc_type}"
      ...
      </div>`}function ke(te,ue){return`<div id="pvp-boost-card"
      ...
      </div>`}const Ee=(te,ue)=>ue?ke(130,175):me(te,130,175),Ce=(te,ue)=>ue?ke(68,95):me(te,68,95),je=A?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",Re=v?be(o)?`<button id="pvp-action" style="${je};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${T.length===0?"disabled":""}>⚔️ ATTAQUEZ <span id="pvp-timer"></span></button>`:`<button id="pvp-action" data-pass="1" style="${je};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER <span id="pvp-timer"></span></button>`:_?`<button id="pvp-action" style="${je};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${T.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="pvp-timer"></span></button>`:`<div style="font-size:11px;color:rgba(255,255,255,0.3);text-align:center;padding:4px">⏳ Tour de ${b}</div>`,Be=v&&!be(o)?'<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">Aucun attaquant — passez la main</div>':v||_?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${T.length}/3 sélectionné(s)</div>`:"",Oe=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${A?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
    ...
    </div>`,Fe=v?"attack":_?"defense":"idle",Ke=`<div style="overflow:hidden;min-width:0;flex:1;min-height:0;display:flex;flex-direction:column" id="match-field">
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="width:min(90vw,420px)">${ut(r[n+"Team"],r[n+"Formation"],null,[],300,300)}</div>
        ...
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${Q.map((se,fe)=>{const me=fe<Q.length-1?at(se,Q[fe+1]):null,ke=!me||me==="#ff3333"||me==="#cc2222",Ee=me==="#00ff88"?"+2":me==="#FFD700"?"+1":"";return`<div class="duel-card duel-card-${ie}" data-idx="${fe}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease,transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
              ${Ye({...se,note:Me(se,"MIL"),_line:"MIL"},58,78)}
            </div>
            ${fe<Q.length-1?`<div class="duel-link duel-link-${ie}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${ke?"rgba(255,255,255,0.12)":me};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${ke?"none":`0 0 8px ${me}`}">
              ${Ee?`<span style="position:absolute;top:-13px;left:50%;transform:translateX(-50%);font-size:8px;font-weight:900;color:${me}">${Ee}</span>`:""}
            </div>`:""}`}).join("")}
        </div>
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div id="duel-shock" style="position:absolute;left:50%;top:50%;width:120px;height:120px;border-radius:50%;border:6px solid #FFD700;opacity:0;pointer-events:none"></div>
      <div id="pvp-duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center,rgba(10,61,30,.4),rgba(10,61,30,.92))"></div>
    </div>`;const T=(Q,K)=>e.querySelectorAll(Q).forEach((ie,se)=>{setTimeout(()=>{ie.style.opacity="1",ie.style.transform="translateY(0) scale(1)"},K+se*90)});T(".duel-card-me",150),T(".duel-card-opp",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((Q,K)=>setTimeout(()=>{Q.style.opacity="1"},K*70)),900),setTimeout(()=>{const Q=e.querySelector("#pvp-vs");Q&&(Q.style.opacity="1",Q.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(K=>K.style.opacity="1")},1250);function $(Q,K,ie){const se=document.getElementById(Q);if(!se)return;const fe=performance.now(),me=ke=>{const Ee=Math.min(1,(ke-fe)/ie);se.textContent=Math.round(K*(1-Math.pow(1-Ee,3))),Ee<1?requestAnimationFrame(me):se.textContent=K};requestAnimationFrame(me)}setTimeout(()=>{$("pvp-score-me",x,800),$("pvp-score-opp",b,800)},1500);const A=r.p1Team.MIL||[],S=r.p2Team.MIL||[],q=f(A)+m(A),B=f(S)+m(S),R=q>=B?"p1":"p2";let O=r.boostValue;O==null&&(O=oi(),r.boostValue=O,r.boostOwner=R,r.boostUsed=!1),Se=!0,setTimeout(()=>{const Q=e.querySelector("#duel-row-"+(v?"me":"opp")),K=e.querySelector("#duel-row-"+(v?"opp":"me")),ie=document.getElementById("pvp-score-me"),se=document.getElementById("pvp-score-opp"),fe=v?ie:se,me=v?se:ie;fe&&(fe.style.fontSize="80px",fe.style.color=v?"#FFD700":"#ff6b6b",fe.style.animation="duelPulse .5s ease"+(v?",duelGlow 1.5s ease infinite .5s":"")),me&&(me.style.opacity="0.25"),setTimeout(()=>{Q&&(Q.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",Q.style.zIndex="5"),setTimeout(()=>{const ke=document.getElementById("duel-shock");ke&&(ke.style.animation="shockwave .5s ease-out forwards"),K&&(K.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var je;const ke=document.getElementById("pvp-duel-finale");if(!ke)return;const Ee=r.boostOwner===o?'<div style="background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:3px solid #cdeffd;border-radius:18px;padding:20px 34px;text-align:center;animation:boostFlipIn .7s cubic-bezier(.34,1.56,.64,1) both;box-shadow:0 10px 36px rgba(135,206,235,.5)"><div style="font-size:10px;color:rgba(0,0,0,.6);letter-spacing:2px;text-transform:uppercase;margin-bottom:6px;font-weight:700">Carte Boost obtenue</div><div style="font-size:46px;line-height:1">⚡</div><div style="font-size:50px;font-weight:900;color:#063;line-height:1.1">+'+O+`</div><div style="font-size:10px;color:rgba(0,0,0,.55);margin-top:4px">Applicable sur n'importe quel joueur</div></div>`:"",Ce=o==="p1"?'<button id="pvp-start-match" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">▶ Commencer le match</button>':`<div style="font-size:14px;color:rgba(255,255,255,0.5);text-align:center;margin-top:8px;animation:fadeUp .4s ease both">⏳ En attente de l'adversaire...</div>`;ke.innerHTML='<div style="font-size:22px;font-weight:900;color:#fff;text-align:center;animation:fadeUp .4s ease both;text-shadow:0 2px 12px rgba(0,0,0,.5)">'+(v?"⚽ "+r[o+"Name"]+"<br>gagne le milieu et attaque !":"😔 "+r[n+"Name"]+"<br>gagne l'engagement et attaque !")+"</div>"+Ee+Ce,ke.style.transition="opacity .45s ease",ke.style.opacity="1",ke.style.pointerEvents="auto",(je=document.getElementById("pvp-start-match"))==null||je.addEventListener("click",async()=>{const Re=R;await oe({phase:Re+"-attack",attacker:Re,round:1,boostValue:O,boostUsed:!1,boostOwner:Re})})},600)},700)},2800)}function _e(w,k,f,m,x){const b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:900;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;overflow:hidden;cursor:pointer";const v=Array.from({length:10},(A,S)=>`<div style="position:absolute;font-size:${16+Math.floor(Math.random()*24)}px;top:${5+Math.floor(Math.random()*65)}%;left:${3+Math.floor(Math.random()*94)}%;animation:fw${S%2===0?"A":"B"} ${.7+Math.random()*.7}s ease ${Math.random()*.9}s both;opacity:0">${["✨","🌟","⭐","💥","🎇","🎆","🔥","🌈"][S%8]}</div>`).join(""),_={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};b.innerHTML=`
      <style>
        @keyframes butPop{0%{transform:scale(0) rotate(-8deg);opacity:0}55%{transform:scale(1.25) rotate(2deg)}85%{transform:scale(0.92) rotate(-1deg)}100%{transform:scale(1);opacity:1}}
        @keyframes ballIn{0%{transform:translate(-70px,18px);opacity:0}65%{opacity:1}100%{transform:translate(26px,-8px);opacity:1}}
        @keyframes scoreIn{from{opacity:0;transform:translateY(-12px)}to{opacity:1;transform:translateY(0)}}
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(b);let T=!1;const $=()=>{T||(T=!0,b.remove(),setTimeout(()=>x(),50))};b.addEventListener("click",$),setTimeout($,3500)}function we(w,k,f){var O,Q;const m=(r.gcDefs||[]).find(K=>{var ie;return K.name===w||((ie=K.name)==null?void 0:ie.toLowerCase().trim())===(w==null?void 0:w.toLowerCase().trim())}),x={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[m==null?void 0:m.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",b={purple:"#b06ce0",light_blue:"#00d4ef"}[m==null?void 0:m.color]||"#b06ce0",v=(m==null?void 0:m.name)||w,_=(m==null?void 0:m.effect)||((O=Ve[w])==null?void 0:O.desc)||"",T=m!=null&&m.image_url?`/manager-wars/icons/${m.image_url}`:null,$=((Q=Ve[w])==null?void 0:Q.icon)||"⚡",S=k===o?"Vous":r[k+"Name"]||"Adversaire",q=document.createElement("div");q.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",q.innerHTML=`
      <style>
        @keyframes gcFlipIn{0%{transform:perspective(800px) rotateY(90deg) scale(.7);opacity:0}55%{transform:perspective(800px) rotateY(-12deg) scale(1.08);opacity:1}100%{transform:perspective(800px) rotateY(0) scale(1);opacity:1}}
      ...
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${k.map(x=>{const b={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[x._line]||"#555",v=Me(x,x._line)+(x.boost||0);return`<div class="bp-item" data-cid="${x.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${b};overflow:hidden;cursor:pointer">
      ...
      </div>`,document.body.appendChild(f),(m=f.querySelector("#bp-close"))==null||m.addEventListener("click",()=>f.remove()),f.querySelectorAll(".bp-item").forEach(x=>{x.addEventListener("click",async()=>{const b=x.dataset.cid,v=k.find(T=>T.cardId===b);if(!v)return;const _=(w[v._line]||[]).find(T=>T.cardId===b);_&&(_.boost=(_.boost||0)+r.boostValue),f.remove(),await oe({[o+"Team"]:w,boostUsed:!0})})})}function xe(w=null){var B,R;if(!(r.phase===o+"-attack")){p("Remplacement uniquement avant votre attaque","warning");return}const f=r[o+"Team"],m=r["usedSubIds_"+o]||[],x=r.maxSubs||3;if(m.length>=x){p(`Maximum ${x} remplacements atteint`,"warning");return}const b=Object.entries(f).flatMap(([O,Q])=>(Q||[]).filter(K=>K.used).map(K=>({...K,_line:O}))),v=(r[o+"Subs"]||[]).filter(O=>!m.includes(O.cardId));if(!b.length){p("Aucun joueur utilisé à remplacer","warning");return}if(!v.length){p("Aucun remplaçant disponible","warning");return}let _=Math.max(0,b.findIndex(O=>O.cardId===w));const T=((B=b[_])==null?void 0:B._line)||((R=b[_])==null?void 0:R.job);let $=Math.max(0,v.findIndex(O=>O.job===T)),A=!1;const S=document.createElement("div");S.id="pvp-sub-overlay",S.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function q(){var me,ke,Ee,Ce,je,Re;const O=b[_],Q=v[$],K=Math.min(130,Math.round((window.innerWidth-90)/2)),ie=Math.round(K*1.35),se=Be=>`background:rgba(255,255,255,0.12);border:none;color:${Be?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Be?"default":"pointer"};flex-shrink:0`;S.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        <div id="pin-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
          ...
          <button id="pin-down" style="${se($>=v.length-1)}" ${$>=v.length-1?"disabled":""}>▼</button>
      ...
      <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
        <button id="psub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
      </div>`,(me=S.querySelector("#psub-close"))==null||me.addEventListener("click",()=>S.remove()),(ke=S.querySelector("#pout-up"))==null||ke.addEventListener("click",()=>{_>0&&(_--,q())}),(Ee=S.querySelector("#pout-down"))==null||Ee.addEventListener("click",()=>{_<b.length-1&&(_++,q())}),(Ce=S.querySelector("#pin-up"))==null||Ce.addEventListener("click",()=>{$>0&&($--,q())}),(je=S.querySelector("#pin-down"))==null||je.addEventListener("click",()=>{$<v.length-1&&($++,q())});const fe=(Be,Oe,Fe,Ke)=>{const Ne=S.querySelector("#"+Be);if(!Ne)return;let He=0;Ne.addEventListener("touchstart",Ue=>{He=Ue.touches[0].clientY},{passive:!0}),Ne.addEventListener("touchend",Ue=>{const Je=Ue.changedTouches[0].clientY-He;if(Math.abs(Je)<30)return;const Ze=Oe();Je<0&&Ze<Ke-1?(Fe(Ze+1),q()):Je>0&&Ze>0&&(Fe(Ze-1),q())},{passive:!0})};fe("pin-panel",()=>$,Be=>$=Be,v.length),fe("pout-panel",()=>_,Be=>_=Be,b.length),(Re=S.querySelector("#psub-confirm"))==null||Re.addEventListener("click",async Be=>{if(Be.preventDefault(),Be.stopPropagation(),A)return;A=!0;const Oe=b[_],Fe=v[$];if(!Oe||!Fe)return;const Ke=Oe._line,Ne=(f[Ke]||[]).findIndex(Je=>Je.cardId===Oe.cardId);if(Ne===-1){p("Erreur : joueur introuvable","error"),S.remove();return}const He={...Fe,_line:Ke,position:Oe.position,used:!1,boost:0};f[Ke].splice(Ne,1,He);const Ue=[...m,Fe.cardId];S.remove(),X(Oe,Fe,async()=>{await oe({[o+"Team"]:f,[n+"Team"]:r[n+"Team"],["usedSubIds_"+o]:Ue})})})}document.body.appendChild(S),q()}function X(w,k,f){const m={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const b=(T,$,A)=>`<div style="text-align:center">
      ...
      <div style="font-size:18px;font-weight:900;color:#ff6b6b">${r[n+"Name"]}</div>
      <div style="width:min(90vw,420px)">${ut(r[n+"Team"],r[n+"Formation"],null,[],300,300)}</div>
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(w),(k=w.querySelector("#pvp-opp-close"))==null||k.addEventListener("click",()=>w.remove())}function ge(){var m;const w=r.log||[],k=document.createElement("div");k.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column";const f=x=>{var v,_,T;if(x.type==="duel"){const $=x.isGoal,A=x.homeScored&&o==="p1"||!x.homeScored&&$&&o==="p2",S=x.homeScored?"#FFD700":$?"#ff6b6b":"rgba(255,255,255,0.3)",q=$?A?"⚽ BUT !":"⚽ BUT adversaire !":(v=x.homePlayers)!=null&&v.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${$?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${S};margin-bottom:4px">
      ...
      </div>`,document.body.appendChild(k),(m=k.querySelector("#pvp-hist-close"))==null||m.addEventListener("click",()=>k.remove())}async function J(){if(r.phase!==o+"-attack")return;const w=o==="p1"?"p2":"p1",k=(r.round||0)+1,f=[...r.log||[]];f.push({type:"info",text:`⏭️ ${r[o+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const m=$e(r),x=be(w),b=m||!x?"finished":w+"-attack";await oe({["selected_"+o]:[],modifiers:{...r.modifiers,[o]:{}},pendingAttack:null,phase:b,attacker:w,round:k,log:f}),b==="finished"&&await I(r)}async function Z(){const w=r[o+"Team"],k=!["GK","DEF","MIL","ATT"].some(b=>(r[n+"Team"][b]||[]).some(v=>!v.used)),f=(r["selected_"+o]||[]).map(b=>{const v=(w[b._role]||[]).find(q=>q.cardId===b.cardId)||b,_=k&&["GK","DEF"].includes(b._role),T=w[b._role]||[],$=T.findIndex(q=>q.cardId===b.cardId),A=lt(T.length),S=$>=0?A[$]:v._col??1;return{...v,_line:b._role,_col:S,..._?{note_a:Math.max(1,Number(v.note_a)||0)}:{}}});if(!f.length)return;const m=Nt(f,r.modifiers[o]||{});G(o,f.map(b=>b.cardId)),f.forEach(b=>{const v=(w[b._role]||[]).find(_=>_.cardId===b.cardId);v&&(v.used=!0)}),r["selected_"+o]=[],pe();const x=[...r.log||[]];if(k){const b=(r[o+"Score"]||0)+1,v=f.map(S=>({name:S.name,note:Me(S,S._line||"ATT"),portrait:Pe(S),job:S.job}));x.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:v,homeTotal:m.total,aiTotal:0});const _=(r.round||0)+1,T=o==="p1"?"p2":"p1",$={...r,[o+"Team"]:w,[o+"Score"]:b},A=$e($);F.add(_),_e(v,b,r[n+"Score"]||0,!0,async()=>{await oe({[o+"Team"]:w,[o+"Score"]:b,["selected_"+o]:[],modifiers:{...r.modifiers,[o]:{}},pendingAttack:null,phase:A?"finished":T+"-attack",attacker:T,round:_,log:x}),A&&await I({...r,[o+"Score"]:b})});return}x.push({type:"pending",text:`⚔️ ${r[o+"Name"]} attaque (${m.total})`}),await oe({[o+"Team"]:w,[n+"Team"]:r[n+"Team"],pendingAttack:{...m,players:f,side:o},["selected_"+o]:[],modifiers:{...r.modifiers,[o]:{}},phase:n+"-defense",log:x})}async function V(){const w=r[o+"Team"],k=(r["selected_"+o]||[]).map(O=>{const Q=(w[O._role]||[]).find(me=>me.cardId===O.cardId)||O,K=w[O._role]||[],ie=K.findIndex(me=>me.cardId===O.cardId),se=lt(K.length),fe=ie>=0?se[ie]:Q._col??1;return{...Q,_line:O._role,_col:fe}}),f=Rt(k,r.modifiers[o]||{});G(o,k.map(O=>O.cardId)),k.forEach(O=>{const Q=(w[O._role]||[]).find(K=>K.cardId===O.cardId);Q&&(Q.used=!0)}),r["selected_"+o]=[],pe();const m=Ot(r.pendingAttack.total,f.total,r.modifiers[o]||{}),x=r.pendingAttack.side,b=m==="attack"||(m==null?void 0:m.goal),v=x==="p1"?"p2":"p1",_=(r.round||0)+1,T=(r.pendingAttack.players||[]).map(O=>({name:O.name,note:Me(O,O._line||"ATT"),portrait:Pe(O),job:O.job})),$=[...r.log||[]];$.push({type:"duel",isGoal:b,homeScored:b&&x===o,text:b?`⚽ BUT de ${r[x+"Name"]} ! (${r.pendingAttack.total} vs ${f.total})`:`✋ Attaque stoppée (${r.pendingAttack.total} vs ${f.total})`,homePlayers:T,aiPlayers:k.map(O=>({name:O.name,note:Me(O,O._line||"DEF"),portrait:Pe(O),job:O.job})),homeTotal:r.pendingAttack.total,aiTotal:f.total});const A=b?(r[x+"Score"]||0)+1:r[x+"Score"]||0,S={...r,[o+"Team"]:w,[x+"Score"]:A},q=$e(S),B=q?"finished":v+"-attack",R=async()=>{await oe({[o+"Team"]:w,[n+"Team"]:r[n+"Team"],[x+"Score"]:A,["selected_"+o]:[],modifiers:{...r.modifiers,[o]:{}},pendingAttack:null,phase:B,attacker:v,round:_,log:$}),(B==="finished"||q)&&await I({...r,[x+"Score"]:A})};if(b){const O=x===o,Q=O?A:r[o+"Score"]||0,K=O?r[n+"Score"]||0:A;F.add(_),_e(T,Q,K,O,R)}else await R()}function ae(w){return["MIL","ATT"].some(k=>(w[k]||[]).some(f=>!f.used))}function he(w){return["GK","DEF","MIL","ATT"].some(k=>(w[k]||[]).some(f=>!f.used))}function Le(w){return he(w)&&!ae(w)}function be(w){const k=r[w+"Team"],f=r[(w==="p1"?"p2":"p1")+"Team"];return!!(ae(k)||!he(f)&&Le(k))}function $e(w){const k=["MIL","ATT"].some($=>(w.p1Team[$]||[]).some(A=>!A.used)),f=["MIL","ATT"].some($=>(w.p2Team[$]||[]).some(A=>!A.used)),m=he(w.p1Team),x=he(w.p2Team);return!k&&!(!x&&m)&&(!f&&!(!m&&x))}function De(w){const k=w.p1Score||0,f=w.p2Score||0;return k===f?null:k>f?y.home_id:y.away_id}async function I(w){try{const k=De(w),f=k?y.home_id===k?y.away_id:y.home_id:null;await E.from("matches").update({status:"finished",winner_id:k,home_score:w.p1Score||0,away_score:w.p2Score||0}).eq("id",i),k&&f&&ki(k,f).catch(()=>{})}catch(k){console.error("[PvP] finishMatch:",k)}}function C(){var x;const w=r[o+"Score"],k=r[n+"Score"],f=w>k,m=w===k;et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="background:linear-gradient(160deg,#1a0a2e,#3b1a6e);border-radius:20px;padding:32px 24px;max-width:320px;width:100%;text-align:center;color:#fff;box-shadow:0 8px 40px rgba(0,0,0,0.6)">
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">🏆 Retour à la Mini League</button>`,document.body.appendChild(_),(q=_.querySelector("#pvp-end-home"))==null||q.addEventListener("click",()=>{_.remove(),We(e),o?l("mini-league",{openLeagueId:o}):l("mini-league")})}const oe=E.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},f=>{const m=f.new;try{if(m.status==="finished"||m.forfeit){if(M||(M=!0,W&&(clearInterval(W),W=null),m.game_state&&(u=m.game_state),u.phase==="finished"&&!m.forfeit&&document.getElementById("pvp-home")))return;U(m);return}if(m.game_state){const x=u;u=m.game_state;const b=u._lastGC;if(b&&b.seq&&!j.has(b.seq)&&(j.add(b.seq),b.by!==n)){ze(b.type,b.by,()=>ye());return}const v=x[n+"Score"]||0,_=x[g+"Score"]||0,T=u[n+"Score"]||0,$=u[g+"Score"]||0,A=T>v,S=$>_;if((A||S)&&!P.has(u.round)){P.add(u.round);const q=[...u.log||[]].reverse().find(R=>R.isGoal),B=((q==null?void 0:q.homePlayers)||[]).map(R=>({name:R.name,note:R.note,portrait:R.portrait,job:R.job}));we(B,T,$,A,()=>ye());return}ye()}}catch(x){console.error("[PvP] crash:",x)}}).subscribe();async function de(f){Object.assign(u,f),u.lastActionAt=new Date().toISOString();const{error:m}=await E.from("matches").update({game_state:u}).eq("id",i);m&&p("Erreur de synchronisation","error");try{ye()}catch(x){console.error("[PvP] renderPvpScreen crash:",x)}}async function N(){if(M)return;M=!0,W&&(clearInterval(W),W=null);const f=s?z.away_id:z.home_id,m=s?"p2":"p1",x=s?"p1":"p2",b={...u,[m+"Score"]:3,[x+"Score"]:0,phase:"finished"};try{await E.from("matches").update({status:"finished",forfeit:!0,winner_id:f,home_score:b.p1Score||0,away_score:b.p2Score||0,game_state:b}).eq("id",i)}catch{}try{E.removeChannel(oe)}catch{}setTimeout(()=>{We(e),l("mini-league")},800)}const Y={BOOST_STAT:({value:f=1,count:m=1,roles:x=[]},b,v)=>{const _=b[n+"Team"],T=Object.entries(_).filter(([$])=>!x.length||x.includes($)).flatMap(([$,A])=>A.filter(S=>!S.used).map(S=>({...S,_line:$})));if(!T.length){b.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),v(b);return}H(T,m,`Choisir ${m} joueur(s) à booster (+${f})`,$=>{$.forEach(A=>{const S=(_[A._line]||[]).find(q=>q.cardId===A.cardId);S&&(S.boost=(S.boost||0)+f,b.log.push({text:`⚡ +${f} sur ${S.name}`,type:"info"}))}),b[n+"Team"]=_,v(b)})},DEBUFF_STAT:({value:f=1,count:m=1,roles:x=[],target:b="ai"},v,_)=>{const T=b==="home"?n:g,$=v[T+"Team"],A=b==="home"?"allié":"adverse",S=Object.entries($).filter(([q])=>!x.length||x.includes(q)).flatMap(([q,B])=>B.map(R=>({...R,_line:q})));if(!S.length){v.log.push({text:`🎯 Aucun joueur ${A}`,type:"info"}),_(v);return}H(S,m,`Choisir ${m} joueur(s) ${A}(s) (-${f})`,q=>{q.forEach(B=>{const R=($[B._line]||[]).find(O=>O.cardId===B.cardId);R&&(R.boost=(R.boost||0)-f,v.log.push({text:`🎯 -${f} sur ${R.name}`,type:"info"}))}),v[T+"Team"]=$,_(v)})},GRAY_PLAYER:({count:f=1,roles:m=[],target:x="ai"},b,v)=>{const _=x==="home"?n:g,T=b[_+"Team"],$=x==="home"?"allié":"adverse",A=Object.entries(T).filter(([S])=>!m.length||m.includes(S)).flatMap(([S,q])=>q.filter(B=>!B.used).map(B=>({...B,_line:S})));if(!A.length){b.log.push({text:`❌ Aucun joueur ${$}`,type:"info"}),v(b);return}H(A,f,`Choisir ${f} joueur(s) ${$}(s) à exclure`,S=>{const q="usedCardIds_"+_,B=new Set(b[q]||[]);S.forEach(R=>{const O=(T[R._line]||[]).find(Q=>Q.cardId===R.cardId);O&&(O.used=!0,B.add(R.cardId),b.log.push({text:`❌ ${O.name} exclu !`,type:"info"}))}),b[q]=[...B],b[_+"Team"]=T,v(b)})},REVIVE_PLAYER:({count:f=1,roles:m=[]},x,b)=>{const v=x[n+"Team"],_=Object.entries(v).filter(([T])=>!m.length||m.includes(T)).flatMap(([T,$])=>$.filter(A=>A.used).map(A=>({...A,_line:T})));if(!_.length){x.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),b(x);return}H(_,f,`Choisir ${f} joueur(s) à ressusciter`,T=>{T.forEach($=>{const A=(v[$._line]||[]).find(S=>S.cardId===$.cardId);A&&(A.used=!1,x.log.push({text:`💫 ${A.name} ressuscité !`,type:"info"}))}),x[n+"Team"]=v,b(x)})},REMOVE_GOAL:({},f,m)=>{const x=g+"Score";f[x]>0?(f[x]--,f.log.push({text:"🚫 Dernier but annulé !",type:"info"})):f.log.push({text:"🚫 Aucun but à annuler",type:"info"}),m(f)},ADD_GOAL_DRAW:({},f,m)=>{f[n+"Score"]===f[g+"Score"]?(f[n+"Score"]++,f.log.push({text:"🎯 But bonus !",type:"info"})):f.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),m(f)},ADD_SUB:({value:f=1},m,x)=>{m.maxSubs=(m.maxSubs||3)+f,m.log.push({text:`🔄 +${f} remplacement(s)`,type:"info"}),x(m)},CUSTOM:({},f,m)=>m(f)};function H(f,m,x,b){const v=document.createElement("div");v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let _=[];function T(){var A,S;const $=f.map(q=>{const B={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[q._line]||"#555",R=Me(q,q._line)+(q.boost||0),Q=_.find(ie=>ie.cardId===q.cardId)?"#FFD700":"rgba(255,255,255,0.25)",K=q.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${q.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${Q};background:${B};overflow:hidden;cursor:pointer;${K}">
        ...
        </div>`,(A=v.querySelector("#pp-close"))==null||A.addEventListener("click",()=>v.remove()),v.querySelectorAll(".pp-item").forEach(q=>{q.addEventListener("click",()=>{const B=q.dataset.cid,R=f.find(Q=>Q.cardId===B),O=_.findIndex(Q=>Q.cardId===B);R&&(O>-1?_.splice(O,1):_.length<m&&_.push(R),T())})}),(S=v.querySelector("#pp-confirm"))==null||S.addEventListener("click",()=>{v.remove(),b(_)})}T(),document.body.appendChild(v)}async function G(f,m){const b=(u["gcCardsFull_"+n]||[]).find($=>$.id===f),v=(b==null?void 0:b._gcDef)||(u.gcDefs||[]).find($=>{var A;return $.name===m||((A=$.name)==null?void 0:A.toLowerCase().trim())===(m==null?void 0:m.toLowerCase().trim())}),_=[...u["usedGc_"+n]||[],f],T={type:m,by:n,seq:(u._gcAnimSeq||0)+1};j.add(T.seq),ze(m,n,async()=>{if(v!=null&&v.effect_type&&v.effect_type!=="CUSTOM"){const A=Y[v.effect_type];if(A){const S={...u};A(v.effect_params||{},S,async q=>{q["usedGc_"+n]=_,q._lastGC=T,q._gcAnimSeq=T.seq,await de(q)});return}}const $={...u};switch(m){case"Remplacement+":$.maxSubs=($.maxSubs||3)+1,$.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const A=g+"Score";$[A]>0&&($[A]--,$.log.push({text:"🚫 But annulé",type:"info"}));break}}$["usedGc_"+n]=_,$._lastGC=T,$._gcAnimSeq=T.seq,await de($)})}function re(f,m){const x="usedCardIds_"+f,b=new Set(u[x]||[]);m.forEach(v=>b.add(v)),u[x]=[...b]}function pe(){for(const f of["p1","p2"]){const m=new Set(u["usedCardIds_"+f]||[]),x=u[f+"Team"];if(x)for(const b of["GK","DEF","MIL","ATT"])(x[b]||[]).forEach(v=>{v.used=m.has(v.cardId)})}}function ye(){var nt,gt,le,Te,te,ue;if(u.phase==="reveal")return Se();if(u.phase==="midfield")return _e();if(u.phase==="finished")return w();const f=u[n+"Team"],m=u[g+"Team"];pe();const x=u[n+"Score"],b=u[g+"Score"],v=u[n+"Name"],_=u[g+"Name"],T=u.phase===n+"-attack",$=u.phase===n+"-defense",A=Array.isArray(u["selected_"+n])?u["selected_"+n]:[],S=A.map(ee=>ee.cardId),q=window.innerWidth>=700,B=u[n+"Subs"]||[],R=u["usedSubIds_"+n]||[],O=B.filter(ee=>!R.includes(ee.cardId)),Q=u["gcCardsFull_"+n]||[],K=u["usedGc_"+n]||[],ie=Q.filter(ee=>!K.includes(ee.id)),se=u.boostOwner===n&&!u.boostUsed,fe=!["GK","DEF","MIL","ATT"].some(ee=>(m[ee]||[]).some(ce=>!ce.used)),me=[...f.MIL||[],...f.ATT||[]].filter(ee=>!ee.used),ke=T&&fe&&me.length===0?[...f.GK||[],...f.DEF||[]].filter(ee=>!ee.used).map(ee=>ee.cardId):[];function Ee(ee,ce,qe){var Si,Li;const Ae=ee._gcDef,tt={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[Ae==null?void 0:Ae.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",it={purple:"#b06ce0",light_blue:"#00d4ef"}[Ae==null?void 0:Ae.color]||"#b06ce0",rt=(Ae==null?void 0:Ae.name)||ee.gc_type,st=(Ae==null?void 0:Ae.effect)||((Si=Ve[ee.gc_type])==null?void 0:Si.desc)||"",ct=Ae!=null&&Ae.image_url?`/manager-wars/icons/${Ae.image_url}`:null,$t=((Li=Ve[ee.gc_type])==null?void 0:Li.icon)||"⚡",pt=Math.round(qe*.22),ht=Math.round(qe*.22),kt=qe-pt-ht,Ut=rt.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${ee.id}" data-gc-type="${ee.gc_type}"
      ...
      </div>`}const je=(ee,ce)=>ce?Ce(130,175):Ee(ee,130,175),Re=(ee,ce)=>ce?Ce(68,95):Ee(ee,68,95),Be=q?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",Oe=T?$e(n)?`<button id="pvp-action" style="${Be};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${A.length===0?"disabled":""}>⚔️ ATTAQUEZ <span id="pvp-timer"></span></button>`:`<button id="pvp-action" data-pass="1" style="${Be};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER <span id="pvp-timer"></span></button>`:$?`<button id="pvp-action" style="${Be};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${A.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="pvp-timer"></span></button>`:`<div style="font-size:11px;color:rgba(255,255,255,0.3);text-align:center;padding:4px">⏳ Tour de ${_}</div>`,Fe=T&&!$e(n)?'<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">Aucun attaquant — passez la main</div>':T||$?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${A.length}/3 sélectionné(s)</div>`:"",Ke=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${q?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${ft(f,u[n+"Formation"],Ne,S,300,300,ke)}
      </div>
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="width:min(90vw,420px)">${ut(u[g+"Team"],u[g+"Formation"],null,[],300,300)}</div>
    </div>`,n==="p1"&&setTimeout(async()=>{await de({phase:"midfield"})},5e3)}let ve=!1;function _e(){if(ve)return;const f=u[n+"Team"].MIL||[],m=u[g+"Team"].MIL||[];function x(ie){return ie.reduce((se,fe)=>se+Me(fe,"MIL"),0)}function b(ie){let se=0;for(let fe=0;fe<ie.length-1;fe++){const me=at(ie[fe],ie[fe+1]);me==="#00ff88"?se+=2:me==="#FFD700"&&(se+=1)}return se}const v=x(f)+b(f),_=x(m)+b(m),T=v>=_;function $(ie,se,fe){return`<div id="duel-row-${fe}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
        ...
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${ie.map((me,ke)=>{const Ee=ke<ie.length-1?at(me,ie[ke+1]):null,Ce=!Ee||Ee==="#ff3333"||Ee==="#cc2222",je=Ee==="#00ff88"?"+2":Ee==="#FFD700"?"+1":"";return`<div class="duel-card duel-card-${fe}" data-idx="${ke}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease,transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
              ${Ye({...me,note:Me(me,"MIL"),_line:"MIL"},58,78)}
            </div>
            ${ke<ie.length-1?`<div class="duel-link duel-link-${fe}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${Ce?"rgba(255,255,255,0.12)":Ee};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${Ce?"none":`0 0 8px ${Ee}`}">
              ${je?`<span style="position:absolute;top:-13px;left:50%;transform:translateX(-50%);font-size:8px;font-weight:900;color:${Ee}">${je}</span>`:""}
            </div>`:""}`}).join("")}
        </div>
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(R);let O=!1;const Q=()=>{O||(O=!0,R.remove(),setTimeout(()=>x&&x(),50))};R.addEventListener("click",Q),setTimeout(Q,3e3)}function Ie(f,m){var R,O,Q,K;const b=(u["gcCardsFull_"+n]||[]).find(ie=>ie.id===f),v=b==null?void 0:b._gcDef,_={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[v==null?void 0:v.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",T={purple:"#b06ce0",light_blue:"#00d4ef"}[v==null?void 0:v.color]||"#b06ce0",$=(v==null?void 0:v.name)||m,A=(v==null?void 0:v.effect)||((R=Ve[m])==null?void 0:R.desc)||"Carte spéciale.",S=v!=null&&v.image_url?`/manager-wars/icons/${v.image_url}`:null,q=((O=Ve[m])==null?void 0:O.icon)||"⚡",B=document.createElement("div");B.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",B.innerHTML=`
      ...
      <div style="display:flex;gap:12px;width:190px">
        <button id="pvp-gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
        <button id="pvp-gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
      </div>`,document.body.appendChild(B),(Q=B.querySelector("#pvp-gc-back"))==null||Q.addEventListener("click",()=>B.remove()),(K=B.querySelector("#pvp-gc-use"))==null||K.addEventListener("click",()=>{B.remove(),G(f,m)})}function xe(){var b;const f=u[n+"Team"],m=Object.entries(f).flatMap(([v,_])=>(_||[]).filter(T=>!T.used).map(T=>({...T,_line:v})));if(!m.length)return;const x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",x.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
        <div style="flex:1;font-size:14px;font-weight:700;color:#fff">⚡ Choisir un joueur pour +${u.boostValue}</div>
        <button id="bp-close" style="background:none;border:none;color:rgba(255,255,255,0.5);font-size:22px;cursor:pointer">✕</button>
      </div>
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${m.map(v=>{const _={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[v._line]||"#555",T=Me(v,v._line)+(v.boost||0);return`<div class="bp-item" data-cid="${v.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${_};overflow:hidden;cursor:pointer">
      ...
      <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
        <button id="psub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
      </div>`,(Ee=B.querySelector("#psub-close"))==null||Ee.addEventListener("click",()=>B.remove()),(Ce=B.querySelector("#pout-up"))==null||Ce.addEventListener("click",()=>{$>0&&($--,R())}),(je=B.querySelector("#pout-down"))==null||je.addEventListener("click",()=>{$<_.length-1&&($++,R())}),(Re=B.querySelector("#pin-up"))==null||Re.addEventListener("click",()=>{S>0&&(S--,R())}),(Be=B.querySelector("#pin-down"))==null||Be.addEventListener("click",()=>{S<T.length-1&&(S++,R())});const ke=(Fe,Ke,Ne,He)=>{const Ue=B.querySelector("#"+Fe);if(!Ue)return;let Je=0;Ue.addEventListener("touchstart",Ze=>{Je=Ze.touches[0].clientY},{passive:!0}),Ue.addEventListener("touchend",Ze=>{const Qe=Ze.changedTouches[0].clientY-Je;if(Math.abs(Qe)<30)return;const nt=Ke();Qe<0&&nt<He-1?(Ne(nt+1),R()):Qe>0&&nt>0&&(Ne(nt-1),R())},{passive:!0})};ke("pin-panel",()=>S,Fe=>S=Fe,T.length),ke("pout-panel",()=>$,Fe=>$=Fe,_.length),(Oe=B.querySelector("#psub-confirm"))==null||Oe.addEventListener("click",async Fe=>{if(Fe.preventDefault(),Fe.stopPropagation(),q)return;q=!0;const Ke=_[$],Ne=T[S];if(!Ke||!Ne)return;const He=Ke._line,Ue=(x[He]||[]).findIndex(Qe=>Qe.cardId===Ke.cardId);if(Ue===-1){p("Erreur : joueur introuvable","error"),B.remove();return}const Je={...Ne,_line:He,position:Ke.position,used:!1,boost:0};x[He].splice(Ue,1,Je);const Ze=[...b,Ne.cardId];B.remove(),ne(Ke,Ne,async()=>{await de({[n+"Team"]:x,[g+"Team"]:u[g+"Team"],["usedSubIds_"+n]:Ze})})})}document.body.appendChild(B),R()}function ne(f,m,x){const b={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},v=document.createElement("div");v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const _=(A,S,q)=>`<div style="text-align:center">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:6px">Appuyer pour continuer</div>`,document.body.appendChild(v);let T=!1;const $=()=>{T||(T=!0,v.remove(),setTimeout(()=>x(),50))};v.addEventListener("click",$),setTimeout($,2200)}function ge(){var m;const f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:20px;overflow-y:auto",f.innerHTML=`
      <div style="font-size:12px;color:rgba(255,255,255,0.5);letter-spacing:2px;text-transform:uppercase">Équipe adverse</div>
      <div style="font-size:18px;font-weight:900;color:#ff6b6b">${u[g+"Name"]}</div>
      <div style="width:min(90vw,420px)">${ut(u[g+"Team"],u[g+"Formation"],null,[],300,300)}</div>
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(f),(m=f.querySelector("#pvp-opp-close"))==null||m.addEventListener("click",()=>f.remove())}function J(){var b;const f=u.log||[],m=document.createElement("div");m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column";const x=v=>{var T,$,A;if(v.type==="duel"){const S=v.isGoal,q=v.homeScored&&n==="p1"||!v.homeScored&&S&&n==="p2",B=v.homeScored?"#FFD700":S?"#ff6b6b":"rgba(255,255,255,0.3)",R=S?q?"⚽ BUT !":"⚽ BUT adversaire !":(T=v.homePlayers)!=null&&T.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${S?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${B};margin-bottom:4px">
      ...
      </div>`,document.body.appendChild(m),(b=m.querySelector("#pvp-hist-close"))==null||b.addEventListener("click",()=>m.remove())}async function Z(){if(u.phase!==n+"-attack")return;const f=n==="p1"?"p2":"p1",m=(u.round||0)+1,x=[...u.log||[]];x.push({type:"info",text:`⏭️ ${u[n+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const b=De(u),v=$e(f),_=b||!v?"finished":f+"-attack";await de({["selected_"+n]:[],modifiers:{...u.modifiers,[n]:{}},pendingAttack:null,phase:_,attacker:f,round:m,log:x}),_==="finished"&&await C(u)}async function V(){const f=u[n+"Team"],m=!["GK","DEF","MIL","ATT"].some(_=>(u[g+"Team"][_]||[]).some(T=>!T.used)),x=(u["selected_"+n]||[]).map(_=>{const T=(f[_._role]||[]).find(R=>R.cardId===_.cardId)||_,$=m&&["GK","DEF"].includes(_._role),A=f[_._role]||[],S=A.findIndex(R=>R.cardId===_.cardId),q=lt(A.length),B=S>=0?q[S]:T._col??1;return{...T,_line:_._role,_col:B,...$?{note_a:Math.max(1,Number(T.note_a)||0)}:{}}});if(!x.length)return;const b=Nt(x,u.modifiers[n]||{});re(n,x.map(_=>_.cardId)),x.forEach(_=>{const T=(f[_._role]||[]).find($=>$.cardId===_.cardId);T&&(T.used=!0)}),u["selected_"+n]=[],ye();const v=[...u.log||[]];if(m){const _=(u[n+"Score"]||0)+1,T=x.map(B=>({name:B.name,note:Me(B,B._line||"ATT"),portrait:Pe(B),job:B.job}));v.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:T,homeTotal:b.total,aiTotal:0});const $=(u.round||0)+1,A=n==="p1"?"p2":"p1",S={...u,[n+"Team"]:f,[n+"Score"]:_},q=De(S);P.add($),we(T,_,u[g+"Score"]||0,!0,async()=>{await de({[n+"Team"]:f,[n+"Score"]:_,["selected_"+n]:[],modifiers:{...u.modifiers,[n]:{}},pendingAttack:null,phase:q?"finished":A+"-attack",attacker:A,round:$,log:v}),q&&await C({...u,[n+"Score"]:_})});return}v.push({type:"pending",text:`⚔️ ${u[n+"Name"]} attaque (${b.total})`}),await de({[n+"Team"]:f,[g+"Team"]:u[g+"Team"],pendingAttack:{...b,players:x,side:n},["selected_"+n]:[],modifiers:{...u.modifiers,[n]:{}},phase:g+"-defense",log:v})}async function ae(){const f=u[n+"Team"],m=(u["selected_"+n]||[]).map(K=>{const ie=(f[K._role]||[]).find(Ee=>Ee.cardId===K.cardId)||K,se=f[K._role]||[],fe=se.findIndex(Ee=>Ee.cardId===K.cardId),me=lt(se.length),ke=fe>=0?me[fe]:ie._col??1;return{...ie,_line:K._role,_col:ke}}),x=Rt(m,u.modifiers[n]||{});re(n,m.map(K=>K.cardId)),m.forEach(K=>{const ie=(f[K._role]||[]).find(se=>se.cardId===K.cardId);ie&&(ie.used=!0)}),u["selected_"+n]=[],ye();const b=Ot(u.pendingAttack.total,x.total,u.modifiers[n]||{}),v=u.pendingAttack.side,_=b==="attack"||(b==null?void 0:b.goal),T=v==="p1"?"p2":"p1",$=(u.round||0)+1,A=(u.pendingAttack.players||[]).map(K=>({name:K.name,note:Me(K,K._line||"ATT"),portrait:Pe(K),job:K.job})),S=[...u.log||[]];S.push({type:"duel",isGoal:_,homeScored:_&&v===n,text:_?`⚽ BUT de ${u[v+"Name"]} ! (${u.pendingAttack.total} vs ${x.total})`:`✋ Attaque stoppée (${u.pendingAttack.total} vs ${x.total})`,homePlayers:A,aiPlayers:m.map(K=>({name:K.name,note:Me(K,K._line||"DEF"),portrait:Pe(K),job:K.job})),homeTotal:u.pendingAttack.total,aiTotal:x.total});const q=_?(u[v+"Score"]||0)+1:u[v+"Score"]||0,B={...u,[n+"Team"]:f,[v+"Score"]:q},R=De(B),O=R?"finished":T+"-attack",Q=async()=>{await de({[n+"Team"]:f,[g+"Team"]:u[g+"Team"],[v+"Score"]:q,["selected_"+n]:[],modifiers:{...u.modifiers,[n]:{}},pendingAttack:null,phase:O,attacker:T,round:$,log:S}),(O==="finished"||R)&&await C({...u,[v+"Score"]:q})};if(_){const K=v===n,ie=K?q:u[n+"Score"]||0,se=K?u[g+"Score"]||0:q;P.add($),we(A,ie,se,K,Q)}else await Q()}function he(f){return["MIL","ATT"].some(m=>(f[m]||[]).some(x=>!x.used))}function Le(f){return["GK","DEF","MIL","ATT"].some(m=>(f[m]||[]).some(x=>!x.used))}function be(f){return Le(f)&&!he(f)}function $e(f){const m=u[f+"Team"],x=u[(f==="p1"?"p2":"p1")+"Team"];return!!(he(m)||!Le(x)&&be(m))}function De(f){const m=["MIL","ATT"].some(S=>(f.p1Team[S]||[]).some(q=>!q.used)),x=["MIL","ATT"].some(S=>(f.p2Team[S]||[]).some(q=>!q.used)),b=Le(f.p1Team),v=Le(f.p2Team);return!m&&!(!v&&b)&&(!x&&!(!b&&v))}function I(f){const m=f.p1Score||0,x=f.p2Score||0;return m===x?null:m>x?z.home_id:z.away_id}async function C(f){try{const m=I(f),x=m?z.home_id===m?z.away_id:z.home_id:null,b=f.p1Score||0,v=f.p2Score||0;await E.from("matches").update({status:"finished",winner_id:m,home_score:b,away_score:v}).eq("id",i);const{data:_}=await E.from("mini_league_matches").select("id, league_id, matchday").eq("match_id",i).single();if(_){await E.from("mini_league_matches").update({home_score:b,away_score:v,status:"finished"}).eq("id",_.id);const{data:T}=await E.from("mini_league_matches").select("id, status").eq("league_id",_.league_id).eq("matchday",_.matchday);if((T||[]).every(A=>A.status==="finished"||A.status==="bye")){const{data:A}=await E.from("mini_leagues").select("current_day,total_days").eq("id",_.league_id).single();if(A){const S=(A.current_day||0)+1,q=S>(A.total_days||0);await E.from("mini_leagues").update({current_day:q?A.total_days:S,status:q?"finished":"active"}).eq("id",_.league_id)}}}m&&x&&ki(m,x).catch(()=>{})}catch(m){console.error("[ML] finishMatch:",m)}}function w(){var v;if(M&&document.getElementById("pvp-end-overlay"))return;M=!0;const f=u[n+"Score"],m=u[g+"Score"],x=f>m,b=f===m;et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
  ...
  `}async function vr(e,{state:t,navigate:i,toast:s}){const c=t.profile;if(!c)return;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:d}=await E.from("matches").select(`id,home_id,away_id,home_score,away_score,status,mode,winner_id,created_at,played_at,
      ...
      away:users!away_id(pseudo,club_name)`).or(`home_id.eq.${c.id},away_id.eq.${c.id}`).order("created_at",{ascending:!1}).limit(50),a={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},l=(d||[]).filter(n=>n.status==="finished"),p=(d||[]).filter(n=>n.status==="in_progress");function o(n){const g=n.home_id===c.id;g?n.home_score:n.away_score,g?n.away_score:n.home_score;const h=n.winner_id===c.id,y=n.home_score===n.away_score&&n.status==="finished",z=n.status!=="finished"?"…":y?"N":h?"V":"D",r=n.status!=="finished"||y?"#888":h?"#1A6B3C":"#c0392b";let u=a[n.mode]||n.mode;n.away_id===null&&!u.startsWith("IA")&&(u="IA");const L=n.home_id===c.id?n.away:n.home;let D;n.away_id===null?D=u:L?D=`${L.club_name||L.pseudo} (${L.pseudo})`:D="Adversaire";let W="";n.status==="in_progress"&&Date.now()-new Date(n.created_at).getTime()>3600*1e3&&(W=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const F=new Date(n.created_at),P=F.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+F.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),j=n.status==="finished"?`${n.home_score} - ${n.away_score}`:`${n.home_score||0} - ${n.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
