import{s as k,l as re,F as Ge,b as ge,c as st,d as Mt}from"./formation-links-CDBKB_z4.js";function Ye(e,{navigate:t,toast:i}){e.innerHTML=`
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
  ...
  </div>`}async function Dt(e,t){const{state:i,navigate:a,toast:c,openModal:o,closeModal:n}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:d}=await k.from("cards").select(`id, card_type, current_note, gc_type, formation, is_for_sale, sale_price,
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),l=(d||[]).filter(A=>A.card_type==="player"&&A.player),s=(d||[]).filter(A=>A.card_type==="game_changer"),p=(d||[]).filter(A=>A.card_type==="formation"),{data:f}=await k.from("gc_definitions").select("name,gc_type,color,effect,image_url"),u={};(f||[]).forEach(A=>{u[A.name]=A});const x=Object.keys(Ge),y=Object.keys(ae),h={};l.forEach(A=>{const E=A.player.id;h[E]=(h[E]||0)+1}),new Set(Object.keys(h).map(A=>String(A)));const _=new Set(p.map(A=>A.formation)),b=new Set(s.map(A=>A.gc_type));let g="player",w="Tous",T="",m=!1;function v(){return[...l].sort((A,E)=>{const j=he.indexOf(A.player.job),C=he.indexOf(E.player.job);return j!==C?j-C:(A.player.surname_encoded||"").localeCompare(E.player.surname_encoded||"")})}function $(){return[...r||[]].sort((A,E)=>{const j=he.indexOf(A.job),C=he.indexOf(E.job);return j!==C?j-C:(A.surname_encoded||"").localeCompare(E.surname_encoded||"")})}function I(){return v().filter(A=>{const E=A.player,j=w==="Tous"||E.job===w,C=!T||`${E.firstname} ${E.surname_encoded}`.toLowerCase().includes(T);return j&&C})}function L(){return $().filter(A=>{const E=w==="Tous"||A.job===w,j=!T||`${A.firstname} ${A.surname_encoded}`.toLowerCase().includes(T);return E&&j})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const ce=1e3;function Rt(e,t,i,a){var y,h,_;const{state:c,toast:o,closeModal:n,navigate:d,refreshProfile:r}=i,l=e.formation,s={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function p(){const b=ge[l]||{},g=Ge[l]||[],w=290,T=360,m=20;function v(I){const L=b[I];return L?{x:L.x*w,y:L.y*T}:null}let $=`<svg width="${w}" height="${T}" viewBox="0 0 ${w} ${T}" xmlns="http://www.w3.org/2000/svg">`;for(const[I,L]of g){const B=v(I),S=v(L);!B||!S||($+=`<line x1="${B.x}" y1="${B.y}" x2="${S.x}" y2="${S.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const I of Object.keys(b)){const L=v(I);if(!L)continue;const B=I.replace(/\d+/,""),S=s[B]||"#555";$+=`<circle cx="${L.x}" cy="${L.y}" r="${m}" fill="${S}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,$+=`<text x="${L.x}" y="${L.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${B}</text>`}return $+="</svg>",$}const f=t.filter(b=>b.formation===l),u=f.length,x=!e.is_for_sale;a(`Formation ${l}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(y=document.getElementById("direct-sell-form-btn"))==null||y.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${l} pour ${ce.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const b=f.find(w=>!w.is_for_sale)||f[0];if(!b){o("Aucune carte à vendre","error");return}const{error:g}=await k.from("cards").delete().eq("id",b.id);if(g){o(g.message,"error");return}await k.from("users").update({credits:(c.profile.credits||0)+ce}).eq("id",c.profile.id),await r(),o(`+${ce.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),n(),d("collection")}),(h=document.getElementById("market-sell-form-btn"))==null||h.addEventListener("click",async()=>{const b=parseInt(document.getElementById("sell-price-form").value);if(!b||b<1){o("Prix invalide","error");return}await k.from("cards").update({is_for_sale:!0,sale_price:b}).eq("id",e.id),await k.from("market_listings").insert({seller_id:c.profile.id,card_id:e.id,price:b}),o("Carte mise en vente sur le marché !","success"),n(),d("collection")}),(_=document.getElementById("cancel-sell-form-btn"))==null||_.addEventListener("click",async()=>{await k.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await k.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),o("Annonce retirée","success"),n(),d("collection")})}function Qe(e,t,i,a){var m,v,$,I,L,B;const{state:c,toast:o,openModal:n,closeModal:d,navigate:r,refreshProfile:l}=a,s=e.player,p=t.filter(S=>S.player.id===s.id),f=p.length,u=Ft[s.rarity]||1e3,x=s.rarity!=="legende",y=gt(s),h=fe(s,s.job),_=s.job2?fe(s,s.job2):null,b=ke[s.job]||"#1A6B3C",g=s.job2?ke[s.job2]:null,w=ut[s.rarity]||"#ccc",T=Re[s.country_code]||s.country_code||"";n(`${s.firstname} ${s.surname_encoded}`,`<div style="display:flex;gap:16px;flex-wrap:wrap;justify-content:center">
      ...
      <div style="width:140px;border-radius:12px;padding:6px;background:${w};flex-shrink:0">
        <div style="background:#f2e8d2;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
          ...
          <div style="background:#f2e8d2;padding:3px 6px;display:flex;align-items:center;justify-content:space-between;min-height:24px">
            ...
            ${(m=s.clubs)!=null&&m.logo_url?`<img src="${s.clubs.logo_url}" style="width:22px;height:16px;object-fit:contain">`:`<div style="background:#1a3a7a;color:#fff;border-radius:2px;padding:1px 3px;font-size:6px;font-weight:800">${(((v=s.clubs)==null?void 0:v.encoded_name)||"").slice(0,6)}</div>`}
          </div>
        </div>
      </div>
    ...
    </div>`:""}`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),($=document.getElementById("close-detail"))==null||$.addEventListener("click",d),(I=document.getElementById("direct-sell-btn"))==null||I.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte ${s.surname_encoded} pour ${u.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const S=p.find(F=>!F.is_for_sale)||p[0];if(!S){o("Aucune carte à vendre","error");return}const{error:R}=await k.from("cards").delete().eq("id",S.id);if(R){o(R.message,"error");return}await k.from("users").update({credits:(c.profile.credits||0)+u}).eq("id",c.profile.id),await l(),o(`+${u.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),d(),r("collection")}),(L=document.getElementById("market-sell-btn"))==null||L.addEventListener("click",async()=>{const S=parseInt(document.getElementById("sell-price").value);if(!S||S<1){o("Prix invalide","error");return}await k.from("cards").update({is_for_sale:!0,sale_price:S}).eq("id",e.id),await k.from("market_listings").insert({seller_id:c.profile.id,card_id:e.id,price:S}),o("Carte mise en vente sur le marché !","success"),d(),r("collection")}),(B=document.getElementById("cancel-sell-btn"))==null||B.addEventListener("click",async()=>{await k.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await k.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),o("Annonce retirée","success"),d(),r("collection")})}const Ee={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},ee={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function Ie(e){const t=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function xt(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function yt(e){var i;const t="https://fcnwclxlkytdfjotqkta.supabase.co";return(i=e==null?void 0:e.clubs)!=null&&i.logo_url?e.clubs.logo_url.startsWith("http")?e.clubs.logo_url:`${t}/storage/v1/object/public/assets/clubs/${e.clubs.logo_url}`:null}function qt(e,t=44,i=58){var u;const a=e?Ie(e):null,c=e?yt(e):null,o=xt(e==null?void 0:e.country_code),n=(e==null?void 0:e.job)||"MIL",d=ee[n]||"#555",r={normal:"#aaa",pépite:"#D4A017",papyte:"#222",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",l=Number(n==="GK"?e==null?void 0:e.note_g:n==="DEF"?e==null?void 0:e.note_d:n==="MIL"?e==null?void 0:e.note_m:e==null?void 0:e.note_a)||0,s=Math.round(i*.19),p=Math.round(i*.25),f=i-s-p;return e?`<div style="width:${t}px;height:${i}px;border-radius:5px;border:2px solid ${r};background:${d};position:relative;overflow:hidden;flex-shrink:0">
  ...
  <div class="page">
  ...
  </div>`,Nt(e,t,c,i),document.getElementById("builder-back").addEventListener("click",()=>a("decks")),document.getElementById("formation-select").addEventListener("change",l=>{t.formation=l.target.value;const s=et(t.formation),p={};s.forEach(f=>{t.slots[f]&&(p[f]=t.slots[f])}),t.slots=p,se(e,t,i)}),document.getElementById("save-deck").addEventListener("click",()=>Ht(t,i)),e.querySelectorAll("[data-remove-sub]").forEach(l=>{l.addEventListener("click",()=>{t.subs=t.subs.filter(s=>s!==l.dataset.removeSub),se(e,t,i)})}),(r=document.getElementById("add-sub-btn"))==null||r.addEventListener("click",()=>{Pt(t,e,i)})}function Nt(e,t,i,a){const c=e.querySelector("#deck-field");if(!c)return;const o=ge[t.formation]||{},n=st(t.formation)||[],d={};for(const _ of i){const b=t.slots[_],g=b?t.playerCards.find(w=>w.id===b):null;d[_]=g?g.player:null}const r=300,l=300,s=48,p=64,f=13,u=16,x=38;function y(_){const b=o[_];return b?{x:b.x*r,y:b.y*l}:null}let h="";for(const[_,b]of n){const g=y(_),w=y(b);if(!g||!w)continue;const T=d[_]?{...d[_],club_id:d[_].club_id,country_code:d[_].country_code,rarity:d[_].rarity}:null,m=d[b]?{...d[b],club_id:d[b].club_id,country_code:d[b].country_code,rarity:d[b].rarity}:null,v=re(T,m);v==="#ff3333"||v==="#cc2222"?h+=`<line x1="${g.x.toFixed(1)}" y1="${g.y.toFixed(1)}" x2="${w.x.toFixed(1)}" y2="${w.y.toFixed(1)}" stroke="${v}" stroke-width="2.5" stroke-linecap="round" opacity="0.4"/>`:(h+=`<line x1="${g.x.toFixed(1)}" y1="${g.y.toFixed(1)}" x2="${w.x.toFixed(1)}" y2="${w.y.toFixed(1)}" stroke="${v}" stroke-width="8" stroke-linecap="round" opacity="0.2"/>`,h+=`<line x1="${g.x.toFixed(1)}" y1="${g.y.toFixed(1)}" x2="${w.x.toFixed(1)}" y2="${w.y.toFixed(1)}" stroke="${v}" stroke-width="2.5" stroke-linecap="round" opacity="0.9"/>`)}for(const _ of i){const b=y(_);if(!b)continue;const g=d[_],w=_.replace(/\d+/,""),T=ee[w]||"#555",m=(b.x-s/2).toFixed(1),v=(b.y-p/2).toFixed(1),$={normal:"#aaa",pépite:"#D4A017",papyte:"#222",légende:"#7a28b8"}[g==null?void 0:g.rarity]||"#aaa";if(g){const I=Ie(g),L=yt(g),B=xt(g.country_code),S=Number(w==="GK"?g.note_g:w==="DEF"?g.note_d:w==="MIL"?g.note_m:g.note_a)||0,R=p-f-u;h+=`<defs><clipPath id="dcp-${_}"><rect x="${m}" y="${(b.y-p/2+f).toFixed(1)}" width="${s}" height="${R}"/></clipPath></defs>`,h+=`<rect x="${m}" y="${v}" width="${s}" height="${p}" rx="5" fill="${T}"/>`,I&&(h+=`<image href="${I}" x="${m}" y="${(b.y-p/2+f).toFixed(1)}" width="${s}" height="${R}" clip-path="url(#dcp-${_})" preserveAspectRatio="xMidYMin slice"/>`),h+=`<rect x="${m}" y="${v}" width="${s}" height="${f}" fill="rgba(255,255,255,0.93)"/>`,h+=`<text x="${b.x.toFixed(1)}" y="${(b.y-p/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(g.surname_encoded||"").slice(0,9)}</text>`;const F=(b.y+p/2-u).toFixed(1);h+=`<rect x="${m}" y="${F}" width="${s}" height="${u}" fill="rgba(255,255,255,0.93)"/>`,B&&(h+=`<image href="${B}" x="${(b.x-21).toFixed(1)}" y="${(b.y+p/2-u+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`),h+=`<text x="${b.x.toFixed(1)}" y="${(b.y+p/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${S}</text>`,L&&(h+=`<image href="${L}" x="${(b.x+s/2-14).toFixed(1)}" y="${(b.y+p/2-u+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`),h+=`<rect x="${m}" y="${v}" width="${s}" height="${p}" rx="5" fill="none" stroke="${$}" stroke-width="2"/>`}else h+=`<rect x="${m}" y="${v}" width="${s}" height="${p}" rx="5" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="5,3"/>`,h+=`<text x="${b.x.toFixed(1)}" y="${b.y.toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="22" fill="rgba(255,255,255,0.35)">+</text>`,h+=`<text x="${b.x.toFixed(1)}" y="${(b.y+16).toFixed(1)}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.3)">${w}</text>`;h+=`<rect x="${m}" y="${v}" width="${s}" height="${p}" rx="5" fill="rgba(0,0,0,0.01)" class="deck-slot-hit" data-pos="${_}" style="cursor:pointer"/>`}c.innerHTML=`<svg viewBox="${-x} ${-x} ${r+x*2} ${l+x*2}" width="100%" style="display:block;max-width:440px;margin:0 auto">${h}</svg>`,c.querySelectorAll(".deck-slot-hit").forEach(_=>{_.addEventListener("click",()=>Ot(_.dataset.pos,t,e,a))})}function Ot(e,t,i,a){var f,u,x;const{openModal:c,closeModal:o}=a,n=e.replace(/\d+/,""),d=t.slots[e],r=d?t.playerCards.find(y=>y.id===d):null;(f=r==null?void 0:r.player)==null||f.id;const l=new Set;Object.entries(t.slots).forEach(([y,h])=>{var b;if(y===e||!h)return;const _=t.playerCards.find(g=>g.id===h);(b=_==null?void 0:_.player)!=null&&b.id&&l.add(_.player.id)}),t.subs.forEach(y=>{var _;const h=t.playerCards.find(b=>b.id===y);(_=h==null?void 0:h.player)!=null&&_.id&&l.add(h.player.id)});const s=new Set,p=t.playerCards.filter(y=>{const h=y.player;return!(h.job===n||h.job2===n)||l.has(h.id)||s.has(h.id)?!1:(s.add(h.id),!0)});p.sort((y,h)=>{const _=n==="GK"?y.player.note_g:n==="DEF"?y.player.note_d:n==="MIL"?y.player.note_m:y.player.note_a;return(n==="GK"?h.player.note_g:n==="DEF"?h.player.note_d:n==="MIL"?h.player.note_m:h.player.note_a)-_}),c(`Choisir ${n} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${p.length>0?p.map(y=>{var w,T;const h=y.player,_=n==="GK"?h.note_g:n==="DEF"?h.note_d:n==="MIL"?h.note_m:h.note_a,b=Ie(h),g={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[h.rarity];return`<div class="player-option" data-card-id="${y.id}"
          ...
          <div style="width:44px;height:44px;border-radius:8px;overflow:hidden;flex-shrink:0;background:#dde;border:2px solid ${ee[n]}">
            ${b?`<img src="${b}" style="width:100%;height:100%;object-fit:cover">`:`<div style="width:100%;height:100%;background:${ee[n]};display:flex;align-items:center;justify-content:center;color:#fff;font-size:10px;font-weight:700">${n}</div>`}
          </div>
          <!-- Infos -->
    ...
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(u=document.getElementById("close-selector"))==null||u.addEventListener("click",o),(x=document.getElementById("remove-player"))==null||x.addEventListener("click",()=>{delete t.slots[e],o(),se(i,t,a)}),document.querySelectorAll(".player-option").forEach(y=>{y.addEventListener("click",()=>{t.slots[e]=y.dataset.cardId,o(),se(i,t,a)})})}function Pt(e,t,i){var r;const{openModal:a,closeModal:c}=i,o=new Set;Object.values(e.slots).filter(Boolean).forEach(l=>{var p;const s=e.playerCards.find(f=>f.id===l);(p=s==null?void 0:s.player)!=null&&p.id&&o.add(s.player.id)}),e.subs.forEach(l=>{var p;const s=e.playerCards.find(f=>f.id===l);(p=s==null?void 0:s.player)!=null&&p.id&&o.add(s.player.id)});const n=new Set,d=e.playerCards.filter(l=>{var s,p,f;return o.has((s=l.player)==null?void 0:s.id)||n.has((p=l.player)==null?void 0:p.id)?!1:(n.add((f=l.player)==null?void 0:f.id),!0)});a("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${d.length>0?d.map(l=>{var u;const s=l.player,p=Ie(s),f=s.job==="GK"?s.note_g:s.job==="DEF"?s.note_d:s.job==="MIL"?s.note_m:s.note_a;return`<div class="player-option" data-card-id="${l.id}"
          style="display:flex;align-items:center;gap:10px;padding:8px;border:1.5px solid var(--gray-200);border-radius:10px;cursor:pointer">
  ...
  `,document.body.appendChild(a);let c=!1;document.getElementById("pack-visual").addEventListener("click",()=>{if(c)return;c=!0;const u=document.getElementById("pack-visual");u.classList.add("shaking"),setTimeout(()=>{u.classList.add("pack-open"),setTimeout(()=>{document.getElementById("pack-phase").style.display="none",n(0)},600)},500)});let o=0;function n(u){o=u;const x=document.getElementById("reveal-phase");x.style.display="flex",d(u)}function d(u){var m;const x=e[u],y=document.getElementById("card-counter"),h=document.getElementById("single-card-slot"),_=document.getElementById("card-tap-hint");y&&(y.textContent=`Carte ${u+1} / ${e.length}`),_&&(_.textContent=u<e.length-1?"Appuie pour continuer →":"Appuie pour voir toutes tes cartes");const b=it(x),g=x.card_type==="player"&&((m=x.player)==null?void 0:m.rarity)==="legende";h.innerHTML=`
      ...
      </div>`,g&&s();const w=document.getElementById("current-card-wrap");let T=!1;w.addEventListener("click",()=>{if(T)return;T=!0;const v=u+1;v<e.length?(w.style.transition="all 0.25s ease",w.style.transform="translateX(-120%) rotate(-15deg)",w.style.opacity="0",setTimeout(()=>d(v),250)):r()})}function r(){p(),document.getElementById("reveal-phase").style.display="none";const u=document.getElementById("recap-phase");u.style.display="flex";const x=document.getElementById("recap-grid");x.innerHTML=e.map((y,h)=>`
    ...
    </div>`}if(e.card_type==="game_changer"){const o=e._gcDef,n=(o==null?void 0:o.gc_type)==="ultra_game_changer",d={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},r={purple:"#b06ce0",light_blue:"#00d4ef"},l=d[o==null?void 0:o.color]||d.purple,s=r[o==null?void 0:o.color]||r.purple,p=(o==null?void 0:o.name)||e.gc_type||"Game Changer",f=(o==null?void 0:o.effect)||((a=Ce[e.gc_type])==null?void 0:a.desc)||"",u=o!=null&&o.image_url?`/manager-wars/icons/${o.image_url}`:null,x=((c=Ce[e.gc_type])==null?void 0:c.icon)||"⚡";return`<div style="width:170px;height:240px;background:${l};border-radius:14px;border:3px solid ${s};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${s}88;flex-shrink:0">
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}const fi="simulation",vt="VOTRE_ZONE_ID";function ui(){switch(fi){case"propellerads":return gi();case"adsense":return xi();default:return qe()}}function qe(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:#111;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:14px";let i=5;t.innerHTML=`
      ...
      <button id="ad-skip" disabled style="margin-top:8px;padding:10px 24px;border-radius:20px;border:none;background:rgba(255,255,255,0.15);color:rgba(255,255,255,0.4);font-size:13px;cursor:default">
        Passer (5)
      </button>`,document.body.appendChild(t);const a=setInterval(()=>{i--;const c=document.getElementById("ad-cd"),o=document.getElementById("ad-skip");c&&(c.textContent=i),o&&(o.textContent=i>0?`Passer (${i})`:"✓ Continuer"),i<=0&&(clearInterval(a),o&&(o.disabled=!1,o.style.cssText=o.style.cssText.replace("rgba(255,255,255,0.15)","#1A6B3C").replace("rgba(255,255,255,0.4)","#fff").replace("default","pointer")),o==null||o.addEventListener("click",()=>{t.remove(),e(!0)},{once:!0}))},1e3)})}function gi(){return new Promise((e,t)=>{if(typeof window.propeller>"u")return console.warn("[Ad] PropellerAds non chargé → simulation"),e(qe());window.propeller.push({zone_id:vt,onComplete:()=>e(!0),onSkip:()=>t(new Error("Publicité ignorée")),onError:()=>t(new Error("Erreur publicité"))})})}function xi(){return new Promise((e,t)=>{if(typeof window.adsbygoogle>"u")return console.warn("[Ad] AdSense non chargé → simulation"),e(qe());try{(window.adsbygoogle=window.adsbygoogle||[]).push({google_ad_client:"ca-pub-VOTRE_PUBLISHER_ID",google_ad_slot:vt,google_ad_format:"rewarded",on_reward:()=>e(!0),on_dismiss:()=>t(new Error("Publicité fermée"))})}catch(i){t(i)}})}const de={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},le={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function bt(e,t,i,a,c){var o;e.innerHTML=`<div class="match-screen" style="display:flex;align-items:center;justify-content:center;min-height:100vh">
    <div style="text-align:center;padding:40px;color:#fff">
      <div style="font-size:48px;margin-bottom:16px">${t}</div>
      <p style="margin-bottom:16px">${i}</p>
      <button class="btn btn-primary" id="msg-btn">${a}</button>
    </div>
  </div>`,(o=document.getElementById("msg-btn"))==null||o.addEventListener("click",c)}function Y(e){const t=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function je(e){var i,a;const t=e.player;return{cardId:e.id,id:t.id,firstname:t.firstname,name:t.surname_encoded,country_code:t.country_code,club_id:t.club_id,job:t.job,job2:t.job2,note_g:Number(t.note_g)||0,note_d:Number(t.note_d)||0,note_m:Number(t.note_m)||0,note_a:Number(t.note_a)||0,rarity:t.rarity,skin:t.skin,hair:t.hair,hair_length:t.hair_length,clubName:((i=t.clubs)==null?void 0:i.encoded_name)||null,clubLogo:((a=t.clubs)==null?void 0:a.logo_url)||null,boost:0,used:!1,_line:null,_col:null}}function Ne(e){return e===1?[1]:e===2?[0,2]:e===3?[0,1,2]:e===4?[0,1,1,2]:e===5?[0,1,1,1,2]:[1]}function yi(){const e=Math.random()*100;return e<.1?4:e<5?3:e<20?2:1}function wt(e,t){const i=de[t]||de["4-4-2"],a={GK:[],DEF:[],MIL:[],ATT:[]},c=[...e];for(const o of["GK","DEF","MIL","ATT"]){const n=[];for(let r=0;r<i[o];r++){let l=c.findIndex(s=>s.job===o);if(l===-1&&(l=c.findIndex(s=>s.job2===o)),l===-1&&(l=0),c[l]){const s={...c[l],_line:o};n.push(s),c.splice(l,1)}}const d=Ne(n.length);n.forEach((r,l)=>{r._col=d[l]}),a[o]=n}return a}async function mi(e,t){var n,d;const{data:i}=await k.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(60);if(!i||i.length<11)return hi(e);const a=de[e]||de["4-4-2"],c={GK:[],DEF:[],MIL:[],ATT:[]},o=[...i];for(const r of["GK","DEF","MIL","ATT"]){const l=o.filter(x=>x.job===r),s=o.filter(x=>x.job!==r),p=[...l,...s],f=[];for(let x=0;x<a[r];x++){const y=p[x]||o[x];y&&f.push({cardId:"ai-"+y.id+"-"+x,id:y.id,firstname:y.firstname,name:y.surname_encoded,country_code:y.country_code,club_id:y.club_id,job:y.job,job2:y.job2,note_g:Number(y.note_g)||0,note_d:Number(y.note_d)||0,note_m:Number(y.note_m)||0,note_a:Number(y.note_a)||0,rarity:y.rarity,skin:y.skin,hair:y.hair,hair_length:y.hair_length,clubName:((n=y.clubs)==null?void 0:n.encoded_name)||null,clubLogo:((d=y.clubs)==null?void 0:d.logo_url)||null,boost:0,used:!1,_line:r})}const u=Ne(f.length);f.forEach((x,y)=>{x._col=u[y]}),c[r]=f}return c}function hi(e){const t=de[e]||de["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},a=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let c=0;for(const o of["GK","DEF","MIL","ATT"]){const n=[];for(let r=0;r<t[o];r++){const l=3+Math.floor(Math.random()*5);n.push({cardId:"fake-"+c,id:"fake-"+c,firstname:"IA",name:a[c%a.length],country_code:"XX",club_id:null,job:o,job2:null,note_g:o==="GK"?l:2,note_d:o==="DEF"?l:2,note_m:o==="MIL"?l:2,note_a:o==="ATT"?l:2,rarity:"normal",boost:0,used:!1,_line:o}),c++}const d=Ne(n.length);n.forEach((r,l)=>{r._col=d[l]}),i[o]=n}return i}async function vi(e,t){var m;const{state:i,navigate:a,toast:c}=t,o=i.params||{};e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const n=o.matchMode||"vs_ai_easy",d=n.replace("vs_ai_",""),r=n;if(!o.deckId)return wi(e,t,n);const l=o.deckId,[{data:s},{data:p}]=await Promise.all([k.from("decks").select("formation,name").eq("id",l).single(),k.from("deck_cards").select(`position, is_starter, slot_order,
        card:cards(id, card_type, formation,
          player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
            note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
            clubs(encoded_name,logo_url)))`).eq("deck_id",l).order("slot_order")]),f=(p||[]).filter(v=>{var $;return v.is_starter&&(($=v.card)==null?void 0:$.player)}).map(v=>je(v.card)),u=(p||[]).filter(v=>{var $;return!v.is_starter&&(($=v.card)==null?void 0:$.player)}).map(v=>je(v.card));if(f.length<11){bt(e,"⚠️",`Deck incomplet (${f.length}/11).`,"Compléter",()=>a("decks"));return}const x=(p||[]).find(v=>{var $;return(($=v.card)==null?void 0:$.card_type)==="formation"}),y=(s==null?void 0:s.formation)||((m=x==null?void 0:x.card)==null?void 0:m.formation)||"4-4-2",{data:h}=await k.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",i.profile.id).eq("card_type","game_changer"),{data:_}=await k.from("gc_definitions").select("*").eq("is_active",!0),b=(h||[]).map(v=>({...v,_gcDef:(_==null?void 0:_.find($=>$.name===v.gc_type||$.id===v.gc_definition_id))||null})),g=wt(f,y),w=await mi(y),T=async v=>{const{data:$}=await k.from("matches").insert({home_id:i.profile.id,away_id:null,mode:r,home_deck_id:l,status:"in_progress"}).select().single(),I={gcDefs:_||[],matchId:$==null?void 0:$.id,mode:r,difficulty:d,formation:y,homeTeam:g,aiTeam:w,homeSubs:u,subsUsed:0,maxSubs:Math.min(u.length,3),homeScore:0,aiScore:0,gcCards:v,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};$i(e,I,t)};if(!b.length){T([]);return}bi(e,b,T)}function bi(e,t,i){const c=new Set,o=t.filter(s=>{const p=s.gc_type||s.id;return c.has(p)?!1:(c.add(p),!0)});let n=[];function d(s,p){const f=s._gcDef,u={purple:"linear-gradient(135deg,#3d0a7a,#7a28b8)",light_blue:"linear-gradient(135deg,#006080,#00bcd4)"},x={purple:"#9b59b6",light_blue:"#00bcd4"},y=u[f==null?void 0:f.color]||u.purple,h=x[f==null?void 0:f.color]||x.purple;return`<div class="gc-select-card" data-id="${s.id}"
    ...
    </div>`,e.querySelectorAll(".gc-select-card").forEach(p=>{p.addEventListener("click",()=>{const f=p.dataset.id,u=o.find(y=>y.id===f);if(!u)return;const x=n.findIndex(y=>y.gc_type===u.gc_type);x>-1?n.splice(x,1):n.length<3&&n.push(u),l(),n.length===3&&r()})}),n.length===3&&r()}l()}async function wi(e,t,i){const{state:a,navigate:c}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await k.from("decks").select("id,name,is_active,formation").eq("owner_id",a.profile.id).order("created_at",{ascending:!1});if(!o||o.length===0){bt(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>c("decks"));return}const n=o.map(s=>s.id),{data:d}=await k.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",n).order("slot_order");let r=0;function l(){var b,g,w,T,m;const s=o[r],p=(d||[]).filter(v=>v.deck_id===s.id),f=p.filter(v=>{var $;return v.is_starter&&(($=v.card)==null?void 0:$.player)}).map(v=>je(v.card)),u=p.find(v=>{var $;return(($=v.card)==null?void 0:$.card_type)==="formation"}),x=s.formation||((b=u==null?void 0:u.card)==null?void 0:b.formation)||"4-4-2",y=f.length>=11?wt(f,x):null,h=f.length>=11;e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:calc(100dvh - 130px);overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div id="deck-swipe-zone" style="flex:1;min-height:0;overflow:hidden;position:relative;touch-action:pan-y;display:flex;align-items:center;justify-content:center">
        ${y?`<div style="width:min(98vw, calc(100dvh - 400px));overflow:hidden;flex-shrink:0">${kt(y,x,null,[],285,285)}</div>`:`<div style="display:flex;align-items:center;justify-content:center;height:100%;opacity:.4;flex-direction:column;gap:8px">
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
  </div>`,setTimeout(()=>_i(e,t,i),5e3)}function _i(e,t,i){const a=t.homeTeam.MIL||[],c=t.aiTeam.MIL||[];function o(p){return p.reduce((f,u)=>f+H(u,"MIL"),0)}function n(p){let f=0;for(let u=0;u<p.length-1;u++){const x=re(p[u],p[u+1]);x==="#00ff88"?f+=2:x==="#FFD700"&&(f+=1)}return f}const d=o(a)+n(a),r=o(c)+n(c),l=d>=r;function s(p,f,u){return`<div style="text-align:center">
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${p.map((x,y)=>{const h=Y(x),_=y<p.length-1?re(x,p[y+1]):null,b=_&&_!=="#ff3333"&&_!=="#cc2222";return`
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div id="midfield-result" style="opacity:0;text-align:center;transition:opacity 0.5s;color:#fff;max-width:320px"></div>
  </div>`,setTimeout(()=>{const p=document.getElementById("score-home"),f=document.getElementById("score-ai"),u=document.getElementById("midfield-result");if(p&&f&&(l?(p.style.fontSize="80px",p.style.color="#FFD700",f.style.opacity="0.25"):(f.style.fontSize="80px",f.style.color="#ff6b6b",p.style.opacity="0.25")),u){const x=yi();u.style.opacity="1",l&&t.clubName,u.innerHTML=`
      ...
      </div>`,(h=document.getElementById("start-match-btn"))==null||h.addEventListener("click",()=>{t.phase=t.attacker==="home"?"attack":"ai-attack",K(e,t,i),t.attacker==="ai"&&setTimeout(()=>Se(e,t,i),800)})},100)},5e3)}function $t(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";return e!=null&&e.clubLogo?e.clubLogo.startsWith("http")?e.clubLogo:`${t}/storage/v1/object/public/assets/clubs/${e.clubLogo}`:null}function oe(e,t=44,i=58){if(!e)return`<div style="width:${t}px;height:${i}px;border:1.5px dashed rgba(255,255,255,0.2);border-radius:5px"></div>`;const a=typeof e.portrait=="string"&&e.portrait.startsWith("http")?e.portrait:Y(e),c=$t(e),o=e._line||e.job||"MIL",n=le[o]||"#555",d={normal:"#aaa",pépite:"#D4A017",papyte:"#222",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",r=e.note!==void 0?Number(e.note)||0:(Number(H(e,o))||0)+(e.boost||0),l=_t(e==null?void 0:e.country_code),s=Math.round(i*.19),p=Math.round(i*.25),f=i-Math.round(i*.19)-Math.round(i*.25),u=e!=null&&e.used?.28:1;return`<div style="width:${t}px;height:${i}px;border-radius:5px;border:2px solid ${d};background:${n};position:relative;overflow:hidden;flex-shrink:0;opacity:${u}">
    <div style="position:absolute;top:0;left:0;right:0;height:${s}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:center;z-index:2">
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;height:${p}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ...
      ${c?`<img src="${c}" style="width:${p-4}px;height:${p-4}px;object-fit:contain">`:e!=null&&e.clubName?`<span style="font-size:${Math.max(4,p-8)}px;font-weight:700;color:#333">${(e.clubName||"").slice(0,3).toUpperCase()}</span>`:""}
        ...
        stroke="${$}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const f=48,u=64,x=13,y=16,h={normal:"#aaaaaa",pépite:"#D4A017",papyte:"#111111",légende:"#7a28b8"};for(const[b,g]of Object.entries(r)){const w=s(b);if(!w||!g)continue;const T=b.replace(/[0-9]/g,""),m=le[T]||"#555",v=i==="attack"&&["MIL","ATT"].includes(T)&&!g.used||i==="defense"&&["GK","DEF","MIL"].includes(T)&&!g.used,$=a.includes(g.cardId);let I;i==="attack"?I=T==="MIL"?Number(g.note_m)||0:Number(g.note_a)||0:i==="defense"?I=T==="GK"?Number(g.note_g)||0:T==="MIL"?Number(g.note_m)||0:Number(g.note_d)||0:I=Number(T==="GK"?g.note_g:T==="DEF"?g.note_d:T==="MIL"?g.note_m:g.note_a)||0,I=I+(g.boost||0);const L=(w.x-f/2).toFixed(1),B=(w.y-u/2).toFixed(1),S=g.used?.25:1,R=h[g==null?void 0:g.rarity]||h.normal,F=$?"#ff3030":R,U=$?3:(g==null?void 0:g.rarity)==="légende"||(g==null?void 0:g.rarity)==="pépite"?2.5:2,W=u-x-y;p+=`<defs><clipPath id="cp-${b}"><rect x="${L}" y="${(w.y-u/2+x).toFixed(1)}" width="${f}" height="${W}"/></clipPath></defs>`,p+=`<rect x="${L}" y="${B}" width="${f}" height="${u}" rx="5" fill="${m}" opacity="${S}"/>`;const J=Y(g);J&&!g.used&&(p+=`<image href="${J}" x="${L}" y="${(w.y-u/2+x).toFixed(1)}" width="${f}" height="${W}" clip-path="url(#cp-${b})" preserveAspectRatio="xMidYMin slice"/>`),p+=`<rect x="${L}" y="${B}" width="${f}" height="${x}" rx="4" fill="rgba(255,255,255,0.92)"/>`,p+=`<text x="${w.x.toFixed(1)}" y="${(w.y-u/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(g.name||"").slice(0,9)}</text>`;const Te=(w.y+u/2-y).toFixed(1);if(p+=`<rect x="${L}" y="${Te}" width="${f}" height="${y}" fill="rgba(255,255,255,0.92)"/>`,g.used)p+=`<text x="${w.x.toFixed(1)}" y="${(w.y+u/2-y/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="13" font-weight="900" fill="rgba(0,0,0,0.4)" font-family="Arial Black">–</text>`;else{const xe=Be(g.country_code);xe?p+=`<image href="${xe}" x="${(w.x-20).toFixed(1)}" y="${(w.y+u/2-y+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:p+=`<text x="${(w.x-13).toFixed(1)}" y="${(w.y+u/2-y/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${_t(g.country_code)}</text>`,p+=`<text x="${w.x.toFixed(1)}" y="${(w.y+u/2-y/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${I}</text>`;const ye=$t(g);ye?p+=`<image href="${ye}" x="${(w.x+f/2-14).toFixed(1)}" y="${(w.y+u/2-y+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:g.clubName&&(p+=`<text x="${(w.x+14).toFixed(1)}" y="${(w.y+u/2-y/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(g.clubName||"").slice(0,3).toUpperCase()}</text>`),g.boost&&(p+=`<rect x="${(w.x+f/2-12).toFixed(1)}" y="${(w.y-u/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,p+=`<text x="${(w.x+f/2-5).toFixed(1)}" y="${(w.y-u/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${g.boost}</text>`)}p+=`<rect x="${L}" y="${B}" width="${f}" height="${u}" rx="5" fill="${$?"rgba(255,255,255,0.12)":"none"}" stroke="${F}" stroke-width="${U}" opacity="${S}"/>`,v&&(p+=`<rect x="${L}" y="${B}" width="${f}" height="${u}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${$?"selected":""}" data-card-id="${g.cardId}" data-role="${T}" style="cursor:pointer"/>`)}const _=38;return`<svg viewBox="${-_} ${-_} ${c+_*2} ${o+_*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${p}
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;height:100%;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:100px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const v=t.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
          ...
          </div>`}const m=t.log[t.log.length-1];return m?'<div style="padding:2px 4px">'+ki(m)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})()}
    </div>
    ...
    ${(()=>{const m=window.innerWidth>=700,v=(F,U)=>{var W,J;return U?`<div id="boost-card" style="width:110px;padding:8px 6px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:2px solid #87CEEB;border-radius:10px;cursor:pointer;display:flex;flex-direction:column;align-items:center;gap:4px;text-align:center">
      ...
      </div>`,R=`<div style="overflow:hidden;min-width:0;flex:0.8;min-height:0;display:flex;align-items:center;justify-content:center" id="match-field">
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(m=>{var v,$,I;if(m.type==="duel"){const L=m.isGoal,B=m.homeScored?"#FFD700":L?"#ff6b6b":"rgba(255,255,255,0.3)",S=m.homeScored?"⚽ BUT !":L?"⚽ BUT IA !":(v=m.homePlayers)!=null&&v.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${L?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${B};margin-bottom:4px">
  ...
  </div>`,function(){const v=e.querySelector(".terrain-wrapper svg");if(!v)return;v.removeAttribute("width"),v.removeAttribute("height"),v.style.width="100%",v.style.height="100%",v.style.maxWidth="none",v.style.margin="0",v.style.display="block",v.getAttribute("viewBox")||v.setAttribute("viewBox","0 0 300 300"),v.setAttribute("preserveAspectRatio","xMidYMid meet");const $=e.querySelector(".match-screen");if($){const I=e.clientHeight;I>50&&($.style.height=I+"px",$.style.overflow="hidden")}}(),t._resizeBound||(t._resizeBound=!0,window.addEventListener("resize",()=>{const m=e.querySelector(".terrain-wrapper svg");m&&(m.style.width="100%",m.style.height="100%",m.style.maxWidth="none")})),t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase==="attack"||t.phase==="defense"){let m=!1,v=30;const $=()=>document.getElementById("match-timer"),I=()=>{const L=$();if(!L)return;const B=String(Math.floor(v/60)).padStart(2,"0"),S=String(v%60).padStart(2,"0");L.textContent=` ${B}:${S}`,L.style.color=m?"#ff2222":"#ff9500",L.style.fontWeight="900"};I(),t._timerInt=setInterval(()=>{if(v--,v<0)if(!m)m=!0,v=15,I();else{clearInterval(t._timerInt),t._timerInt=null,t.homeScore=0,t.aiScore=3;const L=document.createElement("div");L.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;color:#fff;padding:24px;text-align:center",L.innerHTML='<div style="font-size:56px">⏱️</div><div style="font-size:24px;font-weight:900;color:#ff4444">MATCH PERDU PAR FORFAIT</div><div style="font-size:14px;color:rgba(255,255,255,0.6)">Temps écoulé</div>',document.body.appendChild(L),setTimeout(()=>{L.remove(),ne(e,t,i)},2500)}else I()},1e3)}(x=document.getElementById("match-quit"))==null||x.addEventListener("click",()=>{confirm("Abandonner ? Résultat : défaite 3-0")&&(t.homeScore=0,t.aiScore=3,ne(e,t,i))}),(y=document.getElementById("view-ai"))==null||y.addEventListener("click",()=>ji(t,i)),(h=document.getElementById("toggle-history"))==null||h.addEventListener("click",()=>{var m;(m=document.getElementById("match-history-panel"))==null||m.classList.add("open")}),(_=document.getElementById("close-history"))==null||_.addEventListener("click",()=>{var m;(m=document.getElementById("match-history-panel"))==null||m.classList.remove("open")}),(b=document.getElementById("btn-action"))==null||b.addEventListener("click",()=>{t.selected.length!==0&&(r?Ai(e,t,i):l&&Ii(e,t,i))}),(g=document.getElementById("btn-results"))==null||g.addEventListener("click",()=>ne(e,t,i)),e.querySelectorAll(".match-slot-hit").forEach(m=>{m.addEventListener("click",()=>Ei(m,t,e,i))}),e.querySelectorAll(".gc-mini").forEach(m=>{m.addEventListener("click",()=>Mi(m.dataset.gcId,m.dataset.gcType,e,t,i))}),(w=document.getElementById("boost-card"))==null||w.addEventListener("click",()=>Ci(e,t,i)),e.querySelectorAll(".sub-btn-col").forEach(m=>{m.addEventListener("click",()=>ot(e,t,i,m.dataset.subId))}),(T=document.getElementById("sub-btn-main"))==null||T.addEventListener("click",()=>ot(e,t,i))}function Ei(e,t,i,a){const c=e.dataset.cardId,o=e.dataset.role,n=t.selected.findIndex(d=>d.cardId===c);if(n!==-1)t.selected.splice(n,1);else{if(t.selected.length>=3){a.toast("Maximum 3 joueurs","error");return}const d=(t.homeTeam[o]||[]).find(r=>r.cardId===c);d&&t.selected.push({...d,_role:o,_line:o})}K(i,t,a)}function Pe(e,t,i){e.matchId&&k.from("matches").update({last_player_id:i}).eq("id",e.matchId).then(()=>{})}function Ai(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),Pe(t,i,i.state.profile.id);const a=t.selected.map(o=>({...o,_line:o._role})),c=lt(a,t.modifiers.home);t.pendingAttack={...c,players:[...t.selected],side:"home"},t.selected.forEach(o=>{const n=(t.homeTeam[o._role]||[]).find(d=>d.cardId===o.cardId);n&&(n.used=!0)}),t.log.push({text:`⚔️ Vous attaquez : ${c.total} (base ${c.base}${c.links?` +${c.links} liens`:""}) — ${t.selected.map(o=>o.name).join(", ")}`,type:"info"}),t.selected=[],t.modifiers.home={},t.phase="ai-defense",K(e,t,i),setTimeout(()=>Ti(e,t,i),1200)}function Ii(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),Pe(t,i,i.state.profile.id);const a=t.selected.map(d=>({...d,_line:d._role})),c=ct(a,t.modifiers.home);t.selected.forEach(d=>{const r=(t.homeTeam[d._role]||[]).find(l=>l.cardId===d.cardId);r&&(r.used=!0)});const o=pt(t.pendingAttack.total,c.total,t.modifiers.home),n={type:"duel",title:"Défense",aiPlayers:(t.pendingAttack.players||[]).map(d=>({name:d.name,note:d._line==="MIL"?d.note_m:d.note_a,portrait:Y(d),job:d.job,country_code:d.country_code,rarity:d.rarity,clubName:d.clubName,clubLogo:d.clubLogo})),homePlayers:t.selected.map(d=>{const r=(t.homeTeam[d._role]||[]).find(l=>l.cardId===d.cardId)||d;return{name:r.name,note:r._line==="GK"?r.note_g:r._line==="MIL"?r.note_m:r.note_d,portrait:Y(r),job:r.job,country_code:r.country_code,rarity:r.rarity,clubName:r.clubName,clubLogo:r.clubLogo}}),homeTotal:c.total,aiTotal:t.pendingAttack.total,isGoal:!1,homeScored:!1,text:""};if(o.shielded)n.text="🛡️ Bouclier ! But annulé.",t.log.push(n);else if(o.goal){t.aiScore++,n.isGoal=!0,n.homeScored=!1,n.text=`⚽ BUT IA ! (${t.pendingAttack.total} > ${c.total})`,t.log.push(n),t.selected=[],t.modifiers.home={},t.pendingAttack=null,K(e,t,i),He(n.aiPlayers,t.homeScore,t.aiScore,!1,()=>{ue(e,t,i,"home-attack")});return}else n.text=`🧤 Défense réussie ! (${c.total} ≥ ${t.pendingAttack.total})`,t.log.push(n);t.selected=[],t.modifiers.home={},t.pendingAttack=null,ue(e,t,i,"home-attack")}function Se(e,t,i){Pe(t,i,null);const a=[...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]],c=ft(a,"attack",t.difficulty);if(!c.length){At(e,t,i);return}const o=lt(c,t.modifiers.ai);t.pendingAttack={...o,players:c,side:"ai"},c.forEach(l=>{l.used=!0}),t.log.push({text:`🤖 IA attaque : ${o.total} (${c.map(l=>l.name).join(", ")})`,type:"info"}),t.modifiers.ai={};const n=[...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(l=>!l.used),r=(t.homeSubs||[]).filter(l=>!(t.usedSubIds||[]).includes(l.cardId)).length>0&&t.subsUsed<t.maxSubs;if(n.length===0&&!r){t.aiScore++;const l={type:"duel",isGoal:!0,homeScored:!1,aiPlayers:c.map(s=>({name:s.name,note:s._line==="MIL"?s.note_m:s.note_a,portrait:Y(s),job:s.job,country_code:s.country_code,rarity:s.rarity,clubName:s.clubName,clubLogo:s.clubLogo})),aiTotal:o.total,text:"⚽ BUT IA ! (aucun défenseur disponible)"};t.log.push(l),t.pendingAttack=null,K(e,t,i),He(l.aiPlayers,t.homeScore,t.aiScore,!1,()=>{ue(e,t,i,"home-attack")});return}t.phase="defense",K(e,t,i)}function Ti(e,t,i){const a=[...t.aiTeam.GK||[],...t.aiTeam.DEF||[],...t.aiTeam.MIL||[]],c=ft(a,"defense",t.difficulty),o=c.length>0?ct(c,t.modifiers.ai).total:0;c.forEach(r=>{r.used=!0});const n=pt(t.pendingAttack.total,o,t.modifiers.ai),d={type:"duel",title:"Attaque",homePlayers:(t.pendingAttack.players||[]).map(r=>({name:r.name,note:r._line==="MIL"?r.note_m:r.note_a,portrait:Y(r),job:r.job,country_code:r.country_code,rarity:r.rarity,clubName:r.clubName,clubLogo:r.clubLogo})),aiPlayers:c.map(r=>({name:r.name,note:r._line==="GK"?r.note_g:r._line==="MIL"?r.note_m:r.note_d,portrait:Y(r),job:r.job,country_code:r.country_code,rarity:r.rarity,clubName:r.clubName,clubLogo:r.clubLogo})),homeTotal:t.pendingAttack.total,aiTotal:o,isGoal:!1,homeScored:!1,text:""};if(n.shielded)d.text="🛡️ Bouclier IA !",t.log.push(d);else if(n.goal){t.homeScore++,d.isGoal=!0,d.homeScored=!0,d.text=`⚽ BUT ! (${t.pendingAttack.total} > ${o})`,t.log.push(d),t.modifiers.ai={},t.pendingAttack=null,K(e,t,i),He(d.homePlayers,t.homeScore,t.aiScore,!0,()=>{ue(e,t,i,"ai-attack")});return}else d.text=`🧤 IA défend (${o} ≥ ${t.pendingAttack.total})`,t.log.push(d);t.modifiers.ai={},t.pendingAttack=null,ue(e,t,i,"ai-attack")}function ue(e,t,i,a){if(t.round++,Et(t)){ne(e,t,i);return}if(a==="home-attack"){if(![...t.homeTeam.MIL||[],...t.homeTeam.ATT||[]].filter(o=>!o.used).length){if(![...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(n=>!n.used).length){ne(e,t,i);return}t.phase="ai-attack",K(e,t,i),setTimeout(()=>Se(e,t,i),800);return}t.phase="attack",K(e,t,i)}else{if(![...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]].filter(o=>!o.used).length){At(e,t,i);return}t.phase="ai-attack",K(e,t,i),setTimeout(()=>Se(e,t,i),800)}}function Et(e){const t=["MIL","ATT","GK","DEF"].some(a=>(e.homeTeam[a]||[]).some(c=>!c.used)),i=["MIL","ATT","GK","DEF"].some(a=>(e.aiTeam[a]||[]).some(c=>!c.used));return!t&&!i}function At(e,t,i){Et(t)?ne(e,t,i):(t.phase="attack",K(e,t,i))}function Li(e,t,i){const a=document.createElement("div");a.style.cssText=`
  ...
  `;function d(r){const l=document.getElementById("mkt-content"),s=r==="buy"?n:o;if(s.length===0){l.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      ${s.map(p=>{var h,_,b;const f=(h=p.card)==null?void 0:h.player;if(!f)return"";const u=f.job==="GK"?f.note_g:f.job==="DEF"?f.note_d:f.job==="MIL"?f.note_m:f.note_a,x=Bi[f.rarity],y=(i.profile.credits||0)>=p.price;return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px">
          <div style="width:44px;height:44px;border-radius:8px;background:${Gi(f.job)};color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:18px;border:2px solid ${x};flex-shrink:0">${u}</div>
          <div style="flex:1;min-width:0">
  ...
  </div>`}const G={user:null,profile:null,page:"home",params:{}};function we(e,t="info",i=3e3){const a=document.getElementById("toast");a&&(a.textContent=e,a.className=`show ${t}`,clearTimeout(a._t),a._t=setTimeout(()=>{a.className=""},i))}function Ni(e,t,i=""){document.getElementById("modal-title").textContent=e,document.getElementById("modal-body").innerHTML=t,document.getElementById("modal-footer").innerHTML=i,document.getElementById("modal-overlay").classList.remove("hidden")}function Fe(){document.getElementById("modal-overlay").classList.add("hidden")}async function $e(){if(!G.user)return;const{data:e}=await k.from("users").select("*").eq("id",G.user.id).single();e&&(G.profile=e)}const Ue="mw_theme";function Ae(){return localStorage.getItem(Ue)||"light"}function Oi(e){var t;localStorage.setItem(Ue,e),De(e),(t=G.profile)!=null&&t.id&&k.from("users").update({theme:e}).eq("id",G.profile.id).then(()=>{})}function De(e){document.documentElement.setAttribute("data-theme",e)}function _e(e,t={}){G.page=e,G.params=t,Tt()}async function Tt(){const e=document.getElementById("page-content");if(!e)return;document.querySelectorAll(".bottom-nav a").forEach(a=>{a.classList.toggle("active",a.dataset.page===G.page)});const t=document.getElementById("nav-credits");t&&G.profile&&(t.textContent=`💰 ${(G.profile.credits||0).toLocaleString("fr")}`);const i={state:G,navigate:_e,toast:we,openModal:Ni,closeModal:Fe,refreshProfile:$e};switch(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽</div>',G.page){case"home":await Xe(e,i);break;case"collection":await Dt(e,i);break;case"decks":await ze(e,i);break;case"boosters":await ti(e,i);break;case"match":await vi(e,i);break;case"market":await Si(e,i);break;case"rankings":await Ri(e,i);break;case"matches":await qi(e,i);break;default:await Xe(e,i)}}function Pi(){const e=document.getElementById("app"),t=G.profile;if(!t)return;const i="/manager-wars/icons/";e.innerHTML=`
    <nav class="top-nav">
  ...
  `,document.querySelectorAll(".bottom-nav a").forEach(a=>{a.addEventListener("click",c=>{c.preventDefault(),_e(a.dataset.page)})}),document.getElementById("nav-logo").addEventListener("click",()=>_e("home")),document.getElementById("nav-credits").addEventListener("click",()=>_e("boosters")),document.getElementById("theme-toggle").addEventListener("click",()=>{const c=Ae()==="dark"?"light":"dark";Oi(c),nt(c)}),nt(Ae())}function nt(e){const t=document.getElementById("theme-icon");t&&(t.textContent=e==="dark"?"☀️":"🌙")}async function Hi(){De(Ae()),document.getElementById("modal-overlay").addEventListener("click",t=>{t.target===t.currentTarget&&Fe()}),document.getElementById("modal-close").addEventListener("click",Fe);const{data:{session:e}}=await k.auth.getSession();if(!e){at(),Ye(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:we});return}G.user=e.user,await $e(),at();try{const{data:t}=await k.from("formation_links_overrides").select("formation, links"),i={};(t||[]).forEach(a=>{i[a.formation]=a.links}),Mt(i)}catch(t){console.warn("Impossible de charger les overrides de formation:",t)}if(!G.profile){zt(document.getElementById("app"),{state:G,navigate:async()=>{await $e(),rt()},toast:we,refreshProfile:$e});return}G.profile.theme&&G.profile.theme!==Ae()&&(localStorage.setItem(Ue,G.profile.theme),De(G.profile.theme)),rt(),k.auth.onAuthStateChange(async(t,i)=>{t==="SIGNED_OUT"&&(G.user=null,G.profile=null,document.getElementById("app").innerHTML="",Ye(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:we}))})}function rt(){const e=document.getElementById("app");e.style.display="flex",e.style.flexDirection="column",Pi(),Tt()}function at(){const e=document.getElementById("app-loader"),t=document.getElementById("app");t&&(t.style.display=""),e&&(e.classList.add("zoom-out"),setTimeout(()=>e.style.display="none",500))}Hi();
