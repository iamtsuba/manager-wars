...
import{s as v,j as wi,q as So,t as Ut,u as Hi,F as Ui,o as tt,n as Rt,g as Mt,r as Se,m as wt,v as fi,w as fn,T as Ii,p as mn,x as gn,d as xn}from"./special-cards-lEGt-tGs.js";const bn="/";function po(t,{navigate:e,toast:i}){let o="login";const a=()=>{var r,s,d,c,l,f,b;const n=o==="login";if(t.innerHTML=`
    <div style="
  ...
  `;function s(){var g;const c=document.getElementById("logo-preview"),l=document.getElementById("logo-initials"),f=((g=document.getElementById("setup-club"))==null?void 0:g.value)||"MW",b=f.trim().split(" ").filter(Boolean),x=b.length>=2?(b[0][0]+b[1][0]).toUpperCase():f.slice(0,2).toUpperCase();c&&(c.style.background=r,c.style.borderColor=n),l&&(l.textContent=x,l.style.color=n)}document.getElementById("color1").addEventListener("input",c=>{n=c.target.value,document.getElementById("swatch1").style.background=n,s()}),document.getElementById("color2").addEventListener("input",c=>{r=c.target.value,document.getElementById("swatch2").style.background=r,s()});function d(c){document.querySelectorAll(".setup-step").forEach(l=>l.classList.remove("active")),document.getElementById(`step-${c}`).classList.add("active"),document.getElementById("step-num").textContent=c,document.getElementById("progress-fill").style.width=`${Math.round(c/3*100)}%`,c===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("step1-error");if(l.textContent="",c.length<3){l.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:f}=await v.from("users").select("id").eq("pseudo",c).maybeSingle();if(f){l.textContent="Ce pseudo est déjà pris.";return}d(2)}),document.getElementById("step2-back").addEventListener("click",()=>d(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const c=document.getElementById("setup-club").value.trim(),l=document.getElementById("step2-error");if(l.textContent="",c.length<2){l.textContent="Nom trop court (min. 2 caractères).";return}const{data:f}=await v.from("users").select("id").eq("club_name",c).maybeSingle();if(f){l.textContent="Ce nom de club est déjà pris.";return}d(3)}),document.getElementById("step3-back").addEventListener("click",()=>d(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("setup-club").value.trim(),f=document.getElementById("step3-error"),b=document.getElementById("step3-finish");f.textContent="",b.disabled=!0,b.textContent="Création en cours…";try{const{error:x}=await v.from("users").insert({id:e.user.id,pseudo:c,club_name:l,club_color1:n,club_color2:r,credits:1e4});if(x)throw x;await hn(e.user.id),await a(),o(`Bienvenue ${c} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(x){f.textContent=x.message,b.disabled=!1,b.textContent="🚀 Créer mon profil !"}})}async function hn(t){const e=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await v.from("users").update({pending_boosters:e,onboarding_done:!1,first_booster_opened:!1}).eq("id",t)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const vn="modulepreload",wn=function(t){return"/"+t},uo={},Ao=function(e,i,o){let a=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const r=document.querySelector("meta[property=csp-nonce]"),s=(r==null?void 0:r.nonce)||(r==null?void 0:r.getAttribute("nonce"));a=Promise.allSettled(i.map(d=>{if(d=wn(d),d in uo)return;uo[d]=!0;const c=d.endsWith(".css"),l=c?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${l}`))return;const f=document.createElement("link");if(f.rel=c?"stylesheet":vn,c||(f.as="script"),f.crossOrigin="",f.href=d,s&&f.setAttribute("nonce",s),document.head.appendChild(f),c)return new Promise((b,x)=>{f.addEventListener("load",b),f.addEventListener("error",()=>x(new Error(`Unable to preload CSS for ${d}`)))})}))}function n(r){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=r,window.dispatchEvent(s),!s.defaultPrevented)throw r}return a.then(r=>{for(const s of r||[])s.status==="rejected"&&n(s.reason);return e().catch(n)})},ti="#1A6B3C",ii="#cc2222",_n="#D4A017",fo="#888";async function kn(t,e){const{state:i,toast:o}=e;t.innerHTML=`
    ...
    </div>`,await zo(i,o,e),document.getElementById("btn-add-friend").addEventListener("click",()=>En(i,o)),document.getElementById("btn-accept-friend").addEventListener("click",()=>Mo(i,o,null,e))}async function zo(t,e,i={}){const{navigate:o}=i,a=t.user.id,{data:n,error:r}=await v.from("friendships").select("id, requester_id, addressee_id").eq("status","accepted").or(`requester_id.eq.${a},addressee_id.eq.${a}`),{count:s}=await v.from("friendships").select("id",{count:"exact",head:!0}).eq("addressee_id",a).eq("status","pending"),d=document.getElementById("pending-badge");d&&(s>0?(d.style.display="flex",d.textContent=s):d.style.display="none");const c=document.getElementById("friends-list");if(!c)return;if(r){console.error("[Friends] Erreur:",r),c.innerHTML=`<div style="color:${ii};text-align:center;padding:16px">Erreur chargement : ${r.message}</div>`;return}const l=(n||[]).map(x=>x.requester_id===a?x.addressee_id:x.requester_id);let f={};if(l.length){const{data:x}=await v.from("users").select("id, pseudo, club_name, last_seen_at, club_color1, club_color2").in("id",l);(x||[]).forEach(g=>{f[g.id]=g})}const b=(n||[]).map(x=>({friendshipId:x.id,friend:f[x.requester_id===a?x.addressee_id:x.requester_id]||{pseudo:"?"}}));if(!b.length){c.innerHTML=`
    ...
    </div>`,c.querySelectorAll("[data-stats]").forEach(x=>{x.addEventListener("click",()=>Ln(t,x.dataset.stats,x.dataset.friendName))}),c.querySelectorAll("[data-match]").forEach(x=>{x.addEventListener("click",async()=>{const g=x.dataset.friendId,p=x.dataset.friendName;if(typeof o!="function"){e("Erreur navigation","error");return}const u=t.user.id,{data:y}=await v.from("friend_match_invites").select("id").eq("inviter_id",g).eq("invitee_id",u).eq("status","pending").order("created_at",{ascending:!1}).limit(1).maybeSingle(),w=!!y;console.log("[Friends] clic match",{fid:g,fname:p,isAccepting:w}),o("match",{matchMode:"friend",friendId:g,friendName:p,isAccepting:w})})})}function $n(t,e){const i=t.club_name||t.pseudo||"?",o=t.pseudo||"",a=(o||i).slice(0,2).toUpperCase(),n=t.club_color2||ti,r=t.club_color1||"#ffffff",s=t.last_seen_at?new Date(t.last_seen_at):null,d=s&&Date.now()-s.getTime()<3*60*1e3;return`
    ...
    ${Vi()}`,document.body.appendChild(c);const f=()=>c.remove();c.querySelector("#pending-close").addEventListener("click",f),c.addEventListener("click",b=>{b.target===c&&f()}),c.querySelectorAll("[data-accept]").forEach(b=>{b.addEventListener("click",async()=>{const{error:x}=await v.from("friendships").update({status:"accepted"}).eq("id",b.dataset.accept);if(x){e("Erreur : "+x.message,"error");return}b.closest("div[style]").remove(),e("✅ Ami accepté !","success"),zo(t,e,o),i&&i()})}),c.querySelectorAll("[data-decline]").forEach(b=>{b.addEventListener("click",async()=>{await v.from("friendships").delete().eq("id",b.dataset.decline),b.closest("div[style]").remove(),e("Demande refusée","info"),i&&i()})})}async function Ln(t,e,i){const o=t.user.id,[a,n]=[o,e].sort(),r=o===a,{data:s}=await v.from("friend_match_stats").select("*").eq("player1_id",a).eq("player2_id",n).single(),d=t.profile.club_name||t.profile.pseudo||"Moi",c=s||{},l=r?c.wins_p1||0:c.wins_p2||0,f=r?c.wins_p2||0:c.wins_p1||0,b=c.draws||0,x=r?c.goals_p1||0:c.goals_p2||0,g=r?c.goals_p2||0:c.goals_p1||0,p=r?c.gc_used_p1||0:c.gc_used_p2||0,u=r?c.gc_used_p2||0:c.gc_used_p1||0,y=c.matches_total||0,w=(L,m,M,E=ti,I=ii)=>`
  ...
  <div class="home-dark" id="home-dark">
    ...
    <div class="modal-body" style="padding:16px">
      ...
      </div>`}).join(""),d.querySelectorAll(".solo-level-card").forEach(c=>{c.dataset.locked!=="true"&&(c.addEventListener("mouseenter",()=>{c.style.transform="translateY(-1px)"}),c.addEventListener("mouseleave",()=>{c.style.transform=""}),c.addEventListener("click",()=>{o(),Ji(),t("match",{matchMode:"solo",soloLevel:Number(c.dataset.level)})}))})}}async function Pn(t,{state:e,navigate:i,toast:o}){const a=e.profile;if(!a)return;const n="/icons/";qo(i,a,"game",n),t.innerHTML=`
  <style>
    .game-page {
      min-height: 100%;
      background: var(--page-bg);
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    ${Se(t,{width:140})}
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),c=(s||[]).filter($=>$.card_type==="player"&&$.player),l=(s||[]).filter($=>$.card_type==="game_changer"),f=(s||[]).filter($=>$.card_type==="formation"),b=(s||[]).filter($=>$.card_type==="stadium"),{data:x}=await v.from("gc_definitions").select("name,gc_type,color,effect,image_url"),g={};(x||[]).forEach($=>{g[$.name]=$});const{data:p}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),u={};(p||[]).forEach($=>{u[$.id]=$}),b.forEach($=>{$.stadium_def&&(u[$.stadium_id]=$.stadium_def)});const y=Object.keys(Ui),w=Object.keys(Ae),h={};c.forEach($=>{const C=$.player.id;h[C]=(h[C]||0)+1}),new Set(Object.keys(h).map($=>String($)));const L=new Set(f.map($=>$.formation)),m=new Set(l.map($=>$.gc_type));let M="player",E="Tous",I="",D=!1;function O(){return[...c].sort(($,C)=>{const R=si.indexOf($.player.job),P=si.indexOf(C.player.job);return R!==P?R-P:($.player.surname_real||"").localeCompare(C.player.surname_real||"")})}function Y(){return[...d||[]].sort(($,C)=>{const R=si.indexOf($.job),P=si.indexOf(C.job);return R!==P?R-P:($.surname_real||"").localeCompare(C.surname_real||"")})}function J(){return O().filter($=>{const C=$.player,R=E==="Tous"||C.job===E,P=!I||`${C.firstname} ${C.surname_real}`.toLowerCase().includes(I);return R&&P})}function A(){return Y().filter($=>{const C=E==="Tous"||$.job===E,R=!I||`${$.firstname} ${$.surname_real}`.toLowerCase().includes(I);return C&&R})}t.innerHTML=`
        ...
        ${f.filter($=>$.id!==t.id).map(($,C)=>{const R=Y[$.id]||[],P=$.is_for_sale,X=R.length?R[R.length-1]:null,q=$.evolution_bonus||0,K=X?X.source==="booster"?"Booster":X.price?X.price.toLocaleString("fr")+" cr.":"—":"—",pe=X?new Date(X.transferred_at).toLocaleDateString("fr",{day:"2-digit",month:"2-digit",year:"numeric"}):"",oe=80,me=Math.round(oe*657/507),we=Se({...l,_evolution_bonus:q},{width:oe});return`
            <div class="exemplaire-row" data-card-id="${$.id}" data-card-idx="${C}"
              ...
              <input type="checkbox" class="expl-check"
                data-id="${$.id}" data-evo="${q}" data-note="${li(l,l.job)}"
                ${P?"disabled":""}
                style="display:none">
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",l.note_g],["DEF",l.note_d],["MIL",l.note_m],["ATT",l.note_a]].map(([$,C])=>{const R=Ai[$],P=$==="GK"?"#fff":R,X=$===l.job||$===l.job2,q=(Number(C)||0)+(X&&m>0?m:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(j=document.getElementById("close-detail"))==null||j.addEventListener("click",s);let A=new Set;const _=l.rarity==="pepite"?1.3:l.rarity==="papyte"?.7:1;function H(){let $=0;return document.querySelectorAll(".expl-check:checked").forEach(C=>{if(C.dataset.id===t.id)return;const P=Number(C.dataset.evo)||0,X=Number(C.dataset.note)||0;$+=X+P}),Math.round($*_)}const W=()=>{const $=A.size,C=document.getElementById("sell-action-panel");if(!C)return;C.style.display=$>0?"block":"none",document.getElementById("sell-selected-count").textContent=$;const R=document.getElementById("evolve-btn");if(R){const P=H();R.textContent=P>0?`⬆️ Évoluer (+${P})`:"⬆️ Évoluer"}};document.querySelectorAll(".expl-check").forEach($=>{$.addEventListener("change",()=>{const C=$.dataset.id;$.checked?A.add(C):A.delete(C);const R=$.closest(".exemplaire-row");if(R){const P=R.querySelector(".expl-sel-overlay"),X=R.querySelector(".expl-sel-check");P&&(P.style.display=$.checked?"block":"none"),X&&(X.style.display=$.checked?"flex":"none"),R.style.transform=$.checked?"scale(1.05)":"scale(1)"}W()})}),document.querySelectorAll(".exemplaire-row").forEach($=>{$.addEventListener("click",C=>{if(C.target.tagName==="INPUT")return;const R=$.querySelector(".expl-check");R&&!R.disabled&&(R.checked=!R.checked,R.dispatchEvent(new Event("change")))})}),(U=document.getElementById("evolve-btn"))==null||U.addEventListener("click",async()=>{if(b<=1)return;const $=[...A];if(!$.length)return;if(A.has(t.id)){const oe=document.createElement("div");oe.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",oe.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    </div>`;u("⚽ Choisir une formation",w),document.querySelectorAll("#modal-body [data-forma]").forEach(h=>{h.addEventListener("click",()=>{e.formation=h.dataset.forma;const L=wo(e.formation),m={};L.forEach(M=>{e.slots[M]&&(m[M]=e.slots[M])}),e.slots=m,y(),_t(t,e,i)})})};t.querySelectorAll("#formation-mobile-btn, #formation-pc-btn").forEach(u=>u.addEventListener("click",x)),t.querySelectorAll("#add-stad-btn-pc, #add-stad-btn").forEach(u=>u.addEventListener("click",()=>Jn(e,t,i))),t.querySelectorAll("#save-deck, #save-deck-pc").forEach(u=>u.addEventListener("click",()=>er(e,i))),t.querySelectorAll("[data-remove-sub]").forEach(u=>{u.addEventListener("click",()=>{e.subs=e.subs.filter(y=>y!==u.dataset.removeSub),_t(t,e,i)})}),t.querySelectorAll("#add-sub-btn").forEach(u=>u.addEventListener("click",()=>Zn(e,t,i)))}function Xn(t,e,i,o){var h,L,m;const a=window.innerWidth>=900,n=t.querySelector(a?"#deck-field-pc":"#deck-field-mobile");if(!n)return;const r=(h=e.stadiumCards)==null?void 0:h.find(M=>M.id===e.stadiumCardId),s=r&&((L=e.stadDefMap)==null?void 0:L[r.stadium_id])||null,d=wt[e.formation]||{},c=Hi(e.formation)||[],l={};for(const M of i){const E=e.slots[M],I=E?e.playerCards.find(D=>D.id===E):null;I!=null&&I.player?l[M]={...I.player,_evolution_bonus:I.evolution_bonus||0,face:I.player.face||null}:l[M]=null}const f=window.innerWidth>=900,b=f?window.innerWidth-280:window.innerWidth-20,x=f?Math.min(b,860):b,g=Math.round(f?x*.82:x*.85),p=f?84:44;let u="";for(const[M,E]of c){const I=d[M],D=d[E];if(!I||!D)continue;const O=I.x*x,Y=Math.round(.03*g+I.y*.85*g),J=D.x*x,A=Math.round(.03*g+D.y*.85*g),_=l[M],H=l[E],W=Ut(_,H);W==="#ff3333"||W==="#cc2222"?u+=`<line x1="${O.toFixed(1)}" y1="${Y.toFixed(1)}" x2="${J.toFixed(1)}" y2="${A.toFixed(1)}" stroke="${W}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(u+=`<line x1="${O.toFixed(1)}" y1="${Y.toFixed(1)}" x2="${J.toFixed(1)}" y2="${A.toFixed(1)}" stroke="${W}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,u+=`<line x1="${O.toFixed(1)}" y1="${Y.toFixed(1)}" x2="${J.toFixed(1)}" y2="${A.toFixed(1)}" stroke="${W}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let y="";const w=Math.round(p*657/507);for(const M of i){const E=d[M];if(!E)continue;const I=l[M],D=E.x*x;E.y*g;const O=Math.round(.03*g+E.y*(.85*g)),Y=Math.round(D-p/2),J=Math.round(O-w/2);if(I){const A=M.replace(/\d+/,""),_=s&&(s.club_id&&String(I.club_id)===String(s.club_id)||s.country_code&&I.country_code===s.country_code),H=Se({...I,_evolution_bonus:I._evolution_bonus||0},{width:p,showStad:!0,stadDef:s,role:A});_&&((m=s.club)!=null&&m.logo_url||s.image_url),y+=`<div style="position:absolute;left:${Y}px;top:${J}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${M}">
    ...
    <div style="position:relative;width:${x}px;height:${g}px;margin:0 auto">
      ...
      ${n.map(s=>{var b;const d=t.stadDefMap[s.stadium_id],c=((b=d==null?void 0:d.club)==null?void 0:b.logo_url)||(d==null?void 0:d.image_url)||(d!=null&&d.country_code?`https://flagsapi.com/${d.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),l=t.stadiumCardId===s.id,f=Rt((d==null?void 0:d.name)||"Stade",c,"+10⭐",{width:90});return`<div class="stad-choice" data-stad-id="${s.id}" style="cursor:pointer;position:relative;border-radius:8px;${l?"box-shadow:0 0 0 3px #4fc3f7":""}">
          ${f}
        </div>`}).join("")}
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    ${ki(t,e,i,o,a,n,r,s)}
          ...
          card:cards(id, card_type, formation, evolution_bonus,
            player:players(id,firstname,surname_real,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
              clubs(encoded_name,logo_url)))`).eq("deck_id",d).order("slot_order")]);c=E[0].data,f=E[0].error,l=E[1].data,b=E[1].error}catch(E){console.error("[Match] Exception chargement deck:",E),St(t,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>n("home"));return}if(f||b){console.error("[Match] Erreur Supabase:",f||b),St(t,"⚠️","Erreur lors du chargement du deck.","Retour",()=>n("home"));return}const x=(l||[]).filter(E=>{var I;return E.is_starter&&((I=E.card)==null?void 0:I.player)}).map(E=>Fi(E.card,E.position)),g=(l||[]).filter(E=>{var I;return!E.is_starter&&((I=E.card)==null?void 0:I.player)}).map(E=>Fi(E.card,E.position));if(x.length<11){St(t,"⚠️",`Deck incomplet (${x.length}/11).`,"Compléter",()=>n("decks"));return}const p=(l||[]).find(E=>{var I;return((I=E.card)==null?void 0:I.card_type)==="formation"}),u=(c==null?void 0:c.formation)||((M=p==null?void 0:p.card)==null?void 0:M.formation)||"4-4-2",{data:y,error:w}=await v.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",a.profile.id).eq("card_type","game_changer"),{data:h}=await v.from("gc_definitions").select("*").eq("is_active",!0),L=(y||[]).map(E=>({...E,_gcDef:(h==null?void 0:h.find(I=>I.name===E.gc_type||I.id===E.gc_definition_id))||null}));let m=null;if(c!=null&&c.stadium_card_id){const{data:E}=await v.from("cards").select("stadium_id").eq("id",c.stadium_card_id).single();if(E!=null&&E.stadium_id){const{data:I}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").eq("id",E.stadium_id).single();m=I||null}}o({deckId:d,formation:u,starters:x,subsRaw:g,gcCardsEnriched:L,gcDefs:h||[],stadiumDef:m})}function At(){return Math.min(window.innerWidth-40,860)}function Nt(){return Math.round(At()*1.1)}function Qo(t){var a,n;if(!t)return null;const e=t._line||t.job||"MIL",i=e==="GK"?t.note_g||0:e==="DEF"?t.note_d||0:e==="MIL"?t.note_m||0:t.note_a||0,o=t.stadiumBonus?10:0;return{name:t.name,firstname:t.firstname||"",note:i+(t.boost||0)+o,note_g:t.note_g||0,note_d:t.note_d||0,note_m:t.note_m||0,note_a:t.note_a||0,_evolution_bonus:0,stadiumBonus:t.stadiumBonus||!1,boost:t.boost||0,job:t.job,job2:t.job2||null,_line:t._line||t.job,_col:t._col,country_code:t.country_code,club_id:t.club_id,rarity:t.rarity,clubName:t.clubName||((a=t.clubs)==null?void 0:a.encoded_name)||null,clubLogo:t.clubLogo||((n=t.clubs)==null?void 0:n.logo_url)||null,face:t.face||null,portrait:Mt(t)}}function yr(t,e,i,o="Adversaire"){jo("/sounds/match-opening.mp3",.3);const a=At(),n=Nt();return`
    ...
    pointer-events:none;white-space:nowrap;max-width:90vw;text-align:center`,a.textContent=t,!document.getElementById("game-toast-style")){const n=document.createElement("style");n.id="game-toast-style",n.textContent=`
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${t.map((n,r)=>{const s=r<t.length-1?Ut(n,t[r+1]):null,d=!s||s==="#ff3333"||s==="#cc2222",c=s==="#00ff88"?"+10":s==="#FFD700"?"+5":"";return en(n,a),n.stadiumBonus||a&&(a.club_id&&String(n.club_id)===String(a.club_id)||a.country_code&&(n.country_code,a.country_code)),`
  ...
  <div id="match-history-panel">
    ...
    </div>`,document.body.appendChild(c),(f=document.getElementById("res-home"))==null||f.addEventListener("click",()=>{c.remove(),it(t),i.navigate("home")}),(b=document.getElementById("res-replay"))==null||b.addEventListener("click",()=>{c.remove(),it(t),i.navigate("match",e.isSolo?{matchMode:e.mode,soloLevel:e.soloLevel}:{matchMode:e.mode})})}function Br(t,e){e.openModal("Équipe adverse (IA)",`<div style="background:#0a3d1e;padding:12px;border-radius:8px">
    ...
    </div>`,(S=document.getElementById("pvp-home"))==null||S.addEventListener("click",()=>{try{v.removeChannel(J)}catch{}it(t),l?x("mini-league",{openLeagueId:l}):x("home")})}U()}async function Ni(t,e,i=!1){var s,d;const{state:o,navigate:a,toast:n}=e,r=i&&((d=(s=e==null?void 0:e.state)==null?void 0:s.params)==null?void 0:d.rankedData)||null;await $i(t,e,i?"ranked":"random",async({deckId:c,formation:l,starters:f,subsRaw:b,gcCardsEnriched:x,gcDefs:g,stadiumDef:p})=>{lt(t);const u=async y=>{await Nr(t,e,c,l,f,b,y||[],g,p,i,r)};if(!(x!=null&&x.length)){await u([]);return}_i(t,x,u)})}async function Gr(t,e,i){return Pr(t,e,i)}async function Nr(t,e,i,o,a,n,r=[],s=[],d=null,c=!1,l=null){var Y,J;const{state:f,navigate:b,toast:x}=e,g=(l==null?void 0:l.mmr)??((Y=f.profile)==null?void 0:Y.mmr)??1e3,p=c?wi(g):null,u=c?"linear-gradient(135deg, #1a0a2e 0%, #2d1060 50%, #1a0a2e 100%)":"linear-gradient(135deg, #0a1a2e 0%, #0d3d1e 100%)",y=c?(p==null?void 0:p.color)||"#D4A017":"#FFD700";let w=!1,h=null,L=null;t.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    </div>`,document.body.appendChild(x),x.querySelector("#lv-ok").onclick=()=>{x.remove(),b(!0)},x.querySelector("#lv-cancel").onclick=()=>{x.remove(),b(!1)}}))return;const{data:d,error:c}=await v.rpc("leave_mini_league",{p_league_id:i,p_user_id:r});if(c||!(d!=null&&d.success)){a((d==null?void 0:d.error)||"Erreur lors de la désinscription","error");return}const l=d.refund||o;n.profile&&(n.profile.credits=(n.profile.credits||0)+l);const f=document.getElementById("nav-credits");f&&(f.textContent=`💰 ${(n.profile.credits||0).toLocaleString("fr")}`),a(`↩️ Désinscrit · +${l.toLocaleString("fr")} cr. remboursés`,"success"),et(t,e,"waiting")}async function Ri(t,e,i,o,a){const{toast:n}=e;if(!confirm(`Supprimer définitivement "${o}" et tous ses matchs/membres ? Action irréversible.`))return;await v.from("mini_league_matches").delete().eq("league_id",i),await v.from("mini_league_members").delete().eq("league_id",i);const{error:r}=await v.from("mini_leagues").delete().eq("id",i);if(r){n("Erreur suppression ("+r.message+")","error");return}n("Mini League supprimée avec succès","success"),et(t,e,a)}async function ta(t,e,i){await v.from("mini_leagues").update({is_archived:!0}).eq("id",i),e.toast("Mini League archivée","success"),et(t,e,"archived")}async function ct(t,e,i){var L,m,M,E,I,D,O,Y,J;const{state:o,toast:a}=e,n=o.profile.id,[{data:r},{data:s},{data:d}]=await Promise.all([v.from("mini_leagues").select("*").eq("id",i).single(),v.from("mini_league_members").select("*, user:users(id,pseudo,club_name,club_color1,club_color2)").eq("league_id",i),v.from("mini_league_matches").select("*").eq("league_id",i).order("matchday").order("created_at")]);if(!r){a("Introuvable","error"),await et(t,e);return}const c=(s||[]).some(A=>A.user_id===n),l=r.creator_id===n,f=(s||[]).map(A=>A.user).filter(Boolean),b=aa(f,d||[]),x=(d||[]).filter(A=>A.matchday===r.current_day),g=r.pot||0,p=r.entry_fee||100,u=x.length>0&&x.every(A=>A.status==="finished"||A.status==="bye"),y=r.current_day>=r.total_days,w=(s||[]).find(A=>A.user_id===n);t.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="font-size:13px;font-weight:900;min-width:50px;text-align:center;color:${t.status==="finished"?"#4ade80":Ze}">${l}</div>
  ...
  <div class="page">
    ...
    <nav class="top-nav">
      <div class="logo" id="nav-logo" title="Manager Wars v2026.07.26-1851" style="cursor:pointer">
