import{s as A,F as Ot,h as vt,j as $i,l as Je,i as Xi,k as Ji,b as Qi}from"./formation-links-CEzN0rcz.js";function ci(e,{navigate:t,toast:i}){e.innerHTML=`
  ...
  `,e.querySelectorAll(".auth-tab").forEach(n=>{n.addEventListener("click",()=>{e.querySelectorAll(".auth-tab").forEach(p=>p.classList.remove("active")),n.classList.add("active"),document.getElementById("tab-login").style.display=n.dataset.tab==="login"?"block":"none",document.getElementById("tab-register").style.display=n.dataset.tab==="register"?"block":"none"})}),document.getElementById("login-btn").addEventListener("click",async()=>{const n=document.getElementById("login-email").value.trim(),p=document.getElementById("login-password").value,a=document.getElementById("login-error");if(a.textContent="",!n||!p){a.textContent="Remplissez tous les champs.";return}const s=document.getElementById("login-btn");s.textContent="Connexion…",s.disabled=!0;const{error:d}=await A.auth.signInWithPassword({email:n,password:p});if(s.textContent="Se connecter",s.disabled=!1,d){a.textContent=d.message.includes("Invalid")?"Email ou mot de passe incorrect.":d.message;return}window.location.reload()}),document.getElementById("login-password").addEventListener("keydown",n=>{n.key==="Enter"&&document.getElementById("login-btn").click()}),document.getElementById("reg-btn").addEventListener("click",async()=>{const n=document.getElementById("reg-email").value.trim(),p=document.getElementById("reg-password").value,a=document.getElementById("reg-confirm").value,s=document.getElementById("reg-error");if(s.textContent="",!n||!p||!a){s.textContent="Remplissez tous les champs.";return}if(p.length<6){s.textContent="Mot de passe trop court (min. 6 caractères).";return}if(p!==a){s.textContent="Les mots de passe ne correspondent pas.";return}const d=document.getElementById("reg-btn");d.textContent="Création…",d.disabled=!0;const{error:c}=await A.auth.signUp({email:n,password:p});if(d.textContent="Créer mon compte",d.disabled=!1,c){s.textContent=c.message;return}i("Compte créé ! Connectez-vous.","success",4e3),document.querySelector('[data-tab="login"]').click(),document.getElementById("login-email").value=n})}function Zi(e,{state:t,navigate:i,toast:n,refreshProfile:p}){let a="#1A6B3C",s="#D4A017";e.innerHTML=`
  ...
  `;function d(){var y;const o=document.getElementById("logo-preview"),r=document.getElementById("logo-initials"),u=((y=document.getElementById("setup-club"))==null?void 0:y.value)||"MW",f=u.trim().split(" ").filter(Boolean),g=f.length>=2?(f[0][0]+f[1][0]).toUpperCase():u.slice(0,2).toUpperCase();o&&(o.style.background=s,o.style.borderColor=a),r&&(r.textContent=g,r.style.color=a)}document.getElementById("color1").addEventListener("input",o=>{a=o.target.value,document.getElementById("swatch1").style.background=a,d()}),document.getElementById("color2").addEventListener("input",o=>{s=o.target.value,document.getElementById("swatch2").style.background=s,d()});function c(o){document.querySelectorAll(".setup-step").forEach(r=>r.classList.remove("active")),document.getElementById(`step-${o}`).classList.add("active"),document.getElementById("step-num").textContent=o,document.getElementById("progress-fill").style.width=`${Math.round(o/3*100)}%`,o===3&&d()}document.getElementById("step1-next").addEventListener("click",async()=>{const o=document.getElementById("setup-pseudo").value.trim(),r=document.getElementById("step1-error");if(r.textContent="",o.length<3){r.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:u}=await A.from("users").select("id").eq("pseudo",o).maybeSingle();if(u){r.textContent="Ce pseudo est déjà pris.";return}c(2)}),document.getElementById("step2-back").addEventListener("click",()=>c(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const o=document.getElementById("setup-club").value.trim(),r=document.getElementById("step2-error");if(r.textContent="",o.length<2){r.textContent="Nom trop court (min. 2 caractères).";return}const{data:u}=await A.from("users").select("id").eq("club_name",o).maybeSingle();if(u){r.textContent="Ce nom de club est déjà pris.";return}c(3)}),document.getElementById("step3-back").addEventListener("click",()=>c(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const o=document.getElementById("setup-pseudo").value.trim(),r=document.getElementById("setup-club").value.trim(),u=document.getElementById("step3-error"),f=document.getElementById("step3-finish");u.textContent="",f.disabled=!0,f.textContent="Création en cours…";try{const{error:g}=await A.from("users").insert({id:t.user.id,pseudo:o,club_name:r,club_color1:a,club_color2:s,credits:1e4});if(g)throw g;await eo(t.user.id),await p(),n(`Bienvenue ${o} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(g){u.textContent=g.message,f.disabled=!1,f.textContent="🚀 Créer mon profil !"}})}async function eo(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await A.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const mt="#1A6B3C",xt="#cc2222",to="#D4A017",pi="#888";async function io(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    </div>`,await _i(i,n),document.getElementById("btn-add-friend").addEventListener("click",()=>no(i,n)),document.getElementById("btn-accept-friend").addEventListener("click",()=>ki(i,n))}async function _i(e,t){const i=e.user.id,{data:n,error:p}=await A.from("friendships").select(`
    ...
    `).eq("status","accepted").or(`requester_id.eq.${i},addressee_id.eq.${i}`),{count:a}=await A.from("friendships").select("id",{count:"exact",head:!0}).eq("addressee_id",i).eq("status","pending"),s=document.getElementById("pending-badge");s&&(a>0?(s.style.display="flex",s.textContent=a):s.style.display="none");const d=document.getElementById("friends-list");if(!d)return;if(p){d.innerHTML=`<div style="color:${xt};text-align:center;padding:16px">Erreur chargement des amis</div>`;return}const c=(n||[]).map(o=>{const r=o.requester_id===i;return{friendshipId:o.id,friend:r?o.addressee:o.requester}});if(!c.length){d.innerHTML=`
      <div style="text-align:center;padding:32px;color:#aaa">
        <div style="font-size:40px;margin-bottom:8px">👥</div>
        <div>Tu n'as pas encore d'amis.<br>Commence par en ajouter !</div>
      </div>`;return}d.innerHTML=`
    <div style="font-size:12px;color:#999;font-weight:700;letter-spacing:1px;text-transform:uppercase;margin-bottom:8px">
      ${c.length} ami${c.length>1?"s":""}
    </div>
    <div style="display:flex;flex-direction:column;gap:8px">
      ${c.map(({friendshipId:o,friend:r})=>oo(r,o)).join("")}
    </div>`,d.querySelectorAll("[data-stats]").forEach(o=>{o.addEventListener("click",()=>ro(e,o.dataset.stats,o.dataset.friendName))}),d.querySelectorAll("[data-match]").forEach(o=>{o.addEventListener("click",()=>t("⚽ Match ami — Bientôt disponible !","info"))})}function oo(e,t){const i=e.club_name||e.pseudo||"?",n=(e.pseudo||"?").slice(0,2).toUpperCase();return`
    <div style="display:flex;align-items:center;gap:12px;background:#fff;border-radius:12px;padding:12px 14px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
      <!-- Avatar -->
      <div style="width:42px;height:42px;border-radius:50%;background:${mt};display:flex;align-items:center;justify-content:center;font-size:16px;font-weight:900;color:#fff;flex-shrink:0">
        ${n}
      </div>
      <!-- Infos -->
    ...
    ${Ut()}`,document.body.appendChild(i);const n=i.querySelector("#friend-pseudo-input"),p=i.querySelector("#add-friend-error"),a=()=>i.remove();n.focus(),i.querySelector("#add-cancel").addEventListener("click",a),i.addEventListener("click",s=>{s.target===i&&a()}),i.querySelector("#add-ok").addEventListener("click",async()=>{const s=n.value.trim();if(!s){p.textContent="Entre un pseudo";return}p.textContent="";const{data:d}=await A.from("users").select("id, pseudo").ilike("pseudo",s).single();if(!d){p.textContent="Utilisateur introuvable";return}if(d.id===e.user.id){p.textContent="Tu ne peux pas t'ajouter toi-même";return}const{data:c}=await A.from("friendships").select("id, status").or(`and(requester_id.eq.${e.user.id},addressee_id.eq.${d.id}),and(requester_id.eq.${d.id},addressee_id.eq.${e.user.id})`).single();if(c){const r=c.status==="accepted"?"Vous êtes déjà amis !":c.status==="pending"?"Demande déjà envoyée":"Une demande existe déjà";p.textContent=r;return}const{error:o}=await A.from("friendships").insert({requester_id:e.user.id,addressee_id:d.id,status:"pending"});if(o){p.textContent="Erreur : "+o.message;return}a(),t(`✅ Demande envoyée à ${d.pseudo} !`,"success")})}async function ki(e,t,i=null){const n=e.user.id,{data:p}=await A.from("friendships").select("id, requester_id, requester:users!requester_id(id, pseudo, club_name)").eq("addressee_id",n).eq("status","pending").order("created_at",{ascending:!1}),a=Ht(),s=p||[];a.innerHTML=`
    <div class="popup-box">
      ...
      ${s.length?`<div style="display:flex;flex-direction:column;gap:8px;max-height:50vh;overflow-y:auto;margin-bottom:14px">
            ${s.map(c=>{var o,r,u;return`
              <div style="display:flex;align-items:center;gap:10px;background:#f9f9f9;border-radius:10px;padding:10px 12px">
                <div style="flex:1;font-size:14px;font-weight:700">${((o=c.requester)==null?void 0:o.club_name)||((r=c.requester)==null?void 0:r.pseudo)||"?"}
                  <span style="font-size:11px;color:#999;font-weight:400">(${((u=c.requester)==null?void 0:u.pseudo)||""})</span>
                </div>
                <button data-accept="${c.id}" title="Accepter"
                  style="width:34px;height:34px;border-radius:50%;border:none;background:${mt};color:#fff;font-size:18px;cursor:pointer">✓</button>
                <button data-decline="${c.id}" title="Refuser"
    ...
    ${Ut()}`,document.body.appendChild(a);const d=()=>a.remove();a.querySelector("#pending-close").addEventListener("click",d),a.addEventListener("click",c=>{c.target===a&&d()}),a.querySelectorAll("[data-accept]").forEach(c=>{c.addEventListener("click",async()=>{const{error:o}=await A.from("friendships").update({status:"accepted"}).eq("id",c.dataset.accept);if(o){t("Erreur : "+o.message,"error");return}c.closest("div[style]").remove(),t("✅ Ami accepté !","success"),_i(e,t),i&&i()})}),a.querySelectorAll("[data-decline]").forEach(c=>{c.addEventListener("click",async()=>{await A.from("friendships").delete().eq("id",c.dataset.decline),c.closest("div[style]").remove(),t("Demande refusée","info"),i&&i()})})}async function ro(e,t,i){const n=e.user.id,[p,a]=[n,t].sort(),s=n===p,{data:d}=await A.from("friend_match_stats").select("*").eq("player1_id",p).eq("player2_id",a).single(),c=e.profile.club_name||e.profile.pseudo||"Moi",o=d||{},r=s?o.wins_p1||0:o.wins_p2||0,u=s?o.wins_p2||0:o.wins_p1||0,f=o.draws||0,g=s?o.goals_p1||0:o.goals_p2||0,y=s?o.goals_p2||0:o.goals_p1||0,l=s?o.gc_used_p1||0:o.gc_used_p2||0,x=s?o.gc_used_p2||0:o.gc_used_p1||0,v=o.matches_total||0,h=(D,_,w,S=mt,ee=xt)=>`
    ...
    </style>`}const ao="2026.06.27-1452";async function ui(e,{state:t,navigate:i,toast:n}){var a,s;const p=t.profile;p&&(e.innerHTML=`
  <div class="page">
    <div class="page-body">
  ...
  `,document.body.appendChild(i);const n=()=>i.remove();document.getElementById("diff-cancel").addEventListener("click",n),i.addEventListener("click",p=>{p.target===i&&n()}),i.querySelectorAll("[data-mode]").forEach(p=>{p.addEventListener("click",()=>{n(),e("match",{matchMode:p.dataset.mode})})})}const Ce={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé pour ce match."},"Double attaque":{icon:"⚡",desc:"La note d'attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la note d'un joueur adverse."},Gel:{icon:"❄️",desc:"Bloque le meilleur attaquant IA."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function me(e,t){if(!e)return 0;switch(t){case"GK":return Number(e.note_g)||0;case"DEF":return Number(e.note_d)||0;case"MIL":return Number(e.note_m)||0;case"ATT":return Number(e.note_a)||0;default:return 0}}const fi=["ATT","MIL","DEF","GK"];function Ai(e){let t=0;const i=e.length;for(let n=0;n<i;n++)for(let p=n+1;p<i;p++){const a=e[n],s=e[p];if(!a||!s)continue;const d=a._col!=null&&s._col!=null&&a._col===s._col,c=a._col!=null&&s._col!=null&&Math.abs(a._col-s._col)===1,o=fi.indexOf(a._line||a.job),r=fi.indexOf(s._line||s.job),u=Math.abs(o-r)===1;if(!((a._line||a.job)===(s._line||s.job)&&c||d&&u))continue;const y=a.country_code&&s.country_code&&a.country_code===s.country_code,l=a.club_id&&s.club_id&&a.club_id===s.club_id;y&&l?t+=2:(y||l)&&(t+=1)}return t}function Kt(e,t={}){const i=e.reduce((a,s)=>{const d=s._line||s.job;return a+(Number(d==="MIL"?s.note_m:s.note_a)||0)+(s.boost||0)},0),n=Ai(e);let p=i+n;return t.doubleAttack&&(p*=2),t.stolenNote&&(p-=t.stolenNote),{base:i,links:n,total:Math.max(0,p)}}function Vt(e,t={}){const i=e.reduce((a,s)=>{const d=s._line||s.job;let c=0;return d==="GK"?c=Number(s.note_g)||0:d==="MIL"?c=Number(s.note_m)||0:c=Number(s.note_d)||0,a+c+(s.boost||0)},0),n=Ai(e);let p=i+n;return t.stolenNote&&(p-=t.stolenNote),{base:i,links:n,total:Math.max(0,p)}}function Yt(e,t,i={}){return i.shield?{goal:!1,shielded:!0}:{goal:e>t,shielded:!1}}function Ti(e,t,i="easy"){const n=e.filter(s=>!s.used);if(!n.length)return[];const p=[...n].sort((s,d)=>{const c=t==="attack"?me(s,"ATT"):s._line==="GK"?me(s,"GK"):me(s,"DEF");return(t==="attack"?me(d,"ATT"):d._line==="GK"?me(d,"GK"):me(d,"DEF"))-c});let a=i==="easy"?1+Math.floor(Math.random()*2):i==="medium"?2+Math.floor(Math.random()*2):3;return p.slice(0,Math.min(a,p.length,3))}function lo(e,t){const i={vs_ai_easy:{victoire:500,nul:250,defaite:50},vs_ai_medium:{victoire:1e3,nul:500,defaite:50},vs_ai_hard:{victoire:1500,nul:750,defaite:100},vs_ai_club:{victoire:2500,nul:1250,defaite:100}};return(i[e]||i.vs_ai_easy)[t]||0}const Ii={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"},ft={GK:"#111111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},wt=["GK","DEF","MIL","ATT"],co=["Tous","GK","DEF","MIL","ATT"],po={normal:1e3,pepite:5e3,papyte:5e3,legende:1e4},Wt={MA:"MAROC",FR:"FRANCE",AR:"ARGENTINE",PT:"PORTUGAL",BR:"BRESIL",ES:"ESPAGNE",DE:"ALLEMAGNE",GB:"ANGLETERRE",IT:"ITALIE",CM:"CAMEROUN",SN:"SENEGAL",NG:"NIGERIA",DK:"DANEMARK",NL:"PAYS-BAS",BE:"BELGIQUE",CI:"CÔTE D'IVOIRE",AL:"ALBANIE",HR:"CROATIE",RS:"SERBIE",TR:"TURQUIE"};function Li(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";if(!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function yt(e,t){return e&&Number(t==="GK"?e.note_g:t==="DEF"?e.note_d:t==="MIL"?e.note_m:e.note_a)||0}function St(e,t=""){var u,f;const i=e.player;if(!i)return"";const n=i.job||"ATT",p=ft[n],a=Ii[i.rarity]||"#ccc",s=yt(i,n),d=i.job2?yt(i,i.job2):null,c=i.job2?ft[i.job2]:null,o=Li(i),r=Wt[i.country_code]||i.country_code||"";return`
  <div style="
  ...
  </div>`}async function uo(e,t){const{state:i,navigate:n,toast:p,openModal:a,closeModal:s}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:d}=await A.from("cards").select(`id, card_type, current_note, gc_type, formation, is_for_sale, sale_price,
      player:players(id, firstname, surname_encoded, country_code, club_id, job, job2,
        note_g, note_d, note_m, note_a, rarity, note_min, note_max, skin, hair, hair_length, sell_price,
        clubs(encoded_name, logo_url))`).eq("owner_id",i.profile.id),{data:c}=await A.from("players").select(`id, firstname, surname_encoded, country_code, club_id, job, job2,
      note_g, note_d, note_m, note_a, rarity, note_min, note_max, skin, hair, hair_length,
      clubs(encoded_name, logo_url)`).eq("is_active",!0),o=(d||[]).filter(M=>M.card_type==="player"&&M.player),r=(d||[]).filter(M=>M.card_type==="game_changer"),u=(d||[]).filter(M=>M.card_type==="formation"),{data:f}=await A.from("gc_definitions").select("name,gc_type,color,effect,image_url"),g={};(f||[]).forEach(M=>{g[M.name]=M});const y=Object.keys(Ot),l=Object.keys(Ce),x={};o.forEach(M=>{const C=M.player.id;x[C]=(x[C]||0)+1}),new Set(Object.keys(x).map(M=>String(M)));const v=new Set(u.map(M=>M.formation)),h=new Set(r.map(M=>M.gc_type));let I="player",D="Tous",_="",w=!1;function S(){return[...o].sort((M,C)=>{const N=wt.indexOf(M.player.job),j=wt.indexOf(C.player.job);return N!==j?N-j:(M.player.surname_encoded||"").localeCompare(C.player.surname_encoded||"")})}function ee(){return[...c||[]].sort((M,C)=>{const N=wt.indexOf(M.job),j=wt.indexOf(C.job);return N!==j?N-j:(M.surname_encoded||"").localeCompare(C.surname_encoded||"")})}function re(){return S().filter(M=>{const C=M.player,N=D==="Tous"||C.job===D,j=!_||`${C.firstname} ${C.surname_encoded}`.toLowerCase().includes(_);return N&&j})}function te(){return ee().filter(M=>{const C=D==="Tous"||M.job===D,N=!_||`${M.firstname} ${M.surname_encoded}`.toLowerCase().includes(_);return C&&N})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(l=document.getElementById("direct-sell-form-btn"))==null||l.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${o} pour ${lt.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const h=f.find(D=>!D.is_for_sale)||f[0];if(!h){a("Aucune carte à vendre","error");return}const{error:I}=await A.from("cards").delete().eq("id",h.id);if(I){a(I.message,"error");return}await A.from("users").update({credits:(p.profile.credits||0)+lt}).eq("id",p.profile.id),await c(),a(`+${lt.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),s(),d("collection")}),(x=document.getElementById("market-sell-form-btn"))==null||x.addEventListener("click",async()=>{const h=parseInt(document.getElementById("sell-price-form").value);if(!h||h<1){a("Prix invalide","error");return}await A.from("cards").update({is_for_sale:!0,sale_price:h}).eq("id",e.id),await A.from("market_listings").insert({seller_id:p.profile.id,card_id:e.id,price:h}),a("Carte mise en vente sur le marché !","success"),s(),d("collection")}),(v=document.getElementById("cancel-sell-form-btn"))==null||v.addEventListener("click",async()=>{await A.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await A.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),a("Annonce retirée","success"),s(),d("collection")})}async function mi(e,t,i,n){var te,$,R,Y,E,K;const{state:p,toast:a,openModal:s,closeModal:d,navigate:c,refreshProfile:o}=n,r=e.player,u=t.filter(G=>G.player.id===r.id),f=u.length,g=po[r.rarity]||1e3,y=r.rarity!=="legende",l=Li(r),x=yt(r,r.job),v=r.job2?yt(r,r.job2):null,h=ft[r.job]||"#1A6B3C",I=r.job2?ft[r.job2]:null,D=Ii[r.rarity]||"#ccc",_=Wt[r.country_code]||r.country_code||"",w=u.map(G=>G.id);let S={};if(w.length){const{data:G}=await A.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",w).order("transferred_at",{ascending:!0});(G||[]).forEach(J=>{S[J.card_id]||(S[J.card_id]=[]),S[J.card_id].push(J)})}const ee=G=>`
  ...
  </div>`:`<div style="width:${t}px;height:${i}px;border:2px dashed rgba(255,255,255,0.3);border-radius:5px;display:flex;align-items:center;justify-content:center;font-size:20px;color:rgba(255,255,255,0.3)">+</div>`}async function Bt(e,t){const{state:i,navigate:n,toast:p}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:a}=await A.from("decks").select("id,name,formation_card_id").eq("owner_id",i.profile.id).order("created_at",{ascending:!1});e.innerHTML=`
  <div class="page">
  ...
  </div>`,document.getElementById("new-deck-btn").addEventListener("click",async()=>{const s=prompt("Nom du deck :",`Deck ${((a==null?void 0:a.length)||0)+1}`);if(!s)return;const{data:d,error:c}=await A.from("decks").insert({owner_id:i.profile.id,name:s}).select().single();if(c){p(c.message,"error");return}p("Deck créé !","success"),xi(d.id,e,t)}),e.querySelectorAll("[data-open-deck]").forEach(s=>{s.addEventListener("click",()=>xi(s.dataset.openDeck,e,t))}),e.querySelectorAll("[data-rename]").forEach(s=>{s.addEventListener("click",async()=>{const d=prompt("Nouveau nom :",s.dataset.name);if(!d||d===s.dataset.name)return;const{error:c}=await A.from("decks").update({name:d}).eq("id",s.dataset.rename);if(c){p(c.message,"error");return}p("Deck renommé !","success"),Bt(e,t)})}),e.querySelectorAll("[data-delete]").forEach(s=>{s.addEventListener("click",async()=>{if(!confirm(`Supprimer le deck "${s.dataset.name}" ? Cette action est irréversible.`))return;await A.from("deck_cards").delete().eq("deck_id",s.dataset.delete);const{error:d}=await A.from("decks").delete().eq("id",s.dataset.delete);if(d){p(d.message,"error");return}p("Deck supprimé.","success"),Bt(e,t)})})}async function xi(e,t,i){const{state:n,toast:p}=i;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:a}=await A.from("decks").select("*").eq("id",e).single(),{data:s}=await A.from("cards").select(`id, card_type, formation,
      player:players(id, firstname, surname_encoded, country_code, club_id, job, job2,
        note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length,
        clubs(encoded_name, logo_url))`).eq("owner_id",n.profile.id),d=(s||[]).filter(g=>g.card_type==="player"&&g.player),c=(s||[]).filter(g=>g.card_type==="formation"),o=c.map(g=>g.formation).filter(Boolean),{data:r}=await A.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",e);let u=a.formation||"4-4-2";o.length>0&&!o.includes(u)&&(u=o[0]);const f={deckId:e,name:a.name,formation:u,formationCardId:a.formation_card_id,slots:{},subs:[],playerCards:d,formationCards:c,availableFormations:o};(r||[]).forEach(g=>{g.is_starter?f.slots[g.position]=g.card_id:f.subs.includes(g.card_id)||f.subs.push(g.card_id)}),ot(t,f,i)}function ot(e,t,i){var c;const{navigate:n}=i;Et[t.formation];const p=yi(t.formation),a=p.filter(o=>t.slots[o]).length,s=t.availableFormations.length>0?t.availableFormations:Object.keys(Et),d=t.subs.map(o=>t.playerCards.find(r=>r.id===o)).filter(Boolean);[...Object.values(t.slots),...t.subs],e.innerHTML=`
  <div class="page">
    ...
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(g=document.getElementById("close-selector"))==null||g.addEventListener("click",a),(y=document.getElementById("remove-player"))==null||y.addEventListener("click",()=>{delete t.slots[e],a(),ot(i,t,n)}),document.querySelectorAll(".player-option").forEach(l=>{l.addEventListener("click",()=>{t.slots[e]=l.dataset.cardId,a(),ot(i,t,n)})})}function ho(e,t,i){var c;const{openModal:n,closeModal:p}=i,a=new Set;Object.values(e.slots).filter(Boolean).forEach(o=>{var u;const r=e.playerCards.find(f=>f.id===o);(u=r==null?void 0:r.player)!=null&&u.id&&a.add(r.player.id)}),e.subs.forEach(o=>{var u;const r=e.playerCards.find(f=>f.id===o);(u=r==null?void 0:r.player)!=null&&u.id&&a.add(r.player.id)});const s=new Set,d=e.playerCards.filter(o=>{var r,u,f;return a.has((r=o.player)==null?void 0:r.id)||s.has((u=o.player)==null?void 0:u.id)?!1:(s.add((f=o.player)==null?void 0:f.id),!0)});n("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
  ...
  </div>`}function ji(e){var p;const t={};(e.rates||[]).forEach(a=>{t[a.card_type]=(t[a.card_type]||0)+Number(a.percentage||0)});const i=((p=Object.entries(t).sort((a,s)=>s[1]-a[1])[0])==null?void 0:p[0])||"player",n=e.image_url||"booster-players.png";return{id:e.id,img:"/manager-wars/icons/"+n,name:e.name,sub:`${e.card_count} carte(s)`,cost:e.price_type==="credits"&&e.price_credits||0,costLabel:e.price_type==="credits"?`${(e.price_credits||0).toLocaleString("fr")} crédits`:e.price_type==="pub"?"1 pub":"Gratuit",cardCount:e.card_count||5,type:i,isPub:e.price_type==="pub",rates:e.rates||[],_raw:e}}async function Ao(e,{state:t,navigate:i,toast:n}){var s;const p=((s=t.profile)==null?void 0:s.credits)||0;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⏳ Chargement...</div>';let a=[];try{a=(await Si()).map(ji)}catch(d){console.warn("Erreur chargement boosters DB, fallback hardcodé",d)}a.length||(a=wo.map(d=>({...d,rates:[],isPub:d.id==="players_pub"}))),e.innerHTML=`
  <div class="page">
    <div class="page-header">
      <h2>📦 Boosters</h2>
    ...
    <div class="page-body">
      <div class="booster-grid">
        ${a.map(d=>{const c=d.cost===0||p>=d.cost;return`<div class="booster-card ${c?"":"disabled"}" data-booster="${d.id}" style="position:relative">
  ...
  </div>`,e.querySelectorAll(".booster-card:not(.disabled)").forEach(d=>{d.addEventListener("click",async()=>{const c=a.find(o=>o.id===d.dataset.booster);if(c){d.style.opacity="0.5",d.style.pointerEvents="none";try{await To(c,{state:t,toast:n,navigate:i,container:e})}catch(o){n(o.message,"error"),d.style.opacity="",d.style.pointerEvents=""}}})}),e.querySelectorAll(".booster-info-btn").forEach(d=>{d.addEventListener("click",c=>{c.stopPropagation();const o=a.find(r=>r.id===d.dataset.boosterId);Mo(o)})})}async function To(e,{state:t,toast:i,navigate:n,container:p}){var r;if(e.cost>0&&t.profile.credits<e.cost){i("Crédits insuffisants","error");return}if(e.isPub)try{await jo()}catch(u){i(u.message||"Regardez la pub entièrement pour ouvrir le booster","error");return}const{data:a}=await A.from("cards").select("card_type, player_id, formation").eq("owner_id",t.profile.id),s=new Set((a||[]).filter(u=>u.card_type==="player").map(u=>u.player_id)),d=new Set((a||[]).filter(u=>u.card_type==="formation").map(u=>u.formation));let c=[];if((r=e.rates)!=null&&r.length)c=await Dt(t.profile,e);else{const u=e.type||"player";u==="player"?c=await Bi(t.profile,e.cardCount,e.cost):u==="game_changer"?c=await Fi(t.profile,e.cardCount,e.cost):u==="formation"?c=await Di(t.profile,e.cost):c=await Dt(t.profile,e)}c.forEach(u=>{u.card_type==="player"&&u.player?u.isDuplicate=s.has(u.player.id):u.card_type==="formation"&&(u.isDuplicate=d.has(u.formation))});const{data:o}=await A.from("users").select("*").eq("id",t.profile.id).single();o&&(t.profile=o),qi(c,e,n)}function Io(){const e=Math.random()*100;return e<.5?"legende":e<2?"special":e<10?"normal_high":"normal_low"}function Oe(e){return Math.max(Number(e.note_g)||0,Number(e.note_d)||0,Number(e.note_m)||0,Number(e.note_a)||0)}function Lo(e,t){let i;switch(t){case"legende":i=e.filter(n=>n.rarity==="legende"),i.length||(i=e.filter(n=>n.rarity==="pepite"||n.rarity==="papyte")),i.length||(i=e.filter(n=>Oe(n)>=6));break;case"special":i=e.filter(n=>n.rarity==="pepite"||n.rarity==="papyte"),i.length||(i=e.filter(n=>Oe(n)>=6));break;case"normal_high":i=e.filter(n=>n.rarity==="normal"&&Oe(n)>=6),i.length||(i=e.filter(n=>Oe(n)>=6));break;default:i=e.filter(n=>n.rarity==="normal"&&Oe(n)>=1&&Oe(n)<=5),i.length||(i=e.filter(n=>n.rarity==="normal"));break}return i.length||(i=e),i[Math.floor(Math.random()*i.length)]}async function Dt(e,t){if(t.cost>0){const{error:d}=await A.from("users").update({credits:e.credits-t.cost}).eq("id",e.id);if(d)throw d}const{data:i}=await A.from("cards").select("player_id, card_type, formation").eq("owner_id",e.id),n=new Set((i||[]).filter(d=>d.card_type==="player").map(d=>d.player_id)),p=new Set((i||[]).filter(d=>d.card_type==="formation").map(d=>d.formation)),a=new Set,s=[];for(let d=0;d<(t.cardCount||5);d++){const c=vo(t.rates);if(c){if(c.card_type==="player"){const o=h=>({légende:"legende",pépite:"pepite",pépites:"pepite"})[h]||h,r=c.rarity?o(c.rarity):null;let u=A.from("players").select("id,job,firstname,surname_encoded,country_code,club_id,rarity,note_g,note_d,note_m,note_a,skin,hair,hair_length,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);r&&(u=u.eq("rarity",r));const{data:f}=await u;let g=f||[];if((c.note_min||c.note_max)&&(g=g.filter(h=>{const I=Math.max(Number(h.note_g)||0,Number(h.note_d)||0,Number(h.note_m)||0,Number(h.note_a)||0);return(!c.note_min||I>=c.note_min)&&(!c.note_max||I<=c.note_max)})),g.length||(c.note_min||c.note_max?(g=f||[],console.warn("[Booster] Aucun joueur avec note",c.note_min,"-",c.note_max,"— fallback rareté uniquement")):g=f||[]),!g.length)continue;let y=g.filter(h=>!a.has(h.id));y.length||(y=g);const l=y[Math.floor(Math.random()*y.length)];a.add(l.id);const x=n.has(l.id),{data:v}=await A.from("cards").insert({owner_id:e.id,player_id:l.id,card_type:"player"}).select().single();v&&(s.push({...v,player:l,isDuplicate:x}),A.rpc("record_transfer",{p_card_id:v.id,p_player_id:l.id,p_club_name:e.club_name||e.pseudo,p_manager_name:e.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{}))}else if(c.card_type==="game_changer"){const{data:o}=await A.from("gc_definitions").select("id,name").eq("is_active",!0).eq("gc_type","game_changer"),r=o!=null&&o.length?o:[{name:"Ressusciter"},{name:"Double attaque"},{name:"Bouclier"},{name:"Vol de note"},{name:"Gel"}],f=r[Math.floor(Math.random()*r.length)].name,{data:g}=await A.from("cards").insert({owner_id:e.id,card_type:"game_changer",gc_type:f}).select().single();g&&s.push(g)}else if(c.card_type==="formation"){const o=Ci(),r=o[Math.floor(Math.random()*o.length)],u=p.has(r),{data:f}=await A.from("cards").insert({owner_id:e.id,card_type:"formation",formation:r}).select();f!=null&&f[0]&&s.push({...f[0],isDuplicate:u})}}}return s}async function Bi(e,t,i){if(i>0){const{error:o}=await A.from("users").update({credits:e.credits-i}).eq("id",e.id);if(o)throw o}const{data:n}=await A.from("players").select("id,job,firstname,surname_encoded,country_code,club_id,rarity,note_g,note_d,note_m,note_a,note_min,note_max,skin,hair,hair_length,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);if(!(n!=null&&n.length))throw new Error("Pas de joueurs en BDD — ajoutes-en via le panel admin !");const p=n.filter(o=>o.job==="GK"),a=n.filter(o=>o.job!=="GK"),s=!e.first_booster_opened&&p.length>0,d=[];for(let o=0;o<t;o++){const r=o===0&&s?p:o===0?a:n,u=Io(),f=Lo(r,u);f&&d.push(f)}s&&await A.from("users").update({first_booster_opened:!0}).eq("id",e.id);const{data:c}=await A.from("cards").insert(d.map(o=>({owner_id:e.id,player_id:o.id,card_type:"player"}))).select();return(c||[]).forEach((o,r)=>{A.rpc("record_transfer",{p_card_id:o.id,p_player_id:d[r].id,p_club_name:e.club_name||e.pseudo,p_manager_name:e.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{})}),d.map((o,r)=>({...c[r],player:o}))}async function Fi(e,t,i){const{error:n}=await A.from("users").update({credits:e.credits-i}).eq("id",e.id);if(n)throw n;const{data:p}=await A.from("gc_definitions").select("id,name,gc_type,color,effect,image_url").eq("is_active",!0),a=p!=null&&p.length?p:Object.keys(Ft).map(r=>({name:r,gc_type:"game_changer"})),s=Array.from({length:t},()=>{const r=a[Math.floor(Math.random()*a.length)];return{owner_id:e.id,card_type:"game_changer",gc_type:r.name,gc_definition_id:r.id||null}}),{data:d,error:c}=await A.from("cards").insert(s).select();return c&&console.error("[Booster GC] Erreur insert:",c.message,c),(d||[]).map(r=>{const u=p==null?void 0:p.find(f=>f.name===r.gc_type||f.id===r.gc_definition_id);return{...r,_gcDef:u||null}})}async function Di(e,t){const{error:i}=await A.from("users").update({credits:e.credits-t}).eq("id",e.id);if(i)throw i;const{data:n}=await A.from("cards").select("formation").eq("owner_id",e.id).eq("card_type","formation"),p=new Set((n||[]).map(r=>r.formation)),a=Ci(),s=a[Math.floor(Math.random()*a.length)],d=p.has(s),{data:c,error:o}=await A.from("cards").insert({owner_id:e.id,card_type:"formation",formation:s}).select();return o&&console.error("[Booster Formation] Erreur insert:",o.message,o),(c||[]).map(r=>({...r,isDuplicate:d}))}function qi(e,t,i,n=null){var R,Y;if(!e||e.length===0){const E=document.createElement("div");E.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:3000;gap:16px;color:#fff;padding:24px;text-align:center",E.innerHTML=`
      <div style="font-size:48px">😕</div>
      <div style="font-size:20px;font-weight:900">Aucune carte obtenue</div>
      <div style="font-size:13px;color:rgba(255,255,255,0.5)">Erreur lors du tirage (permissions DB ou colonne manquante)</div>
      <button style="margin-top:10px;padding:12px 28px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer" id="anim-close-err">Fermer</button>`,document.body.appendChild(E),(R=E.querySelector("#anim-close-err"))==null||R.addEventListener("click",()=>E.remove());return}e=[...e].sort((E,K)=>{const G=E.player?Oe(E.player):-1;return(K.player?Oe(K.player):-1)-G});const p=document.createElement("div");p.id="booster-anim-overlay",p.innerHTML=`
    <style>
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
  ...
  </div>`;const r=async()=>{await A.from("users").update({pending_boosters:s}).eq("id",t.user.id)};async function u(){var h;if(o>=c||!s.length){await A.from("users").update({pending_boosters:[],onboarding_done:!0}).eq("id",t.user.id),p&&await p(),n("Tous tes boosters sont ouverts ! Bon jeu 🎮","success",4e3),i("home");return}const y=s[0],{data:l}=await A.from("users").select("*").eq("id",t.user.id).single();l&&(t.profile=l);let x=[];try{if(y.type==="formation")x=await Di(t.profile,0);else if(y.type==="game_changer")x=await Fi(t.profile,y.count||3,0);else if(d&&((h=d.rates)!=null&&h.length)){const I={...d,cost:0,cardCount:y.count||d.cardCount||5};x=await Dt(t.profile,I),y.guaranteeGK&&!t.profile.first_booster_opened&&(x.some(_=>_.player&&_.player.job==="GK")||await qo(t.profile,x),await A.from("users").update({first_booster_opened:!0}).eq("id",t.profile.id))}else x=await Bi(t.profile,y.count||5,0)}catch(I){n(I.message||"Erreur ouverture booster","error");return}s.shift(),o++,await r();const v=y.type==="formation"?{name:"Booster Formation",img:"/manager-wars/icons/booster-formation.png"}:y.type==="game_changer"?{name:"Booster Game Changer",img:"/manager-wars/icons/booster-gamechanger.png"}:{name:`Booster Joueurs (${o}/${c})`,img:(d==null?void 0:d.img)||"/manager-wars/icons/booster-players.png"};qi(x,v,i,()=>{u()})}(g=document.getElementById("onboard-start"))==null||g.addEventListener("click",()=>u())}async function qo(e,t){try{const{data:i}=await A.from("players").select("id,job,firstname,surname_encoded,country_code,club_id,rarity,note_g,note_d,note_m,note_a,skin,hair,hair_length,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0).eq("job","GK");if(!(i!=null&&i.length))return;const n=i[Math.floor(Math.random()*i.length)],p=t.findIndex(s=>s.player);if(p===-1)return;const a=t[p];await A.from("cards").update({player_id:n.id}).eq("id",a.id),t[p]={...a,player_id:n.id,player:n}}catch(i){console.warn("[Onboarding] ensureGK échec",i)}}const nt={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},rt={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function Xe(e,t,i,n,p){var a;e.innerHTML=`<div class="match-screen" style="display:flex;align-items:center;justify-content:center;min-height:100vh">
    <div style="text-align:center;padding:40px;color:#fff">
      <div style="font-size:48px;margin-bottom:16px">${t}</div>
      <p style="margin-bottom:16px">${i}</p>
      <button class="btn btn-primary" id="msg-btn">${n}</button>
    </div>
  </div>`,(a=document.getElementById("msg-btn"))==null||a.addEventListener("click",p)}function Te(e){const t=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function qt(e,t){var n,p;const i=e.player;return{cardId:e.id,position:t||null,id:i.id,firstname:i.firstname,name:i.surname_encoded,country_code:i.country_code,club_id:i.club_id,job:i.job,job2:i.job2,note_g:Number(i.note_g)||0,note_d:Number(i.note_d)||0,note_m:Number(i.note_m)||0,note_a:Number(i.note_a)||0,rarity:i.rarity,skin:i.skin,hair:i.hair,hair_length:i.hair_length,clubName:((n=i.clubs)==null?void 0:n.encoded_name)||null,clubLogo:((p=i.clubs)==null?void 0:p.logo_url)||null,boost:0,used:!1,_line:null,_col:null}}function at(e){return e===1?[1]:e===2?[0,2]:e===3?[0,1,2]:e===4?[0,1,1,2]:e===5?[0,1,1,1,2]:[1]}function Ri(){const e=Math.random()*100;return e<10?3:e<30?2:1}function At(e,t){const i=nt[t]||nt["4-4-2"],n={GK:[],DEF:[],MIL:[],ATT:[]};if(e.length&&e.every(s=>s.position)){for(const s of["GK","DEF","MIL","ATT"]){const d=e.filter(o=>o.position&&o.position.replace(/\d+$/,"")===s).sort((o,r)=>parseInt(o.position.replace(/\D+/g,""),10)-parseInt(r.position.replace(/\D+/g,""),10)).map(o=>({...o,_line:s})),c=at(d.length);d.forEach((o,r)=>{o._col=c[r]}),n[s]=d}return n}const a=[...e];for(const s of["GK","DEF","MIL","ATT"]){const d=[];for(let o=0;o<i[s];o++){let r=a.findIndex(u=>u.job===s);if(r===-1&&(r=a.findIndex(u=>u.job2===s)),r===-1&&(r=0),a[r]){const u={...a[r],_line:s};d.push(u),a.splice(r,1)}}const c=at(d.length);d.forEach((o,r)=>{o._col=c[r]}),n[s]=d}return n}function Ye(e){document.querySelectorAll(".top-nav, .bottom-nav").forEach(t=>{t.style.setProperty("display","none","important"),t.dataset.matchHidden="1"}),e&&e.style.setProperty("padding-bottom","0","important")}function Re(e){document.querySelectorAll(".top-nav, .bottom-nav").forEach(t=>{t.style.removeProperty("display"),delete t.dataset.matchHidden}),e&&e.style.removeProperty("padding-bottom")}function Ni(e,t,i){const p=new Set,a=t.filter(r=>{const u=r.gc_type||r.id;return p.has(u)?!1:(p.add(u),!0)});let s=[];function d(r,u){const f=r._gcDef,g={purple:"linear-gradient(135deg,#3d0a7a,#7a28b8)",light_blue:"linear-gradient(135deg,#006080,#00bcd4)"},y={purple:"#9b59b6",light_blue:"#00bcd4"},l=g[f==null?void 0:f.color]||g.purple,x=y[f==null?void 0:f.color]||y.purple;return`<div class="gc-select-card" data-id="${r.id}"
    ...
    </div>`}const c=r=>{e.style.overflow="",e.style.height="",e.style.display="",e.style.flexDirection="",i(r)};function o(){var u,f,g;e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column";const r=s.length>0;e.innerHTML=`
    ...
    </div>`,e.querySelectorAll(".gc-select-card").forEach(y=>{y.addEventListener("click",()=>{const l=y.dataset.id,x=a.find(h=>h.id===l);if(!x)return;const v=s.findIndex(h=>h.gc_type===x.gc_type);v>-1?s.splice(v,1):s.length<3&&s.push(x),o()})}),(u=e.querySelector("#gc-launch"))==null||u.addEventListener("click",()=>{r&&c(s)}),(f=e.querySelector("#gc-no-gc"))==null||f.addEventListener("click",()=>c([])),(g=e.querySelector("#gc-reset"))==null||g.addEventListener("click",()=>{s.length&&(s=[],o())})}o()}async function Go(e,t,i){const{state:n,navigate:p}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:a}=await A.from("decks").select("id,name,is_active,formation").eq("owner_id",n.profile.id).order("created_at",{ascending:!1});if(!a||a.length===0){Xe(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>p("decks"));return}const s=a.map(r=>r.id),{data:d}=await A.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",s).order("slot_order");let c=0;function o(){var h,I,D,_,w;const r=a[c],u=(d||[]).filter(S=>S.deck_id===r.id),f=u.filter(S=>{var ee;return S.is_starter&&((ee=S.card)==null?void 0:ee.player)}).map(S=>qt(S.card,S.position)),g=u.find(S=>{var ee;return((ee=S.card)==null?void 0:ee.card_type)==="formation"}),y=r.formation||((h=g==null?void 0:g.card)==null?void 0:h.formation)||"4-4-2",l=f.length>=11?At(f,y):null,x=f.length>=11;Ye(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">
        ${a.map((S,ee)=>`<div style="width:7px;height:7px;border-radius:50%;background:${ee===c?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}
      </div>`:""}
    ...
    </div>`,function(){const ee=e.querySelector(".deck-preview-wrap svg");ee&&(ee.removeAttribute("width"),ee.removeAttribute("height"),ee.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",ee.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),(I=document.getElementById("prev-deck"))==null||I.addEventListener("click",()=>{c>0&&(c--,o())}),(D=document.getElementById("next-deck"))==null||D.addEventListener("click",()=>{c<a.length-1&&(c++,o())}),(_=document.getElementById("validate-deck"))==null||_.addEventListener("click",()=>{x&&t.navigate("match",{matchMode:i,deckId:r.id})}),(w=document.getElementById("cancel-deck-select"))==null||w.addEventListener("click",()=>{Re(e),p("home")});const v=document.getElementById("deck-swipe-zone");if(v){let S=0,ee=0;v.addEventListener("touchstart",re=>{S=re.touches[0].clientX,ee=re.touches[0].clientY},{passive:!0}),v.addEventListener("touchend",re=>{const te=re.changedTouches[0].clientX-S,$=re.changedTouches[0].clientY-ee;Math.abs(te)<40||Math.abs(te)<Math.abs($)||(te<0&&c<a.length-1?(c++,o()):te>0&&c>0&&(c--,o()))},{passive:!0})}}o()}function Pi(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";return e!=null&&e.clubLogo?e.clubLogo.startsWith("http")?e.clubLogo:`${t}/storage/v1/object/public/assets/clubs/${e.clubLogo}`:null}function Be(e,t=44,i=58){if(!e)return`<div style="width:${t}px;height:${i}px;border:1.5px dashed rgba(255,255,255,0.2);border-radius:5px"></div>`;const n=typeof e.portrait=="string"&&e.portrait.startsWith("http")?e.portrait:Te(e),p=Pi(e),a=e._line||e.job||"MIL",s=rt[a]||"#555",d={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",c=e.note!==void 0?Number(e.note)||0:(Number(me(e,a))||0)+(e.boost||0),o=Oi(e==null?void 0:e.country_code),r=Math.round(i*.19),u=Math.round(i*.25),f=i-Math.round(i*.19)-Math.round(i*.25),g=e!=null&&e.used?.28:1;return`<div style="width:${t}px;height:${i}px;border-radius:5px;border:2px solid ${d};background:${s};position:relative;overflow:hidden;flex-shrink:0;opacity:${g}">
    <div style="position:absolute;top:0;left:0;right:0;height:${r}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:center;z-index:2">
  ...
  </div>`}function et(e,t,i){if(!(e!=null&&e.length))return"";const n=e.slice(0,5);let p='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return n.forEach((a,s)=>{if(p+=Be(a,40,52),s<n.length-1){const d=Je(a,n[s+1]);p+=`<div style="width:7px;height:3px;background:${d==="#ff3333"||d==="#cc2222"?"rgba(255,255,255,0.12)":d};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(p+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),p+="</div>",p}function Gt(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function Oi(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function ht(e,t,i,n,p=310,a=310,s=[]){var I;const d=vt[t]||{},c=$i(t)||Ot[t]||[],o={},r=["ATT","MIL","DEF","GK"];for(const D of r)(e[D]||[]).forEach((w,S)=>{o[`${D}${S+1}`]=w});function u(D){const _=d[D];return _?{x:_.x*p,y:_.y*a}:null}let f="";for(const[D,_]of c){const w=u(D),S=u(_);if(!w||!S)continue;const ee=o[D],re=o[_],te=Je(ee,re);te==="#00ff88"||te==="#FFD700"?(f+=`<line x1="${w.x.toFixed(1)}" y1="${w.y.toFixed(1)}" x2="${S.x.toFixed(1)}" y2="${S.y.toFixed(1)}"
  ...
  </div>`}async function Hi(e,t,i,n){var _;const{state:p,navigate:a,toast:s}=t;Ye(e);const d=p.params||{};if(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!d.deckId)return Go(e,t,i);const c=d.deckId;let o,r,u,f;try{const w=await Promise.all([A.from("decks").select("formation,name").eq("id",c).single(),A.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation,
            player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
              clubs(encoded_name,logo_url)))`).eq("deck_id",c).order("slot_order")]);o=w[0].data,u=w[0].error,r=w[1].data,f=w[1].error}catch(w){console.error("[Match] Exception chargement deck:",w),Xe(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>a("home"));return}if(u||f){console.error("[Match] Erreur Supabase:",u||f),Xe(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>a("home"));return}const g=(r||[]).filter(w=>{var S;return w.is_starter&&((S=w.card)==null?void 0:S.player)}).map(w=>qt(w.card,w.position)),y=(r||[]).filter(w=>{var S;return!w.is_starter&&((S=w.card)==null?void 0:S.player)}).map(w=>qt(w.card,w.position));if(g.length<11){Xe(e,"⚠️",`Deck incomplet (${g.length}/11).`,"Compléter",()=>a("decks"));return}const l=(r||[]).find(w=>{var S;return((S=w.card)==null?void 0:S.card_type)==="formation"}),x=(o==null?void 0:o.formation)||((_=l==null?void 0:l.card)==null?void 0:_.formation)||"4-4-2",{data:v,error:h}=await A.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",p.profile.id).eq("card_type","game_changer"),{data:I}=await A.from("gc_definitions").select("*").eq("is_active",!0),D=(v||[]).map(w=>({...w,_gcDef:(I==null?void 0:I.find(S=>S.name===w.gc_type||S.id===w.gc_definition_id))||null}));n({deckId:c,formation:x,starters:g,subsRaw:y,gcCardsEnriched:D,gcDefs:I||[]})}async function Ro(e,t){const{state:i}=t,p=(i.params||{}).matchMode||"vs_ai_easy",a=p.replace("vs_ai_",""),s=p;await Hi(e,t,p,async({deckId:d,formation:c,starters:o,subsRaw:r,gcCardsEnriched:u,gcDefs:f})=>{try{const g=At(o,c),y=await No(c,a),l=async x=>{try{const{data:v,error:h}=await A.from("matches").insert({home_id:i.profile.id,away_id:null,mode:s,home_deck_id:d,status:"in_progress"}).select().single();if(h){console.error("[MatchIA] Erreur création match:",h),Xe(e,"⚠️","Impossible de créer le match ("+h.message+").","Retour",()=>t.navigate("home"));return}const I={gcDefs:f||[],matchId:v==null?void 0:v.id,mode:s,difficulty:a,formation:c,homeTeam:g,aiTeam:y,homeSubs:r,subsUsed:0,maxSubs:Math.min(r.length,3),homeScore:0,aiScore:0,gcCards:x,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Oo(e,I,t)}catch(v){console.error("[MatchIA] Exception launchMatch:",v),Xe(e,"⚠️","Erreur au lancement du match : "+v.message,"Retour",()=>t.navigate("home"))}};if(!u.length){l([]);return}Ni(e,u,l)}catch(g){console.error("[MatchIA] Exception setup:",g),Xe(e,"⚠️","Erreur de préparation du match : "+g.message,"Retour",()=>t.navigate("home"))}})}async function No(e,t){var s,d;const{data:i}=await A.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(60);if(!i||i.length<11)return Po(e);const n=nt[e]||nt["4-4-2"],p={GK:[],DEF:[],MIL:[],ATT:[]},a=[...i];for(const c of["GK","DEF","MIL","ATT"]){const o=a.filter(y=>y.job===c),r=a.filter(y=>y.job!==c),u=[...o,...r],f=[];for(let y=0;y<n[c];y++){const l=u[y]||a[y];l&&f.push({cardId:"ai-"+l.id+"-"+y,id:l.id,firstname:l.firstname,name:l.surname_encoded,country_code:l.country_code,club_id:l.club_id,job:l.job,job2:l.job2,note_g:Number(l.note_g)||0,note_d:Number(l.note_d)||0,note_m:Number(l.note_m)||0,note_a:Number(l.note_a)||0,rarity:l.rarity,skin:l.skin,hair:l.hair,hair_length:l.hair_length,clubName:((s=l.clubs)==null?void 0:s.encoded_name)||null,clubLogo:((d=l.clubs)==null?void 0:d.logo_url)||null,boost:0,used:!1,_line:c})}const g=at(f.length);f.forEach((y,l)=>{y._col=g[l]}),p[c]=f}return p}function Po(e){const t=nt[e]||nt["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},n=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let p=0;for(const a of["GK","DEF","MIL","ATT"]){const s=[];for(let c=0;c<t[a];c++){const o=3+Math.floor(Math.random()*5);s.push({cardId:"fake-"+p,id:"fake-"+p,firstname:"IA",name:n[p%n.length],country_code:"XX",club_id:null,job:a,job2:null,note_g:a==="GK"?o:2,note_d:a==="DEF"?o:2,note_m:a==="MIL"?o:2,note_a:a==="ATT"?o:2,rarity:"normal",boost:0,used:!1,_line:a}),p++}const d=at(s.length);s.forEach((c,o)=>{c._col=d[o]}),i[a]=s}return i}function Oo(e,t,i){e.innerHTML=`
  ...
  </div>`,setTimeout(()=>Ho(e,t,i),5e3)}function Ho(e,t,i){const n=t.homeTeam.MIL||[],p=t.aiTeam.MIL||[];function a(l){return l.reduce((x,v)=>x+me(v,"MIL"),0)}function s(l){let x=0;for(let v=0;v<l.length-1;v++){const h=Je(l[v],l[v+1]);h==="#00ff88"?x+=2:h==="#FFD700"&&(x+=1)}return x}const d=a(n)+s(n),c=a(p)+s(p),o=d>=c;function r(l,x,v,h){return`<div id="duel-row-${h}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0), opacity .5s ease;transform-origin:center">
  ...
  `,document.body.appendChild(n);let r=!1;const u=()=>{r||(r=!0,n.remove(),i())};n.addEventListener("click",u),setTimeout(u,2e3)}function ct(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function Ct(e,t,i,n=null,p=null){var g,y;if(t.phase!=="attack"){ct("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){ct(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const a=Object.entries(t.homeTeam).flatMap(([l,x])=>(x||[]).filter(v=>v.used).map(v=>({...v,_line:v._line||l}))),s=t.homeSubs.filter(l=>!t.usedSubIds.includes(l.cardId));if(!a.length){ct("Aucun joueur utilisé à remplacer");return}if(!s.length){ct("Aucun remplaçant disponible");return}let d=Math.max(0,a.findIndex(l=>l.cardId===p));const c=((g=a[d])==null?void 0:g._line)||((y=a[d])==null?void 0:y.job);let o=n?Math.max(0,s.findIndex(l=>l.cardId===n)):Math.max(0,s.findIndex(l=>l.job===c)),r=!1;const u=document.createElement("div");u.id="sub-overlay",u.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function f(){var _,w,S,ee,re,te;const l=a[d],x=s[o],v=Math.min(130,Math.round((window.innerWidth-90)/2)),h=Math.round(v*1.35),I=$=>`background:rgba(255,255,255,0.12);border:none;color:${$?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${$?"default":"pointer"};flex-shrink:0`;u.innerHTML=`
    ...
    </div>`,document.body.appendChild(g),(y=g.querySelector("#gc-back"))==null||y.addEventListener("click",()=>g.remove()),(l=g.querySelector("#gc-use"))==null||l.addEventListener("click",()=>{g.remove(),Zo(e,t,i,n,p)})}function $t(e,t,i,n,p,a){const s=document.createElement("div");s.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let d=[];function c(){var o,r;s.innerHTML=`
    ...
    </div>`,(o=s.querySelector("#gc-picker-close"))==null||o.addEventListener("click",()=>s.remove()),s.querySelectorAll(".gc-pick-item").forEach(u=>{u.addEventListener("click",()=>{const f=u.dataset.cid,g=e.find(l=>l.cardId===f);if(!g)return;const y=d.findIndex(l=>l.cardId===f);y>-1?d.splice(y,1):d.length<t&&d.push(g),c()})}),(r=s.querySelector("#gc-picker-confirm"))==null||r.addEventListener("click",()=>{s.remove(),a(d)})}c(),document.body.appendChild(s)}const Qo={BOOST_STAT:({value:e=1,count:t=1,roles:i=[]},n,p,a)=>{const s=Object.entries(n.homeTeam).filter(([d])=>!i.length||i.includes(d)).flatMap(([d,c])=>c.filter(o=>!o.used).map(o=>({...o,_line:d})));return s.length?($t(s,t,`Choisir ${t} joueur(s) à booster (+${e})`,p,n,d=>{d.forEach(c=>{const o=(n.homeTeam[c._line]||[]).find(r=>r.cardId===c.cardId);o&&(o.boost=(o.boost||0)+e,n.log.push({text:`⚡ +${e} sur ${o.name}`,type:"info"}))}),_e(p,n,a)}),!0):(n.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),_e(p,n,a),!0)},DEBUFF_STAT:({value:e=1,count:t=1,roles:i=[],target:n="ai"},p,a,s)=>{const d=n==="home"?p.homeTeam:p.aiTeam,c=n==="ai"?"adverse":"allié",o=Object.entries(d).filter(([r])=>!i.length||i.includes(r)).flatMap(([r,u])=>u.filter(f=>!f.used).map(f=>({...f,_line:r})));return o.length?($t(o,t,`Choisir ${t} joueur(s) ${c}(s) à débuffer (-${e})`,a,p,r=>{r.forEach(u=>{const g=((n==="home"?p.homeTeam:p.aiTeam)[u._line]||[]).find(y=>y.cardId===u.cardId);g&&(g.boost=(g.boost||0)-e,p.log.push({text:`🎯 -${e} sur ${g.name}${n==="ai"?" (IA)":""}`,type:"info"}))}),_e(a,p,s)}),!0):(p.log.push({text:`🎯 Aucun joueur ${c} disponible`,type:"info"}),_e(a,p,s),!0)},GRAY_PLAYER:({count:e=1,roles:t=[],target:i="ai"},n,p,a)=>{const s=i==="home"?n.homeTeam:n.aiTeam,d=i==="ai"?"adverse":"allié",c=Object.entries(s).filter(([o])=>!t.length||t.includes(o)).flatMap(([o,r])=>r.filter(u=>!u.used).map(u=>({...u,_line:o})));return c.length?($t(c,e,`Choisir ${e} joueur(s) ${d}(s) à exclure`,p,n,o=>{o.forEach(r=>{const f=((i==="home"?n.homeTeam:n.aiTeam)[r._line]||[]).find(g=>g.cardId===r.cardId);f&&(f.used=!0,n.log.push({text:`❌ ${f.name}${i==="ai"?" (IA)":""} exclu !`,type:"info"}))}),_e(p,n,a)}),!0):(n.log.push({text:`❌ Aucun joueur ${d} à exclure`,type:"info"}),_e(p,n,a),!0)},REVIVE_PLAYER:({count:e=1,roles:t=[]},i,n,p)=>{const a=Object.entries(i.homeTeam).filter(([s])=>!t.length||t.includes(s)).flatMap(([s,d])=>d.filter(c=>c.used).map(c=>({...c,_line:s})));return a.length?($t(a,e,`Choisir ${e} joueur(s) à ressusciter`,n,i,s=>{s.forEach(d=>{const c=(i.homeTeam[d._line]||[]).find(o=>o.cardId===d.cardId);c&&(c.used=!1,i.log.push({text:`💫 ${c.name} ressuscité !`,type:"info"}))}),_e(n,i,p)}),!0):(i.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),_e(n,i,p),!0)},REMOVE_GOAL:({},e)=>(e.aiScore>0?(e.aiScore--,e.log.push({text:"🚫 Dernier but IA annulé !",type:"info"})):e.log.push({text:"🚫 Aucun but à annuler",type:"info"}),!1),ADD_GOAL_DRAW:({},e)=>(e.homeScore===e.aiScore?(e.homeScore++,e.log.push({text:"🎯 But bonus (match nul) !",type:"info"})):e.log.push({text:"🎯 But bonus : non applicable (pas de match nul)",type:"info"}),!1),ADD_SUB:({value:e=1},t)=>(t.maxSubs=(t.maxSubs||3)+e,t.log.push({text:`🔄 +${e} remplacement(s) débloqué(s)`,type:"info"}),!1),CUSTOM:()=>!1};function Zo(e,t,i,n,p){n.usedGc.push(e);const a=n.gcDefs||[],s=a.find(c=>c.name===t)||a.find(c=>{var o;return((o=c.name)==null?void 0:o.toLowerCase().trim())===(t==null?void 0:t.toLowerCase().trim())});let d=!1;if(s!=null&&s.effect_type&&s.effect_type!=="CUSTOM"){const c=Qo[s.effect_type];c?c(s.effect_params||{},n,i,p)||(d=!0):(p.toast(`Effet "${s.effect_type}" non implémenté`,"error"),d=!0)}else{switch(t){case"Double attaque":n.modifiers.home.doubleAttack=!0,n.log.push({text:"⚡ Double attaque activée !",type:"info"});break;case"Bouclier":n.modifiers.home.shield=!0,n.log.push({text:"🛡️ Bouclier activé !",type:"info"});break;case"Ressusciter":{const c=Object.entries(n.homeTeam).flatMap(([o,r])=>(r||[]).filter(u=>u.used).map(u=>({...u,_line:o})));c.length?(c[0].used=!1,n.log.push({text:`💫 ${c[0].name} ressuscité !`,type:"info"})):n.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"});break}case"Vol de note":n.modifiers.ai.stolenNote=(n.modifiers.ai.stolenNote||0)+1,n.log.push({text:"🎯 -1 à la prochaine attaque IA",type:"info"});break;case"Gel":{const c=[...n.aiTeam.ATT||[],...n.aiTeam.MIL||[]].filter(o=>!o.used);if(c.length){const o=c.sort((r,u)=>me(u,"ATT")-me(r,"ATT"))[0];o.used=!0,n.log.push({text:`❄️ ${o.name} (IA) gelé !`,type:"info"})}break}case"Remplacement+":n.maxSubs++,n.log.push({text:"🔄 +1 remplacement débloqué",type:"info"});break}d=!0}A.from("cards").delete().eq("id",e).then(()=>{}),d&&_e(i,n,p)}function en(e,t,i){const n=Object.values(t.homeTeam).flat().filter(p=>!p.used);if(!n.length){i.toast("Aucun joueur actif à booster","error");return}i.openModal("⚡ Utiliser le Boost",`<div style="margin-bottom:12px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border-radius:10px;padding:12px;text-align:center;color:#000">
    ...
    <div style="display:flex;flex-direction:column;gap:6px">
      ${n.map(p=>`
        <div class="player-boost-opt" data-card-id="${p.cardId}"
          ...
          <div style="width:32px;height:32px;background:${rt[p.job]||"#888"};border-radius:6px;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:900;font-size:13px">${me(p,p._line||p.job)}</div>
          <div style="flex:1"><b>${p.firstname} ${p.name}</b><div style="font-size:11px;color:#888">${p._line||p.job}</div></div>
          <div style="color:#87CEEB;font-weight:700">+${t.boostCard.value}</div>
        </div>`).join("")}
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(p=>{p.addEventListener("click",()=>{const a=p.dataset.cardId;for(const s of["GK","DEF","MIL","ATT"]){const d=(t.homeTeam[s]||[]).find(c=>c.cardId===a);if(d){d.boost=(d.boost||0)+t.boostCard.value,t.log.push({text:`⚡ Boost +${t.boostCard.value} appliqué à ${d.name}`,type:"info"});break}}t.boostUsed=!0,i.closeModal(),_e(e,t,i)})})}function Tt(e,t,i,n,p){const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:900;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;overflow:hidden;cursor:pointer";const s=Array.from({length:10},(o,r)=>`
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(a);let d=!1;const c=()=>{d||(d=!0,a.remove(),setTimeout(()=>p(),50))};a.addEventListener("click",c),setTimeout(c,3500)}async function gt(e,t,i){var r,u;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:n}=i,p=t.homeScore>t.aiScore,a=t.homeScore===t.aiScore,s=p?"victoire":a?"nul":"defaite",d=lo(t.mode,s);t.matchId&&await A.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:p?n.profile.id:null,home_credits_reward:d,played_at:new Date().toISOString()}).eq("id",t.matchId);const c={credits:(n.profile.credits||0)+d,matches_played:(n.profile.matches_played||0)+1};p?c.wins=(n.profile.wins||0)+1:a?c.draws=(n.profile.draws||0)+1:c.losses=(n.profile.losses||0)+1,await A.from("users").update(c).eq("id",n.profile.id),await i.refreshProfile();const o=document.createElement("div");o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",o.innerHTML=`
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      <div style="font-size:72px;margin-bottom:12px">${p?"🏆":a?"🤝":"😔"}</div>
      <h2 style="font-size:28px;font-weight:900;margin-bottom:8px">${p?"Victoire !":a?"Match nul":"Défaite"}</h2>
    ...
    </div>`,`<button class="btn btn-primary" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}async function on(e,t){await Hi(e,t,"random",({deckId:i,formation:n,starters:p,subsRaw:a,gcCardsEnriched:s,gcDefs:d})=>{const c=o=>nn(e,t,i,n,p,a,o,d||[]);if(!s.length){c([]);return}Ni(e,s,c)})}async function nn(e,t,i,n,p,a,s=[],d=[]){var h;const{state:c,navigate:o,toast:r}=t;let u=!1,f=null;Ye(e),e.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const g=async(I=!0)=>{u=!0,f&&(A.removeChannel(f),f=null),I&&await A.rpc("cancel_matchmaking",{p_user_id:c.profile.id}).catch(()=>{})};(h=document.getElementById("mm-cancel"))==null||h.addEventListener("click",async()=>{try{await g(!0)}catch{}Re(e),o("home")});const y=async(I,D,_)=>{if(u)return;u=!0,f&&(A.removeChannel(f),f=null);const w=document.getElementById("mm-status");w&&(w.textContent="Adversaire trouvé !"),await new Promise(S=>setTimeout(S,600)),e.isConnected&&rn(e,t,I,_,s,d)},{data:l,error:x}=await A.rpc("try_matchmake",{p_user_id:c.profile.id,p_deck_id:i});if(x||!(l!=null&&l.success)){r("Erreur de matchmaking","error"),Re(e),o("home");return}if(l.matched){y(l.match_id,l.opponent_id,!1);return}const v=document.getElementById("mm-status");v&&(v.textContent="En attente d'un autre joueur..."),f=A.channel("matchmaking-"+c.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${c.profile.id}`},I=>{const D=I.new;D.status==="matched"&&D.match_id&&y(D.match_id,D.matched_with,!0)}).subscribe()}async function rn(e,t,i,n,p=[],a=[]){const{state:s,navigate:d,toast:c}=t,o=n?"p1":"p2",r=n?"p2":"p1",u=(p||[]).map(m=>m.id),f=(p||[]).map(m=>({id:m.id,gc_type:m.gc_type,_gcDef:m._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:g}=await A.from("matches").select("*").eq("id",i).single();if(!g){c("Match introuvable","error"),d("home");return}async function y(){const[{data:m},{data:b},{data:L},{data:T}]=await Promise.all([A.rpc("get_deck_for_match",{p_deck_id:g.home_deck_id}),A.rpc("get_deck_for_match",{p_deck_id:g.away_deck_id}),A.from("users").select("id,pseudo,club_name").eq("id",g.home_id).single(),A.from("users").select("id,pseudo,club_name").eq("id",g.away_id).single()]),k=F=>({cardId:F.card_id,position:F.position,id:F.id,firstname:F.firstname,name:F.surname_encoded,country_code:F.country_code,club_id:F.club_id,job:F.job,job2:F.job2,note_g:Number(F.note_g)||0,note_d:Number(F.note_d)||0,note_m:Number(F.note_m)||0,note_a:Number(F.note_a)||0,rarity:F.rarity,skin:F.skin,hair:F.hair,hair_length:F.hair_length,clubName:F.club_encoded_name||null,clubLogo:F.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}),z=((m==null?void 0:m.starters)||[]).map(F=>k(F)),B=((b==null?void 0:b.starters)||[]).map(F=>k(F)),q=(m==null?void 0:m.formation)||"4-4-2",H=(b==null?void 0:b.formation)||"4-4-2";return{p1Team:At(z,q),p2Team:At(B,H),p1Subs:((m==null?void 0:m.subs)||[]).map(F=>k(F)),p2Subs:((b==null?void 0:b.subs)||[]).map(F=>k(F)),p1Formation:q,p2Formation:H,p1Name:(L==null?void 0:L.club_name)||(L==null?void 0:L.pseudo)||"Joueur 1",p2Name:(T==null?void 0:T.club_name)||(T==null?void 0:T.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:n?u:[],gc_p2:n?[]:u,gcCardsFull_p1:n?f:[],gcCardsFull_p2:n?[]:f,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:a||[],lastActionAt:new Date().toISOString()}}let l=g.game_state&&Object.keys(g.game_state).length?g.game_state:null;if(!l)if(n){l=await y();const{data:m}=await A.from("matches").select("game_state").eq("id",i).single();!(m!=null&&m.game_state)||!Object.keys(m.game_state).length?await A.from("matches").update({game_state:l,turn_user_id:g.home_id}).eq("id",i):l=m.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let m=0;m<30&&!l;m++){await new Promise(L=>setTimeout(L,400));const{data:b}=await A.from("matches").select("game_state").eq("id",i).single();b!=null&&b.game_state&&Object.keys(b.game_state).length&&(l=b.game_state)}if(!l){c("Erreur de synchronisation","error"),d("home");return}l.gc_p2=u,l.gcCardsFull_p2=f,await A.from("matches").update({game_state:l}).eq("id",i)}let x=!1,v=null,h=!1;const I=new Set,D=new Set;function _(m){var F,U;try{A.removeChannel(w)}catch{}const b=l[o+"Score"]||0,L=l[r+"Score"]||0;let T,k;m.winner_id?(T=m.winner_id===s.profile.id,k=!1):m.forfeit?(T=b>L,k=!1):(k=b===L,T=b>L),(F=document.getElementById("pvp-end-overlay"))==null||F.remove();const z=document.createElement("div");z.id="pvp-end-overlay",z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const B=k?"🤝":T?"🏆":"😞",q=k?"MATCH NUL":T?"VICTOIRE !":"DÉFAITE",H=k?"#fff":T?"#FFD700":"#ff6b6b";z.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(z),(U=z.querySelector("#pvp-end-home"))==null||U.addEventListener("click",()=>{z.remove(),Re(e),d("home")})}const w=A.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},m=>{const b=m.new;try{if(b.status==="finished"||b.forfeit){if(x)return;x=!0,v&&(clearInterval(v),v=null),b.game_state&&(l=b.game_state),_(b);return}if(b.game_state){const L=l;l=b.game_state;const T=l._lastGC;if(T&&T.seq&&!D.has(T.seq)&&(D.add(T.seq),T.by!==o)){de(T.type,T.by,()=>E());return}const k=L[o+"Score"]||0,z=L[r+"Score"]||0,B=l[o+"Score"]||0,q=l[r+"Score"]||0,H=B>k,F=q>z;if((H||F)&&!I.has(l.round)){I.add(l.round);const U=[...l.log||[]].reverse().find(Z=>Z.isGoal),V=((U==null?void 0:U.homePlayers)||[]).map(Z=>({name:Z.name,note:Z.note,portrait:Z.portrait,job:Z.job}));se(V,B,q,H,()=>E());return}E()}}catch(L){console.error("[PvP] crash:",L)}}).subscribe();async function S(m){Object.assign(l,m),l.lastActionAt=new Date().toISOString();const{error:b}=await A.from("matches").update({game_state:l}).eq("id",i);b&&c("Erreur de synchronisation","error");try{E()}catch(L){console.error("[PvP] renderPvpScreen crash:",L)}}async function ee(){if(x)return;x=!0,v&&(clearInterval(v),v=null);const m=n?g.away_id:g.home_id,b=n?"p2":"p1",L=n?"p1":"p2",T={...l,[b+"Score"]:3,[L+"Score"]:0,phase:"finished"};try{await A.from("matches").update({status:"finished",forfeit:!0,winner_id:m,home_score:T.p1Score||0,away_score:T.p2Score||0,game_state:T}).eq("id",i)}catch{}try{A.removeChannel(w)}catch{}setTimeout(()=>{Re(e),d("home")},800)}const re={BOOST_STAT:({value:m=1,count:b=1,roles:L=[]},T,k)=>{const z=T[o+"Team"],B=Object.entries(z).filter(([q])=>!L.length||L.includes(q)).flatMap(([q,H])=>H.filter(F=>!F.used).map(F=>({...F,_line:q})));if(!B.length){T.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),k(T);return}te(B,b,`Choisir ${b} joueur(s) à booster (+${m})`,q=>{q.forEach(H=>{const F=(z[H._line]||[]).find(U=>U.cardId===H.cardId);F&&(F.boost=(F.boost||0)+m,T.log.push({text:`⚡ +${m} sur ${F.name}`,type:"info"}))}),T[o+"Team"]=z,k(T)})},DEBUFF_STAT:({value:m=1,count:b=1,roles:L=[],target:T="ai"},k,z)=>{const B=T==="home"?o:r,q=k[B+"Team"],H=T==="home"?"allié":"adverse",F=Object.entries(q).filter(([U])=>!L.length||L.includes(U)).flatMap(([U,V])=>V.filter(Z=>!Z.used).map(Z=>({...Z,_line:U})));if(!F.length){k.log.push({text:`🎯 Aucun joueur ${H}`,type:"info"}),z(k);return}te(F,b,`Choisir ${b} joueur(s) ${H}(s) (-${m})`,U=>{U.forEach(V=>{const Z=(q[V._line]||[]).find(ue=>ue.cardId===V.cardId);Z&&(Z.boost=(Z.boost||0)-m,k.log.push({text:`🎯 -${m} sur ${Z.name}`,type:"info"}))}),k[B+"Team"]=q,z(k)})},GRAY_PLAYER:({count:m=1,roles:b=[],target:L="ai"},T,k)=>{const z=L==="home"?o:r,B=T[z+"Team"],q=L==="home"?"allié":"adverse",H=Object.entries(B).filter(([F])=>!b.length||b.includes(F)).flatMap(([F,U])=>U.filter(V=>!V.used).map(V=>({...V,_line:F})));if(!H.length){T.log.push({text:`❌ Aucun joueur ${q}`,type:"info"}),k(T);return}te(H,m,`Choisir ${m} joueur(s) ${q}(s) à exclure`,F=>{F.forEach(U=>{const V=(B[U._line]||[]).find(Z=>Z.cardId===U.cardId);V&&(V.used=!0,T.log.push({text:`❌ ${V.name} exclu !`,type:"info"}))}),T[z+"Team"]=B,k(T)})},REVIVE_PLAYER:({count:m=1,roles:b=[]},L,T)=>{const k=L[o+"Team"],z=Object.entries(k).filter(([B])=>!b.length||b.includes(B)).flatMap(([B,q])=>q.filter(H=>H.used).map(H=>({...H,_line:B})));if(!z.length){L.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),T(L);return}te(z,m,`Choisir ${m} joueur(s) à ressusciter`,B=>{B.forEach(q=>{const H=(k[q._line]||[]).find(F=>F.cardId===q.cardId);H&&(H.used=!1,L.log.push({text:`💫 ${H.name} ressuscité !`,type:"info"}))}),L[o+"Team"]=k,T(L)})},REMOVE_GOAL:({},m,b)=>{const L=r+"Score";m[L]>0?(m[L]--,m.log.push({text:"🚫 Dernier but annulé !",type:"info"})):m.log.push({text:"🚫 Aucun but à annuler",type:"info"}),b(m)},ADD_GOAL_DRAW:({},m,b)=>{m[o+"Score"]===m[r+"Score"]?(m[o+"Score"]++,m.log.push({text:"🎯 But bonus !",type:"info"})):m.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),b(m)},ADD_SUB:({value:m=1},b,L)=>{b.maxSubs=(b.maxSubs||3)+m,b.log.push({text:`🔄 +${m} remplacement(s)`,type:"info"}),L(b)},CUSTOM:({},m,b)=>b(m)};function te(m,b,L,T){const k=document.createElement("div");k.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let z=[];function B(){var H,F;const q=m.map(U=>{const V={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[U._line]||"#555",Z=me(U,U._line)+(U.boost||0),we=z.find(Q=>Q.cardId===U.cardId)?"#FFD700":"rgba(255,255,255,0.25)",X=U.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${U.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${we};background:${V};overflow:hidden;cursor:pointer;${X}">
          <div style="background:rgba(255,255,255,0.9);text-align:center;padding:2px;font-size:7px;font-weight:900;color:#111;overflow:hidden;white-space:nowrap;text-overflow:ellipsis">${U.name||"?"}</div>
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      </div>`,document.body.appendChild(L),(T=L.querySelector("#bp-close"))==null||T.addEventListener("click",()=>L.remove()),L.querySelectorAll(".bp-item").forEach(k=>{k.addEventListener("click",async()=>{const z=k.dataset.cid,B=b.find(H=>H.cardId===z);if(!B)return;const q=(m[B._line]||[]).find(H=>H.cardId===z);q&&(q.boost=(q.boost||0)+l.boostValue),L.remove(),await S({[o+"Team"]:m,boostUsed:!0})})})}function C(m=null){var ue,we;if(!(l.phase===o+"-attack")){c("Remplacement uniquement avant votre attaque","warning");return}const L=l[o+"Team"],T=l["usedSubIds_"+o]||[],k=l.maxSubs||3;if(T.length>=k){c(`Maximum ${k} remplacements atteint`,"warning");return}const z=Object.entries(L).flatMap(([X,Q])=>(Q||[]).filter(ne=>ne.used).map(ne=>({...ne,_line:X}))),B=(l[o+"Subs"]||[]).filter(X=>!T.includes(X.cardId));if(!z.length){c("Aucun joueur utilisé à remplacer","warning");return}if(!B.length){c("Aucun remplaçant disponible","warning");return}let q=Math.max(0,z.findIndex(X=>X.cardId===m));const H=((ue=z[q])==null?void 0:ue._line)||((we=z[q])==null?void 0:we.job);let F=Math.max(0,B.findIndex(X=>X.job===H)),U=!1;const V=document.createElement("div");V.id="pvp-sub-overlay",V.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function Z(){var he,Ae,Me,He,De,qe;const X=z[q],Q=B[F],ne=Math.min(130,Math.round((window.innerWidth-90)/2)),ce=Math.round(ne*1.35),ye=ze=>`background:rgba(255,255,255,0.12);border:none;color:${ze?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${ze?"default":"pointer"};flex-shrink:0`;V.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
        <div style="flex:1;font-size:15px;font-weight:900;color:#fff">🔄 Remplacement (${T.length}/${k})</div>
    ...
    </div>`,(k=document.getElementById("pvp-home"))==null||k.addEventListener("click",()=>{try{A.removeChannel(w)}catch{}Re(e),d("home")})}E()}const an={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"};async function sn(e,t){const{state:i,toast:n}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement du marché...</div>',await Qt(e,t)}async function Qt(e,t){const{state:i,toast:n}=t,{data:p}=await A.from("market_listings").select(`id, price, status, listed_at, seller_id,
      seller:users!seller_id(pseudo),
      card:cards(id, card_type,
        player:players(id, firstname, surname_encoded, country_code, job, job2,
          note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length, clubs(encoded_name, logo_url)))`).eq("status","active").order("listed_at",{ascending:!1}).limit(60),{data:a}=await A.from("market_listings").select(`id, price, status, listed_at, sold_at, seller_id, buyer_id,
      buyer:users!buyer_id(pseudo),
      card:cards(id, card_type,
        player:players(id, firstname, surname_encoded, country_code, job, job2,
          note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length, clubs(encoded_name, logo_url)))`).eq("seller_id",i.profile.id).in("status",["active","sold"]).order("listed_at",{ascending:!1}).limit(100),s=(p||[]).filter(o=>o.seller_id!==i.profile.id),d=a||[];d.filter(o=>o.status==="active").length,e.innerHTML=`
  <div class="page">
    <div class="page-header">
  ...
  `;function c(o){const r=document.getElementById("mkt-content"),u=o==="buy"?s:d;if(u.length===0){r.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
    ...
    </div>`,r.querySelectorAll("[data-buy]").forEach(g=>{g.addEventListener("click",()=>dn(g.dataset.buy,u,e,t))}),r.querySelectorAll("[data-cancel]").forEach(g=>{g.addEventListener("click",()=>cn(g.dataset.cancel,e,t))})}c("buy"),e.querySelectorAll(".mkt-tab").forEach(o=>{o.addEventListener("click",()=>{e.querySelectorAll(".mkt-tab").forEach(r=>{const u=r===o;r.style.background=u?"var(--green)":"#fff",r.style.color=u?"#fff":"var(--gray-600)",r.style.borderColor=u?"var(--green)":"var(--gray-200)"}),c(o.dataset.tab)})})}async function dn(e,t,i,n){var g;const{state:p,toast:a,refreshProfile:s}=n,d=t.find(y=>y.id===e);if(!d)return;const c=d.price,o=p.profile.credits||0,r=(g=d.card)==null?void 0:g.player;if(o<c){a("Crédits insuffisants","error");return}if(!await ln(r,c))return;const f=document.querySelector(`[data-buy="${e}"]`);f&&(f.disabled=!0,f.textContent="⏳");try{const{data:y,error:l}=await A.rpc("buy_market_card",{p_listing_id:e,p_buyer_id:p.profile.id});if(l)throw new Error(l.message);if(!(y!=null&&y.success))throw new Error((y==null?void 0:y.error)||"Achat impossible");p.profile.credits=o-c;const x=document.getElementById("nav-credits");x&&(x.textContent=`💰 ${(o-c).toLocaleString("fr")}`),a(`✅ ${r==null?void 0:r.surname_encoded} ajouté à ta collection !`,"success"),Qt(i,n)}catch(y){a("❌ "+y.message,"error"),f&&(f.disabled=!1,f.textContent="Acheter")}}function ln(e,t){return new Promise(i=>{const n=document.createElement("div");n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",n.innerHTML=`
      ...
      </div>`,document.body.appendChild(n);const p=a=>{n.remove(),i(a)};n.querySelector("#buy-ok").addEventListener("click",()=>p(!0)),n.querySelector("#buy-cancel").addEventListener("click",()=>p(!1)),n.addEventListener("click",a=>{a.target===n&&p(!1)})})}async function cn(e,t,i){const{toast:n}=i,{data:p}=await A.from("market_listings").select("card_id").eq("id",e).single();await A.from("market_listings").update({status:"cancelled"}).eq("id",e),p&&await A.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",p.card_id),n("Annonce retirée","success"),Qt(t,i)}function pn(e){return{GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[e]||"#888"}async function un(e,{state:t,navigate:i,toast:n}){e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:p}=await A.from("users").select("id,pseudo,club_name,trophies_top1,trophies_top2,trophies_top3,wins,level").order("trophies_top1",{ascending:!1}).limit(100);e.innerHTML=`
  <div class="page">
    <div class="page-header">
  ...
  `,document.querySelectorAll(".bottom-nav a").forEach(n=>{n.addEventListener("click",p=>{p.preventDefault(),kt(n.dataset.page)})}),document.getElementById("nav-logo").addEventListener("click",()=>kt("home")),document.getElementById("nav-credits").addEventListener("click",()=>kt("boosters")),document.getElementById("theme-toggle").addEventListener("click",()=>{const p=It()==="dark"?"light":"dark";mn(p),vi(p)}),vi(It())}function vi(e){const t=document.getElementById("theme-icon");t&&(t.textContent=e==="dark"?"☀️":"🌙")}async function yn(){Pt(It()),document.getElementById("modal-overlay").addEventListener("click",i=>{i.target===i.currentTarget&&Nt()}),document.getElementById("modal-close").addEventListener("click",Nt);const{data:{session:e}}=await A.auth.getSession();if(!e){wi(),ci(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:pt});return}pe.user=e.user,await ut(),wi();try{const{data:i}=await A.from("formation_links_overrides").select("formation, links"),n={};(i||[]).forEach(p=>{n[p.formation]=p.links}),Ji(n)}catch(i){console.warn("Impossible de charger les overrides de formation:",i)}if(!pe.profile){Zi(document.getElementById("app"),{state:pe,navigate:async()=>{await ut(),jt()},toast:pt,refreshProfile:ut});return}const t=Array.isArray(pe.profile.pending_boosters)?pe.profile.pending_boosters:[];if(!pe.profile.onboarding_done&&t.length>0){Do(document.getElementById("app"),{state:pe,navigate:()=>jt(),toast:pt,refreshProfile:ut});return}pe.profile.theme&&pe.profile.theme!==It()&&(localStorage.setItem(Zt,pe.profile.theme),Pt(pe.profile.theme)),jt(),A.auth.onAuthStateChange(async(i,n)=>{i==="SIGNED_OUT"&&(pe.user=null,pe.profile=null,document.getElementById("app").innerHTML="",ci(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:pt}))})}function hn(){return Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight)}function zt(){const e=document.getElementById("app");e&&(e.style.height=hn()+"px")}window.addEventListener("resize",zt);window.addEventListener("orientationchange",()=>setTimeout(zt,150));window.visualViewport&&window.visualViewport.addEventListener("resize",zt);function jt(){const e=document.getElementById("app");e.style.display="flex",e.style.flexDirection="column",zt(),xn(),Ki()}function wi(){const e=document.getElementById("app-loader"),t=document.getElementById("app");t&&(t.style.display=""),e&&(e.classList.add("zoom-out"),setTimeout(()=>e.style.display="none",500))}function Vi(e){var n;const t=document.getElementById("app-loader");if(t&&(t.style.display="none"),document.getElementById("boot-error"))return;const i=document.createElement("div");i.id="boot-error",i.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:99999;gap:16px;color:#fff;padding:24px;text-align:center",i.innerHTML=`
