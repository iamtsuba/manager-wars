import{s as _,F as oi,h as zt,j as Mi,l as Ze,i as dn,k as ln,b as cn}from"./formation-links-CEzN0rcz.js";function xi(e,{navigate:t,toast:n}){e.innerHTML=`
  ...
  `;function l(){var k;const i=document.getElementById("logo-preview"),r=document.getElementById("logo-initials"),f=((k=document.getElementById("setup-club"))==null?void 0:k.value)||"MW",x=f.trim().split(" ").filter(Boolean),g=x.length>=2?(x[0][0]+x[1][0]).toUpperCase():f.slice(0,2).toUpperCase();i&&(i.style.background=s,i.style.borderColor=d),r&&(r.textContent=g,r.style.color=d)}document.getElementById("color1").addEventListener("input",i=>{d=i.target.value,document.getElementById("swatch1").style.background=d,l()}),document.getElementById("color2").addEventListener("input",i=>{s=i.target.value,document.getElementById("swatch2").style.background=s,l()});function c(i){document.querySelectorAll(".setup-step").forEach(r=>r.classList.remove("active")),document.getElementById(`step-${i}`).classList.add("active"),document.getElementById("step-num").textContent=i,document.getElementById("progress-fill").style.width=`${Math.round(i/3*100)}%`,i===3&&l()}document.getElementById("step1-next").addEventListener("click",async()=>{const i=document.getElementById("setup-pseudo").value.trim(),r=document.getElementById("step1-error");if(r.textContent="",i.length<3){r.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:f}=await _.from("users").select("id").eq("pseudo",i).maybeSingle();if(f){r.textContent="Ce pseudo est déjà pris.";return}c(2)}),document.getElementById("step2-back").addEventListener("click",()=>c(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const i=document.getElementById("setup-club").value.trim(),r=document.getElementById("step2-error");if(r.textContent="",i.length<2){r.textContent="Nom trop court (min. 2 caractères).";return}const{data:f}=await _.from("users").select("id").eq("club_name",i).maybeSingle();if(f){r.textContent="Ce nom de club est déjà pris.";return}c(3)}),document.getElementById("step3-back").addEventListener("click",()=>c(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const i=document.getElementById("setup-pseudo").value.trim(),r=document.getElementById("setup-club").value.trim(),f=document.getElementById("step3-error"),x=document.getElementById("step3-finish");f.textContent="",x.disabled=!0,x.textContent="Création en cours…";try{const{error:g}=await _.from("users").insert({id:t.user.id,pseudo:i,club_name:r,club_color1:d,club_color2:s,credits:1e4});if(g)throw g;await un(t.user.id),await p(),a(`Bienvenue ${i} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(g){f.textContent=g.message,x.disabled=!1,x.textContent="🚀 Créer mon profil !"}})}async function un(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await _.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(n){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",n)}}const fn="modulepreload",gn=function(e){return"/manager-wars/"+e},yi={},mn=function(t,n,a){let p=Promise.resolve();if(n&&n.length>0){document.getElementsByTagName("link");const s=document.querySelector("meta[property=csp-nonce]"),l=(s==null?void 0:s.nonce)||(s==null?void 0:s.getAttribute("nonce"));p=Promise.allSettled(n.map(c=>{if(c=gn(c),c in yi)return;yi[c]=!0;const i=c.endsWith(".css"),r=i?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${c}"]${r}`))return;const f=document.createElement("link");if(f.rel=i?"stylesheet":fn,i||(f.as="script"),f.crossOrigin="",f.href=c,l&&f.setAttribute("nonce",l),document.head.appendChild(f),i)return new Promise((x,g)=>{f.addEventListener("load",x),f.addEventListener("error",()=>g(new Error(`Unable to preload CSS for ${c}`)))})}))}function d(s){const l=new Event("vite:preloadError",{cancelable:!0});if(l.payload=s,window.dispatchEvent(l),!l.defaultPrevented)throw s}return p.then(s=>{for(const l of s||[])l.status==="rejected"&&d(l.reason);return t().catch(d)})},Tt="#1A6B3C",It="#cc2222",xn="#D4A017",hi="#888";async function yn(e,t){const{state:n,toast:a}=t;e.innerHTML=`
    ...
    <div style="display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:8px;padding:10px 0;border-bottom:1px solid #f0f0f0">
      ...
      <div style="text-align:left;font-size:18px;font-weight:900;color:${te}">${B}</div>
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),i=(l||[]).filter(R=>R.card_type==="player"&&R.player),r=(l||[]).filter(R=>R.card_type==="game_changer"),f=(l||[]).filter(R=>R.card_type==="formation"),{data:x}=await _.from("gc_definitions").select("name,gc_type,color,effect,image_url"),g={};(x||[]).forEach(R=>{g[R.name]=R});const k=Object.keys(oi),o=Object.keys(Ne),$={};i.forEach(R=>{const H=R.player.id;$[H]=($[H]||0)+1}),new Set(Object.keys($).map(R=>String(R)));const L=new Set(f.map(R=>R.formation)),z=new Set(r.map(R=>R.gc_type));let F="player",K="Tous",q="",B=!1;function P(){return[...i].sort((R,H)=>{const S=jt.indexOf(R.player.job),j=jt.indexOf(H.player.job);return S!==j?S-j:(R.player.surname_encoded||"").localeCompare(H.player.surname_encoded||"")})}function te(){return[...c||[]].sort((R,H)=>{const S=jt.indexOf(R.job),j=jt.indexOf(H.job);return S!==j?S-j:(R.surname_encoded||"").localeCompare(H.surname_encoded||"")})}function ce(){return P().filter(R=>{const H=R.player,S=K==="Tous"||H.job===K,j=!q||`${H.firstname} ${H.surname_encoded}`.toLowerCase().includes(q);return S&&j})}function re(){return te().filter(R=>{const H=K==="Tous"||R.job===K,S=!q||`${R.firstname} ${R.surname_encoded}`.toLowerCase().includes(q);return H&&S})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",H=>{q=H.target.value.toLowerCase(),Y()}),e.querySelectorAll(".filter-btn").forEach(H=>{H.addEventListener("click",()=>{K=H.dataset.job,M(),Y()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{B=!B,M(),Y()})):(R.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{B=!B,M(),Y()})))}function Y(){const R=document.getElementById("col-grid");R&&(F==="player"?he(R):F==="formation"?xe(R):ge(R))}function ie(R,H,S,j,U){const ne=document.getElementById("col-grid"),ee=document.getElementById("col-big");if(!ne||!ee)return;var _e=0;function $e(){const Ee=window.innerWidth>=768,Me=document.getElementById("col-big"),Ae=document.getElementById("col-grid");Ee&&Me&&(Me.style.minHeight="420px",Me.style.flex="1 1 auto"),Ee&&Ae&&(Ae.style.height=Math.round(310*.76+16)+"px",Ae.style.flexShrink="0",Ae.style.overflowX="auto",Ae.style.overflowY="hidden",Ae.style.alignItems="center",Ae.style.padding="8px 16px"),ee.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+H(R[_e])+"</div>";var Te=ee.querySelector("[data-card-id],[data-form-id],[data-gc-id]");Te&&Te.addEventListener("click",function(){j(R[_e])}),requestAnimationFrame(function(){var ke=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!ke||!ee)){var De=ee.clientHeight-8,Re=ee.clientWidth-24,E=ke.offsetHeight,u=ke.offsetWidth;if(E>0&&u>0&&De>40){var m=Ee?2.2:1,h=Math.min(De/E,Re/u,m);ke.style.transform="scale("+h.toFixed(3)+")",ke.style.transformOrigin="top center"}}}),ne.innerHTML=R.map(function(ke,De){var Re=De===_e,E="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+(Re?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+De+'" style="'+E+'">'+S(ke,Re)+"</div>"}).join(""),ne.querySelectorAll(".col-mini-item").forEach(function(ke){ke.addEventListener("click",function(){_e=Number(ke.dataset.idx),$e(),ke.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}$e()}function D(R){var H=window.innerWidth>=768?.76:.54,S;if(!R||R._fake){var j=R?R.player:null;if(!j)return"";S=$i(j)}else S=Vt(R,"");return'<div style="display:inline-block;zoom:'+H+';pointer-events:none;line-height:0">'+S+"</div>"}function Q(R,H,S){H=H||100,S=S||140;var j=zt[R]||{},U={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},ne=Math.max(3,Math.round(H*.06)),ee=Object.entries(j).map(function($e){var Ee=$e[0],Me=$e[1],Ae=Ee.replace(/\d+$/,""),Te=U[Ae]||"#888",ke=Math.round(Me.x*H),De=Math.round(Me.y*S);return'<circle cx="'+ke+'" cy="'+De+'" r="'+ne+'" fill="'+Te+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),_e=Math.max(1,Math.round(H/50));return'<svg viewBox="0 0 '+H+" "+S+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+H+'" height="'+S+'" fill="#1A6B3C"/><rect x="'+Math.round(H*.2)+'" y="'+Math.round(S*.02)+'" width="'+Math.round(H*.6)+'" height="'+Math.round(S*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+_e+'"/><line x1="0" y1="'+Math.round(S*.5)+'" x2="'+H+'" y2="'+Math.round(S*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+_e+'"/><ellipse cx="'+Math.round(H*.5)+'" cy="'+Math.round(S*.5)+'" rx="'+Math.round(H*.18)+'" ry="'+Math.round(S*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+_e+'"/><rect x="'+Math.round(H*.2)+'" y="'+Math.round(S*.82)+'" width="'+Math.round(H*.6)+'" height="'+Math.round(S*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+_e+'"/>'+ee+"</svg>"}function ae(R,H,S){var j=S>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+S+"</div>":"",U=H?'data-form-id="'+H.id+'"':"",ne=R.length>7?14:R.length>5?16:19,ee=!!H;return"<div "+U+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(ee?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(ee?"":"filter:grayscale(1);opacity:0.5")+'">'+j+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(ee?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+ne+"px;font-weight:900;color:"+(ee?"#1A6B3C":"#aaa")+';line-height:1">'+R+'</div></div><div style="flex:1;overflow:hidden;background:'+(ee?"#1A6B3C":"#ccc")+'">'+Q(R,140,220)+"</div></div>"}function pe(R,H){var S=window.innerWidth>=768?.76:.54,j=140,U=305,ne=Math.round(U*.22),ee=U-ne,_e=R.length>7?5:7,$e=Q(R,j,ee),Ee=H?"1.5px solid #2a7a40":"1px solid #ddd",Me=H?"":"filter:grayscale(1);opacity:0.45;",Ae=H?"#1A6B3C":"#bbb",Te="#fff";return'<div style="display:inline-block;zoom:'+S+';line-height:0;pointer-events:none"><div style="width:'+j+"px;height:"+U+"px;border-radius:6px;border:"+Ee+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+Me+'"><div style="height:'+ne+"px;background:"+Ae+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+_e+"px;font-weight:900;color:"+Te+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(j-4)+'px">'+R+'</span></div><div style="height:'+ee+'px;overflow:hidden;flex-shrink:0">'+$e+"</div></div></div>"}function he(R){if(B){const H=re();if(!H.length){R.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const S=H.map(j=>i.find(U=>U.player.id===j.id)||{_fake:!0,player:j,id:"fake-"+j.id});ie(S,j=>j._fake?$i(j.player):Vt(j,""),j=>j._fake?D({player:j.player,id:"x",_fake:!0}):D(j),j=>{j._fake||ki(j,i,$,t)})}else{const H=ce();if(!H.length){R.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const S={};H.forEach(U=>{const ne=U.player.id;S[ne]||(S[ne]=[]),S[ne].push(U)});const j=Object.values(S).map(U=>Mn(U));ie(j,U=>{const ne=$[U.player.id]||1,ee=ne>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${ne}</div>`:"",$e=i.filter(Ee=>Ee.player.id===U.player.id&&Ee.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Vt(U,ee+$e)},U=>D(U),U=>ki(U,i,$,t))}}function xe(R){const H=B?k:[...L];if(!H.length){R.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const S=H.map(j=>({formation:j,card:f.find(U=>U.formation===j)||null,owned:L.has(j)}));ie(S,({formation:j,card:U,owned:ne})=>ae(j,ne?U:null,ne?f.filter(ee=>ee.formation===j).length:0),({formation:j,owned:U})=>pe(j,U),({card:j,owned:U})=>{U&&j&&jn(j,f,t,d)})}function ge(R){const H=Object.keys(g),S=B?H.length?H:o:[...z];if(!S.length){R.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const j=S.map(U=>({type:U,gc:Ne[U]||{icon:"⚡",desc:""},def:g[U]||null,owned:z.has(U),card:r.find(ne=>ne.gc_type===U)||null}));ie(j,({type:U,gc:ne,def:ee,owned:_e,card:$e})=>{const Ee=_e?r.filter(m=>m.gc_type===U).length:0,Me=Ee>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${Ee}</div>`:"",Ae=(ee==null?void 0:ee.gc_type)==="ultra_game_changer",Te={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},ke={purple:"#b06ce0",light_blue:"#00d4ef"},De=Te[ee==null?void 0:ee.color]||Te.purple,Re=ke[ee==null?void 0:ee.color]||ke.purple,E=(ee==null?void 0:ee.effect)||ne.desc||"",u=ee!=null&&ee.image_url?`/manager-wars/icons/${ee.image_url}`:null;return _e&&$e?`<div data-gc-id="${$e.id}" data-gc-type="${U}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${Re};background:${De};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${Re}66;cursor:pointer">
          ${Me}
        ...
        </div>`},({type:U,gc:ne,def:ee,owned:_e})=>{const $e=window.innerWidth>=768?.76:.54,Ee={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},Me={purple:"#9b59b6",light_blue:"#00bcd4"},Ae=Ee[ee==null?void 0:ee.color]||Ee.purple,Te=Me[ee==null?void 0:ee.color]||Me.purple,ke=ee!=null&&ee.image_url?`/manager-wars/icons/${ee.image_url}`:null;return _e?`<div style="display:inline-block;zoom:${$e};line-height:0;pointer-events:none"><div style="width:140px;height:310px;border-radius:8px;background:${Ae};border:1px solid ${Te};display:flex;flex-direction:column;overflow:hidden"><div style="height:56px;background:rgba(255,255,255,0.15);display:flex;align-items:center;justify-content:center"><span style="font-size:9px;font-weight:900;color:#fff;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:134px">${U}</span></div><div style="flex:1;display:flex;align-items:center;justify-content:center">${ke?`<img src="${ke}" style="max-width:130px;max-height:190px;object-fit:contain">`:`<span style="font-size:48px">${ne.icon}</span>`}</div><div style="height:54px;display:flex;align-items:center;justify-content:center;padding:0 4px"><span style="font-size:8px;color:rgba(255,255,255,0.7);text-align:center">${((ee==null?void 0:ee.effect)||ne.desc||"").slice(0,30)}</span></div></div></div>`:`<div style="display:inline-block;zoom:${$e};line-height:0;pointer-events:none"><div style="width:140px;height:310px;border-radius:8px;background:#eee;border:1px solid #ddd;display:flex;flex-direction:column;align-items:center;justify-content:center;filter:grayscale(1);opacity:0.45"><span style="font-size:36px">${ne.icon}</span><span style="font-size:9px;color:#aaa;margin-top:6px;text-align:center;padding:0 6px">${U}</span></div></div>`},({type:U,owned:ne,def:ee})=>{ne&&Cn(U,ee,d)})}e.querySelectorAll(".col-tab-btn").forEach(R=>{R.addEventListener("click",()=>{F=R.dataset.tab,K="Tous",q="",B=!1,e.querySelectorAll(".col-tab-btn").forEach(H=>{const S=H.dataset.tab===F;H.style.borderBottomColor=S?"var(--green)":"transparent",H.style.color=S?"var(--green)":"var(--gray-600)"}),M(),Y()})}),M(),Y()}function Cn(e,t,n){const a=Ne[e]||{icon:"⚡",desc:"Effet spécial."},p=(t==null?void 0:t.gc_type)==="ultra_game_changer",d={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},s={purple:"#b06ce0",light_blue:"#00d4ef"},l=d[t==null?void 0:t.color]||d.purple,c=s[t==null?void 0:t.color]||s.purple,i=(t==null?void 0:t.name)||e,r=(t==null?void 0:t.effect)||a.desc,f=t!=null&&t.image_url?`/manager-wars/icons/${t.image_url}`:null;n("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${f.map((S,j)=>{const U=M[S.id]||[],ne=S.is_for_sale,ee=U.length?U[U.length-1]:null,$e=(r.rarity==="pepite"||r.rarity==="papyte")&&S.current_note!=null?` (☆${S.current_note})`:"";return`
            <div data-card-id="${S.id}" data-card-idx="${j}" class="exemplaire-row"
              ...
              <div style="display:flex;align-items:center;gap:10px">
                ...
                <div style="flex:1;min-width:0">
                  <div style="font-size:12px;font-weight:700">Exemplaire ${j+1}${$e}${ne?" 🏷️ En vente":""}</div>
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",r.note_g],["DEF",r.note_d],["MIL",r.note_m],["ATT",r.note_a]].map(([S,j])=>{const U=Et[S],ne=Number(j)||0;return`<div style="display:flex;flex-direction:column;align-items:center;gap:2px">
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(xe=document.getElementById("close-detail"))==null||xe.addEventListener("click",l);let D=new Set;const Q=()=>{const S=D.size,j=document.getElementById("sell-action-panel");j&&(j.style.display=S>0?"block":"none",document.getElementById("sell-selected-count").textContent=S,document.getElementById("sell-direct-total").textContent=(S*$).toLocaleString("fr")+" cr.")};document.querySelectorAll(".expl-check").forEach(S=>{S.addEventListener("change",()=>{const j=S.dataset.id;S.checked?D.add(j):D.delete(j);const U=S.closest(".exemplaire-row");U&&(U.style.borderColor=S.checked?"#1A6B3C":"#eee"),Q()})}),document.querySelectorAll(".exemplaire-row").forEach(S=>{S.addEventListener("click",j=>{if(j.target.closest("button")||j.target.tagName==="INPUT")return;const U=S.querySelector(".expl-check");U&&!U.disabled&&(U.checked=!U.checked,U.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(S=>{S.addEventListener("click",j=>{j.stopPropagation();const U=document.querySelector(`.expl-hist[data-hist="${S.dataset.idx}"]`);U&&(U.style.display=U.style.display==="none"?"flex":"none")})}),(ge=document.getElementById("direct-sell-btn"))==null||ge.addEventListener("click",async()=>{const S=[...D];if(!S.length)return;const j=S.length*$;if(!confirm(`Vendre ${S.length} carte${S.length>1?"s":""} ${r.surname_encoded} pour ${j.toLocaleString("fr")} crédits ? Action irréversible.`))return;await _.from("market_listings").delete().in("card_id",S),await _.from("transfer_history").delete().in("card_id",S);const{error:U}=await _.from("cards").delete().in("id",S);if(U){d(U.message,"error");return}await _.from("users").update({credits:(p.profile.credits||0)+j}).eq("id",p.profile.id),await i();const ne=document.getElementById("nav-credits");ne&&(ne.textContent=`💰 ${(p.profile.credits||0).toLocaleString("fr")}`),d(`+${j.toLocaleString("fr")} crédits ! ${S.length} carte${S.length>1?"s":""} vendue${S.length>1?"s":""}.`,"success"),l(),c("collection")}),(R=document.getElementById("market-sell-btn"))==null||R.addEventListener("click",async()=>{var _e;const S=[...D];if(!S.length){d("Sélectionne au moins un exemplaire","warning");return}const j=parseInt((_e=document.getElementById("sell-market-price"))==null?void 0:_e.value);if(!j||j<1){d("Prix invalide","error");return}const{error:U}=await _.from("cards").update({is_for_sale:!0,sale_price:j}).in("id",S);if(U){d(U.message,"error");return}const ne=S.map($e=>({seller_id:p.profile.id,card_id:$e,price:j,status:"active"})),{error:ee}=await _.from("market_listings").insert(ne);ee&&console.warn("[Market] insert listings:",ee.message),d(`${S.length} carte${S.length>1?"s":""} mise${S.length>1?"s":""} en vente à ${j.toLocaleString("fr")} cr. chacune !`,"success"),l(),c("collection")}),(H=document.getElementById("cancel-sell-btn"))==null||H.addEventListener("click",async()=>{await _.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await _.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),d("Annonce retirée","success"),l(),c("collection")})}const Gt={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},rt={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function Ut(e){const t=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const n=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${n}.png`}function Hi(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function Ui(e){var n;const t="https://fcnwclxlkytdfjotqkta.supabase.co";return(n=e==null?void 0:e.clubs)!=null&&n.logo_url?e.clubs.logo_url.startsWith("http")?e.clubs.logo_url:`${t}/storage/v1/object/public/assets/clubs/${e.clubs.logo_url}`:null}function Bn(e,t=44,n=58){var g;const a=e?Ut(e):null,p=e?Ui(e):null,d=Hi(e==null?void 0:e.country_code),s=(e==null?void 0:e.job)||"MIL",l=rt[s]||"#555",c={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",i=Number(s==="GK"?e==null?void 0:e.note_g:s==="DEF"?e==null?void 0:e.note_d:s==="MIL"?e==null?void 0:e.note_m:e==null?void 0:e.note_a)||0,r=Math.round(n*.19),f=Math.round(n*.25),x=n-r-f;return e?`<div style="width:${t}px;height:${n}px;border-radius:5px;border:2px solid ${c};background:${l};position:relative;overflow:hidden;flex-shrink:0">
  ...
  </div>`,Fn(e,t,p,n),document.getElementById("builder-back").addEventListener("click",()=>a("decks")),document.getElementById("formation-select").addEventListener("change",i=>{t.formation=i.target.value;const r=Ai(t.formation),f={};r.forEach(x=>{t.slots[x]&&(f[x]=t.slots[x])}),t.slots=f,pt(e,t,n)}),document.getElementById("save-deck").addEventListener("click",()=>Gn(t,n)),e.querySelectorAll("[data-remove-sub]").forEach(i=>{i.addEventListener("click",()=>{t.subs=t.subs.filter(r=>r!==i.dataset.removeSub),pt(e,t,n)})}),(c=document.getElementById("add-sub-btn"))==null||c.addEventListener("click",()=>{Dn(t,e,n)})}function Fn(e,t,n,a){const p=e.querySelector("#deck-field");if(!p)return;const d=zt[t.formation]||{},s=Mi(t.formation)||[],l={};for(const L of n){const z=t.slots[L],F=z?t.playerCards.find(K=>K.id===z):null;l[L]=F?F.player:null}const c=300,i=300,r=48,f=64,x=13,g=16,k=38;function o(L){const z=d[L];return z?{x:z.x*c,y:z.y*i}:null}let $="";for(const[L,z]of s){const F=o(L),K=o(z);if(!F||!K)continue;const q=l[L]?{...l[L],club_id:l[L].club_id,country_code:l[L].country_code,rarity:l[L].rarity}:null,B=l[z]?{...l[z],club_id:l[z].club_id,country_code:l[z].country_code,rarity:l[z].rarity}:null,P=Ze(q,B);P==="#ff3333"||P==="#cc2222"?$+=`<line x1="${F.x.toFixed(1)}" y1="${F.y.toFixed(1)}" x2="${K.x.toFixed(1)}" y2="${K.y.toFixed(1)}" stroke="${P}" stroke-width="2.5" stroke-linecap="round" opacity="0.4"/>`:($+=`<line x1="${F.x.toFixed(1)}" y1="${F.y.toFixed(1)}" x2="${K.x.toFixed(1)}" y2="${K.y.toFixed(1)}" stroke="${P}" stroke-width="8" stroke-linecap="round" opacity="0.2"/>`,$+=`<line x1="${F.x.toFixed(1)}" y1="${F.y.toFixed(1)}" x2="${K.x.toFixed(1)}" y2="${K.y.toFixed(1)}" stroke="${P}" stroke-width="2.5" stroke-linecap="round" opacity="0.9"/>`)}for(const L of n){const z=o(L);if(!z)continue;const F=l[L],K=L.replace(/\d+/,""),q=rt[K]||"#555",B=(z.x-r/2).toFixed(1),P=(z.y-f/2).toFixed(1),te={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[F==null?void 0:F.rarity]||"#aaa";if(F){const ce=Ut(F),re=Ui(F),M=Hi(F.country_code),Y=Number(K==="GK"?F.note_g:K==="DEF"?F.note_d:K==="MIL"?F.note_m:F.note_a)||0,ie=f-x-g;$+=`<defs><clipPath id="dcp-${L}"><rect x="${B}" y="${(z.y-f/2+x).toFixed(1)}" width="${r}" height="${ie}"/></clipPath></defs>`,$+=`<rect x="${B}" y="${P}" width="${r}" height="${f}" rx="5" fill="${q}"/>`,ce&&($+=`<image href="${ce}" x="${B}" y="${(z.y-f/2+x).toFixed(1)}" width="${r}" height="${ie}" clip-path="url(#dcp-${L})" preserveAspectRatio="xMidYMin slice"/>`),$+=`<rect x="${B}" y="${P}" width="${r}" height="${x}" fill="rgba(255,255,255,0.93)"/>`,$+=`<text x="${z.x.toFixed(1)}" y="${(z.y-f/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(F.surname_encoded||"").slice(0,9)}</text>`;const D=(z.y+f/2-g).toFixed(1);$+=`<rect x="${B}" y="${D}" width="${r}" height="${g}" fill="rgba(255,255,255,0.93)"/>`,M&&($+=`<image href="${M}" x="${(z.x-21).toFixed(1)}" y="${(z.y+f/2-g+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`),$+=`<text x="${z.x.toFixed(1)}" y="${(z.y+f/2-g/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${Y}</text>`,re&&($+=`<image href="${re}" x="${(z.x+r/2-14).toFixed(1)}" y="${(z.y+f/2-g+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`),$+=`<rect x="${B}" y="${P}" width="${r}" height="${f}" rx="5" fill="none" stroke="${te}" stroke-width="2"/>`}else $+=`<rect x="${B}" y="${P}" width="${r}" height="${f}" rx="5" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="5,3"/>`,$+=`<text x="${z.x.toFixed(1)}" y="${z.y.toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="22" fill="rgba(255,255,255,0.35)">+</text>`,$+=`<text x="${z.x.toFixed(1)}" y="${(z.y+16).toFixed(1)}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.3)">${K}</text>`;$+=`<rect x="${B}" y="${P}" width="${r}" height="${f}" rx="5" fill="rgba(0,0,0,0.01)" class="deck-slot-hit" data-pos="${L}" style="cursor:pointer"/>`}p.innerHTML=`<svg viewBox="${-k} ${-k} ${c+k*2} ${i+k*2}" width="100%" style="display:block;max-width:440px;margin:0 auto">${$}</svg>`,p.querySelectorAll(".deck-slot-hit").forEach(L=>{L.addEventListener("click",()=>qn(L.dataset.pos,t,e,a))})}function qn(e,t,n,a){var x,g,k;const{openModal:p,closeModal:d}=a,s=e.replace(/\d+/,""),l=t.slots[e],c=l?t.playerCards.find(o=>o.id===l):null;(x=c==null?void 0:c.player)==null||x.id;const i=new Set;Object.entries(t.slots).forEach(([o,$])=>{var z;if(o===e||!$)return;const L=t.playerCards.find(F=>F.id===$);(z=L==null?void 0:L.player)!=null&&z.id&&i.add(L.player.id)}),t.subs.forEach(o=>{var L;const $=t.playerCards.find(z=>z.id===o);(L=$==null?void 0:$.player)!=null&&L.id&&i.add($.player.id)});const r=new Set,f=t.playerCards.filter(o=>{const $=o.player;return!($.job===s||$.job2===s)||i.has($.id)||r.has($.id)?!1:(r.add($.id),!0)});f.sort((o,$)=>{const L=s==="GK"?o.player.note_g:s==="DEF"?o.player.note_d:s==="MIL"?o.player.note_m:o.player.note_a;return(s==="GK"?$.player.note_g:s==="DEF"?$.player.note_d:s==="MIL"?$.player.note_m:$.player.note_a)-L}),p(`Choisir ${s} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
  ...
  `,document.body.appendChild(p);let d=!1;const s=document.getElementById("pack-cut-zone"),l=document.getElementById("pack-blade");let c=!1;const i=D=>D.touches&&D.touches[0]?D.touches[0].clientX:D.clientX;function r(D){d||(c=!0,l.style.opacity="1",f(D))}function f(D){if(!c||d)return;const Q=s.getBoundingClientRect(),ae=i(D)-Q.left,pe=Math.max(0,Math.min(1,ae/Q.width));l.style.width=pe*Q.width+"px",pe>=.82&&g()}function x(){d||(c=!1,l.style.transition="width .2s ease, opacity .2s ease",l.style.width="0",l.style.opacity="0",setTimeout(()=>{d||(l.style.transition="")},220))}function g(){var Q;if(d)return;d=!0,c=!1,l.style.width="100%",l.style.opacity="1",(Q=document.getElementById("cut-flash"))==null||Q.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const D=document.getElementById("cut-hint");D&&(D.style.opacity="0"),s.classList.add("pack-cut"),setTimeout(()=>{l.style.opacity="0",document.getElementById("pack-phase").style.display="none",L(0)},620)}s.addEventListener("pointerdown",r),window.addEventListener("pointermove",f),window.addEventListener("pointerup",x),s.addEventListener("touchstart",r,{passive:!0}),window.addEventListener("touchmove",f,{passive:!0}),window.addEventListener("touchend",x);let k=0,o=!1;const $=new Set;function L(D){k=D,document.getElementById("reveal-phase").style.display="flex",z(),F(D,0),te()}function z(){const D=document.getElementById("card-dots");D&&(D.innerHTML=e.map((Q,ae)=>`<div class="card-dot" data-i="${ae}" style="width:8px;height:8px;border-radius:50%;background:${ae===k?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),D.querySelectorAll(".card-dot").forEach(Q=>Q.addEventListener("click",()=>q(parseInt(Q.dataset.i)))))}function F(D,Q){var j;const ae=e[D],pe=document.getElementById("card-counter"),he=document.getElementById("card-track");pe&&(pe.textContent=`Carte ${D+1} / ${e.length}`);const xe=document.getElementById("reveal-btns");xe&&(xe.style.display=D===e.length-1?"flex":"none");const ge=ae.card_type==="player"&&((j=ae.player)==null?void 0:j.rarity)==="legende",R=!$.has(D);$.add(D);let H=0;if(ae.card_type==="player"&&ae.player){const U=ae.player,ne=U.job||"ATT";H=Number(ne==="GK"?U.note_g:ne==="DEF"?U.note_d:ne==="MIL"?U.note_m:U.note_a)||0}const S=U=>{he.innerHTML=`
        ...
        </div>`;const ne=document.getElementById("current-card-wrap");Q!==0?(ne.style.transition="none",ne.style.transform=`translateX(${Q>0?100:-100}%)`,requestAnimationFrame(()=>{ne.style.transition="transform .28s cubic-bezier(.25,1,.5,1)",ne.style.transform="translateX(0)"})):ne.animate([{opacity:0,transform:"scale(.7)"},{opacity:1,transform:"scale(1)"}],{duration:300,easing:"cubic-bezier(.34,1.56,.64,1)"}),U||ge?re():M(),z()};R&&H>6&&ae.card_type==="player"&&ae.player?K(ae,()=>S(!0)):S(!1)}function K(D,Q){var H;o=!0;const ae=D.player,pe=`https://flagsapi.com/${ae.country_code}/flat/64.png`,he=(H=ae.clubs)==null?void 0:H.logo_url,xe=document.getElementById("walkout-overlay"),ge=document.getElementById("walkout-stage");if(!xe||!ge){o=!1,Q();return}xe.style.display="flex";const R=()=>{const S=ge.firstElementChild;S&&(S.classList.remove("wo-in"),S.classList.add("wo-out"))};ge.innerHTML=`<img class="wo-in" src="${pe}" style="height:130px;border-radius:10px;box-shadow:0 10px 36px rgba(0,0,0,.6)" onerror="this.style.display='none'">`,navigator.vibrate&&navigator.vibrate(30),setTimeout(R,2e3),setTimeout(()=>{var S;ge.innerHTML=he?`<img class="wo-in" src="${he}" style="max-height:160px;max-width:210px;object-fit:contain">`:`<div class="wo-in" style="font-size:34px;font-weight:900;color:#fff;text-align:center">${((S=ae.clubs)==null?void 0:S.encoded_name)||"CLUB"}</div>`,navigator.vibrate&&navigator.vibrate(30)},2450),setTimeout(R,4450),setTimeout(()=>{xe.style.display="none",ge.innerHTML="",o=!1,navigator.vibrate&&navigator.vibrate([40,30,80]),Q()},4900)}function q(D){if(o||D<0||D>=e.length||D===k)return;const Q=D>k?1:-1;k=D,F(D,Q)}function B(){q(k+1)}function P(){q(k-1)}function te(){const D=document.getElementById("card-viewport");if(!D||D._swipeBound)return;D._swipeBound=!0;let Q=0,ae=0,pe=0,he=!1;const xe=j=>j.touches?j.touches[0].clientX:j.clientX,ge=j=>j.touches?j.touches[0].clientY:j.clientY,R=j=>{he=!0,Q=xe(j),ae=ge(j),pe=0},H=j=>{if(!he)return;pe=xe(j)-Q;const U=ge(j)-ae;if(Math.abs(pe)<Math.abs(U))return;const ne=document.getElementById("current-card-wrap");ne&&(ne.style.transition="none",ne.style.transform=`translateX(${pe*.6}px) rotate(${pe*.02}deg)`)},S=()=>{if(!he)return;he=!1;const j=document.getElementById("current-card-wrap"),U=55;pe<=-U&&k<e.length-1?B():pe>=U&&k>0?P():j&&(j.style.transition="transform .2s ease",j.style.transform="translateX(0)")};D.addEventListener("pointerdown",R),D.addEventListener("pointermove",H),D.addEventListener("pointerup",S),D.addEventListener("pointercancel",S),D.addEventListener("touchstart",R,{passive:!0}),D.addEventListener("touchmove",H,{passive:!0}),D.addEventListener("touchend",S),D.addEventListener("click",j=>{if(Math.abs(pe)>8)return;const U=D.getBoundingClientRect();j.clientX-U.left>U.width/2?B():P()})}let ce=null;function re(){const D=document.getElementById("fireworks-canvas");if(!D)return;D.width=window.innerWidth,D.height=window.innerHeight;const Q=D.getContext("2d"),ae=[];function pe(){const xe=Math.random()*D.width,ge=Math.random()*D.height*.6,R=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],H=R[Math.floor(Math.random()*R.length)];for(let S=0;S<60;S++){const j=Math.PI*2/60*S,U=2+Math.random()*5;ae.push({x:xe,y:ge,vx:Math.cos(j)*U,vy:Math.sin(j)*U,alpha:1,color:H,size:2+Math.random()*3})}}pe(),ce=setInterval(pe,600);function he(){if(document.getElementById("fireworks-canvas")){Q.clearRect(0,0,D.width,D.height);for(let xe=ae.length-1;xe>=0;xe--){const ge=ae[xe];if(ge.x+=ge.vx,ge.y+=ge.vy+.08,ge.vy*=.98,ge.alpha-=.018,ge.alpha<=0){ae.splice(xe,1);continue}Q.globalAlpha=ge.alpha,Q.fillStyle=ge.color,Q.beginPath(),Q.arc(ge.x,ge.y,ge.size,0,Math.PI*2),Q.fill()}Q.globalAlpha=1,(ce!==null||ae.length>0)&&requestAnimationFrame(he)}}he()}function M(){ce!==null&&(clearInterval(ce),ce=null);const D=document.getElementById("fireworks-canvas");D&&D.getContext("2d").clearRect(0,0,D.width,D.height)}if(a){const D=document.getElementById("reveal-btns");D&&(D.innerHTML='<button class="btn btn-primary" id="reveal-next" style="flex:1">Continuer →</button>'),(ie=document.getElementById("reveal-next"))==null||ie.addEventListener("click",()=>{M(),p.remove(),a()})}else document.getElementById("reveal-collection").addEventListener("click",()=>{M(),p.remove(),n("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{M(),p.remove(),n("boosters")})}function Xn(e){var t,n;if(e.card_type==="player"&&e.player)return Un(e);if(e.card_type==="game_changer"){const a=e._gcDef,p=(a==null?void 0:a.gc_type)==="ultra_game_changer",d={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},s={purple:"#b06ce0",light_blue:"#00d4ef"},l=d[a==null?void 0:a.color]||d.purple,c=s[a==null?void 0:a.color]||s.purple,i=(a==null?void 0:a.name)||e.gc_type||"Game Changer",r=(a==null?void 0:a.effect)||((t=Jt[e.gc_type])==null?void 0:t.desc)||"",f=a!=null&&a.image_url?`/manager-wars/icons/${a.image_url}`:null,x=((n=Jt[e.gc_type])==null?void 0:n.icon)||"⚡";return`<div style="width:170px;height:240px;background:${l};border-radius:14px;border:3px solid ${c};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${c}88;flex-shrink:0">
  ...
  </div>`}function it(e,t,n){if(!(e!=null&&e.length))return"";const a=e.slice(0,5);let p='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return a.forEach((d,s)=>{if(p+=Ve(d,40,52),s<a.length-1){const l=Ze(d,a[s+1]);p+=`<div style="width:7px;height:3px;background:${l==="#ff3333"||l==="#cc2222"?"rgba(255,255,255,0.12)":l};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),n!==void 0&&(p+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${n}</div>`),p+="</div>",p}function ei(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function tn(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function st(e,t,n,a,p=310,d=310,s=[]){var F;const l=zt[t]||{},c=Mi(t)||oi[t]||[],i={},r=["ATT","MIL","DEF","GK"];for(const K of r)(e[K]||[]).forEach((B,P)=>{i[`${K}${P+1}`]=B});function f(K){const q=l[K];return q?{x:q.x*p,y:q.y*d}:null}let x="";for(const[K,q]of c){const B=f(K),P=f(q);if(!B||!P)continue;const te=i[K],ce=i[q],re=Ze(te,ce);re==="#00ff88"||re==="#FFD700"?(x+=`<line x1="${B.x.toFixed(1)}" y1="${B.y.toFixed(1)}" x2="${P.x.toFixed(1)}" y2="${P.y.toFixed(1)}"
        stroke="${re}" stroke-width="10" stroke-linecap="round" opacity="0.22"/>`,x+=`<line x1="${B.x.toFixed(1)}" y1="${B.y.toFixed(1)}" x2="${P.x.toFixed(1)}" y2="${P.y.toFixed(1)}"
        stroke="${re}" stroke-width="3.5" stroke-linecap="round" opacity="0.95"/>`):x+=`<line x1="${B.x.toFixed(1)}" y1="${B.y.toFixed(1)}" x2="${P.x.toFixed(1)}" y2="${P.y.toFixed(1)}"
        stroke="${re}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const g=48,k=64,o=13,$=16,L={normal:"#aaaaaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#111111",legende:"#7a28b8",légende:"#7a28b8"};for(const[K,q]of Object.entries(i)){const B=f(K);if(!B||!q)continue;const P=K.replace(/[0-9]/g,""),te=ft[P]||"#555",ce=s.includes(q.cardId),re=n==="attack"&&(["MIL","ATT"].includes(P)||ce)&&!q.used||n==="defense"&&["GK","DEF","MIL"].includes(P)&&!q.used,M=a.includes(q.cardId);let Y;n==="attack"?Y=ce?Math.max(1,Number(q.note_a)||0):P==="MIL"?Number(q.note_m)||0:Number(q.note_a)||0:n==="defense"?Y=P==="GK"?Number(q.note_g)||0:P==="MIL"?Number(q.note_m)||0:Number(q.note_d)||0:Y=Number(P==="GK"?q.note_g:P==="DEF"?q.note_d:P==="MIL"?q.note_m:q.note_a)||0,Y=Y+(q.boost||0);const ie=(B.x-g/2).toFixed(1),D=(B.y-k/2).toFixed(1),Q=L[q==null?void 0:q.rarity]||L.normal;if(q.used){const H=typeof import.meta<"u"&&"/manager-wars/"||"/",j=`${typeof window<"u"&&((F=window.location)==null?void 0:F.origin)||""}${H}icons/carte-dos.png`.replace(/([^:])\/\//g,"$1/");x+=`<rect x="${ie}" y="${D}" width="${g}" height="${k}" rx="5" fill="#161616"/>`,x+=`<image href="${j}" xlink:href="${j}" x="${ie}" y="${D}" width="${g}" height="${k}" preserveAspectRatio="xMidYMid slice"/>`,x+=`<rect x="${ie}" y="${D}" width="${g}" height="${k}" rx="5" fill="none" stroke="${Q}" stroke-width="2" opacity="0.7"/>`,x+=`<rect x="${ie}" y="${D}" width="${g}" height="${k}" rx="5" fill="rgba(0,0,0,0.01)" class="match-used-hit" data-card-id="${q.cardId}" data-role="${P}" style="cursor:pointer"/>`;continue}const ae=M?.45:1,pe=M?"#FFD700":Q,he=M?3:(q==null?void 0:q.rarity)==="legende"||(q==null?void 0:q.rarity)==="légende"||(q==null?void 0:q.rarity)==="pepite"||(q==null?void 0:q.rarity)==="pépite"?2.5:2,xe=k-o-$;x+=`<defs><clipPath id="cp-${K}"><rect x="${ie}" y="${(B.y-k/2+o).toFixed(1)}" width="${g}" height="${xe}"/></clipPath></defs>`,x+=`<rect x="${ie}" y="${D}" width="${g}" height="${k}" rx="5" fill="${te}" opacity="${ae}"/>`;const ge=ze(q);ge&&(x+=`<image href="${ge}" xlink:href="${ge}" x="${ie}" y="${(B.y-k/2+o).toFixed(1)}" width="${g}" height="${xe}" clip-path="url(#cp-${K})" preserveAspectRatio="xMidYMin slice"/>`),x+=`<rect x="${ie}" y="${D}" width="${g}" height="${o}" rx="4" fill="rgba(255,255,255,0.92)"/>`,x+=`<text x="${B.x.toFixed(1)}" y="${(B.y-k/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(q.name||"").slice(0,9)}</text>`;const R=(B.y+k/2-$).toFixed(1);x+=`<rect x="${ie}" y="${R}" width="${g}" height="${$}" fill="rgba(255,255,255,0.92)"/>`;{const H=ei(q.country_code);H?x+=`<image href="${H}" xlink:href="${H}" x="${(B.x-20).toFixed(1)}" y="${(B.y+k/2-$+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:x+=`<text x="${(B.x-13).toFixed(1)}" y="${(B.y+k/2-$/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${tn(q.country_code)}</text>`,x+=`<text x="${B.x.toFixed(1)}" y="${(B.y+k/2-$/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${Y}</text>`;const S=en(q);S?x+=`<image href="${S}" xlink:href="${S}" x="${(B.x+g/2-14).toFixed(1)}" y="${(B.y+k/2-$+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:q.clubName&&(x+=`<text x="${(B.x+14).toFixed(1)}" y="${(B.y+k/2-$/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(q.clubName||"").slice(0,3).toUpperCase()}</text>`),q.boost&&(x+=`<rect x="${(B.x+g/2-12).toFixed(1)}" y="${(B.y-k/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,x+=`<text x="${(B.x+g/2-5).toFixed(1)}" y="${(B.y-k/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${q.boost}</text>`)}x+=`<rect x="${ie}" y="${D}" width="${g}" height="${k}" rx="5" fill="${M?"rgba(255,255,255,0.12)":"none"}" stroke="${pe}" stroke-width="${he}" opacity="${ae}"/>`,re&&(x+=`<rect x="${ie}" y="${D}" width="${g}" height="${k}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${M?"selected":""}" data-card-id="${q.cardId}" data-role="${P}" style="cursor:pointer"/>`)}const z=38;return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-z} ${-z} ${p+z*2} ${d+z*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${x}
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${o.map((F,K)=>{const q=K<o.length-1?Ze(F,o[K+1]):null,B=!q||q==="#ff3333"||q==="#cc2222",P=q==="#00ff88"?"+2":q==="#FFD700"?"+1":"";return`
          ...
          ${K<o.length-1?`<div class="duel-link duel-link-${z}" data-idx="${K}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${B?"rgba(255,255,255,0.12)":q};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${B?"none":`0 0 8px ${q}`}">
  ...
  <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div id="duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center, rgba(10,61,30,0.4), rgba(10,61,30,0.92))"></div>
          ...
          <button id="start-match-btn" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,0.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">
            ▶ Commencer le match
          </button>`,q.style.transition="opacity .45s ease",q.style.opacity="1",q.style.pointerEvents="auto",(B=document.getElementById("start-match-btn"))==null||B.addEventListener("click",k))},600)},700)},2800)}function fo(e){if(e.type==="duel"&&(e.homeTotal!=null||e.aiTotal!=null)){const t=(e.homeTotal||0)>=(e.aiTotal||0);return`
    ...
    <div style="display:flex;align-items:center;gap:4px;${t?"flex-direction:row-reverse":""};background:rgba(255,255,255,0.04);border-radius:8px;padding:5px 8px;border:1px solid rgba(255,255,255,0.07)">
      ...
      ${nt(e.inPlayer||{})}
    </div>`}return`<div style="font-size:11px;color:${e.type==="goal"?"#FFD700":"rgba(255,255,255,0.65)"};font-weight:${e.type==="goal"?700:400};padding:3px 2px">${e.text||""}</div>`}function Se(e,t,n){var F,K,q,B,P,te,ce,re;const a=t.selected.map(M=>M.cardId),p=t.usedSubIds||[],d=t.homeSubs.filter(M=>!p.includes(M.cardId)),l=Object.values(t.homeTeam).flat().filter(M=>M.used).length>0&&d.length>0&&t.subsUsed<t.maxSubs,c=!["GK","DEF","MIL","ATT"].some(M=>(t.aiTeam[M]||[]).some(Y=>!Y.used)),i=[...t.homeTeam.MIL||[],...t.homeTeam.ATT||[]].filter(M=>!M.used),r=t.phase==="attack"&&c&&i.length===0?[...t.homeTeam.GK||[],...t.homeTeam.DEF||[]].filter(M=>!M.used).map(M=>M.cardId):[];t.log[t.log.length-1];const f=t.phase==="ai-attack"||t.phase==="ai-defense",x=t.phase==="attack",g=t.phase==="defense",k=t.phase==="finished",o=t.gcCards.filter(M=>!t.usedGc.includes(M.id)),$=t.boostCard&&!t.boostUsed;e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column",e.innerHTML=`
  <style>
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    ${(()=>{const M=window.innerWidth>=700,Y=(R,H,S)=>{var De,Re;const j=(t.gcDefs||[]).find(E=>E.name===R.gc_type),U={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[j==null?void 0:j.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",ne={purple:"#b06ce0",light_blue:"#00d4ef"}[j==null?void 0:j.color]||"#b06ce0",ee=(j==null?void 0:j.name)||R.gc_type,_e=(j==null?void 0:j.effect)||((De=Ne[R.gc_type])==null?void 0:De.desc)||"",$e=j!=null&&j.image_url?`/manager-wars/icons/${j.image_url}`:null,Ee=((Re=Ne[R.gc_type])==null?void 0:Re.icon)||"⚡",Me=Math.round(S*.22),Ae=Math.round(S*.22),Te=S-Me-Ae,ke=ee.length>12?7:9;return`<div class="gc-mini" data-gc-id="${R.id}" data-gc-type="${R.gc_type}"
          style="box-sizing:border-box;width:${H}px;height:${S}px;border-radius:10px;border:2px solid ${ne};background:${U};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
          <div style="height:${Me}px;background:rgba(255,255,255,0.14);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 3px">
            <span style="font-size:${ke}px;font-weight:900;color:#fff;text-align:center;line-height:1.1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${H-6}px">${ee}</span>
        ...
        </div>`},ie=(R,H)=>{var S;return`<div id="boost-card"
          style="box-sizing:border-box;width:${R}px;height:${H}px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:2px solid #87CEEB;border-radius:10px;cursor:pointer;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:${Math.round(H*.04)}px;text-align:center;flex-shrink:0">
  ...
  `,document.body.appendChild(a);let r=!1;const f=()=>{r||(r=!0,a.remove(),n())};a.addEventListener("click",f),setTimeout(f,2e3)}function _t(e,t="rgba(0,0,0,0.8)"){const n=document.createElement("div");n.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,n.textContent=e,document.body.appendChild(n),setTimeout(()=>n.remove(),2200)}function Yt(e,t,n,a=null,p=null){var g,k;if(t.phase!=="attack"){_t("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){_t(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const d=Object.entries(t.homeTeam).flatMap(([o,$])=>($||[]).filter(L=>L.used).map(L=>({...L,_line:L._line||o}))),s=t.homeSubs.filter(o=>!t.usedSubIds.includes(o.cardId));if(!d.length){_t("Aucun joueur utilisé à remplacer");return}if(!s.length){_t("Aucun remplaçant disponible");return}let l=Math.max(0,d.findIndex(o=>o.cardId===p));const c=((g=d[l])==null?void 0:g._line)||((k=d[l])==null?void 0:k.job);let i=a?Math.max(0,s.findIndex(o=>o.cardId===a)):Math.max(0,s.findIndex(o=>o.job===c)),r=!1;const f=document.createElement("div");f.id="sub-overlay",f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function x(){var q,B,P,te,ce,re;const o=d[l],$=s[i],L=Math.min(130,Math.round((window.innerWidth-90)/2)),z=Math.round(L*1.35),F=M=>`background:rgba(255,255,255,0.12);border:none;color:${M?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${M?"default":"pointer"};flex-shrink:0`;f.innerHTML=`
    ...
    </div>`,(q=f.querySelector("#sub-close"))==null||q.addEventListener("click",()=>f.remove()),(B=f.querySelector("#out-up"))==null||B.addEventListener("click",()=>{l>0&&(l--,x())}),(P=f.querySelector("#out-down"))==null||P.addEventListener("click",()=>{l<d.length-1&&(l++,x())}),(te=f.querySelector("#in-up"))==null||te.addEventListener("click",()=>{i>0&&(i--,x())}),(ce=f.querySelector("#in-down"))==null||ce.addEventListener("click",()=>{i<s.length-1&&(i++,x())});const K=(M,Y,ie,D)=>{const Q=f.querySelector("#"+M);if(!Q)return;let ae=0;Q.addEventListener("touchstart",pe=>{ae=pe.touches[0].clientY},{passive:!0}),Q.addEventListener("touchend",pe=>{const he=pe.changedTouches[0].clientY-ae;if(Math.abs(he)<30)return;const xe=Y();he<0&&xe<D-1?(ie(xe+1),x()):he>0&&xe>0&&(ie(xe-1),x())},{passive:!0})};K("in-panel",()=>i,M=>i=M,s.length),K("out-panel",()=>l,M=>l=M,d.length),(re=f.querySelector("#sub-confirm"))==null||re.addEventListener("click",M=>{if(M.preventDefault(),M.stopPropagation(),r)return;r=!0;const Y=d[l],ie=s[i];if(!Y||!ie)return;let D=null,Q=-1;for(const[pe,he]of Object.entries(t.homeTeam)){const xe=(he||[]).findIndex(ge=>ge.cardId===Y.cardId);if(xe!==-1){D=pe,Q=xe;break}}if(Q===-1||!D){_t("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),f.remove();return}const ae={...ie,_line:D,_col:Y._col||0,used:!1,boost:0};t.homeTeam[D].splice(Q,1,ae),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(ie.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:Y.name,firstname:Y.firstname,note:we(Y,D),portrait:ze(Y),job:Y.job,country_code:Y.country_code,rarity:Y.rarity,clubName:Y.clubName,clubLogo:Y.clubLogo},inPlayer:{name:ie.name,firstname:ie.firstname,note:we(ie,D),portrait:ze(ie),job:ie.job,country_code:ie.country_code,rarity:ie.rarity,clubName:ie.clubName,clubLogo:ie.clubLogo},text:`🔄 ${ie.firstname} ${ie.name} remplace ${Y.firstname} ${Y.name}`}),f.remove(),ho(Y,ie,()=>Se(e,t,n))})}document.body.appendChild(f),x()}function bo(e,t,n,a,p){var k,o;const d=(a.gcDefs||[]).find($=>$.name===t),s=Ne[t]||{icon:"⚡",desc:"Carte spéciale."},l={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[d==null?void 0:d.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",c={purple:"#b06ce0",light_blue:"#00d4ef"}[d==null?void 0:d.color]||"#b06ce0",i=(d==null?void 0:d.name)||t,r=(d==null?void 0:d.effect)||s.desc,f=d!=null&&d.image_url?`/manager-wars/icons/${d.image_url}`:null,x=s.icon||"⚡",g=document.createElement("div");g.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",g.innerHTML=`
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(d);let l=!1;const c=()=>{l||(l=!0,d.remove(),setTimeout(()=>p(),50))};d.addEventListener("click",c),setTimeout(c,3500)}async function At(e,t,n){var r,f;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:a}=n,p=t.homeScore>t.aiScore,d=t.homeScore===t.aiScore,s=p?"victoire":d?"nul":"defaite",l=Tn(t.mode,s);t.matchId&&await _.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:p?a.profile.id:null,home_credits_reward:l,played_at:new Date().toISOString()}).eq("id",t.matchId);const c={credits:(a.profile.credits||0)+l,matches_played:(a.profile.matches_played||0)+1};p?c.wins=(a.profile.wins||0)+1:d?c.draws=(a.profile.draws||0)+1:c.losses=(a.profile.losses||0)+1,await _.from("users").update(c).eq("id",a.profile.id),await n.refreshProfile();const i=document.createElement("div");i.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",i.innerHTML=`
    ...
    </div>`,Be=v?"attack":w?"defense":"idle",Ge=`<div style="overflow:hidden;min-width:0;flex:1;min-height:0;display:flex;flex-direction:column" id="match-field">
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${dt(E,o[i+"Formation"],Be,A,300,300,ue)}
      </div>
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="width:min(90vw,420px)">${st(o[r+"Team"],o[r+"Formation"],null,[],300,300)}</div>
    </div>`,i==="p1"&&setTimeout(async()=>{await P({phase:"midfield"})},5e3)}let ae=!1;function pe(){if(ae)return;const E=o[i+"Team"].MIL||[],u=o[r+"Team"].MIL||[];function m(O){return O.reduce((V,Z)=>V+we(Z,"MIL"),0)}function h(O){let V=0;for(let Z=0;Z<O.length-1;Z++){const de=Ze(O[Z],O[Z+1]);de==="#00ff88"?V+=2:de==="#FFD700"&&(V+=1)}return V}const y=m(E)+h(E),b=m(u)+h(u),v=y>=b;function w(O,V,Z){return`<div id="duel-row-${Z}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
        ...
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${O.map((de,ue)=>{const fe=ue<O.length-1?Ze(de,O[ue+1]):null,me=!fe||fe==="#ff3333"||fe==="#cc2222",be=fe==="#00ff88"?"+2":fe==="#FFD700"?"+1":"";return`<div class="duel-card duel-card-${Z}" data-idx="${ue}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease,transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
              ${Ve({...de,note:we(de,"MIL"),_line:"MIL"},58,78)}
            </div>
            ${ue<O.length-1?`<div class="duel-link duel-link-${Z}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${me?"rgba(255,255,255,0.12)":fe};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${me?"none":`0 0 8px ${fe}`}">
              ${be?`<span style="position:absolute;top:-13px;left:50%;transform:translateX(-50%);font-size:8px;font-weight:900;color:${fe}">${be}</span>`:""}
            </div>`:""}`}).join("")}
        </div>
        <div class="duel-score-line duel-score-line-${Z}" style="margin-top:10px;font-size:12px;color:rgba(255,255,255,0.55);opacity:0;transition:opacity .4s ease">
          Score: ${m(O)} + ${h(O)} liens = <b style="color:#fff">${m(O)+h(O)}</b>
        </div>
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(b);let I=!1;const A=()=>{I||(I=!0,b.remove(),setTimeout(()=>y(),50))};b.addEventListener("click",A),setTimeout(A,3500)}function xe(E,u,m){var J,O;const h=(o.gcDefs||[]).find(V=>{var Z;return V.name===E||((Z=V.name)==null?void 0:Z.toLowerCase().trim())===(E==null?void 0:E.toLowerCase().trim())}),y={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[h==null?void 0:h.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",b={purple:"#b06ce0",light_blue:"#00d4ef"}[h==null?void 0:h.color]||"#b06ce0",v=(h==null?void 0:h.name)||E,w=(h==null?void 0:h.effect)||((J=Ne[E])==null?void 0:J.desc)||"",I=h!=null&&h.image_url?`/manager-wars/icons/${h.image_url}`:null,A=((O=Ne[E])==null?void 0:O.icon)||"⚡",C=u===i?"Vous":o[u+"Name"]||"Adversaire",N=document.createElement("div");N.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",N.innerHTML=`
      <style>
      ...
      </div>`,document.body.appendChild(m),(h=m.querySelector("#bp-close"))==null||h.addEventListener("click",()=>m.remove()),m.querySelectorAll(".bp-item").forEach(y=>{y.addEventListener("click",async()=>{const b=y.dataset.cid,v=u.find(I=>I.cardId===b);if(!v)return;const w=(E[v._line]||[]).find(I=>I.cardId===b);w&&(w.boost=(w.boost||0)+o.boostValue),m.remove(),await P({[i+"Team"]:E,boostUsed:!0})})})}function H(E=null){var W,le;if(!(o.phase===i+"-attack")){c("Remplacement uniquement avant votre attaque","warning");return}const m=o[i+"Team"],h=o["usedSubIds_"+i]||[],y=o.maxSubs||3;if(h.length>=y){c(`Maximum ${y} remplacements atteint`,"warning");return}const b=Object.entries(m).flatMap(([J,O])=>(O||[]).filter(V=>V.used).map(V=>({...V,_line:J}))),v=(o[i+"Subs"]||[]).filter(J=>!h.includes(J.cardId));if(!b.length){c("Aucun joueur utilisé à remplacer","warning");return}if(!v.length){c("Aucun remplaçant disponible","warning");return}let w=Math.max(0,b.findIndex(J=>J.cardId===E));const I=((W=b[w])==null?void 0:W._line)||((le=b[w])==null?void 0:le.job);let A=Math.max(0,v.findIndex(J=>J.job===I)),T=!1;const C=document.createElement("div");C.id="pvp-sub-overlay",C.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function N(){var fe,me,be,Ce,Pe,qe;const J=b[w],O=v[A],V=Math.min(130,Math.round((window.innerWidth-90)/2)),Z=Math.round(V*1.35),de=Ie=>`background:rgba(255,255,255,0.12);border:none;color:${Ie?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Ie?"default":"pointer"};flex-shrink:0`;C.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        <div id="pin-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
          ...
          <div>${O?Ve({...O,used:!1,boost:0},V,Z):"<div>—</div>"}</div>
      ...
      </div>`,(fe=C.querySelector("#psub-close"))==null||fe.addEventListener("click",()=>C.remove()),(me=C.querySelector("#pout-up"))==null||me.addEventListener("click",()=>{w>0&&(w--,N())}),(be=C.querySelector("#pout-down"))==null||be.addEventListener("click",()=>{w<b.length-1&&(w++,N())}),(Ce=C.querySelector("#pin-up"))==null||Ce.addEventListener("click",()=>{A>0&&(A--,N())}),(Pe=C.querySelector("#pin-down"))==null||Pe.addEventListener("click",()=>{A<v.length-1&&(A++,N())});const ue=(Ie,Le,Be,Ge)=>{const Oe=C.querySelector("#"+Ie);if(!Oe)return;let Ue=0;Oe.addEventListener("touchstart",Ke=>{Ue=Ke.touches[0].clientY},{passive:!0}),Oe.addEventListener("touchend",Ke=>{const je=Ke.changedTouches[0].clientY-Ue;if(Math.abs(je)<30)return;const Fe=Le();je<0&&Fe<Ge-1?(Be(Fe+1),N()):je>0&&Fe>0&&(Be(Fe-1),N())},{passive:!0})};ue("pin-panel",()=>A,Ie=>A=Ie,v.length),ue("pout-panel",()=>w,Ie=>w=Ie,b.length),(qe=C.querySelector("#psub-confirm"))==null||qe.addEventListener("click",async Ie=>{if(Ie.preventDefault(),Ie.stopPropagation(),T)return;T=!0;const Le=b[w],Be=v[A];if(!Le||!Be)return;const Ge=Le._line,Oe=(m[Ge]||[]).findIndex(je=>je.cardId===Le.cardId);if(Oe===-1){c("Erreur : joueur introuvable","error"),C.remove();return}const Ue={...Be,_line:Ge,position:Le.position,used:!1,boost:0};m[Ge].splice(Oe,1,Ue);const Ke=[...h,Be.cardId];C.remove(),S(Le,Be,async()=>{await P({[i+"Team"]:m,[r+"Team"]:o[r+"Team"],["usedSubIds_"+i]:Ke})})})}document.body.appendChild(C),N()}function S(E,u,m){const h={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},y=document.createElement("div");y.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const b=(I,A,T)=>`<div style="text-align:center">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:6px">Appuyer pour continuer</div>`,document.body.appendChild(y);let v=!1;const w=()=>{v||(v=!0,y.remove(),setTimeout(()=>m(),50))};y.addEventListener("click",w),setTimeout(w,2200)}function j(){var u;const E=document.createElement("div");E.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:20px;overflow-y:auto",E.innerHTML=`
      ...
      </div>`,document.body.appendChild(u),(m=u.querySelector("#pvp-hist-close"))==null||m.addEventListener("click",()=>u.remove())}async function ne(){if(o.phase!==i+"-attack")return;const E=i==="p1"?"p2":"p1",u=(o.round||0)+1,m=[...o.log||[]];m.push({type:"info",text:`⏭️ ${o[i+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const h=Te(o),y=Ae(E),b=h||!y?"finished":E+"-attack";await P({["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},pendingAttack:null,phase:b,attacker:E,round:u,log:m}),b==="finished"&&await De(o)}async function ee(){const E=o[i+"Team"],u=!["GK","DEF","MIL","ATT"].some(b=>(o[r+"Team"][b]||[]).some(v=>!v.used)),m=(o["selected_"+i]||[]).map(b=>{const v=(E[b._role]||[]).find(N=>N.cardId===b.cardId)||b,w=u&&["GK","DEF"].includes(b._role),I=E[b._role]||[],A=I.findIndex(N=>N.cardId===b.cardId),T=ot(I.length),C=A>=0?T[A]:v._col??1;return{...v,_line:b._role,_col:C,...w?{note_a:Math.max(1,Number(v.note_a)||0)}:{}}});if(!m.length)return;const h=Rt(m,o.modifiers[i]||{});Y(i,m.map(b=>b.cardId)),m.forEach(b=>{const v=(E[b._role]||[]).find(w=>w.cardId===b.cardId);v&&(v.used=!0)}),o["selected_"+i]=[],D();const y=[...o.log||[]];if(u){const b=(o[i+"Score"]||0)+1,v=m.map(C=>({name:C.name,note:we(C,C._line||"ATT"),portrait:ze(C),job:C.job}));y.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:v,homeTotal:h.total,aiTotal:0});const w=(o.round||0)+1,I=i==="p1"?"p2":"p1",A={...o,[i+"Team"]:E,[i+"Score"]:b},T=Te(A);F.add(w),he(v,b,o[r+"Score"]||0,!0,async()=>{await P({[i+"Team"]:E,[i+"Score"]:b,["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},pendingAttack:null,phase:T?"finished":I+"-attack",attacker:I,round:w,log:y}),T&&await De({...o,[i+"Score"]:b})});return}y.push({type:"pending",text:`⚔️ ${o[i+"Name"]} attaque (${h.total})`}),await P({[i+"Team"]:E,[r+"Team"]:o[r+"Team"],pendingAttack:{...h,players:m,side:i},["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},phase:r+"-defense",log:y})}async function _e(){const E=o[i+"Team"],u=(o["selected_"+i]||[]).map(J=>{const O=(E[J._role]||[]).find(fe=>fe.cardId===J.cardId)||J,V=E[J._role]||[],Z=V.findIndex(fe=>fe.cardId===J.cardId),de=ot(V.length),ue=Z>=0?de[Z]:O._col??1;return{...O,_line:J._role,_col:ue}}),m=Ot(u,o.modifiers[i]||{});Y(i,u.map(J=>J.cardId)),u.forEach(J=>{const O=(E[J._role]||[]).find(V=>V.cardId===J.cardId);O&&(O.used=!0)}),o["selected_"+i]=[],D();const h=Ht(o.pendingAttack.total,m.total,o.modifiers[i]||{}),y=o.pendingAttack.side,b=h==="attack"||(h==null?void 0:h.goal),v=y==="p1"?"p2":"p1",w=(o.round||0)+1,I=(o.pendingAttack.players||[]).map(J=>({name:J.name,note:we(J,J._line||"ATT"),portrait:ze(J),job:J.job})),A=[...o.log||[]];A.push({type:"duel",isGoal:b,homeScored:b&&y===i,text:b?`⚽ BUT de ${o[y+"Name"]} ! (${o.pendingAttack.total} vs ${m.total})`:`✋ Attaque stoppée (${o.pendingAttack.total} vs ${m.total})`,homePlayers:I,aiPlayers:u.map(J=>({name:J.name,note:we(J,J._line||"DEF"),portrait:ze(J),job:J.job})),homeTotal:o.pendingAttack.total,aiTotal:m.total});const T=b?(o[y+"Score"]||0)+1:o[y+"Score"]||0,C={...o,[i+"Team"]:E,[y+"Score"]:T},N=Te(C),W=N?"finished":v+"-attack",le=async()=>{await P({[i+"Team"]:E,[r+"Team"]:o[r+"Team"],[y+"Score"]:T,["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},pendingAttack:null,phase:W,attacker:v,round:w,log:A}),(W==="finished"||N)&&await De({...o,[y+"Score"]:T})};if(b){const J=y===i,O=J?T:o[i+"Score"]||0,V=J?o[r+"Score"]||0:T;F.add(w),he(I,O,V,J,le)}else await le()}function $e(E){return["MIL","ATT"].some(u=>(E[u]||[]).some(m=>!m.used))}function Ee(E){return["GK","DEF","MIL","ATT"].some(u=>(E[u]||[]).some(m=>!m.used))}function Me(E){return Ee(E)&&!$e(E)}function Ae(E){const u=o[E+"Team"],m=o[(E==="p1"?"p2":"p1")+"Team"];return!!($e(u)||!Ee(m)&&Me(u))}function Te(E){const u=["MIL","ATT"].some(A=>(E.p1Team[A]||[]).some(T=>!T.used)),m=["MIL","ATT"].some(A=>(E.p2Team[A]||[]).some(T=>!T.used)),h=Ee(E.p1Team),y=Ee(E.p2Team);return!u&&!(!y&&h)&&(!m&&!(!h&&y))}function ke(E){const u=E.p1Score||0,m=E.p2Score||0;return u===m?null:u>m?g.home_id:g.away_id}async function De(E){try{const u=ke(E),m=u?g.home_id===u?g.away_id:g.home_id:null;await _.from("matches").update({status:"finished",winner_id:u,home_score:E.p1Score||0,away_score:E.p2Score||0}).eq("id",n),u&&m&&Ni(u,m).catch(()=>{})}catch(u){console.error("[PvP] finishMatch:",u)}}function Re(){var y;const E=o[i+"Score"],u=o[r+"Score"],m=E>u,h=E===u;Xe(e),e.innerHTML=`
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(v);let A=!1;const T=()=>{A||(A=!0,v.remove(),setTimeout(()=>b(),50))};v.addEventListener("click",T),setTimeout(T,3500)}function ge(u,m,h){var O,V;const y=(o.gcDefs||[]).find(Z=>{var de;return Z.name===u||((de=Z.name)==null?void 0:de.toLowerCase().trim())===(u==null?void 0:u.toLowerCase().trim())}),b={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[y==null?void 0:y.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",v={purple:"#b06ce0",light_blue:"#00d4ef"}[y==null?void 0:y.color]||"#b06ce0",w=(y==null?void 0:y.name)||u,I=(y==null?void 0:y.effect)||((O=Ne[u])==null?void 0:O.desc)||"",A=y!=null&&y.image_url?`/manager-wars/icons/${y.image_url}`:null,T=((V=Ne[u])==null?void 0:V.icon)||"⚡",N=m===i?"Vous":o[m+"Name"]||"Adversaire",W=document.createElement("div");W.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",W.innerHTML=`
      <style>
        @keyframes gcFlipIn{0%{transform:perspective(800px) rotateY(90deg) scale(.7);opacity:0}55%{transform:perspective(800px) rotateY(-12deg) scale(1.08);opacity:1}100%{transform:perspective(800px) rotateY(0) scale(1);opacity:1}}
      ...
      </div>`,document.body.appendChild(h),(y=h.querySelector("#bp-close"))==null||y.addEventListener("click",()=>h.remove()),h.querySelectorAll(".bp-item").forEach(b=>{b.addEventListener("click",async()=>{const v=b.dataset.cid,w=m.find(A=>A.cardId===v);if(!w)return;const I=(u[w._line]||[]).find(A=>A.cardId===v);I&&(I.boost=(I.boost||0)+o.boostValue),h.remove(),await te({[i+"Team"]:u,boostUsed:!0})})})}function S(u=null){var le,J;if(!(o.phase===i+"-attack")){c("Remplacement uniquement avant votre attaque","warning");return}const h=o[i+"Team"],y=o["usedSubIds_"+i]||[],b=o.maxSubs||3;if(y.length>=b){c(`Maximum ${b} remplacements atteint`,"warning");return}const v=Object.entries(h).flatMap(([O,V])=>(V||[]).filter(Z=>Z.used).map(Z=>({...Z,_line:O}))),w=(o[i+"Subs"]||[]).filter(O=>!y.includes(O.cardId));if(!v.length){c("Aucun joueur utilisé à remplacer","warning");return}if(!w.length){c("Aucun remplaçant disponible","warning");return}let I=Math.max(0,v.findIndex(O=>O.cardId===u));const A=((le=v[I])==null?void 0:le._line)||((J=v[I])==null?void 0:J.job);let T=Math.max(0,w.findIndex(O=>O.job===A)),C=!1;const N=document.createElement("div");N.id="pvp-sub-overlay",N.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function W(){var me,be,Ce,Pe,qe,Ie;const O=v[I],V=w[T],Z=Math.min(130,Math.round((window.innerWidth-90)/2)),de=Math.round(Z*1.35),ue=Le=>`background:rgba(255,255,255,0.12);border:none;color:${Le?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Le?"default":"pointer"};flex-shrink:0`;N.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        ...
        <div id="pout-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
          ...
          <div>${O?Ve({...O,used:!1,boost:0},Z,de):"<div>—</div>"}</div>
      ...
      <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
        <button id="psub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
      </div>`,(me=N.querySelector("#psub-close"))==null||me.addEventListener("click",()=>N.remove()),(be=N.querySelector("#pout-up"))==null||be.addEventListener("click",()=>{I>0&&(I--,W())}),(Ce=N.querySelector("#pout-down"))==null||Ce.addEventListener("click",()=>{I<v.length-1&&(I++,W())}),(Pe=N.querySelector("#pin-up"))==null||Pe.addEventListener("click",()=>{T>0&&(T--,W())}),(qe=N.querySelector("#pin-down"))==null||qe.addEventListener("click",()=>{T<w.length-1&&(T++,W())});const fe=(Le,Be,Ge,Oe)=>{const Ue=N.querySelector("#"+Le);if(!Ue)return;let Ke=0;Ue.addEventListener("touchstart",je=>{Ke=je.touches[0].clientY},{passive:!0}),Ue.addEventListener("touchend",je=>{const Fe=je.changedTouches[0].clientY-Ke;if(Math.abs(Fe)<30)return;const Qe=Be();Fe<0&&Qe<Oe-1?(Ge(Qe+1),W()):Fe>0&&Qe>0&&(Ge(Qe-1),W())},{passive:!0})};fe("pin-panel",()=>T,Le=>T=Le,w.length),fe("pout-panel",()=>I,Le=>I=Le,v.length),(Ie=N.querySelector("#psub-confirm"))==null||Ie.addEventListener("click",async Le=>{if(Le.preventDefault(),Le.stopPropagation(),C)return;C=!0;const Be=v[I],Ge=w[T];if(!Be||!Ge)return;const Oe=Be._line,Ue=(h[Oe]||[]).findIndex(Fe=>Fe.cardId===Be.cardId);if(Ue===-1){c("Erreur : joueur introuvable","error"),N.remove();return}const Ke={...Ge,_line:Oe,position:Be.position,used:!1,boost:0};h[Oe].splice(Ue,1,Ke);const je=[...y,Ge.cardId];N.remove(),j(Be,Ge,async()=>{await te({[i+"Team"]:h,[r+"Team"]:o[r+"Team"],["usedSubIds_"+i]:je})})})}document.body.appendChild(N),W()}function j(u,m,h){const y={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const v=(A,T,C)=>`<div style="text-align:center">
      ...
      <div style="font-size:18px;font-weight:900;color:#ff6b6b">${o[r+"Name"]}</div>
      <div style="width:min(90vw,420px)">${st(o[r+"Team"],o[r+"Formation"],null,[],300,300)}</div>
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(u),(m=u.querySelector("#pvp-opp-close"))==null||m.addEventListener("click",()=>u.remove())}function ne(){var h;const u=o.log||[],m=document.createElement("div");m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column",m.innerHTML=`
      <div style="display:flex;align-items:center;padding:14px 16px;border-bottom:1px solid rgba(255,255,255,0.1);flex-shrink:0">
        <div style="flex:1;font-size:14px;font-weight:700;color:#fff">📋 Historique</div>
        <button id="pvp-hist-close" style="background:none;border:none;color:rgba(255,255,255,0.6);font-size:20px;cursor:pointer">✕</button>
      </div>
      ...
      </div>`,document.body.appendChild(m),(h=m.querySelector("#pvp-hist-close"))==null||h.addEventListener("click",()=>m.remove())}async function ee(){if(o.phase!==i+"-attack")return;const u=i==="p1"?"p2":"p1",m=(o.round||0)+1,h=[...o.log||[]];h.push({type:"info",text:`⏭️ ${o[i+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const y=ke(o),b=Te(u),v=y||!b?"finished":u+"-attack";await te({["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},pendingAttack:null,phase:v,attacker:u,round:m,log:h}),v==="finished"&&await Re(o)}async function _e(){const u=o[i+"Team"],m=!["GK","DEF","MIL","ATT"].some(v=>(o[r+"Team"][v]||[]).some(w=>!w.used)),h=(o["selected_"+i]||[]).map(v=>{const w=(u[v._role]||[]).find(W=>W.cardId===v.cardId)||v,I=m&&["GK","DEF"].includes(v._role),A=u[v._role]||[],T=A.findIndex(W=>W.cardId===v.cardId),C=ot(A.length),N=T>=0?C[T]:w._col??1;return{...w,_line:v._role,_col:N,...I?{note_a:Math.max(1,Number(w.note_a)||0)}:{}}});if(!h.length)return;const y=Rt(h,o.modifiers[i]||{});ie(i,h.map(v=>v.cardId)),h.forEach(v=>{const w=(u[v._role]||[]).find(I=>I.cardId===v.cardId);w&&(w.used=!0)}),o["selected_"+i]=[],Q();const b=[...o.log||[]];if(m){const v=(o[i+"Score"]||0)+1,w=h.map(N=>({name:N.name,note:we(N,N._line||"ATT"),portrait:ze(N),job:N.job}));b.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:w,homeTotal:y.total,aiTotal:0});const I=(o.round||0)+1,A=i==="p1"?"p2":"p1",T={...o,[i+"Team"]:u,[i+"Score"]:v},C=ke(T);K.add(I),xe(w,v,o[r+"Score"]||0,!0,async()=>{await te({[i+"Team"]:u,[i+"Score"]:v,["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},pendingAttack:null,phase:C?"finished":A+"-attack",attacker:A,round:I,log:b}),C&&await Re({...o,[i+"Score"]:v})});return}b.push({type:"pending",text:`⚔️ ${o[i+"Name"]} attaque (${y.total})`}),await te({[i+"Team"]:u,[r+"Team"]:o[r+"Team"],pendingAttack:{...y,players:h,side:i},["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},phase:r+"-defense",log:b})}async function $e(){const u=o[i+"Team"],m=(o["selected_"+i]||[]).map(O=>{const V=(u[O._role]||[]).find(me=>me.cardId===O.cardId)||O,Z=u[O._role]||[],de=Z.findIndex(me=>me.cardId===O.cardId),ue=ot(Z.length),fe=de>=0?ue[de]:V._col??1;return{...V,_line:O._role,_col:fe}}),h=Ot(m,o.modifiers[i]||{});ie(i,m.map(O=>O.cardId)),m.forEach(O=>{const V=(u[O._role]||[]).find(Z=>Z.cardId===O.cardId);V&&(V.used=!0)}),o["selected_"+i]=[],Q();const y=Ht(o.pendingAttack.total,h.total,o.modifiers[i]||{}),b=o.pendingAttack.side,v=y==="attack"||(y==null?void 0:y.goal),w=b==="p1"?"p2":"p1",I=(o.round||0)+1,A=(o.pendingAttack.players||[]).map(O=>({name:O.name,note:we(O,O._line||"ATT"),portrait:ze(O),job:O.job})),T=[...o.log||[]];T.push({type:"duel",isGoal:v,homeScored:v&&b===i,text:v?`⚽ BUT de ${o[b+"Name"]} ! (${o.pendingAttack.total} vs ${h.total})`:`✋ Attaque stoppée (${o.pendingAttack.total} vs ${h.total})`,homePlayers:A,aiPlayers:m.map(O=>({name:O.name,note:we(O,O._line||"DEF"),portrait:ze(O),job:O.job})),homeTotal:o.pendingAttack.total,aiTotal:h.total});const C=v?(o[b+"Score"]||0)+1:o[b+"Score"]||0,N={...o,[i+"Team"]:u,[b+"Score"]:C},W=ke(N),le=W?"finished":w+"-attack",J=async()=>{await te({[i+"Team"]:u,[r+"Team"]:o[r+"Team"],[b+"Score"]:C,["selected_"+i]:[],modifiers:{...o.modifiers,[i]:{}},pendingAttack:null,phase:le,attacker:w,round:I,log:T}),(le==="finished"||W)&&await Re({...o,[b+"Score"]:C})};if(v){const O=b===i,V=O?C:o[i+"Score"]||0,Z=O?o[r+"Score"]||0:C;K.add(I),xe(A,V,Z,O,J)}else await J()}function Ee(u){return["MIL","ATT"].some(m=>(u[m]||[]).some(h=>!h.used))}function Me(u){return["GK","DEF","MIL","ATT"].some(m=>(u[m]||[]).some(h=>!h.used))}function Ae(u){return Me(u)&&!Ee(u)}function Te(u){const m=o[u+"Team"],h=o[(u==="p1"?"p2":"p1")+"Team"];return!!(Ee(m)||!Me(h)&&Ae(m))}function ke(u){const m=["MIL","ATT"].some(T=>(u.p1Team[T]||[]).some(C=>!C.used)),h=["MIL","ATT"].some(T=>(u.p2Team[T]||[]).some(C=>!C.used)),y=Me(u.p1Team),b=Me(u.p2Team);return!m&&!(!b&&y)&&(!h&&!(!y&&b))}function De(u){const m=u.p1Score||0,h=u.p2Score||0;return m===h?null:m>h?g.home_id:g.away_id}async function Re(u){try{const m=De(u),h=m?g.home_id===m?g.away_id:g.home_id:null;await _.from("matches").update({status:"finished",winner_id:m,home_score:u.p1Score||0,away_score:u.p2Score||0}).eq("id",n),m&&h&&Ni(m,h).catch(()=>{})}catch(m){console.error("[PvP] finishMatch:",m)}}function E(){var b;const u=o[i+"Score"],m=o[r+"Score"],h=u>m,y=u===m;Xe(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
  ...
  `;function c(i){const r=document.getElementById("mkt-content"),f=i==="buy"?s:l;if(f.length===0){r.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      ${x.map(g=>{var F,K,q,B;const k=(F=g.card)==null?void 0:F.player;if(!k)return"";const o=k.job==="GK"?k.note_g:k.job==="DEF"?k.note_d:k.job==="MIL"?k.note_m:k.note_a,$=zo[k.rarity],L=(n.profile.credits||0)>=g.price,z=g.status==="sold";return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${z?"opacity:0.65":""}">
          ...
          <div style="flex:1;min-width:0">
            ...
            ${i==="buy"?`<div style="font-size:11px;color:var(--gray-600)">Vendeur : ${((q=g.seller)==null?void 0:q.pseudo)||"—"}</div>`:z?`<div style="font-size:11px;color:var(--green)">✅ Vendu à ${((B=g.buyer)==null?void 0:B.pseudo)||"—"} · ${g.sold_at?new Date(g.sold_at).toLocaleDateString("fr"):""}</div>`:`<div style="font-size:11px;color:var(--gray-600)">🟢 En vente depuis le ${new Date(g.listed_at).toLocaleDateString("fr")}</div>`}
          </div>
          <div style="text-align:right;flex-shrink:0">
