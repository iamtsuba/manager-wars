import{s as $,F as it,b as Ce,c as Gt,l as ge,d as di}from"./formation-links-Bt44rMnM.js";function kt(e,{navigate:t,toast:i}){e.innerHTML=`
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),n=(d||[]).filter(_=>_.card_type==="player"&&_.player),r=(d||[]).filter(_=>_.card_type==="game_changer"),p=(d||[]).filter(_=>_.card_type==="formation"),{data:g}=await $.from("gc_definitions").select("name,gc_type,color,effect,image_url"),f={};(g||[]).forEach(_=>{f[_.name]=_});const y=Object.keys(it),u=Object.keys(de),h={};n.forEach(_=>{const b=_.player.id;h[b]=(h[b]||0)+1}),new Set(Object.keys(h).map(_=>String(_)));const L=new Set(p.map(_=>_.formation)),v=new Set(r.map(_=>_.gc_type));let x="player",k="Tous",B="",m=!1;function E(){return[...n].sort((_,b)=>{const C=Be.indexOf(_.player.job),A=Be.indexOf(b.player.job);return C!==A?C-A:(_.player.surname_encoded||"").localeCompare(b.player.surname_encoded||"")})}function M(){return[...l||[]].sort((_,b)=>{const C=Be.indexOf(_.job),A=Be.indexOf(b.job);return C!==A?C-A:(_.surname_encoded||"").localeCompare(b.surname_encoded||"")})}function G(){return E().filter(_=>{const b=_.player,C=k==="Tous"||b.job===k,A=!B||`${b.firstname} ${b.surname_encoded}`.toLowerCase().includes(B);return C&&A})}function D(){return M().filter(_=>{const b=k==="Tous"||_.job===k,C=!B||`${_.firstname} ${_.surname_encoded}`.toLowerCase().includes(B);return b&&C})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",b=>{B=b.target.value.toLowerCase(),U()}),e.querySelectorAll(".filter-btn").forEach(b=>{b.addEventListener("click",()=>{k=b.dataset.job,P(),U()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{m=!m,P(),U()})):(_.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{m=!m,P(),U()})))}function U(){const _=document.getElementById("col-grid");_&&(x==="player"?F(_):x==="formation"?q(_):R(_))}function Y(_,b,C,A,w){w=w||"#7a28b8";const S=document.getElementById("col-grid"),T=document.getElementById("col-big");if(!S||!T)return;var N=0;function j(){T.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+b(_[N])+"</div>";var H=T.querySelector("[data-card-id],[data-form-id],[data-gc-id]");H&&H.addEventListener("click",function(){A(_[N])}),requestAnimationFrame(function(){var K=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!K||!T)){var J=T.clientHeight-8,ie=T.clientWidth-24,re=K.offsetHeight,ae=K.offsetWidth;if(re>0&&ae>0&&J>40){var se=Math.min(J/re,ie/ae,1);K.style.transform="scale("+se.toFixed(3)+")",K.style.transformOrigin="top center"}}}),S.innerHTML=_.map(function(K,J){return'<div class="col-mini-item" data-idx="'+J+'" style="flex-shrink:0;cursor:pointer;border-radius:8px;border:1.5px solid '+(J===N?w:"transparent")+';transition:border-color .15s;overflow:hidden">'+C(K,J===N)+"</div>"}).join(""),S.querySelectorAll(".col-mini-item").forEach(function(K){K.addEventListener("click",function(){N=Number(K.dataset.idx),j(),K.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}j()}function V(_){var b=.54,C=Math.round(140*b),A=Math.round(310*b),w;if(!_||_._fake){var S=_?_.player:null;if(!S)return"";w=Tt(S)}else w=We(_,"");return'<div style="width:'+C+"px;height:"+A+'px;overflow:hidden;position:relative;flex-shrink:0"><div style="transform:scale('+b+');transform-origin:top left;position:absolute;top:0;left:0;pointer-events:none">'+w+"</div></div>"}function ee(_,b,C){b=b||100,C=C||140;var A=Ce[_]||{},w={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},S=Math.max(3,Math.round(b*.06)),T=Object.entries(A).map(function(j){var H=j[0],K=j[1],J=H.replace(/\d+$/,""),ie=w[J]||"#888",re=Math.round(K.x*b),ae=Math.round(K.y*C);return'<circle cx="'+re+'" cy="'+ae+'" r="'+S+'" fill="'+ie+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),N=Math.max(1,Math.round(b/50));return'<svg viewBox="0 0 '+b+" "+C+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+b+'" height="'+C+'" fill="#1A6B3C"/><rect x="'+Math.round(b*.2)+'" y="'+Math.round(C*.02)+'" width="'+Math.round(b*.6)+'" height="'+Math.round(C*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+N+'"/><line x1="0" y1="'+Math.round(C*.5)+'" x2="'+b+'" y2="'+Math.round(C*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+N+'"/><ellipse cx="'+Math.round(b*.5)+'" cy="'+Math.round(C*.5)+'" rx="'+Math.round(b*.18)+'" ry="'+Math.round(C*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+N+'"/><rect x="'+Math.round(b*.2)+'" y="'+Math.round(C*.82)+'" width="'+Math.round(b*.6)+'" height="'+Math.round(C*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+N+'"/>'+T+"</svg>"}function I(_,b,C){var A=C>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+C+"</div>":"",w=b?'data-form-id="'+b.id+'"':"",S=_.length>7?14:_.length>5?16:19,T=!!b;return"<div "+w+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(T?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(T?"":"filter:grayscale(1);opacity:0.5")+'">'+A+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(T?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+S+"px;font-weight:900;color:"+(T?"#1A6B3C":"#aaa")+';line-height:1">'+_+'</div></div><div style="flex:1;overflow:hidden;background:'+(T?"#1A6B3C":"#ccc")+'">'+ee(_,140,220)+"</div></div>"}function z(_,b){var C=.54,A=Math.round(140*C),w=Math.round(305*C),S=Math.round(w*.22),T=w-S,N=_.length>7?5:7,j=ee(_,A,T),H=b?"1.5px solid #2a7a40":"1px solid #ddd",K=b?"":"filter:grayscale(1);opacity:0.45;",J=b?"#1A6B3C":"#bbb",ie="#fff";return'<div style="width:'+A+"px;height:"+w+"px;border-radius:6px;border:"+H+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+K+'"><div style="height:'+S+"px;background:"+J+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+N+"px;font-weight:900;color:"+ie+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(A-4)+'px">'+_+'</span></div><div style="height:'+T+'px;overflow:hidden;flex-shrink:0">'+j+"</div></div>"}function F(_){if(m){const b=D();if(!b.length){_.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const C=b.map(A=>n.find(w=>w.player.id===A.id)||{_fake:!0,player:A,id:"fake-"+A.id});Y(C,A=>A._fake?Tt(A.player):We(A,""),A=>A._fake?V({player:A.player,id:"x",_fake:!0}):V(A),A=>{A._fake||Lt(A,n,h,t)},"#1A6B3C")}else{const b=G();if(!b.length){_.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const C={},A=[];b.forEach(w=>{C[w.player.id]||(C[w.player.id]=!0,A.push(w))}),Y(A,w=>{const S=h[w.player.id]||1,T=S>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${S}</div>`:"",j=n.filter(H=>H.player.id===w.player.id&&H.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return We(w,T+j)},w=>V(w),w=>Lt(w,n,h,t),"#1A6B3C")}}function q(_){const b=m?y:[...L];if(!b.length){_.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const C=b.map(A=>({formation:A,card:p.find(w=>w.formation===A)||null,owned:L.has(A)}));Y(C,({formation:A,card:w,owned:S})=>I(A,S?w:null,S?p.filter(T=>T.formation===A).length:0),({formation:A,owned:w})=>z(A,w),({card:A,owned:w})=>{w&&A&&hi(A,p,t,o)},"#1A6B3C")}function R(_){const b=Object.keys(f),C=m?b.length?b:u:[...v];if(!C.length){_.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const A=C.map(w=>({type:w,gc:de[w]||{icon:"⚡",desc:""},def:f[w]||null,owned:v.has(w),card:r.find(S=>S.gc_type===w)||null}));Y(A,({type:w,gc:S,def:T,owned:N,card:j})=>{const H=N?r.filter(Se=>Se.gc_type===w).length:0,K=H>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${H}</div>`:"",J=(T==null?void 0:T.gc_type)==="ultra_game_changer",ie={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},re={purple:"#b06ce0",light_blue:"#00d4ef"},ae=ie[T==null?void 0:T.color]||ie.purple,se=re[T==null?void 0:T.color]||re.purple,xe=(T==null?void 0:T.effect)||S.desc||"",me=T!=null&&T.image_url?`/manager-wars/icons/${T.image_url}`:null;return N&&j?`<div data-gc-id="${j.id}" data-gc-type="${w}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${se};background:${ae};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${se}66;cursor:pointer">
          ${K}
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Ee=1e3;function hi(e,t,i,s){var u,h,L;const{state:c,toast:o,closeModal:a,navigate:d,refreshProfile:l}=i,n=e.formation,r={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function p(){const v=Ce[n]||{},x=it[n]||[],k=290,B=360,m=20;function E(G){const D=v[G];return D?{x:D.x*k,y:D.y*B}:null}let M=`<svg width="${k}" height="${B}" viewBox="0 0 ${k} ${B}" xmlns="http://www.w3.org/2000/svg">`;for(const[G,D]of x){const P=E(G),U=E(D);!P||!U||(M+=`<line x1="${P.x}" y1="${P.y}" x2="${U.x}" y2="${U.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const G of Object.keys(v)){const D=E(G);if(!D)continue;const P=G.replace(/\d+/,""),U=r[P]||"#555";M+=`<circle cx="${D.x}" cy="${D.y}" r="${m}" fill="${U}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,M+=`<text x="${D.x}" y="${D.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${P}</text>`}return M+="</svg>",M}const g=t.filter(v=>v.formation===n),f=g.length,y=!e.is_for_sale;s(`Formation ${n}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}const Ui="simulation",Vt="VOTRE_ZONE_ID";function Ki(){switch(Ui){case"propellerads":return Vi();case"adsense":return Wi();default:return st()}}function st(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:#111;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:14px";let i=5;t.innerHTML=`
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order");let l=0;function n(){var v,x,k,B,m;const r=o[l],p=(d||[]).filter(E=>E.deck_id===r.id),g=p.filter(E=>{var M;return E.is_starter&&((M=E.card)==null?void 0:M.player)}).map(E=>Xe(E.card,E.position)),f=p.find(E=>{var M;return((M=E.card)==null?void 0:M.card_type)==="formation"}),y=r.formation||((v=f==null?void 0:f.card)==null?void 0:v.formation)||"4-4-2",u=g.length>=11?Oe(g,y):null,h=g.length>=11;Yt(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;height:${p}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ...
      ${c?`<img src="${c}" style="width:${p-4}px;height:${p-4}px;object-fit:contain">`:e!=null&&e.clubName?`<span style="font-size:${Math.max(4,p-8)}px;font-weight:700;color:#333">${(e.clubName||"").slice(0,3).toUpperCase()}</span>`:""}
        ...
        stroke="${M}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const g=48,f=64,y=13,u=16,h={normal:"#aaaaaa",pépite:"#D4A017",papyte:"#111111",légende:"#7a28b8"};for(const[v,x]of Object.entries(l)){const k=r(v);if(!k||!x)continue;const B=v.replace(/[0-9]/g,""),m=ke[B]||"#555",E=i==="attack"&&["MIL","ATT"].includes(B)&&!x.used||i==="defense"&&["GK","DEF","MIL"].includes(B)&&!x.used,M=s.includes(x.cardId);let G;i==="attack"?G=B==="MIL"?Number(x.note_m)||0:Number(x.note_a)||0:i==="defense"?G=B==="GK"?Number(x.note_g)||0:B==="MIL"?Number(x.note_m)||0:Number(x.note_d)||0:G=Number(B==="GK"?x.note_g:B==="DEF"?x.note_d:B==="MIL"?x.note_m:x.note_a)||0,G=G+(x.boost||0);const D=(k.x-g/2).toFixed(1),P=(k.y-f/2).toFixed(1),U=x.used?.25:1,Y=h[x==null?void 0:x.rarity]||h.normal,V=M?"#ff3030":Y,ee=M?3:(x==null?void 0:x.rarity)==="légende"||(x==null?void 0:x.rarity)==="pépite"?2.5:2,I=f-y-u;p+=`<defs><clipPath id="cp-${v}"><rect x="${D}" y="${(k.y-f/2+y).toFixed(1)}" width="${g}" height="${I}"/></clipPath></defs>`,p+=`<rect x="${D}" y="${P}" width="${g}" height="${f}" rx="5" fill="${m}" opacity="${U}"/>`;const z=oe(x);z&&!x.used&&(p+=`<image href="${z}" x="${D}" y="${(k.y-f/2+y).toFixed(1)}" width="${g}" height="${I}" clip-path="url(#cp-${v})" preserveAspectRatio="xMidYMin slice"/>`),p+=`<rect x="${D}" y="${P}" width="${g}" height="${y}" rx="4" fill="rgba(255,255,255,0.92)"/>`,p+=`<text x="${k.x.toFixed(1)}" y="${(k.y-f/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(x.name||"").slice(0,9)}</text>`;const F=(k.y+f/2-u).toFixed(1);if(p+=`<rect x="${D}" y="${F}" width="${g}" height="${u}" fill="rgba(255,255,255,0.92)"/>`,x.used)p+=`<text x="${k.x.toFixed(1)}" y="${(k.y+f/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="13" font-weight="900" fill="rgba(0,0,0,0.4)" font-family="Arial Black">–</text>`;else{const q=Ze(x.country_code);q?p+=`<image href="${q}" x="${(k.x-20).toFixed(1)}" y="${(k.y+f/2-u+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:p+=`<text x="${(k.x-13).toFixed(1)}" y="${(k.y+f/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${Zt(x.country_code)}</text>`,p+=`<text x="${k.x.toFixed(1)}" y="${(k.y+f/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${G}</text>`;const R=Xt(x);R?p+=`<image href="${R}" x="${(k.x+g/2-14).toFixed(1)}" y="${(k.y+f/2-u+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:x.clubName&&(p+=`<text x="${(k.x+14).toFixed(1)}" y="${(k.y+f/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(x.clubName||"").slice(0,3).toUpperCase()}</text>`),x.boost&&(p+=`<rect x="${(k.x+g/2-12).toFixed(1)}" y="${(k.y-f/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,p+=`<text x="${(k.x+g/2-5).toFixed(1)}" y="${(k.y-f/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${x.boost}</text>`)}p+=`<rect x="${D}" y="${P}" width="${g}" height="${f}" rx="5" fill="${M?"rgba(255,255,255,0.12)":"none"}" stroke="${V}" stroke-width="${ee}" opacity="${U}"/>`,E&&(p+=`<rect x="${D}" y="${P}" width="${g}" height="${f}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${M?"selected":""}" data-card-id="${x.cardId}" data-role="${B}" style="cursor:pointer"/>`)}const L=38;return`<svg viewBox="${-L} ${-L} ${c+L*2} ${o+L*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${p}
  ...
  </div>`,setTimeout(()=>{const p=document.getElementById("score-home"),g=document.getElementById("score-ai"),f=document.getElementById("midfield-result");if(p&&g&&(n?(p.style.fontSize="80px",p.style.color="#FFD700",g.style.opacity="0.25"):(g.style.fontSize="80px",g.style.color="#ff6b6b",p.style.opacity="0.25")),f){const y=Wt();f.style.opacity="1",n&&t.clubName,f.innerHTML=`
  ...
  <div class="match-screen" style="position:fixed;inset:0;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;height:100%;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:100px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const E=t.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
            ...
            ${Ie((E.players||[]).map(M=>({...M,used:!1})),"#ff6b6b",E.total)}
            ...
            ${Ie((E.players||[]).map(M=>({...M,used:!1})),"#00ff88",E.total)}
    ...
    ${(()=>{const m=window.innerWidth>=700,E=(I,z,F)=>{var H,K;const q=(t.gcDefs||[]).find(J=>J.name===I.gc_type),R={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[q==null?void 0:q.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",_={purple:"#b06ce0",light_blue:"#00d4ef"}[q==null?void 0:q.color]||"#b06ce0",b=(q==null?void 0:q.name)||I.gc_type,C=(q==null?void 0:q.effect)||((H=de[I.gc_type])==null?void 0:H.desc)||"",A=q!=null&&q.image_url?`/manager-wars/icons/${q.image_url}`:null,w=((K=de[I.gc_type])==null?void 0:K.icon)||"⚡",S=Math.round(F*.22),T=Math.round(F*.22),N=F-S-T,j=b.length>12?7:9;return`<div class="gc-mini" data-gc-id="${I.id}" data-gc-type="${I.gc_type}"
          style="box-sizing:border-box;width:${z}px;height:${F}px;border-radius:10px;border:2px solid ${_};background:${R};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
        ...
        <div style="display:flex;flex:0.8;min-height:0;overflow:hidden">
          ...
          <div style="width:140px;flex-shrink:0;display:flex;flex-direction:column;padding:10px 8px;background:rgba(0,0,0,0.2)">
            ...
            <div style="width:100%;flex-shrink:0;padding-top:8px">${U}${Y}</div>
          </div>
        </div>`:`
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(m=>{var E,M,G;if(m.type==="duel"){const D=m.isGoal,P=m.homeScored?"#FFD700":D?"#ff6b6b":"rgba(255,255,255,0.3)",U=m.homeScored?"⚽ BUT !":D?"⚽ BUT IA !":(E=m.homePlayers)!=null&&E.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${D?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${P};margin-bottom:4px">
                <div style="font-size:9px;color:${P};letter-spacing:1px;margin-bottom:5px;font-weight:700;text-transform:uppercase">${U}</div>
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(o);let d=!1;const l=()=>{d||(d=!0,o.remove(),setTimeout(()=>c(),50))};o.addEventListener("click",l),setTimeout(l,3500)}async function we(e,t,i){var r,p;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:s}=i,c=t.homeScore>t.aiScore,o=t.homeScore===t.aiScore,a=c?"victoire":o?"nul":"defaite",d=fi(t.mode,a);t.matchId&&await $.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:c?s.profile.id:null,home_credits_reward:d,played_at:new Date().toISOString()}).eq("id",t.matchId);const l={credits:(s.profile.credits||0)+d,matches_played:(s.profile.matches_played||0)+1};c?l.wins=(s.profile.wins||0)+1:o?l.draws=(s.profile.draws||0)+1:l.losses=(s.profile.losses||0)+1,await $.from("users").update(l).eq("id",s.profile.id),await i.refreshProfile();const n=document.createElement("div");n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",n.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const f=()=>document.getElementById("mm-status"),y=async(x=!0)=>{p=!0,g&&($.removeChannel(g),g=null),x&&await $.rpc("cancel_matchmaking",{p_user_id:l.profile.id}).catch(()=>{})};(v=document.getElementById("mm-cancel"))==null||v.addEventListener("click",async()=>{try{await y(!0)}catch(x){console.warn("[Matchmaking] cleanup error:",x)}le(e),n("home")});const u=async(x,k,B)=>{p||(p=!0,g&&($.removeChannel(g),g=null),f()&&(f().textContent="Adversaire trouvé !"),await new Promise(m=>setTimeout(m,600)),e.isConnected!==!1&&go(e,t,x,B,a,d))},{data:h,error:L}=await $.rpc("try_matchmake",{p_user_id:l.profile.id,p_deck_id:i});if(L||!(h!=null&&h.success)){r("Erreur de matchmaking","error"),le(e),n("home");return}if(h.matched){u(h.match_id,h.opponent_id,!1);return}f()&&(f().textContent="En attente d'un autre joueur..."),g=$.channel("matchmaking-"+l.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${l.profile.id}`},x=>{const k=x.new;k.status==="matched"&&k.match_id&&u(k.match_id,k.matched_with,!0)}).subscribe()}async function go(e,t,i,s,c=[],o=[]){const{state:a,navigate:d,toast:l}=t,n=s?"p1":"p2",r=s?"p2":"p1",p=(c||[]).map(I=>I.id),g=(c||[]).map(I=>({id:I.id,gc_type:I.gc_type,_gcDef:I._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation du match...</div>';const{data:f}=await $.from("matches").select("*").eq("id",i).single();if(!f){l("Match introuvable","error"),d("home");return}async function y(){const[{data:I,error:z},{data:F,error:q},{data:R},{data:_}]=await Promise.all([$.rpc("get_deck_for_match",{p_deck_id:f.home_deck_id}),$.rpc("get_deck_for_match",{p_deck_id:f.away_deck_id}),$.from("users").select("id,pseudo,club_name").eq("id",f.home_id).single(),$.from("users").select("id,pseudo,club_name").eq("id",f.away_id).single()]);z&&console.error("[PvP] get_deck_for_match p1 error:",z),q&&console.error("[PvP] get_deck_for_match p2 error:",q);function b(j,H){return{cardId:j.card_id,position:H||j.position,id:j.id,firstname:j.firstname,name:j.surname_encoded,country_code:j.country_code,club_id:j.club_id,job:j.job,job2:j.job2,note_g:Number(j.note_g)||0,note_d:Number(j.note_d)||0,note_m:Number(j.note_m)||0,note_a:Number(j.note_a)||0,rarity:j.rarity,skin:j.skin,hair:j.hair,hair_length:j.hair_length,clubName:j.club_encoded_name||null,clubLogo:j.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}}const C=((I==null?void 0:I.starters)||[]).map(j=>b(j,j.position)),A=((F==null?void 0:F.starters)||[]).map(j=>b(j,j.position)),w=((I==null?void 0:I.subs)||[]).map(j=>b(j,j.position)),S=((F==null?void 0:F.subs)||[]).map(j=>b(j,j.position)),T=(I==null?void 0:I.formation)||"4-4-2",N=(F==null?void 0:F.formation)||"4-4-2";return console.log("[PvP] p1Formation:",T,"p1 starters:",C.length,"positions:",C.map(j=>j.position)),console.log("[PvP] p2Formation:",N,"p2 starters:",A.length,"positions:",A.map(j=>j.position)),{p1Team:Oe(C,T),p2Team:Oe(A,N),p1Subs:w,p2Subs:S,p1Formation:T,p2Formation:N,p1Name:(R==null?void 0:R.club_name)||(R==null?void 0:R.pseudo)||"Joueur 1",p2Name:(_==null?void 0:_.club_name)||(_==null?void 0:_.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:s?p:[],gc_p2:s?[]:p,gcCardsFull_p1:s?g:[],gcCardsFull_p2:s?[]:g,gcReady_p1:!0,gcReady_p2:!0,usedGc_p1:[],usedGc_p2:[],lastActionAt:new Date().toISOString()}}let u=f.game_state&&Object.keys(f.game_state).length?f.game_state:null;if(!u)if(s){u=await y();const{data:I}=await $.from("matches").select("game_state").eq("id",i).single();!(I!=null&&I.game_state)||!Object.keys(I.game_state).length?await $.from("matches").update({game_state:u,turn_user_id:f.home_id}).eq("id",i):u=I.game_state}else{e.innerHTML=`<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation avec l'adversaire...</div>`;for(let I=0;I<30&&!u;I++){await new Promise(F=>setTimeout(F,400));const{data:z}=await $.from("matches").select("game_state").eq("id",i).single();z!=null&&z.game_state&&Object.keys(z.game_state).length&&(u=z.game_state)}if(!u){l("Erreur de synchronisation","error"),d("home");return}u.gc_p2=p,u.gcCardsFull_p2=g,await $.from("matches").update({game_state:u}).eq("id",i)}let h=!1;function L(I){var C;try{$.removeChannel(v)}catch{}const z=a.profile.id,F=I.winner_id===z,q=!!I.forfeit,R=u[n+"Score"]??0,_=u[r+"Score"]??0,b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center",b.innerHTML=`
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 14px;background:rgba(0,0,0,0.25);flex-shrink:0">
        ...
        <div style="flex:1;text-align:center;color:#fff;font-size:14px;font-weight:700">
          ${q} <span style="color:#FFD700;font-size:18px">${z}</span> – <span style="color:#FFD700;font-size:18px">${F}</span> ${R}
        </div>
      ...
      <div style="text-align:center;padding:4px;background:rgba(0,0,0,0.15);font-size:11px;color:${C?"rgba(255,255,255,0.4)":"#FFD700"};font-weight:700;flex-shrink:0">
        ${C?`⏳ Tour de ${R}`:_?"⚔️ À vous d'attaquer !":b?"🛡️ À vous de défendre !":""}
      </div>
      <!-- Zone actions -->
      ...
      </div>`;const O=e.querySelector(".terrain-wrapper");O&&(O.innerHTML=Me(I,u[n+"Formation"],_?"attack":b?"defense":"idle",w,300,300))}if(e.querySelectorAll(".match-slot-hit").forEach(O=>{O.addEventListener("click",()=>{if(!_&&!b)return;const X=O.dataset.cardId,Q=O.dataset.role,W=(I[Q]||[]).find(ye=>ye.cardId===X);if(!W||W.used)return;Array.isArray(u["selected_"+n])||(u["selected_"+n]=[]);const ce=u["selected_"+n],je=ce.findIndex(ye=>ye.cardId===X);je>-1?ce.splice(je,1):ce.length<3&&ce.push({...W,_role:Q}),B()})}),(gt=e.querySelector("#pvp-boost-card"))==null||gt.addEventListener("click",()=>{G()}),e.querySelectorAll(".pvp-gc-mini").forEach(O=>{O.addEventListener("click",()=>{U(O.dataset.gcId,O.dataset.gcType)})}),(xt=e.querySelector("#pvp-action"))==null||xt.addEventListener("click",()=>{_?V():b&&ee()}),(mt=e.querySelector("#pvp-quit"))==null||mt.addEventListener("click",()=>{confirm("Quitter le match ? Vous perdrez par forfait.")&&k()}),(yt=e.querySelector("#pvp-view-opp"))==null||yt.addEventListener("click",()=>{P()}),(ht=e.querySelector("#pvp-toggle-history"))==null||ht.addEventListener("click",()=>{D()}),function(){const X=e.querySelector(".terrain-wrapper svg");X&&(X.removeAttribute("width"),X.removeAttribute("height"),X.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",X.setAttribute("viewBox","-26 -26 352 352"),X.setAttribute("preserveAspectRatio","xMidYMid meet"));const Q=e.querySelector(".match-screen");if(Q){const W=e.clientHeight;W>50&&(Q.style.height=W+"px",Q.style.maxHeight=W+"px",Q.style.overflow="hidden")}}(),u._timerInt&&(clearInterval(u._timerInt),u._timerInt=null),(_||b)&&!h){let O=30,X=!1;const Q=()=>document.getElementById("pvp-timer"),W=()=>{Q()&&(Q().textContent=O+"s",Q().style.color=X?"#ff4444":"#fff")};W(),u._timerInt=setInterval(()=>{O--,O<0?X?(clearInterval(u._timerInt),u._timerInt=null,k()):(X=!0,O=15,W()):W()},1e3)}}function m(){var b;const I=u["gcReady_"+n],z=u["gcReady_"+r];Array.isArray(u["gc_"+n])||(u["gc_"+n]=[]);let F=u["gc_"+n];const q=3;function R(C,A){const w=C._gcDef,S={purple:"linear-gradient(135deg,#3d0a7a,#7a28b8)",light_blue:"linear-gradient(135deg,#006080,#00bcd4)"},T={purple:"#9b59b6",light_blue:"#00bcd4"},N=S[w==null?void 0:w.color]||S.purple,j=T[w==null?void 0:w.color]||T.purple;return`<div class="pvp-gc-card" data-id="${C.id}"
    ...
    <div id="pvp-gc-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      <div style="text-align:center;padding:12px 16px 8px;flex-shrink:0">
        ...
        <div style="font-size:20px;font-weight:900;color:#fff">Choisir ses Game Changers</div>
        <div style="font-size:13px;color:rgba(255,255,255,0.5);margin-top:3px">
          Sélectionne <b style="color:#FFD700">${q}</b> cartes · ${F.length}/${q}
        </div>
      </div>
      ${I?`
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      ...
      <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
    </div>`,n==="p1"&&setTimeout(async()=>{await x({phase:"midfield"})},5e3)}function M(){const I=u[n+"Team"].MIL||[],z=u[r+"Team"].MIL||[];function F(A){return A.reduce((w,S)=>w+te(S,"MIL"),0)}function q(A){let w=0;for(let S=0;S<A.length-1;S++){const T=ge(A[S],A[S+1]);T==="#00ff88"?w+=2:T==="#FFD700"&&(w+=1)}return w}const R=F(I)+q(I),_=F(z)+q(z),b=R>=_;function C(A,w,S){return`<div style="text-align:center">
        <div style="font-size:10px;color:rgba(255,255,255,0.5);letter-spacing:2px;margin-bottom:8px;text-transform:uppercase">${w}</div>
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${A.map((T,N)=>{const j=oe(T),H=N<A.length-1?ge(T,A[N+1]):null,K=H&&H!=="#ff3333"&&H!=="#cc2222";return`
        ...
        <div style="margin-top:6px;font-size:11px;color:rgba(255,255,255,0.5)">
          Score: ${F(A)} + ${q(A)} liens = <b style="color:#fff">${F(A)+q(A)}</b>
        </div>
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="display:flex;flex-direction:column;align-items:center;gap:4px">
        <div id="pvp-score-me" style="font-size:48px;font-weight:900;color:#D4A017;transition:all 0.6s ease">${R}</div>
        <div style="font-size:14px;color:rgba(255,255,255,0.4);letter-spacing:2px">VS</div>
      ...
      <div id="pvp-midfield-result" style="opacity:0;text-align:center;transition:opacity 0.5s;color:#fff;max-width:320px"></div>
    </div>`,setTimeout(()=>{const A=document.getElementById("pvp-score-me"),w=document.getElementById("pvp-score-opp"),S=document.getElementById("pvp-midfield-result");A&&w&&(b?(A.style.fontSize="80px",A.style.color="#FFD700",w.style.opacity="0.25"):(w.style.fontSize="80px",w.style.color="#ff6b6b",A.style.opacity="0.25"));const T=u.p1Team.MIL||[],N=u.p2Team.MIL||[],j=F(T)+q(T),H=F(N)+q(N),K=j>=H?"p1":"p2";u[K+"Name"];let J=u.boostValue;J==null&&(J=Wt(),u.boostValue=J,u.boostOwner=K,u.boostUsed=!1),S&&(S.style.opacity="1",S.innerHTML=`<div style="font-size:20px;font-weight:900;margin-bottom:10px">
        ...
        <div style="background:rgba(135,206,235,0.15);border:2px solid #87CEEB;border-radius:14px;padding:14px 24px;display:inline-block;margin-top:4px" id="pvp-boost-display">
          ...
          <div style="font-size:10px;color:rgba(135,206,235,0.7)">Applicable sur n'importe quel joueur</div>
        </div>`:""}`),setTimeout(async()=>{if(n!=="p1")return;const ie=K;await x({phase:ie+"-attack",attacker:ie,round:1,boostValue:u.boostValue,boostUsed:!1,boostOwner:ie})},1800)},600)}function G(){var q;const I=u[n+"Team"],z=Object.entries(I).flatMap(([R,_])=>(_||[]).filter(b=>!b.used).map(b=>({...b,_line:R})));if(!z.length)return;const F=document.createElement("div");F.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",F.innerHTML=`
      ...
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${z.map(R=>{const _={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[R._line]||"#555",b=te(R,R._line)+(R.boost||0);return`<div class="boost-pick-item" data-cid="${R.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${_};overflow:hidden;cursor:pointer">
      ...
      </div>`,document.body.appendChild(F),(q=F.querySelector("#boost-picker-close"))==null||q.addEventListener("click",()=>F.remove()),F.querySelectorAll(".boost-pick-item").forEach(R=>{R.addEventListener("click",async()=>{const _=R.dataset.cid,b=z.find(w=>w.cardId===_);if(!b)return;const C=u[n+"Team"],A=(C[b._line]||[]).find(w=>w.cardId===_);A&&(A.boost=(A.boost||0)+u.boostValue),F.remove(),await x({[n+"Team"]:C,boostUsed:!0})})})}function D(){var F;const I=Array.isArray(u.log)?u.log:[],z=document.createElement("div");z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column",z.innerHTML=`
      <div style="display:flex;align-items:center;padding:14px 16px;border-bottom:1px solid rgba(255,255,255,0.1);flex-shrink:0">
        <div style="flex:1;font-size:14px;font-weight:700;color:#fff">📋 Historique du match</div>
        <button id="pvp-hist-close" style="background:none;border:none;color:rgba(255,255,255,0.6);font-size:20px;cursor:pointer">✕</button>
      </div>
      <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
        ${I.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...I].reverse().map(q=>`<div style="padding:8px 10px;border-radius:8px;background:rgba(255,255,255,0.04);border-left:3px solid ${q.type==="goal"?"#FFD700":q.type==="defense"?"#00ff88":"rgba(255,255,255,0.5)"}">
      ...
      <div style="width:min(90vw,420px)">${Le(u[r+"Team"],u[r+"Formation"],null,[],300,300)}</div>
      <button id="pvp-view-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(I),(z=I.querySelector("#pvp-view-opp-close"))==null||z.addEventListener("click",()=>I.remove())}function U(I,z){var j,H;const q=(u["gcCardsFull_"+n]||[]).find(K=>K.id===I),R=q==null?void 0:q._gcDef,_=de[z]||{icon:"⚡",desc:"Carte spéciale."},b={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[R==null?void 0:R.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",C={purple:"#b06ce0",light_blue:"#00d4ef"}[R==null?void 0:R.color]||"#b06ce0",A=(R==null?void 0:R.name)||z,w=(R==null?void 0:R.effect)||_.desc,S=R!=null&&R.image_url?`/manager-wars/icons/${R.image_url}`:null,T=_.icon||"⚡",N=document.createElement("div");N.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",N.innerHTML=`
      ...
      </div>`,document.body.appendChild(N),(j=N.querySelector("#pvp-gc-back"))==null||j.addEventListener("click",()=>N.remove()),(H=N.querySelector("#pvp-gc-use"))==null||H.addEventListener("click",async()=>{N.remove();const K=u["usedGc_"+n]||[];K.push(I),await x({["usedGc_"+n]:K})})}function Y(){var R;const I=u[n+"Score"],z=u[r+"Score"],F=I>z,q=I===z;e.style.overflow="hidden",e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      <div style="font-size:64px">${F?"🏆":q?"🤝":"😤"}</div>
      <div style="font-size:24px;font-weight:900;color:#fff">${F?"Victoire !":q?"Match nul":"Défaite"}</div>
    ...
    </div>`,(R=document.getElementById("pvp-home"))==null||R.addEventListener("click",()=>{$.removeChannel(v),le(e),d("home")})}async function V(){const I=(u["selected_"+n]||[]).map(q=>({...q,_line:q._role}));if(!I.length)return;const z=ot(I,u.modifiers[n]||{});I.forEach(q=>{const R=(u[n+"Team"][q._role]||[]).find(_=>_.cardId===q.cardId);R&&(R.used=!0)});const F=Array.isArray(u.log)?u.log:[];F.push({type:"attack",text:`⚔️ ${u[n+"Name"]} attaque (${z.total})`,players:I,total:z.total,side:n}),await x({pendingAttack:{...z,players:I,side:n},["selected_"+n]:[],modifiers:{...u.modifiers,[n]:{}},phase:r+"-defense",log:F})}async function ee(){const I=(u["selected_"+n]||[]).map(S=>({...S,_line:S._role})),z=nt(I,u.modifiers[n]||{});I.forEach(S=>{const T=(u[n+"Team"][S._role]||[]).find(N=>N.cardId===S.cardId);T&&(T.used=!0)});const F=rt(u.pendingAttack.total,z.total,u.modifiers[n]||{}),q=u.pendingAttack.side,R=F==="attack",_=u[q+"Score"]+(R?1:0),b=q==="p1"?"p2":"p1",C=(u.round||0)+1,A=C>10,w=Array.isArray(u.log)?u.log:[];w.push({type:"defense",text:`🛡️ ${u[n+"Name"]} défend (${z.total})`,players:I,total:z.total,side:n}),w.push({type:R?"goal":"stop",text:R?`⚽ BUT de ${u[q+"Name"]} ! (${u.pendingAttack.total} vs ${z.total})`:`✋ Attaque stoppée (${u.pendingAttack.total} vs ${z.total})`}),await x({[q+"Score"]:_,["selected_"+n]:[],modifiers:{...u.modifiers,[n]:{}},pendingAttack:null,phase:A?"finished":b+"-attack",attacker:b,round:C,log:w}),A&&await $.from("matches").update({status:"finished"}).eq("id",i)}B()}const xo={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"};async function mo(e,t){const{state:i,toast:s}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement du marché...</div>',await ct(e,t)}async function ct(e,t){const{state:i,toast:s}=t,{data:c}=await $.from("market_listings").select(`id, price, status, listed_at, seller_id,
  ...
  </div>`}const Z={user:null,profile:null,page:"home",params:{}};function De(e,t="info",i=3e3){const s=document.getElementById("toast");s&&(s.textContent=e,s.className=`show ${t}`,clearTimeout(s._t),s._t=setTimeout(()=>{s.className=""},i))}function _o(e,t,i=""){document.getElementById("modal-title").textContent=e,document.getElementById("modal-body").innerHTML=t,document.getElementById("modal-footer").innerHTML=i,document.getElementById("modal-overlay").classList.remove("hidden")}function et(){document.getElementById("modal-overlay").classList.add("hidden")}async function Ge(){if(!Z.user)return;const{data:e}=await $.from("users").select("*").eq("id",Z.user.id).single();e&&(Z.profile=e)}const pt="mw_theme";function He(){return localStorage.getItem(pt)||"light"}function $o(e){var t;localStorage.setItem(pt,e),tt(e),(t=Z.profile)!=null&&t.id&&$.from("users").update({theme:e}).eq("id",Z.profile.id).then(()=>{})}function tt(e){document.documentElement.setAttribute("data-theme",e)}function Re(e,t={}){Z.page=e,Z.params=t,ii()}async function ii(){const e=document.getElementById("page-content");if(!e)return;document.querySelectorAll(".bottom-nav a").forEach(s=>{s.classList.toggle("active",s.dataset.page===Z.page)});const t=document.getElementById("nav-credits");t&&Z.profile&&(t.textContent=`💰 ${(Z.profile.credits||0).toLocaleString("fr")}`);const i={state:Z,navigate:Re,toast:De,openModal:_o,closeModal:et,refreshProfile:Ge};switch(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽</div>',Z.page){case"home":await At(e,i);break;case"collection":await mi(e,i);break;case"decks":await Ye(e,i);break;case"boosters":await ji(e,i);break;case"match":{(Z.params&&Z.params.matchMode||"vs_ai_easy")==="random"?await uo(e,i):await Ji(e,i);break}case"market":await mo(e,i);break;case"rankings":await bo(e,i);break;case"matches":await wo(e,i);break;default:await At(e,i)}}function ko(){const e=document.getElementById("app"),t=Z.profile;if(!t)return;const i="/manager-wars/icons/";e.innerHTML=`
    <nav class="top-nav">
