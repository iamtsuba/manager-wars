...
import{s as h,q as Ut,t as Ri,F as Oi,o as tt,n as ti,g as At,r as Ie,m as zt,u as zi,j as Hi,v as ao,w as so,T as Ei,p as lo,x as co,d as po}from"./special-cards-DZxbxWIu.js";const uo="/";function ln(e,{navigate:t,toast:i}){let n="login";const r=()=>{var a,s,d,c,l,f;const o=n==="login";e.innerHTML=`
    <div style="
  ...
  `;function s(){var y;const c=document.getElementById("logo-preview"),l=document.getElementById("logo-initials"),f=((y=document.getElementById("setup-club"))==null?void 0:y.value)||"MW",x=f.trim().split(" ").filter(Boolean),g=x.length>=2?(x[0][0]+x[1][0]).toUpperCase():f.slice(0,2).toUpperCase();c&&(c.style.background=a,c.style.borderColor=o),l&&(l.textContent=g,l.style.color=o)}document.getElementById("color1").addEventListener("input",c=>{o=c.target.value,document.getElementById("swatch1").style.background=o,s()}),document.getElementById("color2").addEventListener("input",c=>{a=c.target.value,document.getElementById("swatch2").style.background=a,s()});function d(c){document.querySelectorAll(".setup-step").forEach(l=>l.classList.remove("active")),document.getElementById(`step-${c}`).classList.add("active"),document.getElementById("step-num").textContent=c,document.getElementById("progress-fill").style.width=`${Math.round(c/3*100)}%`,c===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("step1-error");if(l.textContent="",c.length<3){l.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:f}=await h.from("users").select("id").eq("pseudo",c).maybeSingle();if(f){l.textContent="Ce pseudo est déjà pris.";return}d(2)}),document.getElementById("step2-back").addEventListener("click",()=>d(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const c=document.getElementById("setup-club").value.trim(),l=document.getElementById("step2-error");if(l.textContent="",c.length<2){l.textContent="Nom trop court (min. 2 caractères).";return}const{data:f}=await h.from("users").select("id").eq("club_name",c).maybeSingle();if(f){l.textContent="Ce nom de club est déjà pris.";return}d(3)}),document.getElementById("step3-back").addEventListener("click",()=>d(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("setup-club").value.trim(),f=document.getElementById("step3-error"),x=document.getElementById("step3-finish");f.textContent="",x.disabled=!0,x.textContent="Création en cours…";try{const{error:g}=await h.from("users").insert({id:t.user.id,pseudo:c,club_name:l,club_color1:o,club_color2:a,credits:1e4});if(g)throw g;await mo(t.user.id),await r(),n(`Bienvenue ${c} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(g){f.textContent=g.message,x.disabled=!1,x.textContent="🚀 Créer mon profil !"}})}async function mo(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await h.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const go="modulepreload",yo=function(e){return"/"+e},cn={},Ln=function(t,i,n){let r=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),s=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));r=Promise.allSettled(i.map(d=>{if(d=yo(d),d in cn)return;cn[d]=!0;const c=d.endsWith(".css"),l=c?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${l}`))return;const f=document.createElement("link");if(f.rel=c?"stylesheet":go,c||(f.as="script"),f.crossOrigin="",f.href=d,s&&f.setAttribute("nonce",s),document.head.appendChild(f),c)return new Promise((x,g)=>{f.addEventListener("load",x),f.addEventListener("error",()=>g(new Error(`Unable to preload CSS for ${d}`)))})}))}function o(a){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=a,window.dispatchEvent(s),!s.defaultPrevented)throw a}return r.then(a=>{for(const s of a||[])s.status==="rejected"&&o(s.reason);return t().catch(o)})},ii="#1A6B3C",ni="#cc2222",xo="#D4A017",pn="#888";async function bo(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    </style>`}const Sn="mw_sound_volume";function Vi(){const e=localStorage.getItem(Sn);if(e===null)return 100;const t=parseInt(e,10);return Number.isFinite(t)?Math.max(0,Math.min(100,t)):100}function wo(e){localStorage.setItem(Sn,String(Math.max(0,Math.min(100,Math.round(e)))))}function Yi(){return Vi()===0}function ui(e){return Math.max(0,Math.min(1,e*(Vi()/100)))}function Wi(e,t=1){if(Yi())return null;try{const i=new Audio(e);return i.volume=ui(t),i.play().catch(()=>{}),i}catch{return null}}let gt=null,Mi=null,un=.3;function An(e,t=.3){if(un=t,gt&&Mi===e&&!gt.paused){gt.volume=ui(un);return}if(Gt(),!Yi())try{const i=new Audio(e);i.loop=!0,i.volume=ui(t),i.play().catch(()=>{}),gt=i,Mi=e}catch{}}function Gt(){if(gt)try{gt.pause(),gt.currentTime=0}catch{}gt=null,Mi=null}let Xt=null;function zn(e,t=.6){if(Ue(),!Yi())try{const i=new Audio(e);i.volume=ui(t),i.play().catch(()=>{}),Xt=i}catch{}}function Ue(){if(Xt)try{Xt.pause(),Xt.currentTime=0}catch{}Xt=null}const ko="2026.07.22-1520";async function fn(e,{state:t,navigate:i,toast:n}){var o,a;const r=t.profile;r&&(e.innerHTML=`
  <style>
    .home-dark {
      min-height: 100%;
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(fe=document.getElementById("close-detail"))==null||fe.addEventListener("click",s);let K=new Set;const Q=()=>{const G=K.size,O=document.getElementById("sell-action-panel");if(!O)return;O.style.display=G>0?"block":"none",document.getElementById("sell-selected-count").textContent=G;const $=document.getElementById("evolve-btn");$&&($.textContent=`⬆️ Évoluer ${G>1?"(+"+G+")":""}`)};document.querySelectorAll(".expl-check").forEach(G=>{G.addEventListener("change",()=>{const O=G.dataset.id;G.checked?K.add(O):K.delete(O);const $=G.closest(".exemplaire-row");$&&($.style.borderColor=G.checked?"#1A6B3C":"#eee"),Q()})}),document.querySelectorAll(".exemplaire-row").forEach(G=>{G.addEventListener("click",O=>{if(O.target.closest("button")||O.target.tagName==="INPUT")return;const $=G.querySelector(".expl-check");$&&!$.disabled&&($.checked=!$.checked,$.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(G=>{G.addEventListener("click",O=>{O.stopPropagation();const $=document.querySelector(`.expl-hist[data-hist="${G.dataset.idx}"]`);$&&($.style.display=$.style.display==="none"?"flex":"none")})}),(M=document.getElementById("evolve-btn"))==null||M.addEventListener("click",async()=>{if(x<=1)return;const G=[...K];if(!G.length)return;if(K.has(e.id)){const B=document.createElement("div");B.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",B.innerHTML=`
  ...
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
        ...
        <div style="width:130px;flex-shrink:0;background:rgba(0,0,0,0.3);display:flex;flex-direction:column;align-items:center;padding:12px 8px;gap:12px;border-left:1px solid rgba(255,255,255,0.1)">
          ...
          <div style="width:100%;text-align:center">
            ...
            <div id="add-stad-btn-pc" style="cursor:pointer;margin:0 auto;width:fit-content">
              ${s?(()=>{var _;const u=t.stadDefMap[s.stadium_id],b=((_=u==null?void 0:u.club)==null?void 0:_.logo_url)||(u==null?void 0:u.image_url)||null;return ti((u==null?void 0:u.name)||"Stade",b,"+10 ⭐",{width:100})})():`<div style="width:100px;height:171px;border:2px dashed rgba(79,195,247,0.4);border-radius:8px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px">
        ...
        <div style="display:flex;gap:10px;align-items:flex-start">
          ...
          <div style="flex-shrink:0;text-align:center">
            ...
            <div id="add-stad-btn" style="cursor:pointer">
              ${s?(()=>{var _;const u=t.stadDefMap[s.stadium_id],b=((_=u==null?void 0:u.club)==null?void 0:_.logo_url)||(u==null?void 0:u.image_url)||null;return ti((u==null?void 0:u.name)||"Stade",b,"+10⭐",{width:66})})():`<div style="width:66px;height:86px;border:2px dashed rgba(79,195,247,0.5);border-radius:6px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px">
  ...
  </div>`;const l=window.innerWidth>=900,f=e.querySelector(".deck-pc-layout"),x=e.querySelector(".deck-mobile-layout");f&&(f.style.display=l?"block":"none"),x&&(x.style.display=l?"none":"block"),Oo(e,t,r,i),e.querySelectorAll("#builder-back").forEach(u=>u.addEventListener("click",()=>n("decks")));const g=()=>{const{openModal:u,closeModal:b}=i,_=`<div style="display:flex;flex-wrap:wrap;gap:8px;padding:8px">
    ...
    </div>`;u("⚽ Choisir une formation",_),document.querySelectorAll("#modal-body [data-forma]").forEach(v=>{v.addEventListener("click",()=>{t.formation=v.dataset.forma;const E=_n(t.formation),m={};E.forEach(z=>{t.slots[z]&&(m[z]=t.slots[z])}),t.slots=m,b(),_t(e,t,i)})})};e.querySelectorAll("#formation-mobile-btn, #formation-pc-btn").forEach(u=>u.addEventListener("click",g)),e.querySelectorAll("#add-stad-btn-pc, #add-stad-btn").forEach(u=>u.addEventListener("click",()=>Ho(t,e,i))),e.querySelectorAll("#save-deck").forEach(u=>u.addEventListener("click",()=>Vo(t,i))),e.querySelectorAll("[data-remove-sub]").forEach(u=>{u.addEventListener("click",()=>{t.subs=t.subs.filter(b=>b!==u.dataset.removeSub),_t(e,t,i)})}),e.querySelectorAll("#add-sub-btn").forEach(u=>u.addEventListener("click",()=>Ko(t,e,i)))}function Oo(e,t,i,n){var v,E,m;const r=window.innerWidth>=900,o=e.querySelector(r?"#deck-field-pc":"#deck-field-mobile");if(!o)return;const a=(v=t.stadiumCards)==null?void 0:v.find(z=>z.id===t.stadiumCardId),s=a&&((E=t.stadDefMap)==null?void 0:E[a.stadium_id])||null,d=zt[t.formation]||{},c=Ri(t.formation)||[],l={};for(const z of i){const k=t.slots[z],A=k?t.playerCards.find(q=>q.id===k):null;A!=null&&A.player?l[z]={...A.player,_evolution_bonus:A.evolution_bonus||0,face:A.player.face||null}:l[z]=null}const f=window.innerWidth>=900,x=f?window.innerWidth-280:window.innerWidth-20,g=f?Math.min(x,860):x,y=Math.round(f?g*.82:g*.85),p=f?84:44;let u="";for(const[z,k]of c){const A=d[z],q=d[k];if(!A||!q)continue;const ie=A.x*g,ne=Math.round(.03*y+A.y*.85*y),Z=q.x*g,j=Math.round(.03*y+q.y*.85*y),I=l[z],K=l[k],Q=Ut(I,K);Q==="#ff3333"||Q==="#cc2222"?u+=`<line x1="${ie.toFixed(1)}" y1="${ne.toFixed(1)}" x2="${Z.toFixed(1)}" y2="${j.toFixed(1)}" stroke="${Q}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(u+=`<line x1="${ie.toFixed(1)}" y1="${ne.toFixed(1)}" x2="${Z.toFixed(1)}" y2="${j.toFixed(1)}" stroke="${Q}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,u+=`<line x1="${ie.toFixed(1)}" y1="${ne.toFixed(1)}" x2="${Z.toFixed(1)}" y2="${j.toFixed(1)}" stroke="${Q}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let b="";const _=Math.round(p*657/507);for(const z of i){const k=d[z];if(!k)continue;const A=l[z],q=k.x*g;k.y*y;const ie=Math.round(.03*y+k.y*(.85*y)),ne=Math.round(q-p/2),Z=Math.round(ie-_/2);if(A){const j=z.replace(/\d+/,""),I=s&&(s.club_id&&String(A.club_id)===String(s.club_id)||s.country_code&&A.country_code===s.country_code),K=Ie({...A,_evolution_bonus:A._evolution_bonus||0},{width:p,showStad:!0,stadDef:s,role:j});I&&((m=s.club)!=null&&m.logo_url||s.image_url),b+=`<div style="position:absolute;left:${ne}px;top:${Z}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${z}">
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    ${(()=>{const M=window.innerWidth>=700,Y=(B,R,ue)=>{var Ae,Be;const le=(t.gcDefs||[]).find(ke=>ke.name===B.gc_type),he=(le==null?void 0:le.name)||B.gc_type,Se=(le==null?void 0:le.effect)||((Ae=je[B.gc_type])==null?void 0:Ae.desc)||"",ve=le!=null&&le.image_url?`/icons/${le.image_url}`:null,Te=((Be=je[B.gc_type])==null?void 0:Be.icon)||"⚡";return`<div class="gc-mini" data-gc-id="${B.id}" data-gc-type="${B.gc_type}" style="flex-shrink:0;cursor:pointer">
        ...
        <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
          ${Ot(t.homeTeam,t.formation,t.phase,n,M?1300:It(),M?600:Nt(),y)}
        </div>
      </div>`;return M?`
  ...
  <div id="match-history-panel">
    ...
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      ...
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${l?"🏆 Retour à la Mini League":a?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(D),(ee=D.querySelector("#pvp-end-home"))==null||ee.addEventListener("click",()=>{D.remove(),it(e),l?g("mini-league",{openLeagueId:l}):g(a?"ranked":"home")})}const Z=h.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},w=>{var S;const L=w.new;try{if(L.status==="finished"||L.forfeit){if(z)return;z=!0,k&&(clearInterval(k),k=null),Ue(),L.game_state&&(m=L.game_state),ne(L);return}if(L.game_state){const C=m;m=L.game_state;const T=m._lastGC;if(T&&T.seq&&!ie.has(T.seq)&&(ie.add(T.seq),T.by!==p)){$(T.type,T.by,()=>Y());return}const P=C[p+"Score"]||0,D=C[u+"Score"]||0,F=m[p+"Score"]||0,U=m[u+"Score"]||0,X=F>P,te=U>D;if((X||te)&&!q.has(m.round)){q.add(m.round);const ee=[...m.log||[]].reverse().find(se=>se.isGoal),J=((ee==null?void 0:ee.homePlayers)||[]).map(se=>({name:se.name,note:se.note,portrait:se.portrait,job:se.job}));!!((S=ee==null?void 0:ee.text)!=null&&S.includes("DERNIER CORNER"))?Se().then(()=>{Qe(J,F,U,X,()=>Y())}):Qe(J,F,U,X,()=>Y());return}Y()}}catch(C){console.error("[PvP] crash:",C)}}).subscribe();async function j(w){Object.assign(m,w),m.lastActionAt=new Date().toISOString();const{error:L}=await h.from("matches").update({game_state:m}).eq("id",i);L&&y("Erreur de synchronisation","error");try{Y()}catch(S){console.error("[PvP] renderPvpScreen crash:",S)}}async function I(){if(z)return;z=!0,Gt(),Ue(),k&&(clearInterval(k),k=null);const w=n?v.away_id:v.home_id,L=n?"p2":"p1",S=n?"p1":"p2",C={...m,[L+"Score"]:3,[S+"Score"]:0,phase:"finished"},T=C.p1Score||0,P=C.p2Score||0;try{await h.from("matches").update({game_state:C}).eq("id",i);const{data:D,error:F}=await h.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:w,p_home_score:T,p_away_score:P,p_forfeit:!0});if(F&&console.error("[PvP] finish_pvp_match (forfeit):",F),l)try{await h.rpc("check_and_finish_mini_league",{p_league_id:l})}catch(U){console.error("[PvP] check_and_finish_mini_league (forfeit):",U)}try{const{data:U}=await h.rpc("apply_match_rewards",{p_match_id:i});U!=null&&U.success&&!(U!=null&&U.skipped)&&typeof t.refreshProfile=="function"&&await t.refreshProfile()}catch(U){console.error("[PvP] apply_match_rewards (forfeit):",U)}}catch{}try{h.removeChannel(Z)}catch{}setTimeout(()=>{it(e),l?g("mini-league",{openLeagueId:l}):g("home")},800)}const K={BOOST_STAT:({value:w=1,count:L=1,roles:S=[]},C,T)=>{const P=C[p+"Team"],D=Object.entries(P).filter(([F])=>!S.length||S.includes(F)).flatMap(([F,U])=>U.filter(X=>!X.used).map(X=>({...X,_line:F})));if(!D.length){C.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),T(C);return}Q(D,L,`Choisir ${L} joueur(s) à booster (+${w})`,F=>{F.forEach(U=>{const X=(P[U._line]||[]).find(te=>te.cardId===U.cardId);X&&(X.boost=(X.boost||0)+w,C.log.push({text:`⚡ +${w} sur ${X.name}`,type:"info"}))}),C[p+"Team"]=P,T(C)})},DEBUFF_STAT:({value:w=1,count:L=1,roles:S=[],target:C="ai"},T,P)=>{const D=C==="home"?p:u,F=T[D+"Team"],U=C==="home"?"allié":"adverse",X=Object.entries(F).filter(([te])=>!S.length||S.includes(te)).flatMap(([te,ee])=>ee.map(J=>({...J,_line:te})));if(!X.length){T.log.push({text:`🎯 Aucun joueur ${U}`,type:"info"}),P(T);return}Q(X,L,`Choisir ${L} joueur(s) ${U}(s) (-${w})`,te=>{te.forEach(ee=>{const J=(F[ee._line]||[]).find(ae=>ae.cardId===ee.cardId);J&&(J.boost=(J.boost||0)-w,T.log.push({text:`🎯 -${w} sur ${J.name}`,type:"info"}))}),T[D+"Team"]=F,P(T)})},GRAY_PLAYER:({count:w=1,roles:L=[],target:S="ai"},C,T)=>{const P=S==="home"?p:u,D=C[P+"Team"],F=S==="home"?"allié":"adverse",U=Object.entries(D).filter(([X])=>!L.length||L.includes(X)).flatMap(([X,te])=>te.filter(ee=>!ee.used).map(ee=>({...ee,_line:X})));if(!U.length){C.log.push({text:`❌ Aucun joueur ${F}`,type:"info"}),T(C);return}Q(U,w,`Choisir ${w} joueur(s) ${F}(s) à exclure`,X=>{const te="usedCardIds_"+P,ee=new Set(C[te]||[]);X.forEach(J=>{const ae=(D[J._line]||[]).find(se=>se.cardId===J.cardId);ae&&(ae.used=!0,ee.add(J.cardId),C.log.push({text:`❌ ${ae.name} exclu !`,type:"info"}))}),C[te]=[...ee],C[P+"Team"]=D,T(C)})},REVIVE_PLAYER:({count:w=1,roles:L=[]},S,C)=>{const T=S[p+"Team"],P=Object.entries(T).filter(([D])=>!L.length||L.includes(D)).flatMap(([D,F])=>F.filter(U=>U.used).map(U=>({...U,_line:D})));if(!P.length){S.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),C(S);return}Q(P,w,`Choisir ${w} joueur(s) à ressusciter`,D=>{D.forEach(F=>{const U=(T[F._line]||[]).find(X=>X.cardId===F.cardId);U&&(U.used=!1,S.log.push({text:`💫 ${U.name} ressuscité !`,type:"info"}))}),S[p+"Team"]=T,C(S)})},REMOVE_GOAL:({},w,L)=>{const S=u+"Score";w[S]>0?(w[S]--,w.log.push({text:"🚫 Dernier but annulé !",type:"info"})):w.log.push({text:"🚫 Aucun but à annuler",type:"info"}),L(w)},ADD_GOAL_DRAW:({},w,L)=>{w[p+"Score"]===w[u+"Score"]?(w[p+"Score"]++,w.log.push({text:"🎯 But bonus !",type:"info"})):w.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),L(w)},ADD_SUB:({value:w=1},L,S)=>{L.maxSubs=(L.maxSubs||3)+w,L.log.push({text:`🔄 +${w} remplacement(s)`,type:"info"}),S(L)},CUSTOM:({},w,L)=>L(w)};function Q(w,L,S,C){const T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let P=[];function D(){var U,X;const F=w.map(te=>{const ee=te._line||te.job||"MIL",J=P.find(se=>se.cardId===te.cardId),ae=Ie({...te,_evolution_bonus:0},{width:90,showStad:!0,role:ee,extraNote:te.boost||0});return`<div class="pp-item" data-cid="${te.cardId}"
        ...
        </div>`,(U=T.querySelector("#pp-close"))==null||U.addEventListener("click",()=>T.remove()),T.querySelectorAll(".pp-item").forEach(te=>{te.addEventListener("click",()=>{const ee=te.dataset.cid,J=w.find(se=>se.cardId===ee),ae=P.findIndex(se=>se.cardId===ee);J&&(ae>-1?P.splice(ae,1):P.length<L&&P.push(J),D())})}),(X=T.querySelector("#pp-confirm"))==null||X.addEventListener("click",()=>{T.remove(),C(P)})}D(),document.body.appendChild(T)}async function pe(w,L){const C=(m["gcCardsFull_"+p]||[]).find(F=>F.id===w),T=(C==null?void 0:C._gcDef)||(m.gcDefs||[]).find(F=>{var U;return F.name===L||((U=F.name)==null?void 0:U.toLowerCase().trim())===(L==null?void 0:L.toLowerCase().trim())}),P=[...m["usedGc_"+p]||[],w],D={type:L,by:p,seq:(m._gcAnimSeq||0)+1};ie.add(D.seq),$(L,p,async()=>{if(T!=null&&T.effect_type&&T.effect_type!=="CUSTOM"){const U=K[T.effect_type];if(U){const X={...m};U(T.effect_params||{},X,async te=>{te["usedGc_"+p]=P,te._lastGC=D,te._gcAnimSeq=D.seq,await j(te)});return}}const F={...m};switch(L){case"Remplacement+":F.maxSubs=(F.maxSubs||3)+1,F.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const U=u+"Score";F[U]>0&&(F[U]--,F.log.push({text:"🚫 But annulé",type:"info"}));break}}F["usedGc_"+p]=P,F._lastGC=D,F._gcAnimSeq=D.seq,await j(F)})}function fe(w,L){const S="usedCardIds_"+w,C=new Set(m[S]||[]);L.forEach(T=>C.add(T)),m[S]=[...C]}function M(){for(const w of["p1","p2"]){const L=new Set(m["usedCardIds_"+w]||[]),S=m[w+"Team"];if(S)for(const C of["GK","DEF","MIL","ATT"])(S[C]||[]).forEach(T=>{T.used=L.has(T.cardId)})}}function Y(){var nn,on,rn,an,sn,dn;if(m.phase==="reveal")return ce();if(m.phase==="midfield")return O();if(m.phase==="finished")return Ge();const w=m[p+"Team"],L=m[u+"Team"];M();const S=m[p+"Score"],C=m[u+"Score"],T=m[p+"Name"],P=m[u+"Name"],D=m.phase===p+"-attack",F=m.phase===p+"-defense",U=Array.isArray(m["selected_"+p])?m["selected_"+p]:[],X=U.map(W=>W.cardId),te=window.innerWidth>=700,ee=m[p+"Subs"]||[],J=m["usedSubIds_"+p]||[],ae=ee.filter(W=>!J.includes(W.cardId)),se=m["gcCardsFull_"+p]||[],ge=m["usedGc_"+p]||[],de=se.filter(W=>!ge.includes(W.id)),ye=m.boostOwner===p&&!m.boostUsed,be=[...w.MIL||[],...w.ATT||[]].filter(W=>!W.used),oe=[...L.MIL||[],...L.ATT||[]].filter(W=>!W.used),Ee=!ke(L),Ne=be.length===0&&oe.length===0&&!Ee,Le=(w.DEF||[]).filter(W=>!W.used),Ve=(w.GK||[]).filter(W=>!W.used);let Oe=[];D&&be.length===0&&(Ee?(Oe=Le.map(W=>W.cardId),Le.length===0&&(Oe=Oe.concat(Ve.map(W=>W.cardId)))):Ne&&(Oe=[...Le,...Ve].map(W=>W.cardId)));function nt(W,me,ze){var Me,ft;const _e=W._gcDef,Bt=(_e==null?void 0:_e.name)||W.gc_type,at=(_e==null?void 0:_e.effect)||((Me=je[W.gc_type])==null?void 0:Me.desc)||"",Fe=_e!=null&&_e.image_url?`/icons/${_e.image_url}`:null,Re=((ft=je[W.gc_type])==null?void 0:ft.icon)||"⚡";return`<div class="pvp-gc-mini" data-gc-id="${W.id}" data-gc-type="${W.gc_type}" style="flex-shrink:0;cursor:pointer">
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${Ot(w,m[p+"Formation"],ut,X,te?1300:It(),te?600:Nt(),Oe)}
      </div>
    </div>`,rt=m[p+"Team"],ro=(()=>{var me,ze,_e,Bt,at;if(F&&((me=m.pendingAttack)!=null&&me.players)){const Fe=m.pendingAttack;let Re="";if(U.length>0){const Me=U.map(We=>{const Vt=(rt[We._role]||[]).find($i=>$i.cardId===We.cardId)||We,Yt=rt[We._role]||[],si=Yt.findIndex($i=>$i.cardId===We.cardId),ki=yt(Yt.length),Wt=si>=0?ki[si]:Vt._col??1;return{...Vt,_line:We._role,_col:Wt}}),ft=ri(Me,((ze=m.modifiers)==null?void 0:ze[p])||{},m[p+"Formation"]);Re=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
        ...
        </div>`}const W=(m.log||[]).slice(-1)[0];return W?'<div style="padding:2px 4px">'+Yn(W)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3);text-align:center">⏳ Match en cours...</div>'})(),tn=`
      <div style="display:flex;align-items:center;padding:8px 10px;background:rgba(0,0,0,0.5);gap:6px;flex-shrink:0">
      ...
      <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
        ...
        <div id="mobile-play-area" style="flex:1;min-height:0;display:flex;overflow:hidden">
          <div id="match-field" style="flex:1;min-width:0;min-height:0;overflow:hidden">
            <div class="terrain-wrapper" style="width:100%;height:100%;overflow:hidden">
              ${Ot(w,m[p+"Formation"],ut,X,It(),Nt(),Oe)}
            </div>
          </div>
        </div>
        <div id="mobile-action-bar" style="position:absolute;left:0;right:0;bottom:0;z-index:20;background:rgba(0,0,0,0.55);padding:6px 8px 8px;display:flex;flex-direction:column;gap:6px;box-shadow:0 -4px 16px rgba(0,0,0,0.5)">
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;overflow-y:auto;background:#0a3d1e">
      ${pr(m[u+"Team"],m[u+"Formation"],null,m[u+"Name"]||"Adversaire")}
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      </div>`,(Ne=ee.querySelector("#psub-close"))==null||Ne.addEventListener("click",()=>ee.remove()),(Le=ee.querySelector("#pout-up"))==null||Le.addEventListener("click",()=>{F>0&&(F--,J())}),(Ve=ee.querySelector("#pout-down"))==null||Ve.addEventListener("click",()=>{F<P.length-1&&(F++,J())}),(Oe=ee.querySelector("#pin-up"))==null||Oe.addEventListener("click",()=>{X>0&&(X--,J())}),(nt=ee.querySelector("#pin-down"))==null||nt.addEventListener("click",()=>{X<D.length-1&&(X++,J())});const Ee=(De,Ye,He,pt)=>{const ot=ee.querySelector("#"+De);if(!ot)return;let Ct=0;ot.addEventListener("touchstart",ut=>{Ct=ut.touches[0].clientY},{passive:!0}),ot.addEventListener("touchend",ut=>{const kt=ut.changedTouches[0].clientY-Ct;if(Math.abs(kt)<30)return;const rt=Ye();kt<0&&rt<pt-1?(He(rt+1),J()):kt>0&&rt>0&&(He(rt-1),J())},{passive:!0})};Ee("pin-panel",()=>X,De=>X=De,D.length),Ee("pout-panel",()=>F,De=>F=De,P.length),(Kt=ee.querySelector("#psub-confirm"))==null||Kt.addEventListener("click",async De=>{if(De.preventDefault(),De.stopPropagation(),te)return;te=!0;const Ye=P[F],He=D[X];if(!Ye||!He)return;const pt=Ye._line,ot=(S[pt]||[]).findIndex(kt=>kt.cardId===Ye.cardId);if(ot===-1){y("Erreur : joueur introuvable","error"),ee.remove();return}const Ct={...He,_line:pt,position:Ye.position,used:!1,boost:0};S[pt].splice(ot,1,Ct);const ut=[...C,He.cardId];ee.remove(),re(Ye,He,async()=>{await j({[p+"Team"]:S,[u+"Team"]:m[u+"Team"],["usedSubIds_"+p]:ut})})})}document.body.appendChild(ee),J()}function re(w,L,S){const C={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const P=(U,X,te)=>`<div style="text-align:center">
      ...
      <div style="width:min(90vw,420px)">${hi(m[u+"Team"],m[u+"Formation"],null,[],It(),Nt())}</div>
    ...
    </div>`,(T=document.getElementById("pvp-home"))==null||T.addEventListener("click",()=>{try{h.removeChannel(Z)}catch{}it(e),l?g("mini-league",{openLeagueId:l}):g("home")})}Y()}async function Pi(e,t,i=!1){var s,d;const{state:n,navigate:r,toast:o}=t,a=i&&((d=(s=t==null?void 0:t.state)==null?void 0:s.params)==null?void 0:d.rankedData)||null;await _i(e,t,i?"ranked":"random",async({deckId:c,formation:l,starters:f,subsRaw:x,gcCardsEnriched:g,gcDefs:y,stadiumDef:p})=>{st(e);const u=async b=>{await zr(e,t,c,l,f,x,b||[],y,p,i,a)};if(!(g!=null&&g.length)){await u([]);return}vi(e,g,u)})}async function Ar(e,t,i){return Sr(e,t,i)}async function zr(e,t,i,n,r,o,a=[],s=[],d=null,c=!1,l=null){var ne,Z;const{state:f,navigate:x,toast:g}=t,y=(l==null?void 0:l.mmr)??((ne=f.profile)==null?void 0:ne.mmr)??1e3,p=c?Hi(y):null,u=c?"linear-gradient(135deg, #1a0a2e 0%, #2d1060 50%, #1a0a2e 100%)":"linear-gradient(135deg, #0a1a2e 0%, #0d3d1e 100%)",b=c?(p==null?void 0:p.color)||"#D4A017":"#FFD700";let _=!1,v=null,E=null;e.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:${Jt};border:1px solid ${Qt};border-radius:12px;padding:16px">
        ...
        ${Array.from({length:Math.max(0,a.status==="active"?a.current_day-1:a.current_day)},(j,I)=>I+1).reverse().map(j=>{const I=(d||[]).filter(K=>K.matchday===j);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:${Ze};margin-bottom:6px">Journée ${j}</div>${I.map(K=>kn(K,f,o,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
  ...
  </div>`}async function Kr(e,t,i,n){const{toast:r,state:o}=t,a=Wr(n.map(c=>c.id),i.mode),s=[];a.forEach((c,l)=>c.forEach(f=>s.push({league_id:i.id,matchday:l+1,home_id:f.home||f.bye,away_id:f.away||null,is_bye:!!f.bye,status:f.bye?"bye":"pending"})));const{error:d}=await h.from("mini_league_matches").insert(s);if(d){r("Erreur calendrier : "+d.message,"error");return}await h.from("mini_leagues").update({status:"active",current_day:1,total_days:a.length}).eq("id",i.id),r(`🚀 Lancée ! Pot : ${(i.pot||0).toLocaleString("fr")} cr.`,"success"),lt(e,t,i.id)}async function Vr(e,t,i){const{data:n}=await h.from("mini_leagues").select("current_day,total_days,pot").eq("id",i).single(),r=(n.current_day||0)+1;if(r>(n.total_days||0)){lt(e,t,i);return}await h.from("mini_leagues").update({current_day:r}).eq("id",i),t.toast(`Journée ${r} commencée !`,"success"),lt(e,t,i)}async function Yr(e,t,i,n,r){var x,g;const{state:o,toast:a}=t,s=[Math.floor((i.pot||0)*.7),Math.floor((i.pot||0)*.2),Math.floor((i.pot||0)*.1)],d=["🥇","🥈","🥉"][r],c=n.prize_amount||s[r]||0,l=n.prize_claimed,f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",f.innerHTML=`
    ...
    </div>`;let l=!1;const f=async u=>{if(!l){if(l=!0,clearInterval(y),g)try{g.unsubscribe()}catch{}u&&await Mt(e,t,u,r,{mlLeagueId:n,mlMatchId:i,myGC:o||[],gcDefs:a,stadiumDef:s}),d()}};(p=document.getElementById("wait-back"))==null||p.addEventListener("click",()=>{if(l=!0,clearInterval(y),g)try{g.unsubscribe()}catch{}c("mini-league",{openLeagueId:n}),d()});const x=async()=>{if(l)return;const{data:u}=await h.from("mini_league_matches").select("match_id").eq("id",i).single();u!=null&&u.match_id&&f(u.match_id)},g=h.channel(`ml_match_${i}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"mini_league_matches",filter:`id=eq.${i}`},u=>{var b;(b=u.new)!=null&&b.match_id&&f(u.new.match_id)}).subscribe(),y=setInterval(x,3e3);x()})}const Qr="/",Zr=[{emoji:"⚽",title:"Bienvenue dans Manager Wars !",color:"#1A6B3C",content:`<p>Tu es désormais un <strong>manager de football</strong> virtuel.</p>
    ...
    <p style="margin-top:12px;font-size:13px;color:#888">Tu peux revoir ce tutoriel depuis les paramètres à tout moment.</p>`}];function ea(e,t,i){let n=0;const r=document.createElement("div");r.id="tutorial-overlay",r.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.65);z-index:9900;display:flex;align-items:center;justify-content:center;padding:16px";const o=()=>{var l,f,x;const s=t[n],d=n===t.length-1,c=Math.round((n+1)/t.length*100);r.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    </div>`}function x(p){var A,q,ie,ne;const u=(A=p.card)==null?void 0:A.player;if(!u)return"";const b=((q=p.card)==null?void 0:q.evolution_bonus)||0,_=Dt(u,u.job,b),v=u.job2?Dt(u,u.job2,b):0,E=p.status==="sold",m=u.country_code?`https://flagsapi.com/${u.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,z=pi[u.job]||"#bbb",k=u.job2?pi[u.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${E?"opacity:0.7":""}">
      ${m?`<img src="${m}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      ${(ie=u.clubs)!=null&&ie.logo_url?`<img src="${u.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${z};display:flex;align-items:center;justify-content:center">
