  ...
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",oe=>{te=oe.target.value.toLowerCase(),ne()}),e.querySelectorAll(".filter-btn").forEach(oe=>{oe.addEventListener("click",()=>{j=oe.dataset.job,G(),ne()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{le=!le,G(),ne()})):(J.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{le=!le,G(),ne()})))}function ne(){const J=document.getElementById("col-grid");J&&(F==="player"?$e(J):F==="formation"?Se(J):F==="stadium"?xe(J):Le(J))}function fe(J,oe,K,Y,re){const ie=document.getElementById("col-grid"),ae=document.getElementById("col-big");if(!ie||!ae)return;var Te=0;function we(){const Ae=window.innerWidth>=768,Ce=document.getElementById("col-big"),De=document.getElementById("col-grid");Ae&&Ce&&(Ce.style.minHeight="420px",Ce.style.flex="1 1 auto"),Ae&&De&&(De.style.height=Math.round(310*.76+16)+"px",De.style.flexShrink="0",De.style.overflowX="auto",De.style.overflowY="hidden",De.style.alignItems="center",De.style.padding="8px 16px"),ae.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+oe(J[Te])+"</div>";var I=ae.querySelector("[data-card-id],[data-form-id],[data-gc-id]");I&&I.addEventListener("click",function(){Y(J[Te])}),requestAnimationFrame(function(){var q=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!q||!ae)){var w=ae.clientHeight-8,k=ae.clientWidth-24,f=q.offsetHeight,m=q.offsetWidth;if(f>0&&m>0&&w>40){var x=Ae?2.2:1,b=Math.min(w/f,k/m,x);q.style.transform="scale("+b.toFixed(3)+")",q.style.transformOrigin="top center"}}}),ie.innerHTML=J.map(function(q,w){var k=w===Te,f="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+(k?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+w+'" style="'+f+'">'+K(q,k)+"</div>"}).join(""),ie.querySelectorAll(".col-mini-item").forEach(function(q){q.addEventListener("click",function(){Te=Number(q.dataset.idx),we(),q.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}we()}function ye(J){var oe=window.innerWidth>=768?.76:.54,K;if(!J||J._fake){var Y=J?J.player:null;if(!Y)return"";K=Fi(Y)}else K=si(J,"");return'<div style="display:inline-block;zoom:'+oe+';pointer-events:none;line-height:0">'+K+"</div>"}function ze(J,oe,K){oe=oe||100,K=K||140;var Y=Pt[J]||{},re={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},ie=Math.max(3,Math.round(oe*.06)),ae=Object.entries(Y).map(function(we){var Ae=we[0],Ce=we[1],De=Ae.replace(/\d+$/,""),I=re[De]||"#888",q=Math.round(Ce.x*oe),w=Math.round(Ce.y*K);return'<circle cx="'+q+'" cy="'+w+'" r="'+ie+'" fill="'+I+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),Te=Math.max(1,Math.round(oe/50));return'<svg viewBox="0 0 '+oe+" "+K+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+oe+'" height="'+K+'" fill="#1A6B3C"/><rect x="'+Math.round(oe*.2)+'" y="'+Math.round(K*.02)+'" width="'+Math.round(oe*.6)+'" height="'+Math.round(K*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+Te+'"/><line x1="0" y1="'+Math.round(K*.5)+'" x2="'+oe+'" y2="'+Math.round(K*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+Te+'"/><ellipse cx="'+Math.round(oe*.5)+'" cy="'+Math.round(K*.5)+'" rx="'+Math.round(oe*.18)+'" ry="'+Math.round(K*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+Te+'"/><rect x="'+Math.round(oe*.2)+'" y="'+Math.round(K*.82)+'" width="'+Math.round(oe*.6)+'" height="'+Math.round(K*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+Te+'"/>'+ae+"</svg>"}function Ee(J,oe,K){var Y=K>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+K+"</div>":"",re=oe?'data-form-id="'+oe.id+'"':"",ie=J.length>7?14:J.length>5?16:19,ae=!!oe;return"<div "+re+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(ae?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(ae?"":"filter:grayscale(1);opacity:0.5")+'">'+Y+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(ae?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+ie+"px;font-weight:900;color:"+(ae?"#1A6B3C":"#aaa")+';line-height:1">'+J+'</div></div><div style="flex:1;overflow:hidden;background:'+(ae?"#1A6B3C":"#ccc")+'">'+ze(J,140,220)+"</div></div>"}function be(J,oe){var K=window.innerWidth>=768?.76:.54,Y=140,re=305,ie=Math.round(re*.22),ae=re-ie,Te=J.length>7?10:13,we=ze(J,Y,ae),Ae=oe?"1.5px solid #2a7a40":"1px solid #ddd",Ce=oe?"":"filter:grayscale(1);opacity:0.45;",De=oe?"#1A6B3C":"#bbb",I="#fff";return'<div style="display:inline-block;zoom:'+K+';line-height:0;pointer-events:none"><div style="width:'+Y+"px;height:"+re+"px;border-radius:6px;border:"+Ae+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+Ce+'"><div style="height:'+ie+"px;background:"+De+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+Te+"px;font-weight:900;color:"+I+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(Y-4)+'px">'+J+'</span></div><div style="height:'+ae+'px;overflow:hidden;flex-shrink:0">'+we+"</div></div></div>"}function $e(J){if(le){const oe=H();if(!oe.length){J.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const K=oe.map(Y=>o.find(re=>re.player.id===Y.id)||{_fake:!0,player:Y,id:"fake-"+Y.id});fe(K,Y=>Y._fake?Fi(Y.player):si(Y,""),Y=>Y._fake?ye({player:Y.player,id:"x",_fake:!0}):ye(Y),Y=>{Y._fake||Di(Y,o,N,t)})}else{const oe=W();if(!oe.length){J.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const K={};oe.forEach(re=>{const ie=re.player.id;K[ie]||(K[ie]=[]),K[ie].push(re)});const Y=Object.values(K).map(re=>Xo(re));fe(Y,re=>{const ie=N[re.player.id]||1,ae=ie>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${ie}</div>`:"",we=o.filter(Ae=>Ae.player.id===re.player.id&&Ae.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return si(re,ae+we)},re=>ye(re),re=>Di(re,o,N,t))}}function Se(J){const oe=le?M:[...V];if(!oe.length){J.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const K=oe.map(Y=>({formation:Y,card:g.find(re=>re.formation===Y)||null,owned:V.has(Y)}));fe(K,({formation:Y,card:re,owned:ie})=>Ee(Y,ie?re:null,ie?g.filter(ae=>ae.formation===Y).length:0),({formation:Y,owned:re})=>be(Y,re),({card:Y,owned:re})=>{re&&Y&&en(Y,g,t,d)})}function Le(J){const oe=Object.keys(z),K=le?oe.length?oe:L:[...C];if(!K.length){J.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const Y=K.map(re=>({type:re,gc:Ke[re]||{icon:"⚡",desc:""},def:z[re]||null,owned:C.has(re),card:n.find(ie=>ie.gc_type===re)||null}));fe(Y,({type:re,gc:ie,def:ae,owned:Te,card:we})=>{var x;const Ae=Te?n.filter(b=>b.gc_type===re).length:0,Ce=Ae>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${Ae}</div>`:"",De=(ae==null?void 0:ae.gc_type)==="ultra_game_changer",I={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},q={purple:"#b06ce0",light_blue:"#00d4ef"},w=I[ae==null?void 0:ae.color]||I.purple,k=q[ae==null?void 0:ae.color]||q.purple,f=(ae==null?void 0:ae.effect)||ie.desc||"",m=ae!=null&&ae.image_url?`/manager-wars/icons/${ae.image_url}`:((x=ae==null?void 0:ae.club)==null?void 0:x.logo_url)||(ae!=null&&ae.country_code?`https://flagcdn.com/w80/${ae.country_code.toLowerCase()}.png`:null);return Te&&we?`<div data-gc-id="${we.id}" data-gc-type="${re}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${k};background:${w};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${k}66;cursor:pointer">
          ${Ce}
    ...
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${g.map((K,Y)=>{const re=ne[K.id]||[],ie=K.is_for_sale,ae=re.length?re[re.length-1]:null,we=(n.rarity==="pepite"||n.rarity==="papyte")&&K.current_note!=null?` (☆${K.current_note})`:"";return`
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",n.note_g],["DEF",n.note_d],["MIL",n.note_m],["ATT",n.note_a]].map(([K,Y])=>{const re=jt[K],ie=Number(Y)||0;return`<div style="display:flex;flex-direction:column;align-items:center;gap:2px">
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(Le=document.getElementById("close-detail"))==null||Le.addEventListener("click",l);let ze=new Set;const Ee=()=>{const K=ze.size,Y=document.getElementById("sell-action-panel");if(!Y)return;Y.style.display=K>0?"block":"none",document.getElementById("sell-selected-count").textContent=K;const re=document.getElementById("evolve-btn");re&&(re.textContent=`⬆️ Évoluer ${K>1?"(+"+K+")":""}`)};document.querySelectorAll(".expl-check").forEach(K=>{K.addEventListener("change",()=>{const Y=K.dataset.id;K.checked?ze.add(Y):ze.delete(Y);const re=K.closest(".exemplaire-row");re&&(re.style.borderColor=K.checked?"#1A6B3C":"#eee"),Ee()})}),document.querySelectorAll(".exemplaire-row").forEach(K=>{K.addEventListener("click",Y=>{if(Y.target.closest("button")||Y.target.tagName==="INPUT")return;const re=K.querySelector(".expl-check");re&&!re.disabled&&(re.checked=!re.checked,re.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(K=>{K.addEventListener("click",Y=>{Y.stopPropagation();const re=document.querySelector(`.expl-hist[data-hist="${K.dataset.idx}"]`);re&&(re.style.display=re.style.display==="none"?"flex":"none")})}),(xe=document.getElementById("evolve-btn"))==null||xe.addEventListener("click",async()=>{const K=[...ze];if(!K.length)return;const Y=K.filter(we=>we!==e.id);if(Y.length,!(!Y.length&&K.length===1&&K[0]===e.id)){if(!Y.length){d("Sélectionne des copies à sacrifier","warning");return}}if(!await new Promise(we=>{const Ae=document.createElement("div");Ae.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",Ae.innerHTML=`
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    ${(()=>{const P=window.innerWidth>=700,W=($e,Se,Le)=>{var Ce,De;const xe=(t.gcDefs||[]).find(I=>I.name===$e.gc_type),J={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[xe==null?void 0:xe.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",oe={purple:"#b06ce0",light_blue:"#00d4ef"}[xe==null?void 0:xe.color]||"#b06ce0",K=(xe==null?void 0:xe.name)||$e.gc_type,Y=(xe==null?void 0:xe.effect)||((Ce=Ke[$e.gc_type])==null?void 0:Ce.desc)||"",re=xe!=null&&xe.image_url?`/manager-wars/icons/${xe.image_url}`:null,ie=((De=Ke[$e.gc_type])==null?void 0:De.icon)||"⚡",ae=Math.round(Le*.22),Te=Math.round(Le*.22),we=Le-ae-Te,Ae=K.length>12?7:9;return`<div class="gc-mini" data-gc-id="${$e.id}" data-gc-type="${$e.gc_type}"
