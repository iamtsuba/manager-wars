...
import{s as v,j as ki,q as zo,t as Bt,u as $i,F as Ei,o as it,n as Ht,g as Ct,r as Se,m as _t,v as gi,w as Mo,T as Ci,p as xn,x as bn,d as yn}from"./special-cards--9JnHrqo.js";const hn="/";function uo(t,{navigate:e,toast:i}){let o="login";const r=()=>{var a,s,l,c,d,f,b;const n=o==="login";if(t.innerHTML=`
  ...
  </div>`,t.querySelectorAll("[data-action]").forEach(a=>{a.addEventListener("click",()=>{a.style.transform="scale(.96)",setTimeout(()=>a.style.transform="",180);const s=a.dataset.action;if(s==="match-ai"){No(i,e);return}if(s==="match-random"){pt(),i("match",{matchMode:"random"});return}if(s==="match-friend"){i("friends");return}if(s==="mini-league"){i("mini-league");return}if(s==="ranked"){i("ranked");return}o("Bientôt disponible","info")})})}async function Oo(t,e){var f;const{state:i,navigate:o}=e,r=un(),n=Yi(),a=(f=i==null?void 0:i.profile)==null?void 0:f.is_admin,s="2026.07.27-1944";t.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    ${Se(t,{width:140})}
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),c=(s||[]).filter(M=>M.card_type==="player"&&M.player),d=(s||[]).filter(M=>M.card_type==="game_changer"),f=(s||[]).filter(M=>M.card_type==="formation"),b=(s||[]).filter(M=>M.card_type==="stadium"),{data:x}=await v.from("gc_definitions").select("name,gc_type,color,effect,image_url"),g={};(x||[]).forEach(M=>{g[M.name]=M});const{data:p}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),u={};(p||[]).forEach(M=>{u[M.id]=M}),b.forEach(M=>{M.stadium_def&&(u[M.stadium_id]=M.stadium_def)});const y=Object.keys(Ei),w=Object.keys(ze),h={};c.forEach(M=>{const Y=M.player.id;h[Y]=(h[Y]||0)+1}),new Set(Object.keys(h).map(M=>String(M)));const E=new Set(f.map(M=>M.formation)),m=new Set(d.map(M=>M.gc_type));let S="player",$="Tous",A="",F=!1;function H(){return[...c].sort((M,Y)=>{const q=li.indexOf(M.player.job),j=li.indexOf(Y.player.job);return q!==j?q-j:(M.player.surname_real||"").localeCompare(Y.player.surname_real||"")})}function U(){return[...l||[]].sort((M,Y)=>{const q=li.indexOf(M.job),j=li.indexOf(Y.job);return q!==j?q-j:(M.surname_real||"").localeCompare(Y.surname_real||"")})}function X(){return H().filter(M=>{const Y=M.player,q=$==="Tous"||Y.job===$,j=!A||`${Y.firstname} ${Y.surname_real}`.toLowerCase().includes(A);return q&&j})}function I(){return U().filter(M=>{const Y=$==="Tous"||M.job===$,q=!A||`${M.firstname} ${M.surname_real}`.toLowerCase().includes(A);return Y&&q})}t.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:0 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none;align-items:center"></div>
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",Y=>{A=Y.target.value.toLowerCase(),O()}),t.querySelectorAll(".filter-btn").forEach(Y=>{Y.addEventListener("click",()=>{$=Y.dataset.job,_(),O()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{F=!F,_(),O()})):(M.innerHTML=`
      ...
      <div style="flex:1;min-width:220px;display:flex;flex-direction:column;gap:12px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",d.note_g],["DEF",d.note_d],["MIL",d.note_m],["ATT",d.note_a]].map(([q,j])=>{const W=Jt[q],C=q==="GK"?"#fff":W,R=q===d.job||q===d.job2,ce=(Number(j)||0)+(R&&S>0?S:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(le=document.getElementById("close-detail"))==null||le.addEventListener("click",s);let _=new Set;const O=d.rarity==="pepite"?1.3:d.rarity==="papyte"?.7:1;function J(){let q=0;return document.querySelectorAll(".expl-check:checked").forEach(j=>{if(j.dataset.id===t.id)return;const C=Number(j.dataset.evo)||0,R=Number(j.dataset.note)||0;q+=R+C}),Math.round(q*O)}const re=()=>{const q=_.size,j=document.getElementById("sell-action-panel");if(!j)return;j.style.display=q>0?"block":"none",document.getElementById("sell-selected-count").textContent=q;const W=document.getElementById("evolve-btn");if(W){const C=J();W.textContent=C>0?`⬆️ Évoluer (+${C})`:"⬆️ Évoluer"}};document.querySelectorAll(".expl-check").forEach(q=>{q.addEventListener("change",()=>{const j=q.dataset.id;q.checked?_.add(j):_.delete(j);const W=q.closest(".exemplaire-row");if(W){const C=W.querySelector(".expl-sel-overlay"),R=W.querySelector(".expl-sel-check");C&&(C.style.display=q.checked?"block":"none"),R&&(R.style.display=q.checked?"flex":"none"),W.style.transform=q.checked?"scale(1.05)":"scale(1)"}re()})}),document.querySelectorAll(".exemplaire-row").forEach(q=>{q.addEventListener("click",j=>{if(j.target.tagName==="INPUT")return;const W=q.querySelector(".expl-check");W&&!W.disabled&&(W.checked=!W.checked,W.dispatchEvent(new Event("change")))})}),(me=document.getElementById("evolve-btn"))==null||me.addEventListener("click",async()=>{if(b<=1)return;const q=[..._];if(!q.length)return;if(_.has(t.id)){const ge=document.createElement("div");ge.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",ge.innerHTML=`
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff;position:relative">
      ...
      <!-- Pagination -->
      ${n.length>1?`<div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">${n.map((X,I)=>`<div style="width:6px;height:6px;border-radius:50%;background:${I===d?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}</div>`:""}
    ...
    </div>`,requestAnimationFrame(()=>requestAnimationFrame(function X(){const I=t.querySelector(".deck-preview-wrap"),_=t.querySelector("#deck-swipe-zone");if(!I||!_||!y)return;const O=_.clientWidth>=900,J=Math.max(200,_.clientHeight-(O?60:40)),de=Math.max(200,_.clientWidth-16),B=O?Math.min(117,Math.max(52,Math.round(de*.22))):Math.max(44,Math.round(de*.168));if(J<220||de<220){requestAnimationFrame(X);return}const K=O?null:Math.round(B*.55);I.innerHTML=Ut(y,u,null,[],de,J,[],K),I.style.cssText=`width:${de}px;height:${J}px;overflow:hidden;margin:${O?0:60}px auto 0`;const le=I.querySelector("svg");le&&(le.style.cssText="display:block;width:100%;height:100%",le.setAttribute("preserveAspectRatio",O?"xMidYMid meet":"none"))})),(A=document.getElementById("prev-deck"))==null||A.addEventListener("click",()=>{d>0&&(d--,f())}),(F=document.getElementById("next-deck"))==null||F.addEventListener("click",()=>{d<n.length-1&&(d++,f())}),(H=document.getElementById("validate-deck"))==null||H.addEventListener("click",()=>{if(!h)return;const X=e.state.params||{};e.navigate("match",{...X,matchMode:X.matchMode||i,deckId:b.id})}),(U=document.getElementById("cancel-deck-select"))==null||U.addEventListener("click",()=>{Ke(t),r("home")});const E=document.getElementById("deck-swipe-zone");if(E){let X=0,I=0;E.addEventListener("touchstart",_=>{X=_.touches[0].clientX,I=_.touches[0].clientY},{passive:!0}),E.addEventListener("touchend",_=>{const O=_.changedTouches[0].clientX-X,J=_.changedTouches[0].clientY-I;Math.abs(O)<40||Math.abs(O)<Math.abs(J)||(O<0&&d<n.length-1?(d++,f()):O>0&&d>0&&(d--,f()))},{passive:!0})}}f()}function wt(t,e=44,i=58,o=null){return Se(t,{width:e,showStad:!!o,stadDef:o,used:t==null?void 0:t.used})}function Qe(t,e,i,o,r){if(!(t!=null&&t.length))return"";const n=t.slice(0,5);let a='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return n.forEach((s,l)=>{const c=s._line||s.job||"MIL";let d=s.boost||0;if(s.stadiumBonus&&(o==="attack"&&(c==="ATT"||c==="MIL")||o==="defense"&&(c==="GK"||c==="DEF"||c==="MIL")?d+=10:o||(d+=10)),a+=Se({...s,_evolution_bonus:0,evolution_bonus:0},{width:40,role:c,extraNote:d}),l<n.length-1){const f=n[l+1],b=Ho(s,f,r)?Bt(s,f):null;a+=`<div style="width:7px;height:3px;background:${!b||b==="#ff3333"||b==="#cc2222"?"rgba(255,255,255,0.12)":b};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(a+=`<div style="margin-left:6px;background:${e};color:${e==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),a+="</div>",a}function Ti(t,e,i,o,r=310,n=310,a=[],s=null){const l=_t[e]||{},c=$i(e)||Ei[e]||[],d={},f=["ATT","MIL","DEF","GK"];for(const y of f)(t[y]||[]).forEach((h,E)=>{d[`${y}${E+1}`]=h});function b(y){const w=l[y];return w?{x:w.x*r,y:w.y*n}:null}let x="";for(const[y,w]of c){const h=b(y),E=b(w);if(!h||!E)continue;const m=d[y],S=d[w],$=Bt(m,S);$==="#00ff88"||$==="#FFD700"?(x+=`<line x1="${h.x.toFixed(1)}" y1="${h.y.toFixed(1)}" x2="${E.x.toFixed(1)}" y2="${E.y.toFixed(1)}"
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${e.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...e.log].reverse().map(B=>{var K,le,me;if(B.type==="duel"){const fe=B.isGoal,M=B.homeScored?"#FFD700":fe?"#ff6b6b":"rgba(255,255,255,0.3)",Y=B.homeScored?"⚽ BUT !":fe?"⚽ BUT IA !":(K=B.homePlayers)!=null&&K.length?"⚔️ Attaque":"🛡️ Défense",q=(B.homeTotal??0)>=(B.aiTotal??0);return`<div style="padding:8px;border-radius:8px;background:${fe?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${M};margin-bottom:4px">
    ...
    </div>`,(m=f.querySelector("#sub-close"))==null||m.addEventListener("click",()=>f.remove()),(S=f.querySelector("#out-up"))==null||S.addEventListener("click",()=>{s>0&&(s--,b())}),($=f.querySelector("#out-down"))==null||$.addEventListener("click",()=>{s<n.length-1&&(s++,b())}),(A=f.querySelector("#in-up"))==null||A.addEventListener("click",()=>{c>0&&(c--,b())}),(F=f.querySelector("#in-down"))==null||F.addEventListener("click",()=>{c<a.length-1&&(c++,b())});const E=(U,X,I,_)=>{const O=f.querySelector("#"+U);if(!O)return;let J=0;O.addEventListener("touchstart",re=>{J=re.touches[0].clientY},{passive:!0}),O.addEventListener("touchend",re=>{const de=re.changedTouches[0].clientY-J;if(Math.abs(de)<30)return;const B=X();de<0&&B<_-1?(I(B+1),b()):de>0&&B>0&&(I(B-1),b())},{passive:!0})};E("in-panel",()=>c,U=>c=U,a.length),E("out-panel",()=>s,U=>s=U,n.length),(H=f.querySelector("#sub-confirm"))==null||H.addEventListener("click",U=>{if(U.preventDefault(),U.stopPropagation(),d)return;d=!0;const X=n[s],I=a[c];if(!X||!I)return;let _=null,O=-1;for(const[re,de]of Object.entries(e.homeTeam)){const B=(de||[]).findIndex(K=>K.cardId===X.cardId);if(B!==-1){_=re,O=B;break}}if(O===-1||!_){It("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),f.remove();return}const J={...I,_line:_,_col:X._col||0,used:!1,boost:0};e.homeTeam[_].splice(O,1,J),e.usedSubIds||(e.usedSubIds=[]),e.usedSubIds.push(I.cardId),e.subsUsed++,e.selected=[],e.log.push({type:"sub",subSide:"home",clubName:e.clubName,outPlayer:{name:X.name,firstname:X.firstname,note:Pe(X,_),portrait:Ct(X),job:X.job,country_code:X.country_code,rarity:X.rarity,clubName:X.clubName,clubLogo:X.clubLogo},inPlayer:{name:I.name,firstname:I.firstname,note:Pe(I,_),portrait:Ct(I),job:I.job,country_code:I.country_code,rarity:I.rarity,clubName:I.clubName,clubLogo:I.clubLogo},text:`🔄 ${I.firstname} ${I.name} remplace ${X.firstname} ${X.name}`}),f.remove(),wr(X,I,()=>$e(t,e,i))})}document.body.appendChild(f),b()}function jr(t,e,i,o,r){var b,x;const n=(o.gcDefs||[]).find(g=>g.name===e),a=ze[e]||{icon:"⚡",desc:"Carte spéciale."},s=(n==null?void 0:n.name)||e,l=(n==null?void 0:n.effect)||a.desc,c=n!=null&&n.image_url?`/icons/${n.image_url}`:null,d=a.icon||"⚡",f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",f.innerHTML=`
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      </div>`,(Ge=te.querySelector("#psub-close"))==null||Ge.addEventListener("click",()=>te.remove()),(Te=te.querySelector("#pout-up"))==null||Te.addEventListener("click",()=>{N>0&&(N--,ee())}),(Ye=te.querySelector("#pout-down"))==null||Ye.addEventListener("click",()=>{N<G.length-1&&(N++,ee())}),(Oe=te.querySelector("#pin-up"))==null||Oe.addEventListener("click",()=>{Z>0&&(Z--,ee())}),(ot=te.querySelector("#pin-down"))==null||ot.addEventListener("click",()=>{Z<P.length-1&&(Z++,ee())});const Ie=(De,We,He,ft)=>{const nt=te.querySelector("#"+De);if(!nt)return;let qt=0;nt.addEventListener("touchstart",mt=>{qt=mt.touches[0].clientY},{passive:!0}),nt.addEventListener("touchend",mt=>{const Lt=mt.changedTouches[0].clientY-qt;if(Math.abs(Lt)<30)return;const rt=We();Lt<0&&rt<ft-1?(He(rt+1),ee()):Lt>0&&rt>0&&(He(rt-1),ee())},{passive:!0})};Ie("pin-panel",()=>Z,De=>Z=De,P.length),Ie("pout-panel",()=>N,De=>N=De,G.length),(Vt=te.querySelector("#psub-confirm"))==null||Vt.addEventListener("click",async De=>{if(De.preventDefault(),De.stopPropagation(),ie)return;ie=!0;const We=G[N],He=P[Z];if(!We||!He)return;const ft=We._line,nt=(z[ft]||[]).findIndex(Lt=>Lt.cardId===We.cardId);if(nt===-1){g("Erreur : joueur introuvable","error"),te.remove();return}const qt={...He,_line:ft,position:We.position,used:!1,boost:0};z[ft].splice(nt,1,qt);const mt=[...D,He.cardId];te.remove(),W(We,He,async()=>{await I({[p+"Team"]:z,[u+"Team"]:m[u+"Team"],["usedSubIds_"+p]:mt})})})}document.body.appendChild(te),ee()}function W(k,L,z){const D={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const G=(V,Z,ie)=>`<div style="text-align:center">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div id="mkt-content" style="padding:10px 16px;display:flex;flex-direction:column;gap:8px"></div>
    ...
    </div>`}function b(p){var A,F,H,U;const u=(A=p.card)==null?void 0:A.player;if(!u)return"";const y=((F=p.card)==null?void 0:F.evolution_bonus)||0,w=mi(u,u.job,y),h=u.job2?mi(u,u.job2,y):0,E=p.status==="sold",m=u.country_code?`https://flagsapi.com/${u.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,S=So[u.job]||"#bbb",$=u.job2?So[u.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${E?"opacity:0.7":""}">
      ...
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${S};display:flex;align-items:center;justify-content:center">
          <span style="font-size:14px;font-weight:900;color:${S};font-family:Arial Black,Arial">${w}</span>
        </div>
      ...
      <div style="flex:1;min-width:0">
        ...
        <div style="font-size:10px;color:${E?"#22c55e":"#999"};margin-top:1px">
          ${E?`✅ Vendu à ${((U=p.buyer)==null?void 0:U.pseudo)||"—"} · ${p.sold_at?new Date(p.sold_at).toLocaleDateString("fr"):""}`:`🟢 En vente depuis le ${new Date(p.listed_at).toLocaleDateString("fr")}`}
        </div>
      </div>
      <div style="text-align:right;flex-shrink:0">
  ...
  <div class="page">
    ...
    <nav class="top-nav">
      <div class="logo" id="nav-logo" title="Manager Wars v2026.07.27-1944" style="cursor:pointer">
