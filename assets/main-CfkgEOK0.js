import{s as k,F as et,b as Me,c as Ft,l as ge,d as ai}from"./formation-links-CDBKB_z4.js";function _t(e,{navigate:t,toast:i}){e.innerHTML=`
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),n=(d||[]).filter($=>$.card_type==="player"&&$.player),r=(d||[]).filter($=>$.card_type==="game_changer"),u=(d||[]).filter($=>$.card_type==="formation"),{data:m}=await k.from("gc_definitions").select("name,gc_type,color,effect,image_url"),f={};(m||[]).forEach($=>{f[$.name]=$});const _=Object.keys(et),p=Object.keys(de),y={};n.forEach($=>{const b=$.player.id;y[b]=(y[b]||0)+1}),new Set(Object.keys(y).map($=>String($)));const E=new Set(u.map($=>$.formation)),h=new Set(r.map($=>$.gc_type));let g="player",v="Tous",L="",I=!1;function x(){return[...n].sort(($,b)=>{const S=je.indexOf($.player.job),A=je.indexOf(b.player.job);return S!==A?S-A:($.player.surname_encoded||"").localeCompare(b.player.surname_encoded||"")})}function T(){return[...l||[]].sort(($,b)=>{const S=je.indexOf($.job),A=je.indexOf(b.job);return S!==A?S-A:($.surname_encoded||"").localeCompare(b.surname_encoded||"")})}function q(){return x().filter($=>{const b=$.player,S=v==="Tous"||b.job===v,A=!L||`${b.firstname} ${b.surname_encoded}`.toLowerCase().includes(L);return S&&A})}function R(){return T().filter($=>{const b=v==="Tous"||$.job===v,S=!L||`${$.firstname} ${$.surname_encoded}`.toLowerCase().includes(L);return b&&S})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const $e=1e3;function xi(e,t,i,s){var p,y,E;const{state:c,toast:o,closeModal:a,navigate:d,refreshProfile:l}=i,n=e.formation,r={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function u(){const h=Me[n]||{},g=et[n]||[],v=290,L=360,I=20;function x(q){const R=h[q];return R?{x:R.x*v,y:R.y*L}:null}let T=`<svg width="${v}" height="${L}" viewBox="0 0 ${v} ${L}" xmlns="http://www.w3.org/2000/svg">`;for(const[q,R]of g){const P=x(q),H=x(R);!P||!H||(T+=`<line x1="${P.x}" y1="${P.y}" x2="${H.x}" y2="${H.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const q of Object.keys(h)){const R=x(q);if(!R)continue;const P=q.replace(/\d+/,""),H=r[P]||"#555";T+=`<circle cx="${R.x}" cy="${R.y}" r="${I}" fill="${H}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,T+=`<text x="${R.x}" y="${R.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${P}</text>`}return T+="</svg>",T}const m=t.filter(h=>h.formation===n),f=m.length,_=!e.is_for_sale;s(`Formation ${n}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}const Oi="simulation",Kt="VOTRE_ZONE_ID";function Hi(){switch(Oi){case"propellerads":return Ki();case"adsense":return Ui();default:return rt()}}function rt(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:#111;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:14px";let i=5;t.innerHTML=`
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order");let l=0;function n(){var h,g,v,L,I;const r=o[l],u=(d||[]).filter(x=>x.deck_id===r.id),m=u.filter(x=>{var T;return x.is_starter&&((T=x.card)==null?void 0:T.player)}).map(x=>We(x.card,x.position)),f=u.find(x=>{var T;return((T=x.card)==null?void 0:T.card_type)==="formation"}),_=r.formation||((h=f==null?void 0:f.card)==null?void 0:h.formation)||"4-4-2",p=m.length>=11?Ne(m,_):null,y=m.length>=11;Vt(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
  ...
  <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div id="duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center, rgba(10,61,30,0.4), rgba(10,61,30,0.92))"></div>
          ...
          <button id="start-match-btn" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,0.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">
            ▶ Commencer le match
          </button>`,L.style.transition="opacity .45s ease",L.style.opacity="1",L.style.pointerEvents="auto",(I=document.getElementById("start-match-btn"))==null||I.addEventListener("click",_))},600)},700)},2800)}function Qi(e){if(e.type==="duel"){const t=e.homeTotal>=e.aiTotal;return`
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    ${(()=>{const x=window.innerWidth>=700,T=(z,B,N)=>{var U,X;const j=(t.gcDefs||[]).find(te=>te.name===z.gc_type),$={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[j==null?void 0:j.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",b={purple:"#b06ce0",light_blue:"#00d4ef"}[j==null?void 0:j.color]||"#b06ce0",S=(j==null?void 0:j.name)||z.gc_type,A=(j==null?void 0:j.effect)||((U=de[z.gc_type])==null?void 0:U.desc)||"",w=j!=null&&j.image_url?`/manager-wars/icons/${j.image_url}`:null,D=((X=de[z.gc_type])==null?void 0:X.icon)||"⚡",M=Math.round(N*.22),G=Math.round(N*.22),F=N-M-G,K=S.length>12?7:9;return`<div class="gc-mini" data-gc-id="${z.id}" data-gc-type="${z.gc_type}"
        ...
        </div>`},q=(z,B)=>{var N;return`<div id="boost-card"
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(o);let d=!1;const l=()=>{d||(d=!0,o.remove(),setTimeout(()=>c(),50))};o.addEventListener("click",l),setTimeout(l,3500)}async function ve(e,t,i){var r,u;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:s}=i,c=t.homeScore>t.aiScore,o=t.homeScore===t.aiScore,a=c?"victoire":o?"nul":"defaite",d=pi(t.mode,a);t.matchId&&await k.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:c?s.profile.id:null,home_credits_reward:d,played_at:new Date().toISOString()}).eq("id",t.matchId);const l={credits:(s.profile.credits||0)+d,matches_played:(s.profile.matches_played||0)+1};c?l.wins=(s.profile.wins||0)+1:o?l.draws=(s.profile.draws||0)+1:l.losses=(s.profile.losses||0)+1,await k.from("users").update(l).eq("id",s.profile.id),await i.refreshProfile();const n=document.createElement("div");n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",n.innerHTML=`
    ...
    </div>`,C)return;const $=n==="p1"?f.home_id:f.away_id;k.from("cards").select("id,gc_type").eq("owner_id",$).eq("card_type","game_changer").then(async({data:S})=>{const{data:A}=await k.from("gc_definitions").select("*").eq("is_active",!0),w=new Set,D=(S||[]).filter(G=>w.has(G.gc_type)?!1:(w.add(G.gc_type),!0)).map(G=>({...G,_gcDef:(A==null?void 0:A.find(F=>F.name===G.gc_type))||null})),M=document.getElementById("pvp-gc-grid");if(M){if(!D.length){M.innerHTML='<div style="color:rgba(255,255,255,0.4);font-size:13px;text-align:center;margin-top:30px">Aucune carte Game Changer disponible.</div>';return}M.innerHTML=D.map(G=>j(G,B.includes(G.id))).join(""),M.querySelectorAll(".pvp-gc-card").forEach(G=>{G.addEventListener("click",()=>{const F=G.dataset.id,K=B.indexOf(F);K>-1?B.splice(K,1):B.length<N&&B.push(F),L()})})}}),(b=e.querySelector("#pvp-gc-validate"))==null||b.addEventListener("click",async()=>{const S=n==="p1"?f.home_id:f.away_id,{data:A}=await k.from("cards").select("id,gc_type").eq("owner_id",S).eq("card_type","game_changer").in("id",B),{data:w}=await k.from("gc_definitions").select("*").eq("is_active",!0),D=(A||[]).map(M=>({...M,_gcDef:(w==null?void 0:w.find(G=>G.name===M.gc_type))||null}));await g({["gcReady_"+n]:!0,["gcCardsFull_"+n]:D}),p["gcReady_"+r]&&await g({phase:"reveal"})})}function x(){e.style.overflow="hidden",e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    </div>`,setTimeout(()=>{const A=document.getElementById("pvp-score-me"),w=document.getElementById("pvp-score-opp"),D=document.getElementById("pvp-midfield-result");A&&w&&(b?(A.style.fontSize="80px",A.style.color="#FFD700",w.style.opacity="0.25"):(w.style.fontSize="80px",w.style.color="#ff6b6b",A.style.opacity="0.25"));const M=p.p1Team.MIL||[],G=p.p2Team.MIL||[],F=B(M)+N(M),K=B(G)+N(G),U=F>=K?"p1":"p2";p[U+"Name"];let X=p.boostValue;X==null&&(X=Ut(),p.boostValue=X,p.boostOwner=U,p.boostUsed=!1),D&&(D.style.opacity="1",D.innerHTML=`<div style="font-size:20px;font-weight:900;margin-bottom:10px">
      ...
      </div>`,document.body.appendChild(G),(F=G.querySelector("#pvp-gc-back"))==null||F.addEventListener("click",()=>G.remove()),(K=G.querySelector("#pvp-gc-use"))==null||K.addEventListener("click",async()=>{G.remove();const U=p["usedGc_"+n]||[];U.push(C),await g({["usedGc_"+n]:U})})}function W(){var j;const C=p[n+"Score"],z=p[r+"Score"],B=C>z,N=C===z;e.style.overflow="hidden",e.innerHTML=`
