import{s as h,l as U,F as te,b as $e,c as ye,d as ot}from"./formation-links-BXiJqmnj.js";function Ae(e,{navigate:t,toast:i}){e.innerHTML=`
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),l=(d||[]).filter(E=>E.card_type==="player"&&E.player),c=(d||[]).filter(E=>E.card_type==="game_changer"),u=(d||[]).filter(E=>E.card_type==="formation"),p=Object.keys(te),g=Object.keys(ce),x={};l.forEach(E=>{const T=E.player.id;x[T]=(x[T]||0)+1});const $=new Set(Object.keys(x).map(E=>String(E))),b=new Set(u.map(E=>E.formation)),w=new Set(c.map(E=>E.gc_type));let v="player",y="Tous",m="",f=!1;function k(){return[...l].sort((E,T)=>{const L=ne.indexOf(E.player.job),j=ne.indexOf(T.player.job);return L!==j?L-j:(E.player.surname_encoded||"").localeCompare(T.player.surname_encoded||"")})}function A(){return[...a||[]].sort((E,T)=>{const L=ne.indexOf(E.job),j=ne.indexOf(T.job);return L!==j?L-j:(E.surname_encoded||"").localeCompare(T.surname_encoded||"")})}function _(){return k().filter(E=>{const T=E.player,L=y==="Tous"||T.job===y,j=!m||`${T.firstname} ${T.surname_encoded}`.toLowerCase().includes(m);return L&&j})}function I(){return A().filter(E=>{const T=y==="Tous"||E.job===y,L=!m||`${E.firstname} ${E.surname_encoded}`.toLowerCase().includes(m);return T&&L})}e.innerHTML=`
  <div class="page">
    ...
    <div class="page-body">
      <div style="display:flex;flex-wrap:wrap;gap:10px;justify-content:flex-start" id="col-grid"></div>
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",T=>{m=T.target.value.toLowerCase(),z()}),e.querySelectorAll(".filter-btn").forEach(T=>{T.addEventListener("click",()=>{y=T.dataset.job,M(),z()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{f=!f,M(),z()})):(E.innerHTML=`
        <div style="display:flex;justify-content:flex-end">
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{f=!f,M(),z()})))}function z(){const E=document.getElementById("col-grid");E&&(v==="player"?C(E):v==="formation"?R(E):O(E))}function C(E){if(f){const T=I();if(!T.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}E.innerHTML=T.map(L=>{if($.has(String(L.id))){const S=l.find(H=>H.player.id===L.id),D=x[L.id]||1,q=D>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${D}</div>`:"",K=l.filter(H=>H.player.id===L.id&&H.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Me(S,q+K)}return ct(L)}).join("")}else{const T=_();if(!T.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const L={},j=[];T.forEach(S=>{const D=S.player.id;L[D]||(L[D]=!0,j.push(S))}),E.innerHTML=j.map(S=>{const D=x[S.player.id]||1,q=D>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${D}</div>`:"",K=l.filter(H=>H.player.id===S.player.id&&H.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Me(S,q+K)}).join("")}E.querySelectorAll("[data-card-id]").forEach(T=>{T.addEventListener("click",()=>{const L=l.find(j=>j.id===T.dataset.cardId);L&&gt(L,l,x,t)})})}function R(E){const T=f?p:[...b];if(!T.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}E.innerHTML=T.map(L=>{if(b.has(L)){const S=u.find(P=>P.formation===L),D=u.filter(P=>P.formation===L).length,q=D>1?`<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${D}</div>`:"";return`
          <div data-form-id="${S.id}" style="
            ...
            <div style="font-weight:900;font-size:18px">${L}</div>
          </div>`}return`
        <div style="
          ...
          <div style="font-weight:900;font-size:18px">${L}</div>
        </div>`}).join(""),E.querySelectorAll("[data-form-id]").forEach(L=>{L.addEventListener("click",()=>{const j=u.find(S=>S.id===L.dataset.formId);j&&ut(j,u,t,n)})})}function O(E){const T=f?g:[...w];if(!T.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}E.innerHTML=T.map(L=>{const j=w.has(L),S=ce[L]||{icon:"⚡"};if(j){const D=c.find(K=>K.gc_type===L),q=c.filter(K=>K.gc_type===L).length,P=q>1?`<div style="position:absolute;top:4px;right:4px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${q}</div>`:"";return`
          <div data-gc-id="${D.id}" data-gc-type="${L}" style="
        ...
        <div style="
          ...
          <div style="font-weight:700;font-size:13px">${L}</div>
        </div>`}).join(""),E.querySelectorAll("[data-gc-id]").forEach(L=>{L.addEventListener("click",()=>ft(L.dataset.gcType,n))})}e.querySelectorAll(".col-tab-btn").forEach(E=>{E.addEventListener("click",()=>{v=E.dataset.tab,y="Tous",m="",f=!1,e.querySelectorAll(".col-tab-btn").forEach(T=>{const L=T.dataset.tab===v;T.style.borderBottomColor=L?"var(--green)":"transparent",T.style.color=L?"var(--green)":"var(--gray-600)"}),M(),z()})}),M(),z()}function ft(e,t){const i=ce[e]||{icon:"⚡",desc:"Effet spécial."};t("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Z=1e3;function ut(e,t,i,o){var $,b,w;const{state:r,toast:n,closeModal:s,navigate:d,refreshProfile:a}=i,l=e.formation,c={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function u(){const v=$e[l]||{},y=te[l]||[],m=290,f=360,k=20;function A(I){const M=v[I];return M?{x:M.x*m,y:M.y*f}:null}let _=`<svg width="${m}" height="${f}" viewBox="0 0 ${m} ${f}" xmlns="http://www.w3.org/2000/svg">`;for(const[I,M]of y){const z=A(I),C=A(M);!z||!C||(_+=`<line x1="${z.x}" y1="${z.y}" x2="${C.x}" y2="${C.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const I of Object.keys(v)){const M=A(I);if(!M)continue;const z=I.replace(/\d+/,""),C=c[z]||"#555";_+=`<circle cx="${M.x}" cy="${M.y}" r="${k}" fill="${C}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,_+=`<text x="${M.x}" y="${M.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${z}</text>`}return _+="</svg>",_}const p=t.filter(v=>v.formation===l),g=p.length,x=!e.is_for_sale;o(`Formation ${l}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
      ...
      <div style="width:140px;border-radius:12px;padding:6px;background:${m};flex-shrink:0">
        <div style="background:#f2e8d2;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
          ...
          <div style="background:#f2e8d2;padding:3px 6px;display:flex;align-items:center;justify-content:space-between;min-height:24px">
            ...
            ${(k=c.clubs)!=null&&k.logo_url?`<img src="${c.clubs.logo_url}" style="width:22px;height:16px;object-fit:contain">`:`<div style="background:#1a3a7a;color:#fff;border-radius:2px;padding:1px 3px;font-size:6px;font-weight:800">${(((A=c.clubs)==null?void 0:A.encoded_name)||"").slice(0,6)}</div>`}
          </div>
        </div>
      </div>
  ...
  <div class="page">
    ...
    <div class="page-body">
      <button class="btn btn-primary" id="save-deck" style="width:100%" ${n<11?"disabled":""}>
  ...
  </div>`,yt(e,t,r,i),document.getElementById("builder-back").addEventListener("click",()=>o("decks")),document.getElementById("formation-select").addEventListener("change",l=>{t.formation=l.target.value;const c=Ce(t.formation),u={};c.forEach(p=>{t.slots[p]&&(u[p]=t.slots[p])}),t.slots=u,J(e,t,i)}),document.getElementById("save-deck").addEventListener("click",()=>vt(t,i)),e.querySelectorAll("[data-remove-sub]").forEach(l=>{l.addEventListener("click",()=>{t.subs=t.subs.filter(c=>c!==l.dataset.removeSub),J(e,t,i)})}),(a=document.getElementById("add-sub-btn"))==null||a.addEventListener("click",()=>{xt(t,e,i)})}function yt(e,t,i,o){const r=document.getElementById("deck-field");if(!r)return;const n=t.formation,s=$e[n]||{},d=ye?ye(n)||te[n]||[]:te[n]||[],a={};i.forEach(y=>{const m=t.slots[y],f=m?t.playerCards.find(k=>k.id===m):null;a[y]=(f==null?void 0:f.player)||null});const l=i.filter(y=>y.startsWith("MIL"));let c=0,u=0;l.forEach(y=>{const m=a[y];m&&(c+=Number(m.note_m)||0)}),d.forEach(([y,m])=>{if(!y.startsWith("MIL")||!m.startsWith("MIL"))return;U(a[y],a[m])!=="#ff3333"&&u++});const p=320,g=320,x=28,$=4;function b(y){const m=s[y];return m?{x:m.x*p,y:m.y*g}:null}let w="";for(const[y,m]of d){const f=b(y),k=b(m);if(!f||!k)continue;const A=a[y],_=a[m],I=U(A,_);if(I==="#00ff88"||I==="#FFD700")w+=`<line x1="${f.x}" y1="${f.y}" x2="${k.x}" y2="${k.y}"
        stroke="${I}" stroke-width="${$*3}" stroke-linecap="round" opacity="0.25"/>`,w+=`<line x1="${f.x}" y1="${f.y}" x2="${k.x}" y2="${k.y}"
        stroke="${I}" stroke-width="${$}" stroke-linecap="round" opacity="0.95"/>`;else{const z=I==="#ff3333"||I==="#cc2222"?.75:.9;w+=`<line x1="${f.x}" y1="${f.y}" x2="${k.x}" y2="${k.y}"
        stroke="${I}" stroke-width="${$}" stroke-linecap="round" opacity="${z}"/>`}}for(const y of i){const m=b(y);if(!m)continue;const f=a[y],k=y.replace(/\d+/,""),_={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[k]||"#555",I=f?Number(k==="GK"?f.note_g:k==="DEF"?f.note_d:k==="MIL"?f.note_m:f.note_a)||0:null,M=f?(f.surname_encoded||"").slice(0,8).toUpperCase():"";if(f){const z=ke(f);z&&(w+=`
          <defs>
            <clipPath id="clip-${y}">
        ...
        <circle cx="${m.x}" cy="${m.y}" r="${x}" fill="${z?"transparent":_}"
          stroke="${_}" stroke-width="2" />
        <circle cx="${m.x}" cy="${m.y}" r="${x}" fill="${_}" opacity="${z?"0.5":"1"}"
    ...
    <svg viewBox="0 0 ${p} ${g}" width="100%" style="display:block;max-width:380px;margin:0 auto">
      ${v}
      ${w}
    </svg>`,r.querySelectorAll(".deck-slot-hit").forEach(y=>{y.addEventListener("click",()=>mt(y.dataset.pos,t,e,o))})}function mt(e,t,i,o){var p,g,x;const{openModal:r,closeModal:n}=o,s=e.replace(/\d+/,""),d=t.slots[e],a=d?t.playerCards.find($=>$.id===d):null;(p=a==null?void 0:a.player)==null||p.id;const l=new Set;Object.entries(t.slots).forEach(([$,b])=>{var v;if($===e||!b)return;const w=t.playerCards.find(y=>y.id===b);(v=w==null?void 0:w.player)!=null&&v.id&&l.add(w.player.id)}),t.subs.forEach($=>{var w;const b=t.playerCards.find(v=>v.id===$);(w=b==null?void 0:b.player)!=null&&w.id&&l.add(b.player.id)});const c=new Set,u=t.playerCards.filter($=>{const b=$.player;return!(b.job===s||b.job2===s)||l.has(b.id)||c.has(b.id)?!1:(c.add(b.id),!0)});u.sort(($,b)=>{const w=s==="GK"?$.player.note_g:s==="DEF"?$.player.note_d:s==="MIL"?$.player.note_m:$.player.note_a;return(s==="GK"?b.player.note_g:s==="DEF"?b.player.note_d:s==="MIL"?b.player.note_m:b.player.note_a)-w}),r(`Choisir ${s} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${u.length>0?u.map($=>{var m,f;const b=$.player,w=s==="GK"?b.note_g:s==="DEF"?b.note_d:s==="MIL"?b.note_m:b.note_a,v=ke(b),y={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[b.rarity];return`<div class="player-option" data-card-id="${$.id}"
    ...
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(g=document.getElementById("close-selector"))==null||g.addEventListener("click",n),(x=document.getElementById("remove-player"))==null||x.addEventListener("click",()=>{delete t.slots[e],n(),J(i,t,o)}),document.querySelectorAll(".player-option").forEach($=>{$.addEventListener("click",()=>{t.slots[e]=$.dataset.cardId,n(),J(i,t,o)})})}function xt(e,t,i){var a;const{openModal:o,closeModal:r}=i,n=new Set;Object.values(e.slots).filter(Boolean).forEach(l=>{var u;const c=e.playerCards.find(p=>p.id===l);(u=c==null?void 0:c.player)!=null&&u.id&&n.add(c.player.id)}),e.subs.forEach(l=>{var u;const c=e.playerCards.find(p=>p.id===l);(u=c==null?void 0:c.player)!=null&&u.id&&n.add(c.player.id)});const s=new Set,d=e.playerCards.filter(l=>{var c,u,p;return n.has((c=l.player)==null?void 0:c.id)||s.has((u=l.player)==null?void 0:u.id)?!1:(s.add((p=l.player)==null?void 0:p.id),!0)});o("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
  ...
  `,document.body.appendChild(o);let r=!1;document.getElementById("pack-visual").addEventListener("click",()=>{if(r)return;r=!0;const p=document.getElementById("pack-visual");p.classList.add("shaking"),setTimeout(()=>{p.classList.add("pack-open"),setTimeout(()=>{document.getElementById("pack-phase").style.display="none",s(0)},600)},500)});let n=0;function s(p){n=p;const g=document.getElementById("reveal-phase");g.style.display="flex",d(p)}function d(p){var f;const g=e[p],x=document.getElementById("card-counter"),$=document.getElementById("single-card-slot"),b=document.getElementById("card-tap-hint");x&&(x.textContent=`Carte ${p+1} / ${e.length}`),b&&(b.textContent=p<e.length-1?"Appuie pour continuer →":"Appuie pour voir toutes tes cartes");const w=je(g),v=g.card_type==="player"&&((f=g.player)==null?void 0:f.rarity)==="legende";$.innerHTML=`
      ...
      </div>`,v&&c();const y=document.getElementById("current-card-wrap");let m=!1;y.addEventListener("click",()=>{if(m)return;m=!0;const k=p+1;k<e.length?(y.style.transition="all 0.25s ease",y.style.transform="translateX(-120%) rotate(-15deg)",y.style.opacity="0",setTimeout(()=>d(k),250)):a()})}function a(){u(),document.getElementById("reveal-phase").style.display="none";const p=document.getElementById("recap-phase");p.style.display="flex";const g=document.getElementById("recap-grid");g.innerHTML=e.map((x,$)=>`
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}function zt(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:#000;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:16px";let i=5;t.innerHTML=`<div style="font-size:48px">📺</div>
    ...
    <div style="text-align:center;padding:40px;color:#fff">
      ...
      <button class="btn btn-primary" id="msg-btn">${o}</button>
        ...
        card:cards(id, card_type, formation,
          player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
            note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
            clubs(encoded_name,logo_url)))`).eq("deck_id",l).order("slot_order")]),p=(u||[]).filter(k=>{var A;return k.is_starter&&((A=k.card)==null?void 0:A.player)}).map(k=>xe(k.card)),g=(u||[]).filter(k=>{var A;return!k.is_starter&&((A=k.card)==null?void 0:A.player)}).map(k=>xe(k.card));if(p.length<11){Ve(e,"⚠️",`Deck incomplet (${p.length}/11).`,"Compléter",()=>o("decks"));return}const x=(u||[]).find(k=>{var A;return((A=k.card)==null?void 0:A.card_type)==="formation"}),$=(c==null?void 0:c.formation)||((f=x==null?void 0:x.card)==null?void 0:f.formation)||"4-4-2",{data:b}=await h.from("cards").select("id,gc_type").eq("owner_id",i.profile.id).eq("card_type","game_changer"),w=Ye(p,$),v=await Bt($),{data:y}=await h.from("matches").insert({home_id:i.profile.id,away_id:null,mode:a,home_deck_id:l,status:"in_progress"}).select().single(),m={matchId:y==null?void 0:y.id,mode:a,difficulty:d,formation:$,homeTeam:w,aiTeam:v,homeSubs:g,subsUsed:0,maxSubs:Math.min(g.length,3),homeScore:0,aiScore:0,gcCards:b||[],usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Dt(e,m,t)}async function Ft(e,t,i){const{state:o,navigate:r}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:n}=await h.from("decks").select("id,name,is_active,formation").eq("owner_id",o.profile.id).order("created_at",{ascending:!1});if(!n||n.length===0){Ve(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>r("decks"));return}const s=n.map(c=>c.id),{data:d}=await h.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",s).order("slot_order");let a=0;function l(){var v,y,m,f,k;const c=n[a],u=(d||[]).filter(A=>A.deck_id===c.id),p=u.filter(A=>{var _;return A.is_starter&&((_=A.card)==null?void 0:_.player)}).map(A=>xe(A.card)),g=u.find(A=>{var _;return((_=A.card)==null?void 0:_.card_type)==="formation"}),x=c.formation||((v=g==null?void 0:g.card)==null?void 0:v.formation)||"4-4-2",$=p.length>=11?Ye(p,x):null,b=p.length>=11;e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:calc(100dvh - 130px);overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">
        ${n.map((A,_)=>`<div style="width:7px;height:7px;border-radius:50%;background:${_===a?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}
      </div>`:""}
    ...
    </div>`,(y=document.getElementById("prev-deck"))==null||y.addEventListener("click",()=>{a>0&&(a--,l())}),(m=document.getElementById("next-deck"))==null||m.addEventListener("click",()=>{a<n.length-1&&(a++,l())}),(f=document.getElementById("validate-deck"))==null||f.addEventListener("click",()=>{b&&t.navigate("match",{matchMode:i,deckId:c.id})}),(k=document.getElementById("cancel-deck-select"))==null||k.addEventListener("click",()=>r("home"));const w=document.getElementById("deck-swipe-zone");if(w){let A=0,_=0;w.addEventListener("touchstart",I=>{A=I.touches[0].clientX,_=I.touches[0].clientY},{passive:!0}),w.addEventListener("touchend",I=>{const M=I.changedTouches[0].clientX-A,z=I.changedTouches[0].clientY-_;Math.abs(M)<40||Math.abs(M)<Math.abs(z)||(M<0&&a<n.length-1?(a++,l()):M>0&&a>0&&(a--,l()))},{passive:!0})}}l()}function Dt(e,t,i){const o=t.homeTeam.MIL||[],r=t.aiTeam.MIL||[];function n(u){return u.reduce((p,g)=>p+F(g,"MIL"),0)}function s(u){let p=0;for(let g=0;g<u.length-1;g++){const x=U(u[g],u[g+1]);x!=="#ff3333"&&x!=="#cc2222"&&p++}return p}const d=n(o)+s(o),a=n(r)+s(r),l=d>=a;function c(u,p,g){return`<div style="text-align:center">
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${u.map((x,$)=>{const b=G(x),w=$<u.length-1?U(x,u[$+1]):null,v=w&&w!=="#ff3333"&&w!=="#cc2222";return`
  ...
  </div>`}function re(e,t,i){if(!(e!=null&&e.length))return"";const o=e.slice(0,5);let r='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return o.forEach((n,s)=>{if(r+=W(n,40,52),s<o.length-1){const d=U(n,o[s+1]);r+=`<div style="width:7px;height:3px;background:${d==="#ff3333"||d==="#cc2222"?"rgba(255,255,255,0.12)":d};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(r+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),r+="</div>",r}function Je(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function Xe(e,t,i,o,r=310,n=310){const s=$e[t]||{},d=ye(t)||te[t]||[],a={},l=["ATT","MIL","DEF","GK"];for(const v of l)(e[v]||[]).forEach((m,f)=>{a[`${v}${f+1}`]=m});function c(v){const y=s[v];return y?{x:y.x*r,y:y.y*n}:null}let u="";for(const[v,y]of d){const m=c(v),f=c(y);if(!m||!f)continue;const k=a[v],A=a[y],_=U(k,A);_==="#00ff88"||_==="#FFD700"?(u+=`<line x1="${m.x.toFixed(1)}" y1="${m.y.toFixed(1)}" x2="${f.x.toFixed(1)}" y2="${f.y.toFixed(1)}"
        stroke="${_}" stroke-width="10" stroke-linecap="round" opacity="0.22"/>`,u+=`<line x1="${m.x.toFixed(1)}" y1="${m.y.toFixed(1)}" x2="${f.x.toFixed(1)}" y2="${f.y.toFixed(1)}"
        stroke="${_}" stroke-width="3.5" stroke-linecap="round" opacity="0.95"/>`):u+=`<line x1="${m.x.toFixed(1)}" y1="${m.y.toFixed(1)}" x2="${f.x.toFixed(1)}" y2="${f.y.toFixed(1)}"
        stroke="${_}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const p=48,g=64,x=13,$=16,b={normal:"#aaaaaa",pépite:"#D4A017",papyte:"#111111",légende:"#7a28b8"};for(const[v,y]of Object.entries(a)){const m=c(v);if(!m||!y)continue;const f=v.replace(/[0-9]/g,""),k=Q[f]||"#555",A=i==="attack"&&["MIL","ATT"].includes(f)&&!y.used||i==="defense"&&["GK","DEF","MIL"].includes(f)&&!y.used,_=o.includes(y.cardId);let I;i==="attack"?I=f==="MIL"?Number(y.note_m)||0:Number(y.note_a)||0:i==="defense"?I=f==="GK"?Number(y.note_g)||0:f==="MIL"?Number(y.note_m)||0:Number(y.note_d)||0:I=Number(f==="GK"?y.note_g:f==="DEF"?y.note_d:f==="MIL"?y.note_m:y.note_a)||0,I=I+(y.boost||0);const M=(m.x-p/2).toFixed(1),z=(m.y-g/2).toFixed(1),C=y.used?.25:1,R=b[y==null?void 0:y.rarity]||b.normal,O=_?"#ffffff":R,E=_?3:(y==null?void 0:y.rarity)==="légende"||(y==null?void 0:y.rarity)==="pépite"?2.5:2,T=g-x-$;u+=`<defs><clipPath id="cp-${v}"><rect x="${M}" y="${(m.y-g/2+x).toFixed(1)}" width="${p}" height="${T}"/></clipPath></defs>`,u+=`<rect x="${M}" y="${z}" width="${p}" height="${g}" rx="5" fill="${k}" opacity="${C}"/>`;const L=G(y);L&&!y.used&&(u+=`<image href="${L}" x="${M}" y="${(m.y-g/2+x).toFixed(1)}" width="${p}" height="${T}" clip-path="url(#cp-${v})" preserveAspectRatio="xMidYMin slice"/>`),u+=`<rect x="${M}" y="${z}" width="${p}" height="${x}" rx="4" fill="rgba(255,255,255,0.92)"/>`,u+=`<text x="${m.x.toFixed(1)}" y="${(m.y-g/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(y.name||"").slice(0,9)}</text>`;const j=(m.y+g/2-$).toFixed(1);if(u+=`<rect x="${M}" y="${j}" width="${p}" height="${$}" fill="rgba(255,255,255,0.92)"/>`,y.used)u+=`<text x="${m.x.toFixed(1)}" y="${(m.y+g/2-$/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="13" font-weight="900" fill="rgba(0,0,0,0.4)" font-family="Arial Black">–</text>`;else{u+=`<text x="${(m.x-13).toFixed(1)}" y="${(m.y+g/2-$/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${Je(y.country_code)}</text>`,u+=`<text x="${m.x.toFixed(1)}" y="${(m.y+g/2-$/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${I}</text>`;const S=We(y);S?u+=`<image href="${S}" x="${(m.x+p/2-14).toFixed(1)}" y="${(m.y+g/2-$+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:y.clubName&&(u+=`<text x="${(m.x+14).toFixed(1)}" y="${(m.y+g/2-$/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(y.clubName||"").slice(0,3).toUpperCase()}</text>`),y.boost&&(u+=`<rect x="${(m.x+p/2-12).toFixed(1)}" y="${(m.y-g/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,u+=`<text x="${(m.x+p/2-5).toFixed(1)}" y="${(m.y-g/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${y.boost}</text>`)}u+=`<rect x="${M}" y="${z}" width="${p}" height="${g}" rx="5" fill="${_?"rgba(255,255,255,0.12)":"none"}" stroke="${O}" stroke-width="${E}" opacity="${C}"/>`,A&&(u+=`<rect x="${M}" y="${z}" width="${p}" height="${g}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${_?"selected":""}" data-card-id="${y.cardId}" data-role="${f}" style="cursor:pointer"/>`)}const w=38;return`<svg viewBox="${-w} ${-w} ${r+w*2} ${n+w*2}" width="100%" style="display:block;width:100%;max-width:380px;margin:0 auto">
    ${u}
      ...
      <div style="flex:1;display:grid;grid-template-columns:repeat(4,1fr);gap:3px;align-content:start">
        ${u.map(f=>{var k;return`
          <div class="gc-mini" data-gc-id="${f.id}" data-gc-type="${f.gc_type}"
            ...
            <div style="font-size:16px">${((k=ce[f.gc_type])==null?void 0:k.icon)||"⚡"}</div>
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(f=>{var k,A,_;if(f.type==="duel"){const I=f.isGoal,M=f.homeScored?"#FFD700":I?"#ff6b6b":"rgba(255,255,255,0.3)",z=f.homeScored?"⚽ BUT !":I?"⚽ BUT IA !":(k=f.homePlayers)!=null&&k.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${I?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${M};margin-bottom:4px">
                <div style="font-size:9px;color:${M};letter-spacing:1px;margin-bottom:5px;font-weight:700;text-transform:uppercase">${z}</div>
                ${(A=f.homePlayers)!=null&&A.length?`<div style="margin-bottom:3px">${re(f.homePlayers,"rgba(255,255,255,0.7)",f.homeTotal)}</div>`:""}
                ${(_=f.aiPlayers)!=null&&_.length?`<div style="opacity:0.7">${re(f.aiPlayers,"#ff6b6b",f.aiTotal)}</div>`:""}
  ...
  `,document.body.appendChild(o);let c=!1;const u=()=>{c||(c=!0,o.remove(),i())};o.addEventListener("click",u),setTimeout(u,2e3)}function ee(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function Se(e,t,i,o=null){if(t.phase!=="attack"){ee("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){ee(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const r=Object.values(t.homeTeam).flat().filter(c=>c.used),n=t.homeSubs.filter(c=>!t.usedSubIds.includes(c.cardId));if(!r.length){ee("Aucun joueur utilisé à remplacer");return}if(!n.length){ee("Aucun remplaçant disponible");return}let s=0,d=Math.max(0,n.findIndex(c=>c.cardId===o));const a=document.createElement("div");a.id="sub-overlay",a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function l(){var w,v,y,m,f,k;const c=r[s],u=n[d],p=Math.min(130,Math.round((window.innerWidth-80)/2)),g=Math.round(p*1.35);a.innerHTML=`
    ...
    </div>`,(w=document.getElementById("sub-close"))==null||w.addEventListener("click",()=>a.remove()),(v=document.getElementById("out-up"))==null||v.addEventListener("click",()=>{s>0&&(s--,l())}),(y=document.getElementById("out-down"))==null||y.addEventListener("click",()=>{s<r.length-1&&(s++,l())}),(m=document.getElementById("in-up"))==null||m.addEventListener("click",()=>{d>0&&(d--,l())}),(f=document.getElementById("in-down"))==null||f.addEventListener("click",()=>{d<n.length-1&&(d++,l())});let x=0;const $=document.getElementById("in-panel");$==null||$.addEventListener("touchstart",A=>{x=A.touches[0].clientY},{passive:!0}),$==null||$.addEventListener("touchend",A=>{const _=A.changedTouches[0].clientY-x;Math.abs(_)<30||(_<0&&d<n.length-1?(d++,l()):_>0&&d>0&&(d--,l()))},{passive:!0});let b=!1;(k=document.getElementById("sub-confirm"))==null||k.addEventListener("click",A=>{if(A.preventDefault(),A.stopPropagation(),b)return;b=!0;const _=r[s],I=n[d];if(!_||!I)return;let M=null,z=-1;for(const[R,O]of Object.entries(t.homeTeam)){const E=(O||[]).findIndex(T=>T.cardId===_.cardId);if(E!==-1){M=R,z=E;break}}if(z===-1||!M){ee("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),a.remove();return}const C={...I,_line:M,_col:_._col||0,used:!1,boost:0};t.homeTeam[M].splice(z,1,C),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(I.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:_.name,firstname:_.firstname,note:F(_,M),portrait:G(_),job:_.job,country_code:_.country_code,rarity:_.rarity,clubName:_.clubName,clubLogo:_.clubLogo},inPlayer:{name:I.name,firstname:I.firstname,note:F(I,M),portrait:G(I),job:I.job,country_code:I.country_code,rarity:I.rarity,clubName:I.clubName,clubLogo:I.clubLogo},text:`🔄 ${I.firstname} ${I.name} remplace ${_.firstname} ${_.name}`}),a.remove(),Pt(_,I,()=>N(e,t,i))})}l(),document.body.appendChild(a)}function Kt(e,t,i,o,r){if(!o.usedGc.includes(e)){switch(o.usedGc.push(e),t){case"Double attaque":o.modifiers.home.doubleAttack=!0,o.log.push({text:"⚡ Double attaque activée !",type:"info"});break;case"Bouclier":o.modifiers.home.shield=!0,o.log.push({text:"🛡️ Bouclier activé !",type:"info"});break;case"Ressusciter":{let n=!1;for(const s of["ATT","MIL","DEF","GK"]){const d=(o.homeTeam[s]||[]).find(a=>a.used);if(d){d.used=!1,n=!0;break}}o.log.push({text:n?"💫 Joueur ressuscité !":"💫 Aucun joueur à ressusciter",type:"info"});break}case"Vol de note":o.modifiers.ai.stolenNote=(o.modifiers.ai.stolenNote||0)+1,o.log.push({text:"🎯 -1 à la prochaine attaque IA",type:"info"});break;case"Gel":{const n=[...o.aiTeam.ATT||[],...o.aiTeam.MIL||[]].filter(s=>!s.used);if(n.length){const s=n.sort((d,a)=>F(a,"ATT")-F(d,"ATT"))[0];s.used=!0,o.log.push({text:`❄️ ${s.name} (IA) gelé !`,type:"info"})}break}case"Remplacement+":o.maxSubs++,o.log.push({text:"🔄 +1 remplacement débloqué",type:"info"});break}h.from("cards").delete().eq("id",e).then(()=>{}),N(i,o,r)}}function Ht(e,t,i){const o=Object.values(t.homeTeam).flat().filter(r=>!r.used);if(!o.length){i.toast("Aucun joueur actif à booster","error");return}i.openModal("⚡ Utiliser le Boost",`<div style="margin-bottom:12px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border-radius:10px;padding:12px;text-align:center;color:#000">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(r=>{r.addEventListener("click",()=>{const n=r.dataset.cardId;for(const s of["GK","DEF","MIL","ATT"]){const d=(t.homeTeam[s]||[]).find(a=>a.cardId===n);if(d){d.boost=(d.boost||0)+t.boostCard.value,t.log.push({text:`⚡ Boost +${t.boostCard.value} appliqué à ${d.name}`,type:"info"});break}}t.boostUsed=!0,i.closeModal(),N(e,t,i)})})}function tt(e,t,i,o,r){const n=document.createElement("div");n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:900;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;overflow:hidden;cursor:pointer";const s=Array.from({length:10},(l,c)=>`
  ...
  `;function d(a){const l=document.getElementById("mkt-content"),c=a==="buy"?s:n;if(c.length===0){l.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      ${c.map(u=>{var b,w,v;const p=(b=u.card)==null?void 0:b.player;if(!p)return"";const g=p.job==="GK"?p.note_g:p.job==="DEF"?p.note_d:p.job==="MIL"?p.note_m:p.note_a,x=Vt[p.rarity],$=(i.profile.credits||0)>=u.price;return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px">
          ...
          <div style="text-align:right;flex-shrink:0">
            ...
            ${a==="buy"?`<button class="btn btn-primary btn-sm" data-buy="${u.id}" ${$?"":"disabled"} style="margin-top:4px">${$?"Acheter":"Trop cher"}</button>`:`<button class="btn btn-danger btn-sm" data-cancel="${u.id}" style="margin-top:4px">Retirer</button>`}
          </div>
        </div>`}).join("")}
