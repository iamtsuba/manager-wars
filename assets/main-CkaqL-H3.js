...
import{s as v,l as Ht,m as Bi,F as qi,r as Ce,j as ii,h as Di,n as Kn,o as Vn,T as bi,k as Yn,p as Wn,b as Xn}from"./formation-links-Bb-Ce4un.js";const Jn="/";function Qi(e,{navigate:t,toast:i}){let n="login";const r=()=>{var a,s,d,c,l,f;const o=n==="login";e.innerHTML=`
    <div style="
  ...
  `;function s(){var x;const c=document.getElementById("logo-preview"),l=document.getElementById("logo-initials"),f=((x=document.getElementById("setup-club"))==null?void 0:x.value)||"MW",y=f.trim().split(" ").filter(Boolean),g=y.length>=2?(y[0][0]+y[1][0]).toUpperCase():f.slice(0,2).toUpperCase();c&&(c.style.background=a,c.style.borderColor=o),l&&(l.textContent=g,l.style.color=o)}document.getElementById("color1").addEventListener("input",c=>{o=c.target.value,document.getElementById("swatch1").style.background=o,s()}),document.getElementById("color2").addEventListener("input",c=>{a=c.target.value,document.getElementById("swatch2").style.background=a,s()});function d(c){document.querySelectorAll(".setup-step").forEach(l=>l.classList.remove("active")),document.getElementById(`step-${c}`).classList.add("active"),document.getElementById("step-num").textContent=c,document.getElementById("progress-fill").style.width=`${Math.round(c/3*100)}%`,c===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("step1-error");if(l.textContent="",c.length<3){l.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:f}=await v.from("users").select("id").eq("pseudo",c).maybeSingle();if(f){l.textContent="Ce pseudo est déjà pris.";return}d(2)}),document.getElementById("step2-back").addEventListener("click",()=>d(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const c=document.getElementById("setup-club").value.trim(),l=document.getElementById("step2-error");if(l.textContent="",c.length<2){l.textContent="Nom trop court (min. 2 caractères).";return}const{data:f}=await v.from("users").select("id").eq("club_name",c).maybeSingle();if(f){l.textContent="Ce nom de club est déjà pris.";return}d(3)}),document.getElementById("step3-back").addEventListener("click",()=>d(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("setup-club").value.trim(),f=document.getElementById("step3-error"),y=document.getElementById("step3-finish");f.textContent="",y.disabled=!0,y.textContent="Création en cours…";try{const{error:g}=await v.from("users").insert({id:t.user.id,pseudo:c,club_name:l,club_color1:o,club_color2:a,credits:1e4});if(g)throw g;await Zn(t.user.id),await r(),n(`Bienvenue ${c} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(g){f.textContent=g.message,y.disabled=!1,y.textContent="🚀 Créer mon profil !"}})}async function Zn(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await v.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const eo="modulepreload",to=function(e){return"/"+e},Zi={},mn=function(t,i,n){let r=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),s=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));r=Promise.allSettled(i.map(d=>{if(d=to(d),d in Zi)return;Zi[d]=!0;const c=d.endsWith(".css"),l=c?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${l}`))return;const f=document.createElement("link");if(f.rel=c?"stylesheet":eo,c||(f.as="script"),f.crossOrigin="",f.href=d,s&&f.setAttribute("nonce",s),document.head.appendChild(f),c)return new Promise((y,g)=>{f.addEventListener("load",y),f.addEventListener("error",()=>g(new Error(`Unable to preload CSS for ${d}`)))})}))}function o(a){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=a,window.dispatchEvent(s),!s.defaultPrevented)throw a}return r.then(a=>{for(const s of a||[])s.status==="rejected"&&o(s.reason);return t().catch(o)})},Xt="#1A6B3C",Jt="#cc2222",io="#D4A017",en="#888";async function no(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    <div style="display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:8px;padding:10px 0;border-bottom:1px solid #f0f0f0">
      ...
      <div style="text-align:left;font-size:18px;font-weight:900;color:${I}">${A}</div>
    ...
    </style>`}const so="2026.07.21-1156";async function tn(e,{state:t,navigate:i,toast:n}){var o,a;const r=t.profile;r&&(e.innerHTML=`
  <style>
    .home-dark {
      min-height: 100%;
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    ${Ce(e,{width:140})}
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),c=(s||[]).filter(C=>C.card_type==="player"&&C.player),l=(s||[]).filter(C=>C.card_type==="game_changer"),f=(s||[]).filter(C=>C.card_type==="formation"),y=(s||[]).filter(C=>C.card_type==="stadium"),{data:g}=await v.from("gc_definitions").select("name,gc_type,color,effect,image_url"),x={};(g||[]).forEach(C=>{x[C.name]=C});const{data:p}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),u={};(p||[]).forEach(C=>{u[C.id]=C}),y.forEach(C=>{C.stadium_def&&(u[C.stadium_id]=C.stadium_def)});const b=Object.keys(qi),w=Object.keys(qe),h={};c.forEach(C=>{const F=C.player.id;h[F]=(h[F]||0)+1}),new Set(Object.keys(h).map(C=>String(C)));const $=new Set(f.map(C=>C.formation)),m=new Set(l.map(C=>C.gc_type));let A="player",k="Tous",I="",q=!1;function ne(){return[...c].sort((C,F)=>{const O=ni.indexOf(C.player.job),Z=ni.indexOf(F.player.job);return O!==Z?O-Z:(C.player.surname_real||"").localeCompare(F.player.surname_real||"")})}function oe(){return[...d||[]].sort((C,F)=>{const O=ni.indexOf(C.job),Z=ni.indexOf(F.job);return O!==Z?O-Z:(C.surname_real||"").localeCompare(F.surname_real||"")})}function ie(){return ne().filter(C=>{const F=C.player,O=k==="Tous"||F.job===k,Z=!I||`${F.firstname} ${F.surname_real}`.toLowerCase().includes(I);return O&&Z})}function j(){return oe().filter(C=>{const F=k==="Tous"||C.job===k,O=!I||`${C.firstname} ${C.surname_real}`.toLowerCase().includes(I);return F&&O})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:0 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none;align-items:center"></div>
  ...
  </div>`,function(F){const O=document.getElementById(F);if(!O)return;O.addEventListener("wheel",ue=>{Math.abs(ue.deltaY)<=Math.abs(ue.deltaX)||(ue.preventDefault(),O.scrollLeft+=ue.deltaY)},{passive:!1});let Z=!1,H=0,B=0,R=!1,le=!1;O.addEventListener("mousedown",ue=>{Z=!0,R=!1,H=ue.pageX,B=O.scrollLeft}),window.addEventListener("mouseup",()=>{Z=!1,R&&(O.style.cursor="",le=!0),R=!1}),window.addEventListener("mousemove",ue=>{if(!Z)return;const be=ue.pageX-H;!R&&Math.abs(be)<6||(R=!0,O.style.cursor="grabbing",ue.preventDefault(),O.scrollLeft=B-be)}),O.addEventListener("click",ue=>{le&&(ue.stopPropagation(),ue.preventDefault(),le=!1)},!0)}("col-grid");function z(){const C=document.getElementById("col-filters");C&&(A==="player"?(C.innerHTML=`
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",F=>{I=F.target.value.toLowerCase(),Y()}),e.querySelectorAll(".filter-btn").forEach(F=>{F.addEventListener("click",()=>{k=F.dataset.job,z(),Y()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{q=!q,z(),Y()})):(C.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{q=!q,z(),Y()})))}function Y(){const C=document.getElementById("col-grid");C&&(A==="player"?fe(C):A==="formation"?N(C):A==="stadium"?U(C):V(C))}function J(C,F,O,Z,H,B=0){const R=document.getElementById("col-grid"),le=document.getElementById("col-big");if(!R||!le)return;var ue=0;function be(){var Ue;const we=window.innerWidth>=768,Te=document.getElementById("col-big"),Ae=document.getElementById("col-gap"),ke=((Ue=document.getElementById("col-grid"))==null?void 0:Ue.parentElement)||null,$e=Te?Te.closest(".page"):null;if(Te&&ke&&$e){let Ee=0;Array.from($e.children).forEach(function(L){L!==Te&&L!==ke&&L!==Ae&&(Ee+=L.offsetHeight)});const Le=Math.max(0,$e.clientHeight-Ee),Fe=Math.round(Le*(B/100)),_=Math.max(0,Le-Fe),E=we?50/71:45/63,T=Math.round(_*E),M=Math.max(0,_-T);Ae&&(Ae.style.height=Fe+"px"),Te.style.flex="none",Te.style.height=T+"px",Te.style.minHeight="0",ke.style.flex="none",ke.style.height=M+"px",ke.style.minHeight="0",ke.style.display="flex",ke.style.overflow="hidden"}const je=document.getElementById("col-grid");je&&(je.style.height="100%",je.style.flexShrink="0",je.style.overflowX="auto",je.style.overflowY="hidden",je.style.alignItems="center",je.style.width="100%"),le.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:center center">'+F(C[ue])+"</div>";var De=le.querySelector("[data-card-id],[data-form-id],[data-gc-id]");De&&De.addEventListener("click",function(){Z(C[ue])}),requestAnimationFrame(function(){var Ee=document.getElementById("big-card-inner");if(!(!Ee||!le)){var Le=le.clientHeight,Fe=le.clientWidth-16,_=Ee.offsetHeight,E=Ee.offsetWidth;if(_>0&&E>0&&Le>40){var T=Math.min(Le/_,Fe/E);Ee.style.transform="scale("+T.toFixed(3)+")",Ee.style.transformOrigin="center center"}}}),R.innerHTML=C.map(function(Ee,Le){var Fe=Le===ue,_="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+(Fe?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+Le+'" style="'+_+'">'+O(Ee,Fe)+"</div>"}).join(""),R.querySelectorAll(".col-mini-item").forEach(function(Ee){Ee.addEventListener("click",function(){ue=Number(Ee.dataset.idx),be(),Ee.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})}),requestAnimationFrame(function(){var Ee=je?je.clientHeight:0,Le=R.querySelector(".col-mini-item > div");if(!(!Ee||!Le)){var Fe=Le.style.zoom;Le.style.zoom="1";var _=Le.offsetHeight;if(Le.style.zoom=Fe,!(_<=0)){var E=Ee/_;R.querySelectorAll(".col-mini-item > div").forEach(function(T){T.style.zoom=E.toFixed(4)})}}})}be()}function pe(C){var F=window.innerWidth>=768?.76:.54,O;if(!C||C._fake){var Z=C?C.player:null;if(!Z)return"";O=an(Z)}else O=wi(C,"");return'<div style="display:inline-block;zoom:'+F+';pointer-events:none;line-height:0">'+O+"</div>"}function ge(C,F,O){F=F||100,O=O||140;var Z=ii[C]||{},H={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},B=Math.max(3,Math.round(F*.06)),R=Object.entries(Z).map(function(ue){var be=ue[0],we=ue[1],Te=be.replace(/\d+$/,""),Ae=H[Te]||"#888",ke=Math.round(we.x*F),$e=Math.round(we.y*O);return'<circle cx="'+ke+'" cy="'+$e+'" r="'+B+'" fill="'+Ae+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),le=Math.max(1,Math.round(F/50));return'<svg viewBox="0 0 '+F+" "+O+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+F+'" height="'+O+'" fill="#1A6B3C"/><rect x="'+Math.round(F*.2)+'" y="'+Math.round(O*.02)+'" width="'+Math.round(F*.6)+'" height="'+Math.round(O*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+le+'"/><line x1="0" y1="'+Math.round(O*.5)+'" x2="'+F+'" y2="'+Math.round(O*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+le+'"/><ellipse cx="'+Math.round(F*.5)+'" cy="'+Math.round(O*.5)+'" rx="'+Math.round(F*.18)+'" ry="'+Math.round(O*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+le+'"/><rect x="'+Math.round(F*.2)+'" y="'+Math.round(O*.82)+'" width="'+Math.round(F*.6)+'" height="'+Math.round(O*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+le+'"/>'+R+"</svg>"}function S(C,F,O){var Z=O>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+O+"</div>":"",H=F?'data-form-id="'+F.id+'"':"",B=C.length>7?14:C.length>5?16:19,R=!!F;return"<div "+H+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(R?"#2a7a40":"#bbb")+";background:var(--tile-bg);display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(R?"":"filter:grayscale(1);opacity:0.5")+'">'+Z+'<div style="padding:8px 6px 6px;background:var(--tile-bg);text-align:center;border-bottom:3px solid '+(R?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+B+"px;font-weight:900;color:"+(R?"#1A6B3C":"#aaa")+';line-height:1">'+C+'</div></div><div style="flex:1;overflow:hidden;background:'+(R?"#1A6B3C":"#ccc")+'">'+ge(C,140,220)+"</div></div>"}function W(C,F){var O=window.innerWidth>=768?.76:.54,Z=140,H=Math.round(140*657/507),B=Math.round(H*.28),R=H-B,le=C.length>7?9:11,ue=ge(C,Z,R),be=F?"1.5px solid #2a7a40":"1px solid #ddd",we=F?"":"filter:grayscale(1);opacity:0.45;",Te=F?"#1A6B3C":"#bbb",Ae="#fff";return'<div style="display:inline-block;zoom:'+O+';line-height:0;pointer-events:none"><div style="width:'+Z+"px;height:"+H+"px;border-radius:6px;border:"+be+";background:var(--tile-bg);display:flex;flex-direction:column;overflow:hidden;"+we+'"><div style="height:'+B+"px;background:"+Te+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+le+"px;font-weight:900;color:"+Ae+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(Z-4)+'px;line-height:1.2;display:inline-block">'+C+'</span></div><div style="height:'+R+'px;overflow:hidden;flex-shrink:0">'+ue+"</div></div></div>"}function fe(C){if(q){const F=j();if(!F.length){C.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucun joueur.</div>';return}const O=F.map(Z=>c.find(H=>H.player.id===Z.id)||{_fake:!0,player:Z,id:"fake-"+Z.id});J(O,Z=>Z._fake?an(Z.player):wi(Z,""),Z=>Z._fake?pe({player:Z.player,id:"x",_fake:!0}):pe(Z),Z=>{Z._fake||sn(Z,c,h,t)})}else{const F=ie();if(!F.length){C.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const O={};F.forEach(H=>{const B=H.player.id;O[B]||(O[B]=[]),O[B].push(H)});const Z=Object.values(O).map(H=>ko(H));J(Z,H=>{const B=h[H.player.id]||1,R=B>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${B}</div>`:"",ue=c.filter(be=>be.player.id===H.player.id&&be.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return wi(H,R+ue)},H=>pe(H),H=>sn(H,c,h,t))}}function N(C){const F=q?b:[...$];if(!F.length){C.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const O=F.map(Z=>({formation:Z,card:f.find(H=>H.formation===Z)||null,owned:$.has(Z)}));J(O,({formation:Z,card:H,owned:B})=>S(Z,B?H:null,B?f.filter(R=>R.formation===Z).length:0),({formation:Z,owned:H})=>W(Z,H),({card:Z,owned:H})=>{H&&Z&&Io(Z,f,t,o)},"#1A6B3C",5)}function V(C){const F=Object.keys(x),O=q?F.length?F:w:[...m];if(!O.length){C.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const Z=O.map(H=>({type:H,gc:qe[H]||{icon:"⚡",desc:""},def:x[H]||null,owned:m.has(H),card:l.find(B=>B.gc_type===H)||null}));J(Z,({type:H,gc:B,def:R,owned:le,card:ue})=>{var Le;const be=(R==null?void 0:R.name)||H,we=le?l.filter(Fe=>Fe.gc_type===H).length:0,Te=we>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${we}</div>`:"",Ae=(R==null?void 0:R.gc_type)==="ultra_game_changer",ke={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},$e={purple:"#b06ce0",light_blue:"#00d4ef"},je=ke[R==null?void 0:R.color]||ke.purple,De=$e[R==null?void 0:R.color]||$e.purple,Ue=(R==null?void 0:R.effect)||B.desc||"",Ee=R!=null&&R.image_url?`/icons/${R.image_url}`:((Le=R==null?void 0:R.club)==null?void 0:Le.logo_url)||(R!=null&&R.country_code?`https://flagsapi.com/${R.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null);return le&&ue?`<div data-gc-id="${ue.id}" data-gc-type="${H}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${De};background:${je};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${De}66;cursor:pointer">
          ${Te}
        ...
        </div>`},({type:H,gc:B,def:R,owned:le})=>{const ue=window.innerWidth>=768?.76:.54,be=(R==null?void 0:R.name)||H,we={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},Te={purple:"#9b59b6",light_blue:"#00bcd4"},Ae=we[R==null?void 0:R.color]||we.purple,ke=Te[R==null?void 0:R.color]||Te.purple,$e=R!=null&&R.image_url?`/icons/${R.image_url}`:null,je=Math.round(140*657/507);return le?`<div style="display:inline-block;zoom:${ue};line-height:0;pointer-events:none"><div style="width:140px;height:${je}px;border-radius:8px;background:${Ae};border:1px solid ${ke};display:flex;flex-direction:column;overflow:hidden"><div style="height:34px;background:rgba(255,255,255,0.15);display:flex;align-items:center;justify-content:center"><span style="font-size:10px;font-weight:900;color:#fff;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:130px;line-height:1.2;display:inline-block">${be}</span></div><div style="flex:1;display:flex;align-items:center;justify-content:center;min-height:0">${$e?`<img src="${$e}" style="max-width:110px;max-height:100px;object-fit:contain">`:`<span style="font-size:28px;line-height:1">${B.icon}</span>`}</div><div style="height:30px;display:flex;align-items:center;justify-content:center;padding:0 4px"><span style="font-size:8px;color:rgba(255,255,255,0.7);text-align:center;line-height:1.2;display:inline-block">${((R==null?void 0:R.effect)||B.desc||"").slice(0,26)}</span></div></div></div>`:`<div style="display:inline-block;zoom:${ue};line-height:0;pointer-events:none"><div style="width:140px;height:${je}px;border-radius:8px;background:#eee;border:1px solid #ddd;display:flex;flex-direction:column;align-items:center;justify-content:center;filter:grayscale(1);opacity:0.45"><span style="font-size:24px;line-height:1">${B.icon}</span><span style="font-size:9px;color:#aaa;margin-top:4px;text-align:center;padding:0 6px;line-height:1.2;display:inline-block">${be}</span></div></div>`},({type:H,owned:B,def:R})=>{B&&zo(H,R,o)},"#7a28b8",5)}function U(C){const F="#4FC3F7",O="/";if(!y.length){C.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Stade.<br><small>Ouvre un booster Stade !</small></div>';return}const Z={};y.forEach(B=>{const R=B.stadium_id||"?";(Z[R]=Z[R]||[]).push(B)});const H=Object.entries(Z).map(([B,R])=>({sid:B,def:u[B]||null,count:R.length,card:R[0]}));J(H,({def:B,count:R})=>{var Ae,ke;const le=(B==null?void 0:B.name)||"?",ue=((Ae=B==null?void 0:B.club)==null?void 0:Ae.encoded_name)||(B==null?void 0:B.country_code)||"—",be=B!=null&&B.image_url?`${O}icons/${B.image_url}`:((ke=B==null?void 0:B.club)==null?void 0:ke.logo_url)||(B!=null&&B.country_code?`https://flagsapi.com/${B.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),we=be?`<img src="${be}" style="width:90px;height:90px;object-fit:contain;border-radius:4px" onerror="this.style.display='none'">`:'<div style="font-size:56px">🏟️</div>',Te=R>1?`<div style="position:absolute;top:8px;right:8px;background:#333;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${R}</div>`:"";return`<div style="position:relative;width:140px;border-radius:12px;border:3px solid #0288D1;background:linear-gradient(160deg,${F},#0288D1);display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${F}66">
          ${Te}
        ...
        </div>`},({def:B,count:R})=>{var ke,$e;const le=window.innerWidth>=768?.76:.54,ue=(B==null?void 0:B.name)||"?",be=((ke=B==null?void 0:B.club)==null?void 0:ke.encoded_name)||(B==null?void 0:B.country_code)||"—",we=B!=null&&B.image_url?`${O}icons/${B.image_url}`:(($e=B==null?void 0:B.club)==null?void 0:$e.logo_url)||(B!=null&&B.country_code?`https://flagsapi.com/${B.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),Te=we?`<img src="${we}" style="width:44px;height:44px;object-fit:contain;border-radius:4px" onerror="this.style.display='none'">`:'<span style="font-size:22px">🏟️</span>',Ae=Math.round(140*657/507);return`<div style="display:inline-block;zoom:${le};line-height:0;pointer-events:none">
          <div style="width:140px;height:${Ae}px;border-radius:8px;background:linear-gradient(160deg,${F},#0288D1);border:1px solid #0288D1;display:flex;flex-direction:column;overflow:hidden">
            <div style="height:36px;background:rgba(0,0,0,0.25);display:flex;align-items:center;justify-content:center;flex-direction:column;padding:2px 6px">
              ...
              <div style="font-size:9px;font-weight:900;color:#fff;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:130px;margin-top:1px;line-height:1.2;display:inline-block">${ue}</div>
            </div>
        ...
        </div>`},null,F,5)}e.querySelectorAll(".col-tab-btn").forEach(C=>{C.addEventListener("click",()=>{A=C.dataset.tab,k="Tous",I="",q=!1,e.querySelectorAll(".col-tab-btn").forEach(F=>{const O=F.dataset.tab===A;F.style.borderBottomColor=O?"var(--green)":"transparent",F.style.color=O?"var(--green)":"var(--tile-fg-dim)"}),z(),Y()})}),z(),Y()}function zo(e,t,i){const n=qe[e]||{icon:"⚡",desc:"Effet spécial."},r=(t==null?void 0:t.gc_type)==="ultra_game_changer",o={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},a={purple:"#b06ce0",light_blue:"#00d4ef"},s=o[t==null?void 0:t.color]||o.purple,d=a[t==null?void 0:t.color]||a.purple,c=(t==null?void 0:t.name)||e,l=(t==null?void 0:t.effect)||n.desc,f=t!=null&&t.image_url?`/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const oi=1e3;function Io(e,t,i,n){var x,p,u;const{state:r,toast:o,closeModal:a,navigate:s,refreshProfile:d}=i,c=e.formation,l={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function f(){const b=ii[c]||{},w=qi[c]||[],h=290,$=360,m=20;function A(I){const q=b[I];return q?{x:q.x*h,y:q.y*$}:null}let k=`<svg width="${h}" height="${$}" viewBox="0 0 ${h} ${$}" xmlns="http://www.w3.org/2000/svg">`;for(const[I,q]of w){const ne=A(I),oe=A(q);!ne||!oe||(k+=`<line x1="${ne.x}" y1="${ne.y}" x2="${oe.x}" y2="${oe.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const I of Object.keys(b)){const q=A(I);if(!q)continue;const ne=I.replace(/\d+/,""),oe=l[ne]||"#555";k+=`<circle cx="${q.x}" cy="${q.y}" r="${m}" fill="${oe}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,k+=`<text x="${q.x}" y="${q.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${ne}</text>`}return k+="</svg>",k}const y=t.filter(b=>b.formation===c);y.length;const g=!e.is_for_sale;n(`Formation ${c}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(x=document.getElementById("direct-sell-form-btn"))==null||x.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${c} pour ${oi.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const b=y.find(h=>!h.is_for_sale)||y[0];if(!b){o("Aucune carte à vendre","error");return}await v.from("market_listings").delete().eq("card_id",b.id),await v.from("transfer_history").delete().eq("card_id",b.id);const{error:w}=await v.from("cards").delete().eq("id",b.id);if(w){o(w.message,"error");return}await v.from("users").update({credits:(r.profile.credits||0)+oi}).eq("id",r.profile.id),await d(),o(`+${oi.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),a(),s("collection")}),(p=document.getElementById("market-sell-form-btn"))==null||p.addEventListener("click",async()=>{const b=parseInt(document.getElementById("sell-price-form").value);if(!b||b<1){o("Prix invalide","error");return}await v.from("cards").update({is_for_sale:!0,sale_price:b}).eq("id",e.id),await v.from("market_listings").insert({seller_id:r.profile.id,card_id:e.id,price:b}),o("Carte mise en vente sur le marché !","success"),a(),s("collection")}),(u=document.getElementById("cancel-sell-form-btn"))==null||u.addEventListener("click",async()=>{await v.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await v.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),o("Annonce retirée","success"),a(),s("collection")})}async function sn(e,t,i,n){var pe,ge,S,W,fe;const{state:r,toast:o,openModal:a,closeModal:s,navigate:d,refreshProfile:c}=n,l=e.player,f=t.filter(N=>N.player.id===l.id),y=f.length,g=e.evolution_bonus||0,x=Math.max((Number(l.note_g)||0)+(l.job==="GK"||l.job2==="GK"?g:0),(Number(l.note_d)||0)+(l.job==="DEF"||l.job2==="DEF"?g:0),(Number(l.note_m)||0)+(l.job==="MIL"||l.job2==="MIL"?g:0),(Number(l.note_a)||0)+(l.job==="ATT"||l.job2==="ATT"?g:0)),p=l.rarity||"normal",{data:u}=await v.from("sell_price_configs").select("*").eq("rarity",p).lte("note_min",x).gte("note_max",x).order("note_min",{ascending:!1}).limit(1);((pe=u==null?void 0:u[0])==null?void 0:pe.price)??_o[p];const b=l.rarity!=="legende";Lo(l);const w=((l.rarity==="pepite"||l.rarity==="papyte")&&e.current_note!=null?e.current_note:vi(l,l.job))+g,h=l.rarity==="pepite"||l.rarity==="papyte",$=l.job2?(h?bo(e,$o(l.job2)):vi(l,l.job2))+(vi(l,l.job2)>0?g:0):null;hi[l.job],l.job2&&hi[l.job2];const m=vo[l.rarity]||"#ccc";Eo[l.country_code]||l.country_code;const A=e.evolution_bonus||0,I=w+A,q=$||0,ne=q>0?q+A:0,oe=f.map(N=>N.id);let ie={};if(oe.length){const{data:N}=await v.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",oe).order("transferred_at",{ascending:!0});(N||[]).forEach(V=>{ie[V.card_id]||(ie[V.card_id]=[]),ie[V.card_id].push(V)})}const j=N=>{const V=N.transferred_at?new Date(N.transferred_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"2-digit",year:"numeric"}):"",U=N.source==="booster"?"Booster":N.price?N.price.toLocaleString("fr"):"—";return`
    ...
    <div style="margin-top:16px;border-top:1px solid var(--tile-border);padding-top:14px">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${f.map((N,V)=>{const U=ie[N.id]||[],C=N.is_for_sale,F=U.length?U[U.length-1]:null,Z=(l.rarity==="pepite"||l.rarity==="papyte")&&N.current_note!=null?` (☆${N.current_note})`:"";return`
            <div data-card-id="${N.id}" data-card-idx="${V}" class="exemplaire-row"
              ...
              <div style="display:flex;align-items:center;gap:10px">
                ...
                <div style="flex:1;min-width:0">
                  <div style="font-size:12px;font-weight:700">Exemplaire ${V+1}${Z}${C?" 🏷️ En vente":""}</div>
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;background:#f0fdf4;border-radius:10px;padding:10px 14px">
          <div>
            ...
            <div style="font-size:11px;color:#555;margin-top:2px">
              ${(e.evolution_bonus||0)>0?`+${e.evolution_bonus} appliqué${e.evolution_bonus>1?"s":""} · Note : ${I}${l.job2&&q>0?` / ${ne}`:""}`:`Note actuelle : ${I}${l.job2&&q>0?` / ${ne}`:""}`}
            </div>
          </div>
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",l.note_g],["DEF",l.note_d],["MIL",l.note_m],["ATT",l.note_a]].map(([N,V])=>{const U=hi[N],C=N==="GK"?"#fff":U,F=N===l.job||N===l.job2,O=(Number(V)||0)+(F&&A>0?A:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
                <div style="width:48px;height:48px;border-radius:8px;background:#111;border:2.5px solid ${U};
                  display:flex;align-items:center;justify-content:center">
                  <span style="font-size:20px;font-weight:900;color:${C};font-family:Arial Black,Arial;line-height:1">${O}</span>
                </div>
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(ge=document.getElementById("close-detail"))==null||ge.addEventListener("click",s);let Y=new Set;const J=()=>{const N=Y.size,V=document.getElementById("sell-action-panel");if(!V)return;V.style.display=N>0?"block":"none",document.getElementById("sell-selected-count").textContent=N;const U=document.getElementById("evolve-btn");U&&(U.textContent=`⬆️ Évoluer ${N>1?"(+"+N+")":""}`)};document.querySelectorAll(".expl-check").forEach(N=>{N.addEventListener("change",()=>{const V=N.dataset.id;N.checked?Y.add(V):Y.delete(V);const U=N.closest(".exemplaire-row");U&&(U.style.borderColor=N.checked?"#1A6B3C":"#eee"),J()})}),document.querySelectorAll(".exemplaire-row").forEach(N=>{N.addEventListener("click",V=>{if(V.target.closest("button")||V.target.tagName==="INPUT")return;const U=N.querySelector(".expl-check");U&&!U.disabled&&(U.checked=!U.checked,U.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(N=>{N.addEventListener("click",V=>{V.stopPropagation();const U=document.querySelector(`.expl-hist[data-hist="${N.dataset.idx}"]`);U&&(U.style.display=U.style.display==="none"?"flex":"none")})}),(S=document.getElementById("evolve-btn"))==null||S.addEventListener("click",async()=>{if(y<=1)return;const N=[...Y];if(!N.length)return;if(Y.has(e.id)){const H=document.createElement("div");H.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",H.innerHTML=`
  ...
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
        ...
        <div style="width:105px;flex-shrink:0;background:rgba(0,0,0,0.3);display:flex;flex-direction:column;align-items:center;padding:12px 6px;gap:8px;border-right:1px solid rgba(255,255,255,0.1)">
          ...
          <div style="display:flex;flex-direction:column;gap:6px;align-items:center" id="subs-list">
            ${c.map(u=>{const b={...u.player,_evolution_bonus:u.evolution_bonus||0};return`<div style="position:relative;flex-shrink:0;overflow:visible;padding-bottom:24px">
                ${Ce({...b,_evolution_bonus:b._evolution_bonus||0},{width:100,showStad:!0,stadDef:d})}
          ...
          <div style="flex:1;min-width:0">
            ...
            <div style="display:flex;gap:4px;align-items:center;flex-wrap:nowrap;overflow-x:auto;scrollbar-width:none;-ms-overflow-style:none" id="subs-list" class="no-scrollbar">
              ${c.map(u=>{const b={...u.player,_evolution_bonus:u.evolution_bonus||0};return`<div style="position:relative;flex-shrink:0;overflow:visible;padding-bottom:20px">
                  ${Ce({...b,_evolution_bonus:b._evolution_bonus||0},{width:50,showStad:!0,stadDef:d})}
                  <button data-remove-sub="${u.id}"
    ...
    </div>`;u("⚽ Choisir une formation",w),document.querySelectorAll("#modal-body [data-forma]").forEach(h=>{h.addEventListener("click",()=>{t.formation=h.dataset.forma;const $=cn(t.formation),m={};$.forEach(A=>{t.slots[A]&&(m[A]=t.slots[A])}),t.slots=m,b(),wt(e,t,i)})})};e.querySelectorAll("#formation-mobile-btn, #formation-pc-btn").forEach(u=>u.addEventListener("click",g)),e.querySelectorAll("#add-stad-btn-pc, #add-stad-btn").forEach(u=>u.addEventListener("click",()=>Mo(t,e,i))),e.querySelectorAll("#save-deck").forEach(u=>u.addEventListener("click",()=>Bo(t,i))),e.querySelectorAll("[data-remove-sub]").forEach(u=>{u.addEventListener("click",()=>{t.subs=t.subs.filter(b=>b!==u.dataset.removeSub),wt(e,t,i)})}),e.querySelectorAll("#add-sub-btn").forEach(u=>u.addEventListener("click",()=>Co(t,e,i)))}function So(e,t,i,n){var h,$,m;const r=window.innerWidth>=900,o=e.querySelector(r?"#deck-field-pc":"#deck-field-mobile");if(!o)return;const a=(h=t.stadiumCards)==null?void 0:h.find(A=>A.id===t.stadiumCardId),s=a&&(($=t.stadDefMap)==null?void 0:$[a.stadium_id])||null,d=ii[t.formation]||{},c=Bi(t.formation)||[],l={};for(const A of i){const k=t.slots[A],I=k?t.playerCards.find(q=>q.id===k):null;I!=null&&I.player?l[A]={...I.player,_evolution_bonus:I.evolution_bonus||0,face:I.player.face||null}:l[A]=null}const f=window.innerWidth>=900,y=f?window.innerWidth-280:window.innerWidth-20,g=f?Math.min(y,860):y,x=Math.round(f?g*.82:g*.85),p=f?84:44;let u="";for(const[A,k]of c){const I=d[A],q=d[k];if(!I||!q)continue;const ne=I.x*g,oe=Math.round(.03*x+I.y*.85*x),ie=q.x*g,j=Math.round(.03*x+q.y*.85*x),z=l[A],Y=l[k],J=Ht(z,Y);J==="#ff3333"||J==="#cc2222"?u+=`<line x1="${ne.toFixed(1)}" y1="${oe.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${j.toFixed(1)}" stroke="${J}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(u+=`<line x1="${ne.toFixed(1)}" y1="${oe.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${j.toFixed(1)}" stroke="${J}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,u+=`<line x1="${ne.toFixed(1)}" y1="${oe.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${j.toFixed(1)}" stroke="${J}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let b="";const w=Math.round(p*657/507);for(const A of i){const k=d[A];if(!k)continue;const I=l[A],q=k.x*g;k.y*x;const ne=Math.round(.03*x+k.y*(.85*x)),oe=Math.round(q-p/2),ie=Math.round(ne-w/2);if(I){const j=A.replace(/\d+/,""),z=s&&(s.club_id&&String(I.club_id)===String(s.club_id)||s.country_code&&I.country_code===s.country_code),Y=Ce({...I,_evolution_bonus:I._evolution_bonus||0},{width:p,showStad:!0,stadDef:s,role:j}),J=z&&(((m=s.club)==null?void 0:m.logo_url)||s.image_url)||null,pe=Math.round(p*.34),ge=z?`<div style="position:absolute;top:2px;right:2px;width:${pe}px;height:${pe}px;border-radius:50%;background:rgba(0,0,0,0.6);border:1.5px solid #4FC3F7;display:flex;align-items:center;justify-content:center;overflow:hidden;z-index:6">${J?`<img src="${J}" style="width:100%;height:100%;object-fit:cover">`:`<span style="font-size:${Math.round(pe*.6)}px;line-height:1">🏟️</span>`}</div>`:"";b+=`<div style="position:absolute;left:${oe}px;top:${ie}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${A}">
        <div style="position:relative">${Y}${ge}</div>
      </div>`}else{const j=A.replace(/\d+/,"");b+=`<div style="position:absolute;left:${oe}px;top:${ie}px;width:${p}px;height:${w}px;
    ...
    <div style="position:relative;width:${g}px;height:${x}px;margin:0 auto">
      ...
      ${b}
    </div>`,o.querySelectorAll(".deck-slot-hit").forEach(A=>{A.addEventListener("click",()=>jo(A.dataset.pos,t,e,n))})}function Mo(e,t,i){var a;const{openModal:n,closeModal:r}=i,o=e.stadiumCards||[];n("🏟️ Choisir un stade",`<div style="display:flex;flex-wrap:wrap;gap:10px;padding:8px">
  ...
  `,document.body.appendChild(r);let o=!1;const a=document.getElementById("pack-cut-zone"),s=document.getElementById("pack-blade");let d=!1;const c=z=>z.touches&&z.touches[0]?z.touches[0].clientX:z.clientX;function l(z){o||(d=!0,s.style.opacity="1",f(z))}function f(z){if(!d||o)return;const Y=a.getBoundingClientRect(),J=c(z)-Y.left,pe=Math.max(0,Math.min(1,J/Y.width));s.style.width=pe*Y.width+"px",pe>=.82&&g()}function y(){o||(d=!1,s.style.transition="width .2s ease, opacity .2s ease",s.style.width="0",s.style.opacity="0",setTimeout(()=>{o||(s.style.transition="")},220))}function g(){var Y;if(o)return;o=!0,d=!1,s.style.width="100%",s.style.opacity="1",(Y=document.getElementById("cut-flash"))==null||Y.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const z=document.getElementById("cut-hint");z&&(z.style.opacity="0"),a.classList.add("pack-cut"),setTimeout(()=>{s.style.opacity="0",document.getElementById("pack-phase").style.display="none",b(0)},620)}a.addEventListener("pointerdown",l),window.addEventListener("pointermove",f),window.addEventListener("pointerup",y),a.addEventListener("touchstart",l,{passive:!0}),window.addEventListener("touchmove",f,{passive:!0}),window.addEventListener("touchend",y);let x=0,p=!1;const u=new Set;function b(z){x=z,document.getElementById("reveal-phase").style.display="flex",w(),h(z,0),I()}function w(){const z=document.getElementById("card-dots");z&&(z.innerHTML=e.map((Y,J)=>`<div class="card-dot" data-i="${J}" style="width:8px;height:8px;border-radius:50%;background:${J===x?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),z.querySelectorAll(".card-dot").forEach(Y=>Y.addEventListener("click",()=>m(parseInt(Y.dataset.i)))))}function h(z,Y){var U,C;const J=e[z],pe=document.getElementById("card-counter"),ge=document.getElementById("card-track");pe&&(pe.textContent=`Carte ${z+1} / ${e.length}`);const S=document.getElementById("reveal-btns");S&&(S.style.display=z===e.length-1?"flex":"none");const W=J.card_type==="player"&&((U=J.player)==null?void 0:U.rarity)==="legende",fe=!u.has(z);u.add(z);let N=0;if(J.card_type==="player"&&J.player){const F=J.player,O=F.job||"ATT";N=Number(O==="GK"?F.note_g:O==="DEF"?F.note_d:O==="MIL"?F.note_m:F.note_a)||0}const V=F=>{ge.innerHTML=`
        <div id="current-card-wrap" style="position:relative;display:flex;flex-direction:column;align-items:center;gap:8px;${W?"filter:drop-shadow(0 0 20px #7a28b8)":""}">
          <div style="transform:scale(1.25);transform-origin:center">${Ho(J)}</div>
        ...
        </div>`;const O=document.getElementById("current-card-wrap");Y!==0?(O.style.transition="none",O.style.transform=`translateX(${Y>0?100:-100}%)`,requestAnimationFrame(()=>{O.style.transition="transform .28s cubic-bezier(.25,1,.5,1)",O.style.transform="translateX(0)"})):O.animate([{opacity:0,transform:"scale(.7)"},{opacity:1,transform:"scale(1)"}],{duration:300,easing:"cubic-bezier(.34,1.56,.64,1)"}),F||W?ne():oe(),w()};fe&&(((C=J.player)==null?void 0:C.rarity)==="legende"||N>=8)&&J.card_type==="player"&&J.player?$(J,()=>V(!0)):V(!1)}function $(z,Y){var be;p=!0;const J=z.player,pe=`https://flagsapi.com/${J.country_code}/flat/64.png`,ge=(be=J.clubs)==null?void 0:be.logo_url,S=J.face?"/"+J.face.replace(/^public\//,"").replace(/^\//,""):null,W=J.job||"ATT",fe=Number(W==="GK"?J.note_g:W==="DEF"?J.note_d:W==="MIL"?J.note_m:J.note_a)||0,N=z.evolution_bonus||0,V=fe+N,U=J.rarity==="legende",C=V>=18,F=document.getElementById("walkout-overlay"),O=document.getElementById("walkout-stage");if(!F||!O){p=!1,Y();return}let Z=null;U&&(Z=mo("/sounds/Legendary.mp3",.8)),F.style.display="flex";const H=()=>{const we=O.firstElementChild;we&&(we.classList.remove("wo-in"),we.classList.add("wo-out"))},B=1800,R=400;O.innerHTML=`<img class="wo-in" src="${pe}" style="height:130px;border-radius:10px;box-shadow:0 10px 36px rgba(0,0,0,.6)" onerror="this.style.display='none'">`,navigator.vibrate&&navigator.vibrate(30),setTimeout(H,B),setTimeout(()=>{var we;O.innerHTML=ge?`<img class="wo-in" src="${ge}" style="max-height:160px;max-width:210px;object-fit:contain">`:`<div class="wo-in" style="font-size:34px;font-weight:900;color:#fff;text-align:center">${((we=J.clubs)==null?void 0:we.encoded_name)||"CLUB"}</div>`,navigator.vibrate&&navigator.vibrate(30)},B+R),setTimeout(H,B*2+R),setTimeout(()=>{O.innerHTML=S?`<img class="wo-in" src="${S}" style="height:200px;border-radius:50%;box-shadow:0 0 40px rgba(255,255,255,0.3);object-fit:cover;object-position:top">`:'<div class="wo-in" style="font-size:80px">👤</div>',navigator.vibrate&&navigator.vibrate(30)},(B+R)*2),setTimeout(H,(B+R)*2+B);const ue=C?"#FFD700":{GK:"#c0c0c0",DEF:"#e03030",MIL:"#D4A017",ATT:"#3fbf5f"}[W]||"#fff";setTimeout(()=>{O.innerHTML=`<div class="wo-in" style="
        font-size:${C?"120px":"90px"};font-weight:900;color:${ue};
        ...
        ${V}
      </div>`,C&&navigator.vibrate&&navigator.vibrate([50,30,100,30,200])},(B+R)*3),setTimeout(H,(B+R)*3+B),setTimeout(()=>{F.style.display="none",O.innerHTML="",p=!1,Z&&!U&&Z.pause(),navigator.vibrate&&navigator.vibrate([40,30,80]),Y()},(B+R)*4)}function m(z){if(p||z<0||z>=e.length||z===x)return;const Y=z>x?1:-1;x=z,h(z,Y)}function A(){m(x+1)}function k(){m(x-1)}function I(){const z=document.getElementById("card-viewport");if(!z||z._swipeBound)return;z._swipeBound=!0;let Y=0,J=0,pe=0,ge=!1;const S=U=>U.touches?U.touches[0].clientX:U.clientX,W=U=>U.touches?U.touches[0].clientY:U.clientY,fe=U=>{ge=!0,Y=S(U),J=W(U),pe=0},N=U=>{if(!ge)return;pe=S(U)-Y;const C=W(U)-J;if(Math.abs(pe)<Math.abs(C))return;const F=document.getElementById("current-card-wrap");F&&(F.style.transition="none",F.style.transform=`translateX(${pe*.6}px) rotate(${pe*.02}deg)`)},V=()=>{if(!ge)return;ge=!1;const U=document.getElementById("current-card-wrap"),C=55;pe<=-C&&x<e.length-1?A():pe>=C&&x>0?k():U&&(U.style.transition="transform .2s ease",U.style.transform="translateX(0)")};z.addEventListener("pointerdown",fe),z.addEventListener("pointermove",N),z.addEventListener("pointerup",V),z.addEventListener("pointercancel",V),z.addEventListener("touchstart",fe,{passive:!0}),z.addEventListener("touchmove",N,{passive:!0}),z.addEventListener("touchend",V),z.addEventListener("click",U=>{if(Math.abs(pe)>8)return;const C=z.getBoundingClientRect();U.clientX-C.left>C.width/2?A():k()})}let q=null;function ne(){const z=document.getElementById("fireworks-canvas");if(!z)return;z.width=window.innerWidth,z.height=window.innerHeight;const Y=z.getContext("2d"),J=[];function pe(){const S=Math.random()*z.width,W=Math.random()*z.height*.6,fe=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],N=fe[Math.floor(Math.random()*fe.length)];for(let V=0;V<60;V++){const U=Math.PI*2/60*V,C=2+Math.random()*5;J.push({x:S,y:W,vx:Math.cos(U)*C,vy:Math.sin(U)*C,alpha:1,color:N,size:2+Math.random()*3})}}pe(),q=setInterval(pe,600);function ge(){if(document.getElementById("fireworks-canvas")){Y.clearRect(0,0,z.width,z.height);for(let S=J.length-1;S>=0;S--){const W=J[S];if(W.x+=W.vx,W.y+=W.vy+.08,W.vy*=.98,W.alpha-=.018,W.alpha<=0){J.splice(S,1);continue}Y.globalAlpha=W.alpha,Y.fillStyle=W.color,Y.beginPath(),Y.arc(W.x,W.y,W.size,0,Math.PI*2),Y.fill()}Y.globalAlpha=1,(q!==null||J.length>0)&&requestAnimationFrame(ge)}}ge()}function oe(){q!==null&&(clearInterval(q),q=null);const z=document.getElementById("fireworks-canvas");z&&z.getContext("2d").clearRect(0,0,z.width,z.height)}if(n){const z=document.getElementById("reveal-btns");z&&(z.innerHTML='<button class="btn btn-primary" id="reveal-next" style="flex:1">Continuer →</button>'),(j=document.getElementById("reveal-next"))==null||j.addEventListener("click",()=>{oe(),r.remove(),n()})}else document.getElementById("reveal-collection").addEventListener("click",()=>{oe(),r.remove(),i("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{oe(),r.remove(),i("boosters")})}function Ho(e){var t,i,n,r;if(e.card_type==="player"&&e.player)return Po(e);if(e.card_type==="game_changer"){const o=e._gcDef,a=(o==null?void 0:o.gc_type)==="ultra_game_changer",s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},d={purple:"#b06ce0",light_blue:"#00d4ef"},c=s[o==null?void 0:o.color]||s.purple,l=d[o==null?void 0:o.color]||d.purple,f=(o==null?void 0:o.name)||e.gc_type||"Game Changer",y=(o==null?void 0:o.effect)||((t=Ti[e.gc_type])==null?void 0:t.desc)||"",g=o!=null&&o.image_url?`/icons/${o.image_url}`:null,x=((i=Ti[e.gc_type])==null?void 0:i.icon)||"⚡";return`<div style="width:170px;height:240px;background:${c};border-radius:14px;border:3px solid ${l};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${l}88;flex-shrink:0">
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    </div>`}const d=l=>{e.style.overflow="",e.style.height="",e.style.display="",e.style.flexDirection="",i(l)};function c(){var f,y,g;e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column";const l=a.length>0;e.innerHTML=`
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      card:cards(id,card_type,formation,stadium_id,evolution_bonus,
        player:players(id,firstname,surname_real,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order"),d=[...new Set((s||[]).filter(y=>{var g,x;return((g=y.card)==null?void 0:g.card_type)==="stadium"&&((x=y.card)==null?void 0:x.stadium_id)}).map(y=>y.card.stadium_id))],c={};if(d.length){const{data:y}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",d);(y||[]).forEach(g=>{c[g.id]=g}),(s||[]).forEach(g=>{var x,p;((x=g.card)==null?void 0:x.card_type)==="stadium"&&((p=g.card)!=null&&p.stadium_id)&&(g.card._stadiumDef=c[g.card.stadium_id]||null)})}let l=0;function f(){var m,A,k,I,q,ne,oe;const y=o[l],g=(s||[]).filter(ie=>ie.deck_id===y.id),x=g.filter(ie=>{var j;return ie.is_starter&&((j=ie.card)==null?void 0:j.player)}).map(ie=>Ii(ie.card,ie.position)),p=g.find(ie=>{var j;return((j=ie.card)==null?void 0:j.card_type)==="formation"}),u=y.formation||((m=p==null?void 0:p.card)==null?void 0:m.formation)||"4-4-2";let b=x.length>=11?li(x,u):null,w=((A=y.stadium_card)==null?void 0:A.stadium_def)||null;w&&b&&(b=ti(b,w));const h=x.length>=11;dt(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff;position:relative">
      ...
      <!-- Pagination -->
      ${o.length>1?`<div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">${o.map((ie,j)=>`<div style="width:6px;height:6px;border-radius:50%;background:${j===l?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}</div>`:""}
      ...
      <div id="deck-bottom-bar" style="flex-shrink:0;padding:10px 14px 14px;display:flex;flex-direction:column;gap:8px;background:rgba(0,0,0,0.2)">
    ...
    </div>`,requestAnimationFrame(()=>requestAnimationFrame(function ie(){const j=e.querySelector(".deck-preview-wrap"),z=e.querySelector("#deck-swipe-zone");if(!j||!z||!b)return;const Y=z.clientWidth>=900,J=Math.max(200,z.clientHeight-(Y?60:40)),ge=Math.max(200,z.clientWidth-16),S=Y?Math.min(117,Math.max(52,Math.round(ge*.22))):Math.max(44,Math.round(ge*.168));if(J<220||ge<220){requestAnimationFrame(ie);return}const W=Y?null:Math.round(S*.55);j.innerHTML=Nt(b,u,null,[],ge,J,[],W),j.style.cssText=`width:${ge}px;height:${J}px;overflow:hidden;margin:${Y?0:60}px auto 0`;const fe=j.querySelector("svg");fe&&(fe.style.cssText="display:block;width:100%;height:100%",fe.setAttribute("preserveAspectRatio",Y?"xMidYMid meet":"none"))})),(I=document.getElementById("prev-deck"))==null||I.addEventListener("click",()=>{l>0&&(l--,f())}),(q=document.getElementById("next-deck"))==null||q.addEventListener("click",()=>{l<o.length-1&&(l++,f())}),(ne=document.getElementById("validate-deck"))==null||ne.addEventListener("click",()=>{if(!h)return;const ie=t.state.params||{};t.navigate("match",{...ie,matchMode:ie.matchMode||i,deckId:y.id})}),(oe=document.getElementById("cancel-deck-select"))==null||oe.addEventListener("click",()=>{it(e),r("home")});const $=document.getElementById("deck-swipe-zone");if($){let ie=0,j=0;$.addEventListener("touchstart",z=>{ie=z.touches[0].clientX,j=z.touches[0].clientY},{passive:!0}),$.addEventListener("touchend",z=>{const Y=z.changedTouches[0].clientX-ie,J=z.changedTouches[0].clientY-j;Math.abs(Y)<40||Math.abs(Y)<Math.abs(J)||(Y<0&&l<o.length-1?(l++,f()):Y>0&&l>0&&(l--,f()))},{passive:!0})}}f()}function vt(e,t=44,i=58,n=null){return Ce(e,{width:t,showStad:!!n,stadDef:n,used:e==null?void 0:e.used})}function Qe(e,t,i,n,r){if(!(e!=null&&e.length))return"";const o=e.slice(0,5);let a='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return o.forEach((s,d)=>{const c=s._line||s.job||"MIL";let l=s.boost||0;if(s.stadiumBonus&&(n==="attack"&&(c==="ATT"||c==="MIL")||n==="defense"&&(c==="GK"||c==="DEF"||c==="MIL")?l+=10:n||(l+=10)),a+=Ce(s,{width:40,role:c,extraNote:l}),d<o.length-1){const f=o[d+1],y=wn(s,f,r)?Ht(s,f):null;a+=`<div style="width:7px;height:3px;background:${!y||y==="#ff3333"||y==="#cc2222"?"rgba(255,255,255,0.12)":y};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(a+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),a+="</div>",a}function gi(e,t,i,n,r=310,o=310,a=[],s=null){const d=ii[t]||{},c=Bi(t)||qi[t]||[],l={},f=["ATT","MIL","DEF","GK"];for(const b of f)(e[b]||[]).forEach((h,$)=>{l[`${b}${$+1}`]=h});function y(b){const w=d[b];return w?{x:w.x*r,y:w.y*o}:null}let g="";for(const[b,w]of c){const h=y(b),$=y(w);if(!h||!$)continue;const m=l[b],A=l[w],k=Ht(m,A);k==="#00ff88"||k==="#FFD700"?(g+=`<line x1="${h.x.toFixed(1)}" y1="${h.y.toFixed(1)}" x2="${$.x.toFixed(1)}" y2="${$.y.toFixed(1)}"
        ...
        stroke="${k}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const x=typeof window<"u"&&window.innerWidth>=900?Math.min(Math.max(81,Math.round(r*.225)),117):Math.max(44,Math.round(r*.168)),p=Math.round(x*657/507);for(const[b,w]of Object.entries(l)){const h=y(b);if(!h||!w)continue;const $=b.replace(/[0-9]/g,""),m=a.includes(w.cardId),A=i==="attack"&&(["MIL","ATT"].includes($)||m)&&!w.used||i==="defense"&&["GK","DEF","MIL"].includes($)&&!w.used,k=n.includes(w.cardId);let I=w.boost||0,q=!1;w.stadiumBonus&&(i==="attack"&&($==="ATT"||$==="MIL")||i==="defense"&&($==="GK"||$==="DEF"||$==="MIL")?(I+=10,q=!0):i||(I+=10,q=!0));const ne=Math.round(h.x-x/2),oe=Math.round(h.y-p/2);if(w.used){g+=`<image href="${`${typeof import.meta<"u"&&"/"||"/"}icons/carte-dos.png`}" x="${ne}" y="${oe}" width="${x}" height="${p}" preserveAspectRatio="xMidYMid slice" class="match-used-hit" data-card-id="${w.cardId}" data-role="${$}" style="cursor:pointer"/>`;continue}const ie=Ce({...w,_evolution_bonus:0,stadiumBonus:!1},{width:x,showStad:!1,stadDef:null,role:$,extraNote:I,_cardOffset:30,_forceStadColor:q}),j=k?`position:absolute;top:30px;left:0;width:${x}px;height:${p}px;outline:3px solid #FFD700;outline-offset:2px;border-radius:8px;pointer-events:none;`:"";g+=`<foreignObject x="${ne-2}" y="${oe-30}" width="${x+8}" height="${p+60}" style="overflow:visible">
      <div xmlns="http://www.w3.org/1999/xhtml" style="position:relative">
        ${ie}
        ${k?`<div style="${j}"></div>`:""}
      </div>
    </foreignObject>`,A&&(g+=`<rect x="${ne}" y="${oe}" width="${x}" height="${p}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${k?"selected":""}" data-card-id="${w.cardId}" data-role="${$}" style="cursor:pointer"/>`)}const u=s!==null?s:Math.round(Math.max(x*.7,80));return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-u} ${-u} ${r+u*2} ${o+u*2}" width="100%" style="display:block;width:100%;margin:0 auto">
    ${g}
  </svg>`}function Nt(e,t,i,n,r=300,o=300,a=[],s=null){return`<div id="match-terrain-wrap" style="position:relative;padding:0 4px">
    ${gi(e,t,i,n,r,o,a,s)}
  </div>`}async function mi(e,t,i,n){var A;const{state:r,navigate:o,toast:a}=t;dt(e);const s=r.params||{};if(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!s.deckId)return Qo(e,t,i);const d=s.deckId;let c,l,f,y;try{const k=await Promise.all([v.from("decks").select("formation,name,stadium_card_id").eq("id",d).single(),v.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation, evolution_bonus,
            player:players(id,firstname,surname_real,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
              clubs(encoded_name,logo_url)))`).eq("deck_id",d).order("slot_order")]);c=k[0].data,f=k[0].error,l=k[1].data,y=k[1].error}catch(k){console.error("[Match] Exception chargement deck:",k),Tt(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>o("home"));return}if(f||y){console.error("[Match] Erreur Supabase:",f||y),Tt(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>o("home"));return}const g=(l||[]).filter(k=>{var I;return k.is_starter&&((I=k.card)==null?void 0:I.player)}).map(k=>Ii(k.card,k.position)),x=(l||[]).filter(k=>{var I;return!k.is_starter&&((I=k.card)==null?void 0:I.player)}).map(k=>Ii(k.card,k.position));if(g.length<11){Tt(e,"⚠️",`Deck incomplet (${g.length}/11).`,"Compléter",()=>o("decks"));return}const p=(l||[]).find(k=>{var I;return((I=k.card)==null?void 0:I.card_type)==="formation"}),u=(c==null?void 0:c.formation)||((A=p==null?void 0:p.card)==null?void 0:A.formation)||"4-4-2",{data:b,error:w}=await v.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",r.profile.id).eq("card_type","game_changer"),{data:h}=await v.from("gc_definitions").select("*").eq("is_active",!0),$=(b||[]).map(k=>({...k,_gcDef:(h==null?void 0:h.find(I=>I.name===k.gc_type||I.id===k.gc_definition_id))||null}));let m=null;if(c!=null&&c.stadium_card_id){const{data:k}=await v.from("cards").select("stadium_id").eq("id",c.stadium_card_id).single();if(k!=null&&k.stadium_id){const{data:I}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").eq("id",k.stadium_id).single();m=I||null}}n({deckId:d,formation:u,starters:g,subsRaw:x,gcCardsEnriched:$,gcDefs:h||[],stadiumDef:m})}function zt(){return Math.min(window.innerWidth-40,860)}function Gt(){return Math.round(zt()*1.1)}function jn(e){var r,o;if(!e)return null;const t=e._line||e.job||"MIL",i=t==="GK"?e.note_g||0:t==="DEF"?e.note_d||0:t==="MIL"?e.note_m||0:e.note_a||0,n=e.stadiumBonus?10:0;return{name:e.name,firstname:e.firstname||"",note:i+(e.boost||0)+n,note_g:e.note_g||0,note_d:e.note_d||0,note_m:e.note_m||0,note_a:e.note_a||0,_evolution_bonus:0,stadiumBonus:e.stadiumBonus||!1,boost:e.boost||0,job:e.job,job2:e.job2||null,_line:e._line||e.job,_col:e._col,country_code:e.country_code,club_id:e.club_id,rarity:e.rarity,clubName:e.clubName||((r=e.clubs)==null?void 0:r.encoded_name)||null,clubLogo:e.clubLogo||((o=e.clubs)==null?void 0:o.logo_url)||null,face:e.face||null,portrait:ei(e)}}function Zo(e,t,i,n="Adversaire"){const r=zt(),o=Gt();return`
    ...
    </div>`}async function tr(e,t){const{state:i}=t,r=(i.params||{}).matchMode||"vs_ai_easy",o=r.replace("vs_ai_",""),a=r;await mi(e,t,r,async({deckId:s,formation:d,starters:c,subsRaw:l,gcCardsEnriched:f,gcDefs:y,stadiumDef:g})=>{try{let x=li(c,d);g&&(x=ti(x,g),di(l,g));const p=await ir(d,o),u=p.lines||p,b=async w=>{try{const h=a==="vs_ai_club"?"club":a,{data:$,error:m}=await v.from("matches").insert({home_id:i.profile.id,away_id:null,mode:h,home_deck_id:s,status:"in_progress"}).select().single();if(m){console.error("[MatchIA] Erreur création match:",m),Tt(e,"⚠️","Impossible de créer le match ("+m.message+").","Retour",()=>t.navigate("home"));return}const A=p.stadiumDef||null;A&&u&&(ti(u,A),di(p.subs||[],A));const k={gcDefs:y||[],matchId:$==null?void 0:$.id,mode:a,difficulty:o,formation:d,homeTeam:x,aiTeam:u,homeSubs:l,subsUsed:0,maxSubs:Math.min(l.length,3),aiSubs:p.subs||[],aiSubsUsed:0,aiMaxSubs:Math.min((p.subs||[]).length,3),aiUsedSubIds:[],aiGcCards:p.gcCards||[],aiUsedGc:[],aiStadiumDef:A,homeScore:0,aiScore:0,gcCards:w,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};or(e,k,t)}catch(h){console.error("[MatchIA] Exception launchMatch:",h),Tt(e,"⚠️","Erreur au lancement du match : "+h.message,"Retour",()=>t.navigate("home"))}};if(!f.length){b([]);return}fi(e,f,b)}catch(x){console.error("[MatchIA] Exception setup:",x),Tt(e,"⚠️","Erreur de préparation du match : "+x.message,"Retour",()=>t.navigate("home"))}})}async function ir(e,t){var u;const{data:i}=await v.from("players").select("id,firstname,surname_real,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(80);if(!i||i.length<11)return{lines:nr(e),subs:[],gcCards:[],stadiumDef:null};const n=Rt[e]||Rt["4-4-2"],r={GK:[],DEF:[],MIL:[],ATT:[]},o=new Set;function a(b,w,h){var $,m;return o.add(b.id),{cardId:"ai-"+b.id+"-"+h,id:b.id,firstname:b.firstname,name:b.surname_real,country_code:b.country_code,club_id:b.club_id,job:b.job,job2:b.job2,note_g:Number(b.note_g)||0,note_d:Number(b.note_d)||0,note_m:Number(b.note_m)||0,note_a:Number(b.note_a)||0,rarity:b.rarity,skin:b.skin,hair:b.hair,hair_length:b.hair_length,clubName:(($=b.clubs)==null?void 0:$.encoded_name)||null,clubLogo:((m=b.clubs)==null?void 0:m.logo_url)||null,boost:0,used:!1,_line:w}}for(const b of["GK","DEF","MIL","ATT"]){const w=i.filter(k=>k.job===b&&!o.has(k.id)),h=i.filter(k=>k.job!==b&&!o.has(k.id)),$=[...w,...h],m=[];for(let k=0;k<n[b];k++){const I=$[k];I&&m.push(a(I,b,k))}const A=xt(m.length);m.forEach((k,I)=>{k._col=A[I]}),r[b]=m}const d=i.filter(b=>!o.has(b.id)).slice(0,5).map((b,w)=>a(b,b.job,100+w)),f=Object.keys(qe).sort(()=>Math.random()-.5).slice(0,3).map((b,w)=>{var h,$;return{id:"ai-gc-"+w,gc_type:b,name:((h=qe[b])==null?void 0:h.name)||b,icon:(($=qe[b])==null?void 0:$.icon)||"⚡"}}),y=Object.values(r).flat(),g={};y.forEach(b=>{b.club_id&&(g[b.club_id]=(g[b.club_id]||0)+1)});const x=(u=Object.entries(g).sort((b,w)=>w[1]-b[1])[0])==null?void 0:u[0];let p=null;if(x){const{data:b}=await v.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",x).single();b&&(p={club_id:b.id,country_code:null,name:b.encoded_name+" Stadium",club:{encoded_name:b.encoded_name,logo_url:b.logo_url}})}return{lines:r,subs:d,gcCards:f,stadiumDef:p}}function nr(e){const t=Rt[e]||Rt["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},n=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let r=0;for(const o of["GK","DEF","MIL","ATT"]){const a=[];for(let d=0;d<t[o];d++){const c=3+Math.floor(Math.random()*5);a.push({cardId:"fake-"+r,id:"fake-"+r,firstname:"IA",name:n[r%n.length],country_code:"XX",club_id:null,job:o,job2:null,note_g:o==="GK"?c:2,note_d:o==="DEF"?c:2,note_m:o==="MIL"?c:2,note_a:o==="ATT"?c:2,rarity:"normal",boost:0,used:!1,_line:o}),r++}const s=xt(a.length);a.forEach((d,c)=>{d._col=s[c]}),i[o]=a}return i}function or(e,t,i){var r;const n=`<div style="position:relative;width:22px;height:22px;flex-shrink:0;display:flex;align-items:center;justify-content:center">
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const W=t.pendingAttack;let fe="";if(t.selected.length>0){const N=t.selected.map(U=>({...(t.homeTeam[U._role]||[]).find(F=>F.cardId===U.cardId)||U,_line:U._role})),V=Zt(N,t.modifiers.home,t.formation);fe=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
            ...
            <div style="display:flex;justify-content:center">${Qe((W.players||[]).map(fe=>({...fe,used:!1})),"#00ff88",W.total,"attack",t.formation)}</div>
          </div>`}if(t.phase==="attack"&&t.selected.length>0){const W=t.selected.map(N=>{const V=(t.homeTeam[N._role]||[]).find(C=>C.cardId===N.cardId)||N,U=["GK","DEF"].includes(N._role);return{...V,_line:N._role,...U?{note_a:Math.max(1,Number(V.note_a)||0)}:{}}}),fe=Qt(W,t.modifiers.home,t.formation);return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #FFD700;text-align:center">
            ...
            <div style="display:flex;justify-content:center">${Qe(W.map(N=>({...N,used:!1})),"#FFD700",fe.total,"attack",t.formation)}</div>
    ...
    ${(()=>{const S=window.innerWidth>=700,W=(H,B,R)=>{var Ee,Le;const le=(t.gcDefs||[]).find(Fe=>Fe.name===H.gc_type),ue={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[le==null?void 0:le.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",be={purple:"#b06ce0",light_blue:"#00d4ef"}[le==null?void 0:le.color]||"#b06ce0",we=(le==null?void 0:le.name)||H.gc_type,Te=(le==null?void 0:le.effect)||((Ee=qe[H.gc_type])==null?void 0:Ee.desc)||"",Ae=le!=null&&le.image_url?`/icons/${le.image_url}`:null,ke=((Le=qe[H.gc_type])==null?void 0:Le.icon)||"⚡",$e=Math.round(R*.22),je=Math.round(R*.22),De=R-$e-je,Ue=we.length>12?7:9;return`<div class="gc-mini" data-gc-id="${H.id}" data-gc-type="${H.gc_type}"
          style="box-sizing:border-box;width:${B}px;height:${R}px;border-radius:10px;border:2px solid ${be};background:${ue};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
          ...
          <div style="height:${De}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
            ${Ae?`<img src="${Ae}" style="max-width:${B-10}px;max-height:${De-6}px;object-fit:contain">`:`<span style="font-size:${Math.round(De*.55)}px">${ke}</span>`}
          </div>
        ...
        </div>`},fe=(H,B)=>{var R;return`<div id="boost-card"
          ...
          </div>`},N=(H,B)=>B?fe(130,175):W(H,130,175),V=(H,B)=>B?fe(68,95):W(H,68,95),U=S?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",C=w?`<button id="btn-results" style="${U};background:linear-gradient(135deg,#D4A017,#FFD700);border:none;color:#000">🏁 Résultats</button>`:p?`<div style="${U};background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);color:rgba(255,255,255,0.4)">⏳ Tour IA</div>`:m?`<button id="btn-pass" style="${U};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER (plus d'attaquants)</button>`:u?`<button id="btn-action" style="${U};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${t.selected.length===0?"disabled":""}> ⚔️ ATTAQUEZ <span id="match-timer" style="font-weight:900"></span></button>`:b?`<button id="btn-action" style="${U};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${t.selected.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="match-timer" style="font-weight:900"></span></button>`:`<div style="${U};background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1)"></div>`,F=u||b?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${t.selected.length}/3 sélectionné(s)</div>`:"",O=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${S?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
      ...
      </div>`,Z=`<div style="overflow:hidden;min-width:0;flex:1;min-height:0;display:flex;flex-direction:column" id="match-field">
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(S=>{var W,fe,N;if(S.type==="duel"){const V=S.isGoal,U=S.homeScored?"#FFD700":V?"#ff6b6b":"rgba(255,255,255,0.3)",C=S.homeScored?"⚽ BUT !":V?"⚽ BUT IA !":(W=S.homePlayers)!=null&&W.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${V?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${U};margin-bottom:4px">
                <div style="font-size:9px;color:${U};letter-spacing:1px;margin-bottom:5px;font-weight:700;text-transform:uppercase">${C}</div>
                ${(fe=S.homePlayers)!=null&&fe.length?`<div style="margin-bottom:3px">${Qe(S.homePlayers,"rgba(255,255,255,0.7)",S.homeTotal,void 0,t.formation)}</div>`:""}
    ...
    </div>`,(m=f.querySelector("#sub-close"))==null||m.addEventListener("click",()=>f.remove()),(A=f.querySelector("#out-up"))==null||A.addEventListener("click",()=>{s>0&&(s--,y())}),(k=f.querySelector("#out-down"))==null||k.addEventListener("click",()=>{s<o.length-1&&(s++,y())}),(I=f.querySelector("#in-up"))==null||I.addEventListener("click",()=>{c>0&&(c--,y())}),(q=f.querySelector("#in-down"))==null||q.addEventListener("click",()=>{c<a.length-1&&(c++,y())});const $=(oe,ie,j,z)=>{const Y=f.querySelector("#"+oe);if(!Y)return;let J=0;Y.addEventListener("touchstart",pe=>{J=pe.touches[0].clientY},{passive:!0}),Y.addEventListener("touchend",pe=>{const ge=pe.changedTouches[0].clientY-J;if(Math.abs(ge)<30)return;const S=ie();ge<0&&S<z-1?(j(S+1),y()):ge>0&&S>0&&(j(S-1),y())},{passive:!0})};$("in-panel",()=>c,oe=>c=oe,a.length),$("out-panel",()=>s,oe=>s=oe,o.length),(ne=f.querySelector("#sub-confirm"))==null||ne.addEventListener("click",oe=>{if(oe.preventDefault(),oe.stopPropagation(),l)return;l=!0;const ie=o[s],j=a[c];if(!ie||!j)return;let z=null,Y=-1;for(const[pe,ge]of Object.entries(t.homeTeam)){const S=(ge||[]).findIndex(W=>W.cardId===ie.cardId);if(S!==-1){z=pe,Y=S;break}}if(Y===-1||!z){$t("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),f.remove();return}const J={...j,_line:z,_col:ie._col||0,used:!1,boost:0};t.homeTeam[z].splice(Y,1,J),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(j.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:ie.name,firstname:ie.firstname,note:Ne(ie,z),portrait:ei(ie),job:ie.job,country_code:ie.country_code,rarity:ie.rarity,clubName:ie.clubName,clubLogo:ie.clubLogo},inPlayer:{name:j.name,firstname:j.firstname,note:Ne(j,z),portrait:ei(j),job:j.job,country_code:j.country_code,rarity:j.rarity,clubName:j.clubName,clubLogo:j.clubLogo},text:`🔄 ${j.firstname} ${j.name} remplace ${ie.firstname} ${ie.name}`}),f.remove(),er(ie,j,()=>ze(e,t,i))})}document.body.appendChild(f),y()}function ur(e,t,i,n,r){var x,p;const o=(n.gcDefs||[]).find(u=>u.name===t),a=qe[t]||{icon:"⚡",desc:"Carte spéciale."},s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[o==null?void 0:o.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",d={purple:"#b06ce0",light_blue:"#00d4ef"}[o==null?void 0:o.color]||"#b06ce0",c=(o==null?void 0:o.name)||t,l=(o==null?void 0:o.effect)||a.desc,f=o!=null&&o.image_url?`/icons/${o.image_url}`:null,y=a.icon||"⚡",g=document.createElement("div");g.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",g.innerHTML=`
    ...
    <div style="width:190px;border-radius:16px;border:3px solid ${d};background:${s};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 40px ${d}66">
      ...
      <div style="height:160px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
        ${f?`<img src="${f}" style="max-width:150px;max-height:150px;object-fit:contain">`:`<span style="font-size:72px">${y}</span>`}
      </div>
    ...
    <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start;justify-content:center">
      ${e.map(f=>{const y=f._line||f.job||"MIL",g=s.find(p=>p.cardId===f.cardId),x=Ce({...f,_evolution_bonus:0},{width:90,showStad:!0,role:y,extraNote:f.boost||0});return`<div class="gc-pick-item" data-cid="${f.cardId}"
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(r=>{r.addEventListener("click",()=>{const o=r.dataset.cardId;for(const a of["GK","DEF","MIL","ATT"]){const s=(t.homeTeam[a]||[]).find(d=>d.cardId===o);if(s){s.boost=(s.boost||0)+t.boostCard.value,t.log.push({text:`⚡ Boost +${t.boostCard.value} appliqué à ${s.name}`,type:"info"});break}}t.boostUsed=!0,i.closeModal(),ze(e,t,i)})})}async function ht(e,t,i){var l,f;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:n}=i,r=t.homeScore>t.aiScore,o=t.homeScore===t.aiScore,a=r?"victoire":o?"nul":"defaite",s=xo(t.mode,a);t.matchId&&await v.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:r?n.profile.id:null,home_credits_reward:s,played_at:new Date().toISOString()}).eq("id",t.matchId);const d={credits:(n.profile.credits||0)+s,matches_played:(n.profile.matches_played||0)+1};r?d.wins=(n.profile.wins||0)+1:o?d.draws=(n.profile.draws||0)+1:d.losses=(n.profile.losses||0)+1,await v.from("users").update(d).eq("id",n.profile.id),await i.refreshProfile();const c=document.createElement("div");c.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",c.innerHTML=`
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      ...
      ${gi(e.aiTeam,e.formation,null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.1))}
    </div>`,`<button class="btn btn-primary" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const qt=e=>jn(e);async function At(e,t,i,n,r={}){return Dn(e,t,i,n,r.myGC||[],r.gcDefs||[],r.isRanked||!1,r.rankedData||null,r.stadiumDef||null,r.onMatchEnd||null,r.mlLeagueId||null,r.mlMatchId||null)}async function yr(e,t,i){const{data:n}=await v.from("matches").select("home_id,away_id,mode,is_ranked").eq("id",i).single();if(!n){t.toast("Match introuvable","error"),t.navigate("home");return}const r=n.home_id===t.state.user.id;let o=null,a=null;if(n.mode==="mini_league"){const{data:s}=await v.from("mini_league_matches").select("id, league_id").eq("match_id",i).maybeSingle();s&&(o=s.league_id,a=s.id)}return Dn(e,t,i,r,[],[],n.is_ranked||!1,null,null,null,o,a)}async function Dn(e,t,i,n,r=[],o=[],a=!1,s=null,d=null,c=null,l=null,f=null){const{state:y,navigate:g,toast:x}=t,p=n?"p1":"p2",u=n?"p2":"p1",b=(r||[]).map(_=>_.id),w=(r||[]).map(_=>({id:_.id,gc_type:_.gc_type,_gcDef:_._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:h}=await v.from("matches").select("*").eq("id",i).single();if(!h){x("Match introuvable","error"),g("home");return}async function $(){var ye,he;console.log("[PvP] buildGameState match:",{id:h.id,home_deck_id:h.home_deck_id,away_deck_id:h.away_deck_id,mode:h.mode,is_ranked:h.is_ranked});const[{data:_,error:E},{data:T,error:M},{data:L},{data:G}]=await Promise.all([v.rpc("get_deck_for_match",{p_deck_id:h.home_deck_id}),v.rpc("get_deck_for_match",{p_deck_id:h.away_deck_id}),v.from("users").select("id,pseudo,club_name").eq("id",h.home_id).single(),v.from("users").select("id,pseudo,club_name").eq("id",h.away_id).single()]);console.log("[PvP] get_deck_for_match p1:",E==null?void 0:E.message,"p2:",M==null?void 0:M.message,"p1D:",(ye=_==null?void 0:_.starters)==null?void 0:ye.length,"p2D:",(he=T==null?void 0:T.starters)==null?void 0:he.length);const D=ae=>{const Ie=Number(ae.evolution_bonus)||0;return{cardId:ae.card_id,position:ae.position,id:ae.id,firstname:ae.firstname,name:ae.surname_real,country_code:ae.country_code,club_id:ae.club_id,job:ae.job,job2:ae.job2,note_g:(Number(ae.note_g)||0)+(ae.job==="GK"||ae.job2==="GK"&&Number(ae.note_g)>0?Ie:0),note_d:(Number(ae.note_d)||0)+(ae.job==="DEF"||ae.job2==="DEF"&&Number(ae.note_d)>0?Ie:0),note_m:(Number(ae.note_m)||0)+(ae.job==="MIL"||ae.job2==="MIL"&&Number(ae.note_m)>0?Ie:0),note_a:(Number(ae.note_a)||0)+(ae.job==="ATT"||ae.job2==="ATT"&&Number(ae.note_a)>0?Ie:0),evolution_bonus:Ie,rarity:ae.rarity,skin:ae.skin,hair:ae.hair,hair_length:ae.hair_length,face:ae.face||null,clubName:ae.club_encoded_name||null,clubLogo:ae.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},P=((_==null?void 0:_.starters)||[]).map(ae=>D(ae)),K=((T==null?void 0:T.starters)||[]).map(ae=>D(ae)),ee=(_==null?void 0:_.formation)||"4-4-2",re=(T==null?void 0:T.formation)||"4-4-2";let te=li(P,ee),Q=li(K,re);const se=((_==null?void 0:_.subs)||[]).map(ae=>D(ae)),de=((T==null?void 0:T.subs)||[]).map(ae=>D(ae)),xe=(_==null?void 0:_.stadium_def)||(n?d:null),ce=(T==null?void 0:T.stadium_def)||(n?null:d);return xe&&(te=ti(te,xe),di(se,xe)),ce&&(Q=ti(Q,ce),di(de,ce)),{p1Team:te,p2Team:Q,p1Subs:se,p2Subs:de,p1Formation:ee,p2Formation:re,p1Name:(L==null?void 0:L.club_name)||(L==null?void 0:L.pseudo)||"Joueur 1",p2Name:(G==null?void 0:G.club_name)||(G==null?void 0:G.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:n?b:[],gc_p2:n?[]:b,gcCardsFull_p1:n?w:[],gcCardsFull_p2:n?[]:w,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:o||[],lastActionAt:new Date().toISOString()}}let m=h.game_state&&Object.keys(h.game_state).length?h.game_state:null;if(console.log("[PvP] init - amIHome:",n,"gameState exists:",!!m,"match.status:",h.status,"home_id:",h.home_id,"away_id:",h.away_id,"myId:",y.profile.id),!m)if(n){m=await $(),console.log("[PvP] home - gameState built, p1Team keys:",Object.keys((m==null?void 0:m.p1Team)||{}));const{data:_}=await v.from("matches").select("game_state").eq("id",i).single();!(_!=null&&_.game_state)||!Object.keys(_.game_state).length?await v.from("matches").update({game_state:m,turn_user_id:h.home_id}).eq("id",i):m=_.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>',console.log("[PvP] away - waiting for home to write game_state...");for(let _=0;_<30&&!m;_++){await new Promise(T=>setTimeout(T,400));const{data:E}=await v.from("matches").select("game_state").eq("id",i).single();E!=null&&E.game_state&&Object.keys(E.game_state).length&&(m=E.game_state),_%5===0&&console.log("[PvP] away - poll",_,"game_state:",!!(E!=null&&E.game_state))}if(!m){x("Erreur de synchronisation","error"),g("home");return}m.gc_p2=b,m.gcCardsFull_p2=w,await v.from("matches").update({game_state:m}).eq("id",i)}let A=!1,k=null,I=!1;const q=new Set,ne=new Set;async function oe(_){var re,te;try{v.removeChannel(ie)}catch{}const E=m[p+"Score"]||0,T=m[u+"Score"]||0;let M,L;_.winner_id?(M=_.winner_id===y.profile.id,L=!1):_.forfeit?(M=E>T,L=!1):(L=E===T,M=E>T);let G="";if(a&&n)try{const{data:Q}=await v.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",h.home_id).single(),{data:se}=await v.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",h.away_id).single();if(Q&&se){const de=M?1:L?.5:0,xe=1-de,ce=Q.placement_matches<10,ye=se.placement_matches<10,he=computeGlicko2(Q.mmr,Q.mmr_deviation,Q.mmr_volatility,se.mmr,se.mmr_deviation,de===1?1:de===0?0:.5,ce),ae=computeGlicko2(se.mmr,se.mmr_deviation,se.mmr_volatility,Q.mmr,Q.mmr_deviation,xe===1?1:xe===0?0:.5,ye);await v.rpc("update_mmr_after_ranked",{p_match_id:i,p_winner_id:L?null:M?h.home_id:h.away_id,p_home_id:h.home_id,p_away_id:h.away_id,p_home_delta:he.delta,p_away_delta:ae.delta,p_home_new_rd:he.newRd,p_away_new_rd:ae.newRd,p_home_new_vol:he.newSigma,p_away_new_vol:ae.newSigma});const Ie=he.delta,Oe=he.newMmr,Se=getTier(Oe),We=Ie>=0?"+":"",Ke=Ie>=0?"#4caf50":"#ff6b6b",nt=getTier(Q.mmr);G=`
            <div style="background:rgba(255,255,255,0.08);border-radius:14px;padding:14px 20px;text-align:center;min-width:220px">
              ...
              ${Se.id!==nt.id?`<div style="font-size:20px;font-weight:900;color:${Se.color}">
                    ${Oe>Q.mmr?"📈":"📉"} ${nt.emoji} ${nt.label} → ${Se.emoji} ${Se.label}
                  </div>
                  <div style="font-size:13px;color:rgba(255,255,255,0.5);margin-top:4px">${Oe>Q.mmr?"Promotion !":"Rétrogradation"}</div>`:`<div style="font-size:36px">${Ie>=0?"📈":"📉"}</div>
            ...
            </div>`}}catch(Q){console.error("[Ranked] MMR update error:",Q)}(re=document.getElementById("pvp-end-overlay"))==null||re.remove();const D=document.createElement("div");D.id="pvp-end-overlay",D.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;color:#fff;padding:24px;text-align:center;overflow-y:auto";const P=L?"🤝":M?"🏆":"😞",K=L?"MATCH NUL":M?"VICTOIRE !":"DÉFAITE",ee=L?"#fff":M?"#FFD700":"#ff6b6b";D.innerHTML=`
      ...
      <div style="font-size:18px">${m[p+"Name"]} ${E} – ${T} ${m[u+"Name"]}</div>
      ${_.forfeit?`<div style="font-size:13px;color:rgba(255,255,255,0.5)">${M?"L'adversaire a quitté":"Perdu par forfait"}</div>`:""}
      ${G}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${l?"🏆 Retour à la Mini League":a?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(D),(te=D.querySelector("#pvp-end-home"))==null||te.addEventListener("click",()=>{D.remove(),it(e),l?g("mini-league",{openLeagueId:l}):g(a?"ranked":"home")})}const ie=v.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},_=>{var T;const E=_.new;try{if(E.status==="finished"||E.forfeit){if(A)return;A=!0,k&&(clearInterval(k),k=null),E.game_state&&(m=E.game_state),oe(E);return}if(E.game_state){const M=m;m=E.game_state;const L=m._lastGC;if(L&&L.seq&&!ne.has(L.seq)&&(ne.add(L.seq),L.by!==p)){U(L.type,L.by,()=>W());return}const G=M[p+"Score"]||0,D=M[u+"Score"]||0,P=m[p+"Score"]||0,K=m[u+"Score"]||0,ee=P>G,re=K>D;if((ee||re)&&!q.has(m.round)){q.add(m.round);const te=[...m.log||[]].reverse().find(de=>de.isGoal),Q=((te==null?void 0:te.homePlayers)||[]).map(de=>({name:de.name,note:de.note,portrait:de.portrait,job:de.job}));!!((T=te==null?void 0:te.text)!=null&&T.includes("DERNIER CORNER"))?be().then(()=>{Ze(Q,P,K,ee,()=>W())}):Ze(Q,P,K,ee,()=>W());return}W()}}catch(M){console.error("[PvP] crash:",M)}}).subscribe();async function j(_){Object.assign(m,_),m.lastActionAt=new Date().toISOString();const{error:E}=await v.from("matches").update({game_state:m}).eq("id",i);E&&x("Erreur de synchronisation","error");try{W()}catch(T){console.error("[PvP] renderPvpScreen crash:",T)}}async function z(){if(A)return;A=!0,k&&(clearInterval(k),k=null);const _=n?h.away_id:h.home_id,E=n?"p2":"p1",T=n?"p1":"p2",M={...m,[E+"Score"]:3,[T+"Score"]:0,phase:"finished"},L=M.p1Score||0,G=M.p2Score||0;try{await v.from("matches").update({game_state:M}).eq("id",i);const{data:D,error:P}=await v.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:_,p_home_score:L,p_away_score:G,p_forfeit:!0});if(P&&console.error("[PvP] finish_pvp_match (forfeit):",P),l)try{await v.rpc("check_and_finish_mini_league",{p_league_id:l})}catch(K){console.error("[PvP] check_and_finish_mini_league (forfeit):",K)}try{const{data:K}=await v.rpc("apply_match_rewards",{p_match_id:i});K!=null&&K.success&&!(K!=null&&K.skipped)&&typeof t.refreshProfile=="function"&&await t.refreshProfile()}catch(K){console.error("[PvP] apply_match_rewards (forfeit):",K)}}catch{}try{v.removeChannel(ie)}catch{}setTimeout(()=>{it(e),l?g("mini-league",{openLeagueId:l}):g("home")},800)}const Y={BOOST_STAT:({value:_=1,count:E=1,roles:T=[]},M,L)=>{const G=M[p+"Team"],D=Object.entries(G).filter(([P])=>!T.length||T.includes(P)).flatMap(([P,K])=>K.filter(ee=>!ee.used).map(ee=>({...ee,_line:P})));if(!D.length){M.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),L(M);return}J(D,E,`Choisir ${E} joueur(s) à booster (+${_})`,P=>{P.forEach(K=>{const ee=(G[K._line]||[]).find(re=>re.cardId===K.cardId);ee&&(ee.boost=(ee.boost||0)+_,M.log.push({text:`⚡ +${_} sur ${ee.name}`,type:"info"}))}),M[p+"Team"]=G,L(M)})},DEBUFF_STAT:({value:_=1,count:E=1,roles:T=[],target:M="ai"},L,G)=>{const D=M==="home"?p:u,P=L[D+"Team"],K=M==="home"?"allié":"adverse",ee=Object.entries(P).filter(([re])=>!T.length||T.includes(re)).flatMap(([re,te])=>te.map(Q=>({...Q,_line:re})));if(!ee.length){L.log.push({text:`🎯 Aucun joueur ${K}`,type:"info"}),G(L);return}J(ee,E,`Choisir ${E} joueur(s) ${K}(s) (-${_})`,re=>{re.forEach(te=>{const Q=(P[te._line]||[]).find(se=>se.cardId===te.cardId);Q&&(Q.boost=(Q.boost||0)-_,L.log.push({text:`🎯 -${_} sur ${Q.name}`,type:"info"}))}),L[D+"Team"]=P,G(L)})},GRAY_PLAYER:({count:_=1,roles:E=[],target:T="ai"},M,L)=>{const G=T==="home"?p:u,D=M[G+"Team"],P=T==="home"?"allié":"adverse",K=Object.entries(D).filter(([ee])=>!E.length||E.includes(ee)).flatMap(([ee,re])=>re.filter(te=>!te.used).map(te=>({...te,_line:ee})));if(!K.length){M.log.push({text:`❌ Aucun joueur ${P}`,type:"info"}),L(M);return}J(K,_,`Choisir ${_} joueur(s) ${P}(s) à exclure`,ee=>{const re="usedCardIds_"+G,te=new Set(M[re]||[]);ee.forEach(Q=>{const se=(D[Q._line]||[]).find(de=>de.cardId===Q.cardId);se&&(se.used=!0,te.add(Q.cardId),M.log.push({text:`❌ ${se.name} exclu !`,type:"info"}))}),M[re]=[...te],M[G+"Team"]=D,L(M)})},REVIVE_PLAYER:({count:_=1,roles:E=[]},T,M)=>{const L=T[p+"Team"],G=Object.entries(L).filter(([D])=>!E.length||E.includes(D)).flatMap(([D,P])=>P.filter(K=>K.used).map(K=>({...K,_line:D})));if(!G.length){T.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),M(T);return}J(G,_,`Choisir ${_} joueur(s) à ressusciter`,D=>{D.forEach(P=>{const K=(L[P._line]||[]).find(ee=>ee.cardId===P.cardId);K&&(K.used=!1,T.log.push({text:`💫 ${K.name} ressuscité !`,type:"info"}))}),T[p+"Team"]=L,M(T)})},REMOVE_GOAL:({},_,E)=>{const T=u+"Score";_[T]>0?(_[T]--,_.log.push({text:"🚫 Dernier but annulé !",type:"info"})):_.log.push({text:"🚫 Aucun but à annuler",type:"info"}),E(_)},ADD_GOAL_DRAW:({},_,E)=>{_[p+"Score"]===_[u+"Score"]?(_[p+"Score"]++,_.log.push({text:"🎯 But bonus !",type:"info"})):_.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),E(_)},ADD_SUB:({value:_=1},E,T)=>{E.maxSubs=(E.maxSubs||3)+_,E.log.push({text:`🔄 +${_} remplacement(s)`,type:"info"}),T(E)},CUSTOM:({},_,E)=>E(_)};function J(_,E,T,M){const L=document.createElement("div");L.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let G=[];function D(){var K,ee;const P=_.map(re=>{const te=re._line||re.job||"MIL",Q=G.find(de=>de.cardId===re.cardId),se=Ce({...re,_evolution_bonus:0},{width:90,showStad:!0,role:te,extraNote:re.boost||0});return`<div class="pp-item" data-cid="${re.cardId}"
          style="position:relative;border-radius:8px;${Q?"outline:3px solid #FFD700;outline-offset:2px;":""}cursor:pointer;flex-shrink:0;${re.used?"opacity:0.3;pointer-events:none":""}">
          ${se}
        ...
        </div>`,(K=L.querySelector("#pp-close"))==null||K.addEventListener("click",()=>L.remove()),L.querySelectorAll(".pp-item").forEach(re=>{re.addEventListener("click",()=>{const te=re.dataset.cid,Q=_.find(de=>de.cardId===te),se=G.findIndex(de=>de.cardId===te);Q&&(se>-1?G.splice(se,1):G.length<E&&G.push(Q),D())})}),(ee=L.querySelector("#pp-confirm"))==null||ee.addEventListener("click",()=>{L.remove(),M(G)})}D(),document.body.appendChild(L)}async function pe(_,E){const M=(m["gcCardsFull_"+p]||[]).find(P=>P.id===_),L=(M==null?void 0:M._gcDef)||(m.gcDefs||[]).find(P=>{var K;return P.name===E||((K=P.name)==null?void 0:K.toLowerCase().trim())===(E==null?void 0:E.toLowerCase().trim())}),G=[...m["usedGc_"+p]||[],_],D={type:E,by:p,seq:(m._gcAnimSeq||0)+1};ne.add(D.seq),U(E,p,async()=>{if(L!=null&&L.effect_type&&L.effect_type!=="CUSTOM"){const K=Y[L.effect_type];if(K){const ee={...m};K(L.effect_params||{},ee,async re=>{re["usedGc_"+p]=G,re._lastGC=D,re._gcAnimSeq=D.seq,await j(re)});return}}const P={...m};switch(E){case"Remplacement+":P.maxSubs=(P.maxSubs||3)+1,P.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const K=u+"Score";P[K]>0&&(P[K]--,P.log.push({text:"🚫 But annulé",type:"info"}));break}}P["usedGc_"+p]=G,P._lastGC=D,P._gcAnimSeq=D.seq,await j(P)})}function ge(_,E){const T="usedCardIds_"+_,M=new Set(m[T]||[]);E.forEach(L=>M.add(L)),m[T]=[...M]}function S(){for(const _ of["p1","p2"]){const E=new Set(m["usedCardIds_"+_]||[]),T=m[_+"Team"];if(T)for(const M of["GK","DEF","MIL","ATT"])(T[M]||[]).forEach(L=>{L.used=E.has(L.cardId)})}}function W(){var Ki,Vi,Yi,Wi,Xi,Ji;if(m.phase==="reveal")return fe();if(m.phase==="midfield")return V();if(m.phase==="finished")return Fe();const _=m[p+"Team"],E=m[u+"Team"];S();const T=m[p+"Score"],M=m[u+"Score"],L=m[p+"Name"],G=m[u+"Name"],D=m.phase===p+"-attack",P=m.phase===p+"-defense",K=Array.isArray(m["selected_"+p])?m["selected_"+p]:[],ee=K.map(X=>X.cardId),re=window.innerWidth>=700,te=m[p+"Subs"]||[],Q=m["usedSubIds_"+p]||[],se=te.filter(X=>!Q.includes(X.cardId)),de=m["gcCardsFull_"+p]||[],xe=m["usedGc_"+p]||[],ce=de.filter(X=>!xe.includes(X.id)),ye=m.boostOwner===p&&!m.boostUsed,he=[..._.MIL||[],..._.ATT||[]].filter(X=>!X.used),ae=[...E.MIL||[],...E.ATT||[]].filter(X=>!X.used),Ie=!$e(E),Oe=he.length===0&&ae.length===0&&!Ie,Se=(_.DEF||[]).filter(X=>!X.used),We=(_.GK||[]).filter(X=>!X.used);let Ke=[];D&&he.length===0&&(Ie?(Ke=Se.map(X=>X.cardId),Se.length===0&&(Ke=Ke.concat(We.map(X=>X.cardId)))):Oe&&(Ke=[...Se,...We].map(X=>X.cardId)));function nt(X,me,Me){var Bt,gt;const _e=X._gcDef,jt={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[_e==null?void 0:_e.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",at={purple:"#b06ce0",light_blue:"#00d4ef"}[_e==null?void 0:_e.color]||"#b06ce0",Ge=(_e==null?void 0:_e.name)||X.gc_type,He=(_e==null?void 0:_e.effect)||((Bt=qe[X.gc_type])==null?void 0:Bt.desc)||"",Be=_e!=null&&_e.image_url?`/icons/${_e.image_url}`:null,kt=((gt=qe[X.gc_type])==null?void 0:gt.icon)||"⚡",Re=Math.round(Me*.22),ft=Math.round(Me*.22),st=Me-Re-ft,Ct=Ge.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${X.id}" data-gc-type="${X.gc_type}"
        style="box-sizing:border-box;width:${me}px;height:${Me}px;border-radius:10px;border:2px solid ${at};background:${jt};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
        <div style="height:${Re}px;background:rgba(255,255,255,0.14);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 3px">
          <span style="font-size:${Ct}px;font-weight:900;color:#fff;text-align:center;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${me-6}px">${Ge}</span>
          <span style="font-size:6px;color:rgba(255,255,255,0.45)">⚡ GC</span>
        </div>
        <div style="height:${st}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
          ${Be?`<img src="${Be}" style="max-width:${me-10}px;max-height:${st-6}px;object-fit:contain">`:`<span style="font-size:${Math.round(st*.55)}px">${kt}</span>`}
        </div>
      ...
      </div>`}function Ut(X,me){return`<div id="pvp-boost-card"
      ...
      </div>`}const Pe=(X,me)=>me?Ut(130,175):nt(X,130,175),Xe=(X,me)=>me?Ut(68,95):nt(X,68,95),Ve=re?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",pt=D?De(p)?`<button id="pvp-action" style="${Ve};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${K.length===0?"disabled":""}>⚔️ ATTAQUEZ <span id="pvp-timer"></span></button>`:`<button id="pvp-action" data-pass="1" style="${Ve};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER <span id="pvp-timer"></span></button>`:P?`<button id="pvp-action" style="${Ve};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${K.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="pvp-timer"></span></button>`:`<div style="font-size:11px;color:rgba(255,255,255,0.3);text-align:center;padding:4px">⏳ Tour de ${G}</div>`,ot=D&&!De(p)?'<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">Aucun attaquant — passez la main</div>':D||P?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${K.length}/3 sélectionné(s)</div>`:"",St=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${re?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
    ...
    </div>`,ut=D?"attack":P?"defense":"idle",_t=`<div style="overflow:hidden;min-width:0;flex:1;min-height:0;display:flex;flex-direction:column" id="match-field">
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${Nt(_,m[p+"Formation"],ut,ee,re?1300:zt(),re?600:Gt(),Ke)}
      </div>
    </div>`,rt=m[p+"Team"],Un=(()=>{var me,Me,_e,jt,at;if(P&&((me=m.pendingAttack)!=null&&me.players)){const Ge=m.pendingAttack;let He="";if(K.length>0){const Be=K.map(Re=>{const ft=(rt[Re._role]||[]).find(yi=>yi.cardId===Re.cardId)||Re,st=rt[Re._role]||[],Ct=st.findIndex(yi=>yi.cardId===Re.cardId),Bt=xt(st.length),gt=Ct>=0?Bt[Ct]:ft._col??1;return{...ft,_line:Re._role,_col:gt}}),kt=Zt(Be,((Me=m.modifiers)==null?void 0:Me[p])||{},m[p+"Formation"]);He=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
            ...
            <div style="display:flex;justify-content:center">${Qe(Be.map(Re=>({...Re,used:!1})),"#3a7bd5",kt.total,"defense",m[p+"Formation"])}</div>
          </div>`}return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b;text-align:center">
          ...
          <div style="display:flex;justify-content:center">${Qe((Ge.players||[]).map(Be=>({...Be,used:!1})),"#ff6b6b",Ge.total,"attack",m[u+"Formation"])}</div>
          ${He}
        </div>`}if(D&&((_e=m.pendingAttack)!=null&&_e.players)){const Ge=m.pendingAttack;return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #00ff88;text-align:center">
          <div style="font-size:8px;color:#00ff88;letter-spacing:2px;margin-bottom:4px;text-transform:uppercase">⚔️ VOUS ATTAQUEZ</div>
          <div style="display:flex;justify-content:center">${Qe((Ge.players||[]).map(He=>({...He,used:!1})),"#00ff88",Ge.total,"attack",m[p+"Formation"])}</div>
        </div>`}if(D&&!((jt=m.pendingAttack)!=null&&jt.players)&&K.length>0){const Ge=K.map(Be=>{const kt=(rt[Be._role]||[]).find(gt=>gt.cardId===Be.cardId)||Be,Re=["GK","DEF"].includes(Be._role),ft=rt[Be._role]||[],st=ft.findIndex(gt=>gt.cardId===Be.cardId),Ct=xt(ft.length),Bt=st>=0?Ct[st]:kt._col??1;return{...kt,_line:Be._role,_col:Bt,...Re?{note_a:Math.max(1,Number(kt.note_a)||0)}:{}}}),He=Qt(Ge,((at=m.modifiers)==null?void 0:at[p])||{},m[p+"Formation"]);return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #FFD700;text-align:center">
          ...
          <div style="display:flex;justify-content:center">${Qe(Ge.map(Be=>({...Be,used:!1})),"#FFD700",He.total,"attack",m[p+"Formation"])}</div>
        </div>`}const X=(m.log||[]).slice(-1)[0];return X?'<div style="padding:2px 4px">'+Cn(X)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3);text-align:center">⏳ Match en cours...</div>'})(),Ui=`
      <div style="display:flex;align-items:center;padding:8px 10px;background:rgba(0,0,0,0.5);gap:6px;flex-shrink:0">
        ...
        <div style="flex:1;display:flex;align-items:center;justify-content:center;gap:8px">
          ...
          <span style="font-size:26px;font-weight:900;color:#FFD700;letter-spacing:2px">${T} – ${M}</span>
      ...
      <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
        ...
        <div id="mobile-play-area" style="flex:1;min-height:0;display:flex;overflow:hidden">
          <div id="match-field" style="flex:1;min-width:0;min-height:0;overflow:hidden">
            <div class="terrain-wrapper" style="width:100%;height:100%;overflow:hidden">
              ${Nt(_,m[p+"Formation"],ut,ee,zt(),Gt(),Ke)}
            </div>
          </div>
        </div>
        <div id="mobile-action-bar" style="position:absolute;left:0;right:0;bottom:0;z-index:20;background:rgba(0,0,0,0.55);padding:6px 8px 8px;display:flex;flex-direction:column;gap:6px;box-shadow:0 -4px 16px rgba(0,0,0,0.5)">
      ...
      </div>`;function Mt(){const X=e.querySelector(".match-screen");if(!X)return;const me=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);X.style.height=me+"px",X.style.minHeight=me+"px",X.style.maxHeight=me+"px",X.style.overflow="hidden";const Me=e.querySelector("#mobile-action-bar"),_e=e.querySelector("#mobile-play-area");Me&&_e&&(_e.style.paddingBottom=Me.offsetHeight+"px")}if(Mt(),setTimeout(Mt,120),setTimeout(Mt,400),I||(I=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",Mt),window.visualViewport.addEventListener("scroll",Mt)),window.addEventListener("resize",Mt)),function(){const me=e.querySelector("#match-field .terrain-wrapper svg")||e.querySelector(".terrain-wrapper svg");if(!me)return;const Me=me.closest("#match-terrain-wrap");Me&&(Me.style.cssText="position:relative;width:100%;height:100%;padding:0"),me.removeAttribute("width"),me.removeAttribute("height"),me.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",me.setAttribute("preserveAspectRatio","xMidYMid meet")}(),m._pvpResizeBound||(m._pvpResizeBound=!0,window.addEventListener("resize",()=>{const X=e.querySelector(".terrain-wrapper svg");if(X){const me=X.closest("#match-terrain-wrap");me&&(me.style.cssText="position:relative;width:100%;height:100%;padding:0"),X.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0"}})),e.querySelectorAll(".match-slot-hit").forEach(X=>{X.addEventListener("click",()=>{if(!D&&!P)return;const me=X.dataset.cardId,Me=X.dataset.role,_e=(_[Me]||[]).find(He=>He.cardId===me);if(!_e||_e.used)return;const jt=Ke.includes(me);if(D&&!["MIL","ATT"].includes(Me)&&!jt)return;Array.isArray(m["selected_"+p])||(m["selected_"+p]=[]);const at=m["selected_"+p],Ge=at.findIndex(He=>He.cardId===me);Ge>-1?at.splice(Ge,1):at.length<3&&at.push({..._e,_role:Me}),W()})}),e.querySelectorAll(".match-used-hit").forEach(X=>{X.addEventListener("click",()=>O(X.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(X=>{X.addEventListener("click",()=>O())}),(Ki=e.querySelector("#pvp-sub-open"))==null||Ki.addEventListener("click",()=>O()),e.querySelectorAll(".pvp-gc-mini").forEach(X=>{D?X.addEventListener("click",()=>C(X.dataset.gcId,X.dataset.gcType)):(X.style.opacity="0.35",X.style.cursor="default",X.addEventListener("click",()=>$t("⚡ Les Game Changers ne sont utilisables qu'en attaque","rgba(180,100,0,0.9)")))}),(Vi=e.querySelector("#pvp-boost-card"))==null||Vi.addEventListener("click",()=>F()),(Yi=e.querySelector("#pvp-action"))==null||Yi.addEventListener("click",X=>{D?X.currentTarget.dataset.pass==="1"||!De(p)?R():Te():P&&Ae()}),(Wi=e.querySelector("#pvp-quit"))==null||Wi.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&z()}),(Xi=e.querySelector("#pvp-view-opp"))==null||Xi.addEventListener("click",()=>H()),(Ji=e.querySelector("#pvp-toggle-history"))==null||Ji.addEventListener("click",()=>B()),k&&(clearInterval(k),k=null),(D||P)&&!A){let X=30,me=!1;const Me=()=>document.getElementById("pvp-timer"),_e=()=>{Me()&&(Me().textContent=X+"s",Me().style.color=me?"#ff4444":"#fff")};_e(),k=setInterval(()=>{X--,X<0?me?(clearInterval(k),k=null,D&&!De(p)?R():z()):(me=!0,X=15,_e()):_e()},1e3)}}function fe(){dt(e),e.innerHTML=`
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(Q);let se=!1;const de=()=>{se||(se=!0,Q.remove(),setTimeout(()=>T&&T(),50))};Q.addEventListener("click",de),setTimeout(de,3e3)}function C(_,E){var Q,se,de,xe;const M=(m["gcCardsFull_"+p]||[]).find(ce=>ce.id===_),L=M==null?void 0:M._gcDef,G={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[L==null?void 0:L.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",D={purple:"#b06ce0",light_blue:"#00d4ef"}[L==null?void 0:L.color]||"#b06ce0",P=(L==null?void 0:L.name)||E,K=(L==null?void 0:L.effect)||((Q=qe[E])==null?void 0:Q.desc)||"Carte spéciale.",ee=L!=null&&L.image_url?`/icons/${L.image_url}`:null,re=((se=qe[E])==null?void 0:se.icon)||"⚡",te=document.createElement("div");te.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",te.innerHTML=`
      <div style="width:190px;border-radius:16px;border:3px solid ${D};background:${G};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 40px ${D}66">
        <div style="padding:10px;background:rgba(255,255,255,0.14);text-align:center">
      ...
      </div>`,document.body.appendChild(te),(de=te.querySelector("#pvp-gc-back"))==null||de.addEventListener("click",()=>te.remove()),(xe=te.querySelector("#pvp-gc-use"))==null||xe.addEventListener("click",()=>{te.remove(),pe(_,E)})}function F(){var M;const _=m[p+"Team"],E=Object.entries(_).flatMap(([L,G])=>(G||[]).filter(D=>!D.used).map(D=>({...D,_line:L})));if(!E.length)return;const T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",T.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
      ...
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${E.map(L=>{const G={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[L._line]||"#555",D=Ne(L,L._line)+(L.boost||0);return`<div class="bp-item" data-cid="${L.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${G};overflow:hidden;cursor:pointer">
      ...
      </div>`,document.body.appendChild(T),(M=T.querySelector("#bp-close"))==null||M.addEventListener("click",()=>T.remove()),T.querySelectorAll(".bp-item").forEach(L=>{L.addEventListener("click",async()=>{const G=L.dataset.cid,D=E.find(K=>K.cardId===G);if(!D)return;const P=(_[D._line]||[]).find(K=>K.cardId===G);P&&(P.boost=(P.boost||0)+m.boostValue),T.remove(),await j({[p+"Team"]:_,boostUsed:!0})})})}function O(_=null){var se,de;if(!(m.phase===p+"-attack")){x("Remplacement uniquement avant votre attaque","warning");return}const T=m[p+"Team"],M=m["usedSubIds_"+p]||[],L=m.maxSubs||3;if(M.length>=L){x(`Maximum ${L} remplacements atteint`,"warning");return}const G=Object.entries(T).flatMap(([xe,ce])=>(ce||[]).filter(ye=>ye.used).map(ye=>({...ye,_line:xe}))),D=(m[p+"Subs"]||[]).filter(xe=>!M.includes(xe.cardId));if(!G.length){x("Aucun joueur utilisé à remplacer","warning");return}if(!D.length){x("Aucun remplaçant disponible","warning");return}let P=Math.max(0,G.findIndex(xe=>xe.cardId===_));const K=((se=G[P])==null?void 0:se._line)||((de=G[P])==null?void 0:de.job);let ee=Math.max(0,D.findIndex(xe=>xe.job===K)),re=!1;const te=document.createElement("div");te.id="pvp-sub-overlay",te.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function Q(){var Oe,Se,We,Ke,nt,Ut;const xe=G[P],ce=D[ee],ye=Math.min(130,Math.round((window.innerWidth-90)/2)),he=Math.round(ye*1.35),ae=Pe=>`background:rgba(255,255,255,0.12);border:none;color:${Pe?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Pe?"default":"pointer"};flex-shrink:0`;te.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        <div id="pin-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
          ...
          <button id="pin-down" style="${ae(ee>=D.length-1)}" ${ee>=D.length-1?"disabled":""}>▼</button>
      ...
      </div>`,(Oe=te.querySelector("#psub-close"))==null||Oe.addEventListener("click",()=>te.remove()),(Se=te.querySelector("#pout-up"))==null||Se.addEventListener("click",()=>{P>0&&(P--,Q())}),(We=te.querySelector("#pout-down"))==null||We.addEventListener("click",()=>{P<G.length-1&&(P++,Q())}),(Ke=te.querySelector("#pin-up"))==null||Ke.addEventListener("click",()=>{ee>0&&(ee--,Q())}),(nt=te.querySelector("#pin-down"))==null||nt.addEventListener("click",()=>{ee<D.length-1&&(ee++,Q())});const Ie=(Pe,Xe,Ve,pt)=>{const ot=te.querySelector("#"+Pe);if(!ot)return;let St=0;ot.addEventListener("touchstart",ut=>{St=ut.touches[0].clientY},{passive:!0}),ot.addEventListener("touchend",ut=>{const _t=ut.changedTouches[0].clientY-St;if(Math.abs(_t)<30)return;const rt=Xe();_t<0&&rt<pt-1?(Ve(rt+1),Q()):_t>0&&rt>0&&(Ve(rt-1),Q())},{passive:!0})};Ie("pin-panel",()=>ee,Pe=>ee=Pe,D.length),Ie("pout-panel",()=>P,Pe=>P=Pe,G.length),(Ut=te.querySelector("#psub-confirm"))==null||Ut.addEventListener("click",async Pe=>{if(Pe.preventDefault(),Pe.stopPropagation(),re)return;re=!0;const Xe=G[P],Ve=D[ee];if(!Xe||!Ve)return;const pt=Xe._line,ot=(T[pt]||[]).findIndex(_t=>_t.cardId===Xe.cardId);if(ot===-1){x("Erreur : joueur introuvable","error"),te.remove();return}const St={...Ve,_line:pt,position:Xe.position,used:!1,boost:0};T[pt].splice(ot,1,St);const ut=[...M,Ve.cardId];te.remove(),Z(Xe,Ve,async()=>{await j({[p+"Team"]:T,[u+"Team"]:m[u+"Team"],["usedSubIds_"+p]:ut})})})}document.body.appendChild(te),Q()}function Z(_,E,T){const M={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},L=document.createElement("div");L.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const G=(K,ee,re)=>`<div style="text-align:center">
      ...
      </div>`,document.body.appendChild(E),(T=E.querySelector("#pvp-hist-close"))==null||T.addEventListener("click",()=>E.remove())}async function R(){if(m.phase!==p+"-attack")return;const _=p==="p1"?"p2":"p1",E=(m.round||0)+1,T=[...m.log||[]];T.push({type:"info",text:`⏭️ ${m[p+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const M=Ue(m),L=De(_),G=M||!L?"finished":_+"-attack";await j({["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},pendingAttack:null,phase:G,attacker:_,round:E,log:T}),G==="finished"&&await Le(m)}function le(_){const E=(_.GK||[]).some(M=>!M.used),T=["DEF","MIL","ATT"].some(M=>(_[M]||[]).some(L=>!L.used));return E&&!T}function ue(_){return(_.p1Score||0)!==(_.p2Score||0)?null:le(_.p1Team)&&!$e(_.p2Team)?{side:"p1",gk:(_.p1Team.GK||[]).find(E=>!E.used)}:le(_.p2Team)&&!$e(_.p1Team)?{side:"p2",gk:(_.p2Team.GK||[]).find(E=>!E.used)}:null}async function be(){const _=document.createElement("div");_.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:950;display:flex;align-items:center;justify-content:center;overflow:hidden;text-align:center;padding:24px",document.body.appendChild(_);const E=(T,M)=>new Promise(L=>{_.innerHTML=`<div style="font-size:32px;font-weight:900;color:${M};letter-spacing:2px;animation:lcFade 1.4s ease both">${T}</div>
      <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(L,1400)});await E("⚽ DERNIER CORNER","#FFD700"),await E("🧤 LE GARDIEN MONTE !","#4fc3f7"),_.remove()}async function we(_,E){const{side:T,gk:M}=_;if(!M)return;const L=T+"Score",G=(E[L]||0)+1;M.used=!0;const D=[...E.log||[]];D.push({type:"duel",isGoal:!0,homeScored:T===p,homePlayers:T==="p1"?[qt(M)]:[],aiPlayers:T==="p2"?[qt(M)]:[],text:`⚽ DERNIER CORNER — Le gardien de ${E[T+"Name"]} marque !`});const P=(E.round||0)+1,K=T==="p1"?"p2":"p1",ee={...E,[L]:G},re=Ue(ee);q.add(P),await be();const te=T===p?G:E[p+"Score"]||0,Q=T===p?E[u+"Score"]||0:G;Ze([qt(M)],te,Q,T===p,async()=>{await j({[L]:G,log:D,round:P,pendingAttack:null,phase:re?"finished":K+"-attack",attacker:K,["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}}}),re&&await Le({...m,[L]:G})})}async function Te(){const _=m[p+"Team"],E=!["GK","DEF","MIL","ATT"].some(G=>(m[u+"Team"][G]||[]).some(D=>!D.used)),T=(m["selected_"+p]||[]).map(G=>{const D=(_[G._role]||[]).find(de=>de.cardId===G.cardId)||G,P=["GK","DEF"].includes(G._role),K=_[G._role]||[],ee=K.findIndex(de=>de.cardId===G.cardId),re=xt(K.length),te=ee>=0?re[ee]:D._col??1,Q=m.stadiumDef||m.homeStadiumDef||null,se=D.stadiumBonus||Q&&(Q.club_id&&String(D.club_id)===String(Q.club_id)||Q.country_code&&D.country_code===Q.country_code)||!1;return{...D,_line:G._role,_col:te,stadiumBonus:se,...P?{note_a:Math.max(1,Number(D.note_a)||0)}:{}}});if(!T.length)return;const M=Qt(T,m.modifiers[p]||{},m[p+"Formation"]);ge(p,T.map(G=>G.cardId)),T.forEach(G=>{const D=(_[G._role]||[]).find(P=>P.cardId===G.cardId);D&&(D.used=!0)}),m["selected_"+p]=[],W();const L=[...m.log||[]];if(E){if(T.length===1&&(T[0]._line==="GK"||T[0].job==="GK")&&(m.p1Score||0)===(m.p2Score||0)){await we({side:p,gk:T[0]},{...m,[p+"Team"]:_,log:L});return}const D=(m[p+"Score"]||0)+1,P=T.map(Q=>qt(Q));L.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:P,homeTotal:M.total,aiTotal:0});const K=(m.round||0)+1,ee=p==="p1"?"p2":"p1",re={...m,[p+"Team"]:_,[p+"Score"]:D},te=Ue(re);q.add(K),Ze(P,D,m[u+"Score"]||0,!0,async()=>{await j({[p+"Team"]:_,[p+"Score"]:D,["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},pendingAttack:null,phase:te?"finished":ee+"-attack",attacker:ee,round:K,log:L}),te&&await Le({...m,[p+"Score"]:D})});return}L.push({type:"pending",text:`⚔️ ${m[p+"Name"]} attaque (${M.total})`}),await j({[p+"Team"]:_,[u+"Team"]:m[u+"Team"],pendingAttack:{...M,players:T,side:p},["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},phase:u+"-defense",log:L})}async function Ae(){const _=m[p+"Team"],E=(m["selected_"+p]||[]).map(ce=>{const ye=(_[ce._role]||[]).find(Se=>Se.cardId===ce.cardId)||ce,he=_[ce._role]||[],ae=he.findIndex(Se=>Se.cardId===ce.cardId),Ie=xt(he.length),Oe=ae>=0?Ie[ae]:ye._col??1;return{...ye,_line:ce._role,_col:Oe}}),T=Zt(E,m.modifiers[p]||{},m[p+"Formation"]);ge(p,E.map(ce=>ce.cardId)),E.forEach(ce=>{const ye=(_[ce._role]||[]).find(he=>he.cardId===ce.cardId);ye&&(ye.used=!0)}),m["selected_"+p]=[],W();const M=Gi(m.pendingAttack.total,T.total,m.modifiers[p]||{}),L=m.pendingAttack.side,G=M==="attack"||(M==null?void 0:M.goal),D=L==="p1"?"p2":"p1",P=(m.round||0)+1,K=(m.pendingAttack.players||[]).map(ce=>qt(ce)),ee=[...m.log||[]];ee.push({type:"duel",isGoal:G,homeScored:G&&L===p,text:G?`⚽ BUT de ${m[L+"Name"]} ! (${m.pendingAttack.total} vs ${T.total})`:`✋ Attaque stoppée (${m.pendingAttack.total} vs ${T.total})`,homePlayers:K,aiPlayers:E.map(ce=>qt(ce)),homeTotal:m.pendingAttack.total,aiTotal:T.total});const re=G?(m[L+"Score"]||0)+1:m[L+"Score"]||0,te={...m,[p+"Team"]:_,[L+"Score"]:re},Q=Ue(te),se=Q?"finished":D+"-attack",de=!Q&&!G?ue(te):null,xe=async()=>{if(de){await we(de,te);return}await j({[p+"Team"]:_,[u+"Team"]:m[u+"Team"],[L+"Score"]:re,["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},pendingAttack:null,phase:se,attacker:D,round:P,log:ee}),(se==="finished"||Q)&&await Le({...m,[L+"Score"]:re})};if(G){const ce=L===p,ye=ce?re:m[p+"Score"]||0,he=ce?m[u+"Score"]||0:re;q.add(P),Ze(K,ye,he,ce,xe)}else await xe()}function ke(_){return["MIL","ATT"].some(E=>(_[E]||[]).some(T=>!T.used))}function $e(_){return["GK","DEF","MIL","ATT"].some(E=>(_[E]||[]).some(T=>!T.used))}function je(_){return $e(_)&&!ke(_)}function De(_){const E=m[_+"Team"],T=m[(_==="p1"?"p2":"p1")+"Team"];return!!(ke(E)||je(E)&&!$e(T)||je(E)&&!ke(T)&&$e(T))}function Ue(_){return!$e(_.p1Team)&&!$e(_.p2Team)}function Ee(_){const E=_.p1Score||0,T=_.p2Score||0;return E===T?null:E>T?h.home_id:h.away_id}async function Le(_){try{const E=Ee(_),T=E?h.home_id===E?h.away_id:h.home_id:null,M=_.p1Score||0,L=_.p2Score||0,{data:G,error:D}=await v.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:E,p_home_score:M,p_away_score:L,p_forfeit:!1});if(D&&console.error("[PvP] finish_pvp_match:",D),l)try{await v.rpc("check_and_finish_mini_league",{p_league_id:l})}catch(P){console.error("[PvP] check_and_finish_mini_league:",P)}E&&T&&ho(E,T).catch(()=>{});try{const{data:P}=await v.rpc("apply_match_rewards",{p_match_id:i});P!=null&&P.success&&!(P!=null&&P.skipped)&&typeof t.refreshProfile=="function"&&await t.refreshProfile()}catch(P){console.error("[PvP] apply_match_rewards:",P)}if(typeof c=="function")try{await c({homeScore:M,awayScore:L,winnerId:E,homeId:h.home_id,awayId:h.away_id,matchId:i})}catch(P){console.error("[PvP] onMatchEnd:",P)}}catch(E){console.error("[PvP] finishMatch:",E)}}function Fe(){var L;const _=m[p+"Score"],E=m[u+"Score"],T=_>E,M=_===E;dt(e),e.innerHTML=`
    ...
    </div>`,(L=document.getElementById("pvp-home"))==null||L.addEventListener("click",()=>{try{v.removeChannel(ie)}catch{}it(e),l?g("mini-league",{openLeagueId:l}):g("home")})}W()}async function Mi(e,t,i=!1){var s,d;const{state:n,navigate:r,toast:o}=t,a=i&&((d=(s=t==null?void 0:t.state)==null?void 0:s.params)==null?void 0:d.rankedData)||null;await mi(e,t,i?"ranked":"random",async({deckId:c,formation:l,starters:f,subsRaw:y,gcCardsEnriched:g,gcDefs:x,stadiumDef:p})=>{dt(e);const u=async b=>{await hr(e,t,c,l,f,y,b||[],x,p,i,a)};if(!(g!=null&&g.length)){await u([]);return}fi(e,g,u)})}async function br(e,t,i){return yr(e,t,i)}async function hr(e,t,i,n,r,o,a=[],s=[],d=null,c=!1,l=null){var oe,ie;const{state:f,navigate:y,toast:g}=t,x=(l==null?void 0:l.mmr)??((oe=f.profile)==null?void 0:oe.mmr)??1e3,p=c?Di(x):null,u=c?"linear-gradient(135deg, #1a0a2e 0%, #2d1060 50%, #1a0a2e 100%)":"linear-gradient(135deg, #0a1a2e 0%, #0d3d1e 100%)",b=c?(p==null?void 0:p.color)||"#D4A017":"#FFD700";let w=!1,h=null,$=null;e.innerHTML=`
    ...
    </div>`;const m=async()=>{w=!0,h&&(h.unsubscribe(),h=null),$&&(clearInterval($),$=null);try{await v.rpc("cancel_matchmaking",{p_user_id:f.profile.id})}catch(j){console.error("[Matchmaking] cancel error:",j)}};(ie=document.getElementById("cancel-mm"))==null||ie.addEventListener("click",async()=>{await m(),it(e),y("home")});const A=async(j,z)=>{w||(w=!0,h&&(h.unsubscribe(),h=null),$&&(clearInterval($),$=null),await At(e,t,j,z,{myGC:a,gcDefs:s,stadiumDef:d,isRanked:c,rankedData:l,onMatchEnd:c?vr:null}))},k=f.user.id,{data:I,error:q}=c?await v.rpc("try_matchmake_ranked",{p_user_id:k,p_deck_id:i,p_mmr:x}):await v.rpc("try_matchmake",{p_user_id:k,p_deck_id:i});if(q||!(I!=null&&I.success)){console.error("[Matchmaking] try_matchmake error:",q||I),g("Erreur de matchmaking","error"),it(e),y("home");return}if(I.matched){await A(I.match_id,!1);return}const ne=async()=>{if(w)return;const{data:j}=await v.from("matchmaking_queue").select("status, match_id").eq("user_id",k).maybeSingle();(j==null?void 0:j.status)==="matched"&&j.match_id&&await A(j.match_id,!0)};h=v.channel(`mm_${k}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${k}`},j=>{const z=j.new;z.status==="matched"&&z.match_id&&A(z.match_id,!0)}).subscribe(),$=setInterval(ne,3e3),setTimeout(()=>{w||m()},12e4)}async function vr(e,t){const{state:i,toast:n}=t,{winnerId:r,homeId:o,awayId:a,homeScore:s,awayScore:d,matchId:c}=e,l=i.user.id,f=o===l,y=r===l,{data:g}=await v.from("users").select("mmr, mmr_rd, mmr_v").eq("id",l).single(),{data:x}=await v.from("users").select("mmr, mmr_rd, mmr_v").eq("id",f?a:o).single();if(g&&x){const p=Kn(g.mmr,g.mmr_rd,g.mmr_v,x.mmr,x.mmr_rd,y?1:0);await v.from("users").update({mmr:p.mmr,mmr_rd:p.rd,mmr_v:p.v,mmr_wins:y?v.sql`mmr_wins + 1`:void 0,mmr_losses:y?void 0:v.sql`mmr_losses + 1`}).eq("id",l),n(y?`+MMR ↑ ${Math.round(p.mmr-g.mmr)}`:`-MMR ↓ ${Math.round(g.mmr-p.mmr)}`,y?"success":"error",4e3)}}const wr=Object.freeze(Object.defineProperty({__proto__:null,renderMatchRandom:Mi,resumePvpMatch:br},Symbol.toStringTag,{value:"Module"}));async function _r(e,t){const{state:i,navigate:n,toast:r}=t,o=i.params||{},a=o.friendId||null,s=o.friendName||"Ami",d=!!o.isAccepting;await mi(e,t,"friend",async({deckId:c,formation:l,starters:f,subsRaw:y,gcCardsEnriched:g,gcDefs:x,stadiumDef:p})=>{dt(e);const u=async b=>{const w=b||[];d?await $r(e,t,c,w,x,p):await kr(e,t,c,w,x,p,a,s)};if(!(g!=null&&g.length)){await u([]);return}fi(e,g,u)})}async function kr(e,t,i,n,r,o,a,s){var w;const{state:d,navigate:c,toast:l}=t,f=d.user.id,{data:y,error:g}=await v.from("friend_match_invites").insert({inviter_id:f,invitee_id:a,friend_id:a,status:"pending",inviter_deck_id:i,expires_at:new Date(Date.now()+2*60*1e3).toISOString()}).select("id").single();if(g||!y){console.error("[Friend] Erreur création invitation:",g),l("Impossible de créer l'invitation","error"),c("home");return}e.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    </div>`,document.body.appendChild(g),g.querySelector("#lv-ok").onclick=()=>{g.remove(),y(!0)},g.querySelector("#lv-cancel").onclick=()=>{g.remove(),y(!1)}}))return;const{data:d,error:c}=await v.rpc("leave_mini_league",{p_league_id:i,p_user_id:a});if(c||!(d!=null&&d.success)){r((d==null?void 0:d.error)||"Erreur lors de la désinscription","error");return}const l=d.refund||n;o.profile&&(o.profile.credits=(o.profile.credits||0)+l);const f=document.getElementById("nav-credits");f&&(f.textContent=`💰 ${(o.profile.credits||0).toLocaleString("fr")}`),r(`↩️ Désinscrit · +${l.toLocaleString("fr")} cr. remboursés`,"success"),tt(e,t,"waiting")}async function ji(e,t,i,n,r){const{toast:o}=t;if(!confirm(`Supprimer définitivement "${n}" et tous ses matchs/membres ? Action irréversible.`))return;await v.from("mini_league_matches").delete().eq("league_id",i),await v.from("mini_league_members").delete().eq("league_id",i);const{error:a}=await v.from("mini_leagues").delete().eq("id",i);if(a){o("Erreur suppression ("+a.message+")","error");return}o("Mini League supprimée avec succès","success"),tt(e,t,r)}async function jr(e,t,i){await v.from("mini_leagues").update({is_archived:!0}).eq("id",i),t.toast("Mini League archivée","success"),tt(e,t,"archived")}async function ct(e,t,i){var $,m,A,k,I,q,ne,oe,ie;const{state:n,toast:r}=t,o=n.profile.id,[{data:a},{data:s},{data:d}]=await Promise.all([v.from("mini_leagues").select("*").eq("id",i).single(),v.from("mini_league_members").select("*, user:users(id,pseudo,club_name,club_color1,club_color2)").eq("league_id",i),v.from("mini_league_matches").select("*").eq("league_id",i).order("matchday").order("created_at")]);if(!a){r("Introuvable","error"),await tt(e,t);return}const c=(s||[]).some(j=>j.user_id===o),l=a.creator_id===o,f=(s||[]).map(j=>j.user).filter(Boolean),y=Fr(f,d||[]),g=(d||[]).filter(j=>j.matchday===a.current_day),x=a.pot||0,p=a.entry_fee||100,u=g.length>0&&g.every(j=>j.status==="finished"||j.status==="bye"),b=a.current_day>=a.total_days,w=(s||[]).find(j=>j.user_id===o);e.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:${Kt};border:1px solid ${Vt};border-radius:12px;padding:16px">
        ...
        ${Array.from({length:Math.max(0,a.status==="active"?a.current_day-1:a.current_day)},(j,z)=>z+1).reverse().map(j=>{const z=(d||[]).filter(Y=>Y.matchday===j);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:${et};margin-bottom:6px">Journée ${j}</div>${z.map(Y=>un(Y,f,o,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
  ...
  </div>`;const h=a.status==="waiting"?"waiting":a.status==="active"?"active":"archived";if(($=document.getElementById("ml-back"))==null||$.addEventListener("click",()=>tt(e,t,h)),(m=document.getElementById("ml-refresh"))==null||m.addEventListener("click",async j=>{const z=j.currentTarget;z.style.opacity="0.5",await ct(e,t,i)}),(A=document.getElementById("ml-start-btn"))==null||A.addEventListener("click",()=>Cr(e,t,a,f)),(k=document.getElementById("ml-next-day"))==null||k.addEventListener("click",()=>Br(e,t,i)),(I=document.getElementById("ml-join-now"))==null||I.addEventListener("click",()=>Gn(e,t,i,a.type)),(q=document.getElementById("ml-leave-btn"))==null||q.addEventListener("click",()=>Mr(e,t,i,p)),(ne=document.getElementById("ml-delete-btn"))==null||ne.addEventListener("click",()=>ji(e,t,i,a.name,"waiting")),(oe=document.getElementById("ml-delete-now"))==null||oe.addEventListener("click",()=>ji(e,t,i,a.name,h)),(ie=document.getElementById("ml-archive-btn"))==null||ie.addEventListener("click",()=>jr(e,t,i)),e.querySelectorAll("[data-play-match]").forEach(j=>{j.addEventListener("click",()=>{const z=g.find(Y=>Y.id===j.dataset.playMatch);z&&t.navigate("match-mini-league",{mlMatchId:z.id,leagueId:i})})}),a.status==="finished"&&w){const j=y.findIndex(z=>z.userId===o);j>=0&&j<3&&w.prize_amount>0&&!w.prize_claimed&&setTimeout(()=>qr(e,t,a,w,j),400)}}function un(e,t,i,n,r=!1){const o=f=>t.find(y=>y.id===f);if(e.is_bye){const f=o(e.home_id);return`<div style="padding:8px;border-radius:8px;background:rgba(255,255,255,0.03);margin-bottom:6px;font-size:12px;color:${Et};text-align:center">🔵 ${(f==null?void 0:f.club_name)||(f==null?void 0:f.pseudo)||"?"} exempté(e)</div>`}const a=o(e.home_id),s=o(e.away_id),d=e.home_id===i||e.away_id===i,c=d&&e.status==="pending"&&n&&!r,l=e.status==="finished"?`${e.home_score} - ${e.away_score}`:"vs";return`<div style="display:flex;align-items:center;gap:8px;padding:10px;border-radius:8px;background:${d?"rgba(26,107,60,0.16)":"rgba(255,255,255,0.03)"};margin-bottom:6px;border:1px solid ${d?"rgba(74,222,128,0.35)":Lt}">
    ...
    </div>`;let l=!1;const f=async u=>{if(!l){if(l=!0,clearInterval(x),g)try{g.unsubscribe()}catch{}u&&await At(e,t,u,r,{mlLeagueId:n,mlMatchId:i,myGC:o||[],gcDefs:a,stadiumDef:s}),d()}};(p=document.getElementById("wait-back"))==null||p.addEventListener("click",()=>{if(l=!0,clearInterval(x),g)try{g.unsubscribe()}catch{}c("mini-league",{openLeagueId:n}),d()});const y=async()=>{if(l)return;const{data:u}=await v.from("mini_league_matches").select("match_id").eq("id",i).single();u!=null&&u.match_id&&f(u.match_id)},g=v.channel(`ml_match_${i}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"mini_league_matches",filter:`id=eq.${i}`},u=>{var b;(b=u.new)!=null&&b.match_id&&f(u.new.match_id)}).subscribe(),x=setInterval(y,3e3);y()})}const Gr="/",Rr=[{emoji:"⚽",title:"Bienvenue dans Manager Wars !",color:"#1A6B3C",content:`<p>Tu es désormais un <strong>manager de football</strong> virtuel.</p>
    ...
    <p style="margin-top:12px;font-size:13px;color:#888">Tu peux revoir ce tutoriel depuis les paramètres à tout moment.</p>`}];function Nr(e,t,i){let n=0;const r=document.createElement("div");r.id="tutorial-overlay",r.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.65);z-index:9900;display:flex;align-items:center;justify-content:center;padding:16px";const o=()=>{var l,f,y;const s=t[n],d=n===t.length-1,c=Math.round((n+1)/t.length*100);r.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
  ...
  </div>`;let d="buy";const c=()=>{var p,u,b,w,h,$,m;return{name:(((p=document.getElementById("flt-name"))==null?void 0:p.value)||"").toLowerCase().trim(),club:(((u=document.getElementById("flt-club"))==null?void 0:u.value)||"").toLowerCase().trim(),country:(((b=document.getElementById("flt-country"))==null?void 0:b.value)||"").toLowerCase().trim(),job:((w=document.getElementById("flt-job"))==null?void 0:w.value)||"",rarity:((h=document.getElementById("flt-rarity"))==null?void 0:h.value)||"",note1:parseInt(($=document.getElementById("flt-note1"))==null?void 0:$.value)||0,note2:parseInt((m=document.getElementById("flt-note2"))==null?void 0:m.value)||0}};function l(p){const u=c();return p.filter(b=>{var q,ne,oe;const w=(q=b.card)==null?void 0:q.player;if(!w)return!1;const h=`${w.firstname} ${w.surname_real}`.toLowerCase(),$=(((ne=w.clubs)==null?void 0:ne.encoded_name)||"").toLowerCase(),m=(w.country_code||"").toLowerCase(),A=((oe=b.card)==null?void 0:oe.evolution_bonus)||0,k=Dt(w,w.job,A),I=w.job2?Dt(w,w.job2,A):0;return!(u.name&&!h.includes(u.name)||u.club&&!$.includes(u.club)||u.country&&!m.includes(u.country)||u.job&&w.job!==u.job||u.rarity&&w.rarity!==u.rarity||u.note1&&k<u.note1||u.note2&&I<u.note2)})}function f(p){var I,q,ne,oe;const u=(I=p.card)==null?void 0:I.player;if(!u)return"";const b=((q=p.card)==null?void 0:q.evolution_bonus)||0,w=Dt(u,u.job,b),h=u.job2?Dt(u,u.job2,b):0,$=(i.profile.credits||0)>=p.price,m=u.country_code?`https://flagsapi.com/${u.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,A=ai[u.job]||"#bbb",k=u.job2?ai[u.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden">
      <!-- Drapeau -->
    ...
    </div>`}function y(p){var I,q,ne,oe;const u=(I=p.card)==null?void 0:I.player;if(!u)return"";const b=((q=p.card)==null?void 0:q.evolution_bonus)||0,w=Dt(u,u.job,b),h=u.job2?Dt(u,u.job2,b):0,$=p.status==="sold",m=u.country_code?`https://flagsapi.com/${u.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,A=ai[u.job]||"#bbb",k=u.job2?ai[u.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${$?"opacity:0.7":""}">
      ${m?`<img src="${m}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      ${(ne=u.clubs)!=null&&ne.logo_url?`<img src="${u.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${A};display:flex;align-items:center;justify-content:center">
    ...
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center">
        ...
        <button id="buy-ok" style="flex:1;padding:12px;border-radius:10px;border:none;background:var(--green);color:#fff;font-size:14px;font-weight:900;cursor:pointer">Confirmer</button>
          ...
          <div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${j?"border:2px solid var(--yellow)":""}">
            ...
            <div style="flex:1;min-width:0">
              ...
              <div style="font-size:11px;color:var(--tile-fg-dim)">${I.club_name} · ${ne.label}</div>
            </div>
            <div style="text-align:right;flex-shrink:0">
              <div style="font-size:20px">${z?"❓":ne.emoji}</div>
              <div style="font-size:11px;font-weight:700;color:${ne.color}">${z?"Placement":ne.label}</div>
              ${z?"":`<div style="font-size:10px;color:var(--tile-fg-dim)">${ie}% WR</div>`}
            </div>
          </div>`}).join(""):'<div style="text-align:center;color:var(--tile-fg-dim);padding:40px">Aucun joueur classé.</div>'}else if(d==="mini-league"){const k=a||[];h.innerHTML=k.length>0?k.map((I,q)=>`
      ...
      `).join(""):`<div style="text-align:center;color:var(--tile-fg-dim);padding:40px">Aucun résultat de Mini League pour l'instant.</div>`}else{const k=s||[];h.innerHTML=k.length>0?k.map((I,q)=>`
        <div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${I.user_id===i.profile.id?"border:2px solid var(--yellow)":""}">
          <div style="width:32px;height:32px;border-radius:50%;background:${c(q)};color:${l(q)};display:flex;align-items:center;justify-content:center;font-weight:900;flex-shrink:0;font-size:${q<3?"16":"13"}px">${q<3?["🥇","🥈","🥉"][q]:q+1}</div>
          <div style="flex:1;min-width:0">
            <div style="font-weight:700;color:var(--tile-fg-on-page);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${I.pseudo}</div>
  ...
  <div class="page">
    ...
    </div>`,(A=document.getElementById("ranked-back"))==null||A.addEventListener("click",()=>n("home"));return}const d=s.mmr??1e3,c=s.mmr_deviation??350,l=s.mmr_volatility??.06,f=s.placement_matches??0,y=f<10,g=Math.max(0,10-f),x=Di(d),p=Vn(d),u=bi.findIndex(ne=>ne.id===x.id),b=bi[u+1]||null,w={bronze:"linear-gradient(160deg,#3d1c00,#7a3e00)",silver:"linear-gradient(160deg,#1a1a2e,#3a3a5e)",gold:"linear-gradient(160deg,#1a1200,#4a3500)",platinum:"linear-gradient(160deg,#001a20,#003040)",diamond:"linear-gradient(160deg,#001030,#1a2860)",master:"linear-gradient(160deg,#1a0030,#3d0070)"},h=(s.ranked_wins||0)+(s.ranked_losses||0)+(s.ranked_draws||0),$=h>0?Math.round((s.ranked_wins||0)/h*100):0,m=bi.map(ne=>`
  ...
  </div>`,(k=document.getElementById("ranked-back"))==null||k.addEventListener("click",()=>n("home")),(I=document.getElementById("ranked-leaderboard-btn"))==null||I.addEventListener("click",()=>n("rankings",{tab:"ranked"})),(q=document.getElementById("ranked-play-btn"))==null||q.addEventListener("click",()=>{n("match",{matchMode:"ranked",rankedData:{mmr:d,rd:c,sigma:l,isPlacement:y}})})}async function Xr(e,{state:t,navigate:i,toast:n}){const r=t.profile;if(!r)return;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await v.from("matches").select(`id,home_id,away_id,home_score,away_score,status,mode,winner_id,created_at,played_at,
      home:users!home_id(pseudo,club_name),
      away:users!away_id(pseudo,club_name)`).or(`home_id.eq.${r.id},away_id.eq.${r.id}`).order("created_at",{ascending:!1}).limit(50),a={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},s=(o||[]).filter(l=>l.status==="finished"),d=(o||[]).filter(l=>l.status==="in_progress");function c(l){const f=l.home_id===r.id;f?l.home_score:l.away_score,f?l.away_score:l.home_score;const y=l.winner_id===r.id,g=l.home_score===l.away_score&&l.status==="finished",x=l.status!=="finished"?"…":g?"N":y?"V":"D",p=l.status!=="finished"||g?"#888":y?"#1A6B3C":"#c0392b";let u=a[l.mode]||l.mode;l.away_id===null&&!u.startsWith("IA")&&(u="IA");const w=l.home_id===r.id?l.away:l.home;let h;l.away_id===null?h=u:w?h=`${w.club_name||w.pseudo} (${w.pseudo})`:h="Adversaire";let $="";l.status==="in_progress"&&Date.now()-new Date(l.created_at).getTime()>3600*1e3&&($=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const m=new Date(l.created_at),A=m.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+m.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),k=l.status==="finished"?`${l.home_score} - ${l.away_score}`:`${l.home_score||0} - ${l.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
  ...
  <div class="page">
    ...
    <nav class="top-nav">
      <div class="logo" id="nav-logo" title="Manager Wars v2026.07.21-1156" style="cursor:pointer">
