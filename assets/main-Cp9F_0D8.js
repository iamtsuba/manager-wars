...
import{s as w,F as si,r as Ae,j as St,l as Pi,m as yt,h as di,n as gn,o as mn,T as Yt,k as xn,p as yn,b as bn}from"./formation-links-D0diUT_D.js";const hn="/";function Li(e,{navigate:t,toast:i}){let n="login";const a=()=>{var r,s,d,l,c,u;const o=n==="login";e.innerHTML=`
    <div style="
  ...
  `;function s(){var f;const l=document.getElementById("logo-preview"),c=document.getElementById("logo-initials"),u=((f=document.getElementById("setup-club"))==null?void 0:f.value)||"MW",g=u.trim().split(" ").filter(Boolean),x=g.length>=2?(g[0][0]+g[1][0]).toUpperCase():u.slice(0,2).toUpperCase();l&&(l.style.background=r,l.style.borderColor=o),c&&(c.textContent=x,c.style.color=o)}document.getElementById("color1").addEventListener("input",l=>{o=l.target.value,document.getElementById("swatch1").style.background=o,s()}),document.getElementById("color2").addEventListener("input",l=>{r=l.target.value,document.getElementById("swatch2").style.background=r,s()});function d(l){document.querySelectorAll(".setup-step").forEach(c=>c.classList.remove("active")),document.getElementById(`step-${l}`).classList.add("active"),document.getElementById("step-num").textContent=l,document.getElementById("progress-fill").style.width=`${Math.round(l/3*100)}%`,l===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const l=document.getElementById("setup-pseudo").value.trim(),c=document.getElementById("step1-error");if(c.textContent="",l.length<3){c.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:u}=await w.from("users").select("id").eq("pseudo",l).maybeSingle();if(u){c.textContent="Ce pseudo est déjà pris.";return}d(2)}),document.getElementById("step2-back").addEventListener("click",()=>d(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const l=document.getElementById("setup-club").value.trim(),c=document.getElementById("step2-error");if(c.textContent="",l.length<2){c.textContent="Nom trop court (min. 2 caractères).";return}const{data:u}=await w.from("users").select("id").eq("club_name",l).maybeSingle();if(u){c.textContent="Ce nom de club est déjà pris.";return}d(3)}),document.getElementById("step3-back").addEventListener("click",()=>d(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const l=document.getElementById("setup-pseudo").value.trim(),c=document.getElementById("setup-club").value.trim(),u=document.getElementById("step3-error"),g=document.getElementById("step3-finish");u.textContent="",g.disabled=!0,g.textContent="Création en cours…";try{const{error:x}=await w.from("users").insert({id:t.user.id,pseudo:l,club_name:c,club_color1:o,club_color2:r,credits:1e4});if(x)throw x;await wn(t.user.id),await a(),n(`Bienvenue ${l} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(x){u.textContent=x.message,g.disabled=!1,g.textContent="🚀 Créer mon profil !"}})}async function wn(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await w.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const _n="modulepreload",kn=function(e){return"/"+e},zi={},Ri=function(t,i,n){let a=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const r=document.querySelector("meta[property=csp-nonce]"),s=(r==null?void 0:r.nonce)||(r==null?void 0:r.getAttribute("nonce"));a=Promise.allSettled(i.map(d=>{if(d=kn(d),d in zi)return;zi[d]=!0;const l=d.endsWith(".css"),c=l?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${c}`))return;const u=document.createElement("link");if(u.rel=l?"stylesheet":_n,l||(u.as="script"),u.crossOrigin="",u.href=d,s&&u.setAttribute("nonce",s),document.head.appendChild(u),l)return new Promise((g,x)=>{u.addEventListener("load",g),u.addEventListener("error",()=>x(new Error(`Unable to preload CSS for ${d}`)))})}))}function o(r){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=r,window.dispatchEvent(s),!s.defaultPrevented)throw r}return a.then(r=>{for(const s of r||[])s.status==="rejected"&&o(s.reason);return t().catch(o)})},Et="#1A6B3C",Lt="#cc2222",$n="#D4A017",Ti="#888";async function En(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    </style>`}const In="2026.07.12-1529";async function Ii(e,{state:t,navigate:i,toast:n}){var o,r;const a=t.profile;a&&(e.innerHTML=`
  <style>
    .home-dark {
      min-height: 100%;
  ...
  <div class="home-dark" id="home-dark">
    ...
    ${Ae(e,{width:140})}
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),l=(s||[]).filter(T=>T.card_type==="player"&&T.player),c=(s||[]).filter(T=>T.card_type==="game_changer"),u=(s||[]).filter(T=>T.card_type==="formation"),g=(s||[]).filter(T=>T.card_type==="stadium"),{data:x}=await w.from("gc_definitions").select("name,gc_type,color,effect,image_url"),f={};(x||[]).forEach(T=>{f[T.name]=T});const{data:p}=await w.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),m={};(p||[]).forEach(T=>{m[T.id]=T}),g.forEach(T=>{T.stadium_def&&(m[T.stadium_id]=T.stadium_def)});const b=Object.keys(si),v=Object.keys(Se),h={};l.forEach(T=>{const S=T.player.id;h[S]=(h[S]||0)+1}),new Set(Object.keys(h).map(T=>String(T)));const $=new Set(u.map(T=>T.formation)),y=new Set(c.map(T=>T.gc_type));let C="player",E="Tous",P="",O=!1;function ee(){return[...l].sort((T,S)=>{const G=jt.indexOf(T.player.job),N=jt.indexOf(S.player.job);return G!==N?G-N:(T.player.surname_real||"").localeCompare(S.player.surname_real||"")})}function ie(){return[...d||[]].sort((T,S)=>{const G=jt.indexOf(T.job),N=jt.indexOf(S.job);return G!==N?G-N:(T.surname_real||"").localeCompare(S.surname_real||"")})}function le(){return ee().filter(T=>{const S=T.player,G=E==="Tous"||S.job===E,N=!P||`${S.firstname} ${S.surname_real}`.toLowerCase().includes(P);return G&&N})}function B(){return ie().filter(T=>{const S=E==="Tous"||T.job===E,G=!P||`${T.firstname} ${T.surname_real}`.toLowerCase().includes(P);return S&&G})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
  ...
  </div>`;function z(){const T=document.getElementById("col-filters");T&&(C==="player"?(T.innerHTML=`
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",S=>{P=S.target.value.toLowerCase(),k()}),e.querySelectorAll(".filter-btn").forEach(S=>{S.addEventListener("click",()=>{E=S.dataset.job,z(),k()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{O=!O,z(),k()})):(T.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{O=!O,z(),k()})))}function k(){const T=document.getElementById("col-grid");T&&(C==="player"?Ee(T):C==="formation"?U(T):C==="stadium"?K(T):Y(T))}function F(T,S,G,N,V){const j=document.getElementById("col-grid"),H=document.getElementById("col-big");if(!j||!H)return;var $e=0;function _e(){const we=window.innerWidth>=768,me=document.getElementById("col-big"),Le=document.getElementById("col-grid");we&&me&&(me.style.minHeight="420px",me.style.flex="1 1 auto"),we&&Le&&(Le.style.height=Math.round(310*.76+16)+"px",Le.style.flexShrink="0",Le.style.overflowX="auto",Le.style.overflowY="hidden",Le.style.alignItems="center",Le.style.padding="8px 16px"),H.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+S(T[$e])+"</div>";var ke=H.querySelector("[data-card-id],[data-form-id],[data-gc-id]");ke&&ke.addEventListener("click",function(){N(T[$e])}),requestAnimationFrame(function(){var he=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!he||!H)){var Ce=H.clientHeight-8,je=H.clientWidth-24,Fe=he.offsetHeight,_=he.offsetWidth;if(Fe>0&&_>0&&Ce>40){var L=we?2.2:1.6,A=Math.min(Ce/Fe,je/_,L);he.style.transform="scale("+A.toFixed(3)+")",he.style.transformOrigin="top center"}}}),j.innerHTML=T.map(function(he,Ce){var je=Ce===$e,Fe="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+(je?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+Ce+'" style="'+Fe+'">'+G(he,je)+"</div>"}).join(""),j.querySelectorAll(".col-mini-item").forEach(function(he){he.addEventListener("click",function(){$e=Number(he.dataset.idx),_e(),he.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}_e()}function re(T){var S=window.innerWidth>=768?.76:.54,G;if(!T||T._fake){var N=T?T.player:null;if(!N)return"";G=Mi(N)}else G=Jt(T,"");return'<div style="display:inline-block;zoom:'+S+';pointer-events:none;line-height:0">'+G+"</div>"}function pe(T,S,G){S=S||100,G=G||140;var N=St[T]||{},V={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},j=Math.max(3,Math.round(S*.06)),H=Object.entries(N).map(function(_e){var we=_e[0],me=_e[1],Le=we.replace(/\d+$/,""),ke=V[Le]||"#888",he=Math.round(me.x*S),Ce=Math.round(me.y*G);return'<circle cx="'+he+'" cy="'+Ce+'" r="'+j+'" fill="'+ke+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),$e=Math.max(1,Math.round(S/50));return'<svg viewBox="0 0 '+S+" "+G+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+S+'" height="'+G+'" fill="#1A6B3C"/><rect x="'+Math.round(S*.2)+'" y="'+Math.round(G*.02)+'" width="'+Math.round(S*.6)+'" height="'+Math.round(G*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+$e+'"/><line x1="0" y1="'+Math.round(G*.5)+'" x2="'+S+'" y2="'+Math.round(G*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+$e+'"/><ellipse cx="'+Math.round(S*.5)+'" cy="'+Math.round(G*.5)+'" rx="'+Math.round(S*.18)+'" ry="'+Math.round(G*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+$e+'"/><rect x="'+Math.round(S*.2)+'" y="'+Math.round(G*.82)+'" width="'+Math.round(S*.6)+'" height="'+Math.round(G*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+$e+'"/>'+H+"</svg>"}function de(T,S,G){var N=G>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+G+"</div>":"",V=S?'data-form-id="'+S.id+'"':"",j=T.length>7?14:T.length>5?16:19,H=!!S;return"<div "+V+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(H?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(H?"":"filter:grayscale(1);opacity:0.5")+'">'+N+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(H?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+j+"px;font-weight:900;color:"+(H?"#1A6B3C":"#aaa")+';line-height:1">'+T+'</div></div><div style="flex:1;overflow:hidden;background:'+(H?"#1A6B3C":"#ccc")+'">'+pe(T,140,220)+"</div></div>"}function ae(T,S){var G=window.innerWidth>=768?.76:.54,N=140,V=305,j=Math.round(V*.22),H=V-j,$e=T.length>7?10:13,_e=pe(T,N,H),we=S?"1.5px solid #2a7a40":"1px solid #ddd",me=S?"":"filter:grayscale(1);opacity:0.45;",Le=S?"#1A6B3C":"#bbb",ke="#fff";return'<div style="display:inline-block;zoom:'+G+';line-height:0;pointer-events:none"><div style="width:'+N+"px;height:"+V+"px;border-radius:6px;border:"+we+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+me+'"><div style="height:'+j+"px;background:"+Le+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+$e+"px;font-weight:900;color:"+ke+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(N-4)+'px">'+T+'</span></div><div style="height:'+H+'px;overflow:hidden;flex-shrink:0">'+_e+"</div></div></div>"}function Ee(T){if(O){const S=B();if(!S.length){T.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const G=S.map(N=>l.find(V=>V.player.id===N.id)||{_fake:!0,player:N,id:"fake-"+N.id});F(G,N=>N._fake?Mi(N.player):Jt(N,""),N=>N._fake?re({player:N.player,id:"x",_fake:!0}):re(N),N=>{N._fake||Ci(N,l,h,t)})}else{const S=le();if(!S.length){T.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const G={};S.forEach(V=>{const j=V.player.id;G[j]||(G[j]=[]),G[j].push(V)});const N=Object.values(G).map(V=>Pn(V));F(N,V=>{const j=h[V.player.id]||1,H=j>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${j}</div>`:"",_e=l.filter(we=>we.player.id===V.player.id&&we.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Jt(V,H+_e)},V=>re(V),V=>Ci(V,l,h,t))}}function U(T){const S=O?b:[...$];if(!S.length){T.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const G=S.map(N=>({formation:N,card:u.find(V=>V.formation===N)||null,owned:$.has(N)}));F(G,({formation:N,card:V,owned:j})=>de(N,j?V:null,j?u.filter(H=>H.formation===N).length:0),({formation:N,owned:V})=>ae(N,V),({card:N,owned:V})=>{V&&N&&Kn(N,u,t,o)})}function Y(T){const S=Object.keys(f),G=O?S.length?S:v:[...y];if(!G.length){T.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const N=G.map(V=>({type:V,gc:Se[V]||{icon:"⚡",desc:""},def:f[V]||null,owned:y.has(V),card:c.find(j=>j.gc_type===V)||null}));F(N,({type:V,gc:j,def:H,owned:$e,card:_e})=>{var L;const we=$e?c.filter(A=>A.gc_type===V).length:0,me=we>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${we}</div>`:"",Le=(H==null?void 0:H.gc_type)==="ultra_game_changer",ke={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},he={purple:"#b06ce0",light_blue:"#00d4ef"},Ce=ke[H==null?void 0:H.color]||ke.purple,je=he[H==null?void 0:H.color]||he.purple,Fe=(H==null?void 0:H.effect)||j.desc||"",_=H!=null&&H.image_url?`/icons/${H.image_url}`:((L=H==null?void 0:H.club)==null?void 0:L.logo_url)||(H!=null&&H.country_code?`https://flagsapi.com/${H.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null);return $e&&_e?`<div data-gc-id="${_e.id}" data-gc-type="${V}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${je};background:${Ce};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${je}66;cursor:pointer">
          ${me}
        ...
        </div>`},({type:V,gc:j,def:H,owned:$e})=>{const _e=window.innerWidth>=768?.76:.54,we={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},me={purple:"#9b59b6",light_blue:"#00bcd4"},Le=we[H==null?void 0:H.color]||we.purple,ke=me[H==null?void 0:H.color]||me.purple,he=H!=null&&H.image_url?`/icons/${H.image_url}`:null;return $e?`<div style="display:inline-block;zoom:${_e};line-height:0;pointer-events:none"><div style="width:140px;height:310px;border-radius:8px;background:${Le};border:1px solid ${ke};display:flex;flex-direction:column;overflow:hidden"><div style="height:56px;background:rgba(255,255,255,0.15);display:flex;align-items:center;justify-content:center"><span style="font-size:13px;font-weight:900;color:#fff;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:130px">${V}</span></div><div style="flex:1;display:flex;align-items:center;justify-content:center">${he?`<img src="${he}" style="max-width:130px;max-height:190px;object-fit:contain">`:`<span style="font-size:48px">${j.icon}</span>`}</div><div style="height:54px;display:flex;align-items:center;justify-content:center;padding:0 4px"><span style="font-size:10px;color:rgba(255,255,255,0.7);text-align:center">${((H==null?void 0:H.effect)||j.desc||"").slice(0,30)}</span></div></div></div>`:`<div style="display:inline-block;zoom:${_e};line-height:0;pointer-events:none"><div style="width:140px;height:310px;border-radius:8px;background:#eee;border:1px solid #ddd;display:flex;flex-direction:column;align-items:center;justify-content:center;filter:grayscale(1);opacity:0.45"><span style="font-size:36px">${j.icon}</span><span style="font-size:12px;color:#aaa;margin-top:6px;text-align:center;padding:0 6px">${V}</span></div></div>`},({type:V,owned:j,def:H})=>{j&&Un(V,H,o)})}function K(T){const S="#E87722",G="/";if(!g.length){T.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Stade.<br><small>Ouvre un booster Stade !</small></div>';return}const N={};g.forEach(j=>{const H=j.stadium_id||"?";(N[H]=N[H]||[]).push(j)});const V=Object.entries(N).map(([j,H])=>({sid:j,def:m[j]||null,count:H.length,card:H[0]}));F(V,({def:j,count:H})=>{var ke,he;const $e=(j==null?void 0:j.name)||"?",_e=((ke=j==null?void 0:j.club)==null?void 0:ke.encoded_name)||(j==null?void 0:j.country_code)||"—",we=j!=null&&j.image_url?`${G}icons/${j.image_url}`:((he=j==null?void 0:j.club)==null?void 0:he.logo_url)||(j!=null&&j.country_code?`https://flagsapi.com/${j.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),me=we?`<img src="${we}" style="width:90px;height:90px;object-fit:contain;border-radius:4px" onerror="this.style.display='none'">`:'<div style="font-size:56px">🏟️</div>',Le=H>1?`<div style="position:absolute;top:8px;right:8px;background:#333;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${H}</div>`:"";return`<div style="position:relative;width:140px;border-radius:12px;border:3px solid #c45a00;background:linear-gradient(160deg,${S},#c45a00);display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${S}66">
          ${Le}
        ...
        </div>`},({def:j,count:H})=>{var ke,he;const $e=window.innerWidth>=768?.76:.54,_e=(j==null?void 0:j.name)||"?",we=((ke=j==null?void 0:j.club)==null?void 0:ke.encoded_name)||(j==null?void 0:j.country_code)||"—",me=j!=null&&j.image_url?`${G}icons/${j.image_url}`:((he=j==null?void 0:j.club)==null?void 0:he.logo_url)||(j!=null&&j.country_code?`https://flagsapi.com/${j.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),Le=me?`<img src="${me}" style="width:64px;height:64px;object-fit:contain;border-radius:4px" onerror="this.style.display='none'">`:'<span style="font-size:32px">🏟️</span>';return`<div style="display:inline-block;zoom:${$e};line-height:0;pointer-events:none">
        ...
        </div>`},null)}e.querySelectorAll(".col-tab-btn").forEach(T=>{T.addEventListener("click",()=>{C=T.dataset.tab,E="Tous",P="",O=!1,e.querySelectorAll(".col-tab-btn").forEach(S=>{const G=S.dataset.tab===C;S.style.borderBottomColor=G?"var(--green)":"transparent",S.style.color=G?"var(--green)":"var(--gray-600)"}),z(),k()})}),z(),k()}function Un(e,t,i){const n=Se[e]||{icon:"⚡",desc:"Effet spécial."},a=(t==null?void 0:t.gc_type)==="ultra_game_changer",o={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},r={purple:"#b06ce0",light_blue:"#00d4ef"},s=o[t==null?void 0:t.color]||o.purple,d=r[t==null?void 0:t.color]||r.purple,l=(t==null?void 0:t.name)||e,c=(t==null?void 0:t.effect)||n.desc,u=t!=null&&t.image_url?`/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Bt=1e3;function Kn(e,t,i,n){var f,p,m;const{state:a,toast:o,closeModal:r,navigate:s,refreshProfile:d}=i,l=e.formation,c={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function u(){const b=St[l]||{},v=si[l]||[],h=290,$=360,y=20;function C(P){const O=b[P];return O?{x:O.x*h,y:O.y*$}:null}let E=`<svg width="${h}" height="${$}" viewBox="0 0 ${h} ${$}" xmlns="http://www.w3.org/2000/svg">`;for(const[P,O]of v){const ee=C(P),ie=C(O);!ee||!ie||(E+=`<line x1="${ee.x}" y1="${ee.y}" x2="${ie.x}" y2="${ie.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const P of Object.keys(b)){const O=C(P);if(!O)continue;const ee=P.replace(/\d+/,""),ie=c[ee]||"#555";E+=`<circle cx="${O.x}" cy="${O.y}" r="${y}" fill="${ie}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,E+=`<text x="${O.x}" y="${O.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${ee}</text>`}return E+="</svg>",E}const g=t.filter(b=>b.formation===l);g.length;const x=!e.is_for_sale;n(`Formation ${l}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${u.map((U,Y)=>{const K=le[U.id]||[],T=U.is_for_sale,S=K.length?K[K.length-1]:null,N=(c.rarity==="pepite"||c.rarity==="papyte")&&U.current_note!=null?` (☆${U.current_note})`:"";return`
            <div data-card-id="${U.id}" data-card-idx="${Y}" class="exemplaire-row"
              style="border:2px solid #eee;border-radius:10px;padding:10px;cursor:pointer;transition:border-color .15s;${T?"opacity:0.6":""}">
              <div style="display:flex;align-items:center;gap:10px">
                <input type="checkbox" class="expl-check" data-id="${U.id}" ${T?"disabled":""} style="width:18px;height:18px;cursor:pointer;accent-color:#1A6B3C;flex-shrink:0">
                <div style="flex:1;min-width:0">
                  <div style="font-size:12px;font-weight:700">Exemplaire ${Y+1}${N}${T?" 🏷️ En vente":""}</div>
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;background:#f0fdf4;border-radius:10px;padding:10px 14px">
          <div>
            ...
            <div style="font-size:11px;color:#555;margin-top:2px">
              ${(e.evolution_bonus||0)>0?`+${e.evolution_bonus} appliqué${e.evolution_bonus>1?"s":""} · Note : ${P}${c.job2&&O>0?` / ${ee}`:""}`:`Note actuelle : ${P}${c.job2&&O>0?` / ${ee}`:""}`}
            </div>
          </div>
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",c.note_g],["DEF",c.note_d],["MIL",c.note_m],["ATT",c.note_a]].map(([U,Y])=>{const K=Wt[U],T=U==="GK"?"#fff":K,S=U===c.job||U===c.job2,G=(Number(Y)||0)+(S&&C>0?C:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
                <div style="width:48px;height:48px;border-radius:8px;background:#111;border:2.5px solid ${K};
                  display:flex;align-items:center;justify-content:center">
                  <span style="font-size:20px;font-weight:900;color:${T};font-family:Arial Black,Arial;line-height:1">${G}</span>
                </div>
                <span style="font-size:10px;font-weight:700;color:${T}">${U}</span>
              </div>`}).join("")}
          </div>
        </div>
        <div>
    ...
    ${z}
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(pe=document.getElementById("close-detail"))==null||pe.addEventListener("click",s);let k=new Set;const F=()=>{const U=k.size,Y=document.getElementById("sell-action-panel");if(!Y)return;Y.style.display=U>0?"block":"none",document.getElementById("sell-selected-count").textContent=U;const K=document.getElementById("evolve-btn");K&&(K.textContent=`⬆️ Évoluer ${U>1?"(+"+U+")":""}`)};document.querySelectorAll(".expl-check").forEach(U=>{U.addEventListener("change",()=>{const Y=U.dataset.id;U.checked?k.add(Y):k.delete(Y);const K=U.closest(".exemplaire-row");K&&(K.style.borderColor=U.checked?"#1A6B3C":"#eee"),F()})}),document.querySelectorAll(".exemplaire-row").forEach(U=>{U.addEventListener("click",Y=>{if(Y.target.closest("button")||Y.target.tagName==="INPUT")return;const K=U.querySelector(".expl-check");K&&!K.disabled&&(K.checked=!K.checked,K.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(U=>{U.addEventListener("click",Y=>{Y.stopPropagation();const K=document.querySelector(`.expl-hist[data-hist="${U.dataset.idx}"]`);K&&(K.style.display=K.style.display==="none"?"flex":"none")})}),(de=document.getElementById("evolve-btn"))==null||de.addEventListener("click",async()=>{if(g<=1)return;const U=[...k];if(!U.length)return;if(k.has(e.id)){const V=document.createElement("div");V.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",V.innerHTML=`
        ...
        </div>`,document.body.appendChild(V),V.querySelector("#err-close").addEventListener("click",()=>V.remove()),V.addEventListener("click",j=>{j.target===V&&V.remove()});return}const Y=U.filter(V=>V!==e.id),K=c.rarity==="legende"?2:1;if((Y.length||1)*K,!(!Y.length&&U.length===1&&U[0]===e.id)){if(!Y.length){o("Sélectionne des copies à sacrifier","warning");return}}if(!await new Promise(V=>{const j=document.createElement("div");j.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",j.innerHTML=`
        <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center;box-shadow:0 16px 48px rgba(0,0,0,0.3)">
          ...
          <div style="background:#f0fdf4;border-radius:10px;padding:12px;margin-bottom:16px;font-size:13px;color:#333">
            ...
            📈 Note : <strong>${v+(e.evolution_bonus||0)}</strong> → <strong style="color:#1A6B3C">${v+(e.evolution_bonus||0)+Y.length*K}</strong>
            ${$&&$>0?`<br>📈 Note 2 : <strong>${$+(e.evolution_bonus||0)}</strong> → <strong style="color:#1A6B3C">${$+(e.evolution_bonus||0)+Y.length*K}</strong>`:""}
  ...
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
    ...
    </div>`;m("⚽ Choisir une formation",v),document.querySelectorAll("#modal-body [data-forma]").forEach(h=>{h.addEventListener("click",()=>{t.formation=h.dataset.forma;const $=Bi(t.formation),y={};$.forEach(C=>{t.slots[C]&&(y[C]=t.slots[C])}),t.slots=y,b(),Ze(e,t,i)})})};e.querySelectorAll("#formation-mobile-btn, #formation-pc-btn").forEach(m=>m.addEventListener("click",x)),e.querySelectorAll("#add-stad-btn-pc, #add-stad-btn").forEach(m=>m.addEventListener("click",()=>Yn(t,e,i))),e.querySelectorAll("#save-deck").forEach(m=>m.addEventListener("click",()=>Jn(t,i))),e.querySelectorAll("[data-remove-sub]").forEach(m=>{m.addEventListener("click",()=>{t.subs=t.subs.filter(b=>b!==m.dataset.removeSub),Ze(e,t,i)})}),e.querySelectorAll("#add-sub-btn").forEach(m=>m.addEventListener("click",()=>Xn(t,e,i)))}function Vn(e,t,i,n){var h,$;const a=window.innerWidth>=900,o=e.querySelector(a?"#deck-field-pc":"#deck-field-mobile");if(!o)return;const r=(h=t.stadiumCards)==null?void 0:h.find(y=>y.id===t.stadiumCardId),s=r&&(($=t.stadDefMap)==null?void 0:$[r.stadium_id])||null,d=St[t.formation]||{},l=Pi(t.formation)||[],c={};for(const y of i){const C=t.slots[y],E=C?t.playerCards.find(P=>P.id===C):null;E!=null&&E.player?c[y]={...E.player,_evolution_bonus:E.evolution_bonus||0,face:E.player.face||null}:c[y]=null}const u=window.innerWidth>=900,g=u?window.innerWidth-280:window.innerWidth-20,x=u?Math.min(g,860):g,f=Math.round(u?x*.82:x*.85),p=u?84:44;let m="";for(const[y,C]of l){const E=d[y],P=d[C];if(!E||!P)continue;const O=E.x*x,ee=Math.round(.03*f+E.y*.85*f),ie=P.x*x,le=Math.round(.03*f+P.y*.85*f),B=c[y],z=c[C],k=yt(B,z);k==="#ff3333"||k==="#cc2222"?m+=`<line x1="${O.toFixed(1)}" y1="${ee.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${le.toFixed(1)}" stroke="${k}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(m+=`<line x1="${O.toFixed(1)}" y1="${ee.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${le.toFixed(1)}" stroke="${k}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,m+=`<line x1="${O.toFixed(1)}" y1="${ee.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${le.toFixed(1)}" stroke="${k}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let b="";const v=Math.round(p*657/507);for(const y of i){const C=d[y];if(!C)continue;const E=c[y],P=C.x*x;C.y*f;const O=Math.round(.03*f+C.y*(.85*f)),ee=Math.round(P-p/2),ie=Math.round(O-v/2);if(E){const le=y.replace(/\d+/,""),B=s&&(s.club_id&&String(E.club_id)===String(s.club_id)||s.country_code&&E.country_code===s.country_code),z=Ae({...E,_evolution_bonus:E._evolution_bonus||0},{width:p,showStad:!0,stadDef:s,role:le});b+=`<div style="position:absolute;left:${ee}px;top:${ie}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${y}">
        <div style="position:relative">${B?'<div style="position:absolute;top:-18px;left:0;right:0;text-align:center;font-size:14px;z-index:5;line-height:1">🏟️</div>':""}${z}</div>
  ...
  `,document.body.appendChild(a);let o=!1;const r=document.getElementById("pack-cut-zone"),s=document.getElementById("pack-blade");let d=!1;const l=z=>z.touches&&z.touches[0]?z.touches[0].clientX:z.clientX;function c(z){o||(d=!0,s.style.opacity="1",u(z))}function u(z){if(!d||o)return;const k=r.getBoundingClientRect(),F=l(z)-k.left,re=Math.max(0,Math.min(1,F/k.width));s.style.width=re*k.width+"px",re>=.82&&x()}function g(){o||(d=!1,s.style.transition="width .2s ease, opacity .2s ease",s.style.width="0",s.style.opacity="0",setTimeout(()=>{o||(s.style.transition="")},220))}function x(){var k;if(o)return;o=!0,d=!1,s.style.width="100%",s.style.opacity="1",(k=document.getElementById("cut-flash"))==null||k.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const z=document.getElementById("cut-hint");z&&(z.style.opacity="0"),r.classList.add("pack-cut"),setTimeout(()=>{s.style.opacity="0",document.getElementById("pack-phase").style.display="none",b(0)},620)}r.addEventListener("pointerdown",c),window.addEventListener("pointermove",u),window.addEventListener("pointerup",g),r.addEventListener("touchstart",c,{passive:!0}),window.addEventListener("touchmove",u,{passive:!0}),window.addEventListener("touchend",g);let f=0,p=!1;const m=new Set;function b(z){f=z,document.getElementById("reveal-phase").style.display="flex",v(),h(z,0),P()}function v(){const z=document.getElementById("card-dots");z&&(z.innerHTML=e.map((k,F)=>`<div class="card-dot" data-i="${F}" style="width:8px;height:8px;border-radius:50%;background:${F===f?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),z.querySelectorAll(".card-dot").forEach(k=>k.addEventListener("click",()=>y(parseInt(k.dataset.i)))))}function h(z,k){var K,T;const F=e[z],re=document.getElementById("card-counter"),pe=document.getElementById("card-track");re&&(re.textContent=`Carte ${z+1} / ${e.length}`);const de=document.getElementById("reveal-btns");de&&(de.style.display=z===e.length-1?"flex":"none");const ae=F.card_type==="player"&&((K=F.player)==null?void 0:K.rarity)==="legende",Ee=!m.has(z);m.add(z);let U=0;if(F.card_type==="player"&&F.player){const S=F.player,G=S.job||"ATT";U=Number(G==="GK"?S.note_g:G==="DEF"?S.note_d:G==="MIL"?S.note_m:S.note_a)||0}const Y=S=>{pe.innerHTML=`
        ...
        </div>`;const G=document.getElementById("current-card-wrap");k!==0?(G.style.transition="none",G.style.transform=`translateX(${k>0?100:-100}%)`,requestAnimationFrame(()=>{G.style.transition="transform .28s cubic-bezier(.25,1,.5,1)",G.style.transform="translateX(0)"})):G.animate([{opacity:0,transform:"scale(.7)"},{opacity:1,transform:"scale(1)"}],{duration:300,easing:"cubic-bezier(.34,1.56,.64,1)"}),S||ae?ee():ie(),v()};Ee&&(((T=F.player)==null?void 0:T.rarity)==="legende"||U>=8)&&F.card_type==="player"&&F.player?$(F,()=>Y(!0)):Y(!1)}function $(z,k){var we;p=!0;const F=z.player,re=`https://flagsapi.com/${F.country_code}/flat/64.png`,pe=(we=F.clubs)==null?void 0:we.logo_url,de=F.face?"/"+F.face.replace(/^public\//,"").replace(/^\//,""):null,ae=F.job||"ATT",Ee=Number(ae==="GK"?F.note_g:ae==="DEF"?F.note_d:ae==="MIL"?F.note_m:F.note_a)||0,U=z.evolution_bonus||0,Y=Ee+U,K=F.rarity==="legende",T=Y>=18,S=document.getElementById("walkout-overlay"),G=document.getElementById("walkout-stage");if(!S||!G){p=!1,k();return}let N=null;K&&(N=new Audio("/sounds/Legendary.mp3"),N.volume=.8,N.play().catch(()=>{})),S.style.display="flex";const V=()=>{const me=G.firstElementChild;me&&(me.classList.remove("wo-in"),me.classList.add("wo-out"))},j=1800,H=400;G.innerHTML=`<img class="wo-in" src="${re}" style="height:130px;border-radius:10px;box-shadow:0 10px 36px rgba(0,0,0,.6)" onerror="this.style.display='none'">`,navigator.vibrate&&navigator.vibrate(30),setTimeout(V,j),setTimeout(()=>{var me;G.innerHTML=pe?`<img class="wo-in" src="${pe}" style="max-height:160px;max-width:210px;object-fit:contain">`:`<div class="wo-in" style="font-size:34px;font-weight:900;color:#fff;text-align:center">${((me=F.clubs)==null?void 0:me.encoded_name)||"CLUB"}</div>`,navigator.vibrate&&navigator.vibrate(30)},j+H),setTimeout(V,j*2+H),setTimeout(()=>{G.innerHTML=de?`<img class="wo-in" src="${de}" style="height:200px;border-radius:50%;box-shadow:0 0 40px rgba(255,255,255,0.3);object-fit:cover;object-position:top">`:'<div class="wo-in" style="font-size:80px">👤</div>',navigator.vibrate&&navigator.vibrate(30)},(j+H)*2),setTimeout(V,(j+H)*2+j);const _e=T?"#FFD700":{GK:"#c0c0c0",DEF:"#e03030",MIL:"#D4A017",ATT:"#3fbf5f"}[ae]||"#fff";setTimeout(()=>{G.innerHTML=`<div class="wo-in" style="
        ...
        ${Y}
      </div>`,T&&navigator.vibrate&&navigator.vibrate([50,30,100,30,200])},(j+H)*3),setTimeout(V,(j+H)*3+j),setTimeout(()=>{S.style.display="none",G.innerHTML="",p=!1,N&&!K&&N.pause(),navigator.vibrate&&navigator.vibrate([40,30,80]),k()},(j+H)*4)}function y(z){if(p||z<0||z>=e.length||z===f)return;const k=z>f?1:-1;f=z,h(z,k)}function C(){y(f+1)}function E(){y(f-1)}function P(){const z=document.getElementById("card-viewport");if(!z||z._swipeBound)return;z._swipeBound=!0;let k=0,F=0,re=0,pe=!1;const de=K=>K.touches?K.touches[0].clientX:K.clientX,ae=K=>K.touches?K.touches[0].clientY:K.clientY,Ee=K=>{pe=!0,k=de(K),F=ae(K),re=0},U=K=>{if(!pe)return;re=de(K)-k;const T=ae(K)-F;if(Math.abs(re)<Math.abs(T))return;const S=document.getElementById("current-card-wrap");S&&(S.style.transition="none",S.style.transform=`translateX(${re*.6}px) rotate(${re*.02}deg)`)},Y=()=>{if(!pe)return;pe=!1;const K=document.getElementById("current-card-wrap"),T=55;re<=-T&&f<e.length-1?C():re>=T&&f>0?E():K&&(K.style.transition="transform .2s ease",K.style.transform="translateX(0)")};z.addEventListener("pointerdown",Ee),z.addEventListener("pointermove",U),z.addEventListener("pointerup",Y),z.addEventListener("pointercancel",Y),z.addEventListener("touchstart",Ee,{passive:!0}),z.addEventListener("touchmove",U,{passive:!0}),z.addEventListener("touchend",Y),z.addEventListener("click",K=>{if(Math.abs(re)>8)return;const T=z.getBoundingClientRect();K.clientX-T.left>T.width/2?C():E()})}let O=null;function ee(){const z=document.getElementById("fireworks-canvas");if(!z)return;z.width=window.innerWidth,z.height=window.innerHeight;const k=z.getContext("2d"),F=[];function re(){const de=Math.random()*z.width,ae=Math.random()*z.height*.6,Ee=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],U=Ee[Math.floor(Math.random()*Ee.length)];for(let Y=0;Y<60;Y++){const K=Math.PI*2/60*Y,T=2+Math.random()*5;F.push({x:de,y:ae,vx:Math.cos(K)*T,vy:Math.sin(K)*T,alpha:1,color:U,size:2+Math.random()*3})}}re(),O=setInterval(re,600);function pe(){if(document.getElementById("fireworks-canvas")){k.clearRect(0,0,z.width,z.height);for(let de=F.length-1;de>=0;de--){const ae=F[de];if(ae.x+=ae.vx,ae.y+=ae.vy+.08,ae.vy*=.98,ae.alpha-=.018,ae.alpha<=0){F.splice(de,1);continue}k.globalAlpha=ae.alpha,k.fillStyle=ae.color,k.beginPath(),k.arc(ae.x,ae.y,ae.size,0,Math.PI*2),k.fill()}k.globalAlpha=1,(O!==null||F.length>0)&&requestAnimationFrame(pe)}}pe()}function ie(){O!==null&&(clearInterval(O),O=null);const z=document.getElementById("fireworks-canvas");z&&z.getContext("2d").clearRect(0,0,z.width,z.height)}if(n){const z=document.getElementById("reveal-btns");z&&(z.innerHTML='<button class="btn btn-primary" id="reveal-next" style="flex:1">Continuer →</button>'),(B=document.getElementById("reveal-next"))==null||B.addEventListener("click",()=>{ie(),a.remove(),n()})}else document.getElementById("reveal-collection").addEventListener("click",()=>{ie(),a.remove(),i("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{ie(),a.remove(),i("boosters")})}function ao(e){var t,i,n,a;if(e.card_type==="player"&&e.player)return to(e);if(e.card_type==="game_changer"){const o=e._gcDef,r=(o==null?void 0:o.gc_type)==="ultra_game_changer",s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},d={purple:"#b06ce0",light_blue:"#00d4ef"},l=s[o==null?void 0:o.color]||s.purple,c=d[o==null?void 0:o.color]||d.purple,u=(o==null?void 0:o.name)||e.gc_type||"Game Changer",g=(o==null?void 0:o.effect)||((t=ti[e.gc_type])==null?void 0:t.desc)||"",x=o!=null&&o.image_url?`/icons/${o.image_url}`:null,f=((i=ti[e.gc_type])==null?void 0:i.icon)||"⚡";return`<div style="width:170px;height:240px;background:${l};border-radius:14px;border:3px solid ${c};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${c}88;flex-shrink:0">
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div id="deck-select-screen" style="display:flex;flex-direction:${window.innerWidth>=900?"row":"column"};height:100%;overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div id="deck-swipe-zone" style="flex:1;min-height:0;overflow:visible;position:relative;touch-action:pan-y;display:flex;align-items:flex-start;justify-content:center;padding:0">
        ${b?`<div class="deck-preview-wrap" style="width:100%;height:100%;overflow:visible">
            ${At(b,m,null,[],300,300)}
      ...
      <div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">
        ${o.map((B,z)=>`<div style="width:7px;height:7px;border-radius:50%;background:${z===c?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}
      </div>`:""}
      ...
      <div style="width:240px;flex-shrink:0;background:rgba(0,0,0,0.3);display:flex;flex-direction:column;gap:16px;padding:24px 16px;border-left:1px solid rgba(255,255,255,0.08)">
        ...
        stroke="${C}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const x=Math.max(52,Math.round(a*.18)),f=Math.round(x*657/507);for(const[m,b]of Object.entries(l)){const v=u(m);if(!v||!b)continue;const h=m.replace(/[0-9]/g,""),$=r.includes(b.cardId),y=i==="attack"&&(["MIL","ATT"].includes(h)||$)&&!b.used||i==="defense"&&["GK","DEF","MIL"].includes(h)&&!b.used,C=n.includes(b.cardId);let E=b.boost||0;b.stadiumBonus&&(i==="attack"&&(h==="ATT"||h==="MIL")||i==="defense"&&(h==="GK"||h==="DEF"||h==="MIL")?E+=10:i||(E+=10));const P=Math.round(v.x-x/2),O=Math.round(v.y-f/2);if(b.used){g+=`<image href="${`${typeof import.meta<"u"&&"/"||"/"}icons/carte-dos.png`}" x="${P}" y="${O}" width="${x}" height="${f}" preserveAspectRatio="xMidYMid slice" class="match-used-hit" data-card-id="${b.cardId}" data-role="${h}" style="cursor:pointer"/>`;continue}const ee=Ae({...b,_evolution_bonus:0},{width:x,showStad:!0,stadDef:null,role:h,extraNote:E}),ie=C?"outline:3px solid #FFD700;outline-offset:2px;border-radius:8px;opacity:0.75;":"";g+=`<foreignObject x="${P-2}" y="${O-30}" width="${x+8}" height="${f+60}" style="overflow:visible">
      <div xmlns="http://www.w3.org/1999/xhtml" style="${ie}position:relative">
        ${ee}
      </div>
    </foreignObject>`,y&&(g+=`<rect x="${P}" y="${O}" width="${x}" height="${f}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${C?"selected":""}" data-card-id="${b.cardId}" data-role="${h}" style="cursor:pointer"/>`)}const p=Math.round(Math.max(x*.7,80));return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-p} ${-p} ${a+p*2} ${o+p*2}" width="100%" style="display:block;width:100%;margin:0 auto">
    ${g}
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    ${(()=>{const k=window.innerWidth>=700,F=(T,S,G)=>{var Ce,je;const N=(t.gcDefs||[]).find(Fe=>Fe.name===T.gc_type),V={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[N==null?void 0:N.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",j={purple:"#b06ce0",light_blue:"#00d4ef"}[N==null?void 0:N.color]||"#b06ce0",H=(N==null?void 0:N.name)||T.gc_type,$e=(N==null?void 0:N.effect)||((Ce=Se[T.gc_type])==null?void 0:Ce.desc)||"",_e=N!=null&&N.image_url?`/icons/${N.image_url}`:null,we=((je=Se[T.gc_type])==null?void 0:je.icon)||"⚡",me=Math.round(G*.22),Le=Math.round(G*.22),ke=G-me-Le,he=H.length>12?7:9;return`<div class="gc-mini" data-gc-id="${T.id}" data-gc-type="${T.gc_type}"
          style="box-sizing:border-box;width:${S}px;height:${G}px;border-radius:10px;border:2px solid ${j};background:${V};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
          <div style="height:${me}px;background:rgba(255,255,255,0.14);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 3px">
            <span style="font-size:${he}px;font-weight:900;color:#fff;text-align:center;line-height:1.1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${S-6}px">${H}</span>
        ...
        </div>`},re=(T,S)=>{var G;return`<div id="boost-card"
          style="box-sizing:border-box;width:${T}px;height:${S}px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:2px solid #87CEEB;border-radius:10px;cursor:pointer;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:${Math.round(S*.04)}px;text-align:center;flex-shrink:0">
            ...
            <div style="font-size:${Math.round(S*.09)}px;color:#000;font-weight:900">+${(G=t.boostCard)==null?void 0:G.value}</div>
          </div>`},pe=(T,S)=>S?re(130,175):F(T,130,175),de=(T,S)=>S?re(68,95):F(T,68,95),ae=k?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",Ee=f?`<button id="btn-results" style="${ae};background:linear-gradient(135deg,#D4A017,#FFD700);border:none;color:#000">🏁 Résultats</button>`:u?`<div style="${ae};background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);color:rgba(255,255,255,0.4)">⏳ Tour IA</div>`:b?`<button id="btn-pass" style="${ae};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER (plus d'attaquants)</button>`:g?`<button id="btn-action" style="${ae};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${t.selected.length===0?"disabled":""}> ⚔️ ATTAQUEZ <span id="match-timer" style="font-weight:900"></span></button>`:x?`<button id="btn-action" style="${ae};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${t.selected.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="match-timer" style="font-weight:900"></span></button>`:`<div style="${ae};background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1)"></div>`,U=g||x?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${t.selected.length}/3 sélectionné(s)</div>`:"",Y=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${k?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(k=>{var F,re,pe;if(k.type==="duel"){const de=k.isGoal,ae=k.homeScored?"#FFD700":de?"#ff6b6b":"rgba(255,255,255,0.3)",Ee=k.homeScored?"⚽ BUT !":de?"⚽ BUT IA !":(F=k.homePlayers)!=null&&F.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${de?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${ae};margin-bottom:4px">
                <div style="font-size:9px;color:${ae};letter-spacing:1px;margin-bottom:5px;font-weight:700;text-transform:uppercase">${Ee}</div>
    ...
    </div>`,(y=u.querySelector("#sub-close"))==null||y.addEventListener("click",()=>u.remove()),(C=u.querySelector("#out-up"))==null||C.addEventListener("click",()=>{s>0&&(s--,g())}),(E=u.querySelector("#out-down"))==null||E.addEventListener("click",()=>{s<o.length-1&&(s++,g())}),(P=u.querySelector("#in-up"))==null||P.addEventListener("click",()=>{l>0&&(l--,g())}),(O=u.querySelector("#in-down"))==null||O.addEventListener("click",()=>{l<r.length-1&&(l++,g())});const $=(ie,le,B,z)=>{const k=u.querySelector("#"+ie);if(!k)return;let F=0;k.addEventListener("touchstart",re=>{F=re.touches[0].clientY},{passive:!0}),k.addEventListener("touchend",re=>{const pe=re.changedTouches[0].clientY-F;if(Math.abs(pe)<30)return;const de=le();pe<0&&de<z-1?(B(de+1),g()):pe>0&&de>0&&(B(de-1),g())},{passive:!0})};$("in-panel",()=>l,ie=>l=ie,r.length),$("out-panel",()=>s,ie=>s=ie,o.length),(ee=u.querySelector("#sub-confirm"))==null||ee.addEventListener("click",ie=>{if(ie.preventDefault(),ie.stopPropagation(),c)return;c=!0;const le=o[s],B=r[l];if(!le||!B)return;let z=null,k=-1;for(const[re,pe]of Object.entries(t.homeTeam)){const de=(pe||[]).findIndex(ae=>ae.cardId===le.cardId);if(de!==-1){z=re,k=de;break}}if(k===-1||!z){wt("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),u.remove();return}const F={...B,_line:z,_col:le._col||0,used:!1,boost:0};t.homeTeam[z].splice(k,1,F),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(B.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:le.name,firstname:le.firstname,note:qe(le,z),portrait:zt(le),job:le.job,country_code:le.country_code,rarity:le.rarity,clubName:le.clubName,clubLogo:le.clubLogo},inPlayer:{name:B.name,firstname:B.firstname,note:qe(B,z),portrait:zt(B),job:B.job,country_code:B.country_code,rarity:B.rarity,clubName:B.clubName,clubLogo:B.clubLogo},text:`🔄 ${B.firstname} ${B.name} remplace ${le.firstname} ${le.name}`}),u.remove(),bo(le,B,()=>ze(e,t,i))})}document.body.appendChild(u),g()}function Ao(e,t,i,n,a){var f,p;const o=(n.gcDefs||[]).find(m=>m.name===t),r=Se[t]||{icon:"⚡",desc:"Carte spéciale."},s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[o==null?void 0:o.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",d={purple:"#b06ce0",light_blue:"#00d4ef"}[o==null?void 0:o.color]||"#b06ce0",l=(o==null?void 0:o.name)||t,c=(o==null?void 0:o.effect)||r.desc,u=o!=null&&o.image_url?`/icons/${o.image_url}`:null,g=r.icon||"⚡",x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",x.innerHTML=`
    ...
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      ...
      ${It(e.aiTeam,e.formation,null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.1))}
            ...
            <div style="background:rgba(255,255,255,0.08);border-radius:14px;padding:14px 20px;text-align:center;min-width:220px">
              ...
              ${Me.id!==Pe.id?`<div style="font-size:20px;font-weight:900;color:${Me.color}">
                    ${Te>Q.mmr?"📈":"📉"} ${Pe.emoji} ${Pe.label} → ${Me.emoji} ${Me.label}
                  </div>
                  <div style="font-size:13px;color:rgba(255,255,255,0.5);margin-top:4px">${Te>Q.mmr?"Promotion !":"Rétrogradation"}</div>`:`<div style="font-size:36px">${be>=0?"📈":"📉"}</div>
            ...
            </div>`}}catch(Q){console.error("[Ranked] MMR update error:",Q)}(J=document.getElementById("pvp-end-overlay"))==null||J.remove();const q=document.createElement("div");q.id="pvp-end-overlay",q.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;color:#fff;padding:24px;text-align:center;overflow-y:auto";const R=I?"🤝":M?"🏆":"😞",W=I?"MATCH NUL":M?"VICTOIRE !":"DÉFAITE",X=I?"#fff":M?"#FFD700":"#ff6b6b";q.innerHTML=`
      ...
      <div style="font-size:18px">${y[p+"Name"]} ${L} – ${A} ${y[m+"Name"]}</div>
      ${_.forfeit?`<div style="font-size:13px;color:rgba(255,255,255,0.5)">${M?"L'adversaire a quitté":"Perdu par forfait"}</div>`:""}
      ${D}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${r?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(q),(oe=q.querySelector("#pvp-end-home"))==null||oe.addEventListener("click",()=>{q.remove(),Ue(e),x(r?"ranked":"home")})}const le=w.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},_=>{const L=_.new;try{if(L.status==="finished"||L.forfeit){if(C)return;C=!0,E&&(clearInterval(E),E=null),L.game_state&&(y=L.game_state),ie(L);return}if(L.game_state){const A=y;y=L.game_state;const M=y._lastGC;if(M&&M.seq&&!ee.has(M.seq)&&(ee.add(M.seq),M.by!==p)){K(M.type,M.by,()=>ae());return}const I=A[p+"Score"]||0,D=A[m+"Score"]||0,q=y[p+"Score"]||0,R=y[m+"Score"]||0,W=q>I,X=R>D;if((W||X)&&!O.has(y.round)){O.add(y.round);const J=[...y.log||[]].reverse().find(Q=>Q.isGoal),oe=((J==null?void 0:J.homePlayers)||[]).map(Q=>({name:Q.name,note:Q.note,portrait:Q.portrait,job:Q.job}));st(oe,q,R,W,()=>ae());return}ae()}}catch(A){console.error("[PvP] crash:",A)}}).subscribe();async function B(_){Object.assign(y,_),y.lastActionAt=new Date().toISOString();const{error:L}=await w.from("matches").update({game_state:y}).eq("id",i);L&&f("Erreur de synchronisation","error");try{ae()}catch(A){console.error("[PvP] renderPvpScreen crash:",A)}}async function z(){if(C)return;C=!0,E&&(clearInterval(E),E=null);const _=n?h.away_id:h.home_id,L=n?"p2":"p1",A=n?"p1":"p2",M={...y,[L+"Score"]:3,[A+"Score"]:0,phase:"finished"};try{await w.from("matches").update({status:"finished",forfeit:!0,winner_id:_,home_score:M.p1Score||0,away_score:M.p2Score||0,game_state:M}).eq("id",i)}catch{}try{w.removeChannel(le)}catch{}setTimeout(()=>{Ue(e),x("home")},800)}const k={BOOST_STAT:({value:_=1,count:L=1,roles:A=[]},M,I)=>{const D=M[p+"Team"],q=Object.entries(D).filter(([R])=>!A.length||A.includes(R)).flatMap(([R,W])=>W.filter(X=>!X.used).map(X=>({...X,_line:R})));if(!q.length){M.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),I(M);return}F(q,L,`Choisir ${L} joueur(s) à booster (+${_})`,R=>{R.forEach(W=>{const X=(D[W._line]||[]).find(J=>J.cardId===W.cardId);X&&(X.boost=(X.boost||0)+_,M.log.push({text:`⚡ +${_} sur ${X.name}`,type:"info"}))}),M[p+"Team"]=D,I(M)})},DEBUFF_STAT:({value:_=1,count:L=1,roles:A=[],target:M="ai"},I,D)=>{const q=M==="home"?p:m,R=I[q+"Team"],W=M==="home"?"allié":"adverse",X=Object.entries(R).filter(([J])=>!A.length||A.includes(J)).flatMap(([J,oe])=>oe.map(Q=>({...Q,_line:J})));if(!X.length){I.log.push({text:`🎯 Aucun joueur ${W}`,type:"info"}),D(I);return}F(X,L,`Choisir ${L} joueur(s) ${W}(s) (-${_})`,J=>{J.forEach(oe=>{const Q=(R[oe._line]||[]).find(ce=>ce.cardId===oe.cardId);Q&&(Q.boost=(Q.boost||0)-_,I.log.push({text:`🎯 -${_} sur ${Q.name}`,type:"info"}))}),I[q+"Team"]=R,D(I)})},GRAY_PLAYER:({count:_=1,roles:L=[],target:A="ai"},M,I)=>{const D=A==="home"?p:m,q=M[D+"Team"],R=A==="home"?"allié":"adverse",W=Object.entries(q).filter(([X])=>!L.length||L.includes(X)).flatMap(([X,J])=>J.filter(oe=>!oe.used).map(oe=>({...oe,_line:X})));if(!W.length){M.log.push({text:`❌ Aucun joueur ${R}`,type:"info"}),I(M);return}F(W,_,`Choisir ${_} joueur(s) ${R}(s) à exclure`,X=>{const J="usedCardIds_"+D,oe=new Set(M[J]||[]);X.forEach(Q=>{const ce=(q[Q._line]||[]).find(se=>se.cardId===Q.cardId);ce&&(ce.used=!0,oe.add(Q.cardId),M.log.push({text:`❌ ${ce.name} exclu !`,type:"info"}))}),M[J]=[...oe],M[D+"Team"]=q,I(M)})},REVIVE_PLAYER:({count:_=1,roles:L=[]},A,M)=>{const I=A[p+"Team"],D=Object.entries(I).filter(([q])=>!L.length||L.includes(q)).flatMap(([q,R])=>R.filter(W=>W.used).map(W=>({...W,_line:q})));if(!D.length){A.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),M(A);return}F(D,_,`Choisir ${_} joueur(s) à ressusciter`,q=>{q.forEach(R=>{const W=(I[R._line]||[]).find(X=>X.cardId===R.cardId);W&&(W.used=!1,A.log.push({text:`💫 ${W.name} ressuscité !`,type:"info"}))}),A[p+"Team"]=I,M(A)})},REMOVE_GOAL:({},_,L)=>{const A=m+"Score";_[A]>0?(_[A]--,_.log.push({text:"🚫 Dernier but annulé !",type:"info"})):_.log.push({text:"🚫 Aucun but à annuler",type:"info"}),L(_)},ADD_GOAL_DRAW:({},_,L)=>{_[p+"Score"]===_[m+"Score"]?(_[p+"Score"]++,_.log.push({text:"🎯 But bonus !",type:"info"})):_.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),L(_)},ADD_SUB:({value:_=1},L,A)=>{L.maxSubs=(L.maxSubs||3)+_,L.log.push({text:`🔄 +${_} remplacement(s)`,type:"info"}),A(L)},CUSTOM:({},_,L)=>L(_)};function F(_,L,A,M){const I=document.createElement("div");I.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let D=[];function q(){var W,X;const R=_.map(J=>{const oe={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[J._line]||"#555",Q=qe(J,J._line)+(J.boost||0),se=D.find(ue=>ue.cardId===J.cardId)?"#FFD700":"rgba(255,255,255,0.25)",ne=J.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${J.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${se};background:${oe};overflow:hidden;cursor:pointer;${ne}">
        ...
        </div>`,(W=I.querySelector("#pp-close"))==null||W.addEventListener("click",()=>I.remove()),I.querySelectorAll(".pp-item").forEach(J=>{J.addEventListener("click",()=>{const oe=J.dataset.cid,Q=_.find(se=>se.cardId===oe),ce=D.findIndex(se=>se.cardId===oe);Q&&(ce>-1?D.splice(ce,1):D.length<L&&D.push(Q),q())})}),(X=I.querySelector("#pp-confirm"))==null||X.addEventListener("click",()=>{I.remove(),M(D)})}q(),document.body.appendChild(I)}async function re(_,L){const M=(y["gcCardsFull_"+p]||[]).find(R=>R.id===_),I=(M==null?void 0:M._gcDef)||(y.gcDefs||[]).find(R=>{var W;return R.name===L||((W=R.name)==null?void 0:W.toLowerCase().trim())===(L==null?void 0:L.toLowerCase().trim())}),D=[...y["usedGc_"+p]||[],_],q={type:L,by:p,seq:(y._gcAnimSeq||0)+1};ee.add(q.seq),K(L,p,async()=>{if(I!=null&&I.effect_type&&I.effect_type!=="CUSTOM"){const W=k[I.effect_type];if(W){const X={...y};W(I.effect_params||{},X,async J=>{J["usedGc_"+p]=D,J._lastGC=q,J._gcAnimSeq=q.seq,await B(J)});return}}const R={...y};switch(L){case"Remplacement+":R.maxSubs=(R.maxSubs||3)+1,R.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const W=m+"Score";R[W]>0&&(R[W]--,R.log.push({text:"🚫 But annulé",type:"info"}));break}}R["usedGc_"+p]=D,R._lastGC=q,R._gcAnimSeq=q.seq,await B(R)})}function pe(_,L){const A="usedCardIds_"+_,M=new Set(y[A]||[]);L.forEach(I=>M.add(I)),y[A]=[...M]}function de(){for(const _ of["p1","p2"]){const L=new Set(y["usedCardIds_"+_]||[]),A=y[_+"Team"];if(A)for(const M of["GK","DEF","MIL","ATT"])(A[M]||[]).forEach(I=>{I.used=L.has(I.cardId)})}}function ae(){var Ve,nt,yi,bi,hi,vi;if(y.phase==="reveal")return Ee();if(y.phase==="midfield")return Y();if(y.phase==="finished")return Fe();const _=y[p+"Team"],L=y[m+"Team"];de();const A=y[p+"Score"],M=y[m+"Score"],I=y[p+"Name"],D=y[m+"Name"],q=y.phase===p+"-attack",R=y.phase===p+"-defense",W=Array.isArray(y["selected_"+p])?y["selected_"+p]:[],X=W.map(te=>te.cardId),J=window.innerWidth>=700,oe=y[p+"Subs"]||[],Q=y["usedSubIds_"+p]||[],ce=oe.filter(te=>!Q.includes(te.cardId)),se=y["gcCardsFull_"+p]||[],ne=y["usedGc_"+p]||[],ue=se.filter(te=>!ne.includes(te.id)),fe=y.boostOwner===p&&!y.boostUsed,xe=!["GK","DEF","MIL","ATT"].some(te=>(L[te]||[]).some(ge=>!ge.used)),Z=[..._.MIL||[],..._.ATT||[]].filter(te=>!te.used),be=q&&xe&&Z.length===0?[..._.GK||[],..._.DEF||[]].filter(te=>!te.used).map(te=>te.cardId):[];function Te(te,ge,Ie){var $i,Ei;const ye=te._gcDef,ot={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ye==null?void 0:ye.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",lt={purple:"#b06ce0",light_blue:"#00d4ef"}[ye==null?void 0:ye.color]||"#b06ce0",ht=(ye==null?void 0:ye.name)||te.gc_type,vt=(ye==null?void 0:ye.effect)||(($i=Se[te.gc_type])==null?void 0:$i.desc)||"",wi=ye!=null&&ye.image_url?`/icons/${ye.image_url}`:null,un=((Ei=Se[te.gc_type])==null?void 0:Ei.icon)||"⚡",_i=Math.round(Ie*.22),ki=Math.round(Ie*.22),Vt=Ie-_i-ki,fn=ht.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${te.id}" data-gc-type="${te.gc_type}"
      ...
      </div>`}const Ne=(te,ge)=>ge?Me(130,175):Te(te,130,175),Ge=(te,ge)=>ge?Me(68,95):Te(te,68,95),Pe=J?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",bt=q?ke(p)?`<button id="pvp-action" style="${Pe};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${W.length===0?"disabled":""}>⚔️ ATTAQUEZ <span id="pvp-timer"></span></button>`:`<button id="pvp-action" data-pass="1" style="${Pe};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER <span id="pvp-timer"></span></button>`:R?`<button id="pvp-action" style="${Pe};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${W.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="pvp-timer"></span></button>`:`<div style="font-size:11px;color:rgba(255,255,255,0.3);text-align:center;padding:4px">⏳ Tour de ${D}</div>`,Be=q&&!ke(p)?'<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">Aucun attaquant — passez la main</div>':q||R?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${W.length}/3 sélectionné(s)</div>`:"",Oe=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${J?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
      ${ce.length===0?'<div style="font-size:7px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':ce.map(te=>`<div class="pvp-sub-btn" data-sub-id="${te.cardId}" style="cursor:pointer;flex-shrink:0">${Je(te,J?76:44,J?100:58)}</div>`).join("")}
    </div>`,Re=q?"attack":R?"defense":"idle",tt=`<div style="overflow:hidden;min-width:0;flex:1;min-height:0;display:flex;flex-direction:column" id="match-field">
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${At(_,y[p+"Formation"],Re,X,We(),rt(),be)}
      </div>
    </div>`,Ke=(()=>{var ge,Ie;if(R&&((ge=y.pendingAttack)!=null&&ge.players)){const ye=y.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
      ...
      </div>`;function De(){const te=e.querySelector(".match-screen");if(!te)return;const ge=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);te.style.height=ge+"px",te.style.minHeight=ge+"px",te.style.maxHeight=ge+"px",te.style.overflow="hidden";const Ie=e.querySelector("#mobile-action-bar"),ye=e.querySelector("#mobile-play-area");Ie&&ye&&(ye.style.paddingBottom=Ie.offsetHeight+"px")}if(De(),setTimeout(De,120),setTimeout(De,400),P||(P=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",De),window.visualViewport.addEventListener("scroll",De)),window.addEventListener("resize",De)),function(){const ge=e.querySelector(".terrain-wrapper svg");ge&&(ge.removeAttribute("width"),ge.removeAttribute("height"),ge.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",ge.setAttribute("viewBox","-26 -26 352 352"),ge.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),e.querySelectorAll(".match-slot-hit").forEach(te=>{te.addEventListener("click",()=>{if(!q&&!R)return;const ge=te.dataset.cardId,Ie=te.dataset.role,ye=(_[Ie]||[]).find(vt=>vt.cardId===ge);if(!ye||ye.used)return;const ot=be.includes(ge);if(q&&!["MIL","ATT"].includes(Ie)&&!ot)return;Array.isArray(y["selected_"+p])||(y["selected_"+p]=[]);const lt=y["selected_"+p],ht=lt.findIndex(vt=>vt.cardId===ge);ht>-1?lt.splice(ht,1):lt.length<3&&lt.push({...ye,_role:Ie}),ae()})}),e.querySelectorAll(".match-used-hit").forEach(te=>{te.addEventListener("click",()=>G(te.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(te=>{te.addEventListener("click",()=>G())}),(Ve=e.querySelector("#pvp-sub-open"))==null||Ve.addEventListener("click",()=>G()),e.querySelectorAll(".pvp-gc-mini").forEach(te=>{te.addEventListener("click",()=>T(te.dataset.gcId,te.dataset.gcType))}),(nt=e.querySelector("#pvp-boost-card"))==null||nt.addEventListener("click",()=>S()),(yi=e.querySelector("#pvp-action"))==null||yi.addEventListener("click",te=>{q?te.currentTarget.dataset.pass==="1"||!ke(p)?H():$e():R&&_e()}),(bi=e.querySelector("#pvp-quit"))==null||bi.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&z()}),(hi=e.querySelector("#pvp-view-opp"))==null||hi.addEventListener("click",()=>V()),(vi=e.querySelector("#pvp-toggle-history"))==null||vi.addEventListener("click",()=>j()),E&&(clearInterval(E),E=null),(q||R)&&!C){let te=30,ge=!1;const Ie=()=>document.getElementById("pvp-timer"),ye=()=>{Ie()&&(Ie().textContent=te+"s",Ie().style.color=ge?"#ff4444":"#fff")};ye(),E=setInterval(()=>{te--,te<0?ge?(clearInterval(E),E=null,q&&!ke(p)?H():z()):(ge=!0,te=15,ye()):ye()},1e3)}}function Ee(){He(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
      ...
      <div style="width:min(90vw,420px)">${It(y[m+"Team"],y[m+"Formation"],null,[],We(),rt())}</div>
    </div>`,p==="p1"&&setTimeout(async()=>{await B({phase:"midfield"})},5e3)}let U=!1;function Y(){if(U)return;const _=y[p+"Team"].MIL||[],L=y[m+"Team"].MIL||[],A=milScore(_)+milLinks(_),M=milScore(L)+milLinks(L),I=A>=M;function D(se,ne,ue){return`<div id="duel-row-${ue}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
        ...
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${se.map((fe,xe)=>{const Z=xe<se.length-1?yt(fe,se[xe+1]):null,be=!Z||Z==="#ff3333"||Z==="#cc2222",Te=Z==="#00ff88"?"+2":Z==="#FFD700"?"+1":"";return`<div class="duel-card duel-card-${ue}" data-idx="${xe}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease,transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
            ...
            ${xe<se.length-1?`<div class="duel-link duel-link-${ue}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${be?"rgba(255,255,255,0.12)":Z};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${be?"none":`0 0 8px ${Z}`}">
        ...
        <div class="duel-score-line duel-score-line-${ue}" style="margin-top:10px;font-size:12px;color:rgba(255,255,255,0.55);opacity:0;transition:opacity .4s ease">
          Score: ${milScore(se)} + ${milLinks(se)} liens = <b style="color:#fff">${milScore(se)+milLinks(se)}</b>
        </div>
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      ${D(L,y[m+"Name"]||"Adversaire","opp")}
      <div id="duel-shock" style="position:absolute;left:50%;top:50%;width:120px;height:120px;border-radius:50%;border:6px solid #FFD700;opacity:0;pointer-events:none"></div>
      <div id="pvp-duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center,rgba(10,61,30,.4),rgba(10,61,30,.92))"></div>
    </div>`;const q=(se,ne)=>e.querySelectorAll(se).forEach((ue,fe)=>{setTimeout(()=>{ue.style.opacity="1",ue.style.transform="translateY(0) scale(1)"},ne+fe*90)});q(".duel-card-me",150),q(".duel-card-opp",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((se,ne)=>setTimeout(()=>{se.style.opacity="1"},ne*70)),900),setTimeout(()=>{const se=e.querySelector("#pvp-vs");se&&(se.style.opacity="1",se.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(ne=>ne.style.opacity="1")},1250);function R(se,ne,ue){const fe=document.getElementById(se);if(!fe)return;const xe=performance.now(),Z=be=>{const Te=Math.min(1,(be-xe)/ue);fe.textContent=Math.round(ne*(1-Math.pow(1-Te,3))),Te<1?requestAnimationFrame(Z):fe.textContent=ne};requestAnimationFrame(Z)}setTimeout(()=>{R("pvp-score-me",A,800),R("pvp-score-opp",M,800)},1500);const W=y.p1Team.MIL||[],X=y.p2Team.MIL||[],J=milScore(W)+milLinks(W),oe=milScore(X)+milLinks(X),Q=J>=oe?"p1":"p2";let ce=y.boostValue;ce==null&&(ce=tn(),y.boostValue=ce,y.boostOwner=Q,y.boostUsed=!1),U=!0,setTimeout(()=>{const se=e.querySelector("#duel-row-"+(I?"me":"opp")),ne=e.querySelector("#duel-row-"+(I?"opp":"me")),ue=document.getElementById("pvp-score-me"),fe=document.getElementById("pvp-score-opp"),xe=I?ue:fe,Z=I?fe:ue;xe&&(xe.style.fontSize="80px",xe.style.color=I?"#FFD700":"#ff6b6b",xe.style.animation="duelPulse .5s ease"+(I?",duelGlow 1.5s ease infinite .5s":"")),Z&&(Z.style.opacity="0.25"),setTimeout(()=>{se&&(se.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",se.style.zIndex="5"),setTimeout(()=>{const be=document.getElementById("duel-shock");be&&(be.style.animation="shockwave .5s ease-out forwards"),ne&&(ne.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var Ne;const be=document.getElementById("pvp-duel-finale");if(!be)return;const Te=y.boostOwner===p?'<div style="background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:3px solid #cdeffd;border-radius:18px;padding:20px 34px;text-align:center;animation:boostFlipIn .7s cubic-bezier(.34,1.56,.64,1) both;box-shadow:0 10px 36px rgba(135,206,235,.5)"><div style="font-size:10px;color:rgba(0,0,0,.6);letter-spacing:2px;text-transform:uppercase;margin-bottom:6px;font-weight:700">Carte Boost obtenue</div><div style="font-size:46px;line-height:1">⚡</div><div style="font-size:50px;font-weight:900;color:#063;line-height:1.1">+'+ce+`</div><div style="font-size:10px;color:rgba(0,0,0,.55);margin-top:4px">Applicable sur n'importe quel joueur</div></div>`:"",Me=p==="p1"?'<button id="pvp-start-match" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">▶ Commencer le match</button>':`<div style="font-size:14px;color:rgba(255,255,255,0.5);text-align:center;margin-top:8px;animation:fadeUp .4s ease both">⏳ En attente de l'adversaire...</div>`;be.innerHTML='<div style="font-size:22px;font-weight:900;color:#fff;text-align:center;animation:fadeUp .4s ease both;text-shadow:0 2px 12px rgba(0,0,0,.5)">'+(I?"⚽ "+y[p+"Name"]+"<br>gagne le milieu et attaque !":"😔 "+y[m+"Name"]+"<br>gagne l'engagement et attaque !")+"</div>"+Te+Me,be.style.transition="opacity .45s ease",be.style.opacity="1",be.style.pointerEvents="auto",(Ne=document.getElementById("pvp-start-match"))==null||Ne.addEventListener("click",async()=>{const Ge=Q;await B({phase:Ge+"-attack",attacker:Ge,round:1,boostValue:ce,boostUsed:!1,boostOwner:Ge})})},600)},700)},2800)}function K(_,L,A){var ne,ue;const M=(y.gcDefs||[]).find(fe=>{var xe;return fe.name===_||((xe=fe.name)==null?void 0:xe.toLowerCase().trim())===(_==null?void 0:_.toLowerCase().trim())}),I={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[M==null?void 0:M.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",D={purple:"#b06ce0",light_blue:"#00d4ef"}[M==null?void 0:M.color]||"#b06ce0",q=(M==null?void 0:M.name)||_,R=(M==null?void 0:M.effect)||((ne=Se[_])==null?void 0:ne.desc)||"",W=M!=null&&M.image_url?`/icons/${M.image_url}`:null,X=((ue=Se[_])==null?void 0:ue.icon)||"⚡",oe=L===p?"Vous":y[L+"Name"]||"Adversaire",Q=document.createElement("div");Q.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",Q.innerHTML=`
      <style>
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(Q);let ce=!1;const se=()=>{ce||(ce=!0,Q.remove(),setTimeout(()=>A&&A(),50))};Q.addEventListener("click",se),setTimeout(se,3e3)}function T(_,L){var Q,ce,se,ne;const M=(y["gcCardsFull_"+p]||[]).find(ue=>ue.id===_),I=M==null?void 0:M._gcDef,D={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[I==null?void 0:I.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",q={purple:"#b06ce0",light_blue:"#00d4ef"}[I==null?void 0:I.color]||"#b06ce0",R=(I==null?void 0:I.name)||L,W=(I==null?void 0:I.effect)||((Q=Se[L])==null?void 0:Q.desc)||"Carte spéciale.",X=I!=null&&I.image_url?`/icons/${I.image_url}`:null,J=((ce=Se[L])==null?void 0:ce.icon)||"⚡",oe=document.createElement("div");oe.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",oe.innerHTML=`
      ...
      </div>`,document.body.appendChild(oe),(se=oe.querySelector("#pvp-gc-back"))==null||se.addEventListener("click",()=>oe.remove()),(ne=oe.querySelector("#pvp-gc-use"))==null||ne.addEventListener("click",()=>{oe.remove(),re(_,L)})}function S(){var M;const _=y[p+"Team"],L=Object.entries(_).flatMap(([I,D])=>(D||[]).filter(q=>!q.used).map(q=>({...q,_line:I})));if(!L.length)return;const A=document.createElement("div");A.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",A.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
        <div style="flex:1;font-size:14px;font-weight:700;color:#fff">⚡ Choisir un joueur pour +${y.boostValue}</div>
        <button id="bp-close" style="background:none;border:none;color:rgba(255,255,255,0.5);font-size:22px;cursor:pointer">✕</button>
      </div>
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${L.map(I=>{const D={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[I._line]||"#555",q=qe(I,I._line)+(I.boost||0);return`<div class="bp-item" data-cid="${I.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${D};overflow:hidden;cursor:pointer">
      ...
      </div>`,document.body.appendChild(A),(M=A.querySelector("#bp-close"))==null||M.addEventListener("click",()=>A.remove()),A.querySelectorAll(".bp-item").forEach(I=>{I.addEventListener("click",async()=>{const D=I.dataset.cid,q=L.find(W=>W.cardId===D);if(!q)return;const R=(_[q._line]||[]).find(W=>W.cardId===D);R&&(R.boost=(R.boost||0)+y.boostValue),A.remove(),await B({[p+"Team"]:_,boostUsed:!0})})})}function G(_=null){var ce,se;if(!(y.phase===p+"-attack")){f("Remplacement uniquement avant votre attaque","warning");return}const A=y[p+"Team"],M=y["usedSubIds_"+p]||[],I=y.maxSubs||3;if(M.length>=I){f(`Maximum ${I} remplacements atteint`,"warning");return}const D=Object.entries(A).flatMap(([ne,ue])=>(ue||[]).filter(fe=>fe.used).map(fe=>({...fe,_line:ne}))),q=(y[p+"Subs"]||[]).filter(ne=>!M.includes(ne.cardId));if(!D.length){f("Aucun joueur utilisé à remplacer","warning");return}if(!q.length){f("Aucun remplaçant disponible","warning");return}let R=Math.max(0,D.findIndex(ne=>ne.cardId===_));const W=((ce=D[R])==null?void 0:ce._line)||((se=D[R])==null?void 0:se.job);let X=Math.max(0,q.findIndex(ne=>ne.job===W)),J=!1;const oe=document.createElement("div");oe.id="pvp-sub-overlay",oe.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function Q(){var Te,Me,Ne,Ge,Pe,bt;const ne=D[R],ue=q[X],fe=Math.min(130,Math.round((window.innerWidth-90)/2)),xe=Math.round(fe*1.35),Z=Be=>`background:rgba(255,255,255,0.12);border:none;color:${Be?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Be?"default":"pointer"};flex-shrink:0`;oe.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        <div id="pin-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
          <div style="font-size:9px;color:#00ff88;letter-spacing:2px;text-transform:uppercase;font-weight:700">Joueur qui entre</div>
          <button id="pin-up" style="${Z(X===0)}" ${X===0?"disabled":""}>▲</button>
          <div>${ue?Je({...ue,used:!1,boost:0},fe,xe):"<div>—</div>"}</div>
          <button id="pin-down" style="${Z(X>=q.length-1)}" ${X>=q.length-1?"disabled":""}>▼</button>
          <div style="font-size:10px;color:rgba(255,255,255,0.35)">${X+1}/${q.length}</div>
        </div>
        <div id="pout-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
          <div style="font-size:9px;color:#ff6b6b;letter-spacing:2px;text-transform:uppercase;font-weight:700">Joueur qui sort</div>
          <button id="pout-up" style="${Z(R===0)}" ${R===0?"disabled":""}>▲</button>
          <div>${ne?Je({...ne,used:!1,boost:0},fe,xe):"<div>—</div>"}</div>
          <button id="pout-down" style="${Z(R>=D.length-1)}" ${R>=D.length-1?"disabled":""}>▼</button>
          <div style="font-size:10px;color:rgba(255,255,255,0.35)">${R+1}/${D.length}</div>
        </div>
      </div>
      <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
        <button id="psub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
      </div>`,(Te=oe.querySelector("#psub-close"))==null||Te.addEventListener("click",()=>oe.remove()),(Me=oe.querySelector("#pout-up"))==null||Me.addEventListener("click",()=>{R>0&&(R--,Q())}),(Ne=oe.querySelector("#pout-down"))==null||Ne.addEventListener("click",()=>{R<D.length-1&&(R++,Q())}),(Ge=oe.querySelector("#pin-up"))==null||Ge.addEventListener("click",()=>{X>0&&(X--,Q())}),(Pe=oe.querySelector("#pin-down"))==null||Pe.addEventListener("click",()=>{X<q.length-1&&(X++,Q())});const be=(Be,Oe,Re,tt)=>{const Ke=oe.querySelector("#"+Be);if(!Ke)return;let it=0;Ke.addEventListener("touchstart",De=>{it=De.touches[0].clientY},{passive:!0}),Ke.addEventListener("touchend",De=>{const Ve=De.changedTouches[0].clientY-it;if(Math.abs(Ve)<30)return;const nt=Oe();Ve<0&&nt<tt-1?(Re(nt+1),Q()):Ve>0&&nt>0&&(Re(nt-1),Q())},{passive:!0})};be("pin-panel",()=>X,Be=>X=Be,q.length),be("pout-panel",()=>R,Be=>R=Be,D.length),(bt=oe.querySelector("#psub-confirm"))==null||bt.addEventListener("click",async Be=>{if(Be.preventDefault(),Be.stopPropagation(),J)return;J=!0;const Oe=D[R],Re=q[X];if(!Oe||!Re)return;const tt=Oe._line,Ke=(A[tt]||[]).findIndex(Ve=>Ve.cardId===Oe.cardId);if(Ke===-1){f("Erreur : joueur introuvable","error"),oe.remove();return}const it={...Re,_line:tt,position:Oe.position,used:!1,boost:0};A[tt].splice(Ke,1,it);const De=[...M,Re.cardId];oe.remove(),N(Oe,Re,async()=>{await B({[p+"Team"]:A,[m+"Team"]:y[m+"Team"],["usedSubIds_"+p]:De})})})}document.body.appendChild(oe),Q()}function N(_,L,A){const M={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},I=document.createElement("div");I.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const D=(W,X,J)=>`<div style="text-align:center">
      <div style="font-size:9px;color:${X};letter-spacing:2px;text-transform:uppercase;font-weight:700;margin-bottom:6px">${J}</div>
      <div style="width:70px;height:70px;border-radius:50%;background:${M[W.job]||"#555"};border:3px solid ${X};position:relative;overflow:hidden;margin:0 auto">
        ${zt(W)?`<img src="${zt(W)}" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover">`:""}
      </div>
      ...
      </div>`,document.body.appendChild(L),(A=L.querySelector("#pvp-hist-close"))==null||A.addEventListener("click",()=>L.remove())}async function H(){if(y.phase!==p+"-attack")return;const _=p==="p1"?"p2":"p1",L=(y.round||0)+1,A=[...y.log||[]];A.push({type:"info",text:`⏭️ ${y[p+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const M=he(y),I=ke(_),D=M||!I?"finished":_+"-attack";await B({["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:D,attacker:_,round:L,log:A}),D==="finished"&&await je(y)}async function $e(){const _=y[p+"Team"],L=!["GK","DEF","MIL","ATT"].some(D=>(y[m+"Team"][D]||[]).some(q=>!q.used)),A=(y["selected_"+p]||[]).map(D=>{const q=(_[D._role]||[]).find(se=>se.cardId===D.cardId)||D,R=L&&["GK","DEF"].includes(D._role),W=_[D._role]||[],X=W.findIndex(se=>se.cardId===D.cardId),J=xt(W.length),oe=X>=0?J[X]:q._col??1,Q=y.stadiumDef||y.homeStadiumDef||null,ce=q.stadiumBonus||Q&&(Q.club_id&&String(q.club_id)===String(Q.club_id)||Q.country_code&&q.country_code===Q.country_code)||!1;return{...q,_line:D._role,_col:oe,stadiumBonus:ce,...R?{note_a:Math.max(1,Number(q.note_a)||0)}:{}}});if(!A.length)return;const M=pi(A,y.modifiers[p]||{});pe(p,A.map(D=>D.cardId)),A.forEach(D=>{const q=(_[D._role]||[]).find(R=>R.cardId===D.cardId);q&&(q.used=!0)}),y["selected_"+p]=[],ae();const I=[...y.log||[]];if(L){const D=(y[p+"Score"]||0)+1,q=A.map(oe=>histPlayer(oe));I.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:q,homeTotal:M.total,aiTotal:0});const R=(y.round||0)+1,W=p==="p1"?"p2":"p1",X={...y,[p+"Team"]:_,[p+"Score"]:D},J=he(X);O.add(R),st(q,D,y[m+"Score"]||0,!0,async()=>{await B({[p+"Team"]:_,[p+"Score"]:D,["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:J?"finished":W+"-attack",attacker:W,round:R,log:I}),J&&await je({...y,[p+"Score"]:D})});return}I.push({type:"pending",text:`⚔️ ${y[p+"Name"]} attaque (${M.total})`}),await B({[p+"Team"]:_,[m+"Team"]:y[m+"Team"],pendingAttack:{...M,players:A,side:p},["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},phase:m+"-defense",log:I})}async function _e(){const _=y[p+"Team"],L=(y["selected_"+p]||[]).map(ne=>{const ue=(_[ne._role]||[]).find(Te=>Te.cardId===ne.cardId)||ne,fe=_[ne._role]||[],xe=fe.findIndex(Te=>Te.cardId===ne.cardId),Z=xt(fe.length),be=xe>=0?Z[xe]:ue._col??1;return{...ue,_line:ne._role,_col:be}}),A=ui(L,y.modifiers[p]||{});pe(p,L.map(ne=>ne.cardId)),L.forEach(ne=>{const ue=(_[ne._role]||[]).find(fe=>fe.cardId===ne.cardId);ue&&(ue.used=!0)}),y["selected_"+p]=[],ae();const M=fi(y.pendingAttack.total,A.total,y.modifiers[p]||{}),I=y.pendingAttack.side,D=M==="attack"||(M==null?void 0:M.goal),q=I==="p1"?"p2":"p1",R=(y.round||0)+1,W=(y.pendingAttack.players||[]).map(ne=>histPlayer(ne)),X=[...y.log||[]];X.push({type:"duel",isGoal:D,homeScored:D&&I===p,text:D?`⚽ BUT de ${y[I+"Name"]} ! (${y.pendingAttack.total} vs ${A.total})`:`✋ Attaque stoppée (${y.pendingAttack.total} vs ${A.total})`,homePlayers:W,aiPlayers:L.map(ne=>histPlayer(ne)),homeTotal:y.pendingAttack.total,aiTotal:A.total});const J=D?(y[I+"Score"]||0)+1:y[I+"Score"]||0,oe={...y,[p+"Team"]:_,[I+"Score"]:J},Q=he(oe),ce=Q?"finished":q+"-attack",se=async()=>{await B({[p+"Team"]:_,[m+"Team"]:y[m+"Team"],[I+"Score"]:J,["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:ce,attacker:q,round:R,log:X}),(ce==="finished"||Q)&&await je({...y,[I+"Score"]:J})};if(D){const ne=I===p,ue=ne?J:y[p+"Score"]||0,fe=ne?y[m+"Score"]||0:J;O.add(R),st(W,ue,fe,ne,se)}else await se()}function we(_){return["MIL","ATT"].some(L=>(_[L]||[]).some(A=>!A.used))}function me(_){return["GK","DEF","MIL","ATT"].some(L=>(_[L]||[]).some(A=>!A.used))}function Le(_){return me(_)&&!we(_)}function ke(_){const L=y[_+"Team"],A=y[(_==="p1"?"p2":"p1")+"Team"];return!!(we(L)||!me(A)&&Le(L))}function he(_){const L=["MIL","ATT"].some(X=>(_.p1Team[X]||[]).some(J=>!J.used)),A=["MIL","ATT"].some(X=>(_.p2Team[X]||[]).some(J=>!J.used)),M=me(_.p1Team),I=me(_.p2Team);return!L&&!(!I&&M)&&(!A&&!(!M&&I))}function Ce(_){const L=_.p1Score||0,A=_.p2Score||0;return L===A?null:L>A?h.home_id:h.away_id}async function je(_){try{const L=Ce(_),A=L?h.home_id===L?h.away_id:h.home_id:null;await w.from("matches").update({status:"finished",winner_id:L,home_score:_.p1Score||0,away_score:_.p2Score||0}).eq("id",i),L&&A&&updateEvolutiveCards(L,A).catch(()=>{})}catch(L){console.error("[PvP] finishMatch:",L)}}function Fe(){var I;const _=y[p+"Score"],L=y[m+"Score"],A=_>L,M=_===L;He(e),e.innerHTML=`
    ...
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        ${u.map(B=>`
          <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid #f5f5f5">
            <div style="width:36px;height:36px;border-radius:50%;background:${B.club_color2||gt};display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:900;color:${B.club_color1||"#fff"}">${(B.pseudo||"?").slice(0,2).toUpperCase()}</div>
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>${g.map((B,z)=>{const k=f>0&&r.status==="finished"?z===0?Math.floor(f*.7):z===1?Math.floor(f*.2):z===2?Math.floor(f*.1):0:0;return`<tr style="border-bottom:1px solid #f5f5f5;${B.userId===o?"background:#f0fdf4;":""}">
              <td style="padding:7px 3px 7px 0;font-weight:700;color:${z===0?Xe:z<3?gt:"#555"}">${["🥇","🥈","🥉"][z]||z+1}</td>
              ...
              <td style="text-align:center;color:${B.goalDiff>=0?gt:"#cc2222"}">${B.goalDiff>=0?"+":""}${B.goalDiff}</td>
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        ${Array.from({length:Math.max(0,r.status==="active"?r.current_day-1:r.current_day)},(B,z)=>z+1).reverse().map(B=>{const z=(d||[]).filter(k=>k.matchday===B);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:#888;margin-bottom:6px">Journée ${B}</div>${z.map(k=>Di(k,u,o,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    </div>`}function g(p){var P,O,ee,ie;const m=(P=p.card)==null?void 0:P.player;if(!m)return"";const b=((O=p.card)==null?void 0:O.evolution_bonus)||0,v=ct(m,m.job,b),h=m.job2?ct(m,m.job2,b):0,$=p.status==="sold",y=m.country_code?`https://flagsapi.com/${m.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,C=Dt[m.job]||"#bbb",E=m.job2?Dt[m.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${$?"opacity:0.7":""}">
      ${y?`<img src="${y}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      ${(ee=m.clubs)!=null&&ee.logo_url?`<img src="${m.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${C};display:flex;align-items:center;justify-content:center">
