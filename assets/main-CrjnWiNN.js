...
import{s as _,l as Gt,m as Ii,F as Si,r as Se,j as Jt,h as Ai,n as An,o as Mn,T as gi,k as jn,p as Cn,b as Bn}from"./formation-links-yJiMLlPw.js";const qn="/";function Ni(e,{navigate:t,toast:i}){let n="login";const r=()=>{var a,s,l,c,d,f;const o=n==="login";e.innerHTML=`
    <div style="
    ...
    </style>`,(a=document.getElementById("tab-login-btn"))==null||a.addEventListener("click",()=>{n="login",r()}),(s=document.getElementById("tab-reg-btn"))==null||s.addEventListener("click",()=>{n="register",r()}),o?((l=document.getElementById("login-password"))==null||l.addEventListener("keydown",x=>{var g;x.key==="Enter"&&((g=document.getElementById("login-btn"))==null||g.click())}),(c=document.getElementById("login-btn"))==null||c.addEventListener("click",async()=>{const x=document.getElementById("login-email").value.trim(),g=document.getElementById("login-password").value,m=document.getElementById("login-error");if(m.textContent="",!x||!g){m.textContent="Remplissez tous les champs.";return}const p=document.getElementById("login-btn");p.textContent="⏳ Connexion…",p.disabled=!0;const{error:u}=await _.auth.signInWithPassword({email:x,password:g});if(p.textContent="⚽ Se connecter",p.disabled=!1,u){m.textContent=u.message.includes("Invalid")?"Email ou mot de passe incorrect.":u.message;return}window.location.reload()})):((d=document.getElementById("reg-confirm"))==null||d.addEventListener("keydown",x=>{var g;x.key==="Enter"&&((g=document.getElementById("reg-btn"))==null||g.click())}),(f=document.getElementById("reg-btn"))==null||f.addEventListener("click",async()=>{const x=document.getElementById("reg-email").value.trim(),g=document.getElementById("reg-password").value,m=document.getElementById("reg-confirm").value,p=document.getElementById("reg-error");if(p.textContent="",!x||!g||!m){p.textContent="Remplissez tous les champs.";return}if(g.length<6){p.textContent="Mot de passe trop court (min. 6 caractères).";return}if(g!==m){p.textContent="Les mots de passe ne correspondent pas.";return}const u=document.getElementById("reg-btn");u.textContent="⏳ Création…",u.disabled=!0;const{error:b}=await _.auth.signUp({email:x,password:g});if(u.textContent="🚀 Créer mon compte",u.disabled=!1,b){p.textContent=b.message;return}i("Compte créé ! Connecte-toi pour commencer.","success",4e3),n="login",r(),setTimeout(()=>{const v=document.getElementById("login-email");v&&(v.value=x)},50)}))};r()}function Dn(e,{state:t,navigate:i,toast:n,refreshProfile:r}){let o="#1A6B3C",a="#D4A017";e.innerHTML=`
  ...
  `;function s(){var m;const c=document.getElementById("logo-preview"),d=document.getElementById("logo-initials"),f=((m=document.getElementById("setup-club"))==null?void 0:m.value)||"MW",x=f.trim().split(" ").filter(Boolean),g=x.length>=2?(x[0][0]+x[1][0]).toUpperCase():f.slice(0,2).toUpperCase();c&&(c.style.background=a,c.style.borderColor=o),d&&(d.textContent=g,d.style.color=o)}document.getElementById("color1").addEventListener("input",c=>{o=c.target.value,document.getElementById("swatch1").style.background=o,s()}),document.getElementById("color2").addEventListener("input",c=>{a=c.target.value,document.getElementById("swatch2").style.background=a,s()});function l(c){document.querySelectorAll(".setup-step").forEach(d=>d.classList.remove("active")),document.getElementById(`step-${c}`).classList.add("active"),document.getElementById("step-num").textContent=c,document.getElementById("progress-fill").style.width=`${Math.round(c/3*100)}%`,c===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),d=document.getElementById("step1-error");if(d.textContent="",c.length<3){d.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:f}=await _.from("users").select("id").eq("pseudo",c).maybeSingle();if(f){d.textContent="Ce pseudo est déjà pris.";return}l(2)}),document.getElementById("step2-back").addEventListener("click",()=>l(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const c=document.getElementById("setup-club").value.trim(),d=document.getElementById("step2-error");if(d.textContent="",c.length<2){d.textContent="Nom trop court (min. 2 caractères).";return}const{data:f}=await _.from("users").select("id").eq("club_name",c).maybeSingle();if(f){d.textContent="Ce nom de club est déjà pris.";return}l(3)}),document.getElementById("step3-back").addEventListener("click",()=>l(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),d=document.getElementById("setup-club").value.trim(),f=document.getElementById("step3-error"),x=document.getElementById("step3-finish");f.textContent="",x.disabled=!0,x.textContent="Création en cours…";try{const{error:g}=await _.from("users").insert({id:t.user.id,pseudo:c,club_name:d,club_color1:o,club_color2:a,credits:1e4});if(g)throw g;await Fn(t.user.id),await r(),n(`Bienvenue ${c} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(g){f.textContent=g.message,x.disabled=!1,x.textContent="🚀 Créer mon profil !"}})}async function Fn(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await _.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const Pn="modulepreload",Gn=function(e){return"/"+e},Oi={},rn=function(t,i,n){let r=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),s=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));r=Promise.allSettled(i.map(l=>{if(l=Gn(l),l in Oi)return;Oi[l]=!0;const c=l.endsWith(".css"),d=c?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${l}"]${d}`))return;const f=document.createElement("link");if(f.rel=c?"stylesheet":Pn,c||(f.as="script"),f.crossOrigin="",f.href=l,s&&f.setAttribute("nonce",s),document.head.appendChild(f),c)return new Promise((x,g)=>{f.addEventListener("load",x),f.addEventListener("error",()=>g(new Error(`Unable to preload CSS for ${l}`)))})}))}function o(a){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=a,window.dispatchEvent(s),!s.defaultPrevented)throw a}return r.then(a=>{for(const s of a||[])s.status==="rejected"&&o(s.reason);return t().catch(o)})},Ut="#1A6B3C",Kt="#cc2222",Rn="#D4A017",Hi="#888";async function Nn(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      ${x.map(({friendshipId:g,friend:m})=>On(m,g)).join("")}
    </div>`,c.querySelectorAll("[data-stats]").forEach(g=>{g.addEventListener("click",()=>Un(e,g.dataset.stats,g.dataset.friendName))}),c.querySelectorAll("[data-match]").forEach(g=>{g.addEventListener("click",async()=>{const m=g.dataset.friendId,p=g.dataset.friendName;if(typeof n!="function"){t("Erreur navigation","error");return}const u=e.user.id,{data:b}=await _.from("friend_match_invites").select("id").eq("inviter_id",m).eq("invitee_id",u).eq("status","pending").order("created_at",{ascending:!1}).limit(1).maybeSingle(),v=!!b;console.log("[Friends] clic match",{fid:m,fname:p,isAccepting:v}),n("match",{matchMode:"friend",friendId:m,friendName:p,isAccepting:v})})})}function On(e,t){const i=e.club_name||e.pseudo||"?",n=e.pseudo||"",r=(n||i).slice(0,2).toUpperCase(),o=e.club_color2||Ut,a=e.club_color1||"#ffffff",s=e.last_seen_at?new Date(e.last_seen_at):null,l=s&&Date.now()-s.getTime()<3*60*1e3;return`
    ...
    <div class="popup-box">
      ...
      ${d.length?`<div style="display:flex;flex-direction:column;gap:8px;max-height:50vh;overflow-y:auto;margin-bottom:14px">
            ${d.map(x=>{var g,m,p;return`
              <div style="display:flex;align-items:center;gap:10px;background:#f9f9f9;border-radius:10px;padding:10px 12px">
                <div style="flex:1;font-size:14px;font-weight:700">${((g=x.requester)==null?void 0:g.club_name)||((m=x.requester)==null?void 0:m.pseudo)||"?"}
    ...
    ${ji()}`,document.body.appendChild(c);const f=()=>c.remove();c.querySelector("#pending-close").addEventListener("click",f),c.addEventListener("click",x=>{x.target===c&&f()}),c.querySelectorAll("[data-accept]").forEach(x=>{x.addEventListener("click",async()=>{const{error:g}=await _.from("friendships").update({status:"accepted"}).eq("id",x.dataset.accept);if(g){t("Erreur : "+g.message,"error");return}x.closest("div[style]").remove(),t("✅ Ami accepté !","success"),an(e,t,n),i&&i()})}),c.querySelectorAll("[data-decline]").forEach(x=>{x.addEventListener("click",async()=>{await _.from("friendships").delete().eq("id",x.dataset.decline),x.closest("div[style]").remove(),t("Demande refusée","info"),i&&i()})})}async function Un(e,t,i){const n=e.user.id,[r,o]=[n,t].sort(),a=n===r,{data:s}=await _.from("friend_match_stats").select("*").eq("player1_id",r).eq("player2_id",o).single(),l=e.profile.club_name||e.profile.pseudo||"Moi",c=s||{},d=a?c.wins_p1||0:c.wins_p2||0,f=a?c.wins_p2||0:c.wins_p1||0,x=c.draws||0,g=a?c.goals_p1||0:c.goals_p2||0,m=a?c.goals_p2||0:c.goals_p1||0,p=a?c.gc_used_p1||0:c.gc_used_p2||0,u=a?c.gc_used_p2||0:c.gc_used_p1||0,b=c.matches_total||0,v=(k,y,C,E=Ut,G=Kt)=>`
    ...
    </style>`}const Kn="2026.07.19-1631";async function Ui(e,{state:t,navigate:i,toast:n}){var o,a;const r=t.profile;r&&(e.innerHTML=`
  <style>
    .home-dark {
      min-height: 100%;
  ...
  <div class="home-dark" id="home-dark">
    ...
    ${Se(e,{width:140})}
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),c=(s||[]).filter(S=>S.card_type==="player"&&S.player),d=(s||[]).filter(S=>S.card_type==="game_changer"),f=(s||[]).filter(S=>S.card_type==="formation"),x=(s||[]).filter(S=>S.card_type==="stadium"),{data:g}=await _.from("gc_definitions").select("name,gc_type,color,effect,image_url"),m={};(g||[]).forEach(S=>{m[S.name]=S});const{data:p}=await _.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),u={};(p||[]).forEach(S=>{u[S.id]=S}),x.forEach(S=>{S.stadium_def&&(u[S.stadium_id]=S.stadium_def)});const b=Object.keys(Si),v=Object.keys(Me),h={};c.forEach(S=>{const q=S.player.id;h[q]=(h[q]||0)+1}),new Set(Object.keys(h).map(S=>String(S)));const k=new Set(f.map(S=>S.formation)),y=new Set(d.map(S=>S.gc_type));let C="player",E="Tous",G="",N=!1;function ne(){return[...c].sort((S,q)=>{const P=Zt.indexOf(S.player.job),X=Zt.indexOf(q.player.job);return P!==X?P-X:(S.player.surname_real||"").localeCompare(q.player.surname_real||"")})}function ie(){return[...l||[]].sort((S,q)=>{const P=Zt.indexOf(S.job),X=Zt.indexOf(q.job);return P!==X?P-X:(S.surname_real||"").localeCompare(q.surname_real||"")})}function Q(){return ne().filter(S=>{const q=S.player,P=E==="Tous"||q.job===E,X=!G||`${q.firstname} ${q.surname_real}`.toLowerCase().includes(G);return P&&X})}function M(){return ie().filter(S=>{const q=E==="Tous"||S.job===E,P=!G||`${S.firstname} ${S.surname_real}`.toLowerCase().includes(G);return q&&P})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
        ...
        </div>`},null)}e.querySelectorAll(".col-tab-btn").forEach(S=>{S.addEventListener("click",()=>{C=S.dataset.tab,E="Tous",G="",N=!1,e.querySelectorAll(".col-tab-btn").forEach(q=>{const P=q.dataset.tab===C;q.style.borderBottomColor=P?"var(--green)":"transparent",q.style.color=P?"var(--green)":"var(--gray-600)"}),w(),j()})}),w(),j()}function co(e,t,i){const n=Me[e]||{icon:"⚡",desc:"Effet spécial."},r=(t==null?void 0:t.gc_type)==="ultra_game_changer",o={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},a={purple:"#b06ce0",light_blue:"#00d4ef"},s=o[t==null?void 0:t.color]||o.purple,l=a[t==null?void 0:t.color]||a.purple,c=(t==null?void 0:t.name)||e,d=(t==null?void 0:t.effect)||n.desc,f=t!=null&&t.image_url?`/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const ei=1e3;function po(e,t,i,n){var m,p,u;const{state:r,toast:o,closeModal:a,navigate:s,refreshProfile:l}=i,c=e.formation,d={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function f(){const b=Jt[c]||{},v=Si[c]||[],h=290,k=360,y=20;function C(G){const N=b[G];return N?{x:N.x*h,y:N.y*k}:null}let E=`<svg width="${h}" height="${k}" viewBox="0 0 ${h} ${k}" xmlns="http://www.w3.org/2000/svg">`;for(const[G,N]of v){const ne=C(G),ie=C(N);!ne||!ie||(E+=`<line x1="${ne.x}" y1="${ne.y}" x2="${ie.x}" y2="${ie.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const G of Object.keys(b)){const N=C(G);if(!N)continue;const ne=G.replace(/\d+/,""),ie=d[ne]||"#555";E+=`<circle cx="${N.x}" cy="${N.y}" r="${y}" fill="${ie}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,E+=`<text x="${N.x}" y="${N.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${ne}</text>`}return E+="</svg>",E}const x=t.filter(b=>b.formation===c);x.length;const g=!e.is_for_sale;n(`Formation ${c}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${f.map((W,J)=>{const R=Q[W.id]||[],S=W.is_for_sale,q=R.length?R[R.length-1]:null,X=(d.rarity==="pepite"||d.rarity==="papyte")&&W.current_note!=null?` (☆${W.current_note})`:"";return`
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",d.note_g],["DEF",d.note_d],["MIL",d.note_m],["ATT",d.note_a]].map(([W,J])=>{const R=mi[W],S=W==="GK"?"#fff":R,q=W===d.job||W===d.job2,P=(Number(J)||0)+(q&&C>0?C:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(ae=document.getElementById("close-detail"))==null||ae.addEventListener("click",s);let j=new Set;const U=()=>{const W=j.size,J=document.getElementById("sell-action-panel");if(!J)return;J.style.display=W>0?"block":"none",document.getElementById("sell-selected-count").textContent=W;const R=document.getElementById("evolve-btn");R&&(R.textContent=`⬆️ Évoluer ${W>1?"(+"+W+")":""}`)};document.querySelectorAll(".expl-check").forEach(W=>{W.addEventListener("change",()=>{const J=W.dataset.id;W.checked?j.add(J):j.delete(J);const R=W.closest(".exemplaire-row");R&&(R.style.borderColor=W.checked?"#1A6B3C":"#eee"),U()})}),document.querySelectorAll(".exemplaire-row").forEach(W=>{W.addEventListener("click",J=>{if(J.target.closest("button")||J.target.tagName==="INPUT")return;const R=W.querySelector(".expl-check");R&&!R.disabled&&(R.checked=!R.checked,R.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(W=>{W.addEventListener("click",J=>{J.stopPropagation();const R=document.querySelector(`.expl-hist[data-hist="${W.dataset.idx}"]`);R&&(R.style.display=R.style.display==="none"?"flex":"none")})}),(se=document.getElementById("evolve-btn"))==null||se.addEventListener("click",async()=>{if(x<=1)return;const W=[...j];if(!W.length)return;if(j.has(e.id)){const V=document.createElement("div");V.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",V.innerHTML=`
  ...
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
    ...
    <div class="page-body">
      ...
      stadium_def:stadium_definitions(id, name, club_id, country_code, image_url,
        club:clubs(encoded_name, logo_url))`).eq("owner_id",n.profile.id).eq("card_type","stadium"),l=(a||[]).filter(b=>b.card_type==="player"&&b.player),c=(a||[]).filter(b=>b.card_type==="formation"),d=(s||[]).filter(b=>b.card_type==="stadium"),f=[...new Set(d.map(b=>b.stadium_id).filter(Boolean))];let x={};if(d.forEach(b=>{b.stadium_def&&b.stadium_id&&(x[b.stadium_id]=b.stadium_def)}),f.length&&Object.keys(x).length<f.length){const{data:b}=await _.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",f);(b||[]).forEach(v=>{x[v.id]=v})}const g=c.map(b=>b.formation).filter(Boolean),{data:m}=await _.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",e);let p=o.formation||"4-4-2";g.length>0&&!g.includes(p)&&(p=g[0]);const u={deckId:e,name:o.name,formation:p,formationCardId:o.formation_card_id,stadiumCardId:o.stadium_card_id||null,slots:{},subs:[],playerCards:l,formationCards:c,stadiumCards:d,stadDefMap:x,availableFormations:g};(m||[]).forEach(b=>{b.is_starter?u.slots[b.position]=b.card_id:u.subs.includes(b.card_id)||u.subs.push(b.card_id)}),xt(t,u,i)}function xt(e,t,i){var m,p;const{navigate:n}=i;ni[t.formation];const r=Zi(t.formation),o=r.filter(u=>t.slots[u]).length,a=t.availableFormations.length>0?t.availableFormations:Object.keys(ni),s=(m=t.stadiumCards)==null?void 0:m.find(u=>u.id===t.stadiumCardId),l=s&&((p=t.stadDefMap)==null?void 0:p[s.stadium_id])||null,c=t.subs.map(u=>t.playerCards.find(b=>b.id===u)).filter(Boolean);[...Object.values(t.slots),...t.subs],e.innerHTML=`
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
  ...
  </div>`;const d=window.innerWidth>=900,f=e.querySelector(".deck-pc-layout"),x=e.querySelector(".deck-mobile-layout");f&&(f.style.display=d?"block":"none"),x&&(x.style.display=d?"none":"block"),fo(e,t,r,i),e.querySelectorAll("#builder-back").forEach(u=>u.addEventListener("click",()=>n("decks")));const g=()=>{const{openModal:u,closeModal:b}=i,v=`<div style="display:flex;flex-wrap:wrap;gap:8px;padding:8px">
    ...
    </div>`;u("⚽ Choisir une formation",v),document.querySelectorAll("#modal-body [data-forma]").forEach(h=>{h.addEventListener("click",()=>{t.formation=h.dataset.forma;const k=Zi(t.formation),y={};k.forEach(C=>{t.slots[C]&&(y[C]=t.slots[C])}),t.slots=y,b(),xt(e,t,i)})})};e.querySelectorAll("#formation-mobile-btn, #formation-pc-btn").forEach(u=>u.addEventListener("click",g)),e.querySelectorAll("#add-stad-btn-pc, #add-stad-btn").forEach(u=>u.addEventListener("click",()=>go(t,e,i))),e.querySelectorAll("#save-deck").forEach(u=>u.addEventListener("click",()=>yo(t,i))),e.querySelectorAll("[data-remove-sub]").forEach(u=>{u.addEventListener("click",()=>{t.subs=t.subs.filter(b=>b!==u.dataset.removeSub),xt(e,t,i)})}),e.querySelectorAll("#add-sub-btn").forEach(u=>u.addEventListener("click",()=>xo(t,e,i)))}function fo(e,t,i,n){var h,k;const r=window.innerWidth>=900,o=e.querySelector(r?"#deck-field-pc":"#deck-field-mobile");if(!o)return;const a=(h=t.stadiumCards)==null?void 0:h.find(y=>y.id===t.stadiumCardId),s=a&&((k=t.stadDefMap)==null?void 0:k[a.stadium_id])||null,l=Jt[t.formation]||{},c=Ii(t.formation)||[],d={};for(const y of i){const C=t.slots[y],E=C?t.playerCards.find(G=>G.id===C):null;E!=null&&E.player?d[y]={...E.player,_evolution_bonus:E.evolution_bonus||0,face:E.player.face||null}:d[y]=null}const f=window.innerWidth>=900,x=f?window.innerWidth-280:window.innerWidth-20,g=f?Math.min(x,860):x,m=Math.round(f?g*.82:g*.85),p=f?84:44;let u="";for(const[y,C]of c){const E=l[y],G=l[C];if(!E||!G)continue;const N=E.x*g,ne=Math.round(.03*m+E.y*.85*m),ie=G.x*g,Q=Math.round(.03*m+G.y*.85*m),M=d[y],w=d[C],j=Gt(M,w);j==="#ff3333"||j==="#cc2222"?u+=`<line x1="${N.toFixed(1)}" y1="${ne.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${Q.toFixed(1)}" stroke="${j}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(u+=`<line x1="${N.toFixed(1)}" y1="${ne.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${Q.toFixed(1)}" stroke="${j}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,u+=`<line x1="${N.toFixed(1)}" y1="${ne.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${Q.toFixed(1)}" stroke="${j}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let b="";const v=Math.round(p*657/507);for(const y of i){const C=l[y];if(!C)continue;const E=d[y],G=C.x*g;C.y*m;const N=Math.round(.03*m+C.y*(.85*m)),ne=Math.round(G-p/2),ie=Math.round(N-v/2);if(E){const Q=y.replace(/\d+/,""),M=s&&(s.club_id&&String(E.club_id)===String(s.club_id)||s.country_code&&E.country_code===s.country_code),w=Se({...E,_evolution_bonus:E._evolution_bonus||0},{width:p,showStad:!0,stadDef:s,role:Q});b+=`<div style="position:absolute;left:${ne}px;top:${ie}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${y}">
    ...
    <div style="position:relative;width:${g}px;height:${m}px;margin:0 auto">
      ...
      ${b}
    </div>`,o.querySelectorAll(".deck-slot-hit").forEach(y=>{y.addEventListener("click",()=>mo(y.dataset.pos,t,e,n))})}function go(e,t,i){var a;const{openModal:n,closeModal:r}=i,o=e.stadiumCards||[];n("🏟️ Choisir un stade",`<div style="display:flex;flex-wrap:wrap;gap:10px;padding:8px">
      ...
      ${o.map(s=>{var b,v;const l=e.stadDefMap[s.stadium_id],c=((b=l==null?void 0:l.club)==null?void 0:b.logo_url)||null,d=l!=null&&l.image_url?"/icons/"+l.image_url:null,f=e.stadiumCardId===s.id,x=l!=null&&l.country_code&&!c?`https://flagsapi.com/${l.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,g=(l==null?void 0:l.name)||"Stade",m=g.match(/^(Stade\s+(?:de\s+)?(?:\w+)?)(.*)?$/i),p=m?m[1].trim():g.slice(0,8),u=((v=m==null?void 0:m[2])==null?void 0:v.trim())||"";return`<div class="stad-choice" data-stad-id="${s.id}" style="cursor:pointer;width:80px;text-align:center">
    ...
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(b=document.getElementById("close-selector"))==null||b.addEventListener("click",o),(v=document.getElementById("remove-player"))==null||v.addEventListener("click",()=>{delete t.slots[e],o(),xt(i,t,n)}),document.querySelectorAll(".player-option").forEach(h=>{h.addEventListener("click",()=>{t.slots[e]=h.dataset.cardId,o(),xt(i,t,n)})})}function xo(e,t,i){var d,f,x;const{openModal:n,closeModal:r}=i,o=(d=e.stadiumCards)==null?void 0:d.find(g=>g.id===e.stadiumCardId),a=o&&((f=e.stadDefMap)==null?void 0:f[o.stadium_id])||null,s=new Set;Object.keys(e.slots).forEach(g=>{var u;const m=e.slots[g];if(!m)return;const p=e.playerCards.find(b=>b.id===m);(u=p==null?void 0:p.player)!=null&&u.id&&s.add(p.player.id)}),e.subs.forEach(g=>{var p;const m=e.playerCards.find(u=>u.id===g);(p=m==null?void 0:m.player)!=null&&p.id&&s.add(m.player.id)});const l=new Set,c=e.playerCards.filter(g=>{var m,p,u;return s.has((m=g.player)==null?void 0:m.id)||l.has((p=g.player)==null?void 0:p.id)?!1:(l.add((u=g.player)==null?void 0:u.id),!0)});n("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}function Io(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText=`
    ...
    `,document.body.appendChild(t);let i=5;const n=setInterval(()=>{i--;const r=document.getElementById("mw-ad-cd");r&&(r.textContent=i),i<=0&&(clearInterval(n),t.remove(),e(!0))},1e3)})}async function So(e,{state:t,navigate:i,toast:n,refreshProfile:r}){var x,g;const{data:o}=await _.from("users").select("*").eq("id",t.user.id).single();o&&(t.profile=o);let a=Array.isArray((x=t.profile)==null?void 0:x.pending_boosters)?[...t.profile.pending_boosters]:[];if(!a.length){await _.from("users").update({onboarding_done:!0}).eq("id",t.user.id),i("home");return}let s=null;try{const p=(await gn()).find(u=>(u.name||"").toLowerCase().includes("new player"));p&&(s=xn(p))}catch(m){console.warn('[Onboarding] Config "Booster (new player)" introuvable, fallback taux par défaut',m)}const l=a.length;let c=0;e.innerHTML=`
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    </div>`}const l=d=>{e.style.overflow="",e.style.height="",e.style.display="",e.style.flexDirection="",i(d)};function c(){var f,x,g;e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column";const d=a.length>0;e.innerHTML=`
    ...
    </div>`,e.querySelectorAll(".gc-select-card").forEach(m=>{m.addEventListener("click",()=>{const p=m.dataset.id,u=o.find(v=>v.id===p);if(!u)return;const b=a.findIndex(v=>v.gc_type===u.gc_type);b>-1?a.splice(b,1):a.length<3&&a.push(u),c()})}),(f=e.querySelector("#gc-launch"))==null||f.addEventListener("click",()=>{d&&l(a)}),(x=e.querySelector("#gc-no-gc"))==null||x.addEventListener("click",()=>l([])),(g=e.querySelector("#gc-reset"))==null||g.addEventListener("click",()=>{a.length&&(a=[],c())})}c()}function jo(e,t){var r;const i=((r=t==null?void 0:t.state)==null?void 0:r.params)||{},n=e||i.matchMode||"vs_ai_easy";return n==="friend"?`Match vs ${i.friendName||"Ami"}`:n==="random"?"Match vs Random":n==="ranked"?"Match Classé":n==="mini_league"||n==="mini-league"?"🏆 Match de Mini League":`Match vs IA — ${n.replace("vs_ai_","").toUpperCase()}`}async function Co(e,t,i){const{state:n,navigate:r}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await _.from("decks").select(`id,name,is_active,formation,stadium_card_id,
      stadium_card:cards!stadium_card_id(id,stadium_id,
        stadium_def:stadium_definitions(id,name,club_id,country_code,image_url,
          club:clubs(encoded_name,logo_url)))`).eq("owner_id",n.profile.id).order("created_at",{ascending:!1});if(!o||o.length===0){Et(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>r("decks"));return}const a=o.map(x=>x.id),{data:s}=await _.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,stadium_id,evolution_bonus,
        player:players(id,firstname,surname_real,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order"),l=[...new Set((s||[]).filter(x=>{var g,m;return((g=x.card)==null?void 0:g.card_type)==="stadium"&&((m=x.card)==null?void 0:m.stadium_id)}).map(x=>x.card.stadium_id))],c={};if(l.length){const{data:x}=await _.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",l);(x||[]).forEach(g=>{c[g.id]=g}),(s||[]).forEach(g=>{var m,p;((m=g.card)==null?void 0:m.card_type)==="stadium"&&((p=g.card)!=null&&p.stadium_id)&&(g.card._stadiumDef=c[g.card.stadium_id]||null)})}let d=0;function f(){var y,C,E,G,N,ne,ie;const x=o[d],g=(s||[]).filter(Q=>Q.deck_id===x.id),m=g.filter(Q=>{var M;return Q.is_starter&&((M=Q.card)==null?void 0:M.player)}).map(Q=>ki(Q.card,Q.position)),p=g.find(Q=>{var M;return((M=Q.card)==null?void 0:M.card_type)==="formation"}),u=x.formation||((y=p==null?void 0:p.card)==null?void 0:y.formation)||"4-4-2";let b=m.length>=11?ri(m,u):null,v=((C=x.stadium_card)==null?void 0:C.stadium_def)||null;v&&b&&(b=Xt(b,v));const h=m.length>=11;nt(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    ...
    </div>`}async function Do(e,t){const{state:i}=t,r=(i.params||{}).matchMode||"vs_ai_easy",o=r.replace("vs_ai_",""),a=r;await pi(e,t,r,async({deckId:s,formation:l,starters:c,subsRaw:d,gcCardsEnriched:f,gcDefs:x,stadiumDef:g})=>{try{let m=ri(c,l);g&&(m=Xt(m,g),oi(d,g));const p=await Fo(l,o),u=p.lines||p,b=async v=>{try{const h=a==="vs_ai_club"?"club":a,{data:k,error:y}=await _.from("matches").insert({home_id:i.profile.id,away_id:null,mode:h,home_deck_id:s,status:"in_progress"}).select().single();if(y){console.error("[MatchIA] Erreur création match:",y),Et(e,"⚠️","Impossible de créer le match ("+y.message+").","Retour",()=>t.navigate("home"));return}const C=p.stadiumDef||null;C&&u&&(Xt(u,C),oi(p.subs||[],C));const E={gcDefs:x||[],matchId:k==null?void 0:k.id,mode:a,difficulty:o,formation:l,homeTeam:m,aiTeam:u,homeSubs:d,subsUsed:0,maxSubs:Math.min(d.length,3),aiSubs:p.subs||[],aiSubsUsed:0,aiMaxSubs:Math.min((p.subs||[]).length,3),aiUsedSubIds:[],aiGcCards:p.gcCards||[],aiUsedGc:[],aiStadiumDef:C,homeScore:0,aiScore:0,gcCards:v,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Go(e,E,t)}catch(h){console.error("[MatchIA] Exception launchMatch:",h),Et(e,"⚠️","Erreur au lancement du match : "+h.message,"Retour",()=>t.navigate("home"))}};if(!f.length){b([]);return}li(e,f,b)}catch(m){console.error("[MatchIA] Exception setup:",m),Et(e,"⚠️","Erreur de préparation du match : "+m.message,"Retour",()=>t.navigate("home"))}})}async function Fo(e,t){var u;const{data:i}=await _.from("players").select("id,firstname,surname_real,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(80);if(!i||i.length<11)return{lines:Po(e),subs:[],gcCards:[],stadiumDef:null};const n=Ft[e]||Ft["4-4-2"],r={GK:[],DEF:[],MIL:[],ATT:[]},o=new Set;function a(b,v,h){var k,y;return o.add(b.id),{cardId:"ai-"+b.id+"-"+h,id:b.id,firstname:b.firstname,name:b.surname_real,country_code:b.country_code,club_id:b.club_id,job:b.job,job2:b.job2,note_g:Number(b.note_g)||0,note_d:Number(b.note_d)||0,note_m:Number(b.note_m)||0,note_a:Number(b.note_a)||0,rarity:b.rarity,skin:b.skin,hair:b.hair,hair_length:b.hair_length,clubName:((k=b.clubs)==null?void 0:k.encoded_name)||null,clubLogo:((y=b.clubs)==null?void 0:y.logo_url)||null,boost:0,used:!1,_line:v}}for(const b of["GK","DEF","MIL","ATT"]){const v=i.filter(E=>E.job===b&&!o.has(E.id)),h=i.filter(E=>E.job!==b&&!o.has(E.id)),k=[...v,...h],y=[];for(let E=0;E<n[b];E++){const G=k[E];G&&y.push(a(G,b,E))}const C=pt(y.length);y.forEach((E,G)=>{E._col=C[G]}),r[b]=y}const l=i.filter(b=>!o.has(b.id)).slice(0,5).map((b,v)=>a(b,b.job,100+v)),f=Object.keys(Me).sort(()=>Math.random()-.5).slice(0,3).map((b,v)=>{var h,k;return{id:"ai-gc-"+v,gc_type:b,name:((h=Me[b])==null?void 0:h.name)||b,icon:((k=Me[b])==null?void 0:k.icon)||"⚡"}}),x=Object.values(r).flat(),g={};x.forEach(b=>{b.club_id&&(g[b.club_id]=(g[b.club_id]||0)+1)});const m=(u=Object.entries(g).sort((b,v)=>v[1]-b[1])[0])==null?void 0:u[0];let p=null;if(m){const{data:b}=await _.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",m).single();b&&(p={club_id:b.id,country_code:null,name:b.encoded_name+" Stadium",club:{encoded_name:b.encoded_name,logo_url:b.logo_url}})}return{lines:r,subs:l,gcCards:f,stadiumDef:p}}function Po(e){const t=Ft[e]||Ft["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},n=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let r=0;for(const o of["GK","DEF","MIL","ATT"]){const a=[];for(let l=0;l<t[o];l++){const c=3+Math.floor(Math.random()*5);a.push({cardId:"fake-"+r,id:"fake-"+r,firstname:"IA",name:n[r%n.length],country_code:"XX",club_id:null,job:o,job2:null,note_g:o==="GK"?c:2,note_d:o==="DEF"?c:2,note_m:o==="MIL"?c:2,note_a:o==="ATT"?c:2,rarity:"normal",boost:0,used:!1,_line:o}),r++}const s=pt(a.length);a.forEach((l,c)=>{l._col=s[c]}),i[o]=a}return i}function Go(e,t,i){var r;const n=`<div style="position:relative;width:22px;height:22px;flex-shrink:0;display:flex;align-items:center;justify-content:center">
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    ${(()=>{const w=window.innerWidth>=700,j=(R,S,q)=>{var ge,Be;const P=(t.gcDefs||[]).find(je=>je.name===R.gc_type),X={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[P==null?void 0:P.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",V={purple:"#b06ce0",light_blue:"#00d4ef"}[P==null?void 0:P.color]||"#b06ce0",D=(P==null?void 0:P.name)||R.gc_type,O=(P==null?void 0:P.effect)||((ge=Me[R.gc_type])==null?void 0:ge.desc)||"",we=P!=null&&P.image_url?`/icons/${P.image_url}`:null,_e=((Be=Me[R.gc_type])==null?void 0:Be.icon)||"⚡",ye=Math.round(q*.22),be=Math.round(q*.22),$e=q-ye-be,Le=D.length>12?7:9;return`<div class="gc-mini" data-gc-id="${R.id}" data-gc-type="${R.gc_type}"
          ...
          </div>`},re=(R,S)=>S?U(130,175):j(R,130,175),ae=(R,S)=>S?U(68,95):j(R,68,95),se=w?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",de=g?`<button id="btn-results" style="${se};background:linear-gradient(135deg,#D4A017,#FFD700);border:none;color:#000">🏁 Résultats</button>`:d?`<div style="${se};background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);color:rgba(255,255,255,0.4)">⏳ Tour IA</div>`:u?`<button id="btn-pass" style="${se};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER (plus d'attaquants)</button>`:f?`<button id="btn-action" style="${se};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${t.selected.length===0?"disabled":""}> ⚔️ ATTAQUEZ <span id="match-timer" style="font-weight:900"></span></button>`:x?`<button id="btn-action" style="${se};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${t.selected.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="match-timer" style="font-weight:900"></span></button>`:`<div style="${se};background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1)"></div>`,ke=f||x?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${t.selected.length}/3 sélectionné(s)</div>`:"",W=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${w?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;display:flex;gap:0;overflow:hidden">
      ...
      <div id="in-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
        ...
        <div>${u?mt({...u,used:!1,boost:0},b,v):"<div>—</div>"}</div>
      ...
      <div id="out-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
        ...
        <div>${p?mt({...p,used:!1,boost:0},b,v):"<div>—</div>"}</div>
    ...
    </div>`,document.body.appendChild(g),(m=g.querySelector("#gc-back"))==null||m.addEventListener("click",()=>g.remove()),(p=g.querySelector("#gc-use"))==null||p.addEventListener("click",()=>{g.remove(),Xo(e,t,i,n,r)})}function ti(e,t,i,n,r,o){const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let s=[];function l(){var c,d;a.innerHTML=`
    ...
    <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start;justify-content:center">
      ${e.map(f=>{const x=f._line||f.job||"MIL",g=s.find(p=>p.cardId===f.cardId),m=Se({...f,_evolution_bonus:0},{width:90,showStad:!0,role:x,extraNote:f.boost||0});return`<div class="gc-pick-item" data-cid="${f.cardId}"
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(r=>{r.addEventListener("click",()=>{const o=r.dataset.cardId;for(const a of["GK","DEF","MIL","ATT"]){const s=(t.homeTeam[a]||[]).find(l=>l.cardId===o);if(s){s.boost=(s.boost||0)+t.boostCard.value,t.log.push({text:`⚡ Boost +${t.boostCard.value} appliqué à ${s.name}`,type:"info"});break}}t.boostUsed=!0,i.closeModal(),Ee(e,t,i)})})}async function Dt(e,t,i){var d,f;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:n}=i,r=t.homeScore>t.aiScore,o=t.homeScore===t.aiScore,a=r?"victoire":o?"nul":"defaite",s=Jn(t.mode,a);t.matchId&&await _.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:r?n.profile.id:null,home_credits_reward:s,played_at:new Date().toISOString()}).eq("id",t.matchId);const l={credits:(n.profile.credits||0)+s,matches_played:(n.profile.matches_played||0)+1};r?l.wins=(n.profile.wins||0)+1:o?l.draws=(n.profile.draws||0)+1:l.losses=(n.profile.losses||0)+1,await _.from("users").update(l).eq("id",n.profile.id),await i.refreshProfile();const c=document.createElement("div");c.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",c.innerHTML=`
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
    ...
    </div>`,`<button class="btn btn-primary" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const jt=e=>_n(e);async function zt(e,t,i,n,r={}){return Ln(e,t,i,n,r.myGC||[],r.gcDefs||[],r.isRanked||!1,r.rankedData||null,r.stadiumDef||null,r.onMatchEnd||null,r.mlLeagueId||null,r.mlMatchId||null)}async function Zo(e,t,i){const{data:n}=await _.from("matches").select("home_id,away_id,mode,is_ranked").eq("id",i).single();if(!n){t.toast("Match introuvable","error"),t.navigate("home");return}const r=n.home_id===t.state.user.id;let o=null,a=null;if(n.mode==="mini_league"){const{data:s}=await _.from("mini_league_matches").select("id, league_id").eq("match_id",i).maybeSingle();s&&(o=s.league_id,a=s.id)}return Ln(e,t,i,r,[],[],n.is_ranked||!1,null,null,null,o,a)}async function Ln(e,t,i,n,r=[],o=[],a=!1,s=null,l=null,c=null,d=null,f=null){const{state:x,navigate:g,toast:m}=t,p=n?"p1":"p2",u=n?"p2":"p1",b=(r||[]).map($=>$.id),v=(r||[]).map($=>({id:$.id,gc_type:$.gc_type,_gcDef:$._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:h}=await _.from("matches").select("*").eq("id",i).single();if(!h){m("Match introuvable","error"),g("home");return}async function k(){var me,xe;console.log("[PvP] buildGameState match:",{id:h.id,home_deck_id:h.home_deck_id,away_deck_id:h.away_deck_id,mode:h.mode,is_ranked:h.is_ranked});const[{data:$,error:L},{data:z,error:I},{data:T},{data:A}]=await Promise.all([_.rpc("get_deck_for_match",{p_deck_id:h.home_deck_id}),_.rpc("get_deck_for_match",{p_deck_id:h.away_deck_id}),_.from("users").select("id,pseudo,club_name").eq("id",h.home_id).single(),_.from("users").select("id,pseudo,club_name").eq("id",h.away_id).single()]);console.log("[PvP] get_deck_for_match p1:",L==null?void 0:L.message,"p2:",I==null?void 0:I.message,"p1D:",(me=$==null?void 0:$.starters)==null?void 0:me.length,"p2D:",(xe=z==null?void 0:z.starters)==null?void 0:xe.length);const F=te=>{const Te=Number(te.evolution_bonus)||0;return{cardId:te.card_id,position:te.position,id:te.id,firstname:te.firstname,name:te.surname_real,country_code:te.country_code,club_id:te.club_id,job:te.job,job2:te.job2,note_g:(Number(te.note_g)||0)+(te.job==="GK"||te.job2==="GK"&&Number(te.note_g)>0?Te:0),note_d:(Number(te.note_d)||0)+(te.job==="DEF"||te.job2==="DEF"&&Number(te.note_d)>0?Te:0),note_m:(Number(te.note_m)||0)+(te.job==="MIL"||te.job2==="MIL"&&Number(te.note_m)>0?Te:0),note_a:(Number(te.note_a)||0)+(te.job==="ATT"||te.job2==="ATT"&&Number(te.note_a)>0?Te:0),evolution_bonus:Te,rarity:te.rarity,skin:te.skin,hair:te.hair,hair_length:te.hair_length,face:te.face||null,clubName:te.club_encoded_name||null,clubLogo:te.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},B=(($==null?void 0:$.starters)||[]).map(te=>F(te)),K=((z==null?void 0:z.starters)||[]).map(te=>F(te)),Y=($==null?void 0:$.formation)||"4-4-2",oe=(z==null?void 0:z.formation)||"4-4-2";let Z=ri(B,Y),H=ri(K,oe);const le=(($==null?void 0:$.subs)||[]).map(te=>F(te)),pe=((z==null?void 0:z.subs)||[]).map(te=>F(te)),ue=($==null?void 0:$.stadium_def)||(n?l:null),ce=(z==null?void 0:z.stadium_def)||(n?null:l);return ue&&(Z=Xt(Z,ue),oi(le,ue)),ce&&(H=Xt(H,ce),oi(pe,ce)),{p1Team:Z,p2Team:H,p1Subs:le,p2Subs:pe,p1Formation:Y,p2Formation:oe,p1Name:(T==null?void 0:T.club_name)||(T==null?void 0:T.pseudo)||"Joueur 1",p2Name:(A==null?void 0:A.club_name)||(A==null?void 0:A.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:n?b:[],gc_p2:n?[]:b,gcCardsFull_p1:n?v:[],gcCardsFull_p2:n?[]:v,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:o||[],lastActionAt:new Date().toISOString()}}let y=h.game_state&&Object.keys(h.game_state).length?h.game_state:null;if(console.log("[PvP] init - amIHome:",n,"gameState exists:",!!y,"match.status:",h.status,"home_id:",h.home_id,"away_id:",h.away_id,"myId:",x.profile.id),!y)if(n){y=await k(),console.log("[PvP] home - gameState built, p1Team keys:",Object.keys((y==null?void 0:y.p1Team)||{}));const{data:$}=await _.from("matches").select("game_state").eq("id",i).single();!($!=null&&$.game_state)||!Object.keys($.game_state).length?await _.from("matches").update({game_state:y,turn_user_id:h.home_id}).eq("id",i):y=$.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>',console.log("[PvP] away - waiting for home to write game_state...");for(let $=0;$<30&&!y;$++){await new Promise(z=>setTimeout(z,400));const{data:L}=await _.from("matches").select("game_state").eq("id",i).single();L!=null&&L.game_state&&Object.keys(L.game_state).length&&(y=L.game_state),$%5===0&&console.log("[PvP] away - poll",$,"game_state:",!!(L!=null&&L.game_state))}if(!y){m("Erreur de synchronisation","error"),g("home");return}y.gc_p2=b,y.gcCardsFull_p2=v,await _.from("matches").update({game_state:y}).eq("id",i)}let C=!1,E=null,G=!1;const N=new Set,ne=new Set;async function ie($){var oe,Z;try{_.removeChannel(Q)}catch{}const L=y[p+"Score"]||0,z=y[u+"Score"]||0;let I,T;$.winner_id?(I=$.winner_id===x.profile.id,T=!1):$.forfeit?(I=L>z,T=!1):(T=L===z,I=L>z);let A="";if(a&&n)try{const{data:H}=await _.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",h.home_id).single(),{data:le}=await _.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",h.away_id).single();if(H&&le){const pe=I?1:T?.5:0,ue=1-pe,ce=H.placement_matches<10,me=le.placement_matches<10,xe=computeGlicko2(H.mmr,H.mmr_deviation,H.mmr_volatility,le.mmr,le.mmr_deviation,pe===1?1:pe===0?0:.5,ce),te=computeGlicko2(le.mmr,le.mmr_deviation,le.mmr_volatility,H.mmr,H.mmr_deviation,ue===1?1:ue===0?0:.5,me);await _.rpc("update_mmr_after_ranked",{p_match_id:i,p_winner_id:T?null:I?h.home_id:h.away_id,p_home_id:h.home_id,p_away_id:h.away_id,p_home_delta:xe.delta,p_away_delta:te.delta,p_home_new_rd:xe.newRd,p_away_new_rd:te.newRd,p_home_new_vol:xe.newSigma,p_away_new_vol:te.newSigma});const Te=xe.delta,Fe=xe.newMmr,Ie=getTier(Fe),He=Te>=0?"+":"",It=Te>=0?"#4caf50":"#ff6b6b",Je=getTier(H.mmr);A=`
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${Pt($,y[p+"Formation"],qe,K,Y?1300:Lt(),Y?600:qt(),xe)}
      </div>
      ...
      <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
        ...
        <div id="mobile-play-area" style="flex:1;min-height:0;display:flex;overflow:hidden">
          <div id="match-field" style="flex:1;min-width:0;min-height:0;overflow:hidden">
            <div class="terrain-wrapper" style="width:100%;height:100%;overflow:hidden">
              ${Pt($,y[p+"Formation"],qe,K,Lt(),qt(),xe)}
            </div>
          </div>
        </div>
        <div id="mobile-action-bar" style="position:absolute;left:0;right:0;bottom:0;z-index:20;background:rgba(0,0,0,0.55);padding:6px 8px 8px;display:flex;flex-direction:column;gap:6px;box-shadow:0 -4px 16px rgba(0,0,0,0.5)">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      ${ai($,y[p+"Name"]||"Vous","#FFD700","me",null)}
      <div style="display:flex;flex-direction:column;align-items:center;gap:2px;margin:4px 0">
      ...
      </div>`,(Fe=Z.querySelector("#psub-close"))==null||Fe.addEventListener("click",()=>Z.remove()),(Ie=Z.querySelector("#pout-up"))==null||Ie.addEventListener("click",()=>{B>0&&(B--,H())}),(He=Z.querySelector("#pout-down"))==null||He.addEventListener("click",()=>{B<A.length-1&&(B++,H())}),(It=Z.querySelector("#pin-up"))==null||It.addEventListener("click",()=>{Y>0&&(Y--,H())}),(Je=Z.querySelector("#pin-down"))==null||Je.addEventListener("click",()=>{Y<F.length-1&&(Y++,H())});const Te=(qe,Qe,Re,ht)=>{const Ze=Z.querySelector("#"+qe);if(!Ze)return;let Ue=0;Ze.addEventListener("touchstart",at=>{Ue=at.touches[0].clientY},{passive:!0}),Ze.addEventListener("touchend",at=>{const st=at.changedTouches[0].clientY-Ue;if(Math.abs(st)<30)return;const vt=Qe();st<0&&vt<ht-1?(Re(vt+1),H()):st>0&&vt>0&&(Re(vt-1),H())},{passive:!0})};Te("pin-panel",()=>Y,qe=>Y=qe,F.length),Te("pout-panel",()=>B,qe=>B=qe,A.length),(Qt=Z.querySelector("#psub-confirm"))==null||Qt.addEventListener("click",async qe=>{if(qe.preventDefault(),qe.stopPropagation(),oe)return;oe=!0;const Qe=A[B],Re=F[Y];if(!Qe||!Re)return;const ht=Qe._line,Ze=(z[ht]||[]).findIndex(st=>st.cardId===Qe.cardId);if(Ze===-1){m("Erreur : joueur introuvable","error"),Z.remove();return}const Ue={...Re,_line:ht,position:Qe.position,used:!1,boost:0};z[ht].splice(Ze,1,Ue);const at=[...I,Re.cardId];Z.remove(),X(Qe,Re,async()=>{await M({[p+"Team"]:z,[u+"Team"]:y[u+"Team"],["usedSubIds_"+p]:at})})})}document.body.appendChild(Z),H()}function X($,L,z){const I={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const A=(K,Y,oe)=>`<div style="text-align:center">
      ...
      </div>`,document.body.appendChild(L),(z=L.querySelector("#pvp-hist-close"))==null||z.addEventListener("click",()=>L.remove())}async function O(){if(y.phase!==p+"-attack")return;const $=p==="p1"?"p2":"p1",L=(y.round||0)+1,z=[...y.log||[]];z.push({type:"info",text:`⏭️ ${y[p+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const I=Oe(y),T=je($),A=I||!T?"finished":$+"-attack";await M({["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:A,attacker:$,round:L,log:z}),A==="finished"&&await Xe(y)}function we($){const L=($.GK||[]).some(I=>!I.used),z=["DEF","MIL","ATT"].some(I=>($[I]||[]).some(T=>!T.used));return L&&!z}function _e($){return($.p1Score||0)!==($.p2Score||0)?null:we($.p1Team)&&!ge($.p2Team)?{side:"p1",gk:($.p1Team.GK||[]).find(L=>!L.used)}:we($.p2Team)&&!ge($.p1Team)?{side:"p2",gk:($.p2Team.GK||[]).find(L=>!L.used)}:null}async function ye($,L){const{side:z,gk:I}=$;if(!I)return;const T=z+"Score",A=(L[T]||0)+1;I.used=!0;const F=[...L.log||[]];F.push({type:"duel",isGoal:!0,homeScored:z===p,homePlayers:z==="p1"?[jt(I)]:[],aiPlayers:z==="p2"?[jt(I)]:[],text:`⚽ DERNIER CORNER — Le gardien de ${L[z+"Name"]} marque !`});const B=(L.round||0)+1,K=z==="p1"?"p2":"p1",Y={...L,[T]:A},oe=Oe(Y);N.add(B);const Z=document.createElement("div");Z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:950;display:flex;align-items:center;justify-content:center;overflow:hidden;text-align:center;padding:24px",document.body.appendChild(Z);const H=(ue,ce)=>new Promise(me=>{Z.innerHTML=`<div style="font-size:32px;font-weight:900;color:${ce};letter-spacing:2px;animation:lcFade 1.4s ease both">${ue}</div>
      <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(me,1400)});await H("⚽ DERNIER CORNER","#FFD700"),await H("🧤 LE GARDIEN MONTE !","#4fc3f7"),Z.remove();const le=z===p?A:L[p+"Score"]||0,pe=z===p?L[u+"Score"]||0:A;ot([jt(I)],le,pe,z===p,async()=>{await M({[T]:A,log:F,round:B,pendingAttack:null,phase:oe?"finished":K+"-attack",attacker:K,["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}}}),oe&&await Xe({...y,[T]:A})})}async function be(){const $=y[p+"Team"],L=!["GK","DEF","MIL","ATT"].some(A=>(y[u+"Team"][A]||[]).some(F=>!F.used)),z=(y["selected_"+p]||[]).map(A=>{const F=($[A._role]||[]).find(pe=>pe.cardId===A.cardId)||A,B=["GK","DEF"].includes(A._role),K=$[A._role]||[],Y=K.findIndex(pe=>pe.cardId===A.cardId),oe=pt(K.length),Z=Y>=0?oe[Y]:F._col??1,H=y.stadiumDef||y.homeStadiumDef||null,le=F.stadiumBonus||H&&(H.club_id&&String(F.club_id)===String(H.club_id)||H.country_code&&F.country_code===H.country_code)||!1;return{...F,_line:A._role,_col:Z,stadiumBonus:le,...B?{note_a:Math.max(1,Number(F.note_a)||0)}:{}}});if(!z.length)return;const I=Vt(z,y.modifiers[p]||{},y[p+"Formation"]);ae(p,z.map(A=>A.cardId)),z.forEach(A=>{const F=($[A._role]||[]).find(B=>B.cardId===A.cardId);F&&(F.used=!0)}),y["selected_"+p]=[],de();const T=[...y.log||[]];if(L){if(z.length===1&&(z[0]._line==="GK"||z[0].job==="GK")&&(y.p1Score||0)===(y.p2Score||0)){await ye({side:p,gk:z[0]},{...y,[p+"Team"]:$,log:T});return}const F=(y[p+"Score"]||0)+1,B=z.map(H=>jt(H));T.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:B,homeTotal:I.total,aiTotal:0});const K=(y.round||0)+1,Y=p==="p1"?"p2":"p1",oe={...y,[p+"Team"]:$,[p+"Score"]:F},Z=Oe(oe);N.add(K),ot(B,F,y[u+"Score"]||0,!0,async()=>{await M({[p+"Team"]:$,[p+"Score"]:F,["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:Z?"finished":Y+"-attack",attacker:Y,round:K,log:T}),Z&&await Xe({...y,[p+"Score"]:F})});return}T.push({type:"pending",text:`⚔️ ${y[p+"Name"]} attaque (${I.total})`}),await M({[p+"Team"]:$,[u+"Team"]:y[u+"Team"],pendingAttack:{...I,players:z,side:p},["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},phase:u+"-defense",log:T})}async function $e(){const $=y[p+"Team"],L=(y["selected_"+p]||[]).map(ce=>{const me=($[ce._role]||[]).find(Ie=>Ie.cardId===ce.cardId)||ce,xe=$[ce._role]||[],te=xe.findIndex(Ie=>Ie.cardId===ce.cardId),Te=pt(xe.length),Fe=te>=0?Te[te]:me._col??1;return{...me,_line:ce._role,_col:Fe}}),z=Yt(L,y.modifiers[p]||{},y[p+"Formation"]);ae(p,L.map(ce=>ce.cardId)),L.forEach(ce=>{const me=($[ce._role]||[]).find(xe=>xe.cardId===ce.cardId);me&&(me.used=!0)}),y["selected_"+p]=[],de();const I=Ci(y.pendingAttack.total,z.total,y.modifiers[p]||{}),T=y.pendingAttack.side,A=I==="attack"||(I==null?void 0:I.goal),F=T==="p1"?"p2":"p1",B=(y.round||0)+1,K=(y.pendingAttack.players||[]).map(ce=>jt(ce)),Y=[...y.log||[]];Y.push({type:"duel",isGoal:A,homeScored:A&&T===p,text:A?`⚽ BUT de ${y[T+"Name"]} ! (${y.pendingAttack.total} vs ${z.total})`:`✋ Attaque stoppée (${y.pendingAttack.total} vs ${z.total})`,homePlayers:K,aiPlayers:L.map(ce=>jt(ce)),homeTotal:y.pendingAttack.total,aiTotal:z.total});const oe=A?(y[T+"Score"]||0)+1:y[T+"Score"]||0,Z={...y,[p+"Team"]:$,[T+"Score"]:oe},H=Oe(Z),le=H?"finished":F+"-attack",pe=!H&&!A?_e(Z):null,ue=async()=>{if(pe){await ye(pe,Z);return}await M({[p+"Team"]:$,[u+"Team"]:y[u+"Team"],[T+"Score"]:oe,["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:le,attacker:F,round:B,log:Y}),(le==="finished"||H)&&await Xe({...y,[T+"Score"]:oe})};if(A){const ce=T===p,me=ce?oe:y[p+"Score"]||0,xe=ce?y[u+"Score"]||0:oe;N.add(B),ot(K,me,xe,ce,ue)}else await ue()}function Le($){return["MIL","ATT"].some(L=>($[L]||[]).some(z=>!z.used))}function ge($){return["GK","DEF","MIL","ATT"].some(L=>($[L]||[]).some(z=>!z.used))}function Be($){return ge($)&&!Le($)}function je($){const L=y[$+"Team"];return!!(Le(L)||Be(L))}function Oe($){return!ge($.p1Team)&&!ge($.p2Team)}function bt($){const L=$.p1Score||0,z=$.p2Score||0;return L===z?null:L>z?h.home_id:h.away_id}async function Xe($){try{const L=bt($),z=L?h.home_id===L?h.away_id:h.home_id:null,I=$.p1Score||0,T=$.p2Score||0,{data:A,error:F}=await _.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:L,p_home_score:I,p_away_score:T,p_forfeit:!1});if(F&&console.error("[PvP] finish_pvp_match:",F),d)try{await _.rpc("check_and_finish_mini_league",{p_league_id:d})}catch(B){console.error("[PvP] check_and_finish_mini_league:",B)}L&&z&&eo(L,z).catch(()=>{});try{const{data:B}=await _.rpc("apply_match_rewards",{p_match_id:i});B!=null&&B.success&&!(B!=null&&B.skipped)&&typeof t.refreshProfile=="function"&&await t.refreshProfile()}catch(B){console.error("[PvP] apply_match_rewards:",B)}if(typeof c=="function")try{await c({homeScore:I,awayScore:T,winnerId:L,homeId:h.home_id,awayId:h.away_id,matchId:i})}catch(B){console.error("[PvP] onMatchEnd:",B)}}catch(L){console.error("[PvP] finishMatch:",L)}}function Rt(){var T;const $=y[p+"Score"],L=y[u+"Score"],z=$>L,I=$===L;nt(e),e.innerHTML=`
    ...
    </div>`,(T=document.getElementById("pvp-home"))==null||T.addEventListener("click",()=>{try{_.removeChannel(Q)}catch{}We(e),d?g("mini-league",{openLeagueId:d}):g("home")})}de()}async function Li(e,t,i=!1){var s,l;const{state:n,navigate:r,toast:o}=t,a=i&&((l=(s=t==null?void 0:t.state)==null?void 0:s.params)==null?void 0:l.rankedData)||null;await pi(e,t,i?"ranked":"random",async({deckId:c,formation:d,starters:f,subsRaw:x,gcCardsEnriched:g,gcDefs:m,stadiumDef:p})=>{nt(e);const u=async b=>{await tr(e,t,c,d,f,x,b||[],m,p,i,a)};if(!(g!=null&&g.length)){await u([]);return}li(e,g,u)})}async function er(e,t,i){return Zo(e,t,i)}async function tr(e,t,i,n,r,o,a=[],s=[],l=null,c=!1,d=null){var ie,Q;const{state:f,navigate:x,toast:g}=t,m=(d==null?void 0:d.mmr)??((ie=f.profile)==null?void 0:ie.mmr)??1e3,p=c?Ai(m):null,u=c?"linear-gradient(135deg, #1a0a2e 0%, #2d1060 50%, #1a0a2e 100%)":"linear-gradient(135deg, #0a1a2e 0%, #0d3d1e 100%)",b=c?(p==null?void 0:p.color)||"#D4A017":"#FFD700";let v=!1,h=null,k=null;e.innerHTML=`
    ...
    </div>`;const y=async()=>{v=!0,h&&(h.unsubscribe(),h=null),k&&(clearInterval(k),k=null);try{await _.rpc("cancel_matchmaking",{p_user_id:f.profile.id})}catch(M){console.error("[Matchmaking] cancel error:",M)}};(Q=document.getElementById("cancel-mm"))==null||Q.addEventListener("click",async()=>{await y(),We(e),x("home")});const C=async(M,w)=>{v||(v=!0,h&&(h.unsubscribe(),h=null),k&&(clearInterval(k),k=null),await zt(e,t,M,w,{myGC:a,gcDefs:s,stadiumDef:l,isRanked:c,rankedData:d,onMatchEnd:c?ir:null}))},E=f.user.id,{data:G,error:N}=c?await _.rpc("try_matchmake_ranked",{p_user_id:E,p_deck_id:i,p_mmr:m}):await _.rpc("try_matchmake",{p_user_id:E,p_deck_id:i});if(N||!(G!=null&&G.success)){console.error("[Matchmaking] try_matchmake error:",N||G),g("Erreur de matchmaking","error"),We(e),x("home");return}if(G.matched){await C(G.match_id,!1);return}const ne=async()=>{if(v)return;const{data:M}=await _.from("matchmaking_queue").select("status, match_id").eq("user_id",E).maybeSingle();(M==null?void 0:M.status)==="matched"&&M.match_id&&await C(M.match_id,!0)};h=_.channel(`mm_${E}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${E}`},M=>{const w=M.new;w.status==="matched"&&w.match_id&&C(w.match_id,!0)}).subscribe(),k=setInterval(ne,3e3),setTimeout(()=>{v||y()},12e4)}async function ir(e,t){const{state:i,toast:n}=t,{winnerId:r,homeId:o,awayId:a,homeScore:s,awayScore:l,matchId:c}=e,d=i.user.id,f=o===d,x=r===d,{data:g}=await _.from("users").select("mmr, mmr_rd, mmr_v").eq("id",d).single(),{data:m}=await _.from("users").select("mmr, mmr_rd, mmr_v").eq("id",f?a:o).single();if(g&&m){const p=An(g.mmr,g.mmr_rd,g.mmr_v,m.mmr,m.mmr_rd,x?1:0);await _.from("users").update({mmr:p.mmr,mmr_rd:p.rd,mmr_v:p.v,mmr_wins:x?_.sql`mmr_wins + 1`:void 0,mmr_losses:x?void 0:_.sql`mmr_losses + 1`}).eq("id",d),n(x?`+MMR ↑ ${Math.round(p.mmr-g.mmr)}`:`-MMR ↓ ${Math.round(g.mmr-p.mmr)}`,x?"success":"error",4e3)}}const nr=Object.freeze(Object.defineProperty({__proto__:null,renderMatchRandom:Li,resumePvpMatch:er},Symbol.toStringTag,{value:"Module"}));async function or(e,t){const{state:i,navigate:n,toast:r}=t,o=i.params||{},a=o.friendId||null,s=o.friendName||"Ami",l=!!o.isAccepting;await pi(e,t,"friend",async({deckId:c,formation:d,starters:f,subsRaw:x,gcCardsEnriched:g,gcDefs:m,stadiumDef:p})=>{nt(e);const u=async b=>{const v=b||[];l?await ar(e,t,c,v,m,p):await rr(e,t,c,v,m,p,a,s)};if(!(g!=null&&g.length)){await u([]);return}li(e,g,u)})}async function rr(e,t,i,n,r,o,a,s){var v;const{state:l,navigate:c,toast:d}=t,f=l.user.id,{data:x,error:g}=await _.from("friend_match_invites").insert({inviter_id:f,invitee_id:a,friend_id:a,status:"pending",inviter_deck_id:i,expires_at:new Date(Date.now()+2*60*1e3).toISOString()}).select("id").single();if(g||!x){console.error("[Friend] Erreur création invitation:",g),d("Impossible de créer l'invitation","error"),c("home");return}e.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="display:flex;background:var(--nav-bg,#0d1a0f);border-bottom:1px solid ${$t}">
      ${b.map(k=>`<button class="ml-tab" data-tab="${k.key}" style="flex:1;padding:11px 4px;border:none;border-bottom:2px solid ${i===k.key?Tn:"transparent"};background:none;font-size:12px;font-weight:${i===k.key?"900":"600"};color:${i===k.key?"#4ade80":Ke};cursor:pointer">${k.label}${k.count?` (${k.count})`:""}</button>`).join("")}
    </div>
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:10px">
      ${i==="waiting"?dr(x,u,r):i==="active"?lr(g,r):cr(p,r)}
    </div>
  </div>`,(v=document.getElementById("ml-create-btn"))==null||v.addEventListener("click",()=>pr(e,t)),(h=document.getElementById("ml-refresh-list"))==null||h.addEventListener("click",()=>yt(e,t,i)),e.querySelectorAll(".ml-tab").forEach(k=>k.addEventListener("click",()=>yt(e,t,k.dataset.tab))),e.querySelectorAll("[data-league-id]").forEach(k=>k.addEventListener("click",()=>rt(e,t,k.dataset.leagueId))),e.querySelectorAll("[data-join]").forEach(k=>k.addEventListener("click",y=>{y.stopPropagation(),zn(e,t,k.dataset.join,k.dataset.type)})),e.querySelectorAll("[data-delete]").forEach(k=>k.addEventListener("click",y=>{y.stopPropagation(),Ti(e,t,k.dataset.delete,k.dataset.name,i)}))}function di(e,t,i=!1){var a,s;const n=e.creator_id===t,r=e.pot||0,o=((s=(a=e.mini_league_members)==null?void 0:a[0])==null?void 0:s.count)||0;return`<div data-league-id="${e.id}" style="background:${Nt};border:1px solid ${kt};border-radius:12px;padding:14px 16px;cursor:pointer;margin-bottom:8px">
    ...
    </div>`,document.body.appendChild(g),g.querySelector("#lv-ok").onclick=()=>{g.remove(),x(!0)},g.querySelector("#lv-cancel").onclick=()=>{g.remove(),x(!1)}}))return;const{data:l,error:c}=await _.rpc("leave_mini_league",{p_league_id:i,p_user_id:a});if(c||!(l!=null&&l.success)){r((l==null?void 0:l.error)||"Erreur lors de la désinscription","error");return}const d=l.refund||n;o.profile&&(o.profile.credits=(o.profile.credits||0)+d);const f=document.getElementById("nav-credits");f&&(f.textContent=`💰 ${(o.profile.credits||0).toLocaleString("fr")}`),r(`↩️ Désinscrit · +${d.toLocaleString("fr")} cr. remboursés`,"success"),yt(e,t,"waiting")}async function Ti(e,t,i,n,r){const{toast:o}=t;if(!confirm(`Supprimer définitivement "${n}" et tous ses matchs/membres ? Action irréversible.`))return;await _.from("mini_league_matches").delete().eq("league_id",i),await _.from("mini_league_members").delete().eq("league_id",i);const{error:a}=await _.from("mini_leagues").delete().eq("id",i);if(a){o("Erreur suppression ("+a.message+")","error");return}o("Mini League supprimée avec succès","success"),yt(e,t,r)}async function gr(e,t,i){await _.from("mini_leagues").update({is_archived:!0}).eq("id",i),t.toast("Mini League archivée","success"),yt(e,t,"archived")}async function rt(e,t,i){var k,y,C,E,G,N,ne,ie,Q;const{state:n,toast:r}=t,o=n.profile.id,[{data:a},{data:s},{data:l}]=await Promise.all([_.from("mini_leagues").select("*").eq("id",i).single(),_.from("mini_league_members").select("*, user:users(id,pseudo,club_name,club_color1,club_color2)").eq("league_id",i),_.from("mini_league_matches").select("*").eq("league_id",i).order("matchday").order("created_at")]);if(!a){r("Introuvable","error"),await yt(e,t);return}const c=(s||[]).some(M=>M.user_id===o),d=a.creator_id===o,f=(s||[]).map(M=>M.user).filter(Boolean),x=hr(f,l||[]),g=(l||[]).filter(M=>M.matchday===a.current_day),m=a.pot||0,p=a.entry_fee||100,u=g.length>0&&g.every(M=>M.status==="finished"||M.status==="bye"),b=a.current_day>=a.total_days,v=(s||[]).find(M=>M.user_id===o);e.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    <div style="padding:14px 16px;background:var(--nav-bg,#0d1a0f);border-bottom:1px solid ${$t};display:flex;align-items:center;gap:10px">
      ...
      <div style="text-align:right;flex-shrink:0">
        ...
        ${m>0?`<div style="font-size:12px;font-weight:900;color:${gt};margin-top:4px">🏆 ${m.toLocaleString("fr")} cr.</div>`:""}
      </div>
    </div>
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:${Nt};border:1px solid ${kt};border-radius:12px;padding:16px">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          <thead><tr style="font-size:10px;color:${Ke};text-transform:uppercase;border-bottom:2px solid ${$t}">
            ...
            ${m>0&&a.status==="finished"?`<th style="text-align:right;padding:5px 0;color:${gt}">💰</th>`:""}
          </tr></thead>
          <tbody>${x.map((M,w)=>{const j=m>0&&a.status==="finished"?w===0?Math.floor(m*.7):w===1?Math.floor(m*.2):w===2?Math.floor(m*.1):0:0;return`<tr style="border-bottom:1px solid ${$t};${M.userId===o?"background:rgba(74,222,128,0.08);":""}">
              ...
              ${m>0&&a.status==="finished"?`<td style="text-align:right;font-weight:700;color:${gt}">${j?j.toLocaleString("fr")+" cr.":"—"}</td>`:""}
            </tr>`}).join("")}</tbody>
        </table>
      </div>`:""}
      ...
      <div style="background:${Nt};border:1px solid ${kt};border-radius:12px;padding:16px">
        ...
        ${Array.from({length:Math.max(0,a.status==="active"?a.current_day-1:a.current_day)},(M,w)=>w+1).reverse().map(M=>{const w=(l||[]).filter(j=>j.matchday===M);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:${Ke};margin-bottom:6px">Journée ${M}</div>${w.map(j=>tn(j,f,o,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
    ...
    </div>`;let d=!1;const f=async u=>{if(!d){if(d=!0,clearInterval(m),g)try{g.unsubscribe()}catch{}u&&await zt(e,t,u,r,{mlLeagueId:n,mlMatchId:i,myGC:o||[],gcDefs:a,stadiumDef:s}),l()}};(p=document.getElementById("wait-back"))==null||p.addEventListener("click",()=>{if(d=!0,clearInterval(m),g)try{g.unsubscribe()}catch{}c("mini-league",{openLeagueId:n}),l()});const x=async()=>{if(d)return;const{data:u}=await _.from("mini_league_matches").select("match_id").eq("id",i).single();u!=null&&u.match_id&&f(u.match_id)},g=_.channel(`ml_match_${i}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"mini_league_matches",filter:`id=eq.${i}`},u=>{var b;(b=u.new)!=null&&b.match_id&&f(u.new.match_id)}).subscribe(),m=setInterval(x,3e3);x()})}const wr="/",_r=[{emoji:"⚽",title:"Bienvenue dans Manager Wars !",color:"#1A6B3C",content:`<p>Tu es désormais un <strong>manager de football</strong> virtuel.</p>
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
  ...
  </div>`;let l="buy";const c=()=>{var p,u,b,v,h,k,y;return{name:(((p=document.getElementById("flt-name"))==null?void 0:p.value)||"").toLowerCase().trim(),club:(((u=document.getElementById("flt-club"))==null?void 0:u.value)||"").toLowerCase().trim(),country:(((b=document.getElementById("flt-country"))==null?void 0:b.value)||"").toLowerCase().trim(),job:((v=document.getElementById("flt-job"))==null?void 0:v.value)||"",rarity:((h=document.getElementById("flt-rarity"))==null?void 0:h.value)||"",note1:parseInt((k=document.getElementById("flt-note1"))==null?void 0:k.value)||0,note2:parseInt((y=document.getElementById("flt-note2"))==null?void 0:y.value)||0}};function d(p){const u=c();return p.filter(b=>{var N,ne,ie;const v=(N=b.card)==null?void 0:N.player;if(!v)return!1;const h=`${v.firstname} ${v.surname_real}`.toLowerCase(),k=(((ne=v.clubs)==null?void 0:ne.encoded_name)||"").toLowerCase(),y=(v.country_code||"").toLowerCase(),C=((ie=b.card)==null?void 0:ie.evolution_bonus)||0,E=Ct(v,v.job,C),G=v.job2?Ct(v,v.job2,C):0;return!(u.name&&!h.includes(u.name)||u.club&&!k.includes(u.club)||u.country&&!y.includes(u.country)||u.job&&v.job!==u.job||u.rarity&&v.rarity!==u.rarity||u.note1&&E<u.note1||u.note2&&G<u.note2)})}function f(p){var G,N,ne,ie;const u=(G=p.card)==null?void 0:G.player;if(!u)return"";const b=((N=p.card)==null?void 0:N.evolution_bonus)||0,v=Ct(u,u.job,b),h=u.job2?Ct(u,u.job2,b):0,k=(i.profile.credits||0)>=p.price,y=u.country_code?`https://flagsapi.com/${u.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,C=ii[u.job]||"#bbb",E=u.job2?ii[u.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden">
      <!-- Drapeau -->
    ...
    </div>`}function x(p){var G,N,ne,ie;const u=(G=p.card)==null?void 0:G.player;if(!u)return"";const b=((N=p.card)==null?void 0:N.evolution_bonus)||0,v=Ct(u,u.job,b),h=u.job2?Ct(u,u.job2,b):0,k=p.status==="sold",y=u.country_code?`https://flagsapi.com/${u.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,C=ii[u.job]||"#bbb",E=u.job2?ii[u.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${k?"opacity:0.7":""}">
      ${y?`<img src="${y}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      ${(ne=u.clubs)!=null&&ne.logo_url?`<img src="${u.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${C};display:flex;align-items:center;justify-content:center">
    ...
    </div>`}function g(){const p=document.getElementById("mkt-content"),u=document.getElementById("mkt-filters");if(p){if(u.style.display=l==="buy"?"flex":"none",l==="buy"){const b=d(a);p.innerHTML=b.length?b.map(f).join(""):'<div style="text-align:center;color:#aaa;padding:40px">Aucune carte trouvée.</div>'}else{const b=s.filter(h=>h.status==="active").sort((h,k)=>new Date(k.listed_at)-new Date(h.listed_at)),v=s.filter(h=>h.status==="sold").sort((h,k)=>new Date(k.sold_at||k.listed_at)-new Date(h.sold_at||h.listed_at));p.innerHTML=(b.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:4px 0 6px;text-transform:uppercase;letter-spacing:1px">🟢 En vente (${b.length})</div>`+b.map(x).join(""):"")+(v.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:12px 0 6px;text-transform:uppercase;letter-spacing:1px">✅ Ventes réussies (${v.length})</div>`+v.map(x).join(""):"")+(!b.length&&!v.length?'<div style="text-align:center;color:#aaa;padding:40px">Aucune vente pour le moment.</div>':"")}p.querySelectorAll("[data-buy]").forEach(b=>b.addEventListener("click",()=>Lr(b.dataset.buy,a,e,t))),p.querySelectorAll("[data-cancel]").forEach(b=>b.addEventListener("click",()=>zr(b.dataset.cancel,e,t)))}}e.querySelectorAll(".mkt-tab").forEach(p=>{p.addEventListener("click",()=>{l=p.dataset.tab,e.querySelectorAll(".mkt-tab").forEach(u=>{const b=u===p;u.style.background=b?"var(--green)":"#fff",u.style.color=b?"#fff":"var(--gray-600)",u.style.borderColor=b?"var(--green)":"var(--gray-200)"}),g()})});let m;["flt-name","flt-club","flt-country","flt-job","flt-rarity","flt-note1","flt-note2"].forEach(p=>{var u;(u=document.getElementById(p))==null||u.addEventListener("input",()=>{clearTimeout(m),m=setTimeout(g,250)})}),g()}async function Lr(e,t,i,n){const{state:r,toast:o,refreshProfile:a}=n,s=t.find(d=>d.id===e);if(!s)return;const l=s.price;if((r.profile.credits||0)<l){o("Crédits insuffisants","error");return}Tr(s,async()=>{const{error:d}=await _.rpc("buy_market_card",{p_listing_id:e,p_buyer_id:r.profile.id});if(d){o("Erreur achat : "+d.message,"error");return}await a();const f=document.getElementById("nav-credits");f&&(f.textContent=`💰 ${(r.profile.credits||0).toLocaleString("fr")}`),o("✅ Carte achetée !","success"),await Fi(i,n)})}function Tr(e,t){var a;const i=(a=e.card)==null?void 0:a.player,n=i?`${i.firstname} ${i.surname_real}`:"cette carte",r=document.createElement("div");r.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",r.innerHTML=`
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center">
      ...
      `).join(""):'<div style="text-align:center;color:var(--gray-600);padding:40px">Aucun manager.</div>'}else{const b=a||[];m.innerHTML=b.length>0?b.map((v,h)=>{const k=Ai(v.mmr??1e3),y=(v.ranked_wins||0)+(v.ranked_losses||0)+(v.ranked_draws||0),C=y>0?Math.round((v.ranked_wins||0)/y*100):0,E=v.id===i.profile.id,G=(v.placement_matches||0)<10;return`
