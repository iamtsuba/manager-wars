import{s as k,F as it,b as Ce,c as Gt,l as ge,d as di}from"./formation-links-Bt44rMnM.js";function kt(e,{navigate:t,toast:i}){e.innerHTML=`
  ...
  </div>`}function Tt(e){const t=e.job||"ATT",i=Te(e,t),d=at[e.country_code]||e.country_code||"";return`
  <div style="
    width:140px;border-radius:12px;padding:6px;
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
  ...
  </div>`}async function xi(e,t){const{state:i,navigate:d,toast:c,openModal:o,closeModal:a}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:s}=await k.from("cards").select(`id, card_type, current_note, gc_type, formation, is_for_sale, sale_price,
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),n=(s||[]).filter($=>$.card_type==="player"&&$.player),r=(s||[]).filter($=>$.card_type==="game_changer"),u=(s||[]).filter($=>$.card_type==="formation"),{data:g}=await k.from("gc_definitions").select("name,gc_type,color,effect,image_url"),f={};(g||[]).forEach($=>{f[$.name]=$});const L=Object.keys(it),p=Object.keys(de),h={};n.forEach($=>{const b=$.player.id;h[b]=(h[b]||0)+1}),new Set(Object.keys(h).map($=>String($)));const I=new Set(u.map($=>$.formation)),v=new Set(r.map($=>$.gc_type));let y="player",x="Tous",w="",E=!1;function m(){return[...n].sort(($,b)=>{const j=Be.indexOf($.player.job),A=Be.indexOf(b.player.job);return j!==A?j-A:($.player.surname_encoded||"").localeCompare(b.player.surname_encoded||"")})}function T(){return[...l||[]].sort(($,b)=>{const j=Be.indexOf($.job),A=Be.indexOf(b.job);return j!==A?j-A:($.surname_encoded||"").localeCompare(b.surname_encoded||"")})}function q(){return m().filter($=>{const b=$.player,j=x==="Tous"||b.job===x,A=!w||`${b.firstname} ${b.surname_encoded}`.toLowerCase().includes(w);return j&&A})}function G(){return T().filter($=>{const b=x==="Tous"||$.job===x,j=!w||`${$.firstname} ${$.surname_encoded}`.toLowerCase().includes(w);return b&&j})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Ee=1e3;function hi(e,t,i,d){var p,h,I;const{state:c,toast:o,closeModal:a,navigate:s,refreshProfile:l}=i,n=e.formation,r={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function u(){const v=Ce[n]||{},y=it[n]||[],x=290,w=360,E=20;function m(q){const G=v[q];return G?{x:G.x*x,y:G.y*w}:null}let T=`<svg width="${x}" height="${w}" viewBox="0 0 ${x} ${w}" xmlns="http://www.w3.org/2000/svg">`;for(const[q,G]of y){const N=m(q),O=m(G);!N||!O||(T+=`<line x1="${N.x}" y1="${N.y}" x2="${O.x}" y2="${O.y}"
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(p=document.getElementById("direct-sell-form-btn"))==null||p.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${n} pour ${Ee.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const v=g.find(x=>!x.is_for_sale)||g[0];if(!v){o("Aucune carte à vendre","error");return}const{error:y}=await k.from("cards").delete().eq("id",v.id);if(y){o(y.message,"error");return}await k.from("users").update({credits:(c.profile.credits||0)+Ee}).eq("id",c.profile.id),await l(),o(`+${Ee.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),a(),s("collection")}),(h=document.getElementById("market-sell-form-btn"))==null||h.addEventListener("click",async()=>{const v=parseInt(document.getElementById("sell-price-form").value);if(!v||v<1){o("Prix invalide","error");return}await k.from("cards").update({is_for_sale:!0,sale_price:v}).eq("id",e.id),await k.from("market_listings").insert({seller_id:c.profile.id,card_id:e.id,price:v}),o("Carte mise en vente sur le marché !","success"),a(),s("collection")}),(I=document.getElementById("cancel-sell-form-btn"))==null||I.addEventListener("click",async()=>{await k.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await k.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),o("Annonce retirée","success"),a(),s("collection")})}async function Lt(e,t,i,d){var T,q,G,N,O,Y;const{state:c,toast:o,openModal:a,closeModal:s,navigate:l,refreshProfile:n}=d,r=e.player,u=t.filter(H=>H.player.id===r.id),g=u.length,f=mi[r.rarity]||1e3,L=r.rarity!=="legende",p=Pt(),h=Te(r,r.job),I=r.job2?Te(r,r.job2):null,v=Re[r.job]||"#1A6B3C",y=r.job2?Re[r.job2]:null,x=Nt[r.rarity]||"#ccc",w=at[r.country_code]||r.country_code||"",{data:E}=await k.from("transfer_history").select("club_name, manager_name, source, price, transferred_at").eq("player_id",r.id).order("transferred_at",{ascending:!0}),m=E&&E.length?`
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
  ...
  </div>`,document.getElementById("new-deck-btn").addEventListener("click",async()=>{const a=prompt("Nom du deck :",`Deck ${((o==null?void 0:o.length)||0)+1}`);if(!a)return;const{data:s,error:l}=await k.from("decks").insert({owner_id:i.profile.id,name:a}).select().single();if(l){c(l.message,"error");return}c("Deck créé !","success"),Mt(s.id,e,t)}),e.querySelectorAll("[data-open-deck]").forEach(a=>{a.addEventListener("click",()=>Mt(a.dataset.openDeck,e,t))}),e.querySelectorAll("[data-rename]").forEach(a=>{a.addEventListener("click",async()=>{const s=prompt("Nouveau nom :",a.dataset.name);if(!s||s===a.dataset.name)return;const{error:l}=await k.from("decks").update({name:s}).eq("id",a.dataset.rename);if(l){c(l.message,"error");return}c("Deck renommé !","success"),We(e,t)})}),e.querySelectorAll("[data-delete]").forEach(a=>{a.addEventListener("click",async()=>{if(!confirm(`Supprimer le deck "${a.dataset.name}" ? Cette action est irréversible.`))return;await k.from("deck_cards").delete().eq("deck_id",a.dataset.delete);const{error:s}=await k.from("decks").delete().eq("id",a.dataset.delete);if(s){c(s.message,"error");return}c("Deck supprimé.","success"),We(e,t)})})}async function Mt(e,t,i){const{state:d,toast:c}=i;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await k.from("decks").select("*").eq("id",e).single(),{data:a}=await k.from("cards").select(`id, card_type, formation,
      player:players(id, firstname, surname_encoded, country_code, club_id, job, job2,
        note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length,
        clubs(encoded_name, logo_url))`).eq("owner_id",d.profile.id),s=(a||[]).filter(f=>f.card_type==="player"&&f.player),l=(a||[]).filter(f=>f.card_type==="formation"),n=l.map(f=>f.formation).filter(Boolean),{data:r}=await k.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",e);let u=o.formation||"4-4-2";n.length>0&&!n.includes(u)&&(u=n[0]);const g={deckId:e,name:o.name,formation:u,formationCardId:o.formation_card_id,slots:{},subs:[],playerCards:s,formationCards:l,availableFormations:n};(r||[]).forEach(f=>{f.is_starter?g.slots[f.position]=f.card_id:g.subs.includes(f.card_id)||g.subs.push(f.card_id)}),_e(t,g,i)}function _e(e,t,i){var l;const{navigate:d}=i;Ne[t.formation];const c=zt(t.formation),o=c.filter(n=>t.slots[n]).length,a=t.availableFormations.length>0?t.availableFormations:Object.keys(Ne),s=t.subs.map(n=>t.playerCards.find(r=>r.id===n)).filter(Boolean);[...Object.values(t.slots),...t.subs],e.innerHTML=`
  <div class="page">
    ...
    </div>`,'<button class="btn btn-ghost" id="close-sub-selector">Fermer</button>'),(l=document.getElementById("close-sub-selector"))==null||l.addEventListener("click",c),document.querySelectorAll(".player-option").forEach(n=>{n.addEventListener("click",()=>{e.subs.push(n.dataset.cardId),c(),_e(t,e,i)})})}async function $i(e,t){const{state:i,toast:d,navigate:c}=t,o=e.formationCards.find(l=>l.formation===e.formation),a=(o==null?void 0:o.id)||e.formationCardId;await k.from("decks").update({formation:e.formation,formation_card_id:a||null}).eq("id",e.deckId),await k.from("deck_cards").delete().eq("deck_id",e.deckId);const s=[];if(Object.entries(e.slots).forEach(([l,n],r)=>{s.push({deck_id:e.deckId,card_id:n,position:l,is_starter:!0,slot_order:r})}),e.subs.forEach((l,n)=>{s.push({deck_id:e.deckId,card_id:l,position:`SUB${n+1}`,is_starter:!1,slot_order:100+n})}),s.length>0){const{error:l}=await k.from("deck_cards").insert(s);if(l){d(l.message,"error");return}}d("Deck enregistré ✅","success"),c("decks")}function zt(e){const t=Ne[e]||Ne["4-4-2"],i=["GK1"];for(let d=1;d<=t.DEF;d++)i.push(`DEF${d}`);for(let d=1;d<=t.MIL;d++)i.push(`MIL${d}`);for(let d=1;d<=t.ATT;d++)i.push(`ATT${d}`);return i}const ki="modulepreload",Ei=function(e){return"/manager-wars/"+e},Ct={},Ai=function(t,i,d){let c=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),s=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));c=Promise.allSettled(i.map(l=>{if(l=Ei(l),l in Ct)return;Ct[l]=!0;const n=l.endsWith(".css"),r=n?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${l}"]${r}`))return;const u=document.createElement("link");if(u.rel=n?"stylesheet":ki,n||(u.as="script"),u.crossOrigin="",u.href=l,s&&u.setAttribute("nonce",s),document.head.appendChild(u),n)return new Promise((g,f)=>{u.addEventListener("load",g),u.addEventListener("error",()=>f(new Error(`Unable to preload CSS for ${l}`)))})}))}function o(a){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=a,window.dispatchEvent(s),!s.defaultPrevented)throw a}return c.then(a=>{for(const s of a||[])s.status==="rejected"&&o(s.reason);return t().catch(o)})};async function Ut(){const{data:e}=await k.from("booster_configs").select("*").eq("is_active",!0).order("sort_order");if(!(e!=null&&e.length))return[];const{data:t}=await k.from("booster_drop_rates").select("*").in("booster_id",e.map(i=>i.id)).order("sort_order");return e.map(i=>({...i,rates:(t||[]).filter(d=>d.booster_id===i.id)}))}function Ii(e){if(!(e!=null&&e.length))return null;const t=e.reduce((d,c)=>d+Number(c.percentage),0);let i=Math.random()*t;for(const d of e)if(i-=Number(d.percentage),i<=0)return d;return e[e.length-1]}const Ti=Object.freeze(Object.defineProperty({__proto__:null,loadActiveBoosters:Ut,rollDropRate:Ii},Symbol.toStringTag,{value:"Module"})),Kt=()=>Object.keys(Ce),Li=[{id:"players_std",img:"/manager-wars/icons/booster-players.png",name:"Players",sub:"5 cartes joueurs",cost:5e3,costLabel:"5 000 crédits",cardCount:5,type:"player"},{id:"players_pub",img:"/manager-wars/icons/booster-silver.png",name:"Players (pub)",sub:"3 cartes joueurs",cost:0,costLabel:"1 pub",cardCount:3,type:"player"},{id:"game_changer",img:"/manager-wars/icons/booster-gamechanger.png",name:"Game Changer",sub:"3 cartes spéciales",cost:1e4,costLabel:"10 000 crédits",cardCount:3,type:"game_changer"},{id:"formation",img:"/manager-wars/icons/booster-formation.png",name:"Formation",sub:"1 carte formation",cost:1e4,costLabel:"10 000 crédits",cardCount:1,type:"formation"}],Xe={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé."},"Double attaque":{icon:"⚡",desc:"La prochaine attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la prochaine action IA."},Gel:{icon:"❄️",desc:"Bloque le meilleur attaquant IA."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function Mi(e){return null}const zi={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},Ci={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"};function Si(e){var c;const t={};(e.rates||[]).forEach(o=>{t[o.card_type]=(t[o.card_type]||0)+Number(o.percentage||0)});const i=((c=Object.entries(t).sort((o,a)=>a[1]-o[1])[0])==null?void 0:c[0])||"player",d=e.image_url||"booster-players.png";return{id:e.id,img:"/manager-wars/icons/"+d,name:e.name,sub:`${e.card_count} carte(s)`,cost:e.price_type==="credits"&&e.price_credits||0,costLabel:e.price_type==="credits"?`${(e.price_credits||0).toLocaleString("fr")} crédits`:e.price_type==="pub"?"1 pub":"Gratuit",cardCount:e.card_count||5,type:i,isPub:e.price_type==="pub",rates:e.rates||[],_raw:e}}async function ji(e,{state:t,navigate:i,toast:d}){var a;const c=((a=t.profile)==null?void 0:a.credits)||0;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⏳ Chargement...</div>';let o=[];try{o=(await Ut()).map(Si)}catch(s){console.warn("Erreur chargement boosters DB, fallback hardcodé",s)}o.length||(o=Li.map(s=>({...s,rates:[],isPub:s.id==="players_pub"}))),e.innerHTML=`
  <div class="page">
    <div class="page-header">
      <h2>📦 Boosters</h2>
  ...
  `,document.body.appendChild(d);let c=!1;const o=document.getElementById("pack-cut-zone"),a=document.getElementById("pack-blade");let s=!1;const l=x=>x.touches&&x.touches[0]?x.touches[0].clientX:x.clientX;function n(x){c||(s=!0,a.style.opacity="1",r(x))}function r(x){if(!s||c)return;const w=o.getBoundingClientRect(),E=l(x)-w.left,m=Math.max(0,Math.min(1,E/w.width));a.style.width=m*w.width+"px",m>=.82&&g()}function u(){c||(s=!1,a.style.transition="width .2s ease, opacity .2s ease",a.style.width="0",a.style.opacity="0",setTimeout(()=>{c||(a.style.transition="")},220))}function g(){var w;if(c)return;c=!0,s=!1,a.style.width="100%",a.style.opacity="1",(w=document.getElementById("cut-flash"))==null||w.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const x=document.getElementById("cut-hint");x&&(x.style.opacity="0"),o.classList.add("pack-cut"),setTimeout(()=>{a.style.opacity="0",document.getElementById("pack-phase").style.display="none",f(0)},620)}o.addEventListener("pointerdown",n),window.addEventListener("pointermove",r),window.addEventListener("pointerup",u),o.addEventListener("touchstart",n,{passive:!0}),window.addEventListener("touchmove",r,{passive:!0}),window.addEventListener("touchend",u);function f(x){const w=document.getElementById("reveal-phase");w.style.display="flex",L(x)}function L(x){var Y;const w=e[x],E=document.getElementById("card-counter"),m=document.getElementById("single-card-slot"),T=document.getElementById("card-tap-hint");E&&(E.textContent=`Carte ${x+1} / ${e.length}`),T&&(T.textContent=x<e.length-1?"Appuie pour continuer →":"Appuie pour voir toutes tes cartes");const q=St(w),G=w.card_type==="player"&&((Y=w.player)==null?void 0:Y.rarity)==="legende";m.innerHTML=`
      ...
      </div>`,document.body.appendChild(d),d.addEventListener("click",a=>{a.target===d&&d.remove()}),(i=document.getElementById("odds-close"))==null||i.addEventListener("click",()=>d.remove());return}Hi()}function Hi(){const e=document.createElement("div");e.style.cssText=`position:fixed;inset:0;background:rgba(0,0,0,0.6);display:flex;
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}const Ui="simulation",Vt="VOTRE_ZONE_ID";function Ki(){switch(Ui){case"propellerads":return Vi();case"adsense":return Yi();default:return st()}}function st(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:#111;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:14px";let i=5;t.innerHTML=`
      ...
      <button id="ad-skip" disabled style="margin-top:8px;padding:10px 24px;border-radius:20px;border:none;background:rgba(255,255,255,0.15);color:rgba(255,255,255,0.4);font-size:13px;cursor:default">
        Passer (5)
      </button>`,document.body.appendChild(t);const d=setInterval(()=>{i--;const c=document.getElementById("ad-cd"),o=document.getElementById("ad-skip");c&&(c.textContent=i),o&&(o.textContent=i>0?`Passer (${i})`:"✓ Continuer"),i<=0&&(clearInterval(d),o&&(o.disabled=!1,o.style.cssText=o.style.cssText.replace("rgba(255,255,255,0.15)","#1A6B3C").replace("rgba(255,255,255,0.4)","#fff").replace("default","pointer")),o==null||o.addEventListener("click",()=>{t.remove(),e(!0)},{once:!0}))},1e3)})}function Vi(){return new Promise((e,t)=>{if(typeof window.propeller>"u")return console.warn("[Ad] PropellerAds non chargé → simulation"),e(st());window.propeller.push({zone_id:Vt,onComplete:()=>e(!0),onSkip:()=>t(new Error("Publicité ignorée")),onError:()=>t(new Error("Erreur publicité"))})})}function Yi(){return new Promise((e,t)=>{if(typeof window.adsbygoogle>"u")return console.warn("[Ad] AdSense non chargé → simulation"),e(st());try{(window.adsbygoogle=window.adsbygoogle||[]).push({google_ad_client:"ca-pub-VOTRE_PUBLISHER_ID",google_ad_slot:Vt,google_ad_format:"rewarded",on_reward:()=>e(!0),on_dismiss:()=>t(new Error("Publicité fermée"))})}catch(i){t(i)}})}const be={},$e={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},ke={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function fe(e,t,i,d,c){var o;e.innerHTML=`<div class="match-screen" style="display:flex;align-items:center;justify-content:center;min-height:100vh">
    <div style="text-align:center;padding:40px;color:#fff">
      <div style="font-size:48px;margin-bottom:16px">${t}</div>
      <p style="margin-bottom:16px">${i}</p>
      <button class="btn btn-primary" id="msg-btn">${d}</button>
    </div>
  </div>`,(o=document.getElementById("msg-btn"))==null||o.addEventListener("click",c)}function ne(e){const t=typeof import.meta<"u"?be==null?void 0:be.VITE_SUPABASE_URL:"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function Je(e,t){var d,c;const i=e.player;return{cardId:e.id,position:t||null,id:i.id,firstname:i.firstname,name:i.surname_encoded,country_code:i.country_code,club_id:i.club_id,job:i.job,job2:i.job2,note_g:Number(i.note_g)||0,note_d:Number(i.note_d)||0,note_m:Number(i.note_m)||0,note_a:Number(i.note_a)||0,rarity:i.rarity,skin:i.skin,hair:i.hair,hair_length:i.hair_length,clubName:((d=i.clubs)==null?void 0:d.encoded_name)||null,clubLogo:((c=i.clubs)==null?void 0:c.logo_url)||null,boost:0,used:!1,_line:null,_col:null}}function Pe(e){return e===1?[1]:e===2?[0,2]:e===3?[0,1,2]:e===4?[0,1,1,2]:e===5?[0,1,1,1,2]:[1]}function Yt(){const e=Math.random()*100;return e<10?3:e<30?2:1}function Oe(e,t){const i=$e[t]||$e["4-4-2"],d={GK:[],DEF:[],MIL:[],ATT:[]};if(e.length&&e.every(a=>a.position)){for(const a of["GK","DEF","MIL","ATT"]){const s=e.filter(n=>n.position&&n.position.replace(/\d+$/,"")===a).sort((n,r)=>parseInt(n.position.replace(/\D+/g,""),10)-parseInt(r.position.replace(/\D+/g,""),10)).map(n=>({...n,_line:a})),l=Pe(s.length);s.forEach((n,r)=>{n._col=l[r]}),d[a]=s}return d}const o=[...e];for(const a of["GK","DEF","MIL","ATT"]){const s=[];for(let n=0;n<i[a];n++){let r=o.findIndex(u=>u.job===a);if(r===-1&&(r=o.findIndex(u=>u.job2===a)),r===-1&&(r=0),o[r]){const u={...o[r],_line:a};s.push(u),o.splice(r,1)}}const l=Pe(s.length);s.forEach((n,r)=>{n._col=l[r]}),d[a]=s}return d}function Wt(e){document.querySelectorAll(".top-nav, .bottom-nav").forEach(t=>{t.style.setProperty("display","none","important"),t.dataset.matchHidden="1"}),e&&e.style.setProperty("padding-bottom","0","important")}function le(e){document.querySelectorAll(".top-nav, .bottom-nav").forEach(t=>{t.style.removeProperty("display"),delete t.dataset.matchHidden}),e&&e.style.removeProperty("padding-bottom")}function Xt(e,t,i){const c=new Set,o=t.filter(r=>{const u=r.gc_type||r.id;return c.has(u)?!1:(c.add(u),!0)});let a=[];function s(r,u){const g=r._gcDef,f={purple:"linear-gradient(135deg,#3d0a7a,#7a28b8)",light_blue:"linear-gradient(135deg,#006080,#00bcd4)"},L={purple:"#9b59b6",light_blue:"#00bcd4"},p=f[g==null?void 0:g.color]||f.purple,h=L[g==null?void 0:g.color]||L.purple;return`<div class="gc-select-card" data-id="${r.id}"
    ...
    </div>`,e.querySelectorAll(".gc-select-card").forEach(L=>{L.addEventListener("click",()=>{const p=L.dataset.id,h=o.find(v=>v.id===p);if(!h)return;const I=a.findIndex(v=>v.gc_type===h.gc_type);I>-1?a.splice(I,1):a.length<3&&a.push(h),n()})}),(u=e.querySelector("#gc-launch"))==null||u.addEventListener("click",()=>{r&&l(a)}),(g=e.querySelector("#gc-no-gc"))==null||g.addEventListener("click",()=>l([])),(f=e.querySelector("#gc-reset"))==null||f.addEventListener("click",()=>{a.length&&(a=[],n())})}n()}async function Wi(e,t,i){const{state:d,navigate:c}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await k.from("decks").select("id,name,is_active,formation").eq("owner_id",d.profile.id).order("created_at",{ascending:!1});if(!o||o.length===0){fe(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>c("decks"));return}const a=o.map(r=>r.id),{data:s}=await k.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order");let l=0;function n(){var v,y,x,w,E;const r=o[l],u=(s||[]).filter(m=>m.deck_id===r.id),g=u.filter(m=>{var T;return m.is_starter&&((T=m.card)==null?void 0:T.player)}).map(m=>Je(m.card,m.position)),f=u.find(m=>{var T;return((T=m.card)==null?void 0:T.card_type)==="formation"}),L=r.formation||((v=f==null?void 0:f.card)==null?void 0:v.formation)||"4-4-2",p=g.length>=11?Oe(g,L):null,h=g.length>=11;Wt(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div id="deck-swipe-zone" style="flex:1;min-height:0;overflow:hidden;position:relative;touch-action:pan-y;display:flex;align-items:center;justify-content:center;padding:4px">
        ${p?`<div class="deck-preview-wrap" style="aspect-ratio:1/1;max-width:100%;max-height:100%;width:auto;height:100%;overflow:hidden">${Me(p,L,null,[],285,285)}</div>`:`<div style="display:flex;align-items:center;justify-content:center;height:100%;opacity:.4;flex-direction:column;gap:8px">
  ...
  </div>`}async function Qt(e,t,i,d){var w;const{state:c,navigate:o,toast:a}=t;Wt(e);const s=c.params||{};if(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!s.deckId)return Wi(e,t,i);const l=s.deckId;let n,r,u,g;try{const E=await Promise.all([k.from("decks").select("formation,name").eq("id",l).single(),k.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation,
            player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
              clubs(encoded_name,logo_url)))`).eq("deck_id",l).order("slot_order")]);n=E[0].data,u=E[0].error,r=E[1].data,g=E[1].error}catch(E){console.error("[Match] Exception chargement deck:",E),fe(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>o("home"));return}if(u||g){console.error("[Match] Erreur Supabase:",u||g),fe(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>o("home"));return}const f=(r||[]).filter(E=>{var m;return E.is_starter&&((m=E.card)==null?void 0:m.player)}).map(E=>Je(E.card,E.position)),L=(r||[]).filter(E=>{var m;return!E.is_starter&&((m=E.card)==null?void 0:m.player)}).map(E=>Je(E.card,E.position));if(f.length<11){fe(e,"⚠️",`Deck incomplet (${f.length}/11).`,"Compléter",()=>o("decks"));return}const p=(r||[]).find(E=>{var m;return((m=E.card)==null?void 0:m.card_type)==="formation"}),h=(n==null?void 0:n.formation)||((w=p==null?void 0:p.card)==null?void 0:w.formation)||"4-4-2",{data:I,error:v}=await k.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",c.profile.id).eq("card_type","game_changer"),{data:y}=await k.from("gc_definitions").select("*").eq("is_active",!0),x=(I||[]).map(E=>({...E,_gcDef:(y==null?void 0:y.find(m=>m.name===E.gc_type||m.id===E.gc_definition_id))||null}));d({deckId:l,formation:h,starters:f,subsRaw:L,gcCardsEnriched:x,gcDefs:y||[]})}async function Xi(e,t){const{state:i}=t,c=(i.params||{}).matchMode||"vs_ai_easy",o=c.replace("vs_ai_",""),a=c;await Qt(e,t,c,async({deckId:s,formation:l,starters:n,subsRaw:r,gcCardsEnriched:u,gcDefs:g})=>{try{const f=Oe(n,l),L=await Ji(l,o),p=async h=>{try{const{data:I,error:v}=await k.from("matches").insert({home_id:i.profile.id,away_id:null,mode:a,home_deck_id:s,status:"in_progress"}).select().single();if(v){console.error("[MatchIA] Erreur création match:",v),fe(e,"⚠️","Impossible de créer le match ("+v.message+").","Retour",()=>t.navigate("home"));return}const y={gcDefs:g||[],matchId:I==null?void 0:I.id,mode:a,difficulty:o,formation:l,homeTeam:f,aiTeam:L,homeSubs:r,subsUsed:0,maxSubs:Math.min(r.length,3),homeScore:0,aiScore:0,gcCards:h,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Qi(e,y,t)}catch(I){console.error("[MatchIA] Exception launchMatch:",I),fe(e,"⚠️","Erreur au lancement du match : "+I.message,"Retour",()=>t.navigate("home"))}};if(!u.length){p([]);return}Xt(e,u,p)}catch(f){console.error("[MatchIA] Exception setup:",f),fe(e,"⚠️","Erreur de préparation du match : "+f.message,"Retour",()=>t.navigate("home"))}})}async function Ji(e,t){var a,s;const{data:i}=await k.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(60);if(!i||i.length<11)return Zi(e);const d=$e[e]||$e["4-4-2"],c={GK:[],DEF:[],MIL:[],ATT:[]},o=[...i];for(const l of["GK","DEF","MIL","ATT"]){const n=o.filter(L=>L.job===l),r=o.filter(L=>L.job!==l),u=[...n,...r],g=[];for(let L=0;L<d[l];L++){const p=u[L]||o[L];p&&g.push({cardId:"ai-"+p.id+"-"+L,id:p.id,firstname:p.firstname,name:p.surname_encoded,country_code:p.country_code,club_id:p.club_id,job:p.job,job2:p.job2,note_g:Number(p.note_g)||0,note_d:Number(p.note_d)||0,note_m:Number(p.note_m)||0,note_a:Number(p.note_a)||0,rarity:p.rarity,skin:p.skin,hair:p.hair,hair_length:p.hair_length,clubName:((a=p.clubs)==null?void 0:a.encoded_name)||null,clubLogo:((s=p.clubs)==null?void 0:s.logo_url)||null,boost:0,used:!1,_line:l})}const f=Pe(g.length);g.forEach((L,p)=>{L._col=f[p]}),c[l]=g}return c}function Zi(e){const t=$e[e]||$e["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},d=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let c=0;for(const o of["GK","DEF","MIL","ATT"]){const a=[];for(let l=0;l<t[o];l++){const n=3+Math.floor(Math.random()*5);a.push({cardId:"fake-"+c,id:"fake-"+c,firstname:"IA",name:d[c%d.length],country_code:"XX",club_id:null,job:o,job2:null,note_g:o==="GK"?n:2,note_d:o==="DEF"?n:2,note_m:o==="MIL"?n:2,note_a:o==="ATT"?n:2,rarity:"normal",boost:0,used:!1,_line:o}),c++}const s=Pe(a.length);a.forEach((l,n)=>{l._col=s[n]}),i[o]=a}return i}function Qi(e,t,i){e.innerHTML=`
  ...
  </div>`,setTimeout(()=>eo(e,t,i),5e3)}function eo(e,t,i){const d=t.homeTeam.MIL||[],c=t.aiTeam.MIL||[];function o(p){return p.reduce((h,I)=>h+ie(I,"MIL"),0)}function a(p){let h=0;for(let I=0;I<p.length-1;I++){const v=ge(p[I],p[I+1]);v==="#00ff88"?h+=2:v==="#FFD700"&&(h+=1)}return h}const s=o(d)+a(d),l=o(c)+a(c),n=s>=l;function r(p,h,I,v){return`<div id="duel-row-${v}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0), opacity .5s ease;transform-origin:center">
  ...
  <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div id="duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center, rgba(10,61,30,0.4), rgba(10,61,30,0.92))"></div>
          ...
          <button id="start-match-btn" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,0.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">
            ▶ Commencer le match
          </button>`,w.style.transition="opacity .45s ease",w.style.opacity="1",w.style.pointerEvents="auto",(E=document.getElementById("start-match-btn"))==null||E.addEventListener("click",L))},600)},700)},2800)}function to(e){if(e.type==="duel"){const t=e.homeTotal>=e.aiTotal;return`
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:100px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const T=t.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
            ...
            ${Ie((T.players||[]).map(q=>({...q,used:!1})),"#ff6b6b",T.total)}
            ...
            ${Ie((T.players||[]).map(q=>({...q,used:!1})),"#00ff88",T.total)}
    ...
    ${(()=>{const m=window.innerWidth>=700,T=(z,B,P)=>{var V,X;const S=(t.gcDefs||[]).find(te=>te.name===z.gc_type),$={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[S==null?void 0:S.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",b={purple:"#b06ce0",light_blue:"#00d4ef"}[S==null?void 0:S.color]||"#b06ce0",j=(S==null?void 0:S.name)||z.gc_type,A=(S==null?void 0:S.effect)||((V=de[z.gc_type])==null?void 0:V.desc)||"",_=S!=null&&S.image_url?`/manager-wars/icons/${S.image_url}`:null,D=((X=de[z.gc_type])==null?void 0:X.icon)||"⚡",M=Math.round(P*.22),R=Math.round(P*.22),F=P-M-R,K=j.length>12?7:9;return`<div class="gc-mini" data-gc-id="${z.id}" data-gc-type="${z.gc_type}"
          ...
          <div style="height:${M}px;background:rgba(255,255,255,0.14);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 3px">
            <span style="font-size:${K}px;font-weight:900;color:#fff;text-align:center;line-height:1.1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${B-6}px">${j}</span>
        ...
        <div class="terrain-wrapper" style="overflow:hidden;flex-shrink:0;display:flex;align-items:center;justify-content:center">
          ${Me(t.homeTeam,t.formation,t.phase,d,300,300)}
        </div>
      </div>`;return m?`
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(m=>{var T,q,G;if(m.type==="duel"){const N=m.isGoal,O=m.homeScored?"#FFD700":N?"#ff6b6b":"rgba(255,255,255,0.3)",Y=m.homeScored?"⚽ BUT !":N?"⚽ BUT IA !":(T=m.homePlayers)!=null&&T.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${N?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${O};margin-bottom:4px">
                ...
                ${(q=m.homePlayers)!=null&&q.length?`<div style="margin-bottom:3px">${Ie(m.homePlayers,"rgba(255,255,255,0.7)",m.homeTotal)}</div>`:""}
                ${(G=m.aiPlayers)!=null&&G.length?`<div style="opacity:0.7">${Ie(m.aiPlayers,"#ff6b6b",m.aiTotal)}</div>`:""}
  ...
  `,document.body.appendChild(d);let r=!1;const u=()=>{r||(r=!0,d.remove(),i())};d.addEventListener("click",u),setTimeout(u,2e3)}function Ae(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function jt(e,t,i,d=null){if(t.phase!=="attack"){Ae("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){Ae(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const c=Object.values(t.homeTeam).flat().filter(u=>u.used),o=t.homeSubs.filter(u=>!t.usedSubIds.includes(u.cardId));if(!c.length){Ae("Aucun joueur utilisé à remplacer");return}if(!o.length){Ae("Aucun remplaçant disponible");return}let a=0,s=Math.max(0,o.findIndex(u=>u.cardId===d)),l=!1;const n=document.createElement("div");n.id="sub-overlay",n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function r(){var I,v,y,x,w,E;const u=c[a],g=o[s],f=Math.min(130,Math.round((window.innerWidth-90)/2)),L=Math.round(f*1.35),p=m=>`background:rgba(255,255,255,0.12);border:none;color:${m?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${m?"default":"pointer"};flex-shrink:0`;n.innerHTML=`
    ...
    </div>`,document.body.appendChild(f),(L=f.querySelector("#gc-back"))==null||L.addEventListener("click",()=>f.remove()),(p=f.querySelector("#gc-use"))==null||p.addEventListener("click",()=>{f.remove(),lo(e,t,i,d,c)})}function lo(e,t,i,d,c){if(d.usedGc.includes(e))return;d.usedGc.push(e);const o=(d.gcDefs||[]).find(s=>s.name===t);let a=!1;if(o!=null&&o.effect_type&&o.effect_type!=="CUSTOM"){const s=GC_ENGINE[o.effect_type];s?s(o.effect_params||{},d,i,c)||(a=!0):(c.toast(`Effet "${o.effect_type}" non implémenté`,"error"),a=!0)}else{switch(t){case"Double attaque":d.modifiers.home.doubleAttack=!0,d.log.push({text:"⚡ Double attaque activée !",type:"info"});break;case"Bouclier":d.modifiers.home.shield=!0,d.log.push({text:"🛡️ Bouclier activé !",type:"info"});break;case"Ressusciter":{const s=Object.entries(d.homeTeam).flatMap(([l,n])=>(n||[]).filter(r=>r.used).map(r=>({...r,_line:l})));s.length?(s[0].used=!1,d.log.push({text:`💫 ${s[0].name} ressuscité !`,type:"info"})):d.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"});break}case"Vol de note":d.modifiers.ai.stolenNote=(d.modifiers.ai.stolenNote||0)+1,d.log.push({text:"🎯 -1 à la prochaine attaque IA",type:"info"});break;case"Gel":{const s=[...d.aiTeam.ATT||[],...d.aiTeam.MIL||[]].filter(l=>!l.used);if(s.length){const l=s.sort((n,r)=>ie(r,"ATT")-ie(n,"ATT"))[0];l.used=!0,d.log.push({text:`❄️ ${l.name} (IA) gelé !`,type:"info"})}break}case"Remplacement+":d.maxSubs++,d.log.push({text:"🔄 +1 remplacement débloqué",type:"info"});break}a=!0}k.from("cards").delete().eq("id",e).then(()=>{}),a&&oe(i,d,c)}function co(e,t,i){const d=Object.values(t.homeTeam).flat().filter(c=>!c.used);if(!d.length){i.toast("Aucun joueur actif à booster","error");return}i.openModal("⚡ Utiliser le Boost",`<div style="margin-bottom:12px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border-radius:10px;padding:12px;text-align:center;color:#000">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(c=>{c.addEventListener("click",()=>{const o=c.dataset.cardId;for(const a of["GK","DEF","MIL","ATT"]){const s=(t.homeTeam[a]||[]).find(l=>l.cardId===o);if(s){s.boost=(s.boost||0)+t.boostCard.value,t.log.push({text:`⚡ Boost +${t.boostCard.value} appliqué à ${s.name}`,type:"info"});break}}t.boostUsed=!0,i.closeModal(),oe(e,t,i)})})}function lt(e,t,i,d,c){const o=document.createElement("div");o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:900;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;overflow:hidden;cursor:pointer";const a=Array.from({length:10},(n,r)=>`
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(o);let s=!1;const l=()=>{s||(s=!0,o.remove(),setTimeout(()=>c(),50))};o.addEventListener("click",l),setTimeout(l,3500)}async function we(e,t,i){var r,u;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:d}=i,c=t.homeScore>t.aiScore,o=t.homeScore===t.aiScore,a=c?"victoire":o?"nul":"defaite",s=fi(t.mode,a);t.matchId&&await k.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:c?d.profile.id:null,home_credits_reward:s,played_at:new Date().toISOString()}).eq("id",t.matchId);const l={credits:(d.profile.credits||0)+s,matches_played:(d.profile.matches_played||0)+1};c?l.wins=(d.profile.wins||0)+1:o?l.draws=(d.profile.draws||0)+1:l.losses=(d.profile.losses||0)+1,await k.from("users").update(l).eq("id",d.profile.id),await i.refreshProfile();const n=document.createElement("div");n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",n.innerHTML=`
    ...
    </div>`,`<button class="btn btn-primary" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}async function uo(e,t){const{state:i}=t;await Qt(e,t,"random",({deckId:c,formation:o,starters:a,subsRaw:s,gcCardsEnriched:l,gcDefs:n})=>{const r=u=>{fo(e,t,c,o,a,s,u,n||[])};if(!l.length){r([]);return}Xt(e,l,r)})}async function fo(e,t,i,d,c,o,a=[],s=[]){var v;const{state:l,navigate:n,toast:r}=t;let u=!1,g=null;e.style.overflow="hidden",e.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const f=()=>document.getElementById("mm-status"),L=async(y=!0)=>{u=!0,g&&(k.removeChannel(g),g=null),y&&await k.rpc("cancel_matchmaking",{p_user_id:l.profile.id}).catch(()=>{})};(v=document.getElementById("mm-cancel"))==null||v.addEventListener("click",async()=>{try{await L(!0)}catch(y){console.warn("[Matchmaking] cleanup error:",y)}le(e),n("home")});const p=async(y,x,w)=>{u||(u=!0,g&&(k.removeChannel(g),g=null),f()&&(f().textContent="Adversaire trouvé !"),await new Promise(E=>setTimeout(E,600)),e.isConnected!==!1&&go(e,t,y,w,a,s))},{data:h,error:I}=await k.rpc("try_matchmake",{p_user_id:l.profile.id,p_deck_id:i});if(I||!(h!=null&&h.success)){r("Erreur de matchmaking","error"),le(e),n("home");return}if(h.matched){p(h.match_id,h.opponent_id,!1);return}f()&&(f().textContent="En attente d'un autre joueur..."),g=k.channel("matchmaking-"+l.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${l.profile.id}`},y=>{const x=y.new;x.status==="matched"&&x.match_id&&p(x.match_id,x.matched_with,!0)}).subscribe()}async function go(e,t,i,d,c=[],o=[]){const{state:a,navigate:s,toast:l}=t,n=d?"p1":"p2",r=d?"p2":"p1",u=(c||[]).map(C=>C.id),g=(c||[]).map(C=>({id:C.id,gc_type:C.gc_type,_gcDef:C._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation du match...</div>';const{data:f}=await k.from("matches").select("*").eq("id",i).single();if(!f){l("Match introuvable","error"),s("home");return}async function L(){const[{data:C,error:z},{data:B,error:P},{data:S},{data:$}]=await Promise.all([k.rpc("get_deck_for_match",{p_deck_id:f.home_deck_id}),k.rpc("get_deck_for_match",{p_deck_id:f.away_deck_id}),k.from("users").select("id,pseudo,club_name").eq("id",f.home_id).single(),k.from("users").select("id,pseudo,club_name").eq("id",f.away_id).single()]);z&&console.error("[PvP] get_deck_for_match p1 error:",z),P&&console.error("[PvP] get_deck_for_match p2 error:",P);function b(F,K){return{cardId:F.card_id,position:K||F.position,id:F.id,firstname:F.firstname,name:F.surname_encoded,country_code:F.country_code,club_id:F.club_id,job:F.job,job2:F.job2,note_g:Number(F.note_g)||0,note_d:Number(F.note_d)||0,note_m:Number(F.note_m)||0,note_a:Number(F.note_a)||0,rarity:F.rarity,skin:F.skin,hair:F.hair,hair_length:F.hair_length,clubName:F.club_encoded_name||null,clubLogo:F.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}}const j=((C==null?void 0:C.starters)||[]).map(F=>b(F,F.position)),A=((B==null?void 0:B.starters)||[]).map(F=>b(F,F.position)),_=((C==null?void 0:C.subs)||[]).map(F=>b(F,F.position)),D=((B==null?void 0:B.subs)||[]).map(F=>b(F,F.position)),M=(C==null?void 0:C.formation)||"4-4-2",R=(B==null?void 0:B.formation)||"4-4-2";return console.log("[PvP] p1Formation:",M,"p1 starters:",j.length,"positions:",j.map(F=>F.position)),console.log("[PvP] p2Formation:",R,"p2 starters:",A.length,"positions:",A.map(F=>F.position)),{p1Team:Oe(j,M),p2Team:Oe(A,R),p1Subs:_,p2Subs:D,p1Formation:M,p2Formation:R,p1Name:(S==null?void 0:S.club_name)||(S==null?void 0:S.pseudo)||"Joueur 1",p2Name:($==null?void 0:$.club_name)||($==null?void 0:$.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:d?u:[],gc_p2:d?[]:u,gcCardsFull_p1:d?g:[],gcCardsFull_p2:d?[]:g,gcReady_p1:!0,gcReady_p2:!0,usedGc_p1:[],usedGc_p2:[],lastActionAt:new Date().toISOString()}}let p=f.game_state&&Object.keys(f.game_state).length?f.game_state:null;if(!p)if(d){p=await L();const{data:C}=await k.from("matches").select("game_state").eq("id",i).single();!(C!=null&&C.game_state)||!Object.keys(C.game_state).length?await k.from("matches").update({game_state:p,turn_user_id:f.home_id}).eq("id",i):p=C.game_state}else{e.innerHTML=`<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation avec l'adversaire...</div>`;for(let C=0;C<30&&!p;C++){await new Promise(B=>setTimeout(B,400));const{data:z}=await k.from("matches").select("game_state").eq("id",i).single();z!=null&&z.game_state&&Object.keys(z.game_state).length&&(p=z.game_state)}if(!p){l("Erreur de synchronisation","error"),s("home");return}p.gc_p2=u,p.gcCardsFull_p2=g,await k.from("matches").update({game_state:p}).eq("id",i)}let h=!1;function I(C){var j;try{k.removeChannel(v)}catch{}const z=a.profile.id,B=C.winner_id===z,P=!!C.forfeit,S=p[n+"Score"]??0,$=p[r+"Score"]??0,b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center",b.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(b),(j=b.querySelector("#pvp-end-home"))==null||j.addEventListener("click",()=>{b.remove(),le(e),s("home")})}const v=k.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},C=>{const z=C.new;try{if(z.status==="finished"||z.forfeit){if(h)return;h=!0,p._timerInt&&(clearInterval(p._timerInt),p._timerInt=null),z.game_state&&(p=z.game_state),I(z);return}z.game_state&&(p=z.game_state,w())}catch(B){console.error("[PvP] Realtime render crash:",B,"gameState:",p)}}).subscribe();async function y(C){Object.assign(p,C),p.lastActionAt=new Date().toISOString();const{error:z}=await k.from("matches").update({game_state:p}).eq("id",i);z&&(console.error("[PvP] pushState DB error:",z.message,z),l("Erreur de synchronisation : "+z.message,"error"));try{w()}catch(B){console.error("[PvP] renderPvpScreen crash après pushState:",B),l("Erreur d'affichage : "+B.message,"error")}}async function x(){if(h)return;h=!0,p._timerInt&&(clearInterval(p._timerInt),p._timerInt=null);const C=d?f.away_id:f.home_id;try{await k.from("matches").update({status:"finished",forfeit:!0,winner_id:C}).eq("id",i)}catch(z){console.warn("[PvP] forfeit DB error:",z)}try{k.removeChannel(v)}catch{}le(e),s("home")}function w(){var gt,mt,xt,yt,ht;const C=p[n+"Team"];p[r+"Team"];const z=p[n+"Score"],B=p[r+"Score"],P=p[n+"Name"],S=p[r+"Name"];if(p.phase==="gc-select")return E();if(p.phase==="reveal")return m();if(p.phase==="midfield")return T();if(p.phase==="finished")return Y();const $=p.phase===n+"-attack",b=p.phase===n+"-defense",j=p.phase===r+"-attack"||p.phase===r+"-defense",A=Array.isArray(p["selected_"+n])?p["selected_"+n]:[],_=A.map(U=>U.cardId);e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column";const D=window.innerWidth>=700,M=p[n+"Subs"]||[],R=p["usedSubIds_"+n]||[],F=M.filter(U=>!R.includes(U.cardId)),K=p["gcCardsFull_"+n]||[],V=p["usedGc_"+n]||[],X=K.filter(U=>!V.includes(U.id)),te=p.boostOwner===n&&!p.boostUsed,re=(U,J,ee)=>{var _t,$t;const W=U._gcDef,ce={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[W==null?void 0:W.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",je={purple:"#b06ce0",light_blue:"#00d4ef"}[W==null?void 0:W.color]||"#b06ce0",ye=(W==null?void 0:W.name)||U.gc_type,ri=(W==null?void 0:W.effect)||((_t=de[U.gc_type])==null?void 0:_t.desc)||"",vt=W!=null&&W.image_url?`/manager-wars/icons/${W.image_url}`:null,ai=(($t=de[U.gc_type])==null?void 0:$t.icon)||"⚡",bt=Math.round(ee*.22),wt=Math.round(ee*.22),Ve=ee-bt-wt,si=ye.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${U.id}" data-gc-type="${U.gc_type}"
        style="width:${J}px;height:${ee}px;border-radius:10px;border:2px solid ${je};background:${ce};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
      ...
      <div class="terrain-wrapper" style="width:100%;height:100%;overflow:hidden">
        ${Me(C,p[n+"Formation"],$?"attack":b?"defense":"idle",_,300,300)}
          ...
          ${Ie(U.players.map(ce=>({...ce,used:!1})),"#ff6b6b",U.total)}
      ...
      </div>`;const U=e.querySelector(".terrain-wrapper");U&&(U.innerHTML=Me(C,p[n+"Formation"],$?"attack":b?"defense":"idle",_,300,300))}if(e.querySelectorAll(".match-slot-hit").forEach(U=>{U.addEventListener("click",()=>{if(!$&&!b)return;const J=U.dataset.cardId,ee=U.dataset.role,W=(C[ee]||[]).find(ye=>ye.cardId===J);if(!W||W.used)return;Array.isArray(p["selected_"+n])||(p["selected_"+n]=[]);const ce=p["selected_"+n],je=ce.findIndex(ye=>ye.cardId===J);je>-1?ce.splice(je,1):ce.length<3&&ce.push({...W,_role:ee}),w()})}),(gt=e.querySelector("#pvp-boost-card"))==null||gt.addEventListener("click",()=>{q()}),e.querySelectorAll(".pvp-gc-mini").forEach(U=>{U.addEventListener("click",()=>{O(U.dataset.gcId,U.dataset.gcType)})}),(mt=e.querySelector("#pvp-action"))==null||mt.addEventListener("click",()=>{$?H():b&&Q()}),(xt=e.querySelector("#pvp-quit"))==null||xt.addEventListener("click",()=>{confirm("Quitter le match ? Vous perdrez par forfait.")&&x()}),(yt=e.querySelector("#pvp-view-opp"))==null||yt.addEventListener("click",()=>{N()}),(ht=e.querySelector("#pvp-toggle-history"))==null||ht.addEventListener("click",()=>{G()}),function(){const J=e.querySelector(".terrain-wrapper svg");J&&(J.removeAttribute("width"),J.removeAttribute("height"),J.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",J.setAttribute("viewBox","-26 -26 352 352"),J.setAttribute("preserveAspectRatio","xMidYMid meet"));const ee=e.querySelector(".match-screen");if(ee){const W=e.clientHeight;W>50&&(ee.style.height=W+"px",ee.style.maxHeight=W+"px",ee.style.overflow="hidden")}}(),p._timerInt&&(clearInterval(p._timerInt),p._timerInt=null),($||b)&&!h){let U=30,J=!1;const ee=()=>document.getElementById("pvp-timer"),W=()=>{ee()&&(ee().textContent=U+"s",ee().style.color=J?"#ff4444":"#fff")};W(),p._timerInt=setInterval(()=>{U--,U<0?J?(clearInterval(p._timerInt),p._timerInt=null,x()):(J=!0,U=15,W()):W()},1e3)}}function E(){var b;const C=p["gcReady_"+n],z=p["gcReady_"+r];Array.isArray(p["gc_"+n])||(p["gc_"+n]=[]);let B=p["gc_"+n];const P=3;function S(j,A){const _=j._gcDef,D={purple:"linear-gradient(135deg,#3d0a7a,#7a28b8)",light_blue:"linear-gradient(135deg,#006080,#00bcd4)"},M={purple:"#9b59b6",light_blue:"#00bcd4"},R=D[_==null?void 0:_.color]||D.purple,F=M[_==null?void 0:_.color]||M.purple;return`<div class="pvp-gc-card" data-id="${j.id}"
    ...
    </div>`,C)return;const $=n==="p1"?f.home_id:f.away_id;k.from("cards").select("id,gc_type").eq("owner_id",$).eq("card_type","game_changer").then(async({data:j})=>{const{data:A}=await k.from("gc_definitions").select("*").eq("is_active",!0),_=new Set,D=(j||[]).filter(R=>_.has(R.gc_type)?!1:(_.add(R.gc_type),!0)).map(R=>({...R,_gcDef:(A==null?void 0:A.find(F=>F.name===R.gc_type))||null})),M=document.getElementById("pvp-gc-grid");if(M){if(!D.length){M.innerHTML='<div style="color:rgba(255,255,255,0.4);font-size:13px;text-align:center;margin-top:30px">Aucune carte Game Changer disponible.</div>';return}M.innerHTML=D.map(R=>S(R,B.includes(R.id))).join(""),M.querySelectorAll(".pvp-gc-card").forEach(R=>{R.addEventListener("click",()=>{const F=R.dataset.id,K=B.indexOf(F);K>-1?B.splice(K,1):B.length<P&&B.push(F),w()})})}}),(b=e.querySelector("#pvp-gc-validate"))==null||b.addEventListener("click",async()=>{const j=n==="p1"?f.home_id:f.away_id,{data:A}=await k.from("cards").select("id,gc_type").eq("owner_id",j).eq("card_type","game_changer").in("id",B),{data:_}=await k.from("gc_definitions").select("*").eq("is_active",!0),D=(A||[]).map(M=>({...M,_gcDef:(_==null?void 0:_.find(R=>R.name===M.gc_type))||null}));await y({["gcReady_"+n]:!0,["gcCardsFull_"+n]:D}),p["gcReady_"+r]&&await y({phase:"reveal"})})}function m(){e.style.overflow="hidden",e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    </div>`,setTimeout(()=>{const A=document.getElementById("pvp-score-me"),_=document.getElementById("pvp-score-opp"),D=document.getElementById("pvp-midfield-result");A&&_&&(b?(A.style.fontSize="80px",A.style.color="#FFD700",_.style.opacity="0.25"):(_.style.fontSize="80px",_.style.color="#ff6b6b",A.style.opacity="0.25"));const M=p.p1Team.MIL||[],R=p.p2Team.MIL||[],F=B(M)+P(M),K=B(R)+P(R),V=F>=K?"p1":"p2";p[V+"Name"];let X=p.boostValue;X==null&&(X=Yt(),p.boostValue=X,p.boostOwner=V,p.boostUsed=!1),D&&(D.style.opacity="1",D.innerHTML=`<div style="font-size:20px;font-weight:900;margin-bottom:10px">
      ...
      </div>`,document.body.appendChild(R),(F=R.querySelector("#pvp-gc-back"))==null||F.addEventListener("click",()=>R.remove()),(K=R.querySelector("#pvp-gc-use"))==null||K.addEventListener("click",async()=>{R.remove();const V=p["usedGc_"+n]||[];V.push(C),await y({["usedGc_"+n]:V})})}function Y(){var S;const C=p[n+"Score"],z=p[r+"Score"],B=C>z,P=C===z;e.style.overflow="hidden",e.innerHTML=`
    ...
    </div>`,(S=document.getElementById("pvp-home"))==null||S.addEventListener("click",()=>{k.removeChannel(v),le(e),s("home")})}async function H(){const C=(p["selected_"+n]||[]).map(P=>({...(p[n+"Team"][P._role]||[]).find($=>$.cardId===P.cardId)||P,_line:P._role}));if(!C.length)return;const z=ot(C,p.modifiers[n]||{});C.forEach(P=>{const S=(p[n+"Team"][P._role]||[]).find($=>$.cardId===P.cardId);S&&(S.used=!0)});const B=Array.isArray(p.log)?p.log:[];B.push({type:"attack",text:`⚔️ ${p[n+"Name"]} attaque (${z.total})`,players:C,total:z.total,side:n}),await y({pendingAttack:{...z,players:C,side:n},["selected_"+n]:[],modifiers:{...p.modifiers,[n]:{}},phase:r+"-defense",log:B})}async function Q(){const C=(p["selected_"+n]||[]).map(D=>({...(p[n+"Team"][D._role]||[]).find(R=>R.cardId===D.cardId)||D,_line:D._role})),z=nt(C,p.modifiers[n]||{});C.forEach(D=>{const M=(p[n+"Team"][D._role]||[]).find(R=>R.cardId===D.cardId);M&&(M.used=!0)});const B=rt(p.pendingAttack.total,z.total,p.modifiers[n]||{}),P=p.pendingAttack.side,S=B==="attack",$=p[P+"Score"]+(S?1:0),b=P==="p1"?"p2":"p1",j=(p.round||0)+1,A=j>10,_=Array.isArray(p.log)?p.log:[];_.push({type:"defense",text:`🛡️ ${p[n+"Name"]} défend (${z.total})`,players:C,total:z.total,side:n}),_.push({type:S?"goal":"stop",text:S?`⚽ BUT de ${p[P+"Name"]} ! (${p.pendingAttack.total} vs ${z.total})`:`✋ Attaque stoppée (${p.pendingAttack.total} vs ${z.total})`}),await y({[P+"Score"]:$,["selected_"+n]:[],modifiers:{...p.modifiers,[n]:{}},pendingAttack:null,phase:A?"finished":b+"-attack",attacker:b,round:j,log:_}),A&&await k.from("matches").update({status:"finished"}).eq("id",i)}w()}const mo={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"};async function xo(e,t){const{state:i,toast:d}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement du marché...</div>',await ct(e,t)}async function ct(e,t){const{state:i,toast:d}=t,{data:c}=await k.from("market_listings").select(`id, price, status, listed_at, seller_id,
  ...
  `;function l(n){const r=document.getElementById("mkt-content"),u=n==="buy"?a:s;if(u.length===0){r.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      ${g.map(f=>{var y,x,w,E;const L=(y=f.card)==null?void 0:y.player;if(!L)return"";const p=L.job==="GK"?L.note_g:L.job==="DEF"?L.note_d:L.job==="MIL"?L.note_m:L.note_a,h=mo[L.rarity],I=(i.profile.credits||0)>=f.price,v=f.status==="sold";return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${v?"opacity:0.65":""}">
          <div style="width:44px;height:44px;border-radius:8px;background:${vo(L.job)};color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:18px;border:2px solid ${h};flex-shrink:0">${p}</div>
          <div style="flex:1;min-width:0">
  ...
  `}async function wo(e,{state:t,navigate:i,toast:d}){const c=t.profile;if(!c)return;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await k.from("matches").select(`id,home_id,away_id,home_score,away_score,status,mode,winner_id,created_at,played_at,
