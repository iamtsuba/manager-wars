...
import{s as v,t as Ho,u as Uo,n as ut,g as Bt,p as nt,v as si,o as Ut,r as ze,k as zt,w as Gi,x as Ft,y as Ii,F as Ai,z as Ko,T as Di,q as Tn,C as In,d as An}from"./special-cards-DNsLLa5x.js";const zn="/";function _o(t,{navigate:e,toast:i}){let o="login";const r=()=>{var a,d,c,s,l,p,x;const n=o==="login";if(t.innerHTML=`
    <div style="
  ...
  <div class="page">
    ...
    <div class="page-body">
      <div class="booster-grid">
        ${n.map(b=>{const f=b.cost===0||r>=b.cost,y=l[b.id]===!0;return`<div class="booster-card ${!f||y?"disabled":""}" data-booster="${b.id}" style="position:relative">
  ...
  `,document.body.appendChild(r);let n=!1;const a=document.getElementById("pack-cut-zone"),d=document.getElementById("pack-blade");let c=!1;const s=_=>_.touches&&_.touches[0]?_.touches[0].clientX:_.clientX;function l(_){n||(c=!0,d.style.opacity="1",p(_))}function p(_){if(!c||n)return;const G=a.getBoundingClientRect(),O=s(_)-G.left,le=Math.max(0,Math.min(1,O/G.width));d.style.width=le*G.width+"px",le>=.82&&g()}function x(){n||(c=!1,d.style.transition="width .2s ease, opacity .2s ease",d.style.width="0",d.style.opacity="0",setTimeout(()=>{n||(d.style.transition="")},220))}function g(){var G;if(n)return;n=!0,c=!1,d.style.width="100%",d.style.opacity="1",(G=document.getElementById("cut-flash"))==null||G.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const _=document.getElementById("cut-hint");_&&(_.style.opacity="0"),a.classList.add("pack-cut"),setTimeout(()=>{d.style.opacity="0",document.getElementById("pack-phase").style.display="none",u(0)},620)}a.addEventListener("pointerdown",l),window.addEventListener("pointermove",p),window.addEventListener("pointerup",x),a.addEventListener("touchstart",l,{passive:!0}),window.addEventListener("touchmove",p,{passive:!0}),window.addEventListener("touchend",x);let b=0,f=!1;const y=new Set;function u(_){b=_,document.getElementById("reveal-phase").style.display="flex",w(),h(_,0),I()}function w(){const _=document.getElementById("card-dots");_&&(_.innerHTML=t.map((G,O)=>`<div class="card-dot" data-i="${O}" style="width:8px;height:8px;border-radius:50%;background:${O===b?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),_.querySelectorAll(".card-dot").forEach(G=>G.addEventListener("click",()=>m(parseInt(G.dataset.i)))))}function h(_,G){var de,q;const O=t[_],le=document.getElementById("card-counter"),pe=document.getElementById("card-track");le&&(le.textContent=`Carte ${_+1} / ${t.length}`);const M=document.getElementById("reveal-btns");M&&(M.style.display=_===t.length-1?"flex":"none");const H=O.card_type==="player"&&((de=O.player)==null?void 0:de.rarity)==="legende",fe=!y.has(_);y.add(_);let be=0;if(O.card_type==="player"&&O.player){const W=O.player,ee=W.job||"ATT";be=(Number(ee==="GK"?W.note_g:ee==="DEF"?W.note_d:ee==="MIL"?W.note_m:W.note_a)||0)+(O.evolution_bonus||0)}const ge=W=>{pe.innerHTML=`
        <div id="current-card-wrap" style="position:relative;display:flex;flex-direction:column;align-items:center;gap:8px;${H?"filter:drop-shadow(0 0 20px #7a28b8)":""}">
          <div style="transform:scale(1.25);transform-origin:center">${Vn(O)}</div>
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
  ...
  </div>`;const l=async()=>{await v.from("users").update({pending_boosters:a}).eq("id",e.user.id)};async function p(){var w;if(s>=c||!a.length){await v.from("users").update({pending_boosters:[],onboarding_done:!0}).eq("id",e.user.id),r&&await r(),o("Tous tes boosters sont ouverts ! Bon jeu 🎮","success",4e3),i("home");return}const b=a[0],{data:f}=await v.from("users").select("*").eq("id",e.user.id).single();f&&(e.profile=f);let y=[];try{if(b.type==="formation")y=await ro(e.profile,0);else if(b.type==="game_changer")y=await no(e.profile,b.count||3,0);else if(b.type==="stadium")y=await Kn(e.profile,0);else if(d&&((w=d.rates)!=null&&w.length)){const h={...d,cost:0,cardCount:b.count||d.cardCount||5};y=await _i(e.profile,h),b.guaranteeGK&&!e.profile.first_booster_opened&&(y.some(m=>m.player&&m.player.job==="GK")||await Qn(e.profile,y),await v.from("users").update({first_booster_opened:!0}).eq("id",e.profile.id))}else y=await oo(e.profile,b.count||5,0)}catch(h){o(h.message||"Erreur ouverture booster","error");return}a.shift(),s++,await l();const u=b.type==="formation"?{name:"Booster Formation",img:"/icons/booster-formation.png"}:b.type==="game_changer"?{name:"Booster Game Changer",img:"/icons/booster-gamechanger.png"}:b.type==="stadium"?{name:"Booster Stade",img:"/icons/booster-stadium.png"}:{name:`Booster Joueurs (${s}/${c})`,img:(d==null?void 0:d.img)||"/icons/booster-players.png"};ao(y,u,i,()=>{p()})}(g=document.getElementById("onboard-start"))==null||g.addEventListener("click",()=>p())}async function Qn(t,e){try{const{data:i}=await v.from("players").select("id,job,firstname,surname_real,country_code,club_id,rarity,note_g,note_d,note_m,note_a,skin,hair,hair_length,face,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0).eq("job","GK");if(!(i!=null&&i.length))return;const o=i[Math.floor(Math.random()*i.length)],r=e.findIndex(a=>a.player);if(r===-1)return;const n=e[r];await v.from("cards").update({player_id:o.id}).eq("id",n.id),e[r]={...n,player_id:o.id,player:o}}catch(i){console.warn("[Onboarding] ensureGK échec",i)}}const Zn=[{key:"home2",route:"home2",label:"ACCUEIL",icon:"nav-home.png"},{key:"cards",route:"collection",label:"CARTES",icon:"nav-collection.png"},{key:"decks",route:"decks",label:"EQUIPES",icon:"nav-decks.png"},{key:"boosters",route:"boosters",label:"BOUTIQUE",icon:"nav-boosters.png"},{key:"market",route:"market",label:"MERCATO",icon:"nav-market.png"}];function Yt(t,e,i,o,r){if(!document.getElementById("home2-chrome-style")){const p=document.createElement("style");p.id="home2-chrome-style",p.textContent=`
    ...
    </div>`,document.body.appendChild(e),e.querySelector("#journal-close-v2").addEventListener("click",()=>e.remove()),e.addEventListener("click",o=>{o.target===e&&e.remove()})}async function Fi(t,{state:e,navigate:i,toast:o}){var m,T,$,I,D,V,Y;const r=e.profile;if(!r)return;const n="/icons/";Yt(i,r,"home2",n,o);const a=r.mmr??1e3,d=zt(a),c=or(a,d),s=Gi(a),l=isFinite(d.max)?d.max+1:null,[{list:p,iAmInTop:x,myPosition:g},b,f,{count:y}]=await Promise.all([nr(r.id),rr(5),ar(),v.from("pending_rewards").select("*",{count:"exact",head:!0}).eq("user_id",r.id).eq("claimed",!1)]),u=(j,L)=>`
    ...
    <div class="news-item">
      ...
      <div class="news-body">
        ...
        <div class="news-time">${ir(j.published_at)}</div>
      </div>
  ...
  <div class="home-dark" id="home-dark">
      ...
      <div class="home2-dash">
        ...
        <div class="home2-col-right">
          <div class="news-widget">
            ...
            ${b.length?b.map(w).join(""):'<div style="font-size:11px;color:var(--tile-fg-dim);padding:8px 0">Aucune actualité pour le moment.</div>'}
          </div>
        </div>
    ...
    </div>`,s.querySelectorAll("[data-stats]").forEach(g=>{g.addEventListener("click",()=>br(t,g.dataset.stats,g.dataset.friendName))}),s.querySelectorAll("[data-match]").forEach(g=>{g.addEventListener("click",async()=>{const b=g.dataset.friendId,f=g.dataset.friendName;if(typeof o!="function"){e("Erreur navigation","error");return}const y=t.user.id,{data:u}=await v.from("friend_match_invites").select("id").eq("inviter_id",b).eq("invitee_id",y).eq("status","pending").order("created_at",{ascending:!1}).limit(1).maybeSingle(),w=!!u;console.log("[Friends] clic match",{fid:b,fname:f,isAccepting:w}),mt(),o("match",{matchMode:"friend",friendId:b,friendName:f,isAccepting:w})})})}function mr(t,e){const i=t.club_name||t.pseudo||"?",o=t.pseudo||"",r=(o||i).slice(0,2).toUpperCase(),n=t.club_color2||di,a=t.club_color1||"#ffffff",d=t.last_seen_at?new Date(t.last_seen_at):null,c=d&&Date.now()-d.getTime()<3*60*1e3;return`
    ...
    </style>`}async function xr(t,{state:e,navigate:i,toast:o}){const r=e.profile;if(!r)return;const n="/icons/";Yt(i,r,"game",n,o),t.innerHTML=`
  <style>
    .game-page {
      min-height: 100%;
      background: var(--page-bg);
  ...
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:0 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none;align-items:center"></div>
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{D=!D,G(),O()})))}function O(){const q=document.getElementById("col-grid");q&&(T==="player"?fe(q):T==="formation"?be(q):T==="stadium"?de(q):ge(q))}function le(q,W,ee,ie,X,R=0){const J=document.getElementById("col-grid"),P=document.getElementById("col-big");if(!J||!P)return;var U=0;function ae(){var De;const ce=window.innerWidth>=768,xe=document.getElementById("col-big"),ke=document.getElementById("col-gap"),Ee=((De=document.getElementById("col-grid"))==null?void 0:De.parentElement)||null,Se=xe?xe.closest(".page"):null;if(xe&&Ee&&Se){let Me=0;Array.from(Se.children).forEach(function(A){A!==xe&&A!==Ee&&A!==ke&&(Me+=A.offsetHeight)});const Ce=Math.max(0,Se.clientHeight-Me),He=Math.round(Ce*(R/100)),k=Math.max(0,Ce-He),S=ce?50/71:45/63,z=Math.round(k*S),C=Math.max(0,k-z);ke&&(ke.style.height=He+"px"),xe.style.flex="none",xe.style.height=z+"px",xe.style.minHeight="0",Ee.style.flex="none",Ee.style.height=C+"px",Ee.style.minHeight="0",Ee.style.display="flex",Ee.style.overflow="hidden"}const ye=document.getElementById("col-grid");ye&&(ye.style.height="100%",ye.style.flexShrink="0",ye.style.overflowX="auto",ye.style.overflowY="hidden",ye.style.alignItems="center",ye.style.width="100%"),P.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:center center">'+W(q[U])+"</div>";var Le=P.querySelector("[data-card-id],[data-form-id],[data-gc-id]");Le&&Le.addEventListener("click",function(){ie(q[U])}),requestAnimationFrame(function(){var Me=document.getElementById("big-card-inner");if(!(!Me||!P)){var Ce=P.clientHeight,He=P.clientWidth-16,k=Me.offsetHeight,S=Me.offsetWidth;if(k>0&&S>0&&Ce>40){var z=Math.min(Ce/k,He/S);Me.style.transform="scale("+z.toFixed(3)+")",Me.style.transformOrigin="center center"}}}),J.innerHTML=q.map(function(Me,Ce){var He=Ce===U,k="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+(He?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+Ce+'" style="'+k+'">'+ee(Me,He)+"</div>"}).join(""),J.querySelectorAll(".col-mini-item").forEach(function(Me){Me.addEventListener("click",function(){U=Number(Me.dataset.idx),ae(),Me.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})}),requestAnimationFrame(function(){var Me=ye?ye.clientHeight:0,Ce=J.querySelector(".col-mini-item > div");if(!(!Me||!Ce)){var He=Ce.style.zoom;Ce.style.zoom="1";var k=Ce.offsetHeight;if(Ce.style.zoom=He,!(k<=0)){var S=Me/k;J.querySelectorAll(".col-mini-item > div").forEach(function(z){z.style.zoom=S.toFixed(4)})}}})}ae()}function pe(q,W){var ee=window.innerWidth>=768?.76:.54,ie;if(!q||q._fake){var X=q?q.player:null;if(!X)return"";ie=Ao(X)}else ie=Pi(q,"");var R=q&&!q._fake?h[q.player&&q.player.id]||1:0,J=R>0?'<div style="position:absolute;top:6px;right:6px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:11px;font-weight:900;padding:2px 7px;z-index:3;box-shadow:0 1px 4px rgba(0,0,0,0.4)">×'+R+"</div>":"";return'<div style="display:inline-block;position:relative;zoom:'+ee+';pointer-events:none;line-height:0">'+J+ie+"</div>"}function M(q,W,ee){var ie=ee>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+ee+"</div>":"",X=!!W,R=si(q,ut[q],{width:160});return X||(R='<div style="filter:grayscale(1);opacity:0.5">'+R+"</div>"),"<div "+(W?'data-form-id="'+W.id+'"':"")+' style="position:relative;cursor:pointer">'+ie+R+"</div>"}function H(q,W){var ee=window.innerWidth>=768?.76:.54,ie=si(q,ut[q],{width:140});return W||(ie='<div style="filter:grayscale(1);opacity:0.45">'+ie+"</div>"),'<div style="display:inline-block;zoom:'+ee+';line-height:0;pointer-events:none">'+ie+"</div>"}function fe(q){if(D){const W=_();if(!W.length){q.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucun joueur.</div>';return}const ee=W.map(ie=>s.find(X=>X.player.id===ie.id)||{_fake:!0,player:ie,id:"fake-"+ie.id});le(ee,ie=>ie._fake?Ao(ie.player):Pi(ie,""),ie=>ie._fake?pe({player:ie.player,id:"x",_fake:!0}):pe(ie),ie=>{ie._fake||zo(ie,s,h,e)})}else{const W=L();if(!W.length){q.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const ee={};W.forEach(X=>{const R=X.player.id;ee[R]||(ee[R]=[]),ee[R].push(X)});const ie=Object.values(ee).map(X=>kr(X)).filter(X=>!V||(h[X.player.id]||1)>1);if(V&&!ie.length){q.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte en plusieurs exemplaires.</div>';return}le(ie,X=>{const R=h[X.player.id]||1,J=R>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${R}</div>`:"",U=s.filter(ae=>ae.player.id===X.player.id&&ae.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Pi(X,J+U)},X=>pe(X),X=>zo(X,s,h,e))}}function be(q){const W=D?u:[...E];if(!W.length){q.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const ee=W.map(ie=>({formation:ie,card:p.find(X=>X.formation===ie)||null,owned:E.has(ie)}));le(ee,({formation:ie,card:X,owned:R})=>M(ie,R?X:null,R?p.filter(J=>J.formation===ie).length:0),({formation:ie,owned:X})=>H(ie,X),({card:ie,owned:X})=>{X&&ie&&Sr(ie,p,e,n)},"#1A6B3C",5)}function ge(q){const W=Object.keys(b),ee=D?W.length?W:w:[...m];if(!ee.length){q.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const ie=ee.map(X=>({type:X,gc:Oe[X]||{icon:"⚡",desc:""},def:b[X]||null,owned:m.has(X),card:l.find(R=>R.gc_type===X)||null}));le(ie,({type:X,gc:R,def:J,owned:P,card:U})=>{var ye;const ae=(J==null?void 0:J.name)||X,ce=P?l.filter(Le=>Le.gc_type===X).length:0,xe=ce>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${ce}</div>`:"",ke=(J==null?void 0:J.effect)||R.desc||"",Ee=J!=null&&J.image_url?`/icons/${J.image_url}`:((ye=J==null?void 0:J.club)==null?void 0:ye.logo_url)||(J!=null&&J.country_code?`https://flagsapi.com/${J.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null);let Se=nt(ae,Ee,R.icon,ke,{width:160,onClick:P});return P||(Se=`<div style="filter:grayscale(1);opacity:0.5">${Se}</div>`),`<div ${P&&U?`data-gc-id="${U.id}" data-gc-type="${X}"`:""} style="position:relative">${xe}${Se}</div>`},({type:X,gc:R,def:J,owned:P})=>{const U=window.innerWidth>=768?.76:.54,ae=(J==null?void 0:J.name)||X,ce=(J==null?void 0:J.effect)||R.desc||"",xe=J!=null&&J.image_url?`/icons/${J.image_url}`:null;let ke=nt(ae,xe,R.icon,ce,{width:140});return P||(ke=`<div style="filter:grayscale(1);opacity:0.45">${ke}</div>`),`<div style="display:inline-block;zoom:${U};line-height:0;pointer-events:none">${ke}</div>`},({type:X,owned:R,def:J})=>{R&&Lr(X,J,n)},"#7a28b8",5)}function de(q){const W="#4FC3F7",ee="/";if(!x.length){q.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Stade.<br><small>Ouvre un booster Stade !</small></div>';return}const ie={};x.forEach(R=>{const J=R.stadium_id||"?";(ie[J]=ie[J]||[]).push(R)});const X=Object.entries(ie).map(([R,J])=>({sid:R,def:y[R]||null,count:J.length,card:J[0]}));le(X,({def:R,count:J})=>{var ke,Ee;const P=(R==null?void 0:R.name)||"?",U=((ke=R==null?void 0:R.club)==null?void 0:ke.encoded_name)||(R==null?void 0:R.country_code)||"—",ae=R!=null&&R.image_url?`${ee}icons/${R.image_url}`:((Ee=R==null?void 0:R.club)==null?void 0:Ee.logo_url)||(R!=null&&R.country_code?`https://flagsapi.com/${R.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),ce=J>1?`<div style="position:absolute;top:8px;right:8px;background:#333;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${J}</div>`:"",xe=`${U}<br>+10 ⭐ joueurs alliés`;return`<div style="position:relative">${ce}${Ut(P,ae,xe,{width:160})}</div>`},({def:R})=>{var xe,ke;const J=window.innerWidth>=768?.76:.54,P=(R==null?void 0:R.name)||"?",U=((xe=R==null?void 0:R.club)==null?void 0:xe.encoded_name)||(R==null?void 0:R.country_code)||"—",ae=R!=null&&R.image_url?`${ee}icons/${R.image_url}`:((ke=R==null?void 0:R.club)==null?void 0:ke.logo_url)||(R!=null&&R.country_code?`https://flagsapi.com/${R.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),ce=`${U}<br>+10 ⭐`;return`<div style="display:inline-block;zoom:${J};line-height:0;pointer-events:none">${Ut(P,ae,ce,{width:140})}</div>`},null,W,5)}t.querySelectorAll(".col-tab-btn").forEach(q=>{q.addEventListener("click",()=>{T=q.dataset.tab,$="Tous",I="",D=!1,t.querySelectorAll(".col-tab-btn").forEach(W=>{const ee=W.dataset.tab===T;W.style.borderBottomColor=ee?"var(--green)":"transparent",W.style.color=ee?"var(--green)":"var(--tile-fg-dim)"}),G(),O()})}),G(),O()}function Lr(t,e,i){const o=Oe[t]||{icon:"⚡",desc:"Effet spécial."},r=(e==null?void 0:e.name)||t,n=(e==null?void 0:e.effect)||o.desc,a=e!=null&&e.image_url?`/icons/${e.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
      ${nt(r,a,o.icon,n,{width:200})}
      <div style="background:#fff3cd;border-radius:10px;padding:10px 14px;width:100%">
        <div style="font-size:12px;color:#856404">⚠️ Cette carte est à <b>usage unique</b>. Une fois jouée en match, elle est définitivement supprimée de ta collection.</div>
      </div>
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const bi=1e3;function Sr(t,e,i,o){var b,f,y;const{state:r,toast:n,closeModal:a,navigate:d,refreshProfile:c}=i,s=t.formation,l={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function p(){const u=ut[s]||{},w=Ai[s]||[],h=290,E=360,m=20;function T(I){const D=u[I];return D?{x:D.x*h,y:D.y*E}:null}let $=`<svg width="${h}" height="${E}" viewBox="0 0 ${h} ${E}" xmlns="http://www.w3.org/2000/svg">`;for(const[I,D]of w){const V=T(I),Y=T(D);!V||!Y||($+=`<line x1="${V.x}" y1="${V.y}" x2="${Y.x}" y2="${Y.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const I of Object.keys(u)){const D=T(I);if(!D)continue;const V=I.replace(/\d+/,""),Y=l[V]||"#555";$+=`<circle cx="${D.x}" cy="${D.y}" r="${m}" fill="${Y}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,$+=`<text x="${D.x}" y="${D.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${V}</text>`}return $+="</svg>",$}const x=e.filter(u=>u.formation===s);x.length;const g=!t.is_for_sale;o(`Formation ${s}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(b=document.getElementById("direct-sell-form-btn"))==null||b.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${s} pour ${bi.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const u=x.find(h=>!h.is_for_sale)||x[0];if(!u){n("Aucune carte à vendre","error");return}await v.from("market_listings").delete().eq("card_id",u.id),await v.from("transfer_history").delete().eq("card_id",u.id);const{error:w}=await v.from("cards").delete().eq("id",u.id);if(w){n(w.message,"error");return}await v.from("users").update({credits:(r.profile.credits||0)+bi}).eq("id",r.profile.id),await c(),n(`+${bi.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),a(),d("collection")}),(f=document.getElementById("market-sell-form-btn"))==null||f.addEventListener("click",async()=>{const u=parseInt(document.getElementById("sell-price-form").value);if(!u||u<1){n("Prix invalide","error");return}await v.from("cards").update({is_for_sale:!0,sale_price:u}).eq("id",t.id),await v.from("market_listings").insert({seller_id:r.profile.id,card_id:t.id,price:u}),n("Carte mise en vente sur le marché !","success"),a(),d("collection")}),(y=document.getElementById("cancel-sell-form-btn"))==null||y.addEventListener("click",async()=>{await v.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",t.id),await v.from("market_listings").update({status:"cancelled"}).eq("card_id",t.id).eq("status","active"),n("Annonce retirée","success"),a(),d("collection")})}async function zo(t,e,i,o){var fe,be,ge,de,q,W,ee,ie,X,R,J;const{state:r,toast:n,openModal:a,closeModal:d,navigate:c,refreshProfile:s}=o,l=t.player,p=e.filter(P=>P.player.id===l.id),x=p.length,g=t.evolution_bonus||0,b=Math.max((Number(l.note_g)||0)+(l.job==="GK"||l.job2==="GK"?g:0),(Number(l.note_d)||0)+(l.job==="DEF"||l.job2==="DEF"?g:0),(Number(l.note_m)||0)+(l.job==="MIL"||l.job2==="MIL"?g:0),(Number(l.note_a)||0)+(l.job==="ATT"||l.job2==="ATT"?g:0)),f=l.rarity||"normal",{data:y}=await v.from("sell_price_configs").select("*").eq("rarity",f).lte("note_min",b).gte("note_max",b).order("note_min",{ascending:!1}).limit(1),u=((fe=y==null?void 0:y[0])==null?void 0:fe.price)??_r[f]??1e3,w=((be=y==null?void 0:y[0])==null?void 0:be.price_min)??null,h=((ge=y==null?void 0:y[0])==null?void 0:ge.price_max)??null;Bt(l);const E=gi(l,l.job)+g,m=l.job2?gi(l,l.job2)+(gi(l,l.job2)>0?g:0):null;ei[l.job],l.job2&&ei[l.job2];const T=vr[l.rarity]||"#ccc",$=$r[l.country_code]||l.country_code||"",I=t.evolution_bonus||0,V=E+I,Y=m||0,j=Y>0?Y+I:0,L=p.map(P=>P.id);let _={};if(L.length){const{data:P}=await v.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",L).order("transferred_at",{ascending:!0});(P||[]).forEach(U=>{_[U.card_id]||(_[U.card_id]=[]),_[U.card_id].push(U)})}const G=L.length?`
    <div style="margin-top:16px;border-top:1px solid var(--tile-border);padding-top:14px">
      ...
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px">
        ${p.filter(P=>P.id!==t.id).map((P,U)=>{const ae=_[P.id]||[],ce=P.is_for_sale,xe=ae.length?ae[ae.length-1]:null,ke=P.evolution_bonus||0,Ee=xe?xe.source==="booster"?"Booster":xe.price?xe.price.toLocaleString("fr")+" cr.":"—":"—",Se=xe?new Date(xe.transferred_at).toLocaleDateString("fr",{day:"2-digit",month:"2-digit",year:"numeric"}):"",ye=80,Le=Math.round(ye*657/507),De=ze({...l,_evolution_bonus:ke},{width:ye});return`
      ...
      <!-- Carte visuelle -->
      ${ze({...l,_evolution_bonus:g},{width:160})}
      ...
      <div style="flex:1;min-width:220px;display:flex;flex-direction:column;gap:12px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",l.note_g],["DEF",l.note_d],["MIL",l.note_m],["ATT",l.note_a]].map(([P,U])=>{const ae=ei[P],ce=P==="GK"?"#fff":ae,xe=P===l.job||P===l.job2,ke=(Number(U)||0)+(xe&&I>0?I:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(q=document.getElementById("close-detail"))==null||q.addEventListener("click",d);let O=new Set;const le=l.rarity==="pepite"?1.3:l.rarity==="papyte"?.7:1;function pe(){let P=0;return document.querySelectorAll(".expl-check:checked").forEach(U=>{if(U.dataset.id===t.id)return;const ce=Number(U.dataset.evo)||0,xe=Number(U.dataset.note)||0;P+=xe+ce}),Math.round(P*le)}const M=()=>{const P=O.size,U=document.getElementById("sell-action-panel");if(!U)return;U.style.display=P>0?"block":"none",document.getElementById("sell-selected-count").textContent=P;const ae=document.getElementById("evolve-btn");if(ae){const ce=pe();ae.textContent=ce>0?`⬆️ Évoluer (+${ce})`:"⬆️ Évoluer"}};document.querySelectorAll(".expl-check").forEach(P=>{P.addEventListener("change",()=>{const U=P.dataset.id;P.checked?O.add(U):O.delete(U);const ae=P.closest(".exemplaire-row");if(ae){const ce=ae.querySelector(".expl-sel-overlay"),xe=ae.querySelector(".expl-sel-check");ce&&(ce.style.display=P.checked?"block":"none"),xe&&(xe.style.display=P.checked?"flex":"none"),ae.style.transform=P.checked?"scale(1.05)":"scale(1)"}M()})}),document.querySelectorAll(".exemplaire-row").forEach(P=>{P.addEventListener("click",U=>{if(U.target.tagName==="INPUT")return;const ae=P.querySelector(".expl-check");ae&&!ae.disabled&&(ae.checked=!ae.checked,ae.dispatchEvent(new Event("change")))})}),(W=document.getElementById("evolve-btn"))==null||W.addEventListener("click",async()=>{if(x<=1)return;const P=[...O];if(!P.length)return;if(O.has(t.id)){const ye=document.createElement("div");ye.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",ye.innerHTML=`
  ...
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
    ...
    <div class="page-body">
      ...
      stadium_def:stadium_definitions(id, name, club_id, country_code, image_url,
        club:clubs(encoded_name, logo_url))`).eq("owner_id",o.profile.id).eq("card_type","stadium"),c=(a||[]).filter(u=>u.card_type==="player"&&u.player),s=(a||[]).filter(u=>u.card_type==="formation"),l=(d||[]).filter(u=>u.card_type==="stadium"),p=[...new Set(l.map(u=>u.stadium_id).filter(Boolean))];let x={};if(l.forEach(u=>{u.stadium_def&&u.stadium_id&&(x[u.stadium_id]=u.stadium_def)}),p.length&&Object.keys(x).length<p.length){const{data:u}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",p);(u||[]).forEach(w=>{x[w.id]=w})}const g=s.map(u=>u.formation).filter(Boolean),{data:b}=await v.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",t);let f=n.formation||"4-4-2";g.length>0&&!g.includes(f)&&(f=g[0]);const y={deckId:t,name:n.name,formation:f,formationCardId:n.formation_card_id,stadiumCardId:n.stadium_card_id||null,slots:{},subs:[],playerCards:c,formationCards:s,stadiumCards:l,stadDefMap:x,availableFormations:g};(b||[]).forEach(u=>{u.is_starter?y.slots[u.position]=u.card_id:y.subs.includes(u.card_id)||y.subs.push(u.card_id)}),Lt(e,y,i,!0)}function Lt(t,e,i,o=!1){var f,y;const{navigate:r}=i;o||Cr(e,i),ki[e.formation];const n=jo(e.formation),a=n.filter(u=>e.slots[u]).length,d=e.availableFormations.length>0?e.availableFormations:Object.keys(ki),c=(f=e.stadiumCards)==null?void 0:f.find(u=>u.id===e.stadiumCardId),s=c&&((y=e.stadDefMap)==null?void 0:y[c.stadium_id])||null,l=e.subs.map(u=>e.playerCards.find(w=>w.id===u)).filter(Boolean);l.length!==e.subs.length&&(e.subs=l.map(u=>u.id)),[...Object.values(e.slots),...e.subs],t.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
        ...
        <div style="width:105px;flex-shrink:0;background:rgba(0,0,0,0.3);display:flex;flex-direction:column;align-items:center;padding:12px 6px;gap:8px;border-right:1px solid rgba(255,255,255,0.1)">
          ...
          <div style="display:flex;flex-direction:column;gap:6px;align-items:center" id="subs-list">
            ${l.map(u=>{const w={...u.player,_evolution_bonus:u.evolution_bonus||0};return`<div style="position:relative;flex-shrink:0;overflow:visible;padding-bottom:24px">
                ${ze({...w,_evolution_bonus:w._evolution_bonus||0},{width:90,showStad:!0,stadDef:s})}
          ...
          <div style="flex:1;min-width:0">
            ...
            <div style="display:flex;gap:2px;align-items:center;flex-wrap:nowrap;overflow-x:auto;scrollbar-width:none;-ms-overflow-style:none" id="subs-list" class="no-scrollbar">
              ${l.map(u=>{const w={...u.player,_evolution_bonus:u.evolution_bonus||0};return`<div style="position:relative;flex-shrink:0;overflow:visible;padding-bottom:20px">
                  ${ze({...w,_evolution_bonus:w._evolution_bonus||0},{width:44,showStad:!0,stadDef:s})}
                  <button data-remove-sub="${u.id}"
    ...
    </div>`;u("⚽ Choisir une formation",E),document.querySelectorAll("#modal-body [data-forma]").forEach(m=>{m.addEventListener("click",()=>{e.formation=m.dataset.forma;const T=jo(e.formation),$={};T.forEach(I=>{e.slots[I]&&($[I]=e.slots[I])}),e.slots=$,w(),Lt(t,e,i)})})};t.querySelectorAll("#formation-mobile-btn, #formation-pc-btn").forEach(u=>u.addEventListener("click",b)),t.querySelectorAll("#add-stad-btn-pc, #add-stad-btn").forEach(u=>u.addEventListener("click",()=>Ar(e,t,i))),t.querySelectorAll("#save-deck, #save-deck-pc").forEach(u=>u.addEventListener("click",()=>jr(e,i))),t.querySelectorAll("[data-remove-sub]").forEach(u=>{u.addEventListener("click",()=>{e.subs=e.subs.filter(w=>w!==u.dataset.removeSub),Lt(t,e,i)})}),t.querySelectorAll("#add-sub-btn").forEach(u=>u.addEventListener("click",()=>Mr(e,t,i)))}function Ir(t,e,i,o){var h,E,m;const r=window.innerWidth>=900,n=t.querySelector(r?"#deck-field-pc":"#deck-field-mobile");if(!n)return;const a=(h=e.stadiumCards)==null?void 0:h.find(T=>T.id===e.stadiumCardId),d=a&&((E=e.stadDefMap)==null?void 0:E[a.stadium_id])||null,c=ut[e.formation]||{},s=Ii(e.formation)||[],l={};for(const T of i){const $=e.slots[T],I=$?e.playerCards.find(D=>D.id===$):null;I!=null&&I.player?l[T]={...I.player,_evolution_bonus:I.evolution_bonus||0,face:I.player.face||null}:l[T]=null}const p=window.innerWidth>=900,x=p?window.innerWidth-280:window.innerWidth-20,g=p?Math.min(x,860):x,b=Math.round(p?g*.82:g*.85),f=p?84:44;let y="";for(const[T,$]of s){const I=c[T],D=c[$];if(!I||!D)continue;const V=I.x*g,Y=Math.round(.03*b+I.y*.85*b),j=D.x*g,L=Math.round(.03*b+D.y*.85*b),_=l[T],G=l[$],O=Ft(_,G);O==="#ff3333"||O==="#cc2222"?y+=`<line x1="${V.toFixed(1)}" y1="${Y.toFixed(1)}" x2="${j.toFixed(1)}" y2="${L.toFixed(1)}" stroke="${O}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(y+=`<line x1="${V.toFixed(1)}" y1="${Y.toFixed(1)}" x2="${j.toFixed(1)}" y2="${L.toFixed(1)}" stroke="${O}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,y+=`<line x1="${V.toFixed(1)}" y1="${Y.toFixed(1)}" x2="${j.toFixed(1)}" y2="${L.toFixed(1)}" stroke="${O}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let u="";const w=Math.round(f*657/507);for(const T of i){const $=c[T];if(!$)continue;const I=l[T],D=$.x*g;$.y*b;const V=Math.round(.03*b+$.y*(.85*b)),Y=Math.round(D-f/2),j=Math.round(V-w/2);if(I){const L=T.replace(/\d+/,""),_=d&&(d.club_id&&String(I.club_id)===String(d.club_id)||d.country_code&&I.country_code===d.country_code),G=ze({...I,_evolution_bonus:I._evolution_bonus||0},{width:f,showStad:!0,stadDef:d,role:L});_&&((m=d.club)!=null&&m.logo_url||d.image_url),u+=`<div style="position:absolute;left:${Y}px;top:${j}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${T}">
    ...
    <div style="position:relative;width:${g}px;height:${b}px;margin:0 auto">
      ...
      ${u}
    </div>`,n.querySelectorAll(".deck-slot-hit").forEach(T=>{T.addEventListener("click",()=>zr(T.dataset.pos,e,t,o))})}function Ar(t,e,i){var d;const{openModal:o,closeModal:r}=i,n=new Set,a=(t.stadiumCards||[]).filter(c=>{const s=c.stadium_id||c.id;return n.has(s)?!1:(n.add(s),!0)});o("🏟️ Choisir un stade",`<div style="display:flex;flex-wrap:wrap;gap:10px;padding:8px">
      ...
      ${s.length>0?'<div style="display:flex;flex-wrap:wrap;gap:8px;justify-content:center">'+s.map(g=>{const b={...g.player,_evolution_bonus:g.evolution_bonus||0};return`<div class="player-option" data-card-id="${g.id}" style="cursor:pointer">
          ${ze(b,{width:100,showStad:!0,stadDef:a})}
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      <div style="flex:1;overflow-y:auto;display:flex;flex-wrap:wrap;gap:10px;justify-content:center;align-content:flex-start;padding:8px 16px 16px">
        ${n.map(b=>{const f=a.find(y=>y.gc_type===b.gc_type);return d(b,!!f)}).join("")}
      </div>
      ...
      card:cards(id,card_type,formation,stadium_id,evolution_bonus,
        player:players(id,firstname,surname_real,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order"),c=[...new Set((d||[]).filter(x=>{var g,b;return((g=x.card)==null?void 0:g.card_type)==="stadium"&&((b=x.card)==null?void 0:b.stadium_id)}).map(x=>x.card.stadium_id))],s={};if(c.length){const{data:x}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",c);(x||[]).forEach(g=>{s[g.id]=g}),(d||[]).forEach(g=>{var b,f;((b=g.card)==null?void 0:b.card_type)==="stadium"&&((f=g.card)!=null&&f.stadium_id)&&(g.card._stadiumDef=s[g.card.stadium_id]||null)})}let l=0;function p(){var m,T,$,I,D,V,Y;const x=n[l],g=(d||[]).filter(j=>j.deck_id===x.id),b=g.filter(j=>{var L;return j.is_starter&&((L=j.card)==null?void 0:L.player)}).map(j=>Vi(j.card,j.position)),f=g.find(j=>{var L;return((L=j.card)==null?void 0:L.card_type)==="formation"}),y=x.formation||((m=f==null?void 0:f.card)==null?void 0:m.formation)||"4-4-2";let u=b.length>=11?Ei(b,y):null,w=((T=x.stadium_card)==null?void 0:T.stadium_def)||null;w&&u&&(u=ui(u,w));const h=b.length>=11;ct(t),t.style.height="100%",t.style.overflow="hidden",t.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff;position:relative">
      ...
      <div id="deck-bottom-bar" style="flex-shrink:0;padding:10px 14px 14px;display:flex;flex-direction:column;gap:8px;background:rgba(0,0,0,0.2)">
        ...
        stroke="${$}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const b=typeof window<"u"&&window.innerWidth>=900?Math.min(Math.max(81,Math.round(r*.225)),117):Math.max(44,Math.round(r*.168)),f=Math.round(b*657/507);for(const[u,w]of Object.entries(l)){const h=x(u);if(!h||!w)continue;const E=u.replace(/[0-9]/g,""),m=a.includes(w.cardId),T=i==="attack"&&(["MIL","ATT"].includes(E)||m)&&!w.used||i==="defense"&&["GK","DEF","MIL"].includes(E)&&!w.used,$=o.includes(w.cardId);let I=w.boost||0,D=!1;w.stadiumBonus&&(i==="attack"&&(E==="ATT"||E==="MIL")||i==="defense"&&(E==="GK"||E==="DEF"||E==="MIL")?(I+=10,D=!0):i||(I+=10,D=!0));const V=Math.round(h.x-b/2),Y=Math.round(h.y-f/2);if(w.used){g+=`<image href="${`${typeof import.meta<"u"&&"/"||"/"}icons/carte-dos.png`}" x="${V}" y="${Y}" width="${b}" height="${f}" preserveAspectRatio="xMidYMid slice" class="match-used-hit" data-card-id="${w.cardId}" data-role="${E}" style="cursor:pointer"/>`;continue}const j=ze({...w,_evolution_bonus:0,stadiumBonus:!1},{width:b,showStad:!1,stadDef:null,role:E,extraNote:I,_cardOffset:30,_forceStadColor:D}),L=$?`position:absolute;top:30px;left:0;width:${b}px;height:${f}px;outline:3px solid #FFD700;outline-offset:2px;border-radius:8px;pointer-events:none;`:"";g+=`<foreignObject x="${V-2}" y="${Y-30}" width="${b+8}" height="${f+60}" style="overflow:visible">
      <div xmlns="http://www.w3.org/1999/xhtml" style="position:relative">
        ${j}
        ${$?`<div style="${L}"></div>`:""}
      </div>
    </foreignObject>`,T&&(g+=`<rect x="${V}" y="${Y}" width="${b}" height="${f}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${$?"selected":""}" data-card-id="${w.cardId}" data-role="${E}" style="cursor:pointer"/>`)}const y=d!==null?d:Math.round(Math.max(b*.7,80));return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-y} ${-y} ${r+y*2} ${n+y*2}" width="100%" style="display:block;width:100%;margin:0 auto">
    ${g}
  </svg>`}function Kt(t,e,i,o,r=300,n=300,a=[],d=null){return`<div id="match-terrain-wrap" style="position:relative;padding:0 4px">
    ${Mi(t,e,i,o,r,n,a,d)}
  </div>`}async function Ci(t,e,i,o){var T;const{state:r,navigate:n,toast:a}=e;ct(t);const d=r.params||{};if(t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!d.deckId)return Dr(t,e,i);const c=d.deckId;let s,l,p,x;try{const $=await Promise.all([v.from("decks").select("formation,name,stadium_card_id").eq("id",c).single(),v.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation, evolution_bonus,
            player:players(id,firstname,surname_real,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
              clubs(encoded_name,logo_url)))`).eq("deck_id",c).order("slot_order")]);s=$[0].data,p=$[0].error,l=$[1].data,x=$[1].error}catch($){console.error("[Match] Exception chargement deck:",$),Ct(t,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>n("home"));return}if(p||x){console.error("[Match] Erreur Supabase:",p||x),Ct(t,"⚠️","Erreur lors du chargement du deck.","Retour",()=>n("home"));return}const g=(l||[]).filter($=>{var I;return $.is_starter&&((I=$.card)==null?void 0:I.player)}).map($=>Vi($.card,$.position)),b=(l||[]).filter($=>{var I;return!$.is_starter&&((I=$.card)==null?void 0:I.player)}).map($=>Vi($.card,$.position));if(g.length<11){Ct(t,"⚠️",`Deck incomplet (${g.length}/11).`,"Compléter",()=>n("decks"));return}const f=(l||[]).find($=>{var I;return((I=$.card)==null?void 0:I.card_type)==="formation"}),y=(s==null?void 0:s.formation)||((T=f==null?void 0:f.card)==null?void 0:T.formation)||"4-4-2",{data:u,error:w}=await v.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",r.profile.id).eq("card_type","game_changer"),{data:h}=await v.from("gc_definitions").select("*").eq("is_active",!0),E=(u||[]).map($=>({...$,_gcDef:(h==null?void 0:h.find(I=>I.name===$.gc_type||I.id===$.gc_definition_id))||null}));let m=null;if(s!=null&&s.stadium_card_id){const{data:$}=await v.from("cards").select("stadium_id").eq("id",s.stadium_card_id).single();if($!=null&&$.stadium_id){const{data:I}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").eq("id",$.stadium_id).single();m=I||null}}o({deckId:c,formation:y,starters:g,subsRaw:b,gcCardsEnriched:E,gcDefs:h||[],stadiumDef:m})}function jt(){return Math.min(window.innerWidth-40,860)}function Ht(){return Math.round(jt()*1.1)}function gn(t){var r,n;if(!t)return null;const e=t._line||t.job||"MIL",i=e==="GK"?t.note_g||0:e==="DEF"?t.note_d||0:e==="MIL"?t.note_m||0:t.note_a||0,o=t.stadiumBonus?10:0;return{name:t.name,firstname:t.firstname||"",note:i+(t.boost||0)+o,note_g:t.note_g||0,note_d:t.note_d||0,note_m:t.note_m||0,note_a:t.note_a||0,_evolution_bonus:0,stadiumBonus:t.stadiumBonus||!1,boost:t.boost||0,job:t.job,job2:t.job2||null,_line:t._line||t.job,_col:t._col,country_code:t.country_code,club_id:t.club_id,rarity:t.rarity,clubName:t.clubName||((r=t.clubs)==null?void 0:r.encoded_name)||null,clubLogo:t.clubLogo||((n=t.clubs)==null?void 0:n.logo_url)||null,face:t.face||null,portrait:Bt(t)}}function Fr(t,e,i,o="Adversaire"){Wo("/sounds/match-opening.mp3",.3);const r=jt(),n=Ht();return`
    ...
    </div>`}function bn(t){var n,a,d;if(!t)return"";const e=c=>c?ze({...c,_evolution_bonus:0},{width:52,role:c._line||c.job,showStad:!!c.stadiumBonus,extraNote:c.boost||0}):"",o={goal:"⚽","goal-home":"⚽","goal-ai":"⚽",duel:"⚔️",midfield:"🎯",sub:"🔄",gc:"⚡",boost:"💥","defense-won":"🛡️","attack-won":"⚔️","defense-lost":"😓","attack-lost":"😓"}[t.type]||"📋";return`
    ...
    pointer-events:none;white-space:nowrap;max-width:90vw;text-align:center`,r.textContent=t,!document.getElementById("game-toast-style")){const n=document.createElement("style");n.id="game-toast-style",n.textContent=`
      ...
      }`,document.head.appendChild(n)}document.body.appendChild(r),setTimeout(()=>{r.style.opacity="0",r.style.transition="opacity 0.3s",setTimeout(()=>r.remove(),300)},i)}function xn(t,e){const i=Re(t,"MIL"),o=t.stadiumBonus||e&&(e.club_id&&String(t.club_id)===String(e.club_id)||e.country_code&&t.country_code===e.country_code)?10:0;return i+o}function wt(t,e){return t.reduce((i,o)=>i+xn(o,e),0)}function _t(t){let e=0;for(let i=0;i<t.length-1;i++){const o=Ft(t[i],t[i+1]);o==="#00ff88"?e+=10:o==="#FFD700"&&(e+=5)}return e}function Li(t,e,i,o,r){return`<div id="duel-row-${o}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0), opacity .5s ease;transform-origin:center">
    ...
    </div>`}async function Rr(t,e){const{state:i}=e,o=i.params||{},r=o.matchMode||"vs_ai_easy",n=r==="solo",a=r==="ranked_ai",d=o.soloLevel||1,c=r.replace("vs_ai_",""),s=r;let l=null;if(n){const{data:b}=await v.from("solo_levels").select("*").eq("level_number",d).maybeSingle();l=b||{level_number:d,target_note_avg:10,nb_liens_jaune:2,nb_liens_vert:1,nb_joueurs_stade:2,reward_credits:500}}const p=o.rankedData||null,x=o.presetSetup||null;async function g({deckId:b,formation:f,starters:y,subsRaw:u,gcCardsEnriched:w,gcDefs:h,stadiumDef:E}){try{let m=Ei(y,f);E&&(m=ui(m,E),$i(u,E));let T=null;if(a){const V=Gr(m),{jaune:Y,vert:j}=Or(m,f),L=.9+Math.random()*.2;T={target_note_avg:Math.max(1,Math.round(V*L/16)),nb_liens_jaune:Y,nb_liens_vert:j,nb_joueurs_stade:E?3:0}}const $=n?await Bo(f,l):a?await Bo(f,T):await Nr(f,c),I=$.lines||$,D=async V=>{try{const Y=s==="vs_ai_club"||s==="solo"||s==="ranked_ai"?"club":s,{data:j,error:L}=await v.from("matches").insert({home_id:i.profile.id,away_id:null,mode:Y,home_deck_id:b,status:"in_progress"}).select().single();if(L){console.error("[MatchIA] Erreur création match:",L),Ct(t,"⚠️","Impossible de créer le match ("+L.message+").","Retour",()=>e.navigate("home"));return}const _=$.stadiumDef||null;_&&I&&(ui(I,_),$i($.subs||[],_));const G={gcDefs:h||[],matchId:j==null?void 0:j.id,mode:s,difficulty:c,formation:f,isSolo:n,soloLevel:d,soloLevelConfig:l,isRankedAI:a,rankedAIData:p,homeTeam:m,aiTeam:I,homeSubs:u,subsUsed:0,maxSubs:Math.min(u.length,3),aiSubs:$.subs||[],aiSubsUsed:0,aiMaxSubs:Math.min(($.subs||[]).length,3),aiUsedSubIds:[],aiGcCards:$.gcCards||[],aiUsedGc:[],aiStadiumDef:_,homeScore:0,aiScore:0,gcCards:V,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Hr(t,G,e)}catch(Y){console.error("[MatchIA] Exception launchMatch:",Y),Ct(t,"⚠️","Erreur au lancement du match : "+Y.message,"Retour",()=>e.navigate("home"))}};if(a&&x){D(x.gcCardsEnriched||[]);return}if(!w.length){D([]);return}zi(t,w,D)}catch(m){console.error("[MatchIA] Exception setup:",m),Ct(t,"⚠️","Erreur de préparation du match : "+m.message,"Retour",()=>e.navigate("home"))}}x?await g(x):await Ci(t,e,r,g)}async function Nr(t,e){var y;const{data:i}=await v.from("players").select("id,firstname,surname_real,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(80);if(!i||i.length<11)return{lines:yn(t),subs:[],gcCards:[],stadiumDef:null};const o=St[t]||St["4-4-2"],r={GK:[],DEF:[],MIL:[],ATT:[]},n=new Set;function a(u,w,h){var E,m;return n.add(u.id),{cardId:"ai-"+u.id+"-"+h,id:u.id,firstname:u.firstname,name:u.surname_real,country_code:u.country_code,club_id:u.club_id,job:u.job,job2:u.job2,note_g:Number(u.note_g)||0,note_d:Number(u.note_d)||0,note_m:Number(u.note_m)||0,note_a:Number(u.note_a)||0,rarity:u.rarity,skin:u.skin,hair:u.hair,hair_length:u.hair_length,face:u.face,clubName:((E=u.clubs)==null?void 0:E.encoded_name)||null,clubLogo:((m=u.clubs)==null?void 0:m.logo_url)||null,boost:0,used:!1,_line:w}}for(const u of["GK","DEF","MIL","ATT"]){const w=i.filter($=>$.job===u&&!n.has($.id)),h=i.filter($=>$.job!==u&&!n.has($.id)),E=[...w,...h],m=[];for(let $=0;$<o[u];$++){const I=E[$];I&&m.push(a(I,u,$))}const T=lt(m.length);m.forEach(($,I)=>{$._col=T[I]}),r[u]=m}const c=i.filter(u=>!n.has(u.id)).slice(0,5).map((u,w)=>a(u,u.job,100+w)),{data:s}=await v.from("gc_definitions").select("*").eq("is_active",!0),p=[...s||[]].sort(()=>Math.random()-.5).slice(0,3).map((u,w)=>({id:"ai-gc-"+w,gc_type:u.name,name:u.name,icon:"⚡",effect_type:u.effect_type,effect_params:u.effect_params||{}})),x=Object.values(r).flat(),g={};x.forEach(u=>{u.club_id&&(g[u.club_id]=(g[u.club_id]||0)+1)});const b=(y=Object.entries(g).sort((u,w)=>w[1]-u[1])[0])==null?void 0:y[0];let f=null;if(b){const{data:u}=await v.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",b).single();u&&(f={club_id:u.id,country_code:null,name:u.encoded_name+" Stadium",club:{encoded_name:u.encoded_name,logo_url:u.logo_url}})}return{lines:r,subs:c,gcCards:p,stadiumDef:f}}function Gr(t){let e=0;for(const i of["GK","DEF","MIL","ATT"])(t[i]||[]).forEach(o=>{const r=o._line||i,n=Number(r==="GK"?o.note_g:r==="DEF"?o.note_d:r==="MIL"?o.note_m:o.note_a)||0;e+=n+(o.boost||0)});return e}function Or(t,e){const i={};for(const a of["GK","DEF","MIL","ATT"])(t[a]||[]).forEach((d,c)=>{i[`${a}${c+1}`]=d});const o=Ii(e)||Ai[e]||[];let r=0,n=0;for(const[a,d]of o){const c=i[a],s=i[d];if(!c||!s)continue;const l=Ft(c,s);l==="#00ff88"?n++:l==="#FFD700"&&r++}return{jaune:r,vert:n}}function qo(t){const e=t.job||"ATT";return Number(e==="GK"?t.note_g:e==="DEF"?t.note_d:e==="MIL"?t.note_m:t.note_a)||0}function xi(t,e,i){if(i<=0||!t.length)return[];const o=[...t].sort((r,n)=>Math.abs(qo(r)-e)-Math.abs(qo(n)-e));return o.slice(0,Math.min(i,o.length))}async function Bo(t,e){var j,L;const i=Number(e==null?void 0:e.target_note_avg)||10,o=Number(e==null?void 0:e.nb_liens_jaune)||0,r=Number(e==null?void 0:e.nb_liens_vert)||0,n=Number(e==null?void 0:e.nb_joueurs_stade)||0,{data:a}=await v.from("players").select("id,firstname,surname_real,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,clubs(encoded_name,logo_url,country_code)").eq("is_active",!0).limit(300);if(!a||a.length<16)return{lines:yn(t),subs:[],gcCards:[],stadiumDef:null};const d={};a.forEach(_=>{_.club_id&&(d[_.club_id]=(d[_.club_id]||0)+1)});const c=((j=Object.entries(d).sort((_,G)=>G[1]-_[1])[0])==null?void 0:j[0])||null,s=c?a.filter(_=>_.club_id===c):[],l=((L=s[0])==null?void 0:L.country_code)||null,p=l?a.filter(_=>_.country_code===l&&_.club_id!==c):[],x=Math.min(s.length,Math.max(2,r+1),16),g=Math.min(p.length,Math.max(0,o),16-x),b=[],f=new Set;xi(s,i,x).forEach(_=>{b.push(_),f.add(_.id)}),xi(p,i,g).forEach(_=>{b.push(_),f.add(_.id)});const y=Math.max(0,16-b.length),u=a.filter(_=>!f.has(_.id));xi(u,i,y).forEach(_=>{b.push(_),f.add(_.id)});const w=St[t]||St["4-4-2"],h={GK:[],DEF:[],MIL:[],ATT:[]},E=new Set;function m(_,G,O){var le,pe;return E.add(_.id),{cardId:"ai-"+_.id+"-"+O,id:_.id,firstname:_.firstname,name:_.surname_real,country_code:_.country_code,club_id:_.club_id,job:_.job,job2:_.job2,note_g:Number(_.note_g)||0,note_d:Number(_.note_d)||0,note_m:Number(_.note_m)||0,note_a:Number(_.note_a)||0,rarity:_.rarity,skin:_.skin,hair:_.hair,hair_length:_.hair_length,face:_.face,clubName:((le=_.clubs)==null?void 0:le.encoded_name)||null,clubLogo:((pe=_.clubs)==null?void 0:pe.logo_url)||null,boost:0,used:!1,_line:G}}for(const _ of["GK","DEF","MIL","ATT"]){const G=b.filter(H=>H.job===_&&!E.has(H.id)),O=b.filter(H=>H.job!==_&&!E.has(H.id)),le=[...G,...O],pe=[];for(let H=0;H<w[_];H++){const fe=le[H];fe&&pe.push(m(fe,_,H))}const M=lt(pe.length);pe.forEach((H,fe)=>{H._col=M[fe]}),h[_]=pe}let T=b.filter(_=>!E.has(_.id));if(T.length<5){const _=a.filter(G=>!E.has(G.id)&&!T.some(O=>O.id===G.id));T=T.concat(xi(_,i,5-T.length))}const $=T.slice(0,5).map((_,G)=>m(_,_.job,100+G)),{data:I}=await v.from("gc_definitions").select("*").eq("is_active",!0),V=[...I||[]].sort(()=>Math.random()-.5).slice(0,3).map((_,G)=>({id:"ai-gc-"+G,gc_type:_.name,name:_.name,icon:"⚡",effect_type:_.effect_type,effect_params:_.effect_params||{}}));let Y=null;if(n>0&&c){const{data:_}=await v.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",c).single();_&&(Y={club_id:_.id,country_code:null,name:_.encoded_name+" Stadium",club:{encoded_name:_.encoded_name,logo_url:_.logo_url}})}return{lines:h,subs:$,gcCards:V,stadiumDef:Y}}function yn(t){const e=St[t]||St["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},o=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let r=0;for(const n of["GK","DEF","MIL","ATT"]){const a=[];for(let c=0;c<e[n];c++){const s=3+Math.floor(Math.random()*5);a.push({cardId:"fake-"+r,id:"fake-"+r,firstname:"IA",name:o[r%o.length],country_code:"XX",club_id:null,job:n,job2:null,note_g:n==="GK"?s:2,note_d:n==="DEF"?s:2,note_m:n==="MIL"?s:2,note_a:n==="ATT"?s:2,rarity:"normal",boost:0,used:!1,_line:n}),r++}const d=lt(a.length);a.forEach((c,s)=>{c._col=d[s]}),i[n]=a}return i}function Hr(t,e,i){var r;Wo("/sounds/match-opening.mp3",.3);const o=`<div style="position:relative;width:22px;height:22px;flex-shrink:0;display:flex;align-items:center;justify-content:center">
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">
      ${(()=>{if(e.phase==="defense"&&e.pendingAttack){const H=e.pendingAttack;let fe="";if(e.selected.length>0){const be=e.selected.map(de=>({...(e.homeTeam[de._role]||[]).find(W=>W.cardId===de.cardId)||de,_line:de._role})),ge=pi(be,e.modifiers.home,e.formation);fe=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
              ...
              <div style="display:flex;justify-content:center">${et(be.map(de=>({...de,used:!1})),"#3a7bd5",ge.total,"defense",e.formation)}</div>
          ...
          </div>`}if(e.phase==="attack"&&e.selected.length>0){const H=e.selected.map(be=>{const ge=(e.homeTeam[be._role]||[]).find(q=>q.cardId===be.cardId)||be,de=["GK","DEF"].includes(be._role);return{...ge,_line:be._role,...de?{note_a:Math.max(1,Number(ge.note_a)||0)}:{}}}),fe=ci(H,e.modifiers.home,e.formation);return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #FFD700;text-align:center">
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start;justify-content:center">
      ${t.map(p=>{const x=p._line||p.job||"MIL",g=d.find(f=>f.cardId===p.cardId),b=ze({...p,_evolution_bonus:0},{width:90,showStad:!0,role:x,extraNote:p.boost||0});return`<div class="gc-pick-item" data-cid="${p.cardId}"
    ...
    <div style="text-align:center;padding:40px;color:#fff;max-width:380px;width:100%">
      ...
      </div>`}const Fe=(Q,ve)=>ve?Xt(95,162):rt(Q,95),Je=(Q,ve)=>ve?Xt(68,116):rt(Q,68),Ke=ne?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",gt=B?Le(f)?`<button id="pvp-action" style="${Ke};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${K.length===0?"disabled":""}>⚔️ ATTAQUEZ <span id="pvp-timer"></span></button>`:`<button id="pvp-action" data-pass="1" style="${Ke};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER <span id="pvp-timer"></span></button>`:F?`<button id="pvp-action" style="${Ke};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${K.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="pvp-timer"></span></button>`:`<div style="font-size:11px;color:rgba(255,255,255,0.3);text-align:center;padding:4px">⏳ Tour de ${N}</div>`,at=B&&!Le(f)?'<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">Aucun attaquant — passez la main</div>':B||F?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${K.length}/3 sélectionné(s)</div>`:"",Pt=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${ne?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
      ${se.length===0?'<div style="font-size:7px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':se.map(Q=>`<div class="pvp-sub-btn" data-sub-id="${Q.cardId}" style="cursor:pointer;flex-shrink:0">${Et(Q,ne?76:44,ne?100:58)}</div>`).join("")}
    </div>`,bt=B?"attack":F?"defense":"idle",Tt=`<div style="overflow:hidden;min-width:0;flex:1;min-height:0;display:flex;flex-direction:column" id="match-field">
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${Kt(k,m[f+"Formation"],bt,Z,ne?1300:jt(),ne?600:Ht(),Ue)}
      </div>
    </div>`,st=m[f+"Team"],Sn=(()=>{var ve,qe,Te,Nt,dt;if(F&&((ve=m.pendingAttack)!=null&&ve.players)){const Pe=m.pendingAttack;let Ge="";if(K.length>0){const Be=K.map(Qe=>{const Jt=(st[Qe._role]||[]).find(Bi=>Bi.cardId===Qe.cardId)||Qe,Qt=st[Qe._role]||[],fi=Qt.findIndex(Bi=>Bi.cardId===Qe.cardId),qi=lt(Qt.length),Zt=fi>=0?qi[fi]:Jt._col??1;return{...Jt,_line:Qe._role,_col:Zt}}),xt=pi(Be,((qe=m.modifiers)==null?void 0:qe[f])||{},m[f+"Formation"]);Ge=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
        ...
        </div>`}if(B&&!((Nt=m.pendingAttack)!=null&&Nt.players)&&K.length>0){const Pe=K.map(Be=>{const xt=(st[Be._role]||[]).find(Zt=>Zt.cardId===Be.cardId)||Be,Qe=["GK","DEF"].includes(Be._role),Jt=st[Be._role]||[],Qt=Jt.findIndex(Zt=>Zt.cardId===Be.cardId),fi=lt(Jt.length),qi=Qt>=0?fi[Qt]:xt._col??1;return{...xt,_line:Be._role,_col:qi,...Qe?{note_a:Math.max(1,Number(xt.note_a)||0)}:{}}}),Ge=ci(Pe,((dt=m.modifiers)==null?void 0:dt[f])||{},m[f+"Formation"]);return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #FFD700;text-align:center">
        ...
        </div>`}const Q=(m.log||[]).slice(-1)[0];return Q?'<div style="padding:2px 4px">'+bn(Q)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3);text-align:center">⏳ Match en cours...</div>'})(),go=`
      <div style="display:flex;align-items:center;padding:8px 10px;background:rgba(0,0,0,0.5);gap:6px;flex-shrink:0">
      ...
      <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
        ...
        <div id="mobile-play-area" style="flex:1;min-height:0;display:flex;overflow:hidden">
          <div id="match-field" style="flex:1;min-width:0;min-height:0;overflow:hidden">
            <div class="terrain-wrapper" style="width:100%;height:100%;overflow:hidden">
              ${Kt(k,m[f+"Formation"],bt,Z,jt(),Ht(),Ue)}
            </div>
          </div>
        </div>
        <div id="mobile-action-bar" style="position:absolute;left:0;right:0;bottom:0;z-index:20;background:rgba(0,0,0,0.55);padding:6px 8px 8px;display:flex;flex-direction:column;gap:6px;box-shadow:0 -4px 16px rgba(0,0,0,0.5)">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      </div>`,(Ne=oe.querySelector("#psub-close"))==null||Ne.addEventListener("click",()=>oe.remove()),(je=oe.querySelector("#pout-up"))==null||je.addEventListener("click",()=>{F>0&&(F--,te())}),(Xe=oe.querySelector("#pout-down"))==null||Xe.addEventListener("click",()=>{F<N.length-1&&(F++,te())}),(Ue=oe.querySelector("#pin-up"))==null||Ue.addEventListener("click",()=>{Z>0&&(Z--,te())}),(rt=oe.querySelector("#pin-down"))==null||rt.addEventListener("click",()=>{Z<B.length-1&&(Z++,te())});const Ae=(Fe,Je,Ke,gt)=>{const at=oe.querySelector("#"+Fe);if(!at)return;let Pt=0;at.addEventListener("touchstart",bt=>{Pt=bt.touches[0].clientY},{passive:!0}),at.addEventListener("touchend",bt=>{const Tt=bt.changedTouches[0].clientY-Pt;if(Math.abs(Tt)<30)return;const st=Je();Tt<0&&st<gt-1?(Ke(st+1),te()):Tt>0&&st>0&&(Ke(st-1),te())},{passive:!0})};Ae("pin-panel",()=>Z,Fe=>Z=Fe,B.length),Ae("pout-panel",()=>F,Fe=>F=Fe,N.length),(Xt=oe.querySelector("#psub-confirm"))==null||Xt.addEventListener("click",async Fe=>{if(Fe.preventDefault(),Fe.stopPropagation(),ne)return;ne=!0;const Je=N[F],Ke=B[Z];if(!Je||!Ke)return;const gt=Je._line,at=(z[gt]||[]).findIndex(Tt=>Tt.cardId===Je.cardId);if(at===-1){b("Erreur : joueur introuvable","error"),oe.remove();return}const Pt={...Ke,_line:gt,position:Je.position,used:!1,boost:0};z[gt].splice(at,1,Pt);const bt=[...C,Ke.cardId];oe.remove(),ie(Je,Ke,async()=>{await L({[f+"Team"]:z,[y+"Team"]:m[y+"Team"],["usedSubIds_"+f]:bt})})})}document.body.appendChild(oe),te()}function ie(k,S,z){const C={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},A=document.createElement("div");A.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const N=(K,Z,ne)=>`<div style="text-align:center">
    ...
    </div>`,(A=document.getElementById("pvp-home"))==null||A.addEventListener("click",()=>{try{v.removeChannel(j)}catch{}Ye(t),l?g("mini-league",{openLeagueId:l}):g("home")})}H()}async function Xi(t,e,i=!1){var d,c;const{state:o,navigate:r,toast:n}=e,a=i&&((c=(d=e==null?void 0:e.state)==null?void 0:d.params)==null?void 0:c.rankedData)||null;await Ci(t,e,i?"ranked":"random",async({deckId:s,formation:l,starters:p,subsRaw:x,gcCardsEnriched:g,gcDefs:b,stadiumDef:f})=>{ct(t);const y=async u=>{await la(t,e,s,l,p,x,u||[],b,f,i,a)};if(!(g!=null&&g.length)){await y([]);return}zi(t,g,y)})}async function da(t,e,i){return sa(t,e,i)}async function la(t,e,i,o,r,n,a=[],d=[],c=null,s=!1,l=null){var Y,j;const{state:p,navigate:x,toast:g}=e,b=(l==null?void 0:l.mmr)??((Y=p.profile)==null?void 0:Y.mmr)??1e3,f=s?zt(b):null,y=s?"linear-gradient(135deg, #1a0a2e 0%, #2d1060 50%, #1a0a2e 100%)":"linear-gradient(135deg, #0a1a2e 0%, #0d3d1e 100%)",u=s?(f==null?void 0:f.color)||"#D4A017":"#FFD700";let w=!1,h=null,E=null;t.innerHTML=`
    <div style="min-height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;background:${y};color:#fff;padding:32px;text-align:center;gap:20px">
      ...
      ${s?`<div style="font-size:13px;color:${(f==null?void 0:f.color)||"#D4A017"}">${(f==null?void 0:f.name)||""} · ${Math.round(b)} MMR</div>`:""}
    ...
    </div>`;const m=async()=>{w=!0,h&&(h.unsubscribe(),h=null),E&&(clearInterval(E),E=null);try{await v.rpc("cancel_matchmaking",{p_user_id:p.profile.id})}catch(L){console.error("[Matchmaking] cancel error:",L)}};(j=document.getElementById("cancel-mm"))==null||j.addEventListener("click",async()=>{await m(),Ye(t),x("home")});const T=async(L,_)=>{w||(w=!0,h&&(h.unsubscribe(),h=null),E&&(clearInterval(E),E=null),await Dt(t,e,L,_,{myGC:a,gcDefs:d,stadiumDef:c,isRanked:s,rankedData:l,onMatchEnd:s?ca:null}))},$=p.user.id,{data:I,error:D}=s?await v.rpc("try_matchmake_ranked",{p_user_id:$,p_deck_id:i,p_mmr:b,p_range:200}):await v.rpc("try_matchmake",{p_user_id:$,p_deck_id:i});if(D||!(I!=null&&I.success)){console.error("[Matchmaking] try_matchmake error:",D||I),g("Erreur de matchmaking","error"),Ye(t),x("home");return}if(I.matched){await T(I.match_id,!1);return}const V=async()=>{if(w)return;const{data:L}=await v.from("matchmaking_queue").select("status, match_id").eq("user_id",$).maybeSingle();(L==null?void 0:L.status)==="matched"&&L.match_id&&await T(L.match_id,!0)};h=v.channel(`mm_${$}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${$}`},L=>{const _=L.new;_.status==="matched"&&_.match_id&&T(_.match_id,!0)}).subscribe(),E=setInterval(V,3e3),s?setTimeout(async()=>{if(!w){w=!0,h&&(h.unsubscribe(),h=null),E&&(clearInterval(E),E=null);try{await v.rpc("cancel_matchmaking",{p_user_id:p.profile.id})}catch(L){console.error("[Matchmaking] cancel error:",L)}g("Aucun adversaire trouvé — match contre une IA calibrée sur votre niveau","info",4e3),x("match",{matchMode:"ranked_ai",rankedData:l,presetSetup:{deckId:i,formation:o,starters:r,subsRaw:n,gcCardsEnriched:a,gcDefs:d,stadiumDef:c}})}},2e4):setTimeout(()=>{w||m()},12e4)}async function ca(t,e){const{state:i,toast:o}=e,{winnerId:r,homeId:n,awayId:a,homeScore:d,awayScore:c,matchId:s}=t,l=i.user.id,p=n===l,x=r===l,{data:g}=await v.from("users").select("mmr, mmr_rd, mmr_v").eq("id",l).single(),{data:b}=await v.from("users").select("mmr, mmr_rd, mmr_v").eq("id",p?a:n).single();if(g&&b){const f=Ko(g.mmr,g.mmr_rd,g.mmr_v,b.mmr,b.mmr_rd,x?1:0);await v.from("users").update({mmr:f.mmr,mmr_rd:f.rd,mmr_v:f.v,mmr_wins:x?v.sql`mmr_wins + 1`:void 0,mmr_losses:x?void 0:v.sql`mmr_losses + 1`}).eq("id",l),o(x?`+MMR ↑ ${Math.round(f.mmr-g.mmr)}`:`-MMR ↓ ${Math.round(g.mmr-f.mmr)}`,x?"success":"error",4e3)}}const pa=Object.freeze(Object.defineProperty({__proto__:null,renderMatchRandom:Xi,resumePvpMatch:da},Symbol.toStringTag,{value:"Module"}));async function ua(t,e){const{state:i,navigate:o,toast:r}=e,n=i.params||{},a=n.friendId||null,d=n.friendName||"Ami",c=!!n.isAccepting;await Ci(t,e,"friend",async({deckId:s,formation:l,starters:p,subsRaw:x,gcCardsEnriched:g,gcDefs:b,stadiumDef:f})=>{ct(t);const y=async u=>{const w=u||[];c?await ma(t,e,s,w,b,f):await fa(t,e,s,w,b,f,a,d)};if(!(g!=null&&g.length)){await y([]);return}zi(t,g,y)})}async function fa(t,e,i,o,r,n,a,d){var w;const{state:c,navigate:s,toast:l}=e,p=c.user.id,{data:x,error:g}=await v.from("friend_match_invites").insert({inviter_id:p,invitee_id:a,friend_id:a,status:"pending",inviter_deck_id:i,expires_at:new Date(Date.now()+2*60*1e3).toISOString()}).select("id").single();if(g||!x){console.error("[Friend] Erreur création invitation:",g),l("Impossible de créer l'invitation","error"),s("home");return}t.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="display:flex;flex-wrap:wrap;gap:8px;font-size:11px;color:${At};margin-bottom:${i||d?"10px":"0"}">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:${ii};border:1px solid ${oi};border-radius:12px;padding:16px">
        ...
        <div style="font-size:12px;color:${At};margin-bottom:10px">💰 ${f} cr./joueur → Pot estimé : ${(f*p.length).toLocaleString("fr")} cr. (🥇${Math.floor(f*p.length*.7).toLocaleString("fr")} · 🥈${Math.floor(f*p.length*.2).toLocaleString("fr")} · 🥉${Math.floor(f*p.length*.1).toLocaleString("fr")})</div>
        ${p.map(L=>`
      ...
      <div style="background:${ii};border:1px solid ${oi};border-radius:12px;padding:16px">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>${x.map((L,_)=>{const G=b>0&&a.status==="finished"?_===0?Math.floor(b*.7):_===1?Math.floor(b*.2):_===2?Math.floor(b*.1):0:0;return`<tr style="border-bottom:1px solid ${Mt};${L.userId===n?"background:rgba(74,222,128,0.08);":""}">
              <td style="padding:7px 3px 7px 0;font-weight:700;color:${_===0?pt:_<3?"#4ade80":At}">${["🥇","🥈","🥉"][_]||_+1}</td>
              ...
              <td style="text-align:center;color:${At}">${L.played}</td><td style="text-align:center;color:${At}">${L.won}-${L.drawn}-${L.lost}</td>
              ...
              ${b>0&&a.status==="finished"?`<td style="text-align:right;font-weight:700;color:${pt}">${G?G.toLocaleString("fr")+" cr.":"—"}</td>`:""}
            </tr>`}).join("")}</tbody>
        </table>
      </div>`:""}
    ...
    </div>`;let l=!1;const p=async y=>{if(!l){if(l=!0,clearInterval(b),g)try{g.unsubscribe()}catch{}y&&await Dt(t,e,y,r,{mlLeagueId:o,mlMatchId:i,myGC:n||[],gcDefs:a,stadiumDef:d}),c()}};(f=document.getElementById("wait-back"))==null||f.addEventListener("click",()=>{if(l=!0,clearInterval(b),g)try{g.unsubscribe()}catch{}s("mini-league",{openLeagueId:o}),c()});const x=async()=>{if(l)return;const{data:y}=await v.from("mini_league_matches").select("match_id").eq("id",i).single();y!=null&&y.match_id&&p(y.match_id)},g=v.channel(`ml_match_${i}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"mini_league_matches",filter:`id=eq.${i}`},y=>{var u;(u=y.new)!=null&&u.match_id&&p(y.new.match_id)}).subscribe(),b=setInterval(x,3e3);x()})}const No={GK:"#111111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function hi(t,e,i=0){return e?(Number(e==="GK"?t.note_g:e==="DEF"?t.note_d:e==="MIL"?t.note_m:t.note_a)||0)+(e===t.job||e===t.job2?i:0):0}async function Aa(t,e){t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',await mo(t,e)}async function mo(t,e){const{state:i,toast:o,navigate:r}=e;Yt(r,i.profile,"market","/icons/",o);const{data:n}=await v.from("market_listings").select(`id, price, status, listed_at, seller_id,
  ...
  <div style="min-height:100%;background:${w[b.id]};padding:16px;overflow-y:auto;display:flex;flex-direction:column;gap:16px">
    ...
    <div style="background:rgba(0,0,0,0.3);border-radius:12px;padding:12px 16px">
      ...
      <div style="background:rgba(255,255,255,0.1);border-radius:6px;height:10px;overflow:hidden">
        <div style="height:100%;width:${f}%;background:linear-gradient(90deg,${b.color},${b.color}aa);border-radius:6px;transition:width 0.8s ease"></div>
      </div>
    ...
    <div style="display:flex;flex-direction:column;gap:10px;margin-top:auto;padding-top:8px">
      ...
      away:users!away_id(pseudo,club_name)`).or(`home_id.eq.${r.id},away_id.eq.${r.id}`).order("created_at",{ascending:!1}).limit(50),a={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},d=(n||[]).filter(l=>l.status==="finished"),c=(n||[]).filter(l=>l.status==="in_progress");function s(l){const p=l.home_id===r.id;p?l.home_score:l.away_score,p?l.away_score:l.home_score;const x=l.winner_id===r.id,g=l.home_score===l.away_score&&l.status==="finished",b=l.status!=="finished"?"…":g?"N":x?"V":"D",f=l.status!=="finished"||g?"#888":x?"#1A6B3C":"#c0392b";let y=a[l.mode]||l.mode;l.away_id===null&&!y.startsWith("IA")&&(y="IA");const w=l.home_id===r.id?l.away:l.home;let h;l.away_id===null?h=y:w?h=`${w.club_name||w.pseudo} (${w.pseudo})`:h="Adversaire";let E="";l.status==="in_progress"&&Date.now()-new Date(l.created_at).getTime()>3600*1e3&&(E=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const m=new Date(l.created_at),T=m.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+m.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),$=l.status==="finished"?`${l.home_score} - ${l.away_score}`:`${l.home_score||0} - ${l.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
      ...
      <div style="display:flex;align-items:center;gap:8px">
        ...
        <span style="background:${f};color:#fff;width:22px;height:22px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:900">${b}</span>
      </div>
    </div>`}t.innerHTML=`
  <div class="page">
    <div class="page-header">
