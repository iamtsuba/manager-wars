...
import{s as v,n as Ht,o as Oi,F as Hi,l as tt,k as ei,r as Ie,j as At,p as zi,h as Ui,q as io,t as no,T as Ei,m as oo,u as ro,b as ao}from"./special-cards-CiJC3HtT.js";const so="/";function sn(e,{navigate:t,toast:i}){let n="login";const r=()=>{var a,s,d,c,l,f;const o=n==="login";e.innerHTML=`
    <div style="
    ...
    ">
      ...
      <div style="margin-bottom:28px;text-align:center;animation:authFadeIn .5s ease">
        <img src="${so}icons/logo-withname.png" alt="Manager Wars" style="height:72px;width:auto;filter:drop-shadow(0 4px 16px rgba(212,160,23,0.4))">
  ...
  `;function s(){var y;const c=document.getElementById("logo-preview"),l=document.getElementById("logo-initials"),f=((y=document.getElementById("setup-club"))==null?void 0:y.value)||"MW",x=f.trim().split(" ").filter(Boolean),g=x.length>=2?(x[0][0]+x[1][0]).toUpperCase():f.slice(0,2).toUpperCase();c&&(c.style.background=a,c.style.borderColor=o),l&&(l.textContent=g,l.style.color=o)}document.getElementById("color1").addEventListener("input",c=>{o=c.target.value,document.getElementById("swatch1").style.background=o,s()}),document.getElementById("color2").addEventListener("input",c=>{a=c.target.value,document.getElementById("swatch2").style.background=a,s()});function d(c){document.querySelectorAll(".setup-step").forEach(l=>l.classList.remove("active")),document.getElementById(`step-${c}`).classList.add("active"),document.getElementById("step-num").textContent=c,document.getElementById("progress-fill").style.width=`${Math.round(c/3*100)}%`,c===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("step1-error");if(l.textContent="",c.length<3){l.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:f}=await v.from("users").select("id").eq("pseudo",c).maybeSingle();if(f){l.textContent="Ce pseudo est déjà pris.";return}d(2)}),document.getElementById("step2-back").addEventListener("click",()=>d(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const c=document.getElementById("setup-club").value.trim(),l=document.getElementById("step2-error");if(l.textContent="",c.length<2){l.textContent="Nom trop court (min. 2 caractères).";return}const{data:f}=await v.from("users").select("id").eq("club_name",c).maybeSingle();if(f){l.textContent="Ce nom de club est déjà pris.";return}d(3)}),document.getElementById("step3-back").addEventListener("click",()=>d(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("setup-club").value.trim(),f=document.getElementById("step3-error"),x=document.getElementById("step3-finish");f.textContent="",x.disabled=!0,x.textContent="Création en cours…";try{const{error:g}=await v.from("users").insert({id:t.user.id,pseudo:c,club_name:l,club_color1:o,club_color2:a,credits:1e4});if(g)throw g;await co(t.user.id),await r(),n(`Bienvenue ${c} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(g){f.textContent=g.message,x.disabled=!1,x.textContent="🚀 Créer mon profil !"}})}async function co(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await v.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const po="modulepreload",uo=function(e){return"/"+e},dn={},kn=function(t,i,n){let r=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),s=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));r=Promise.allSettled(i.map(d=>{if(d=uo(d),d in dn)return;dn[d]=!0;const c=d.endsWith(".css"),l=c?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${l}`))return;const f=document.createElement("link");if(f.rel=c?"stylesheet":po,c||(f.as="script"),f.crossOrigin="",f.href=d,s&&f.setAttribute("nonce",s),document.head.appendChild(f),c)return new Promise((x,g)=>{f.addEventListener("load",x),f.addEventListener("error",()=>g(new Error(`Unable to preload CSS for ${d}`)))})}))}function o(a){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=a,window.dispatchEvent(s),!s.defaultPrevented)throw a}return r.then(a=>{for(const s of a||[])s.status==="rejected"&&o(s.reason);return t().catch(o)})},ti="#1A6B3C",ii="#cc2222",fo="#D4A017",ln="#888";async function mo(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      ${x.map(({friendshipId:g,friend:y})=>go(y,g)).join("")}
    </div>`,c.querySelectorAll("[data-stats]").forEach(g=>{g.addEventListener("click",()=>xo(e,g.dataset.stats,g.dataset.friendName))}),c.querySelectorAll("[data-match]").forEach(g=>{g.addEventListener("click",async()=>{const y=g.dataset.friendId,p=g.dataset.friendName;if(typeof n!="function"){t("Erreur navigation","error");return}const u=e.user.id,{data:b}=await v.from("friend_match_invites").select("id").eq("inviter_id",y).eq("invitee_id",u).eq("status","pending").order("created_at",{ascending:!1}).limit(1).maybeSingle(),_=!!b;console.log("[Friends] clic match",{fid:y,fname:p,isAccepting:_}),n("match",{matchMode:"friend",friendId:y,friendName:p,isAccepting:_})})})}function go(e,t){const i=e.club_name||e.pseudo||"?",n=e.pseudo||"",r=(n||i).slice(0,2).toUpperCase(),o=e.club_color2||ti,a=e.club_color1||"#ffffff",s=e.last_seen_at?new Date(e.last_seen_at):null,d=s&&Date.now()-s.getTime()<3*60*1e3;return`
    <div style="display:flex;align-items:center;gap:12px;background:var(--tile-bg);border-radius:12px;padding:12px 14px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
      ...
      <div style="display:flex;gap:8px;flex-shrink:0">
        ...
        <button data-stats="${e.id}" data-friend-name="${i}" title="Voir les stats"
          style="width:38px;height:38px;border-radius:50%;border:2px solid ${fo};background:var(--tile-bg);font-size:16px;cursor:pointer;display:flex;align-items:center;justify-content:center">📊</button>
      </div>
    </div>`}function yo(e,t){const i=Ki();i.innerHTML=`
    <div class="popup-box">
    ...
    </style>`}const Ln="mw_sound_muted";function ni(){return localStorage.getItem(Ln)==="1"}function bo(e){localStorage.setItem(Ln,e?"1":"0")}function ho(e,t=1){if(ni())return null;try{const i=new Audio(e);return i.volume=t,i.play().catch(()=>{}),i}catch{return null}}let Et=null,Mi=null;function Tn(e,t=.3){if(!(Et&&Mi===e&&!Et.paused)&&(Pt(),!ni()))try{const i=new Audio(e);i.loop=!0,i.volume=t,i.play().catch(()=>{}),Et=i,Mi=e}catch{}}function Pt(){if(Et)try{Et.pause(),Et.currentTime=0}catch{}Et=null,Mi=null}let Wt=null;function In(e,t=.6){if(Ue(),!ni())try{const i=new Audio(e);i.volume=t,i.play().catch(()=>{}),Wt=i}catch{}}function Ue(){if(Wt)try{Wt.pause(),Wt.currentTime=0}catch{}Wt=null}const vo="2026.07.21-1723";async function cn(e,{state:t,navigate:i,toast:n}){var o,a;const r=t.profile;r&&(e.innerHTML=`
  <style>
    .home-dark {
      min-height: 100%;
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(fe=document.getElementById("close-detail"))==null||fe.addEventListener("click",s);let K=new Set;const X=()=>{const G=K.size,O=document.getElementById("sell-action-panel");if(!O)return;O.style.display=G>0?"block":"none",document.getElementById("sell-selected-count").textContent=G;const $=document.getElementById("evolve-btn");$&&($.textContent=`⬆️ Évoluer ${G>1?"(+"+G+")":""}`)};document.querySelectorAll(".expl-check").forEach(G=>{G.addEventListener("change",()=>{const O=G.dataset.id;G.checked?K.add(O):K.delete(O);const $=G.closest(".exemplaire-row");$&&($.style.borderColor=G.checked?"#1A6B3C":"#eee"),X()})}),document.querySelectorAll(".exemplaire-row").forEach(G=>{G.addEventListener("click",O=>{if(O.target.closest("button")||O.target.tagName==="INPUT")return;const $=G.querySelector(".expl-check");$&&!$.disabled&&($.checked=!$.checked,$.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(G=>{G.addEventListener("click",O=>{O.stopPropagation();const $=document.querySelector(`.expl-hist[data-hist="${G.dataset.idx}"]`);$&&($.style.display=$.style.display==="none"?"flex":"none")})}),(M=document.getElementById("evolve-btn"))==null||M.addEventListener("click",async()=>{if(x<=1)return;const G=[...K];if(!G.length)return;if(K.has(e.id)){const B=document.createElement("div");B.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",B.innerHTML=`
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    </div>`}function Hn(e){var o,a,s;if(!e)return"";const t=d=>d?Ie({...d,_evolution_bonus:0},{width:52,role:d._line||d.job,showStad:!!d.stadiumBonus,extraNote:d.boost||0}):"",n={goal:"⚽","goal-home":"⚽","goal-ai":"⚽",duel:"⚔️",midfield:"🎯",sub:"🔄",gc:"⚡",boost:"💥","defense-won":"🛡️","attack-won":"⚔️","defense-lost":"😓","attack-lost":"😓"}[e.type]||"📋";return`
    ...
    pointer-events:none;white-space:nowrap;max-width:90vw;text-align:center`,r.textContent=e,!document.getElementById("game-toast-style")){const o=document.createElement("style");o.id="game-toast-style",o.textContent=`
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${e.map((o,a)=>{const s=a<e.length-1?Ht(o,e[a+1]):null,d=!s||s==="#ff3333"||s==="#cc2222",c=s==="#00ff88"?"+10":s==="#FFD700"?"+5":"";return Un(o,r),o.stadiumBonus||r&&(r.club_id&&String(o.club_id)===String(r.club_id)||r.country_code&&(o.country_code,r.country_code)),`
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const Y=t.pendingAttack;let pe="";if(t.selected.length>0){const G=t.selected.map($=>({...(t.homeTeam[$._role]||[]).find(V=>V.cardId===$.cardId)||$,_line:$._role})),O=ri(G,t.modifiers.home,t.formation);pe=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
          ...
          </div>`}const M=t.log[t.log.length-1];return M?'<div style="padding:2px 4px">'+Hn(M)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})()}
    </div>
  ...
  <div id="match-history-panel">
    ...
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
    ...
    </div>`,rt=m[p+"Team"],to=(()=>{var me,ze,_e,jt,at;if(F&&((me=m.pendingAttack)!=null&&me.players)){const Fe=m.pendingAttack;let Re="";if(U.length>0){const Me=U.map(We=>{const Kt=(rt[We._role]||[]).find($i=>$i.cardId===We.cardId)||We,Vt=rt[We._role]||[],di=Vt.findIndex($i=>$i.cardId===We.cardId),ki=gt(Vt.length),Yt=di>=0?ki[di]:Kt._col??1;return{...Kt,_line:We._role,_col:Yt}}),ft=ri(Me,((ze=m.modifiers)==null?void 0:ze[p])||{},m[p+"Formation"]);Re=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
        ...
        </div>`}const W=(m.log||[]).slice(-1)[0];return W?'<div style="padding:2px 4px">'+Hn(W)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3);text-align:center">⏳ Match en cours...</div>'})(),Zi=`
      <div style="display:flex;align-items:center;padding:8px 10px;background:rgba(0,0,0,0.5);gap:6px;flex-shrink:0">
      ...
      <div style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">${to}</div>
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    <div style="padding:14px 16px;background:var(--nav-bg,#0d1a0f);border-bottom:1px solid ${Lt};display:flex;align-items:center;gap:10px">
      ...
      <div style="flex:1">
        ...
        <div style="font-size:11px;color:${Wn}">${a.mode==="aller-retour"?"Aller-Retour":"Aller"} · max ${a.max_players} · 💰 ${p} cr./joueur</div>
      </div>
      <div style="text-align:right;flex-shrink:0">
      ...
      <div style="background:${Xt};border:1px solid ${Jt};border-radius:12px;padding:16px">
        ...
        ${f.map(j=>`
          <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid ${Lt}">
            <div style="width:36px;height:36px;border-radius:50%;background:${j.club_color2||Yn};display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:900;color:${j.club_color1||"#fff"}">${(j.pseudo||"?").slice(0,2).toUpperCase()}</div>
  ...
  </div>`}oo(ao);const xe={user:null,profile:null,page:"home",params:{}};function Dt(e,t="info",i=3e3){const n=document.getElementById("toast");n&&(n.textContent=e,n.className=`show ${t}`,clearTimeout(n._t),n._t=setTimeout(()=>{n.className=""},i))}function sa(e,t,i=""){document.getElementById("modal-title").textContent=e,document.getElementById("modal-body").innerHTML=t,document.getElementById("modal-footer").innerHTML=i,document.getElementById("modal-overlay").classList.remove("hidden")}function Ri(){document.getElementById("modal-overlay").classList.add("hidden")}async function Qt(){if(!xe.user)return;const{data:e}=await v.from("users").select("*").eq("id",xe.user.id).single();e&&(xe.profile=e)}const Jn="mw_theme";function Qn(){return localStorage.getItem(Jn)||"dark"}function da(e){var t;localStorage.setItem(Jn,e),la(e),(t=xe.profile)!=null&&t.id&&v.from("users").update({theme:e}).eq("id",xe.profile.id).then(()=>{})}function la(e){document.documentElement.setAttribute("data-theme",e)}function Ft(e,t={}){xe.page=e,xe.params=t,Zn()}async function Zn(){var n,r,o,a;const e=document.getElementById("page-content");if(!e)return;document.querySelectorAll(".bottom-nav a").forEach(s=>{s.classList.toggle("active",s.dataset.page===xe.page)});const t=document.getElementById("nav-credits");t&&xe.profile&&(t.textContent=`💰 ${(xe.profile.credits||0).toLocaleString("fr")}`);const i={state:xe,navigate:Ft,toast:Dt,openModal:sa,closeModal:Ri,refreshProfile:Qt};switch(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽</div>',xe.page){case"home":await cn(e,i);break;case"settings":await Ci(e,i);break;case"collection":await Do(e,i);break;case"decks":await ji(e,i);break;case"boosters":await Xo(e,i);break;case"ranked":await ra(e,i);break;case"match":{const s=xe.params&&xe.params.matchMode||"vs_ai_easy";s==="random"?await Gi(e,i,!1):s==="ranked"?await Gi(e,i,!0):s==="friend"?await Mr(e,i,(n=xe.params)==null?void 0:n.friendId,(r=xe.params)==null?void 0:r.friendName):s==="mini_league"||s==="mini-league"?await _n(e,i,(o=xe.params)==null?void 0:o.mlMatchId,(a=xe.params)==null?void 0:a.leagueId):await pr(e,i);break}case"market":await ea(e,i);break;case"rankings":await oa(e,i);break;case"matches":await aa(e,i);break;case"friends":await mo(e,i);break;case"mini-league":await qr(e,i);break;case"match-mini-league":{const s=xe.params||{};await _n(e,i,s.mlMatchId,s.leagueId);break}default:await cn(e,i)}}function ca(){var n,r;const e=document.getElementById("app"),t=xe.profile;if(!t)return;const i="/icons/";e.innerHTML=`
    <nav class="top-nav">
      <div class="logo" id="nav-logo" title="Manager Wars v2026.07.21-1723" style="cursor:pointer">
  ...
  </div>`,(n=e.querySelector("#pl-play-btn"))==null||n.addEventListener("click",t)}async function fa(){document.documentElement.setAttribute("data-theme",Qn()),document.getElementById("modal-overlay").addEventListener("click",i=>{i.target===i.currentTarget&&Ri()}),document.getElementById("modal-close").addEventListener("click",Ri);const{data:{session:e}}=await v.auth.getSession();if(!e){wn(),ua(document.getElementById("app"),{onPlay:()=>sn(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:Dt})});return}xe.user=e.user,await Qt(),wn();try{const{data:i}=await v.from("formation_links_overrides").select("formation, links"),n={};(i||[]).forEach(r=>{n[r.formation]=r.links}),ro(n)}catch(i){console.warn("Impossible de charger les overrides de formation:",i)}if(!xe.profile){lo(document.getElementById("app"),{state:xe,navigate:async()=>{await Qt(),Ai()},toast:Dt,refreshProfile:Qt});return}const t=Array.isArray(xe.profile.pending_boosters)?xe.profile.pending_boosters:[];if(!xe.profile.onboarding_done&&t.length>0){or(document.getElementById("app"),{state:xe,navigate:()=>Ai(),toast:Dt,refreshProfile:Qt});return}Ai(),setTimeout(()=>Zr(xe.profile,Ft,Dt),800),v.auth.onAuthStateChange(async(i,n)=>{i==="SIGNED_OUT"&&(xe.user=null,xe.profile=null,document.getElementById("app").innerHTML="",sn(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:Dt}))})}function ma(){return Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight)}function wi(){const e=document.getElementById("app");e&&(e.style.height=ma()+"px")}window.addEventListener("resize",wi);window.addEventListener("orientationchange",()=>setTimeout(wi,150));window.visualViewport&&window.visualViewport.addEventListener("resize",wi);function Ai(){const e=()=>{var i;(i=xe.user)!=null&&i.id&&v.from("users").update({last_seen_at:new Date().toISOString()}).eq("id",xe.user.id).then(()=>{})};e(),window._presencePingInterval&&clearInterval(window._presencePingInterval),window._presencePingInterval=setInterval(e,6e4);const t=document.getElementById("app");t.style.display="flex",t.style.flexDirection="column",wi(),ca(),Zn()}function wn(){const e=document.getElementById("app-loader"),t=document.getElementById("app");t&&(t.style.display=""),e&&(e.classList.add("zoom-out"),setTimeout(()=>e.style.display="none",500))}function eo(e){var n;const t=document.getElementById("app-loader");if(t&&(t.style.display="none"),document.getElementById("boot-error"))return;const i=document.createElement("div");i.id="boot-error",i.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:99999;gap:16px;color:#fff;padding:24px;text-align:center",i.innerHTML=`
    <div style="font-size:42px">📡</div>
    <div style="font-size:18px;font-weight:900">Connexion impossible</div>
    <div style="font-size:13px;color:rgba(255,255,255,0.6);max-width:280px">${e||"Le chargement a échoué. Vérifie ta connexion et réessaie."}</div>
    <button id="boot-retry" style="margin-top:8px;padding:12px 30px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer">Réessayer</button>`,document.body.appendChild(i),(n=document.getElementById("boot-retry"))==null||n.addEventListener("click",()=>window.location.reload())}fa().catch(e=>{console.error("Échec du démarrage:",e),eo()});setTimeout(()=>{const e=document.getElementById("app-loader");e&&e.style.display!=="none"&&!e.classList.contains("zoom-out")&&!document.getElementById("boot-error")&&eo("Le serveur met trop de temps à répondre.")},12e3);
