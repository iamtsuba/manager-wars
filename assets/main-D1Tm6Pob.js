import{s as k,l as J,F as ee,b as ve,c as ue,d as et}from"./formation-links-BD9zicgB.js";function _e(t,{navigate:e,toast:i}){t.innerHTML=`
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),d=(l||[]).filter(_=>_.card_type==="player"&&_.player),c=(l||[]).filter(_=>_.card_type==="game_changer"),f=(l||[]).filter(_=>_.card_type==="formation"),p=Object.keys(ee),u=Object.keys(de),g={};d.forEach(_=>{const A=_.player.id;g[A]=(g[A]||0)+1});const m=new Set(Object.keys(g).map(_=>String(_))),x=new Set(f.map(_=>_.formation)),b=new Set(c.map(_=>_.gc_type));let h="player",w="Tous",v="",$=!1;function y(){return[...d].sort((_,A)=>{const T=oe.indexOf(_.player.job),j=oe.indexOf(A.player.job);return T!==j?T-j:(_.player.surname_encoded||"").localeCompare(A.player.surname_encoded||"")})}function E(){return[...r||[]].sort((_,A)=>{const T=oe.indexOf(_.job),j=oe.indexOf(A.job);return T!==j?T-j:(_.surname_encoded||"").localeCompare(A.surname_encoded||"")})}function I(){return y().filter(_=>{const A=_.player,T=w==="Tous"||A.job===w,j=!v||`${A.firstname} ${A.surname_encoded}`.toLowerCase().includes(v);return T&&j})}function L(){return E().filter(_=>{const A=w==="Tous"||_.job===w,T=!v||`${_.firstname} ${_.surname_encoded}`.toLowerCase().includes(v);return A&&T})}t.innerHTML=`
  <div class="page">
    ...
    <div class="page-body">
      <div style="display:flex;flex-wrap:wrap;gap:10px;justify-content:flex-start" id="col-grid"></div>
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",A=>{v=A.target.value.toLowerCase(),z()}),t.querySelectorAll(".filter-btn").forEach(A=>{A.addEventListener("click",()=>{w=A.dataset.job,M(),z()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{$=!$,M(),z()})):(_.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{$=!$,M(),z()})))}function z(){const _=document.getElementById("col-grid");_&&(h==="player"?B(_):h==="formation"?H(_):U(_))}function B(_){if($){const A=L();if(!A.length){_.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}_.innerHTML=A.map(T=>{if(m.has(String(T.id))){const F=d.find(P=>P.player.id===T.id),R=g[T.id]||1,q=R>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${R}</div>`:"",K=d.filter(P=>P.player.id===T.id&&P.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Ae(F,q+K)}return st(T)}).join("")}else{const A=I();if(!A.length){_.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const T={},j=[];A.forEach(F=>{const R=F.player.id;T[R]||(T[R]=!0,j.push(F))}),_.innerHTML=j.map(F=>{const R=g[F.player.id]||1,q=R>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${R}</div>`:"",K=d.filter(P=>P.player.id===F.player.id&&P.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Ae(F,q+K)}).join("")}_.querySelectorAll("[data-card-id]").forEach(A=>{A.addEventListener("click",()=>{const T=d.find(j=>j.id===A.dataset.cardId);T&&pt(T,d,g,e)})})}function H(_){const A=$?p:[...x];if(!A.length){_.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}_.innerHTML=A.map(T=>{if(x.has(T)){const F=f.find(O=>O.formation===T),R=f.filter(O=>O.formation===T).length,q=R>1?`<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${R}</div>`:"";return`
        ...
        <div style="
          ...
          <div style="font-weight:900;font-size:18px">${T}</div>
        </div>`}).join(""),_.querySelectorAll("[data-form-id]").forEach(T=>{T.addEventListener("click",()=>{const j=f.find(F=>F.id===T.dataset.formId);j&&ct(j,f,e,n)})})}function U(_){const A=$?u:[...b];if(!A.length){_.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}_.innerHTML=A.map(T=>{const j=b.has(T),F=de[T]||{icon:"⚡"};if(j){const R=c.find(K=>K.gc_type===T),q=c.filter(K=>K.gc_type===T).length,O=q>1?`<div style="position:absolute;top:4px;right:4px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${q}</div>`:"";return`
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Q=1e3;function ct(t,e,i,o){var m,x,b;const{state:a,toast:n,closeModal:s,navigate:l,refreshProfile:r}=i,d=t.formation,c={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function f(){const h=ve[d]||{},w=ee[d]||[],v=290,$=360,y=20;function E(L){const M=h[L];return M?{x:M.x*v,y:M.y*$}:null}let I=`<svg width="${v}" height="${$}" viewBox="0 0 ${v} ${$}" xmlns="http://www.w3.org/2000/svg">`;for(const[L,M]of w){const z=E(L),B=E(M);!z||!B||(I+=`<line x1="${z.x}" y1="${z.y}" x2="${B.x}" y2="${B.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const L of Object.keys(h)){const M=E(L);if(!M)continue;const z=L.replace(/\d+/,""),B=c[z]||"#555";I+=`<circle cx="${M.x}" cy="${M.y}" r="${y}" fill="${B}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,I+=`<text x="${M.x}" y="${M.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${z}</text>`}return I+="</svg>",I}const p=e.filter(h=>h.formation===d),u=p.length,g=!t.is_for_sale;o(`Formation ${d}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(m=document.getElementById("direct-sell-form-btn"))==null||m.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${d} pour ${Q.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const h=p.find(v=>!v.is_for_sale)||p[0];if(!h){n("Aucune carte à vendre","error");return}const{error:w}=await k.from("cards").delete().eq("id",h.id);if(w){n(w.message,"error");return}await k.from("users").update({credits:(a.profile.credits||0)+Q}).eq("id",a.profile.id),await r(),n(`+${Q.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),s(),l("collection")}),(x=document.getElementById("market-sell-form-btn"))==null||x.addEventListener("click",async()=>{const h=parseInt(document.getElementById("sell-price-form").value);if(!h||h<1){n("Prix invalide","error");return}await k.from("cards").update({is_for_sale:!0,sale_price:h}).eq("id",t.id),await k.from("market_listings").insert({seller_id:a.profile.id,card_id:t.id,price:h}),n("Carte mise en vente sur le marché !","success"),s(),l("collection")}),(b=document.getElementById("cancel-sell-form-btn"))==null||b.addEventListener("click",async()=>{await k.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",t.id),await k.from("market_listings").update({status:"cancelled"}).eq("card_id",t.id).eq("status","active"),n("Annonce retirée","success"),s(),l("collection")})}function pt(t,e,i,o){var y,E,I,L,M,z;const{state:a,toast:n,openModal:s,closeModal:l,navigate:r,refreshProfile:d}=o,c=t.player,f=e.filter(B=>B.player.id===c.id),p=f.length,u=at[c.rarity]||1e3,g=c.rarity!=="legende",m=Ke(c),x=te(c,c.job),b=c.job2?te(c,c.job2):null,h=le[c.job]||"#1A6B3C",w=c.job2?le[c.job2]:null,v=Oe[c.rarity]||"#ccc",$=be[c.country_code]||c.country_code||"";s(`${c.firstname} ${c.surname_encoded}`,`<div style="display:flex;gap:16px;flex-wrap:wrap;justify-content:center">
      ...
      <div style="width:140px;border-radius:12px;padding:6px;background:${v};flex-shrink:0">
        <div style="background:#f2e8d2;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
          ...
          <div style="background:#f2e8d2;padding:3px 6px;display:flex;align-items:center;justify-content:space-between;min-height:24px">
            ...
            ${(y=c.clubs)!=null&&y.logo_url?`<img src="${c.clubs.logo_url}" style="width:22px;height:16px;object-fit:contain">`:`<div style="background:#1a3a7a;color:#fff;border-radius:2px;padding:1px 3px;font-size:6px;font-weight:800">${(((E=c.clubs)==null?void 0:E.encoded_name)||"").slice(0,6)}</div>`}
          </div>
        </div>
      </div>
    ...
    </div>`:""}`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(I=document.getElementById("close-detail"))==null||I.addEventListener("click",l),(L=document.getElementById("direct-sell-btn"))==null||L.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte ${c.surname_encoded} pour ${u.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const B=f.find(U=>!U.is_for_sale)||f[0];if(!B){n("Aucune carte à vendre","error");return}const{error:H}=await k.from("cards").delete().eq("id",B.id);if(H){n(H.message,"error");return}await k.from("users").update({credits:(a.profile.credits||0)+u}).eq("id",a.profile.id),await d(),n(`+${u.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),l(),r("collection")}),(M=document.getElementById("market-sell-btn"))==null||M.addEventListener("click",async()=>{const B=parseInt(document.getElementById("sell-price").value);if(!B||B<1){n("Prix invalide","error");return}await k.from("cards").update({is_for_sale:!0,sale_price:B}).eq("id",t.id),await k.from("market_listings").insert({seller_id:a.profile.id,card_id:t.id,price:B}),n("Carte mise en vente sur le marché !","success"),l(),r("collection")}),(z=document.getElementById("cancel-sell-btn"))==null||z.addEventListener("click",async()=>{await k.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",t.id),await k.from("market_listings").update({status:"cancelled"}).eq("card_id",t.id).eq("status","active"),n("Annonce retirée","success"),l(),r("collection")})}const ce={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},Y={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function he(t){const e=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!e||!(t!=null&&t.skin)||!(t!=null&&t.hair))return null;const i=t.hair==="chauve"?`${t.skin}-chauve-rase`:`${t.skin}-${t.hair}-${t.hair_length}`;return`${e}/storage/v1/object/public/assets/tetes/${i}.png`}async function Pe(t,e){const{state:i,navigate:o,toast:a}=e;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:n}=await k.from("decks").select("id,name,formation_card_id,is_active").eq("owner_id",i.profile.id).order("created_at",{ascending:!1});t.innerHTML=`
  <div class="page">
    <div class="page-header">
      <h2>Mes decks</h2>
  ...
  <div class="page">
  ...
  </div>`,ft(t,e,a,i),document.getElementById("builder-back").addEventListener("click",()=>o("decks")),document.getElementById("formation-select").addEventListener("change",d=>{e.formation=d.target.value;const c=Le(e.formation),f={};c.forEach(p=>{e.slots[p]&&(f[p]=e.slots[p])}),e.slots=f,W(t,e,i)}),document.getElementById("save-deck").addEventListener("click",()=>xt(e,i)),t.querySelectorAll("[data-remove-sub]").forEach(d=>{d.addEventListener("click",()=>{e.subs=e.subs.filter(c=>c!==d.dataset.removeSub),W(t,e,i)})}),(r=document.getElementById("add-sub-btn"))==null||r.addEventListener("click",()=>{gt(e,t,i)})}function ft(t,e,i,o){const a=document.getElementById("deck-field");if(!a)return;const n=e.formation,s=ve[n]||{},l=ue?ue(n)||ee[n]||[]:ee[n]||[],r={};i.forEach(w=>{const v=e.slots[w],$=v?e.playerCards.find(y=>y.id===v):null;r[w]=($==null?void 0:$.player)||null});const d=i.filter(w=>w.startsWith("MIL"));let c=0,f=0;d.forEach(w=>{const v=r[w];v&&(c+=Number(v.note_m)||0)}),l.forEach(([w,v])=>{if(!w.startsWith("MIL")||!v.startsWith("MIL"))return;J(r[w],r[v])!=="#ff3333"&&f++});const p=320,u=320,g=28,m=4;function x(w){const v=s[w];return v?{x:v.x*p,y:v.y*u}:null}let b="";for(const[w,v]of l){const $=x(w),y=x(v);if(!$||!y)continue;const E=r[w],I=r[v],L=J(E,I);if(L==="#00ff88"||L==="#FFD700")b+=`<line x1="${$.x}" y1="${$.y}" x2="${y.x}" y2="${y.y}"
        stroke="${L}" stroke-width="${m*3}" stroke-linecap="round" opacity="0.25"/>`,b+=`<line x1="${$.x}" y1="${$.y}" x2="${y.x}" y2="${y.y}"
        stroke="${L}" stroke-width="${m}" stroke-linecap="round" opacity="0.95"/>`;else{const z=L==="#ff3333"||L==="#cc2222"?.75:.9;b+=`<line x1="${$.x}" y1="${$.y}" x2="${y.x}" y2="${y.y}"
        stroke="${L}" stroke-width="${m}" stroke-linecap="round" opacity="${z}"/>`}}for(const w of i){const v=x(w);if(!v)continue;const $=r[w],y=w.replace(/\d+/,""),I={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[y]||"#555",L=$?Number(y==="GK"?$.note_g:y==="DEF"?$.note_d:y==="MIL"?$.note_m:$.note_a)||0:null,M=$?($.surname_encoded||"").slice(0,8).toUpperCase():"";if($){const z=he($);z&&(b+=`
          <defs>
            <clipPath id="clip-${w}">
              <circle cx="${v.x}" cy="${v.y}" r="${g}"/>
            </clipPath>
          </defs>
          <image href="${z}" x="${v.x-g}" y="${v.y-g}" width="${g*2}" height="${g*2}"
            clip-path="url(#clip-${w})" preserveAspectRatio="xMidYMid slice" opacity="0.85"/>`),b+=`
        <circle cx="${v.x}" cy="${v.y}" r="${g}" fill="${z?"transparent":I}"
          stroke="${I}" stroke-width="2" />
        <circle cx="${v.x}" cy="${v.y}" r="${g}" fill="${I}" opacity="${z?"0.5":"1"}"
          stroke="rgba(255,255,255,0.6)" stroke-width="2"/>
        ${z?`<image href="${z}" x="${v.x-g}" y="${v.y-g}" width="${g*2}" height="${g*2}"
    ...
    <svg viewBox="0 0 ${p} ${u}" width="100%" style="display:block;max-width:380px;margin:0 auto">
      ${h}
      ${b}
    </svg>`,a.querySelectorAll(".deck-slot-hit").forEach(w=>{w.addEventListener("click",()=>ut(w.dataset.pos,e,t,o))})}function ut(t,e,i,o){var p,u,g;const{openModal:a,closeModal:n}=o,s=t.replace(/\d+/,""),l=e.slots[t],r=l?e.playerCards.find(m=>m.id===l):null;(p=r==null?void 0:r.player)==null||p.id;const d=new Set;Object.entries(e.slots).forEach(([m,x])=>{var h;if(m===t||!x)return;const b=e.playerCards.find(w=>w.id===x);(h=b==null?void 0:b.player)!=null&&h.id&&d.add(b.player.id)}),e.subs.forEach(m=>{var b;const x=e.playerCards.find(h=>h.id===m);(b=x==null?void 0:x.player)!=null&&b.id&&d.add(x.player.id)});const c=new Set,f=e.playerCards.filter(m=>{const x=m.player;return!(x.job===s||x.job2===s)||d.has(x.id)||c.has(x.id)?!1:(c.add(x.id),!0)});f.sort((m,x)=>{const b=s==="GK"?m.player.note_g:s==="DEF"?m.player.note_d:s==="MIL"?m.player.note_m:m.player.note_a;return(s==="GK"?x.player.note_g:s==="DEF"?x.player.note_d:s==="MIL"?x.player.note_m:x.player.note_a)-b}),a(`Choisir ${s} — ${t}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${e.slots[t]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${f.length>0?f.map(m=>{var v,$;const x=m.player,b=s==="GK"?x.note_g:s==="DEF"?x.note_d:s==="MIL"?x.note_m:x.note_a,h=he(x),w={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[x.rarity];return`<div class="player-option" data-card-id="${m.id}"
          ...
          <div style="flex:1;min-width:0">
            ...
            <div style="display:flex;align-items:center;gap:6px;margin-top:3px">
              ...
              ${(v=x.clubs)!=null&&v.logo_url?`<img src="${x.clubs.logo_url}" style="width:18px;height:18px;object-fit:contain">`:`<span style="font-size:10px;color:var(--gray-600)">${(($=x.clubs)==null?void 0:$.encoded_name)||"—"}</span>`}
              <span style="font-size:10px;color:var(--gray-600)">${x.country_code}</span>
            </div>
          </div>
          <!-- Note -->
          <div style="width:36px;height:36px;border-radius:8px;background:${Y[s]};color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:16px;border:2px solid ${w};flex-shrink:0">
            ${b}
          </div>
        </div>`}).join(""):'<div style="text-align:center;color:var(--gray-600);padding:20px">Aucun joueur pour ce poste.<br><small>Ouvre des boosters !</small></div>'}
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(u=document.getElementById("close-selector"))==null||u.addEventListener("click",n),(g=document.getElementById("remove-player"))==null||g.addEventListener("click",()=>{delete e.slots[t],n(),W(i,e,o)}),document.querySelectorAll(".player-option").forEach(m=>{m.addEventListener("click",()=>{e.slots[t]=m.dataset.cardId,n(),W(i,e,o)})})}function gt(t,e,i){var r;const{openModal:o,closeModal:a}=i,n=new Set;Object.values(t.slots).filter(Boolean).forEach(d=>{var f;const c=t.playerCards.find(p=>p.id===d);(f=c==null?void 0:c.player)!=null&&f.id&&n.add(c.player.id)}),t.subs.forEach(d=>{var f;const c=t.playerCards.find(p=>p.id===d);(f=c==null?void 0:c.player)!=null&&f.id&&n.add(c.player.id)});const s=new Set,l=t.playerCards.filter(d=>{var c,f,p;return n.has((c=d.player)==null?void 0:c.id)||s.has((f=d.player)==null?void 0:f.id)?!1:(s.add((p=d.player)==null?void 0:p.id),!0)});o("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
  ...
  `,document.body.appendChild(o);let a=!1;document.getElementById("pack-visual").addEventListener("click",()=>{if(a)return;a=!0;const p=document.getElementById("pack-visual");p.classList.add("shaking"),setTimeout(()=>{p.classList.add("pack-open"),setTimeout(()=>{document.getElementById("pack-phase").style.display="none",s(0)},600)},500)});let n=0;function s(p){n=p;const u=document.getElementById("reveal-phase");u.style.display="flex",l(p)}function l(p){var $;const u=t[p],g=document.getElementById("card-counter"),m=document.getElementById("single-card-slot"),x=document.getElementById("card-tap-hint");g&&(g.textContent=`Carte ${p+1} / ${t.length}`),x&&(x.textContent=p<t.length-1?"Appuie pour continuer →":"Appuie pour voir toutes tes cartes");const b=ze(u),h=u.card_type==="player"&&(($=u.player)==null?void 0:$.rarity)==="legende";m.innerHTML=`
      ...
      </div>`,h&&c();const w=document.getElementById("current-card-wrap");let v=!1;w.addEventListener("click",()=>{if(v)return;v=!0;const y=p+1;y<t.length?(w.style.transition="all 0.25s ease",w.style.transform="translateX(-120%) rotate(-15deg)",w.style.opacity="0",setTimeout(()=>l(y),250)):r()})}function r(){f(),document.getElementById("reveal-phase").style.display="none";const p=document.getElementById("recap-phase");p.style.display="flex";const u=document.getElementById("recap-grid");u.innerHTML=t.map((g,m)=>`
  ...
  `,document.body.appendChild(t),t.addEventListener("click",e=>{e.target===t&&t.remove()}),document.getElementById("odds-close").addEventListener("click",()=>t.remove())}function Tt(){return new Promise(t=>{const e=document.createElement("div");e.style.cssText="position:fixed;inset:0;background:#000;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:16px";let i=5;e.innerHTML=`<div style="font-size:48px">📺</div>
      ...
      <div style="font-size:36px;font-weight:900" id="ad-cd">5</div>`,document.body.appendChild(e);const o=setInterval(()=>{i--;const a=document.getElementById("ad-cd");a&&(a.textContent=i),i<=0&&(clearInterval(o),e.remove(),t(!0))},1e3)})}const X={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},N={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function Ue(t,e,i,o,a){var n;t.innerHTML=`<div class="match-screen" style="display:flex;align-items:center;justify-content:center;min-height:100vh">
    <div style="text-align:center;padding:40px;color:#fff">
      <div style="font-size:48px;margin-bottom:16px">${e}</div>
      <p style="margin-bottom:16px">${i}</p>
      <button class="btn btn-primary" id="msg-btn">${o}</button>
    </div>
  </div>`,(n=document.getElementById("msg-btn"))==null||n.addEventListener("click",a)}function S(t){const e=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!e||!(t!=null&&t.skin)||!(t!=null&&t.hair))return null;const i=t.hair==="chauve"?`${t.skin}-chauve-rase`:`${t.skin}-${t.hair}-${t.hair_length}`;return`${e}/storage/v1/object/public/assets/tetes/${i}.png`}function ge(t){var i,o;const e=t.player;return{cardId:t.id,id:e.id,firstname:e.firstname,name:e.surname_encoded,country_code:e.country_code,club_id:e.club_id,job:e.job,job2:e.job2,note_g:Number(e.note_g)||0,note_d:Number(e.note_d)||0,note_m:Number(e.note_m)||0,note_a:Number(e.note_a)||0,rarity:e.rarity,skin:e.skin,hair:e.hair,hair_length:e.hair_length,clubName:((i=e.clubs)==null?void 0:i.encoded_name)||null,clubLogo:((o=e.clubs)==null?void 0:o.logo_url)||null,boost:0,used:!1,_line:null,_col:null}}function we(t){return t===1?[1]:t===2?[0,2]:t===3?[0,1,2]:t===4?[0,1,1,2]:t===5?[0,1,1,1,2]:[1]}function Lt(){const t=Math.random()*100;return t<.1?4:t<5?3:t<20?2:1}function Ve(t,e){const i=X[e]||X["4-4-2"],o={GK:[],DEF:[],MIL:[],ATT:[]},a=[...t];for(const n of["GK","DEF","MIL","ATT"]){const s=[];for(let r=0;r<i[n];r++){let d=a.findIndex(c=>c.job===n);if(d===-1&&(d=a.findIndex(c=>c.job2===n)),d===-1&&(d=0),a[d]){const c={...a[d],_line:n};s.push(c),a.splice(d,1)}}const l=we(s.length);s.forEach((r,d)=>{r._col=l[d]}),o[n]=s}return o}async function Mt(t,e){const{data:i}=await k.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length").eq("is_active",!0).limit(60);if(!i||i.length<11)return zt(t);const o=X[t]||X["4-4-2"],a={GK:[],DEF:[],MIL:[],ATT:[]},n=[...i];for(const s of["GK","DEF","MIL","ATT"]){const l=n.filter(p=>p.job===s),r=n.filter(p=>p.job!==s),d=[...l,...r],c=[];for(let p=0;p<o[s];p++){const u=d[p]||n[p];u&&c.push({cardId:"ai-"+u.id+"-"+p,id:u.id,firstname:u.firstname,name:u.surname_encoded,country_code:u.country_code,club_id:u.club_id,job:u.job,job2:u.job2,note_g:Number(u.note_g)||0,note_d:Number(u.note_d)||0,note_m:Number(u.note_m)||0,note_a:Number(u.note_a)||0,rarity:u.rarity,skin:u.skin,hair:u.hair,hair_length:u.hair_length,boost:0,used:!1,_line:s})}const f=we(c.length);c.forEach((p,u)=>{p._col=f[u]}),a[s]=c}return a}function zt(t){const e=X[t]||X["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},o=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let a=0;for(const n of["GK","DEF","MIL","ATT"]){const s=[];for(let r=0;r<e[n];r++){const d=3+Math.floor(Math.random()*5);s.push({cardId:"fake-"+a,id:"fake-"+a,firstname:"IA",name:o[a%o.length],country_code:"XX",club_id:null,job:n,job2:null,note_g:n==="GK"?d:2,note_d:n==="DEF"?d:2,note_m:n==="MIL"?d:2,note_a:n==="ATT"?d:2,rarity:"normal",boost:0,used:!1,_line:n}),a++}const l=we(s.length);s.forEach((r,d)=>{r._col=l[d]}),i[n]=s}return i}async function jt(t,e){var $;const{state:i,navigate:o,toast:a}=e,n=i.params||{};t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const s=n.matchMode||"vs_ai_easy",l=s.replace("vs_ai_",""),r=s;if(!n.deckId)return Ct(t,e,s);const d=n.deckId,[{data:c},{data:f}]=await Promise.all([k.from("decks").select("formation,name").eq("id",d).single(),k.from("deck_cards").select(`position, is_starter, slot_order,
        card:cards(id, card_type, formation,
          player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
            note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
            clubs(encoded_name,logo_url)))`).eq("deck_id",d).order("slot_order")]),p=(f||[]).filter(y=>{var E;return y.is_starter&&((E=y.card)==null?void 0:E.player)}).map(y=>ge(y.card)),u=(f||[]).filter(y=>{var E;return!y.is_starter&&((E=y.card)==null?void 0:E.player)}).map(y=>ge(y.card));if(p.length<11){Ue(t,"⚠️",`Deck incomplet (${p.length}/11).`,"Compléter",()=>o("decks"));return}const g=(f||[]).find(y=>{var E;return((E=y.card)==null?void 0:E.card_type)==="formation"}),m=(c==null?void 0:c.formation)||(($=g==null?void 0:g.card)==null?void 0:$.formation)||"4-4-2",{data:x}=await k.from("cards").select("id,gc_type").eq("owner_id",i.profile.id).eq("card_type","game_changer"),b=Ve(p,m),h=await Mt(m),{data:w}=await k.from("matches").insert({home_id:i.profile.id,away_id:null,mode:r,home_deck_id:d,status:"in_progress"}).select().single(),v={matchId:w==null?void 0:w.id,mode:r,difficulty:l,formation:m,homeTeam:b,aiTeam:h,homeSubs:u,subsUsed:0,maxSubs:Math.min(u.length,3),homeScore:0,aiScore:0,gcCards:x||[],usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Bt(t,v,e)}async function Ct(t,e,i){const{state:o,navigate:a}=e;t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:n}=await k.from("decks").select("id,name,is_active,formation").eq("owner_id",o.profile.id).order("created_at",{ascending:!1});if(!n||n.length===0){Ue(t,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>a("decks"));return}const s=n.map(c=>c.id),{data:l}=await k.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length))`).in("deck_id",s).order("slot_order");let r=0;function d(){var h,w,v,$,y;const c=n[r],f=(l||[]).filter(E=>E.deck_id===c.id),p=f.filter(E=>{var I;return E.is_starter&&((I=E.card)==null?void 0:I.player)}).map(E=>ge(E.card)),u=f.find(E=>{var I;return((I=E.card)==null?void 0:I.card_type)==="formation"}),g=c.formation||((h=u==null?void 0:u.card)==null?void 0:h.formation)||"4-4-2",m=p.length>=11?Ve(p,g):null,x=p.length>=11;t.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:calc(100dvh - 130px);overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div id="deck-swipe-zone" style="flex:1;min-height:0;overflow:hidden;position:relative;touch-action:pan-y;display:flex;align-items:center;justify-content:center">
        ${m?`<div style="width:min(97vw, calc(100dvh - 430px));overflow:hidden;flex-shrink:0">${Je(m,g,null,[],270,270)}</div>`:`<div style="display:flex;align-items:center;justify-content:center;height:100%;opacity:.4;flex-direction:column;gap:8px">
      ...
      <div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">
        ${n.map((E,I)=>`<div style="width:7px;height:7px;border-radius:50%;background:${I===r?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}
      </div>`:""}
    ...
    </div>`,(w=document.getElementById("prev-deck"))==null||w.addEventListener("click",()=>{r>0&&(r--,d())}),(v=document.getElementById("next-deck"))==null||v.addEventListener("click",()=>{r<n.length-1&&(r++,d())}),($=document.getElementById("validate-deck"))==null||$.addEventListener("click",()=>{x&&e.navigate("match",{matchMode:i,deckId:c.id})}),(y=document.getElementById("cancel-deck-select"))==null||y.addEventListener("click",()=>a("home"));const b=document.getElementById("deck-swipe-zone");if(b){let E=0,I=0;b.addEventListener("touchstart",L=>{E=L.touches[0].clientX,I=L.touches[0].clientY},{passive:!0}),b.addEventListener("touchend",L=>{const M=L.changedTouches[0].clientX-E,z=L.changedTouches[0].clientY-I;Math.abs(M)<40||Math.abs(M)<Math.abs(z)||(M<0&&r<n.length-1?(r++,d()):M>0&&r>0&&(r--,d()))},{passive:!0})}}d()}function Bt(t,e,i){const o=e.homeTeam.MIL||[],a=e.aiTeam.MIL||[];function n(f){return f.reduce((p,u)=>p+D(u,"MIL"),0)}function s(f){let p=0;for(let u=0;u<f.length-1;u++){const g=J(f[u],f[u+1]);g!=="#ff3333"&&g!=="#cc2222"&&p++}return p}const l=n(o)+s(o),r=n(a)+s(a),d=l>=r;function c(f,p,u){return`<div style="text-align:center">
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${f.map((g,m)=>{const x=S(g),b=m<f.length-1?J(g,f[m+1]):null,h=b&&b!=="#ff3333"&&b!=="#cc2222";return`
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:100vh;gap:24px;padding:24px;background:#0a3d1e">
    ...
    <div id="midfield-result" style="opacity:0;text-align:center;transition:opacity 0.5s;color:#fff;max-width:320px"></div>
      ...
      `,d&&(e.boostCard={value:g})}e.attacker=d?"home":"ai",e.log.push({type:"duel",title:"Milieu de Terrain",homePlayers:o.map(g=>({name:g.name,note:D(g,"MIL"),portrait:S(g),job:g.job})),aiPlayers:a.map(g=>({name:g.name,note:D(g,"MIL"),portrait:S(g),job:g.job})),homeTotal:l,aiTotal:r,text:`Duel milieu : ${e.clubName} ${l} – ${r} IA → ${d?e.clubName+" attaque":"IA attaque"}`}),setTimeout(()=>{e.phase=e.attacker==="home"?"attack":"ai-attack",G(t,e,i),e.attacker==="ai"&&setTimeout(()=>xe(t,e,i),1e3)},2800)},1200)}function St(t){if(!t||t.length<2)return"🌍";try{return String.fromCodePoint(127462+t.charCodeAt(0)-65)+String.fromCodePoint(127462+t.charCodeAt(1)-65)}catch{return"🌍"}}function Ye(t,e,i,o,a=310,n=310){const s=ve[e]||{},l=ue(e)||ee[e]||[],r=26,d={},c=["ATT","MIL","DEF","GK"];for(const g of c)(t[g]||[]).forEach((x,b)=>{d[`${g}${b+1}`]=x});function f(g){const m=s[g];return m?{x:m.x*a,y:m.y*n}:null}let p="";for(const[g,m]of l){const x=f(g),b=f(m);if(!x||!b)continue;const h=d[g],w=d[m],v=J(h,w);v==="#00ff88"||v==="#FFD700"?(p+=`<line x1="${x.x.toFixed(1)}" y1="${x.y.toFixed(1)}" x2="${b.x.toFixed(1)}" y2="${b.y.toFixed(1)}"
        stroke="${v}" stroke-width="10" stroke-linecap="round" opacity="0.22"/>`,p+=`<line x1="${x.x.toFixed(1)}" y1="${x.y.toFixed(1)}" x2="${b.x.toFixed(1)}" y2="${b.y.toFixed(1)}"
        stroke="${v}" stroke-width="3.5" stroke-linecap="round" opacity="0.95"/>`):p+=`<line x1="${x.x.toFixed(1)}" y1="${x.y.toFixed(1)}" x2="${b.x.toFixed(1)}" y2="${b.y.toFixed(1)}"
        stroke="${v}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}for(const[g,m]of Object.entries(d)){const x=f(g);if(!x)continue;const b=g.replace(/[0-9]/g,""),h=N[b]||"#555",w=i==="attack"&&["MIL","ATT"].includes(b)&&!m.used||i==="defense"&&["GK","DEF","MIL"].includes(b)&&!m.used,v=o.includes(m.cardId);let $;i==="attack"?$=b==="MIL"?Number(m.note_m)||0:Number(m.note_a)||0:i==="defense"?$=b==="GK"?Number(m.note_g)||0:b==="MIL"?Number(m.note_m)||0:Number(m.note_d)||0:$=Number(b==="GK"?m.note_g:b==="DEF"?m.note_d:b==="MIL"?m.note_m:m.note_a)||0,$=$+(m.boost||0);const y=v?"#FFD700":w?"rgba(255,255,255,0.7)":"rgba(255,255,255,0.25)",E=v?3:2,I=m.used?.2:1,L=S(m);if(L&&(p+=`<defs><clipPath id="mc-${g}"><circle cx="${x.x}" cy="${x.y}" r="${r}"/></clipPath></defs>`),p+=`<circle cx="${x.x}" cy="${x.y}" r="${r}" fill="${h}" opacity="${I}"
      stroke="${y}" stroke-width="${E}"/>`,L&&!m.used&&(p+=`<image href="${L}" x="${x.x-r}" y="${x.y-r}" width="${r*2}" height="${r*2}"
        clip-path="url(#mc-${g})" preserveAspectRatio="xMidYMid slice" opacity="0.8"/>`,p+=`<circle cx="${x.x}" cy="${x.y}" r="${r}" fill="${h}" opacity="0.3"
        stroke="${y}" stroke-width="${E}"/>`),m.boost&&(p+=`<rect x="${x.x+r-10}" y="${x.y-r}" width="14" height="10" rx="3" fill="#87CEEB"/>
        <text x="${x.x+r-3}" y="${x.y-r+8}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${m.boost}</text>`),m.used)p+=`<text x="${x.x}" y="${x.y+2}" text-anchor="middle" dominant-baseline="central"
        font-size="14" font-weight="900" fill="rgba(255,255,255,0.15)" font-family="Arial Black">–</text>`;else{const M=(x.y-r-9).toFixed(1);p+=`<circle cx="${x.x.toFixed(1)}" cy="${M}" r="9" fill="white" stroke="rgba(0,0,0,0.3)" stroke-width="0.8"/>`,p+=`<text x="${x.x.toFixed(1)}" y="${M}" text-anchor="middle" dominant-baseline="central"
        font-size="9" font-weight="900" fill="#111" font-family="Arial Black">${$}</text>`;const z=(x.x+r*.72).toFixed(1),B=(x.y-r*.72).toFixed(1),H=St(m.country_code);p+=`<circle cx="${z}" cy="${B}" r="8.5" fill="rgba(0,0,0,0.6)" stroke="rgba(255,255,255,0.2)" stroke-width="0.8"/>`,p+=`<text x="${z}" y="${B}" text-anchor="middle" dominant-baseline="central" font-size="7.5">${H}</text>`;const U=(x.x-r*.72).toFixed(1),_=(x.y-r*.72).toFixed(1);m.clubName&&(p+=`<circle cx="${U}" cy="${_}" r="8.5" fill="rgba(10,20,60,0.75)" stroke="rgba(255,255,255,0.2)" stroke-width="0.8"/>`,p+=`<text x="${U}" y="${_}" text-anchor="middle" dominant-baseline="central"
          font-size="5.5" font-weight="700" fill="rgba(255,255,255,0.9)">${(m.clubName||"").slice(0,3).toUpperCase()}</text>`);const A=(m.name||"").slice(0,9),T=Math.max(A.length*4.5,28),j=x.y+r+3;p+=`<rect x="${(x.x-T/2).toFixed(1)}" y="${j.toFixed(1)}" width="${T.toFixed(1)}" height="11" rx="3" ry="3"
        fill="rgba(255,255,255,0.88)" stroke="rgba(0,0,0,0.1)" stroke-width="0.5"/>`,p+=`<text x="${x.x.toFixed(1)}" y="${(j+5.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central"
        font-size="6" font-weight="700" fill="#111" font-family="Arial">${A}</text>`}w&&(p+=`<circle cx="${x.x}" cy="${x.y}" r="${r}" fill="rgba(255,255,255,0.08)"
        class="match-slot-hit ${v?"selected":""}" data-card-id="${m.cardId}" data-role="${b}"
        style="cursor:pointer"/>`)}const u=r+20;return`<svg viewBox="${-u} ${-u} ${a+u*2} ${n+u*2}" width="100%" style="display:block;width:100%;max-width:380px;margin:0 auto">
    ${p}
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;height:calc(100dvh - 130px);overflow:hidden;background:#0a3d1e;position:relative">
      ...
      ${(()=>{if(e.phase==="defense"&&e.pendingAttack){const E=e.pendingAttack;return`<div style="padding:6px 8px;background:rgba(180,30,30,0.25);border-left:3px solid #ff6b6b">
            ...
            <div style="display:flex;align-items:center;gap:6px">
              <div style="display:flex;gap:5px;flex:1">
                ${(E.players||[]).map(I=>{const L=I._line==="MIL"?I.note_m:I.note_a,M=S(I);return'<div style="text-align:center"><div style="width:36px;height:36px;border-radius:6px;background:'+(N[I.job]||"#555")+';position:relative;overflow:hidden;border:1.5px solid rgba(255,255,255,0.3)">'+(M?'<img src="'+M+'" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover">':"")+'<div style="position:absolute;bottom:0;left:0;right:0;background:rgba(0,0,0,0.7);font-size:9px;color:#fff;font-weight:900;text-align:center">'+L+'</div></div><div style="font-size:6px;color:rgba(255,255,255,0.5)">'+(I.name||"").slice(0,6)+"</div></div>"}).join("")}
              </div>
          ...
          </div>`}const y=e.log[e.log.length-1];return y?'<div style="padding:2px 4px">'+Ft(y)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})()}
    </div>
    ...
    <div style="display:flex;flex:1;min-height:0;overflow:hidden">
      ...
      <div style="display:flex;flex-direction:column;gap:5px;padding:6px 3px;width:42px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
        ${n.length===0?'<div style="font-size:8px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':n.map(y=>{const E=S(y),I=N[y.job]||"#555",L=y.job==="GK"?y.note_g:y.job==="DEF"?y.note_d:y.job==="MIL"?y.note_m:y.note_a;return`
              <div class="sub-btn-col" data-sub-id="${y.cardId}" title="${y.firstname} ${y.name}"
      ...
      <div style="flex:1;display:grid;grid-template-columns:repeat(4,1fr);gap:3px;align-content:start">
        ${p.map(y=>{var E;return`
          <div class="gc-mini" data-gc-id="${y.id}" data-gc-type="${y.gc_type}"
            ...
            <div style="font-size:16px">${((E=de[y.gc_type])==null?void 0:E.icon)||"⚡"}</div>
  ...
  `;function l(r){const d=document.getElementById("mkt-content"),c=r==="buy"?s:n;if(c.length===0){d.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      ${c.map(f=>{var x,b,h;const p=(x=f.card)==null?void 0:x.player;if(!p)return"";const u=p.job==="GK"?p.note_g:p.job==="DEF"?p.note_d:p.job==="MIL"?p.note_m:p.note_a,g=Ht[p.rarity],m=(i.profile.credits||0)>=f.price;return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px">
          ...
          <div style="text-align:right;flex-shrink:0">
            ...
            ${r==="buy"?`<button class="btn btn-primary btn-sm" data-buy="${f.id}" ${m?"":"disabled"} style="margin-top:4px">${m?"Acheter":"Trop cher"}</button>`:`<button class="btn btn-danger btn-sm" data-cancel="${f.id}" style="margin-top:4px">Retirer</button>`}
          </div>
        </div>`}).join("")}
  ...
  `}const C={user:null,profile:null,page:"home",params:{}};function re(t,e="info",i=3e3){const o=document.getElementById("toast");o&&(o.textContent=t,o.className=`show ${e}`,clearTimeout(o._t),o._t=setTimeout(()=>{o.className=""},i))}function Xt(t,e,i=""){document.getElementById("modal-title").textContent=t,document.getElementById("modal-body").innerHTML=e,document.getElementById("modal-footer").innerHTML=i,document.getElementById("modal-overlay").classList.remove("hidden")}function ye(){document.getElementById("modal-overlay").classList.add("hidden")}async function ae(){if(!C.user)return;const{data:t}=await k.from("users").select("*").eq("id",C.user.id).single();t&&(C.profile=t)}const ke="mw_theme";function fe(){return localStorage.getItem(ke)||"light"}function Qt(t){var e;localStorage.setItem(ke,t),me(t),(e=C.profile)!=null&&e.id&&k.from("users").update({theme:t}).eq("id",C.profile.id).then(()=>{})}function me(t){document.documentElement.setAttribute("data-theme",t)}function se(t,e={}){C.page=t,C.params=e,Ze()}async function Ze(){const t=document.getElementById("page-content");if(!t)return;document.querySelectorAll(".bottom-nav a").forEach(o=>{o.classList.toggle("active",o.dataset.page===C.page)});const e=document.getElementById("nav-credits");e&&C.profile&&(e.textContent=`💰 ${(C.profile.credits||0).toLocaleString("fr")}`);const i={state:C,navigate:se,toast:re,openModal:Xt,closeModal:ye,refreshProfile:ae};switch(t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽</div>',C.page){case"home":await Ie(t,i);break;case"collection":await dt(t,i);break;case"decks":await Pe(t,i);break;case"boosters":await bt(t,i);break;case"match":await jt(t,i);break;case"market":await Ut(t,i);break;case"rankings":await Wt(t,i);break;default:await Ie(t,i)}}function Zt(){const t=document.getElementById("app"),e=C.profile;if(!e)return;const i="/manager-wars/icons/";t.innerHTML=`
    <nav class="top-nav">
  ...
  `,document.querySelectorAll(".bottom-nav a").forEach(o=>{o.addEventListener("click",a=>{a.preventDefault(),se(o.dataset.page)})}),document.getElementById("nav-logo").addEventListener("click",()=>se("home")),document.getElementById("nav-credits").addEventListener("click",()=>se("boosters")),document.getElementById("theme-toggle").addEventListener("click",()=>{const a=fe()==="dark"?"light":"dark";Qt(a),Be(a)}),Be(fe())}function Be(t){const e=document.getElementById("theme-icon");e&&(e.textContent=t==="dark"?"☀️":"🌙")}async function ei(){me(fe()),document.getElementById("modal-overlay").addEventListener("click",e=>{e.target===e.currentTarget&&ye()}),document.getElementById("modal-close").addEventListener("click",ye);const{data:{session:t}}=await k.auth.getSession();if(!t){Fe(),_e(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:re});return}C.user=t.user,await ae(),Fe();try{const{data:e}=await k.from("formation_links_overrides").select("formation, links"),i={};(e||[]).forEach(o=>{i[o.formation]=o.links}),et(i)}catch(e){console.warn("Impossible de charger les overrides de formation:",e)}if(!C.profile){tt(document.getElementById("app"),{state:C,navigate:async()=>{await ae(),Se()},toast:re,refreshProfile:ae});return}C.profile.theme&&C.profile.theme!==fe()&&(localStorage.setItem(ke,C.profile.theme),me(C.profile.theme)),Se(),k.auth.onAuthStateChange(async(e,i)=>{e==="SIGNED_OUT"&&(C.user=null,C.profile=null,document.getElementById("app").innerHTML="",_e(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:re}))})}function Se(){const t=document.getElementById("app");t.style.display="flex",t.style.flexDirection="column",Zt(),Ze()}function Fe(){const t=document.getElementById("app-loader"),e=document.getElementById("app");e&&(e.style.display=""),t&&(t.classList.add("zoom-out"),setTimeout(()=>t.style.display="none",500))}ei();
