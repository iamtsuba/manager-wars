import{s as z,F as Tt,h as it,j as li,l as Fe,i as Ii,k as Li,b as Mi}from"./formation-links-CEzN0rcz.js";function Vt(e,{navigate:t,toast:i}){e.innerHTML=`
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
  ...
  </div>`}async function Gi(e,t){const{state:i,navigate:r,toast:d,openModal:s,closeModal:a}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:l}=await z.from("cards").select(`id, card_type, current_note, gc_type, formation, is_for_sale, sale_price,
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),o=(l||[]).filter(M=>M.card_type==="player"&&M.player),n=(l||[]).filter(M=>M.card_type==="game_changer"),u=(l||[]).filter(M=>M.card_type==="formation"),{data:f}=await z.from("gc_definitions").select("name,gc_type,color,effect,image_url"),g={};(f||[]).forEach(M=>{g[M.name]=M});const k=Object.keys(Tt),p=Object.keys(Ae),x={};o.forEach(M=>{const $=M.player.id;x[$]=(x[$]||0)+1}),new Set(Object.keys(x).map(M=>String(M)));const E=new Set(u.map(M=>M.formation)),_=new Set(n.map(M=>M.gc_type));let j="player",D="Tous",I="",b=!1;function B(){return[...o].sort((M,$)=>{const G=ot.indexOf(M.player.job),N=ot.indexOf($.player.job);return G!==N?G-N:(M.player.surname_encoded||"").localeCompare($.player.surname_encoded||"")})}function H(){return[...c||[]].sort((M,$)=>{const G=ot.indexOf(M.job),N=ot.indexOf($.job);return G!==N?G-N:(M.surname_encoded||"").localeCompare($.surname_encoded||"")})}function X(){return B().filter(M=>{const $=M.player,G=D==="Tous"||$.job===D,N=!I||`${$.firstname} ${$.surname_encoded}`.toLowerCase().includes(I);return G&&N})}function Q(){return H().filter(M=>{const $=D==="Tous"||M.job===D,G=!I||`${M.firstname} ${M.surname_encoded}`.toLowerCase().includes(I);return $&&G})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
  ...
  </div>`;function T(){const M=document.getElementById("col-filters");M&&(j==="player"?(M.innerHTML=`
        <input id="col-search" placeholder="🔍 Rechercher un joueur..." style="font-size:13px" value="${I}">
        <div style="display:flex;gap:6px;overflow-x:auto;padding-bottom:2px;align-items:center">
          ${Fi.map($=>`
            <button class="filter-btn" data-job="${$}"
              style="flex-shrink:0;padding:4px 12px;border-radius:20px;font-size:12px;font-weight:600;cursor:pointer;
                border:1.5px solid ${$===D?"var(--green)":"var(--gray-200)"};
                background:${$===D?"var(--green)":"#fff"};
                color:${$===D?"#fff":"var(--gray-600)"}">
              ${$}
            </button>`).join("")}
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",$=>{I=$.target.value.toLowerCase(),v()}),e.querySelectorAll(".filter-btn").forEach($=>{$.addEventListener("click",()=>{D=$.dataset.job,T(),v()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{b=!b,T(),v()})):(M.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{b=!b,T(),v()})))}function v(){const M=document.getElementById("col-grid");M&&(j==="player"?oe(M):j==="formation"?xe(M):me(M))}function q(M,$,G,N,V){V=V||"#7a28b8";const te=document.getElementById("col-grid"),K=document.getElementById("col-big");if(!te||!K)return;var m=0;function y(){K.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+$(M[m])+"</div>";var w=K.querySelector("[data-card-id],[data-form-id],[data-gc-id]");w&&w.addEventListener("click",function(){N(M[m])}),requestAnimationFrame(function(){var A=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!A||!K)){var h=K.clientHeight-8,S=K.clientWidth-24,L=A.offsetHeight,F=A.offsetWidth;if(L>0&&F>0&&h>40){var O=Math.min(h/L,S/F,1);A.style.transform="scale("+O.toFixed(3)+")",A.style.transformOrigin="top center"}}}),te.innerHTML=M.map(function(A,h){return'<div class="col-mini-item" data-idx="'+h+'" style="flex-shrink:0;cursor:pointer;border-radius:8px;border:1.5px solid '+(h===m?V:"transparent")+';transition:border-color .15s;overflow:hidden">'+G(A,h===m)+"</div>"}).join(""),te.querySelectorAll(".col-mini-item").forEach(function(A){A.addEventListener("click",function(){m=Number(A.dataset.idx),y(),A.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}y()}function R(M){var $=.54,G=Math.round(140*$),N=Math.round(310*$),V;if(!M||M._fake){var te=M?M.player:null;if(!te)return"";V=Jt(te)}else V=ht(M,"");return'<div style="width:'+G+"px;height:"+N+'px;overflow:hidden;position:relative;flex-shrink:0"><div style="transform:scale('+$+');transform-origin:top left;position:absolute;top:0;left:0;pointer-events:none">'+V+"</div></div>"}function W(M,$,G){$=$||100,G=G||140;var N=it[M]||{},V={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},te=Math.max(3,Math.round($*.06)),K=Object.entries(N).map(function(y){var w=y[0],A=y[1],h=w.replace(/\d+$/,""),S=V[h]||"#888",L=Math.round(A.x*$),F=Math.round(A.y*G);return'<circle cx="'+L+'" cy="'+F+'" r="'+te+'" fill="'+S+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),m=Math.max(1,Math.round($/50));return'<svg viewBox="0 0 '+$+" "+G+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+$+'" height="'+G+'" fill="#1A6B3C"/><rect x="'+Math.round($*.2)+'" y="'+Math.round(G*.02)+'" width="'+Math.round($*.6)+'" height="'+Math.round(G*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+m+'"/><line x1="0" y1="'+Math.round(G*.5)+'" x2="'+$+'" y2="'+Math.round(G*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+m+'"/><ellipse cx="'+Math.round($*.5)+'" cy="'+Math.round(G*.5)+'" rx="'+Math.round($*.18)+'" ry="'+Math.round(G*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+m+'"/><rect x="'+Math.round($*.2)+'" y="'+Math.round(G*.82)+'" width="'+Math.round($*.6)+'" height="'+Math.round(G*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+m+'"/>'+K+"</svg>"}function ie(M,$,G){var N=G>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+G+"</div>":"",V=$?'data-form-id="'+$.id+'"':"",te=M.length>7?14:M.length>5?16:19,K=!!$;return"<div "+V+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(K?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(K?"":"filter:grayscale(1);opacity:0.5")+'">'+N+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(K?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+te+"px;font-weight:900;color:"+(K?"#1A6B3C":"#aaa")+';line-height:1">'+M+'</div></div><div style="flex:1;overflow:hidden;background:'+(K?"#1A6B3C":"#ccc")+'">'+W(M,140,220)+"</div></div>"}function ae(M,$){var G=.54,N=Math.round(140*G),V=Math.round(305*G),te=Math.round(V*.22),K=V-te,m=M.length>7?5:7,y=W(M,N,K),w=$?"1.5px solid #2a7a40":"1px solid #ddd",A=$?"":"filter:grayscale(1);opacity:0.45;",h=$?"#1A6B3C":"#bbb",S="#fff";return'<div style="width:'+N+"px;height:"+V+"px;border-radius:6px;border:"+w+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+A+'"><div style="height:'+te+"px;background:"+h+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+m+"px;font-weight:900;color:"+S+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(N-4)+'px">'+M+'</span></div><div style="height:'+K+'px;overflow:hidden;flex-shrink:0">'+y+"</div></div>"}function oe(M){if(b){const $=Q();if(!$.length){M.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const G=$.map(N=>o.find(V=>V.player.id===N.id)||{_fake:!0,player:N,id:"fake-"+N.id});q(G,N=>N._fake?Jt(N.player):ht(N,""),N=>N._fake?R({player:N.player,id:"x",_fake:!0}):R(N),N=>{N._fake||Qt(N,o,x,t)},"#1A6B3C")}else{const $=X();if(!$.length){M.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const G={},N=[];$.forEach(V=>{G[V.player.id]||(G[V.player.id]=!0,N.push(V))}),q(N,V=>{const te=x[V.player.id]||1,K=te>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${te}</div>`:"",y=o.filter(w=>w.player.id===V.player.id&&w.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return ht(V,K+y)},V=>R(V),V=>Qt(V,o,x,t),"#1A6B3C")}}function xe(M){const $=b?k:[...E];if(!$.length){M.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const G=$.map(N=>({formation:N,card:u.find(V=>V.formation===N)||null,owned:E.has(N)}));q(G,({formation:N,card:V,owned:te})=>ie(N,te?V:null,te?u.filter(K=>K.formation===N).length:0),({formation:N,owned:V})=>ae(N,V),({card:N,owned:V})=>{V&&N&&Ri(N,u,t,s)},"#1A6B3C")}function me(M){const $=Object.keys(g),G=b?$.length?$:p:[..._];if(!G.length){M.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const N=G.map(V=>({type:V,gc:Ae[V]||{icon:"⚡",desc:""},def:g[V]||null,owned:_.has(V),card:n.find(te=>te.gc_type===V)||null}));q(N,({type:V,gc:te,def:K,owned:m,card:y})=>{const w=m?n.filter(Y=>Y.gc_type===V).length:0,A=w>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${w}</div>`:"",h=(K==null?void 0:K.gc_type)==="ultra_game_changer",S={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},L={purple:"#b06ce0",light_blue:"#00d4ef"},F=S[K==null?void 0:K.color]||S.purple,O=L[K==null?void 0:K.color]||L.purple,C=(K==null?void 0:K.effect)||te.desc||"",U=K!=null&&K.image_url?`/manager-wars/icons/${K.image_url}`:null;return m&&y?`<div data-gc-id="${y.id}" data-gc-type="${V}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${O};background:${F};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${O}66;cursor:pointer">
          ${A}
        ...
        </div>`},({type:V,gc:te,def:K,owned:m})=>m?(()=>{const y=Math.round(75.60000000000001),w=Math.round(310*.54),A=Math.round(w*.65),h=Math.round(w*.18),S={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},L={purple:"#9b59b6",light_blue:"#00bcd4"},F=S[K==null?void 0:K.color]||S.purple,O=L[K==null?void 0:K.color]||L.purple,C=K!=null&&K.image_url?`/manager-wars/icons/${K.image_url}`:null;return`<div style="width:${y}px;height:${w}px;border-radius:8px;background:${F};border:1px solid ${O};display:flex;flex-direction:column;overflow:hidden"><div style="height:${h}px;background:rgba(255,255,255,0.15);display:flex;align-items:center;justify-content:center"><span style="font-size:7px;font-weight:900;color:#fff;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${y-6}px">${V}</span></div><div style="height:${A}px;display:flex;align-items:center;justify-content:center">${C?`<img src="${C}" style="max-width:${y-8}px;max-height:${A-4}px;object-fit:contain">`:`<span style="font-size:24px">${te.icon}</span>`}</div><div style="flex:1;display:flex;align-items:center;justify-content:center"><span style="font-size:7px;color:rgba(255,255,255,0.7);text-align:center;padding:0 2px">${((K==null?void 0:K.effect)||te.desc||"").slice(0,20)}</span></div></div>`})():(()=>{const y=Math.round(75.60000000000001),w=Math.round(310*.54);return`<div style="width:${y}px;height:${w}px;border-radius:8px;background:#eee;border:1px solid #ddd;display:flex;flex-direction:column;align-items:center;justify-content:center;filter:grayscale(1);opacity:0.45"><span style="font-size:22px">${te.icon}</span><span style="font-size:7px;color:#aaa;margin-top:4px;text-align:center;padding:0 4px">${V}</span></div>`})(),({type:V,owned:te,def:K})=>{te&&qi(V,K,s)},"#7a28b8")}e.querySelectorAll(".col-tab-btn").forEach(M=>{M.addEventListener("click",()=>{j=M.dataset.tab,D="Tous",I="",b=!1,e.querySelectorAll(".col-tab-btn").forEach($=>{const G=$.dataset.tab===j;$.style.borderBottomColor=G?"var(--green)":"transparent",$.style.color=G?"var(--green)":"var(--gray-600)"}),T(),v()})}),T(),v()}function qi(e,t,i){const r=Ae[e]||{icon:"⚡",desc:"Effet spécial."},d=(t==null?void 0:t.gc_type)==="ultra_game_changer",s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},a={purple:"#b06ce0",light_blue:"#00d4ef"},l=s[t==null?void 0:t.color]||s.purple,c=a[t==null?void 0:t.color]||a.purple,o=(t==null?void 0:t.name)||e,n=(t==null?void 0:t.effect)||r.desc,u=t!=null&&t.image_url?`/manager-wars/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Xe=1e3;function Ri(e,t,i,r){var p,x,E;const{state:d,toast:s,closeModal:a,navigate:l,refreshProfile:c}=i,o=e.formation,n={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function u(){const _=it[o]||{},j=Tt[o]||[],D=290,I=360,b=20;function B(X){const Q=_[X];return Q?{x:Q.x*D,y:Q.y*I}:null}let H=`<svg width="${D}" height="${I}" viewBox="0 0 ${D} ${I}" xmlns="http://www.w3.org/2000/svg">`;for(const[X,Q]of j){const T=B(X),v=B(Q);!T||!v||(H+=`<line x1="${T.x}" y1="${T.y}" x2="${v.x}" y2="${v.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const X of Object.keys(_)){const Q=B(X);if(!Q)continue;const T=X.replace(/\d+/,""),v=n[T]||"#555";H+=`<circle cx="${Q.x}" cy="${Q.y}" r="${b}" fill="${v}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,H+=`<text x="${Q.x}" y="${Q.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${T}</text>`}return H+="</svg>",H}const f=t.filter(_=>_.formation===o),g=f.length,k=!e.is_for_sale;r(`Formation ${o}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(p=document.getElementById("direct-sell-form-btn"))==null||p.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${o} pour ${Xe.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const _=f.find(D=>!D.is_for_sale)||f[0];if(!_){s("Aucune carte à vendre","error");return}const{error:j}=await z.from("cards").delete().eq("id",_.id);if(j){s(j.message,"error");return}await z.from("users").update({credits:(d.profile.credits||0)+Xe}).eq("id",d.profile.id),await c(),s(`+${Xe.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),a(),l("collection")}),(x=document.getElementById("market-sell-form-btn"))==null||x.addEventListener("click",async()=>{const _=parseInt(document.getElementById("sell-price-form").value);if(!_||_<1){s("Prix invalide","error");return}await z.from("cards").update({is_for_sale:!0,sale_price:_}).eq("id",e.id),await z.from("market_listings").insert({seller_id:d.profile.id,card_id:e.id,price:_}),s("Carte mise en vente sur le marché !","success"),a(),l("collection")}),(E=document.getElementById("cancel-sell-form-btn"))==null||E.addEventListener("click",async()=>{await z.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await z.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),s("Annonce retirée","success"),a(),l("collection")})}async function Qt(e,t,i,r){var H,X,Q,T,v,q;const{state:d,toast:s,openModal:a,closeModal:l,navigate:c,refreshProfile:o}=r,n=e.player,u=t.filter(R=>R.player.id===n.id),f=u.length,g=Di[n.rarity]||1e3,k=n.rarity!=="legende",p=ui(n),x=Ze(n,n.job),E=n.job2?Ze(n,n.job2):null,_=dt[n.job]||"#1A6B3C",j=n.job2?dt[n.job2]:null,D=pi[n.rarity]||"#ccc",I=Mt[n.country_code]||n.country_code||"",{data:b}=await z.from("transfer_history").select("club_name, manager_name, source, price, transferred_at").eq("player_id",n.id).order("transferred_at",{ascending:!0}),B=b&&b.length?`
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      ...
      <div style="width:140px;border-radius:12px;padding:6px;background:${D};flex-shrink:0">
        <div style="background:#f2e8d2;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
          ...
          <div style="background:#f2e8d2;padding:3px 6px;display:flex;align-items:center;justify-content:space-between;min-height:24px">
            ...
            ${(H=n.clubs)!=null&&H.logo_url?`<img src="${n.clubs.logo_url}" style="width:22px;height:16px;object-fit:contain">`:`<div style="background:#1a3a7a;color:#fff;border-radius:2px;padding:1px 3px;font-size:6px;font-weight:800">${(((X=n.clubs)==null?void 0:X.encoded_name)||"").slice(0,6)}</div>`}
          </div>
        </div>
      </div>
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        <div>
          ...
          <div style="font-weight:700;color:${D}">${n.rarity.toUpperCase()}</div>
        </div>
        <div>
  ...
  </div>`,Oi(e,t,d,i),document.getElementById("builder-back").addEventListener("click",()=>r("decks")),document.getElementById("formation-select").addEventListener("change",o=>{t.formation=o.target.value;const n=ei(t.formation),u={};n.forEach(f=>{t.slots[f]&&(u[f]=t.slots[f])}),t.slots=u,Ue(e,t,i)}),document.getElementById("save-deck").addEventListener("click",()=>Ui(t,i)),e.querySelectorAll("[data-remove-sub]").forEach(o=>{o.addEventListener("click",()=>{t.subs=t.subs.filter(n=>n!==o.dataset.removeSub),Ue(e,t,i)})}),(c=document.getElementById("add-sub-btn"))==null||c.addEventListener("click",()=>{Hi(t,e,i)})}function Oi(e,t,i,r){const d=e.querySelector("#deck-field");if(!d)return;const s=it[t.formation]||{},a=li(t.formation)||[],l={};for(const E of i){const _=t.slots[E],j=_?t.playerCards.find(D=>D.id===_):null;l[E]=j?j.player:null}const c=300,o=300,n=48,u=64,f=13,g=16,k=38;function p(E){const _=s[E];return _?{x:_.x*c,y:_.y*o}:null}let x="";for(const[E,_]of a){const j=p(E),D=p(_);if(!j||!D)continue;const I=l[E]?{...l[E],club_id:l[E].club_id,country_code:l[E].country_code,rarity:l[E].rarity}:null,b=l[_]?{...l[_],club_id:l[_].club_id,country_code:l[_].country_code,rarity:l[_].rarity}:null,B=Fe(I,b);B==="#ff3333"||B==="#cc2222"?x+=`<line x1="${j.x.toFixed(1)}" y1="${j.y.toFixed(1)}" x2="${D.x.toFixed(1)}" y2="${D.y.toFixed(1)}" stroke="${B}" stroke-width="2.5" stroke-linecap="round" opacity="0.4"/>`:(x+=`<line x1="${j.x.toFixed(1)}" y1="${j.y.toFixed(1)}" x2="${D.x.toFixed(1)}" y2="${D.y.toFixed(1)}" stroke="${B}" stroke-width="8" stroke-linecap="round" opacity="0.2"/>`,x+=`<line x1="${j.x.toFixed(1)}" y1="${j.y.toFixed(1)}" x2="${D.x.toFixed(1)}" y2="${D.y.toFixed(1)}" stroke="${B}" stroke-width="2.5" stroke-linecap="round" opacity="0.9"/>`)}for(const E of i){const _=p(E);if(!_)continue;const j=l[E],D=E.replace(/\d+/,""),I=Ce[D]||"#555",b=(_.x-n/2).toFixed(1),B=(_.y-u/2).toFixed(1),H={normal:"#aaa",pépite:"#D4A017",papyte:"#222",légende:"#7a28b8"}[j==null?void 0:j.rarity]||"#aaa";if(j){const X=mt(j),Q=gi(j),T=fi(j.country_code),v=Number(D==="GK"?j.note_g:D==="DEF"?j.note_d:D==="MIL"?j.note_m:j.note_a)||0,q=u-f-g;x+=`<defs><clipPath id="dcp-${E}"><rect x="${b}" y="${(_.y-u/2+f).toFixed(1)}" width="${n}" height="${q}"/></clipPath></defs>`,x+=`<rect x="${b}" y="${B}" width="${n}" height="${u}" rx="5" fill="${I}"/>`,X&&(x+=`<image href="${X}" x="${b}" y="${(_.y-u/2+f).toFixed(1)}" width="${n}" height="${q}" clip-path="url(#dcp-${E})" preserveAspectRatio="xMidYMin slice"/>`),x+=`<rect x="${b}" y="${B}" width="${n}" height="${f}" fill="rgba(255,255,255,0.93)"/>`,x+=`<text x="${_.x.toFixed(1)}" y="${(_.y-u/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(j.surname_encoded||"").slice(0,9)}</text>`;const R=(_.y+u/2-g).toFixed(1);x+=`<rect x="${b}" y="${R}" width="${n}" height="${g}" fill="rgba(255,255,255,0.93)"/>`,T&&(x+=`<image href="${T}" x="${(_.x-21).toFixed(1)}" y="${(_.y+u/2-g+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`),x+=`<text x="${_.x.toFixed(1)}" y="${(_.y+u/2-g/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${v}</text>`,Q&&(x+=`<image href="${Q}" x="${(_.x+n/2-14).toFixed(1)}" y="${(_.y+u/2-g+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`),x+=`<rect x="${b}" y="${B}" width="${n}" height="${u}" rx="5" fill="none" stroke="${H}" stroke-width="2"/>`}else x+=`<rect x="${b}" y="${B}" width="${n}" height="${u}" rx="5" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="5,3"/>`,x+=`<text x="${_.x.toFixed(1)}" y="${_.y.toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="22" fill="rgba(255,255,255,0.35)">+</text>`,x+=`<text x="${_.x.toFixed(1)}" y="${(_.y+16).toFixed(1)}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.3)">${D}</text>`;x+=`<rect x="${b}" y="${B}" width="${n}" height="${u}" rx="5" fill="rgba(0,0,0,0.01)" class="deck-slot-hit" data-pos="${E}" style="cursor:pointer"/>`}d.innerHTML=`<svg viewBox="${-k} ${-k} ${c+k*2} ${o+k*2}" width="100%" style="display:block;max-width:440px;margin:0 auto">${x}</svg>`,d.querySelectorAll(".deck-slot-hit").forEach(E=>{E.addEventListener("click",()=>Pi(E.dataset.pos,t,e,r))})}function Pi(e,t,i,r){var f,g,k;const{openModal:d,closeModal:s}=r,a=e.replace(/\d+/,""),l=t.slots[e],c=l?t.playerCards.find(p=>p.id===l):null;(f=c==null?void 0:c.player)==null||f.id;const o=new Set;Object.entries(t.slots).forEach(([p,x])=>{var _;if(p===e||!x)return;const E=t.playerCards.find(j=>j.id===x);(_=E==null?void 0:E.player)!=null&&_.id&&o.add(E.player.id)}),t.subs.forEach(p=>{var E;const x=t.playerCards.find(_=>_.id===p);(E=x==null?void 0:x.player)!=null&&E.id&&o.add(x.player.id)});const n=new Set,u=t.playerCards.filter(p=>{const x=p.player;return!(x.job===a||x.job2===a)||o.has(x.id)||n.has(x.id)?!1:(n.add(x.id),!0)});u.sort((p,x)=>{const E=a==="GK"?p.player.note_g:a==="DEF"?p.player.note_d:a==="MIL"?p.player.note_m:p.player.note_a;return(a==="GK"?x.player.note_g:a==="DEF"?x.player.note_d:a==="MIL"?x.player.note_m:x.player.note_a)-E}),d(`Choisir ${a} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${u.length>0?u.map(p=>{var D,I;const x=p.player,E=a==="GK"?x.note_g:a==="DEF"?x.note_d:a==="MIL"?x.note_m:x.note_a,_=mt(x),j={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[x.rarity];return`<div class="player-option" data-card-id="${p.id}"
          ...
          <!-- Note -->
          <div style="width:36px;height:36px;border-radius:8px;background:${Ce[a]};color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:16px;border:2px solid ${j};flex-shrink:0">
            ${E}
          </div>
        </div>`}).join(""):'<div style="text-align:center;color:var(--gray-600);padding:20px">Aucun joueur pour ce poste.<br><small>Ouvre des boosters !</small></div>'}
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(g=document.getElementById("close-selector"))==null||g.addEventListener("click",s),(k=document.getElementById("remove-player"))==null||k.addEventListener("click",()=>{delete t.slots[e],s(),Ue(i,t,r)}),document.querySelectorAll(".player-option").forEach(p=>{p.addEventListener("click",()=>{t.slots[e]=p.dataset.cardId,s(),Ue(i,t,r)})})}function Hi(e,t,i){var c;const{openModal:r,closeModal:d}=i,s=new Set;Object.values(e.slots).filter(Boolean).forEach(o=>{var u;const n=e.playerCards.find(f=>f.id===o);(u=n==null?void 0:n.player)!=null&&u.id&&s.add(n.player.id)}),e.subs.forEach(o=>{var u;const n=e.playerCards.find(f=>f.id===o);(u=n==null?void 0:n.player)!=null&&u.id&&s.add(n.player.id)});const a=new Set,l=e.playerCards.filter(o=>{var n,u,f;return s.has((n=o.player)==null?void 0:n.id)||a.has((u=o.player)==null?void 0:u.id)?!1:(a.add((f=o.player)==null?void 0:f.id),!0)});r("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
  ...
  `,document.body.appendChild(r);let d=!1;const s=document.getElementById("pack-cut-zone"),a=document.getElementById("pack-blade");let l=!1;const c=v=>v.touches&&v.touches[0]?v.touches[0].clientX:v.clientX;function o(v){d||(l=!0,a.style.opacity="1",n(v))}function n(v){if(!l||d)return;const q=s.getBoundingClientRect(),R=c(v)-q.left,W=Math.max(0,Math.min(1,R/q.width));a.style.width=W*q.width+"px",W>=.82&&f()}function u(){d||(l=!1,a.style.transition="width .2s ease, opacity .2s ease",a.style.width="0",a.style.opacity="0",setTimeout(()=>{d||(a.style.transition="")},220))}function f(){var q;if(d)return;d=!0,l=!1,a.style.width="100%",a.style.opacity="1",(q=document.getElementById("cut-flash"))==null||q.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const v=document.getElementById("cut-hint");v&&(v.style.opacity="0"),s.classList.add("pack-cut"),setTimeout(()=>{a.style.opacity="0",document.getElementById("pack-phase").style.display="none",x(0)},620)}s.addEventListener("pointerdown",o),window.addEventListener("pointermove",n),window.addEventListener("pointerup",u),s.addEventListener("touchstart",o,{passive:!0}),window.addEventListener("touchmove",n,{passive:!0}),window.addEventListener("touchend",u);let g=0,k=!1;const p=new Set;function x(v){g=v,document.getElementById("reveal-phase").style.display="flex",E(),_(v,0),B()}function E(){const v=document.getElementById("card-dots");v&&(v.innerHTML=e.map((q,R)=>`<div class="card-dot" data-i="${R}" style="width:8px;height:8px;border-radius:50%;background:${R===g?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),v.querySelectorAll(".card-dot").forEach(q=>q.addEventListener("click",()=>D(parseInt(q.dataset.i)))))}function _(v,q){var M;const R=e[v],W=document.getElementById("card-counter"),ie=document.getElementById("card-track");W&&(W.textContent=`Carte ${v+1} / ${e.length}`);const ae=R.card_type==="player"&&((M=R.player)==null?void 0:M.rarity)==="legende",oe=!p.has(v);p.add(v);let xe=0;if(R.card_type==="player"&&R.player){const $=R.player,G=$.job||"ATT";xe=Number(G==="GK"?$.note_g:G==="DEF"?$.note_d:G==="MIL"?$.note_m:$.note_a)||0}const me=$=>{ie.innerHTML=`
        ...
        </div>`;const G=document.getElementById("current-card-wrap");q!==0?(G.style.transition="none",G.style.transform=`translateX(${q>0?100:-100}%)`,requestAnimationFrame(()=>{G.style.transition="transform .28s cubic-bezier(.25,1,.5,1)",G.style.transform="translateX(0)"})):G.animate([{opacity:0,transform:"scale(.7)"},{opacity:1,transform:"scale(1)"}],{duration:300,easing:"cubic-bezier(.34,1.56,.64,1)"}),$||ae?X():Q(),E()};oe&&xe>6&&R.card_type==="player"&&R.player?j(R,()=>me(!0)):me(!1)}function j(v,q){var me;k=!0;const R=v.player,W=`https://flagsapi.com/${R.country_code}/flat/64.png`,ie=(me=R.clubs)==null?void 0:me.logo_url,ae=document.getElementById("walkout-overlay"),oe=document.getElementById("walkout-stage");if(!ae||!oe){k=!1,q();return}ae.style.display="flex";const xe=()=>{const M=oe.firstElementChild;M&&(M.classList.remove("wo-in"),M.classList.add("wo-out"))};oe.innerHTML=`<img class="wo-in" src="${W}" style="height:130px;border-radius:10px;box-shadow:0 10px 36px rgba(0,0,0,.6)" onerror="this.style.display='none'">`,navigator.vibrate&&navigator.vibrate(30),setTimeout(xe,2e3),setTimeout(()=>{var M;oe.innerHTML=ie?`<img class="wo-in" src="${ie}" style="max-height:160px;max-width:210px;object-fit:contain">`:`<div class="wo-in" style="font-size:34px;font-weight:900;color:#fff;text-align:center">${((M=R.clubs)==null?void 0:M.encoded_name)||"CLUB"}</div>`,navigator.vibrate&&navigator.vibrate(30)},2450),setTimeout(xe,4450),setTimeout(()=>{ae.style.display="none",oe.innerHTML="",k=!1,navigator.vibrate&&navigator.vibrate([40,30,80]),q()},4900)}function D(v){if(k||v<0||v>=e.length||v===g)return;const q=v>g?1:-1;g=v,_(v,q)}function I(){D(g+1)}function b(){D(g-1)}function B(){const v=document.getElementById("card-viewport");if(!v||v._swipeBound)return;v._swipeBound=!0;let q=0,R=0,W=0,ie=!1;const ae=$=>$.touches?$.touches[0].clientX:$.clientX,oe=$=>$.touches?$.touches[0].clientY:$.clientY,xe=$=>{ie=!0,q=ae($),R=oe($),W=0},me=$=>{if(!ie)return;W=ae($)-q;const G=oe($)-R;if(Math.abs(W)<Math.abs(G))return;const N=document.getElementById("current-card-wrap");N&&(N.style.transition="none",N.style.transform=`translateX(${W*.6}px) rotate(${W*.02}deg)`)},M=()=>{if(!ie)return;ie=!1;const $=document.getElementById("current-card-wrap"),G=55;W<=-G&&g<e.length-1?I():W>=G&&g>0?b():$&&($.style.transition="transform .2s ease",$.style.transform="translateX(0)")};v.addEventListener("pointerdown",xe),v.addEventListener("pointermove",me),v.addEventListener("pointerup",M),v.addEventListener("pointercancel",M),v.addEventListener("touchstart",xe,{passive:!0}),v.addEventListener("touchmove",me,{passive:!0}),v.addEventListener("touchend",M),v.addEventListener("click",$=>{if(Math.abs(W)>8)return;const G=v.getBoundingClientRect();$.clientX-G.left>G.width/2?I():b()})}let H=null;function X(){const v=document.getElementById("fireworks-canvas");if(!v)return;v.width=window.innerWidth,v.height=window.innerHeight;const q=v.getContext("2d"),R=[];function W(){const ae=Math.random()*v.width,oe=Math.random()*v.height*.6,xe=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],me=xe[Math.floor(Math.random()*xe.length)];for(let M=0;M<60;M++){const $=Math.PI*2/60*M,G=2+Math.random()*5;R.push({x:ae,y:oe,vx:Math.cos($)*G,vy:Math.sin($)*G,alpha:1,color:me,size:2+Math.random()*3})}}W(),H=setInterval(W,600);function ie(){if(document.getElementById("fireworks-canvas")){q.clearRect(0,0,v.width,v.height);for(let ae=R.length-1;ae>=0;ae--){const oe=R[ae];if(oe.x+=oe.vx,oe.y+=oe.vy+.08,oe.vy*=.98,oe.alpha-=.018,oe.alpha<=0){R.splice(ae,1);continue}q.globalAlpha=oe.alpha,q.fillStyle=oe.color,q.beginPath(),q.arc(oe.x,oe.y,oe.size,0,Math.PI*2),q.fill()}q.globalAlpha=1,(H!==null||R.length>0)&&requestAnimationFrame(ie)}}ie()}function Q(){H!==null&&(clearInterval(H),H=null);const v=document.getElementById("fireworks-canvas");v&&v.getContext("2d").clearRect(0,0,v.width,v.height)}document.getElementById("reveal-collection").addEventListener("click",()=>{Q(),r.remove(),i("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{Q(),r.remove(),i("boosters")})}function lo(e){var t,i;if(e.card_type==="player"&&e.player)return Qi(e);if(e.card_type==="game_changer"){const r=e._gcDef,d=(r==null?void 0:r.gc_type)==="ultra_game_changer",s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},a={purple:"#b06ce0",light_blue:"#00d4ef"},l=s[r==null?void 0:r.color]||s.purple,c=a[r==null?void 0:r.color]||a.purple,o=(r==null?void 0:r.name)||e.gc_type||"Game Changer",n=(r==null?void 0:r.effect)||((t=vt[e.gc_type])==null?void 0:t.desc)||"",u=r!=null&&r.image_url?`/manager-wars/icons/${r.image_url}`:null,f=((i=vt[e.gc_type])==null?void 0:i.icon)||"⚡";return`<div style="width:170px;height:240px;background:${l};border-radius:14px;border:3px solid ${c};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${c}88;flex-shrink:0">
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}const uo="simulation",xi="VOTRE_ZONE_ID";function fo(){switch(uo){case"propellerads":return go();case"adsense":return mo();default:return zt()}}function zt(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:#111;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:14px";let i=5;t.innerHTML=`
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      <div style="flex:1;overflow-y:auto;display:flex;flex-wrap:wrap;gap:10px;justify-content:center;align-content:flex-start;padding:8px 16px 16px">
        ${s.map(k=>{const p=a.find(x=>x.gc_type===k.gc_type);return l(k,!!p)}).join("")}
      </div>
    ...
    </div>`,e.querySelectorAll(".gc-select-card").forEach(k=>{k.addEventListener("click",()=>{const p=k.dataset.id,x=s.find(_=>_.id===p);if(!x)return;const E=a.findIndex(_=>_.gc_type===x.gc_type);E>-1?a.splice(E,1):a.length<3&&a.push(x),o()})}),(u=e.querySelector("#gc-launch"))==null||u.addEventListener("click",()=>{n&&c(a)}),(f=e.querySelector("#gc-no-gc"))==null||f.addEventListener("click",()=>c([])),(g=e.querySelector("#gc-reset"))==null||g.addEventListener("click",()=>{a.length&&(a=[],o())})}o()}async function xo(e,t,i){const{state:r,navigate:d}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:s}=await z.from("decks").select("id,name,is_active,formation").eq("owner_id",r.profile.id).order("created_at",{ascending:!1});if(!s||s.length===0){Be(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>d("decks"));return}const a=s.map(n=>n.id),{data:l}=await z.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order");let c=0;function o(){var _,j,D,I,b;const n=s[c],u=(l||[]).filter(B=>B.deck_id===n.id),f=u.filter(B=>{var H;return B.is_starter&&((H=B.card)==null?void 0:H.player)}).map(B=>wt(B.card,B.position)),g=u.find(B=>{var H;return((H=B.card)==null?void 0:H.card_type)==="formation"}),k=n.formation||((_=g==null?void 0:g.card)==null?void 0:_.formation)||"4-4-2",p=f.length>=11?ut(f,k):null,x=f.length>=11;je(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div style="display:flex;align-items:center;gap:8px;padding:8px;flex-shrink:0">
        ...
        <div style="flex:1;text-align:center">
          ...
          <div style="font-size:11px;opacity:.6;margin-top:2px">${k} · ${f.length}/11 ${n.is_active?"· ⭐ Actif":""}</div>
        </div>
      ...
      <div id="deck-swipe-zone" style="flex:1;min-height:0;overflow:hidden;position:relative;touch-action:pan-y;display:flex;align-items:center;justify-content:center;padding:4px">
        ${p?`<div class="deck-preview-wrap" style="aspect-ratio:1/1;max-width:100%;max-height:100%;width:auto;height:100%;overflow:hidden">${tt(p,k,null,[],285,285)}</div>`:`<div style="display:flex;align-items:center;justify-content:center;height:100%;opacity:.4;flex-direction:column;gap:8px">
      ...
      <div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">
        ${s.map((B,H)=>`<div style="width:7px;height:7px;border-radius:50%;background:${H===c?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}
      </div>`:""}
    ...
    </div>`,function(){const H=e.querySelector(".deck-preview-wrap svg");H&&(H.removeAttribute("width"),H.removeAttribute("height"),H.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",H.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),(j=document.getElementById("prev-deck"))==null||j.addEventListener("click",()=>{c>0&&(c--,o())}),(D=document.getElementById("next-deck"))==null||D.addEventListener("click",()=>{c<s.length-1&&(c++,o())}),(I=document.getElementById("validate-deck"))==null||I.addEventListener("click",()=>{x&&t.navigate("match",{matchMode:i,deckId:n.id})}),(b=document.getElementById("cancel-deck-select"))==null||b.addEventListener("click",()=>{Le(e),d("home")});const E=document.getElementById("deck-swipe-zone");if(E){let B=0,H=0;E.addEventListener("touchstart",X=>{B=X.touches[0].clientX,H=X.touches[0].clientY},{passive:!0}),E.addEventListener("touchend",X=>{const Q=X.changedTouches[0].clientX-B,T=X.changedTouches[0].clientY-H;Math.abs(Q)<40||Math.abs(Q)<Math.abs(T)||(Q<0&&c<s.length-1?(c++,o()):Q>0&&c>0&&(c--,o()))},{passive:!0})}}o()}function bi(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";return e!=null&&e.clubLogo?e.clubLogo.startsWith("http")?e.clubLogo:`${t}/storage/v1/object/public/assets/clubs/${e.clubLogo}`:null}function Te(e,t=44,i=58){if(!e)return`<div style="width:${t}px;height:${i}px;border:1.5px dashed rgba(255,255,255,0.2);border-radius:5px"></div>`;const r=typeof e.portrait=="string"&&e.portrait.startsWith("http")?e.portrait:ve(e),d=bi(e),s=e._line||e.job||"MIL",a=Ve[s]||"#555",l={normal:"#aaa",pépite:"#D4A017",papyte:"#222",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",c=e.note!==void 0?Number(e.note)||0:(Number(ce(e,s))||0)+(e.boost||0),o=vi(e==null?void 0:e.country_code),n=Math.round(i*.19),u=Math.round(i*.25),f=i-Math.round(i*.19)-Math.round(i*.25),g=e!=null&&e.used?.28:1;return`<div style="width:${t}px;height:${i}px;border-radius:5px;border:2px solid ${l};background:${a};position:relative;overflow:hidden;flex-shrink:0;opacity:${g}">
    <div style="position:absolute;top:0;left:0;right:0;height:${n}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:center;z-index:2">
  ...
  </div>`}function Oe(e,t,i){if(!(e!=null&&e.length))return"";const r=e.slice(0,5);let d='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return r.forEach((s,a)=>{if(d+=Te(s,40,52),a<r.length-1){const l=Fe(s,r[a+1]);d+=`<div style="width:7px;height:3px;background:${l==="#ff3333"||l==="#cc2222"?"rgba(255,255,255,0.12)":l};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(d+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),d+="</div>",d}function $t(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function vi(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function et(e,t,i,r,d=310,s=310,a=[]){var j;const l=it[t]||{},c=li(t)||Tt[t]||[],o={},n=["ATT","MIL","DEF","GK"];for(const D of n)(e[D]||[]).forEach((b,B)=>{o[`${D}${B+1}`]=b});function u(D){const I=l[D];return I?{x:I.x*d,y:I.y*s}:null}let f="";for(const[D,I]of c){const b=u(D),B=u(I);if(!b||!B)continue;const H=o[D],X=o[I],Q=Fe(H,X);Q==="#00ff88"||Q==="#FFD700"?(f+=`<line x1="${b.x.toFixed(1)}" y1="${b.y.toFixed(1)}" x2="${B.x.toFixed(1)}" y2="${B.y.toFixed(1)}"
        stroke="${Q}" stroke-width="10" stroke-linecap="round" opacity="0.22"/>`,f+=`<line x1="${b.x.toFixed(1)}" y1="${b.y.toFixed(1)}" x2="${B.x.toFixed(1)}" y2="${B.y.toFixed(1)}"
        stroke="${Q}" stroke-width="3.5" stroke-linecap="round" opacity="0.95"/>`):f+=`<line x1="${b.x.toFixed(1)}" y1="${b.y.toFixed(1)}" x2="${B.x.toFixed(1)}" y2="${B.y.toFixed(1)}"
        stroke="${Q}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const g=48,k=64,p=13,x=16,E={normal:"#aaaaaa",pépite:"#D4A017",papyte:"#111111",légende:"#7a28b8"};for(const[D,I]of Object.entries(o)){const b=u(D);if(!b||!I)continue;const B=D.replace(/[0-9]/g,""),H=Ve[B]||"#555",X=a.includes(I.cardId),Q=i==="attack"&&(["MIL","ATT"].includes(B)||X)&&!I.used||i==="defense"&&["GK","DEF","MIL"].includes(B)&&!I.used,T=r.includes(I.cardId);let v;i==="attack"?v=X?Math.max(1,Number(I.note_a)||0):B==="MIL"?Number(I.note_m)||0:Number(I.note_a)||0:i==="defense"?v=B==="GK"?Number(I.note_g)||0:B==="MIL"?Number(I.note_m)||0:Number(I.note_d)||0:v=Number(B==="GK"?I.note_g:B==="DEF"?I.note_d:B==="MIL"?I.note_m:I.note_a)||0,v=v+(I.boost||0);const q=(b.x-g/2).toFixed(1),R=(b.y-k/2).toFixed(1),W=E[I==null?void 0:I.rarity]||E.normal;if(I.used){const $=typeof import.meta<"u"&&"/manager-wars/"||"/",N=`${typeof window<"u"&&((j=window.location)==null?void 0:j.origin)||""}${$}icons/carte-dos.png`.replace(/([^:])\/\//g,"$1/");f+=`<rect x="${q}" y="${R}" width="${g}" height="${k}" rx="5" fill="#161616"/>`,f+=`<image href="${N}" xlink:href="${N}" x="${q}" y="${R}" width="${g}" height="${k}" preserveAspectRatio="xMidYMid slice"/>`,f+=`<rect x="${q}" y="${R}" width="${g}" height="${k}" rx="5" fill="none" stroke="${W}" stroke-width="2" opacity="0.7"/>`;continue}const ie=T?.45:1,ae=T?"#FFD700":W,oe=T?3:(I==null?void 0:I.rarity)==="légende"||(I==null?void 0:I.rarity)==="pépite"?2.5:2,xe=k-p-x;f+=`<defs><clipPath id="cp-${D}"><rect x="${q}" y="${(b.y-k/2+p).toFixed(1)}" width="${g}" height="${xe}"/></clipPath></defs>`,f+=`<rect x="${q}" y="${R}" width="${g}" height="${k}" rx="5" fill="${H}" opacity="${ie}"/>`;const me=ve(I);me&&(f+=`<image href="${me}" xlink:href="${me}" x="${q}" y="${(b.y-k/2+p).toFixed(1)}" width="${g}" height="${xe}" clip-path="url(#cp-${D})" preserveAspectRatio="xMidYMin slice"/>`),f+=`<rect x="${q}" y="${R}" width="${g}" height="${p}" rx="4" fill="rgba(255,255,255,0.92)"/>`,f+=`<text x="${b.x.toFixed(1)}" y="${(b.y-k/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(I.name||"").slice(0,9)}</text>`;const M=(b.y+k/2-x).toFixed(1);f+=`<rect x="${q}" y="${M}" width="${g}" height="${x}" fill="rgba(255,255,255,0.92)"/>`;{const $=$t(I.country_code);$?f+=`<image href="${$}" xlink:href="${$}" x="${(b.x-20).toFixed(1)}" y="${(b.y+k/2-x+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:f+=`<text x="${(b.x-13).toFixed(1)}" y="${(b.y+k/2-x/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${vi(I.country_code)}</text>`,f+=`<text x="${b.x.toFixed(1)}" y="${(b.y+k/2-x/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${v}</text>`;const G=bi(I);G?f+=`<image href="${G}" xlink:href="${G}" x="${(b.x+g/2-14).toFixed(1)}" y="${(b.y+k/2-x+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:I.clubName&&(f+=`<text x="${(b.x+14).toFixed(1)}" y="${(b.y+k/2-x/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(I.clubName||"").slice(0,3).toUpperCase()}</text>`),I.boost&&(f+=`<rect x="${(b.x+g/2-12).toFixed(1)}" y="${(b.y-k/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,f+=`<text x="${(b.x+g/2-5).toFixed(1)}" y="${(b.y-k/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${I.boost}</text>`)}f+=`<rect x="${q}" y="${R}" width="${g}" height="${k}" rx="5" fill="${T?"rgba(255,255,255,0.12)":"none"}" stroke="${ae}" stroke-width="${oe}" opacity="${ie}"/>`,Q&&(f+=`<rect x="${q}" y="${R}" width="${g}" height="${k}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${T?"selected":""}" data-card-id="${I.cardId}" data-role="${B}" style="cursor:pointer"/>`)}const _=38;return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-_} ${-_} ${d+_*2} ${s+_*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${f}
  ...
  </div>`}async function wi(e,t,i,r){var I;const{state:d,navigate:s,toast:a}=t;je(e);const l=d.params||{};if(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!l.deckId)return xo(e,t,i);const c=l.deckId;let o,n,u,f;try{const b=await Promise.all([z.from("decks").select("formation,name").eq("id",c).single(),z.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation,
            player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
              clubs(encoded_name,logo_url)))`).eq("deck_id",c).order("slot_order")]);o=b[0].data,u=b[0].error,n=b[1].data,f=b[1].error}catch(b){console.error("[Match] Exception chargement deck:",b),Be(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>s("home"));return}if(u||f){console.error("[Match] Erreur Supabase:",u||f),Be(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>s("home"));return}const g=(n||[]).filter(b=>{var B;return b.is_starter&&((B=b.card)==null?void 0:B.player)}).map(b=>wt(b.card,b.position)),k=(n||[]).filter(b=>{var B;return!b.is_starter&&((B=b.card)==null?void 0:B.player)}).map(b=>wt(b.card,b.position));if(g.length<11){Be(e,"⚠️",`Deck incomplet (${g.length}/11).`,"Compléter",()=>s("decks"));return}const p=(n||[]).find(b=>{var B;return((B=b.card)==null?void 0:B.card_type)==="formation"}),x=(o==null?void 0:o.formation)||((I=p==null?void 0:p.card)==null?void 0:I.formation)||"4-4-2",{data:E,error:_}=await z.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",d.profile.id).eq("card_type","game_changer"),{data:j}=await z.from("gc_definitions").select("*").eq("is_active",!0),D=(E||[]).map(b=>({...b,_gcDef:(j==null?void 0:j.find(B=>B.name===b.gc_type||B.id===b.gc_definition_id))||null}));r({deckId:c,formation:x,starters:g,subsRaw:k,gcCardsEnriched:D,gcDefs:j||[]})}async function yo(e,t){const{state:i}=t,d=(i.params||{}).matchMode||"vs_ai_easy",s=d.replace("vs_ai_",""),a=d;await wi(e,t,d,async({deckId:l,formation:c,starters:o,subsRaw:n,gcCardsEnriched:u,gcDefs:f})=>{try{const g=ut(o,c),k=await ho(c,s),p=async x=>{try{const{data:E,error:_}=await z.from("matches").insert({home_id:i.profile.id,away_id:null,mode:a,home_deck_id:l,status:"in_progress"}).select().single();if(_){console.error("[MatchIA] Erreur création match:",_),Be(e,"⚠️","Impossible de créer le match ("+_.message+").","Retour",()=>t.navigate("home"));return}const j={gcDefs:f||[],matchId:E==null?void 0:E.id,mode:a,difficulty:s,formation:c,homeTeam:g,aiTeam:k,homeSubs:n,subsUsed:0,maxSubs:Math.min(n.length,3),homeScore:0,aiScore:0,gcCards:x,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};vo(e,j,t)}catch(E){console.error("[MatchIA] Exception launchMatch:",E),Be(e,"⚠️","Erreur au lancement du match : "+E.message,"Retour",()=>t.navigate("home"))}};if(!u.length){p([]);return}hi(e,u,p)}catch(g){console.error("[MatchIA] Exception setup:",g),Be(e,"⚠️","Erreur de préparation du match : "+g.message,"Retour",()=>t.navigate("home"))}})}async function ho(e,t){var a,l;const{data:i}=await z.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(60);if(!i||i.length<11)return bo(e);const r=Ke[e]||Ke["4-4-2"],d={GK:[],DEF:[],MIL:[],ATT:[]},s=[...i];for(const c of["GK","DEF","MIL","ATT"]){const o=s.filter(k=>k.job===c),n=s.filter(k=>k.job!==c),u=[...o,...n],f=[];for(let k=0;k<r[c];k++){const p=u[k]||s[k];p&&f.push({cardId:"ai-"+p.id+"-"+k,id:p.id,firstname:p.firstname,name:p.surname_encoded,country_code:p.country_code,club_id:p.club_id,job:p.job,job2:p.job2,note_g:Number(p.note_g)||0,note_d:Number(p.note_d)||0,note_m:Number(p.note_m)||0,note_a:Number(p.note_a)||0,rarity:p.rarity,skin:p.skin,hair:p.hair,hair_length:p.hair_length,clubName:((a=p.clubs)==null?void 0:a.encoded_name)||null,clubLogo:((l=p.clubs)==null?void 0:l.logo_url)||null,boost:0,used:!1,_line:c})}const g=pt(f.length);f.forEach((k,p)=>{k._col=g[p]}),d[c]=f}return d}function bo(e){const t=Ke[e]||Ke["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},r=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let d=0;for(const s of["GK","DEF","MIL","ATT"]){const a=[];for(let c=0;c<t[s];c++){const o=3+Math.floor(Math.random()*5);a.push({cardId:"fake-"+d,id:"fake-"+d,firstname:"IA",name:r[d%r.length],country_code:"XX",club_id:null,job:s,job2:null,note_g:s==="GK"?o:2,note_d:s==="DEF"?o:2,note_m:s==="MIL"?o:2,note_a:s==="ATT"?o:2,rarity:"normal",boost:0,used:!1,_line:s}),d++}const l=pt(a.length);a.forEach((c,o)=>{c._col=l[o]}),i[s]=a}return i}function vo(e,t,i){e.innerHTML=`
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
  </div>`,setTimeout(()=>wo(e,t,i),5e3)}function wo(e,t,i){const r=t.homeTeam.MIL||[],d=t.aiTeam.MIL||[];function s(p){return p.reduce((x,E)=>x+ce(E,"MIL"),0)}function a(p){let x=0;for(let E=0;E<p.length-1;E++){const _=Fe(p[E],p[E+1]);_==="#00ff88"?x+=2:_==="#FFD700"&&(x+=1)}return x}const l=s(r)+a(r),c=s(d)+a(d),o=l>=c;function n(p,x,E,_){return`<div id="duel-row-${_}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0), opacity .5s ease;transform-origin:center">
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${p.map((j,D)=>{const I=D<p.length-1?Fe(j,p[D+1]):null,b=!I||I==="#ff3333"||I==="#cc2222",B=I==="#00ff88"?"+2":I==="#FFD700"?"+1":"";return`
          <div class="duel-card duel-card-${_}" data-idx="${D}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease, transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
            ${Te({...j,note:ce(j,"MIL"),_line:"MIL"},58,78)}
          </div>
          ${D<p.length-1?`<div class="duel-link duel-link-${_}" data-idx="${D}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${b?"rgba(255,255,255,0.12)":I};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${b?"none":`0 0 8px ${I}`}">
  ...
  <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div id="duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center, rgba(10,61,30,0.4), rgba(10,61,30,0.92))"></div>
  </div>`;const u=()=>{const p=(x,E)=>e.querySelectorAll(x).forEach((_,j)=>{setTimeout(()=>{_.style.opacity="1",_.style.transform="translateY(0) scale(1)"},E+j*90)});p(".duel-card-home",150),p(".duel-card-ai",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((x,E)=>{setTimeout(()=>{x.style.opacity="1"},E*70)}),900),setTimeout(()=>{const x=e.querySelector("#vs-label");x&&(x.style.opacity="1",x.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(E=>E.style.opacity="1")},1250),setTimeout(()=>{f("score-home",l,800),f("score-ai",c,800)},1500)};function f(p,x,E){const _=document.getElementById(p);if(!_)return;const j=performance.now(),D=I=>{const b=Math.min(1,(I-j)/E);_.textContent=Math.round(x*(1-Math.pow(1-b,3))),b<1?requestAnimationFrame(D):_.textContent=x};requestAnimationFrame(D)}requestAnimationFrame(u),t.attacker=o?"home":"ai";const g=o?yi():null;o&&(t.boostCard={value:g}),t.log.push({type:"duel",title:"Milieu de Terrain",homePlayers:r.map(p=>({name:p.name,note:ce(p,"MIL"),portrait:ve(p),job:p.job,country_code:p.country_code,rarity:p.rarity,clubName:p.clubName,clubLogo:p.clubLogo})),aiPlayers:d.map(p=>({name:p.name,note:ce(p,"MIL"),portrait:ve(p),job:p.job,country_code:p.country_code,rarity:p.rarity,clubName:p.clubName,clubLogo:p.clubLogo})),homeTotal:l,aiTotal:c,text:`Duel milieu : ${t.clubName} ${l} – ${c} IA → ${o?t.clubName+" attaque":"IA attaque"}`});const k=()=>{t.phase=t.attacker==="home"?"attack":"ai-attack",ge(e,t,i),t.attacker==="ai"&&setTimeout(()=>_t(e,t,i),800)};setTimeout(()=>{const p=document.getElementById("score-home"),x=document.getElementById("score-ai"),E=document.getElementById(o?"duel-row-home":"duel-row-ai"),_=document.getElementById(o?"duel-row-ai":"duel-row-home"),j=o?p:x,D=o?x:p;j&&(j.style.fontSize="80px",j.style.color=o?"#FFD700":"#ff6b6b",j.style.animation="duelPulse .5s ease"+(o?", duelGlow 1.5s ease infinite .5s":"")),D&&(D.style.opacity="0.25"),setTimeout(()=>{E&&(E.style.transformOrigin="center",E.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",E.style.zIndex="5"),setTimeout(()=>{var b;const I=document.getElementById("duel-shock");if(I){const B=(b=_||E)==null?void 0:b.getBoundingClientRect(),H=e.querySelector(".match-screen").getBoundingClientRect();B&&(I.style.top=B.top-H.top+B.height/2+"px"),I.style.animation="shockwave .5s ease-out forwards"}_&&(_.style.transformOrigin="center",_.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var b;const I=document.getElementById("duel-finale");I&&(I.innerHTML=`
          ...
          <button id="start-match-btn" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,0.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">
            ▶ Commencer le match
          </button>`,I.style.transition="opacity .45s ease",I.style.opacity="1",I.style.pointerEvents="auto",(b=document.getElementById("start-match-btn"))==null||b.addEventListener("click",k))},600)},700)},2800)}function $o(e){if(e.type==="duel"){const t=e.homeTotal>=e.aiTotal;return`
    ...
    <div style="display:flex;align-items:center;gap:4px;${t?"flex-direction:row-reverse":""};background:rgba(255,255,255,0.04);border-radius:8px;padding:5px 8px;border:1px solid rgba(255,255,255,0.07)">
      ...
      ${Pe(e.inPlayer||{})}
    </div>`}return`<div style="font-size:11px;color:${e.type==="goal"?"#FFD700":"rgba(255,255,255,0.65)"};font-weight:${e.type==="goal"?700:400};padding:3px 2px">${e.text||""}</div>`}function ge(e,t,i){var j,D,I,b,B,H,X,Q;const r=t.selected.map(T=>T.cardId),d=t.usedSubIds||[],s=t.homeSubs.filter(T=>!d.includes(T.cardId)),l=Object.values(t.homeTeam).flat().filter(T=>T.used).length>0&&s.length>0&&t.subsUsed<t.maxSubs,c=!["GK","DEF","MIL","ATT"].some(T=>(t.aiTeam[T]||[]).some(v=>!v.used)),o=[...t.homeTeam.MIL||[],...t.homeTeam.ATT||[]].filter(T=>!T.used),n=t.phase==="attack"&&c&&o.length===0?[...t.homeTeam.GK||[],...t.homeTeam.DEF||[]].filter(T=>!T.used).map(T=>T.cardId):[];t.log[t.log.length-1];const u=t.phase==="ai-attack"||t.phase==="ai-defense",f=t.phase==="attack",g=t.phase==="defense",k=t.phase==="finished",p=t.gcCards.filter(T=>!t.usedGc.includes(T.id)),x=t.boostCard&&!t.boostUsed;e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column",e.innerHTML=`
  <style>
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const v=t.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
            ...
            ${Oe((v.players||[]).map(q=>({...q,used:!1})),"#ff6b6b",v.total)}
            ...
            ${Oe((v.players||[]).map(q=>({...q,used:!1})),"#00ff88",v.total)}
          </div>`}const T=t.log[t.log.length-1];return T?'<div style="padding:2px 4px">'+$o(T)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})()}
    </div>
    ...
    ${(()=>{const T=window.innerWidth>=700,v=(M,$,G)=>{var F,O;const N=(t.gcDefs||[]).find(C=>C.name===M.gc_type),V={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[N==null?void 0:N.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",te={purple:"#b06ce0",light_blue:"#00d4ef"}[N==null?void 0:N.color]||"#b06ce0",K=(N==null?void 0:N.name)||M.gc_type,m=(N==null?void 0:N.effect)||((F=Ae[M.gc_type])==null?void 0:F.desc)||"",y=N!=null&&N.image_url?`/manager-wars/icons/${N.image_url}`:null,w=((O=Ae[M.gc_type])==null?void 0:O.icon)||"⚡",A=Math.round(G*.22),h=Math.round(G*.22),S=G-A-h,L=K.length>12?7:9;return`<div class="gc-mini" data-gc-id="${M.id}" data-gc-type="${M.gc_type}"
          style="box-sizing:border-box;width:${$}px;height:${G}px;border-radius:10px;border:2px solid ${te};background:${V};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
          <div style="height:${A}px;background:rgba(255,255,255,0.14);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 3px">
            <span style="font-size:${L}px;font-weight:900;color:#fff;text-align:center;line-height:1.1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${$-6}px">${K}</span>
          ...
          <div style="height:${S}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
            ${y?`<img src="${y}" style="max-width:${$-10}px;max-height:${S-6}px;object-fit:contain">`:`<span style="font-size:${Math.round(S*.55)}px">${w}</span>`}
          </div>
        ...
        </div>`},q=(M,$)=>{var G;return`<div id="boost-card"
          style="box-sizing:border-box;width:${M}px;height:${$}px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:2px solid #87CEEB;border-radius:10px;cursor:pointer;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:${Math.round($*.04)}px;text-align:center;flex-shrink:0">
            <div style="font-size:${Math.round($*.2)}px">⚡</div>
            <div style="font-size:${Math.round($*.09)}px;color:#000;font-weight:900">+${(G=t.boostCard)==null?void 0:G.value}</div>
          </div>`},R=(M,$)=>$?q(130,175):v(M,130,175),W=(M,$)=>$?q(68,95):v(M,68,95),ie=T?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",ae=k?`<button id="btn-results" style="${ie};background:linear-gradient(135deg,#D4A017,#FFD700);border:none;color:#000">🏁 Résultats</button>`:u?`<div style="${ie};background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);color:rgba(255,255,255,0.4)">⏳ Tour IA</div>`:f?`<button id="btn-action" style="${ie};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${t.selected.length===0?"disabled":""}> ⚔️ ATTAQUEZ <span id="match-timer" style="font-weight:900"></span></button>`:g?`<button id="btn-action" style="${ie};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${t.selected.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="match-timer" style="font-weight:900"></span></button>`:`<div style="${ie};background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1)"></div>`,oe=f||g?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${t.selected.length}/3 sélectionné(s)</div>`:"",xe=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${T?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
        ${s.length===0?'<div style="font-size:7px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':s.map(M=>`<div class="sub-btn-col" data-sub-id="${M.cardId}" style="cursor:pointer;flex-shrink:0">${Te(M,76,100)}</div>`).join("")}
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(T=>{var v,q,R;if(T.type==="duel"){const W=T.isGoal,ie=T.homeScored?"#FFD700":W?"#ff6b6b":"rgba(255,255,255,0.3)",ae=T.homeScored?"⚽ BUT !":W?"⚽ BUT IA !":(v=T.homePlayers)!=null&&v.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${W?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${ie};margin-bottom:4px">
                <div style="font-size:9px;color:${ie};letter-spacing:1px;margin-bottom:5px;font-weight:700;text-transform:uppercase">${ae}</div>
                ${(q=T.homePlayers)!=null&&q.length?`<div style="margin-bottom:3px">${Oe(T.homePlayers,"rgba(255,255,255,0.7)",T.homeTotal)}</div>`:""}
                ${(R=T.aiPlayers)!=null&&R.length?`<div style="opacity:0.7">${Oe(T.aiPlayers,"#ff6b6b",T.aiTotal)}</div>`:""}
                ...
                <div style="display:flex;align-items:center;gap:8px">
                  ${T.outPlayer?Te({...T.outPlayer,used:!0,_line:T.outPlayer.job,rarity:"normal"},38,50):""}
                  <span style="color:rgba(255,255,255,0.4);font-size:18px">→</span>
                  ${T.inPlayer?Te({...T.inPlayer,_line:T.inPlayer.job,rarity:"normal"},38,50):""}
                </div>
  ...
  `,document.body.appendChild(r);let n=!1;const u=()=>{n||(n=!0,r.remove(),i())};r.addEventListener("click",u),setTimeout(u,2e3)}function Je(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function ni(e,t,i,r=null){if(t.phase!=="attack"){Je("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){Je(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const d=Object.values(t.homeTeam).flat().filter(u=>u.used),s=t.homeSubs.filter(u=>!t.usedSubIds.includes(u.cardId));if(!d.length){Je("Aucun joueur utilisé à remplacer");return}if(!s.length){Je("Aucun remplaçant disponible");return}let a=0,l=Math.max(0,s.findIndex(u=>u.cardId===r)),c=!1;const o=document.createElement("div");o.id="sub-overlay",o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function n(){var E,_,j,D,I,b;const u=d[a],f=s[l],g=Math.min(130,Math.round((window.innerWidth-90)/2)),k=Math.round(g*1.35),p=B=>`background:rgba(255,255,255,0.12);border:none;color:${B?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${B?"default":"pointer"};flex-shrink:0`;o.innerHTML=`
    ...
    <div style="flex:1;display:flex;gap:0;overflow:hidden">
      ...
      <div id="in-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
        ...
        <div>${f?Te({...f,used:!1,boost:0},g,k):"<div>—</div>"}</div>
      ...
      <div id="out-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
        ...
        <div>${u?Te({...u,used:!1,boost:0},g,k):"<div>—</div>"}</div>
    ...
    </div>`,(E=o.querySelector("#sub-close"))==null||E.addEventListener("click",()=>o.remove()),(_=o.querySelector("#out-up"))==null||_.addEventListener("click",()=>{a>0&&(a--,n())}),(j=o.querySelector("#out-down"))==null||j.addEventListener("click",()=>{a<d.length-1&&(a++,n())}),(D=o.querySelector("#in-up"))==null||D.addEventListener("click",()=>{l>0&&(l--,n())}),(I=o.querySelector("#in-down"))==null||I.addEventListener("click",()=>{l<s.length-1&&(l++,n())});const x=(B,H,X,Q)=>{const T=o.querySelector("#"+B);if(!T)return;let v=0;T.addEventListener("touchstart",q=>{v=q.touches[0].clientY},{passive:!0}),T.addEventListener("touchend",q=>{const R=q.changedTouches[0].clientY-v;if(Math.abs(R)<30)return;const W=H();R<0&&W<Q-1?(X(W+1),n()):R>0&&W>0&&(X(W-1),n())},{passive:!0})};x("in-panel",()=>l,B=>l=B,s.length),x("out-panel",()=>a,B=>a=B,d.length),(b=o.querySelector("#sub-confirm"))==null||b.addEventListener("click",B=>{if(B.preventDefault(),B.stopPropagation(),c)return;c=!0;const H=d[a],X=s[l];if(!H||!X)return;let Q=null,T=-1;for(const[q,R]of Object.entries(t.homeTeam)){const W=(R||[]).findIndex(ie=>ie.cardId===H.cardId);if(W!==-1){Q=q,T=W;break}}if(T===-1||!Q){Je("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),o.remove();return}const v={...X,_line:Q,_col:H._col||0,used:!1,boost:0};t.homeTeam[Q].splice(T,1,v),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(X.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:H.name,firstname:H.firstname,note:ce(H,Q),portrait:ve(H),job:H.job,country_code:H.country_code,rarity:H.rarity,clubName:H.clubName,clubLogo:H.clubLogo},inPlayer:{name:X.name,firstname:X.firstname,note:ce(X,Q),portrait:ve(X),job:X.job,country_code:X.country_code,rarity:X.rarity,clubName:X.clubName,clubLogo:X.clubLogo},text:`🔄 ${X.firstname} ${X.name} remplace ${H.firstname} ${H.name}`}),o.remove(),Ao(H,X,()=>ge(e,t,i))})}document.body.appendChild(o),n()}function Io(e,t,i,r,d){var k,p;const s=(r.gcDefs||[]).find(x=>x.name===t),a=Ae[t]||{icon:"⚡",desc:"Carte spéciale."},l={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[s==null?void 0:s.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",c={purple:"#b06ce0",light_blue:"#00d4ef"}[s==null?void 0:s.color]||"#b06ce0",o=(s==null?void 0:s.name)||t,n=(s==null?void 0:s.effect)||a.desc,u=s!=null&&s.image_url?`/manager-wars/icons/${s.image_url}`:null,f=a.icon||"⚡",g=document.createElement("div");g.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",g.innerHTML=`
    ...
    </div>`,document.body.appendChild(g),(k=g.querySelector("#gc-back"))==null||k.addEventListener("click",()=>g.remove()),(p=g.querySelector("#gc-use"))==null||p.addEventListener("click",()=>{g.remove(),Mo(e,t,i,r,d)})}function nt(e,t,i,r,d,s){const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let l=[];function c(){var o,n;a.innerHTML=`
    ...
    <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
      ${e.map(u=>{const f=u._line||u.job||"MIL",g={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[f]||"#555",k=ce(u,f)+(u.boost||0),p=l.find(x=>x.cardId===u.cardId);return`<div class="gc-pick-item" data-cid="${u.cardId}"
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(s);let l=!1;const c=()=>{l||(l=!0,s.remove(),setTimeout(()=>d(),50))};s.addEventListener("click",c),setTimeout(c,3500)}async function Qe(e,t,i){var n,u;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:r}=i,d=t.homeScore>t.aiScore,s=t.homeScore===t.aiScore,a=d?"victoire":s?"nul":"defaite",l=Bi(t.mode,a);t.matchId&&await z.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:d?r.profile.id:null,home_credits_reward:l,played_at:new Date().toISOString()}).eq("id",t.matchId);const c={credits:(r.profile.credits||0)+l,matches_played:(r.profile.matches_played||0)+1};d?c.wins=(r.profile.wins||0)+1:s?c.draws=(r.profile.draws||0)+1:c.losses=(r.profile.losses||0)+1,await z.from("users").update(c).eq("id",r.profile.id),await i.refreshProfile();const o=document.createElement("div");o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",o.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const g=async(j=!0)=>{u=!0,f&&(z.removeChannel(f),f=null),j&&await z.rpc("cancel_matchmaking",{p_user_id:c.profile.id}).catch(()=>{})};(_=document.getElementById("mm-cancel"))==null||_.addEventListener("click",async()=>{try{await g(!0)}catch{}Le(e),o("home")});const k=async(j,D,I)=>{if(u)return;u=!0,f&&(z.removeChannel(f),f=null);const b=document.getElementById("mm-status");b&&(b.textContent="Adversaire trouvé !"),await new Promise(B=>setTimeout(B,600)),e.isConnected&&Bo(e,t,j,I,a,l)},{data:p,error:x}=await z.rpc("try_matchmake",{p_user_id:c.profile.id,p_deck_id:i});if(x||!(p!=null&&p.success)){n("Erreur de matchmaking","error"),Le(e),o("home");return}if(p.matched){k(p.match_id,p.opponent_id,!1);return}const E=document.getElementById("mm-status");E&&(E.textContent="En attente d'un autre joueur..."),f=z.channel("matchmaking-"+c.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${c.profile.id}`},j=>{const D=j.new;D.status==="matched"&&D.match_id&&k(D.match_id,D.matched_with,!0)}).subscribe()}async function Bo(e,t,i,r,d=[],s=[]){const{state:a,navigate:l,toast:c}=t,o=r?"p1":"p2",n=r?"p2":"p1",u=(d||[]).map(m=>m.id),f=(d||[]).map(m=>({id:m.id,gc_type:m.gc_type,_gcDef:m._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:g}=await z.from("matches").select("*").eq("id",i).single();if(!g){c("Match introuvable","error"),l("home");return}async function k(){const[{data:m},{data:y},{data:w},{data:A}]=await Promise.all([z.rpc("get_deck_for_match",{p_deck_id:g.home_deck_id}),z.rpc("get_deck_for_match",{p_deck_id:g.away_deck_id}),z.from("users").select("id,pseudo,club_name").eq("id",g.home_id).single(),z.from("users").select("id,pseudo,club_name").eq("id",g.away_id).single()]),h=C=>({cardId:C.card_id,position:C.position,id:C.id,firstname:C.firstname,name:C.surname_encoded,country_code:C.country_code,club_id:C.club_id,job:C.job,job2:C.job2,note_g:Number(C.note_g)||0,note_d:Number(C.note_d)||0,note_m:Number(C.note_m)||0,note_a:Number(C.note_a)||0,rarity:C.rarity,skin:C.skin,hair:C.hair,hair_length:C.hair_length,clubName:C.club_encoded_name||null,clubLogo:C.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}),S=((m==null?void 0:m.starters)||[]).map(C=>h(C)),L=((y==null?void 0:y.starters)||[]).map(C=>h(C)),F=(m==null?void 0:m.formation)||"4-4-2",O=(y==null?void 0:y.formation)||"4-4-2";return{p1Team:ut(S,F),p2Team:ut(L,O),p1Subs:((m==null?void 0:m.subs)||[]).map(C=>h(C)),p2Subs:((y==null?void 0:y.subs)||[]).map(C=>h(C)),p1Formation:F,p2Formation:O,p1Name:(w==null?void 0:w.club_name)||(w==null?void 0:w.pseudo)||"Joueur 1",p2Name:(A==null?void 0:A.club_name)||(A==null?void 0:A.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:r?u:[],gc_p2:r?[]:u,gcCardsFull_p1:r?f:[],gcCardsFull_p2:r?[]:f,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,lastActionAt:new Date().toISOString()}}let p=g.game_state&&Object.keys(g.game_state).length?g.game_state:null;if(!p)if(r){p=await k();const{data:m}=await z.from("matches").select("game_state").eq("id",i).single();!(m!=null&&m.game_state)||!Object.keys(m.game_state).length?await z.from("matches").update({game_state:p,turn_user_id:g.home_id}).eq("id",i):p=m.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let m=0;m<30&&!p;m++){await new Promise(w=>setTimeout(w,400));const{data:y}=await z.from("matches").select("game_state").eq("id",i).single();y!=null&&y.game_state&&Object.keys(y.game_state).length&&(p=y.game_state)}if(!p){c("Erreur de synchronisation","error"),l("home");return}p.gc_p2=u,p.gcCardsFull_p2=f,await z.from("matches").update({game_state:p}).eq("id",i)}let x=!1,E=null,_=!1;const j=new Set;function D(m){var A,h;try{z.removeChannel(I)}catch{}const y=m.winner_id===a.profile.id;(A=document.getElementById("pvp-end-overlay"))==null||A.remove();const w=document.createElement("div");w.id="pvp-end-overlay",w.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center",w.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(w),(h=w.querySelector("#pvp-end-home"))==null||h.addEventListener("click",()=>{w.remove(),Le(e),l("home")})}const I=z.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},m=>{const y=m.new;try{if(y.status==="finished"||y.forfeit){if(x)return;x=!0,E&&(clearInterval(E),E=null),y.game_state&&(p=y.game_state),D(y);return}if(y.game_state){const w=p;p=y.game_state;const A=w[o+"Score"]||0,h=w[n+"Score"]||0,S=p[o+"Score"]||0,L=p[n+"Score"]||0,F=S>A,O=L>h;if((F||O)&&!j.has(p.round)){j.add(p.round);const C=[...p.log||[]].reverse().find(Y=>Y.isGoal),U=((C==null?void 0:C.homePlayers)||[]).map(Y=>({name:Y.name,note:Y.note,portrait:Y.portrait,job:Y.job}));ae(U,S,L,F,()=>q());return}q()}}catch(w){console.error("[PvP] crash:",w)}}).subscribe();async function b(m){Object.assign(p,m),p.lastActionAt=new Date().toISOString();const{error:y}=await z.from("matches").update({game_state:p}).eq("id",i);y&&c("Erreur de synchronisation","error");try{q()}catch(w){console.error("[PvP] renderPvpScreen crash:",w)}}async function B(){if(x)return;x=!0,E&&(clearInterval(E),E=null);const m=r?g.away_id:g.home_id,y=r?"p2":"p1",w=r?"p1":"p2",A={...p,[y+"Score"]:3,[w+"Score"]:0,phase:"finished"};try{await z.from("matches").update({status:"finished",forfeit:!0,winner_id:m,game_state:A}).eq("id",i)}catch{}try{z.removeChannel(I)}catch{}setTimeout(()=>{Le(e),l("home")},800)}const H={BOOST_STAT:({value:m=1,count:y=1,roles:w=[]},A,h)=>{const S=A[o+"Team"],L=Object.entries(S).filter(([F])=>!w.length||w.includes(F)).flatMap(([F,O])=>O.filter(C=>!C.used).map(C=>({...C,_line:F})));if(!L.length){A.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),h(A);return}X(L,y,`Choisir ${y} joueur(s) à booster (+${m})`,F=>{F.forEach(O=>{const C=(S[O._line]||[]).find(U=>U.cardId===O.cardId);C&&(C.boost=(C.boost||0)+m,A.log.push({text:`⚡ +${m} sur ${C.name}`,type:"info"}))}),A[o+"Team"]=S,h(A)})},DEBUFF_STAT:({value:m=1,count:y=1,roles:w=[],target:A="ai"},h,S)=>{const L=A==="home"?o:n,F=h[L+"Team"],O=A==="home"?"allié":"adverse",C=Object.entries(F).filter(([U])=>!w.length||w.includes(U)).flatMap(([U,Y])=>Y.filter(se=>!se.used).map(se=>({...se,_line:U})));if(!C.length){h.log.push({text:`🎯 Aucun joueur ${O}`,type:"info"}),S(h);return}X(C,y,`Choisir ${y} joueur(s) ${O}(s) (-${m})`,U=>{U.forEach(Y=>{const se=(F[Y._line]||[]).find(le=>le.cardId===Y.cardId);se&&(se.boost=(se.boost||0)-m,h.log.push({text:`🎯 -${m} sur ${se.name}`,type:"info"}))}),h[L+"Team"]=F,S(h)})},GRAY_PLAYER:({count:m=1,roles:y=[],target:w="ai"},A,h)=>{const S=w==="home"?o:n,L=A[S+"Team"],F=w==="home"?"allié":"adverse",O=Object.entries(L).filter(([C])=>!y.length||y.includes(C)).flatMap(([C,U])=>U.filter(Y=>!Y.used).map(Y=>({...Y,_line:C})));if(!O.length){A.log.push({text:`❌ Aucun joueur ${F}`,type:"info"}),h(A);return}X(O,m,`Choisir ${m} joueur(s) ${F}(s) à exclure`,C=>{C.forEach(U=>{const Y=(L[U._line]||[]).find(se=>se.cardId===U.cardId);Y&&(Y.used=!0,A.log.push({text:`❌ ${Y.name} exclu !`,type:"info"}))}),A[S+"Team"]=L,h(A)})},REVIVE_PLAYER:({count:m=1,roles:y=[]},w,A)=>{const h=w[o+"Team"],S=Object.entries(h).filter(([L])=>!y.length||y.includes(L)).flatMap(([L,F])=>F.filter(O=>O.used).map(O=>({...O,_line:L})));if(!S.length){w.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),A(w);return}X(S,m,`Choisir ${m} joueur(s) à ressusciter`,L=>{L.forEach(F=>{const O=(h[F._line]||[]).find(C=>C.cardId===F.cardId);O&&(O.used=!1,w.log.push({text:`💫 ${O.name} ressuscité !`,type:"info"}))}),w[o+"Team"]=h,A(w)})},REMOVE_GOAL:({},m,y)=>{const w=n+"Score";m[w]>0?(m[w]--,m.log.push({text:"🚫 Dernier but annulé !",type:"info"})):m.log.push({text:"🚫 Aucun but à annuler",type:"info"}),y(m)},ADD_GOAL_DRAW:({},m,y)=>{m[o+"Score"]===m[n+"Score"]?(m[o+"Score"]++,m.log.push({text:"🎯 But bonus !",type:"info"})):m.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),y(m)},ADD_SUB:({value:m=1},y,w)=>{y.maxSubs=(y.maxSubs||3)+m,y.log.push({text:`🔄 +${m} remplacement(s)`,type:"info"}),w(y)},CUSTOM:({},m,y)=>y(m)};function X(m,y,w,A){const h=document.createElement("div");h.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let S=[];function L(){var O,C;const F=m.map(U=>{const Y={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[U._line]||"#555",se=ce(U,U._line)+(U.boost||0),$e=S.find(Z=>Z.cardId===U.cardId)?"#FFD700":"rgba(255,255,255,0.25)",ye=U.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${U.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${$e};background:${Y};overflow:hidden;cursor:pointer;${ye}">
          <div style="background:rgba(255,255,255,0.9);text-align:center;padding:2px;font-size:7px;font-weight:900;color:#111;overflow:hidden;white-space:nowrap;text-overflow:ellipsis">${U.name||"?"}</div>
        ...
        <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
          ...
          <span style="font-size:12px;color:rgba(255,255,255,0.5)">${S.length}/${y}</span>
        ...
        <div style="padding:12px;background:rgba(0,0,0,0.4);flex-shrink:0">
          <button id="pp-confirm" ${S.length===0?'disabled style="opacity:0.4"':""} style="width:100%;padding:13px;border-radius:10px;border:none;background:#7a28b8;color:#fff;font-size:15px;font-weight:900;cursor:pointer">
            ✅ Confirmer (${S.length}/${y})
          </button>
        </div>`,(O=h.querySelector("#pp-close"))==null||O.addEventListener("click",()=>h.remove()),h.querySelectorAll(".pp-item").forEach(U=>{U.addEventListener("click",()=>{const Y=U.dataset.cid,se=m.find($e=>$e.cardId===Y),le=S.findIndex($e=>$e.cardId===Y);se&&(le>-1?S.splice(le,1):S.length<y&&S.push(se),L())})}),(C=h.querySelector("#pp-confirm"))==null||C.addEventListener("click",()=>{h.remove(),A(S)})}L(),document.body.appendChild(h)}async function Q(m,y){const A=(p["gcCardsFull_"+o]||[]).find(F=>F.id===m),h=(A==null?void 0:A._gcDef)||(p.gcDefs||[]).find(F=>{var O;return F.name===y||((O=F.name)==null?void 0:O.toLowerCase().trim())===(y==null?void 0:y.toLowerCase().trim())}),S=[...p["usedGc_"+o]||[],m];if(h!=null&&h.effect_type&&h.effect_type!=="CUSTOM"){const F=H[h.effect_type];if(F){const O={...p};F(h.effect_params||{},O,async C=>{C["usedGc_"+o]=S,await b(C)});return}}const L={...p};switch(y){case"Remplacement+":L.maxSubs=(L.maxSubs||3)+1,L.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const F=n+"Score";L[F]>0&&(L[F]--,L.log.push({text:"🚫 But annulé",type:"info"}));break}}L["usedGc_"+o]=S,await b(L)}function T(m,y){const w="usedCardIds_"+m,A=new Set(p[w]||[]);y.forEach(h=>A.add(h)),p[w]=[...A]}function v(){for(const m of["p1","p2"]){const y=new Set(p["usedCardIds_"+m]||[]),w=p[m+"Team"];if(w)for(const A of["GK","DEF","MIL","ATT"])(w[A]||[]).forEach(h=>{h.used=y.has(h.cardId)})}}function q(){var Ft,Dt,Gt,qt,Rt,Nt;if(p.phase==="reveal")return R();if(p.phase==="midfield")return ie();if(p.phase==="finished")return K();const m=p[o+"Team"],y=p[n+"Team"];v();const w=p[o+"Score"],A=p[n+"Score"],h=p[o+"Name"],S=p[n+"Name"],L=p.phase===o+"-attack",F=p.phase===o+"-defense",O=Array.isArray(p["selected_"+o])?p["selected_"+o]:[],C=O.map(P=>P.cardId),U=window.innerWidth>=700,Y=p[o+"Subs"]||[],se=p["usedSubIds_"+o]||[],le=Y.filter(P=>!se.includes(P.cardId)),$e=p["gcCardsFull_"+o]||[],ye=p["usedGc_"+o]||[],Z=$e.filter(P=>!ye.includes(P.id)),J=p.boostOwner===o&&!p.boostUsed,re=!["GK","DEF","MIL","ATT"].some(P=>(y[P]||[]).some(ee=>!ee.used)),de=[...m.MIL||[],...m.ATT||[]].filter(P=>!P.used),we=L&&re&&de.length===0?[...m.GK||[],...m.DEF||[]].filter(P=>!P.used).map(P=>P.cardId):[];function he(P,ee,fe){var Ut,Kt;const ne=P._gcDef,Se={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ne==null?void 0:ne.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",Re={purple:"#b06ce0",light_blue:"#00d4ef"}[ne==null?void 0:ne.color]||"#b06ce0",Ye=(ne==null?void 0:ne.name)||P.gc_type,We=(ne==null?void 0:ne.effect)||((Ut=Ae[P.gc_type])==null?void 0:Ut.desc)||"",Ot=ne!=null&&ne.image_url?`/manager-wars/icons/${ne.image_url}`:null,Ti=((Kt=Ae[P.gc_type])==null?void 0:Kt.icon)||"⚡",Pt=Math.round(fe*.22),Ht=Math.round(fe*.22),yt=fe-Pt-Ht,Ai=Ye.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${P.id}" data-gc-type="${P.gc_type}"
    ...
    </div>`,Me=L?"attack":F?"defense":"idle",ze=`<div style="overflow:hidden;min-width:0;flex:1;min-height:0;display:flex;flex-direction:column" id="match-field">
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${tt(m,p[o+"Formation"],Me,C,300,300,we)}
      </div>
        ...
        </div>`}if(L&&((fe=p.pendingAttack)!=null&&fe.players)){const ne=p.pendingAttack;return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #00ff88">
          <div style="font-size:8px;color:#00ff88;letter-spacing:2px;margin-bottom:4px;text-transform:uppercase">⚔️ VOUS ATTAQUEZ</div>
      ...
      <div style="display:flex;align-items:center;padding:8px 10px;background:rgba(0,0,0,0.5);gap:6px;flex-shrink:0">
        ...
        <div style="flex:1;display:flex;align-items:center;justify-content:center;gap:8px">
          ...
          <span style="font-size:26px;font-weight:900;color:#FFD700;letter-spacing:2px">${w} – ${A}</span>
      ...
      <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
        ...
        <div id="mobile-action-bar" style="position:absolute;left:0;right:0;bottom:0;z-index:20;background:rgba(0,0,0,0.55);padding:6px 8px 8px;display:flex;flex-direction:column;gap:6px;box-shadow:0 -4px 16px rgba(0,0,0,0.5)">
          <div style="display:flex;gap:6px;overflow-x:auto;align-items:flex-end;min-height:96px;padding-bottom:2px">
            ${Z.map(P=>ke(P,!1)).join("")}
            ${J?ke(null,!0):""}
            <div id="pvp-sub-open" style="cursor:${L&&le.length>0?"pointer":"default"};flex-shrink:0;box-sizing:border-box;width:68px;height:95px;border-radius:10px;border:2px solid ${L&&le.length>0?"rgba(255,255,255,0.5)":"rgba(255,255,255,0.15)"};background:${L&&le.length>0?"rgba(60,60,60,0.9)":"rgba(40,40,40,0.5)"};display:flex;flex-direction:column;align-items:center;justify-content:center;gap:4px;opacity:${L&&le.length>0?1:.4}">
              <div style="display:flex;gap:6px;align-items:center">
                <div style="text-align:center">
                  <div style="font-size:7px;color:#00ff88;font-weight:700;letter-spacing:1px">IN</div>
      ...
      </div>`;function qe(){const P=e.querySelector(".match-screen");if(!P)return;const ee=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);P.style.height=ee+"px",P.style.minHeight=ee+"px",P.style.maxHeight=ee+"px",P.style.overflow="hidden";const fe=e.querySelector("#mobile-action-bar"),ne=e.querySelector("#mobile-play-area");fe&&ne&&(ne.style.paddingBottom=fe.offsetHeight+"px")}if(qe(),setTimeout(qe,120),setTimeout(qe,400),_||(_=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",qe),window.visualViewport.addEventListener("scroll",qe)),window.addEventListener("resize",qe)),function(){const ee=e.querySelector(".terrain-wrapper svg");ee&&(ee.removeAttribute("width"),ee.removeAttribute("height"),ee.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",ee.setAttribute("viewBox","-26 -26 352 352"),ee.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),e.querySelectorAll(".match-slot-hit").forEach(P=>{P.addEventListener("click",()=>{if(!L&&!F)return;const ee=P.dataset.cardId,fe=P.dataset.role,ne=(m[fe]||[]).find(We=>We.cardId===ee);if(!ne||ne.used)return;const Se=we.includes(ee);if(L&&!["MIL","ATT"].includes(fe)&&!Se)return;Array.isArray(p["selected_"+o])||(p["selected_"+o]=[]);const Re=p["selected_"+o],Ye=Re.findIndex(We=>We.cardId===ee);Ye>-1?Re.splice(Ye,1):Re.length<3&&Re.push({...ne,_role:fe}),q()})}),e.querySelectorAll(".pvp-sub-btn").forEach(P=>{P.addEventListener("click",()=>me())}),(Ft=e.querySelector("#pvp-sub-open"))==null||Ft.addEventListener("click",()=>me()),e.querySelectorAll(".pvp-gc-mini").forEach(P=>{P.addEventListener("click",()=>oe(P.dataset.gcId,P.dataset.gcType))}),(Dt=e.querySelector("#pvp-boost-card"))==null||Dt.addEventListener("click",()=>xe()),(Gt=e.querySelector("#pvp-action"))==null||Gt.addEventListener("click",()=>{L?N():F&&V()}),(qt=e.querySelector("#pvp-quit"))==null||qt.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&B()}),(Rt=e.querySelector("#pvp-view-opp"))==null||Rt.addEventListener("click",()=>$()),(Nt=e.querySelector("#pvp-toggle-history"))==null||Nt.addEventListener("click",()=>G()),E&&(clearInterval(E),E=null),(L||F)&&!x){let P=30,ee=!1;const fe=()=>document.getElementById("pvp-timer"),ne=()=>{fe()&&(fe().textContent=P+"s",fe().style.color=ee?"#ff4444":"#fff")};ne(),E=setInterval(()=>{P--,P<0?ee?(clearInterval(E),E=null,B()):(ee=!0,P=15,ne()):ne()},1e3)}}function R(){je(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
      ...
      <div style="width:min(90vw,420px)">${et(p[n+"Team"],p[n+"Formation"],null,[],300,300)}</div>
    </div>`,o==="p1"&&setTimeout(async()=>{await b({phase:"midfield"})},5e3)}let W=!1;function ie(){if(W)return;const m=p[o+"Team"].MIL||[],y=p[n+"Team"].MIL||[];function w(Z){return Z.reduce((J,re)=>J+ce(re,"MIL"),0)}function A(Z){let J=0;for(let re=0;re<Z.length-1;re++){const de=Fe(Z[re],Z[re+1]);de==="#00ff88"?J+=2:de==="#FFD700"&&(J+=1)}return J}const h=w(m)+A(m),S=w(y)+A(y),L=h>=S;function F(Z,J,re){return`<div id="duel-row-${re}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
        ...
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${Z.map((de,we)=>{const he=we<Z.length-1?Fe(de,Z[we+1]):null,be=!he||he==="#ff3333"||he==="#cc2222",ue=he==="#00ff88"?"+2":he==="#FFD700"?"+1":"";return`<div class="duel-card duel-card-${re}" data-idx="${we}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease,transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
              ${Te({...de,note:ce(de,"MIL"),_line:"MIL"},58,78)}
            </div>
            ${we<Z.length-1?`<div class="duel-link duel-link-${re}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${be?"rgba(255,255,255,0.12)":he};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${be?"none":`0 0 8px ${he}`}">
              ${ue?`<span style="position:absolute;top:-13px;left:50%;transform:translateX(-50%);font-size:8px;font-weight:900;color:${he}">${ue}</span>`:""}
            </div>`:""}`}).join("")}
        </div>
        <div class="duel-score-line duel-score-line-${re}" style="margin-top:10px;font-size:12px;color:rgba(255,255,255,0.55);opacity:0;transition:opacity .4s ease">
          Score: ${w(Z)} + ${A(Z)} liens = <b style="color:#fff">${w(Z)+A(Z)}</b>
        </div>
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    </div>`;const O=(Z,J)=>e.querySelectorAll(Z).forEach((re,de)=>{setTimeout(()=>{re.style.opacity="1",re.style.transform="translateY(0) scale(1)"},J+de*90)});O(".duel-card-me",150),O(".duel-card-opp",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((Z,J)=>setTimeout(()=>{Z.style.opacity="1"},J*70)),900),setTimeout(()=>{const Z=e.querySelector("#pvp-vs");Z&&(Z.style.opacity="1",Z.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(J=>J.style.opacity="1")},1250);function C(Z,J,re){const de=document.getElementById(Z);if(!de)return;const we=performance.now(),he=be=>{const ue=Math.min(1,(be-we)/re);de.textContent=Math.round(J*(1-Math.pow(1-ue,3))),ue<1?requestAnimationFrame(he):de.textContent=J};requestAnimationFrame(he)}setTimeout(()=>{C("pvp-score-me",h,800),C("pvp-score-opp",S,800)},1500);const U=p.p1Team.MIL||[],Y=p.p2Team.MIL||[],se=w(U)+A(U),le=w(Y)+A(Y),$e=se>=le?"p1":"p2";let ye=p.boostValue;ye==null&&(ye=yi(),p.boostValue=ye,p.boostOwner=$e,p.boostUsed=!1),W=!0,setTimeout(()=>{const Z=e.querySelector("#duel-row-"+(L?"me":"opp")),J=e.querySelector("#duel-row-"+(L?"opp":"me")),re=document.getElementById("pvp-score-me"),de=document.getElementById("pvp-score-opp"),we=L?re:de,he=L?de:re;we&&(we.style.fontSize="80px",we.style.color=L?"#FFD700":"#ff6b6b",we.style.animation="duelPulse .5s ease"+(L?",duelGlow 1.5s ease infinite .5s":"")),he&&(he.style.opacity="0.25"),setTimeout(()=>{Z&&(Z.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",Z.style.zIndex="5"),setTimeout(()=>{const be=document.getElementById("duel-shock");be&&(be.style.animation="shockwave .5s ease-out forwards"),J&&(J.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var _e;const be=document.getElementById("pvp-duel-finale");if(!be)return;const ue=p.boostOwner===o?'<div style="background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:3px solid #cdeffd;border-radius:18px;padding:20px 34px;text-align:center;animation:boostFlipIn .7s cubic-bezier(.34,1.56,.64,1) both;box-shadow:0 10px 36px rgba(135,206,235,.5)"><div style="font-size:10px;color:rgba(0,0,0,.6);letter-spacing:2px;text-transform:uppercase;margin-bottom:6px;font-weight:700">Carte Boost obtenue</div><div style="font-size:46px;line-height:1">⚡</div><div style="font-size:50px;font-weight:900;color:#063;line-height:1.1">+'+ye+`</div><div style="font-size:10px;color:rgba(0,0,0,.55);margin-top:4px">Applicable sur n'importe quel joueur</div></div>`:"",ke=o==="p1"?'<button id="pvp-start-match" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">▶ Commencer le match</button>':`<div style="font-size:14px;color:rgba(255,255,255,0.5);text-align:center;margin-top:8px;animation:fadeUp .4s ease both">⏳ En attente de l'adversaire...</div>`;be.innerHTML='<div style="font-size:22px;font-weight:900;color:#fff;text-align:center;animation:fadeUp .4s ease both;text-shadow:0 2px 12px rgba(0,0,0,.5)">'+(L?"⚽ "+p[o+"Name"]+"<br>gagne le milieu et attaque !":"😔 "+p[n+"Name"]+"<br>gagne l'engagement et attaque !")+"</div>"+ue+ke,be.style.transition="opacity .45s ease",be.style.opacity="1",be.style.pointerEvents="auto",(_e=document.getElementById("pvp-start-match"))==null||_e.addEventListener("click",async()=>{const Ee=$e;await b({phase:Ee+"-attack",attacker:Ee,round:1,boostValue:ye,boostUsed:!1,boostOwner:Ee})})},600)},700)},2800)}function ae(m,y,w,A,h){const S=document.createElement("div");S.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:900;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;overflow:hidden;cursor:pointer";const L=Array.from({length:10},(U,Y)=>`<div style="position:absolute;font-size:${16+Math.floor(Math.random()*24)}px;top:${5+Math.floor(Math.random()*65)}%;left:${3+Math.floor(Math.random()*94)}%;animation:fw${Y%2===0?"A":"B"} ${.7+Math.random()*.7}s ease ${Math.random()*.9}s both;opacity:0">${["✨","🌟","⭐","💥","🎇","🎆","🔥","🌈"][Y%8]}</div>`).join(""),F={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};S.innerHTML=`
      <style>
        @keyframes butPop{0%{transform:scale(0) rotate(-8deg);opacity:0}55%{transform:scale(1.25) rotate(2deg)}85%{transform:scale(0.92) rotate(-1deg)}100%{transform:scale(1);opacity:1}}
        @keyframes ballIn{0%{transform:translate(-70px,18px);opacity:0}65%{opacity:1}100%{transform:translate(26px,-8px);opacity:1}}
        @keyframes scoreIn{from{opacity:0;transform:translateY(-12px)}to{opacity:1;transform:translateY(0)}}
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(S);let O=!1;const C=()=>{O||(O=!0,S.remove(),setTimeout(()=>h(),50))};S.addEventListener("click",C),setTimeout(C,3500)}function oe(m,y){var se,le,$e,ye;const A=(p["gcCardsFull_"+o]||[]).find(Z=>Z.id===m),h=A==null?void 0:A._gcDef,S={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[h==null?void 0:h.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",L={purple:"#b06ce0",light_blue:"#00d4ef"}[h==null?void 0:h.color]||"#b06ce0",F=(h==null?void 0:h.name)||y,O=(h==null?void 0:h.effect)||((se=Ae[y])==null?void 0:se.desc)||"Carte spéciale.",C=h!=null&&h.image_url?`/manager-wars/icons/${h.image_url}`:null,U=((le=Ae[y])==null?void 0:le.icon)||"⚡",Y=document.createElement("div");Y.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",Y.innerHTML=`
      <div style="width:190px;border-radius:16px;border:3px solid ${L};background:${S};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 40px ${L}66">
        ...
        <div style="height:160px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
          ${C?`<img src="${C}" style="max-width:150px;max-height:150px;object-fit:contain">`:`<span style="font-size:72px">${U}</span>`}
        </div>
        <div style="padding:10px;background:rgba(0,0,0,0.38);text-align:center">
      ...
      </div>`,document.body.appendChild(Y),($e=Y.querySelector("#pvp-gc-back"))==null||$e.addEventListener("click",()=>Y.remove()),(ye=Y.querySelector("#pvp-gc-use"))==null||ye.addEventListener("click",()=>{Y.remove(),Q(m,y)})}function xe(){var A;const m=p[o+"Team"],y=Object.entries(m).flatMap(([h,S])=>(S||[]).filter(L=>!L.used).map(L=>({...L,_line:h})));if(!y.length)return;const w=document.createElement("div");w.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",w.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
        <div style="flex:1;font-size:14px;font-weight:700;color:#fff">⚡ Choisir un joueur pour +${p.boostValue}</div>
        <button id="bp-close" style="background:none;border:none;color:rgba(255,255,255,0.5);font-size:22px;cursor:pointer">✕</button>
      </div>
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${y.map(h=>{const S={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[h._line]||"#555",L=ce(h,h._line)+(h.boost||0);return`<div class="bp-item" data-cid="${h.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${S};overflow:hidden;cursor:pointer">
      ...
      </div>`,document.body.appendChild(w),(A=w.querySelector("#bp-close"))==null||A.addEventListener("click",()=>w.remove()),w.querySelectorAll(".bp-item").forEach(h=>{h.addEventListener("click",async()=>{const S=h.dataset.cid,L=y.find(O=>O.cardId===S);if(!L)return;const F=(m[L._line]||[]).find(O=>O.cardId===S);F&&(F.boost=(F.boost||0)+p.boostValue),w.remove(),await b({[o+"Team"]:m,boostUsed:!0})})})}function me(){if(!(p.phase===o+"-attack")){c("Remplacement uniquement avant votre attaque","warning");return}const y=p[o+"Team"],w=p["usedSubIds_"+o]||[],A=p.maxSubs||3;if(w.length>=A){c(`Maximum ${A} remplacements atteint`,"warning");return}const h=Object.entries(y).flatMap(([Y,se])=>(se||[]).filter(le=>le.used).map(le=>({...le,_line:Y}))),S=(p[o+"Subs"]||[]).filter(Y=>!w.includes(Y.cardId));if(!h.length){c("Aucun joueur utilisé à remplacer","warning");return}if(!S.length){c("Aucun remplaçant disponible","warning");return}let L=0,F=0,O=!1;const C=document.createElement("div");C.id="pvp-sub-overlay",C.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function U(){var J,re,de,we,he,be;const Y=h[L],se=S[F],le=Math.min(130,Math.round((window.innerWidth-90)/2)),$e=Math.round(le*1.35),ye=ue=>`background:rgba(255,255,255,0.12);border:none;color:${ue?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${ue?"default":"pointer"};flex-shrink:0`;C.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
        <div style="flex:1;font-size:15px;font-weight:900;color:#fff">🔄 Remplacement (${w.length}/${A})</div>
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        <div id="pin-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
          ...
          <button id="pin-down" style="${ye(F>=S.length-1)}" ${F>=S.length-1?"disabled":""}>▼</button>
          <div style="font-size:10px;color:rgba(255,255,255,0.35)">${F+1}/${S.length}</div>
        </div>
        <div id="pout-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
          <div style="font-size:9px;color:#ff6b6b;letter-spacing:2px;text-transform:uppercase;font-weight:700">Joueur qui sort</div>
          <button id="pout-up" style="${ye(L===0)}" ${L===0?"disabled":""}>▲</button>
          <div>${Y?Te({...Y,used:!1,boost:0},le,$e):"<div>—</div>"}</div>
          <button id="pout-down" style="${ye(L>=h.length-1)}" ${L>=h.length-1?"disabled":""}>▼</button>
          <div style="font-size:10px;color:rgba(255,255,255,0.35)">${L+1}/${h.length}</div>
        </div>
      </div>
      <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
        <button id="psub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
      </div>`,(J=C.querySelector("#psub-close"))==null||J.addEventListener("click",()=>C.remove()),(re=C.querySelector("#pout-up"))==null||re.addEventListener("click",()=>{L>0&&(L--,U())}),(de=C.querySelector("#pout-down"))==null||de.addEventListener("click",()=>{L<h.length-1&&(L++,U())}),(we=C.querySelector("#pin-up"))==null||we.addEventListener("click",()=>{F>0&&(F--,U())}),(he=C.querySelector("#pin-down"))==null||he.addEventListener("click",()=>{F<S.length-1&&(F++,U())});const Z=(ue,ke,_e,Ee)=>{const Ie=C.querySelector("#"+ue);if(!Ie)return;let De=0;Ie.addEventListener("touchstart",Me=>{De=Me.touches[0].clientY},{passive:!0}),Ie.addEventListener("touchend",Me=>{const ze=Me.changedTouches[0].clientY-De;if(Math.abs(ze)<30)return;const Ge=ke();ze<0&&Ge<Ee-1?(_e(Ge+1),U()):ze>0&&Ge>0&&(_e(Ge-1),U())},{passive:!0})};Z("pin-panel",()=>F,ue=>F=ue,S.length),Z("pout-panel",()=>L,ue=>L=ue,h.length),(be=C.querySelector("#psub-confirm"))==null||be.addEventListener("click",async ue=>{if(ue.preventDefault(),ue.stopPropagation(),O)return;O=!0;const ke=h[L],_e=S[F];if(!ke||!_e)return;const Ee=ke._line,Ie=(y[Ee]||[]).findIndex(ze=>ze.cardId===ke.cardId);if(Ie===-1){c("Erreur : joueur introuvable","error"),C.remove();return}const De={..._e,_line:Ee,position:ke.position,used:!1,boost:0};y[Ee].splice(Ie,1,De);const Me=[...w,_e.cardId];C.remove(),M(ke,_e,async()=>{await b({[o+"Team"]:y,[n+"Team"]:p[n+"Team"],["usedSubIds_"+o]:Me})})})}document.body.appendChild(C),U()}function M(m,y,w){const A={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},h=document.createElement("div");h.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const S=(O,C,U)=>`<div style="text-align:center">
      <div style="font-size:9px;color:${C};letter-spacing:2px;text-transform:uppercase;font-weight:700;margin-bottom:6px">${U}</div>
      <div style="width:70px;height:70px;border-radius:50%;background:${A[O.job]||"#555"};border:3px solid ${C};position:relative;overflow:hidden;margin:0 auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:6px">Appuyer pour continuer</div>`,document.body.appendChild(h);let L=!1;const F=()=>{L||(L=!0,h.remove(),setTimeout(()=>w(),50))};h.addEventListener("click",F),setTimeout(F,2200)}function $(){var y;const m=document.createElement("div");m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:20px;overflow-y:auto",m.innerHTML=`
      <div style="font-size:12px;color:rgba(255,255,255,0.5);letter-spacing:2px;text-transform:uppercase">Équipe adverse</div>
      <div style="font-size:18px;font-weight:900;color:#ff6b6b">${p[n+"Name"]}</div>
      <div style="width:min(90vw,420px)">${et(p[n+"Team"],p[n+"Formation"],null,[],300,300)}</div>
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(m),(y=m.querySelector("#pvp-opp-close"))==null||y.addEventListener("click",()=>m.remove())}function G(){var w;const m=p.log||[],y=document.createElement("div");y.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column",y.innerHTML=`
      ...
      </div>`,document.body.appendChild(y),(w=y.querySelector("#pvp-hist-close"))==null||w.addEventListener("click",()=>y.remove())}async function N(){const m=p[o+"Team"],y=!["GK","DEF","MIL","ATT"].some(S=>(p[n+"Team"][S]||[]).some(L=>!L.used)),w=(p["selected_"+o]||[]).map(S=>{const L=(m[S._role]||[]).find(O=>O.cardId===S.cardId)||S,F=y&&["GK","DEF"].includes(S._role);return{...L,_line:S._role,...F?{note_a:Math.max(1,Number(L.note_a)||0)}:{}}});if(!w.length)return;const A=At(w,p.modifiers[o]||{});T(o,w.map(S=>S.cardId)),w.forEach(S=>{const L=(m[S._role]||[]).find(F=>F.cardId===S.cardId);L&&(L.used=!0)}),p["selected_"+o]=[],q();const h=[...p.log||[]];if(y){const S=(p[o+"Score"]||0)+1,L=w.map(Y=>({name:Y.name,note:ce(Y,Y._line||"ATT"),portrait:ve(Y),job:Y.job}));h.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:L,homeTotal:A.total,aiTotal:0});const F=(p.round||0)+1,O=o==="p1"?"p2":"p1",C={...p,[o+"Team"]:m,[o+"Score"]:S},U=te(C);j.add(F),ae(L,S,p[n+"Score"]||0,!0,async()=>{await b({[o+"Team"]:m,[o+"Score"]:S,["selected_"+o]:[],modifiers:{...p.modifiers,[o]:{}},pendingAttack:null,phase:U?"finished":O+"-attack",attacker:O,round:F,log:h}),U&&await z.from("matches").update({status:"finished"}).eq("id",i)});return}h.push({type:"duel",text:`⚔️ ${p[o+"Name"]} attaque (${A.total})`,players:w,total:A.total,side:o}),await b({[o+"Team"]:m,[n+"Team"]:p[n+"Team"],pendingAttack:{...A,players:w,side:o},["selected_"+o]:[],modifiers:{...p.modifiers,[o]:{}},phase:n+"-defense",log:h})}async function V(){const m=p[o+"Team"],y=(p["selected_"+o]||[]).map(J=>({...(m[J._role]||[]).find(de=>de.cardId===J.cardId)||J,_line:J._role})),w=It(y,p.modifiers[o]||{});T(o,y.map(J=>J.cardId)),y.forEach(J=>{const re=(m[J._role]||[]).find(de=>de.cardId===J.cardId);re&&(re.used=!0)}),p["selected_"+o]=[],q();const A=Lt(p.pendingAttack.total,w.total,p.modifiers[o]||{}),h=p.pendingAttack.side,S=A==="attack"||(A==null?void 0:A.goal),L=h==="p1"?"p2":"p1",F=(p.round||0)+1,O=(p.pendingAttack.players||[]).map(J=>({name:J.name,note:ce(J,J._line||"ATT"),portrait:ve(J),job:J.job})),C=[...p.log||[]];C.push({type:"duel",isGoal:S,homeScored:S&&h===o,text:S?`⚽ BUT de ${p[h+"Name"]} ! (${p.pendingAttack.total} vs ${w.total})`:`✋ Attaque stoppée (${p.pendingAttack.total} vs ${w.total})`,homePlayers:O,aiPlayers:y.map(J=>({name:J.name,note:ce(J,J._line||"DEF"),portrait:ve(J),job:J.job})),homeTotal:p.pendingAttack.total,aiTotal:w.total});const U=S?(p[h+"Score"]||0)+1:p[h+"Score"]||0,Y={...p,[o+"Team"]:m,[h+"Score"]:U},se=te(Y),le=Y[L+"Team"],ye=!["GK","DEF","MIL","ATT"].some(J=>(le[J]||[]).some(re=>!re.used))||se?"finished":L+"-attack",Z=async()=>{await b({[o+"Team"]:m,[n+"Team"]:p[n+"Team"],[h+"Score"]:U,["selected_"+o]:[],modifiers:{...p.modifiers,[o]:{}},pendingAttack:null,phase:ye,attacker:L,round:F,log:C}),(ye==="finished"||se)&&await z.from("matches").update({status:"finished"}).eq("id",i)};if(S){const J=h===o,re=J?U:p[o+"Score"]||0,de=J?p[n+"Score"]||0:U;j.add(F),ae(O,re,de,J,Z)}else await Z()}function te(m){const y=["MIL","ATT","GK","DEF"].some(A=>(m.p1Team[A]||[]).some(h=>!h.used)),w=["MIL","ATT","GK","DEF"].some(A=>(m.p2Team[A]||[]).some(h=>!h.used));return!y&&!w}function K(){var h;const m=p[o+"Score"],y=p[n+"Score"],w=m>y,A=m===y;je(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      <div style="font-size:64px">${w?"🏆":A?"🤝":"😤"}</div>
      <div style="font-size:24px;font-weight:900;color:#fff">${w?"Victoire !":A?"Match nul":"Défaite"}</div>
      <div style="font-size:32px;font-weight:900;color:#FFD700">${m} - ${y}</div>
      <button id="pvp-home" style="padding:16px 40px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">🏠 Retour</button>
    </div>`,(h=document.getElementById("pvp-home"))==null||h.addEventListener("click",()=>{try{z.removeChannel(I)}catch{}Le(e),l("home")})}q()}const Fo={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"};async function Do(e,t){const{state:i,toast:r}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement du marché...</div>',await jt(e,t)}async function jt(e,t){const{state:i,toast:r}=t,{data:d}=await z.from("market_listings").select(`id, price, status, listed_at, seller_id,
  ...
  `;function c(o){const n=document.getElementById("mkt-content"),u=o==="buy"?a:l;if(u.length===0){n.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      </div>`;return}const f=o==="mine"?[...u].sort((g,k)=>g.status!==k.status?g.status==="active"?-1:1:new Date(k.listed_at)-new Date(g.listed_at)):u;n.innerHTML=`<div style="display:flex;flex-direction:column;gap:10px">
      ${f.map(g=>{var j,D,I,b;const k=(j=g.card)==null?void 0:j.player;if(!k)return"";const p=k.job==="GK"?k.note_g:k.job==="DEF"?k.note_d:k.job==="MIL"?k.note_m:k.note_a,x=Fo[k.rarity],E=(i.profile.credits||0)>=g.price,_=g.status==="sold";return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${_?"opacity:0.65":""}">
          <div style="width:44px;height:44px;border-radius:8px;background:${Ro(k.job)};color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:18px;border:2px solid ${x};flex-shrink:0">${p}</div>
          <div style="flex:1;min-width:0">
  ...
  `}async function Oo(e,{state:t,navigate:i,toast:r}){const d=t.profile;if(!d)return;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:s}=await z.from("matches").select(`id,home_id,away_id,home_score,away_score,status,mode,winner_id,created_at,played_at,
      home:users!home_id(pseudo,club_name),
      away:users!away_id(pseudo,club_name)`).or(`home_id.eq.${d.id},away_id.eq.${d.id}`).order("created_at",{ascending:!1}).limit(50),a={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},l=(s||[]).filter(n=>n.status==="finished"),c=(s||[]).filter(n=>n.status==="in_progress");function o(n){const u=n.home_id===d.id;u?n.home_score:n.away_score,u?n.away_score:n.home_score;const f=n.winner_id===d.id,g=n.home_score===n.away_score&&n.status==="finished",k=n.status!=="finished"?"…":g?"N":f?"V":"D",p=n.status!=="finished"||g?"#888":f?"#1A6B3C":"#c0392b";let x=a[n.mode]||n.mode;n.away_id===null&&!x.startsWith("IA")&&(x="IA");const _=n.home_id===d.id?n.away:n.home;let j;n.away_id===null?j=x:_?j=`${_.club_name||_.pseudo} (${_.pseudo})`:j="Adversaire";let D="";n.status==="in_progress"&&Date.now()-new Date(n.created_at).getTime()>3600*1e3&&(D=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const I=new Date(n.created_at),b=I.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+I.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),B=n.status==="finished"?`${n.home_score} - ${n.away_score}`:`${n.home_score||0} - ${n.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
        <div style="font-size:13px;font-weight:600">${j}${D}</div>
        <div style="font-size:11px;color:var(--gray-600)">${x} · ${b}${n.status==="in_progress"?" · en cours":""}</div>
      </div>
      <div style="display:flex;align-items:center;gap:8px">
