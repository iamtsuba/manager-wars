...
import{s as w,l as Gt,F as Ii,r as Se,j as Jt,m as on,h as Si,n as Mn,o as jn,T as gi,k as Cn,p as Bn,b as qn}from"./formation-links-yJiMLlPw.js";const Dn="/";function Ri(e,{navigate:t,toast:i}){let n="login";const r=()=>{var a,s,l,c,d,u;const o=n==="login";e.innerHTML=`
    <div style="
  ...
  `;function s(){var f;const c=document.getElementById("logo-preview"),d=document.getElementById("logo-initials"),u=((f=document.getElementById("setup-club"))==null?void 0:f.value)||"MW",x=u.trim().split(" ").filter(Boolean),m=x.length>=2?(x[0][0]+x[1][0]).toUpperCase():u.slice(0,2).toUpperCase();c&&(c.style.background=a,c.style.borderColor=o),d&&(d.textContent=m,d.style.color=o)}document.getElementById("color1").addEventListener("input",c=>{o=c.target.value,document.getElementById("swatch1").style.background=o,s()}),document.getElementById("color2").addEventListener("input",c=>{a=c.target.value,document.getElementById("swatch2").style.background=a,s()});function l(c){document.querySelectorAll(".setup-step").forEach(d=>d.classList.remove("active")),document.getElementById(`step-${c}`).classList.add("active"),document.getElementById("step-num").textContent=c,document.getElementById("progress-fill").style.width=`${Math.round(c/3*100)}%`,c===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),d=document.getElementById("step1-error");if(d.textContent="",c.length<3){d.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:u}=await w.from("users").select("id").eq("pseudo",c).maybeSingle();if(u){d.textContent="Ce pseudo est déjà pris.";return}l(2)}),document.getElementById("step2-back").addEventListener("click",()=>l(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const c=document.getElementById("setup-club").value.trim(),d=document.getElementById("step2-error");if(d.textContent="",c.length<2){d.textContent="Nom trop court (min. 2 caractères).";return}const{data:u}=await w.from("users").select("id").eq("club_name",c).maybeSingle();if(u){d.textContent="Ce nom de club est déjà pris.";return}l(3)}),document.getElementById("step3-back").addEventListener("click",()=>l(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),d=document.getElementById("setup-club").value.trim(),u=document.getElementById("step3-error"),x=document.getElementById("step3-finish");u.textContent="",x.disabled=!0,x.textContent="Création en cours…";try{const{error:m}=await w.from("users").insert({id:t.user.id,pseudo:c,club_name:d,club_color1:o,club_color2:a,credits:1e4});if(m)throw m;await Pn(t.user.id),await r(),n(`Bienvenue ${c} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(m){u.textContent=m.message,x.disabled=!1,x.textContent="🚀 Créer mon profil !"}})}async function Pn(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await w.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const Gn="modulepreload",Rn=function(e){return"/"+e},Ni={},rn=function(t,i,n){let r=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),s=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));r=Promise.allSettled(i.map(l=>{if(l=Rn(l),l in Ni)return;Ni[l]=!0;const c=l.endsWith(".css"),d=c?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${l}"]${d}`))return;const u=document.createElement("link");if(u.rel=c?"stylesheet":Gn,c||(u.as="script"),u.crossOrigin="",u.href=l,s&&u.setAttribute("nonce",s),document.head.appendChild(u),c)return new Promise((x,m)=>{u.addEventListener("load",x),u.addEventListener("error",()=>m(new Error(`Unable to preload CSS for ${l}`)))})}))}function o(a){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=a,window.dispatchEvent(s),!s.defaultPrevented)throw a}return r.then(a=>{for(const s of a||[])s.status==="rejected"&&o(s.reason);return t().catch(o)})},Ut="#1A6B3C",Kt="#cc2222",Nn="#D4A017",Oi="#888";async function On(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    </style>`}const Vn="2026.07.18-2233";async function Hi(e,{state:t,navigate:i,toast:n}){var o,a;const r=t.profile;r&&(e.innerHTML=`
  <style>
    .home-dark {
      min-height: 100%;
  ...
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
        ...
        </div>`},null)}e.querySelectorAll(".col-tab-btn").forEach(M=>{M.addEventListener("click",()=>{j=M.dataset.tab,E="Tous",F="",N=!1,e.querySelectorAll(".col-tab-btn").forEach(C=>{const D=C.dataset.tab===j;C.style.borderBottomColor=D?"var(--green)":"transparent",C.style.color=D?"var(--green)":"var(--gray-600)"}),v(),S()})}),v(),S()}function po(e,t,i){const n=Me[e]||{icon:"⚡",desc:"Effet spécial."},r=(t==null?void 0:t.gc_type)==="ultra_game_changer",o={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},a={purple:"#b06ce0",light_blue:"#00d4ef"},s=o[t==null?void 0:t.color]||o.purple,l=a[t==null?void 0:t.color]||a.purple,c=(t==null?void 0:t.name)||e,d=(t==null?void 0:t.effect)||n.desc,u=t!=null&&t.image_url?`/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(ae=document.getElementById("close-detail"))==null||ae.addEventListener("click",s);let S=new Set;const G=()=>{const Y=S.size,J=document.getElementById("sell-action-panel");if(!J)return;J.style.display=Y>0?"block":"none",document.getElementById("sell-selected-count").textContent=Y;const R=document.getElementById("evolve-btn");R&&(R.textContent=`⬆️ Évoluer ${Y>1?"(+"+Y+")":""}`)};document.querySelectorAll(".expl-check").forEach(Y=>{Y.addEventListener("change",()=>{const J=Y.dataset.id;Y.checked?S.add(J):S.delete(J);const R=Y.closest(".exemplaire-row");R&&(R.style.borderColor=Y.checked?"#1A6B3C":"#eee"),G()})}),document.querySelectorAll(".exemplaire-row").forEach(Y=>{Y.addEventListener("click",J=>{if(J.target.closest("button")||J.target.tagName==="INPUT")return;const R=Y.querySelector(".expl-check");R&&!R.disabled&&(R.checked=!R.checked,R.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(Y=>{Y.addEventListener("click",J=>{J.stopPropagation();const R=document.querySelector(`.expl-hist[data-hist="${Y.dataset.idx}"]`);R&&(R.style.display=R.style.display==="none"?"flex":"none")})}),(se=document.getElementById("evolve-btn"))==null||se.addEventListener("click",async()=>{if(x<=1)return;const Y=[...S];if(!Y.length)return;if(S.has(e.id)){const K=document.createElement("div");K.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",K.innerHTML=`
  ...
  `,document.body.appendChild(r);let o=!1;const a=document.getElementById("pack-cut-zone"),s=document.getElementById("pack-blade");let l=!1;const c=v=>v.touches&&v.touches[0]?v.touches[0].clientX:v.clientX;function d(v){o||(l=!0,s.style.opacity="1",u(v))}function u(v){if(!l||o)return;const S=a.getBoundingClientRect(),G=c(v)-S.left,re=Math.max(0,Math.min(1,G/S.width));s.style.width=re*S.width+"px",re>=.82&&m()}function x(){o||(l=!1,s.style.transition="width .2s ease, opacity .2s ease",s.style.width="0",s.style.opacity="0",setTimeout(()=>{o||(s.style.transition="")},220))}function m(){var S;if(o)return;o=!0,l=!1,s.style.width="100%",s.style.opacity="1",(S=document.getElementById("cut-flash"))==null||S.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const v=document.getElementById("cut-hint");v&&(v.style.opacity="0"),a.classList.add("pack-cut"),setTimeout(()=>{s.style.opacity="0",document.getElementById("pack-phase").style.display="none",h(0)},620)}a.addEventListener("pointerdown",d),window.addEventListener("pointermove",u),window.addEventListener("pointerup",x),a.addEventListener("touchstart",d,{passive:!0}),window.addEventListener("touchmove",u,{passive:!0}),window.addEventListener("touchend",x);let f=0,p=!1;const g=new Set;function h(v){f=v,document.getElementById("reveal-phase").style.display="flex",_(),b(v,0),F()}function _(){const v=document.getElementById("card-dots");v&&(v.innerHTML=e.map((S,G)=>`<div class="card-dot" data-i="${G}" style="width:8px;height:8px;border-radius:50%;background:${G===f?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),v.querySelectorAll(".card-dot").forEach(S=>S.addEventListener("click",()=>y(parseInt(S.dataset.i)))))}function b(v,S){var R,M;const G=e[v],re=document.getElementById("card-counter"),ae=document.getElementById("card-track");re&&(re.textContent=`Carte ${v+1} / ${e.length}`);const se=document.getElementById("reveal-btns");se&&(se.style.display=v===e.length-1?"flex":"none");const de=G.card_type==="player"&&((R=G.player)==null?void 0:R.rarity)==="legende",ke=!g.has(v);g.add(v);let Y=0;if(G.card_type==="player"&&G.player){const C=G.player,D=C.job||"ATT";Y=Number(D==="GK"?C.note_g:D==="DEF"?C.note_d:D==="MIL"?C.note_m:C.note_a)||0}const J=C=>{ae.innerHTML=`
  ...
  <div id="match-history-panel">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      </div>`,(Fe=Z.querySelector("#psub-close"))==null||Fe.addEventListener("click",()=>Z.remove()),(Ie=Z.querySelector("#pout-up"))==null||Ie.addEventListener("click",()=>{P>0&&(P--,H())}),(He=Z.querySelector("#pout-down"))==null||He.addEventListener("click",()=>{P<z.length-1&&(P++,H())}),(It=Z.querySelector("#pin-up"))==null||It.addEventListener("click",()=>{V>0&&(V--,H())}),(Je=Z.querySelector("#pin-down"))==null||Je.addEventListener("click",()=>{V<B.length-1&&(V++,H())});const Te=(qe,Qe,Re,ht)=>{const Ze=Z.querySelector("#"+qe);if(!Ze)return;let Ue=0;Ze.addEventListener("touchstart",at=>{Ue=at.touches[0].clientY},{passive:!0}),Ze.addEventListener("touchend",at=>{const st=at.changedTouches[0].clientY-Ue;if(Math.abs(st)<30)return;const vt=Qe();st<0&&vt<ht-1?(Re(vt+1),H()):st>0&&vt>0&&(Re(vt-1),H())},{passive:!0})};Te("pin-panel",()=>V,qe=>V=qe,B.length),Te("pout-panel",()=>P,qe=>P=qe,z.length),(Qt=Z.querySelector("#psub-confirm"))==null||Qt.addEventListener("click",async qe=>{if(qe.preventDefault(),qe.stopPropagation(),oe)return;oe=!0;const Qe=z[P],Re=B[V];if(!Qe||!Re)return;const ht=Qe._line,Ze=(I[ht]||[]).findIndex(st=>st.cardId===Qe.cardId);if(Ze===-1){f("Erreur : joueur introuvable","error"),Z.remove();return}const Ue={...Re,_line:ht,position:Qe.position,used:!1,boost:0};I[ht].splice(Ze,1,Ue);const at=[...A,Re.cardId];Z.remove(),W(Qe,Re,async()=>{await U({[p+"Team"]:I,[g+"Team"]:y[g+"Team"],["usedSubIds_"+p]:at})})})}document.body.appendChild(Z),H()}function W(k,L,I){const A={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const z=(X,V,oe)=>`<div style="text-align:center">
      ...
      <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(me,1400)});await H("⚽ DERNIER CORNER","#FFD700"),await H("🧤 LE GARDIEN MONTE !","#4fc3f7"),Z.remove();const le=I===p?z:L[p+"Score"]||0,pe=I===p?L[g+"Score"]||0:z;ot([jt(A)],le,pe,I===p,async()=>{await U({[T]:z,log:B,round:P,pendingAttack:null,phase:oe?"finished":X+"-attack",attacker:X,["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}}}),oe&&await Xe({...y,[T]:z})})}async function be(){const k=y[p+"Team"],L=!["GK","DEF","MIL","ATT"].some(z=>(y[g+"Team"][z]||[]).some(B=>!B.used)),I=(y["selected_"+p]||[]).map(z=>{const B=(k[z._role]||[]).find(pe=>pe.cardId===z.cardId)||z,P=["GK","DEF"].includes(z._role),X=k[z._role]||[],V=X.findIndex(pe=>pe.cardId===z.cardId),oe=pt(X.length),Z=V>=0?oe[V]:B._col??1,H=y.stadiumDef||y.homeStadiumDef||null,le=B.stadiumBonus||H&&(H.club_id&&String(B.club_id)===String(H.club_id)||H.country_code&&B.country_code===H.country_code)||!1;return{...B,_line:z._role,_col:Z,stadiumBonus:le,...P?{note_a:Math.max(1,Number(B.note_a)||0)}:{}}});if(!I.length)return;const A=Vt(I,y.modifiers[p]||{});ae(p,I.map(z=>z.cardId)),I.forEach(z=>{const B=(k[z._role]||[]).find(P=>P.cardId===z.cardId);B&&(B.used=!0)}),y["selected_"+p]=[],de();const T=[...y.log||[]];if(L){if(I.length===1&&(I[0]._line==="GK"||I[0].job==="GK")&&(y.p1Score||0)===(y.p2Score||0)){await ye({side:p,gk:I[0]},{...y,[p+"Team"]:k,log:T});return}const B=(y[p+"Score"]||0)+1,P=I.map(H=>jt(H));T.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:P,homeTotal:A.total,aiTotal:0});const X=(y.round||0)+1,V=p==="p1"?"p2":"p1",oe={...y,[p+"Team"]:k,[p+"Score"]:B},Z=Oe(oe);N.add(X),ot(P,B,y[g+"Score"]||0,!0,async()=>{await U({[p+"Team"]:k,[p+"Score"]:B,["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:Z?"finished":V+"-attack",attacker:V,round:X,log:T}),Z&&await Xe({...y,[p+"Score"]:B})});return}T.push({type:"pending",text:`⚔️ ${y[p+"Name"]} attaque (${A.total})`}),await U({[p+"Team"]:k,[g+"Team"]:y[g+"Team"],pendingAttack:{...A,players:I,side:p},["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},phase:g+"-defense",log:T})}async function $e(){const k=y[p+"Team"],L=(y["selected_"+p]||[]).map(ce=>{const me=(k[ce._role]||[]).find(Ie=>Ie.cardId===ce.cardId)||ce,xe=k[ce._role]||[],te=xe.findIndex(Ie=>Ie.cardId===ce.cardId),Te=pt(xe.length),Fe=te>=0?Te[te]:me._col??1;return{...me,_line:ce._role,_col:Fe}}),I=Yt(L,y.modifiers[p]||{});ae(p,L.map(ce=>ce.cardId)),L.forEach(ce=>{const me=(k[ce._role]||[]).find(xe=>xe.cardId===ce.cardId);me&&(me.used=!0)}),y["selected_"+p]=[],de();const A=ji(y.pendingAttack.total,I.total,y.modifiers[p]||{}),T=y.pendingAttack.side,z=A==="attack"||(A==null?void 0:A.goal),B=T==="p1"?"p2":"p1",P=(y.round||0)+1,X=(y.pendingAttack.players||[]).map(ce=>jt(ce)),V=[...y.log||[]];V.push({type:"duel",isGoal:z,homeScored:z&&T===p,text:z?`⚽ BUT de ${y[T+"Name"]} ! (${y.pendingAttack.total} vs ${I.total})`:`✋ Attaque stoppée (${y.pendingAttack.total} vs ${I.total})`,homePlayers:X,aiPlayers:L.map(ce=>jt(ce)),homeTotal:y.pendingAttack.total,aiTotal:I.total});const oe=z?(y[T+"Score"]||0)+1:y[T+"Score"]||0,Z={...y,[p+"Team"]:k,[T+"Score"]:oe},H=Oe(Z),le=H?"finished":B+"-attack",pe=!H&&!z?_e(Z):null,ue=async()=>{if(pe){await ye(pe,Z);return}await U({[p+"Team"]:k,[g+"Team"]:y[g+"Team"],[T+"Score"]:oe,["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:le,attacker:B,round:P,log:V}),(le==="finished"||H)&&await Xe({...y,[T+"Score"]:oe})};if(z){const ce=T===p,me=ce?oe:y[p+"Score"]||0,xe=ce?y[g+"Score"]||0:oe;N.add(P),ot(X,me,xe,ce,ue)}else await ue()}function Le(k){return["MIL","ATT"].some(L=>(k[L]||[]).some(I=>!I.used))}function ge(k){return["GK","DEF","MIL","ATT"].some(L=>(k[L]||[]).some(I=>!I.used))}function Be(k){return ge(k)&&!Le(k)}function je(k){const L=y[k+"Team"];return!!(Le(L)||Be(L))}function Oe(k){return!ge(k.p1Team)&&!ge(k.p2Team)}function bt(k){const L=k.p1Score||0,I=k.p2Score||0;return L===I?null:L>I?b.home_id:b.away_id}async function Xe(k){try{const L=bt(k),I=L?b.home_id===L?b.away_id:b.home_id:null,A=k.p1Score||0,T=k.p2Score||0;await w.from("matches").update({status:"finished",winner_id:L,home_score:A,away_score:T}).eq("id",i),L&&I&&to(L,I).catch(()=>{}),u&&await w.from("mini_league_matches").update({status:"finished",home_score:A,away_score:T,match_id:i}).eq("id",u);try{const{data:z}=await w.rpc("apply_match_rewards",{p_match_id:i});z!=null&&z.success&&!(z!=null&&z.skipped)&&typeof t.refreshProfile=="function"&&await t.refreshProfile()}catch(z){console.error("[PvP] apply_match_rewards:",z)}if(typeof c=="function")try{await c({homeScore:A,awayScore:T,winnerId:L,homeId:b.home_id,awayId:b.away_id,matchId:i})}catch(z){console.error("[PvP] onMatchEnd:",z)}}catch(L){console.error("[PvP] finishMatch:",L)}}function Rt(){var T;const k=y[p+"Score"],L=y[g+"Score"],I=k>L,A=k===L;nt(e),e.innerHTML=`
    ...
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:${Nt};border:1px solid ${kt};border-radius:12px;padding:16px">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>${x.map((v,S)=>{const G=f>0&&a.status==="finished"?S===0?Math.floor(f*.7):S===1?Math.floor(f*.2):S===2?Math.floor(f*.1):0:0;return`<tr style="border-bottom:1px solid ${$t};${v.userId===o?"background:rgba(74,222,128,0.08);":""}">
      ...
      <div style="background:${Nt};border:1px solid ${kt};border-radius:12px;padding:16px">
        ...
        ${Array.from({length:Math.max(0,a.status==="active"?a.current_day-1:a.current_day)},(v,S)=>S+1).reverse().map(v=>{const S=(l||[]).filter(G=>G.matchday===v);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:${Ke};margin-bottom:6px">Journée ${v}</div>${S.map(G=>en(G,u,o,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center">
      ...
      `).join(""):'<div style="text-align:center;color:var(--gray-600);padding:40px">Aucun manager.</div>'}else{const h=a||[];f.innerHTML=h.length>0?h.map((_,b)=>{const $=Si(_.mmr??1e3),y=(_.ranked_wins||0)+(_.ranked_losses||0)+(_.ranked_draws||0),j=y>0?Math.round((_.ranked_wins||0)/y*100):0,E=_.id===i.profile.id,F=(_.placement_matches||0)<10;return`
