...
import{s as _,l as Rt,m as Si,F as Ai,r as Ae,j as Qt,h as Mi,n as Mn,o as jn,T as mi,k as Cn,p as Bn,b as qn}from"./formation-links-yJiMLlPw.js";const Dn="/";function Oi(e,{navigate:t,toast:i}){let n="login";const r=()=>{var a,s,l,c,d,f;const o=n==="login";e.innerHTML=`
    <div style="
    ...
    </style>`,(a=document.getElementById("tab-login-btn"))==null||a.addEventListener("click",()=>{n="login",r()}),(s=document.getElementById("tab-reg-btn"))==null||s.addEventListener("click",()=>{n="register",r()}),o?((l=document.getElementById("login-password"))==null||l.addEventListener("keydown",y=>{var m;y.key==="Enter"&&((m=document.getElementById("login-btn"))==null||m.click())}),(c=document.getElementById("login-btn"))==null||c.addEventListener("click",async()=>{const y=document.getElementById("login-email").value.trim(),m=document.getElementById("login-password").value,g=document.getElementById("login-error");if(g.textContent="",!y||!m){g.textContent="Remplissez tous les champs.";return}const p=document.getElementById("login-btn");p.textContent="⏳ Connexion…",p.disabled=!0;const{error:u}=await _.auth.signInWithPassword({email:y,password:m});if(p.textContent="⚽ Se connecter",p.disabled=!1,u){g.textContent=u.message.includes("Invalid")?"Email ou mot de passe incorrect.":u.message;return}window.location.reload()})):((d=document.getElementById("reg-confirm"))==null||d.addEventListener("keydown",y=>{var m;y.key==="Enter"&&((m=document.getElementById("reg-btn"))==null||m.click())}),(f=document.getElementById("reg-btn"))==null||f.addEventListener("click",async()=>{const y=document.getElementById("reg-email").value.trim(),m=document.getElementById("reg-password").value,g=document.getElementById("reg-confirm").value,p=document.getElementById("reg-error");if(p.textContent="",!y||!m||!g){p.textContent="Remplissez tous les champs.";return}if(m.length<6){p.textContent="Mot de passe trop court (min. 6 caractères).";return}if(m!==g){p.textContent="Les mots de passe ne correspondent pas.";return}const u=document.getElementById("reg-btn");u.textContent="⏳ Création…",u.disabled=!0;const{error:b}=await _.auth.signUp({email:y,password:m});if(u.textContent="🚀 Créer mon compte",u.disabled=!1,b){p.textContent=b.message;return}i("Compte créé ! Connecte-toi pour commencer.","success",4e3),n="login",r(),setTimeout(()=>{const v=document.getElementById("login-email");v&&(v.value=y)},50)}))};r()}function Fn(e,{state:t,navigate:i,toast:n,refreshProfile:r}){let o="#1A6B3C",a="#D4A017";e.innerHTML=`
  ...
  `;function s(){var g;const c=document.getElementById("logo-preview"),d=document.getElementById("logo-initials"),f=((g=document.getElementById("setup-club"))==null?void 0:g.value)||"MW",y=f.trim().split(" ").filter(Boolean),m=y.length>=2?(y[0][0]+y[1][0]).toUpperCase():f.slice(0,2).toUpperCase();c&&(c.style.background=a,c.style.borderColor=o),d&&(d.textContent=m,d.style.color=o)}document.getElementById("color1").addEventListener("input",c=>{o=c.target.value,document.getElementById("swatch1").style.background=o,s()}),document.getElementById("color2").addEventListener("input",c=>{a=c.target.value,document.getElementById("swatch2").style.background=a,s()});function l(c){document.querySelectorAll(".setup-step").forEach(d=>d.classList.remove("active")),document.getElementById(`step-${c}`).classList.add("active"),document.getElementById("step-num").textContent=c,document.getElementById("progress-fill").style.width=`${Math.round(c/3*100)}%`,c===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),d=document.getElementById("step1-error");if(d.textContent="",c.length<3){d.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:f}=await _.from("users").select("id").eq("pseudo",c).maybeSingle();if(f){d.textContent="Ce pseudo est déjà pris.";return}l(2)}),document.getElementById("step2-back").addEventListener("click",()=>l(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const c=document.getElementById("setup-club").value.trim(),d=document.getElementById("step2-error");if(d.textContent="",c.length<2){d.textContent="Nom trop court (min. 2 caractères).";return}const{data:f}=await _.from("users").select("id").eq("club_name",c).maybeSingle();if(f){d.textContent="Ce nom de club est déjà pris.";return}l(3)}),document.getElementById("step3-back").addEventListener("click",()=>l(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),d=document.getElementById("setup-club").value.trim(),f=document.getElementById("step3-error"),y=document.getElementById("step3-finish");f.textContent="",y.disabled=!0,y.textContent="Création en cours…";try{const{error:m}=await _.from("users").insert({id:t.user.id,pseudo:c,club_name:d,club_color1:o,club_color2:a,credits:1e4});if(m)throw m;await Pn(t.user.id),await r(),n(`Bienvenue ${c} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(m){f.textContent=m.message,y.disabled=!1,y.textContent="🚀 Créer mon profil !"}})}async function Pn(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await _.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const Gn="modulepreload",Rn=function(e){return"/"+e},Hi={},an=function(t,i,n){let r=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),s=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));r=Promise.allSettled(i.map(l=>{if(l=Rn(l),l in Hi)return;Hi[l]=!0;const c=l.endsWith(".css"),d=c?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${l}"]${d}`))return;const f=document.createElement("link");if(f.rel=c?"stylesheet":Gn,c||(f.as="script"),f.crossOrigin="",f.href=l,s&&f.setAttribute("nonce",s),document.head.appendChild(f),c)return new Promise((y,m)=>{f.addEventListener("load",y),f.addEventListener("error",()=>m(new Error(`Unable to preload CSS for ${l}`)))})}))}function o(a){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=a,window.dispatchEvent(s),!s.defaultPrevented)throw a}return r.then(a=>{for(const s of a||[])s.status==="rejected"&&o(s.reason);return t().catch(o)})},Kt="#1A6B3C",Vt="#cc2222",Nn="#D4A017",Ui="#888";async function On(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      ${y.map(({friendshipId:m,friend:g})=>Hn(g,m)).join("")}
    </div>`,c.querySelectorAll("[data-stats]").forEach(m=>{m.addEventListener("click",()=>Kn(e,m.dataset.stats,m.dataset.friendName))}),c.querySelectorAll("[data-match]").forEach(m=>{m.addEventListener("click",async()=>{const g=m.dataset.friendId,p=m.dataset.friendName;if(typeof n!="function"){t("Erreur navigation","error");return}const u=e.user.id,{data:b}=await _.from("friend_match_invites").select("id").eq("inviter_id",g).eq("invitee_id",u).eq("status","pending").order("created_at",{ascending:!1}).limit(1).maybeSingle(),v=!!b;console.log("[Friends] clic match",{fid:g,fname:p,isAccepting:v}),n("match",{matchMode:"friend",friendId:g,friendName:p,isAccepting:v})})})}function Hn(e,t){const i=e.club_name||e.pseudo||"?",n=e.pseudo||"",r=(n||i).slice(0,2).toUpperCase(),o=e.club_color2||Kt,a=e.club_color1||"#ffffff",s=e.last_seen_at?new Date(e.last_seen_at):null,l=s&&Date.now()-s.getTime()<3*60*1e3;return`
    ...
    <div class="popup-box">
      ...
      ${d.length?`<div style="display:flex;flex-direction:column;gap:8px;max-height:50vh;overflow-y:auto;margin-bottom:14px">
            ${d.map(y=>{var m,g,p;return`
              <div style="display:flex;align-items:center;gap:10px;background:#f9f9f9;border-radius:10px;padding:10px 12px">
                <div style="flex:1;font-size:14px;font-weight:700">${((m=y.requester)==null?void 0:m.club_name)||((g=y.requester)==null?void 0:g.pseudo)||"?"}
    ...
    ${Ci()}`,document.body.appendChild(c);const f=()=>c.remove();c.querySelector("#pending-close").addEventListener("click",f),c.addEventListener("click",y=>{y.target===c&&f()}),c.querySelectorAll("[data-accept]").forEach(y=>{y.addEventListener("click",async()=>{const{error:m}=await _.from("friendships").update({status:"accepted"}).eq("id",y.dataset.accept);if(m){t("Erreur : "+m.message,"error");return}y.closest("div[style]").remove(),t("✅ Ami accepté !","success"),sn(e,t,n),i&&i()})}),c.querySelectorAll("[data-decline]").forEach(y=>{y.addEventListener("click",async()=>{await _.from("friendships").delete().eq("id",y.dataset.decline),y.closest("div[style]").remove(),t("Demande refusée","info"),i&&i()})})}async function Kn(e,t,i){const n=e.user.id,[r,o]=[n,t].sort(),a=n===r,{data:s}=await _.from("friend_match_stats").select("*").eq("player1_id",r).eq("player2_id",o).single(),l=e.profile.club_name||e.profile.pseudo||"Moi",c=s||{},d=a?c.wins_p1||0:c.wins_p2||0,f=a?c.wins_p2||0:c.wins_p1||0,y=c.draws||0,m=a?c.goals_p1||0:c.goals_p2||0,g=a?c.goals_p2||0:c.goals_p1||0,p=a?c.gc_used_p1||0:c.gc_used_p2||0,u=a?c.gc_used_p2||0:c.gc_used_p1||0,b=c.matches_total||0,v=(k,x,C,E=Kt,G=Vt)=>`
    <div style="display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:8px;padding:10px 0;border-bottom:1px solid #f0f0f0">
      <div style="text-align:right;font-size:18px;font-weight:900;color:${E}">${x}</div>
    ...
    </style>`}const Vn="2026.07.19-1101";async function Ki(e,{state:t,navigate:i,toast:n}){var o,a;const r=t.profile;r&&(e.innerHTML=`
  <style>
    .home-dark {
      min-height: 100%;
  ...
  <div class="home-dark" id="home-dark">
    ...
    ${Ae(e,{width:140})}
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),c=(s||[]).filter(S=>S.card_type==="player"&&S.player),d=(s||[]).filter(S=>S.card_type==="game_changer"),f=(s||[]).filter(S=>S.card_type==="formation"),y=(s||[]).filter(S=>S.card_type==="stadium"),{data:m}=await _.from("gc_definitions").select("name,gc_type,color,effect,image_url"),g={};(m||[]).forEach(S=>{g[S.name]=S});const{data:p}=await _.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),u={};(p||[]).forEach(S=>{u[S.id]=S}),y.forEach(S=>{S.stadium_def&&(u[S.stadium_id]=S.stadium_def)});const b=Object.keys(Ai),v=Object.keys(je),h={};c.forEach(S=>{const q=S.player.id;h[q]=(h[q]||0)+1}),new Set(Object.keys(h).map(S=>String(S)));const k=new Set(f.map(S=>S.formation)),x=new Set(d.map(S=>S.gc_type));let C="player",E="Tous",G="",N=!1;function ne(){return[...c].sort((S,q)=>{const P=ei.indexOf(S.player.job),X=ei.indexOf(q.player.job);return P!==X?P-X:(S.player.surname_real||"").localeCompare(q.player.surname_real||"")})}function ie(){return[...l||[]].sort((S,q)=>{const P=ei.indexOf(S.job),X=ei.indexOf(q.job);return P!==X?P-X:(S.surname_real||"").localeCompare(q.surname_real||"")})}function Q(){return ne().filter(S=>{const q=S.player,P=E==="Tous"||q.job===E,X=!G||`${q.firstname} ${q.surname_real}`.toLowerCase().includes(G);return P&&X})}function M(){return ie().filter(S=>{const q=E==="Tous"||S.job===E,P=!G||`${S.firstname} ${S.surname_real}`.toLowerCase().includes(G);return q&&P})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
        ...
        </div>`},null)}e.querySelectorAll(".col-tab-btn").forEach(S=>{S.addEventListener("click",()=>{C=S.dataset.tab,E="Tous",G="",N=!1,e.querySelectorAll(".col-tab-btn").forEach(q=>{const P=q.dataset.tab===C;q.style.borderBottomColor=P?"var(--green)":"transparent",q.style.color=P?"var(--green)":"var(--gray-600)"}),w(),j()})}),w(),j()}function po(e,t,i){const n=je[e]||{icon:"⚡",desc:"Effet spécial."},r=(t==null?void 0:t.gc_type)==="ultra_game_changer",o={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},a={purple:"#b06ce0",light_blue:"#00d4ef"},s=o[t==null?void 0:t.color]||o.purple,l=a[t==null?void 0:t.color]||a.purple,c=(t==null?void 0:t.name)||e,d=(t==null?void 0:t.effect)||n.desc,f=t!=null&&t.image_url?`/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const ti=1e3;function uo(e,t,i,n){var g,p,u;const{state:r,toast:o,closeModal:a,navigate:s,refreshProfile:l}=i,c=e.formation,d={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function f(){const b=Qt[c]||{},v=Ai[c]||[],h=290,k=360,x=20;function C(G){const N=b[G];return N?{x:N.x*h,y:N.y*k}:null}let E=`<svg width="${h}" height="${k}" viewBox="0 0 ${h} ${k}" xmlns="http://www.w3.org/2000/svg">`;for(const[G,N]of v){const ne=C(G),ie=C(N);!ne||!ie||(E+=`<line x1="${ne.x}" y1="${ne.y}" x2="${ie.x}" y2="${ie.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const G of Object.keys(b)){const N=C(G);if(!N)continue;const ne=G.replace(/\d+/,""),ie=d[ne]||"#555";E+=`<circle cx="${N.x}" cy="${N.y}" r="${x}" fill="${ie}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,E+=`<text x="${N.x}" y="${N.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${ne}</text>`}return E+="</svg>",E}const y=t.filter(b=>b.formation===c);y.length;const m=!e.is_for_sale;n(`Formation ${c}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(ae=document.getElementById("close-detail"))==null||ae.addEventListener("click",s);let j=new Set;const U=()=>{const W=j.size,J=document.getElementById("sell-action-panel");if(!J)return;J.style.display=W>0?"block":"none",document.getElementById("sell-selected-count").textContent=W;const R=document.getElementById("evolve-btn");R&&(R.textContent=`⬆️ Évoluer ${W>1?"(+"+W+")":""}`)};document.querySelectorAll(".expl-check").forEach(W=>{W.addEventListener("change",()=>{const J=W.dataset.id;W.checked?j.add(J):j.delete(J);const R=W.closest(".exemplaire-row");R&&(R.style.borderColor=W.checked?"#1A6B3C":"#eee"),U()})}),document.querySelectorAll(".exemplaire-row").forEach(W=>{W.addEventListener("click",J=>{if(J.target.closest("button")||J.target.tagName==="INPUT")return;const R=W.querySelector(".expl-check");R&&!R.disabled&&(R.checked=!R.checked,R.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(W=>{W.addEventListener("click",J=>{J.stopPropagation();const R=document.querySelector(`.expl-hist[data-hist="${W.dataset.idx}"]`);R&&(R.style.display=R.style.display==="none"?"flex":"none")})}),(se=document.getElementById("evolve-btn"))==null||se.addEventListener("click",async()=>{if(y<=1)return;const W=[...j];if(!W.length)return;if(j.has(e.id)){const V=document.createElement("div");V.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",V.innerHTML=`
  ...
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
    ...
    <div class="page-body">
      ...
      stadium_def:stadium_definitions(id, name, club_id, country_code, image_url,
        club:clubs(encoded_name, logo_url))`).eq("owner_id",n.profile.id).eq("card_type","stadium"),l=(a||[]).filter(b=>b.card_type==="player"&&b.player),c=(a||[]).filter(b=>b.card_type==="formation"),d=(s||[]).filter(b=>b.card_type==="stadium"),f=[...new Set(d.map(b=>b.stadium_id).filter(Boolean))];let y={};if(d.forEach(b=>{b.stadium_def&&b.stadium_id&&(y[b.stadium_id]=b.stadium_def)}),f.length&&Object.keys(y).length<f.length){const{data:b}=await _.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",f);(b||[]).forEach(v=>{y[v.id]=v})}const m=c.map(b=>b.formation).filter(Boolean),{data:g}=await _.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",e);let p=o.formation||"4-4-2";m.length>0&&!m.includes(p)&&(p=m[0]);const u={deckId:e,name:o.name,formation:p,formationCardId:o.formation_card_id,stadiumCardId:o.stadium_card_id||null,slots:{},subs:[],playerCards:l,formationCards:c,stadiumCards:d,stadDefMap:y,availableFormations:m};(g||[]).forEach(b=>{b.is_starter?u.slots[b.position]=b.card_id:u.subs.includes(b.card_id)||u.subs.push(b.card_id)}),yt(t,u,i)}function yt(e,t,i){var g,p;const{navigate:n}=i;oi[t.formation];const r=en(t.formation),o=r.filter(u=>t.slots[u]).length,a=t.availableFormations.length>0?t.availableFormations:Object.keys(oi),s=(g=t.stadiumCards)==null?void 0:g.find(u=>u.id===t.stadiumCardId),l=s&&((p=t.stadDefMap)==null?void 0:p[s.stadium_id])||null,c=t.subs.map(u=>t.playerCards.find(b=>b.id===u)).filter(Boolean);[...Object.values(t.slots),...t.subs],e.innerHTML=`
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
    ...
    <div class="page-body" style="padding-bottom:20px">
  ...
  </div>`;const d=window.innerWidth>=900,f=e.querySelector(".deck-pc-layout"),y=e.querySelector(".deck-mobile-layout");f&&(f.style.display=d?"block":"none"),y&&(y.style.display=d?"none":"block"),go(e,t,r,i),e.querySelectorAll("#builder-back").forEach(u=>u.addEventListener("click",()=>n("decks")));const m=()=>{const{openModal:u,closeModal:b}=i,v=`<div style="display:flex;flex-wrap:wrap;gap:8px;padding:8px">
    ...
    </div>`;u("⚽ Choisir une formation",v),document.querySelectorAll("#modal-body [data-forma]").forEach(h=>{h.addEventListener("click",()=>{t.formation=h.dataset.forma;const k=en(t.formation),x={};k.forEach(C=>{t.slots[C]&&(x[C]=t.slots[C])}),t.slots=x,b(),yt(e,t,i)})})};e.querySelectorAll("#formation-mobile-btn, #formation-pc-btn").forEach(u=>u.addEventListener("click",m)),e.querySelectorAll("#add-stad-btn-pc, #add-stad-btn").forEach(u=>u.addEventListener("click",()=>mo(t,e,i))),e.querySelectorAll("#save-deck").forEach(u=>u.addEventListener("click",()=>bo(t,i))),e.querySelectorAll("[data-remove-sub]").forEach(u=>{u.addEventListener("click",()=>{t.subs=t.subs.filter(b=>b!==u.dataset.removeSub),yt(e,t,i)})}),e.querySelectorAll("#add-sub-btn").forEach(u=>u.addEventListener("click",()=>yo(t,e,i)))}function go(e,t,i,n){var h,k;const r=window.innerWidth>=900,o=e.querySelector(r?"#deck-field-pc":"#deck-field-mobile");if(!o)return;const a=(h=t.stadiumCards)==null?void 0:h.find(x=>x.id===t.stadiumCardId),s=a&&((k=t.stadDefMap)==null?void 0:k[a.stadium_id])||null,l=Qt[t.formation]||{},c=Si(t.formation)||[],d={};for(const x of i){const C=t.slots[x],E=C?t.playerCards.find(G=>G.id===C):null;E!=null&&E.player?d[x]={...E.player,_evolution_bonus:E.evolution_bonus||0,face:E.player.face||null}:d[x]=null}const f=window.innerWidth>=900,y=f?window.innerWidth-280:window.innerWidth-20,m=f?Math.min(y,860):y,g=Math.round(f?m*.82:m*.85),p=f?84:44;let u="";for(const[x,C]of c){const E=l[x],G=l[C];if(!E||!G)continue;const N=E.x*m,ne=Math.round(.03*g+E.y*.85*g),ie=G.x*m,Q=Math.round(.03*g+G.y*.85*g),M=d[x],w=d[C],j=Rt(M,w);j==="#ff3333"||j==="#cc2222"?u+=`<line x1="${N.toFixed(1)}" y1="${ne.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${Q.toFixed(1)}" stroke="${j}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(u+=`<line x1="${N.toFixed(1)}" y1="${ne.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${Q.toFixed(1)}" stroke="${j}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,u+=`<line x1="${N.toFixed(1)}" y1="${ne.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${Q.toFixed(1)}" stroke="${j}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let b="";const v=Math.round(p*657/507);for(const x of i){const C=l[x];if(!C)continue;const E=d[x],G=C.x*m;C.y*g;const N=Math.round(.03*g+C.y*(.85*g)),ne=Math.round(G-p/2),ie=Math.round(N-v/2);if(E){const Q=x.replace(/\d+/,""),M=s&&(s.club_id&&String(E.club_id)===String(s.club_id)||s.country_code&&E.country_code===s.country_code),w=Ae({...E,_evolution_bonus:E._evolution_bonus||0},{width:p,showStad:!0,stadDef:s,role:Q});b+=`<div style="position:absolute;left:${ne}px;top:${ie}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${x}">
      ...
      </div>`}else{const Q=x.replace(/\d+/,"");b+=`<div style="position:absolute;left:${ne}px;top:${ie}px;width:${p}px;height:${v}px;
    ...
    <div style="position:relative;width:${m}px;height:${g}px;margin:0 auto">
      ...
      ${b}
    </div>`,o.querySelectorAll(".deck-slot-hit").forEach(x=>{x.addEventListener("click",()=>xo(x.dataset.pos,t,e,n))})}function mo(e,t,i){var a;const{openModal:n,closeModal:r}=i,o=e.stadiumCards||[];n("🏟️ Choisir un stade",`<div style="display:flex;flex-wrap:wrap;gap:10px;padding:8px">
      ...
      ${o.map(s=>{var b,v;const l=e.stadDefMap[s.stadium_id],c=((b=l==null?void 0:l.club)==null?void 0:b.logo_url)||null,d=l!=null&&l.image_url?"/icons/"+l.image_url:null,f=e.stadiumCardId===s.id,y=l!=null&&l.country_code&&!c?`https://flagsapi.com/${l.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,m=(l==null?void 0:l.name)||"Stade",g=m.match(/^(Stade\s+(?:de\s+)?(?:\w+)?)(.*)?$/i),p=g?g[1].trim():m.slice(0,8),u=((v=g==null?void 0:g[2])==null?void 0:v.trim())||"";return`<div class="stad-choice" data-stad-id="${s.id}" style="cursor:pointer;width:80px;text-align:center">
    ...
    </div>`),(a=document.getElementById("stad-none"))==null||a.addEventListener("click",()=>{e.stadiumCardId=null,r(),yt(t,e,i)}),document.querySelectorAll(".stad-choice").forEach(s=>{s.addEventListener("click",()=>{e.stadiumCardId=s.dataset.stadId,r(),yt(t,e,i)})})}function xo(e,t,i,n){var g,p,u,b,v;const{openModal:r,closeModal:o}=n,a=e.replace(/\d+/,""),s=(g=t.stadiumCards)==null?void 0:g.find(h=>h.id===t.stadiumCardId),l=s&&((p=t.stadDefMap)==null?void 0:p[s.stadium_id])||null,c=t.slots[e],d=c?t.playerCards.find(h=>h.id===c):null;(u=d==null?void 0:d.player)==null||u.id;const f=new Set;Object.entries(t.slots).forEach(([h,k])=>{var C;if(h===e||!k)return;const x=t.playerCards.find(E=>E.id===k);(C=x==null?void 0:x.player)!=null&&C.id&&f.add(x.player.id)}),t.subs.forEach(h=>{var x;const k=t.playerCards.find(C=>C.id===h);(x=k==null?void 0:k.player)!=null&&x.id&&f.add(k.player.id)});const y=new Set,m=t.playerCards.filter(h=>{const k=h.player;return!(k.job===a||k.job2===a)||f.has(k.id)||y.has(k.id)?!1:(y.add(k.id),!0)});m.sort((h,k)=>{const x=h.evolution_bonus||0,C=k.evolution_bonus||0,E=(a==="GK"?h.player.note_g:a==="DEF"?h.player.note_d:a==="MIL"?h.player.note_m:h.player.note_a)+(a===h.player.job||a===h.player.job2?x:0);return(a==="GK"?k.player.note_g:a==="DEF"?k.player.note_d:a==="MIL"?k.player.note_m:k.player.note_a)+(a===k.player.job||a===k.player.job2?C:0)-E}),r(`Choisir ${a} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
    ...
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(b=document.getElementById("close-selector"))==null||b.addEventListener("click",o),(v=document.getElementById("remove-player"))==null||v.addEventListener("click",()=>{delete t.slots[e],o(),yt(i,t,n)}),document.querySelectorAll(".player-option").forEach(h=>{h.addEventListener("click",()=>{t.slots[e]=h.dataset.cardId,o(),yt(i,t,n)})})}function yo(e,t,i){var d,f,y;const{openModal:n,closeModal:r}=i,o=(d=e.stadiumCards)==null?void 0:d.find(m=>m.id===e.stadiumCardId),a=o&&((f=e.stadDefMap)==null?void 0:f[o.stadium_id])||null,s=new Set;Object.keys(e.slots).forEach(m=>{var u;const g=e.slots[m];if(!g)return;const p=e.playerCards.find(b=>b.id===g);(u=p==null?void 0:p.player)!=null&&u.id&&s.add(p.player.id)}),e.subs.forEach(m=>{var p;const g=e.playerCards.find(u=>u.id===m);(p=g==null?void 0:g.player)!=null&&p.id&&s.add(g.player.id)});const l=new Set,c=e.playerCards.filter(m=>{var g,p,u;return s.has((g=m.player)==null?void 0:g.id)||l.has((p=m.player)==null?void 0:p.id)?!1:(l.add((u=m.player)==null?void 0:u.id),!0)});n("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
  ...
  <div class="page">
    ...
    <div class="page-body">
      <div class="booster-grid">
        ${o.map(g=>{const p=g.cost===0||r>=g.cost,u=d[g.id]===!0;return`<div class="booster-card ${!p||u?"disabled":""}" data-booster="${g.id}" style="position:relative">
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}function So(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText=`
    ...
    `,document.body.appendChild(t);let i=5;const n=setInterval(()=>{i--;const r=document.getElementById("mw-ad-cd");r&&(r.textContent=i),i<=0&&(clearInterval(n),t.remove(),e(!0))},1e3)})}async function Ao(e,{state:t,navigate:i,toast:n,refreshProfile:r}){var y,m;const{data:o}=await _.from("users").select("*").eq("id",t.user.id).single();o&&(t.profile=o);let a=Array.isArray((y=t.profile)==null?void 0:y.pending_boosters)?[...t.profile.pending_boosters]:[];if(!a.length){await _.from("users").update({onboarding_done:!0}).eq("id",t.user.id),i("home");return}let s=null;try{const p=(await mn()).find(u=>(u.name||"").toLowerCase().includes("new player"));p&&(s=yn(p))}catch(g){console.warn('[Onboarding] Config "Booster (new player)" introuvable, fallback taux par défaut',g)}const l=a.length;let c=0;e.innerHTML=`
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    </div>`}const l=d=>{e.style.overflow="",e.style.height="",e.style.display="",e.style.flexDirection="",i(d)};function c(){var f,y,m;e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column";const d=a.length>0;e.innerHTML=`
    ...
    </div>`,e.querySelectorAll(".gc-select-card").forEach(g=>{g.addEventListener("click",()=>{const p=g.dataset.id,u=o.find(v=>v.id===p);if(!u)return;const b=a.findIndex(v=>v.gc_type===u.gc_type);b>-1?a.splice(b,1):a.length<3&&a.push(u),c()})}),(f=e.querySelector("#gc-launch"))==null||f.addEventListener("click",()=>{d&&l(a)}),(y=e.querySelector("#gc-no-gc"))==null||y.addEventListener("click",()=>l([])),(m=e.querySelector("#gc-reset"))==null||m.addEventListener("click",()=>{a.length&&(a=[],c())})}c()}function Co(e,t){var r;const i=((r=t==null?void 0:t.state)==null?void 0:r.params)||{},n=e||i.matchMode||"vs_ai_easy";return n==="friend"?`Match vs ${i.friendName||"Ami"}`:n==="random"?"Match vs Random":n==="ranked"?"Match Classé":n==="mini_league"||n==="mini-league"?"🏆 Match de Mini League":`Match vs IA — ${n.replace("vs_ai_","").toUpperCase()}`}async function Bo(e,t,i){const{state:n,navigate:r}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await _.from("decks").select(`id,name,is_active,formation,stadium_card_id,
      stadium_card:cards!stadium_card_id(id,stadium_id,
        stadium_def:stadium_definitions(id,name,club_id,country_code,image_url,
          club:clubs(encoded_name,logo_url)))`).eq("owner_id",n.profile.id).order("created_at",{ascending:!1});if(!o||o.length===0){Lt(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>r("decks"));return}const a=o.map(y=>y.id),{data:s}=await _.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,stadium_id,evolution_bonus,
        player:players(id,firstname,surname_real,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order"),l=[...new Set((s||[]).filter(y=>{var m,g;return((m=y.card)==null?void 0:m.card_type)==="stadium"&&((g=y.card)==null?void 0:g.stadium_id)}).map(y=>y.card.stadium_id))],c={};if(l.length){const{data:y}=await _.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",l);(y||[]).forEach(m=>{c[m.id]=m}),(s||[]).forEach(m=>{var g,p;((g=m.card)==null?void 0:g.card_type)==="stadium"&&((p=m.card)!=null&&p.stadium_id)&&(m.card._stadiumDef=c[m.card.stadium_id]||null)})}let d=0;function f(){var x,C,E,G,N,ne,ie;const y=o[d],m=(s||[]).filter(Q=>Q.deck_id===y.id),g=m.filter(Q=>{var M;return Q.is_starter&&((M=Q.card)==null?void 0:M.player)}).map(Q=>$i(Q.card,Q.position)),p=m.find(Q=>{var M;return((M=Q.card)==null?void 0:M.card_type)==="formation"}),u=y.formation||((x=p==null?void 0:p.card)==null?void 0:x.formation)||"4-4-2";let b=g.length>=11?ai(g,u):null,v=((C=y.stadium_card)==null?void 0:C.stadium_def)||null;v&&b&&(b=Jt(b,v));const h=g.length>=11;ot(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    ...
    </div>`}async function Fo(e,t){const{state:i}=t,r=(i.params||{}).matchMode||"vs_ai_easy",o=r.replace("vs_ai_",""),a=r;await ui(e,t,r,async({deckId:s,formation:l,starters:c,subsRaw:d,gcCardsEnriched:f,gcDefs:y,stadiumDef:m})=>{try{let g=ai(c,l);m&&(g=Jt(g,m),ri(d,m));const p=await Po(l,o),u=p.lines||p,b=async v=>{try{const h=a==="vs_ai_club"?"club":a,{data:k,error:x}=await _.from("matches").insert({home_id:i.profile.id,away_id:null,mode:h,home_deck_id:s,status:"in_progress"}).select().single();if(x){console.error("[MatchIA] Erreur création match:",x),Lt(e,"⚠️","Impossible de créer le match ("+x.message+").","Retour",()=>t.navigate("home"));return}const C=p.stadiumDef||null;C&&u&&(Jt(u,C),ri(p.subs||[],C));const E={gcDefs:y||[],matchId:k==null?void 0:k.id,mode:a,difficulty:o,formation:l,homeTeam:g,aiTeam:u,homeSubs:d,subsUsed:0,maxSubs:Math.min(d.length,3),aiSubs:p.subs||[],aiSubsUsed:0,aiMaxSubs:Math.min((p.subs||[]).length,3),aiUsedSubIds:[],aiGcCards:p.gcCards||[],aiUsedGc:[],aiStadiumDef:C,homeScore:0,aiScore:0,gcCards:v,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Ro(e,E,t)}catch(h){console.error("[MatchIA] Exception launchMatch:",h),Lt(e,"⚠️","Erreur au lancement du match : "+h.message,"Retour",()=>t.navigate("home"))}};if(!f.length){b([]);return}ci(e,f,b)}catch(g){console.error("[MatchIA] Exception setup:",g),Lt(e,"⚠️","Erreur de préparation du match : "+g.message,"Retour",()=>t.navigate("home"))}})}async function Po(e,t){var u;const{data:i}=await _.from("players").select("id,firstname,surname_real,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(80);if(!i||i.length<11)return{lines:Go(e),subs:[],gcCards:[],stadiumDef:null};const n=Pt[e]||Pt["4-4-2"],r={GK:[],DEF:[],MIL:[],ATT:[]},o=new Set;function a(b,v,h){var k,x;return o.add(b.id),{cardId:"ai-"+b.id+"-"+h,id:b.id,firstname:b.firstname,name:b.surname_real,country_code:b.country_code,club_id:b.club_id,job:b.job,job2:b.job2,note_g:Number(b.note_g)||0,note_d:Number(b.note_d)||0,note_m:Number(b.note_m)||0,note_a:Number(b.note_a)||0,rarity:b.rarity,skin:b.skin,hair:b.hair,hair_length:b.hair_length,clubName:((k=b.clubs)==null?void 0:k.encoded_name)||null,clubLogo:((x=b.clubs)==null?void 0:x.logo_url)||null,boost:0,used:!1,_line:v}}for(const b of["GK","DEF","MIL","ATT"]){const v=i.filter(E=>E.job===b&&!o.has(E.id)),h=i.filter(E=>E.job!==b&&!o.has(E.id)),k=[...v,...h],x=[];for(let E=0;E<n[b];E++){const G=k[E];G&&x.push(a(G,b,E))}const C=ut(x.length);x.forEach((E,G)=>{E._col=C[G]}),r[b]=x}const l=i.filter(b=>!o.has(b.id)).slice(0,5).map((b,v)=>a(b,b.job,100+v)),f=Object.keys(je).sort(()=>Math.random()-.5).slice(0,3).map((b,v)=>{var h,k;return{id:"ai-gc-"+v,gc_type:b,name:((h=je[b])==null?void 0:h.name)||b,icon:((k=je[b])==null?void 0:k.icon)||"⚡"}}),y=Object.values(r).flat(),m={};y.forEach(b=>{b.club_id&&(m[b.club_id]=(m[b.club_id]||0)+1)});const g=(u=Object.entries(m).sort((b,v)=>v[1]-b[1])[0])==null?void 0:u[0];let p=null;if(g){const{data:b}=await _.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",g).single();b&&(p={club_id:b.id,country_code:null,name:b.encoded_name+" Stadium",club:{encoded_name:b.encoded_name,logo_url:b.logo_url}})}return{lines:r,subs:l,gcCards:f,stadiumDef:p}}function Go(e){const t=Pt[e]||Pt["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},n=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let r=0;for(const o of["GK","DEF","MIL","ATT"]){const a=[];for(let l=0;l<t[o];l++){const c=3+Math.floor(Math.random()*5);a.push({cardId:"fake-"+r,id:"fake-"+r,firstname:"IA",name:n[r%n.length],country_code:"XX",club_id:null,job:o,job2:null,note_g:o==="GK"?c:2,note_d:o==="DEF"?c:2,note_m:o==="MIL"?c:2,note_a:o==="ATT"?c:2,rarity:"normal",boost:0,used:!1,_line:o}),r++}const s=ut(a.length);a.forEach((l,c)=>{l._col=s[c]}),i[o]=a}return i}function Ro(e,t,i){var r;const n=`<div style="position:relative;width:22px;height:22px;flex-shrink:0;display:flex;align-items:center;justify-content:center">
  ...
  </div>`;const d=()=>{const g=(p,u)=>e.querySelectorAll(p).forEach((b,v)=>{setTimeout(()=>{b.style.opacity="1",b.style.transform="translateY(0) scale(1)"},u+v*90)});g(".duel-card-home",150),g(".duel-card-ai",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((p,u)=>{setTimeout(()=>{p.style.opacity="1"},u*70)}),900),setTimeout(()=>{const p=e.querySelector("#vs-label");p&&(p.style.opacity="1",p.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(u=>u.style.opacity="1")},1250),setTimeout(()=>{f("score-home",s,800),f("score-ai",l,800)},1500)};function f(g,p,u){const b=document.getElementById(g);if(!b)return;const v=performance.now(),h=k=>{const x=Math.min(1,(k-v)/u);b.textContent=Math.round(p*(1-Math.pow(1-x,3))),x<1?requestAnimationFrame(h):b.textContent=p};requestAnimationFrame(h)}requestAnimationFrame(d),t.attacker=c?"home":"ai";const y=c?_n():null;c&&(t.boostCard={value:y}),t.log.push({type:"duel",title:"Milieu de Terrain",homePlayers:n.map(g=>Ye(g)),aiPlayers:r.map(g=>Ye(g)),homeTotal:s,aiTotal:l,text:`Duel milieu : ${t.clubName} ${s} – ${l} IA → ${c?t.clubName+" attaque":"IA attaque"}`});const m=()=>{t.phase=t.attacker==="home"?"attack":"ai-attack",Le(e,t,i),t.attacker==="ai"&&setTimeout(()=>Di(e,t,i),800)};setTimeout(()=>{const g=document.getElementById("score-home"),p=document.getElementById("score-ai"),u=document.getElementById(c?"duel-row-home":"duel-row-ai"),b=document.getElementById(c?"duel-row-ai":"duel-row-home"),v=c?g:p,h=c?p:g;v&&(v.style.fontSize="80px",v.style.color=c?"#FFD700":"#ff6b6b",v.style.animation="duelPulse .5s ease"+(c?", duelGlow 1.5s ease infinite .5s":"")),h&&(h.style.opacity="0.25"),setTimeout(()=>{u&&(u.style.transformOrigin="center",u.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",u.style.zIndex="5"),setTimeout(()=>{var x;const k=document.getElementById("duel-shock");if(k){const C=(x=b||u)==null?void 0:x.getBoundingClientRect(),E=e.querySelector(".match-screen").getBoundingClientRect();C&&(k.style.top=C.top-E.top+C.height/2+"px"),k.style.animation="shockwave .5s ease-out forwards"}b&&(b.style.transformOrigin="center",b.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var x;const k=document.getElementById("duel-finale");k&&(k.innerHTML=`
  ...
  <div id="match-history-panel">
    ...
    </div>`,(x=f.querySelector("#sub-close"))==null||x.addEventListener("click",()=>f.remove()),(C=f.querySelector("#out-up"))==null||C.addEventListener("click",()=>{s>0&&(s--,y())}),(E=f.querySelector("#out-down"))==null||E.addEventListener("click",()=>{s<o.length-1&&(s++,y())}),(G=f.querySelector("#in-up"))==null||G.addEventListener("click",()=>{c>0&&(c--,y())}),(N=f.querySelector("#in-down"))==null||N.addEventListener("click",()=>{c<a.length-1&&(c++,y())});const k=(ie,Q,M,w)=>{const j=f.querySelector("#"+ie);if(!j)return;let U=0;j.addEventListener("touchstart",re=>{U=re.touches[0].clientY},{passive:!0}),j.addEventListener("touchend",re=>{const ae=re.changedTouches[0].clientY-U;if(Math.abs(ae)<30)return;const se=Q();ae<0&&se<w-1?(M(se+1),y()):ae>0&&se>0&&(M(se-1),y())},{passive:!0})};k("in-panel",()=>c,ie=>c=ie,a.length),k("out-panel",()=>s,ie=>s=ie,o.length),(ne=f.querySelector("#sub-confirm"))==null||ne.addEventListener("click",ie=>{if(ie.preventDefault(),ie.stopPropagation(),d)return;d=!0;const Q=o[s],M=a[c];if(!Q||!M)return;let w=null,j=-1;for(const[re,ae]of Object.entries(t.homeTeam)){const se=(ae||[]).findIndex(de=>de.cardId===Q.cardId);if(se!==-1){w=re,j=se;break}}if(j===-1||!w){kt("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),f.remove();return}const U={...M,_line:w,_col:Q._col||0,used:!1,boost:0};t.homeTeam[w].splice(j,1,U),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(M.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:Q.name,firstname:Q.firstname,note:Re(Q,w),portrait:Xt(Q),job:Q.job,country_code:Q.country_code,rarity:Q.rarity,clubName:Q.clubName,clubLogo:Q.clubLogo},inPlayer:{name:M.name,firstname:M.firstname,note:Re(M,w),portrait:Xt(M),job:M.job,country_code:M.country_code,rarity:M.rarity,clubName:M.clubName,clubLogo:M.clubLogo},text:`🔄 ${M.firstname} ${M.name} remplace ${Q.firstname} ${Q.name}`}),f.remove(),Do(Q,M,()=>Le(e,t,i))})}document.body.appendChild(f),y()}function Wo(e,t,i,n,r){var g,p;const o=(n.gcDefs||[]).find(u=>u.name===t),a=je[t]||{icon:"⚡",desc:"Carte spéciale."},s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[o==null?void 0:o.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",l={purple:"#b06ce0",light_blue:"#00d4ef"}[o==null?void 0:o.color]||"#b06ce0",c=(o==null?void 0:o.name)||t,d=(o==null?void 0:o.effect)||a.desc,f=o!=null&&o.image_url?`/icons/${o.image_url}`:null,y=a.icon||"⚡",m=document.createElement("div");m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",m.innerHTML=`
    ...
    </div>`,document.body.appendChild(m),(g=m.querySelector("#gc-back"))==null||g.addEventListener("click",()=>m.remove()),(p=m.querySelector("#gc-use"))==null||p.addEventListener("click",()=>{m.remove(),Jo(e,t,i,n,r)})}function ii(e,t,i,n,r,o){const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let s=[];function l(){var c,d;a.innerHTML=`
    ...
    <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start;justify-content:center">
      ${e.map(f=>{const y=f._line||f.job||"MIL",m=s.find(p=>p.cardId===f.cardId),g=Ae({...f,_evolution_bonus:0},{width:90,showStad:!0,role:y,extraNote:f.boost||0});return`<div class="gc-pick-item" data-cid="${f.cardId}"
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(r=>{r.addEventListener("click",()=>{const o=r.dataset.cardId;for(const a of["GK","DEF","MIL","ATT"]){const s=(t.homeTeam[a]||[]).find(l=>l.cardId===o);if(s){s.boost=(s.boost||0)+t.boostCard.value,t.log.push({text:`⚡ Boost +${t.boostCard.value} appliqué à ${s.name}`,type:"info"});break}}t.boostUsed=!0,i.closeModal(),Le(e,t,i)})})}async function Ft(e,t,i){var d,f;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:n}=i,r=t.homeScore>t.aiScore,o=t.homeScore===t.aiScore,a=r?"victoire":o?"nul":"defaite",s=Qn(t.mode,a);t.matchId&&await _.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:r?n.profile.id:null,home_credits_reward:s,played_at:new Date().toISOString()}).eq("id",t.matchId);const l={credits:(n.profile.credits||0)+s,matches_played:(n.profile.matches_played||0)+1};r?l.wins=(n.profile.wins||0)+1:o?l.draws=(n.profile.draws||0)+1:l.losses=(n.profile.losses||0)+1,await _.from("users").update(l).eq("id",n.profile.id),await i.refreshProfile();const c=document.createElement("div");c.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",c.innerHTML=`
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      ...
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${a?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(F),(Z=F.querySelector("#pvp-end-home"))==null||Z.addEventListener("click",()=>{F.remove(),Xe(e),m(a?"ranked":"home")})}const Q=_.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},$=>{const L=$.new;try{if(L.status==="finished"||L.forfeit){if(C)return;C=!0,E&&(clearInterval(E),E=null),L.game_state&&(x=L.game_state),ie(L);return}if(L.game_state){const z=x;x=L.game_state;const I=x._lastGC;if(I&&I.seq&&!ne.has(I.seq)&&(ne.add(I.seq),I.by!==p)){R(I.type,I.by,()=>de());return}const T=z[p+"Score"]||0,A=z[u+"Score"]||0,F=x[p+"Score"]||0,B=x[u+"Score"]||0,K=F>T,Y=B>A;if((K||Y)&&!N.has(x.round)){N.add(x.round);const oe=[...x.log||[]].reverse().find(H=>H.isGoal),Z=((oe==null?void 0:oe.homePlayers)||[]).map(H=>({name:H.name,note:H.note,portrait:H.portrait,job:H.job}));rt(Z,F,B,K,()=>de());return}de()}}catch(z){console.error("[PvP] crash:",z)}}).subscribe();async function M($){Object.assign(x,$),x.lastActionAt=new Date().toISOString();const{error:L}=await _.from("matches").update({game_state:x}).eq("id",i);L&&g("Erreur de synchronisation","error");try{de()}catch(z){console.error("[PvP] renderPvpScreen crash:",z)}}async function w(){if(C)return;C=!0,E&&(clearInterval(E),E=null);const $=n?h.away_id:h.home_id,L=n?"p2":"p1",z=n?"p1":"p2",I={...x,[L+"Score"]:3,[z+"Score"]:0,phase:"finished"},T=I.p1Score||0,A=I.p2Score||0;try{await _.from("matches").update({game_state:I}).eq("id",i);const{data:F,error:B}=await _.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:$,p_home_score:T,p_away_score:A,p_forfeit:!0});if(B&&console.error("[PvP] finish_pvp_match (forfeit):",B),d)try{await _.rpc("check_and_finish_mini_league",{p_league_id:d})}catch(K){console.error("[PvP] check_and_finish_mini_league (forfeit):",K)}try{const{data:K}=await _.rpc("apply_match_rewards",{p_match_id:i});K!=null&&K.success&&!(K!=null&&K.skipped)&&typeof t.refreshProfile=="function"&&await t.refreshProfile()}catch(K){console.error("[PvP] apply_match_rewards (forfeit):",K)}}catch{}try{_.removeChannel(Q)}catch{}setTimeout(()=>{Xe(e),d?m("mini-league",{openLeagueId:d}):m("home")},800)}const j={BOOST_STAT:({value:$=1,count:L=1,roles:z=[]},I,T)=>{const A=I[p+"Team"],F=Object.entries(A).filter(([B])=>!z.length||z.includes(B)).flatMap(([B,K])=>K.filter(Y=>!Y.used).map(Y=>({...Y,_line:B})));if(!F.length){I.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),T(I);return}U(F,L,`Choisir ${L} joueur(s) à booster (+${$})`,B=>{B.forEach(K=>{const Y=(A[K._line]||[]).find(oe=>oe.cardId===K.cardId);Y&&(Y.boost=(Y.boost||0)+$,I.log.push({text:`⚡ +${$} sur ${Y.name}`,type:"info"}))}),I[p+"Team"]=A,T(I)})},DEBUFF_STAT:({value:$=1,count:L=1,roles:z=[],target:I="ai"},T,A)=>{const F=I==="home"?p:u,B=T[F+"Team"],K=I==="home"?"allié":"adverse",Y=Object.entries(B).filter(([oe])=>!z.length||z.includes(oe)).flatMap(([oe,Z])=>Z.map(H=>({...H,_line:oe})));if(!Y.length){T.log.push({text:`🎯 Aucun joueur ${K}`,type:"info"}),A(T);return}U(Y,L,`Choisir ${L} joueur(s) ${K}(s) (-${$})`,oe=>{oe.forEach(Z=>{const H=(B[Z._line]||[]).find(le=>le.cardId===Z.cardId);H&&(H.boost=(H.boost||0)-$,T.log.push({text:`🎯 -${$} sur ${H.name}`,type:"info"}))}),T[F+"Team"]=B,A(T)})},GRAY_PLAYER:({count:$=1,roles:L=[],target:z="ai"},I,T)=>{const A=z==="home"?p:u,F=I[A+"Team"],B=z==="home"?"allié":"adverse",K=Object.entries(F).filter(([Y])=>!L.length||L.includes(Y)).flatMap(([Y,oe])=>oe.filter(Z=>!Z.used).map(Z=>({...Z,_line:Y})));if(!K.length){I.log.push({text:`❌ Aucun joueur ${B}`,type:"info"}),T(I);return}U(K,$,`Choisir ${$} joueur(s) ${B}(s) à exclure`,Y=>{const oe="usedCardIds_"+A,Z=new Set(I[oe]||[]);Y.forEach(H=>{const le=(F[H._line]||[]).find(pe=>pe.cardId===H.cardId);le&&(le.used=!0,Z.add(H.cardId),I.log.push({text:`❌ ${le.name} exclu !`,type:"info"}))}),I[oe]=[...Z],I[A+"Team"]=F,T(I)})},REVIVE_PLAYER:({count:$=1,roles:L=[]},z,I)=>{const T=z[p+"Team"],A=Object.entries(T).filter(([F])=>!L.length||L.includes(F)).flatMap(([F,B])=>B.filter(K=>K.used).map(K=>({...K,_line:F})));if(!A.length){z.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),I(z);return}U(A,$,`Choisir ${$} joueur(s) à ressusciter`,F=>{F.forEach(B=>{const K=(T[B._line]||[]).find(Y=>Y.cardId===B.cardId);K&&(K.used=!1,z.log.push({text:`💫 ${K.name} ressuscité !`,type:"info"}))}),z[p+"Team"]=T,I(z)})},REMOVE_GOAL:({},$,L)=>{const z=u+"Score";$[z]>0?($[z]--,$.log.push({text:"🚫 Dernier but annulé !",type:"info"})):$.log.push({text:"🚫 Aucun but à annuler",type:"info"}),L($)},ADD_GOAL_DRAW:({},$,L)=>{$[p+"Score"]===$[u+"Score"]?($[p+"Score"]++,$.log.push({text:"🎯 But bonus !",type:"info"})):$.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),L($)},ADD_SUB:({value:$=1},L,z)=>{L.maxSubs=(L.maxSubs||3)+$,L.log.push({text:`🔄 +${$} remplacement(s)`,type:"info"}),z(L)},CUSTOM:({},$,L)=>L($)};function U($,L,z,I){const T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let A=[];function F(){var K,Y;const B=$.map(oe=>{const Z=oe._line||oe.job||"MIL",H=A.find(pe=>pe.cardId===oe.cardId),le=Ae({...oe,_evolution_bonus:0},{width:90,showStad:!0,role:Z,extraNote:oe.boost||0});return`<div class="pp-item" data-cid="${oe.cardId}"
        ...
        </div>`,(K=T.querySelector("#pp-close"))==null||K.addEventListener("click",()=>T.remove()),T.querySelectorAll(".pp-item").forEach(oe=>{oe.addEventListener("click",()=>{const Z=oe.dataset.cid,H=$.find(pe=>pe.cardId===Z),le=A.findIndex(pe=>pe.cardId===Z);H&&(le>-1?A.splice(le,1):A.length<L&&A.push(H),F())})}),(Y=T.querySelector("#pp-confirm"))==null||Y.addEventListener("click",()=>{T.remove(),I(A)})}F(),document.body.appendChild(T)}async function re($,L){const I=(x["gcCardsFull_"+p]||[]).find(B=>B.id===$),T=(I==null?void 0:I._gcDef)||(x.gcDefs||[]).find(B=>{var K;return B.name===L||((K=B.name)==null?void 0:K.toLowerCase().trim())===(L==null?void 0:L.toLowerCase().trim())}),A=[...x["usedGc_"+p]||[],$],F={type:L,by:p,seq:(x._gcAnimSeq||0)+1};ne.add(F.seq),R(L,p,async()=>{if(T!=null&&T.effect_type&&T.effect_type!=="CUSTOM"){const K=j[T.effect_type];if(K){const Y={...x};K(T.effect_params||{},Y,async oe=>{oe["usedGc_"+p]=A,oe._lastGC=F,oe._gcAnimSeq=F.seq,await M(oe)});return}}const B={...x};switch(L){case"Remplacement+":B.maxSubs=(B.maxSubs||3)+1,B.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const K=u+"Score";B[K]>0&&(B[K]--,B.log.push({text:"🚫 But annulé",type:"info"}));break}}B["usedGc_"+p]=A,B._lastGC=F,B._gcAnimSeq=F.seq,await M(B)})}function ae($,L){const z="usedCardIds_"+$,I=new Set(x[z]||[]);L.forEach(T=>I.add(T)),x[z]=[...I]}function se(){for(const $ of["p1","p2"]){const L=new Set(x["usedCardIds_"+$]||[]),z=x[$+"Team"];if(z)for(const I of["GK","DEF","MIL","ATT"])(z[I]||[]).forEach(T=>{T.used=L.has(T.cardId)})}}function de(){var st,dt,wt,Gi,Ri,Ni;if(x.phase==="reveal")return $e();if(x.phase==="midfield")return J();if(x.phase==="finished")return Nt();const $=x[p+"Team"];x[u+"Team"],se();const L=x[p+"Score"],z=x[u+"Score"],I=x[p+"Name"],T=x[u+"Name"],A=x.phase===p+"-attack",F=x.phase===p+"-defense",B=Array.isArray(x["selected_"+p])?x["selected_"+p]:[],K=B.map(ee=>ee.cardId),Y=window.innerWidth>=700,oe=x[p+"Subs"]||[],Z=x["usedSubIds_"+p]||[],H=oe.filter(ee=>!Z.includes(ee.cardId)),le=x["gcCardsFull_"+p]||[],pe=x["usedGc_"+p]||[],ue=le.filter(ee=>!pe.includes(ee.id)),ce=x.boostOwner===p&&!x.boostUsed,xe=[...$.MIL||[],...$.ATT||[]].filter(ee=>!ee.used),ye=A&&xe.length===0?[...$.GK||[],...$.DEF||[]].filter(ee=>!ee.used).map(ee=>ee.cardId):[];function te(ee,fe,Ie){var jt,ct;const ve=ee._gcDef,At={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ve==null?void 0:ve.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",tt={purple:"#b06ce0",light_blue:"#00d4ef"}[ve==null?void 0:ve.color]||"#b06ce0",Fe=(ve==null?void 0:ve.name)||ee.gc_type,Oe=(ve==null?void 0:ve.effect)||((jt=je[ee.gc_type])==null?void 0:jt.desc)||"",Me=ve!=null&&ve.image_url?`/icons/${ve.image_url}`:null,_t=((ct=je[ee.gc_type])==null?void 0:ct.icon)||"⚡",Ge=Math.round(Ie*.22),lt=Math.round(Ie*.22),it=Ie-Ge-lt,Mt=Fe.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${ee.id}" data-gc-type="${ee.gc_type}"
        ...
        <div style="font-size:${Math.round(fe*.09)}px;color:#000;font-weight:900">+${x.boostValue}</div>
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${Gt($,x[p+"Formation"],De,K,Y?1300:Tt(),Y?600:Dt(),ye)}
      </div>
    </div>`,Ne=x[p+"Team"],vt=(()=>{var fe,Ie,ve,At,tt;if(F&&((fe=x.pendingAttack)!=null&&fe.players)){const Fe=x.pendingAttack;let Oe="";if(B.length>0){const Me=B.map(Ge=>{const lt=(Ne[Ge._role]||[]).find(gi=>gi.cardId===Ge.cardId)||Ge,it=Ne[Ge._role]||[],Mt=it.findIndex(gi=>gi.cardId===Ge.cardId),jt=ut(it.length),ct=Mt>=0?jt[Mt]:lt._col??1;return{...lt,_line:Ge._role,_col:ct}}),_t=Wt(Me,((Ie=x.modifiers)==null?void 0:Ie[p])||{},x[p+"Formation"]);Oe=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
            ...
            <div style="display:flex;justify-content:center">${We(Me.map(Ge=>({...Ge,used:!1})),"#3a7bd5",_t.total,"defense",x[p+"Formation"])}</div>
          </div>`}return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b;text-align:center">
          ...
          <div style="display:flex;justify-content:center">${We((Fe.players||[]).map(Me=>({...Me,used:!1})),"#ff6b6b",Fe.total,"attack",x[u+"Formation"])}</div>
          ${Oe}
        </div>`}if(A&&((ve=x.pendingAttack)!=null&&ve.players)){const Fe=x.pendingAttack;return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #00ff88;text-align:center">
          <div style="font-size:8px;color:#00ff88;letter-spacing:2px;margin-bottom:4px;text-transform:uppercase">⚔️ VOUS ATTAQUEZ</div>
          <div style="display:flex;justify-content:center">${We((Fe.players||[]).map(Oe=>({...Oe,used:!1})),"#00ff88",Fe.total,"attack",x[p+"Formation"])}</div>
        </div>`}if(A&&!((At=x.pendingAttack)!=null&&At.players)&&B.length>0){const Fe=B.map(Me=>{const _t=(Ne[Me._role]||[]).find(ct=>ct.cardId===Me.cardId)||Me,Ge=["GK","DEF"].includes(Me._role),lt=Ne[Me._role]||[],it=lt.findIndex(ct=>ct.cardId===Me.cardId),Mt=ut(lt.length),jt=it>=0?Mt[it]:_t._col??1;return{..._t,_line:Me._role,_col:jt,...Ge?{note_a:Math.max(1,Number(_t.note_a)||0)}:{}}}),Oe=Yt(Fe,((tt=x.modifiers)==null?void 0:tt[p])||{},x[p+"Formation"]);return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #FFD700;text-align:center">
          ...
          <div style="display:flex;justify-content:center">${We(Fe.map(Me=>({...Me,used:!1})),"#FFD700",Oe.total,"attack",x[p+"Formation"])}</div>
        </div>`}const ee=(x.log||[]).slice(-1)[0];return ee?'<div style="padding:2px 4px">'+$n(ee)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3);text-align:center">⏳ Match en cours...</div>'})(),et=`
      <div style="display:flex;align-items:center;padding:8px 10px;background:rgba(0,0,0,0.5);gap:6px;flex-shrink:0">
      ...
      <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
        ...
        <div id="mobile-play-area" style="flex:1;min-height:0;display:flex;overflow:hidden">
          <div id="match-field" style="flex:1;min-width:0;min-height:0;overflow:hidden">
            <div class="terrain-wrapper" style="width:100%;height:100%;overflow:hidden">
              ${Gt($,x[p+"Formation"],De,K,Tt(),Dt(),ye)}
            </div>
          </div>
        </div>
        <div id="mobile-action-bar" style="position:absolute;left:0;right:0;bottom:0;z-index:20;background:rgba(0,0,0,0.55);padding:6px 8px 8px;display:flex;flex-direction:column;gap:6px;box-shadow:0 -4px 16px rgba(0,0,0,0.5)">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      </div>`,(Pe=Z.querySelector("#psub-close"))==null||Pe.addEventListener("click",()=>Z.remove()),(Se=Z.querySelector("#pout-up"))==null||Se.addEventListener("click",()=>{B>0&&(B--,H())}),(Ue=Z.querySelector("#pout-down"))==null||Ue.addEventListener("click",()=>{B<A.length-1&&(B++,H())}),(St=Z.querySelector("#pin-up"))==null||St.addEventListener("click",()=>{Y>0&&(Y--,H())}),(Qe=Z.querySelector("#pin-down"))==null||Qe.addEventListener("click",()=>{Y<F.length-1&&(Y++,H())});const ze=(De,Ze,Ne,vt)=>{const et=Z.querySelector("#"+De);if(!et)return;let Ke=0;et.addEventListener("touchstart",st=>{Ke=st.touches[0].clientY},{passive:!0}),et.addEventListener("touchend",st=>{const dt=st.changedTouches[0].clientY-Ke;if(Math.abs(dt)<30)return;const wt=Ze();dt<0&&wt<vt-1?(Ne(wt+1),H()):dt>0&&wt>0&&(Ne(wt-1),H())},{passive:!0})};ze("pin-panel",()=>Y,De=>Y=De,F.length),ze("pout-panel",()=>B,De=>B=De,A.length),(Zt=Z.querySelector("#psub-confirm"))==null||Zt.addEventListener("click",async De=>{if(De.preventDefault(),De.stopPropagation(),oe)return;oe=!0;const Ze=A[B],Ne=F[Y];if(!Ze||!Ne)return;const vt=Ze._line,et=(z[vt]||[]).findIndex(dt=>dt.cardId===Ze.cardId);if(et===-1){g("Erreur : joueur introuvable","error"),Z.remove();return}const Ke={...Ne,_line:vt,position:Ze.position,used:!1,boost:0};z[vt].splice(et,1,Ke);const st=[...I,Ne.cardId];Z.remove(),X(Ze,Ne,async()=>{await M({[p+"Team"]:z,[u+"Team"]:x[u+"Team"],["usedSubIds_"+p]:st})})})}document.body.appendChild(Z),H()}function X($,L,z){const I={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const A=(K,Y,oe)=>`<div style="text-align:center">
      ...
      <div style="font-size:18px;font-weight:900;color:#ff6b6b">${x[u+"Name"]}</div>
      <div style="width:min(90vw,420px)">${pi(x[u+"Team"],x[u+"Formation"],null,[],Tt(),Dt())}</div>
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild($),(L=$.querySelector("#pvp-opp-close"))==null||L.addEventListener("click",()=>$.remove())}function D(){var z;const $=x.log||[],L=document.createElement("div");L.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column",L.innerHTML=`
      ...
      </div>`,document.body.appendChild(L),(z=L.querySelector("#pvp-hist-close"))==null||z.addEventListener("click",()=>L.remove())}async function O(){if(x.phase!==p+"-attack")return;const $=p==="p1"?"p2":"p1",L=(x.round||0)+1,z=[...x.log||[]];z.push({type:"info",text:`⏭️ ${x[p+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const I=He(x),T=Ce($),A=I||!T?"finished":$+"-attack";await M({["selected_"+p]:[],modifiers:{...x.modifiers,[p]:{}},pendingAttack:null,phase:A,attacker:$,round:L,log:z}),A==="finished"&&await Je(x)}function _e($){const L=($.GK||[]).some(I=>!I.used),z=["DEF","MIL","ATT"].some(I=>($[I]||[]).some(T=>!T.used));return L&&!z}function ke($){return($.p1Score||0)!==($.p2Score||0)?null:_e($.p1Team)&&!ge($.p2Team)?{side:"p1",gk:($.p1Team.GK||[]).find(L=>!L.used)}:_e($.p2Team)&&!ge($.p1Team)?{side:"p2",gk:($.p2Team.GK||[]).find(L=>!L.used)}:null}async function be($,L){const{side:z,gk:I}=$;if(!I)return;const T=z+"Score",A=(L[T]||0)+1;I.used=!0;const F=[...L.log||[]];F.push({type:"duel",isGoal:!0,homeScored:z===p,homePlayers:z==="p1"?[Ct(I)]:[],aiPlayers:z==="p2"?[Ct(I)]:[],text:`⚽ DERNIER CORNER — Le gardien de ${L[z+"Name"]} marque !`});const B=(L.round||0)+1,K=z==="p1"?"p2":"p1",Y={...L,[T]:A},oe=He(Y);N.add(B);const Z=document.createElement("div");Z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:950;display:flex;align-items:center;justify-content:center;overflow:hidden;text-align:center;padding:24px",document.body.appendChild(Z);const H=(ue,ce)=>new Promise(xe=>{Z.innerHTML=`<div style="font-size:32px;font-weight:900;color:${ce};letter-spacing:2px;animation:lcFade 1.4s ease both">${ue}</div>
      <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(xe,1400)});await H("⚽ DERNIER CORNER","#FFD700"),await H("🧤 LE GARDIEN MONTE !","#4fc3f7"),Z.remove();const le=z===p?A:L[p+"Score"]||0,pe=z===p?L[u+"Score"]||0:A;rt([Ct(I)],le,pe,z===p,async()=>{await M({[T]:A,log:F,round:B,pendingAttack:null,phase:oe?"finished":K+"-attack",attacker:K,["selected_"+p]:[],modifiers:{...x.modifiers,[p]:{}}}),oe&&await Je({...x,[T]:A})})}async function he(){const $=x[p+"Team"],L=!["GK","DEF","MIL","ATT"].some(A=>(x[u+"Team"][A]||[]).some(F=>!F.used)),z=(x["selected_"+p]||[]).map(A=>{const F=($[A._role]||[]).find(pe=>pe.cardId===A.cardId)||A,B=["GK","DEF"].includes(A._role),K=$[A._role]||[],Y=K.findIndex(pe=>pe.cardId===A.cardId),oe=ut(K.length),Z=Y>=0?oe[Y]:F._col??1,H=x.stadiumDef||x.homeStadiumDef||null,le=F.stadiumBonus||H&&(H.club_id&&String(F.club_id)===String(H.club_id)||H.country_code&&F.country_code===H.country_code)||!1;return{...F,_line:A._role,_col:Z,stadiumBonus:le,...B?{note_a:Math.max(1,Number(F.note_a)||0)}:{}}});if(!z.length)return;const I=Yt(z,x.modifiers[p]||{},x[p+"Formation"]);ae(p,z.map(A=>A.cardId)),z.forEach(A=>{const F=($[A._role]||[]).find(B=>B.cardId===A.cardId);F&&(F.used=!0)}),x["selected_"+p]=[],de();const T=[...x.log||[]];if(L){if(z.length===1&&(z[0]._line==="GK"||z[0].job==="GK")&&(x.p1Score||0)===(x.p2Score||0)){await be({side:p,gk:z[0]},{...x,[p+"Team"]:$,log:T});return}const F=(x[p+"Score"]||0)+1,B=z.map(H=>Ct(H));T.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:B,homeTotal:I.total,aiTotal:0});const K=(x.round||0)+1,Y=p==="p1"?"p2":"p1",oe={...x,[p+"Team"]:$,[p+"Score"]:F},Z=He(oe);N.add(K),rt(B,F,x[u+"Score"]||0,!0,async()=>{await M({[p+"Team"]:$,[p+"Score"]:F,["selected_"+p]:[],modifiers:{...x.modifiers,[p]:{}},pendingAttack:null,phase:Z?"finished":Y+"-attack",attacker:Y,round:K,log:T}),Z&&await Je({...x,[p+"Score"]:F})});return}T.push({type:"pending",text:`⚔️ ${x[p+"Name"]} attaque (${I.total})`}),await M({[p+"Team"]:$,[u+"Team"]:x[u+"Team"],pendingAttack:{...I,players:z,side:p},["selected_"+p]:[],modifiers:{...x.modifiers,[p]:{}},phase:u+"-defense",log:T})}async function Ee(){const $=x[p+"Team"],L=(x["selected_"+p]||[]).map(ce=>{const xe=($[ce._role]||[]).find(Se=>Se.cardId===ce.cardId)||ce,ye=$[ce._role]||[],te=ye.findIndex(Se=>Se.cardId===ce.cardId),ze=ut(ye.length),Pe=te>=0?ze[te]:xe._col??1;return{...xe,_line:ce._role,_col:Pe}}),z=Wt(L,x.modifiers[p]||{},x[p+"Formation"]);ae(p,L.map(ce=>ce.cardId)),L.forEach(ce=>{const xe=($[ce._role]||[]).find(ye=>ye.cardId===ce.cardId);xe&&(xe.used=!0)}),x["selected_"+p]=[],de();const I=Bi(x.pendingAttack.total,z.total,x.modifiers[p]||{}),T=x.pendingAttack.side,A=I==="attack"||(I==null?void 0:I.goal),F=T==="p1"?"p2":"p1",B=(x.round||0)+1,K=(x.pendingAttack.players||[]).map(ce=>Ct(ce)),Y=[...x.log||[]];Y.push({type:"duel",isGoal:A,homeScored:A&&T===p,text:A?`⚽ BUT de ${x[T+"Name"]} ! (${x.pendingAttack.total} vs ${z.total})`:`✋ Attaque stoppée (${x.pendingAttack.total} vs ${z.total})`,homePlayers:K,aiPlayers:L.map(ce=>Ct(ce)),homeTotal:x.pendingAttack.total,aiTotal:z.total});const oe=A?(x[T+"Score"]||0)+1:x[T+"Score"]||0,Z={...x,[p+"Team"]:$,[T+"Score"]:oe},H=He(Z),le=H?"finished":F+"-attack",pe=!H&&!A?ke(Z):null,ue=async()=>{if(pe){await be(pe,Z);return}await M({[p+"Team"]:$,[u+"Team"]:x[u+"Team"],[T+"Score"]:oe,["selected_"+p]:[],modifiers:{...x.modifiers,[p]:{}},pendingAttack:null,phase:le,attacker:F,round:B,log:Y}),(le==="finished"||H)&&await Je({...x,[T+"Score"]:oe})};if(A){const ce=T===p,xe=ce?oe:x[p+"Score"]||0,ye=ce?x[u+"Score"]||0:oe;N.add(B),rt(K,xe,ye,ce,ue)}else await ue()}function Te($){return["MIL","ATT"].some(L=>($[L]||[]).some(z=>!z.used))}function ge($){return["GK","DEF","MIL","ATT"].some(L=>($[L]||[]).some(z=>!z.used))}function qe($){return ge($)&&!Te($)}function Ce($){const L=x[$+"Team"];return!!(Te(L)||qe(L))}function He($){return!ge($.p1Team)&&!ge($.p2Team)}function ht($){const L=$.p1Score||0,z=$.p2Score||0;return L===z?null:L>z?h.home_id:h.away_id}async function Je($){try{const L=ht($),z=L?h.home_id===L?h.away_id:h.home_id:null,I=$.p1Score||0,T=$.p2Score||0,{data:A,error:F}=await _.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:L,p_home_score:I,p_away_score:T,p_forfeit:!1});if(F&&console.error("[PvP] finish_pvp_match:",F),d)try{await _.rpc("check_and_finish_mini_league",{p_league_id:d})}catch(B){console.error("[PvP] check_and_finish_mini_league:",B)}L&&z&&to(L,z).catch(()=>{});try{const{data:B}=await _.rpc("apply_match_rewards",{p_match_id:i});B!=null&&B.success&&!(B!=null&&B.skipped)&&typeof t.refreshProfile=="function"&&await t.refreshProfile()}catch(B){console.error("[PvP] apply_match_rewards:",B)}if(typeof c=="function")try{await c({homeScore:I,awayScore:T,winnerId:L,homeId:h.home_id,awayId:h.away_id,matchId:i})}catch(B){console.error("[PvP] onMatchEnd:",B)}}catch(L){console.error("[PvP] finishMatch:",L)}}function Nt(){var T;const $=x[p+"Score"],L=x[u+"Score"],z=$>L,I=$===L;ot(e),e.innerHTML=`
    ...
    </div>`,(T=document.getElementById("pvp-home"))==null||T.addEventListener("click",()=>{try{_.removeChannel(Q)}catch{}Xe(e),d?m("mini-league",{openLeagueId:d}):m("home")})}de()}async function Ti(e,t,i=!1){var s,l;const{state:n,navigate:r,toast:o}=t,a=i&&((l=(s=t==null?void 0:t.state)==null?void 0:s.params)==null?void 0:l.rankedData)||null;await ui(e,t,i?"ranked":"random",async({deckId:c,formation:d,starters:f,subsRaw:y,gcCardsEnriched:m,gcDefs:g,stadiumDef:p})=>{ot(e);const u=async b=>{await ir(e,t,c,d,f,y,b||[],g,p,i,a)};if(!(m!=null&&m.length)){await u([]);return}ci(e,m,u)})}async function tr(e,t,i){return er(e,t,i)}async function ir(e,t,i,n,r,o,a=[],s=[],l=null,c=!1,d=null){var ie,Q;const{state:f,navigate:y,toast:m}=t,g=(d==null?void 0:d.mmr)??((ie=f.profile)==null?void 0:ie.mmr)??1e3,p=c?Mi(g):null,u=c?"linear-gradient(135deg, #1a0a2e 0%, #2d1060 50%, #1a0a2e 100%)":"linear-gradient(135deg, #0a1a2e 0%, #0d3d1e 100%)",b=c?(p==null?void 0:p.color)||"#D4A017":"#FFD700";let v=!1,h=null,k=null;e.innerHTML=`
    ...
    </div>`;const x=async()=>{v=!0,h&&(h.unsubscribe(),h=null),k&&(clearInterval(k),k=null);try{await _.rpc("cancel_matchmaking",{p_user_id:f.profile.id})}catch(M){console.error("[Matchmaking] cancel error:",M)}};(Q=document.getElementById("cancel-mm"))==null||Q.addEventListener("click",async()=>{await x(),Xe(e),y("home")});const C=async(M,w)=>{v||(v=!0,h&&(h.unsubscribe(),h=null),k&&(clearInterval(k),k=null),await It(e,t,M,w,{myGC:a,gcDefs:s,stadiumDef:l,isRanked:c,rankedData:d,onMatchEnd:c?nr:null}))},E=f.user.id,{data:G,error:N}=c?await _.rpc("try_matchmake_ranked",{p_user_id:E,p_deck_id:i,p_mmr:g}):await _.rpc("try_matchmake",{p_user_id:E,p_deck_id:i});if(N||!(G!=null&&G.success)){console.error("[Matchmaking] try_matchmake error:",N||G),m("Erreur de matchmaking","error"),Xe(e),y("home");return}if(G.matched){await C(G.match_id,!1);return}const ne=async()=>{if(v)return;const{data:M}=await _.from("matchmaking_queue").select("status, match_id").eq("user_id",E).maybeSingle();(M==null?void 0:M.status)==="matched"&&M.match_id&&await C(M.match_id,!0)};h=_.channel(`mm_${E}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${E}`},M=>{const w=M.new;w.status==="matched"&&w.match_id&&C(w.match_id,!0)}).subscribe(),k=setInterval(ne,3e3),setTimeout(()=>{v||x()},12e4)}async function nr(e,t){const{state:i,toast:n}=t,{winnerId:r,homeId:o,awayId:a,homeScore:s,awayScore:l,matchId:c}=e,d=i.user.id,f=o===d,y=r===d,{data:m}=await _.from("users").select("mmr, mmr_rd, mmr_v").eq("id",d).single(),{data:g}=await _.from("users").select("mmr, mmr_rd, mmr_v").eq("id",f?a:o).single();if(m&&g){const p=Mn(m.mmr,m.mmr_rd,m.mmr_v,g.mmr,g.mmr_rd,y?1:0);await _.from("users").update({mmr:p.mmr,mmr_rd:p.rd,mmr_v:p.v,mmr_wins:y?_.sql`mmr_wins + 1`:void 0,mmr_losses:y?void 0:_.sql`mmr_losses + 1`}).eq("id",d),n(y?`+MMR ↑ ${Math.round(p.mmr-m.mmr)}`:`-MMR ↓ ${Math.round(m.mmr-p.mmr)}`,y?"success":"error",4e3)}}const or=Object.freeze(Object.defineProperty({__proto__:null,renderMatchRandom:Ti,resumePvpMatch:tr},Symbol.toStringTag,{value:"Module"}));async function rr(e,t){const{state:i,navigate:n,toast:r}=t,o=i.params||{},a=o.friendId||null,s=o.friendName||"Ami",l=!!o.isAccepting;await ui(e,t,"friend",async({deckId:c,formation:d,starters:f,subsRaw:y,gcCardsEnriched:m,gcDefs:g,stadiumDef:p})=>{ot(e);const u=async b=>{const v=b||[];l?await sr(e,t,c,v,g,p):await ar(e,t,c,v,g,p,a,s)};if(!(m!=null&&m.length)){await u([]);return}ci(e,m,u)})}async function ar(e,t,i,n,r,o,a,s){var v;const{state:l,navigate:c,toast:d}=t,f=l.user.id,{data:y,error:m}=await _.from("friend_match_invites").insert({inviter_id:f,invitee_id:a,friend_id:a,status:"pending",inviter_deck_id:i,expires_at:new Date(Date.now()+2*60*1e3).toISOString()}).select("id").single();if(m||!y){console.error("[Friend] Erreur création invitation:",m),d("Impossible de créer l'invitation","error"),c("home");return}e.innerHTML=`
    ...
    </div>`;let g=!1,p=null;const u=async()=>{p&&(p.unsubscribe(),p=null)};(v=document.getElementById("cancel-friend"))==null||v.addEventListener("click",async()=>{await u(),await _.from("friend_match_invites").update({status:"declined"}).eq("id",y.id),Xe(e),c("home")}),p=_.channel(`friend_invite_${y.id}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"friend_match_invites",filter:`id=eq.${y.id}`},async({new:h})=>{g||h.status!=="accepted"||!h.match_id||(g=!0,await u(),await It(e,t,h.match_id,!0,{myGC:n,gcDefs:r,stadiumDef:o}))}).subscribe();const b=setInterval(async()=>{if(g)return;const{data:h}=await _.from("friend_match_invites").select("status, match_id").eq("id",y.id).single();(h==null?void 0:h.status)==="accepted"&&h.match_id&&(g=!0,clearInterval(b),await u(),await It(e,t,h.match_id,!0,{myGC:n,gcDefs:r,stadiumDef:o}))},3e3);setTimeout(async()=>{g||(clearInterval(b),await u(),d("Invitation expirée","info"),c("home"))},12e4)}async function sr(e,t,i,n,r,o){const{state:a,navigate:s,toast:l}=t,c=a.user.id,{data:d}=await _.from("friend_match_invites").select("id, inviter_id").eq("invitee_id",c).eq("status","pending").order("created_at",{ascending:!1}).limit(1).maybeSingle();if(!d){l("Aucune invitation en attente","error"),s("home");return}const{data:f,error:y}=await _.rpc("accept_friend_invite",{p_invite_id:d.id,p_invitee_deck_id:i});if(y||!(f!=null&&f.success)){console.error("[Friend] Erreur accept_friend_invite:",y||f),l((f==null?void 0:f.error)||"Impossible de rejoindre le match","error"),s("home");return}await It(e,t,f.match_id,!1,{myGC:n,gcDefs:r,stadiumDef:o})}const zn="#1A6B3C",mt="#D4A017",Ot="rgba(255,255,255,0.045)",$t="rgba(255,255,255,0.09)",Et="rgba(255,255,255,0.08)",Be="#f3f5f2",nt="rgba(243,245,242,0.62)",Ve="rgba(243,245,242,0.4)";async function dr(e,t){var n;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const i=(n=t.state.params)==null?void 0:n.openLeagueId;if(i){t.state.params.openLeagueId=null,await at(e,t,i);return}await bt(e,t)}async function bt(e,t,i="waiting"){var v,h;const{state:n}=t,r=n.profile.id,{data:o}=await _.from("mini_league_members").select("league_id").eq("user_id",r),a=(o||[]).map(k=>k.league_id),{data:s,error:l}=await _.from("mini_leagues").select("*, mini_league_members(count)").eq("status","waiting").eq("is_archived",!1).order("created_at",{ascending:!1}).limit(30),c=l?(await _.from("mini_leagues").select("*, mini_league_members(count)").eq("status","waiting").order("created_at",{ascending:!1}).limit(30)).data||[]:s||[],f=(a.length?await _.from("mini_leagues").select("*, mini_league_members(count)").in("id",a).order("created_at",{ascending:!1}):{data:[]}).data||[],y=f.filter(k=>k.status==="waiting"&&!k.is_archived),m=f.filter(k=>k.status==="active"&&!k.is_archived),g=f.filter(k=>k.is_archived||k.status==="finished");let p=g;if(g.length){const{data:k}=await _.from("mini_league_matches").select("league_id").in("league_id",g.map(C=>C.id)).or(`home_id.eq.${r},away_id.eq.${r}`).not("status","eq","bye"),x=new Set((k||[]).map(C=>C.league_id));p=g.filter(C=>x.has(C.id))}const u=c.filter(k=>!a.includes(k.id)),b=[{key:"waiting",label:"🟡 En attente",count:y.length+u.length},{key:"active",label:"🟢 En cours",count:m.length},{key:"archived",label:"📁 Archivées",count:p.length}];e.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:10px">
      ${i==="waiting"?lr(y,u,r):i==="active"?cr(m,r):pr(p,r)}
    </div>
  </div>`,(v=document.getElementById("ml-create-btn"))==null||v.addEventListener("click",()=>ur(e,t)),(h=document.getElementById("ml-refresh-list"))==null||h.addEventListener("click",()=>bt(e,t,i)),e.querySelectorAll(".ml-tab").forEach(k=>k.addEventListener("click",()=>bt(e,t,k.dataset.tab))),e.querySelectorAll("[data-league-id]").forEach(k=>k.addEventListener("click",()=>at(e,t,k.dataset.leagueId))),e.querySelectorAll("[data-join]").forEach(k=>k.addEventListener("click",x=>{x.stopPropagation(),In(e,t,k.dataset.join,k.dataset.type)})),e.querySelectorAll("[data-delete]").forEach(k=>k.addEventListener("click",x=>{x.stopPropagation(),zi(e,t,k.dataset.delete,k.dataset.name,i)}))}function li(e,t,i=!1){var a,s;const n=e.creator_id===t,r=e.pot||0,o=((s=(a=e.mini_league_members)==null?void 0:a[0])==null?void 0:s.count)||0;return`<div data-league-id="${e.id}" style="background:${Ot};border:1px solid ${$t};border-radius:12px;padding:14px 16px;cursor:pointer;margin-bottom:8px">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    <div style="padding:14px 16px;background:var(--nav-bg,#0d1a0f);border-bottom:1px solid ${Et};display:flex;align-items:center;gap:10px">
      ...
      <div style="text-align:right;flex-shrink:0">
        ...
        ${g>0?`<div style="font-size:12px;font-weight:900;color:${mt};margin-top:4px">🏆 ${g.toLocaleString("fr")} cr.</div>`:""}
      </div>
    </div>
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:${Ot};border:1px solid ${$t};border-radius:12px;padding:16px">
        <div style="font-size:14px;font-weight:900;margin-bottom:4px;color:${Be}">👥 Joueurs (${f.length}/${a.max_players})</div>
        <div style="font-size:12px;color:${nt};margin-bottom:10px">💰 ${p} cr./joueur → Pot estimé : ${(p*f.length).toLocaleString("fr")} cr. (🥇${Math.floor(p*f.length*.7).toLocaleString("fr")} · 🥈${Math.floor(p*f.length*.2).toLocaleString("fr")} · 🥉${Math.floor(p*f.length*.1).toLocaleString("fr")})</div>
        ${f.map(M=>`
      ...
      <div style="background:${Ot};border:1px solid ${$t};border-radius:12px;padding:16px">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          <thead><tr style="font-size:10px;color:${Ve};text-transform:uppercase;border-bottom:2px solid ${Et}">
            ...
            ${g>0&&a.status==="finished"?`<th style="text-align:right;padding:5px 0;color:${mt}">💰</th>`:""}
          </tr></thead>
          <tbody>${y.map((M,w)=>{const j=g>0&&a.status==="finished"?w===0?Math.floor(g*.7):w===1?Math.floor(g*.2):w===2?Math.floor(g*.1):0:0;return`<tr style="border-bottom:1px solid ${Et};${M.userId===o?"background:rgba(74,222,128,0.08);":""}">
              ...
              ${g>0&&a.status==="finished"?`<td style="text-align:right;font-weight:700;color:${mt}">${j?j.toLocaleString("fr")+" cr.":"—"}</td>`:""}
            </tr>`}).join("")}</tbody>
        </table>
      </div>`:""}
      ...
      <div style="background:${Ot};border:1px solid ${$t};border-radius:12px;padding:16px">
        ...
        ${Array.from({length:Math.max(0,a.status==="active"?a.current_day-1:a.current_day)},(M,w)=>w+1).reverse().map(M=>{const w=(l||[]).filter(j=>j.matchday===M);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:${Ve};margin-bottom:6px">Journée ${M}</div>${w.map(j=>nn(j,f,o,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
    ...
    </div>`;let d=!1;const f=async u=>{if(!d){if(d=!0,clearInterval(g),m)try{m.unsubscribe()}catch{}u&&await It(e,t,u,r,{mlLeagueId:n,mlMatchId:i,myGC:o||[],gcDefs:a,stadiumDef:s}),l()}};(p=document.getElementById("wait-back"))==null||p.addEventListener("click",()=>{if(d=!0,clearInterval(g),m)try{m.unsubscribe()}catch{}c("mini-league",{openLeagueId:n}),l()});const y=async()=>{if(d)return;const{data:u}=await _.from("mini_league_matches").select("match_id").eq("id",i).single();u!=null&&u.match_id&&f(u.match_id)},m=_.channel(`ml_match_${i}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"mini_league_matches",filter:`id=eq.${i}`},u=>{var b;(b=u.new)!=null&&b.match_id&&f(u.new.match_id)}).subscribe(),g=setInterval(y,3e3);y()})}const _r="/",kr=[{emoji:"⚽",title:"Bienvenue dans Manager Wars !",color:"#1A6B3C",content:`<p>Tu es désormais un <strong>manager de football</strong> virtuel.</p>
    ...
    <p style="margin-top:12px;font-size:13px;color:#888">Tu peux revoir ce tutoriel depuis les paramètres à tout moment.</p>`}];function $r(e,t,i){let n=0;const r=document.createElement("div");r.id="tutorial-overlay",r.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.65);z-index:9900;display:flex;align-items:center;justify-content:center;padding:16px";const o=()=>{var d,f,y;const s=t[n],l=n===t.length-1,c=Math.round((n+1)/t.length*100);r.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
  ...
  </div>`;let l="buy";const c=()=>{var p,u,b,v,h,k,x;return{name:(((p=document.getElementById("flt-name"))==null?void 0:p.value)||"").toLowerCase().trim(),club:(((u=document.getElementById("flt-club"))==null?void 0:u.value)||"").toLowerCase().trim(),country:(((b=document.getElementById("flt-country"))==null?void 0:b.value)||"").toLowerCase().trim(),job:((v=document.getElementById("flt-job"))==null?void 0:v.value)||"",rarity:((h=document.getElementById("flt-rarity"))==null?void 0:h.value)||"",note1:parseInt((k=document.getElementById("flt-note1"))==null?void 0:k.value)||0,note2:parseInt((x=document.getElementById("flt-note2"))==null?void 0:x.value)||0}};function d(p){const u=c();return p.filter(b=>{var N,ne,ie;const v=(N=b.card)==null?void 0:N.player;if(!v)return!1;const h=`${v.firstname} ${v.surname_real}`.toLowerCase(),k=(((ne=v.clubs)==null?void 0:ne.encoded_name)||"").toLowerCase(),x=(v.country_code||"").toLowerCase(),C=((ie=b.card)==null?void 0:ie.evolution_bonus)||0,E=Bt(v,v.job,C),G=v.job2?Bt(v,v.job2,C):0;return!(u.name&&!h.includes(u.name)||u.club&&!k.includes(u.club)||u.country&&!x.includes(u.country)||u.job&&v.job!==u.job||u.rarity&&v.rarity!==u.rarity||u.note1&&E<u.note1||u.note2&&G<u.note2)})}function f(p){var G,N,ne,ie;const u=(G=p.card)==null?void 0:G.player;if(!u)return"";const b=((N=p.card)==null?void 0:N.evolution_bonus)||0,v=Bt(u,u.job,b),h=u.job2?Bt(u,u.job2,b):0,k=(i.profile.credits||0)>=p.price,x=u.country_code?`https://flagsapi.com/${u.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,C=ni[u.job]||"#bbb",E=u.job2?ni[u.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden">
      <!-- Drapeau -->
    ...
    </div>`}function y(p){var G,N,ne,ie;const u=(G=p.card)==null?void 0:G.player;if(!u)return"";const b=((N=p.card)==null?void 0:N.evolution_bonus)||0,v=Bt(u,u.job,b),h=u.job2?Bt(u,u.job2,b):0,k=p.status==="sold",x=u.country_code?`https://flagsapi.com/${u.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,C=ni[u.job]||"#bbb",E=u.job2?ni[u.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${k?"opacity:0.7":""}">
      ${x?`<img src="${x}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      ${(ne=u.clubs)!=null&&ne.logo_url?`<img src="${u.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${C};display:flex;align-items:center;justify-content:center">
    ...
    </div>`}function m(){const p=document.getElementById("mkt-content"),u=document.getElementById("mkt-filters");if(p){if(u.style.display=l==="buy"?"flex":"none",l==="buy"){const b=d(a);p.innerHTML=b.length?b.map(f).join(""):'<div style="text-align:center;color:#aaa;padding:40px">Aucune carte trouvée.</div>'}else{const b=s.filter(h=>h.status==="active").sort((h,k)=>new Date(k.listed_at)-new Date(h.listed_at)),v=s.filter(h=>h.status==="sold").sort((h,k)=>new Date(k.sold_at||k.listed_at)-new Date(h.sold_at||h.listed_at));p.innerHTML=(b.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:4px 0 6px;text-transform:uppercase;letter-spacing:1px">🟢 En vente (${b.length})</div>`+b.map(y).join(""):"")+(v.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:12px 0 6px;text-transform:uppercase;letter-spacing:1px">✅ Ventes réussies (${v.length})</div>`+v.map(y).join(""):"")+(!b.length&&!v.length?'<div style="text-align:center;color:#aaa;padding:40px">Aucune vente pour le moment.</div>':"")}p.querySelectorAll("[data-buy]").forEach(b=>b.addEventListener("click",()=>Tr(b.dataset.buy,a,e,t))),p.querySelectorAll("[data-cancel]").forEach(b=>b.addEventListener("click",()=>Ir(b.dataset.cancel,e,t)))}}e.querySelectorAll(".mkt-tab").forEach(p=>{p.addEventListener("click",()=>{l=p.dataset.tab,e.querySelectorAll(".mkt-tab").forEach(u=>{const b=u===p;u.style.background=b?"var(--green)":"#fff",u.style.color=b?"#fff":"var(--gray-600)",u.style.borderColor=b?"var(--green)":"var(--gray-200)"}),m()})});let g;["flt-name","flt-club","flt-country","flt-job","flt-rarity","flt-note1","flt-note2"].forEach(p=>{var u;(u=document.getElementById(p))==null||u.addEventListener("input",()=>{clearTimeout(g),g=setTimeout(m,250)})}),m()}async function Tr(e,t,i,n){const{state:r,toast:o,refreshProfile:a}=n,s=t.find(d=>d.id===e);if(!s)return;const l=s.price;if((r.profile.credits||0)<l){o("Crédits insuffisants","error");return}zr(s,async()=>{const{error:d}=await _.rpc("buy_market_card",{p_listing_id:e,p_buyer_id:r.profile.id});if(d){o("Erreur achat : "+d.message,"error");return}await a();const f=document.getElementById("nav-credits");f&&(f.textContent=`💰 ${(r.profile.credits||0).toLocaleString("fr")}`),o("✅ Carte achetée !","success"),await Pi(i,n)})}function zr(e,t){var a;const i=(a=e.card)==null?void 0:a.player,n=i?`${i.firstname} ${i.surname_real}`:"cette carte",r=document.createElement("div");r.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",r.innerHTML=`
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center">
      ...
      `).join(""):'<div style="text-align:center;color:var(--gray-600);padding:40px">Aucun manager.</div>'}else{const b=a||[];g.innerHTML=b.length>0?b.map((v,h)=>{const k=Mi(v.mmr??1e3),x=(v.ranked_wins||0)+(v.ranked_losses||0)+(v.ranked_draws||0),C=x>0?Math.round((v.ranked_wins||0)/x*100):0,E=v.id===i.profile.id,G=(v.placement_matches||0)<10;return`
  ...
  <div class="page">
    ...
    </div>`,(C=document.getElementById("ranked-back"))==null||C.addEventListener("click",()=>n("home"));return}const l=s.mmr??1e3,c=s.mmr_deviation??350,d=s.mmr_volatility??.06,f=s.placement_matches??0,y=f<10,m=Math.max(0,10-f),g=Mi(l),p=jn(l),u=mi.findIndex(ne=>ne.id===g.id),b=mi[u+1]||null,v={bronze:"linear-gradient(160deg,#3d1c00,#7a3e00)",silver:"linear-gradient(160deg,#1a1a2e,#3a3a5e)",gold:"linear-gradient(160deg,#1a1200,#4a3500)",platinum:"linear-gradient(160deg,#001a20,#003040)",diamond:"linear-gradient(160deg,#001030,#1a2860)",master:"linear-gradient(160deg,#1a0030,#3d0070)"},h=(s.ranked_wins||0)+(s.ranked_losses||0)+(s.ranked_draws||0),k=h>0?Math.round((s.ranked_wins||0)/h*100):0,x=mi.map(ne=>`
  ...
  <div style="min-height:100%;background:${v[g.id]};padding:16px;overflow-y:auto;display:flex;flex-direction:column;gap:16px">
    ...
    <div style="display:flex;flex-direction:column;gap:10px;margin-top:auto;padding-top:8px">
      ...
      away:users!away_id(pseudo,club_name)`).or(`home_id.eq.${r.id},away_id.eq.${r.id}`).order("created_at",{ascending:!1}).limit(50),a={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},s=(o||[]).filter(d=>d.status==="finished"),l=(o||[]).filter(d=>d.status==="in_progress");function c(d){const f=d.home_id===r.id;f?d.home_score:d.away_score,f?d.away_score:d.home_score;const y=d.winner_id===r.id,m=d.home_score===d.away_score&&d.status==="finished",g=d.status!=="finished"?"…":m?"N":y?"V":"D",p=d.status!=="finished"||m?"#888":y?"#1A6B3C":"#c0392b";let u=a[d.mode]||d.mode;d.away_id===null&&!u.startsWith("IA")&&(u="IA");const v=d.home_id===r.id?d.away:d.home;let h;d.away_id===null?h=u:v?h=`${v.club_name||v.pseudo} (${v.pseudo})`:h="Adversaire";let k="";d.status==="in_progress"&&Date.now()-new Date(d.created_at).getTime()>3600*1e3&&(k=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const x=new Date(d.created_at),C=x.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+x.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),E=d.status==="finished"?`${d.home_score} - ${d.away_score}`:`${d.home_score||0} - ${d.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
