...
import{s as w,F as si,r as Ae,j as St,l as Gi,m as yt,h as di,n as gn,o as mn,T as Yt,k as xn,p as yn,b as bn}from"./formation-links-D0diUT_D.js";const hn="/";function Li(e,{navigate:t,toast:i}){let n="login";const a=()=>{var r,s,d,l,c,u;const o=n==="login";e.innerHTML=`
    <div style="
    ...
    </style>`,(r=document.getElementById("tab-login-btn"))==null||r.addEventListener("click",()=>{n="login",a()}),(s=document.getElementById("tab-reg-btn"))==null||s.addEventListener("click",()=>{n="register",a()}),o?((d=document.getElementById("login-password"))==null||d.addEventListener("keydown",g=>{var x;g.key==="Enter"&&((x=document.getElementById("login-btn"))==null||x.click())}),(l=document.getElementById("login-btn"))==null||l.addEventListener("click",async()=>{const g=document.getElementById("login-email").value.trim(),x=document.getElementById("login-password").value,f=document.getElementById("login-error");if(f.textContent="",!g||!x){f.textContent="Remplissez tous les champs.";return}const p=document.getElementById("login-btn");p.textContent="⏳ Connexion…",p.disabled=!0;const{error:m}=await w.auth.signInWithPassword({email:g,password:x});if(p.textContent="⚽ Se connecter",p.disabled=!1,m){f.textContent=m.message.includes("Invalid")?"Email ou mot de passe incorrect.":m.message;return}window.location.reload()})):((c=document.getElementById("reg-confirm"))==null||c.addEventListener("keydown",g=>{var x;g.key==="Enter"&&((x=document.getElementById("reg-btn"))==null||x.click())}),(u=document.getElementById("reg-btn"))==null||u.addEventListener("click",async()=>{const g=document.getElementById("reg-email").value.trim(),x=document.getElementById("reg-password").value,f=document.getElementById("reg-confirm").value,p=document.getElementById("reg-error");if(p.textContent="",!g||!x||!f){p.textContent="Remplissez tous les champs.";return}if(x.length<6){p.textContent="Mot de passe trop court (min. 6 caractères).";return}if(x!==f){p.textContent="Les mots de passe ne correspondent pas.";return}const m=document.getElementById("reg-btn");m.textContent="⏳ Création…",m.disabled=!0;const{error:b}=await w.auth.signUp({email:g,password:x});if(m.textContent="🚀 Créer mon compte",m.disabled=!1,b){p.textContent=b.message;return}i("Compte créé ! Connecte-toi pour commencer.","success",4e3),n="login",a(),setTimeout(()=>{const v=document.getElementById("login-email");v&&(v.value=g)},50)}))};a()}function vn(e,{state:t,navigate:i,toast:n,refreshProfile:a}){let o="#1A6B3C",r="#D4A017";e.innerHTML=`
  ...
  `;function s(){var f;const l=document.getElementById("logo-preview"),c=document.getElementById("logo-initials"),u=((f=document.getElementById("setup-club"))==null?void 0:f.value)||"MW",g=u.trim().split(" ").filter(Boolean),x=g.length>=2?(g[0][0]+g[1][0]).toUpperCase():u.slice(0,2).toUpperCase();l&&(l.style.background=r,l.style.borderColor=o),c&&(c.textContent=x,c.style.color=o)}document.getElementById("color1").addEventListener("input",l=>{o=l.target.value,document.getElementById("swatch1").style.background=o,s()}),document.getElementById("color2").addEventListener("input",l=>{r=l.target.value,document.getElementById("swatch2").style.background=r,s()});function d(l){document.querySelectorAll(".setup-step").forEach(c=>c.classList.remove("active")),document.getElementById(`step-${l}`).classList.add("active"),document.getElementById("step-num").textContent=l,document.getElementById("progress-fill").style.width=`${Math.round(l/3*100)}%`,l===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const l=document.getElementById("setup-pseudo").value.trim(),c=document.getElementById("step1-error");if(c.textContent="",l.length<3){c.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:u}=await w.from("users").select("id").eq("pseudo",l).maybeSingle();if(u){c.textContent="Ce pseudo est déjà pris.";return}d(2)}),document.getElementById("step2-back").addEventListener("click",()=>d(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const l=document.getElementById("setup-club").value.trim(),c=document.getElementById("step2-error");if(c.textContent="",l.length<2){c.textContent="Nom trop court (min. 2 caractères).";return}const{data:u}=await w.from("users").select("id").eq("club_name",l).maybeSingle();if(u){c.textContent="Ce nom de club est déjà pris.";return}d(3)}),document.getElementById("step3-back").addEventListener("click",()=>d(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const l=document.getElementById("setup-pseudo").value.trim(),c=document.getElementById("setup-club").value.trim(),u=document.getElementById("step3-error"),g=document.getElementById("step3-finish");u.textContent="",g.disabled=!0,g.textContent="Création en cours…";try{const{error:x}=await w.from("users").insert({id:t.user.id,pseudo:l,club_name:c,club_color1:o,club_color2:r,credits:1e4});if(x)throw x;await wn(t.user.id),await a(),n(`Bienvenue ${l} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(x){u.textContent=x.message,g.disabled=!1,g.textContent="🚀 Créer mon profil !"}})}async function wn(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await w.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const _n="modulepreload",kn=function(e){return"/"+e},zi={},Ri=function(t,i,n){let a=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const r=document.querySelector("meta[property=csp-nonce]"),s=(r==null?void 0:r.nonce)||(r==null?void 0:r.getAttribute("nonce"));a=Promise.allSettled(i.map(d=>{if(d=kn(d),d in zi)return;zi[d]=!0;const l=d.endsWith(".css"),c=l?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${c}`))return;const u=document.createElement("link");if(u.rel=l?"stylesheet":_n,l||(u.as="script"),u.crossOrigin="",u.href=d,s&&u.setAttribute("nonce",s),document.head.appendChild(u),l)return new Promise((g,x)=>{u.addEventListener("load",g),u.addEventListener("error",()=>x(new Error(`Unable to preload CSS for ${d}`)))})}))}function o(r){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=r,window.dispatchEvent(s),!s.defaultPrevented)throw r}return a.then(r=>{for(const s of r||[])s.status==="rejected"&&o(s.reason);return t().catch(o)})},Et="#1A6B3C",Lt="#cc2222",$n="#D4A017",Ti="#888";async function En(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    </div>`,await Ni(i,n,t),document.getElementById("btn-add-friend").addEventListener("click",()=>zn(i,n)),document.getElementById("btn-accept-friend").addEventListener("click",()=>Oi(i,n,null,t))}async function Ni(e,t,i={}){const{navigate:n}=i,a=e.user.id,{data:o,error:r}=await w.from("friendships").select("id, requester_id, addressee_id").eq("status","accepted").or(`requester_id.eq.${a},addressee_id.eq.${a}`),{count:s}=await w.from("friendships").select("id",{count:"exact",head:!0}).eq("addressee_id",a).eq("status","pending"),d=document.getElementById("pending-badge");d&&(s>0?(d.style.display="flex",d.textContent=s):d.style.display="none");const l=document.getElementById("friends-list");if(!l)return;if(r){console.error("[Friends] Erreur:",r),l.innerHTML=`<div style="color:${Lt};text-align:center;padding:16px">Erreur chargement : ${r.message}</div>`;return}const c=(o||[]).map(x=>x.requester_id===a?x.addressee_id:x.requester_id);let u={};if(c.length){const{data:x}=await w.from("users").select("id, pseudo, club_name, last_seen_at, club_color1, club_color2").in("id",c);(x||[]).forEach(f=>{u[f.id]=f})}const g=(o||[]).map(x=>({friendshipId:x.id,friend:u[x.requester_id===a?x.addressee_id:x.requester_id]||{pseudo:"?"}}));if(!g.length){l.innerHTML=`
    ...
    </div>`}function zn(e,t){const i=li();i.innerHTML=`
    <div class="popup-box">
    ...
    ${ci()}`,document.body.appendChild(l);const u=()=>l.remove();l.querySelector("#pending-close").addEventListener("click",u),l.addEventListener("click",g=>{g.target===l&&u()}),l.querySelectorAll("[data-accept]").forEach(g=>{g.addEventListener("click",async()=>{const{error:x}=await w.from("friendships").update({status:"accepted"}).eq("id",g.dataset.accept);if(x){t("Erreur : "+x.message,"error");return}g.closest("div[style]").remove(),t("✅ Ami accepté !","success"),Ni(e,t,n),i&&i()})}),l.querySelectorAll("[data-decline]").forEach(g=>{g.addEventListener("click",async()=>{await w.from("friendships").delete().eq("id",g.dataset.decline),g.closest("div[style]").remove(),t("Demande refusée","info"),i&&i()})})}async function Tn(e,t,i){const n=e.user.id,[a,o]=[n,t].sort(),r=n===a,{data:s}=await w.from("friend_match_stats").select("*").eq("player1_id",a).eq("player2_id",o).single(),d=e.profile.club_name||e.profile.pseudo||"Moi",l=s||{},c=r?l.wins_p1||0:l.wins_p2||0,u=r?l.wins_p2||0:l.wins_p1||0,g=l.draws||0,x=r?l.goals_p1||0:l.goals_p2||0,f=r?l.goals_p2||0:l.goals_p1||0,p=r?l.gc_used_p1||0:l.gc_used_p2||0,m=r?l.gc_used_p2||0:l.gc_used_p1||0,b=l.matches_total||0,v=($,y,C,E=Et,P=Lt)=>`
    ...
    </style>`}const In="2026.07.12-1516";async function Ii(e,{state:t,navigate:i,toast:n}){var o,r;const a=t.profile;a&&(e.innerHTML=`
  <style>
    .home-dark {
      min-height: 100%;
  ...
  <div class="home-dark" id="home-dark">
    ...
    ${Ae(e,{width:140})}
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),l=(s||[]).filter(T=>T.card_type==="player"&&T.player),c=(s||[]).filter(T=>T.card_type==="game_changer"),u=(s||[]).filter(T=>T.card_type==="formation"),g=(s||[]).filter(T=>T.card_type==="stadium"),{data:x}=await w.from("gc_definitions").select("name,gc_type,color,effect,image_url"),f={};(x||[]).forEach(T=>{f[T.name]=T});const{data:p}=await w.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),m={};(p||[]).forEach(T=>{m[T.id]=T}),g.forEach(T=>{T.stadium_def&&(m[T.stadium_id]=T.stadium_def)});const b=Object.keys(si),v=Object.keys(Se),h={};l.forEach(T=>{const S=T.player.id;h[S]=(h[S]||0)+1}),new Set(Object.keys(h).map(T=>String(T)));const $=new Set(u.map(T=>T.formation)),y=new Set(c.map(T=>T.gc_type));let C="player",E="Tous",P="",N=!1;function ee(){return[...l].sort((T,S)=>{const R=jt.indexOf(T.player.job),H=jt.indexOf(S.player.job);return R!==H?R-H:(T.player.surname_real||"").localeCompare(S.player.surname_real||"")})}function ie(){return[...d||[]].sort((T,S)=>{const R=jt.indexOf(T.job),H=jt.indexOf(S.job);return R!==H?R-H:(T.surname_real||"").localeCompare(S.surname_real||"")})}function le(){return ee().filter(T=>{const S=T.player,R=E==="Tous"||S.job===E,H=!P||`${S.firstname} ${S.surname_real}`.toLowerCase().includes(P);return R&&H})}function j(){return ie().filter(T=>{const S=E==="Tous"||T.job===E,R=!P||`${T.firstname} ${T.surname_real}`.toLowerCase().includes(P);return S&&R})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
  ...
  </div>`;function L(){const T=document.getElementById("col-filters");T&&(C==="player"?(T.innerHTML=`
          ...
          ${Fn.map(S=>`
            <button class="filter-btn" data-job="${S}"
              style="flex-shrink:0;padding:4px 12px;border-radius:20px;font-size:12px;font-weight:600;cursor:pointer;
                border:1.5px solid ${S===E?"var(--green)":"var(--gray-200)"};
                background:${S===E?"var(--green)":"#fff"};
                color:${S===E?"#fff":"var(--gray-600)"}">
              ${S}
            </button>`).join("")}
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",S=>{P=S.target.value.toLowerCase(),k()}),e.querySelectorAll(".filter-btn").forEach(S=>{S.addEventListener("click",()=>{E=S.dataset.job,L(),k()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{N=!N,L(),k()})):(T.innerHTML=`
        <div style="display:flex;justify-content:flex-end">
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{N=!N,L(),k()})))}function k(){const T=document.getElementById("col-grid");T&&(C==="player"?Le(T):C==="formation"?K(T):C==="stadium"?V(T):Q(T))}function F(T,S,R,H,U){const B=document.getElementById("col-grid"),O=document.getElementById("col-big");if(!B||!O)return;var $e=0;function _e(){const we=window.innerWidth>=768,me=document.getElementById("col-big"),Ee=document.getElementById("col-grid");we&&me&&(me.style.minHeight="420px",me.style.flex="1 1 auto"),we&&Ee&&(Ee.style.height=Math.round(310*.76+16)+"px",Ee.style.flexShrink="0",Ee.style.overflowX="auto",Ee.style.overflowY="hidden",Ee.style.alignItems="center",Ee.style.padding="8px 16px"),O.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+S(T[$e])+"</div>";var ke=O.querySelector("[data-card-id],[data-form-id],[data-gc-id]");ke&&ke.addEventListener("click",function(){H(T[$e])}),requestAnimationFrame(function(){var he=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!he||!O)){var Ce=O.clientHeight-8,je=O.clientWidth-24,Fe=he.offsetHeight,_=he.offsetWidth;if(Fe>0&&_>0&&Ce>40){var z=we?2.2:1.6,A=Math.min(Ce/Fe,je/_,z);he.style.transform="scale("+A.toFixed(3)+")",he.style.transformOrigin="top center"}}}),B.innerHTML=T.map(function(he,Ce){var je=Ce===$e,Fe="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+(je?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+Ce+'" style="'+Fe+'">'+R(he,je)+"</div>"}).join(""),B.querySelectorAll(".col-mini-item").forEach(function(he){he.addEventListener("click",function(){$e=Number(he.dataset.idx),_e(),he.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}_e()}function re(T){var S=window.innerWidth>=768?.76:.54,R;if(!T||T._fake){var H=T?T.player:null;if(!H)return"";R=Mi(H)}else R=Jt(T,"");return'<div style="display:inline-block;zoom:'+S+';pointer-events:none;line-height:0">'+R+"</div>"}function pe(T,S,R){S=S||100,R=R||140;var H=St[T]||{},U={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},B=Math.max(3,Math.round(S*.06)),O=Object.entries(H).map(function(_e){var we=_e[0],me=_e[1],Ee=we.replace(/\d+$/,""),ke=U[Ee]||"#888",he=Math.round(me.x*S),Ce=Math.round(me.y*R);return'<circle cx="'+he+'" cy="'+Ce+'" r="'+B+'" fill="'+ke+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),$e=Math.max(1,Math.round(S/50));return'<svg viewBox="0 0 '+S+" "+R+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+S+'" height="'+R+'" fill="#1A6B3C"/><rect x="'+Math.round(S*.2)+'" y="'+Math.round(R*.02)+'" width="'+Math.round(S*.6)+'" height="'+Math.round(R*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+$e+'"/><line x1="0" y1="'+Math.round(R*.5)+'" x2="'+S+'" y2="'+Math.round(R*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+$e+'"/><ellipse cx="'+Math.round(S*.5)+'" cy="'+Math.round(R*.5)+'" rx="'+Math.round(S*.18)+'" ry="'+Math.round(R*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+$e+'"/><rect x="'+Math.round(S*.2)+'" y="'+Math.round(R*.82)+'" width="'+Math.round(S*.6)+'" height="'+Math.round(R*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+$e+'"/>'+O+"</svg>"}function de(T,S,R){var H=R>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+R+"</div>":"",U=S?'data-form-id="'+S.id+'"':"",B=T.length>7?14:T.length>5?16:19,O=!!S;return"<div "+U+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(O?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(O?"":"filter:grayscale(1);opacity:0.5")+'">'+H+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(O?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+B+"px;font-weight:900;color:"+(O?"#1A6B3C":"#aaa")+';line-height:1">'+T+'</div></div><div style="flex:1;overflow:hidden;background:'+(O?"#1A6B3C":"#ccc")+'">'+pe(T,140,220)+"</div></div>"}function se(T,S){var R=window.innerWidth>=768?.76:.54,H=140,U=305,B=Math.round(U*.22),O=U-B,$e=T.length>7?10:13,_e=pe(T,H,O),we=S?"1.5px solid #2a7a40":"1px solid #ddd",me=S?"":"filter:grayscale(1);opacity:0.45;",Ee=S?"#1A6B3C":"#bbb",ke="#fff";return'<div style="display:inline-block;zoom:'+R+';line-height:0;pointer-events:none"><div style="width:'+H+"px;height:"+U+"px;border-radius:6px;border:"+we+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+me+'"><div style="height:'+B+"px;background:"+Ee+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+$e+"px;font-weight:900;color:"+ke+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(H-4)+'px">'+T+'</span></div><div style="height:'+O+'px;overflow:hidden;flex-shrink:0">'+_e+"</div></div></div>"}function Le(T){if(N){const S=j();if(!S.length){T.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const R=S.map(H=>l.find(U=>U.player.id===H.id)||{_fake:!0,player:H,id:"fake-"+H.id});F(R,H=>H._fake?Mi(H.player):Jt(H,""),H=>H._fake?re({player:H.player,id:"x",_fake:!0}):re(H),H=>{H._fake||Ci(H,l,h,t)})}else{const S=le();if(!S.length){T.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const R={};S.forEach(U=>{const B=U.player.id;R[B]||(R[B]=[]),R[B].push(U)});const H=Object.values(R).map(U=>Gn(U));F(H,U=>{const B=h[U.player.id]||1,O=B>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${B}</div>`:"",_e=l.filter(we=>we.player.id===U.player.id&&we.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Jt(U,O+_e)},U=>re(U),U=>Ci(U,l,h,t))}}function K(T){const S=N?b:[...$];if(!S.length){T.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const R=S.map(H=>({formation:H,card:u.find(U=>U.formation===H)||null,owned:$.has(H)}));F(R,({formation:H,card:U,owned:B})=>de(H,B?U:null,B?u.filter(O=>O.formation===H).length:0),({formation:H,owned:U})=>se(H,U),({card:H,owned:U})=>{U&&H&&Kn(H,u,t,o)})}function Q(T){const S=Object.keys(f),R=N?S.length?S:v:[...y];if(!R.length){T.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const H=R.map(U=>({type:U,gc:Se[U]||{icon:"⚡",desc:""},def:f[U]||null,owned:y.has(U),card:c.find(B=>B.gc_type===U)||null}));F(H,({type:U,gc:B,def:O,owned:$e,card:_e})=>{var z;const we=$e?c.filter(A=>A.gc_type===U).length:0,me=we>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${we}</div>`:"",Ee=(O==null?void 0:O.gc_type)==="ultra_game_changer",ke={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},he={purple:"#b06ce0",light_blue:"#00d4ef"},Ce=ke[O==null?void 0:O.color]||ke.purple,je=he[O==null?void 0:O.color]||he.purple,Fe=(O==null?void 0:O.effect)||B.desc||"",_=O!=null&&O.image_url?`/icons/${O.image_url}`:((z=O==null?void 0:O.club)==null?void 0:z.logo_url)||(O!=null&&O.country_code?`https://flagsapi.com/${O.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null);return $e&&_e?`<div data-gc-id="${_e.id}" data-gc-type="${U}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${je};background:${Ce};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${je}66;cursor:pointer">
          ${me}
          ...
          <div style="height:150px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
            ${_?`<img src="${_}" style="max-width:120px;max-height:120px;object-fit:contain;border-radius:6px">`:`<span style="font-size:64px">${B.icon}</span>`}
          </div>
        ...
        </div>`},({type:U,gc:B,def:O,owned:$e})=>{const _e=window.innerWidth>=768?.76:.54,we={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},me={purple:"#9b59b6",light_blue:"#00bcd4"},Ee=we[O==null?void 0:O.color]||we.purple,ke=me[O==null?void 0:O.color]||me.purple,he=O!=null&&O.image_url?`/icons/${O.image_url}`:null;return $e?`<div style="display:inline-block;zoom:${_e};line-height:0;pointer-events:none"><div style="width:140px;height:310px;border-radius:8px;background:${Ee};border:1px solid ${ke};display:flex;flex-direction:column;overflow:hidden"><div style="height:56px;background:rgba(255,255,255,0.15);display:flex;align-items:center;justify-content:center"><span style="font-size:13px;font-weight:900;color:#fff;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:130px">${U}</span></div><div style="flex:1;display:flex;align-items:center;justify-content:center">${he?`<img src="${he}" style="max-width:130px;max-height:190px;object-fit:contain">`:`<span style="font-size:48px">${B.icon}</span>`}</div><div style="height:54px;display:flex;align-items:center;justify-content:center;padding:0 4px"><span style="font-size:10px;color:rgba(255,255,255,0.7);text-align:center">${((O==null?void 0:O.effect)||B.desc||"").slice(0,30)}</span></div></div></div>`:`<div style="display:inline-block;zoom:${_e};line-height:0;pointer-events:none"><div style="width:140px;height:310px;border-radius:8px;background:#eee;border:1px solid #ddd;display:flex;flex-direction:column;align-items:center;justify-content:center;filter:grayscale(1);opacity:0.45"><span style="font-size:36px">${B.icon}</span><span style="font-size:12px;color:#aaa;margin-top:6px;text-align:center;padding:0 6px">${U}</span></div></div>`},({type:U,owned:B,def:O})=>{B&&Un(U,O,o)})}function V(T){const S="#E87722",R="/";if(!g.length){T.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Stade.<br><small>Ouvre un booster Stade !</small></div>';return}const H={};g.forEach(B=>{const O=B.stadium_id||"?";(H[O]=H[O]||[]).push(B)});const U=Object.entries(H).map(([B,O])=>({sid:B,def:m[B]||null,count:O.length,card:O[0]}));F(U,({def:B,count:O})=>{var ke,he;const $e=(B==null?void 0:B.name)||"?",_e=((ke=B==null?void 0:B.club)==null?void 0:ke.encoded_name)||(B==null?void 0:B.country_code)||"—",we=B!=null&&B.image_url?`${R}icons/${B.image_url}`:((he=B==null?void 0:B.club)==null?void 0:he.logo_url)||(B!=null&&B.country_code?`https://flagsapi.com/${B.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),me=we?`<img src="${we}" style="width:90px;height:90px;object-fit:contain;border-radius:4px" onerror="this.style.display='none'">`:'<div style="font-size:56px">🏟️</div>',Ee=O>1?`<div style="position:absolute;top:8px;right:8px;background:#333;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${O}</div>`:"";return`<div style="position:relative;width:140px;border-radius:12px;border:3px solid #c45a00;background:linear-gradient(160deg,${S},#c45a00);display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${S}66">
          ${Ee}
        ...
        </div>`},({def:B,count:O})=>{var ke,he;const $e=window.innerWidth>=768?.76:.54,_e=(B==null?void 0:B.name)||"?",we=((ke=B==null?void 0:B.club)==null?void 0:ke.encoded_name)||(B==null?void 0:B.country_code)||"—",me=B!=null&&B.image_url?`${R}icons/${B.image_url}`:((he=B==null?void 0:B.club)==null?void 0:he.logo_url)||(B!=null&&B.country_code?`https://flagsapi.com/${B.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),Ee=me?`<img src="${me}" style="width:64px;height:64px;object-fit:contain;border-radius:4px" onerror="this.style.display='none'">`:'<span style="font-size:32px">🏟️</span>';return`<div style="display:inline-block;zoom:${$e};line-height:0;pointer-events:none">
        ...
        </div>`},null)}e.querySelectorAll(".col-tab-btn").forEach(T=>{T.addEventListener("click",()=>{C=T.dataset.tab,E="Tous",P="",N=!1,e.querySelectorAll(".col-tab-btn").forEach(S=>{const R=S.dataset.tab===C;S.style.borderBottomColor=R?"var(--green)":"transparent",S.style.color=R?"var(--green)":"var(--gray-600)"}),L(),k()})}),L(),k()}function Un(e,t,i){const n=Se[e]||{icon:"⚡",desc:"Effet spécial."},a=(t==null?void 0:t.gc_type)==="ultra_game_changer",o={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},r={purple:"#b06ce0",light_blue:"#00d4ef"},s=o[t==null?void 0:t.color]||o.purple,d=r[t==null?void 0:t.color]||r.purple,l=(t==null?void 0:t.name)||e,c=(t==null?void 0:t.effect)||n.desc,u=t!=null&&t.image_url?`/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Bt=1e3;function Kn(e,t,i,n){var f,p,m;const{state:a,toast:o,closeModal:r,navigate:s,refreshProfile:d}=i,l=e.formation,c={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function u(){const b=St[l]||{},v=si[l]||[],h=290,$=360,y=20;function C(P){const N=b[P];return N?{x:N.x*h,y:N.y*$}:null}let E=`<svg width="${h}" height="${$}" viewBox="0 0 ${h} ${$}" xmlns="http://www.w3.org/2000/svg">`;for(const[P,N]of v){const ee=C(P),ie=C(N);!ee||!ie||(E+=`<line x1="${ee.x}" y1="${ee.y}" x2="${ie.x}" y2="${ie.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const P of Object.keys(b)){const N=C(P);if(!N)continue;const ee=P.replace(/\d+/,""),ie=c[ee]||"#555";E+=`<circle cx="${N.x}" cy="${N.y}" r="${y}" fill="${ie}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,E+=`<text x="${N.x}" y="${N.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${ee}</text>`}return E+="</svg>",E}const g=t.filter(b=>b.formation===l);g.length;const x=!e.is_for_sale;n(`Formation ${l}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(f=document.getElementById("direct-sell-form-btn"))==null||f.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${l} pour ${Bt.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const b=g.find(h=>!h.is_for_sale)||g[0];if(!b){o("Aucune carte à vendre","error");return}await w.from("market_listings").delete().eq("card_id",b.id),await w.from("transfer_history").delete().eq("card_id",b.id);const{error:v}=await w.from("cards").delete().eq("id",b.id);if(v){o(v.message,"error");return}await w.from("users").update({credits:(a.profile.credits||0)+Bt}).eq("id",a.profile.id),await d(),o(`+${Bt.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),r(),s("collection")}),(p=document.getElementById("market-sell-form-btn"))==null||p.addEventListener("click",async()=>{const b=parseInt(document.getElementById("sell-price-form").value);if(!b||b<1){o("Prix invalide","error");return}await w.from("cards").update({is_for_sale:!0,sale_price:b}).eq("id",e.id),await w.from("market_listings").insert({seller_id:a.profile.id,card_id:e.id,price:b}),o("Carte mise en vente sur le marché !","success"),r(),s("collection")}),(m=document.getElementById("cancel-sell-form-btn"))==null||m.addEventListener("click",async()=>{await w.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await w.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),o("Annonce retirée","success"),r(),s("collection")})}async function Ci(e,t,i,n){var re,pe,de,se,Le;const{state:a,toast:o,openModal:r,closeModal:s,navigate:d,refreshProfile:l}=n,c=e.player,u=t.filter(K=>K.player.id===c.id),g=u.length,x=e.evolution_bonus||0,f=Math.max((Number(c.note_g)||0)+(c.job==="GK"||c.job2==="GK"?x:0),(Number(c.note_d)||0)+(c.job==="DEF"||c.job2==="DEF"?x:0),(Number(c.note_m)||0)+(c.job==="MIL"||c.job2==="MIL"?x:0),(Number(c.note_a)||0)+(c.job==="ATT"||c.job2==="ATT"?x:0)),p=c.rarity||"normal",{data:m}=await w.from("sell_price_configs").select("*").eq("rarity",p).lte("note_min",f).gte("note_max",f).order("note_min",{ascending:!1}).limit(1);((re=m==null?void 0:m[0])==null?void 0:re.price)??Pn[p];const b=c.rarity!=="legende";On(c);const v=((c.rarity==="pepite"||c.rarity==="papyte")&&e.current_note!=null?e.current_note:Xt(c,c.job))+x,h=c.rarity==="pepite"||c.rarity==="papyte",$=c.job2?(h?Bn(e,Rn(c.job2)):Xt(c,c.job2))+(Xt(c,c.job2)>0?x:0):null;Wt[c.job],c.job2&&Wt[c.job2];const y=Dn[c.rarity]||"#ccc";Nn[c.country_code]||c.country_code;const C=e.evolution_bonus||0,P=v+C,N=$||0,ee=N>0?N+C:0,ie=u.map(K=>K.id);let le={};if(ie.length){const{data:K}=await w.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",ie).order("transferred_at",{ascending:!0});(K||[]).forEach(Q=>{le[Q.card_id]||(le[Q.card_id]=[]),le[Q.card_id].push(Q)})}const j=K=>{const Q=K.transferred_at?new Date(K.transferred_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"2-digit",year:"numeric"}):"",V=K.source==="booster"?"Booster":K.price?K.price.toLocaleString("fr"):"—";return`
    ...
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${u.map((K,Q)=>{const V=le[K.id]||[],T=K.is_for_sale,S=V.length?V[V.length-1]:null,H=(c.rarity==="pepite"||c.rarity==="papyte")&&K.current_note!=null?` (☆${K.current_note})`:"";return`
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;background:#f0fdf4;border-radius:10px;padding:10px 14px">
          <div>
            ...
            <div style="font-size:11px;color:#555;margin-top:2px">
              ${(e.evolution_bonus||0)>0?`+${e.evolution_bonus} appliqué${e.evolution_bonus>1?"s":""} · Note : ${P}${c.job2&&N>0?` / ${ee}`:""}`:`Note actuelle : ${P}${c.job2&&N>0?` / ${ee}`:""}`}
            </div>
          </div>
      ...
      <!-- Carte visuelle -->
      ${Ae({...c,_evolution_bonus:x},{width:160})}
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",c.note_g],["DEF",c.note_d],["MIL",c.note_m],["ATT",c.note_a]].map(([K,Q])=>{const V=Wt[K],T=K==="GK"?"#fff":V,S=K===c.job||K===c.job2,R=(Number(Q)||0)+(S&&C>0?C:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(pe=document.getElementById("close-detail"))==null||pe.addEventListener("click",s);let k=new Set;const F=()=>{const K=k.size,Q=document.getElementById("sell-action-panel");if(!Q)return;Q.style.display=K>0?"block":"none",document.getElementById("sell-selected-count").textContent=K;const V=document.getElementById("evolve-btn");V&&(V.textContent=`⬆️ Évoluer ${K>1?"(+"+K+")":""}`)};document.querySelectorAll(".expl-check").forEach(K=>{K.addEventListener("change",()=>{const Q=K.dataset.id;K.checked?k.add(Q):k.delete(Q);const V=K.closest(".exemplaire-row");V&&(V.style.borderColor=K.checked?"#1A6B3C":"#eee"),F()})}),document.querySelectorAll(".exemplaire-row").forEach(K=>{K.addEventListener("click",Q=>{if(Q.target.closest("button")||Q.target.tagName==="INPUT")return;const V=K.querySelector(".expl-check");V&&!V.disabled&&(V.checked=!V.checked,V.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(K=>{K.addEventListener("click",Q=>{Q.stopPropagation();const V=document.querySelector(`.expl-hist[data-hist="${K.dataset.idx}"]`);V&&(V.style.display=V.style.display==="none"?"flex":"none")})}),(de=document.getElementById("evolve-btn"))==null||de.addEventListener("click",async()=>{if(g<=1)return;const K=[...k];if(!K.length)return;if(k.has(e.id)){const U=document.createElement("div");U.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",U.innerHTML=`
        ...
        <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center;box-shadow:0 16px 48px rgba(0,0,0,0.3)">
          ...
          <div style="background:#f0fdf4;border-radius:10px;padding:12px;margin-bottom:16px;font-size:13px;color:#333">
            ...
            📈 Note : <strong>${v+(e.evolution_bonus||0)}</strong> → <strong style="color:#1A6B3C">${v+(e.evolution_bonus||0)+Q.length*V}</strong>
        ...
        </div>`,document.body.appendChild(B),B.querySelector("#sac-cancel").addEventListener("click",()=>{B.remove(),U(!1)}),B.querySelector("#sac-ok").addEventListener("click",()=>{B.remove(),U(!0)}),B.addEventListener("click",O=>{O.target===B&&(B.remove(),U(!1))})}))return;if(Q.length){await w.from("market_listings").delete().in("card_id",Q),await w.from("deck_cards").delete().in("card_id",Q),await w.from("transfer_history").delete().in("card_id",Q),await w.from("decks").update({stadium_card_id:null}).in("stadium_card_id",Q);const{error:U}=await w.from("cards").delete().in("id",Q);if(U){o("Erreur suppression : "+U.message,"error");return}}const S=(e.evolution_bonus||0)+Q.length*V,{error:R}=await w.from("cards").update({evolution_bonus:S}).eq("id",e.id);if(R){o("Erreur évolution : "+R.message,"error");return}const H=v+S;o(`⬆️ ${c.firstname} ${c.surname_real} : note ${v+e.evolution_bonus||v} → ${H}${Q.length?` · ${Q.length} copie${Q.length>1?"s":""} sacrifiée${Q.length>1?"s":""}`:""} !`,"success",4e3),s(),d("collection")}),(se=document.getElementById("market-sell-btn"))==null||se.addEventListener("click",async()=>{var R;const K=[...k];if(!K.length){o("Sélectionne au moins un exemplaire","warning");return}const Q=parseInt((R=document.getElementById("sell-market-price"))==null?void 0:R.value);if(!Q||Q<1){o("Prix invalide","error");return}const{error:V}=await w.from("cards").update({is_for_sale:!0,sale_price:Q}).in("id",K);if(V){o(V.message,"error");return}const T=K.map(H=>({seller_id:a.profile.id,card_id:H,price:Q,status:"active"})),{error:S}=await w.from("market_listings").insert(T);S&&console.warn("[Market] insert listings:",S.message),o(`${K.length} carte${K.length>1?"s":""} mise${K.length>1?"s":""} en vente à ${Q.toLocaleString("fr")} cr. chacune !`,"success"),s(),d("collection")}),(Le=document.getElementById("cancel-sell-btn"))==null||Le.addEventListener("click",async()=>{await w.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await w.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),o("Annonce retirée","success"),s(),d("collection")})}const Pt={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}};async function ei(e,t){const{state:i,navigate:n,toast:a}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await w.from("decks").select("id,name,formation_card_id").eq("owner_id",i.profile.id).order("created_at",{ascending:!1});e.innerHTML=`
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
    ...
    <div class="page-body">
      ...
      stadium_def:stadium_definitions(id, name, club_id, country_code, image_url,
        club:clubs(encoded_name, logo_url))`).eq("owner_id",n.profile.id).eq("card_type","stadium"),d=(r||[]).filter(b=>b.card_type==="player"&&b.player),l=(r||[]).filter(b=>b.card_type==="formation"),c=(s||[]).filter(b=>b.card_type==="stadium"),u=[...new Set(c.map(b=>b.stadium_id).filter(Boolean))];let g={};if(c.forEach(b=>{b.stadium_def&&b.stadium_id&&(g[b.stadium_id]=b.stadium_def)}),u.length&&Object.keys(g).length<u.length){const{data:b}=await w.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",u);(b||[]).forEach(v=>{g[v.id]=v})}const x=l.map(b=>b.formation).filter(Boolean),{data:f}=await w.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",e);let p=o.formation||"4-4-2";x.length>0&&!x.includes(p)&&(p=x[0]);const m={deckId:e,name:o.name,formation:p,formationCardId:o.formation_card_id,stadiumCardId:o.stadium_card_id||null,slots:{},subs:[],playerCards:d,formationCards:l,stadiumCards:c,stadDefMap:g,availableFormations:x};(f||[]).forEach(b=>{b.is_starter?m.slots[b.position]=b.card_id:m.subs.includes(b.card_id)||m.subs.push(b.card_id)}),Ze(t,m,i)}function Ze(e,t,i){var f,p;const{navigate:n}=i;Pt[t.formation];const a=Bi(t.formation),o=a.filter(m=>t.slots[m]).length,r=t.availableFormations.length>0?t.availableFormations:Object.keys(Pt),s=(f=t.stadiumCards)==null?void 0:f.find(m=>m.id===t.stadiumCardId),d=s&&((p=t.stadDefMap)==null?void 0:p[s.stadium_id])||null,l=t.subs.map(m=>t.playerCards.find(b=>b.id===m)).filter(Boolean);[...Object.values(t.slots),...t.subs],e.innerHTML=`
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
      ...
      <div style="display:flex;gap:0;min-height:600px">
        ...
        <div style="width:105px;flex-shrink:0;background:rgba(0,0,0,0.3);display:flex;flex-direction:column;align-items:center;padding:12px 6px;gap:8px;border-right:1px solid rgba(255,255,255,0.1)">
          ...
          <div style="display:flex;flex-direction:column;gap:6px;align-items:center" id="subs-list">
            ${l.map(m=>{const b={...m.player,_evolution_bonus:m.evolution_bonus||0};return`<div style="position:relative;flex-shrink:0;overflow:visible">
    ...
    <div class="page-body" style="padding-bottom:20px">
  ...
  </div>`;const c=window.innerWidth>=900,u=e.querySelector(".deck-pc-layout"),g=e.querySelector(".deck-mobile-layout");u&&(u.style.display=c?"block":"none"),g&&(g.style.display=c?"none":"block"),Vn(e,t,a,i),e.querySelectorAll("#builder-back").forEach(m=>m.addEventListener("click",()=>n("decks")));const x=()=>{const{openModal:m,closeModal:b}=i,v=`<div style="display:flex;flex-wrap:wrap;gap:8px;padding:8px">
    ...
    </div>`;m("⚽ Choisir une formation",v),document.querySelectorAll("#modal-body [data-forma]").forEach(h=>{h.addEventListener("click",()=>{t.formation=h.dataset.forma;const $=Bi(t.formation),y={};$.forEach(C=>{t.slots[C]&&(y[C]=t.slots[C])}),t.slots=y,b(),Ze(e,t,i)})})};e.querySelectorAll("#formation-mobile-btn, #formation-pc-btn").forEach(m=>m.addEventListener("click",x)),e.querySelectorAll("#add-stad-btn-pc, #add-stad-btn").forEach(m=>m.addEventListener("click",()=>Yn(t,e,i))),e.querySelectorAll("#save-deck").forEach(m=>m.addEventListener("click",()=>Jn(t,i))),e.querySelectorAll("[data-remove-sub]").forEach(m=>{m.addEventListener("click",()=>{t.subs=t.subs.filter(b=>b!==m.dataset.removeSub),Ze(e,t,i)})}),e.querySelectorAll("#add-sub-btn").forEach(m=>m.addEventListener("click",()=>Xn(t,e,i)))}function Vn(e,t,i,n){var h,$;const a=window.innerWidth>=900,o=e.querySelector(a?"#deck-field-pc":"#deck-field-mobile");if(!o)return;const r=(h=t.stadiumCards)==null?void 0:h.find(y=>y.id===t.stadiumCardId),s=r&&(($=t.stadDefMap)==null?void 0:$[r.stadium_id])||null,d=St[t.formation]||{},l=Gi(t.formation)||[],c={};for(const y of i){const C=t.slots[y],E=C?t.playerCards.find(P=>P.id===C):null;E!=null&&E.player?c[y]={...E.player,_evolution_bonus:E.evolution_bonus||0,face:E.player.face||null}:c[y]=null}const u=window.innerWidth>=900,g=u?window.innerWidth-280:window.innerWidth-20,x=u?Math.min(g,860):g,f=Math.round(u?x*.82:x*.85),p=u?84:44;let m="";for(const[y,C]of l){const E=d[y],P=d[C];if(!E||!P)continue;const N=E.x*x,ee=Math.round(.03*f+E.y*.85*f),ie=P.x*x,le=Math.round(.03*f+P.y*.85*f),j=c[y],L=c[C],k=yt(j,L);k==="#ff3333"||k==="#cc2222"?m+=`<line x1="${N.toFixed(1)}" y1="${ee.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${le.toFixed(1)}" stroke="${k}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(m+=`<line x1="${N.toFixed(1)}" y1="${ee.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${le.toFixed(1)}" stroke="${k}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,m+=`<line x1="${N.toFixed(1)}" y1="${ee.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${le.toFixed(1)}" stroke="${k}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let b="";const v=Math.round(p*657/507);for(const y of i){const C=d[y];if(!C)continue;const E=c[y],P=C.x*x;C.y*f;const N=Math.round(.03*f+C.y*(.85*f)),ee=Math.round(P-p/2),ie=Math.round(N-v/2);if(E){const le=y.replace(/\d+/,""),j=s&&(s.club_id&&String(E.club_id)===String(s.club_id)||s.country_code&&E.country_code===s.country_code),L=Ae({...E,_evolution_bonus:E._evolution_bonus||0},{width:p,showStad:!0,stadDef:s,role:le});b+=`<div style="position:absolute;left:${ee}px;top:${ie}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${y}">
      ...
      </div>`}else{const le=y.replace(/\d+/,"");b+=`<div style="position:absolute;left:${ee}px;top:${ie}px;width:${p}px;height:${v}px;
    ...
    <div style="position:relative;width:${x}px;height:${f}px;margin:0 auto">
      ...
      ${x.length>0?'<div style="display:flex;flex-wrap:wrap;gap:8px;justify-content:center">'+x.map(h=>{const $={...h.player,_evolution_bonus:h.evolution_bonus||0};return`<div class="player-option" data-card-id="${h.id}" style="cursor:pointer">
          ${Ae($,{width:100,showStad:!0,stadDef:d,role:r})}
    ...
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(b=document.getElementById("close-selector"))==null||b.addEventListener("click",o),(v=document.getElementById("remove-player"))==null||v.addEventListener("click",()=>{delete t.slots[e],o(),Ze(i,t,n)}),document.querySelectorAll(".player-option").forEach(h=>{h.addEventListener("click",()=>{t.slots[e]=h.dataset.cardId,o(),Ze(i,t,n)})})}function Xn(e,t,i){var c,u,g;const{openModal:n,closeModal:a}=i,o=(c=e.stadiumCards)==null?void 0:c.find(x=>x.id===e.stadiumCardId),r=o&&((u=e.stadDefMap)==null?void 0:u[o.stadium_id])||null,s=new Set;Object.keys(e.slots).forEach(x=>{var m;const f=e.slots[x];if(!f)return;const p=e.playerCards.find(b=>b.id===f);(m=p==null?void 0:p.player)!=null&&m.id&&s.add(p.player.id)}),e.subs.forEach(x=>{var p;const f=e.playerCards.find(m=>m.id===x);(p=f==null?void 0:f.player)!=null&&p.id&&s.add(f.player.id)});const d=new Set,l=e.playerCards.filter(x=>{var f,p,m;return s.has((f=x.player)==null?void 0:f.id)||d.has((p=x.player)==null?void 0:p.id)?!1:(d.add((m=x.player)==null?void 0:m.id),!0)});n("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${l.length>0?'<div style="display:flex;flex-wrap:wrap;gap:8px;justify-content:center">'+l.map(x=>{const f={...x.player,_evolution_bonus:x.evolution_bonus||0};return`<div class="player-option" data-card-id="${x.id}" style="cursor:pointer">
  ...
  <div class="page">
    ...
    <div class="page-body">
      <div class="booster-grid">
        ${o.map(f=>{const p=f.cost===0||a>=f.cost,m=c[f.id]===!0;return`<div class="booster-card ${!p||m?"disabled":""}" data-booster="${f.id}" style="position:relative">
  ...
  `,document.body.appendChild(a);let o=!1;const r=document.getElementById("pack-cut-zone"),s=document.getElementById("pack-blade");let d=!1;const l=L=>L.touches&&L.touches[0]?L.touches[0].clientX:L.clientX;function c(L){o||(d=!0,s.style.opacity="1",u(L))}function u(L){if(!d||o)return;const k=r.getBoundingClientRect(),F=l(L)-k.left,re=Math.max(0,Math.min(1,F/k.width));s.style.width=re*k.width+"px",re>=.82&&x()}function g(){o||(d=!1,s.style.transition="width .2s ease, opacity .2s ease",s.style.width="0",s.style.opacity="0",setTimeout(()=>{o||(s.style.transition="")},220))}function x(){var k;if(o)return;o=!0,d=!1,s.style.width="100%",s.style.opacity="1",(k=document.getElementById("cut-flash"))==null||k.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const L=document.getElementById("cut-hint");L&&(L.style.opacity="0"),r.classList.add("pack-cut"),setTimeout(()=>{s.style.opacity="0",document.getElementById("pack-phase").style.display="none",b(0)},620)}r.addEventListener("pointerdown",c),window.addEventListener("pointermove",u),window.addEventListener("pointerup",g),r.addEventListener("touchstart",c,{passive:!0}),window.addEventListener("touchmove",u,{passive:!0}),window.addEventListener("touchend",g);let f=0,p=!1;const m=new Set;function b(L){f=L,document.getElementById("reveal-phase").style.display="flex",v(),h(L,0),P()}function v(){const L=document.getElementById("card-dots");L&&(L.innerHTML=e.map((k,F)=>`<div class="card-dot" data-i="${F}" style="width:8px;height:8px;border-radius:50%;background:${F===f?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),L.querySelectorAll(".card-dot").forEach(k=>k.addEventListener("click",()=>y(parseInt(k.dataset.i)))))}function h(L,k){var V,T;const F=e[L],re=document.getElementById("card-counter"),pe=document.getElementById("card-track");re&&(re.textContent=`Carte ${L+1} / ${e.length}`);const de=document.getElementById("reveal-btns");de&&(de.style.display=L===e.length-1?"flex":"none");const se=F.card_type==="player"&&((V=F.player)==null?void 0:V.rarity)==="legende",Le=!m.has(L);m.add(L);let K=0;if(F.card_type==="player"&&F.player){const S=F.player,R=S.job||"ATT";K=Number(R==="GK"?S.note_g:R==="DEF"?S.note_d:R==="MIL"?S.note_m:S.note_a)||0}const Q=S=>{pe.innerHTML=`
        <div id="current-card-wrap" style="position:relative;display:flex;flex-direction:column;align-items:center;gap:8px;${se?"filter:drop-shadow(0 0 20px #7a28b8)":""}">
          <div style="transform:scale(1.25);transform-origin:center">${ao(F)}</div>
        ...
        </div>`;const R=document.getElementById("current-card-wrap");k!==0?(R.style.transition="none",R.style.transform=`translateX(${k>0?100:-100}%)`,requestAnimationFrame(()=>{R.style.transition="transform .28s cubic-bezier(.25,1,.5,1)",R.style.transform="translateX(0)"})):R.animate([{opacity:0,transform:"scale(.7)"},{opacity:1,transform:"scale(1)"}],{duration:300,easing:"cubic-bezier(.34,1.56,.64,1)"}),S||se?ee():ie(),v()};Le&&(((T=F.player)==null?void 0:T.rarity)==="legende"||K>=8)&&F.card_type==="player"&&F.player?$(F,()=>Q(!0)):Q(!1)}function $(L,k){var we;p=!0;const F=L.player,re=`https://flagsapi.com/${F.country_code}/flat/64.png`,pe=(we=F.clubs)==null?void 0:we.logo_url,de=F.face?"/"+F.face.replace(/^public\//,"").replace(/^\//,""):null,se=F.job||"ATT",Le=Number(se==="GK"?F.note_g:se==="DEF"?F.note_d:se==="MIL"?F.note_m:F.note_a)||0,K=L.evolution_bonus||0,Q=Le+K,V=F.rarity==="legende",T=Q>=18,S=document.getElementById("walkout-overlay"),R=document.getElementById("walkout-stage");if(!S||!R){p=!1,k();return}let H=null;V&&(H=new Audio("/sounds/Legendary.mp3"),H.volume=.8,H.play().catch(()=>{})),S.style.display="flex";const U=()=>{const me=R.firstElementChild;me&&(me.classList.remove("wo-in"),me.classList.add("wo-out"))},B=1800,O=400;R.innerHTML=`<img class="wo-in" src="${re}" style="height:130px;border-radius:10px;box-shadow:0 10px 36px rgba(0,0,0,.6)" onerror="this.style.display='none'">`,navigator.vibrate&&navigator.vibrate(30),setTimeout(U,B),setTimeout(()=>{var me;R.innerHTML=pe?`<img class="wo-in" src="${pe}" style="max-height:160px;max-width:210px;object-fit:contain">`:`<div class="wo-in" style="font-size:34px;font-weight:900;color:#fff;text-align:center">${((me=F.clubs)==null?void 0:me.encoded_name)||"CLUB"}</div>`,navigator.vibrate&&navigator.vibrate(30)},B+O),setTimeout(U,B*2+O),setTimeout(()=>{R.innerHTML=de?`<img class="wo-in" src="${de}" style="height:200px;border-radius:50%;box-shadow:0 0 40px rgba(255,255,255,0.3);object-fit:cover;object-position:top">`:'<div class="wo-in" style="font-size:80px">👤</div>',navigator.vibrate&&navigator.vibrate(30)},(B+O)*2),setTimeout(U,(B+O)*2+B);const _e=T?"#FFD700":{GK:"#c0c0c0",DEF:"#e03030",MIL:"#D4A017",ATT:"#3fbf5f"}[se]||"#fff";setTimeout(()=>{R.innerHTML=`<div class="wo-in" style="
        ...
        ${Q}
      </div>`,T&&navigator.vibrate&&navigator.vibrate([50,30,100,30,200])},(B+O)*3),setTimeout(U,(B+O)*3+B),setTimeout(()=>{S.style.display="none",R.innerHTML="",p=!1,H&&!V&&H.pause(),navigator.vibrate&&navigator.vibrate([40,30,80]),k()},(B+O)*4)}function y(L){if(p||L<0||L>=e.length||L===f)return;const k=L>f?1:-1;f=L,h(L,k)}function C(){y(f+1)}function E(){y(f-1)}function P(){const L=document.getElementById("card-viewport");if(!L||L._swipeBound)return;L._swipeBound=!0;let k=0,F=0,re=0,pe=!1;const de=V=>V.touches?V.touches[0].clientX:V.clientX,se=V=>V.touches?V.touches[0].clientY:V.clientY,Le=V=>{pe=!0,k=de(V),F=se(V),re=0},K=V=>{if(!pe)return;re=de(V)-k;const T=se(V)-F;if(Math.abs(re)<Math.abs(T))return;const S=document.getElementById("current-card-wrap");S&&(S.style.transition="none",S.style.transform=`translateX(${re*.6}px) rotate(${re*.02}deg)`)},Q=()=>{if(!pe)return;pe=!1;const V=document.getElementById("current-card-wrap"),T=55;re<=-T&&f<e.length-1?C():re>=T&&f>0?E():V&&(V.style.transition="transform .2s ease",V.style.transform="translateX(0)")};L.addEventListener("pointerdown",Le),L.addEventListener("pointermove",K),L.addEventListener("pointerup",Q),L.addEventListener("pointercancel",Q),L.addEventListener("touchstart",Le,{passive:!0}),L.addEventListener("touchmove",K,{passive:!0}),L.addEventListener("touchend",Q),L.addEventListener("click",V=>{if(Math.abs(re)>8)return;const T=L.getBoundingClientRect();V.clientX-T.left>T.width/2?C():E()})}let N=null;function ee(){const L=document.getElementById("fireworks-canvas");if(!L)return;L.width=window.innerWidth,L.height=window.innerHeight;const k=L.getContext("2d"),F=[];function re(){const de=Math.random()*L.width,se=Math.random()*L.height*.6,Le=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],K=Le[Math.floor(Math.random()*Le.length)];for(let Q=0;Q<60;Q++){const V=Math.PI*2/60*Q,T=2+Math.random()*5;F.push({x:de,y:se,vx:Math.cos(V)*T,vy:Math.sin(V)*T,alpha:1,color:K,size:2+Math.random()*3})}}re(),N=setInterval(re,600);function pe(){if(document.getElementById("fireworks-canvas")){k.clearRect(0,0,L.width,L.height);for(let de=F.length-1;de>=0;de--){const se=F[de];if(se.x+=se.vx,se.y+=se.vy+.08,se.vy*=.98,se.alpha-=.018,se.alpha<=0){F.splice(de,1);continue}k.globalAlpha=se.alpha,k.fillStyle=se.color,k.beginPath(),k.arc(se.x,se.y,se.size,0,Math.PI*2),k.fill()}k.globalAlpha=1,(N!==null||F.length>0)&&requestAnimationFrame(pe)}}pe()}function ie(){N!==null&&(clearInterval(N),N=null);const L=document.getElementById("fireworks-canvas");L&&L.getContext("2d").clearRect(0,0,L.width,L.height)}if(n){const L=document.getElementById("reveal-btns");L&&(L.innerHTML='<button class="btn btn-primary" id="reveal-next" style="flex:1">Continuer →</button>'),(j=document.getElementById("reveal-next"))==null||j.addEventListener("click",()=>{ie(),a.remove(),n()})}else document.getElementById("reveal-collection").addEventListener("click",()=>{ie(),a.remove(),i("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{ie(),a.remove(),i("boosters")})}function ao(e){var t,i,n,a;if(e.card_type==="player"&&e.player)return to(e);if(e.card_type==="game_changer"){const o=e._gcDef,r=(o==null?void 0:o.gc_type)==="ultra_game_changer",s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},d={purple:"#b06ce0",light_blue:"#00d4ef"},l=s[o==null?void 0:o.color]||s.purple,c=d[o==null?void 0:o.color]||d.purple,u=(o==null?void 0:o.name)||e.gc_type||"Game Changer",g=(o==null?void 0:o.effect)||((t=ti[e.gc_type])==null?void 0:t.desc)||"",x=o!=null&&o.image_url?`/icons/${o.image_url}`:null,f=((i=ti[e.gc_type])==null?void 0:i.icon)||"⚡";return`<div style="width:170px;height:240px;background:${l};border-radius:14px;border:3px solid ${c};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${c}88;flex-shrink:0">
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}function co(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText=`
    ...
    `,document.body.appendChild(t);let i=5;const n=setInterval(()=>{i--;const a=document.getElementById("mw-ad-cd");a&&(a.textContent=i),i<=0&&(clearInterval(n),t.remove(),e(!0))},1e3)})}async function po(e,{state:t,navigate:i,toast:n,refreshProfile:a}){var g,x;const{data:o}=await w.from("users").select("*").eq("id",t.user.id).single();o&&(t.profile=o);let r=Array.isArray((g=t.profile)==null?void 0:g.pending_boosters)?[...t.profile.pending_boosters]:[];if(!r.length){await w.from("users").update({onboarding_done:!0}).eq("id",t.user.id),i("home");return}let s=null;try{const p=(await Yi()).find(m=>(m.name||"").toLowerCase().includes("new player"));p&&(s=Xi(p))}catch(f){console.warn('[Onboarding] Config "Booster (new player)" introuvable, fallback taux par défaut',f)}const d=r.length;let l=0;e.innerHTML=`
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    </div>`}const d=c=>{e.style.overflow="",e.style.height="",e.style.display="",e.style.flexDirection="",i(c)};function l(){var u,g,x;e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column";const c=r.length>0;e.innerHTML=`
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      card:cards(id,card_type,formation,stadium_id,evolution_bonus,
        player:players(id,firstname,surname_real,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
          clubs(encoded_name,logo_url)))`).in("deck_id",r).order("slot_order"),d=[...new Set((s||[]).filter(g=>{var x,f;return((x=g.card)==null?void 0:x.card_type)==="stadium"&&((f=g.card)==null?void 0:f.stadium_id)}).map(g=>g.card.stadium_id))],l={};if(d.length){const{data:g}=await w.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",d);(g||[]).forEach(x=>{l[x.id]=x}),(s||[]).forEach(x=>{var f,p;((f=x.card)==null?void 0:f.card_type)==="stadium"&&((p=x.card)!=null&&p.stadium_id)&&(x.card._stadiumDef=l[x.card.stadium_id]||null)})}let c=0;function u(){var y,C,E,P,N,ee,ie,le;const g=o[c],x=(s||[]).filter(j=>j.deck_id===g.id),f=x.filter(j=>{var L;return j.is_starter&&((L=j.card)==null?void 0:L.player)}).map(j=>ni(j.card,j.position)),p=x.find(j=>{var L;return((L=j.card)==null?void 0:L.card_type)==="formation"}),m=g.formation||((y=p==null?void 0:p.card)==null?void 0:y.formation)||"4-4-2";let b=f.length>=11?Rt(f,m):null,v=((C=g.stadium_card)==null?void 0:C.stadium_def)||null;v&&b&&(b=Tt(b,v));const h=f.length>=11;He(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:${window.innerWidth>=900?"row":"column"};height:100%;overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div style="display:flex;align-items:center;gap:8px;padding:8px;flex-shrink:0">
        ...
        <div style="flex:1;text-align:center">
          <div style="font-size:19px;font-weight:900">${g.name}</div>
          <div style="font-size:11px;opacity:.6;margin-top:2px">${m} · ${f.length}/11 ${g.is_active?"· ⭐ Actif":""}</div>
        </div>
        <button id="next-deck" style="width:46px;height:46px;border-radius:50%;background:rgba(255,255,255,${c===o.length-1?"0.05":"0.15"});border:2px solid rgba(255,255,255,${c===o.length-1?"0.1":"0.3"});color:${c===o.length-1?"rgba(255,255,255,0.2)":"#fff"};font-size:20px;cursor:${c===o.length-1?"default":"pointer"};flex-shrink:0">▶</button>
      </div>
      ...
      <div id="deck-swipe-zone" style="flex:1;min-height:0;overflow:hidden;position:relative;touch-action:pan-y;display:flex;align-items:center;justify-content:center;padding:4px">
        ${b?(()=>{const j=window.innerWidth>=900,L=window.innerHeight-220,k=window.innerWidth-(j?260:8),F=j?Math.min(k,Math.round(L*.9)):Math.min(k*.98,L*1.2);return`<div class="deck-preview-wrap" style="width:${F}px;height:${Math.round(F*1.25)}px;overflow:visible">
            ${At(b,m,null,[],F,Math.round(F*1))}
      ...
      <div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">
        ${o.map((j,L)=>`<div style="width:7px;height:7px;border-radius:50%;background:${L===c?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}
      </div>`:""}
      ...
      <div style="width:240px;flex-shrink:0;background:rgba(0,0,0,0.3);display:flex;flex-direction:column;gap:16px;padding:24px 16px;border-left:1px solid rgba(255,255,255,0.08)">
        <div>
          ...
          <div style="font-size:20px;font-weight:900">${g.name}</div>
          <div style="font-size:12px;opacity:.6;margin-top:2px">${m} · ${f.length}/11 ${g.is_active?"· ⭐ Actif":""}</div>
        </div>
        ${v?`
        <div style="padding:10px;background:rgba(232,119,34,0.15);border-radius:8px;border:1px solid rgba(232,119,34,0.3)">
          ...
          <div style="font-size:11px;color:#FFD700;margin-top:2px">+10 aux joueurs ${((P=v.club)==null?void 0:P.encoded_name)||v.country_code||""}</div>
        </div>`:""}
    ...
    </div>`,function(){const L=e.querySelector(".deck-preview-wrap svg");if(!L)return;L.removeAttribute("width"),L.removeAttribute("height"),L.style.cssText="width:100%;height:auto;display:block;max-width:none;margin:0;overflow:visible",L.setAttribute("preserveAspectRatio","xMidYMid meet");const k=e.querySelector(".deck-preview-wrap");if(k)if(window.innerWidth>=900)k.style.marginTop="50px";else{const F=e.querySelector("#deck-select-screen > div:nth-child(1)"),re=e.querySelector("#deck-select-screen > div:nth-child(2)"),pe=e.querySelector("#deck-select-screen > div:nth-child(3)"),de=((F==null?void 0:F.offsetHeight)||0)+((re==null?void 0:re.offsetHeight)||0)+((pe==null?void 0:pe.offsetHeight)||0);de>0&&(k.style.marginTop=de+"px")}}(),(N=document.getElementById("prev-deck"))==null||N.addEventListener("click",()=>{c>0&&(c--,u())}),(ee=document.getElementById("next-deck"))==null||ee.addEventListener("click",()=>{c<o.length-1&&(c++,u())}),(ie=document.getElementById("validate-deck"))==null||ie.addEventListener("click",()=>{if(!h)return;const j=t.state.params||{};t.navigate("match",{...j,matchMode:j.matchMode||i,deckId:g.id})}),(le=document.getElementById("cancel-deck-select"))==null||le.addEventListener("click",()=>{Ue(e),a("home")});const $=document.getElementById("deck-swipe-zone");if($){let j=0,L=0;$.addEventListener("touchstart",k=>{j=k.touches[0].clientX,L=k.touches[0].clientY},{passive:!0}),$.addEventListener("touchend",k=>{const F=k.changedTouches[0].clientX-j,re=k.changedTouches[0].clientY-L;Math.abs(F)<40||Math.abs(F)<Math.abs(re)||(F<0&&c<o.length-1?(c++,u()):F>0&&c>0&&(c--,u()))},{passive:!0})}}u()}function Je(e,t=44,i=58,n=null){return Ae(e,{width:t,showStad:!!n,stadDef:n,used:e==null?void 0:e.used})}function ut(e,t,i){if(!(e!=null&&e.length))return"";const n=e.slice(0,5);let a='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return n.forEach((o,r)=>{const s=o._line||o.job||"MIL";if(a+=Ae(o,{width:40,role:s,extraNote:o.boost||0}),r<n.length-1){const d=yt(o,n[r+1]);a+=`<div style="width:7px;height:3px;background:${d==="#ff3333"||d==="#cc2222"?"rgba(255,255,255,0.12)":d};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(a+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),a+="</div>",a}function It(e,t,i,n,a=310,o=310,r=[]){const s=St[t]||{},d=Gi(t)||si[t]||[],l={},c=["ATT","MIL","DEF","GK"];for(const m of c)(e[m]||[]).forEach((v,h)=>{l[`${m}${h+1}`]=v});function u(m){const b=s[m];return b?{x:b.x*a,y:b.y*o}:null}let g="";for(const[m,b]of d){const v=u(m),h=u(b);if(!v||!h)continue;const $=l[m],y=l[b],C=yt($,y);C==="#00ff88"||C==="#FFD700"?(g+=`<line x1="${v.x.toFixed(1)}" y1="${v.y.toFixed(1)}" x2="${h.x.toFixed(1)}" y2="${h.y.toFixed(1)}"
        stroke="${C}" stroke-width="10" stroke-linecap="round" opacity="0.22"/>`,g+=`<line x1="${v.x.toFixed(1)}" y1="${v.y.toFixed(1)}" x2="${h.x.toFixed(1)}" y2="${h.y.toFixed(1)}"
        stroke="${C}" stroke-width="3.5" stroke-linecap="round" opacity="0.95"/>`):g+=`<line x1="${v.x.toFixed(1)}" y1="${v.y.toFixed(1)}" x2="${h.x.toFixed(1)}" y2="${h.y.toFixed(1)}"
        stroke="${C}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const x=Math.max(52,Math.round(a*.18)),f=Math.round(x*657/507);for(const[m,b]of Object.entries(l)){const v=u(m);if(!v||!b)continue;const h=m.replace(/[0-9]/g,""),$=r.includes(b.cardId),y=i==="attack"&&(["MIL","ATT"].includes(h)||$)&&!b.used||i==="defense"&&["GK","DEF","MIL"].includes(h)&&!b.used,C=n.includes(b.cardId);let E=b.boost||0;b.stadiumBonus&&(i==="attack"&&(h==="ATT"||h==="MIL")||i==="defense"&&(h==="GK"||h==="DEF"||h==="MIL")?E+=10:i||(E+=10));const P=Math.round(v.x-x/2),N=Math.round(v.y-f/2);if(b.used){g+=`<image href="${`${typeof import.meta<"u"&&"/"||"/"}icons/carte-dos.png`}" x="${P}" y="${N}" width="${x}" height="${f}" preserveAspectRatio="xMidYMid slice" class="match-used-hit" data-card-id="${b.cardId}" data-role="${h}" style="cursor:pointer"/>`;continue}const ee=Ae({...b,_evolution_bonus:0},{width:x,showStad:!0,stadDef:null,role:h,extraNote:E}),ie=C?"outline:3px solid #FFD700;outline-offset:2px;border-radius:8px;opacity:0.75;":"";g+=`<foreignObject x="${P-2}" y="${N-30}" width="${x+8}" height="${f+60}" style="overflow:visible">
      <div xmlns="http://www.w3.org/1999/xhtml" style="${ie}position:relative">
        ${ee}
      </div>
    </foreignObject>`,y&&(g+=`<rect x="${P}" y="${N}" width="${x}" height="${f}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${C?"selected":""}" data-card-id="${b.cardId}" data-role="${h}" style="cursor:pointer"/>`)}const p=Math.round(Math.max(x*.7,80));return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-p} ${-p} ${a+p*2} ${o+p*2}" width="100%" style="display:block;width:100%;margin:0 auto">
    ${g}
    ...
    ${It(e,t,i,n,a,o,r)}
          ...
          card:cards(id, card_type, formation, evolution_bonus,
            player:players(id,firstname,surname_real,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
              clubs(encoded_name,logo_url)))`).eq("deck_id",d).order("slot_order")]);l=E[0].data,u=E[0].error,c=E[1].data,g=E[1].error}catch(E){console.error("[Match] Exception chargement deck:",E),at(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>o("home"));return}if(u||g){console.error("[Match] Erreur Supabase:",u||g),at(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>o("home"));return}const x=(c||[]).filter(E=>{var P;return E.is_starter&&((P=E.card)==null?void 0:P.player)}).map(E=>ni(E.card,E.position)),f=(c||[]).filter(E=>{var P;return!E.is_starter&&((P=E.card)==null?void 0:P.player)}).map(E=>ni(E.card,E.position));if(x.length<11){at(e,"⚠️",`Deck incomplet (${x.length}/11).`,"Compléter",()=>o("decks"));return}const p=(c||[]).find(E=>{var P;return((P=E.card)==null?void 0:P.card_type)==="formation"}),m=(l==null?void 0:l.formation)||((C=p==null?void 0:p.card)==null?void 0:C.formation)||"4-4-2",{data:b,error:v}=await w.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",a.profile.id).eq("card_type","game_changer"),{data:h}=await w.from("gc_definitions").select("*").eq("is_active",!0),$=(b||[]).map(E=>({...E,_gcDef:(h==null?void 0:h.find(P=>P.name===E.gc_type||P.id===E.gc_definition_id))||null}));let y=null;if(l!=null&&l.stadium_card_id){const{data:E}=await w.from("cards").select("stadium_id").eq("id",l.stadium_card_id).single();if(E!=null&&E.stadium_id){const{data:P}=await w.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").eq("id",E.stadium_id).single();y=P||null}}n({deckId:d,formation:m,starters:x,subsRaw:f,gcCardsEnriched:$,gcDefs:h||[],stadiumDef:y})}function We(){return Math.min(window.innerWidth-40,860)}function rt(){return Math.round(We()*1.1)}function yo(e){var a,o;if(!e)return null;const t=e._line||e.job||"MIL",i=t==="GK"?e.note_g||0:t==="DEF"?e.note_d||0:t==="MIL"?e.note_m||0:e.note_a||0,n=e.stadiumBonus?10:0;return{name:e.name,firstname:e.firstname||"",note:i+(e.boost||0)+n,note_g:e.note_g||0,note_d:e.note_d||0,note_m:e.note_m||0,note_a:e.note_a||0,_evolution_bonus:0,stadiumBonus:e.stadiumBonus||!1,boost:e.boost||0,job:e.job,job2:e.job2||null,_line:e._line||e.job,_col:e._col,country_code:e.country_code,club_id:e.club_id,rarity:e.rarity,clubName:e.clubName||((a=e.clubs)==null?void 0:a.encoded_name)||null,clubLogo:e.clubLogo||((o=e.clubs)==null?void 0:o.logo_url)||null,face:e.face||null,portrait:zt(e)}}function nn(e){var o,r,s;if(!e)return"";const t=d=>d?Ae({...d,_evolution_bonus:0},{width:52,role:d._line||d.job,showStad:!!d.stadiumBonus,extraNote:d.boost||0}):"",n={goal:"⚽","goal-home":"⚽","goal-ai":"⚽",duel:"⚔️",midfield:"🎯",sub:"🔄",gc:"⚡",boost:"💥","defense-won":"🛡️","attack-won":"⚔️","defense-lost":"😓","attack-lost":"😓"}[e.type]||"📋";return`
  ...
  <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div id="duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center, rgba(10,61,30,0.4), rgba(10,61,30,0.92))"></div>
          ...
          <button id="start-match-btn" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,0.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">
            ▶ Commencer le match
          </button>`,$.style.transition="opacity .45s ease",$.style.opacity="1",$.style.pointerEvents="auto",(y=document.getElementById("start-match-btn"))==null||y.addEventListener("click",x))},600)},700)},2800)}function ze(e,t,i){var C,E,P,N,ee,ie,le,j,L;const n=t.selected.map(k=>k.cardId),a=t.usedSubIds||[],o=t.homeSubs.filter(k=>!a.includes(k.cardId)),s=Object.values(t.homeTeam).flat().filter(k=>k.used).length>0&&o.length>0&&t.subsUsed<t.maxSubs,d=!["GK","DEF","MIL","ATT"].some(k=>(t.aiTeam[k]||[]).some(F=>!F.used)),l=[...t.homeTeam.MIL||[],...t.homeTeam.ATT||[]].filter(k=>!k.used),c=t.phase==="attack"&&d&&l.length===0?[...t.homeTeam.GK||[],...t.homeTeam.DEF||[]].filter(k=>!k.used).map(k=>k.cardId):[];t.log[t.log.length-1];const u=t.phase==="ai-attack"||t.phase==="ai-defense",g=t.phase==="attack",x=t.phase==="defense",f=t.phase==="finished",m=(t.homeSubs||[]).filter(k=>!(t.usedSubIds||[]).includes(k.cardId)).length>0&&t.subsUsed<t.maxSubs,b=g&&l.length===0&&!m,v=t.gcCards.filter(k=>!t.usedGc.includes(k.id)),h=t.boostCard&&!t.boostUsed;e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column",e.innerHTML=`
  <style>
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const F=t.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
            ...
            ${ut((F.players||[]).map(re=>({...re,used:!1})),"#ff6b6b",F.total)}
            ...
            ${ut((F.players||[]).map(re=>({...re,used:!1})),"#00ff88",F.total)}
          </div>`}const k=t.log[t.log.length-1];return k?'<div style="padding:2px 4px">'+nn(k)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})()}
    </div>
    ...
    ${(()=>{const k=window.innerWidth>=700,F=(T,S,R)=>{var Ce,je;const H=(t.gcDefs||[]).find(Fe=>Fe.name===T.gc_type),U={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[H==null?void 0:H.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",B={purple:"#b06ce0",light_blue:"#00d4ef"}[H==null?void 0:H.color]||"#b06ce0",O=(H==null?void 0:H.name)||T.gc_type,$e=(H==null?void 0:H.effect)||((Ce=Se[T.gc_type])==null?void 0:Ce.desc)||"",_e=H!=null&&H.image_url?`/icons/${H.image_url}`:null,we=((je=Se[T.gc_type])==null?void 0:je.icon)||"⚡",me=Math.round(R*.22),Ee=Math.round(R*.22),ke=R-me-Ee,he=O.length>12?7:9;return`<div class="gc-mini" data-gc-id="${T.id}" data-gc-type="${T.gc_type}"
          style="box-sizing:border-box;width:${S}px;height:${R}px;border-radius:10px;border:2px solid ${B};background:${U};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
          <div style="height:${me}px;background:rgba(255,255,255,0.14);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 3px">
            <span style="font-size:${he}px;font-weight:900;color:#fff;text-align:center;line-height:1.1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${S-6}px">${O}</span>
        ...
        </div>`},re=(T,S)=>{var R;return`<div id="boost-card"
          style="box-sizing:border-box;width:${T}px;height:${S}px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:2px solid #87CEEB;border-radius:10px;cursor:pointer;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:${Math.round(S*.04)}px;text-align:center;flex-shrink:0">
          ...
          </div>`},pe=(T,S)=>S?re(130,175):F(T,130,175),de=(T,S)=>S?re(68,95):F(T,68,95),se=k?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",Le=f?`<button id="btn-results" style="${se};background:linear-gradient(135deg,#D4A017,#FFD700);border:none;color:#000">🏁 Résultats</button>`:u?`<div style="${se};background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);color:rgba(255,255,255,0.4)">⏳ Tour IA</div>`:b?`<button id="btn-pass" style="${se};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER (plus d'attaquants)</button>`:g?`<button id="btn-action" style="${se};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${t.selected.length===0?"disabled":""}> ⚔️ ATTAQUEZ <span id="match-timer" style="font-weight:900"></span></button>`:x?`<button id="btn-action" style="${se};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${t.selected.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="match-timer" style="font-weight:900"></span></button>`:`<div style="${se};background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1)"></div>`,K=g||x?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${t.selected.length}/3 sélectionné(s)</div>`:"",Q=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${k?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(k=>{var F,re,pe;if(k.type==="duel"){const de=k.isGoal,se=k.homeScored?"#FFD700":de?"#ff6b6b":"rgba(255,255,255,0.3)",Le=k.homeScored?"⚽ BUT !":de?"⚽ BUT IA !":(F=k.homePlayers)!=null&&F.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${de?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${se};margin-bottom:4px">
                <div style="font-size:9px;color:${se};letter-spacing:1px;margin-bottom:5px;font-weight:700;text-transform:uppercase">${Le}</div>
                ${(re=k.homePlayers)!=null&&re.length?`<div style="margin-bottom:3px">${ut(k.homePlayers,"rgba(255,255,255,0.7)",k.homeTotal)}</div>`:""}
                ${(pe=k.aiPlayers)!=null&&pe.length?`<div style="opacity:0.7">${ut(k.aiPlayers,"#ff6b6b",k.aiTotal)}</div>`:""}
  ...
  </div>`;function $(){const k=e.querySelector(".match-screen");if(!k)return;const F=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);k.style.bottom="auto",k.style.height=F+"px",k.style.minHeight=F+"px",k.style.maxHeight=F+"px",k.style.overflow="hidden";const re=e.querySelector("#mobile-action-bar"),pe=e.querySelector("#mobile-play-area");re&&pe&&(pe.style.paddingBottom=re.offsetHeight+"px")}if($(),setTimeout($,120),setTimeout($,400),setTimeout($,1e3),t._vvBound||(t._vvBound=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",$),window.visualViewport.addEventListener("scroll",$)),window.addEventListener("resize",$)),function(){const F=e.querySelector(".terrain-wrapper svg");F&&(F.removeAttribute("width"),F.removeAttribute("height"),F.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",F.setAttribute("viewBox","-26 -26 352 352"),F.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),t._resizeBound||(t._resizeBound=!0,window.addEventListener("resize",()=>{const k=e.querySelector(".terrain-wrapper svg");k&&(k.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0")})),t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase==="attack"||t.phase==="defense"){let k=!1,F=30;const re=()=>document.getElementById("match-timer"),pe=()=>{const de=re();if(!de)return;const se=String(Math.floor(F/60)).padStart(2,"0"),Le=String(F%60).padStart(2,"0");de.textContent=` ${se}:${Le}`,de.style.color=k?"#ff2222":"#ff9500",de.style.fontWeight="900"};pe(),t._timerInt=setInterval(()=>{if(F--,F<0)if(!k)k=!0,F=15,pe();else{clearInterval(t._timerInt),t._timerInt=null,t.homeScore=0,t.aiScore=3;const de=document.createElement("div");de.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;color:#fff;padding:24px;text-align:center",de.innerHTML='<div style="font-size:56px">⏱️</div><div style="font-size:24px;font-weight:900;color:#ff4444">MATCH PERDU PAR FORFAIT</div><div style="font-size:14px;color:rgba(255,255,255,0.6)">Temps écoulé</div>',document.body.appendChild(de),setTimeout(()=>{de.remove(),kt(e,t,i)},2500)}else pe()},1e3)}(C=document.getElementById("match-quit"))==null||C.addEventListener("click",()=>{Ue(e),confirm("Abandonner ? Résultat : défaite 3-0")&&(t.homeScore=0,t.aiScore=3,kt(e,t,i))}),(E=document.getElementById("view-ai"))==null||E.addEventListener("click",()=>jo(t,i)),(P=document.getElementById("toggle-history"))==null||P.addEventListener("click",()=>{var k;(k=document.getElementById("match-history-panel"))==null||k.classList.add("open")}),(N=document.getElementById("close-history"))==null||N.addEventListener("click",()=>{var k;(k=document.getElementById("match-history-panel"))==null||k.classList.remove("open")}),(ee=document.getElementById("btn-action"))==null||ee.addEventListener("click",()=>{t.selected.length!==0&&(g?Eo(e,t,i):x&&Lo(e,t,i))}),(ie=document.getElementById("btn-results"))==null||ie.addEventListener("click",()=>kt(e,t,i)),(le=document.getElementById("btn-pass"))==null||le.addEventListener("click",()=>{t.log.push({text:"⏭️ Vous passez votre tour (plus d'attaquants)",type:"info"}),t.phase="ai-attack",ze(e,t,i),setTimeout(()=>Ht(e,t,i),800)}),e.querySelectorAll(".match-slot-hit").forEach(k=>{k.addEventListener("click",()=>$o(k,t,e,i))}),e.querySelectorAll(".match-used-hit").forEach(k=>{k.addEventListener("click",()=>Qt(e,t,i,null,k.dataset.cardId))}),e.querySelectorAll(".gc-mini").forEach(k=>{k.addEventListener("click",()=>Ao(k.dataset.gcId,k.dataset.gcType,e,t,i))}),(j=document.getElementById("boost-card"))==null||j.addEventListener("click",()=>Co(e,t,i)),e.querySelectorAll(".sub-btn-col").forEach(k=>{k.addEventListener("click",()=>Qt(e,t,i,k.dataset.subId))}),(L=document.getElementById("sub-btn-main"))==null||L.addEventListener("click",()=>Qt(e,t,i))}function $o(e,t,i,n){const a=e.dataset.cardId,o=e.dataset.role,r=t.selected.findIndex(s=>s.cardId===a);if(r!==-1)t.selected.splice(r,1);else{if(t.selected.length>=3){n.toast("Maximum 3 joueurs","error");return}const s=(t.homeTeam[o]||[]).find(d=>d.cardId===a);s&&t.selected.push({...s,_role:o,_line:o})}ze(i,t,n)}function gi(e,t,i){e.matchId&&w.from("matches").update({last_player_id:i}).eq("id",e.matchId).then(()=>{})}function Eo(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),gi(t,i,i.state.profile.id);const n=!["GK","DEF","MIL","ATT"].some(r=>(t.aiTeam[r]||[]).some(s=>!s.used)),a=t.selected.map(r=>{const s=(t.homeTeam[r._role]||[]).find(l=>l.cardId===r.cardId)||r,d=n&&["GK","DEF"].includes(r._role);return{...s,_line:r._role,...d?{note_a:Math.max(1,Number(s.note_a)||0)}:{}}}),o=pi(a,t.modifiers.home);t.pendingAttack={...o,players:[...a],side:"home"},t.selected.forEach(r=>{const s=(t.homeTeam[r._role]||[]).find(d=>d.cardId===r.cardId);s&&(s.used=!0)}),t.log.push({text:`⚔️ Vous attaquez : ${o.total} (base ${o.base}${o.links?` +${o.links} liens`:""}) — ${t.selected.map(r=>r.name).join(", ")}`,type:"info"}),t.selected=[],t.modifiers.home={},t.phase="ai-defense",ze(e,t,i),setTimeout(()=>Io(e,t,i),1200)}function Lo(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),gi(t,i,i.state.profile.id);const n=t.stadiumDef||null,a=t.selected.map(d=>{const l=(t.homeTeam[d._role]||[]).find(u=>u.cardId===d.cardId)||d,c=l.stadiumBonus||n&&(n.club_id&&String(l.club_id)===String(n.club_id)||n.country_code&&l.country_code===n.country_code)||!1;return{...l,_line:d._role,stadiumBonus:c}}),o=ui(a,t.modifiers.home);t.selected.forEach(d=>{const l=(t.homeTeam[d._role]||[]).find(c=>c.cardId===d.cardId);l&&(l.used=!0)});const r=fi(t.pendingAttack.total,o.total,t.modifiers.home),s={type:"duel",title:"Défense",aiPlayers:(t.pendingAttack.players||[]).map(d=>Qe(d)),homePlayers:t.selected.map(d=>{const l=(t.homeTeam[d._role]||[]).find(c=>c.cardId===d.cardId)||d;return Qe(l)}),homeTotal:o.total,aiTotal:t.pendingAttack.total,isGoal:!1,homeScored:!1,text:""};if(r.shielded)s.text="🛡️ Bouclier ! But annulé.",t.log.push(s);else if(r.goal){t.aiScore++,s.isGoal=!0,s.homeScored=!1,s.text=`⚽ BUT IA ! (${t.pendingAttack.total} > ${o.total})`,t.log.push(s),t.selected=[],t.modifiers.home={},t.pendingAttack=null,ze(e,t,i),st(s.aiPlayers,t.homeScore,t.aiScore,!1,()=>{ft(e,t,i,"home-attack")});return}else s.text=`🧤 Défense réussie ! (${o.total} ≥ ${t.pendingAttack.total})`,t.log.push(s);t.selected=[],t.modifiers.home={},t.pendingAttack=null,ft(e,t,i,"home-attack")}function zo(e){if(e.aiSubsUsed>=e.aiMaxSubs)return;const t=Object.values(e.aiTeam).flat().filter(d=>d.used);if(!t.length)return;const i=(e.aiSubs||[]).filter(d=>!e.aiUsedSubIds.includes(d.cardId));if(!i.length)return;const n=t[Math.floor(Math.random()*t.length)],a=i.find(d=>d.job===n.job)||i[0],o={...a,used:!1,_line:n._line,_col:n._col},r=e.aiTeam[n._line],s=r.findIndex(d=>d.cardId===n.cardId);s!==-1&&(r[s]=o),e.aiUsedSubIds.push(a.cardId),e.aiSubsUsed++,e.log.push({text:`🔄 IA : ${a.firstname} ${a.name} remplace ${n.firstname} ${n.name}`,type:"info"})}function To(e){var n;if(!((n=e.aiGcCards)!=null&&n.length))return;const t=e.aiGcCards.filter(a=>!e.aiUsedGc.includes(a.id));if(!t.length||Math.random()>.3)return;const i=t[Math.floor(Math.random()*t.length)];switch(e.aiUsedGc.push(i.id),Se[i.gc_type],i.gc_type){case"Boost+2":{const a=Object.values(e.aiTeam).flat().filter(o=>!o.used);if(a.length){const o=a[Math.floor(Math.random()*a.length)];o.boost=(o.boost||0)+2}break}case"Boost+3":{const a=Object.values(e.aiTeam).flat().filter(o=>!o.used);if(a.length){const o=a[Math.floor(Math.random()*a.length)];o.boost=(o.boost||0)+3}break}case"Remplacement+":e.aiMaxSubs=(e.aiMaxSubs||3)+1;break;case"Bouclier":e.modifiers.ai.shield=!0;break;case"Nul+1":e.modifiers.ai.drawBonus=(e.modifiers.ai.drawBonus||0)+1;break}e.log.push({text:`⚡ IA joue ${i.icon||"⚡"} ${i.name}`,type:"gc"})}function Ht(e,t,i){gi(t,i,null),zo(t),To(t);const n=[...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]],a=Vi(n,"attack",t.difficulty);if(!a.length){Ft(e,t,i);return}const o=pi(a,t.modifiers.ai);t.pendingAttack={...o,players:a,side:"ai"},a.forEach(l=>{l.used=!0}),t.log.push({text:`🤖 IA attaque : ${o.total} (${a.map(l=>l.name).join(", ")})`,type:"info"}),t.modifiers.ai={};const r=[...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(l=>!l.used),d=(t.homeSubs||[]).filter(l=>!(t.usedSubIds||[]).includes(l.cardId)).length>0&&t.subsUsed<t.maxSubs;if(r.length===0&&!d){t.aiScore++;const l={type:"duel",isGoal:!0,homeScored:!1,aiPlayers:a.map(c=>Qe(c)),aiTotal:o.total,text:"⚽ BUT IA ! (aucun défenseur disponible)"};t.log.push(l),t.pendingAttack=null,ze(e,t,i),st(l.aiPlayers,t.homeScore,t.aiScore,!1,()=>{ft(e,t,i,"home-attack")});return}t.phase="defense",ze(e,t,i)}function Io(e,t,i){var l,c;const n=[...t.aiTeam.GK||[],...t.aiTeam.DEF||[],...t.aiTeam.MIL||[]],a=Vi(n,"defense",t.difficulty);if(!["GK","DEF","MIL","ATT"].flatMap(u=>(t.aiTeam[u]||[]).filter(g=>!g.used)).length){t.homeScore++;const g={type:"duel",isGoal:!0,homeScored:!0,homePlayers:(((l=t.pendingAttack)==null?void 0:l.players)||[]).map(x=>Qe(x)),homeTotal:((c=t.pendingAttack)==null?void 0:c.total)||0,aiTotal:0,text:"⚽ BUT ! L'IA n'a plus de joueurs — but automatique !"};t.log.push(g),t.modifiers.ai={},t.pendingAttack=null,ze(e,t,i),st(g.homePlayers,t.homeScore,t.aiScore,!0,()=>{ft(e,t,i,"ai-attack")});return}const r=a.length>0?ui(a,t.modifiers.ai).total:0;a.forEach(u=>{u.used=!0});const s=fi(t.pendingAttack.total,r,t.modifiers.ai),d={type:"duel",title:"Attaque",homePlayers:(t.pendingAttack.players||[]).map(u=>Qe(u)),aiPlayers:a.map(u=>Qe(u)),homeTotal:t.pendingAttack.total,aiTotal:r,isGoal:!1,homeScored:!1,text:""};if(s.shielded)d.text="🛡️ Bouclier IA !",t.log.push(d);else if(s.goal){t.homeScore++,d.isGoal=!0,d.homeScored=!0,d.text=`⚽ BUT ! (${t.pendingAttack.total} > ${r})`,t.log.push(d),t.modifiers.ai={},t.pendingAttack=null,ze(e,t,i),st(d.homePlayers,t.homeScore,t.aiScore,!0,()=>{ft(e,t,i,"ai-attack")});return}else d.text=`🧤 IA défend (${r} ≥ ${t.pendingAttack.total})`,t.log.push(d);t.modifiers.ai={},t.pendingAttack=null,ft(e,t,i,"ai-attack")}function ft(e,t,i,n){if(t.round++,on(t)){kt(e,t,i);return}if(n==="home-attack"){if(![...t.homeTeam.MIL||[],...t.homeTeam.ATT||[]].filter(o=>!o.used).length){if(![...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(s=>!s.used).length){Ft(e,t,i);return}if(![...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]].filter(s=>!s.used).length){Ft(e,t,i);return}setTimeout(()=>Ht(e,t,i),100);return}t.phase="attack",ze(e,t,i)}else{if(![...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]].filter(o=>!o.used).length){Ft(e,t,i);return}t.phase="ai-attack",ze(e,t,i),setTimeout(()=>Ht(e,t,i),800)}}function on(e){const t=["MIL","ATT","GK","DEF"].some(n=>(e.homeTeam[n]||[]).some(a=>!a.used)),i=["MIL","ATT","GK","DEF"].some(n=>(e.aiTeam[n]||[]).some(a=>!a.used));return!t&&!i}function Ft(e,t,i){on(t)?kt(e,t,i):(t.phase="attack",ze(e,t,i))}function Qt(e,t,i,n=null,a=null){var x,f;if(t.phase!=="attack"){wt("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){wt(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const o=Object.entries(t.homeTeam).flatMap(([p,m])=>(m||[]).filter(b=>b.used).map(b=>({...b,_line:b._line||p}))),r=t.homeSubs.filter(p=>!t.usedSubIds.includes(p.cardId));if(!o.length){wt("Aucun joueur utilisé à remplacer");return}if(!r.length){wt("Aucun remplaçant disponible");return}let s=Math.max(0,o.findIndex(p=>p.cardId===a));const d=((x=o[s])==null?void 0:x._line)||((f=o[s])==null?void 0:f.job);let l=n?Math.max(0,r.findIndex(p=>p.cardId===n)):Math.max(0,r.findIndex(p=>p.job===d)),c=!1;const u=document.createElement("div");u.id="sub-overlay",u.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function g(){var y,C,E,P,N,ee;const p=o[s],m=r[l],b=Math.min(130,Math.round((window.innerWidth-90)/2)),v=Math.round(b*1.35),h=ie=>`background:rgba(255,255,255,0.12);border:none;color:${ie?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${ie?"default":"pointer"};flex-shrink:0`;u.innerHTML=`
    ...
    <div style="flex:1;display:flex;gap:0;overflow:hidden">
      ...
      <div id="in-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
        ...
        <div>${m?Je({...m,used:!1,boost:0},b,v):"<div>—</div>"}</div>
    ...
    </div>`,(y=u.querySelector("#sub-close"))==null||y.addEventListener("click",()=>u.remove()),(C=u.querySelector("#out-up"))==null||C.addEventListener("click",()=>{s>0&&(s--,g())}),(E=u.querySelector("#out-down"))==null||E.addEventListener("click",()=>{s<o.length-1&&(s++,g())}),(P=u.querySelector("#in-up"))==null||P.addEventListener("click",()=>{l>0&&(l--,g())}),(N=u.querySelector("#in-down"))==null||N.addEventListener("click",()=>{l<r.length-1&&(l++,g())});const $=(ie,le,j,L)=>{const k=u.querySelector("#"+ie);if(!k)return;let F=0;k.addEventListener("touchstart",re=>{F=re.touches[0].clientY},{passive:!0}),k.addEventListener("touchend",re=>{const pe=re.changedTouches[0].clientY-F;if(Math.abs(pe)<30)return;const de=le();pe<0&&de<L-1?(j(de+1),g()):pe>0&&de>0&&(j(de-1),g())},{passive:!0})};$("in-panel",()=>l,ie=>l=ie,r.length),$("out-panel",()=>s,ie=>s=ie,o.length),(ee=u.querySelector("#sub-confirm"))==null||ee.addEventListener("click",ie=>{if(ie.preventDefault(),ie.stopPropagation(),c)return;c=!0;const le=o[s],j=r[l];if(!le||!j)return;let L=null,k=-1;for(const[re,pe]of Object.entries(t.homeTeam)){const de=(pe||[]).findIndex(se=>se.cardId===le.cardId);if(de!==-1){L=re,k=de;break}}if(k===-1||!L){wt("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),u.remove();return}const F={...j,_line:L,_col:le._col||0,used:!1,boost:0};t.homeTeam[L].splice(k,1,F),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(j.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:le.name,firstname:le.firstname,note:qe(le,L),portrait:zt(le),job:le.job,country_code:le.country_code,rarity:le.rarity,clubName:le.clubName,clubLogo:le.clubLogo},inPlayer:{name:j.name,firstname:j.firstname,note:qe(j,L),portrait:zt(j),job:j.job,country_code:j.country_code,rarity:j.rarity,clubName:j.clubName,clubLogo:j.clubLogo},text:`🔄 ${j.firstname} ${j.name} remplace ${le.firstname} ${le.name}`}),u.remove(),bo(le,j,()=>ze(e,t,i))})}document.body.appendChild(u),g()}function Ao(e,t,i,n,a){var f,p;const o=(n.gcDefs||[]).find(m=>m.name===t),r=Se[t]||{icon:"⚡",desc:"Carte spéciale."},s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[o==null?void 0:o.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",d={purple:"#b06ce0",light_blue:"#00d4ef"}[o==null?void 0:o.color]||"#b06ce0",l=(o==null?void 0:o.name)||t,c=(o==null?void 0:o.effect)||r.desc,u=o!=null&&o.image_url?`/icons/${o.image_url}`:null,g=r.icon||"⚡",x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",x.innerHTML=`
    ...
    </div>`,document.body.appendChild(x),(f=x.querySelector("#gc-back"))==null||f.addEventListener("click",()=>x.remove()),(p=x.querySelector("#gc-use"))==null||p.addEventListener("click",()=>{x.remove(),Mo(e,t,i,n,a)})}function qt(e,t,i,n,a,o){const r=document.createElement("div");r.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let s=[];function d(){var l,c;r.innerHTML=`
    ...
    <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
      ${e.map(u=>{const g=u._line||u.job||"MIL",x={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[g]||"#555",f=qe(u,g)+(u.boost||0),p=s.find(m=>m.cardId===u.cardId);return`<div class="gc-pick-item" data-cid="${u.cardId}"
    ...
    </div>`,(l=r.querySelector("#gc-picker-close"))==null||l.addEventListener("click",()=>r.remove()),r.querySelectorAll(".gc-pick-item").forEach(u=>{u.addEventListener("click",()=>{const g=u.dataset.cid,x=e.find(p=>p.cardId===g);if(!x)return;const f=s.findIndex(p=>p.cardId===g);f>-1?s.splice(f,1):s.length<t&&s.push(x),d()})}),(c=r.querySelector("#gc-picker-confirm"))==null||c.addEventListener("click",()=>{r.remove(),o(s)})}d(),document.body.appendChild(r)}const So={BOOST_STAT:({value:e=1,count:t=1,roles:i=[]},n,a,o)=>{const r=Object.entries(n.homeTeam).filter(([s])=>!i.length||i.includes(s)).flatMap(([s,d])=>d.filter(l=>!l.used).map(l=>({...l,_line:s})));return r.length?(qt(r,t,`Choisir ${t} joueur(s) à booster (+${e})`,a,n,s=>{s.forEach(d=>{const l=(n.homeTeam[d._line]||[]).find(c=>c.cardId===d.cardId);l&&(l.boost=(l.boost||0)+e,n.log.push({text:`⚡ +${e} sur ${l.name}`,type:"info"}))}),ze(a,n,o)}),!0):(n.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),ze(a,n,o),!0)},DEBUFF_STAT:({value:e=1,count:t=1,roles:i=[],target:n="ai"},a,o,r)=>{const s=n==="home"?a.homeTeam:a.aiTeam,d=n==="ai"?"adverse":"allié",l=Object.entries(s).filter(([c])=>!i.length||i.includes(c)).flatMap(([c,u])=>u.filter(g=>!g.used).map(g=>({...g,_line:c})));return l.length?(qt(l,t,`Choisir ${t} joueur(s) ${d}(s) à débuffer (-${e})`,o,a,c=>{c.forEach(u=>{const x=((n==="home"?a.homeTeam:a.aiTeam)[u._line]||[]).find(f=>f.cardId===u.cardId);x&&(x.boost=(x.boost||0)-e,a.log.push({text:`🎯 -${e} sur ${x.name}${n==="ai"?" (IA)":""}`,type:"info"}))}),ze(o,a,r)}),!0):(a.log.push({text:`🎯 Aucun joueur ${d} disponible`,type:"info"}),ze(o,a,r),!0)},GRAY_PLAYER:({count:e=1,roles:t=[],target:i="ai"},n,a,o)=>{const r=i==="home"?n.homeTeam:n.aiTeam,s=i==="ai"?"adverse":"allié",d=Object.entries(r).filter(([l])=>!t.length||t.includes(l)).flatMap(([l,c])=>c.filter(u=>!u.used).map(u=>({...u,_line:l})));return d.length?(qt(d,e,`Choisir ${e} joueur(s) ${s}(s) à exclure`,a,n,l=>{l.forEach(c=>{const g=((i==="home"?n.homeTeam:n.aiTeam)[c._line]||[]).find(x=>x.cardId===c.cardId);g&&(g.used=!0,n.log.push({text:`❌ ${g.name}${i==="ai"?" (IA)":""} exclu !`,type:"info"}))}),ze(a,n,o)}),!0):(n.log.push({text:`❌ Aucun joueur ${s} à exclure`,type:"info"}),ze(a,n,o),!0)},REVIVE_PLAYER:({count:e=1,roles:t=[]},i,n,a)=>{const o=Object.entries(i.homeTeam).filter(([r])=>!t.length||t.includes(r)).flatMap(([r,s])=>s.filter(d=>d.used).map(d=>({...d,_line:r})));return o.length?(qt(o,e,`Choisir ${e} joueur(s) à ressusciter`,n,i,r=>{r.forEach(s=>{const d=(i.homeTeam[s._line]||[]).find(l=>l.cardId===s.cardId);d&&(d.used=!1,i.log.push({text:`💫 ${d.name} ressuscité !`,type:"info"}))}),ze(n,i,a)}),!0):(i.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),ze(n,i,a),!0)},REMOVE_GOAL:({},e)=>(e.aiScore>0?(e.aiScore--,e.log.push({text:"🚫 Dernier but IA annulé !",type:"info"})):e.log.push({text:"🚫 Aucun but à annuler",type:"info"}),!1),ADD_GOAL_DRAW:({},e)=>(e.homeScore===e.aiScore?(e.homeScore++,e.log.push({text:"🎯 But bonus (match nul) !",type:"info"})):e.log.push({text:"🎯 But bonus : non applicable (pas de match nul)",type:"info"}),!1),ADD_SUB:({value:e=1},t)=>(t.maxSubs=(t.maxSubs||3)+e,t.log.push({text:`🔄 +${e} remplacement(s) débloqué(s)`,type:"info"}),!1),CUSTOM:()=>!1};function Mo(e,t,i,n,a){n.usedGc.push(e);const o=n.gcDefs||[],r=o.find(d=>d.name===t)||o.find(d=>{var l;return((l=d.name)==null?void 0:l.toLowerCase().trim())===(t==null?void 0:t.toLowerCase().trim())});let s=!1;if(r!=null&&r.effect_type&&r.effect_type!=="CUSTOM"){const d=So[r.effect_type];d?d(r.effect_params||{},n,i,a)||(s=!0):(a.toast(`Effet "${r.effect_type}" non implémenté`,"error"),s=!0)}else{switch(t){case"Double attaque":n.modifiers.home.doubleAttack=!0,n.log.push({text:"⚡ Double attaque activée !",type:"info"});break;case"Bouclier":n.modifiers.home.shield=!0,n.log.push({text:"🛡️ Bouclier activé !",type:"info"});break;case"Ressusciter":{const d=Object.entries(n.homeTeam).flatMap(([l,c])=>(c||[]).filter(u=>u.used).map(u=>({...u,_line:l})));d.length?(d[0].used=!1,n.log.push({text:`💫 ${d[0].name} ressuscité !`,type:"info"})):n.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"});break}case"Vol de note":n.modifiers.ai.stolenNote=(n.modifiers.ai.stolenNote||0)+1,n.log.push({text:"🎯 -1 à la prochaine attaque IA",type:"info"});break;case"Gel":{const d=[...n.aiTeam.ATT||[],...n.aiTeam.MIL||[]].filter(l=>!l.used);if(d.length){const l=d.sort((c,u)=>qe(u,"ATT")-qe(c,"ATT"))[0];l.used=!0,n.log.push({text:`❄️ ${l.name} (IA) gelé !`,type:"info"})}break}case"Remplacement+":n.maxSubs++,n.log.push({text:"🔄 +1 remplacement débloqué",type:"info"});break}s=!0}w.from("cards").delete().eq("id",e).then(()=>{}),s&&ze(i,n,a)}function Co(e,t,i){const n=Object.values(t.homeTeam).flat().filter(a=>!a.used);if(!n.length){i.toast("Aucun joueur actif à booster","error");return}i.openModal("⚡ Utiliser le Boost",`<div style="margin-bottom:12px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border-radius:10px;padding:12px;text-align:center;color:#000">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(a=>{a.addEventListener("click",()=>{const o=a.dataset.cardId;for(const r of["GK","DEF","MIL","ATT"]){const s=(t.homeTeam[r]||[]).find(d=>d.cardId===o);if(s){s.boost=(s.boost||0)+t.boostCard.value,t.log.push({text:`⚡ Boost +${t.boostCard.value} appliqué à ${s.name}`,type:"info"});break}}t.boostUsed=!0,i.closeModal(),ze(e,t,i)})})}async function kt(e,t,i){var c,u;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:n}=i,a=t.homeScore>t.aiScore,o=t.homeScore===t.aiScore,r=a?"victoire":o?"nul":"defaite",s=jn(t.mode,r);t.matchId&&await w.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:a?n.profile.id:null,home_credits_reward:s,played_at:new Date().toISOString()}).eq("id",t.matchId);const d={credits:(n.profile.credits||0)+s,matches_played:(n.profile.matches_played||0)+1};a?d.wins=(n.profile.wins||0)+1:o?d.draws=(n.profile.draws||0)+1:d.losses=(n.profile.losses||0)+1,await w.from("users").update(d).eq("id",n.profile.id),await i.refreshProfile();const l=document.createElement("div");l.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",l.innerHTML=`
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      ...
      ${It(e.aiTeam,e.formation,null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.1))}
    </div>`,`<button class="btn btn-primary" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}async function Ct(e,t,i,n,a={}){return rn(e,t,i,n,a.myGC||[],a.gcDefs||[],a.isRanked||!1,a.rankedData||null,a.stadiumDef||null,a.onMatchEnd||null,a.mlLeagueId||null,a.mlMatchId||null)}async function Bo(e,t,i){const{data:n}=await w.from("matches").select("home_id,away_id,mode,is_ranked").eq("id",i).single();if(!n){t.toast("Match introuvable","error"),t.navigate("home");return}const a=n.home_id===t.state.user.id;return rn(e,t,i,a,[],[],n.is_ranked||!1,null,null,null,null,null)}async function rn(e,t,i,n,a=[],o=[],r=!1,s=null,d=null,l=null,c=null,u=null){const{state:g,navigate:x,toast:f}=t,p=n?"p1":"p2",m=n?"p2":"p1",b=(a||[]).map(_=>_.id),v=(a||[]).map(_=>({id:_.id,gc_type:_.gc_type,_gcDef:_._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:h}=await w.from("matches").select("*").eq("id",i).single();if(!h){f("Match introuvable","error"),x("home");return}async function $(){var fe,xe;console.log("[PvP] buildGameState match:",{id:h.id,home_deck_id:h.home_deck_id,away_deck_id:h.away_deck_id,mode:h.mode,is_ranked:h.is_ranked});const[{data:_,error:z},{data:A,error:M},{data:I},{data:D}]=await Promise.all([w.rpc("get_deck_for_match",{p_deck_id:h.home_deck_id}),w.rpc("get_deck_for_match",{p_deck_id:h.away_deck_id}),w.from("users").select("id,pseudo,club_name").eq("id",h.home_id).single(),w.from("users").select("id,pseudo,club_name").eq("id",h.away_id).single()]);console.log("[PvP] get_deck_for_match p1:",z==null?void 0:z.message,"p2:",M==null?void 0:M.message,"p1D:",(fe=_==null?void 0:_.starters)==null?void 0:fe.length,"p2D:",(xe=A==null?void 0:A.starters)==null?void 0:xe.length);const q=Z=>{const be=Number(Z.evolution_bonus)||0;return{cardId:Z.card_id,position:Z.position,id:Z.id,firstname:Z.firstname,name:Z.surname_real,country_code:Z.country_code,club_id:Z.club_id,job:Z.job,job2:Z.job2,note_g:(Number(Z.note_g)||0)+(Z.job==="GK"||Z.job2==="GK"&&Number(Z.note_g)>0?be:0),note_d:(Number(Z.note_d)||0)+(Z.job==="DEF"||Z.job2==="DEF"&&Number(Z.note_d)>0?be:0),note_m:(Number(Z.note_m)||0)+(Z.job==="MIL"||Z.job2==="MIL"&&Number(Z.note_m)>0?be:0),note_a:(Number(Z.note_a)||0)+(Z.job==="ATT"||Z.job2==="ATT"&&Number(Z.note_a)>0?be:0),evolution_bonus:be,rarity:Z.rarity,skin:Z.skin,hair:Z.hair,hair_length:Z.hair_length,face:Z.face||null,clubName:Z.club_encoded_name||null,clubLogo:Z.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},G=((_==null?void 0:_.starters)||[]).map(Z=>q(Z)),Y=((A==null?void 0:A.starters)||[]).map(Z=>q(Z)),W=(_==null?void 0:_.formation)||"4-4-2",X=(A==null?void 0:A.formation)||"4-4-2";let oe=Rt(G,W),J=Rt(Y,X);const ce=((_==null?void 0:_.subs)||[]).map(Z=>q(Z)),ae=((A==null?void 0:A.subs)||[]).map(Z=>q(Z)),ne=(_==null?void 0:_.stadium_def)||(n?d:null),ue=(A==null?void 0:A.stadium_def)||(n?null:d);return ne&&(oe=Tt(oe,ne),Gt(ce,ne)),ue&&(J=Tt(J,ue),Gt(ae,ue)),{p1Team:oe,p2Team:J,p1Subs:ce,p2Subs:ae,p1Formation:W,p2Formation:X,p1Name:(I==null?void 0:I.club_name)||(I==null?void 0:I.pseudo)||"Joueur 1",p2Name:(D==null?void 0:D.club_name)||(D==null?void 0:D.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:n?b:[],gc_p2:n?[]:b,gcCardsFull_p1:n?v:[],gcCardsFull_p2:n?[]:v,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:o||[],lastActionAt:new Date().toISOString()}}let y=h.game_state&&Object.keys(h.game_state).length?h.game_state:null;if(console.log("[PvP] init - amIHome:",n,"gameState exists:",!!y,"match.status:",h.status,"home_id:",h.home_id,"away_id:",h.away_id,"myId:",g.profile.id),!y)if(n){y=await $(),console.log("[PvP] home - gameState built, p1Team keys:",Object.keys((y==null?void 0:y.p1Team)||{}));const{data:_}=await w.from("matches").select("game_state").eq("id",i).single();!(_!=null&&_.game_state)||!Object.keys(_.game_state).length?await w.from("matches").update({game_state:y,turn_user_id:h.home_id}).eq("id",i):y=_.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>',console.log("[PvP] away - waiting for home to write game_state...");for(let _=0;_<30&&!y;_++){await new Promise(A=>setTimeout(A,400));const{data:z}=await w.from("matches").select("game_state").eq("id",i).single();z!=null&&z.game_state&&Object.keys(z.game_state).length&&(y=z.game_state),_%5===0&&console.log("[PvP] away - poll",_,"game_state:",!!(z!=null&&z.game_state))}if(!y){f("Erreur de synchronisation","error"),x("home");return}y.gc_p2=b,y.gcCardsFull_p2=v,await w.from("matches").update({game_state:y}).eq("id",i)}let C=!1,E=null,P=!1;const N=new Set,ee=new Set;async function ie(_){var X,oe;try{w.removeChannel(le)}catch{}const z=y[p+"Score"]||0,A=y[m+"Score"]||0;let M,I;_.winner_id?(M=_.winner_id===g.profile.id,I=!1):_.forfeit?(M=z>A,I=!1):(I=z===A,M=z>A);let D="";if(r&&n)try{const{data:J}=await w.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",h.home_id).single(),{data:ce}=await w.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",h.away_id).single();if(J&&ce){const ae=M?1:I?.5:0,ne=1-ae,ue=J.placement_matches<10,fe=ce.placement_matches<10,xe=computeGlicko2(J.mmr,J.mmr_deviation,J.mmr_volatility,ce.mmr,ce.mmr_deviation,ae===1?1:ae===0?0:.5,ue),Z=computeGlicko2(ce.mmr,ce.mmr_deviation,ce.mmr_volatility,J.mmr,J.mmr_deviation,ne===1?1:ne===0?0:.5,fe);await w.rpc("update_mmr_after_ranked",{p_match_id:i,p_winner_id:I?null:M?h.home_id:h.away_id,p_home_id:h.home_id,p_away_id:h.away_id,p_home_delta:xe.delta,p_away_delta:Z.delta,p_home_new_rd:xe.newRd,p_away_new_rd:Z.newRd,p_home_new_vol:xe.newSigma,p_away_new_vol:Z.newSigma});const be=xe.delta,Te=xe.newMmr,Me=getTier(Te),Ne=be>=0?"+":"",Pe=be>=0?"#4caf50":"#ff6b6b",Ge=getTier(J.mmr);D=`
            <div style="background:rgba(255,255,255,0.08);border-radius:14px;padding:14px 20px;text-align:center;min-width:220px">
              ...
              ${Me.id!==Ge.id?`<div style="font-size:20px;font-weight:900;color:${Me.color}">
                    ${Te>J.mmr?"📈":"📉"} ${Ge.emoji} ${Ge.label} → ${Me.emoji} ${Me.label}
                  </div>
                  <div style="font-size:13px;color:rgba(255,255,255,0.5);margin-top:4px">${Te>J.mmr?"Promotion !":"Rétrogradation"}</div>`:`<div style="font-size:36px">${be>=0?"📈":"📉"}</div>
            ...
            </div>`}}catch(J){console.error("[Ranked] MMR update error:",J)}(X=document.getElementById("pvp-end-overlay"))==null||X.remove();const q=document.createElement("div");q.id="pvp-end-overlay",q.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;color:#fff;padding:24px;text-align:center;overflow-y:auto";const G=I?"🤝":M?"🏆":"😞",Y=I?"MATCH NUL":M?"VICTOIRE !":"DÉFAITE",W=I?"#fff":M?"#FFD700":"#ff6b6b";q.innerHTML=`
      ...
      <div style="font-size:18px">${y[p+"Name"]} ${z} – ${A} ${y[m+"Name"]}</div>
      ${_.forfeit?`<div style="font-size:13px;color:rgba(255,255,255,0.5)">${M?"L'adversaire a quitté":"Perdu par forfait"}</div>`:""}
      ${D}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${r?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(q),(oe=q.querySelector("#pvp-end-home"))==null||oe.addEventListener("click",()=>{q.remove(),Ue(e),x(r?"ranked":"home")})}const le=w.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},_=>{const z=_.new;try{if(z.status==="finished"||z.forfeit){if(C)return;C=!0,E&&(clearInterval(E),E=null),z.game_state&&(y=z.game_state),ie(z);return}if(z.game_state){const A=y;y=z.game_state;const M=y._lastGC;if(M&&M.seq&&!ee.has(M.seq)&&(ee.add(M.seq),M.by!==p)){V(M.type,M.by,()=>se());return}const I=A[p+"Score"]||0,D=A[m+"Score"]||0,q=y[p+"Score"]||0,G=y[m+"Score"]||0,Y=q>I,W=G>D;if((Y||W)&&!N.has(y.round)){N.add(y.round);const X=[...y.log||[]].reverse().find(J=>J.isGoal),oe=((X==null?void 0:X.homePlayers)||[]).map(J=>({name:J.name,note:J.note,portrait:J.portrait,job:J.job}));st(oe,q,G,Y,()=>se());return}se()}}catch(A){console.error("[PvP] crash:",A)}}).subscribe();async function j(_){Object.assign(y,_),y.lastActionAt=new Date().toISOString();const{error:z}=await w.from("matches").update({game_state:y}).eq("id",i);z&&f("Erreur de synchronisation","error");try{se()}catch(A){console.error("[PvP] renderPvpScreen crash:",A)}}async function L(){if(C)return;C=!0,E&&(clearInterval(E),E=null);const _=n?h.away_id:h.home_id,z=n?"p2":"p1",A=n?"p1":"p2",M={...y,[z+"Score"]:3,[A+"Score"]:0,phase:"finished"};try{await w.from("matches").update({status:"finished",forfeit:!0,winner_id:_,home_score:M.p1Score||0,away_score:M.p2Score||0,game_state:M}).eq("id",i)}catch{}try{w.removeChannel(le)}catch{}setTimeout(()=>{Ue(e),x("home")},800)}const k={BOOST_STAT:({value:_=1,count:z=1,roles:A=[]},M,I)=>{const D=M[p+"Team"],q=Object.entries(D).filter(([G])=>!A.length||A.includes(G)).flatMap(([G,Y])=>Y.filter(W=>!W.used).map(W=>({...W,_line:G})));if(!q.length){M.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),I(M);return}F(q,z,`Choisir ${z} joueur(s) à booster (+${_})`,G=>{G.forEach(Y=>{const W=(D[Y._line]||[]).find(X=>X.cardId===Y.cardId);W&&(W.boost=(W.boost||0)+_,M.log.push({text:`⚡ +${_} sur ${W.name}`,type:"info"}))}),M[p+"Team"]=D,I(M)})},DEBUFF_STAT:({value:_=1,count:z=1,roles:A=[],target:M="ai"},I,D)=>{const q=M==="home"?p:m,G=I[q+"Team"],Y=M==="home"?"allié":"adverse",W=Object.entries(G).filter(([X])=>!A.length||A.includes(X)).flatMap(([X,oe])=>oe.map(J=>({...J,_line:X})));if(!W.length){I.log.push({text:`🎯 Aucun joueur ${Y}`,type:"info"}),D(I);return}F(W,z,`Choisir ${z} joueur(s) ${Y}(s) (-${_})`,X=>{X.forEach(oe=>{const J=(G[oe._line]||[]).find(ce=>ce.cardId===oe.cardId);J&&(J.boost=(J.boost||0)-_,I.log.push({text:`🎯 -${_} sur ${J.name}`,type:"info"}))}),I[q+"Team"]=G,D(I)})},GRAY_PLAYER:({count:_=1,roles:z=[],target:A="ai"},M,I)=>{const D=A==="home"?p:m,q=M[D+"Team"],G=A==="home"?"allié":"adverse",Y=Object.entries(q).filter(([W])=>!z.length||z.includes(W)).flatMap(([W,X])=>X.filter(oe=>!oe.used).map(oe=>({...oe,_line:W})));if(!Y.length){M.log.push({text:`❌ Aucun joueur ${G}`,type:"info"}),I(M);return}F(Y,_,`Choisir ${_} joueur(s) ${G}(s) à exclure`,W=>{const X="usedCardIds_"+D,oe=new Set(M[X]||[]);W.forEach(J=>{const ce=(q[J._line]||[]).find(ae=>ae.cardId===J.cardId);ce&&(ce.used=!0,oe.add(J.cardId),M.log.push({text:`❌ ${ce.name} exclu !`,type:"info"}))}),M[X]=[...oe],M[D+"Team"]=q,I(M)})},REVIVE_PLAYER:({count:_=1,roles:z=[]},A,M)=>{const I=A[p+"Team"],D=Object.entries(I).filter(([q])=>!z.length||z.includes(q)).flatMap(([q,G])=>G.filter(Y=>Y.used).map(Y=>({...Y,_line:q})));if(!D.length){A.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),M(A);return}F(D,_,`Choisir ${_} joueur(s) à ressusciter`,q=>{q.forEach(G=>{const Y=(I[G._line]||[]).find(W=>W.cardId===G.cardId);Y&&(Y.used=!1,A.log.push({text:`💫 ${Y.name} ressuscité !`,type:"info"}))}),A[p+"Team"]=I,M(A)})},REMOVE_GOAL:({},_,z)=>{const A=m+"Score";_[A]>0?(_[A]--,_.log.push({text:"🚫 Dernier but annulé !",type:"info"})):_.log.push({text:"🚫 Aucun but à annuler",type:"info"}),z(_)},ADD_GOAL_DRAW:({},_,z)=>{_[p+"Score"]===_[m+"Score"]?(_[p+"Score"]++,_.log.push({text:"🎯 But bonus !",type:"info"})):_.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),z(_)},ADD_SUB:({value:_=1},z,A)=>{z.maxSubs=(z.maxSubs||3)+_,z.log.push({text:`🔄 +${_} remplacement(s)`,type:"info"}),A(z)},CUSTOM:({},_,z)=>z(_)};function F(_,z,A,M){const I=document.createElement("div");I.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let D=[];function q(){var Y,W;const G=_.map(X=>{const oe={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[X._line]||"#555",J=qe(X,X._line)+(X.boost||0),ae=D.find(ue=>ue.cardId===X.cardId)?"#FFD700":"rgba(255,255,255,0.25)",ne=X.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${X.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${ae};background:${oe};overflow:hidden;cursor:pointer;${ne}">
        ...
        <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
          ...
          <span style="font-size:12px;color:rgba(255,255,255,0.5)">${D.length}/${z}</span>
        ...
        <div style="padding:12px;background:rgba(0,0,0,0.4);flex-shrink:0">
          <button id="pp-confirm" ${D.length===0?'disabled style="opacity:0.4"':""} style="width:100%;padding:13px;border-radius:10px;border:none;background:#7a28b8;color:#fff;font-size:15px;font-weight:900;cursor:pointer">
            ✅ Confirmer (${D.length}/${z})
          </button>
        </div>`,(Y=I.querySelector("#pp-close"))==null||Y.addEventListener("click",()=>I.remove()),I.querySelectorAll(".pp-item").forEach(X=>{X.addEventListener("click",()=>{const oe=X.dataset.cid,J=_.find(ae=>ae.cardId===oe),ce=D.findIndex(ae=>ae.cardId===oe);J&&(ce>-1?D.splice(ce,1):D.length<z&&D.push(J),q())})}),(W=I.querySelector("#pp-confirm"))==null||W.addEventListener("click",()=>{I.remove(),M(D)})}q(),document.body.appendChild(I)}async function re(_,z){const M=(y["gcCardsFull_"+p]||[]).find(G=>G.id===_),I=(M==null?void 0:M._gcDef)||(y.gcDefs||[]).find(G=>{var Y;return G.name===z||((Y=G.name)==null?void 0:Y.toLowerCase().trim())===(z==null?void 0:z.toLowerCase().trim())}),D=[...y["usedGc_"+p]||[],_],q={type:z,by:p,seq:(y._gcAnimSeq||0)+1};ee.add(q.seq),V(z,p,async()=>{if(I!=null&&I.effect_type&&I.effect_type!=="CUSTOM"){const Y=k[I.effect_type];if(Y){const W={...y};Y(I.effect_params||{},W,async X=>{X["usedGc_"+p]=D,X._lastGC=q,X._gcAnimSeq=q.seq,await j(X)});return}}const G={...y};switch(z){case"Remplacement+":G.maxSubs=(G.maxSubs||3)+1,G.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const Y=m+"Score";G[Y]>0&&(G[Y]--,G.log.push({text:"🚫 But annulé",type:"info"}));break}}G["usedGc_"+p]=D,G._lastGC=q,G._gcAnimSeq=q.seq,await j(G)})}function pe(_,z){const A="usedCardIds_"+_,M=new Set(y[A]||[]);z.forEach(I=>M.add(I)),y[A]=[...M]}function de(){for(const _ of["p1","p2"]){const z=new Set(y["usedCardIds_"+_]||[]),A=y[_+"Team"];if(A)for(const M of["GK","DEF","MIL","ATT"])(A[M]||[]).forEach(I=>{I.used=z.has(I.cardId)})}}function se(){var Ve,nt,yi,bi,hi,vi;if(y.phase==="reveal")return Le();if(y.phase==="midfield")return Q();if(y.phase==="finished")return Fe();const _=y[p+"Team"],z=y[m+"Team"];de();const A=y[p+"Score"],M=y[m+"Score"],I=y[p+"Name"],D=y[m+"Name"],q=y.phase===p+"-attack",G=y.phase===p+"-defense",Y=Array.isArray(y["selected_"+p])?y["selected_"+p]:[],W=Y.map(te=>te.cardId),X=window.innerWidth>=700,oe=y[p+"Subs"]||[],J=y["usedSubIds_"+p]||[],ce=oe.filter(te=>!J.includes(te.cardId)),ae=y["gcCardsFull_"+p]||[],ne=y["usedGc_"+p]||[],ue=ae.filter(te=>!ne.includes(te.id)),fe=y.boostOwner===p&&!y.boostUsed,xe=!["GK","DEF","MIL","ATT"].some(te=>(z[te]||[]).some(ge=>!ge.used)),Z=[..._.MIL||[],..._.ATT||[]].filter(te=>!te.used),be=q&&xe&&Z.length===0?[..._.GK||[],..._.DEF||[]].filter(te=>!te.used).map(te=>te.cardId):[];function Te(te,ge,Ie){var $i,Ei;const ye=te._gcDef,ot={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ye==null?void 0:ye.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",lt={purple:"#b06ce0",light_blue:"#00d4ef"}[ye==null?void 0:ye.color]||"#b06ce0",ht=(ye==null?void 0:ye.name)||te.gc_type,vt=(ye==null?void 0:ye.effect)||(($i=Se[te.gc_type])==null?void 0:$i.desc)||"",wi=ye!=null&&ye.image_url?`/icons/${ye.image_url}`:null,un=((Ei=Se[te.gc_type])==null?void 0:Ei.icon)||"⚡",_i=Math.round(Ie*.22),ki=Math.round(Ie*.22),Vt=Ie-_i-ki,fn=ht.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${te.id}" data-gc-type="${te.gc_type}"
      ...
      </div>`}function Me(te,ge){return`<div id="pvp-boost-card"
        style="box-sizing:border-box;width:${te}px;height:${ge}px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:2px solid #87CEEB;border-radius:10px;cursor:pointer;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:${Math.round(ge*.04)}px;text-align:center;flex-shrink:0">
      ...
      </div>`}const Ne=(te,ge)=>ge?Me(130,175):Te(te,130,175),Pe=(te,ge)=>ge?Me(68,95):Te(te,68,95),Ge=X?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",bt=q?ke(p)?`<button id="pvp-action" style="${Ge};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${Y.length===0?"disabled":""}>⚔️ ATTAQUEZ <span id="pvp-timer"></span></button>`:`<button id="pvp-action" data-pass="1" style="${Ge};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER <span id="pvp-timer"></span></button>`:G?`<button id="pvp-action" style="${Ge};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${Y.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="pvp-timer"></span></button>`:`<div style="font-size:11px;color:rgba(255,255,255,0.3);text-align:center;padding:4px">⏳ Tour de ${D}</div>`,Be=q&&!ke(p)?'<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">Aucun attaquant — passez la main</div>':q||G?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${Y.length}/3 sélectionné(s)</div>`:"",Oe=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${X?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
      ${ce.length===0?'<div style="font-size:7px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':ce.map(te=>`<div class="pvp-sub-btn" data-sub-id="${te.cardId}" style="cursor:pointer;flex-shrink:0">${Je(te,X?76:44,X?100:58)}</div>`).join("")}
    </div>`,Re=q?"attack":G?"defense":"idle",tt=`<div style="overflow:hidden;min-width:0;flex:1;min-height:0;display:flex;flex-direction:column" id="match-field">
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${At(_,y[p+"Formation"],Re,W,We(),rt(),be)}
      </div>
    </div>`,Ke=(()=>{var ge,Ie;if(G&&((ge=y.pendingAttack)!=null&&ge.players)){const ye=y.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
        ...
        </div>`}const te=(y.log||[]).slice(-1)[0];return te?'<div style="padding:2px 4px">'+nn(te)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})(),it=`
      <div style="display:flex;align-items:center;padding:8px 10px;background:rgba(0,0,0,0.5);gap:6px;flex-shrink:0">
        ...
        <div style="flex:1;display:flex;align-items:center;justify-content:center;gap:8px">
          ...
          <span style="font-size:26px;font-weight:900;color:#FFD700;letter-spacing:2px">${A} – ${M}</span>
      ...
      </div>`;function De(){const te=e.querySelector(".match-screen");if(!te)return;const ge=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);te.style.height=ge+"px",te.style.minHeight=ge+"px",te.style.maxHeight=ge+"px",te.style.overflow="hidden";const Ie=e.querySelector("#mobile-action-bar"),ye=e.querySelector("#mobile-play-area");Ie&&ye&&(ye.style.paddingBottom=Ie.offsetHeight+"px")}if(De(),setTimeout(De,120),setTimeout(De,400),P||(P=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",De),window.visualViewport.addEventListener("scroll",De)),window.addEventListener("resize",De)),function(){const ge=e.querySelector(".terrain-wrapper svg");ge&&(ge.removeAttribute("width"),ge.removeAttribute("height"),ge.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",ge.setAttribute("viewBox","-26 -26 352 352"),ge.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),e.querySelectorAll(".match-slot-hit").forEach(te=>{te.addEventListener("click",()=>{if(!q&&!G)return;const ge=te.dataset.cardId,Ie=te.dataset.role,ye=(_[Ie]||[]).find(vt=>vt.cardId===ge);if(!ye||ye.used)return;const ot=be.includes(ge);if(q&&!["MIL","ATT"].includes(Ie)&&!ot)return;Array.isArray(y["selected_"+p])||(y["selected_"+p]=[]);const lt=y["selected_"+p],ht=lt.findIndex(vt=>vt.cardId===ge);ht>-1?lt.splice(ht,1):lt.length<3&&lt.push({...ye,_role:Ie}),se()})}),e.querySelectorAll(".match-used-hit").forEach(te=>{te.addEventListener("click",()=>R(te.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(te=>{te.addEventListener("click",()=>R())}),(Ve=e.querySelector("#pvp-sub-open"))==null||Ve.addEventListener("click",()=>R()),e.querySelectorAll(".pvp-gc-mini").forEach(te=>{te.addEventListener("click",()=>T(te.dataset.gcId,te.dataset.gcType))}),(nt=e.querySelector("#pvp-boost-card"))==null||nt.addEventListener("click",()=>S()),(yi=e.querySelector("#pvp-action"))==null||yi.addEventListener("click",te=>{q?te.currentTarget.dataset.pass==="1"||!ke(p)?O():$e():G&&_e()}),(bi=e.querySelector("#pvp-quit"))==null||bi.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&L()}),(hi=e.querySelector("#pvp-view-opp"))==null||hi.addEventListener("click",()=>U()),(vi=e.querySelector("#pvp-toggle-history"))==null||vi.addEventListener("click",()=>B()),E&&(clearInterval(E),E=null),(q||G)&&!C){let te=30,ge=!1;const Ie=()=>document.getElementById("pvp-timer"),ye=()=>{Ie()&&(Ie().textContent=te+"s",Ie().style.color=ge?"#ff4444":"#fff")};ye(),E=setInterval(()=>{te--,te<0?ge?(clearInterval(E),E=null,q&&!ke(p)?O():L()):(ge=!0,te=15,ye()):ye()},1e3)}}function Le(){He(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
      <div style="font-size:20px;font-weight:900;color:#ff6b6b">${y[m+"Name"]||"Adversaire"}</div>
      <div style="width:min(90vw,420px)">${It(y[m+"Team"],y[m+"Formation"],null,[],We(),rt())}</div>
    </div>`,p==="p1"&&setTimeout(async()=>{await j({phase:"midfield"})},5e3)}let K=!1;function Q(){if(K)return;const _=y[p+"Team"].MIL||[],z=y[m+"Team"].MIL||[],A=milScore(_)+milLinks(_),M=milScore(z)+milLinks(z),I=A>=M;function D(ae,ne,ue){return`<div id="duel-row-${ue}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
        ...
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${ae.map((fe,xe)=>{const Z=xe<ae.length-1?yt(fe,ae[xe+1]):null,be=!Z||Z==="#ff3333"||Z==="#cc2222",Te=Z==="#00ff88"?"+2":Z==="#FFD700"?"+1":"";return`<div class="duel-card duel-card-${ue}" data-idx="${xe}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease,transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
            ...
            ${xe<ae.length-1?`<div class="duel-link duel-link-${ue}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${be?"rgba(255,255,255,0.12)":Z};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${be?"none":`0 0 8px ${Z}`}">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      ${D(z,y[m+"Name"]||"Adversaire","opp")}
      <div id="duel-shock" style="position:absolute;left:50%;top:50%;width:120px;height:120px;border-radius:50%;border:6px solid #FFD700;opacity:0;pointer-events:none"></div>
      <div id="pvp-duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center,rgba(10,61,30,.4),rgba(10,61,30,.92))"></div>
    </div>`;const q=(ae,ne)=>e.querySelectorAll(ae).forEach((ue,fe)=>{setTimeout(()=>{ue.style.opacity="1",ue.style.transform="translateY(0) scale(1)"},ne+fe*90)});q(".duel-card-me",150),q(".duel-card-opp",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((ae,ne)=>setTimeout(()=>{ae.style.opacity="1"},ne*70)),900),setTimeout(()=>{const ae=e.querySelector("#pvp-vs");ae&&(ae.style.opacity="1",ae.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(ne=>ne.style.opacity="1")},1250);function G(ae,ne,ue){const fe=document.getElementById(ae);if(!fe)return;const xe=performance.now(),Z=be=>{const Te=Math.min(1,(be-xe)/ue);fe.textContent=Math.round(ne*(1-Math.pow(1-Te,3))),Te<1?requestAnimationFrame(Z):fe.textContent=ne};requestAnimationFrame(Z)}setTimeout(()=>{G("pvp-score-me",A,800),G("pvp-score-opp",M,800)},1500);const Y=y.p1Team.MIL||[],W=y.p2Team.MIL||[],X=milScore(Y)+milLinks(Y),oe=milScore(W)+milLinks(W),J=X>=oe?"p1":"p2";let ce=y.boostValue;ce==null&&(ce=tn(),y.boostValue=ce,y.boostOwner=J,y.boostUsed=!1),K=!0,setTimeout(()=>{const ae=e.querySelector("#duel-row-"+(I?"me":"opp")),ne=e.querySelector("#duel-row-"+(I?"opp":"me")),ue=document.getElementById("pvp-score-me"),fe=document.getElementById("pvp-score-opp"),xe=I?ue:fe,Z=I?fe:ue;xe&&(xe.style.fontSize="80px",xe.style.color=I?"#FFD700":"#ff6b6b",xe.style.animation="duelPulse .5s ease"+(I?",duelGlow 1.5s ease infinite .5s":"")),Z&&(Z.style.opacity="0.25"),setTimeout(()=>{ae&&(ae.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",ae.style.zIndex="5"),setTimeout(()=>{const be=document.getElementById("duel-shock");be&&(be.style.animation="shockwave .5s ease-out forwards"),ne&&(ne.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var Ne;const be=document.getElementById("pvp-duel-finale");if(!be)return;const Te=y.boostOwner===p?'<div style="background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:3px solid #cdeffd;border-radius:18px;padding:20px 34px;text-align:center;animation:boostFlipIn .7s cubic-bezier(.34,1.56,.64,1) both;box-shadow:0 10px 36px rgba(135,206,235,.5)"><div style="font-size:10px;color:rgba(0,0,0,.6);letter-spacing:2px;text-transform:uppercase;margin-bottom:6px;font-weight:700">Carte Boost obtenue</div><div style="font-size:46px;line-height:1">⚡</div><div style="font-size:50px;font-weight:900;color:#063;line-height:1.1">+'+ce+`</div><div style="font-size:10px;color:rgba(0,0,0,.55);margin-top:4px">Applicable sur n'importe quel joueur</div></div>`:"",Me=p==="p1"?'<button id="pvp-start-match" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">▶ Commencer le match</button>':`<div style="font-size:14px;color:rgba(255,255,255,0.5);text-align:center;margin-top:8px;animation:fadeUp .4s ease both">⏳ En attente de l'adversaire...</div>`;be.innerHTML='<div style="font-size:22px;font-weight:900;color:#fff;text-align:center;animation:fadeUp .4s ease both;text-shadow:0 2px 12px rgba(0,0,0,.5)">'+(I?"⚽ "+y[p+"Name"]+"<br>gagne le milieu et attaque !":"😔 "+y[m+"Name"]+"<br>gagne l'engagement et attaque !")+"</div>"+Te+Me,be.style.transition="opacity .45s ease",be.style.opacity="1",be.style.pointerEvents="auto",(Ne=document.getElementById("pvp-start-match"))==null||Ne.addEventListener("click",async()=>{const Pe=J;await j({phase:Pe+"-attack",attacker:Pe,round:1,boostValue:ce,boostUsed:!1,boostOwner:Pe})})},600)},700)},2800)}function V(_,z,A){var ne,ue;const M=(y.gcDefs||[]).find(fe=>{var xe;return fe.name===_||((xe=fe.name)==null?void 0:xe.toLowerCase().trim())===(_==null?void 0:_.toLowerCase().trim())}),I={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[M==null?void 0:M.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",D={purple:"#b06ce0",light_blue:"#00d4ef"}[M==null?void 0:M.color]||"#b06ce0",q=(M==null?void 0:M.name)||_,G=(M==null?void 0:M.effect)||((ne=Se[_])==null?void 0:ne.desc)||"",Y=M!=null&&M.image_url?`/icons/${M.image_url}`:null,W=((ue=Se[_])==null?void 0:ue.icon)||"⚡",oe=z===p?"Vous":y[z+"Name"]||"Adversaire",J=document.createElement("div");J.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",J.innerHTML=`
      <style>
      ...
      <div style="width:200px;border-radius:18px;border:3px solid ${D};background:${I};display:flex;flex-direction:column;overflow:hidden;animation:gcFlipIn .7s cubic-bezier(.34,1.56,.64,1) both,gcGlow 1.8s ease infinite .7s">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(J);let ce=!1;const ae=()=>{ce||(ce=!0,J.remove(),setTimeout(()=>A&&A(),50))};J.addEventListener("click",ae),setTimeout(ae,3e3)}function T(_,z){var J,ce,ae,ne;const M=(y["gcCardsFull_"+p]||[]).find(ue=>ue.id===_),I=M==null?void 0:M._gcDef,D={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[I==null?void 0:I.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",q={purple:"#b06ce0",light_blue:"#00d4ef"}[I==null?void 0:I.color]||"#b06ce0",G=(I==null?void 0:I.name)||z,Y=(I==null?void 0:I.effect)||((J=Se[z])==null?void 0:J.desc)||"Carte spéciale.",W=I!=null&&I.image_url?`/icons/${I.image_url}`:null,X=((ce=Se[z])==null?void 0:ce.icon)||"⚡",oe=document.createElement("div");oe.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",oe.innerHTML=`
      ...
      </div>`,document.body.appendChild(oe),(ae=oe.querySelector("#pvp-gc-back"))==null||ae.addEventListener("click",()=>oe.remove()),(ne=oe.querySelector("#pvp-gc-use"))==null||ne.addEventListener("click",()=>{oe.remove(),re(_,z)})}function S(){var M;const _=y[p+"Team"],z=Object.entries(_).flatMap(([I,D])=>(D||[]).filter(q=>!q.used).map(q=>({...q,_line:I})));if(!z.length)return;const A=document.createElement("div");A.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",A.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
        <div style="flex:1;font-size:14px;font-weight:700;color:#fff">⚡ Choisir un joueur pour +${y.boostValue}</div>
        <button id="bp-close" style="background:none;border:none;color:rgba(255,255,255,0.5);font-size:22px;cursor:pointer">✕</button>
      </div>
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${z.map(I=>{const D={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[I._line]||"#555",q=qe(I,I._line)+(I.boost||0);return`<div class="bp-item" data-cid="${I.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${D};overflow:hidden;cursor:pointer">
      ...
      </div>`,document.body.appendChild(A),(M=A.querySelector("#bp-close"))==null||M.addEventListener("click",()=>A.remove()),A.querySelectorAll(".bp-item").forEach(I=>{I.addEventListener("click",async()=>{const D=I.dataset.cid,q=z.find(Y=>Y.cardId===D);if(!q)return;const G=(_[q._line]||[]).find(Y=>Y.cardId===D);G&&(G.boost=(G.boost||0)+y.boostValue),A.remove(),await j({[p+"Team"]:_,boostUsed:!0})})})}function R(_=null){var ce,ae;if(!(y.phase===p+"-attack")){f("Remplacement uniquement avant votre attaque","warning");return}const A=y[p+"Team"],M=y["usedSubIds_"+p]||[],I=y.maxSubs||3;if(M.length>=I){f(`Maximum ${I} remplacements atteint`,"warning");return}const D=Object.entries(A).flatMap(([ne,ue])=>(ue||[]).filter(fe=>fe.used).map(fe=>({...fe,_line:ne}))),q=(y[p+"Subs"]||[]).filter(ne=>!M.includes(ne.cardId));if(!D.length){f("Aucun joueur utilisé à remplacer","warning");return}if(!q.length){f("Aucun remplaçant disponible","warning");return}let G=Math.max(0,D.findIndex(ne=>ne.cardId===_));const Y=((ce=D[G])==null?void 0:ce._line)||((ae=D[G])==null?void 0:ae.job);let W=Math.max(0,q.findIndex(ne=>ne.job===Y)),X=!1;const oe=document.createElement("div");oe.id="pvp-sub-overlay",oe.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function J(){var Te,Me,Ne,Pe,Ge,bt;const ne=D[G],ue=q[W],fe=Math.min(130,Math.round((window.innerWidth-90)/2)),xe=Math.round(fe*1.35),Z=Be=>`background:rgba(255,255,255,0.12);border:none;color:${Be?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Be?"default":"pointer"};flex-shrink:0`;oe.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
        <div style="flex:1;font-size:15px;font-weight:900;color:#fff">🔄 Remplacement (${M.length}/${I})</div>
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        ...
        <div id="pout-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
          <div style="font-size:9px;color:#ff6b6b;letter-spacing:2px;text-transform:uppercase;font-weight:700">Joueur qui sort</div>
          <button id="pout-up" style="${Z(G===0)}" ${G===0?"disabled":""}>▲</button>
          <div>${ne?Je({...ne,used:!1,boost:0},fe,xe):"<div>—</div>"}</div>
          <button id="pout-down" style="${Z(G>=D.length-1)}" ${G>=D.length-1?"disabled":""}>▼</button>
          <div style="font-size:10px;color:rgba(255,255,255,0.35)">${G+1}/${D.length}</div>
        </div>
      </div>
      <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
        <button id="psub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
      </div>`,(Te=oe.querySelector("#psub-close"))==null||Te.addEventListener("click",()=>oe.remove()),(Me=oe.querySelector("#pout-up"))==null||Me.addEventListener("click",()=>{G>0&&(G--,J())}),(Ne=oe.querySelector("#pout-down"))==null||Ne.addEventListener("click",()=>{G<D.length-1&&(G++,J())}),(Pe=oe.querySelector("#pin-up"))==null||Pe.addEventListener("click",()=>{W>0&&(W--,J())}),(Ge=oe.querySelector("#pin-down"))==null||Ge.addEventListener("click",()=>{W<q.length-1&&(W++,J())});const be=(Be,Oe,Re,tt)=>{const Ke=oe.querySelector("#"+Be);if(!Ke)return;let it=0;Ke.addEventListener("touchstart",De=>{it=De.touches[0].clientY},{passive:!0}),Ke.addEventListener("touchend",De=>{const Ve=De.changedTouches[0].clientY-it;if(Math.abs(Ve)<30)return;const nt=Oe();Ve<0&&nt<tt-1?(Re(nt+1),J()):Ve>0&&nt>0&&(Re(nt-1),J())},{passive:!0})};be("pin-panel",()=>W,Be=>W=Be,q.length),be("pout-panel",()=>G,Be=>G=Be,D.length),(bt=oe.querySelector("#psub-confirm"))==null||bt.addEventListener("click",async Be=>{if(Be.preventDefault(),Be.stopPropagation(),X)return;X=!0;const Oe=D[G],Re=q[W];if(!Oe||!Re)return;const tt=Oe._line,Ke=(A[tt]||[]).findIndex(Ve=>Ve.cardId===Oe.cardId);if(Ke===-1){f("Erreur : joueur introuvable","error"),oe.remove();return}const it={...Re,_line:tt,position:Oe.position,used:!1,boost:0};A[tt].splice(Ke,1,it);const De=[...M,Re.cardId];oe.remove(),H(Oe,Re,async()=>{await j({[p+"Team"]:A,[m+"Team"]:y[m+"Team"],["usedSubIds_"+p]:De})})})}document.body.appendChild(oe),J()}function H(_,z,A){const M={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},I=document.createElement("div");I.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const D=(Y,W,X)=>`<div style="text-align:center">
      ...
      </div>`,document.body.appendChild(z),(A=z.querySelector("#pvp-hist-close"))==null||A.addEventListener("click",()=>z.remove())}async function O(){if(y.phase!==p+"-attack")return;const _=p==="p1"?"p2":"p1",z=(y.round||0)+1,A=[...y.log||[]];A.push({type:"info",text:`⏭️ ${y[p+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const M=he(y),I=ke(_),D=M||!I?"finished":_+"-attack";await j({["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:D,attacker:_,round:z,log:A}),D==="finished"&&await je(y)}async function $e(){const _=y[p+"Team"],z=!["GK","DEF","MIL","ATT"].some(D=>(y[m+"Team"][D]||[]).some(q=>!q.used)),A=(y["selected_"+p]||[]).map(D=>{const q=(_[D._role]||[]).find(ae=>ae.cardId===D.cardId)||D,G=z&&["GK","DEF"].includes(D._role),Y=_[D._role]||[],W=Y.findIndex(ae=>ae.cardId===D.cardId),X=xt(Y.length),oe=W>=0?X[W]:q._col??1,J=y.stadiumDef||y.homeStadiumDef||null,ce=q.stadiumBonus||J&&(J.club_id&&String(q.club_id)===String(J.club_id)||J.country_code&&q.country_code===J.country_code)||!1;return{...q,_line:D._role,_col:oe,stadiumBonus:ce,...G?{note_a:Math.max(1,Number(q.note_a)||0)}:{}}});if(!A.length)return;const M=pi(A,y.modifiers[p]||{});pe(p,A.map(D=>D.cardId)),A.forEach(D=>{const q=(_[D._role]||[]).find(G=>G.cardId===D.cardId);q&&(q.used=!0)}),y["selected_"+p]=[],se();const I=[...y.log||[]];if(z){const D=(y[p+"Score"]||0)+1,q=A.map(oe=>histPlayer(oe));I.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:q,homeTotal:M.total,aiTotal:0});const G=(y.round||0)+1,Y=p==="p1"?"p2":"p1",W={...y,[p+"Team"]:_,[p+"Score"]:D},X=he(W);N.add(G),st(q,D,y[m+"Score"]||0,!0,async()=>{await j({[p+"Team"]:_,[p+"Score"]:D,["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:X?"finished":Y+"-attack",attacker:Y,round:G,log:I}),X&&await je({...y,[p+"Score"]:D})});return}I.push({type:"pending",text:`⚔️ ${y[p+"Name"]} attaque (${M.total})`}),await j({[p+"Team"]:_,[m+"Team"]:y[m+"Team"],pendingAttack:{...M,players:A,side:p},["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},phase:m+"-defense",log:I})}async function _e(){const _=y[p+"Team"],z=(y["selected_"+p]||[]).map(ne=>{const ue=(_[ne._role]||[]).find(Te=>Te.cardId===ne.cardId)||ne,fe=_[ne._role]||[],xe=fe.findIndex(Te=>Te.cardId===ne.cardId),Z=xt(fe.length),be=xe>=0?Z[xe]:ue._col??1;return{...ue,_line:ne._role,_col:be}}),A=ui(z,y.modifiers[p]||{});pe(p,z.map(ne=>ne.cardId)),z.forEach(ne=>{const ue=(_[ne._role]||[]).find(fe=>fe.cardId===ne.cardId);ue&&(ue.used=!0)}),y["selected_"+p]=[],se();const M=fi(y.pendingAttack.total,A.total,y.modifiers[p]||{}),I=y.pendingAttack.side,D=M==="attack"||(M==null?void 0:M.goal),q=I==="p1"?"p2":"p1",G=(y.round||0)+1,Y=(y.pendingAttack.players||[]).map(ne=>histPlayer(ne)),W=[...y.log||[]];W.push({type:"duel",isGoal:D,homeScored:D&&I===p,text:D?`⚽ BUT de ${y[I+"Name"]} ! (${y.pendingAttack.total} vs ${A.total})`:`✋ Attaque stoppée (${y.pendingAttack.total} vs ${A.total})`,homePlayers:Y,aiPlayers:z.map(ne=>histPlayer(ne)),homeTotal:y.pendingAttack.total,aiTotal:A.total});const X=D?(y[I+"Score"]||0)+1:y[I+"Score"]||0,oe={...y,[p+"Team"]:_,[I+"Score"]:X},J=he(oe),ce=J?"finished":q+"-attack",ae=async()=>{await j({[p+"Team"]:_,[m+"Team"]:y[m+"Team"],[I+"Score"]:X,["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:ce,attacker:q,round:G,log:W}),(ce==="finished"||J)&&await je({...y,[I+"Score"]:X})};if(D){const ne=I===p,ue=ne?X:y[p+"Score"]||0,fe=ne?y[m+"Score"]||0:X;N.add(G),st(Y,ue,fe,ne,ae)}else await ae()}function we(_){return["MIL","ATT"].some(z=>(_[z]||[]).some(A=>!A.used))}function me(_){return["GK","DEF","MIL","ATT"].some(z=>(_[z]||[]).some(A=>!A.used))}function Ee(_){return me(_)&&!we(_)}function ke(_){const z=y[_+"Team"],A=y[(_==="p1"?"p2":"p1")+"Team"];return!!(we(z)||!me(A)&&Ee(z))}function he(_){const z=["MIL","ATT"].some(W=>(_.p1Team[W]||[]).some(X=>!X.used)),A=["MIL","ATT"].some(W=>(_.p2Team[W]||[]).some(X=>!X.used)),M=me(_.p1Team),I=me(_.p2Team);return!z&&!(!I&&M)&&(!A&&!(!M&&I))}function Ce(_){const z=_.p1Score||0,A=_.p2Score||0;return z===A?null:z>A?h.home_id:h.away_id}async function je(_){try{const z=Ce(_),A=z?h.home_id===z?h.away_id:h.home_id:null;await w.from("matches").update({status:"finished",winner_id:z,home_score:_.p1Score||0,away_score:_.p2Score||0}).eq("id",i),z&&A&&updateEvolutiveCards(z,A).catch(()=>{})}catch(z){console.error("[PvP] finishMatch:",z)}}function Fe(){var I;const _=y[p+"Score"],z=y[m+"Score"],A=_>z,M=_===z;He(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      <div style="font-size:64px">${A?"🏆":M?"🤝":"😤"}</div>
      <div style="font-size:24px;font-weight:900;color:#fff">${A?"Victoire !":M?"Match nul":"Défaite"}</div>
      <div style="font-size:32px;font-weight:900;color:#FFD700">${_} - ${z}</div>
      <button id="pvp-home" style="padding:16px 40px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">🏠 Retour</button>
    </div>`,(I=document.getElementById("pvp-home"))==null||I.addEventListener("click",()=>{try{w.removeChannel(le)}catch{}Ue(e),x("home")})}se()}async function oi(e,t,i=!1){var s,d;const{state:n,navigate:a,toast:o}=t,r=i&&((d=(s=t==null?void 0:t.state)==null?void 0:s.params)==null?void 0:d.rankedData)||null;await Mt(e,t,i?"ranked":"random",async({deckId:l,formation:c,starters:u,subsRaw:g,gcCardsEnriched:x,gcDefs:f,stadiumDef:p})=>{const m=x||[];He(e),await Do(e,t,l,c,u,g,m,f,p,i,r)})}async function qo(e,t,i){return Bo(e,t,i)}async function Do(e,t,i,n,a,o,r=[],s=[],d=null,l=!1,c=null){var P,N;const{state:u,navigate:g,toast:x}=t,f=(c==null?void 0:c.mmr)??((P=u.profile)==null?void 0:P.mmr)??1e3,p=l?di(f):null,m=l?"linear-gradient(135deg, #1a0a2e 0%, #2d1060 50%, #1a0a2e 100%)":"linear-gradient(135deg, #0a1a2e 0%, #0d3d1e 100%)",b=l?(p==null?void 0:p.color)||"#D4A017":"#FFD700";e.innerHTML=`
    ...
    </div>`,(N=document.getElementById("cancel-mm"))==null||N.addEventListener("click",()=>{$==null||$.unsubscribe(),Ue(e),g("home")});const v=u.user.id,{data:h}=await w.from("matchmaking_queue").select("id").eq("user_id",v).single();h||await w.from("matchmaking_queue").insert({user_id:v,deck_id:i,formation:n,mmr:f,is_ranked:l});let $,y=!1;const C=async()=>{if(y)return;const{data:ee}=await w.from("matches").select("id, home_id, away_id").or(`home_id.eq.${v},away_id.eq.${v}`).eq("status","in_progress").order("created_at",{ascending:!1}).limit(1).maybeSingle();if(ee){y=!0,$==null||$.unsubscribe(),await w.from("matchmaking_queue").delete().eq("user_id",v);const ie=ee.home_id===v;await Ct(e,t,ee.id,ie,{myGC:r,gcDefs:s,stadiumDef:d,isRanked:l,rankedData:c,onMatchEnd:l?Fo:null})}};if(await C(),y)return;$=w.channel(`mm_${v}`).on("postgres_changes",{event:"INSERT",schema:"public",table:"matches",filter:`home_id=eq.${v}`},C).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`away_id=eq.${v}`},C).subscribe();const E=setInterval(C,3e3);setTimeout(()=>{clearInterval(E),$==null||$.unsubscribe()},12e4)}async function Fo(e,t){const{state:i,toast:n}=t,{winnerId:a,homeId:o,awayId:r,homeScore:s,awayScore:d,matchId:l}=e,c=i.user.id,u=o===c,g=a===c,{data:x}=await w.from("users").select("mmr, mmr_rd, mmr_v").eq("id",c).single(),{data:f}=await w.from("users").select("mmr, mmr_rd, mmr_v").eq("id",u?r:o).single();if(x&&f){const p=gn(x.mmr,x.mmr_rd,x.mmr_v,f.mmr,f.mmr_rd,g?1:0);await w.from("users").update({mmr:p.mmr,mmr_rd:p.rd,mmr_v:p.v,mmr_wins:g?w.sql`mmr_wins + 1`:void 0,mmr_losses:g?void 0:w.sql`mmr_losses + 1`}).eq("id",c),n(g?`+MMR ↑ ${Math.round(p.mmr-x.mmr)}`:`-MMR ↓ ${Math.round(x.mmr-p.mmr)}`,g?"success":"error",4e3)}}const Po=Object.freeze(Object.defineProperty({__proto__:null,renderMatchRandom:oi,resumePvpMatch:qo},Symbol.toStringTag,{value:"Module"}));async function Go(e,t){const{state:i,navigate:n,toast:a}=t,o=i.params||{},r=o.friendId||null,s=o.friendName||"Ami";await Mt(e,t,"friend",async({deckId:d,formation:l,starters:c,subsRaw:u,gcCardsEnriched:g,gcDefs:x,stadiumDef:f})=>{const p=g||[];He(e),r?await Ro(e,t,d,p,x,f,r,s):await No(e,t,d,p,x,f)})}async function Ro(e,t,i,n,a,o,r,s){var p;const{state:d,navigate:l,toast:c}=t,u=d.user.id,{data:g}=await w.from("friend_match_invites").insert({inviter_id:u,invitee_id:r,status:"pending"}).select("id").single();e.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="display:flex;background:#fff;border-bottom:1px solid #eee">
      ${m.map(v=>`<button class="ml-tab" data-tab="${v.key}" style="flex:1;padding:10px 4px;border:none;border-bottom:2px solid ${i===v.key?gt:"transparent"};background:none;font-size:12px;font-weight:${i===v.key?"900":"600"};color:${i===v.key?gt:"#888"};cursor:pointer">${v.label}${v.count?` (${v.count})`:""}</button>`).join("")}
    </div>
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:10px">
      ${i==="waiting"?Ho(g,p,a):i==="active"?Uo(x,a):Ko(f,a)}
    </div>
  </div>`,(b=document.getElementById("ml-create-btn"))==null||b.addEventListener("click",()=>Vo(e,t)),e.querySelectorAll(".ml-tab").forEach(v=>v.addEventListener("click",()=>dt(e,t,v.dataset.tab))),e.querySelectorAll("[data-league-id]").forEach(v=>v.addEventListener("click",()=>et(e,t,v.dataset.leagueId))),e.querySelectorAll("[data-join]").forEach(v=>v.addEventListener("click",h=>{h.stopPropagation(),an(e,t,v.dataset.join,v.dataset.type)})),e.querySelectorAll("[data-delete]").forEach(v=>v.addEventListener("click",h=>{h.stopPropagation(),ri(e,t,v.dataset.delete,v.dataset.name,i)}))}function Ut(e,t,i=!1){var r,s;const n=e.creator_id===t,a=e.pot||0,o=((s=(r=e.mini_league_members)==null?void 0:r[0])==null?void 0:s.count)||0;return`<div data-league-id="${e.id}" style="background:#fff;border-radius:12px;padding:14px 16px;box-shadow:0 1px 6px rgba(0,0,0,0.08);cursor:pointer;margin-bottom:8px">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        ${u.map(j=>`
          <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid #f5f5f5">
            <div style="width:36px;height:36px;border-radius:50%;background:${j.club_color2||gt};display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:900;color:${j.club_color1||"#fff"}">${(j.pseudo||"?").slice(0,2).toUpperCase()}</div>
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        ${c&&m&&!b?'<button id="ml-next-day" class="btn btn-primary" style="width:100%;margin-top:12px">➡️ Journée suivante</button>':""}
        ${c&&m&&b?'<button id="ml-finish-btn" class="btn btn-primary" style="width:100%;margin-top:12px;background:#7a28b8">🏆 Terminer et distribuer le pot</button>':""}
      </div>`:""}
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>${g.map((j,L)=>{const k=f>0&&r.status==="finished"?L===0?Math.floor(f*.7):L===1?Math.floor(f*.2):L===2?Math.floor(f*.1):0:0;return`<tr style="border-bottom:1px solid #f5f5f5;${j.userId===o?"background:#f0fdf4;":""}">
              <td style="padding:7px 3px 7px 0;font-weight:700;color:${L===0?Xe:L<3?gt:"#555"}">${["🥇","🥈","🥉"][L]||L+1}</td>
              ...
              <td style="text-align:center;color:${j.goalDiff>=0?gt:"#cc2222"}">${j.goalDiff>=0?"+":""}${j.goalDiff}</td>
              <td style="text-align:center;font-weight:900;font-size:14px">${j.points}</td>
              ${f>0&&r.status==="finished"?`<td style="text-align:right;font-weight:700;color:${Xe}">${k?k.toLocaleString("fr")+" cr.":"—"}</td>`:""}
            </tr>`}).join("")}</tbody>
        </table>
      </div>`:""}
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        ${Array.from({length:Math.max(0,r.status==="active"?r.current_day-1:r.current_day)},(j,L)=>L+1).reverse().map(j=>{const L=(d||[]).filter(k=>k.matchday===j);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:#888;margin-bottom:6px">Journée ${j}</div>${L.map(k=>Di(k,u,o,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
  ...
  </div>`;const h=r.status==="waiting"?"waiting":r.status==="active"?"active":"archived";if(($=document.getElementById("ml-back"))==null||$.addEventListener("click",()=>dt(e,t,h)),(y=document.getElementById("ml-start-btn"))==null||y.addEventListener("click",()=>Jo(e,t,r,u)),(C=document.getElementById("ml-next-day"))==null||C.addEventListener("click",()=>Qo(e,t,i)),(E=document.getElementById("ml-finish-btn"))==null||E.addEventListener("click",()=>sn(e,t,i,f,g,u)),(P=document.getElementById("ml-join-now"))==null||P.addEventListener("click",()=>an(e,t,i,r.type)),(N=document.getElementById("ml-leave-btn"))==null||N.addEventListener("click",()=>Wo(e,t,i,p)),(ee=document.getElementById("ml-delete-btn"))==null||ee.addEventListener("click",()=>ri(e,t,i,r.name,"waiting")),(ie=document.getElementById("ml-delete-now"))==null||ie.addEventListener("click",()=>ri(e,t,i,r.name,h)),(le=document.getElementById("ml-archive-btn"))==null||le.addEventListener("click",()=>Xo(e,t,i)),e.querySelectorAll("[data-play-match]").forEach(j=>{j.addEventListener("click",()=>{const L=x.find(k=>k.id===j.dataset.playMatch);L&&t.navigate("match-mini-league",{mlMatchId:L.id,leagueId:i})})}),r.status==="finished"&&v){const j=g.findIndex(L=>L.userId===o);j>=0&&j<3&&v.prize_amount>0&&setTimeout(()=>Zo(e,t,r,v,j),400)}}function Di(e,t,i,n,a=!1){const o=u=>t.find(g=>g.id===u);if(e.is_bye){const u=o(e.home_id);return`<div style="padding:8px;border-radius:8px;background:#f9f9f9;margin-bottom:6px;font-size:12px;color:#888;text-align:center">🔵 ${(u==null?void 0:u.club_name)||(u==null?void 0:u.pseudo)||"?"} exempté(e)</div>`}const r=o(e.home_id),s=o(e.away_id),d=e.home_id===i||e.away_id===i,l=d&&e.status==="pending"&&n&&!a,c=e.status==="finished"?`${e.home_score} - ${e.away_score}`:"vs";return`<div style="display:flex;align-items:center;gap:8px;padding:10px;border-radius:8px;background:${d?"#f0fdf4":"#f9f9f9"};margin-bottom:6px;border:1px solid ${d?"#86efac":"#f0f0f0"}">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
  ...
  </div>`;let d="buy";const l=()=>{var p,m,b,v,h,$,y;return{name:(((p=document.getElementById("flt-name"))==null?void 0:p.value)||"").toLowerCase().trim(),club:(((m=document.getElementById("flt-club"))==null?void 0:m.value)||"").toLowerCase().trim(),country:(((b=document.getElementById("flt-country"))==null?void 0:b.value)||"").toLowerCase().trim(),job:((v=document.getElementById("flt-job"))==null?void 0:v.value)||"",rarity:((h=document.getElementById("flt-rarity"))==null?void 0:h.value)||"",note1:parseInt(($=document.getElementById("flt-note1"))==null?void 0:$.value)||0,note2:parseInt((y=document.getElementById("flt-note2"))==null?void 0:y.value)||0}};function c(p){const m=l();return p.filter(b=>{var N,ee,ie;const v=(N=b.card)==null?void 0:N.player;if(!v)return!1;const h=`${v.firstname} ${v.surname_real}`.toLowerCase(),$=(((ee=v.clubs)==null?void 0:ee.encoded_name)||"").toLowerCase(),y=(v.country_code||"").toLowerCase(),C=((ie=b.card)==null?void 0:ie.evolution_bonus)||0,E=ct(v,v.job,C),P=v.job2?ct(v,v.job2,C):0;return!(m.name&&!h.includes(m.name)||m.club&&!$.includes(m.club)||m.country&&!y.includes(m.country)||m.job&&v.job!==m.job||m.rarity&&v.rarity!==m.rarity||m.note1&&E<m.note1||m.note2&&P<m.note2)})}function u(p){var P,N,ee,ie;const m=(P=p.card)==null?void 0:P.player;if(!m)return"";const b=((N=p.card)==null?void 0:N.evolution_bonus)||0,v=ct(m,m.job,b),h=m.job2?ct(m,m.job2,b):0,$=(i.profile.credits||0)>=p.price,y=m.country_code?`https://flagsapi.com/${m.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,C=Dt[m.job]||"#bbb",E=m.job2?Dt[m.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden">
      <!-- Drapeau -->
    ...
    </div>`}function g(p){var P,N,ee,ie;const m=(P=p.card)==null?void 0:P.player;if(!m)return"";const b=((N=p.card)==null?void 0:N.evolution_bonus)||0,v=ct(m,m.job,b),h=m.job2?ct(m,m.job2,b):0,$=p.status==="sold",y=m.country_code?`https://flagsapi.com/${m.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,C=Dt[m.job]||"#bbb",E=m.job2?Dt[m.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${$?"opacity:0.7":""}">
      ${y?`<img src="${y}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      ${(ee=m.clubs)!=null&&ee.logo_url?`<img src="${m.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${C};display:flex;align-items:center;justify-content:center">
    ...
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center">
      ...
      `).join(""):'<div style="text-align:center;color:var(--gray-600);padding:40px">Aucun manager.</div>'}else{const b=r||[];f.innerHTML=b.length>0?b.map((v,h)=>{const $=di(v.mmr??1e3),y=(v.ranked_wins||0)+(v.ranked_losses||0)+(v.ranked_draws||0),C=y>0?Math.round((v.ranked_wins||0)/y*100):0,E=v.id===i.profile.id,P=(v.placement_matches||0)<10;return`
  ...
  <div class="page">
    ...
    </div>`,(C=document.getElementById("ranked-back"))==null||C.addEventListener("click",()=>n("home"));return}const d=s.mmr??1e3,l=s.mmr_deviation??350,c=s.mmr_volatility??.06,u=s.placement_matches??0,g=u<10,x=Math.max(0,10-u),f=di(d),p=mn(d),m=Yt.findIndex(ee=>ee.id===f.id),b=Yt[m+1]||null,v={bronze:"linear-gradient(160deg,#3d1c00,#7a3e00)",silver:"linear-gradient(160deg,#1a1a2e,#3a3a5e)",gold:"linear-gradient(160deg,#1a1200,#4a3500)",platinum:"linear-gradient(160deg,#001a20,#003040)",diamond:"linear-gradient(160deg,#001030,#1a2860)",master:"linear-gradient(160deg,#1a0030,#3d0070)"},h=(s.ranked_wins||0)+(s.ranked_losses||0)+(s.ranked_draws||0),$=h>0?Math.round((s.ranked_wins||0)/h*100):0,y=Yt.map(ee=>`
    <div style="display:flex;flex-direction:column;align-items:center;gap:2px;opacity:${f.id===ee.id?1:.35};
      transform:${f.id===ee.id?"scale(1.15)":"scale(1)"};transition:all 0.3s">
      <div style="font-size:${f.id===ee.id?"28px":"20px"}">${ee.emoji}</div>
      <div style="font-size:9px;color:${ee.color};font-weight:${f.id===ee.id?"900":"400"};letter-spacing:0.5px">${ee.label.toUpperCase()}</div>
    </div>
  `).join("");e.innerHTML=`
  <div style="min-height:100%;background:${v[f.id]};padding:16px;overflow-y:auto;display:flex;flex-direction:column;gap:16px">
    ...
    <div style="display:flex;flex-direction:column;gap:10px;margin-top:auto;padding-top:8px">
      ...
      away:users!away_id(pseudo,club_name)`).or(`home_id.eq.${a.id},away_id.eq.${a.id}`).order("created_at",{ascending:!1}).limit(50),r={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},s=(o||[]).filter(c=>c.status==="finished"),d=(o||[]).filter(c=>c.status==="in_progress");function l(c){const u=c.home_id===a.id;u?c.home_score:c.away_score,u?c.away_score:c.home_score;const g=c.winner_id===a.id,x=c.home_score===c.away_score&&c.status==="finished",f=c.status!=="finished"?"…":x?"N":g?"V":"D",p=c.status!=="finished"||x?"#888":g?"#1A6B3C":"#c0392b";let m=r[c.mode]||c.mode;c.away_id===null&&!m.startsWith("IA")&&(m="IA");const v=c.home_id===a.id?c.away:c.home;let h;c.away_id===null?h=m:v?h=`${v.club_name||v.pseudo} (${v.pseudo})`:h="Adversaire";let $="";c.status==="in_progress"&&Date.now()-new Date(c.created_at).getTime()>3600*1e3&&($=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const y=new Date(c.created_at),C=y.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+y.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),E=c.status==="finished"?`${c.home_score} - ${c.away_score}`:`${c.home_score||0} - ${c.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
        <div style="font-size:13px;font-weight:600">${h}${$}</div>
        <div style="font-size:11px;color:var(--gray-600)">${m} · ${C}${c.status==="in_progress"?" · en cours":""}</div>
      </div>
      <div style="display:flex;align-items:center;gap:8px">
