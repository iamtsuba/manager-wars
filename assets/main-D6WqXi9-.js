import{s as S,F as ht,b as Ve,c as ii,l as Ee,d as ki}from"./formation-links-Bt44rMnM.js";function Nt(e,{navigate:t,toast:i}){e.innerHTML=`
  ...
  </div>`}async function Si(e,t){const{state:i,navigate:n,toast:d,openModal:s,closeModal:a}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:l}=await S.from("cards").select(`id, card_type, current_note, gc_type, formation, is_for_sale, sale_price,
  ...
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(u=document.getElementById("direct-sell-form-btn"))==null||u.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${o} pour ${Ne.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const $=g.find(v=>!v.is_for_sale)||g[0];if(!$){s("Aucune carte à vendre","error");return}const{error:M}=await S.from("cards").delete().eq("id",$.id);if(M){s(M.message,"error");return}await S.from("users").update({credits:(d.profile.credits||0)+Ne}).eq("id",d.profile.id),await c(),s(`+${Ne.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),a(),l("collection")}),(h=document.getElementById("market-sell-form-btn"))==null||h.addEventListener("click",async()=>{const $=parseInt(document.getElementById("sell-price-form").value);if(!$||$<1){s("Prix invalide","error");return}await S.from("cards").update({is_for_sale:!0,sale_price:$}).eq("id",e.id),await S.from("market_listings").insert({seller_id:d.profile.id,card_id:e.id,price:$}),s("Carte mise en vente sur le marché !","success"),a(),l("collection")}),(I=document.getElementById("cancel-sell-form-btn"))==null||I.addEventListener("click",async()=>{await S.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await S.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),s("Annonce retirée","success"),a(),l("collection")})}async function Kt(e,t,i,n){var P,U,w,N,E,H;const{state:d,toast:s,openModal:a,closeModal:l,navigate:c,refreshProfile:o}=n,r=e.player,p=t.filter(q=>q.player.id===r.id),g=p.length,m=zi[r.rarity]||1e3,_=r.rarity!=="legende",u=ai(),h=He(r,r.job),I=r.job2?He(r,r.job2):null,$=tt[r.job]||"#1A6B3C",M=r.job2?tt[r.job2]:null,v=ri[r.rarity]||"#ccc",L=$t[r.country_code]||r.country_code||"",{data:k}=await S.from("transfer_history").select("club_name, manager_name, source, price, transferred_at").eq("player_id",r.id).order("transferred_at",{ascending:!0}),G=k&&k.length?`
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
    ...
    </div>`:""}`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(w=document.getElementById("close-detail"))==null||w.addEventListener("click",l),(N=document.getElementById("direct-sell-btn"))==null||N.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte ${r.surname_encoded} pour ${m.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const q=p.find(te=>!te.is_for_sale)||p[0];if(!q){s("Aucune carte à vendre","error");return}const{error:V}=await S.from("cards").delete().eq("id",q.id);if(V){s(V.message,"error");return}await S.from("users").update({credits:(d.profile.credits||0)+m}).eq("id",d.profile.id),await o(),s(`+${m.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),l(),c("collection")}),(E=document.getElementById("market-sell-btn"))==null||E.addEventListener("click",async()=>{const q=parseInt(document.getElementById("sell-price").value);if(!q||q<1){s("Prix invalide","error");return}await S.from("cards").update({is_for_sale:!0,sale_price:q}).eq("id",e.id),await S.from("market_listings").insert({seller_id:d.profile.id,card_id:e.id,price:q}),s("Carte mise en vente sur le marché !","success"),l(),c("collection")}),(H=document.getElementById("cancel-sell-btn"))==null||H.addEventListener("click",async()=>{await S.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await S.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),s("Annonce retirée","success"),l(),c("collection")})}const Se={},it={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},_e={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function st(e){const t=typeof import.meta<"u"?Se==null?void 0:Se.VITE_SUPABASE_URL:"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function si(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function li(e){var i;const t=(Se==null?void 0:Se.VITE_SUPABASE_URL)||"";return(i=e==null?void 0:e.clubs)!=null&&i.logo_url?e.clubs.logo_url.startsWith("http")?e.clubs.logo_url:t?`${t}/storage/v1/object/public/assets/clubs/${e.clubs.logo_url}`:null:null}function Bi(e,t=44,i=58){var m;const n=e?st(e):null,d=e?li(e):null,s=si(e==null?void 0:e.country_code),a=(e==null?void 0:e.job)||"MIL",l=_e[a]||"#555",c={normal:"#aaa",pépite:"#D4A017",papyte:"#222",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",o=Number(a==="GK"?e==null?void 0:e.note_g:a==="DEF"?e==null?void 0:e.note_d:a==="MIL"?e==null?void 0:e.note_m:e==null?void 0:e.note_a)||0,r=Math.round(i*.19),p=Math.round(i*.25),g=i-r-p;return e?`<div style="width:${t}px;height:${i}px;border-radius:5px;border:2px solid ${c};background:${l};position:relative;overflow:hidden;flex-shrink:0">
  ...
  <div class="page">
    ...
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(m=document.getElementById("close-selector"))==null||m.addEventListener("click",s),(_=document.getElementById("remove-player"))==null||_.addEventListener("click",()=>{delete t.slots[e],s(),Be(i,t,n)}),document.querySelectorAll(".player-option").forEach(u=>{u.addEventListener("click",()=>{t.slots[e]=u.dataset.cardId,s(),Be(i,t,n)})})}function Gi(e,t,i){var c;const{openModal:n,closeModal:d}=i,s=new Set;Object.values(e.slots).filter(Boolean).forEach(o=>{var p;const r=e.playerCards.find(g=>g.id===o);(p=r==null?void 0:r.player)!=null&&p.id&&s.add(r.player.id)}),e.subs.forEach(o=>{var p;const r=e.playerCards.find(g=>g.id===o);(p=r==null?void 0:r.player)!=null&&p.id&&s.add(r.player.id)});const a=new Set,l=e.playerCards.filter(o=>{var r,p,g;return s.has((r=o.player)==null?void 0:r.id)||a.has((p=o.player)==null?void 0:p.id)?!1:(a.add((g=o.player)==null?void 0:g.id),!0)});n("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${l.length>0?l.map(o=>{var m;const r=o.player,p=st(r),g=r.job==="GK"?r.note_g:r.job==="DEF"?r.note_d:r.job==="MIL"?r.note_m:r.note_a;return`<div class="player-option" data-card-id="${o.id}"
          style="display:flex;align-items:center;gap:10px;padding:8px;border:1.5px solid var(--gray-200);border-radius:10px;cursor:pointer">
    ...
    </div>`,'<button class="btn btn-ghost" id="close-sub-selector">Fermer</button>'),(c=document.getElementById("close-sub-selector"))==null||c.addEventListener("click",d),document.querySelectorAll(".player-option").forEach(o=>{o.addEventListener("click",()=>{e.subs.push(o.dataset.cardId),d(),Be(t,e,i)})})}async function qi(e,t){const{state:i,toast:n,navigate:d}=t,s=e.formationCards.find(c=>c.formation===e.formation),a=(s==null?void 0:s.id)||e.formationCardId;await S.from("decks").update({formation:e.formation,formation_card_id:a||null}).eq("id",e.deckId),await S.from("deck_cards").delete().eq("deck_id",e.deckId);const l=[];if(Object.entries(e.slots).forEach(([c,o],r)=>{l.push({deck_id:e.deckId,card_id:o,position:c,is_starter:!0,slot_order:r})}),e.subs.forEach((c,o)=>{l.push({deck_id:e.deckId,card_id:c,position:`SUB${o+1}`,is_starter:!1,slot_order:100+o})}),l.length>0){const{error:c}=await S.from("deck_cards").insert(l);if(c){n(c.message,"error");return}}n("Deck enregistré ✅","success"),d("decks")}function Yt(e){const t=it[e]||it["4-4-2"],i=["GK1"];for(let n=1;n<=t.DEF;n++)i.push(`DEF${n}`);for(let n=1;n<=t.MIL;n++)i.push(`MIL${n}`);for(let n=1;n<=t.ATT;n++)i.push(`ATT${n}`);return i}async function Ri(){const{data:e}=await S.from("booster_configs").select("*").eq("is_active",!0).order("sort_order");if(!(e!=null&&e.length))return[];const{data:t}=await S.from("booster_drop_rates").select("*").in("booster_id",e.map(i=>i.id)).order("sort_order");return e.map(i=>({...i,rates:(t||[]).filter(n=>n.booster_id===i.id)}))}function Ni(e){if(!(e!=null&&e.length))return null;const t=e.reduce((n,d)=>n+Number(d.percentage),0);let i=Math.random()*t;for(const n of e)if(i-=Number(n.percentage),i<=0)return n;return e[e.length-1]}const di=()=>Object.keys(Ve),Oi=[{id:"players_std",img:"/manager-wars/icons/booster-players.png",name:"Players",sub:"5 cartes joueurs",cost:5e3,costLabel:"5 000 crédits",cardCount:5,type:"player"},{id:"players_pub",img:"/manager-wars/icons/booster-silver.png",name:"Players (pub)",sub:"3 cartes joueurs",cost:0,costLabel:"1 pub",cardCount:3,type:"player"},{id:"game_changer",img:"/manager-wars/icons/booster-gamechanger.png",name:"Game Changer",sub:"3 cartes spéciales",cost:1e4,costLabel:"10 000 crédits",cardCount:3,type:"game_changer"},{id:"formation",img:"/manager-wars/icons/booster-formation.png",name:"Formation",sub:"1 carte formation",cost:1e4,costLabel:"10 000 crédits",cardCount:1,type:"formation"}],ut={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé."},"Double attaque":{icon:"⚡",desc:"La prochaine attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la prochaine action IA."},Gel:{icon:"❄️",desc:"Bloque le meilleur attaquant IA."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function Pi(e){return null}const Wt={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},Hi={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"},Ui={MA:"MAROC",FR:"FRANCE",AR:"ARGENTINE",PT:"PORTUGAL",BR:"BRESIL",ES:"ESPAGNE",DE:"ALLEMAGNE",GB:"ANGLETERRE",IT:"ITALIE",CM:"CAMEROUN",SN:"SENEGAL",NG:"NIGERIA",DK:"DANEMARK",NL:"PAYS-BAS",BE:"BELGIQUE",CI:"CÔTE D'IVOIRE",AL:"ALBANIE",HR:"CROATIE",RS:"SERBIE",TR:"TURQUIE"};function Xt(e,t){return e&&Number(t==="GK"?e.note_g:t==="DEF"?e.note_d:t==="MIL"?e.note_m:e.note_a)||0}function Ki(e){var r,p;const t=e.player;if(!t)return"";const i=t.job||"ATT",n=Wt[i],d=Hi[t.rarity]||"#ccc",s=Xt(t,i),a=t.job2?Xt(t,t.job2):null,l=t.job2?Wt[t.job2]:null,c=Pi(),o=Ui[t.country_code]||t.country_code||"";return`
  ...
  </div>`,e.querySelectorAll(".booster-card:not(.disabled)").forEach(l=>{l.addEventListener("click",async()=>{const c=s.find(o=>o.id===l.dataset.booster);if(c){l.style.opacity="0.5",l.style.pointerEvents="none";try{await Wi(c,{state:t,toast:n,navigate:i,container:e})}catch(o){n(o.message,"error"),l.style.opacity="",l.style.pointerEvents=""}}})}),e.querySelectorAll(".booster-info-btn").forEach(l=>{l.addEventListener("click",c=>{c.stopPropagation();const o=s.find(r=>r.id===l.dataset.boosterId);oo(o)})})}async function Wi(e,{state:t,toast:i,navigate:n,container:d}){var r;if(e.cost>0&&t.profile.credits<e.cost){i("Crédits insuffisants","error");return}if(e.isPub)try{await ao()}catch(p){i(p.message||"Regardez la pub entièrement pour ouvrir le booster","error");return}const{data:s}=await S.from("cards").select("card_type, player_id, formation").eq("owner_id",t.profile.id),a=new Set((s||[]).filter(p=>p.card_type==="player").map(p=>p.player_id)),l=new Set((s||[]).filter(p=>p.card_type==="formation").map(p=>p.formation));let c=[];if((r=e.rates)!=null&&r.length)c=await Jt(t.profile,e);else{const p=e.type||"player";p==="player"?c=await Qi(t.profile,e.cardCount,e.cost):p==="game_changer"?c=await Zi(t.profile,e.cardCount,e.cost):p==="formation"?c=await eo(t.profile,e.cost):c=await Jt(t.profile,e)}c.forEach(p=>{p.card_type==="player"&&p.player?p.isDuplicate=a.has(p.player.id):p.card_type==="formation"&&(p.isDuplicate=l.has(p.formation))});const{data:o}=await S.from("users").select("*").eq("id",t.profile.id).single();o&&(t.profile=o),to(c,e,n)}function Xi(){const e=Math.random()*100;return e<.5?"legende":e<2?"special":e<10?"normal_high":"normal_low"}function Le(e){return Math.max(Number(e.note_g)||0,Number(e.note_d)||0,Number(e.note_m)||0,Number(e.note_a)||0)}function Ji(e,t){let i;switch(t){case"legende":i=e.filter(n=>n.rarity==="legende"),i.length||(i=e.filter(n=>n.rarity==="pepite"||n.rarity==="papyte")),i.length||(i=e.filter(n=>Le(n)>=6));break;case"special":i=e.filter(n=>n.rarity==="pepite"||n.rarity==="papyte"),i.length||(i=e.filter(n=>Le(n)>=6));break;case"normal_high":i=e.filter(n=>n.rarity==="normal"&&Le(n)>=6),i.length||(i=e.filter(n=>Le(n)>=6));break;default:i=e.filter(n=>n.rarity==="normal"&&Le(n)>=1&&Le(n)<=5),i.length||(i=e.filter(n=>n.rarity==="normal"));break}return i.length||(i=e),i[Math.floor(Math.random()*i.length)]}async function Jt(e,t){if(t.cost>0){const{error:n}=await S.from("users").update({credits:e.credits-t.cost}).eq("id",e.id);if(n)throw n}const i=[];for(let n=0;n<(t.cardCount||5);n++){const d=Ni(t.rates);if(d){if(d.card_type==="player"){const s=g=>({légende:"legende",pépite:"pepite",pépites:"pepite"})[g]||g,a=d.rarity?s(d.rarity):null;let l=S.from("players").select("id,job,firstname,surname_encoded,country_code,club_id,rarity,note_g,note_d,note_m,note_a,skin,hair,hair_length,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);a&&(l=l.eq("rarity",a));const{data:c}=await l;let o=c||[];if((d.note_min||d.note_max)&&(o=o.filter(g=>{const m=Math.max(Number(g.note_g)||0,Number(g.note_d)||0,Number(g.note_m)||0,Number(g.note_a)||0);return(!d.note_min||m>=d.note_min)&&(!d.note_max||m<=d.note_max)})),o.length||(d.note_min||d.note_max?(o=c||[],console.warn("[Booster] Aucun joueur avec note",d.note_min,"-",d.note_max,"— fallback rareté uniquement")):o=c||[]),!o.length)continue;const r=o[Math.floor(Math.random()*o.length)],{data:p}=await S.from("cards").insert({owner_id:e.id,player_id:r.id,card_type:"player"}).select().single();p&&(i.push({...p,player:r}),S.rpc("record_transfer",{p_card_id:p.id,p_player_id:r.id,p_club_name:e.club_name||e.pseudo,p_manager_name:e.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{}))}else if(d.card_type==="game_changer"){const{data:s}=await S.from("gc_definitions").select("id,name").eq("is_active",!0).eq("gc_type","game_changer"),a=s!=null&&s.length?s:[{name:"Ressusciter"},{name:"Double attaque"},{name:"Bouclier"},{name:"Vol de note"},{name:"Gel"}],c=a[Math.floor(Math.random()*a.length)].name,{data:o}=await S.from("cards").insert({owner_id:e.id,card_type:"game_changer",gc_type:c}).select().single();o&&i.push(o)}else if(d.card_type==="formation"){const s=di(),a=s[Math.floor(Math.random()*s.length)],{data:l}=await S.from("cards").insert({owner_id:e.id,card_type:"formation",formation:a}).select();l!=null&&l[0]&&i.push(l[0])}}}return i}async function Qi(e,t,i){if(i>0){const{error:o}=await S.from("users").update({credits:e.credits-i}).eq("id",e.id);if(o)throw o}const{data:n}=await S.from("players").select("id,job,firstname,surname_encoded,country_code,club_id,rarity,note_g,note_d,note_m,note_a,note_min,note_max,skin,hair,hair_length,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);if(!(n!=null&&n.length))throw new Error("Pas de joueurs en BDD — ajoutes-en via le panel admin !");const d=n.filter(o=>o.job==="GK"),s=n.filter(o=>o.job!=="GK"),a=!e.first_booster_opened&&d.length>0,l=[];for(let o=0;o<t;o++){const r=o===0&&a?d:o===0?s:n,p=Xi(),g=Ji(r,p);g&&l.push(g)}a&&await S.from("users").update({first_booster_opened:!0}).eq("id",e.id);const{data:c}=await S.from("cards").insert(l.map(o=>({owner_id:e.id,player_id:o.id,card_type:"player"}))).select();return(c||[]).forEach((o,r)=>{S.rpc("record_transfer",{p_card_id:o.id,p_player_id:l[r].id,p_club_name:e.club_name||e.pseudo,p_manager_name:e.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{})}),l.map((o,r)=>({...c[r],player:o}))}async function Zi(e,t,i){const{error:n}=await S.from("users").update({credits:e.credits-i}).eq("id",e.id);if(n)throw n;const{data:d}=await S.from("gc_definitions").select("id,name,gc_type,color,effect,image_url").eq("is_active",!0),s=d!=null&&d.length?d:Object.keys(ut).map(r=>({name:r,gc_type:"game_changer"})),a=Array.from({length:t},()=>{const r=s[Math.floor(Math.random()*s.length)];return{owner_id:e.id,card_type:"game_changer",gc_type:r.name,gc_definition_id:r.id||null}}),{data:l,error:c}=await S.from("cards").insert(a).select();return c&&console.error("[Booster GC] Erreur insert:",c.message,c),(l||[]).map(r=>{const p=d==null?void 0:d.find(g=>g.name===r.gc_type||g.id===r.gc_definition_id);return{...r,_gcDef:p||null}})}async function eo(e,t){const{error:i}=await S.from("users").update({credits:e.credits-t}).eq("id",e.id);if(i)throw i;const n=di(),d=n[Math.floor(Math.random()*n.length)],{data:s,error:a}=await S.from("cards").insert({owner_id:e.id,card_type:"formation",formation:d}).select();return a&&console.error("[Booster Formation] Erreur insert:",a.message,a),s||[]}function to(e,t,i){var N;if(!e||e.length===0){const E=document.createElement("div");E.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:3000;gap:16px;color:#fff;padding:24px;text-align:center",E.innerHTML=`
  ...
  `,document.body.appendChild(n);let d=!1;const s=document.getElementById("pack-cut-zone"),a=document.getElementById("pack-blade");let l=!1;const c=E=>E.touches&&E.touches[0]?E.touches[0].clientX:E.clientX;function o(E){d||(l=!0,a.style.opacity="1",r(E))}function r(E){if(!l||d)return;const H=s.getBoundingClientRect(),q=c(E)-H.left,V=Math.max(0,Math.min(1,q/H.width));a.style.width=V*H.width+"px",V>=.82&&g()}function p(){d||(l=!1,a.style.transition="width .2s ease, opacity .2s ease",a.style.width="0",a.style.opacity="0",setTimeout(()=>{d||(a.style.transition="")},220))}function g(){var H;if(d)return;d=!0,l=!1,a.style.width="100%",a.style.opacity="1",(H=document.getElementById("cut-flash"))==null||H.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const E=document.getElementById("cut-hint");E&&(E.style.opacity="0"),s.classList.add("pack-cut"),setTimeout(()=>{a.style.opacity="0",document.getElementById("pack-phase").style.display="none",h(0)},620)}s.addEventListener("pointerdown",o),window.addEventListener("pointermove",r),window.addEventListener("pointerup",p),s.addEventListener("touchstart",o,{passive:!0}),window.addEventListener("touchmove",r,{passive:!0}),window.addEventListener("touchend",p);let m=0,_=!1;const u=new Set;function h(E){m=E,document.getElementById("reveal-phase").style.display="flex",I(),$(E,0),G()}function I(){const E=document.getElementById("card-dots");E&&(E.innerHTML=e.map((H,q)=>`<div class="card-dot" data-i="${q}" style="width:8px;height:8px;border-radius:50%;background:${q===m?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),E.querySelectorAll(".card-dot").forEach(H=>H.addEventListener("click",()=>v(parseInt(H.dataset.i)))))}function $(E,H){var z;const q=e[E],V=document.getElementById("card-counter"),te=document.getElementById("card-track");V&&(V.textContent=`Carte ${E+1} / ${e.length}`);const ne=q.card_type==="player"&&((z=q.player)==null?void 0:z.rarity)==="legende",Z=!u.has(E);u.add(E);let pe=0;if(q.card_type==="player"&&q.player){const T=q.player,f=T.job||"ATT";pe=Number(f==="GK"?T.note_g:f==="DEF"?T.note_d:f==="MIL"?T.note_m:T.note_a)||0}const Q=T=>{te.innerHTML=`
        <div id="current-card-wrap" style="display:flex;flex-direction:column;align-items:center;gap:8px;${ne?"filter:drop-shadow(0 0 20px #7a28b8)":""}">
          <div style="transform:scale(1.3);transform-origin:center">${io(q)}</div>
      ...
      </div>`,document.body.appendChild(n),n.addEventListener("click",a=>{a.target===n&&n.remove()}),(i=document.getElementById("odds-close"))==null||i.addEventListener("click",()=>n.remove());return}no()}function no(){const e=document.createElement("div");e.style.cssText=`position:fixed;inset:0;background:rgba(0,0,0,0.6);display:flex;
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}const ro="simulation",ci="VOTRE_ZONE_ID";function ao(){switch(ro){case"propellerads":return so();case"adsense":return lo();default:return _t()}}function _t(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:#111;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:14px";let i=5;t.innerHTML=`
      ...
      <button id="ad-skip" disabled style="margin-top:8px;padding:10px 24px;border-radius:20px;border:none;background:rgba(255,255,255,0.15);color:rgba(255,255,255,0.4);font-size:13px;cursor:default">
        Passer (5)
      </button>`,document.body.appendChild(t);const n=setInterval(()=>{i--;const d=document.getElementById("ad-cd"),s=document.getElementById("ad-skip");d&&(d.textContent=i),s&&(s.textContent=i>0?`Passer (${i})`:"✓ Continuer"),i<=0&&(clearInterval(n),s&&(s.disabled=!1,s.style.cssText=s.style.cssText.replace("rgba(255,255,255,0.15)","#1A6B3C").replace("rgba(255,255,255,0.4)","#fff").replace("default","pointer")),s==null||s.addEventListener("click",()=>{t.remove(),e(!0)},{once:!0}))},1e3)})}function so(){return new Promise((e,t)=>{if(typeof window.propeller>"u")return console.warn("[Ad] PropellerAds non chargé → simulation"),e(_t());window.propeller.push({zone_id:ci,onComplete:()=>e(!0),onSkip:()=>t(new Error("Publicité ignorée")),onError:()=>t(new Error("Erreur publicité"))})})}function lo(){return new Promise((e,t)=>{if(typeof window.adsbygoogle>"u")return console.warn("[Ad] AdSense non chargé → simulation"),e(_t());try{(window.adsbygoogle=window.adsbygoogle||[]).push({google_ad_client:"ca-pub-VOTRE_PUBLISHER_ID",google_ad_slot:ci,google_ad_format:"rewarded",on_reward:()=>e(!0),on_dismiss:()=>t(new Error("Publicité fermée"))})}catch(i){t(i)}})}const je={},Fe={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},De={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function ke(e,t,i,n,d){var s;e.innerHTML=`<div class="match-screen" style="display:flex;align-items:center;justify-content:center;min-height:100vh">
    <div style="text-align:center;padding:40px;color:#fff">
      <div style="font-size:48px;margin-bottom:16px">${t}</div>
      <p style="margin-bottom:16px">${i}</p>
      <button class="btn btn-primary" id="msg-btn">${n}</button>
    </div>
  </div>`,(s=document.getElementById("msg-btn"))==null||s.addEventListener("click",d)}function ge(e){const t=typeof import.meta<"u"?je==null?void 0:je.VITE_SUPABASE_URL:"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function ft(e,t){var n,d;const i=e.player;return{cardId:e.id,position:t||null,id:i.id,firstname:i.firstname,name:i.surname_encoded,country_code:i.country_code,club_id:i.club_id,job:i.job,job2:i.job2,note_g:Number(i.note_g)||0,note_d:Number(i.note_d)||0,note_m:Number(i.note_m)||0,note_a:Number(i.note_a)||0,rarity:i.rarity,skin:i.skin,hair:i.hair,hair_length:i.hair_length,clubName:((n=i.clubs)==null?void 0:n.encoded_name)||null,clubLogo:((d=i.clubs)==null?void 0:d.logo_url)||null,boost:0,used:!1,_line:null,_col:null}}function ot(e){return e===1?[1]:e===2?[0,2]:e===3?[0,1,2]:e===4?[0,1,1,2]:e===5?[0,1,1,1,2]:[1]}function pi(){const e=Math.random()*100;return e<10?3:e<30?2:1}function nt(e,t){const i=Fe[t]||Fe["4-4-2"],n={GK:[],DEF:[],MIL:[],ATT:[]};if(e.length&&e.every(a=>a.position)){for(const a of["GK","DEF","MIL","ATT"]){const l=e.filter(o=>o.position&&o.position.replace(/\d+$/,"")===a).sort((o,r)=>parseInt(o.position.replace(/\D+/g,""),10)-parseInt(r.position.replace(/\D+/g,""),10)).map(o=>({...o,_line:a})),c=ot(l.length);l.forEach((o,r)=>{o._col=c[r]}),n[a]=l}return n}const s=[...e];for(const a of["GK","DEF","MIL","ATT"]){const l=[];for(let o=0;o<i[a];o++){let r=s.findIndex(p=>p.job===a);if(r===-1&&(r=s.findIndex(p=>p.job2===a)),r===-1&&(r=0),s[r]){const p={...s[r],_line:a};l.push(p),s.splice(r,1)}}const c=ot(l.length);l.forEach((o,r)=>{o._col=c[r]}),n[a]=l}return n}function $e(e){document.querySelectorAll(".top-nav, .bottom-nav").forEach(t=>{t.style.setProperty("display","none","important"),t.dataset.matchHidden="1"}),e&&e.style.setProperty("padding-bottom","0","important")}function be(e){document.querySelectorAll(".top-nav, .bottom-nav").forEach(t=>{t.style.removeProperty("display"),delete t.dataset.matchHidden}),e&&e.style.removeProperty("padding-bottom")}function ui(e,t,i){const d=new Set,s=t.filter(r=>{const p=r.gc_type||r.id;return d.has(p)?!1:(d.add(p),!0)});let a=[];function l(r,p){const g=r._gcDef,m={purple:"linear-gradient(135deg,#3d0a7a,#7a28b8)",light_blue:"linear-gradient(135deg,#006080,#00bcd4)"},_={purple:"#9b59b6",light_blue:"#00bcd4"},u=m[g==null?void 0:g.color]||m.purple,h=_[g==null?void 0:g.color]||_.purple;return`<div class="gc-select-card" data-id="${r.id}"
    ...
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div id="deck-swipe-zone" style="flex:1;min-height:0;overflow:hidden;position:relative;touch-action:pan-y;display:flex;align-items:center;justify-content:center;padding:4px">
        ${u?`<div class="deck-preview-wrap" style="aspect-ratio:1/1;max-width:100%;max-height:100%;width:auto;height:100%;overflow:hidden">${Ke(u,_,null,[],285,285)}</div>`:`<div style="display:flex;align-items:center;justify-content:center;height:100%;opacity:.4;flex-direction:column;gap:8px">
  ...
  </div>`}async function mi(e,t,i,n){var L;const{state:d,navigate:s,toast:a}=t;$e(e);const l=d.params||{};if(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!l.deckId)return co(e,t,i);const c=l.deckId;let o,r,p,g;try{const k=await Promise.all([S.from("decks").select("formation,name").eq("id",c).single(),S.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation,
            player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
              clubs(encoded_name,logo_url)))`).eq("deck_id",c).order("slot_order")]);o=k[0].data,p=k[0].error,r=k[1].data,g=k[1].error}catch(k){console.error("[Match] Exception chargement deck:",k),ke(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>s("home"));return}if(p||g){console.error("[Match] Erreur Supabase:",p||g),ke(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>s("home"));return}const m=(r||[]).filter(k=>{var G;return k.is_starter&&((G=k.card)==null?void 0:G.player)}).map(k=>ft(k.card,k.position)),_=(r||[]).filter(k=>{var G;return!k.is_starter&&((G=k.card)==null?void 0:G.player)}).map(k=>ft(k.card,k.position));if(m.length<11){ke(e,"⚠️",`Deck incomplet (${m.length}/11).`,"Compléter",()=>s("decks"));return}const u=(r||[]).find(k=>{var G;return((G=k.card)==null?void 0:G.card_type)==="formation"}),h=(o==null?void 0:o.formation)||((L=u==null?void 0:u.card)==null?void 0:L.formation)||"4-4-2",{data:I,error:$}=await S.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",d.profile.id).eq("card_type","game_changer"),{data:M}=await S.from("gc_definitions").select("*").eq("is_active",!0),v=(I||[]).map(k=>({...k,_gcDef:(M==null?void 0:M.find(G=>G.name===k.gc_type||G.id===k.gc_definition_id))||null}));n({deckId:c,formation:h,starters:m,subsRaw:_,gcCardsEnriched:v,gcDefs:M||[]})}async function po(e,t){const{state:i}=t,d=(i.params||{}).matchMode||"vs_ai_easy",s=d.replace("vs_ai_",""),a=d;await mi(e,t,d,async({deckId:l,formation:c,starters:o,subsRaw:r,gcCardsEnriched:p,gcDefs:g})=>{try{const m=nt(o,c),_=await uo(c,s),u=async h=>{try{const{data:I,error:$}=await S.from("matches").insert({home_id:i.profile.id,away_id:null,mode:a,home_deck_id:l,status:"in_progress"}).select().single();if($){console.error("[MatchIA] Erreur création match:",$),ke(e,"⚠️","Impossible de créer le match ("+$.message+").","Retour",()=>t.navigate("home"));return}const M={gcDefs:g||[],matchId:I==null?void 0:I.id,mode:a,difficulty:s,formation:c,homeTeam:m,aiTeam:_,homeSubs:r,subsUsed:0,maxSubs:Math.min(r.length,3),homeScore:0,aiScore:0,gcCards:h,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};go(e,M,t)}catch(I){console.error("[MatchIA] Exception launchMatch:",I),ke(e,"⚠️","Erreur au lancement du match : "+I.message,"Retour",()=>t.navigate("home"))}};if(!p.length){u([]);return}ui(e,p,u)}catch(m){console.error("[MatchIA] Exception setup:",m),ke(e,"⚠️","Erreur de préparation du match : "+m.message,"Retour",()=>t.navigate("home"))}})}async function uo(e,t){var a,l;const{data:i}=await S.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(60);if(!i||i.length<11)return fo(e);const n=Fe[e]||Fe["4-4-2"],d={GK:[],DEF:[],MIL:[],ATT:[]},s=[...i];for(const c of["GK","DEF","MIL","ATT"]){const o=s.filter(_=>_.job===c),r=s.filter(_=>_.job!==c),p=[...o,...r],g=[];for(let _=0;_<n[c];_++){const u=p[_]||s[_];u&&g.push({cardId:"ai-"+u.id+"-"+_,id:u.id,firstname:u.firstname,name:u.surname_encoded,country_code:u.country_code,club_id:u.club_id,job:u.job,job2:u.job2,note_g:Number(u.note_g)||0,note_d:Number(u.note_d)||0,note_m:Number(u.note_m)||0,note_a:Number(u.note_a)||0,rarity:u.rarity,skin:u.skin,hair:u.hair,hair_length:u.hair_length,clubName:((a=u.clubs)==null?void 0:a.encoded_name)||null,clubLogo:((l=u.clubs)==null?void 0:l.logo_url)||null,boost:0,used:!1,_line:c})}const m=ot(g.length);g.forEach((_,u)=>{_._col=m[u]}),d[c]=g}return d}function fo(e){const t=Fe[e]||Fe["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},n=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let d=0;for(const s of["GK","DEF","MIL","ATT"]){const a=[];for(let c=0;c<t[s];c++){const o=3+Math.floor(Math.random()*5);a.push({cardId:"fake-"+d,id:"fake-"+d,firstname:"IA",name:n[d%n.length],country_code:"XX",club_id:null,job:s,job2:null,note_g:s==="GK"?o:2,note_d:s==="DEF"?o:2,note_m:s==="MIL"?o:2,note_a:s==="ATT"?o:2,rarity:"normal",boost:0,used:!1,_line:s}),d++}const l=ot(a.length);a.forEach((c,o)=>{c._col=l[o]}),i[s]=a}return i}function go(e,t,i){e.innerHTML=`
  ...
  <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div id="duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center, rgba(10,61,30,0.4), rgba(10,61,30,0.92))"></div>
          ...
          <button id="start-match-btn" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,0.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">
            ▶ Commencer le match
          </button>`,L.style.transition="opacity .45s ease",L.style.opacity="1",L.style.pointerEvents="auto",(k=document.getElementById("start-match-btn"))==null||k.addEventListener("click",_))},600)},700)},2800)}function xo(e){if(e.type==="duel"){const t=e.homeTotal>=e.aiTotal;return`
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:100px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const N=t.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
          ...
          </div>`}const w=t.log[t.log.length-1];return w?'<div style="padding:2px 4px">'+xo(w)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})()}
    </div>
    ...
    ${(()=>{const w=window.innerWidth>=700,N=(Q,z,T)=>{var K,X;const f=(t.gcDefs||[]).find(re=>re.name===Q.gc_type),x={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[f==null?void 0:f.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",b={purple:"#b06ce0",light_blue:"#00d4ef"}[f==null?void 0:f.color]||"#b06ce0",A=(f==null?void 0:f.name)||Q.gc_type,y=(f==null?void 0:f.effect)||((K=he[Q.gc_type])==null?void 0:K.desc)||"",F=f!=null&&f.image_url?`/manager-wars/icons/${f.image_url}`:null,j=((X=he[Q.gc_type])==null?void 0:X.icon)||"⚡",D=Math.round(T*.22),B=Math.round(T*.22),C=T-D-B,O=A.length>12?7:9;return`<div class="gc-mini" data-gc-id="${Q.id}" data-gc-type="${Q.gc_type}"
        ...
        <div class="terrain-wrapper" style="overflow:hidden;flex-shrink:0;display:flex;align-items:center;justify-content:center">
          ${Ke(t.homeTeam,t.formation,t.phase,n,300,300,o)}
        </div>
      </div>`;return w?`
  ...
  </div>`;function h(){const w=e.querySelector(".match-screen");if(!w)return;const N=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);w.style.bottom="auto",w.style.height=N+"px",w.style.minHeight=N+"px",w.style.maxHeight=N+"px",w.style.overflow="hidden";const E=e.querySelector("#mobile-action-bar"),H=e.querySelector("#mobile-play-area");E&&H&&(H.style.paddingBottom=E.offsetHeight+"px")}if(h(),setTimeout(h,120),setTimeout(h,400),setTimeout(h,1e3),t._vvBound||(t._vvBound=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",h),window.visualViewport.addEventListener("scroll",h)),window.addEventListener("resize",h)),function(){const N=e.querySelector(".terrain-wrapper svg");N&&(N.removeAttribute("width"),N.removeAttribute("height"),N.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",N.setAttribute("viewBox","-26 -26 352 352"),N.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),t._resizeBound||(t._resizeBound=!0,window.addEventListener("resize",()=>{const w=e.querySelector(".terrain-wrapper svg");w&&(w.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0")})),t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase==="attack"||t.phase==="defense"){let w=!1,N=30;const E=()=>document.getElementById("match-timer"),H=()=>{const q=E();if(!q)return;const V=String(Math.floor(N/60)).padStart(2,"0"),te=String(N%60).padStart(2,"0");q.textContent=` ${V}:${te}`,q.style.color=w?"#ff2222":"#ff9500",q.style.fontWeight="900"};H(),t._timerInt=setInterval(()=>{if(N--,N<0)if(!w)w=!0,N=15,H();else{clearInterval(t._timerInt),t._timerInt=null,t.homeScore=0,t.aiScore=3;const q=document.createElement("div");q.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;color:#fff;padding:24px;text-align:center",q.innerHTML='<div style="font-size:56px">⏱️</div><div style="font-size:24px;font-weight:900;color:#ff4444">MATCH PERDU PAR FORFAIT</div><div style="font-size:14px;color:rgba(255,255,255,0.6)">Temps écoulé</div>',document.body.appendChild(q),setTimeout(()=>{q.remove(),Pe(e,t,i)},2500)}else H()},1e3)}($=document.getElementById("match-quit"))==null||$.addEventListener("click",()=>{be(e),confirm("Abandonner ? Résultat : défaite 3-0")&&(t.homeScore=0,t.aiScore=3,Pe(e,t,i))}),(M=document.getElementById("view-ai"))==null||M.addEventListener("click",()=>To(t,i)),(v=document.getElementById("toggle-history"))==null||v.addEventListener("click",()=>{var w;(w=document.getElementById("match-history-panel"))==null||w.classList.add("open")}),(L=document.getElementById("close-history"))==null||L.addEventListener("click",()=>{var w;(w=document.getElementById("match-history-panel"))==null||w.classList.remove("open")}),(k=document.getElementById("btn-action"))==null||k.addEventListener("click",()=>{t.selected.length!==0&&(p?ho(e,t,i):g&&bo(e,t,i))}),(G=document.getElementById("btn-results"))==null||G.addEventListener("click",()=>Pe(e,t,i)),e.querySelectorAll(".match-slot-hit").forEach(w=>{w.addEventListener("click",()=>yo(w,t,e,i))}),e.querySelectorAll(".gc-mini").forEach(w=>{w.addEventListener("click",()=>$o(w.dataset.gcId,w.dataset.gcType,e,t,i))}),(P=document.getElementById("boost-card"))==null||P.addEventListener("click",()=>Eo(e,t,i)),e.querySelectorAll(".sub-btn-col").forEach(w=>{w.addEventListener("click",()=>Qt(e,t,i,w.dataset.subId))}),(U=document.getElementById("sub-btn-main"))==null||U.addEventListener("click",()=>Qt(e,t,i))}function yo(e,t,i,n){const d=e.dataset.cardId,s=e.dataset.role,a=t.selected.findIndex(l=>l.cardId===d);if(a!==-1)t.selected.splice(a,1);else{if(t.selected.length>=3){n.toast("Maximum 3 joueurs","error");return}const l=(t.homeTeam[s]||[]).find(c=>c.cardId===d);l&&t.selected.push({...l,_role:s,_line:s})}de(i,t,n)}function kt(e,t,i){e.matchId&&S.from("matches").update({last_player_id:i}).eq("id",e.matchId).then(()=>{})}function ho(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),kt(t,i,i.state.profile.id);const n=!["GK","DEF","MIL","ATT"].some(a=>(t.aiTeam[a]||[]).some(l=>!l.used)),d=t.selected.map(a=>{const l=(t.homeTeam[a._role]||[]).find(o=>o.cardId===a.cardId)||a,c=n&&["GK","DEF"].includes(a._role);return{...l,_line:a._role,...c?{note_a:Math.max(1,Number(l.note_a)||0)}:{}}}),s=bt(d,t.modifiers.home);t.pendingAttack={...s,players:[...d],side:"home"},t.selected.forEach(a=>{const l=(t.homeTeam[a._role]||[]).find(c=>c.cardId===a.cardId);l&&(l.used=!0)}),t.log.push({text:`⚔️ Vous attaquez : ${s.total} (base ${s.base}${s.links?` +${s.links} liens`:""}) — ${t.selected.map(a=>a.name).join(", ")}`,type:"info"}),t.selected=[],t.modifiers.home={},t.phase="ai-defense",de(e,t,i),setTimeout(()=>vo(e,t,i),1200)}function bo(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),kt(t,i,i.state.profile.id);const n=t.selected.map(l=>({...(t.homeTeam[l._role]||[]).find(o=>o.cardId===l.cardId)||l,_line:l._role})),d=vt(n,t.modifiers.home);t.selected.forEach(l=>{const c=(t.homeTeam[l._role]||[]).find(o=>o.cardId===l.cardId);c&&(c.used=!0)});const s=wt(t.pendingAttack.total,d.total,t.modifiers.home),a={type:"duel",title:"Défense",aiPlayers:(t.pendingAttack.players||[]).map(l=>({name:l.name,note:l._line==="MIL"?l.note_m:l.note_a,portrait:ge(l),job:l.job,country_code:l.country_code,rarity:l.rarity,clubName:l.clubName,clubLogo:l.clubLogo})),homePlayers:t.selected.map(l=>{const c=(t.homeTeam[l._role]||[]).find(o=>o.cardId===l.cardId)||l;return{name:c.name,note:(c._line==="GK"?Number(c.note_g)||0:c._line==="MIL"?Number(c.note_m)||0:Number(c.note_d)||0)+(c.boost||0),portrait:ge(c),job:c.job,country_code:c.country_code,rarity:c.rarity,clubName:c.clubName,clubLogo:c.clubLogo}}),homeTotal:d.total,aiTotal:t.pendingAttack.total,isGoal:!1,homeScored:!1,text:""};if(s.shielded)a.text="🛡️ Bouclier ! But annulé.",t.log.push(a);else if(s.goal){t.aiScore++,a.isGoal=!0,a.homeScored=!1,a.text=`⚽ BUT IA ! (${t.pendingAttack.total} > ${d.total})`,t.log.push(a),t.selected=[],t.modifiers.home={},t.pendingAttack=null,de(e,t,i),rt(a.aiPlayers,t.homeScore,t.aiScore,!1,()=>{Ce(e,t,i,"home-attack")});return}else a.text=`🧤 Défense réussie ! (${d.total} ≥ ${t.pendingAttack.total})`,t.log.push(a);t.selected=[],t.modifiers.home={},t.pendingAttack=null,Ce(e,t,i,"home-attack")}function mt(e,t,i){kt(t,i,null);const n=[...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]],d=ni(n,"attack",t.difficulty);if(!d.length){Je(e,t,i);return}const s=bt(d,t.modifiers.ai);t.pendingAttack={...s,players:d,side:"ai"},d.forEach(o=>{o.used=!0}),t.log.push({text:`🤖 IA attaque : ${s.total} (${d.map(o=>o.name).join(", ")})`,type:"info"}),t.modifiers.ai={};const a=[...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(o=>!o.used),c=(t.homeSubs||[]).filter(o=>!(t.usedSubIds||[]).includes(o.cardId)).length>0&&t.subsUsed<t.maxSubs;if(a.length===0&&!c){t.aiScore++;const o={type:"duel",isGoal:!0,homeScored:!1,aiPlayers:d.map(r=>({name:r.name,note:r._line==="MIL"?r.note_m:r.note_a,portrait:ge(r),job:r.job,country_code:r.country_code,rarity:r.rarity,clubName:r.clubName,clubLogo:r.clubLogo})),aiTotal:s.total,text:"⚽ BUT IA ! (aucun défenseur disponible)"};t.log.push(o),t.pendingAttack=null,de(e,t,i),rt(o.aiPlayers,t.homeScore,t.aiScore,!1,()=>{Ce(e,t,i,"home-attack")});return}t.phase="defense",de(e,t,i)}function vo(e,t,i){var o,r;const n=[...t.aiTeam.GK||[],...t.aiTeam.DEF||[],...t.aiTeam.MIL||[]],d=ni(n,"defense",t.difficulty);if(!["GK","DEF","MIL","ATT"].flatMap(p=>(t.aiTeam[p]||[]).filter(g=>!g.used)).length){t.homeScore++;const g={type:"duel",isGoal:!0,homeScored:!0,homePlayers:(((o=t.pendingAttack)==null?void 0:o.players)||[]).map(m=>({name:m.name,note:oe(m,m._line||m.job),portrait:ge(m),job:m.job,country_code:m.country_code,rarity:m.rarity,clubName:m.clubName,clubLogo:m.clubLogo})),homeTotal:((r=t.pendingAttack)==null?void 0:r.total)||0,aiTotal:0,text:"⚽ BUT ! L'IA n'a plus de joueurs — but automatique !"};t.log.push(g),t.modifiers.ai={},t.pendingAttack=null,de(e,t,i),rt(g.homePlayers,t.homeScore,t.aiScore,!0,()=>{Ce(e,t,i,"ai-attack")});return}const a=d.length>0?vt(d,t.modifiers.ai).total:0;d.forEach(p=>{p.used=!0});const l=wt(t.pendingAttack.total,a,t.modifiers.ai),c={type:"duel",title:"Attaque",homePlayers:(t.pendingAttack.players||[]).map(p=>({name:p.name,note:p._line==="MIL"?p.note_m:p.note_a,portrait:ge(p),job:p.job,country_code:p.country_code,rarity:p.rarity,clubName:p.clubName,clubLogo:p.clubLogo})),aiPlayers:d.map(p=>({name:p.name,note:p._line==="GK"?p.note_g:p._line==="MIL"?p.note_m:p.note_d,portrait:ge(p),job:p.job,country_code:p.country_code,rarity:p.rarity,clubName:p.clubName,clubLogo:p.clubLogo})),homeTotal:t.pendingAttack.total,aiTotal:a,isGoal:!1,homeScored:!1,text:""};if(l.shielded)c.text="🛡️ Bouclier IA !",t.log.push(c);else if(l.goal){t.homeScore++,c.isGoal=!0,c.homeScored=!0,c.text=`⚽ BUT ! (${t.pendingAttack.total} > ${a})`,t.log.push(c),t.modifiers.ai={},t.pendingAttack=null,de(e,t,i),rt(c.homePlayers,t.homeScore,t.aiScore,!0,()=>{Ce(e,t,i,"ai-attack")});return}else c.text=`🧤 IA défend (${a} ≥ ${t.pendingAttack.total})`,t.log.push(c);t.modifiers.ai={},t.pendingAttack=null,Ce(e,t,i,"ai-attack")}function Ce(e,t,i,n){if(t.round++,xi(t)){Pe(e,t,i);return}if(n==="home-attack"){if(![...t.homeTeam.MIL||[],...t.homeTeam.ATT||[]].filter(s=>!s.used).length){if(![...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(l=>!l.used).length){Je(e,t,i);return}if(![...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]].filter(l=>!l.used).length){Je(e,t,i);return}setTimeout(()=>mt(e,t,i),100);return}t.phase="attack",de(e,t,i)}else{if(![...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]].filter(s=>!s.used).length){Je(e,t,i);return}t.phase="ai-attack",de(e,t,i),setTimeout(()=>mt(e,t,i),800)}}function xi(e){const t=["MIL","ATT","GK","DEF"].some(n=>(e.homeTeam[n]||[]).some(d=>!d.used)),i=["MIL","ATT","GK","DEF"].some(n=>(e.aiTeam[n]||[]).some(d=>!d.used));return!t&&!i}function Je(e,t,i){xi(t)?Pe(e,t,i):(t.phase="attack",de(e,t,i))}function wo(e,t,i){const n=document.createElement("div");n.style.cssText=`
    position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:800;
    display:flex;flex-direction:column;align-items:center;justify-content:center;gap:20px;
    animation:subFadeIn 0.2s ease;
  `;const d=ge(e),s=ge(t),a=De[e.job]||"#555",l=De[t.job]||"#555",c=e.job==="GK"?e.note_g:e.job==="DEF"?e.note_d:e.job==="MIL"?e.note_m:e.note_a,o=t.job==="GK"?t.note_g:t.job==="DEF"?t.note_d:t.job==="MIL"?t.note_m:t.note_a;n.innerHTML=`
    <style>
  ...
  `,document.body.appendChild(n);let r=!1;const p=()=>{r||(r=!0,n.remove(),i())};n.addEventListener("click",p),setTimeout(p,2e3)}function Oe(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function Qt(e,t,i,n=null){if(t.phase!=="attack"){Oe("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){Oe(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const d=Object.values(t.homeTeam).flat().filter(p=>p.used),s=t.homeSubs.filter(p=>!t.usedSubIds.includes(p.cardId));if(!d.length){Oe("Aucun joueur utilisé à remplacer");return}if(!s.length){Oe("Aucun remplaçant disponible");return}let a=0,l=Math.max(0,s.findIndex(p=>p.cardId===n)),c=!1;const o=document.createElement("div");o.id="sub-overlay",o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function r(){var I,$,M,v,L,k;const p=d[a],g=s[l],m=Math.min(130,Math.round((window.innerWidth-90)/2)),_=Math.round(m*1.35),u=G=>`background:rgba(255,255,255,0.12);border:none;color:${G?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${G?"default":"pointer"};flex-shrink:0`;o.innerHTML=`
    ...
    </div>`,document.body.appendChild(m),(_=m.querySelector("#gc-back"))==null||_.addEventListener("click",()=>m.remove()),(u=m.querySelector("#gc-use"))==null||u.addEventListener("click",()=>{m.remove(),ko(e,t,i,n,d)})}function Xe(e,t,i,n,d,s){const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let l=[];function c(){var o,r;a.innerHTML=`
    ...
    <div style="display:flex;flex-direction:column;gap:6px">
      ${n.map(d=>`
        <div class="player-boost-opt" data-card-id="${d.cardId}"
          ...
          <div style="width:32px;height:32px;background:${De[d.job]||"#888"};border-radius:6px;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:900;font-size:13px">${oe(d,d._line||d.job)}</div>
          <div style="flex:1"><b>${d.firstname} ${d.name}</b><div style="font-size:11px;color:#888">${d._line||d.job}</div></div>
          <div style="color:#87CEEB;font-weight:700">+${t.boostCard.value}</div>
        </div>`).join("")}
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(d=>{d.addEventListener("click",()=>{const s=d.dataset.cardId;for(const a of["GK","DEF","MIL","ATT"]){const l=(t.homeTeam[a]||[]).find(c=>c.cardId===s);if(l){l.boost=(l.boost||0)+t.boostCard.value,t.log.push({text:`⚡ Boost +${t.boostCard.value} appliqué à ${l.name}`,type:"info"});break}}t.boostUsed=!0,i.closeModal(),de(e,t,i)})})}function rt(e,t,i,n,d){const s=document.createElement("div");s.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:900;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;overflow:hidden;cursor:pointer";const a=Array.from({length:10},(o,r)=>`
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(s);let l=!1;const c=()=>{l||(l=!0,s.remove(),setTimeout(()=>d(),50))};s.addEventListener("click",c),setTimeout(c,3500)}async function Pe(e,t,i){var r,p;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:n}=i,d=t.homeScore>t.aiScore,s=t.homeScore===t.aiScore,a=d?"victoire":s?"nul":"defaite",l=Li(t.mode,a);t.matchId&&await S.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:d?n.profile.id:null,home_credits_reward:l,played_at:new Date().toISOString()}).eq("id",t.matchId);const c={credits:(n.profile.credits||0)+l,matches_played:(n.profile.matches_played||0)+1};d?c.wins=(n.profile.wins||0)+1:s?c.draws=(n.profile.draws||0)+1:c.losses=(n.profile.losses||0)+1,await S.from("users").update(c).eq("id",n.profile.id),await i.refreshProfile();const o=document.createElement("div");o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",o.innerHTML=`
    ...
    </div>`,`<button class="btn btn-primary" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}async function Ao(e,t){await mi(e,t,"random",({deckId:i,formation:n,starters:d,subsRaw:s,gcCardsEnriched:a,gcDefs:l})=>{const c=o=>Io(e,t,i,n,d,s,o,l||[]);if(!a.length){c([]);return}ui(e,a,c)})}async function Io(e,t,i,n,d,s,a=[],l=[]){var $;const{state:c,navigate:o,toast:r}=t;let p=!1,g=null;$e(e),e.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const m=async(M=!0)=>{p=!0,g&&(S.removeChannel(g),g=null),M&&await S.rpc("cancel_matchmaking",{p_user_id:c.profile.id}).catch(()=>{})};($=document.getElementById("mm-cancel"))==null||$.addEventListener("click",async()=>{try{await m(!0)}catch{}be(e),o("home")});const _=async(M,v,L)=>{if(p)return;p=!0,g&&(S.removeChannel(g),g=null);const k=document.getElementById("mm-status");k&&(k.textContent="Adversaire trouvé !"),await new Promise(G=>setTimeout(G,600)),e.isConnected&&Lo(e,t,M,L,a,l)},{data:u,error:h}=await S.rpc("try_matchmake",{p_user_id:c.profile.id,p_deck_id:i});if(h||!(u!=null&&u.success)){r("Erreur de matchmaking","error"),be(e),o("home");return}if(u.matched){_(u.match_id,u.opponent_id,!1);return}const I=document.getElementById("mm-status");I&&(I.textContent="En attente d'un autre joueur..."),g=S.channel("matchmaking-"+c.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${c.profile.id}`},M=>{const v=M.new;v.status==="matched"&&v.match_id&&_(v.match_id,v.matched_with,!0)}).subscribe()}async function Lo(e,t,i,n,d=[],s=[]){const{state:a,navigate:l,toast:c}=t,o=n?"p1":"p2",r=n?"p2":"p1",p=(d||[]).map(f=>f.id),g=(d||[]).map(f=>({id:f.id,gc_type:f.gc_type,_gcDef:f._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:m}=await S.from("matches").select("*").eq("id",i).single();if(!m){c("Match introuvable","error"),l("home");return}async function _(){const[{data:f},{data:x},{data:b},{data:A}]=await Promise.all([S.rpc("get_deck_for_match",{p_deck_id:m.home_deck_id}),S.rpc("get_deck_for_match",{p_deck_id:m.away_deck_id}),S.from("users").select("id,pseudo,club_name").eq("id",m.home_id).single(),S.from("users").select("id,pseudo,club_name").eq("id",m.away_id).single()]),y=C=>({cardId:C.card_id,position:C.position,id:C.id,firstname:C.firstname,name:C.surname_encoded,country_code:C.country_code,club_id:C.club_id,job:C.job,job2:C.job2,note_g:Number(C.note_g)||0,note_d:Number(C.note_d)||0,note_m:Number(C.note_m)||0,note_a:Number(C.note_a)||0,rarity:C.rarity,skin:C.skin,hair:C.hair,hair_length:C.hair_length,clubName:C.club_encoded_name||null,clubLogo:C.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}),F=((f==null?void 0:f.starters)||[]).map(C=>y(C)),j=((x==null?void 0:x.starters)||[]).map(C=>y(C)),D=(f==null?void 0:f.formation)||"4-4-2",B=(x==null?void 0:x.formation)||"4-4-2";return{p1Team:nt(F,D),p2Team:nt(j,B),p1Subs:((f==null?void 0:f.subs)||[]).map(C=>y(C)),p2Subs:((x==null?void 0:x.subs)||[]).map(C=>y(C)),p1Formation:D,p2Formation:B,p1Name:(b==null?void 0:b.club_name)||(b==null?void 0:b.pseudo)||"Joueur 1",p2Name:(A==null?void 0:A.club_name)||(A==null?void 0:A.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:n?p:[],gc_p2:n?[]:p,gcCardsFull_p1:n?g:[],gcCardsFull_p2:n?[]:g,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,lastActionAt:new Date().toISOString()}}let u=m.game_state&&Object.keys(m.game_state).length?m.game_state:null;if(!u)if(n){u=await _();const{data:f}=await S.from("matches").select("game_state").eq("id",i).single();!(f!=null&&f.game_state)||!Object.keys(f.game_state).length?await S.from("matches").update({game_state:u,turn_user_id:m.home_id}).eq("id",i):u=f.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let f=0;f<30&&!u;f++){await new Promise(b=>setTimeout(b,400));const{data:x}=await S.from("matches").select("game_state").eq("id",i).single();x!=null&&x.game_state&&Object.keys(x.game_state).length&&(u=x.game_state)}if(!u){c("Erreur de synchronisation","error"),l("home");return}u.gc_p2=p,u.gcCardsFull_p2=g,await S.from("matches").update({game_state:u}).eq("id",i)}let h=!1,I=null,$=!1;function M(f){var A,y;try{S.removeChannel(v)}catch{}const x=f.winner_id===a.profile.id;(A=b.remove)==null||A.call(b);const b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center",b.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(b),(y=b.querySelector("#pvp-end-home"))==null||y.addEventListener("click",()=>{b.remove(),be(e),l("home")})}const v=S.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},f=>{const x=f.new;try{if(x.status==="finished"||x.forfeit){if(h)return;h=!0,I&&(clearInterval(I),I=null),x.game_state&&(u=x.game_state),M(x);return}x.game_state&&(u=x.game_state,w())}catch(b){console.error("[PvP] crash:",b)}}).subscribe();async function L(f){Object.assign(u,f),u.lastActionAt=new Date().toISOString();const{error:x}=await S.from("matches").update({game_state:u}).eq("id",i);x&&c("Erreur de synchronisation","error");try{w()}catch(b){console.error("[PvP] renderPvpScreen crash:",b)}}async function k(){if(h)return;h=!0,I&&(clearInterval(I),I=null);const f=n?m.away_id:m.home_id;try{await S.from("matches").update({status:"finished",forfeit:!0,winner_id:f}).eq("id",i)}catch{}try{S.removeChannel(v)}catch{}be(e),l("home")}const G={BOOST_STAT:({value:f=1,count:x=1,roles:b=[]},A,y)=>{const F=A[o+"Team"],j=Object.entries(F).filter(([D])=>!b.length||b.includes(D)).flatMap(([D,B])=>B.filter(C=>!C.used).map(C=>({...C,_line:D})));if(!j.length){A.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),y(A);return}P(j,x,`Choisir ${x} joueur(s) à booster (+${f})`,D=>{D.forEach(B=>{const C=(F[B._line]||[]).find(O=>O.cardId===B.cardId);C&&(C.boost=(C.boost||0)+f,A.log.push({text:`⚡ +${f} sur ${C.name}`,type:"info"}))}),A[o+"Team"]=F,y(A)})},DEBUFF_STAT:({value:f=1,count:x=1,roles:b=[],target:A="ai"},y,F)=>{const j=A==="home"?o:r,D=y[j+"Team"],B=A==="home"?"allié":"adverse",C=Object.entries(D).filter(([O])=>!b.length||b.includes(O)).flatMap(([O,K])=>K.filter(X=>!X.used).map(X=>({...X,_line:O})));if(!C.length){y.log.push({text:`🎯 Aucun joueur ${B}`,type:"info"}),F(y);return}P(C,x,`Choisir ${x} joueur(s) ${B}(s) (-${f})`,O=>{O.forEach(K=>{const X=(D[K._line]||[]).find(re=>re.cardId===K.cardId);X&&(X.boost=(X.boost||0)-f,y.log.push({text:`🎯 -${f} sur ${X.name}`,type:"info"}))}),y[j+"Team"]=D,F(y)})},GRAY_PLAYER:({count:f=1,roles:x=[],target:b="ai"},A,y)=>{const F=b==="home"?o:r,j=A[F+"Team"],D=b==="home"?"allié":"adverse",B=Object.entries(j).filter(([C])=>!x.length||x.includes(C)).flatMap(([C,O])=>O.filter(K=>!K.used).map(K=>({...K,_line:C})));if(!B.length){A.log.push({text:`❌ Aucun joueur ${D}`,type:"info"}),y(A);return}P(B,f,`Choisir ${f} joueur(s) ${D}(s) à exclure`,C=>{C.forEach(O=>{const K=(j[O._line]||[]).find(X=>X.cardId===O.cardId);K&&(K.used=!0,A.log.push({text:`❌ ${K.name} exclu !`,type:"info"}))}),A[F+"Team"]=j,y(A)})},REVIVE_PLAYER:({count:f=1,roles:x=[]},b,A)=>{const y=b[o+"Team"],F=Object.entries(y).filter(([j])=>!x.length||x.includes(j)).flatMap(([j,D])=>D.filter(B=>B.used).map(B=>({...B,_line:j})));if(!F.length){b.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),A(b);return}P(F,f,`Choisir ${f} joueur(s) à ressusciter`,j=>{j.forEach(D=>{const B=(y[D._line]||[]).find(C=>C.cardId===D.cardId);B&&(B.used=!1,b.log.push({text:`💫 ${B.name} ressuscité !`,type:"info"}))}),b[o+"Team"]=y,A(b)})},REMOVE_GOAL:({},f,x)=>{const b=r+"Score";f[b]>0?(f[b]--,f.log.push({text:"🚫 Dernier but annulé !",type:"info"})):f.log.push({text:"🚫 Aucun but à annuler",type:"info"}),x(f)},ADD_GOAL_DRAW:({},f,x)=>{f[o+"Score"]===f[r+"Score"]?(f[o+"Score"]++,f.log.push({text:"🎯 But bonus !",type:"info"})):f.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),x(f)},ADD_SUB:({value:f=1},x,b)=>{x.maxSubs=(x.maxSubs||3)+f,x.log.push({text:`🔄 +${f} remplacement(s)`,type:"info"}),b(x)},CUSTOM:({},f,x)=>x(f)};function P(f,x,b,A){const y=document.createElement("div");y.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let F=[];function j(){var B,C;const D=f.map(O=>{const K={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[O._line]||"#555",X=oe(O,O._line)+(O.boost||0),se=F.find(Y=>Y.cardId===O.cardId)?"#FFD700":"rgba(255,255,255,0.25)",J=O.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${O.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${se};background:${K};overflow:hidden;cursor:pointer;${J}">
        ...
        </div>`,(B=y.querySelector("#pp-close"))==null||B.addEventListener("click",()=>y.remove()),y.querySelectorAll(".pp-item").forEach(O=>{O.addEventListener("click",()=>{const K=O.dataset.cid,X=f.find(se=>se.cardId===K),re=F.findIndex(se=>se.cardId===K);X&&(re>-1?F.splice(re,1):F.length<x&&F.push(X),j())})}),(C=y.querySelector("#pp-confirm"))==null||C.addEventListener("click",()=>{y.remove(),A(F)})}j(),document.body.appendChild(y)}async function U(f,x){const A=(u["gcCardsFull_"+o]||[]).find(D=>D.id===f),y=(A==null?void 0:A._gcDef)||(u.gcDefs||[]).find(D=>{var B;return D.name===x||((B=D.name)==null?void 0:B.toLowerCase().trim())===(x==null?void 0:x.toLowerCase().trim())}),F=[...u["usedGc_"+o]||[],f];if(y!=null&&y.effect_type&&y.effect_type!=="CUSTOM"){const D=G[y.effect_type];if(D){const B={...u};D(y.effect_params||{},B,async C=>{C["usedGc_"+o]=F,await L(C)});return}}const j={...u};switch(x){case"Remplacement+":j.maxSubs=(j.maxSubs||3)+1,j.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const D=r+"Score";j[D]>0&&(j[D]--,j.log.push({text:"🚫 But annulé",type:"info"}));break}}j["usedGc_"+o]=F,await L(j)}function w(){var zt,St,jt,Ct,Bt;if(u.phase==="reveal")return N();if(u.phase==="midfield")return H();if(u.phase==="finished")return T();const f=u[o+"Team"],x=u[r+"Team"],b=u[o+"Score"],A=u[r+"Score"],y=u[o+"Name"],F=u[r+"Name"],j=u.phase===o+"-attack",D=u.phase===o+"-defense",B=Array.isArray(u["selected_"+o])?u["selected_"+o]:[],C=B.map(R=>R.cardId),O=window.innerWidth>=700,K=u[o+"Subs"]||[],X=u["usedSubIds_"+o]||[],re=K.filter(R=>!X.includes(R.cardId)),se=u["gcCardsFull_"+o]||[],J=u["usedGc_"+o]||[],Y=se.filter(R=>!J.includes(R.id)),ie=u.boostOwner===o&&!u.boostUsed,ce=!["GK","DEF","MIL","ATT"].some(R=>(x[R]||[]).some(W=>!W.used)),ue=[...f.MIL||[],...f.ATT||[]].filter(R=>!R.used),xe=j&&ce&&ue.length===0?[...f.GK||[],...f.DEF||[]].filter(R=>!R.used).map(R=>R.cardId):[];function fe(R,W,le){var qt,Rt;const ee=R._gcDef,we={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ee==null?void 0:ee.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",Ie={purple:"#b06ce0",light_blue:"#00d4ef"}[ee==null?void 0:ee.color]||"#b06ce0",qe=(ee==null?void 0:ee.name)||R.gc_type,Re=(ee==null?void 0:ee.effect)||((qt=he[R.gc_type])==null?void 0:qt.desc)||"",Ft=ee!=null&&ee.image_url?`/manager-wars/icons/${ee.image_url}`:null,$i=((Rt=he[R.gc_type])==null?void 0:Rt.icon)||"⚡",Dt=Math.round(le*.22),Gt=Math.round(le*.22),dt=le-Dt-Gt,_i=qe.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${R.id}" data-gc-type="${R.gc_type}"
        ...
        <div style="height:${Dt}px;background:rgba(255,255,255,0.14);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 3px">
          <span style="font-size:${_i}px;font-weight:900;color:#fff;text-align:center;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${W-6}px">${qe}</span>
          <span style="font-size:6px;color:rgba(255,255,255,0.45)">⚡ GC</span>
        </div>
        <div style="height:${dt}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
          ${Ft?`<img src="${Ft}" style="max-width:${W-10}px;max-height:${dt-6}px;object-fit:contain">`:`<span style="font-size:${Math.round(dt*.55)}px">${$i}</span>`}
        </div>
      ...
      </div>`}const ye=(R,W)=>W?me(110,150):fe(R,110,150),Ye=(R,W)=>W?me(68,95):fe(R,68,95),Ge=O?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",Te=j?`<button id="pvp-action" style="${Ge};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${B.length===0?"disabled":""}>⚔️ ATTAQUEZ <span id="pvp-timer"></span></button>`:D?`<button id="pvp-action" style="${Ge};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${B.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="pvp-timer"></span></button>`:`<div style="font-size:11px;color:rgba(255,255,255,0.3);text-align:center;padding:4px">⏳ Tour de ${F}</div>`,At=j||D?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${B.length}/3 sélectionné(s)</div>`:"",It=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${O?56:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
    ...
    </div>`,Lt=j?"attack":D?"defense":"idle",bi=`<div style="overflow:hidden;min-width:0;flex:1;min-height:0;display:flex;align-items:center;justify-content:center" id="match-field">
      <div class="terrain-wrapper" style="overflow:hidden;flex-shrink:0;display:flex;align-items:center;justify-content:center">
        ${Ke(f,u[o+"Formation"],Lt,C,300,300,xe)}
      </div>
    </div>`;function vi(R){if(R.type==="duel"){const W=(R.homeTotal||0)>=(R.aiTotal||0);return`<div style="background:rgba(255,255,255,0.05);border-radius:8px;padding:5px 6px;border:1px solid rgba(255,255,255,0.08)">
        ...
        </div>`}const R=(u.log||[]).slice(-1)[0];return R?'<div style="padding:2px 4px">'+vi(R)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})(),Mt=`
      <div style="display:flex;align-items:center;padding:8px 10px;background:rgba(0,0,0,0.5);gap:6px;flex-shrink:0">
      ...
      <div style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:100px">${wi}</div>
      ...
      <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
        ...
        <div style="display:flex;flex:0.8;min-height:0;overflow:hidden">
          ...
          <div style="width:140px;flex-shrink:0;display:flex;flex-direction:column;padding:10px 8px;background:rgba(0,0,0,0.2)">
            ...
            <div style="width:100%;flex-shrink:0;padding-top:8px">${Te}${At}</div>
          </div>
        </div>
      </div>`:e.innerHTML=`
      <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
      ...
      </div>`;function Ae(){const R=e.querySelector(".match-screen");if(!R)return;const W=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);R.style.height=W+"px",R.style.minHeight=W+"px",R.style.maxHeight=W+"px",R.style.overflow="hidden";const le=e.querySelector("#mobile-action-bar"),ee=e.querySelector("#mobile-play-area");le&&ee&&(ee.style.paddingBottom=le.offsetHeight+"px")}if(Ae(),setTimeout(Ae,120),setTimeout(Ae,400),$||($=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",Ae),window.visualViewport.addEventListener("scroll",Ae)),window.addEventListener("resize",Ae)),function(){const W=e.querySelector(".terrain-wrapper svg");W&&(W.removeAttribute("width"),W.removeAttribute("height"),W.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",W.setAttribute("viewBox","-26 -26 352 352"),W.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),e.querySelectorAll(".match-slot-hit").forEach(R=>{R.addEventListener("click",()=>{if(!j&&!D)return;const W=R.dataset.cardId,le=R.dataset.role,ee=(f[le]||[]).find(Re=>Re.cardId===W);if(!ee||ee.used)return;const we=xe.includes(W);if(j&&!["MIL","ATT"].includes(le)&&!we)return;Array.isArray(u["selected_"+o])||(u["selected_"+o]=[]);const Ie=u["selected_"+o],qe=Ie.findIndex(Re=>Re.cardId===W);qe>-1?Ie.splice(qe,1):Ie.length<3&&Ie.push({...ee,_role:le}),w()})}),e.querySelectorAll(".pvp-sub-btn").forEach(R=>{R.addEventListener("click",()=>{if(!j){c("Remplacement uniquement avant une attaque","warning");return}te(R.dataset.subId)})}),e.querySelectorAll(".pvp-gc-mini").forEach(R=>{R.addEventListener("click",()=>q(R.dataset.gcId,R.dataset.gcType))}),(zt=e.querySelector("#pvp-boost-card"))==null||zt.addEventListener("click",()=>V()),(St=e.querySelector("#pvp-action"))==null||St.addEventListener("click",()=>{j?pe():D&&Q()}),(jt=e.querySelector("#pvp-quit"))==null||jt.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&k()}),(Ct=e.querySelector("#pvp-view-opp"))==null||Ct.addEventListener("click",()=>ne()),(Bt=e.querySelector("#pvp-toggle-history"))==null||Bt.addEventListener("click",()=>Z()),I&&(clearInterval(I),I=null),(j||D)&&!h){let R=30,W=!1;const le=()=>document.getElementById("pvp-timer"),ee=()=>{le()&&(le().textContent=R+"s",le().style.color=W?"#ff4444":"#fff")};ee(),I=setInterval(()=>{R--,R<0?W?(clearInterval(I),I=null,k()):(W=!0,R=15,ee()):ee()},1e3)}}function N(){$e(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
      <div style="font-size:20px;font-weight:900;color:#ff6b6b">${u[r+"Name"]}</div>
      <div style="width:min(90vw,420px)">${Ue(u[r+"Team"],u[r+"Formation"],null,[],300,300)}</div>
  ...
  `;function c(o){const r=document.getElementById("mkt-content"),p=o==="buy"?a:l;if(p.length===0){r.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      ${g.map(m=>{var M,v,L,k;const _=(M=m.card)==null?void 0:M.player;if(!_)return"";const u=_.job==="GK"?_.note_g:_.job==="DEF"?_.note_d:_.job==="MIL"?_.note_m:_.note_a,h=Mo[_.rarity],I=(i.profile.credits||0)>=m.price,$=m.status==="sold";return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${$?"opacity:0.65":""}">
          <div style="width:44px;height:44px;border-radius:8px;background:${Co(_.job)};color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:18px;border:2px solid ${h};flex-shrink:0">${u}</div>
          <div style="flex:1;min-width:0">
    ...
    </div>`,r.querySelectorAll("[data-buy]").forEach(m=>{m.addEventListener("click",()=>So(m.dataset.buy,p,e,t))}),r.querySelectorAll("[data-cancel]").forEach(m=>{m.addEventListener("click",()=>jo(m.dataset.cancel,e,t))})}c("buy"),e.querySelectorAll(".mkt-tab").forEach(o=>{o.addEventListener("click",()=>{e.querySelectorAll(".mkt-tab").forEach(r=>{const p=r===o;r.style.background=p?"var(--green)":"#fff",r.style.color=p?"#fff":"var(--gray-600)",r.style.borderColor=p?"var(--green)":"var(--gray-200)"}),c(o.dataset.tab)})})}async function So(e,t,i,n){var p;const{state:d,toast:s}=n,a=t.find(g=>g.id===e);if(!a)return;const l=a.price,c=d.profile.credits||0,o=(p=a.card)==null?void 0:p.player;if(c<l){s("Crédits insuffisants","error");return}if(!confirm(`Acheter ${o==null?void 0:o.firstname} ${o==null?void 0:o.surname_encoded} pour ${l.toLocaleString("fr")} crédits ?`))return;const r=document.querySelector(`[data-buy="${e}"]`);r&&(r.disabled=!0,r.textContent="⏳");try{const{data:g,error:m}=await S.rpc("buy_market_card",{p_listing_id:e,p_buyer_id:d.profile.id});if(m)throw new Error(m.message);if(!(g!=null&&g.success))throw new Error((g==null?void 0:g.error)||"Achat impossible");d.profile.credits=c-l;const _=document.querySelector("[data-credits]")||document.querySelector(".credits-display");_&&(_.textContent=(c-l).toLocaleString("fr")+" cr."),s(`✅ ${o==null?void 0:o.surname_encoded} ajouté à ta collection !`,"success"),Et(i,n)}catch(g){s("❌ "+g.message,"error"),r&&(r.disabled=!1,r.textContent="Acheter")}}async function jo(e,t,i){const{toast:n}=i,{data:d}=await S.from("market_listings").select("card_id").eq("id",e).single();await S.from("market_listings").update({status:"cancelled"}).eq("id",e),d&&await S.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",d.card_id),n("Annonce retirée","success"),Et(t,i)}function Co(e){return{GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[e]||"#888"}async function Bo(e,{state:t,navigate:i,toast:n}){e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:d}=await S.from("users").select("id,pseudo,club_name,trophies_top1,trophies_top2,trophies_top3,wins,level").order("trophies_top1",{ascending:!1}).limit(100);e.innerHTML=`
  <div class="page">
    <div class="page-header">
  ...
  `}async function Fo(e,{state:t,navigate:i,toast:n}){const d=t.profile;if(!d)return;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:s}=await S.from("matches").select(`id,home_id,away_id,home_score,away_score,status,mode,winner_id,created_at,played_at,
  ...
  </div>`}const ae={user:null,profile:null,page:"home",params:{}};function Qe(e,t="info",i=3e3){const n=document.getElementById("toast");n&&(n.textContent=e,n.className=`show ${t}`,clearTimeout(n._t),n._t=setTimeout(()=>{n.className=""},i))}function Do(e,t,i=""){document.getElementById("modal-title").textContent=e,document.getElementById("modal-body").innerHTML=t,document.getElementById("modal-footer").innerHTML=i,document.getElementById("modal-overlay").classList.remove("hidden")}function xt(){document.getElementById("modal-overlay").classList.add("hidden")}async function Ze(){if(!ae.user)return;const{data:e}=await S.from("users").select("*").eq("id",ae.user.id).single();e&&(ae.profile=e)}const Tt="mw_theme";function at(){return localStorage.getItem(Tt)||"light"}function Go(e){var t;localStorage.setItem(Tt,e),yt(e),(t=ae.profile)!=null&&t.id&&S.from("users").update({theme:e}).eq("id",ae.profile.id).then(()=>{})}function yt(e){document.documentElement.setAttribute("data-theme",e)}function et(e,t={}){ae.page=e,ae.params=t,yi()}async function yi(){const e=document.getElementById("page-content");if(!e)return;document.querySelectorAll(".bottom-nav a").forEach(n=>{n.classList.toggle("active",n.dataset.page===ae.page)});const t=document.getElementById("nav-credits");t&&ae.profile&&(t.textContent=`💰 ${(ae.profile.credits||0).toLocaleString("fr")}`);const i={state:ae,navigate:et,toast:Qe,openModal:Do,closeModal:xt,refreshProfile:Ze};switch(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽</div>',ae.page){case"home":await Pt(e,i);break;case"collection":await Si(e,i);break;case"decks":await pt(e,i);break;case"boosters":await Yi(e,i);break;case"match":{(ae.params&&ae.params.matchMode||"vs_ai_easy")==="random"?await Ao(e,i):await po(e,i);break}case"market":await zo(e,i);break;case"rankings":await Bo(e,i);break;case"matches":await Fo(e,i);break;default:await Pt(e,i)}}function qo(){const e=document.getElementById("app"),t=ae.profile;if(!t)return;const i="/manager-wars/icons/";e.innerHTML=`
    <nav class="top-nav">
  ...
  `,document.querySelectorAll(".bottom-nav a").forEach(n=>{n.addEventListener("click",d=>{d.preventDefault(),et(n.dataset.page)})}),document.getElementById("nav-logo").addEventListener("click",()=>et("home")),document.getElementById("nav-credits").addEventListener("click",()=>et("boosters")),document.getElementById("theme-toggle").addEventListener("click",()=>{const d=at()==="dark"?"light":"dark";Go(d),Zt(d)}),Zt(at())}function Zt(e){const t=document.getElementById("theme-icon");t&&(t.textContent=e==="dark"?"☀️":"🌙")}async function Ro(){yt(at()),document.getElementById("modal-overlay").addEventListener("click",t=>{t.target===t.currentTarget&&xt()}),document.getElementById("modal-close").addEventListener("click",xt);const{data:{session:e}}=await S.auth.getSession();if(!e){ti(),Nt(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:Qe});return}ae.user=e.user,await Ze(),ti();try{const{data:t}=await S.from("formation_links_overrides").select("formation, links"),i={};(t||[]).forEach(n=>{i[n.formation]=n.links}),ki(i)}catch(t){console.warn("Impossible de charger les overrides de formation:",t)}if(!ae.profile){Ei(document.getElementById("app"),{state:ae,navigate:async()=>{await Ze(),ei()},toast:Qe,refreshProfile:Ze});return}ae.profile.theme&&ae.profile.theme!==at()&&(localStorage.setItem(Tt,ae.profile.theme),yt(ae.profile.theme)),ei(),S.auth.onAuthStateChange(async(t,i)=>{t==="SIGNED_OUT"&&(ae.user=null,ae.profile=null,document.getElementById("app").innerHTML="",Nt(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:Qe}))})}function No(){return Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight)}function lt(){const e=document.getElementById("app");e&&(e.style.height=No()+"px")}window.addEventListener("resize",lt);window.addEventListener("orientationchange",()=>setTimeout(lt,150));window.visualViewport&&window.visualViewport.addEventListener("resize",lt);function ei(){const e=document.getElementById("app");e.style.display="flex",e.style.flexDirection="column",lt(),qo(),yi()}function ti(){const e=document.getElementById("app-loader"),t=document.getElementById("app");t&&(t.style.display=""),e&&(e.classList.add("zoom-out"),setTimeout(()=>e.style.display="none",500))}function hi(e){var n;const t=document.getElementById("app-loader");if(t&&(t.style.display="none"),document.getElementById("boot-error"))return;const i=document.createElement("div");i.id="boot-error",i.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:99999;gap:16px;color:#fff;padding:24px;text-align:center",i.innerHTML=`
    <div style="font-size:42px">📡</div>
    <div style="font-size:18px;font-weight:900">Connexion impossible</div>
    <div style="font-size:13px;color:rgba(255,255,255,0.6);max-width:280px">${e||"Le chargement a échoué. Vérifie ta connexion et réessaie."}</div>
    <button id="boot-retry" style="margin-top:8px;padding:12px 30px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer">Réessayer</button>`,document.body.appendChild(i),(n=document.getElementById("boot-retry"))==null||n.addEventListener("click",()=>window.location.reload())}Ro().catch(e=>{console.error("Échec du démarrage:",e),hi()});setTimeout(()=>{const e=document.getElementById("app-loader");e&&e.style.display!=="none"&&!e.classList.contains("zoom-out")&&!document.getElementById("boot-error")&&hi("Le serveur met trop de temps à répondre.")},12e3);
