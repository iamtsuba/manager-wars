import{s as _,l as ae,F as Ge,b as xe,c as st,d as Mt}from"./formation-links-CDBKB_z4.js";function We(e,{navigate:t,toast:i}){e.innerHTML=`
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),l=(d||[]).filter(E=>E.card_type==="player"&&E.player),s=(d||[]).filter(E=>E.card_type==="game_changer"),p=(d||[]).filter(E=>E.card_type==="formation"),{data:u}=await _.from("gc_definitions").select("name,gc_type,color,effect,image_url"),f={};(u||[]).forEach(E=>{f[E.name]=E});const m=Object.keys(Ge),y=Object.keys(se),h={};l.forEach(E=>{const k=E.player.id;h[k]=(h[k]||0)+1}),new Set(Object.keys(h).map(E=>String(E)));const $=new Set(p.map(E=>E.formation)),v=new Set(s.map(E=>E.gc_type));let x="player",w="Tous",I="",L=!1;function g(){return[...l].sort((E,k)=>{const B=ve.indexOf(E.player.job),j=ve.indexOf(k.player.job);return B!==j?B-j:(E.player.surname_encoded||"").localeCompare(k.player.surname_encoded||"")})}function b(){return[...r||[]].sort((E,k)=>{const B=ve.indexOf(E.job),j=ve.indexOf(k.job);return B!==j?B-j:(E.surname_encoded||"").localeCompare(k.surname_encoded||"")})}function A(){return g().filter(E=>{const k=E.player,B=w==="Tous"||k.job===w,j=!I||`${k.firstname} ${k.surname_encoded}`.toLowerCase().includes(I);return B&&j})}function T(){return b().filter(E=>{const k=w==="Tous"||E.job===w,B=!I||`${E.firstname} ${E.surname_encoded}`.toLowerCase().includes(I);return k&&B})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
  ...
  </div>`;function z(){const E=document.getElementById("col-filters");E&&(x==="player"?(E.innerHTML=`
        ...
        <div style="display:flex;gap:6px;overflow-x:auto;padding-bottom:2px;align-items:center">
          ...
          <button id="show-all-btn"
            style="flex-shrink:0;margin-left:auto;padding:4px 12px;border-radius:20px;font-size:12px;font-weight:600;cursor:pointer;
              border:1.5px solid ${L?"var(--yellow)":"var(--gray-200)"};
              background:${L?"var(--yellow)":"#fff"};
              color:${L?"#111":"var(--gray-600)"}; font-size:18px; padding:5px 10px">
            ${L?"👁️":"🚫👁️"}
          </button>
        </div>`,document.getElementById("col-search").addEventListener("input",k=>{I=k.target.value.toLowerCase(),S()}),e.querySelectorAll(".filter-btn").forEach(k=>{k.addEventListener("click",()=>{w=k.dataset.job,z(),S()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{L=!L,z(),S()})):(E.innerHTML=`
        <div style="display:flex;justify-content:flex-end">
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{L=!L,z(),S()})))}function S(){const E=document.getElementById("col-grid");E&&(x==="player"?N(E):x==="formation"?ye(E):me(E))}function G(E,k,B,j,M){M=M||"#7a28b8";const D=document.getElementById("col-grid"),C=document.getElementById("col-big");if(!D||!C)return;var P=0;function H(){C.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+k(E[P])+"</div>";var K=C.querySelector("[data-card-id],[data-form-id],[data-gc-id]");K&&K.addEventListener("click",function(){j(E[P])}),requestAnimationFrame(function(){var O=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!O||!C)){var Y=C.clientHeight-8,Q=C.clientWidth-24,Z=O.offsetHeight,ee=O.offsetWidth;if(Z>0&&ee>0&&Y>40){var ie=Math.min(Y/Z,Q/ee,1);O.style.transform="scale("+ie.toFixed(3)+")",O.style.transformOrigin="top center"}}}),D.innerHTML=E.map(function(O,Y){return'<div class="col-mini-item" data-idx="'+Y+'" style="flex-shrink:0;cursor:pointer;border-radius:8px;border:1.5px solid '+(Y===P?M:"transparent")+';transition:border-color .15s;overflow:hidden">'+B(O,Y===P)+"</div>"}).join(""),D.querySelectorAll(".col-mini-item").forEach(function(O){O.addEventListener("click",function(){P=Number(O.dataset.idx),H(),O.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}H()}function R(E){var k=.54,B=Math.round(140*k),j=Math.round(310*k),M;if(!E||E._fake){var D=E?E.player:null;if(!D)return"";M=Je(D)}else M=Le(E,"");return'<div style="width:'+B+"px;height:"+j+'px;overflow:hidden;position:relative;flex-shrink:0"><div style="transform:scale('+k+');transform-origin:top left;position:absolute;top:0;left:0;pointer-events:none">'+M+"</div></div>"}function F(E,k,B){k=k||100,B=B||140;var j=xe[E]||{},M={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},D=Math.max(3,Math.round(k*.06)),C=Object.entries(j).map(function(H){var K=H[0],O=H[1],Y=K.replace(/\d+$/,""),Q=M[Y]||"#888",Z=Math.round(O.x*k),ee=Math.round(O.y*B);return'<circle cx="'+Z+'" cy="'+ee+'" r="'+D+'" fill="'+Q+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),P=Math.max(1,Math.round(k/50));return'<svg viewBox="0 0 '+k+" "+B+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+k+'" height="'+B+'" fill="#1A6B3C"/><rect x="'+Math.round(k*.2)+'" y="'+Math.round(B*.02)+'" width="'+Math.round(k*.6)+'" height="'+Math.round(B*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+P+'"/><line x1="0" y1="'+Math.round(B*.5)+'" x2="'+k+'" y2="'+Math.round(B*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+P+'"/><ellipse cx="'+Math.round(k*.5)+'" cy="'+Math.round(B*.5)+'" rx="'+Math.round(k*.18)+'" ry="'+Math.round(B*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+P+'"/><rect x="'+Math.round(k*.2)+'" y="'+Math.round(B*.82)+'" width="'+Math.round(k*.6)+'" height="'+Math.round(B*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+P+'"/>'+C+"</svg>"}function J(E,k,B){var j=B>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+B+"</div>":"",M=k?'data-form-id="'+k.id+'"':"",D=E.length>7?14:E.length>5?16:19,C=!!k;return"<div "+M+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(C?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(C?"":"filter:grayscale(1);opacity:0.5")+'">'+j+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(C?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+D+"px;font-weight:900;color:"+(C?"#1A6B3C":"#aaa")+';line-height:1">'+E+'</div></div><div style="flex:1;overflow:hidden;background:'+(C?"#1A6B3C":"#ccc")+'">'+F(E,140,220)+"</div></div>"}function W(E,k){var B=.54,j=Math.round(140*B),M=Math.round(305*B),D=Math.round(M*.22),C=M-D,P=E.length>7?5:7,H=F(E,j,C),K=k?"1.5px solid #2a7a40":"1px solid #ddd",O=k?"":"filter:grayscale(1);opacity:0.45;",Y=k?"#1A6B3C":"#bbb",Q="#fff";return'<div style="width:'+j+"px;height:"+M+"px;border-radius:6px;border:"+K+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+O+'"><div style="height:'+D+"px;background:"+Y+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+P+"px;font-weight:900;color:"+Q+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(j-4)+'px">'+E+'</span></div><div style="height:'+C+'px;overflow:hidden;flex-shrink:0">'+H+"</div></div>"}function N(E){if(L){const k=T();if(!k.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const B=k.map(j=>l.find(M=>M.player.id===j.id)||{_fake:!0,player:j,id:"fake-"+j.id});G(B,j=>j._fake?Je(j.player):Le(j,""),j=>j._fake?R({player:j.player,id:"x",_fake:!0}):R(j),j=>{j._fake||Qe(j,l,h,t)},"#1A6B3C")}else{const k=A();if(!k.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const B={},j=[];k.forEach(M=>{B[M.player.id]||(B[M.player.id]=!0,j.push(M))}),G(j,M=>{const D=h[M.player.id]||1,C=D>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${D}</div>`:"",H=l.filter(K=>K.player.id===M.player.id&&K.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Le(M,C+H)},M=>R(M),M=>Qe(M,l,h,t),"#1A6B3C")}}function ye(E){const k=L?m:[...$];if(!k.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const B=k.map(j=>({formation:j,card:p.find(M=>M.formation===j)||null,owned:$.has(j)}));G(B,({formation:j,card:M,owned:D})=>J(j,D?M:null,D?p.filter(C=>C.formation===j).length:0),({formation:j,owned:M})=>W(j,M),({card:j,owned:M})=>{M&&j&&Rt(j,p,t,o)},"#1A6B3C")}function me(E){const k=Object.keys(f),B=L?k.length?k:y:[...v];if(!B.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const j=B.map(M=>({type:M,gc:se[M]||{icon:"⚡",desc:""},def:f[M]||null,owned:v.has(M),card:s.find(D=>D.gc_type===M)||null}));G(j,({type:M,gc:D,def:C,owned:P,card:H})=>{const K=P?s.filter(Lt=>Lt.gc_type===M).length:0,O=K>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${K}</div>`:"",Y=(C==null?void 0:C.gc_type)==="ultra_game_changer",Q={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},Z={purple:"#b06ce0",light_blue:"#00d4ef"},ee=Q[C==null?void 0:C.color]||Q.purple,ie=Z[C==null?void 0:C.color]||Z.purple,he=(C==null?void 0:C.effect)||D.desc||"",Ve=C!=null&&C.image_url?`/manager-wars/icons/${C.image_url}`:null;return P&&H?`<div data-gc-id="${H.id}" data-gc-type="${M}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${ie};background:${ee};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${ie}66;cursor:pointer">
          ${O}
          ...
          <div style="height:220px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
            ${Ve?`<img src="${Ve}" style="max-width:120px;max-height:120px;object-fit:contain;border-radius:6px">`:`<span style="font-size:64px">${D.icon}</span>`}
          </div>
        ...
        </div>`},({type:M,gc:D,def:C,owned:P})=>P?(()=>{const H=Math.round(75.60000000000001),K=Math.round(310*.54),O=Math.round(K*.65),Y=Math.round(K*.18),Q={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},Z={purple:"#9b59b6",light_blue:"#00bcd4"},ee=Q[C==null?void 0:C.color]||Q.purple,ie=Z[C==null?void 0:C.color]||Z.purple,he=C!=null&&C.image_url?`/manager-wars/icons/${C.image_url}`:null;return`<div style="width:${H}px;height:${K}px;border-radius:8px;background:${ee};border:1px solid ${ie};display:flex;flex-direction:column;overflow:hidden"><div style="height:${Y}px;background:rgba(255,255,255,0.15);display:flex;align-items:center;justify-content:center"><span style="font-size:7px;font-weight:900;color:#fff;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${H-6}px">${M}</span></div><div style="height:${O}px;display:flex;align-items:center;justify-content:center">${he?`<img src="${he}" style="max-width:${H-8}px;max-height:${O-4}px;object-fit:contain">`:`<span style="font-size:24px">${D.icon}</span>`}</div><div style="flex:1;display:flex;align-items:center;justify-content:center"><span style="font-size:7px;color:rgba(255,255,255,0.7);text-align:center;padding:0 2px">${((C==null?void 0:C.effect)||D.desc||"").slice(0,20)}</span></div></div>`})():(()=>{const H=Math.round(75.60000000000001),K=Math.round(310*.54);return`<div style="width:${H}px;height:${K}px;border-radius:8px;background:#eee;border:1px solid #ddd;display:flex;flex-direction:column;align-items:center;justify-content:center;filter:grayscale(1);opacity:0.45"><span style="font-size:22px">${D.icon}</span><span style="font-size:7px;color:#aaa;margin-top:4px;text-align:center;padding:0 4px">${M}</span></div>`})(),({type:M,owned:D})=>{D&&Gt(M,o)},"#7a28b8")}e.querySelectorAll(".col-tab-btn").forEach(E=>{E.addEventListener("click",()=>{x=E.dataset.tab,w="Tous",I="",L=!1,e.querySelectorAll(".col-tab-btn").forEach(k=>{const B=k.dataset.tab===x;k.style.borderBottomColor=B?"var(--green)":"transparent",k.style.color=B?"var(--green)":"var(--gray-600)"}),z(),S()})}),z(),S()}function Gt(e,t){const i=se[e]||{icon:"⚡",desc:"Effet spécial."};t("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const pe=1e3;function Rt(e,t,i,a){var y,h,$;const{state:c,toast:o,closeModal:n,navigate:d,refreshProfile:r}=i,l=e.formation,s={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function p(){const v=xe[l]||{},x=Ge[l]||[],w=290,I=360,L=20;function g(A){const T=v[A];return T?{x:T.x*w,y:T.y*I}:null}let b=`<svg width="${w}" height="${I}" viewBox="0 0 ${w} ${I}" xmlns="http://www.w3.org/2000/svg">`;for(const[A,T]of x){const z=g(A),S=g(T);!z||!S||(b+=`<line x1="${z.x}" y1="${z.y}" x2="${S.x}" y2="${S.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const A of Object.keys(v)){const T=g(A);if(!T)continue;const z=A.replace(/\d+/,""),S=s[z]||"#555";b+=`<circle cx="${T.x}" cy="${T.y}" r="${L}" fill="${S}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,b+=`<text x="${T.x}" y="${T.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${z}</text>`}return b+="</svg>",b}const u=t.filter(v=>v.formation===l),f=u.length,m=!e.is_for_sale;a(`Formation ${l}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      ...
      <div style="background:#f9f9f9;border-radius:10px;padding:12px;display:flex;justify-content:space-between;align-items:center">
        <div>
          ...
          <div style="font-size:18px;font-weight:900;color:var(--yellow)">${pe.toLocaleString("fr")} crédits</div>
    ...
    <div style="margin-top:12px;border-top:1px solid var(--gray-200);padding-top:12px">
      ...
      <div style="display:flex;gap:8px">
        <input type="number" id="sell-price-form" min="1" placeholder="Prix en crédits" value="${pe}"
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(y=document.getElementById("direct-sell-form-btn"))==null||y.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${l} pour ${pe.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const v=u.find(w=>!w.is_for_sale)||u[0];if(!v){o("Aucune carte à vendre","error");return}const{error:x}=await _.from("cards").delete().eq("id",v.id);if(x){o(x.message,"error");return}await _.from("users").update({credits:(c.profile.credits||0)+pe}).eq("id",c.profile.id),await r(),o(`+${pe.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),n(),d("collection")}),(h=document.getElementById("market-sell-form-btn"))==null||h.addEventListener("click",async()=>{const v=parseInt(document.getElementById("sell-price-form").value);if(!v||v<1){o("Prix invalide","error");return}await _.from("cards").update({is_for_sale:!0,sale_price:v}).eq("id",e.id),await _.from("market_listings").insert({seller_id:c.profile.id,card_id:e.id,price:v}),o("Carte mise en vente sur le marché !","success"),n(),d("collection")}),($=document.getElementById("cancel-sell-form-btn"))==null||$.addEventListener("click",async()=>{await _.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await _.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),o("Annonce retirée","success"),n(),d("collection")})}function Qe(e,t,i,a){var L,g,b,A,T,z;const{state:c,toast:o,openModal:n,closeModal:d,navigate:r,refreshProfile:l}=a,s=e.player,p=t.filter(S=>S.player.id===s.id),u=p.length,f=Ft[s.rarity]||1e3,m=s.rarity!=="legende",y=gt(s),h=fe(s,s.job),$=s.job2?fe(s,s.job2):null,v=Ee[s.job]||"#1A6B3C",x=s.job2?Ee[s.job2]:null,w=ft[s.rarity]||"#ccc",I=Re[s.country_code]||s.country_code||"";n(`${s.firstname} ${s.surname_encoded}`,`<div style="display:flex;gap:16px;flex-wrap:wrap;justify-content:center">
      ...
      <div style="width:140px;border-radius:12px;padding:6px;background:${w};flex-shrink:0">
        <div style="background:#f2e8d2;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
          ...
          <div style="background:#f2e8d2;padding:3px 6px;display:flex;align-items:center;justify-content:space-between;min-height:24px">
            ...
            ${(L=s.clubs)!=null&&L.logo_url?`<img src="${s.clubs.logo_url}" style="width:22px;height:16px;object-fit:contain">`:`<div style="background:#1a3a7a;color:#fff;border-radius:2px;padding:1px 3px;font-size:6px;font-weight:800">${(((g=s.clubs)==null?void 0:g.encoded_name)||"").slice(0,6)}</div>`}
          </div>
        </div>
      </div>
  ...
  </div>`,document.getElementById("new-deck-btn").addEventListener("click",async()=>{const n=prompt("Nom du deck :",`Deck ${((o==null?void 0:o.length)||0)+1}`);if(!n)return;const{data:d,error:r}=await _.from("decks").insert({owner_id:i.profile.id,name:n}).select().single();if(r){c(r.message,"error");return}c("Deck créé !","success"),Ze(d.id,e,t)}),e.querySelectorAll("[data-open-deck]").forEach(n=>{n.addEventListener("click",()=>Ze(n.dataset.openDeck,e,t))}),e.querySelectorAll("[data-rename]").forEach(n=>{n.addEventListener("click",async()=>{const d=prompt("Nouveau nom :",n.dataset.name);if(!d||d===n.dataset.name)return;const{error:r}=await _.from("decks").update({name:d}).eq("id",n.dataset.rename);if(r){c(r.message,"error");return}c("Deck renommé !","success"),ze(e,t)})}),e.querySelectorAll("[data-delete]").forEach(n=>{n.addEventListener("click",async()=>{if(!confirm(`Supprimer le deck "${n.dataset.name}" ? Cette action est irréversible.`))return;await _.from("deck_cards").delete().eq("deck_id",n.dataset.delete);const{error:d}=await _.from("decks").delete().eq("id",n.dataset.delete);if(d){c(d.message,"error");return}c("Deck supprimé.","success"),ze(e,t)})})}async function Ze(e,t,i){const{state:a,toast:c}=i;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await _.from("decks").select("*").eq("id",e).single(),{data:n}=await _.from("cards").select(`id, card_type, formation,
      player:players(id, firstname, surname_encoded, country_code, club_id, job, job2,
        note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length,
        clubs(encoded_name, logo_url))`).eq("owner_id",a.profile.id),d=(n||[]).filter(f=>f.card_type==="player"&&f.player),r=(n||[]).filter(f=>f.card_type==="formation"),l=r.map(f=>f.formation).filter(Boolean),{data:s}=await _.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",e);let p=o.formation||"4-4-2";l.length>0&&!l.includes(p)&&(p=l[0]);const u={deckId:e,name:o.name,formation:p,formationCardId:o.formation_card_id,slots:{},subs:[],playerCards:d,formationCards:r,availableFormations:l};(s||[]).forEach(f=>{f.is_starter?u.slots[f.position]=f.card_id:u.subs.includes(f.card_id)||u.subs.push(f.card_id)}),de(t,u,i)}function de(e,t,i){var r;const{navigate:a}=i;Ae[t.formation];const c=et(t.formation),o=c.filter(l=>t.slots[l]).length,n=t.availableFormations.length>0?t.availableFormations:Object.keys(Ae),d=t.subs.map(l=>t.playerCards.find(s=>s.id===l)).filter(Boolean);[...Object.values(t.slots),...t.subs],e.innerHTML=`
  <div class="page">
  ...
  </div>`,Nt(e,t,c,i),document.getElementById("builder-back").addEventListener("click",()=>a("decks")),document.getElementById("formation-select").addEventListener("change",l=>{t.formation=l.target.value;const s=et(t.formation),p={};s.forEach(u=>{t.slots[u]&&(p[u]=t.slots[u])}),t.slots=p,de(e,t,i)}),document.getElementById("save-deck").addEventListener("click",()=>Ht(t,i)),e.querySelectorAll("[data-remove-sub]").forEach(l=>{l.addEventListener("click",()=>{t.subs=t.subs.filter(s=>s!==l.dataset.removeSub),de(e,t,i)})}),(r=document.getElementById("add-sub-btn"))==null||r.addEventListener("click",()=>{Pt(t,e,i)})}function Nt(e,t,i,a){const c=e.querySelector("#deck-field");if(!c)return;const o=xe[t.formation]||{},n=st(t.formation)||[],d={};for(const $ of i){const v=t.slots[$],x=v?t.playerCards.find(w=>w.id===v):null;d[$]=x?x.player:null}const r=300,l=300,s=48,p=64,u=13,f=16,m=38;function y($){const v=o[$];return v?{x:v.x*r,y:v.y*l}:null}let h="";for(const[$,v]of n){const x=y($),w=y(v);if(!x||!w)continue;const I=d[$]?{...d[$],club_id:d[$].club_id,country_code:d[$].country_code,rarity:d[$].rarity}:null,L=d[v]?{...d[v],club_id:d[v].club_id,country_code:d[v].country_code,rarity:d[v].rarity}:null,g=ae(I,L);g==="#ff3333"||g==="#cc2222"?h+=`<line x1="${x.x.toFixed(1)}" y1="${x.y.toFixed(1)}" x2="${w.x.toFixed(1)}" y2="${w.y.toFixed(1)}" stroke="${g}" stroke-width="2.5" stroke-linecap="round" opacity="0.4"/>`:(h+=`<line x1="${x.x.toFixed(1)}" y1="${x.y.toFixed(1)}" x2="${w.x.toFixed(1)}" y2="${w.y.toFixed(1)}" stroke="${g}" stroke-width="8" stroke-linecap="round" opacity="0.2"/>`,h+=`<line x1="${x.x.toFixed(1)}" y1="${x.y.toFixed(1)}" x2="${w.x.toFixed(1)}" y2="${w.y.toFixed(1)}" stroke="${g}" stroke-width="2.5" stroke-linecap="round" opacity="0.9"/>`)}for(const $ of i){const v=y($);if(!v)continue;const x=d[$],w=$.replace(/\d+/,""),I=te[w]||"#555",L=(v.x-s/2).toFixed(1),g=(v.y-p/2).toFixed(1),b={normal:"#aaa",pépite:"#D4A017",papyte:"#222",légende:"#7a28b8"}[x==null?void 0:x.rarity]||"#aaa";if(x){const A=Te(x),T=yt(x),z=xt(x.country_code),S=Number(w==="GK"?x.note_g:w==="DEF"?x.note_d:w==="MIL"?x.note_m:x.note_a)||0,G=p-u-f;h+=`<defs><clipPath id="dcp-${$}"><rect x="${L}" y="${(v.y-p/2+u).toFixed(1)}" width="${s}" height="${G}"/></clipPath></defs>`,h+=`<rect x="${L}" y="${g}" width="${s}" height="${p}" rx="5" fill="${I}"/>`,A&&(h+=`<image href="${A}" x="${L}" y="${(v.y-p/2+u).toFixed(1)}" width="${s}" height="${G}" clip-path="url(#dcp-${$})" preserveAspectRatio="xMidYMin slice"/>`),h+=`<rect x="${L}" y="${g}" width="${s}" height="${u}" fill="rgba(255,255,255,0.93)"/>`,h+=`<text x="${v.x.toFixed(1)}" y="${(v.y-p/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(x.surname_encoded||"").slice(0,9)}</text>`;const R=(v.y+p/2-f).toFixed(1);h+=`<rect x="${L}" y="${R}" width="${s}" height="${f}" fill="rgba(255,255,255,0.93)"/>`,z&&(h+=`<image href="${z}" x="${(v.x-21).toFixed(1)}" y="${(v.y+p/2-f+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`),h+=`<text x="${v.x.toFixed(1)}" y="${(v.y+p/2-f/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${S}</text>`,T&&(h+=`<image href="${T}" x="${(v.x+s/2-14).toFixed(1)}" y="${(v.y+p/2-f+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`),h+=`<rect x="${L}" y="${g}" width="${s}" height="${p}" rx="5" fill="none" stroke="${b}" stroke-width="2"/>`}else h+=`<rect x="${L}" y="${g}" width="${s}" height="${p}" rx="5" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="5,3"/>`,h+=`<text x="${v.x.toFixed(1)}" y="${v.y.toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="22" fill="rgba(255,255,255,0.35)">+</text>`,h+=`<text x="${v.x.toFixed(1)}" y="${(v.y+16).toFixed(1)}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.3)">${w}</text>`;h+=`<rect x="${L}" y="${g}" width="${s}" height="${p}" rx="5" fill="rgba(0,0,0,0.01)" class="deck-slot-hit" data-pos="${$}" style="cursor:pointer"/>`}c.innerHTML=`<svg viewBox="${-m} ${-m} ${r+m*2} ${l+m*2}" width="100%" style="display:block;max-width:440px;margin:0 auto">${h}</svg>`,c.querySelectorAll(".deck-slot-hit").forEach($=>{$.addEventListener("click",()=>Ot($.dataset.pos,t,e,a))})}function Ot(e,t,i,a){var u,f,m;const{openModal:c,closeModal:o}=a,n=e.replace(/\d+/,""),d=t.slots[e],r=d?t.playerCards.find(y=>y.id===d):null;(u=r==null?void 0:r.player)==null||u.id;const l=new Set;Object.entries(t.slots).forEach(([y,h])=>{var v;if(y===e||!h)return;const $=t.playerCards.find(x=>x.id===h);(v=$==null?void 0:$.player)!=null&&v.id&&l.add($.player.id)}),t.subs.forEach(y=>{var $;const h=t.playerCards.find(v=>v.id===y);($=h==null?void 0:h.player)!=null&&$.id&&l.add(h.player.id)});const s=new Set,p=t.playerCards.filter(y=>{const h=y.player;return!(h.job===n||h.job2===n)||l.has(h.id)||s.has(h.id)?!1:(s.add(h.id),!0)});p.sort((y,h)=>{const $=n==="GK"?y.player.note_g:n==="DEF"?y.player.note_d:n==="MIL"?y.player.note_m:y.player.note_a;return(n==="GK"?h.player.note_g:n==="DEF"?h.player.note_d:n==="MIL"?h.player.note_m:h.player.note_a)-$}),c(`Choisir ${n} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${p.length>0?p.map(y=>{var w,I;const h=y.player,$=n==="GK"?h.note_g:n==="DEF"?h.note_d:n==="MIL"?h.note_m:h.note_a,v=Te(h),x={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[h.rarity];return`<div class="player-option" data-card-id="${y.id}"
          ...
          <div style="width:44px;height:44px;border-radius:8px;overflow:hidden;flex-shrink:0;background:#dde;border:2px solid ${te[n]}">
            ${v?`<img src="${v}" style="width:100%;height:100%;object-fit:cover">`:`<div style="width:100%;height:100%;background:${te[n]};display:flex;align-items:center;justify-content:center;color:#fff;font-size:10px;font-weight:700">${n}</div>`}
          </div>
          <!-- Infos -->
  ...
  `,document.body.appendChild(a);let c=!1;document.getElementById("pack-visual").addEventListener("click",()=>{if(c)return;c=!0;const f=document.getElementById("pack-visual");f.classList.add("shaking"),setTimeout(()=>{f.classList.add("pack-open"),setTimeout(()=>{document.getElementById("pack-phase").style.display="none",n(0)},600)},500)});let o=0;function n(f){o=f;const m=document.getElementById("reveal-phase");m.style.display="flex",d(f)}function d(f){var L;const m=e[f],y=document.getElementById("card-counter"),h=document.getElementById("single-card-slot"),$=document.getElementById("card-tap-hint");y&&(y.textContent=`Carte ${f+1} / ${e.length}`),$&&($.textContent=f<e.length-1?"Appuie pour continuer →":"Appuie pour voir toutes tes cartes");const v=it(m),x=m.card_type==="player"&&((L=m.player)==null?void 0:L.rarity)==="legende";h.innerHTML=`
      ...
      </div>`).join("")}let l=null;function s(){const f=document.getElementById("fireworks-canvas");if(!f)return;f.width=window.innerWidth,f.height=window.innerHeight;const m=f.getContext("2d"),y=[];function h(){const v=Math.random()*f.width,x=Math.random()*f.height*.6,w=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],I=w[Math.floor(Math.random()*w.length)];for(let L=0;L<60;L++){const g=Math.PI*2/60*L,b=2+Math.random()*5;y.push({x:v,y:x,vx:Math.cos(g)*b,vy:Math.sin(g)*b,alpha:1,color:I,size:2+Math.random()*3})}}h(),l=setInterval(h,600);function $(){if(document.getElementById("fireworks-canvas")){m.clearRect(0,0,f.width,f.height);for(let v=y.length-1;v>=0;v--){const x=y[v];if(x.x+=x.vx,x.y+=x.vy+.08,x.vy*=.98,x.alpha-=.018,x.alpha<=0){y.splice(v,1);continue}m.globalAlpha=x.alpha,m.fillStyle=x.color,m.beginPath(),m.arc(x.x,x.y,x.size,0,Math.PI*2),m.fill()}m.globalAlpha=1,(l!==null||y.length>0)&&requestAnimationFrame($)}}$()}function p(){l!==null&&(clearInterval(l),l=null);const f=document.getElementById("fireworks-canvas");f&&f.getContext("2d").clearRect(0,0,f.width,f.height)}document.getElementById("reveal-collection").addEventListener("click",()=>{p(),a.remove(),i("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{p(),a.remove(),i("boosters")})}function it(e){var t,i,a,c;if(e.card_type==="player"&&e.player){const o=e.player,n=o.job||"ATT",d=Qt[n]||"#1A6B3C",r=Zt[o.rarity]||"#ccc",l=n==="GK"?o.note_g:n==="DEF"?o.note_d:n==="MIL"?o.note_m:o.note_a,s=Jt(o),p={MA:"MAROC",FR:"FRANCE",AR:"ARGENTINE",PT:"PORTUGAL",BR:"BRESIL",ES:"ESPAGNE",DE:"ALLEMAGNE",GB:"ANGLETERRE",IT:"ITALIE",CM:"CAMEROUN",SN:"SENEGAL"}[o.country_code]||o.country_code;return`<div style="width:140px;height:200px;background:#f2e8d2;border-radius:12px;border:3px solid ${r};overflow:hidden;display:flex;flex-direction:column">
      <!-- Nom -->
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}const ui="simulation",vt="VOTRE_ZONE_ID";function fi(){switch(ui){case"propellerads":return gi();case"adsense":return xi();default:return qe()}}function qe(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:#111;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:14px";let i=5;t.innerHTML=`
      ...
      <button id="ad-skip" disabled style="margin-top:8px;padding:10px 24px;border-radius:20px;border:none;background:rgba(255,255,255,0.15);color:rgba(255,255,255,0.4);font-size:13px;cursor:default">
        Passer (5)
      </button>`,document.body.appendChild(t);const a=setInterval(()=>{i--;const c=document.getElementById("ad-cd"),o=document.getElementById("ad-skip");c&&(c.textContent=i),o&&(o.textContent=i>0?`Passer (${i})`:"✓ Continuer"),i<=0&&(clearInterval(a),o&&(o.disabled=!1,o.style.cssText=o.style.cssText.replace("rgba(255,255,255,0.15)","#1A6B3C").replace("rgba(255,255,255,0.4)","#fff").replace("default","pointer")),o==null||o.addEventListener("click",()=>{t.remove(),e(!0)},{once:!0}))},1e3)})}function gi(){return new Promise((e,t)=>{if(typeof window.propeller>"u")return console.warn("[Ad] PropellerAds non chargé → simulation"),e(qe());window.propeller.push({zone_id:vt,onComplete:()=>e(!0),onSkip:()=>t(new Error("Publicité ignorée")),onError:()=>t(new Error("Erreur publicité"))})})}function xi(){return new Promise((e,t)=>{if(typeof window.adsbygoogle>"u")return console.warn("[Ad] AdSense non chargé → simulation"),e(qe());try{(window.adsbygoogle=window.adsbygoogle||[]).push({google_ad_client:"ca-pub-VOTRE_PUBLISHER_ID",google_ad_slot:vt,google_ad_format:"rewarded",on_reward:()=>e(!0),on_dismiss:()=>t(new Error("Publicité fermée"))})}catch(i){t(i)}})}const le={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},ce={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function bt(e,t,i,a,c){var o;e.innerHTML=`<div class="match-screen" style="display:flex;align-items:center;justify-content:center;min-height:100vh">
    <div style="text-align:center;padding:40px;color:#fff">
      <div style="font-size:48px;margin-bottom:16px">${t}</div>
      <p style="margin-bottom:16px">${i}</p>
      <button class="btn btn-primary" id="msg-btn">${a}</button>
    </div>
  </div>`,(o=document.getElementById("msg-btn"))==null||o.addEventListener("click",c)}function X(e){const t=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function je(e){var i,a;const t=e.player;return{cardId:e.id,id:t.id,firstname:t.firstname,name:t.surname_encoded,country_code:t.country_code,club_id:t.club_id,job:t.job,job2:t.job2,note_g:Number(t.note_g)||0,note_d:Number(t.note_d)||0,note_m:Number(t.note_m)||0,note_a:Number(t.note_a)||0,rarity:t.rarity,skin:t.skin,hair:t.hair,hair_length:t.hair_length,clubName:((i=t.clubs)==null?void 0:i.encoded_name)||null,clubLogo:((a=t.clubs)==null?void 0:a.logo_url)||null,boost:0,used:!1,_line:null,_col:null}}function Ne(e){return e===1?[1]:e===2?[0,2]:e===3?[0,1,2]:e===4?[0,1,1,2]:e===5?[0,1,1,1,2]:[1]}function yi(){const e=Math.random()*100;return e<.1?4:e<5?3:e<20?2:1}function wt(e,t){const i=le[t]||le["4-4-2"],a={GK:[],DEF:[],MIL:[],ATT:[]},c=[...e];for(const o of["GK","DEF","MIL","ATT"]){const n=[];for(let r=0;r<i[o];r++){let l=c.findIndex(s=>s.job===o);if(l===-1&&(l=c.findIndex(s=>s.job2===o)),l===-1&&(l=0),c[l]){const s={...c[l],_line:o};n.push(s),c.splice(l,1)}}const d=Ne(n.length);n.forEach((r,l)=>{r._col=d[l]}),a[o]=n}return a}async function mi(e,t){var n,d;const{data:i}=await _.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(60);if(!i||i.length<11)return hi(e);const a=le[e]||le["4-4-2"],c={GK:[],DEF:[],MIL:[],ATT:[]},o=[...i];for(const r of["GK","DEF","MIL","ATT"]){const l=o.filter(m=>m.job===r),s=o.filter(m=>m.job!==r),p=[...l,...s],u=[];for(let m=0;m<a[r];m++){const y=p[m]||o[m];y&&u.push({cardId:"ai-"+y.id+"-"+m,id:y.id,firstname:y.firstname,name:y.surname_encoded,country_code:y.country_code,club_id:y.club_id,job:y.job,job2:y.job2,note_g:Number(y.note_g)||0,note_d:Number(y.note_d)||0,note_m:Number(y.note_m)||0,note_a:Number(y.note_a)||0,rarity:y.rarity,skin:y.skin,hair:y.hair,hair_length:y.hair_length,clubName:((n=y.clubs)==null?void 0:n.encoded_name)||null,clubLogo:((d=y.clubs)==null?void 0:d.logo_url)||null,boost:0,used:!1,_line:r})}const f=Ne(u.length);u.forEach((m,y)=>{m._col=f[y]}),c[r]=u}return c}function hi(e){const t=le[e]||le["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},a=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let c=0;for(const o of["GK","DEF","MIL","ATT"]){const n=[];for(let r=0;r<t[o];r++){const l=3+Math.floor(Math.random()*5);n.push({cardId:"fake-"+c,id:"fake-"+c,firstname:"IA",name:a[c%a.length],country_code:"XX",club_id:null,job:o,job2:null,note_g:o==="GK"?l:2,note_d:o==="DEF"?l:2,note_m:o==="MIL"?l:2,note_a:o==="ATT"?l:2,rarity:"normal",boost:0,used:!1,_line:o}),c++}const d=Ne(n.length);n.forEach((r,l)=>{r._col=d[l]}),i[o]=n}return i}async function vi(e,t){var L;const{state:i,navigate:a,toast:c}=t,o=i.params||{};e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const n=o.matchMode||"vs_ai_easy",d=n.replace("vs_ai_",""),r=n;if(!o.deckId)return wi(e,t,n);const l=o.deckId,[{data:s},{data:p}]=await Promise.all([_.from("decks").select("formation,name").eq("id",l).single(),_.from("deck_cards").select(`position, is_starter, slot_order,
        card:cards(id, card_type, formation,
          player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
            note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
            clubs(encoded_name,logo_url)))`).eq("deck_id",l).order("slot_order")]),u=(p||[]).filter(g=>{var b;return g.is_starter&&((b=g.card)==null?void 0:b.player)}).map(g=>je(g.card)),f=(p||[]).filter(g=>{var b;return!g.is_starter&&((b=g.card)==null?void 0:b.player)}).map(g=>je(g.card));if(u.length<11){bt(e,"⚠️",`Deck incomplet (${u.length}/11).`,"Compléter",()=>a("decks"));return}const m=(p||[]).find(g=>{var b;return((b=g.card)==null?void 0:b.card_type)==="formation"}),y=(s==null?void 0:s.formation)||((L=m==null?void 0:m.card)==null?void 0:L.formation)||"4-4-2",{data:h}=await _.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",i.profile.id).eq("card_type","game_changer"),{data:$}=await _.from("gc_definitions").select("*").eq("is_active",!0),v=(h||[]).map(g=>({...g,_gcDef:($==null?void 0:$.find(b=>b.name===g.gc_type||b.id===g.gc_definition_id))||null})),x=wt(u,y),w=await mi(y),I=async g=>{const{data:b}=await _.from("matches").insert({home_id:i.profile.id,away_id:null,mode:r,home_deck_id:l,status:"in_progress"}).select().single(),A={gcDefs:$||[],matchId:b==null?void 0:b.id,mode:r,difficulty:d,formation:y,homeTeam:x,aiTeam:w,homeSubs:f,subsUsed:0,maxSubs:Math.min(f.length,3),homeScore:0,aiScore:0,gcCards:g,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};$i(e,A,t)};if(!v.length){I([]);return}bi(e,v,I)}function bi(e,t,i){const c=new Set,o=t.filter(s=>{const p=s.gc_type||s.id;return c.has(p)?!1:(c.add(p),!0)});let n=[];function d(s,p){const u=s._gcDef,f={purple:"linear-gradient(135deg,#3d0a7a,#7a28b8)",light_blue:"linear-gradient(135deg,#006080,#00bcd4)"},m={purple:"#9b59b6",light_blue:"#00bcd4"},y=f[u==null?void 0:u.color]||f.purple,h=m[u==null?void 0:u.color]||m.purple;return`<div class="gc-select-card" data-id="${s.id}"
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",n).order("slot_order");let r=0;function l(){var v,x,w,I,L;const s=o[r],p=(d||[]).filter(g=>g.deck_id===s.id),u=p.filter(g=>{var b;return g.is_starter&&((b=g.card)==null?void 0:b.player)}).map(g=>je(g.card)),f=p.find(g=>{var b;return((b=g.card)==null?void 0:b.card_type)==="formation"}),m=s.formation||((v=f==null?void 0:f.card)==null?void 0:v.formation)||"4-4-2",y=u.length>=11?wt(u,m):null,h=u.length>=11;e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:calc(100dvh - 130px);overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">
        ${o.map((g,b)=>`<div style="width:7px;height:7px;border-radius:50%;background:${b===r?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}
      </div>`:""}
    ...
    </div>`,(x=document.getElementById("prev-deck"))==null||x.addEventListener("click",()=>{r>0&&(r--,l())}),(w=document.getElementById("next-deck"))==null||w.addEventListener("click",()=>{r<o.length-1&&(r++,l())}),(I=document.getElementById("validate-deck"))==null||I.addEventListener("click",()=>{h&&t.navigate("match",{matchMode:i,deckId:s.id})}),(L=document.getElementById("cancel-deck-select"))==null||L.addEventListener("click",()=>c("home"));const $=document.getElementById("deck-swipe-zone");if($){let g=0,b=0;$.addEventListener("touchstart",A=>{g=A.touches[0].clientX,b=A.touches[0].clientY},{passive:!0}),$.addEventListener("touchend",A=>{const T=A.changedTouches[0].clientX-g,z=A.changedTouches[0].clientY-b;Math.abs(T)<40||Math.abs(T)<Math.abs(z)||(T<0&&r<o.length-1?(r++,l()):T>0&&r>0&&(r--,l()))},{passive:!0})}}l()}function $i(e,t,i){e.innerHTML=`
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
  </div>`,setTimeout(()=>_i(e,t,i),5e3)}function _i(e,t,i){const a=t.homeTeam.MIL||[],c=t.aiTeam.MIL||[];function o(p){return p.reduce((u,f)=>u+U(f,"MIL"),0)}function n(p){let u=0;for(let f=0;f<p.length-1;f++){const m=ae(p[f],p[f+1]);m==="#00ff88"?u+=2:m==="#FFD700"&&(u+=1)}return u}const d=o(a)+n(a),r=o(c)+n(c),l=d>=r;function s(p,u,f){return`<div style="text-align:center">
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${p.map((m,y)=>{const h=X(m),$=y<p.length-1?ae(m,p[y+1]):null,v=$&&$!=="#ff3333"&&$!=="#cc2222";return`
          <div style="width:52px;height:52px;border-radius:8px;background:${f};position:relative;flex-shrink:0;overflow:hidden;border:2px solid rgba(255,255,255,0.3)">
            ...
            <div style="position:relative;z-index:1;font-size:15px;font-weight:900;color:#fff;text-shadow:0 1px 3px #000;text-align:center;padding-top:4px">${U(m,"MIL")}</div>
          ...
          ${$?`<div style="width:14px;height:4px;border-radius:2px;background:${$};flex-shrink:0;opacity:${v?.9:.3}"></div>`:""}
          `}).join("")}
      </div>
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div id="midfield-result" style="opacity:0;text-align:center;transition:opacity 0.5s;color:#fff;max-width:320px"></div>
      ...
      `,l&&(t.boostCard={value:m})}t.attacker=l?"home":"ai",t.log.push({type:"duel",title:"Milieu de Terrain",homePlayers:a.map(m=>({name:m.name,note:U(m,"MIL"),portrait:X(m),job:m.job,country_code:m.country_code,rarity:m.rarity,clubName:m.clubName,clubLogo:m.clubLogo})),aiPlayers:c.map(m=>({name:m.name,note:U(m,"MIL"),portrait:X(m),job:m.job,country_code:m.country_code,rarity:m.rarity,clubName:m.clubName,clubLogo:m.clubLogo})),homeTotal:d,aiTotal:r,text:`Duel milieu : ${t.clubName} ${d} – ${r} IA → ${l?t.clubName+" attaque":"IA attaque"}`}),setTimeout(()=>{var y,h;const m=(y=t.boostCard)==null?void 0:y.value;e.innerHTML=`
  ...
  </div>`}function be(e,t,i){if(!(e!=null&&e.length))return"";const a=e.slice(0,5);let c='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return a.forEach((o,n)=>{if(c+=ne(o,40,52),n<a.length-1){const d=ae(o,a[n+1]);c+=`<div style="width:7px;height:3px;background:${d==="#ff3333"||d==="#cc2222"?"rgba(255,255,255,0.12)":d};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(c+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),c+="</div>",c}function Be(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function _t(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function Oe(e,t,i,a,c=310,o=310){const n=xe[t]||{},d=st(t)||Ge[t]||[],r={},l=["ATT","MIL","DEF","GK"];for(const v of l)(e[v]||[]).forEach((w,I)=>{r[`${v}${I+1}`]=w});function s(v){const x=n[v];return x?{x:x.x*c,y:x.y*o}:null}let p="";for(const[v,x]of d){const w=s(v),I=s(x);if(!w||!I)continue;const L=r[v],g=r[x],b=ae(L,g);b==="#00ff88"||b==="#FFD700"?(p+=`<line x1="${w.x.toFixed(1)}" y1="${w.y.toFixed(1)}" x2="${I.x.toFixed(1)}" y2="${I.y.toFixed(1)}"
        stroke="${b}" stroke-width="10" stroke-linecap="round" opacity="0.22"/>`,p+=`<line x1="${w.x.toFixed(1)}" y1="${w.y.toFixed(1)}" x2="${I.x.toFixed(1)}" y2="${I.y.toFixed(1)}"
        stroke="${b}" stroke-width="3.5" stroke-linecap="round" opacity="0.95"/>`):p+=`<line x1="${w.x.toFixed(1)}" y1="${w.y.toFixed(1)}" x2="${I.x.toFixed(1)}" y2="${I.y.toFixed(1)}"
        stroke="${b}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const u=48,f=64,m=13,y=16,h={normal:"#aaaaaa",pépite:"#D4A017",papyte:"#111111",légende:"#7a28b8"};for(const[v,x]of Object.entries(r)){const w=s(v);if(!w||!x)continue;const I=v.replace(/[0-9]/g,""),L=ce[I]||"#555",g=i==="attack"&&["MIL","ATT"].includes(I)&&!x.used||i==="defense"&&["GK","DEF","MIL"].includes(I)&&!x.used,b=a.includes(x.cardId);let A;i==="attack"?A=I==="MIL"?Number(x.note_m)||0:Number(x.note_a)||0:i==="defense"?A=I==="GK"?Number(x.note_g)||0:I==="MIL"?Number(x.note_m)||0:Number(x.note_d)||0:A=Number(I==="GK"?x.note_g:I==="DEF"?x.note_d:I==="MIL"?x.note_m:x.note_a)||0,A=A+(x.boost||0);const T=(w.x-u/2).toFixed(1),z=(w.y-f/2).toFixed(1),S=x.used?.25:1,G=h[x==null?void 0:x.rarity]||h.normal,R=b?"#ff3030":G,F=b?3:(x==null?void 0:x.rarity)==="légende"||(x==null?void 0:x.rarity)==="pépite"?2.5:2,J=f-m-y;p+=`<defs><clipPath id="cp-${v}"><rect x="${T}" y="${(w.y-f/2+m).toFixed(1)}" width="${u}" height="${J}"/></clipPath></defs>`,p+=`<rect x="${T}" y="${z}" width="${u}" height="${f}" rx="5" fill="${L}" opacity="${S}"/>`;const W=X(x);W&&!x.used&&(p+=`<image href="${W}" x="${T}" y="${(w.y-f/2+m).toFixed(1)}" width="${u}" height="${J}" clip-path="url(#cp-${v})" preserveAspectRatio="xMidYMin slice"/>`),p+=`<rect x="${T}" y="${z}" width="${u}" height="${m}" rx="4" fill="rgba(255,255,255,0.92)"/>`,p+=`<text x="${w.x.toFixed(1)}" y="${(w.y-f/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(x.name||"").slice(0,9)}</text>`;const N=(w.y+f/2-y).toFixed(1);if(p+=`<rect x="${T}" y="${N}" width="${u}" height="${y}" fill="rgba(255,255,255,0.92)"/>`,x.used)p+=`<text x="${w.x.toFixed(1)}" y="${(w.y+f/2-y/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="13" font-weight="900" fill="rgba(0,0,0,0.4)" font-family="Arial Black">–</text>`;else{const ye=Be(x.country_code);ye?p+=`<image href="${ye}" x="${(w.x-20).toFixed(1)}" y="${(w.y+f/2-y+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:p+=`<text x="${(w.x-13).toFixed(1)}" y="${(w.y+f/2-y/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${_t(x.country_code)}</text>`,p+=`<text x="${w.x.toFixed(1)}" y="${(w.y+f/2-y/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${A}</text>`;const me=$t(x);me?p+=`<image href="${me}" x="${(w.x+u/2-14).toFixed(1)}" y="${(w.y+f/2-y+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:x.clubName&&(p+=`<text x="${(w.x+14).toFixed(1)}" y="${(w.y+f/2-y/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(x.clubName||"").slice(0,3).toUpperCase()}</text>`),x.boost&&(p+=`<rect x="${(w.x+u/2-12).toFixed(1)}" y="${(w.y-f/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,p+=`<text x="${(w.x+u/2-5).toFixed(1)}" y="${(w.y-f/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${x.boost}</text>`)}p+=`<rect x="${T}" y="${z}" width="${u}" height="${f}" rx="5" fill="${b?"rgba(255,255,255,0.12)":"none"}" stroke="${R}" stroke-width="${F}" opacity="${S}"/>`,g&&(p+=`<rect x="${T}" y="${z}" width="${u}" height="${f}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${b?"selected":""}" data-card-id="${x.cardId}" data-role="${I}" style="cursor:pointer"/>`)}const $=38;return`<svg viewBox="${-$} ${-$} ${c+$*2} ${o+$*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${p}
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;height:100%;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:100px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const b=t.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
            ...
            ${be((b.players||[]).map(A=>({...A,used:!1})),"#ff6b6b",b.total)}
            ...
            ${be((b.players||[]).map(A=>({...A,used:!1})),"#00ff88",b.total)}
    ...
    ${(()=>{const g=window.innerWidth>=700,b=(F,J)=>{var W,N;return J?`<div id="boost-card" style="width:110px;padding:8px 6px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:2px solid #87CEEB;border-radius:10px;cursor:pointer;display:flex;flex-direction:column;align-items:center;gap:4px;text-align:center">
            ...
            <div style="font-size:22px">${((N=se[F.gc_type])==null?void 0:N.icon)||"⚡"}</div>
          ...
          </div>`},A=(F,J)=>{var W,N;return J?`<div id="boost-card" style="padding:4px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:2px solid #87CEEB;border-radius:7px;cursor:pointer;display:flex;flex-direction:column;align-items:center;gap:1px;text-align:center;min-width:42px">
            ...
            <div style="font-size:15px">${((N=se[F.gc_type])==null?void 0:N.icon)||"⚡"}</div>
          ...
          </div>`},T=g?"padding:14px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:10px 8px;border-radius:12px;font-size:13px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",z=s?`<button id="btn-results" style="${T};background:linear-gradient(135deg,#D4A017,#FFD700);border:none;color:#000">🏁 Résultats</button>`:d?`<div style="${T};background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);color:rgba(255,255,255,0.4)">⏳ Tour IA</div>`:r?`<button id="btn-action" style="${T};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${t.selected.length===0?"disabled":""}> ⚔️ ATTAQUEZ <span id="match-timer" style="font-weight:900"></span></button>`:l?`<button id="btn-action" style="${T};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${t.selected.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="match-timer" style="font-weight:900"></span></button>`:`<div style="${T};background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1)"></div>`,S=r||l?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${t.selected.length}/3 sélectionné(s)</div>`:"",G=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${g?56:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
        ${o.length===0?'<div style="font-size:7px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':o.map(F=>`<div class="sub-btn-col" data-sub-id="${F.cardId}" style="cursor:pointer;flex-shrink:0">${ne(F,44,58)}</div>`).join("")}
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(g=>{var b,A,T;if(g.type==="duel"){const z=g.isGoal,S=g.homeScored?"#FFD700":z?"#ff6b6b":"rgba(255,255,255,0.3)",G=g.homeScored?"⚽ BUT !":z?"⚽ BUT IA !":(b=g.homePlayers)!=null&&b.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${z?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${S};margin-bottom:4px">
                <div style="font-size:9px;color:${S};letter-spacing:1px;margin-bottom:5px;font-weight:700;text-transform:uppercase">${G}</div>
                ${(A=g.homePlayers)!=null&&A.length?`<div style="margin-bottom:3px">${be(g.homePlayers,"rgba(255,255,255,0.7)",g.homeTotal)}</div>`:""}
                ${(T=g.aiPlayers)!=null&&T.length?`<div style="opacity:0.7">${be(g.aiPlayers,"#ff6b6b",g.aiTotal)}</div>`:""}
  ...
  </div>`;function f(){const g=e.querySelector(".match-screen"),b=e.querySelector("#match-field"),A=e.querySelector(".terrain-wrapper");if(!g||!b||!A)return;const T=window.innerWidth>=700,z=document.getElementById("app"),S=T&&z?z.clientHeight:window.innerHeight,G=g.getBoundingClientRect().top,R=S-G;R>100&&(g.style.height=R+"px",g.style.maxHeight=R+"px"),g.style.overflow="hidden",g.offsetHeight;const F=b.clientHeight,J=b.clientWidth;if(F<20||J<20)return;let W=Math.min(F,J)-6;W=Math.max(120,W),A.style.width=W+"px",A.style.height=W+"px",A.style.aspectRatio="";const N=A.querySelector("svg");N&&(N.removeAttribute("width"),N.removeAttribute("height"),N.style.width="100%",N.style.height="100%",N.style.maxWidth="none",N.style.margin="0",N.style.display="block",N.getAttribute("viewBox")||N.setAttribute("viewBox","0 0 300 300"),N.setAttribute("preserveAspectRatio","xMidYMid meet"))}if(requestAnimationFrame(f),t._resizeBound||(t._resizeBound=!0,window.addEventListener("resize",()=>requestAnimationFrame(f))),t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase==="attack"||t.phase==="defense"){let g=!1,b=30;const A=()=>document.getElementById("match-timer"),T=()=>{const z=A();if(!z)return;const S=String(Math.floor(b/60)).padStart(2,"0"),G=String(b%60).padStart(2,"0");z.textContent=` ${S}:${G}`,z.style.color=g?"#ff2222":"#ff9500",z.style.fontWeight="900"};T(),t._timerInt=setInterval(()=>{if(b--,b<0)if(!g)g=!0,b=15,T();else{clearInterval(t._timerInt),t._timerInt=null,t.homeScore=0,t.aiScore=3;const z=document.createElement("div");z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;color:#fff;padding:24px;text-align:center",z.innerHTML='<div style="font-size:56px">⏱️</div><div style="font-size:24px;font-weight:900;color:#ff4444">MATCH PERDU PAR FORFAIT</div><div style="font-size:14px;color:rgba(255,255,255,0.6)">Temps écoulé</div>',document.body.appendChild(z),setTimeout(()=>{z.remove(),re(e,t,i)},2500)}else T()},1e3)}(y=document.getElementById("match-quit"))==null||y.addEventListener("click",()=>{confirm("Abandonner ? Résultat : défaite 3-0")&&(t.homeScore=0,t.aiScore=3,re(e,t,i))}),(h=document.getElementById("view-ai"))==null||h.addEventListener("click",()=>ji(t,i)),($=document.getElementById("toggle-history"))==null||$.addEventListener("click",()=>{var g;(g=document.getElementById("match-history-panel"))==null||g.classList.add("open")}),(v=document.getElementById("close-history"))==null||v.addEventListener("click",()=>{var g;(g=document.getElementById("match-history-panel"))==null||g.classList.remove("open")}),(x=document.getElementById("btn-action"))==null||x.addEventListener("click",()=>{t.selected.length!==0&&(r?Ai(e,t,i):l&&Ii(e,t,i))}),(w=document.getElementById("btn-results"))==null||w.addEventListener("click",()=>re(e,t,i)),e.querySelectorAll(".match-slot-hit").forEach(g=>{g.addEventListener("click",()=>Ei(g,t,e,i))}),e.querySelectorAll(".gc-mini").forEach(g=>{g.addEventListener("click",()=>Mi(g.dataset.gcId,g.dataset.gcType,e,t,i))}),(I=document.getElementById("boost-card"))==null||I.addEventListener("click",()=>Ci(e,t,i)),e.querySelectorAll(".sub-btn-col").forEach(g=>{g.addEventListener("click",()=>ot(e,t,i,g.dataset.subId))}),(L=document.getElementById("sub-btn-main"))==null||L.addEventListener("click",()=>ot(e,t,i))}function Ei(e,t,i,a){const c=e.dataset.cardId,o=e.dataset.role,n=t.selected.findIndex(d=>d.cardId===c);if(n!==-1)t.selected.splice(n,1);else{if(t.selected.length>=3){a.toast("Maximum 3 joueurs","error");return}const d=(t.homeTeam[o]||[]).find(r=>r.cardId===c);d&&t.selected.push({...d,_role:o,_line:o})}V(i,t,a)}function Pe(e,t,i){e.matchId&&_.from("matches").update({last_player_id:i}).eq("id",e.matchId).then(()=>{})}function Ai(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),Pe(t,i,i.state.profile.id);const a=t.selected.map(o=>({...o,_line:o._role})),c=lt(a,t.modifiers.home);t.pendingAttack={...c,players:[...t.selected],side:"home"},t.selected.forEach(o=>{const n=(t.homeTeam[o._role]||[]).find(d=>d.cardId===o.cardId);n&&(n.used=!0)}),t.log.push({text:`⚔️ Vous attaquez : ${c.total} (base ${c.base}${c.links?` +${c.links} liens`:""}) — ${t.selected.map(o=>o.name).join(", ")}`,type:"info"}),t.selected=[],t.modifiers.home={},t.phase="ai-defense",V(e,t,i),setTimeout(()=>Ti(e,t,i),1200)}function Ii(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),Pe(t,i,i.state.profile.id);const a=t.selected.map(d=>({...d,_line:d._role})),c=ct(a,t.modifiers.home);t.selected.forEach(d=>{const r=(t.homeTeam[d._role]||[]).find(l=>l.cardId===d.cardId);r&&(r.used=!0)});const o=pt(t.pendingAttack.total,c.total,t.modifiers.home),n={type:"duel",title:"Défense",aiPlayers:(t.pendingAttack.players||[]).map(d=>({name:d.name,note:d._line==="MIL"?d.note_m:d.note_a,portrait:X(d),job:d.job,country_code:d.country_code,rarity:d.rarity,clubName:d.clubName,clubLogo:d.clubLogo})),homePlayers:t.selected.map(d=>{const r=(t.homeTeam[d._role]||[]).find(l=>l.cardId===d.cardId)||d;return{name:r.name,note:r._line==="GK"?r.note_g:r._line==="MIL"?r.note_m:r.note_d,portrait:X(r),job:r.job,country_code:r.country_code,rarity:r.rarity,clubName:r.clubName,clubLogo:r.clubLogo}}),homeTotal:c.total,aiTotal:t.pendingAttack.total,isGoal:!1,homeScored:!1,text:""};if(o.shielded)n.text="🛡️ Bouclier ! But annulé.",t.log.push(n);else if(o.goal){t.aiScore++,n.isGoal=!0,n.homeScored=!1,n.text=`⚽ BUT IA ! (${t.pendingAttack.total} > ${c.total})`,t.log.push(n),t.selected=[],t.modifiers.home={},t.pendingAttack=null,V(e,t,i),He(n.aiPlayers,t.homeScore,t.aiScore,!1,()=>{ge(e,t,i,"home-attack")});return}else n.text=`🧤 Défense réussie ! (${c.total} ≥ ${t.pendingAttack.total})`,t.log.push(n);t.selected=[],t.modifiers.home={},t.pendingAttack=null,ge(e,t,i,"home-attack")}function Se(e,t,i){Pe(t,i,null);const a=[...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]],c=ut(a,"attack",t.difficulty);if(!c.length){At(e,t,i);return}const o=lt(c,t.modifiers.ai);t.pendingAttack={...o,players:c,side:"ai"},c.forEach(l=>{l.used=!0}),t.log.push({text:`🤖 IA attaque : ${o.total} (${c.map(l=>l.name).join(", ")})`,type:"info"}),t.modifiers.ai={};const n=[...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(l=>!l.used),r=(t.homeSubs||[]).filter(l=>!(t.usedSubIds||[]).includes(l.cardId)).length>0&&t.subsUsed<t.maxSubs;if(n.length===0&&!r){t.aiScore++;const l={type:"duel",isGoal:!0,homeScored:!1,aiPlayers:c.map(s=>({name:s.name,note:s._line==="MIL"?s.note_m:s.note_a,portrait:X(s),job:s.job,country_code:s.country_code,rarity:s.rarity,clubName:s.clubName,clubLogo:s.clubLogo})),aiTotal:o.total,text:"⚽ BUT IA ! (aucun défenseur disponible)"};t.log.push(l),t.pendingAttack=null,V(e,t,i),He(l.aiPlayers,t.homeScore,t.aiScore,!1,()=>{ge(e,t,i,"home-attack")});return}t.phase="defense",V(e,t,i)}function Ti(e,t,i){const a=[...t.aiTeam.GK||[],...t.aiTeam.DEF||[],...t.aiTeam.MIL||[]],c=ut(a,"defense",t.difficulty),o=c.length>0?ct(c,t.modifiers.ai).total:0;c.forEach(r=>{r.used=!0});const n=pt(t.pendingAttack.total,o,t.modifiers.ai),d={type:"duel",title:"Attaque",homePlayers:(t.pendingAttack.players||[]).map(r=>({name:r.name,note:r._line==="MIL"?r.note_m:r.note_a,portrait:X(r),job:r.job,country_code:r.country_code,rarity:r.rarity,clubName:r.clubName,clubLogo:r.clubLogo})),aiPlayers:c.map(r=>({name:r.name,note:r._line==="GK"?r.note_g:r._line==="MIL"?r.note_m:r.note_d,portrait:X(r),job:r.job,country_code:r.country_code,rarity:r.rarity,clubName:r.clubName,clubLogo:r.clubLogo})),homeTotal:t.pendingAttack.total,aiTotal:o,isGoal:!1,homeScored:!1,text:""};if(n.shielded)d.text="🛡️ Bouclier IA !",t.log.push(d);else if(n.goal){t.homeScore++,d.isGoal=!0,d.homeScored=!0,d.text=`⚽ BUT ! (${t.pendingAttack.total} > ${o})`,t.log.push(d),t.modifiers.ai={},t.pendingAttack=null,V(e,t,i),He(d.homePlayers,t.homeScore,t.aiScore,!0,()=>{ge(e,t,i,"ai-attack")});return}else d.text=`🧤 IA défend (${o} ≥ ${t.pendingAttack.total})`,t.log.push(d);t.modifiers.ai={},t.pendingAttack=null,ge(e,t,i,"ai-attack")}function ge(e,t,i,a){if(t.round++,Et(t)){re(e,t,i);return}if(a==="home-attack"){if(![...t.homeTeam.MIL||[],...t.homeTeam.ATT||[]].filter(o=>!o.used).length){if(![...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(n=>!n.used).length){re(e,t,i);return}t.phase="ai-attack",V(e,t,i),setTimeout(()=>Se(e,t,i),800);return}t.phase="attack",V(e,t,i)}else{if(![...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]].filter(o=>!o.used).length){At(e,t,i);return}t.phase="ai-attack",V(e,t,i),setTimeout(()=>Se(e,t,i),800)}}function Et(e){const t=["MIL","ATT","GK","DEF"].some(a=>(e.homeTeam[a]||[]).some(c=>!c.used)),i=["MIL","ATT","GK","DEF"].some(a=>(e.aiTeam[a]||[]).some(c=>!c.used));return!t&&!i}function At(e,t,i){Et(t)?re(e,t,i):(t.phase="attack",V(e,t,i))}function Li(e,t,i){const a=document.createElement("div");a.style.cssText=`
    position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:800;
    display:flex;flex-direction:column;align-items:center;justify-content:center;gap:20px;
    animation:subFadeIn 0.2s ease;
  `;const c=X(e),o=X(t),n=ce[e.job]||"#555",d=ce[t.job]||"#555",r=e.job==="GK"?e.note_g:e.job==="DEF"?e.note_d:e.job==="MIL"?e.note_m:e.note_a,l=t.job==="GK"?t.note_g:t.job==="DEF"?t.note_d:t.job==="MIL"?t.note_m:t.note_a;a.innerHTML=`
    <style>
  ...
  `,document.body.appendChild(a);let s=!1;const p=()=>{s||(s=!0,a.remove(),i())};a.addEventListener("click",p),setTimeout(p,2e3)}function ue(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function ot(e,t,i,a=null){if(t.phase!=="attack"){ue("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){ue(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const c=Object.values(t.homeTeam).flat().filter(p=>p.used),o=t.homeSubs.filter(p=>!t.usedSubIds.includes(p.cardId));if(!c.length){ue("Aucun joueur utilisé à remplacer");return}if(!o.length){ue("Aucun remplaçant disponible");return}let n=0,d=Math.max(0,o.findIndex(p=>p.cardId===a)),r=!1;const l=document.createElement("div");l.id="sub-overlay",l.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function s(){var $,v,x,w,I,L;const p=c[n],u=o[d],f=Math.min(130,Math.round((window.innerWidth-90)/2)),m=Math.round(f*1.35),y=g=>`background:rgba(255,255,255,0.12);border:none;color:${g?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${g?"default":"pointer"};flex-shrink:0`;l.innerHTML=`
    ...
    </div>`,($=l.querySelector("#sub-close"))==null||$.addEventListener("click",()=>l.remove()),(v=l.querySelector("#out-up"))==null||v.addEventListener("click",()=>{n>0&&(n--,s())}),(x=l.querySelector("#out-down"))==null||x.addEventListener("click",()=>{n<c.length-1&&(n++,s())}),(w=l.querySelector("#in-up"))==null||w.addEventListener("click",()=>{d>0&&(d--,s())}),(I=l.querySelector("#in-down"))==null||I.addEventListener("click",()=>{d<o.length-1&&(d++,s())});const h=(g,b,A,T)=>{const z=l.querySelector("#"+g);if(!z)return;let S=0;z.addEventListener("touchstart",G=>{S=G.touches[0].clientY},{passive:!0}),z.addEventListener("touchend",G=>{const R=G.changedTouches[0].clientY-S;if(Math.abs(R)<30)return;const F=b();R<0&&F<T-1?(A(F+1),s()):R>0&&F>0&&(A(F-1),s())},{passive:!0})};h("in-panel",()=>d,g=>d=g,o.length),h("out-panel",()=>n,g=>n=g,c.length),(L=l.querySelector("#sub-confirm"))==null||L.addEventListener("click",g=>{if(g.preventDefault(),g.stopPropagation(),r)return;r=!0;const b=c[n],A=o[d];if(!b||!A)return;let T=null,z=-1;for(const[G,R]of Object.entries(t.homeTeam)){const F=(R||[]).findIndex(J=>J.cardId===b.cardId);if(F!==-1){T=G,z=F;break}}if(z===-1||!T){ue("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),l.remove();return}const S={...A,_line:T,_col:b._col||0,used:!1,boost:0};t.homeTeam[T].splice(z,1,S),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(A.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:b.name,firstname:b.firstname,note:U(b,T),portrait:X(b),job:b.job,country_code:b.country_code,rarity:b.rarity,clubName:b.clubName,clubLogo:b.clubLogo},inPlayer:{name:A.name,firstname:A.firstname,note:U(A,T),portrait:X(A),job:A.job,country_code:A.country_code,rarity:A.rarity,clubName:A.clubName,clubLogo:A.clubLogo},text:`🔄 ${A.firstname} ${A.name} remplace ${b.firstname} ${b.name}`}),l.remove(),Li(b,A,()=>V(e,t,i))})}document.body.appendChild(l),s()}function Mi(e,t,i,a,c){var d,r;const o=se[t]||{icon:"⚡",desc:"Carte spéciale."},n=document.createElement("div");n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;align-items:center;justify-content:center;padding:24px",n.innerHTML=`
    ...
    <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
      ${e.map(p=>{const u=p._line||p.job||"MIL",f={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[u]||"#555",m=U(p,u)+(p.boost||0),y=d.find(h=>h.cardId===p.cardId);return`<div class="gc-pick-item" data-cid="${p.cardId}"
    ...
    <div style="display:flex;flex-direction:column;gap:6px">
      ${a.map(c=>`
        <div class="player-boost-opt" data-card-id="${c.cardId}"
          ...
          <div style="width:32px;height:32px;background:${ce[c.job]||"#888"};border-radius:6px;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:900;font-size:13px">${U(c,c._line||c.job)}</div>
          <div style="flex:1"><b>${c.firstname} ${c.name}</b><div style="font-size:11px;color:#888">${c._line||c.job}</div></div>
          <div style="color:#87CEEB;font-weight:700">+${t.boostCard.value}</div>
        </div>`).join("")}
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(c=>{c.addEventListener("click",()=>{const o=c.dataset.cardId;for(const n of["GK","DEF","MIL","ATT"]){const d=(t.homeTeam[n]||[]).find(r=>r.cardId===o);if(d){d.boost=(d.boost||0)+t.boostCard.value,t.log.push({text:`⚡ Boost +${t.boostCard.value} appliqué à ${d.name}`,type:"info"});break}}t.boostUsed=!0,i.closeModal(),V(e,t,i)})})}function He(e,t,i,a,c){const o=document.createElement("div");o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:900;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;overflow:hidden;cursor:pointer";const n=Array.from({length:10},(l,s)=>`
  ...
  <div style="display:flex;gap:10px;animation:scoreIn 0.4s ease 1s both;position:relative;z-index:1">
    ${e.map(l=>`
    <div style="text-align:center">
      <div style="width:50px;height:50px;border-radius:50%;background:${ce[l.job]||"#555"};border:2px solid #FFD700;position:relative;overflow:hidden;margin:0 auto">
  ...
  `;function d(r){const l=document.getElementById("mkt-content"),s=r==="buy"?n:o;if(s.length===0){l.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      ${s.map(p=>{var h,$,v;const u=(h=p.card)==null?void 0:h.player;if(!u)return"";const f=u.job==="GK"?u.note_g:u.job==="DEF"?u.note_d:u.job==="MIL"?u.note_m:u.note_a,m=Bi[u.rarity],y=(i.profile.credits||0)>=p.price;return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px">
  ...
  </div>`}const q={user:null,profile:null,page:"home",params:{}};function $e(e,t="info",i=3e3){const a=document.getElementById("toast");a&&(a.textContent=e,a.className=`show ${t}`,clearTimeout(a._t),a._t=setTimeout(()=>{a.className=""},i))}function Ni(e,t,i=""){document.getElementById("modal-title").textContent=e,document.getElementById("modal-body").innerHTML=t,document.getElementById("modal-footer").innerHTML=i,document.getElementById("modal-overlay").classList.remove("hidden")}function Fe(){document.getElementById("modal-overlay").classList.add("hidden")}async function _e(){if(!q.user)return;const{data:e}=await _.from("users").select("*").eq("id",q.user.id).single();e&&(q.profile=e)}const Ue="mw_theme";function Ie(){return localStorage.getItem(Ue)||"light"}function Oi(e){var t;localStorage.setItem(Ue,e),De(e),(t=q.profile)!=null&&t.id&&_.from("users").update({theme:e}).eq("id",q.profile.id).then(()=>{})}function De(e){document.documentElement.setAttribute("data-theme",e)}function ke(e,t={}){q.page=e,q.params=t,Tt()}async function Tt(){const e=document.getElementById("page-content");if(!e)return;document.querySelectorAll(".bottom-nav a").forEach(a=>{a.classList.toggle("active",a.dataset.page===q.page)});const t=document.getElementById("nav-credits");t&&q.profile&&(t.textContent=`💰 ${(q.profile.credits||0).toLocaleString("fr")}`);const i={state:q,navigate:ke,toast:$e,openModal:Ni,closeModal:Fe,refreshProfile:_e};switch(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽</div>',q.page){case"home":await Xe(e,i);break;case"collection":await Dt(e,i);break;case"decks":await ze(e,i);break;case"boosters":await ti(e,i);break;case"match":await vi(e,i);break;case"market":await Si(e,i);break;case"rankings":await Ri(e,i);break;case"matches":await qi(e,i);break;default:await Xe(e,i)}}function Pi(){const e=document.getElementById("app"),t=q.profile;if(!t)return;const i="/manager-wars/icons/";e.innerHTML=`
    <nav class="top-nav">
  ...
  `,document.querySelectorAll(".bottom-nav a").forEach(a=>{a.addEventListener("click",c=>{c.preventDefault(),ke(a.dataset.page)})}),document.getElementById("nav-logo").addEventListener("click",()=>ke("home")),document.getElementById("nav-credits").addEventListener("click",()=>ke("boosters")),document.getElementById("theme-toggle").addEventListener("click",()=>{const c=Ie()==="dark"?"light":"dark";Oi(c),nt(c)}),nt(Ie())}function nt(e){const t=document.getElementById("theme-icon");t&&(t.textContent=e==="dark"?"☀️":"🌙")}async function Hi(){De(Ie()),document.getElementById("modal-overlay").addEventListener("click",t=>{t.target===t.currentTarget&&Fe()}),document.getElementById("modal-close").addEventListener("click",Fe);const{data:{session:e}}=await _.auth.getSession();if(!e){at(),We(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:$e});return}q.user=e.user,await _e(),at();try{const{data:t}=await _.from("formation_links_overrides").select("formation, links"),i={};(t||[]).forEach(a=>{i[a.formation]=a.links}),Mt(i)}catch(t){console.warn("Impossible de charger les overrides de formation:",t)}if(!q.profile){zt(document.getElementById("app"),{state:q,navigate:async()=>{await _e(),rt()},toast:$e,refreshProfile:_e});return}q.profile.theme&&q.profile.theme!==Ie()&&(localStorage.setItem(Ue,q.profile.theme),De(q.profile.theme)),rt(),_.auth.onAuthStateChange(async(t,i)=>{t==="SIGNED_OUT"&&(q.user=null,q.profile=null,document.getElementById("app").innerHTML="",We(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:$e}))})}function rt(){const e=document.getElementById("app");e.style.display="flex",e.style.flexDirection="column",Pi(),Tt()}function at(){const e=document.getElementById("app-loader"),t=document.getElementById("app");t&&(t.style.display=""),e&&(e.classList.add("zoom-out"),setTimeout(()=>e.style.display="none",500))}Hi();
