...
import{s as w,F as si,r as Ae,j as St,l as Pi,m as yt,h as di,n as gn,o as mn,T as Yt,k as xn,p as yn,b as bn}from"./formation-links-D0diUT_D.js";const hn="/";function Li(e,{navigate:t,toast:i}){let n="login";const a=()=>{var r,s,d,l,c,u;const o=n==="login";e.innerHTML=`
  ...
  `;function s(){var f;const l=document.getElementById("logo-preview"),c=document.getElementById("logo-initials"),u=((f=document.getElementById("setup-club"))==null?void 0:f.value)||"MW",g=u.trim().split(" ").filter(Boolean),x=g.length>=2?(g[0][0]+g[1][0]).toUpperCase():u.slice(0,2).toUpperCase();l&&(l.style.background=r,l.style.borderColor=o),c&&(c.textContent=x,c.style.color=o)}document.getElementById("color1").addEventListener("input",l=>{o=l.target.value,document.getElementById("swatch1").style.background=o,s()}),document.getElementById("color2").addEventListener("input",l=>{r=l.target.value,document.getElementById("swatch2").style.background=r,s()});function d(l){document.querySelectorAll(".setup-step").forEach(c=>c.classList.remove("active")),document.getElementById(`step-${l}`).classList.add("active"),document.getElementById("step-num").textContent=l,document.getElementById("progress-fill").style.width=`${Math.round(l/3*100)}%`,l===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const l=document.getElementById("setup-pseudo").value.trim(),c=document.getElementById("step1-error");if(c.textContent="",l.length<3){c.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:u}=await w.from("users").select("id").eq("pseudo",l).maybeSingle();if(u){c.textContent="Ce pseudo est déjà pris.";return}d(2)}),document.getElementById("step2-back").addEventListener("click",()=>d(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const l=document.getElementById("setup-club").value.trim(),c=document.getElementById("step2-error");if(c.textContent="",l.length<2){c.textContent="Nom trop court (min. 2 caractères).";return}const{data:u}=await w.from("users").select("id").eq("club_name",l).maybeSingle();if(u){c.textContent="Ce nom de club est déjà pris.";return}d(3)}),document.getElementById("step3-back").addEventListener("click",()=>d(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const l=document.getElementById("setup-pseudo").value.trim(),c=document.getElementById("setup-club").value.trim(),u=document.getElementById("step3-error"),g=document.getElementById("step3-finish");u.textContent="",g.disabled=!0,g.textContent="Création en cours…";try{const{error:x}=await w.from("users").insert({id:t.user.id,pseudo:l,club_name:c,club_color1:o,club_color2:r,credits:1e4});if(x)throw x;await wn(t.user.id),await a(),n(`Bienvenue ${l} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(x){u.textContent=x.message,g.disabled=!1,g.textContent="🚀 Créer mon profil !"}})}async function wn(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await w.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const _n="modulepreload",kn=function(e){return"/"+e},Ti={},Ri=function(t,i,n){let a=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const r=document.querySelector("meta[property=csp-nonce]"),s=(r==null?void 0:r.nonce)||(r==null?void 0:r.getAttribute("nonce"));a=Promise.allSettled(i.map(d=>{if(d=kn(d),d in Ti)return;Ti[d]=!0;const l=d.endsWith(".css"),c=l?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${c}`))return;const u=document.createElement("link");if(u.rel=l?"stylesheet":_n,l||(u.as="script"),u.crossOrigin="",u.href=d,s&&u.setAttribute("nonce",s),document.head.appendChild(u),l)return new Promise((g,x)=>{u.addEventListener("load",g),u.addEventListener("error",()=>x(new Error(`Unable to preload CSS for ${d}`)))})}))}function o(r){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=r,window.dispatchEvent(s),!s.defaultPrevented)throw r}return a.then(r=>{for(const s of r||[])s.status==="rejected"&&o(s.reason);return t().catch(o)})},Et="#1A6B3C",Lt="#cc2222",$n="#D4A017",zi="#888";async function En(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    </style>`}const In="2026.07.12-1736";async function Ii(e,{state:t,navigate:i,toast:n}){var o,r;const a=t.profile;a&&(e.innerHTML=`
  <style>
    .home-dark {
      min-height: 100%;
  ...
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
        ...
        </div>`},({def:B,count:O})=>{var $e,he;const Ee=window.innerWidth>=768?.76:.54,ke=(B==null?void 0:B.name)||"?",_e=(($e=B==null?void 0:B.club)==null?void 0:$e.encoded_name)||(B==null?void 0:B.country_code)||"—",me=B!=null&&B.image_url?`${R}icons/${B.image_url}`:((he=B==null?void 0:B.club)==null?void 0:he.logo_url)||(B!=null&&B.country_code?`https://flagsapi.com/${B.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),Le=me?`<img src="${me}" style="width:64px;height:64px;object-fit:contain;border-radius:4px" onerror="this.style.display='none'">`:'<span style="font-size:32px">🏟️</span>';return`<div style="display:inline-block;zoom:${Ee};line-height:0;pointer-events:none">
    ...
    <div style="display:flex;justify-content:space-between;align-items:flex-start;background:#f9f9f9;border-radius:8px;padding:8px 12px;gap:8px">
      ...
      <div style="text-align:right;flex-shrink:0">
        <div style="font-size:13px;font-weight:700;color:${U.source==="booster"?"var(--gray-600)":"var(--yellow)"}">${V}</div>
    ...
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${u.map((U,Q)=>{const V=ee[U.id]||[],T=U.is_for_sale,S=V.length?V[V.length-1]:null,H=(c.rarity==="pepite"||c.rarity==="papyte")&&U.current_note!=null?` (☆${U.current_note})`:"";return`
            <div data-card-id="${U.id}" data-card-idx="${Q}" class="exemplaire-row"
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",c.note_g],["DEF",c.note_d],["MIL",c.note_m],["ATT",c.note_a]].map(([U,Q])=>{const V=Wt[U],T=U==="GK"?"#fff":V,S=U===c.job||U===c.job2,R=(Number(Q)||0)+(S&&C>0?C:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
    ...
    ${I}
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(pe=document.getElementById("close-detail"))==null||pe.addEventListener("click",s);let k=new Set;const F=()=>{const U=k.size,Q=document.getElementById("sell-action-panel");if(!Q)return;Q.style.display=U>0?"block":"none",document.getElementById("sell-selected-count").textContent=U;const V=document.getElementById("evolve-btn");V&&(V.textContent=`⬆️ Évoluer ${U>1?"(+"+U+")":""}`)};document.querySelectorAll(".expl-check").forEach(U=>{U.addEventListener("change",()=>{const Q=U.dataset.id;U.checked?k.add(Q):k.delete(Q);const V=U.closest(".exemplaire-row");V&&(V.style.borderColor=U.checked?"#1A6B3C":"#eee"),F()})}),document.querySelectorAll(".exemplaire-row").forEach(U=>{U.addEventListener("click",Q=>{if(Q.target.closest("button")||Q.target.tagName==="INPUT")return;const V=U.querySelector(".expl-check");V&&!V.disabled&&(V.checked=!V.checked,V.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(U=>{U.addEventListener("click",Q=>{Q.stopPropagation();const V=document.querySelector(`.expl-hist[data-hist="${U.dataset.idx}"]`);V&&(V.style.display=V.style.display==="none"?"flex":"none")})}),(le=document.getElementById("evolve-btn"))==null||le.addEventListener("click",async()=>{if(g<=1)return;const U=[...k];if(!U.length)return;if(k.has(e.id)){const K=document.createElement("div");K.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",K.innerHTML=`
  ...
  `,document.body.appendChild(a);let o=!1;const r=document.getElementById("pack-cut-zone"),s=document.getElementById("pack-blade");let d=!1;const l=I=>I.touches&&I.touches[0]?I.touches[0].clientX:I.clientX;function c(I){o||(d=!0,s.style.opacity="1",u(I))}function u(I){if(!d||o)return;const k=r.getBoundingClientRect(),F=l(I)-k.left,de=Math.max(0,Math.min(1,F/k.width));s.style.width=de*k.width+"px",de>=.82&&x()}function g(){o||(d=!1,s.style.transition="width .2s ease, opacity .2s ease",s.style.width="0",s.style.opacity="0",setTimeout(()=>{o||(s.style.transition="")},220))}function x(){var k;if(o)return;o=!0,d=!1,s.style.width="100%",s.style.opacity="1",(k=document.getElementById("cut-flash"))==null||k.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const I=document.getElementById("cut-hint");I&&(I.style.opacity="0"),r.classList.add("pack-cut"),setTimeout(()=>{s.style.opacity="0",document.getElementById("pack-phase").style.display="none",b(0)},620)}r.addEventListener("pointerdown",c),window.addEventListener("pointermove",u),window.addEventListener("pointerup",g),r.addEventListener("touchstart",c,{passive:!0}),window.addEventListener("touchmove",u,{passive:!0}),window.addEventListener("touchend",g);let f=0,p=!1;const m=new Set;function b(I){f=I,document.getElementById("reveal-phase").style.display="flex",v(),h(I,0),G()}function v(){const I=document.getElementById("card-dots");I&&(I.innerHTML=e.map((k,F)=>`<div class="card-dot" data-i="${F}" style="width:8px;height:8px;border-radius:50%;background:${F===f?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),I.querySelectorAll(".card-dot").forEach(k=>k.addEventListener("click",()=>y(parseInt(k.dataset.i)))))}function h(I,k){var V,T;const F=e[I],de=document.getElementById("card-counter"),pe=document.getElementById("card-track");de&&(de.textContent=`Carte ${I+1} / ${e.length}`);const le=document.getElementById("reveal-btns");le&&(le.style.display=I===e.length-1?"flex":"none");const ae=F.card_type==="player"&&((V=F.player)==null?void 0:V.rarity)==="legende",we=!m.has(I);m.add(I);let U=0;if(F.card_type==="player"&&F.player){const S=F.player,R=S.job||"ATT";U=Number(R==="GK"?S.note_g:R==="DEF"?S.note_d:R==="MIL"?S.note_m:S.note_a)||0}const Q=S=>{pe.innerHTML=`
        ...
        </div>`;const R=document.getElementById("current-card-wrap");k!==0?(R.style.transition="none",R.style.transform=`translateX(${k>0?100:-100}%)`,requestAnimationFrame(()=>{R.style.transition="transform .28s cubic-bezier(.25,1,.5,1)",R.style.transform="translateX(0)"})):R.animate([{opacity:0,transform:"scale(.7)"},{opacity:1,transform:"scale(1)"}],{duration:300,easing:"cubic-bezier(.34,1.56,.64,1)"}),S||ae?te():ne(),v()};we&&(((T=F.player)==null?void 0:T.rarity)==="legende"||U>=8)&&F.card_type==="player"&&F.player?$(F,()=>Q(!0)):Q(!1)}function $(I,k){var _e;p=!0;const F=I.player,de=`https://flagsapi.com/${F.country_code}/flat/64.png`,pe=(_e=F.clubs)==null?void 0:_e.logo_url,le=F.face?"/"+F.face.replace(/^public\//,"").replace(/^\//,""):null,ae=F.job||"ATT",we=Number(ae==="GK"?F.note_g:ae==="DEF"?F.note_d:ae==="MIL"?F.note_m:F.note_a)||0,U=I.evolution_bonus||0,Q=we+U,V=F.rarity==="legende",T=Q>=18,S=document.getElementById("walkout-overlay"),R=document.getElementById("walkout-stage");if(!S||!R){p=!1,k();return}let H=null;V&&(H=new Audio("/sounds/Legendary.mp3"),H.volume=.8,H.play().catch(()=>{})),S.style.display="flex";const K=()=>{const me=R.firstElementChild;me&&(me.classList.remove("wo-in"),me.classList.add("wo-out"))},B=1800,O=400;R.innerHTML=`<img class="wo-in" src="${de}" style="height:130px;border-radius:10px;box-shadow:0 10px 36px rgba(0,0,0,.6)" onerror="this.style.display='none'">`,navigator.vibrate&&navigator.vibrate(30),setTimeout(K,B),setTimeout(()=>{var me;R.innerHTML=pe?`<img class="wo-in" src="${pe}" style="max-height:160px;max-width:210px;object-fit:contain">`:`<div class="wo-in" style="font-size:34px;font-weight:900;color:#fff;text-align:center">${((me=F.clubs)==null?void 0:me.encoded_name)||"CLUB"}</div>`,navigator.vibrate&&navigator.vibrate(30)},B+O),setTimeout(K,B*2+O),setTimeout(()=>{R.innerHTML=le?`<img class="wo-in" src="${le}" style="height:200px;border-radius:50%;box-shadow:0 0 40px rgba(255,255,255,0.3);object-fit:cover;object-position:top">`:'<div class="wo-in" style="font-size:80px">👤</div>',navigator.vibrate&&navigator.vibrate(30)},(B+O)*2),setTimeout(K,(B+O)*2+B);const ke=T?"#FFD700":{GK:"#c0c0c0",DEF:"#e03030",MIL:"#D4A017",ATT:"#3fbf5f"}[ae]||"#fff";setTimeout(()=>{R.innerHTML=`<div class="wo-in" style="
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    ${(()=>{const k=window.innerWidth>=700,F=(T,S,R)=>{var Ce,je;const H=(t.gcDefs||[]).find(Fe=>Fe.name===T.gc_type),K={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[H==null?void 0:H.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",B={purple:"#b06ce0",light_blue:"#00d4ef"}[H==null?void 0:H.color]||"#b06ce0",O=(H==null?void 0:H.name)||T.gc_type,Ee=(H==null?void 0:H.effect)||((Ce=Se[T.gc_type])==null?void 0:Ce.desc)||"",ke=H!=null&&H.image_url?`/icons/${H.image_url}`:null,_e=((je=Se[T.gc_type])==null?void 0:je.icon)||"⚡",me=Math.round(R*.22),Le=Math.round(R*.22),$e=R-me-Le,he=O.length>12?7:9;return`<div class="gc-mini" data-gc-id="${T.id}" data-gc-type="${T.gc_type}"
          ...
          <div style="height:${$e}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
            ${ke?`<img src="${ke}" style="max-width:${S-10}px;max-height:${$e-6}px;object-fit:contain">`:`<span style="font-size:${Math.round($e*.55)}px">${_e}</span>`}
          </div>
          ...
          </div>`},pe=(T,S)=>S?de(130,175):F(T,130,175),le=(T,S)=>S?de(68,95):F(T,68,95),ae=k?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",we=f?`<button id="btn-results" style="${ae};background:linear-gradient(135deg,#D4A017,#FFD700);border:none;color:#000">🏁 Résultats</button>`:u?`<div style="${ae};background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);color:rgba(255,255,255,0.4)">⏳ Tour IA</div>`:b?`<button id="btn-pass" style="${ae};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER (plus d'attaquants)</button>`:g?`<button id="btn-action" style="${ae};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${t.selected.length===0?"disabled":""}> ⚔️ ATTAQUEZ <span id="match-timer" style="font-weight:900"></span></button>`:x?`<button id="btn-action" style="${ae};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${t.selected.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="match-timer" style="font-weight:900"></span></button>`:`<div style="${ae};background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1)"></div>`,U=g||x?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${t.selected.length}/3 sélectionné(s)</div>`:"",Q=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${k?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(k=>{var F,de,pe;if(k.type==="duel"){const le=k.isGoal,ae=k.homeScored?"#FFD700":le?"#ff6b6b":"rgba(255,255,255,0.3)",we=k.homeScored?"⚽ BUT !":le?"⚽ BUT IA !":(F=k.homePlayers)!=null&&F.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${le?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${ae};margin-bottom:4px">
                <div style="font-size:9px;color:${ae};letter-spacing:1px;margin-bottom:5px;font-weight:700;text-transform:uppercase">${we}</div>
    ...
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      ...
      ${D}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${r?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(q),(re=q.querySelector("#pvp-end-home"))==null||re.addEventListener("click",()=>{q.remove(),Ue(e),x(r?"ranked":"home")})}const ee=w.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},_=>{const L=_.new;try{if(L.status==="finished"||L.forfeit){if(C)return;C=!0,E&&(clearInterval(E),E=null),L.game_state&&(y=L.game_state),ne(L);return}if(L.game_state){const A=y;y=L.game_state;const M=y._lastGC;if(M&&M.seq&&!te.has(M.seq)&&(te.add(M.seq),M.by!==p)){V(M.type,M.by,()=>ae());return}const z=A[p+"Score"]||0,D=A[m+"Score"]||0,q=y[p+"Score"]||0,P=y[m+"Score"]||0,Y=q>z,W=P>D;if((Y||W)&&!N.has(y.round)){N.add(y.round);const X=[...y.log||[]].reverse().find(J=>J.isGoal),re=((X==null?void 0:X.homePlayers)||[]).map(J=>({name:J.name,note:J.note,portrait:J.portrait,job:J.job}));st(re,q,P,Y,()=>ae());return}ae()}}catch(A){console.error("[PvP] crash:",A)}}).subscribe();async function j(_){Object.assign(y,_),y.lastActionAt=new Date().toISOString();const{error:L}=await w.from("matches").update({game_state:y}).eq("id",i);L&&f("Erreur de synchronisation","error");try{ae()}catch(A){console.error("[PvP] renderPvpScreen crash:",A)}}async function I(){if(C)return;C=!0,E&&(clearInterval(E),E=null);const _=n?h.away_id:h.home_id,L=n?"p2":"p1",A=n?"p1":"p2",M={...y,[L+"Score"]:3,[A+"Score"]:0,phase:"finished"};try{await w.from("matches").update({status:"finished",forfeit:!0,winner_id:_,home_score:M.p1Score||0,away_score:M.p2Score||0,game_state:M}).eq("id",i)}catch{}try{w.removeChannel(ee)}catch{}setTimeout(()=>{Ue(e),x("home")},800)}const k={BOOST_STAT:({value:_=1,count:L=1,roles:A=[]},M,z)=>{const D=M[p+"Team"],q=Object.entries(D).filter(([P])=>!A.length||A.includes(P)).flatMap(([P,Y])=>Y.filter(W=>!W.used).map(W=>({...W,_line:P})));if(!q.length){M.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),z(M);return}F(q,L,`Choisir ${L} joueur(s) à booster (+${_})`,P=>{P.forEach(Y=>{const W=(D[Y._line]||[]).find(X=>X.cardId===Y.cardId);W&&(W.boost=(W.boost||0)+_,M.log.push({text:`⚡ +${_} sur ${W.name}`,type:"info"}))}),M[p+"Team"]=D,z(M)})},DEBUFF_STAT:({value:_=1,count:L=1,roles:A=[],target:M="ai"},z,D)=>{const q=M==="home"?p:m,P=z[q+"Team"],Y=M==="home"?"allié":"adverse",W=Object.entries(P).filter(([X])=>!A.length||A.includes(X)).flatMap(([X,re])=>re.map(J=>({...J,_line:X})));if(!W.length){z.log.push({text:`🎯 Aucun joueur ${Y}`,type:"info"}),D(z);return}F(W,L,`Choisir ${L} joueur(s) ${Y}(s) (-${_})`,X=>{X.forEach(re=>{const J=(P[re._line]||[]).find(ce=>ce.cardId===re.cardId);J&&(J.boost=(J.boost||0)-_,z.log.push({text:`🎯 -${_} sur ${J.name}`,type:"info"}))}),z[q+"Team"]=P,D(z)})},GRAY_PLAYER:({count:_=1,roles:L=[],target:A="ai"},M,z)=>{const D=A==="home"?p:m,q=M[D+"Team"],P=A==="home"?"allié":"adverse",Y=Object.entries(q).filter(([W])=>!L.length||L.includes(W)).flatMap(([W,X])=>X.filter(re=>!re.used).map(re=>({...re,_line:W})));if(!Y.length){M.log.push({text:`❌ Aucun joueur ${P}`,type:"info"}),z(M);return}F(Y,_,`Choisir ${_} joueur(s) ${P}(s) à exclure`,W=>{const X="usedCardIds_"+D,re=new Set(M[X]||[]);W.forEach(J=>{const ce=(q[J._line]||[]).find(se=>se.cardId===J.cardId);ce&&(ce.used=!0,re.add(J.cardId),M.log.push({text:`❌ ${ce.name} exclu !`,type:"info"}))}),M[X]=[...re],M[D+"Team"]=q,z(M)})},REVIVE_PLAYER:({count:_=1,roles:L=[]},A,M)=>{const z=A[p+"Team"],D=Object.entries(z).filter(([q])=>!L.length||L.includes(q)).flatMap(([q,P])=>P.filter(Y=>Y.used).map(Y=>({...Y,_line:q})));if(!D.length){A.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),M(A);return}F(D,_,`Choisir ${_} joueur(s) à ressusciter`,q=>{q.forEach(P=>{const Y=(z[P._line]||[]).find(W=>W.cardId===P.cardId);Y&&(Y.used=!1,A.log.push({text:`💫 ${Y.name} ressuscité !`,type:"info"}))}),A[p+"Team"]=z,M(A)})},REMOVE_GOAL:({},_,L)=>{const A=m+"Score";_[A]>0?(_[A]--,_.log.push({text:"🚫 Dernier but annulé !",type:"info"})):_.log.push({text:"🚫 Aucun but à annuler",type:"info"}),L(_)},ADD_GOAL_DRAW:({},_,L)=>{_[p+"Score"]===_[m+"Score"]?(_[p+"Score"]++,_.log.push({text:"🎯 But bonus !",type:"info"})):_.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),L(_)},ADD_SUB:({value:_=1},L,A)=>{L.maxSubs=(L.maxSubs||3)+_,L.log.push({text:`🔄 +${_} remplacement(s)`,type:"info"}),A(L)},CUSTOM:({},_,L)=>L(_)};function F(_,L,A,M){const z=document.createElement("div");z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let D=[];function q(){var Y,W;const P=_.map(X=>{const re={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[X._line]||"#555",J=qe(X,X._line)+(X.boost||0),se=D.find(ue=>ue.cardId===X.cardId)?"#FFD700":"rgba(255,255,255,0.25)",oe=X.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${X.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${se};background:${re};overflow:hidden;cursor:pointer;${oe}">
    ...
    </div>`,p==="p1"&&setTimeout(async()=>{await j({phase:"midfield"})},5e3)}let U=!1;function Q(){if(U)return;const _=y[p+"Team"].MIL||[],L=y[m+"Team"].MIL||[],A=milScore(_)+milLinks(_),M=milScore(L)+milLinks(L),z=A>=M;function D(se,oe,ue){return`<div id="duel-row-${ue}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(J);let ce=!1;const se=()=>{ce||(ce=!0,J.remove(),setTimeout(()=>A&&A(),50))};J.addEventListener("click",se),setTimeout(se,3e3)}function T(_,L){var J,ce,se,oe;const M=(y["gcCardsFull_"+p]||[]).find(ue=>ue.id===_),z=M==null?void 0:M._gcDef,D={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[z==null?void 0:z.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",q={purple:"#b06ce0",light_blue:"#00d4ef"}[z==null?void 0:z.color]||"#b06ce0",P=(z==null?void 0:z.name)||L,Y=(z==null?void 0:z.effect)||((J=Se[L])==null?void 0:J.desc)||"Carte spéciale.",W=z!=null&&z.image_url?`/icons/${z.image_url}`:null,X=((ce=Se[L])==null?void 0:ce.icon)||"⚡",re=document.createElement("div");re.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",re.innerHTML=`
      ...
      </div>`,(ze=re.querySelector("#psub-close"))==null||ze.addEventListener("click",()=>re.remove()),(Me=re.querySelector("#pout-up"))==null||Me.addEventListener("click",()=>{P>0&&(P--,J())}),(Ne=re.querySelector("#pout-down"))==null||Ne.addEventListener("click",()=>{P<D.length-1&&(P++,J())}),(Ge=re.querySelector("#pin-up"))==null||Ge.addEventListener("click",()=>{W>0&&(W--,J())}),(Pe=re.querySelector("#pin-down"))==null||Pe.addEventListener("click",()=>{W<q.length-1&&(W++,J())});const be=(Be,Oe,Re,tt)=>{const Ke=re.querySelector("#"+Be);if(!Ke)return;let it=0;Ke.addEventListener("touchstart",De=>{it=De.touches[0].clientY},{passive:!0}),Ke.addEventListener("touchend",De=>{const Ve=De.changedTouches[0].clientY-it;if(Math.abs(Ve)<30)return;const nt=Oe();Ve<0&&nt<tt-1?(Re(nt+1),J()):Ve>0&&nt>0&&(Re(nt-1),J())},{passive:!0})};be("pin-panel",()=>W,Be=>W=Be,q.length),be("pout-panel",()=>P,Be=>P=Be,D.length),(bt=re.querySelector("#psub-confirm"))==null||bt.addEventListener("click",async Be=>{if(Be.preventDefault(),Be.stopPropagation(),X)return;X=!0;const Oe=D[P],Re=q[W];if(!Oe||!Re)return;const tt=Oe._line,Ke=(A[tt]||[]).findIndex(Ve=>Ve.cardId===Oe.cardId);if(Ke===-1){f("Erreur : joueur introuvable","error"),re.remove();return}const it={...Re,_line:tt,position:Oe.position,used:!1,boost:0};A[tt].splice(Ke,1,it);const De=[...M,Re.cardId];re.remove(),H(Oe,Re,async()=>{await j({[p+"Team"]:A,[m+"Team"]:y[m+"Team"],["usedSubIds_"+p]:De})})})}document.body.appendChild(re),J()}function H(_,L,A){const M={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},z=document.createElement("div");z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const D=(Y,W,X)=>`<div style="text-align:center">
