...
import{s as T,F as wi,r as st,j as Ot,l as Oi,m as nt,n as Ai,h as Gt,o as mo,T as si,k as xo,p as bo,b as yo}from"./formation-links-BuPyvnnm.js";const ho="/";function Li(e,{navigate:t,toast:i}){let o="login";const d=()=>{var r,c,p,s,n,h;const a=o==="login";e.innerHTML=`
    <div style="
    ...
    </style>`,(r=document.getElementById("tab-login-btn"))==null||r.addEventListener("click",()=>{o="login",d()}),(c=document.getElementById("tab-reg-btn"))==null||c.addEventListener("click",()=>{o="register",d()}),a?((p=document.getElementById("login-password"))==null||p.addEventListener("keydown",f=>{var y;f.key==="Enter"&&((y=document.getElementById("login-btn"))==null||y.click())}),(s=document.getElementById("login-btn"))==null||s.addEventListener("click",async()=>{const f=document.getElementById("login-email").value.trim(),y=document.getElementById("login-password").value,_=document.getElementById("login-error");if(_.textContent="",!f||!y){_.textContent="Remplissez tous les champs.";return}const m=document.getElementById("login-btn");m.textContent="⏳ Connexion…",m.disabled=!0;const{error:l}=await T.auth.signInWithPassword({email:f,password:y});if(m.textContent="⚽ Se connecter",m.disabled=!1,l){_.textContent=l.message.includes("Invalid")?"Email ou mot de passe incorrect.":l.message;return}window.location.reload()})):((n=document.getElementById("reg-confirm"))==null||n.addEventListener("keydown",f=>{var y;f.key==="Enter"&&((y=document.getElementById("reg-btn"))==null||y.click())}),(h=document.getElementById("reg-btn"))==null||h.addEventListener("click",async()=>{const f=document.getElementById("reg-email").value.trim(),y=document.getElementById("reg-password").value,_=document.getElementById("reg-confirm").value,m=document.getElementById("reg-error");if(m.textContent="",!f||!y||!_){m.textContent="Remplissez tous les champs.";return}if(y.length<6){m.textContent="Mot de passe trop court (min. 6 caractères).";return}if(y!==_){m.textContent="Les mots de passe ne correspondent pas.";return}const l=document.getElementById("reg-btn");l.textContent="⏳ Création…",l.disabled=!0;const{error:A}=await T.auth.signUp({email:f,password:y});if(l.textContent="🚀 Créer mon compte",l.disabled=!1,A){m.textContent=A.message;return}i("Compte créé ! Connecte-toi pour commencer.","success",4e3),o="login",d(),setTimeout(()=>{const g=document.getElementById("login-email");g&&(g.value=f)},50)}))};d()}function vo(e,{state:t,navigate:i,toast:o,refreshProfile:d}){let a="#1A6B3C",r="#D4A017";e.innerHTML=`
  ...
  `;function c(){var _;const s=document.getElementById("logo-preview"),n=document.getElementById("logo-initials"),h=((_=document.getElementById("setup-club"))==null?void 0:_.value)||"MW",f=h.trim().split(" ").filter(Boolean),y=f.length>=2?(f[0][0]+f[1][0]).toUpperCase():h.slice(0,2).toUpperCase();s&&(s.style.background=r,s.style.borderColor=a),n&&(n.textContent=y,n.style.color=a)}document.getElementById("color1").addEventListener("input",s=>{a=s.target.value,document.getElementById("swatch1").style.background=a,c()}),document.getElementById("color2").addEventListener("input",s=>{r=s.target.value,document.getElementById("swatch2").style.background=r,c()});function p(s){document.querySelectorAll(".setup-step").forEach(n=>n.classList.remove("active")),document.getElementById(`step-${s}`).classList.add("active"),document.getElementById("step-num").textContent=s,document.getElementById("progress-fill").style.width=`${Math.round(s/3*100)}%`,s===3&&c()}document.getElementById("step1-next").addEventListener("click",async()=>{const s=document.getElementById("setup-pseudo").value.trim(),n=document.getElementById("step1-error");if(n.textContent="",s.length<3){n.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:h}=await T.from("users").select("id").eq("pseudo",s).maybeSingle();if(h){n.textContent="Ce pseudo est déjà pris.";return}p(2)}),document.getElementById("step2-back").addEventListener("click",()=>p(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const s=document.getElementById("setup-club").value.trim(),n=document.getElementById("step2-error");if(n.textContent="",s.length<2){n.textContent="Nom trop court (min. 2 caractères).";return}const{data:h}=await T.from("users").select("id").eq("club_name",s).maybeSingle();if(h){n.textContent="Ce nom de club est déjà pris.";return}p(3)}),document.getElementById("step3-back").addEventListener("click",()=>p(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const s=document.getElementById("setup-pseudo").value.trim(),n=document.getElementById("setup-club").value.trim(),h=document.getElementById("step3-error"),f=document.getElementById("step3-finish");h.textContent="",f.disabled=!0,f.textContent="Création en cours…";try{const{error:y}=await T.from("users").insert({id:t.user.id,pseudo:s,club_name:n,club_color1:a,club_color2:r,credits:1e4});if(y)throw y;await wo(t.user.id),await d(),o(`Bienvenue ${s} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(y){h.textContent=y.message,f.disabled=!1,f.textContent="🚀 Créer mon profil !"}})}async function wo(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await T.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const _o="modulepreload",$o=function(e){return"/"+e},Ii={},ii=function(t,i,o){let d=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const r=document.querySelector("meta[property=csp-nonce]"),c=(r==null?void 0:r.nonce)||(r==null?void 0:r.getAttribute("nonce"));d=Promise.allSettled(i.map(p=>{if(p=$o(p),p in Ii)return;Ii[p]=!0;const s=p.endsWith(".css"),n=s?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${p}"]${n}`))return;const h=document.createElement("link");if(h.rel=s?"stylesheet":_o,s||(h.as="script"),h.crossOrigin="",h.href=p,c&&h.setAttribute("nonce",c),document.head.appendChild(h),s)return new Promise((f,y)=>{h.addEventListener("load",f),h.addEventListener("error",()=>y(new Error(`Unable to preload CSS for ${p}`)))})}))}function a(r){const c=new Event("vite:preloadError",{cancelable:!0});if(c.payload=r,window.dispatchEvent(c),!c.defaultPrevented)throw r}return d.then(r=>{for(const c of r||[])c.status==="rejected"&&a(c.reason);return t().catch(a)})},Pt="#1A6B3C",Nt="#cc2222",ko="#D4A017",Mi="#888";async function Eo(e,t){const{state:i,toast:o}=t;e.innerHTML=`
    ...
    <div class="popup-box">
      ...
      ${n.length?`<div style="display:flex;flex-direction:column;gap:8px;max-height:50vh;overflow-y:auto;margin-bottom:14px">
            ${n.map(f=>{var y,_,m;return`
              <div style="display:flex;align-items:center;gap:10px;background:#f9f9f9;border-radius:10px;padding:10px 12px">
                <div style="flex:1;font-size:14px;font-weight:700">${((y=f.requester)==null?void 0:y.club_name)||((_=f.requester)==null?void 0:_.pseudo)||"?"}
    ...
    ${$i()}`,document.body.appendChild(s);const h=()=>s.remove();s.querySelector("#pending-close").addEventListener("click",h),s.addEventListener("click",f=>{f.target===s&&h()}),s.querySelectorAll("[data-accept]").forEach(f=>{f.addEventListener("click",async()=>{const{error:y}=await T.from("friendships").update({status:"accepted"}).eq("id",f.dataset.accept);if(y){t("Erreur : "+y.message,"error");return}f.closest("div[style]").remove(),t("✅ Ami accepté !","success"),Hi(e,t,o),i&&i()})}),s.querySelectorAll("[data-decline]").forEach(f=>{f.addEventListener("click",async()=>{await T.from("friendships").delete().eq("id",f.dataset.decline),f.closest("div[style]").remove(),t("Demande refusée","info"),i&&i()})})}async function zo(e,t,i){const o=e.user.id,[d,a]=[o,t].sort(),r=o===d,{data:c}=await T.from("friend_match_stats").select("*").eq("player1_id",d).eq("player2_id",a).single(),p=e.profile.club_name||e.profile.pseudo||"Moi",s=c||{},n=r?s.wins_p1||0:s.wins_p2||0,h=r?s.wins_p2||0:s.wins_p1||0,f=s.draws||0,y=r?s.goals_p1||0:s.goals_p2||0,_=r?s.goals_p2||0:s.goals_p1||0,m=r?s.gc_used_p1||0:s.gc_used_p2||0,l=r?s.gc_used_p2||0:s.gc_used_p1||0,A=s.matches_total||0,g=(C,j,D,q=Pt,R=Nt)=>`
    ...
    </style>`}async function Ki({player1Id:e,player2Id:t,score1:i,score2:o,gc1:d,gc2:a}){const[r,c]=[e,t].sort(),p=e!==r,s=p?o:i,n=p?i:o,h=p?a:d,f=p?d:a,y=s>n?1:0,_=n>s?1:0,m=s===n?1:0,{data:l}=await T.from("friend_match_stats").select("*").eq("player1_id",r).eq("player2_id",c).single();l?await T.from("friend_match_stats").update({wins_p1:l.wins_p1+y,wins_p2:l.wins_p2+_,draws:l.draws+m,goals_p1:l.goals_p1+s,goals_p2:l.goals_p2+n,gc_used_p1:l.gc_used_p1+h,gc_used_p2:l.gc_used_p2+f,matches_total:l.matches_total+1}).eq("player1_id",r).eq("player2_id",c):await T.from("friend_match_stats").insert({player1_id:r,player2_id:c,wins_p1:y,wins_p2:_,draws:m,goals_p1:s,goals_p2:n,gc_used_p1:h,gc_used_p2:f,matches_total:1})}const Ao="2026.07.08-1735";async function ji(e,{state:t,navigate:i,toast:o}){var a,r;const d=t.profile;d&&(e.innerHTML=`
  <style>
    .home-dark {
      min-height: 100%;
  ...
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      ...
      <div style="background:#fff3cd;border-radius:10px;padding:10px 14px;width:100%">
        ...
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const R of Object.keys(A)){const K=D(R);if(!K)continue;const J=R.replace(/\d+/,""),re=n[J]||"#555";q+=`<circle cx="${K.x}" cy="${K.y}" r="${j}" fill="${re}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,q+=`<text x="${K.x}" y="${K.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${J}</text>`}return q+="</svg>",q}const f=t.filter(A=>A.formation===s);f.length;const y=!e.is_for_sale;o(`Formation ${s}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      ...
      <!-- Carte visuelle -->
      ${st({...n,_evolution_bonus:y},{width:160})}
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(_e=document.getElementById("close-detail"))==null||_e.addEventListener("click",c);let G=new Set;const oe=()=>{const ce=G.size,me=document.getElementById("sell-action-panel");if(!me)return;me.style.display=ce>0?"block":"none",document.getElementById("sell-selected-count").textContent=ce;const pe=document.getElementById("evolve-btn");pe&&(pe.textContent=`⬆️ Évoluer ${ce>1?"(+"+ce+")":""}`)};document.querySelectorAll(".expl-check").forEach(ce=>{ce.addEventListener("change",()=>{const me=ce.dataset.id;ce.checked?G.add(me):G.delete(me);const pe=ce.closest(".exemplaire-row");pe&&(pe.style.borderColor=ce.checked?"#1A6B3C":"#eee"),oe()})}),document.querySelectorAll(".exemplaire-row").forEach(ce=>{ce.addEventListener("click",me=>{if(me.target.closest("button")||me.target.tagName==="INPUT")return;const pe=ce.querySelector(".expl-check");pe&&!pe.disabled&&(pe.checked=!pe.checked,pe.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(ce=>{ce.addEventListener("click",me=>{me.stopPropagation();const pe=document.querySelector(`.expl-hist[data-hist="${ce.dataset.idx}"]`);pe&&(pe.style.display=pe.style.display==="none"?"flex":"none")})}),(ve=document.getElementById("evolve-btn"))==null||ve.addEventListener("click",async()=>{if(f<=1)return;const ce=[...G];if(!ce.length)return;if(G.has(e.id)){const fe=document.createElement("div");fe.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",fe.innerHTML=`
  ...
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
    ...
    <div class="page-body">
      ...
      stadium_def:stadium_definitions(id, name, club_id, country_code, image_url,
        club:clubs(encoded_name, logo_url))`).eq("owner_id",o.profile.id).eq("card_type","stadium"),p=(r||[]).filter(A=>A.card_type==="player"&&A.player),s=(r||[]).filter(A=>A.card_type==="formation"),n=(c||[]).filter(A=>A.card_type==="stadium"),h=[...new Set(n.map(A=>A.stadium_id).filter(Boolean))];let f={};if(n.forEach(A=>{A.stadium_def&&A.stadium_id&&(f[A.stadium_id]=A.stadium_def)}),h.length&&Object.keys(f).length<h.length){const{data:A}=await T.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",h);(A||[]).forEach(g=>{f[g.id]=g})}const y=s.map(A=>A.formation).filter(Boolean),{data:_}=await T.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",e);let m=a.formation||"4-4-2";y.length>0&&!y.includes(m)&&(m=y[0]);const l={deckId:e,name:a.name,formation:m,formationCardId:a.formation_card_id,stadiumCardId:a.stadium_card_id||null,slots:{},subs:[],playerCards:p,formationCards:s,stadiumCards:n,stadDefMap:f,availableFormations:y};(_||[]).forEach(A=>{A.is_starter?l.slots[A.position]=A.card_id:l.subs.includes(A.card_id)||l.subs.push(A.card_id)}),ht(t,l,i)}function ht(e,t,i){var _,m;const{navigate:o}=i;Qt[t.formation];const d=Gi(t.formation),a=d.filter(l=>t.slots[l]).length,r=t.availableFormations.length>0?t.availableFormations:Object.keys(Qt),c=(_=t.stadiumCards)==null?void 0:_.find(l=>l.id===t.stadiumCardId),p=c&&((m=t.stadDefMap)==null?void 0:m[c.stadium_id])||null,s=t.subs.map(l=>t.playerCards.find(A=>A.id===l)).filter(Boolean);[...Object.values(t.slots),...t.subs],e.innerHTML=`
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
    ...
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(A=document.getElementById("close-selector"))==null||A.addEventListener("click",a),(g=document.getElementById("remove-player"))==null||g.addEventListener("click",()=>{delete t.slots[e],a(),ht(i,t,o)}),document.querySelectorAll(".player-option").forEach(I=>{I.addEventListener("click",()=>{t.slots[e]=I.dataset.cardId,a(),ht(i,t,o)})})}function Wo(e,t,i){var n,h,f;const{openModal:o,closeModal:d}=i,a=(n=e.stadiumCards)==null?void 0:n.find(y=>y.id===e.stadiumCardId),r=a&&((h=e.stadDefMap)==null?void 0:h[a.stadium_id])||null,c=new Set;Object.keys(e.slots).forEach(y=>{var l;const _=e.slots[y];if(!_)return;const m=e.playerCards.find(A=>A.id===_);(l=m==null?void 0:m.player)!=null&&l.id&&c.add(m.player.id)}),e.subs.forEach(y=>{var m;const _=e.playerCards.find(l=>l.id===y);(m=_==null?void 0:_.player)!=null&&m.id&&c.add(_.player.id)});const p=new Set,s=e.playerCards.filter(y=>{var _,m,l;return c.has((_=y.player)==null?void 0:_.id)||p.has((m=y.player)==null?void 0:m.id)?!1:(p.add((l=y.player)==null?void 0:l.id),!0)});o("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    </div>`}const p=n=>{e.style.overflow="",e.style.height="",e.style.display="",e.style.flexDirection="",i(n)};function s(){var h,f,y;e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column";const n=r.length>0;e.innerHTML=`
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      card:cards(id,card_type,formation,stadium_id,evolution_bonus,
        player:players(id,firstname,surname_real,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
          clubs(encoded_name,logo_url)))`).in("deck_id",r).order("slot_order"),p=[...new Set((c||[]).filter(f=>{var y,_;return((y=f.card)==null?void 0:y.card_type)==="stadium"&&((_=f.card)==null?void 0:_.stadium_id)}).map(f=>f.card.stadium_id))],s={};if(p.length){const{data:f}=await T.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",p);(f||[]).forEach(y=>{s[y.id]=y}),(c||[]).forEach(y=>{var _,m;((_=y.card)==null?void 0:_.card_type)==="stadium"&&((m=y.card)!=null&&m.stadium_id)&&(y.card._stadiumDef=s[y.card.stadium_id]||null)})}let n=0;function h(){var j,D,q,R,K,J,re;const f=a[n],y=(c||[]).filter(ne=>ne.deck_id===f.id),_=y.filter(ne=>{var Q;return ne.is_starter&&((Q=ne.card)==null?void 0:Q.player)}).map(ne=>bi(ne.card,ne.position)),m=y.find(ne=>{var Q;return((Q=ne.card)==null?void 0:Q.card_type)==="formation"}),l=f.formation||((j=m==null?void 0:m.card)==null?void 0:j.formation)||"4-4-2";let A=_.length>=11?vt(_,l):null,g=((D=f.stadium_card)==null?void 0:D.stadium_def)||null;g&&A&&(A=Tt(A,g));const I=_.length>=11;et(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div style="padding:10px 14px 16px;flex-shrink:0;display:flex;flex-direction:column;gap:8px;background:rgba(0,0,0,0.2)">
        ...
        stroke="${J}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const y=48,_=64,m=13,l=16,A={normal:"#aaaaaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#111111",legende:"#7a28b8",légende:"#7a28b8"};for(const[C,j]of Object.entries(s)){const D=h(C);if(!D||!j)continue;const q=C.replace(/[0-9]/g,""),R=Rt[q]||"#555",K=r.includes(j.cardId),J=i==="attack"&&(["MIL","ATT"].includes(q)||K)&&!j.used||i==="defense"&&["GK","DEF","MIL"].includes(q)&&!j.used,re=o.includes(j.cardId);let ne;i==="attack"?ne=K?Math.max(1,Number(j.note_a)||0):q==="MIL"?Number(j.note_m)||0:Number(j.note_a)||0:i==="defense"?ne=q==="GK"?Number(j.note_g)||0:q==="MIL"?Number(j.note_m)||0:Number(j.note_d)||0:ne=Number(q==="GK"?j.note_g:q==="DEF"?j.note_d:q==="MIL"?j.note_m:j.note_a)||0,ne=ne+(j.boost||0),j.stadiumBonus&&(i==="attack"&&(q==="ATT"||q==="MIL")||i==="defense"&&(q==="GK"||q==="DEF"||q==="MIL")?ne+=10:i||(ne+=10));const Q=(D.x-y/2).toFixed(1),N=(D.y-_/2).toFixed(1),G=A[j==null?void 0:j.rarity]||A.normal;if(j.used){const ce=typeof import.meta<"u"&&"/"||"/",pe=`${typeof window<"u"&&((I=window.location)==null?void 0:I.origin)||""}${ce}icons/carte-dos.png`.replace(/([^:])\/\//g,"$1/");f+=`<rect x="${Q}" y="${N}" width="${y}" height="${_}" rx="5" fill="#161616"/>`,f+=`<image href="${pe}" xlink:href="${pe}" x="${Q}" y="${N}" width="${y}" height="${_}" preserveAspectRatio="xMidYMid slice"/>`,f+=`<rect x="${Q}" y="${N}" width="${y}" height="${_}" rx="5" fill="none" stroke="${G}" stroke-width="2" opacity="0.7"/>`,f+=`<rect x="${Q}" y="${N}" width="${y}" height="${_}" rx="5" fill="rgba(0,0,0,0.01)" class="match-used-hit" data-card-id="${j.cardId}" data-role="${q}" style="cursor:pointer"/>`;continue}const oe=re?.45:1,xe=re?"#FFD700":G,_e=re?3:(j==null?void 0:j.rarity)==="legende"||(j==null?void 0:j.rarity)==="légende"||(j==null?void 0:j.rarity)==="pepite"||(j==null?void 0:j.rarity)==="pépite"?2.5:2,ve=_-m-l;f+=`<defs><clipPath id="cp-${C}"><rect x="${Q}" y="${(D.y-_/2+m).toFixed(1)}" width="${y}" height="${ve}"/></clipPath></defs>`,f+=`<rect x="${Q}" y="${N}" width="${y}" height="${_}" rx="5" fill="${R}" opacity="${oe}"/>`;const we=He(j);we&&(f+=`<image href="${we}" xlink:href="${we}" x="${Q}" y="${(D.y-_/2+m).toFixed(1)}" width="${y}" height="${ve}" clip-path="url(#cp-${C})" preserveAspectRatio="xMidYMin slice"/>`),f+=`<rect x="${Q}" y="${N}" width="${y}" height="${m}" rx="4" fill="rgba(255,255,255,0.92)"/>`,f+=`<text x="${D.x.toFixed(1)}" y="${(D.y-_/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(j.name||"").slice(0,9)}</text>`;const Me=(D.y+_/2-l).toFixed(1);f+=`<rect x="${Q}" y="${Me}" width="${y}" height="${l}" fill="rgba(255,255,255,0.92)"/>`;{const ce=mn(j.country_code);ce?f+=`<image href="${ce}" xlink:href="${ce}" x="${(D.x-20).toFixed(1)}" y="${(D.y+_/2-l+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:f+=`<text x="${(D.x-13).toFixed(1)}" y="${(D.y+_/2-l/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${xn(j.country_code)}</text>`;const me=j.stadiumBonus?"#E87722":"#111";f+=`<text x="${D.x.toFixed(1)}" y="${(D.y+_/2-l/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="${me}" font-family="Arial Black">${ne}</text>`;const pe=gn(j);pe?f+=`<image href="${pe}" xlink:href="${pe}" x="${(D.x+y/2-14).toFixed(1)}" y="${(D.y+_/2-l+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:j.clubName&&(f+=`<text x="${(D.x+14).toFixed(1)}" y="${(D.y+_/2-l/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(j.clubName||"").slice(0,3).toUpperCase()}</text>`),j.stadiumBonus&&(f+=`<rect x="${(D.x-y/2).toFixed(1)}" y="${(D.y-_/2).toFixed(1)}" width="${y}" height="${_}" rx="5" fill="none" stroke="#E87722" stroke-width="2" opacity="0.8"/>`)}f+=`<rect x="${Q}" y="${N}" width="${y}" height="${_}" rx="5" fill="${re?"rgba(255,255,255,0.12)":"none"}" stroke="${xe}" stroke-width="${_e}" opacity="${oe}"/>`,J&&(f+=`<rect x="${Q}" y="${N}" width="${y}" height="${_}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${re?"selected":""}" data-card-id="${j.cardId}" data-role="${q}" style="cursor:pointer"/>`)}const g=38;return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-g} ${-g} ${d+g*2} ${a+g*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${f}
  ...
  <div style="text-align:center;flex-shrink:0;width:38px">
    ...
    <div style="font-size:7px;color:rgba(255,255,255,0.5);margin-top:1px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${(e.name||"").slice(0,7)}</div>
          ...
          card:cards(id, card_type, formation, evolution_bonus,
            player:players(id,firstname,surname_real,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
              clubs(encoded_name,logo_url)))`).eq("deck_id",p).order("slot_order")]);s=q[0].data,h=q[0].error,n=q[1].data,f=q[1].error}catch(q){console.error("[Match] Exception chargement deck:",q),Et(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>a("home"));return}if(h||f){console.error("[Match] Erreur Supabase:",h||f),Et(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>a("home"));return}const y=(n||[]).filter(q=>{var R;return q.is_starter&&((R=q.card)==null?void 0:R.player)}).map(q=>bi(q.card,q.position)),_=(n||[]).filter(q=>{var R;return!q.is_starter&&((R=q.card)==null?void 0:R.player)}).map(q=>bi(q.card,q.position));if(y.length<11){Et(e,"⚠️",`Deck incomplet (${y.length}/11).`,"Compléter",()=>a("decks"));return}const m=(n||[]).find(q=>{var R;return((R=q.card)==null?void 0:R.card_type)==="formation"}),l=(s==null?void 0:s.formation)||((D=m==null?void 0:m.card)==null?void 0:D.formation)||"4-4-2",{data:A,error:g}=await T.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",d.profile.id).eq("card_type","game_changer"),{data:I}=await T.from("gc_definitions").select("*").eq("is_active",!0),C=(A||[]).map(q=>({...q,_gcDef:(I==null?void 0:I.find(R=>R.name===q.gc_type||R.id===q.gc_definition_id))||null}));let j=null;if(s!=null&&s.stadium_card_id){const{data:q}=await T.from("cards").select("stadium_id").eq("id",s.stadium_card_id).single();if(q!=null&&q.stadium_id){const{data:R}=await T.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").eq("id",q.stadium_id).single();j=R||null}}o({deckId:p,formation:l,starters:y,subsRaw:_,gcCardsEnriched:C,gcDefs:I||[],stadiumDef:j})}async function bn(e,t){const{state:i}=t,d=(i.params||{}).matchMode||"vs_ai_easy",a=d.replace("vs_ai_",""),r=d;await ri(e,t,d,async({deckId:c,formation:p,starters:s,subsRaw:n,gcCardsEnriched:h,gcDefs:f,stadiumDef:y})=>{try{let _=vt(s,p);y&&(_=Tt(_,y),jt(n,y));const m=await yn(p,a),l=m.lines||m,A=async g=>{try{const{data:I,error:C}=await T.from("matches").insert({home_id:i.profile.id,away_id:null,mode:r,home_deck_id:c,status:"in_progress"}).select().single();if(C){console.error("[MatchIA] Erreur création match:",C),Et(e,"⚠️","Impossible de créer le match ("+C.message+").","Retour",()=>t.navigate("home"));return}const j=m.stadiumDef||null;j&&l&&(Tt(l,j),jt(m.subs||[],j));const D={gcDefs:f||[],matchId:I==null?void 0:I.id,mode:r,difficulty:a,formation:p,homeTeam:_,aiTeam:l,homeSubs:n,subsUsed:0,maxSubs:Math.min(n.length,3),aiSubs:m.subs||[],aiSubsUsed:0,aiMaxSubs:Math.min((m.subs||[]).length,3),aiUsedSubIds:[],aiGcCards:m.gcCards||[],aiUsedGc:[],aiStadiumDef:j,homeScore:0,aiScore:0,gcCards:g,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};vn(e,D,t)}catch(I){console.error("[MatchIA] Exception launchMatch:",I),Et(e,"⚠️","Erreur au lancement du match : "+I.message,"Retour",()=>t.navigate("home"))}};if(!h.length){A([]);return}ni(e,h,A)}catch(_){console.error("[MatchIA] Exception setup:",_),Et(e,"⚠️","Erreur de préparation du match : "+_.message,"Retour",()=>t.navigate("home"))}})}async function yn(e,t){var l;const{data:i}=await T.from("players").select("id,firstname,surname_real,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(80);if(!i||i.length<11)return{lines:hn(e),subs:[],gcCards:[],stadiumDef:null};const o=Mt[e]||Mt["4-4-2"],d={GK:[],DEF:[],MIL:[],ATT:[]},a=new Set;function r(A,g,I){var C,j;return a.add(A.id),{cardId:"ai-"+A.id+"-"+I,id:A.id,firstname:A.firstname,name:A.surname_real,country_code:A.country_code,club_id:A.club_id,job:A.job,job2:A.job2,note_g:Number(A.note_g)||0,note_d:Number(A.note_d)||0,note_m:Number(A.note_m)||0,note_a:Number(A.note_a)||0,rarity:A.rarity,skin:A.skin,hair:A.hair,hair_length:A.hair_length,clubName:((C=A.clubs)==null?void 0:C.encoded_name)||null,clubLogo:((j=A.clubs)==null?void 0:j.logo_url)||null,boost:0,used:!1,_line:g}}for(const A of["GK","DEF","MIL","ATT"]){const g=i.filter(q=>q.job===A&&!a.has(q.id)),I=i.filter(q=>q.job!==A&&!a.has(q.id)),C=[...g,...I],j=[];for(let q=0;q<o[A];q++){const R=C[q];R&&j.push(r(R,A,q))}const D=dt(j.length);j.forEach((q,R)=>{q._col=D[R]}),d[A]=j}const p=i.filter(A=>!a.has(A.id)).slice(0,5).map((A,g)=>r(A,A.job,100+g)),h=Object.keys(Oe).sort(()=>Math.random()-.5).slice(0,3).map((A,g)=>{var I,C;return{id:"ai-gc-"+g,gc_type:A,name:((I=Oe[A])==null?void 0:I.name)||A,icon:((C=Oe[A])==null?void 0:C.icon)||"⚡"}}),f=Object.values(d).flat(),y={};f.forEach(A=>{A.club_id&&(y[A.club_id]=(y[A.club_id]||0)+1)});const _=(l=Object.entries(y).sort((A,g)=>g[1]-A[1])[0])==null?void 0:l[0];let m=null;if(_){const{data:A}=await T.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",_).single();A&&(m={club_id:A.id,country_code:null,name:A.encoded_name+" Stadium",club:{encoded_name:A.encoded_name,logo_url:A.logo_url}})}return{lines:d,subs:p,gcCards:h,stadiumDef:m}}function hn(e){const t=Mt[e]||Mt["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},o=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let d=0;for(const a of["GK","DEF","MIL","ATT"]){const r=[];for(let p=0;p<t[a];p++){const s=3+Math.floor(Math.random()*5);r.push({cardId:"fake-"+d,id:"fake-"+d,firstname:"IA",name:o[d%o.length],country_code:"XX",club_id:null,job:a,job2:null,note_g:a==="GK"?s:2,note_d:a==="DEF"?s:2,note_m:a==="MIL"?s:2,note_a:a==="ATT"?s:2,rarity:"normal",boost:0,used:!1,_line:a}),d++}const c=dt(r.length);r.forEach((p,s)=>{p._col=c[s]}),i[a]=r}return i}function vn(e,t,i){var o;e.innerHTML=`
  ...
  `,document.body.appendChild(o);let n=!1;const h=()=>{n||(n=!0,o.remove(),i())};o.addEventListener("click",h),setTimeout(h,2e3)}function qt(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function pi(e,t,i,o=null,d=null){var y,_;if(t.phase!=="attack"){qt("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){qt(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const a=Object.entries(t.homeTeam).flatMap(([m,l])=>(l||[]).filter(A=>A.used).map(A=>({...A,_line:A._line||m}))),r=t.homeSubs.filter(m=>!t.usedSubIds.includes(m.cardId));if(!a.length){qt("Aucun joueur utilisé à remplacer");return}if(!r.length){qt("Aucun remplaçant disponible");return}let c=Math.max(0,a.findIndex(m=>m.cardId===d));const p=((y=a[c])==null?void 0:y._line)||((_=a[c])==null?void 0:_.job);let s=o?Math.max(0,r.findIndex(m=>m.cardId===o)):Math.max(0,r.findIndex(m=>m.job===p)),n=!1;const h=document.createElement("div");h.id="sub-overlay",h.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function f(){var j,D,q,R,K,J;const m=a[c],l=r[s],A=Math.min(130,Math.round((window.innerWidth-90)/2)),g=Math.round(A*1.35),I=re=>`background:rgba(255,255,255,0.12);border:none;color:${re?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${re?"default":"pointer"};flex-shrink:0`;h.innerHTML=`
    ...
    <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
      ${e.map(h=>{const f=h._line||h.job||"MIL",y={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[f]||"#555",_=ze(h,f)+(h.boost||0),m=c.find(l=>l.cardId===h.cardId);return`<div class="gc-pick-item" data-cid="${h.cardId}"
          style="width:80px;border-radius:8px;border:2.5px solid ${m?"#FFD700":"rgba(255,255,255,0.25)"};background:${y};overflow:hidden;cursor:pointer;flex-shrink:0;${h.used?"opacity:0.3;pointer-events:none":""}">
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(a);let c=!1;const p=()=>{c||(c=!0,a.remove(),setTimeout(()=>d(),50))};a.addEventListener("click",p),setTimeout(p,3500)}async function Ft(e,t,i){var n,h;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:o}=i,d=t.homeScore>t.aiScore,a=t.homeScore===t.aiScore,r=d?"victoire":a?"nul":"defaite",c=Co(t.mode,r);t.matchId&&await T.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:d?o.profile.id:null,home_credits_reward:c,played_at:new Date().toISOString()}).eq("id",t.matchId);const p={credits:(o.profile.credits||0)+c,matches_played:(o.profile.matches_played||0)+1};d?p.wins=(o.profile.wins||0)+1:a?p.draws=(o.profile.draws||0)+1:p.losses=(o.profile.losses||0)+1,await T.from("users").update(p).eq("id",o.profile.id),await i.refreshProfile();const s=document.createElement("div");s.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",s.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const C=async(R=!0)=>{_=!0,m&&(T.removeChannel(m),m=null),R&&await T.rpc("cancel_matchmaking",{p_user_id:h.profile.id}).catch(()=>{})};(q=document.getElementById("mm-cancel"))==null||q.addEventListener("click",async()=>{try{await C(!0)}catch{}Ve(e),f("home")});const j=async(R,K,J)=>{if(_)return;_=!0,m&&(T.removeChannel(m),m=null);const re=document.getElementById("mm-status");re&&(re.textContent="Adversaire trouvé !"),await new Promise(ne=>setTimeout(ne,600)),e.isConnected&&lo(e,t,R,J,r,c,s,n,p)};if(s){const R=[{range:50,delay:0},{range:100,delay:15e3},{range:200,delay:3e4},{range:400,delay:45e3},{range:800,delay:6e4}];let K=0,J=!1;const re=async()=>{if(_||J)return;const{range:ne}=R[K],Q=document.getElementById("mm-range");Q&&(Q.textContent=`Plage MMR : ±${ne}`);const{data:N,error:G}=await T.rpc("try_matchmake_ranked",{p_user_id:h.profile.id,p_deck_id:i,p_mmr:l,p_range:ne});if(console.log("[Ranked] try_matchmake_ranked result:",N,"error:",JSON.stringify(G)),_)return;if(G||!(N!=null&&N.success)){y("Erreur matchmaking ranked","error"),Ve(e),f("home");return}if(N.matched){J=!0,j(N.match_id,N.opponent_id,!1);return}const oe=document.getElementById("mm-status");if(oe&&(oe.textContent="En attente d'un adversaire..."),m||(m=T.channel("matchmaking-"+h.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${h.profile.id}`},xe=>{const _e=xe.new;_e.status==="matched"&&_e.match_id&&!J&&(J=!0,j(_e.match_id,_e.matched_with,!0))}).subscribe()),K<R.length-1){const xe=R[K+1].delay-R[K].delay;K++,setTimeout(()=>{!_&&!J&&re()},xe)}};await re()}else{const{data:R,error:K}=await T.rpc("try_matchmake",{p_user_id:h.profile.id,p_deck_id:i});if(K||!(R!=null&&R.success)){y("Erreur de matchmaking","error"),Ve(e),f("home");return}if(R.matched){j(R.match_id,R.opponent_id,!1);return}const J=document.getElementById("mm-status");J&&(J.textContent="En attente d'un autre joueur..."),m=T.channel("matchmaking-"+h.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${h.profile.id}`},re=>{const ne=re.new;ne.status==="matched"&&ne.match_id&&j(ne.match_id,ne.matched_with,!0)}).subscribe()}}async function Bn(e,t,i){const{state:o,navigate:d,toast:a}=t;try{(T.getChannels?T.getChannels():[]).forEach(s=>{const n=s.topic||"";(n.includes("matchmaking")||n.includes("pvp-match")||n.includes("friend-invite"))&&T.removeChannel(s)})}catch{}const{data:r}=await T.from("matches").select("home_id, away_id, status").eq("id",i).single();if(!r){a("Match introuvable","error"),d("home");return}if(r.status==="finished"){a("Ce match est terminé","info"),d("home");return}const c=r.home_id===o.profile.id;lo(e,t,i,c,[],[])}async function lo(e,t,i,o,d=[],a=[],r=!1,c=null,p=null){const{state:s,navigate:n,toast:h}=t,f=o?"p1":"p2",y=o?"p2":"p1",_=(d||[]).map($=>$.id),m=(d||[]).map($=>({id:$.id,gc_type:$.gc_type,_gcDef:$._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:l}=await T.from("matches").select("*").eq("id",i).single();if(!l){h("Match introuvable","error"),n("home");return}async function A(){var Y,ie;console.log("[PvP] buildGameState match:",{id:l.id,home_deck_id:l.home_deck_id,away_deck_id:l.away_deck_id,mode:l.mode,is_ranked:l.is_ranked});const[{data:$,error:u},{data:x,error:b},{data:v},{data:w}]=await Promise.all([T.rpc("get_deck_for_match",{p_deck_id:l.home_deck_id}),T.rpc("get_deck_for_match",{p_deck_id:l.away_deck_id}),T.from("users").select("id,pseudo,club_name").eq("id",l.home_id).single(),T.from("users").select("id,pseudo,club_name").eq("id",l.away_id).single()]);console.log("[PvP] get_deck_for_match p1:",u==null?void 0:u.message,"p2:",b==null?void 0:b.message,"p1D:",(Y=$==null?void 0:$.starters)==null?void 0:Y.length,"p2D:",(ie=x==null?void 0:x.starters)==null?void 0:ie.length);const k=ee=>{const ge=Number(ee.evolution_bonus)||0;return{cardId:ee.card_id,position:ee.position,id:ee.id,firstname:ee.firstname,name:ee.surname_real,country_code:ee.country_code,club_id:ee.club_id,job:ee.job,job2:ee.job2,note_g:(Number(ee.note_g)||0)+(ee.job==="GK"||ee.job2==="GK"&&Number(ee.note_g)>0?ge:0),note_d:(Number(ee.note_d)||0)+(ee.job==="DEF"||ee.job2==="DEF"&&Number(ee.note_d)>0?ge:0),note_m:(Number(ee.note_m)||0)+(ee.job==="MIL"||ee.job2==="MIL"&&Number(ee.note_m)>0?ge:0),note_a:(Number(ee.note_a)||0)+(ee.job==="ATT"||ee.job2==="ATT"&&Number(ee.note_a)>0?ge:0),evolution_bonus:ge,rarity:ee.rarity,skin:ee.skin,hair:ee.hair,hair_length:ee.hair_length,clubName:ee.club_encoded_name||null,clubLogo:ee.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},S=(($==null?void 0:$.starters)||[]).map(ee=>k(ee)),E=((x==null?void 0:x.starters)||[]).map(ee=>k(ee)),L=($==null?void 0:$.formation)||"4-4-2",z=(x==null?void 0:x.formation)||"4-4-2";let M=vt(S,L),F=vt(E,z);const V=(($==null?void 0:$.subs)||[]).map(ee=>k(ee)),W=((x==null?void 0:x.subs)||[]).map(ee=>k(ee)),H=($==null?void 0:$.stadium_def)||(o?p:null),P=(x==null?void 0:x.stadium_def)||(o?null:p);return H&&(M=Tt(M,H),jt(V,H)),P&&(F=Tt(F,P),jt(W,P)),{p1Team:M,p2Team:F,p1Subs:V,p2Subs:W,p1Formation:L,p2Formation:z,p1Name:(v==null?void 0:v.club_name)||(v==null?void 0:v.pseudo)||"Joueur 1",p2Name:(w==null?void 0:w.club_name)||(w==null?void 0:w.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:o?_:[],gc_p2:o?[]:_,gcCardsFull_p1:o?m:[],gcCardsFull_p2:o?[]:m,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:a||[],lastActionAt:new Date().toISOString()}}let g=l.game_state&&Object.keys(l.game_state).length?l.game_state:null;if(console.log("[PvP] init - amIHome:",o,"gameState exists:",!!g,"match.status:",l.status,"home_id:",l.home_id,"away_id:",l.away_id,"myId:",s.profile.id),!g)if(o){g=await A(),console.log("[PvP] home - gameState built, p1Team keys:",Object.keys((g==null?void 0:g.p1Team)||{}));const{data:$}=await T.from("matches").select("game_state").eq("id",i).single();!($!=null&&$.game_state)||!Object.keys($.game_state).length?await T.from("matches").update({game_state:g,turn_user_id:l.home_id}).eq("id",i):g=$.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>',console.log("[PvP] away - waiting for home to write game_state...");for(let $=0;$<30&&!g;$++){await new Promise(x=>setTimeout(x,400));const{data:u}=await T.from("matches").select("game_state").eq("id",i).single();u!=null&&u.game_state&&Object.keys(u.game_state).length&&(g=u.game_state),$%5===0&&console.log("[PvP] away - poll",$,"game_state:",!!(u!=null&&u.game_state))}if(!g){h("Erreur de synchronisation","error"),n("home");return}g.gc_p2=_,g.gcCardsFull_p2=m,await T.from("matches").update({game_state:g}).eq("id",i)}let I=!1,C=null,j=!1;const D=new Set,q=new Set;async function R($){var z,M;try{T.removeChannel(K)}catch{}const u=g[f+"Score"]||0,x=g[y+"Score"]||0;let b,v;$.winner_id?(b=$.winner_id===s.profile.id,v=!1):$.forfeit?(b=u>x,v=!1):(v=u===x,b=u>x);let w="";if(r&&o)try{const{data:F}=await T.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",l.home_id).single(),{data:V}=await T.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",l.away_id).single();if(F&&V){const W=b?1:v?.5:0,H=1-W,P=F.placement_matches<10,Y=V.placement_matches<10,ie=Ai(F.mmr,F.mmr_deviation,F.mmr_volatility,V.mmr,V.mmr_deviation,W===1?1:W===0?0:.5,P),ee=Ai(V.mmr,V.mmr_deviation,V.mmr_volatility,F.mmr,F.mmr_deviation,H===1?1:H===0?0:.5,Y);await T.rpc("update_mmr_after_ranked",{p_match_id:i,p_winner_id:v?null:b?l.home_id:l.away_id,p_home_id:l.home_id,p_away_id:l.away_id,p_home_delta:ie.delta,p_away_delta:ee.delta,p_home_new_rd:ie.newRd,p_away_new_rd:ee.newRd,p_home_new_vol:ie.newSigma,p_away_new_vol:ee.newSigma});const ge=ie.delta,be=ie.newMmr,ye=Gt(be),Ee=ge>=0?"+":"",Be=ge>=0?"#4caf50":"#ff6b6b",Fe=Gt(F.mmr);w=`
            <div style="background:rgba(255,255,255,0.08);border-radius:14px;padding:14px 20px;text-align:center;min-width:220px">
              ...
              ${ye.id!==Fe.id?`<div style="font-size:20px;font-weight:900;color:${ye.color}">
                    ${be>F.mmr?"📈":"📉"} ${Fe.emoji} ${Fe.label} → ${ye.emoji} ${ye.label}
                  </div>
                  <div style="font-size:13px;color:rgba(255,255,255,0.5);margin-top:4px">${be>F.mmr?"Promotion !":"Rétrogradation"}</div>`:`<div style="font-size:36px">${ge>=0?"📈":"📉"}</div>
            ...
            </div>`}}catch(F){console.error("[Ranked] MMR update error:",F)}(z=document.getElementById("pvp-end-overlay"))==null||z.remove();const k=document.createElement("div");k.id="pvp-end-overlay",k.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;color:#fff;padding:24px;text-align:center;overflow-y:auto";const S=v?"🤝":b?"🏆":"😞",E=v?"MATCH NUL":b?"VICTOIRE !":"DÉFAITE",L=v?"#fff":b?"#FFD700":"#ff6b6b";k.innerHTML=`
      ...
      <div style="font-size:18px">${g[f+"Name"]} ${u} – ${x} ${g[y+"Name"]}</div>
      ${$.forfeit?`<div style="font-size:13px;color:rgba(255,255,255,0.5)">${b?"L'adversaire a quitté":"Perdu par forfait"}</div>`:""}
      ${w}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${r?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(k),(M=k.querySelector("#pvp-end-home"))==null||M.addEventListener("click",()=>{k.remove(),Ve(e),n(r?"ranked":"home")})}const K=T.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},$=>{const u=$.new;try{if(u.status==="finished"||u.forfeit){if(I)return;I=!0,C&&(clearInterval(C),C=null),u.game_state&&(g=u.game_state),R(u);return}if(u.game_state){const x=g;g=u.game_state;const b=g._lastGC;if(b&&b.seq&&!q.has(b.seq)&&(q.add(b.seq),b.by!==f)){ce(b.type,b.by,()=>xe());return}const v=x[f+"Score"]||0,w=x[y+"Score"]||0,k=g[f+"Score"]||0,S=g[y+"Score"]||0,E=k>v,L=S>w;if((E||L)&&!D.has(g.round)){D.add(g.round);const z=[...g.log||[]].reverse().find(F=>F.isGoal),M=((z==null?void 0:z.homePlayers)||[]).map(F=>({name:F.name,note:F.note,portrait:F.portrait,job:F.job}));Me(M,k,S,E,()=>xe());return}xe()}}catch(x){console.error("[PvP] crash:",x)}}).subscribe();async function J($){Object.assign(g,$),g.lastActionAt=new Date().toISOString();const{error:u}=await T.from("matches").update({game_state:g}).eq("id",i);u&&h("Erreur de synchronisation","error");try{xe()}catch(x){console.error("[PvP] renderPvpScreen crash:",x)}}async function re(){if(I)return;I=!0,C&&(clearInterval(C),C=null);const $=o?l.away_id:l.home_id,u=o?"p2":"p1",x=o?"p1":"p2",b={...g,[u+"Score"]:3,[x+"Score"]:0,phase:"finished"};try{await T.from("matches").update({status:"finished",forfeit:!0,winner_id:$,home_score:b.p1Score||0,away_score:b.p2Score||0,game_state:b}).eq("id",i)}catch{}try{T.removeChannel(K)}catch{}setTimeout(()=>{Ve(e),n("home")},800)}const ne={BOOST_STAT:({value:$=1,count:u=1,roles:x=[]},b,v)=>{const w=b[f+"Team"],k=Object.entries(w).filter(([S])=>!x.length||x.includes(S)).flatMap(([S,E])=>E.filter(L=>!L.used).map(L=>({...L,_line:S})));if(!k.length){b.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),v(b);return}Q(k,u,`Choisir ${u} joueur(s) à booster (+${$})`,S=>{S.forEach(E=>{const L=(w[E._line]||[]).find(z=>z.cardId===E.cardId);L&&(L.boost=(L.boost||0)+$,b.log.push({text:`⚡ +${$} sur ${L.name}`,type:"info"}))}),b[f+"Team"]=w,v(b)})},DEBUFF_STAT:({value:$=1,count:u=1,roles:x=[],target:b="ai"},v,w)=>{const k=b==="home"?f:y,S=v[k+"Team"],E=b==="home"?"allié":"adverse",L=Object.entries(S).filter(([z])=>!x.length||x.includes(z)).flatMap(([z,M])=>M.map(F=>({...F,_line:z})));if(!L.length){v.log.push({text:`🎯 Aucun joueur ${E}`,type:"info"}),w(v);return}Q(L,u,`Choisir ${u} joueur(s) ${E}(s) (-${$})`,z=>{z.forEach(M=>{const F=(S[M._line]||[]).find(V=>V.cardId===M.cardId);F&&(F.boost=(F.boost||0)-$,v.log.push({text:`🎯 -${$} sur ${F.name}`,type:"info"}))}),v[k+"Team"]=S,w(v)})},GRAY_PLAYER:({count:$=1,roles:u=[],target:x="ai"},b,v)=>{const w=x==="home"?f:y,k=b[w+"Team"],S=x==="home"?"allié":"adverse",E=Object.entries(k).filter(([L])=>!u.length||u.includes(L)).flatMap(([L,z])=>z.filter(M=>!M.used).map(M=>({...M,_line:L})));if(!E.length){b.log.push({text:`❌ Aucun joueur ${S}`,type:"info"}),v(b);return}Q(E,$,`Choisir ${$} joueur(s) ${S}(s) à exclure`,L=>{const z="usedCardIds_"+w,M=new Set(b[z]||[]);L.forEach(F=>{const V=(k[F._line]||[]).find(W=>W.cardId===F.cardId);V&&(V.used=!0,M.add(F.cardId),b.log.push({text:`❌ ${V.name} exclu !`,type:"info"}))}),b[z]=[...M],b[w+"Team"]=k,v(b)})},REVIVE_PLAYER:({count:$=1,roles:u=[]},x,b)=>{const v=x[f+"Team"],w=Object.entries(v).filter(([k])=>!u.length||u.includes(k)).flatMap(([k,S])=>S.filter(E=>E.used).map(E=>({...E,_line:k})));if(!w.length){x.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),b(x);return}Q(w,$,`Choisir ${$} joueur(s) à ressusciter`,k=>{k.forEach(S=>{const E=(v[S._line]||[]).find(L=>L.cardId===S.cardId);E&&(E.used=!1,x.log.push({text:`💫 ${E.name} ressuscité !`,type:"info"}))}),x[f+"Team"]=v,b(x)})},REMOVE_GOAL:({},$,u)=>{const x=y+"Score";$[x]>0?($[x]--,$.log.push({text:"🚫 Dernier but annulé !",type:"info"})):$.log.push({text:"🚫 Aucun but à annuler",type:"info"}),u($)},ADD_GOAL_DRAW:({},$,u)=>{$[f+"Score"]===$[y+"Score"]?($[f+"Score"]++,$.log.push({text:"🎯 But bonus !",type:"info"})):$.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),u($)},ADD_SUB:({value:$=1},u,x)=>{u.maxSubs=(u.maxSubs||3)+$,u.log.push({text:`🔄 +${$} remplacement(s)`,type:"info"}),x(u)},CUSTOM:({},$,u)=>u($)};function Q($,u,x,b){const v=document.createElement("div");v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let w=[];function k(){var E,L;const S=$.map(z=>{const M={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[z._line]||"#555",F=ze(z,z._line)+(z.boost||0),W=w.find(P=>P.cardId===z.cardId)?"#FFD700":"rgba(255,255,255,0.25)",H=z.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${z.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${W};background:${M};overflow:hidden;cursor:pointer;${H}">
        ...
        </div>`,(E=v.querySelector("#pp-close"))==null||E.addEventListener("click",()=>v.remove()),v.querySelectorAll(".pp-item").forEach(z=>{z.addEventListener("click",()=>{const M=z.dataset.cid,F=$.find(W=>W.cardId===M),V=w.findIndex(W=>W.cardId===M);F&&(V>-1?w.splice(V,1):w.length<u&&w.push(F),k())})}),(L=v.querySelector("#pp-confirm"))==null||L.addEventListener("click",()=>{v.remove(),b(w)})}k(),document.body.appendChild(v)}async function N($,u){const b=(g["gcCardsFull_"+f]||[]).find(S=>S.id===$),v=(b==null?void 0:b._gcDef)||(g.gcDefs||[]).find(S=>{var E;return S.name===u||((E=S.name)==null?void 0:E.toLowerCase().trim())===(u==null?void 0:u.toLowerCase().trim())}),w=[...g["usedGc_"+f]||[],$],k={type:u,by:f,seq:(g._gcAnimSeq||0)+1};q.add(k.seq),ce(u,f,async()=>{if(v!=null&&v.effect_type&&v.effect_type!=="CUSTOM"){const E=ne[v.effect_type];if(E){const L={...g};E(v.effect_params||{},L,async z=>{z["usedGc_"+f]=w,z._lastGC=k,z._gcAnimSeq=k.seq,await J(z)});return}}const S={...g};switch(u){case"Remplacement+":S.maxSubs=(S.maxSubs||3)+1,S.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const E=y+"Score";S[E]>0&&(S[E]--,S.log.push({text:"🚫 But annulé",type:"info"}));break}}S["usedGc_"+f]=w,S._lastGC=k,S._gcAnimSeq=k.seq,await J(S)})}function G($,u){const x="usedCardIds_"+$,b=new Set(g[x]||[]);u.forEach(v=>b.add(v)),g[x]=[...b]}function oe(){for(const $ of["p1","p2"]){const u=new Set(g["usedCardIds_"+$]||[]),x=g[$+"Team"];if(x)for(const b of["GK","DEF","MIL","ATT"])(x[b]||[]).forEach(v=>{v.used=u.has(v.cardId)})}}function xe(){var We,ot,ut,ft,gt,ue;if(g.phase==="reveal")return _e();if(g.phase==="midfield")return we();if(g.phase==="finished")return B();const $=g[f+"Team"],u=g[y+"Team"];oe();const x=g[f+"Score"],b=g[y+"Score"],v=g[f+"Name"],w=g[y+"Name"],k=g.phase===f+"-attack",S=g.phase===f+"-defense",E=Array.isArray(g["selected_"+f])?g["selected_"+f]:[],L=E.map(X=>X.cardId),z=window.innerWidth>=700,M=g[f+"Subs"]||[],F=g["usedSubIds_"+f]||[],V=M.filter(X=>!F.includes(X.cardId)),W=g["gcCardsFull_"+f]||[],H=g["usedGc_"+f]||[],P=W.filter(X=>!H.includes(X.id)),Y=g.boostOwner===f&&!g.boostUsed,ie=!["GK","DEF","MIL","ATT"].some(X=>(u[X]||[]).some(O=>!O.used)),ee=[...$.MIL||[],...$.ATT||[]].filter(X=>!X.used),ge=k&&ie&&ee.length===0?[...$.GK||[],...$.DEF||[]].filter(X=>!X.used).map(X=>X.cardId):[];function be(X,O,ae){var kt,Ct;const he=X._gcDef,$e={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[he==null?void 0:he.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",Qe={purple:"#b06ce0",light_blue:"#00d4ef"}[he==null?void 0:he.color]||"#b06ce0",Ze=(he==null?void 0:he.name)||X.gc_type,tt=(he==null?void 0:he.effect)||((kt=Oe[X.gc_type])==null?void 0:kt.desc)||"",rt=he!=null&&he.image_url?`/icons/${he.image_url}`:null,_t=((Ct=Oe[X.gc_type])==null?void 0:Ct.icon)||"⚡",$t=Math.round(ae*.22),lt=Math.round(ae*.22),mt=ae-$t-lt,xt=Ze.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${X.id}" data-gc-type="${X.gc_type}"
      ...
      </div>`}function ye(X,O){return`<div id="pvp-boost-card"
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
      <div style="font-size:20px;font-weight:900;color:#ff6b6b">${g[y+"Name"]||"Adversaire"}</div>
      <div style="width:min(90vw,420px)">${ct(g[y+"Team"],g[y+"Formation"],null,[],300,300)}</div>
    </div>`,f==="p1"&&setTimeout(async()=>{await J({phase:"midfield"})},5e3)}let ve=!1;function we(){if(ve)return;const $=g[f+"Team"].MIL||[],u=g[y+"Team"].MIL||[];function x(P){return P.reduce((Y,ie)=>Y+ze(ie,"MIL"),0)}function b(P){let Y=0;for(let ie=0;ie<P.length-1;ie++){const ee=nt(P[ie],P[ie+1]);ee==="#00ff88"?Y+=2:ee==="#FFD700"&&(Y+=1)}return Y}const v=x($)+b($),w=x(u)+b(u),k=v>=w;function S(P,Y,ie){return`<div id="duel-row-${ie}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
        ...
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${P.map((ee,ge)=>{const be=ge<P.length-1?nt(ee,P[ge+1]):null,ye=!be||be==="#ff3333"||be==="#cc2222",Ee=be==="#00ff88"?"+2":be==="#FFD700"?"+1":"";return`<div class="duel-card duel-card-${ie}" data-idx="${ge}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease,transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
              ${Xe({...ee,note:ze(ee,"MIL"),_line:"MIL"},58,78)}
            </div>
            ${ge<P.length-1?`<div class="duel-link duel-link-${ie}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${ye?"rgba(255,255,255,0.12)":be};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${ye?"none":`0 0 8px ${be}`}">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(w);let E=!1;const L=()=>{E||(E=!0,w.remove(),setTimeout(()=>v(),50))};w.addEventListener("click",L),setTimeout(L,3500)}function ce($,u,x){var H,P;const b=(g.gcDefs||[]).find(Y=>{var ie;return Y.name===$||((ie=Y.name)==null?void 0:ie.toLowerCase().trim())===($==null?void 0:$.toLowerCase().trim())}),v={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[b==null?void 0:b.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",w={purple:"#b06ce0",light_blue:"#00d4ef"}[b==null?void 0:b.color]||"#b06ce0",k=(b==null?void 0:b.name)||$,S=(b==null?void 0:b.effect)||((H=Oe[$])==null?void 0:H.desc)||"",E=b!=null&&b.image_url?`/icons/${b.image_url}`:null,L=((P=Oe[$])==null?void 0:P.icon)||"⚡",M=u===f?"Vous":g[u+"Name"]||"Adversaire",F=document.createElement("div");F.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",F.innerHTML=`
      <style>
      ...
      </div>`,document.body.appendChild(x),(b=x.querySelector("#bp-close"))==null||b.addEventListener("click",()=>x.remove()),x.querySelectorAll(".bp-item").forEach(v=>{v.addEventListener("click",async()=>{const w=v.dataset.cid,k=u.find(E=>E.cardId===w);if(!k)return;const S=($[k._line]||[]).find(E=>E.cardId===w);S&&(S.boost=(S.boost||0)+g.boostValue),x.remove(),await J({[f+"Team"]:$,boostUsed:!0})})})}function U($=null){var V,W;if(!(g.phase===f+"-attack")){h("Remplacement uniquement avant votre attaque","warning");return}const x=g[f+"Team"],b=g["usedSubIds_"+f]||[],v=g.maxSubs||3;if(b.length>=v){h(`Maximum ${v} remplacements atteint`,"warning");return}const w=Object.entries(x).flatMap(([H,P])=>(P||[]).filter(Y=>Y.used).map(Y=>({...Y,_line:H}))),k=(g[f+"Subs"]||[]).filter(H=>!b.includes(H.cardId));if(!w.length){h("Aucun joueur utilisé à remplacer","warning");return}if(!k.length){h("Aucun remplaçant disponible","warning");return}let S=Math.max(0,w.findIndex(H=>H.cardId===$));const E=((V=w[S])==null?void 0:V._line)||((W=w[S])==null?void 0:W.job);let L=Math.max(0,k.findIndex(H=>H.job===E)),z=!1;const M=document.createElement("div");M.id="pvp-sub-overlay",M.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function F(){var be,ye,Ee,Be,Fe,je;const H=w[S],P=k[L],Y=Math.min(130,Math.round((window.innerWidth-90)/2)),ie=Math.round(Y*1.35),ee=Ce=>`background:rgba(255,255,255,0.12);border:none;color:${Ce?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Ce?"default":"pointer"};flex-shrink:0`;M.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      </div>`,(be=M.querySelector("#psub-close"))==null||be.addEventListener("click",()=>M.remove()),(ye=M.querySelector("#pout-up"))==null||ye.addEventListener("click",()=>{S>0&&(S--,F())}),(Ee=M.querySelector("#pout-down"))==null||Ee.addEventListener("click",()=>{S<w.length-1&&(S++,F())}),(Be=M.querySelector("#pin-up"))==null||Be.addEventListener("click",()=>{L>0&&(L--,F())}),(Fe=M.querySelector("#pin-down"))==null||Fe.addEventListener("click",()=>{L<k.length-1&&(L++,F())});const ge=(Ce,Ie,Ne,Ue)=>{const Ye=M.querySelector("#"+Ce);if(!Ye)return;let Ke=0;Ye.addEventListener("touchstart",Re=>{Ke=Re.touches[0].clientY},{passive:!0}),Ye.addEventListener("touchend",Re=>{const Pe=Re.changedTouches[0].clientY-Ke;if(Math.abs(Pe)<30)return;const We=Ie();Pe<0&&We<Ue-1?(Ne(We+1),F()):Pe>0&&We>0&&(Ne(We-1),F())},{passive:!0})};ge("pin-panel",()=>L,Ce=>L=Ce,k.length),ge("pout-panel",()=>S,Ce=>S=Ce,w.length),(je=M.querySelector("#psub-confirm"))==null||je.addEventListener("click",async Ce=>{if(Ce.preventDefault(),Ce.stopPropagation(),z)return;z=!0;const Ie=w[S],Ne=k[L];if(!Ie||!Ne)return;const Ue=Ie._line,Ye=(x[Ue]||[]).findIndex(Pe=>Pe.cardId===Ie.cardId);if(Ye===-1){h("Erreur : joueur introuvable","error"),M.remove();return}const Ke={...Ne,_line:Ue,position:Ie.position,used:!1,boost:0};x[Ue].splice(Ye,1,Ke);const Re=[...b,Ne.cardId];M.remove(),Z(Ie,Ne,async()=>{await J({[f+"Team"]:x,[y+"Team"]:g[y+"Team"],["usedSubIds_"+f]:Re})})})}document.body.appendChild(M),F()}function Z($,u,x){const b={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},v=document.createElement("div");v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const w=(E,L,z)=>`<div style="text-align:center">
      ...
      <div style="font-size:18px;font-weight:900;color:#ff6b6b">${g[y+"Name"]}</div>
      <div style="width:min(90vw,420px)">${ct(g[y+"Team"],g[y+"Formation"],null,[],300,300)}</div>
      ...
      </div>`,document.body.appendChild(u),(x=u.querySelector("#pvp-hist-close"))==null||x.addEventListener("click",()=>u.remove())}async function fe(){if(g.phase!==f+"-attack")return;const $=f==="p1"?"p2":"p1",u=(g.round||0)+1,x=[...g.log||[]];x.push({type:"info",text:`⏭️ ${g[f+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const b=qe(g),v=ke($),w=b||!v?"finished":$+"-attack";await J({["selected_"+f]:[],modifiers:{...g.modifiers,[f]:{}},pendingAttack:null,phase:w,attacker:$,round:u,log:x}),w==="finished"&&await Se(g)}async function te(){const $=g[f+"Team"],u=!["GK","DEF","MIL","ATT"].some(w=>(g[y+"Team"][w]||[]).some(k=>!k.used)),x=(g["selected_"+f]||[]).map(w=>{const k=($[w._role]||[]).find(F=>F.cardId===w.cardId)||w,S=u&&["GK","DEF"].includes(w._role),E=$[w._role]||[],L=E.findIndex(F=>F.cardId===w.cardId),z=dt(E.length),M=L>=0?z[L]:k._col??1;return{...k,_line:w._role,_col:M,...S?{note_a:Math.max(1,Number(k.note_a)||0)}:{}}});if(!x.length)return;const b=Ht(x,g.modifiers[f]||{});G(f,x.map(w=>w.cardId)),x.forEach(w=>{const k=($[w._role]||[]).find(S=>S.cardId===w.cardId);k&&(k.used=!0)}),g["selected_"+f]=[],xe();const v=[...g.log||[]];if(u){const w=(g[f+"Score"]||0)+1,k=x.map(M=>({name:M.name,note:ze(M,M._line||"ATT"),portrait:He(M),job:M.job}));v.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:k,homeTotal:b.total,aiTotal:0});const S=(g.round||0)+1,E=f==="p1"?"p2":"p1",L={...g,[f+"Team"]:$,[f+"Score"]:w},z=qe(L);D.add(S),Me(k,w,g[y+"Score"]||0,!0,async()=>{await J({[f+"Team"]:$,[f+"Score"]:w,["selected_"+f]:[],modifiers:{...g.modifiers,[f]:{}},pendingAttack:null,phase:z?"finished":E+"-attack",attacker:E,round:S,log:v}),z&&await Se({...g,[f+"Score"]:w})});return}v.push({type:"pending",text:`⚔️ ${g[f+"Name"]} attaque (${b.total})`}),await J({[f+"Team"]:$,[y+"Team"]:g[y+"Team"],pendingAttack:{...b,players:x,side:f},["selected_"+f]:[],modifiers:{...g.modifiers,[f]:{}},phase:y+"-defense",log:v})}async function le(){const $=g[f+"Team"],u=(g["selected_"+f]||[]).map(H=>{const P=($[H._role]||[]).find(be=>be.cardId===H.cardId)||H,Y=$[H._role]||[],ie=Y.findIndex(be=>be.cardId===H.cardId),ee=dt(Y.length),ge=ie>=0?ee[ie]:P._col??1;return{...P,_line:H._role,_col:ge}}),x=Ut(u,g.modifiers[f]||{});G(f,u.map(H=>H.cardId)),u.forEach(H=>{const P=($[H._role]||[]).find(Y=>Y.cardId===H.cardId);P&&(P.used=!0)}),g["selected_"+f]=[],xe();const b=Kt(g.pendingAttack.total,x.total,g.modifiers[f]||{}),v=g.pendingAttack.side,w=b==="attack"||(b==null?void 0:b.goal),k=v==="p1"?"p2":"p1",S=(g.round||0)+1,E=(g.pendingAttack.players||[]).map(H=>({name:H.name,note:ze(H,H._line||"ATT"),portrait:He(H),job:H.job})),L=[...g.log||[]];L.push({type:"duel",isGoal:w,homeScored:w&&v===f,text:w?`⚽ BUT de ${g[v+"Name"]} ! (${g.pendingAttack.total} vs ${x.total})`:`✋ Attaque stoppée (${g.pendingAttack.total} vs ${x.total})`,homePlayers:E,aiPlayers:u.map(H=>({name:H.name,note:ze(H,H._line||"DEF"),portrait:He(H),job:H.job})),homeTotal:g.pendingAttack.total,aiTotal:x.total});const z=w?(g[v+"Score"]||0)+1:g[v+"Score"]||0,M={...g,[f+"Team"]:$,[v+"Score"]:z},F=qe(M),V=F?"finished":k+"-attack",W=async()=>{await J({[f+"Team"]:$,[y+"Team"]:g[y+"Team"],[v+"Score"]:z,["selected_"+f]:[],modifiers:{...g.modifiers,[f]:{}},pendingAttack:null,phase:V,attacker:k,round:S,log:L}),(V==="finished"||F)&&await Se({...g,[v+"Score"]:z})};if(w){const H=v===f,P=H?z:g[f+"Score"]||0,Y=H?g[y+"Score"]||0:z;D.add(S),Me(E,P,Y,H,W)}else await W()}function Le($){return["MIL","ATT"].some(u=>($[u]||[]).some(x=>!x.used))}function Te($){return["GK","DEF","MIL","ATT"].some(u=>($[u]||[]).some(x=>!x.used))}function Ae($){return Te($)&&!Le($)}function ke($){const u=g[$+"Team"],x=g[($==="p1"?"p2":"p1")+"Team"];return!!(Le(u)||!Te(x)&&Ae(u))}function qe($){const u=["MIL","ATT"].some(L=>($.p1Team[L]||[]).some(z=>!z.used)),x=["MIL","ATT"].some(L=>($.p2Team[L]||[]).some(z=>!z.used)),b=Te($.p1Team),v=Te($.p2Team);return!u&&!(!v&&b)&&(!x&&!(!b&&v))}function Ge($){const u=$.p1Score||0,x=$.p2Score||0;return u===x?null:u>x?l.home_id:l.away_id}async function Se($){try{const u=Ge($),x=u?l.home_id===u?l.away_id:l.home_id:null;await T.from("matches").update({status:"finished",winner_id:u,home_score:$.p1Score||0,away_score:$.p2Score||0}).eq("id",i),u&&x&&updateEvolutiveCards(u,x).catch(()=>{})}catch(u){console.error("[PvP] finishMatch:",u)}}function B(){var v;const $=g[f+"Score"],u=g[y+"Score"],x=$>u,b=$===u;et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
    ...
    </div>`,(v=document.getElementById("pvp-home"))==null||v.addEventListener("click",()=>{try{T.removeChannel(K)}catch{}Ve(e),n("home")})}xe()}const Fn=Object.freeze(Object.defineProperty({__proto__:null,renderMatchRandom:yi,resumePvpMatch:Bn},Symbol.toStringTag,{value:"Module"}));async function Dn(e,t,i,o){var d,a;try{const r=(a=(d=t==null?void 0:t.state)==null?void 0:d.profile)==null?void 0:a.id;try{(T.getChannels?T.getChannels():[]).forEach(p=>{const s=p.topic||"";(s.includes("matchmaking")||s.includes("pvp-match"))&&T.removeChannel(p)})}catch(c){console.warn("[FriendMatch] cleanup canaux:",c)}r&&(await T.rpc("cancel_matchmaking",{p_user_id:r}).catch(()=>{}),await T.from("matchmaking_queue").delete().eq("user_id",r).then(()=>{},()=>{}))}catch{}await ri(e,t,"random",({deckId:r,formation:c,starters:p,subsRaw:s,gcCardsEnriched:n,gcDefs:h,stadiumDef:f})=>{const y=_=>Gn(e,t,r,c,p,s,_,h||[],i,o);if(!n.length){y([]);return}ni(e,n,y)})}async function Gn(e,t,i,o,d,a,r=[],c=[],p,s){var K;const{state:n,navigate:h,toast:f}=t,y=n.profile.id;let _=!1,m=null;et(e);try{await T.rpc("cancel_matchmaking",{p_user_id:y})}catch{}try{await T.from("matchmaking_queue").delete().eq("user_id",y)}catch{}const l=(J,re,ne,Q)=>{var N;e.innerHTML=`
      ...
      </div>`,(N=document.getElementById("lobby-cancel"))==null||N.addEventListener("click",async()=>{_=!0,m&&(T.removeChannel(m),m=null),ne&&await T.from("friend_match_invites").update({status:"declined"}).eq("id",ne),Ve(e),h("home")})},A=async(J,re)=>{_=!0,m&&(T.removeChannel(m),m=null),await new Promise(ne=>setTimeout(ne,600)),e.isConnected&&Pn(e,t,J,re,r,c)},{data:g}=await T.from("friend_match_invites").select("*").eq("status","pending").or(`and(inviter_id.eq.${y},invitee_id.eq.${p}),and(inviter_id.eq.${p},invitee_id.eq.${y})`).order("created_at",{ascending:!1}).limit(1).maybeSingle();let I,C;if(g&&g.inviter_id===p)C=!1,I=g.id,await T.from("friend_match_invites").update({invitee_deck_id:i,invitee_ready:!0,status:"accepted"}).eq("id",I);else{C=!0;const{data:J,error:re}=await T.from("friend_match_invites").insert({inviter_id:y,invitee_id:p,friend_id:p,inviter_deck_id:i,inviter_ready:!0,status:"pending"}).select().single();if(re||!J){f("Erreur création invitation","error"),Ve(e),h("home");return}I=J.id}if(l(!0,!C,I),!C){const J={home_id:p,away_id:y,home_deck_id:g.inviter_deck_id,away_deck_id:i,status:"active",mode:"friend"},re=await T.from("matches").insert(J).select().single(),ne=re.data;if(!ne){f("Erreur création match: "+(((K=re.error)==null?void 0:K.message)||""),"error"),Ve(e),h("home");return}await T.from("friend_match_invites").update({match_id:ne.id,status:"matched"}).eq("id",I),A(ne.id,!1);return}let j=!1;const D=J=>{_||j||(J.status==="matched"&&J.match_id?(j=!0,clearInterval(q),clearInterval(R),A(J.match_id,!0)):J.status==="declined"?(clearInterval(q),clearInterval(R),f(`${s} a décliné l'invitation`,"warning"),Ve(e),h("home")):J.invitee_ready&&l(!0,!0,I))},q=setInterval(()=>{if(_){clearInterval(q);return}T.from("matchmaking_queue").delete().eq("user_id",y).then(()=>{},()=>{})},3e3),R=setInterval(async()=>{if(_||j){clearInterval(R);return}const{data:J}=await T.from("friend_match_invites").select("*").eq("id",I).maybeSingle();J&&D(J)},1200);m=T.channel("friend-invite-"+I).on("postgres_changes",{event:"UPDATE",schema:"public",table:"friend_match_invites",filter:`id=eq.${I}`},J=>D(J.new)).subscribe()}async function Pn(e,t,i,o,d=[],a=[]){const{state:r,navigate:c,toast:p}=t,s=o?"p1":"p2",n=o?"p2":"p1",h=(d||[]).map(B=>B.id),f=(d||[]).map(B=>({id:B.id,gc_type:B.gc_type,_gcDef:B._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:y}=await T.from("matches").select("*").eq("id",i).single();if(!y){p("Match introuvable","error"),c("home");return}async function _(){const[{data:B},{data:$},{data:u},{data:x}]=await Promise.all([T.rpc("get_deck_for_match",{p_deck_id:y.home_deck_id}),T.rpc("get_deck_for_match",{p_deck_id:y.away_deck_id}),T.from("users").select("id,pseudo,club_name").eq("id",y.home_id).single(),T.from("users").select("id,pseudo,club_name").eq("id",y.away_id).single()]),b=E=>{const L=Number(E.evolution_bonus)||0;return{cardId:E.card_id,position:E.position,id:E.id,firstname:E.firstname,name:E.surname_real,country_code:E.country_code,club_id:E.club_id,job:E.job,job2:E.job2,note_g:(Number(E.note_g)||0)+(E.job==="GK"||E.job2==="GK"&&Number(E.note_g)>0?L:0),note_d:(Number(E.note_d)||0)+(E.job==="DEF"||E.job2==="DEF"&&Number(E.note_d)>0?L:0),note_m:(Number(E.note_m)||0)+(E.job==="MIL"||E.job2==="MIL"&&Number(E.note_m)>0?L:0),note_a:(Number(E.note_a)||0)+(E.job==="ATT"||E.job2==="ATT"&&Number(E.note_a)>0?L:0),evolution_bonus:L,rarity:E.rarity,skin:E.skin,hair:E.hair,hair_length:E.hair_length,clubName:E.club_encoded_name||null,clubLogo:E.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},v=((B==null?void 0:B.starters)||[]).map(E=>b(E)),w=(($==null?void 0:$.starters)||[]).map(E=>b(E)),k=(B==null?void 0:B.formation)||"4-4-2",S=($==null?void 0:$.formation)||"4-4-2";return{p1Team:(()=>{const E=vt(v,k);return stadiumDef?Tt(E,stadiumDef):E})(),p2Team:vt(w,S),p1Subs:(()=>{const E=((B==null?void 0:B.subs)||[]).map(L=>b(L));return stadiumDef&&jt(E,stadiumDef),E})(),p2Subs:(($==null?void 0:$.subs)||[]).map(E=>b(E)),p1Formation:k,p2Formation:S,p1Name:(u==null?void 0:u.club_name)||(u==null?void 0:u.pseudo)||"Joueur 1",p2Name:(x==null?void 0:x.club_name)||(x==null?void 0:x.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:o?h:[],gc_p2:o?[]:h,gcCardsFull_p1:o?f:[],gcCardsFull_p2:o?[]:f,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:a||[],lastActionAt:new Date().toISOString()}}let m=y.game_state&&Object.keys(y.game_state).length?y.game_state:null;if(!m)if(o){m=await _();const{data:B}=await T.from("matches").select("game_state").eq("id",i).single();!(B!=null&&B.game_state)||!Object.keys(B.game_state).length?await T.from("matches").update({game_state:m,turn_user_id:y.home_id}).eq("id",i):m=B.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let B=0;B<60&&!m;B++){await new Promise(u=>setTimeout(u,400));const{data:$}=await T.from("matches").select("game_state").eq("id",i).single();$!=null&&$.game_state&&Object.keys($.game_state).length&&(m=$.game_state)}if(!m){p("Erreur de synchronisation","error"),c("home");return}m.gc_p2=h,m.gcCardsFull_p2=f,await T.from("matches").update({game_state:m}).eq("id",i)}let l=!1,A=!1,g=!1,I=null,C=!1;const j=new Set,D=new Set;function q(B){var E,L;try{T.removeChannel(R)}catch{}const $=m[s+"Score"]||0,u=m[n+"Score"]||0;if(!g){g=!0;const z=B.winner_id||($>u?r.profile.id:u>$?"opp":null),M=z===r.profile.id?"win":z?"loss":null;M&&ii(()=>Promise.resolve().then(()=>Zi),void 0).then(F=>F.applyOwnEvolution(r.profile.id,M)).catch(()=>{})}if(!A){const z=m.p1Score||0,M=m.p2Score||0,F=(m.usedGc_p1||[]).length,V=(m.usedGc_p2||[]).length,W=B.winner_id||(z>M?y.home_id:M>z?y.away_id:null);(W?r.profile.id===W:r.profile.id<(o?y.away_id:y.home_id))&&(A=!0,Ki({player1Id:y.home_id,player2Id:y.away_id,score1:z,score2:M,gc1:F,gc2:V}).catch(P=>console.warn("[FriendMatch] updateStats:",P)))}let x,b;B.winner_id?(x=B.winner_id===r.profile.id,b=!1):B.forfeit?(x=$>u,b=!1):(b=$===u,x=$>u),(E=document.getElementById("pvp-end-overlay"))==null||E.remove();const v=document.createElement("div");v.id="pvp-end-overlay",v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const w=b?"🤝":x?"🏆":"😞",k=b?"MATCH NUL":x?"VICTOIRE !":"DÉFAITE",S=b?"#fff":x?"#FFD700":"#ff6b6b";v.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(v),(L=v.querySelector("#pvp-end-home"))==null||L.addEventListener("click",()=>{v.remove(),Ve(e),c("home")})}const R=T.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},B=>{const $=B.new;try{if($.status==="finished"||$.forfeit){if(l)return;l=!0,I&&(clearInterval(I),I=null),$.game_state&&(m=$.game_state),q($);return}if($.game_state){const u=m;m=$.game_state;const x=m._lastGC;if(x&&x.seq&&!D.has(x.seq)&&(D.add(x.seq),x.by!==s)){Me(x.type,x.by,()=>oe());return}const b=u[s+"Score"]||0,v=u[n+"Score"]||0,w=m[s+"Score"]||0,k=m[n+"Score"]||0,S=w>b,E=k>v;if((S||E)&&!j.has(m.round)){j.add(m.round);const L=[...m.log||[]].reverse().find(M=>M.isGoal),z=((L==null?void 0:L.homePlayers)||[]).map(M=>({name:M.name,note:M.note,portrait:M.portrait,job:M.job}));we(z,w,k,S,()=>oe());return}oe()}}catch(u){console.error("[PvP] crash:",u)}}).subscribe();async function K(B){Object.assign(m,B),m.lastActionAt=new Date().toISOString();const{error:$}=await T.from("matches").update({game_state:m}).eq("id",i);$&&p("Erreur de synchronisation","error");try{oe()}catch(u){console.error("[PvP] renderPvpScreen crash:",u)}}async function J(){if(l)return;l=!0,I&&(clearInterval(I),I=null);const B=o?y.away_id:y.home_id,$=o?"p2":"p1",u=o?"p1":"p2",x={...m,[$+"Score"]:3,[u+"Score"]:0,phase:"finished"};try{await T.from("matches").update({status:"finished",forfeit:!0,winner_id:B,home_score:x.p1Score||0,away_score:x.p2Score||0,game_state:x}).eq("id",i)}catch{}try{T.removeChannel(R)}catch{}setTimeout(()=>{Ve(e),c("home")},800)}const re={BOOST_STAT:({value:B=1,count:$=1,roles:u=[]},x,b)=>{const v=x[s+"Team"],w=Object.entries(v).filter(([k])=>!u.length||u.includes(k)).flatMap(([k,S])=>S.filter(E=>!E.used).map(E=>({...E,_line:k})));if(!w.length){x.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),b(x);return}ne(w,$,`Choisir ${$} joueur(s) à booster (+${B})`,k=>{k.forEach(S=>{const E=(v[S._line]||[]).find(L=>L.cardId===S.cardId);E&&(E.boost=(E.boost||0)+B,x.log.push({text:`⚡ +${B} sur ${E.name}`,type:"info"}))}),x[s+"Team"]=v,b(x)})},DEBUFF_STAT:({value:B=1,count:$=1,roles:u=[],target:x="ai"},b,v)=>{const w=x==="home"?s:n,k=b[w+"Team"],S=x==="home"?"allié":"adverse",E=Object.entries(k).filter(([L])=>!u.length||u.includes(L)).flatMap(([L,z])=>z.map(M=>({...M,_line:L})));if(!E.length){b.log.push({text:`🎯 Aucun joueur ${S}`,type:"info"}),v(b);return}ne(E,$,`Choisir ${$} joueur(s) ${S}(s) (-${B})`,L=>{L.forEach(z=>{const M=(k[z._line]||[]).find(F=>F.cardId===z.cardId);M&&(M.boost=(M.boost||0)-B,b.log.push({text:`🎯 -${B} sur ${M.name}`,type:"info"}))}),b[w+"Team"]=k,v(b)})},GRAY_PLAYER:({count:B=1,roles:$=[],target:u="ai"},x,b)=>{const v=u==="home"?s:n,w=x[v+"Team"],k=u==="home"?"allié":"adverse",S=Object.entries(w).filter(([E])=>!$.length||$.includes(E)).flatMap(([E,L])=>L.filter(z=>!z.used).map(z=>({...z,_line:E})));if(!S.length){x.log.push({text:`❌ Aucun joueur ${k}`,type:"info"}),b(x);return}ne(S,B,`Choisir ${B} joueur(s) ${k}(s) à exclure`,E=>{const L="usedCardIds_"+v,z=new Set(x[L]||[]);E.forEach(M=>{const F=(w[M._line]||[]).find(V=>V.cardId===M.cardId);F&&(F.used=!0,z.add(M.cardId),x.log.push({text:`❌ ${F.name} exclu !`,type:"info"}))}),x[L]=[...z],x[v+"Team"]=w,b(x)})},REVIVE_PLAYER:({count:B=1,roles:$=[]},u,x)=>{const b=u[s+"Team"],v=Object.entries(b).filter(([w])=>!$.length||$.includes(w)).flatMap(([w,k])=>k.filter(S=>S.used).map(S=>({...S,_line:w})));if(!v.length){u.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),x(u);return}ne(v,B,`Choisir ${B} joueur(s) à ressusciter`,w=>{w.forEach(k=>{const S=(b[k._line]||[]).find(E=>E.cardId===k.cardId);S&&(S.used=!1,u.log.push({text:`💫 ${S.name} ressuscité !`,type:"info"}))}),u[s+"Team"]=b,x(u)})},REMOVE_GOAL:({},B,$)=>{const u=n+"Score";B[u]>0?(B[u]--,B.log.push({text:"🚫 Dernier but annulé !",type:"info"})):B.log.push({text:"🚫 Aucun but à annuler",type:"info"}),$(B)},ADD_GOAL_DRAW:({},B,$)=>{B[s+"Score"]===B[n+"Score"]?(B[s+"Score"]++,B.log.push({text:"🎯 But bonus !",type:"info"})):B.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),$(B)},ADD_SUB:({value:B=1},$,u)=>{$.maxSubs=($.maxSubs||3)+B,$.log.push({text:`🔄 +${B} remplacement(s)`,type:"info"}),u($)},CUSTOM:({},B,$)=>$(B)};function ne(B,$,u,x){const b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let v=[];function w(){var S,E;const k=B.map(L=>{const z={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[L._line]||"#555",M=ze(L,L._line)+(L.boost||0),V=v.find(H=>H.cardId===L.cardId)?"#FFD700":"rgba(255,255,255,0.25)",W=L.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${L.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${V};background:${z};overflow:hidden;cursor:pointer;${W}">
      ...
      </div>`}function be(ue,X){return`<div id="pvp-boost-card"
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="width:min(90vw,420px)">${ct(m[n+"Team"],m[n+"Formation"],null,[],300,300)}</div>
        ...
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${H.map((ie,ee)=>{const ge=ee<H.length-1?nt(ie,H[ee+1]):null,be=!ge||ge==="#ff3333"||ge==="#cc2222",ye=ge==="#00ff88"?"+2":ge==="#FFD700"?"+1":"";return`<div class="duel-card duel-card-${Y}" data-idx="${ee}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease,transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
              ${Xe({...ie,note:ze(ie,"MIL"),_line:"MIL"},58,78)}
            </div>
            ${ee<H.length-1?`<div class="duel-link duel-link-${Y}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${be?"rgba(255,255,255,0.12)":ge};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${be?"none":`0 0 8px ${ge}`}">
              ${ye?`<span style="position:absolute;top:-13px;left:50%;transform:translateX(-50%);font-size:8px;font-weight:900;color:${ge}">${ye}</span>`:""}
            </div>`:""}`}).join("")}
        </div>
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div id="duel-shock" style="position:absolute;left:50%;top:50%;width:120px;height:120px;border-radius:50%;border:6px solid #FFD700;opacity:0;pointer-events:none"></div>
      <div id="pvp-duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center,rgba(10,61,30,.4),rgba(10,61,30,.92))"></div>
    </div>`;const S=(H,P)=>e.querySelectorAll(H).forEach((Y,ie)=>{setTimeout(()=>{Y.style.opacity="1",Y.style.transform="translateY(0) scale(1)"},P+ie*90)});S(".duel-card-me",150),S(".duel-card-opp",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((H,P)=>setTimeout(()=>{H.style.opacity="1"},P*70)),900),setTimeout(()=>{const H=e.querySelector("#pvp-vs");H&&(H.style.opacity="1",H.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(P=>P.style.opacity="1")},1250);function E(H,P,Y){const ie=document.getElementById(H);if(!ie)return;const ee=performance.now(),ge=be=>{const ye=Math.min(1,(be-ee)/Y);ie.textContent=Math.round(P*(1-Math.pow(1-ye,3))),ye<1?requestAnimationFrame(ge):ie.textContent=P};requestAnimationFrame(ge)}setTimeout(()=>{E("pvp-score-me",b,800),E("pvp-score-opp",v,800)},1500);const L=m.p1Team.MIL||[],z=m.p2Team.MIL||[],M=u(L)+x(L),F=u(z)+x(z),V=M>=F?"p1":"p2";let W=m.boostValue;W==null&&(W=oi(),m.boostValue=W,m.boostOwner=V,m.boostUsed=!1),_e=!0,setTimeout(()=>{const H=e.querySelector("#duel-row-"+(w?"me":"opp")),P=e.querySelector("#duel-row-"+(w?"opp":"me")),Y=document.getElementById("pvp-score-me"),ie=document.getElementById("pvp-score-opp"),ee=w?Y:ie,ge=w?ie:Y;ee&&(ee.style.fontSize="80px",ee.style.color=w?"#FFD700":"#ff6b6b",ee.style.animation="duelPulse .5s ease"+(w?",duelGlow 1.5s ease infinite .5s":"")),ge&&(ge.style.opacity="0.25"),setTimeout(()=>{H&&(H.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",H.style.zIndex="5"),setTimeout(()=>{const be=document.getElementById("duel-shock");be&&(be.style.animation="shockwave .5s ease-out forwards"),P&&(P.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var Be;const be=document.getElementById("pvp-duel-finale");if(!be)return;const ye=m.boostOwner===s?'<div style="background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:3px solid #cdeffd;border-radius:18px;padding:20px 34px;text-align:center;animation:boostFlipIn .7s cubic-bezier(.34,1.56,.64,1) both;box-shadow:0 10px 36px rgba(135,206,235,.5)"><div style="font-size:10px;color:rgba(0,0,0,.6);letter-spacing:2px;text-transform:uppercase;margin-bottom:6px;font-weight:700">Carte Boost obtenue</div><div style="font-size:46px;line-height:1">⚡</div><div style="font-size:50px;font-weight:900;color:#063;line-height:1.1">+'+W+`</div><div style="font-size:10px;color:rgba(0,0,0,.55);margin-top:4px">Applicable sur n'importe quel joueur</div></div>`:"",Ee=s==="p1"?'<button id="pvp-start-match" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">▶ Commencer le match</button>':`<div style="font-size:14px;color:rgba(255,255,255,0.5);text-align:center;margin-top:8px;animation:fadeUp .4s ease both">⏳ En attente de l'adversaire...</div>`;be.innerHTML='<div style="font-size:22px;font-weight:900;color:#fff;text-align:center;animation:fadeUp .4s ease both;text-shadow:0 2px 12px rgba(0,0,0,.5)">'+(w?"⚽ "+m[s+"Name"]+"<br>gagne le milieu et attaque !":"😔 "+m[n+"Name"]+"<br>gagne l'engagement et attaque !")+"</div>"+ye+Ee,be.style.transition="opacity .45s ease",be.style.opacity="1",be.style.pointerEvents="auto",(Be=document.getElementById("pvp-start-match"))==null||Be.addEventListener("click",async()=>{const Fe=V;await K({phase:Fe+"-attack",attacker:Fe,round:1,boostValue:W,boostUsed:!1,boostOwner:Fe})})},600)},700)},2800)}function we(B,$,u,x,b){const v=document.createElement("div");v.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:900;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;overflow:hidden;cursor:pointer";const w=Array.from({length:10},(L,z)=>`<div style="position:absolute;font-size:${16+Math.floor(Math.random()*24)}px;top:${5+Math.floor(Math.random()*65)}%;left:${3+Math.floor(Math.random()*94)}%;animation:fw${z%2===0?"A":"B"} ${.7+Math.random()*.7}s ease ${Math.random()*.9}s both;opacity:0">${["✨","🌟","⭐","💥","🎇","🎆","🔥","🌈"][z%8]}</div>`).join(""),k={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};v.innerHTML=`
      <style>
        @keyframes butPop{0%{transform:scale(0) rotate(-8deg);opacity:0}55%{transform:scale(1.25) rotate(2deg)}85%{transform:scale(0.92) rotate(-1deg)}100%{transform:scale(1);opacity:1}}
        @keyframes ballIn{0%{transform:translate(-70px,18px);opacity:0}65%{opacity:1}100%{transform:translate(26px,-8px);opacity:1}}
        @keyframes scoreIn{from{opacity:0;transform:translateY(-12px)}to{opacity:1;transform:translateY(0)}}
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(M);let F=!1;const V=()=>{F||(F=!0,M.remove(),setTimeout(()=>u&&u(),50))};M.addEventListener("click",V),setTimeout(V,3e3)}function ce(B,$){var M,F,V,W;const x=(m["gcCardsFull_"+s]||[]).find(H=>H.id===B),b=x==null?void 0:x._gcDef,v={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[b==null?void 0:b.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",w={purple:"#b06ce0",light_blue:"#00d4ef"}[b==null?void 0:b.color]||"#b06ce0",k=(b==null?void 0:b.name)||$,S=(b==null?void 0:b.effect)||((M=Oe[$])==null?void 0:M.desc)||"Carte spéciale.",E=b!=null&&b.image_url?`/icons/${b.image_url}`:null,L=((F=Oe[$])==null?void 0:F.icon)||"⚡",z=document.createElement("div");z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",z.innerHTML=`
      ...
      <div style="display:flex;gap:12px;width:190px">
        <button id="pvp-gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
        <button id="pvp-gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
      </div>`,document.body.appendChild(z),(V=z.querySelector("#pvp-gc-back"))==null||V.addEventListener("click",()=>z.remove()),(W=z.querySelector("#pvp-gc-use"))==null||W.addEventListener("click",()=>{z.remove(),Q(B,$)})}function me(){var x;const B=m[s+"Team"],$=Object.entries(B).flatMap(([b,v])=>(v||[]).filter(w=>!w.used).map(w=>({...w,_line:b})));if(!$.length)return;const u=document.createElement("div");u.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",u.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
      ...
      </div>`,document.body.appendChild(u),(x=u.querySelector("#bp-close"))==null||x.addEventListener("click",()=>u.remove()),u.querySelectorAll(".bp-item").forEach(b=>{b.addEventListener("click",async()=>{const v=b.dataset.cid,w=$.find(S=>S.cardId===v);if(!w)return;const k=(B[w._line]||[]).find(S=>S.cardId===v);k&&(k.boost=(k.boost||0)+m.boostValue),u.remove(),await K({[s+"Team"]:B,boostUsed:!0})})})}function pe(B=null){var F,V;if(!(m.phase===s+"-attack")){p("Remplacement uniquement avant votre attaque","warning");return}const u=m[s+"Team"],x=m["usedSubIds_"+s]||[],b=m.maxSubs||3;if(x.length>=b){p(`Maximum ${b} remplacements atteint`,"warning");return}const v=Object.entries(u).flatMap(([W,H])=>(H||[]).filter(P=>P.used).map(P=>({...P,_line:W}))),w=(m[s+"Subs"]||[]).filter(W=>!x.includes(W.cardId));if(!v.length){p("Aucun joueur utilisé à remplacer","warning");return}if(!w.length){p("Aucun remplaçant disponible","warning");return}let k=Math.max(0,v.findIndex(W=>W.cardId===B));const S=((F=v[k])==null?void 0:F._line)||((V=v[k])==null?void 0:V.job);let E=Math.max(0,w.findIndex(W=>W.job===S)),L=!1;const z=document.createElement("div");z.id="pvp-sub-overlay",z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function M(){var ge,be,ye,Ee,Be,Fe;const W=v[k],H=w[E],P=Math.min(130,Math.round((window.innerWidth-90)/2)),Y=Math.round(P*1.35),ie=je=>`background:rgba(255,255,255,0.12);border:none;color:${je?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${je?"default":"pointer"};flex-shrink:0`;z.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      </div>`,document.body.appendChild($),(x=$.querySelector("#pvp-hist-close"))==null||x.addEventListener("click",()=>$.remove())}async function de(){if(m.phase!==s+"-attack")return;const B=s==="p1"?"p2":"p1",$=(m.round||0)+1,u=[...m.log||[]];u.push({type:"info",text:`⏭️ ${m[s+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const x=ke(m),b=Ae(B),v=x||!b?"finished":B+"-attack";await K({["selected_"+s]:[],modifiers:{...m.modifiers,[s]:{}},pendingAttack:null,phase:v,attacker:B,round:$,log:u}),v==="finished"&&await Ge(m)}async function fe(){const B=m[s+"Team"],$=!["GK","DEF","MIL","ATT"].some(v=>(m[n+"Team"][v]||[]).some(w=>!w.used)),u=(m["selected_"+s]||[]).map(v=>{const w=(B[v._role]||[]).find(M=>M.cardId===v.cardId)||v,k=$&&["GK","DEF"].includes(v._role),S=B[v._role]||[],E=S.findIndex(M=>M.cardId===v.cardId),L=dt(S.length),z=E>=0?L[E]:w._col??1;return{...w,_line:v._role,_col:z,...k?{note_a:Math.max(1,Number(w.note_a)||0)}:{}}});if(!u.length)return;const x=Ht(u,m.modifiers[s]||{});N(s,u.map(v=>v.cardId)),u.forEach(v=>{const w=(B[v._role]||[]).find(k=>k.cardId===v.cardId);w&&(w.used=!0)}),m["selected_"+s]=[],oe();const b=[...m.log||[]];if($){const v=(m[s+"Score"]||0)+1,w=u.map(z=>({name:z.name,note:ze(z,z._line||"ATT"),portrait:He(z),job:z.job}));b.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:w,homeTotal:x.total,aiTotal:0});const k=(m.round||0)+1,S=s==="p1"?"p2":"p1",E={...m,[s+"Team"]:B,[s+"Score"]:v},L=ke(E);j.add(k),we(w,v,m[n+"Score"]||0,!0,async()=>{await K({[s+"Team"]:B,[s+"Score"]:v,["selected_"+s]:[],modifiers:{...m.modifiers,[s]:{}},pendingAttack:null,phase:L?"finished":S+"-attack",attacker:S,round:k,log:b}),L&&await Ge({...m,[s+"Score"]:v})});return}b.push({type:"pending",text:`⚔️ ${m[s+"Name"]} attaque (${x.total})`}),await K({[s+"Team"]:B,[n+"Team"]:m[n+"Team"],pendingAttack:{...x,players:u,side:s},["selected_"+s]:[],modifiers:{...m.modifiers,[s]:{}},phase:n+"-defense",log:b})}async function te(){const B=m[s+"Team"],$=(m["selected_"+s]||[]).map(W=>{const H=(B[W._role]||[]).find(ge=>ge.cardId===W.cardId)||W,P=B[W._role]||[],Y=P.findIndex(ge=>ge.cardId===W.cardId),ie=dt(P.length),ee=Y>=0?ie[Y]:H._col??1;return{...H,_line:W._role,_col:ee}}),u=Ut($,m.modifiers[s]||{});N(s,$.map(W=>W.cardId)),$.forEach(W=>{const H=(B[W._role]||[]).find(P=>P.cardId===W.cardId);H&&(H.used=!0)}),m["selected_"+s]=[],oe();const x=Kt(m.pendingAttack.total,u.total,m.modifiers[s]||{}),b=m.pendingAttack.side,v=x==="attack"||(x==null?void 0:x.goal),w=b==="p1"?"p2":"p1",k=(m.round||0)+1,S=(m.pendingAttack.players||[]).map(W=>({name:W.name,note:ze(W,W._line||"ATT"),portrait:He(W),job:W.job})),E=[...m.log||[]];E.push({type:"duel",isGoal:v,homeScored:v&&b===s,text:v?`⚽ BUT de ${m[b+"Name"]} ! (${m.pendingAttack.total} vs ${u.total})`:`✋ Attaque stoppée (${m.pendingAttack.total} vs ${u.total})`,homePlayers:S,aiPlayers:$.map(W=>({name:W.name,note:ze(W,W._line||"DEF"),portrait:He(W),job:W.job})),homeTotal:m.pendingAttack.total,aiTotal:u.total});const L=v?(m[b+"Score"]||0)+1:m[b+"Score"]||0,z={...m,[s+"Team"]:B,[b+"Score"]:L},M=ke(z),F=M?"finished":w+"-attack",V=async()=>{await K({[s+"Team"]:B,[n+"Team"]:m[n+"Team"],[b+"Score"]:L,["selected_"+s]:[],modifiers:{...m.modifiers,[s]:{}},pendingAttack:null,phase:F,attacker:w,round:k,log:E}),(F==="finished"||M)&&await Ge({...m,[b+"Score"]:L})};if(v){const W=b===s,H=W?L:m[s+"Score"]||0,P=W?m[n+"Score"]||0:L;j.add(k),we(S,H,P,W,V)}else await V()}function le(B){return["MIL","ATT"].some($=>(B[$]||[]).some(u=>!u.used))}function Le(B){return["GK","DEF","MIL","ATT"].some($=>(B[$]||[]).some(u=>!u.used))}function Te(B){return Le(B)&&!le(B)}function Ae(B){const $=m[B+"Team"],u=m[(B==="p1"?"p2":"p1")+"Team"];return!!(le($)||!Le(u)&&Te($))}function ke(B){const $=["MIL","ATT"].some(E=>(B.p1Team[E]||[]).some(L=>!L.used)),u=["MIL","ATT"].some(E=>(B.p2Team[E]||[]).some(L=>!L.used)),x=Le(B.p1Team),b=Le(B.p2Team);return!$&&!(!b&&x)&&(!u&&!(!x&&b))}function qe(B){const $=B.p1Score||0,u=B.p2Score||0;return $===u?null:$>u?y.home_id:y.away_id}async function Ge(B){try{const $=qe(B),u=$?y.home_id===$?y.away_id:y.home_id:null;await T.from("matches").update({status:"finished",winner_id:$,home_score:B.p1Score||0,away_score:B.p2Score||0}).eq("id",i),$&&u&&Ei($,u).catch(()=>{})}catch($){console.error("[PvP] finishMatch:",$)}}function Se(){var b;const B=m[s+"Score"],$=m[n+"Score"],u=B>$,x=B===$;et(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      ...
      <button id="pvp-home" style="padding:16px 40px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">🏠 Retour</button>
    </div>`,(b=document.getElementById("pvp-home"))==null||b.addEventListener("click",()=>{try{T.removeChannel(R)}catch{}Ve(e),c("home")})}oe()}const It="#1A6B3C",yt="#D4A017";async function Nn(e,t){var o;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const i=(o=t.state.params)==null?void 0:o.openLeagueId;if(i){t.state.params.openLeagueId=null,await wt(e,t,i);return}await St(e,t)}async function St(e,t,i="waiting"){var A;const{state:o}=t,d=o.profile.id,{data:a}=await T.from("mini_league_members").select("league_id").eq("user_id",d),r=(a||[]).map(g=>g.league_id),{data:c,error:p}=await T.from("mini_leagues").select("*, mini_league_members(count)").eq("status","waiting").eq("is_archived",!1).order("created_at",{ascending:!1}).limit(30),s=p?(await T.from("mini_leagues").select("*, mini_league_members(count)").eq("status","waiting").order("created_at",{ascending:!1}).limit(30)).data||[]:c||[],h=(r.length?await T.from("mini_leagues").select("*, mini_league_members(count)").in("id",r).order("created_at",{ascending:!1}):{data:[]}).data||[],f=h.filter(g=>g.status==="waiting"&&!g.is_archived),y=h.filter(g=>g.status==="active"&&!g.is_archived),_=h.filter(g=>g.is_archived||g.status==="finished"),m=s.filter(g=>!r.includes(g.id)),l=[{key:"waiting",label:"🟡 En attente",count:f.length+m.length},{key:"active",label:"🟢 En cours",count:y.length},{key:"archived",label:"📁 Archivées",count:_.length}];e.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:10px">
      ${i==="waiting"?Rn(f,m,d):i==="active"?On(y,d):Hn(_,d)}
    </div>
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="background:linear-gradient(160deg,#1a0a2e,#3b1a6e);border-radius:20px;padding:32px 24px;max-width:320px;width:100%;text-align:center;color:#fff;box-shadow:0 8px 40px rgba(0,0,0,0.6)">
    ...
    </div>`,document.body.appendChild(h),(f=h.querySelector("#prize-close"))==null||f.addEventListener("click",()=>h.remove()),h.addEventListener("click",_=>{_.target===h&&h.remove()}),(y=h.querySelector("#claim-prize-btn"))==null||y.addEventListener("click",async()=>{const{data:_}=await T.from("users").select("credits").eq("id",a.profile.id).single();await T.from("users").update({credits:((_==null?void 0:_.credits)||0)+s}).eq("id",a.profile.id),await T.from("mini_league_members").update({prize_claimed:!0}).eq("league_id",i.id).eq("user_id",a.profile.id),a.profile&&(a.profile.credits=((_==null?void 0:_.credits)||0)+s);const m=document.getElementById("nav-credits");m&&(m.textContent=`💰 ${a.profile.credits.toLocaleString("fr")}`),r(`💰 +${s.toLocaleString("fr")} cr. ajoutés à ton solde !`,"success"),h.remove(),wt(e,t,i.id)})}function Qn(e,t){const o=e.length%2===0?[...e]:[...e,null],d=o.length;let a=o.slice(1);const r=[];for(let c=0;c<d-1;c++){const p=[],s=[o[0],...a];for(let n=0;n<d/2;n++){const h=s[n],f=s[d-1-n];h===null?p.push({bye:f}):f===null?p.push({bye:h}):p.push({home:h,away:f})}r.push(p),a=[a[a.length-1],...a.slice(0,-1)]}return t==="aller-retour"?[...r,...r.map(c=>c.map(p=>p.bye?p:{home:p.away,away:p.home}))]:r}function uo(e,t){const i={};return e.forEach(o=>{i[o.id]={userId:o.id,pseudo:o.pseudo,clubName:o.club_name||o.pseudo,played:0,won:0,drawn:0,lost:0,goalsFor:0,goalsAgainst:0,goalDiff:0,points:0}}),t.filter(o=>o.status==="finished"&&!o.is_bye&&o.home_score!=null).forEach(o=>{const d=i[o.home_id],a=i[o.away_id];!d||!a||(d.played++,a.played++,d.goalsFor+=o.home_score,d.goalsAgainst+=o.away_score,a.goalsFor+=o.away_score,a.goalsAgainst+=o.home_score,o.home_score>o.away_score?(d.won++,d.points+=3,a.lost++):o.home_score<o.away_score?(a.won++,a.points+=3,d.lost++):(d.drawn++,d.points++,a.drawn++,a.points++),d.goalDiff=d.goalsFor-d.goalsAgainst,a.goalDiff=a.goalsFor-a.goalsAgainst)}),Object.values(i).sort((o,d)=>d.points-o.points||d.goalDiff-o.goalDiff||d.goalsFor-o.goalsFor)}async function Ni(e,t,i,o){var d,a;t.state.params={...t.state.params,matchMode:"mini-league",mlMatchId:i,leagueId:o};try{const r=(a=(d=t==null?void 0:t.state)==null?void 0:d.profile)==null?void 0:a.id;try{(T.getChannels?T.getChannels():[]).forEach(p=>{const s=p.topic||"";(s.includes("matchmaking")||s.includes("pvp-match"))&&T.removeChannel(p)})}catch(c){console.warn("[FriendMatch] cleanup canaux:",c)}r&&(await T.rpc("cancel_matchmaking",{p_user_id:r}).catch(()=>{}),await T.from("matchmaking_queue").delete().eq("user_id",r).then(()=>{},()=>{}))}catch{}await ri(e,t,"random",({deckId:r,formation:c,starters:p,subsRaw:s,gcCardsEnriched:n,gcDefs:h,stadiumDef:f})=>{const y=_=>Zn(e,t,r,c,p,s,_,h||[],i);if(!n.length){y([]);return}ni(e,n,y)})}async function Zn(e,t,i,o,d,a,r=[],c=[],p,s){const{state:n,navigate:h,toast:f}=t,y=n.profile.id;let _=!1,m=null;et(e);try{await T.rpc("cancel_matchmaking",{p_user_id:y})}catch{}try{await T.from("matchmaking_queue").delete().eq("user_id",y)}catch{}const{data:l}=await T.from("mini_league_matches").select("*, home:users!home_id(id,pseudo,club_name), away:users!away_id(id,pseudo,club_name)").eq("id",p).single();if(!l){f("Match introuvable","error"),Ve(e),h("mini-league");return}if(l.home_id!==y&&l.away_id!==y){f("Tu ne fais pas partie de ce match","error"),Ve(e),h("mini-league");return}const A=l.home_id===y,g=A?l.away:l.home,I=A?l.away_id:l.home_id,C=(D,q)=>{var R;e.innerHTML=`
      <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:20px;background:linear-gradient(180deg,#0a1628,#1a0a2e);padding:24px;text-align:center;color:#fff">
        ...
        <button id="lobby-cancel" style="padding:10px 24px;border-radius:10px;border:1.5px solid rgba(255,255,255,0.2);background:transparent;color:rgba(255,255,255,0.6);font-size:13px;cursor:pointer">Annuler</button>
        <style>@keyframes lobbyPulse{0%,100%{opacity:1}50%{opacity:0.4}}</style>
      </div>`,(R=document.getElementById("lobby-cancel"))==null||R.addEventListener("click",()=>{_=!0,clearInterval(m),Ve(e),h("mini-league")})},j=async(D,q)=>{_=!0,clearInterval(m),await new Promise(R=>setTimeout(R,600)),e.isConnected&&er(e,t,D,q,r,c)};if(C(),A){let D=l.match_id;if(!D){const{data:R,error:K}=await T.from("matches").insert({home_id:y,away_id:I,home_deck_id:i,away_deck_id:null,status:"active",mode:"mini_league"}).select().single();if(K||!R){f("Erreur création match","error"),Ve(e),h("mini-league");return}D=R.id,await T.from("mini_league_matches").update({match_id:D,status:"playing"}).eq("id",p)}const q=D;m=setInterval(async()=>{if(_){clearInterval(m);return}const{data:R}=await T.from("matches").select("away_deck_id").eq("id",q).single();R!=null&&R.away_deck_id&&(clearInterval(m),j(q,!0))},1500)}else{let D=l.match_id;m=setInterval(async()=>{if(_){clearInterval(m);return}const{data:q}=await T.from("mini_league_matches").select("match_id").eq("id",p).single();q!=null&&q.match_id&&(clearInterval(m),D=q.match_id,await T.from("matches").update({away_deck_id:i}).eq("id",D),j(D,!1))},1500)}}async function er(e,t,i,o,d=[],a=[]){var $;const{state:r,navigate:c,toast:p}=t,s=(($=r.params)==null?void 0:$.leagueId)||null,n=o?"p1":"p2",h=o?"p2":"p1",f=(d||[]).map(u=>u.id),y=(d||[]).map(u=>({id:u.id,gc_type:u.gc_type,_gcDef:u._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:_}=await T.from("matches").select("*").eq("id",i).single();if(!_){p("Match introuvable","error"),c("mini-league");return}async function m(){const[{data:u},{data:x},{data:b},{data:v}]=await Promise.all([T.rpc("get_deck_for_match",{p_deck_id:_.home_deck_id}),T.rpc("get_deck_for_match",{p_deck_id:_.away_deck_id}),T.from("users").select("id,pseudo,club_name").eq("id",_.home_id).single(),T.from("users").select("id,pseudo,club_name").eq("id",_.away_id).single()]),w=z=>{const M=Number(z.evolution_bonus)||0;return{cardId:z.card_id,position:z.position,id:z.id,firstname:z.firstname,name:z.surname_real,country_code:z.country_code,club_id:z.club_id,job:z.job,job2:z.job2,note_g:(Number(z.note_g)||0)+(z.job==="GK"||z.job2==="GK"&&Number(z.note_g)>0?M:0),note_d:(Number(z.note_d)||0)+(z.job==="DEF"||z.job2==="DEF"&&Number(z.note_d)>0?M:0),note_m:(Number(z.note_m)||0)+(z.job==="MIL"||z.job2==="MIL"&&Number(z.note_m)>0?M:0),note_a:(Number(z.note_a)||0)+(z.job==="ATT"||z.job2==="ATT"&&Number(z.note_a)>0?M:0),evolution_bonus:M,rarity:z.rarity,skin:z.skin,hair:z.hair,hair_length:z.hair_length,clubName:z.club_encoded_name||null,clubLogo:z.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},k=((u==null?void 0:u.starters)||[]).map(z=>w(z)),S=((x==null?void 0:x.starters)||[]).map(z=>w(z)),E=(u==null?void 0:u.formation)||"4-4-2",L=(x==null?void 0:x.formation)||"4-4-2";return{p1Team:(()=>{const z=vt(k,E);return stadiumDef?Tt(z,stadiumDef):z})(),p2Team:vt(S,L),p1Subs:(()=>{const z=((u==null?void 0:u.subs)||[]).map(M=>w(M));return stadiumDef&&jt(z,stadiumDef),z})(),p2Subs:((x==null?void 0:x.subs)||[]).map(z=>w(z)),p1Formation:E,p2Formation:L,p1Name:(b==null?void 0:b.club_name)||(b==null?void 0:b.pseudo)||"Joueur 1",p2Name:(v==null?void 0:v.club_name)||(v==null?void 0:v.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:o?f:[],gc_p2:o?[]:f,gcCardsFull_p1:o?y:[],gcCardsFull_p2:o?[]:y,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:a||[],lastActionAt:new Date().toISOString()}}let l=_.game_state&&Object.keys(_.game_state).length?_.game_state:null;if(!l)if(o){l=await m();const{data:u}=await T.from("matches").select("game_state").eq("id",i).single();!(u!=null&&u.game_state)||!Object.keys(u.game_state).length?await T.from("matches").update({game_state:l,turn_user_id:_.home_id}).eq("id",i):l=u.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let u=0;u<60&&!l;u++){await new Promise(b=>setTimeout(b,400));const{data:x}=await T.from("matches").select("game_state").eq("id",i).single();x!=null&&x.game_state&&Object.keys(x.game_state).length&&(l=x.game_state)}if(!l){p("Erreur de synchronisation","error"),c("mini-league");return}l.gc_p2=f,l.gcCardsFull_p2=y,await T.from("matches").update({game_state:l}).eq("id",i)}let A=!1,g=!1,I=!1,C=null,j=!1;const D=new Set,q=new Set;function R(u){var z,M;try{T.removeChannel(K)}catch{}const x=l[n+"Score"]||0,b=l[h+"Score"]||0;if(!I){I=!0;const F=u.winner_id||(x>b?r.profile.id:b>x?"opp":null),V=F===r.profile.id?"win":F?"loss":null;V&&ii(()=>Promise.resolve().then(()=>Zi),void 0).then(W=>W.applyOwnEvolution(r.profile.id,V)).catch(()=>{})}if(!g){const F=l.p1Score||0,V=l.p2Score||0,W=(l.usedGc_p1||[]).length,H=(l.usedGc_p2||[]).length,P=u.winner_id||(F>V?_.home_id:V>F?_.away_id:null);(P?r.profile.id===P:r.profile.id<(o?_.away_id:_.home_id))&&(g=!0,Ki({player1Id:_.home_id,player2Id:_.away_id,score1:F,score2:V,gc1:W,gc2:H}).catch(ie=>console.warn("[FriendMatch] updateStats:",ie)))}let v,w;u.winner_id?(v=u.winner_id===r.profile.id,w=!1):u.forfeit?(v=x>b,w=!1):(w=x===b,v=x>b),(z=document.getElementById("pvp-end-overlay"))==null||z.remove();const k=document.createElement("div");k.id="pvp-end-overlay",k.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const S=w?"🤝":v?"🏆":"😞",E=w?"MATCH NUL":v?"VICTOIRE !":"DÉFAITE",L=w?"#fff":v?"#FFD700":"#ff6b6b";k.innerHTML=`
      <div style="font-size:64px">${S}</div>
      <div style="font-size:26px;font-weight:900;color:${L}">${E}</div>
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">🏆 Retour à la Mini League</button>`,document.body.appendChild(k),(M=k.querySelector("#pvp-end-home"))==null||M.addEventListener("click",()=>{k.remove(),Ve(e),s?c("mini-league",{openLeagueId:s}):c("mini-league")})}const K=T.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},u=>{const x=u.new;try{if(x.status==="finished"||x.forfeit){if(A||(A=!0,C&&(clearInterval(C),C=null),x.game_state&&(l=x.game_state),l.phase==="finished"&&!x.forfeit&&document.getElementById("pvp-home")))return;R(x);return}if(x.game_state){const b=l;l=x.game_state;const v=l._lastGC;if(v&&v.seq&&!q.has(v.seq)&&(q.add(v.seq),v.by!==n)){ce(v.type,v.by,()=>xe());return}const w=b[n+"Score"]||0,k=b[h+"Score"]||0,S=l[n+"Score"]||0,E=l[h+"Score"]||0,L=S>w,z=E>k;if((L||z)&&!D.has(l.round)){D.add(l.round);const M=[...l.log||[]].reverse().find(V=>V.isGoal),F=((M==null?void 0:M.homePlayers)||[]).map(V=>({name:V.name,note:V.note,portrait:V.portrait,job:V.job}));Me(F,S,E,L,()=>xe());return}xe()}}catch(b){console.error("[PvP] crash:",b)}}).subscribe();async function J(u){Object.assign(l,u),l.lastActionAt=new Date().toISOString();const{error:x}=await T.from("matches").update({game_state:l}).eq("id",i);x&&p("Erreur de synchronisation","error");try{xe()}catch(b){console.error("[PvP] renderPvpScreen crash:",b)}}async function re(){if(A)return;A=!0,C&&(clearInterval(C),C=null);const u=o?_.away_id:_.home_id,x=o?"p2":"p1",b=o?"p1":"p2",v={...l,[x+"Score"]:3,[b+"Score"]:0,phase:"finished"};try{await T.from("matches").update({status:"finished",forfeit:!0,winner_id:u,home_score:v.p1Score||0,away_score:v.p2Score||0,game_state:v}).eq("id",i)}catch{}try{T.removeChannel(K)}catch{}setTimeout(()=>{Ve(e),c("mini-league")},800)}const ne={BOOST_STAT:({value:u=1,count:x=1,roles:b=[]},v,w)=>{const k=v[n+"Team"],S=Object.entries(k).filter(([E])=>!b.length||b.includes(E)).flatMap(([E,L])=>L.filter(z=>!z.used).map(z=>({...z,_line:E})));if(!S.length){v.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),w(v);return}Q(S,x,`Choisir ${x} joueur(s) à booster (+${u})`,E=>{E.forEach(L=>{const z=(k[L._line]||[]).find(M=>M.cardId===L.cardId);z&&(z.boost=(z.boost||0)+u,v.log.push({text:`⚡ +${u} sur ${z.name}`,type:"info"}))}),v[n+"Team"]=k,w(v)})},DEBUFF_STAT:({value:u=1,count:x=1,roles:b=[],target:v="ai"},w,k)=>{const S=v==="home"?n:h,E=w[S+"Team"],L=v==="home"?"allié":"adverse",z=Object.entries(E).filter(([M])=>!b.length||b.includes(M)).flatMap(([M,F])=>F.map(V=>({...V,_line:M})));if(!z.length){w.log.push({text:`🎯 Aucun joueur ${L}`,type:"info"}),k(w);return}Q(z,x,`Choisir ${x} joueur(s) ${L}(s) (-${u})`,M=>{M.forEach(F=>{const V=(E[F._line]||[]).find(W=>W.cardId===F.cardId);V&&(V.boost=(V.boost||0)-u,w.log.push({text:`🎯 -${u} sur ${V.name}`,type:"info"}))}),w[S+"Team"]=E,k(w)})},GRAY_PLAYER:({count:u=1,roles:x=[],target:b="ai"},v,w)=>{const k=b==="home"?n:h,S=v[k+"Team"],E=b==="home"?"allié":"adverse",L=Object.entries(S).filter(([z])=>!x.length||x.includes(z)).flatMap(([z,M])=>M.filter(F=>!F.used).map(F=>({...F,_line:z})));if(!L.length){v.log.push({text:`❌ Aucun joueur ${E}`,type:"info"}),w(v);return}Q(L,u,`Choisir ${u} joueur(s) ${E}(s) à exclure`,z=>{const M="usedCardIds_"+k,F=new Set(v[M]||[]);z.forEach(V=>{const W=(S[V._line]||[]).find(H=>H.cardId===V.cardId);W&&(W.used=!0,F.add(V.cardId),v.log.push({text:`❌ ${W.name} exclu !`,type:"info"}))}),v[M]=[...F],v[k+"Team"]=S,w(v)})},REVIVE_PLAYER:({count:u=1,roles:x=[]},b,v)=>{const w=b[n+"Team"],k=Object.entries(w).filter(([S])=>!x.length||x.includes(S)).flatMap(([S,E])=>E.filter(L=>L.used).map(L=>({...L,_line:S})));if(!k.length){b.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),v(b);return}Q(k,u,`Choisir ${u} joueur(s) à ressusciter`,S=>{S.forEach(E=>{const L=(w[E._line]||[]).find(z=>z.cardId===E.cardId);L&&(L.used=!1,b.log.push({text:`💫 ${L.name} ressuscité !`,type:"info"}))}),b[n+"Team"]=w,v(b)})},REMOVE_GOAL:({},u,x)=>{const b=h+"Score";u[b]>0?(u[b]--,u.log.push({text:"🚫 Dernier but annulé !",type:"info"})):u.log.push({text:"🚫 Aucun but à annuler",type:"info"}),x(u)},ADD_GOAL_DRAW:({},u,x)=>{u[n+"Score"]===u[h+"Score"]?(u[n+"Score"]++,u.log.push({text:"🎯 But bonus !",type:"info"})):u.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),x(u)},ADD_SUB:({value:u=1},x,b)=>{x.maxSubs=(x.maxSubs||3)+u,x.log.push({text:`🔄 +${u} remplacement(s)`,type:"info"}),b(x)},CUSTOM:({},u,x)=>x(u)};function Q(u,x,b,v){const w=document.createElement("div");w.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let k=[];function S(){var L,z;const E=u.map(M=>{const F={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[M._line]||"#555",V=ze(M,M._line)+(M.boost||0),H=k.find(Y=>Y.cardId===M.cardId)?"#FFD700":"rgba(255,255,255,0.25)",P=M.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${M.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${H};background:${F};overflow:hidden;cursor:pointer;${P}">
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${pt(u,l[n+"Formation"],Ue,z,300,300,be)}
      </div>
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="width:min(90vw,420px)">${ct(l[h+"Team"],l[h+"Formation"],null,[],300,300)}</div>
    </div>`,n==="p1"&&setTimeout(async()=>{await J({phase:"midfield"})},5e3)}let ve=!1;function we(){if(ve)return;const u=l[n+"Team"].MIL||[],x=l[h+"Team"].MIL||[];function b(Y){return Y.reduce((ie,ee)=>ie+ze(ee,"MIL"),0)}function v(Y){let ie=0;for(let ee=0;ee<Y.length-1;ee++){const ge=nt(Y[ee],Y[ee+1]);ge==="#00ff88"?ie+=2:ge==="#FFD700"&&(ie+=1)}return ie}const w=b(u)+v(u),k=b(x)+v(x),S=w>=k;function E(Y,ie,ee){return`<div id="duel-row-${ee}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
        ...
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${Y.map((ge,be)=>{const ye=be<Y.length-1?nt(ge,Y[be+1]):null,Ee=!ye||ye==="#ff3333"||ye==="#cc2222",Be=ye==="#00ff88"?"+2":ye==="#FFD700"?"+1":"";return`<div class="duel-card duel-card-${ee}" data-idx="${be}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease,transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
              ${Xe({...ge,note:ze(ge,"MIL"),_line:"MIL"},58,78)}
            </div>
            ${be<Y.length-1?`<div class="duel-link duel-link-${ee}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${Ee?"rgba(255,255,255,0.12)":ye};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${Ee?"none":`0 0 8px ${ye}`}">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      </div>`,document.body.appendChild(b),(v=b.querySelector("#bp-close"))==null||v.addEventListener("click",()=>b.remove()),b.querySelectorAll(".bp-item").forEach(w=>{w.addEventListener("click",async()=>{const k=w.dataset.cid,S=x.find(L=>L.cardId===k);if(!S)return;const E=(u[S._line]||[]).find(L=>L.cardId===k);E&&(E.boost=(E.boost||0)+l.boostValue),b.remove(),await J({[n+"Team"]:u,boostUsed:!0})})})}function U(u=null){var W,H;if(!(l.phase===n+"-attack")){p("Remplacement uniquement avant votre attaque","warning");return}const b=l[n+"Team"],v=l["usedSubIds_"+n]||[],w=l.maxSubs||3;if(v.length>=w){p(`Maximum ${w} remplacements atteint`,"warning");return}const k=Object.entries(b).flatMap(([P,Y])=>(Y||[]).filter(ie=>ie.used).map(ie=>({...ie,_line:P}))),S=(l[n+"Subs"]||[]).filter(P=>!v.includes(P.cardId));if(!k.length){p("Aucun joueur utilisé à remplacer","warning");return}if(!S.length){p("Aucun remplaçant disponible","warning");return}let E=Math.max(0,k.findIndex(P=>P.cardId===u));const L=((W=k[E])==null?void 0:W._line)||((H=k[E])==null?void 0:H.job);let z=Math.max(0,S.findIndex(P=>P.job===L)),M=!1;const F=document.createElement("div");F.id="pvp-sub-overlay",F.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function V(){var ye,Ee,Be,Fe,je,Ce;const P=k[E],Y=S[z],ie=Math.min(130,Math.round((window.innerWidth-90)/2)),ee=Math.round(ie*1.35),ge=Ie=>`background:rgba(255,255,255,0.12);border:none;color:${Ie?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Ie?"default":"pointer"};flex-shrink:0`;F.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
        <button id="psub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
      </div>`,(ye=F.querySelector("#psub-close"))==null||ye.addEventListener("click",()=>F.remove()),(Ee=F.querySelector("#pout-up"))==null||Ee.addEventListener("click",()=>{E>0&&(E--,V())}),(Be=F.querySelector("#pout-down"))==null||Be.addEventListener("click",()=>{E<k.length-1&&(E++,V())}),(Fe=F.querySelector("#pin-up"))==null||Fe.addEventListener("click",()=>{z>0&&(z--,V())}),(je=F.querySelector("#pin-down"))==null||je.addEventListener("click",()=>{z<S.length-1&&(z++,V())});const be=(Ie,Ne,Ue,Ye)=>{const Ke=F.querySelector("#"+Ie);if(!Ke)return;let Re=0;Ke.addEventListener("touchstart",Pe=>{Re=Pe.touches[0].clientY},{passive:!0}),Ke.addEventListener("touchend",Pe=>{const We=Pe.changedTouches[0].clientY-Re;if(Math.abs(We)<30)return;const ot=Ne();We<0&&ot<Ye-1?(Ue(ot+1),V()):We>0&&ot>0&&(Ue(ot-1),V())},{passive:!0})};be("pin-panel",()=>z,Ie=>z=Ie,S.length),be("pout-panel",()=>E,Ie=>E=Ie,k.length),(Ce=F.querySelector("#psub-confirm"))==null||Ce.addEventListener("click",async Ie=>{if(Ie.preventDefault(),Ie.stopPropagation(),M)return;M=!0;const Ne=k[E],Ue=S[z];if(!Ne||!Ue)return;const Ye=Ne._line,Ke=(b[Ye]||[]).findIndex(We=>We.cardId===Ne.cardId);if(Ke===-1){p("Erreur : joueur introuvable","error"),F.remove();return}const Re={...Ue,_line:Ye,position:Ne.position,used:!1,boost:0};b[Ye].splice(Ke,1,Re);const Pe=[...v,Ue.cardId];F.remove(),Z(Ne,Ue,async()=>{await J({[n+"Team"]:b,[h+"Team"]:l[h+"Team"],["usedSubIds_"+n]:Pe})})})}document.body.appendChild(F),V()}function Z(u,x,b){const v={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},w=document.createElement("div");w.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const k=(L,z,M)=>`<div style="text-align:center">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:6px">Appuyer pour continuer</div>`,document.body.appendChild(w);let S=!1;const E=()=>{S||(S=!0,w.remove(),setTimeout(()=>b(),50))};w.addEventListener("click",E),setTimeout(E,2200)}function se(){var x;const u=document.createElement("div");u.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:20px;overflow-y:auto",u.innerHTML=`
      <div style="font-size:12px;color:rgba(255,255,255,0.5);letter-spacing:2px;text-transform:uppercase">Équipe adverse</div>
      <div style="font-size:18px;font-weight:900;color:#ff6b6b">${l[h+"Name"]}</div>
      <div style="width:min(90vw,420px)">${ct(l[h+"Team"],l[h+"Formation"],null,[],300,300)}</div>
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(u),(x=u.querySelector("#pvp-opp-close"))==null||x.addEventListener("click",()=>u.remove())}function de(){var v;const u=l.log||[],x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column";const b=w=>{var S,E,L;if(w.type==="duel"){const z=w.isGoal,M=w.homeScored&&n==="p1"||!w.homeScored&&z&&n==="p2",F=w.homeScored?"#FFD700":z?"#ff6b6b":"rgba(255,255,255,0.3)",V=z?M?"⚽ BUT !":"⚽ BUT adversaire !":(S=w.homePlayers)!=null&&S.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${z?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${F};margin-bottom:4px">
  ...
  <div style="min-height:100%;background:${g[_.id]};padding:16px;overflow-y:auto;display:flex;flex-direction:column;gap:16px">
    ...
    <div style="display:flex;flex-direction:column;gap:10px;margin-top:auto;padding-top:8px">
      ...
      away:users!away_id(pseudo,club_name)`).or(`home_id.eq.${d.id},away_id.eq.${d.id}`).order("created_at",{ascending:!1}).limit(50),r={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},c=(a||[]).filter(n=>n.status==="finished"),p=(a||[]).filter(n=>n.status==="in_progress");function s(n){const h=n.home_id===d.id;h?n.home_score:n.away_score,h?n.away_score:n.home_score;const f=n.winner_id===d.id,y=n.home_score===n.away_score&&n.status==="finished",_=n.status!=="finished"?"…":y?"N":f?"V":"D",m=n.status!=="finished"||y?"#888":f?"#1A6B3C":"#c0392b";let l=r[n.mode]||n.mode;n.away_id===null&&!l.startsWith("IA")&&(l="IA");const g=n.home_id===d.id?n.away:n.home;let I;n.away_id===null?I=l:g?I=`${g.club_name||g.pseudo} (${g.pseudo})`:I="Adversaire";let C="";n.status==="in_progress"&&Date.now()-new Date(n.created_at).getTime()>3600*1e3&&(C=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const j=new Date(n.created_at),D=j.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+j.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),q=n.status==="finished"?`${n.home_score} - ${n.away_score}`:`${n.home_score||0} - ${n.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
