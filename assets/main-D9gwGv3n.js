...
import{s as v,j as ki,q as zo,t as Bt,u as $i,F as Ei,o as nt,n as Ht,g as Ct,r as ze,m as kt,v as gi,w as Mo,T as Ci,p as xn,x as bn,d as yn}from"./special-cards--9JnHrqo.js";const hn="/";function uo(t,{navigate:e,toast:i}){let o="login";const r=()=>{var a,s,d,c,l,f,b;const n=o==="login";if(t.innerHTML=`
    <div style="
  ...
  <div class="home-dark" id="home-dark">
    ...
    <div class="modal-body" style="padding:16px">
      ...
      </div>`}).join(""),d.querySelectorAll(".solo-level-card").forEach(c=>{c.dataset.locked!=="true"&&(c.addEventListener("mouseenter",()=>{c.style.transform="translateY(-1px)"}),c.addEventListener("mouseleave",()=>{c.style.transform=""}),c.addEventListener("click",()=>{o(),ft(),t("match",{matchMode:"solo",soloLevel:Number(c.dataset.level)})}))})}}const oi="#1A6B3C",ni="#cc2222",qn="#D4A017",go="#888";async function Dn(t,e){const{state:i,toast:o}=e;t.innerHTML=`
    ...
    <div style="display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:8px;padding:10px 0;border-bottom:1px solid #f0f0f0">
      ...
      <div style="text-align:left;font-size:18px;font-weight:900;color:${A}">${T}</div>
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    ${ze(t,{width:140})}
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),c=(s||[]).filter(C=>C.card_type==="player"&&C.player),l=(s||[]).filter(C=>C.card_type==="game_changer"),f=(s||[]).filter(C=>C.card_type==="formation"),b=(s||[]).filter(C=>C.card_type==="stadium"),{data:x}=await v.from("gc_definitions").select("name,gc_type,color,effect,image_url"),g={};(x||[]).forEach(C=>{g[C.name]=C});const{data:p}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),u={};(p||[]).forEach(C=>{u[C.id]=C}),b.forEach(C=>{C.stadium_def&&(u[C.stadium_id]=C.stadium_def)});const y=Object.keys(Ei),w=Object.keys(Me),h={};c.forEach(C=>{const V=C.player.id;h[V]=(h[V]||0)+1}),new Set(Object.keys(h).map(C=>String(C)));const E=new Set(f.map(C=>C.formation)),m=new Set(l.map(C=>C.gc_type));let T="player",$="Tous",A="",P=!1;function R(){return[...c].sort((C,V)=>{const Q=di.indexOf(C.player.job),X=di.indexOf(V.player.job);return Q!==X?Q-X:(C.player.surname_real||"").localeCompare(V.player.surname_real||"")})}function O(){return[...d||[]].sort((C,V)=>{const Q=di.indexOf(C.job),X=di.indexOf(V.job);return Q!==X?Q-X:(C.surname_real||"").localeCompare(V.surname_real||"")})}function Y(){return R().filter(C=>{const V=C.player,Q=$==="Tous"||V.job===$,X=!A||`${V.firstname} ${V.surname_real}`.toLowerCase().includes(A);return Q&&X})}function I(){return O().filter(C=>{const V=$==="Tous"||C.job===$,Q=!A||`${C.firstname} ${C.surname_real}`.toLowerCase().includes(A);return V&&Q})}t.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:0 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none;align-items:center"></div>
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{P=!P,_(),G()})))}function G(){const C=document.getElementById("col-grid");C&&(T==="player"?H(C):T==="formation"?pe(C):T==="stadium"?fe(C):me(C))}function K(C,V,Q,X,re,F=0){const z=document.getElementById("col-grid"),W=document.getElementById("col-big");if(!z||!W)return;var Z=0;function se(){var We;const ge=window.innerWidth>=768,ye=document.getElementById("col-big"),$e=document.getElementById("col-gap"),Ee=((We=document.getElementById("col-grid"))==null?void 0:We.parentElement)||null,be=ye?ye.closest(".page"):null;if(ye&&Ee&&be){let Le=0;Array.from(be.children).forEach(function(B){B!==ye&&B!==Ee&&B!==$e&&(Le+=B.offsetHeight)});const qe=Math.max(0,be.clientHeight-Le),De=Math.round(qe*(F/100)),Ge=Math.max(0,qe-De),k=ge?50/71:45/63,L=Math.round(Ge*k),M=Math.max(0,Ge-L);$e&&($e.style.height=De+"px"),ye.style.flex="none",ye.style.height=L+"px",ye.style.minHeight="0",Ee.style.flex="none",Ee.style.height=M+"px",Ee.style.minHeight="0",Ee.style.display="flex",Ee.style.overflow="hidden"}const xe=document.getElementById("col-grid");xe&&(xe.style.height="100%",xe.style.flexShrink="0",xe.style.overflowX="auto",xe.style.overflowY="hidden",xe.style.alignItems="center",xe.style.width="100%"),W.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:center center">'+V(C[Z])+"</div>";var Be=W.querySelector("[data-card-id],[data-form-id],[data-gc-id]");Be&&Be.addEventListener("click",function(){X(C[Z])}),requestAnimationFrame(function(){var Le=document.getElementById("big-card-inner");if(!(!Le||!W)){var qe=W.clientHeight,De=W.clientWidth-16,Ge=Le.offsetHeight,k=Le.offsetWidth;if(Ge>0&&k>0&&qe>40){var L=Math.min(qe/Ge,De/k);Le.style.transform="scale("+L.toFixed(3)+")",Le.style.transformOrigin="center center"}}}),z.innerHTML=C.map(function(Le,qe){var De=qe===Z,Ge="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+(De?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+qe+'" style="'+Ge+'">'+Q(Le,De)+"</div>"}).join(""),z.querySelectorAll(".col-mini-item").forEach(function(Le){Le.addEventListener("click",function(){Z=Number(Le.dataset.idx),se(),Le.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})}),requestAnimationFrame(function(){var Le=xe?xe.clientHeight:0,qe=z.querySelector(".col-mini-item > div");if(!(!Le||!qe)){var De=qe.style.zoom;qe.style.zoom="1";var Ge=qe.offsetHeight;if(qe.style.zoom=De,!(Ge<=0)){var k=Le/Ge;z.querySelectorAll(".col-mini-item > div").forEach(function(L){L.style.zoom=k.toFixed(4)})}}})}se()}function le(C,V){var Q=window.innerWidth>=768?.76:.54,X;if(!C||C._fake){var re=C?C.player:null;if(!re)return"";X=yo(re)}else X=Bi(C,"");var F=C&&!C._fake?h[C.player&&C.player.id]||1:0,z=F>0?'<div style="position:absolute;top:6px;right:6px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:11px;font-weight:900;padding:2px 7px;z-index:3;box-shadow:0 1px 4px rgba(0,0,0,0.4)">×'+F+"</div>":"";return'<div style="display:inline-block;position:relative;zoom:'+Q+';pointer-events:none;line-height:0">'+z+X+"</div>"}function ce(C,V,Q){var X=Q>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+Q+"</div>":"",re=!!V,F=gi(C,kt[C],{width:160});return re||(F='<div style="filter:grayscale(1);opacity:0.5">'+F+"</div>"),"<div "+(V?'data-form-id="'+V.id+'"':"")+' style="position:relative;cursor:pointer">'+X+F+"</div>"}function j(C,V){var Q=window.innerWidth>=768?.76:.54,X=gi(C,kt[C],{width:140});return V||(X='<div style="filter:grayscale(1);opacity:0.45">'+X+"</div>"),'<div style="display:inline-block;zoom:'+Q+';line-height:0;pointer-events:none">'+X+"</div>"}function H(C){if(P){const V=I();if(!V.length){C.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucun joueur.</div>';return}const Q=V.map(X=>c.find(re=>re.player.id===X.id)||{_fake:!0,player:X,id:"fake-"+X.id});K(Q,X=>X._fake?yo(X.player):Bi(X,""),X=>X._fake?le({player:X.player,id:"x",_fake:!0}):le(X),X=>{X._fake||ho(X,c,h,e)})}else{const V=Y();if(!V.length){C.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const Q={};V.forEach(re=>{const F=re.player.id;Q[F]||(Q[F]=[]),Q[F].push(re)});const X=Object.values(Q).map(re=>Kn(re));K(X,re=>{const F=h[re.player.id]||1,z=F>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${F}</div>`:"",Z=c.filter(se=>se.player.id===re.player.id&&se.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Bi(re,z+Z)},re=>le(re),re=>ho(re,c,h,e))}}function pe(C){const V=P?y:[...E];if(!V.length){C.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const Q=V.map(X=>({formation:X,card:f.find(re=>re.formation===X)||null,owned:E.has(X)}));K(Q,({formation:X,card:re,owned:F})=>ce(X,F?re:null,F?f.filter(z=>z.formation===X).length:0),({formation:X,owned:re})=>j(X,re),({card:X,owned:re})=>{re&&X&&Xn(X,f,e,n)},"#1A6B3C",5)}function me(C){const V=Object.keys(g),Q=P?V.length?V:w:[...m];if(!Q.length){C.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const X=Q.map(re=>({type:re,gc:Me[re]||{icon:"⚡",desc:""},def:g[re]||null,owned:m.has(re),card:l.find(F=>F.gc_type===re)||null}));K(X,({type:re,gc:F,def:z,owned:W,card:Z})=>{var xe;const se=(z==null?void 0:z.name)||re,ge=W?l.filter(Be=>Be.gc_type===re).length:0,ye=ge>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${ge}</div>`:"",$e=(z==null?void 0:z.effect)||F.desc||"",Ee=z!=null&&z.image_url?`/icons/${z.image_url}`:((xe=z==null?void 0:z.club)==null?void 0:xe.logo_url)||(z!=null&&z.country_code?`https://flagsapi.com/${z.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null);let be=nt(se,Ee,F.icon,$e,{width:160,onClick:W});return W||(be=`<div style="filter:grayscale(1);opacity:0.5">${be}</div>`),`<div ${W&&Z?`data-gc-id="${Z.id}" data-gc-type="${re}"`:""} style="position:relative">${ye}${be}</div>`},({type:re,gc:F,def:z,owned:W})=>{const Z=window.innerWidth>=768?.76:.54,se=(z==null?void 0:z.name)||re,ge=(z==null?void 0:z.effect)||F.desc||"",ye=z!=null&&z.image_url?`/icons/${z.image_url}`:null;let $e=nt(se,ye,F.icon,ge,{width:140});return W||($e=`<div style="filter:grayscale(1);opacity:0.45">${$e}</div>`),`<div style="display:inline-block;zoom:${Z};line-height:0;pointer-events:none">${$e}</div>`},({type:re,owned:F,def:z})=>{F&&Wn(re,z,n)},"#7a28b8",5)}function fe(C){const V="#4FC3F7",Q="/";if(!b.length){C.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Stade.<br><small>Ouvre un booster Stade !</small></div>';return}const X={};b.forEach(F=>{const z=F.stadium_id||"?";(X[z]=X[z]||[]).push(F)});const re=Object.entries(X).map(([F,z])=>({sid:F,def:u[F]||null,count:z.length,card:z[0]}));K(re,({def:F,count:z})=>{var $e,Ee;const W=(F==null?void 0:F.name)||"?",Z=(($e=F==null?void 0:F.club)==null?void 0:$e.encoded_name)||(F==null?void 0:F.country_code)||"—",se=F!=null&&F.image_url?`${Q}icons/${F.image_url}`:((Ee=F==null?void 0:F.club)==null?void 0:Ee.logo_url)||(F!=null&&F.country_code?`https://flagsapi.com/${F.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),ge=z>1?`<div style="position:absolute;top:8px;right:8px;background:#333;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${z}</div>`:"",ye=`${Z}<br>+10 ⭐ joueurs alliés`;return`<div style="position:relative">${ge}${Ht(W,se,ye,{width:160})}</div>`},({def:F})=>{var ye,$e;const z=window.innerWidth>=768?.76:.54,W=(F==null?void 0:F.name)||"?",Z=((ye=F==null?void 0:F.club)==null?void 0:ye.encoded_name)||(F==null?void 0:F.country_code)||"—",se=F!=null&&F.image_url?`${Q}icons/${F.image_url}`:(($e=F==null?void 0:F.club)==null?void 0:$e.logo_url)||(F!=null&&F.country_code?`https://flagsapi.com/${F.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),ge=`${Z}<br>+10 ⭐`;return`<div style="display:inline-block;zoom:${z};line-height:0;pointer-events:none">${Ht(W,se,ge,{width:140})}</div>`},null,V,5)}t.querySelectorAll(".col-tab-btn").forEach(C=>{C.addEventListener("click",()=>{T=C.dataset.tab,$="Tous",A="",P=!1,t.querySelectorAll(".col-tab-btn").forEach(V=>{const Q=V.dataset.tab===T;V.style.borderBottomColor=Q?"var(--green)":"transparent",V.style.color=Q?"var(--green)":"var(--tile-fg-dim)"}),_(),G()})}),_(),G()}function Wn(t,e,i){const o=Me[t]||{icon:"⚡",desc:"Effet spécial."},r=(e==null?void 0:e.name)||t,n=(e==null?void 0:e.effect)||o.desc,a=e!=null&&e.image_url?`/icons/${e.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
      ${nt(r,a,o.icon,n,{width:200})}
      <div style="background:#fff3cd;border-radius:10px;padding:10px 14px;width:100%">
        <div style="font-size:12px;color:#856404">⚠️ Cette carte est à <b>usage unique</b>. Une fois jouée en match, elle est définitivement supprimée de ta collection.</div>
      </div>
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const pi=1e3;function Xn(t,e,i,o){var g,p,u;const{state:r,toast:n,closeModal:a,navigate:s,refreshProfile:d}=i,c=t.formation,l={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function f(){const y=kt[c]||{},w=Ei[c]||[],h=290,E=360,m=20;function T(A){const P=y[A];return P?{x:P.x*h,y:P.y*E}:null}let $=`<svg width="${h}" height="${E}" viewBox="0 0 ${h} ${E}" xmlns="http://www.w3.org/2000/svg">`;for(const[A,P]of w){const R=T(A),O=T(P);!R||!O||($+=`<line x1="${R.x}" y1="${R.y}" x2="${O.x}" y2="${O.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const A of Object.keys(y)){const P=T(A);if(!P)continue;const R=A.replace(/\d+/,""),O=l[R]||"#555";$+=`<circle cx="${P.x}" cy="${P.y}" r="${m}" fill="${O}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,$+=`<text x="${P.x}" y="${P.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${R}</text>`}return $+="</svg>",$}const b=e.filter(y=>y.formation===c);b.length;const x=!t.is_for_sale;o(`Formation ${c}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    <div style="margin-top:16px;border-top:1px solid var(--tile-border);padding-top:14px">
        ...
        ${f.filter(z=>z.id!==t.id).map((z,W)=>{const Z=_[z.id]||[],se=z.is_for_sale,ge=Z.length?Z[Z.length-1]:null,ye=z.evolution_bonus||0,$e=ge?ge.source==="booster"?"Booster":ge.price?ge.price.toLocaleString("fr")+" cr.":"—":"—",Ee=ge?new Date(ge.transferred_at).toLocaleDateString("fr",{day:"2-digit",month:"2-digit",year:"numeric"}):"",be=80,xe=Math.round(be*657/507),Be=ze({...l,_evolution_bonus:ye},{width:be});return`
            <div class="exemplaire-row" data-card-id="${z.id}" data-card-idx="${W}"
              ...
              <input type="checkbox" class="expl-check"
                data-id="${z.id}" data-evo="${ye}" data-note="${ci(l,l.job)}"
                ${se?"disabled":""}
                style="display:none">
              <!-- Wrapper exactement aux dimensions de la carte -->
              <div class="expl-mini-card" style="position:relative;width:${be}px;height:${xe}px;border-radius:8px;overflow:hidden">
                ${Be}
      ...
      <!-- Carte visuelle -->
      ${ze({...l,_evolution_bonus:x},{width:160})}
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(V=document.getElementById("close-detail"))==null||V.addEventListener("click",s);let K=new Set;const le=l.rarity==="pepite"?1.3:l.rarity==="papyte"?.7:1;function ce(){let z=0;return document.querySelectorAll(".expl-check:checked").forEach(W=>{if(W.dataset.id===t.id)return;const se=Number(W.dataset.evo)||0,ge=Number(W.dataset.note)||0;z+=ge+se}),Math.round(z*le)}const j=()=>{const z=K.size,W=document.getElementById("sell-action-panel");if(!W)return;W.style.display=z>0?"block":"none",document.getElementById("sell-selected-count").textContent=z;const Z=document.getElementById("evolve-btn");if(Z){const se=ce();Z.textContent=se>0?`⬆️ Évoluer (+${se})`:"⬆️ Évoluer"}};document.querySelectorAll(".expl-check").forEach(z=>{z.addEventListener("change",()=>{const W=z.dataset.id;z.checked?K.add(W):K.delete(W);const Z=z.closest(".exemplaire-row");if(Z){const se=Z.querySelector(".expl-sel-overlay"),ge=Z.querySelector(".expl-sel-check");se&&(se.style.display=z.checked?"block":"none"),ge&&(ge.style.display=z.checked?"flex":"none"),Z.style.transform=z.checked?"scale(1.05)":"scale(1)"}j()})}),document.querySelectorAll(".exemplaire-row").forEach(z=>{z.addEventListener("click",W=>{if(W.target.tagName==="INPUT")return;const Z=z.querySelector(".expl-check");Z&&!Z.disabled&&(Z.checked=!Z.checked,Z.dispatchEvent(new Event("change")))})}),(Q=document.getElementById("evolve-btn"))==null||Q.addEventListener("click",async()=>{if(b<=1)return;const z=[...K];if(!z.length)return;if(K.has(t.id)){const be=document.createElement("div");be.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",be.innerHTML=`
        ...
        </div>`,document.body.appendChild(be),be.querySelector("#err-close").addEventListener("click",()=>be.remove()),be.addEventListener("click",xe=>{xe.target===be&&be.remove()});return}const W=z.filter(be=>be!==t.id);if(!W.length){n("Sélectionne des copies à sacrifier","warning");return}const Z=W.reduce((be,xe)=>{const Be=document.querySelector(`.expl-check[data-id="${xe}"]`),We=Be&&Number(Be.dataset.evo)||0,Le=Be&&Number(Be.dataset.note)||0;return be+Le+We},0),se=Math.round(Z*le),ge=l.rarity==="pepite"?" (+30% pépite ✨)":l.rarity==="papyte"?" (-30% papyte)":"";if(!await new Promise(be=>{const xe=document.createElement("div");xe.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",xe.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
        ...
        <div style="width:130px;flex-shrink:0;background:rgba(0,0,0,0.3);display:flex;flex-direction:column;align-items:center;padding:12px 8px;gap:12px;border-left:1px solid rgba(255,255,255,0.1)">
          ...
          <div style="width:100%;text-align:center">
            ...
            <div id="formation-pc-btn" style="cursor:pointer;margin:0 auto;width:fit-content">
              ${gi(e.formation,kt[e.formation],{width:100})}
            </div>
          </div>
          <!-- Stade -->
          <div style="width:100%;text-align:center">
    ...
    </div>`;u("⚽ Choisir une formation",w),document.querySelectorAll("#modal-body [data-forma]").forEach(h=>{h.addEventListener("click",()=>{e.formation=h.dataset.forma;const E=_o(e.formation),m={};E.forEach(T=>{e.slots[T]&&(m[T]=e.slots[T])}),e.slots=m,y(),$t(t,e,i)})})};t.querySelectorAll("#formation-mobile-btn, #formation-pc-btn").forEach(u=>u.addEventListener("click",x)),t.querySelectorAll("#add-stad-btn-pc, #add-stad-btn").forEach(u=>u.addEventListener("click",()=>Zn(e,t,i))),t.querySelectorAll("#save-deck, #save-deck-pc").forEach(u=>u.addEventListener("click",()=>ir(e,i))),t.querySelectorAll("[data-remove-sub]").forEach(u=>{u.addEventListener("click",()=>{e.subs=e.subs.filter(y=>y!==u.dataset.removeSub),$t(t,e,i)})}),t.querySelectorAll("#add-sub-btn").forEach(u=>u.addEventListener("click",()=>tr(e,t,i)))}function Qn(t,e,i,o){var h,E,m;const r=window.innerWidth>=900,n=t.querySelector(r?"#deck-field-pc":"#deck-field-mobile");if(!n)return;const a=(h=e.stadiumCards)==null?void 0:h.find(T=>T.id===e.stadiumCardId),s=a&&((E=e.stadDefMap)==null?void 0:E[a.stadium_id])||null,d=kt[e.formation]||{},c=$i(e.formation)||[],l={};for(const T of i){const $=e.slots[T],A=$?e.playerCards.find(P=>P.id===$):null;A!=null&&A.player?l[T]={...A.player,_evolution_bonus:A.evolution_bonus||0,face:A.player.face||null}:l[T]=null}const f=window.innerWidth>=900,b=f?window.innerWidth-280:window.innerWidth-20,x=f?Math.min(b,860):b,g=Math.round(f?x*.82:x*.85),p=f?84:44;let u="";for(const[T,$]of c){const A=d[T],P=d[$];if(!A||!P)continue;const R=A.x*x,O=Math.round(.03*g+A.y*.85*g),Y=P.x*x,I=Math.round(.03*g+P.y*.85*g),_=l[T],G=l[$],K=Bt(_,G);K==="#ff3333"||K==="#cc2222"?u+=`<line x1="${R.toFixed(1)}" y1="${O.toFixed(1)}" x2="${Y.toFixed(1)}" y2="${I.toFixed(1)}" stroke="${K}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(u+=`<line x1="${R.toFixed(1)}" y1="${O.toFixed(1)}" x2="${Y.toFixed(1)}" y2="${I.toFixed(1)}" stroke="${K}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,u+=`<line x1="${R.toFixed(1)}" y1="${O.toFixed(1)}" x2="${Y.toFixed(1)}" y2="${I.toFixed(1)}" stroke="${K}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let y="";const w=Math.round(p*657/507);for(const T of i){const $=d[T];if(!$)continue;const A=l[T],P=$.x*x;$.y*g;const R=Math.round(.03*g+$.y*(.85*g)),O=Math.round(P-p/2),Y=Math.round(R-w/2);if(A){const I=T.replace(/\d+/,""),_=s&&(s.club_id&&String(A.club_id)===String(s.club_id)||s.country_code&&A.country_code===s.country_code),G=ze({...A,_evolution_bonus:A._evolution_bonus||0},{width:p,showStad:!0,stadDef:s,role:I});_&&((m=s.club)!=null&&m.logo_url||s.image_url),y+=`<div style="position:absolute;left:${O}px;top:${Y}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${T}">
        <div style="position:relative">${G}</div>
      </div>`}else{const I=T.replace(/\d+/,"");y+=`<div style="position:absolute;left:${O}px;top:${Y}px;width:${p}px;height:${w}px;
    ...
    <div style="position:relative;width:${x}px;height:${g}px;margin:0 auto">
      ...
      ${y}
    </div>`,n.querySelectorAll(".deck-slot-hit").forEach(T=>{T.addEventListener("click",()=>er(T.dataset.pos,e,t,o))})}function Zn(t,e,i){var a;const{openModal:o,closeModal:r}=i,n=t.stadiumCards||[];o("🏟️ Choisir un stade",`<div style="display:flex;flex-wrap:wrap;gap:10px;padding:8px">
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    </div>`,requestAnimationFrame(()=>requestAnimationFrame(function Y(){const I=t.querySelector(".deck-preview-wrap"),_=t.querySelector("#deck-swipe-zone");if(!I||!_||!y)return;const G=_.clientWidth>=900,K=Math.max(200,_.clientHeight-(G?60:40)),ce=Math.max(200,_.clientWidth-16),j=G?Math.min(117,Math.max(52,Math.round(ce*.22))):Math.max(44,Math.round(ce*.168));if(K<220||ce<220){requestAnimationFrame(Y);return}const H=G?null:Math.round(j*.55);I.innerHTML=Ut(y,u,null,[],ce,K,[],H),I.style.cssText=`width:${ce}px;height:${K}px;overflow:hidden;margin:${G?0:60}px auto 0`;const pe=I.querySelector("svg");pe&&(pe.style.cssText="display:block;width:100%;height:100%",pe.setAttribute("preserveAspectRatio",G?"xMidYMid meet":"none"))})),(A=document.getElementById("prev-deck"))==null||A.addEventListener("click",()=>{l>0&&(l--,f())}),(P=document.getElementById("next-deck"))==null||P.addEventListener("click",()=>{l<n.length-1&&(l++,f())}),(R=document.getElementById("validate-deck"))==null||R.addEventListener("click",()=>{if(!h)return;const Y=e.state.params||{};e.navigate("match",{...Y,matchMode:Y.matchMode||i,deckId:b.id})}),(O=document.getElementById("cancel-deck-select"))==null||O.addEventListener("click",()=>{Ve(t),r("home")});const E=document.getElementById("deck-swipe-zone");if(E){let Y=0,I=0;E.addEventListener("touchstart",_=>{Y=_.touches[0].clientX,I=_.touches[0].clientY},{passive:!0}),E.addEventListener("touchend",_=>{const G=_.changedTouches[0].clientX-Y,K=_.changedTouches[0].clientY-I;Math.abs(G)<40||Math.abs(G)<Math.abs(K)||(G<0&&l<n.length-1?(l++,f()):G>0&&l>0&&(l--,f()))},{passive:!0})}}f()}function _t(t,e=44,i=58,o=null){return ze(t,{width:e,showStad:!!o,stadDef:o,used:t==null?void 0:t.used})}function et(t,e,i,o,r){if(!(t!=null&&t.length))return"";const n=t.slice(0,5);let a='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return n.forEach((s,d)=>{const c=s._line||s.job||"MIL";let l=s.boost||0;if(s.stadiumBonus&&(o==="attack"&&(c==="ATT"||c==="MIL")||o==="defense"&&(c==="GK"||c==="DEF"||c==="MIL")?l+=10:o||(l+=10)),a+=ze({...s,_evolution_bonus:0,evolution_bonus:0},{width:40,role:c,extraNote:l}),d<n.length-1){const f=n[d+1],b=Ho(s,f,r)?Bt(s,f):null;a+=`<div style="width:7px;height:3px;background:${!b||b==="#ff3333"||b==="#cc2222"?"rgba(255,255,255,0.12)":b};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(a+=`<div style="margin-left:6px;background:${e};color:${e==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),a+="</div>",a}function Ti(t,e,i,o,r=310,n=310,a=[],s=null){const d=kt[e]||{},c=$i(e)||Ei[e]||[],l={},f=["ATT","MIL","DEF","GK"];for(const y of f)(t[y]||[]).forEach((h,E)=>{l[`${y}${E+1}`]=h});function b(y){const w=d[y];return w?{x:w.x*r,y:w.y*n}:null}let x="";for(const[y,w]of c){const h=b(y),E=b(w);if(!h||!E)continue;const m=l[y],T=l[w],$=Bt(m,T);$==="#00ff88"||$==="#FFD700"?(x+=`<line x1="${h.x.toFixed(1)}" y1="${h.y.toFixed(1)}" x2="${E.x.toFixed(1)}" y2="${E.y.toFixed(1)}"
    ...
    </div>`}function on(t){var n,a,s;if(!t)return"";const e=d=>d?ze({...d,_evolution_bonus:0},{width:52,role:d._line||d.job,showStad:!!d.stadiumBonus,extraNote:d.boost||0}):"",o={goal:"⚽","goal-home":"⚽","goal-ai":"⚽",duel:"⚔️",midfield:"🎯",sub:"🔄",gc:"⚡",boost:"💥","defense-won":"🛡️","attack-won":"⚔️","defense-lost":"😓","attack-lost":"😓"}[t.type]||"📋";return`
    ...
    display:flex;flex-direction:column;align-items:center;justify-content:center;
    background:rgba(0,0,0,0.85);pointer-events:none`;const s=(t||[]).slice(0,3).map(d=>ze({...d,_evolution_bonus:0},{width:Math.min(140,Math.round(window.innerWidth/4)),role:d._line||d.job,showStad:!!d.stadiumBonus})).join("");if(a.innerHTML=`
    ...
    pointer-events:none;white-space:nowrap;max-width:90vw;text-align:center`,r.textContent=t,!document.getElementById("game-toast-style")){const n=document.createElement("style");n.id="game-toast-style",n.textContent=`
      ...
      }`,document.head.appendChild(n)}document.body.appendChild(r),setTimeout(()=>{r.style.opacity="0",r.style.transition="opacity 0.3s",setTimeout(()=>r.remove(),300)},i)}function nn(t,e){const i=Ne(t,"MIL"),o=t.stadiumBonus||e&&(e.club_id&&String(t.club_id)===String(e.club_id)||e.country_code&&t.country_code===e.country_code)?10:0;return i+o}function ht(t,e){return t.reduce((i,o)=>i+nn(o,e),0)}function vt(t){let e=0;for(let i=0;i<t.length-1;i++){const o=Bt(t[i],t[i+1]);o==="#00ff88"?e+=10:o==="#FFD700"&&(e+=5)}return e}function vi(t,e,i,o,r){return`<div id="duel-row-${o}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0), opacity .5s ease;transform-origin:center">
      ...
      <div class="duel-score-line duel-score-line-${o}" style="margin-top:10px;font-size:12px;color:rgba(255,255,255,0.55);opacity:0;transition:opacity .4s ease">
        Score: ${ht(t,r)} + ${vt(t)} liens = <b style="color:#fff">${ht(t,r)+vt(t)}</b>
      </div>
    </div>`}async function _r(t,e){const{state:i}=e,o=i.params||{},r=o.matchMode||"vs_ai_easy",n=r==="solo",a=r==="ranked_ai",s=o.soloLevel||1,d=r.replace("vs_ai_",""),c=r;let l=null;if(n){const{data:g}=await v.from("solo_levels").select("*").eq("level_number",s).maybeSingle();l=g||{level_number:s,target_note_avg:10,nb_liens_jaune:2,nb_liens_vert:1,nb_joueurs_stade:2,reward_credits:500}}const f=o.rankedData||null,b=o.presetSetup||null;async function x({deckId:g,formation:p,starters:u,subsRaw:y,gcCardsEnriched:w,gcDefs:h,stadiumDef:E}){try{let m=hi(u,p);E&&(m=si(m,E),yi(y,E));let T=null;if(a){const R=$r(m),{jaune:O,vert:Y}=Er(m,p),I=.9+Math.random()*.2;T={target_note_avg:Math.max(1,Math.round(R*I/16)),nb_liens_jaune:O,nb_liens_vert:Y,nb_joueurs_stade:E?3:0}}const $=n?await $o(p,l):a?await $o(p,T):await kr(p,d),A=$.lines||$,P=async R=>{try{const O=c==="vs_ai_club"||c==="solo"||c==="ranked_ai"?"club":c,{data:Y,error:I}=await v.from("matches").insert({home_id:i.profile.id,away_id:null,mode:O,home_deck_id:g,status:"in_progress"}).select().single();if(I){console.error("[MatchIA] Erreur création match:",I),At(t,"⚠️","Impossible de créer le match ("+I.message+").","Retour",()=>e.navigate("home"));return}const _=$.stadiumDef||null;_&&A&&(si(A,_),yi($.subs||[],_));const G={gcDefs:h||[],matchId:Y==null?void 0:Y.id,mode:c,difficulty:d,formation:p,isSolo:n,soloLevel:s,soloLevelConfig:l,isRankedAI:a,rankedAIData:f,homeTeam:m,aiTeam:A,homeSubs:y,subsUsed:0,maxSubs:Math.min(y.length,3),aiSubs:$.subs||[],aiSubsUsed:0,aiMaxSubs:Math.min(($.subs||[]).length,3),aiUsedSubIds:[],aiGcCards:$.gcCards||[],aiUsedGc:[],aiStadiumDef:_,homeScore:0,aiScore:0,gcCards:R,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Lr(t,G,e)}catch(O){console.error("[MatchIA] Exception launchMatch:",O),At(t,"⚠️","Erreur au lancement du match : "+O.message,"Retour",()=>e.navigate("home"))}};if(a&&b){P(b.gcCardsEnriched||[]);return}if(!w.length){P([]);return}Ii(t,w,P)}catch(m){console.error("[MatchIA] Exception setup:",m),At(t,"⚠️","Erreur de préparation du match : "+m.message,"Retour",()=>e.navigate("home"))}}b?await x(b):await Si(t,e,r,x)}async function kr(t,e){var u;const{data:i}=await v.from("players").select("id,firstname,surname_real,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(80);if(!i||i.length<11)return{lines:rn(t),subs:[],gcCards:[],stadiumDef:null};const o=Et[t]||Et["4-4-2"],r={GK:[],DEF:[],MIL:[],ATT:[]},n=new Set;function a(y,w,h){var E,m;return n.add(y.id),{cardId:"ai-"+y.id+"-"+h,id:y.id,firstname:y.firstname,name:y.surname_real,country_code:y.country_code,club_id:y.club_id,job:y.job,job2:y.job2,note_g:Number(y.note_g)||0,note_d:Number(y.note_d)||0,note_m:Number(y.note_m)||0,note_a:Number(y.note_a)||0,rarity:y.rarity,skin:y.skin,hair:y.hair,hair_length:y.hair_length,face:y.face,clubName:((E=y.clubs)==null?void 0:E.encoded_name)||null,clubLogo:((m=y.clubs)==null?void 0:m.logo_url)||null,boost:0,used:!1,_line:w}}for(const y of["GK","DEF","MIL","ATT"]){const w=i.filter($=>$.job===y&&!n.has($.id)),h=i.filter($=>$.job!==y&&!n.has($.id)),E=[...w,...h],m=[];for(let $=0;$<o[y];$++){const A=E[$];A&&m.push(a(A,y,$))}const T=dt(m.length);m.forEach(($,A)=>{$._col=T[A]}),r[y]=m}const d=i.filter(y=>!n.has(y.id)).slice(0,5).map((y,w)=>a(y,y.job,100+w)),f=Object.keys(Me).sort(()=>Math.random()-.5).slice(0,3).map((y,w)=>{var h,E;return{id:"ai-gc-"+w,gc_type:y,name:((h=Me[y])==null?void 0:h.name)||y,icon:((E=Me[y])==null?void 0:E.icon)||"⚡"}}),b=Object.values(r).flat(),x={};b.forEach(y=>{y.club_id&&(x[y.club_id]=(x[y.club_id]||0)+1)});const g=(u=Object.entries(x).sort((y,w)=>w[1]-y[1])[0])==null?void 0:u[0];let p=null;if(g){const{data:y}=await v.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",g).single();y&&(p={club_id:y.id,country_code:null,name:y.encoded_name+" Stadium",club:{encoded_name:y.encoded_name,logo_url:y.logo_url}})}return{lines:r,subs:d,gcCards:f,stadiumDef:p}}function $r(t){let e=0;for(const i of["GK","DEF","MIL","ATT"])(t[i]||[]).forEach(o=>{const r=o._line||i,n=Number(r==="GK"?o.note_g:r==="DEF"?o.note_d:r==="MIL"?o.note_m:o.note_a)||0;e+=n+(o.boost||0)});return e}function Er(t,e){const i={};for(const a of["GK","DEF","MIL","ATT"])(t[a]||[]).forEach((s,d)=>{i[`${a}${d+1}`]=s});const o=$i(e)||Ei[e]||[];let r=0,n=0;for(const[a,s]of o){const d=i[a],c=i[s];if(!d||!c)continue;const l=Bt(d,c);l==="#00ff88"?n++:l==="#FFD700"&&r++}return{jaune:r,vert:n}}function ko(t){const e=t.job||"ATT";return Number(e==="GK"?t.note_g:e==="DEF"?t.note_d:e==="MIL"?t.note_m:t.note_a)||0}function ui(t,e,i){if(i<=0||!t.length)return[];const o=[...t].sort((r,n)=>Math.abs(ko(r)-e)-Math.abs(ko(n)-e));return o.slice(0,Math.min(i,o.length))}async function $o(t,e){var Y,I;const i=Number(e==null?void 0:e.target_note_avg)||10,o=Number(e==null?void 0:e.nb_liens_jaune)||0,r=Number(e==null?void 0:e.nb_liens_vert)||0,n=Number(e==null?void 0:e.nb_joueurs_stade)||0,{data:a}=await v.from("players").select("id,firstname,surname_real,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,clubs(encoded_name,logo_url,country_code)").eq("is_active",!0).limit(300);if(!a||a.length<16)return{lines:rn(t),subs:[],gcCards:[],stadiumDef:null};const s={};a.forEach(_=>{_.club_id&&(s[_.club_id]=(s[_.club_id]||0)+1)});const d=((Y=Object.entries(s).sort((_,G)=>G[1]-_[1])[0])==null?void 0:Y[0])||null,c=d?a.filter(_=>_.club_id===d):[],l=((I=c[0])==null?void 0:I.country_code)||null,f=l?a.filter(_=>_.country_code===l&&_.club_id!==d):[],b=Math.min(c.length,Math.max(2,r+1),16),x=Math.min(f.length,Math.max(0,o),16-b),g=[],p=new Set;ui(c,i,b).forEach(_=>{g.push(_),p.add(_.id)}),ui(f,i,x).forEach(_=>{g.push(_),p.add(_.id)});const u=Math.max(0,16-g.length),y=a.filter(_=>!p.has(_.id));ui(y,i,u).forEach(_=>{g.push(_),p.add(_.id)});const w=Et[t]||Et["4-4-2"],h={GK:[],DEF:[],MIL:[],ATT:[]},E=new Set;function m(_,G,K){var le,ce;return E.add(_.id),{cardId:"ai-"+_.id+"-"+K,id:_.id,firstname:_.firstname,name:_.surname_real,country_code:_.country_code,club_id:_.club_id,job:_.job,job2:_.job2,note_g:Number(_.note_g)||0,note_d:Number(_.note_d)||0,note_m:Number(_.note_m)||0,note_a:Number(_.note_a)||0,rarity:_.rarity,skin:_.skin,hair:_.hair,hair_length:_.hair_length,face:_.face,clubName:((le=_.clubs)==null?void 0:le.encoded_name)||null,clubLogo:((ce=_.clubs)==null?void 0:ce.logo_url)||null,boost:0,used:!1,_line:G}}for(const _ of["GK","DEF","MIL","ATT"]){const G=g.filter(H=>H.job===_&&!E.has(H.id)),K=g.filter(H=>H.job!==_&&!E.has(H.id)),le=[...G,...K],ce=[];for(let H=0;H<w[_];H++){const pe=le[H];pe&&ce.push(m(pe,_,H))}const j=dt(ce.length);ce.forEach((H,pe)=>{H._col=j[pe]}),h[_]=ce}let T=g.filter(_=>!E.has(_.id));if(T.length<5){const _=a.filter(G=>!E.has(G.id)&&!T.some(K=>K.id===G.id));T=T.concat(ui(_,i,5-T.length))}const $=T.slice(0,5).map((_,G)=>m(_,_.job,100+G)),R=Object.keys(Me).sort(()=>Math.random()-.5).slice(0,3).map((_,G)=>{var K,le;return{id:"ai-gc-"+G,gc_type:_,name:((K=Me[_])==null?void 0:K.name)||_,icon:((le=Me[_])==null?void 0:le.icon)||"⚡"}});let O=null;if(n>0&&d){const{data:_}=await v.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",d).single();_&&(O={club_id:_.id,country_code:null,name:_.encoded_name+" Stadium",club:{encoded_name:_.encoded_name,logo_url:_.logo_url}})}return{lines:h,subs:$,gcCards:R,stadiumDef:O}}function rn(t){const e=Et[t]||Et["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},o=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let r=0;for(const n of["GK","DEF","MIL","ATT"]){const a=[];for(let d=0;d<e[n];d++){const c=3+Math.floor(Math.random()*5);a.push({cardId:"fake-"+r,id:"fake-"+r,firstname:"IA",name:o[r%o.length],country_code:"XX",club_id:null,job:n,job2:null,note_g:n==="GK"?c:2,note_d:n==="DEF"?c:2,note_m:n==="MIL"?c:2,note_a:n==="ATT"?c:2,rarity:"normal",boost:0,used:!1,_line:n}),r++}const s=dt(a.length);a.forEach((d,c)=>{d._col=s[c]}),i[n]=a}return i}function Lr(t,e,i){var r;Bo("/sounds/match-opening.mp3",.3);const o=`<div style="position:relative;width:22px;height:22px;flex-shrink:0;display:flex;align-items:center;justify-content:center">
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">
      ${(()=>{if(e.phase==="defense"&&e.pendingAttack){const H=e.pendingAttack;let pe="";if(e.selected.length>0){const me=e.selected.map(C=>({...(e.homeTeam[C._role]||[]).find(Q=>Q.cardId===C.cardId)||C,_line:C._role})),fe=ai(me,e.modifiers.home,e.formation);pe=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
    ...
    ${(()=>{const j=window.innerWidth>=700,H=(F,z,W)=>{var Ee,be;const Z=(e.gcDefs||[]).find(xe=>xe.name===F.gc_type),se=(Z==null?void 0:Z.name)||F.gc_type,ge=(Z==null?void 0:Z.effect)||((Ee=Me[F.gc_type])==null?void 0:Ee.desc)||"",ye=Z!=null&&Z.image_url?`/icons/${Z.image_url}`:null,$e=((be=Me[F.gc_type])==null?void 0:be.icon)||"⚡";return`<div class="gc-mini" data-gc-id="${F.id}" data-gc-type="${F.gc_type}" style="flex-shrink:0;cursor:pointer">
        ...
        ${n.length===0?'<div style="font-size:7px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':n.map(F=>`<div class="sub-btn-col" data-sub-id="${F.cardId}" style="cursor:pointer;flex-shrink:0">${_t(F,76,100)}</div>`).join("")}
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${e.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...e.log].reverse().map(j=>{var H,pe,me;if(j.type==="duel"){const fe=j.isGoal,C=j.homeScored?"#FFD700":fe?"#ff6b6b":"rgba(255,255,255,0.3)",V=j.homeScored?"⚽ BUT !":fe?"⚽ BUT IA !":(H=j.homePlayers)!=null&&H.length?"⚔️ Attaque":"🛡️ Défense",Q=(j.homeTotal??0)>=(j.aiTotal??0);return`<div style="padding:8px;border-radius:8px;background:${fe?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${C};margin-bottom:4px">
                ...
                <div style="display:flex;align-items:center;gap:8px">
                  ${j.outPlayer?_t({...j.outPlayer,used:!0,_line:j.outPlayer.job,rarity:"normal"},38,50):""}
                  <span style="color:rgba(255,255,255,0.4);font-size:18px">→</span>
                  ${j.inPlayer?_t({...j.inPlayer,_line:j.inPlayer.job,rarity:"normal"},38,50):""}
                </div>
    ...
    <div style="flex:1;display:flex;gap:0;overflow:hidden">
      ...
      <div id="in-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
        ...
        <div>${u?_t({...u,used:!1,boost:0},y,w):"<div>—</div>"}</div>
      ...
      <div id="out-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
        ...
        <div>${p?_t({...p,used:!1,boost:0},y,w):"<div>—</div>"}</div>
    ...
    </div>`,(m=f.querySelector("#sub-close"))==null||m.addEventListener("click",()=>f.remove()),(T=f.querySelector("#out-up"))==null||T.addEventListener("click",()=>{s>0&&(s--,b())}),($=f.querySelector("#out-down"))==null||$.addEventListener("click",()=>{s<n.length-1&&(s++,b())}),(A=f.querySelector("#in-up"))==null||A.addEventListener("click",()=>{c>0&&(c--,b())}),(P=f.querySelector("#in-down"))==null||P.addEventListener("click",()=>{c<a.length-1&&(c++,b())});const E=(O,Y,I,_)=>{const G=f.querySelector("#"+O);if(!G)return;let K=0;G.addEventListener("touchstart",le=>{K=le.touches[0].clientY},{passive:!0}),G.addEventListener("touchend",le=>{const ce=le.changedTouches[0].clientY-K;if(Math.abs(ce)<30)return;const j=Y();ce<0&&j<_-1?(I(j+1),b()):ce>0&&j>0&&(I(j-1),b())},{passive:!0})};E("in-panel",()=>c,O=>c=O,a.length),E("out-panel",()=>s,O=>s=O,n.length),(R=f.querySelector("#sub-confirm"))==null||R.addEventListener("click",O=>{if(O.preventDefault(),O.stopPropagation(),l)return;l=!0;const Y=n[s],I=a[c];if(!Y||!I)return;let _=null,G=-1;for(const[le,ce]of Object.entries(e.homeTeam)){const j=(ce||[]).findIndex(H=>H.cardId===Y.cardId);if(j!==-1){_=le,G=j;break}}if(G===-1||!_){It("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),f.remove();return}const K={...I,_line:_,_col:Y._col||0,used:!1,boost:0};e.homeTeam[_].splice(G,1,K),e.usedSubIds||(e.usedSubIds=[]),e.usedSubIds.push(I.cardId),e.subsUsed++,e.selected=[],e.log.push({type:"sub",subSide:"home",clubName:e.clubName,outPlayer:{name:Y.name,firstname:Y.firstname,note:Ne(Y,_),portrait:Ct(Y),job:Y.job,country_code:Y.country_code,rarity:Y.rarity,clubName:Y.clubName,clubLogo:Y.clubLogo},inPlayer:{name:I.name,firstname:I.firstname,note:Ne(I,_),portrait:Ct(I),job:I.job,country_code:I.country_code,rarity:I.rarity,clubName:I.clubName,clubLogo:I.clubLogo},text:`🔄 ${I.firstname} ${I.name} remplace ${Y.firstname} ${Y.name}`}),f.remove(),wr(Y,I,()=>Te(t,e,i))})}document.body.appendChild(f),b()}function jr(t,e,i,o,r){var b,x;const n=(o.gcDefs||[]).find(g=>g.name===e),a=Me[e]||{icon:"⚡",desc:"Carte spéciale."},s=(n==null?void 0:n.name)||e,d=(n==null?void 0:n.effect)||a.desc,c=n!=null&&n.image_url?`/icons/${n.image_url}`:null,l=a.icon||"⚡",f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",f.innerHTML=`
    ${nt(s,c,l,d,{width:190})}
    <!-- Boutons -->
    ...
    <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start;justify-content:center">
      ${t.map(f=>{const b=f._line||f.job||"MIL",x=s.find(p=>p.cardId===f.cardId),g=ze({...f,_evolution_bonus:0},{width:90,showStad:!0,role:b,extraNote:f.boost||0});return`<div class="gc-pick-item" data-cid="${f.cardId}"
    ...
    <div style="display:flex;flex-direction:column;gap:6px">
      ${o.map(r=>`
        <div class="player-boost-opt" data-card-id="${r.cardId}"
          ...
          <div style="width:32px;height:32px;background:${br[r.job]||"#888"};border-radius:6px;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:900;font-size:13px">${Ne(r,r._line||r.job)}</div>
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(r=>{r.addEventListener("click",()=>{const n=r.dataset.cardId;for(const a of["GK","DEF","MIL","ATT"]){const s=(e.homeTeam[a]||[]).find(d=>d.cardId===n);if(s){s.boost=(s.boost||0)+e.boostCard.value,e.log.push({text:`⚡ Boost +${e.boostCard.value} appliqué à ${s.name}`,type:"info"});break}}e.boostUsed=!0,i.closeModal(),Te(t,e,i)})})}async function wt(t,e,i){var b,x,g,p;Rt(),Ke(),e._timerInt&&(clearInterval(e._timerInt),e._timerInt=null),e.phase="finished";const{state:o}=i,r=e.homeScore>e.aiScore,n=e.homeScore===e.aiScore,a=r?"victoire":n?"nul":"defaite",s=e.isSolo&&((b=e.soloLevelConfig)!=null&&b.reward_credits)&&r?Number(e.soloLevelConfig.reward_credits):Rn(e.mode,a);if(e.isSolo&&r)try{const{data:u}=await v.from("user_solo_progress").select("unlocked_level").eq("user_id",o.profile.id).maybeSingle(),y=(u==null?void 0:u.unlocked_level)||1;e.soloLevel>=y&&await v.from("user_solo_progress").upsert({user_id:o.profile.id,unlocked_level:e.soloLevel+1,updated_at:new Date().toISOString()})}catch(u){console.warn("[Solo] Erreur mise à jour progression:",u.message)}const d="00000000-0000-0000-0000-000000000000";let c=null;if(e.isRankedAI)try{const{data:u}=await v.from("users").select("mmr, mmr_deviation, mmr_volatility, placement_matches").eq("id",o.profile.id).single();if(u){const y=u.mmr??1e3,w=u.mmr_deviation??350,h=u.mmr_volatility??.06,E=r?1:n?.5:0,m=(u.placement_matches??0)<10,T=Mo(y,w,h,y,350,E,m),$=n?null:r?o.profile.id:d;await v.rpc("update_mmr_after_ranked",{p_match_id:e.matchId,p_winner_id:$,p_home_id:o.profile.id,p_away_id:d,p_home_delta:T.delta,p_away_delta:0,p_home_new_rd:T.newRd,p_away_new_rd:350,p_home_new_vol:T.newSigma,p_away_new_vol:.06}),c=T.delta}}catch(u){console.warn("[RankedAI] Erreur mise à jour MMR:",u.message)}e.matchId&&await v.from("matches").update({status:"finished",home_score:e.homeScore,away_score:e.aiScore,winner_id:r?o.profile.id:null,home_credits_reward:s,played_at:new Date().toISOString()}).eq("id",e.matchId);const l={credits:(o.profile.credits||0)+s,matches_played:(o.profile.matches_played||0)+1};r?l.wins=(o.profile.wins||0)+1:n?l.draws=(o.profile.draws||0)+1:l.losses=(o.profile.losses||0)+1,await v.from("users").update(l).eq("id",o.profile.id),await i.refreshProfile();const f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",f.innerHTML=`
    <div style="text-align:center;padding:40px;color:#fff;max-width:380px;width:100%">
      ...
      ${N}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${l?"🏆 Retour à la Mini League":a?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(q),(ie=q.querySelector("#pvp-end-home"))==null||ie.addEventListener("click",()=>{q.remove(),Ve(t),l?x("mini-league",{openLeagueId:l}):x(a?"ranked":"home")})}const Y=v.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},k=>{var M;const L=k.new;try{if(L.status==="finished"||L.forfeit){if(T)return;T=!0,$&&(clearInterval($),$=null),Ke(),L.game_state&&(m=L.game_state),O(L);return}if(L.game_state){const B=m;m=L.game_state;const S=m._lastGC;if(S&&S.seq&&!R.has(S.seq)&&(R.add(S.seq),S.by!==p)){C(S.type,S.by,()=>H());return}const N=B[p+"Score"]||0,q=B[u+"Score"]||0,D=m[p+"Score"]||0,U=m[u+"Score"]||0,ee=D>N,oe=U>q;if((ee||oe)&&!P.has(m.round)){P.add(m.round);const ie=[...m.log||[]].reverse().find(de=>de.isGoal),te=((ie==null?void 0:ie.homePlayers)||[]).map(de=>({name:de.name,note:de.note,portrait:de.portrait,job:de.job}));!!((M=ie==null?void 0:ie.text)!=null&&M.includes("DERNIER CORNER"))?ge().then(()=>{tt(te,D,U,ee,()=>H())}):tt(te,D,U,ee,()=>H());return}H()}}catch(B){console.error("[PvP] crash:",B)}}).subscribe();async function I(k){Object.assign(m,k),m.lastActionAt=new Date().toISOString();const{error:L}=await v.from("matches").update({game_state:m}).eq("id",i);L&&g("Erreur de synchronisation","error");try{H()}catch(M){console.error("[PvP] renderPvpScreen crash:",M)}}async function _(){if(T)return;T=!0,Rt(),Ke(),$&&(clearInterval($),$=null);const k=o?h.away_id:h.home_id,L=o?"p2":"p1",M=o?"p1":"p2",B={...m,[L+"Score"]:3,[M+"Score"]:0,phase:"finished"},S=B.p1Score||0,N=B.p2Score||0;try{await v.from("matches").update({game_state:B}).eq("id",i);const{data:q,error:D}=await v.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:k,p_home_score:S,p_away_score:N,p_forfeit:!0});if(D&&console.error("[PvP] finish_pvp_match (forfeit):",D),l)try{await v.rpc("check_and_finish_mini_league",{p_league_id:l})}catch(U){console.error("[PvP] check_and_finish_mini_league (forfeit):",U)}try{const{data:U}=await v.rpc("apply_match_rewards",{p_match_id:i});U!=null&&U.success&&!(U!=null&&U.skipped)&&typeof e.refreshProfile=="function"&&await e.refreshProfile()}catch(U){console.error("[PvP] apply_match_rewards (forfeit):",U)}}catch{}try{v.removeChannel(Y)}catch{}setTimeout(()=>{Ve(t),l?x("mini-league",{openLeagueId:l}):x("home")},800)}const G={BOOST_STAT:({value:k=1,count:L=1,roles:M=[]},B,S)=>{const N=B[p+"Team"],q=Object.entries(N).filter(([D])=>!M.length||M.includes(D)).flatMap(([D,U])=>U.filter(ee=>!ee.used).map(ee=>({...ee,_line:D})));if(!q.length){B.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),S(B);return}K(q,L,`Choisir ${L} joueur(s) à booster (+${k})`,D=>{D.forEach(U=>{const ee=(N[U._line]||[]).find(oe=>oe.cardId===U.cardId);ee&&(ee.boost=(ee.boost||0)+k,B.log.push({text:`⚡ +${k} sur ${ee.name}`,type:"info"}))}),B[p+"Team"]=N,S(B)})},DEBUFF_STAT:({value:k=1,count:L=1,roles:M=[],target:B="ai"},S,N)=>{const q=B==="home"?p:u,D=S[q+"Team"],U=B==="home"?"allié":"adverse",ee=Object.entries(D).filter(([oe])=>!M.length||M.includes(oe)).flatMap(([oe,ie])=>ie.map(te=>({...te,_line:oe})));if(!ee.length){S.log.push({text:`🎯 Aucun joueur ${U}`,type:"info"}),N(S);return}K(ee,L,`Choisir ${L} joueur(s) ${U}(s) (-${k})`,oe=>{oe.forEach(ie=>{const te=(D[ie._line]||[]).find(ae=>ae.cardId===ie.cardId);te&&(te.boost=(te.boost||0)-k,S.log.push({text:`🎯 -${k} sur ${te.name}`,type:"info"}))}),S[q+"Team"]=D,N(S)})},GRAY_PLAYER:({count:k=1,roles:L=[],target:M="ai"},B,S)=>{const N=M==="home"?p:u,q=B[N+"Team"],D=M==="home"?"allié":"adverse",U=Object.entries(q).filter(([ee])=>!L.length||L.includes(ee)).flatMap(([ee,oe])=>oe.filter(ie=>!ie.used).map(ie=>({...ie,_line:ee})));if(!U.length){B.log.push({text:`❌ Aucun joueur ${D}`,type:"info"}),S(B);return}K(U,k,`Choisir ${k} joueur(s) ${D}(s) à exclure`,ee=>{const oe="usedCardIds_"+N,ie=new Set(B[oe]||[]);ee.forEach(te=>{const ae=(q[te._line]||[]).find(de=>de.cardId===te.cardId);ae&&(ae.used=!0,ie.add(te.cardId),B.log.push({text:`❌ ${ae.name} exclu !`,type:"info"}))}),B[oe]=[...ie],B[N+"Team"]=q,S(B)})},REVIVE_PLAYER:({count:k=1,roles:L=[]},M,B)=>{const S=M[p+"Team"],N=Object.entries(S).filter(([q])=>!L.length||L.includes(q)).flatMap(([q,D])=>D.filter(U=>U.used).map(U=>({...U,_line:q})));if(!N.length){M.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),B(M);return}K(N,k,`Choisir ${k} joueur(s) à ressusciter`,q=>{q.forEach(D=>{const U=(S[D._line]||[]).find(ee=>ee.cardId===D.cardId);U&&(U.used=!1,M.log.push({text:`💫 ${U.name} ressuscité !`,type:"info"}))}),M[p+"Team"]=S,B(M)})},REMOVE_GOAL:({},k,L)=>{const M=u+"Score";k[M]>0?(k[M]--,k.log.push({text:"🚫 Dernier but annulé !",type:"info"})):k.log.push({text:"🚫 Aucun but à annuler",type:"info"}),L(k)},ADD_GOAL_DRAW:({},k,L)=>{k[p+"Score"]===k[u+"Score"]?(k[p+"Score"]++,k.log.push({text:"🎯 But bonus !",type:"info"})):k.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),L(k)},ADD_SUB:({value:k=1},L,M)=>{L.maxSubs=(L.maxSubs||3)+k,L.log.push({text:`🔄 +${k} remplacement(s)`,type:"info"}),M(L)},CUSTOM:({},k,L)=>L(k)};function K(k,L,M,B){const S=document.createElement("div");S.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let N=[];function q(){var U,ee;const D=k.map(oe=>{const ie=oe._line||oe.job||"MIL",te=N.find(de=>de.cardId===oe.cardId),ae=ze({...oe,_evolution_bonus:0},{width:90,showStad:!0,role:ie,extraNote:oe.boost||0});return`<div class="pp-item" data-cid="${oe.cardId}"
        ...
        </div>`,(U=S.querySelector("#pp-close"))==null||U.addEventListener("click",()=>S.remove()),S.querySelectorAll(".pp-item").forEach(oe=>{oe.addEventListener("click",()=>{const ie=oe.dataset.cid,te=k.find(de=>de.cardId===ie),ae=N.findIndex(de=>de.cardId===ie);te&&(ae>-1?N.splice(ae,1):N.length<L&&N.push(te),q())})}),(ee=S.querySelector("#pp-confirm"))==null||ee.addEventListener("click",()=>{S.remove(),B(N)})}q(),document.body.appendChild(S)}async function le(k,L){const B=(m["gcCardsFull_"+p]||[]).find(D=>D.id===k),S=(B==null?void 0:B._gcDef)||(m.gcDefs||[]).find(D=>{var U;return D.name===L||((U=D.name)==null?void 0:U.toLowerCase().trim())===(L==null?void 0:L.toLowerCase().trim())}),N=[...m["usedGc_"+p]||[],k],q={type:L,by:p,seq:(m._gcAnimSeq||0)+1};R.add(q.seq),C(L,p,async()=>{if(S!=null&&S.effect_type&&S.effect_type!=="CUSTOM"){const U=G[S.effect_type];if(U){const ee={...m};U(S.effect_params||{},ee,async oe=>{oe["usedGc_"+p]=N,oe._lastGC=q,oe._gcAnimSeq=q.seq,await I(oe)});return}}const D={...m};switch(L){case"Remplacement+":D.maxSubs=(D.maxSubs||3)+1,D.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const U=u+"Score";D[U]>0&&(D[U]--,D.log.push({text:"🚫 But annulé",type:"info"}));break}}D["usedGc_"+p]=N,D._lastGC=q,D._gcAnimSeq=q.seq,await I(D)})}function ce(k,L){const M="usedCardIds_"+k,B=new Set(m[M]||[]);L.forEach(S=>B.add(S)),m[M]=[...B]}function j(){for(const k of["p1","p2"]){const L=new Set(m["usedCardIds_"+k]||[]),M=m[k+"Team"];if(M)for(const B of["GK","DEF","MIL","ATT"])(M[B]||[]).forEach(S=>{S.used=L.has(S.cardId)})}}function H(){var ro,ao,so,lo,co,po;if(m.phase==="reveal")return pe();if(m.phase==="midfield")return fe();if(m.phase==="finished")return Ge();const k=m[p+"Team"],L=m[u+"Team"];j();const M=m[p+"Score"],B=m[u+"Score"],S=m[p+"Name"],N=m[u+"Name"],q=m.phase===p+"-attack",D=m.phase===p+"-defense",U=Array.isArray(m["selected_"+p])?m["selected_"+p]:[],ee=U.map(J=>J.cardId),oe=window.innerWidth>=700,ie=m[p+"Subs"]||[],te=m["usedSubIds_"+p]||[],ae=ie.filter(J=>!te.includes(J.cardId)),de=m["gcCardsFull_"+p]||[],ve=m["usedGc_"+p]||[],ue=de.filter(J=>!ve.includes(J.id)),we=m.boostOwner===p&&!m.boostUsed,ke=[...k.MIL||[],...k.ATT||[]].filter(J=>!J.used),ne=[...L.MIL||[],...L.ATT||[]].filter(J=>!J.used),Se=!xe(L),Re=ke.length===0&&ne.length===0&&!Se,Ae=(k.DEF||[]).filter(J=>!J.used),Xe=(k.GK||[]).filter(J=>!J.used);let He=[];q&&ke.length===0&&(Se?(He=Ae.map(J=>J.cardId),Ae.length===0&&(He=He.concat(Xe.map(J=>J.cardId)))):Re&&(He=[...Ae,...Xe].map(J=>J.cardId)));function rt(J,he,Ce){var je,xt;const Ie=J._gcDef,Ft=(Ie==null?void 0:Ie.name)||J.gc_type,lt=(Ie==null?void 0:Ie.effect)||((je=Me[J.gc_type])==null?void 0:je.desc)||"",Pe=Ie!=null&&Ie.image_url?`/icons/${Ie.image_url}`:null,Oe=((xt=Me[J.gc_type])==null?void 0:xt.icon)||"⚡";return`<div class="pvp-gc-mini" data-gc-id="${J.id}" data-gc-type="${J.gc_type}" style="flex-shrink:0;cursor:pointer">
      ...
      </div>`}const Fe=(J,he)=>he?Vt(95,162):rt(J,95),Je=(J,he)=>he?Vt(68,116):rt(J,68),Ue=oe?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",mt=q?We(p)?`<button id="pvp-action" style="${Ue};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${U.length===0?"disabled":""}>⚔️ ATTAQUEZ <span id="pvp-timer"></span></button>`:`<button id="pvp-action" data-pass="1" style="${Ue};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER <span id="pvp-timer"></span></button>`:D?`<button id="pvp-action" style="${Ue};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${U.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="pvp-timer"></span></button>`:`<div style="font-size:11px;color:rgba(255,255,255,0.3);text-align:center;padding:4px">⏳ Tour de ${N}</div>`,at=q&&!We(p)?'<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">Aucun attaquant — passez la main</div>':q||D?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${U.length}/3 sélectionné(s)</div>`:"",qt=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${oe?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
      ${ae.length===0?'<div style="font-size:7px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':ae.map(J=>`<div class="pvp-sub-btn" data-sub-id="${J.cardId}" style="cursor:pointer;flex-shrink:0">${_t(J,oe?76:44,oe?100:58)}</div>`).join("")}
    </div>`,gt=q?"attack":D?"defense":"idle",Lt=`<div style="overflow:hidden;min-width:0;flex:1;min-height:0;display:flex;flex-direction:column" id="match-field">
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${Ut(k,m[p+"Formation"],gt,ee,oe?1300:zt(),oe?600:Ot(),He)}
      </div>
    </div>`,st=m[p+"Team"],gn=(()=>{var he,Ce,Ie,Ft,lt;if(D&&((he=m.pendingAttack)!=null&&he.players)){const Pe=m.pendingAttack;let Oe="";if(U.length>0){const je=U.map(Qe=>{const Yt=(st[Qe._role]||[]).find(Mi=>Mi.cardId===Qe.cardId)||Qe,Wt=st[Qe._role]||[],li=Wt.findIndex(Mi=>Mi.cardId===Qe.cardId),zi=dt(Wt.length),Xt=li>=0?zi[li]:Yt._col??1;return{...Yt,_line:Qe._role,_col:Xt}}),xt=ai(je,((Ce=m.modifiers)==null?void 0:Ce[p])||{},m[p+"Formation"]);Oe=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
        ...
        </div>`}if(q&&!((Ft=m.pendingAttack)!=null&&Ft.players)&&U.length>0){const Pe=U.map(je=>{const xt=(st[je._role]||[]).find(Xt=>Xt.cardId===je.cardId)||je,Qe=["GK","DEF"].includes(je._role),Yt=st[je._role]||[],Wt=Yt.findIndex(Xt=>Xt.cardId===je.cardId),li=dt(Yt.length),zi=Wt>=0?li[Wt]:xt._col??1;return{...xt,_line:je._role,_col:zi,...Qe?{note_a:Math.max(1,Number(xt.note_a)||0)}:{}}}),Oe=ri(Pe,((lt=m.modifiers)==null?void 0:lt[p])||{},m[p+"Formation"]);return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #FFD700;text-align:center">
        ...
        </div>`}const J=(m.log||[]).slice(-1)[0];return J?'<div style="padding:2px 4px">'+on(J)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3);text-align:center">⏳ Match en cours...</div>'})(),no=`
      <div style="display:flex;align-items:center;padding:8px 10px;background:rgba(0,0,0,0.5);gap:6px;flex-shrink:0">
      ...
      <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
        ...
        <div id="mobile-play-area" style="flex:1;min-height:0;display:flex;overflow:hidden">
          <div id="match-field" style="flex:1;min-width:0;min-height:0;overflow:hidden">
            <div class="terrain-wrapper" style="width:100%;height:100%;overflow:hidden">
              ${Ut(k,m[p+"Formation"],gt,ee,zt(),Ot(),He)}
            </div>
          </div>
        </div>
        <div id="mobile-action-bar" style="position:absolute;left:0;right:0;bottom:0;z-index:20;background:rgba(0,0,0,0.55);padding:6px 8px 8px;display:flex;flex-direction:column;gap:6px;box-shadow:0 -4px 16px rgba(0,0,0,0.5)">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${L.map(S=>{const N={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[S._line]||"#555",q=Ne(S,S._line)+(S.boost||0);return`<div class="bp-item" data-cid="${S.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${N};overflow:hidden;cursor:pointer">
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        <div id="pin-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
          ...
          <div>${ue?_t({...ue,used:!1,boost:0},we,ke):"<div>—</div>"}</div>
        ...
        <div id="pout-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
          ...
          <div>${ve?_t({...ve,used:!1,boost:0},we,ke):"<div>—</div>"}</div>
      ...
      </div>`,(Re=ie.querySelector("#psub-close"))==null||Re.addEventListener("click",()=>ie.remove()),(Ae=ie.querySelector("#pout-up"))==null||Ae.addEventListener("click",()=>{D>0&&(D--,te())}),(Xe=ie.querySelector("#pout-down"))==null||Xe.addEventListener("click",()=>{D<N.length-1&&(D++,te())}),(He=ie.querySelector("#pin-up"))==null||He.addEventListener("click",()=>{ee>0&&(ee--,te())}),(rt=ie.querySelector("#pin-down"))==null||rt.addEventListener("click",()=>{ee<q.length-1&&(ee++,te())});const Se=(Fe,Je,Ue,mt)=>{const at=ie.querySelector("#"+Fe);if(!at)return;let qt=0;at.addEventListener("touchstart",gt=>{qt=gt.touches[0].clientY},{passive:!0}),at.addEventListener("touchend",gt=>{const Lt=gt.changedTouches[0].clientY-qt;if(Math.abs(Lt)<30)return;const st=Je();Lt<0&&st<mt-1?(Ue(st+1),te()):Lt>0&&st>0&&(Ue(st-1),te())},{passive:!0})};Se("pin-panel",()=>ee,Fe=>ee=Fe,q.length),Se("pout-panel",()=>D,Fe=>D=Fe,N.length),(Vt=ie.querySelector("#psub-confirm"))==null||Vt.addEventListener("click",async Fe=>{if(Fe.preventDefault(),Fe.stopPropagation(),oe)return;oe=!0;const Je=N[D],Ue=q[ee];if(!Je||!Ue)return;const mt=Je._line,at=(M[mt]||[]).findIndex(Lt=>Lt.cardId===Je.cardId);if(at===-1){g("Erreur : joueur introuvable","error"),ie.remove();return}const qt={...Ue,_line:mt,position:Je.position,used:!1,boost:0};M[mt].splice(at,1,qt);const gt=[...B,Ue.cardId];ie.remove(),re(Je,Ue,async()=>{await I({[p+"Team"]:M,[u+"Team"]:m[u+"Team"],["usedSubIds_"+p]:gt})})})}document.body.appendChild(ie),te()}function re(k,L,M){const B={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},S=document.createElement("div");S.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const N=(U,ee,oe)=>`<div style="text-align:center">
      ...
      </div>`,document.body.appendChild(L),(M=L.querySelector("#pvp-hist-close"))==null||M.addEventListener("click",()=>L.remove())}async function W(){if(m.phase!==p+"-attack")return;const k=p==="p1"?"p2":"p1",L=(m.round||0)+1,M=[...m.log||[]];M.push({type:"info",text:`⏭️ ${m[p+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const B=Le(m),S=We(k),N=B||!S?"finished":k+"-attack";await I({["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},pendingAttack:null,phase:N,attacker:k,round:L,log:M}),N==="finished"&&await De(m)}function Z(k){const L=(k.GK||[]).some(B=>!B.used),M=["DEF","MIL","ATT"].some(B=>(k[B]||[]).some(S=>!S.used));return L&&!M}function se(k){return(k.p1Score||0)!==(k.p2Score||0)?null:Z(k.p1Team)&&!xe(k.p2Team)?{side:"p1",gk:(k.p1Team.GK||[]).find(L=>!L.used)}:Z(k.p2Team)&&!xe(k.p1Team)?{side:"p2",gk:(k.p2Team.GK||[]).find(L=>!L.used)}:null}async function ge(){const k=document.createElement("div");k.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:950;display:flex;align-items:center;justify-content:center;overflow:hidden;text-align:center;padding:24px",document.body.appendChild(k);const L=(M,B)=>new Promise(S=>{k.innerHTML=`<div style="font-size:32px;font-weight:900;color:${B};letter-spacing:2px;animation:lcFade 1.4s ease both">${M}</div>
      <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(S,1400)});await L("⚽ DERNIER CORNER","#FFD700"),await L("🧤 LE GARDIEN MONTE !","#4fc3f7"),k.remove()}async function ye(k,L){const{side:M,gk:B}=k;if(!B)return;const S=M+"Score",N=(L[S]||0)+1;B.used=!0;const q=[...L.log||[]];q.push({type:"duel",isGoal:!0,homeScored:M===p,homePlayers:M==="p1"?[Pt(B)]:[],aiPlayers:M==="p2"?[Pt(B)]:[],text:`⚽ DERNIER CORNER — Le gardien de ${L[M+"Name"]} marque !`});const D=(L.round||0)+1,U=M==="p1"?"p2":"p1",ee={...L,[S]:N},oe=Le(ee);P.add(D),await ge();const ie=M===p?N:L[p+"Score"]||0,te=M===p?L[u+"Score"]||0:N;tt([Pt(B)],ie,te,M===p,async()=>{await I({[S]:N,log:q,round:D,pendingAttack:null,phase:oe?"finished":U+"-attack",attacker:U,["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}}}),oe&&await De({...m,[S]:N})})}async function $e(){const k=m[p+"Team"],L=!["GK","DEF","MIL","ATT"].some(N=>(m[u+"Team"][N]||[]).some(q=>!q.used)),M=(m["selected_"+p]||[]).map(N=>{const q=(k[N._role]||[]).find(de=>de.cardId===N.cardId)||N,D=["GK","DEF"].includes(N._role),U=k[N._role]||[],ee=U.findIndex(de=>de.cardId===N.cardId),oe=dt(U.length),ie=ee>=0?oe[ee]:q._col??1,te=m.stadiumDef||m.homeStadiumDef||null,ae=q.stadiumBonus||te&&(te.club_id&&String(q.club_id)===String(te.club_id)||te.country_code&&q.country_code===te.country_code)||!1;return{...q,_line:N._role,_col:ie,stadiumBonus:ae,...D?{note_a:Math.max(1,Number(q.note_a)||0)}:{}}});if(!M.length)return;const B=ri(M,m.modifiers[p]||{},m[p+"Formation"]);ce(p,M.map(N=>N.cardId)),M.forEach(N=>{const q=(k[N._role]||[]).find(D=>D.cardId===N.cardId);q&&(q.used=!0)}),m["selected_"+p]=[],H();const S=[...m.log||[]];if(L){if(M.length===1&&(M[0]._line==="GK"||M[0].job==="GK")&&(m.p1Score||0)===(m.p2Score||0)){await ye({side:p,gk:M[0]},{...m,[p+"Team"]:k,log:S});return}const q=(m[p+"Score"]||0)+1,D=M.map(te=>Pt(te));S.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:D,homeTotal:B.total,aiTotal:0});const U=(m.round||0)+1,ee=p==="p1"?"p2":"p1",oe={...m,[p+"Team"]:k,[p+"Score"]:q},ie=Le(oe);P.add(U),tt(D,q,m[u+"Score"]||0,!0,async()=>{await I({[p+"Team"]:k,[p+"Score"]:q,["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},pendingAttack:null,phase:ie?"finished":ee+"-attack",attacker:ee,round:U,log:S}),ie&&await De({...m,[p+"Score"]:q})});return}S.push({type:"pending",text:`⚔️ ${m[p+"Name"]} attaque (${B.total})`}),await I({[p+"Team"]:k,[u+"Team"]:m[u+"Team"],pendingAttack:{...B,players:M,side:p},["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},phase:u+"-defense",log:S})}async function Ee(){const k=m[p+"Team"],L=(m["selected_"+p]||[]).map(ue=>{const we=(k[ue._role]||[]).find(Ae=>Ae.cardId===ue.cardId)||ue,ke=k[ue._role]||[],ne=ke.findIndex(Ae=>Ae.cardId===ue.cardId),Se=dt(ke.length),Re=ne>=0?Se[ne]:we._col??1;return{...we,_line:ue._role,_col:Re}}),M=ai(L,m.modifiers[p]||{},m[p+"Formation"]);ce(p,L.map(ue=>ue.cardId)),L.forEach(ue=>{const we=(k[ue._role]||[]).find(ke=>ke.cardId===ue.cardId);we&&(we.used=!0)}),m["selected_"+p]=[],H();const B=Zi(m.pendingAttack.total,M.total,m.modifiers[p]||{}),S=m.pendingAttack.side,N=B==="attack"||(B==null?void 0:B.goal),q=S==="p1"?"p2":"p1",D=(m.round||0)+1,U=(m.pendingAttack.players||[]).map(ue=>Pt(ue)),ee=[...m.log||[]];ee.push({type:"duel",isGoal:N,homeScored:N&&S===p,text:N?`⚽ BUT de ${m[S+"Name"]} ! (${m.pendingAttack.total} vs ${M.total})`:`✋ Attaque stoppée (${m.pendingAttack.total} vs ${M.total})`,homePlayers:U,aiPlayers:L.map(ue=>Pt(ue)),homeTotal:m.pendingAttack.total,aiTotal:M.total});const oe=N?(m[S+"Score"]||0)+1:m[S+"Score"]||0,ie={...m,[p+"Team"]:k,[S+"Score"]:oe},te=Le(ie),ae=te?"finished":q+"-attack",de=!te&&!N?se(ie):null,ve=async()=>{if(de){await ye(de,ie);return}await I({[p+"Team"]:k,[u+"Team"]:m[u+"Team"],[S+"Score"]:oe,["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},pendingAttack:null,phase:ae,attacker:q,round:D,log:ee}),(ae==="finished"||te)&&await De({...m,[S+"Score"]:oe})};if(N){const ue=S===p,we=ue?oe:m[p+"Score"]||0,ke=ue?m[u+"Score"]||0:oe;P.add(D),tt(U,we,ke,ue,ve)}else await ve()}function be(k){return["MIL","ATT"].some(L=>(k[L]||[]).some(M=>!M.used))}function xe(k){return["GK","DEF","MIL","ATT"].some(L=>(k[L]||[]).some(M=>!M.used))}function Be(k){return xe(k)&&!be(k)}function We(k){const L=m[k+"Team"],M=m[(k==="p1"?"p2":"p1")+"Team"];return!!(be(L)||Be(L)&&!xe(M)||Be(L)&&!be(M)&&xe(M))}function Le(k){return!xe(k.p1Team)&&!xe(k.p2Team)}function qe(k){const L=k.p1Score||0,M=k.p2Score||0;return L===M?null:L>M?h.home_id:h.away_id}async function De(k){Rt(),Ke();try{const L=qe(k),M=L?h.home_id===L?h.away_id:h.home_id:null,B=k.p1Score||0,S=k.p2Score||0,{data:N,error:q}=await v.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:L,p_home_score:B,p_away_score:S,p_forfeit:!1});if(q&&console.error("[PvP] finish_pvp_match:",q),l)try{await v.rpc("check_and_finish_mini_league",{p_league_id:l})}catch(D){console.error("[PvP] check_and_finish_mini_league:",D)}L&&M&&Gr(L,M).catch(()=>{});try{const{data:D}=await v.rpc("apply_match_rewards",{p_match_id:i});D!=null&&D.success&&!(D!=null&&D.skipped)&&typeof e.refreshProfile=="function"&&await e.refreshProfile()}catch(D){console.error("[PvP] apply_match_rewards:",D)}if(typeof c=="function")try{await c({homeScore:B,awayScore:S,winnerId:L,homeId:h.home_id,awayId:h.away_id,matchId:i})}catch(D){console.error("[PvP] onMatchEnd:",D)}}catch(L){console.error("[PvP] finishMatch:",L)}}function Ge(){var S;const k=m[p+"Score"],L=m[u+"Score"],M=k>L,B=k===L;ct(t),t.innerHTML=`
    ...
    </div>`,(S=document.getElementById("pvp-home"))==null||S.addEventListener("click",()=>{try{v.removeChannel(Y)}catch{}Ve(t),l?x("mini-league",{openLeagueId:l}):x("home")})}H()}async function Ui(t,e,i=!1){var s,d;const{state:o,navigate:r,toast:n}=e,a=i&&((d=(s=e==null?void 0:e.state)==null?void 0:s.params)==null?void 0:d.rankedData)||null;await Si(t,e,i?"ranked":"random",async({deckId:c,formation:l,starters:f,subsRaw:b,gcCardsEnriched:x,gcDefs:g,stadiumDef:p})=>{ct(t);const u=async y=>{await Hr(t,e,c,l,f,b,y||[],g,p,i,a)};if(!(x!=null&&x.length)){await u([]);return}Ii(t,x,u)})}async function Or(t,e,i){return Rr(t,e,i)}async function Hr(t,e,i,o,r,n,a=[],s=[],d=null,c=!1,l=null){var O,Y;const{state:f,navigate:b,toast:x}=e,g=(l==null?void 0:l.mmr)??((O=f.profile)==null?void 0:O.mmr)??1e3,p=c?ki(g):null,u=c?"linear-gradient(135deg, #1a0a2e 0%, #2d1060 50%, #1a0a2e 100%)":"linear-gradient(135deg, #0a1a2e 0%, #0d3d1e 100%)",y=c?(p==null?void 0:p.color)||"#D4A017":"#FFD700";let w=!1,h=null,E=null;t.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    </div>`,document.body.appendChild(x),x.querySelector("#lv-ok").onclick=()=>{x.remove(),b(!0)},x.querySelector("#lv-cancel").onclick=()=>{x.remove(),b(!1)}}))return;const{data:d,error:c}=await v.rpc("leave_mini_league",{p_league_id:i,p_user_id:a});if(c||!(d!=null&&d.success)){r((d==null?void 0:d.error)||"Erreur lors de la désinscription","error");return}const l=d.refund||o;n.profile&&(n.profile.credits=(n.profile.credits||0)+l);const f=document.getElementById("nav-credits");f&&(f.textContent=`💰 ${(n.profile.credits||0).toLocaleString("fr")}`),r(`↩️ Désinscrit · +${l.toLocaleString("fr")} cr. remboursés`,"success"),ot(t,e,"waiting")}async function Ki(t,e,i,o,r){const{toast:n}=e;if(!confirm(`Supprimer définitivement "${o}" et tous ses matchs/membres ? Action irréversible.`))return;await v.from("mini_league_matches").delete().eq("league_id",i),await v.from("mini_league_members").delete().eq("league_id",i);const{error:a}=await v.from("mini_leagues").delete().eq("id",i);if(a){n("Erreur suppression ("+a.message+")","error");return}n("Mini League supprimée avec succès","success"),ot(t,e,r)}async function na(t,e,i){await v.from("mini_leagues").update({is_archived:!0}).eq("id",i),e.toast("Mini League archivée","success"),ot(t,e,"archived")}async function ut(t,e,i){var E,m,T,$,A,P,R,O,Y;const{state:o,toast:r}=e,n=o.profile.id,[{data:a},{data:s},{data:d}]=await Promise.all([v.from("mini_leagues").select("*").eq("id",i).single(),v.from("mini_league_members").select("*, user:users(id,pseudo,club_name,club_color1,club_color2)").eq("league_id",i),v.from("mini_league_matches").select("*").eq("league_id",i).order("matchday").order("created_at")]);if(!a){r("Introuvable","error"),await ot(t,e);return}const c=(s||[]).some(I=>I.user_id===n),l=a.creator_id===n,f=(s||[]).map(I=>I.user).filter(Boolean),b=da(f,d||[]),x=(d||[]).filter(I=>I.matchday===a.current_day),g=a.pot||0,p=a.entry_fee||100,u=x.length>0&&x.every(I=>I.status==="finished"||I.status==="bye"),y=a.current_day>=a.total_days,w=(s||[]).find(I=>I.user_id===n);t.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:${Zt};border:1px solid ${ei};border-radius:12px;padding:16px">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>${b.map((I,_)=>{const G=g>0&&a.status==="finished"?_===0?Math.floor(g*.7):_===1?Math.floor(g*.2):_===2?Math.floor(g*.1):0:0;return`<tr style="border-bottom:1px solid ${St};${I.userId===n?"background:rgba(74,222,128,0.08);":""}">
      ...
      <div style="background:${Zt};border:1px solid ${ei};border-radius:12px;padding:16px">
        ...
        ${Array.from({length:Math.max(0,a.status==="active"?a.current_day-1:a.current_day)},(I,_)=>_+1).reverse().map(I=>{const _=(d||[]).filter(G=>G.matchday===I);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:${it};margin-bottom:6px">Journée ${I}</div>${_.map(G=>Io(G,f,n,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
    ...
    <div style="background:linear-gradient(160deg,#1a0a2e,#3b1a6e);border-radius:20px;padding:32px 24px;max-width:320px;width:100%;text-align:center;color:#fff;box-shadow:0 8px 40px rgba(0,0,0,0.6)">
      ...
      <div style="background:rgba(212,160,23,0.2);border:2px solid ${pt};border-radius:14px;padding:16px;margin-bottom:24px">
        ...
        <div style="font-size:32px;font-weight:900;color:${pt}">${c.toLocaleString("fr")} cr.</div>
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    </div>`}function b(p){var R,O,Y,I;const u=(R=p.card)==null?void 0:R.player;if(!u)return"";const y=((O=p.card)==null?void 0:O.evolution_bonus)||0,w=mi(u,u.job,y),h=u.job2?mi(u,u.job2,y):0,E=p.status==="sold",m=u.country_code?`https://flagsapi.com/${u.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,T=So[u.job]||"#bbb",$=u.job2?So[u.job2]||"#bbb":null,A=u.job==="GK"?"#fff":T,P=u.job2==="GK"?"#fff":$;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${E?"opacity:0.7":""}">
    ...
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center">
      ...
      <div style="display:flex;gap:10px">
        ...
        <button id="buy-ok" style="flex:1;padding:12px;border-radius:10px;border:none;background:var(--green);color:#fff;font-size:14px;font-weight:900;cursor:pointer">Confirmer</button>
          ...
          </div>`}).join(""):'<div style="text-align:center;color:var(--tile-fg-dim);padding:40px">Aucun joueur classé.</div>'}else if(d==="mini-league"){const $=a||[];h.innerHTML=$.length>0?$.map((A,P)=>`
  ...
  <div class="page">
    ...
    </div>`,(T=document.getElementById("ranked-back"))==null||T.addEventListener("click",()=>o("home"));return}const d=s.mmr??1e3,c=s.mmr_deviation??350,l=s.mmr_volatility??.06,f=s.placement_matches??0,b=f<10,x=Math.max(0,10-f),g=ki(d),p=zo(d),u=Ci.findIndex(R=>R.id===g.id),y=Ci[u+1]||null,w={bronze:"linear-gradient(160deg,#3d1c00,#7a3e00)",silver:"linear-gradient(160deg,#1a1a2e,#3a3a5e)",gold:"linear-gradient(160deg,#1a1200,#4a3500)",platinum:"linear-gradient(160deg,#001a20,#003040)",diamond:"linear-gradient(160deg,#001030,#1a2860)",master:"linear-gradient(160deg,#1a0030,#3d0070)"},h=(s.ranked_wins||0)+(s.ranked_losses||0)+(s.ranked_draws||0),E=h>0?Math.round((s.ranked_wins||0)/h*100):0,m=Ci.map(R=>`
