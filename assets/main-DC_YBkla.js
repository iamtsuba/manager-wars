...
import{s as v,j as vi,q as In,t as Ut,u as Oi,F as Hi,o as tt,n as ti,g as At,r as Te,m as Mt,v as Mi,w as fo,T as zi,p as mo,x as go,d as xo}from"./special-cards-q1oKjNQl.js";const bo="/";function pn(e,{navigate:t,toast:i}){let n="login";const o=()=>{var a,s,d,c,l,f,b;const r=n==="login";if(e.innerHTML=`
  ...
  `;function s(){var g;const c=document.getElementById("logo-preview"),l=document.getElementById("logo-initials"),f=((g=document.getElementById("setup-club"))==null?void 0:g.value)||"MW",b=f.trim().split(" ").filter(Boolean),x=b.length>=2?(b[0][0]+b[1][0]).toUpperCase():f.slice(0,2).toUpperCase();c&&(c.style.background=a,c.style.borderColor=r),l&&(l.textContent=x,l.style.color=r)}document.getElementById("color1").addEventListener("input",c=>{r=c.target.value,document.getElementById("swatch1").style.background=r,s()}),document.getElementById("color2").addEventListener("input",c=>{a=c.target.value,document.getElementById("swatch2").style.background=a,s()});function d(c){document.querySelectorAll(".setup-step").forEach(l=>l.classList.remove("active")),document.getElementById(`step-${c}`).classList.add("active"),document.getElementById("step-num").textContent=c,document.getElementById("progress-fill").style.width=`${Math.round(c/3*100)}%`,c===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("step1-error");if(l.textContent="",c.length<3){l.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:f}=await v.from("users").select("id").eq("pseudo",c).maybeSingle();if(f){l.textContent="Ce pseudo est déjà pris.";return}d(2)}),document.getElementById("step2-back").addEventListener("click",()=>d(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const c=document.getElementById("setup-club").value.trim(),l=document.getElementById("step2-error");if(l.textContent="",c.length<2){l.textContent="Nom trop court (min. 2 caractères).";return}const{data:f}=await v.from("users").select("id").eq("club_name",c).maybeSingle();if(f){l.textContent="Ce nom de club est déjà pris.";return}d(3)}),document.getElementById("step3-back").addEventListener("click",()=>d(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("setup-club").value.trim(),f=document.getElementById("step3-error"),b=document.getElementById("step3-finish");f.textContent="",b.disabled=!0,b.textContent="Création en cours…";try{const{error:x}=await v.from("users").insert({id:t.user.id,pseudo:c,club_name:l,club_color1:r,club_color2:a,credits:1e4});if(x)throw x;await ho(t.user.id),await o(),n(`Bienvenue ${c} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(x){f.textContent=x.message,b.disabled=!1,b.textContent="🚀 Créer mon profil !"}})}async function ho(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await v.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const vo="modulepreload",wo=function(e){return"/"+e},un={},Ui=function(t,i,n){let o=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),s=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));o=Promise.allSettled(i.map(d=>{if(d=wo(d),d in un)return;un[d]=!0;const c=d.endsWith(".css"),l=c?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${l}`))return;const f=document.createElement("link");if(f.rel=c?"stylesheet":vo,c||(f.as="script"),f.crossOrigin="",f.href=d,s&&f.setAttribute("nonce",s),document.head.appendChild(f),c)return new Promise((b,x)=>{f.addEventListener("load",b),f.addEventListener("error",()=>x(new Error(`Unable to preload CSS for ${d}`)))})}))}function r(a){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=a,window.dispatchEvent(s),!s.defaultPrevented)throw a}return o.then(a=>{for(const s of a||[])s.status==="rejected"&&r(s.reason);return t().catch(r)})},ii="#1A6B3C",ni="#cc2222",_o="#D4A017",fn="#888";async function ko(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    <div style="display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:8px;padding:10px 0;border-bottom:1px solid #f0f0f0">
      ...
      <div style="text-align:left;font-size:18px;font-weight:900;color:${T}">${M}</div>
  ...
  <div class="home-dark" id="home-dark">
    ...
    <div class="news-item">
      ...
      <div class="news-body">
        ...
        <div class="news-time">${qo($.published_at)}</div>
      </div>
    </div>`;let w=0;const h=$=>`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    ${Te(e,{width:140})}
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),c=(s||[]).filter(L=>L.card_type==="player"&&L.player),l=(s||[]).filter(L=>L.card_type==="game_changer"),f=(s||[]).filter(L=>L.card_type==="formation"),b=(s||[]).filter(L=>L.card_type==="stadium"),{data:x}=await v.from("gc_definitions").select("name,gc_type,color,effect,image_url"),g={};(x||[]).forEach(L=>{g[L.name]=L});const{data:p}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),u={};(p||[]).forEach(L=>{u[L.id]=L}),b.forEach(L=>{L.stadium_def&&(u[L.stadium_id]=L.stadium_def)});const y=Object.keys(Hi),w=Object.keys(je),h={};c.forEach(L=>{const F=L.player.id;h[F]=(h[F]||0)+1}),new Set(Object.keys(h).map(L=>String(L)));const E=new Set(f.map(L=>L.formation)),m=new Set(l.map(L=>L.gc_type));let M="player",k="Tous",T="",B=!1;function W(){return[...c].sort((L,F)=>{const U=di.indexOf(L.player.job),O=di.indexOf(F.player.job);return U!==O?U-O:(L.player.surname_real||"").localeCompare(F.player.surname_real||"")})}function te(){return[...d||[]].sort((L,F)=>{const U=di.indexOf(L.job),O=di.indexOf(F.job);return U!==O?U-O:(L.surname_real||"").localeCompare(F.surname_real||"")})}function Z(){return W().filter(L=>{const F=L.player,U=k==="Tous"||F.job===k,O=!T||`${F.firstname} ${F.surname_real}`.toLowerCase().includes(T);return U&&O})}function A(){return te().filter(L=>{const F=k==="Tous"||L.job===k,U=!T||`${L.firstname} ${L.surname_real}`.toLowerCase().includes(T);return F&&U})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:0 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none;align-items:center"></div>
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",F=>{T=F.target.value.toLowerCase(),N()}),e.querySelectorAll(".filter-btn").forEach(F=>{F.addEventListener("click",()=>{k=F.dataset.job,$(),N()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{B=!B,$(),N()})):(L.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{B=!B,$(),N()})))}function N(){const L=document.getElementById("col-grid");L&&(M==="player"?Y(L):M==="formation"?ue(L):M==="stadium"?V(L):K(L))}function X(L,F,U,O,re,q=0){const G=document.getElementById("col-grid"),se=document.getElementById("col-big");if(!G||!se)return;var de=0;function ve(){var ct;const ze=window.innerWidth>=768,he=document.getElementById("col-big"),Ie=document.getElementById("col-gap"),Se=((ct=document.getElementById("col-grid"))==null?void 0:ct.parentElement)||null,Be=he?he.closest(".page"):null;if(he&&Se&&Be){let $e=0;Array.from(Be.children).forEach(function(j){j!==he&&j!==Se&&j!==Ie&&($e+=j.offsetHeight)});const Ce=Math.max(0,Be.clientHeight-$e),qe=Math.round(Ce*(q/100)),Ge=Math.max(0,Ce-qe),_=ze?50/71:45/63,z=Math.round(Ge*_),S=Math.max(0,Ge-z);Ie&&(Ie.style.height=qe+"px"),he.style.flex="none",he.style.height=z+"px",he.style.minHeight="0",Se.style.flex="none",Se.style.height=S+"px",Se.style.minHeight="0",Se.style.display="flex",Se.style.overflow="hidden"}const ke=document.getElementById("col-grid");ke&&(ke.style.height="100%",ke.style.flexShrink="0",ke.style.overflowX="auto",ke.style.overflowY="hidden",ke.style.alignItems="center",ke.style.width="100%"),se.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:center center">'+F(L[de])+"</div>";var _t=se.querySelector("[data-card-id],[data-form-id],[data-gc-id]");_t&&_t.addEventListener("click",function(){O(L[de])}),requestAnimationFrame(function(){var $e=document.getElementById("big-card-inner");if(!(!$e||!se)){var Ce=se.clientHeight,qe=se.clientWidth-16,Ge=$e.offsetHeight,_=$e.offsetWidth;if(Ge>0&&_>0&&Ce>40){var z=Math.min(Ce/Ge,qe/_);$e.style.transform="scale("+z.toFixed(3)+")",$e.style.transformOrigin="center center"}}}),G.innerHTML=L.map(function($e,Ce){var qe=Ce===de,Ge="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+(qe?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+Ce+'" style="'+Ge+'">'+U($e,qe)+"</div>"}).join(""),G.querySelectorAll(".col-mini-item").forEach(function($e){$e.addEventListener("click",function(){de=Number($e.dataset.idx),ve(),$e.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})}),requestAnimationFrame(function(){var $e=ke?ke.clientHeight:0,Ce=G.querySelector(".col-mini-item > div");if(!(!$e||!Ce)){var qe=Ce.style.zoom;Ce.style.zoom="1";var Ge=Ce.offsetHeight;if(Ce.style.zoom=qe,!(Ge<=0)){var _=$e/Ge;G.querySelectorAll(".col-mini-item > div").forEach(function(z){z.style.zoom=_.toFixed(4)})}}})}ve()}function ce(L,F){var U=window.innerWidth>=768?.76:.54,O;if(!L||L._fake){var re=L?L.player:null;if(!re)return"";O=hn(re)}else O=Ii(L,"");var q=L&&!L._fake?h[L.player&&L.player.id]||1:0,G=q>0?'<div style="position:absolute;top:6px;right:6px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:11px;font-weight:900;padding:2px 7px;z-index:3;box-shadow:0 1px 4px rgba(0,0,0,0.4)">×'+q+"</div>":"";return'<div style="display:inline-block;position:relative;zoom:'+U+';pointer-events:none;line-height:0">'+G+O+"</div>"}function fe(L,F,U){var O=U>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+U+"</div>":"",re=!!F,q=Mi(L,Mt[L],{width:160});return re||(q='<div style="filter:grayscale(1);opacity:0.5">'+q+"</div>"),"<div "+(F?'data-form-id="'+F.id+'"':"")+' style="position:relative;cursor:pointer">'+O+q+"</div>"}function C(L,F){var U=window.innerWidth>=768?.76:.54,O=Mi(L,Mt[L],{width:140});return F||(O='<div style="filter:grayscale(1);opacity:0.45">'+O+"</div>"),'<div style="display:inline-block;zoom:'+U+';line-height:0;pointer-events:none">'+O+"</div>"}function Y(L){if(B){const F=A();if(!F.length){L.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucun joueur.</div>';return}const U=F.map(O=>c.find(re=>re.player.id===O.id)||{_fake:!0,player:O,id:"fake-"+O.id});X(U,O=>O._fake?hn(O.player):Ii(O,""),O=>O._fake?ce({player:O.player,id:"x",_fake:!0}):ce(O),O=>{O._fake||vn(O,c,h,t)})}else{const F=Z();if(!F.length){L.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const U={};F.forEach(re=>{const q=re.player.id;U[q]||(U[q]=[]),U[q].push(re)});const O=Object.values(U).map(re=>Qo(re));X(O,re=>{const q=h[re.player.id]||1,G=q>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${q}</div>`:"",de=c.filter(ve=>ve.player.id===re.player.id&&ve.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Ii(re,G+de)},re=>ce(re),re=>vn(re,c,h,t))}}function ue(L){const F=B?y:[...E];if(!F.length){L.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const U=F.map(O=>({formation:O,card:f.find(re=>re.formation===O)||null,owned:E.has(O)}));X(U,({formation:O,card:re,owned:q})=>fe(O,q?re:null,q?f.filter(G=>G.formation===O).length:0),({formation:O,owned:re})=>C(O,re),({card:O,owned:re})=>{re&&O&&ir(O,f,t,r)},"#1A6B3C",5)}function K(L){const F=Object.keys(g),U=B?F.length?F:w:[...m];if(!U.length){L.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const O=U.map(re=>({type:re,gc:je[re]||{icon:"⚡",desc:""},def:g[re]||null,owned:m.has(re),card:l.find(q=>q.gc_type===re)||null}));X(O,({type:re,gc:q,def:G,owned:se,card:de})=>{var ke;const ve=(G==null?void 0:G.name)||re,ze=se?l.filter(_t=>_t.gc_type===re).length:0,he=ze>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${ze}</div>`:"",Ie=(G==null?void 0:G.effect)||q.desc||"",Se=G!=null&&G.image_url?`/icons/${G.image_url}`:((ke=G==null?void 0:G.club)==null?void 0:ke.logo_url)||(G!=null&&G.country_code?`https://flagsapi.com/${G.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null);let Be=tt(ve,Se,q.icon,Ie,{width:160,onClick:se});return se||(Be=`<div style="filter:grayscale(1);opacity:0.5">${Be}</div>`),`<div ${se&&de?`data-gc-id="${de.id}" data-gc-type="${re}"`:""} style="position:relative">${he}${Be}</div>`},({type:re,gc:q,def:G,owned:se})=>{const de=window.innerWidth>=768?.76:.54,ve=(G==null?void 0:G.name)||re,ze=(G==null?void 0:G.effect)||q.desc||"",he=G!=null&&G.image_url?`/icons/${G.image_url}`:null;let Ie=tt(ve,he,q.icon,ze,{width:140});return se||(Ie=`<div style="filter:grayscale(1);opacity:0.45">${Ie}</div>`),`<div style="display:inline-block;zoom:${de};line-height:0;pointer-events:none">${Ie}</div>`},({type:re,owned:q,def:G})=>{q&&tr(re,G,r)},"#7a28b8",5)}function V(L){const F="#4FC3F7",U="/";if(!b.length){L.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Stade.<br><small>Ouvre un booster Stade !</small></div>';return}const O={};b.forEach(q=>{const G=q.stadium_id||"?";(O[G]=O[G]||[]).push(q)});const re=Object.entries(O).map(([q,G])=>({sid:q,def:u[q]||null,count:G.length,card:G[0]}));X(re,({def:q,count:G})=>{var Ie,Se;const se=(q==null?void 0:q.name)||"?",de=((Ie=q==null?void 0:q.club)==null?void 0:Ie.encoded_name)||(q==null?void 0:q.country_code)||"—",ve=q!=null&&q.image_url?`${U}icons/${q.image_url}`:((Se=q==null?void 0:q.club)==null?void 0:Se.logo_url)||(q!=null&&q.country_code?`https://flagsapi.com/${q.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),ze=G>1?`<div style="position:absolute;top:8px;right:8px;background:#333;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${G}</div>`:"",he=`${de}<br>+10 ⭐ joueurs alliés`;return`<div style="position:relative">${ze}${ti(se,ve,he,{width:160})}</div>`},({def:q})=>{var he,Ie;const G=window.innerWidth>=768?.76:.54,se=(q==null?void 0:q.name)||"?",de=((he=q==null?void 0:q.club)==null?void 0:he.encoded_name)||(q==null?void 0:q.country_code)||"—",ve=q!=null&&q.image_url?`${U}icons/${q.image_url}`:((Ie=q==null?void 0:q.club)==null?void 0:Ie.logo_url)||(q!=null&&q.country_code?`https://flagsapi.com/${q.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),ze=`${de}<br>+10 ⭐`;return`<div style="display:inline-block;zoom:${G};line-height:0;pointer-events:none">${ti(se,ve,ze,{width:140})}</div>`},null,F,5)}e.querySelectorAll(".col-tab-btn").forEach(L=>{L.addEventListener("click",()=>{M=L.dataset.tab,k="Tous",T="",B=!1,e.querySelectorAll(".col-tab-btn").forEach(F=>{const U=F.dataset.tab===M;F.style.borderBottomColor=U?"var(--green)":"transparent",F.style.color=U?"var(--green)":"var(--tile-fg-dim)"}),$(),N()})}),$(),N()}function tr(e,t,i){const n=je[e]||{icon:"⚡",desc:"Effet spécial."},o=(t==null?void 0:t.name)||e,r=(t==null?void 0:t.effect)||n.desc,a=t!=null&&t.image_url?`/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
      ${tt(o,a,n.icon,r,{width:200})}
      <div style="background:#fff3cd;border-radius:10px;padding:10px 14px;width:100%">
        <div style="font-size:12px;color:#856404">⚠️ Cette carte est à <b>usage unique</b>. Une fois jouée en match, elle est définitivement supprimée de ta collection.</div>
      </div>
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const ci=1e3;function ir(e,t,i,n){var g,p,u;const{state:o,toast:r,closeModal:a,navigate:s,refreshProfile:d}=i,c=e.formation,l={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function f(){const y=Mt[c]||{},w=Hi[c]||[],h=290,E=360,m=20;function M(T){const B=y[T];return B?{x:B.x*h,y:B.y*E}:null}let k=`<svg width="${h}" height="${E}" viewBox="0 0 ${h} ${E}" xmlns="http://www.w3.org/2000/svg">`;for(const[T,B]of w){const W=M(T),te=M(B);!W||!te||(k+=`<line x1="${W.x}" y1="${W.y}" x2="${te.x}" y2="${te.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const T of Object.keys(y)){const B=M(T);if(!B)continue;const W=T.replace(/\d+/,""),te=l[W]||"#555";k+=`<circle cx="${B.x}" cy="${B.y}" r="${m}" fill="${te}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,k+=`<text x="${B.x}" y="${B.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${W}</text>`}return k+="</svg>",k}const b=t.filter(y=>y.formation===c);b.length;const x=!e.is_for_sale;n(`Formation ${c}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;background:#f0fdf4;border-radius:10px;padding:10px 14px">
          <div>
            ...
            <div style="font-size:11px;color:#555;margin-top:2px">
              ${(e.evolution_bonus||0)>0?`+${e.evolution_bonus} appliqué${e.evolution_bonus>1?"s":""} · Note : ${k}${l.job2&&T>0?` / ${B}`:""}`:`Note actuelle : ${k}${l.job2&&T>0?` / ${B}`:""}`}
            </div>
          </div>
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",l.note_g],["DEF",l.note_d],["MIL",l.note_m],["ATT",l.note_a]].map(([K,V])=>{const L=Ti[K],F=K==="GK"?"#fff":L,U=K===l.job||K===l.job2,O=(Number(V)||0)+(U&&m>0?m:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(fe=document.getElementById("close-detail"))==null||fe.addEventListener("click",s);let A=new Set;const $=l.rarity==="pepite"?1.3:l.rarity==="papyte"?.7:1;function N(){let K=0;return document.querySelectorAll(".expl-check:checked").forEach(V=>{if(V.dataset.id===e.id)return;const F=Number(V.dataset.evo)||0,U=Number(V.dataset.note)||0;K+=U+F}),Math.round(K*$)}const X=()=>{const K=A.size,V=document.getElementById("sell-action-panel");if(!V)return;V.style.display=K>0?"block":"none",document.getElementById("sell-selected-count").textContent=K;const L=document.getElementById("evolve-btn");if(L){const F=N();L.textContent=F>0?`⬆️ Évoluer (+${F})`:"⬆️ Évoluer"}};document.querySelectorAll(".expl-check").forEach(K=>{K.addEventListener("change",()=>{const V=K.dataset.id;K.checked?A.add(V):A.delete(V);const L=K.closest(".exemplaire-row");if(L){const F=L.querySelector(".expl-sel-overlay"),U=L.querySelector(".expl-sel-check");F&&(F.style.display=K.checked?"block":"none"),U&&(U.style.display=K.checked?"flex":"none"),L.style.transform=K.checked?"scale(1.05)":"scale(1)"}X()})}),document.querySelectorAll(".exemplaire-row").forEach(K=>{K.addEventListener("click",V=>{if(V.target.tagName==="INPUT")return;const L=K.querySelector(".expl-check");L&&!L.disabled&&(L.checked=!L.checked,L.dispatchEvent(new Event("change")))})}),(C=document.getElementById("evolve-btn"))==null||C.addEventListener("click",async()=>{if(b<=1)return;const K=[...A];if(!K.length)return;if(A.has(e.id)){const G=document.createElement("div");G.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",G.innerHTML=`
  ...
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
    ...
    </div>`;u("⚽ Choisir une formation",w),document.querySelectorAll("#modal-body [data-forma]").forEach(h=>{h.addEventListener("click",()=>{t.formation=h.dataset.forma;const E=kn(t.formation),m={};E.forEach(M=>{t.slots[M]&&(m[M]=t.slots[M])}),t.slots=m,y(),wt(e,t,i)})})};e.querySelectorAll("#formation-mobile-btn, #formation-pc-btn").forEach(u=>u.addEventListener("click",x)),e.querySelectorAll("#add-stad-btn-pc, #add-stad-btn").forEach(u=>u.addEventListener("click",()=>rr(t,e,i))),e.querySelectorAll("#save-deck").forEach(u=>u.addEventListener("click",()=>dr(t,i))),e.querySelectorAll("[data-remove-sub]").forEach(u=>{u.addEventListener("click",()=>{t.subs=t.subs.filter(y=>y!==u.dataset.removeSub),wt(e,t,i)})}),e.querySelectorAll("#add-sub-btn").forEach(u=>u.addEventListener("click",()=>sr(t,e,i)))}function or(e,t,i,n){var h,E,m;const o=window.innerWidth>=900,r=e.querySelector(o?"#deck-field-pc":"#deck-field-mobile");if(!r)return;const a=(h=t.stadiumCards)==null?void 0:h.find(M=>M.id===t.stadiumCardId),s=a&&((E=t.stadDefMap)==null?void 0:E[a.stadium_id])||null,d=Mt[t.formation]||{},c=Oi(t.formation)||[],l={};for(const M of i){const k=t.slots[M],T=k?t.playerCards.find(B=>B.id===k):null;T!=null&&T.player?l[M]={...T.player,_evolution_bonus:T.evolution_bonus||0,face:T.player.face||null}:l[M]=null}const f=window.innerWidth>=900,b=f?window.innerWidth-280:window.innerWidth-20,x=f?Math.min(b,860):b,g=Math.round(f?x*.82:x*.85),p=f?84:44;let u="";for(const[M,k]of c){const T=d[M],B=d[k];if(!T||!B)continue;const W=T.x*x,te=Math.round(.03*g+T.y*.85*g),Z=B.x*x,A=Math.round(.03*g+B.y*.85*g),$=l[M],N=l[k],X=Ut($,N);X==="#ff3333"||X==="#cc2222"?u+=`<line x1="${W.toFixed(1)}" y1="${te.toFixed(1)}" x2="${Z.toFixed(1)}" y2="${A.toFixed(1)}" stroke="${X}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(u+=`<line x1="${W.toFixed(1)}" y1="${te.toFixed(1)}" x2="${Z.toFixed(1)}" y2="${A.toFixed(1)}" stroke="${X}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,u+=`<line x1="${W.toFixed(1)}" y1="${te.toFixed(1)}" x2="${Z.toFixed(1)}" y2="${A.toFixed(1)}" stroke="${X}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let y="";const w=Math.round(p*657/507);for(const M of i){const k=d[M];if(!k)continue;const T=l[M],B=k.x*x;k.y*g;const W=Math.round(.03*g+k.y*(.85*g)),te=Math.round(B-p/2),Z=Math.round(W-w/2);if(T){const A=M.replace(/\d+/,""),$=s&&(s.club_id&&String(T.club_id)===String(s.club_id)||s.country_code&&T.country_code===s.country_code),N=Te({...T,_evolution_bonus:T._evolution_bonus||0},{width:p,showStad:!0,stadDef:s,role:A});$&&((m=s.club)!=null&&m.logo_url||s.image_url),y+=`<div style="position:absolute;left:${te}px;top:${Z}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${M}">
  ...
  `,document.body.appendChild(o);let r=!1;const a=document.getElementById("pack-cut-zone"),s=document.getElementById("pack-blade");let d=!1;const c=$=>$.touches&&$.touches[0]?$.touches[0].clientX:$.clientX;function l($){r||(d=!0,s.style.opacity="1",f($))}function f($){if(!d||r)return;const N=a.getBoundingClientRect(),X=c($)-N.left,ce=Math.max(0,Math.min(1,X/N.width));s.style.width=ce*N.width+"px",ce>=.82&&x()}function b(){r||(d=!1,s.style.transition="width .2s ease, opacity .2s ease",s.style.width="0",s.style.opacity="0",setTimeout(()=>{r||(s.style.transition="")},220))}function x(){var N;if(r)return;r=!0,d=!1,s.style.width="100%",s.style.opacity="1",(N=document.getElementById("cut-flash"))==null||N.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const $=document.getElementById("cut-hint");$&&($.style.opacity="0"),a.classList.add("pack-cut"),setTimeout(()=>{s.style.opacity="0",document.getElementById("pack-phase").style.display="none",y(0)},620)}a.addEventListener("pointerdown",l),window.addEventListener("pointermove",f),window.addEventListener("pointerup",b),a.addEventListener("touchstart",l,{passive:!0}),window.addEventListener("touchmove",f,{passive:!0}),window.addEventListener("touchend",b);let g=0,p=!1;const u=new Set;function y($){g=$,document.getElementById("reveal-phase").style.display="flex",w(),h($,0),T()}function w(){const $=document.getElementById("card-dots");$&&($.innerHTML=e.map((N,X)=>`<div class="card-dot" data-i="${X}" style="width:8px;height:8px;border-radius:50%;background:${X===g?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),$.querySelectorAll(".card-dot").forEach(N=>N.addEventListener("click",()=>m(parseInt(N.dataset.i)))))}function h($,N){var L,F;const X=e[$],ce=document.getElementById("card-counter"),fe=document.getElementById("card-track");ce&&(ce.textContent=`Carte ${$+1} / ${e.length}`);const C=document.getElementById("reveal-btns");C&&(C.style.display=$===e.length-1?"flex":"none");const Y=X.card_type==="player"&&((L=X.player)==null?void 0:L.rarity)==="legende",ue=!u.has($);u.add($);let K=0;if(X.card_type==="player"&&X.player){const U=X.player,O=U.job||"ATT";K=(Number(O==="GK"?U.note_g:O==="DEF"?U.note_d:O==="MIL"?U.note_m:U.note_a)||0)+(X.evolution_bonus||0)}const V=U=>{fe.innerHTML=`
        <div id="current-card-wrap" style="position:relative;display:flex;flex-direction:column;align-items:center;gap:8px;${Y?"filter:drop-shadow(0 0 20px #7a28b8)":""}">
          <div style="transform:scale(1.25);transform-origin:center">${br(X)}</div>
  ...
  <div id="match-history-panel">
    ...
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      ...
      ${_i(e.aiTeam,e.formation,null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.1))}
            ...
            </div>`}}catch(ee){console.error("[Ranked] MMR update error:",ee)}(ne=document.getElementById("pvp-end-overlay"))==null||ne.remove();const D=document.createElement("div");D.id="pvp-end-overlay",D.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;color:#fff;padding:24px;text-align:center;overflow-y:auto";const P=I?"🤝":j?"🏆":"😞",H=I?"MATCH NUL":j?"VICTOIRE !":"DÉFAITE",Q=I?"#fff":j?"#FFD700":"#ff6b6b";D.innerHTML=`
      ...
      <div style="font-size:18px">${m[p+"Name"]} ${z} – ${S} ${m[u+"Name"]}</div>
      ${_.forfeit?`<div style="font-size:13px;color:rgba(255,255,255,0.5)">${j?"L'adversaire a quitté":"Perdu par forfait"}</div>`:""}
      ${R}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${l?"🏆 Retour à la Mini League":a?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(D),(ie=D.querySelector("#pvp-end-home"))==null||ie.addEventListener("click",()=>{D.remove(),it(e),l?x("mini-league",{openLeagueId:l}):x(a?"ranked":"home")})}const Z=v.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},_=>{var S;const z=_.new;try{if(z.status==="finished"||z.forfeit){if(M)return;M=!0,k&&(clearInterval(k),k=null),Ue(),z.game_state&&(m=z.game_state),te(z);return}if(z.game_state){const j=m;m=z.game_state;const I=m._lastGC;if(I&&I.seq&&!W.has(I.seq)&&(W.add(I.seq),I.by!==p)){L(I.type,I.by,()=>Y());return}const R=j[p+"Score"]||0,D=j[u+"Score"]||0,P=m[p+"Score"]||0,H=m[u+"Score"]||0,Q=P>R,ne=H>D;if((Q||ne)&&!B.has(m.round)){B.add(m.round);const ie=[...m.log||[]].reverse().find(le=>le.isGoal),ee=((ie==null?void 0:ie.homePlayers)||[]).map(le=>({name:le.name,note:le.note,portrait:le.portrait,job:le.job}));!!((S=ie==null?void 0:ie.text)!=null&&S.includes("DERNIER CORNER"))?ze().then(()=>{Qe(ee,P,H,Q,()=>Y())}):Qe(ee,P,H,Q,()=>Y());return}Y()}}catch(j){console.error("[PvP] crash:",j)}}).subscribe();async function A(_){Object.assign(m,_),m.lastActionAt=new Date().toISOString();const{error:z}=await v.from("matches").update({game_state:m}).eq("id",i);z&&g("Erreur de synchronisation","error");try{Y()}catch(S){console.error("[PvP] renderPvpScreen crash:",S)}}async function $(){if(M)return;M=!0,zt(),Ue(),k&&(clearInterval(k),k=null);const _=n?h.away_id:h.home_id,z=n?"p2":"p1",S=n?"p1":"p2",j={...m,[z+"Score"]:3,[S+"Score"]:0,phase:"finished"},I=j.p1Score||0,R=j.p2Score||0;try{await v.from("matches").update({game_state:j}).eq("id",i);const{data:D,error:P}=await v.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:_,p_home_score:I,p_away_score:R,p_forfeit:!0});if(P&&console.error("[PvP] finish_pvp_match (forfeit):",P),l)try{await v.rpc("check_and_finish_mini_league",{p_league_id:l})}catch(H){console.error("[PvP] check_and_finish_mini_league (forfeit):",H)}try{const{data:H}=await v.rpc("apply_match_rewards",{p_match_id:i});H!=null&&H.success&&!(H!=null&&H.skipped)&&typeof t.refreshProfile=="function"&&await t.refreshProfile()}catch(H){console.error("[PvP] apply_match_rewards (forfeit):",H)}}catch{}try{v.removeChannel(Z)}catch{}setTimeout(()=>{it(e),l?x("mini-league",{openLeagueId:l}):x("home")},800)}const N={BOOST_STAT:({value:_=1,count:z=1,roles:S=[]},j,I)=>{const R=j[p+"Team"],D=Object.entries(R).filter(([P])=>!S.length||S.includes(P)).flatMap(([P,H])=>H.filter(Q=>!Q.used).map(Q=>({...Q,_line:P})));if(!D.length){j.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),I(j);return}X(D,z,`Choisir ${z} joueur(s) à booster (+${_})`,P=>{P.forEach(H=>{const Q=(R[H._line]||[]).find(ne=>ne.cardId===H.cardId);Q&&(Q.boost=(Q.boost||0)+_,j.log.push({text:`⚡ +${_} sur ${Q.name}`,type:"info"}))}),j[p+"Team"]=R,I(j)})},DEBUFF_STAT:({value:_=1,count:z=1,roles:S=[],target:j="ai"},I,R)=>{const D=j==="home"?p:u,P=I[D+"Team"],H=j==="home"?"allié":"adverse",Q=Object.entries(P).filter(([ne])=>!S.length||S.includes(ne)).flatMap(([ne,ie])=>ie.map(ee=>({...ee,_line:ne})));if(!Q.length){I.log.push({text:`🎯 Aucun joueur ${H}`,type:"info"}),R(I);return}X(Q,z,`Choisir ${z} joueur(s) ${H}(s) (-${_})`,ne=>{ne.forEach(ie=>{const ee=(P[ie._line]||[]).find(ae=>ae.cardId===ie.cardId);ee&&(ee.boost=(ee.boost||0)-_,I.log.push({text:`🎯 -${_} sur ${ee.name}`,type:"info"}))}),I[D+"Team"]=P,R(I)})},GRAY_PLAYER:({count:_=1,roles:z=[],target:S="ai"},j,I)=>{const R=S==="home"?p:u,D=j[R+"Team"],P=S==="home"?"allié":"adverse",H=Object.entries(D).filter(([Q])=>!z.length||z.includes(Q)).flatMap(([Q,ne])=>ne.filter(ie=>!ie.used).map(ie=>({...ie,_line:Q})));if(!H.length){j.log.push({text:`❌ Aucun joueur ${P}`,type:"info"}),I(j);return}X(H,_,`Choisir ${_} joueur(s) ${P}(s) à exclure`,Q=>{const ne="usedCardIds_"+R,ie=new Set(j[ne]||[]);Q.forEach(ee=>{const ae=(D[ee._line]||[]).find(le=>le.cardId===ee.cardId);ae&&(ae.used=!0,ie.add(ee.cardId),j.log.push({text:`❌ ${ae.name} exclu !`,type:"info"}))}),j[ne]=[...ie],j[R+"Team"]=D,I(j)})},REVIVE_PLAYER:({count:_=1,roles:z=[]},S,j)=>{const I=S[p+"Team"],R=Object.entries(I).filter(([D])=>!z.length||z.includes(D)).flatMap(([D,P])=>P.filter(H=>H.used).map(H=>({...H,_line:D})));if(!R.length){S.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),j(S);return}X(R,_,`Choisir ${_} joueur(s) à ressusciter`,D=>{D.forEach(P=>{const H=(I[P._line]||[]).find(Q=>Q.cardId===P.cardId);H&&(H.used=!1,S.log.push({text:`💫 ${H.name} ressuscité !`,type:"info"}))}),S[p+"Team"]=I,j(S)})},REMOVE_GOAL:({},_,z)=>{const S=u+"Score";_[S]>0?(_[S]--,_.log.push({text:"🚫 Dernier but annulé !",type:"info"})):_.log.push({text:"🚫 Aucun but à annuler",type:"info"}),z(_)},ADD_GOAL_DRAW:({},_,z)=>{_[p+"Score"]===_[u+"Score"]?(_[p+"Score"]++,_.log.push({text:"🎯 But bonus !",type:"info"})):_.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),z(_)},ADD_SUB:({value:_=1},z,S)=>{z.maxSubs=(z.maxSubs||3)+_,z.log.push({text:`🔄 +${_} remplacement(s)`,type:"info"}),S(z)},CUSTOM:({},_,z)=>z(_)};function X(_,z,S,j){const I=document.createElement("div");I.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let R=[];function D(){var H,Q;const P=_.map(ne=>{const ie=ne._line||ne.job||"MIL",ee=R.find(le=>le.cardId===ne.cardId),ae=Te({...ne,_evolution_bonus:0},{width:90,showStad:!0,role:ie,extraNote:ne.boost||0});return`<div class="pp-item" data-cid="${ne.cardId}"
        ...
        </div>`,(H=I.querySelector("#pp-close"))==null||H.addEventListener("click",()=>I.remove()),I.querySelectorAll(".pp-item").forEach(ne=>{ne.addEventListener("click",()=>{const ie=ne.dataset.cid,ee=_.find(le=>le.cardId===ie),ae=R.findIndex(le=>le.cardId===ie);ee&&(ae>-1?R.splice(ae,1):R.length<z&&R.push(ee),D())})}),(Q=I.querySelector("#pp-confirm"))==null||Q.addEventListener("click",()=>{I.remove(),j(R)})}D(),document.body.appendChild(I)}async function ce(_,z){const j=(m["gcCardsFull_"+p]||[]).find(P=>P.id===_),I=(j==null?void 0:j._gcDef)||(m.gcDefs||[]).find(P=>{var H;return P.name===z||((H=P.name)==null?void 0:H.toLowerCase().trim())===(z==null?void 0:z.toLowerCase().trim())}),R=[...m["usedGc_"+p]||[],_],D={type:z,by:p,seq:(m._gcAnimSeq||0)+1};W.add(D.seq),L(z,p,async()=>{if(I!=null&&I.effect_type&&I.effect_type!=="CUSTOM"){const H=N[I.effect_type];if(H){const Q={...m};H(I.effect_params||{},Q,async ne=>{ne["usedGc_"+p]=R,ne._lastGC=D,ne._gcAnimSeq=D.seq,await A(ne)});return}}const P={...m};switch(z){case"Remplacement+":P.maxSubs=(P.maxSubs||3)+1,P.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const H=u+"Score";P[H]>0&&(P[H]--,P.log.push({text:"🚫 But annulé",type:"info"}));break}}P["usedGc_"+p]=R,P._lastGC=D,P._gcAnimSeq=D.seq,await A(P)})}function fe(_,z){const S="usedCardIds_"+_,j=new Set(m[S]||[]);z.forEach(I=>j.add(I)),m[S]=[...j]}function C(){for(const _ of["p1","p2"]){const z=new Set(m["usedCardIds_"+_]||[]),S=m[_+"Team"];if(S)for(const j of["GK","DEF","MIL","ATT"])(S[j]||[]).forEach(I=>{I.used=z.has(I.cardId)})}}function Y(){var rn,an,sn,dn,ln,cn;if(m.phase==="reveal")return ue();if(m.phase==="midfield")return V();if(m.phase==="finished")return Ge();const _=m[p+"Team"],z=m[u+"Team"];C();const S=m[p+"Score"],j=m[u+"Score"],I=m[p+"Name"],R=m[u+"Name"],D=m.phase===p+"-attack",P=m.phase===p+"-defense",H=Array.isArray(m["selected_"+p])?m["selected_"+p]:[],Q=H.map(J=>J.cardId),ne=window.innerWidth>=700,ie=m[p+"Subs"]||[],ee=m["usedSubIds_"+p]||[],ae=ie.filter(J=>!ee.includes(J.cardId)),le=m["gcCardsFull_"+p]||[],ge=m["usedGc_"+p]||[],pe=le.filter(J=>!ge.includes(J.id)),xe=m.boostOwner===p&&!m.boostUsed,ye=[..._.MIL||[],..._.ATT||[]].filter(J=>!J.used),oe=[...z.MIL||[],...z.ATT||[]].filter(J=>!J.used),Ee=!ke(z),Re=ye.length===0&&oe.length===0&&!Ee,Le=(_.DEF||[]).filter(J=>!J.used),Ve=(_.GK||[]).filter(J=>!J.used);let Oe=[];D&&ye.length===0&&(Ee?(Oe=Le.map(J=>J.cardId),Le.length===0&&(Oe=Oe.concat(Ve.map(J=>J.cardId)))):Re&&(Oe=[...Le,...Ve].map(J=>J.cardId)));function nt(J,me,Ae){var Me,ft;const we=J._gcDef,qt=(we==null?void 0:we.name)||J.gc_type,at=(we==null?void 0:we.effect)||((Me=je[J.gc_type])==null?void 0:Me.desc)||"",Fe=we!=null&&we.image_url?`/icons/${we.image_url}`:null,Ne=((ft=je[J.gc_type])==null?void 0:ft.icon)||"⚡";return`<div class="pvp-gc-mini" data-gc-id="${J.id}" data-gc-type="${J.gc_type}" style="flex-shrink:0;cursor:pointer">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:${I};letter-spacing:3px;text-transform:uppercase;font-weight:700;animation:gcLabel .4s ease both">${ne} joue une carte</div>
      <div style="animation:gcFlipIn .7s cubic-bezier(.34,1.56,.64,1) both,gcGlow 1.8s ease infinite .7s">
        ${tt(R,P,H,D,{width:200})}
      </div>
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(ie);let ee=!1;const ae=()=>{ee||(ee=!0,ie.remove(),setTimeout(()=>S&&S(),50))};ie.addEventListener("click",ae),setTimeout(ae,3e3)}function F(_,z){var ne,ie,ee,ae;const j=(m["gcCardsFull_"+p]||[]).find(le=>le.id===_),I=j==null?void 0:j._gcDef,R=(I==null?void 0:I.name)||z,D=(I==null?void 0:I.effect)||((ne=je[z])==null?void 0:ne.desc)||"Carte spéciale.",P=I!=null&&I.image_url?`/icons/${I.image_url}`:null,H=((ie=je[z])==null?void 0:ie.icon)||"⚡",Q=document.createElement("div");Q.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",Q.innerHTML=`
      ${tt(R,P,H,D,{width:190})}
      <div style="display:flex;gap:12px;width:190px">
        <button id="pvp-gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
        <button id="pvp-gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
      </div>`,document.body.appendChild(Q),(ee=Q.querySelector("#pvp-gc-back"))==null||ee.addEventListener("click",()=>Q.remove()),(ae=Q.querySelector("#pvp-gc-use"))==null||ae.addEventListener("click",()=>{Q.remove(),ce(_,z)})}function U(){var j;const _=m[p+"Team"],z=Object.entries(_).flatMap(([I,R])=>(R||[]).filter(D=>!D.used).map(D=>({...D,_line:I})));if(!z.length)return;const S=document.createElement("div");S.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",S.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        <div id="pin-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
          ...
          <div>${pe?vt({...pe,used:!1,boost:0},xe,ye):"<div>—</div>"}</div>
      ...
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(_),(z=_.querySelector("#pvp-opp-close"))==null||z.addEventListener("click",()=>_.remove())}function G(){var S;const _=m.log||[],z=document.createElement("div");z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column",z.innerHTML=`
      ...
      </div>`,document.body.appendChild(z),(S=z.querySelector("#pvp-hist-close"))==null||S.addEventListener("click",()=>z.remove())}async function se(){if(m.phase!==p+"-attack")return;const _=p==="p1"?"p2":"p1",z=(m.round||0)+1,S=[...m.log||[]];S.push({type:"info",text:`⏭️ ${m[p+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const j=$e(m),I=ct(_),R=j||!I?"finished":_+"-attack";await A({["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},pendingAttack:null,phase:R,attacker:_,round:z,log:S}),R==="finished"&&await qe(m)}function de(_){const z=(_.GK||[]).some(j=>!j.used),S=["DEF","MIL","ATT"].some(j=>(_[j]||[]).some(I=>!I.used));return z&&!S}function ve(_){return(_.p1Score||0)!==(_.p2Score||0)?null:de(_.p1Team)&&!ke(_.p2Team)?{side:"p1",gk:(_.p1Team.GK||[]).find(z=>!z.used)}:de(_.p2Team)&&!ke(_.p1Team)?{side:"p2",gk:(_.p2Team.GK||[]).find(z=>!z.used)}:null}async function ze(){const _=document.createElement("div");_.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:950;display:flex;align-items:center;justify-content:center;overflow:hidden;text-align:center;padding:24px",document.body.appendChild(_);const z=(S,j)=>new Promise(I=>{_.innerHTML=`<div style="font-size:32px;font-weight:900;color:${j};letter-spacing:2px;animation:lcFade 1.4s ease both">${S}</div>
      <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(I,1400)});await z("⚽ DERNIER CORNER","#FFD700"),await z("🧤 LE GARDIEN MONTE !","#4fc3f7"),_.remove()}async function he(_,z){const{side:S,gk:j}=_;if(!j)return;const I=S+"Score",R=(z[I]||0)+1;j.used=!0;const D=[...z.log||[]];D.push({type:"duel",isGoal:!0,homeScored:S===p,homePlayers:S==="p1"?[Dt(j)]:[],aiPlayers:S==="p2"?[Dt(j)]:[],text:`⚽ DERNIER CORNER — Le gardien de ${z[S+"Name"]} marque !`});const P=(z.round||0)+1,H=S==="p1"?"p2":"p1",Q={...z,[I]:R},ne=$e(Q);B.add(P),await ze();const ie=S===p?R:z[p+"Score"]||0,ee=S===p?z[u+"Score"]||0:R;Qe([Dt(j)],ie,ee,S===p,async()=>{await A({[I]:R,log:D,round:P,pendingAttack:null,phase:ne?"finished":H+"-attack",attacker:H,["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}}}),ne&&await qe({...m,[I]:R})})}async function Ie(){const _=m[p+"Team"],z=!["GK","DEF","MIL","ATT"].some(R=>(m[u+"Team"][R]||[]).some(D=>!D.used)),S=(m["selected_"+p]||[]).map(R=>{const D=(_[R._role]||[]).find(le=>le.cardId===R.cardId)||R,P=["GK","DEF"].includes(R._role),H=_[R._role]||[],Q=H.findIndex(le=>le.cardId===R.cardId),ne=xt(H.length),ie=Q>=0?ne[Q]:D._col??1,ee=m.stadiumDef||m.homeStadiumDef||null,ae=D.stadiumBonus||ee&&(ee.club_id&&String(D.club_id)===String(ee.club_id)||ee.country_code&&D.country_code===ee.country_code)||!1;return{...D,_line:R._role,_col:ie,stadiumBonus:ae,...P?{note_a:Math.max(1,Number(D.note_a)||0)}:{}}});if(!S.length)return;const j=oi(S,m.modifiers[p]||{},m[p+"Formation"]);fe(p,S.map(R=>R.cardId)),S.forEach(R=>{const D=(_[R._role]||[]).find(P=>P.cardId===R.cardId);D&&(D.used=!0)}),m["selected_"+p]=[],Y();const I=[...m.log||[]];if(z){if(S.length===1&&(S[0]._line==="GK"||S[0].job==="GK")&&(m.p1Score||0)===(m.p2Score||0)){await he({side:p,gk:S[0]},{...m,[p+"Team"]:_,log:I});return}const D=(m[p+"Score"]||0)+1,P=S.map(ee=>Dt(ee));I.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:P,homeTotal:j.total,aiTotal:0});const H=(m.round||0)+1,Q=p==="p1"?"p2":"p1",ne={...m,[p+"Team"]:_,[p+"Score"]:D},ie=$e(ne);B.add(H),Qe(P,D,m[u+"Score"]||0,!0,async()=>{await A({[p+"Team"]:_,[p+"Score"]:D,["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},pendingAttack:null,phase:ie?"finished":Q+"-attack",attacker:Q,round:H,log:I}),ie&&await qe({...m,[p+"Score"]:D})});return}I.push({type:"pending",text:`⚔️ ${m[p+"Name"]} attaque (${j.total})`}),await A({[p+"Team"]:_,[u+"Team"]:m[u+"Team"],pendingAttack:{...j,players:S,side:p},["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},phase:u+"-defense",log:I})}async function Se(){const _=m[p+"Team"],z=(m["selected_"+p]||[]).map(pe=>{const xe=(_[pe._role]||[]).find(Le=>Le.cardId===pe.cardId)||pe,ye=_[pe._role]||[],oe=ye.findIndex(Le=>Le.cardId===pe.cardId),Ee=xt(ye.length),Re=oe>=0?Ee[oe]:xe._col??1;return{...xe,_line:pe._role,_col:Re}}),S=ri(z,m.modifiers[p]||{},m[p+"Formation"]);fe(p,z.map(pe=>pe.cardId)),z.forEach(pe=>{const xe=(_[pe._role]||[]).find(ye=>ye.cardId===pe.cardId);xe&&(xe.used=!0)}),m["selected_"+p]=[],Y();const j=Qi(m.pendingAttack.total,S.total,m.modifiers[p]||{}),I=m.pendingAttack.side,R=j==="attack"||(j==null?void 0:j.goal),D=I==="p1"?"p2":"p1",P=(m.round||0)+1,H=(m.pendingAttack.players||[]).map(pe=>Dt(pe)),Q=[...m.log||[]];Q.push({type:"duel",isGoal:R,homeScored:R&&I===p,text:R?`⚽ BUT de ${m[I+"Name"]} ! (${m.pendingAttack.total} vs ${S.total})`:`✋ Attaque stoppée (${m.pendingAttack.total} vs ${S.total})`,homePlayers:H,aiPlayers:z.map(pe=>Dt(pe)),homeTotal:m.pendingAttack.total,aiTotal:S.total});const ne=R?(m[I+"Score"]||0)+1:m[I+"Score"]||0,ie={...m,[p+"Team"]:_,[I+"Score"]:ne},ee=$e(ie),ae=ee?"finished":D+"-attack",le=!ee&&!R?ve(ie):null,ge=async()=>{if(le){await he(le,ie);return}await A({[p+"Team"]:_,[u+"Team"]:m[u+"Team"],[I+"Score"]:ne,["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},pendingAttack:null,phase:ae,attacker:D,round:P,log:Q}),(ae==="finished"||ee)&&await qe({...m,[I+"Score"]:ne})};if(R){const pe=I===p,xe=pe?ne:m[p+"Score"]||0,ye=pe?m[u+"Score"]||0:ne;B.add(P),Qe(H,xe,ye,pe,ge)}else await ge()}function Be(_){return["MIL","ATT"].some(z=>(_[z]||[]).some(S=>!S.used))}function ke(_){return["GK","DEF","MIL","ATT"].some(z=>(_[z]||[]).some(S=>!S.used))}function _t(_){return ke(_)&&!Be(_)}function ct(_){const z=m[_+"Team"],S=m[(_==="p1"?"p2":"p1")+"Team"];return!!(Be(z)||_t(z)&&!ke(S)||_t(z)&&!Be(S)&&ke(S))}function $e(_){return!ke(_.p1Team)&&!ke(_.p2Team)}function Ce(_){const z=_.p1Score||0,S=_.p2Score||0;return z===S?null:z>S?h.home_id:h.away_id}async function qe(_){zt(),Ue();try{const z=Ce(_),S=z?h.home_id===z?h.away_id:h.home_id:null,j=_.p1Score||0,I=_.p2Score||0,{data:R,error:D}=await v.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:z,p_home_score:j,p_away_score:I,p_forfeit:!1});if(D&&console.error("[PvP] finish_pvp_match:",D),l)try{await v.rpc("check_and_finish_mini_league",{p_league_id:l})}catch(P){console.error("[PvP] check_and_finish_mini_league:",P)}z&&S&&Kr(z,S).catch(()=>{});try{const{data:P}=await v.rpc("apply_match_rewards",{p_match_id:i});P!=null&&P.success&&!(P!=null&&P.skipped)&&typeof t.refreshProfile=="function"&&await t.refreshProfile()}catch(P){console.error("[PvP] apply_match_rewards:",P)}if(typeof c=="function")try{await c({homeScore:j,awayScore:I,winnerId:z,homeId:h.home_id,awayId:h.away_id,matchId:i})}catch(P){console.error("[PvP] onMatchEnd:",P)}}catch(z){console.error("[PvP] finishMatch:",z)}}function Ge(){var I;const _=m[p+"Score"],z=m[u+"Score"],S=_>z,j=_===z;st(e),e.innerHTML=`
    ...
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:${Jt};border:1px solid ${Qt};border-radius:12px;padding:16px">
        ...
        ${f.map(A=>`
          <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid ${Lt}">
            <div style="width:36px;height:36px;border-radius:50%;background:${A.club_color2||oo};display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:900;color:${A.club_color1||"#fff"}">${(A.pseudo||"?").slice(0,2).toUpperCase()}</div>
      ...
      <div style="background:${Jt};border:1px solid ${Qt};border-radius:12px;padding:16px">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>${b.map((A,$)=>{const N=g>0&&a.status==="finished"?$===0?Math.floor(g*.7):$===1?Math.floor(g*.2):$===2?Math.floor(g*.1):0:0;return`<tr style="border-bottom:1px solid ${Lt};${A.userId===r?"background:rgba(74,222,128,0.08);":""}">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
  ...
  </div>`;let d="buy";const c=()=>{var p,u,y,w,h,E,m;return{name:(((p=document.getElementById("flt-name"))==null?void 0:p.value)||"").toLowerCase().trim(),club:(((u=document.getElementById("flt-club"))==null?void 0:u.value)||"").toLowerCase().trim(),country:(((y=document.getElementById("flt-country"))==null?void 0:y.value)||"").toLowerCase().trim(),job:((w=document.getElementById("flt-job"))==null?void 0:w.value)||"",rarity:((h=document.getElementById("flt-rarity"))==null?void 0:h.value)||"",note1:parseInt((E=document.getElementById("flt-note1"))==null?void 0:E.value)||0,note2:parseInt((m=document.getElementById("flt-note2"))==null?void 0:m.value)||0}};function l(p){const u=c();return p.filter(y=>{var B,W,te;const w=(B=y.card)==null?void 0:B.player;if(!w)return!1;const h=`${w.firstname} ${w.surname_real}`.toLowerCase(),E=(((W=w.clubs)==null?void 0:W.encoded_name)||"").toLowerCase(),m=(w.country_code||"").toLowerCase(),M=((te=y.card)==null?void 0:te.evolution_bonus)||0,k=Ft(w,w.job,M),T=w.job2?Ft(w,w.job2,M):0;return!(u.name&&!h.includes(u.name)||u.club&&!E.includes(u.club)||u.country&&!m.includes(u.country)||u.job&&w.job!==u.job||u.rarity&&w.rarity!==u.rarity||u.note1&&k<u.note1||u.note2&&T<u.note2)})}function f(p){var T,B,W,te;const u=(T=p.card)==null?void 0:T.player;if(!u)return"";const y=((B=p.card)==null?void 0:B.evolution_bonus)||0,w=Ft(u,u.job,y),h=u.job2?Ft(u,u.job2,y):0,E=(i.profile.credits||0)>=p.price,m=u.country_code?`https://flagsapi.com/${u.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,M=ui[u.job]||"#bbb",k=u.job2?ui[u.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden">
      <!-- Drapeau -->
    ...
    </div>`}function b(p){var T,B,W,te;const u=(T=p.card)==null?void 0:T.player;if(!u)return"";const y=((B=p.card)==null?void 0:B.evolution_bonus)||0,w=Ft(u,u.job,y),h=u.job2?Ft(u,u.job2,y):0,E=p.status==="sold",m=u.country_code?`https://flagsapi.com/${u.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,M=ui[u.job]||"#bbb",k=u.job2?ui[u.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${E?"opacity:0.7":""}">
      ${m?`<img src="${m}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      ${(W=u.clubs)!=null&&W.logo_url?`<img src="${u.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${M};display:flex;align-items:center;justify-content:center">
    ...
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center">
        ...
        <button id="buy-ok" style="flex:1;padding:12px;border-radius:10px;border:none;background:var(--green);color:#fff;font-size:14px;font-weight:900;cursor:pointer">Confirmer</button>
          ...
          <div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${A?"border:2px solid var(--yellow)":""}">
            ...
            <div style="flex:1;min-width:0">
              ...
              <div style="font-size:11px;color:var(--tile-fg-dim)">${T.club_name} · ${W.label}</div>
            </div>
            <div style="text-align:right;flex-shrink:0">
          ...
          </div>`}).join(""):'<div style="text-align:center;color:var(--tile-fg-dim);padding:40px">Aucun joueur classé.</div>'}else if(d==="mini-league"){const k=a||[];h.innerHTML=k.length>0?k.map((T,B)=>`
      ...
      `).join(""):`<div style="text-align:center;color:var(--tile-fg-dim);padding:40px">Aucun résultat de Mini League pour l'instant.</div>`}else{const k=s||[];h.innerHTML=k.length>0?k.map((T,B)=>`
        <div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${T.user_id===i.profile.id?"border:2px solid var(--yellow)":""}">
          <div style="width:32px;height:32px;border-radius:50%;background:${c(B)};color:${l(B)};display:flex;align-items:center;justify-content:center;font-weight:900;flex-shrink:0;font-size:${B<3?"16":"13"}px">${B<3?["🥇","🥈","🥉"][B]:B+1}</div>
          <div style="flex:1;min-width:0">
            <div style="font-weight:700;color:var(--tile-fg-on-page);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${T.pseudo}</div>
  ...
  <div class="page">
    ...
    <nav class="top-nav">
      <div class="logo" id="nav-logo" title="Manager Wars v2026.07.23-1533" style="cursor:pointer">
