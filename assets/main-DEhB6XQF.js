import{s as I,F as Nt,h as yt,j as yi,l as Xe,i as Oi,k as Pi,b as Hi}from"./formation-links-CEzN0rcz.js";function ai(e,{navigate:t,toast:i}){e.innerHTML=`
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),o=(l||[]).filter(z=>z.card_type==="player"&&z.player),r=(l||[]).filter(z=>z.card_type==="game_changer"),u=(l||[]).filter(z=>z.card_type==="formation"),{data:f}=await I.from("gc_definitions").select("name,gc_type,color,effect,image_url"),g={};(f||[]).forEach(z=>{g[z.name]=z});const y=Object.keys(Nt),d=Object.keys(Ce),x={};o.forEach(z=>{const S=z.player.id;x[S]=(x[S]||0)+1}),new Set(Object.keys(x).map(z=>String(z)));const $=new Set(u.map(z=>z.formation)),_=new Set(r.map(z=>z.gc_type));let M="player",G="Tous",k="",v=!1;function F(){return[...o].sort((z,S)=>{const R=ht.indexOf(z.player.job),C=ht.indexOf(S.player.job);return R!==C?R-C:(z.player.surname_encoded||"").localeCompare(S.player.surname_encoded||"")})}function Z(){return[...p||[]].sort((z,S)=>{const R=ht.indexOf(z.job),C=ht.indexOf(S.job);return R!==C?R-C:(z.surname_encoded||"").localeCompare(S.surname_encoded||"")})}function ne(){return F().filter(z=>{const S=z.player,R=G==="Tous"||S.job===G,C=!k||`${S.firstname} ${S.surname_encoded}`.toLowerCase().includes(k);return R&&C})}function ee(){return Z().filter(z=>{const S=G==="Tous"||z.job===G,R=!k||`${z.firstname} ${z.surname_encoded}`.toLowerCase().includes(k);return S&&R})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",S=>{k=S.target.value.toLowerCase(),q()}),e.querySelectorAll(".filter-btn").forEach(S=>{S.addEventListener("click",()=>{G=S.dataset.job,w(),q()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{v=!v,w(),q()})):(z.innerHTML=`
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const dt=1e3;function eo(e,t,i,n){var d,x,$;const{state:c,toast:s,closeModal:a,navigate:l,refreshProfile:p}=i,o=e.formation,r={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function u(){const _=yt[o]||{},M=Nt[o]||[],G=290,k=360,v=20;function F(ne){const ee=_[ne];return ee?{x:ee.x*G,y:ee.y*k}:null}let Z=`<svg width="${G}" height="${k}" viewBox="0 0 ${G} ${k}" xmlns="http://www.w3.org/2000/svg">`;for(const[ne,ee]of M){const w=F(ne),q=F(ee);!w||!q||(Z+=`<line x1="${w.x}" y1="${w.y}" x2="${q.x}" y2="${q.y}"
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;height:${u}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ...
      ${c?`<img src="${c}" style="width:${u-4}px;height:${u-4}px;object-fit:contain">`:e!=null&&e.clubName?`<span style="font-size:${Math.max(4,u-8)}px;font-weight:700;color:#333">${(e.clubName||"").slice(0,3).toUpperCase()}</span>`:""}
        ...
        stroke="${ee}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const g=48,y=64,d=13,x=16,$={normal:"#aaaaaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#111111",legende:"#7a28b8",légende:"#7a28b8"};for(const[G,k]of Object.entries(o)){const v=u(G);if(!v||!k)continue;const F=G.replace(/[0-9]/g,""),Z=rt[F]||"#555",ne=a.includes(k.cardId),ee=i==="attack"&&(["MIL","ATT"].includes(F)||ne)&&!k.used||i==="defense"&&["GK","DEF","MIL"].includes(F)&&!k.used,w=n.includes(k.cardId);let q;i==="attack"?q=ne?Math.max(1,Number(k.note_a)||0):F==="MIL"?Number(k.note_m)||0:Number(k.note_a)||0:i==="defense"?q=F==="GK"?Number(k.note_g)||0:F==="MIL"?Number(k.note_m)||0:Number(k.note_d)||0:q=Number(F==="GK"?k.note_g:F==="DEF"?k.note_d:F==="MIL"?k.note_m:k.note_a)||0,q=q+(k.boost||0);const V=(v.x-g/2).toFixed(1),b=(v.y-y/2).toFixed(1),U=$[k==null?void 0:k.rarity]||$.normal;if(k.used){const S=typeof import.meta<"u"&&"/manager-wars/"||"/",C=`${typeof window<"u"&&((M=window.location)==null?void 0:M.origin)||""}${S}icons/carte-dos.png`.replace(/([^:])\/\//g,"$1/");f+=`<rect x="${V}" y="${b}" width="${g}" height="${y}" rx="5" fill="#161616"/>`,f+=`<image href="${C}" xlink:href="${C}" x="${V}" y="${b}" width="${g}" height="${y}" preserveAspectRatio="xMidYMid slice"/>`,f+=`<rect x="${V}" y="${b}" width="${g}" height="${y}" rx="5" fill="none" stroke="${U}" stroke-width="2" opacity="0.7"/>`,f+=`<rect x="${V}" y="${b}" width="${g}" height="${y}" rx="5" fill="rgba(0,0,0,0.01)" class="match-used-hit" data-card-id="${k.cardId}" data-role="${F}" style="cursor:pointer"/>`;continue}const W=w?.45:1,re=w?"#FFD700":U,ce=w?3:(k==null?void 0:k.rarity)==="legende"||(k==null?void 0:k.rarity)==="légende"||(k==null?void 0:k.rarity)==="pepite"||(k==null?void 0:k.rarity)==="pépite"?2.5:2,se=y-d-x;f+=`<defs><clipPath id="cp-${G}"><rect x="${V}" y="${(v.y-y/2+d).toFixed(1)}" width="${g}" height="${se}"/></clipPath></defs>`,f+=`<rect x="${V}" y="${b}" width="${g}" height="${y}" rx="5" fill="${Z}" opacity="${W}"/>`;const ae=Te(k);ae&&(f+=`<image href="${ae}" xlink:href="${ae}" x="${V}" y="${(v.y-y/2+d).toFixed(1)}" width="${g}" height="${se}" clip-path="url(#cp-${G})" preserveAspectRatio="xMidYMin slice"/>`),f+=`<rect x="${V}" y="${b}" width="${g}" height="${d}" rx="4" fill="rgba(255,255,255,0.92)"/>`,f+=`<text x="${v.x.toFixed(1)}" y="${(v.y-y/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(k.name||"").slice(0,9)}</text>`;const z=(v.y+y/2-x).toFixed(1);f+=`<rect x="${V}" y="${z}" width="${g}" height="${x}" fill="rgba(255,255,255,0.92)"/>`;{const S=Dt(k.country_code);S?f+=`<image href="${S}" xlink:href="${S}" x="${(v.x-20).toFixed(1)}" y="${(v.y+y/2-x+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:f+=`<text x="${(v.x-13).toFixed(1)}" y="${(v.y+y/2-x/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${Bi(k.country_code)}</text>`,f+=`<text x="${v.x.toFixed(1)}" y="${(v.y+y/2-x/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${q}</text>`;const R=ji(k);R?f+=`<image href="${R}" xlink:href="${R}" x="${(v.x+g/2-14).toFixed(1)}" y="${(v.y+y/2-x+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:k.clubName&&(f+=`<text x="${(v.x+14).toFixed(1)}" y="${(v.y+y/2-x/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(k.clubName||"").slice(0,3).toUpperCase()}</text>`),k.boost&&(f+=`<rect x="${(v.x+g/2-12).toFixed(1)}" y="${(v.y-y/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,f+=`<text x="${(v.x+g/2-5).toFixed(1)}" y="${(v.y-y/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${k.boost}</text>`)}f+=`<rect x="${V}" y="${b}" width="${g}" height="${y}" rx="5" fill="${w?"rgba(255,255,255,0.12)":"none"}" stroke="${re}" stroke-width="${ce}" opacity="${W}"/>`,ee&&(f+=`<rect x="${V}" y="${b}" width="${g}" height="${y}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${w?"selected":""}" data-card-id="${k.cardId}" data-role="${F}" style="cursor:pointer"/>`)}const _=38;return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-_} ${-_} ${c+_*2} ${s+_*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${f}
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${d.map((M,G)=>{const k=G<d.length-1?Xe(M,d[G+1]):null,v=!k||k==="#ff3333"||k==="#cc2222",F=k==="#00ff88"?"+2":k==="#FFD700"?"+1":"";return`
          ...
          ${G<d.length-1?`<div class="duel-link duel-link-${_}" data-idx="${G}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${v?"rgba(255,255,255,0.12)":k};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${v?"none":`0 0 8px ${k}`}">
  ...
  <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div id="duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center, rgba(10,61,30,0.4), rgba(10,61,30,0.92))"></div>
          ...
          <button id="start-match-btn" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,0.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">
            ▶ Commencer le match
          </button>`,k.style.transition="opacity .45s ease",k.style.opacity="1",k.style.pointerEvents="auto",(v=document.getElementById("start-match-btn"))==null||v.addEventListener("click",y))},600)},700)},2800)}function So(e){if(e.type==="duel"&&(e.homeTotal!=null||e.aiTotal!=null)){const t=(e.homeTotal||0)>=(e.aiTotal||0);return`
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(s);let l=!1;const p=()=>{l||(l=!0,s.remove(),setTimeout(()=>c(),50))};s.addEventListener("click",p),setTimeout(p,3500)}async function ft(e,t,i){var r,u;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:n}=i,c=t.homeScore>t.aiScore,s=t.homeScore===t.aiScore,a=c?"victoire":s?"nul":"defaite",l=Wi(t.mode,a);t.matchId&&await I.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:c?n.profile.id:null,home_credits_reward:l,played_at:new Date().toISOString()}).eq("id",t.matchId);const p={credits:(n.profile.credits||0)+l,matches_played:(n.profile.matches_played||0)+1};c?p.wins=(n.profile.wins||0)+1:s?p.draws=(n.profile.draws||0)+1:p.losses=(n.profile.losses||0)+1,await I.from("users").update(p).eq("id",n.profile.id),await i.refreshProfile();const o=document.createElement("div");o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",o.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const g=async(M=!0)=>{u=!0,f&&(I.removeChannel(f),f=null),M&&await I.rpc("cancel_matchmaking",{p_user_id:p.profile.id}).catch(()=>{})};(_=document.getElementById("mm-cancel"))==null||_.addEventListener("click",async()=>{try{await g(!0)}catch{}Re(e),o("home")});const y=async(M,G,k)=>{if(u)return;u=!0,f&&(I.removeChannel(f),f=null);const v=document.getElementById("mm-status");v&&(v.textContent="Adversaire trouvé !"),await new Promise(F=>setTimeout(F,600)),e.isConnected&&Uo(e,t,M,k,a,l)},{data:d,error:x}=await I.rpc("try_matchmake",{p_user_id:p.profile.id,p_deck_id:i});if(x||!(d!=null&&d.success)){r("Erreur de matchmaking","error"),Re(e),o("home");return}if(d.matched){y(d.match_id,d.opponent_id,!1);return}const $=document.getElementById("mm-status");$&&($.textContent="En attente d'un autre joueur..."),f=I.channel("matchmaking-"+p.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${p.profile.id}`},M=>{const G=M.new;G.status==="matched"&&G.match_id&&y(G.match_id,G.matched_with,!0)}).subscribe()}async function Uo(e,t,i,n,c=[],s=[]){const{state:a,navigate:l,toast:p}=t,o=n?"p1":"p2",r=n?"p2":"p1",u=(c||[]).map(m=>m.id),f=(c||[]).map(m=>({id:m.id,gc_type:m.gc_type,_gcDef:m._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:g}=await I.from("matches").select("*").eq("id",i).single();if(!g){p("Match introuvable","error"),l("home");return}async function y(){const[{data:m},{data:h},{data:T},{data:A}]=await Promise.all([I.rpc("get_deck_for_match",{p_deck_id:g.home_deck_id}),I.rpc("get_deck_for_match",{p_deck_id:g.away_deck_id}),I.from("users").select("id,pseudo,club_name").eq("id",g.home_id).single(),I.from("users").select("id,pseudo,club_name").eq("id",g.away_id).single()]),E=B=>({cardId:B.card_id,position:B.position,id:B.id,firstname:B.firstname,name:B.surname_encoded,country_code:B.country_code,club_id:B.club_id,job:B.job,job2:B.job2,note_g:Number(B.note_g)||0,note_d:Number(B.note_d)||0,note_m:Number(B.note_m)||0,note_a:Number(B.note_a)||0,rarity:B.rarity,skin:B.skin,hair:B.hair,hair_length:B.hair_length,clubName:B.club_encoded_name||null,clubLogo:B.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}),L=((m==null?void 0:m.starters)||[]).map(B=>E(B)),j=((h==null?void 0:h.starters)||[]).map(B=>E(B)),D=(m==null?void 0:m.formation)||"4-4-2",P=(h==null?void 0:h.formation)||"4-4-2";return{p1Team:kt(L,D),p2Team:kt(j,P),p1Subs:((m==null?void 0:m.subs)||[]).map(B=>E(B)),p2Subs:((h==null?void 0:h.subs)||[]).map(B=>E(B)),p1Formation:D,p2Formation:P,p1Name:(T==null?void 0:T.club_name)||(T==null?void 0:T.pseudo)||"Joueur 1",p2Name:(A==null?void 0:A.club_name)||(A==null?void 0:A.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:n?u:[],gc_p2:n?[]:u,gcCardsFull_p1:n?f:[],gcCardsFull_p2:n?[]:f,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:s||[],lastActionAt:new Date().toISOString()}}let d=g.game_state&&Object.keys(g.game_state).length?g.game_state:null;if(!d)if(n){d=await y();const{data:m}=await I.from("matches").select("game_state").eq("id",i).single();!(m!=null&&m.game_state)||!Object.keys(m.game_state).length?await I.from("matches").update({game_state:d,turn_user_id:g.home_id}).eq("id",i):d=m.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let m=0;m<30&&!d;m++){await new Promise(T=>setTimeout(T,400));const{data:h}=await I.from("matches").select("game_state").eq("id",i).single();h!=null&&h.game_state&&Object.keys(h.game_state).length&&(d=h.game_state)}if(!d){p("Erreur de synchronisation","error"),l("home");return}d.gc_p2=u,d.gcCardsFull_p2=f,await I.from("matches").update({game_state:d}).eq("id",i)}let x=!1,$=null,_=!1;const M=new Set,G=new Set;function k(m){var B,H;try{I.removeChannel(v)}catch{}const h=d[o+"Score"]||0,T=d[r+"Score"]||0;let A,E;m.winner_id?(A=m.winner_id===a.profile.id,E=!1):m.forfeit?(A=h>T,E=!1):(E=h===T,A=h>T),(B=document.getElementById("pvp-end-overlay"))==null||B.remove();const L=document.createElement("div");L.id="pvp-end-overlay",L.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const j=E?"🤝":A?"🏆":"😞",D=E?"MATCH NUL":A?"VICTOIRE !":"DÉFAITE",P=E?"#fff":A?"#FFD700":"#ff6b6b";L.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(L),(H=L.querySelector("#pvp-end-home"))==null||H.addEventListener("click",()=>{L.remove(),Re(e),l("home")})}const v=I.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},m=>{const h=m.new;try{if(h.status==="finished"||h.forfeit){if(x)return;x=!0,$&&(clearInterval($),$=null),h.game_state&&(d=h.game_state),k(h);return}if(h.game_state){const T=d;d=h.game_state;const A=d._lastGC;if(A&&A.seq&&!G.has(A.seq)&&(G.add(A.seq),A.by!==o)){se(A.type,A.by,()=>b());return}const E=T[o+"Score"]||0,L=T[r+"Score"]||0,j=d[o+"Score"]||0,D=d[r+"Score"]||0,P=j>E,B=D>L;if((P||B)&&!M.has(d.round)){M.add(d.round);const H=[...d.log||[]].reverse().find(Q=>Q.isGoal),K=((H==null?void 0:H.homePlayers)||[]).map(Q=>({name:Q.name,note:Q.note,portrait:Q.portrait,job:Q.job}));ce(K,j,D,P,()=>b());return}b()}}catch(T){console.error("[PvP] crash:",T)}}).subscribe();async function F(m){Object.assign(d,m),d.lastActionAt=new Date().toISOString();const{error:h}=await I.from("matches").update({game_state:d}).eq("id",i);h&&p("Erreur de synchronisation","error");try{b()}catch(T){console.error("[PvP] renderPvpScreen crash:",T)}}async function Z(){if(x)return;x=!0,$&&(clearInterval($),$=null);const m=n?g.away_id:g.home_id,h=n?"p2":"p1",T=n?"p1":"p2",A={...d,[h+"Score"]:3,[T+"Score"]:0,phase:"finished"};try{await I.from("matches").update({status:"finished",forfeit:!0,winner_id:m,home_score:A.p1Score||0,away_score:A.p2Score||0,game_state:A}).eq("id",i)}catch{}try{I.removeChannel(v)}catch{}setTimeout(()=>{Re(e),l("home")},800)}const ne={BOOST_STAT:({value:m=1,count:h=1,roles:T=[]},A,E)=>{const L=A[o+"Team"],j=Object.entries(L).filter(([D])=>!T.length||T.includes(D)).flatMap(([D,P])=>P.filter(B=>!B.used).map(B=>({...B,_line:D})));if(!j.length){A.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),E(A);return}ee(j,h,`Choisir ${h} joueur(s) à booster (+${m})`,D=>{D.forEach(P=>{const B=(L[P._line]||[]).find(H=>H.cardId===P.cardId);B&&(B.boost=(B.boost||0)+m,A.log.push({text:`⚡ +${m} sur ${B.name}`,type:"info"}))}),A[o+"Team"]=L,E(A)})},DEBUFF_STAT:({value:m=1,count:h=1,roles:T=[],target:A="ai"},E,L)=>{const j=A==="home"?o:r,D=E[j+"Team"],P=A==="home"?"allié":"adverse",B=Object.entries(D).filter(([H])=>!T.length||T.includes(H)).flatMap(([H,K])=>K.filter(Q=>!Q.used).map(Q=>({...Q,_line:H})));if(!B.length){E.log.push({text:`🎯 Aucun joueur ${P}`,type:"info"}),L(E);return}ee(B,h,`Choisir ${h} joueur(s) ${P}(s) (-${m})`,H=>{H.forEach(K=>{const Q=(D[K._line]||[]).find(ue=>ue.cardId===K.cardId);Q&&(Q.boost=(Q.boost||0)-m,E.log.push({text:`🎯 -${m} sur ${Q.name}`,type:"info"}))}),E[j+"Team"]=D,L(E)})},GRAY_PLAYER:({count:m=1,roles:h=[],target:T="ai"},A,E)=>{const L=T==="home"?o:r,j=A[L+"Team"],D=T==="home"?"allié":"adverse",P=Object.entries(j).filter(([B])=>!h.length||h.includes(B)).flatMap(([B,H])=>H.filter(K=>!K.used).map(K=>({...K,_line:B})));if(!P.length){A.log.push({text:`❌ Aucun joueur ${D}`,type:"info"}),E(A);return}ee(P,m,`Choisir ${m} joueur(s) ${D}(s) à exclure`,B=>{B.forEach(H=>{const K=(j[H._line]||[]).find(Q=>Q.cardId===H.cardId);K&&(K.used=!0,A.log.push({text:`❌ ${K.name} exclu !`,type:"info"}))}),A[L+"Team"]=j,E(A)})},REVIVE_PLAYER:({count:m=1,roles:h=[]},T,A)=>{const E=T[o+"Team"],L=Object.entries(E).filter(([j])=>!h.length||h.includes(j)).flatMap(([j,D])=>D.filter(P=>P.used).map(P=>({...P,_line:j})));if(!L.length){T.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),A(T);return}ee(L,m,`Choisir ${m} joueur(s) à ressusciter`,j=>{j.forEach(D=>{const P=(E[D._line]||[]).find(B=>B.cardId===D.cardId);P&&(P.used=!1,T.log.push({text:`💫 ${P.name} ressuscité !`,type:"info"}))}),T[o+"Team"]=E,A(T)})},REMOVE_GOAL:({},m,h)=>{const T=r+"Score";m[T]>0?(m[T]--,m.log.push({text:"🚫 Dernier but annulé !",type:"info"})):m.log.push({text:"🚫 Aucun but à annuler",type:"info"}),h(m)},ADD_GOAL_DRAW:({},m,h)=>{m[o+"Score"]===m[r+"Score"]?(m[o+"Score"]++,m.log.push({text:"🎯 But bonus !",type:"info"})):m.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),h(m)},ADD_SUB:({value:m=1},h,T)=>{h.maxSubs=(h.maxSubs||3)+m,h.log.push({text:`🔄 +${m} remplacement(s)`,type:"info"}),T(h)},CUSTOM:({},m,h)=>h(m)};function ee(m,h,T,A){const E=document.createElement("div");E.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let L=[];function j(){var P,B;const D=m.map(H=>{const K={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[H._line]||"#555",Q=me(H,H._line)+(H.boost||0),we=L.find(J=>J.cardId===H.cardId)?"#FFD700":"rgba(255,255,255,0.25)",X=H.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${H.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${we};background:${K};overflow:hidden;cursor:pointer;${X}">
    ...
    </div>`,o==="p1"&&setTimeout(async()=>{await F({phase:"midfield"})},5e3)}let W=!1;function re(){if(W)return;const m=d[o+"Team"].MIL||[],h=d[r+"Team"].MIL||[];function T(J){return J.reduce((oe,de)=>oe+me(de,"MIL"),0)}function A(J){let oe=0;for(let de=0;de<J.length-1;de++){const ye=Xe(J[de],J[de+1]);ye==="#00ff88"?oe+=2:ye==="#FFD700"&&(oe+=1)}return oe}const E=T(m)+A(m),L=T(h)+A(h),j=E>=L;function D(J,oe,de){return`<div id="duel-row-${de}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="width:200px;border-radius:18px;border:3px solid ${L};background:${E};display:flex;flex-direction:column;overflow:hidden;animation:gcFlipIn .7s cubic-bezier(.34,1.56,.64,1) both,gcGlow 1.8s ease infinite .7s">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(Q);let ue=!1;const we=()=>{ue||(ue=!0,Q.remove(),setTimeout(()=>T&&T(),50))};Q.addEventListener("click",we),setTimeout(we,3e3)}function ae(m,h){var Q,ue,we,X;const A=(d["gcCardsFull_"+o]||[]).find(J=>J.id===m),E=A==null?void 0:A._gcDef,L={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[E==null?void 0:E.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",j={purple:"#b06ce0",light_blue:"#00d4ef"}[E==null?void 0:E.color]||"#b06ce0",D=(E==null?void 0:E.name)||h,P=(E==null?void 0:E.effect)||((Q=Ce[h])==null?void 0:Q.desc)||"Carte spéciale.",B=E!=null&&E.image_url?`/manager-wars/icons/${E.image_url}`:null,H=((ue=Ce[h])==null?void 0:ue.icon)||"⚡",K=document.createElement("div");K.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",K.innerHTML=`
      ...
      </div>`,document.body.appendChild(K),(we=K.querySelector("#pvp-gc-back"))==null||we.addEventListener("click",()=>K.remove()),(X=K.querySelector("#pvp-gc-use"))==null||X.addEventListener("click",()=>{K.remove(),w(m,h)})}function z(){var A;const m=d[o+"Team"],h=Object.entries(m).flatMap(([E,L])=>(L||[]).filter(j=>!j.used).map(j=>({...j,_line:E})));if(!h.length)return;const T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",T.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
        <div style="flex:1;font-size:14px;font-weight:700;color:#fff">⚡ Choisir un joueur pour +${d.boostValue}</div>
        <button id="bp-close" style="background:none;border:none;color:rgba(255,255,255,0.5);font-size:22px;cursor:pointer">✕</button>
      </div>
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${h.map(E=>{const L={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[E._line]||"#555",j=me(E,E._line)+(E.boost||0);return`<div class="bp-item" data-cid="${E.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${L};overflow:hidden;cursor:pointer">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:6px">Appuyer pour continuer</div>`,document.body.appendChild(E);let j=!1;const D=()=>{j||(j=!0,E.remove(),setTimeout(()=>T(),50))};E.addEventListener("click",D),setTimeout(D,2200)}function C(){var h;const m=document.createElement("div");m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:20px;overflow-y:auto",m.innerHTML=`
      ...
      </div>`,document.body.appendChild(h),(T=h.querySelector("#pvp-hist-close"))==null||T.addEventListener("click",()=>h.remove())}async function ie(){if(d.phase!==o+"-attack")return;const m=o==="p1"?"p2":"p1",h=(d.round||0)+1,T=[...d.log||[]];T.push({type:"info",text:`⏭️ ${d[o+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const A=Ee(d),E=be(m),L=A||!E?"finished":m+"-attack";await F({["selected_"+o]:[],modifiers:{...d.modifiers,[o]:{}},pendingAttack:null,phase:L,attacker:m,round:h,log:T}),L==="finished"&&await Ie(d)}async function Y(){const m=d[o+"Team"],h=!["GK","DEF","MIL","ATT"].some(L=>(d[r+"Team"][L]||[]).some(j=>!j.used)),T=(d["selected_"+o]||[]).map(L=>{const j=(m[L._role]||[]).find(Q=>Q.cardId===L.cardId)||L,D=h&&["GK","DEF"].includes(L._role),P=m[L._role]||[],B=P.findIndex(Q=>Q.cardId===L.cardId),H=at(P.length),K=B>=0?H[B]:j._col??1;return{...j,_line:L._role,_col:K,...D?{note_a:Math.max(1,Number(j.note_a)||0)}:{}}});if(!T.length)return;const A=Ot(T,d.modifiers[o]||{});q(o,T.map(L=>L.cardId)),T.forEach(L=>{const j=(m[L._role]||[]).find(D=>D.cardId===L.cardId);j&&(j.used=!0)}),d["selected_"+o]=[],b();const E=[...d.log||[]];if(h){const L=(d[o+"Score"]||0)+1,j=T.map(K=>({name:K.name,note:me(K,K._line||"ATT"),portrait:Te(K),job:K.job}));E.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:j,homeTotal:A.total,aiTotal:0});const D=(d.round||0)+1,P=o==="p1"?"p2":"p1",B={...d,[o+"Team"]:m,[o+"Score"]:L},H=Ee(B);M.add(D),ce(j,L,d[r+"Score"]||0,!0,async()=>{await F({[o+"Team"]:m,[o+"Score"]:L,["selected_"+o]:[],modifiers:{...d.modifiers,[o]:{}},pendingAttack:null,phase:H?"finished":P+"-attack",attacker:P,round:D,log:E}),H&&await Ie({...d,[o+"Score"]:L})});return}E.push({type:"pending",text:`⚔️ ${d[o+"Name"]} attaque (${A.total})`}),await F({[o+"Team"]:m,[r+"Team"]:d[r+"Team"],pendingAttack:{...A,players:T,side:o},["selected_"+o]:[],modifiers:{...d.modifiers,[o]:{}},phase:r+"-defense",log:E})}async function ve(){const m=d[o+"Team"],h=(d["selected_"+o]||[]).map(X=>{const J=(m[X._role]||[]).find(he=>he.cardId===X.cardId)||X,oe=m[X._role]||[],de=oe.findIndex(he=>he.cardId===X.cardId),ye=at(oe.length),ke=de>=0?ye[de]:J._col??1;return{...J,_line:X._role,_col:ke}}),T=Pt(h,d.modifiers[o]||{});q(o,h.map(X=>X.cardId)),h.forEach(X=>{const J=(m[X._role]||[]).find(oe=>oe.cardId===X.cardId);J&&(J.used=!0)}),d["selected_"+o]=[],b();const A=Ht(d.pendingAttack.total,T.total,d.modifiers[o]||{}),E=d.pendingAttack.side,L=A==="attack"||(A==null?void 0:A.goal),j=E==="p1"?"p2":"p1",D=(d.round||0)+1,P=(d.pendingAttack.players||[]).map(X=>({name:X.name,note:me(X,X._line||"ATT"),portrait:Te(X),job:X.job})),B=[...d.log||[]];B.push({type:"duel",isGoal:L,homeScored:L&&E===o,text:L?`⚽ BUT de ${d[E+"Name"]} ! (${d.pendingAttack.total} vs ${T.total})`:`✋ Attaque stoppée (${d.pendingAttack.total} vs ${T.total})`,homePlayers:P,aiPlayers:h.map(X=>({name:X.name,note:me(X,X._line||"DEF"),portrait:Te(X),job:X.job})),homeTotal:d.pendingAttack.total,aiTotal:T.total});const H=L?(d[E+"Score"]||0)+1:d[E+"Score"]||0,K={...d,[o+"Team"]:m,[E+"Score"]:H},Q=Ee(K),ue=Q?"finished":j+"-attack",we=async()=>{await F({[o+"Team"]:m,[r+"Team"]:d[r+"Team"],[E+"Score"]:H,["selected_"+o]:[],modifiers:{...d.modifiers,[o]:{}},pendingAttack:null,phase:ue,attacker:j,round:D,log:B}),(ue==="finished"||Q)&&await Ie({...d,[E+"Score"]:H})};if(L){const X=E===o,J=X?H:d[o+"Score"]||0,oe=X?d[r+"Score"]||0:H;M.add(D),ce(P,J,oe,X,we)}else await we()}function xe(m){return["MIL","ATT"].some(h=>(m[h]||[]).some(T=>!T.used))}function ge(m){return["GK","DEF","MIL","ATT"].some(h=>(m[h]||[]).some(T=>!T.used))}function fe(m){return ge(m)&&!xe(m)}function be(m){const h=d[m+"Team"],T=d[(m==="p1"?"p2":"p1")+"Team"];return!!(xe(h)||!ge(T)&&fe(h))}function Ee(m){const h=["MIL","ATT"].some(B=>(m.p1Team[B]||[]).some(H=>!H.used)),T=["MIL","ATT"].some(B=>(m.p2Team[B]||[]).some(H=>!H.used)),A=ge(m.p1Team),E=ge(m.p2Team);return!h&&!(!E&&A)&&(!T&&!(!A&&E))}function Le(m){const h=m.p1Score||0,T=m.p2Score||0;return h===T?null:h>T?g.home_id:g.away_id}async function Ie(m){try{await I.from("matches").update({status:"finished",winner_id:Le(m),home_score:m.p1Score||0,away_score:m.p2Score||0}).eq("id",i)}catch(h){console.error("[PvP] finishMatch:",h)}}function je(){var E;const m=d[o+"Score"],h=d[r+"Score"],T=m>h,A=m===h;Ve(e),e.innerHTML=`
  ...
  `;function p(o){const r=document.getElementById("mkt-content"),u=o==="buy"?a:l;if(u.length===0){r.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      ${f.map(g=>{var M,G,k,v;const y=(M=g.card)==null?void 0:M.player;if(!y)return"";const d=y.job==="GK"?y.note_g:y.job==="DEF"?y.note_d:y.job==="MIL"?y.note_m:y.note_a,x=Ko[y.rarity],$=(i.profile.credits||0)>=g.price,_=g.status==="sold";return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${_?"opacity:0.65":""}">
          ...
          <div style="flex:1;min-width:0">
            ...
            ${o==="buy"?`<div style="font-size:11px;color:var(--gray-600)">Vendeur : ${((k=g.seller)==null?void 0:k.pseudo)||"—"}</div>`:_?`<div style="font-size:11px;color:var(--green)">✅ Vendu à ${((v=g.buyer)==null?void 0:v.pseudo)||"—"} · ${g.sold_at?new Date(g.sold_at).toLocaleDateString("fr"):""}</div>`:`<div style="font-size:11px;color:var(--gray-600)">🟢 En vente depuis le ${new Date(g.listed_at).toLocaleDateString("fr")}</div>`}
          </div>
