...
import{s as z,F as _i,i as Ht,k as Zi,l as nt,m as ji,f as Pt,n as Lo,T as ci,j as Io,o as Mo,b as jo}from"./formation-links-5A-YoKwp.js";const Co="/";function Ci(e,{navigate:t,toast:i}){let n="login";const d=()=>{var r,c,f,a,o,l;const s=n==="login";e.innerHTML=`
  ...
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Wt=1e3;function on(e,t,i,n){var $,p,u;const{state:d,toast:s,closeModal:r,navigate:c,refreshProfile:f}=i,a=e.formation,o={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function l(){const m=Ht[a]||{},A=_i[a]||[],M=290,G=360,q=20;function B(N){const J=m[N];return J?{x:J.x*M,y:J.y*G}:null}let C=`<svg width="${M}" height="${G}" viewBox="0 0 ${M} ${G}" xmlns="http://www.w3.org/2000/svg">`;for(const[N,J]of A){const Z=B(N),se=B(J);!Z||!se||(C+=`<line x1="${Z.x}" y1="${Z.y}" x2="${se.x}" y2="${se.y}"
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),($=document.getElementById("direct-sell-form-btn"))==null||$.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${a} pour ${Wt.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const m=b.find(M=>!M.is_for_sale)||b[0];if(!m){s("Aucune carte à vendre","error");return}await z.from("market_listings").delete().eq("card_id",m.id),await z.from("transfer_history").delete().eq("card_id",m.id);const{error:A}=await z.from("cards").delete().eq("id",m.id);if(A){s(A.message,"error");return}await z.from("users").update({credits:(d.profile.credits||0)+Wt}).eq("id",d.profile.id),await f(),s(`+${Wt.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),r(),c("collection")}),(p=document.getElementById("market-sell-form-btn"))==null||p.addEventListener("click",async()=>{const m=parseInt(document.getElementById("sell-price-form").value);if(!m||m<1){s("Prix invalide","error");return}await z.from("cards").update({is_for_sale:!0,sale_price:m}).eq("id",e.id),await z.from("market_listings").insert({seller_id:d.profile.id,card_id:e.id,price:m}),s("Carte mise en vente sur le marché !","success"),r(),c("collection")}),(u=document.getElementById("cancel-sell-form-btn"))==null||u.addEventListener("click",async()=>{await z.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await z.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),s("Annonce retirée","success"),r(),c("collection")})}async function Ni(e,t,i,n){var Le,Fe,Ce,we,V,oe,fe;const{state:d,toast:s,openModal:r,closeModal:c,navigate:f,refreshProfile:a}=n,o=e.player,l=t.filter(K=>K.player.id===o.id),b=l.length,h=e.evolution_bonus||0,$=Math.max((Number(o.note_g)||0)+(o.job==="GK"||o.job2==="GK"?h:0),(Number(o.note_d)||0)+(o.job==="DEF"||o.job2==="DEF"?h:0),(Number(o.note_m)||0)+(o.job==="MIL"||o.job2==="MIL"?h:0),(Number(o.note_a)||0)+(o.job==="ATT"||o.job2==="ATT"?h:0)),p=o.rarity||"normal",{data:u}=await z.from("sell_price_configs").select("*").eq("rarity",p).lte("note_min",$).gte("note_max",$).order("note_min",{ascending:!1}).limit(1);((Le=u==null?void 0:u[0])==null?void 0:Le.price)??Zo[p];const m=o.rarity!=="legende",A=po(o),M=((o.rarity==="pepite"||o.rarity==="papyte")&&e.current_note!=null?e.current_note:St(o,o.job))+h,G=o.rarity==="pepite"||o.rarity==="papyte",q=o.job2?(G?Ei(e,co(o.job2)):St(o,o.job2))+(St(o,o.job2)>0?h:0):null,B=Ft[o.job]||"#1A6B3C",C=o.job2?Ft[o.job2]:null,N=lo[o.rarity]||"#ccc",J=zi[o.country_code]||o.country_code||"",Z=e.evolution_bonus||0,re=M+Z,Q=q||0,U=Q>0?Q+Z:0,D=l.map(K=>K.id);let ne={};if(D.length){const{data:K}=await z.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",D).order("transferred_at",{ascending:!0});(K||[]).forEach(te=>{ne[te.card_id]||(ne[te.card_id]=[]),ne[te.card_id].push(te)})}const ue=K=>{const te=K.transferred_at?new Date(K.transferred_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"2-digit",year:"numeric"}):"",Y=K.source==="booster"?"Booster":K.price?K.price.toLocaleString("fr"):"—";return`
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(we=document.getElementById("close-detail"))==null||we.addEventListener("click",c);let me=new Set;const xe=()=>{const K=me.size,te=document.getElementById("sell-action-panel");if(!te)return;te.style.display=K>0?"block":"none",document.getElementById("sell-selected-count").textContent=K;const Y=document.getElementById("evolve-btn");Y&&(Y.textContent=`⬆️ Évoluer ${K>1?"(+"+K+")":""}`)};document.querySelectorAll(".expl-check").forEach(K=>{K.addEventListener("change",()=>{const te=K.dataset.id;K.checked?me.add(te):me.delete(te);const Y=K.closest(".exemplaire-row");Y&&(Y.style.borderColor=K.checked?"#1A6B3C":"#eee"),xe()})}),document.querySelectorAll(".exemplaire-row").forEach(K=>{K.addEventListener("click",te=>{if(te.target.closest("button")||te.target.tagName==="INPUT")return;const Y=K.querySelector(".expl-check");Y&&!Y.disabled&&(Y.checked=!Y.checked,Y.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(K=>{K.addEventListener("click",te=>{te.stopPropagation();const Y=document.querySelector(`.expl-hist[data-hist="${K.dataset.idx}"]`);Y&&(Y.style.display=Y.style.display==="none"?"flex":"none")})}),(V=document.getElementById("evolve-btn"))==null||V.addEventListener("click",async()=>{if(b<=1)return;const K=[...me];if(!K.length)return;if(me.has(e.id)){const be=document.createElement("div");be.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",be.innerHTML=`
  ...
  <div style="text-align:center;flex-shrink:0;width:38px">
    ...
    <div style="font-size:7px;color:rgba(255,255,255,0.5);margin-top:1px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${(e.name||"").slice(0,7)}</div>
          ...
          card:cards(id, card_type, formation, evolution_bonus,
            player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
              clubs(encoded_name,logo_url)))`).eq("deck_id",f).order("slot_order")]);a=C[0].data,l=C[0].error,o=C[1].data,b=C[1].error}catch(C){console.error("[Match] Exception chargement deck:",C),kt(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>s("home"));return}if(l||b){console.error("[Match] Erreur Supabase:",l||b),kt(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>s("home"));return}const h=(o||[]).filter(C=>{var N;return C.is_starter&&((N=C.card)==null?void 0:N.player)}).map(C=>bi(C.card,C.position)),$=(o||[]).filter(C=>{var N;return!C.is_starter&&((N=C.card)==null?void 0:N.player)}).map(C=>bi(C.card,C.position));if(h.length<11){kt(e,"⚠️",`Deck incomplet (${h.length}/11).`,"Compléter",()=>s("decks"));return}const p=(o||[]).find(C=>{var N;return((N=C.card)==null?void 0:N.card_type)==="formation"}),u=(a==null?void 0:a.formation)||((B=p==null?void 0:p.card)==null?void 0:B.formation)||"4-4-2",{data:m,error:A}=await z.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",d.profile.id).eq("card_type","game_changer"),{data:M}=await z.from("gc_definitions").select("*").eq("is_active",!0),G=(m||[]).map(C=>({...C,_gcDef:(M==null?void 0:M.find(N=>N.name===C.gc_type||N.id===C.gc_definition_id))||null}));let q=null;if(a!=null&&a.stadium_card_id){const{data:C}=await z.from("cards").select("stadium_id").eq("id",a.stadium_card_id).single();if(C!=null&&C.stadium_id){const{data:N}=await z.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").eq("id",C.stadium_id).single();q=N||null}}n({deckId:f,formation:u,starters:h,subsRaw:$,gcCardsEnriched:G,gcDefs:M||[],stadiumDef:q})}async function zn(e,t){const{state:i}=t,d=(i.params||{}).matchMode||"vs_ai_easy",s=d.replace("vs_ai_",""),r=d;await di(e,t,d,async({deckId:c,formation:f,starters:a,subsRaw:o,gcCardsEnriched:l,gcDefs:b,stadiumDef:h})=>{try{let $=bt(a,f);h&&($=Ct($,h),Ot(o,h));const p=await Sn(f,s),u=p.lines||p,m=async A=>{try{const{data:M,error:G}=await z.from("matches").insert({home_id:i.profile.id,away_id:null,mode:r,home_deck_id:c,status:"in_progress"}).select().single();if(G){console.error("[MatchIA] Erreur création match:",G),kt(e,"⚠️","Impossible de créer le match ("+G.message+").","Retour",()=>t.navigate("home"));return}const q=p.stadiumDef||null;q&&u&&(Ct(u,q),Ot(p.subs||[],q));const B={gcDefs:b||[],matchId:M==null?void 0:M.id,mode:r,difficulty:s,formation:f,homeTeam:$,aiTeam:u,homeSubs:o,subsUsed:0,maxSubs:Math.min(o.length,3),aiSubs:p.subs||[],aiSubsUsed:0,aiMaxSubs:Math.min((p.subs||[]).length,3),aiUsedSubIds:[],aiGcCards:p.gcCards||[],aiUsedGc:[],aiStadiumDef:q,homeScore:0,aiScore:0,gcCards:A,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};In(e,B,t)}catch(M){console.error("[MatchIA] Exception launchMatch:",M),kt(e,"⚠️","Erreur au lancement du match : "+M.message,"Retour",()=>t.navigate("home"))}};if(!l.length){m([]);return}si(e,l,m)}catch($){console.error("[MatchIA] Exception setup:",$),kt(e,"⚠️","Erreur de préparation du match : "+$.message,"Retour",()=>t.navigate("home"))}})}async function Sn(e,t){var u;const{data:i}=await z.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(80);if(!i||i.length<11)return{lines:Ln(e),subs:[],gcCards:[],stadiumDef:null};const n=Mt[e]||Mt["4-4-2"],d={GK:[],DEF:[],MIL:[],ATT:[]},s=new Set;function r(m,A,M){var G,q;return s.add(m.id),{cardId:"ai-"+m.id+"-"+M,id:m.id,firstname:m.firstname,name:m.surname_encoded,country_code:m.country_code,club_id:m.club_id,job:m.job,job2:m.job2,note_g:Number(m.note_g)||0,note_d:Number(m.note_d)||0,note_m:Number(m.note_m)||0,note_a:Number(m.note_a)||0,rarity:m.rarity,skin:m.skin,hair:m.hair,hair_length:m.hair_length,clubName:((G=m.clubs)==null?void 0:G.encoded_name)||null,clubLogo:((q=m.clubs)==null?void 0:q.logo_url)||null,boost:0,used:!1,_line:A}}for(const m of["GK","DEF","MIL","ATT"]){const A=i.filter(C=>C.job===m&&!s.has(C.id)),M=i.filter(C=>C.job!==m&&!s.has(C.id)),G=[...A,...M],q=[];for(let C=0;C<n[m];C++){const N=G[C];N&&q.push(r(N,m,C))}const B=st(q.length);q.forEach((C,N)=>{C._col=B[N]}),d[m]=q}const f=i.filter(m=>!s.has(m.id)).slice(0,5).map((m,A)=>r(m,m.job,100+A)),l=Object.keys(Re).sort(()=>Math.random()-.5).slice(0,3).map((m,A)=>{var M,G;return{id:"ai-gc-"+A,gc_type:m,name:((M=Re[m])==null?void 0:M.name)||m,icon:((G=Re[m])==null?void 0:G.icon)||"⚡"}}),b=Object.values(d).flat(),h={};b.forEach(m=>{m.club_id&&(h[m.club_id]=(h[m.club_id]||0)+1)});const $=(u=Object.entries(h).sort((m,A)=>A[1]-m[1])[0])==null?void 0:u[0];let p=null;if($){const{data:m}=await z.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",$).single();m&&(p={club_id:m.id,country_code:null,name:m.encoded_name+" Stadium",club:{encoded_name:m.encoded_name,logo_url:m.logo_url}})}return{lines:d,subs:f,gcCards:l,stadiumDef:p}}function Ln(e){const t=Mt[e]||Mt["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},n=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let d=0;for(const s of["GK","DEF","MIL","ATT"]){const r=[];for(let f=0;f<t[s];f++){const a=3+Math.floor(Math.random()*5);r.push({cardId:"fake-"+d,id:"fake-"+d,firstname:"IA",name:n[d%n.length],country_code:"XX",club_id:null,job:s,job2:null,note_g:s==="GK"?a:2,note_d:s==="DEF"?a:2,note_m:s==="MIL"?a:2,note_a:s==="ATT"?a:2,rarity:"normal",boost:0,used:!1,_line:s}),d++}const c=st(r.length);r.forEach((f,a)=>{f._col=c[a]}),i[s]=r}return i}function In(e,t,i){var n;e.innerHTML=`
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
  </div>`,setTimeout(()=>Mn(e,t,i),5e3)}function Mn(e,t,i){const n=t.homeTeam.MIL||[],d=t.aiTeam.MIL||[],s=t.stadiumDef||null,r=t.aiStadiumDef||null;function c(A,M){const G=ze(A,"MIL"),q=A.evolution_bonus||A._evolution_bonus||0,B=A.job==="MIL"||A.job2==="MIL"?q:0,C=A.stadiumBonus||M&&(M.club_id&&String(A.club_id)===String(M.club_id)||M.country_code&&A.country_code===M.country_code)?10:0;return G+B+C}function f(A,M){return A.reduce((G,q)=>G+c(q,M),0)}function a(A){let M=0;for(let G=0;G<A.length-1;G++){const q=nt(A[G],A[G+1]);q==="#00ff88"?M+=2:q==="#FFD700"&&(M+=1)}return M}const o=f(n,s)+a(n),l=f(d,r)+a(d),b=o>=l;function h(A,M,G,q,B){return`<div id="duel-row-${q}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0), opacity .5s ease;transform-origin:center">
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${A.map((C,N)=>{const J=N<A.length-1?nt(C,A[N+1]):null,Z=!J||J==="#ff3333"||J==="#cc2222",se=J==="#00ff88"?"+2":J==="#FFD700"?"+1":"";c(C,B),C.stadiumBonus||B&&(B.club_id&&String(C.club_id)===String(B.club_id)||B.country_code&&(C.country_code,B.country_code));const re=C.evolution_bonus||C._evolution_bonus||0;return`
          ...
          ${N<A.length-1?`<div class="duel-link duel-link-${q}" data-idx="${N}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${Z?"rgba(255,255,255,0.12)":J};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${Z?"none":`0 0 8px ${J}`}">
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    ${(()=>{const D=window.innerWidth>=700,ne=(V,oe,fe)=>{var w,T;const K=(t.gcDefs||[]).find(g=>g.name===V.gc_type),te={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[K==null?void 0:K.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",Y={purple:"#b06ce0",light_blue:"#00d4ef"}[K==null?void 0:K.color]||"#b06ce0",de=(K==null?void 0:K.name)||V.gc_type,ve=(K==null?void 0:K.effect)||((w=Re[V.gc_type])==null?void 0:w.desc)||"",Te=K!=null&&K.image_url?`/icons/${K.image_url}`:null,$e=((T=Re[V.gc_type])==null?void 0:T.icon)||"⚡",be=Math.round(fe*.22),ke=Math.round(fe*.22),qe=fe-be-ke,je=de.length>12?7:9;return`<div class="gc-mini" data-gc-id="${V.id}" data-gc-type="${V.gc_type}"
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(s);let c=!1;const f=()=>{c||(c=!0,s.remove(),setTimeout(()=>d(),50))};s.addEventListener("click",f),setTimeout(f,3500)}async function Dt(e,t,i){var o,l;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:n}=i,d=t.homeScore>t.aiScore,s=t.homeScore===t.aiScore,r=d?"victoire":s?"nul":"defaite",c=Wo(t.mode,r);t.matchId&&await z.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:d?n.profile.id:null,home_credits_reward:c,played_at:new Date().toISOString()}).eq("id",t.matchId);const f={credits:(n.profile.credits||0)+c,matches_played:(n.profile.matches_played||0)+1};d?f.wins=(n.profile.wins||0)+1:s?f.draws=(n.profile.draws||0)+1:f.losses=(n.profile.losses||0)+1,await z.from("users").update(f).eq("id",n.profile.id),await i.refreshProfile();const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",a.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const G=async(N=!0)=>{$=!0,p&&(z.removeChannel(p),p=null),N&&await z.rpc("cancel_matchmaking",{p_user_id:l.profile.id}).catch(()=>{})};(C=document.getElementById("mm-cancel"))==null||C.addEventListener("click",async()=>{try{await G(!0)}catch{}Ve(e),b("home")});const q=async(N,J,Z)=>{if($)return;$=!0,p&&(z.removeChannel(p),p=null);const se=document.getElementById("mm-status");se&&(se.textContent="Adversaire trouvé !"),await new Promise(re=>setTimeout(re,600)),e.isConnected&&ko(e,t,N,Z,r,c,a,o)};if(a){const N=[{range:50,delay:0},{range:100,delay:15e3},{range:200,delay:3e4},{range:400,delay:45e3},{range:800,delay:6e4}];let J=0,Z=!1;const se=async()=>{if($||Z)return;const{range:re}=N[J],Q=document.getElementById("mm-range");Q&&(Q.textContent=`Plage MMR : ±${re}`);const{data:U,error:D}=await z.rpc("try_matchmake_ranked",{p_user_id:l.profile.id,p_deck_id:i,p_mmr:u,p_range:re});if(console.log("[Ranked] try_matchmake_ranked result:",U,"error:",JSON.stringify(D)),$)return;if(D||!(U!=null&&U.success)){h("Erreur matchmaking ranked","error"),Ve(e),b("home");return}if(U.matched){Z=!0,q(U.match_id,U.opponent_id,!1);return}const ne=document.getElementById("mm-status");if(ne&&(ne.textContent="En attente d'un adversaire..."),p||(p=z.channel("matchmaking-"+l.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${l.profile.id}`},ue=>{const ye=ue.new;ye.status==="matched"&&ye.match_id&&!Z&&(Z=!0,q(ye.match_id,ye.matched_with,!0))}).subscribe()),J<N.length-1){const ue=N[J+1].delay-N[J].delay;J++,setTimeout(()=>{!$&&!Z&&se()},ue)}};await se()}else{const{data:N,error:J}=await z.rpc("try_matchmake",{p_user_id:l.profile.id,p_deck_id:i});if(J||!(N!=null&&N.success)){h("Erreur de matchmaking","error"),Ve(e),b("home");return}if(N.matched){q(N.match_id,N.opponent_id,!1);return}const Z=document.getElementById("mm-status");Z&&(Z.textContent="En attente d'un autre joueur..."),p=z.channel("matchmaking-"+l.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${l.profile.id}`},se=>{const re=se.new;re.status==="matched"&&re.match_id&&q(re.match_id,re.matched_with,!0)}).subscribe()}}async function Vn(e,t,i){const{state:n,navigate:d,toast:s}=t;try{(z.getChannels?z.getChannels():[]).forEach(a=>{const o=a.topic||"";(o.includes("matchmaking")||o.includes("pvp-match")||o.includes("friend-invite"))&&z.removeChannel(a)})}catch{}const{data:r}=await z.from("matches").select("home_id, away_id, status").eq("id",i).single();if(!r){s("Match introuvable","error"),d("home");return}if(r.status==="finished"){s("Ce match est terminé","info"),d("home");return}const c=r.home_id===n.profile.id;ko(e,t,i,c,[],[])}async function ko(e,t,i,n,d=[],s=[],r=!1,c=null){const{state:f,navigate:a,toast:o}=t,l=n?"p1":"p2",b=n?"p2":"p1",h=(d||[]).map(w=>w.id),$=(d||[]).map(w=>({id:w.id,gc_type:w.gc_type,_gcDef:w._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:p}=await z.from("matches").select("*").eq("id",i).single();if(!p){o("Match introuvable","error"),a("home");return}async function u(){var ee,R;console.log("[PvP] buildGameState match:",{id:p.id,home_deck_id:p.home_deck_id,away_deck_id:p.away_deck_id,mode:p.mode,is_ranked:p.is_ranked});const[{data:w,error:T},{data:g,error:y},{data:x},{data:_}]=await Promise.all([z.rpc("get_deck_for_match",{p_deck_id:p.home_deck_id}),z.rpc("get_deck_for_match",{p_deck_id:p.away_deck_id}),z.from("users").select("id,pseudo,club_name").eq("id",p.home_id).single(),z.from("users").select("id,pseudo,club_name").eq("id",p.away_id).single()]);console.log("[PvP] get_deck_for_match p1:",T==null?void 0:T.message,"p2:",y==null?void 0:y.message,"p1D:",(ee=w==null?void 0:w.starters)==null?void 0:ee.length,"p2D:",(R=g==null?void 0:g.starters)==null?void 0:R.length);const v=F=>{const H=Number(F.evolution_bonus)||0;return{cardId:F.card_id,position:F.position,id:F.id,firstname:F.firstname,name:F.surname_encoded,country_code:F.country_code,club_id:F.club_id,job:F.job,job2:F.job2,note_g:(Number(F.note_g)||0)+(F.job==="GK"||F.job2==="GK"&&Number(F.note_g)>0?H:0),note_d:(Number(F.note_d)||0)+(F.job==="DEF"||F.job2==="DEF"&&Number(F.note_d)>0?H:0),note_m:(Number(F.note_m)||0)+(F.job==="MIL"||F.job2==="MIL"&&Number(F.note_m)>0?H:0),note_a:(Number(F.note_a)||0)+(F.job==="ATT"||F.job2==="ATT"&&Number(F.note_a)>0?H:0),evolution_bonus:H,rarity:F.rarity,skin:F.skin,hair:F.hair,hair_length:F.hair_length,clubName:F.club_encoded_name||null,clubLogo:F.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},k=((w==null?void 0:w.starters)||[]).map(F=>v(F)),L=((g==null?void 0:g.starters)||[]).map(F=>v(F)),E=(w==null?void 0:w.formation)||"4-4-2",I=(g==null?void 0:g.formation)||"4-4-2";let S=bt(k,E),j=bt(L,I);const O=((w==null?void 0:w.subs)||[]).map(F=>v(F));return stadiumDef&&n&&(S=Ct(S,stadiumDef),Ot(O,stadiumDef)),{p1Team:S,p2Team:j,p1Subs:O,p2Subs:((g==null?void 0:g.subs)||[]).map(F=>v(F)),p1Formation:E,p2Formation:I,p1Name:(x==null?void 0:x.club_name)||(x==null?void 0:x.pseudo)||"Joueur 1",p2Name:(_==null?void 0:_.club_name)||(_==null?void 0:_.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:n?h:[],gc_p2:n?[]:h,gcCardsFull_p1:n?$:[],gcCardsFull_p2:n?[]:$,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:s||[],lastActionAt:new Date().toISOString()}}let m=p.game_state&&Object.keys(p.game_state).length?p.game_state:null;if(console.log("[PvP] init - amIHome:",n,"gameState exists:",!!m,"match.status:",p.status,"home_id:",p.home_id,"away_id:",p.away_id,"myId:",f.profile.id),!m)if(n){m=await u(),console.log("[PvP] home - gameState built, p1Team keys:",Object.keys((m==null?void 0:m.p1Team)||{}));const{data:w}=await z.from("matches").select("game_state").eq("id",i).single();!(w!=null&&w.game_state)||!Object.keys(w.game_state).length?await z.from("matches").update({game_state:m,turn_user_id:p.home_id}).eq("id",i):m=w.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>',console.log("[PvP] away - waiting for home to write game_state...");for(let w=0;w<30&&!m;w++){await new Promise(g=>setTimeout(g,400));const{data:T}=await z.from("matches").select("game_state").eq("id",i).single();T!=null&&T.game_state&&Object.keys(T.game_state).length&&(m=T.game_state),w%5===0&&console.log("[PvP] away - poll",w,"game_state:",!!(T!=null&&T.game_state))}if(!m){o("Erreur de synchronisation","error"),a("home");return}m.gc_p2=h,m.gcCardsFull_p2=$,await z.from("matches").update({game_state:m}).eq("id",i)}let A=!1,M=null,G=!1;const q=new Set,B=new Set;async function C(w){var I,S;try{z.removeChannel(N)}catch{}const T=m[l+"Score"]||0,g=m[b+"Score"]||0;let y,x;w.winner_id?(y=w.winner_id===f.profile.id,x=!1):w.forfeit?(y=T>g,x=!1):(x=T===g,y=T>g);let _="";if(r&&n)try{const{data:j}=await z.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",p.home_id).single(),{data:O}=await z.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",p.away_id).single();if(j&&O){const ee=y?1:x?.5:0,R=1-ee,F=j.placement_matches<10,H=O.placement_matches<10,X=ji(j.mmr,j.mmr_deviation,j.mmr_volatility,O.mmr,O.mmr_deviation,ee===1?1:ee===0?0:.5,F),ae=ji(O.mmr,O.mmr_deviation,O.mmr_volatility,j.mmr,j.mmr_deviation,R===1?1:R===0?0:.5,H);await z.rpc("update_mmr_after_ranked",{p_match_id:i,p_winner_id:x?null:y?p.home_id:p.away_id,p_home_id:p.home_id,p_away_id:p.away_id,p_home_delta:X.delta,p_away_delta:ae.delta,p_home_new_rd:X.newRd,p_away_new_rd:ae.newRd,p_home_new_vol:X.newSigma,p_away_new_vol:ae.newSigma});const pe=X.delta,ce=X.newMmr,ge=Pt(ce),he=pe>=0?"+":"",Ie=pe>=0?"#4caf50":"#ff6b6b",Se=Pt(j.mmr);_=`
      ...
      ${_}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(v),(S=v.querySelector("#pvp-end-home"))==null||S.addEventListener("click",()=>{v.remove(),Ve(e),a("home")})}const N=z.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},w=>{const T=w.new;try{if(T.status==="finished"||T.forfeit){if(A)return;A=!0,M&&(clearInterval(M),M=null),T.game_state&&(m=T.game_state),C(T);return}if(T.game_state){const g=m;m=T.game_state;const y=m._lastGC;if(y&&y.seq&&!B.has(y.seq)&&(B.add(y.seq),y.by!==l)){Le(y.type,y.by,()=>ne());return}const x=g[l+"Score"]||0,_=g[b+"Score"]||0,v=m[l+"Score"]||0,k=m[b+"Score"]||0,L=v>x,E=k>_;if((L||E)&&!q.has(m.round)){q.add(m.round);const I=[...m.log||[]].reverse().find(j=>j.isGoal),S=((I==null?void 0:I.homePlayers)||[]).map(j=>({name:j.name,note:j.note,portrait:j.portrait,job:j.job}));xe(S,v,k,L,()=>ne());return}ne()}}catch(g){console.error("[PvP] crash:",g)}}).subscribe();async function J(w){Object.assign(m,w),m.lastActionAt=new Date().toISOString();const{error:T}=await z.from("matches").update({game_state:m}).eq("id",i);T&&o("Erreur de synchronisation","error");try{ne()}catch(g){console.error("[PvP] renderPvpScreen crash:",g)}}async function Z(){if(A)return;A=!0,M&&(clearInterval(M),M=null);const w=n?p.away_id:p.home_id,T=n?"p2":"p1",g=n?"p1":"p2",y={...m,[T+"Score"]:3,[g+"Score"]:0,phase:"finished"};try{await z.from("matches").update({status:"finished",forfeit:!0,winner_id:w,home_score:y.p1Score||0,away_score:y.p2Score||0,game_state:y}).eq("id",i)}catch{}try{z.removeChannel(N)}catch{}setTimeout(()=>{Ve(e),a("home")},800)}const se={BOOST_STAT:({value:w=1,count:T=1,roles:g=[]},y,x)=>{const _=y[l+"Team"],v=Object.entries(_).filter(([k])=>!g.length||g.includes(k)).flatMap(([k,L])=>L.filter(E=>!E.used).map(E=>({...E,_line:k})));if(!v.length){y.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),x(y);return}re(v,T,`Choisir ${T} joueur(s) à booster (+${w})`,k=>{k.forEach(L=>{const E=(_[L._line]||[]).find(I=>I.cardId===L.cardId);E&&(E.boost=(E.boost||0)+w,y.log.push({text:`⚡ +${w} sur ${E.name}`,type:"info"}))}),y[l+"Team"]=_,x(y)})},DEBUFF_STAT:({value:w=1,count:T=1,roles:g=[],target:y="ai"},x,_)=>{const v=y==="home"?l:b,k=x[v+"Team"],L=y==="home"?"allié":"adverse",E=Object.entries(k).filter(([I])=>!g.length||g.includes(I)).flatMap(([I,S])=>S.map(j=>({...j,_line:I})));if(!E.length){x.log.push({text:`🎯 Aucun joueur ${L}`,type:"info"}),_(x);return}re(E,T,`Choisir ${T} joueur(s) ${L}(s) (-${w})`,I=>{I.forEach(S=>{const j=(k[S._line]||[]).find(O=>O.cardId===S.cardId);j&&(j.boost=(j.boost||0)-w,x.log.push({text:`🎯 -${w} sur ${j.name}`,type:"info"}))}),x[v+"Team"]=k,_(x)})},GRAY_PLAYER:({count:w=1,roles:T=[],target:g="ai"},y,x)=>{const _=g==="home"?l:b,v=y[_+"Team"],k=g==="home"?"allié":"adverse",L=Object.entries(v).filter(([E])=>!T.length||T.includes(E)).flatMap(([E,I])=>I.filter(S=>!S.used).map(S=>({...S,_line:E})));if(!L.length){y.log.push({text:`❌ Aucun joueur ${k}`,type:"info"}),x(y);return}re(L,w,`Choisir ${w} joueur(s) ${k}(s) à exclure`,E=>{const I="usedCardIds_"+_,S=new Set(y[I]||[]);E.forEach(j=>{const O=(v[j._line]||[]).find(ee=>ee.cardId===j.cardId);O&&(O.used=!0,S.add(j.cardId),y.log.push({text:`❌ ${O.name} exclu !`,type:"info"}))}),y[I]=[...S],y[_+"Team"]=v,x(y)})},REVIVE_PLAYER:({count:w=1,roles:T=[]},g,y)=>{const x=g[l+"Team"],_=Object.entries(x).filter(([v])=>!T.length||T.includes(v)).flatMap(([v,k])=>k.filter(L=>L.used).map(L=>({...L,_line:v})));if(!_.length){g.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),y(g);return}re(_,w,`Choisir ${w} joueur(s) à ressusciter`,v=>{v.forEach(k=>{const L=(x[k._line]||[]).find(E=>E.cardId===k.cardId);L&&(L.used=!1,g.log.push({text:`💫 ${L.name} ressuscité !`,type:"info"}))}),g[l+"Team"]=x,y(g)})},REMOVE_GOAL:({},w,T)=>{const g=b+"Score";w[g]>0?(w[g]--,w.log.push({text:"🚫 Dernier but annulé !",type:"info"})):w.log.push({text:"🚫 Aucun but à annuler",type:"info"}),T(w)},ADD_GOAL_DRAW:({},w,T)=>{w[l+"Score"]===w[b+"Score"]?(w[l+"Score"]++,w.log.push({text:"🎯 But bonus !",type:"info"})):w.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),T(w)},ADD_SUB:({value:w=1},T,g)=>{T.maxSubs=(T.maxSubs||3)+w,T.log.push({text:`🔄 +${w} remplacement(s)`,type:"info"}),g(T)},CUSTOM:({},w,T)=>T(w)};function re(w,T,g,y){const x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let _=[];function v(){var L,E;const k=w.map(I=>{const S={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[I._line]||"#555",j=ze(I,I._line)+(I.boost||0),ee=_.find(F=>F.cardId===I.cardId)?"#FFD700":"rgba(255,255,255,0.25)",R=I.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${I.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${ee};background:${S};overflow:hidden;cursor:pointer;${R}">
        ...
        </div>`,(L=x.querySelector("#pp-close"))==null||L.addEventListener("click",()=>x.remove()),x.querySelectorAll(".pp-item").forEach(I=>{I.addEventListener("click",()=>{const S=I.dataset.cid,j=w.find(ee=>ee.cardId===S),O=_.findIndex(ee=>ee.cardId===S);j&&(O>-1?_.splice(O,1):_.length<T&&_.push(j),v())})}),(E=x.querySelector("#pp-confirm"))==null||E.addEventListener("click",()=>{x.remove(),y(_)})}v(),document.body.appendChild(x)}async function Q(w,T){const y=(m["gcCardsFull_"+l]||[]).find(k=>k.id===w),x=(y==null?void 0:y._gcDef)||(m.gcDefs||[]).find(k=>{var L;return k.name===T||((L=k.name)==null?void 0:L.toLowerCase().trim())===(T==null?void 0:T.toLowerCase().trim())}),_=[...m["usedGc_"+l]||[],w],v={type:T,by:l,seq:(m._gcAnimSeq||0)+1};B.add(v.seq),Le(T,l,async()=>{if(x!=null&&x.effect_type&&x.effect_type!=="CUSTOM"){const L=se[x.effect_type];if(L){const E={...m};L(x.effect_params||{},E,async I=>{I["usedGc_"+l]=_,I._lastGC=v,I._gcAnimSeq=v.seq,await J(I)});return}}const k={...m};switch(T){case"Remplacement+":k.maxSubs=(k.maxSubs||3)+1,k.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const L=b+"Score";k[L]>0&&(k[L]--,k.log.push({text:"🚫 But annulé",type:"info"}));break}}k["usedGc_"+l]=_,k._lastGC=v,k._gcAnimSeq=v.seq,await J(k)})}function U(w,T){const g="usedCardIds_"+w,y=new Set(m[g]||[]);T.forEach(x=>y.add(x)),m[g]=[...y]}function D(){for(const w of["p1","p2"]){const T=new Set(m["usedCardIds_"+w]||[]),g=m[w+"Team"];if(g)for(const y of["GK","DEF","MIL","ATT"])(g[y]||[]).forEach(x=>{x.used=T.has(x.cardId)})}}function ne(){var Ke,Ze,it,ut,ft,gt;if(m.phase==="reveal")return ue();if(m.phase==="midfield")return me();if(m.phase==="finished")return je();const w=m[l+"Team"],T=m[b+"Team"];D();const g=m[l+"Score"],y=m[b+"Score"],x=m[l+"Name"],_=m[b+"Name"],v=m.phase===l+"-attack",k=m.phase===l+"-defense",L=Array.isArray(m["selected_"+l])?m["selected_"+l]:[],E=L.map(P=>P.cardId),I=window.innerWidth>=700,S=m[l+"Subs"]||[],j=m["usedSubIds_"+l]||[],O=S.filter(P=>!j.includes(P.cardId)),ee=m["gcCardsFull_"+l]||[],R=m["usedGc_"+l]||[],F=ee.filter(P=>!R.includes(P.id)),H=m.boostOwner===l&&!m.boostUsed,X=!["GK","DEF","MIL","ATT"].some(P=>(T[P]||[]).some(le=>!le.used)),ae=[...w.MIL||[],...w.ATT||[]].filter(P=>!P.used),pe=v&&X&&ae.length===0?[...w.GK||[],...w.DEF||[]].filter(P=>!P.used).map(P=>P.cardId):[];function ce(P,le,W){var lt,wt;const ie=P._gcDef,Ae={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ie==null?void 0:ie.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",_e={purple:"#b06ce0",light_blue:"#00d4ef"}[ie==null?void 0:ie.color]||"#b06ce0",Xe=(ie==null?void 0:ie.name)||P.gc_type,Qe=(ie==null?void 0:ie.effect)||((lt=Re[P.gc_type])==null?void 0:lt.desc)||"",ot=ie!=null&&ie.image_url?`/icons/${ie.image_url}`:null,dt=((wt=Re[P.gc_type])==null?void 0:wt.icon)||"⚡",mt=Math.round(W*.22),vt=Math.round(W*.22),rt=W-mt-vt,Tt=Xe.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${P.id}" data-gc-type="${P.gc_type}"
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(_);let L=!1;const E=()=>{L||(L=!0,_.remove(),setTimeout(()=>x(),50))};_.addEventListener("click",E),setTimeout(E,3500)}function Le(w,T,g){var R,F;const y=(m.gcDefs||[]).find(H=>{var X;return H.name===w||((X=H.name)==null?void 0:X.toLowerCase().trim())===(w==null?void 0:w.toLowerCase().trim())}),x={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[y==null?void 0:y.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",_={purple:"#b06ce0",light_blue:"#00d4ef"}[y==null?void 0:y.color]||"#b06ce0",v=(y==null?void 0:y.name)||w,k=(y==null?void 0:y.effect)||((R=Re[w])==null?void 0:R.desc)||"",L=y!=null&&y.image_url?`/icons/${y.image_url}`:null,E=((F=Re[w])==null?void 0:F.icon)||"⚡",S=T===l?"Vous":m[T+"Name"]||"Adversaire",j=document.createElement("div");j.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",j.innerHTML=`
      <style>
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(j);let O=!1;const ee=()=>{O||(O=!0,j.remove(),setTimeout(()=>g&&g(),50))};j.addEventListener("click",ee),setTimeout(ee,3e3)}function Fe(w,T){var j,O,ee,R;const y=(m["gcCardsFull_"+l]||[]).find(F=>F.id===w),x=y==null?void 0:y._gcDef,_={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[x==null?void 0:x.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",v={purple:"#b06ce0",light_blue:"#00d4ef"}[x==null?void 0:x.color]||"#b06ce0",k=(x==null?void 0:x.name)||T,L=(x==null?void 0:x.effect)||((j=Re[T])==null?void 0:j.desc)||"Carte spéciale.",E=x!=null&&x.image_url?`/icons/${x.image_url}`:null,I=((O=Re[T])==null?void 0:O.icon)||"⚡",S=document.createElement("div");S.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",S.innerHTML=`
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      ...
      ${w.forfeit?`<div style="font-size:13px;color:rgba(255,255,255,0.5)">${y?"L'adversaire a quitté":"Perdu par forfait"}</div>`:""}
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(_),(I=_.querySelector("#pvp-end-home"))==null||I.addEventListener("click",()=>{_.remove(),Ve(e),c("home")})}const N=z.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},w=>{const T=w.new;try{if(T.status==="finished"||T.forfeit){if(u)return;u=!0,M&&(clearInterval(M),M=null),T.game_state&&(p=T.game_state),C(T);return}if(T.game_state){const g=p;p=T.game_state;const y=p._lastGC;if(y&&y.seq&&!B.has(y.seq)&&(B.add(y.seq),y.by!==a)){Le(y.type,y.by,()=>ne());return}const x=g[a+"Score"]||0,_=g[o+"Score"]||0,v=p[a+"Score"]||0,k=p[o+"Score"]||0,L=v>x,E=k>_;if((L||E)&&!q.has(p.round)){q.add(p.round);const I=[...p.log||[]].reverse().find(j=>j.isGoal),S=((I==null?void 0:I.homePlayers)||[]).map(j=>({name:j.name,note:j.note,portrait:j.portrait,job:j.job}));xe(S,v,k,L,()=>ne());return}ne()}}catch(g){console.error("[PvP] crash:",g)}}).subscribe();async function J(w){Object.assign(p,w),p.lastActionAt=new Date().toISOString();const{error:T}=await z.from("matches").update({game_state:p}).eq("id",i);T&&f("Erreur de synchronisation","error");try{ne()}catch(g){console.error("[PvP] renderPvpScreen crash:",g)}}async function Z(){if(u)return;u=!0,M&&(clearInterval(M),M=null);const w=n?h.away_id:h.home_id,T=n?"p2":"p1",g=n?"p1":"p2",y={...p,[T+"Score"]:3,[g+"Score"]:0,phase:"finished"};try{await z.from("matches").update({status:"finished",forfeit:!0,winner_id:w,home_score:y.p1Score||0,away_score:y.p2Score||0,game_state:y}).eq("id",i)}catch{}try{z.removeChannel(N)}catch{}setTimeout(()=>{Ve(e),c("home")},800)}const se={BOOST_STAT:({value:w=1,count:T=1,roles:g=[]},y,x)=>{const _=y[a+"Team"],v=Object.entries(_).filter(([k])=>!g.length||g.includes(k)).flatMap(([k,L])=>L.filter(E=>!E.used).map(E=>({...E,_line:k})));if(!v.length){y.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),x(y);return}re(v,T,`Choisir ${T} joueur(s) à booster (+${w})`,k=>{k.forEach(L=>{const E=(_[L._line]||[]).find(I=>I.cardId===L.cardId);E&&(E.boost=(E.boost||0)+w,y.log.push({text:`⚡ +${w} sur ${E.name}`,type:"info"}))}),y[a+"Team"]=_,x(y)})},DEBUFF_STAT:({value:w=1,count:T=1,roles:g=[],target:y="ai"},x,_)=>{const v=y==="home"?a:o,k=x[v+"Team"],L=y==="home"?"allié":"adverse",E=Object.entries(k).filter(([I])=>!g.length||g.includes(I)).flatMap(([I,S])=>S.map(j=>({...j,_line:I})));if(!E.length){x.log.push({text:`🎯 Aucun joueur ${L}`,type:"info"}),_(x);return}re(E,T,`Choisir ${T} joueur(s) ${L}(s) (-${w})`,I=>{I.forEach(S=>{const j=(k[S._line]||[]).find(O=>O.cardId===S.cardId);j&&(j.boost=(j.boost||0)-w,x.log.push({text:`🎯 -${w} sur ${j.name}`,type:"info"}))}),x[v+"Team"]=k,_(x)})},GRAY_PLAYER:({count:w=1,roles:T=[],target:g="ai"},y,x)=>{const _=g==="home"?a:o,v=y[_+"Team"],k=g==="home"?"allié":"adverse",L=Object.entries(v).filter(([E])=>!T.length||T.includes(E)).flatMap(([E,I])=>I.filter(S=>!S.used).map(S=>({...S,_line:E})));if(!L.length){y.log.push({text:`❌ Aucun joueur ${k}`,type:"info"}),x(y);return}re(L,w,`Choisir ${w} joueur(s) ${k}(s) à exclure`,E=>{const I="usedCardIds_"+_,S=new Set(y[I]||[]);E.forEach(j=>{const O=(v[j._line]||[]).find(ee=>ee.cardId===j.cardId);O&&(O.used=!0,S.add(j.cardId),y.log.push({text:`❌ ${O.name} exclu !`,type:"info"}))}),y[I]=[...S],y[_+"Team"]=v,x(y)})},REVIVE_PLAYER:({count:w=1,roles:T=[]},g,y)=>{const x=g[a+"Team"],_=Object.entries(x).filter(([v])=>!T.length||T.includes(v)).flatMap(([v,k])=>k.filter(L=>L.used).map(L=>({...L,_line:v})));if(!_.length){g.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),y(g);return}re(_,w,`Choisir ${w} joueur(s) à ressusciter`,v=>{v.forEach(k=>{const L=(x[k._line]||[]).find(E=>E.cardId===k.cardId);L&&(L.used=!1,g.log.push({text:`💫 ${L.name} ressuscité !`,type:"info"}))}),g[a+"Team"]=x,y(g)})},REMOVE_GOAL:({},w,T)=>{const g=o+"Score";w[g]>0?(w[g]--,w.log.push({text:"🚫 Dernier but annulé !",type:"info"})):w.log.push({text:"🚫 Aucun but à annuler",type:"info"}),T(w)},ADD_GOAL_DRAW:({},w,T)=>{w[a+"Score"]===w[o+"Score"]?(w[a+"Score"]++,w.log.push({text:"🎯 But bonus !",type:"info"})):w.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),T(w)},ADD_SUB:({value:w=1},T,g)=>{T.maxSubs=(T.maxSubs||3)+w,T.log.push({text:`🔄 +${w} remplacement(s)`,type:"info"}),g(T)},CUSTOM:({},w,T)=>T(w)};function re(w,T,g,y){const x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let _=[];function v(){var L,E;const k=w.map(I=>{const S={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[I._line]||"#555",j=ze(I,I._line)+(I.boost||0),ee=_.find(F=>F.cardId===I.cardId)?"#FFD700":"rgba(255,255,255,0.25)",R=I.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${I.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${ee};background:${S};overflow:hidden;cursor:pointer;${R}">
          <div style="background:rgba(255,255,255,0.9);text-align:center;padding:2px;font-size:7px;font-weight:900;color:#111;overflow:hidden;white-space:nowrap;text-overflow:ellipsis">${I.name||"?"}</div>
          <div style="height:50px;display:flex;align-items:center;justify-content:center;font-size:22px;font-weight:900;color:#fff">${j}</div>
          <div style="background:rgba(255,255,255,0.9);text-align:center;padding:2px;font-size:8px;font-weight:700;color:#333">${I._line}</div>
        </div>`}).join("");x.innerHTML=`
        ...
        </div>`,(L=x.querySelector("#pp-close"))==null||L.addEventListener("click",()=>x.remove()),x.querySelectorAll(".pp-item").forEach(I=>{I.addEventListener("click",()=>{const S=I.dataset.cid,j=w.find(ee=>ee.cardId===S),O=_.findIndex(ee=>ee.cardId===S);j&&(O>-1?_.splice(O,1):_.length<T&&_.push(j),v())})}),(E=x.querySelector("#pp-confirm"))==null||E.addEventListener("click",()=>{x.remove(),y(_)})}v(),document.body.appendChild(x)}async function Q(w,T){const y=(p["gcCardsFull_"+a]||[]).find(k=>k.id===w),x=(y==null?void 0:y._gcDef)||(p.gcDefs||[]).find(k=>{var L;return k.name===T||((L=k.name)==null?void 0:L.toLowerCase().trim())===(T==null?void 0:T.toLowerCase().trim())}),_=[...p["usedGc_"+a]||[],w],v={type:T,by:a,seq:(p._gcAnimSeq||0)+1};B.add(v.seq),Le(T,a,async()=>{if(x!=null&&x.effect_type&&x.effect_type!=="CUSTOM"){const L=se[x.effect_type];if(L){const E={...p};L(x.effect_params||{},E,async I=>{I["usedGc_"+a]=_,I._lastGC=v,I._gcAnimSeq=v.seq,await J(I)});return}}const k={...p};switch(T){case"Remplacement+":k.maxSubs=(k.maxSubs||3)+1,k.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const L=o+"Score";k[L]>0&&(k[L]--,k.log.push({text:"🚫 But annulé",type:"info"}));break}}k["usedGc_"+a]=_,k._lastGC=v,k._gcAnimSeq=v.seq,await J(k)})}function U(w,T){const g="usedCardIds_"+w,y=new Set(p[g]||[]);T.forEach(x=>y.add(x)),p[g]=[...y]}function D(){for(const w of["p1","p2"]){const T=new Set(p["usedCardIds_"+w]||[]),g=p[w+"Team"];if(g)for(const y of["GK","DEF","MIL","ATT"])(g[y]||[]).forEach(x=>{x.used=T.has(x.cardId)})}}function ne(){var Ke,Ze,it,ut,ft,gt;if(p.phase==="reveal")return ue();if(p.phase==="midfield")return me();if(p.phase==="finished")return je();const w=p[a+"Team"],T=p[o+"Team"];D();const g=p[a+"Score"],y=p[o+"Score"],x=p[a+"Name"],_=p[o+"Name"],v=p.phase===a+"-attack",k=p.phase===a+"-defense",L=Array.isArray(p["selected_"+a])?p["selected_"+a]:[],E=L.map(P=>P.cardId),I=window.innerWidth>=700,S=p[a+"Subs"]||[],j=p["usedSubIds_"+a]||[],O=S.filter(P=>!j.includes(P.cardId)),ee=p["gcCardsFull_"+a]||[],R=p["usedGc_"+a]||[],F=ee.filter(P=>!R.includes(P.id)),H=p.boostOwner===a&&!p.boostUsed,X=!["GK","DEF","MIL","ATT"].some(P=>(T[P]||[]).some(le=>!le.used)),ae=[...w.MIL||[],...w.ATT||[]].filter(P=>!P.used),pe=v&&X&&ae.length===0?[...w.GK||[],...w.DEF||[]].filter(P=>!P.used).map(P=>P.cardId):[];function ce(P,le,W){var lt,wt;const ie=P._gcDef,Ae={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ie==null?void 0:ie.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",_e={purple:"#b06ce0",light_blue:"#00d4ef"}[ie==null?void 0:ie.color]||"#b06ce0",Xe=(ie==null?void 0:ie.name)||P.gc_type,Qe=(ie==null?void 0:ie.effect)||((lt=Re[P.gc_type])==null?void 0:lt.desc)||"",ot=ie!=null&&ie.image_url?`/icons/${ie.image_url}`:null,dt=((wt=Re[P.gc_type])==null?void 0:wt.icon)||"⚡",mt=Math.round(W*.22),vt=Math.round(W*.22),rt=W-mt-vt,Tt=Xe.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${P.id}" data-gc-type="${P.gc_type}"
        style="box-sizing:border-box;width:${le}px;height:${W}px;border-radius:10px;border:2px solid ${_e};background:${Ae};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
        <div style="height:${mt}px;background:rgba(255,255,255,0.14);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 3px">
          <span style="font-size:${Tt}px;font-weight:900;color:#fff;text-align:center;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${le-6}px">${Xe}</span>
          <span style="font-size:6px;color:rgba(255,255,255,0.45)">⚡ GC</span>
    ...
    </div>`,a==="p1"&&setTimeout(async()=>{await J({phase:"midfield"})},5e3)}let ye=!1;function me(){if(ye)return;const w=p[a+"Team"].MIL||[],T=p[o+"Team"].MIL||[];function g(F){return F.reduce((H,X)=>H+ze(X,"MIL"),0)}function y(F){let H=0;for(let X=0;X<F.length-1;X++){const ae=nt(F[X],F[X+1]);ae==="#00ff88"?H+=2:ae==="#FFD700"&&(H+=1)}return H}const x=g(w)+y(w),_=g(T)+y(T),v=x>=_;function k(F,H,X){return`<div id="duel-row-${X}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
        <div style="font-size:11px;color:rgba(255,255,255,0.55);letter-spacing:2px;margin-bottom:10px;text-transform:uppercase;font-weight:700">${H}</div>
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${F.map((ae,pe)=>{const ce=pe<F.length-1?nt(ae,F[pe+1]):null,ge=!ce||ce==="#ff3333"||ce==="#cc2222",he=ce==="#00ff88"?"+2":ce==="#FFD700"?"+1":"";return`<div class="duel-card duel-card-${X}" data-idx="${pe}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease,transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
              ${We({...ae,note:ze(ae,"MIL"),_line:"MIL"},58,78)}
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(_);let L=!1;const E=()=>{L||(L=!0,_.remove(),setTimeout(()=>x(),50))};_.addEventListener("click",E),setTimeout(E,3500)}function Le(w,T,g){var R,F;const y=(p.gcDefs||[]).find(H=>{var X;return H.name===w||((X=H.name)==null?void 0:X.toLowerCase().trim())===(w==null?void 0:w.toLowerCase().trim())}),x={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[y==null?void 0:y.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",_={purple:"#b06ce0",light_blue:"#00d4ef"}[y==null?void 0:y.color]||"#b06ce0",v=(y==null?void 0:y.name)||w,k=(y==null?void 0:y.effect)||((R=Re[w])==null?void 0:R.desc)||"",L=y!=null&&y.image_url?`/icons/${y.image_url}`:null,E=((F=Re[w])==null?void 0:F.icon)||"⚡",S=T===a?"Vous":p[T+"Name"]||"Adversaire",j=document.createElement("div");j.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",j.innerHTML=`
      <style>
        @keyframes gcFlipIn{0%{transform:perspective(800px) rotateY(90deg) scale(.7);opacity:0}55%{transform:perspective(800px) rotateY(-12deg) scale(1.08);opacity:1}100%{transform:perspective(800px) rotateY(0) scale(1);opacity:1}}
        @keyframes gcGlow{0%,100%{box-shadow:0 0 30px ${_}66}50%{box-shadow:0 0 60px ${_}cc}}
        @keyframes gcLabel{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:translateY(0)}}
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(j);let O=!1;const ee=()=>{O||(O=!0,j.remove(),setTimeout(()=>g&&g(),50))};j.addEventListener("click",ee),setTimeout(ee,3e3)}function Fe(w,T){var j,O,ee,R;const y=(p["gcCardsFull_"+a]||[]).find(F=>F.id===w),x=y==null?void 0:y._gcDef,_={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[x==null?void 0:x.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",v={purple:"#b06ce0",light_blue:"#00d4ef"}[x==null?void 0:x.color]||"#b06ce0",k=(x==null?void 0:x.name)||T,L=(x==null?void 0:x.effect)||((j=Re[T])==null?void 0:j.desc)||"Carte spéciale.",E=x!=null&&x.image_url?`/icons/${x.image_url}`:null,I=((O=Re[T])==null?void 0:O.icon)||"⚡",S=document.createElement("div");S.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",S.innerHTML=`
      <div style="width:190px;border-radius:16px;border:3px solid ${v};background:${_};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 40px ${v}66">
        <div style="padding:10px;background:rgba(255,255,255,0.14);text-align:center">
          <div style="font-size:${k.length>14?11:14}px;font-weight:900;color:#fff;letter-spacing:.5px;text-transform:uppercase">${k}</div>
          <div style="font-size:8px;color:rgba(255,255,255,0.5);margin-top:2px">⚡ GAME CHANGER</div>
      ...
      <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
        <button id="psub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
      </div>`,(ce=S.querySelector("#psub-close"))==null||ce.addEventListener("click",()=>S.remove()),(ge=S.querySelector("#pout-up"))==null||ge.addEventListener("click",()=>{k>0&&(k--,j())}),(he=S.querySelector("#pout-down"))==null||he.addEventListener("click",()=>{k<_.length-1&&(k++,j())}),(Ie=S.querySelector("#pin-up"))==null||Ie.addEventListener("click",()=>{E>0&&(E--,j())}),(Se=S.querySelector("#pin-down"))==null||Se.addEventListener("click",()=>{E<v.length-1&&(E++,j())});const pe=(Ee,Ge,Me,Ue)=>{const Ne=S.querySelector("#"+Ee);if(!Ne)return;let Je=0;Ne.addEventListener("touchstart",He=>{Je=He.touches[0].clientY},{passive:!0}),Ne.addEventListener("touchend",He=>{const Be=He.changedTouches[0].clientY-Je;if(Math.abs(Be)<30)return;const Ke=Ge();Be<0&&Ke<Ue-1?(Me(Ke+1),j()):Be>0&&Ke>0&&(Me(Ke-1),j())},{passive:!0})};pe("pin-panel",()=>E,Ee=>E=Ee,v.length),pe("pout-panel",()=>k,Ee=>k=Ee,_.length),(Oe=S.querySelector("#psub-confirm"))==null||Oe.addEventListener("click",async Ee=>{if(Ee.preventDefault(),Ee.stopPropagation(),I)return;I=!0;const Ge=_[k],Me=v[E];if(!Ge||!Me)return;const Ue=Ge._line,Ne=(g[Ue]||[]).findIndex(Be=>Be.cardId===Ge.cardId);if(Ne===-1){f("Erreur : joueur introuvable","error"),S.remove();return}const Je={...Me,_line:Ue,position:Ge.position,used:!1,boost:0};g[Ue].splice(Ne,1,Je);const He=[...y,Me.cardId];S.remove(),V(Ge,Me,async()=>{await J({[a+"Team"]:g,[o+"Team"]:p[o+"Team"],["usedSubIds_"+a]:He})})})}document.body.appendChild(S),j()}function V(w,T,g){const y={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const _=(L,E,I)=>`<div style="text-align:center">
      <div style="font-size:9px;color:${E};letter-spacing:2px;text-transform:uppercase;font-weight:700;margin-bottom:6px">${I}</div>
      <div style="width:70px;height:70px;border-radius:50%;background:${y[L.job]||"#555"};border:3px solid ${E};position:relative;overflow:hidden;margin:0 auto">
        ${Pe(L)?`<img src="${Pe(L)}" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover">`:""}
      </div>
      ...
      <div style="width:min(90vw,420px)">${ct(p[o+"Team"],p[o+"Formation"],null,[],300,300)}</div>
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(w),(T=w.querySelector("#pvp-opp-close"))==null||T.addEventListener("click",()=>w.remove())}function fe(){var y;const w=p.log||[],T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column";const g=x=>{var v,k,L;if(x.type==="duel"){const E=x.isGoal,I=x.homeScored&&a==="p1"||!x.homeScored&&E&&a==="p2",S=x.homeScored?"#FFD700":E?"#ff6b6b":"rgba(255,255,255,0.3)",j=E?I?"⚽ BUT !":"⚽ BUT adversaire !":(v=x.homePlayers)!=null&&v.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${E?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${S};margin-bottom:4px">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
  ...
  </div>`;let f="buy";const a=()=>{var p,u,m,A,M,G,q;return{name:(((p=document.getElementById("flt-name"))==null?void 0:p.value)||"").toLowerCase().trim(),club:(((u=document.getElementById("flt-club"))==null?void 0:u.value)||"").toLowerCase().trim(),country:(((m=document.getElementById("flt-country"))==null?void 0:m.value)||"").toLowerCase().trim(),job:((A=document.getElementById("flt-job"))==null?void 0:A.value)||"",rarity:((M=document.getElementById("flt-rarity"))==null?void 0:M.value)||"",note1:parseInt((G=document.getElementById("flt-note1"))==null?void 0:G.value)||0,note2:parseInt((q=document.getElementById("flt-note2"))==null?void 0:q.value)||0}};function o(p){const u=a();return p.filter(m=>{var J,Z,se;const A=(J=m.card)==null?void 0:J.player;if(!A)return!1;const M=`${A.firstname} ${A.surname_encoded}`.toLowerCase(),G=(((Z=A.clubs)==null?void 0:Z.encoded_name)||"").toLowerCase(),q=(A.country_code||"").toLowerCase(),B=((se=m.card)==null?void 0:se.evolution_bonus)||0,C=At(A,A.job,B),N=A.job2?At(A,A.job2,B):0;return!(u.name&&!M.includes(u.name)||u.club&&!G.includes(u.club)||u.country&&!q.includes(u.country)||u.job&&A.job!==u.job||u.rarity&&A.rarity!==u.rarity||u.note1&&C<u.note1||u.note2&&N<u.note2)})}function l(p){var q,B,C,N;const u=(q=p.card)==null?void 0:q.player;if(!u)return"";const m=((B=p.card)==null?void 0:B.evolution_bonus)||0,A=At(u,u.job,m),M=u.job2?At(u,u.job2,m):0,G=(i.profile.credits||0)>=p.price;return`<div class="card-panel" style="display:flex;align-items:center;gap:8px;padding:10px 12px;overflow:hidden;padding-left:6px">
      ${Yi(u.rarity)}
      ${Xt(A,u.job)}
      ${Xt(M,u.job2||null)}
      ${Wi(u.country_code)}
    ...
    </div>`}function b(p){var q,B,C,N;const u=(q=p.card)==null?void 0:q.player;if(!u)return"";const m=((B=p.card)==null?void 0:B.evolution_bonus)||0,A=At(u,u.job,m),M=u.job2?At(u,u.job2,m):0,G=p.status==="sold";return`<div class="card-panel" style="display:flex;align-items:center;gap:8px;padding:10px 12px;overflow:hidden;padding-left:6px;${G?"opacity:0.7":""}">
      ${Yi(u.rarity)}
      ${Xt(A,u.job)}
      ${Xt(M,u.job2||null)}
      ${Wi(u.country_code)}
    ...
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center">
      ...
      `).join(""):'<div style="text-align:center;color:var(--gray-600);padding:40px">Aucun manager.</div>'}else{const m=r||[];$.innerHTML=m.length>0?m.map((A,M)=>{const G=Pt(A.mmr??1e3),q=(A.ranked_wins||0)+(A.ranked_losses||0)+(A.ranked_draws||0),B=q>0?Math.round((A.ranked_wins||0)/q*100):0,C=A.id===i.profile.id,N=(A.placement_matches||0)<10;return`
          <div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${C?"border:2px solid var(--yellow)":""}">
            ...
            <div style="flex:1;min-width:0">
              ...
              <div style="font-size:11px;color:var(--gray-600)">${A.club_name} · ${G.label}</div>
            </div>
            <div style="text-align:right;flex-shrink:0">
  ...
  <div style="min-height:100%;background:${A[$.id]};padding:16px;overflow-y:auto;display:flex;flex-direction:column;gap:16px">
    ...
    <div style="display:flex;flex-direction:column;gap:10px;margin-top:auto;padding-top:8px">
      ...
      away:users!away_id(pseudo,club_name)`).or(`home_id.eq.${d.id},away_id.eq.${d.id}`).order("created_at",{ascending:!1}).limit(50),r={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},c=(s||[]).filter(o=>o.status==="finished"),f=(s||[]).filter(o=>o.status==="in_progress");function a(o){const l=o.home_id===d.id;l?o.home_score:o.away_score,l?o.away_score:o.home_score;const b=o.winner_id===d.id,h=o.home_score===o.away_score&&o.status==="finished",$=o.status!=="finished"?"…":h?"N":b?"V":"D",p=o.status!=="finished"||h?"#888":b?"#1A6B3C":"#c0392b";let u=r[o.mode]||o.mode;o.away_id===null&&!u.startsWith("IA")&&(u="IA");const A=o.home_id===d.id?o.away:o.home;let M;o.away_id===null?M=u:A?M=`${A.club_name||A.pseudo} (${A.pseudo})`:M="Adversaire";let G="";o.status==="in_progress"&&Date.now()-new Date(o.created_at).getTime()>3600*1e3&&(G=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const q=new Date(o.created_at),B=q.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+q.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),C=o.status==="finished"?`${o.home_score} - ${o.away_score}`:`${o.home_score||0} - ${o.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
