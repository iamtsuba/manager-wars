...
import{s as h,j as hi,q as In,t as Ut,u as Oi,F as Hi,o as tt,n as ti,g as At,r as Ie,m as Mt,v as Mi,w as co,T as Ti,p as po,x as uo,d as fo}from"./special-cards-q1oKjNQl.js";const mo="/";function pn(e,{navigate:t,toast:i}){let n="login";const o=()=>{var a,s,d,c,l,f,y;const r=n==="login";if(e.innerHTML=`
    <div style="
  ...
  `;function s(){var g;const c=document.getElementById("logo-preview"),l=document.getElementById("logo-initials"),f=((g=document.getElementById("setup-club"))==null?void 0:g.value)||"MW",y=f.trim().split(" ").filter(Boolean),x=y.length>=2?(y[0][0]+y[1][0]).toUpperCase():f.slice(0,2).toUpperCase();c&&(c.style.background=a,c.style.borderColor=r),l&&(l.textContent=x,l.style.color=r)}document.getElementById("color1").addEventListener("input",c=>{r=c.target.value,document.getElementById("swatch1").style.background=r,s()}),document.getElementById("color2").addEventListener("input",c=>{a=c.target.value,document.getElementById("swatch2").style.background=a,s()});function d(c){document.querySelectorAll(".setup-step").forEach(l=>l.classList.remove("active")),document.getElementById(`step-${c}`).classList.add("active"),document.getElementById("step-num").textContent=c,document.getElementById("progress-fill").style.width=`${Math.round(c/3*100)}%`,c===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("step1-error");if(l.textContent="",c.length<3){l.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:f}=await h.from("users").select("id").eq("pseudo",c).maybeSingle();if(f){l.textContent="Ce pseudo est déjà pris.";return}d(2)}),document.getElementById("step2-back").addEventListener("click",()=>d(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const c=document.getElementById("setup-club").value.trim(),l=document.getElementById("step2-error");if(l.textContent="",c.length<2){l.textContent="Nom trop court (min. 2 caractères).";return}const{data:f}=await h.from("users").select("id").eq("club_name",c).maybeSingle();if(f){l.textContent="Ce nom de club est déjà pris.";return}d(3)}),document.getElementById("step3-back").addEventListener("click",()=>d(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("setup-club").value.trim(),f=document.getElementById("step3-error"),y=document.getElementById("step3-finish");f.textContent="",y.disabled=!0,y.textContent="Création en cours…";try{const{error:x}=await h.from("users").insert({id:t.user.id,pseudo:c,club_name:l,club_color1:r,club_color2:a,credits:1e4});if(x)throw x;await xo(t.user.id),await o(),n(`Bienvenue ${c} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(x){f.textContent=x.message,y.disabled=!1,y.textContent="🚀 Créer mon profil !"}})}async function xo(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await h.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const yo="modulepreload",bo=function(e){return"/"+e},un={},Ui=function(t,i,n){let o=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),s=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));o=Promise.allSettled(i.map(d=>{if(d=bo(d),d in un)return;un[d]=!0;const c=d.endsWith(".css"),l=c?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${l}`))return;const f=document.createElement("link");if(f.rel=c?"stylesheet":yo,c||(f.as="script"),f.crossOrigin="",f.href=d,s&&f.setAttribute("nonce",s),document.head.appendChild(f),c)return new Promise((y,x)=>{f.addEventListener("load",y),f.addEventListener("error",()=>x(new Error(`Unable to preload CSS for ${d}`)))})}))}function r(a){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=a,window.dispatchEvent(s),!s.defaultPrevented)throw a}return o.then(a=>{for(const s of a||[])s.status==="rejected"&&r(s.reason);return t().catch(r)})},ii="#1A6B3C",ni="#cc2222",vo="#D4A017",fn="#888";async function ho(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      ${y.map(({friendshipId:x,friend:g})=>wo(g,x)).join("")}
    </div>`,c.querySelectorAll("[data-stats]").forEach(x=>{x.addEventListener("click",()=>ko(e,x.dataset.stats,x.dataset.friendName))}),c.querySelectorAll("[data-match]").forEach(x=>{x.addEventListener("click",async()=>{const g=x.dataset.friendId,p=x.dataset.friendName;if(typeof n!="function"){t("Erreur navigation","error");return}const u=e.user.id,{data:b}=await h.from("friend_match_invites").select("id").eq("inviter_id",g).eq("invitee_id",u).eq("status","pending").order("created_at",{ascending:!1}).limit(1).maybeSingle(),w=!!b;console.log("[Friends] clic match",{fid:g,fname:p,isAccepting:w}),n("match",{matchMode:"friend",friendId:g,friendName:p,isAccepting:w})})})}function wo(e,t){const i=e.club_name||e.pseudo||"?",n=e.pseudo||"",o=(n||i).slice(0,2).toUpperCase(),r=e.club_color2||ii,a=e.club_color1||"#ffffff",s=e.last_seen_at?new Date(e.last_seen_at):null,d=s&&Date.now()-s.getTime()<3*60*1e3;return`
    ...
    ${Yi()}`,document.body.appendChild(c);const f=()=>c.remove();c.querySelector("#pending-close").addEventListener("click",f),c.addEventListener("click",y=>{y.target===c&&f()}),c.querySelectorAll("[data-accept]").forEach(y=>{y.addEventListener("click",async()=>{const{error:x}=await h.from("friendships").update({status:"accepted"}).eq("id",y.dataset.accept);if(x){t("Erreur : "+x.message,"error");return}y.closest("div[style]").remove(),t("✅ Ami accepté !","success"),zn(e,t,n),i&&i()})}),c.querySelectorAll("[data-decline]").forEach(y=>{y.addEventListener("click",async()=>{await h.from("friendships").delete().eq("id",y.dataset.decline),y.closest("div[style]").remove(),t("Demande refusée","info"),i&&i()})})}async function ko(e,t,i){const n=e.user.id,[o,r]=[n,t].sort(),a=n===o,{data:s}=await h.from("friend_match_stats").select("*").eq("player1_id",o).eq("player2_id",r).single(),d=e.profile.club_name||e.profile.pseudo||"Moi",c=s||{},l=a?c.wins_p1||0:c.wins_p2||0,f=a?c.wins_p2||0:c.wins_p1||0,y=c.draws||0,x=a?c.goals_p1||0:c.goals_p2||0,g=a?c.goals_p2||0:c.goals_p1||0,p=a?c.gc_used_p1||0:c.gc_used_p2||0,u=a?c.gc_used_p2||0:c.gc_used_p1||0,b=c.matches_total||0,w=(E,m,C,k=ii,z=ni)=>`
  ...
  <div class="home-dark" id="home-dark">
    ...
    <div class="news-item">
      ...
      <div class="news-body">
        ...
        <div class="news-time">${Mo(L.published_at)}</div>
      </div>
    </div>`;let w=0;const v=L=>`
      ...
      <div class="home2-dash">
        ...
        <div class="home2-col-center">
          ...
          <div class="promo-banner" id="promo-banner">
            ...
            ${p.length>1?`<div class="promo-dots">${p.map((L,N)=>`<div class="promo-dot ${N===0?"active":""}" data-dot="${N}"></div>`).join("")}</div>`:""}
          </div>`:""}
        </div>
  ...
  </div>`,requestAnimationFrame(()=>{var $,B,H;const L=(($=window.visualViewport)==null?void 0:$.height)||window.innerHeight,N=((B=document.querySelector(".top-nav"))==null?void 0:B.offsetHeight)||56,ce=((H=document.querySelector(".bottom-nav"))==null?void 0:H.offsetHeight)||60,O=L-N-ce,V=e.querySelector(".home-dark");V&&(V.style.minHeight=O+"px")});const E=()=>{const L=document.getElementById("mode-indicator");L&&(L.textContent=window.innerWidth>=1024?"🖥️ Mode PC":"📱 Mode Mobile")};if(E(),window.addEventListener("resize",E),(m=document.getElementById("tab-home"))==null||m.addEventListener("click",()=>i("home2")),(C=document.getElementById("tab-cards"))==null||C.addEventListener("click",()=>i("collection")),(k=document.getElementById("tab-decks"))==null||k.addEventListener("click",()=>i("decks")),(z=document.getElementById("tab-boosters"))==null||z.addEventListener("click",()=>i("boosters")),(q=document.getElementById("tab-market"))==null||q.addEventListener("click",()=>i("market")),(W=document.getElementById("nav-settings-btn"))==null||W.addEventListener("click",()=>i("settings")),(te=document.getElementById("nav-edit-btn"))==null||te.addEventListener("click",()=>i("settings")),(Z=document.getElementById("nav-profile-btn"))==null||Z.addEventListener("click",()=>i("settings")),(M=document.getElementById("nav-rankings-link"))==null||M.addEventListener("click",()=>i("rankings")),(I=document.getElementById("nav-rankings-cta"))==null||I.addEventListener("click",()=>i("rankings")),(Y=document.getElementById("rank-inline-link-btn"))==null||Y.addEventListener("click",()=>i("rankings")),(J=document.getElementById("nav-journal-link"))==null||J.addEventListener("click",()=>Do()),e.querySelectorAll("[data-action]").forEach(L=>{L.addEventListener("click",()=>{L.style.transform="scale(.96)",setTimeout(()=>L.style.transform="",180);const N=L.dataset.action;if(N==="match-ai"){Oo(i);return}if(N==="match-random"){i("match",{matchMode:"random"});return}if(N==="match-friend"){i("friends");return}if(N==="mini-league"){i("mini-league");return}if(N==="ranked"){i("ranked");return}n("Bientôt disponible","info")})}),p.length>1){const L=document.getElementById("promo-slide-content"),N=()=>Array.from(document.querySelectorAll(".promo-dot")),ce=O=>{w=O,L&&(L.innerHTML=v(p[w])),N().forEach((V,$)=>V.classList.toggle("active",$===w))};setInterval(()=>ce((w+1)%p.length),5e3),document.querySelectorAll(".promo-dot").forEach(O=>{O.addEventListener("click",()=>ce(Number(O.dataset.dot)))})}(ue=document.getElementById("promo-cta-btn"))==null||ue.addEventListener("click",()=>i("boosters")),document.getElementById("logout-btn").addEventListener("click",async()=>{await h.auth.signOut(),window.location.reload()}),(fe=document.getElementById("back-to-v1-btn"))==null||fe.addEventListener("click",()=>i("home")),qn(t,n),No(t,n,i),Bn(t,n,i),Po(t,n)}async function Po(e,t){const i=e.profile.id,{data:n}=await h.from("mini_league_members").select("league_id, prize_amount, mini_leagues(name)").eq("user_id",i).eq("prize_claimed",!1).gt("prize_amount",0);if(!(n!=null&&n.length))return;const o=n.reduce((a,s)=>a+(s.prize_amount||0),0),r=document.createElement("div");r.className="modal-overlay",r.style.zIndex="2200",r.innerHTML=`<div class="modal" style="max-width:400px">
    <div class="modal-header"><h2 style="color:#1A6B3C">🏆 Gains à récupérer</h2></div>
    <div class="modal-body" style="padding:18px 20px">
      <p style="font-size:14px;color:#555;margin:0 0 14px">Tu as terminé sur le podium de ${n.length>1?"plusieurs mini leagues":"une mini league"} !</p>
      <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:16px">
      ...
      <button data-resume="${d.id}" data-mini="${f?"1":""}" style="width:38px;height:38px;border-radius:50%;border:none;background:#22c55e;color:#fff;font-size:18px;cursor:pointer;flex-shrink:0">⚽</button>
      <button data-abandon="${d.id}" data-opp="${c}" style="width:38px;height:38px;border-radius:50%;border:none;background:#cc2222;color:#fff;font-size:18px;cursor:pointer;flex-shrink:0">✕</button>
    </div>`}).join(""),n.querySelectorAll("[data-resume]").forEach(d=>{d.addEventListener("click",async()=>{const c=document.getElementById("page-content")||document.getElementById("app");if(d.dataset.mini==="1"){const{data:l}=await h.from("mini_league_matches").select("id, league_id").eq("match_id",d.dataset.resume).single();l?i("match-mini-league",{mlMatchId:l.id,leagueId:l.league_id}):i("mini-league")}else{const{resumePvpMatch:l}=await Ui(async()=>{const{resumePvpMatch:f}=await Promise.resolve().then(()=>eo);return{resumePvpMatch:f}},void 0);l(c,{state:e,navigate:i,toast:t,openModal:null,closeModal:null,refreshProfile:null},d.dataset.resume)}})}),n.querySelectorAll("[data-abandon]").forEach(d=>{d.addEventListener("click",()=>{Ro(async()=>{await Go(d.dataset.abandon,d.dataset.opp,o),t("Match abandonné (défaite 3-0)","info"),Bn(e,t,i)})})})}async function Go(e,t,i){Tt();const{data:n}=await h.from("matches").select("home_id, away_id, game_state, mode").eq("id",e).single();if(!n)return;const o=n.home_id===i,r=o?0:3,a=o?3:0,s=n.game_state||{};s.p1Score=r,s.p2Score=a,s.phase="finished",s.forfeit=!0,await h.from("matches").update({status:"finished",forfeit:!0,winner_id:t,home_score:r,away_score:a,game_state:s}).eq("id",e),n.mode==="mini_league"&&await h.from("mini_league_matches").update({status:"finished",home_score:r,away_score:a}).eq("match_id",e)}function Ro(e){const t=document.createElement("div");t.className="modal-overlay",t.innerHTML=`<div class="modal" style="max-width:340px">
    <div class="modal-body" style="padding:26px 22px 20px;text-align:center">
      <div style="font-size:40px;margin-bottom:8px">⚠️</div>
      <div style="font-size:17px;font-weight:900;margin-bottom:6px;color:#1a1a1a">Abandonner le match ?</div>
      <div style="font-size:13px;color:#666;margin-bottom:18px">Tu perdras par forfait <b>3-0</b>.</div>
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    ${Ie(e,{width:140})}
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),c=(s||[]).filter($=>$.card_type==="player"&&$.player),l=(s||[]).filter($=>$.card_type==="game_changer"),f=(s||[]).filter($=>$.card_type==="formation"),y=(s||[]).filter($=>$.card_type==="stadium"),{data:x}=await h.from("gc_definitions").select("name,gc_type,color,effect,image_url"),g={};(x||[]).forEach($=>{g[$.name]=$});const{data:p}=await h.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),u={};(p||[]).forEach($=>{u[$.id]=$}),y.forEach($=>{$.stadium_def&&(u[$.stadium_id]=$.stadium_def)});const b=Object.keys(Hi),w=Object.keys(je),v={};c.forEach($=>{const B=$.player.id;v[B]=(v[B]||0)+1}),new Set(Object.keys(v).map($=>String($)));const E=new Set(f.map($=>$.formation)),m=new Set(l.map($=>$.gc_type));let C="player",k="Tous",z="",q=!1;function W(){return[...c].sort(($,B)=>{const H=di.indexOf($.player.job),U=di.indexOf(B.player.job);return H!==U?H-U:($.player.surname_real||"").localeCompare(B.player.surname_real||"")})}function te(){return[...d||[]].sort(($,B)=>{const H=di.indexOf($.job),U=di.indexOf(B.job);return H!==U?H-U:($.surname_real||"").localeCompare(B.surname_real||"")})}function Z(){return W().filter($=>{const B=$.player,H=k==="Tous"||B.job===k,U=!z||`${B.firstname} ${B.surname_real}`.toLowerCase().includes(z);return H&&U})}function M(){return te().filter($=>{const B=k==="Tous"||$.job===k,H=!z||`${$.firstname} ${$.surname_real}`.toLowerCase().includes(z);return B&&H})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:0 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none;align-items:center"></div>
  ...
  </div>`,function(B){const H=document.getElementById(B);if(!H)return;H.addEventListener("wheel",de=>{Math.abs(de.deltaY)<=Math.abs(de.deltaX)||(de.preventDefault(),H.scrollLeft+=de.deltaY)},{passive:!1});let U=!1,re=0,D=0,G=!1,se=!1;H.addEventListener("mousedown",de=>{U=!0,G=!1,re=de.pageX,D=H.scrollLeft}),window.addEventListener("mouseup",()=>{U=!1,G&&(H.style.cursor="",se=!0),G=!1}),window.addEventListener("mousemove",de=>{if(!U)return;const he=de.pageX-re;!G&&Math.abs(he)<6||(G=!0,H.style.cursor="grabbing",de.preventDefault(),H.scrollLeft=D-he)}),H.addEventListener("click",de=>{se&&(de.stopPropagation(),de.preventDefault(),se=!1)},!0)}("col-grid");function I(){const $=document.getElementById("col-filters");$&&(C==="player"?($.innerHTML=`
          ...
          ${Ko.map(B=>`
            <button class="filter-btn" data-job="${B}"
              style="flex-shrink:0;padding:4px 12px;border-radius:20px;font-size:12px;font-weight:600;cursor:pointer;
                border:1.5px solid ${B===k?"var(--green)":"var(--tile-border)"};
                background:${B===k?"var(--green)":"#fff"};
                color:${B===k?"#fff":"var(--tile-fg-dim)"}">
              ${B}
            </button>`).join("")}
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",B=>{z=B.target.value.toLowerCase(),Y()}),e.querySelectorAll(".filter-btn").forEach(B=>{B.addEventListener("click",()=>{k=B.dataset.job,I(),Y()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{q=!q,I(),Y()})):($.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{q=!q,I(),Y()})))}function Y(){const $=document.getElementById("col-grid");$&&(C==="player"?N($):C==="formation"?ce($):C==="stadium"?V($):O($))}function J($,B,H,U,re,D=0){const G=document.getElementById("col-grid"),se=document.getElementById("col-big");if(!G||!se)return;var de=0;function he(){var ct;const Te=window.innerWidth>=768,ve=document.getElementById("col-big"),ze=document.getElementById("col-gap"),Se=((ct=document.getElementById("col-grid"))==null?void 0:ct.parentElement)||null,Be=ve?ve.closest(".page"):null;if(ve&&Se&&Be){let $e=0;Array.from(Be.children).forEach(function(j){j!==ve&&j!==Se&&j!==ze&&($e+=j.offsetHeight)});const Ce=Math.max(0,Be.clientHeight-$e),qe=Math.round(Ce*(D/100)),Ge=Math.max(0,Ce-qe),_=Te?50/71:45/63,T=Math.round(Ge*_),A=Math.max(0,Ge-T);ze&&(ze.style.height=qe+"px"),ve.style.flex="none",ve.style.height=T+"px",ve.style.minHeight="0",Se.style.flex="none",Se.style.height=A+"px",Se.style.minHeight="0",Se.style.display="flex",Se.style.overflow="hidden"}const ke=document.getElementById("col-grid");ke&&(ke.style.height="100%",ke.style.flexShrink="0",ke.style.overflowX="auto",ke.style.overflowY="hidden",ke.style.alignItems="center",ke.style.width="100%"),se.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:center center">'+B($[de])+"</div>";var _t=se.querySelector("[data-card-id],[data-form-id],[data-gc-id]");_t&&_t.addEventListener("click",function(){U($[de])}),requestAnimationFrame(function(){var $e=document.getElementById("big-card-inner");if(!(!$e||!se)){var Ce=se.clientHeight,qe=se.clientWidth-16,Ge=$e.offsetHeight,_=$e.offsetWidth;if(Ge>0&&_>0&&Ce>40){var T=Math.min(Ce/Ge,qe/_);$e.style.transform="scale("+T.toFixed(3)+")",$e.style.transformOrigin="center center"}}}),G.innerHTML=$.map(function($e,Ce){var qe=Ce===de,Ge="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+(qe?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+Ce+'" style="'+Ge+'">'+H($e,qe)+"</div>"}).join(""),G.querySelectorAll(".col-mini-item").forEach(function($e){$e.addEventListener("click",function(){de=Number($e.dataset.idx),he(),$e.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})}),requestAnimationFrame(function(){var $e=ke?ke.clientHeight:0,Ce=G.querySelector(".col-mini-item > div");if(!(!$e||!Ce)){var qe=Ce.style.zoom;Ce.style.zoom="1";var Ge=Ce.offsetHeight;if(Ce.style.zoom=qe,!(Ge<=0)){var _=$e/Ge;G.querySelectorAll(".col-mini-item > div").forEach(function(T){T.style.zoom=_.toFixed(4)})}}})}he()}function ue($,B){var H=window.innerWidth>=768?.76:.54,U;if(!$||$._fake){var re=$?$.player:null;if(!re)return"";U=bn(re)}else U=zi($,"");var D=$&&!$._fake?v[$.player&&$.player.id]||1:0,G=D>0?'<div style="position:absolute;top:6px;right:6px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:11px;font-weight:900;padding:2px 7px;z-index:3;box-shadow:0 1px 4px rgba(0,0,0,0.4)">×'+D+"</div>":"";return'<div style="display:inline-block;position:relative;zoom:'+H+';pointer-events:none;line-height:0">'+G+U+"</div>"}function fe($,B,H){var U=H>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+H+"</div>":"",re=!!B,D=Mi($,Mt[$],{width:160});return re||(D='<div style="filter:grayscale(1);opacity:0.5">'+D+"</div>"),"<div "+(B?'data-form-id="'+B.id+'"':"")+' style="position:relative;cursor:pointer">'+U+D+"</div>"}function L($,B){var H=window.innerWidth>=768?.76:.54,U=Mi($,Mt[$],{width:140});return B||(U='<div style="filter:grayscale(1);opacity:0.45">'+U+"</div>"),'<div style="display:inline-block;zoom:'+H+';line-height:0;pointer-events:none">'+U+"</div>"}function N($){if(q){const B=M();if(!B.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucun joueur.</div>';return}const H=B.map(U=>c.find(re=>re.player.id===U.id)||{_fake:!0,player:U,id:"fake-"+U.id});J(H,U=>U._fake?bn(U.player):zi(U,""),U=>U._fake?ue({player:U.player,id:"x",_fake:!0}):ue(U),U=>{U._fake||vn(U,c,v,t)})}else{const B=Z();if(!B.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const H={};B.forEach(re=>{const D=re.player.id;H[D]||(H[D]=[]),H[D].push(re)});const U=Object.values(H).map(re=>Yo(re));J(U,re=>{const D=v[re.player.id]||1,G=D>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${D}</div>`:"",de=c.filter(he=>he.player.id===re.player.id&&he.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return zi(re,G+de)},re=>ue(re),re=>vn(re,c,v,t))}}function ce($){const B=q?b:[...E];if(!B.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const H=B.map(U=>({formation:U,card:f.find(re=>re.formation===U)||null,owned:E.has(U)}));J(H,({formation:U,card:re,owned:D})=>fe(U,D?re:null,D?f.filter(G=>G.formation===U).length:0),({formation:U,owned:re})=>L(U,re),({card:U,owned:re})=>{re&&U&&Qo(U,f,t,r)},"#1A6B3C",5)}function O($){const B=Object.keys(g),H=q?B.length?B:w:[...m];if(!H.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const U=H.map(re=>({type:re,gc:je[re]||{icon:"⚡",desc:""},def:g[re]||null,owned:m.has(re),card:l.find(D=>D.gc_type===re)||null}));J(U,({type:re,gc:D,def:G,owned:se,card:de})=>{var ke;const he=(G==null?void 0:G.name)||re,Te=se?l.filter(_t=>_t.gc_type===re).length:0,ve=Te>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${Te}</div>`:"",ze=(G==null?void 0:G.effect)||D.desc||"",Se=G!=null&&G.image_url?`/icons/${G.image_url}`:((ke=G==null?void 0:G.club)==null?void 0:ke.logo_url)||(G!=null&&G.country_code?`https://flagsapi.com/${G.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null);let Be=tt(he,Se,D.icon,ze,{width:160,onClick:se});return se||(Be=`<div style="filter:grayscale(1);opacity:0.5">${Be}</div>`),`<div ${se&&de?`data-gc-id="${de.id}" data-gc-type="${re}"`:""} style="position:relative">${ve}${Be}</div>`},({type:re,gc:D,def:G,owned:se})=>{const de=window.innerWidth>=768?.76:.54,he=(G==null?void 0:G.name)||re,Te=(G==null?void 0:G.effect)||D.desc||"",ve=G!=null&&G.image_url?`/icons/${G.image_url}`:null;let ze=tt(he,ve,D.icon,Te,{width:140});return se||(ze=`<div style="filter:grayscale(1);opacity:0.45">${ze}</div>`),`<div style="display:inline-block;zoom:${de};line-height:0;pointer-events:none">${ze}</div>`},({type:re,owned:D,def:G})=>{D&&Jo(re,G,r)},"#7a28b8",5)}function V($){const B="#4FC3F7",H="/";if(!y.length){$.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Stade.<br><small>Ouvre un booster Stade !</small></div>';return}const U={};y.forEach(D=>{const G=D.stadium_id||"?";(U[G]=U[G]||[]).push(D)});const re=Object.entries(U).map(([D,G])=>({sid:D,def:u[D]||null,count:G.length,card:G[0]}));J(re,({def:D,count:G})=>{var ze,Se;const se=(D==null?void 0:D.name)||"?",de=((ze=D==null?void 0:D.club)==null?void 0:ze.encoded_name)||(D==null?void 0:D.country_code)||"—",he=D!=null&&D.image_url?`${H}icons/${D.image_url}`:((Se=D==null?void 0:D.club)==null?void 0:Se.logo_url)||(D!=null&&D.country_code?`https://flagsapi.com/${D.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),Te=G>1?`<div style="position:absolute;top:8px;right:8px;background:#333;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${G}</div>`:"",ve=`${de}<br>+10 ⭐ joueurs alliés`;return`<div style="position:relative">${Te}${ti(se,he,ve,{width:160})}</div>`},({def:D})=>{var ve,ze;const G=window.innerWidth>=768?.76:.54,se=(D==null?void 0:D.name)||"?",de=((ve=D==null?void 0:D.club)==null?void 0:ve.encoded_name)||(D==null?void 0:D.country_code)||"—",he=D!=null&&D.image_url?`${H}icons/${D.image_url}`:((ze=D==null?void 0:D.club)==null?void 0:ze.logo_url)||(D!=null&&D.country_code?`https://flagsapi.com/${D.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),Te=`${de}<br>+10 ⭐`;return`<div style="display:inline-block;zoom:${G};line-height:0;pointer-events:none">${ti(se,he,Te,{width:140})}</div>`},null,B,5)}e.querySelectorAll(".col-tab-btn").forEach($=>{$.addEventListener("click",()=>{C=$.dataset.tab,k="Tous",z="",q=!1,e.querySelectorAll(".col-tab-btn").forEach(B=>{const H=B.dataset.tab===C;B.style.borderBottomColor=H?"var(--green)":"transparent",B.style.color=H?"var(--green)":"var(--tile-fg-dim)"}),I(),Y()})}),I(),Y()}function Jo(e,t,i){const n=je[e]||{icon:"⚡",desc:"Effet spécial."},o=(t==null?void 0:t.name)||e,r=(t==null?void 0:t.effect)||n.desc,a=t!=null&&t.image_url?`/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
      ${tt(o,a,n.icon,r,{width:200})}
      <div style="background:#fff3cd;border-radius:10px;padding:10px 14px;width:100%">
        <div style="font-size:12px;color:#856404">⚠️ Cette carte est à <b>usage unique</b>. Une fois jouée en match, elle est définitivement supprimée de ta collection.</div>
      </div>
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const ci=1e3;function Qo(e,t,i,n){var g,p,u;const{state:o,toast:r,closeModal:a,navigate:s,refreshProfile:d}=i,c=e.formation,l={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function f(){const b=Mt[c]||{},w=Hi[c]||[],v=290,E=360,m=20;function C(z){const q=b[z];return q?{x:q.x*v,y:q.y*E}:null}let k=`<svg width="${v}" height="${E}" viewBox="0 0 ${v} ${E}" xmlns="http://www.w3.org/2000/svg">`;for(const[z,q]of w){const W=C(z),te=C(q);!W||!te||(k+=`<line x1="${W.x}" y1="${W.y}" x2="${te.x}" y2="${te.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const z of Object.keys(b)){const q=C(z);if(!q)continue;const W=z.replace(/\d+/,""),te=l[W]||"#555";k+=`<circle cx="${q.x}" cy="${q.y}" r="${m}" fill="${te}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,k+=`<text x="${q.x}" y="${q.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${W}</text>`}return k+="</svg>",k}const y=t.filter(b=>b.formation===c);y.length;const x=!e.is_for_sale;n(`Formation ${c}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(g=document.getElementById("direct-sell-form-btn"))==null||g.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${c} pour ${ci.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const b=y.find(v=>!v.is_for_sale)||y[0];if(!b){r("Aucune carte à vendre","error");return}await h.from("market_listings").delete().eq("card_id",b.id),await h.from("transfer_history").delete().eq("card_id",b.id);const{error:w}=await h.from("cards").delete().eq("id",b.id);if(w){r(w.message,"error");return}await h.from("users").update({credits:(o.profile.credits||0)+ci}).eq("id",o.profile.id),await d(),r(`+${ci.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),a(),s("collection")}),(p=document.getElementById("market-sell-form-btn"))==null||p.addEventListener("click",async()=>{const b=parseInt(document.getElementById("sell-price-form").value);if(!b||b<1){r("Prix invalide","error");return}await h.from("cards").update({is_for_sale:!0,sale_price:b}).eq("id",e.id),await h.from("market_listings").insert({seller_id:o.profile.id,card_id:e.id,price:b}),r("Carte mise en vente sur le marché !","success"),a(),s("collection")}),(u=document.getElementById("cancel-sell-form-btn"))==null||u.addEventListener("click",async()=>{await h.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await h.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),r("Annonce retirée","success"),a(),s("collection")})}async function vn(e,t,i,n){var ue,fe,L,N,ce;const{state:o,toast:r,openModal:a,closeModal:s,navigate:d,refreshProfile:c}=n,l=e.player,f=t.filter(O=>O.player.id===l.id),y=f.length,x=e.evolution_bonus||0,g=Math.max((Number(l.note_g)||0)+(l.job==="GK"||l.job2==="GK"?x:0),(Number(l.note_d)||0)+(l.job==="DEF"||l.job2==="DEF"?x:0),(Number(l.note_m)||0)+(l.job==="MIL"||l.job2==="MIL"?x:0),(Number(l.note_a)||0)+(l.job==="ATT"||l.job2==="ATT"?x:0)),p=l.rarity||"normal",{data:u}=await h.from("sell_price_configs").select("*").eq("rarity",p).lte("note_min",g).gte("note_max",g).order("note_min",{ascending:!1}).limit(1);((ue=u==null?void 0:u[0])==null?void 0:ue.price)??Vo[p];const b=l.rarity!=="legende";At(l);const w=li(l,l.job)+x,v=l.job2?li(l,l.job2)+(li(l,l.job2)>0?x:0):null;Ii[l.job],l.job2&&Ii[l.job2];const E=Uo[l.rarity]||"#ccc";Wo[l.country_code]||l.country_code;const m=e.evolution_bonus||0,k=w+m,z=v||0,q=z>0?z+m:0,W=f.map(O=>O.id);let te={};if(W.length){const{data:O}=await h.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",W).order("transferred_at",{ascending:!0});(O||[]).forEach(V=>{te[V.card_id]||(te[V.card_id]=[]),te[V.card_id].push(V)})}const Z=W.length?`
    <div style="margin-top:16px;border-top:1px solid var(--tile-border);padding-top:14px">
      ...
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px">
        ${f.filter(O=>O.id!==e.id).map((O,V)=>{const $=te[O.id]||[],B=O.is_for_sale,H=$.length?$[$.length-1]:null,U=O.evolution_bonus||0,re=H?H.source==="booster"?"Booster":H.price?H.price.toLocaleString("fr")+" cr.":"—":"—",D=H?new Date(H.transferred_at).toLocaleDateString("fr",{day:"2-digit",month:"2-digit",year:"numeric"}):"",G=80,se=Math.round(G*657/507),de=Ie({...l,_evolution_bonus:U},{width:G});return`
            <div class="exemplaire-row" data-card-id="${O.id}" data-card-idx="${V}"
              style="position:relative;cursor:${B?"not-allowed":"pointer"};opacity:${B?.55:1};transition:transform .1s">
              <!-- Checkbox cachée -->
              <input type="checkbox" class="expl-check"
                data-id="${O.id}" data-evo="${U}" data-note="${li(l,l.job)}"
                ${B?"disabled":""}
                style="display:none">
              ...
              <div class="expl-mini-card" style="position:relative;width:${G}px;height:${se}px;border-radius:8px;overflow:hidden">
                ...
                ${B?'<div style="position:absolute;top:0;right:0;background:#e67e22;color:#fff;font-size:6px;font-weight:900;padding:2px 4px;border-radius:0 6px 0 4px;z-index:12">VENTE</div>':""}
              </div>
              <!-- Source + date -->
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;background:#f0fdf4;border-radius:10px;padding:10px 14px">
          <div>
            ...
            <div style="font-size:11px;color:#555;margin-top:2px">
              ${(e.evolution_bonus||0)>0?`+${e.evolution_bonus} appliqué${e.evolution_bonus>1?"s":""} · Note : ${k}${l.job2&&z>0?` / ${q}`:""}`:`Note actuelle : ${k}${l.job2&&z>0?` / ${q}`:""}`}
            </div>
          </div>
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",l.note_g],["DEF",l.note_d],["MIL",l.note_m],["ATT",l.note_a]].map(([O,V])=>{const $=Ii[O],B=O==="GK"?"#fff":$,H=O===l.job||O===l.job2,U=(Number(V)||0)+(H&&m>0?m:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
                <div style="width:48px;height:48px;border-radius:8px;background:#111;border:2.5px solid ${$};
                  display:flex;align-items:center;justify-content:center">
                  <span style="font-size:20px;font-weight:900;color:${B};font-family:Arial Black,Arial;line-height:1">${U}</span>
                </div>
                <span style="font-size:10px;font-weight:700;color:${B}">${O}</span>
              </div>`}).join("")}
          </div>
        </div>
        <div>
    ...
    ${Z}
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(fe=document.getElementById("close-detail"))==null||fe.addEventListener("click",s);let M=new Set;const I=l.rarity==="pepite"?1.3:l.rarity==="papyte"?.7:1;function Y(){let O=0;return document.querySelectorAll(".expl-check:checked").forEach(V=>{if(V.dataset.id===e.id)return;const B=Number(V.dataset.evo)||0,H=Number(V.dataset.note)||0;O+=H+B}),Math.round(O*I)}const J=()=>{const O=M.size,V=document.getElementById("sell-action-panel");if(!V)return;V.style.display=O>0?"block":"none",document.getElementById("sell-selected-count").textContent=O;const $=document.getElementById("evolve-btn");if($){const B=Y();$.textContent=B>0?`⬆️ Évoluer (+${B})`:"⬆️ Évoluer"}};document.querySelectorAll(".expl-check").forEach(O=>{O.addEventListener("change",()=>{const V=O.dataset.id;O.checked?M.add(V):M.delete(V);const $=O.closest(".exemplaire-row");if($){const B=$.querySelector(".expl-sel-overlay"),H=$.querySelector(".expl-sel-check");B&&(B.style.display=O.checked?"block":"none"),H&&(H.style.display=O.checked?"flex":"none"),$.style.transform=O.checked?"scale(1.05)":"scale(1)"}J()})}),document.querySelectorAll(".exemplaire-row").forEach(O=>{O.addEventListener("click",V=>{if(V.target.tagName==="INPUT")return;const $=O.querySelector(".expl-check");$&&!$.disabled&&($.checked=!$.checked,$.dispatchEvent(new Event("change")))})}),(L=document.getElementById("evolve-btn"))==null||L.addEventListener("click",async()=>{if(y<=1)return;const O=[...M];if(!O.length)return;if(M.has(e.id)){const G=document.createElement("div");G.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",G.innerHTML=`
        ...
        </div>`,document.body.appendChild(G),G.querySelector("#err-close").addEventListener("click",()=>G.remove()),G.addEventListener("click",se=>{se.target===G&&G.remove()});return}const V=O.filter(G=>G!==e.id);if(!V.length){r("Sélectionne des copies à sacrifier","warning");return}const $=V.reduce((G,se)=>{const de=document.querySelector(`.expl-check[data-id="${se}"]`),he=de&&Number(de.dataset.evo)||0,Te=de&&Number(de.dataset.note)||0;return G+Te+he},0),B=Math.round($*I),H=l.rarity==="pepite"?" (+30% pépite ✨)":l.rarity==="papyte"?" (-30% papyte)":"";if(!await new Promise(G=>{const se=document.createElement("div");se.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",se.innerHTML=`
        <div style="background:var(--tile-bg);border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center;box-shadow:0 16px 48px rgba(0,0,0,0.3)">
          ...
          <div style="background:#f0fdf4;border-radius:10px;padding:12px;margin-bottom:16px;font-size:13px;color:#333">
            ...
            ➕ Brut : <strong>+${$}</strong>${H?`<span style="font-size:11px;color:#888"> ${H}</span>`:""}<br>
            ⬆️ Bonus final : <strong style="color:#1A6B3C">+${B}</strong><br>
            📈 Évolution : <strong>${e.evolution_bonus||0}</strong> → <strong style="color:#1A6B3C">${(e.evolution_bonus||0)+B}</strong><br>
            📊 Note finale : <strong>${w}</strong> → <strong style="color:#1A6B3C">${w+B}</strong>
            ${v&&v>0?`<br>📊 Note 2 finale : <strong>${v}</strong> → <strong style="color:#1A6B3C">${v+B}</strong>`:""}
          </div>
        ...
        </div>`,document.body.appendChild(se),se.querySelector("#sac-cancel").addEventListener("click",()=>{se.remove(),G(!1)}),se.querySelector("#sac-ok").addEventListener("click",()=>{se.remove(),G(!0)}),se.addEventListener("click",de=>{de.target===se&&(se.remove(),G(!1))})}))return;if(V.length){await h.from("market_listings").delete().in("card_id",V),await h.from("deck_cards").delete().in("card_id",V),await h.from("transfer_history").delete().in("card_id",V),await h.from("decks").update({stadium_card_id:null}).in("stadium_card_id",V);const{error:G}=await h.from("cards").delete().in("id",V);if(G){r("Erreur suppression : "+G.message,"error");return}}const re=(e.evolution_bonus||0)+B,{error:D}=await h.from("cards").update({evolution_bonus:re}).eq("id",e.id);if(D){r("Erreur évolution : "+D.message,"error");return}r(`⬆️ ${l.firstname} ${l.surname_real} : note ${w} → ${w+re} (+${B}) !`,"success",4e3),s(),d("collection")}),(N=document.getElementById("market-sell-btn"))==null||N.addEventListener("click",async()=>{var U;const O=[...M];if(!O.length){r("Sélectionne au moins un exemplaire","warning");return}const V=parseInt((U=document.getElementById("sell-market-price"))==null?void 0:U.value);if(!V||V<1){r("Prix invalide","error");return}const{error:$}=await h.from("cards").update({is_for_sale:!0,sale_price:V}).in("id",O);if($){r($.message,"error");return}const B=O.map(re=>({seller_id:o.profile.id,card_id:re,price:V,status:"active"})),{error:H}=await h.from("market_listings").insert(B);H&&console.warn("[Market] insert listings:",H.message),r(`${O.length} carte${O.length>1?"s":""} mise${O.length>1?"s":""} en vente à ${V.toLocaleString("fr")} cr. chacune !`,"success"),s(),d("collection")}),(ce=document.getElementById("cancel-sell-btn"))==null||ce.addEventListener("click",async()=>{await h.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await h.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),r("Annonce retirée","success"),s(),d("collection")})}function hn(e,t=""){return new Promise(i=>{const n=document.createElement("div");n.className="modal-overlay",n.style.zIndex="2100",n.innerHTML=`<div class="modal" style="max-width:360px">
  ...
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
    ...
    <div class="page-body" style="padding-bottom:20px">
  ...
  </div>`;const l=window.innerWidth>=900,f=e.querySelector(".deck-pc-layout"),y=e.querySelector(".deck-mobile-layout");f&&(f.style.display=l?"block":"none"),y&&(y.style.display=l?"none":"block"),er(e,t,o,i),e.querySelectorAll("#builder-back").forEach(u=>u.addEventListener("click",()=>n("decks")));const x=()=>{const{openModal:u,closeModal:b}=i,w=`<div style="display:flex;flex-wrap:wrap;gap:8px;padding:8px">
      ${a.map(v=>`<div data-forma="${v}" style="cursor:pointer;padding:10px 16px;border-radius:8px;background:${v===t.formation?"#1A6B3C":"#f0f0f0"};color:${v===t.formation?"#fff":"#111"};font-weight:900;font-size:16px;border:2px solid ${v===t.formation?"#1A6B3C":"#ddd"}">${v}</div>`).join("")}
    </div>`;u("⚽ Choisir une formation",w),document.querySelectorAll("#modal-body [data-forma]").forEach(v=>{v.addEventListener("click",()=>{t.formation=v.dataset.forma;const E=_n(t.formation),m={};E.forEach(C=>{t.slots[C]&&(m[C]=t.slots[C])}),t.slots=m,b(),wt(e,t,i)})})};e.querySelectorAll("#formation-mobile-btn, #formation-pc-btn").forEach(u=>u.addEventListener("click",x)),e.querySelectorAll("#add-stad-btn-pc, #add-stad-btn").forEach(u=>u.addEventListener("click",()=>tr(t,e,i))),e.querySelectorAll("#save-deck").forEach(u=>u.addEventListener("click",()=>or(t,i))),e.querySelectorAll("[data-remove-sub]").forEach(u=>{u.addEventListener("click",()=>{t.subs=t.subs.filter(b=>b!==u.dataset.removeSub),wt(e,t,i)})}),e.querySelectorAll("#add-sub-btn").forEach(u=>u.addEventListener("click",()=>nr(t,e,i)))}function er(e,t,i,n){var v,E,m;const o=window.innerWidth>=900,r=e.querySelector(o?"#deck-field-pc":"#deck-field-mobile");if(!r)return;const a=(v=t.stadiumCards)==null?void 0:v.find(C=>C.id===t.stadiumCardId),s=a&&((E=t.stadDefMap)==null?void 0:E[a.stadium_id])||null,d=Mt[t.formation]||{},c=Oi(t.formation)||[],l={};for(const C of i){const k=t.slots[C],z=k?t.playerCards.find(q=>q.id===k):null;z!=null&&z.player?l[C]={...z.player,_evolution_bonus:z.evolution_bonus||0,face:z.player.face||null}:l[C]=null}const f=window.innerWidth>=900,y=f?window.innerWidth-280:window.innerWidth-20,x=f?Math.min(y,860):y,g=Math.round(f?x*.82:x*.85),p=f?84:44;let u="";for(const[C,k]of c){const z=d[C],q=d[k];if(!z||!q)continue;const W=z.x*x,te=Math.round(.03*g+z.y*.85*g),Z=q.x*x,M=Math.round(.03*g+q.y*.85*g),I=l[C],Y=l[k],J=Ut(I,Y);J==="#ff3333"||J==="#cc2222"?u+=`<line x1="${W.toFixed(1)}" y1="${te.toFixed(1)}" x2="${Z.toFixed(1)}" y2="${M.toFixed(1)}" stroke="${J}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(u+=`<line x1="${W.toFixed(1)}" y1="${te.toFixed(1)}" x2="${Z.toFixed(1)}" y2="${M.toFixed(1)}" stroke="${J}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,u+=`<line x1="${W.toFixed(1)}" y1="${te.toFixed(1)}" x2="${Z.toFixed(1)}" y2="${M.toFixed(1)}" stroke="${J}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let b="";const w=Math.round(p*657/507);for(const C of i){const k=d[C];if(!k)continue;const z=l[C],q=k.x*x;k.y*g;const W=Math.round(.03*g+k.y*(.85*g)),te=Math.round(q-p/2),Z=Math.round(W-w/2);if(z){const M=C.replace(/\d+/,""),I=s&&(s.club_id&&String(z.club_id)===String(s.club_id)||s.country_code&&z.country_code===s.country_code),Y=Ie({...z,_evolution_bonus:z._evolution_bonus||0},{width:p,showStad:!0,stadDef:s,role:M});I&&((m=s.club)!=null&&m.logo_url||s.image_url),b+=`<div style="position:absolute;left:${te}px;top:${Z}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${C}">
        <div style="position:relative">${Y}</div>
      </div>`}else{const M=C.replace(/\d+/,"");b+=`<div style="position:absolute;left:${te}px;top:${Z}px;width:${p}px;height:${w}px;
        ...
        <span style="font-size:8px;color:rgba(255,255,255,0.3);margin-top:2px">${M}</span>
      </div>`}}r.innerHTML=`
    <div style="position:relative;width:${x}px;height:${g}px;margin:0 auto">
      ...
      ${b}
    </div>`,r.querySelectorAll(".deck-slot-hit").forEach(C=>{C.addEventListener("click",()=>ir(C.dataset.pos,t,e,n))})}function tr(e,t,i){var a;const{openModal:n,closeModal:o}=i,r=e.stadiumCards||[];n("🏟️ Choisir un stade",`<div style="display:flex;flex-wrap:wrap;gap:10px;padding:8px">
    ...
    </div>`),(a=document.getElementById("stad-none"))==null||a.addEventListener("click",()=>{e.stadiumCardId=null,o(),wt(t,e,i)}),document.querySelectorAll(".stad-choice").forEach(s=>{s.addEventListener("click",()=>{e.stadiumCardId=s.dataset.stadId,o(),wt(t,e,i)})})}function ir(e,t,i,n){var g,p,u,b,w;const{openModal:o,closeModal:r}=n,a=e.replace(/\d+/,""),s=(g=t.stadiumCards)==null?void 0:g.find(v=>v.id===t.stadiumCardId),d=s&&((p=t.stadDefMap)==null?void 0:p[s.stadium_id])||null,c=t.slots[e],l=c?t.playerCards.find(v=>v.id===c):null;(u=l==null?void 0:l.player)==null||u.id;const f=new Set;Object.entries(t.slots).forEach(([v,E])=>{var C;if(v===e||!E)return;const m=t.playerCards.find(k=>k.id===E);(C=m==null?void 0:m.player)!=null&&C.id&&f.add(m.player.id)}),t.subs.forEach(v=>{var m;const E=t.playerCards.find(C=>C.id===v);(m=E==null?void 0:E.player)!=null&&m.id&&f.add(E.player.id)});const y=new Set,x=t.playerCards.filter(v=>{const E=v.player;return!(E.job===a||E.job2===a)||f.has(E.id)||y.has(E.id)?!1:(y.add(E.id),!0)});x.sort((v,E)=>{const m=v.evolution_bonus||0,C=E.evolution_bonus||0,k=(a==="GK"?v.player.note_g:a==="DEF"?v.player.note_d:a==="MIL"?v.player.note_m:v.player.note_a)+(a===v.player.job||a===v.player.job2?m:0);return(a==="GK"?E.player.note_g:a==="DEF"?E.player.note_d:a==="MIL"?E.player.note_m:E.player.note_a)+(a===E.player.job||a===E.player.job2?C:0)-k}),o(`Choisir ${a} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
  ...
  `,document.body.appendChild(o);let r=!1;const a=document.getElementById("pack-cut-zone"),s=document.getElementById("pack-blade");let d=!1;const c=I=>I.touches&&I.touches[0]?I.touches[0].clientX:I.clientX;function l(I){r||(d=!0,s.style.opacity="1",f(I))}function f(I){if(!d||r)return;const Y=a.getBoundingClientRect(),J=c(I)-Y.left,ue=Math.max(0,Math.min(1,J/Y.width));s.style.width=ue*Y.width+"px",ue>=.82&&x()}function y(){r||(d=!1,s.style.transition="width .2s ease, opacity .2s ease",s.style.width="0",s.style.opacity="0",setTimeout(()=>{r||(s.style.transition="")},220))}function x(){var Y;if(r)return;r=!0,d=!1,s.style.width="100%",s.style.opacity="1",(Y=document.getElementById("cut-flash"))==null||Y.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const I=document.getElementById("cut-hint");I&&(I.style.opacity="0"),a.classList.add("pack-cut"),setTimeout(()=>{s.style.opacity="0",document.getElementById("pack-phase").style.display="none",b(0)},620)}a.addEventListener("pointerdown",l),window.addEventListener("pointermove",f),window.addEventListener("pointerup",y),a.addEventListener("touchstart",l,{passive:!0}),window.addEventListener("touchmove",f,{passive:!0}),window.addEventListener("touchend",y);let g=0,p=!1;const u=new Set;function b(I){g=I,document.getElementById("reveal-phase").style.display="flex",w(),v(I,0),z()}function w(){const I=document.getElementById("card-dots");I&&(I.innerHTML=e.map((Y,J)=>`<div class="card-dot" data-i="${J}" style="width:8px;height:8px;border-radius:50%;background:${J===g?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),I.querySelectorAll(".card-dot").forEach(Y=>Y.addEventListener("click",()=>m(parseInt(Y.dataset.i)))))}function v(I,Y){var $,B;const J=e[I],ue=document.getElementById("card-counter"),fe=document.getElementById("card-track");ue&&(ue.textContent=`Carte ${I+1} / ${e.length}`);const L=document.getElementById("reveal-btns");L&&(L.style.display=I===e.length-1?"flex":"none");const N=J.card_type==="player"&&(($=J.player)==null?void 0:$.rarity)==="legende",ce=!u.has(I);u.add(I);let O=0;if(J.card_type==="player"&&J.player){const H=J.player,U=H.job||"ATT";O=(Number(U==="GK"?H.note_g:U==="DEF"?H.note_d:U==="MIL"?H.note_m:H.note_a)||0)+(J.evolution_bonus||0)}const V=H=>{fe.innerHTML=`
        <div id="current-card-wrap" style="position:relative;display:flex;flex-direction:column;align-items:center;gap:8px;${N?"filter:drop-shadow(0 0 20px #7a28b8)":""}">
        ...
        </div>`;const U=document.getElementById("current-card-wrap");Y!==0?(U.style.transition="none",U.style.transform=`translateX(${Y>0?100:-100}%)`,requestAnimationFrame(()=>{U.style.transition="transform .28s cubic-bezier(.25,1,.5,1)",U.style.transform="translateX(0)"})):U.animate([{opacity:0,transform:"scale(.7)"},{opacity:1,transform:"scale(1)"}],{duration:300,easing:"cubic-bezier(.34,1.56,.64,1)"}),H||N?W():te(),w()};ce&&(((B=J.player)==null?void 0:B.rarity)==="legende"||O>=18)&&J.card_type==="player"&&J.player?E(J,()=>V(!0)):V(!1)}function E(I,Y){var Te;p=!0;const J=I.player,ue=`https://flagsapi.com/${J.country_code}/flat/64.png`,fe=(Te=J.clubs)==null?void 0:Te.logo_url,L=At(J),N=J.job||"ATT",ce=Number(N==="GK"?J.note_g:N==="DEF"?J.note_d:N==="MIL"?J.note_m:J.note_a)||0,O=I.evolution_bonus||0,V=ce+O,$=V>=18&&V<=20,B=V>=18,H=document.getElementById("walkout-overlay"),U=document.getElementById("walkout-stage");if(!H||!U){p=!1,Y();return}let re=null;$&&(re=Ji("/sounds/Legendary.mp3",.8)),H.style.display="flex";const D=()=>{const ve=U.firstElementChild;ve&&(ve.classList.remove("wo-in"),ve.classList.add("wo-out"))},G=1800,se=400;U.innerHTML=`<img class="wo-in" src="${ue}" style="height:130px;border-radius:10px;box-shadow:0 10px 36px rgba(0,0,0,.6)" onerror="this.style.display='none'">`,navigator.vibrate&&navigator.vibrate(30),setTimeout(D,G),setTimeout(()=>{var ve;U.innerHTML=fe?`<img class="wo-in" src="${fe}" style="max-height:160px;max-width:210px;object-fit:contain">`:`<div class="wo-in" style="font-size:34px;font-weight:900;color:#fff;text-align:center">${((ve=J.clubs)==null?void 0:ve.encoded_name)||"CLUB"}</div>`,navigator.vibrate&&navigator.vibrate(30)},G+se),setTimeout(D,G*2+se),setTimeout(()=>{U.innerHTML=L?`<img class="wo-in" src="${L}" style="height:200px;border-radius:50%;box-shadow:0 0 40px rgba(255,255,255,0.3);object-fit:cover;object-position:top">`:'<div class="wo-in" style="font-size:80px">👤</div>',navigator.vibrate&&navigator.vibrate(30)},(G+se)*2),setTimeout(D,(G+se)*2+G);const he=B?"#FFD700":{GK:"#c0c0c0",DEF:"#e03030",MIL:"#D4A017",ATT:"#3fbf5f"}[N]||"#fff";setTimeout(()=>{U.innerHTML=`<div class="wo-in" style="
        font-size:${B?"120px":"90px"};font-weight:900;color:${he};
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    </div>`,e.querySelectorAll(".gc-select-card").forEach(g=>{g.addEventListener("click",()=>{const p=g.dataset.id,u=r.find(w=>w.id===p);if(!u)return;const b=a.findIndex(w=>w.gc_type===u.gc_type);b>-1?a.splice(b,1):a.length<3&&a.push(u),c()})}),(f=e.querySelector("#gc-launch"))==null||f.addEventListener("click",()=>{l&&d(a)}),(y=e.querySelector("#gc-no-gc"))==null||y.addEventListener("click",()=>d([])),(x=e.querySelector("#gc-reset"))==null||x.addEventListener("click",()=>{a.length&&(a=[],c())})}c()}function hr(e,t){var o;const i=((o=t==null?void 0:t.state)==null?void 0:o.params)||{},n=e||i.matchMode||"vs_ai_easy";return n==="friend"?`Match vs ${i.friendName||"Ami"}`:n==="random"?"Match vs Random":n==="ranked"?"Match Classé":n==="mini_league"||n==="mini-league"?"🏆 Match de Mini League":`Match vs IA — ${n.replace("vs_ai_","").toUpperCase()}`}async function wr(e,t,i){const{state:n,navigate:o}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:r}=await h.from("decks").select(`id,name,is_active,formation,stadium_card_id,
      stadium_card:cards!stadium_card_id(id,stadium_id,
        stadium_def:stadium_definitions(id,name,club_id,country_code,image_url,
          club:clubs(encoded_name,logo_url)))`).eq("owner_id",n.profile.id).order("created_at",{ascending:!1});if(!r||r.length===0){It(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>o("decks"));return}const a=r.map(y=>y.id),{data:s}=await h.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,stadium_id,evolution_bonus,
        player:players(id,firstname,surname_real,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order"),d=[...new Set((s||[]).filter(y=>{var x,g;return((x=y.card)==null?void 0:x.card_type)==="stadium"&&((g=y.card)==null?void 0:g.stadium_id)}).map(y=>y.card.stadium_id))],c={};if(d.length){const{data:y}=await h.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",d);(y||[]).forEach(x=>{c[x.id]=x}),(s||[]).forEach(x=>{var g,p;((g=x.card)==null?void 0:g.card_type)==="stadium"&&((p=x.card)!=null&&p.stadium_id)&&(x.card._stadiumDef=c[x.card.stadium_id]||null)})}let l=0;function f(){var m,C,k,z,q,W,te;const y=r[l],x=(s||[]).filter(Z=>Z.deck_id===y.id),g=x.filter(Z=>{var M;return Z.is_starter&&((M=Z.card)==null?void 0:M.player)}).map(Z=>Di(Z.card,Z.position)),p=x.find(Z=>{var M;return((M=Z.card)==null?void 0:M.card_type)==="formation"}),u=y.formation||((m=p==null?void 0:p.card)==null?void 0:m.formation)||"4-4-2";let b=g.length>=11?xi(g,u):null,w=((C=y.stadium_card)==null?void 0:C.stadium_def)||null;w&&b&&(b=ai(b,w));const v=g.length>=11;st(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff;position:relative">
      ...
      <!-- Pagination -->
      ${r.length>1?`<div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">${r.map((Z,M)=>`<div style="width:6px;height:6px;border-radius:50%;background:${M===l?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}</div>`:""}
      ...
      <div id="deck-bottom-bar" style="flex-shrink:0;padding:10px 14px 14px;display:flex;flex-direction:column;gap:8px;background:rgba(0,0,0,0.2)">
    ...
    </div>`,requestAnimationFrame(()=>requestAnimationFrame(function Z(){const M=e.querySelector(".deck-preview-wrap"),I=e.querySelector("#deck-swipe-zone");if(!M||!I||!b)return;const Y=I.clientWidth>=900,J=Math.max(200,I.clientHeight-(Y?60:40)),fe=Math.max(200,I.clientWidth-16),L=Y?Math.min(117,Math.max(52,Math.round(fe*.22))):Math.max(44,Math.round(fe*.168));if(J<220||fe<220){requestAnimationFrame(Z);return}const N=Y?null:Math.round(L*.55);M.innerHTML=Ot(b,u,null,[],fe,J,[],N),M.style.cssText=`width:${fe}px;height:${J}px;overflow:hidden;margin:${Y?0:60}px auto 0`;const ce=M.querySelector("svg");ce&&(ce.style.cssText="display:block;width:100%;height:100%",ce.setAttribute("preserveAspectRatio",Y?"xMidYMid meet":"none"))})),(z=document.getElementById("prev-deck"))==null||z.addEventListener("click",()=>{l>0&&(l--,f())}),(q=document.getElementById("next-deck"))==null||q.addEventListener("click",()=>{l<r.length-1&&(l++,f())}),(W=document.getElementById("validate-deck"))==null||W.addEventListener("click",()=>{if(!v)return;const Z=t.state.params||{};t.navigate("match",{...Z,matchMode:Z.matchMode||i,deckId:y.id})}),(te=document.getElementById("cancel-deck-select"))==null||te.addEventListener("click",()=>{it(e),o("home")});const E=document.getElementById("deck-swipe-zone");if(E){let Z=0,M=0;E.addEventListener("touchstart",I=>{Z=I.touches[0].clientX,M=I.touches[0].clientY},{passive:!0}),E.addEventListener("touchend",I=>{const Y=I.changedTouches[0].clientX-Z,J=I.changedTouches[0].clientY-M;Math.abs(Y)<40||Math.abs(Y)<Math.abs(J)||(Y<0&&l<r.length-1?(l++,f()):Y>0&&l>0&&(l--,f()))},{passive:!0})}}f()}function ht(e,t=44,i=58,n=null){return Ie(e,{width:t,showStad:!!n,stadDef:n,used:e==null?void 0:e.used})}function Je(e,t,i,n,o){if(!(e!=null&&e.length))return"";const r=e.slice(0,5);let a='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return r.forEach((s,d)=>{const c=s._line||s.job||"MIL";let l=s.boost||0;if(s.stadiumBonus&&(n==="attack"&&(c==="ATT"||c==="MIL")||n==="defense"&&(c==="GK"||c==="DEF"||c==="MIL")?l+=10:n||(l+=10)),a+=Ie(s,{width:40,role:c,extraNote:l}),d<r.length-1){const f=r[d+1],y=Fn(s,f,o)?Ut(s,f):null;a+=`<div style="width:7px;height:3px;background:${!y||y==="#ff3333"||y==="#cc2222"?"rgba(255,255,255,0.12)":y};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(a+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),a+="</div>",a}function _i(e,t,i,n,o=310,r=310,a=[],s=null){const d=Mt[t]||{},c=Oi(t)||Hi[t]||[],l={},f=["ATT","MIL","DEF","GK"];for(const b of f)(e[b]||[]).forEach((v,E)=>{l[`${b}${E+1}`]=v});function y(b){const w=d[b];return w?{x:w.x*o,y:w.y*r}:null}let x="";for(const[b,w]of c){const v=y(b),E=y(w);if(!v||!E)continue;const m=l[b],C=l[w],k=Ut(m,C);k==="#00ff88"||k==="#FFD700"?(x+=`<line x1="${v.x.toFixed(1)}" y1="${v.y.toFixed(1)}" x2="${E.x.toFixed(1)}" y2="${E.y.toFixed(1)}"
        ...
        stroke="${k}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const g=typeof window<"u"&&window.innerWidth>=900?Math.min(Math.max(81,Math.round(o*.225)),117):Math.max(44,Math.round(o*.168)),p=Math.round(g*657/507);for(const[b,w]of Object.entries(l)){const v=y(b);if(!v||!w)continue;const E=b.replace(/[0-9]/g,""),m=a.includes(w.cardId),C=i==="attack"&&(["MIL","ATT"].includes(E)||m)&&!w.used||i==="defense"&&["GK","DEF","MIL"].includes(E)&&!w.used,k=n.includes(w.cardId);let z=w.boost||0,q=!1;w.stadiumBonus&&(i==="attack"&&(E==="ATT"||E==="MIL")||i==="defense"&&(E==="GK"||E==="DEF"||E==="MIL")?(z+=10,q=!0):i||(z+=10,q=!0));const W=Math.round(v.x-g/2),te=Math.round(v.y-p/2);if(w.used){x+=`<image href="${`${typeof import.meta<"u"&&"/"||"/"}icons/carte-dos.png`}" x="${W}" y="${te}" width="${g}" height="${p}" preserveAspectRatio="xMidYMid slice" class="match-used-hit" data-card-id="${w.cardId}" data-role="${E}" style="cursor:pointer"/>`;continue}const Z=Ie({...w,_evolution_bonus:0,stadiumBonus:!1},{width:g,showStad:!1,stadDef:null,role:E,extraNote:z,_cardOffset:30,_forceStadColor:q}),M=k?`position:absolute;top:30px;left:0;width:${g}px;height:${p}px;outline:3px solid #FFD700;outline-offset:2px;border-radius:8px;pointer-events:none;`:"";x+=`<foreignObject x="${W-2}" y="${te-30}" width="${g+8}" height="${p+60}" style="overflow:visible">
      <div xmlns="http://www.w3.org/1999/xhtml" style="position:relative">
        ${Z}
        ${k?`<div style="${M}"></div>`:""}
      </div>
    </foreignObject>`,C&&(x+=`<rect x="${W}" y="${te}" width="${g}" height="${p}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${k?"selected":""}" data-card-id="${w.cardId}" data-role="${E}" style="cursor:pointer"/>`)}const u=s!==null?s:Math.round(Math.max(g*.7,80));return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-u} ${-u} ${o+u*2} ${r+u*2}" width="100%" style="display:block;width:100%;margin:0 auto">
    ${x}
  </svg>`}function Ot(e,t,i,n,o=300,r=300,a=[],s=null){return`<div id="match-terrain-wrap" style="position:relative;padding:0 4px">
    ${_i(e,t,i,n,o,r,a,s)}
  </div>`}async function ki(e,t,i,n){var C;const{state:o,navigate:r,toast:a}=t;st(e);const s=o.params||{};if(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!s.deckId)return wr(e,t,i);const d=s.deckId;let c,l,f,y;try{const k=await Promise.all([h.from("decks").select("formation,name,stadium_card_id").eq("id",d).single(),h.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation, evolution_bonus,
            player:players(id,firstname,surname_real,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
              clubs(encoded_name,logo_url)))`).eq("deck_id",d).order("slot_order")]);c=k[0].data,f=k[0].error,l=k[1].data,y=k[1].error}catch(k){console.error("[Match] Exception chargement deck:",k),It(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>r("home"));return}if(f||y){console.error("[Match] Erreur Supabase:",f||y),It(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>r("home"));return}const x=(l||[]).filter(k=>{var z;return k.is_starter&&((z=k.card)==null?void 0:z.player)}).map(k=>Di(k.card,k.position)),g=(l||[]).filter(k=>{var z;return!k.is_starter&&((z=k.card)==null?void 0:z.player)}).map(k=>Di(k.card,k.position));if(x.length<11){It(e,"⚠️",`Deck incomplet (${x.length}/11).`,"Compléter",()=>r("decks"));return}const p=(l||[]).find(k=>{var z;return((z=k.card)==null?void 0:z.card_type)==="formation"}),u=(c==null?void 0:c.formation)||((C=p==null?void 0:p.card)==null?void 0:C.formation)||"4-4-2",{data:b,error:w}=await h.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",o.profile.id).eq("card_type","game_changer"),{data:v}=await h.from("gc_definitions").select("*").eq("is_active",!0),E=(b||[]).map(k=>({...k,_gcDef:(v==null?void 0:v.find(z=>z.name===k.gc_type||z.id===k.gc_definition_id))||null}));let m=null;if(c!=null&&c.stadium_card_id){const{data:k}=await h.from("cards").select("stadium_id").eq("id",c.stadium_card_id).single();if(k!=null&&k.stadium_id){const{data:z}=await h.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").eq("id",k.stadium_id).single();m=z||null}}n({deckId:d,formation:u,starters:x,subsRaw:g,gcCardsEnriched:E,gcDefs:v||[],stadiumDef:m})}function zt(){return Math.min(window.innerWidth-40,860)}function Rt(){return Math.round(zt()*1.1)}function Wn(e){var o,r;if(!e)return null;const t=e._line||e.job||"MIL",i=t==="GK"?e.note_g||0:t==="DEF"?e.note_d||0:t==="MIL"?e.note_m||0:e.note_a||0,n=e.stadiumBonus?10:0;return{name:e.name,firstname:e.firstname||"",note:i+(e.boost||0)+n,note_g:e.note_g||0,note_d:e.note_d||0,note_m:e.note_m||0,note_a:e.note_a||0,_evolution_bonus:0,stadiumBonus:e.stadiumBonus||!1,boost:e.boost||0,job:e.job,job2:e.job2||null,_line:e._line||e.job,_col:e._col,country_code:e.country_code,club_id:e.club_id,rarity:e.rarity,clubName:e.clubName||((o=e.clubs)==null?void 0:o.encoded_name)||null,clubLogo:e.clubLogo||((r=e.clubs)==null?void 0:r.logo_url)||null,face:e.face||null,portrait:At(e)}}function _r(e,t,i,n="Adversaire"){An("/sounds/match-opening.mp3",.3);const o=zt(),r=Rt();return`
    ...
    </div>`}async function $r(e,t){const{state:i}=t,o=(i.params||{}).matchMode||"vs_ai_easy",r=o.replace("vs_ai_",""),a=o;await ki(e,t,o,async({deckId:s,formation:d,starters:c,subsRaw:l,gcCardsEnriched:f,gcDefs:y,stadiumDef:x})=>{try{let g=xi(c,d);x&&(g=ai(g,x),gi(l,x));const p=await Er(d,r),u=p.lines||p,b=async w=>{try{const v=a==="vs_ai_club"?"club":a,{data:E,error:m}=await h.from("matches").insert({home_id:i.profile.id,away_id:null,mode:v,home_deck_id:s,status:"in_progress"}).select().single();if(m){console.error("[MatchIA] Erreur création match:",m),It(e,"⚠️","Impossible de créer le match ("+m.message+").","Retour",()=>t.navigate("home"));return}const C=p.stadiumDef||null;C&&u&&(ai(u,C),gi(p.subs||[],C));const k={gcDefs:y||[],matchId:E==null?void 0:E.id,mode:a,difficulty:r,formation:d,homeTeam:g,aiTeam:u,homeSubs:l,subsUsed:0,maxSubs:Math.min(l.length,3),aiSubs:p.subs||[],aiSubsUsed:0,aiMaxSubs:Math.min((p.subs||[]).length,3),aiUsedSubIds:[],aiGcCards:p.gcCards||[],aiUsedGc:[],aiStadiumDef:C,homeScore:0,aiScore:0,gcCards:w,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Tr(e,k,t)}catch(v){console.error("[MatchIA] Exception launchMatch:",v),It(e,"⚠️","Erreur au lancement du match : "+v.message,"Retour",()=>t.navigate("home"))}};if(!f.length){b([]);return}wi(e,f,b)}catch(g){console.error("[MatchIA] Exception setup:",g),It(e,"⚠️","Erreur de préparation du match : "+g.message,"Retour",()=>t.navigate("home"))}})}async function Er(e,t){var u;const{data:i}=await h.from("players").select("id,firstname,surname_real,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(80);if(!i||i.length<11)return{lines:Lr(e),subs:[],gcCards:[],stadiumDef:null};const n=Nt[e]||Nt["4-4-2"],o={GK:[],DEF:[],MIL:[],ATT:[]},r=new Set;function a(b,w,v){var E,m;return r.add(b.id),{cardId:"ai-"+b.id+"-"+v,id:b.id,firstname:b.firstname,name:b.surname_real,country_code:b.country_code,club_id:b.club_id,job:b.job,job2:b.job2,note_g:Number(b.note_g)||0,note_d:Number(b.note_d)||0,note_m:Number(b.note_m)||0,note_a:Number(b.note_a)||0,rarity:b.rarity,skin:b.skin,hair:b.hair,hair_length:b.hair_length,clubName:((E=b.clubs)==null?void 0:E.encoded_name)||null,clubLogo:((m=b.clubs)==null?void 0:m.logo_url)||null,boost:0,used:!1,_line:w}}for(const b of["GK","DEF","MIL","ATT"]){const w=i.filter(k=>k.job===b&&!r.has(k.id)),v=i.filter(k=>k.job!==b&&!r.has(k.id)),E=[...w,...v],m=[];for(let k=0;k<n[b];k++){const z=E[k];z&&m.push(a(z,b,k))}const C=xt(m.length);m.forEach((k,z)=>{k._col=C[z]}),o[b]=m}const d=i.filter(b=>!r.has(b.id)).slice(0,5).map((b,w)=>a(b,b.job,100+w)),f=Object.keys(je).sort(()=>Math.random()-.5).slice(0,3).map((b,w)=>{var v,E;return{id:"ai-gc-"+w,gc_type:b,name:((v=je[b])==null?void 0:v.name)||b,icon:((E=je[b])==null?void 0:E.icon)||"⚡"}}),y=Object.values(o).flat(),x={};y.forEach(b=>{b.club_id&&(x[b.club_id]=(x[b.club_id]||0)+1)});const g=(u=Object.entries(x).sort((b,w)=>w[1]-b[1])[0])==null?void 0:u[0];let p=null;if(g){const{data:b}=await h.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",g).single();b&&(p={club_id:b.id,country_code:null,name:b.encoded_name+" Stadium",club:{encoded_name:b.encoded_name,logo_url:b.logo_url}})}return{lines:o,subs:d,gcCards:f,stadiumDef:p}}function Lr(e){const t=Nt[e]||Nt["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},n=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let o=0;for(const r of["GK","DEF","MIL","ATT"]){const a=[];for(let d=0;d<t[r];d++){const c=3+Math.floor(Math.random()*5);a.push({cardId:"fake-"+o,id:"fake-"+o,firstname:"IA",name:n[o%n.length],country_code:"XX",club_id:null,job:r,job2:null,note_g:r==="GK"?c:2,note_d:r==="DEF"?c:2,note_m:r==="MIL"?c:2,note_a:r==="ATT"?c:2,rarity:"normal",boost:0,used:!1,_line:r}),o++}const s=xt(a.length);a.forEach((d,c)=>{d._col=s[c]}),i[r]=a}return i}function Tr(e,t,i){var o;An("/sounds/match-opening.mp3",.3);const n=`<div style="position:relative;width:22px;height:22px;flex-shrink:0;display:flex;align-items:center;justify-content:center">
  ...
  <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:clamp(6px,1.5vh,20px);padding:clamp(8px,1.5vw,24px);background:#0a3d1e">
    ...
    <div id="duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center, rgba(10,61,30,0.4), rgba(10,61,30,0.92))"></div>
          ...
          <button id="start-match-btn" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,0.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">
            ▶ Commencer le match
          </button>`,E.style.transition="opacity .45s ease",E.style.opacity="1",E.style.pointerEvents="auto",(m=document.getElementById("start-match-btn"))==null||m.addEventListener("click",x))},600)},700)},2800)}function _e(e,t,i){var W,te,Z,M,I,Y,J,ue,fe;const n=t.selected.map(L=>L.cardId),o=t.usedSubIds||[],r=t.homeSubs.filter(L=>!o.includes(L.cardId)),s=Object.values(t.homeTeam).flat().filter(L=>L.used).length>0&&r.length>0&&t.subsUsed<t.maxSubs,d=[...t.homeTeam.MIL||[],...t.homeTeam.ATT||[]].filter(L=>!L.used),c=[...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]].filter(L=>!L.used),l=Ht(t.aiTeam),f=d.length===0&&c.length===0&&!l,y=(t.homeTeam.DEF||[]).filter(L=>!L.used),x=(t.homeTeam.GK||[]).filter(L=>!L.used);let g=[];t.phase==="attack"&&d.length===0&&(l?(g=y.map(L=>L.cardId),y.length===0&&(g=g.concat(x.map(L=>L.cardId)))):f&&(g=[...y,...x].map(L=>L.cardId))),t.log[t.log.length-1];const p=t.phase==="ai-attack"||t.phase==="ai-defense",u=t.phase==="attack",b=t.phase==="defense",w=t.phase==="finished",E=(t.homeSubs||[]).filter(L=>!(t.usedSubIds||[]).includes(L.cardId)).length>0&&t.subsUsed<t.maxSubs,m=u&&d.length===0&&g.length===0&&!E,C=t.gcCards.filter(L=>!t.usedGc.includes(L.id)),k=t.boostCard&&!t.boostUsed;e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column",e.innerHTML=`
  <style>
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const N=t.pendingAttack;let ce="";if(t.selected.length>0){const O=t.selected.map($=>({...(t.homeTeam[$._role]||[]).find(H=>H.cardId===$.cardId)||$,_line:$._role})),V=ri(O,t.modifiers.home,t.formation);ce=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
              ...
              <div style="display:flex;justify-content:center">${Je(O.map($=>({...$,used:!1})),"#3a7bd5",V.total,"defense",t.formation)}</div>
            ...
            <div style="display:flex;justify-content:center">${Je((N.players||[]).map(O=>({...O,used:!1})),"#ff6b6b",N.total,"attack",t.formation)}</div>
            ${ce}
          </div>`}if(t.phase==="ai-defense"&&t.pendingAttack){const N=t.pendingAttack;return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #00ff88;text-align:center">
            <div style="font-size:8px;color:#00ff88;letter-spacing:2px;margin-bottom:4px;text-transform:uppercase">⚔️ VOUS ATTAQUEZ</div>
            <div style="display:flex;justify-content:center">${Je((N.players||[]).map(ce=>({...ce,used:!1})),"#00ff88",N.total,"attack",t.formation)}</div>
          </div>`}if(t.phase==="attack"&&t.selected.length>0){const N=t.selected.map(O=>{const V=(t.homeTeam[O._role]||[]).find(B=>B.cardId===O.cardId)||O,$=["GK","DEF"].includes(O._role);return{...V,_line:O._role,...$?{note_a:Math.max(1,Number(V.note_a)||0)}:{}}}),ce=oi(N,t.modifiers.home,t.formation);return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #FFD700;text-align:center">
            ...
            <div style="display:flex;justify-content:center">${Je(N.map(O=>({...O,used:!1})),"#FFD700",ce.total,"attack",t.formation)}</div>
          </div>`}const L=t.log[t.log.length-1];return L?'<div style="padding:2px 4px">'+Xn(L)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})()}
    </div>
    ...
    ${(()=>{const L=window.innerWidth>=700,N=(D,G,se)=>{var Se,Be;const de=(t.gcDefs||[]).find(ke=>ke.name===D.gc_type),he=(de==null?void 0:de.name)||D.gc_type,Te=(de==null?void 0:de.effect)||((Se=je[D.gc_type])==null?void 0:Se.desc)||"",ve=de!=null&&de.image_url?`/icons/${de.image_url}`:null,ze=((Be=je[D.gc_type])==null?void 0:Be.icon)||"⚡";return`<div class="gc-mini" data-gc-id="${D.id}" data-gc-type="${D.gc_type}" style="flex-shrink:0;cursor:pointer">
        ...
        <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
          ${Ot(t.homeTeam,t.formation,t.phase,n,L?1300:zt(),L?600:Rt(),g)}
        </div>
      </div>`;return L?`
  ...
  <div id="match-history-panel">
    ...
    </div>`,(m=f.querySelector("#sub-close"))==null||m.addEventListener("click",()=>f.remove()),(C=f.querySelector("#out-up"))==null||C.addEventListener("click",()=>{s>0&&(s--,y())}),(k=f.querySelector("#out-down"))==null||k.addEventListener("click",()=>{s<r.length-1&&(s++,y())}),(z=f.querySelector("#in-up"))==null||z.addEventListener("click",()=>{c>0&&(c--,y())}),(q=f.querySelector("#in-down"))==null||q.addEventListener("click",()=>{c<a.length-1&&(c++,y())});const E=(te,Z,M,I)=>{const Y=f.querySelector("#"+te);if(!Y)return;let J=0;Y.addEventListener("touchstart",ue=>{J=ue.touches[0].clientY},{passive:!0}),Y.addEventListener("touchend",ue=>{const fe=ue.changedTouches[0].clientY-J;if(Math.abs(fe)<30)return;const L=Z();fe<0&&L<I-1?(M(L+1),y()):fe>0&&L>0&&(M(L-1),y())},{passive:!0})};E("in-panel",()=>c,te=>c=te,a.length),E("out-panel",()=>s,te=>s=te,r.length),(W=f.querySelector("#sub-confirm"))==null||W.addEventListener("click",te=>{if(te.preventDefault(),te.stopPropagation(),l)return;l=!0;const Z=r[s],M=a[c];if(!Z||!M)return;let I=null,Y=-1;for(const[ue,fe]of Object.entries(t.homeTeam)){const L=(fe||[]).findIndex(N=>N.cardId===Z.cardId);if(L!==-1){I=ue,Y=L;break}}if(Y===-1||!I){$t("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),f.remove();return}const J={...M,_line:I,_col:Z._col||0,used:!1,boost:0};t.homeTeam[I].splice(Y,1,J),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(M.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:Z.name,firstname:Z.firstname,note:Pe(Z,I),portrait:At(Z),job:Z.job,country_code:Z.country_code,rarity:Z.rarity,clubName:Z.clubName,clubLogo:Z.clubLogo},inPlayer:{name:M.name,firstname:M.firstname,note:Pe(M,I),portrait:At(M),job:M.job,country_code:M.country_code,rarity:M.rarity,clubName:M.clubName,clubLogo:M.clubLogo},text:`🔄 ${M.firstname} ${M.name} remplace ${Z.firstname} ${Z.name}`}),f.remove(),kr(Z,M,()=>_e(e,t,i))})}document.body.appendChild(f),y()}function Br(e,t,i,n,o){var y,x;const r=(n.gcDefs||[]).find(g=>g.name===t),a=je[t]||{icon:"⚡",desc:"Carte spéciale."},s=(r==null?void 0:r.name)||t,d=(r==null?void 0:r.effect)||a.desc,c=r!=null&&r.image_url?`/icons/${r.image_url}`:null,l=a.icon||"⚡",f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",f.innerHTML=`
    ${tt(s,c,l,d,{width:190})}
    <!-- Boutons -->
    <div style="display:flex;gap:12px;width:190px">
      <button id="gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
      <button id="gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
    </div>`,document.body.appendChild(f),(y=f.querySelector("#gc-back"))==null||y.addEventListener("click",()=>f.remove()),(x=f.querySelector("#gc-use"))==null||x.addEventListener("click",()=>{f.remove(),Dr(e,t,i,n,o)})}function pi(e,t,i,n,o,r){const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let s=[];function d(){var c,l;a.innerHTML=`
    ...
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      ...
      ${_i(e.aiTeam,e.formation,null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.1))}
    </div>`,`<button class="btn btn-primary" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Gr={pepite:{win:2,loss:-1,boosterStart:"min"},papyte:{win:1,loss:-2,boosterStart:"max"}};function Rr(e){const t=e.job||"ATT";return Number(t==="GK"?e.note_g:t==="DEF"?e.note_d:t==="MIL"?e.note_m:e.note_a)||0}async function Nr(e,t){!e||!t||await Promise.all([$n(e,"win"),$n(t,"loss")])}async function $n(e,t){const{data:i}=await h.from("cards").select("id, current_note, player:players(rarity, job, note_g, note_d, note_m, note_a, note_min, note_max)").eq("owner_id",e).eq("card_type","player");if(!(i!=null&&i.length))return;const n=i.filter(o=>{var r,a;return((r=o.player)==null?void 0:r.rarity)==="pepite"||((a=o.player)==null?void 0:a.rarity)==="papyte"});n.length&&await Promise.all(n.map(o=>{const r=Gr[o.player.rarity],a=t==="win"?r.win:r.loss,s=o.player.note_min??1,d=o.player.note_max??10,c=Rr(o.player),l=o.current_note??c,f=Math.min(d,Math.max(s,l+a));return h.from("cards").update({current_note:f}).eq("id",o.id)}))}const Dt=e=>Wn(e);async function Ct(e,t,i,n,o={}){return Zn(e,t,i,n,o.myGC||[],o.gcDefs||[],o.isRanked||!1,o.rankedData||null,o.stadiumDef||null,o.onMatchEnd||null,o.mlLeagueId||null,o.mlMatchId||null)}async function Or(e,t,i){const{data:n}=await h.from("matches").select("home_id,away_id,mode,is_ranked").eq("id",i).single();if(!n){t.toast("Match introuvable","error"),t.navigate("home");return}const o=n.home_id===t.state.user.id;let r=null,a=null;if(n.mode==="mini_league"){const{data:s}=await h.from("mini_league_matches").select("id, league_id").eq("match_id",i).maybeSingle();s&&(r=s.league_id,a=s.id)}return Zn(e,t,i,o,[],[],n.is_ranked||!1,null,null,null,r,a)}async function Zn(e,t,i,n,o=[],r=[],a=!1,s=null,d=null,c=null,l=null,f=null){const{state:y,navigate:x,toast:g}=t,p=n?"p1":"p2",u=n?"p2":"p1",b=(o||[]).map(_=>_.id),w=(o||[]).map(_=>({id:_.id,gc_type:_.gc_type,_gcDef:_._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:v}=await h.from("matches").select("*").eq("id",i).single();if(!v){g("Match introuvable","error"),x("home");return}async function E(){var xe,be;console.log("[PvP] buildGameState match:",{id:v.id,home_deck_id:v.home_deck_id,away_deck_id:v.away_deck_id,mode:v.mode,is_ranked:v.is_ranked});const[{data:_,error:T},{data:A,error:j},{data:S},{data:R}]=await Promise.all([h.rpc("get_deck_for_match",{p_deck_id:v.home_deck_id}),h.rpc("get_deck_for_match",{p_deck_id:v.away_deck_id}),h.from("users").select("id,pseudo,club_name").eq("id",v.home_id).single(),h.from("users").select("id,pseudo,club_name").eq("id",v.away_id).single()]);console.log("[PvP] get_deck_for_match p1:",T==null?void 0:T.message,"p2:",j==null?void 0:j.message,"p1D:",(xe=_==null?void 0:_.starters)==null?void 0:xe.length,"p2D:",(be=A==null?void 0:A.starters)==null?void 0:be.length);const F=oe=>{const Ee=Number(oe.evolution_bonus)||0;return{cardId:oe.card_id,position:oe.position,id:oe.id,firstname:oe.firstname,name:oe.surname_real,country_code:oe.country_code,club_id:oe.club_id,job:oe.job,job2:oe.job2,note_g:(Number(oe.note_g)||0)+(oe.job==="GK"||oe.job2==="GK"&&Number(oe.note_g)>0?Ee:0),note_d:(Number(oe.note_d)||0)+(oe.job==="DEF"||oe.job2==="DEF"&&Number(oe.note_d)>0?Ee:0),note_m:(Number(oe.note_m)||0)+(oe.job==="MIL"||oe.job2==="MIL"&&Number(oe.note_m)>0?Ee:0),note_a:(Number(oe.note_a)||0)+(oe.job==="ATT"||oe.job2==="ATT"&&Number(oe.note_a)>0?Ee:0),evolution_bonus:Ee,rarity:oe.rarity,skin:oe.skin,hair:oe.hair,hair_length:oe.hair_length,face:oe.face||null,clubName:oe.club_encoded_name||null,clubLogo:oe.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},P=((_==null?void 0:_.starters)||[]).map(oe=>F(oe)),K=((A==null?void 0:A.starters)||[]).map(oe=>F(oe)),Q=(_==null?void 0:_.formation)||"4-4-2",ne=(A==null?void 0:A.formation)||"4-4-2";let ie=xi(P,Q),ee=xi(K,ne);const ae=((_==null?void 0:_.subs)||[]).map(oe=>F(oe)),le=((A==null?void 0:A.subs)||[]).map(oe=>F(oe)),ge=(_==null?void 0:_.stadium_def)||(n?d:null),pe=(A==null?void 0:A.stadium_def)||(n?null:d);return ge&&(ie=ai(ie,ge),gi(ae,ge)),pe&&(ee=ai(ee,pe),gi(le,pe)),{p1Team:ie,p2Team:ee,p1Subs:ae,p2Subs:le,p1Formation:Q,p2Formation:ne,p1Name:(S==null?void 0:S.club_name)||(S==null?void 0:S.pseudo)||"Joueur 1",p2Name:(R==null?void 0:R.club_name)||(R==null?void 0:R.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:n?b:[],gc_p2:n?[]:b,gcCardsFull_p1:n?w:[],gcCardsFull_p2:n?[]:w,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:r||[],lastActionAt:new Date().toISOString()}}let m=v.game_state&&Object.keys(v.game_state).length?v.game_state:null;if(console.log("[PvP] init - amIHome:",n,"gameState exists:",!!m,"match.status:",v.status,"home_id:",v.home_id,"away_id:",v.away_id,"myId:",y.profile.id),!m)if(n){m=await E(),console.log("[PvP] home - gameState built, p1Team keys:",Object.keys((m==null?void 0:m.p1Team)||{}));const{data:_}=await h.from("matches").select("game_state").eq("id",i).single();!(_!=null&&_.game_state)||!Object.keys(_.game_state).length?await h.from("matches").update({game_state:m,turn_user_id:v.home_id}).eq("id",i):m=_.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>',console.log("[PvP] away - waiting for home to write game_state...");for(let _=0;_<30&&!m;_++){await new Promise(A=>setTimeout(A,400));const{data:T}=await h.from("matches").select("game_state").eq("id",i).single();T!=null&&T.game_state&&Object.keys(T.game_state).length&&(m=T.game_state),_%5===0&&console.log("[PvP] away - poll",_,"game_state:",!!(T!=null&&T.game_state))}if(!m){g("Erreur de synchronisation","error"),x("home");return}m.gc_p2=b,m.gcCardsFull_p2=w,await h.from("matches").update({game_state:m}).eq("id",i)}let C=!1,k=null,z=!1;const q=new Set,W=new Set;async function te(_){var ne,ie;Tt();try{h.removeChannel(Z)}catch{}const T=m[p+"Score"]||0,A=m[u+"Score"]||0;let j,S;_.winner_id?(j=_.winner_id===y.profile.id,S=!1):_.forfeit?(j=T>A,S=!1):(S=T===A,j=T>A);let R="";if(a&&n)try{const{data:ee}=await h.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",v.home_id).single(),{data:ae}=await h.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",v.away_id).single();if(ee&&ae){const le=j?1:S?.5:0,ge=1-le,pe=ee.placement_matches<10,xe=ae.placement_matches<10,be=computeGlicko2(ee.mmr,ee.mmr_deviation,ee.mmr_volatility,ae.mmr,ae.mmr_deviation,le===1?1:le===0?0:.5,pe),oe=computeGlicko2(ae.mmr,ae.mmr_deviation,ae.mmr_volatility,ee.mmr,ee.mmr_deviation,ge===1?1:ge===0?0:.5,xe);await h.rpc("update_mmr_after_ranked",{p_match_id:i,p_winner_id:S?null:j?v.home_id:v.away_id,p_home_id:v.home_id,p_away_id:v.away_id,p_home_delta:be.delta,p_away_delta:oe.delta,p_home_new_rd:be.newRd,p_away_new_rd:oe.newRd,p_home_new_vol:be.newSigma,p_away_new_vol:oe.newSigma});const Ee=be.delta,Re=be.newMmr,Le=getTier(Re),Ve=Ee>=0?"+":"",Oe=Ee>=0?"#4caf50":"#ff6b6b",nt=getTier(ee.mmr);R=`
            ...
            </div>`}}catch(ee){console.error("[Ranked] MMR update error:",ee)}(ne=document.getElementById("pvp-end-overlay"))==null||ne.remove();const F=document.createElement("div");F.id="pvp-end-overlay",F.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;color:#fff;padding:24px;text-align:center;overflow-y:auto";const P=S?"🤝":j?"🏆":"😞",K=S?"MATCH NUL":j?"VICTOIRE !":"DÉFAITE",Q=S?"#fff":j?"#FFD700":"#ff6b6b";F.innerHTML=`
      ...
      <div style="font-size:18px">${m[p+"Name"]} ${T} – ${A} ${m[u+"Name"]}</div>
      ${_.forfeit?`<div style="font-size:13px;color:rgba(255,255,255,0.5)">${j?"L'adversaire a quitté":"Perdu par forfait"}</div>`:""}
      ${R}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${l?"🏆 Retour à la Mini League":a?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(F),(ie=F.querySelector("#pvp-end-home"))==null||ie.addEventListener("click",()=>{F.remove(),it(e),l?x("mini-league",{openLeagueId:l}):x(a?"ranked":"home")})}const Z=h.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},_=>{var A;const T=_.new;try{if(T.status==="finished"||T.forfeit){if(C)return;C=!0,k&&(clearInterval(k),k=null),Ue(),T.game_state&&(m=T.game_state),te(T);return}if(T.game_state){const j=m;m=T.game_state;const S=m._lastGC;if(S&&S.seq&&!W.has(S.seq)&&(W.add(S.seq),S.by!==p)){$(S.type,S.by,()=>N());return}const R=j[p+"Score"]||0,F=j[u+"Score"]||0,P=m[p+"Score"]||0,K=m[u+"Score"]||0,Q=P>R,ne=K>F;if((Q||ne)&&!q.has(m.round)){q.add(m.round);const ie=[...m.log||[]].reverse().find(le=>le.isGoal),ee=((ie==null?void 0:ie.homePlayers)||[]).map(le=>({name:le.name,note:le.note,portrait:le.portrait,job:le.job}));!!((A=ie==null?void 0:ie.text)!=null&&A.includes("DERNIER CORNER"))?Te().then(()=>{Qe(ee,P,K,Q,()=>N())}):Qe(ee,P,K,Q,()=>N());return}N()}}catch(j){console.error("[PvP] crash:",j)}}).subscribe();async function M(_){Object.assign(m,_),m.lastActionAt=new Date().toISOString();const{error:T}=await h.from("matches").update({game_state:m}).eq("id",i);T&&g("Erreur de synchronisation","error");try{N()}catch(A){console.error("[PvP] renderPvpScreen crash:",A)}}async function I(){if(C)return;C=!0,Tt(),Ue(),k&&(clearInterval(k),k=null);const _=n?v.away_id:v.home_id,T=n?"p2":"p1",A=n?"p1":"p2",j={...m,[T+"Score"]:3,[A+"Score"]:0,phase:"finished"},S=j.p1Score||0,R=j.p2Score||0;try{await h.from("matches").update({game_state:j}).eq("id",i);const{data:F,error:P}=await h.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:_,p_home_score:S,p_away_score:R,p_forfeit:!0});if(P&&console.error("[PvP] finish_pvp_match (forfeit):",P),l)try{await h.rpc("check_and_finish_mini_league",{p_league_id:l})}catch(K){console.error("[PvP] check_and_finish_mini_league (forfeit):",K)}try{const{data:K}=await h.rpc("apply_match_rewards",{p_match_id:i});K!=null&&K.success&&!(K!=null&&K.skipped)&&typeof t.refreshProfile=="function"&&await t.refreshProfile()}catch(K){console.error("[PvP] apply_match_rewards (forfeit):",K)}}catch{}try{h.removeChannel(Z)}catch{}setTimeout(()=>{it(e),l?x("mini-league",{openLeagueId:l}):x("home")},800)}const Y={BOOST_STAT:({value:_=1,count:T=1,roles:A=[]},j,S)=>{const R=j[p+"Team"],F=Object.entries(R).filter(([P])=>!A.length||A.includes(P)).flatMap(([P,K])=>K.filter(Q=>!Q.used).map(Q=>({...Q,_line:P})));if(!F.length){j.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),S(j);return}J(F,T,`Choisir ${T} joueur(s) à booster (+${_})`,P=>{P.forEach(K=>{const Q=(R[K._line]||[]).find(ne=>ne.cardId===K.cardId);Q&&(Q.boost=(Q.boost||0)+_,j.log.push({text:`⚡ +${_} sur ${Q.name}`,type:"info"}))}),j[p+"Team"]=R,S(j)})},DEBUFF_STAT:({value:_=1,count:T=1,roles:A=[],target:j="ai"},S,R)=>{const F=j==="home"?p:u,P=S[F+"Team"],K=j==="home"?"allié":"adverse",Q=Object.entries(P).filter(([ne])=>!A.length||A.includes(ne)).flatMap(([ne,ie])=>ie.map(ee=>({...ee,_line:ne})));if(!Q.length){S.log.push({text:`🎯 Aucun joueur ${K}`,type:"info"}),R(S);return}J(Q,T,`Choisir ${T} joueur(s) ${K}(s) (-${_})`,ne=>{ne.forEach(ie=>{const ee=(P[ie._line]||[]).find(ae=>ae.cardId===ie.cardId);ee&&(ee.boost=(ee.boost||0)-_,S.log.push({text:`🎯 -${_} sur ${ee.name}`,type:"info"}))}),S[F+"Team"]=P,R(S)})},GRAY_PLAYER:({count:_=1,roles:T=[],target:A="ai"},j,S)=>{const R=A==="home"?p:u,F=j[R+"Team"],P=A==="home"?"allié":"adverse",K=Object.entries(F).filter(([Q])=>!T.length||T.includes(Q)).flatMap(([Q,ne])=>ne.filter(ie=>!ie.used).map(ie=>({...ie,_line:Q})));if(!K.length){j.log.push({text:`❌ Aucun joueur ${P}`,type:"info"}),S(j);return}J(K,_,`Choisir ${_} joueur(s) ${P}(s) à exclure`,Q=>{const ne="usedCardIds_"+R,ie=new Set(j[ne]||[]);Q.forEach(ee=>{const ae=(F[ee._line]||[]).find(le=>le.cardId===ee.cardId);ae&&(ae.used=!0,ie.add(ee.cardId),j.log.push({text:`❌ ${ae.name} exclu !`,type:"info"}))}),j[ne]=[...ie],j[R+"Team"]=F,S(j)})},REVIVE_PLAYER:({count:_=1,roles:T=[]},A,j)=>{const S=A[p+"Team"],R=Object.entries(S).filter(([F])=>!T.length||T.includes(F)).flatMap(([F,P])=>P.filter(K=>K.used).map(K=>({...K,_line:F})));if(!R.length){A.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),j(A);return}J(R,_,`Choisir ${_} joueur(s) à ressusciter`,F=>{F.forEach(P=>{const K=(S[P._line]||[]).find(Q=>Q.cardId===P.cardId);K&&(K.used=!1,A.log.push({text:`💫 ${K.name} ressuscité !`,type:"info"}))}),A[p+"Team"]=S,j(A)})},REMOVE_GOAL:({},_,T)=>{const A=u+"Score";_[A]>0?(_[A]--,_.log.push({text:"🚫 Dernier but annulé !",type:"info"})):_.log.push({text:"🚫 Aucun but à annuler",type:"info"}),T(_)},ADD_GOAL_DRAW:({},_,T)=>{_[p+"Score"]===_[u+"Score"]?(_[p+"Score"]++,_.log.push({text:"🎯 But bonus !",type:"info"})):_.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),T(_)},ADD_SUB:({value:_=1},T,A)=>{T.maxSubs=(T.maxSubs||3)+_,T.log.push({text:`🔄 +${_} remplacement(s)`,type:"info"}),A(T)},CUSTOM:({},_,T)=>T(_)};function J(_,T,A,j){const S=document.createElement("div");S.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let R=[];function F(){var K,Q;const P=_.map(ne=>{const ie=ne._line||ne.job||"MIL",ee=R.find(le=>le.cardId===ne.cardId),ae=Ie({...ne,_evolution_bonus:0},{width:90,showStad:!0,role:ie,extraNote:ne.boost||0});return`<div class="pp-item" data-cid="${ne.cardId}"
        ...
        </div>`,(K=S.querySelector("#pp-close"))==null||K.addEventListener("click",()=>S.remove()),S.querySelectorAll(".pp-item").forEach(ne=>{ne.addEventListener("click",()=>{const ie=ne.dataset.cid,ee=_.find(le=>le.cardId===ie),ae=R.findIndex(le=>le.cardId===ie);ee&&(ae>-1?R.splice(ae,1):R.length<T&&R.push(ee),F())})}),(Q=S.querySelector("#pp-confirm"))==null||Q.addEventListener("click",()=>{S.remove(),j(R)})}F(),document.body.appendChild(S)}async function ue(_,T){const j=(m["gcCardsFull_"+p]||[]).find(P=>P.id===_),S=(j==null?void 0:j._gcDef)||(m.gcDefs||[]).find(P=>{var K;return P.name===T||((K=P.name)==null?void 0:K.toLowerCase().trim())===(T==null?void 0:T.toLowerCase().trim())}),R=[...m["usedGc_"+p]||[],_],F={type:T,by:p,seq:(m._gcAnimSeq||0)+1};W.add(F.seq),$(T,p,async()=>{if(S!=null&&S.effect_type&&S.effect_type!=="CUSTOM"){const K=Y[S.effect_type];if(K){const Q={...m};K(S.effect_params||{},Q,async ne=>{ne["usedGc_"+p]=R,ne._lastGC=F,ne._gcAnimSeq=F.seq,await M(ne)});return}}const P={...m};switch(T){case"Remplacement+":P.maxSubs=(P.maxSubs||3)+1,P.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const K=u+"Score";P[K]>0&&(P[K]--,P.log.push({text:"🚫 But annulé",type:"info"}));break}}P["usedGc_"+p]=R,P._lastGC=F,P._gcAnimSeq=F.seq,await M(P)})}function fe(_,T){const A="usedCardIds_"+_,j=new Set(m[A]||[]);T.forEach(S=>j.add(S)),m[A]=[...j]}function L(){for(const _ of["p1","p2"]){const T=new Set(m["usedCardIds_"+_]||[]),A=m[_+"Team"];if(A)for(const j of["GK","DEF","MIL","ATT"])(A[j]||[]).forEach(S=>{S.used=T.has(S.cardId)})}}function N(){var rn,an,sn,dn,ln,cn;if(m.phase==="reveal")return ce();if(m.phase==="midfield")return V();if(m.phase==="finished")return Ge();const _=m[p+"Team"],T=m[u+"Team"];L();const A=m[p+"Score"],j=m[u+"Score"],S=m[p+"Name"],R=m[u+"Name"],F=m.phase===p+"-attack",P=m.phase===p+"-defense",K=Array.isArray(m["selected_"+p])?m["selected_"+p]:[],Q=K.map(X=>X.cardId),ne=window.innerWidth>=700,ie=m[p+"Subs"]||[],ee=m["usedSubIds_"+p]||[],ae=ie.filter(X=>!ee.includes(X.cardId)),le=m["gcCardsFull_"+p]||[],ge=m["usedGc_"+p]||[],pe=le.filter(X=>!ge.includes(X.id)),xe=m.boostOwner===p&&!m.boostUsed,be=[..._.MIL||[],..._.ATT||[]].filter(X=>!X.used),oe=[...T.MIL||[],...T.ATT||[]].filter(X=>!X.used),Ee=!ke(T),Re=be.length===0&&oe.length===0&&!Ee,Le=(_.DEF||[]).filter(X=>!X.used),Ve=(_.GK||[]).filter(X=>!X.used);let Oe=[];F&&be.length===0&&(Ee?(Oe=Le.map(X=>X.cardId),Le.length===0&&(Oe=Oe.concat(Ve.map(X=>X.cardId)))):Re&&(Oe=[...Le,...Ve].map(X=>X.cardId)));function nt(X,me,Ae){var Me,ft;const we=X._gcDef,qt=(we==null?void 0:we.name)||X.gc_type,at=(we==null?void 0:we.effect)||((Me=je[X.gc_type])==null?void 0:Me.desc)||"",Fe=we!=null&&we.image_url?`/icons/${we.image_url}`:null,Ne=((ft=je[X.gc_type])==null?void 0:ft.icon)||"⚡";return`<div class="pvp-gc-mini" data-gc-id="${X.id}" data-gc-type="${X.gc_type}" style="flex-shrink:0;cursor:pointer">
      ...
      </div>`}const De=(X,me)=>me?Kt(95,162):nt(X,95),Ye=(X,me)=>me?Kt(68,116):nt(X,68),He=ne?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",pt=F?ct(p)?`<button id="pvp-action" style="${He};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${K.length===0?"disabled":""}>⚔️ ATTAQUEZ <span id="pvp-timer"></span></button>`:`<button id="pvp-action" data-pass="1" style="${He};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER <span id="pvp-timer"></span></button>`:P?`<button id="pvp-action" style="${He};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${K.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="pvp-timer"></span></button>`:`<div style="font-size:11px;color:rgba(255,255,255,0.3);text-align:center;padding:4px">⏳ Tour de ${R}</div>`,ot=F&&!ct(p)?'<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">Aucun attaquant — passez la main</div>':F||P?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${K.length}/3 sélectionné(s)</div>`:"",jt=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${ne?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${Ot(_,m[p+"Formation"],ut,Q,ne?1300:zt(),ne?600:Rt(),Oe)}
          ...
          ${Ne}
        </div>`}if(F&&((we=m.pendingAttack)!=null&&we.players)){const Fe=m.pendingAttack;return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #00ff88;text-align:center">
          <div style="font-size:8px;color:#00ff88;letter-spacing:2px;margin-bottom:4px;text-transform:uppercase">⚔️ VOUS ATTAQUEZ</div>
      ...
      <div style="display:flex;align-items:center;padding:8px 10px;background:rgba(0,0,0,0.5);gap:6px;flex-shrink:0">
        ...
        <div style="flex:1;display:flex;align-items:center;justify-content:center;gap:8px">
          ...
          <span style="font-size:26px;font-weight:900;color:#FFD700;letter-spacing:2px">${A} – ${j}</span>
      ...
      <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
        ...
        <div id="mobile-action-bar" style="position:absolute;left:0;right:0;bottom:0;z-index:20;background:rgba(0,0,0,0.55);padding:6px 8px 8px;display:flex;flex-direction:column;gap:6px;box-shadow:0 -4px 16px rgba(0,0,0,0.5)">
          <div style="display:flex;gap:6px;overflow-x:auto;align-items:flex-end;min-height:96px;padding-bottom:2px">
            ${pe.map(X=>Ye(X,!1)).join("")}
            ${xe?Ye(null,!0):""}
            <div id="pvp-sub-open" style="cursor:${F&&ae.length>0?"pointer":"default"};flex-shrink:0;box-sizing:border-box;width:68px;height:95px;border-radius:10px;border:2px solid ${F&&ae.length>0?"rgba(255,255,255,0.5)":"rgba(255,255,255,0.15)"};background:${F&&ae.length>0?"rgba(60,60,60,0.9)":"rgba(40,40,40,0.5)"};display:flex;flex-direction:column;align-items:center;justify-content:center;gap:4px;opacity:${F&&ae.length>0?1:.4}">
              <div style="display:flex;gap:6px;align-items:center">
                <div style="text-align:center">
                  <div style="font-size:7px;color:#00ff88;font-weight:700;letter-spacing:1px">IN</div>
      ...
      </div>`;function Bt(){const X=e.querySelector(".match-screen");if(!X)return;const me=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);X.style.height=me+"px",X.style.minHeight=me+"px",X.style.maxHeight=me+"px",X.style.overflow="hidden";const Ae=e.querySelector("#mobile-action-bar"),we=e.querySelector("#mobile-play-area");Ae&&we&&(we.style.paddingBottom=Ae.offsetHeight+"px")}if(Bt(),setTimeout(Bt,120),setTimeout(Bt,400),z||(z=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",Bt),window.visualViewport.addEventListener("scroll",Bt)),window.addEventListener("resize",Bt)),function(){const me=e.querySelector("#match-field .terrain-wrapper svg")||e.querySelector(".terrain-wrapper svg");if(!me)return;const Ae=me.closest("#match-terrain-wrap");Ae&&(Ae.style.cssText="position:relative;width:100%;height:100%;padding:0"),me.removeAttribute("width"),me.removeAttribute("height"),me.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",me.setAttribute("preserveAspectRatio","xMidYMid meet")}(),m._pvpResizeBound||(m._pvpResizeBound=!0,window.addEventListener("resize",()=>{const X=e.querySelector(".terrain-wrapper svg");if(X){const me=X.closest("#match-terrain-wrap");me&&(me.style.cssText="position:relative;width:100%;height:100%;padding:0"),X.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0"}})),e.querySelectorAll(".match-slot-hit").forEach(X=>{X.addEventListener("click",()=>{if(!F&&!P)return;const me=X.dataset.cardId,Ae=X.dataset.role,we=(_[Ae]||[]).find(Ne=>Ne.cardId===me);if(!we||we.used)return;const qt=Oe.includes(me);if(F&&!["MIL","ATT"].includes(Ae)&&!qt)return;Array.isArray(m["selected_"+p])||(m["selected_"+p]=[]);const at=m["selected_"+p],Fe=at.findIndex(Ne=>Ne.cardId===me);Fe>-1?at.splice(Fe,1):at.length<3&&at.push({...we,_role:Ae}),N()})}),e.querySelectorAll(".match-used-hit").forEach(X=>{X.addEventListener("click",()=>U(X.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(X=>{X.addEventListener("click",()=>U())}),(rn=e.querySelector("#pvp-sub-open"))==null||rn.addEventListener("click",()=>U()),e.querySelectorAll(".pvp-gc-mini").forEach(X=>{F?X.addEventListener("click",()=>B(X.dataset.gcId,X.dataset.gcType)):(X.style.opacity="0.35",X.style.cursor="default",X.addEventListener("click",()=>$t("⚡ Les Game Changers ne sont utilisables qu'en attaque","rgba(180,100,0,0.9)")))}),(an=e.querySelector("#pvp-boost-card"))==null||an.addEventListener("click",()=>H()),(sn=e.querySelector("#pvp-action"))==null||sn.addEventListener("click",X=>{F?X.currentTarget.dataset.pass==="1"||!ct(p)?se():ze():P&&Se()}),(dn=e.querySelector("#pvp-quit"))==null||dn.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&I()}),(ln=e.querySelector("#pvp-view-opp"))==null||ln.addEventListener("click",()=>D()),(cn=e.querySelector("#pvp-toggle-history"))==null||cn.addEventListener("click",()=>G()),k&&(clearInterval(k),k=null),Ue(),(F||P)&&!C){let X=30,me=!1;const Ae=()=>document.getElementById("pvp-timer"),we=()=>{Ae()&&(Ae().textContent=X+"s",Ae().style.color=me?"#ff4444":"#fff")};we(),k=setInterval(()=>{X--,X<0?me?(clearInterval(k),k=null,Ue(),F&&!ct(p)?se():I()):(me=!0,X=15,we(),Mn("/sounds/timer-urgent.mp3",.6)):we()},1e3)}}function ce(){st(e),e.innerHTML=`
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    </div>`;const R=(ae,le)=>e.querySelectorAll(ae).forEach((ge,pe)=>{setTimeout(()=>{ge.style.opacity="1",ge.style.transform="translateY(0) scale(1)"},le+pe*90)});R(".duel-card-me",150),R(".duel-card-opp",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((ae,le)=>setTimeout(()=>{ae.style.opacity="1"},le*70)),900),setTimeout(()=>{const ae=e.querySelector("#pvp-vs");ae&&(ae.style.opacity="1",ae.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(le=>le.style.opacity="1")},1250);function F(ae,le,ge){const pe=document.getElementById(ae);if(!pe)return;const xe=performance.now(),be=oe=>{const Ee=Math.min(1,(oe-xe)/ge);pe.textContent=Math.round(le*(1-Math.pow(1-Ee,3))),Ee<1?requestAnimationFrame(be):pe.textContent=le};requestAnimationFrame(be)}setTimeout(()=>{F("pvp-score-me",A,800),F("pvp-score-opp",j,800)},1500);const P=m.p1Team.MIL||[],K=m.p2Team.MIL||[],Q=yt(P)+bt(P),ne=yt(K)+bt(K),ie=Q>=ne?"p1":"p2";let ee=m.boostValue;ee==null&&(ee=Yn(),m.boostValue=ee,m.boostOwner=ie,m.boostUsed=!1),O=!0,setTimeout(()=>{const ae=e.querySelector("#duel-row-"+(S?"me":"opp")),le=e.querySelector("#duel-row-"+(S?"opp":"me")),ge=document.getElementById("pvp-score-me"),pe=document.getElementById("pvp-score-opp"),xe=S?ge:pe,be=S?pe:ge;xe&&(xe.style.fontSize="80px",xe.style.color=S?"#FFD700":"#ff6b6b",xe.style.animation="duelPulse .5s ease"+(S?",duelGlow 1.5s ease infinite .5s":"")),be&&(be.style.opacity="0.25"),setTimeout(()=>{ae&&(ae.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",ae.style.zIndex="5"),setTimeout(()=>{const oe=document.getElementById("duel-shock");oe&&(oe.style.animation="shockwave .5s ease-out forwards"),le&&(le.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var Le;const oe=document.getElementById("pvp-duel-finale");if(!oe)return;const Ee=m.boostOwner===p?'<div style="background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:3px solid #cdeffd;border-radius:18px;padding:20px 34px;text-align:center;animation:boostFlipIn .7s cubic-bezier(.34,1.56,.64,1) both;box-shadow:0 10px 36px rgba(135,206,235,.5)"><div style="font-size:10px;color:rgba(0,0,0,.6);letter-spacing:2px;text-transform:uppercase;margin-bottom:6px;font-weight:700">Carte Boost obtenue</div><div style="font-size:46px;line-height:1">⚡</div><div style="font-size:50px;font-weight:900;color:#063;line-height:1.1">+'+ee+`</div><div style="font-size:10px;color:rgba(0,0,0,.55);margin-top:4px">Applicable sur n'importe quel joueur</div></div>`:"",Re=p==="p1"?'<button id="pvp-start-match" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">▶ Commencer le match</button>':`<div style="font-size:14px;color:rgba(255,255,255,0.5);text-align:center;margin-top:8px;animation:fadeUp .4s ease both">⏳ En attente de l'adversaire...</div>`;oe.innerHTML='<div style="font-size:22px;font-weight:900;color:#fff;text-align:center;animation:fadeUp .4s ease both;text-shadow:0 2px 12px rgba(0,0,0,.5)">'+(S?"⚽ "+m[p+"Name"]+"<br>gagne le milieu et attaque !":"😔 "+m[u+"Name"]+"<br>gagne l'engagement et attaque !")+"</div>"+Ee+Re,oe.style.transition="opacity .45s ease",oe.style.opacity="1",oe.style.pointerEvents="auto",(Le=document.getElementById("pvp-start-match"))==null||Le.addEventListener("click",async()=>{const Ve=ie;await M({phase:Ve+"-attack",attacker:Ve,round:1,boostValue:ee,boostUsed:!1,boostOwner:Ve})})},600)},700)},2800)}function $(_,T,A){var le,ge;const j=(m.gcDefs||[]).find(pe=>{var xe;return pe.name===_||((xe=pe.name)==null?void 0:xe.toLowerCase().trim())===(_==null?void 0:_.toLowerCase().trim())}),S={purple:"#b06ce0",light_blue:"#00d4ef"}[j==null?void 0:j.color]||"#b06ce0",R=(j==null?void 0:j.name)||_,F=(j==null?void 0:j.effect)||((le=je[_])==null?void 0:le.desc)||"",P=j!=null&&j.image_url?`/icons/${j.image_url}`:null,K=((ge=je[_])==null?void 0:ge.icon)||"⚡",ne=T===p?"Vous":m[T+"Name"]||"Adversaire",ie=document.createElement("div");ie.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",ie.innerHTML=`
      <style>
      ...
      <div style="font-size:11px;color:${S};letter-spacing:3px;text-transform:uppercase;font-weight:700;animation:gcLabel .4s ease both">${ne} joue une carte</div>
      <div style="animation:gcFlipIn .7s cubic-bezier(.34,1.56,.64,1) both,gcGlow 1.8s ease infinite .7s">
        ${tt(R,P,K,F,{width:200})}
      </div>
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(ie);let ee=!1;const ae=()=>{ee||(ee=!0,ie.remove(),setTimeout(()=>A&&A(),50))};ie.addEventListener("click",ae),setTimeout(ae,3e3)}function B(_,T){var ne,ie,ee,ae;const j=(m["gcCardsFull_"+p]||[]).find(le=>le.id===_),S=j==null?void 0:j._gcDef,R=(S==null?void 0:S.name)||T,F=(S==null?void 0:S.effect)||((ne=je[T])==null?void 0:ne.desc)||"Carte spéciale.",P=S!=null&&S.image_url?`/icons/${S.image_url}`:null,K=((ie=je[T])==null?void 0:ie.icon)||"⚡",Q=document.createElement("div");Q.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",Q.innerHTML=`
      ${tt(R,P,K,F,{width:190})}
      <div style="display:flex;gap:12px;width:190px">
        <button id="pvp-gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
        <button id="pvp-gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
      </div>`,document.body.appendChild(Q),(ee=Q.querySelector("#pvp-gc-back"))==null||ee.addEventListener("click",()=>Q.remove()),(ae=Q.querySelector("#pvp-gc-use"))==null||ae.addEventListener("click",()=>{Q.remove(),ue(_,T)})}function H(){var j;const _=m[p+"Team"],T=Object.entries(_).flatMap(([S,R])=>(R||[]).filter(F=>!F.used).map(F=>({...F,_line:S})));if(!T.length)return;const A=document.createElement("div");A.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",A.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
      ...
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${T.map(S=>{const R={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[S._line]||"#555",F=Pe(S,S._line)+(S.boost||0);return`<div class="bp-item" data-cid="${S.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${R};overflow:hidden;cursor:pointer">
      ...
      </div>`,document.body.appendChild(A),(j=A.querySelector("#bp-close"))==null||j.addEventListener("click",()=>A.remove()),A.querySelectorAll(".bp-item").forEach(S=>{S.addEventListener("click",async()=>{const R=S.dataset.cid,F=T.find(K=>K.cardId===R);if(!F)return;const P=(_[F._line]||[]).find(K=>K.cardId===R);P&&(P.boost=(P.boost||0)+m.boostValue),A.remove(),await M({[p+"Team"]:_,boostUsed:!0})})})}function U(_=null){var ae,le;if(!(m.phase===p+"-attack")){g("Remplacement uniquement avant votre attaque","warning");return}const A=m[p+"Team"],j=m["usedSubIds_"+p]||[],S=m.maxSubs||3;if(j.length>=S){g(`Maximum ${S} remplacements atteint`,"warning");return}const R=Object.entries(A).flatMap(([ge,pe])=>(pe||[]).filter(xe=>xe.used).map(xe=>({...xe,_line:ge}))),F=(m[p+"Subs"]||[]).filter(ge=>!j.includes(ge.cardId));if(!R.length){g("Aucun joueur utilisé à remplacer","warning");return}if(!F.length){g("Aucun remplaçant disponible","warning");return}let P=Math.max(0,R.findIndex(ge=>ge.cardId===_));const K=((ae=R[P])==null?void 0:ae._line)||((le=R[P])==null?void 0:le.job);let Q=Math.max(0,F.findIndex(ge=>ge.job===K)),ne=!1;const ie=document.createElement("div");ie.id="pvp-sub-overlay",ie.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function ee(){var Re,Le,Ve,Oe,nt,Kt;const ge=R[P],pe=F[Q],xe=Math.min(130,Math.round((window.innerWidth-90)/2)),be=Math.round(xe*1.35),oe=De=>`background:rgba(255,255,255,0.12);border:none;color:${De?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${De?"default":"pointer"};flex-shrink:0`;ie.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
        <div style="flex:1;font-size:15px;font-weight:900;color:#fff">🔄 Remplacement (${j.length}/${S})</div>
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        <div id="pin-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
          ...
          <div>${pe?ht({...pe,used:!1,boost:0},xe,be):"<div>—</div>"}</div>
      ...
      </div>`,(Re=ie.querySelector("#psub-close"))==null||Re.addEventListener("click",()=>ie.remove()),(Le=ie.querySelector("#pout-up"))==null||Le.addEventListener("click",()=>{P>0&&(P--,ee())}),(Ve=ie.querySelector("#pout-down"))==null||Ve.addEventListener("click",()=>{P<R.length-1&&(P++,ee())}),(Oe=ie.querySelector("#pin-up"))==null||Oe.addEventListener("click",()=>{Q>0&&(Q--,ee())}),(nt=ie.querySelector("#pin-down"))==null||nt.addEventListener("click",()=>{Q<F.length-1&&(Q++,ee())});const Ee=(De,Ye,He,pt)=>{const ot=ie.querySelector("#"+De);if(!ot)return;let jt=0;ot.addEventListener("touchstart",ut=>{jt=ut.touches[0].clientY},{passive:!0}),ot.addEventListener("touchend",ut=>{const kt=ut.changedTouches[0].clientY-jt;if(Math.abs(kt)<30)return;const rt=Ye();kt<0&&rt<pt-1?(He(rt+1),ee()):kt>0&&rt>0&&(He(rt-1),ee())},{passive:!0})};Ee("pin-panel",()=>Q,De=>Q=De,F.length),Ee("pout-panel",()=>P,De=>P=De,R.length),(Kt=ie.querySelector("#psub-confirm"))==null||Kt.addEventListener("click",async De=>{if(De.preventDefault(),De.stopPropagation(),ne)return;ne=!0;const Ye=R[P],He=F[Q];if(!Ye||!He)return;const pt=Ye._line,ot=(A[pt]||[]).findIndex(kt=>kt.cardId===Ye.cardId);if(ot===-1){g("Erreur : joueur introuvable","error"),ie.remove();return}const jt={...He,_line:pt,position:Ye.position,used:!1,boost:0};A[pt].splice(ot,1,jt);const ut=[...j,He.cardId];ie.remove(),re(Ye,He,async()=>{await M({[p+"Team"]:A,[u+"Team"]:m[u+"Team"],["usedSubIds_"+p]:ut})})})}document.body.appendChild(ie),ee()}function re(_,T,A){const j={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},S=document.createElement("div");S.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const R=(K,Q,ne)=>`<div style="text-align:center">
      ...
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(_),(T=_.querySelector("#pvp-opp-close"))==null||T.addEventListener("click",()=>_.remove())}function G(){var A;const _=m.log||[],T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column",T.innerHTML=`
      ...
      </div>`,document.body.appendChild(T),(A=T.querySelector("#pvp-hist-close"))==null||A.addEventListener("click",()=>T.remove())}async function se(){if(m.phase!==p+"-attack")return;const _=p==="p1"?"p2":"p1",T=(m.round||0)+1,A=[...m.log||[]];A.push({type:"info",text:`⏭️ ${m[p+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const j=$e(m),S=ct(_),R=j||!S?"finished":_+"-attack";await M({["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},pendingAttack:null,phase:R,attacker:_,round:T,log:A}),R==="finished"&&await qe(m)}function de(_){const T=(_.GK||[]).some(j=>!j.used),A=["DEF","MIL","ATT"].some(j=>(_[j]||[]).some(S=>!S.used));return T&&!A}function he(_){return(_.p1Score||0)!==(_.p2Score||0)?null:de(_.p1Team)&&!ke(_.p2Team)?{side:"p1",gk:(_.p1Team.GK||[]).find(T=>!T.used)}:de(_.p2Team)&&!ke(_.p1Team)?{side:"p2",gk:(_.p2Team.GK||[]).find(T=>!T.used)}:null}async function Te(){const _=document.createElement("div");_.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:950;display:flex;align-items:center;justify-content:center;overflow:hidden;text-align:center;padding:24px",document.body.appendChild(_);const T=(A,j)=>new Promise(S=>{_.innerHTML=`<div style="font-size:32px;font-weight:900;color:${j};letter-spacing:2px;animation:lcFade 1.4s ease both">${A}</div>
      <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(S,1400)});await T("⚽ DERNIER CORNER","#FFD700"),await T("🧤 LE GARDIEN MONTE !","#4fc3f7"),_.remove()}async function ve(_,T){const{side:A,gk:j}=_;if(!j)return;const S=A+"Score",R=(T[S]||0)+1;j.used=!0;const F=[...T.log||[]];F.push({type:"duel",isGoal:!0,homeScored:A===p,homePlayers:A==="p1"?[Dt(j)]:[],aiPlayers:A==="p2"?[Dt(j)]:[],text:`⚽ DERNIER CORNER — Le gardien de ${T[A+"Name"]} marque !`});const P=(T.round||0)+1,K=A==="p1"?"p2":"p1",Q={...T,[S]:R},ne=$e(Q);q.add(P),await Te();const ie=A===p?R:T[p+"Score"]||0,ee=A===p?T[u+"Score"]||0:R;Qe([Dt(j)],ie,ee,A===p,async()=>{await M({[S]:R,log:F,round:P,pendingAttack:null,phase:ne?"finished":K+"-attack",attacker:K,["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}}}),ne&&await qe({...m,[S]:R})})}async function ze(){const _=m[p+"Team"],T=!["GK","DEF","MIL","ATT"].some(R=>(m[u+"Team"][R]||[]).some(F=>!F.used)),A=(m["selected_"+p]||[]).map(R=>{const F=(_[R._role]||[]).find(le=>le.cardId===R.cardId)||R,P=["GK","DEF"].includes(R._role),K=_[R._role]||[],Q=K.findIndex(le=>le.cardId===R.cardId),ne=xt(K.length),ie=Q>=0?ne[Q]:F._col??1,ee=m.stadiumDef||m.homeStadiumDef||null,ae=F.stadiumBonus||ee&&(ee.club_id&&String(F.club_id)===String(ee.club_id)||ee.country_code&&F.country_code===ee.country_code)||!1;return{...F,_line:R._role,_col:ie,stadiumBonus:ae,...P?{note_a:Math.max(1,Number(F.note_a)||0)}:{}}});if(!A.length)return;const j=oi(A,m.modifiers[p]||{},m[p+"Formation"]);fe(p,A.map(R=>R.cardId)),A.forEach(R=>{const F=(_[R._role]||[]).find(P=>P.cardId===R.cardId);F&&(F.used=!0)}),m["selected_"+p]=[],N();const S=[...m.log||[]];if(T){if(A.length===1&&(A[0]._line==="GK"||A[0].job==="GK")&&(m.p1Score||0)===(m.p2Score||0)){await ve({side:p,gk:A[0]},{...m,[p+"Team"]:_,log:S});return}const F=(m[p+"Score"]||0)+1,P=A.map(ee=>Dt(ee));S.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:P,homeTotal:j.total,aiTotal:0});const K=(m.round||0)+1,Q=p==="p1"?"p2":"p1",ne={...m,[p+"Team"]:_,[p+"Score"]:F},ie=$e(ne);q.add(K),Qe(P,F,m[u+"Score"]||0,!0,async()=>{await M({[p+"Team"]:_,[p+"Score"]:F,["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},pendingAttack:null,phase:ie?"finished":Q+"-attack",attacker:Q,round:K,log:S}),ie&&await qe({...m,[p+"Score"]:F})});return}S.push({type:"pending",text:`⚔️ ${m[p+"Name"]} attaque (${j.total})`}),await M({[p+"Team"]:_,[u+"Team"]:m[u+"Team"],pendingAttack:{...j,players:A,side:p},["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},phase:u+"-defense",log:S})}async function Se(){const _=m[p+"Team"],T=(m["selected_"+p]||[]).map(pe=>{const xe=(_[pe._role]||[]).find(Le=>Le.cardId===pe.cardId)||pe,be=_[pe._role]||[],oe=be.findIndex(Le=>Le.cardId===pe.cardId),Ee=xt(be.length),Re=oe>=0?Ee[oe]:xe._col??1;return{...xe,_line:pe._role,_col:Re}}),A=ri(T,m.modifiers[p]||{},m[p+"Formation"]);fe(p,T.map(pe=>pe.cardId)),T.forEach(pe=>{const xe=(_[pe._role]||[]).find(be=>be.cardId===pe.cardId);xe&&(xe.used=!0)}),m["selected_"+p]=[],N();const j=Qi(m.pendingAttack.total,A.total,m.modifiers[p]||{}),S=m.pendingAttack.side,R=j==="attack"||(j==null?void 0:j.goal),F=S==="p1"?"p2":"p1",P=(m.round||0)+1,K=(m.pendingAttack.players||[]).map(pe=>Dt(pe)),Q=[...m.log||[]];Q.push({type:"duel",isGoal:R,homeScored:R&&S===p,text:R?`⚽ BUT de ${m[S+"Name"]} ! (${m.pendingAttack.total} vs ${A.total})`:`✋ Attaque stoppée (${m.pendingAttack.total} vs ${A.total})`,homePlayers:K,aiPlayers:T.map(pe=>Dt(pe)),homeTotal:m.pendingAttack.total,aiTotal:A.total});const ne=R?(m[S+"Score"]||0)+1:m[S+"Score"]||0,ie={...m,[p+"Team"]:_,[S+"Score"]:ne},ee=$e(ie),ae=ee?"finished":F+"-attack",le=!ee&&!R?he(ie):null,ge=async()=>{if(le){await ve(le,ie);return}await M({[p+"Team"]:_,[u+"Team"]:m[u+"Team"],[S+"Score"]:ne,["selected_"+p]:[],modifiers:{...m.modifiers,[p]:{}},pendingAttack:null,phase:ae,attacker:F,round:P,log:Q}),(ae==="finished"||ee)&&await qe({...m,[S+"Score"]:ne})};if(R){const pe=S===p,xe=pe?ne:m[p+"Score"]||0,be=pe?m[u+"Score"]||0:ne;q.add(P),Qe(K,xe,be,pe,ge)}else await ge()}function Be(_){return["MIL","ATT"].some(T=>(_[T]||[]).some(A=>!A.used))}function ke(_){return["GK","DEF","MIL","ATT"].some(T=>(_[T]||[]).some(A=>!A.used))}function _t(_){return ke(_)&&!Be(_)}function ct(_){const T=m[_+"Team"],A=m[(_==="p1"?"p2":"p1")+"Team"];return!!(Be(T)||_t(T)&&!ke(A)||_t(T)&&!Be(A)&&ke(A))}function $e(_){return!ke(_.p1Team)&&!ke(_.p2Team)}function Ce(_){const T=_.p1Score||0,A=_.p2Score||0;return T===A?null:T>A?v.home_id:v.away_id}async function qe(_){Tt(),Ue();try{const T=Ce(_),A=T?v.home_id===T?v.away_id:v.home_id:null,j=_.p1Score||0,S=_.p2Score||0,{data:R,error:F}=await h.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:T,p_home_score:j,p_away_score:S,p_forfeit:!1});if(F&&console.error("[PvP] finish_pvp_match:",F),l)try{await h.rpc("check_and_finish_mini_league",{p_league_id:l})}catch(P){console.error("[PvP] check_and_finish_mini_league:",P)}T&&A&&Nr(T,A).catch(()=>{});try{const{data:P}=await h.rpc("apply_match_rewards",{p_match_id:i});P!=null&&P.success&&!(P!=null&&P.skipped)&&typeof t.refreshProfile=="function"&&await t.refreshProfile()}catch(P){console.error("[PvP] apply_match_rewards:",P)}if(typeof c=="function")try{await c({homeScore:j,awayScore:S,winnerId:T,homeId:v.home_id,awayId:v.away_id,matchId:i})}catch(P){console.error("[PvP] onMatchEnd:",P)}}catch(T){console.error("[PvP] finishMatch:",T)}}function Ge(){var S;const _=m[p+"Score"],T=m[u+"Score"],A=_>T,j=_===T;st(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      <div style="font-size:64px">${A?"🏆":j?"🤝":"😤"}</div>
      <div style="font-size:24px;font-weight:900;color:#fff">${A?"Victoire !":j?"Match nul":"Défaite"}</div>
    ...
    </div>`,(S=document.getElementById("pvp-home"))==null||S.addEventListener("click",()=>{try{h.removeChannel(Z)}catch{}it(e),l?x("mini-league",{openLeagueId:l}):x("home")})}N()}async function Gi(e,t,i=!1){var s,d;const{state:n,navigate:o,toast:r}=t,a=i&&((d=(s=t==null?void 0:t.state)==null?void 0:s.params)==null?void 0:d.rankedData)||null;await ki(e,t,i?"ranked":"random",async({deckId:c,formation:l,starters:f,subsRaw:y,gcCardsEnriched:x,gcDefs:g,stadiumDef:p})=>{st(e);const u=async b=>{await Ur(e,t,c,l,f,y,b||[],g,p,i,a)};if(!(x!=null&&x.length)){await u([]);return}wi(e,x,u)})}async function Hr(e,t,i){return Or(e,t,i)}async function Ur(e,t,i,n,o,r,a=[],s=[],d=null,c=!1,l=null){var te,Z;const{state:f,navigate:y,toast:x}=t,g=(l==null?void 0:l.mmr)??((te=f.profile)==null?void 0:te.mmr)??1e3,p=c?hi(g):null,u=c?"linear-gradient(135deg, #1a0a2e 0%, #2d1060 50%, #1a0a2e 100%)":"linear-gradient(135deg, #0a1a2e 0%, #0d3d1e 100%)",b=c?(p==null?void 0:p.color)||"#D4A017":"#FFD700";let w=!1,v=null,E=null;e.innerHTML=`
    ...
    </div>`;const m=async()=>{w=!0,v&&(v.unsubscribe(),v=null),E&&(clearInterval(E),E=null);try{await h.rpc("cancel_matchmaking",{p_user_id:f.profile.id})}catch(M){console.error("[Matchmaking] cancel error:",M)}};(Z=document.getElementById("cancel-mm"))==null||Z.addEventListener("click",async()=>{await m(),it(e),y("home")});const C=async(M,I)=>{w||(w=!0,v&&(v.unsubscribe(),v=null),E&&(clearInterval(E),E=null),await Ct(e,t,M,I,{myGC:a,gcDefs:s,stadiumDef:d,isRanked:c,rankedData:l,onMatchEnd:c?Kr:null}))},k=f.user.id,{data:z,error:q}=c?await h.rpc("try_matchmake_ranked",{p_user_id:k,p_deck_id:i,p_mmr:g}):await h.rpc("try_matchmake",{p_user_id:k,p_deck_id:i});if(q||!(z!=null&&z.success)){console.error("[Matchmaking] try_matchmake error:",q||z),x("Erreur de matchmaking","error"),it(e),y("home");return}if(z.matched){await C(z.match_id,!1);return}const W=async()=>{if(w)return;const{data:M}=await h.from("matchmaking_queue").select("status, match_id").eq("user_id",k).maybeSingle();(M==null?void 0:M.status)==="matched"&&M.match_id&&await C(M.match_id,!0)};v=h.channel(`mm_${k}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${k}`},M=>{const I=M.new;I.status==="matched"&&I.match_id&&C(I.match_id,!0)}).subscribe(),E=setInterval(W,3e3),setTimeout(()=>{w||m()},12e4)}async function Kr(e,t){const{state:i,toast:n}=t,{winnerId:o,homeId:r,awayId:a,homeScore:s,awayScore:d,matchId:c}=e,l=i.user.id,f=r===l,y=o===l,{data:x}=await h.from("users").select("mmr, mmr_rd, mmr_v").eq("id",l).single(),{data:g}=await h.from("users").select("mmr, mmr_rd, mmr_v").eq("id",f?a:r).single();if(x&&g){const p=co(x.mmr,x.mmr_rd,x.mmr_v,g.mmr,g.mmr_rd,y?1:0);await h.from("users").update({mmr:p.mmr,mmr_rd:p.rd,mmr_v:p.v,mmr_wins:y?h.sql`mmr_wins + 1`:void 0,mmr_losses:y?void 0:h.sql`mmr_losses + 1`}).eq("id",l),n(y?`+MMR ↑ ${Math.round(p.mmr-x.mmr)}`:`-MMR ↓ ${Math.round(x.mmr-p.mmr)}`,y?"success":"error",4e3)}}const eo=Object.freeze(Object.defineProperty({__proto__:null,renderMatchRandom:Gi,resumePvpMatch:Hr},Symbol.toStringTag,{value:"Module"}));async function Vr(e,t){const{state:i,navigate:n,toast:o}=t,r=i.params||{},a=r.friendId||null,s=r.friendName||"Ami",d=!!r.isAccepting;await ki(e,t,"friend",async({deckId:c,formation:l,starters:f,subsRaw:y,gcCardsEnriched:x,gcDefs:g,stadiumDef:p})=>{st(e);const u=async b=>{const w=b||[];d?await Wr(e,t,c,w,g,p):await Yr(e,t,c,w,g,p,a,s)};if(!(x!=null&&x.length)){await u([]);return}wi(e,x,u)})}async function Yr(e,t,i,n,o,r,a,s){var w;const{state:d,navigate:c,toast:l}=t,f=d.user.id,{data:y,error:x}=await h.from("friend_match_invites").insert({inviter_id:f,invitee_id:a,friend_id:a,status:"pending",inviter_deck_id:i,expires_at:new Date(Date.now()+2*60*1e3).toISOString()}).select("id").single();if(x||!y){console.error("[Friend] Erreur création invitation:",x),l("Impossible de créer l'invitation","error"),c("home");return}e.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:10px">
      ${i==="waiting"?Qr(x,b,o):i==="active"?Zr(g,o):ea(u,o,s)}
    </div>
  </div>`,(v=document.getElementById("ml-create-btn"))==null||v.addEventListener("click",()=>ta(e,t)),(E=document.getElementById("ml-refresh-list"))==null||E.addEventListener("click",()=>et(e,t,i)),e.querySelectorAll(".ml-tab").forEach(m=>m.addEventListener("click",()=>et(e,t,m.dataset.tab))),e.querySelectorAll("[data-league-id]").forEach(m=>m.addEventListener("click",()=>lt(e,t,m.dataset.leagueId))),e.querySelectorAll("[data-join]").forEach(m=>m.addEventListener("click",C=>{C.stopPropagation(),no(e,t,m.dataset.join,m.dataset.type)})),e.querySelectorAll("[data-delete]").forEach(m=>m.addEventListener("click",C=>{C.stopPropagation(),Ri(e,t,m.dataset.delete,m.dataset.name,i)})),e.querySelectorAll("[data-claim-league]").forEach(m=>m.addEventListener("click",async C=>{C.stopPropagation(),m.disabled=!0,m.textContent="...";const{data:k,error:z}=await h.rpc("claim_mini_league_prize",{p_league_id:m.dataset.claimLeague,p_user_id:n.profile.id});if(z||!(k!=null&&k.success)){t.toast((k==null?void 0:k.error)||"Erreur lors de la récupération","error"),et(e,t,i);return}if(!k.already_claimed){n.profile.credits=(n.profile.credits||0)+k.prize;const q=document.getElementById("nav-credits");q&&(q.textContent=`💰 ${n.profile.credits.toLocaleString("fr")}`),t.toast(`💰 +${k.prize.toLocaleString("fr")} cr. ajoutés !`,"success")}et(e,t,i)}))}function vi(e,t,i=!1,n=null){var d,c;const o=e.creator_id===t,r=e.pot||0,a=((c=(d=e.mini_league_members)==null?void 0:d[0])==null?void 0:c.count)||0,s=n&&n.amount>0&&!n.claimed;return`<div data-league-id="${e.id}" style="background:${Jt};border:1px solid ${s?"rgba(212,160,23,0.5)":Qt};border-radius:12px;padding:14px 16px;cursor:pointer;margin-bottom:8px">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:${Jt};border:1px solid ${Qt};border-radius:12px;padding:16px">
        ...
        ${f.map(M=>`
          <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid ${Lt}">
            <div style="width:36px;height:36px;border-radius:50%;background:${M.club_color2||to};display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:900;color:${M.club_color1||"#fff"}">${(M.pseudo||"?").slice(0,2).toUpperCase()}</div>
            <div style="flex:1"><div style="font-size:13px;font-weight:700;color:${Xe}">${M.club_name||M.pseudo}</div><div style="font-size:11px;color:${Ze}">@${M.pseudo}</div></div>
      ...
      <div style="background:${Jt};border:1px solid ${Qt};border-radius:12px;padding:16px">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>${y.map((M,I)=>{const Y=g>0&&a.status==="finished"?I===0?Math.floor(g*.7):I===1?Math.floor(g*.2):I===2?Math.floor(g*.1):0:0;return`<tr style="border-bottom:1px solid ${Lt};${M.userId===r?"background:rgba(74,222,128,0.08);":""}">
              <td style="padding:7px 3px 7px 0;font-weight:700;color:${I===0?dt:I<3?"#4ade80":Et}">${["🥇","🥈","🥉"][I]||I+1}</td>
              <td style="padding:7px 3px"><div style="font-weight:700;color:${Xe}">${M.clubName}</div><div style="font-size:10px;color:${Ze}">@${M.pseudo}</div></td>
              <td style="text-align:center;color:${Et}">${M.played}</td><td style="text-align:center;color:${Et}">${M.won}-${M.drawn}-${M.lost}</td>
              <td style="text-align:center;color:${M.goalDiff>=0?"#4ade80":"#ff6b6b"}">${M.goalDiff>=0?"+":""}${M.goalDiff}</td>
              <td style="text-align:center;font-weight:900;font-size:14px;color:${Xe}">${M.points}</td>
              ${g>0&&a.status==="finished"?`<td style="text-align:right;font-weight:700;color:${dt}">${Y?Y.toLocaleString("fr")+" cr.":"—"}</td>`:""}
            </tr>`}).join("")}</tbody>
        </table>
      </div>`:""}
      ...
      <div style="background:${Jt};border:1px solid ${Qt};border-radius:12px;padding:16px">
        ...
        ${Array.from({length:Math.max(0,a.status==="active"?a.current_day-1:a.current_day)},(M,I)=>I+1).reverse().map(M=>{const I=(d||[]).filter(Y=>Y.matchday===M);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:${Ze};margin-bottom:6px">Journée ${M}</div>${I.map(Y=>En(Y,f,r,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
  ...
  </div>`;const v=a.status==="waiting"?"waiting":a.status==="active"?"active":"archived";if((E=document.getElementById("ml-back"))==null||E.addEventListener("click",()=>et(e,t,v)),(m=document.getElementById("ml-refresh"))==null||m.addEventListener("click",async M=>{const I=M.currentTarget;I.style.opacity="0.5",await lt(e,t,i)}),(C=document.getElementById("ml-start-btn"))==null||C.addEventListener("click",()=>ra(e,t,a,f)),(k=document.getElementById("ml-next-day"))==null||k.addEventListener("click",()=>aa(e,t,i)),(z=document.getElementById("ml-join-now"))==null||z.addEventListener("click",()=>no(e,t,i,a.type)),(q=document.getElementById("ml-leave-btn"))==null||q.addEventListener("click",()=>na(e,t,i,p)),(W=document.getElementById("ml-delete-btn"))==null||W.addEventListener("click",()=>Ri(e,t,i,a.name,"waiting")),(te=document.getElementById("ml-delete-now"))==null||te.addEventListener("click",()=>Ri(e,t,i,a.name,v)),(Z=document.getElementById("ml-archive-btn"))==null||Z.addEventListener("click",()=>oa(e,t,i)),e.querySelectorAll("[data-play-match]").forEach(M=>{M.addEventListener("click",()=>{const I=x.find(Y=>Y.id===M.dataset.playMatch);I&&t.navigate("match-mini-league",{mlMatchId:I.id,leagueId:i})})}),a.status==="finished"&&w){const M=y.findIndex(I=>I.userId===r);M>=0&&M<3&&w.prize_amount>0&&!w.prize_claimed&&setTimeout(()=>sa(e,t,a,w,M),400)}}function En(e,t,i,n,o=!1){const r=f=>t.find(y=>y.id===f);if(e.is_bye){const f=r(e.home_id);return`<div style="padding:8px;border-radius:8px;background:rgba(255,255,255,0.03);margin-bottom:6px;font-size:12px;color:${Et};text-align:center">🔵 ${(f==null?void 0:f.club_name)||(f==null?void 0:f.pseudo)||"?"} exempté(e)</div>`}const a=r(e.home_id),s=r(e.away_id),d=e.home_id===i||e.away_id===i,c=d&&e.status==="pending"&&n&&!o,l=e.status==="finished"?`${e.home_score} - ${e.away_score}`:"vs";return`<div style="display:flex;align-items:center;gap:8px;padding:10px;border-radius:8px;background:${d?"rgba(26,107,60,0.16)":"rgba(255,255,255,0.03)"};margin-bottom:6px;border:1px solid ${d?"rgba(74,222,128,0.35)":Lt}">
    ...
    </div>`;let l=!1;const f=async u=>{if(!l){if(l=!0,clearInterval(g),x)try{x.unsubscribe()}catch{}u&&await Ct(e,t,u,o,{mlLeagueId:n,mlMatchId:i,myGC:r||[],gcDefs:a,stadiumDef:s}),d()}};(p=document.getElementById("wait-back"))==null||p.addEventListener("click",()=>{if(l=!0,clearInterval(g),x)try{x.unsubscribe()}catch{}c("mini-league",{openLeagueId:n}),d()});const y=async()=>{if(l)return;const{data:u}=await h.from("mini_league_matches").select("match_id").eq("id",i).single();u!=null&&u.match_id&&f(u.match_id)},x=h.channel(`ml_match_${i}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"mini_league_matches",filter:`id=eq.${i}`},u=>{var b;(b=u.new)!=null&&b.match_id&&f(u.new.match_id)}).subscribe(),g=setInterval(y,3e3);y()})}const pa="/",ua=[{emoji:"⚽",title:"Bienvenue dans Manager Wars !",color:"#1A6B3C",content:`<p>Tu es désormais un <strong>manager de football</strong> virtuel.</p>
    ...
    <p style="margin-top:12px;font-size:13px;color:#888">Tu peux revoir ce tutoriel depuis les paramètres à tout moment.</p>`}];function fa(e,t,i){let n=0;const o=document.createElement("div");o.id="tutorial-overlay",o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.65);z-index:9900;display:flex;align-items:center;justify-content:center;padding:16px";const r=()=>{var l,f,y;const s=t[n],d=n===t.length-1,c=Math.round((n+1)/t.length*100);o.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
  ...
  </div>`;let d="buy";const c=()=>{var p,u,b,w,v,E,m;return{name:(((p=document.getElementById("flt-name"))==null?void 0:p.value)||"").toLowerCase().trim(),club:(((u=document.getElementById("flt-club"))==null?void 0:u.value)||"").toLowerCase().trim(),country:(((b=document.getElementById("flt-country"))==null?void 0:b.value)||"").toLowerCase().trim(),job:((w=document.getElementById("flt-job"))==null?void 0:w.value)||"",rarity:((v=document.getElementById("flt-rarity"))==null?void 0:v.value)||"",note1:parseInt((E=document.getElementById("flt-note1"))==null?void 0:E.value)||0,note2:parseInt((m=document.getElementById("flt-note2"))==null?void 0:m.value)||0}};function l(p){const u=c();return p.filter(b=>{var q,W,te;const w=(q=b.card)==null?void 0:q.player;if(!w)return!1;const v=`${w.firstname} ${w.surname_real}`.toLowerCase(),E=(((W=w.clubs)==null?void 0:W.encoded_name)||"").toLowerCase(),m=(w.country_code||"").toLowerCase(),C=((te=b.card)==null?void 0:te.evolution_bonus)||0,k=Ft(w,w.job,C),z=w.job2?Ft(w,w.job2,C):0;return!(u.name&&!v.includes(u.name)||u.club&&!E.includes(u.club)||u.country&&!m.includes(u.country)||u.job&&w.job!==u.job||u.rarity&&w.rarity!==u.rarity||u.note1&&k<u.note1||u.note2&&z<u.note2)})}function f(p){var z,q,W,te;const u=(z=p.card)==null?void 0:z.player;if(!u)return"";const b=((q=p.card)==null?void 0:q.evolution_bonus)||0,w=Ft(u,u.job,b),v=u.job2?Ft(u,u.job2,b):0,E=(i.profile.credits||0)>=p.price,m=u.country_code?`https://flagsapi.com/${u.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,C=ui[u.job]||"#bbb",k=u.job2?ui[u.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden">
      <!-- Drapeau -->
    ...
    </div>`}function y(p){var z,q,W,te;const u=(z=p.card)==null?void 0:z.player;if(!u)return"";const b=((q=p.card)==null?void 0:q.evolution_bonus)||0,w=Ft(u,u.job,b),v=u.job2?Ft(u,u.job2,b):0,E=p.status==="sold",m=u.country_code?`https://flagsapi.com/${u.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,C=ui[u.job]||"#bbb",k=u.job2?ui[u.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${E?"opacity:0.7":""}">
      ${m?`<img src="${m}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      ${(W=u.clubs)!=null&&W.logo_url?`<img src="${u.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${C};display:flex;align-items:center;justify-content:center">
    ...
    </div>`}function x(){const p=document.getElementById("mkt-content"),u=document.getElementById("mkt-filters");if(p){if(u.style.display=d==="buy"?"flex":"none",d==="buy"){const b=l(a);p.innerHTML=b.length?b.map(f).join(""):'<div style="text-align:center;color:#aaa;padding:40px">Aucune carte trouvée.</div>'}else{const b=s.filter(v=>v.status==="active").sort((v,E)=>new Date(E.listed_at)-new Date(v.listed_at)),w=s.filter(v=>v.status==="sold").sort((v,E)=>new Date(E.sold_at||E.listed_at)-new Date(v.sold_at||v.listed_at));p.innerHTML=(b.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:4px 0 6px;text-transform:uppercase;letter-spacing:1px">🟢 En vente (${b.length})</div>`+b.map(y).join(""):"")+(w.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:12px 0 6px;text-transform:uppercase;letter-spacing:1px">✅ Ventes réussies (${w.length})</div>`+w.map(y).join(""):"")+(!b.length&&!w.length?'<div style="text-align:center;color:#aaa;padding:40px">Aucune vente pour le moment.</div>':"")}p.querySelectorAll("[data-buy]").forEach(b=>b.addEventListener("click",()=>xa(b.dataset.buy,a,e,t))),p.querySelectorAll("[data-cancel]").forEach(b=>b.addEventListener("click",()=>ba(b.dataset.cancel,e,t)))}}e.querySelectorAll(".mkt-tab").forEach(p=>{p.addEventListener("click",()=>{d=p.dataset.tab,e.querySelectorAll(".mkt-tab").forEach(u=>{const b=u===p;u.style.background=b?"var(--green)":"var(--tile-bg)",u.style.color=b?"#fff":"var(--tile-fg-dim)",u.style.borderColor=b?"var(--green)":"var(--tile-border)"}),x()})});let g;["flt-name","flt-club","flt-country","flt-job","flt-rarity","flt-note1","flt-note2"].forEach(p=>{var u;(u=document.getElementById(p))==null||u.addEventListener("input",()=>{clearTimeout(g),g=setTimeout(x,250)})}),x()}async function xa(e,t,i,n){const{state:o,toast:r,refreshProfile:a}=n,s=t.find(l=>l.id===e);if(!s)return;const d=s.price;if((o.profile.credits||0)<d){r("Crédits insuffisants","error");return}ya(s,async()=>{const{error:l}=await h.rpc("buy_market_card",{p_listing_id:e,p_buyer_id:o.profile.id});if(l){r("Erreur achat : "+l.message,"error");return}await a();const f=document.getElementById("nav-credits");f&&(f.textContent=`💰 ${(o.profile.credits||0).toLocaleString("fr")}`),r("✅ Carte achetée !","success"),await nn(i,n)})}function ya(e,t){var a;const i=(a=e.card)==null?void 0:a.player,n=i?`${i.firstname} ${i.surname_real}`:"cette carte",o=document.createElement("div");o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",o.innerHTML=`
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center">
    ...
    </div>`,document.body.appendChild(o);const r=s=>{o.remove(),s&&t()};o.querySelector("#buy-cancel").addEventListener("click",()=>r(!1)),o.querySelector("#buy-ok").addEventListener("click",()=>r(!0)),o.addEventListener("click",s=>{s.target===o&&r(!1)})}async function ba(e,t,i){const{toast:n}=i,{data:o}=await h.from("market_listings").select("card_id").eq("id",e).single();if(await h.from("market_listings").update({status:"cancelled"}).eq("id",e),o!=null&&o.card_id){const{count:r}=await h.from("market_listings").select("id",{count:"exact",head:!0}).eq("card_id",o.card_id).eq("status","active");r||await h.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",o.card_id)}n("Annonce retirée","success"),nn(t,i)}async function va(e,t){var x,g,p,u,b;const{state:i,navigate:n}=t,o=((g=(x=t==null?void 0:t.state)==null?void 0:x.params)==null?void 0:g.tab)||"ranked";e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:var(--tile-fg-dim)">⚽ Chargement...</div>';const[{data:r},{data:a},{data:s}]=await Promise.all([h.from("users").select("id,pseudo,club_name,mmr,rank_tier,ranked_wins,ranked_losses,ranked_draws,placement_matches").gte("placement_matches",1).order("mmr",{ascending:!1}).limit(100),h.rpc("get_mini_league_leaderboard"),h.rpc("get_random_leaderboard")]);let d=o;const c=w=>w<3?["#D4A017","#a0a0a0","#cd7f32"][w]:"var(--green)",l=w=>w<3?"#000":"#fff";function f(){var E,m,C;const w=document.getElementById("rankings-legend");w&&(w.innerHTML=d==="random"?'<div style="font-size:11px;color:var(--tile-fg-dim);text-align:center;margin-bottom:10px">📊 = score de classement fiable (tient compte du nombre de matchs joués, pas seulement du %)</div>':"");const v=document.getElementById("rankings-list");if(v){if(d==="ranked"){const k=r||[];v.innerHTML=k.length>0?k.map((z,q)=>{const W=hi(z.mmr??1e3),te=(z.ranked_wins||0)+(z.ranked_losses||0)+(z.ranked_draws||0),Z=te>0?Math.round((z.ranked_wins||0)/te*100):0,M=z.id===i.profile.id,I=(z.placement_matches||0)<10;return`
          <div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${M?"border:2px solid var(--yellow)":""}">
            ...
            <div style="flex:1;min-width:0">
              ...
              <div style="font-size:11px;color:var(--tile-fg-dim)">${z.club_name} · ${W.label}</div>
            </div>
            <div style="text-align:right;flex-shrink:0">
              <div style="font-size:20px">${I?"❓":W.emoji}</div>
              <div style="font-size:11px;font-weight:700;color:${W.color}">${I?"Placement":W.label}</div>
              ${I?"":`<div style="font-size:10px;color:var(--tile-fg-dim)">${Z}% WR</div>`}
            </div>
          </div>`}).join(""):'<div style="text-align:center;color:var(--tile-fg-dim);padding:40px">Aucun joueur classé.</div>'}else if(d==="mini-league"){const k=a||[];v.innerHTML=k.length>0?k.map((z,q)=>`
      ...
      `).join(""):`<div style="text-align:center;color:var(--tile-fg-dim);padding:40px">Aucun résultat de Mini League pour l'instant.</div>`}else{const k=s||[];v.innerHTML=k.length>0?k.map((z,q)=>`
        <div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${z.user_id===i.profile.id?"border:2px solid var(--yellow)":""}">
          <div style="width:32px;height:32px;border-radius:50%;background:${c(q)};color:${l(q)};display:flex;align-items:center;justify-content:center;font-weight:900;flex-shrink:0;font-size:${q<3?"16":"13"}px">${q<3?["🥇","🥈","🥉"][q]:q+1}</div>
          <div style="flex:1;min-width:0">
            <div style="font-weight:700;color:var(--tile-fg-on-page);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${z.pseudo}</div>
  ...
  </div>`,(k=document.getElementById("ranked-back"))==null||k.addEventListener("click",()=>n("home")),(z=document.getElementById("ranked-leaderboard-btn"))==null||z.addEventListener("click",()=>n("rankings",{tab:"ranked"})),(q=document.getElementById("ranked-play-btn"))==null||q.addEventListener("click",()=>{n("match",{matchMode:"ranked",rankedData:{mmr:d,rd:c,sigma:l,isPlacement:y}})})}async function wa(e,{state:t,navigate:i,toast:n}){const o=t.profile;if(!o)return;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:r}=await h.from("matches").select(`id,home_id,away_id,home_score,away_score,status,mode,winner_id,created_at,played_at,
      home:users!home_id(pseudo,club_name),
      away:users!away_id(pseudo,club_name)`).or(`home_id.eq.${o.id},away_id.eq.${o.id}`).order("created_at",{ascending:!1}).limit(50),a={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},s=(r||[]).filter(l=>l.status==="finished"),d=(r||[]).filter(l=>l.status==="in_progress");function c(l){const f=l.home_id===o.id;f?l.home_score:l.away_score,f?l.away_score:l.home_score;const y=l.winner_id===o.id,x=l.home_score===l.away_score&&l.status==="finished",g=l.status!=="finished"?"…":x?"N":y?"V":"D",p=l.status!=="finished"||x?"#888":y?"#1A6B3C":"#c0392b";let u=a[l.mode]||l.mode;l.away_id===null&&!u.startsWith("IA")&&(u="IA");const w=l.home_id===o.id?l.away:l.home;let v;l.away_id===null?v=u:w?v=`${w.club_name||w.pseudo} (${w.pseudo})`:v="Adversaire";let E="";l.status==="in_progress"&&Date.now()-new Date(l.created_at).getTime()>3600*1e3&&(E=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const m=new Date(l.created_at),C=m.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+m.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),k=l.status==="finished"?`${l.home_score} - ${l.away_score}`:`${l.home_score||0} - ${l.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
  ...
  </div>`}po(fo);const ye={user:null,profile:null,page:"home",params:{}};function Pt(e,t="info",i=3e3){const n=document.getElementById("toast");n&&(n.textContent=e,n.className=`show ${t}`,clearTimeout(n._t),n._t=setTimeout(()=>{n.className=""},i))}function _a(e,t,i=""){document.getElementById("modal-title").textContent=e,document.getElementById("modal-body").innerHTML=t,document.getElementById("modal-footer").innerHTML=i,document.getElementById("modal-overlay").classList.remove("hidden")}function Ni(){document.getElementById("modal-overlay").classList.add("hidden")}async function Zt(){if(!ye.user)return;const{data:e}=await h.from("users").select("*").eq("id",ye.user.id).single();e&&(ye.profile=e)}const oo="mw_theme";function ro(){return localStorage.getItem(oo)||"dark"}function ka(e){var t;localStorage.setItem(oo,e),$a(e),(t=ye.profile)!=null&&t.id&&h.from("users").update({theme:e}).eq("id",ye.profile.id).then(()=>{})}function $a(e){document.documentElement.setAttribute("data-theme",e)}function Gt(e,t={}){ye.page=e,ye.params=t,ao()}async function ao(){var n,o,r,a;const e=document.getElementById("page-content");if(!e)return;document.querySelectorAll(".bottom-nav a").forEach(s=>{s.classList.toggle("active",s.dataset.page===ye.page)});const t=document.getElementById("nav-credits");t&&ye.profile&&(t.textContent=`💰 ${(ye.profile.credits||0).toLocaleString("fr")}`);const i={state:ye,navigate:Gt,toast:Pt,openModal:_a,closeModal:Ni,refreshProfile:Zt};switch(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽</div>',ye.page){case"home":await gn(e,i);break;case"home2":await Fo(e,i);break;case"settings":await Dn(e,i);break;case"collection":await Xo(e,i);break;case"decks":await ji(e,i);break;case"boosters":await lr(e,i);break;case"ranked":await ha(e,i);break;case"match":{const s=ye.params&&ye.params.matchMode||"vs_ai_easy";s==="random"?await Gi(e,i,!1):s==="ranked"?await Gi(e,i,!0):s==="friend"?await Vr(e,i,(n=ye.params)==null?void 0:n.friendId,(o=ye.params)==null?void 0:o.friendName):s==="mini_league"||s==="mini-league"?await Ln(e,i,(r=ye.params)==null?void 0:r.mlMatchId,(a=ye.params)==null?void 0:a.leagueId):await $r(e,i);break}case"market":await ga(e,i);break;case"rankings":await va(e,i);break;case"matches":await wa(e,i);break;case"friends":await ho(e,i);break;case"mini-league":await Jr(e,i);break;case"match-mini-league":{const s=ye.params||{};await Ln(e,i,s.mlMatchId,s.leagueId);break}default:await gn(e,i)}}function Ea(){var n,o;const e=document.getElementById("app"),t=ye.profile;if(!t)return;const i="/icons/";e.innerHTML=`
    <nav class="top-nav">
      <div class="logo" id="nav-logo" title="Manager Wars v2026.07.23-1239" style="cursor:pointer">
  ...
  </div>`,(n=e.querySelector("#pl-play-btn"))==null||n.addEventListener("click",t)}async function Ia(){document.documentElement.setAttribute("data-theme",ro()),document.getElementById("modal-overlay").addEventListener("click",i=>{i.target===i.currentTarget&&Ni()}),document.getElementById("modal-close").addEventListener("click",Ni);const{data:{session:e}}=await h.auth.getSession();if(!e){Tn(),Ta(document.getElementById("app"),{onPlay:()=>pn(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:Pt})});return}ye.user=e.user,await Zt(),Tn();try{const{data:i}=await h.from("formation_links_overrides").select("formation, links"),n={};(i||[]).forEach(o=>{n[o.formation]=o.links}),uo(n)}catch(i){console.warn("Impossible de charger les overrides de formation:",i)}if(!ye.profile){go(document.getElementById("app"),{state:ye,navigate:async()=>{await Zt(),Ai()},toast:Pt,refreshProfile:Zt});return}const t=Array.isArray(ye.profile.pending_boosters)?ye.profile.pending_boosters:[];if(!ye.profile.onboarding_done&&t.length>0){yr(document.getElementById("app"),{state:ye,navigate:()=>Ai(),toast:Pt,refreshProfile:Zt});return}Ai(),setTimeout(()=>ma(ye.profile,Gt,Pt),800),h.auth.onAuthStateChange(async(i,n)=>{i==="SIGNED_OUT"&&(ye.user=null,ye.profile=null,document.getElementById("app").innerHTML="",pn(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:Pt}))})}function za(){return Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight)}function $i(){const e=document.getElementById("app");e&&(e.style.height=za()+"px")}window.addEventListener("resize",$i);window.addEventListener("orientationchange",()=>setTimeout($i,150));window.visualViewport&&window.visualViewport.addEventListener("resize",$i);function Ai(){const e=()=>{var i;(i=ye.user)!=null&&i.id&&h.from("users").update({last_seen_at:new Date().toISOString()}).eq("id",ye.user.id).then(()=>{})};e(),window._presencePingInterval&&clearInterval(window._presencePingInterval),window._presencePingInterval=setInterval(e,6e4);const t=document.getElementById("app");t.style.display="flex",t.style.flexDirection="column",$i(),Ea(),ao()}function Tn(){const e=document.getElementById("app-loader"),t=document.getElementById("app");t&&(t.style.display=""),e&&(e.classList.add("zoom-out"),setTimeout(()=>e.style.display="none",500))}function so(e){var n;const t=document.getElementById("app-loader");if(t&&(t.style.display="none"),document.getElementById("boot-error"))return;const i=document.createElement("div");i.id="boot-error",i.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:99999;gap:16px;color:#fff;padding:24px;text-align:center",i.innerHTML=`
