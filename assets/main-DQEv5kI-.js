...
import{s as w,F as si,r as Me,j as Mt,l as Gi,m as bt,h as di,n as mn,o as xn,T as Yt,k as yn,p as bn,b as hn}from"./formation-links-TDtFd05S.js";const vn="/";function Li(e,{navigate:t,toast:i}){let n="login";const a=()=>{var r,s,d,l,c,u;const o=n==="login";e.innerHTML=`
  ...
  `;function s(){var f;const l=document.getElementById("logo-preview"),c=document.getElementById("logo-initials"),u=((f=document.getElementById("setup-club"))==null?void 0:f.value)||"MW",g=u.trim().split(" ").filter(Boolean),m=g.length>=2?(g[0][0]+g[1][0]).toUpperCase():u.slice(0,2).toUpperCase();l&&(l.style.background=r,l.style.borderColor=o),c&&(c.textContent=m,c.style.color=o)}document.getElementById("color1").addEventListener("input",l=>{o=l.target.value,document.getElementById("swatch1").style.background=o,s()}),document.getElementById("color2").addEventListener("input",l=>{r=l.target.value,document.getElementById("swatch2").style.background=r,s()});function d(l){document.querySelectorAll(".setup-step").forEach(c=>c.classList.remove("active")),document.getElementById(`step-${l}`).classList.add("active"),document.getElementById("step-num").textContent=l,document.getElementById("progress-fill").style.width=`${Math.round(l/3*100)}%`,l===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const l=document.getElementById("setup-pseudo").value.trim(),c=document.getElementById("step1-error");if(c.textContent="",l.length<3){c.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:u}=await w.from("users").select("id").eq("pseudo",l).maybeSingle();if(u){c.textContent="Ce pseudo est déjà pris.";return}d(2)}),document.getElementById("step2-back").addEventListener("click",()=>d(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const l=document.getElementById("setup-club").value.trim(),c=document.getElementById("step2-error");if(c.textContent="",l.length<2){c.textContent="Nom trop court (min. 2 caractères).";return}const{data:u}=await w.from("users").select("id").eq("club_name",l).maybeSingle();if(u){c.textContent="Ce nom de club est déjà pris.";return}d(3)}),document.getElementById("step3-back").addEventListener("click",()=>d(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const l=document.getElementById("setup-pseudo").value.trim(),c=document.getElementById("setup-club").value.trim(),u=document.getElementById("step3-error"),g=document.getElementById("step3-finish");u.textContent="",g.disabled=!0,g.textContent="Création en cours…";try{const{error:m}=await w.from("users").insert({id:t.user.id,pseudo:l,club_name:c,club_color1:o,club_color2:r,credits:1e4});if(m)throw m;await _n(t.user.id),await a(),n(`Bienvenue ${l} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(m){u.textContent=m.message,g.disabled=!1,g.textContent="🚀 Créer mon profil !"}})}async function _n(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await w.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const kn="modulepreload",$n=function(e){return"/"+e},Ti={},Ri=function(t,i,n){let a=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const r=document.querySelector("meta[property=csp-nonce]"),s=(r==null?void 0:r.nonce)||(r==null?void 0:r.getAttribute("nonce"));a=Promise.allSettled(i.map(d=>{if(d=$n(d),d in Ti)return;Ti[d]=!0;const l=d.endsWith(".css"),c=l?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${c}`))return;const u=document.createElement("link");if(u.rel=l?"stylesheet":kn,l||(u.as="script"),u.crossOrigin="",u.href=d,s&&u.setAttribute("nonce",s),document.head.appendChild(u),l)return new Promise((g,m)=>{u.addEventListener("load",g),u.addEventListener("error",()=>m(new Error(`Unable to preload CSS for ${d}`)))})}))}function o(r){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=r,window.dispatchEvent(s),!s.defaultPrevented)throw r}return a.then(r=>{for(const s of r||[])s.status==="rejected"&&o(s.reason);return t().catch(o)})},Lt="#1A6B3C",Tt="#cc2222",En="#D4A017",zi="#888";async function Ln(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    </style>`}const Mn="2026.07.15-2312";async function Ii(e,{state:t,navigate:i,toast:n}){var o,r;const a=t.profile;a&&(e.innerHTML=`
  <style>
    .home-dark {
      min-height: 100%;
  ...
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const jt=1e3;function Vn(e,t,i,n){var f,p,x;const{state:a,toast:o,closeModal:r,navigate:s,refreshProfile:d}=i,l=e.formation,c={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function u(){const b=Mt[l]||{},h=si[l]||[],v=290,$=360,y=20;function C(P){const N=b[P];return N?{x:N.x*v,y:N.y*$}:null}let E=`<svg width="${v}" height="${$}" viewBox="0 0 ${v} ${$}" xmlns="http://www.w3.org/2000/svg">`;for(const[P,N]of h){const te=C(P),ne=C(N);!te||!ne||(E+=`<line x1="${te.x}" y1="${te.y}" x2="${ne.x}" y2="${ne.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const P of Object.keys(b)){const N=C(P);if(!N)continue;const te=P.replace(/\d+/,""),ne=c[te]||"#555";E+=`<circle cx="${N.x}" cy="${N.y}" r="${y}" fill="${ne}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,E+=`<text x="${N.x}" y="${N.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${te}</text>`}return E+="</svg>",E}const g=t.filter(b=>b.formation===l);g.length;const m=!e.is_for_sale;n(`Formation ${l}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(pe=document.getElementById("close-detail"))==null||pe.addEventListener("click",s);let k=new Set;const F=()=>{const K=k.size,Q=document.getElementById("sell-action-panel");if(!Q)return;Q.style.display=K>0?"block":"none",document.getElementById("sell-selected-count").textContent=K;const V=document.getElementById("evolve-btn");V&&(V.textContent=`⬆️ Évoluer ${K>1?"(+"+K+")":""}`)};document.querySelectorAll(".expl-check").forEach(K=>{K.addEventListener("change",()=>{const Q=K.dataset.id;K.checked?k.add(Q):k.delete(Q);const V=K.closest(".exemplaire-row");V&&(V.style.borderColor=K.checked?"#1A6B3C":"#eee"),F()})}),document.querySelectorAll(".exemplaire-row").forEach(K=>{K.addEventListener("click",Q=>{if(Q.target.closest("button")||Q.target.tagName==="INPUT")return;const V=K.querySelector(".expl-check");V&&!V.disabled&&(V.checked=!V.checked,V.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(K=>{K.addEventListener("click",Q=>{Q.stopPropagation();const V=document.querySelector(`.expl-hist[data-hist="${K.dataset.idx}"]`);V&&(V.style.display=V.style.display==="none"?"flex":"none")})}),(le=document.getElementById("evolve-btn"))==null||le.addEventListener("click",async()=>{if(g<=1)return;const K=[...k];if(!K.length)return;if(k.has(e.id)){const U=document.createElement("div");U.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",U.innerHTML=`
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const F=t.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
            ...
            ${pt((F.players||[]).map(de=>({...de,used:!1})),"#ff6b6b",F.total)}
            ...
            ${pt((F.players||[]).map(de=>({...de,used:!1})),"#00ff88",F.total)}
  ...
  <div id="match-history-panel">
    ...
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      ...
      ${Rt(e.aiTeam,e.formation,null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.1))}
            ...
            </div>`}}catch(J){console.error("[Ranked] MMR update error:",J)}(X=document.getElementById("pvp-end-overlay"))==null||X.remove();const B=document.createElement("div");B.id="pvp-end-overlay",B.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;color:#fff;padding:24px;text-align:center;overflow-y:auto";const G=z?"🤝":S?"🏆":"😞",Y=z?"MATCH NUL":S?"VICTOIRE !":"DÉFAITE",W=z?"#fff":S?"#FFD700":"#ff6b6b";B.innerHTML=`
      ...
      ${D}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${r?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(B),(re=B.querySelector("#pvp-end-home"))==null||re.addEventListener("click",()=>{B.remove(),Ue(e),m(r?"ranked":"home")})}const ee=w.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},_=>{const L=_.new;try{if(L.status==="finished"||L.forfeit){if(C)return;C=!0,E&&(clearInterval(E),E=null),L.game_state&&(y=L.game_state),ne(L);return}if(L.game_state){const M=y;y=L.game_state;const S=y._lastGC;if(S&&S.seq&&!te.has(S.seq)&&(te.add(S.seq),S.by!==p)){V(S.type,S.by,()=>ae());return}const z=M[p+"Score"]||0,D=M[x+"Score"]||0,B=y[p+"Score"]||0,G=y[x+"Score"]||0,Y=B>z,W=G>D;if((Y||W)&&!N.has(y.round)){N.add(y.round);const X=[...y.log||[]].reverse().find(J=>J.isGoal),re=((X==null?void 0:X.homePlayers)||[]).map(J=>({name:J.name,note:J.note,portrait:J.portrait,job:J.job}));at(re,B,G,Y,()=>ae());return}ae()}}catch(M){console.error("[PvP] crash:",M)}}).subscribe();async function q(_){Object.assign(y,_),y.lastActionAt=new Date().toISOString();const{error:L}=await w.from("matches").update({game_state:y}).eq("id",i);L&&f("Erreur de synchronisation","error");try{ae()}catch(M){console.error("[PvP] renderPvpScreen crash:",M)}}async function I(){if(C)return;C=!0,E&&(clearInterval(E),E=null);const _=n?v.away_id:v.home_id,L=n?"p2":"p1",M=n?"p1":"p2",S={...y,[L+"Score"]:3,[M+"Score"]:0,phase:"finished"};try{await w.from("matches").update({status:"finished",forfeit:!0,winner_id:_,home_score:S.p1Score||0,away_score:S.p2Score||0,game_state:S}).eq("id",i)}catch{}try{w.removeChannel(ee)}catch{}setTimeout(()=>{Ue(e),m("home")},800)}const k={BOOST_STAT:({value:_=1,count:L=1,roles:M=[]},S,z)=>{const D=S[p+"Team"],B=Object.entries(D).filter(([G])=>!M.length||M.includes(G)).flatMap(([G,Y])=>Y.filter(W=>!W.used).map(W=>({...W,_line:G})));if(!B.length){S.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),z(S);return}F(B,L,`Choisir ${L} joueur(s) à booster (+${_})`,G=>{G.forEach(Y=>{const W=(D[Y._line]||[]).find(X=>X.cardId===Y.cardId);W&&(W.boost=(W.boost||0)+_,S.log.push({text:`⚡ +${_} sur ${W.name}`,type:"info"}))}),S[p+"Team"]=D,z(S)})},DEBUFF_STAT:({value:_=1,count:L=1,roles:M=[],target:S="ai"},z,D)=>{const B=S==="home"?p:x,G=z[B+"Team"],Y=S==="home"?"allié":"adverse",W=Object.entries(G).filter(([X])=>!M.length||M.includes(X)).flatMap(([X,re])=>re.map(J=>({...J,_line:X})));if(!W.length){z.log.push({text:`🎯 Aucun joueur ${Y}`,type:"info"}),D(z);return}F(W,L,`Choisir ${L} joueur(s) ${Y}(s) (-${_})`,X=>{X.forEach(re=>{const J=(G[re._line]||[]).find(ce=>ce.cardId===re.cardId);J&&(J.boost=(J.boost||0)-_,z.log.push({text:`🎯 -${_} sur ${J.name}`,type:"info"}))}),z[B+"Team"]=G,D(z)})},GRAY_PLAYER:({count:_=1,roles:L=[],target:M="ai"},S,z)=>{const D=M==="home"?p:x,B=S[D+"Team"],G=M==="home"?"allié":"adverse",Y=Object.entries(B).filter(([W])=>!L.length||L.includes(W)).flatMap(([W,X])=>X.filter(re=>!re.used).map(re=>({...re,_line:W})));if(!Y.length){S.log.push({text:`❌ Aucun joueur ${G}`,type:"info"}),z(S);return}F(Y,_,`Choisir ${_} joueur(s) ${G}(s) à exclure`,W=>{const X="usedCardIds_"+D,re=new Set(S[X]||[]);W.forEach(J=>{const ce=(B[J._line]||[]).find(se=>se.cardId===J.cardId);ce&&(ce.used=!0,re.add(J.cardId),S.log.push({text:`❌ ${ce.name} exclu !`,type:"info"}))}),S[X]=[...re],S[D+"Team"]=B,z(S)})},REVIVE_PLAYER:({count:_=1,roles:L=[]},M,S)=>{const z=M[p+"Team"],D=Object.entries(z).filter(([B])=>!L.length||L.includes(B)).flatMap(([B,G])=>G.filter(Y=>Y.used).map(Y=>({...Y,_line:B})));if(!D.length){M.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),S(M);return}F(D,_,`Choisir ${_} joueur(s) à ressusciter`,B=>{B.forEach(G=>{const Y=(z[G._line]||[]).find(W=>W.cardId===G.cardId);Y&&(Y.used=!1,M.log.push({text:`💫 ${Y.name} ressuscité !`,type:"info"}))}),M[p+"Team"]=z,S(M)})},REMOVE_GOAL:({},_,L)=>{const M=x+"Score";_[M]>0?(_[M]--,_.log.push({text:"🚫 Dernier but annulé !",type:"info"})):_.log.push({text:"🚫 Aucun but à annuler",type:"info"}),L(_)},ADD_GOAL_DRAW:({},_,L)=>{_[p+"Score"]===_[x+"Score"]?(_[p+"Score"]++,_.log.push({text:"🎯 But bonus !",type:"info"})):_.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),L(_)},ADD_SUB:({value:_=1},L,M)=>{L.maxSubs=(L.maxSubs||3)+_,L.log.push({text:`🔄 +${_} remplacement(s)`,type:"info"}),M(L)},CUSTOM:({},_,L)=>L(_)};function F(_,L,M,S){const z=document.createElement("div");z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let D=[];function B(){var Y,W;const G=_.map(X=>{const re={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[X._line]||"#555",J=Ce(X,X._line)+(X.boost||0),se=D.find(ue=>ue.cardId===X.cardId)?"#FFD700":"rgba(255,255,255,0.25)",oe=X.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${X.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${se};background:${re};overflow:hidden;cursor:pointer;${oe}">
        ...
        </div>`,(Y=z.querySelector("#pp-close"))==null||Y.addEventListener("click",()=>z.remove()),z.querySelectorAll(".pp-item").forEach(X=>{X.addEventListener("click",()=>{const re=X.dataset.cid,J=_.find(se=>se.cardId===re),ce=D.findIndex(se=>se.cardId===re);J&&(ce>-1?D.splice(ce,1):D.length<L&&D.push(J),B())})}),(W=z.querySelector("#pp-confirm"))==null||W.addEventListener("click",()=>{z.remove(),S(D)})}B(),document.body.appendChild(z)}async function de(_,L){const S=(y["gcCardsFull_"+p]||[]).find(G=>G.id===_),z=(S==null?void 0:S._gcDef)||(y.gcDefs||[]).find(G=>{var Y;return G.name===L||((Y=G.name)==null?void 0:Y.toLowerCase().trim())===(L==null?void 0:L.toLowerCase().trim())}),D=[...y["usedGc_"+p]||[],_],B={type:L,by:p,seq:(y._gcAnimSeq||0)+1};te.add(B.seq),V(L,p,async()=>{if(z!=null&&z.effect_type&&z.effect_type!=="CUSTOM"){const Y=k[z.effect_type];if(Y){const W={...y};Y(z.effect_params||{},W,async X=>{X["usedGc_"+p]=D,X._lastGC=B,X._gcAnimSeq=B.seq,await q(X)});return}}const G={...y};switch(L){case"Remplacement+":G.maxSubs=(G.maxSubs||3)+1,G.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const Y=x+"Score";G[Y]>0&&(G[Y]--,G.log.push({text:"🚫 But annulé",type:"info"}));break}}G["usedGc_"+p]=D,G._lastGC=B,G._gcAnimSeq=B.seq,await q(G)})}function pe(_,L){const M="usedCardIds_"+_,S=new Set(y[M]||[]);L.forEach(z=>S.add(z)),y[M]=[...S]}function le(){for(const _ of["p1","p2"]){const L=new Set(y["usedCardIds_"+_]||[]),M=y[_+"Team"];if(M)for(const S of["GK","DEF","MIL","ATT"])(M[S]||[]).forEach(z=>{z.used=L.has(z.cardId)})}}function ae(){var Ve,it,yi,bi,hi,vi;if(y.phase==="reveal")return ke();if(y.phase==="midfield")return Q();if(y.phase==="finished")return Fe();const _=y[p+"Team"],L=y[x+"Team"];le();const M=y[p+"Score"],S=y[x+"Score"],z=y[p+"Name"],D=y[x+"Name"],B=y.phase===p+"-attack",G=y.phase===p+"-defense",Y=Array.isArray(y["selected_"+p])?y["selected_"+p]:[],W=Y.map(ie=>ie.cardId),X=window.innerWidth>=700,re=y[p+"Subs"]||[],J=y["usedSubIds_"+p]||[],ce=re.filter(ie=>!J.includes(ie.cardId)),se=y["gcCardsFull_"+p]||[],oe=y["usedGc_"+p]||[],ue=se.filter(ie=>!oe.includes(ie.id)),fe=y.boostOwner===p&&!y.boostUsed,xe=!["GK","DEF","MIL","ATT"].some(ie=>(L[ie]||[]).some(ge=>!ge.used)),Z=[..._.MIL||[],..._.ATT||[]].filter(ie=>!ie.used),be=B&&xe&&Z.length===0?[..._.GK||[],..._.DEF||[]].filter(ie=>!ie.used).map(ie=>ie.cardId):[];function ze(ie,ge,Ie){var $i,Ei;const ye=ie._gcDef,nt={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ye==null?void 0:ye.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",dt={purple:"#b06ce0",light_blue:"#00d4ef"}[ye==null?void 0:ye.color]||"#b06ce0",vt=(ye==null?void 0:ye.name)||ie.gc_type,wt=(ye==null?void 0:ye.effect)||(($i=Ae[ie.gc_type])==null?void 0:$i.desc)||"",wi=ye!=null&&ye.image_url?`/icons/${ye.image_url}`:null,fn=((Ei=Ae[ie.gc_type])==null?void 0:Ei.icon)||"⚡",_i=Math.round(Ie*.22),ki=Math.round(Ie*.22),Vt=Ie-_i-ki,gn=vt.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${ie.id}" data-gc-type="${ie.gc_type}"
      ...
      </div>`}const Ne=(ie,ge)=>ge?Se(130,175):ze(ie,130,175),Pe=(ie,ge)=>ge?Se(68,95):ze(ie,68,95),Ge=X?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",ht=B?$e(p)?`<button id="pvp-action" style="${Ge};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${Y.length===0?"disabled":""}>⚔️ ATTAQUEZ <span id="pvp-timer"></span></button>`:`<button id="pvp-action" data-pass="1" style="${Ge};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER <span id="pvp-timer"></span></button>`:G?`<button id="pvp-action" style="${Ge};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${Y.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="pvp-timer"></span></button>`:`<div style="font-size:11px;color:rgba(255,255,255,0.3);text-align:center;padding:4px">⏳ Tour de ${D}</div>`,qe=B&&!$e(p)?'<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">Aucun attaquant — passez la main</div>':B||G?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${Y.length}/3 sélectionné(s)</div>`:"",Oe=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${X?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
      ${ce.length===0?'<div style="font-size:7px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':ce.map(ie=>`<div class="pvp-sub-btn" data-sub-id="${ie.cardId}" style="cursor:pointer;flex-shrink:0">${Xe(ie,X?76:44,X?100:58)}</div>`).join("")}
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ...
        </div>`}const ie=(y.log||[]).slice(-1)[0];return ie?'<div style="padding:2px 4px">'+nn(ie)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})(),tt=`
      <div style="display:flex;align-items:center;padding:8px 10px;background:rgba(0,0,0,0.5);gap:6px;flex-shrink:0">
      ...
      </div>`;function De(){const ie=e.querySelector(".match-screen");if(!ie)return;const ge=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);ie.style.height=ge+"px",ie.style.minHeight=ge+"px",ie.style.maxHeight=ge+"px",ie.style.overflow="hidden";const Ie=e.querySelector("#mobile-action-bar"),ye=e.querySelector("#mobile-play-area");Ie&&ye&&(ye.style.paddingBottom=Ie.offsetHeight+"px")}if(De(),setTimeout(De,120),setTimeout(De,400),P||(P=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",De),window.visualViewport.addEventListener("scroll",De)),window.addEventListener("resize",De)),function(){const ge=e.querySelector(".terrain-wrapper svg");ge&&(ge.removeAttribute("width"),ge.removeAttribute("height"),ge.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",ge.setAttribute("viewBox","-26 -26 352 352"),ge.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),e.querySelectorAll(".match-slot-hit").forEach(ie=>{ie.addEventListener("click",()=>{if(!B&&!G)return;const ge=ie.dataset.cardId,Ie=ie.dataset.role,ye=(_[Ie]||[]).find(wt=>wt.cardId===ge);if(!ye||ye.used)return;const nt=be.includes(ge);if(B&&!["MIL","ATT"].includes(Ie)&&!nt)return;Array.isArray(y["selected_"+p])||(y["selected_"+p]=[]);const dt=y["selected_"+p],vt=dt.findIndex(wt=>wt.cardId===ge);vt>-1?dt.splice(vt,1):dt.length<3&&dt.push({...ye,_role:Ie}),ae()})}),e.querySelectorAll(".match-used-hit").forEach(ie=>{ie.addEventListener("click",()=>R(ie.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(ie=>{ie.addEventListener("click",()=>R())}),(Ve=e.querySelector("#pvp-sub-open"))==null||Ve.addEventListener("click",()=>R()),e.querySelectorAll(".pvp-gc-mini").forEach(ie=>{ie.addEventListener("click",()=>T(ie.dataset.gcId,ie.dataset.gcType))}),(it=e.querySelector("#pvp-boost-card"))==null||it.addEventListener("click",()=>A()),(yi=e.querySelector("#pvp-action"))==null||yi.addEventListener("click",ie=>{B?ie.currentTarget.dataset.pass==="1"||!$e(p)?O():Ee():G&&_e()}),(bi=e.querySelector("#pvp-quit"))==null||bi.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&I()}),(hi=e.querySelector("#pvp-view-opp"))==null||hi.addEventListener("click",()=>U()),(vi=e.querySelector("#pvp-toggle-history"))==null||vi.addEventListener("click",()=>j()),E&&(clearInterval(E),E=null),(B||G)&&!C){let ie=30,ge=!1;const Ie=()=>document.getElementById("pvp-timer"),ye=()=>{Ie()&&(Ie().textContent=ie+"s",Ie().style.color=ge?"#ff4444":"#fff")};ye(),E=setInterval(()=>{ie--,ie<0?ge?(clearInterval(E),E=null,B&&!$e(p)?O():I()):(ge=!0,ie=15,ye()):ye()},1e3)}}function ke(){He(e),e.innerHTML=`
    ...
    </div>`,p==="p1"&&setTimeout(async()=>{await q({phase:"midfield"})},5e3)}let K=!1;function Q(){if(K)return;const _=y[p+"Team"].MIL||[],L=y[x+"Team"].MIL||[],M=milScore(_)+milLinks(_),S=milScore(L)+milLinks(L),z=M>=S;function D(se,oe,ue){return`<div id="duel-row-${ue}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      </div>`,document.body.appendChild(re),(se=re.querySelector("#pvp-gc-back"))==null||se.addEventListener("click",()=>re.remove()),(oe=re.querySelector("#pvp-gc-use"))==null||oe.addEventListener("click",()=>{re.remove(),de(_,L)})}function A(){var S;const _=y[p+"Team"],L=Object.entries(_).flatMap(([z,D])=>(D||[]).filter(B=>!B.used).map(B=>({...B,_line:z})));if(!L.length)return;const M=document.createElement("div");M.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",M.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
        <div style="flex:1;font-size:14px;font-weight:700;color:#fff">⚡ Choisir un joueur pour +${y.boostValue}</div>
        <button id="bp-close" style="background:none;border:none;color:rgba(255,255,255,0.5);font-size:22px;cursor:pointer">✕</button>
      </div>
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${L.map(z=>{const D={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[z._line]||"#555",B=Ce(z,z._line)+(z.boost||0);return`<div class="bp-item" data-cid="${z.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${D};overflow:hidden;cursor:pointer">
      ...
      </div>`,document.body.appendChild(M),(S=M.querySelector("#bp-close"))==null||S.addEventListener("click",()=>M.remove()),M.querySelectorAll(".bp-item").forEach(z=>{z.addEventListener("click",async()=>{const D=z.dataset.cid,B=L.find(Y=>Y.cardId===D);if(!B)return;const G=(_[B._line]||[]).find(Y=>Y.cardId===D);G&&(G.boost=(G.boost||0)+y.boostValue),M.remove(),await q({[p+"Team"]:_,boostUsed:!0})})})}function R(_=null){var ce,se;if(!(y.phase===p+"-attack")){f("Remplacement uniquement avant votre attaque","warning");return}const M=y[p+"Team"],S=y["usedSubIds_"+p]||[],z=y.maxSubs||3;if(S.length>=z){f(`Maximum ${z} remplacements atteint`,"warning");return}const D=Object.entries(M).flatMap(([oe,ue])=>(ue||[]).filter(fe=>fe.used).map(fe=>({...fe,_line:oe}))),B=(y[p+"Subs"]||[]).filter(oe=>!S.includes(oe.cardId));if(!D.length){f("Aucun joueur utilisé à remplacer","warning");return}if(!B.length){f("Aucun remplaçant disponible","warning");return}let G=Math.max(0,D.findIndex(oe=>oe.cardId===_));const Y=((ce=D[G])==null?void 0:ce._line)||((se=D[G])==null?void 0:se.job);let W=Math.max(0,B.findIndex(oe=>oe.job===Y)),X=!1;const re=document.createElement("div");re.id="pvp-sub-overlay",re.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function J(){var ze,Se,Ne,Pe,Ge,ht;const oe=D[G],ue=B[W],fe=Math.min(130,Math.round((window.innerWidth-90)/2)),xe=Math.round(fe*1.35),Z=qe=>`background:rgba(255,255,255,0.12);border:none;color:${qe?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${qe?"default":"pointer"};flex-shrink:0`;re.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      </div>`,(ze=re.querySelector("#psub-close"))==null||ze.addEventListener("click",()=>re.remove()),(Se=re.querySelector("#pout-up"))==null||Se.addEventListener("click",()=>{G>0&&(G--,J())}),(Ne=re.querySelector("#pout-down"))==null||Ne.addEventListener("click",()=>{G<D.length-1&&(G++,J())}),(Pe=re.querySelector("#pin-up"))==null||Pe.addEventListener("click",()=>{W>0&&(W--,J())}),(Ge=re.querySelector("#pin-down"))==null||Ge.addEventListener("click",()=>{W<B.length-1&&(W++,J())});const be=(qe,Oe,Re,et)=>{const Ke=re.querySelector("#"+qe);if(!Ke)return;let tt=0;Ke.addEventListener("touchstart",De=>{tt=De.touches[0].clientY},{passive:!0}),Ke.addEventListener("touchend",De=>{const Ve=De.changedTouches[0].clientY-tt;if(Math.abs(Ve)<30)return;const it=Oe();Ve<0&&it<et-1?(Re(it+1),J()):Ve>0&&it>0&&(Re(it-1),J())},{passive:!0})};be("pin-panel",()=>W,qe=>W=qe,B.length),be("pout-panel",()=>G,qe=>G=qe,D.length),(ht=re.querySelector("#psub-confirm"))==null||ht.addEventListener("click",async qe=>{if(qe.preventDefault(),qe.stopPropagation(),X)return;X=!0;const Oe=D[G],Re=B[W];if(!Oe||!Re)return;const et=Oe._line,Ke=(M[et]||[]).findIndex(Ve=>Ve.cardId===Oe.cardId);if(Ke===-1){f("Erreur : joueur introuvable","error"),re.remove();return}const tt={...Re,_line:et,position:Oe.position,used:!1,boost:0};M[et].splice(Ke,1,tt);const De=[...S,Re.cardId];re.remove(),H(Oe,Re,async()=>{await q({[p+"Team"]:M,[x+"Team"]:y[x+"Team"],["usedSubIds_"+p]:De})})})}document.body.appendChild(re),J()}function H(_,L,M){const S={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},z=document.createElement("div");z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const D=(Y,W,X)=>`<div style="text-align:center">
      ...
      </div>`,document.body.appendChild(L),(M=L.querySelector("#pvp-hist-close"))==null||M.addEventListener("click",()=>L.remove())}async function O(){if(y.phase!==p+"-attack")return;const _=p==="p1"?"p2":"p1",L=(y.round||0)+1,M=[...y.log||[]];M.push({type:"info",text:`⏭️ ${y[p+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const S=he(y),z=$e(_),D=S||!z?"finished":_+"-attack";await q({["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:D,attacker:_,round:L,log:M}),D==="finished"&&await Be(y)}async function Ee(){const _=y[p+"Team"],L=!["GK","DEF","MIL","ATT"].some(D=>(y[x+"Team"][D]||[]).some(B=>!B.used)),M=(y["selected_"+p]||[]).map(D=>{const B=(_[D._role]||[]).find(se=>se.cardId===D.cardId)||D,G=L&&["GK","DEF"].includes(D._role),Y=_[D._role]||[],W=Y.findIndex(se=>se.cardId===D.cardId),X=xt(Y.length),re=W>=0?X[W]:B._col??1,J=y.stadiumDef||y.homeStadiumDef||null,ce=B.stadiumBonus||J&&(J.club_id&&String(B.club_id)===String(J.club_id)||J.country_code&&B.country_code===J.country_code)||!1;return{...B,_line:D._role,_col:re,stadiumBonus:ce,...G?{note_a:Math.max(1,Number(B.note_a)||0)}:{}}});if(!M.length)return;const S=pi(M,y.modifiers[p]||{});pe(p,M.map(D=>D.cardId)),M.forEach(D=>{const B=(_[D._role]||[]).find(G=>G.cardId===D.cardId);B&&(B.used=!0)}),y["selected_"+p]=[],ae();const z=[...y.log||[]];if(L){const D=(y[p+"Score"]||0)+1,B=M.map(re=>histPlayer(re));z.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:B,homeTotal:S.total,aiTotal:0});const G=(y.round||0)+1,Y=p==="p1"?"p2":"p1",W={...y,[p+"Team"]:_,[p+"Score"]:D},X=he(W);N.add(G),at(B,D,y[x+"Score"]||0,!0,async()=>{await q({[p+"Team"]:_,[p+"Score"]:D,["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:X?"finished":Y+"-attack",attacker:Y,round:G,log:z}),X&&await Be({...y,[p+"Score"]:D})});return}z.push({type:"pending",text:`⚔️ ${y[p+"Name"]} attaque (${S.total})`}),await q({[p+"Team"]:_,[x+"Team"]:y[x+"Team"],pendingAttack:{...S,players:M,side:p},["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},phase:x+"-defense",log:z})}async function _e(){const _=y[p+"Team"],L=(y["selected_"+p]||[]).map(oe=>{const ue=(_[oe._role]||[]).find(ze=>ze.cardId===oe.cardId)||oe,fe=_[oe._role]||[],xe=fe.findIndex(ze=>ze.cardId===oe.cardId),Z=xt(fe.length),be=xe>=0?Z[xe]:ue._col??1;return{...ue,_line:oe._role,_col:be}}),M=ui(L,y.modifiers[p]||{});pe(p,L.map(oe=>oe.cardId)),L.forEach(oe=>{const ue=(_[oe._role]||[]).find(fe=>fe.cardId===oe.cardId);ue&&(ue.used=!0)}),y["selected_"+p]=[],ae();const S=fi(y.pendingAttack.total,M.total,y.modifiers[p]||{}),z=y.pendingAttack.side,D=S==="attack"||(S==null?void 0:S.goal),B=z==="p1"?"p2":"p1",G=(y.round||0)+1,Y=(y.pendingAttack.players||[]).map(oe=>histPlayer(oe)),W=[...y.log||[]];W.push({type:"duel",isGoal:D,homeScored:D&&z===p,text:D?`⚽ BUT de ${y[z+"Name"]} ! (${y.pendingAttack.total} vs ${M.total})`:`✋ Attaque stoppée (${y.pendingAttack.total} vs ${M.total})`,homePlayers:Y,aiPlayers:L.map(oe=>histPlayer(oe)),homeTotal:y.pendingAttack.total,aiTotal:M.total});const X=D?(y[z+"Score"]||0)+1:y[z+"Score"]||0,re={...y,[p+"Team"]:_,[z+"Score"]:X},J=he(re),ce=J?"finished":B+"-attack",se=async()=>{await q({[p+"Team"]:_,[x+"Team"]:y[x+"Team"],[z+"Score"]:X,["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:ce,attacker:B,round:G,log:W}),(ce==="finished"||J)&&await Be({...y,[z+"Score"]:X})};if(D){const oe=z===p,ue=oe?X:y[p+"Score"]||0,fe=oe?y[x+"Score"]||0:X;N.add(G),at(Y,ue,fe,oe,se)}else await se()}function we(_){return["MIL","ATT"].some(L=>(_[L]||[]).some(M=>!M.used))}function me(_){return["GK","DEF","MIL","ATT"].some(L=>(_[L]||[]).some(M=>!M.used))}function Le(_){return me(_)&&!we(_)}function $e(_){const L=y[_+"Team"],M=y[(_==="p1"?"p2":"p1")+"Team"];return!!(we(L)||!me(M)&&Le(L))}function he(_){const L=["MIL","ATT"].some(W=>(_.p1Team[W]||[]).some(X=>!X.used)),M=["MIL","ATT"].some(W=>(_.p2Team[W]||[]).some(X=>!X.used)),S=me(_.p1Team),z=me(_.p2Team);return!L&&!(!z&&S)&&(!M&&!(!S&&z))}function je(_){const L=_.p1Score||0,M=_.p2Score||0;return L===M?null:L>M?v.home_id:v.away_id}async function Be(_){try{const L=je(_),M=L?v.home_id===L?v.away_id:v.home_id:null;await w.from("matches").update({status:"finished",winner_id:L,home_score:_.p1Score||0,away_score:_.p2Score||0}).eq("id",i),L&&M&&updateEvolutiveCards(L,M).catch(()=>{})}catch(L){console.error("[PvP] finishMatch:",L)}}function Fe(){var z;const _=y[p+"Score"],L=y[x+"Score"],M=_>L,S=_===L;He(e),e.innerHTML=`
    ...
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>${g.map((q,I)=>{const k=f>0&&r.status==="finished"?I===0?Math.floor(f*.7):I===1?Math.floor(f*.2):I===2?Math.floor(f*.1):0:0;return`<tr style="border-bottom:1px solid #f5f5f5;${q.userId===o?"background:#f0fdf4;":""}">
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        ${Array.from({length:Math.max(0,r.status==="active"?r.current_day-1:r.current_day)},(q,I)=>I+1).reverse().map(q=>{const I=(d||[]).filter(k=>k.matchday===q);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:#888;margin-bottom:6px">Journée ${q}</div>${I.map(k=>Di(k,u,o,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    </div>`}function g(p){var P,N,te,ne;const x=(P=p.card)==null?void 0:P.player;if(!x)return"";const b=((N=p.card)==null?void 0:N.evolution_bonus)||0,h=lt(x,x.job,b),v=x.job2?lt(x,x.job2,b):0,$=p.status==="sold",y=x.country_code?`https://flagsapi.com/${x.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,C=qt[x.job]||"#bbb",E=x.job2?qt[x.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${$?"opacity:0.7":""}">
      ${y?`<img src="${y}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      ${(te=x.clubs)!=null&&te.logo_url?`<img src="${x.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${C};display:flex;align-items:center;justify-content:center">
