...
import{s as T,F as ki,i as Ht,k as eo,l as nt,m as ji,f as Nt,n as Lo,T as ci,j as Io,o as Mo,b as jo}from"./formation-links-BZe0pGr4.js";const Co="/";function Ci(e,{navigate:t,toast:i}){let n="login";const d=()=>{var r,l,m,a,o,y;const s=n==="login";e.innerHTML=`
  ...
  <div style="position:absolute;left:${G}px;top:${ge}px;width:${ne}px;height:${_e}px;z-index:3;
    ...
    ${Ai(e,{width:140})}
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),a=(l||[]).filter(U=>U.card_type==="player"&&U.player),o=(l||[]).filter(U=>U.card_type==="game_changer"),y=(l||[]).filter(U=>U.card_type==="formation"),p=(l||[]).filter(U=>U.card_type==="stadium"),{data:x}=await T.from("gc_definitions").select("name,gc_type,color,effect,image_url"),_={};(x||[]).forEach(U=>{_[U.name]=U});const{data:f}=await T.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),c={};(f||[]).forEach(U=>{c[U.id]=U}),p.forEach(U=>{U.stadium_def&&(c[U.stadium_id]=U.stadium_def)});const z=Object.keys(ki),u=Object.keys(He),M={};a.forEach(U=>{const ee=U.player.id;M[ee]=(M[ee]||0)+1}),new Set(Object.keys(M).map(U=>String(U)));const F=new Set(y.map(U=>U.formation)),C=new Set(o.map(U=>U.gc_type));let q="player",j="Tous",N="",X=!1;function K(){return[...a].sort((U,ee)=>{const xe=Yt.indexOf(U.player.job),le=Yt.indexOf(ee.player.job);return xe!==le?xe-le:(U.player.surname_encoded||"").localeCompare(ee.player.surname_encoded||"")})}function re(){return[...m||[]].sort((U,ee)=>{const xe=Yt.indexOf(U.job),le=Yt.indexOf(ee.job);return xe!==le?xe-le:(U.surname_encoded||"").localeCompare(ee.surname_encoded||"")})}function oe(){return K().filter(U=>{const ee=U.player,xe=j==="Tous"||ee.job===j,le=!N||`${ee.firstname} ${ee.surname_encoded}`.toLowerCase().includes(N);return xe&&le})}function Z(){return re().filter(U=>{const ee=j==="Tous"||U.job===j,xe=!N||`${U.firstname} ${U.surname_encoded}`.toLowerCase().includes(N);return ee&&xe})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${y.map((se,pe)=>{const de=oe[se.id]||[],U=se.is_for_sale,ee=de.length?de[de.length-1]:null,le=(o.rarity==="pepite"||o.rarity==="papyte")&&se.current_note!=null?` (☆${se.current_note})`:"";return`
            <div data-card-id="${se.id}" data-card-idx="${pe}" class="exemplaire-row"
              ...
              <div style="display:flex;align-items:center;gap:10px">
                ...
                <div style="flex:1;min-width:0">
                  <div style="font-size:12px;font-weight:700">Exemplaire ${pe+1}${le}${U?" 🏷️ En vente":""}</div>
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",o.note_g],["DEF",o.note_d],["MIL",o.note_m],["ATT",o.note_a]].map(([se,pe])=>{const de=pi[se],U=se===o.job||se===o.job2,ee=(Number(pe)||0)+(U&&q>0?q:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:2px">
                <svg width="38" height="37" viewBox="0 0 38 37" style="display:block">
                  <polygon points="19,2 24,13 36,13 26,21 30,33 19,26 8,33 12,21 2,13 14,13" fill="${de}" stroke="white" stroke-width="1.5"/>
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(_e=document.getElementById("close-detail"))==null||_e.addEventListener("click",l);let G=new Set;const ne=()=>{const se=G.size,pe=document.getElementById("sell-action-panel");if(!pe)return;pe.style.display=se>0?"block":"none",document.getElementById("sell-selected-count").textContent=se;const de=document.getElementById("evolve-btn");de&&(de.textContent=`⬆️ Évoluer ${se>1?"(+"+se+")":""}`)};document.querySelectorAll(".expl-check").forEach(se=>{se.addEventListener("change",()=>{const pe=se.dataset.id;se.checked?G.add(pe):G.delete(pe);const de=se.closest(".exemplaire-row");de&&(de.style.borderColor=se.checked?"#1A6B3C":"#eee"),ne()})}),document.querySelectorAll(".exemplaire-row").forEach(se=>{se.addEventListener("click",pe=>{if(pe.target.closest("button")||pe.target.tagName==="INPUT")return;const de=se.querySelector(".expl-check");de&&!de.disabled&&(de.checked=!de.checked,de.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(se=>{se.addEventListener("click",pe=>{pe.stopPropagation();const de=document.querySelector(`.expl-hist[data-hist="${se.dataset.idx}"]`);de&&(de.style.display=de.style.display==="none"?"flex":"none")})}),(we=document.getElementById("evolve-btn"))==null||we.addEventListener("click",async()=>{if(p<=1)return;const se=[...G];if(!se.length)return;if(G.has(e.id)){const me=document.createElement("div");me.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",me.innerHTML=`
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    ${(()=>{const G=window.innerWidth>=700,ne=(U,ee,xe)=>{var B,$;const le=(t.gcDefs||[]).find(g=>g.name===U.gc_type),me={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[le==null?void 0:le.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",ie={purple:"#b06ce0",light_blue:"#00d4ef"}[le==null?void 0:le.color]||"#b06ce0",ue=(le==null?void 0:le.name)||U.gc_type,Le=(le==null?void 0:le.effect)||((B=He[U.gc_type])==null?void 0:B.desc)||"",Ie=le!=null&&le.image_url?`/icons/${le.image_url}`:null,Ce=(($=He[U.gc_type])==null?void 0:$.icon)||"⚡",Ae=Math.round(xe*.22),qe=Math.round(xe*.22),Ge=xe-Ae-qe,Se=ue.length>12?7:9;return`<div class="gc-mini" data-gc-id="${U.id}" data-gc-type="${U.gc_type}"
