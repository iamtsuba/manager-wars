import{s as S,F as zt,h as at,j as ci,l as Oe,i as Li,k as Mi,b as zi}from"./formation-links-CEzN0rcz.js";function Xt(e,{navigate:t,toast:i}){e.innerHTML=`
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),o=(l||[]).filter(L=>L.card_type==="player"&&L.player),r=(l||[]).filter(L=>L.card_type==="game_changer"),u=(l||[]).filter(L=>L.card_type==="formation"),{data:f}=await S.from("gc_definitions").select("name,gc_type,color,effect,image_url"),g={};(f||[]).forEach(L=>{g[L.name]=L});const T=Object.keys(zt),c=Object.keys(Ie),x={};o.forEach(L=>{const k=L.player.id;x[k]=(x[k]||0)+1}),new Set(Object.keys(x).map(L=>String(L)));const _=new Set(u.map(L=>L.formation)),E=new Set(r.map(L=>L.gc_type));let j="player",D="Tous",A="",v=!1;function G(){return[...o].sort((L,k)=>{const R=st.indexOf(L.player.job),N=st.indexOf(k.player.job);return R!==N?R-N:(L.player.surname_encoded||"").localeCompare(k.player.surname_encoded||"")})}function X(){return[...p||[]].sort((L,k)=>{const R=st.indexOf(L.job),N=st.indexOf(k.job);return R!==N?R-N:(L.surname_encoded||"").localeCompare(k.surname_encoded||"")})}function te(){return G().filter(L=>{const k=L.player,R=D==="Tous"||k.job===D,N=!A||`${k.firstname} ${k.surname_encoded}`.toLowerCase().includes(A);return R&&N})}function J(){return X().filter(L=>{const k=D==="Tous"||L.job===D,R=!A||`${L.firstname} ${L.surname_encoded}`.toLowerCase().includes(A);return k&&R})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
  ...
  </div>`;function $(){const L=document.getElementById("col-filters");L&&(j==="player"?(L.innerHTML=`
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",k=>{A=k.target.value.toLowerCase(),y()}),e.querySelectorAll(".filter-btn").forEach(k=>{k.addEventListener("click",()=>{D=k.dataset.job,$(),y()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{v=!v,$(),y()})):(L.innerHTML=`
        <div style="display:flex;justify-content:flex-end">
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{v=!v,$(),y()})))}function y(){const L=document.getElementById("col-grid");L&&(j==="player"?ee(L):j==="formation"?le(L):ue(L))}function F(L,k,R,N,V){V=V||"#7a28b8";const ne=document.getElementById("col-grid"),K=document.getElementById("col-big");if(!ne||!K)return;var me=0;function m(){K.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+k(L[me])+"</div>";var h=K.querySelector("[data-card-id],[data-form-id],[data-gc-id]");h&&h.addEventListener("click",function(){N(L[me])}),requestAnimationFrame(function(){var w=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!w||!K)){var I=K.clientHeight-8,b=K.clientWidth-24,M=w.offsetHeight,z=w.offsetWidth;if(M>0&&z>0&&I>40){var C=Math.min(I/M,b/z,1);w.style.transform="scale("+C.toFixed(3)+")",w.style.transformOrigin="top center"}}}),ne.innerHTML=L.map(function(w,I){return'<div class="col-mini-item" data-idx="'+I+'" style="flex-shrink:0;cursor:pointer;border-radius:8px;border:1.5px solid '+(I===me?V:"transparent")+';transition:border-color .15s;overflow:hidden">'+R(w,I===me)+"</div>"}).join(""),ne.querySelectorAll(".col-mini-item").forEach(function(w){w.addEventListener("click",function(){me=Number(w.dataset.idx),m(),w.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}m()}function q(L){var k=.54,R=Math.round(140*k),N=Math.round(310*k),V;if(!L||L._fake){var ne=L?L.player:null;if(!ne)return"";V=ei(ne)}else V=$t(L,"");return'<div style="width:'+R+"px;height:"+N+'px;overflow:hidden;position:relative;flex-shrink:0"><div style="transform:scale('+k+');transform-origin:top left;position:absolute;top:0;left:0;pointer-events:none">'+V+"</div></div>"}function W(L,k,R){k=k||100,R=R||140;var N=at[L]||{},V={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},ne=Math.max(3,Math.round(k*.06)),K=Object.entries(N).map(function(m){var h=m[0],w=m[1],I=h.replace(/\d+$/,""),b=V[I]||"#888",M=Math.round(w.x*k),z=Math.round(w.y*R);return'<circle cx="'+M+'" cy="'+z+'" r="'+ne+'" fill="'+b+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),me=Math.max(1,Math.round(k/50));return'<svg viewBox="0 0 '+k+" "+R+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+k+'" height="'+R+'" fill="#1A6B3C"/><rect x="'+Math.round(k*.2)+'" y="'+Math.round(R*.02)+'" width="'+Math.round(k*.6)+'" height="'+Math.round(R*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+me+'"/><line x1="0" y1="'+Math.round(R*.5)+'" x2="'+k+'" y2="'+Math.round(R*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+me+'"/><ellipse cx="'+Math.round(k*.5)+'" cy="'+Math.round(R*.5)+'" rx="'+Math.round(k*.18)+'" ry="'+Math.round(R*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+me+'"/><rect x="'+Math.round(k*.2)+'" y="'+Math.round(R*.82)+'" width="'+Math.round(k*.6)+'" height="'+Math.round(R*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+me+'"/>'+K+"</svg>"}function ie(L,k,R){var N=R>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+R+"</div>":"",V=k?'data-form-id="'+k.id+'"':"",ne=L.length>7?14:L.length>5?16:19,K=!!k;return"<div "+V+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(K?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(K?"":"filter:grayscale(1);opacity:0.5")+'">'+N+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(K?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+ne+"px;font-weight:900;color:"+(K?"#1A6B3C":"#aaa")+';line-height:1">'+L+'</div></div><div style="flex:1;overflow:hidden;background:'+(K?"#1A6B3C":"#ccc")+'">'+W(L,140,220)+"</div></div>"}function oe(L,k){var R=.54,N=Math.round(140*R),V=Math.round(305*R),ne=Math.round(V*.22),K=V-ne,me=L.length>7?5:7,m=W(L,N,K),h=k?"1.5px solid #2a7a40":"1px solid #ddd",w=k?"":"filter:grayscale(1);opacity:0.45;",I=k?"#1A6B3C":"#bbb",b="#fff";return'<div style="width:'+N+"px;height:"+V+"px;border-radius:6px;border:"+h+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+w+'"><div style="height:'+ne+"px;background:"+I+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+me+"px;font-weight:900;color:"+b+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(N-4)+'px">'+L+'</span></div><div style="height:'+K+'px;overflow:hidden;flex-shrink:0">'+m+"</div></div>"}function ee(L){if(v){const k=J();if(!k.length){L.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const R=k.map(N=>o.find(V=>V.player.id===N.id)||{_fake:!0,player:N,id:"fake-"+N.id});F(R,N=>N._fake?ei(N.player):$t(N,""),N=>N._fake?q({player:N.player,id:"x",_fake:!0}):q(N),N=>{N._fake||ti(N,o,x,t)},"#1A6B3C")}else{const k=te();if(!k.length){L.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const R={},N=[];k.forEach(V=>{R[V.player.id]||(R[V.player.id]=!0,N.push(V))}),F(N,V=>{const ne=x[V.player.id]||1,K=ne>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${ne}</div>`:"",m=o.filter(h=>h.player.id===V.player.id&&h.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return $t(V,K+m)},V=>q(V),V=>ti(V,o,x,t),"#1A6B3C")}}function le(L){const k=v?T:[..._];if(!k.length){L.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const R=k.map(N=>({formation:N,card:u.find(V=>V.formation===N)||null,owned:_.has(N)}));F(R,({formation:N,card:V,owned:ne})=>ie(N,ne?V:null,ne?u.filter(K=>K.formation===N).length:0),({formation:N,owned:V})=>oe(N,V),({card:N,owned:V})=>{V&&N&&Ni(N,u,t,s)},"#1A6B3C")}function ue(L){const k=Object.keys(g),R=v?k.length?k:c:[...E];if(!R.length){L.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const N=R.map(V=>({type:V,gc:Ie[V]||{icon:"⚡",desc:""},def:g[V]||null,owned:E.has(V),card:r.find(ne=>ne.gc_type===V)||null}));F(N,({type:V,gc:ne,def:K,owned:me,card:m})=>{const h=me?r.filter(U=>U.gc_type===V).length:0,w=h>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${h}</div>`:"",I=(K==null?void 0:K.gc_type)==="ultra_game_changer",b={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},M={purple:"#b06ce0",light_blue:"#00d4ef"},z=b[K==null?void 0:K.color]||b.purple,C=M[K==null?void 0:K.color]||M.purple,P=(K==null?void 0:K.effect)||ne.desc||"",B=K!=null&&K.image_url?`/manager-wars/icons/${K.image_url}`:null;return me&&m?`<div data-gc-id="${m.id}" data-gc-type="${V}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${C};background:${z};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${C}66;cursor:pointer">
          ${w}
          <div style="padding:10px 12px;background:rgba(255,255,255,0.14);text-align:center">
            ...
            <div style="font-size:8px;color:rgba(255,255,255,0.55);margin-top:2px">${I?"💎 ULTRA GC":"⚡ GAME CHANGER"}</div>
          </div>
        ...
        </div>`},({type:V,gc:ne,def:K,owned:me})=>me?(()=>{const m=Math.round(75.60000000000001),h=Math.round(310*.54),w=Math.round(h*.65),I=Math.round(h*.18),b={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},M={purple:"#9b59b6",light_blue:"#00bcd4"},z=b[K==null?void 0:K.color]||b.purple,C=M[K==null?void 0:K.color]||M.purple,P=K!=null&&K.image_url?`/manager-wars/icons/${K.image_url}`:null;return`<div style="width:${m}px;height:${h}px;border-radius:8px;background:${z};border:1px solid ${C};display:flex;flex-direction:column;overflow:hidden"><div style="height:${I}px;background:rgba(255,255,255,0.15);display:flex;align-items:center;justify-content:center"><span style="font-size:7px;font-weight:900;color:#fff;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${m-6}px">${V}</span></div><div style="height:${w}px;display:flex;align-items:center;justify-content:center">${P?`<img src="${P}" style="max-width:${m-8}px;max-height:${w-4}px;object-fit:contain">`:`<span style="font-size:24px">${ne.icon}</span>`}</div><div style="flex:1;display:flex;align-items:center;justify-content:center"><span style="font-size:7px;color:rgba(255,255,255,0.7);text-align:center;padding:0 2px">${((K==null?void 0:K.effect)||ne.desc||"").slice(0,20)}</span></div></div>`})():(()=>{const m=Math.round(75.60000000000001),h=Math.round(310*.54);return`<div style="width:${m}px;height:${h}px;border-radius:8px;background:#eee;border:1px solid #ddd;display:flex;flex-direction:column;align-items:center;justify-content:center;filter:grayscale(1);opacity:0.45"><span style="font-size:22px">${ne.icon}</span><span style="font-size:7px;color:#aaa;margin-top:4px;text-align:center;padding:0 4px">${V}</span></div>`})(),({type:V,owned:ne,def:K})=>{ne&&Ri(V,K,s)},"#7a28b8")}e.querySelectorAll(".col-tab-btn").forEach(L=>{L.addEventListener("click",()=>{j=L.dataset.tab,D="Tous",A="",v=!1,e.querySelectorAll(".col-tab-btn").forEach(k=>{const R=k.dataset.tab===j;k.style.borderBottomColor=R?"var(--green)":"transparent",k.style.color=R?"var(--green)":"var(--gray-600)"}),$(),y()})}),$(),y()}function Ri(e,t,i){const n=Ie[e]||{icon:"⚡",desc:"Effet spécial."},d=(t==null?void 0:t.gc_type)==="ultra_game_changer",s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},a={purple:"#b06ce0",light_blue:"#00d4ef"},l=s[t==null?void 0:t.color]||s.purple,p=a[t==null?void 0:t.color]||a.purple,o=(t==null?void 0:t.name)||e,r=(t==null?void 0:t.effect)||n.desc,u=t!=null&&t.image_url?`/manager-wars/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const et=1e3;function Ni(e,t,i,n){var c,x,_;const{state:d,toast:s,closeModal:a,navigate:l,refreshProfile:p}=i,o=e.formation,r={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function u(){const E=at[o]||{},j=zt[o]||[],D=290,A=360,v=20;function G(te){const J=E[te];return J?{x:J.x*D,y:J.y*A}:null}let X=`<svg width="${D}" height="${A}" viewBox="0 0 ${D} ${A}" xmlns="http://www.w3.org/2000/svg">`;for(const[te,J]of j){const $=G(te),y=G(J);!$||!y||(X+=`<line x1="${$.x}" y1="${$.y}" x2="${y.x}" y2="${y.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const te of Object.keys(E)){const J=G(te);if(!J)continue;const $=te.replace(/\d+/,""),y=r[$]||"#555";X+=`<circle cx="${J.x}" cy="${J.y}" r="${v}" fill="${y}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,X+=`<text x="${J.x}" y="${J.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${$}</text>`}return X+="</svg>",X}const f=t.filter(E=>E.formation===o),g=f.length,T=!e.is_for_sale;n(`Formation ${o}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
  ...
  `,document.body.appendChild(n);let d=!1;const s=document.getElementById("pack-cut-zone"),a=document.getElementById("pack-blade");let l=!1;const p=y=>y.touches&&y.touches[0]?y.touches[0].clientX:y.clientX;function o(y){d||(l=!0,a.style.opacity="1",r(y))}function r(y){if(!l||d)return;const F=s.getBoundingClientRect(),q=p(y)-F.left,W=Math.max(0,Math.min(1,q/F.width));a.style.width=W*F.width+"px",W>=.82&&f()}function u(){d||(l=!1,a.style.transition="width .2s ease, opacity .2s ease",a.style.width="0",a.style.opacity="0",setTimeout(()=>{d||(a.style.transition="")},220))}function f(){var F;if(d)return;d=!0,l=!1,a.style.width="100%",a.style.opacity="1",(F=document.getElementById("cut-flash"))==null||F.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const y=document.getElementById("cut-hint");y&&(y.style.opacity="0"),s.classList.add("pack-cut"),setTimeout(()=>{a.style.opacity="0",document.getElementById("pack-phase").style.display="none",x(0)},620)}s.addEventListener("pointerdown",o),window.addEventListener("pointermove",r),window.addEventListener("pointerup",u),s.addEventListener("touchstart",o,{passive:!0}),window.addEventListener("touchmove",r,{passive:!0}),window.addEventListener("touchend",u);let g=0,T=!1;const c=new Set;function x(y){g=y,document.getElementById("reveal-phase").style.display="flex",_(),E(y,0),G()}function _(){const y=document.getElementById("card-dots");y&&(y.innerHTML=e.map((F,q)=>`<div class="card-dot" data-i="${q}" style="width:8px;height:8px;border-radius:50%;background:${q===g?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),y.querySelectorAll(".card-dot").forEach(F=>F.addEventListener("click",()=>D(parseInt(F.dataset.i)))))}function E(y,F){var L;const q=e[y],W=document.getElementById("card-counter"),ie=document.getElementById("card-track");W&&(W.textContent=`Carte ${y+1} / ${e.length}`);const oe=q.card_type==="player"&&((L=q.player)==null?void 0:L.rarity)==="legende",ee=!c.has(y);c.add(y);let le=0;if(q.card_type==="player"&&q.player){const k=q.player,R=k.job||"ATT";le=Number(R==="GK"?k.note_g:R==="DEF"?k.note_d:R==="MIL"?k.note_m:k.note_a)||0}const ue=k=>{ie.innerHTML=`
        ...
        </div>`;const R=document.getElementById("current-card-wrap");F!==0?(R.style.transition="none",R.style.transform=`translateX(${F>0?100:-100}%)`,requestAnimationFrame(()=>{R.style.transition="transform .28s cubic-bezier(.25,1,.5,1)",R.style.transform="translateX(0)"})):R.animate([{opacity:0,transform:"scale(.7)"},{opacity:1,transform:"scale(1)"}],{duration:300,easing:"cubic-bezier(.34,1.56,.64,1)"}),k||oe?te():J(),_()};ee&&le>6&&q.card_type==="player"&&q.player?j(q,()=>ue(!0)):ue(!1)}function j(y,F){var ue;T=!0;const q=y.player,W=`https://flagsapi.com/${q.country_code}/flat/64.png`,ie=(ue=q.clubs)==null?void 0:ue.logo_url,oe=document.getElementById("walkout-overlay"),ee=document.getElementById("walkout-stage");if(!oe||!ee){T=!1,F();return}oe.style.display="flex";const le=()=>{const L=ee.firstElementChild;L&&(L.classList.remove("wo-in"),L.classList.add("wo-out"))};ee.innerHTML=`<img class="wo-in" src="${W}" style="height:130px;border-radius:10px;box-shadow:0 10px 36px rgba(0,0,0,.6)" onerror="this.style.display='none'">`,navigator.vibrate&&navigator.vibrate(30),setTimeout(le,2e3),setTimeout(()=>{var L;ee.innerHTML=ie?`<img class="wo-in" src="${ie}" style="max-height:160px;max-width:210px;object-fit:contain">`:`<div class="wo-in" style="font-size:34px;font-weight:900;color:#fff;text-align:center">${((L=q.clubs)==null?void 0:L.encoded_name)||"CLUB"}</div>`,navigator.vibrate&&navigator.vibrate(30)},2450),setTimeout(le,4450),setTimeout(()=>{oe.style.display="none",ee.innerHTML="",T=!1,navigator.vibrate&&navigator.vibrate([40,30,80]),F()},4900)}function D(y){if(T||y<0||y>=e.length||y===g)return;const F=y>g?1:-1;g=y,E(y,F)}function A(){D(g+1)}function v(){D(g-1)}function G(){const y=document.getElementById("card-viewport");if(!y||y._swipeBound)return;y._swipeBound=!0;let F=0,q=0,W=0,ie=!1;const oe=k=>k.touches?k.touches[0].clientX:k.clientX,ee=k=>k.touches?k.touches[0].clientY:k.clientY,le=k=>{ie=!0,F=oe(k),q=ee(k),W=0},ue=k=>{if(!ie)return;W=oe(k)-F;const R=ee(k)-q;if(Math.abs(W)<Math.abs(R))return;const N=document.getElementById("current-card-wrap");N&&(N.style.transition="none",N.style.transform=`translateX(${W*.6}px) rotate(${W*.02}deg)`)},L=()=>{if(!ie)return;ie=!1;const k=document.getElementById("current-card-wrap"),R=55;W<=-R&&g<e.length-1?A():W>=R&&g>0?v():k&&(k.style.transition="transform .2s ease",k.style.transform="translateX(0)")};y.addEventListener("pointerdown",le),y.addEventListener("pointermove",ue),y.addEventListener("pointerup",L),y.addEventListener("pointercancel",L),y.addEventListener("touchstart",le,{passive:!0}),y.addEventListener("touchmove",ue,{passive:!0}),y.addEventListener("touchend",L),y.addEventListener("click",k=>{if(Math.abs(W)>8)return;const R=y.getBoundingClientRect();k.clientX-R.left>R.width/2?A():v()})}let X=null;function te(){const y=document.getElementById("fireworks-canvas");if(!y)return;y.width=window.innerWidth,y.height=window.innerHeight;const F=y.getContext("2d"),q=[];function W(){const oe=Math.random()*y.width,ee=Math.random()*y.height*.6,le=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],ue=le[Math.floor(Math.random()*le.length)];for(let L=0;L<60;L++){const k=Math.PI*2/60*L,R=2+Math.random()*5;q.push({x:oe,y:ee,vx:Math.cos(k)*R,vy:Math.sin(k)*R,alpha:1,color:ue,size:2+Math.random()*3})}}W(),X=setInterval(W,600);function ie(){if(document.getElementById("fireworks-canvas")){F.clearRect(0,0,y.width,y.height);for(let oe=q.length-1;oe>=0;oe--){const ee=q[oe];if(ee.x+=ee.vx,ee.y+=ee.vy+.08,ee.vy*=.98,ee.alpha-=.018,ee.alpha<=0){q.splice(oe,1);continue}F.globalAlpha=ee.alpha,F.fillStyle=ee.color,F.beginPath(),F.arc(ee.x,ee.y,ee.size,0,Math.PI*2),F.fill()}F.globalAlpha=1,(X!==null||q.length>0)&&requestAnimationFrame(ie)}}ie()}function J(){X!==null&&(clearInterval(X),X=null);const y=document.getElementById("fireworks-canvas");y&&y.getContext("2d").clearRect(0,0,y.width,y.height)}document.getElementById("reveal-collection").addEventListener("click",()=>{J(),n.remove(),i("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{J(),n.remove(),i("boosters")})}function co(e){var t,i;if(e.card_type==="player"&&e.player)return Zi(e);if(e.card_type==="game_changer"){const n=e._gcDef,d=(n==null?void 0:n.gc_type)==="ultra_game_changer",s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},a={purple:"#b06ce0",light_blue:"#00d4ef"},l=s[n==null?void 0:n.color]||s.purple,p=a[n==null?void 0:n.color]||a.purple,o=(n==null?void 0:n.name)||e.gc_type||"Game Changer",r=(n==null?void 0:n.effect)||((t=Et[e.gc_type])==null?void 0:t.desc)||"",u=n!=null&&n.image_url?`/manager-wars/icons/${n.image_url}`:null,f=((i=Et[e.gc_type])==null?void 0:i.icon)||"⚡";return`<div style="width:170px;height:240px;background:${l};border-radius:14px;border:3px solid ${p};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${p}88;flex-shrink:0">
  ...
  </div>`}function Ke(e,t,i){if(!(e!=null&&e.length))return"";const n=e.slice(0,5);let d='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return n.forEach((s,a)=>{if(d+=Te(s,40,52),a<n.length-1){const l=Oe(s,n[a+1]);d+=`<div style="width:7px;height:3px;background:${l==="#ff3333"||l==="#cc2222"?"rgba(255,255,255,0.12)":l};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(d+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),d+="</div>",d}function At(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function $i(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function nt(e,t,i,n,d=310,s=310,a=[]){var j;const l=at[t]||{},p=ci(t)||zt[t]||[],o={},r=["ATT","MIL","DEF","GK"];for(const D of r)(e[D]||[]).forEach((v,G)=>{o[`${D}${G+1}`]=v});function u(D){const A=l[D];return A?{x:A.x*d,y:A.y*s}:null}let f="";for(const[D,A]of p){const v=u(D),G=u(A);if(!v||!G)continue;const X=o[D],te=o[A],J=Oe(X,te);J==="#00ff88"||J==="#FFD700"?(f+=`<line x1="${v.x.toFixed(1)}" y1="${v.y.toFixed(1)}" x2="${G.x.toFixed(1)}" y2="${G.y.toFixed(1)}"
        stroke="${J}" stroke-width="10" stroke-linecap="round" opacity="0.22"/>`,f+=`<line x1="${v.x.toFixed(1)}" y1="${v.y.toFixed(1)}" x2="${G.x.toFixed(1)}" y2="${G.y.toFixed(1)}"
        stroke="${J}" stroke-width="3.5" stroke-linecap="round" opacity="0.95"/>`):f+=`<line x1="${v.x.toFixed(1)}" y1="${v.y.toFixed(1)}" x2="${G.x.toFixed(1)}" y2="${G.y.toFixed(1)}"
        stroke="${J}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const g=48,T=64,c=13,x=16,_={normal:"#aaaaaa",pépite:"#D4A017",papyte:"#111111",légende:"#7a28b8"};for(const[D,A]of Object.entries(o)){const v=u(D);if(!v||!A)continue;const G=D.replace(/[0-9]/g,""),X=Je[G]||"#555",te=a.includes(A.cardId),J=i==="attack"&&(["MIL","ATT"].includes(G)||te)&&!A.used||i==="defense"&&["GK","DEF","MIL"].includes(G)&&!A.used,$=n.includes(A.cardId);let y;i==="attack"?y=te?Math.max(1,Number(A.note_a)||0):G==="MIL"?Number(A.note_m)||0:Number(A.note_a)||0:i==="defense"?y=G==="GK"?Number(A.note_g)||0:G==="MIL"?Number(A.note_m)||0:Number(A.note_d)||0:y=Number(G==="GK"?A.note_g:G==="DEF"?A.note_d:G==="MIL"?A.note_m:A.note_a)||0,y=y+(A.boost||0);const F=(v.x-g/2).toFixed(1),q=(v.y-T/2).toFixed(1),W=_[A==null?void 0:A.rarity]||_.normal;if(A.used){const k=typeof import.meta<"u"&&"/manager-wars/"||"/",N=`${typeof window<"u"&&((j=window.location)==null?void 0:j.origin)||""}${k}icons/carte-dos.png`.replace(/([^:])\/\//g,"$1/");f+=`<rect x="${F}" y="${q}" width="${g}" height="${T}" rx="5" fill="#161616"/>`,f+=`<image href="${N}" xlink:href="${N}" x="${F}" y="${q}" width="${g}" height="${T}" preserveAspectRatio="xMidYMid slice"/>`,f+=`<rect x="${F}" y="${q}" width="${g}" height="${T}" rx="5" fill="none" stroke="${W}" stroke-width="2" opacity="0.7"/>`,f+=`<rect x="${F}" y="${q}" width="${g}" height="${T}" rx="5" fill="rgba(0,0,0,0.01)" class="match-used-hit" data-card-id="${A.cardId}" data-role="${G}" style="cursor:pointer"/>`;continue}const ie=$?.45:1,oe=$?"#FFD700":W,ee=$?3:(A==null?void 0:A.rarity)==="légende"||(A==null?void 0:A.rarity)==="pépite"?2.5:2,le=T-c-x;f+=`<defs><clipPath id="cp-${D}"><rect x="${F}" y="${(v.y-T/2+c).toFixed(1)}" width="${g}" height="${le}"/></clipPath></defs>`,f+=`<rect x="${F}" y="${q}" width="${g}" height="${T}" rx="5" fill="${X}" opacity="${ie}"/>`;const ue=ve(A);ue&&(f+=`<image href="${ue}" xlink:href="${ue}" x="${F}" y="${(v.y-T/2+c).toFixed(1)}" width="${g}" height="${le}" clip-path="url(#cp-${D})" preserveAspectRatio="xMidYMin slice"/>`),f+=`<rect x="${F}" y="${q}" width="${g}" height="${c}" rx="4" fill="rgba(255,255,255,0.92)"/>`,f+=`<text x="${v.x.toFixed(1)}" y="${(v.y-T/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(A.name||"").slice(0,9)}</text>`;const L=(v.y+T/2-x).toFixed(1);f+=`<rect x="${F}" y="${L}" width="${g}" height="${x}" fill="rgba(255,255,255,0.92)"/>`;{const k=At(A.country_code);k?f+=`<image href="${k}" xlink:href="${k}" x="${(v.x-20).toFixed(1)}" y="${(v.y+T/2-x+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:f+=`<text x="${(v.x-13).toFixed(1)}" y="${(v.y+T/2-x/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${$i(A.country_code)}</text>`,f+=`<text x="${v.x.toFixed(1)}" y="${(v.y+T/2-x/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${y}</text>`;const R=wi(A);R?f+=`<image href="${R}" xlink:href="${R}" x="${(v.x+g/2-14).toFixed(1)}" y="${(v.y+T/2-x+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:A.clubName&&(f+=`<text x="${(v.x+14).toFixed(1)}" y="${(v.y+T/2-x/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(A.clubName||"").slice(0,3).toUpperCase()}</text>`),A.boost&&(f+=`<rect x="${(v.x+g/2-12).toFixed(1)}" y="${(v.y-T/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,f+=`<text x="${(v.x+g/2-5).toFixed(1)}" y="${(v.y-T/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${A.boost}</text>`)}f+=`<rect x="${F}" y="${q}" width="${g}" height="${T}" rx="5" fill="${$?"rgba(255,255,255,0.12)":"none"}" stroke="${oe}" stroke-width="${ee}" opacity="${ie}"/>`,J&&(f+=`<rect x="${F}" y="${q}" width="${g}" height="${T}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${$?"selected":""}" data-card-id="${A.cardId}" data-role="${G}" style="cursor:pointer"/>`)}const E=38;return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-E} ${-E} ${d+E*2} ${s+E*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${f}
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${c.map((j,D)=>{const A=D<c.length-1?Oe(j,c[D+1]):null,v=!A||A==="#ff3333"||A==="#cc2222",G=A==="#00ff88"?"+2":A==="#FFD700"?"+1":"";return`
          ...
          ${D<c.length-1?`<div class="duel-link duel-link-${E}" data-idx="${D}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${v?"rgba(255,255,255,0.12)":A};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${v?"none":`0 0 8px ${A}`}">
  ...
  <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div id="duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center, rgba(10,61,30,0.4), rgba(10,61,30,0.92))"></div>
          ...
          <button id="start-match-btn" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,0.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">
            ▶ Commencer le match
          </button>`,A.style.transition="opacity .45s ease",A.style.opacity="1",A.style.pointerEvents="auto",(v=document.getElementById("start-match-btn"))==null||v.addEventListener("click",T))},600)},700)},2800)}function _o(e){if(e.type==="duel"&&(e.homeTotal!=null||e.aiTotal!=null)){const t=(e.homeTotal||0)>=(e.aiTotal||0);return`
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const y=t.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
            ...
            ${Ke((y.players||[]).map(F=>({...F,used:!1})),"#ff6b6b",y.total)}
            ...
            ${Ke((y.players||[]).map(F=>({...F,used:!1})),"#00ff88",y.total)}
    ...
    ${(()=>{const $=window.innerWidth>=700,y=(L,k,R)=>{var z,C;const N=(t.gcDefs||[]).find(P=>P.name===L.gc_type),V={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[N==null?void 0:N.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",ne={purple:"#b06ce0",light_blue:"#00d4ef"}[N==null?void 0:N.color]||"#b06ce0",K=(N==null?void 0:N.name)||L.gc_type,me=(N==null?void 0:N.effect)||((z=Ie[L.gc_type])==null?void 0:z.desc)||"",m=N!=null&&N.image_url?`/manager-wars/icons/${N.image_url}`:null,h=((C=Ie[L.gc_type])==null?void 0:C.icon)||"⚡",w=Math.round(R*.22),I=Math.round(R*.22),b=R-w-I,M=K.length>12?7:9;return`<div class="gc-mini" data-gc-id="${L.id}" data-gc-type="${L.gc_type}"
          ...
          <div style="height:${b}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
            ${m?`<img src="${m}" style="max-width:${k-10}px;max-height:${b-6}px;object-fit:contain">`:`<span style="font-size:${Math.round(b*.55)}px">${h}</span>`}
          </div>
        ...
        </div>`},F=(L,k)=>{var R;return`<div id="boost-card"
  ...
  `,document.body.appendChild(n);let r=!1;const u=()=>{r||(r=!0,n.remove(),i())};n.addEventListener("click",u),setTimeout(u,2e3)}function tt(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function _t(e,t,i,n=null,d=null){var g,T;if(t.phase!=="attack"){tt("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){tt(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const s=Object.entries(t.homeTeam).flatMap(([c,x])=>(x||[]).filter(_=>_.used).map(_=>({..._,_line:_._line||c}))),a=t.homeSubs.filter(c=>!t.usedSubIds.includes(c.cardId));if(!s.length){tt("Aucun joueur utilisé à remplacer");return}if(!a.length){tt("Aucun remplaçant disponible");return}let l=Math.max(0,s.findIndex(c=>c.cardId===d));const p=((g=s[l])==null?void 0:g._line)||((T=s[l])==null?void 0:T.job);let o=n?Math.max(0,a.findIndex(c=>c.cardId===n)):Math.max(0,a.findIndex(c=>c.job===p)),r=!1;const u=document.createElement("div");u.id="sub-overlay",u.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function f(){var A,v,G,X,te,J;const c=s[l],x=a[o],_=Math.min(130,Math.round((window.innerWidth-90)/2)),E=Math.round(_*1.35),j=$=>`background:rgba(255,255,255,0.12);border:none;color:${$?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${$?"default":"pointer"};flex-shrink:0`;u.innerHTML=`
    ...
    </div>`,(A=u.querySelector("#sub-close"))==null||A.addEventListener("click",()=>u.remove()),(v=u.querySelector("#out-up"))==null||v.addEventListener("click",()=>{l>0&&(l--,f())}),(G=u.querySelector("#out-down"))==null||G.addEventListener("click",()=>{l<s.length-1&&(l++,f())}),(X=u.querySelector("#in-up"))==null||X.addEventListener("click",()=>{o>0&&(o--,f())}),(te=u.querySelector("#in-down"))==null||te.addEventListener("click",()=>{o<a.length-1&&(o++,f())});const D=($,y,F,q)=>{const W=u.querySelector("#"+$);if(!W)return;let ie=0;W.addEventListener("touchstart",oe=>{ie=oe.touches[0].clientY},{passive:!0}),W.addEventListener("touchend",oe=>{const ee=oe.changedTouches[0].clientY-ie;if(Math.abs(ee)<30)return;const le=y();ee<0&&le<q-1?(F(le+1),f()):ee>0&&le>0&&(F(le-1),f())},{passive:!0})};D("in-panel",()=>o,$=>o=$,a.length),D("out-panel",()=>l,$=>l=$,s.length),(J=u.querySelector("#sub-confirm"))==null||J.addEventListener("click",$=>{if($.preventDefault(),$.stopPropagation(),r)return;r=!0;const y=s[l],F=a[o];if(!y||!F)return;let q=null,W=-1;for(const[oe,ee]of Object.entries(t.homeTeam)){const le=(ee||[]).findIndex(ue=>ue.cardId===y.cardId);if(le!==-1){q=oe,W=le;break}}if(W===-1||!q){tt("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),u.remove();return}const ie={...F,_line:q,_col:y._col||0,used:!1,boost:0};t.homeTeam[q].splice(W,1,ie),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(F.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:y.name,firstname:y.firstname,note:pe(y,q),portrait:ve(y),job:y.job,country_code:y.country_code,rarity:y.rarity,clubName:y.clubName,clubLogo:y.clubLogo},inPlayer:{name:F.name,firstname:F.firstname,note:pe(F,q),portrait:ve(F),job:F.job,country_code:F.country_code,rarity:F.rarity,clubName:F.clubName,clubLogo:F.clubLogo},text:`🔄 ${F.firstname} ${F.name} remplace ${y.firstname} ${y.name}`}),u.remove(),Io(y,F,()=>ye(e,t,i))})}document.body.appendChild(u),f()}function Lo(e,t,i,n,d){var T,c;const s=(n.gcDefs||[]).find(x=>x.name===t),a=Ie[t]||{icon:"⚡",desc:"Carte spéciale."},l={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[s==null?void 0:s.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",p={purple:"#b06ce0",light_blue:"#00d4ef"}[s==null?void 0:s.color]||"#b06ce0",o=(s==null?void 0:s.name)||t,r=(s==null?void 0:s.effect)||a.desc,u=s!=null&&s.image_url?`/manager-wars/icons/${s.image_url}`:null,f=a.icon||"⚡",g=document.createElement("div");g.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",g.innerHTML=`
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(s);let l=!1;const p=()=>{l||(l=!0,s.remove(),setTimeout(()=>d(),50))};s.addEventListener("click",p),setTimeout(p,3500)}async function it(e,t,i){var r,u;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:n}=i,d=t.homeScore>t.aiScore,s=t.homeScore===t.aiScore,a=d?"victoire":s?"nul":"defaite",l=Fi(t.mode,a);t.matchId&&await S.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:d?n.profile.id:null,home_credits_reward:l,played_at:new Date().toISOString()}).eq("id",t.matchId);const p={credits:(n.profile.credits||0)+l,matches_played:(n.profile.matches_played||0)+1};d?p.wins=(n.profile.wins||0)+1:s?p.draws=(n.profile.draws||0)+1:p.losses=(n.profile.losses||0)+1,await S.from("users").update(p).eq("id",n.profile.id),await i.refreshProfile();const o=document.createElement("div");o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",o.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const g=async(j=!0)=>{u=!0,f&&(S.removeChannel(f),f=null),j&&await S.rpc("cancel_matchmaking",{p_user_id:p.profile.id}).catch(()=>{})};(E=document.getElementById("mm-cancel"))==null||E.addEventListener("click",async()=>{try{await g(!0)}catch{}ze(e),o("home")});const T=async(j,D,A)=>{if(u)return;u=!0,f&&(S.removeChannel(f),f=null);const v=document.getElementById("mm-status");v&&(v.textContent="Adversaire trouvé !"),await new Promise(G=>setTimeout(G,600)),e.isConnected&&Fo(e,t,j,A,a,l)},{data:c,error:x}=await S.rpc("try_matchmake",{p_user_id:p.profile.id,p_deck_id:i});if(x||!(c!=null&&c.success)){r("Erreur de matchmaking","error"),ze(e),o("home");return}if(c.matched){T(c.match_id,c.opponent_id,!1);return}const _=document.getElementById("mm-status");_&&(_.textContent="En attente d'un autre joueur..."),f=S.channel("matchmaking-"+p.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${p.profile.id}`},j=>{const D=j.new;D.status==="matched"&&D.match_id&&T(D.match_id,D.matched_with,!0)}).subscribe()}async function Fo(e,t,i,n,d=[],s=[]){const{state:a,navigate:l,toast:p}=t,o=n?"p1":"p2",r=n?"p2":"p1",u=(d||[]).map(m=>m.id),f=(d||[]).map(m=>({id:m.id,gc_type:m.gc_type,_gcDef:m._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:g}=await S.from("matches").select("*").eq("id",i).single();if(!g){p("Match introuvable","error"),l("home");return}async function T(){const[{data:m},{data:h},{data:w},{data:I}]=await Promise.all([S.rpc("get_deck_for_match",{p_deck_id:g.home_deck_id}),S.rpc("get_deck_for_match",{p_deck_id:g.away_deck_id}),S.from("users").select("id,pseudo,club_name").eq("id",g.home_id).single(),S.from("users").select("id,pseudo,club_name").eq("id",g.away_id).single()]),b=B=>({cardId:B.card_id,position:B.position,id:B.id,firstname:B.firstname,name:B.surname_encoded,country_code:B.country_code,club_id:B.club_id,job:B.job,job2:B.job2,note_g:Number(B.note_g)||0,note_d:Number(B.note_d)||0,note_m:Number(B.note_m)||0,note_a:Number(B.note_a)||0,rarity:B.rarity,skin:B.skin,hair:B.hair,hair_length:B.hair_length,clubName:B.club_encoded_name||null,clubLogo:B.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}),M=((m==null?void 0:m.starters)||[]).map(B=>b(B)),z=((h==null?void 0:h.starters)||[]).map(B=>b(B)),C=(m==null?void 0:m.formation)||"4-4-2",P=(h==null?void 0:h.formation)||"4-4-2";return{p1Team:xt(M,C),p2Team:xt(z,P),p1Subs:((m==null?void 0:m.subs)||[]).map(B=>b(B)),p2Subs:((h==null?void 0:h.subs)||[]).map(B=>b(B)),p1Formation:C,p2Formation:P,p1Name:(w==null?void 0:w.club_name)||(w==null?void 0:w.pseudo)||"Joueur 1",p2Name:(I==null?void 0:I.club_name)||(I==null?void 0:I.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:n?u:[],gc_p2:n?[]:u,gcCardsFull_p1:n?f:[],gcCardsFull_p2:n?[]:f,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,lastActionAt:new Date().toISOString()}}let c=g.game_state&&Object.keys(g.game_state).length?g.game_state:null;if(!c)if(n){c=await T();const{data:m}=await S.from("matches").select("game_state").eq("id",i).single();!(m!=null&&m.game_state)||!Object.keys(m.game_state).length?await S.from("matches").update({game_state:c,turn_user_id:g.home_id}).eq("id",i):c=m.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let m=0;m<30&&!c;m++){await new Promise(w=>setTimeout(w,400));const{data:h}=await S.from("matches").select("game_state").eq("id",i).single();h!=null&&h.game_state&&Object.keys(h.game_state).length&&(c=h.game_state)}if(!c){p("Erreur de synchronisation","error"),l("home");return}c.gc_p2=u,c.gcCardsFull_p2=f,await S.from("matches").update({game_state:c}).eq("id",i)}let x=!1,_=null,E=!1;const j=new Set;function D(m){var B,U;try{S.removeChannel(A)}catch{}const h=c[o+"Score"]||0,w=c[r+"Score"]||0;let I,b;m.winner_id?(I=m.winner_id===a.profile.id,b=!1):m.forfeit?(I=h>w,b=!1):(b=h===w,I=h>w),(B=document.getElementById("pvp-end-overlay"))==null||B.remove();const M=document.createElement("div");M.id="pvp-end-overlay",M.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const z=b?"🤝":I?"🏆":"😞",C=b?"MATCH NUL":I?"VICTOIRE !":"DÉFAITE",P=b?"#fff":I?"#FFD700":"#ff6b6b";M.innerHTML=`
      <div style="font-size:64px">${z}</div>
      <div style="font-size:26px;font-weight:900;color:${P}">${C}</div>
      <div style="font-size:18px">${c[o+"Name"]} ${h} – ${w} ${c[r+"Name"]}</div>
      ${m.forfeit?`<div style="font-size:13px;color:rgba(255,255,255,0.5)">${I?"L'adversaire a quitté":"Perdu par forfait"}</div>`:""}
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(M),(U=M.querySelector("#pvp-end-home"))==null||U.addEventListener("click",()=>{M.remove(),ze(e),l("home")})}const A=S.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},m=>{const h=m.new;try{if(h.status==="finished"||h.forfeit){if(x)return;x=!0,_&&(clearInterval(_),_=null),h.game_state&&(c=h.game_state),D(h);return}if(h.game_state){const w=c;c=h.game_state;const I=w[o+"Score"]||0,b=w[r+"Score"]||0,M=c[o+"Score"]||0,z=c[r+"Score"]||0,C=M>I,P=z>b;if((C||P)&&!j.has(c.round)){j.add(c.round);const B=[...c.log||[]].reverse().find(H=>H.isGoal),U=((B==null?void 0:B.homePlayers)||[]).map(H=>({name:H.name,note:H.note,portrait:H.portrait,job:H.job}));oe(U,M,z,C,()=>F());return}F()}}catch(w){console.error("[PvP] crash:",w)}}).subscribe();async function v(m){Object.assign(c,m),c.lastActionAt=new Date().toISOString();const{error:h}=await S.from("matches").update({game_state:c}).eq("id",i);h&&p("Erreur de synchronisation","error");try{F()}catch(w){console.error("[PvP] renderPvpScreen crash:",w)}}async function G(){if(x)return;x=!0,_&&(clearInterval(_),_=null);const m=n?g.away_id:g.home_id,h=n?"p2":"p1",w=n?"p1":"p2",I={...c,[h+"Score"]:3,[w+"Score"]:0,phase:"finished"};try{await S.from("matches").update({status:"finished",forfeit:!0,winner_id:m,game_state:I}).eq("id",i)}catch{}try{S.removeChannel(A)}catch{}setTimeout(()=>{ze(e),l("home")},800)}const X={BOOST_STAT:({value:m=1,count:h=1,roles:w=[]},I,b)=>{const M=I[o+"Team"],z=Object.entries(M).filter(([C])=>!w.length||w.includes(C)).flatMap(([C,P])=>P.filter(B=>!B.used).map(B=>({...B,_line:C})));if(!z.length){I.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),b(I);return}te(z,h,`Choisir ${h} joueur(s) à booster (+${m})`,C=>{C.forEach(P=>{const B=(M[P._line]||[]).find(U=>U.cardId===P.cardId);B&&(B.boost=(B.boost||0)+m,I.log.push({text:`⚡ +${m} sur ${B.name}`,type:"info"}))}),I[o+"Team"]=M,b(I)})},DEBUFF_STAT:({value:m=1,count:h=1,roles:w=[],target:I="ai"},b,M)=>{const z=I==="home"?o:r,C=b[z+"Team"],P=I==="home"?"allié":"adverse",B=Object.entries(C).filter(([U])=>!w.length||w.includes(U)).flatMap(([U,H])=>H.filter(re=>!re.used).map(re=>({...re,_line:U})));if(!B.length){b.log.push({text:`🎯 Aucun joueur ${P}`,type:"info"}),M(b);return}te(B,h,`Choisir ${h} joueur(s) ${P}(s) (-${m})`,U=>{U.forEach(H=>{const re=(C[H._line]||[]).find(ge=>ge.cardId===H.cardId);re&&(re.boost=(re.boost||0)-m,b.log.push({text:`🎯 -${m} sur ${re.name}`,type:"info"}))}),b[z+"Team"]=C,M(b)})},GRAY_PLAYER:({count:m=1,roles:h=[],target:w="ai"},I,b)=>{const M=w==="home"?o:r,z=I[M+"Team"],C=w==="home"?"allié":"adverse",P=Object.entries(z).filter(([B])=>!h.length||h.includes(B)).flatMap(([B,U])=>U.filter(H=>!H.used).map(H=>({...H,_line:B})));if(!P.length){I.log.push({text:`❌ Aucun joueur ${C}`,type:"info"}),b(I);return}te(P,m,`Choisir ${m} joueur(s) ${C}(s) à exclure`,B=>{B.forEach(U=>{const H=(z[U._line]||[]).find(re=>re.cardId===U.cardId);H&&(H.used=!0,I.log.push({text:`❌ ${H.name} exclu !`,type:"info"}))}),I[M+"Team"]=z,b(I)})},REVIVE_PLAYER:({count:m=1,roles:h=[]},w,I)=>{const b=w[o+"Team"],M=Object.entries(b).filter(([z])=>!h.length||h.includes(z)).flatMap(([z,C])=>C.filter(P=>P.used).map(P=>({...P,_line:z})));if(!M.length){w.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),I(w);return}te(M,m,`Choisir ${m} joueur(s) à ressusciter`,z=>{z.forEach(C=>{const P=(b[C._line]||[]).find(B=>B.cardId===C.cardId);P&&(P.used=!1,w.log.push({text:`💫 ${P.name} ressuscité !`,type:"info"}))}),w[o+"Team"]=b,I(w)})},REMOVE_GOAL:({},m,h)=>{const w=r+"Score";m[w]>0?(m[w]--,m.log.push({text:"🚫 Dernier but annulé !",type:"info"})):m.log.push({text:"🚫 Aucun but à annuler",type:"info"}),h(m)},ADD_GOAL_DRAW:({},m,h)=>{m[o+"Score"]===m[r+"Score"]?(m[o+"Score"]++,m.log.push({text:"🎯 But bonus !",type:"info"})):m.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),h(m)},ADD_SUB:({value:m=1},h,w)=>{h.maxSubs=(h.maxSubs||3)+m,h.log.push({text:`🔄 +${m} remplacement(s)`,type:"info"}),w(h)},CUSTOM:({},m,h)=>h(m)};function te(m,h,w,I){const b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let M=[];function z(){var P,B;const C=m.map(U=>{const H={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[U._line]||"#555",re=pe(U,U._line)+(U.boost||0),$e=M.find(Q=>Q.cardId===U.cardId)?"#FFD700":"rgba(255,255,255,0.25)",ce=U.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${U.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${$e};background:${H};overflow:hidden;cursor:pointer;${ce}">
          <div style="background:rgba(255,255,255,0.9);text-align:center;padding:2px;font-size:7px;font-weight:900;color:#111;overflow:hidden;white-space:nowrap;text-overflow:ellipsis">${U.name||"?"}</div>
        ...
        </div>`,(P=b.querySelector("#pp-close"))==null||P.addEventListener("click",()=>b.remove()),b.querySelectorAll(".pp-item").forEach(U=>{U.addEventListener("click",()=>{const H=U.dataset.cid,re=m.find($e=>$e.cardId===H),ge=M.findIndex($e=>$e.cardId===H);re&&(ge>-1?M.splice(ge,1):M.length<h&&M.push(re),z())})}),(B=b.querySelector("#pp-confirm"))==null||B.addEventListener("click",()=>{b.remove(),I(M)})}z(),document.body.appendChild(b)}async function J(m,h){const I=(c["gcCardsFull_"+o]||[]).find(C=>C.id===m),b=(I==null?void 0:I._gcDef)||(c.gcDefs||[]).find(C=>{var P;return C.name===h||((P=C.name)==null?void 0:P.toLowerCase().trim())===(h==null?void 0:h.toLowerCase().trim())}),M=[...c["usedGc_"+o]||[],m];if(b!=null&&b.effect_type&&b.effect_type!=="CUSTOM"){const C=X[b.effect_type];if(C){const P={...c};C(b.effect_params||{},P,async B=>{B["usedGc_"+o]=M,await v(B)});return}}const z={...c};switch(h){case"Remplacement+":z.maxSubs=(z.maxSubs||3)+1,z.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const C=r+"Score";z[C]>0&&(z[C]--,z.log.push({text:"🚫 But annulé",type:"info"}));break}}z["usedGc_"+o]=M,await v(z)}function $(m,h){const w="usedCardIds_"+m,I=new Set(c[w]||[]);h.forEach(b=>I.add(b)),c[w]=[...I]}function y(){for(const m of["p1","p2"]){const h=new Set(c["usedCardIds_"+m]||[]),w=c[m+"Team"];if(w)for(const I of["GK","DEF","MIL","ATT"])(w[I]||[]).forEach(b=>{b.used=h.has(b.cardId)})}}function F(){var De,Rt,Nt,Ot,Pt,Ht;if(c.phase==="reveal")return q();if(c.phase==="midfield")return ie();if(c.phase==="finished")return me();const m=c[o+"Team"],h=c[r+"Team"];y();const w=c[o+"Score"],I=c[r+"Score"],b=c[o+"Name"],M=c[r+"Name"],z=c.phase===o+"-attack",C=c.phase===o+"-defense",P=Array.isArray(c["selected_"+o])?c["selected_"+o]:[],B=P.map(O=>O.cardId),U=window.innerWidth>=700,H=c[o+"Subs"]||[],re=c["usedSubIds_"+o]||[],ge=H.filter(O=>!re.includes(O.cardId)),$e=c["gcCardsFull_"+o]||[],ce=c["usedGc_"+o]||[],Q=$e.filter(O=>!ce.includes(O.id)),Y=c.boostOwner===o&&!c.boostUsed,se=!["GK","DEF","MIL","ATT"].some(O=>(h[O]||[]).some(Z=>!Z.used)),de=[...m.MIL||[],...m.ATT||[]].filter(O=>!O.used),we=z&&se&&de.length===0?[...m.GK||[],...m.DEF||[]].filter(O=>!O.used).map(O=>O.cardId):[];function he(O,Z,xe){var Yt,Wt;const ae=O._gcDef,Ge={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ae==null?void 0:ae.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",He={purple:"#b06ce0",light_blue:"#00d4ef"}[ae==null?void 0:ae.color]||"#b06ce0",Qe=(ae==null?void 0:ae.name)||O.gc_type,Ze=(ae==null?void 0:ae.effect)||((Yt=Ie[O.gc_type])==null?void 0:Yt.desc)||"",Ut=ae!=null&&ae.image_url?`/manager-wars/icons/${ae.image_url}`:null,Ai=((Wt=Ie[O.gc_type])==null?void 0:Wt.icon)||"⚡",Kt=Math.round(xe*.22),Vt=Math.round(xe*.22),wt=xe-Kt-Vt,Ii=Qe.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${O.id}" data-gc-type="${O.gc_type}"
      ...
      </div>`}const ke=(O,Z)=>Z?be(130,175):he(O,130,175),Be=(O,Z)=>Z?be(68,95):he(O,68,95),Se=U?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",Le=z?`<button id="pvp-action" style="${Se};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${P.length===0?"disabled":""}>⚔️ ATTAQUEZ <span id="pvp-timer"></span></button>`:C?`<button id="pvp-action" style="${Se};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${P.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="pvp-timer"></span></button>`:`<div style="font-size:11px;color:rgba(255,255,255,0.3);text-align:center;padding:4px">⏳ Tour de ${M}</div>`,_e=z||C?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${P.length}/3 sélectionné(s)</div>`:"",Me=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${U?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
      ${ge.length===0?'<div style="font-size:7px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':ge.map(O=>`<div class="pvp-sub-btn" data-sub-id="${O.cardId}" style="cursor:pointer;flex-shrink:0">${Te(O,U?76:44,U?100:58)}</div>`).join("")}
    </div>`,Ae=z?"attack":C?"defense":"idle",Fe=`<div style="overflow:hidden;min-width:0;flex:1;min-height:0;display:flex;flex-direction:column" id="match-field">
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${rt(m,c[o+"Formation"],Ae,B,300,300,we)}
      </div>
        ...
        </div>`}if(z&&((xe=c.pendingAttack)!=null&&xe.players)){const ae=c.pendingAttack;return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #00ff88">
          <div style="font-size:8px;color:#00ff88;letter-spacing:2px;margin-bottom:4px;text-transform:uppercase">⚔️ VOUS ATTAQUEZ</div>
      ...
      <div style="display:flex;align-items:center;padding:8px 10px;background:rgba(0,0,0,0.5);gap:6px;flex-shrink:0">
        ...
        <div style="flex:1;display:flex;align-items:center;justify-content:center;gap:8px">
          ...
          <span style="font-size:26px;font-weight:900;color:#FFD700;letter-spacing:2px">${w} – ${I}</span>
      ...
      <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
        ...
        <div id="mobile-action-bar" style="position:absolute;left:0;right:0;bottom:0;z-index:20;background:rgba(0,0,0,0.55);padding:6px 8px 8px;display:flex;flex-direction:column;gap:6px;box-shadow:0 -4px 16px rgba(0,0,0,0.5)">
          <div style="display:flex;gap:6px;overflow-x:auto;align-items:flex-end;min-height:96px;padding-bottom:2px">
            ${Q.map(O=>Be(O,!1)).join("")}
            ${Y?Be(null,!0):""}
            <div id="pvp-sub-open" style="cursor:${z&&ge.length>0?"pointer":"default"};flex-shrink:0;box-sizing:border-box;width:68px;height:95px;border-radius:10px;border:2px solid ${z&&ge.length>0?"rgba(255,255,255,0.5)":"rgba(255,255,255,0.15)"};background:${z&&ge.length>0?"rgba(60,60,60,0.9)":"rgba(40,40,40,0.5)"};display:flex;flex-direction:column;align-items:center;justify-content:center;gap:4px;opacity:${z&&ge.length>0?1:.4}">
              <div style="display:flex;gap:6px;align-items:center">
                <div style="text-align:center">
                  <div style="font-size:7px;color:#00ff88;font-weight:700;letter-spacing:1px">IN</div>
      ...
      </div>`;function Ee(){const O=e.querySelector(".match-screen");if(!O)return;const Z=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);O.style.height=Z+"px",O.style.minHeight=Z+"px",O.style.maxHeight=Z+"px",O.style.overflow="hidden";const xe=e.querySelector("#mobile-action-bar"),ae=e.querySelector("#mobile-play-area");xe&&ae&&(ae.style.paddingBottom=xe.offsetHeight+"px")}if(Ee(),setTimeout(Ee,120),setTimeout(Ee,400),E||(E=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",Ee),window.visualViewport.addEventListener("scroll",Ee)),window.addEventListener("resize",Ee)),function(){const Z=e.querySelector(".terrain-wrapper svg");Z&&(Z.removeAttribute("width"),Z.removeAttribute("height"),Z.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",Z.setAttribute("viewBox","-26 -26 352 352"),Z.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),e.querySelectorAll(".match-slot-hit").forEach(O=>{O.addEventListener("click",()=>{if(!z&&!C)return;const Z=O.dataset.cardId,xe=O.dataset.role,ae=(m[xe]||[]).find(Ze=>Ze.cardId===Z);if(!ae||ae.used)return;const Ge=we.includes(Z);if(z&&!["MIL","ATT"].includes(xe)&&!Ge)return;Array.isArray(c["selected_"+o])||(c["selected_"+o]=[]);const He=c["selected_"+o],Qe=He.findIndex(Ze=>Ze.cardId===Z);Qe>-1?He.splice(Qe,1):He.length<3&&He.push({...ae,_role:xe}),F()})}),e.querySelectorAll(".match-used-hit").forEach(O=>{O.addEventListener("click",()=>ue(O.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(O=>{O.addEventListener("click",()=>ue())}),(De=e.querySelector("#pvp-sub-open"))==null||De.addEventListener("click",()=>ue()),e.querySelectorAll(".pvp-gc-mini").forEach(O=>{O.addEventListener("click",()=>ee(O.dataset.gcId,O.dataset.gcType))}),(Rt=e.querySelector("#pvp-boost-card"))==null||Rt.addEventListener("click",()=>le()),(Nt=e.querySelector("#pvp-action"))==null||Nt.addEventListener("click",()=>{z?N():C&&V()}),(Ot=e.querySelector("#pvp-quit"))==null||Ot.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&G()}),(Pt=e.querySelector("#pvp-view-opp"))==null||Pt.addEventListener("click",()=>k()),(Ht=e.querySelector("#pvp-toggle-history"))==null||Ht.addEventListener("click",()=>R()),_&&(clearInterval(_),_=null),(z||C)&&!x){let O=30,Z=!1;const xe=()=>document.getElementById("pvp-timer"),ae=()=>{xe()&&(xe().textContent=O+"s",xe().style.color=Z?"#ff4444":"#fff")};ae(),_=setInterval(()=>{O--,O<0?Z?(clearInterval(_),_=null,G()):(Z=!0,O=15,ae()):ae()},1e3)}}function q(){qe(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
      ...
      <div style="width:min(90vw,420px)">${nt(c[r+"Team"],c[r+"Formation"],null,[],300,300)}</div>
    </div>`,o==="p1"&&setTimeout(async()=>{await v({phase:"midfield"})},5e3)}let W=!1;function ie(){if(W)return;const m=c[o+"Team"].MIL||[],h=c[r+"Team"].MIL||[];function w(Q){return Q.reduce((Y,se)=>Y+pe(se,"MIL"),0)}function I(Q){let Y=0;for(let se=0;se<Q.length-1;se++){const de=Oe(Q[se],Q[se+1]);de==="#00ff88"?Y+=2:de==="#FFD700"&&(Y+=1)}return Y}const b=w(m)+I(m),M=w(h)+I(h),z=b>=M;function C(Q,Y,se){return`<div id="duel-row-${se}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0),opacity .5s ease;transform-origin:center">
        ...
        <div class="duel-score-line duel-score-line-${se}" style="margin-top:10px;font-size:12px;color:rgba(255,255,255,0.55);opacity:0;transition:opacity .4s ease">
          Score: ${w(Q)} + ${I(Q)} liens = <b style="color:#fff">${w(Q)+I(Q)}</b>
        </div>
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      ${C(m,c[o+"Name"]||"Vous","me")}
      <div style="display:flex;flex-direction:column;align-items:center;gap:2px;margin:4px 0">
        <div id="pvp-score-me" style="font-size:48px;font-weight:900;color:#D4A017;transition:all .5s ease">0</div>
        <div id="pvp-vs" style="font-size:14px;color:rgba(255,255,255,.4);letter-spacing:3px;opacity:0">VS</div>
        <div id="pvp-score-opp" style="font-size:48px;font-weight:900;color:rgba(255,255,255,.7);transition:all .5s ease">0</div>
      </div>
      ${C(h,c[r+"Name"]||"Adversaire","opp")}
      <div id="duel-shock" style="position:absolute;left:50%;top:50%;width:120px;height:120px;border-radius:50%;border:6px solid #FFD700;opacity:0;pointer-events:none"></div>
      <div id="pvp-duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center,rgba(10,61,30,.4),rgba(10,61,30,.92))"></div>
    </div>`;const P=(Q,Y)=>e.querySelectorAll(Q).forEach((se,de)=>{setTimeout(()=>{se.style.opacity="1",se.style.transform="translateY(0) scale(1)"},Y+de*90)});P(".duel-card-me",150),P(".duel-card-opp",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((Q,Y)=>setTimeout(()=>{Q.style.opacity="1"},Y*70)),900),setTimeout(()=>{const Q=e.querySelector("#pvp-vs");Q&&(Q.style.opacity="1",Q.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(Y=>Y.style.opacity="1")},1250);function B(Q,Y,se){const de=document.getElementById(Q);if(!de)return;const we=performance.now(),he=be=>{const ke=Math.min(1,(be-we)/se);de.textContent=Math.round(Y*(1-Math.pow(1-ke,3))),ke<1?requestAnimationFrame(he):de.textContent=Y};requestAnimationFrame(he)}setTimeout(()=>{B("pvp-score-me",b,800),B("pvp-score-opp",M,800)},1500);const U=c.p1Team.MIL||[],H=c.p2Team.MIL||[],re=w(U)+I(U),ge=w(H)+I(H),$e=re>=ge?"p1":"p2";let ce=c.boostValue;ce==null&&(ce=bi(),c.boostValue=ce,c.boostOwner=$e,c.boostUsed=!1),W=!0,setTimeout(()=>{const Q=e.querySelector("#duel-row-"+(z?"me":"opp")),Y=e.querySelector("#duel-row-"+(z?"opp":"me")),se=document.getElementById("pvp-score-me"),de=document.getElementById("pvp-score-opp"),we=z?se:de,he=z?de:se;we&&(we.style.fontSize="80px",we.style.color=z?"#FFD700":"#ff6b6b",we.style.animation="duelPulse .5s ease"+(z?",duelGlow 1.5s ease infinite .5s":"")),he&&(he.style.opacity="0.25"),setTimeout(()=>{Q&&(Q.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",Q.style.zIndex="5"),setTimeout(()=>{const be=document.getElementById("duel-shock");be&&(be.style.animation="shockwave .5s ease-out forwards"),Y&&(Y.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var Se;const be=document.getElementById("pvp-duel-finale");if(!be)return;const ke=c.boostOwner===o?'<div style="background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:3px solid #cdeffd;border-radius:18px;padding:20px 34px;text-align:center;animation:boostFlipIn .7s cubic-bezier(.34,1.56,.64,1) both;box-shadow:0 10px 36px rgba(135,206,235,.5)"><div style="font-size:10px;color:rgba(0,0,0,.6);letter-spacing:2px;text-transform:uppercase;margin-bottom:6px;font-weight:700">Carte Boost obtenue</div><div style="font-size:46px;line-height:1">⚡</div><div style="font-size:50px;font-weight:900;color:#063;line-height:1.1">+'+ce+`</div><div style="font-size:10px;color:rgba(0,0,0,.55);margin-top:4px">Applicable sur n'importe quel joueur</div></div>`:"",Be=o==="p1"?'<button id="pvp-start-match" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">▶ Commencer le match</button>':`<div style="font-size:14px;color:rgba(255,255,255,0.5);text-align:center;margin-top:8px;animation:fadeUp .4s ease both">⏳ En attente de l'adversaire...</div>`;be.innerHTML='<div style="font-size:22px;font-weight:900;color:#fff;text-align:center;animation:fadeUp .4s ease both;text-shadow:0 2px 12px rgba(0,0,0,.5)">'+(z?"⚽ "+c[o+"Name"]+"<br>gagne le milieu et attaque !":"😔 "+c[r+"Name"]+"<br>gagne l'engagement et attaque !")+"</div>"+ke+Be,be.style.transition="opacity .45s ease",be.style.opacity="1",be.style.pointerEvents="auto",(Se=document.getElementById("pvp-start-match"))==null||Se.addEventListener("click",async()=>{const Le=$e;await v({phase:Le+"-attack",attacker:Le,round:1,boostValue:ce,boostUsed:!1,boostOwner:Le})})},600)},700)},2800)}function oe(m,h,w,I,b){const M=document.createElement("div");M.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:900;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;overflow:hidden;cursor:pointer";const z=Array.from({length:10},(U,H)=>`<div style="position:absolute;font-size:${16+Math.floor(Math.random()*24)}px;top:${5+Math.floor(Math.random()*65)}%;left:${3+Math.floor(Math.random()*94)}%;animation:fw${H%2===0?"A":"B"} ${.7+Math.random()*.7}s ease ${Math.random()*.9}s both;opacity:0">${["✨","🌟","⭐","💥","🎇","🎆","🔥","🌈"][H%8]}</div>`).join(""),C={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};M.innerHTML=`
      <style>
        @keyframes butPop{0%{transform:scale(0) rotate(-8deg);opacity:0}55%{transform:scale(1.25) rotate(2deg)}85%{transform:scale(0.92) rotate(-1deg)}100%{transform:scale(1);opacity:1}}
        @keyframes ballIn{0%{transform:translate(-70px,18px);opacity:0}65%{opacity:1}100%{transform:translate(26px,-8px);opacity:1}}
        @keyframes scoreIn{from{opacity:0;transform:translateY(-12px)}to{opacity:1;transform:translateY(0)}}
      ...
      ${m!=null&&m.length?`<div style="display:flex;gap:10px;animation:scoreIn .4s ease 1s both;position:relative;z-index:1">
        ${m.map(U=>`<div style="text-align:center">
          <div style="width:50px;height:50px;border-radius:50%;background:${C[U.job]||"#555"};border:2px solid #FFD700;position:relative;overflow:hidden;margin:0 auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(M);let P=!1;const B=()=>{P||(P=!0,M.remove(),setTimeout(()=>b(),50))};M.addEventListener("click",B),setTimeout(B,3500)}function ee(m,h){var re,ge,$e,ce;const I=(c["gcCardsFull_"+o]||[]).find(Q=>Q.id===m),b=I==null?void 0:I._gcDef,M={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[b==null?void 0:b.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",z={purple:"#b06ce0",light_blue:"#00d4ef"}[b==null?void 0:b.color]||"#b06ce0",C=(b==null?void 0:b.name)||h,P=(b==null?void 0:b.effect)||((re=Ie[h])==null?void 0:re.desc)||"Carte spéciale.",B=b!=null&&b.image_url?`/manager-wars/icons/${b.image_url}`:null,U=((ge=Ie[h])==null?void 0:ge.icon)||"⚡",H=document.createElement("div");H.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",H.innerHTML=`
      <div style="width:190px;border-radius:16px;border:3px solid ${z};background:${M};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 40px ${z}66">
        <div style="padding:10px;background:rgba(255,255,255,0.14);text-align:center">
      ...
      </div>`,document.body.appendChild(H),($e=H.querySelector("#pvp-gc-back"))==null||$e.addEventListener("click",()=>H.remove()),(ce=H.querySelector("#pvp-gc-use"))==null||ce.addEventListener("click",()=>{H.remove(),J(m,h)})}function le(){var I;const m=c[o+"Team"],h=Object.entries(m).flatMap(([b,M])=>(M||[]).filter(z=>!z.used).map(z=>({...z,_line:b})));if(!h.length)return;const w=document.createElement("div");w.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",w.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
        <div style="flex:1;font-size:14px;font-weight:700;color:#fff">⚡ Choisir un joueur pour +${c.boostValue}</div>
        <button id="bp-close" style="background:none;border:none;color:rgba(255,255,255,0.5);font-size:22px;cursor:pointer">✕</button>
      </div>
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${h.map(b=>{const M={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[b._line]||"#555",z=pe(b,b._line)+(b.boost||0);return`<div class="bp-item" data-cid="${b.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${M};overflow:hidden;cursor:pointer">
      ...
      </div>`,document.body.appendChild(w),(I=w.querySelector("#bp-close"))==null||I.addEventListener("click",()=>w.remove()),w.querySelectorAll(".bp-item").forEach(b=>{b.addEventListener("click",async()=>{const M=b.dataset.cid,z=h.find(P=>P.cardId===M);if(!z)return;const C=(m[z._line]||[]).find(P=>P.cardId===M);C&&(C.boost=(C.boost||0)+c.boostValue),w.remove(),await v({[o+"Team"]:m,boostUsed:!0})})})}function ue(m=null){var ge,$e;if(!(c.phase===o+"-attack")){p("Remplacement uniquement avant votre attaque","warning");return}const w=c[o+"Team"],I=c["usedSubIds_"+o]||[],b=c.maxSubs||3;if(I.length>=b){p(`Maximum ${b} remplacements atteint`,"warning");return}const M=Object.entries(w).flatMap(([ce,Q])=>(Q||[]).filter(Y=>Y.used).map(Y=>({...Y,_line:ce}))),z=(c[o+"Subs"]||[]).filter(ce=>!I.includes(ce.cardId));if(!M.length){p("Aucun joueur utilisé à remplacer","warning");return}if(!z.length){p("Aucun remplaçant disponible","warning");return}let C=Math.max(0,M.findIndex(ce=>ce.cardId===m));const P=((ge=M[C])==null?void 0:ge._line)||(($e=M[C])==null?void 0:$e.job);let B=Math.max(0,z.findIndex(ce=>ce.job===P)),U=!1;const H=document.createElement("div");H.id="pvp-sub-overlay",H.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function re(){var he,be,ke,Be,Se,Le;const ce=M[C],Q=z[B],Y=Math.min(130,Math.round((window.innerWidth-90)/2)),se=Math.round(Y*1.35),de=_e=>`background:rgba(255,255,255,0.12);border:none;color:${_e?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${_e?"default":"pointer"};flex-shrink:0`;H.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
        <div style="flex:1;font-size:15px;font-weight:900;color:#fff">🔄 Remplacement (${I.length}/${b})</div>
        <button id="psub-close" style="background:none;border:none;color:rgba(255,255,255,0.5);font-size:24px;cursor:pointer;padding:0">✕</button>
      </div>
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        <div id="pin-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
          <div style="font-size:9px;color:#00ff88;letter-spacing:2px;text-transform:uppercase;font-weight:700">Joueur qui entre</div>
          <button id="pin-up" style="${de(B===0)}" ${B===0?"disabled":""}>▲</button>
          <div>${Q?Te({...Q,used:!1,boost:0},Y,se):"<div>—</div>"}</div>
          <button id="pin-down" style="${de(B>=z.length-1)}" ${B>=z.length-1?"disabled":""}>▼</button>
          <div style="font-size:10px;color:rgba(255,255,255,0.35)">${B+1}/${z.length}</div>
        </div>
        <div id="pout-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
          <div style="font-size:9px;color:#ff6b6b;letter-spacing:2px;text-transform:uppercase;font-weight:700">Joueur qui sort</div>
          <button id="pout-up" style="${de(C===0)}" ${C===0?"disabled":""}>▲</button>
          <div>${ce?Te({...ce,used:!1,boost:0},Y,se):"<div>—</div>"}</div>
          <button id="pout-down" style="${de(C>=M.length-1)}" ${C>=M.length-1?"disabled":""}>▼</button>
          <div style="font-size:10px;color:rgba(255,255,255,0.35)">${C+1}/${M.length}</div>
        </div>
      </div>
      <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
        <button id="psub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
      </div>`,(he=H.querySelector("#psub-close"))==null||he.addEventListener("click",()=>H.remove()),(be=H.querySelector("#pout-up"))==null||be.addEventListener("click",()=>{C>0&&(C--,re())}),(ke=H.querySelector("#pout-down"))==null||ke.addEventListener("click",()=>{C<M.length-1&&(C++,re())}),(Be=H.querySelector("#pin-up"))==null||Be.addEventListener("click",()=>{B>0&&(B--,re())}),(Se=H.querySelector("#pin-down"))==null||Se.addEventListener("click",()=>{B<z.length-1&&(B++,re())});const we=(_e,Me,Ae,Fe)=>{const je=H.querySelector("#"+_e);if(!je)return;let Pe=0;je.addEventListener("touchstart",Ce=>{Pe=Ce.touches[0].clientY},{passive:!0}),je.addEventListener("touchend",Ce=>{const Ee=Ce.changedTouches[0].clientY-Pe;if(Math.abs(Ee)<30)return;const De=Me();Ee<0&&De<Fe-1?(Ae(De+1),re()):Ee>0&&De>0&&(Ae(De-1),re())},{passive:!0})};we("pin-panel",()=>B,_e=>B=_e,z.length),we("pout-panel",()=>C,_e=>C=_e,M.length),(Le=H.querySelector("#psub-confirm"))==null||Le.addEventListener("click",async _e=>{if(_e.preventDefault(),_e.stopPropagation(),U)return;U=!0;const Me=M[C],Ae=z[B];if(!Me||!Ae)return;const Fe=Me._line,je=(w[Fe]||[]).findIndex(Ee=>Ee.cardId===Me.cardId);if(je===-1){p("Erreur : joueur introuvable","error"),H.remove();return}const Pe={...Ae,_line:Fe,position:Me.position,used:!1,boost:0};w[Fe].splice(je,1,Pe);const Ce=[...I,Ae.cardId];H.remove(),L(Me,Ae,async()=>{await v({[o+"Team"]:w,[r+"Team"]:c[r+"Team"],["usedSubIds_"+o]:Ce})})})}document.body.appendChild(H),re()}function L(m,h,w){const I={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const M=(P,B,U)=>`<div style="text-align:center">
      <div style="font-size:9px;color:${B};letter-spacing:2px;text-transform:uppercase;font-weight:700;margin-bottom:6px">${U}</div>
      <div style="width:70px;height:70px;border-radius:50%;background:${I[P.job]||"#555"};border:3px solid ${B};position:relative;overflow:hidden;margin:0 auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:6px">Appuyer pour continuer</div>`,document.body.appendChild(b);let z=!1;const C=()=>{z||(z=!0,b.remove(),setTimeout(()=>w(),50))};b.addEventListener("click",C),setTimeout(C,2200)}function k(){var h;const m=document.createElement("div");m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:20px;overflow-y:auto",m.innerHTML=`
      ...
      <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
        ${m.length===0?'<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action</div>':[...m].reverse().map(I=>`<div style="padding:8px 10px;border-radius:8px;background:rgba(255,255,255,0.04);border-left:3px solid ${I.type==="goal"?"#FFD700":I.type==="stop"?"#00ff88":"rgba(255,255,255,0.5)"}"><div style="font-size:12px;color:#fff">${I.text||""}</div></div>`).join("")}
      </div>`,document.body.appendChild(h),(w=h.querySelector("#pvp-hist-close"))==null||w.addEventListener("click",()=>h.remove())}async function N(){const m=c[o+"Team"],h=!["GK","DEF","MIL","ATT"].some(M=>(c[r+"Team"][M]||[]).some(z=>!z.used)),w=(c["selected_"+o]||[]).map(M=>{const z=(m[M._role]||[]).find(P=>P.cardId===M.cardId)||M,C=h&&["GK","DEF"].includes(M._role);return{...z,_line:M._role,...C?{note_a:Math.max(1,Number(z.note_a)||0)}:{}}});if(!w.length)return;const I=St(w,c.modifiers[o]||{});$(o,w.map(M=>M.cardId)),w.forEach(M=>{const z=(m[M._role]||[]).find(C=>C.cardId===M.cardId);z&&(z.used=!0)}),c["selected_"+o]=[],F();const b=[...c.log||[]];if(h){const M=(c[o+"Score"]||0)+1,z=w.map(H=>({name:H.name,note:pe(H,H._line||"ATT"),portrait:ve(H),job:H.job}));b.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:z,homeTotal:I.total,aiTotal:0});const C=(c.round||0)+1,P=o==="p1"?"p2":"p1",B={...c,[o+"Team"]:m,[o+"Score"]:M},U=ne(B);j.add(C),oe(z,M,c[r+"Score"]||0,!0,async()=>{await v({[o+"Team"]:m,[o+"Score"]:M,["selected_"+o]:[],modifiers:{...c.modifiers,[o]:{}},pendingAttack:null,phase:U?"finished":P+"-attack",attacker:P,round:C,log:b}),U&&await S.from("matches").update({status:"finished",winner_id:K({...c,[o+"Score"]:M})}).eq("id",i)});return}b.push({type:"pending",text:`⚔️ ${c[o+"Name"]} attaque (${I.total})`}),await v({[o+"Team"]:m,[r+"Team"]:c[r+"Team"],pendingAttack:{...I,players:w,side:o},["selected_"+o]:[],modifiers:{...c.modifiers,[o]:{}},phase:r+"-defense",log:b})}async function V(){const m=c[o+"Team"],h=(c["selected_"+o]||[]).map(Y=>({...(m[Y._role]||[]).find(de=>de.cardId===Y.cardId)||Y,_line:Y._role})),w=jt(h,c.modifiers[o]||{});$(o,h.map(Y=>Y.cardId)),h.forEach(Y=>{const se=(m[Y._role]||[]).find(de=>de.cardId===Y.cardId);se&&(se.used=!0)}),c["selected_"+o]=[],F();const I=Ct(c.pendingAttack.total,w.total,c.modifiers[o]||{}),b=c.pendingAttack.side,M=I==="attack"||(I==null?void 0:I.goal),z=b==="p1"?"p2":"p1",C=(c.round||0)+1,P=(c.pendingAttack.players||[]).map(Y=>({name:Y.name,note:pe(Y,Y._line||"ATT"),portrait:ve(Y),job:Y.job})),B=[...c.log||[]];B.push({type:"duel",isGoal:M,homeScored:M&&b===o,text:M?`⚽ BUT de ${c[b+"Name"]} ! (${c.pendingAttack.total} vs ${w.total})`:`✋ Attaque stoppée (${c.pendingAttack.total} vs ${w.total})`,homePlayers:P,aiPlayers:h.map(Y=>({name:Y.name,note:pe(Y,Y._line||"DEF"),portrait:ve(Y),job:Y.job})),homeTotal:c.pendingAttack.total,aiTotal:w.total});const U=M?(c[b+"Score"]||0)+1:c[b+"Score"]||0,H={...c,[o+"Team"]:m,[b+"Score"]:U},re=ne(H),ge=H[z+"Team"],ce=!["GK","DEF","MIL","ATT"].some(Y=>(ge[Y]||[]).some(se=>!se.used))||re?"finished":z+"-attack",Q=async()=>{await v({[o+"Team"]:m,[r+"Team"]:c[r+"Team"],[b+"Score"]:U,["selected_"+o]:[],modifiers:{...c.modifiers,[o]:{}},pendingAttack:null,phase:ce,attacker:z,round:C,log:B}),(ce==="finished"||re)&&await S.from("matches").update({status:"finished",winner_id:K({...c,[b+"Score"]:U})}).eq("id",i)};if(M){const Y=b===o,se=Y?U:c[o+"Score"]||0,de=Y?c[r+"Score"]||0:U;j.add(C),oe(P,se,de,Y,Q)}else await Q()}function ne(m){const h=["MIL","ATT","GK","DEF"].some(I=>(m.p1Team[I]||[]).some(b=>!b.used)),w=["MIL","ATT","GK","DEF"].some(I=>(m.p2Team[I]||[]).some(b=>!b.used));return!h&&!w}function K(m){const h=m.p1Score||0,w=m.p2Score||0;return h===w?null:h>w?g.home_id:g.away_id}function me(){var b;const m=c[o+"Score"],h=c[r+"Score"],w=m>h,I=m===h;qe(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      <div style="font-size:64px">${w?"🏆":I?"🤝":"😤"}</div>
      <div style="font-size:24px;font-weight:900;color:#fff">${w?"Victoire !":I?"Match nul":"Défaite"}</div>
      <div style="font-size:32px;font-weight:900;color:#FFD700">${m} - ${h}</div>
      <button id="pvp-home" style="padding:16px 40px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">🏠 Retour</button>
    </div>`,(b=document.getElementById("pvp-home"))==null||b.addEventListener("click",()=>{try{S.removeChannel(A)}catch{}ze(e),l("home")})}F()}const Do={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"};async function Go(e,t){const{state:i,toast:n}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement du marché...</div>',await Gt(e,t)}async function Gt(e,t){const{state:i,toast:n}=t,{data:d}=await S.from("market_listings").select(`id, price, status, listed_at, seller_id,
  ...
  `;function p(o){const r=document.getElementById("mkt-content"),u=o==="buy"?a:l;if(u.length===0){r.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      ${f.map(g=>{var j,D,A,v;const T=(j=g.card)==null?void 0:j.player;if(!T)return"";const c=T.job==="GK"?T.note_g:T.job==="DEF"?T.note_d:T.job==="MIL"?T.note_m:T.note_a,x=Do[T.rarity],_=(i.profile.credits||0)>=g.price,E=g.status==="sold";return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${E?"opacity:0.65":""}">
          ...
          <div style="flex:1;min-width:0">
            ...
            ${o==="buy"?`<div style="font-size:11px;color:var(--gray-600)">Vendeur : ${((A=g.seller)==null?void 0:A.pseudo)||"—"}</div>`:E?`<div style="font-size:11px;color:var(--green)">✅ Vendu à ${((v=g.buyer)==null?void 0:v.pseudo)||"—"} · ${g.sold_at?new Date(g.sold_at).toLocaleDateString("fr"):""}</div>`:`<div style="font-size:11px;color:var(--gray-600)">🟢 En vente depuis le ${new Date(g.listed_at).toLocaleDateString("fr")}</div>`}
          </div>
