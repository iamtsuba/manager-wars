...
import{s as w,l as Gt,m as Ii,F as Si,r as Se,j as Jt,h as Mi,n as jn,o as Cn,T as gi,k as Bn,p as qn,b as Dn}from"./formation-links-yJiMLlPw.js";const Fn="/";function Ni(e,{navigate:t,toast:i}){let n="login";const r=()=>{var a,s,l,c,d,u;const o=n==="login";e.innerHTML=`
    <div style="
    ...
    ">
      ...
      <div style="margin-bottom:28px;text-align:center;animation:authFadeIn .5s ease">
        <img src="${Fn}icons/logo-withname.png" alt="Manager Wars" style="height:72px;width:auto;filter:drop-shadow(0 4px 16px rgba(212,160,23,0.4))">
    ...
    </style>`,(a=document.getElementById("tab-login-btn"))==null||a.addEventListener("click",()=>{n="login",r()}),(s=document.getElementById("tab-reg-btn"))==null||s.addEventListener("click",()=>{n="register",r()}),o?((l=document.getElementById("login-password"))==null||l.addEventListener("keydown",x=>{var m;x.key==="Enter"&&((m=document.getElementById("login-btn"))==null||m.click())}),(c=document.getElementById("login-btn"))==null||c.addEventListener("click",async()=>{const x=document.getElementById("login-email").value.trim(),m=document.getElementById("login-password").value,f=document.getElementById("login-error");if(f.textContent="",!x||!m){f.textContent="Remplissez tous les champs.";return}const p=document.getElementById("login-btn");p.textContent="⏳ Connexion…",p.disabled=!0;const{error:g}=await w.auth.signInWithPassword({email:x,password:m});if(p.textContent="⚽ Se connecter",p.disabled=!1,g){f.textContent=g.message.includes("Invalid")?"Email ou mot de passe incorrect.":g.message;return}window.location.reload()})):((d=document.getElementById("reg-confirm"))==null||d.addEventListener("keydown",x=>{var m;x.key==="Enter"&&((m=document.getElementById("reg-btn"))==null||m.click())}),(u=document.getElementById("reg-btn"))==null||u.addEventListener("click",async()=>{const x=document.getElementById("reg-email").value.trim(),m=document.getElementById("reg-password").value,f=document.getElementById("reg-confirm").value,p=document.getElementById("reg-error");if(p.textContent="",!x||!m||!f){p.textContent="Remplissez tous les champs.";return}if(m.length<6){p.textContent="Mot de passe trop court (min. 6 caractères).";return}if(m!==f){p.textContent="Les mots de passe ne correspondent pas.";return}const g=document.getElementById("reg-btn");g.textContent="⏳ Création…",g.disabled=!0;const{error:h}=await w.auth.signUp({email:x,password:m});if(g.textContent="🚀 Créer mon compte",g.disabled=!1,h){p.textContent=h.message;return}i("Compte créé ! Connecte-toi pour commencer.","success",4e3),n="login",r(),setTimeout(()=>{const _=document.getElementById("login-email");_&&(_.value=x)},50)}))};r()}function Pn(e,{state:t,navigate:i,toast:n,refreshProfile:r}){let o="#1A6B3C",a="#D4A017";e.innerHTML=`
  ...
  `;function s(){var f;const c=document.getElementById("logo-preview"),d=document.getElementById("logo-initials"),u=((f=document.getElementById("setup-club"))==null?void 0:f.value)||"MW",x=u.trim().split(" ").filter(Boolean),m=x.length>=2?(x[0][0]+x[1][0]).toUpperCase():u.slice(0,2).toUpperCase();c&&(c.style.background=a,c.style.borderColor=o),d&&(d.textContent=m,d.style.color=o)}document.getElementById("color1").addEventListener("input",c=>{o=c.target.value,document.getElementById("swatch1").style.background=o,s()}),document.getElementById("color2").addEventListener("input",c=>{a=c.target.value,document.getElementById("swatch2").style.background=a,s()});function l(c){document.querySelectorAll(".setup-step").forEach(d=>d.classList.remove("active")),document.getElementById(`step-${c}`).classList.add("active"),document.getElementById("step-num").textContent=c,document.getElementById("progress-fill").style.width=`${Math.round(c/3*100)}%`,c===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),d=document.getElementById("step1-error");if(d.textContent="",c.length<3){d.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:u}=await w.from("users").select("id").eq("pseudo",c).maybeSingle();if(u){d.textContent="Ce pseudo est déjà pris.";return}l(2)}),document.getElementById("step2-back").addEventListener("click",()=>l(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const c=document.getElementById("setup-club").value.trim(),d=document.getElementById("step2-error");if(d.textContent="",c.length<2){d.textContent="Nom trop court (min. 2 caractères).";return}const{data:u}=await w.from("users").select("id").eq("club_name",c).maybeSingle();if(u){d.textContent="Ce nom de club est déjà pris.";return}l(3)}),document.getElementById("step3-back").addEventListener("click",()=>l(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),d=document.getElementById("setup-club").value.trim(),u=document.getElementById("step3-error"),x=document.getElementById("step3-finish");u.textContent="",x.disabled=!0,x.textContent="Création en cours…";try{const{error:m}=await w.from("users").insert({id:t.user.id,pseudo:c,club_name:d,club_color1:o,club_color2:a,credits:1e4});if(m)throw m;await Gn(t.user.id),await r(),n(`Bienvenue ${c} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(m){u.textContent=m.message,x.disabled=!1,x.textContent="🚀 Créer mon profil !"}})}async function Gn(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await w.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const Rn="modulepreload",Nn=function(e){return"/"+e},Oi={},rn=function(t,i,n){let r=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),s=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));r=Promise.allSettled(i.map(l=>{if(l=Nn(l),l in Oi)return;Oi[l]=!0;const c=l.endsWith(".css"),d=c?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${l}"]${d}`))return;const u=document.createElement("link");if(u.rel=c?"stylesheet":Rn,c||(u.as="script"),u.crossOrigin="",u.href=l,s&&u.setAttribute("nonce",s),document.head.appendChild(u),c)return new Promise((x,m)=>{u.addEventListener("load",x),u.addEventListener("error",()=>m(new Error(`Unable to preload CSS for ${l}`)))})}))}function o(a){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=a,window.dispatchEvent(s),!s.defaultPrevented)throw a}return r.then(a=>{for(const s of a||[])s.status==="rejected"&&o(s.reason);return t().catch(o)})},Ut="#1A6B3C",Kt="#cc2222",On="#D4A017",Hi="#888";async function Hn(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    </div>`,await an(i,n,t),document.getElementById("btn-add-friend").addEventListener("click",()=>Kn(i,n)),document.getElementById("btn-accept-friend").addEventListener("click",()=>sn(i,n,null,t))}async function an(e,t,i={}){const{navigate:n}=i,r=e.user.id,{data:o,error:a}=await w.from("friendships").select("id, requester_id, addressee_id").eq("status","accepted").or(`requester_id.eq.${r},addressee_id.eq.${r}`),{count:s}=await w.from("friendships").select("id",{count:"exact",head:!0}).eq("addressee_id",r).eq("status","pending"),l=document.getElementById("pending-badge");l&&(s>0?(l.style.display="flex",l.textContent=s):l.style.display="none");const c=document.getElementById("friends-list");if(!c)return;if(a){console.error("[Friends] Erreur:",a),c.innerHTML=`<div style="color:${Kt};text-align:center;padding:16px">Erreur chargement : ${a.message}</div>`;return}const d=(o||[]).map(m=>m.requester_id===r?m.addressee_id:m.requester_id);let u={};if(d.length){const{data:m}=await w.from("users").select("id, pseudo, club_name, last_seen_at, club_color1, club_color2").in("id",d);(m||[]).forEach(f=>{u[f.id]=f})}const x=(o||[]).map(m=>({friendshipId:m.id,friend:u[m.requester_id===r?m.addressee_id:m.requester_id]||{pseudo:"?"}}));if(!x.length){c.innerHTML=`
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      ${x.map(({friendshipId:m,friend:f})=>Un(f,m)).join("")}
    </div>`,c.querySelectorAll("[data-stats]").forEach(m=>{m.addEventListener("click",()=>Vn(e,m.dataset.stats,m.dataset.friendName))}),c.querySelectorAll("[data-match]").forEach(m=>{m.addEventListener("click",async()=>{const f=m.dataset.friendId,p=m.dataset.friendName;if(typeof n!="function"){t("Erreur navigation","error");return}const g=e.user.id,{data:h}=await w.from("friend_match_invites").select("id").eq("inviter_id",f).eq("invitee_id",g).eq("status","pending").order("created_at",{ascending:!1}).limit(1).maybeSingle(),_=!!h;console.log("[Friends] clic match",{fid:f,fname:p,isAccepting:_}),n("match",{matchMode:"friend",friendId:f,friendName:p,isAccepting:_})})})}function Un(e,t){const i=e.club_name||e.pseudo||"?",n=e.pseudo||"",r=(n||i).slice(0,2).toUpperCase(),o=e.club_color2||Ut,a=e.club_color1||"#ffffff",s=e.last_seen_at?new Date(e.last_seen_at):null,l=s&&Date.now()-s.getTime()<3*60*1e3;return`
    <div style="display:flex;align-items:center;gap:12px;background:#fff;border-radius:12px;padding:12px 14px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
      ...
      <div style="display:flex;gap:8px;flex-shrink:0">
        ...
        <button data-stats="${e.id}" data-friend-name="${i}" title="Voir les stats"
          style="width:38px;height:38px;border-radius:50%;border:2px solid ${On};background:#fff;font-size:16px;cursor:pointer;display:flex;align-items:center;justify-content:center">📊</button>
      </div>
    </div>`}function Kn(e,t){const i=Ai();i.innerHTML=`
    <div class="popup-box">
    ...
    <div class="popup-box">
      ...
      ${d.length?`<div style="display:flex;flex-direction:column;gap:8px;max-height:50vh;overflow-y:auto;margin-bottom:14px">
            ${d.map(x=>{var m,f,p;return`
              <div style="display:flex;align-items:center;gap:10px;background:#f9f9f9;border-radius:10px;padding:10px 12px">
                <div style="flex:1;font-size:14px;font-weight:700">${((m=x.requester)==null?void 0:m.club_name)||((f=x.requester)==null?void 0:f.pseudo)||"?"}
    ...
    <div class="popup-box" style="max-width:380px">
      ...
      ${h===0?`
        ...
        ${_("Défaites",u,d)}
        ${_("Buts marqués",m,f)}
        ${_("Buts encaissés",f,m,Kt,Ut)}
        ${_("GC utilisés ⚡",p,g,"#7a28b8","#7a28b8")}
        <div style="text-align:center;font-size:12px;color:#aaa;padding-top:8px">${h} match${h>1?"s":""} joué${h>1?"s":""}</div>`}
      <button id="stats-close" class="popup-btn-cancel" style="width:100%;margin-top:14px">Fermer</button>
    </div>
    ${ji()}`,document.body.appendChild(b),b.querySelector("#stats-close").addEventListener("click",()=>b.remove()),b.addEventListener("click",$=>{$.target===b&&b.remove()})}function Ai(){const e=document.createElement("div");return e.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:8000;display:flex;align-items:center;justify-content:center;padding:20px",e}function ji(){return`
    <style>
      .popup-box {
    ...
    </style>`}const Yn="2026.07.19-0021";async function Ui(e,{state:t,navigate:i,toast:n}){var o,a;const r=t.profile;r&&(e.innerHTML=`
  <style>
    .home-dark {
      min-height: 100%;
  ...
  <div class="home-dark" id="home-dark">
    ...
    ${Se(e,{width:140})}
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),c=(s||[]).filter(M=>M.card_type==="player"&&M.player),d=(s||[]).filter(M=>M.card_type==="game_changer"),u=(s||[]).filter(M=>M.card_type==="formation"),x=(s||[]).filter(M=>M.card_type==="stadium"),{data:m}=await w.from("gc_definitions").select("name,gc_type,color,effect,image_url"),f={};(m||[]).forEach(M=>{f[M.name]=M});const{data:p}=await w.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),g={};(p||[]).forEach(M=>{g[M.id]=M}),x.forEach(M=>{M.stadium_def&&(g[M.stadium_id]=M.stadium_def)});const h=Object.keys(Si),_=Object.keys(Ae),b={};c.forEach(M=>{const C=M.player.id;b[C]=(b[C]||0)+1}),new Set(Object.keys(b).map(M=>String(M)));const $=new Set(u.map(M=>M.formation)),y=new Set(d.map(M=>M.gc_type));let j="player",E="Tous",P="",N=!1;function ne(){return[...c].sort((M,C)=>{const F=Zt.indexOf(M.player.job),X=Zt.indexOf(C.player.job);return F!==X?F-X:(M.player.surname_real||"").localeCompare(C.player.surname_real||"")})}function ie(){return[...l||[]].sort((M,C)=>{const F=Zt.indexOf(M.job),X=Zt.indexOf(C.job);return F!==X?F-X:(M.surname_real||"").localeCompare(C.surname_real||"")})}function Q(){return ne().filter(M=>{const C=M.player,F=E==="Tous"||C.job===E,X=!P||`${C.firstname} ${C.surname_real}`.toLowerCase().includes(P);return F&&X})}function K(){return ie().filter(M=>{const C=E==="Tous"||M.job===E,F=!P||`${M.firstname} ${M.surname_real}`.toLowerCase().includes(P);return C&&F})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",C=>{P=C.target.value.toLowerCase(),I()}),e.querySelectorAll(".filter-btn").forEach(C=>{C.addEventListener("click",()=>{E=C.dataset.job,v(),I()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{N=!N,v(),I()})):(M.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{N=!N,v(),I()})))}function I(){const M=document.getElementById("col-grid");M&&(j==="player"?ke(M):j==="formation"?W(M):j==="stadium"?R(M):J(M))}function G(M,C,F,X,V){const q=document.getElementById("col-grid"),O=document.getElementById("col-big");if(!q||!O)return;var we=0;function _e(){const ye=window.innerWidth>=768,be=document.getElementById("col-big"),$e=document.getElementById("col-grid");ye&&be&&(be.style.minHeight="420px",be.style.flex="1 1 auto"),ye&&$e&&($e.style.height=Math.round(310*.76+16)+"px",$e.style.flexShrink="0",$e.style.overflowX="auto",$e.style.overflowY="hidden",$e.style.alignItems="center",$e.style.padding="8px 16px"),O.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+C(M[we])+"</div>";var Le=O.querySelector("[data-card-id],[data-form-id],[data-gc-id]");Le&&Le.addEventListener("click",function(){X(M[we])}),requestAnimationFrame(function(){var ge=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!ge||!O)){var Be=O.clientHeight-8,je=O.clientWidth-24,Oe=ge.offsetHeight,bt=ge.offsetWidth;if(Oe>0&&bt>0&&Be>40){var Xe=ye?2.2:1.6,Rt=Math.min(Be/Oe,je/bt,Xe);ge.style.transform="scale("+Rt.toFixed(3)+")",ge.style.transformOrigin="top center"}}}),q.innerHTML=M.map(function(ge,Be){var je=Be===we,Oe="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+(je?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+Be+'" style="'+Oe+'">'+F(ge,je)+"</div>"}).join(""),q.querySelectorAll(".col-mini-item").forEach(function(ge){ge.addEventListener("click",function(){we=Number(ge.dataset.idx),_e(),ge.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}_e()}function re(M){var C=window.innerWidth>=768?.76:.54,F;if(!M||M._fake){var X=M?M.player:null;if(!X)return"";F=Wi(X)}else F=yi(M,"");return'<div style="display:inline-block;zoom:'+C+';pointer-events:none;line-height:0">'+F+"</div>"}function ae(M,C,F){C=C||100,F=F||140;var X=Jt[M]||{},V={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},q=Math.max(3,Math.round(C*.06)),O=Object.entries(X).map(function(_e){var ye=_e[0],be=_e[1],$e=ye.replace(/\d+$/,""),Le=V[$e]||"#888",ge=Math.round(be.x*C),Be=Math.round(be.y*F);return'<circle cx="'+ge+'" cy="'+Be+'" r="'+q+'" fill="'+Le+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),we=Math.max(1,Math.round(C/50));return'<svg viewBox="0 0 '+C+" "+F+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+C+'" height="'+F+'" fill="#1A6B3C"/><rect x="'+Math.round(C*.2)+'" y="'+Math.round(F*.02)+'" width="'+Math.round(C*.6)+'" height="'+Math.round(F*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+we+'"/><line x1="0" y1="'+Math.round(F*.5)+'" x2="'+C+'" y2="'+Math.round(F*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+we+'"/><ellipse cx="'+Math.round(C*.5)+'" cy="'+Math.round(F*.5)+'" rx="'+Math.round(C*.18)+'" ry="'+Math.round(F*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+we+'"/><rect x="'+Math.round(C*.2)+'" y="'+Math.round(F*.82)+'" width="'+Math.round(C*.6)+'" height="'+Math.round(F*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+we+'"/>'+O+"</svg>"}function se(M,C,F){var X=F>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+F+"</div>":"",V=C?'data-form-id="'+C.id+'"':"",q=M.length>7?14:M.length>5?16:19,O=!!C;return"<div "+V+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(O?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(O?"":"filter:grayscale(1);opacity:0.5")+'">'+X+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(O?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+q+"px;font-weight:900;color:"+(O?"#1A6B3C":"#aaa")+';line-height:1">'+M+'</div></div><div style="flex:1;overflow:hidden;background:'+(O?"#1A6B3C":"#ccc")+'">'+ae(M,140,220)+"</div></div>"}function de(M,C){var F=window.innerWidth>=768?.76:.54,X=140,V=305,q=Math.round(V*.22),O=V-q,we=M.length>7?10:13,_e=ae(M,X,O),ye=C?"1.5px solid #2a7a40":"1px solid #ddd",be=C?"":"filter:grayscale(1);opacity:0.45;",$e=C?"#1A6B3C":"#bbb",Le="#fff";return'<div style="display:inline-block;zoom:'+F+';line-height:0;pointer-events:none"><div style="width:'+X+"px;height:"+V+"px;border-radius:6px;border:"+ye+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+be+'"><div style="height:'+q+"px;background:"+$e+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+we+"px;font-weight:900;color:"+Le+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(X-4)+'px">'+M+'</span></div><div style="height:'+O+'px;overflow:hidden;flex-shrink:0">'+_e+"</div></div></div>"}function ke(M){if(N){const C=K();if(!C.length){M.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const F=C.map(X=>c.find(V=>V.player.id===X.id)||{_fake:!0,player:X,id:"fake-"+X.id});G(F,X=>X._fake?Wi(X.player):yi(X,""),X=>X._fake?re({player:X.player,id:"x",_fake:!0}):re(X),X=>{X._fake||Xi(X,c,b,t)})}else{const C=Q();if(!C.length){M.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const F={};C.forEach(V=>{const q=V.player.id;F[q]||(F[q]=[]),F[q].push(V)});const X=Object.values(F).map(V=>ao(V));G(X,V=>{const q=b[V.player.id]||1,O=q>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${q}</div>`:"",_e=c.filter(ye=>ye.player.id===V.player.id&&ye.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return yi(V,O+_e)},V=>re(V),V=>Xi(V,c,b,t))}}function W(M){const C=N?h:[...$];if(!C.length){M.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const F=C.map(X=>({formation:X,card:u.find(V=>V.formation===X)||null,owned:$.has(X)}));G(F,({formation:X,card:V,owned:q})=>se(X,q?V:null,q?u.filter(O=>O.formation===X).length:0),({formation:X,owned:V})=>de(X,V),({card:X,owned:V})=>{V&&X&&fo(X,u,t,o)})}function J(M){const C=Object.keys(f),F=N?C.length?C:_:[...y];if(!F.length){M.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const X=F.map(V=>({type:V,gc:Ae[V]||{icon:"⚡",desc:""},def:f[V]||null,owned:y.has(V),card:d.find(q=>q.gc_type===V)||null}));G(X,({type:V,gc:q,def:O,owned:we,card:_e})=>{var Xe;const ye=we?d.filter(Rt=>Rt.gc_type===V).length:0,be=ye>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${ye}</div>`:"",$e=(O==null?void 0:O.gc_type)==="ultra_game_changer",Le={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},ge={purple:"#b06ce0",light_blue:"#00d4ef"},Be=Le[O==null?void 0:O.color]||Le.purple,je=ge[O==null?void 0:O.color]||ge.purple,Oe=(O==null?void 0:O.effect)||q.desc||"",bt=O!=null&&O.image_url?`/icons/${O.image_url}`:((Xe=O==null?void 0:O.club)==null?void 0:Xe.logo_url)||(O!=null&&O.country_code?`https://flagsapi.com/${O.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null);return we&&_e?`<div data-gc-id="${_e.id}" data-gc-type="${V}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${je};background:${Be};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${je}66;cursor:pointer">
          ${be}
        ...
        </div>`},null)}e.querySelectorAll(".col-tab-btn").forEach(M=>{M.addEventListener("click",()=>{j=M.dataset.tab,E="Tous",P="",N=!1,e.querySelectorAll(".col-tab-btn").forEach(C=>{const F=C.dataset.tab===j;C.style.borderBottomColor=F?"var(--green)":"transparent",C.style.color=F?"var(--green)":"var(--gray-600)"}),v(),I()})}),v(),I()}function uo(e,t,i){const n=Ae[e]||{icon:"⚡",desc:"Effet spécial."},r=(t==null?void 0:t.gc_type)==="ultra_game_changer",o={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},a={purple:"#b06ce0",light_blue:"#00d4ef"},s=o[t==null?void 0:t.color]||o.purple,l=a[t==null?void 0:t.color]||a.purple,c=(t==null?void 0:t.name)||e,d=(t==null?void 0:t.effect)||n.desc,u=t!=null&&t.image_url?`/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const ei=1e3;function fo(e,t,i,n){var f,p,g;const{state:r,toast:o,closeModal:a,navigate:s,refreshProfile:l}=i,c=e.formation,d={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function u(){const h=Jt[c]||{},_=Si[c]||[],b=290,$=360,y=20;function j(P){const N=h[P];return N?{x:N.x*b,y:N.y*$}:null}let E=`<svg width="${b}" height="${$}" viewBox="0 0 ${b} ${$}" xmlns="http://www.w3.org/2000/svg">`;for(const[P,N]of _){const ne=j(P),ie=j(N);!ne||!ie||(E+=`<line x1="${ne.x}" y1="${ne.y}" x2="${ie.x}" y2="${ie.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const P of Object.keys(h)){const N=j(P);if(!N)continue;const ne=P.replace(/\d+/,""),ie=d[ne]||"#555";E+=`<circle cx="${N.x}" cy="${N.y}" r="${y}" fill="${ie}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,E+=`<text x="${N.x}" y="${N.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${ne}</text>`}return E+="</svg>",E}const x=t.filter(h=>h.formation===c);x.length;const m=!e.is_for_sale;n(`Formation ${c}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(f=document.getElementById("direct-sell-form-btn"))==null||f.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${c} pour ${ei.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const h=x.find(b=>!b.is_for_sale)||x[0];if(!h){o("Aucune carte à vendre","error");return}await w.from("market_listings").delete().eq("card_id",h.id),await w.from("transfer_history").delete().eq("card_id",h.id);const{error:_}=await w.from("cards").delete().eq("id",h.id);if(_){o(_.message,"error");return}await w.from("users").update({credits:(r.profile.credits||0)+ei}).eq("id",r.profile.id),await l(),o(`+${ei.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),a(),s("collection")}),(p=document.getElementById("market-sell-form-btn"))==null||p.addEventListener("click",async()=>{const h=parseInt(document.getElementById("sell-price-form").value);if(!h||h<1){o("Prix invalide","error");return}await w.from("cards").update({is_for_sale:!0,sale_price:h}).eq("id",e.id),await w.from("market_listings").insert({seller_id:r.profile.id,card_id:e.id,price:h}),o("Carte mise en vente sur le marché !","success"),a(),s("collection")}),(g=document.getElementById("cancel-sell-form-btn"))==null||g.addEventListener("click",async()=>{await w.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await w.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),o("Annonce retirée","success"),a(),s("collection")})}async function Xi(e,t,i,n){var re,ae,se,de,ke;const{state:r,toast:o,openModal:a,closeModal:s,navigate:l,refreshProfile:c}=n,d=e.player,u=t.filter(W=>W.player.id===d.id),x=u.length,m=e.evolution_bonus||0,f=Math.max((Number(d.note_g)||0)+(d.job==="GK"||d.job2==="GK"?m:0),(Number(d.note_d)||0)+(d.job==="DEF"||d.job2==="DEF"?m:0),(Number(d.note_m)||0)+(d.job==="MIL"||d.job2==="MIL"?m:0),(Number(d.note_a)||0)+(d.job==="ATT"||d.job2==="ATT"?m:0)),p=d.rarity||"normal",{data:g}=await w.from("sell_price_configs").select("*").eq("rarity",p).lte("note_min",f).gte("note_max",f).order("note_min",{ascending:!1}).limit(1);((re=g==null?void 0:g[0])==null?void 0:re.price)??ro[p];const h=d.rarity!=="legende";co(d);const _=((d.rarity==="pepite"||d.rarity==="papyte")&&e.current_note!=null?e.current_note:xi(d,d.job))+m,b=d.rarity==="pepite"||d.rarity==="papyte",$=d.job2?(b?to(e,so(d.job2)):xi(d,d.job2))+(xi(d,d.job2)>0?m:0):null;mi[d.job],d.job2&&mi[d.job2];const y=no[d.rarity]||"#ccc";lo[d.country_code]||d.country_code;const j=e.evolution_bonus||0,P=_+j,N=$||0,ne=N>0?N+j:0,ie=u.map(W=>W.id);let Q={};if(ie.length){const{data:W}=await w.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",ie).order("transferred_at",{ascending:!0});(W||[]).forEach(J=>{Q[J.card_id]||(Q[J.card_id]=[]),Q[J.card_id].push(J)})}const K=W=>{const J=W.transferred_at?new Date(W.transferred_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"2-digit",year:"numeric"}):"",R=W.source==="booster"?"Booster":W.price?W.price.toLocaleString("fr"):"—";return`
    ...
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${u.map((W,J)=>{const R=Q[W.id]||[],M=W.is_for_sale,C=R.length?R[R.length-1]:null,X=(d.rarity==="pepite"||d.rarity==="papyte")&&W.current_note!=null?` (☆${W.current_note})`:"";return`
            <div data-card-id="${W.id}" data-card-idx="${J}" class="exemplaire-row"
              ...
              <div style="display:flex;align-items:center;gap:10px">
                <input type="checkbox" class="expl-check" data-id="${W.id}" ${M?"disabled":""} style="width:18px;height:18px;cursor:pointer;accent-color:#1A6B3C;flex-shrink:0">
                <div style="flex:1;min-width:0">
                  <div style="font-size:12px;font-weight:700">Exemplaire ${J+1}${X}${M?" 🏷️ En vente":""}</div>
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",d.note_g],["DEF",d.note_d],["MIL",d.note_m],["ATT",d.note_a]].map(([W,J])=>{const R=mi[W],M=W==="GK"?"#fff":R,C=W===d.job||W===d.job2,F=(Number(J)||0)+(C&&j>0?j:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
                <div style="width:48px;height:48px;border-radius:8px;background:#111;border:2.5px solid ${R};
                  display:flex;align-items:center;justify-content:center">
                  <span style="font-size:20px;font-weight:900;color:${M};font-family:Arial Black,Arial;line-height:1">${F}</span>
                </div>
                <span style="font-size:10px;font-weight:700;color:${M}">${W}</span>
              </div>`}).join("")}
          </div>
        </div>
        <div>
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(ae=document.getElementById("close-detail"))==null||ae.addEventListener("click",s);let I=new Set;const G=()=>{const W=I.size,J=document.getElementById("sell-action-panel");if(!J)return;J.style.display=W>0?"block":"none",document.getElementById("sell-selected-count").textContent=W;const R=document.getElementById("evolve-btn");R&&(R.textContent=`⬆️ Évoluer ${W>1?"(+"+W+")":""}`)};document.querySelectorAll(".expl-check").forEach(W=>{W.addEventListener("change",()=>{const J=W.dataset.id;W.checked?I.add(J):I.delete(J);const R=W.closest(".exemplaire-row");R&&(R.style.borderColor=W.checked?"#1A6B3C":"#eee"),G()})}),document.querySelectorAll(".exemplaire-row").forEach(W=>{W.addEventListener("click",J=>{if(J.target.closest("button")||J.target.tagName==="INPUT")return;const R=W.querySelector(".expl-check");R&&!R.disabled&&(R.checked=!R.checked,R.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(W=>{W.addEventListener("click",J=>{J.stopPropagation();const R=document.querySelector(`.expl-hist[data-hist="${W.dataset.idx}"]`);R&&(R.style.display=R.style.display==="none"?"flex":"none")})}),(se=document.getElementById("evolve-btn"))==null||se.addEventListener("click",async()=>{if(x<=1)return;const W=[...I];if(!W.length)return;if(I.has(e.id)){const V=document.createElement("div");V.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",V.innerHTML=`
        ...
        </div>`,document.body.appendChild(q),q.querySelector("#sac-cancel").addEventListener("click",()=>{q.remove(),V(!1)}),q.querySelector("#sac-ok").addEventListener("click",()=>{q.remove(),V(!0)}),q.addEventListener("click",O=>{O.target===q&&(q.remove(),V(!1))})}))return;if(J.length){await w.from("market_listings").delete().in("card_id",J),await w.from("deck_cards").delete().in("card_id",J),await w.from("transfer_history").delete().in("card_id",J),await w.from("decks").update({stadium_card_id:null}).in("stadium_card_id",J);const{error:V}=await w.from("cards").delete().in("id",J);if(V){o("Erreur suppression : "+V.message,"error");return}}const C=(e.evolution_bonus||0)+J.length*R,{error:F}=await w.from("cards").update({evolution_bonus:C}).eq("id",e.id);if(F){o("Erreur évolution : "+F.message,"error");return}const X=_+C;o(`⬆️ ${d.firstname} ${d.surname_real} : note ${_+e.evolution_bonus||_} → ${X}${J.length?` · ${J.length} copie${J.length>1?"s":""} sacrifiée${J.length>1?"s":""}`:""} !`,"success",4e3),s(),l("collection")}),(de=document.getElementById("market-sell-btn"))==null||de.addEventListener("click",async()=>{var F;const W=[...I];if(!W.length){o("Sélectionne au moins un exemplaire","warning");return}const J=parseInt((F=document.getElementById("sell-market-price"))==null?void 0:F.value);if(!J||J<1){o("Prix invalide","error");return}const{error:R}=await w.from("cards").update({is_for_sale:!0,sale_price:J}).in("id",W);if(R){o(R.message,"error");return}const M=W.map(X=>({seller_id:r.profile.id,card_id:X,price:J,status:"active"})),{error:C}=await w.from("market_listings").insert(M);C&&console.warn("[Market] insert listings:",C.message),o(`${W.length} carte${W.length>1?"s":""} mise${W.length>1?"s":""} en vente à ${J.toLocaleString("fr")} cr. chacune !`,"success"),s(),l("collection")}),(ke=document.getElementById("cancel-sell-btn"))==null||ke.addEventListener("click",async()=>{await w.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await w.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),o("Annonce retirée","success"),s(),l("collection")})}function Ji(e,t=""){return new Promise(i=>{const n=document.createElement("div");n.className="modal-overlay",n.style.zIndex="2100",n.innerHTML=`<div class="modal" style="max-width:360px">
  ...
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
    ...
    <div class="page-body">
      ...
      stadium_def:stadium_definitions(id, name, club_id, country_code, image_url,
        club:clubs(encoded_name, logo_url))`).eq("owner_id",n.profile.id).eq("card_type","stadium"),l=(a||[]).filter(h=>h.card_type==="player"&&h.player),c=(a||[]).filter(h=>h.card_type==="formation"),d=(s||[]).filter(h=>h.card_type==="stadium"),u=[...new Set(d.map(h=>h.stadium_id).filter(Boolean))];let x={};if(d.forEach(h=>{h.stadium_def&&h.stadium_id&&(x[h.stadium_id]=h.stadium_def)}),u.length&&Object.keys(x).length<u.length){const{data:h}=await w.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",u);(h||[]).forEach(_=>{x[_.id]=_})}const m=c.map(h=>h.formation).filter(Boolean),{data:f}=await w.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",e);let p=o.formation||"4-4-2";m.length>0&&!m.includes(p)&&(p=m[0]);const g={deckId:e,name:o.name,formation:p,formationCardId:o.formation_card_id,stadiumCardId:o.stadium_card_id||null,slots:{},subs:[],playerCards:l,formationCards:c,stadiumCards:d,stadDefMap:x,availableFormations:m};(f||[]).forEach(h=>{h.is_starter?g.slots[h.position]=h.card_id:g.subs.includes(h.card_id)||g.subs.push(h.card_id)}),xt(t,g,i)}function xt(e,t,i){var f,p;const{navigate:n}=i;ni[t.formation];const r=Zi(t.formation),o=r.filter(g=>t.slots[g]).length,a=t.availableFormations.length>0?t.availableFormations:Object.keys(ni),s=(f=t.stadiumCards)==null?void 0:f.find(g=>g.id===t.stadiumCardId),l=s&&((p=t.stadDefMap)==null?void 0:p[s.stadium_id])||null,c=t.subs.map(g=>t.playerCards.find(h=>h.id===g)).filter(Boolean);[...Object.values(t.slots),...t.subs],e.innerHTML=`
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
      ...
      <div style="display:flex;gap:0;min-height:600px">
        ...
        <div style="width:105px;flex-shrink:0;background:rgba(0,0,0,0.3);display:flex;flex-direction:column;align-items:center;padding:12px 6px;gap:8px;border-right:1px solid rgba(255,255,255,0.1)">
          ...
          <div style="display:flex;flex-direction:column;gap:6px;align-items:center" id="subs-list">
            ${c.map(g=>{const h={...g.player,_evolution_bonus:g.evolution_bonus||0};return`<div style="position:relative;flex-shrink:0;overflow:visible">
        ...
        <div style="width:130px;flex-shrink:0;background:rgba(0,0,0,0.3);display:flex;flex-direction:column;align-items:center;padding:12px 8px;gap:12px;border-left:1px solid rgba(255,255,255,0.1)">
          ...
          <div style="width:100%;text-align:center">
            ...
            <div id="add-stad-btn-pc" style="cursor:pointer;margin:0 auto;width:fit-content">
              ${s?(()=>{var _;const g=t.stadDefMap[s.stadium_id],h=((_=g==null?void 0:g.club)==null?void 0:_.logo_url)||null;return`<div style="width:100px;height:130px;border-radius:8px;background:linear-gradient(135deg,#1a3a6b,#0a1a3a);border:2px solid #4fc3f7;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px">
  ...
  </div>`;const d=window.innerWidth>=900,u=e.querySelector(".deck-pc-layout"),x=e.querySelector(".deck-mobile-layout");u&&(u.style.display=d?"block":"none"),x&&(x.style.display=d?"none":"block"),mo(e,t,r,i),e.querySelectorAll("#builder-back").forEach(g=>g.addEventListener("click",()=>n("decks")));const m=()=>{const{openModal:g,closeModal:h}=i,_=`<div style="display:flex;flex-wrap:wrap;gap:8px;padding:8px">
    ...
    </div>`;g("⚽ Choisir une formation",_),document.querySelectorAll("#modal-body [data-forma]").forEach(b=>{b.addEventListener("click",()=>{t.formation=b.dataset.forma;const $=Zi(t.formation),y={};$.forEach(j=>{t.slots[j]&&(y[j]=t.slots[j])}),t.slots=y,h(),xt(e,t,i)})})};e.querySelectorAll("#formation-mobile-btn, #formation-pc-btn").forEach(g=>g.addEventListener("click",m)),e.querySelectorAll("#add-stad-btn-pc, #add-stad-btn").forEach(g=>g.addEventListener("click",()=>xo(t,e,i))),e.querySelectorAll("#save-deck").forEach(g=>g.addEventListener("click",()=>ho(t,i))),e.querySelectorAll("[data-remove-sub]").forEach(g=>{g.addEventListener("click",()=>{t.subs=t.subs.filter(h=>h!==g.dataset.removeSub),xt(e,t,i)})}),e.querySelectorAll("#add-sub-btn").forEach(g=>g.addEventListener("click",()=>bo(t,e,i)))}function mo(e,t,i,n){var b,$;const r=window.innerWidth>=900,o=e.querySelector(r?"#deck-field-pc":"#deck-field-mobile");if(!o)return;const a=(b=t.stadiumCards)==null?void 0:b.find(y=>y.id===t.stadiumCardId),s=a&&(($=t.stadDefMap)==null?void 0:$[a.stadium_id])||null,l=Jt[t.formation]||{},c=Ii(t.formation)||[],d={};for(const y of i){const j=t.slots[y],E=j?t.playerCards.find(P=>P.id===j):null;E!=null&&E.player?d[y]={...E.player,_evolution_bonus:E.evolution_bonus||0,face:E.player.face||null}:d[y]=null}const u=window.innerWidth>=900,x=u?window.innerWidth-280:window.innerWidth-20,m=u?Math.min(x,860):x,f=Math.round(u?m*.82:m*.85),p=u?84:44;let g="";for(const[y,j]of c){const E=l[y],P=l[j];if(!E||!P)continue;const N=E.x*m,ne=Math.round(.03*f+E.y*.85*f),ie=P.x*m,Q=Math.round(.03*f+P.y*.85*f),K=d[y],v=d[j],I=Gt(K,v);I==="#ff3333"||I==="#cc2222"?g+=`<line x1="${N.toFixed(1)}" y1="${ne.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${Q.toFixed(1)}" stroke="${I}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(g+=`<line x1="${N.toFixed(1)}" y1="${ne.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${Q.toFixed(1)}" stroke="${I}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,g+=`<line x1="${N.toFixed(1)}" y1="${ne.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${Q.toFixed(1)}" stroke="${I}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let h="";const _=Math.round(p*657/507);for(const y of i){const j=l[y];if(!j)continue;const E=d[y],P=j.x*m;j.y*f;const N=Math.round(.03*f+j.y*(.85*f)),ne=Math.round(P-p/2),ie=Math.round(N-_/2);if(E){const Q=y.replace(/\d+/,""),K=s&&(s.club_id&&String(E.club_id)===String(s.club_id)||s.country_code&&E.country_code===s.country_code),v=Se({...E,_evolution_bonus:E._evolution_bonus||0},{width:p,showStad:!0,stadDef:s,role:Q});h+=`<div style="position:absolute;left:${ne}px;top:${ie}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${y}">
        <div style="position:relative">${K?'<div style="position:absolute;top:-18px;left:0;right:0;text-align:center;font-size:14px;z-index:5;line-height:1">🏟️</div>':""}${v}</div>
    ...
    <div style="position:relative;width:${m}px;height:${f}px;margin:0 auto">
      <svg style="position:absolute;inset:0;width:100%;height:100%;pointer-events:none" viewBox="0 0 ${m} ${f}">${g}</svg>
      ${h}
    </div>`,o.querySelectorAll(".deck-slot-hit").forEach(y=>{y.addEventListener("click",()=>yo(y.dataset.pos,t,e,n))})}function xo(e,t,i){var a;const{openModal:n,closeModal:r}=i,o=e.stadiumCards||[];n("🏟️ Choisir un stade",`<div style="display:flex;flex-wrap:wrap;gap:10px;padding:8px">
      ...
      ${o.map(s=>{var h,_;const l=e.stadDefMap[s.stadium_id],c=((h=l==null?void 0:l.club)==null?void 0:h.logo_url)||null,d=l!=null&&l.image_url?"/icons/"+l.image_url:null,u=e.stadiumCardId===s.id,x=l!=null&&l.country_code&&!c?`https://flagsapi.com/${l.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,m=(l==null?void 0:l.name)||"Stade",f=m.match(/^(Stade\s+(?:de\s+)?(?:\w+)?)(.*)?$/i),p=f?f[1].trim():m.slice(0,8),g=((_=f==null?void 0:f[2])==null?void 0:_.trim())||"";return`<div class="stad-choice" data-stad-id="${s.id}" style="cursor:pointer;width:80px;text-align:center">
    ...
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(h=document.getElementById("close-selector"))==null||h.addEventListener("click",o),(_=document.getElementById("remove-player"))==null||_.addEventListener("click",()=>{delete t.slots[e],o(),xt(i,t,n)}),document.querySelectorAll(".player-option").forEach(b=>{b.addEventListener("click",()=>{t.slots[e]=b.dataset.cardId,o(),xt(i,t,n)})})}function bo(e,t,i){var d,u,x;const{openModal:n,closeModal:r}=i,o=(d=e.stadiumCards)==null?void 0:d.find(m=>m.id===e.stadiumCardId),a=o&&((u=e.stadDefMap)==null?void 0:u[o.stadium_id])||null,s=new Set;Object.keys(e.slots).forEach(m=>{var g;const f=e.slots[m];if(!f)return;const p=e.playerCards.find(h=>h.id===f);(g=p==null?void 0:p.player)!=null&&g.id&&s.add(p.player.id)}),e.subs.forEach(m=>{var p;const f=e.playerCards.find(g=>g.id===m);(p=f==null?void 0:f.player)!=null&&p.id&&s.add(f.player.id)});const l=new Set,c=e.playerCards.filter(m=>{var f,p,g;return s.has((f=m.player)==null?void 0:f.id)||l.has((p=m.player)==null?void 0:p.id)?!1:(l.add((g=m.player)==null?void 0:g.id),!0)});n("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
  ...
  <div class="page">
    ...
    <div class="page-body">
      <div class="booster-grid">
        ${o.map(f=>{const p=f.cost===0||r>=f.cost,g=d[f.id]===!0;return`<div class="booster-card ${!p||g?"disabled":""}" data-booster="${f.id}" style="position:relative">
  ...
  `,document.body.appendChild(r);let o=!1;const a=document.getElementById("pack-cut-zone"),s=document.getElementById("pack-blade");let l=!1;const c=v=>v.touches&&v.touches[0]?v.touches[0].clientX:v.clientX;function d(v){o||(l=!0,s.style.opacity="1",u(v))}function u(v){if(!l||o)return;const I=a.getBoundingClientRect(),G=c(v)-I.left,re=Math.max(0,Math.min(1,G/I.width));s.style.width=re*I.width+"px",re>=.82&&m()}function x(){o||(l=!1,s.style.transition="width .2s ease, opacity .2s ease",s.style.width="0",s.style.opacity="0",setTimeout(()=>{o||(s.style.transition="")},220))}function m(){var I;if(o)return;o=!0,l=!1,s.style.width="100%",s.style.opacity="1",(I=document.getElementById("cut-flash"))==null||I.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const v=document.getElementById("cut-hint");v&&(v.style.opacity="0"),a.classList.add("pack-cut"),setTimeout(()=>{s.style.opacity="0",document.getElementById("pack-phase").style.display="none",h(0)},620)}a.addEventListener("pointerdown",d),window.addEventListener("pointermove",u),window.addEventListener("pointerup",x),a.addEventListener("touchstart",d,{passive:!0}),window.addEventListener("touchmove",u,{passive:!0}),window.addEventListener("touchend",x);let f=0,p=!1;const g=new Set;function h(v){f=v,document.getElementById("reveal-phase").style.display="flex",_(),b(v,0),P()}function _(){const v=document.getElementById("card-dots");v&&(v.innerHTML=e.map((I,G)=>`<div class="card-dot" data-i="${G}" style="width:8px;height:8px;border-radius:50%;background:${G===f?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),v.querySelectorAll(".card-dot").forEach(I=>I.addEventListener("click",()=>y(parseInt(I.dataset.i)))))}function b(v,I){var R,M;const G=e[v],re=document.getElementById("card-counter"),ae=document.getElementById("card-track");re&&(re.textContent=`Carte ${v+1} / ${e.length}`);const se=document.getElementById("reveal-btns");se&&(se.style.display=v===e.length-1?"flex":"none");const de=G.card_type==="player"&&((R=G.player)==null?void 0:R.rarity)==="legende",ke=!g.has(v);g.add(v);let W=0;if(G.card_type==="player"&&G.player){const C=G.player,F=C.job||"ATT";W=Number(F==="GK"?C.note_g:F==="DEF"?C.note_d:F==="MIL"?C.note_m:C.note_a)||0}const J=C=>{ae.innerHTML=`
        ...
        ${J}
      </div>`,M&&navigator.vibrate&&navigator.vibrate([50,30,100,30,200])},(q+O)*3),setTimeout(V,(q+O)*3+q),setTimeout(()=>{C.style.display="none",F.innerHTML="",p=!1,X&&!R&&X.pause(),navigator.vibrate&&navigator.vibrate([40,30,80]),I()},(q+O)*4)}function y(v){if(p||v<0||v>=e.length||v===f)return;const I=v>f?1:-1;f=v,b(v,I)}function j(){y(f+1)}function E(){y(f-1)}function P(){const v=document.getElementById("card-viewport");if(!v||v._swipeBound)return;v._swipeBound=!0;let I=0,G=0,re=0,ae=!1;const se=R=>R.touches?R.touches[0].clientX:R.clientX,de=R=>R.touches?R.touches[0].clientY:R.clientY,ke=R=>{ae=!0,I=se(R),G=de(R),re=0},W=R=>{if(!ae)return;re=se(R)-I;const M=de(R)-G;if(Math.abs(re)<Math.abs(M))return;const C=document.getElementById("current-card-wrap");C&&(C.style.transition="none",C.style.transform=`translateX(${re*.6}px) rotate(${re*.02}deg)`)},J=()=>{if(!ae)return;ae=!1;const R=document.getElementById("current-card-wrap"),M=55;re<=-M&&f<e.length-1?j():re>=M&&f>0?E():R&&(R.style.transition="transform .2s ease",R.style.transform="translateX(0)")};v.addEventListener("pointerdown",ke),v.addEventListener("pointermove",W),v.addEventListener("pointerup",J),v.addEventListener("pointercancel",J),v.addEventListener("touchstart",ke,{passive:!0}),v.addEventListener("touchmove",W,{passive:!0}),v.addEventListener("touchend",J),v.addEventListener("click",R=>{if(Math.abs(re)>8)return;const M=v.getBoundingClientRect();R.clientX-M.left>M.width/2?j():E()})}let N=null;function ne(){const v=document.getElementById("fireworks-canvas");if(!v)return;v.width=window.innerWidth,v.height=window.innerHeight;const I=v.getContext("2d"),G=[];function re(){const se=Math.random()*v.width,de=Math.random()*v.height*.6,ke=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],W=ke[Math.floor(Math.random()*ke.length)];for(let J=0;J<60;J++){const R=Math.PI*2/60*J,M=2+Math.random()*5;G.push({x:se,y:de,vx:Math.cos(R)*M,vy:Math.sin(R)*M,alpha:1,color:W,size:2+Math.random()*3})}}re(),N=setInterval(re,600);function ae(){if(document.getElementById("fireworks-canvas")){I.clearRect(0,0,v.width,v.height);for(let se=G.length-1;se>=0;se--){const de=G[se];if(de.x+=de.vx,de.y+=de.vy+.08,de.vy*=.98,de.alpha-=.018,de.alpha<=0){G.splice(se,1);continue}I.globalAlpha=de.alpha,I.fillStyle=de.color,I.beginPath(),I.arc(de.x,de.y,de.size,0,Math.PI*2),I.fill()}I.globalAlpha=1,(N!==null||G.length>0)&&requestAnimationFrame(ae)}}ae()}function ie(){N!==null&&(clearInterval(N),N=null);const v=document.getElementById("fireworks-canvas");v&&v.getContext("2d").clearRect(0,0,v.width,v.height)}if(n){const v=document.getElementById("reveal-btns");v&&(v.innerHTML='<button class="btn btn-primary" id="reveal-next" style="flex:1">Continuer →</button>'),(K=document.getElementById("reveal-next"))==null||K.addEventListener("click",()=>{ie(),r.remove(),n()})}else document.getElementById("reveal-collection").addEventListener("click",()=>{ie(),r.remove(),i("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{ie(),r.remove(),i("boosters")})}function zo(e){var t,i,n,r;if(e.card_type==="player"&&e.player)return ko(e);if(e.card_type==="game_changer"){const o=e._gcDef,a=(o==null?void 0:o.gc_type)==="ultra_game_changer",s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},l={purple:"#b06ce0",light_blue:"#00d4ef"},c=s[o==null?void 0:o.color]||s.purple,d=l[o==null?void 0:o.color]||l.purple,u=(o==null?void 0:o.name)||e.gc_type||"Game Changer",x=(o==null?void 0:o.effect)||((t=wi[e.gc_type])==null?void 0:t.desc)||"",m=o!=null&&o.image_url?`/icons/${o.image_url}`:null,f=((i=wi[e.gc_type])==null?void 0:i.icon)||"⚡";return`<div style="width:170px;height:240px;background:${c};border-radius:14px;border:3px solid ${d};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${d}88;flex-shrink:0">
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
  ...
  </div>`;const d=async()=>{await w.from("users").update({pending_boosters:a}).eq("id",t.user.id)};async function u(){var _;if(c>=l||!a.length){await w.from("users").update({pending_boosters:[],onboarding_done:!0}).eq("id",t.user.id),r&&await r(),n("Tous tes boosters sont ouverts ! Bon jeu 🎮","success",4e3),i("home");return}const f=a[0],{data:p}=await w.from("users").select("*").eq("id",t.user.id).single();p&&(t.profile=p);let g=[];try{if(f.type==="formation")g=await hn(t.profile,0);else if(f.type==="game_changer")g=await bn(t.profile,f.count||3,0);else if(s&&((_=s.rates)!=null&&_.length)){const b={...s,cost:0,cardCount:f.count||s.cardCount||5};g=await _i(t.profile,b),f.guaranteeGK&&!t.profile.first_booster_opened&&(g.some(y=>y.player&&y.player.job==="GK")||await jo(t.profile,g),await w.from("users").update({first_booster_opened:!0}).eq("id",t.profile.id))}else g=await yn(t.profile,f.count||5,0)}catch(b){n(b.message||"Erreur ouverture booster","error");return}a.shift(),c++,await d();const h=f.type==="formation"?{name:"Booster Formation",img:"/icons/booster-formation.png"}:f.type==="game_changer"?{name:"Booster Game Changer",img:"/icons/booster-gamechanger.png"}:{name:`Booster Joueurs (${c}/${l})`,img:(s==null?void 0:s.img)||"/icons/booster-players.png"};vn(g,h,i,()=>{u()})}(m=document.getElementById("onboard-start"))==null||m.addEventListener("click",()=>u())}async function jo(e,t){try{const{data:i}=await w.from("players").select("id,job,firstname,surname_real,country_code,club_id,rarity,note_g,note_d,note_m,note_a,skin,hair,hair_length,face,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0).eq("job","GK");if(!(i!=null&&i.length))return;const n=i[Math.floor(Math.random()*i.length)],r=t.findIndex(a=>a.player);if(r===-1)return;const o=t[r];await w.from("cards").update({player_id:n.id}).eq("id",o.id),t[r]={...o,player_id:n.id,player:n}}catch(i){console.warn("[Onboarding] ensureGK échec",i)}}const Ft={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},Co={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function Et(e,t,i,n,r){var o;e.innerHTML=`<div class="match-screen" style="display:flex;align-items:center;justify-content:center;min-height:100vh">
    <div style="text-align:center;padding:40px;color:#fff">
      <div style="font-size:48px;margin-bottom:16px">${t}</div>
      <p style="margin-bottom:16px">${i}</p>
      <button class="btn btn-primary" id="msg-btn">${n}</button>
    </div>
  </div>`,(o=document.getElementById("msg-btn"))==null||o.addEventListener("click",r)}function Wt(e){if(e!=null&&e.face){const n=(typeof import.meta<"u"?"/":null)||"/",r=e.face.replace(/^public\//,"").replace(/^\//,"");return n+r}const t=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function ki(e,t){var o,a;const i=e.player,n=e.evolution_bonus||0,r=i.job2&&Number(i[`note_${i.job2.toLowerCase()}`])||0;return{cardId:e.id,position:t||null,id:i.id,firstname:i.firstname,name:i.surname_real,country_code:i.country_code,club_id:i.club_id,job:i.job,job2:i.job2,note_g:(Number(i.note_g)||0)+(i.job==="GK"?n:0)+(i.job2==="GK"&&r>0?n:0),note_d:(Number(i.note_d)||0)+(i.job==="DEF"?n:0)+(i.job2==="DEF"&&r>0?n:0),note_m:(Number(i.note_m)||0)+(i.job==="MIL"?n:0)+(i.job2==="MIL"&&r>0?n:0),note_a:(Number(i.note_a)||0)+(i.job==="ATT"?n:0)+(i.job2==="ATT"&&r>0?n:0),evolution_bonus:n,rarity:i.rarity,skin:i.skin,hair:i.hair,hair_length:i.hair_length,face:i.face||null,clubName:((o=i.clubs)==null?void 0:o.encoded_name)||null,clubLogo:((a=i.clubs)==null?void 0:a.logo_url)||null,boost:0,used:!1,_line:null,_col:null}}function Xt(e,t){if(!t||!e)return e;const{club_id:i,country_code:n}=t;return Object.values(e).forEach(r=>{Array.isArray(r)&&r.forEach(o=>{const a=i&&String(o.club_id)===String(i),s=n&&String(o.country_code)===String(n);(a||s)&&(o.stadiumBonus=!0)})}),e}function oi(e,t){if(!t||!(e!=null&&e.length))return e;const{club_id:i,country_code:n}=t;return e.forEach(r=>{if(!r)return;const o=i&&String(r.club_id)===String(i),a=n&&String(r.country_code)===String(n);(o||a)&&(r.stadiumBonus=!0)}),e}function pt(e){return e===1?[1]:e===2?[0,2]:e===3?[0,1,2]:e===4?[0,1,1,2]:e===5?[0,1,1,1,2]:[1]}function wn(){const e=Math.random()*100;return e<10?10:e<30?5:3}function ri(e,t){const i=Ft[t]||Ft["4-4-2"],n={GK:[],DEF:[],MIL:[],ATT:[]};if(e.length&&e.every(a=>a.position)){for(const a of["GK","DEF","MIL","ATT"]){const s=e.filter(c=>c.position&&c.position.replace(/\d+$/,"")===a).sort((c,d)=>parseInt(c.position.replace(/\D+/g,""),10)-parseInt(d.position.replace(/\D+/g,""),10)).map(c=>({...c,_line:a})),l=pt(s.length);s.forEach((c,d)=>{c._col=l[d]}),n[a]=s}return n}const o=[...e];for(const a of["GK","DEF","MIL","ATT"]){const s=[];for(let c=0;c<i[a];c++){let d=o.findIndex(u=>u.job===a);if(d===-1&&(d=o.findIndex(u=>u.job2===a)),d===-1&&(d=0),o[d]){const u={...o[d],_line:a};s.push(u),o.splice(d,1)}}const l=pt(s.length);s.forEach((c,d)=>{c._col=l[d]}),n[a]=s}return n}function nt(e){document.querySelectorAll(".top-nav, .bottom-nav").forEach(t=>{t.style.setProperty("display","none","important"),t.dataset.matchHidden="1"}),e&&e.style.setProperty("padding-bottom","0","important")}function We(e){document.querySelectorAll(".top-nav, .bottom-nav").forEach(t=>{t.style.removeProperty("display"),delete t.dataset.matchHidden}),e&&e.style.removeProperty("padding-bottom")}function li(e,t,i){const r=new Set,o=t.filter(d=>{const u=d.gc_type||d.id;return r.has(u)?!1:(r.add(u),!0)});let a=[];function s(d,u){const x=d._gcDef,m={purple:"linear-gradient(135deg,#3d0a7a,#7a28b8)",light_blue:"linear-gradient(135deg,#006080,#00bcd4)"},f={purple:"#9b59b6",light_blue:"#00bcd4"},p=m[x==null?void 0:x.color]||m.purple,g=f[x==null?void 0:x.color]||f.purple;return`<div class="gc-select-card" data-id="${d.id}"
      style="width:100px;border-radius:10px;border:3px solid ${u?"#FFD700":g};background:${p};
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      <div style="flex:1;overflow-y:auto;display:flex;flex-wrap:wrap;gap:10px;justify-content:center;align-content:flex-start;padding:8px 16px 16px">
        ${o.map(f=>{const p=a.find(g=>g.gc_type===f.gc_type);return s(f,!!p)}).join("")}
      </div>
    ...
    </div>`,e.querySelectorAll(".gc-select-card").forEach(f=>{f.addEventListener("click",()=>{const p=f.dataset.id,g=o.find(_=>_.id===p);if(!g)return;const h=a.findIndex(_=>_.gc_type===g.gc_type);h>-1?a.splice(h,1):a.length<3&&a.push(g),c()})}),(u=e.querySelector("#gc-launch"))==null||u.addEventListener("click",()=>{d&&l(a)}),(x=e.querySelector("#gc-no-gc"))==null||x.addEventListener("click",()=>l([])),(m=e.querySelector("#gc-reset"))==null||m.addEventListener("click",()=>{a.length&&(a=[],c())})}c()}function Bo(e,t){var r;const i=((r=t==null?void 0:t.state)==null?void 0:r.params)||{},n=e||i.matchMode||"vs_ai_easy";return n==="friend"?`Match vs ${i.friendName||"Ami"}`:n==="random"?"Match vs Random":n==="ranked"?"Match Classé":n==="mini_league"||n==="mini-league"?"🏆 Match de Mini League":`Match vs IA — ${n.replace("vs_ai_","").toUpperCase()}`}async function qo(e,t,i){const{state:n,navigate:r}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await w.from("decks").select(`id,name,is_active,formation,stadium_card_id,
      stadium_card:cards!stadium_card_id(id,stadium_id,
        stadium_def:stadium_definitions(id,name,club_id,country_code,image_url,
          club:clubs(encoded_name,logo_url)))`).eq("owner_id",n.profile.id).order("created_at",{ascending:!1});if(!o||o.length===0){Et(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>r("decks"));return}const a=o.map(x=>x.id),{data:s}=await w.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,stadium_id,evolution_bonus,
        player:players(id,firstname,surname_real,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order"),l=[...new Set((s||[]).filter(x=>{var m,f;return((m=x.card)==null?void 0:m.card_type)==="stadium"&&((f=x.card)==null?void 0:f.stadium_id)}).map(x=>x.card.stadium_id))],c={};if(l.length){const{data:x}=await w.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",l);(x||[]).forEach(m=>{c[m.id]=m}),(s||[]).forEach(m=>{var f,p;((f=m.card)==null?void 0:f.card_type)==="stadium"&&((p=m.card)!=null&&p.stadium_id)&&(m.card._stadiumDef=c[m.card.stadium_id]||null)})}let d=0;function u(){var y,j,E,P,N,ne,ie;const x=o[d],m=(s||[]).filter(Q=>Q.deck_id===x.id),f=m.filter(Q=>{var K;return Q.is_starter&&((K=Q.card)==null?void 0:K.player)}).map(Q=>ki(Q.card,Q.position)),p=m.find(Q=>{var K;return((K=Q.card)==null?void 0:K.card_type)==="formation"}),g=x.formation||((y=p==null?void 0:p.card)==null?void 0:y.formation)||"4-4-2";let h=f.length>=11?ri(f,g):null,_=((j=x.stadium_card)==null?void 0:j.stadium_def)||null;_&&h&&(h=Xt(h,_));const b=f.length>=11;nt(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff;position:relative">
      ...
      <div id="deck-top-bar" style="flex-shrink:0">
        ...
        <div style="display:flex;align-items:center;gap:8px;padding:6px 8px">
          ...
          <div style="flex:1;text-align:center">
            ...
            <div style="font-size:11px;opacity:.6">${g} · ${f.length}/11${x.is_active?" · ⭐":""}</div>
          </div>
      ...
      <div id="deck-swipe-zone" style="flex:1;min-height:0;overflow:hidden;position:relative;touch-action:pan-y;display:flex;align-items:center;justify-content:center">
        ${h?'<div class="deck-preview-wrap" style="overflow:hidden;width:100%;height:100%;display:flex;align-items:center;justify-content:center"></div>':`<div style="opacity:.4;text-align:center"><div style="font-size:32px">⚠️</div><div>Deck incomplet (${f.length}/11)</div></div>`}
      </div>

      <!-- Pagination -->
      ${o.length>1?`<div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">${o.map((Q,K)=>`<div style="width:6px;height:6px;border-radius:50%;background:${K===d?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}</div>`:""}
      ...
      <div id="deck-bottom-bar" style="flex-shrink:0;padding:10px 14px 14px;display:flex;flex-direction:column;gap:8px;background:rgba(0,0,0,0.2)">
    ...
    </div>`,requestAnimationFrame(()=>requestAnimationFrame(function Q(){const K=e.querySelector(".deck-preview-wrap"),v=e.querySelector("#deck-swipe-zone");if(!K||!v||!h)return;const I=v.clientWidth>=900,G=Math.max(200,v.clientHeight-(I?60:40)),ae=Math.max(200,v.clientWidth-16),se=I?Math.min(117,Math.max(52,Math.round(ae*.22))):Math.max(44,Math.round(ae*.168));if(G<220||ae<220){requestAnimationFrame(Q);return}const de=I?null:Math.round(se*.55);K.innerHTML=Pt(h,g,null,[],ae,G,[],de),K.style.cssText=`width:${ae}px;height:${G}px;overflow:hidden;margin:${I?0:60}px auto 0`;const ke=K.querySelector("svg");ke&&(ke.style.cssText="display:block;width:100%;height:100%",ke.setAttribute("preserveAspectRatio",I?"xMidYMid meet":"none"))})),(P=document.getElementById("prev-deck"))==null||P.addEventListener("click",()=>{d>0&&(d--,u())}),(N=document.getElementById("next-deck"))==null||N.addEventListener("click",()=>{d<o.length-1&&(d++,u())}),(ne=document.getElementById("validate-deck"))==null||ne.addEventListener("click",()=>{if(!b)return;const Q=t.state.params||{};t.navigate("match",{...Q,matchMode:Q.matchMode||i,deckId:x.id})}),(ie=document.getElementById("cancel-deck-select"))==null||ie.addEventListener("click",()=>{We(e),r("home")});const $=document.getElementById("deck-swipe-zone");if($){let Q=0,K=0;$.addEventListener("touchstart",v=>{Q=v.touches[0].clientX,K=v.touches[0].clientY},{passive:!0}),$.addEventListener("touchend",v=>{const I=v.changedTouches[0].clientX-Q,G=v.changedTouches[0].clientY-K;Math.abs(I)<40||Math.abs(I)<Math.abs(G)||(I<0&&d<o.length-1?(d++,u()):I>0&&d>0&&(d--,u()))},{passive:!0})}}u()}function mt(e,t=44,i=58,n=null){return Se(e,{width:t,showStad:!!n,stadDef:n,used:e==null?void 0:e.used})}function Ye(e,t,i,n,r){if(!(e!=null&&e.length))return"";const o=e.slice(0,5);let a='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return o.forEach((s,l)=>{const c=s._line||s.job||"MIL";let d=s.boost||0;if(s.stadiumBonus&&(n==="attack"&&(c==="ATT"||c==="MIL")||n==="defense"&&(c==="GK"||c==="DEF"||c==="MIL")?d+=10:n||(d+=10)),a+=Se(s,{width:40,role:c,extraNote:d}),l<o.length-1){const u=o[l+1],x=cn(s,u,r)?Gt(s,u):null;a+=`<div style="width:7px;height:3px;background:${!x||x==="#ff3333"||x==="#cc2222"?"rgba(255,255,255,0.12)":x};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(a+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),a+="</div>",a}function ci(e,t,i,n,r=310,o=310,a=[],s=null){const l=Jt[t]||{},c=Ii(t)||Si[t]||[],d={},u=["ATT","MIL","DEF","GK"];for(const h of u)(e[h]||[]).forEach((b,$)=>{d[`${h}${$+1}`]=b});function x(h){const _=l[h];return _?{x:_.x*r,y:_.y*o}:null}let m="";for(const[h,_]of c){const b=x(h),$=x(_);if(!b||!$)continue;const y=d[h],j=d[_],E=Gt(y,j);E==="#00ff88"||E==="#FFD700"?(m+=`<line x1="${b.x.toFixed(1)}" y1="${b.y.toFixed(1)}" x2="${$.x.toFixed(1)}" y2="${$.y.toFixed(1)}"
        ...
        stroke="${E}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const f=typeof window<"u"&&window.innerWidth>=900?Math.min(Math.max(81,Math.round(r*.225)),117):Math.max(44,Math.round(r*.168)),p=Math.round(f*657/507);for(const[h,_]of Object.entries(d)){const b=x(h);if(!b||!_)continue;const $=h.replace(/[0-9]/g,""),y=a.includes(_.cardId),j=i==="attack"&&(["MIL","ATT"].includes($)||y)&&!_.used||i==="defense"&&["GK","DEF","MIL"].includes($)&&!_.used,E=n.includes(_.cardId);let P=_.boost||0,N=!1;_.stadiumBonus&&(i==="attack"&&($==="ATT"||$==="MIL")||i==="defense"&&($==="GK"||$==="DEF"||$==="MIL")?(P+=10,N=!0):i||(P+=10,N=!0));const ne=Math.round(b.x-f/2),ie=Math.round(b.y-p/2);if(_.used){m+=`<image href="${`${typeof import.meta<"u"&&"/"||"/"}icons/carte-dos.png`}" x="${ne}" y="${ie}" width="${f}" height="${p}" preserveAspectRatio="xMidYMid slice" class="match-used-hit" data-card-id="${_.cardId}" data-role="${$}" style="cursor:pointer"/>`;continue}const Q=Se({..._,_evolution_bonus:0,stadiumBonus:!1},{width:f,showStad:!1,stadDef:null,role:$,extraNote:P,_cardOffset:30,_forceStadColor:N}),K=E?`position:absolute;top:30px;left:0;width:${f}px;height:${p}px;outline:3px solid #FFD700;outline-offset:2px;border-radius:8px;pointer-events:none;`:"";m+=`<foreignObject x="${ne-2}" y="${ie-30}" width="${f+8}" height="${p+60}" style="overflow:visible">
      <div xmlns="http://www.w3.org/1999/xhtml" style="position:relative">
        ${Q}
        ${E?`<div style="${K}"></div>`:""}
      </div>
    </foreignObject>`,j&&(m+=`<rect x="${ne}" y="${ie}" width="${f}" height="${p}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${E?"selected":""}" data-card-id="${_.cardId}" data-role="${$}" style="cursor:pointer"/>`)}const g=s!==null?s:Math.round(Math.max(f*.7,80));return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-g} ${-g} ${r+g*2} ${o+g*2}" width="100%" style="display:block;width:100%;margin:0 auto">
    ${m}
  </svg>`}function Pt(e,t,i,n,r=300,o=300,a=[],s=null){return`<div id="match-terrain-wrap" style="position:relative;padding:0 4px">
    ${ci(e,t,i,n,r,o,a,s)}
  </div>`}async function pi(e,t,i,n){var j;const{state:r,navigate:o,toast:a}=t;nt(e);const s=r.params||{};if(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!s.deckId)return qo(e,t,i);const l=s.deckId;let c,d,u,x;try{const E=await Promise.all([w.from("decks").select("formation,name,stadium_card_id").eq("id",l).single(),w.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation, evolution_bonus,
            player:players(id,firstname,surname_real,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
              clubs(encoded_name,logo_url)))`).eq("deck_id",l).order("slot_order")]);c=E[0].data,u=E[0].error,d=E[1].data,x=E[1].error}catch(E){console.error("[Match] Exception chargement deck:",E),Et(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>o("home"));return}if(u||x){console.error("[Match] Erreur Supabase:",u||x),Et(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>o("home"));return}const m=(d||[]).filter(E=>{var P;return E.is_starter&&((P=E.card)==null?void 0:P.player)}).map(E=>ki(E.card,E.position)),f=(d||[]).filter(E=>{var P;return!E.is_starter&&((P=E.card)==null?void 0:P.player)}).map(E=>ki(E.card,E.position));if(m.length<11){Et(e,"⚠️",`Deck incomplet (${m.length}/11).`,"Compléter",()=>o("decks"));return}const p=(d||[]).find(E=>{var P;return((P=E.card)==null?void 0:P.card_type)==="formation"}),g=(c==null?void 0:c.formation)||((j=p==null?void 0:p.card)==null?void 0:j.formation)||"4-4-2",{data:h,error:_}=await w.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",r.profile.id).eq("card_type","game_changer"),{data:b}=await w.from("gc_definitions").select("*").eq("is_active",!0),$=(h||[]).map(E=>({...E,_gcDef:(b==null?void 0:b.find(P=>P.name===E.gc_type||P.id===E.gc_definition_id))||null}));let y=null;if(c!=null&&c.stadium_card_id){const{data:E}=await w.from("cards").select("stadium_id").eq("id",c.stadium_card_id).single();if(E!=null&&E.stadium_id){const{data:P}=await w.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").eq("id",E.stadium_id).single();y=P||null}}n({deckId:l,formation:g,starters:m,subsRaw:f,gcCardsEnriched:$,gcDefs:b||[],stadiumDef:y})}function Lt(){return Math.min(window.innerWidth-40,860)}function qt(){return Math.round(Lt()*1.1)}function _n(e){var r,o;if(!e)return null;const t=e._line||e.job||"MIL",i=t==="GK"?e.note_g||0:t==="DEF"?e.note_d||0:t==="MIL"?e.note_m||0:e.note_a||0,n=e.stadiumBonus?10:0;return{name:e.name,firstname:e.firstname||"",note:i+(e.boost||0)+n,note_g:e.note_g||0,note_d:e.note_d||0,note_m:e.note_m||0,note_a:e.note_a||0,_evolution_bonus:0,stadiumBonus:e.stadiumBonus||!1,boost:e.boost||0,job:e.job,job2:e.job2||null,_line:e._line||e.job,_col:e._col,country_code:e.country_code,club_id:e.club_id,rarity:e.rarity,clubName:e.clubName||((r=e.clubs)==null?void 0:r.encoded_name)||null,clubLogo:e.clubLogo||((o=e.clubs)==null?void 0:o.logo_url)||null,face:e.face||null,portrait:Wt(e)}}function Do(e,t,i,n="Adversaire"){const r=Lt(),o=qt();return`
    ...
    pointer-events:none;white-space:nowrap;max-width:90vw;text-align:center`,r.textContent=e,!document.getElementById("game-toast-style")){const o=document.createElement("style");o.id="game-toast-style",o.textContent=`
      ...
      }`,document.head.appendChild(o)}document.body.appendChild(r),setTimeout(()=>{r.style.opacity="0",r.style.transition="opacity 0.3s",setTimeout(()=>r.remove(),300)},i)}function $n(e,t){const i=Ge(e,"MIL"),n=e.stadiumBonus||t&&(t.club_id&&String(e.club_id)===String(t.club_id)||t.country_code&&e.country_code===t.country_code)?10:0;return i+n}function ut(e,t){return e.reduce((i,n)=>i+$n(n,t),0)}function ft(e){let t=0;for(let i=0;i<e.length-1;i++){const n=Gt(e[i],e[i+1]);n==="#00ff88"?t+=10:n==="#FFD700"&&(t+=5)}return t}function ai(e,t,i,n,r){return`<div id="duel-row-${n}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0), opacity .5s ease;transform-origin:center">
      ...
      <div class="duel-score-line duel-score-line-${n}" style="margin-top:10px;font-size:12px;color:rgba(255,255,255,0.55);opacity:0;transition:opacity .4s ease">
        Score: ${ut(e,r)} + ${ft(e)} liens = <b style="color:#fff">${ut(e,r)+ft(e)}</b>
      </div>
    </div>`}async function Po(e,t){const{state:i}=t,r=(i.params||{}).matchMode||"vs_ai_easy",o=r.replace("vs_ai_",""),a=r;await pi(e,t,r,async({deckId:s,formation:l,starters:c,subsRaw:d,gcCardsEnriched:u,gcDefs:x,stadiumDef:m})=>{try{let f=ri(c,l);m&&(f=Xt(f,m),oi(d,m));const p=await Go(l,o),g=p.lines||p,h=async _=>{try{const b=a==="vs_ai_club"?"club":a,{data:$,error:y}=await w.from("matches").insert({home_id:i.profile.id,away_id:null,mode:b,home_deck_id:s,status:"in_progress"}).select().single();if(y){console.error("[MatchIA] Erreur création match:",y),Et(e,"⚠️","Impossible de créer le match ("+y.message+").","Retour",()=>t.navigate("home"));return}const j=p.stadiumDef||null;j&&g&&(Xt(g,j),oi(p.subs||[],j));const E={gcDefs:x||[],matchId:$==null?void 0:$.id,mode:a,difficulty:o,formation:l,homeTeam:f,aiTeam:g,homeSubs:d,subsUsed:0,maxSubs:Math.min(d.length,3),aiSubs:p.subs||[],aiSubsUsed:0,aiMaxSubs:Math.min((p.subs||[]).length,3),aiUsedSubIds:[],aiGcCards:p.gcCards||[],aiUsedGc:[],aiStadiumDef:j,homeScore:0,aiScore:0,gcCards:_,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};No(e,E,t)}catch(b){console.error("[MatchIA] Exception launchMatch:",b),Et(e,"⚠️","Erreur au lancement du match : "+b.message,"Retour",()=>t.navigate("home"))}};if(!u.length){h([]);return}li(e,u,h)}catch(f){console.error("[MatchIA] Exception setup:",f),Et(e,"⚠️","Erreur de préparation du match : "+f.message,"Retour",()=>t.navigate("home"))}})}async function Go(e,t){var g;const{data:i}=await w.from("players").select("id,firstname,surname_real,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(80);if(!i||i.length<11)return{lines:Ro(e),subs:[],gcCards:[],stadiumDef:null};const n=Ft[e]||Ft["4-4-2"],r={GK:[],DEF:[],MIL:[],ATT:[]},o=new Set;function a(h,_,b){var $,y;return o.add(h.id),{cardId:"ai-"+h.id+"-"+b,id:h.id,firstname:h.firstname,name:h.surname_real,country_code:h.country_code,club_id:h.club_id,job:h.job,job2:h.job2,note_g:Number(h.note_g)||0,note_d:Number(h.note_d)||0,note_m:Number(h.note_m)||0,note_a:Number(h.note_a)||0,rarity:h.rarity,skin:h.skin,hair:h.hair,hair_length:h.hair_length,clubName:(($=h.clubs)==null?void 0:$.encoded_name)||null,clubLogo:((y=h.clubs)==null?void 0:y.logo_url)||null,boost:0,used:!1,_line:_}}for(const h of["GK","DEF","MIL","ATT"]){const _=i.filter(E=>E.job===h&&!o.has(E.id)),b=i.filter(E=>E.job!==h&&!o.has(E.id)),$=[..._,...b],y=[];for(let E=0;E<n[h];E++){const P=$[E];P&&y.push(a(P,h,E))}const j=pt(y.length);y.forEach((E,P)=>{E._col=j[P]}),r[h]=y}const l=i.filter(h=>!o.has(h.id)).slice(0,5).map((h,_)=>a(h,h.job,100+_)),u=Object.keys(Ae).sort(()=>Math.random()-.5).slice(0,3).map((h,_)=>{var b,$;return{id:"ai-gc-"+_,gc_type:h,name:((b=Ae[h])==null?void 0:b.name)||h,icon:(($=Ae[h])==null?void 0:$.icon)||"⚡"}}),x=Object.values(r).flat(),m={};x.forEach(h=>{h.club_id&&(m[h.club_id]=(m[h.club_id]||0)+1)});const f=(g=Object.entries(m).sort((h,_)=>_[1]-h[1])[0])==null?void 0:g[0];let p=null;if(f){const{data:h}=await w.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",f).single();h&&(p={club_id:h.id,country_code:null,name:h.encoded_name+" Stadium",club:{encoded_name:h.encoded_name,logo_url:h.logo_url}})}return{lines:r,subs:l,gcCards:u,stadiumDef:p}}function Ro(e){const t=Ft[e]||Ft["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},n=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let r=0;for(const o of["GK","DEF","MIL","ATT"]){const a=[];for(let l=0;l<t[o];l++){const c=3+Math.floor(Math.random()*5);a.push({cardId:"fake-"+r,id:"fake-"+r,firstname:"IA",name:n[r%n.length],country_code:"XX",club_id:null,job:o,job2:null,note_g:o==="GK"?c:2,note_d:o==="DEF"?c:2,note_m:o==="MIL"?c:2,note_a:o==="ATT"?c:2,rarity:"normal",boost:0,used:!1,_line:o}),r++}const s=pt(a.length);a.forEach((l,c)=>{l._col=s[c]}),i[o]=a}return i}function No(e,t,i){var r;const n=`<div style="position:relative;width:22px;height:22px;flex-shrink:0;display:flex;align-items:center;justify-content:center">
  ...
  </div>`;const d=()=>{const f=(p,g)=>e.querySelectorAll(p).forEach((h,_)=>{setTimeout(()=>{h.style.opacity="1",h.style.transform="translateY(0) scale(1)"},g+_*90)});f(".duel-card-home",150),f(".duel-card-ai",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((p,g)=>{setTimeout(()=>{p.style.opacity="1"},g*70)}),900),setTimeout(()=>{const p=e.querySelector("#vs-label");p&&(p.style.opacity="1",p.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(g=>g.style.opacity="1")},1250),setTimeout(()=>{u("score-home",s,800),u("score-ai",l,800)},1500)};function u(f,p,g){const h=document.getElementById(f);if(!h)return;const _=performance.now(),b=$=>{const y=Math.min(1,($-_)/g);h.textContent=Math.round(p*(1-Math.pow(1-y,3))),y<1?requestAnimationFrame(b):h.textContent=p};requestAnimationFrame(b)}requestAnimationFrame(d),t.attacker=c?"home":"ai";const x=c?wn():null;c&&(t.boostCard={value:x}),t.log.push({type:"duel",title:"Milieu de Terrain",homePlayers:n.map(f=>Ve(f)),aiPlayers:r.map(f=>Ve(f)),homeTotal:s,aiTotal:l,text:`Duel milieu : ${t.clubName} ${s} – ${l} IA → ${c?t.clubName+" attaque":"IA attaque"}`});const m=()=>{t.phase=t.attacker==="home"?"attack":"ai-attack",Ee(e,t,i),t.attacker==="ai"&&setTimeout(()=>qi(e,t,i),800)};setTimeout(()=>{const f=document.getElementById("score-home"),p=document.getElementById("score-ai"),g=document.getElementById(c?"duel-row-home":"duel-row-ai"),h=document.getElementById(c?"duel-row-ai":"duel-row-home"),_=c?f:p,b=c?p:f;_&&(_.style.fontSize="80px",_.style.color=c?"#FFD700":"#ff6b6b",_.style.animation="duelPulse .5s ease"+(c?", duelGlow 1.5s ease infinite .5s":"")),b&&(b.style.opacity="0.25"),setTimeout(()=>{g&&(g.style.transformOrigin="center",g.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",g.style.zIndex="5"),setTimeout(()=>{var y;const $=document.getElementById("duel-shock");if($){const j=(y=h||g)==null?void 0:y.getBoundingClientRect(),E=e.querySelector(".match-screen").getBoundingClientRect();j&&($.style.top=j.top-E.top+j.height/2+"px"),$.style.animation="shockwave .5s ease-out forwards"}h&&(h.style.transformOrigin="center",h.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var y;const $=document.getElementById("duel-finale");$&&($.innerHTML=`
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const I=t.pendingAttack;let G="";if(t.selected.length>0){const re=t.selected.map(se=>({...(t.homeTeam[se._role]||[]).find(ke=>ke.cardId===se.cardId)||se,_line:se._role})),ae=Yt(re,t.modifiers.home,t.formation);G=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
            ...
            <div style="display:flex;justify-content:center">${Ye((I.players||[]).map(re=>({...re,used:!1})),"#ff6b6b",I.total,"attack",t.formation)}</div>
            ${G}
            ...
            <div style="display:flex;justify-content:center">${Ye((I.players||[]).map(G=>({...G,used:!1})),"#00ff88",I.total,"attack",t.formation)}</div>
          </div>`}if(t.phase==="attack"&&t.selected.length>0){const I=t.selected.map(re=>{const ae=(t.homeTeam[re._role]||[]).find(de=>de.cardId===re.cardId)||re,se=["GK","DEF"].includes(re._role);return{...ae,_line:re._role,...se?{note_a:Math.max(1,Number(ae.note_a)||0)}:{}}}),G=Vt(I,t.modifiers.home,t.formation);return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #FFD700;text-align:center">
            ...
            <div style="display:flex;justify-content:center">${Ye(I.map(re=>({...re,used:!1})),"#FFD700",G.total,"attack",t.formation)}</div>
          </div>`}const v=t.log[t.log.length-1];return v?'<div style="padding:2px 4px">'+kn(v)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})()}
    </div>
    ...
    ${(()=>{const v=window.innerWidth>=700,I=(R,M,C)=>{var ge,Be;const F=(t.gcDefs||[]).find(je=>je.name===R.gc_type),X={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[F==null?void 0:F.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",V={purple:"#b06ce0",light_blue:"#00d4ef"}[F==null?void 0:F.color]||"#b06ce0",q=(F==null?void 0:F.name)||R.gc_type,O=(F==null?void 0:F.effect)||((ge=Ae[R.gc_type])==null?void 0:ge.desc)||"",we=F!=null&&F.image_url?`/icons/${F.image_url}`:null,_e=((Be=Ae[R.gc_type])==null?void 0:Be.icon)||"⚡",ye=Math.round(C*.22),be=Math.round(C*.22),$e=C-ye-be,Le=q.length>12?7:9;return`<div class="gc-mini" data-gc-id="${R.id}" data-gc-type="${R.gc_type}"
          style="box-sizing:border-box;width:${M}px;height:${C}px;border-radius:10px;border:2px solid ${V};background:${X};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
          ...
          </div>`},re=(R,M)=>M?G(130,175):I(R,130,175),ae=(R,M)=>M?G(68,95):I(R,68,95),se=v?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",de=m?`<button id="btn-results" style="${se};background:linear-gradient(135deg,#D4A017,#FFD700);border:none;color:#000">🏁 Résultats</button>`:d?`<div style="${se};background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);color:rgba(255,255,255,0.4)">⏳ Tour IA</div>`:g?`<button id="btn-pass" style="${se};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER (plus d'attaquants)</button>`:u?`<button id="btn-action" style="${se};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${t.selected.length===0?"disabled":""}> ⚔️ ATTAQUEZ <span id="match-timer" style="font-weight:900"></span></button>`:x?`<button id="btn-action" style="${se};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${t.selected.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="match-timer" style="font-weight:900"></span></button>`:`<div style="${se};background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1)"></div>`,ke=u||x?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${t.selected.length}/3 sélectionné(s)</div>`:"",W=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${v?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
        ${o.length===0?'<div style="font-size:7px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':o.map(R=>`<div class="sub-btn-col" data-sub-id="${R.cardId}" style="cursor:pointer;flex-shrink:0">${mt(R,76,100)}</div>`).join("")}
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(v=>{var I,G,re;if(v.type==="duel"){const ae=v.isGoal,se=v.homeScored?"#FFD700":ae?"#ff6b6b":"rgba(255,255,255,0.3)",de=v.homeScored?"⚽ BUT !":ae?"⚽ BUT IA !":(I=v.homePlayers)!=null&&I.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${ae?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${se};margin-bottom:4px">
                <div style="font-size:9px;color:${se};letter-spacing:1px;margin-bottom:5px;font-weight:700;text-transform:uppercase">${de}</div>
                ${(G=v.homePlayers)!=null&&G.length?`<div style="margin-bottom:3px">${Ye(v.homePlayers,"rgba(255,255,255,0.7)",v.homeTotal,void 0,t.formation)}</div>`:""}
                ${(re=v.aiPlayers)!=null&&re.length?`<div style="opacity:0.7">${Ye(v.aiPlayers,"#ff6b6b",v.aiTotal,void 0,t.formation)}</div>`:""}
  ...
  </div>`;function b(){const v=e.querySelector(".match-screen");if(!v)return;const I=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);v.style.bottom="auto",v.style.height=I+"px",v.style.minHeight=I+"px",v.style.maxHeight=I+"px",v.style.overflow="hidden";const G=e.querySelector("#mobile-action-bar"),re=e.querySelector("#mobile-play-area");G&&re&&(re.style.paddingBottom=G.offsetHeight+"px")}if(b(),setTimeout(b,120),setTimeout(b,400),setTimeout(b,1e3),t._vvBound||(t._vvBound=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",b),window.visualViewport.addEventListener("scroll",b)),window.addEventListener("resize",b)),function(){const I=e.querySelector("#match-field .terrain-wrapper svg")||e.querySelector(".terrain-wrapper svg");if(!I)return;const G=I.closest("#match-terrain-wrap");G&&(G.style.cssText="position:relative;width:100%;height:100%;padding:0"),I.removeAttribute("width"),I.removeAttribute("height"),I.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",I.setAttribute("preserveAspectRatio","xMidYMid meet")}(),t._resizeBound||(t._resizeBound=!0,window.addEventListener("resize",()=>{const v=e.querySelector(".terrain-wrapper svg");if(v){const I=v.closest("#match-terrain-wrap");I&&(I.style.cssText="position:relative;width:100%;height:100%;padding:0"),v.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0"}})),t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase==="attack"||t.phase==="defense"){let v=!1,I=30;const G=()=>document.getElementById("match-timer"),re=()=>{const ae=G();if(!ae)return;const se=String(Math.floor(I/60)).padStart(2,"0"),de=String(I%60).padStart(2,"0");ae.textContent=` ${se}:${de}`,ae.style.color=v?"#ff2222":"#ff9500",ae.style.fontWeight="900"};re(),t._timerInt=setInterval(()=>{if(I--,I<0)if(!v)v=!0,I=15,re();else{clearInterval(t._timerInt),t._timerInt=null,t.homeScore=0,t.aiScore=3;const ae=document.createElement("div");ae.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;color:#fff;padding:24px;text-align:center",ae.innerHTML='<div style="font-size:56px">⏱️</div><div style="font-size:24px;font-weight:900;color:#ff4444">MATCH PERDU PAR FORFAIT</div><div style="font-size:14px;color:rgba(255,255,255,0.6)">Temps écoulé</div>',document.body.appendChild(ae),setTimeout(()=>{ae.remove(),Dt(e,t,i)},2500)}else re()},1e3)}(y=document.getElementById("match-quit"))==null||y.addEventListener("click",()=>{We(e),confirm("Abandonner ? Résultat : défaite 3-0")&&(t.homeScore=0,t.aiScore=3,Dt(e,t,i))}),(j=document.getElementById("view-ai"))==null||j.addEventListener("click",()=>er(t,i)),(E=document.getElementById("toggle-history"))==null||E.addEventListener("click",()=>{var v;(v=document.getElementById("match-history-panel"))==null||v.classList.add("open")}),(P=document.getElementById("close-history"))==null||P.addEventListener("click",()=>{var v;(v=document.getElementById("match-history-panel"))==null||v.classList.remove("open")}),(N=document.getElementById("btn-action"))==null||N.addEventListener("click",()=>{t.selected.length!==0&&(u?Uo(e,t,i):x&&Ko(e,t,i))}),(ne=document.getElementById("btn-results"))==null||ne.addEventListener("click",()=>Dt(e,t,i)),(ie=document.getElementById("btn-pass"))==null||ie.addEventListener("click",()=>{t.log.push({text:"⏭️ Vous passez votre tour (plus d'attaquants)",type:"info"}),t.phase="ai-attack",Ee(e,t,i),setTimeout(()=>qi(e,t,i),800)}),e.querySelectorAll(".match-slot-hit").forEach(v=>{v.addEventListener("click",()=>Ho(v,t,e,i))}),e.querySelectorAll(".match-used-hit").forEach(v=>{v.addEventListener("click",()=>bi(e,t,i,null,v.dataset.cardId))}),e.querySelectorAll(".gc-mini").forEach(v=>{u?v.addEventListener("click",()=>Xo(v.dataset.gcId,v.dataset.gcType,e,t,i)):(v.style.opacity="0.35",v.style.cursor="default",v.addEventListener("click",()=>_t("⚡ Les Game Changers ne sont utilisables qu'en attaque","rgba(180,100,0,0.9)")))}),(Q=document.getElementById("boost-card"))==null||Q.addEventListener("click",()=>Zo(e,t,i)),e.querySelectorAll(".sub-btn-col").forEach(v=>{v.addEventListener("click",()=>bi(e,t,i,v.dataset.subId))}),(K=document.getElementById("sub-btn-main"))==null||K.addEventListener("click",()=>bi(e,t,i))}function Ho(e,t,i,n){const r=e.dataset.cardId,o=e.dataset.role,a=t.selected.findIndex(s=>s.cardId===r);if(a!==-1)t.selected.splice(a,1);else{if(t.selected.length>=3){n.toast("Maximum 3 joueurs","error");return}const s=(t.homeTeam[o]||[]).find(l=>l.cardId===r);s&&t.selected.push({...s,_role:o,_line:o})}Ee(i,t,n)}function Bi(e,t,i){e.matchId&&w.from("matches").update({last_player_id:i}).eq("id",e.matchId).then(()=>{})}function Uo(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),Bi(t,i,i.state.profile.id);const n=t.selected.map(o=>{const a=(t.homeTeam[o._role]||[]).find(l=>l.cardId===o.cardId)||o,s=["GK","DEF"].includes(o._role);return{...a,_line:o._role,...s?{note_a:Math.max(1,Number(a.note_a)||0)}:{}}}),r=Vt(n,t.modifiers.home,t.formation);t.pendingAttack={...r,players:[...n],side:"home"},t.selected.forEach(o=>{const a=(t.homeTeam[o._role]||[]).find(s=>s.cardId===o.cardId);a&&(a.used=!0)}),t.log.push({text:`⚔️ Vous attaquez : ${r.total} (base ${r.base}${r.links?` +${r.links} liens`:""}) — ${t.selected.map(o=>o.name).join(", ")}`,type:"info"}),t.selected=[],t.modifiers.home={},t.phase="ai-defense",Ee(e,t,i),setTimeout(()=>Wo(e,t,i),1200)}function Ko(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),Bi(t,i,i.state.profile.id);const n=t.stadiumDef||null,r=t.selected.map(l=>{const c=(t.homeTeam[l._role]||[]).find(u=>u.cardId===l.cardId)||l,d=c.stadiumBonus||n&&(n.club_id&&String(c.club_id)===String(n.club_id)||n.country_code&&c.country_code===n.country_code)||!1;return{...c,_line:l._role,stadiumBonus:d}}),o=Yt(r,t.modifiers.home,t.formation);t.selected.forEach(l=>{const c=(t.homeTeam[l._role]||[]).find(d=>d.cardId===l.cardId);c&&(c.used=!0)});const a=Ci(t.pendingAttack.total,o.total,t.modifiers.home),s={type:"duel",title:"Défense",aiPlayers:(t.pendingAttack.players||[]).map(l=>Ve(l)),homePlayers:t.selected.map(l=>{const c=(t.homeTeam[l._role]||[]).find(d=>d.cardId===l.cardId)||l;return Ve(c)}),homeTotal:o.total,aiTotal:t.pendingAttack.total,isGoal:!1,homeScored:!1,text:""};if(a.shielded)s.text="🛡️ Bouclier ! But annulé.",t.log.push(s);else if(a.goal){t.aiScore++,s.isGoal=!0,s.homeScored=!1,s.text=`⚽ BUT IA ! (${t.pendingAttack.total} > ${o.total})`,t.log.push(s),t.selected=[],t.modifiers.home={},t.pendingAttack=null,Ee(e,t,i),ot(s.aiPlayers,t.homeScore,t.aiScore,!1,()=>{Tt(e,t,i,"home-attack")});return}else s.text=`🧤 Défense réussie ! (${o.total} ≥ ${t.pendingAttack.total})`,t.log.push(s);t.selected=[],t.modifiers.home={},t.pendingAttack=null,Tt(e,t,i,"home-attack")}function Vo(e){if(e.aiSubsUsed>=e.aiMaxSubs)return;const t=Object.values(e.aiTeam).flat().filter(l=>l.used);if(!t.length)return;const i=(e.aiSubs||[]).filter(l=>!e.aiUsedSubIds.includes(l.cardId));if(!i.length)return;const n=t[Math.floor(Math.random()*t.length)],r=i.find(l=>l.job===n.job)||i[0],o={...r,used:!1,_line:n._line,_col:n._col},a=e.aiTeam[n._line],s=a.findIndex(l=>l.cardId===n.cardId);s!==-1&&(a[s]=o),e.aiUsedSubIds.push(r.cardId),e.aiSubsUsed++,e.log.push({text:`🔄 IA : ${r.firstname} ${r.name} remplace ${n.firstname} ${n.name}`,type:"info"})}function Yo(e){var n;if(!((n=e.aiGcCards)!=null&&n.length))return;const t=e.aiGcCards.filter(r=>!e.aiUsedGc.includes(r.id));if(!t.length||Math.random()>.3)return;const i=t[Math.floor(Math.random()*t.length)];switch(e.aiUsedGc.push(i.id),Ae[i.gc_type],i.gc_type){case"Boost+2":{const r=Object.values(e.aiTeam).flat().filter(o=>!o.used);if(r.length){const o=r[Math.floor(Math.random()*r.length)];o.boost=(o.boost||0)+2}break}case"Boost+3":{const r=Object.values(e.aiTeam).flat().filter(o=>!o.used);if(r.length){const o=r[Math.floor(Math.random()*r.length)];o.boost=(o.boost||0)+3}break}case"Remplacement+":e.aiMaxSubs=(e.aiMaxSubs||3)+1;break;case"Bouclier":e.modifiers.ai.shield=!0;break;case"Nul+1":e.modifiers.ai.drawBonus=(e.modifiers.ai.drawBonus||0)+1;break}e.log.push({text:`⚡ IA joue ${i.icon||"⚡"} ${i.name}`,type:"gc"})}function qi(e,t,i){Bi(t,i,null),Vo(t),Yo(t);let n=[...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]].filter(d=>!d.used),r=!1;n.length||(n=[...t.aiTeam.GK||[],...t.aiTeam.DEF||[]].filter(d=>!d.used),r=!0);const o=un(n,"attack",t.difficulty);if(!o.length){Ei(e,t,i);return}r&&o.forEach(d=>{d._line=d._line||d.job,d.note_a=Math.max(1,Number(d.note_a)||0)});const a=Vt(o,t.modifiers.ai,t.formation);t.pendingAttack={...a,players:o,side:"ai"},o.forEach(d=>{d.used=!0}),t.log.push({text:`🤖 IA attaque : ${a.total} (${o.map(d=>d.name).join(", ")})`,type:"info"}),t.modifiers.ai={};const s=[...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(d=>!d.used),c=(t.homeSubs||[]).filter(d=>!(t.usedSubIds||[]).includes(d.cardId)).length>0&&t.subsUsed<t.maxSubs;if(s.length===0&&!c){if(o.length===1&&(o[0]._line==="GK"||o[0].job==="GK")&&$i(t.homeTeam)&&t.homeScore===t.aiScore){t.aiScore++,si(e,t,i,"ai",o[0]);return}t.aiScore++;const u={type:"duel",isGoal:!0,homeScored:!1,aiPlayers:o.map(x=>Ve(x)),aiTotal:a.total,text:"⚽ BUT IA ! (aucun défenseur disponible)"};t.log.push(u),t.pendingAttack=null,Ee(e,t,i),ot(u.aiPlayers,t.homeScore,t.aiScore,!1,()=>{Tt(e,t,i,"home-attack")});return}t.phase="defense",Ee(e,t,i)}function Wo(e,t,i){var c,d;const n=[...t.aiTeam.GK||[],...t.aiTeam.DEF||[],...t.aiTeam.MIL||[]],r=un(n,"defense",t.difficulty);if(!["GK","DEF","MIL","ATT"].flatMap(u=>(t.aiTeam[u]||[]).filter(x=>!x.used)).length){const u=((c=t.pendingAttack)==null?void 0:c.players)||[];if(u.length===1&&(u[0]._line==="GK"||u[0].job==="GK")&&t.homeScore===t.aiScore){t.homeScore++,si(e,t,i,"home",u[0]);return}t.homeScore++;const m={type:"duel",isGoal:!0,homeScored:!0,homePlayers:u.map(f=>Ve(f)),homeTotal:((d=t.pendingAttack)==null?void 0:d.total)||0,aiTotal:0,text:"⚽ BUT ! L'IA n'a plus de joueurs — but automatique !"};t.log.push(m),t.modifiers.ai={},t.pendingAttack=null,Ee(e,t,i),ot(m.homePlayers,t.homeScore,t.aiScore,!0,()=>{Tt(e,t,i,"ai-attack")});return}const a=r.length>0?Yt(r,t.modifiers.ai,t.formation).total:0;r.forEach(u=>{u.used=!0});const s=Ci(t.pendingAttack.total,a,t.modifiers.ai),l={type:"duel",title:"Attaque",homePlayers:(t.pendingAttack.players||[]).map(u=>Ve(u)),aiPlayers:r.map(u=>Ve(u)),homeTotal:t.pendingAttack.total,aiTotal:a,isGoal:!1,homeScored:!1,text:""};if(s.shielded)l.text="🛡️ Bouclier IA !",t.log.push(l);else if(s.goal){t.homeScore++,l.isGoal=!0,l.homeScored=!0,l.text=`⚽ BUT ! (${t.pendingAttack.total} > ${a})`,t.log.push(l),t.modifiers.ai={},t.pendingAttack=null,Ee(e,t,i),ot(l.homePlayers,t.homeScore,t.aiScore,!0,()=>{Tt(e,t,i,"ai-attack")});return}else l.text=`🧤 IA défend (${a} ≥ ${t.pendingAttack.total})`,t.log.push(l);t.modifiers.ai={},t.pendingAttack=null,Tt(e,t,i,"ai-attack")}function Tt(e,t,i,n){if(t.round++,!En(e,t,i)){if(Di(t)){Dt(e,t,i);return}if(n==="home-attack"){if(!["MIL","ATT","GK","DEF"].some(o=>(t.homeTeam[o]||[]).some(a=>!a.used))){Ei(e,t,i);return}t.phase="attack",Ee(e,t,i)}else{if(!["MIL","ATT","GK","DEF"].some(o=>(t.aiTeam[o]||[]).some(a=>!a.used))){Ei(e,t,i);return}t.phase="ai-attack",Ee(e,t,i),setTimeout(()=>qi(e,t,i),800)}}}function $i(e){return!["GK","DEF","MIL","ATT"].some(t=>(e[t]||[]).some(i=>!i.used))}function en(e){const t=(e.GK||[]).some(n=>!n.used),i=["DEF","MIL","ATT"].some(n=>(e[n]||[]).some(r=>!r.used));return t&&!i}function En(e,t,i){if(t.homeScore!==t.aiScore)return!1;if(en(t.homeTeam)&&$i(t.aiTeam)){const n=(t.homeTeam.GK||[]).find(r=>!r.used);return n?(n.used=!0,t.homeScore++,si(e,t,i,"home",n),!0):!1}if(en(t.aiTeam)&&$i(t.homeTeam)){const n=(t.aiTeam.GK||[]).find(r=>!r.used);return n?(n.used=!0,t.aiScore++,si(e,t,i,"ai",n),!0):!1}return!1}function si(e,t,i,n,r){t.log.push({type:"duel",isGoal:!0,homeScored:n==="home",homePlayers:n==="home"?[Ve(r)]:[],aiPlayers:n==="ai"?[Ve(r)]:[],text:`⚽ DERNIER CORNER — Le gardien ${n==="home"?"":"adverse "}marque !`});const o=document.createElement("div");o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:950;display:flex;align-items:center;justify-content:center;overflow:hidden;text-align:center;padding:24px",document.body.appendChild(o);const a=(s,l)=>new Promise(c=>{o.innerHTML=`<div style="font-size:32px;font-weight:900;color:${l};letter-spacing:2px;animation:lcFade 1.4s ease both">${s}</div>
    <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(c,1400)});(async()=>(await a("⚽ DERNIER CORNER","#FFD700"),await a("🧤 LE GARDIEN MONTE !","#4fc3f7"),o.remove(),t.pendingAttack=null,Ee(e,t,i),ot([Ve(r)],t.homeScore,t.aiScore,n==="home",()=>{if(Di(t)){Dt(e,t,i);return}Tt(e,t,i,n==="home"?"ai-attack":"home-attack")})))()}function Di(e){const t=["MIL","ATT","GK","DEF"].some(n=>(e.homeTeam[n]||[]).some(r=>!r.used)),i=["MIL","ATT","GK","DEF"].some(n=>(e.aiTeam[n]||[]).some(r=>!r.used));return!t&&!i}function Ei(e,t,i){En(e,t,i)||(Di(t)?Dt(e,t,i):(t.phase="attack",Ee(e,t,i)))}function bi(e,t,i,n=null,r=null){var m,f;if(t.phase!=="attack"){_t("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){_t(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const o=Object.entries(t.homeTeam).flatMap(([p,g])=>(g||[]).filter(h=>h.used).map(h=>({...h,_line:h._line||p}))),a=t.homeSubs.filter(p=>!t.usedSubIds.includes(p.cardId));if(!o.length){_t("Aucun joueur utilisé à remplacer");return}if(!a.length){_t("Aucun remplaçant disponible");return}let s=Math.max(0,o.findIndex(p=>p.cardId===r));const l=((m=o[s])==null?void 0:m._line)||((f=o[s])==null?void 0:f.job);let c=n?Math.max(0,a.findIndex(p=>p.cardId===n)):Math.max(0,a.findIndex(p=>p.job===l)),d=!1;const u=document.createElement("div");u.id="sub-overlay",u.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function x(){var y,j,E,P,N,ne;const p=o[s],g=a[c],h=Math.min(130,Math.round((window.innerWidth-90)/2)),_=Math.round(h*1.35),b=ie=>`background:rgba(255,255,255,0.12);border:none;color:${ie?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${ie?"default":"pointer"};flex-shrink:0`;u.innerHTML=`
    ...
    <div style="flex:1;display:flex;gap:0;overflow:hidden">
      ...
      <div id="in-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
        ...
        <div>${g?mt({...g,used:!1,boost:0},h,_):"<div>—</div>"}</div>
      ...
      <div id="out-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
        ...
        <button id="out-down" style="${b(s>=o.length-1)}" ${s>=o.length-1?"disabled":""}>▼</button>
        <div style="font-size:10px;color:rgba(255,255,255,0.35)">${s+1}/${o.length}</div>
      </div>
    </div>
    <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
      <button id="sub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
    </div>`,(y=u.querySelector("#sub-close"))==null||y.addEventListener("click",()=>u.remove()),(j=u.querySelector("#out-up"))==null||j.addEventListener("click",()=>{s>0&&(s--,x())}),(E=u.querySelector("#out-down"))==null||E.addEventListener("click",()=>{s<o.length-1&&(s++,x())}),(P=u.querySelector("#in-up"))==null||P.addEventListener("click",()=>{c>0&&(c--,x())}),(N=u.querySelector("#in-down"))==null||N.addEventListener("click",()=>{c<a.length-1&&(c++,x())});const $=(ie,Q,K,v)=>{const I=u.querySelector("#"+ie);if(!I)return;let G=0;I.addEventListener("touchstart",re=>{G=re.touches[0].clientY},{passive:!0}),I.addEventListener("touchend",re=>{const ae=re.changedTouches[0].clientY-G;if(Math.abs(ae)<30)return;const se=Q();ae<0&&se<v-1?(K(se+1),x()):ae>0&&se>0&&(K(se-1),x())},{passive:!0})};$("in-panel",()=>c,ie=>c=ie,a.length),$("out-panel",()=>s,ie=>s=ie,o.length),(ne=u.querySelector("#sub-confirm"))==null||ne.addEventListener("click",ie=>{if(ie.preventDefault(),ie.stopPropagation(),d)return;d=!0;const Q=o[s],K=a[c];if(!Q||!K)return;let v=null,I=-1;for(const[re,ae]of Object.entries(t.homeTeam)){const se=(ae||[]).findIndex(de=>de.cardId===Q.cardId);if(se!==-1){v=re,I=se;break}}if(I===-1||!v){_t("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),u.remove();return}const G={...K,_line:v,_col:Q._col||0,used:!1,boost:0};t.homeTeam[v].splice(I,1,G),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(K.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:Q.name,firstname:Q.firstname,note:Ge(Q,v),portrait:Wt(Q),job:Q.job,country_code:Q.country_code,rarity:Q.rarity,clubName:Q.clubName,clubLogo:Q.clubLogo},inPlayer:{name:K.name,firstname:K.firstname,note:Ge(K,v),portrait:Wt(K),job:K.job,country_code:K.country_code,rarity:K.rarity,clubName:K.clubName,clubLogo:K.clubLogo},text:`🔄 ${K.firstname} ${K.name} remplace ${Q.firstname} ${Q.name}`}),u.remove(),Fo(Q,K,()=>Ee(e,t,i))})}document.body.appendChild(u),x()}function Xo(e,t,i,n,r){var f,p;const o=(n.gcDefs||[]).find(g=>g.name===t),a=Ae[t]||{icon:"⚡",desc:"Carte spéciale."},s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[o==null?void 0:o.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",l={purple:"#b06ce0",light_blue:"#00d4ef"}[o==null?void 0:o.color]||"#b06ce0",c=(o==null?void 0:o.name)||t,d=(o==null?void 0:o.effect)||a.desc,u=o!=null&&o.image_url?`/icons/${o.image_url}`:null,x=a.icon||"⚡",m=document.createElement("div");m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",m.innerHTML=`
    ...
    </div>`,document.body.appendChild(m),(f=m.querySelector("#gc-back"))==null||f.addEventListener("click",()=>m.remove()),(p=m.querySelector("#gc-use"))==null||p.addEventListener("click",()=>{m.remove(),Qo(e,t,i,n,r)})}function ti(e,t,i,n,r,o){const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let s=[];function l(){var c,d;a.innerHTML=`
    ...
    <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start;justify-content:center">
      ${e.map(u=>{const x=u._line||u.job||"MIL",m=s.find(p=>p.cardId===u.cardId),f=Se({...u,_evolution_bonus:0},{width:90,showStad:!0,role:x,extraNote:u.boost||0});return`<div class="gc-pick-item" data-cid="${u.cardId}"
    ...
    <div style="display:flex;flex-direction:column;gap:6px">
      ${n.map(r=>`
        <div class="player-boost-opt" data-card-id="${r.cardId}"
          ...
          <div style="width:32px;height:32px;background:${Co[r.job]||"#888"};border-radius:6px;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:900;font-size:13px">${Ge(r,r._line||r.job)}</div>
          <div style="flex:1"><b>${r.firstname} ${r.name}</b><div style="font-size:11px;color:#888">${r._line||r.job}</div></div>
          <div style="color:#87CEEB;font-weight:700">+${t.boostCard.value}</div>
        </div>`).join("")}
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(r=>{r.addEventListener("click",()=>{const o=r.dataset.cardId;for(const a of["GK","DEF","MIL","ATT"]){const s=(t.homeTeam[a]||[]).find(l=>l.cardId===o);if(s){s.boost=(s.boost||0)+t.boostCard.value,t.log.push({text:`⚡ Boost +${t.boostCard.value} appliqué à ${s.name}`,type:"info"});break}}t.boostUsed=!0,i.closeModal(),Ee(e,t,i)})})}async function Dt(e,t,i){var d,u;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:n}=i,r=t.homeScore>t.aiScore,o=t.homeScore===t.aiScore,a=r?"victoire":o?"nul":"defaite",s=Zn(t.mode,a);t.matchId&&await w.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:r?n.profile.id:null,home_credits_reward:s,played_at:new Date().toISOString()}).eq("id",t.matchId);const l={credits:(n.profile.credits||0)+s,matches_played:(n.profile.matches_played||0)+1};r?l.wins=(n.profile.wins||0)+1:o?l.draws=(n.profile.draws||0)+1:l.losses=(n.profile.losses||0)+1,await w.from("users").update(l).eq("id",n.profile.id),await i.refreshProfile();const c=document.createElement("div");c.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",c.innerHTML=`
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      ...
      ${ci(e.aiTeam,e.formation,null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.1))}
    </div>`,`<button class="btn btn-primary" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const jt=e=>_n(e);async function zt(e,t,i,n,r={}){return Ln(e,t,i,n,r.myGC||[],r.gcDefs||[],r.isRanked||!1,r.rankedData||null,r.stadiumDef||null,r.onMatchEnd||null,r.mlLeagueId||null,r.mlMatchId||null)}async function tr(e,t,i){const{data:n}=await w.from("matches").select("home_id,away_id,mode,is_ranked").eq("id",i).single();if(!n){t.toast("Match introuvable","error"),t.navigate("home");return}const r=n.home_id===t.state.user.id;let o=null,a=null;if(n.mode==="mini_league"){const{data:s}=await w.from("mini_league_matches").select("id, league_id").eq("match_id",i).maybeSingle();s&&(o=s.league_id,a=s.id)}return Ln(e,t,i,r,[],[],n.is_ranked||!1,null,null,null,o,a)}async function Ln(e,t,i,n,r=[],o=[],a=!1,s=null,l=null,c=null,d=null,u=null){const{state:x,navigate:m,toast:f}=t,p=n?"p1":"p2",g=n?"p2":"p1",h=(r||[]).map(k=>k.id),_=(r||[]).map(k=>({id:k.id,gc_type:k.gc_type,_gcDef:k._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:b}=await w.from("matches").select("*").eq("id",i).single();if(!b){f("Match introuvable","error"),m("home");return}async function $(){var me,xe;console.log("[PvP] buildGameState match:",{id:b.id,home_deck_id:b.home_deck_id,away_deck_id:b.away_deck_id,mode:b.mode,is_ranked:b.is_ranked});const[{data:k,error:L},{data:z,error:S},{data:T},{data:A}]=await Promise.all([w.rpc("get_deck_for_match",{p_deck_id:b.home_deck_id}),w.rpc("get_deck_for_match",{p_deck_id:b.away_deck_id}),w.from("users").select("id,pseudo,club_name").eq("id",b.home_id).single(),w.from("users").select("id,pseudo,club_name").eq("id",b.away_id).single()]);console.log("[PvP] get_deck_for_match p1:",L==null?void 0:L.message,"p2:",S==null?void 0:S.message,"p1D:",(me=k==null?void 0:k.starters)==null?void 0:me.length,"p2D:",(xe=z==null?void 0:z.starters)==null?void 0:xe.length);const D=te=>{const Te=Number(te.evolution_bonus)||0;return{cardId:te.card_id,position:te.position,id:te.id,firstname:te.firstname,name:te.surname_real,country_code:te.country_code,club_id:te.club_id,job:te.job,job2:te.job2,note_g:(Number(te.note_g)||0)+(te.job==="GK"||te.job2==="GK"&&Number(te.note_g)>0?Te:0),note_d:(Number(te.note_d)||0)+(te.job==="DEF"||te.job2==="DEF"&&Number(te.note_d)>0?Te:0),note_m:(Number(te.note_m)||0)+(te.job==="MIL"||te.job2==="MIL"&&Number(te.note_m)>0?Te:0),note_a:(Number(te.note_a)||0)+(te.job==="ATT"||te.job2==="ATT"&&Number(te.note_a)>0?Te:0),evolution_bonus:Te,rarity:te.rarity,skin:te.skin,hair:te.hair,hair_length:te.hair_length,face:te.face||null,clubName:te.club_encoded_name||null,clubLogo:te.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},B=((k==null?void 0:k.starters)||[]).map(te=>D(te)),U=((z==null?void 0:z.starters)||[]).map(te=>D(te)),Y=(k==null?void 0:k.formation)||"4-4-2",oe=(z==null?void 0:z.formation)||"4-4-2";let Z=ri(B,Y),H=ri(U,oe);const le=((k==null?void 0:k.subs)||[]).map(te=>D(te)),pe=((z==null?void 0:z.subs)||[]).map(te=>D(te)),ue=(k==null?void 0:k.stadium_def)||(n?l:null),ce=(z==null?void 0:z.stadium_def)||(n?null:l);return ue&&(Z=Xt(Z,ue),oi(le,ue)),ce&&(H=Xt(H,ce),oi(pe,ce)),{p1Team:Z,p2Team:H,p1Subs:le,p2Subs:pe,p1Formation:Y,p2Formation:oe,p1Name:(T==null?void 0:T.club_name)||(T==null?void 0:T.pseudo)||"Joueur 1",p2Name:(A==null?void 0:A.club_name)||(A==null?void 0:A.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:n?h:[],gc_p2:n?[]:h,gcCardsFull_p1:n?_:[],gcCardsFull_p2:n?[]:_,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:o||[],lastActionAt:new Date().toISOString()}}let y=b.game_state&&Object.keys(b.game_state).length?b.game_state:null;if(console.log("[PvP] init - amIHome:",n,"gameState exists:",!!y,"match.status:",b.status,"home_id:",b.home_id,"away_id:",b.away_id,"myId:",x.profile.id),!y)if(n){y=await $(),console.log("[PvP] home - gameState built, p1Team keys:",Object.keys((y==null?void 0:y.p1Team)||{}));const{data:k}=await w.from("matches").select("game_state").eq("id",i).single();!(k!=null&&k.game_state)||!Object.keys(k.game_state).length?await w.from("matches").update({game_state:y,turn_user_id:b.home_id}).eq("id",i):y=k.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>',console.log("[PvP] away - waiting for home to write game_state...");for(let k=0;k<30&&!y;k++){await new Promise(z=>setTimeout(z,400));const{data:L}=await w.from("matches").select("game_state").eq("id",i).single();L!=null&&L.game_state&&Object.keys(L.game_state).length&&(y=L.game_state),k%5===0&&console.log("[PvP] away - poll",k,"game_state:",!!(L!=null&&L.game_state))}if(!y){f("Erreur de synchronisation","error"),m("home");return}y.gc_p2=h,y.gcCardsFull_p2=_,await w.from("matches").update({game_state:y}).eq("id",i)}let j=!1,E=null,P=!1;const N=new Set,ne=new Set;async function ie(k){var oe,Z;try{w.removeChannel(Q)}catch{}const L=y[p+"Score"]||0,z=y[g+"Score"]||0;let S,T;k.winner_id?(S=k.winner_id===x.profile.id,T=!1):k.forfeit?(S=L>z,T=!1):(T=L===z,S=L>z);let A="";if(a&&n)try{const{data:H}=await w.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",b.home_id).single(),{data:le}=await w.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",b.away_id).single();if(H&&le){const pe=S?1:T?.5:0,ue=1-pe,ce=H.placement_matches<10,me=le.placement_matches<10,xe=computeGlicko2(H.mmr,H.mmr_deviation,H.mmr_volatility,le.mmr,le.mmr_deviation,pe===1?1:pe===0?0:.5,ce),te=computeGlicko2(le.mmr,le.mmr_deviation,le.mmr_volatility,H.mmr,H.mmr_deviation,ue===1?1:ue===0?0:.5,me);await w.rpc("update_mmr_after_ranked",{p_match_id:i,p_winner_id:T?null:S?b.home_id:b.away_id,p_home_id:b.home_id,p_away_id:b.away_id,p_home_delta:xe.delta,p_away_delta:te.delta,p_home_new_rd:xe.newRd,p_away_new_rd:te.newRd,p_home_new_vol:xe.newSigma,p_away_new_vol:te.newSigma});const Te=xe.delta,Fe=xe.newMmr,Ie=getTier(Fe),He=Te>=0?"+":"",It=Te>=0?"#4caf50":"#ff6b6b",Je=getTier(H.mmr);A=`
            ...
            </div>`}}catch(H){console.error("[Ranked] MMR update error:",H)}(oe=document.getElementById("pvp-end-overlay"))==null||oe.remove();const D=document.createElement("div");D.id="pvp-end-overlay",D.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;color:#fff;padding:24px;text-align:center;overflow-y:auto";const B=T?"🤝":S?"🏆":"😞",U=T?"MATCH NUL":S?"VICTOIRE !":"DÉFAITE",Y=T?"#fff":S?"#FFD700":"#ff6b6b";D.innerHTML=`
      <div style="font-size:64px">${B}</div>
      <div style="font-size:26px;font-weight:900;color:${Y}">${U}</div>
      ...
      ${A}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${a?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(D),(Z=D.querySelector("#pvp-end-home"))==null||Z.addEventListener("click",()=>{D.remove(),We(e),m(a?"ranked":"home")})}const Q=w.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},k=>{const L=k.new;try{if(L.status==="finished"||L.forfeit){if(j)return;j=!0,E&&(clearInterval(E),E=null),L.game_state&&(y=L.game_state),ie(L);return}if(L.game_state){const z=y;y=L.game_state;const S=y._lastGC;if(S&&S.seq&&!ne.has(S.seq)&&(ne.add(S.seq),S.by!==p)){R(S.type,S.by,()=>de());return}const T=z[p+"Score"]||0,A=z[g+"Score"]||0,D=y[p+"Score"]||0,B=y[g+"Score"]||0,U=D>T,Y=B>A;if((U||Y)&&!N.has(y.round)){N.add(y.round);const oe=[...y.log||[]].reverse().find(H=>H.isGoal),Z=((oe==null?void 0:oe.homePlayers)||[]).map(H=>({name:H.name,note:H.note,portrait:H.portrait,job:H.job}));ot(Z,D,B,U,()=>de());return}de()}}catch(z){console.error("[PvP] crash:",z)}}).subscribe();async function K(k){Object.assign(y,k),y.lastActionAt=new Date().toISOString();const{error:L}=await w.from("matches").update({game_state:y}).eq("id",i);L&&f("Erreur de synchronisation","error");try{de()}catch(z){console.error("[PvP] renderPvpScreen crash:",z)}}async function v(){if(j)return;j=!0,E&&(clearInterval(E),E=null);const k=n?b.away_id:b.home_id,L=n?"p2":"p1",z=n?"p1":"p2",S={...y,[L+"Score"]:3,[z+"Score"]:0,phase:"finished"},T=S.p1Score||0,A=S.p2Score||0;try{await w.from("matches").update({game_state:S}).eq("id",i);const{data:D,error:B}=await w.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:k,p_home_score:T,p_away_score:A,p_forfeit:!0});B&&console.error("[PvP] finish_pvp_match (forfeit):",B);try{const{data:U}=await w.rpc("apply_match_rewards",{p_match_id:i});U!=null&&U.success&&!(U!=null&&U.skipped)&&typeof t.refreshProfile=="function"&&await t.refreshProfile()}catch(U){console.error("[PvP] apply_match_rewards (forfeit):",U)}}catch{}try{w.removeChannel(Q)}catch{}setTimeout(()=>{We(e),d?m("mini-league",{openLeagueId:d}):m("home")},800)}const I={BOOST_STAT:({value:k=1,count:L=1,roles:z=[]},S,T)=>{const A=S[p+"Team"],D=Object.entries(A).filter(([B])=>!z.length||z.includes(B)).flatMap(([B,U])=>U.filter(Y=>!Y.used).map(Y=>({...Y,_line:B})));if(!D.length){S.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),T(S);return}G(D,L,`Choisir ${L} joueur(s) à booster (+${k})`,B=>{B.forEach(U=>{const Y=(A[U._line]||[]).find(oe=>oe.cardId===U.cardId);Y&&(Y.boost=(Y.boost||0)+k,S.log.push({text:`⚡ +${k} sur ${Y.name}`,type:"info"}))}),S[p+"Team"]=A,T(S)})},DEBUFF_STAT:({value:k=1,count:L=1,roles:z=[],target:S="ai"},T,A)=>{const D=S==="home"?p:g,B=T[D+"Team"],U=S==="home"?"allié":"adverse",Y=Object.entries(B).filter(([oe])=>!z.length||z.includes(oe)).flatMap(([oe,Z])=>Z.map(H=>({...H,_line:oe})));if(!Y.length){T.log.push({text:`🎯 Aucun joueur ${U}`,type:"info"}),A(T);return}G(Y,L,`Choisir ${L} joueur(s) ${U}(s) (-${k})`,oe=>{oe.forEach(Z=>{const H=(B[Z._line]||[]).find(le=>le.cardId===Z.cardId);H&&(H.boost=(H.boost||0)-k,T.log.push({text:`🎯 -${k} sur ${H.name}`,type:"info"}))}),T[D+"Team"]=B,A(T)})},GRAY_PLAYER:({count:k=1,roles:L=[],target:z="ai"},S,T)=>{const A=z==="home"?p:g,D=S[A+"Team"],B=z==="home"?"allié":"adverse",U=Object.entries(D).filter(([Y])=>!L.length||L.includes(Y)).flatMap(([Y,oe])=>oe.filter(Z=>!Z.used).map(Z=>({...Z,_line:Y})));if(!U.length){S.log.push({text:`❌ Aucun joueur ${B}`,type:"info"}),T(S);return}G(U,k,`Choisir ${k} joueur(s) ${B}(s) à exclure`,Y=>{const oe="usedCardIds_"+A,Z=new Set(S[oe]||[]);Y.forEach(H=>{const le=(D[H._line]||[]).find(pe=>pe.cardId===H.cardId);le&&(le.used=!0,Z.add(H.cardId),S.log.push({text:`❌ ${le.name} exclu !`,type:"info"}))}),S[oe]=[...Z],S[A+"Team"]=D,T(S)})},REVIVE_PLAYER:({count:k=1,roles:L=[]},z,S)=>{const T=z[p+"Team"],A=Object.entries(T).filter(([D])=>!L.length||L.includes(D)).flatMap(([D,B])=>B.filter(U=>U.used).map(U=>({...U,_line:D})));if(!A.length){z.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),S(z);return}G(A,k,`Choisir ${k} joueur(s) à ressusciter`,D=>{D.forEach(B=>{const U=(T[B._line]||[]).find(Y=>Y.cardId===B.cardId);U&&(U.used=!1,z.log.push({text:`💫 ${U.name} ressuscité !`,type:"info"}))}),z[p+"Team"]=T,S(z)})},REMOVE_GOAL:({},k,L)=>{const z=g+"Score";k[z]>0?(k[z]--,k.log.push({text:"🚫 Dernier but annulé !",type:"info"})):k.log.push({text:"🚫 Aucun but à annuler",type:"info"}),L(k)},ADD_GOAL_DRAW:({},k,L)=>{k[p+"Score"]===k[g+"Score"]?(k[p+"Score"]++,k.log.push({text:"🎯 But bonus !",type:"info"})):k.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),L(k)},ADD_SUB:({value:k=1},L,z)=>{L.maxSubs=(L.maxSubs||3)+k,L.log.push({text:`🔄 +${k} remplacement(s)`,type:"info"}),z(L)},CUSTOM:({},k,L)=>L(k)};function G(k,L,z,S){const T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let A=[];function D(){var U,Y;const B=k.map(oe=>{const Z=oe._line||oe.job||"MIL",H=A.find(pe=>pe.cardId===oe.cardId),le=Se({...oe,_evolution_bonus:0},{width:90,showStad:!0,role:Z,extraNote:oe.boost||0});return`<div class="pp-item" data-cid="${oe.cardId}"
        ...
        </div>`,(U=T.querySelector("#pp-close"))==null||U.addEventListener("click",()=>T.remove()),T.querySelectorAll(".pp-item").forEach(oe=>{oe.addEventListener("click",()=>{const Z=oe.dataset.cid,H=k.find(pe=>pe.cardId===Z),le=A.findIndex(pe=>pe.cardId===Z);H&&(le>-1?A.splice(le,1):A.length<L&&A.push(H),D())})}),(Y=T.querySelector("#pp-confirm"))==null||Y.addEventListener("click",()=>{T.remove(),S(A)})}D(),document.body.appendChild(T)}async function re(k,L){const S=(y["gcCardsFull_"+p]||[]).find(B=>B.id===k),T=(S==null?void 0:S._gcDef)||(y.gcDefs||[]).find(B=>{var U;return B.name===L||((U=B.name)==null?void 0:U.toLowerCase().trim())===(L==null?void 0:L.toLowerCase().trim())}),A=[...y["usedGc_"+p]||[],k],D={type:L,by:p,seq:(y._gcAnimSeq||0)+1};ne.add(D.seq),R(L,p,async()=>{if(T!=null&&T.effect_type&&T.effect_type!=="CUSTOM"){const U=I[T.effect_type];if(U){const Y={...y};U(T.effect_params||{},Y,async oe=>{oe["usedGc_"+p]=A,oe._lastGC=D,oe._gcAnimSeq=D.seq,await K(oe)});return}}const B={...y};switch(L){case"Remplacement+":B.maxSubs=(B.maxSubs||3)+1,B.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const U=g+"Score";B[U]>0&&(B[U]--,B.log.push({text:"🚫 But annulé",type:"info"}));break}}B["usedGc_"+p]=A,B._lastGC=D,B._gcAnimSeq=D.seq,await K(B)})}function ae(k,L){const z="usedCardIds_"+k,S=new Set(y[z]||[]);L.forEach(T=>S.add(T)),y[z]=[...S]}function se(){for(const k of["p1","p2"]){const L=new Set(y["usedCardIds_"+k]||[]),z=y[k+"Team"];if(z)for(const S of["GK","DEF","MIL","ATT"])(z[S]||[]).forEach(T=>{T.used=L.has(T.cardId)})}}function de(){var at,st,vt,Pi,Gi,Ri;if(y.phase==="reveal")return ke();if(y.phase==="midfield")return J();if(y.phase==="finished")return Rt();const k=y[p+"Team"];y[g+"Team"],se();const L=y[p+"Score"],z=y[g+"Score"],S=y[p+"Name"],T=y[g+"Name"],A=y.phase===p+"-attack",D=y.phase===p+"-defense",B=Array.isArray(y["selected_"+p])?y["selected_"+p]:[],U=B.map(ee=>ee.cardId),Y=window.innerWidth>=700,oe=y[p+"Subs"]||[],Z=y["usedSubIds_"+p]||[],H=oe.filter(ee=>!Z.includes(ee.cardId)),le=y["gcCardsFull_"+p]||[],pe=y["usedGc_"+p]||[],ue=le.filter(ee=>!pe.includes(ee.id)),ce=y.boostOwner===p&&!y.boostUsed,me=[...k.MIL||[],...k.ATT||[]].filter(ee=>!ee.used),xe=A&&me.length===0?[...k.GK||[],...k.DEF||[]].filter(ee=>!ee.used).map(ee=>ee.cardId):[];function te(ee,fe,ze){var At,lt;const he=ee._gcDef,St={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[he==null?void 0:he.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",et={purple:"#b06ce0",light_blue:"#00d4ef"}[he==null?void 0:he.color]||"#b06ce0",De=(he==null?void 0:he.name)||ee.gc_type,Ne=(he==null?void 0:he.effect)||((At=Ae[ee.gc_type])==null?void 0:At.desc)||"",Me=he!=null&&he.image_url?`/icons/${he.image_url}`:null,wt=((lt=Ae[ee.gc_type])==null?void 0:lt.icon)||"⚡",Pe=Math.round(ze*.22),dt=Math.round(ze*.22),tt=ze-Pe-dt,Mt=De.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${ee.id}" data-gc-type="${ee.gc_type}"
        ...
        <div style="height:${Pe}px;background:rgba(255,255,255,0.14);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 3px">
          <span style="font-size:${Mt}px;font-weight:900;color:#fff;text-align:center;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${fe-6}px">${De}</span>
          <span style="font-size:6px;color:rgba(255,255,255,0.45)">⚡ GC</span>
        </div>
        <div style="height:${tt}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
          ${Me?`<img src="${Me}" style="max-width:${fe-10}px;max-height:${tt-6}px;object-fit:contain">`:`<span style="font-size:${Math.round(tt*.55)}px">${wt}</span>`}
        </div>
    ...
    </div>`,qe=A?"attack":D?"defense":"idle",Qe=`<div style="overflow:hidden;min-width:0;flex:1;min-height:0;display:flex;flex-direction:column" id="match-field">
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${Pt(k,y[p+"Formation"],qe,U,Y?1300:Lt(),Y?600:qt(),xe)}
      </div>
    </div>`,Re=y[p+"Team"],ht=(()=>{var fe,ze,he,St,et;if(D&&((fe=y.pendingAttack)!=null&&fe.players)){const De=y.pendingAttack;let Ne="";if(B.length>0){const Me=B.map(Pe=>{const dt=(Re[Pe._role]||[]).find(fi=>fi.cardId===Pe.cardId)||Pe,tt=Re[Pe._role]||[],Mt=tt.findIndex(fi=>fi.cardId===Pe.cardId),At=pt(tt.length),lt=Mt>=0?At[Mt]:dt._col??1;return{...dt,_line:Pe._role,_col:lt}}),wt=Yt(Me,((ze=y.modifiers)==null?void 0:ze[p])||{},y[p+"Formation"]);Ne=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
            ...
            <div style="display:flex;justify-content:center">${Ye(Me.map(Pe=>({...Pe,used:!1})),"#3a7bd5",wt.total,"defense",y[p+"Formation"])}</div>
          </div>`}return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b;text-align:center">
          ...
          <div style="display:flex;justify-content:center">${Ye((De.players||[]).map(Me=>({...Me,used:!1})),"#ff6b6b",De.total,"attack",y[g+"Formation"])}</div>
          ${Ne}
        ...
        </div>`}if(A&&!((St=y.pendingAttack)!=null&&St.players)&&B.length>0){const De=B.map(Me=>{const wt=(Re[Me._role]||[]).find(lt=>lt.cardId===Me.cardId)||Me,Pe=["GK","DEF"].includes(Me._role),dt=Re[Me._role]||[],tt=dt.findIndex(lt=>lt.cardId===Me.cardId),Mt=pt(dt.length),At=tt>=0?Mt[tt]:wt._col??1;return{...wt,_line:Me._role,_col:At,...Pe?{note_a:Math.max(1,Number(wt.note_a)||0)}:{}}}),Ne=Vt(De,((et=y.modifiers)==null?void 0:et[p])||{},y[p+"Formation"]);return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #FFD700;text-align:center">
          ...
          <div style="display:flex;justify-content:center">${Ye(De.map(Me=>({...Me,used:!1})),"#FFD700",Ne.total,"attack",y[p+"Formation"])}</div>
      ...
      <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
        ...
        <div id="mobile-play-area" style="flex:1;min-height:0;display:flex;overflow:hidden">
          <div id="match-field" style="flex:1;min-width:0;min-height:0;overflow:hidden">
            <div class="terrain-wrapper" style="width:100%;height:100%;overflow:hidden">
              ${Pt(k,y[p+"Formation"],qe,U,Lt(),qt(),xe)}
            </div>
          </div>
        </div>
        <div id="mobile-action-bar" style="position:absolute;left:0;right:0;bottom:0;z-index:20;background:rgba(0,0,0,0.55);padding:6px 8px 8px;display:flex;flex-direction:column;gap:6px;box-shadow:0 -4px 16px rgba(0,0,0,0.5)">
      ...
      </div>`;function Ue(){const ee=e.querySelector(".match-screen");if(!ee)return;const fe=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);ee.style.height=fe+"px",ee.style.minHeight=fe+"px",ee.style.maxHeight=fe+"px",ee.style.overflow="hidden";const ze=e.querySelector("#mobile-action-bar"),he=e.querySelector("#mobile-play-area");ze&&he&&(he.style.paddingBottom=ze.offsetHeight+"px")}if(Ue(),setTimeout(Ue,120),setTimeout(Ue,400),P||(P=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",Ue),window.visualViewport.addEventListener("scroll",Ue)),window.addEventListener("resize",Ue)),function(){const fe=e.querySelector("#match-field .terrain-wrapper svg")||e.querySelector(".terrain-wrapper svg");if(!fe)return;const ze=fe.closest("#match-terrain-wrap");ze&&(ze.style.cssText="position:relative;width:100%;height:100%;padding:0"),fe.removeAttribute("width"),fe.removeAttribute("height"),fe.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",fe.setAttribute("preserveAspectRatio","xMidYMid meet")}(),y._pvpResizeBound||(y._pvpResizeBound=!0,window.addEventListener("resize",()=>{const ee=e.querySelector(".terrain-wrapper svg");if(ee){const fe=ee.closest("#match-terrain-wrap");fe&&(fe.style.cssText="position:relative;width:100%;height:100%;padding:0"),ee.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0"}})),e.querySelectorAll(".match-slot-hit").forEach(ee=>{ee.addEventListener("click",()=>{if(!A&&!D)return;const fe=ee.dataset.cardId,ze=ee.dataset.role,he=(k[ze]||[]).find(Ne=>Ne.cardId===fe);if(!he||he.used)return;const St=xe.includes(fe);if(A&&!["MIL","ATT"].includes(ze)&&!St)return;Array.isArray(y["selected_"+p])||(y["selected_"+p]=[]);const et=y["selected_"+p],De=et.findIndex(Ne=>Ne.cardId===fe);De>-1?et.splice(De,1):et.length<3&&et.push({...he,_role:ze}),de()})}),e.querySelectorAll(".match-used-hit").forEach(ee=>{ee.addEventListener("click",()=>F(ee.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(ee=>{ee.addEventListener("click",()=>F())}),(at=e.querySelector("#pvp-sub-open"))==null||at.addEventListener("click",()=>F()),e.querySelectorAll(".pvp-gc-mini").forEach(ee=>{A?ee.addEventListener("click",()=>M(ee.dataset.gcId,ee.dataset.gcType)):(ee.style.opacity="0.35",ee.style.cursor="default",ee.addEventListener("click",()=>_t("⚡ Les Game Changers ne sont utilisables qu'en attaque","rgba(180,100,0,0.9)")))}),(st=e.querySelector("#pvp-boost-card"))==null||st.addEventListener("click",()=>C()),(vt=e.querySelector("#pvp-action"))==null||vt.addEventListener("click",ee=>{A?ee.currentTarget.dataset.pass==="1"||!je(p)?O():be():D&&$e()}),(Pi=e.querySelector("#pvp-quit"))==null||Pi.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&v()}),(Gi=e.querySelector("#pvp-view-opp"))==null||Gi.addEventListener("click",()=>V()),(Ri=e.querySelector("#pvp-toggle-history"))==null||Ri.addEventListener("click",()=>q()),E&&(clearInterval(E),E=null),(A||D)&&!j){let ee=30,fe=!1;const ze=()=>document.getElementById("pvp-timer"),he=()=>{ze()&&(ze().textContent=ee+"s",ze().style.color=fe?"#ff4444":"#fff")};he(),E=setInterval(()=>{ee--,ee<0?fe?(clearInterval(E),E=null,A&&!je(p)?O():v()):(fe=!0,ee=15,he()):he()},1e3)}}function ke(){nt(e),e.innerHTML=`
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="width:200px;border-radius:18px;border:3px solid ${A};background:${T};display:flex;flex-direction:column;overflow:hidden;animation:gcFlipIn .7s cubic-bezier(.34,1.56,.64,1) both,gcGlow 1.8s ease infinite .7s">
        ...
        <div style="height:170px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
          ${U?`<img src="${U}" style="max-width:160px;max-height:160px;object-fit:contain">`:`<span style="font-size:76px">${Y}</span>`}
        </div>
        <div style="padding:12px;background:rgba(0,0,0,0.38);text-align:center">
          <div style="font-size:12px;color:rgba(255,255,255,0.92);line-height:1.5">${B}</div>
        </div>
      </div>
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(H);let le=!1;const pe=()=>{le||(le=!0,H.remove(),setTimeout(()=>z&&z(),50))};H.addEventListener("click",pe),setTimeout(pe,3e3)}function M(k,L){var H,le,pe,ue;const S=(y["gcCardsFull_"+p]||[]).find(ce=>ce.id===k),T=S==null?void 0:S._gcDef,A={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[T==null?void 0:T.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",D={purple:"#b06ce0",light_blue:"#00d4ef"}[T==null?void 0:T.color]||"#b06ce0",B=(T==null?void 0:T.name)||L,U=(T==null?void 0:T.effect)||((H=Ae[L])==null?void 0:H.desc)||"Carte spéciale.",Y=T!=null&&T.image_url?`/icons/${T.image_url}`:null,oe=((le=Ae[L])==null?void 0:le.icon)||"⚡",Z=document.createElement("div");Z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",Z.innerHTML=`
      ...
      </div>`,document.body.appendChild(z),(S=z.querySelector("#bp-close"))==null||S.addEventListener("click",()=>z.remove()),z.querySelectorAll(".bp-item").forEach(T=>{T.addEventListener("click",async()=>{const A=T.dataset.cid,D=L.find(U=>U.cardId===A);if(!D)return;const B=(k[D._line]||[]).find(U=>U.cardId===A);B&&(B.boost=(B.boost||0)+y.boostValue),z.remove(),await K({[p+"Team"]:k,boostUsed:!0})})})}function F(k=null){var le,pe;if(!(y.phase===p+"-attack")){f("Remplacement uniquement avant votre attaque","warning");return}const z=y[p+"Team"],S=y["usedSubIds_"+p]||[],T=y.maxSubs||3;if(S.length>=T){f(`Maximum ${T} remplacements atteint`,"warning");return}const A=Object.entries(z).flatMap(([ue,ce])=>(ce||[]).filter(me=>me.used).map(me=>({...me,_line:ue}))),D=(y[p+"Subs"]||[]).filter(ue=>!S.includes(ue.cardId));if(!A.length){f("Aucun joueur utilisé à remplacer","warning");return}if(!D.length){f("Aucun remplaçant disponible","warning");return}let B=Math.max(0,A.findIndex(ue=>ue.cardId===k));const U=((le=A[B])==null?void 0:le._line)||((pe=A[B])==null?void 0:pe.job);let Y=Math.max(0,D.findIndex(ue=>ue.job===U)),oe=!1;const Z=document.createElement("div");Z.id="pvp-sub-overlay",Z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function H(){var Fe,Ie,He,It,Je,Qt;const ue=A[B],ce=D[Y],me=Math.min(130,Math.round((window.innerWidth-90)/2)),xe=Math.round(me*1.35),te=qe=>`background:rgba(255,255,255,0.12);border:none;color:${qe?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${qe?"default":"pointer"};flex-shrink:0`;Z.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        <div id="pin-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
          ...
          <button id="pin-down" style="${te(Y>=D.length-1)}" ${Y>=D.length-1?"disabled":""}>▼</button>
          <div style="font-size:10px;color:rgba(255,255,255,0.35)">${Y+1}/${D.length}</div>
        </div>
      ...
      </div>`,(Fe=Z.querySelector("#psub-close"))==null||Fe.addEventListener("click",()=>Z.remove()),(Ie=Z.querySelector("#pout-up"))==null||Ie.addEventListener("click",()=>{B>0&&(B--,H())}),(He=Z.querySelector("#pout-down"))==null||He.addEventListener("click",()=>{B<A.length-1&&(B++,H())}),(It=Z.querySelector("#pin-up"))==null||It.addEventListener("click",()=>{Y>0&&(Y--,H())}),(Je=Z.querySelector("#pin-down"))==null||Je.addEventListener("click",()=>{Y<D.length-1&&(Y++,H())});const Te=(qe,Qe,Re,ht)=>{const Ze=Z.querySelector("#"+qe);if(!Ze)return;let Ue=0;Ze.addEventListener("touchstart",at=>{Ue=at.touches[0].clientY},{passive:!0}),Ze.addEventListener("touchend",at=>{const st=at.changedTouches[0].clientY-Ue;if(Math.abs(st)<30)return;const vt=Qe();st<0&&vt<ht-1?(Re(vt+1),H()):st>0&&vt>0&&(Re(vt-1),H())},{passive:!0})};Te("pin-panel",()=>Y,qe=>Y=qe,D.length),Te("pout-panel",()=>B,qe=>B=qe,A.length),(Qt=Z.querySelector("#psub-confirm"))==null||Qt.addEventListener("click",async qe=>{if(qe.preventDefault(),qe.stopPropagation(),oe)return;oe=!0;const Qe=A[B],Re=D[Y];if(!Qe||!Re)return;const ht=Qe._line,Ze=(z[ht]||[]).findIndex(st=>st.cardId===Qe.cardId);if(Ze===-1){f("Erreur : joueur introuvable","error"),Z.remove();return}const Ue={...Re,_line:ht,position:Qe.position,used:!1,boost:0};z[ht].splice(Ze,1,Ue);const at=[...S,Re.cardId];Z.remove(),X(Qe,Re,async()=>{await K({[p+"Team"]:z,[g+"Team"]:y[g+"Team"],["usedSubIds_"+p]:at})})})}document.body.appendChild(Z),H()}function X(k,L,z){const S={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const A=(U,Y,oe)=>`<div style="text-align:center">
      <div style="font-size:9px;color:${Y};letter-spacing:2px;text-transform:uppercase;font-weight:700;margin-bottom:6px">${oe}</div>
      <div style="width:70px;height:70px;border-radius:50%;background:${S[U.job]||"#555"};border:3px solid ${Y};position:relative;overflow:hidden;margin:0 auto">
        ${Wt(U)?`<img src="${Wt(U)}" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover">`:""}
      </div>
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:6px">Appuyer pour continuer</div>`,document.body.appendChild(T);let D=!1;const B=()=>{D||(D=!0,T.remove(),setTimeout(()=>z(),50))};T.addEventListener("click",B),setTimeout(B,2200)}function V(){var L;const k=document.createElement("div");k.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:20px;overflow-y:auto",k.innerHTML=`
      <div style="font-size:12px;color:rgba(255,255,255,0.5);letter-spacing:2px;text-transform:uppercase">Équipe adverse</div>
      <div style="font-size:18px;font-weight:900;color:#ff6b6b">${y[g+"Name"]}</div>
      <div style="width:min(90vw,420px)">${ci(y[g+"Team"],y[g+"Formation"],null,[],Lt(),qt())}</div>
      ...
      </div>`,document.body.appendChild(L),(z=L.querySelector("#pvp-hist-close"))==null||z.addEventListener("click",()=>L.remove())}async function O(){if(y.phase!==p+"-attack")return;const k=p==="p1"?"p2":"p1",L=(y.round||0)+1,z=[...y.log||[]];z.push({type:"info",text:`⏭️ ${y[p+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const S=Oe(y),T=je(k),A=S||!T?"finished":k+"-attack";await K({["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:A,attacker:k,round:L,log:z}),A==="finished"&&await Xe(y)}function we(k){const L=(k.GK||[]).some(S=>!S.used),z=["DEF","MIL","ATT"].some(S=>(k[S]||[]).some(T=>!T.used));return L&&!z}function _e(k){return(k.p1Score||0)!==(k.p2Score||0)?null:we(k.p1Team)&&!ge(k.p2Team)?{side:"p1",gk:(k.p1Team.GK||[]).find(L=>!L.used)}:we(k.p2Team)&&!ge(k.p1Team)?{side:"p2",gk:(k.p2Team.GK||[]).find(L=>!L.used)}:null}async function ye(k,L){const{side:z,gk:S}=k;if(!S)return;const T=z+"Score",A=(L[T]||0)+1;S.used=!0;const D=[...L.log||[]];D.push({type:"duel",isGoal:!0,homeScored:z===p,homePlayers:z==="p1"?[jt(S)]:[],aiPlayers:z==="p2"?[jt(S)]:[],text:`⚽ DERNIER CORNER — Le gardien de ${L[z+"Name"]} marque !`});const B=(L.round||0)+1,U=z==="p1"?"p2":"p1",Y={...L,[T]:A},oe=Oe(Y);N.add(B);const Z=document.createElement("div");Z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:950;display:flex;align-items:center;justify-content:center;overflow:hidden;text-align:center;padding:24px",document.body.appendChild(Z);const H=(ue,ce)=>new Promise(me=>{Z.innerHTML=`<div style="font-size:32px;font-weight:900;color:${ce};letter-spacing:2px;animation:lcFade 1.4s ease both">${ue}</div>
      <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(me,1400)});await H("⚽ DERNIER CORNER","#FFD700"),await H("🧤 LE GARDIEN MONTE !","#4fc3f7"),Z.remove();const le=z===p?A:L[p+"Score"]||0,pe=z===p?L[g+"Score"]||0:A;ot([jt(S)],le,pe,z===p,async()=>{await K({[T]:A,log:D,round:B,pendingAttack:null,phase:oe?"finished":U+"-attack",attacker:U,["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}}}),oe&&await Xe({...y,[T]:A})})}async function be(){const k=y[p+"Team"],L=!["GK","DEF","MIL","ATT"].some(A=>(y[g+"Team"][A]||[]).some(D=>!D.used)),z=(y["selected_"+p]||[]).map(A=>{const D=(k[A._role]||[]).find(pe=>pe.cardId===A.cardId)||A,B=["GK","DEF"].includes(A._role),U=k[A._role]||[],Y=U.findIndex(pe=>pe.cardId===A.cardId),oe=pt(U.length),Z=Y>=0?oe[Y]:D._col??1,H=y.stadiumDef||y.homeStadiumDef||null,le=D.stadiumBonus||H&&(H.club_id&&String(D.club_id)===String(H.club_id)||H.country_code&&D.country_code===H.country_code)||!1;return{...D,_line:A._role,_col:Z,stadiumBonus:le,...B?{note_a:Math.max(1,Number(D.note_a)||0)}:{}}});if(!z.length)return;const S=Vt(z,y.modifiers[p]||{},y[p+"Formation"]);ae(p,z.map(A=>A.cardId)),z.forEach(A=>{const D=(k[A._role]||[]).find(B=>B.cardId===A.cardId);D&&(D.used=!0)}),y["selected_"+p]=[],de();const T=[...y.log||[]];if(L){if(z.length===1&&(z[0]._line==="GK"||z[0].job==="GK")&&(y.p1Score||0)===(y.p2Score||0)){await ye({side:p,gk:z[0]},{...y,[p+"Team"]:k,log:T});return}const D=(y[p+"Score"]||0)+1,B=z.map(H=>jt(H));T.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:B,homeTotal:S.total,aiTotal:0});const U=(y.round||0)+1,Y=p==="p1"?"p2":"p1",oe={...y,[p+"Team"]:k,[p+"Score"]:D},Z=Oe(oe);N.add(U),ot(B,D,y[g+"Score"]||0,!0,async()=>{await K({[p+"Team"]:k,[p+"Score"]:D,["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:Z?"finished":Y+"-attack",attacker:Y,round:U,log:T}),Z&&await Xe({...y,[p+"Score"]:D})});return}T.push({type:"pending",text:`⚔️ ${y[p+"Name"]} attaque (${S.total})`}),await K({[p+"Team"]:k,[g+"Team"]:y[g+"Team"],pendingAttack:{...S,players:z,side:p},["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},phase:g+"-defense",log:T})}async function $e(){const k=y[p+"Team"],L=(y["selected_"+p]||[]).map(ce=>{const me=(k[ce._role]||[]).find(Ie=>Ie.cardId===ce.cardId)||ce,xe=k[ce._role]||[],te=xe.findIndex(Ie=>Ie.cardId===ce.cardId),Te=pt(xe.length),Fe=te>=0?Te[te]:me._col??1;return{...me,_line:ce._role,_col:Fe}}),z=Yt(L,y.modifiers[p]||{},y[p+"Formation"]);ae(p,L.map(ce=>ce.cardId)),L.forEach(ce=>{const me=(k[ce._role]||[]).find(xe=>xe.cardId===ce.cardId);me&&(me.used=!0)}),y["selected_"+p]=[],de();const S=Ci(y.pendingAttack.total,z.total,y.modifiers[p]||{}),T=y.pendingAttack.side,A=S==="attack"||(S==null?void 0:S.goal),D=T==="p1"?"p2":"p1",B=(y.round||0)+1,U=(y.pendingAttack.players||[]).map(ce=>jt(ce)),Y=[...y.log||[]];Y.push({type:"duel",isGoal:A,homeScored:A&&T===p,text:A?`⚽ BUT de ${y[T+"Name"]} ! (${y.pendingAttack.total} vs ${z.total})`:`✋ Attaque stoppée (${y.pendingAttack.total} vs ${z.total})`,homePlayers:U,aiPlayers:L.map(ce=>jt(ce)),homeTotal:y.pendingAttack.total,aiTotal:z.total});const oe=A?(y[T+"Score"]||0)+1:y[T+"Score"]||0,Z={...y,[p+"Team"]:k,[T+"Score"]:oe},H=Oe(Z),le=H?"finished":D+"-attack",pe=!H&&!A?_e(Z):null,ue=async()=>{if(pe){await ye(pe,Z);return}await K({[p+"Team"]:k,[g+"Team"]:y[g+"Team"],[T+"Score"]:oe,["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:le,attacker:D,round:B,log:Y}),(le==="finished"||H)&&await Xe({...y,[T+"Score"]:oe})};if(A){const ce=T===p,me=ce?oe:y[p+"Score"]||0,xe=ce?y[g+"Score"]||0:oe;N.add(B),ot(U,me,xe,ce,ue)}else await ue()}function Le(k){return["MIL","ATT"].some(L=>(k[L]||[]).some(z=>!z.used))}function ge(k){return["GK","DEF","MIL","ATT"].some(L=>(k[L]||[]).some(z=>!z.used))}function Be(k){return ge(k)&&!Le(k)}function je(k){const L=y[k+"Team"];return!!(Le(L)||Be(L))}function Oe(k){return!ge(k.p1Team)&&!ge(k.p2Team)}function bt(k){const L=k.p1Score||0,z=k.p2Score||0;return L===z?null:L>z?b.home_id:b.away_id}async function Xe(k){try{const L=bt(k),z=L?b.home_id===L?b.away_id:b.home_id:null,S=k.p1Score||0,T=k.p2Score||0,{data:A,error:D}=await w.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:L,p_home_score:S,p_away_score:T,p_forfeit:!1});D&&console.error("[PvP] finish_pvp_match:",D),L&&z&&io(L,z).catch(()=>{});try{const{data:B}=await w.rpc("apply_match_rewards",{p_match_id:i});B!=null&&B.success&&!(B!=null&&B.skipped)&&typeof t.refreshProfile=="function"&&await t.refreshProfile()}catch(B){console.error("[PvP] apply_match_rewards:",B)}if(typeof c=="function")try{await c({homeScore:S,awayScore:T,winnerId:L,homeId:b.home_id,awayId:b.away_id,matchId:i})}catch(B){console.error("[PvP] onMatchEnd:",B)}}catch(L){console.error("[PvP] finishMatch:",L)}}function Rt(){var T;const k=y[p+"Score"],L=y[g+"Score"],z=k>L,S=k===L;nt(e),e.innerHTML=`
    ...
    </div>`,(T=document.getElementById("pvp-home"))==null||T.addEventListener("click",()=>{try{w.removeChannel(Q)}catch{}We(e),d?m("mini-league",{openLeagueId:d}):m("home")})}de()}async function Li(e,t,i=!1){var s,l;const{state:n,navigate:r,toast:o}=t,a=i&&((l=(s=t==null?void 0:t.state)==null?void 0:s.params)==null?void 0:l.rankedData)||null;await pi(e,t,i?"ranked":"random",async({deckId:c,formation:d,starters:u,subsRaw:x,gcCardsEnriched:m,gcDefs:f,stadiumDef:p})=>{nt(e);const g=async h=>{await nr(e,t,c,d,u,x,h||[],f,p,i,a)};if(!(m!=null&&m.length)){await g([]);return}li(e,m,g)})}async function ir(e,t,i){return tr(e,t,i)}async function nr(e,t,i,n,r,o,a=[],s=[],l=null,c=!1,d=null){var ie,Q;const{state:u,navigate:x,toast:m}=t,f=(d==null?void 0:d.mmr)??((ie=u.profile)==null?void 0:ie.mmr)??1e3,p=c?Mi(f):null,g=c?"linear-gradient(135deg, #1a0a2e 0%, #2d1060 50%, #1a0a2e 100%)":"linear-gradient(135deg, #0a1a2e 0%, #0d3d1e 100%)",h=c?(p==null?void 0:p.color)||"#D4A017":"#FFD700";let _=!1,b=null,$=null;e.innerHTML=`
    ...
    </div>`;const y=async()=>{_=!0,b&&(b.unsubscribe(),b=null),$&&(clearInterval($),$=null);try{await w.rpc("cancel_matchmaking",{p_user_id:u.profile.id})}catch(K){console.error("[Matchmaking] cancel error:",K)}};(Q=document.getElementById("cancel-mm"))==null||Q.addEventListener("click",async()=>{await y(),We(e),x("home")});const j=async(K,v)=>{_||(_=!0,b&&(b.unsubscribe(),b=null),$&&(clearInterval($),$=null),await zt(e,t,K,v,{myGC:a,gcDefs:s,stadiumDef:l,isRanked:c,rankedData:d,onMatchEnd:c?or:null}))},E=u.user.id,{data:P,error:N}=c?await w.rpc("try_matchmake_ranked",{p_user_id:E,p_deck_id:i,p_mmr:f}):await w.rpc("try_matchmake",{p_user_id:E,p_deck_id:i});if(N||!(P!=null&&P.success)){console.error("[Matchmaking] try_matchmake error:",N||P),m("Erreur de matchmaking","error"),We(e),x("home");return}if(P.matched){await j(P.match_id,!1);return}const ne=async()=>{if(_)return;const{data:K}=await w.from("matchmaking_queue").select("status, match_id").eq("user_id",E).maybeSingle();(K==null?void 0:K.status)==="matched"&&K.match_id&&await j(K.match_id,!0)};b=w.channel(`mm_${E}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${E}`},K=>{const v=K.new;v.status==="matched"&&v.match_id&&j(v.match_id,!0)}).subscribe(),$=setInterval(ne,3e3),setTimeout(()=>{_||y()},12e4)}async function or(e,t){const{state:i,toast:n}=t,{winnerId:r,homeId:o,awayId:a,homeScore:s,awayScore:l,matchId:c}=e,d=i.user.id,u=o===d,x=r===d,{data:m}=await w.from("users").select("mmr, mmr_rd, mmr_v").eq("id",d).single(),{data:f}=await w.from("users").select("mmr, mmr_rd, mmr_v").eq("id",u?a:o).single();if(m&&f){const p=jn(m.mmr,m.mmr_rd,m.mmr_v,f.mmr,f.mmr_rd,x?1:0);await w.from("users").update({mmr:p.mmr,mmr_rd:p.rd,mmr_v:p.v,mmr_wins:x?w.sql`mmr_wins + 1`:void 0,mmr_losses:x?void 0:w.sql`mmr_losses + 1`}).eq("id",d),n(x?`+MMR ↑ ${Math.round(p.mmr-m.mmr)}`:`-MMR ↓ ${Math.round(m.mmr-p.mmr)}`,x?"success":"error",4e3)}}const rr=Object.freeze(Object.defineProperty({__proto__:null,renderMatchRandom:Li,resumePvpMatch:ir},Symbol.toStringTag,{value:"Module"}));async function ar(e,t){const{state:i,navigate:n,toast:r}=t,o=i.params||{},a=o.friendId||null,s=o.friendName||"Ami",l=!!o.isAccepting;await pi(e,t,"friend",async({deckId:c,formation:d,starters:u,subsRaw:x,gcCardsEnriched:m,gcDefs:f,stadiumDef:p})=>{nt(e);const g=async h=>{const _=h||[];l?await dr(e,t,c,_,f,p):await sr(e,t,c,_,f,p,a,s)};if(!(m!=null&&m.length)){await g([]);return}li(e,m,g)})}async function sr(e,t,i,n,r,o,a,s){var _;const{state:l,navigate:c,toast:d}=t,u=l.user.id,{data:x,error:m}=await w.from("friend_match_invites").insert({inviter_id:u,invitee_id:a,friend_id:a,status:"pending",inviter_deck_id:i,expires_at:new Date(Date.now()+2*60*1e3).toISOString()}).select("id").single();if(m||!x){console.error("[Friend] Erreur création invitation:",m),d("Impossible de créer l'invitation","error"),c("home");return}e.innerHTML=`
    ...
    </div>`;let f=!1,p=null;const g=async()=>{p&&(p.unsubscribe(),p=null)};(_=document.getElementById("cancel-friend"))==null||_.addEventListener("click",async()=>{await g(),await w.from("friend_match_invites").update({status:"declined"}).eq("id",x.id),We(e),c("home")}),p=w.channel(`friend_invite_${x.id}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"friend_match_invites",filter:`id=eq.${x.id}`},async({new:b})=>{f||b.status!=="accepted"||!b.match_id||(f=!0,await g(),await zt(e,t,b.match_id,!0,{myGC:n,gcDefs:r,stadiumDef:o}))}).subscribe();const h=setInterval(async()=>{if(f)return;const{data:b}=await w.from("friend_match_invites").select("status, match_id").eq("id",x.id).single();(b==null?void 0:b.status)==="accepted"&&b.match_id&&(f=!0,clearInterval(h),await g(),await zt(e,t,b.match_id,!0,{myGC:n,gcDefs:r,stadiumDef:o}))},3e3);setTimeout(async()=>{f||(clearInterval(h),await g(),d("Invitation expirée","info"),c("home"))},12e4)}async function dr(e,t,i,n,r,o){const{state:a,navigate:s,toast:l}=t,c=a.user.id,{data:d}=await w.from("friend_match_invites").select("id, inviter_id").eq("invitee_id",c).eq("status","pending").order("created_at",{ascending:!1}).limit(1).maybeSingle();if(!d){l("Aucune invitation en attente","error"),s("home");return}const{data:u,error:x}=await w.rpc("accept_friend_invite",{p_invite_id:d.id,p_invitee_deck_id:i});if(x||!(u!=null&&u.success)){console.error("[Friend] Erreur accept_friend_invite:",x||u),l((u==null?void 0:u.error)||"Impossible de rejoindre le match","error"),s("home");return}await zt(e,t,u.match_id,!1,{myGC:n,gcDefs:r,stadiumDef:o})}const Tn="#1A6B3C",gt="#D4A017",Nt="rgba(255,255,255,0.045)",kt="rgba(255,255,255,0.09)",$t="rgba(255,255,255,0.08)",Ce="#f3f5f2",it="rgba(243,245,242,0.62)",Ke="rgba(243,245,242,0.4)";async function lr(e,t){var n;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const i=(n=t.state.params)==null?void 0:n.openLeagueId;if(i){t.state.params.openLeagueId=null,await rt(e,t,i);return}await yt(e,t)}async function yt(e,t,i="waiting"){var h,_;const{state:n}=t,r=n.profile.id,{data:o}=await w.from("mini_league_members").select("league_id").eq("user_id",r),a=(o||[]).map(b=>b.league_id),{data:s,error:l}=await w.from("mini_leagues").select("*, mini_league_members(count)").eq("status","waiting").eq("is_archived",!1).order("created_at",{ascending:!1}).limit(30),c=l?(await w.from("mini_leagues").select("*, mini_league_members(count)").eq("status","waiting").order("created_at",{ascending:!1}).limit(30)).data||[]:s||[],u=(a.length?await w.from("mini_leagues").select("*, mini_league_members(count)").in("id",a).order("created_at",{ascending:!1}):{data:[]}).data||[],x=u.filter(b=>b.status==="waiting"&&!b.is_archived),m=u.filter(b=>b.status==="active"&&!b.is_archived),f=u.filter(b=>b.is_archived||b.status==="finished"),p=c.filter(b=>!a.includes(b.id)),g=[{key:"waiting",label:"🟡 En attente",count:x.length+p.length},{key:"active",label:"🟢 En cours",count:m.length},{key:"archived",label:"📁 Archivées",count:f.length}];e.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="display:flex;background:var(--nav-bg,#0d1a0f);border-bottom:1px solid ${$t}">
      ${g.map(b=>`<button class="ml-tab" data-tab="${b.key}" style="flex:1;padding:11px 4px;border:none;border-bottom:2px solid ${i===b.key?Tn:"transparent"};background:none;font-size:12px;font-weight:${i===b.key?"900":"600"};color:${i===b.key?"#4ade80":Ke};cursor:pointer">${b.label}${b.count?` (${b.count})`:""}</button>`).join("")}
    </div>
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:10px">
      ${i==="waiting"?cr(x,p,r):i==="active"?pr(m,r):ur(f,r)}
    </div>
  </div>`,(h=document.getElementById("ml-create-btn"))==null||h.addEventListener("click",()=>fr(e,t)),(_=document.getElementById("ml-refresh-list"))==null||_.addEventListener("click",()=>yt(e,t,i)),e.querySelectorAll(".ml-tab").forEach(b=>b.addEventListener("click",()=>yt(e,t,b.dataset.tab))),e.querySelectorAll("[data-league-id]").forEach(b=>b.addEventListener("click",()=>rt(e,t,b.dataset.leagueId))),e.querySelectorAll("[data-join]").forEach(b=>b.addEventListener("click",$=>{$.stopPropagation(),zn(e,t,b.dataset.join,b.dataset.type)})),e.querySelectorAll("[data-delete]").forEach(b=>b.addEventListener("click",$=>{$.stopPropagation(),Ti(e,t,b.dataset.delete,b.dataset.name,i)}))}function di(e,t,i=!1){var a,s;const n=e.creator_id===t,r=e.pot||0,o=((s=(a=e.mini_league_members)==null?void 0:a[0])==null?void 0:s.count)||0;return`<div data-league-id="${e.id}" style="background:${Nt};border:1px solid ${kt};border-radius:12px;padding:14px 16px;cursor:pointer;margin-bottom:8px">
    ...
    <div style="display:flex;flex-wrap:wrap;gap:8px;font-size:11px;color:${it};margin-bottom:${i?"10px":"0"}">
      ...
      ${r>0?`<span style="color:${gt};font-weight:700">🏆 ${r.toLocaleString("fr")} cr.</span>`:""}
    ...
    <div style="padding:20px;display:flex;flex-direction:column;gap:14px">
      ...
      </div>`,document.body.appendChild(t);const i=t.querySelector("#ml-pwd-input"),n=r=>{t.remove(),e(r)};setTimeout(()=>i==null?void 0:i.focus(),80),t.querySelector("#pwd-cancel").addEventListener("click",()=>n(null)),t.querySelector("#pwd-ok").addEventListener("click",()=>n(i.value.trim())),i.addEventListener("keydown",r=>{r.key==="Enter"&&n(i.value.trim())}),t.addEventListener("click",r=>{r.target===t&&n(null)})})}async function zn(e,t,i,n){const{toast:r,state:o}=t,a=o.profile.id,{data:s}=await w.from("mini_leagues").select("password,status,entry_fee,pot,max_players").eq("id",i).single();if(!s){r("Mini League introuvable","error");return}if(s.status!=="waiting"){r("Cette Mini League a déjà démarré","warning");return}const{count:l}=await w.from("mini_league_members").select("id",{count:"exact",head:!0}).eq("league_id",i);if(l>=s.max_players){r("La Mini League est complète","warning");return}if(n==="private"){const p=await gr();if(p===null)return;if(s.password!==p){r("Mot de passe incorrect","error");return}}const c=s.entry_fee||100,{data:d}=await w.from("users").select("credits").eq("id",a).single();if(((d==null?void 0:d.credits)||0)<c){r(`Crédits insuffisants — il te faut ${c.toLocaleString("fr")} cr. (solde : ${((d==null?void 0:d.credits)||0).toLocaleString("fr")} cr.)`,"error");return}const{error:u}=await w.from("mini_league_members").insert({league_id:i,user_id:a});if(u){r("Erreur inscription : "+u.message,"error");return}const{error:x}=await w.from("users").update({credits:d.credits-c}).eq("id",a),{error:m}=await w.from("mini_leagues").update({pot:(s.pot||0)+c}).eq("id",i);(x||m)&&console.error("Erreur post-inscription:",x,m),o.profile&&(o.profile.credits=d.credits-c);const f=document.getElementById("nav-credits");f&&(f.textContent=`💰 ${(o.profile.credits||0).toLocaleString("fr")}`),r(`✅ Inscrit ! −${c.toLocaleString("fr")} cr. · Pot : ${((s.pot||0)+c).toLocaleString("fr")} cr.`,"success"),rt(e,t,i)}async function mr(e,t,i,n){const{toast:r,state:o}=t,a=o.profile.id;if(!confirm(`Te désinscrire et récupérer ${n.toLocaleString("fr")} cr. ?`))return;const{data:s}=await w.from("mini_leagues").select("pot,status").eq("id",i).single();if(!s||s.status!=="waiting"){r("Impossible — la league a déjà démarré","error");return}const{data:l}=await w.from("users").select("credits").eq("id",a).single();await w.from("mini_leagues").update({pot:Math.max(0,(s.pot||0)-n)}).eq("id",i),await w.from("users").update({credits:((l==null?void 0:l.credits)||0)+n}).eq("id",a),await w.from("mini_league_members").delete().eq("league_id",i).eq("user_id",a),o.profile&&(o.profile.credits=((l==null?void 0:l.credits)||0)+n);const c=document.getElementById("nav-credits");c&&(c.textContent=`💰 ${(o.profile.credits||0).toLocaleString("fr")}`),r(`↩️ Désinscrit · +${n.toLocaleString("fr")} cr. remboursés`,"success"),yt(e,t,"waiting")}async function Ti(e,t,i,n,r){const{toast:o}=t;if(!confirm(`Supprimer définitivement "${n}" et tous ses matchs/membres ? Action irréversible.`))return;await w.from("mini_league_matches").delete().eq("league_id",i),await w.from("mini_league_members").delete().eq("league_id",i);const{error:a}=await w.from("mini_leagues").delete().eq("id",i);if(a){o("Erreur suppression ("+a.message+")","error");return}o("Mini League supprimée avec succès","success"),yt(e,t,r)}async function xr(e,t,i){await w.from("mini_leagues").update({is_archived:!0}).eq("id",i),t.toast("Mini League archivée","success"),yt(e,t,"archived")}async function rt(e,t,i){var $,y,j,E,P,N,ne,ie,Q,K;const{state:n,toast:r}=t,o=n.profile.id,[{data:a},{data:s},{data:l}]=await Promise.all([w.from("mini_leagues").select("*").eq("id",i).single(),w.from("mini_league_members").select("*, user:users(id,pseudo,club_name,club_color1,club_color2)").eq("league_id",i),w.from("mini_league_matches").select("*").eq("league_id",i).order("matchday").order("created_at")]);if(!a){r("Introuvable","error"),await yt(e,t);return}const c=(s||[]).some(v=>v.user_id===o),d=a.creator_id===o,u=(s||[]).map(v=>v.user).filter(Boolean),x=Sn(u,l||[]),m=(l||[]).filter(v=>v.matchday===a.current_day),f=a.pot||0,p=a.entry_fee||100,g=m.length>0&&m.every(v=>v.status==="finished"||v.status==="bye"),h=a.current_day>=a.total_days,_=(s||[]).find(v=>v.user_id===o);e.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    <div style="padding:14px 16px;background:var(--nav-bg,#0d1a0f);border-bottom:1px solid ${$t};display:flex;align-items:center;gap:10px">
      ...
      <div style="text-align:right;flex-shrink:0">
        ...
        ${f>0?`<div style="font-size:12px;font-weight:900;color:${gt};margin-top:4px">🏆 ${f.toLocaleString("fr")} cr.</div>`:""}
      </div>
    </div>
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:${Nt};border:1px solid ${kt};border-radius:12px;padding:16px">
        ...
        ${u.map(v=>`
          <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid ${$t}">
            <div style="width:36px;height:36px;border-radius:50%;background:${v.club_color2||Tn};display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:900;color:${v.club_color1||"#fff"}">${(v.pseudo||"?").slice(0,2).toUpperCase()}</div>
      ...
      <div style="background:${Nt};border:1px solid ${kt};border-radius:12px;padding:16px">
        ...
        ${m.map(v=>tn(v,u,o,c)).join("")}
        ${d&&g&&!h?'<button id="ml-next-day" class="btn btn-primary" style="width:100%;margin-top:12px">➡️ Journée suivante</button>':""}
        ${d&&g&&h?'<button id="ml-finish-btn" class="btn btn-primary" style="width:100%;margin-top:12px;background:#7a28b8">🏆 Terminer et distribuer le pot</button>':""}
      </div>`:""}
      ...
      <div style="background:${Nt};border:1px solid ${kt};border-radius:12px;padding:16px">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          <thead><tr style="font-size:10px;color:${Ke};text-transform:uppercase;border-bottom:2px solid ${$t}">
            ...
            ${f>0&&a.status==="finished"?`<th style="text-align:right;padding:5px 0;color:${gt}">💰</th>`:""}
          </tr></thead>
          <tbody>${x.map((v,I)=>{const G=f>0&&a.status==="finished"?I===0?Math.floor(f*.7):I===1?Math.floor(f*.2):I===2?Math.floor(f*.1):0:0;return`<tr style="border-bottom:1px solid ${$t};${v.userId===o?"background:rgba(74,222,128,0.08);":""}">
              <td style="padding:7px 3px 7px 0;font-weight:700;color:${I===0?gt:I<3?"#4ade80":it}">${["🥇","🥈","🥉"][I]||I+1}</td>
              ...
              ${f>0&&a.status==="finished"?`<td style="text-align:right;font-weight:700;color:${gt}">${G?G.toLocaleString("fr")+" cr.":"—"}</td>`:""}
            </tr>`}).join("")}</tbody>
        </table>
      </div>`:""}
      ...
      <div style="background:${Nt};border:1px solid ${kt};border-radius:12px;padding:16px">
        ...
        ${Array.from({length:Math.max(0,a.status==="active"?a.current_day-1:a.current_day)},(v,I)=>I+1).reverse().map(v=>{const I=(l||[]).filter(G=>G.matchday===v);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:${Ke};margin-bottom:6px">Journée ${v}</div>${I.map(G=>tn(G,u,o,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
  ...
  </div>`;const b=a.status==="waiting"?"waiting":a.status==="active"?"active":"archived";if(($=document.getElementById("ml-back"))==null||$.addEventListener("click",()=>yt(e,t,b)),(y=document.getElementById("ml-refresh"))==null||y.addEventListener("click",async v=>{const I=v.currentTarget;I.style.opacity="0.5",await rt(e,t,i)}),(j=document.getElementById("ml-start-btn"))==null||j.addEventListener("click",()=>yr(e,t,a,u)),(E=document.getElementById("ml-next-day"))==null||E.addEventListener("click",()=>br(e,t,i)),(P=document.getElementById("ml-finish-btn"))==null||P.addEventListener("click",()=>In(e,t,i,f,x,u)),(N=document.getElementById("ml-join-now"))==null||N.addEventListener("click",()=>zn(e,t,i,a.type)),(ne=document.getElementById("ml-leave-btn"))==null||ne.addEventListener("click",()=>mr(e,t,i,p)),(ie=document.getElementById("ml-delete-btn"))==null||ie.addEventListener("click",()=>Ti(e,t,i,a.name,"waiting")),(Q=document.getElementById("ml-delete-now"))==null||Q.addEventListener("click",()=>Ti(e,t,i,a.name,b)),(K=document.getElementById("ml-archive-btn"))==null||K.addEventListener("click",()=>xr(e,t,i)),e.querySelectorAll("[data-play-match]").forEach(v=>{v.addEventListener("click",()=>{const I=m.find(G=>G.id===v.dataset.playMatch);I&&t.navigate("match-mini-league",{mlMatchId:I.id,leagueId:i})})}),a.status==="finished"&&_){const v=x.findIndex(I=>I.userId===o);v>=0&&v<3&&_.prize_amount>0&&setTimeout(()=>hr(e,t,a,_,v),400)}}function tn(e,t,i,n,r=!1){const o=u=>t.find(x=>x.id===u);if(e.is_bye){const u=o(e.home_id);return`<div style="padding:8px;border-radius:8px;background:rgba(255,255,255,0.03);margin-bottom:6px;font-size:12px;color:${it};text-align:center">🔵 ${(u==null?void 0:u.club_name)||(u==null?void 0:u.pseudo)||"?"} exempté(e)</div>`}const a=o(e.home_id),s=o(e.away_id),l=e.home_id===i||e.away_id===i,c=l&&e.status==="pending"&&n&&!r,d=e.status==="finished"?`${e.home_score} - ${e.away_score}`:"vs";return`<div style="display:flex;align-items:center;gap:8px;padding:10px;border-radius:8px;background:${l?"rgba(26,107,60,0.16)":"rgba(255,255,255,0.03)"};margin-bottom:6px;border:1px solid ${l?"rgba(74,222,128,0.35)":$t}">
  ...
  </div>`}async function yr(e,t,i,n){const{toast:r,state:o}=t,a=vr(n.map(c=>c.id),i.mode),s=[];a.forEach((c,d)=>c.forEach(u=>s.push({league_id:i.id,matchday:d+1,home_id:u.home||u.bye,away_id:u.away||null,is_bye:!!u.bye,status:u.bye?"bye":"pending"})));const{error:l}=await w.from("mini_league_matches").insert(s);if(l){r("Erreur calendrier : "+l.message,"error");return}await w.from("mini_leagues").update({status:"active",current_day:1,total_days:a.length}).eq("id",i.id),r(`🚀 Lancée ! Pot : ${(i.pot||0).toLocaleString("fr")} cr.`,"success"),rt(e,t,i.id)}async function br(e,t,i){const{data:n}=await w.from("mini_leagues").select("current_day,total_days,pot").eq("id",i).single(),r=(n.current_day||0)+1;if(r>(n.total_days||0)){await In(e,t,i,n.pot||0,null,null);return}await w.from("mini_leagues").update({current_day:r}).eq("id",i),t.toast(`Journée ${r} commencée !`,"success"),rt(e,t,i)}async function In(e,t,i,n,r,o){const{toast:a,state:s}=t;let l=r,c=o;if(!l){const{data:f}=await w.from("mini_league_matches").select("*").eq("league_id",i),{data:p}=await w.from("mini_league_members").select("*, user:users(id,pseudo,club_name)").eq("league_id",i);c=(p||[]).map(g=>g.user).filter(Boolean),l=Sn(c,f||[])}const d=[Math.floor(n*.7),Math.floor(n*.2),Math.floor(n*.1)],u=l.slice(0,3).map(f=>f.userId),{data:x,error:m}=await w.rpc("finish_mini_league",{p_league_id:i,p_winner_ids:u,p_prizes:d});if(m||!(x!=null&&x.success)){console.error("[MiniLeague] finish_mini_league:",m),a("Erreur lors de la distribution du pot","error");return}a("🏆 Mini League terminée ! Les gagnants peuvent récupérer leurs crédits.","success"),rt(e,t,i)}async function hr(e,t,i,n,r){var x,m;const{state:o,toast:a}=t,s=[Math.floor((i.pot||0)*.7),Math.floor((i.pot||0)*.2),Math.floor((i.pot||0)*.1)],l=["🥇","🥈","🥉"][r],c=n.prize_amount||s[r]||0,d=n.prize_claimed,u=document.createElement("div");u.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",u.innerHTML=`
    ...
    </div>`,document.body.appendChild(u),(x=u.querySelector("#prize-close"))==null||x.addEventListener("click",()=>u.remove()),u.addEventListener("click",f=>{f.target===u&&u.remove()}),(m=u.querySelector("#claim-prize-btn"))==null||m.addEventListener("click",async()=>{const{data:f}=await w.from("users").select("credits").eq("id",o.profile.id).single();await w.from("users").update({credits:((f==null?void 0:f.credits)||0)+c}).eq("id",o.profile.id),await w.from("mini_league_members").update({prize_claimed:!0}).eq("league_id",i.id).eq("user_id",o.profile.id),o.profile&&(o.profile.credits=((f==null?void 0:f.credits)||0)+c);const p=document.getElementById("nav-credits");p&&(p.textContent=`💰 ${o.profile.credits.toLocaleString("fr")}`),a(`💰 +${c.toLocaleString("fr")} cr. ajoutés à ton solde !`,"success"),u.remove(),rt(e,t,i.id)})}function vr(e,t){const n=e.length%2===0?[...e]:[...e,null],r=n.length;let o=n.slice(1);const a=[];for(let s=0;s<r-1;s++){const l=[],c=[n[0],...o];for(let d=0;d<r/2;d++){const u=c[d],x=c[r-1-d];u===null?l.push({bye:x}):x===null?l.push({bye:u}):l.push({home:u,away:x})}a.push(l),o=[o[o.length-1],...o.slice(0,-1)]}return t==="aller-retour"?[...a,...a.map(s=>s.map(l=>l.bye?l:{home:l.away,away:l.home}))]:a}function Sn(e,t){const i={};return e.forEach(n=>{i[n.id]={userId:n.id,pseudo:n.pseudo,clubName:n.club_name||n.pseudo,played:0,won:0,drawn:0,lost:0,goalsFor:0,goalsAgainst:0,goalDiff:0,points:0}}),t.filter(n=>n.status==="finished"&&!n.is_bye&&n.home_score!=null).forEach(n=>{const r=i[n.home_id],o=i[n.away_id];!r||!o||(r.played++,o.played++,r.goalsFor+=n.home_score,r.goalsAgainst+=n.away_score,o.goalsFor+=n.away_score,o.goalsAgainst+=n.home_score,n.home_score>n.away_score?(r.won++,r.points+=3,o.lost++):n.home_score<n.away_score?(o.won++,o.points+=3,r.lost++):(r.drawn++,r.points++,o.drawn++,o.points++),r.goalDiff=r.goalsFor-r.goalsAgainst,o.goalDiff=o.goalsFor-o.goalsAgainst)}),Object.values(i).sort((n,r)=>r.points-n.points||r.goalDiff-n.goalDiff||r.goalsFor-n.goalsFor)}async function nn(e,t){const{state:i,navigate:n,toast:r}=t,o=i.params||{},a=o.leagueId||null,s=o.mlMatchId||null,l=i.user.id;if(!s||!a){r("Match introuvable","error"),n("mini-league");return}const{data:c,error:d}=await w.from("mini_league_matches").select("id, league_id, home_id, away_id, status, match_id").eq("id",s).single();if(d||!c){r("Match introuvable","error"),n("mini-league",{openLeagueId:a});return}if(c.home_id!==l&&c.away_id!==l){r("Vous ne faites pas partie de ce match","error"),n("mini-league",{openLeagueId:a});return}const u=c.home_id===l;if(c.match_id){await zt(e,t,c.match_id,u,{mlLeagueId:a,mlMatchId:s});return}await pi(e,t,"mini_league",async({deckId:x,gcCardsEnriched:m,gcDefs:f,stadiumDef:p})=>{nt(e);const g=async h=>{const{data:_,error:b}=await w.rpc("start_mini_league_match",{p_ml_match_id:s,p_user_id:l,p_deck_id:x});if(b||!(_!=null&&_.success)){console.error("[MiniLeague] start_mini_league_match error:",b||_),r((_==null?void 0:_.error)||"Impossible de lancer le match","error"),n("mini-league",{openLeagueId:a});return}if(_.matched){await zt(e,t,_.match_id,u,{mlLeagueId:a,mlMatchId:s,myGC:h||[],gcDefs:f,stadiumDef:p});return}await wr(e,t,{mlMatchId:s,leagueId:a,amIHome:u,chosenGC:h,gcDefs:f,stadiumDef:p})};if(!(m!=null&&m.length)){await g([]);return}li(e,m,g)})}function wr(e,t,{mlMatchId:i,leagueId:n,amIHome:r,chosenGC:o,gcDefs:a,stadiumDef:s}){return new Promise(l=>{var p;const{navigate:c}=t;e.innerHTML=`
    <div style="min-height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;background:linear-gradient(135deg,#0a1a2e,#0d3d1e);color:#fff;padding:32px;text-align:center;gap:20px">
      <div style="font-size:36px">🏆</div>
    ...
    </div>`;let d=!1;const u=async g=>{if(!d){if(d=!0,clearInterval(f),m)try{m.unsubscribe()}catch{}g&&await zt(e,t,g,r,{mlLeagueId:n,mlMatchId:i,myGC:o||[],gcDefs:a,stadiumDef:s}),l()}};(p=document.getElementById("wait-back"))==null||p.addEventListener("click",()=>{if(d=!0,clearInterval(f),m)try{m.unsubscribe()}catch{}c("mini-league",{openLeagueId:n}),l()});const x=async()=>{if(d)return;const{data:g}=await w.from("mini_league_matches").select("match_id").eq("id",i).single();g!=null&&g.match_id&&u(g.match_id)},m=w.channel(`ml_match_${i}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"mini_league_matches",filter:`id=eq.${i}`},g=>{var h;(h=g.new)!=null&&h.match_id&&u(g.new.match_id)}).subscribe(),f=setInterval(x,3e3);x()})}const _r="/",kr=[{emoji:"⚽",title:"Bienvenue dans Manager Wars !",color:"#1A6B3C",content:`<p>Tu es désormais un <strong>manager de football</strong> virtuel.</p>
    ...
    <p style="margin-top:12px;font-size:13px;color:#888">Tu peux revoir ce tutoriel depuis les paramètres à tout moment.</p>`}];function $r(e,t,i){let n=0;const r=document.createElement("div");r.id="tutorial-overlay",r.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.65);z-index:9900;display:flex;align-items:center;justify-content:center;padding:16px";const o=()=>{var d,u,x;const s=t[n],l=n===t.length-1,c=Math.round((n+1)/t.length*100);r.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
  ...
  </div>`;let l="buy";const c=()=>{var p,g,h,_,b,$,y;return{name:(((p=document.getElementById("flt-name"))==null?void 0:p.value)||"").toLowerCase().trim(),club:(((g=document.getElementById("flt-club"))==null?void 0:g.value)||"").toLowerCase().trim(),country:(((h=document.getElementById("flt-country"))==null?void 0:h.value)||"").toLowerCase().trim(),job:((_=document.getElementById("flt-job"))==null?void 0:_.value)||"",rarity:((b=document.getElementById("flt-rarity"))==null?void 0:b.value)||"",note1:parseInt(($=document.getElementById("flt-note1"))==null?void 0:$.value)||0,note2:parseInt((y=document.getElementById("flt-note2"))==null?void 0:y.value)||0}};function d(p){const g=c();return p.filter(h=>{var N,ne,ie;const _=(N=h.card)==null?void 0:N.player;if(!_)return!1;const b=`${_.firstname} ${_.surname_real}`.toLowerCase(),$=(((ne=_.clubs)==null?void 0:ne.encoded_name)||"").toLowerCase(),y=(_.country_code||"").toLowerCase(),j=((ie=h.card)==null?void 0:ie.evolution_bonus)||0,E=Ct(_,_.job,j),P=_.job2?Ct(_,_.job2,j):0;return!(g.name&&!b.includes(g.name)||g.club&&!$.includes(g.club)||g.country&&!y.includes(g.country)||g.job&&_.job!==g.job||g.rarity&&_.rarity!==g.rarity||g.note1&&E<g.note1||g.note2&&P<g.note2)})}function u(p){var P,N,ne,ie;const g=(P=p.card)==null?void 0:P.player;if(!g)return"";const h=((N=p.card)==null?void 0:N.evolution_bonus)||0,_=Ct(g,g.job,h),b=g.job2?Ct(g,g.job2,h):0,$=(i.profile.credits||0)>=p.price,y=g.country_code?`https://flagsapi.com/${g.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,j=ii[g.job]||"#bbb",E=g.job2?ii[g.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden">
      <!-- Drapeau -->
    ...
    </div>`}function x(p){var P,N,ne,ie;const g=(P=p.card)==null?void 0:P.player;if(!g)return"";const h=((N=p.card)==null?void 0:N.evolution_bonus)||0,_=Ct(g,g.job,h),b=g.job2?Ct(g,g.job2,h):0,$=p.status==="sold",y=g.country_code?`https://flagsapi.com/${g.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,j=ii[g.job]||"#bbb",E=g.job2?ii[g.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${$?"opacity:0.7":""}">
      ${y?`<img src="${y}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      ${(ne=g.clubs)!=null&&ne.logo_url?`<img src="${g.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${j};display:flex;align-items:center;justify-content:center">
    ...
    </div>`}function m(){const p=document.getElementById("mkt-content"),g=document.getElementById("mkt-filters");if(p){if(g.style.display=l==="buy"?"flex":"none",l==="buy"){const h=d(a);p.innerHTML=h.length?h.map(u).join(""):'<div style="text-align:center;color:#aaa;padding:40px">Aucune carte trouvée.</div>'}else{const h=s.filter(b=>b.status==="active").sort((b,$)=>new Date($.listed_at)-new Date(b.listed_at)),_=s.filter(b=>b.status==="sold").sort((b,$)=>new Date($.sold_at||$.listed_at)-new Date(b.sold_at||b.listed_at));p.innerHTML=(h.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:4px 0 6px;text-transform:uppercase;letter-spacing:1px">🟢 En vente (${h.length})</div>`+h.map(x).join(""):"")+(_.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:12px 0 6px;text-transform:uppercase;letter-spacing:1px">✅ Ventes réussies (${_.length})</div>`+_.map(x).join(""):"")+(!h.length&&!_.length?'<div style="text-align:center;color:#aaa;padding:40px">Aucune vente pour le moment.</div>':"")}p.querySelectorAll("[data-buy]").forEach(h=>h.addEventListener("click",()=>Tr(h.dataset.buy,a,e,t))),p.querySelectorAll("[data-cancel]").forEach(h=>h.addEventListener("click",()=>Ir(h.dataset.cancel,e,t)))}}e.querySelectorAll(".mkt-tab").forEach(p=>{p.addEventListener("click",()=>{l=p.dataset.tab,e.querySelectorAll(".mkt-tab").forEach(g=>{const h=g===p;g.style.background=h?"var(--green)":"#fff",g.style.color=h?"#fff":"var(--gray-600)",g.style.borderColor=h?"var(--green)":"var(--gray-200)"}),m()})});let f;["flt-name","flt-club","flt-country","flt-job","flt-rarity","flt-note1","flt-note2"].forEach(p=>{var g;(g=document.getElementById(p))==null||g.addEventListener("input",()=>{clearTimeout(f),f=setTimeout(m,250)})}),m()}async function Tr(e,t,i,n){const{state:r,toast:o,refreshProfile:a}=n,s=t.find(d=>d.id===e);if(!s)return;const l=s.price;if((r.profile.credits||0)<l){o("Crédits insuffisants","error");return}zr(s,async()=>{const{error:d}=await w.rpc("buy_market_card",{p_listing_id:e,p_buyer_id:r.profile.id});if(d){o("Erreur achat : "+d.message,"error");return}await a();const u=document.getElementById("nav-credits");u&&(u.textContent=`💰 ${(r.profile.credits||0).toLocaleString("fr")}`),o("✅ Carte achetée !","success"),await Fi(i,n)})}function zr(e,t){var a;const i=(a=e.card)==null?void 0:a.player,n=i?`${i.firstname} ${i.surname_real}`:"cette carte",r=document.createElement("div");r.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",r.innerHTML=`
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center">
    ...
    </div>`,document.body.appendChild(r);const o=s=>{r.remove(),s&&t()};r.querySelector("#buy-cancel").addEventListener("click",()=>o(!1)),r.querySelector("#buy-ok").addEventListener("click",()=>o(!0)),r.addEventListener("click",s=>{s.target===r&&o(!1)})}async function Ir(e,t,i){const{toast:n}=i,{data:r}=await w.from("market_listings").select("card_id").eq("id",e).single();if(await w.from("market_listings").update({status:"cancelled"}).eq("id",e),r!=null&&r.card_id){const{count:o}=await w.from("market_listings").select("id",{count:"exact",head:!0}).eq("card_id",r.card_id).eq("status","active");o||await w.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",r.card_id)}n("Annonce retirée","success"),Fi(t,i)}async function Sr(e,t){var d,u,x,m;const{state:i,navigate:n}=t,r=((u=(d=t==null?void 0:t.state)==null?void 0:d.params)==null?void 0:u.tab)||"global";e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const[{data:o},{data:a}]=await Promise.all([w.from("users").select("id,pseudo,club_name,trophies_top1,trophies_top2,trophies_top3,wins,level").order("trophies_top1",{ascending:!1}).limit(100),w.from("users").select("id,pseudo,club_name,mmr,rank_tier,ranked_wins,ranked_losses,ranked_draws,placement_matches").gte("placement_matches",1).order("mmr",{ascending:!1}).limit(100)]);let s=r;function l(){var p,g;const f=document.getElementById("rankings-list");if(f){if(s==="global"){const h=o||[];f.innerHTML=h.length>0?h.map((_,b)=>`
      ...
      `).join(""):'<div style="text-align:center;color:var(--gray-600);padding:40px">Aucun manager.</div>'}else{const h=a||[];f.innerHTML=h.length>0?h.map((_,b)=>{const $=Mi(_.mmr??1e3),y=(_.ranked_wins||0)+(_.ranked_losses||0)+(_.ranked_draws||0),j=y>0?Math.round((_.ranked_wins||0)/y*100):0,E=_.id===i.profile.id,P=(_.placement_matches||0)<10;return`
          <div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${E?"border:2px solid var(--yellow)":""}">
            ...
            <div style="text-align:right;flex-shrink:0">
              ...
              ${P?"":`<div style="font-size:10px;color:var(--gray-600)">${j}% WR</div>`}
            </div>
  ...
  `).join("");e.innerHTML=`
  <div style="min-height:100%;background:${_[f.id]};padding:16px;overflow-y:auto;display:flex;flex-direction:column;gap:16px">
  ...
  </div>`}Bn(Dn);const ve={user:null,profile:null,page:"home",params:{}};function Bt(e,t="info",i=3e3){const n=document.getElementById("toast");n&&(n.textContent=e,n.className=`show ${t}`,clearTimeout(n._t),n._t=setTimeout(()=>{n.className=""},i))}function jr(e,t,i=""){document.getElementById("modal-title").textContent=e,document.getElementById("modal-body").innerHTML=t,document.getElementById("modal-footer").innerHTML=i,document.getElementById("modal-overlay").classList.remove("hidden")}function zi(){document.getElementById("modal-overlay").classList.add("hidden")}async function Ot(){if(!ve.user)return;const{data:e}=await w.from("users").select("*").eq("id",ve.user.id).single();e&&(ve.profile=e)}function Ht(e,t={}){ve.page=e,ve.params=t,Mn()}async function Mn(){var n,r,o,a;const e=document.getElementById("page-content");if(!e)return;document.querySelectorAll(".bottom-nav a").forEach(s=>{s.classList.toggle("active",s.dataset.page===ve.page)});const t=document.getElementById("nav-credits");t&&ve.profile&&(t.textContent=`💰 ${(ve.profile.credits||0).toLocaleString("fr")}`);const i={state:ve,navigate:Ht,toast:Bt,openModal:jr,closeModal:zi,refreshProfile:Ot};switch(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽</div>',ve.page){case"home":await Ui(e,i);break;case"collection":await po(e,i);break;case"decks":await vi(e,i);break;case"boosters":await $o(e,i);break;case"ranked":await Mr(e,i);break;case"match":{const s=ve.params&&ve.params.matchMode||"vs_ai_easy";s==="random"?await Li(e,i,!1):s==="ranked"?await Li(e,i,!0):s==="friend"?await ar(e,i,(n=ve.params)==null?void 0:n.friendId,(r=ve.params)==null?void 0:r.friendName):s==="mini_league"||s==="mini-league"?await nn(e,i,(o=ve.params)==null?void 0:o.mlMatchId,(a=ve.params)==null?void 0:a.leagueId):await Po(e,i);break}case"market":await Lr(e,i);break;case"rankings":await Sr(e,i);break;case"matches":await Ar(e,i);break;case"friends":await Hn(e,i);break;case"mini-league":await lr(e,i);break;case"match-mini-league":{const s=ve.params||{};await nn(e,i,s.mlMatchId,s.leagueId);break}default:await Ui(e,i)}}function Cr(){var n;const e=document.getElementById("app"),t=ve.profile;if(!t)return;const i="/icons/";e.innerHTML=`
    <nav class="top-nav">
  ...
  </div>`,(n=e.querySelector("#pl-play-btn"))==null||n.addEventListener("click",t)}async function Dr(){document.documentElement.setAttribute("data-theme","light"),document.getElementById("modal-overlay").addEventListener("click",i=>{i.target===i.currentTarget&&zi()}),document.getElementById("modal-close").addEventListener("click",zi);const{data:{session:e}}=await w.auth.getSession();if(!e){on(),qr(document.getElementById("app"),{onPlay:()=>Ni(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:Bt})});return}ve.user=e.user,await Ot(),on();try{const{data:i}=await w.from("formation_links_overrides").select("formation, links"),n={};(i||[]).forEach(r=>{n[r.formation]=r.links}),qn(n)}catch(i){console.warn("Impossible de charger les overrides de formation:",i)}if(!ve.profile){Pn(document.getElementById("app"),{state:ve,navigate:async()=>{await Ot(),hi()},toast:Bt,refreshProfile:Ot});return}const t=Array.isArray(ve.profile.pending_boosters)?ve.profile.pending_boosters:[];if(!ve.profile.onboarding_done&&t.length>0){Ao(document.getElementById("app"),{state:ve,navigate:()=>hi(),toast:Bt,refreshProfile:Ot});return}hi(),setTimeout(()=>Er(ve.profile,Ht,Bt),800),w.auth.onAuthStateChange(async(i,n)=>{i==="SIGNED_OUT"&&(ve.user=null,ve.profile=null,document.getElementById("app").innerHTML="",Ni(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:Bt}))})}function Fr(){return Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight)}function ui(){const e=document.getElementById("app");e&&(e.style.height=Fr()+"px")}window.addEventListener("resize",ui);window.addEventListener("orientationchange",()=>setTimeout(ui,150));window.visualViewport&&window.visualViewport.addEventListener("resize",ui);function hi(){const e=()=>{var i;(i=ve.user)!=null&&i.id&&w.from("users").update({last_seen_at:new Date().toISOString()}).eq("id",ve.user.id).then(()=>{})};e(),window._presencePingInterval&&clearInterval(window._presencePingInterval),window._presencePingInterval=setInterval(e,6e4);const t=document.getElementById("app");t.style.display="flex",t.style.flexDirection="column",ui(),Cr(),Mn()}function on(){const e=document.getElementById("app-loader"),t=document.getElementById("app");t&&(t.style.display=""),e&&(e.classList.add("zoom-out"),setTimeout(()=>e.style.display="none",500))}function An(e){var n;const t=document.getElementById("app-loader");if(t&&(t.style.display="none"),document.getElementById("boot-error"))return;const i=document.createElement("div");i.id="boot-error",i.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:99999;gap:16px;color:#fff;padding:24px;text-align:center",i.innerHTML=`
