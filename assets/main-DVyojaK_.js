...
import{v as bo,s as v,r as qe,q as si,w as Gt,x as Ht,g as Ut,m as oi,n as Kt,o as Ze,_ as ln,y as dn,F as vt,k as zt,z as Bi,G as oo,I as Qe,t as zi,J as Hi,L as Jn,O as Ui,P as no,p as Di,Q as Fi,R as Xi,U as Ki,V as ni,W as _i,X as Ft,Y as cn,Z as ki,$ as lt,a0 as $i,a1 as jt,a2 as ut,a3 as Xn,a4 as pn,a5 as xo,a6 as Qn,a7 as un,a8 as Pt,T as Fo,a9 as Zn,aa as er}from"./match-shared-BPr03YEd.js";const tr="/";async function Po(t,{navigate:e,toast:i}){let o="login";const n=await bo("signup_access_code_required"),r=()=>{var d,l,s,p,c,y,x,m;const a=o==="login";if(t.innerHTML=`
    <div style="
    ...
    </style>`,(d=document.getElementById("tab-login-btn"))==null||d.addEventListener("click",()=>{o="login",r()}),(l=document.getElementById("tab-reg-btn"))==null||l.addEventListener("click",()=>{o="register",r()}),a){(s=document.getElementById("login-password"))==null||s.addEventListener("keydown",_=>{var $;_.key==="Enter"&&(($=document.getElementById("login-btn"))==null||$.click())});let f=null,h=!1;n&&((p=document.getElementById("login-access-code"))==null||p.addEventListener("input",_=>{const $=_.target.value,g=document.getElementById("login-access-code-status"),L=document.getElementById("login-btn");if(clearTimeout(f),h=!1,L&&(L.disabled=!0,L.style.opacity="0.45",L.style.cursor="not-allowed"),!$){g&&(g.textContent="");return}g&&(g.textContent="⏳ Vérification…",g.style.color="rgba(255,255,255,0.4)"),f=setTimeout(async()=>{const{data:u,error:j}=await v.rpc("check_signup_password",{input_password:$});if(j){g&&(g.textContent="Erreur de vérification.",g.style.color="#f87171");return}h=!!u,g&&(g.textContent=h?"✅ Code valide":"❌ Code incorrect",g.style.color=h?"#4ade80":"#f87171"),L&&(L.disabled=!h,L.style.opacity=h?"1":"0.45",L.style.cursor=h?"pointer":"not-allowed")},400)})),(c=document.getElementById("login-btn"))==null||c.addEventListener("click",async()=>{var T;const _=document.getElementById("login-email").value.trim(),$=document.getElementById("login-password").value,g=((T=document.getElementById("login-access-code"))==null?void 0:T.value)||"",L=document.getElementById("login-error");if(L.textContent="",!_||!$){L.textContent="Remplissez tous les champs.";return}if(n){const{data:F}=await v.rpc("check_signup_password",{input_password:g});if(!F){L.textContent="Code d'accès incorrect.";return}}const u=document.getElementById("login-btn");u.textContent="⏳ Connexion…",u.disabled=!0;const{error:j}=await v.auth.signInWithPassword({email:_,password:$});if(u.textContent="⚽ Se connecter",u.disabled=!1,j){L.textContent=j.message.includes("Invalid")?"Email ou mot de passe incorrect.":j.message;return}window.location.reload()})}else{(y=document.getElementById("reg-confirm"))==null||y.addEventListener("keydown",_=>{var $;_.key==="Enter"&&(($=document.getElementById("reg-btn"))==null||$.click())});let f=null,h=!1;n&&((x=document.getElementById("reg-access-code"))==null||x.addEventListener("input",_=>{const $=_.target.value,g=document.getElementById("access-code-status"),L=document.getElementById("reg-btn");if(clearTimeout(f),h=!1,L&&(L.disabled=!0,L.style.opacity="0.45",L.style.cursor="not-allowed"),!$){g&&(g.textContent="");return}g&&(g.textContent="⏳ Vérification…",g.style.color="rgba(255,255,255,0.4)"),f=setTimeout(async()=>{const{data:u,error:j}=await v.rpc("check_signup_password",{input_password:$});if(j){g&&(g.textContent="Erreur de vérification.",g.style.color="#f87171");return}h=!!u,g&&(g.textContent=h?"✅ Code valide":"❌ Code incorrect",g.style.color=h?"#4ade80":"#f87171"),L&&(L.disabled=!h,L.style.opacity=h?"1":"0.45",L.style.cursor=h?"pointer":"not-allowed")},400)})),(m=document.getElementById("reg-btn"))==null||m.addEventListener("click",async()=>{var Y;const _=document.getElementById("reg-email").value.trim(),$=document.getElementById("reg-password").value,g=document.getElementById("reg-confirm").value,L=((Y=document.getElementById("reg-access-code"))==null?void 0:Y.value)||"",u=document.getElementById("reg-error");if(u.textContent="",!_||!$||!g){u.textContent="Remplissez tous les champs.";return}if(!_.includes("@")||!_.includes(".")){u.textContent="Adresse email invalide.";return}if($.length<6){u.textContent="Mot de passe trop court (min. 6 caractères).";return}if($!==g){u.textContent="Les mots de passe ne correspondent pas.";return}if(n){const{data:X}=await v.rpc("check_signup_password",{input_password:L});if(!X){u.textContent="Code d'accès incorrect.";return}}const j=document.getElementById("reg-btn");j.textContent="⏳ Création…",j.disabled=!0;const{data:T,error:F}=await v.auth.signUp({email:_,password:$});if(j.textContent="🚀 Créer mon compte",j.disabled=!1,F){F.message.includes("already registered")||F.message.includes("already exists")||F.message.includes("User already")?u.textContent="Un compte existe déjà avec cette adresse email.":u.textContent=F.message;return}if(T!=null&&T.user&&Array.isArray(T.user.identities)&&T.user.identities.length===0){u.textContent="Un compte existe déjà avec cette adresse email.";return}i("Compte créé ! Connecte-toi pour commencer.","success",4e3),o="login",r(),setTimeout(()=>{const X=document.getElementById("login-email");X&&(X.value=_)},50)})}};r()}function ir(t,{state:e,navigate:i,toast:o,refreshProfile:n}){let r="#1A6B3C",a="#D4A017";t.innerHTML=`
  ...
  `;function d(){var m;const s=document.getElementById("logo-preview"),p=document.getElementById("logo-initials"),c=((m=document.getElementById("setup-club"))==null?void 0:m.value)||"MW",y=c.trim().split(" ").filter(Boolean),x=y.length>=2?(y[0][0]+y[1][0]).toUpperCase():c.slice(0,2).toUpperCase();s&&(s.style.background=a,s.style.borderColor=r),p&&(p.textContent=x,p.style.color=r)}document.getElementById("color1").addEventListener("input",s=>{r=s.target.value,document.getElementById("swatch1").style.background=r,d()}),document.getElementById("color2").addEventListener("input",s=>{a=s.target.value,document.getElementById("swatch2").style.background=a,d()});function l(s){document.querySelectorAll(".setup-step").forEach(p=>p.classList.remove("active")),document.getElementById(`step-${s}`).classList.add("active"),document.getElementById("step-num").textContent=s,document.getElementById("progress-fill").style.width=`${Math.round(s/3*100)}%`,s===3&&d()}document.getElementById("step1-next").addEventListener("click",async()=>{const s=document.getElementById("setup-pseudo").value.trim(),p=document.getElementById("step1-error");if(p.textContent="",s.length<3){p.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:c}=await v.from("users").select("id").eq("pseudo",s).maybeSingle();if(c){p.textContent="Ce pseudo est déjà pris.";return}l(2)}),document.getElementById("step2-back").addEventListener("click",()=>l(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const s=document.getElementById("setup-club").value.trim(),p=document.getElementById("step2-error");if(p.textContent="",s.length<2){p.textContent="Nom trop court (min. 2 caractères).";return}const{data:c}=await v.from("users").select("id").eq("club_name",s).maybeSingle();if(c){p.textContent="Ce nom de club est déjà pris.";return}l(3)}),document.getElementById("step3-back").addEventListener("click",()=>l(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const s=document.getElementById("setup-pseudo").value.trim(),p=document.getElementById("setup-club").value.trim(),c=document.getElementById("step3-error"),y=document.getElementById("step3-finish");c.textContent="",y.disabled=!0,y.textContent="Création en cours…";try{const{error:x}=await v.from("users").insert({id:e.user.id,pseudo:s,club_name:p,club_color1:r,club_color2:a,credits:15e3});if(x)throw x;await or(e.user.id),await n(),o(`Bienvenue ${s} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(x){c.textContent=x.message,y.disabled=!1,y.textContent="🚀 Créer mon profil !"}})}async function or(t){const e=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1},{type:"stadium",count:1}];try{await v.from("users").update({pending_boosters:e,onboarding_done:!1,first_booster_opened:!1}).eq("id",t)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const fn="mw_sound_volume";function yo(){const t=localStorage.getItem(fn);if(t===null)return 100;const e=parseInt(t,10);return Number.isFinite(e)?Math.max(0,Math.min(100,e)):100}function nr(t){localStorage.setItem(fn,String(Math.max(0,Math.min(100,Math.round(t)))))}function vo(){return yo()===0}function Pi(t){return Math.max(0,Math.min(1,t*(yo()/100)))}function ho(t,e=1){if(vo())return null;try{const i=new Audio(t);return i.volume=Pi(e),i.play().catch(()=>{}),i}catch{return null}}let St=null,ro=null,Ro=.3;function mn(t,e=.3){if(Ro=e,St&&ro===t&&!St.paused){St.volume=Pi(Ro);return}if(ti(),!vo())try{const i=new Audio(t);i.loop=!0,i.volume=Pi(e),i.play().catch(()=>{}),St=i,ro=t}catch{}}function ti(){if(St)try{St.pause(),St.currentTime=0}catch{}St=null,ro=null}let bi=null;function gn(t,e=.6){if(it(),!vo())try{const i=new Audio(t);i.volume=Pi(e),i.play().catch(()=>{}),bi=i}catch{}}function it(){if(bi)try{bi.pause(),bi.currentTime=0}catch{}bi=null}async function bn(t=null){const e=new Date().toISOString().slice(0,10),{data:i}=await v.from("booster_configs").select("*").eq("is_active",!0).order("sort_order");if(!(i!=null&&i.length))return[];const o=i.filter(a=>!(a.available_from&&e<a.available_from||a.available_until&&e>a.available_until));if(!o.length)return[];let n=o;if(t){const a=o.filter(d=>d.max_per_user!=null);if(a.length){const{data:d}=await v.from("booster_claims").select("booster_id").eq("user_id",t).in("booster_id",a.map(s=>s.id)),l={};(d||[]).forEach(s=>{l[s.booster_id]=(l[s.booster_id]||0)+1}),n=o.filter(s=>s.max_per_user==null?!0:(l[s.id]||0)<s.max_per_user)}}if(!n.length)return[];const{data:r}=await v.from("booster_drop_rates").select("*").in("booster_id",n.map(a=>a.id)).order("sort_order");return n.map(a=>({...a,rates:(r||[]).filter(d=>d.booster_id===a.id)}))}async function rr(t,e){const{data:i}=await v.from("booster_configs").select("max_per_user").eq("id",e).single();i!=null&&i.max_per_user&&await v.from("booster_claims").insert({user_id:t,booster_id:e})}function ar(t){if(!(t!=null&&t.length))return null;const e=t.reduce((o,n)=>o+Number(n.percentage),0);let i=Math.random()*e;for(const o of t)if(i-=Number(o.percentage),i<=0)return o;return t[t.length-1]}const sr="/",xn=[{emoji:"⚽",title:"Bienvenue dans Manager Wars !",color:"#1A6B3C",content:`<p>Tu es désormais un <strong>manager de football</strong> virtuel.</p>
    ...
    </div>`}const cr=typeof import.meta<"u"&&"/"||"/",Oo=cr+"icons/";function Ke(t,e,i,o,n,r,a,d,l,s,p,c){return{id:t,firstname:e,surname_real:i,job:o,job2:n,note_g:r,note_d:a,note_m:d,note_a:l,rarity:s,country_code:p,club_id:c,clubs:{encoded_name:c,logo_url:null},evolution_bonus:0,_evolution_bonus:0,face:null}}const ge={barthez:Ke("10dfdba1-5793-47b5-99f7-f72c1bbc60df","Fabien","Barthez","GK",null,17,0,0,0,"normal","FR","MONACO"),djetou:Ke("470568c5-a79f-44f9-bd96-27fe73415e20","Martin","Djetou","DEF",null,0,15,0,0,"normal","FR","MONACO"),sagnol:Ke("b43818be-80c6-468b-b81d-d1b02f9dc11b","Willy","Sagnol","DEF",null,0,13,0,0,"normal","FR","MONACO"),girard:Ke("ba5aa9cf-73b2-4846-8c09-008ee6ef4db9","Ming","Girard","DEF",null,0,20,0,0,"papyte","FR","LILLE"),bruno:Ke("8696cf6c-3162-4e2e-b66e-36331dae148e","James","Bruno","DEF",null,0,15,0,0,"papyte","FR","LILLE"),lamouchi:Ke("e9481065-766c-4e65-aeaf-3c23215babe0","Sabri","Lamouchi","MIL",null,0,0,15,0,"normal","FR","MONACO"),legwinski:Ke("0fe9f0be-3215-4634-9df2-ecb39cb23e2f","Sylvain","Legwinski","MIL",null,0,0,14,0,"normal","FR","MONACO"),bianchi:Ke("7cbd6088-3b6e-40ac-9f79-23be6a4b288a","Noah","Bianchi","MIL","DEF",0,13,15,0,"normal","FR","LILLE"),trezeguet:Ke("17ebe089-7d5a-44fe-bc51-5a51dc0a22bc","David","Trezeguet","ATT",null,0,0,0,16,"normal","FR","MONACO"),giuly:Ke("78181125-55e0-4241-b929-419b5c7feb56","Ludovic","Giuly","ATT",null,0,0,0,16,"normal","FR","MONACO"),boukhris:Ke("fd6bb2ab-ee46-4c28-80e9-d359622c71b2","Rayan","Boukhris","ATT",null,0,0,0,20,"normal","FR","CAPITALE SG"),pires:Ke("f675e51a-2cdc-4b44-bbc1-7c76cddfa519","Robert","Pires","MIL",null,0,0,17,0,"normal","FR","OLYMPIC PHOCEENS"),ai1:Ke("ai1","Marco","ROSSI","MIL",null,0,0,16,0,"normal","IT","GEN"),ai2:Ke("ai2","Luis","GOMEZ","MIL",null,0,0,14,0,"normal","ES","GEN"),ai3:Ke("ai3","Hans","MULLER","MIL",null,0,0,15,0,"normal","DE","GEN")},pr={GK:[ge.barthez],DEF:[ge.djetou,ge.sagnol,ge.girard,ge.bruno],MIL:[ge.lamouchi,null,ge.legwinski],ATT:[ge.trezeguet,ge.giuly,ge.boukhris]},kn={GK:[ge.barthez],DEF:[ge.djetou,ge.sagnol,ge.girard,ge.bruno],MIL:[ge.lamouchi,ge.bianchi,ge.legwinski],ATT:[ge.trezeguet,ge.giuly,ge.boukhris]},ur=[{id:"bc1",card_type:"player",evolution_bonus:0,player:ge.girard},{id:"bc2",card_type:"player",evolution_bonus:0,player:ge.trezeguet},{id:"bc3",card_type:"player",evolution_bonus:0,player:ge.bianchi},{id:"bc4",card_type:"player",evolution_bonus:0,player:ge.djetou},{id:"bc5",card_type:"player",evolution_bonus:0,player:ge.pires}],ne=()=>window.innerWidth<900,fr=()=>ne()?100:150;let Te=null,Ci=null;function $n(t){Te||(Ci=t,ao=0,mr(),xr(0))}function mr(){Te&&Te.remove(),Te=document.createElement("div"),Te.id="tv2",Te.style.cssText="position:fixed;inset:0;z-index:9700;background:#0d1f14;overflow:hidden;display:flex;flex-direction:column",Te.innerHTML=`
    ...
    </div>`,document.body.appendChild(Te),Te.querySelector("#tv2-skip").addEventListener("click",Ln)}function gr(t){Te.querySelectorAll(".tv2-tab").forEach(e=>{const i=e.dataset.key===t;e.style.color=i?"#D4A017":"rgba(255,255,255,0.4)";const o=e.querySelector("img");o&&(o.style.opacity=i?"1":"0.5")})}function He(t,e){Te.querySelector("#tv2-screen").innerHTML=`<div style="padding:${ne()?"16px 16px 8px":"24px"};color:#fff">${t}</div>`,e&&gr(e),Te.querySelector("#tv2-bub").style.display="none",Te.querySelector("#tv2-dim").style.display="none",Te.querySelector("#tv2-dim").innerHTML="",Te.querySelector("#tv2-dim").style.pointerEvents="none"}function En(t,e){Te.querySelector("#tv2-progbar").style.width=Math.round(t/e*100)+"%"}function Ge({title:t,text:e,targetSel:i,side:o="bottom",btnLabel:n="Suivant →",onNext:r,isAction:a=!1}){const d=Te.querySelector("#tv2-bub"),l=Te.querySelector("#tv2-dim");d.innerHTML=`
    ...
    ${a?`<div style="margin-top:12px;font-size:11px;color:#1A6B3C;font-weight:800">👆 ${n}</div>`:`<button id="tv2-next" style="margin-top:14px;float:right;background:#1A6B3C;color:#fff;border:none;
         ...
         <div style="clear:both"></div>`}`;const s=ne()?Math.round(window.innerWidth*.88):340;d.style.width=s+"px";const p=window.innerWidth,c=window.innerHeight;if(i){const y=Te.querySelector(i);if(y){const x=y.getBoundingClientRect();l.style.display="block",l.style.pointerEvents="none",l.style.background="rgba(0,0,0,0)",l.innerHTML=`<div style="position:absolute;left:${x.left-5}px;top:${x.top-5}px;
        width:${x.width+10}px;height:${x.height+10}px;border-radius:12px;
        box-shadow:0 0 0 4000px rgba(0,0,0,0.65);z-index:9721;pointer-events:none"></div>`;const m=180;let f,h;o==="bottom"?(h=x.bottom+10,f=x.left+x.width/2-s/2):o==="top"?(h=x.top-m-10,f=x.left+x.width/2-s/2):o==="right"?(h=x.top+x.height/2-m/2,f=x.right+10):(h=x.top+x.height/2-m/2,f=x.left-s-10),f=Math.max(8,Math.min(f,p-s-8)),h=Math.max(8,Math.min(h,c-200)),d.style.left=f+"px",d.style.top=h+"px",d.style.removeProperty("transform")}}else l.style.display="block",l.style.pointerEvents="none",l.style.background="rgba(0,0,0,0)",l.innerHTML="",d.style.left="50%",d.style.top="50%",d.style.transform="translate(-50%,-50%)";if(d.style.display="block",a){if(i){const y=Te.querySelector(i);if(y){const x=m=>{const f=y.getBoundingClientRect();m.clientX>=f.left&&m.clientX<=f.right&&m.clientY>=f.top&&m.clientY<=f.bottom&&(d.style.display="none",l.style.display="none",l.innerHTML="",r==null||r())};l.style.pointerEvents="auto",l.addEventListener("click",x,{once:!0}),y.style.cursor="pointer",y.addEventListener("click",()=>{d.style.display="none",l.style.display="none",l.innerHTML="",r==null||r()},{once:!0})}}}else{const y=d.querySelector("#tv2-next");y==null||y.addEventListener("click",()=>{d.style.display="none",l.style.display="none",l.innerHTML="",r==null||r()},{once:!0})}}function Oe(t){return new Promise(e=>setTimeout(e,t))}let ao=0;const br=20;function Ve(){var t;En(++ao,br),(t=It[ao])==null||t.call(It)}const It=[];function Ue(t){It.push(t)}Ue(()=>{He(`
...
🎁 Les boosters
🏪 Le Mercato
⚔️ Comment gagner un match !`,btnLabel:"C'est parti !",onNext:Ve})});Ue(()=>{const t=[ge.barthez,ge.djetou,ge.girard,ge.bianchi,ge.trezeguet,ge.giuly,ge.boukhris],e=ge.girard;He(`
...
La carte GIRARD est entourée en argent : c'est une carte Papyte, une rareté puissante avec des joueurs d'exception !`,targetSel:"#tv2-main-card",side:"bottom",onNext:Ve}))});Ue(()=>{const t=ge.bianchi,e=ne()?140:200;He(`
    ...
    <div style="display:flex;${ne()?"flex-direction:column;align-items:center":"gap:28px;align-items:flex-start"}">
      ...
      <div style="flex:1;display:flex;flex-direction:column;gap:8px;${ne()?"width:100%;margin-top:12px":""}">
        ...
        <div id="tv2-d-note2" style="padding:10px 14px;border-radius:10px;background:rgba(212,160,23,0.1);border:1px solid rgba(212,160,23,0.25)"><b>🔸 Note secondaire :</b> ${t.note_d} (DEF) — peut jouer 2 postes !</div>
...
Les cartes Papyte (argent) et Légende (violet) ont des joueurs d'exception avec de meilleures notes.`},{sel:"#tv2-d-note",t:"🔢 La note principale",tx:`La force du joueur à son poste naturel. C'est cette note qui est utilisée lors des duels de match.

Bianchi a 15 en MIL — solide !`},{sel:"#tv2-d-note2",t:"🔸 Note secondaire",tx:`Certains joueurs polyvalents peuvent jouer à deux postes !

Bianchi peut jouer MIL (15) ou DEF (13). Cette flexibilité est un gros avantage tactique.`},{sel:"#tv2-d-flag",t:"🇫🇷 Le pays",tx:`Le pays crée des liens avec les compatriotes dans ta formation.
...
Girard + Bruno + Bianchi jouent tous à Lille → +5 pts entre voisins !`}];let o=0;const n=()=>{if(o>=i.length){Ve();return}const r=i[o++];Ge({title:r.t,text:r.tx,targetSel:r.sel,side:"right",onNext:n})};Oe(150).then(n)});Ue(()=>{const t=ge.giuly;He(`
...
Chaque doublon fusionné augmente définitivement la note du joueur.`}];let o=0;const n=()=>{if(o>=i.length){Ve();return}const r=i[o++];Ge({title:r.t,text:r.tx,targetSel:r.sel,side:"right",onNext:n})};Oe(300).then(n)}})})});Ue(()=>{const t=oi({formation:"4-3-3",rarity:"normal"},ne()?120:150),e=Kt({id:"s1",name:"Stade de France",country_code:"FR"},ne()?120:150),i=Ze({gc_type:"red_card",rarity:"pepite"},ne()?120:150);He(`
...
Le Carton Rouge expulse un milieu adverse et réduit son score. Utilisable une seule fois !`}];let n=0;const r=()=>{if(n>=o.length){Ve();return}const a=o[n++];Ge({title:a.t,text:a.tx,targetSel:a.sel,side:"bottom",onNext:r})};Oe(150).then(r)});Ue(()=>{const t=ne()?Math.min(window.innerWidth-32,340):380,e=Math.round(t*.92);He(`
    ...
    </div>`,"decks");const i=[{sel:"#tv2-terrain",side:"right",t:"⚽ Le terrain",tx:`Voici ta formation. Chaque cercle = un joueur, le cercle grisé = slot vide (Bianchi n'est pas encore placé).

Les lignes entre joueurs représentent les liens — plus de liens = plus de bonus !`},{sel:"#tv2-l-pays",side:"left",t:"🇫🇷 Lien Pays +5 pts",tx:`Toute cette équipe est française ! Deux joueurs français côte à côte = +5 pts par lien lors des duels.

Mets des joueurs de même nationalité ensemble pour maximiser tes bonus.`},{sel:"#tv2-l-club",side:"left",t:"🛡️ Lien Club +5 pts",tx:`Girard et Bruno jouent tous les deux à Lille. Placés côte à côte en défense = +5 pts chacun !
...
C'est le bonus le plus puissant du jeu !`}];let o=0;const n=()=>{if(o>=i.length){Ve();return}const r=i[o++];Ge({title:r.t,text:r.tx,targetSel:r.sel,side:r.side,onNext:n})};Oe(150).then(n)});Ue(()=>{He(`
...
Tu peux rejouer ce tutoriel depuis les Réglages à tout moment.`}));let i=0;const o=()=>{if(i>=e.length){Ve();return}const n=e[i++];Ge({title:n.t,text:n.tx,targetSel:n.sel,side:"right",btnLabel:i>=e.length?"Jouer ! →":"Suivant →",onNext:o})};Oe(150).then(o)});Ue(()=>{He(`
...
Sélectionne le deck France 98 !`,targetSel:"#tv2-deckrow",isAction:!0,btnLabel:"Sélectionne le deck !",onNext:Ve}))});Ue(()=>{const t=Ze({gc_type:"red_card",rarity:"pepite"},ne()?100:130);He(`
...
Choisis le Carton Rouge — il expulse un milieu adverse.`,targetSel:"#tv2-gcchoice",isAction:!0,btnLabel:"Sélectionner !",onNext:Ve}))});Ue(()=>{const t=[ge.lamouchi,ge.bianchi,ge.legwinski],e=[ge.ai1,ge.ai2,ge.ai3];He(`
    ...
    </div>`,"home"),Oe(400).then(()=>{Te.querySelectorAll(".duel-card").forEach((i,o)=>setTimeout(()=>{i.style.opacity="1",i.style.transform="translateY(0) scale(1)"},100+o*100)),Te.querySelectorAll(".duel-link").forEach((i,o)=>setTimeout(()=>i.style.opacity="1",600+o*60)),Te.querySelectorAll(".duel-score-line").forEach(i=>setTimeout(()=>i.style.opacity="1",1100)),Oe(800).then(()=>{const i=ge.lamouchi.note_m+ge.bianchi.note_m+ge.legwinski.note_m,o=ge.ai1.note_m+ge.ai2.note_m+ge.ai3.note_m,n=()=>{Te.querySelector("#tv2-gc-banner").style.display="block";const r=o-ge.ai2.note_m;Te.querySelector("#tv2-scores").innerHTML=`
...
Tu prends l'avantage grâce au Game Changer ! Maintenant tu peux attaquer.`,onNext:Ve}))};Ge({title:"⚔️ Le duel du milieu",text:`Les scores actuels :
Toi : ${i} pts | Adversaire : ${o} pts

Tu es légèrement derrière... C'est le moment d'utiliser ton Carton Rouge sur Luis Gomez !`,targetSel:"#tv2-ai-row",side:"bottom",btnLabel:"Jouer le Carton Rouge !",onNext:n})})})});Ue(()=>{const t=ne()?Math.min(window.innerWidth-32,320):340,e=Math.round(t*.92);He(`
...
Sur le terrain, les joueurs en surbrillance sont ceux que tu peux sélectionner. Choisis 3 attaquants ou milieux offensifs.`,targetSel:"#tv2-atk",isAction:!0,btnLabel:"Lancer l'attaque !",onNext:Ve}))});Ue(()=>{const t=ne()?Math.min(window.innerWidth-32,320):340,e=Math.round(t*.92);He(`
...
Choisis 3 défenseurs (DEF) ou milieux défensifs. Plus leurs notes sont élevées, plus tu as de chances de stopper l'attaque.`,targetSel:"#tv2-def",isAction:!0,btnLabel:"Défendre !",onNext:Ve}))});Ue(()=>{He(`
...
Le joueur avec le plus de buts gagne et monte dans le classement !`,targetSel:"#tv2-winbox",side:"top",btnLabel:"Terminer ! 🚀",onNext:Ve}))});Ue(()=>{He(`
  ...
  `,document.body.appendChild(r);let a=!1;const d=document.getElementById("pack-cut-zone"),l=document.getElementById("pack-blade");let s=!1;const p=z=>z.touches&&z.touches[0]?z.touches[0].clientX:z.clientX;function c(z){a||(s=!0,l.style.opacity="1",y(z))}function y(z){if(!s||a)return;const Z=d.getBoundingClientRect(),U=p(z)-Z.left,te=Math.max(0,Math.min(1,U/Z.width));l.style.width=te*Z.width+"px",te>=.82&&m()}function x(){a||(s=!1,l.style.transition="width .2s ease, opacity .2s ease",l.style.width="0",l.style.opacity="0",setTimeout(()=>{a||(l.style.transition="")},220))}function m(){var Z;if(a)return;a=!0,s=!1,l.style.width="100%",l.style.opacity="1",(Z=document.getElementById("cut-flash"))==null||Z.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const z=document.getElementById("cut-hint");z&&(z.style.opacity="0"),d.classList.add("pack-cut"),setTimeout(()=>{l.style.opacity="0",document.getElementById("pack-phase").style.display="none",$(0)},620)}d.addEventListener("pointerdown",c),window.addEventListener("pointermove",y),window.addEventListener("pointerup",x),d.addEventListener("touchstart",c,{passive:!0}),window.addEventListener("touchmove",y,{passive:!0}),window.addEventListener("touchend",x);let f=0,h=!1;const _=new Set;function $(z){f=z,document.getElementById("reveal-phase").style.display="flex",g(),L(z,0),Y()}function g(){const z=document.getElementById("card-dots");z&&(z.innerHTML=t.map((Z,U)=>`<div class="card-dot" data-i="${U}" style="width:8px;height:8px;border-radius:50%;background:${U===f?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),z.querySelectorAll(".card-dot").forEach(Z=>Z.addEventListener("click",()=>j(parseInt(Z.dataset.i)))))}function L(z,Z){var _e,ye;const U=t[z],te=document.getElementById("card-counter"),pe=document.getElementById("card-track");te&&(te.textContent=`Carte ${z+1} / ${t.length}`);const he=document.getElementById("reveal-btns");he&&(he.style.display=z===t.length-1?"flex":"none");const fe=U.card_type==="player"&&((_e=U.player)==null?void 0:_e.rarity)==="legende",Be=!_.has(z);if(_.add(z),U.card_type==="player"&&U.player){const Ee=U.player,D=Ee.job||"ATT";(Number(D==="GK"?Ee.note_g:D==="DEF"?Ee.note_d:D==="MIL"?Ee.note_m:Ee.note_a)||0)+(U.evolution_bonus||0)}const Pe=Ee=>{pe.innerHTML=`
        ...
        </div>`;const D=document.getElementById("current-card-wrap");Z!==0?(D.style.transition="none",D.style.transform=`translateX(${Z>0?100:-100}%)`,requestAnimationFrame(()=>{D.style.transition="transform .28s cubic-bezier(.25,1,.5,1)",D.style.transform="translateX(0)"})):D.animate([{opacity:0,transform:"scale(.7)"},{opacity:1,transform:"scale(1)"}],{duration:300,easing:"cubic-bezier(.34,1.56,.64,1)"}),Ee||fe?M():w(),g()};Be&&((ye=U.player)==null?void 0:ye.rarity)==="legende"&&U.card_type==="player"&&U.player?u(U,()=>Pe(!0)):Pe(!1)}function u(z,Z){var xe;h=!0;const U=z.player,te=`https://flagsapi.com/${U.country_code}/flat/64.png`,pe=(xe=U.clubs)==null?void 0:xe.logo_url,he=Ut(U),fe=U.job||"ATT",Be=Number(fe==="GK"?U.note_g:fe==="DEF"?U.note_d:fe==="MIL"?U.note_m:U.note_a)||0,Pe=z.evolution_bonus||0,_e=Be+Pe,ye=U.rarity==="legende",Ee=_e>=18,D=document.getElementById("walkout-overlay"),C=document.getElementById("walkout-stage");if(!D||!C){h=!1,Z();return}let K=null;ye&&(K=ho("/sounds/Legendary.mp3",.8)),D.style.display="flex";const W=()=>{const ue=C.firstElementChild;ue&&(ue.classList.remove("wo-in"),ue.classList.add("wo-out"))},J=1800,P=400;C.innerHTML=`<img class="wo-in" src="${te}" style="height:130px;border-radius:10px;box-shadow:0 10px 36px rgba(0,0,0,.6)" onerror="this.style.display='none'">`,navigator.vibrate&&navigator.vibrate(30),setTimeout(W,J),setTimeout(()=>{var ue;C.innerHTML=pe?`<img class="wo-in" src="${pe}" style="max-height:160px;max-width:210px;object-fit:contain">`:`<div class="wo-in" style="font-size:34px;font-weight:900;color:#fff;text-align:center">${((ue=U.clubs)==null?void 0:ue.encoded_name)||"CLUB"}</div>`,navigator.vibrate&&navigator.vibrate(30)},J+P),setTimeout(W,J*2+P),setTimeout(()=>{C.innerHTML=he?`<img class="wo-in" src="${he}" style="height:200px;border-radius:50%;box-shadow:0 0 40px rgba(255,255,255,0.3);object-fit:cover;object-position:top">`:'<div class="wo-in" style="font-size:80px">👤</div>',navigator.vibrate&&navigator.vibrate(30)},(J+P)*2),setTimeout(W,(J+P)*2+J);const me=Ee?"#FFD700":{GK:"#c0c0c0",DEF:"#e03030",MIL:"#D4A017",ATT:"#3fbf5f"}[fe]||"#fff";setTimeout(()=>{C.innerHTML=`<div class="wo-in" style="
        ...
        ${_e}
      </div>`,Ee&&navigator.vibrate&&navigator.vibrate([50,30,100,30,200])},(J+P)*3),setTimeout(W,(J+P)*3+J),setTimeout(()=>{D.style.display="none",C.innerHTML="",h=!1,K&&!ye&&K.pause(),navigator.vibrate&&navigator.vibrate([40,30,80]),Z()},(J+P)*4)}function j(z){if(h||z<0||z>=t.length||z===f)return;const Z=z>f?1:-1;f=z,L(z,Z)}function T(){j(f+1)}function F(){j(f-1)}function Y(){const z=document.getElementById("card-viewport");if(!z||z._swipeBound)return;z._swipeBound=!0;let Z=0,U=0,te=0,pe=!1;const he=ye=>ye.touches?ye.touches[0].clientX:ye.clientX,fe=ye=>ye.touches?ye.touches[0].clientY:ye.clientY,Be=ye=>{pe=!0,Z=he(ye),U=fe(ye),te=0},Pe=ye=>{if(!pe)return;te=he(ye)-Z;const Ee=fe(ye)-U;if(Math.abs(te)<Math.abs(Ee))return;const D=document.getElementById("current-card-wrap");D&&(D.style.transition="none",D.style.transform=`translateX(${te*.6}px) rotate(${te*.02}deg)`)},_e=()=>{if(!pe)return;pe=!1;const ye=document.getElementById("current-card-wrap"),Ee=55;te<=-Ee&&f<t.length-1?T():te>=Ee&&f>0?F():ye&&(ye.style.transition="transform .2s ease",ye.style.transform="translateX(0)")};z.addEventListener("pointerdown",Be),z.addEventListener("pointermove",Pe),z.addEventListener("pointerup",_e),z.addEventListener("pointercancel",_e),z.addEventListener("touchstart",Be,{passive:!0}),z.addEventListener("touchmove",Pe,{passive:!0}),z.addEventListener("touchend",_e),z.addEventListener("click",ye=>{if(Math.abs(te)>8)return;const Ee=z.getBoundingClientRect();ye.clientX-Ee.left>Ee.width/2?T():F()})}let X=null;function M(){const z=document.getElementById("fireworks-canvas");if(!z)return;z.width=window.innerWidth,z.height=window.innerHeight;const Z=z.getContext("2d"),U=[];function te(){const he=Math.random()*z.width,fe=Math.random()*z.height*.6,Be=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],Pe=Be[Math.floor(Math.random()*Be.length)];for(let _e=0;_e<60;_e++){const ye=Math.PI*2/60*_e,Ee=2+Math.random()*5;U.push({x:he,y:fe,vx:Math.cos(ye)*Ee,vy:Math.sin(ye)*Ee,alpha:1,color:Pe,size:2+Math.random()*3})}}te(),X=setInterval(te,600);function pe(){if(document.getElementById("fireworks-canvas")){Z.clearRect(0,0,z.width,z.height);for(let he=U.length-1;he>=0;he--){const fe=U[he];if(fe.x+=fe.vx,fe.y+=fe.vy+.08,fe.vy*=.98,fe.alpha-=.018,fe.alpha<=0){U.splice(he,1);continue}Z.globalAlpha=fe.alpha,Z.fillStyle=fe.color,Z.beginPath(),Z.arc(fe.x,fe.y,fe.size,0,Math.PI*2),Z.fill()}Z.globalAlpha=1,(X!==null||U.length>0)&&requestAnimationFrame(pe)}}pe()}function w(){X!==null&&(clearInterval(X),X=null);const z=document.getElementById("fireworks-canvas");z&&z.getContext("2d").clearRect(0,0,z.width,z.height)}if(o){const z=document.getElementById("reveal-btns");z&&(z.style.flexDirection="row",z.innerHTML='<button class="btn btn-primary" id="reveal-next" style="flex:1">Continuer →</button>'),(E=document.getElementById("reveal-next"))==null||E.addEventListener("click",()=>{w(),r.remove(),o()})}else{const z=document.getElementById("reveal-reopen"),Z=e.cost||0,U=Number((S=(H=n==null?void 0:n.state)==null?void 0:H.profile)==null?void 0:S.credits)||0,te=!Z||U>=Z;if(z)if(n){z.textContent=`🎁 Ouvrir un autre booster${Z?` (${Z.toLocaleString("fr")} cr.)`:""}`;const pe=fe=>{z.disabled=!0,z.style.opacity="0.45",z.style.cursor="not-allowed",fe&&(z.title=fe)};te||pe("Crédits insuffisants");const he=((G=e._raw)==null?void 0:G.max_per_user)??null;te&&he!=null&&((I=(de=n==null?void 0:n.state)==null?void 0:de.user)!=null&&I.id)&&v.from("booster_claims").select("id",{count:"exact",head:!0}).eq("user_id",n.state.user.id).eq("booster_id",e._boosterId).then(({count:fe})=>{(fe||0)>=he&&pe(`Quota atteint (${he} max.)`)}),z.addEventListener("click",()=>{if(z.disabled)return;w(),r.remove();const fe=Sn();An(e,{state:n.state,toast:n.toast,navigate:i,container:n.container}).finally(fe)})}else z.textContent="🎁 Ouvrir un autre booster",z.addEventListener("click",()=>{w(),r.remove(),i("boosters")});document.getElementById("reveal-collection").addEventListener("click",()=>{w(),r.remove(),i("collection")}),document.getElementById("reveal-shop").addEventListener("click",()=>{w(),r.remove(),i("boosters")})}}function Er(t){var e,i,o,n;if(t.card_type==="player"&&t.player)return vr(t);if(t.card_type==="game_changer"){const r=t._gcDef,a=(r==null?void 0:r.name)||t.gc_type||"Game Changer",d=(r==null?void 0:r.effect)||((e=so[t.gc_type])==null?void 0:e.desc)||"",l=r!=null&&r.image_url?`/icons/${r.image_url}`:null,s=((i=so[t.gc_type])==null?void 0:i.icon)||"⚡";return Ze(a,l,s,d,{width:170})}if(t.card_type==="formation")return oi(t.formation,vt[t.formation],{width:160});if(t.card_type==="stadium"){const r=t._stadiumDef,a=(r==null?void 0:r.name)||"Stade",d=((o=r==null?void 0:r.club)==null?void 0:o.encoded_name)||(r==null?void 0:r.country_code)||"—",l=r!=null&&r.image_url?`/icons/${r.image_url}`:((n=r==null?void 0:r.club)==null?void 0:n.logo_url)||(r!=null&&r.country_code?`https://flagsapi.com/${r.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),s=`${d}<br>+10 ⭐ joueurs alliés`;return Kt(a,l,s,{width:170})}return'<div style="width:140px;height:200px;background:#333;border-radius:12px"></div>'}function Lr(t){var e,i;if((e=t==null?void 0:t.rates)!=null&&e.length){const o=document.createElement("div");o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.6);display:flex;align-items:center;justify-content:center;z-index:4000;padding:16px";const n={normal:"#ccc",pepite:"#D4A017",pépite:"#D4A017",papyte:"#909090",legende:"#7a28b8",légende:"#7a28b8"},r={player:"Joueur",formation:"Formation",game_changer:"Game Changer",game_helper:"Game Helper"};o.innerHTML=`
      ...
      </div>`,document.body.appendChild(o),o.addEventListener("click",a=>{a.target===o&&o.remove()}),(i=document.getElementById("odds-close"))==null||i.addEventListener("click",()=>o.remove());return}zr()}function zr(){const t=document.createElement("div");t.style.cssText=`position:fixed;inset:0;background:rgba(0,0,0,0.6);display:flex;
  ...
  `,document.body.appendChild(t),t.addEventListener("click",e=>{e.target===t&&t.remove()}),document.getElementById("odds-close").addEventListener("click",()=>t.remove())}function Sr(){return new Promise(t=>{const e=document.createElement("div");e.style.cssText=`
    ...
    `,document.body.appendChild(e);try{(window.adsbygoogle=window.adsbygoogle||[]).push({})}catch(n){console.warn("[AdSense]",n)}let i=5;const o=setInterval(()=>{i--;const n=document.getElementById("mw-ad-cd");n&&(n.textContent=i),i<=0&&(clearInterval(o),e.remove(),t(!0))},1e3)})}async function Ar(t,{state:e,navigate:i,toast:o,refreshProfile:n}){var $;const{data:r}=await v.from("users").select("*").eq("id",e.user.id).single();r&&(e.profile=r);let a=Array.isArray(($=e.profile)==null?void 0:$.pending_boosters)?[...e.profile.pending_boosters]:[];{let g=0;a.forEach(L=>{L.type==="player"&&(L._ordinal=++g)})}if(!a.length){await v.from("users").update({onboarding_done:!0}).eq("id",e.user.id),i("home");return}let d=null;try{const L=(await bn()).find(u=>(u.name||"").toLowerCase().includes("new player"));L&&(d=wo(L))}catch(g){console.warn('[Onboarding] Config "Booster (new player)" introuvable, fallback taux par défaut',g)}const l=a.length;let s=0,p=0;const c={1:"1er",2:"2e",3:"3e",4:"4e"},y=[{type:"stadium",label:"Booster Stade",icon:"/icons/booster-stade.png"},{type:"formation",label:"Booster Formation",icon:"/icons/booster-formation.png"},{type:"game_changer",label:"Booster Game Changer",icon:"/icons/booster-gamechanger.png"}];function x(){const g=[];return a.filter(L=>L.type==="player").forEach(L=>{g.push({type:"player",queueItem:L,label:`Ouvrir mon ${c[L._ordinal]||L._ordinal+"e"} booster`,icon:"/icons/booster-players.png"})}),y.forEach(L=>{a.some(u=>u.type===L.type)&&g.push({type:L.type,label:L.label,icon:L.icon})}),g}async function m(){const{data:g,error:L}=await v.rpc("get_tutorial_steps");if(!L&&(g==null?void 0:g.length)>0)return g.map(j=>({emoji:j.emoji,title:j.title,color:j.color,content:j.content,image_url:j.image_url||null}));const{data:u}=await v.from("tutorial_steps").select("*").eq("is_active",!0).order("step_order");return(u==null?void 0:u.length)>0?u.map(j=>({emoji:j.emoji,title:j.title,color:j.color,content:j.content,image_url:j.image_url||null})):null}function f(){var j;const g=!!e.profile.tutorial_done,L=x(),u=l-L.length;t.innerHTML=`
    <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
      <div style="max-width:440px;width:100%;background:rgba(0,0,0,0.35);border:1px solid rgba(255,215,0,0.35);border-radius:16px;padding:20px;color:#fff">
        ...
        <p style="font-size:12.5px;color:rgba(255,255,255,0.55);margin:0 0 16px">
          ${L.length} récompense${L.length>1?"s":""} à récupérer${g?"":" — termine le tutoriel pour les débloquer"}
        </p>
        ...
        <div style="display:flex;align-items:center;gap:12px;padding:12px;border-radius:12px;
          ...
          <button class="btn btn-sm reward-open-btn" data-type="${T.type}" data-ordinal="${((F=T.queueItem)==null?void 0:F._ordinal)||""}" ${g?"":"disabled"}
    ...
    </div>`,(j=document.getElementById("btn-do-tutorial"))==null||j.addEventListener("click",async()=>{await m(),$n(async()=>{await v.from("users").update({tutorial_done:!0}).eq("id",e.user.id),n&&await n();const{data:T}=await v.from("users").select("*").eq("id",e.user.id).single();T&&(e.profile=T),f()})}),t.querySelectorAll(".reward-open-btn").forEach(T=>{T.addEventListener("click",()=>{if(T.disabled)return;const F=T.dataset.type,Y=T.dataset.ordinal?Number(T.dataset.ordinal):null,X=Y!=null?a.find(M=>M.type==="player"&&M._ordinal===Y):a.find(M=>M.type===F);X&&(p=1,_(X))})})}f();const h=async()=>{await v.from("users").update({pending_boosters:a}).eq("id",e.user.id)};async function _(g=null){var Y;if(!a.length){await v.from("users").update({pending_boosters:[],onboarding_done:!0}).eq("id",e.user.id),n&&await n(),o("Tous tes boosters sont ouverts ! Bon jeu 🎮","success",4e3),i("home");return}const L=g||a[0],{data:u}=await v.from("users").select("*").eq("id",e.user.id).single();u&&(e.profile=u);let j=[];try{if(L.type==="formation")j=await $o(e.profile,0);else if(L.type==="game_changer")j=await ko(e.profile,L.count||3,0);else if(L.type==="stadium")j=await $r(e.profile,0);else if(d&&((Y=d.rates)!=null&&Y.length)){const X={...d,cost:0,cardCount:L.count||d.cardCount||5};j=await Ri(e.profile,X),L.guaranteeGK&&!e.profile.first_booster_opened&&(j.some(w=>w.player&&w.player.job==="GK")||await Tr(e.profile,j),await v.from("users").update({first_booster_opened:!0}).eq("id",e.profile.id))}else j=await _o(e.profile,L.count||5,0)}catch(X){o(X.message||"Erreur ouverture booster","error");return}const T=a.indexOf(L);T!==-1&&a.splice(T,1),s++,await h();const F=L.type==="formation"?{name:"Booster Formation",type:"formation",img:"/icons/booster-formation.png"}:L.type==="game_changer"?{name:"Booster Game Changer",type:"game_changer",img:"/icons/booster-gamechanger.png"}:L.type==="stadium"?{name:"Booster Stade",type:"stadium",img:"/icons/booster-stade.png"}:{name:`Booster Joueurs (${L._ordinal||s}/4)`,type:"player",img:(d==null?void 0:d.img)||"/icons/booster-players.png"};Tn(e.profile.id,F,j),p--,Vi(j,F,i,()=>{p>0&&a.length?_():a.length?f():_()})}}async function Tr(t,e){try{const{data:i}=await v.from("players").select("id,job,firstname,surname_real,country_code,club_id,rarity,note_g,note_d,note_m,note_a,skin,hair,hair_length,face,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0).eq("job","GK");if(!(i!=null&&i.length))return;const o=i[Math.floor(Math.random()*i.length)],n=e.findIndex(a=>a.player);if(n===-1)return;const r=e[n];await v.from("cards").update({player_id:o.id}).eq("id",r.id),e[n]={...r,player_id:o.id,player:o}}catch(i){console.warn("[Onboarding] ensureGK échec",i)}}const Ir=[{key:"home2",route:"home2",label:"ACCUEIL",icon:"nav-home.png"},{key:"cards",route:"collection",label:"CARTES",icon:"nav-collection.png"},{key:"decks",route:"decks",label:"EQUIPES",icon:"nav-decks.png"},{key:"boosters",route:"boosters",label:"BOUTIQUE",icon:"nav-boosters.png"},{key:"market",route:"market",label:"MERCATO",icon:"nav-market.png"}];function li(t,e,i,o,n){if(!document.getElementById("home2-chrome-style")){const c=document.createElement("style");c.id="home2-chrome-style",c.textContent=`
    ...
    </div>`,document.body.appendChild(e),e.querySelector("#journal-close-v2").addEventListener("click",()=>e.remove()),e.addEventListener("click",o=>{o.target===e&&e.remove()})}async function Qi(t,{state:e,navigate:i,toast:o,openModal:n,closeModal:r}){var T,F,Y,X,M,w;const a=e.profile;if(!a)return;const d="/icons/";li(i,a,"home2",d,o);const l=a.mmr??450,s=zt(l),p=Bi(l,s),c=oo(l),y=isFinite(s.max)?s.max+1:null,[{list:x,iAmInTop:m,myPosition:f},h,_,{count:$}]=await Promise.all([qr(a.id),Br(5),Dr(),v.from("pending_rewards").select("*",{count:"exact",head:!0}).eq("user_id",a.id).eq("claimed",!1)]),g=(b,E)=>{const H=zt(b.mmr??0),S=Bi(b.mmr??0,H);return`
  ...
  <div class="home-dark" id="home-dark">
      ...
      <div class="home2-dash">
        ...
        <div class="home2-col-right">
          <div class="news-widget">
            ...
            ${h.length?h.map(L).join(""):'<div style="font-size:11px;color:var(--tile-fg-dim);padding:8px 0">Aucune actualité pour le moment.</div>'}
          </div>
        </div>
  ...
  </div>`,document.body.appendChild(i);const o=()=>i.remove();document.getElementById("solo-cancel").addEventListener("click",o),i.addEventListener("click",s=>{s.target===i&&o()});const[{data:n},{data:r}]=await Promise.all([v.from("solo_levels").select("*").eq("is_active",!0).order("level_number"),v.from("user_solo_progress").select("unlocked_level").eq("user_id",e.profile.id).maybeSingle()]),a=(r==null?void 0:r.unlocked_level)||1,d=n||[],l=document.getElementById("solo-levels-list");if(l){if(!d.length){l.innerHTML='<div style="text-align:center;color:#999;padding:20px">Aucun niveau configuré.</div>';return}l.innerHTML=d.map(s=>{const p=s.level_number>a,c=s.level_number===a,y=p?"#f0f0f0":c?"#eefaf2":"#f7f7f7",x=p?"#ddd":c?"#bfe8cf":"#e0e0e0",m=p?"#999":"#12401f";return`
      <div class="solo-level-card" data-level="${s.level_number}" data-locked="${p}"
        style="cursor:${p?"not-allowed":"pointer"};display:flex;align-items:center;gap:14px;padding:14px 16px;border-radius:14px;background:${y};border:1px solid ${x};opacity:${p?.6:1};transition:transform .12s ease">
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      ${y.map(({friendshipId:x,friend:m})=>Kr(m,x)).join("")}
    </div>`,s.querySelectorAll("[data-stats]").forEach(x=>{x.addEventListener("click",()=>Wr(t,x.dataset.stats,x.dataset.friendName))}),s.querySelectorAll("[data-match]").forEach(x=>{x.addEventListener("click",async()=>{const m=x.dataset.friendId,f=x.dataset.friendName;if(typeof o!="function"){e("Erreur navigation","error");return}const h=t.user.id,{data:_}=await v.from("friend_match_invites").select("id").eq("inviter_id",m).eq("invitee_id",h).eq("status","pending").order("created_at",{ascending:!1}).limit(1).maybeSingle(),$=!!_;console.log("[Friends] clic match",{fid:m,fname:f,isAccepting:$}),wt(),o("match",{matchMode:"friend",friendId:m,friendName:f,isAccepting:$})})})}function Kr(t,e){const i=t.club_name||t.pseudo||"?",o=t.pseudo||"",n=(o||i).slice(0,2).toUpperCase(),r=t.club_color2||Ei,a=t.club_color1||"#ffffff",d=t.last_seen_at?new Date(t.last_seen_at):null,l=d&&Date.now()-d.getTime()<3*60*1e3;return`
    ...
    <div class="popup-box">
      ...
      ${p.length?`<div style="display:flex;flex-direction:column;gap:8px;max-height:50vh;overflow-y:auto;margin-bottom:14px">
            ${p.map(y=>{var x,m,f;return`
              <div style="display:flex;align-items:center;gap:10px;background:#f9f9f9;border-radius:10px;padding:10px 12px">
                <div style="flex:1;font-size:14px;font-weight:700">${((x=y.requester)==null?void 0:x.club_name)||((m=y.requester)==null?void 0:m.pseudo)||"?"}
    ...
    ${Lo()}`,document.body.appendChild(s);const c=()=>s.remove();s.querySelector("#pending-close").addEventListener("click",c),s.addEventListener("click",y=>{y.target===s&&c()}),s.querySelectorAll("[data-accept]").forEach(y=>{y.addEventListener("click",async()=>{const{error:x}=await v.from("friendships").update({status:"accepted"}).eq("id",y.dataset.accept);if(x){e("Erreur : "+x.message,"error");return}y.closest("div[style]").remove(),e("✅ Ami accepté !","success"),qn(t,e,o),i&&i()})}),s.querySelectorAll("[data-decline]").forEach(y=>{y.addEventListener("click",async()=>{await v.from("friendships").delete().eq("id",y.dataset.decline),y.closest("div[style]").remove(),e("Demande refusée","info"),i&&i()})})}async function Wr(t,e,i){const o=t.user.id,[n,r]=[o,e].sort(),a=o===n,{data:d}=await v.from("friend_match_stats").select("*").eq("player1_id",n).eq("player2_id",r).single(),l=t.profile.club_name||t.profile.pseudo||"Moi",s=d||{},p=a?s.wins_p1||0:s.wins_p2||0,c=a?s.wins_p2||0:s.wins_p1||0,y=s.draws||0,x=a?s.goals_p1||0:s.goals_p2||0,m=a?s.goals_p2||0:s.goals_p1||0,f=a?s.gc_used_p1||0:s.gc_used_p2||0,h=a?s.gc_used_p2||0:s.gc_used_p1||0,_=s.matches_total||0,$=(L,u,j,T=Ei,F=Li)=>`
    ...
    ${Lo()}`,document.body.appendChild(g),g.querySelector("#stats-close").addEventListener("click",()=>g.remove()),g.addEventListener("click",L=>{L.target===g&&g.remove()})}function Eo(){const t=document.createElement("div");return t.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:8000;display:flex;align-items:center;justify-content:center;padding:20px",t}function Lo(){return`
    <style>
      .popup-box {
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
  ...
  </div>`,t.querySelectorAll("[data-theme-choice]").forEach(g=>{g.addEventListener("click",()=>{an(g.dataset.themeChoice),Dn(t,e)})});const s=t.querySelector("#team-color1"),p=t.querySelector("#team-color2"),c=t.querySelector("#team-preview"),y=()=>{c&&(c.style.background=`linear-gradient(135deg, ${s.value} 50%, ${p.value} 50%)`)};y(),s==null||s.addEventListener("input",y),p==null||p.addEventListener("input",y),(_=t.querySelector("#team-save"))==null||_.addEventListener("click",async()=>{const g=t.querySelector("#team-error"),L=t.querySelector("#team-save"),u=t.querySelector("#team-club-name").value.trim();if(u.length<2){g.textContent="Nom de club trop court.";return}L.disabled=!0,L.textContent="⏳ Enregistrement...",g.textContent="";const{error:j}=await v.from("users").update({club_name:u,club_color1:s.value,club_color2:p.value}).eq("id",i.profile.id);if(L.disabled=!1,L.textContent="💾 Enregistrer",j){g.textContent=j.message;return}i.profile.club_name=u,i.profile.club_color1=s.value,i.profile.club_color2=p.value,g.style.color="#2ecc71",g.textContent="✅ Modifications enregistrées.",wi()==="club"&&an("club")});const x=t.querySelector("#volume-slider"),m=t.querySelector("#volume-label");let f=null;x.addEventListener("input",()=>{nr(Number(x.value)),m.textContent=`${x.value}%`,f&&(f.volume=Math.max(0,Math.min(1,Number(x.value)/100)))}),t.querySelector("#volume-test").addEventListener("click",()=>{f=ho("/sounds/match-opening.mp3",1)}),($=t.querySelector("#settings-tutorial"))==null||$.addEventListener("click",()=>{$n(()=>{})}),t.querySelector("#settings-logout").addEventListener("click",async()=>{await v.auth.signOut(),window.location.reload()})}const Jr={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"},mi={GK:"#111111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},Uo=["GK","DEF","MIL","ATT"],Xr=["Tous","GK","DEF","MIL","ATT"],Qr={normal:1e3,pepite:5e3,papyte:5e3,legende:1e4};function Ko(t){const e=t.player;return e?(e.rarity,Math.max(Number(e.note_g)||0,Number(e.note_d)||0,Number(e.note_m)||0,Number(e.note_a)||0)):null}function Zr(t){return t.length?t.reduce((e,i)=>Ko(i)>Ko(e)?i:e,t[0]):t[0]}const ea={MA:"MAROC",FR:"FRANCE",AR:"ARGENTINE",PT:"PORTUGAL",BR:"BRESIL",ES:"ESPAGNE",DE:"ALLEMAGNE",GB:"ANGLETERRE",IT:"ITALIE",CM:"CAMEROUN",SN:"SENEGAL",NG:"NIGERIA",DK:"DANEMARK",NL:"PAYS-BAS",BE:"BELGIQUE",CI:"CÔTE D'IVOIRE",AL:"ALBANIE",HR:"CROATIE",RS:"SERBIE",TR:"TURQUIE"};function Ti(t,e){return t&&Number(e==="GK"?t.note_g:e==="DEF"?t.note_d:e==="MIL"?t.note_m:t.note_a)||0}function Zi(t,e="",i=1){const o=t.player;if(!o)return"";const n=t.evolution_bonus||0,r={...o,_evolution_bonus:n},a=e?`<div style="position:absolute;top:6px;right:6px;z-index:10;background:#0a3d1e;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 7px">${e}</div>`:"",d=i>1?`
    <style>
    ...
    ${qe(t,{width:140,context:"collection"})}
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),s=(d||[]).filter(D=>D.card_type==="player"&&D.player),p=(d||[]).filter(D=>D.card_type==="game_changer"),c=(d||[]).filter(D=>D.card_type==="formation"),y=(d||[]).filter(D=>D.card_type==="stadium"),{data:x}=await v.from("gc_definitions").select("name,gc_type,color,effect,image_url"),m={};(x||[]).forEach(D=>{m[D.name]=D});const{data:f}=await v.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),h={};(f||[]).forEach(D=>{h[D.id]=D}),y.forEach(D=>{D.stadium_def&&(h[D.stadium_id]=D.stadium_def)});const _=Object.keys(zi),$=Object.keys(Qe),g={};s.forEach(D=>{const C=D.player.id;g[C]=(g[C]||0)+1}),new Set(Object.keys(g).map(D=>String(D)));const L=new Set(c.map(D=>D.formation)),u=new Set(p.map(D=>D.gc_type));let j="player",T="Tous",F="",Y=!1,X=!1,M=new Set,w=new Set,b="job";function E(D){return D.job==="GK"?Number(D.note_g)||0:D.job==="DEF"?Number(D.note_d)||0:D.job==="MIL"?Number(D.note_m)||0:Number(D.note_a)||0}function H(D,C){return b==="note_desc"?[...D].sort((K,W)=>E(C(W))-E(C(K))):b==="note_asc"?[...D].sort((K,W)=>E(C(K))-E(C(W))):[...D].sort((K,W)=>{const J=C(K),P=C(W),ee=Uo.indexOf(J.job),me=Uo.indexOf(P.job);return ee!==me?ee-me:(J.surname_real||"").localeCompare(P.surname_real||"")})}function S(){return H(s,D=>D.player)}function G(){return H(l||[],D=>D)}function de(D){return(!M.size||M.has(D.country_code))&&(!w.size||w.has(D.club_id))}function I(){return S().filter(D=>{const C=D.player,K=T==="Tous"||C.job===T,W=!F||`${C.firstname} ${C.surname_real}`.toLowerCase().includes(F);return K&&W&&de(C)})}function z(){return G().filter(D=>{const C=T==="Tous"||D.job===T,K=!F||`${D.firstname} ${D.surname_real}`.toLowerCase().includes(F);return C&&K&&de(D)})}t.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:0 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none;align-items:center"></div>
  ...
  </div>`,function(C){const K=document.getElementById(C);if(!K)return;K.addEventListener("wheel",xe=>{Math.abs(xe.deltaY)<=Math.abs(xe.deltaX)||(xe.preventDefault(),K.scrollLeft+=xe.deltaY)},{passive:!1});let W=!1,J=0,P=0,ee=!1,me=!1;K.addEventListener("mousedown",xe=>{W=!0,ee=!1,J=xe.pageX,P=K.scrollLeft}),window.addEventListener("mouseup",()=>{W=!1,ee&&(K.style.cursor="",me=!0),ee=!1}),window.addEventListener("mousemove",xe=>{if(!W)return;const ue=xe.pageX-J;!ee&&Math.abs(ue)<6||(ee=!0,K.style.cursor="grabbing",xe.preventDefault(),K.scrollLeft=P-ue)}),K.addEventListener("click",xe=>{me&&(xe.stopPropagation(),xe.preventDefault(),me=!1)},!0)}("col-grid");function Z(){const D=document.getElementById("col-filters");D&&(j==="player"?(D.innerHTML=`
        <input id="col-search" placeholder="🔍 Rechercher un joueur..." style="font-size:13px;background:#fff;color:#1a1a1a" value="${F}">
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",C=>{F=C.target.value.toLowerCase(),te()}),t.querySelectorAll(".filter-btn").forEach(C=>{C.addEventListener("click",()=>{T=C.dataset.job,Z(),te()})}),document.getElementById("dupes-only-btn").addEventListener("click",()=>{X=!X,Z(),te()}),document.getElementById("show-all-btn").addEventListener("click",()=>{Y=!Y,Z(),te()}),document.getElementById("sort-note-btn").addEventListener("click",()=>{b=b==="job"?"note_desc":b==="note_desc"?"note_asc":"job",Z(),te()}),document.getElementById("country-filter-btn").addEventListener("click",()=>{U("country","🌍 Filtrer par pays",e)}),document.getElementById("club-filter-btn").addEventListener("click",()=>{U("club","🏟️ Filtrer par club",e)})):(D.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{Y=!Y,Z(),te()})))}function U(D,C,K){var ue,Le,Ie;const W=D==="country",J=W?M:w,P=Y?l||[]:s.map(we=>we.player);let ee;if(W){const we=new Map;P.forEach(ke=>{ke!=null&&ke.country_code&&!we.has(ke.country_code)&&we.set(ke.country_code,ke.country_code)}),ee=[...we.keys()].sort().map(ke=>({value:ke,label:ke}))}else{const we=new Map;P.forEach(ke=>{var k;ke!=null&&ke.club_id&&!we.has(ke.club_id)&&we.set(ke.club_id,((k=ke.clubs)==null?void 0:k.encoded_name)||ke.club_id)}),ee=[...we.entries()].sort((ke,k)=>ke[1].localeCompare(k[1])).map(([ke,k])=>({value:ke,label:k}))}const me=`
      ...
      </div>`;K.openModal(C,me,'<button class="btn btn-primary" id="mf-apply" style="width:100%">Appliquer</button>'),(ue=document.getElementById("mf-all"))==null||ue.addEventListener("click",()=>{document.querySelectorAll(".mf-check").forEach(we=>we.checked=!0)}),(Le=document.getElementById("mf-none"))==null||Le.addEventListener("click",()=>{document.querySelectorAll(".mf-check").forEach(we=>we.checked=!1)}),(Ie=document.getElementById("mf-apply"))==null||Ie.addEventListener("click",()=>{J.clear(),document.querySelectorAll(".mf-check:checked").forEach(we=>J.add(we.value)),K.closeModal(),Z(),te()})}function te(){const D=document.getElementById("col-grid");D&&(j==="player"?Pe(D):j==="formation"?_e(D):j==="stadium"?Ee(D):ye(D))}function pe(D,C,K,W,J,P=0){const ee=document.getElementById("col-grid"),me=document.getElementById("col-big");if(!ee||!me)return;var xe=0;function ue(){var q;const Le=window.innerWidth>=768,Ie=document.getElementById("col-big"),we=document.getElementById("col-gap"),ke=((q=document.getElementById("col-grid"))==null?void 0:q.parentElement)||null,k=Ie?Ie.closest(".page"):null;if(Ie&&ke&&k){let N=0;Array.from(k.children).forEach(function(oe){oe!==Ie&&oe!==ke&&oe!==we&&(N+=oe.offsetHeight)});const R=Math.max(0,k.clientHeight-N),V=Math.round(R*(P/100)),Q=Math.max(0,R-V),ie=Le?50/71:45/63,re=Math.round(Q*ie),se=Math.max(0,Q-re);we&&(we.style.height=V+"px"),Ie.style.flex="none",Ie.style.height=re+"px",Ie.style.minHeight="0",ke.style.flex="none",ke.style.height=se+"px",ke.style.minHeight="0",ke.style.display="flex",ke.style.overflow="hidden"}const A=document.getElementById("col-grid");A&&(A.style.height="100%",A.style.flexShrink="0",A.style.overflowX="auto",A.style.overflowY="hidden",A.style.alignItems="center",A.style.width="100%"),me.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:center center">'+C(D[xe])+"</div>";var B=me.querySelector("[data-card-id],[data-form-id],[data-gc-id]");B&&B.addEventListener("click",function(){W(D[xe])});var O=me.querySelector(".big-evolve-btn");O&&O.addEventListener("click",function(N){N.stopPropagation(),W(D[xe],{autoEvolve:!0})}),requestAnimationFrame(function(){var N=document.getElementById("big-card-inner");if(!(!N||!me)){var R=me.clientHeight,V=me.clientWidth-16,Q=N.offsetHeight,ie=N.offsetWidth;if(Q>0&&ie>0&&R>40){var re=Math.min(R/Q,V/ie);N.style.transform="scale("+re.toFixed(3)+")",N.style.transformOrigin="center center"}}}),ee.innerHTML=D.map(function(N,R){var V=R===xe,Q="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+(V?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+R+'" style="'+Q+'">'+K(N,V)+"</div>"}).join(""),ee.querySelectorAll(".col-mini-item").forEach(function(N){N.addEventListener("click",function(){xe=Number(N.dataset.idx),ue(),N.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})}),requestAnimationFrame(function(){var N=A?A.clientHeight:0,R=ee.querySelector(".col-mini-item > div");if(!(!N||!R)){var V=R.style.zoom;R.style.zoom="1";var Q=R.offsetHeight;if(R.style.zoom=V,!(Q<=0)){var ie=N/Q;ee.querySelectorAll(".col-mini-item > div").forEach(function(re){re.style.zoom=ie.toFixed(4)})}}})}ue()}function he(D,C){var K=window.innerWidth>=768?.76:.54,W;if(!D||D._fake){var J=D?D.player:null;if(!J)return"";W=Vo(J)}else W=Zi(D,"");var P=D&&!D._fake?g[D.player&&D.player.id]||1:0,ee=P>0?'<div style="position:absolute;top:6px;right:6px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:11px;font-weight:900;padding:2px 7px;z-index:3;box-shadow:0 1px 4px rgba(0,0,0,0.4)">×'+P+"</div>":"";return'<div style="display:inline-block;position:relative;zoom:'+K+';pointer-events:none;line-height:0">'+ee+W+"</div>"}function fe(D,C,K){var W=K>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+K+"</div>":"",J=!!C,P=oi(D,vt[D],{width:160});return J||(P='<div style="filter:grayscale(1);opacity:0.5">'+P+"</div>"),"<div "+(C?'data-form-id="'+C.id+'"':"")+' style="position:relative;cursor:pointer">'+W+P+"</div>"}function Be(D,C){var K=window.innerWidth>=768?.76:.54,W=oi(D,vt[D],{width:140});return C||(W='<div style="filter:grayscale(1);opacity:0.45">'+W+"</div>"),'<div style="display:inline-block;zoom:'+K+';line-height:0;pointer-events:none">'+W+"</div>"}function Pe(D){if(Y){const C=z();if(!C.length){D.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucun joueur.</div>';return}const K=C.map(W=>s.find(J=>J.player.id===W.id)||{_fake:!0,player:W,id:"fake-"+W.id});pe(K,W=>W._fake?Vo(W.player):Zi(W,""),W=>W._fake?he({player:W.player,id:"x",_fake:!0}):he(W),W=>{W._fake||Wo(W,s,g,e)})}else{const C=I();if(!C.length){D.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const K={};C.forEach(J=>{const P=J.player.id;K[P]||(K[P]=[]),K[P].push(J)});const W=Object.values(K).map(J=>Zr(J)).filter(J=>!X||(g[J.player.id]||1)>1);if(X&&!W.length){D.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte en plusieurs exemplaires.</div>';return}pe(W,J=>{const P=g[J.player.id]||1,ee=P>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${P}</div>`:"",xe=s.filter(ue=>ue.player.id===J.player.id&&ue.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Zi(J,ee+xe,P)},J=>he(J),(J,P)=>Wo(J,s,g,e,P))}}function _e(D){const C=Y?_:[...L];if(!C.length){D.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const K=C.map(W=>({formation:W,card:c.find(J=>J.formation===W)||null,owned:L.has(W)}));pe(K,({formation:W,card:J,owned:P})=>fe(W,P?J:null,P?c.filter(ee=>ee.formation===W).length:0),({formation:W,owned:J})=>Be(W,J),({card:W,owned:J})=>{J&&W&&oa(W,c,e,r)},"#1A6B3C",5)}function ye(D){const C=Object.keys(m),K=Y?C.length?C:$:[...u];if(!K.length){D.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const W=K.map(J=>({type:J,gc:Qe[J]||{icon:"⚡",desc:""},def:m[J]||null,owned:u.has(J),card:p.find(P=>P.gc_type===J)||null}));pe(W,({type:J,gc:P,def:ee,owned:me,card:xe})=>{var A;const ue=(ee==null?void 0:ee.name)||J,Le=me?p.filter(B=>B.gc_type===J).length:0,Ie=Le>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${Le}</div>`:"",we=(ee==null?void 0:ee.effect)||P.desc||"",ke=ee!=null&&ee.image_url?`/icons/${ee.image_url}`:((A=ee==null?void 0:ee.club)==null?void 0:A.logo_url)||(ee!=null&&ee.country_code?`https://flagsapi.com/${ee.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null);let k=Ze(ue,ke,P.icon,we,{width:160,onClick:me});return me||(k=`<div style="filter:grayscale(1);opacity:0.5">${k}</div>`),`<div ${me&&xe?`data-gc-id="${xe.id}" data-gc-type="${J}"`:""} style="position:relative">${Ie}${k}</div>`},({type:J,gc:P,def:ee,owned:me})=>{const xe=window.innerWidth>=768?.76:.54,ue=(ee==null?void 0:ee.name)||J,Le=(ee==null?void 0:ee.effect)||P.desc||"",Ie=ee!=null&&ee.image_url?`/icons/${ee.image_url}`:null;let we=Ze(ue,Ie,P.icon,Le,{width:140});return me||(we=`<div style="filter:grayscale(1);opacity:0.45">${we}</div>`),`<div style="display:inline-block;zoom:${xe};line-height:0;pointer-events:none">${we}</div>`},({type:J,owned:P,def:ee})=>{P&&ia(J,ee,r)},"#7a28b8",5)}function Ee(D){const C="#4FC3F7",K="/";if(!y.length){D.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Stade.<br><small>Ouvre un booster Stade !</small></div>';return}const W={};y.forEach(P=>{const ee=P.stadium_id||"?";(W[ee]=W[ee]||[]).push(P)});const J=Object.entries(W).map(([P,ee])=>({sid:P,def:h[P]||null,count:ee.length,card:ee[0]}));pe(J,({def:P,count:ee})=>{var we,ke;const me=(P==null?void 0:P.name)||"?",xe=((we=P==null?void 0:P.club)==null?void 0:we.encoded_name)||(P==null?void 0:P.country_code)||"—",ue=P!=null&&P.image_url?`${K}icons/${P.image_url}`:((ke=P==null?void 0:P.club)==null?void 0:ke.logo_url)||(P!=null&&P.country_code?`https://flagsapi.com/${P.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),Le=ee>1?`<div style="position:absolute;top:8px;right:8px;background:#333;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${ee}</div>`:"",Ie=`${xe}<br>+10 ⭐ joueurs alliés`;return`<div style="position:relative">${Le}${Kt(me,ue,Ie,{width:160})}</div>`},({def:P})=>{var Ie,we;const ee=window.innerWidth>=768?.76:.54,me=(P==null?void 0:P.name)||"?",xe=((Ie=P==null?void 0:P.club)==null?void 0:Ie.encoded_name)||(P==null?void 0:P.country_code)||"—",ue=P!=null&&P.image_url?`${K}icons/${P.image_url}`:((we=P==null?void 0:P.club)==null?void 0:we.logo_url)||(P!=null&&P.country_code?`https://flagsapi.com/${P.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),Le=`${xe}<br>+10 ⭐`;return`<div style="display:inline-block;zoom:${ee};line-height:0;pointer-events:none">${Kt(me,ue,Le,{width:140})}</div>`},null,C,5)}t.querySelectorAll(".col-tab-btn").forEach(D=>{D.addEventListener("click",()=>{j=D.dataset.tab,T="Tous",F="",Y=!1,t.querySelectorAll(".col-tab-btn").forEach(C=>{const K=C.dataset.tab===j;C.style.borderBottomColor=K?"var(--green)":"transparent",C.style.color=K?"var(--green)":"var(--tile-fg-dim)"}),Z(),te()})}),Z(),te()}function ia(t,e,i){const o=Qe[t]||{icon:"⚡",desc:"Effet spécial."},n=(e==null?void 0:e.name)||t,r=(e==null?void 0:e.effect)||o.desc,a=e!=null&&e.image_url?`/icons/${e.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
      ${Ze(n,a,o.icon,r,{width:200})}
      <div style="background:#fff3cd;border-radius:10px;padding:10px 14px;width:100%">
        <div style="font-size:12px;color:#856404">⚠️ Cette carte est à <b>usage unique</b>. Une fois jouée en match, elle est définitivement supprimée de ta collection.</div>
      </div>
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Ii=1e3;function oa(t,e,i,o){var m,f,h;const{state:n,toast:r,closeModal:a,navigate:d,refreshProfile:l}=i,s=t.formation,p={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function c(){const _=vt[s]||{},$=zi[s]||[],g=290,L=360,u=20;function j(F){const Y=_[F];return Y?{x:Y.x*g,y:Y.y*L}:null}let T=`<svg width="${g}" height="${L}" viewBox="0 0 ${g} ${L}" xmlns="http://www.w3.org/2000/svg">`;for(const[F,Y]of $){const X=j(F),M=j(Y);!X||!M||(T+=`<line x1="${X.x}" y1="${X.y}" x2="${M.x}" y2="${M.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const F of Object.keys(_)){const Y=j(F);if(!Y)continue;const X=F.replace(/\d+/,""),M=p[X]||"#555";T+=`<circle cx="${Y.x}" cy="${Y.y}" r="${u}" fill="${M}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,T+=`<text x="${Y.x}" y="${Y.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${X}</text>`}return T+="</svg>",T}const y=e.filter(_=>_.formation===s);y.length;const x=!t.is_for_sale;o(`Formation ${s}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(m=document.getElementById("direct-sell-form-btn"))==null||m.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${s} pour ${Ii.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const _=y.find(g=>!g.is_for_sale)||y[0];if(!_){r("Aucune carte à vendre","error");return}await v.from("market_listings").delete().eq("card_id",_.id),await v.from("transfer_history").delete().eq("card_id",_.id);const{error:$}=await v.from("cards").delete().eq("id",_.id);if($){r($.message,"error");return}await v.from("users").update({credits:(n.profile.credits||0)+Ii}).eq("id",n.profile.id),await l(),r(`+${Ii.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),a(),d("collection")}),(f=document.getElementById("market-sell-form-btn"))==null||f.addEventListener("click",async()=>{const _=parseInt(document.getElementById("sell-price-form").value);if(!_||_<1){r("Prix invalide","error");return}await v.from("cards").update({is_for_sale:!0,sale_price:_}).eq("id",t.id),await v.from("market_listings").insert({seller_id:n.profile.id,card_id:t.id,price:_}),r("Carte mise en vente sur le marché !","success"),a(),d("collection")}),(h=document.getElementById("cancel-sell-form-btn"))==null||h.addEventListener("click",async()=>{await v.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",t.id),await v.from("market_listings").update({status:"cancelled"}).eq("card_id",t.id).eq("status","active"),r("Annonce retirée","success"),a(),d("collection")})}async function Wo(t,e,i,o,n={}){var U,te,pe,he,fe,Be,Pe,_e,ye,Ee,D;const{state:r,toast:a,openModal:d,closeModal:l,navigate:s,refreshProfile:p}=o,c=t.player,y=e.filter(C=>C.player.id===c.id),x=y.length,m=t.evolution_bonus||0,f=Math.max((Number(c.note_g)||0)+(c.job==="GK"||c.job2==="GK"?m:0),(Number(c.note_d)||0)+(c.job==="DEF"||c.job2==="DEF"?m:0),(Number(c.note_m)||0)+(c.job==="MIL"||c.job2==="MIL"?m:0),(Number(c.note_a)||0)+(c.job==="ATT"||c.job2==="ATT"?m:0)),h=c.rarity||"normal",{data:_}=await v.from("sell_price_configs").select("*").eq("rarity",h).lte("note_min",f).gte("note_max",f).order("note_min",{ascending:!1}).limit(1),$=((U=_==null?void 0:_[0])==null?void 0:U.price)??Qr[h]??1e3,g=((te=_==null?void 0:_[0])==null?void 0:te.price_min)??null,L=((pe=_==null?void 0:_[0])==null?void 0:pe.price_max)??null;Ut(c);const u=Ti(c,c.job)+m,j=c.job2?Ti(c,c.job2)+(Ti(c,c.job2)>0?m:0):null;mi[c.job],c.job2&&mi[c.job2];const T=Jr[c.rarity]||"#ccc",F=ea[c.country_code]||c.country_code||"",Y=t.evolution_bonus||0,M=u+Y,w=j||0,b=w>0?w+Y:0,E=y.map(C=>C.id);let H={};if(E.length){const{data:C}=await v.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",E).order("transferred_at",{ascending:!0});(C||[]).forEach(K=>{H[K.card_id]||(H[K.card_id]=[]),H[K.card_id].push(K)})}const S=E.length?`
    <div style="margin-top:16px;border-top:1px solid var(--tile-border);padding-top:14px">
      ...
      `:`
        <div style="background:#f0fdf4;border:2px solid #1A6B3C;border-radius:12px;padding:14px;margin-bottom:14px">
          ...
          <div style="font-size:11px;color:#555;margin-bottom:8px;background:#fff;border-radius:6px;padding:6px 10px">
            💰 Fourchette autorisée : <b>${g.toLocaleString("fr")}</b> – <b>${L.toLocaleString("fr")}</b> cr.
          </div>`:""}
      ...
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px">
        ${y.filter(C=>C.id!==t.id).map((C,K)=>{const W=H[C.id]||[],J=C.is_for_sale,P=W.length?W[W.length-1]:null,ee=C.evolution_bonus||0,me=P?P.source==="booster"?"Booster":P.price?P.price.toLocaleString("fr")+" cr.":"—":"—",xe=P?new Date(P.transferred_at).toLocaleDateString("fr",{day:"2-digit",month:"2-digit",year:"numeric"}):"",ue=80,Le=Math.round(ue*657/507),Ie=qe({...c,_evolution_bonus:ee},{width:ue,context:"collection"});return`
            <div class="exemplaire-row" data-card-id="${C.id}" data-card-idx="${K}"
              ...
              <div class="expl-mini-card" style="position:relative;width:${ue}px;height:${Le}px;border-radius:8px;overflow:hidden">
                ${Ie}
      ...
      <div id="sell-action-panel" style="display:none;margin-top:12px;background:#f0fdf4;border:2px solid #1A6B3C;border-radius:12px;padding:14px">
        ...
        <div style="border-top:1px solid #d1fae5;padding-top:8px">
          ...
          ${g!==null&&L!==null?`
          <div style="font-size:11px;color:#555;margin-bottom:8px;background:#fff;border-radius:6px;padding:6px 10px">
            💰 Fourchette autorisée : <b>${g.toLocaleString("fr")}</b> – <b>${L.toLocaleString("fr")}</b> cr. / carte
          </div>`:""}
          <div style="display:flex;gap:8px;margin-bottom:8px">
      ...
      <div style="flex:1;min-width:220px;display:flex;flex-direction:column;gap:12px">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px 16px;background:#f8f8f8;border-radius:10px;padding:10px 12px">
          <div>
            ...
            <div style="font-weight:700;font-size:13px;color:#333">${F||"—"}</div>
          </div>
          <div>
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",c.note_g],["DEF",c.note_d],["MIL",c.note_m],["ATT",c.note_a]].map(([C,K])=>{const W=mi[C],J=C==="GK"?"#fff":W,P=C===c.job||C===c.job2,ee=(Number(K)||0)+(P&&Y>0?Y:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(fe=document.getElementById("close-detail"))==null||fe.addEventListener("click",l);let G=new Set;const de=c.rarity==="pepite"?1.3:c.rarity==="papyte"?.7:1;function I(){let C=0;return document.querySelectorAll(".expl-check:checked").forEach(K=>{if(K.dataset.id===t.id)return;const J=Number(K.dataset.evo)||0,P=Number(K.dataset.note)||0;C+=P+J}),Math.round(C*de)}const z=()=>{const C=G.size,K=document.getElementById("sell-action-panel");if(!K)return;K.style.display=C>0?"block":"none",document.getElementById("sell-selected-count").textContent=C;const W=document.getElementById("evolve-btn");if(W){const J=I();W.textContent=J>0?`⬆️ Évoluer (+${J})`:"⬆️ Évoluer"}};if(document.querySelectorAll(".expl-check").forEach(C=>{C.addEventListener("change",()=>{const K=C.dataset.id;C.checked?G.add(K):G.delete(K);const W=C.closest(".exemplaire-row");if(W){const J=W.querySelector(".expl-sel-overlay"),P=W.querySelector(".expl-sel-check");J&&(J.style.display=C.checked?"block":"none"),P&&(P.style.display=C.checked?"flex":"none"),W.style.transform=C.checked?"scale(1.05)":"scale(1)"}z()})}),n.autoEvolve){document.querySelectorAll(".expl-check").forEach(K=>{if(K.dataset.id===t.id)return;K.checked=!0,G.add(K.dataset.id);const W=K.closest(".exemplaire-row");if(W){const J=W.querySelector(".expl-sel-overlay"),P=W.querySelector(".expl-sel-check");J&&(J.style.display="block"),P&&(P.style.display="flex"),W.style.transform="scale(1.05)"}}),z();const C=document.getElementById("modal-overlay");C&&(C.style.visibility="hidden"),requestAnimationFrame(()=>{const K=document.getElementById("evolve-btn");if(!K||K.disabled){C&&(C.style.visibility="");return}K.click();const W=()=>{C&&(C.style.visibility="",C.classList.contains("hidden")||l())},J=new MutationObserver(()=>{[...document.body.children].some(me=>{var xe;return((xe=me.style)==null?void 0:xe.zIndex)==="9999"})||(J.disconnect(),clearTimeout(P),W())});J.observe(document.body,{childList:!0});const P=setTimeout(()=>{J.disconnect(),W()},6e4)})}document.querySelectorAll(".exemplaire-row").forEach(C=>{C.addEventListener("click",K=>{if(K.target.tagName==="INPUT")return;const W=C.querySelector(".expl-check");W&&!W.disabled&&(W.checked=!W.checked,W.dispatchEvent(new Event("change")))})}),(Be=document.getElementById("evolve-btn"))==null||Be.addEventListener("click",async()=>{if(x<=1)return;const C=[...G];if(!C.length)return;if(G.has(t.id)){const ue=document.createElement("div");ue.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",ue.innerHTML=`
        ...
        </div>`,document.body.appendChild(ue),ue.querySelector("#err-close").addEventListener("click",()=>ue.remove()),ue.addEventListener("click",Le=>{Le.target===ue&&ue.remove()});return}const K=C.filter(ue=>ue!==t.id);if(!K.length){a("Sélectionne des copies à sacrifier","warning");return}const W=K.reduce((ue,Le)=>{const Ie=document.querySelector(`.expl-check[data-id="${Le}"]`),we=Ie&&Number(Ie.dataset.evo)||0,ke=Ie&&Number(Ie.dataset.note)||0;return ue+ke+we},0),J=Math.round(W*de),P=c.rarity==="pepite"?" (+30% pépite ✨)":c.rarity==="papyte"?" (-30% papyte)":"";if(!await new Promise(ue=>{const Le=document.createElement("div");Le.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",Le.innerHTML=`
        <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center;box-shadow:0 16px 48px rgba(0,0,0,0.3)">
          ...
          <div style="background:#f0fdf4;border-radius:10px;padding:12px;margin-bottom:16px;font-size:13px;color:#333">
            ...
            ➕ Brut : <strong>+${W}</strong>${P?`<span style="font-size:11px;color:#888"> ${P}</span>`:""}<br>
        ...
        </div>`,document.body.appendChild(Le),Le.querySelector("#sac-cancel").addEventListener("click",()=>{Le.remove(),ue(!1)}),Le.querySelector("#sac-ok").addEventListener("click",()=>{Le.remove(),ue(!0)}),Le.addEventListener("click",Ie=>{Ie.target===Le&&(Le.remove(),ue(!1))})}))return;if(K.length){await v.from("market_listings").delete().in("card_id",K),await v.from("deck_cards").delete().in("card_id",K),await v.from("transfer_history").delete().in("card_id",K),await v.from("decks").update({stadium_card_id:null}).in("stadium_card_id",K);const{error:ue}=await v.from("cards").delete().in("id",K);if(ue){a("Erreur suppression : "+ue.message,"error");return}}const me=(t.evolution_bonus||0)+J,{error:xe}=await v.from("cards").update({evolution_bonus:me}).eq("id",t.id);if(xe){a("Erreur évolution : "+xe.message,"error");return}a(`⬆️ ${c.firstname} ${c.surname_real} : note ${u} → ${u+me} (+${J}) !`,"success",4e3),l(),s("collection")});async function Z(C){const{data:K}=await v.from("sell_price_configs").select("price_min, price_max").eq("rarity",c.rarity).lte("note_min",u).gte("note_max",u).order("note_max",{ascending:!0}).limit(1).maybeSingle();return K?C<K.price_min||C>K.price_max?{ok:!1,min:K.price_min,max:K.price_max}:{ok:!0}:{ok:!0}}(Pe=document.getElementById("market-sell-btn"))==null||Pe.addEventListener("click",async()=>{var me;const C=[...G];if(!C.length){a("Sélectionne au moins un exemplaire","warning");return}const K=parseInt((me=document.getElementById("sell-market-price"))==null?void 0:me.value);if(!K||K<1){a("Prix invalide","error");return}const W=await Z(K);if(!W.ok){a(`Prix hors grille : entre ${W.min.toLocaleString("fr")} et ${W.max.toLocaleString("fr")} cr. pour cette rareté/note`,"error");return}const{error:J}=await v.from("cards").update({is_for_sale:!0,sale_price:K}).in("id",C);if(J){a(J.message,"error");return}const P=C.map(xe=>({seller_id:r.profile.id,card_id:xe,price:K,status:"active"})),{error:ee}=await v.from("market_listings").insert(P);ee&&console.warn("[Market] insert listings:",ee.message),a(`${C.length} carte${C.length>1?"s":""} mise${C.length>1?"s":""} en vente à ${K.toLocaleString("fr")} cr. chacune !`,"success"),l(),s("collection")}),(_e=document.getElementById("single-sell-btn"))==null||_e.addEventListener("click",async()=>{var P;const C=parseInt((P=document.getElementById("single-sell-price"))==null?void 0:P.value);if(!C||C<1){a("Prix invalide","error");return}const K=await Z(C);if(!K.ok){a(`Prix hors grille : entre ${K.min.toLocaleString("fr")} et ${K.max.toLocaleString("fr")} cr. pour cette rareté/note`,"error");return}const{error:W}=await v.from("cards").update({is_for_sale:!0,sale_price:C}).eq("id",t.id);if(W){a(W.message,"error");return}const{error:J}=await v.from("market_listings").insert({seller_id:r.profile.id,card_id:t.id,price:C,status:"active"});J&&console.warn("[Market] insert listing:",J.message),a(`Carte mise en vente à ${C.toLocaleString("fr")} cr. !`,"success"),l(),s("collection")}),(ye=document.getElementById("single-direct-sell-btn"))==null||ye.addEventListener("click",async()=>{const C=g??$;if(!confirm(`Vendre cette carte immédiatement pour ${C.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;await v.from("market_listings").delete().eq("card_id",t.id),await v.from("transfer_history").delete().eq("card_id",t.id);const{error:K}=await v.from("cards").delete().eq("id",t.id);if(K){a(K.message,"error");return}await v.from("users").update({credits:(r.profile.credits||0)+C}).eq("id",r.profile.id),await p(),a(`+${C.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),l(),s("collection")}),(Ee=document.getElementById("market-direct-sell-btn"))==null||Ee.addEventListener("click",async()=>{const C=[...G];if(!C.length){a("Sélectionne au moins un exemplaire","warning");return}const W=(g??$)*C.length;if(!confirm(`Vendre ${C.length} carte${C.length>1?"s":""} immédiatement pour ${W.toLocaleString("fr")} crédits au total ? Cette action est irréversible.`))return;await v.from("market_listings").delete().in("card_id",C),await v.from("transfer_history").delete().in("card_id",C);const{error:J}=await v.from("cards").delete().in("id",C);if(J){a(J.message,"error");return}await v.from("users").update({credits:(r.profile.credits||0)+W}).eq("id",r.profile.id),await p(),a(`+${W.toLocaleString("fr")} crédits ! ${C.length} carte${C.length>1?"s":""} vendue${C.length>1?"s":""}.`,"success"),l(),s("collection")}),(D=document.getElementById("cancel-sell-btn"))==null||D.addEventListener("click",async()=>{await v.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",t.id),await v.from("market_listings").update({status:"cancelled"}).eq("card_id",t.id).eq("status","active"),a("Annonce retirée","success"),l(),s("collection")})}const na={GK:"note_g",DEF:"note_d",MIL:"note_m",ATT:"note_a"};function ei(t,e){const i=t==null?void 0:t.player;if(!i)return 0;const o=t.evolution_bonus||0;return(Number(i[na[e]])||0)+(e===i.job||e===i.job2?o:0)}function ji(t){return t.replace(/[0-9]/g,"")}function Fn(t,e){if(!(t!=null&&t.player)||!e)return 0;const i=t.player,o=e.club_id&&String(i.club_id)===String(e.club_id),n=e.country_code&&String(i.country_code)===String(e.country_code);return o||n?10:0}function eo(t,e,i){let o=0;for(const[r,a]of Object.entries(t))a&&(o+=ei(a,ji(r)),o+=Fn(a,i));const n=Hi(e)||zi[e]||[];for(const[r,a]of n){const d=t[r],l=t[a];if(!d||!l)continue;const s=Gt(d.player,l.player);s==="#00ff88"?o+=10:s==="#FFD700"&&(o+=5)}return o}function ra(t,e,i){const o={},n=new Set,r=[...t].sort((a,d)=>{const l={GK:0,DEF:1,ATT:2,MIL:3};return l[ji(a)]-l[ji(d)]});for(const a of r){const d=ji(a);let l=null,s=-1;for(const p of e){if(n.has(p.id))continue;const c=ei(p,d)+Fn(p,i);c>s&&(s=c,l=p)}l&&(o[a]=l,n.add(l.id))}return{assign:o,used:n}}function aa(t,e,i,o,n,r,a=6){let d=eo(t,n,r);const l=o.filter(s=>!e.has(s.id));for(let s=0;s<a;s++){let p=!1;for(let c=0;c<i.length;c++)for(let y=c+1;y<i.length;y++){const x=i[c],m=i[y],f=t[x],h=t[m];if(!f||!h)continue;t[x]=h,t[m]=f;const _=eo(t,n,r);_>d?(d=_,p=!0):(t[x]=f,t[m]=h)}for(const c of i){const y=t[c];for(let x=0;x<l.length;x++){const m=l[x];t[c]=m;const f=eo(t,n,r);if(f>d){d=f,p=!0,l[x]=y,e.delete(y==null?void 0:y.id),e.add(m.id);break}else t[c]=y}}if(!p)break}return d}function sa({playerCards:t,availableFormations:e,stadiumCards:i,stadDefMap:o}){const n=(t||[]).filter(d=>d.player);if(n.length<11)return null;const r=[{id:null,def:null}];(i||[]).forEach(d=>{const l=o==null?void 0:o[d.stadium_id];l&&r.push({id:d.id,def:l})});let a=null;for(const d of e||[]){const l=vt[d];if(!l)continue;const s=Object.keys(l);if(s.length===11)for(const p of r){const{assign:c,used:y}=ra(s,n,p.def);if(Object.keys(c).length!==11)continue;const x=aa(c,y,s,n,d,p.def);if(!a||x>a.score){const f=n.filter(_=>!Object.values(c).some($=>$.id===_.id)).map(_=>({c:_,v:Math.max(ei(_,"GK"),ei(_,"DEF"),ei(_,"MIL"),ei(_,"ATT"))})).sort((_,$)=>$.v-_.v).slice(0,5).map(_=>_.c.id),h={};Object.entries(c).forEach(([_,$])=>{h[_]=$.id}),a={formation:d,stadiumCardId:p.id,slots:h,subs:f,score:x}}}}return a}function Yo(t,e=""){return new Promise(i=>{const o=document.createElement("div");o.className="modal-overlay",o.style.zIndex="2100",o.innerHTML=`<div class="modal" style="max-width:360px">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
      ...
      <div style="display:flex;gap:0;min-height:600px">
        ...
        <div style="width:105px;flex-shrink:0;background:rgba(0,0,0,0.3);display:flex;flex-direction:column;align-items:center;padding:12px 6px;gap:8px;border-right:1px solid rgba(255,255,255,0.1)">
          ...
          <div style="display:flex;flex-direction:column;gap:6px;align-items:center" id="subs-list">
            ${c.map(g=>{const L={...g.player,_evolution_bonus:g.evolution_bonus||0};return`<div style="position:relative;flex-shrink:0;overflow:visible;padding-bottom:24px">
                ${qe({...L,_evolution_bonus:L._evolution_bonus||0},{width:90,showStad:!0,stadDef:p,context:"formation"})}
        ...
        <div style="width:130px;flex-shrink:0;background:rgba(0,0,0,0.3);display:flex;flex-direction:column;align-items:center;padding:12px 8px;gap:12px;border-left:1px solid rgba(255,255,255,0.1)">
          ...
          <div style="width:100%;text-align:center">
            ...
            <div id="add-stad-btn-pc" style="cursor:pointer;margin:0 auto;width:fit-content">
              ${s?(()=>{var u;const g=e.stadDefMap[s.stadium_id],L=((u=g==null?void 0:g.club)==null?void 0:u.logo_url)||(g==null?void 0:g.image_url)||(g!=null&&g.country_code?`https://flagsapi.com/${g.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null);return Kt((g==null?void 0:g.name)||"Stade",L,"+10 ⭐",{width:100})})():`<div style="width:100px;height:171px;border:2px dashed rgba(79,195,247,0.4);border-radius:8px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px">
        ...
        <div style="display:flex;gap:6px;align-items:flex-start">
          ...
          <div style="flex:1;min-width:0">
            ...
            <div style="display:flex;gap:2px;align-items:center;flex-wrap:nowrap;overflow-x:auto;scrollbar-width:none;-ms-overflow-style:none" id="subs-list" class="no-scrollbar">
              ${c.map(g=>{const L={...g.player,_evolution_bonus:g.evolution_bonus||0};return`<div style="position:relative;flex-shrink:0;overflow:visible;padding-bottom:20px">
                  ${qe({...L,_evolution_bonus:L._evolution_bonus||0},{width:44,showStad:!0,stadDef:p,context:"formation"})}
                  <button data-remove-sub="${g.id}"
          ...
          <div style="flex-shrink:0;text-align:center">
            ...
            <div id="add-stad-btn" style="cursor:pointer">
              ${s?(()=>{var u;const g=e.stadDefMap[s.stadium_id],L=((u=g==null?void 0:g.club)==null?void 0:u.logo_url)||(g==null?void 0:g.image_url)||(g!=null&&g.country_code?`https://flagsapi.com/${g.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null);return Kt((g==null?void 0:g.name)||"Stade",L,"+10⭐",{width:44})})():`<div style="width:44px;height:57px;border:2px dashed rgba(79,195,247,0.5);border-radius:6px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:2px">
    ...
    <div class="page-body" style="padding:12px 16px calc(80px + env(safe-area-inset-bottom, 0px))">
      ...
      <div style="font-size:16px;font-weight:800">✨ Deck en cours de préparation…</div>`,document.body.appendChild(g),()=>g.remove()}t.querySelectorAll("#auto-deck-pc, #auto-deck-mobile").forEach(g=>{g.addEventListener("click",async()=>{if(e.playerCards.length<11){i.toast(`Il faut au moins 11 joueurs (tu en as ${e.playerCards.length})`,"error");return}g.disabled=!0;const L=h();await new Promise(j=>setTimeout(j,30));const u=sa({playerCards:e.playerCards,availableFormations:e.availableFormations,stadiumCards:e.stadiumCards,stadDefMap:e.stadDefMap});if(g.disabled=!1,L(),!u){i.toast("Impossible de composer une équipe complète","error");return}e.formation=u.formation,e.slots=u.slots,e.wanted={},e.subs=u.subs,e.stadiumCardId=u.stadiumCardId,ft(t,e,i),i.toast(`✨ Deck optimisé : ${u.formation} · score ${u.score}`,"success")})}),t.querySelectorAll("[data-remove-sub]").forEach(g=>{g.addEventListener("click",()=>{e.subs=e.subs.filter(L=>L!==g.dataset.removeSub),ft(t,e,i)})}),t.querySelectorAll("#add-sub-btn").forEach(g=>g.addEventListener("click",()=>ua(e,t,i)))}function da(t,e,i,o){var L,u,j,T,F,Y;const n=window.innerWidth>=900,r=t.querySelector(n?"#deck-field-pc":"#deck-field-mobile");if(!r)return;const a=(L=e.stadiumCards)==null?void 0:L.find(X=>X.id===e.stadiumCardId),d=a&&((u=e.stadDefMap)==null?void 0:u[a.stadium_id])||null,l=vt[e.formation]||{},s=Hi(e.formation)||[],p={},c=new Set;for(const X of i){const M=e.slots[X],w=M?e.playerCards.find(b=>b.id===M):null;w!=null&&w.player?p[X]={...w.player,_evolution_bonus:w.evolution_bonus||0,face:w.player.face||null}:(j=e.wanted)!=null&&j[X]&&((T=e._playersCache)!=null&&T[e.wanted[X]])?(p[X]={...e._playersCache[e.wanted[X]],_evolution_bonus:0},c.add(X)):p[X]=null}const y=window.innerWidth>=900,x=y?window.innerWidth-280:window.innerWidth-20,m=y?Math.min(x,860):x,f=Math.round(y?m*.82:m*.85),h=y?84:Jn(e.formation,m,f);let _="";for(const[X,M]of s){const w=l[X],b=l[M];if(!w||!b)continue;const E=w.x*m,H=Math.round(.03*f+w.y*.85*f),S=b.x*m,G=Math.round(.03*f+b.y*.85*f),de=p[X],I=p[M],z=Gt(de,I);z==="#ff3333"||z==="#cc2222"?_+=`<line x1="${E.toFixed(1)}" y1="${H.toFixed(1)}" x2="${S.toFixed(1)}" y2="${G.toFixed(1)}" stroke="${z}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(_+=`<line x1="${E.toFixed(1)}" y1="${H.toFixed(1)}" x2="${S.toFixed(1)}" y2="${G.toFixed(1)}" stroke="${z}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,_+=`<line x1="${E.toFixed(1)}" y1="${H.toFixed(1)}" x2="${S.toFixed(1)}" y2="${G.toFixed(1)}" stroke="${z}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let $="";const g=y?Math.round(h*574/372):h;for(const X of i){const M=l[X];if(!M)continue;const w=p[X],b=M.x*m;M.y*f;const E=Math.round(.03*f+M.y*(.85*f)),H=Math.round(b-h/2),S=Math.round(E-g/2);if(w){const G=X.replace(/\d+/,""),de=c.has(X),I=!de&&d&&(d.club_id&&String(w.club_id)===String(d.club_id)||d.country_code&&w.country_code===d.country_code),z=qe({...w,_evolution_bonus:w._evolution_bonus||0},{width:h,showStad:!0,stadDef:d,role:G,compactSquare:!y,context:"formation"});I&&((F=d.club)!=null&&F.logo_url||d.image_url);const Z="",U=de?`<button class="wanted-market-btn" data-wanted-pos="${X}"
        ...
        <div style="position:relative;${de?"filter:grayscale(1) brightness(.65)":""}">${z}${Z}</div>
        ${U}
    ...
    <div style="position:relative;width:${m}px;height:${f}px;margin:0 auto">
      ...
      ${a.map(l=>{var x;const s=t.stadDefMap[l.stadium_id],p=((x=s==null?void 0:s.club)==null?void 0:x.logo_url)||(s==null?void 0:s.image_url)||(s!=null&&s.country_code?`https://flagsapi.com/${s.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),c=t.stadiumCardId===l.id,y=Kt((s==null?void 0:s.name)||"Stade",p,"+10⭐",{width:90});return`<div class="stad-choice" data-stad-id="${l.id}" style="cursor:pointer;position:relative;border-radius:8px;${c?"box-shadow:0 0 0 3px #4fc3f7":""}">
          ${y}
        </div>`}).join("")}
    </div>`),(d=document.getElementById("stad-none"))==null||d.addEventListener("click",()=>{t.stadiumCardId=null,n(),ft(e,t,i)}),document.querySelectorAll(".stad-choice").forEach(l=>{l.addEventListener("click",()=>{t.stadiumCardId=l.dataset.stadId,n(),ft(e,t,i)})})}async function pa(t,e,i,o){var j,T,F,Y,X;const{openModal:n,closeModal:r,navigate:a}=o,d=t.replace(/\d+/,""),l=(j=e.stadiumCards)==null?void 0:j.find(M=>M.id===e.stadiumCardId),s=l&&((T=e.stadDefMap)==null?void 0:T[l.stadium_id])||null,p=e.slots[t],c=p?e.playerCards.find(M=>M.id===p):null;(F=c==null?void 0:c.player)==null||F.id;const y=new Set;Object.entries(e.slots).forEach(([M,w])=>{var E;if(M===t||!w)return;const b=e.playerCards.find(H=>H.id===w);(E=b==null?void 0:b.player)!=null&&E.id&&y.add(b.player.id)}),e.subs.forEach(M=>{var b;const w=e.playerCards.find(E=>E.id===M);(b=w==null?void 0:w.player)!=null&&b.id&&y.add(w.player.id)}),Object.entries(e.wanted||{}).forEach(([M,w])=>{M===t||!w||y.add(w)});const x=new Set,m=e.playerCards.filter(M=>{const w=M.player;return!(w.job===d||w.job2===d)||y.has(w.id)||x.has(w.id)?!1:(x.add(w.id),!0)}),h=(Hi(e.formation)||zi[e.formation]||[]).filter(([M,w])=>M===t||w===t).map(([M,w])=>M===t?w:M).map(M=>{var w;return(w=e.playerCards.find(b=>b.id===e.slots[M]))==null?void 0:w.player}).filter(Boolean);function _(M){const w=M.player,b=M.evolution_bonus||0,E=(d==="GK"?w.note_g:d==="DEF"?w.note_d:d==="MIL"?w.note_m:w.note_a)+(d===w.job||d===w.job2?b:0);let H=0;for(const S of h){const G=Gt(w,S);G==="#00ff88"?H+=10:G==="#FFD700"&&(H+=5)}if(s){const S=s.club_id&&String(w.club_id)===String(s.club_id),G=s.country_code&&String(w.country_code)===String(s.country_code);(S||G)&&(H+=10)}return{total:E+H,note:E,bonus:H}}const $=new Map;if(m.forEach(M=>$.set(M.id,_(M))),m.sort((M,w)=>{const b=$.get(w.id).total-$.get(M.id).total;return b!==0?b:$.get(w.id).note-$.get(M.id).note}),!e._allPlayers){const{data:M}=await v.from("players").select("id, firstname, surname_real, country_code, club_id, job, job2, note_g, note_d, note_m, note_a, rarity, face, clubs(encoded_name, logo_url)").eq("is_active",!0);e._allPlayers=M||[],e._playersCache||(e._playersCache={}),e._allPlayers.forEach(w=>{e._playersCache[w.id]=w})}const g=new Set(e.playerCards.map(M=>{var w;return(w=M.player)==null?void 0:w.id}).filter(Boolean)),L=e._allPlayers.filter(M=>(M.job===d||M.job2===d)&&!y.has(M.id)).map(M=>{const w=(d==="GK"?M.note_g:d==="DEF"?M.note_d:d==="MIL"?M.note_m:M.note_a)||0;let b=0;for(const E of h){const H=Gt(M,E);H==="#00ff88"?b+=10:H==="#FFD700"&&(b+=5)}if(s){const E=s.club_id&&String(M.club_id)===String(s.club_id),H=s.country_code&&String(M.country_code)===String(s.country_code);(E||H)&&(b+=10)}return{p:M,total:w+b,note:w,bonus:b,owned:g.has(M.id)}}).sort((M,w)=>w.total-M.total||w.note-M.note).slice(0,10),u=L.length?'<div style="display:flex;flex-wrap:wrap;gap:8px;justify-content:center">'+L.map(M=>{const w={...M.p,_evolution_bonus:0},b=M.bonus>0?`<div style="position:absolute;top:2px;left:2px;z-index:6;background:#1A6B3C;color:#fff;font-size:9px;font-weight:900;padding:1px 5px;border-radius:8px">+${M.bonus}</div>`:"",E=M.owned?e.playerCards.find(S=>{var G;return((G=S.player)==null?void 0:G.id)===M.p.id}):null,H=M.owned?"":'title="Ajouter au deck (non possédé, à acheter)"';return`<div style="position:relative">
      ${b}
    ...
    </div>`,'<button class="btn btn-ghost" id="close-sub-selector">Fermer</button>'),(y=document.getElementById("close-sub-selector"))==null||y.addEventListener("click",n),document.querySelectorAll(".player-option").forEach(x=>{x.addEventListener("click",()=>{t.subs.push(x.dataset.cardId),n(),ft(e,t,i)})})}async function Pn(t){const e=t.formationCards.find(n=>n.formation===t.formation),i=(e==null?void 0:e.id)||t.formationCardId;await v.from("decks").update({formation:t.formation,formation_card_id:i||null,stadium_card_id:t.stadiumCardId||null}).eq("id",t.deckId),await v.from("deck_cards").delete().eq("deck_id",t.deckId);const o=[];return Object.entries(t.slots).forEach(([n,r],a)=>{o.push({deck_id:t.deckId,card_id:r,position:n,is_starter:!0,slot_order:a})}),Object.entries(t.wanted||{}).forEach(([n,r],a)=>{o.push({deck_id:t.deckId,card_id:null,wanted_player_id:r,position:n,is_starter:!0,slot_order:1e3+a})}),t.subs.forEach((n,r)=>{o.push({deck_id:t.deckId,card_id:n,position:`SUB${r+1}`,is_starter:!1,slot_order:100+r})}),o.length>0?await v.from("deck_cards").insert(o):{error:null}}let co=null;function fa(t,e){const{toast:i}=e;clearTimeout(co),document.querySelectorAll(".autosave-indicator").forEach(n=>{n.textContent="⏳ Enregistrement...",n.style.opacity="1",n.style.color=""}),co=setTimeout(async()=>{const{error:n}=await Pn(t),r=document.querySelectorAll(".autosave-indicator");if(n){r.forEach(a=>{a.textContent="⚠️ Erreur d'enregistrement",a.style.color="#ff6b6b"}),i(n.message,"error");return}r.forEach(a=>{a.textContent="✅ Enregistré",a.style.color="",setTimeout(()=>{a.style.opacity="0"},1500)})},600)}async function ma(t,e){const{toast:i,navigate:o}=e;clearTimeout(co);const{error:n}=await Pn(t);if(n){i(n.message,"error");return}i("Deck enregistré ✅","success"),o("decks")}function Xo(t){const e=Ni[t]||Ni["4-4-2"],i=["GK1"];for(let o=1;o<=e.DEF;o++)i.push(`DEF${o}`);for(let o=1;o<=e.MIL;o++)i.push(`MIL${o}`);for(let o=1;o<=e.ATT;o++)i.push(`ATT${o}`);return i}async function ga(t,e){const{state:i}=e,o=i.params||{},n=o.matchMode||"vs_ai_easy",r=n==="solo",a=n==="ranked_ai",d=o.soloLevel||1,l=n.replace("vs_ai_",""),s=n;let p=null;if(r){const{data:m}=await v.from("solo_levels").select("*").eq("level_number",d).maybeSingle();p=m||{level_number:d,target_note_avg:10,nb_liens_jaune:2,nb_liens_vert:1,nb_joueurs_stade:2,reward_credits:500}}const c=o.rankedData||null,y=o.presetSetup||null;async function x({deckId:m,formation:f,starters:h,subsRaw:_,gcCardsEnriched:$,gcDefs:g,stadiumDef:L}){try{let u=no(h,f);L&&(u=Di(u,L),Fi(_,L));let j=null;if(a){const M=xa(u)+ya(u)*10,{jaune:w,vert:b}=va(u,f),E=[.9,1,1.2],H=E[Math.floor(Math.random()*E.length)],S=Math.max(11,Math.round(M*H));j={target_note_avg:Math.max(1,Math.round(S/11)),target_total_note:S,nb_liens_jaune:w,nb_liens_vert:b,nb_joueurs_stade:L?3:0,_variance:H}}const T=p&&{...p,target_total_note:Math.max(11,Math.round((Number(p.target_note_avg)||10)*11))},F=r?await Zo(f,T):a?await Zo(f,j):await ba(f,l),Y=F.lines||F,X=async M=>{try{const w=s==="vs_ai_club"||s==="solo"||s==="ranked_ai"?"club":s,{data:b,error:E}=await v.from("matches").insert({home_id:i.profile.id,away_id:null,mode:w,home_deck_id:m,status:"in_progress"}).select().single();if(E){console.error("[MatchIA] Erreur création match:",E),Xi(t,"⚠️","Impossible de créer le match ("+E.message+").","Retour",()=>e.navigate("home"));return}const H=F.stadiumDef||null;H&&Y&&(Di(Y,H),Fi(F.subs||[],H));const S={gcDefs:g||[],matchId:b==null?void 0:b.id,mode:s,difficulty:l,formation:f,isSolo:r,soloLevel:d,soloLevelConfig:p,isRankedAI:a,rankedAIData:c,homeTeam:u,aiTeam:Y,homeSubs:_,subsUsed:0,maxSubs:Math.min(_.length,3),aiSubs:F.subs||[],aiSubsUsed:0,aiMaxSubs:Math.min((F.subs||[]).length,3),aiUsedSubIds:[],aiGcCards:F.gcCards||[],aiUsedGc:[],aiGcTarget:F.gcTarget||2,aiStadiumDef:H,homeScore:0,aiScore:0,gcCards:M,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};ha(t,S,e)}catch(w){console.error("[MatchIA] Exception launchMatch:",w),Xi(t,"⚠️","Erreur au lancement du match : "+w.message,"Retour",()=>e.navigate("home"))}};if(a&&y){X(y.gcCardsEnriched||[]);return}if(!$.length){X([]);return}Ki(t,$,X)}catch(u){console.error("[MatchIA] Exception setup:",u),Xi(t,"⚠️","Erreur de préparation du match : "+u.message,"Retour",()=>e.navigate("home"))}}y?await x(y):await Ui(t,e,n,x)}async function ba(t,e){var h;const{data:i}=await v.from("players").select("id,firstname,surname_real,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(80);if(!i||i.length<11)return{lines:Rn(t),subs:[],gcCards:[],stadiumDef:null};const o=ni[t]||ni["4-4-2"],n={GK:[],DEF:[],MIL:[],ATT:[]},r=new Set;function a(_,$,g){var L,u;return r.add(_.id),{cardId:"ai-"+_.id+"-"+g,id:_.id,firstname:_.firstname,name:_.surname_real,country_code:_.country_code,club_id:_.club_id,job:_.job,job2:_.job2,note_g:Number(_.note_g)||0,note_d:Number(_.note_d)||0,note_m:Number(_.note_m)||0,note_a:Number(_.note_a)||0,rarity:_.rarity,skin:_.skin,hair:_.hair,hair_length:_.hair_length,face:_.face,clubName:((L=_.clubs)==null?void 0:L.encoded_name)||null,clubLogo:((u=_.clubs)==null?void 0:u.logo_url)||null,boost:0,used:!1,_line:$}}for(const _ of["GK","DEF","MIL","ATT"]){const $=i.filter(T=>T.job===_&&!r.has(T.id)),g=i.filter(T=>T.job!==_&&!r.has(T.id)),L=[...$,...g],u=[];for(let T=0;T<o[_];T++){const F=L[T];F&&u.push(a(F,_,T))}const j=Ft(u.length);u.forEach((T,F)=>{T._col=j[F]}),n[_]=u}const l=i.filter(_=>!r.has(_.id)).slice(0,5).map((_,$)=>a(_,_.job,100+$)),{data:s}=await v.from("gc_definitions").select("*").eq("is_active",!0),c=[...s||[]].sort(()=>Math.random()-.5).slice(0,3).map((_,$)=>({id:"ai-gc-"+$,gc_type:_.name,name:_.name,icon:"⚡",effect_type:_.effect_type,effect_params:_.effect_params||{}})),y=Object.values(n).flat(),x={};y.forEach(_=>{_.club_id&&(x[_.club_id]=(x[_.club_id]||0)+1)});const m=(h=Object.entries(x).sort((_,$)=>$[1]-_[1])[0])==null?void 0:h[0];let f=null;if(m){const{data:_}=await v.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",m).single();_&&(f={club_id:_.id,country_code:null,name:_.encoded_name+" Stadium",club:{encoded_name:_.encoded_name,logo_url:_.logo_url}})}return{lines:n,subs:l,gcCards:c,stadiumDef:f}}function xa(t){let e=0;for(const i of["GK","DEF","MIL","ATT"])(t[i]||[]).forEach(o=>{const n=o._line||i,r=Number(n==="GK"?o.note_g:n==="DEF"?o.note_d:n==="MIL"?o.note_m:o.note_a)||0;e+=r+(o.boost||0)});return e}function ya(t){let e=0;for(const i of["GK","DEF","MIL","ATT"])(t[i]||[]).forEach(o=>{o!=null&&o.stadiumBonus&&e++});return e}function va(t,e){const i={};for(const a of["GK","DEF","MIL","ATT"])(t[a]||[]).forEach((d,l)=>{i[`${a}${l+1}`]=d});const o=Hi(e)||zi[e]||[];let n=0,r=0;for(const[a,d]of o){const l=i[a],s=i[d];if(!l||!s)continue;const p=Gt(l,s);p==="#00ff88"?r++:p==="#FFD700"&&n++}return{jaune:n,vert:r}}function Qo(t){const e=t.job||"ATT";return Number(e==="GK"?t.note_g:e==="DEF"?t.note_d:e==="MIL"?t.note_m:t.note_a)||0}function gi(t,e,i){if(i<=0||!t.length)return[];const o=[...t].sort((n,r)=>Math.abs(Qo(n)-e)-Math.abs(Qo(r)-e));return o.slice(0,Math.min(i,o.length))}async function Zo(t,e){var E,H;const i=Number(e==null?void 0:e.target_note_avg)||10,o=Number(e==null?void 0:e.nb_liens_jaune)||0,n=Number(e==null?void 0:e.nb_liens_vert)||0,r=Number(e==null?void 0:e.nb_joueurs_stade)||0,{data:a}=await v.from("players").select("id,firstname,surname_real,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,clubs(encoded_name,logo_url,country_code)").eq("is_active",!0).limit(300);if(!a||a.length<16)return{lines:Rn(t),subs:[],gcCards:[],stadiumDef:null};const d={};a.forEach(S=>{S.club_id&&(d[S.club_id]=(d[S.club_id]||0)+1)});const l=((E=Object.entries(d).sort((S,G)=>G[1]-S[1])[0])==null?void 0:E[0])||null,s=l?a.filter(S=>S.club_id===l):[],p=((H=s[0])==null?void 0:H.country_code)||null,c=p?a.filter(S=>S.country_code===p&&S.club_id!==l):[],y=Math.min(s.length,Math.max(2,n+1),16),x=Math.min(c.length,Math.max(0,o),16-y),m=[],f=new Set;gi(s,i,y).forEach(S=>{m.push(S),f.add(S.id)}),gi(c,i,x).forEach(S=>{m.push(S),f.add(S.id)});const h=Math.max(0,16-m.length),_=a.filter(S=>!f.has(S.id));gi(_,i,h).forEach(S=>{m.push(S),f.add(S.id)});const $=ni[t]||ni["4-4-2"],g={GK:[],DEF:[],MIL:[],ATT:[]},L=new Set;function u(S,G,de){var I,z;return L.add(S.id),{cardId:"ai-"+S.id+"-"+de,id:S.id,firstname:S.firstname,name:S.surname_real,country_code:S.country_code,club_id:S.club_id,job:S.job,job2:S.job2,note_g:Number(S.note_g)||0,note_d:Number(S.note_d)||0,note_m:Number(S.note_m)||0,note_a:Number(S.note_a)||0,rarity:S.rarity,skin:S.skin,hair:S.hair,hair_length:S.hair_length,face:S.face,clubName:((I=S.clubs)==null?void 0:I.encoded_name)||null,clubLogo:((z=S.clubs)==null?void 0:z.logo_url)||null,boost:0,used:!1,_line:G}}for(const S of["GK","DEF","MIL","ATT"]){let G=m.filter(U=>U.job===S&&!L.has(U.id));if(S==="GK"&&!G.length){const U=a.filter(te=>te.job==="GK"&&!L.has(te.id));U.length&&(G=gi(U,i,1))}const de=m.filter(U=>U.job!==S&&!L.has(U.id)),I=[...G,...de],z=[];for(let U=0;U<$[S];U++){const te=I[U];te&&z.push(u(te,S,U))}const Z=Ft(z.length);z.forEach((U,te)=>{U._col=Z[te]}),g[S]=z}let j=m.filter(S=>!L.has(S.id));if(j.length<5){const S=a.filter(G=>!L.has(G.id)&&!j.some(de=>de.id===G.id));j=j.concat(gi(S,i,5-j.length))}const T=j.slice(0,5).map((S,G)=>u(S,S.job,100+G)),F=Number(e==null?void 0:e.target_total_note)||0;if(F>0){const S={GK:"note_g",DEF:"note_d",MIL:"note_m",ATT:"note_a"},G=[];for(const I of["GK","DEF","MIL","ATT"])(g[I]||[]).forEach(z=>G.push({p:z,key:S[I]}));const de=G.reduce((I,z)=>I+(Number(z.p[z.key])||0),0);if(de>0&&G.length){const I=F/de;G.forEach(U=>{U.p[U.key]=Math.max(1,Math.round((Number(U.p[U.key])||0)*I))});let z=F-G.reduce((U,te)=>U+te.p[te.key],0),Z=0;for(;z!==0&&Z++<500;){const U=G[Math.floor(Math.random()*G.length)];z>0?(U.p[U.key]++,z--):U.p[U.key]>1&&(U.p[U.key]--,z++)}T.forEach(U=>{const te=S[U.job]||"note_m";U[te]=Math.max(1,Math.round((Number(U[te])||0)*I))})}}const{data:Y}=await v.from("gc_definitions").select("*").eq("is_active",!0),M=[...Y||[]].sort(()=>Math.random()-.5).slice(0,3).map((S,G)=>({id:"ai-gc-"+G,gc_type:S.name,name:S.name,icon:"⚡",effect_type:S.effect_type,effect_params:S.effect_params||{}}));let w=null;if(r>0&&l){const{data:S}=await v.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",l).single();S&&(w={club_id:S.id,country_code:null,name:S.encoded_name+" Stadium",club:{encoded_name:S.encoded_name,logo_url:S.logo_url}})}const b=1+Math.floor(Math.random()*3);return{lines:g,subs:T,gcCards:M,stadiumDef:w,gcTarget:b}}function Rn(t){const e=ni[t]||ni["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},o=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let n=0;for(const r of["GK","DEF","MIL","ATT"]){const a=[];for(let l=0;l<e[r];l++){const s=3+Math.floor(Math.random()*5);a.push({cardId:"fake-"+n,id:"fake-"+n,firstname:"IA",name:o[n%o.length],country_code:"XX",club_id:null,job:r,job2:null,note_g:r==="GK"?s:2,note_d:r==="DEF"?s:2,note_m:r==="MIL"?s:2,note_a:r==="ATT"?s:2,rarity:"normal",boost:0,used:!1,_line:r}),n++}const d=Ft(a.length);a.forEach((l,s)=>{l._col=d[s]}),i[r]=a}return i}function ha(t,e,i){var n;mn("/sounds/match-opening.mp3",.3);const o=`<div style="position:relative;width:22px;height:22px;flex-shrink:0;display:flex;align-items:center;justify-content:center">
  ...
  <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:clamp(6px,1.5vh,20px);padding:clamp(8px,1.5vw,24px);background:#0a3d1e">
    ...
    <div id="duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center, rgba(10,61,30,0.4), rgba(10,61,30,0.92))"></div>
          ...
          <button id="start-match-btn" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,0.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">
            ▶ Commencer le match
          </button>`,L.style.transition="opacity .45s ease",L.style.opacity="1",L.style.pointerEvents="auto",(u=document.getElementById("start-match-btn"))==null||u.addEventListener("click",x))},600)},700)},2800)}function Ce(t,e,i){var X,M,w,b,E,H,S,G,de;const o=e.selected.map(I=>I.cardId),n=e.usedSubIds||[],r=e.homeSubs.filter(I=>!n.includes(I.cardId)),d=Object.values(e.homeTeam).flat().filter(I=>I.used).length>0&&r.length>0&&e.subsUsed<e.maxSubs,l=[...e.homeTeam.MIL||[],...e.homeTeam.ATT||[]].filter(I=>!I.used),s=[...e.aiTeam.MIL||[],...e.aiTeam.ATT||[]].filter(I=>!I.used),p=ai(e.aiTeam),c=l.length===0&&s.length===0&&!p,y=(e.homeTeam.DEF||[]).filter(I=>!I.used),x=(e.homeTeam.GK||[]).filter(I=>!I.used);let m=[];e.phase==="attack"&&l.length===0&&(p?(m=y.map(I=>I.cardId),y.length===0&&(m=m.concat(x.map(I=>I.cardId)))):c&&(m=[...y,...x].map(I=>I.cardId))),e.log[e.log.length-1];const f=e.phase==="ai-attack"||e.phase==="ai-defense",h=e.phase==="attack",_=e.phase==="defense",$=e.phase==="finished",L=(e.homeSubs||[]).filter(I=>!(e.usedSubIds||[]).includes(I.cardId)).length>0&&e.subsUsed<e.maxSubs,u=h&&l.length===0&&m.length===0&&!L,j=e.gcCards.filter(I=>!e.usedGc.includes(I.id)),T=e.boostCard&&!e.boostUsed;t.style.overflow="hidden",t.style.height="100%",t.style.display="flex",t.style.flexDirection="column",t.innerHTML=`
  <style>
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">
      ${(()=>{if(e.phase==="defense"&&e.pendingAttack){const z=e.pendingAttack;let Z="";if(e.selected.length>0){const U=e.selected.map(pe=>({...(e.homeTeam[pe._role]||[]).find(fe=>fe.cardId===pe.cardId)||pe,_line:pe._role})),te=ki(U,e.modifiers.home,e.formation);Z=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
            ...
            <div style="display:flex;justify-content:center">${lt((z.players||[]).map(U=>({...U,used:!1})),"#ff6b6b",z.total,"attack",e.formation)}</div>
            ${Z}
          </div>`}if(e.phase==="ai-defense"&&e.pendingAttack){const z=e.pendingAttack;return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #00ff88;text-align:center">
            <div style="font-size:8px;color:#00ff88;letter-spacing:2px;margin-bottom:4px;text-transform:uppercase">⚔️ VOUS ATTAQUEZ</div>
            <div style="display:flex;justify-content:center">${lt((z.players||[]).map(Z=>({...Z,used:!1})),"#00ff88",z.total,"attack",e.formation)}</div>
          </div>`}if(e.phase==="attack"&&e.selected.length>0){const z=e.selected.map(U=>{const te=(e.homeTeam[U._role]||[]).find(he=>he.cardId===U.cardId)||U,pe=["GK","DEF"].includes(U._role);return{...te,_line:U._role,...pe?{note_a:Math.max(1,Number(te.note_a)||0)}:{}}}),Z=$i(z,e.modifiers.home,e.formation);return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #FFD700;text-align:center">
            ...
            <div style="display:flex;justify-content:center">${lt(z.map(U=>({...U,used:!1})),"#FFD700",Z.total,"attack",e.formation)}</div>
    ...
    ${(()=>{const I=window.innerWidth>=700,z=(_e,ye,Ee)=>{var P,ee;const D=(e.gcDefs||[]).find(me=>me.name===_e.gc_type),C=(D==null?void 0:D.name)||_e.gc_type,K=(D==null?void 0:D.effect)||((P=Qe[_e.gc_type])==null?void 0:P.desc)||"",W=D!=null&&D.image_url?`/icons/${D.image_url}`:null,J=((ee=Qe[_e.gc_type])==null?void 0:ee.icon)||"⚡";return`<div class="gc-mini" data-gc-id="${_e.id}" data-gc-type="${_e.gc_type}" style="flex-shrink:0;cursor:pointer">
          ${Ze(C,W,J,K,{width:ye})}
        </div>`},Z=(_e,ye)=>{var Ee;return`<div id="boost-card"
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${e.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...e.log].reverse().map(I=>{var z,Z,U;if(I.type==="duel"){const te=I.isGoal,pe=I.homeScored?"#FFD700":te?"#ff6b6b":"rgba(255,255,255,0.3)",he=I.homeScored?"⚽ BUT !":te?"⚽ BUT IA !":(z=I.homePlayers)!=null&&z.length?"⚔️ Attaque":"🛡️ Défense",fe=(I.homeTotal??0)>=(I.aiTotal??0);return`<div style="padding:8px;border-radius:8px;background:${te?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${pe};margin-bottom:4px">
                ...
                <div style="display:flex;align-items:center;justify-content:center;gap:10px">
                  ${(Z=I.aiPlayers)!=null&&Z.length?`<div style="flex:1;display:flex;justify-content:flex-end;opacity:0.75">${lt(I.aiPlayers,"#ff6b6b",void 0,void 0,e.formation)}</div>`:'<div style="flex:1"></div>'}
    ...
    </div>`,(u=c.querySelector("#sub-close"))==null||u.addEventListener("click",()=>c.remove()),(j=c.querySelector("#out-up"))==null||j.addEventListener("click",()=>{d>0&&(d--,y())}),(T=c.querySelector("#out-down"))==null||T.addEventListener("click",()=>{d<r.length-1&&(d++,y())}),(F=c.querySelector("#in-up"))==null||F.addEventListener("click",()=>{s>0&&(s--,y())}),(Y=c.querySelector("#in-down"))==null||Y.addEventListener("click",()=>{s<a.length-1&&(s++,y())});const L=(M,w,b,E)=>{const H=c.querySelector("#"+M);if(!H)return;let S=0;H.addEventListener("touchstart",G=>{S=G.touches[0].clientY},{passive:!0}),H.addEventListener("touchend",G=>{const de=G.changedTouches[0].clientY-S;if(Math.abs(de)<30)return;const I=w();de<0&&I<E-1?(b(I+1),y()):de>0&&I>0&&(b(I-1),y())},{passive:!0})};L("in-panel",()=>s,M=>s=M,a.length),L("out-panel",()=>d,M=>d=M,r.length),(X=c.querySelector("#sub-confirm"))==null||X.addEventListener("click",M=>{if(M.preventDefault(),M.stopPropagation(),p)return;p=!0;const w=r[d],b=a[s];if(!w||!b)return;let E=null,H=-1;for(const[G,de]of Object.entries(e.homeTeam)){const I=(de||[]).findIndex(z=>z.cardId===w.cardId);if(I!==-1){E=G,H=I;break}}if(H===-1||!E){Bt("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),c.remove();return}const S={...b,_line:E,_col:w._col||0,used:!1,boost:0};e.homeTeam[E].splice(H,1,S),e.usedSubIds||(e.usedSubIds=[]),e.usedSubIds.push(b.cardId),e.subsUsed++,e.selected=[],e.log.push({type:"sub",subSide:"home",clubName:e.clubName,outPlayer:{name:w.name,firstname:w.firstname,note:Ht(w,E),portrait:Ut(w),job:w.job,country_code:w.country_code,rarity:w.rarity,clubName:w.clubName,clubLogo:w.clubLogo},inPlayer:{name:b.name,firstname:b.firstname,note:Ht(b,E),portrait:Ut(b),job:b.job,country_code:b.country_code,rarity:b.rarity,clubName:b.clubName,clubLogo:b.clubLogo},text:`🔄 ${b.firstname} ${b.name} remplace ${w.firstname} ${w.name}`}),c.remove(),wn(w,b,()=>Ce(t,e,i))})}document.body.appendChild(c),y()}function Ia(t,e,i,o,n){var y,x;const r=(o.gcDefs||[]).find(m=>m.name===e),a=Qe[e]||{icon:"⚡",desc:"Carte spéciale."},d=(r==null?void 0:r.name)||e,l=(r==null?void 0:r.effect)||a.desc,s=r!=null&&r.image_url?`/icons/${r.image_url}`:null,p=a.icon||"⚡",c=document.createElement("div");c.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",c.innerHTML=`
    ${Ze(d,s,p,l,{width:190})}
    <!-- Boutons -->
    <div style="display:flex;gap:12px;width:190px">
      <button id="gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
      <button id="gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
    </div>`,document.body.appendChild(c),(y=c.querySelector("#gc-back"))==null||y.addEventListener("click",()=>c.remove()),(x=c.querySelector("#gc-use"))==null||x.addEventListener("click",()=>{c.remove(),Ca(t,e,i,o,n)})}function Mi(t,e,i,o,n,r){const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let d=[];function l(){var s,p;a.innerHTML=`
    ...
    <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start;justify-content:center">
      ${t.map(c=>{const y=c._line||c.job||"MIL",x=d.find(f=>f.cardId===c.cardId),m=qe({...c,_evolution_bonus:0},{width:90,showStad:!0,role:y,extraNote:c.boost||0,context:"match"});return`<div class="gc-pick-item" data-cid="${c.cardId}"
    ...
    <div style="display:flex;flex-direction:column;gap:6px">
      ${o.map(n=>`
        <div class="player-boost-opt" data-card-id="${n.cardId}"
          ...
          <div style="width:32px;height:32px;background:${Qn[n.job]||"#888"};border-radius:6px;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:900;font-size:13px">${Ht(n,n._line||n.job)}</div>
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(n=>{n.addEventListener("click",()=>{const r=n.dataset.cardId;for(const a of["GK","DEF","MIL","ATT"]){const d=(e.homeTeam[a]||[]).find(l=>l.cardId===r);if(d){d.boost=(d.boost||0)+e.boostCard.value,e.log.push({text:`⚡ Boost +${e.boostCard.value} appliqué à ${d.name}`,type:"info"});break}}e.boostUsed=!0,i.closeModal(),Ce(t,e,i)})})}async function Mt(t,e,i){var y,x,m,f;ti(),it(),e._timerInt&&(clearInterval(e._timerInt),e._timerInt=null),e.phase="finished";const{state:o}=i,n=e.homeScore>e.aiScore,r=e.homeScore===e.aiScore,a=n?"victoire":r?"nul":"defaite",d=e.isSolo&&((y=e.soloLevelConfig)!=null&&y.reward_credits)&&n?Number(e.soloLevelConfig.reward_credits):Xn(e.mode,a);if(e.isSolo&&n)try{const{data:h}=await v.from("user_solo_progress").select("unlocked_level").eq("user_id",o.profile.id).maybeSingle(),_=(h==null?void 0:h.unlocked_level)||1;e.soloLevel>=_&&await v.from("user_solo_progress").upsert({user_id:o.profile.id,unlocked_level:e.soloLevel+1,updated_at:new Date().toISOString()})}catch(h){console.warn("[Solo] Erreur mise à jour progression:",h.message)}const l="00000000-0000-0000-0000-000000000000";let s=null;if(e.isRankedAI)try{const{data:h}=await v.from("users").select("mmr, mmr_deviation, mmr_volatility, placement_matches").eq("id",o.profile.id).single();if(h){const _=h.mmr??450,$=h.mmr_deviation??350,g=h.mmr_volatility??.06,L=n?1:r?.5:0,u=(h.placement_matches??0)<10,j=pn(_,$,g,_,350,L,u),T=r?null:n?o.profile.id:l;await v.rpc("update_mmr_after_ranked",{p_match_id:e.matchId,p_winner_id:T,p_home_id:o.profile.id,p_away_id:l,p_home_delta:j.delta,p_away_delta:0,p_home_new_rd:j.newRd,p_away_new_rd:350,p_home_new_vol:j.newSigma,p_away_new_vol:.06}),s=j.delta}}catch(h){console.warn("[RankedAI] Erreur mise à jour MMR:",h.message)}e.matchId&&await v.from("matches").update({status:"finished",home_score:e.homeScore,away_score:e.aiScore,winner_id:n?o.profile.id:null,home_credits_reward:d,played_at:new Date().toISOString()}).eq("id",e.matchId);const p={credits:(o.profile.credits||0)+d,matches_played:(o.profile.matches_played||0)+1};n?p.wins=(o.profile.wins||0)+1:r?p.draws=(o.profile.draws||0)+1:p.losses=(o.profile.losses||0)+1,await v.from("users").update(p).eq("id",o.profile.id),await i.refreshProfile();const c=document.createElement("div");c.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",c.innerHTML=`
    <div style="text-align:center;padding:40px;color:#fff;max-width:380px;width:100%">
      ...
      ${si(t.aiTeam,t.formation,null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.1))}
            ...
            <div style="background:rgba(255,255,255,0.08);border-radius:14px;padding:14px 20px;text-align:center;min-width:220px">
              ...
              ${Fe.id!==mt.id?`<div style="font-size:20px;font-weight:900;color:${Fe.color}">
                    ${Je>oe.mmr?"📈":"📉"} ${mt.emoji} ${mt.label} → ${Fe.emoji} ${Fe.label}
                  </div>
                  <div style="font-size:13px;color:rgba(255,255,255,0.5);margin-top:4px">${Je>oe.mmr?"Promotion !":"Rétrogradation"}</div>`:`<div style="font-size:36px">${De>=0?"📈":"📉"}</div>
                   <div style="font-size:18px;font-weight:900;color:${Fe.color}">${Fe.emoji} ${Fe.label}</div>`}
              ${ve?`<div style="font-size:11px;color:rgba(255,255,255,0.4);margin-top:6px">Match de placement (${oe.placement_matches+1}/10)</div>`:""}
            </div>`}}catch(oe){console.error("[Ranked] MMR update error:",oe)}(re=document.getElementById("pvp-end-overlay"))==null||re.remove();const R=document.createElement("div");R.id="pvp-end-overlay",R.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;color:#fff;padding:24px;text-align:center;overflow-y:auto";const V=q?"🤝":O?"🏆":"😞",Q=q?"MATCH NUL":O?"VICTOIRE !":"DÉFAITE",ie=q?"#fff":O?"#FFD700":"#ff6b6b";R.innerHTML=`
      ...
      ${N}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${p?"🏆 Retour à la Mini League":a?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(R),(se=R.querySelector("#pvp-end-home"))==null||se.addEventListener("click",()=>{R.remove(),ut(t),p?x("mini-league",{openLeagueId:p}):x(a?"ranked":"home")})}const w=v.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},k=>{var B;const A=k.new;try{if(A.status==="finished"||A.forfeit){if(j)return;j=!0,T&&(clearInterval(T),T=null),it(),A.game_state&&(u=A.game_state),M(A);return}if(A.game_state){const O=u;u=A.game_state;const q=u._lastGC;if(q&&q.seq&&!X.has(q.seq)&&(X.add(q.seq),q.by!==f)){pe(q.type,q.by,()=>z());return}const N=O[f+"Score"]||0,R=O[h+"Score"]||0,V=u[f+"Score"]||0,Q=u[h+"Score"]||0,ie=V>N,re=Q>R;if((ie||re)&&!Y.has(u.round)){Y.add(u.round);const se=[...u.log||[]].reverse().find(be=>be.isGoal),oe=((se==null?void 0:se.homePlayers)||[]).map(be=>({name:be.name,note:be.note,portrait:be.portrait,job:be.job}));!!((B=se==null?void 0:se.text)!=null&&B.includes("DERNIER CORNER"))?K().then(()=>{dt(oe,V,Q,ie,()=>z())}):dt(oe,V,Q,ie,()=>z());return}z()}}catch(O){console.error("[PvP] crash:",O)}}).subscribe();async function b(k){Object.assign(u,k),u.lastActionAt=new Date().toISOString();const{error:A}=await v.from("matches").update({game_state:u}).eq("id",i);A&&m("Erreur de synchronisation","error");try{z()}catch(B){console.error("[PvP] renderPvpScreen crash:",B)}}async function E(){if(j)return;j=!0,ti(),it(),T&&(clearInterval(T),T=null);const k=o?g.away_id:g.home_id,A=o?"p2":"p1",B=o?"p1":"p2",O={...u,[A+"Score"]:3,[B+"Score"]:0,phase:"finished"},q=O.p1Score||0,N=O.p2Score||0;try{await v.from("matches").update({game_state:O}).eq("id",i);const{data:R,error:V}=await v.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:k,p_home_score:q,p_away_score:N,p_forfeit:!0});if(V&&console.error("[PvP] finish_pvp_match (forfeit):",V),p)try{await v.rpc("check_and_finish_mini_league",{p_league_id:p})}catch(Q){console.error("[PvP] check_and_finish_mini_league (forfeit):",Q)}try{const{data:Q}=await v.rpc("apply_match_rewards",{p_match_id:i});Q!=null&&Q.success&&!(Q!=null&&Q.skipped)&&typeof e.refreshProfile=="function"&&await e.refreshProfile()}catch(Q){console.error("[PvP] apply_match_rewards (forfeit):",Q)}}catch{}try{v.removeChannel(w)}catch{}setTimeout(()=>{ut(t),p?x("mini-league",{openLeagueId:p}):x("home")},800)}const H={BOOST_STAT:({value:k=1,count:A=1,roles:B=[]},O,q)=>{const N=O[f+"Team"],R=Object.entries(N).filter(([V])=>!B.length||B.includes(V)).flatMap(([V,Q])=>Q.filter(ie=>!ie.used).map(ie=>({...ie,_line:V})));if(!R.length){O.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),q(O);return}S(R,A,`Choisir ${A} joueur(s) à booster (+${k})`,V=>{V.forEach(Q=>{const ie=(N[Q._line]||[]).find(re=>re.cardId===Q.cardId);ie&&(ie.boost=(ie.boost||0)+k,O.log.push({text:`⚡ +${k} sur ${ie.name}`,type:"info"}))}),O[f+"Team"]=N,q(O)})},DEBUFF_STAT:({value:k=1,count:A=1,roles:B=[],target:O="ai"},q,N)=>{const R=O==="home"?f:h,V=q[R+"Team"],Q=O==="home"?"allié":"adverse",ie=Object.entries(V).filter(([re])=>!B.length||B.includes(re)).flatMap(([re,se])=>se.map(oe=>({...oe,_line:re})));if(!ie.length){q.log.push({text:`🎯 Aucun joueur ${Q}`,type:"info"}),N(q);return}S(ie,A,`Choisir ${A} joueur(s) ${Q}(s) (-${k})`,re=>{re.forEach(se=>{const oe=(V[se._line]||[]).find(ce=>ce.cardId===se.cardId);oe&&(oe.boost=(oe.boost||0)-k,q.log.push({text:`🎯 -${k} sur ${oe.name}`,type:"info"}))}),q[R+"Team"]=V,N(q)})},GRAY_PLAYER:({count:k=1,roles:A=[],target:B="ai"},O,q)=>{const N=B==="home"?f:h,R=O[N+"Team"],V=B==="home"?"allié":"adverse",Q=Object.entries(R).filter(([ie])=>!A.length||A.includes(ie)).flatMap(([ie,re])=>re.filter(se=>!se.used).map(se=>({...se,_line:ie})));if(!Q.length){O.log.push({text:`❌ Aucun joueur ${V}`,type:"info"}),q(O);return}S(Q,k,`Choisir ${k} joueur(s) ${V}(s) à exclure`,ie=>{const re="usedCardIds_"+N,se=new Set(O[re]||[]);ie.forEach(oe=>{const ce=(R[oe._line]||[]).find(be=>be.cardId===oe.cardId);ce&&(ce.used=!0,se.add(oe.cardId),O.log.push({text:`❌ ${ce.name} exclu !`,type:"info"}))}),O[re]=[...se],O[N+"Team"]=R,q(O)})},REVIVE_PLAYER:({count:k=1,roles:A=[]},B,O)=>{const q=B[f+"Team"],N=Object.entries(q).filter(([R])=>!A.length||A.includes(R)).flatMap(([R,V])=>V.filter(Q=>Q.used).map(Q=>({...Q,_line:R})));if(!N.length){B.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),O(B);return}S(N,k,`Choisir ${k} joueur(s) à ressusciter`,R=>{R.forEach(V=>{const Q=(q[V._line]||[]).find(ie=>ie.cardId===V.cardId);Q&&(Q.used=!1,B.log.push({text:`💫 ${Q.name} ressuscité !`,type:"info"}))}),B[f+"Team"]=q,O(B)})},REMOVE_GOAL:({},k,A)=>{const B=h+"Score";k[B]>0?(k[B]--,k.log.push({text:"🚫 Dernier but annulé !",type:"info"})):k.log.push({text:"🚫 Aucun but à annuler",type:"info"}),A(k)},ADD_GOAL_DRAW:({},k,A)=>{k[f+"Score"]===k[h+"Score"]?(k[f+"Score"]++,k.log.push({text:"🎯 But bonus !",type:"info"})):k.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),A(k)},ADD_SUB:({value:k=1},A,B)=>{A.maxSubs=(A.maxSubs||3)+k,A.log.push({text:`🔄 +${k} remplacement(s)`,type:"info"}),B(A)},CUSTOM:({},k,A)=>A(k)};function S(k,A,B,O){const q=document.createElement("div");q.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let N=[];function R(){var Q,ie;const V=k.map(re=>{const se=re._line||re.job||"MIL",oe=N.find(be=>be.cardId===re.cardId),ce=qe({...re,_evolution_bonus:0},{width:90,showStad:!0,role:se,extraNote:re.boost||0,context:"match"});return`<div class="pp-item" data-cid="${re.cardId}"
        ...
        </div>`,(Q=q.querySelector("#pp-close"))==null||Q.addEventListener("click",()=>q.remove()),q.querySelectorAll(".pp-item").forEach(re=>{re.addEventListener("click",()=>{const se=re.dataset.cid,oe=k.find(be=>be.cardId===se),ce=N.findIndex(be=>be.cardId===se);oe&&(ce>-1?N.splice(ce,1):N.length<A&&N.push(oe),R())})}),(ie=q.querySelector("#pp-confirm"))==null||ie.addEventListener("click",()=>{q.remove(),O(N)})}R(),document.body.appendChild(q)}async function G(k,A){const O=(u["gcCardsFull_"+f]||[]).find(V=>V.id===k),q=(O==null?void 0:O._gcDef)||(u.gcDefs||[]).find(V=>{var Q;return V.name===A||((Q=V.name)==null?void 0:Q.toLowerCase().trim())===(A==null?void 0:A.toLowerCase().trim())}),N=[...u["usedGc_"+f]||[],k],R={type:A,by:f,seq:(u._gcAnimSeq||0)+1};X.add(R.seq),pe(A,f,async()=>{if(q!=null&&q.effect_type&&q.effect_type!=="CUSTOM"){const Q=H[q.effect_type];if(Q){const ie={...u};Q(q.effect_params||{},ie,async re=>{re["usedGc_"+f]=N,re._lastGC=R,re._gcAnimSeq=R.seq,await b(re)});return}}const V={...u};switch(A){case"Remplacement+":V.maxSubs=(V.maxSubs||3)+1,V.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const Q=h+"Score";V[Q]>0&&(V[Q]--,V.log.push({text:"🚫 But annulé",type:"info"}));break}}V["usedGc_"+f]=N,V._lastGC=R,V._gcAnimSeq=R.seq,await b(V)})}function de(k,A){const B="usedCardIds_"+k,O=new Set(u[B]||[]);A.forEach(q=>O.add(q)),u[B]=[...O]}function I(){for(const k of["p1","p2"]){const A=new Set(u["usedCardIds_"+k]||[]),B=u[k+"Team"];if(B)for(const O of["GK","DEF","MIL","ATT"])(B[O]||[]).forEach(q=>{q.used=A.has(q.cardId)})}}function z(){var Mo,Co,jo,qo,Bo,Do;if(u.phase==="reveal")return Z();if(u.phase==="midfield")return te();if(u.phase==="finished")return ke();const k=u[f+"Team"],A=u[h+"Team"];I();const B=u[f+"Score"],O=u[h+"Score"],q=u[f+"Name"],N=u[h+"Name"],R=u.phase===f+"-attack",V=u.phase===f+"-defense",Q=Array.isArray(u["selected_"+f])?u["selected_"+f]:[],ie=Q.map(ae=>ae.cardId),re=window.innerWidth>=700,se=u[f+"Subs"]||[],oe=u["usedSubIds_"+f]||[],ce=se.filter(ae=>!oe.includes(ae.cardId)),be=u["gcCardsFull_"+f]||[],Se=u["usedGc_"+f]||[],ve=be.filter(ae=>!Se.includes(ae.id)),Ae=u.boostOwner===f&&!u.boostUsed,Me=[...k.MIL||[],...k.ATT||[]].filter(ae=>!ae.used),le=[...A.MIL||[],...A.ATT||[]].filter(ae=>!ae.used),De=!me(A),Je=Me.length===0&&le.length===0&&!De,Fe=(k.DEF||[]).filter(ae=>!ae.used),nt=(k.GK||[]).filter(ae=>!ae.used);let et=[];R&&Me.length===0&&(De?(et=Fe.map(ae=>ae.cardId),Fe.length===0&&(et=et.concat(nt.map(ae=>ae.cardId)))):Je&&(et=[...Fe,...nt].map(ae=>ae.cardId)));function mt(ae,ze,Re){var Ne,$t;const je=ae._gcDef,Jt=(je==null?void 0:je.name)||ae.gc_type,xt=(je==null?void 0:je.effect)||((Ne=Qe[ae.gc_type])==null?void 0:Ne.desc)||"",Ye=je!=null&&je.image_url?`/icons/${je.image_url}`:null,Xe=(($t=Qe[ae.gc_type])==null?void 0:$t.icon)||"⚡";return`<div class="pvp-gc-mini" data-gc-id="${ae.id}" data-gc-type="${ae.gc_type}" style="flex-shrink:0;cursor:pointer">
        ${Ze(Jt,Ye,Xe,xt,{width:ze})}
      </div>`}function ci(ae,ze){return`<div id="pvp-boost-card"
        style="box-sizing:border-box;width:${ae}px;height:${ze}px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:2px solid #87CEEB;border-radius:10px;cursor:pointer;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:${Math.round(ze*.04)}px;text-align:center;flex-shrink:0">
        <div style="font-size:${Math.round(ze*.2)}px">⚡</div>
        <div style="font-size:${Math.round(ze*.09)}px;color:#000;font-weight:900">+${u.boostValue}</div>
      </div>`}const We=(ae,ze)=>ze?ci(95,162):mt(ae,95),rt=(ae,ze)=>ze?ci(68,116):mt(ae,68),tt=re?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",_t=R?ue(f)?`<button id="pvp-action" style="${tt};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${Q.length===0?"disabled":""}>⚔️ ATTAQUEZ <span id="pvp-timer"></span></button>`:`<button id="pvp-action" data-pass="1" style="${tt};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER <span id="pvp-timer"></span></button>`:V?`<button id="pvp-action" style="${tt};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${Q.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="pvp-timer"></span></button>`:`<div style="font-size:11px;color:rgba(255,255,255,0.3);text-align:center;padding:4px">⏳ Tour de ${N}</div>`,gt=R&&!ue(f)?'<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">Aucun attaquant — passez la main</div>':R||V?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${Q.length}/3 sélectionné(s)</div>`:"",Wt=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${re?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
    ...
    </div>`,bt=u[f+"Team"],Yn=(()=>{var ze,Re,je,Jt,xt;if(V&&((ze=u.pendingAttack)!=null&&ze.players)){const Ye=u.pendingAttack;let Xe="";if(Q.length>0){const Ne=Q.map(at=>{const pi=(bt[at._role]||[]).find(Ji=>Ji.cardId===at.cardId)||at,ui=bt[at._role]||[],Si=ui.findIndex(Ji=>Ji.cardId===at.cardId),Yi=Ft(ui.length),fi=Si>=0?Yi[Si]:pi._col??1;return{...pi,_line:at._role,_col:fi}}),$t=ki(Ne,((Re=u.modifiers)==null?void 0:Re[f])||{},u[f+"Formation"]);Xe=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
        ...
        </div>`}if(R&&!((Jt=u.pendingAttack)!=null&&Jt.players)&&Q.length>0){const Ye=Q.map(Ne=>{const $t=(bt[Ne._role]||[]).find(fi=>fi.cardId===Ne.cardId)||Ne,at=["GK","DEF"].includes(Ne._role),pi=bt[Ne._role]||[],ui=pi.findIndex(fi=>fi.cardId===Ne.cardId),Si=Ft(pi.length),Yi=ui>=0?Si[ui]:$t._col??1;return{...$t,_line:Ne._role,_col:Yi,...at?{note_a:Math.max(1,Number($t.note_a)||0)}:{}}}),Xe=$i(Ye,((xt=u.modifiers)==null?void 0:xt[f])||{},u[f+"Formation"]);return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #FFD700;text-align:center">
      ...
      </button>`;Pt(t),t.style.overflow="hidden",re?t.innerHTML=`
      <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
        ${Io}
        <div style="display:flex;flex:1;min-height:0;overflow:hidden">
          ${Wt}
      ...
      </div>`;function Yt(){const ae=t.querySelector(".match-screen");if(!ae)return;const ze=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);ae.style.height=ze+"px",ae.style.minHeight=ze+"px",ae.style.maxHeight=ze+"px",ae.style.overflow="hidden";const Re=t.querySelector("#mobile-action-bar"),je=t.querySelector("#mobile-play-area");Re&&je&&(je.style.paddingBottom=Re.offsetHeight+"px")}if(Yt(),setTimeout(Yt,120),setTimeout(Yt,400),F||(F=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",Yt),window.visualViewport.addEventListener("scroll",Yt)),window.addEventListener("resize",Yt)),function(){const ze=t.querySelector("#match-field .terrain-wrapper svg")||t.querySelector(".terrain-wrapper svg");if(!ze)return;const Re=ze.closest("#match-terrain-wrap");Re&&(Re.style.cssText="position:relative;width:100%;height:100%;padding:0"),ze.removeAttribute("width"),ze.removeAttribute("height"),ze.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",ze.setAttribute("preserveAspectRatio","xMidYMid meet")}(),u._pvpResizeBound||(u._pvpResizeBound=!0,window.addEventListener("resize",()=>{const ae=t.querySelector(".terrain-wrapper svg");if(ae){const ze=ae.closest("#match-terrain-wrap");ze&&(ze.style.cssText="position:relative;width:100%;height:100%;padding:0"),ae.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0"}})),t.querySelectorAll(".match-slot-hit").forEach(ae=>{ae.addEventListener("click",()=>{if(!R&&!V)return;const ze=ae.dataset.cardId,Re=ae.dataset.role,je=(k[Re]||[]).find(Xe=>Xe.cardId===ze);if(!je||je.used)return;const Jt=et.includes(ze);if(R&&!["MIL","ATT"].includes(Re)&&!Jt)return;Array.isArray(u["selected_"+f])||(u["selected_"+f]=[]);const xt=u["selected_"+f],Ye=xt.findIndex(Xe=>Xe.cardId===ze);Ye>-1?xt.splice(Ye,1):xt.length<3&&xt.push({...je,_role:Re}),z()})}),t.querySelectorAll(".match-used-hit").forEach(ae=>{ae.addEventListener("click",()=>Be(ae.dataset.cardId))}),t.querySelectorAll(".pvp-sub-btn").forEach(ae=>{ae.addEventListener("click",()=>Be())}),(Mo=t.querySelector("#pvp-sub-open"))==null||Mo.addEventListener("click",()=>Be()),t.querySelectorAll(".pvp-gc-mini").forEach(ae=>{R?ae.addEventListener("click",()=>he(ae.dataset.gcId,ae.dataset.gcType)):(ae.style.opacity="0.35",ae.style.cursor="default",ae.addEventListener("click",()=>Bt("⚡ Les Game Changers ne sont utilisables qu'en attaque","rgba(180,100,0,0.9)")))}),(Co=t.querySelector("#pvp-boost-card"))==null||Co.addEventListener("click",()=>fe()),(jo=t.querySelector("#pvp-action"))==null||jo.addEventListener("click",ae=>{R?ae.currentTarget.dataset.pass==="1"||!ue(f)?Ee():J():V&&P()}),(qo=t.querySelector("#pvp-quit"))==null||qo.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&E()}),(Bo=t.querySelector("#pvp-view-opp"))==null||Bo.addEventListener("click",()=>_e()),(Do=t.querySelector("#pvp-toggle-history"))==null||Do.addEventListener("click",()=>ye()),T&&(clearInterval(T),T=null),it(),(R||V)&&!j){let ae=30,ze=!1;const Re=()=>document.getElementById("pvp-timer"),je=()=>{Re()&&(Re().textContent=ae+"s",Re().style.color=ze?"#ff4444":"#fff")};je(),T=setInterval(()=>{ae--,ae<0?ze?(clearInterval(T),T=null,it(),R&&!ue(f)?Ee():E()):(ze=!0,ae=15,je(),gn("/sounds/timer-urgent.mp3",.6)):je()},1e3)}}function Z(){Pt(t),t.innerHTML=`
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="animation:gcFlipIn .7s cubic-bezier(.34,1.56,.64,1) both,gcGlow 1.8s ease infinite .7s">
        ${Ze(N,V,Q,R,{width:200})}
      </div>
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(se);let oe=!1;const ce=()=>{oe||(oe=!0,se.remove(),setTimeout(()=>B&&B(),50))};se.addEventListener("click",ce),setTimeout(ce,3e3)}function he(k,A){var re,se,oe,ce;const O=(u["gcCardsFull_"+f]||[]).find(be=>be.id===k),q=O==null?void 0:O._gcDef,N=(q==null?void 0:q.name)||A,R=(q==null?void 0:q.effect)||((re=Qe[A])==null?void 0:re.desc)||"Carte spéciale.",V=q!=null&&q.image_url?`/icons/${q.image_url}`:null,Q=((se=Qe[A])==null?void 0:se.icon)||"⚡",ie=document.createElement("div");ie.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",ie.innerHTML=`
      ${Ze(N,V,Q,R,{width:190})}
      <div style="display:flex;gap:12px;width:190px">
      ...
      </div>`,document.body.appendChild(B),(O=B.querySelector("#bp-close"))==null||O.addEventListener("click",()=>B.remove()),B.querySelectorAll(".bp-item").forEach(q=>{q.addEventListener("click",async()=>{const N=q.dataset.cid,R=A.find(Q=>Q.cardId===N);if(!R)return;const V=(k[R._line]||[]).find(Q=>Q.cardId===N);V&&(V.boost=(V.boost||0)+u.boostValue),B.remove(),await b({[f+"Team"]:k,boostUsed:!0})})})}function Be(k=null){var ce,be;if(!(u.phase===f+"-attack")){m("Remplacement uniquement avant votre attaque","warning");return}const B=u[f+"Team"],O=u["usedSubIds_"+f]||[],q=u.maxSubs||3;if(O.length>=q){m(`Maximum ${q} remplacements atteint`,"warning");return}const N=Object.entries(B).flatMap(([Se,ve])=>(ve||[]).filter(Ae=>Ae.used).map(Ae=>({...Ae,_line:Se}))),R=(u[f+"Subs"]||[]).filter(Se=>!O.includes(Se.cardId));if(!N.length){m("Aucun joueur utilisé à remplacer","warning");return}if(!R.length){m("Aucun remplaçant disponible","warning");return}let V=Math.max(0,N.findIndex(Se=>Se.cardId===k));const Q=((ce=N[V])==null?void 0:ce._line)||((be=N[V])==null?void 0:be.job);let ie=Math.max(0,R.findIndex(Se=>Se.job===Q)),re=!1;const se=document.createElement("div");se.id="pvp-sub-overlay",se.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function oe(){var Je,Fe,nt,et,mt,ci;const Se=N[V],ve=R[ie],Ae=Math.min(130,Math.round((window.innerWidth-90)/2)),Me=Math.round(Ae*1.35),le=We=>`background:rgba(255,255,255,0.12);border:none;color:${We?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${We?"default":"pointer"};flex-shrink:0`;se.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      </div>`,(Je=se.querySelector("#psub-close"))==null||Je.addEventListener("click",()=>se.remove()),(Fe=se.querySelector("#pout-up"))==null||Fe.addEventListener("click",()=>{V>0&&(V--,oe())}),(nt=se.querySelector("#pout-down"))==null||nt.addEventListener("click",()=>{V<N.length-1&&(V++,oe())}),(et=se.querySelector("#pin-up"))==null||et.addEventListener("click",()=>{ie>0&&(ie--,oe())}),(mt=se.querySelector("#pin-down"))==null||mt.addEventListener("click",()=>{ie<R.length-1&&(ie++,oe())});const De=(We,rt,tt,_t)=>{const gt=se.querySelector("#"+We);if(!gt)return;let Wt=0;gt.addEventListener("touchstart",kt=>{Wt=kt.touches[0].clientY},{passive:!0}),gt.addEventListener("touchend",kt=>{const qt=kt.changedTouches[0].clientY-Wt;if(Math.abs(qt)<30)return;const bt=rt();qt<0&&bt<_t-1?(tt(bt+1),oe()):qt>0&&bt>0&&(tt(bt-1),oe())},{passive:!0})};De("pin-panel",()=>ie,We=>ie=We,R.length),De("pout-panel",()=>V,We=>V=We,N.length),(ci=se.querySelector("#psub-confirm"))==null||ci.addEventListener("click",async We=>{if(We.preventDefault(),We.stopPropagation(),re)return;re=!0;const rt=N[V],tt=R[ie];if(!rt||!tt)return;const _t=rt._line,gt=(B[_t]||[]).findIndex(qt=>qt.cardId===rt.cardId);if(gt===-1){m("Erreur : joueur introuvable","error"),se.remove();return}const Wt={...tt,_line:_t,position:rt.position,used:!1,boost:0};B[_t].splice(gt,1,Wt);const kt=[...O,tt.cardId];se.remove(),Pe(rt,tt,async()=>{await b({[f+"Team"]:B,[h+"Team"]:u[h+"Team"],["usedSubIds_"+f]:kt})})})}document.body.appendChild(se),oe()}function Pe(k,A,B){const O={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},q=document.createElement("div");q.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const N=(Q,ie,re)=>`<div style="text-align:center">
      <div style="font-size:9px;color:${ie};letter-spacing:2px;text-transform:uppercase;font-weight:700;margin-bottom:6px">${re}</div>
      <div style="width:70px;height:70px;border-radius:50%;background:${O[Q.job]||"#555"};border:3px solid ${ie};position:relative;overflow:hidden;margin:0 auto">
        ${Ut(Q)?`<img src="${Ut(Q)}" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover">`:""}
      </div>
      ...
      </div>`,document.body.appendChild(A),(B=A.querySelector("#pvp-hist-close"))==null||B.addEventListener("click",()=>A.remove())}async function Ee(){if(u.phase!==f+"-attack")return;const k=f==="p1"?"p2":"p1",A=(u.round||0)+1,B=[...u.log||[]];B.push({type:"info",text:`⏭️ ${u[f+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const O=Le(u),q=ue(k),N=O||!q?"finished":k+"-attack";await b({["selected_"+f]:[],modifiers:{...u.modifiers,[f]:{}},pendingAttack:null,phase:N,attacker:k,round:A,log:B}),N==="finished"&&await we(u)}function D(k){const A=(k.GK||[]).some(O=>!O.used),B=["DEF","MIL","ATT"].some(O=>(k[O]||[]).some(q=>!q.used));return A&&!B}function C(k){return(k.p1Score||0)!==(k.p2Score||0)?null:D(k.p1Team)&&!me(k.p2Team)?{side:"p1",gk:(k.p1Team.GK||[]).find(A=>!A.used)}:D(k.p2Team)&&!me(k.p1Team)?{side:"p2",gk:(k.p2Team.GK||[]).find(A=>!A.used)}:null}async function K(){const k=document.createElement("div");k.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:950;display:flex;align-items:center;justify-content:center;overflow:hidden;text-align:center;padding:24px",document.body.appendChild(k);const A=(B,O)=>new Promise(q=>{k.innerHTML=`<div style="font-size:32px;font-weight:900;color:${O};letter-spacing:2px;animation:lcFade 1.4s ease both">${B}</div>
      <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(q,1400)});await A("⚽ DERNIER CORNER","#FFD700"),await A("🧤 LE GARDIEN MONTE !","#4fc3f7"),k.remove()}async function W(k,A){const{side:B,gk:O}=k;if(!O)return;const q=B+"Score",N=(A[q]||0)+1;O.used=!0;const R=[...A.log||[]];R.push({type:"duel",isGoal:!0,homeScored:B===f,homePlayers:B==="p1"?[Qt(O)]:[],aiPlayers:B==="p2"?[Qt(O)]:[],text:`⚽ DERNIER CORNER — Le gardien de ${A[B+"Name"]} marque !`});const V=(A.round||0)+1,Q=B==="p1"?"p2":"p1",ie={...A,[q]:N},re=Le(ie);Y.add(V),await K();const se=B===f?N:A[f+"Score"]||0,oe=B===f?A[h+"Score"]||0:N;dt([Qt(O)],se,oe,B===f,async()=>{await b({[q]:N,log:R,round:V,pendingAttack:null,phase:re?"finished":Q+"-attack",attacker:Q,["selected_"+f]:[],modifiers:{...u.modifiers,[f]:{}}}),re&&await we({...u,[q]:N})})}async function J(){const k=u[f+"Team"],A=!["GK","DEF","MIL","ATT"].some(N=>(u[h+"Team"][N]||[]).some(R=>!R.used)),B=(u["selected_"+f]||[]).map(N=>{const R=(k[N._role]||[]).find(be=>be.cardId===N.cardId)||N,V=["GK","DEF"].includes(N._role),Q=k[N._role]||[],ie=Q.findIndex(be=>be.cardId===N.cardId),re=Ft(Q.length),se=ie>=0?re[ie]:R._col??1,oe=u.stadiumDef||u.homeStadiumDef||null,ce=R.stadiumBonus||oe&&(oe.club_id&&String(R.club_id)===String(oe.club_id)||oe.country_code&&R.country_code===oe.country_code)||!1;return{...R,_line:N._role,_col:se,stadiumBonus:ce,...V?{note_a:Math.max(1,Number(R.note_a)||0)}:{}}});if(!B.length)return;const O=$i(B,u.modifiers[f]||{},u[f+"Formation"]);de(f,B.map(N=>N.cardId)),B.forEach(N=>{const R=(k[N._role]||[]).find(V=>V.cardId===N.cardId);R&&(R.used=!0)}),u["selected_"+f]=[],z();const q=[...u.log||[]];if(A){if(B.length===1&&(B[0]._line==="GK"||B[0].job==="GK")&&(u.p1Score||0)===(u.p2Score||0)){await W({side:f,gk:B[0]},{...u,[f+"Team"]:k,log:q});return}const R=(u[f+"Score"]||0)+1,V=B.map(oe=>Qt(oe));q.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:V,homeTotal:O.total,aiTotal:0});const Q=(u.round||0)+1,ie=f==="p1"?"p2":"p1",re={...u,[f+"Team"]:k,[f+"Score"]:R},se=Le(re);Y.add(Q),dt(V,R,u[h+"Score"]||0,!0,async()=>{await b({[f+"Team"]:k,[f+"Score"]:R,["selected_"+f]:[],modifiers:{...u.modifiers,[f]:{}},pendingAttack:null,phase:se?"finished":ie+"-attack",attacker:ie,round:Q,log:q}),se&&await we({...u,[f+"Score"]:R})});return}q.push({type:"pending",text:`⚔️ ${u[f+"Name"]} attaque (${O.total})`}),await b({[f+"Team"]:k,[h+"Team"]:u[h+"Team"],pendingAttack:{...O,players:B,side:f},["selected_"+f]:[],modifiers:{...u.modifiers,[f]:{}},phase:h+"-defense",log:q})}async function P(){const k=u[f+"Team"],A=(u["selected_"+f]||[]).map(ve=>{const Ae=(k[ve._role]||[]).find(Fe=>Fe.cardId===ve.cardId)||ve,Me=k[ve._role]||[],le=Me.findIndex(Fe=>Fe.cardId===ve.cardId),De=Ft(Me.length),Je=le>=0?De[le]:Ae._col??1;return{...Ae,_line:ve._role,_col:Je}}),B=ki(A,u.modifiers[f]||{},u[f+"Formation"]);de(f,A.map(ve=>ve.cardId)),A.forEach(ve=>{const Ae=(k[ve._role]||[]).find(Me=>Me.cardId===ve.cardId);Ae&&(Ae.used=!0)}),u["selected_"+f]=[],z();const O=xo(u.pendingAttack.total,B.total,u.modifiers[f]||{}),q=u.pendingAttack.side,N=O==="attack"||(O==null?void 0:O.goal),R=q==="p1"?"p2":"p1",V=(u.round||0)+1,Q=(u.pendingAttack.players||[]).map(ve=>Qt(ve)),ie=[...u.log||[]];ie.push({type:"duel",isGoal:N,homeScored:N&&q===f,text:N?`⚽ BUT de ${u[q+"Name"]} ! (${u.pendingAttack.total} vs ${B.total})`:`✋ Attaque stoppée (${u.pendingAttack.total} vs ${B.total})`,homePlayers:Q,aiPlayers:A.map(ve=>Qt(ve)),homeTotal:u.pendingAttack.total,aiTotal:B.total});const re=N?(u[q+"Score"]||0)+1:u[q+"Score"]||0,se={...u,[f+"Team"]:k,[q+"Score"]:re},oe=Le(se),ce=oe?"finished":R+"-attack",be=!oe&&!N?C(se):null,Se=async()=>{if(be){await W(be,se);return}await b({[f+"Team"]:k,[h+"Team"]:u[h+"Team"],[q+"Score"]:re,["selected_"+f]:[],modifiers:{...u.modifiers,[f]:{}},pendingAttack:null,phase:ce,attacker:R,round:V,log:ie}),(ce==="finished"||oe)&&await we({...u,[q+"Score"]:re})};if(N){const ve=q===f,Ae=ve?re:u[f+"Score"]||0,Me=ve?u[h+"Score"]||0:re;Y.add(V),dt(Q,Ae,Me,ve,Se)}else await Se()}function ee(k){return["MIL","ATT"].some(A=>(k[A]||[]).some(B=>!B.used))}function me(k){return["GK","DEF","MIL","ATT"].some(A=>(k[A]||[]).some(B=>!B.used))}function xe(k){return me(k)&&!ee(k)}function ue(k){const A=u[k+"Team"],B=u[(k==="p1"?"p2":"p1")+"Team"];return!!(ee(A)||xe(A)&&!me(B)||xe(A)&&!ee(B)&&me(B))}function Le(k){return!me(k.p1Team)&&!me(k.p2Team)}function Ie(k){const A=k.p1Score||0,B=k.p2Score||0;return A===B?null:A>B?g.home_id:g.away_id}async function we(k){ti(),it();try{const A=Ie(k),B=A?g.home_id===A?g.away_id:g.home_id:null,O=k.p1Score||0,q=k.p2Score||0,{data:N,error:R}=await v.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:A,p_home_score:O,p_away_score:q,p_forfeit:!1});if(R&&console.error("[PvP] finish_pvp_match:",R),p)try{await v.rpc("check_and_finish_mini_league",{p_league_id:p})}catch(V){console.error("[PvP] check_and_finish_mini_league:",V)}A&&B&&Fa(A,B).catch(()=>{});try{const{data:V}=await v.rpc("apply_match_rewards",{p_match_id:i});V!=null&&V.success&&!(V!=null&&V.skipped)&&typeof e.refreshProfile=="function"&&await e.refreshProfile()}catch(V){console.error("[PvP] apply_match_rewards:",V)}if(typeof s=="function")try{await s({homeScore:O,awayScore:q,winnerId:A,homeId:g.home_id,awayId:g.away_id,matchId:i})}catch(V){console.error("[PvP] onMatchEnd:",V)}}catch(A){console.error("[PvP] finishMatch:",A)}}function ke(){var q;const k=u[f+"Score"],A=u[h+"Score"],B=k>A,O=k===A;Pt(t),t.innerHTML=`
    ...
    </div>`,(q=document.getElementById("pvp-home"))==null||q.addEventListener("click",()=>{try{v.removeChannel(w)}catch{}ut(t),p?x("mini-league",{openLeagueId:p}):x("home")})}z()}async function fo(t,e,i=!1){var d,l;const{state:o,navigate:n,toast:r}=e,a=i&&((l=(d=e==null?void 0:e.state)==null?void 0:d.params)==null?void 0:l.rankedData)||null;await Ui(t,e,i?"ranked":"random",async({deckId:s,formation:p,starters:c,subsRaw:y,gcCardsEnriched:x,gcDefs:m,stadiumDef:f})=>{Pt(t);const h=async _=>{await Na(t,e,s,p,c,y,_||[],m,f,i,a)};if(!(x!=null&&x.length)){await h([]);return}Ki(t,x,h)})}async function Ra(t,e,i){return Pa(t,e,i)}async function Na(t,e,i,o,n,r,a=[],d=[],l=null,s=!1,p=null){var M,w;const{state:c,navigate:y,toast:x}=e,m=(p==null?void 0:p.mmr)??((M=c.profile)==null?void 0:M.mmr)??450,f=s?zt(m):null,h=s?"linear-gradient(135deg, #1a0a2e 0%, #2d1060 50%, #1a0a2e 100%)":"linear-gradient(135deg, #0a1a2e 0%, #0d3d1e 100%)",_=s?(f==null?void 0:f.color)||"#D4A017":"#FFD700";let $=!1,g=null,L=null;t.innerHTML=`
    ...
    </div>`;const u=async()=>{$=!0,g&&(g.unsubscribe(),g=null),L&&(clearInterval(L),L=null);try{await v.rpc("cancel_matchmaking",{p_user_id:c.profile.id})}catch(b){console.error("[Matchmaking] cancel error:",b)}};(w=document.getElementById("cancel-mm"))==null||w.addEventListener("click",async()=>{await u(),ut(t),y("home")});const j=async(b,E)=>{$||($=!0,g&&(g.unsubscribe(),g=null),L&&(clearInterval(L),L=null),await Vt(t,e,b,E,{myGC:a,gcDefs:d,stadiumDef:l,isRanked:s,rankedData:p,onMatchEnd:s?Oa:null}))},T=c.user.id,{data:F,error:Y}=s?await v.rpc("try_matchmake_ranked",{p_user_id:T,p_deck_id:i,p_mmr:m,p_range:200}):await v.rpc("try_matchmake",{p_user_id:T,p_deck_id:i});if(Y||!(F!=null&&F.success)){console.error("[Matchmaking] try_matchmake error:",Y||F),x("Erreur de matchmaking","error"),ut(t),y("home");return}if(F.matched){await j(F.match_id,!1);return}const X=async()=>{if($)return;const{data:b}=await v.from("matchmaking_queue").select("status, match_id").eq("user_id",T).maybeSingle();(b==null?void 0:b.status)==="matched"&&b.match_id&&await j(b.match_id,!0)};g=v.channel(`mm_${T}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${T}`},b=>{const E=b.new;E.status==="matched"&&E.match_id&&j(E.match_id,!0)}).subscribe(),L=setInterval(X,3e3),s?setTimeout(async()=>{if(!$){$=!0,g&&(g.unsubscribe(),g=null),L&&(clearInterval(L),L=null);try{await v.rpc("cancel_matchmaking",{p_user_id:c.profile.id})}catch(b){console.error("[Matchmaking] cancel error:",b)}x("Aucun adversaire trouvé — match contre une IA calibrée sur votre niveau","info",4e3),y("match",{matchMode:"ranked_ai",rankedData:p,presetSetup:{deckId:i,formation:o,starters:n,subsRaw:r,gcCardsEnriched:a,gcDefs:d,stadiumDef:l}})}},2e4):setTimeout(()=>{$||u()},12e4)}async function Oa(t,e){const{state:i,toast:o}=e,{winnerId:n,homeId:r,awayId:a,homeScore:d,awayScore:l,matchId:s}=t,p=i.user.id,c=r===p,y=n===p,{data:x}=await v.from("users").select("mmr, mmr_rd, mmr_v").eq("id",p).single(),{data:m}=await v.from("users").select("mmr, mmr_rd, mmr_v").eq("id",c?a:r).single();if(x&&m){const f=pn(x.mmr,x.mmr_rd,x.mmr_v,m.mmr,m.mmr_rd,y?1:0);await v.from("users").update({mmr:f.mmr,mmr_rd:f.rd,mmr_v:f.v,mmr_wins:y?v.sql`mmr_wins + 1`:void 0,mmr_losses:y?void 0:v.sql`mmr_losses + 1`}).eq("id",p),o(y?`+MMR ↑ ${Math.round(f.mmr-x.mmr)}`:`-MMR ↓ ${Math.round(x.mmr-f.mmr)}`,y?"success":"error",4e3)}}const Ga=Object.freeze(Object.defineProperty({__proto__:null,renderMatchRandom:fo,resumePvpMatch:Ra},Symbol.toStringTag,{value:"Module"}));async function Ha(t,e){const{state:i,navigate:o,toast:n}=e,r=i.params||{},a=r.friendId||null,d=r.friendName||"Ami",l=!!r.isAccepting;await Ui(t,e,"friend",async({deckId:s,formation:p,starters:c,subsRaw:y,gcCardsEnriched:x,gcDefs:m,stadiumDef:f})=>{Pt(t);const h=async _=>{const $=_||[];l?await Ka(t,e,s,$,m,f):await Ua(t,e,s,$,m,f,a,d)};if(!(x!=null&&x.length)){await h([]);return}Ki(t,x,h)})}async function Ua(t,e,i,o,n,r,a,d){var $;const{state:l,navigate:s,toast:p}=e,c=l.user.id,{data:y,error:x}=await v.from("friend_match_invites").insert({inviter_id:c,invitee_id:a,friend_id:a,status:"pending",inviter_deck_id:i,expires_at:new Date(Date.now()+2*60*1e3).toISOString()}).select("id").single();if(x||!y){console.error("[Friend] Erreur création invitation:",x),p("Impossible de créer l'invitation","error"),s("home");return}t.innerHTML=`
    ...
    </div>`;let m=!1,f=null;const h=async()=>{f&&(f.unsubscribe(),f=null)};($=document.getElementById("cancel-friend"))==null||$.addEventListener("click",async()=>{await h(),await v.from("friend_match_invites").update({status:"declined"}).eq("id",y.id),ut(t),s("home")}),f=v.channel(`friend_invite_${y.id}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"friend_match_invites",filter:`id=eq.${y.id}`},async({new:g})=>{m||g.status!=="accepted"||!g.match_id||(m=!0,await h(),await Vt(t,e,g.match_id,!0,{myGC:o,gcDefs:n,stadiumDef:r}))}).subscribe();const _=setInterval(async()=>{if(m)return;const{data:g}=await v.from("friend_match_invites").select("status, match_id").eq("id",y.id).single();(g==null?void 0:g.status)==="accepted"&&g.match_id&&(m=!0,clearInterval(_),await h(),await Vt(t,e,g.match_id,!0,{myGC:o,gcDefs:n,stadiumDef:r}))},3e3);setTimeout(async()=>{m||(clearInterval(_),await h(),p("Invitation expirée","info"),s("home"))},12e4)}async function Ka(t,e,i,o,n,r){const{state:a,navigate:d,toast:l}=e,s=a.user.id,{data:p}=await v.from("friend_match_invites").select("id, inviter_id").eq("invitee_id",s).eq("status","pending").order("created_at",{ascending:!1}).limit(1).maybeSingle();if(!p){l("Aucune invitation en attente","error"),d("home");return}const{data:c,error:y}=await v.rpc("accept_friend_invite",{p_invite_id:p.id,p_invitee_deck_id:i});if(y||!(c!=null&&c.success)){console.error("[Friend] Erreur accept_friend_invite:",y||c),l((c==null?void 0:c.error)||"Impossible de rejoindre le match","error"),d("home");return}await Vt(t,e,c.match_id,!1,{myGC:o,gcDefs:n,stadiumDef:r})}const Gn="#1A6B3C",yt="#D4A017",xi="var(--tile-bg)",yi="var(--tile-border)",Rt="var(--divider)",st="var(--tile-fg-on-page)",Dt="var(--tile-fg-dim)",ct="var(--tile-fg-faint)",hi="var(--nav-fg,#fff)",Hn="rgba(255,255,255,0.62)",Va="rgba(255,255,255,0.4)";async function Wa(t,e){var o;t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const i=(o=e.state.params)==null?void 0:o.openLeagueId;if(i){e.state.params.openLeagueId=null,await ht(t,e,i);return}await pt(t,e)}async function pt(t,e,i="waiting"){var g,L;const{state:o}=e,n=o.profile.id,{data:r}=await v.from("mini_league_members").select("league_id, prize_amount, prize_claimed").eq("user_id",n),a=(r||[]).map(u=>u.league_id),d={};(r||[]).forEach(u=>{d[u.league_id]={amount:u.prize_amount,claimed:u.prize_claimed}});const{data:l,error:s}=await v.from("mini_leagues").select("*, mini_league_members(count)").eq("status","waiting").eq("is_archived",!1).order("created_at",{ascending:!1}).limit(30),p=s?(await v.from("mini_leagues").select("*, mini_league_members(count)").eq("status","waiting").order("created_at",{ascending:!1}).limit(30)).data||[]:l||[],y=(a.length?await v.from("mini_leagues").select("*, mini_league_members(count)").in("id",a).order("created_at",{ascending:!1}):{data:[]}).data||[],x=y.filter(u=>u.status==="waiting"&&!u.is_archived),m=y.filter(u=>u.status==="active"&&!u.is_archived),f=y.filter(u=>u.is_archived||u.status==="finished");let h=f;if(f.length){const{data:u}=await v.from("mini_league_matches").select("league_id").in("league_id",f.map(T=>T.id)).or(`home_id.eq.${n},away_id.eq.${n}`).not("status","eq","bye"),j=new Set((u||[]).map(T=>T.league_id));h=f.filter(T=>j.has(T.id))}const _=p.filter(u=>!a.includes(u.id)),$=[{key:"waiting",label:"🟡 En attente",count:x.length+_.length},{key:"active",label:"🟢 En cours",count:m.length},{key:"archived",label:"📁 Archivées",count:h.length}];t.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    </div></div>`,document.body.appendChild(i),i.querySelector("#ml-form-close").addEventListener("click",()=>i.remove()),i.querySelectorAll('input[name="ml-type"]').forEach(o=>o.addEventListener("change",()=>{document.getElementById("ml-pwd-block").style.display=o.value==="private"?"block":"none"})),i.querySelector("#ml-create-confirm").addEventListener("click",async()=>{var m,f,h,_;const{toast:o}=e,n=document.getElementById("ml-name").value.trim(),r=parseInt(document.getElementById("ml-max").value)||6,a=parseInt(document.getElementById("ml-fee").value)||500,d=((m=i.querySelector('input[name="ml-type"]:checked'))==null?void 0:m.value)||"public",l=((f=i.querySelector('input[name="ml-mode"]:checked'))==null?void 0:f.value)||"aller",s=((_=(h=document.getElementById("ml-pwd"))==null?void 0:h.value)==null?void 0:_.trim())||null;if(!n){o("Le nom est obligatoire","error");return}if(a<100){o("Mise minimum : 100 crédits","error");return}if(d==="private"&&!s){o("Mot de passe requis","error");return}const{data:p,error:c}=await v.from("mini_leagues").insert({name:n,creator_id:e.state.profile.id,type:d,password:s,mode:l,max_players:r,entry_fee:a}).select().single();if(c){o("Erreur : "+c.message,"error");return}const{data:y}=await v.from("users").select("credits").eq("id",e.state.profile.id).single();if(((y==null?void 0:y.credits)||0)<a){await v.from("mini_leagues").delete().eq("id",p.id),o(`Crédits insuffisants pour la mise (${a.toLocaleString("fr")} cr.)`,"error");return}await v.from("users").update({credits:y.credits-a}).eq("id",e.state.profile.id),await v.from("mini_leagues").update({pot:a}).eq("id",p.id),await v.from("mini_league_members").insert({league_id:p.id,user_id:e.state.profile.id}),e.state.profile&&(e.state.profile.credits=y.credits-a);const x=document.getElementById("nav-credits");x&&(x.textContent=`💰 ${(e.state.profile.credits||0).toLocaleString("fr")}`),i.remove(),o(`Mini League créée ! ${a.toLocaleString("fr")} cr. déduits.`,"success"),ht(t,e,p.id)})}function Za(){return new Promise(t=>{const e=document.createElement("div");e.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9500;display:flex;align-items:center;justify-content:center;padding:20px",e.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:${xi};border:1px solid ${yi};border-radius:12px;padding:16px">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>${y.map((b,E)=>{const H=m>0&&a.status==="finished"?E===0?Math.floor(m*.7):E===1?Math.floor(m*.2):E===2?Math.floor(m*.1):0:0;return`<tr style="border-bottom:1px solid ${Rt};${b.userId===r?"background:rgba(74,222,128,0.08);":""}">
  ...
  </div>`;const g=a.status==="waiting"?"waiting":a.status==="active"?"active":"archived";if((L=document.getElementById("ml-back"))==null||L.addEventListener("click",()=>pt(t,e,g)),(u=document.getElementById("ml-refresh"))==null||u.addEventListener("click",async b=>{const E=b.currentTarget;E.style.opacity="0.5",await ht(t,e,i)}),(j=document.getElementById("ml-start-btn"))==null||j.addEventListener("click",()=>is(t,e,a,c)),(T=document.getElementById("ml-next-day"))==null||T.addEventListener("click",()=>os(t,e,i)),(F=document.getElementById("ml-join-now"))==null||F.addEventListener("click",()=>Un(t,e,i,a.type)),(Y=document.getElementById("ml-leave-btn"))==null||Y.addEventListener("click",()=>es(t,e,i,f)),(X=document.getElementById("ml-delete-btn"))==null||X.addEventListener("click",()=>mo(t,e,i,a.name,"waiting")),(M=document.getElementById("ml-delete-now"))==null||M.addEventListener("click",()=>mo(t,e,i,a.name,g)),(w=document.getElementById("ml-archive-btn"))==null||w.addEventListener("click",()=>ts(t,e,i)),t.querySelectorAll("[data-play-match]").forEach(b=>{b.addEventListener("click",()=>{const E=x.find(H=>H.id===b.dataset.playMatch);E&&(wt(),e.navigate("match-mini-league",{mlMatchId:E.id,leagueId:i}))})}),a.status==="finished"&&$){const b=y.findIndex(E=>E.userId===r);b>=0&&b<3&&$.prize_amount>0&&!$.prize_claimed&&setTimeout(()=>ns(t,e,a,$,b),400)}}function on(t,e,i,o,n=!1){const r=c=>e.find(y=>y.id===c);if(t.is_bye){const c=r(t.home_id);return`<div style="padding:8px;border-radius:8px;background:rgba(255,255,255,0.03);margin-bottom:6px;font-size:12px;color:${Dt};text-align:center">🔵 ${(c==null?void 0:c.club_name)||(c==null?void 0:c.pseudo)||"?"} exempté(e)</div>`}const a=r(t.home_id),d=r(t.away_id),l=t.home_id===i||t.away_id===i,s=l&&t.status==="pending"&&o&&!n,p=t.status==="finished"?`${t.home_score} - ${t.away_score}`:"vs";return`<div style="display:flex;align-items:center;gap:8px;padding:10px;border-radius:8px;background:${l?"rgba(26,107,60,0.16)":"rgba(255,255,255,0.03)"};margin-bottom:6px;border:1px solid ${l?"rgba(74,222,128,0.35)":Rt}">
    ...
    <div style="min-height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;background:linear-gradient(135deg,#0a1a2e,#0d3d1e);color:#fff;padding:32px;text-align:center;gap:20px">
    ...
    </div>`;let p=!1;const c=async h=>{if(!p){if(p=!0,clearInterval(m),x)try{x.unsubscribe()}catch{}h&&await Vt(t,e,h,n,{mlLeagueId:o,mlMatchId:i,myGC:r||[],gcDefs:a,stadiumDef:d}),l()}};(f=document.getElementById("wait-back"))==null||f.addEventListener("click",()=>{if(p=!0,clearInterval(m),x)try{x.unsubscribe()}catch{}s("mini-league",{openLeagueId:o}),l()});const y=async()=>{if(p)return;const{data:h}=await v.from("mini_league_matches").select("match_id").eq("id",i).single();h!=null&&h.match_id&&c(h.match_id)},x=v.channel(`ml_match_${i}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"mini_league_matches",filter:`id=eq.${i}`},h=>{var _;(_=h.new)!=null&&_.match_id&&c(h.new.match_id)}).subscribe(),m=setInterval(y,3e3);y()})}const rn={GK:"#111111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function Zt(t,e,i=0){return e?(Number(e==="GK"?t.note_g:e==="DEF"?t.note_d:e==="MIL"?t.note_m:t.note_a)||0)+(e===t.job||e===t.job2?i:0):0}async function ls(t,e){t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',await To(t,e)}async function To(t,e){var j,T,F,Y,X,M;const{state:i,toast:o,navigate:n}=e;li(n,i.profile,"market","/icons/",o);const{data:r}=await v.from("market_listings").select(`id, price, status, listed_at, seller_id,
      ...
      card:cards(id, card_type, current_note, evolution_bonus,
        player:players(id, firstname, surname_real, country_code, job, job2,
          ...
          clubs(encoded_name, logo_url)))`).eq("seller_id",i.profile.id).in("status",["active","sold"]).order("listed_at",{ascending:!1}).limit(100),{data:d}=await v.from("cards").select("player_id").eq("owner_id",i.profile.id).eq("card_type","player").not("player_id","is",null),l=new Set((d||[]).map(w=>w.player_id)),{data:s}=await v.from("decks").select("id").eq("owner_id",i.profile.id),p=(s||[]).map(w=>w.id);let c=new Set;if(p.length){const{data:w}=await v.from("deck_cards").select("card:cards(player_id)").in("deck_id",p);c=new Set((w||[]).map(b=>{var E;return(E=b.card)==null?void 0:E.player_id}).filter(Boolean))}const y=(r||[]).filter(w=>w.seller_id!==i.profile.id),x=a||[];t.innerHTML=`
  <style>
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div id="mkt-content" style="padding:10px 16px;display:flex;flex-direction:column;gap:8px"></div>
      ...
      <div style="display:flex;gap:4px;flex-shrink:0">
        ...
        ${S?`<div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${z};display:flex;align-items:center;justify-content:center">
        ...
        <button id="buy-ok" style="flex:1;padding:12px;border-radius:10px;border:none;background:var(--green);color:#fff;font-size:14px;font-weight:900;cursor:pointer">Confirmer</button>
          ...
          <div style="display:flex;align-items:center;gap:12px;padding:12px;background:var(--tile-bg,rgba(255,255,255,0.05));border-radius:12px;border:1px solid var(--tile-border,rgba(255,255,255,0.1));${H?"border:2px solid var(--yellow)":""}">
            ...
            <div style="flex:1;min-width:0">
              ...
              <div style="font-size:11px;color:var(--tile-fg-dim)">${F.club_name}${S?"":` · ${E}% WR`}</div>
            </div>
            <div style="text-align:right;flex-shrink:0">
              ...
              ${S?"":`<div style="font-size:11px;color:var(--tile-fg-dim);margin-top:1px">${(F.mmr??450).toLocaleString("fr")} MMR</div>`}
            </div>
          </div>`}).join(""):'<div style="text-align:center;color:var(--tile-fg-dim);padding:40px">Aucun joueur classé.</div>'}else if(l==="mini-league"){const T=a||[];g.innerHTML=T.length>0?T.map((F,Y)=>`
        <div style="display:flex;align-items:center;gap:12px;padding:12px;background:var(--tile-bg,rgba(255,255,255,0.05));border-radius:12px;border:1px solid var(--tile-border,rgba(255,255,255,0.1));${F.user_id===i.profile.id?"border:2px solid var(--yellow)":""}">
          ...
          <div style="flex:1;min-width:0">
            <div style="font-weight:700;color:var(--tile-fg-on-page);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${F.pseudo}</div>
            <div style="font-size:11px;color:var(--tile-fg-dim)">${F.club_name}</div>
          </div>
          <div style="text-align:right;font-size:12px;flex-shrink:0">
            <div style="color:var(--tile-fg-on-page)">🥇${F.top1} 🥈${F.top2} 🥉${F.top3}</div>
            <div style="color:var(--tile-fg-dim)">${F.ml_wins} match(s) gagné(s)</div>
          </div>
        </div>
      `).join(""):`<div style="text-align:center;color:var(--tile-fg-dim);padding:40px">Aucun résultat de Mini League pour l'instant.</div>`}else{const T=d||[];g.innerHTML=T.length>0?T.map((F,Y)=>`
        <div style="display:flex;align-items:center;gap:12px;padding:12px;background:var(--tile-bg,rgba(255,255,255,0.05));border-radius:12px;border:1px solid var(--tile-border,rgba(255,255,255,0.1));${F.user_id===i.profile.id?"border:2px solid var(--yellow)":""}">
          <div style="width:32px;height:32px;border-radius:50%;background:${s(Y)};color:${p(Y)};display:flex;align-items:center;justify-content:center;font-weight:900;flex-shrink:0;font-size:${Y<3?"16":"13"}px">${Y<3?["🥇","🥈","🥉"][Y]:Y+1}</div>
          <div style="flex:1;min-width:0">
            <div style="font-weight:700;color:var(--tile-fg-on-page);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${F.pseudo}</div>
          ...
          <div style="text-align:right;flex-shrink:0">
            <div style="font-size:16px;font-weight:900;color:#D4A017" title="Score de classement — tient compte du nombre de matchs joués">📊 ${F.wilson_score}</div>
            <div style="font-size:10px;color:var(--tile-fg-dim)">${F.wins}/${F.total} V</div>
            <div style="font-size:9px;color:var(--green);font-weight:700;margin-top:2px">${F.win_pct}%</div>
          </div>
        </div>
      `).join(""):`<div style="text-align:center;color:var(--tile-fg-dim);padding:40px">Aucun match random joué pour l'instant.</div>`}(L=document.getElementById("tab-ranked"))!=null&&L.style&&(document.getElementById("tab-ranked").style.cssText=y(l==="ranked")),(u=document.getElementById("tab-mini-league"))!=null&&u.style&&(document.getElementById("tab-mini-league").style.cssText=y(l==="mini-league")),(j=document.getElementById("tab-random"))!=null&&j.style&&(document.getElementById("tab-random").style.cssText=y(l==="random"))}}const y=$=>`flex:1;padding:10px 4px;border:none;border-radius:10px;cursor:pointer;font-size:12px;font-weight:${$?"900":"400"};background:${$?"var(--green)":"rgba(255,255,255,0.06)"};color:${$?"#fff":"var(--tile-fg-dim)"};transition:all 0.2s`;t.innerHTML=`
  <div class="page">
    <div class="page-header">
      <h2>📊 Classements</h2>
    </div>
    ...
    <div style="min-height:100%;background:linear-gradient(160deg,#1a1a1a,#2a2a2a);padding:16px;overflow-y:auto;display:flex;flex-direction:column;gap:16px">
      ...
      <div>
        ...
        <div style="display:flex;flex-direction:column;gap:6px;max-height:320px;overflow-y:auto">
          ${(G||[]).map((I,z)=>{const Z=zt(I.mmr??450),U=I.id===d.id;return`<div style="display:flex;align-items:center;gap:10px;padding:8px 12px;border-radius:10px;
              background:${U?"rgba(212,160,23,0.15)":"rgba(0,0,0,0.25)"};
              border:1px solid ${U?"#D4A017":"transparent"}">
              <div style="width:26px;text-align:center;font-size:12px;font-weight:900;color:rgba(255,255,255,0.5)">#${z+1}</div>
    ...
    </div>`,(Y=document.getElementById("ranked-back"))==null||Y.addEventListener("click",()=>o("home"));return}const l=d.mmr??450,s=d.mmr_deviation??350,p=d.mmr_volatility??.06,c=d.placement_matches??0,y=c<10,x=Math.max(0,10-c),m=zt(l),f=oo(l),h=Bi(l,m),_=m.label+(h?" "+h:""),$=Fo.findIndex(b=>b.id===m.id),g=Fo[$+1]||null,L=m.id==="master"?"master":`${m.id}_${h}`,u={bronze:"linear-gradient(160deg,#3d1c00,#7a3e00)",silver:"linear-gradient(160deg,#1a1a2e,#3a3a5e)",gold:"linear-gradient(160deg,#1a1200,#4a3500)",platinum:"linear-gradient(160deg,#001a20,#003040)",diamond:"linear-gradient(160deg,#001030,#1a2860)",master:"linear-gradient(160deg,#1a0030,#3d0070)"},j=(d.ranked_wins||0)+(d.ranked_losses||0)+(d.ranked_draws||0),T=j>0?Math.round((d.ranked_wins||0)/j*100):0,F=Zn.map(b=>{const E=b.key===L,H=b.division==="III"||b.division===null;return`
