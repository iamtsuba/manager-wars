import{s as I,F as Nt,h as yt,j as yi,l as Je,i as Oi,k as Pi,b as Hi}from"./formation-links-CEzN0rcz.js";function ai(e,{navigate:t,toast:i}){e.innerHTML=`
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),o=(s||[]).filter(z=>z.card_type==="player"&&z.player),n=(s||[]).filter(z=>z.card_type==="game_changer"),u=(s||[]).filter(z=>z.card_type==="formation"),{data:f}=await I.from("gc_definitions").select("name,gc_type,color,effect,image_url"),g={};(f||[]).forEach(z=>{g[z.name]=z});const y=Object.keys(Nt),d=Object.keys(Ce),x={};o.forEach(z=>{const S=z.player.id;x[S]=(x[S]||0)+1}),new Set(Object.keys(x).map(z=>String(z)));const v=new Set(u.map(z=>z.formation)),_=new Set(n.map(z=>z.gc_type));let M="player",G="Tous",k="",w=!1;function F(){return[...o].sort((z,S)=>{const R=ht.indexOf(z.player.job),C=ht.indexOf(S.player.job);return R!==C?R-C:(z.player.surname_encoded||"").localeCompare(S.player.surname_encoded||"")})}function Z(){return[...p||[]].sort((z,S)=>{const R=ht.indexOf(z.job),C=ht.indexOf(S.job);return R!==C?R-C:(z.surname_encoded||"").localeCompare(S.surname_encoded||"")})}function ne(){return F().filter(z=>{const S=z.player,R=G==="Tous"||S.job===G,C=!k||`${S.firstname} ${S.surname_encoded}`.toLowerCase().includes(k);return R&&C})}function ee(){return Z().filter(z=>{const S=G==="Tous"||z.job===G,R=!k||`${z.firstname} ${z.surname_encoded}`.toLowerCase().includes(k);return S&&R})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
  ...
  </div>`;function $(){const z=document.getElementById("col-filters");z&&(M==="player"?(z.innerHTML=`
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",S=>{k=S.target.value.toLowerCase(),q()}),e.querySelectorAll(".filter-btn").forEach(S=>{S.addEventListener("click",()=>{G=S.dataset.job,$(),q()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{w=!w,$(),q()})):(z.innerHTML=`
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const dt=1e3;function eo(e,t,i,r){var d,x,v;const{state:c,toast:l,closeModal:a,navigate:s,refreshProfile:p}=i,o=e.formation,n={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function u(){const _=yt[o]||{},M=Nt[o]||[],G=290,k=360,w=20;function F(ne){const ee=_[ne];return ee?{x:ee.x*G,y:ee.y*k}:null}let Z=`<svg width="${G}" height="${k}" viewBox="0 0 ${G} ${k}" xmlns="http://www.w3.org/2000/svg">`;for(const[ne,ee]of M){const $=F(ne),q=F(ee);!$||!q||(Z+=`<line x1="${$.x}" y1="${$.y}" x2="${q.x}" y2="${q.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const ne of Object.keys(_)){const ee=F(ne);if(!ee)continue;const $=ne.replace(/\d+/,""),q=n[$]||"#555";Z+=`<circle cx="${ee.x}" cy="${ee.y}" r="${w}" fill="${q}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,Z+=`<text x="${ee.x}" y="${ee.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${$}</text>`}return Z+="</svg>",Z}const f=t.filter(_=>_.formation===o),g=f.length,y=!e.is_for_sale;r(`Formation ${o}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(d=document.getElementById("direct-sell-form-btn"))==null||d.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${o} pour ${dt.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const _=f.find(G=>!G.is_for_sale)||f[0];if(!_){l("Aucune carte à vendre","error");return}const{error:M}=await I.from("cards").delete().eq("id",_.id);if(M){l(M.message,"error");return}await I.from("users").update({credits:(c.profile.credits||0)+dt}).eq("id",c.profile.id),await p(),l(`+${dt.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),a(),s("collection")}),(x=document.getElementById("market-sell-form-btn"))==null||x.addEventListener("click",async()=>{const _=parseInt(document.getElementById("sell-price-form").value);if(!_||_<1){l("Prix invalide","error");return}await I.from("cards").update({is_for_sale:!0,sale_price:_}).eq("id",e.id),await I.from("market_listings").insert({seller_id:c.profile.id,card_id:e.id,price:_}),l("Carte mise en vente sur le marché !","success"),a(),s("collection")}),(v=document.getElementById("cancel-sell-form-btn"))==null||v.addEventListener("click",async()=>{await I.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await I.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),l("Annonce retirée","success"),a(),s("collection")})}async function ci(e,t,i,r){var Z,ne,ee,$,q,V;const{state:c,toast:l,openModal:a,closeModal:s,navigate:p,refreshProfile:o}=r,n=e.player,u=t.filter(h=>h.player.id===n.id),f=u.length,g=Ji[n.rarity]||1e3,y=n.rarity!=="legende",d=wi(n),x=gt(n,n.job),v=n.job2?gt(n,n.job2):null,_=$t[n.job]||"#1A6B3C",M=n.job2?$t[n.job2]:null,G=vi[n.rarity]||"#ccc",k=Ut[n.country_code]||n.country_code||"",{data:w}=await I.from("transfer_history").select("club_name, manager_name, source, price, transferred_at").eq("player_id",n.id).order("transferred_at",{ascending:!0}),F=w&&w.length?`
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
  ...
  </div>`,io(e,t,c,i),document.getElementById("builder-back").addEventListener("click",()=>r("decks")),document.getElementById("formation-select").addEventListener("change",o=>{t.formation=o.target.value;const n=ui(t.formation),u={};n.forEach(f=>{t.slots[f]&&(u[f]=t.slots[f])}),t.slots=u,ot(e,t,i)}),document.getElementById("save-deck").addEventListener("click",()=>ro(t,i)),e.querySelectorAll("[data-remove-sub]").forEach(o=>{o.addEventListener("click",()=>{t.subs=t.subs.filter(n=>n!==o.dataset.removeSub),ot(e,t,i)})}),(p=document.getElementById("add-sub-btn"))==null||p.addEventListener("click",()=>{no(t,e,i)})}function io(e,t,i,r){const c=e.querySelector("#deck-field");if(!c)return;const l=yt[t.formation]||{},a=yi(t.formation)||[],s={};for(const v of i){const _=t.slots[v],M=_?t.playerCards.find(G=>G.id===_):null;s[v]=M?M.player:null}const p=300,o=300,n=48,u=64,f=13,g=16,y=38;function d(v){const _=l[v];return _?{x:_.x*p,y:_.y*o}:null}let x="";for(const[v,_]of a){const M=d(v),G=d(_);if(!M||!G)continue;const k=s[v]?{...s[v],club_id:s[v].club_id,country_code:s[v].country_code,rarity:s[v].rarity}:null,w=s[_]?{...s[_],club_id:s[_].club_id,country_code:s[_].country_code,rarity:s[_].rarity}:null,F=Je(k,w);F==="#ff3333"||F==="#cc2222"?x+=`<line x1="${M.x.toFixed(1)}" y1="${M.y.toFixed(1)}" x2="${G.x.toFixed(1)}" y2="${G.y.toFixed(1)}" stroke="${F}" stroke-width="2.5" stroke-linecap="round" opacity="0.4"/>`:(x+=`<line x1="${M.x.toFixed(1)}" y1="${M.y.toFixed(1)}" x2="${G.x.toFixed(1)}" y2="${G.y.toFixed(1)}" stroke="${F}" stroke-width="8" stroke-linecap="round" opacity="0.2"/>`,x+=`<line x1="${M.x.toFixed(1)}" y1="${M.y.toFixed(1)}" x2="${G.x.toFixed(1)}" y2="${G.y.toFixed(1)}" stroke="${F}" stroke-width="2.5" stroke-linecap="round" opacity="0.9"/>`)}for(const v of i){const _=d(v);if(!_)continue;const M=s[v],G=v.replace(/\d+/,""),k=We[G]||"#555",w=(_.x-n/2).toFixed(1),F=(_.y-u/2).toFixed(1),Z={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[M==null?void 0:M.rarity]||"#aaa";if(M){const ne=Tt(M),ee=_i(M),$=$i(M.country_code),q=Number(G==="GK"?M.note_g:G==="DEF"?M.note_d:G==="MIL"?M.note_m:M.note_a)||0,V=u-f-g;x+=`<defs><clipPath id="dcp-${v}"><rect x="${w}" y="${(_.y-u/2+f).toFixed(1)}" width="${n}" height="${V}"/></clipPath></defs>`,x+=`<rect x="${w}" y="${F}" width="${n}" height="${u}" rx="5" fill="${k}"/>`,ne&&(x+=`<image href="${ne}" x="${w}" y="${(_.y-u/2+f).toFixed(1)}" width="${n}" height="${V}" clip-path="url(#dcp-${v})" preserveAspectRatio="xMidYMin slice"/>`),x+=`<rect x="${w}" y="${F}" width="${n}" height="${f}" fill="rgba(255,255,255,0.93)"/>`,x+=`<text x="${_.x.toFixed(1)}" y="${(_.y-u/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(M.surname_encoded||"").slice(0,9)}</text>`;const h=(_.y+u/2-g).toFixed(1);x+=`<rect x="${w}" y="${h}" width="${n}" height="${g}" fill="rgba(255,255,255,0.93)"/>`,$&&(x+=`<image href="${$}" x="${(_.x-21).toFixed(1)}" y="${(_.y+u/2-g+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`),x+=`<text x="${_.x.toFixed(1)}" y="${(_.y+u/2-g/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${q}</text>`,ee&&(x+=`<image href="${ee}" x="${(_.x+n/2-14).toFixed(1)}" y="${(_.y+u/2-g+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`),x+=`<rect x="${w}" y="${F}" width="${n}" height="${u}" rx="5" fill="none" stroke="${Z}" stroke-width="2"/>`}else x+=`<rect x="${w}" y="${F}" width="${n}" height="${u}" rx="5" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="5,3"/>`,x+=`<text x="${_.x.toFixed(1)}" y="${_.y.toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="22" fill="rgba(255,255,255,0.35)">+</text>`,x+=`<text x="${_.x.toFixed(1)}" y="${(_.y+16).toFixed(1)}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.3)">${G}</text>`;x+=`<rect x="${w}" y="${F}" width="${n}" height="${u}" rx="5" fill="rgba(0,0,0,0.01)" class="deck-slot-hit" data-pos="${v}" style="cursor:pointer"/>`}c.innerHTML=`<svg viewBox="${-y} ${-y} ${p+y*2} ${o+y*2}" width="100%" style="display:block;max-width:440px;margin:0 auto">${x}</svg>`,c.querySelectorAll(".deck-slot-hit").forEach(v=>{v.addEventListener("click",()=>oo(v.dataset.pos,t,e,r))})}function oo(e,t,i,r){var f,g,y;const{openModal:c,closeModal:l}=r,a=e.replace(/\d+/,""),s=t.slots[e],p=s?t.playerCards.find(d=>d.id===s):null;(f=p==null?void 0:p.player)==null||f.id;const o=new Set;Object.entries(t.slots).forEach(([d,x])=>{var _;if(d===e||!x)return;const v=t.playerCards.find(M=>M.id===x);(_=v==null?void 0:v.player)!=null&&_.id&&o.add(v.player.id)}),t.subs.forEach(d=>{var v;const x=t.playerCards.find(_=>_.id===d);(v=x==null?void 0:x.player)!=null&&v.id&&o.add(x.player.id)});const n=new Set,u=t.playerCards.filter(d=>{const x=d.player;return!(x.job===a||x.job2===a)||o.has(x.id)||n.has(x.id)?!1:(n.add(x.id),!0)});u.sort((d,x)=>{const v=a==="GK"?d.player.note_g:a==="DEF"?d.player.note_d:a==="MIL"?d.player.note_m:d.player.note_a;return(a==="GK"?x.player.note_g:a==="DEF"?x.player.note_d:a==="MIL"?x.player.note_m:x.player.note_a)-v}),c(`Choisir ${a} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${u.length>0?u.map(d=>{var G,k;const x=d.player,v=a==="GK"?x.note_g:a==="DEF"?x.note_d:a==="MIL"?x.note_m:x.note_a,_=Tt(x),M={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[x.rarity];return`<div class="player-option" data-card-id="${d.id}"
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    </div>`,e.querySelectorAll(".gc-select-card").forEach(y=>{y.addEventListener("click",()=>{const d=y.dataset.id,x=l.find(_=>_.id===d);if(!x)return;const v=a.findIndex(_=>_.gc_type===x.gc_type);v>-1?a.splice(v,1):a.length<3&&a.push(x),o()})}),(u=e.querySelector("#gc-launch"))==null||u.addEventListener("click",()=>{n&&p(a)}),(f=e.querySelector("#gc-no-gc"))==null||f.addEventListener("click",()=>p([])),(g=e.querySelector("#gc-reset"))==null||g.addEventListener("click",()=>{a.length&&(a=[],o())})}o()}async function Ao(e,t,i){const{state:r,navigate:c}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:l}=await I.from("decks").select("id,name,is_active,formation").eq("owner_id",r.profile.id).order("created_at",{ascending:!1});if(!l||l.length===0){Xe(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>c("decks"));return}const a=l.map(n=>n.id),{data:s}=await I.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order");let p=0;function o(){var _,M,G,k,w;const n=l[p],u=(s||[]).filter(F=>F.deck_id===n.id),f=u.filter(F=>{var Z;return F.is_starter&&((Z=F.card)==null?void 0:Z.player)}).map(F=>Ft(F.card,F.position)),g=u.find(F=>{var Z;return((Z=F.card)==null?void 0:Z.card_type)==="formation"}),y=n.formation||((_=g==null?void 0:g.card)==null?void 0:_.formation)||"4-4-2",d=f.length>=11?kt(f,y):null,x=f.length>=11;Ye(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    ...
    </div>`,function(){const Z=e.querySelector(".deck-preview-wrap svg");Z&&(Z.removeAttribute("width"),Z.removeAttribute("height"),Z.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",Z.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),(M=document.getElementById("prev-deck"))==null||M.addEventListener("click",()=>{p>0&&(p--,o())}),(G=document.getElementById("next-deck"))==null||G.addEventListener("click",()=>{p<l.length-1&&(p++,o())}),(k=document.getElementById("validate-deck"))==null||k.addEventListener("click",()=>{x&&t.navigate("match",{matchMode:i,deckId:n.id})}),(w=document.getElementById("cancel-deck-select"))==null||w.addEventListener("click",()=>{Re(e),c("home")});const v=document.getElementById("deck-swipe-zone");if(v){let F=0,Z=0;v.addEventListener("touchstart",ne=>{F=ne.touches[0].clientX,Z=ne.touches[0].clientY},{passive:!0}),v.addEventListener("touchend",ne=>{const ee=ne.changedTouches[0].clientX-F,$=ne.changedTouches[0].clientY-Z;Math.abs(ee)<40||Math.abs(ee)<Math.abs($)||(ee<0&&p<l.length-1?(p++,o()):ee>0&&p>0&&(p--,o()))},{passive:!0})}}o()}function ji(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";return e!=null&&e.clubLogo?e.clubLogo.startsWith("http")?e.clubLogo:`${t}/storage/v1/object/public/assets/clubs/${e.clubLogo}`:null}function Be(e,t=44,i=58){if(!e)return`<div style="width:${t}px;height:${i}px;border:1.5px dashed rgba(255,255,255,0.2);border-radius:5px"></div>`;const r=typeof e.portrait=="string"&&e.portrait.startsWith("http")?e.portrait:Te(e),c=ji(e),l=e._line||e.job||"MIL",a=rt[l]||"#555",s={normal:"#aaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#222",legende:"#7a28b8",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",p=e.note!==void 0?Number(e.note)||0:(Number(me(e,l))||0)+(e.boost||0),o=Bi(e==null?void 0:e.country_code),n=Math.round(i*.19),u=Math.round(i*.25),f=i-Math.round(i*.19)-Math.round(i*.25),g=e!=null&&e.used?.28:1;return`<div style="width:${t}px;height:${i}px;border-radius:5px;border:2px solid ${s};background:${a};position:relative;overflow:hidden;flex-shrink:0;opacity:${g}">
    <div style="position:absolute;top:0;left:0;right:0;height:${n}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:center;z-index:2">
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;height:${u}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ...
      ${c?`<img src="${c}" style="width:${u-4}px;height:${u-4}px;object-fit:contain">`:e!=null&&e.clubName?`<span style="font-size:${Math.max(4,u-8)}px;font-weight:700;color:#333">${(e.clubName||"").slice(0,3).toUpperCase()}</span>`:""}
        ...
        stroke="${ee}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const g=48,y=64,d=13,x=16,v={normal:"#aaaaaa",pepite:"#D4A017",pépite:"#D4A017",papyte:"#111111",legende:"#7a28b8",légende:"#7a28b8"};for(const[G,k]of Object.entries(o)){const w=u(G);if(!w||!k)continue;const F=G.replace(/[0-9]/g,""),Z=rt[F]||"#555",ne=a.includes(k.cardId),ee=i==="attack"&&(["MIL","ATT"].includes(F)||ne)&&!k.used||i==="defense"&&["GK","DEF","MIL"].includes(F)&&!k.used,$=r.includes(k.cardId);let q;i==="attack"?q=ne?Math.max(1,Number(k.note_a)||0):F==="MIL"?Number(k.note_m)||0:Number(k.note_a)||0:i==="defense"?q=F==="GK"?Number(k.note_g)||0:F==="MIL"?Number(k.note_m)||0:Number(k.note_d)||0:q=Number(F==="GK"?k.note_g:F==="DEF"?k.note_d:F==="MIL"?k.note_m:k.note_a)||0,q=q+(k.boost||0);const V=(w.x-g/2).toFixed(1),h=(w.y-y/2).toFixed(1),U=v[k==null?void 0:k.rarity]||v.normal;if(k.used){const S=typeof import.meta<"u"&&"/manager-wars/"||"/",C=`${typeof window<"u"&&((M=window.location)==null?void 0:M.origin)||""}${S}icons/carte-dos.png`.replace(/([^:])\/\//g,"$1/");f+=`<rect x="${V}" y="${h}" width="${g}" height="${y}" rx="5" fill="#161616"/>`,f+=`<image href="${C}" xlink:href="${C}" x="${V}" y="${h}" width="${g}" height="${y}" preserveAspectRatio="xMidYMid slice"/>`,f+=`<rect x="${V}" y="${h}" width="${g}" height="${y}" rx="5" fill="none" stroke="${U}" stroke-width="2" opacity="0.7"/>`,f+=`<rect x="${V}" y="${h}" width="${g}" height="${y}" rx="5" fill="rgba(0,0,0,0.01)" class="match-used-hit" data-card-id="${k.cardId}" data-role="${F}" style="cursor:pointer"/>`;continue}const W=$?.45:1,re=$?"#FFD700":U,ce=$?3:(k==null?void 0:k.rarity)==="legende"||(k==null?void 0:k.rarity)==="légende"||(k==null?void 0:k.rarity)==="pepite"||(k==null?void 0:k.rarity)==="pépite"?2.5:2,se=y-d-x;f+=`<defs><clipPath id="cp-${G}"><rect x="${V}" y="${(w.y-y/2+d).toFixed(1)}" width="${g}" height="${se}"/></clipPath></defs>`,f+=`<rect x="${V}" y="${h}" width="${g}" height="${y}" rx="5" fill="${Z}" opacity="${W}"/>`;const ae=Te(k);ae&&(f+=`<image href="${ae}" xlink:href="${ae}" x="${V}" y="${(w.y-y/2+d).toFixed(1)}" width="${g}" height="${se}" clip-path="url(#cp-${G})" preserveAspectRatio="xMidYMin slice"/>`),f+=`<rect x="${V}" y="${h}" width="${g}" height="${d}" rx="4" fill="rgba(255,255,255,0.92)"/>`,f+=`<text x="${w.x.toFixed(1)}" y="${(w.y-y/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(k.name||"").slice(0,9)}</text>`;const z=(w.y+y/2-x).toFixed(1);f+=`<rect x="${V}" y="${z}" width="${g}" height="${x}" fill="rgba(255,255,255,0.92)"/>`;{const S=Dt(k.country_code);S?f+=`<image href="${S}" xlink:href="${S}" x="${(w.x-20).toFixed(1)}" y="${(w.y+y/2-x+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:f+=`<text x="${(w.x-13).toFixed(1)}" y="${(w.y+y/2-x/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${Bi(k.country_code)}</text>`,f+=`<text x="${w.x.toFixed(1)}" y="${(w.y+y/2-x/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${q}</text>`;const R=ji(k);R?f+=`<image href="${R}" xlink:href="${R}" x="${(w.x+g/2-14).toFixed(1)}" y="${(w.y+y/2-x+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:k.clubName&&(f+=`<text x="${(w.x+14).toFixed(1)}" y="${(w.y+y/2-x/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(k.clubName||"").slice(0,3).toUpperCase()}</text>`),k.boost&&(f+=`<rect x="${(w.x+g/2-12).toFixed(1)}" y="${(w.y-y/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,f+=`<text x="${(w.x+g/2-5).toFixed(1)}" y="${(w.y-y/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${k.boost}</text>`)}f+=`<rect x="${V}" y="${h}" width="${g}" height="${y}" rx="5" fill="${$?"rgba(255,255,255,0.12)":"none"}" stroke="${re}" stroke-width="${ce}" opacity="${W}"/>`,ee&&(f+=`<rect x="${V}" y="${h}" width="${g}" height="${y}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${$?"selected":""}" data-card-id="${k.cardId}" data-role="${F}" style="cursor:pointer"/>`)}const _=38;return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-_} ${-_} ${c+_*2} ${l+_*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${f}
  ...
  </div>`}async function Fi(e,t,i,r){var k;const{state:c,navigate:l,toast:a}=t;Ye(e);const s=c.params||{};if(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!s.deckId)return Ao(e,t,i);const p=s.deckId;let o,n,u,f;try{const w=await Promise.all([I.from("decks").select("formation,name").eq("id",p).single(),I.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation,
            player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
              clubs(encoded_name,logo_url)))`).eq("deck_id",p).order("slot_order")]);o=w[0].data,u=w[0].error,n=w[1].data,f=w[1].error}catch(w){console.error("[Match] Exception chargement deck:",w),Xe(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>l("home"));return}if(u||f){console.error("[Match] Erreur Supabase:",u||f),Xe(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>l("home"));return}const g=(n||[]).filter(w=>{var F;return w.is_starter&&((F=w.card)==null?void 0:F.player)}).map(w=>Ft(w.card,w.position)),y=(n||[]).filter(w=>{var F;return!w.is_starter&&((F=w.card)==null?void 0:F.player)}).map(w=>Ft(w.card,w.position));if(g.length<11){Xe(e,"⚠️",`Deck incomplet (${g.length}/11).`,"Compléter",()=>l("decks"));return}const d=(n||[]).find(w=>{var F;return((F=w.card)==null?void 0:F.card_type)==="formation"}),x=(o==null?void 0:o.formation)||((k=d==null?void 0:d.card)==null?void 0:k.formation)||"4-4-2",{data:v,error:_}=await I.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",c.profile.id).eq("card_type","game_changer"),{data:M}=await I.from("gc_definitions").select("*").eq("is_active",!0),G=(v||[]).map(w=>({...w,_gcDef:(M==null?void 0:M.find(F=>F.name===w.gc_type||F.id===w.gc_definition_id))||null}));r({deckId:p,formation:x,starters:g,subsRaw:y,gcCardsEnriched:G,gcDefs:M||[]})}async function To(e,t){const{state:i}=t,c=(i.params||{}).matchMode||"vs_ai_easy",l=c.replace("vs_ai_",""),a=c;await Fi(e,t,c,async({deckId:s,formation:p,starters:o,subsRaw:n,gcCardsEnriched:u,gcDefs:f})=>{try{const g=kt(o,p),y=await Io(p,l),d=async x=>{try{const{data:v,error:_}=await I.from("matches").insert({home_id:i.profile.id,away_id:null,mode:a,home_deck_id:s,status:"in_progress"}).select().single();if(_){console.error("[MatchIA] Erreur création match:",_),Xe(e,"⚠️","Impossible de créer le match ("+_.message+").","Retour",()=>t.navigate("home"));return}const M={gcDefs:f||[],matchId:v==null?void 0:v.id,mode:a,difficulty:l,formation:p,homeTeam:g,aiTeam:y,homeSubs:n,subsUsed:0,maxSubs:Math.min(n.length,3),homeScore:0,aiScore:0,gcCards:x,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Mo(e,M,t)}catch(v){console.error("[MatchIA] Exception launchMatch:",v),Xe(e,"⚠️","Erreur au lancement du match : "+v.message,"Retour",()=>t.navigate("home"))}};if(!u.length){d([]);return}Ci(e,u,d)}catch(g){console.error("[MatchIA] Exception setup:",g),Xe(e,"⚠️","Erreur de préparation du match : "+g.message,"Retour",()=>t.navigate("home"))}})}async function Io(e,t){var a,s;const{data:i}=await I.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(60);if(!i||i.length<11)return Lo(e);const r=nt[e]||nt["4-4-2"],c={GK:[],DEF:[],MIL:[],ATT:[]},l=[...i];for(const p of["GK","DEF","MIL","ATT"]){const o=l.filter(y=>y.job===p),n=l.filter(y=>y.job!==p),u=[...o,...n],f=[];for(let y=0;y<r[p];y++){const d=u[y]||l[y];d&&f.push({cardId:"ai-"+d.id+"-"+y,id:d.id,firstname:d.firstname,name:d.surname_encoded,country_code:d.country_code,club_id:d.club_id,job:d.job,job2:d.job2,note_g:Number(d.note_g)||0,note_d:Number(d.note_d)||0,note_m:Number(d.note_m)||0,note_a:Number(d.note_a)||0,rarity:d.rarity,skin:d.skin,hair:d.hair,hair_length:d.hair_length,clubName:((a=d.clubs)==null?void 0:a.encoded_name)||null,clubLogo:((s=d.clubs)==null?void 0:s.logo_url)||null,boost:0,used:!1,_line:p})}const g=at(f.length);f.forEach((y,d)=>{y._col=g[d]}),c[p]=f}return c}function Lo(e){const t=nt[e]||nt["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},r=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let c=0;for(const l of["GK","DEF","MIL","ATT"]){const a=[];for(let p=0;p<t[l];p++){const o=3+Math.floor(Math.random()*5);a.push({cardId:"fake-"+c,id:"fake-"+c,firstname:"IA",name:r[c%r.length],country_code:"XX",club_id:null,job:l,job2:null,note_g:l==="GK"?o:2,note_d:l==="DEF"?o:2,note_m:l==="MIL"?o:2,note_a:l==="ATT"?o:2,rarity:"normal",boost:0,used:!1,_line:l}),c++}const s=at(a.length);a.forEach((p,o)=>{p._col=s[o]}),i[l]=a}return i}function Mo(e,t,i){e.innerHTML=`
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
  </div>`,setTimeout(()=>zo(e,t,i),5e3)}function zo(e,t,i){const r=t.homeTeam.MIL||[],c=t.aiTeam.MIL||[];function l(d){return d.reduce((x,v)=>x+me(v,"MIL"),0)}function a(d){let x=0;for(let v=0;v<d.length-1;v++){const _=Je(d[v],d[v+1]);_==="#00ff88"?x+=2:_==="#FFD700"&&(x+=1)}return x}const s=l(r)+a(r),p=l(c)+a(c),o=s>=p;function n(d,x,v,_){return`<div id="duel-row-${_}" style="text-align:center;width:100%;transition:transform .5s cubic-bezier(.5,0,.75,0), opacity .5s ease;transform-origin:center">
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${d.map((M,G)=>{const k=G<d.length-1?Je(M,d[G+1]):null,w=!k||k==="#ff3333"||k==="#cc2222",F=k==="#00ff88"?"+2":k==="#FFD700"?"+1":"";return`
          ...
          ${G<d.length-1?`<div class="duel-link duel-link-${_}" data-idx="${G}" style="position:relative;width:18px;height:5px;border-radius:3px;background:${w?"rgba(255,255,255,0.12)":k};flex-shrink:0;opacity:0;transition:opacity .3s ease;box-shadow:${w?"none":`0 0 8px ${k}`}">
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map($=>{var q,V,h;if($.type==="duel"){const U=$.isGoal,W=$.homeScored?"#FFD700":U?"#ff6b6b":"rgba(255,255,255,0.3)",re=$.homeScored?"⚽ BUT !":U?"⚽ BUT IA !":(q=$.homePlayers)!=null&&q.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${U?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${W};margin-bottom:4px">
                <div style="font-size:9px;color:${W};letter-spacing:1px;margin-bottom:5px;font-weight:700;text-transform:uppercase">${re}</div>
                ${(V=$.homePlayers)!=null&&V.length?`<div style="margin-bottom:3px">${et($.homePlayers,"rgba(255,255,255,0.7)",$.homeTotal)}</div>`:""}
                ${(h=$.aiPlayers)!=null&&h.length?`<div style="opacity:0.7">${et($.aiPlayers,"#ff6b6b",$.aiTotal)}</div>`:""}
  ...
  `,document.body.appendChild(r);let n=!1;const u=()=>{n||(n=!0,r.remove(),i())};r.addEventListener("click",u),setTimeout(u,2e3)}function ct(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function zt(e,t,i,r=null,c=null){var g,y;if(t.phase!=="attack"){ct("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){ct(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const l=Object.entries(t.homeTeam).flatMap(([d,x])=>(x||[]).filter(v=>v.used).map(v=>({...v,_line:v._line||d}))),a=t.homeSubs.filter(d=>!t.usedSubIds.includes(d.cardId));if(!l.length){ct("Aucun joueur utilisé à remplacer");return}if(!a.length){ct("Aucun remplaçant disponible");return}let s=Math.max(0,l.findIndex(d=>d.cardId===c));const p=((g=l[s])==null?void 0:g._line)||((y=l[s])==null?void 0:y.job);let o=r?Math.max(0,a.findIndex(d=>d.cardId===r)):Math.max(0,a.findIndex(d=>d.job===p)),n=!1;const u=document.createElement("div");u.id="sub-overlay",u.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function f(){var k,w,F,Z,ne,ee;const d=l[s],x=a[o],v=Math.min(130,Math.round((window.innerWidth-90)/2)),_=Math.round(v*1.35),M=$=>`background:rgba(255,255,255,0.12);border:none;color:${$?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${$?"default":"pointer"};flex-shrink:0`;u.innerHTML=`
    ...
    <div style="flex:1;display:flex;gap:0;overflow:hidden">
      ...
      <div id="in-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
        ...
        <div>${x?Be({...x,used:!1,boost:0},v,_):"<div>—</div>"}</div>
      ...
      <div id="out-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
        ...
        <div>${d?Be({...d,used:!1,boost:0},v,_):"<div>—</div>"}</div>
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(l);let s=!1;const p=()=>{s||(s=!0,l.remove(),setTimeout(()=>c(),50))};l.addEventListener("click",p),setTimeout(p,3500)}async function ft(e,t,i){var n,u;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:r}=i,c=t.homeScore>t.aiScore,l=t.homeScore===t.aiScore,a=c?"victoire":l?"nul":"defaite",s=Wi(t.mode,a);t.matchId&&await I.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:c?r.profile.id:null,home_credits_reward:s,played_at:new Date().toISOString()}).eq("id",t.matchId);const p={credits:(r.profile.credits||0)+s,matches_played:(r.profile.matches_played||0)+1};c?p.wins=(r.profile.wins||0)+1:l?p.draws=(r.profile.draws||0)+1:p.losses=(r.profile.losses||0)+1,await I.from("users").update(p).eq("id",r.profile.id),await i.refreshProfile();const o=document.createElement("div");o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",o.innerHTML=`
    ...
    </div>`,`<button class="btn btn-primary" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}async function Po(e,t){await Fi(e,t,"random",({deckId:i,formation:r,starters:c,subsRaw:l,gcCardsEnriched:a,gcDefs:s})=>{const p=o=>Ho(e,t,i,r,c,l,o,s||[]);if(!a.length){p([]);return}Ci(e,a,p)})}async function Ho(e,t,i,r,c,l,a=[],s=[]){var _;const{state:p,navigate:o,toast:n}=t;let u=!1,f=null;Ye(e),e.innerHTML=`
  ...
  `;function p(o){const n=document.getElementById("mkt-content"),u=o==="buy"?a:s;if(u.length===0){n.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      ${f.map(g=>{var M,G,k,w;const y=(M=g.card)==null?void 0:M.player;if(!y)return"";const d=y.job==="GK"?y.note_g:y.job==="DEF"?y.note_d:y.job==="MIL"?y.note_m:y.note_a,x=Ko[y.rarity],v=(i.profile.credits||0)>=g.price,_=g.status==="sold";return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${_?"opacity:0.65":""}">
          ...
          <div style="flex:1;min-width:0">
            ...
            ${o==="buy"?`<div style="font-size:11px;color:var(--gray-600)">Vendeur : ${((k=g.seller)==null?void 0:k.pseudo)||"—"}</div>`:_?`<div style="font-size:11px;color:var(--green)">✅ Vendu à ${((w=g.buyer)==null?void 0:w.pseudo)||"—"} · ${g.sold_at?new Date(g.sold_at).toLocaleDateString("fr"):""}</div>`:`<div style="font-size:11px;color:var(--gray-600)">🟢 En vente depuis le ${new Date(g.listed_at).toLocaleDateString("fr")}</div>`}
          </div>
          <div style="text-align:right;flex-shrink:0">
            <div style="font-weight:900;color:var(--yellow);font-size:15px">${g.price.toLocaleString("fr")}</div>
            ${o==="buy"?`<button class="btn btn-primary btn-sm" data-buy="${g.id}" ${v?"":"disabled"} style="margin-top:4px">${v?"Acheter":"Trop cher"}</button>`:_?'<span style="display:inline-block;margin-top:4px;font-size:10px;font-weight:700;color:#fff;background:var(--green);padding:3px 10px;border-radius:10px">VENDU</span>':`<button class="btn btn-danger btn-sm" data-cancel="${g.id}" style="margin-top:4px">Retirer</button>`}
          </div>
        </div>`}).join("")}
  ...
  `}async function Qo(e,{state:t,navigate:i,toast:r}){const c=t.profile;if(!c)return;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:l}=await I.from("matches").select(`id,home_id,away_id,home_score,away_score,status,mode,winner_id,created_at,played_at,
      ...
      away:users!away_id(pseudo,club_name)`).or(`home_id.eq.${c.id},away_id.eq.${c.id}`).order("created_at",{ascending:!1}).limit(50),a={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},s=(l||[]).filter(n=>n.status==="finished"),p=(l||[]).filter(n=>n.status==="in_progress");function o(n){const u=n.home_id===c.id;u?n.home_score:n.away_score,u?n.away_score:n.home_score;const f=n.winner_id===c.id,g=n.home_score===n.away_score&&n.status==="finished",y=n.status!=="finished"?"…":g?"N":f?"V":"D",d=n.status!=="finished"||g?"#888":f?"#1A6B3C":"#c0392b";let x=a[n.mode]||n.mode;n.away_id===null&&!x.startsWith("IA")&&(x="IA");const _=n.home_id===c.id?n.away:n.home;let M;n.away_id===null?M=x:_?M=`${_.club_name||_.pseudo} (${_.pseudo})`:M="Adversaire";let G="";n.status==="in_progress"&&Date.now()-new Date(n.created_at).getTime()>3600*1e3&&(G=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const k=new Date(n.created_at),w=k.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+k.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),F=n.status==="finished"?`${n.home_score} - ${n.away_score}`:`${n.home_score||0} - ${n.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
