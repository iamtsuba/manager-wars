...
import{s as w,F as fi,r as Se,j as Bt,l as Yi,m as qt,h as gi,n as En,o as Ln,T as Qt,k as Tn,p as zn,b as Mn}from"./formation-links-EITwj297.js";const Sn="/";function ji(e,{navigate:t,toast:i}){let n="login";const a=()=>{var r,s,l,c,d,u;const o=n==="login";e.innerHTML=`
    <div style="
    ...
    </style>`,(r=document.getElementById("tab-login-btn"))==null||r.addEventListener("click",()=>{n="login",a()}),(s=document.getElementById("tab-reg-btn"))==null||s.addEventListener("click",()=>{n="register",a()}),o?((l=document.getElementById("login-password"))==null||l.addEventListener("keydown",g=>{var x;g.key==="Enter"&&((x=document.getElementById("login-btn"))==null||x.click())}),(c=document.getElementById("login-btn"))==null||c.addEventListener("click",async()=>{const g=document.getElementById("login-email").value.trim(),x=document.getElementById("login-password").value,f=document.getElementById("login-error");if(f.textContent="",!g||!x){f.textContent="Remplissez tous les champs.";return}const p=document.getElementById("login-btn");p.textContent="⏳ Connexion…",p.disabled=!0;const{error:m}=await w.auth.signInWithPassword({email:g,password:x});if(p.textContent="⚽ Se connecter",p.disabled=!1,m){f.textContent=m.message.includes("Invalid")?"Email ou mot de passe incorrect.":m.message;return}window.location.reload()})):((d=document.getElementById("reg-confirm"))==null||d.addEventListener("keydown",g=>{var x;g.key==="Enter"&&((x=document.getElementById("reg-btn"))==null||x.click())}),(u=document.getElementById("reg-btn"))==null||u.addEventListener("click",async()=>{const g=document.getElementById("reg-email").value.trim(),x=document.getElementById("reg-password").value,f=document.getElementById("reg-confirm").value,p=document.getElementById("reg-error");if(p.textContent="",!g||!x||!f){p.textContent="Remplissez tous les champs.";return}if(x.length<6){p.textContent="Mot de passe trop court (min. 6 caractères).";return}if(x!==f){p.textContent="Les mots de passe ne correspondent pas.";return}const m=document.getElementById("reg-btn");m.textContent="⏳ Création…",m.disabled=!0;const{error:b}=await w.auth.signUp({email:g,password:x});if(m.textContent="🚀 Créer mon compte",m.disabled=!1,b){p.textContent=b.message;return}i("Compte créé ! Connecte-toi pour commencer.","success",4e3),n="login",a(),setTimeout(()=>{const v=document.getElementById("login-email");v&&(v.value=g)},50)}))};a()}function In(e,{state:t,navigate:i,toast:n,refreshProfile:a}){let o="#1A6B3C",r="#D4A017";e.innerHTML=`
  ...
  `;function s(){var f;const c=document.getElementById("logo-preview"),d=document.getElementById("logo-initials"),u=((f=document.getElementById("setup-club"))==null?void 0:f.value)||"MW",g=u.trim().split(" ").filter(Boolean),x=g.length>=2?(g[0][0]+g[1][0]).toUpperCase():u.slice(0,2).toUpperCase();c&&(c.style.background=r,c.style.borderColor=o),d&&(d.textContent=x,d.style.color=o)}document.getElementById("color1").addEventListener("input",c=>{o=c.target.value,document.getElementById("swatch1").style.background=o,s()}),document.getElementById("color2").addEventListener("input",c=>{r=c.target.value,document.getElementById("swatch2").style.background=r,s()});function l(c){document.querySelectorAll(".setup-step").forEach(d=>d.classList.remove("active")),document.getElementById(`step-${c}`).classList.add("active"),document.getElementById("step-num").textContent=c,document.getElementById("progress-fill").style.width=`${Math.round(c/3*100)}%`,c===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),d=document.getElementById("step1-error");if(d.textContent="",c.length<3){d.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:u}=await w.from("users").select("id").eq("pseudo",c).maybeSingle();if(u){d.textContent="Ce pseudo est déjà pris.";return}l(2)}),document.getElementById("step2-back").addEventListener("click",()=>l(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const c=document.getElementById("setup-club").value.trim(),d=document.getElementById("step2-error");if(d.textContent="",c.length<2){d.textContent="Nom trop court (min. 2 caractères).";return}const{data:u}=await w.from("users").select("id").eq("club_name",c).maybeSingle();if(u){d.textContent="Ce nom de club est déjà pris.";return}l(3)}),document.getElementById("step3-back").addEventListener("click",()=>l(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),d=document.getElementById("setup-club").value.trim(),u=document.getElementById("step3-error"),g=document.getElementById("step3-finish");u.textContent="",g.disabled=!0,g.textContent="Création en cours…";try{const{error:x}=await w.from("users").insert({id:t.user.id,pseudo:c,club_name:d,club_color1:o,club_color2:r,credits:1e4});if(x)throw x;await An(t.user.id),await a(),n(`Bienvenue ${c} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(x){u.textContent=x.message,g.disabled=!1,g.textContent="🚀 Créer mon profil !"}})}async function An(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await w.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const jn="modulepreload",Cn=function(e){return"/"+e},Ci={},Wi=function(t,i,n){let a=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const r=document.querySelector("meta[property=csp-nonce]"),s=(r==null?void 0:r.nonce)||(r==null?void 0:r.getAttribute("nonce"));a=Promise.allSettled(i.map(l=>{if(l=Cn(l),l in Ci)return;Ci[l]=!0;const c=l.endsWith(".css"),d=c?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${l}"]${d}`))return;const u=document.createElement("link");if(u.rel=c?"stylesheet":jn,c||(u.as="script"),u.crossOrigin="",u.href=l,s&&u.setAttribute("nonce",s),document.head.appendChild(u),c)return new Promise((g,x)=>{u.addEventListener("load",g),u.addEventListener("error",()=>x(new Error(`Unable to preload CSS for ${l}`)))})}))}function o(r){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=r,window.dispatchEvent(s),!s.defaultPrevented)throw r}return a.then(r=>{for(const s of r||[])s.status==="rejected"&&o(s.reason);return t().catch(o)})},It="#1A6B3C",At="#cc2222",Bn="#D4A017",Bi="#888";async function qn(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    </div>`,await Xi(i,n,t),document.getElementById("btn-add-friend").addEventListener("click",()=>Fn(i,n)),document.getElementById("btn-accept-friend").addEventListener("click",()=>Ji(i,n,null,t))}async function Xi(e,t,i={}){const{navigate:n}=i,a=e.user.id,{data:o,error:r}=await w.from("friendships").select("id, requester_id, addressee_id").eq("status","accepted").or(`requester_id.eq.${a},addressee_id.eq.${a}`),{count:s}=await w.from("friendships").select("id",{count:"exact",head:!0}).eq("addressee_id",a).eq("status","pending"),l=document.getElementById("pending-badge");l&&(s>0?(l.style.display="flex",l.textContent=s):l.style.display="none");const c=document.getElementById("friends-list");if(!c)return;if(r){console.error("[Friends] Erreur:",r),c.innerHTML=`<div style="color:${At};text-align:center;padding:16px">Erreur chargement : ${r.message}</div>`;return}const d=(o||[]).map(x=>x.requester_id===a?x.addressee_id:x.requester_id);let u={};if(d.length){const{data:x}=await w.from("users").select("id, pseudo, club_name, last_seen_at, club_color1, club_color2").in("id",d);(x||[]).forEach(f=>{u[f.id]=f})}const g=(o||[]).map(x=>({friendshipId:x.id,friend:u[x.requester_id===a?x.addressee_id:x.requester_id]||{pseudo:"?"}}));if(!g.length){c.innerHTML=`
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      ${g.map(({friendshipId:x,friend:f})=>Dn(f,x)).join("")}
    </div>`,c.querySelectorAll("[data-stats]").forEach(x=>{x.addEventListener("click",()=>Gn(e,x.dataset.stats,x.dataset.friendName))}),c.querySelectorAll("[data-match]").forEach(x=>{x.addEventListener("click",async()=>{const f=x.dataset.friendId,p=x.dataset.friendName;if(typeof n!="function"){t("Erreur navigation","error");return}const m=e.user.id,{data:b}=await w.from("friend_match_invites").select("id").eq("inviter_id",f).eq("invitee_id",m).eq("status","pending").order("created_at",{ascending:!1}).limit(1).maybeSingle(),v=!!b;console.log("[Friends] clic match",{fid:f,fname:p,isAccepting:v}),n("match",{matchMode:"friend",friendId:f,friendName:p,isAccepting:v})})})}function Dn(e,t){const i=e.club_name||e.pseudo||"?",n=e.pseudo||"",a=(n||i).slice(0,2).toUpperCase(),o=e.club_color2||It,r=e.club_color1||"#ffffff",s=e.last_seen_at?new Date(e.last_seen_at):null,l=s&&Date.now()-s.getTime()<3*60*1e3;return`
    ...
    </div>`}function Fn(e,t){const i=mi();i.innerHTML=`
    <div class="popup-box">
    ...
    <div class="popup-box">
      ...
      ${d.length?`<div style="display:flex;flex-direction:column;gap:8px;max-height:50vh;overflow-y:auto;margin-bottom:14px">
            ${d.map(g=>{var x,f,p;return`
              <div style="display:flex;align-items:center;gap:10px;background:#f9f9f9;border-radius:10px;padding:10px 12px">
                <div style="flex:1;font-size:14px;font-weight:700">${((x=g.requester)==null?void 0:x.club_name)||((f=g.requester)==null?void 0:f.pseudo)||"?"}
    ...
    ${xi()}`,document.body.appendChild(c);const u=()=>c.remove();c.querySelector("#pending-close").addEventListener("click",u),c.addEventListener("click",g=>{g.target===c&&u()}),c.querySelectorAll("[data-accept]").forEach(g=>{g.addEventListener("click",async()=>{const{error:x}=await w.from("friendships").update({status:"accepted"}).eq("id",g.dataset.accept);if(x){t("Erreur : "+x.message,"error");return}g.closest("div[style]").remove(),t("✅ Ami accepté !","success"),Xi(e,t,n),i&&i()})}),c.querySelectorAll("[data-decline]").forEach(g=>{g.addEventListener("click",async()=>{await w.from("friendships").delete().eq("id",g.dataset.decline),g.closest("div[style]").remove(),t("Demande refusée","info"),i&&i()})})}async function Gn(e,t,i){const n=e.user.id,[a,o]=[n,t].sort(),r=n===a,{data:s}=await w.from("friend_match_stats").select("*").eq("player1_id",a).eq("player2_id",o).single(),l=e.profile.club_name||e.profile.pseudo||"Moi",c=s||{},d=r?c.wins_p1||0:c.wins_p2||0,u=r?c.wins_p2||0:c.wins_p1||0,g=c.draws||0,x=r?c.goals_p1||0:c.goals_p2||0,f=r?c.goals_p2||0:c.goals_p1||0,p=r?c.gc_used_p1||0:c.gc_used_p2||0,m=r?c.gc_used_p2||0:c.gc_used_p1||0,b=c.matches_total||0,v=($,y,B,E=It,G=At)=>`
    ...
    <div class="popup-box" style="max-width:380px">
      ...
      ${b===0?`
        ...
        ${v("Défaites",u,d)}
        ${v("Buts marqués",x,f)}
        ${v("Buts encaissés",f,x,At,It)}
        ${v("GC utilisés ⚡",p,m,"#7a28b8","#7a28b8")}
    ...
    </style>`}const Pn="2026.07.17-1401";async function qi(e,{state:t,navigate:i,toast:n}){var o,r;const a=t.profile;a&&(e.innerHTML=`
  <style>
    .home-dark {
      min-height: 100%;
  ...
  </div>`,document.getElementById("friend-req-btn").addEventListener("click",()=>Ji(e,t,()=>Zi(e,t)))}function Hn(e){const t=[{mode:"vs_ai_easy",label:"Facile",sub:"Gain 500 cr.",icon:"🟢"},{mode:"vs_ai_medium",label:"Moyen",sub:"Gain 1 000 cr.",icon:"🟡"},{mode:"vs_ai_hard",label:"Difficile",sub:"Gain 1 500 cr.",icon:"🟠"},{mode:"vs_ai_club",label:"Club",sub:"Gain 2 500 cr.",icon:"🔴"}],i=document.createElement("div");i.className="modal-overlay",i.style.zIndex="2000",i.innerHTML=`<div class="modal" style="max-width:380px">
  ...
  </div>`,document.body.appendChild(i);const n=()=>i.remove();document.getElementById("diff-cancel").addEventListener("click",n),i.addEventListener("click",a=>{a.target===i&&n()}),i.querySelectorAll("[data-mode]").forEach(a=>{a.addEventListener("click",()=>{n(),e("match",{matchMode:a.dataset.mode})})})}const Ie={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé pour ce match."},"Double attaque":{icon:"⚡",desc:"La note d'attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la note d'un joueur adverse."},Gel:{icon:"❄️",desc:"Bloque le meilleur attaquant IA."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function De(e,t){if(!e)return 0;let i=0;switch(t){case"GK":i=Number(e.note_g)||0;break;case"DEF":i=Number(e.note_d)||0;break;case"MIL":i=Number(e.note_m)||0;break;case"ATT":i=Number(e.note_a)||0;break;default:i=0}return i+(e.boost||0)}const Di=["ATT","MIL","DEF","GK"];function en(e){let t=0;const i=e.length;for(let n=0;n<i;n++)for(let a=n+1;a<i;a++){const o=e[n],r=e[a];if(!o||!r)continue;const s=o._col!=null&&r._col!=null&&o._col===r._col,l=o._col!=null&&r._col!=null&&Math.abs(o._col-r._col)===1,c=Di.indexOf(o._line||o.job),d=Di.indexOf(r._line||r.job),u=Math.abs(c-d)===1;if(!((o._line||o.job)===(r._line||r.job)&&l||s&&u))continue;const f=o.country_code&&r.country_code&&o.country_code===r.country_code,p=o.club_id&&r.club_id&&o.club_id===r.club_id;f&&p?t+=2:(f||p)&&(t+=1)}return t}function yi(e,t={}){let i=0,n=0;e.forEach(s=>{const l=s._line||s.job,c=Number(l==="MIL"?s.note_m:s.note_a)||0,d=s.stadiumBonus&&(l==="MIL"||l==="ATT")?10:0;i+=t.doubleAttack?c*2:c,n+=(s.boost||0)+d});const a=i+n,o=en(e);let r=a+o;return t.stolenNote&&(r-=t.stolenNote),{base:a,links:o,total:Math.max(0,r)}}function bi(e,t={}){const i=e.reduce((o,r)=>{const s=r._line||r.job;let l=0;s==="GK"?l=Number(r.note_g)||0:s==="MIL"?l=Number(r.note_m)||0:l=Number(r.note_d)||0;const c=r.stadiumBonus&&(s==="GK"||s==="DEF"||s==="MIL")?10:0;return o+l+(r.boost||0)+c},0),n=en(e);let a=i+n;return t.stolenNote&&(a-=t.stolenNote),{base:i,links:n,total:Math.max(0,a)}}function hi(e,t,i={}){return i.shield?{goal:!1,shielded:!0}:{goal:e>t,shielded:!1}}function tn(e,t,i="easy"){const n=e.filter(r=>!r.used);if(!n.length)return[];const a=[...n].sort((r,s)=>{const l=t==="attack"?De(r,"ATT"):r._line==="GK"?De(r,"GK"):De(r,"DEF");return(t==="attack"?De(s,"ATT"):s._line==="GK"?De(s,"GK"):De(s,"DEF"))-l});let o=i==="easy"?1+Math.floor(Math.random()*2):i==="medium"?2+Math.floor(Math.random()*2):3;return a.slice(0,Math.min(o,a.length,3))}function Un(e,t){const i={vs_ai_easy:{victoire:500,nul:250,defaite:50},vs_ai_medium:{victoire:1e3,nul:500,defaite:50},vs_ai_hard:{victoire:1500,nul:750,defaite:100},vs_ai_club:{victoire:2500,nul:1250,defaite:100}};return(i[e]||i.vs_ai_easy)[t]||0}const Kn={pepite:{win:2,loss:-1,boosterStart:"min"},papyte:{win:1,loss:-2,boosterStart:"max"}};function Vn(e,t){const i=e.player;if(!i)return 0;const n=i.rarity;if(n!=="pepite"&&n!=="papyte")return Number(i[t])||0;const a=nn(i),o=Number(i[t])||0;if(o<=0)return 0;const r=i.note_min??1,s=i.note_max??10,c=(e.current_note??a)-a;return Math.min(s,Math.max(r,o+c))}function nn(e){const t=e.job||"ATT";return Number(t==="GK"?e.note_g:t==="DEF"?e.note_d:t==="MIL"?e.note_m:e.note_a)||0}async function Yn(e,t){!e||!t||await Promise.all([Fi(e,"win"),Fi(t,"loss")])}async function Fi(e,t){const{data:i}=await w.from("cards").select("id, current_note, player:players(rarity, job, note_g, note_d, note_m, note_a, note_min, note_max)").eq("owner_id",e).eq("card_type","player");if(!(i!=null&&i.length))return;const n=i.filter(a=>{var o,r;return((o=a.player)==null?void 0:o.rarity)==="pepite"||((r=a.player)==null?void 0:r.rarity)==="papyte"});n.length&&await Promise.all(n.map(a=>{const o=Kn[a.player.rarity],r=t==="win"?o.win:o.loss,s=a.player.note_min??1,l=a.player.note_max??10,c=nn(a.player),d=a.current_note??c,u=Math.min(l,Math.max(s,d+r));return w.from("cards").update({current_note:u}).eq("id",a.id)}))}const Wn={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"},Zt={GK:"#111111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},Gt=["GK","DEF","MIL","ATT"],Xn=["Tous","GK","DEF","MIL","ATT"],Jn={normal:1e3,pepite:5e3,papyte:5e3,legende:1e4};function Gi(e){const t=e.player;if(!t)return null;const i=t.rarity;return(i==="pepite"||i==="papyte")&&e.current_note!=null?e.current_note:Math.max(Number(t.note_g)||0,Number(t.note_d)||0,Number(t.note_m)||0,Number(t.note_a)||0)}function Qn(e){return e.length?e.reduce((t,i)=>Gi(i)>Gi(t)?i:t,e[0]):e[0]}function Zn(e){return e==="GK"?"note_g":e==="DEF"?"note_d":e==="MIL"?"note_m":"note_a"}const eo={MA:"MAROC",FR:"FRANCE",AR:"ARGENTINE",PT:"PORTUGAL",BR:"BRESIL",ES:"ESPAGNE",DE:"ALLEMAGNE",GB:"ANGLETERRE",IT:"ITALIE",CM:"CAMEROUN",SN:"SENEGAL",NG:"NIGERIA",DK:"DANEMARK",NL:"PAYS-BAS",BE:"BELGIQUE",CI:"CÔTE D'IVOIRE",AL:"ALBANIE",HR:"CROATIE",RS:"SERBIE",TR:"TURQUIE"};function to(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";if(!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function ei(e,t){return e&&Number(t==="GK"?e.note_g:t==="DEF"?e.note_d:t==="MIL"?e.note_m:e.note_a)||0}function ti(e,t=""){const i=e.player;if(!i)return"";const n=e.evolution_bonus||0,a={...i,_evolution_bonus:n},o=t?`<div style="position:absolute;top:6px;right:6px;z-index:10;background:#0a3d1e;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 7px">${t}</div>`:"";return`<div style="position:relative;display:inline-block;cursor:pointer" data-card-id="${e.id}">
    ${o}
    ${Se(a,{width:140})}
  </div>`}function Pi(e){return`<div style="display:inline-block;filter:grayscale(1);opacity:0.4">
    ${Se(e,{width:140})}
  </div>`}async function io(e,t){const{state:i,navigate:n,toast:a,openModal:o,closeModal:r}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:s}=await w.from("cards").select(`id, card_type, current_note, gc_type, formation, is_for_sale, sale_price, stadium_id, evolution_bonus,
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),c=(s||[]).filter(M=>M.card_type==="player"&&M.player),d=(s||[]).filter(M=>M.card_type==="game_changer"),u=(s||[]).filter(M=>M.card_type==="formation"),g=(s||[]).filter(M=>M.card_type==="stadium"),{data:x}=await w.from("gc_definitions").select("name,gc_type,color,effect,image_url"),f={};(x||[]).forEach(M=>{f[M.name]=M});const{data:p}=await w.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),m={};(p||[]).forEach(M=>{m[M.id]=M}),g.forEach(M=>{M.stadium_def&&(m[M.stadium_id]=M.stadium_def)});const b=Object.keys(fi),v=Object.keys(Ie),h={};c.forEach(M=>{const C=M.player.id;h[C]=(h[C]||0)+1}),new Set(Object.keys(h).map(M=>String(M)));const $=new Set(u.map(M=>M.formation)),y=new Set(d.map(M=>M.gc_type));let B="player",E="Tous",G="",N=!1;function ne(){return[...c].sort((M,C)=>{const F=Gt.indexOf(M.player.job),W=Gt.indexOf(C.player.job);return F!==W?F-W:(M.player.surname_real||"").localeCompare(C.player.surname_real||"")})}function te(){return[...l||[]].sort((M,C)=>{const F=Gt.indexOf(M.job),W=Gt.indexOf(C.job);return F!==W?F-W:(M.surname_real||"").localeCompare(C.surname_real||"")})}function Q(){return ne().filter(M=>{const C=M.player,F=E==="Tous"||C.job===E,W=!G||`${C.firstname} ${C.surname_real}`.toLowerCase().includes(G);return F&&W})}function I(){return te().filter(M=>{const C=E==="Tous"||M.job===E,F=!G||`${M.firstname} ${M.surname_real}`.toLowerCase().includes(G);return C&&F})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
  ...
  </div>`;function _(){const M=document.getElementById("col-filters");M&&(B==="player"?(M.innerHTML=`
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",C=>{G=C.target.value.toLowerCase(),j()}),e.querySelectorAll(".filter-btn").forEach(C=>{C.addEventListener("click",()=>{E=C.dataset.job,_(),j()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{N=!N,_(),j()})):(M.innerHTML=`
        ...
        </div>`},null)}e.querySelectorAll(".col-tab-btn").forEach(M=>{M.addEventListener("click",()=>{B=M.dataset.tab,E="Tous",G="",N=!1,e.querySelectorAll(".col-tab-btn").forEach(C=>{const F=C.dataset.tab===B;C.style.borderBottomColor=F?"var(--green)":"transparent",C.style.color=F?"var(--green)":"var(--gray-600)"}),_(),j()})}),_(),j()}function no(e,t,i){const n=Ie[e]||{icon:"⚡",desc:"Effet spécial."},a=(t==null?void 0:t.gc_type)==="ultra_game_changer",o={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},r={purple:"#b06ce0",light_blue:"#00d4ef"},s=o[t==null?void 0:t.color]||o.purple,l=r[t==null?void 0:t.color]||r.purple,c=(t==null?void 0:t.name)||e,d=(t==null?void 0:t.effect)||n.desc,u=t!=null&&t.image_url?`/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(f=document.getElementById("direct-sell-form-btn"))==null||f.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${c} pour ${Pt.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const b=g.find(h=>!h.is_for_sale)||g[0];if(!b){o("Aucune carte à vendre","error");return}await w.from("market_listings").delete().eq("card_id",b.id),await w.from("transfer_history").delete().eq("card_id",b.id);const{error:v}=await w.from("cards").delete().eq("id",b.id);if(v){o(v.message,"error");return}await w.from("users").update({credits:(a.profile.credits||0)+Pt}).eq("id",a.profile.id),await l(),o(`+${Pt.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),r(),s("collection")}),(p=document.getElementById("market-sell-form-btn"))==null||p.addEventListener("click",async()=>{const b=parseInt(document.getElementById("sell-price-form").value);if(!b||b<1){o("Prix invalide","error");return}await w.from("cards").update({is_for_sale:!0,sale_price:b}).eq("id",e.id),await w.from("market_listings").insert({seller_id:a.profile.id,card_id:e.id,price:b}),o("Carte mise en vente sur le marché !","success"),r(),s("collection")}),(m=document.getElementById("cancel-sell-form-btn"))==null||m.addEventListener("click",async()=>{await w.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await w.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),o("Annonce retirée","success"),r(),s("collection")})}async function Ri(e,t,i,n){var le,ae,pe,se,Ee;const{state:a,toast:o,openModal:r,closeModal:s,navigate:l,refreshProfile:c}=n,d=e.player,u=t.filter(Y=>Y.player.id===d.id),g=u.length,x=e.evolution_bonus||0,f=Math.max((Number(d.note_g)||0)+(d.job==="GK"||d.job2==="GK"?x:0),(Number(d.note_d)||0)+(d.job==="DEF"||d.job2==="DEF"?x:0),(Number(d.note_m)||0)+(d.job==="MIL"||d.job2==="MIL"?x:0),(Number(d.note_a)||0)+(d.job==="ATT"||d.job2==="ATT"?x:0)),p=d.rarity||"normal",{data:m}=await w.from("sell_price_configs").select("*").eq("rarity",p).lte("note_min",f).gte("note_max",f).order("note_min",{ascending:!1}).limit(1);((le=m==null?void 0:m[0])==null?void 0:le.price)??Jn[p];const b=d.rarity!=="legende";to(d);const v=((d.rarity==="pepite"||d.rarity==="papyte")&&e.current_note!=null?e.current_note:ei(d,d.job))+x,h=d.rarity==="pepite"||d.rarity==="papyte",$=d.job2?(h?Vn(e,Zn(d.job2)):ei(d,d.job2))+(ei(d,d.job2)>0?x:0):null;Zt[d.job],d.job2&&Zt[d.job2];const y=Wn[d.rarity]||"#ccc";eo[d.country_code]||d.country_code;const B=e.evolution_bonus||0,G=v+B,N=$||0,ne=N>0?N+B:0,te=u.map(Y=>Y.id);let Q={};if(te.length){const{data:Y}=await w.from("transfer_history").select("card_id, club_name, manager_name, source, price, transferred_at").in("card_id",te).order("transferred_at",{ascending:!0});(Y||[]).forEach(J=>{Q[J.card_id]||(Q[J.card_id]=[]),Q[J.card_id].push(J)})}const I=Y=>{const J=Y.transferred_at?new Date(Y.transferred_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"2-digit",year:"numeric"}):"",P=Y.source==="booster"?"Booster":Y.price?Y.price.toLocaleString("fr"):"—";return`
    ...
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${u.map((Y,J)=>{const P=Q[Y.id]||[],M=Y.is_for_sale,C=P.length?P[P.length-1]:null,W=(d.rarity==="pepite"||d.rarity==="papyte")&&Y.current_note!=null?` (☆${Y.current_note})`:"";return`
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",d.note_g],["DEF",d.note_d],["MIL",d.note_m],["ATT",d.note_a]].map(([Y,J])=>{const P=Zt[Y],M=Y==="GK"?"#fff":P,C=Y===d.job||Y===d.job2,F=(Number(J)||0)+(C&&B>0?B:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(ae=document.getElementById("close-detail"))==null||ae.addEventListener("click",s);let j=new Set;const U=()=>{const Y=j.size,J=document.getElementById("sell-action-panel");if(!J)return;J.style.display=Y>0?"block":"none",document.getElementById("sell-selected-count").textContent=Y;const P=document.getElementById("evolve-btn");P&&(P.textContent=`⬆️ Évoluer ${Y>1?"(+"+Y+")":""}`)};document.querySelectorAll(".expl-check").forEach(Y=>{Y.addEventListener("change",()=>{const J=Y.dataset.id;Y.checked?j.add(J):j.delete(J);const P=Y.closest(".exemplaire-row");P&&(P.style.borderColor=Y.checked?"#1A6B3C":"#eee"),U()})}),document.querySelectorAll(".exemplaire-row").forEach(Y=>{Y.addEventListener("click",J=>{if(J.target.closest("button")||J.target.tagName==="INPUT")return;const P=Y.querySelector(".expl-check");P&&!P.disabled&&(P.checked=!P.checked,P.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(Y=>{Y.addEventListener("click",J=>{J.stopPropagation();const P=document.querySelector(`.expl-hist[data-hist="${Y.dataset.idx}"]`);P&&(P.style.display=P.style.display==="none"?"flex":"none")})}),(pe=document.getElementById("evolve-btn"))==null||pe.addEventListener("click",async()=>{if(g<=1)return;const Y=[...j];if(!Y.length)return;if(j.has(e.id)){const K=document.createElement("div");K.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",K.innerHTML=`
  ...
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
    ...
    <div class="page-body">
  ...
  </div>`,document.getElementById("new-deck-btn").addEventListener("click",async()=>{const r=prompt("Nom du deck :",`Deck ${((o==null?void 0:o.length)||0)+1}`);if(!r)return;const{data:s,error:l}=await w.from("decks").insert({owner_id:i.profile.id,name:r}).select().single();if(l){a(l.message,"error");return}a("Deck créé !","success"),Ni(s.id,e,t)}),e.querySelectorAll("[data-open-deck]").forEach(r=>{r.addEventListener("click",()=>Ni(r.dataset.openDeck,e,t))}),e.querySelectorAll("[data-rename]").forEach(r=>{r.addEventListener("click",async()=>{const s=prompt("Nouveau nom :",r.dataset.name);if(!s||s===r.dataset.name)return;const{error:l}=await w.from("decks").update({name:s}).eq("id",r.dataset.rename);if(l){a(l.message,"error");return}a("Deck renommé !","success"),oi(e,t)})}),e.querySelectorAll("[data-delete]").forEach(r=>{r.addEventListener("click",async()=>{if(!confirm(`Supprimer le deck "${r.dataset.name}" ? Cette action est irréversible.`))return;await w.from("deck_cards").delete().eq("deck_id",r.dataset.delete);const{error:s}=await w.from("decks").delete().eq("id",r.dataset.delete);if(s){a(s.message,"error");return}a("Deck supprimé.","success"),oi(e,t)})})}async function Ni(e,t,i){const{state:n,toast:a}=i;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await w.from("decks").select("*").eq("id",e).single(),{data:r}=await w.from("cards").select(`id, card_type, formation, stadium_id, evolution_bonus,
      player:players(id, firstname, surname_real, country_code, club_id, job, job2,
      ...
      stadium_def:stadium_definitions(id, name, club_id, country_code, image_url,
        club:clubs(encoded_name, logo_url))`).eq("owner_id",n.profile.id).eq("card_type","stadium"),l=(r||[]).filter(b=>b.card_type==="player"&&b.player),c=(r||[]).filter(b=>b.card_type==="formation"),d=(s||[]).filter(b=>b.card_type==="stadium"),u=[...new Set(d.map(b=>b.stadium_id).filter(Boolean))];let g={};if(d.forEach(b=>{b.stadium_def&&b.stadium_id&&(g[b.stadium_id]=b.stadium_def)}),u.length&&Object.keys(g).length<u.length){const{data:b}=await w.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",u);(b||[]).forEach(v=>{g[v.id]=v})}const x=c.map(b=>b.formation).filter(Boolean),{data:f}=await w.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",e);let p=o.formation||"4-4-2";x.length>0&&!x.includes(p)&&(p=x[0]);const m={deckId:e,name:o.name,formation:p,formationCardId:o.formation_card_id,stadiumCardId:o.stadium_card_id||null,slots:{},subs:[],playerCards:l,formationCards:c,stadiumCards:d,stadDefMap:g,availableFormations:x};(f||[]).forEach(b=>{b.is_starter?m.slots[b.position]=b.card_id:m.subs.includes(b.card_id)||m.subs.push(b.card_id)}),it(t,m,i)}function it(e,t,i){var f,p;const{navigate:n}=i;Ot[t.formation];const a=Oi(t.formation),o=a.filter(m=>t.slots[m]).length,r=t.availableFormations.length>0?t.availableFormations:Object.keys(Ot),s=(f=t.stadiumCards)==null?void 0:f.find(m=>m.id===t.stadiumCardId),l=s&&((p=t.stadDefMap)==null?void 0:p[s.stadium_id])||null,c=t.subs.map(m=>t.playerCards.find(b=>b.id===m)).filter(Boolean);[...Object.values(t.slots),...t.subs],e.innerHTML=`
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
      ...
      <div style="display:flex;gap:0;min-height:600px">
        ...
        <div style="width:105px;flex-shrink:0;background:rgba(0,0,0,0.3);display:flex;flex-direction:column;align-items:center;padding:12px 6px;gap:8px;border-right:1px solid rgba(255,255,255,0.1)">
          ...
          <div style="display:flex;flex-direction:column;gap:6px;align-items:center" id="subs-list">
            ${c.map(m=>{const b={...m.player,_evolution_bonus:m.evolution_bonus||0};return`<div style="position:relative;flex-shrink:0;overflow:visible">
                ${Se({...b,_evolution_bonus:b._evolution_bonus||0},{width:60,showStad:!0,stadDef:l})}
                <button data-remove-sub="${m.id}"
        ...
        <div style="width:130px;flex-shrink:0;background:rgba(0,0,0,0.3);display:flex;flex-direction:column;align-items:center;padding:12px 8px;gap:12px;border-left:1px solid rgba(255,255,255,0.1)">
          ...
          <div style="width:100%;text-align:center">
            ...
            <div id="add-stad-btn-pc" style="cursor:pointer;margin:0 auto;width:fit-content">
              ${s?(()=>{var v;const m=t.stadDefMap[s.stadium_id],b=((v=m==null?void 0:m.club)==null?void 0:v.logo_url)||null;return`<div style="width:100px;height:130px;border-radius:8px;background:linear-gradient(135deg,#1a3a6b,#0a1a3a);border:2px solid #4fc3f7;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px">
                  ${b?`<img src="${b}" style="width:48px;height:48px;object-fit:contain">`:'<span style="font-size:36px">🏟️</span>'}
                  <span style="font-size:10px;font-weight:700;color:#E87722;text-align:center;padding:0 4px">${((m==null?void 0:m.name)||"Stade").slice(0,14)}</span>
  ...
  </div>`;const d=window.innerWidth>=900,u=e.querySelector(".deck-pc-layout"),g=e.querySelector(".deck-mobile-layout");u&&(u.style.display=d?"block":"none"),g&&(g.style.display=d?"none":"block"),ro(e,t,a,i),e.querySelectorAll("#builder-back").forEach(m=>m.addEventListener("click",()=>n("decks")));const x=()=>{const{openModal:m,closeModal:b}=i,v=`<div style="display:flex;flex-wrap:wrap;gap:8px;padding:8px">
    ...
    </div>`;m("⚽ Choisir une formation",v),document.querySelectorAll("#modal-body [data-forma]").forEach(h=>{h.addEventListener("click",()=>{t.formation=h.dataset.forma;const $=Oi(t.formation),y={};$.forEach(B=>{t.slots[B]&&(y[B]=t.slots[B])}),t.slots=y,b(),it(e,t,i)})})};e.querySelectorAll("#formation-mobile-btn, #formation-pc-btn").forEach(m=>m.addEventListener("click",x)),e.querySelectorAll("#add-stad-btn-pc, #add-stad-btn").forEach(m=>m.addEventListener("click",()=>ao(t,e,i))),e.querySelectorAll("#save-deck").forEach(m=>m.addEventListener("click",()=>co(t,i))),e.querySelectorAll("[data-remove-sub]").forEach(m=>{m.addEventListener("click",()=>{t.subs=t.subs.filter(b=>b!==m.dataset.removeSub),it(e,t,i)})}),e.querySelectorAll("#add-sub-btn").forEach(m=>m.addEventListener("click",()=>lo(t,e,i)))}function ro(e,t,i,n){var h,$;const a=window.innerWidth>=900,o=e.querySelector(a?"#deck-field-pc":"#deck-field-mobile");if(!o)return;const r=(h=t.stadiumCards)==null?void 0:h.find(y=>y.id===t.stadiumCardId),s=r&&(($=t.stadDefMap)==null?void 0:$[r.stadium_id])||null,l=Bt[t.formation]||{},c=Yi(t.formation)||[],d={};for(const y of i){const B=t.slots[y],E=B?t.playerCards.find(G=>G.id===B):null;E!=null&&E.player?d[y]={...E.player,_evolution_bonus:E.evolution_bonus||0,face:E.player.face||null}:d[y]=null}const u=window.innerWidth>=900,g=u?window.innerWidth-280:window.innerWidth-20,x=u?Math.min(g,860):g,f=Math.round(u?x*.82:x*.85),p=u?84:44;let m="";for(const[y,B]of c){const E=l[y],G=l[B];if(!E||!G)continue;const N=E.x*x,ne=Math.round(.03*f+E.y*.85*f),te=G.x*x,Q=Math.round(.03*f+G.y*.85*f),I=d[y],_=d[B],j=qt(I,_);j==="#ff3333"||j==="#cc2222"?m+=`<line x1="${N.toFixed(1)}" y1="${ne.toFixed(1)}" x2="${te.toFixed(1)}" y2="${Q.toFixed(1)}" stroke="${j}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(m+=`<line x1="${N.toFixed(1)}" y1="${ne.toFixed(1)}" x2="${te.toFixed(1)}" y2="${Q.toFixed(1)}" stroke="${j}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,m+=`<line x1="${N.toFixed(1)}" y1="${ne.toFixed(1)}" x2="${te.toFixed(1)}" y2="${Q.toFixed(1)}" stroke="${j}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let b="";const v=Math.round(p*657/507);for(const y of i){const B=l[y];if(!B)continue;const E=d[y],G=B.x*x;B.y*f;const N=Math.round(.03*f+B.y*(.85*f)),ne=Math.round(G-p/2),te=Math.round(N-v/2);if(E){const Q=y.replace(/\d+/,""),I=s&&(s.club_id&&String(E.club_id)===String(s.club_id)||s.country_code&&E.country_code===s.country_code),_=Se({...E,_evolution_bonus:E._evolution_bonus||0},{width:p,showStad:!0,stadDef:s,role:Q});b+=`<div style="position:absolute;left:${ne}px;top:${te}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${y}">
        <div style="position:relative">${I?'<div style="position:absolute;top:-18px;left:0;right:0;text-align:center;font-size:14px;z-index:5;line-height:1">🏟️</div>':""}${_}</div>
    ...
    <div style="position:relative;width:${x}px;height:${f}px;margin:0 auto">
      <svg style="position:absolute;inset:0;width:100%;height:100%;pointer-events:none" viewBox="0 0 ${x} ${f}">${m}</svg>
      ${b}
    </div>`,o.querySelectorAll(".deck-slot-hit").forEach(y=>{y.addEventListener("click",()=>so(y.dataset.pos,t,e,n))})}function ao(e,t,i){var r;const{openModal:n,closeModal:a}=i,o=e.stadiumCards||[];n("🏟️ Choisir un stade",`<div style="display:flex;flex-wrap:wrap;gap:10px;padding:8px">
      ...
      ${o.map(s=>{var b,v;const l=e.stadDefMap[s.stadium_id],c=((b=l==null?void 0:l.club)==null?void 0:b.logo_url)||null,d=l!=null&&l.image_url?"/icons/"+l.image_url:null,u=e.stadiumCardId===s.id,g=l!=null&&l.country_code&&!c?`https://flagsapi.com/${l.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,x=(l==null?void 0:l.name)||"Stade",f=x.match(/^(Stade\s+(?:de\s+)?(?:\w+)?)(.*)?$/i),p=f?f[1].trim():x.slice(0,8),m=((v=f==null?void 0:f[2])==null?void 0:v.trim())||"";return`<div class="stad-choice" data-stad-id="${s.id}" style="cursor:pointer;width:80px;text-align:center">
    ...
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(b=document.getElementById("close-selector"))==null||b.addEventListener("click",o),(v=document.getElementById("remove-player"))==null||v.addEventListener("click",()=>{delete t.slots[e],o(),it(i,t,n)}),document.querySelectorAll(".player-option").forEach(h=>{h.addEventListener("click",()=>{t.slots[e]=h.dataset.cardId,o(),it(i,t,n)})})}function lo(e,t,i){var d,u,g;const{openModal:n,closeModal:a}=i,o=(d=e.stadiumCards)==null?void 0:d.find(x=>x.id===e.stadiumCardId),r=o&&((u=e.stadDefMap)==null?void 0:u[o.stadium_id])||null,s=new Set;Object.keys(e.slots).forEach(x=>{var m;const f=e.slots[x];if(!f)return;const p=e.playerCards.find(b=>b.id===f);(m=p==null?void 0:p.player)!=null&&m.id&&s.add(p.player.id)}),e.subs.forEach(x=>{var p;const f=e.playerCards.find(m=>m.id===x);(p=f==null?void 0:f.player)!=null&&p.id&&s.add(f.player.id)});const l=new Set,c=e.playerCards.filter(x=>{var f,p,m;return s.has((f=x.player)==null?void 0:f.id)||l.has((p=x.player)==null?void 0:p.id)?!1:(l.add((m=x.player)==null?void 0:m.id),!0)});n("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${c.length>0?'<div style="display:flex;flex-wrap:wrap;gap:8px;justify-content:center">'+c.map(x=>{const f={...x.player,_evolution_bonus:x.evolution_bonus||0};return`<div class="player-option" data-card-id="${x.id}" style="cursor:pointer">
    ...
    </div>`,'<button class="btn btn-ghost" id="close-sub-selector">Fermer</button>'),(g=document.getElementById("close-sub-selector"))==null||g.addEventListener("click",a),document.querySelectorAll(".player-option").forEach(x=>{x.addEventListener("click",()=>{e.subs.push(x.dataset.cardId),a(),it(t,e,i)})})}async function co(e,t){const{state:i,toast:n,navigate:a}=t,o=e.formationCards.find(l=>l.formation===e.formation),r=(o==null?void 0:o.id)||e.formationCardId;await w.from("decks").update({formation:e.formation,formation_card_id:r||null,stadium_card_id:e.stadiumCardId||null}).eq("id",e.deckId),await w.from("deck_cards").delete().eq("deck_id",e.deckId);const s=[];if(Object.entries(e.slots).forEach(([l,c],d)=>{s.push({deck_id:e.deckId,card_id:c,position:l,is_starter:!0,slot_order:d})}),e.subs.forEach((l,c)=>{s.push({deck_id:e.deckId,card_id:l,position:`SUB${c+1}`,is_starter:!1,slot_order:100+c})}),s.length>0){const{error:l}=await w.from("deck_cards").insert(s);if(l){n(l.message,"error");return}}n("Deck enregistré ✅","success"),a("decks")}function Oi(e){const t=Ot[e]||Ot["4-4-2"],i=["GK1"];for(let n=1;n<=t.DEF;n++)i.push(`DEF${n}`);for(let n=1;n<=t.MIL;n++)i.push(`MIL${n}`);for(let n=1;n<=t.ATT;n++)i.push(`ATT${n}`);return i}async function on(e=null){const t=new Date().toISOString().slice(0,10),{data:i}=await w.from("booster_configs").select("*").eq("is_active",!0).order("sort_order");if(!(i!=null&&i.length))return[];const n=i.filter(r=>!(r.available_from&&t<r.available_from||r.available_until&&t>r.available_until));if(!n.length)return[];let a=n;if(e){const r=n.filter(s=>s.max_per_user!=null);if(r.length){const{data:s}=await w.from("booster_claims").select("booster_id").eq("user_id",e).in("booster_id",r.map(c=>c.id)),l={};(s||[]).forEach(c=>{l[c.booster_id]=(l[c.booster_id]||0)+1}),a=n.filter(c=>c.max_per_user==null?!0:(l[c.id]||0)<c.max_per_user)}}if(!a.length)return[];const{data:o}=await w.from("booster_drop_rates").select("*").in("booster_id",a.map(r=>r.id)).order("sort_order");return a.map(r=>({...r,rates:(o||[]).filter(s=>s.booster_id===r.id)}))}async function po(e,t){const{data:i}=await w.from("booster_configs").select("max_per_user").eq("id",t).single();i!=null&&i.max_per_user&&await w.from("booster_claims").insert({user_id:e,booster_id:t})}function uo(e){if(!(e!=null&&e.length))return null;const t=e.reduce((n,a)=>n+Number(a.percentage),0);let i=Math.random()*t;for(const n of e)if(i-=Number(n.percentage),i<=0)return n;return e[e.length-1]}const rn=()=>Object.keys(Bt),fo=[{id:"players_std",img:"/icons/booster-players.png",name:"Players",sub:"5 cartes joueurs",cost:5e3,costLabel:"5 000 crédits",cardCount:5,type:"player"},{id:"players_pub",img:"/icons/booster-silver.png",name:"Players (pub)",sub:"3 cartes joueurs",cost:0,costLabel:"1 pub",cardCount:3,type:"player"},{id:"game_changer",img:"/icons/booster-gamechanger.png",name:"Game Changer",sub:"3 cartes spéciales",cost:1e4,costLabel:"10 000 crédits",cardCount:3,type:"game_changer"},{id:"formation",img:"/icons/booster-formation.png",name:"Formation",sub:"1 carte formation",cost:1e4,costLabel:"10 000 crédits",cardCount:1,type:"formation"}],ri={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé."},"Double attaque":{icon:"⚡",desc:"La prochaine attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la prochaine action IA."},Gel:{icon:"❄️",desc:"Bloque le meilleur attaquant IA."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function go(e){const t=e.player;if(!t)return"";const i=e.evolution_bonus||0;return Se({...t,_evolution_bonus:i},{width:140})}function an(e){var a;const t={};(e.rates||[]).forEach(o=>{t[o.card_type]=(t[o.card_type]||0)+Number(o.percentage||0)});const i=((a=Object.entries(t).sort((o,r)=>r[1]-o[1])[0])==null?void 0:a[0])||"player",n=e.image_url||"booster-players.png";return{id:e.id,img:"/icons/"+n,name:e.name,sub:`${e.card_count} carte(s)`,cost:e.price_type==="credits"&&e.price_credits||0,costLabel:e.price_type==="credits"?`${(e.price_credits||0).toLocaleString("fr")} crédits`:e.price_type==="pub"?"1 pub":"Gratuit",cardCount:e.card_count||5,type:i,isPub:e.price_type==="pub",rates:e.rates||[],allow_duplicates:e.allow_duplicates!==!1,_boosterId:e.id,_raw:e}}async function mo(e,{state:t,navigate:i,toast:n}){var u,g,x;const a=((u=t.profile)==null?void 0:u.credits)||0;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⏳ Chargement...</div>';let o=[];try{o=(await on((g=t.user)==null?void 0:g.id)).map(an)}catch(f){console.warn("Erreur chargement boosters DB, fallback hardcodé",f)}o.length||(o=fo.map(f=>({...f,rates:[],isPub:f.id==="players_pub"})));const r=await w.from("cards").select("player_id, card_type, formation, stadium_id, gc_type").eq("owner_id",t.profile.id).then(f=>f.data||[]),s=new Set(r.filter(f=>f.card_type==="stadium").map(f=>f.stadium_id)),l=new Set(r.filter(f=>f.card_type==="formation").map(f=>f.formation)),c=new Set(r.filter(f=>f.card_type==="game_changer").map(f=>f.gc_type)),d={};for(const f of o){if(f.allow_duplicates!==!1||!((x=f.rates)!=null&&x.length))continue;const p=[...new Set((f.rates||[]).map(b=>b.card_type))];let m=!1;for(const b of p)if(b==="stadium"){const{data:v}=await w.from("stadium_definitions").select("id");if((v||[]).some(h=>!s.has(h.id))){m=!0;break}}else if(b==="game_changer"){const{data:v}=await w.from("gc_definitions").select("name").eq("is_active",!0);if((v||[]).some(h=>!c.has(h.name))){m=!0;break}}else if(b==="formation"){const{FORMATION_LINKS:v}=await Wi(async()=>{const{FORMATION_LINKS:h}=await import("./formation-links-EITwj297.js").then($=>$.q);return{FORMATION_LINKS:h}},__vite__mapDeps([0,1]));if(Object.keys(v).some(h=>!l.has(h))){m=!0;break}}else{m=!0;break}m||(d[f.id]=!0)}e.innerHTML=`
  <div class="page">
    <div class="page-header">
      <h2>📦 Boosters</h2>
    ...
    <div class="page-body">
      <div class="booster-grid">
        ${o.map(f=>{const p=f.cost===0||a>=f.cost,m=d[f.id]===!0;return`<div class="booster-card ${!p||m?"disabled":""}" data-booster="${f.id}" style="position:relative">
  ...
  </div>`,e.querySelectorAll(".booster-card:not(.disabled)").forEach(f=>{f.addEventListener("click",async()=>{const p=o.find(m=>m.id===f.dataset.booster);if(p){f.style.opacity="0.5",f.style.pointerEvents="none";try{await xo(p,{state:t,toast:n,navigate:i,container:e})}catch(m){n(m.message,"error"),f.style.opacity="",f.style.pointerEvents=""}}})}),e.querySelectorAll(".booster-info-btn").forEach(f=>{f.addEventListener("click",p=>{p.stopPropagation();const m=o.find(b=>b.id===f.dataset.boosterId);vo(m)})})}async function xo(e,{state:t,toast:i,navigate:n,container:a}){var u,g;if(e.cost>0&&t.profile.credits<e.cost){i("Crédits insuffisants","error");return}e.isPub&&await _o();const{data:o}=await w.from("cards").select("card_type, player_id, formation").eq("owner_id",t.profile.id),r=new Set((o||[]).filter(x=>x.card_type==="player").map(x=>x.player_id)),s=new Set((o||[]).filter(x=>x.card_type==="formation").map(x=>x.formation));let l=[],c=null;try{if((u=e.rates)!=null&&u.length)l=await ai(t.profile,e);else{const x=e.type||"player";x==="player"?l=await sn(t.profile,e.cardCount,e.cost):x==="game_changer"?l=await dn(t.profile,e.cardCount,e.cost):x==="formation"?l=await ln(t.profile,e.cost):l=await ai(t.profile,e)}}catch(x){c=x.message||String(x),console.error("[Booster] Erreur:",x)}if(l!=null&&l.length&&e._boosterId&&await po(t.user.id,e._boosterId),!(l!=null&&l.length)){const x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:3000;gap:16px;color:#fff;padding:24px;text-align:center",x.innerHTML=`
      <div style="font-size:48px">😕</div>
      <div style="font-size:20px;font-weight:900">Aucune carte obtenue</div>
      <div style="font-size:12px;color:rgba(255,255,255,0.6);max-width:320px;word-break:break-all;background:rgba(255,255,255,0.05);padding:10px;border-radius:8px;margin-top:6px">
        ${c||"Vérifie la console (F12) pour plus de détails"}
      </div>
      <button style="margin-top:10px;padding:12px 28px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer" id="anim-close-err">Fermer</button>`,document.body.appendChild(x),(g=x.querySelector("#anim-close-err"))==null||g.addEventListener("click",()=>x.remove());return}l.forEach(x=>{x.card_type==="player"&&x.player?x.isDuplicate=r.has(x.player.id):x.card_type==="formation"&&(x.isDuplicate=s.has(x.formation))});const{data:d}=await w.from("users").select("*").eq("id",t.profile.id).single();d&&(t.profile=d),cn(l,e,n)}function yo(){const e=Math.random()*100;return e<.5?"legende":e<2?"special":e<10?"normal_high":"normal_low"}function Je(e){return Math.max(Number(e.note_g)||0,Number(e.note_d)||0,Number(e.note_m)||0,Number(e.note_a)||0)}function bo(e,t){let i;switch(t){case"legende":i=e.filter(n=>n.rarity==="legende"),i.length||(i=e.filter(n=>n.rarity==="pepite"||n.rarity==="papyte")),i.length||(i=e.filter(n=>Je(n)>=6));break;case"special":i=e.filter(n=>n.rarity==="pepite"||n.rarity==="papyte"),i.length||(i=e.filter(n=>Je(n)>=6));break;case"normal_high":i=e.filter(n=>n.rarity==="normal"&&Je(n)>=6),i.length||(i=e.filter(n=>Je(n)>=6));break;default:i=e.filter(n=>n.rarity==="normal"&&Je(n)>=1&&Je(n)<=5),i.length||(i=e.filter(n=>n.rarity==="normal"));break}return i.length||(i=e),i[Math.floor(Math.random()*i.length)]}async function ai(e,t){if(t.cost>0){const{error:g}=await w.from("users").update({credits:e.credits-t.cost}).eq("id",e.id);if(g)throw g}const i=t.allow_duplicates!==!1;let n=[];const{data:a,error:o}=await w.from("cards").select("player_id, card_type, formation, stadium_id, gc_type").eq("owner_id",e.id);if(o){const{data:g}=await w.from("cards").select("player_id, card_type, formation, gc_type").eq("owner_id",e.id);n=g||[]}else n=a||[];const r=new Set(n.filter(g=>g.card_type==="player").map(g=>g.player_id)),s=new Set(n.filter(g=>g.card_type==="formation").map(g=>g.formation)),l=new Set(n.filter(g=>g.card_type==="game_changer").map(g=>g.gc_type)),c=new Set(n.filter(g=>g.card_type==="stadium").map(g=>g.stadium_id).filter(Boolean)),d=new Set,u=[];for(let g=0;g<(t.cardCount||5);g++){const x=uo(t.rates);if(x){if(x.card_type==="player"){const f=E=>({légende:"legende",pépite:"pepite",pépites:"pepite"})[E]||E,p=x.rarity?f(x.rarity):null;let m=w.from("players").select("id,job,firstname,surname_real,country_code,club_id,rarity,note_g,note_d,note_m,note_a,skin,hair,hair_length,face,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);p&&(m=m.eq("rarity",p));const{data:b}=await m;let v=b||[];if((x.note_min||x.note_max)&&(v=v.filter(E=>{const G=Math.max(Number(E.note_g)||0,Number(E.note_d)||0,Number(E.note_m)||0,Number(E.note_a)||0);return(!x.note_min||G>=x.note_min)&&(!x.note_max||G<=x.note_max)})),v.length||(x.note_min||x.note_max?(v=b||[],console.warn("[Booster] Aucun joueur avec note",x.note_min,"-",x.note_max,"— fallback rareté uniquement")):v=b||[]),!v.length)continue;let h=v.filter(E=>!d.has(E.id));if(i)h.length||(h=v);else if(h=h.filter(E=>!r.has(E.id)),!h.length)continue;const $=h[Math.floor(Math.random()*h.length)];d.add($.id);const y=r.has($.id),{data:B}=await w.from("cards").insert({owner_id:e.id,player_id:$.id,card_type:"player"}).select().single();B&&(u.push({...B,player:$,isDuplicate:y}),w.rpc("record_transfer",{p_card_id:B.id,p_player_id:$.id,p_club_name:e.club_name||e.pseudo,p_manager_name:e.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{}))}else if(x.card_type==="game_changer"){const{data:f}=await w.from("gc_definitions").select("id,name").eq("is_active",!0).eq("gc_type","game_changer"),p=f!=null&&f.length?f:[{name:"Ressusciter"},{name:"Double attaque"},{name:"Bouclier"},{name:"Vol de note"},{name:"Gel"}],m=i?p:p.filter($=>!l.has($.name));if(!i&&!m.length)continue;const v=m[Math.floor(Math.random()*m.length)].name,{data:h}=await w.from("cards").insert({owner_id:e.id,card_type:"game_changer",gc_type:v}).select().single();h&&u.push(h)}else if(x.card_type==="formation"){const f=rn(),p=i?f:f.filter(h=>!s.has(h));if(!i&&!p.length)continue;const m=p[Math.floor(Math.random()*p.length)],b=s.has(m),{data:v}=await w.from("cards").insert({owner_id:e.id,card_type:"formation",formation:m}).select();v!=null&&v[0]&&u.push({...v[0],isDuplicate:b})}else if(x.card_type==="stadium"){const{data:f,error:p}=await w.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)");if(p){console.error("[Booster] stadium_definitions:",p.message);continue}if(!(f!=null&&f.length)){console.warn("[Booster] Aucun stade en DB");continue}const m=i?f:f.filter($=>!c.has($.id));if(!i&&!m.length)continue;const b=m[Math.floor(Math.random()*m.length)],{data:v,error:h}=await w.from("cards").insert({owner_id:e.id,card_type:"stadium",stadium_id:b.id}).select("id,card_type,stadium_id").single();if(h){console.error("[Booster] insert stadium card:",h.message);continue}v&&u.push({...v,rarity:"normal",_stadiumDef:b})}}}return u}async function sn(e,t,i){if(i>0){const{error:c}=await w.from("users").update({credits:e.credits-i}).eq("id",e.id);if(c)throw c}const{data:n}=await w.from("players").select("id,job,firstname,surname_real,country_code,club_id,rarity,note_g,note_d,note_m,note_a,note_min,note_max,skin,hair,hair_length,face,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);if(!(n!=null&&n.length))throw new Error("Pas de joueurs en BDD — ajoutes-en via le panel admin !");const a=n.filter(c=>c.job==="GK"),o=n.filter(c=>c.job!=="GK"),r=!e.first_booster_opened&&a.length>0,s=[];for(let c=0;c<t;c++){const d=c===0&&r?a:c===0?o:n,u=yo(),g=bo(d,u);g&&s.push(g)}r&&await w.from("users").update({first_booster_opened:!0}).eq("id",e.id);const{data:l}=await w.from("cards").insert(s.map(c=>({owner_id:e.id,player_id:c.id,card_type:"player"}))).select();return(l||[]).forEach((c,d)=>{w.rpc("record_transfer",{p_card_id:c.id,p_player_id:s[d].id,p_club_name:e.club_name||e.pseudo,p_manager_name:e.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{})}),s.map((c,d)=>({...l[d],player:c}))}async function dn(e,t,i){const{error:n}=await w.from("users").update({credits:e.credits-i}).eq("id",e.id);if(n)throw n;const{data:a}=await w.from("gc_definitions").select("id,name,gc_type,color,effect,image_url").eq("is_active",!0),o=a!=null&&a.length?a:Object.keys(ri).map(d=>({name:d,gc_type:"game_changer"})),r=Array.from({length:t},()=>{const d=o[Math.floor(Math.random()*o.length)];return{owner_id:e.id,card_type:"game_changer",gc_type:d.name,gc_definition_id:d.id||null}}),{data:s,error:l}=await w.from("cards").insert(r).select();return l&&console.error("[Booster GC] Erreur insert:",l.message,l),(s||[]).map(d=>{const u=a==null?void 0:a.find(g=>g.name===d.gc_type||g.id===d.gc_definition_id);return{...d,_gcDef:u||null}})}async function ln(e,t){const{error:i}=await w.from("users").update({credits:e.credits-t}).eq("id",e.id);if(i)throw i;const{data:n}=await w.from("cards").select("formation").eq("owner_id",e.id).eq("card_type","formation"),a=new Set((n||[]).map(d=>d.formation)),o=rn(),r=o[Math.floor(Math.random()*o.length)],s=a.has(r),{data:l,error:c}=await w.from("cards").insert({owner_id:e.id,card_type:"formation",formation:r}).select();return c&&console.error("[Booster Formation] Erreur insert:",c.message,c),(l||[]).map(d=>({...d,isDuplicate:s}))}function cn(e,t,i,n=null){var Q,I;if(!e||e.length===0){const _=document.createElement("div");_.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:3000;gap:16px;color:#fff;padding:24px;text-align:center",_.innerHTML=`
      <div style="font-size:48px">😕</div>
      <div style="font-size:20px;font-weight:900">Aucune carte obtenue</div>
      <div style="font-size:13px;color:rgba(255,255,255,0.5)">Erreur lors du tirage (permissions DB ou colonne manquante)</div>
      <button style="margin-top:10px;padding:12px 28px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer" id="anim-close-err">Fermer</button>`,document.body.appendChild(_),(Q=_.querySelector("#anim-close-err"))==null||Q.addEventListener("click",()=>_.remove());return}e=[...e].sort((_,j)=>{const U=_.player?Je(_.player):-1;return(j.player?Je(j.player):-1)-U});const a=document.createElement("div");a.id="booster-anim-overlay",a.innerHTML=`
    <style>
  ...
  `,document.body.appendChild(a);let o=!1;const r=document.getElementById("pack-cut-zone"),s=document.getElementById("pack-blade");let l=!1;const c=_=>_.touches&&_.touches[0]?_.touches[0].clientX:_.clientX;function d(_){o||(l=!0,s.style.opacity="1",u(_))}function u(_){if(!l||o)return;const j=r.getBoundingClientRect(),U=c(_)-j.left,le=Math.max(0,Math.min(1,U/j.width));s.style.width=le*j.width+"px",le>=.82&&x()}function g(){o||(l=!1,s.style.transition="width .2s ease, opacity .2s ease",s.style.width="0",s.style.opacity="0",setTimeout(()=>{o||(s.style.transition="")},220))}function x(){var j;if(o)return;o=!0,l=!1,s.style.width="100%",s.style.opacity="1",(j=document.getElementById("cut-flash"))==null||j.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const _=document.getElementById("cut-hint");_&&(_.style.opacity="0"),r.classList.add("pack-cut"),setTimeout(()=>{s.style.opacity="0",document.getElementById("pack-phase").style.display="none",b(0)},620)}r.addEventListener("pointerdown",d),window.addEventListener("pointermove",u),window.addEventListener("pointerup",g),r.addEventListener("touchstart",d,{passive:!0}),window.addEventListener("touchmove",u,{passive:!0}),window.addEventListener("touchend",g);let f=0,p=!1;const m=new Set;function b(_){f=_,document.getElementById("reveal-phase").style.display="flex",v(),h(_,0),G()}function v(){const _=document.getElementById("card-dots");_&&(_.innerHTML=e.map((j,U)=>`<div class="card-dot" data-i="${U}" style="width:8px;height:8px;border-radius:50%;background:${U===f?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),_.querySelectorAll(".card-dot").forEach(j=>j.addEventListener("click",()=>y(parseInt(j.dataset.i)))))}function h(_,j){var P,M;const U=e[_],le=document.getElementById("card-counter"),ae=document.getElementById("card-track");le&&(le.textContent=`Carte ${_+1} / ${e.length}`);const pe=document.getElementById("reveal-btns");pe&&(pe.style.display=_===e.length-1?"flex":"none");const se=U.card_type==="player"&&((P=U.player)==null?void 0:P.rarity)==="legende",Ee=!m.has(_);m.add(_);let Y=0;if(U.card_type==="player"&&U.player){const C=U.player,F=C.job||"ATT";Y=Number(F==="GK"?C.note_g:F==="DEF"?C.note_d:F==="MIL"?C.note_m:C.note_a)||0}const J=C=>{ae.innerHTML=`
        <div id="current-card-wrap" style="position:relative;display:flex;flex-direction:column;align-items:center;gap:8px;${se?"filter:drop-shadow(0 0 20px #7a28b8)":""}">
          <div style="transform:scale(1.25);transform-origin:center">${ho(U)}</div>
        ...
        </div>`;const F=document.getElementById("current-card-wrap");j!==0?(F.style.transition="none",F.style.transform=`translateX(${j>0?100:-100}%)`,requestAnimationFrame(()=>{F.style.transition="transform .28s cubic-bezier(.25,1,.5,1)",F.style.transform="translateX(0)"})):F.animate([{opacity:0,transform:"scale(.7)"},{opacity:1,transform:"scale(1)"}],{duration:300,easing:"cubic-bezier(.34,1.56,.64,1)"}),C||se?ne():te(),v()};Ee&&(((M=U.player)==null?void 0:M.rarity)==="legende"||Y>=8)&&U.card_type==="player"&&U.player?$(U,()=>J(!0)):J(!1)}function $(_,j){var be;p=!0;const U=_.player,le=`https://flagsapi.com/${U.country_code}/flat/64.png`,ae=(be=U.clubs)==null?void 0:be.logo_url,pe=U.face?"/"+U.face.replace(/^public\//,"").replace(/^\//,""):null,se=U.job||"ATT",Ee=Number(se==="GK"?U.note_g:se==="DEF"?U.note_d:se==="MIL"?U.note_m:U.note_a)||0,Y=_.evolution_bonus||0,J=Ee+Y,P=U.rarity==="legende",M=J>=18,C=document.getElementById("walkout-overlay"),F=document.getElementById("walkout-stage");if(!C||!F){p=!1,j();return}let W=null;P&&(W=new Audio("/sounds/Legendary.mp3"),W.volume=.8,W.play().catch(()=>{})),C.style.display="flex";const K=()=>{const he=F.firstElementChild;he&&(he.classList.remove("wo-in"),he.classList.add("wo-out"))},q=1800,O=400;F.innerHTML=`<img class="wo-in" src="${le}" style="height:130px;border-radius:10px;box-shadow:0 10px 36px rgba(0,0,0,.6)" onerror="this.style.display='none'">`,navigator.vibrate&&navigator.vibrate(30),setTimeout(K,q),setTimeout(()=>{var he;F.innerHTML=ae?`<img class="wo-in" src="${ae}" style="max-height:160px;max-width:210px;object-fit:contain">`:`<div class="wo-in" style="font-size:34px;font-weight:900;color:#fff;text-align:center">${((he=U.clubs)==null?void 0:he.encoded_name)||"CLUB"}</div>`,navigator.vibrate&&navigator.vibrate(30)},q+O),setTimeout(K,q*2+O),setTimeout(()=>{F.innerHTML=pe?`<img class="wo-in" src="${pe}" style="height:200px;border-radius:50%;box-shadow:0 0 40px rgba(255,255,255,0.3);object-fit:cover;object-position:top">`:'<div class="wo-in" style="font-size:80px">👤</div>',navigator.vibrate&&navigator.vibrate(30)},(q+O)*2),setTimeout(K,(q+O)*2+q);const _e=M?"#FFD700":{GK:"#c0c0c0",DEF:"#e03030",MIL:"#D4A017",ATT:"#3fbf5f"}[se]||"#fff";setTimeout(()=>{F.innerHTML=`<div class="wo-in" style="
        ...
        ${J}
      </div>`,M&&navigator.vibrate&&navigator.vibrate([50,30,100,30,200])},(q+O)*3),setTimeout(K,(q+O)*3+q),setTimeout(()=>{C.style.display="none",F.innerHTML="",p=!1,W&&!P&&W.pause(),navigator.vibrate&&navigator.vibrate([40,30,80]),j()},(q+O)*4)}function y(_){if(p||_<0||_>=e.length||_===f)return;const j=_>f?1:-1;f=_,h(_,j)}function B(){y(f+1)}function E(){y(f-1)}function G(){const _=document.getElementById("card-viewport");if(!_||_._swipeBound)return;_._swipeBound=!0;let j=0,U=0,le=0,ae=!1;const pe=P=>P.touches?P.touches[0].clientX:P.clientX,se=P=>P.touches?P.touches[0].clientY:P.clientY,Ee=P=>{ae=!0,j=pe(P),U=se(P),le=0},Y=P=>{if(!ae)return;le=pe(P)-j;const M=se(P)-U;if(Math.abs(le)<Math.abs(M))return;const C=document.getElementById("current-card-wrap");C&&(C.style.transition="none",C.style.transform=`translateX(${le*.6}px) rotate(${le*.02}deg)`)},J=()=>{if(!ae)return;ae=!1;const P=document.getElementById("current-card-wrap"),M=55;le<=-M&&f<e.length-1?B():le>=M&&f>0?E():P&&(P.style.transition="transform .2s ease",P.style.transform="translateX(0)")};_.addEventListener("pointerdown",Ee),_.addEventListener("pointermove",Y),_.addEventListener("pointerup",J),_.addEventListener("pointercancel",J),_.addEventListener("touchstart",Ee,{passive:!0}),_.addEventListener("touchmove",Y,{passive:!0}),_.addEventListener("touchend",J),_.addEventListener("click",P=>{if(Math.abs(le)>8)return;const M=_.getBoundingClientRect();P.clientX-M.left>M.width/2?B():E()})}let N=null;function ne(){const _=document.getElementById("fireworks-canvas");if(!_)return;_.width=window.innerWidth,_.height=window.innerHeight;const j=_.getContext("2d"),U=[];function le(){const pe=Math.random()*_.width,se=Math.random()*_.height*.6,Ee=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],Y=Ee[Math.floor(Math.random()*Ee.length)];for(let J=0;J<60;J++){const P=Math.PI*2/60*J,M=2+Math.random()*5;U.push({x:pe,y:se,vx:Math.cos(P)*M,vy:Math.sin(P)*M,alpha:1,color:Y,size:2+Math.random()*3})}}le(),N=setInterval(le,600);function ae(){if(document.getElementById("fireworks-canvas")){j.clearRect(0,0,_.width,_.height);for(let pe=U.length-1;pe>=0;pe--){const se=U[pe];if(se.x+=se.vx,se.y+=se.vy+.08,se.vy*=.98,se.alpha-=.018,se.alpha<=0){U.splice(pe,1);continue}j.globalAlpha=se.alpha,j.fillStyle=se.color,j.beginPath(),j.arc(se.x,se.y,se.size,0,Math.PI*2),j.fill()}j.globalAlpha=1,(N!==null||U.length>0)&&requestAnimationFrame(ae)}}ae()}function te(){N!==null&&(clearInterval(N),N=null);const _=document.getElementById("fireworks-canvas");_&&_.getContext("2d").clearRect(0,0,_.width,_.height)}if(n){const _=document.getElementById("reveal-btns");_&&(_.innerHTML='<button class="btn btn-primary" id="reveal-next" style="flex:1">Continuer →</button>'),(I=document.getElementById("reveal-next"))==null||I.addEventListener("click",()=>{te(),a.remove(),n()})}else document.getElementById("reveal-collection").addEventListener("click",()=>{te(),a.remove(),i("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{te(),a.remove(),i("boosters")})}function ho(e){var t,i,n,a;if(e.card_type==="player"&&e.player)return go(e);if(e.card_type==="game_changer"){const o=e._gcDef,r=(o==null?void 0:o.gc_type)==="ultra_game_changer",s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},l={purple:"#b06ce0",light_blue:"#00d4ef"},c=s[o==null?void 0:o.color]||s.purple,d=l[o==null?void 0:o.color]||l.purple,u=(o==null?void 0:o.name)||e.gc_type||"Game Changer",g=(o==null?void 0:o.effect)||((t=ri[e.gc_type])==null?void 0:t.desc)||"",x=o!=null&&o.image_url?`/icons/${o.image_url}`:null,f=((i=ri[e.gc_type])==null?void 0:i.icon)||"⚡";return`<div style="width:170px;height:240px;background:${c};border-radius:14px;border:3px solid ${d};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${d}88;flex-shrink:0">
      ...
      <div style="flex:1;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06);margin:0">
        ${x?`<img src="${x}" style="max-width:120px;max-height:120px;width:auto;height:auto;object-fit:contain;border-radius:6px">`:`<span style="font-size:56px">${f}</span>`}
      </div>
      ...
      </div>`,document.body.appendChild(n),n.addEventListener("click",r=>{r.target===n&&n.remove()}),(i=document.getElementById("odds-close"))==null||i.addEventListener("click",()=>n.remove());return}wo()}function wo(){const e=document.createElement("div");e.style.cssText=`position:fixed;inset:0;background:rgba(0,0,0,0.6);display:flex;
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}function _o(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText=`
    ...
    `,document.body.appendChild(t);let i=5;const n=setInterval(()=>{i--;const a=document.getElementById("mw-ad-cd");a&&(a.textContent=i),i<=0&&(clearInterval(n),t.remove(),e(!0))},1e3)})}async function ko(e,{state:t,navigate:i,toast:n,refreshProfile:a}){var g,x;const{data:o}=await w.from("users").select("*").eq("id",t.user.id).single();o&&(t.profile=o);let r=Array.isArray((g=t.profile)==null?void 0:g.pending_boosters)?[...t.profile.pending_boosters]:[];if(!r.length){await w.from("users").update({onboarding_done:!0}).eq("id",t.user.id),i("home");return}let s=null;try{const p=(await on()).find(m=>(m.name||"").toLowerCase().includes("new player"));p&&(s=an(p))}catch(f){console.warn('[Onboarding] Config "Booster (new player)" introuvable, fallback taux par défaut',f)}const l=r.length;let c=0;e.innerHTML=`
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    </div>`}const l=d=>{e.style.overflow="",e.style.height="",e.style.display="",e.style.flexDirection="",i(d)};function c(){var u,g,x;e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column";const d=r.length>0;e.innerHTML=`
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      <div style="flex:1;overflow-y:auto;display:flex;flex-wrap:wrap;gap:10px;justify-content:center;align-content:flex-start;padding:8px 16px 16px">
        ${o.map(f=>{const p=r.find(m=>m.gc_type===f.gc_type);return s(f,!!p)}).join("")}
      </div>
    ...
    </div>`,e.querySelectorAll(".gc-select-card").forEach(f=>{f.addEventListener("click",()=>{const p=f.dataset.id,m=o.find(v=>v.id===p);if(!m)return;const b=r.findIndex(v=>v.gc_type===m.gc_type);b>-1?r.splice(b,1):r.length<3&&r.push(m),c()})}),(u=e.querySelector("#gc-launch"))==null||u.addEventListener("click",()=>{d&&l(r)}),(g=e.querySelector("#gc-no-gc"))==null||g.addEventListener("click",()=>l([])),(x=e.querySelector("#gc-reset"))==null||x.addEventListener("click",()=>{r.length&&(r=[],c())})}c()}function Lo(e){var n;const t=((n=e==null?void 0:e.state)==null?void 0:n.params)||{},i=t.matchMode||"vs_ai_easy";return i==="friend"?`Match vs ${t.friendName||"Ami"}`:i==="random"?"Match vs Random":`Match vs IA — ${i.replace("vs_ai_","").toUpperCase()}`}async function To(e,t,i){const{state:n,navigate:a}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await w.from("decks").select(`id,name,is_active,formation,stadium_card_id,
      ...
      card:cards(id,card_type,formation,stadium_id,evolution_bonus,
        player:players(id,firstname,surname_real,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
          clubs(encoded_name,logo_url)))`).in("deck_id",r).order("slot_order"),l=[...new Set((s||[]).filter(g=>{var x,f;return((x=g.card)==null?void 0:x.card_type)==="stadium"&&((f=g.card)==null?void 0:f.stadium_id)}).map(g=>g.card.stadium_id))],c={};if(l.length){const{data:g}=await w.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",l);(g||[]).forEach(x=>{c[x.id]=x}),(s||[]).forEach(x=>{var f,p;((f=x.card)==null?void 0:f.card_type)==="stadium"&&((p=x.card)!=null&&p.stadium_id)&&(x.card._stadiumDef=c[x.card.stadium_id]||null)})}let d=0;function u(){var y,B,E,G,N,ne,te;const g=o[d],x=(s||[]).filter(Q=>Q.deck_id===g.id),f=x.filter(Q=>{var I;return Q.is_starter&&((I=Q.card)==null?void 0:I.player)}).map(Q=>si(Q.card,Q.position)),p=x.find(Q=>{var I;return((I=Q.card)==null?void 0:I.card_type)==="formation"}),m=g.formation||((y=p==null?void 0:p.card)==null?void 0:y.formation)||"4-4-2";let b=f.length>=11?Ut(f,m):null,v=((B=g.stadium_card)==null?void 0:B.stadium_def)||null;v&&b&&(b=Ct(b,v));const h=f.length>=11;Ke(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff;position:relative">
      ...
      <div id="deck-top-bar" style="flex-shrink:0">
        <div style="padding:8px 16px;background:rgba(0,0,0,0.4);text-align:center">
          <div style="font-size:10px;opacity:.6;letter-spacing:2px;text-transform:uppercase">${Lo(t)}</div>
        ...
        <div style="display:flex;align-items:center;gap:8px;padding:6px 8px">
          ...
          <div style="flex:1;text-align:center">
            ...
            <div style="font-size:11px;opacity:.6">${m} · ${f.length}/11${g.is_active?" · ⭐":""}</div>
          </div>
      ...
      <!-- Pagination -->
      ${o.length>1?`<div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">${o.map((Q,I)=>`<div style="width:6px;height:6px;border-radius:50%;background:${I===d?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}</div>`:""}
    ...
    </div>`,requestAnimationFrame(()=>requestAnimationFrame(function Q(){const I=e.querySelector(".deck-preview-wrap"),_=e.querySelector("#deck-swipe-zone");if(!I||!_||!b)return;const j=_.clientWidth>=900,U=Math.max(200,_.clientHeight-(j?60:40)),ae=Math.max(200,_.clientWidth-16),pe=j?Math.min(117,Math.max(52,Math.round(ae*.22))):Math.max(44,Math.round(ae*.168));if(U<220||ae<220){requestAnimationFrame(Q);return}const se=j?null:Math.round(pe*.55);I.innerHTML=kt(b,m,null,[],ae,U,[],se),I.style.cssText=`width:${ae}px;height:${U}px;overflow:hidden;margin:${j?0:60}px auto 0`;const Ee=I.querySelector("svg");Ee&&(Ee.style.cssText="display:block;width:100%;height:100%",Ee.setAttribute("preserveAspectRatio",j?"xMidYMid meet":"none"))})),(G=document.getElementById("prev-deck"))==null||G.addEventListener("click",()=>{d>0&&(d--,u())}),(N=document.getElementById("next-deck"))==null||N.addEventListener("click",()=>{d<o.length-1&&(d++,u())}),(ne=document.getElementById("validate-deck"))==null||ne.addEventListener("click",()=>{if(!h)return;const Q=t.state.params||{};t.navigate("match",{...Q,matchMode:Q.matchMode||i,deckId:g.id})}),(te=document.getElementById("cancel-deck-select"))==null||te.addEventListener("click",()=>{Re(e),a("home")});const $=document.getElementById("deck-swipe-zone");if($){let Q=0,I=0;$.addEventListener("touchstart",_=>{Q=_.touches[0].clientX,I=_.touches[0].clientY},{passive:!0}),$.addEventListener("touchend",_=>{const j=_.changedTouches[0].clientX-Q,U=_.changedTouches[0].clientY-I;Math.abs(j)<40||Math.abs(j)<Math.abs(U)||(j<0&&d<o.length-1?(d++,u()):j>0&&d>0&&(d--,u()))},{passive:!0})}}u()}function tt(e,t=44,i=58,n=null){return Se(e,{width:t,showStad:!!n,stadDef:n,used:e==null?void 0:e.used})}function yt(e,t,i){if(!(e!=null&&e.length))return"";const n=e.slice(0,5);let a='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return n.forEach((o,r)=>{const s=o._line||o.job||"MIL";if(a+=Se(o,{width:40,role:s,extraNote:o.boost||0}),r<n.length-1){const l=qt(o,n[r+1]);a+=`<div style="width:7px;height:3px;background:${l==="#ff3333"||l==="#cc2222"?"rgba(255,255,255,0.12)":l};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(a+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),a+="</div>",a}function Wt(e,t,i,n,a=310,o=310,r=[],s=null){const l=Bt[t]||{},c=Yi(t)||fi[t]||[],d={},u=["ATT","MIL","DEF","GK"];for(const b of u)(e[b]||[]).forEach((h,$)=>{d[`${b}${$+1}`]=h});function g(b){const v=l[b];return v?{x:v.x*a,y:v.y*o}:null}let x="";for(const[b,v]of c){const h=g(b),$=g(v);if(!h||!$)continue;const y=d[b],B=d[v],E=qt(y,B);E==="#00ff88"||E==="#FFD700"?(x+=`<line x1="${h.x.toFixed(1)}" y1="${h.y.toFixed(1)}" x2="${$.x.toFixed(1)}" y2="${$.y.toFixed(1)}"
        stroke="${E}" stroke-width="10" stroke-linecap="round" opacity="0.22"/>`,x+=`<line x1="${h.x.toFixed(1)}" y1="${h.y.toFixed(1)}" x2="${$.x.toFixed(1)}" y2="${$.y.toFixed(1)}"
        stroke="${E}" stroke-width="3.5" stroke-linecap="round" opacity="0.95"/>`):x+=`<line x1="${h.x.toFixed(1)}" y1="${h.y.toFixed(1)}" x2="${$.x.toFixed(1)}" y2="${$.y.toFixed(1)}"
        stroke="${E}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const f=typeof window<"u"&&window.innerWidth>=900?Math.min(Math.max(81,Math.round(a*.225)),117):Math.max(44,Math.round(a*.168)),p=Math.round(f*657/507);for(const[b,v]of Object.entries(d)){const h=g(b);if(!h||!v)continue;const $=b.replace(/[0-9]/g,""),y=r.includes(v.cardId),B=i==="attack"&&(["MIL","ATT"].includes($)||y)&&!v.used||i==="defense"&&["GK","DEF","MIL"].includes($)&&!v.used,E=n.includes(v.cardId);let G=v.boost||0,N=!1;v.stadiumBonus&&(i==="attack"&&($==="ATT"||$==="MIL")||i==="defense"&&($==="GK"||$==="DEF"||$==="MIL")?(G+=10,N=!0):i||(G+=10,N=!0));const ne=Math.round(h.x-f/2),te=Math.round(h.y-p/2);if(v.used){x+=`<image href="${`${typeof import.meta<"u"&&"/"||"/"}icons/carte-dos.png`}" x="${ne}" y="${te}" width="${f}" height="${p}" preserveAspectRatio="xMidYMid slice" class="match-used-hit" data-card-id="${v.cardId}" data-role="${$}" style="cursor:pointer"/>`;continue}const Q=Se({...v,_evolution_bonus:0,stadiumBonus:!1},{width:f,showStad:!1,stadDef:null,role:$,extraNote:G,_cardOffset:30,_forceStadColor:N}),I=E?`position:absolute;top:30px;left:0;width:${f}px;height:${p}px;outline:3px solid #FFD700;outline-offset:2px;border-radius:8px;pointer-events:none;`:"";x+=`<foreignObject x="${ne-2}" y="${te-30}" width="${f+8}" height="${p+60}" style="overflow:visible">
      <div xmlns="http://www.w3.org/1999/xhtml" style="position:relative">
        ${Q}
        ${E?`<div style="${I}"></div>`:""}
      </div>
    </foreignObject>`,B&&(x+=`<rect x="${ne}" y="${te}" width="${f}" height="${p}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${E?"selected":""}" data-card-id="${v.cardId}" data-role="${$}" style="cursor:pointer"/>`)}const m=s!==null?s:Math.round(Math.max(f*.7,80));return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-m} ${-m} ${a+m*2} ${o+m*2}" width="100%" style="display:block;width:100%;margin:0 auto">
    ${x}
    ...
    ${Wt(e,t,i,n,a,o,r,s)}
          ...
          card:cards(id, card_type, formation, evolution_bonus,
            player:players(id,firstname,surname_real,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
              clubs(encoded_name,logo_url)))`).eq("deck_id",l).order("slot_order")]);c=E[0].data,u=E[0].error,d=E[1].data,g=E[1].error}catch(E){console.error("[Match] Exception chargement deck:",E),dt(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>o("home"));return}if(u||g){console.error("[Match] Erreur Supabase:",u||g),dt(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>o("home"));return}const x=(d||[]).filter(E=>{var G;return E.is_starter&&((G=E.card)==null?void 0:G.player)}).map(E=>si(E.card,E.position)),f=(d||[]).filter(E=>{var G;return!E.is_starter&&((G=E.card)==null?void 0:G.player)}).map(E=>si(E.card,E.position));if(x.length<11){dt(e,"⚠️",`Deck incomplet (${x.length}/11).`,"Compléter",()=>o("decks"));return}const p=(d||[]).find(E=>{var G;return((G=E.card)==null?void 0:G.card_type)==="formation"}),m=(c==null?void 0:c.formation)||((B=p==null?void 0:p.card)==null?void 0:B.formation)||"4-4-2",{data:b,error:v}=await w.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",a.profile.id).eq("card_type","game_changer"),{data:h}=await w.from("gc_definitions").select("*").eq("is_active",!0),$=(b||[]).map(E=>({...E,_gcDef:(h==null?void 0:h.find(G=>G.name===E.gc_type||G.id===E.gc_definition_id))||null}));let y=null;if(c!=null&&c.stadium_card_id){const{data:E}=await w.from("cards").select("stadium_id").eq("id",c.stadium_card_id).single();if(E!=null&&E.stadium_id){const{data:G}=await w.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").eq("id",E.stadium_id).single();y=G||null}}n({deckId:l,formation:m,starters:x,subsRaw:f,gcCardsEnriched:$,gcDefs:h||[],stadiumDef:y})}function lt(){return Math.min(window.innerWidth-40,860)}function bt(){return Math.round(lt()*1.1)}function un(e){var a,o;if(!e)return null;const t=e._line||e.job||"MIL",i=t==="GK"?e.note_g||0:t==="DEF"?e.note_d||0:t==="MIL"?e.note_m||0:e.note_a||0,n=e.stadiumBonus?10:0;return{name:e.name,firstname:e.firstname||"",note:i+(e.boost||0)+n,note_g:e.note_g||0,note_d:e.note_d||0,note_m:e.note_m||0,note_a:e.note_a||0,_evolution_bonus:0,stadiumBonus:e.stadiumBonus||!1,boost:e.boost||0,job:e.job,job2:e.job2||null,_line:e._line||e.job,_col:e._col,country_code:e.country_code,club_id:e.club_id,rarity:e.rarity,clubName:e.clubName||((a=e.clubs)==null?void 0:a.encoded_name)||null,clubLogo:e.clubLogo||((o=e.clubs)==null?void 0:o.logo_url)||null,face:e.face||null,portrait:jt(e)}}function zo(e,t,i,n="Adversaire"){const a=lt(),o=bt();return`
    ...
    </div>`}function fn(e){var o,r,s;if(!e)return"";const t=l=>l?Se({...l,_evolution_bonus:0},{width:52,role:l._line||l.job,showStad:!!l.stadiumBonus,extraNote:l.boost||0}):"",n={goal:"⚽","goal-home":"⚽","goal-ai":"⚽",duel:"⚔️",midfield:"🎯",sub:"🔄",gc:"⚡",boost:"💥","defense-won":"🛡️","attack-won":"⚔️","defense-lost":"😓","attack-lost":"😓"}[e.type]||"📋";return`
    ...
    pointer-events:none;white-space:nowrap;max-width:90vw;text-align:center`,a.textContent=e,!document.getElementById("game-toast-style")){const o=document.createElement("style");o.id="game-toast-style",o.textContent=`
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${e.map((o,r)=>{const s=r<e.length-1?qt(o,e[r+1]):null,l=!s||s==="#ff3333"||s==="#cc2222",c=s==="#00ff88"?"+2":s==="#FFD700"?"+1":"";return gn(o,a),o.stadiumBonus||a&&(a.club_id&&String(o.club_id)===String(a.club_id)||a.country_code&&(o.country_code,a.country_code)),`
    ...
    </div>`}async function So(e,t){const{state:i}=t,a=(i.params||{}).matchMode||"vs_ai_easy",o=a.replace("vs_ai_",""),r=a;await Dt(e,t,a,async({deckId:s,formation:l,starters:c,subsRaw:d,gcCardsEnriched:u,gcDefs:g,stadiumDef:x})=>{try{let f=Ut(c,l);x&&(f=Ct(f,x),Ht(d,x));const p=await Io(l,o),m=p.lines||p,b=async v=>{try{const{data:h,error:$}=await w.from("matches").insert({home_id:i.profile.id,away_id:null,mode:r,home_deck_id:s,status:"in_progress"}).select().single();if($){console.error("[MatchIA] Erreur création match:",$),dt(e,"⚠️","Impossible de créer le match ("+$.message+").","Retour",()=>t.navigate("home"));return}const y=p.stadiumDef||null;y&&m&&(Ct(m,y),Ht(p.subs||[],y));const B={gcDefs:g||[],matchId:h==null?void 0:h.id,mode:r,difficulty:o,formation:l,homeTeam:f,aiTeam:m,homeSubs:d,subsUsed:0,maxSubs:Math.min(d.length,3),aiSubs:p.subs||[],aiSubsUsed:0,aiMaxSubs:Math.min((p.subs||[]).length,3),aiUsedSubIds:[],aiGcCards:p.gcCards||[],aiUsedGc:[],aiStadiumDef:y,homeScore:0,aiScore:0,gcCards:v,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};jo(e,B,t)}catch(h){console.error("[MatchIA] Exception launchMatch:",h),dt(e,"⚠️","Erreur au lancement du match : "+h.message,"Retour",()=>t.navigate("home"))}};if(!u.length){b([]);return}vi(e,u,b)}catch(f){console.error("[MatchIA] Exception setup:",f),dt(e,"⚠️","Erreur de préparation du match : "+f.message,"Retour",()=>t.navigate("home"))}})}async function Io(e,t){var m;const{data:i}=await w.from("players").select("id,firstname,surname_real,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(80);if(!i||i.length<11)return{lines:Ao(e),subs:[],gcCards:[],stadiumDef:null};const n=wt[e]||wt["4-4-2"],a={GK:[],DEF:[],MIL:[],ATT:[]},o=new Set;function r(b,v,h){var $,y;return o.add(b.id),{cardId:"ai-"+b.id+"-"+h,id:b.id,firstname:b.firstname,name:b.surname_real,country_code:b.country_code,club_id:b.club_id,job:b.job,job2:b.job2,note_g:Number(b.note_g)||0,note_d:Number(b.note_d)||0,note_m:Number(b.note_m)||0,note_a:Number(b.note_a)||0,rarity:b.rarity,skin:b.skin,hair:b.hair,hair_length:b.hair_length,clubName:(($=b.clubs)==null?void 0:$.encoded_name)||null,clubLogo:((y=b.clubs)==null?void 0:y.logo_url)||null,boost:0,used:!1,_line:v}}for(const b of["GK","DEF","MIL","ATT"]){const v=i.filter(E=>E.job===b&&!o.has(E.id)),h=i.filter(E=>E.job!==b&&!o.has(E.id)),$=[...v,...h],y=[];for(let E=0;E<n[b];E++){const G=$[E];G&&y.push(r(G,b,E))}const B=_t(y.length);y.forEach((E,G)=>{E._col=B[G]}),a[b]=y}const l=i.filter(b=>!o.has(b.id)).slice(0,5).map((b,v)=>r(b,b.job,100+v)),u=Object.keys(Ie).sort(()=>Math.random()-.5).slice(0,3).map((b,v)=>{var h,$;return{id:"ai-gc-"+v,gc_type:b,name:((h=Ie[b])==null?void 0:h.name)||b,icon:(($=Ie[b])==null?void 0:$.icon)||"⚡"}}),g=Object.values(a).flat(),x={};g.forEach(b=>{b.club_id&&(x[b.club_id]=(x[b.club_id]||0)+1)});const f=(m=Object.entries(x).sort((b,v)=>v[1]-b[1])[0])==null?void 0:m[0];let p=null;if(f){const{data:b}=await w.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",f).single();b&&(p={club_id:b.id,country_code:null,name:b.encoded_name+" Stadium",club:{encoded_name:b.encoded_name,logo_url:b.logo_url}})}return{lines:a,subs:l,gcCards:u,stadiumDef:p}}function Ao(e){const t=wt[e]||wt["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},n=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let a=0;for(const o of["GK","DEF","MIL","ATT"]){const r=[];for(let l=0;l<t[o];l++){const c=3+Math.floor(Math.random()*5);r.push({cardId:"fake-"+a,id:"fake-"+a,firstname:"IA",name:n[a%n.length],country_code:"XX",club_id:null,job:o,job2:null,note_g:o==="GK"?c:2,note_d:o==="DEF"?c:2,note_m:o==="MIL"?c:2,note_a:o==="ATT"?c:2,rarity:"normal",boost:0,used:!1,_line:o}),a++}const s=_t(r.length);r.forEach((l,c)=>{l._col=s[c]}),i[o]=r}return i}function jo(e,t,i){var a;const n=`<div style="position:relative;width:22px;height:22px;flex-shrink:0;display:flex;align-items:center;justify-content:center">
  ...
  </div>`;const d=()=>{const f=(p,m)=>e.querySelectorAll(p).forEach((b,v)=>{setTimeout(()=>{b.style.opacity="1",b.style.transform="translateY(0) scale(1)"},m+v*90)});f(".duel-card-home",150),f(".duel-card-ai",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((p,m)=>{setTimeout(()=>{p.style.opacity="1"},m*70)}),900),setTimeout(()=>{const p=e.querySelector("#vs-label");p&&(p.style.opacity="1",p.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(m=>m.style.opacity="1")},1250),setTimeout(()=>{u("score-home",s,800),u("score-ai",l,800)},1500)};function u(f,p,m){const b=document.getElementById(f);if(!b)return;const v=performance.now(),h=$=>{const y=Math.min(1,($-v)/m);b.textContent=Math.round(p*(1-Math.pow(1-y,3))),y<1?requestAnimationFrame(h):b.textContent=p};requestAnimationFrame(h)}requestAnimationFrame(d),t.attacker=c?"home":"ai";const g=c?pn():null;c&&(t.boostCard={value:g}),t.log.push({type:"duel",title:"Milieu de Terrain",homePlayers:n.map(f=>Pe(f)),aiPlayers:a.map(f=>Pe(f)),homeTotal:s,aiTotal:l,text:`Duel milieu : ${t.clubName} ${s} – ${l} IA → ${c?t.clubName+" attaque":"IA attaque"}`});const x=()=>{t.phase=t.attacker==="home"?"attack":"ai-attack",$e(e,t,i),t.attacker==="ai"&&setTimeout(()=>_i(e,t,i),800)};setTimeout(()=>{const f=document.getElementById("score-home"),p=document.getElementById("score-ai"),m=document.getElementById(c?"duel-row-home":"duel-row-ai"),b=document.getElementById(c?"duel-row-ai":"duel-row-home"),v=c?f:p,h=c?p:f;v&&(v.style.fontSize="80px",v.style.color=c?"#FFD700":"#ff6b6b",v.style.animation="duelPulse .5s ease"+(c?", duelGlow 1.5s ease infinite .5s":"")),h&&(h.style.opacity="0.25"),setTimeout(()=>{m&&(m.style.transformOrigin="center",m.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",m.style.zIndex="5"),setTimeout(()=>{var y;const $=document.getElementById("duel-shock");if($){const B=(y=b||m)==null?void 0:y.getBoundingClientRect(),E=e.querySelector(".match-screen").getBoundingClientRect();B&&($.style.top=B.top-E.top+B.height/2+"px"),$.style.animation="shockwave .5s ease-out forwards"}b&&(b.style.transformOrigin="center",b.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var y;const $=document.getElementById("duel-finale");$&&($.innerHTML=`
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const j=t.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b;text-align:center">
            <div style="font-size:8px;color:#ff6b6b;letter-spacing:2px;margin-bottom:4px;text-transform:uppercase">⚔️ IA ATTAQUE — Défendez !</div>
            <div style="display:flex;justify-content:center">${yt((j.players||[]).map(U=>({...U,used:!1})),"#ff6b6b",j.total)}</div>
            ...
            <div style="display:flex;justify-content:center">${yt((j.players||[]).map(U=>({...U,used:!1})),"#00ff88",j.total)}</div>
          </div>`}const _=t.log[t.log.length-1];return _?'<div style="padding:2px 4px">'+fn(_)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})()}
    </div>
    ...
    ${(()=>{const _=window.innerWidth>=700,j=(P,M,C)=>{var ge,je;const F=(t.gcDefs||[]).find(Ae=>Ae.name===P.gc_type),W={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[F==null?void 0:F.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",K={purple:"#b06ce0",light_blue:"#00d4ef"}[F==null?void 0:F.color]||"#b06ce0",q=(F==null?void 0:F.name)||P.gc_type,O=(F==null?void 0:F.effect)||((ge=Ie[P.gc_type])==null?void 0:ge.desc)||"",we=F!=null&&F.image_url?`/icons/${F.image_url}`:null,_e=((je=Ie[P.gc_type])==null?void 0:je.icon)||"⚡",be=Math.round(C*.22),he=Math.round(C*.22),ke=C-be-he,Le=q.length>12?7:9;return`<div class="gc-mini" data-gc-id="${P.id}" data-gc-type="${P.gc_type}"
          ...
          <div style="height:${ke}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
            ${we?`<img src="${we}" style="max-width:${M-10}px;max-height:${ke-6}px;object-fit:contain">`:`<span style="font-size:${Math.round(ke*.55)}px">${_e}</span>`}
          </div>
        ...
        </div>`},U=(P,M)=>{var C;return`<div id="boost-card"
          style="box-sizing:border-box;width:${P}px;height:${M}px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:2px solid #87CEEB;border-radius:10px;cursor:pointer;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:${Math.round(M*.04)}px;text-align:center;flex-shrink:0">
          ...
          </div>`},le=(P,M)=>M?U(130,175):j(P,130,175),ae=(P,M)=>M?U(68,95):j(P,68,95),pe=_?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",se=x?`<button id="btn-results" style="${pe};background:linear-gradient(135deg,#D4A017,#FFD700);border:none;color:#000">🏁 Résultats</button>`:d?`<div style="${pe};background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);color:rgba(255,255,255,0.4)">⏳ Tour IA</div>`:m?`<button id="btn-pass" style="${pe};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER (plus d'attaquants)</button>`:u?`<button id="btn-action" style="${pe};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${t.selected.length===0?"disabled":""}> ⚔️ ATTAQUEZ <span id="match-timer" style="font-weight:900"></span></button>`:g?`<button id="btn-action" style="${pe};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${t.selected.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="match-timer" style="font-weight:900"></span></button>`:`<div style="${pe};background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1)"></div>`,Ee=u||g?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${t.selected.length}/3 sélectionné(s)</div>`:"",Y=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${_?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(_=>{var j,U,le;if(_.type==="duel"){const ae=_.isGoal,pe=_.homeScored?"#FFD700":ae?"#ff6b6b":"rgba(255,255,255,0.3)",se=_.homeScored?"⚽ BUT !":ae?"⚽ BUT IA !":(j=_.homePlayers)!=null&&j.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${ae?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${pe};margin-bottom:4px">
  ...
  </div>`;function h(){const _=e.querySelector(".match-screen");if(!_)return;const j=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);_.style.bottom="auto",_.style.height=j+"px",_.style.minHeight=j+"px",_.style.maxHeight=j+"px",_.style.overflow="hidden";const U=e.querySelector("#mobile-action-bar"),le=e.querySelector("#mobile-play-area");U&&le&&(le.style.paddingBottom=U.offsetHeight+"px")}if(h(),setTimeout(h,120),setTimeout(h,400),setTimeout(h,1e3),t._vvBound||(t._vvBound=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",h),window.visualViewport.addEventListener("scroll",h)),window.addEventListener("resize",h)),function(){const j=e.querySelector("#match-field .terrain-wrapper svg")||e.querySelector(".terrain-wrapper svg");if(!j)return;const U=j.closest("#match-terrain-wrap");U&&(U.style.cssText="position:relative;width:100%;height:100%;padding:0"),j.removeAttribute("width"),j.removeAttribute("height"),j.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",j.setAttribute("preserveAspectRatio","xMidYMid meet")}(),t._resizeBound||(t._resizeBound=!0,window.addEventListener("resize",()=>{const _=e.querySelector(".terrain-wrapper svg");if(_){const j=_.closest("#match-terrain-wrap");j&&(j.style.cssText="position:relative;width:100%;height:100%;padding:0"),_.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0"}})),t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase==="attack"||t.phase==="defense"){let _=!1,j=30;const U=()=>document.getElementById("match-timer"),le=()=>{const ae=U();if(!ae)return;const pe=String(Math.floor(j/60)).padStart(2,"0"),se=String(j%60).padStart(2,"0");ae.textContent=` ${pe}:${se}`,ae.style.color=_?"#ff2222":"#ff9500",ae.style.fontWeight="900"};le(),t._timerInt=setInterval(()=>{if(j--,j<0)if(!_)_=!0,j=15,le();else{clearInterval(t._timerInt),t._timerInt=null,t.homeScore=0,t.aiScore=3;const ae=document.createElement("div");ae.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;color:#fff;padding:24px;text-align:center",ae.innerHTML='<div style="font-size:56px">⏱️</div><div style="font-size:24px;font-weight:900;color:#ff4444">MATCH PERDU PAR FORFAIT</div><div style="font-size:14px;color:rgba(255,255,255,0.6)">Temps écoulé</div>',document.body.appendChild(ae),setTimeout(()=>{ae.remove(),ht(e,t,i)},2500)}else le()},1e3)}(y=document.getElementById("match-quit"))==null||y.addEventListener("click",()=>{Re(e),confirm("Abandonner ? Résultat : défaite 3-0")&&(t.homeScore=0,t.aiScore=3,ht(e,t,i))}),(B=document.getElementById("view-ai"))==null||B.addEventListener("click",()=>Uo(t,i)),(E=document.getElementById("toggle-history"))==null||E.addEventListener("click",()=>{var _;(_=document.getElementById("match-history-panel"))==null||_.classList.add("open")}),(G=document.getElementById("close-history"))==null||G.addEventListener("click",()=>{var _;(_=document.getElementById("match-history-panel"))==null||_.classList.remove("open")}),(N=document.getElementById("btn-action"))==null||N.addEventListener("click",()=>{t.selected.length!==0&&(u?qo(e,t,i):g&&Do(e,t,i))}),(ne=document.getElementById("btn-results"))==null||ne.addEventListener("click",()=>ht(e,t,i)),(te=document.getElementById("btn-pass"))==null||te.addEventListener("click",()=>{t.log.push({text:"⏭️ Vous passez votre tour (plus d'attaquants)",type:"info"}),t.phase="ai-attack",$e(e,t,i),setTimeout(()=>_i(e,t,i),800)}),e.querySelectorAll(".match-slot-hit").forEach(_=>{_.addEventListener("click",()=>Bo(_,t,e,i))}),e.querySelectorAll(".match-used-hit").forEach(_=>{_.addEventListener("click",()=>ii(e,t,i,null,_.dataset.cardId))}),e.querySelectorAll(".gc-mini").forEach(_=>{_.addEventListener("click",()=>Ro(_.dataset.gcId,_.dataset.gcType,e,t,i))}),(Q=document.getElementById("boost-card"))==null||Q.addEventListener("click",()=>Ho(e,t,i)),e.querySelectorAll(".sub-btn-col").forEach(_=>{_.addEventListener("click",()=>ii(e,t,i,_.dataset.subId))}),(I=document.getElementById("sub-btn-main"))==null||I.addEventListener("click",()=>ii(e,t,i))}function Bo(e,t,i,n){const a=e.dataset.cardId,o=e.dataset.role,r=t.selected.findIndex(s=>s.cardId===a);if(r!==-1)t.selected.splice(r,1);else{if(t.selected.length>=3){n.toast("Maximum 3 joueurs","error");return}const s=(t.homeTeam[o]||[]).find(l=>l.cardId===a);s&&t.selected.push({...s,_role:o,_line:o})}$e(i,t,n)}function wi(e,t,i){e.matchId&&w.from("matches").update({last_player_id:i}).eq("id",e.matchId).then(()=>{})}function qo(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),wi(t,i,i.state.profile.id);const n=t.selected.map(o=>{const r=(t.homeTeam[o._role]||[]).find(l=>l.cardId===o.cardId)||o,s=["GK","DEF"].includes(o._role);return{...r,_line:o._role,...s?{note_a:Math.max(1,Number(r.note_a)||0)}:{}}}),a=yi(n,t.modifiers.home);t.pendingAttack={...a,players:[...n],side:"home"},t.selected.forEach(o=>{const r=(t.homeTeam[o._role]||[]).find(s=>s.cardId===o.cardId);r&&(r.used=!0)}),t.log.push({text:`⚔️ Vous attaquez : ${a.total} (base ${a.base}${a.links?` +${a.links} liens`:""}) — ${t.selected.map(o=>o.name).join(", ")}`,type:"info"}),t.selected=[],t.modifiers.home={},t.phase="ai-defense",$e(e,t,i),setTimeout(()=>Po(e,t,i),1200)}function Do(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),wi(t,i,i.state.profile.id);const n=t.stadiumDef||null,a=t.selected.map(l=>{const c=(t.homeTeam[l._role]||[]).find(u=>u.cardId===l.cardId)||l,d=c.stadiumBonus||n&&(n.club_id&&String(c.club_id)===String(n.club_id)||n.country_code&&c.country_code===n.country_code)||!1;return{...c,_line:l._role,stadiumBonus:d}}),o=bi(a,t.modifiers.home);t.selected.forEach(l=>{const c=(t.homeTeam[l._role]||[]).find(d=>d.cardId===l.cardId);c&&(c.used=!0)});const r=hi(t.pendingAttack.total,o.total,t.modifiers.home),s={type:"duel",title:"Défense",aiPlayers:(t.pendingAttack.players||[]).map(l=>Pe(l)),homePlayers:t.selected.map(l=>{const c=(t.homeTeam[l._role]||[]).find(d=>d.cardId===l.cardId)||l;return Pe(c)}),homeTotal:o.total,aiTotal:t.pendingAttack.total,isGoal:!1,homeScored:!1,text:""};if(r.shielded)s.text="🛡️ Bouclier ! But annulé.",t.log.push(s);else if(r.goal){t.aiScore++,s.isGoal=!0,s.homeScored=!1,s.text=`⚽ BUT IA ! (${t.pendingAttack.total} > ${o.total})`,t.log.push(s),t.selected=[],t.modifiers.home={},t.pendingAttack=null,$e(e,t,i),Ve(s.aiPlayers,t.homeScore,t.aiScore,!1,()=>{ct(e,t,i,"home-attack")});return}else s.text=`🧤 Défense réussie ! (${o.total} ≥ ${t.pendingAttack.total})`,t.log.push(s);t.selected=[],t.modifiers.home={},t.pendingAttack=null,ct(e,t,i,"home-attack")}function Fo(e){if(e.aiSubsUsed>=e.aiMaxSubs)return;const t=Object.values(e.aiTeam).flat().filter(l=>l.used);if(!t.length)return;const i=(e.aiSubs||[]).filter(l=>!e.aiUsedSubIds.includes(l.cardId));if(!i.length)return;const n=t[Math.floor(Math.random()*t.length)],a=i.find(l=>l.job===n.job)||i[0],o={...a,used:!1,_line:n._line,_col:n._col},r=e.aiTeam[n._line],s=r.findIndex(l=>l.cardId===n.cardId);s!==-1&&(r[s]=o),e.aiUsedSubIds.push(a.cardId),e.aiSubsUsed++,e.log.push({text:`🔄 IA : ${a.firstname} ${a.name} remplace ${n.firstname} ${n.name}`,type:"info"})}function Go(e){var n;if(!((n=e.aiGcCards)!=null&&n.length))return;const t=e.aiGcCards.filter(a=>!e.aiUsedGc.includes(a.id));if(!t.length||Math.random()>.3)return;const i=t[Math.floor(Math.random()*t.length)];switch(e.aiUsedGc.push(i.id),Ie[i.gc_type],i.gc_type){case"Boost+2":{const a=Object.values(e.aiTeam).flat().filter(o=>!o.used);if(a.length){const o=a[Math.floor(Math.random()*a.length)];o.boost=(o.boost||0)+2}break}case"Boost+3":{const a=Object.values(e.aiTeam).flat().filter(o=>!o.used);if(a.length){const o=a[Math.floor(Math.random()*a.length)];o.boost=(o.boost||0)+3}break}case"Remplacement+":e.aiMaxSubs=(e.aiMaxSubs||3)+1;break;case"Bouclier":e.modifiers.ai.shield=!0;break;case"Nul+1":e.modifiers.ai.drawBonus=(e.modifiers.ai.drawBonus||0)+1;break}e.log.push({text:`⚡ IA joue ${i.icon||"⚡"} ${i.name}`,type:"gc"})}function _i(e,t,i){wi(t,i,null),Fo(t),Go(t);let n=[...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]].filter(d=>!d.used),a=!1;n.length||(n=[...t.aiTeam.GK||[],...t.aiTeam.DEF||[]].filter(d=>!d.used),a=!0);const o=tn(n,"attack",t.difficulty);if(!o.length){li(e,t,i);return}a&&o.forEach(d=>{d._line=d._line||d.job,d.note_a=Math.max(1,Number(d.note_a)||0)});const r=yi(o,t.modifiers.ai);t.pendingAttack={...r,players:o,side:"ai"},o.forEach(d=>{d.used=!0}),t.log.push({text:`🤖 IA attaque : ${r.total} (${o.map(d=>d.name).join(", ")})`,type:"info"}),t.modifiers.ai={};const s=[...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(d=>!d.used),c=(t.homeSubs||[]).filter(d=>!(t.usedSubIds||[]).includes(d.cardId)).length>0&&t.subsUsed<t.maxSubs;if(s.length===0&&!c){if(o.length===1&&(o[0]._line==="GK"||o[0].job==="GK")&&di(t.homeTeam)&&t.homeScore===t.aiScore){t.aiScore++,Vt(e,t,i,"ai",o[0]);return}t.aiScore++;const u={type:"duel",isGoal:!0,homeScored:!1,aiPlayers:o.map(g=>Pe(g)),aiTotal:r.total,text:"⚽ BUT IA ! (aucun défenseur disponible)"};t.log.push(u),t.pendingAttack=null,$e(e,t,i),Ve(u.aiPlayers,t.homeScore,t.aiScore,!1,()=>{ct(e,t,i,"home-attack")});return}t.phase="defense",$e(e,t,i)}function Po(e,t,i){var c,d;const n=[...t.aiTeam.GK||[],...t.aiTeam.DEF||[],...t.aiTeam.MIL||[]],a=tn(n,"defense",t.difficulty);if(!["GK","DEF","MIL","ATT"].flatMap(u=>(t.aiTeam[u]||[]).filter(g=>!g.used)).length){const u=((c=t.pendingAttack)==null?void 0:c.players)||[];if(u.length===1&&(u[0]._line==="GK"||u[0].job==="GK")&&t.homeScore===t.aiScore){t.homeScore++,Vt(e,t,i,"home",u[0]);return}t.homeScore++;const x={type:"duel",isGoal:!0,homeScored:!0,homePlayers:u.map(f=>Pe(f)),homeTotal:((d=t.pendingAttack)==null?void 0:d.total)||0,aiTotal:0,text:"⚽ BUT ! L'IA n'a plus de joueurs — but automatique !"};t.log.push(x),t.modifiers.ai={},t.pendingAttack=null,$e(e,t,i),Ve(x.homePlayers,t.homeScore,t.aiScore,!0,()=>{ct(e,t,i,"ai-attack")});return}const r=a.length>0?bi(a,t.modifiers.ai).total:0;a.forEach(u=>{u.used=!0});const s=hi(t.pendingAttack.total,r,t.modifiers.ai),l={type:"duel",title:"Attaque",homePlayers:(t.pendingAttack.players||[]).map(u=>Pe(u)),aiPlayers:a.map(u=>Pe(u)),homeTotal:t.pendingAttack.total,aiTotal:r,isGoal:!1,homeScored:!1,text:""};if(s.shielded)l.text="🛡️ Bouclier IA !",t.log.push(l);else if(s.goal){t.homeScore++,l.isGoal=!0,l.homeScored=!0,l.text=`⚽ BUT ! (${t.pendingAttack.total} > ${r})`,t.log.push(l),t.modifiers.ai={},t.pendingAttack=null,$e(e,t,i),Ve(l.homePlayers,t.homeScore,t.aiScore,!0,()=>{ct(e,t,i,"ai-attack")});return}else l.text=`🧤 IA défend (${r} ≥ ${t.pendingAttack.total})`,t.log.push(l);t.modifiers.ai={},t.pendingAttack=null,ct(e,t,i,"ai-attack")}function ct(e,t,i,n){if(t.round++,!mn(e,t,i)){if(ki(t)){ht(e,t,i);return}if(n==="home-attack"){if(!["MIL","ATT","GK","DEF"].some(o=>(t.homeTeam[o]||[]).some(r=>!r.used))){li(e,t,i);return}t.phase="attack",$e(e,t,i)}else{if(!["MIL","ATT","GK","DEF"].some(o=>(t.aiTeam[o]||[]).some(r=>!r.used))){li(e,t,i);return}t.phase="ai-attack",$e(e,t,i),setTimeout(()=>_i(e,t,i),800)}}}function di(e){return!["GK","DEF","MIL","ATT"].some(t=>(e[t]||[]).some(i=>!i.used))}function Hi(e){const t=(e.GK||[]).some(n=>!n.used),i=["DEF","MIL","ATT"].some(n=>(e[n]||[]).some(a=>!a.used));return t&&!i}function mn(e,t,i){if(t.homeScore!==t.aiScore)return!1;if(Hi(t.homeTeam)&&di(t.aiTeam)){const n=(t.homeTeam.GK||[]).find(a=>!a.used);return n?(n.used=!0,t.homeScore++,Vt(e,t,i,"home",n),!0):!1}if(Hi(t.aiTeam)&&di(t.homeTeam)){const n=(t.aiTeam.GK||[]).find(a=>!a.used);return n?(n.used=!0,t.aiScore++,Vt(e,t,i,"ai",n),!0):!1}return!1}function Vt(e,t,i,n,a){t.log.push({type:"duel",isGoal:!0,homeScored:n==="home",homePlayers:n==="home"?[Pe(a)]:[],aiPlayers:n==="ai"?[Pe(a)]:[],text:`⚽ DERNIER CORNER — Le gardien ${n==="home"?"":"adverse "}marque !`});const o=document.createElement("div");o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:950;display:flex;align-items:center;justify-content:center;overflow:hidden;text-align:center;padding:24px",document.body.appendChild(o);const r=(s,l)=>new Promise(c=>{o.innerHTML=`<div style="font-size:32px;font-weight:900;color:${l};letter-spacing:2px;animation:lcFade 1.4s ease both">${s}</div>
    <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(c,1400)});(async()=>(await r("⚽ DERNIER CORNER","#FFD700"),await r("🧤 LE GARDIEN MONTE !","#4fc3f7"),o.remove(),t.pendingAttack=null,$e(e,t,i),Ve([Pe(a)],t.homeScore,t.aiScore,n==="home",()=>{if(ki(t)){ht(e,t,i);return}ct(e,t,i,n==="home"?"ai-attack":"home-attack")})))()}function ki(e){const t=["MIL","ATT","GK","DEF"].some(n=>(e.homeTeam[n]||[]).some(a=>!a.used)),i=["MIL","ATT","GK","DEF"].some(n=>(e.aiTeam[n]||[]).some(a=>!a.used));return!t&&!i}function li(e,t,i){mn(e,t,i)||(ki(t)?ht(e,t,i):(t.phase="attack",$e(e,t,i)))}function ii(e,t,i,n=null,a=null){var x,f;if(t.phase!=="attack"){zt("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){zt(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const o=Object.entries(t.homeTeam).flatMap(([p,m])=>(m||[]).filter(b=>b.used).map(b=>({...b,_line:b._line||p}))),r=t.homeSubs.filter(p=>!t.usedSubIds.includes(p.cardId));if(!o.length){zt("Aucun joueur utilisé à remplacer");return}if(!r.length){zt("Aucun remplaçant disponible");return}let s=Math.max(0,o.findIndex(p=>p.cardId===a));const l=((x=o[s])==null?void 0:x._line)||((f=o[s])==null?void 0:f.job);let c=n?Math.max(0,r.findIndex(p=>p.cardId===n)):Math.max(0,r.findIndex(p=>p.job===l)),d=!1;const u=document.createElement("div");u.id="sub-overlay",u.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function g(){var y,B,E,G,N,ne;const p=o[s],m=r[c],b=Math.min(130,Math.round((window.innerWidth-90)/2)),v=Math.round(b*1.35),h=te=>`background:rgba(255,255,255,0.12);border:none;color:${te?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${te?"default":"pointer"};flex-shrink:0`;u.innerHTML=`
    ...
    <div style="flex:1;display:flex;gap:0;overflow:hidden">
      ...
      <div id="in-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
        ...
        <div>${m?tt({...m,used:!1,boost:0},b,v):"<div>—</div>"}</div>
    ...
    </div>`,(y=u.querySelector("#sub-close"))==null||y.addEventListener("click",()=>u.remove()),(B=u.querySelector("#out-up"))==null||B.addEventListener("click",()=>{s>0&&(s--,g())}),(E=u.querySelector("#out-down"))==null||E.addEventListener("click",()=>{s<o.length-1&&(s++,g())}),(G=u.querySelector("#in-up"))==null||G.addEventListener("click",()=>{c>0&&(c--,g())}),(N=u.querySelector("#in-down"))==null||N.addEventListener("click",()=>{c<r.length-1&&(c++,g())});const $=(te,Q,I,_)=>{const j=u.querySelector("#"+te);if(!j)return;let U=0;j.addEventListener("touchstart",le=>{U=le.touches[0].clientY},{passive:!0}),j.addEventListener("touchend",le=>{const ae=le.changedTouches[0].clientY-U;if(Math.abs(ae)<30)return;const pe=Q();ae<0&&pe<_-1?(I(pe+1),g()):ae>0&&pe>0&&(I(pe-1),g())},{passive:!0})};$("in-panel",()=>c,te=>c=te,r.length),$("out-panel",()=>s,te=>s=te,o.length),(ne=u.querySelector("#sub-confirm"))==null||ne.addEventListener("click",te=>{if(te.preventDefault(),te.stopPropagation(),d)return;d=!0;const Q=o[s],I=r[c];if(!Q||!I)return;let _=null,j=-1;for(const[le,ae]of Object.entries(t.homeTeam)){const pe=(ae||[]).findIndex(se=>se.cardId===Q.cardId);if(pe!==-1){_=le,j=pe;break}}if(j===-1||!_){zt("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),u.remove();return}const U={...I,_line:_,_col:Q._col||0,used:!1,boost:0};t.homeTeam[_].splice(j,1,U),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(I.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:Q.name,firstname:Q.firstname,note:De(Q,_),portrait:jt(Q),job:Q.job,country_code:Q.country_code,rarity:Q.rarity,clubName:Q.clubName,clubLogo:Q.clubLogo},inPlayer:{name:I.name,firstname:I.firstname,note:De(I,_),portrait:jt(I),job:I.job,country_code:I.country_code,rarity:I.rarity,clubName:I.clubName,clubLogo:I.clubLogo},text:`🔄 ${I.firstname} ${I.name} remplace ${Q.firstname} ${Q.name}`}),u.remove(),Mo(Q,I,()=>$e(e,t,i))})}document.body.appendChild(u),g()}function Ro(e,t,i,n,a){var f,p;const o=(n.gcDefs||[]).find(m=>m.name===t),r=Ie[t]||{icon:"⚡",desc:"Carte spéciale."},s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[o==null?void 0:o.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",l={purple:"#b06ce0",light_blue:"#00d4ef"}[o==null?void 0:o.color]||"#b06ce0",c=(o==null?void 0:o.name)||t,d=(o==null?void 0:o.effect)||r.desc,u=o!=null&&o.image_url?`/icons/${o.image_url}`:null,g=r.icon||"⚡",x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",x.innerHTML=`
    ...
    </div>`,document.body.appendChild(x),(f=x.querySelector("#gc-back"))==null||f.addEventListener("click",()=>x.remove()),(p=x.querySelector("#gc-use"))==null||p.addEventListener("click",()=>{x.remove(),Oo(e,t,i,n,a)})}function Rt(e,t,i,n,a,o){const r=document.createElement("div");r.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let s=[];function l(){var c,d;r.innerHTML=`
    ...
    <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start;justify-content:center">
      ${e.map(u=>{const g=u._line||u.job||"MIL",x=s.find(p=>p.cardId===u.cardId),f=Se({...u,_evolution_bonus:0},{width:90,showStad:!0,role:g,extraNote:u.boost||0});return`<div class="gc-pick-item" data-cid="${u.cardId}"
    ...
    </div>`,(c=r.querySelector("#gc-picker-close"))==null||c.addEventListener("click",()=>r.remove()),r.querySelectorAll(".gc-pick-item").forEach(u=>{u.addEventListener("click",()=>{const g=u.dataset.cid,x=e.find(p=>p.cardId===g);if(!x)return;const f=s.findIndex(p=>p.cardId===g);f>-1?s.splice(f,1):s.length<t&&s.push(x),l()})}),(d=r.querySelector("#gc-picker-confirm"))==null||d.addEventListener("click",()=>{r.remove(),o(s)})}l(),document.body.appendChild(r)}const No={BOOST_STAT:({value:e=1,count:t=1,roles:i=[]},n,a,o)=>{const r=Object.entries(n.homeTeam).filter(([s])=>!i.length||i.includes(s)).flatMap(([s,l])=>l.filter(c=>!c.used).map(c=>({...c,_line:s})));return r.length?(Rt(r,t,`Choisir ${t} joueur(s) à booster (+${e})`,a,n,s=>{s.forEach(l=>{const c=(n.homeTeam[l._line]||[]).find(d=>d.cardId===l.cardId);c&&(c.boost=(c.boost||0)+e,n.log.push({text:`⚡ +${e} sur ${c.name}`,type:"info"}))}),$e(a,n,o)}),!0):(n.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),$e(a,n,o),!0)},DEBUFF_STAT:({value:e=1,count:t=1,roles:i=[],target:n="ai"},a,o,r)=>{const s=n==="home"?a.homeTeam:a.aiTeam,l=n==="ai"?"adverse":"allié",c=Object.entries(s).filter(([d])=>!i.length||i.includes(d)).flatMap(([d,u])=>u.filter(g=>!g.used).map(g=>({...g,_line:d})));return c.length?(Rt(c,t,`Choisir ${t} joueur(s) ${l}(s) à débuffer (-${e})`,o,a,d=>{d.forEach(u=>{const x=((n==="home"?a.homeTeam:a.aiTeam)[u._line]||[]).find(f=>f.cardId===u.cardId);x&&(x.boost=(x.boost||0)-e,a.log.push({text:`🎯 -${e} sur ${x.name}${n==="ai"?" (IA)":""}`,type:"info"}))}),$e(o,a,r)}),!0):(a.log.push({text:`🎯 Aucun joueur ${l} disponible`,type:"info"}),$e(o,a,r),!0)},GRAY_PLAYER:({count:e=1,roles:t=[],target:i="ai"},n,a,o)=>{const r=i==="home"?n.homeTeam:n.aiTeam,s=i==="ai"?"adverse":"allié",l=Object.entries(r).filter(([c])=>!t.length||t.includes(c)).flatMap(([c,d])=>d.filter(u=>!u.used).map(u=>({...u,_line:c})));return l.length?(Rt(l,e,`Choisir ${e} joueur(s) ${s}(s) à exclure`,a,n,c=>{c.forEach(d=>{const g=((i==="home"?n.homeTeam:n.aiTeam)[d._line]||[]).find(x=>x.cardId===d.cardId);g&&(g.used=!0,n.log.push({text:`❌ ${g.name}${i==="ai"?" (IA)":""} exclu !`,type:"info"}))}),$e(a,n,o)}),!0):(n.log.push({text:`❌ Aucun joueur ${s} à exclure`,type:"info"}),$e(a,n,o),!0)},REVIVE_PLAYER:({count:e=1,roles:t=[]},i,n,a)=>{const o=Object.entries(i.homeTeam).filter(([r])=>!t.length||t.includes(r)).flatMap(([r,s])=>s.filter(l=>l.used).map(l=>({...l,_line:r})));return o.length?(Rt(o,e,`Choisir ${e} joueur(s) à ressusciter`,n,i,r=>{r.forEach(s=>{const l=(i.homeTeam[s._line]||[]).find(c=>c.cardId===s.cardId);l&&(l.used=!1,i.log.push({text:`💫 ${l.name} ressuscité !`,type:"info"}))}),$e(n,i,a)}),!0):(i.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),$e(n,i,a),!0)},REMOVE_GOAL:({},e)=>(e.aiScore>0?(e.aiScore--,e.log.push({text:"🚫 Dernier but IA annulé !",type:"info"})):e.log.push({text:"🚫 Aucun but à annuler",type:"info"}),!1),ADD_GOAL_DRAW:({},e)=>(e.homeScore===e.aiScore?(e.homeScore++,e.log.push({text:"🎯 But bonus (match nul) !",type:"info"})):e.log.push({text:"🎯 But bonus : non applicable (pas de match nul)",type:"info"}),!1),ADD_SUB:({value:e=1},t)=>(t.maxSubs=(t.maxSubs||3)+e,t.log.push({text:`🔄 +${e} remplacement(s) débloqué(s)`,type:"info"}),!1),CUSTOM:()=>!1};function Oo(e,t,i,n,a){n.usedGc.push(e);const o=n.gcDefs||[],r=o.find(l=>l.name===t)||o.find(l=>{var c;return((c=l.name)==null?void 0:c.toLowerCase().trim())===(t==null?void 0:t.toLowerCase().trim())});let s=!1;if(r!=null&&r.effect_type&&r.effect_type!=="CUSTOM"){const l=No[r.effect_type];l?l(r.effect_params||{},n,i,a)||(s=!0):(a.toast(`Effet "${r.effect_type}" non implémenté`,"error"),s=!0)}else{switch(t){case"Double attaque":n.modifiers.home.doubleAttack=!0,n.log.push({text:"⚡ Double attaque activée !",type:"info"});break;case"Bouclier":n.modifiers.home.shield=!0,n.log.push({text:"🛡️ Bouclier activé !",type:"info"});break;case"Ressusciter":{const l=Object.entries(n.homeTeam).flatMap(([c,d])=>(d||[]).filter(u=>u.used).map(u=>({...u,_line:c})));l.length?(l[0].used=!1,n.log.push({text:`💫 ${l[0].name} ressuscité !`,type:"info"})):n.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"});break}case"Vol de note":n.modifiers.ai.stolenNote=(n.modifiers.ai.stolenNote||0)+1,n.log.push({text:"🎯 -1 à la prochaine attaque IA",type:"info"});break;case"Gel":{const l=[...n.aiTeam.ATT||[],...n.aiTeam.MIL||[]].filter(c=>!c.used);if(l.length){const c=l.sort((d,u)=>De(u,"ATT")-De(d,"ATT"))[0];c.used=!0,n.log.push({text:`❄️ ${c.name} (IA) gelé !`,type:"info"})}break}case"Remplacement+":n.maxSubs++,n.log.push({text:"🔄 +1 remplacement débloqué",type:"info"});break}s=!0}w.from("cards").delete().eq("id",e).then(()=>{}),s&&$e(i,n,a)}function Ho(e,t,i){const n=Object.values(t.homeTeam).flat().filter(a=>!a.used);if(!n.length){i.toast("Aucun joueur actif à booster","error");return}i.openModal("⚡ Utiliser le Boost",`<div style="margin-bottom:12px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border-radius:10px;padding:12px;text-align:center;color:#000">
    ...
    <div style="display:flex;flex-direction:column;gap:6px">
      ${n.map(a=>`
        <div class="player-boost-opt" data-card-id="${a.cardId}"
          ...
          <div style="width:32px;height:32px;background:${Eo[a.job]||"#888"};border-radius:6px;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:900;font-size:13px">${De(a,a._line||a.job)}</div>
          <div style="flex:1"><b>${a.firstname} ${a.name}</b><div style="font-size:11px;color:#888">${a._line||a.job}</div></div>
          <div style="color:#87CEEB;font-weight:700">+${t.boostCard.value}</div>
        </div>`).join("")}
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(a=>{a.addEventListener("click",()=>{const o=a.dataset.cardId;for(const r of["GK","DEF","MIL","ATT"]){const s=(t.homeTeam[r]||[]).find(l=>l.cardId===o);if(s){s.boost=(s.boost||0)+t.boostCard.value,t.log.push({text:`⚡ Boost +${t.boostCard.value} appliqué à ${s.name}`,type:"info"});break}}t.boostUsed=!0,i.closeModal(),$e(e,t,i)})})}async function ht(e,t,i){var d,u;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:n}=i,a=t.homeScore>t.aiScore,o=t.homeScore===t.aiScore,r=a?"victoire":o?"nul":"defaite",s=Un(t.mode,r);t.matchId&&await w.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:a?n.profile.id:null,home_credits_reward:s,played_at:new Date().toISOString()}).eq("id",t.matchId);const l={credits:(n.profile.credits||0)+s,matches_played:(n.profile.matches_played||0)+1};a?l.wins=(n.profile.wins||0)+1:o?l.draws=(n.profile.draws||0)+1:l.losses=(n.profile.losses||0)+1,await w.from("users").update(l).eq("id",n.profile.id),await i.refreshProfile();const c=document.createElement("div");c.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",c.innerHTML=`
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      ...
      ${Wt(e.aiTeam,e.formation,null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.1))}
            ...
            <div style="background:rgba(255,255,255,0.08);border-radius:14px;padding:14px 20px;text-align:center;min-width:220px">
              ...
              ${Me.id!==Oe.id?`<div style="font-size:20px;font-weight:900;color:${Me.color}">
                    ${Be>H.mmr?"📈":"📉"} ${Oe.emoji} ${Oe.label} → ${Me.emoji} ${Me.label}
                  </div>
      ...
      <div style="font-size:18px">${y[p+"Name"]} ${L} – ${z} ${y[m+"Name"]}</div>
      ${k.forfeit?`<div style="font-size:13px;color:rgba(255,255,255,0.5)">${S?"L'adversaire a quitté":"Perdu par forfait"}</div>`:""}
      ${A}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${r?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(D),(Z=D.querySelector("#pvp-end-home"))==null||Z.addEventListener("click",()=>{D.remove(),Re(e),x(r?"ranked":"home")})}const Q=w.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},k=>{const L=k.new;try{if(L.status==="finished"||L.forfeit){if(B)return;B=!0,E&&(clearInterval(E),E=null),L.game_state&&(y=L.game_state),te(L);return}if(L.game_state){const z=y;y=L.game_state;const S=y._lastGC;if(S&&S.seq&&!ne.has(S.seq)&&(ne.add(S.seq),S.by!==p)){P(S.type,S.by,()=>se());return}const T=z[p+"Score"]||0,A=z[m+"Score"]||0,D=y[p+"Score"]||0,R=y[m+"Score"]||0,X=D>T,V=R>A;if((X||V)&&!N.has(y.round)){N.add(y.round);const oe=[...y.log||[]].reverse().find(H=>H.isGoal),Z=((oe==null?void 0:oe.homePlayers)||[]).map(H=>({name:H.name,note:H.note,portrait:H.portrait,job:H.job}));Ve(Z,D,R,X,()=>se());return}se()}}catch(z){console.error("[PvP] crash:",z)}}).subscribe();async function I(k){Object.assign(y,k),y.lastActionAt=new Date().toISOString();const{error:L}=await w.from("matches").update({game_state:y}).eq("id",i);L&&f("Erreur de synchronisation","error");try{se()}catch(z){console.error("[PvP] renderPvpScreen crash:",z)}}async function _(){if(B)return;B=!0,E&&(clearInterval(E),E=null);const k=n?h.away_id:h.home_id,L=n?"p2":"p1",z=n?"p1":"p2",S={...y,[L+"Score"]:3,[z+"Score"]:0,phase:"finished"};try{await w.from("matches").update({status:"finished",forfeit:!0,winner_id:k,home_score:S.p1Score||0,away_score:S.p2Score||0,game_state:S}).eq("id",i)}catch{}try{w.removeChannel(Q)}catch{}setTimeout(()=>{Re(e),x("home")},800)}const j={BOOST_STAT:({value:k=1,count:L=1,roles:z=[]},S,T)=>{const A=S[p+"Team"],D=Object.entries(A).filter(([R])=>!z.length||z.includes(R)).flatMap(([R,X])=>X.filter(V=>!V.used).map(V=>({...V,_line:R})));if(!D.length){S.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),T(S);return}U(D,L,`Choisir ${L} joueur(s) à booster (+${k})`,R=>{R.forEach(X=>{const V=(A[X._line]||[]).find(oe=>oe.cardId===X.cardId);V&&(V.boost=(V.boost||0)+k,S.log.push({text:`⚡ +${k} sur ${V.name}`,type:"info"}))}),S[p+"Team"]=A,T(S)})},DEBUFF_STAT:({value:k=1,count:L=1,roles:z=[],target:S="ai"},T,A)=>{const D=S==="home"?p:m,R=T[D+"Team"],X=S==="home"?"allié":"adverse",V=Object.entries(R).filter(([oe])=>!z.length||z.includes(oe)).flatMap(([oe,Z])=>Z.map(H=>({...H,_line:oe})));if(!V.length){T.log.push({text:`🎯 Aucun joueur ${X}`,type:"info"}),A(T);return}U(V,L,`Choisir ${L} joueur(s) ${X}(s) (-${k})`,oe=>{oe.forEach(Z=>{const H=(R[Z._line]||[]).find(re=>re.cardId===Z.cardId);H&&(H.boost=(H.boost||0)-k,T.log.push({text:`🎯 -${k} sur ${H.name}`,type:"info"}))}),T[D+"Team"]=R,A(T)})},GRAY_PLAYER:({count:k=1,roles:L=[],target:z="ai"},S,T)=>{const A=z==="home"?p:m,D=S[A+"Team"],R=z==="home"?"allié":"adverse",X=Object.entries(D).filter(([V])=>!L.length||L.includes(V)).flatMap(([V,oe])=>oe.filter(Z=>!Z.used).map(Z=>({...Z,_line:V})));if(!X.length){S.log.push({text:`❌ Aucun joueur ${R}`,type:"info"}),T(S);return}U(X,k,`Choisir ${k} joueur(s) ${R}(s) à exclure`,V=>{const oe="usedCardIds_"+A,Z=new Set(S[oe]||[]);V.forEach(H=>{const re=(D[H._line]||[]).find(ce=>ce.cardId===H.cardId);re&&(re.used=!0,Z.add(H.cardId),S.log.push({text:`❌ ${re.name} exclu !`,type:"info"}))}),S[oe]=[...Z],S[A+"Team"]=D,T(S)})},REVIVE_PLAYER:({count:k=1,roles:L=[]},z,S)=>{const T=z[p+"Team"],A=Object.entries(T).filter(([D])=>!L.length||L.includes(D)).flatMap(([D,R])=>R.filter(X=>X.used).map(X=>({...X,_line:D})));if(!A.length){z.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),S(z);return}U(A,k,`Choisir ${k} joueur(s) à ressusciter`,D=>{D.forEach(R=>{const X=(T[R._line]||[]).find(V=>V.cardId===R.cardId);X&&(X.used=!1,z.log.push({text:`💫 ${X.name} ressuscité !`,type:"info"}))}),z[p+"Team"]=T,S(z)})},REMOVE_GOAL:({},k,L)=>{const z=m+"Score";k[z]>0?(k[z]--,k.log.push({text:"🚫 Dernier but annulé !",type:"info"})):k.log.push({text:"🚫 Aucun but à annuler",type:"info"}),L(k)},ADD_GOAL_DRAW:({},k,L)=>{k[p+"Score"]===k[m+"Score"]?(k[p+"Score"]++,k.log.push({text:"🎯 But bonus !",type:"info"})):k.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),L(k)},ADD_SUB:({value:k=1},L,z)=>{L.maxSubs=(L.maxSubs||3)+k,L.log.push({text:`🔄 +${k} remplacement(s)`,type:"info"}),z(L)},CUSTOM:({},k,L)=>L(k)};function U(k,L,z,S){const T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let A=[];function D(){var X,V;const R=k.map(oe=>{const Z=oe._line||oe.job||"MIL",H=A.find(ce=>ce.cardId===oe.cardId),re=Se({...oe,_evolution_bonus:0},{width:90,showStad:!0,role:Z,extraNote:oe.boost||0});return`<div class="pp-item" data-cid="${oe.cardId}"
        ...
        </div>`,(X=T.querySelector("#pp-close"))==null||X.addEventListener("click",()=>T.remove()),T.querySelectorAll(".pp-item").forEach(oe=>{oe.addEventListener("click",()=>{const Z=oe.dataset.cid,H=k.find(ce=>ce.cardId===Z),re=A.findIndex(ce=>ce.cardId===Z);H&&(re>-1?A.splice(re,1):A.length<L&&A.push(H),D())})}),(V=T.querySelector("#pp-confirm"))==null||V.addEventListener("click",()=>{T.remove(),S(A)})}D(),document.body.appendChild(T)}async function le(k,L){const S=(y["gcCardsFull_"+p]||[]).find(R=>R.id===k),T=(S==null?void 0:S._gcDef)||(y.gcDefs||[]).find(R=>{var X;return R.name===L||((X=R.name)==null?void 0:X.toLowerCase().trim())===(L==null?void 0:L.toLowerCase().trim())}),A=[...y["usedGc_"+p]||[],k],D={type:L,by:p,seq:(y._gcAnimSeq||0)+1};ne.add(D.seq),P(L,p,async()=>{if(T!=null&&T.effect_type&&T.effect_type!=="CUSTOM"){const X=j[T.effect_type];if(X){const V={...y};X(T.effect_params||{},V,async oe=>{oe["usedGc_"+p]=A,oe._lastGC=D,oe._gcAnimSeq=D.seq,await I(oe)});return}}const R={...y};switch(L){case"Remplacement+":R.maxSubs=(R.maxSubs||3)+1,R.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const X=m+"Score";R[X]>0&&(R[X]--,R.log.push({text:"🚫 But annulé",type:"info"}));break}}R["usedGc_"+p]=A,R._lastGC=D,R._gcAnimSeq=D.seq,await I(R)})}function ae(k,L){const z="usedCardIds_"+k,S=new Set(y[z]||[]);L.forEach(T=>S.add(T)),y[z]=[...S]}function pe(){for(const k of["p1","p2"]){const L=new Set(y["usedCardIds_"+k]||[]),z=y[k+"Team"];if(z)for(const S of["GK","DEF","MIL","ATT"])(z[S]||[]).forEach(T=>{T.used=L.has(T.cardId)})}}function se(){var rt,We,Xe,at,Li,Ti;if(y.phase==="reveal")return Ee();if(y.phase==="midfield")return J();if(y.phase==="finished")return Et();const k=y[p+"Team"];y[m+"Team"],pe();const L=y[p+"Score"],z=y[m+"Score"],S=y[p+"Name"],T=y[m+"Name"],A=y.phase===p+"-attack",D=y.phase===p+"-defense",R=Array.isArray(y["selected_"+p])?y["selected_"+p]:[],X=R.map(ie=>ie.cardId),V=window.innerWidth>=700,oe=y[p+"Subs"]||[],Z=y["usedSubIds_"+p]||[],H=oe.filter(ie=>!Z.includes(ie.cardId)),re=y["gcCardsFull_"+p]||[],ce=y["usedGc_"+p]||[],ue=re.filter(ie=>!ce.includes(ie.id)),de=y.boostOwner===p&&!y.boostUsed,me=[...k.MIL||[],...k.ATT||[]].filter(ie=>!ie.used),xe=A&&me.length===0?[...k.GK||[],...k.DEF||[]].filter(ie=>!ie.used).map(ie=>ie.cardId):[];function ee(ie,fe,ze){var Ii,Ai;const ye=ie._gcDef,st={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ye==null?void 0:ye.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",ft={purple:"#b06ce0",light_blue:"#00d4ef"}[ye==null?void 0:ye.color]||"#b06ce0",Lt=(ye==null?void 0:ye.name)||ie.gc_type,Tt=(ye==null?void 0:ye.effect)||((Ii=Ie[ie.gc_type])==null?void 0:Ii.desc)||"",zi=ye!=null&&ye.image_url?`/icons/${ye.image_url}`:null,kn=((Ai=Ie[ie.gc_type])==null?void 0:Ai.icon)||"⚡",Mi=Math.round(ze*.22),Si=Math.round(ze*.22),Jt=ze-Mi-Si,$n=Lt.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${ie.id}" data-gc-type="${ie.gc_type}"
        ...
        <div style="height:${Mi}px;background:rgba(255,255,255,0.14);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 3px">
          <span style="font-size:${$n}px;font-weight:900;color:#fff;text-align:center;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${fe-6}px">${Lt}</span>
          <span style="font-size:6px;color:rgba(255,255,255,0.45)">⚡ GC</span>
        </div>
        <div style="height:${Jt}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
          ${zi?`<img src="${zi}" style="max-width:${fe-10}px;max-height:${Jt-6}px;object-fit:contain">`:`<span style="font-size:${Math.round(Jt*.55)}px">${kn}</span>`}
        </div>
      ...
      </div>`}const Be=(ie,fe)=>fe?Te(130,175):ee(ie,130,175),Me=(ie,fe)=>fe?Te(68,95):ee(ie,68,95),Ge=V?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",ut=A?Ae(p)?`<button id="pvp-action" style="${Ge};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${R.length===0?"disabled":""}>⚔️ ATTAQUEZ <span id="pvp-timer"></span></button>`:`<button id="pvp-action" data-pass="1" style="${Ge};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER <span id="pvp-timer"></span></button>`:D?`<button id="pvp-action" style="${Ge};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${R.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="pvp-timer"></span></button>`:`<div style="font-size:11px;color:rgba(255,255,255,0.3);text-align:center;padding:4px">⏳ Tour de ${T}</div>`,Oe=A&&!Ae(p)?'<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">Aucun attaquant — passez la main</div>':A||D?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${R.length}/3 sélectionné(s)</div>`:"",Ft=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${V?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
    ...
    </div>`,Ce=A?"attack":D?"defense":"idle",He=`<div style="overflow:hidden;min-width:0;flex:1;min-height:0;display:flex;flex-direction:column" id="match-field">
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ...
        </div>`}if(A&&((ze=y.pendingAttack)!=null&&ze.players)){const ye=y.pendingAttack;return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #00ff88;text-align:center">
          <div style="font-size:8px;color:#00ff88;letter-spacing:2px;margin-bottom:4px;text-transform:uppercase">⚔️ VOUS ATTAQUEZ</div>
          <div style="display:flex;justify-content:center">${yt((ye.players||[]).map(st=>({...st,used:!1})),"#00ff88",ye.total)}</div>
        </div>`}const ie=(y.log||[]).slice(-1)[0];return ie?'<div style="padding:2px 4px">'+fn(ie)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3);text-align:center">⏳ Match en cours...</div>'})(),Ye=`
      <div style="display:flex;align-items:center;padding:8px 10px;background:rgba(0,0,0,0.5);gap:6px;flex-shrink:0">
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(H);let re=!1;const ce=()=>{re||(re=!0,H.remove(),setTimeout(()=>z&&z(),50))};H.addEventListener("click",ce),setTimeout(ce,3e3)}function M(k,L){var H,re,ce,ue;const S=(y["gcCardsFull_"+p]||[]).find(de=>de.id===k),T=S==null?void 0:S._gcDef,A={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[T==null?void 0:T.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",D={purple:"#b06ce0",light_blue:"#00d4ef"}[T==null?void 0:T.color]||"#b06ce0",R=(T==null?void 0:T.name)||L,X=(T==null?void 0:T.effect)||((H=Ie[L])==null?void 0:H.desc)||"Carte spéciale.",V=T!=null&&T.image_url?`/icons/${T.image_url}`:null,oe=((re=Ie[L])==null?void 0:re.icon)||"⚡",Z=document.createElement("div");Z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",Z.innerHTML=`
      ...
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${L.map(T=>{const A={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[T._line]||"#555",D=De(T,T._line)+(T.boost||0);return`<div class="bp-item" data-cid="${T.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${A};overflow:hidden;cursor:pointer">
      ...
      </div>`,document.body.appendChild(z),(S=z.querySelector("#bp-close"))==null||S.addEventListener("click",()=>z.remove()),z.querySelectorAll(".bp-item").forEach(T=>{T.addEventListener("click",async()=>{const A=T.dataset.cid,D=L.find(X=>X.cardId===A);if(!D)return;const R=(k[D._line]||[]).find(X=>X.cardId===A);R&&(R.boost=(R.boost||0)+y.boostValue),z.remove(),await I({[p+"Team"]:k,boostUsed:!0})})})}function F(k=null){var re,ce;if(!(y.phase===p+"-attack")){f("Remplacement uniquement avant votre attaque","warning");return}const z=y[p+"Team"],S=y["usedSubIds_"+p]||[],T=y.maxSubs||3;if(S.length>=T){f(`Maximum ${T} remplacements atteint`,"warning");return}const A=Object.entries(z).flatMap(([ue,de])=>(de||[]).filter(me=>me.used).map(me=>({...me,_line:ue}))),D=(y[p+"Subs"]||[]).filter(ue=>!S.includes(ue.cardId));if(!A.length){f("Aucun joueur utilisé à remplacer","warning");return}if(!D.length){f("Aucun remplaçant disponible","warning");return}let R=Math.max(0,A.findIndex(ue=>ue.cardId===k));const X=((re=A[R])==null?void 0:re._line)||((ce=A[R])==null?void 0:ce.job);let V=Math.max(0,D.findIndex(ue=>ue.job===X)),oe=!1;const Z=document.createElement("div");Z.id="pvp-sub-overlay",Z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function H(){var Be,Me,Ge,ut,Oe,Ft;const ue=A[R],de=D[V],me=Math.min(130,Math.round((window.innerWidth-90)/2)),xe=Math.round(me*1.35),ee=Ce=>`background:rgba(255,255,255,0.12);border:none;color:${Ce?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Ce?"default":"pointer"};flex-shrink:0`;Z.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        ...
        <div id="pout-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
          ...
          <button id="pout-down" style="${ee(R>=A.length-1)}" ${R>=A.length-1?"disabled":""}>▼</button>
          <div style="font-size:10px;color:rgba(255,255,255,0.35)">${R+1}/${A.length}</div>
        </div>
      </div>
      <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
        <button id="psub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
      </div>`,(Be=Z.querySelector("#psub-close"))==null||Be.addEventListener("click",()=>Z.remove()),(Me=Z.querySelector("#pout-up"))==null||Me.addEventListener("click",()=>{R>0&&(R--,H())}),(Ge=Z.querySelector("#pout-down"))==null||Ge.addEventListener("click",()=>{R<A.length-1&&(R++,H())}),(ut=Z.querySelector("#pin-up"))==null||ut.addEventListener("click",()=>{V>0&&(V--,H())}),(Oe=Z.querySelector("#pin-down"))==null||Oe.addEventListener("click",()=>{V<D.length-1&&(V++,H())});const Te=(Ce,He,Ue,Ye)=>{const qe=Z.querySelector("#"+Ce);if(!qe)return;let rt=0;qe.addEventListener("touchstart",We=>{rt=We.touches[0].clientY},{passive:!0}),qe.addEventListener("touchend",We=>{const Xe=We.changedTouches[0].clientY-rt;if(Math.abs(Xe)<30)return;const at=He();Xe<0&&at<Ye-1?(Ue(at+1),H()):Xe>0&&at>0&&(Ue(at-1),H())},{passive:!0})};Te("pin-panel",()=>V,Ce=>V=Ce,D.length),Te("pout-panel",()=>R,Ce=>R=Ce,A.length),(Ft=Z.querySelector("#psub-confirm"))==null||Ft.addEventListener("click",async Ce=>{if(Ce.preventDefault(),Ce.stopPropagation(),oe)return;oe=!0;const He=A[R],Ue=D[V];if(!He||!Ue)return;const Ye=He._line,qe=(z[Ye]||[]).findIndex(Xe=>Xe.cardId===He.cardId);if(qe===-1){f("Erreur : joueur introuvable","error"),Z.remove();return}const rt={...Ue,_line:Ye,position:He.position,used:!1,boost:0};z[Ye].splice(qe,1,rt);const We=[...S,Ue.cardId];Z.remove(),W(He,Ue,async()=>{await I({[p+"Team"]:z,[m+"Team"]:y[m+"Team"],["usedSubIds_"+p]:We})})})}document.body.appendChild(Z),H()}function W(k,L,z){const S={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const A=(X,V,oe)=>`<div style="text-align:center">
      ...
      <div style="font-size:18px;font-weight:900;color:#ff6b6b">${y[m+"Name"]}</div>
      <div style="width:min(90vw,420px)">${Wt(y[m+"Team"],y[m+"Formation"],null,[],lt(),bt())}</div>
      ...
      </div>`,document.body.appendChild(L),(z=L.querySelector("#pvp-hist-close"))==null||z.addEventListener("click",()=>L.remove())}async function O(){if(y.phase!==p+"-attack")return;const k=p==="p1"?"p2":"p1",L=(y.round||0)+1,z=[...y.log||[]];z.push({type:"info",text:`⏭️ ${y[p+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const S=Fe(y),T=Ae(k),A=S||!T?"finished":k+"-attack";await I({["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:A,attacker:k,round:L,log:z}),A==="finished"&&await Ne(y)}function we(k){const L=(k.GK||[]).some(S=>!S.used),z=["DEF","MIL","ATT"].some(S=>(k[S]||[]).some(T=>!T.used));return L&&!z}function _e(k){return(k.p1Score||0)!==(k.p2Score||0)?null:we(k.p1Team)&&!ge(k.p2Team)?{side:"p1",gk:(k.p1Team.GK||[]).find(L=>!L.used)}:we(k.p2Team)&&!ge(k.p1Team)?{side:"p2",gk:(k.p2Team.GK||[]).find(L=>!L.used)}:null}async function be(k,L){const{side:z,gk:S}=k;if(!S)return;const T=z+"Score",A=(L[T]||0)+1;S.used=!0;const D=[...L.log||[]];D.push({type:"duel",isGoal:!0,homeScored:z===p,homePlayers:z==="p1"?[gt(S)]:[],aiPlayers:z==="p2"?[gt(S)]:[],text:`⚽ DERNIER CORNER — Le gardien de ${L[z+"Name"]} marque !`});const R=(L.round||0)+1,X=z==="p1"?"p2":"p1",V={...L,[T]:A},oe=Fe(V);N.add(R);const Z=document.createElement("div");Z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:950;display:flex;align-items:center;justify-content:center;overflow:hidden;text-align:center;padding:24px",document.body.appendChild(Z);const H=(ue,de)=>new Promise(me=>{Z.innerHTML=`<div style="font-size:32px;font-weight:900;color:${de};letter-spacing:2px;animation:lcFade 1.4s ease both">${ue}</div>
      <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(me,1400)});await H("⚽ DERNIER CORNER","#FFD700"),await H("🧤 LE GARDIEN MONTE !","#4fc3f7"),Z.remove();const re=z===p?A:L[p+"Score"]||0,ce=z===p?L[m+"Score"]||0:A;Ve([gt(S)],re,ce,z===p,async()=>{await I({[T]:A,log:D,round:R,pendingAttack:null,phase:oe?"finished":X+"-attack",attacker:X,["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}}}),oe&&await Ne({...y,[T]:A})})}async function he(){const k=y[p+"Team"],L=!["GK","DEF","MIL","ATT"].some(A=>(y[m+"Team"][A]||[]).some(D=>!D.used)),z=(y["selected_"+p]||[]).map(A=>{const D=(k[A._role]||[]).find(ce=>ce.cardId===A.cardId)||A,R=["GK","DEF"].includes(A._role),X=k[A._role]||[],V=X.findIndex(ce=>ce.cardId===A.cardId),oe=_t(X.length),Z=V>=0?oe[V]:D._col??1,H=y.stadiumDef||y.homeStadiumDef||null,re=D.stadiumBonus||H&&(H.club_id&&String(D.club_id)===String(H.club_id)||H.country_code&&D.country_code===H.country_code)||!1;return{...D,_line:A._role,_col:Z,stadiumBonus:re,...R?{note_a:Math.max(1,Number(D.note_a)||0)}:{}}});if(!z.length)return;const S=yi(z,y.modifiers[p]||{});ae(p,z.map(A=>A.cardId)),z.forEach(A=>{const D=(k[A._role]||[]).find(R=>R.cardId===A.cardId);D&&(D.used=!0)}),y["selected_"+p]=[],se();const T=[...y.log||[]];if(L){if(z.length===1&&(z[0]._line==="GK"||z[0].job==="GK")&&(y.p1Score||0)===(y.p2Score||0)){await be({side:p,gk:z[0]},{...y,[p+"Team"]:k,log:T});return}const D=(y[p+"Score"]||0)+1,R=z.map(H=>gt(H));T.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:R,homeTotal:S.total,aiTotal:0});const X=(y.round||0)+1,V=p==="p1"?"p2":"p1",oe={...y,[p+"Team"]:k,[p+"Score"]:D},Z=Fe(oe);N.add(X),Ve(R,D,y[m+"Score"]||0,!0,async()=>{await I({[p+"Team"]:k,[p+"Score"]:D,["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:Z?"finished":V+"-attack",attacker:V,round:X,log:T}),Z&&await Ne({...y,[p+"Score"]:D})});return}T.push({type:"pending",text:`⚔️ ${y[p+"Name"]} attaque (${S.total})`}),await I({[p+"Team"]:k,[m+"Team"]:y[m+"Team"],pendingAttack:{...S,players:z,side:p},["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},phase:m+"-defense",log:T})}async function ke(){const k=y[p+"Team"],L=(y["selected_"+p]||[]).map(de=>{const me=(k[de._role]||[]).find(Me=>Me.cardId===de.cardId)||de,xe=k[de._role]||[],ee=xe.findIndex(Me=>Me.cardId===de.cardId),Te=_t(xe.length),Be=ee>=0?Te[ee]:me._col??1;return{...me,_line:de._role,_col:Be}}),z=bi(L,y.modifiers[p]||{});ae(p,L.map(de=>de.cardId)),L.forEach(de=>{const me=(k[de._role]||[]).find(xe=>xe.cardId===de.cardId);me&&(me.used=!0)}),y["selected_"+p]=[],se();const S=hi(y.pendingAttack.total,z.total,y.modifiers[p]||{}),T=y.pendingAttack.side,A=S==="attack"||(S==null?void 0:S.goal),D=T==="p1"?"p2":"p1",R=(y.round||0)+1,X=(y.pendingAttack.players||[]).map(de=>gt(de)),V=[...y.log||[]];V.push({type:"duel",isGoal:A,homeScored:A&&T===p,text:A?`⚽ BUT de ${y[T+"Name"]} ! (${y.pendingAttack.total} vs ${z.total})`:`✋ Attaque stoppée (${y.pendingAttack.total} vs ${z.total})`,homePlayers:X,aiPlayers:L.map(de=>gt(de)),homeTotal:y.pendingAttack.total,aiTotal:z.total});const oe=A?(y[T+"Score"]||0)+1:y[T+"Score"]||0,Z={...y,[p+"Team"]:k,[T+"Score"]:oe},H=Fe(Z),re=H?"finished":D+"-attack",ce=!H&&!A?_e(Z):null,ue=async()=>{if(ce){await be(ce,Z);return}await I({[p+"Team"]:k,[m+"Team"]:y[m+"Team"],[T+"Score"]:oe,["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:re,attacker:D,round:R,log:V}),(re==="finished"||H)&&await Ne({...y,[T+"Score"]:oe})};if(A){const de=T===p,me=de?oe:y[p+"Score"]||0,xe=de?y[m+"Score"]||0:oe;N.add(R),Ve(X,me,xe,de,ue)}else await ue()}function Le(k){return["MIL","ATT"].some(L=>(k[L]||[]).some(z=>!z.used))}function ge(k){return["GK","DEF","MIL","ATT"].some(L=>(k[L]||[]).some(z=>!z.used))}function je(k){return ge(k)&&!Le(k)}function Ae(k){const L=y[k+"Team"];return!!(Le(L)||je(L))}function Fe(k){return!ge(k.p1Team)&&!ge(k.p2Team)}function ot(k){const L=k.p1Score||0,z=k.p2Score||0;return L===z?null:L>z?h.home_id:h.away_id}async function Ne(k){try{const L=ot(k),z=L?h.home_id===L?h.away_id:h.home_id:null;await w.from("matches").update({status:"finished",winner_id:L,home_score:k.p1Score||0,away_score:k.p2Score||0}).eq("id",i),L&&z&&Yn(L,z).catch(()=>{})}catch(L){console.error("[PvP] finishMatch:",L)}}function Et(){var T;const k=y[p+"Score"],L=y[m+"Score"],z=k>L,S=k===L;Ke(e),e.innerHTML=`
    ...
    </div>`,(T=document.getElementById("pvp-home"))==null||T.addEventListener("click",()=>{try{w.removeChannel(Q)}catch{}Re(e),x("home")})}se()}async function ci(e,t,i=!1){var s,l;const{state:n,navigate:a,toast:o}=t,r=i&&((l=(s=t==null?void 0:t.state)==null?void 0:s.params)==null?void 0:l.rankedData)||null;await Dt(e,t,i?"ranked":"random",async({deckId:c,formation:d,starters:u,subsRaw:g,gcCardsEnriched:x,gcDefs:f,stadiumDef:p})=>{Ke(e);const m=async b=>{await Yo(e,t,c,d,u,g,b||[],f,p,i,r)};if(!(x!=null&&x.length)){await m([]);return}vi(e,x,m)})}async function Vo(e,t,i){return Ko(e,t,i)}async function Yo(e,t,i,n,a,o,r=[],s=[],l=null,c=!1,d=null){var te,Q;const{state:u,navigate:g,toast:x}=t,f=(d==null?void 0:d.mmr)??((te=u.profile)==null?void 0:te.mmr)??1e3,p=c?gi(f):null,m=c?"linear-gradient(135deg, #1a0a2e 0%, #2d1060 50%, #1a0a2e 100%)":"linear-gradient(135deg, #0a1a2e 0%, #0d3d1e 100%)",b=c?(p==null?void 0:p.color)||"#D4A017":"#FFD700";let v=!1,h=null,$=null;e.innerHTML=`
    ...
    </div>`;const y=async()=>{v=!0,h&&(h.unsubscribe(),h=null),$&&(clearInterval($),$=null);try{await w.rpc("cancel_matchmaking",{p_user_id:u.profile.id})}catch(I){console.error("[Matchmaking] cancel error:",I)}};(Q=document.getElementById("cancel-mm"))==null||Q.addEventListener("click",async()=>{await y(),Re(e),g("home")});const B=async(I,_)=>{v||(v=!0,h&&(h.unsubscribe(),h=null),$&&(clearInterval($),$=null),await $t(e,t,I,_,{myGC:r,gcDefs:s,stadiumDef:l,isRanked:c,rankedData:d,onMatchEnd:c?Wo:null}))},E=u.user.id,{data:G,error:N}=c?await w.rpc("try_matchmake_ranked",{p_user_id:E,p_deck_id:i,p_mmr:f}):await w.rpc("try_matchmake",{p_user_id:E,p_deck_id:i});if(N||!(G!=null&&G.success)){console.error("[Matchmaking] try_matchmake error:",N||G),x("Erreur de matchmaking","error"),Re(e),g("home");return}if(G.matched){await B(G.match_id,!1);return}const ne=async()=>{if(v)return;const{data:I}=await w.from("matchmaking_queue").select("status, match_id").eq("user_id",E).maybeSingle();(I==null?void 0:I.status)==="matched"&&I.match_id&&await B(I.match_id,!0)};h=w.channel(`mm_${E}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${E}`},I=>{const _=I.new;_.status==="matched"&&_.match_id&&B(_.match_id,!0)}).subscribe(),$=setInterval(ne,3e3),setTimeout(()=>{v||y()},12e4)}async function Wo(e,t){const{state:i,toast:n}=t,{winnerId:a,homeId:o,awayId:r,homeScore:s,awayScore:l,matchId:c}=e,d=i.user.id,u=o===d,g=a===d,{data:x}=await w.from("users").select("mmr, mmr_rd, mmr_v").eq("id",d).single(),{data:f}=await w.from("users").select("mmr, mmr_rd, mmr_v").eq("id",u?r:o).single();if(x&&f){const p=En(x.mmr,x.mmr_rd,x.mmr_v,f.mmr,f.mmr_rd,g?1:0);await w.from("users").update({mmr:p.mmr,mmr_rd:p.rd,mmr_v:p.v,mmr_wins:g?w.sql`mmr_wins + 1`:void 0,mmr_losses:g?void 0:w.sql`mmr_losses + 1`}).eq("id",d),n(g?`+MMR ↑ ${Math.round(p.mmr-x.mmr)}`:`-MMR ↓ ${Math.round(x.mmr-p.mmr)}`,g?"success":"error",4e3)}}const Xo=Object.freeze(Object.defineProperty({__proto__:null,renderMatchRandom:ci,resumePvpMatch:Vo},Symbol.toStringTag,{value:"Module"}));async function Jo(e,t){const{state:i,navigate:n,toast:a}=t,o=i.params||{},r=o.friendId||null,s=o.friendName||"Ami";await Dt(e,t,"friend",async({deckId:l,formation:c,starters:d,subsRaw:u,gcCardsEnriched:g,gcDefs:x,stadiumDef:f})=>{Ke(e);const p=async m=>{const b=m||[];r?await Qo(e,t,l,b,x,f,r,s):await Zo(e,t,l,b,x,f)};if(!(g!=null&&g.length)){await p([]);return}vi(e,g,p)})}async function Qo(e,t,i,n,a,o,r,s){var v;const{state:l,navigate:c,toast:d}=t,u=l.user.id,{data:g,error:x}=await w.from("friend_match_invites").insert({inviter_id:u,invitee_id:r,friend_id:r,status:"pending",inviter_deck_id:i,expires_at:new Date(Date.now()+2*60*1e3).toISOString()}).select("id").single();if(x||!g){console.error("[Friend] Erreur création invitation:",x),d("Impossible de créer l'invitation","error"),c("home");return}e.innerHTML=`
    ...
    </div>`;let f=!1,p=null;const m=async()=>{p&&(p.unsubscribe(),p=null)};(v=document.getElementById("cancel-friend"))==null||v.addEventListener("click",async()=>{await m(),await w.from("friend_match_invites").update({status:"declined"}).eq("id",g.id),Re(e),c("home")}),p=w.channel(`friend_invite_${g.id}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"friend_match_invites",filter:`id=eq.${g.id}`},async({new:h})=>{f||h.status!=="accepted"||!h.match_id||(f=!0,await m(),await $t(e,t,h.match_id,!0,{myGC:n,gcDefs:a,stadiumDef:o}))}).subscribe();const b=setInterval(async()=>{if(f)return;const{data:h}=await w.from("friend_match_invites").select("status, match_id").eq("id",g.id).single();(h==null?void 0:h.status)==="accepted"&&h.match_id&&(f=!0,clearInterval(b),await m(),await $t(e,t,h.match_id,!0,{myGC:n,gcDefs:a,stadiumDef:o}))},3e3);setTimeout(async()=>{f||(clearInterval(b),await m(),d("Invitation expirée","info"),c("home"))},12e4)}async function Zo(e,t,i,n,a,o){const{state:r,navigate:s,toast:l}=t,c=r.user.id,{data:d}=await w.from("friend_match_invites").select("id, inviter_id").eq("invitee_id",c).eq("status","pending").order("created_at",{ascending:!1}).limit(1).maybeSingle();if(!d){l("Aucune invitation en attente","error"),s("home");return}const{data:u,error:g}=await w.rpc("accept_friend_invite",{p_invite_id:d.id,p_invitee_deck_id:i});if(g||!(u!=null&&u.success)){console.error("[Friend] Erreur accept_friend_invite:",g||u),l((u==null?void 0:u.error)||"Impossible de rejoindre le match","error"),s("home");return}await $t(e,t,u.match_id,!1,{myGC:n,gcDefs:a,stadiumDef:o})}const vt="#1A6B3C",et="#D4A017";async function er(e,t){var n;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const i=(n=t.state.params)==null?void 0:n.openLeagueId;if(i){t.state.params.openLeagueId=null,await nt(e,t,i);return}await pt(e,t)}async function pt(e,t,i="waiting"){var b;const{state:n}=t,a=n.profile.id,{data:o}=await w.from("mini_league_members").select("league_id").eq("user_id",a),r=(o||[]).map(v=>v.league_id),{data:s,error:l}=await w.from("mini_leagues").select("*, mini_league_members(count)").eq("status","waiting").eq("is_archived",!1).order("created_at",{ascending:!1}).limit(30),c=l?(await w.from("mini_leagues").select("*, mini_league_members(count)").eq("status","waiting").order("created_at",{ascending:!1}).limit(30)).data||[]:s||[],u=(r.length?await w.from("mini_leagues").select("*, mini_league_members(count)").in("id",r).order("created_at",{ascending:!1}):{data:[]}).data||[],g=u.filter(v=>v.status==="waiting"&&!v.is_archived),x=u.filter(v=>v.status==="active"&&!v.is_archived),f=u.filter(v=>v.is_archived||v.status==="finished"),p=c.filter(v=>!r.includes(v.id)),m=[{key:"waiting",label:"🟡 En attente",count:g.length+p.length},{key:"active",label:"🟢 En cours",count:x.length},{key:"archived",label:"📁 Archivées",count:f.length}];e.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="display:flex;background:#fff;border-bottom:1px solid #eee">
      ${m.map(v=>`<button class="ml-tab" data-tab="${v.key}" style="flex:1;padding:10px 4px;border:none;border-bottom:2px solid ${i===v.key?vt:"transparent"};background:none;font-size:12px;font-weight:${i===v.key?"900":"600"};color:${i===v.key?vt:"#888"};cursor:pointer">${v.label}${v.count?` (${v.count})`:""}</button>`).join("")}
    </div>
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:10px">
      ${i==="waiting"?tr(g,p,a):i==="active"?ir(x,a):nr(f,a)}
    </div>
  </div>`,(b=document.getElementById("ml-create-btn"))==null||b.addEventListener("click",()=>or(e,t)),e.querySelectorAll(".ml-tab").forEach(v=>v.addEventListener("click",()=>pt(e,t,v.dataset.tab))),e.querySelectorAll("[data-league-id]").forEach(v=>v.addEventListener("click",()=>nt(e,t,v.dataset.leagueId))),e.querySelectorAll("[data-join]").forEach(v=>v.addEventListener("click",h=>{h.stopPropagation(),yn(e,t,v.dataset.join,v.dataset.type)})),e.querySelectorAll("[data-delete]").forEach(v=>v.addEventListener("click",h=>{h.stopPropagation(),pi(e,t,v.dataset.delete,v.dataset.name,i)}))}function Yt(e,t,i=!1){var r,s;const n=e.creator_id===t,a=e.pot||0,o=((s=(r=e.mini_league_members)==null?void 0:r[0])==null?void 0:s.count)||0;return`<div data-league-id="${e.id}" style="background:#fff;border-radius:12px;padding:14px 16px;box-shadow:0 1px 6px rgba(0,0,0,0.08);cursor:pointer;margin-bottom:8px">
    ...
    <div style="padding:20px;display:flex;flex-direction:column;gap:14px">
      ...
      </div>`,document.body.appendChild(t);const i=t.querySelector("#ml-pwd-input"),n=a=>{t.remove(),e(a)};setTimeout(()=>i==null?void 0:i.focus(),80),t.querySelector("#pwd-cancel").addEventListener("click",()=>n(null)),t.querySelector("#pwd-ok").addEventListener("click",()=>n(i.value.trim())),i.addEventListener("keydown",a=>{a.key==="Enter"&&n(i.value.trim())}),t.addEventListener("click",a=>{a.target===t&&n(null)})})}async function yn(e,t,i,n){const{toast:a,state:o}=t,r=o.profile.id,{data:s}=await w.from("mini_leagues").select("password,status,entry_fee,pot,max_players").eq("id",i).single();if(!s){a("Mini League introuvable","error");return}if(s.status!=="waiting"){a("Cette Mini League a déjà démarré","warning");return}const{count:l}=await w.from("mini_league_members").select("id",{count:"exact",head:!0}).eq("league_id",i);if(l>=s.max_players){a("La Mini League est complète","warning");return}if(n==="private"){const p=await rr();if(p===null)return;if(s.password!==p){a("Mot de passe incorrect","error");return}}const c=s.entry_fee||100,{data:d}=await w.from("users").select("credits").eq("id",r).single();if(((d==null?void 0:d.credits)||0)<c){a(`Crédits insuffisants — il te faut ${c.toLocaleString("fr")} cr. (solde : ${((d==null?void 0:d.credits)||0).toLocaleString("fr")} cr.)`,"error");return}const{error:u}=await w.from("mini_league_members").insert({league_id:i,user_id:r});if(u){a("Erreur inscription : "+u.message,"error");return}const{error:g}=await w.from("users").update({credits:d.credits-c}).eq("id",r),{error:x}=await w.from("mini_leagues").update({pot:(s.pot||0)+c}).eq("id",i);(g||x)&&console.error("Erreur post-inscription:",g,x),o.profile&&(o.profile.credits=d.credits-c);const f=document.getElementById("nav-credits");f&&(f.textContent=`💰 ${(o.profile.credits||0).toLocaleString("fr")}`),a(`✅ Inscrit ! −${c.toLocaleString("fr")} cr. · Pot : ${((s.pot||0)+c).toLocaleString("fr")} cr.`,"success"),nt(e,t,i)}async function ar(e,t,i,n){const{toast:a,state:o}=t,r=o.profile.id;if(!confirm(`Te désinscrire et récupérer ${n.toLocaleString("fr")} cr. ?`))return;const{data:s}=await w.from("mini_leagues").select("pot,status").eq("id",i).single();if(!s||s.status!=="waiting"){a("Impossible — la league a déjà démarré","error");return}const{data:l}=await w.from("users").select("credits").eq("id",r).single();await w.from("mini_leagues").update({pot:Math.max(0,(s.pot||0)-n)}).eq("id",i),await w.from("users").update({credits:((l==null?void 0:l.credits)||0)+n}).eq("id",r),await w.from("mini_league_members").delete().eq("league_id",i).eq("user_id",r),o.profile&&(o.profile.credits=((l==null?void 0:l.credits)||0)+n);const c=document.getElementById("nav-credits");c&&(c.textContent=`💰 ${(o.profile.credits||0).toLocaleString("fr")}`),a(`↩️ Désinscrit · +${n.toLocaleString("fr")} cr. remboursés`,"success"),pt(e,t,"waiting")}async function pi(e,t,i,n,a){const{toast:o}=t;if(!confirm(`Supprimer définitivement "${n}" et tous ses matchs/membres ? Action irréversible.`))return;await w.from("mini_league_matches").delete().eq("league_id",i),await w.from("mini_league_members").delete().eq("league_id",i);const{error:r}=await w.from("mini_leagues").delete().eq("id",i);if(r){o("Erreur suppression ("+r.message+")","error");return}o("Mini League supprimée avec succès","success"),pt(e,t,a)}async function sr(e,t,i){await w.from("mini_leagues").update({is_archived:!0}).eq("id",i),t.toast("Mini League archivée","success"),pt(e,t,"archived")}async function nt(e,t,i){var $,y,B,E,G,N,ne,te,Q;const{state:n,toast:a}=t,o=n.profile.id,[{data:r},{data:s},{data:l}]=await Promise.all([w.from("mini_leagues").select("*").eq("id",i).single(),w.from("mini_league_members").select("*, user:users(id,pseudo,club_name,club_color1,club_color2)").eq("league_id",i),w.from("mini_league_matches").select("*").eq("league_id",i).order("matchday").order("created_at")]);if(!r){a("Introuvable","error"),await pt(e,t);return}const c=(s||[]).some(I=>I.user_id===o),d=r.creator_id===o,u=(s||[]).map(I=>I.user).filter(Boolean),g=hn(u,l||[]),x=(l||[]).filter(I=>I.matchday===r.current_day),f=r.pot||0,p=r.entry_fee||100,m=x.length>0&&x.every(I=>I.status==="finished"||I.status==="bye"),b=r.current_day>=r.total_days,v=(s||[]).find(I=>I.user_id===o);e.innerHTML=`
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        ${u.map(I=>`
          <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid #f5f5f5">
            <div style="width:36px;height:36px;border-radius:50%;background:${I.club_color2||vt};display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:900;color:${I.club_color1||"#fff"}">${(I.pseudo||"?").slice(0,2).toUpperCase()}</div>
            <div style="flex:1"><div style="font-size:13px;font-weight:700">${I.club_name||I.pseudo}</div><div style="font-size:11px;color:#999">@${I.pseudo}</div></div>
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        ${x.map(I=>Ui(I,u,o,c)).join("")}
        ${d&&m&&!b?'<button id="ml-next-day" class="btn btn-primary" style="width:100%;margin-top:12px">➡️ Journée suivante</button>':""}
        ${d&&m&&b?'<button id="ml-finish-btn" class="btn btn-primary" style="width:100%;margin-top:12px;background:#7a28b8">🏆 Terminer et distribuer le pot</button>':""}
      </div>`:""}
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>${g.map((I,_)=>{const j=f>0&&r.status==="finished"?_===0?Math.floor(f*.7):_===1?Math.floor(f*.2):_===2?Math.floor(f*.1):0:0;return`<tr style="border-bottom:1px solid #f5f5f5;${I.userId===o?"background:#f0fdf4;":""}">
              <td style="padding:7px 3px 7px 0;font-weight:700;color:${_===0?et:_<3?vt:"#555"}">${["🥇","🥈","🥉"][_]||_+1}</td>
              <td style="padding:7px 3px"><div style="font-weight:700">${I.clubName}</div><div style="font-size:10px;color:#999">@${I.pseudo}</div></td>
              <td style="text-align:center">${I.played}</td><td style="text-align:center">${I.won}-${I.drawn}-${I.lost}</td>
              <td style="text-align:center;color:${I.goalDiff>=0?vt:"#cc2222"}">${I.goalDiff>=0?"+":""}${I.goalDiff}</td>
              <td style="text-align:center;font-weight:900;font-size:14px">${I.points}</td>
              ${f>0&&r.status==="finished"?`<td style="text-align:right;font-weight:700;color:${et}">${j?j.toLocaleString("fr")+" cr.":"—"}</td>`:""}
            </tr>`}).join("")}</tbody>
        </table>
      </div>`:""}
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        ${Array.from({length:Math.max(0,r.status==="active"?r.current_day-1:r.current_day)},(I,_)=>_+1).reverse().map(I=>{const _=(l||[]).filter(j=>j.matchday===I);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:#888;margin-bottom:6px">Journée ${I}</div>${_.map(j=>Ui(j,u,o,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
  ...
  </div>`}async function dr(e,t,i,n){const{toast:a,state:o}=t,r=pr(n.map(c=>c.id),i.mode),s=[];r.forEach((c,d)=>c.forEach(u=>s.push({league_id:i.id,matchday:d+1,home_id:u.home||u.bye,away_id:u.away||null,is_bye:!!u.bye,status:u.bye?"bye":"pending"})));const{error:l}=await w.from("mini_league_matches").insert(s);if(l){a("Erreur calendrier : "+l.message,"error");return}await w.from("mini_leagues").update({status:"active",current_day:1,total_days:r.length}).eq("id",i.id),a(`🚀 Lancée ! Pot : ${(i.pot||0).toLocaleString("fr")} cr.`,"success"),nt(e,t,i.id)}async function lr(e,t,i){const{data:n}=await w.from("mini_leagues").select("current_day,total_days,pot").eq("id",i).single(),a=(n.current_day||0)+1;if(a>(n.total_days||0)){await bn(e,t,i,n.pot||0,null,null);return}await w.from("mini_leagues").update({current_day:a}).eq("id",i),t.toast(`Journée ${a} commencée !`,"success"),nt(e,t,i)}async function bn(e,t,i,n,a,o){const{toast:r,state:s}=t;let l=a,c=o;if(!l){const{data:u}=await w.from("mini_league_matches").select("*").eq("league_id",i),{data:g}=await w.from("mini_league_members").select("*, user:users(id,pseudo,club_name)").eq("league_id",i);c=(g||[]).map(x=>x.user).filter(Boolean),l=hn(c,u||[])}const d=[Math.floor(n*.7),Math.floor(n*.2),Math.floor(n*.1)];await Promise.all(l.slice(0,3).map((u,g)=>d[g]?w.from("mini_league_members").update({prize_amount:d[g],prize_claimed:!1}).eq("league_id",i).eq("user_id",u.userId):Promise.resolve())),await w.from("mini_leagues").update({status:"finished"}).eq("id",i),r("🏆 Mini League terminée ! Les gagnants peuvent récupérer leurs crédits.","success"),nt(e,t,i)}async function cr(e,t,i,n,a){var g,x;const{state:o,toast:r}=t,s=[Math.floor((i.pot||0)*.7),Math.floor((i.pot||0)*.2),Math.floor((i.pot||0)*.1)],l=["🥇","🥈","🥉"][a],c=n.prize_amount||s[a]||0,d=n.prize_claimed,u=document.createElement("div");u.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",u.innerHTML=`
    ...
    </div>`,(u=document.getElementById("start-league"))==null||u.addEventListener("click",async()=>{await w.from("mini_leagues").update({status:"in_progress"}).eq("id",i),await vn(e,t,i)}),(g=document.getElementById("join-league"))==null||g.addEventListener("click",async()=>{await Dt(e,t,"mini_league",async({deckId:f,gcCardsEnriched:p})=>{const m=p||[];await w.from("mini_league_participants").insert({league_id:i,user_id:r,deck_id:f,gc_cards:m.map(b=>b.id)}),o("Rejoint !","success"),await $i(e,t,i)})}),(x=document.getElementById("back-home"))==null||x.addEventListener("click",()=>a("home"))}async function vn(e,t,i){const{navigate:n,toast:a}=t,{data:o}=await w.from("mini_league_matches").select("id, match_id, home_id, away_id, status").eq("league_id",i).eq("status","pending").order("round",{ascending:!0}).limit(1),r=o==null?void 0:o[0];if(!r){await gr(e,t,i);return}const s=t.state.user.id,l=r.home_id===s;await $t(e,t,r.match_id,l,{mlLeagueId:i,mlMatchId:r.id,onMatchEnd:async c=>{await w.from("mini_league_matches").update({status:"finished",home_score:c.homeScore,away_score:c.awayScore,winner_id:c.winnerId}).eq("id",r.id),await vn(e,t,i)}})}async function fr(e,t,i,n){const{data:a}=await w.from("mini_league_matches").select("match_id, home_id, away_id").eq("id",i).single();if(!a){t.toast("Match introuvable","error"),t.navigate("home");return}const o=a.home_id===t.state.user.id;await $t(e,t,a.match_id,o,{mlLeagueId:n,mlMatchId:i,onMatchEnd:async r=>{await w.from("mini_league_matches").update({status:"finished",home_score:r.homeScore,away_score:r.awayScore,winner_id:r.winnerId}).eq("id",i),t.navigate("home")}})}async function gr(e,t,i){var d;const{navigate:n}=t,{data:a}=await w.from("mini_league_matches").select("home_id, away_id, home_score, away_score, winner_id, home:users!home_id(pseudo), away:users!away_id(pseudo)").eq("league_id",i),o={},r={};(a||[]).forEach(u=>{[u.home_id,u.away_id].forEach(g=>{o[g]||(o[g]=0),r[g]||(r[g]=0)}),u.winner_id?o[u.winner_id]=(o[u.winner_id]||0)+3:(o[u.home_id]=(o[u.home_id]||0)+1,o[u.away_id]=(o[u.away_id]||0)+1),r[u.home_id]=(r[u.home_id]||0)+(u.home_score||0),r[u.away_id]=(r[u.away_id]||0)+(u.away_score||0)});const s={};(a||[]).forEach(u=>{var g,x;s[u.home_id]=(g=u.home)==null?void 0:g.pseudo,s[u.away_id]=(x=u.away)==null?void 0:x.pseudo});const l=Object.keys(o).sort((u,g)=>o[g]-o[u]||r[g]-r[u]),c=["🥇","🥈","🥉","4️⃣"];e.innerHTML=`
    <div style="min-height:100%;background:linear-gradient(135deg,#0a1a2e,#0d3d1e);color:#fff;padding:24px;display:flex;flex-direction:column;gap:16px">
      <div style="text-align:center">
        <div style="font-size:36px">🏆</div>
    ...
    </div>`,(d=document.getElementById("back-home"))==null||d.addEventListener("click",()=>n("home"))}const mr="/",xr=[{emoji:"⚽",title:"Bienvenue dans Manager Wars !",color:"#1A6B3C",content:`<p>Tu es désormais un <strong>manager de football</strong> virtuel.</p>
    ...
    <p style="margin-top:12px;font-size:13px;color:#888">Tu peux revoir ce tutoriel depuis les paramètres à tout moment.</p>`}];function yr(e,t,i){let n=0;const a=document.createElement("div");a.id="tutorial-overlay",a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.65);z-index:9900;display:flex;align-items:center;justify-content:center;padding:16px";const o=()=>{var d,u,g;const s=t[n],l=n===t.length-1,c=Math.round((n+1)/t.length*100);a.innerHTML=`
      <div style="background:#fff;border-radius:20px;width:100%;max-width:420px;max-height:90vh;display:flex;flex-direction:column;overflow:hidden;box-shadow:0 16px 64px rgba(0,0,0,0.4)">
        ...
        <!-- Image optionnelle -->
        ${s.image_url?`<div style="padding:0 24px 8px;text-align:center"><img src="${mr}icons/${s.image_url}" style="max-height:160px;max-width:100%;border-radius:12px;object-fit:contain"></div>`:""}
        <!-- Contenu -->
    ...
    `,a.querySelectorAll("ul,ol").forEach(x=>{x.style.paddingLeft="20px",x.style.marginTop="6px",x.style.marginBottom="6px"}),a.querySelectorAll("li").forEach(x=>{x.style.marginBottom="4px"}),a.querySelectorAll("p").forEach(x=>{x.style.marginBottom="8px"}),(d=a.querySelector("#tuto-prev"))==null||d.addEventListener("click",()=>{n--,o()}),(u=a.querySelector("#tuto-next"))==null||u.addEventListener("click",()=>{l?r():(n++,o())}),(g=a.querySelector("#tuto-skip"))==null||g.addEventListener("click",()=>{confirm("Passer le tutoriel ? Tu pourras le revoir plus tard depuis les paramètres.")&&r()})},r=async()=>{a.remove(),e!=null&&e.id&&await w.from("users").update({tutorial_done:!0}).eq("id",e.id),i==null||i()};document.body.appendChild(a),o()}async function br(e,t,i){if(!e||e.tutorial_done)return;let n=[];const{data:a,error:o}=await w.rpc("get_tutorial_steps");if(!o&&(a==null?void 0:a.length)>0)n=a,console.log(`[Tutorial] RPC OK → ${n.length} étapes`);else{const{data:s,error:l}=await w.from("tutorial_steps").select("*").eq("is_active",!0).order("step_order");!l&&(s==null?void 0:s.length)>0?(n=s,console.log(`[Tutorial] Direct OK → ${n.length} étapes`)):(console.warn(`[Tutorial] Aucune étape DB (RPC: ${o==null?void 0:o.message}, Direct: ${l==null?void 0:l.message})`),i&&i("[Tutorial] DB vide ou inaccessible — tuto local utilisé","warning",5e3))}const r=n.length>0?n.map(s=>({emoji:s.emoji,title:s.title,color:s.color,content:s.content,image_url:s.image_url||null})):xr;yr(e,r,()=>t("boosters"))}const Nt={GK:"#111111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function mt(e,t,i=0){return t?(Number(t==="GK"?e.note_g:t==="DEF"?e.note_d:t==="MIL"?e.note_m:e.note_a)||0)+(t===e.job||t===e.job2?i:0):0}async function hr(e,t){e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',await Ei(e,t)}async function Ei(e,t){const{state:i,toast:n}=t,{data:a}=await w.from("market_listings").select(`id, price, status, listed_at, seller_id,
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
  ...
  </div>`;let l="buy";const c=()=>{var p,m,b,v,h,$,y;return{name:(((p=document.getElementById("flt-name"))==null?void 0:p.value)||"").toLowerCase().trim(),club:(((m=document.getElementById("flt-club"))==null?void 0:m.value)||"").toLowerCase().trim(),country:(((b=document.getElementById("flt-country"))==null?void 0:b.value)||"").toLowerCase().trim(),job:((v=document.getElementById("flt-job"))==null?void 0:v.value)||"",rarity:((h=document.getElementById("flt-rarity"))==null?void 0:h.value)||"",note1:parseInt(($=document.getElementById("flt-note1"))==null?void 0:$.value)||0,note2:parseInt((y=document.getElementById("flt-note2"))==null?void 0:y.value)||0}};function d(p){const m=c();return p.filter(b=>{var N,ne,te;const v=(N=b.card)==null?void 0:N.player;if(!v)return!1;const h=`${v.firstname} ${v.surname_real}`.toLowerCase(),$=(((ne=v.clubs)==null?void 0:ne.encoded_name)||"").toLowerCase(),y=(v.country_code||"").toLowerCase(),B=((te=b.card)==null?void 0:te.evolution_bonus)||0,E=mt(v,v.job,B),G=v.job2?mt(v,v.job2,B):0;return!(m.name&&!h.includes(m.name)||m.club&&!$.includes(m.club)||m.country&&!y.includes(m.country)||m.job&&v.job!==m.job||m.rarity&&v.rarity!==m.rarity||m.note1&&E<m.note1||m.note2&&G<m.note2)})}function u(p){var G,N,ne,te;const m=(G=p.card)==null?void 0:G.player;if(!m)return"";const b=((N=p.card)==null?void 0:N.evolution_bonus)||0,v=mt(m,m.job,b),h=m.job2?mt(m,m.job2,b):0,$=(i.profile.credits||0)>=p.price,y=m.country_code?`https://flagsapi.com/${m.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,B=Nt[m.job]||"#bbb",E=m.job2?Nt[m.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden">
      <!-- Drapeau -->
      ${y?`<img src="${y}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      <!-- Logo club -->
      ${(ne=m.clubs)!=null&&ne.logo_url?`<img src="${m.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <!-- Notes -->
    ...
    </div>`}function g(p){var G,N,ne,te;const m=(G=p.card)==null?void 0:G.player;if(!m)return"";const b=((N=p.card)==null?void 0:N.evolution_bonus)||0,v=mt(m,m.job,b),h=m.job2?mt(m,m.job2,b):0,$=p.status==="sold",y=m.country_code?`https://flagsapi.com/${m.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,B=Nt[m.job]||"#bbb",E=m.job2?Nt[m.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${$?"opacity:0.7":""}">
      ${y?`<img src="${y}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      ${(ne=m.clubs)!=null&&ne.logo_url?`<img src="${m.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${B};display:flex;align-items:center;justify-content:center">
    ...
    </div>`}function x(){const p=document.getElementById("mkt-content"),m=document.getElementById("mkt-filters");if(p){if(m.style.display=l==="buy"?"flex":"none",l==="buy"){const b=d(r);p.innerHTML=b.length?b.map(u).join(""):'<div style="text-align:center;color:#aaa;padding:40px">Aucune carte trouvée.</div>'}else{const b=s.filter(h=>h.status==="active").sort((h,$)=>new Date($.listed_at)-new Date(h.listed_at)),v=s.filter(h=>h.status==="sold").sort((h,$)=>new Date($.sold_at||$.listed_at)-new Date(h.sold_at||h.listed_at));p.innerHTML=(b.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:4px 0 6px;text-transform:uppercase;letter-spacing:1px">🟢 En vente (${b.length})</div>`+b.map(g).join(""):"")+(v.length?`<div style="font-size:11px;font-weight:700;color:#555;padding:12px 0 6px;text-transform:uppercase;letter-spacing:1px">✅ Ventes réussies (${v.length})</div>`+v.map(g).join(""):"")+(!b.length&&!v.length?'<div style="text-align:center;color:#aaa;padding:40px">Aucune vente pour le moment.</div>':"")}p.querySelectorAll("[data-buy]").forEach(b=>b.addEventListener("click",()=>vr(b.dataset.buy,r,e,t))),p.querySelectorAll("[data-cancel]").forEach(b=>b.addEventListener("click",()=>_r(b.dataset.cancel,e,t)))}}e.querySelectorAll(".mkt-tab").forEach(p=>{p.addEventListener("click",()=>{l=p.dataset.tab,e.querySelectorAll(".mkt-tab").forEach(m=>{const b=m===p;m.style.background=b?"var(--green)":"#fff",m.style.color=b?"#fff":"var(--gray-600)",m.style.borderColor=b?"var(--green)":"var(--gray-200)"}),x()})});let f;["flt-name","flt-club","flt-country","flt-job","flt-rarity","flt-note1","flt-note2"].forEach(p=>{var m;(m=document.getElementById(p))==null||m.addEventListener("input",()=>{clearTimeout(f),f=setTimeout(x,250)})}),x()}async function vr(e,t,i,n){const{state:a,toast:o,refreshProfile:r}=n,s=t.find(d=>d.id===e);if(!s)return;const l=s.price;if((a.profile.credits||0)<l){o("Crédits insuffisants","error");return}wr(s,async()=>{const{error:d}=await w.rpc("buy_market_card",{p_listing_id:e,p_buyer_id:a.profile.id});if(d){o("Erreur achat : "+d.message,"error");return}await r();const u=document.getElementById("nav-credits");u&&(u.textContent=`💰 ${(a.profile.credits||0).toLocaleString("fr")}`),o("✅ Carte achetée !","success"),await Ei(i,n)})}function wr(e,t){var r;const i=(r=e.card)==null?void 0:r.player,n=i?`${i.firstname} ${i.surname_real}`:"cette carte",a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px",a.innerHTML=`
    ...
    </div>`,document.body.appendChild(a);const o=s=>{a.remove(),s&&t()};a.querySelector("#buy-cancel").addEventListener("click",()=>o(!1)),a.querySelector("#buy-ok").addEventListener("click",()=>o(!0)),a.addEventListener("click",s=>{s.target===a&&o(!1)})}async function _r(e,t,i){const{toast:n}=i,{data:a}=await w.from("market_listings").select("card_id").eq("id",e).single();if(await w.from("market_listings").update({status:"cancelled"}).eq("id",e),a!=null&&a.card_id){const{count:o}=await w.from("market_listings").select("id",{count:"exact",head:!0}).eq("card_id",a.card_id).eq("status","active");o||await w.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",a.card_id)}n("Annonce retirée","success"),Ei(t,i)}async function kr(e,t){var d,u,g,x;const{state:i,navigate:n}=t,a=((u=(d=t==null?void 0:t.state)==null?void 0:d.params)==null?void 0:u.tab)||"global";e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const[{data:o},{data:r}]=await Promise.all([w.from("users").select("id,pseudo,club_name,trophies_top1,trophies_top2,trophies_top3,wins,level").order("trophies_top1",{ascending:!1}).limit(100),w.from("users").select("id,pseudo,club_name,mmr,rank_tier,ranked_wins,ranked_losses,ranked_draws,placement_matches").gte("placement_matches",1).order("mmr",{ascending:!1}).limit(100)]);let s=a;function l(){var p,m;const f=document.getElementById("rankings-list");if(f){if(s==="global"){const b=o||[];f.innerHTML=b.length>0?b.map((v,h)=>`
  ...
  </div>`}Tn(Mn);const ve={user:null,profile:null,page:"home",params:{}};function xt(e,t="info",i=3e3){const n=document.getElementById("toast");n&&(n.textContent=e,n.className=`show ${t}`,clearTimeout(n._t),n._t=setTimeout(()=>{n.className=""},i))}function Lr(e,t,i=""){document.getElementById("modal-title").textContent=e,document.getElementById("modal-body").innerHTML=t,document.getElementById("modal-footer").innerHTML=i,document.getElementById("modal-overlay").classList.remove("hidden")}function ui(){document.getElementById("modal-overlay").classList.add("hidden")}async function Mt(){if(!ve.user)return;const{data:e}=await w.from("users").select("*").eq("id",ve.user.id).single();e&&(ve.profile=e)}function St(e,t={}){ve.page=e,ve.params=t,wn()}async function wn(){var n,a,o,r;const e=document.getElementById("page-content");if(!e)return;document.querySelectorAll(".bottom-nav a").forEach(s=>{s.classList.toggle("active",s.dataset.page===ve.page)});const t=document.getElementById("nav-credits");t&&ve.profile&&(t.textContent=`💰 ${(ve.profile.credits||0).toLocaleString("fr")}`);const i={state:ve,navigate:St,toast:xt,openModal:Lr,closeModal:ui,refreshProfile:Mt};switch(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽</div>',ve.page){case"home":await qi(e,i);break;case"collection":await io(e,i);break;case"decks":await oi(e,i);break;case"boosters":await mo(e,i);break;case"ranked":await $r(e,i);break;case"match":{const s=ve.params&&ve.params.matchMode||"vs_ai_easy";s==="random"?await ci(e,i,!1):s==="ranked"?await ci(e,i,!0):s==="friend"?await Jo(e,i,(n=ve.params)==null?void 0:n.friendId,(a=ve.params)==null?void 0:a.friendName):s==="mini-league"?await Ki(e,i,(o=ve.params)==null?void 0:o.mlMatchId,(r=ve.params)==null?void 0:r.leagueId):await So(e,i);break}case"market":await hr(e,i);break;case"rankings":await kr(e,i);break;case"matches":await Er(e,i);break;case"friends":await qn(e,i);break;case"mini-league":await er(e,i);break;case"match-mini-league":{const s=ve.params||{};await Ki(e,i,s.mlMatchId,s.leagueId);break}default:await qi(e,i)}}function Tr(){var n;const e=document.getElementById("app"),t=ve.profile;if(!t)return;const i="/icons/";e.innerHTML=`
    <nav class="top-nav">
  ...
  `,document.querySelectorAll(".bottom-nav a").forEach(a=>{a.addEventListener("click",o=>{o.preventDefault(),St(a.dataset.page)})}),document.getElementById("nav-logo").addEventListener("click",()=>St("home")),document.getElementById("nav-credits").addEventListener("click",()=>St("boosters")),(n=document.getElementById("journal-btn"))==null||n.addEventListener("click",()=>zr())}async function zr(){const{data:e}=await w.from("patch_notes").select("*").eq("is_published",!0).order("published_at",{ascending:!1}).limit(20),t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9000;display:flex;align-items:center;justify-content:center;padding:16px";const i=(e||[]).map(n=>{const a=new Date(n.published_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"long",year:"numeric"});return`<div style="padding:14px 0;border-bottom:1px solid #f0f0f0">
    ...
    </div>`,document.body.appendChild(t),t.querySelector("#journal-close").addEventListener("click",()=>t.remove()),t.addEventListener("click",n=>{n.target===t&&t.remove()})}async function Mr(){document.documentElement.setAttribute("data-theme","light"),document.getElementById("modal-overlay").addEventListener("click",i=>{i.target===i.currentTarget&&ui()}),document.getElementById("modal-close").addEventListener("click",ui);const{data:{session:e}}=await w.auth.getSession();if(!e){Vi(),ji(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:xt});return}ve.user=e.user,await Mt(),Vi();try{const{data:i}=await w.from("formation_links_overrides").select("formation, links"),n={};(i||[]).forEach(a=>{n[a.formation]=a.links}),zn(n)}catch(i){console.warn("Impossible de charger les overrides de formation:",i)}if(!ve.profile){In(document.getElementById("app"),{state:ve,navigate:async()=>{await Mt(),ni()},toast:xt,refreshProfile:Mt});return}const t=Array.isArray(ve.profile.pending_boosters)?ve.profile.pending_boosters:[];if(!ve.profile.onboarding_done&&t.length>0){ko(document.getElementById("app"),{state:ve,navigate:()=>ni(),toast:xt,refreshProfile:Mt});return}ni(),setTimeout(()=>br(ve.profile,St,xt),800),w.auth.onAuthStateChange(async(i,n)=>{i==="SIGNED_OUT"&&(ve.user=null,ve.profile=null,document.getElementById("app").innerHTML="",ji(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:xt}))})}function Sr(){return Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight)}function Xt(){const e=document.getElementById("app");e&&(e.style.height=Sr()+"px")}window.addEventListener("resize",Xt);window.addEventListener("orientationchange",()=>setTimeout(Xt,150));window.visualViewport&&window.visualViewport.addEventListener("resize",Xt);function ni(){const e=()=>{var i;(i=ve.user)!=null&&i.id&&w.from("users").update({last_seen_at:new Date().toISOString()}).eq("id",ve.user.id).then(()=>{})};e(),window._presencePingInterval&&clearInterval(window._presencePingInterval),window._presencePingInterval=setInterval(e,6e4);const t=document.getElementById("app");t.style.display="flex",t.style.flexDirection="column",Xt(),Tr(),wn()}function Vi(){const e=document.getElementById("app-loader"),t=document.getElementById("app");t&&(t.style.display=""),e&&(e.classList.add("zoom-out"),setTimeout(()=>e.style.display="none",500))}function _n(e){var n;const t=document.getElementById("app-loader");if(t&&(t.style.display="none"),document.getElementById("boot-error"))return;const i=document.createElement("div");i.id="boot-error",i.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:99999;gap:16px;color:#fff;padding:24px;text-align:center",i.innerHTML=`
    <div style="font-size:42px">📡</div>
    <div style="font-size:18px;font-weight:900">Connexion impossible</div>
    <div style="font-size:13px;color:rgba(255,255,255,0.6);max-width:280px">${e||"Le chargement a échoué. Vérifie ta connexion et réessaie."}</div>
    <button id="boot-retry" style="margin-top:8px;padding:12px 30px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer">Réessayer</button>`,document.body.appendChild(i),(n=document.getElementById("boot-retry"))==null||n.addEventListener("click",()=>window.location.reload())}Mr().catch(e=>{console.error("Échec du démarrage:",e),_n()});setTimeout(()=>{const e=document.getElementById("app-loader");e&&e.style.display!=="none"&&!e.classList.contains("zoom-out")&&!document.getElementById("boot-error")&&_n("Le serveur met trop de temps à répondre.")},12e3);
