...
import{s as S,F as Ei,r as ot,j as Ot,l as Vi,m as rt,n as ji,h as Pt,o as hn,T as di,k as vn,p as wn,b as _n}from"./formation-links-D0diUT_D.js";const $n="/";function Ci(e,{navigate:t,toast:i}){let n="login";const d=()=>{var r,c,p,s,o,v;const a=n==="login";e.innerHTML=`
  ...
  `;function c(){var _;const s=document.getElementById("logo-preview"),o=document.getElementById("logo-initials"),v=((_=document.getElementById("setup-club"))==null?void 0:_.value)||"MW",m=v.trim().split(" ").filter(Boolean),y=m.length>=2?(m[0][0]+m[1][0]).toUpperCase():v.slice(0,2).toUpperCase();s&&(s.style.background=r,s.style.borderColor=a),o&&(o.textContent=y,o.style.color=a)}document.getElementById("color1").addEventListener("input",s=>{a=s.target.value,document.getElementById("swatch1").style.background=a,c()}),document.getElementById("color2").addEventListener("input",s=>{r=s.target.value,document.getElementById("swatch2").style.background=r,c()});function p(s){document.querySelectorAll(".setup-step").forEach(o=>o.classList.remove("active")),document.getElementById(`step-${s}`).classList.add("active"),document.getElementById("step-num").textContent=s,document.getElementById("progress-fill").style.width=`${Math.round(s/3*100)}%`,s===3&&c()}document.getElementById("step1-next").addEventListener("click",async()=>{const s=document.getElementById("setup-pseudo").value.trim(),o=document.getElementById("step1-error");if(o.textContent="",s.length<3){o.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:v}=await S.from("users").select("id").eq("pseudo",s).maybeSingle();if(v){o.textContent="Ce pseudo est déjà pris.";return}p(2)}),document.getElementById("step2-back").addEventListener("click",()=>p(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const s=document.getElementById("setup-club").value.trim(),o=document.getElementById("step2-error");if(o.textContent="",s.length<2){o.textContent="Nom trop court (min. 2 caractères).";return}const{data:v}=await S.from("users").select("id").eq("club_name",s).maybeSingle();if(v){o.textContent="Ce nom de club est déjà pris.";return}p(3)}),document.getElementById("step3-back").addEventListener("click",()=>p(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const s=document.getElementById("setup-pseudo").value.trim(),o=document.getElementById("setup-club").value.trim(),v=document.getElementById("step3-error"),m=document.getElementById("step3-finish");v.textContent="",m.disabled=!0,m.textContent="Création en cours…";try{const{error:y}=await S.from("users").insert({id:t.user.id,pseudo:s,club_name:o,club_color1:a,club_color2:r,credits:1e4});if(y)throw y;await En(t.user.id),await d(),n(`Bienvenue ${s} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(y){v.textContent=y.message,m.disabled=!1,m.textContent="🚀 Créer mon profil !"}})}async function En(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await S.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const Tn="modulepreload",Sn=function(e){return"/"+e},qi={},ni=function(t,i,n){let d=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const r=document.querySelector("meta[property=csp-nonce]"),c=(r==null?void 0:r.nonce)||(r==null?void 0:r.getAttribute("nonce"));d=Promise.allSettled(i.map(p=>{if(p=Sn(p),p in qi)return;qi[p]=!0;const s=p.endsWith(".css"),o=s?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${p}"]${o}`))return;const v=document.createElement("link");if(v.rel=s?"stylesheet":Tn,s||(v.as="script"),v.crossOrigin="",v.href=p,c&&v.setAttribute("nonce",c),document.head.appendChild(v),s)return new Promise((m,y)=>{v.addEventListener("load",m),v.addEventListener("error",()=>y(new Error(`Unable to preload CSS for ${p}`)))})}))}function a(r){const c=new Event("vite:preloadError",{cancelable:!0});if(c.payload=r,window.dispatchEvent(c),!c.defaultPrevented)throw r}return d.then(r=>{for(const c of r||[])c.status==="rejected"&&a(c.reason);return t().catch(a)})},Nt="#1A6B3C",Rt="#cc2222",zn="#D4A017",Bi="#888";async function An(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    </div>`}function Ln(e,t){const i=Ti();i.innerHTML=`
    <div class="popup-box">
  ...
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Yt=1e3;function Wn(e,t,i,n){var _,g,l;const{state:d,toast:a,closeModal:r,navigate:c,refreshProfile:p}=i,s=e.formation,o={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function v(){const T=Ot[s]||{},f=Ei[s]||[],M=290,j=360,F=20;function G(N){const K=T[N];return K?{x:K.x*M,y:K.y*j}:null}let q=`<svg width="${M}" height="${j}" viewBox="0 0 ${M} ${j}" xmlns="http://www.w3.org/2000/svg">`;for(const[N,K]of f){const Z=G(N),re=G(K);!Z||!re||(q+=`<line x1="${Z.x}" y1="${Z.y}" x2="${re.x}" y2="${re.y}"
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(_e=document.getElementById("close-detail"))==null||_e.addEventListener("click",c);let D=new Set;const ne=()=>{const fe=D.size,me=document.getElementById("sell-action-panel");if(!me)return;me.style.display=fe>0?"block":"none",document.getElementById("sell-selected-count").textContent=fe;const ge=document.getElementById("evolve-btn");ge&&(ge.textContent=`⬆️ Évoluer ${fe>1?"(+"+fe+")":""}`)};document.querySelectorAll(".expl-check").forEach(fe=>{fe.addEventListener("change",()=>{const me=fe.dataset.id;fe.checked?D.add(me):D.delete(me);const ge=fe.closest(".exemplaire-row");ge&&(ge.style.borderColor=fe.checked?"#1A6B3C":"#eee"),ne()})}),document.querySelectorAll(".exemplaire-row").forEach(fe=>{fe.addEventListener("click",me=>{if(me.target.closest("button")||me.target.tagName==="INPUT")return;const ge=fe.querySelector(".expl-check");ge&&!ge.disabled&&(ge.checked=!ge.checked,ge.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(fe=>{fe.addEventListener("click",me=>{me.stopPropagation();const ge=document.querySelector(`.expl-hist[data-hist="${fe.dataset.idx}"]`);ge&&(ge.style.display=ge.style.display==="none"?"flex":"none")})}),(ve=document.getElementById("evolve-btn"))==null||ve.addEventListener("click",async()=>{if(m<=1)return;const fe=[...D];if(!fe.length)return;if(D.has(e.id)){const pe=document.createElement("div");pe.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",pe.innerHTML=`
  ...
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
    ...
    </div>`),(r=document.getElementById("stad-none"))==null||r.addEventListener("click",()=>{e.stadiumCardId=null,d(),vt(t,e,i)}),document.querySelectorAll(".stad-choice").forEach(c=>{c.addEventListener("click",()=>{e.stadiumCardId=c.dataset.stadId,d(),vt(t,e,i)})})}function Qn(e,t,i,n){var _,g,l,T,f;const{openModal:d,closeModal:a}=n,r=e.replace(/\d+/,""),c=(_=t.stadiumCards)==null?void 0:_.find(M=>M.id===t.stadiumCardId),p=c&&((g=t.stadDefMap)==null?void 0:g[c.stadium_id])||null,s=t.slots[e],o=s?t.playerCards.find(M=>M.id===s):null;(l=o==null?void 0:o.player)==null||l.id;const v=new Set;Object.entries(t.slots).forEach(([M,j])=>{var G;if(M===e||!j)return;const F=t.playerCards.find(q=>q.id===j);(G=F==null?void 0:F.player)!=null&&G.id&&v.add(F.player.id)}),t.subs.forEach(M=>{var F;const j=t.playerCards.find(G=>G.id===M);(F=j==null?void 0:j.player)!=null&&F.id&&v.add(j.player.id)});const m=new Set,y=t.playerCards.filter(M=>{const j=M.player;return!(j.job===r||j.job2===r)||v.has(j.id)||m.has(j.id)?!1:(m.add(j.id),!0)});y.sort((M,j)=>{const F=M.evolution_bonus||0,G=j.evolution_bonus||0,q=(r==="GK"?M.player.note_g:r==="DEF"?M.player.note_d:r==="MIL"?M.player.note_m:M.player.note_a)+(r===M.player.job||r===M.player.job2?F:0);return(r==="GK"?j.player.note_g:r==="DEF"?j.player.note_d:r==="MIL"?j.player.note_m:j.player.note_a)+(r===j.player.job||r===j.player.job2?G:0)-q}),d(`Choisir ${r} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${y.length>0?'<div style="display:flex;flex-wrap:wrap;gap:8px;justify-content:center">'+y.map(M=>{const j={...M.player,_evolution_bonus:M.evolution_bonus||0};return`<div class="player-option" data-card-id="${M.id}" style="cursor:pointer">
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    </div>`,function(){const te=e.querySelector(".deck-preview-wrap svg");te&&(te.removeAttribute("width"),te.removeAttribute("height"),te.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",te.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),(N=document.getElementById("prev-deck"))==null||N.addEventListener("click",()=>{o>0&&(o--,v())}),(K=document.getElementById("next-deck"))==null||K.addEventListener("click",()=>{o<a.length-1&&(o++,v())}),(Z=document.getElementById("validate-deck"))==null||Z.addEventListener("click",()=>{if(!M)return;const le=t.state.params||{};t.navigate("match",{...le,matchMode:le.matchMode||i,deckId:m.id})}),(re=document.getElementById("cancel-deck-select"))==null||re.addEventListener("click",()=>{Ue(e),d("home")});const j=document.getElementById("deck-swipe-zone");if(j){let le=0,te=0;j.addEventListener("touchstart",H=>{le=H.touches[0].clientX,te=H.touches[0].clientY},{passive:!0}),j.addEventListener("touchend",H=>{const D=H.changedTouches[0].clientX-le,ne=H.changedTouches[0].clientY-te;Math.abs(D)<40||Math.abs(D)<Math.abs(ne)||(D<0&&o<a.length-1?(o++,v()):D>0&&o>0&&(o--,v()))},{passive:!0})}}v()}function Qe(e,t=44,i=58,n=null){return ot(e,{width:t,showStad:!!n,stadDef:n,used:e==null?void 0:e.used})}function tt(e,t,i){if(!(e!=null&&e.length))return"";const n=e.slice(0,5);let d='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return n.forEach((a,r)=>{const c=a._line||a.job||"MIL";if(d+=ot(a,{width:40,role:c,extraNote:a.boost||0}),r<n.length-1){const p=rt(a,n[r+1]);d+=`<div style="width:7px;height:3px;background:${p==="#ff3333"||p==="#cc2222"?"rgba(255,255,255,0.12)":p};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(d+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),d+="</div>",d}function ct(e,t,i,n,d=310,a=310,r=[]){const c=Ot[t]||{},p=Vi(t)||Ei[t]||[],s={},o=["ATT","MIL","DEF","GK"];for(const l of o)(e[l]||[]).forEach((f,M)=>{s[`${l}${M+1}`]=f});function v(l){const T=c[l];return T?{x:T.x*d,y:T.y*a}:null}let m="";for(const[l,T]of p){const f=v(l),M=v(T);if(!f||!M)continue;const j=s[l],F=s[T],G=rt(j,F);G==="#00ff88"||G==="#FFD700"?(m+=`<line x1="${f.x.toFixed(1)}" y1="${f.y.toFixed(1)}" x2="${M.x.toFixed(1)}" y2="${M.y.toFixed(1)}"
        stroke="${G}" stroke-width="10" stroke-linecap="round" opacity="0.22"/>`,m+=`<line x1="${f.x.toFixed(1)}" y1="${f.y.toFixed(1)}" x2="${M.x.toFixed(1)}" y2="${M.y.toFixed(1)}"
        stroke="${G}" stroke-width="3.5" stroke-linecap="round" opacity="0.95"/>`):m+=`<line x1="${f.x.toFixed(1)}" y1="${f.y.toFixed(1)}" x2="${M.x.toFixed(1)}" y2="${M.y.toFixed(1)}"
        stroke="${G}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const y=Math.max(44,Math.round(d*.095)),_=Math.round(y*657/507);for(const[l,T]of Object.entries(s)){const f=v(l);if(!f||!T)continue;const M=l.replace(/[0-9]/g,""),j=r.includes(T.cardId),F=i==="attack"&&(["MIL","ATT"].includes(M)||j)&&!T.used||i==="defense"&&["GK","DEF","MIL"].includes(M)&&!T.used,G=n.includes(T.cardId);let q=T.boost||0;T.stadiumBonus&&(i==="attack"&&(M==="ATT"||M==="MIL")||i==="defense"&&(M==="GK"||M==="DEF"||M==="MIL")?q+=10:i||(q+=10));const N=Math.round(f.x-y/2),K=Math.round(f.y-_/2);if(T.used){m+=`<image href="${`${typeof import.meta<"u"&&"/"||"/"}icons/carte-dos.png`}" x="${N}" y="${K}" width="${y}" height="${_}" preserveAspectRatio="xMidYMid slice" class="match-used-hit" data-card-id="${T.cardId}" data-role="${M}" style="cursor:pointer"/>`;continue}const Z=ot({...T,_evolution_bonus:0},{width:y,showStad:!0,stadDef:null,role:M,extraNote:q}),re=G?"outline:3px solid #FFD700;outline-offset:2px;border-radius:8px;opacity:0.75;":"";m+=`<foreignObject x="${N}" y="${K-26}" width="${y+4}" height="${_+56}" style="overflow:visible">
      <div xmlns="http://www.w3.org/1999/xhtml" style="${re}position:relative">
        ${Z}
      </div>
    </foreignObject>`,F&&(m+=`<rect x="${N}" y="${K}" width="${y}" height="${_}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${G?"selected":""}" data-card-id="${T.cardId}" data-role="${M}" style="cursor:pointer"/>`)}const g=80;return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-g} ${-g} ${d+g*2} ${a+g*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${m}
  ...
  <div style="text-align:center;flex-shrink:0;width:38px">
    ...
    <div style="font-size:7px;color:rgba(255,255,255,0.5);margin-top:1px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${(e.name||"").slice(0,7)}</div>
          ...
          card:cards(id, card_type, formation, evolution_bonus,
            player:players(id,firstname,surname_real,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
              clubs(encoded_name,logo_url)))`).eq("deck_id",p).order("slot_order")]);s=q[0].data,v=q[0].error,o=q[1].data,m=q[1].error}catch(q){console.error("[Match] Exception chargement deck:",q),Tt(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>a("home"));return}if(v||m){console.error("[Match] Erreur Supabase:",v||m),Tt(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>a("home"));return}const y=(o||[]).filter(q=>{var N;return q.is_starter&&((N=q.card)==null?void 0:N.player)}).map(q=>wi(q.card,q.position)),_=(o||[]).filter(q=>{var N;return!q.is_starter&&((N=q.card)==null?void 0:N.player)}).map(q=>wi(q.card,q.position));if(y.length<11){Tt(e,"⚠️",`Deck incomplet (${y.length}/11).`,"Compléter",()=>a("decks"));return}const g=(o||[]).find(q=>{var N;return((N=q.card)==null?void 0:N.card_type)==="formation"}),l=(s==null?void 0:s.formation)||((G=g==null?void 0:g.card)==null?void 0:G.formation)||"4-4-2",{data:T,error:f}=await S.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",d.profile.id).eq("card_type","game_changer"),{data:M}=await S.from("gc_definitions").select("*").eq("is_active",!0),j=(T||[]).map(q=>({...q,_gcDef:(M==null?void 0:M.find(N=>N.name===q.gc_type||N.id===q.gc_definition_id))||null}));let F=null;if(s!=null&&s.stadium_card_id){const{data:q}=await S.from("cards").select("stadium_id").eq("id",s.stadium_card_id).single();if(q!=null&&q.stadium_id){const{data:N}=await S.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").eq("id",q.stadium_id).single();F=N||null}}n({deckId:p,formation:l,starters:y,subsRaw:_,gcCardsEnriched:j,gcDefs:M||[],stadiumDef:F})}async function yo(e,t){const{state:i}=t,d=(i.params||{}).matchMode||"vs_ai_easy",a=d.replace("vs_ai_",""),r=d;await ai(e,t,d,async({deckId:c,formation:p,starters:s,subsRaw:o,gcCardsEnriched:v,gcDefs:m,stadiumDef:y})=>{try{let _=wt(s,p);y&&(_=St(_,y),Ct(o,y));const g=await ho(p,a),l=g.lines||g,T=async f=>{try{const{data:M,error:j}=await S.from("matches").insert({home_id:i.profile.id,away_id:null,mode:r,home_deck_id:c,status:"in_progress"}).select().single();if(j){console.error("[MatchIA] Erreur création match:",j),Tt(e,"⚠️","Impossible de créer le match ("+j.message+").","Retour",()=>t.navigate("home"));return}const F=g.stadiumDef||null;F&&l&&(St(l,F),Ct(g.subs||[],F));const G={gcDefs:m||[],matchId:M==null?void 0:M.id,mode:r,difficulty:a,formation:p,homeTeam:_,aiTeam:l,homeSubs:o,subsUsed:0,maxSubs:Math.min(o.length,3),aiSubs:g.subs||[],aiSubsUsed:0,aiMaxSubs:Math.min((g.subs||[]).length,3),aiUsedSubIds:[],aiGcCards:g.gcCards||[],aiUsedGc:[],aiStadiumDef:F,homeScore:0,aiScore:0,gcCards:f,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};wo(e,G,t)}catch(M){console.error("[MatchIA] Exception launchMatch:",M),Tt(e,"⚠️","Erreur au lancement du match : "+M.message,"Retour",()=>t.navigate("home"))}};if(!v.length){T([]);return}ri(e,v,T)}catch(_){console.error("[MatchIA] Exception setup:",_),Tt(e,"⚠️","Erreur de préparation du match : "+_.message,"Retour",()=>t.navigate("home"))}})}async function ho(e,t){var l;const{data:i}=await S.from("players").select("id,firstname,surname_real,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(80);if(!i||i.length<11)return{lines:vo(e),subs:[],gcCards:[],stadiumDef:null};const n=jt[e]||jt["4-4-2"],d={GK:[],DEF:[],MIL:[],ATT:[]},a=new Set;function r(T,f,M){var j,F;return a.add(T.id),{cardId:"ai-"+T.id+"-"+M,id:T.id,firstname:T.firstname,name:T.surname_real,country_code:T.country_code,club_id:T.club_id,job:T.job,job2:T.job2,note_g:Number(T.note_g)||0,note_d:Number(T.note_d)||0,note_m:Number(T.note_m)||0,note_a:Number(T.note_a)||0,rarity:T.rarity,skin:T.skin,hair:T.hair,hair_length:T.hair_length,clubName:((j=T.clubs)==null?void 0:j.encoded_name)||null,clubLogo:((F=T.clubs)==null?void 0:F.logo_url)||null,boost:0,used:!1,_line:f}}for(const T of["GK","DEF","MIL","ATT"]){const f=i.filter(q=>q.job===T&&!a.has(q.id)),M=i.filter(q=>q.job!==T&&!a.has(q.id)),j=[...f,...M],F=[];for(let q=0;q<n[T];q++){const N=j[q];N&&F.push(r(N,T,q))}const G=dt(F.length);F.forEach((q,N)=>{q._col=G[N]}),d[T]=F}const p=i.filter(T=>!a.has(T.id)).slice(0,5).map((T,f)=>r(T,T.job,100+f)),v=Object.keys(Re).sort(()=>Math.random()-.5).slice(0,3).map((T,f)=>{var M,j;return{id:"ai-gc-"+f,gc_type:T,name:((M=Re[T])==null?void 0:M.name)||T,icon:((j=Re[T])==null?void 0:j.icon)||"⚡"}}),m=Object.values(d).flat(),y={};m.forEach(T=>{T.club_id&&(y[T.club_id]=(y[T.club_id]||0)+1)});const _=(l=Object.entries(y).sort((T,f)=>f[1]-T[1])[0])==null?void 0:l[0];let g=null;if(_){const{data:T}=await S.from("clubs").select("id,encoded_name,logo_url,country_code").eq("id",_).single();T&&(g={club_id:T.id,country_code:null,name:T.encoded_name+" Stadium",club:{encoded_name:T.encoded_name,logo_url:T.logo_url}})}return{lines:d,subs:p,gcCards:v,stadiumDef:g}}function vo(e){const t=jt[e]||jt["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},n=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let d=0;for(const a of["GK","DEF","MIL","ATT"]){const r=[];for(let p=0;p<t[a];p++){const s=3+Math.floor(Math.random()*5);r.push({cardId:"fake-"+d,id:"fake-"+d,firstname:"IA",name:n[d%n.length],country_code:"XX",club_id:null,job:a,job2:null,note_g:a==="GK"?s:2,note_d:a==="DEF"?s:2,note_m:a==="MIL"?s:2,note_a:a==="ATT"?s:2,rarity:"normal",boost:0,used:!1,_line:a}),d++}const c=dt(r.length);r.forEach((p,s)=>{p._col=c[s]}),i[a]=r}return i}function wo(e,t,i){var n;e.innerHTML=`
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${f.map((q,N)=>{const K=N<f.length-1?rt(q,f[N+1]):null,Z=!K||K==="#ff3333"||K==="#cc2222",re=K==="#00ff88"?"+2":K==="#FFD700"?"+1":"";return c(q,G),q.stadiumBonus||G&&(G.club_id&&String(q.club_id)===String(G.club_id)||G.country_code&&(q.country_code,G.country_code)),`
          <div class="duel-card duel-card-${F}" data-idx="${N}" style="opacity:0;transform:translateY(18px) scale(0.7);transition:opacity .35s ease, transform .35s cubic-bezier(.34,1.56,.64,1);flex-shrink:0">
            ${ot({...q,_evolution_bonus:0},{width:window.innerWidth>=900?90:58,showStad:!0,stadDef:G,role:"MIL",extraNote:q.boost||0})}
          </div>
  ...
  </div>`;const _=()=>{const f=(M,j)=>e.querySelectorAll(M).forEach((F,G)=>{setTimeout(()=>{F.style.opacity="1",F.style.transform="translateY(0) scale(1)"},j+G*90)});f(".duel-card-home",150),f(".duel-card-ai",500),setTimeout(()=>e.querySelectorAll(".duel-link").forEach((M,j)=>{setTimeout(()=>{M.style.opacity="1"},j*70)}),900),setTimeout(()=>{const M=e.querySelector("#vs-label");M&&(M.style.opacity="1",M.style.animation="vsFlash .5s ease"),e.querySelectorAll(".duel-score-line").forEach(j=>j.style.opacity="1")},1250),setTimeout(()=>{g("score-home",o,800),g("score-ai",v,800)},1500)};function g(f,M,j){const F=document.getElementById(f);if(!F)return;const G=performance.now(),q=N=>{const K=Math.min(1,(N-G)/j);F.textContent=Math.round(M*(1-Math.pow(1-K,3))),K<1?requestAnimationFrame(q):F.textContent=M};requestAnimationFrame(q)}requestAnimationFrame(_),t.attacker=m?"home":"ai";const l=m?oi():null;m&&(t.boostCard={value:l}),t.log.push({type:"duel",title:"Milieu de Terrain",homePlayers:n.map(f=>ht(f)),aiPlayers:d.map(f=>ht(f)),homeTotal:o,aiTotal:v,text:`Duel milieu : ${t.clubName} ${o} – ${v} IA → ${m?t.clubName+" attaque":"IA attaque"}`});const T=()=>{t.phase=t.attacker==="home"?"attack":"ai-attack",Ye(e,t,i),t.attacker==="ai"&&setTimeout(()=>ei(e,t,i),800)};setTimeout(()=>{const f=document.getElementById("score-home"),M=document.getElementById("score-ai"),j=document.getElementById(m?"duel-row-home":"duel-row-ai"),F=document.getElementById(m?"duel-row-ai":"duel-row-home"),G=m?f:M,q=m?M:f;G&&(G.style.fontSize="80px",G.style.color=m?"#FFD700":"#ff6b6b",G.style.animation="duelPulse .5s ease"+(m?", duelGlow 1.5s ease infinite .5s":"")),q&&(q.style.opacity="0.25"),setTimeout(()=>{j&&(j.style.transformOrigin="center",j.style.animation="winnerSlam .5s cubic-bezier(.4,0,.7,1) forwards",j.style.zIndex="5"),setTimeout(()=>{var K;const N=document.getElementById("duel-shock");if(N){const Z=(K=F||j)==null?void 0:K.getBoundingClientRect(),re=e.querySelector(".match-screen").getBoundingClientRect();Z&&(N.style.top=Z.top-re.top+Z.height/2+"px"),N.style.animation="shockwave .5s ease-out forwards"}F&&(F.style.transformOrigin="center",F.style.animation="crushSquash .45s ease-in forwards"),navigator.vibrate&&navigator.vibrate([40,30,60])},320),setTimeout(()=>{var K;const N=document.getElementById("duel-finale");N&&(N.innerHTML=`
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    ${(()=>{const D=window.innerWidth>=700,ne=(O,X,ae)=>{var C,k;const se=(t.gcDefs||[]).find(u=>u.name===O.gc_type),pe={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[se==null?void 0:se.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",ee={purple:"#b06ce0",light_blue:"#00d4ef"}[se==null?void 0:se.color]||"#b06ce0",de=(se==null?void 0:se.name)||O.gc_type,Ae=(se==null?void 0:se.effect)||((C=Re[O.gc_type])==null?void 0:C.desc)||"",Te=se!=null&&se.image_url?`/icons/${se.image_url}`:null,ze=((k=Re[O.gc_type])==null?void 0:k.icon)||"⚡",ke=Math.round(ae*.22),Ce=Math.round(ae*.22),Fe=ae-ke-Ce,Se=de.length>12?7:9;return`<div class="gc-mini" data-gc-id="${O.id}" data-gc-type="${O.gc_type}"
        ...
        <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
          ${pt(t.homeTeam,t.formation,t.phase,n,Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.05),o)}
        </div>
      </div>`;return D?`
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(a);let c=!1;const p=()=>{c||(c=!0,a.remove(),setTimeout(()=>d(),50))};a.addEventListener("click",p),setTimeout(p,3500)}async function Ft(e,t,i){var o,v;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:n}=i,d=t.homeScore>t.aiScore,a=t.homeScore===t.aiScore,r=d?"victoire":a?"nul":"defaite",c=Fn(t.mode,r);t.matchId&&await S.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:d?n.profile.id:null,home_credits_reward:c,played_at:new Date().toISOString()}).eq("id",t.matchId);const p={credits:(n.profile.credits||0)+c,matches_played:(n.profile.matches_played||0)+1};d?p.wins=(n.profile.wins||0)+1:a?p.draws=(n.profile.draws||0)+1:p.losses=(n.profile.losses||0)+1,await S.from("users").update(p).eq("id",n.profile.id),await i.refreshProfile();const s=document.createElement("div");s.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",s.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const j=async(N=!0)=>{_=!0,g&&(S.removeChannel(g),g=null),N&&await S.rpc("cancel_matchmaking",{p_user_id:v.profile.id}).catch(()=>{})};(q=document.getElementById("mm-cancel"))==null||q.addEventListener("click",async()=>{try{await j(!0)}catch{}Ue(e),m("home")});const F=async(N,K,Z)=>{if(_)return;_=!0,g&&(S.removeChannel(g),g=null);const re=document.getElementById("mm-status");re&&(re.textContent="Adversaire trouvé !"),await new Promise(le=>setTimeout(le,600)),e.isConnected&&fn(e,t,N,Z,r,c,s,o,p)};if(s){const N=[{range:50,delay:0},{range:100,delay:15e3},{range:200,delay:3e4},{range:400,delay:45e3},{range:800,delay:6e4}];let K=0,Z=!1;const re=async()=>{if(_||Z)return;const{range:le}=N[K],te=document.getElementById("mm-range");te&&(te.textContent=`Plage MMR : ±${le}`);const{data:H,error:D}=await S.rpc("try_matchmake_ranked",{p_user_id:v.profile.id,p_deck_id:i,p_mmr:l,p_range:le});if(console.log("[Ranked] try_matchmake_ranked result:",H,"error:",JSON.stringify(D)),_)return;if(D||!(H!=null&&H.success)){y("Erreur matchmaking ranked","error"),Ue(e),m("home");return}if(H.matched){Z=!0,F(H.match_id,H.opponent_id,!1);return}const ne=document.getElementById("mm-status");if(ne&&(ne.textContent="En attente d'un adversaire..."),g||(g=S.channel("matchmaking-"+v.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${v.profile.id}`},xe=>{const _e=xe.new;_e.status==="matched"&&_e.match_id&&!Z&&(Z=!0,F(_e.match_id,_e.matched_with,!0))}).subscribe()),K<N.length-1){const xe=N[K+1].delay-N[K].delay;K++,setTimeout(()=>{!_&&!Z&&re()},xe)}};await re()}else{const{data:N,error:K}=await S.rpc("try_matchmake",{p_user_id:v.profile.id,p_deck_id:i});if(K||!(N!=null&&N.success)){y("Erreur de matchmaking","error"),Ue(e),m("home");return}if(N.matched){F(N.match_id,N.opponent_id,!1);return}const Z=document.getElementById("mm-status");Z&&(Z.textContent="En attente d'un autre joueur..."),g=S.channel("matchmaking-"+v.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${v.profile.id}`},re=>{const le=re.new;le.status==="matched"&&le.match_id&&F(le.match_id,le.matched_with,!0)}).subscribe()}}async function Do(e,t,i){const{state:n,navigate:d,toast:a}=t;try{(S.getChannels?S.getChannels():[]).forEach(s=>{const o=s.topic||"";(o.includes("matchmaking")||o.includes("pvp-match")||o.includes("friend-invite"))&&S.removeChannel(s)})}catch{}const{data:r}=await S.from("matches").select("home_id, away_id, status").eq("id",i).single();if(!r){a("Match introuvable","error"),d("home");return}if(r.status==="finished"){a("Ce match est terminé","info"),d("home");return}const c=r.home_id===n.profile.id;fn(e,t,i,c,[],[])}async function fn(e,t,i,n,d=[],a=[],r=!1,c=null,p=null){const{state:s,navigate:o,toast:v}=t,m=n?"p1":"p2",y=n?"p2":"p1",_=(d||[]).map(k=>k.id),g=(d||[]).map(k=>({id:k.id,gc_type:k.gc_type,_gcDef:k._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:l}=await S.from("matches").select("*").eq("id",i).single();if(!l){v("Match introuvable","error"),o("home");return}async function T(){var Y,ie;console.log("[PvP] buildGameState match:",{id:l.id,home_deck_id:l.home_deck_id,away_deck_id:l.away_deck_id,mode:l.mode,is_ranked:l.is_ranked});const[{data:k,error:u},{data:x,error:b},{data:h},{data:w}]=await Promise.all([S.rpc("get_deck_for_match",{p_deck_id:l.home_deck_id}),S.rpc("get_deck_for_match",{p_deck_id:l.away_deck_id}),S.from("users").select("id,pseudo,club_name").eq("id",l.home_id).single(),S.from("users").select("id,pseudo,club_name").eq("id",l.away_id).single()]);console.log("[PvP] get_deck_for_match p1:",u==null?void 0:u.message,"p2:",b==null?void 0:b.message,"p1D:",(Y=k==null?void 0:k.starters)==null?void 0:Y.length,"p2D:",(ie=x==null?void 0:x.starters)==null?void 0:ie.length);const $=J=>{const ue=Number(J.evolution_bonus)||0;return{cardId:J.card_id,position:J.position,id:J.id,firstname:J.firstname,name:J.surname_real,country_code:J.country_code,club_id:J.club_id,job:J.job,job2:J.job2,note_g:(Number(J.note_g)||0)+(J.job==="GK"||J.job2==="GK"&&Number(J.note_g)>0?ue:0),note_d:(Number(J.note_d)||0)+(J.job==="DEF"||J.job2==="DEF"&&Number(J.note_d)>0?ue:0),note_m:(Number(J.note_m)||0)+(J.job==="MIL"||J.job2==="MIL"&&Number(J.note_m)>0?ue:0),note_a:(Number(J.note_a)||0)+(J.job==="ATT"||J.job2==="ATT"&&Number(J.note_a)>0?ue:0),evolution_bonus:ue,rarity:J.rarity,skin:J.skin,hair:J.hair,hair_length:J.hair_length,face:J.face||null,clubName:J.club_encoded_name||null,clubLogo:J.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},z=((k==null?void 0:k.starters)||[]).map(J=>$(J)),E=((x==null?void 0:x.starters)||[]).map(J=>$(J)),L=(k==null?void 0:k.formation)||"4-4-2",A=(x==null?void 0:x.formation)||"4-4-2";let I=wt(z,L),B=wt(E,A);const V=((k==null?void 0:k.subs)||[]).map(J=>$(J)),Q=((x==null?void 0:x.subs)||[]).map(J=>$(J)),U=(k==null?void 0:k.stadium_def)||(n?p:null),P=(x==null?void 0:x.stadium_def)||(n?null:p);return U&&(I=St(I,U),Ct(V,U)),P&&(B=St(B,P),Ct(Q,P)),{p1Team:I,p2Team:B,p1Subs:V,p2Subs:Q,p1Formation:L,p2Formation:A,p1Name:(h==null?void 0:h.club_name)||(h==null?void 0:h.pseudo)||"Joueur 1",p2Name:(w==null?void 0:w.club_name)||(w==null?void 0:w.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:n?_:[],gc_p2:n?[]:_,gcCardsFull_p1:n?g:[],gcCardsFull_p2:n?[]:g,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:a||[],lastActionAt:new Date().toISOString()}}let f=l.game_state&&Object.keys(l.game_state).length?l.game_state:null;if(console.log("[PvP] init - amIHome:",n,"gameState exists:",!!f,"match.status:",l.status,"home_id:",l.home_id,"away_id:",l.away_id,"myId:",s.profile.id),!f)if(n){f=await T(),console.log("[PvP] home - gameState built, p1Team keys:",Object.keys((f==null?void 0:f.p1Team)||{}));const{data:k}=await S.from("matches").select("game_state").eq("id",i).single();!(k!=null&&k.game_state)||!Object.keys(k.game_state).length?await S.from("matches").update({game_state:f,turn_user_id:l.home_id}).eq("id",i):f=k.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>',console.log("[PvP] away - waiting for home to write game_state...");for(let k=0;k<30&&!f;k++){await new Promise(x=>setTimeout(x,400));const{data:u}=await S.from("matches").select("game_state").eq("id",i).single();u!=null&&u.game_state&&Object.keys(u.game_state).length&&(f=u.game_state),k%5===0&&console.log("[PvP] away - poll",k,"game_state:",!!(u!=null&&u.game_state))}if(!f){v("Erreur de synchronisation","error"),o("home");return}f.gc_p2=_,f.gcCardsFull_p2=g,await S.from("matches").update({game_state:f}).eq("id",i)}let M=!1,j=null,F=!1;const G=new Set,q=new Set;async function N(k){var A,I;try{S.removeChannel(K)}catch{}const u=f[m+"Score"]||0,x=f[y+"Score"]||0;let b,h;k.winner_id?(b=k.winner_id===s.profile.id,h=!1):k.forfeit?(b=u>x,h=!1):(h=u===x,b=u>x);let w="";if(r&&n)try{const{data:B}=await S.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",l.home_id).single(),{data:V}=await S.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",l.away_id).single();if(B&&V){const Q=b?1:h?.5:0,U=1-Q,P=B.placement_matches<10,Y=V.placement_matches<10,ie=ji(B.mmr,B.mmr_deviation,B.mmr_volatility,V.mmr,V.mmr_deviation,Q===1?1:Q===0?0:.5,P),J=ji(V.mmr,V.mmr_deviation,V.mmr_volatility,B.mmr,B.mmr_deviation,U===1?1:U===0?0:.5,Y);await S.rpc("update_mmr_after_ranked",{p_match_id:i,p_winner_id:h?null:b?l.home_id:l.away_id,p_home_id:l.home_id,p_away_id:l.away_id,p_home_delta:ie.delta,p_away_delta:J.delta,p_home_new_rd:ie.newRd,p_away_new_rd:J.newRd,p_home_new_vol:ie.newSigma,p_away_new_vol:J.newSigma});const ue=ie.delta,be=ie.newMmr,ye=Pt(be),Ee=ue>=0?"+":"",qe=ue>=0?"#4caf50":"#ff6b6b",Be=Pt(B.mmr);w=`
            ...
            </div>`}}catch(B){console.error("[Ranked] MMR update error:",B)}(A=document.getElementById("pvp-end-overlay"))==null||A.remove();const $=document.createElement("div");$.id="pvp-end-overlay",$.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;color:#fff;padding:24px;text-align:center;overflow-y:auto";const z=h?"🤝":b?"🏆":"😞",E=h?"MATCH NUL":b?"VICTOIRE !":"DÉFAITE",L=h?"#fff":b?"#FFD700":"#ff6b6b";$.innerHTML=`
      ...
      ${w}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${r?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild($),(I=$.querySelector("#pvp-end-home"))==null||I.addEventListener("click",()=>{$.remove(),Ue(e),o(r?"ranked":"home")})}const K=S.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},k=>{const u=k.new;try{if(u.status==="finished"||u.forfeit){if(M)return;M=!0,j&&(clearInterval(j),j=null),u.game_state&&(f=u.game_state),N(u);return}if(u.game_state){const x=f;f=u.game_state;const b=f._lastGC;if(b&&b.seq&&!q.has(b.seq)&&(q.add(b.seq),b.by!==m)){fe(b.type,b.by,()=>xe());return}const h=x[m+"Score"]||0,w=x[y+"Score"]||0,$=f[m+"Score"]||0,z=f[y+"Score"]||0,E=$>h,L=z>w;if((E||L)&&!G.has(f.round)){G.add(f.round);const A=[...f.log||[]].reverse().find(B=>B.isGoal),I=((A==null?void 0:A.homePlayers)||[]).map(B=>({name:B.name,note:B.note,portrait:B.portrait,job:B.job}));je(I,$,z,E,()=>xe());return}xe()}}catch(x){console.error("[PvP] crash:",x)}}).subscribe();async function Z(k){Object.assign(f,k),f.lastActionAt=new Date().toISOString();const{error:u}=await S.from("matches").update({game_state:f}).eq("id",i);u&&v("Erreur de synchronisation","error");try{xe()}catch(x){console.error("[PvP] renderPvpScreen crash:",x)}}async function re(){if(M)return;M=!0,j&&(clearInterval(j),j=null);const k=n?l.away_id:l.home_id,u=n?"p2":"p1",x=n?"p1":"p2",b={...f,[u+"Score"]:3,[x+"Score"]:0,phase:"finished"};try{await S.from("matches").update({status:"finished",forfeit:!0,winner_id:k,home_score:b.p1Score||0,away_score:b.p2Score||0,game_state:b}).eq("id",i)}catch{}try{S.removeChannel(K)}catch{}setTimeout(()=>{Ue(e),o("home")},800)}const le={BOOST_STAT:({value:k=1,count:u=1,roles:x=[]},b,h)=>{const w=b[m+"Team"],$=Object.entries(w).filter(([z])=>!x.length||x.includes(z)).flatMap(([z,E])=>E.filter(L=>!L.used).map(L=>({...L,_line:z})));if(!$.length){b.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),h(b);return}te($,u,`Choisir ${u} joueur(s) à booster (+${k})`,z=>{z.forEach(E=>{const L=(w[E._line]||[]).find(A=>A.cardId===E.cardId);L&&(L.boost=(L.boost||0)+k,b.log.push({text:`⚡ +${k} sur ${L.name}`,type:"info"}))}),b[m+"Team"]=w,h(b)})},DEBUFF_STAT:({value:k=1,count:u=1,roles:x=[],target:b="ai"},h,w)=>{const $=b==="home"?m:y,z=h[$+"Team"],E=b==="home"?"allié":"adverse",L=Object.entries(z).filter(([A])=>!x.length||x.includes(A)).flatMap(([A,I])=>I.map(B=>({...B,_line:A})));if(!L.length){h.log.push({text:`🎯 Aucun joueur ${E}`,type:"info"}),w(h);return}te(L,u,`Choisir ${u} joueur(s) ${E}(s) (-${k})`,A=>{A.forEach(I=>{const B=(z[I._line]||[]).find(V=>V.cardId===I.cardId);B&&(B.boost=(B.boost||0)-k,h.log.push({text:`🎯 -${k} sur ${B.name}`,type:"info"}))}),h[$+"Team"]=z,w(h)})},GRAY_PLAYER:({count:k=1,roles:u=[],target:x="ai"},b,h)=>{const w=x==="home"?m:y,$=b[w+"Team"],z=x==="home"?"allié":"adverse",E=Object.entries($).filter(([L])=>!u.length||u.includes(L)).flatMap(([L,A])=>A.filter(I=>!I.used).map(I=>({...I,_line:L})));if(!E.length){b.log.push({text:`❌ Aucun joueur ${z}`,type:"info"}),h(b);return}te(E,k,`Choisir ${k} joueur(s) ${z}(s) à exclure`,L=>{const A="usedCardIds_"+w,I=new Set(b[A]||[]);L.forEach(B=>{const V=($[B._line]||[]).find(Q=>Q.cardId===B.cardId);V&&(V.used=!0,I.add(B.cardId),b.log.push({text:`❌ ${V.name} exclu !`,type:"info"}))}),b[A]=[...I],b[w+"Team"]=$,h(b)})},REVIVE_PLAYER:({count:k=1,roles:u=[]},x,b)=>{const h=x[m+"Team"],w=Object.entries(h).filter(([$])=>!u.length||u.includes($)).flatMap(([$,z])=>z.filter(E=>E.used).map(E=>({...E,_line:$})));if(!w.length){x.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),b(x);return}te(w,k,`Choisir ${k} joueur(s) à ressusciter`,$=>{$.forEach(z=>{const E=(h[z._line]||[]).find(L=>L.cardId===z.cardId);E&&(E.used=!1,x.log.push({text:`💫 ${E.name} ressuscité !`,type:"info"}))}),x[m+"Team"]=h,b(x)})},REMOVE_GOAL:({},k,u)=>{const x=y+"Score";k[x]>0?(k[x]--,k.log.push({text:"🚫 Dernier but annulé !",type:"info"})):k.log.push({text:"🚫 Aucun but à annuler",type:"info"}),u(k)},ADD_GOAL_DRAW:({},k,u)=>{k[m+"Score"]===k[y+"Score"]?(k[m+"Score"]++,k.log.push({text:"🎯 But bonus !",type:"info"})):k.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),u(k)},ADD_SUB:({value:k=1},u,x)=>{u.maxSubs=(u.maxSubs||3)+k,u.log.push({text:`🔄 +${k} remplacement(s)`,type:"info"}),x(u)},CUSTOM:({},k,u)=>u(k)};function te(k,u,x,b){const h=document.createElement("div");h.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let w=[];function $(){var E,L;const z=k.map(A=>{const I={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[A._line]||"#555",B=We(A,A._line)+(A.boost||0),Q=w.find(P=>P.cardId===A.cardId)?"#FFD700":"rgba(255,255,255,0.25)",U=A.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${A.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${Q};background:${I};overflow:hidden;cursor:pointer;${U}">
      ...
      </div>`}const Ee=(W,R)=>R?ye(130,175):be(W,130,175),qe=(W,R)=>R?ye(68,95):be(W,68,95),Be=A?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",Le=$?ke(m)?`<button id="pvp-action" style="${Be};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${E.length===0?"disabled":""}>⚔️ ATTAQUEZ <span id="pvp-timer"></span></button>`:`<button id="pvp-action" data-pass="1" style="${Be};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER <span id="pvp-timer"></span></button>`:z?`<button id="pvp-action" style="${Be};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${E.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="pvp-timer"></span></button>`:`<div style="font-size:11px;color:rgba(255,255,255,0.3);text-align:center;padding:4px">⏳ Tour de ${w}</div>`,Ie=$&&!ke(m)?'<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">Aucun attaquant — passez la main</div>':$||z?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${E.length}/3 sélectionné(s)</div>`:"",Me=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${A?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${pt(k,f[m+"Formation"],Pe,L,Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.05),ue)}
      </div>
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn .3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(w);let E=!1;const L=()=>{E||(E=!0,w.remove(),setTimeout(()=>h(),50))};w.addEventListener("click",L),setTimeout(L,3500)}function fe(k,u,x){var U,P;const b=(f.gcDefs||[]).find(Y=>{var ie;return Y.name===k||((ie=Y.name)==null?void 0:ie.toLowerCase().trim())===(k==null?void 0:k.toLowerCase().trim())}),h={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[b==null?void 0:b.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",w={purple:"#b06ce0",light_blue:"#00d4ef"}[b==null?void 0:b.color]||"#b06ce0",$=(b==null?void 0:b.name)||k,z=(b==null?void 0:b.effect)||((U=Re[k])==null?void 0:U.desc)||"",E=b!=null&&b.image_url?`/icons/${b.image_url}`:null,L=((P=Re[k])==null?void 0:P.icon)||"⚡",I=u===m?"Vous":f[u+"Name"]||"Adversaire",B=document.createElement("div");B.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1100;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;overflow:hidden;cursor:pointer;padding:24px",B.innerHTML=`
      <style>
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        <div id="pin-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
          <div style="font-size:9px;color:#00ff88;letter-spacing:2px;text-transform:uppercase;font-weight:700">Joueur qui entre</div>
          <button id="pin-up" style="${J(L===0)}" ${L===0?"disabled":""}>▲</button>
          <div>${P?Qe({...P,used:!1,boost:0},Y,ie):"<div>—</div>"}</div>
          <button id="pin-down" style="${J(L>=$.length-1)}" ${L>=$.length-1?"disabled":""}>▼</button>
      ...
      </div>`,(be=I.querySelector("#psub-close"))==null||be.addEventListener("click",()=>I.remove()),(ye=I.querySelector("#pout-up"))==null||ye.addEventListener("click",()=>{z>0&&(z--,B())}),(Ee=I.querySelector("#pout-down"))==null||Ee.addEventListener("click",()=>{z<w.length-1&&(z++,B())}),(qe=I.querySelector("#pin-up"))==null||qe.addEventListener("click",()=>{L>0&&(L--,B())}),(Be=I.querySelector("#pin-down"))==null||Be.addEventListener("click",()=>{L<$.length-1&&(L++,B())});const ue=(Ie,Me,Pe,Oe)=>{const Ke=I.querySelector("#"+Ie);if(!Ke)return;let He=0;Ke.addEventListener("touchstart",Ne=>{He=Ne.touches[0].clientY},{passive:!0}),Ke.addEventListener("touchend",Ne=>{const Ge=Ne.changedTouches[0].clientY-He;if(Math.abs(Ge)<30)return;const Ve=Me();Ge<0&&Ve<Oe-1?(Pe(Ve+1),B()):Ge>0&&Ve>0&&(Pe(Ve-1),B())},{passive:!0})};ue("pin-panel",()=>L,Ie=>L=Ie,$.length),ue("pout-panel",()=>z,Ie=>z=Ie,w.length),(Le=I.querySelector("#psub-confirm"))==null||Le.addEventListener("click",async Ie=>{if(Ie.preventDefault(),Ie.stopPropagation(),A)return;A=!0;const Me=w[z],Pe=$[L];if(!Me||!Pe)return;const Oe=Me._line,Ke=(x[Oe]||[]).findIndex(Ge=>Ge.cardId===Me.cardId);if(Ke===-1){v("Erreur : joueur introuvable","error"),I.remove();return}const He={...Pe,_line:Oe,position:Me.position,used:!1,boost:0};x[Oe].splice(Ke,1,He);const Ne=[...b,Pe.cardId];I.remove(),X(Me,Pe,async()=>{await Z({[m+"Team"]:x,[y+"Team"]:f[y+"Team"],["usedSubIds_"+m]:Ne})})})}document.body.appendChild(I),B()}function X(k,u,x){const b={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},h=document.createElement("div");h.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const w=(E,L,A)=>`<div style="text-align:center">
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      ...
      </div>`,(H=document.getElementById("lobby-cancel"))==null||H.addEventListener("click",async()=>{_=!0,g&&(S.removeChannel(g),g=null),le&&await S.from("friend_match_invites").update({status:"declined"}).eq("id",le),Ue(e),v("home")})},T=async(Z,re)=>{_=!0,g&&(S.removeChannel(g),g=null),await new Promise(le=>setTimeout(le,600)),e.isConnected&&No(e,t,Z,re,r,c)},{data:f}=await S.from("friend_match_invites").select("*").eq("status","pending").or(`and(inviter_id.eq.${y},invitee_id.eq.${p}),and(inviter_id.eq.${p},invitee_id.eq.${y})`).order("created_at",{ascending:!1}).limit(1).maybeSingle();let M,j;if(f&&f.inviter_id===p)j=!1,M=f.id,await S.from("friend_match_invites").update({invitee_deck_id:i,invitee_ready:!0,status:"accepted"}).eq("id",M);else{j=!0;const{data:Z,error:re}=await S.from("friend_match_invites").insert({inviter_id:y,invitee_id:p,friend_id:p,inviter_deck_id:i,inviter_ready:!0,status:"pending"}).select().single();if(re||!Z){m("Erreur création invitation","error"),Ue(e),v("home");return}M=Z.id}if(l(!0,!j,M),!j){const Z={home_id:p,away_id:y,home_deck_id:f.inviter_deck_id,away_deck_id:i,status:"active",mode:"friend"},re=await S.from("matches").insert(Z).select().single(),le=re.data;if(!le){m("Erreur création match: "+(((K=re.error)==null?void 0:K.message)||""),"error"),Ue(e),v("home");return}await S.from("friend_match_invites").update({match_id:le.id,status:"matched"}).eq("id",M),T(le.id,!1);return}let F=!1;const G=Z=>{_||F||(Z.status==="matched"&&Z.match_id?(F=!0,clearInterval(q),clearInterval(N),T(Z.match_id,!0)):Z.status==="declined"?(clearInterval(q),clearInterval(N),m(`${s} a décliné l'invitation`,"warning"),Ue(e),v("home")):Z.invitee_ready&&l(!0,!0,M))},q=setInterval(()=>{if(_){clearInterval(q);return}S.from("matchmaking_queue").delete().eq("user_id",y).then(()=>{},()=>{})},3e3),N=setInterval(async()=>{if(_||F){clearInterval(N);return}const{data:Z}=await S.from("friend_match_invites").select("*").eq("id",M).maybeSingle();Z&&G(Z)},1200);g=S.channel("friend-invite-"+M).on("postgres_changes",{event:"UPDATE",schema:"public",table:"friend_match_invites",filter:`id=eq.${M}`},Z=>G(Z.new)).subscribe()}async function No(e,t,i,n,d=[],a=[]){const{state:r,navigate:c,toast:p}=t,s=n?"p1":"p2",o=n?"p2":"p1",v=(d||[]).map(C=>C.id),m=(d||[]).map(C=>({id:C.id,gc_type:C.gc_type,_gcDef:C._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:y}=await S.from("matches").select("*").eq("id",i).single();if(!y){p("Match introuvable","error"),c("home");return}async function _(){const[{data:C},{data:k},{data:u},{data:x}]=await Promise.all([S.rpc("get_deck_for_match",{p_deck_id:y.home_deck_id}),S.rpc("get_deck_for_match",{p_deck_id:y.away_deck_id}),S.from("users").select("id,pseudo,club_name").eq("id",y.home_id).single(),S.from("users").select("id,pseudo,club_name").eq("id",y.away_id).single()]),b=E=>{const L=Number(E.evolution_bonus)||0;return{cardId:E.card_id,position:E.position,id:E.id,firstname:E.firstname,name:E.surname_real,country_code:E.country_code,club_id:E.club_id,job:E.job,job2:E.job2,note_g:(Number(E.note_g)||0)+(E.job==="GK"||E.job2==="GK"&&Number(E.note_g)>0?L:0),note_d:(Number(E.note_d)||0)+(E.job==="DEF"||E.job2==="DEF"&&Number(E.note_d)>0?L:0),note_m:(Number(E.note_m)||0)+(E.job==="MIL"||E.job2==="MIL"&&Number(E.note_m)>0?L:0),note_a:(Number(E.note_a)||0)+(E.job==="ATT"||E.job2==="ATT"&&Number(E.note_a)>0?L:0),evolution_bonus:L,rarity:E.rarity,skin:E.skin,hair:E.hair,hair_length:E.hair_length,face:E.face||null,clubName:E.club_encoded_name||null,clubLogo:E.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},h=((C==null?void 0:C.starters)||[]).map(E=>b(E)),w=((k==null?void 0:k.starters)||[]).map(E=>b(E)),$=(C==null?void 0:C.formation)||"4-4-2",z=(k==null?void 0:k.formation)||"4-4-2";return{p1Team:(()=>{const E=wt(h,$);return stadiumDef?St(E,stadiumDef):E})(),p2Team:wt(w,z),p1Subs:(()=>{const E=((C==null?void 0:C.subs)||[]).map(L=>b(L));return stadiumDef&&Ct(E,stadiumDef),E})(),p2Subs:((k==null?void 0:k.subs)||[]).map(E=>b(E)),p1Formation:$,p2Formation:z,p1Name:(u==null?void 0:u.club_name)||(u==null?void 0:u.pseudo)||"Joueur 1",p2Name:(x==null?void 0:x.club_name)||(x==null?void 0:x.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:n?v:[],gc_p2:n?[]:v,gcCardsFull_p1:n?m:[],gcCardsFull_p2:n?[]:m,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:a||[],lastActionAt:new Date().toISOString()}}let g=y.game_state&&Object.keys(y.game_state).length?y.game_state:null;if(!g)if(n){g=await _();const{data:C}=await S.from("matches").select("game_state").eq("id",i).single();!(C!=null&&C.game_state)||!Object.keys(C.game_state).length?await S.from("matches").update({game_state:g,turn_user_id:y.home_id}).eq("id",i):g=C.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>';for(let C=0;C<60&&!g;C++){await new Promise(u=>setTimeout(u,400));const{data:k}=await S.from("matches").select("game_state").eq("id",i).single();k!=null&&k.game_state&&Object.keys(k.game_state).length&&(g=k.game_state)}if(!g){p("Erreur de synchronisation","error"),c("home");return}g.gc_p2=v,g.gcCardsFull_p2=m,await S.from("matches").update({game_state:g}).eq("id",i)}let l=!1,T=!1,f=!1,M=null,j=!1;const F=new Set,G=new Set;function q(C){var E,L;try{S.removeChannel(N)}catch{}const k=g[s+"Score"]||0,u=g[o+"Score"]||0;if(!f){f=!0;const A=C.winner_id||(k>u?r.profile.id:u>k?"opp":null),I=A===r.profile.id?"win":A?"loss":null;I&&ni(()=>Promise.resolve().then(()=>on),void 0).then(B=>B.applyOwnEvolution(r.profile.id,I)).catch(()=>{})}if(!T){const A=g.p1Score||0,I=g.p2Score||0,B=(g.usedGc_p1||[]).length,V=(g.usedGc_p2||[]).length,Q=C.winner_id||(A>I?y.home_id:I>A?y.away_id:null);(Q?r.profile.id===Q:r.profile.id<(n?y.away_id:y.home_id))&&(T=!0,Ji({player1Id:y.home_id,player2Id:y.away_id,score1:A,score2:I,gc1:B,gc2:V}).catch(P=>console.warn("[FriendMatch] updateStats:",P)))}let x,b;C.winner_id?(x=C.winner_id===r.profile.id,b=!1):C.forfeit?(x=k>u,b=!1):(b=k===u,x=k>u),(E=document.getElementById("pvp-end-overlay"))==null||E.remove();const h=document.createElement("div");h.id="pvp-end-overlay",h.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center";const w=b?"🤝":x?"🏆":"😞",$=b?"MATCH NUL":x?"VICTOIRE !":"DÉFAITE",z=b?"#fff":x?"#FFD700":"#ff6b6b";h.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(h),(L=h.querySelector("#pvp-end-home"))==null||L.addEventListener("click",()=>{h.remove(),Ue(e),c("home")})}const N=S.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},C=>{const k=C.new;try{if(k.status==="finished"||k.forfeit){if(l)return;l=!0,M&&(clearInterval(M),M=null),k.game_state&&(g=k.game_state),q(k);return}if(k.game_state){const u=g;g=k.game_state;const x=g._lastGC;if(x&&x.seq&&!G.has(x.seq)&&(G.add(x.seq),x.by!==s)){je(x.type,x.by,()=>ne());return}const b=u[s+"Score"]||0,h=u[o+"Score"]||0,w=g[s+"Score"]||0,$=g[o+"Score"]||0,z=w>b,E=$>h;if((z||E)&&!F.has(g.round)){F.add(g.round);const L=[...g.log||[]].reverse().find(I=>I.isGoal),A=((L==null?void 0:L.homePlayers)||[]).map(I=>({name:I.name,note:I.note,portrait:I.portrait,job:I.job}));we(A,w,$,z,()=>ne());return}ne()}}catch(u){console.error("[PvP] crash:",u)}}).subscribe();async function K(C){Object.assign(g,C),g.lastActionAt=new Date().toISOString();const{error:k}=await S.from("matches").update({game_state:g}).eq("id",i);k&&p("Erreur de synchronisation","error");try{ne()}catch(u){console.error("[PvP] renderPvpScreen crash:",u)}}async function Z(){if(l)return;l=!0,M&&(clearInterval(M),M=null);const C=n?y.away_id:y.home_id,k=n?"p2":"p1",u=n?"p1":"p2",x={...g,[k+"Score"]:3,[u+"Score"]:0,phase:"finished"};try{await S.from("matches").update({status:"finished",forfeit:!0,winner_id:C,home_score:x.p1Score||0,away_score:x.p2Score||0,game_state:x}).eq("id",i)}catch{}try{S.removeChannel(N)}catch{}setTimeout(()=>{Ue(e),c("home")},800)}const re={BOOST_STAT:({value:C=1,count:k=1,roles:u=[]},x,b)=>{const h=x[s+"Team"],w=Object.entries(h).filter(([$])=>!u.length||u.includes($)).flatMap(([$,z])=>z.filter(E=>!E.used).map(E=>({...E,_line:$})));if(!w.length){x.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),b(x);return}le(w,k,`Choisir ${k} joueur(s) à booster (+${C})`,$=>{$.forEach(z=>{const E=(h[z._line]||[]).find(L=>L.cardId===z.cardId);E&&(E.boost=(E.boost||0)+C,x.log.push({text:`⚡ +${C} sur ${E.name}`,type:"info"}))}),x[s+"Team"]=h,b(x)})},DEBUFF_STAT:({value:C=1,count:k=1,roles:u=[],target:x="ai"},b,h)=>{const w=x==="home"?s:o,$=b[w+"Team"],z=x==="home"?"allié":"adverse",E=Object.entries($).filter(([L])=>!u.length||u.includes(L)).flatMap(([L,A])=>A.map(I=>({...I,_line:L})));if(!E.length){b.log.push({text:`🎯 Aucun joueur ${z}`,type:"info"}),h(b);return}le(E,k,`Choisir ${k} joueur(s) ${z}(s) (-${C})`,L=>{L.forEach(A=>{const I=($[A._line]||[]).find(B=>B.cardId===A.cardId);I&&(I.boost=(I.boost||0)-C,b.log.push({text:`🎯 -${C} sur ${I.name}`,type:"info"}))}),b[w+"Team"]=$,h(b)})},GRAY_PLAYER:({count:C=1,roles:k=[],target:u="ai"},x,b)=>{const h=u==="home"?s:o,w=x[h+"Team"],$=u==="home"?"allié":"adverse",z=Object.entries(w).filter(([E])=>!k.length||k.includes(E)).flatMap(([E,L])=>L.filter(A=>!A.used).map(A=>({...A,_line:E})));if(!z.length){x.log.push({text:`❌ Aucun joueur ${$}`,type:"info"}),b(x);return}le(z,C,`Choisir ${C} joueur(s) ${$}(s) à exclure`,E=>{const L="usedCardIds_"+h,A=new Set(x[L]||[]);E.forEach(I=>{const B=(w[I._line]||[]).find(V=>V.cardId===I.cardId);B&&(B.used=!0,A.add(I.cardId),x.log.push({text:`❌ ${B.name} exclu !`,type:"info"}))}),x[L]=[...A],x[h+"Team"]=w,b(x)})},REVIVE_PLAYER:({count:C=1,roles:k=[]},u,x)=>{const b=u[s+"Team"],h=Object.entries(b).filter(([w])=>!k.length||k.includes(w)).flatMap(([w,$])=>$.filter(z=>z.used).map(z=>({...z,_line:w})));if(!h.length){u.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),x(u);return}le(h,C,`Choisir ${C} joueur(s) à ressusciter`,w=>{w.forEach($=>{const z=(b[$._line]||[]).find(E=>E.cardId===$.cardId);z&&(z.used=!1,u.log.push({text:`💫 ${z.name} ressuscité !`,type:"info"}))}),u[s+"Team"]=b,x(u)})},REMOVE_GOAL:({},C,k)=>{const u=o+"Score";C[u]>0?(C[u]--,C.log.push({text:"🚫 Dernier but annulé !",type:"info"})):C.log.push({text:"🚫 Aucun but à annuler",type:"info"}),k(C)},ADD_GOAL_DRAW:({},C,k)=>{C[s+"Score"]===C[o+"Score"]?(C[s+"Score"]++,C.log.push({text:"🎯 But bonus !",type:"info"})):C.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),k(C)},ADD_SUB:({value:C=1},k,u)=>{k.maxSubs=(k.maxSubs||3)+C,k.log.push({text:`🔄 +${C} remplacement(s)`,type:"info"}),u(k)},CUSTOM:({},C,k)=>k(C)};function le(C,k,u,x){const b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let h=[];function w(){var z,E;const $=C.map(L=>{const A={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[L._line]||"#555",I=We(L,L._line)+(L.boost||0),V=h.find(U=>U.cardId===L.cardId)?"#FFD700":"rgba(255,255,255,0.25)",Q=L.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${L.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${V};background:${A};overflow:hidden;cursor:pointer;${Q}">
      ...
      </div>`}const ye=(ce,W)=>W?be(130,175):ue(ce,130,175),Ee=(ce,W)=>W?be(68,95):ue(ce,68,95),qe=L?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",Be=w?ze(s)?`<button id="pvp-action" style="${qe};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${z.length===0?"disabled":""}>⚔️ ATTAQUEZ <span id="pvp-timer"></span></button>`:`<button id="pvp-action" data-pass="1" style="${qe};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER <span id="pvp-timer"></span></button>`:$?`<button id="pvp-action" style="${qe};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${z.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="pvp-timer"></span></button>`:`<div style="font-size:11px;color:rgba(255,255,255,0.3);text-align:center;padding:4px">⏳ Tour de ${h}</div>`,Le=w&&!ze(s)?'<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">Aucun attaquant — passez la main</div>':w||$?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${z.length}/3 sélectionné(s)</div>`:"",Ie=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${L?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
      ${B.length===0?'<div style="font-size:7px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':B.map(ce=>`<div class="pvp-sub-btn" data-sub-id="${ce.cardId}" style="cursor:pointer;flex-shrink:0">${Qe(ce,L?76:44,L?100:58)}</div>`).join("")}
    </div>`,Me=w?"attack":$?"defense":"idle",Pe=`<div style="overflow:hidden;min-width:0;flex:1;min-height:0;display:flex;flex-direction:column" id="match-field">
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${pt(C,g[s+"Formation"],Me,E,Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.05),J)}
      </div>
      ...
      </button>`;Ze(e),e.style.overflow="hidden",L?e.innerHTML=`
      <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
        ${He}
        <div style="display:flex;flex:1;min-height:0;overflow:hidden">
          ${Ie}
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(I);let B=!1;const V=()=>{B||(B=!0,I.remove(),setTimeout(()=>u&&u(),50))};I.addEventListener("click",V),setTimeout(V,3e3)}function fe(C,k){var I,B,V,Q;const x=(g["gcCardsFull_"+s]||[]).find(U=>U.id===C),b=x==null?void 0:x._gcDef,h={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[b==null?void 0:b.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",w={purple:"#b06ce0",light_blue:"#00d4ef"}[b==null?void 0:b.color]||"#b06ce0",$=(b==null?void 0:b.name)||k,z=(b==null?void 0:b.effect)||((I=Re[k])==null?void 0:I.desc)||"Carte spéciale.",E=b!=null&&b.image_url?`/icons/${b.image_url}`:null,L=((B=Re[k])==null?void 0:B.icon)||"⚡",A=document.createElement("div");A.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",A.innerHTML=`
      ...
      </div>`,document.body.appendChild(u),(x=u.querySelector("#bp-close"))==null||x.addEventListener("click",()=>u.remove()),u.querySelectorAll(".bp-item").forEach(b=>{b.addEventListener("click",async()=>{const h=b.dataset.cid,w=k.find(z=>z.cardId===h);if(!w)return;const $=(C[w._line]||[]).find(z=>z.cardId===h);$&&($.boost=($.boost||0)+g.boostValue),u.remove(),await K({[s+"Team"]:C,boostUsed:!0})})})}function ge(C=null){var B,V;if(!(g.phase===s+"-attack")){p("Remplacement uniquement avant votre attaque","warning");return}const u=g[s+"Team"],x=g["usedSubIds_"+s]||[],b=g.maxSubs||3;if(x.length>=b){p(`Maximum ${b} remplacements atteint`,"warning");return}const h=Object.entries(u).flatMap(([Q,U])=>(U||[]).filter(P=>P.used).map(P=>({...P,_line:Q}))),w=(g[s+"Subs"]||[]).filter(Q=>!x.includes(Q.cardId));if(!h.length){p("Aucun joueur utilisé à remplacer","warning");return}if(!w.length){p("Aucun remplaçant disponible","warning");return}let $=Math.max(0,h.findIndex(Q=>Q.cardId===C));const z=((B=h[$])==null?void 0:B._line)||((V=h[$])==null?void 0:V.job);let E=Math.max(0,w.findIndex(Q=>Q.job===z)),L=!1;const A=document.createElement("div");A.id="pvp-sub-overlay",A.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function I(){var ue,be,ye,Ee,qe,Be;const Q=h[$],U=w[E],P=Math.min(130,Math.round((window.innerWidth-90)/2)),Y=Math.round(P*1.35),ie=Le=>`background:rgba(255,255,255,0.12);border:none;color:${Le?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Le?"default":"pointer"};flex-shrink:0`;A.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
        <button id="psub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
      </div>`,(ue=A.querySelector("#psub-close"))==null||ue.addEventListener("click",()=>A.remove()),(be=A.querySelector("#pout-up"))==null||be.addEventListener("click",()=>{$>0&&($--,I())}),(ye=A.querySelector("#pout-down"))==null||ye.addEventListener("click",()=>{$<h.length-1&&($++,I())}),(Ee=A.querySelector("#pin-up"))==null||Ee.addEventListener("click",()=>{E>0&&(E--,I())}),(qe=A.querySelector("#pin-down"))==null||qe.addEventListener("click",()=>{E<w.length-1&&(E++,I())});const J=(Le,Ie,Me,Pe)=>{const Oe=A.querySelector("#"+Le);if(!Oe)return;let Ke=0;Oe.addEventListener("touchstart",He=>{Ke=He.touches[0].clientY},{passive:!0}),Oe.addEventListener("touchend",He=>{const Ne=He.changedTouches[0].clientY-Ke;if(Math.abs(Ne)<30)return;const Ge=Ie();Ne<0&&Ge<Pe-1?(Me(Ge+1),I()):Ne>0&&Ge>0&&(Me(Ge-1),I())},{passive:!0})};J("pin-panel",()=>E,Le=>E=Le,w.length),J("pout-panel",()=>$,Le=>$=Le,h.length),(Be=A.querySelector("#psub-confirm"))==null||Be.addEventListener("click",async Le=>{if(Le.preventDefault(),Le.stopPropagation(),L)return;L=!0;const Ie=h[$],Me=w[E];if(!Ie||!Me)return;const Pe=Ie._line,Oe=(u[Pe]||[]).findIndex(Ne=>Ne.cardId===Ie.cardId);if(Oe===-1){p("Erreur : joueur introuvable","error"),A.remove();return}const Ke={...Me,_line:Pe,position:Ie.position,used:!1,boost:0};u[Pe].splice(Oe,1,Ke);const He=[...x,Me.cardId];A.remove(),O(Ie,Me,async()=>{await K({[s+"Team"]:u,[o+"Team"]:g[o+"Team"],["usedSubIds_"+s]:He})})})}document.body.appendChild(A),I()}function O(C,k,u){const x={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const h=(z,E,L)=>`<div style="text-align:center">
      ...
      <div style="width:min(90vw,420px)">${ct(g[o+"Team"],g[o+"Formation"],null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.05))}</div>
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(C),(k=C.querySelector("#pvp-opp-close"))==null||k.addEventListener("click",()=>C.remove())}function ae(){var x;const C=g.log||[],k=document.createElement("div");k.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column";const u=b=>{var w,$,z;if(b.type==="duel"){const E=b.isGoal,L=b.homeScored&&s==="p1"||!b.homeScored&&E&&s==="p2",A=b.homeScored?"#FFD700":E?"#ff6b6b":"rgba(255,255,255,0.3)",I=E?L?"⚽ BUT !":"⚽ BUT adversaire !":(w=b.homePlayers)!=null&&w.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${E?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${A};margin-bottom:4px">
  ...
  </div>`,(T=document.getElementById("ml-create-btn"))==null||T.addEventListener("click",()=>Ko(e,t)),e.querySelectorAll(".ml-tab").forEach(f=>f.addEventListener("click",()=>zt(e,t,f.dataset.tab))),e.querySelectorAll("[data-league-id]").forEach(f=>f.addEventListener("click",()=>_t(e,t,f.dataset.leagueId))),e.querySelectorAll("[data-join]").forEach(f=>f.addEventListener("click",M=>{M.stopPropagation(),gn(e,t,f.dataset.join,f.dataset.type)})),e.querySelectorAll("[data-delete]").forEach(f=>f.addEventListener("click",M=>{M.stopPropagation(),$i(e,t,f.dataset.delete,f.dataset.name,i)}))}function ii(e,t,i=!1){var r,c;const n=e.creator_id===t,d=e.pot||0,a=((c=(r=e.mini_league_members)==null?void 0:r[0])==null?void 0:c.count)||0;return`<div data-league-id="${e.id}" style="background:#fff;border-radius:12px;padding:14px 16px;box-shadow:0 1px 6px rgba(0,0,0,0.08);cursor:pointer;margin-bottom:8px">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>${m.map((te,H)=>{const D=_>0&&r.status==="finished"?H===0?Math.floor(_*.7):H===1?Math.floor(_*.2):H===2?Math.floor(_*.1):0:0;return`<tr style="border-bottom:1px solid #f5f5f5;${te.userId===a?"background:#f0fdf4;":""}">
              <td style="padding:7px 3px 7px 0;font-weight:700;color:${H===0?yt:H<3?It:"#555"}">${["🥇","🥈","🥉"][H]||H+1}</td>
    ...
    <div style="background:linear-gradient(160deg,#1a0a2e,#3b1a6e);border-radius:20px;padding:32px 24px;max-width:320px;width:100%;text-align:center;color:#fff;box-shadow:0 8px 40px rgba(0,0,0,0.6)">
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">🏆 Retour à la Mini League</button>`,document.body.appendChild($),(I=$.querySelector("#pvp-end-home"))==null||I.addEventListener("click",()=>{$.remove(),Ue(e),s?c("mini-league",{openLeagueId:s}):c("mini-league")})}const K=S.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},u=>{const x=u.new;try{if(x.status==="finished"||x.forfeit){if(T||(T=!0,j&&(clearInterval(j),j=null),x.game_state&&(l=x.game_state),l.phase==="finished"&&!x.forfeit&&document.getElementById("pvp-home")))return;N(x);return}if(x.game_state){const b=l;l=x.game_state;const h=l._lastGC;if(h&&h.seq&&!q.has(h.seq)&&(q.add(h.seq),h.by!==o)){fe(h.type,h.by,()=>xe());return}const w=b[o+"Score"]||0,$=b[v+"Score"]||0,z=l[o+"Score"]||0,E=l[v+"Score"]||0,L=z>w,A=E>$;if((L||A)&&!G.has(l.round)){G.add(l.round);const I=[...l.log||[]].reverse().find(V=>V.isGoal),B=((I==null?void 0:I.homePlayers)||[]).map(V=>({name:V.name,note:V.note,portrait:V.portrait,job:V.job}));je(B,z,E,L,()=>xe());return}xe()}}catch(b){console.error("[PvP] crash:",b)}}).subscribe();async function Z(u){Object.assign(l,u),l.lastActionAt=new Date().toISOString();const{error:x}=await S.from("matches").update({game_state:l}).eq("id",i);x&&p("Erreur de synchronisation","error");try{xe()}catch(b){console.error("[PvP] renderPvpScreen crash:",b)}}async function re(){if(T)return;T=!0,j&&(clearInterval(j),j=null);const u=n?_.away_id:_.home_id,x=n?"p2":"p1",b=n?"p1":"p2",h={...l,[x+"Score"]:3,[b+"Score"]:0,phase:"finished"};try{await S.from("matches").update({status:"finished",forfeit:!0,winner_id:u,home_score:h.p1Score||0,away_score:h.p2Score||0,game_state:h}).eq("id",i)}catch{}try{S.removeChannel(K)}catch{}setTimeout(()=>{Ue(e),c("mini-league")},800)}const le={BOOST_STAT:({value:u=1,count:x=1,roles:b=[]},h,w)=>{const $=h[o+"Team"],z=Object.entries($).filter(([E])=>!b.length||b.includes(E)).flatMap(([E,L])=>L.filter(A=>!A.used).map(A=>({...A,_line:E})));if(!z.length){h.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),w(h);return}te(z,x,`Choisir ${x} joueur(s) à booster (+${u})`,E=>{E.forEach(L=>{const A=($[L._line]||[]).find(I=>I.cardId===L.cardId);A&&(A.boost=(A.boost||0)+u,h.log.push({text:`⚡ +${u} sur ${A.name}`,type:"info"}))}),h[o+"Team"]=$,w(h)})},DEBUFF_STAT:({value:u=1,count:x=1,roles:b=[],target:h="ai"},w,$)=>{const z=h==="home"?o:v,E=w[z+"Team"],L=h==="home"?"allié":"adverse",A=Object.entries(E).filter(([I])=>!b.length||b.includes(I)).flatMap(([I,B])=>B.map(V=>({...V,_line:I})));if(!A.length){w.log.push({text:`🎯 Aucun joueur ${L}`,type:"info"}),$(w);return}te(A,x,`Choisir ${x} joueur(s) ${L}(s) (-${u})`,I=>{I.forEach(B=>{const V=(E[B._line]||[]).find(Q=>Q.cardId===B.cardId);V&&(V.boost=(V.boost||0)-u,w.log.push({text:`🎯 -${u} sur ${V.name}`,type:"info"}))}),w[z+"Team"]=E,$(w)})},GRAY_PLAYER:({count:u=1,roles:x=[],target:b="ai"},h,w)=>{const $=b==="home"?o:v,z=h[$+"Team"],E=b==="home"?"allié":"adverse",L=Object.entries(z).filter(([A])=>!x.length||x.includes(A)).flatMap(([A,I])=>I.filter(B=>!B.used).map(B=>({...B,_line:A})));if(!L.length){h.log.push({text:`❌ Aucun joueur ${E}`,type:"info"}),w(h);return}te(L,u,`Choisir ${u} joueur(s) ${E}(s) à exclure`,A=>{const I="usedCardIds_"+$,B=new Set(h[I]||[]);A.forEach(V=>{const Q=(z[V._line]||[]).find(U=>U.cardId===V.cardId);Q&&(Q.used=!0,B.add(V.cardId),h.log.push({text:`❌ ${Q.name} exclu !`,type:"info"}))}),h[I]=[...B],h[$+"Team"]=z,w(h)})},REVIVE_PLAYER:({count:u=1,roles:x=[]},b,h)=>{const w=b[o+"Team"],$=Object.entries(w).filter(([z])=>!x.length||x.includes(z)).flatMap(([z,E])=>E.filter(L=>L.used).map(L=>({...L,_line:z})));if(!$.length){b.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),h(b);return}te($,u,`Choisir ${u} joueur(s) à ressusciter`,z=>{z.forEach(E=>{const L=(w[E._line]||[]).find(A=>A.cardId===E.cardId);L&&(L.used=!1,b.log.push({text:`💫 ${L.name} ressuscité !`,type:"info"}))}),b[o+"Team"]=w,h(b)})},REMOVE_GOAL:({},u,x)=>{const b=v+"Score";u[b]>0?(u[b]--,u.log.push({text:"🚫 Dernier but annulé !",type:"info"})):u.log.push({text:"🚫 Aucun but à annuler",type:"info"}),x(u)},ADD_GOAL_DRAW:({},u,x)=>{u[o+"Score"]===u[v+"Score"]?(u[o+"Score"]++,u.log.push({text:"🎯 But bonus !",type:"info"})):u.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),x(u)},ADD_SUB:({value:u=1},x,b)=>{x.maxSubs=(x.maxSubs||3)+u,x.log.push({text:`🔄 +${u} remplacement(s)`,type:"info"}),b(x)},CUSTOM:({},u,x)=>x(u)};function te(u,x,b,h){const w=document.createElement("div");w.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let $=[];function z(){var L,A;const E=u.map(I=>{const B={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[I._line]||"#555",V=We(I,I._line)+(I.boost||0),U=$.find(Y=>Y.cardId===I.cardId)?"#FFD700":"rgba(255,255,255,0.25)",P=I.used?"opacity:0.3;pointer-events:none":"";return`<div class="pp-item" data-cid="${I.cardId}" style="width:80px;border-radius:8px;border:2.5px solid ${U};background:${B};overflow:hidden;cursor:pointer;${P}">
        ...
        </div>`,(L=w.querySelector("#pp-close"))==null||L.addEventListener("click",()=>w.remove()),w.querySelectorAll(".pp-item").forEach(I=>{I.addEventListener("click",()=>{const B=I.dataset.cid,V=u.find(U=>U.cardId===B),Q=$.findIndex(U=>U.cardId===B);V&&(Q>-1?$.splice(Q,1):$.length<x&&$.push(V),z())})}),(A=w.querySelector("#pp-confirm"))==null||A.addEventListener("click",()=>{w.remove(),h($)})}z(),document.body.appendChild(w)}async function H(u,x){const h=(l["gcCardsFull_"+o]||[]).find(E=>E.id===u),w=(h==null?void 0:h._gcDef)||(l.gcDefs||[]).find(E=>{var L;return E.name===x||((L=E.name)==null?void 0:L.toLowerCase().trim())===(x==null?void 0:x.toLowerCase().trim())}),$=[...l["usedGc_"+o]||[],u],z={type:x,by:o,seq:(l._gcAnimSeq||0)+1};q.add(z.seq),fe(x,o,async()=>{if(w!=null&&w.effect_type&&w.effect_type!=="CUSTOM"){const L=le[w.effect_type];if(L){const A={...l};L(w.effect_params||{},A,async I=>{I["usedGc_"+o]=$,I._lastGC=z,I._gcAnimSeq=z.seq,await Z(I)});return}}const E={...l};switch(x){case"Remplacement+":E.maxSubs=(E.maxSubs||3)+1,E.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const L=v+"Score";E[L]>0&&(E[L]--,E.log.push({text:"🚫 But annulé",type:"info"}));break}}E["usedGc_"+o]=$,E._lastGC=z,E._gcAnimSeq=z.seq,await Z(E)})}function D(u,x){const b="usedCardIds_"+u,h=new Set(l[b]||[]);x.forEach(w=>h.add(w)),l[b]=[...h]}function ne(){for(const u of["p1","p2"]){const x=new Set(l["usedCardIds_"+u]||[]),b=l[u+"Team"];if(b)for(const h of["GK","DEF","MIL","ATT"])(b[h]||[]).forEach(w=>{w.used=x.has(w.cardId)})}}function xe(){var nt,ut,ft,gt,ce,W;if(l.phase==="reveal")return _e();if(l.phase==="midfield")return we();if(l.phase==="finished")return C();const u=l[o+"Team"],x=l[v+"Team"];ne();const b=l[o+"Score"],h=l[v+"Score"],w=l[o+"Name"],$=l[v+"Name"],z=l.phase===o+"-attack",E=l.phase===o+"-defense",L=Array.isArray(l["selected_"+o])?l["selected_"+o]:[],A=L.map(R=>R.cardId),I=window.innerWidth>=700,B=l[o+"Subs"]||[],V=l["usedSubIds_"+o]||[],Q=B.filter(R=>!V.includes(R.cardId)),U=l["gcCardsFull_"+o]||[],P=l["usedGc_"+o]||[],Y=U.filter(R=>!P.includes(R.id)),ie=l.boostOwner===o&&!l.boostUsed,J=!["GK","DEF","MIL","ATT"].some(R=>(x[R]||[]).some(oe=>!oe.used)),ue=[...u.MIL||[],...u.ATT||[]].filter(R=>!R.used),be=z&&J&&ue.length===0?[...u.GK||[],...u.DEF||[]].filter(R=>!R.used).map(R=>R.cardId):[];function ye(R,oe,he){var qt,Ii;const $e=R._gcDef,Je={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[$e==null?void 0:$e.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",Xe={purple:"#b06ce0",light_blue:"#00d4ef"}[$e==null?void 0:$e.color]||"#b06ce0",et=($e==null?void 0:$e.name)||R.gc_type,at=($e==null?void 0:$e.effect)||((qt=Re[R.gc_type])==null?void 0:qt.desc)||"",$t=$e!=null&&$e.image_url?`/icons/${$e.image_url}`:null,kt=((Ii=Re[R.gc_type])==null?void 0:Ii.icon)||"⚡",lt=Math.round(he*.22),mt=Math.round(he*.22),xt=he-lt-mt,Et=et.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${R.id}" data-gc-type="${R.gc_type}"
      ...
      </div>`}const qe=(R,oe)=>oe?Ee(130,175):ye(R,130,175),Be=(R,oe)=>oe?Ee(68,95):ye(R,68,95),Le=I?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",Ie=z?ke(o)?`<button id="pvp-action" style="${Le};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${L.length===0?"disabled":""}>⚔️ ATTAQUEZ <span id="pvp-timer"></span></button>`:`<button id="pvp-action" data-pass="1" style="${Le};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER <span id="pvp-timer"></span></button>`:E?`<button id="pvp-action" style="${Le};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${L.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="pvp-timer"></span></button>`:`<div style="font-size:11px;color:rgba(255,255,255,0.3);text-align:center;padding:4px">⏳ Tour de ${$}</div>`,Me=z&&!ke(o)?'<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">Aucun attaquant — passez la main</div>':z||E?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${L.length}/3 sélectionné(s)</div>`:"",Pe=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${I?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
      ${Q.length===0?'<div style="font-size:7px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':Q.map(R=>`<div class="pvp-sub-btn" data-sub-id="${R.cardId}" style="cursor:pointer;flex-shrink:0">${Qe(R,I?76:44,I?100:58)}</div>`).join("")}
      ...
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${pt(u,l[o+"Formation"],Oe,A,Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.05),be)}
      </div>
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="width:min(90vw,420px)">${ct(l[v+"Team"],l[v+"Formation"],null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.05))}</div>
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      </div>`,document.body.appendChild(b),(h=b.querySelector("#bp-close"))==null||h.addEventListener("click",()=>b.remove()),b.querySelectorAll(".bp-item").forEach(w=>{w.addEventListener("click",async()=>{const $=w.dataset.cid,z=x.find(L=>L.cardId===$);if(!z)return;const E=(u[z._line]||[]).find(L=>L.cardId===$);E&&(E.boost=(E.boost||0)+l.boostValue),b.remove(),await Z({[o+"Team"]:u,boostUsed:!0})})})}function O(u=null){var Q,U;if(!(l.phase===o+"-attack")){p("Remplacement uniquement avant votre attaque","warning");return}const b=l[o+"Team"],h=l["usedSubIds_"+o]||[],w=l.maxSubs||3;if(h.length>=w){p(`Maximum ${w} remplacements atteint`,"warning");return}const $=Object.entries(b).flatMap(([P,Y])=>(Y||[]).filter(ie=>ie.used).map(ie=>({...ie,_line:P}))),z=(l[o+"Subs"]||[]).filter(P=>!h.includes(P.cardId));if(!$.length){p("Aucun joueur utilisé à remplacer","warning");return}if(!z.length){p("Aucun remplaçant disponible","warning");return}let E=Math.max(0,$.findIndex(P=>P.cardId===u));const L=((Q=$[E])==null?void 0:Q._line)||((U=$[E])==null?void 0:U.job);let A=Math.max(0,z.findIndex(P=>P.job===L)),I=!1;const B=document.createElement("div");B.id="pvp-sub-overlay",B.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function V(){var ye,Ee,qe,Be,Le,Ie;const P=$[E],Y=z[A],ie=Math.min(130,Math.round((window.innerWidth-90)/2)),J=Math.round(ie*1.35),ue=Me=>`background:rgba(255,255,255,0.12);border:none;color:${Me?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Me?"default":"pointer"};flex-shrink:0`;B.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="width:70px;height:70px;border-radius:50%;background:${h[L.job]||"#555"};border:3px solid ${A};position:relative;overflow:hidden;margin:0 auto">
        ${it(L)?`<img src="${it(L)}" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover">`:""}
      </div>
      ...
      <div style="width:min(90vw,420px)">${ct(l[v+"Team"],l[v+"Formation"],null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.05))}</div>
      <button id="pvp-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(u),(x=u.querySelector("#pvp-opp-close"))==null||x.addEventListener("click",()=>u.remove())}function se(){var h;const u=l.log||[],x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column";const b=w=>{var z,E,L;if(w.type==="duel"){const A=w.isGoal,I=w.homeScored&&o==="p1"||!w.homeScored&&A&&o==="p2",B=w.homeScored?"#FFD700":A?"#ff6b6b":"rgba(255,255,255,0.3)",V=A?I?"⚽ BUT !":"⚽ BUT adversaire !":(z=w.homePlayers)!=null&&z.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${A?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${B};margin-bottom:4px">
          ...
          ${(L=w.aiPlayers)!=null&&L.length?`<div style="opacity:0.7">${tt(w.aiPlayers,"#ff6b6b",w.aiTotal)}</div>`:""}
      ...
      </div>`,document.body.appendChild(x),(h=x.querySelector("#pvp-hist-close"))==null||h.addEventListener("click",()=>x.remove())}async function pe(){if(l.phase!==o+"-attack")return;const u=o==="p1"?"p2":"p1",x=(l.round||0)+1,b=[...l.log||[]];b.push({type:"info",text:`⏭️ ${l[o+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const h=Ce(l),w=ke(u),$=h||!w?"finished":u+"-attack";await Z({["selected_"+o]:[],modifiers:{...l.modifiers,[o]:{}},pendingAttack:null,phase:$,attacker:u,round:x,log:b}),$==="finished"&&await Se(l)}async function ee(){const u=l[o+"Team"],x=!["GK","DEF","MIL","ATT"].some($=>(l[v+"Team"][$]||[]).some(z=>!z.used)),b=(l["selected_"+o]||[]).map($=>{const z=(u[$._role]||[]).find(V=>V.cardId===$.cardId)||$,E=x&&["GK","DEF"].includes($._role),L=u[$._role]||[],A=L.findIndex(V=>V.cardId===$.cardId),I=dt(L.length),B=A>=0?I[A]:z._col??1;return{...z,_line:$._role,_col:B,...E?{note_a:Math.max(1,Number(z.note_a)||0)}:{}}});if(!b.length)return;const h=Ht(b,l.modifiers[o]||{});D(o,b.map($=>$.cardId)),b.forEach($=>{const z=(u[$._role]||[]).find(E=>E.cardId===$.cardId);z&&(z.used=!0)}),l["selected_"+o]=[],xe();const w=[...l.log||[]];if(x){const $=(l[o+"Score"]||0)+1,z=b.map(B=>mi(B));w.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:z,homeTotal:h.total,aiTotal:0});const E=(l.round||0)+1,L=o==="p1"?"p2":"p1",A={...l,[o+"Team"]:u,[o+"Score"]:$},I=Ce(A);G.add(E),je(z,$,l[v+"Score"]||0,!0,async()=>{await Z({[o+"Team"]:u,[o+"Score"]:$,["selected_"+o]:[],modifiers:{...l.modifiers,[o]:{}},pendingAttack:null,phase:I?"finished":L+"-attack",attacker:L,round:E,log:w}),I&&await Se({...l,[o+"Score"]:$})});return}w.push({type:"pending",text:`⚔️ ${l[o+"Name"]} attaque (${h.total})`}),await Z({[o+"Team"]:u,[v+"Team"]:l[v+"Team"],pendingAttack:{...h,players:b,side:o},["selected_"+o]:[],modifiers:{...l.modifiers,[o]:{}},phase:v+"-defense",log:w})}async function de(){const u=l[o+"Team"],x=(l["selected_"+o]||[]).map(P=>{const Y=(u[P._role]||[]).find(ye=>ye.cardId===P.cardId)||P,ie=u[P._role]||[],J=ie.findIndex(ye=>ye.cardId===P.cardId),ue=dt(ie.length),be=J>=0?ue[J]:Y._col??1;return{...Y,_line:P._role,_col:be}}),b=Ut(x,l.modifiers[o]||{});D(o,x.map(P=>P.cardId)),x.forEach(P=>{const Y=(u[P._role]||[]).find(ie=>ie.cardId===P.cardId);Y&&(Y.used=!0)}),l["selected_"+o]=[],xe();const h=Kt(l.pendingAttack.total,b.total,l.modifiers[o]||{}),w=l.pendingAttack.side,$=h==="attack"||(h==null?void 0:h.goal),z=w==="p1"?"p2":"p1",E=(l.round||0)+1,L=(l.pendingAttack.players||[]).map(P=>mi(P)),A=[...l.log||[]];A.push({type:"duel",isGoal:$,homeScored:$&&w===o,text:$?`⚽ BUT de ${l[w+"Name"]} ! (${l.pendingAttack.total} vs ${b.total})`:`✋ Attaque stoppée (${l.pendingAttack.total} vs ${b.total})`,homePlayers:L,aiPlayers:x.map(P=>mi(P)),homeTotal:l.pendingAttack.total,aiTotal:b.total});const I=$?(l[w+"Score"]||0)+1:l[w+"Score"]||0,B={...l,[o+"Team"]:u,[w+"Score"]:I},V=Ce(B),Q=V?"finished":z+"-attack",U=async()=>{await Z({[o+"Team"]:u,[v+"Team"]:l[v+"Team"],[w+"Score"]:I,["selected_"+o]:[],modifiers:{...l.modifiers,[o]:{}},pendingAttack:null,phase:Q,attacker:z,round:E,log:A}),(Q==="finished"||V)&&await Se({...l,[w+"Score"]:I})};if($){const P=w===o,Y=P?I:l[o+"Score"]||0,ie=P?l[v+"Score"]||0:I;G.add(E),je(L,Y,ie,P,U)}else await U()}function Ae(u){return["MIL","ATT"].some(x=>(u[x]||[]).some(b=>!b.used))}function Te(u){return["GK","DEF","MIL","ATT"].some(x=>(u[x]||[]).some(b=>!b.used))}function ze(u){return Te(u)&&!Ae(u)}function ke(u){const x=l[u+"Team"],b=l[(u==="p1"?"p2":"p1")+"Team"];return!!(Ae(x)||!Te(b)&&ze(x))}function Ce(u){const x=["MIL","ATT"].some(A=>(u.p1Team[A]||[]).some(I=>!I.used)),b=["MIL","ATT"].some(A=>(u.p2Team[A]||[]).some(I=>!I.used)),h=Te(u.p1Team),w=Te(u.p2Team);return!x&&!(!w&&h)&&(!b&&!(!h&&w))}function Fe(u){const x=u.p1Score||0,b=u.p2Score||0;return x===b?null:x>b?_.home_id:_.away_id}async function Se(u){try{const x=Fe(u),b=x?_.home_id===x?_.away_id:_.home_id:null,h=u.p1Score||0,w=u.p2Score||0;await S.from("matches").update({status:"finished",winner_id:x,home_score:h,away_score:w}).eq("id",i);const{data:$}=await S.from("mini_league_matches").select("id, league_id, matchday").eq("match_id",i).single();if($){await S.from("mini_league_matches").update({home_score:h,away_score:w,status:"finished"}).eq("id",$.id);const{data:z}=await S.from("mini_league_matches").select("id, status").eq("league_id",$.league_id).eq("matchday",$.matchday);if((z||[]).every(L=>L.status==="finished"||L.status==="bye")){const{data:L}=await S.from("mini_leagues").select("current_day,total_days").eq("id",$.league_id).single();if(L){const A=(L.current_day||0)+1,I=A>(L.total_days||0);await S.from("mini_leagues").update({current_day:I?L.total_days:A,status:I?"finished":"active"}).eq("id",$.league_id)}}}x&&b&&Ai(x,b).catch(()=>{})}catch(x){console.error("[ML] finishMatch:",x)}}function C(){var w;if(T&&document.getElementById("pvp-end-overlay"))return;T=!0;const u=l[o+"Score"],x=l[v+"Score"],b=u>x,h=u===x;Ze(e),e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div id="mkt-content" style="padding:10px 16px;display:flex;flex-direction:column;gap:8px"></div>
  </div>`;let p="buy";const s=()=>{var g,l,T,f,M,j,F;return{name:(((g=document.getElementById("flt-name"))==null?void 0:g.value)||"").toLowerCase().trim(),club:(((l=document.getElementById("flt-club"))==null?void 0:l.value)||"").toLowerCase().trim(),country:(((T=document.getElementById("flt-country"))==null?void 0:T.value)||"").toLowerCase().trim(),job:((f=document.getElementById("flt-job"))==null?void 0:f.value)||"",rarity:((M=document.getElementById("flt-rarity"))==null?void 0:M.value)||"",note1:parseInt((j=document.getElementById("flt-note1"))==null?void 0:j.value)||0,note2:parseInt((F=document.getElementById("flt-note2"))==null?void 0:F.value)||0}};function o(g){const l=s();return g.filter(T=>{var K,Z,re;const f=(K=T.card)==null?void 0:K.player;if(!f)return!1;const M=`${f.firstname} ${f.surname_real}`.toLowerCase(),j=(((Z=f.clubs)==null?void 0:Z.encoded_name)||"").toLowerCase(),F=(f.country_code||"").toLowerCase(),G=((re=T.card)==null?void 0:re.evolution_bonus)||0,q=At(f,f.job,G),N=f.job2?At(f,f.job2,G):0;return!(l.name&&!M.includes(l.name)||l.club&&!j.includes(l.club)||l.country&&!F.includes(l.country)||l.job&&f.job!==l.job||l.rarity&&f.rarity!==l.rarity||l.note1&&q<l.note1||l.note2&&N<l.note2)})}function v(g){var N,K,Z,re;const l=(N=g.card)==null?void 0:N.player;if(!l)return"";const T=((K=g.card)==null?void 0:K.evolution_bonus)||0,f=At(l,l.job,T),M=l.job2?At(l,l.job2,T):0,j=(i.profile.credits||0)>=g.price,F=l.country_code?`https://flagsapi.com/${l.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,G=Jt[l.job]||"#bbb",q=l.job2?Jt[l.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden">
      <!-- Drapeau -->
      ...
      <div style="display:flex;gap:4px;flex-shrink:0">
        ...
        ${M?`<div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${q};display:flex;align-items:center;justify-content:center">
    ...
    </div>`}function m(g){var N,K,Z,re;const l=(N=g.card)==null?void 0:N.player;if(!l)return"";const T=((K=g.card)==null?void 0:K.evolution_bonus)||0,f=At(l,l.job,T),M=l.job2?At(l,l.job2,T):0,j=g.status==="sold",F=l.country_code?`https://flagsapi.com/${l.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,G=Jt[l.job]||"#bbb",q=l.job2?Jt[l.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${j?"opacity:0.7":""}">
      ${F?`<img src="${F}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      ${(Z=l.clubs)!=null&&Z.logo_url?`<img src="${l.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${G};display:flex;align-items:center;justify-content:center">
    ...
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center">
      ...
      `).join(""):'<div style="text-align:center;color:var(--gray-600);padding:40px">Aucun manager.</div>'}else{const T=r||[];_.innerHTML=T.length>0?T.map((f,M)=>{const j=Pt(f.mmr??1e3),F=(f.ranked_wins||0)+(f.ranked_losses||0)+(f.ranked_draws||0),G=F>0?Math.round((f.ranked_wins||0)/F*100):0,q=f.id===i.profile.id,N=(f.placement_matches||0)<10;return`
  ...
  <div style="min-height:100%;background:${f[_.id]};padding:16px;overflow-y:auto;display:flex;flex-direction:column;gap:16px">
    ...
    <div style="display:flex;flex-direction:column;gap:10px;margin-top:auto;padding-top:8px">
      ...
      away:users!away_id(pseudo,club_name)`).or(`home_id.eq.${d.id},away_id.eq.${d.id}`).order("created_at",{ascending:!1}).limit(50),r={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat",vs_random:"Match Random"},c=(a||[]).filter(o=>o.status==="finished"),p=(a||[]).filter(o=>o.status==="in_progress");function s(o){const v=o.home_id===d.id;v?o.home_score:o.away_score,v?o.away_score:o.home_score;const m=o.winner_id===d.id,y=o.home_score===o.away_score&&o.status==="finished",_=o.status!=="finished"?"…":y?"N":m?"V":"D",g=o.status!=="finished"||y?"#888":m?"#1A6B3C":"#c0392b";let l=r[o.mode]||o.mode;o.away_id===null&&!l.startsWith("IA")&&(l="IA");const f=o.home_id===d.id?o.away:o.home;let M;o.away_id===null?M=l:f?M=`${f.club_name||f.pseudo} (${f.pseudo})`:M="Adversaire";let j="";o.status==="in_progress"&&Date.now()-new Date(o.created_at).getTime()>3600*1e3&&(j=' <span style="color:#e67e22;font-weight:700">(VAR en cours)</span>');const F=new Date(o.created_at),G=F.toLocaleDateString("fr",{day:"numeric",month:"short"})+" "+F.toLocaleTimeString("fr",{hour:"2-digit",minute:"2-digit"}),q=o.status==="finished"?`${o.home_score} - ${o.away_score}`:`${o.home_score||0} - ${o.away_score||0}`;return`<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 14px;border-bottom:1px solid var(--gray-200)">
      <div style="flex:1">
