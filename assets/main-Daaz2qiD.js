...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(he=document.getElementById("close-detail"))==null||he.addEventListener("click",d);let B=new Set;const pe=c.rarity==="pepite"?1.3:c.rarity==="papyte"?.7:1;function ve(){let k=0;return document.querySelectorAll(".expl-check:checked").forEach(j=>{if(j.dataset.id===t.id)return;const G=Number(j.dataset.evo)||0,V=Number(j.dataset.note)||0;k+=V+G}),Math.round(k*pe)}const C=()=>{const k=B.size,j=document.getElementById("sell-action-panel");if(!j)return;j.style.display=k>0?"block":"none",document.getElementById("sell-selected-count").textContent=k;const K=document.getElementById("evolve-btn");if(K){const G=ve();K.textContent=G>0?`⬆️ Évoluer (+${G})`:"⬆️ Évoluer"}};document.querySelectorAll(".expl-check").forEach(k=>{k.addEventListener("change",()=>{const j=k.dataset.id;k.checked?B.add(j):B.delete(j);const K=k.closest(".exemplaire-row");if(K){const G=K.querySelector(".expl-sel-overlay"),V=K.querySelector(".expl-sel-check");G&&(G.style.display=k.checked?"block":"none"),V&&(V.style.display=k.checked?"flex":"none"),K.style.transform=k.checked?"scale(1.05)":"scale(1)"}C()})}),document.querySelectorAll(".exemplaire-row").forEach(k=>{k.addEventListener("click",j=>{if(j.target.tagName==="INPUT")return;const K=k.querySelector(".expl-check");K&&!K.disabled&&(K.checked=!K.checked,K.dispatchEvent(new Event("change")))})}),(be=document.getElementById("evolve-btn"))==null||be.addEventListener("click",async()=>{if(y<=1)return;const k=[...B];if(!k.length)return;if(B.has(t.id)){const ae=document.createElement("div");ae.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",ae.innerHTML=`
  ...
  <div class="page">
    ...
    <nav class="top-nav">
      <div class="logo" id="nav-logo" title="Manager Wars v2026.08.02-0041" style="cursor:pointer">
