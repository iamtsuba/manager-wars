import{s as k,l as U,F as _e,b as ue,c as He,d as ut}from"./formation-links-BrrCMBql.js";function Ce(e,{navigate:t,toast:i}){e.innerHTML=`
  ...
  </div>`}function vt(e){const t=e.job||"ATT",i=te(e,t),a=ke[e.country_code]||e.country_code||"";return`
  <div style="
    width:140px;border-radius:12px;padding:6px;
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),n=(l||[]).filter(A=>A.card_type==="player"&&A.player),c=(l||[]).filter(A=>A.card_type==="game_changer"),p=(l||[]).filter(A=>A.card_type==="formation"),f=Object.keys(_e),u=Object.keys(ee),m={};n.forEach(A=>{const z=A.player.id;m[z]=(m[z]||0)+1}),new Set(Object.keys(m).map(A=>String(A)));const x=new Set(p.map(A=>A.formation)),b=new Set(c.map(A=>A.gc_type));let $="player",y="Tous",g="",v=!1;function L(){return[...n].sort((A,z)=>{const B=ne.indexOf(A.player.job),T=ne.indexOf(z.player.job);return B!==T?B-T:(A.player.surname_encoded||"").localeCompare(z.player.surname_encoded||"")})}function h(){return[...r||[]].sort((A,z)=>{const B=ne.indexOf(A.job),T=ne.indexOf(z.job);return B!==T?B-T:(A.surname_encoded||"").localeCompare(z.surname_encoded||"")})}function w(){return L().filter(A=>{const z=A.player,B=y==="Tous"||z.job===y,T=!g||`${z.firstname} ${z.surname_encoded}`.toLowerCase().includes(g);return B&&T})}function _(){return h().filter(A=>{const z=y==="Tous"||A.job===y,B=!g||`${A.firstname} ${A.surname_encoded}`.toLowerCase().includes(g);return z&&B})}e.innerHTML=`
  <div class="page">
    ...
    <div class="page-body">
      <div id="col-grid" style="width:100%;min-height:100px;user-select:none"></div>
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{v=!v,I(),E()})))}function E(){const A=document.getElementById("col-grid");A&&($==="player"?C(A):$==="formation"?j(A):G(A))}function C(A){if(v){const z=_();if(!z.length){A.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const B=z.map(T=>n.find(M=>M.player.id===T.id)||{_fake:!0,player:T,id:"fake-"+T.id});renderBigAndStrip(B,T=>T._fake?vt(T.player):Se(T,""),T=>T._fake?miniPlayerCard({player:T.player,id:"x",_fake:!0}):miniPlayerCard(T),T=>{T._fake||Fe(T,n,m,t)},"#1A6B3C")}else{const z=w();if(!z.length){A.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const B={},T=[];z.forEach(M=>{B[M.player.id]||(B[M.player.id]=!0,T.push(M))}),renderBigAndStrip(T,M=>{const R=m[M.player.id]||1,q=R>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${R}</div>`:"",X=n.filter(oe=>oe.player.id===M.player.id&&oe.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Se(M,q+X)},M=>miniPlayerCard(M),M=>Fe(M,n,m,t),"#1A6B3C")}}function j(A){const z=v?f:[...x];if(!z.length){A.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const B=z.map(T=>({formation:T,card:p.find(M=>M.formation===T)||null,owned:x.has(T)}));renderBigAndStrip(B,({formation:T,card:M,owned:R})=>{const q=R?p.filter(X=>X.formation===T).length:0,J=q>1?`<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${q}</div>`:"";return R&&M?`<div data-form-id="${M.id}" style="position:relative;background:linear-gradient(135deg,#1A6B3C,#2a8f52);border:3px solid #2a8f52;border-radius:16px;padding:28px 24px;color:#fff;text-align:center;cursor:pointer;min-width:200px">${J}<div style="font-size:52px;margin-bottom:8px">🏟️</div><div style="font-size:10px;background:rgba(255,255,255,0.2);padding:3px 12px;border-radius:10px;display:inline-block;margin-bottom:8px">FORMATION</div><div style="font-weight:900;font-size:28px">${T}</div></div>`:`<div style="background:#f0f0f0;border:2px solid #ddd;border-radius:16px;padding:28px 24px;color:#aaa;text-align:center;min-width:200px;filter:grayscale(1);opacity:0.5"><div style="font-size:52px;margin-bottom:8px">🏟️</div><div style="font-size:10px;background:rgba(0,0,0,0.1);padding:3px 12px;border-radius:10px;display:inline-block;margin-bottom:8px">FORMATION</div><div style="font-weight:900;font-size:28px">${T}</div><div style="font-size:11px;margin-top:6px">Non possédée</div></div>`},({formation:T,owned:M})=>M?`<div style="background:linear-gradient(135deg,#1A6B3C,#2a8f52);border-radius:8px;padding:8px 6px;color:#fff;text-align:center;width:70px"><div style="font-size:18px">🏟️</div><div style="font-size:7px;margin-top:2px">${T}</div></div>`:`<div style="background:#eee;border-radius:8px;padding:8px 6px;color:#aaa;text-align:center;width:70px;filter:grayscale(1);opacity:0.5"><div style="font-size:18px">🏟️</div><div style="font-size:7px;margin-top:2px">${T}</div></div>`,({card:T,owned:M})=>{M&&T&&_t(T,p,t,o)},"#1A6B3C")}function G(A){const z=v?u:[...b];if(!z.length){A.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const B=z.map(T=>({type:T,gc:ee[T]||{icon:"⚡",desc:""},owned:b.has(T),card:c.find(M=>M.gc_type===T)||null}));renderBigAndStrip(B,({type:T,gc:M,owned:R,card:q})=>{const J=R?c.filter(oe=>oe.gc_type===T).length:0,X=J>1?`<div style="position:absolute;top:4px;right:4px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${J}</div>`:"";return R&&q?`<div data-gc-id="${q.id}" data-gc-type="${T}" style="position:relative;background:linear-gradient(135deg,#3d0a7a,#7a28b8);border:3px solid #9b59b6;border-radius:16px;padding:28px 24px;color:#fff;text-align:center;cursor:pointer;min-width:200px">${X}<div style="font-size:56px;margin-bottom:8px">${M.icon}</div><div style="font-size:10px;background:rgba(255,255,255,0.2);padding:3px 12px;border-radius:10px;display:inline-block;margin-bottom:8px">GAME CHANGER</div><div style="font-weight:900;font-size:22px;margin-bottom:6px">${T}</div><div style="font-size:12px;color:rgba(255,255,255,0.75);max-width:200px;margin:0 auto">${M.desc}</div></div>`:`<div style="background:#f0f0f0;border:2px solid #ddd;border-radius:16px;padding:28px 24px;color:#aaa;text-align:center;min-width:200px;filter:grayscale(1);opacity:0.5"><div style="font-size:56px;margin-bottom:8px">${M.icon}</div><div style="font-size:10px;background:rgba(0,0,0,0.1);padding:3px 12px;border-radius:10px;display:inline-block;margin-bottom:8px">GAME CHANGER</div><div style="font-weight:900;font-size:20px">${T}</div><div style="font-size:11px;margin-top:6px">Non possédée</div></div>`},({type:T,gc:M,owned:R})=>R?`<div style="background:linear-gradient(135deg,#3d0a7a,#7a28b8);border-radius:8px;padding:8px 6px;color:#fff;text-align:center;width:70px"><div style="font-size:22px">${M.icon}</div><div style="font-size:7px;margin-top:2px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${T}</div></div>`:`<div style="background:#eee;border-radius:8px;padding:8px 6px;color:#aaa;text-align:center;width:70px;filter:grayscale(1);opacity:0.5"><div style="font-size:22px">${M.icon}</div><div style="font-size:7px;margin-top:2px">${T}</div></div>`,({type:T,owned:M})=>{M&&$t(T,o)},"#7a28b8")}e.querySelectorAll(".col-tab-btn").forEach(A=>{A.addEventListener("click",()=>{$=A.dataset.tab,y="Tous",g="",v=!1,e.querySelectorAll(".col-tab-btn").forEach(z=>{const B=z.dataset.tab===$;z.style.borderBottomColor=B?"var(--green)":"transparent",z.style.color=B?"var(--green)":"var(--gray-600)"}),I(),E()})}),I(),E()}function $t(e,t){const i=ee[e]||{icon:"⚡",desc:"Effet spécial."};t("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Q=1e3;function _t(e,t,i,a){var x,b,$;const{state:d,toast:o,closeModal:s,navigate:l,refreshProfile:r}=i,n=e.formation,c={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function p(){const y=ue[n]||{},g=_e[n]||[],v=290,L=360,h=20;function w(I){const E=y[I];return E?{x:E.x*v,y:E.y*L}:null}let _=`<svg width="${v}" height="${L}" viewBox="0 0 ${v} ${L}" xmlns="http://www.w3.org/2000/svg">`;for(const[I,E]of g){const C=w(I),j=w(E);!C||!j||(_+=`<line x1="${C.x}" y1="${C.y}" x2="${j.x}" y2="${j.y}"
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(x=document.getElementById("direct-sell-form-btn"))==null||x.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${n} pour ${Q.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const y=f.find(v=>!v.is_for_sale)||f[0];if(!y){o("Aucune carte à vendre","error");return}const{error:g}=await k.from("cards").delete().eq("id",y.id);if(g){o(g.message,"error");return}await k.from("users").update({credits:(d.profile.credits||0)+Q}).eq("id",d.profile.id),await r(),o(`+${Q.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),s(),l("collection")}),(b=document.getElementById("market-sell-form-btn"))==null||b.addEventListener("click",async()=>{const y=parseInt(document.getElementById("sell-price-form").value);if(!y||y<1){o("Prix invalide","error");return}await k.from("cards").update({is_for_sale:!0,sale_price:y}).eq("id",e.id),await k.from("market_listings").insert({seller_id:d.profile.id,card_id:e.id,price:y}),o("Carte mise en vente sur le marché !","success"),s(),l("collection")}),($=document.getElementById("cancel-sell-form-btn"))==null||$.addEventListener("click",async()=>{await k.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await k.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),o("Annonce retirée","success"),s(),l("collection")})}function Fe(e,t,i,a){var h,w,_,I,E,C;const{state:d,toast:o,openModal:s,closeModal:l,navigate:r,refreshProfile:n}=a,c=e.player,p=t.filter(j=>j.player.id===c.id),f=p.length,u=bt[c.rarity]||1e3,m=c.rarity!=="legende",x=Qe(c),b=te(c,c.job),$=c.job2?te(c,c.job2):null,y=ce[c.job]||"#1A6B3C",g=c.job2?ce[c.job2]:null,v=Xe[c.rarity]||"#ccc",L=ke[c.country_code]||c.country_code||"";s(`${c.firstname} ${c.surname_encoded}`,`<div style="display:flex;gap:16px;flex-wrap:wrap;justify-content:center">
    ...
    </div>`:""}`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(_=document.getElementById("close-detail"))==null||_.addEventListener("click",l),(I=document.getElementById("direct-sell-btn"))==null||I.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte ${c.surname_encoded} pour ${u.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const j=p.find(A=>!A.is_for_sale)||p[0];if(!j){o("Aucune carte à vendre","error");return}const{error:G}=await k.from("cards").delete().eq("id",j.id);if(G){o(G.message,"error");return}await k.from("users").update({credits:(d.profile.credits||0)+u}).eq("id",d.profile.id),await n(),o(`+${u.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),l(),r("collection")}),(E=document.getElementById("market-sell-btn"))==null||E.addEventListener("click",async()=>{const j=parseInt(document.getElementById("sell-price").value);if(!j||j<1){o("Prix invalide","error");return}await k.from("cards").update({is_for_sale:!0,sale_price:j}).eq("id",e.id),await k.from("market_listings").insert({seller_id:d.profile.id,card_id:e.id,price:j}),o("Carte mise en vente sur le marché !","success"),l(),r("collection")}),(C=document.getElementById("cancel-sell-btn"))==null||C.addEventListener("click",async()=>{await k.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await k.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),o("Annonce retirée","success"),l(),r("collection")})}const pe={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},O={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function ge(e){const t=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function Ze(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function et(e){var i;const t="https://fcnwclxlkytdfjotqkta.supabase.co";return(i=e==null?void 0:e.clubs)!=null&&i.logo_url?e.clubs.logo_url.startsWith("http")?e.clubs.logo_url:`${t}/storage/v1/object/public/assets/clubs/${e.clubs.logo_url}`:null}function kt(e,t=44,i=58){var u;const a=e?ge(e):null,d=e?et(e):null,o=Ze(e==null?void 0:e.country_code),s=(e==null?void 0:e.job)||"MIL",l=O[s]||"#555",r={normal:"#aaa",pépite:"#D4A017",papyte:"#222",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",n=Number(s==="GK"?e==null?void 0:e.note_g:s==="DEF"?e==null?void 0:e.note_d:s==="MIL"?e==null?void 0:e.note_m:e==null?void 0:e.note_a)||0,c=Math.round(i*.19),p=Math.round(i*.25),f=i-c-p;return e?`<div style="width:${t}px;height:${i}px;border-radius:5px;border:2px solid ${r};background:${l};position:relative;overflow:hidden;flex-shrink:0">
  ...
  </div>`,document.getElementById("new-deck-btn").addEventListener("click",async()=>{const s=prompt("Nom du deck :",`Deck ${((o==null?void 0:o.length)||0)+1}`);if(!s)return;const{data:l,error:r}=await k.from("decks").insert({owner_id:i.profile.id,name:s}).select().single();if(r){d(r.message,"error");return}d("Deck créé !","success"),De(l.id,e,t)}),e.querySelectorAll("[data-open-deck]").forEach(s=>{s.addEventListener("click",()=>De(s.dataset.openDeck,e,t))}),e.querySelectorAll("[data-rename]").forEach(s=>{s.addEventListener("click",async()=>{const l=prompt("Nouveau nom :",s.dataset.name);if(!l||l===s.dataset.name)return;const{error:r}=await k.from("decks").update({name:l}).eq("id",s.dataset.rename);if(r){d(r.message,"error");return}d("Deck renommé !","success"),xe(e,t)})}),e.querySelectorAll("[data-delete]").forEach(s=>{s.addEventListener("click",async()=>{if(!confirm(`Supprimer le deck "${s.dataset.name}" ? Cette action est irréversible.`))return;await k.from("deck_cards").delete().eq("deck_id",s.dataset.delete);const{error:l}=await k.from("decks").delete().eq("id",s.dataset.delete);if(l){d(l.message,"error");return}d("Deck supprimé.","success"),xe(e,t)})})}async function De(e,t,i){const{state:a,toast:d}=i;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await k.from("decks").select("*").eq("id",e).single(),{data:s}=await k.from("cards").select(`id, card_type, formation,
      player:players(id, firstname, surname_encoded, country_code, club_id, job, job2,
        note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length,
        clubs(encoded_name, logo_url))`).eq("owner_id",a.profile.id),l=(s||[]).filter(u=>u.card_type==="player"&&u.player),r=(s||[]).filter(u=>u.card_type==="formation"),n=r.map(u=>u.formation).filter(Boolean),{data:c}=await k.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",e);let p=o.formation||"4-4-2";n.length>0&&!n.includes(p)&&(p=n[0]);const f={deckId:e,name:o.name,formation:p,formationCardId:o.formation_card_id,slots:{},subs:[],playerCards:l,formationCards:r,availableFormations:n};(c||[]).forEach(u=>{u.is_starter?f.slots[u.position]=u.card_id:f.subs.includes(u.card_id)||f.subs.push(u.card_id)}),V(t,f,i)}function V(e,t,i){var r;const{navigate:a}=i;pe[t.formation];const d=Ge(t.formation),o=d.filter(n=>t.slots[n]).length,s=t.availableFormations.length>0?t.availableFormations:Object.keys(pe),l=t.subs.map(n=>t.playerCards.find(c=>c.id===n)).filter(Boolean);[...Object.values(t.slots),...t.subs],e.innerHTML=`
  <div class="page">
  ...
  </div>`,Et(e,t,d,i),document.getElementById("builder-back").addEventListener("click",()=>a("decks")),document.getElementById("formation-select").addEventListener("change",n=>{t.formation=n.target.value;const c=Ge(t.formation),p={};c.forEach(f=>{t.slots[f]&&(p[f]=t.slots[f])}),t.slots=p,V(e,t,i)}),document.getElementById("save-deck").addEventListener("click",()=>Tt(t,i)),e.querySelectorAll("[data-remove-sub]").forEach(n=>{n.addEventListener("click",()=>{t.subs=t.subs.filter(c=>c!==n.dataset.removeSub),V(e,t,i)})}),(r=document.getElementById("add-sub-btn"))==null||r.addEventListener("click",()=>{At(t,e,i)})}function Et(e,t,i,a){const d=e.querySelector("#deck-field");if(!d)return;const o=ue[t.formation]||{},s=He(t.formation)||[],l={};for(const $ of i){const y=t.slots[$],g=y?t.playerCards.find(v=>v.id===y):null;l[$]=g?g.player:null}const r=300,n=300,c=48,p=64,f=13,u=16,m=38;function x($){const y=o[$];return y?{x:y.x*r,y:y.y*n}:null}let b="";for(const[$,y]of s){const g=x($),v=x(y);if(!g||!v)continue;const L=l[$]?{...l[$],club_id:l[$].club_id,country_code:l[$].country_code,rarity:l[$].rarity}:null,h=l[y]?{...l[y],club_id:l[y].club_id,country_code:l[y].country_code,rarity:l[y].rarity}:null,w=U(L,h);w==="#ff3333"||w==="#cc2222"?b+=`<line x1="${g.x.toFixed(1)}" y1="${g.y.toFixed(1)}" x2="${v.x.toFixed(1)}" y2="${v.y.toFixed(1)}" stroke="${w}" stroke-width="2.5" stroke-linecap="round" opacity="0.4"/>`:(b+=`<line x1="${g.x.toFixed(1)}" y1="${g.y.toFixed(1)}" x2="${v.x.toFixed(1)}" y2="${v.y.toFixed(1)}" stroke="${w}" stroke-width="8" stroke-linecap="round" opacity="0.2"/>`,b+=`<line x1="${g.x.toFixed(1)}" y1="${g.y.toFixed(1)}" x2="${v.x.toFixed(1)}" y2="${v.y.toFixed(1)}" stroke="${w}" stroke-width="2.5" stroke-linecap="round" opacity="0.9"/>`)}for(const $ of i){const y=x($);if(!y)continue;const g=l[$],v=$.replace(/\d+/,""),L=O[v]||"#555",h=(y.x-c/2).toFixed(1),w=(y.y-p/2).toFixed(1),_={normal:"#aaa",pépite:"#D4A017",papyte:"#222",légende:"#7a28b8"}[g==null?void 0:g.rarity]||"#aaa";if(g){const I=ge(g),E=et(g),C=Ze(g.country_code),j=Number(v==="GK"?g.note_g:v==="DEF"?g.note_d:v==="MIL"?g.note_m:g.note_a)||0,G=p-f-u;b+=`<defs><clipPath id="dcp-${$}"><rect x="${h}" y="${(y.y-p/2+f).toFixed(1)}" width="${c}" height="${G}"/></clipPath></defs>`,b+=`<rect x="${h}" y="${w}" width="${c}" height="${p}" rx="5" fill="${L}"/>`,I&&(b+=`<image href="${I}" x="${h}" y="${(y.y-p/2+f).toFixed(1)}" width="${c}" height="${G}" clip-path="url(#dcp-${$})" preserveAspectRatio="xMidYMin slice"/>`),b+=`<rect x="${h}" y="${w}" width="${c}" height="${f}" fill="rgba(255,255,255,0.93)"/>`,b+=`<text x="${y.x.toFixed(1)}" y="${(y.y-p/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(g.surname_encoded||"").slice(0,9)}</text>`;const A=(y.y+p/2-u).toFixed(1);b+=`<rect x="${h}" y="${A}" width="${c}" height="${u}" fill="rgba(255,255,255,0.93)"/>`,C&&(b+=`<image href="${C}" x="${(y.x-21).toFixed(1)}" y="${(y.y+p/2-u+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`),b+=`<text x="${y.x.toFixed(1)}" y="${(y.y+p/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${j}</text>`,E&&(b+=`<image href="${E}" x="${(y.x+c/2-14).toFixed(1)}" y="${(y.y+p/2-u+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`),b+=`<rect x="${h}" y="${w}" width="${c}" height="${p}" rx="5" fill="none" stroke="${_}" stroke-width="2"/>`}else b+=`<rect x="${h}" y="${w}" width="${c}" height="${p}" rx="5" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="5,3"/>`,b+=`<text x="${y.x.toFixed(1)}" y="${y.y.toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="22" fill="rgba(255,255,255,0.35)">+</text>`,b+=`<text x="${y.x.toFixed(1)}" y="${(y.y+16).toFixed(1)}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.3)">${v}</text>`;b+=`<rect x="${h}" y="${w}" width="${c}" height="${p}" rx="5" fill="rgba(0,0,0,0.01)" class="deck-slot-hit" data-pos="${$}" style="cursor:pointer"/>`}d.innerHTML=`<svg viewBox="${-m} ${-m} ${r+m*2} ${n+m*2}" width="100%" style="display:block;max-width:440px;margin:0 auto">${b}</svg>`,d.querySelectorAll(".deck-slot-hit").forEach($=>{$.addEventListener("click",()=>It($.dataset.pos,t,e,a))})}function It(e,t,i,a){var f,u,m;const{openModal:d,closeModal:o}=a,s=e.replace(/\d+/,""),l=t.slots[e],r=l?t.playerCards.find(x=>x.id===l):null;(f=r==null?void 0:r.player)==null||f.id;const n=new Set;Object.entries(t.slots).forEach(([x,b])=>{var y;if(x===e||!b)return;const $=t.playerCards.find(g=>g.id===b);(y=$==null?void 0:$.player)!=null&&y.id&&n.add($.player.id)}),t.subs.forEach(x=>{var $;const b=t.playerCards.find(y=>y.id===x);($=b==null?void 0:b.player)!=null&&$.id&&n.add(b.player.id)});const c=new Set,p=t.playerCards.filter(x=>{const b=x.player;return!(b.job===s||b.job2===s)||n.has(b.id)||c.has(b.id)?!1:(c.add(b.id),!0)});p.sort((x,b)=>{const $=s==="GK"?x.player.note_g:s==="DEF"?x.player.note_d:s==="MIL"?x.player.note_m:x.player.note_a;return(s==="GK"?b.player.note_g:s==="DEF"?b.player.note_d:s==="MIL"?b.player.note_m:b.player.note_a)-$}),d(`Choisir ${s} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${p.length>0?p.map(x=>{var v,L;const b=x.player,$=s==="GK"?b.note_g:s==="DEF"?b.note_d:s==="MIL"?b.note_m:b.note_a,y=ge(b),g={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[b.rarity];return`<div class="player-option" data-card-id="${x.id}"
          style="display:flex;align-items:center;gap:10px;padding:8px;border:1.5px solid var(--gray-200);border-radius:10px;cursor:pointer">
          <!-- Portrait -->
          <div style="width:44px;height:44px;border-radius:8px;overflow:hidden;flex-shrink:0;background:#dde;border:2px solid ${O[s]}">
            ${y?`<img src="${y}" style="width:100%;height:100%;object-fit:cover">`:`<div style="width:100%;height:100%;background:${O[s]};display:flex;align-items:center;justify-content:center;color:#fff;font-size:10px;font-weight:700">${s}</div>`}
          </div>
          <!-- Infos -->
          ...
          <!-- Note -->
          <div style="width:36px;height:36px;border-radius:8px;background:${O[s]};color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:16px;border:2px solid ${g};flex-shrink:0">
            ${$}
          </div>
        </div>`}).join(""):'<div style="text-align:center;color:var(--gray-600);padding:20px">Aucun joueur pour ce poste.<br><small>Ouvre des boosters !</small></div>'}
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(u=document.getElementById("close-selector"))==null||u.addEventListener("click",o),(m=document.getElementById("remove-player"))==null||m.addEventListener("click",()=>{delete t.slots[e],o(),V(i,t,a)}),document.querySelectorAll(".player-option").forEach(x=>{x.addEventListener("click",()=>{t.slots[e]=x.dataset.cardId,o(),V(i,t,a)})})}function At(e,t,i){var r;const{openModal:a,closeModal:d}=i,o=new Set;Object.values(e.slots).filter(Boolean).forEach(n=>{var p;const c=e.playerCards.find(f=>f.id===n);(p=c==null?void 0:c.player)!=null&&p.id&&o.add(c.player.id)}),e.subs.forEach(n=>{var p;const c=e.playerCards.find(f=>f.id===n);(p=c==null?void 0:c.player)!=null&&p.id&&o.add(c.player.id)});const s=new Set,l=e.playerCards.filter(n=>{var c,p,f;return o.has((c=n.player)==null?void 0:c.id)||s.has((p=n.player)==null?void 0:p.id)?!1:(s.add((f=n.player)==null?void 0:f.id),!0)});a("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${l.length>0?l.map(n=>{var u;const c=n.player,p=ge(c),f=c.job==="GK"?c.note_g:c.job==="DEF"?c.note_d:c.job==="MIL"?c.note_m:c.note_a;return`<div class="player-option" data-card-id="${n.id}"
          style="display:flex;align-items:center;gap:10px;padding:8px;border:1.5px solid var(--gray-200);border-radius:10px;cursor:pointer">
          <div style="width:40px;height:40px;border-radius:8px;overflow:hidden;flex-shrink:0;background:#dde;border:2px solid ${O[c.job]}">
          ...
          <div style="width:32px;height:32px;border-radius:6px;background:${O[c.job]};color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900">
            ${f}
          </div>
        </div>`}).join(""):'<div style="text-align:center;padding:20px;color:var(--gray-600)">Tous vos joueurs sont déjà utilisés.</div>'}
    </div>`,'<button class="btn btn-ghost" id="close-sub-selector">Fermer</button>'),(r=document.getElementById("close-sub-selector"))==null||r.addEventListener("click",d),document.querySelectorAll(".player-option").forEach(n=>{n.addEventListener("click",()=>{e.subs.push(n.dataset.cardId),d(),V(t,e,i)})})}async function Tt(e,t){const{state:i,toast:a,navigate:d}=t,o=e.formationCards.find(r=>r.formation===e.formation),s=(o==null?void 0:o.id)||e.formationCardId;await k.from("decks").update({formation:e.formation,formation_card_id:s||null}).eq("id",e.deckId),await k.from("deck_cards").delete().eq("deck_id",e.deckId);const l=[];if(Object.entries(e.slots).forEach(([r,n],c)=>{l.push({deck_id:e.deckId,card_id:n,position:r,is_starter:!0,slot_order:c})}),e.subs.forEach((r,n)=>{l.push({deck_id:e.deckId,card_id:r,position:`SUB${n+1}`,is_starter:!1,slot_order:100+n})}),l.length>0){const{error:r}=await k.from("deck_cards").insert(l);if(r){a(r.message,"error");return}}a("Deck enregistré ✅","success"),d("decks")}function Ge(e){const t=pe[e]||pe["4-4-2"],i=["GK1"];for(let a=1;a<=t.DEF;a++)i.push(`DEF${a}`);for(let a=1;a<=t.MIL;a++)i.push(`MIL${a}`);for(let a=1;a<=t.ATT;a++)i.push(`ATT${a}`);return i}const Lt="modulepreload",zt=function(e){return"/manager-wars/"+e},Ne={},Mt=function(t,i,a){let d=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const s=document.querySelector("meta[property=csp-nonce]"),l=(s==null?void 0:s.nonce)||(s==null?void 0:s.getAttribute("nonce"));d=Promise.allSettled(i.map(r=>{if(r=zt(r),r in Ne)return;Ne[r]=!0;const n=r.endsWith(".css"),c=n?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${r}"]${c}`))return;const p=document.createElement("link");if(p.rel=n?"stylesheet":Lt,n||(p.as="script"),p.crossOrigin="",p.href=r,l&&p.setAttribute("nonce",l),document.head.appendChild(p),n)return new Promise((f,u)=>{p.addEventListener("load",f),p.addEventListener("error",()=>u(new Error(`Unable to preload CSS for ${r}`)))})}))}function o(s){const l=new Event("vite:preloadError",{cancelable:!0});if(l.payload=s,window.dispatchEvent(l),!l.defaultPrevented)throw s}return d.then(s=>{for(const l of s||[])l.status==="rejected"&&o(l.reason);return t().catch(o)})};async function tt(){const{data:e}=await k.from("booster_configs").select("*").eq("is_active",!0).order("sort_order");if(!(e!=null&&e.length))return[];const{data:t}=await k.from("booster_drop_rates").select("*").in("booster_id",e.map(i=>i.id)).order("sort_order");return e.map(i=>({...i,rates:(t||[]).filter(a=>a.booster_id===i.id)}))}function Ct(e){if(!(e!=null&&e.length))return null;const t=e.reduce((a,d)=>a+Number(d.percentage),0);let i=Math.random()*t;for(const a of e)if(i-=Number(a.percentage),i<=0)return a;return e[e.length-1]}const jt=Object.freeze(Object.defineProperty({__proto__:null,loadActiveBoosters:tt,rollDropRate:Ct},Symbol.toStringTag,{value:"Module"})),it=()=>Object.keys(ue),Bt=[{id:"players_std",img:"/manager-wars/icons/booster-players.png",name:"Players",sub:"5 cartes joueurs",cost:5e3,costLabel:"5 000 crédits",cardCount:5,type:"player"},{id:"players_pub",img:"/manager-wars/icons/booster-silver.png",name:"Players (pub)",sub:"3 cartes joueurs",cost:0,costLabel:"1 pub",cardCount:3,type:"player"},{id:"game_changer",img:"/manager-wars/icons/booster-gamechanger.png",name:"Game Changer",sub:"3 cartes spéciales",cost:1e4,costLabel:"10 000 crédits",cardCount:3,type:"game_changer"},{id:"formation",img:"/manager-wars/icons/booster-formation.png",name:"Formation",sub:"1 carte formation",cost:1e4,costLabel:"10 000 crédits",cardCount:1,type:"formation"}],ye={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé."},"Double attaque":{icon:"⚡",desc:"La prochaine attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la prochaine action IA."},Gel:{icon:"❄️",desc:"Bloque le meilleur attaquant IA."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function St(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";if(!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}const Ft={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},Dt={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"};function Gt(e){var d;const t={};(e.rates||[]).forEach(o=>{t[o.card_type]=(t[o.card_type]||0)+Number(o.percentage||0)});const i=((d=Object.entries(t).sort((o,s)=>s[1]-o[1])[0])==null?void 0:d[0])||"player",a=e.image_url||"booster-players.png";return{id:e.id,img:"/manager-wars/icons/"+a,name:e.name,sub:`${e.card_count} carte(s)`,cost:e.price_type==="credits"&&e.price_credits||0,costLabel:e.price_type==="credits"?`${(e.price_credits||0).toLocaleString("fr")} crédits`:e.price_type==="pub"?"1 pub":"Gratuit",cardCount:e.card_count||5,type:i,isPub:e.price_type==="pub",rates:e.rates||[],_raw:e}}async function Nt(e,{state:t,navigate:i,toast:a}){var s;const d=((s=t.profile)==null?void 0:s.credits)||0;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⏳ Chargement...</div>';let o=[];try{o=(await tt()).map(Gt)}catch(l){console.warn("Erreur chargement boosters DB, fallback hardcodé",l)}o.length||(o=Bt.map(l=>({...l,rates:[],isPub:l.id==="players_pub"}))),e.innerHTML=`
  <div class="page">
    <div class="page-header">
      <h2>📦 Boosters</h2>
  ...
  </div>`,e.querySelectorAll(".booster-card:not(.disabled)").forEach(l=>{l.addEventListener("click",async()=>{const r=o.find(n=>n.id===l.dataset.booster);if(r){l.style.opacity="0.5",l.style.pointerEvents="none";try{await Rt(r,{state:t,toast:a,navigate:i,container:e})}catch(n){a(n.message,"error"),l.style.opacity="",l.style.pointerEvents=""}}})}),e.querySelectorAll(".booster-info-btn").forEach(l=>{l.addEventListener("click",r=>{r.stopPropagation();const n=o.find(c=>c.id===l.dataset.boosterId);Yt(n)})})}async function Rt(e,{state:t,toast:i,navigate:a,container:d}){if(e.cost>0&&t.profile.credits<e.cost){i("Crédits insuffisants","error");return}if(e.isPub)try{await Xt()}catch(p){i(p.message||"Regardez la pub entièrement pour ouvrir le booster","error");return}const{data:o}=await k.from("cards").select("card_type, player_id, formation").eq("owner_id",t.profile.id),s=new Set((o||[]).filter(p=>p.card_type==="player").map(p=>p.player_id)),l=new Set((o||[]).filter(p=>p.card_type==="formation").map(p=>p.formation));let r=[];const n=e.type||"player";n==="player"?r=await Kt(t.profile,e.cardCount,e.cost):n==="game_changer"?r=await Ht(t.profile,e.cardCount,e.cost):n==="formation"?r=await Ut(t.profile,e.cost):r=await Pt(t.profile,e),r.forEach(p=>{p.card_type==="player"&&p.player?p.isDuplicate=s.has(p.player.id):p.card_type==="formation"&&(p.isDuplicate=l.has(p.formation))});const{data:c}=await k.from("users").select("*").eq("id",t.profile.id).single();c&&(t.profile=c),Vt(r,e,a)}function qt(){const e=Math.random()*100;return e<.5?"legende":e<2?"special":e<10?"normal_high":"normal_low"}function P(e){return Math.max(Number(e.note_g)||0,Number(e.note_d)||0,Number(e.note_m)||0,Number(e.note_a)||0)}function Ot(e,t){let i;switch(t){case"legende":i=e.filter(a=>a.rarity==="legende"),i.length||(i=e.filter(a=>a.rarity==="pepite"||a.rarity==="papyte")),i.length||(i=e.filter(a=>P(a)>=6));break;case"special":i=e.filter(a=>a.rarity==="pepite"||a.rarity==="papyte"),i.length||(i=e.filter(a=>P(a)>=6));break;case"normal_high":i=e.filter(a=>a.rarity==="normal"&&P(a)>=6),i.length||(i=e.filter(a=>P(a)>=6));break;default:i=e.filter(a=>a.rarity==="normal"&&P(a)>=1&&P(a)<=5),i.length||(i=e.filter(a=>a.rarity==="normal"));break}return i.length||(i=e),i[Math.floor(Math.random()*i.length)]}async function Pt(e,t){if(t.cost>0){const{error:d}=await k.from("users").update({credits:e.credits-t.cost}).eq("id",e.id);if(d)throw d}const{rollDropRate:i}=await Mt(async()=>{const{rollDropRate:d}=await Promise.resolve().then(()=>jt);return{rollDropRate:d}},void 0),a=[];for(let d=0;d<(t.cardCount||5);d++){const o=i(t.rates);if(o){if(o.card_type==="player"){let s=k.from("players").select("id,job,firstname,surname_encoded,country_code,club_id,rarity,note_g,note_d,note_m,note_a,skin,hair,hair_length,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);o.rarity&&(s=s.eq("rarity",o.rarity));const{data:l}=await s;let r=l||[];if((o.note_min||o.note_max)&&(r=r.filter(p=>{const f=Math.max(Number(p.note_g)||0,Number(p.note_d)||0,Number(p.note_m)||0,Number(p.note_a)||0);return(!o.note_min||f>=o.note_min)&&(!o.note_max||f<=o.note_max)})),r.length||(r=l||[]),!r.length)continue;const n=r[Math.floor(Math.random()*r.length)],{data:c}=await k.from("cards").insert({owner_id:e.id,player_id:n.id,card_type:"player"}).select().single();c&&a.push({...c,player:n})}else if(o.card_type==="game_changer"){const{data:s}=await k.from("gc_definitions").select("id,name").eq("is_active",!0).eq("gc_type","game_changer"),l=s!=null&&s.length?s:[{name:"Ressusciter"},{name:"Double attaque"},{name:"Bouclier"},{name:"Vol de note"},{name:"Gel"}],n=l[Math.floor(Math.random()*l.length)].name,{data:c}=await k.from("cards").insert({owner_id:e.id,card_type:"game_changer",gc_type:n}).select().single();c&&a.push(c)}else if(o.card_type==="formation"){const s=it(),l=s[Math.floor(Math.random()*s.length)],{data:r}=await k.from("cards").insert({owner_id:e.id,card_type:"formation",formation:l}).select();r!=null&&r[0]&&a.push(r[0])}}}return a}async function Kt(e,t,i){if(i>0){const{error:n}=await k.from("users").update({credits:e.credits-i}).eq("id",e.id);if(n)throw n}const{data:a}=await k.from("players").select("id,job,firstname,surname_encoded,country_code,club_id,rarity,note_g,note_d,note_m,note_a,note_min,note_max,skin,hair,hair_length,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);if(!(a!=null&&a.length))throw new Error("Pas de joueurs en BDD — ajoutes-en via le panel admin !");const d=a.filter(n=>n.job==="GK"),o=a.filter(n=>n.job!=="GK"),s=!e.first_booster_opened&&d.length>0,l=[];for(let n=0;n<t;n++){const c=n===0&&s?d:n===0?o:a,p=qt(),f=Ot(c,p);f&&l.push(f)}s&&await k.from("users").update({first_booster_opened:!0}).eq("id",e.id);const{data:r}=await k.from("cards").insert(l.map(n=>({owner_id:e.id,player_id:n.id,card_type:"player"}))).select();return l.map((n,c)=>({...r[c],player:n}))}async function Ht(e,t,i){const{error:a}=await k.from("users").update({credits:e.credits-i}).eq("id",e.id);if(a)throw a;const{data:d}=await k.from("gc_definitions").select("id,name,gc_type").eq("is_active",!0),o=d!=null&&d.length?d:Object.keys(ye).map(c=>({name:c,gc_type:"game_changer"})),s=Array.from({length:t},()=>{const c=o[Math.floor(Math.random()*o.length)];return{owner_id:e.id,card_type:"game_changer",gc_type:c.name,gc_definition_id:c.id||null}}),{data:l,error:r}=await k.from("cards").insert(s).select();return r&&console.error("[Booster GC] Erreur insert:",r.message,r),(l||[]).map(c=>{const p=d==null?void 0:d.find(f=>f.name===c.gc_type||f.id===c.gc_definition_id);return{...c,_gcDef:p||null}})}async function Ut(e,t){const{error:i}=await k.from("users").update({credits:e.credits-t}).eq("id",e.id);if(i)throw i;const a=it(),d=a[Math.floor(Math.random()*a.length)],{data:o,error:s}=await k.from("cards").insert({owner_id:e.id,card_type:"formation",formation:d}).select();return s&&console.error("[Booster Formation] Erreur insert:",s.message,s),o||[]}function Vt(e,t,i){var f;if(!e||e.length===0){const u=document.createElement("div");u.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:3000;gap:16px;color:#fff;padding:24px;text-align:center",u.innerHTML=`
  ...
  `,document.body.appendChild(a);let d=!1;document.getElementById("pack-visual").addEventListener("click",()=>{if(d)return;d=!0;const u=document.getElementById("pack-visual");u.classList.add("shaking"),setTimeout(()=>{u.classList.add("pack-open"),setTimeout(()=>{document.getElementById("pack-phase").style.display="none",s(0)},600)},500)});let o=0;function s(u){o=u;const m=document.getElementById("reveal-phase");m.style.display="flex",l(u)}function l(u){var h;const m=e[u],x=document.getElementById("card-counter"),b=document.getElementById("single-card-slot"),$=document.getElementById("card-tap-hint");x&&(x.textContent=`Carte ${u+1} / ${e.length}`),$&&($.textContent=u<e.length-1?"Appuie pour continuer →":"Appuie pour voir toutes tes cartes");const y=Re(m),g=m.card_type==="player"&&((h=m.player)==null?void 0:h.rarity)==="legende";b.innerHTML=`
      ...
      <div style="padding:6px 8px;background:rgba(0,0,0,0.3);text-align:center">
        <div style="font-size:9px;color:rgba(255,255,255,0.85);line-height:1.3">${((o==null?void 0:o.effect)||((d=ye[e.gc_type])==null?void 0:d.desc)||"").slice(0,55)}</div>
      </div>
      ...
      </div>`,document.body.appendChild(a),a.addEventListener("click",s=>{s.target===a&&a.remove()}),(i=document.getElementById("odds-close"))==null||i.addEventListener("click",()=>a.remove());return}Wt()}function Wt(){const e=document.createElement("div");e.style.cssText=`position:fixed;inset:0;background:rgba(0,0,0,0.6);display:flex;
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}const Jt="simulation",ot="VOTRE_ZONE_ID";function Xt(){switch(Jt){case"propellerads":return Qt();case"adsense":return Zt();default:return Ee()}}function Ee(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:#111;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:14px";let i=5;t.innerHTML=`
      ...
      <button id="ad-skip" disabled style="margin-top:8px;padding:10px 24px;border-radius:20px;border:none;background:rgba(255,255,255,0.15);color:rgba(255,255,255,0.4);font-size:13px;cursor:default">
        Passer (5)
      </button>`,document.body.appendChild(t);const a=setInterval(()=>{i--;const d=document.getElementById("ad-cd"),o=document.getElementById("ad-skip");d&&(d.textContent=i),o&&(o.textContent=i>0?`Passer (${i})`:"✓ Continuer"),i<=0&&(clearInterval(a),o&&(o.disabled=!1,o.style.cssText=o.style.cssText.replace("rgba(255,255,255,0.15)","#1A6B3C").replace("rgba(255,255,255,0.4)","#fff").replace("default","pointer")),o==null||o.addEventListener("click",()=>{t.remove(),e(!0)},{once:!0}))},1e3)})}function Qt(){return new Promise((e,t)=>{if(typeof window.propeller>"u")return console.warn("[Ad] PropellerAds non chargé → simulation"),e(Ee());window.propeller.push({zone_id:ot,onComplete:()=>e(!0),onSkip:()=>t(new Error("Publicité ignorée")),onError:()=>t(new Error("Erreur publicité"))})})}function Zt(){return new Promise((e,t)=>{if(typeof window.adsbygoogle>"u")return console.warn("[Ad] AdSense non chargé → simulation"),e(Ee());try{(window.adsbygoogle=window.adsbygoogle||[]).push({google_ad_client:"ca-pub-VOTRE_PUBLISHER_ID",google_ad_slot:ot,google_ad_format:"rewarded",on_reward:()=>e(!0),on_dismiss:()=>t(new Error("Publicité fermée"))})}catch(i){t(i)}})}const Y={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},W={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function nt(e,t,i,a,d){var o;e.innerHTML=`<div class="match-screen" style="display:flex;align-items:center;justify-content:center;min-height:100vh">
    <div style="text-align:center;padding:40px;color:#fff">
      <div style="font-size:48px;margin-bottom:16px">${t}</div>
      <p style="margin-bottom:16px">${i}</p>
      <button class="btn btn-primary" id="msg-btn">${a}</button>
    </div>
  </div>`,(o=document.getElementById("msg-btn"))==null||o.addEventListener("click",d)}function N(e){const t=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function he(e){var i,a;const t=e.player;return{cardId:e.id,id:t.id,firstname:t.firstname,name:t.surname_encoded,country_code:t.country_code,club_id:t.club_id,job:t.job,job2:t.job2,note_g:Number(t.note_g)||0,note_d:Number(t.note_d)||0,note_m:Number(t.note_m)||0,note_a:Number(t.note_a)||0,rarity:t.rarity,skin:t.skin,hair:t.hair,hair_length:t.hair_length,clubName:((i=t.clubs)==null?void 0:i.encoded_name)||null,clubLogo:((a=t.clubs)==null?void 0:a.logo_url)||null,boost:0,used:!1,_line:null,_col:null}}function Ie(e){return e===1?[1]:e===2?[0,2]:e===3?[0,1,2]:e===4?[0,1,1,2]:e===5?[0,1,1,1,2]:[1]}function ei(){const e=Math.random()*100;return e<.1?4:e<5?3:e<20?2:1}function rt(e,t){const i=Y[t]||Y["4-4-2"],a={GK:[],DEF:[],MIL:[],ATT:[]},d=[...e];for(const o of["GK","DEF","MIL","ATT"]){const s=[];for(let r=0;r<i[o];r++){let n=d.findIndex(c=>c.job===o);if(n===-1&&(n=d.findIndex(c=>c.job2===o)),n===-1&&(n=0),d[n]){const c={...d[n],_line:o};s.push(c),d.splice(n,1)}}const l=Ie(s.length);s.forEach((r,n)=>{r._col=l[n]}),a[o]=s}return a}async function ti(e,t){var s,l;const{data:i}=await k.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(60);if(!i||i.length<11)return ii(e);const a=Y[e]||Y["4-4-2"],d={GK:[],DEF:[],MIL:[],ATT:[]},o=[...i];for(const r of["GK","DEF","MIL","ATT"]){const n=o.filter(m=>m.job===r),c=o.filter(m=>m.job!==r),p=[...n,...c],f=[];for(let m=0;m<a[r];m++){const x=p[m]||o[m];x&&f.push({cardId:"ai-"+x.id+"-"+m,id:x.id,firstname:x.firstname,name:x.surname_encoded,country_code:x.country_code,club_id:x.club_id,job:x.job,job2:x.job2,note_g:Number(x.note_g)||0,note_d:Number(x.note_d)||0,note_m:Number(x.note_m)||0,note_a:Number(x.note_a)||0,rarity:x.rarity,skin:x.skin,hair:x.hair,hair_length:x.hair_length,clubName:((s=x.clubs)==null?void 0:s.encoded_name)||null,clubLogo:((l=x.clubs)==null?void 0:l.logo_url)||null,boost:0,used:!1,_line:r})}const u=Ie(f.length);f.forEach((m,x)=>{m._col=u[x]}),d[r]=f}return d}function ii(e){const t=Y[e]||Y["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},a=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let d=0;for(const o of["GK","DEF","MIL","ATT"]){const s=[];for(let r=0;r<t[o];r++){const n=3+Math.floor(Math.random()*5);s.push({cardId:"fake-"+d,id:"fake-"+d,firstname:"IA",name:a[d%a.length],country_code:"XX",club_id:null,job:o,job2:null,note_g:o==="GK"?n:2,note_d:o==="DEF"?n:2,note_m:o==="MIL"?n:2,note_a:o==="ATT"?n:2,rarity:"normal",boost:0,used:!1,_line:o}),d++}const l=Ie(s.length);s.forEach((r,n)=>{r._col=l[n]}),i[o]=s}return i}async function oi(e,t){var h;const{state:i,navigate:a,toast:d}=t,o=i.params||{};e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const s=o.matchMode||"vs_ai_easy",l=s.replace("vs_ai_",""),r=s;if(!o.deckId)return ri(e,t,s);const n=o.deckId,[{data:c},{data:p}]=await Promise.all([k.from("decks").select("formation,name").eq("id",n).single(),k.from("deck_cards").select(`position, is_starter, slot_order,
        card:cards(id, card_type, formation,
          player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
            note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
            clubs(encoded_name,logo_url)))`).eq("deck_id",n).order("slot_order")]),f=(p||[]).filter(w=>{var _;return w.is_starter&&((_=w.card)==null?void 0:_.player)}).map(w=>he(w.card)),u=(p||[]).filter(w=>{var _;return!w.is_starter&&((_=w.card)==null?void 0:_.player)}).map(w=>he(w.card));if(f.length<11){nt(e,"⚠️",`Deck incomplet (${f.length}/11).`,"Compléter",()=>a("decks"));return}const m=(p||[]).find(w=>{var _;return((_=w.card)==null?void 0:_.card_type)==="formation"}),x=(c==null?void 0:c.formation)||((h=m==null?void 0:m.card)==null?void 0:h.formation)||"4-4-2",{data:b}=await k.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",i.profile.id).eq("card_type","game_changer"),{data:$}=await k.from("gc_definitions").select("*").eq("is_active",!0),y=(b||[]).map(w=>({...w,_gcDef:($==null?void 0:$.find(_=>_.name===w.gc_type||_.id===w.gc_definition_id))||null})),g=rt(f,x),v=await ti(x),L=async w=>{const{data:_}=await k.from("matches").insert({home_id:i.profile.id,away_id:null,mode:r,home_deck_id:n,status:"in_progress"}).select().single(),I={gcDefs:$||[],matchId:_==null?void 0:_.id,mode:r,difficulty:l,formation:x,homeTeam:g,aiTeam:v,homeSubs:u,subsUsed:0,maxSubs:Math.min(u.length,3),homeScore:0,aiScore:0,gcCards:w,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};ai(e,I,t)};if(!y.length){L([]);return}ni(e,y,L)}function ni(e,t,i){let d=[];function o(l,r){const n=l._gcDef,c={purple:"linear-gradient(135deg,#3d0a7a,#7a28b8)",light_blue:"linear-gradient(135deg,#006080,#00bcd4)"},p={purple:"#9b59b6",light_blue:"#00bcd4"},f=c[n==null?void 0:n.color]||c.purple,u=p[n==null?void 0:n.color]||p.purple;return`<div class="gc-select-card" data-id="${l.id}"
    ...
    </div>`,e.querySelectorAll(".gc-select-card").forEach(n=>{n.addEventListener("click",()=>{const c=n.dataset.id,p=d.findIndex(f=>f.id===c);if(p>-1)d.splice(p,1);else{if(d.length>=3)return;const f=t.find(u=>u.id===c);f&&d.push(f)}s()})}),(l=e.querySelector("#gc-sel-confirm"))==null||l.addEventListener("click",()=>{d.length&&i(d)}),(r=e.querySelector("#gc-sel-skip"))==null||r.addEventListener("click",()=>{i([])})}s()}async function ri(e,t,i){const{state:a,navigate:d}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await k.from("decks").select("id,name,is_active,formation").eq("owner_id",a.profile.id).order("created_at",{ascending:!1});if(!o||o.length===0){nt(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>d("decks"));return}const s=o.map(c=>c.id),{data:l}=await k.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",s).order("slot_order");let r=0;function n(){var y,g,v,L,h;const c=o[r],p=(l||[]).filter(w=>w.deck_id===c.id),f=p.filter(w=>{var _;return w.is_starter&&((_=w.card)==null?void 0:_.player)}).map(w=>he(w.card)),u=p.find(w=>{var _;return((_=w.card)==null?void 0:_.card_type)==="formation"}),m=c.formation||((y=u==null?void 0:u.card)==null?void 0:y.formation)||"4-4-2",x=f.length>=11?rt(f,m):null,b=f.length>=11;e.innerHTML=`
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;min-height:100%;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
  </div>`,setTimeout(()=>si(e,t,i),5e3)}function si(e,t,i){const a=t.homeTeam.MIL||[],d=t.aiTeam.MIL||[];function o(p){return p.reduce((f,u)=>f+F(u,"MIL"),0)}function s(p){let f=0;for(let u=0;u<p.length-1;u++){const m=U(p[u],p[u+1]);m==="#00ff88"?f+=2:m==="#FFD700"&&(f+=1)}return f}const l=o(a)+s(a),r=o(d)+s(d),n=l>=r;function c(p,f,u){return`<div style="text-align:center">
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${p.map((m,x)=>{const b=N(m),$=x<p.length-1?U(m,p[x+1]):null,y=$&&$!=="#ff3333"&&$!=="#cc2222";return`
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;min-height:100%;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div id="midfield-result" style="opacity:0;text-align:center;transition:opacity 0.5s;color:#fff;max-width:320px"></div>
  </div>`,setTimeout(()=>{const p=document.getElementById("score-home"),f=document.getElementById("score-ai"),u=document.getElementById("midfield-result");if(p&&f&&(n?(p.style.fontSize="80px",p.style.color="#FFD700",f.style.opacity="0.25"):(f.style.fontSize="80px",f.style.color="#ff6b6b",p.style.opacity="0.25")),u){const m=ei();u.style.opacity="1",n&&t.clubName,u.innerHTML=`
      ...
      `,n&&(t.boostCard={value:m})}t.attacker=n?"home":"ai",t.log.push({type:"duel",title:"Milieu de Terrain",homePlayers:a.map(m=>({name:m.name,note:F(m,"MIL"),portrait:N(m),job:m.job,country_code:m.country_code,rarity:m.rarity,clubName:m.clubName,clubLogo:m.clubLogo})),aiPlayers:d.map(m=>({name:m.name,note:F(m,"MIL"),portrait:N(m),job:m.job,country_code:m.country_code,rarity:m.rarity,clubName:m.clubName,clubLogo:m.clubLogo})),homeTotal:l,aiTotal:r,text:`Duel milieu : ${t.clubName} ${l} – ${r} IA → ${n?t.clubName+" attaque":"IA attaque"}`}),setTimeout(()=>{var x,b;const m=(x=t.boostCard)==null?void 0:x.value;e.innerHTML=`
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;height:${p}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ...
      ${d?`<img src="${d}" style="width:${p-4}px;height:${p-4}px;object-fit:contain">`:e!=null&&e.clubName?`<span style="font-size:${Math.max(4,p-8)}px;font-weight:700;color:#333">${(e.clubName||"").slice(0,3).toUpperCase()}</span>`:""}
  ...
  </div>`}function re(e,t,i){if(!(e!=null&&e.length))return"";const a=e.slice(0,5);let d='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return a.forEach((o,s)=>{if(d+=K(o,40,52),s<a.length-1){const l=U(o,a[s+1]);d+=`<div style="width:7px;height:3px;background:${l==="#ff3333"||l==="#cc2222"?"rgba(255,255,255,0.12)":l};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(d+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),d+="</div>",d}function be(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function st(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function Ae(e,t,i,a,d=310,o=310){const s=ue[t]||{},l=He(t)||_e[t]||[],r={},n=["ATT","MIL","DEF","GK"];for(const y of n)(e[y]||[]).forEach((v,L)=>{r[`${y}${L+1}`]=v});function c(y){const g=s[y];return g?{x:g.x*d,y:g.y*o}:null}let p="";for(const[y,g]of l){const v=c(y),L=c(g);if(!v||!L)continue;const h=r[y],w=r[g],_=U(h,w);_==="#00ff88"||_==="#FFD700"?(p+=`<line x1="${v.x.toFixed(1)}" y1="${v.y.toFixed(1)}" x2="${L.x.toFixed(1)}" y2="${L.y.toFixed(1)}"
        ...
        stroke="${_}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const f=48,u=64,m=13,x=16,b={normal:"#aaaaaa",pépite:"#D4A017",papyte:"#111111",légende:"#7a28b8"};for(const[y,g]of Object.entries(r)){const v=c(y);if(!v||!g)continue;const L=y.replace(/[0-9]/g,""),h=W[L]||"#555",w=i==="attack"&&["MIL","ATT"].includes(L)&&!g.used||i==="defense"&&["GK","DEF","MIL"].includes(L)&&!g.used,_=a.includes(g.cardId);let I;i==="attack"?I=L==="MIL"?Number(g.note_m)||0:Number(g.note_a)||0:i==="defense"?I=L==="GK"?Number(g.note_g)||0:L==="MIL"?Number(g.note_m)||0:Number(g.note_d)||0:I=Number(L==="GK"?g.note_g:L==="DEF"?g.note_d:L==="MIL"?g.note_m:g.note_a)||0,I=I+(g.boost||0);const E=(v.x-f/2).toFixed(1),C=(v.y-u/2).toFixed(1),j=g.used?.25:1,G=b[g==null?void 0:g.rarity]||b.normal,A=_?"#ff3030":G,z=_?3:(g==null?void 0:g.rarity)==="légende"||(g==null?void 0:g.rarity)==="pépite"?2.5:2,B=u-m-x;p+=`<defs><clipPath id="cp-${y}"><rect x="${E}" y="${(v.y-u/2+m).toFixed(1)}" width="${f}" height="${B}"/></clipPath></defs>`,p+=`<rect x="${E}" y="${C}" width="${f}" height="${u}" rx="5" fill="${h}" opacity="${j}"/>`;const T=N(g);T&&!g.used&&(p+=`<image href="${T}" x="${E}" y="${(v.y-u/2+m).toFixed(1)}" width="${f}" height="${B}" clip-path="url(#cp-${y})" preserveAspectRatio="xMidYMin slice"/>`),p+=`<rect x="${E}" y="${C}" width="${f}" height="${m}" rx="4" fill="rgba(255,255,255,0.92)"/>`,p+=`<text x="${v.x.toFixed(1)}" y="${(v.y-u/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(g.name||"").slice(0,9)}</text>`;const M=(v.y+u/2-x).toFixed(1);if(p+=`<rect x="${E}" y="${M}" width="${f}" height="${x}" fill="rgba(255,255,255,0.92)"/>`,g.used)p+=`<text x="${v.x.toFixed(1)}" y="${(v.y+u/2-x/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="13" font-weight="900" fill="rgba(0,0,0,0.4)" font-family="Arial Black">–</text>`;else{const R=be(g.country_code);R?p+=`<image href="${R}" x="${(v.x-20).toFixed(1)}" y="${(v.y+u/2-x+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:p+=`<text x="${(v.x-13).toFixed(1)}" y="${(v.y+u/2-x/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${st(g.country_code)}</text>`,p+=`<text x="${v.x.toFixed(1)}" y="${(v.y+u/2-x/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${I}</text>`;const q=at(g);q?p+=`<image href="${q}" x="${(v.x+f/2-14).toFixed(1)}" y="${(v.y+u/2-x+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:g.clubName&&(p+=`<text x="${(v.x+14).toFixed(1)}" y="${(v.y+u/2-x/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(g.clubName||"").slice(0,3).toUpperCase()}</text>`),g.boost&&(p+=`<rect x="${(v.x+f/2-12).toFixed(1)}" y="${(v.y-u/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,p+=`<text x="${(v.x+f/2-5).toFixed(1)}" y="${(v.y-u/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${g.boost}</text>`)}p+=`<rect x="${E}" y="${C}" width="${f}" height="${u}" rx="5" fill="${_?"rgba(255,255,255,0.12)":"none"}" stroke="${A}" stroke-width="${z}" opacity="${j}"/>`,w&&(p+=`<rect x="${E}" y="${C}" width="${f}" height="${u}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${_?"selected":""}" data-card-id="${g.cardId}" data-role="${L}" style="cursor:pointer"/>`)}const $=38;return`<svg viewBox="${-$} ${-$} ${d+$*2} ${o+$*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${p}
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;height:calc(100dvh - 130px);max-height:calc(100dvh - 130px);overflow:hidden;background:#0a3d1e;position:relative">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:100px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const w=t.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
            ...
            ${re((w.players||[]).map(_=>({..._,used:!1})),"#ff6b6b",w.total)}
            ...
            ${re((w.players||[]).map(_=>({..._,used:!1})),"#00ff88",w.total)}
          </div>`}const h=t.log[t.log.length-1];return h?'<div style="padding:2px 4px">'+di(h)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})()}
    </div>
      ...
      <div style="flex:1;display:grid;grid-template-columns:repeat(4,1fr);gap:3px;align-content:start">
        ${p.map(h=>{var w;return`
          <div class="gc-mini" data-gc-id="${h.id}" data-gc-type="${h.gc_type}"
            ...
            <div style="font-size:16px">${((w=ee[h.gc_type])==null?void 0:w.icon)||"⚡"}</div>
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(h=>{var w,_,I;if(h.type==="duel"){const E=h.isGoal,C=h.homeScored?"#FFD700":E?"#ff6b6b":"rgba(255,255,255,0.3)",j=h.homeScored?"⚽ BUT !":E?"⚽ BUT IA !":(w=h.homePlayers)!=null&&w.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${E?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${C};margin-bottom:4px">
                ...
                ${(_=h.homePlayers)!=null&&_.length?`<div style="margin-bottom:3px">${re(h.homePlayers,"rgba(255,255,255,0.7)",h.homeTotal)}</div>`:""}
                ${(I=h.aiPlayers)!=null&&I.length?`<div style="opacity:0.7">${re(h.aiPlayers,"#ff6b6b",h.aiTotal)}</div>`:""}
                ...
                <div style="display:flex;align-items:center;gap:8px">
                  ${h.outPlayer?K({...h.outPlayer,used:!0,_line:h.outPlayer.job,rarity:"normal"},38,50):""}
                  <span style="color:rgba(255,255,255,0.4);font-size:18px">→</span>
                  ${h.inPlayer?K({...h.inPlayer,_line:h.inPlayer.job,rarity:"normal"},38,50):""}
                </div>
  ...
  </div>`,requestAnimationFrame(()=>{const h=e.querySelector("#match-field"),w=e.querySelector(".terrain-wrapper");if(!h||!w)return;const _=h.clientHeight||h.offsetHeight,I=h.clientWidth||h.offsetWidth,E=Math.min(_,I)||280;w.style.width=E+"px",w.style.height=E+"px"}),t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase==="attack"||t.phase==="defense"){let h=!1,w=30;const _=()=>document.getElementById("match-timer"),I=()=>{const E=_();if(!E)return;const C=String(Math.floor(w/60)).padStart(2,"0"),j=String(w%60).padStart(2,"0");E.textContent=` ${C}:${j}`,E.style.color=h?"#ff2222":"#ff9500",E.style.fontWeight="900"};I(),t._timerInt=setInterval(()=>{if(w--,w<0)if(!h)h=!0,w=15,I();else{clearInterval(t._timerInt),t._timerInt=null,t.homeScore=0,t.aiScore=3;const E=document.createElement("div");E.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;color:#fff;padding:24px;text-align:center",E.innerHTML='<div style="font-size:56px">⏱️</div><div style="font-size:24px;font-weight:900;color:#ff4444">MATCH PERDU PAR FORFAIT</div><div style="font-size:14px;color:rgba(255,255,255,0.6)">Temps écoulé</div>',document.body.appendChild(E),setTimeout(()=>{E.remove(),H(e,t,i)},2500)}else I()},1e3)}(m=document.getElementById("match-quit"))==null||m.addEventListener("click",()=>{confirm("Abandonner ? Résultat : défaite 3-0")&&(t.homeScore=0,t.aiScore=3,H(e,t,i))}),(x=document.getElementById("view-ai"))==null||x.addEventListener("click",()=>yi(t,i)),(b=document.getElementById("toggle-history"))==null||b.addEventListener("click",()=>{var h;(h=document.getElementById("match-history-panel"))==null||h.classList.add("open")}),($=document.getElementById("close-history"))==null||$.addEventListener("click",()=>{var h;(h=document.getElementById("match-history-panel"))==null||h.classList.remove("open")}),(y=document.getElementById("btn-action"))==null||y.addEventListener("click",()=>{t.selected.length!==0&&(r?ci(e,t,i):n&&pi(e,t,i))}),(g=document.getElementById("btn-results"))==null||g.addEventListener("click",()=>H(e,t,i)),e.querySelectorAll(".match-slot-hit").forEach(h=>{h.addEventListener("click",()=>li(h,t,e,i))}),e.querySelectorAll(".gc-mini").forEach(h=>{h.addEventListener("click",()=>gi(h.dataset.gcId,h.dataset.gcType,e,t,i))}),(v=document.getElementById("boost-card"))==null||v.addEventListener("click",()=>xi(e,t,i)),e.querySelectorAll(".sub-btn-col").forEach(h=>{h.addEventListener("click",()=>qe(e,t,i,h.dataset.subId))}),(L=document.getElementById("sub-btn-main"))==null||L.addEventListener("click",()=>qe(e,t,i))}function li(e,t,i,a){const d=e.dataset.cardId,o=e.dataset.role,s=t.selected.findIndex(l=>l.cardId===d);if(s!==-1)t.selected.splice(s,1);else{if(t.selected.length>=3){a.toast("Maximum 3 joueurs","error");return}const l=(t.homeTeam[o]||[]).find(r=>r.cardId===d);l&&t.selected.push({...l,_role:o,_line:o})}D(i,t,a)}function Te(e,t,i){e.matchId&&k.from("matches").update({last_player_id:i}).eq("id",e.matchId).then(()=>{})}function ci(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),Te(t,i,i.state.profile.id);const a=t.selected.map(o=>({...o,_line:o._role})),d=Ve(a,t.modifiers.home);t.pendingAttack={...d,players:[...t.selected],side:"home"},t.selected.forEach(o=>{const s=(t.homeTeam[o._role]||[]).find(l=>l.cardId===o.cardId);s&&(s.used=!0)}),t.log.push({text:`⚔️ Vous attaquez : ${d.total} (base ${d.base}${d.links?` +${d.links} liens`:""}) — ${t.selected.map(o=>o.name).join(", ")}`,type:"info"}),t.selected=[],t.modifiers.home={},t.phase="ai-defense",D(e,t,i),setTimeout(()=>fi(e,t,i),1200)}function pi(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),Te(t,i,i.state.profile.id);const a=t.selected.map(l=>({...l,_line:l._role})),d=Ye(a,t.modifiers.home);t.selected.forEach(l=>{const r=(t.homeTeam[l._role]||[]).find(n=>n.cardId===l.cardId);r&&(r.used=!0)});const o=We(t.pendingAttack.total,d.total,t.modifiers.home),s={type:"duel",title:"Défense",aiPlayers:(t.pendingAttack.players||[]).map(l=>({name:l.name,note:l._line==="MIL"?l.note_m:l.note_a,portrait:N(l),job:l.job,country_code:l.country_code,rarity:l.rarity,clubName:l.clubName,clubLogo:l.clubLogo})),homePlayers:t.selected.map(l=>{const r=(t.homeTeam[l._role]||[]).find(n=>n.cardId===l.cardId)||l;return{name:r.name,note:r._line==="GK"?r.note_g:r._line==="MIL"?r.note_m:r.note_d,portrait:N(r),job:r.job,country_code:r.country_code,rarity:r.rarity,clubName:r.clubName,clubLogo:r.clubLogo}}),homeTotal:d.total,aiTotal:t.pendingAttack.total,isGoal:!1,homeScored:!1,text:""};if(o.shielded)s.text="🛡️ Bouclier ! But annulé.",t.log.push(s);else if(o.goal){t.aiScore++,s.isGoal=!0,s.homeScored=!1,s.text=`⚽ BUT IA ! (${t.pendingAttack.total} > ${d.total})`,t.log.push(s),t.selected=[],t.modifiers.home={},t.pendingAttack=null,D(e,t,i),Le(s.aiPlayers,t.homeScore,t.aiScore,!1,()=>{ie(e,t,i,"home-attack")});return}else s.text=`🧤 Défense réussie ! (${d.total} ≥ ${t.pendingAttack.total})`,t.log.push(s);t.selected=[],t.modifiers.home={},t.pendingAttack=null,ie(e,t,i,"home-attack")}function ve(e,t,i){Te(t,i,null);const a=[...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]],d=Je(a,"attack",t.difficulty);if(!d.length){ct(e,t,i);return}const o=Ve(d,t.modifiers.ai);t.pendingAttack={...o,players:d,side:"ai"},d.forEach(n=>{n.used=!0}),t.log.push({text:`🤖 IA attaque : ${o.total} (${d.map(n=>n.name).join(", ")})`,type:"info"}),t.modifiers.ai={};const s=[...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(n=>!n.used),r=(t.homeSubs||[]).filter(n=>!(t.usedSubIds||[]).includes(n.cardId)).length>0&&t.subsUsed<t.maxSubs;if(s.length===0&&!r){t.aiScore++;const n={type:"duel",isGoal:!0,homeScored:!1,aiPlayers:d.map(c=>({name:c.name,note:c._line==="MIL"?c.note_m:c.note_a,portrait:N(c),job:c.job,country_code:c.country_code,rarity:c.rarity,clubName:c.clubName,clubLogo:c.clubLogo})),aiTotal:o.total,text:"⚽ BUT IA ! (aucun défenseur disponible)"};t.log.push(n),t.pendingAttack=null,D(e,t,i),Le(n.aiPlayers,t.homeScore,t.aiScore,!1,()=>{ie(e,t,i,"home-attack")});return}t.phase="defense",D(e,t,i)}function fi(e,t,i){const a=[...t.aiTeam.GK||[],...t.aiTeam.DEF||[],...t.aiTeam.MIL||[]],d=Je(a,"defense",t.difficulty),o=d.length>0?Ye(d,t.modifiers.ai).total:0;d.forEach(r=>{r.used=!0});const s=We(t.pendingAttack.total,o,t.modifiers.ai),l={type:"duel",title:"Attaque",homePlayers:(t.pendingAttack.players||[]).map(r=>({name:r.name,note:r._line==="MIL"?r.note_m:r.note_a,portrait:N(r),job:r.job,country_code:r.country_code,rarity:r.rarity,clubName:r.clubName,clubLogo:r.clubLogo})),aiPlayers:d.map(r=>({name:r.name,note:r._line==="GK"?r.note_g:r._line==="MIL"?r.note_m:r.note_d,portrait:N(r),job:r.job,country_code:r.country_code,rarity:r.rarity,clubName:r.clubName,clubLogo:r.clubLogo})),homeTotal:t.pendingAttack.total,aiTotal:o,isGoal:!1,homeScored:!1,text:""};if(s.shielded)l.text="🛡️ Bouclier IA !",t.log.push(l);else if(s.goal){t.homeScore++,l.isGoal=!0,l.homeScored=!0,l.text=`⚽ BUT ! (${t.pendingAttack.total} > ${o})`,t.log.push(l),t.modifiers.ai={},t.pendingAttack=null,D(e,t,i),Le(l.homePlayers,t.homeScore,t.aiScore,!0,()=>{ie(e,t,i,"ai-attack")});return}else l.text=`🧤 IA défend (${o} ≥ ${t.pendingAttack.total})`,t.log.push(l);t.modifiers.ai={},t.pendingAttack=null,ie(e,t,i,"ai-attack")}function ie(e,t,i,a){if(t.round++,lt(t)){H(e,t,i);return}if(a==="home-attack"){if(![...t.homeTeam.MIL||[],...t.homeTeam.ATT||[]].filter(o=>!o.used).length){if(![...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(s=>!s.used).length){H(e,t,i);return}t.phase="ai-attack",D(e,t,i),setTimeout(()=>ve(e,t,i),800);return}t.phase="attack",D(e,t,i)}else{if(![...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]].filter(o=>!o.used).length){ct(e,t,i);return}t.phase="ai-attack",D(e,t,i),setTimeout(()=>ve(e,t,i),800)}}function lt(e){const t=["MIL","ATT","GK","DEF"].some(a=>(e.homeTeam[a]||[]).some(d=>!d.used)),i=["MIL","ATT","GK","DEF"].some(a=>(e.aiTeam[a]||[]).some(d=>!d.used));return!t&&!i}function ct(e,t,i){lt(t)?H(e,t,i):(t.phase="attack",D(e,t,i))}function ui(e,t,i){const a=document.createElement("div");a.style.cssText=`
    position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:800;
    display:flex;flex-direction:column;align-items:center;justify-content:center;gap:20px;
    animation:subFadeIn 0.2s ease;
  `;const d=N(e),o=N(t),s=W[e.job]||"#555",l=W[t.job]||"#555",r=e.job==="GK"?e.note_g:e.job==="DEF"?e.note_d:e.job==="MIL"?e.note_m:e.note_a,n=t.job==="GK"?t.note_g:t.job==="DEF"?t.note_d:t.job==="MIL"?t.note_m:t.note_a;a.innerHTML=`
    <style>
  ...
  `,document.body.appendChild(a);let c=!1;const p=()=>{c||(c=!0,a.remove(),i())};a.addEventListener("click",p),setTimeout(p,2e3)}function Z(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function qe(e,t,i,a=null){if(t.phase!=="attack"){Z("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){Z(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const d=Object.values(t.homeTeam).flat().filter(p=>p.used),o=t.homeSubs.filter(p=>!t.usedSubIds.includes(p.cardId));if(!d.length){Z("Aucun joueur utilisé à remplacer");return}if(!o.length){Z("Aucun remplaçant disponible");return}let s=0,l=Math.max(0,o.findIndex(p=>p.cardId===a)),r=!1;const n=document.createElement("div");n.id="sub-overlay",n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function c(){var $,y,g,v,L,h;const p=d[s],f=o[l],u=Math.min(130,Math.round((window.innerWidth-90)/2)),m=Math.round(u*1.35),x=w=>`background:rgba(255,255,255,0.12);border:none;color:${w?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${w?"default":"pointer"};flex-shrink:0`;n.innerHTML=`
    ...
    <div style="flex:1;display:flex;gap:0;overflow:hidden">
      ...
      <div id="in-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
        ...
        <div>${f?K({...f,used:!1,boost:0},u,m):"<div>—</div>"}</div>
      ...
      <div id="out-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
        ...
        <div>${p?K({...p,used:!1,boost:0},u,m):"<div>—</div>"}</div>
    ...
    </div>`,($=n.querySelector("#sub-close"))==null||$.addEventListener("click",()=>n.remove()),(y=n.querySelector("#out-up"))==null||y.addEventListener("click",()=>{s>0&&(s--,c())}),(g=n.querySelector("#out-down"))==null||g.addEventListener("click",()=>{s<d.length-1&&(s++,c())}),(v=n.querySelector("#in-up"))==null||v.addEventListener("click",()=>{l>0&&(l--,c())}),(L=n.querySelector("#in-down"))==null||L.addEventListener("click",()=>{l<o.length-1&&(l++,c())});const b=(w,_,I,E)=>{const C=n.querySelector("#"+w);if(!C)return;let j=0;C.addEventListener("touchstart",G=>{j=G.touches[0].clientY},{passive:!0}),C.addEventListener("touchend",G=>{const A=G.changedTouches[0].clientY-j;if(Math.abs(A)<30)return;const z=_();A<0&&z<E-1?(I(z+1),c()):A>0&&z>0&&(I(z-1),c())},{passive:!0})};b("in-panel",()=>l,w=>l=w,o.length),b("out-panel",()=>s,w=>s=w,d.length),(h=n.querySelector("#sub-confirm"))==null||h.addEventListener("click",w=>{if(w.preventDefault(),w.stopPropagation(),r)return;r=!0;const _=d[s],I=o[l];if(!_||!I)return;let E=null,C=-1;for(const[G,A]of Object.entries(t.homeTeam)){const z=(A||[]).findIndex(B=>B.cardId===_.cardId);if(z!==-1){E=G,C=z;break}}if(C===-1||!E){Z("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),n.remove();return}const j={...I,_line:E,_col:_._col||0,used:!1,boost:0};t.homeTeam[E].splice(C,1,j),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(I.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:_.name,firstname:_.firstname,note:F(_,E),portrait:N(_),job:_.job,country_code:_.country_code,rarity:_.rarity,clubName:_.clubName,clubLogo:_.clubLogo},inPlayer:{name:I.name,firstname:I.firstname,note:F(I,E),portrait:N(I),job:I.job,country_code:I.country_code,rarity:I.rarity,clubName:I.clubName,clubLogo:I.clubLogo},text:`🔄 ${I.firstname} ${I.name} remplace ${_.firstname} ${_.name}`}),n.remove(),ui(_,I,()=>D(e,t,i))})}document.body.appendChild(n),c()}function gi(e,t,i,a,d){var l,r;const o=ee[t]||{icon:"⚡",desc:"Carte spéciale."},s=document.createElement("div");s.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;align-items:center;justify-content:center;padding:24px",s.innerHTML=`
    ...
    <div style="display:flex;flex-direction:column;gap:6px">
      ${a.map(d=>`
        <div class="player-boost-opt" data-card-id="${d.cardId}"
          ...
          <div style="width:32px;height:32px;background:${W[d.job]||"#888"};border-radius:6px;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:900;font-size:13px">${F(d,d._line||d.job)}</div>
          <div style="flex:1"><b>${d.firstname} ${d.name}</b><div style="font-size:11px;color:#888">${d._line||d.job}</div></div>
          <div style="color:#87CEEB;font-weight:700">+${t.boostCard.value}</div>
        </div>`).join("")}
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(d=>{d.addEventListener("click",()=>{const o=d.dataset.cardId;for(const s of["GK","DEF","MIL","ATT"]){const l=(t.homeTeam[s]||[]).find(r=>r.cardId===o);if(l){l.boost=(l.boost||0)+t.boostCard.value,t.log.push({text:`⚡ Boost +${t.boostCard.value} appliqué à ${l.name}`,type:"info"});break}}t.boostUsed=!0,i.closeModal(),D(e,t,i)})})}function Le(e,t,i,a,d){const o=document.createElement("div");o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:900;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;overflow:hidden;cursor:pointer";const s=Array.from({length:10},(n,c)=>`
  ...
  <div style="display:flex;gap:10px;animation:scoreIn 0.4s ease 1s both;position:relative;z-index:1">
    ${e.map(n=>`
    <div style="text-align:center">
      <div style="width:50px;height:50px;border-radius:50%;background:${W[n.job]||"#555"};border:2px solid #FFD700;position:relative;overflow:hidden;margin:0 auto">
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(o);let l=!1;const r=()=>{l||(l=!0,o.remove(),setTimeout(()=>d(),50))};o.addEventListener("click",r),setTimeout(r,3500)}async function H(e,t,i){var c,p;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:a}=i,d=t.homeScore>t.aiScore,o=t.homeScore===t.aiScore,s=d?"victoire":o?"nul":"defaite",l=yt(t.mode,s);t.matchId&&await k.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:d?a.profile.id:null,home_credits_reward:l,played_at:new Date().toISOString()}).eq("id",t.matchId);const r={credits:(a.profile.credits||0)+l,matches_played:(a.profile.matches_played||0)+1};d?r.wins=(a.profile.wins||0)+1:o?r.draws=(a.profile.draws||0)+1:r.losses=(a.profile.losses||0)+1,await k.from("users").update(r).eq("id",a.profile.id),await i.refreshProfile();const n=document.createElement("div");n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",n.innerHTML=`
    ...
    </div>`,document.body.appendChild(n),(c=document.getElementById("res-home"))==null||c.addEventListener("click",()=>{n.remove(),i.navigate("home")}),(p=document.getElementById("res-replay"))==null||p.addEventListener("click",()=>{n.remove(),i.navigate("match",{matchMode:t.mode})})}function yi(e,t){t.openModal("Équipe adverse (IA)",`<div style="background:#0a3d1e;padding:12px;border-radius:8px">
      ${Ae(e.aiTeam,e.formation,null,[],300,300)}
    </div>`,`<button class="btn btn-primary" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const hi={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"};async function bi(e,t){const{state:i,toast:a}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement du marché...</div>',await ze(e,t)}async function ze(e,t){const{state:i,toast:a}=t,{data:d}=await k.from("market_listings").select(`id, price, status, listed_at, seller_id,
  ...
  `;function l(r){const n=document.getElementById("mkt-content"),c=r==="buy"?s:o;if(c.length===0){n.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      ${c.map(p=>{var b,$,y;const f=(b=p.card)==null?void 0:b.player;if(!f)return"";const u=f.job==="GK"?f.note_g:f.job==="DEF"?f.note_d:f.job==="MIL"?f.note_m:f.note_a,m=hi[f.rarity],x=(i.profile.credits||0)>=p.price;return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px">
  ...
  </div>`}const S={user:null,profile:null,page:"home",params:{}};function se(e,t="info",i=3e3){const a=document.getElementById("toast");a&&(a.textContent=e,a.className=`show ${t}`,clearTimeout(a._t),a._t=setTimeout(()=>{a.className=""},i))}function Ei(e,t,i=""){document.getElementById("modal-title").textContent=e,document.getElementById("modal-body").innerHTML=t,document.getElementById("modal-footer").innerHTML=i,document.getElementById("modal-overlay").classList.remove("hidden")}function we(){document.getElementById("modal-overlay").classList.add("hidden")}async function de(){if(!S.user)return;const{data:e}=await k.from("users").select("*").eq("id",S.user.id).single();e&&(S.profile=e)}const Me="mw_theme";function fe(){return localStorage.getItem(Me)||"light"}function Ii(e){var t;localStorage.setItem(Me,e),$e(e),(t=S.profile)!=null&&t.id&&k.from("users").update({theme:e}).eq("id",S.profile.id).then(()=>{})}function $e(e){document.documentElement.setAttribute("data-theme",e)}function le(e,t={}){S.page=e,S.params=t,ft()}async function ft(){const e=document.getElementById("page-content");if(!e)return;document.querySelectorAll(".bottom-nav a").forEach(a=>{a.classList.toggle("active",a.dataset.page===S.page)});const t=document.getElementById("nav-credits");t&&S.profile&&(t.textContent=`💰 ${(S.profile.credits||0).toLocaleString("fr")}`);const i={state:S,navigate:le,toast:se,openModal:Ei,closeModal:we,refreshProfile:de};switch(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽</div>',S.page){case"home":await Be(e,i);break;case"collection":await wt(e,i);break;case"decks":await xe(e,i);break;case"boosters":await Nt(e,i);break;case"match":await oi(e,i);break;case"market":await bi(e,i);break;case"rankings":await _i(e,i);break;case"matches":await ki(e,i);break;default:await Be(e,i)}}function Ai(){const e=document.getElementById("app"),t=S.profile;if(!t)return;const i="/manager-wars/icons/";e.innerHTML=`
    <nav class="top-nav">
  ...
  `,document.querySelectorAll(".bottom-nav a").forEach(a=>{a.addEventListener("click",d=>{d.preventDefault(),le(a.dataset.page)})}),document.getElementById("nav-logo").addEventListener("click",()=>le("home")),document.getElementById("nav-credits").addEventListener("click",()=>le("boosters")),document.getElementById("theme-toggle").addEventListener("click",()=>{const d=fe()==="dark"?"light":"dark";Ii(d),Oe(d)}),Oe(fe())}function Oe(e){const t=document.getElementById("theme-icon");t&&(t.textContent=e==="dark"?"☀️":"🌙")}async function Ti(){$e(fe()),document.getElementById("modal-overlay").addEventListener("click",t=>{t.target===t.currentTarget&&we()}),document.getElementById("modal-close").addEventListener("click",we);const{data:{session:e}}=await k.auth.getSession();if(!e){Ke(),Ce(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:se});return}S.user=e.user,await de(),Ke();try{const{data:t}=await k.from("formation_links_overrides").select("formation, links"),i={};(t||[]).forEach(a=>{i[a.formation]=a.links}),ut(i)}catch(t){console.warn("Impossible de charger les overrides de formation:",t)}if(!S.profile){gt(document.getElementById("app"),{state:S,navigate:async()=>{await de(),Pe()},toast:se,refreshProfile:de});return}S.profile.theme&&S.profile.theme!==fe()&&(localStorage.setItem(Me,S.profile.theme),$e(S.profile.theme)),Pe(),k.auth.onAuthStateChange(async(t,i)=>{t==="SIGNED_OUT"&&(S.user=null,S.profile=null,document.getElementById("app").innerHTML="",Ce(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:se}))})}function Pe(){const e=document.getElementById("app");e.style.display="flex",e.style.flexDirection="column",Ai(),ft()}function Ke(){const e=document.getElementById("app-loader"),t=document.getElementById("app");t&&(t.style.display=""),e&&(e.classList.add("zoom-out"),setTimeout(()=>e.style.display="none",500))}Ti();
