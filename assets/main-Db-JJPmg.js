...
import{s as _,q as Ut,t as Ri,F as Oi,o as tt,n as ti,g as At,r as Ie,m as zt,u as zi,j as Hi,v as ro,w as ao,T as Li,p as so,x as lo,d as co}from"./special-cards-BY0MTcrl.js";const po="/";function ln(e,{navigate:t,toast:i}){let n="login";const r=()=>{var a,s,d,c,l,m,b;const o=n==="login";if(e.innerHTML=`
  ...
  `;function s(){var g;const c=document.getElementById("logo-preview"),l=document.getElementById("logo-initials"),m=((g=document.getElementById("setup-club"))==null?void 0:g.value)||"MW",b=m.trim().split(" ").filter(Boolean),y=b.length>=2?(b[0][0]+b[1][0]).toUpperCase():m.slice(0,2).toUpperCase();c&&(c.style.background=a,c.style.borderColor=o),l&&(l.textContent=y,l.style.color=o)}document.getElementById("color1").addEventListener("input",c=>{o=c.target.value,document.getElementById("swatch1").style.background=o,s()}),document.getElementById("color2").addEventListener("input",c=>{a=c.target.value,document.getElementById("swatch2").style.background=a,s()});function d(c){document.querySelectorAll(".setup-step").forEach(l=>l.classList.remove("active")),document.getElementById(`step-${c}`).classList.add("active"),document.getElementById("step-num").textContent=c,document.getElementById("progress-fill").style.width=`${Math.round(c/3*100)}%`,c===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("step1-error");if(l.textContent="",c.length<3){l.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:m}=await _.from("users").select("id").eq("pseudo",c).maybeSingle();if(m){l.textContent="Ce pseudo est déjà pris.";return}d(2)}),document.getElementById("step2-back").addEventListener("click",()=>d(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const c=document.getElementById("setup-club").value.trim(),l=document.getElementById("step2-error");if(l.textContent="",c.length<2){l.textContent="Nom trop court (min. 2 caractères).";return}const{data:m}=await _.from("users").select("id").eq("club_name",c).maybeSingle();if(m){l.textContent="Ce nom de club est déjà pris.";return}d(3)}),document.getElementById("step3-back").addEventListener("click",()=>d(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("setup-club").value.trim(),m=document.getElementById("step3-error"),b=document.getElementById("step3-finish");m.textContent="",b.disabled=!0,b.textContent="Création en cours…";try{const{error:y}=await _.from("users").insert({id:t.user.id,pseudo:c,club_name:l,club_color1:o,club_color2:a,credits:1e4});if(y)throw y;await fo(t.user.id),await r(),n(`Bienvenue ${c} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(y){m.textContent=y.message,b.disabled=!1,b.textContent="🚀 Créer mon profil !"}})}async function fo(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await _.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const mo="modulepreload",go=function(e){return"/"+e},cn={},Ln=function(t,i,n){let r=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),s=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));r=Promise.allSettled(i.map(d=>{if(d=go(d),d in cn)return;cn[d]=!0;const c=d.endsWith(".css"),l=c?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${l}`))return;const m=document.createElement("link");if(m.rel=c?"stylesheet":mo,c||(m.as="script"),m.crossOrigin="",m.href=d,s&&m.setAttribute("nonce",s),document.head.appendChild(m),c)return new Promise((b,y)=>{m.addEventListener("load",b),m.addEventListener("error",()=>y(new Error(`Unable to preload CSS for ${d}`)))})}))}function o(a){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=a,window.dispatchEvent(s),!s.defaultPrevented)throw a}return r.then(a=>{for(const s of a||[])s.status==="rejected"&&o(s.reason);return t().catch(o)})},ii="#1A6B3C",ni="#cc2222",yo="#D4A017",pn="#888";async function xo(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    </style>`}const Sn="mw_sound_volume";function Vi(){const e=localStorage.getItem(Sn);if(e===null)return 100;const t=parseInt(e,10);return Number.isFinite(t)?Math.max(0,Math.min(100,t)):100}function _o(e){localStorage.setItem(Sn,String(Math.max(0,Math.min(100,Math.round(e)))))}function Yi(){return Vi()===0}function fi(e){return Math.max(0,Math.min(1,e*(Vi()/100)))}function Wi(e,t=1){if(Yi())return null;try{const i=new Audio(e);return i.volume=fi(t),i.play().catch(()=>{}),i}catch{return null}}let gt=null,Mi=null,un=.3;function An(e,t=.3){if(un=t,gt&&Mi===e&&!gt.paused){gt.volume=fi(un);return}if(Gt(),!Yi())try{const i=new Audio(e);i.loop=!0,i.volume=fi(t),i.play().catch(()=>{}),gt=i,Mi=e}catch{}}function Gt(){if(gt)try{gt.pause(),gt.currentTime=0}catch{}gt=null,Mi=null}let Xt=null;function zn(e,t=.6){if(Ue(),!Yi())try{const i=new Audio(e);i.volume=fi(t),i.play().catch(()=>{}),Xt=i}catch{}}function Ue(){if(Xt)try{Xt.pause(),Xt.currentTime=0}catch{}Xt=null}const wo="2026.07.23-1159";async function fn(e,{state:t,navigate:i,toast:n}){var o,a,s;const r=t.profile;r&&(e.innerHTML=`
  <style>
    .home-dark {
      min-height: 100%;
  ...
  </div>`,requestAnimationFrame(()=>{var b,y,g,p,u;const d=((b=window.visualViewport)==null?void 0:b.height)||window.innerHeight,c=((y=document.querySelector(".top-nav"))==null?void 0:y.offsetHeight)||56,l=((g=document.querySelector(".bottom-nav"))==null?void 0:g.offsetHeight)||60;if(e.querySelector(".home-inner")){const x=d-c-l;e.querySelector(".home-dark").style.minHeight=x+"px"}if(window.innerWidth<768){const x=((p=e.querySelector(".home-hero"))==null?void 0:p.offsetHeight)||60,h=e.querySelector(".ranked-tile");e.querySelector(".play-grid");const v=((u=e.querySelector(".home-footer"))==null?void 0:u.offsetHeight)||44,$=["friend-requests-banner","match-invite-banner","ongoing-match-banner"].reduce((J,ne)=>{var ee;return J+(((ee=document.getElementById(ne))==null?void 0:ee.offsetHeight)||0)},0),f=12*5,M=d-c-l-x-v-$-f-32,k=Math.max(80,Math.round(M*.28)),S=Math.max(160,Math.round(M*.72)),P=Math.floor((S-10)/2);h&&(h.style.minHeight=h.style.maxHeight=k+"px"),e.querySelectorAll(".play-tile").forEach(J=>{J.style.minHeight=J.style.height=P+"px"}),e.querySelectorAll(".play-tile .play-icon").forEach(J=>{J.style.height=Math.round(P*.55)+"px"})}}),(o=document.getElementById("nav-rankings"))==null||o.addEventListener("click",()=>i("rankings")),(a=document.getElementById("nav-matches"))==null||a.addEventListener("click",()=>i("matches")),e.querySelectorAll("[data-action]").forEach(d=>{d.addEventListener("click",()=>{d.style.transform="scale(.96)",setTimeout(()=>d.style.transform="",180);const c=d.dataset.action;if(c==="match-ai"){To(i);return}if(c==="match-random"){i("match",{matchMode:"random"});return}if(c==="match-friend"){i("friends");return}if(c==="mini-league"){i("mini-league");return}if(c==="ranked"){i("ranked");return}n("Bientôt disponible","info")})}),document.getElementById("logout-btn").addEventListener("click",async()=>{await _.auth.signOut(),window.location.reload()}),(s=document.getElementById("try-home2-btn"))==null||s.addEventListener("click",()=>i("home2")),Cn(t,n),Lo(t,n,i),Mn(t,n,i),ko(t,n))}async function ko(e,t){const i=e.profile.id,{data:n}=await _.from("mini_league_members").select("league_id, prize_amount, mini_leagues(name)").eq("user_id",i).eq("prize_claimed",!1).gt("prize_amount",0);if(!(n!=null&&n.length))return;const r=n.reduce((a,s)=>a+(s.prize_amount||0),0),o=document.createElement("div");o.className="modal-overlay",o.style.zIndex="2200",o.innerHTML=`<div class="modal" style="max-width:400px">
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    ${Ie(e,{width:140})}
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),c=(s||[]).filter(E=>E.card_type==="player"&&E.player),l=(s||[]).filter(E=>E.card_type==="game_changer"),m=(s||[]).filter(E=>E.card_type==="formation"),b=(s||[]).filter(E=>E.card_type==="stadium"),{data:y}=await _.from("gc_definitions").select("name,gc_type,color,effect,image_url"),g={};(y||[]).forEach(E=>{g[E.name]=E});const{data:p}=await _.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),u={};(p||[]).forEach(E=>{u[E.id]=E}),b.forEach(E=>{E.stadium_def&&(u[E.stadium_id]=E.stadium_def)});const x=Object.keys(Oi),h=Object.keys(je),v={};c.forEach(E=>{const D=E.player.id;v[D]=(v[D]||0)+1}),new Set(Object.keys(v).map(E=>String(E)));const $=new Set(m.map(E=>E.formation)),f=new Set(l.map(E=>E.gc_type));let M="player",k="Tous",S="",P=!1;function J(){return[...c].sort((E,D)=>{const H=di.indexOf(E.player.job),R=di.indexOf(D.player.job);return H!==R?H-R:(E.player.surname_real||"").localeCompare(D.player.surname_real||"")})}function ne(){return[...d||[]].sort((E,D)=>{const H=di.indexOf(E.job),R=di.indexOf(D.job);return H!==R?H-R:(E.surname_real||"").localeCompare(D.surname_real||"")})}function ee(){return J().filter(E=>{const D=E.player,H=k==="Tous"||D.job===k,R=!S||`${D.firstname} ${D.surname_real}`.toLowerCase().includes(S);return H&&R})}function z(){return ne().filter(E=>{const D=k==="Tous"||E.job===k,H=!S||`${E.firstname} ${E.surname_real}`.toLowerCase().includes(S);return D&&H})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:0 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none;align-items:center"></div>
  ...
  </div>`,function(D){const H=document.getElementById(D);if(!H)return;H.addEventListener("wheel",de=>{Math.abs(de.deltaY)<=Math.abs(de.deltaX)||(de.preventDefault(),H.scrollLeft+=de.deltaY)},{passive:!1});let R=!1,re=0,B=0,G=!1,se=!1;H.addEventListener("mousedown",de=>{R=!0,G=!1,re=de.pageX,B=H.scrollLeft}),window.addEventListener("mouseup",()=>{R=!1,G&&(H.style.cursor="",se=!0),G=!1}),window.addEventListener("mousemove",de=>{if(!R)return;const he=de.pageX-re;!G&&Math.abs(he)<6||(G=!0,H.style.cursor="grabbing",de.preventDefault(),H.scrollLeft=B-he)}),H.addEventListener("click",de=>{se&&(de.stopPropagation(),de.preventDefault(),se=!1)},!0)}("col-grid");function I(){const E=document.getElementById("col-filters");E&&(M==="player"?(E.innerHTML=`
        ...
        <div style="display:flex;gap:6px;overflow-x:auto;padding-bottom:2px;align-items:center">
          ${Ao.map(D=>`
            <button class="filter-btn" data-job="${D}"
              style="flex-shrink:0;padding:4px 12px;border-radius:20px;font-size:12px;font-weight:600;cursor:pointer;
                border:1.5px solid ${D===k?"var(--green)":"var(--tile-border)"};
                background:${D===k?"var(--green)":"#fff"};
                color:${D===k?"#fff":"var(--tile-fg-dim)"}">
              ${D}
            </button>`).join("")}
          <button id="show-all-btn"
            style="flex-shrink:0;margin-left:auto;padding:4px 12px;border-radius:20px;font-size:12px;font-weight:600;cursor:pointer;
              ...
              color:${P?"#111":"var(--tile-fg-dim)"}; font-size:18px; padding:5px 10px">
            ${P?"👁️":"🚫👁️"}
          </button>
        </div>`,document.getElementById("col-search").addEventListener("input",D=>{S=D.target.value.toLowerCase(),K()}),e.querySelectorAll(".filter-btn").forEach(D=>{D.addEventListener("click",()=>{k=D.dataset.job,I(),K()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{P=!P,I(),K()})):(E.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{P=!P,I(),K()})))}function K(){const E=document.getElementById("col-grid");E&&(M==="player"?Y(E):M==="formation"?pe(E):M==="stadium"?V(E):U(E))}function Q(E,D,H,R,re,B=0){const G=document.getElementById("col-grid"),se=document.getElementById("col-big");if(!G||!se)return;var de=0;function he(){var ct;const Te=window.innerWidth>=768,ve=document.getElementById("col-big"),Se=document.getElementById("col-gap"),Ae=((ct=document.getElementById("col-grid"))==null?void 0:ct.parentElement)||null,Be=ve?ve.closest(".page"):null;if(ve&&Ae&&Be){let $e=0;Array.from(Be.children).forEach(function(j){j!==ve&&j!==Ae&&j!==Se&&($e+=j.offsetHeight)});const Ce=Math.max(0,Be.clientHeight-$e),qe=Math.round(Ce*(B/100)),Ge=Math.max(0,Ce-qe),w=Te?50/71:45/63,L=Math.round(Ge*w),A=Math.max(0,Ge-L);Se&&(Se.style.height=qe+"px"),ve.style.flex="none",ve.style.height=L+"px",ve.style.minHeight="0",Ae.style.flex="none",Ae.style.height=A+"px",Ae.style.minHeight="0",Ae.style.display="flex",Ae.style.overflow="hidden"}const ke=document.getElementById("col-grid");ke&&(ke.style.height="100%",ke.style.flexShrink="0",ke.style.overflowX="auto",ke.style.overflowY="hidden",ke.style.alignItems="center",ke.style.width="100%"),se.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:center center">'+D(E[de])+"</div>";var wt=se.querySelector("[data-card-id],[data-form-id],[data-gc-id]");wt&&wt.addEventListener("click",function(){R(E[de])}),requestAnimationFrame(function(){var $e=document.getElementById("big-card-inner");if(!(!$e||!se)){var Ce=se.clientHeight,qe=se.clientWidth-16,Ge=$e.offsetHeight,w=$e.offsetWidth;if(Ge>0&&w>0&&Ce>40){var L=Math.min(Ce/Ge,qe/w);$e.style.transform="scale("+L.toFixed(3)+")",$e.style.transformOrigin="center center"}}}),G.innerHTML=E.map(function($e,Ce){var qe=Ce===de,Ge="flex-shrink:0;cursor:pointer;border-radius:6px;overflow:hidden;display:inline-block;line-height:0;"+(qe?"outline:2.5px solid #D4A017;outline-offset:1px;background:rgba(212,160,23,0.25);":"");return'<div class="col-mini-item" data-idx="'+Ce+'" style="'+Ge+'">'+H($e,qe)+"</div>"}).join(""),G.querySelectorAll(".col-mini-item").forEach(function($e){$e.addEventListener("click",function(){de=Number($e.dataset.idx),he(),$e.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})}),requestAnimationFrame(function(){var $e=ke?ke.clientHeight:0,Ce=G.querySelector(".col-mini-item > div");if(!(!$e||!Ce)){var qe=Ce.style.zoom;Ce.style.zoom="1";var Ge=Ce.offsetHeight;if(Ce.style.zoom=qe,!(Ge<=0)){var w=$e/Ge;G.querySelectorAll(".col-mini-item > div").forEach(function(L){L.style.zoom=w.toFixed(4)})}}})}he()}function ue(E,D){var H=window.innerWidth>=768?.76:.54,R;if(!E||E._fake){var re=E?E.player:null;if(!re)return"";R=yn(re)}else R=Ii(E,"");var B=E&&!E._fake?v[E.player&&E.player.id]||1:0,G=B>0?'<div style="position:absolute;top:6px;right:6px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:11px;font-weight:900;padding:2px 7px;z-index:3;box-shadow:0 1px 4px rgba(0,0,0,0.4)">×'+B+"</div>":"";return'<div style="display:inline-block;position:relative;zoom:'+H+';pointer-events:none;line-height:0">'+G+R+"</div>"}function fe(E,D,H){var R=H>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+H+"</div>":"",re=!!D,B=zi(E,zt[E],{width:160});return re||(B='<div style="filter:grayscale(1);opacity:0.5">'+B+"</div>"),"<div "+(D?'data-form-id="'+D.id+'"':"")+' style="position:relative;cursor:pointer">'+R+B+"</div>"}function C(E,D){var H=window.innerWidth>=768?.76:.54,R=zi(E,zt[E],{width:140});return D||(R='<div style="filter:grayscale(1);opacity:0.45">'+R+"</div>"),'<div style="display:inline-block;zoom:'+H+';line-height:0;pointer-events:none">'+R+"</div>"}function Y(E){if(P){const D=z();if(!D.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucun joueur.</div>';return}const H=D.map(R=>c.find(re=>re.player.id===R.id)||{_fake:!0,player:R,id:"fake-"+R.id});Q(H,R=>R._fake?yn(R.player):Ii(R,""),R=>R._fake?ue({player:R.player,id:"x",_fake:!0}):ue(R),R=>{R._fake||xn(R,c,v,t)})}else{const D=ee();if(!D.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const H={};D.forEach(re=>{const B=re.player.id;H[B]||(H[B]=[]),H[B].push(re)});const R=Object.values(H).map(re=>Mo(re));Q(R,re=>{const B=v[re.player.id]||1,G=B>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${B}</div>`:"",de=c.filter(he=>he.player.id===re.player.id&&he.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Ii(re,G+de)},re=>ue(re),re=>xn(re,c,v,t))}}function pe(E){const D=P?x:[...$];if(!D.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const H=D.map(R=>({formation:R,card:m.find(re=>re.formation===R)||null,owned:$.has(R)}));Q(H,({formation:R,card:re,owned:B})=>fe(R,B?re:null,B?m.filter(G=>G.formation===R).length:0),({formation:R,owned:re})=>C(R,re),({card:R,owned:re})=>{re&&R&&qo(R,m,t,o)},"#1A6B3C",5)}function U(E){const D=Object.keys(g),H=P?D.length?D:h:[...f];if(!H.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const R=H.map(re=>({type:re,gc:je[re]||{icon:"⚡",desc:""},def:g[re]||null,owned:f.has(re),card:l.find(B=>B.gc_type===re)||null}));Q(R,({type:re,gc:B,def:G,owned:se,card:de})=>{var ke;const he=(G==null?void 0:G.name)||re,Te=se?l.filter(wt=>wt.gc_type===re).length:0,ve=Te>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${Te}</div>`:"",Se=(G==null?void 0:G.effect)||B.desc||"",Ae=G!=null&&G.image_url?`/icons/${G.image_url}`:((ke=G==null?void 0:G.club)==null?void 0:ke.logo_url)||(G!=null&&G.country_code?`https://flagsapi.com/${G.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null);let Be=tt(he,Ae,B.icon,Se,{width:160,onClick:se});return se||(Be=`<div style="filter:grayscale(1);opacity:0.5">${Be}</div>`),`<div ${se&&de?`data-gc-id="${de.id}" data-gc-type="${re}"`:""} style="position:relative">${ve}${Be}</div>`},({type:re,gc:B,def:G,owned:se})=>{const de=window.innerWidth>=768?.76:.54,he=(G==null?void 0:G.name)||re,Te=(G==null?void 0:G.effect)||B.desc||"",ve=G!=null&&G.image_url?`/icons/${G.image_url}`:null;let Se=tt(he,ve,B.icon,Te,{width:140});return se||(Se=`<div style="filter:grayscale(1);opacity:0.45">${Se}</div>`),`<div style="display:inline-block;zoom:${de};line-height:0;pointer-events:none">${Se}</div>`},({type:re,owned:B,def:G})=>{B&&Bo(re,G,o)},"#7a28b8",5)}function V(E){const D="#4FC3F7",H="/";if(!b.length){E.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--tile-fg-dim)">Aucune carte Stade.<br><small>Ouvre un booster Stade !</small></div>';return}const R={};b.forEach(B=>{const G=B.stadium_id||"?";(R[G]=R[G]||[]).push(B)});const re=Object.entries(R).map(([B,G])=>({sid:B,def:u[B]||null,count:G.length,card:G[0]}));Q(re,({def:B,count:G})=>{var Se,Ae;const se=(B==null?void 0:B.name)||"?",de=((Se=B==null?void 0:B.club)==null?void 0:Se.encoded_name)||(B==null?void 0:B.country_code)||"—",he=B!=null&&B.image_url?`${H}icons/${B.image_url}`:((Ae=B==null?void 0:B.club)==null?void 0:Ae.logo_url)||(B!=null&&B.country_code?`https://flagsapi.com/${B.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),Te=G>1?`<div style="position:absolute;top:8px;right:8px;background:#333;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${G}</div>`:"",ve=`${de}<br>+10 ⭐ joueurs alliés`;return`<div style="position:relative">${Te}${ti(se,he,ve,{width:160})}</div>`},({def:B})=>{var ve,Se;const G=window.innerWidth>=768?.76:.54,se=(B==null?void 0:B.name)||"?",de=((ve=B==null?void 0:B.club)==null?void 0:ve.encoded_name)||(B==null?void 0:B.country_code)||"—",he=B!=null&&B.image_url?`${H}icons/${B.image_url}`:((Se=B==null?void 0:B.club)==null?void 0:Se.logo_url)||(B!=null&&B.country_code?`https://flagsapi.com/${B.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),Te=`${de}<br>+10 ⭐`;return`<div style="display:inline-block;zoom:${G};line-height:0;pointer-events:none">${ti(se,he,Te,{width:140})}</div>`},null,D,5)}e.querySelectorAll(".col-tab-btn").forEach(E=>{E.addEventListener("click",()=>{M=E.dataset.tab,k="Tous",S="",P=!1,e.querySelectorAll(".col-tab-btn").forEach(D=>{const H=D.dataset.tab===M;D.style.borderBottomColor=H?"var(--green)":"transparent",D.style.color=H?"var(--green)":"var(--tile-fg-dim)"}),I(),K()})}),I(),K()}function Bo(e,t,i){const n=je[e]||{icon:"⚡",desc:"Effet spécial."},r=(t==null?void 0:t.name)||e,o=(t==null?void 0:t.effect)||n.desc,a=t!=null&&t.image_url?`/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
      ${tt(r,a,n.icon,o,{width:200})}
      <div style="background:#fff3cd;border-radius:10px;padding:10px 14px;width:100%">
        <div style="font-size:12px;color:#856404">⚠️ Cette carte est à <b>usage unique</b>. Une fois jouée en match, elle est définitivement supprimée de ta collection.</div>
      </div>
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const ci=1e3;function qo(e,t,i,n){var g,p,u;const{state:r,toast:o,closeModal:a,navigate:s,refreshProfile:d}=i,c=e.formation,l={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function m(){const x=zt[c]||{},h=Oi[c]||[],v=290,$=360,f=20;function M(S){const P=x[S];return P?{x:P.x*v,y:P.y*$}:null}let k=`<svg width="${v}" height="${$}" viewBox="0 0 ${v} ${$}" xmlns="http://www.w3.org/2000/svg">`;for(const[S,P]of h){const J=M(S),ne=M(P);!J||!ne||(k+=`<line x1="${J.x}" y1="${J.y}" x2="${ne.x}" y2="${ne.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const S of Object.keys(x)){const P=M(S);if(!P)continue;const J=S.replace(/\d+/,""),ne=l[J]||"#555";k+=`<circle cx="${P.x}" cy="${P.y}" r="${f}" fill="${ne}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,k+=`<text x="${P.x}" y="${P.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${J}</text>`}return k+="</svg>",k}const b=t.filter(x=>x.formation===c);b.length;const y=!e.is_for_sale;n(`Formation ${c}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    <div style="margin-top:16px;border-top:1px solid var(--tile-border);padding-top:14px">
      ...
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px">
        ${m.filter(U=>U.id!==e.id).map((U,V)=>{const E=ne[U.id]||[],D=U.is_for_sale,H=E.length?E[E.length-1]:null,R=U.evolution_bonus||0,re=H?H.source==="booster"?"Booster":H.price?H.price.toLocaleString("fr")+" cr.":"—":"—",B=H?new Date(H.transferred_at).toLocaleDateString("fr",{day:"2-digit",month:"2-digit",year:"numeric"}):"",G=80,se=Math.round(G*657/507),de=Ie({...l,_evolution_bonus:R},{width:G});return`
            <div class="exemplaire-row" data-card-id="${U.id}" data-card-idx="${V}"
              style="position:relative;cursor:${D?"not-allowed":"pointer"};opacity:${D?.55:1};transition:transform .1s">
              <!-- Checkbox cachée -->
              ...
              <div class="expl-mini-card" style="position:relative;width:${G}px;height:${se}px;border-radius:8px;overflow:hidden">
                ...
                ${D?'<div style="position:absolute;top:0;right:0;background:#e67e22;color:#fff;font-size:6px;font-weight:900;padding:2px 4px;border-radius:0 6px 0 4px;z-index:12">VENTE</div>':""}
              </div>
              <!-- Source + date -->
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;background:#f0fdf4;border-radius:10px;padding:10px 14px">
          <div>
            ...
            <div style="font-size:11px;color:#555;margin-top:2px">
              ${(e.evolution_bonus||0)>0?`+${e.evolution_bonus} appliqué${e.evolution_bonus>1?"s":""} · Note : ${k}${l.job2&&S>0?` / ${P}`:""}`:`Note actuelle : ${k}${l.job2&&S>0?` / ${P}`:""}`}
            </div>
          </div>
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",l.note_g],["DEF",l.note_d],["MIL",l.note_m],["ATT",l.note_a]].map(([U,V])=>{const E=Ti[U],D=U==="GK"?"#fff":E,H=U===l.job||U===l.job2,R=(Number(V)||0)+(H&&f>0?f:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
                <div style="width:48px;height:48px;border-radius:8px;background:#111;border:2.5px solid ${E};
                  display:flex;align-items:center;justify-content:center">
                  <span style="font-size:20px;font-weight:900;color:${D};font-family:Arial Black,Arial;line-height:1">${R}</span>
                </div>
                <span style="font-size:10px;font-weight:700;color:${D}">${U}</span>
              </div>`}).join("")}
          </div>
        </div>
        <div>
    ...
    ${ee}
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(fe=document.getElementById("close-detail"))==null||fe.addEventListener("click",s);let z=new Set;const I=l.rarity==="pepite"?1.3:l.rarity==="papyte"?.7:1;function K(){let U=0;return document.querySelectorAll(".expl-check:checked").forEach(V=>{if(V.dataset.id===e.id)return;const D=Number(V.dataset.evo)||0,H=Number(V.dataset.note)||0;U+=H+D}),Math.round(U*I)}const Q=()=>{const U=z.size,V=document.getElementById("sell-action-panel");if(!V)return;V.style.display=U>0?"block":"none",document.getElementById("sell-selected-count").textContent=U;const E=document.getElementById("evolve-btn");if(E){const D=K();E.textContent=D>0?`⬆️ Évoluer (+${D})`:"⬆️ Évoluer"}};document.querySelectorAll(".expl-check").forEach(U=>{U.addEventListener("change",()=>{const V=U.dataset.id;U.checked?z.add(V):z.delete(V);const E=U.closest(".exemplaire-row");if(E){const D=E.querySelector(".expl-sel-overlay"),H=E.querySelector(".expl-sel-check");D&&(D.style.display=U.checked?"block":"none"),H&&(H.style.display=U.checked?"flex":"none"),E.style.transform=U.checked?"scale(1.05)":"scale(1)"}Q()})}),document.querySelectorAll(".exemplaire-row").forEach(U=>{U.addEventListener("click",V=>{if(V.target.tagName==="INPUT")return;const E=U.querySelector(".expl-check");E&&!E.disabled&&(E.checked=!E.checked,E.dispatchEvent(new Event("change")))})}),(C=document.getElementById("evolve-btn"))==null||C.addEventListener("click",async()=>{if(b<=1)return;const U=[...z];if(!U.length)return;if(z.has(e.id)){const G=document.createElement("div");G.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",G.innerHTML=`
        ...
        <div style="background:var(--tile-bg);border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center;box-shadow:0 16px 48px rgba(0,0,0,0.3)">
          ...
          <div style="background:#f0fdf4;border-radius:10px;padding:12px;margin-bottom:16px;font-size:13px;color:#333">
            ...
            ⬆️ Bonus final : <strong style="color:#1A6B3C">+${D}</strong><br>
            📈 Évolution : <strong>${e.evolution_bonus||0}</strong> → <strong style="color:#1A6B3C">${(e.evolution_bonus||0)+D}</strong><br>
            📊 Note finale : <strong>${h}</strong> → <strong style="color:#1A6B3C">${h+D}</strong>
            ${v&&v>0?`<br>📊 Note 2 finale : <strong>${v}</strong> → <strong style="color:#1A6B3C">${v+D}</strong>`:""}
          </div>
  ...
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
    ...
    </div>`;u("⚽ Choisir une formation",h),document.querySelectorAll("#modal-body [data-forma]").forEach(v=>{v.addEventListener("click",()=>{t.formation=v.dataset.forma;const $=hn(t.formation),f={};$.forEach(M=>{t.slots[M]&&(f[M]=t.slots[M])}),t.slots=f,x(),_t(e,t,i)})})};e.querySelectorAll("#formation-mobile-btn, #formation-pc-btn").forEach(u=>u.addEventListener("click",y)),e.querySelectorAll("#add-stad-btn-pc, #add-stad-btn").forEach(u=>u.addEventListener("click",()=>Po(t,e,i))),e.querySelectorAll("#save-deck").forEach(u=>u.addEventListener("click",()=>Ro(t,i))),e.querySelectorAll("[data-remove-sub]").forEach(u=>{u.addEventListener("click",()=>{t.subs=t.subs.filter(x=>x!==u.dataset.removeSub),_t(e,t,i)})}),e.querySelectorAll("#add-sub-btn").forEach(u=>u.addEventListener("click",()=>No(t,e,i)))}function Fo(e,t,i,n){var v,$,f;const r=window.innerWidth>=900,o=e.querySelector(r?"#deck-field-pc":"#deck-field-mobile");if(!o)return;const a=(v=t.stadiumCards)==null?void 0:v.find(M=>M.id===t.stadiumCardId),s=a&&(($=t.stadDefMap)==null?void 0:$[a.stadium_id])||null,d=zt[t.formation]||{},c=Ri(t.formation)||[],l={};for(const M of i){const k=t.slots[M],S=k?t.playerCards.find(P=>P.id===k):null;S!=null&&S.player?l[M]={...S.player,_evolution_bonus:S.evolution_bonus||0,face:S.player.face||null}:l[M]=null}const m=window.innerWidth>=900,b=m?window.innerWidth-280:window.innerWidth-20,y=m?Math.min(b,860):b,g=Math.round(m?y*.82:y*.85),p=m?84:44;let u="";for(const[M,k]of c){const S=d[M],P=d[k];if(!S||!P)continue;const J=S.x*y,ne=Math.round(.03*g+S.y*.85*g),ee=P.x*y,z=Math.round(.03*g+P.y*.85*g),I=l[M],K=l[k],Q=Ut(I,K);Q==="#ff3333"||Q==="#cc2222"?u+=`<line x1="${J.toFixed(1)}" y1="${ne.toFixed(1)}" x2="${ee.toFixed(1)}" y2="${z.toFixed(1)}" stroke="${Q}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(u+=`<line x1="${J.toFixed(1)}" y1="${ne.toFixed(1)}" x2="${ee.toFixed(1)}" y2="${z.toFixed(1)}" stroke="${Q}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,u+=`<line x1="${J.toFixed(1)}" y1="${ne.toFixed(1)}" x2="${ee.toFixed(1)}" y2="${z.toFixed(1)}" stroke="${Q}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let x="";const h=Math.round(p*657/507);for(const M of i){const k=d[M];if(!k)continue;const S=l[M],P=k.x*y;k.y*g;const J=Math.round(.03*g+k.y*(.85*g)),ne=Math.round(P-p/2),ee=Math.round(J-h/2);if(S){const z=M.replace(/\d+/,""),I=s&&(s.club_id&&String(S.club_id)===String(s.club_id)||s.country_code&&S.country_code===s.country_code),K=Ie({...S,_evolution_bonus:S._evolution_bonus||0},{width:p,showStad:!0,stadDef:s,role:z});I&&((f=s.club)!=null&&f.logo_url||s.image_url),x+=`<div style="position:absolute;left:${ne}px;top:${ee}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${M}">
        <div style="position:relative">${K}</div>
      </div>`}else{const z=M.replace(/\d+/,"");x+=`<div style="position:absolute;left:${ne}px;top:${ee}px;width:${p}px;height:${h}px;
  ...
  `,document.body.appendChild(r);let o=!1;const a=document.getElementById("pack-cut-zone"),s=document.getElementById("pack-blade");let d=!1;const c=I=>I.touches&&I.touches[0]?I.touches[0].clientX:I.clientX;function l(I){o||(d=!0,s.style.opacity="1",m(I))}function m(I){if(!d||o)return;const K=a.getBoundingClientRect(),Q=c(I)-K.left,ue=Math.max(0,Math.min(1,Q/K.width));s.style.width=ue*K.width+"px",ue>=.82&&y()}function b(){o||(d=!1,s.style.transition="width .2s ease, opacity .2s ease",s.style.width="0",s.style.opacity="0",setTimeout(()=>{o||(s.style.transition="")},220))}function y(){var K;if(o)return;o=!0,d=!1,s.style.width="100%",s.style.opacity="1",(K=document.getElementById("cut-flash"))==null||K.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const I=document.getElementById("cut-hint");I&&(I.style.opacity="0"),a.classList.add("pack-cut"),setTimeout(()=>{s.style.opacity="0",document.getElementById("pack-phase").style.display="none",x(0)},620)}a.addEventListener("pointerdown",l),window.addEventListener("pointermove",m),window.addEventListener("pointerup",b),a.addEventListener("touchstart",l,{passive:!0}),window.addEventListener("touchmove",m,{passive:!0}),window.addEventListener("touchend",b);let g=0,p=!1;const u=new Set;function x(I){g=I,document.getElementById("reveal-phase").style.display="flex",h(),v(I,0),S()}function h(){const I=document.getElementById("card-dots");I&&(I.innerHTML=e.map((K,Q)=>`<div class="card-dot" data-i="${Q}" style="width:8px;height:8px;border-radius:50%;background:${Q===g?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),I.querySelectorAll(".card-dot").forEach(K=>K.addEventListener("click",()=>f(parseInt(K.dataset.i)))))}function v(I,K){var E,D;const Q=e[I],ue=document.getElementById("card-counter"),fe=document.getElementById("card-track");ue&&(ue.textContent=`Carte ${I+1} / ${e.length}`);const C=document.getElementById("reveal-btns");C&&(C.style.display=I===e.length-1?"flex":"none");const Y=Q.card_type==="player"&&((E=Q.player)==null?void 0:E.rarity)==="legende",pe=!u.has(I);u.add(I);let U=0;if(Q.card_type==="player"&&Q.player){const H=Q.player,R=H.job||"ATT";U=(Number(R==="GK"?H.note_g:R==="DEF"?H.note_d:R==="MIL"?H.note_m:H.note_a)||0)+(Q.evolution_bonus||0)}const V=H=>{fe.innerHTML=`
        <div id="current-card-wrap" style="position:relative;display:flex;flex-direction:column;align-items:center;gap:8px;${Y?"filter:drop-shadow(0 0 20px #7a28b8)":""}">
          <div style="transform:scale(1.25);transform-origin:center">${Jo(Q)}</div>
        ...
        </div>`;const R=document.getElementById("current-card-wrap");K!==0?(R.style.transition="none",R.style.transform=`translateX(${K>0?100:-100}%)`,requestAnimationFrame(()=>{R.style.transition="transform .28s cubic-bezier(.25,1,.5,1)",R.style.transform="translateX(0)"})):R.animate([{opacity:0,transform:"scale(.7)"},{opacity:1,transform:"scale(1)"}],{duration:300,easing:"cubic-bezier(.34,1.56,.64,1)"}),H||Y?J():ne(),h()};pe&&(((D=Q.player)==null?void 0:D.rarity)==="legende"||U>=18)&&Q.card_type==="player"&&Q.player?$(Q,()=>V(!0)):V(!1)}function $(I,K){var Te;p=!0;const Q=I.player,ue=`https://flagsapi.com/${Q.country_code}/flat/64.png`,fe=(Te=Q.clubs)==null?void 0:Te.logo_url,C=At(Q),Y=Q.job||"ATT",pe=Number(Y==="GK"?Q.note_g:Y==="DEF"?Q.note_d:Y==="MIL"?Q.note_m:Q.note_a)||0,U=I.evolution_bonus||0,V=pe+U,E=V>=18&&V<=20,D=V>=18,H=document.getElementById("walkout-overlay"),R=document.getElementById("walkout-stage");if(!H||!R){p=!1,K();return}let re=null;E&&(re=Wi("/sounds/Legendary.mp3",.8)),H.style.display="flex";const B=()=>{const ve=R.firstElementChild;ve&&(ve.classList.remove("wo-in"),ve.classList.add("wo-out"))},G=1800,se=400;R.innerHTML=`<img class="wo-in" src="${ue}" style="height:130px;border-radius:10px;box-shadow:0 10px 36px rgba(0,0,0,.6)" onerror="this.style.display='none'">`,navigator.vibrate&&navigator.vibrate(30),setTimeout(B,G),setTimeout(()=>{var ve;R.innerHTML=fe?`<img class="wo-in" src="${fe}" style="max-height:160px;max-width:210px;object-fit:contain">`:`<div class="wo-in" style="font-size:34px;font-weight:900;color:#fff;text-align:center">${((ve=Q.clubs)==null?void 0:ve.encoded_name)||"CLUB"}</div>`,navigator.vibrate&&navigator.vibrate(30)},G+se),setTimeout(B,G*2+se),setTimeout(()=>{R.innerHTML=C?`<img class="wo-in" src="${C}" style="height:200px;border-radius:50%;box-shadow:0 0 40px rgba(255,255,255,0.3);object-fit:cover;object-position:top">`:'<div class="wo-in" style="font-size:80px">👤</div>',navigator.vibrate&&navigator.vibrate(30)},(G+se)*2),setTimeout(B,(G+se)*2+G);const he=D?"#FFD700":{GK:"#c0c0c0",DEF:"#e03030",MIL:"#D4A017",ATT:"#3fbf5f"}[Y]||"#fff";setTimeout(()=>{R.innerHTML=`<div class="wo-in" style="
        font-size:${D?"120px":"90px"};font-weight:900;color:${he};
        font-family:Arial Black,Arial;line-height:1;
        text-shadow:0 0 30px ${he}, 0 0 60px ${he};
        ${D?"animation:woGlow 0.8s ease-in-out infinite;":""}">
        ${V}
      </div>`,D&&navigator.vibrate&&navigator.vibrate([50,30,100,30,200])},(G+se)*3),setTimeout(B,(G+se)*3+G),setTimeout(()=>{H.style.display="none",R.innerHTML="",p=!1,re&&!E&&re.pause(),navigator.vibrate&&navigator.vibrate([40,30,80]),K()},(G+se)*4)}function f(I){if(p||I<0||I>=e.length||I===g)return;const K=I>g?1:-1;g=I,v(I,K)}function M(){f(g+1)}function k(){f(g-1)}function S(){const I=document.getElementById("card-viewport");if(!I||I._swipeBound)return;I._swipeBound=!0;let K=0,Q=0,ue=0,fe=!1;const C=E=>E.touches?E.touches[0].clientX:E.clientX,Y=E=>E.touches?E.touches[0].clientY:E.clientY,pe=E=>{fe=!0,K=C(E),Q=Y(E),ue=0},U=E=>{if(!fe)return;ue=C(E)-K;const D=Y(E)-Q;if(Math.abs(ue)<Math.abs(D))return;const H=document.getElementById("current-card-wrap");H&&(H.style.transition="none",H.style.transform=`translateX(${ue*.6}px) rotate(${ue*.02}deg)`)},V=()=>{if(!fe)return;fe=!1;const E=document.getElementById("current-card-wrap"),D=55;ue<=-D&&g<e.length-1?M():ue>=D&&g>0?k():E&&(E.style.transition="transform .2s ease",E.style.transform="translateX(0)")};I.addEventListener("pointerdown",pe),I.addEventListener("pointermove",U),I.addEventListener("pointerup",V),I.addEventListener("pointercancel",V),I.addEventListener("touchstart",pe,{passive:!0}),I.addEventListener("touchmove",U,{passive:!0}),I.addEventListener("touchend",V),I.addEventListener("click",E=>{if(Math.abs(ue)>8)return;const D=I.getBoundingClientRect();E.clientX-D.left>D.width/2?M():k()})}let P=null;function J(){const I=document.getElementById("fireworks-canvas");if(!I)return;I.width=window.innerWidth,I.height=window.innerHeight;const K=I.getContext("2d"),Q=[];function ue(){const C=Math.random()*I.width,Y=Math.random()*I.height*.6,pe=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],U=pe[Math.floor(Math.random()*pe.length)];for(let V=0;V<60;V++){const E=Math.PI*2/60*V,D=2+Math.random()*5;Q.push({x:C,y:Y,vx:Math.cos(E)*D,vy:Math.sin(E)*D,alpha:1,color:U,size:2+Math.random()*3})}}ue(),P=setInterval(ue,600);function fe(){if(document.getElementById("fireworks-canvas")){K.clearRect(0,0,I.width,I.height);for(let C=Q.length-1;C>=0;C--){const Y=Q[C];if(Y.x+=Y.vx,Y.y+=Y.vy+.08,Y.vy*=.98,Y.alpha-=.018,Y.alpha<=0){Q.splice(C,1);continue}K.globalAlpha=Y.alpha,K.fillStyle=Y.color,K.beginPath(),K.arc(Y.x,Y.y,Y.size,0,Math.PI*2),K.fill()}K.globalAlpha=1,(P!==null||Q.length>0)&&requestAnimationFrame(fe)}}fe()}function ne(){P!==null&&(clearInterval(P),P=null);const I=document.getElementById("fireworks-canvas");I&&I.getContext("2d").clearRect(0,0,I.width,I.height)}if(n){const I=document.getElementById("reveal-btns");I&&(I.innerHTML='<button class="btn btn-primary" id="reveal-next" style="flex:1">Continuer →</button>'),(z=document.getElementById("reveal-next"))==null||z.addEventListener("click",()=>{ne(),r.remove(),n()})}else document.getElementById("reveal-collection").addEventListener("click",()=>{ne(),r.remove(),i("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{ne(),r.remove(),i("boosters")})}function Jo(e){var t,i,n,r;if(e.card_type==="player"&&e.player)return Ko(e);if(e.card_type==="game_changer"){const o=e._gcDef,a=(o==null?void 0:o.name)||e.gc_type||"Game Changer",s=(o==null?void 0:o.effect)||((t=ji[e.gc_type])==null?void 0:t.desc)||"",d=o!=null&&o.image_url?`/icons/${o.image_url}`:null,c=((i=ji[e.gc_type])==null?void 0:i.icon)||"⚡";return tt(a,d,c,s,{width:170})}if(e.card_type==="formation")return zi(e.formation,zt[e.formation],{width:160});if(e.card_type==="stadium"){const o=e._stadiumDef,a=(o==null?void 0:o.name)||"Stade",s=((n=o==null?void 0:o.club)==null?void 0:n.encoded_name)||(o==null?void 0:o.country_code)||"—",d=o!=null&&o.image_url?`/icons/${o.image_url}`:((r=o==null?void 0:o.club)==null?void 0:r.logo_url)||(o!=null&&o.country_code?`https://flagsapi.com/${o.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null),c=`${s}<br>+10 ⭐ joueurs alliés`;return ti(a,d,c,{width:170})}return'<div style="width:140px;height:200px;background:#333;border-radius:12px"></div>'}function Qo(e){var t,i;if((t=e==null?void 0:e.rates)!=null&&t.length){const n=document.createElement("div");n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.6);display:flex;align-items:center;justify-content:center;z-index:4000;padding:16px";const r={normal:"#ccc",pepite:"#D4A017",pépite:"#D4A017",papyte:"#909090",legende:"#7a28b8",légende:"#7a28b8"},o={player:"Joueur",formation:"Formation",game_changer:"Game Changer",game_helper:"Game Helper"};n.innerHTML=`
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff;position:relative">
      ...
      <!-- Pagination -->
      ${o.length>1?`<div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">${o.map((ee,z)=>`<div style="width:6px;height:6px;border-radius:50%;background:${z===l?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}</div>`:""}
      ...
      <div id="deck-bottom-bar" style="flex-shrink:0;padding:10px 14px 14px;display:flex;flex-direction:column;gap:8px;background:rgba(0,0,0,0.2)">
        ...
        stroke="${k}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const g=typeof window<"u"&&window.innerWidth>=900?Math.min(Math.max(81,Math.round(r*.225)),117):Math.max(44,Math.round(r*.168)),p=Math.round(g*657/507);for(const[x,h]of Object.entries(l)){const v=b(x);if(!v||!h)continue;const $=x.replace(/[0-9]/g,""),f=a.includes(h.cardId),M=i==="attack"&&(["MIL","ATT"].includes($)||f)&&!h.used||i==="defense"&&["GK","DEF","MIL"].includes($)&&!h.used,k=n.includes(h.cardId);let S=h.boost||0,P=!1;h.stadiumBonus&&(i==="attack"&&($==="ATT"||$==="MIL")||i==="defense"&&($==="GK"||$==="DEF"||$==="MIL")?(S+=10,P=!0):i||(S+=10,P=!0));const J=Math.round(v.x-g/2),ne=Math.round(v.y-p/2);if(h.used){y+=`<image href="${`${typeof import.meta<"u"&&"/"||"/"}icons/carte-dos.png`}" x="${J}" y="${ne}" width="${g}" height="${p}" preserveAspectRatio="xMidYMid slice" class="match-used-hit" data-card-id="${h.cardId}" data-role="${$}" style="cursor:pointer"/>`;continue}const ee=Ie({...h,_evolution_bonus:0,stadiumBonus:!1},{width:g,showStad:!1,stadDef:null,role:$,extraNote:S,_cardOffset:30,_forceStadColor:P}),z=k?`position:absolute;top:30px;left:0;width:${g}px;height:${p}px;outline:3px solid #FFD700;outline-offset:2px;border-radius:8px;pointer-events:none;`:"";y+=`<foreignObject x="${J-2}" y="${ne-30}" width="${g+8}" height="${p+60}" style="overflow:visible">
      <div xmlns="http://www.w3.org/1999/xhtml" style="position:relative">
        ${ee}
        ${k?`<div style="${z}"></div>`:""}
      </div>
    </foreignObject>`,M&&(y+=`<rect x="${J}" y="${ne}" width="${g}" height="${p}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${k?"selected":""}" data-card-id="${h.cardId}" data-role="${$}" style="cursor:pointer"/>`)}const u=s!==null?s:Math.round(Math.max(g*.7,80));return`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="${-u} ${-u} ${r+u*2} ${o+u*2}" width="100%" style="display:block;width:100%;margin:0 auto">
    ${y}
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    ${(()=>{const C=window.innerWidth>=700,Y=(B,G,se)=>{var Ae,Be;const de=(t.gcDefs||[]).find(ke=>ke.name===B.gc_type),he=(de==null?void 0:de.name)||B.gc_type,Te=(de==null?void 0:de.effect)||((Ae=je[B.gc_type])==null?void 0:Ae.desc)||"",ve=de!=null&&de.image_url?`/icons/${de.image_url}`:null,Se=((Be=je[B.gc_type])==null?void 0:Be.icon)||"⚡";return`<div class="gc-mini" data-gc-id="${B.id}" data-gc-type="${B.gc_type}" style="flex-shrink:0;cursor:pointer">
          ${tt(he,ve,Se,Te,{width:G})}
        </div>`},pe=(B,G)=>{var se;return`<div id="boost-card"
          style="box-sizing:border-box;width:${B}px;height:${G}px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:2px solid #87CEEB;border-radius:10px;cursor:pointer;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:${Math.round(G*.04)}px;text-align:center;flex-shrink:0">
          ...
          </div>`},U=(B,G)=>G?pe(95,162):Y(B,95),V=(B,G)=>G?pe(68,116):Y(B,68),E=C?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",D=h?`<button id="btn-results" style="${E};background:linear-gradient(135deg,#D4A017,#FFD700);border:none;color:#000">🏁 Résultats</button>`:p?`<div style="${E};background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);color:rgba(255,255,255,0.4)">⏳ Tour IA</div>`:f?`<button id="btn-pass" style="${E};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER (plus d'attaquants)</button>`:u?`<button id="btn-action" style="${E};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${t.selected.length===0?"disabled":""}> ⚔️ ATTAQUEZ <span id="match-timer" style="font-weight:900"></span></button>`:x?`<button id="btn-action" style="${E};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${t.selected.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="match-timer" style="font-weight:900"></span></button>`:`<div style="${E};background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1)"></div>`,H=u||x?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${t.selected.length}/3 sélectionné(s)</div>`:"",R=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${C?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
  ...
  <div id="match-history-panel">
    ...
    </div>`,(f=m.querySelector("#sub-close"))==null||f.addEventListener("click",()=>m.remove()),(M=m.querySelector("#out-up"))==null||M.addEventListener("click",()=>{s>0&&(s--,b())}),(k=m.querySelector("#out-down"))==null||k.addEventListener("click",()=>{s<o.length-1&&(s++,b())}),(S=m.querySelector("#in-up"))==null||S.addEventListener("click",()=>{c>0&&(c--,b())}),(P=m.querySelector("#in-down"))==null||P.addEventListener("click",()=>{c<a.length-1&&(c++,b())});const $=(ne,ee,z,I)=>{const K=m.querySelector("#"+ne);if(!K)return;let Q=0;K.addEventListener("touchstart",ue=>{Q=ue.touches[0].clientY},{passive:!0}),K.addEventListener("touchend",ue=>{const fe=ue.changedTouches[0].clientY-Q;if(Math.abs(fe)<30)return;const C=ee();fe<0&&C<I-1?(z(C+1),b()):fe>0&&C>0&&(z(C-1),b())},{passive:!0})};$("in-panel",()=>c,ne=>c=ne,a.length),$("out-panel",()=>s,ne=>s=ne,o.length),(J=m.querySelector("#sub-confirm"))==null||J.addEventListener("click",ne=>{if(ne.preventDefault(),ne.stopPropagation(),l)return;l=!0;const ee=o[s],z=a[c];if(!ee||!z)return;let I=null,K=-1;for(const[ue,fe]of Object.entries(t.homeTeam)){const C=(fe||[]).findIndex(Y=>Y.cardId===ee.cardId);if(C!==-1){I=ue,K=C;break}}if(K===-1||!I){$t("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),m.remove();return}const Q={...z,_line:I,_col:ee._col||0,used:!1,boost:0};t.homeTeam[I].splice(K,1,Q),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(z.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:ee.name,firstname:ee.firstname,note:Pe(ee,I),portrait:At(ee),job:ee.job,country_code:ee.country_code,rarity:ee.rarity,clubName:ee.clubName,clubLogo:ee.clubLogo},inPlayer:{name:z.name,firstname:z.firstname,note:Pe(z,I),portrait:At(z),job:z.job,country_code:z.country_code,rarity:z.rarity,clubName:z.clubName,clubLogo:z.clubLogo},text:`🔄 ${z.firstname} ${z.name} remplace ${ee.firstname} ${ee.name}`}),m.remove(),sr(ee,z,()=>we(e,t,i))})}document.body.appendChild(m),b()}function vr(e,t,i,n,r){var b,y;const o=(n.gcDefs||[]).find(g=>g.name===t),a=je[t]||{icon:"⚡",desc:"Carte spéciale."},s=(o==null?void 0:o.name)||t,d=(o==null?void 0:o.effect)||a.desc,c=o!=null&&o.image_url?`/icons/${o.image_url}`:null,l=a.icon||"⚡",m=document.createElement("div");m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",m.innerHTML=`
    ...
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      ...
      ${_i(e.aiTeam,e.formation,null,[],Math.min(window.innerWidth-40,860),Math.round(Math.min(window.innerWidth-40,860)*1.1))}
            ...
            <div style="background:rgba(255,255,255,0.08);border-radius:14px;padding:14px 20px;text-align:center;min-width:220px">
              ...
              ${Le.id!==nt.id?`<div style="font-size:20px;font-weight:900;color:${Le.color}">
                    ${Ne>Z.mmr?"📈":"📉"} ${nt.emoji} ${nt.label} → ${Le.emoji} ${Le.label}
                  </div>
                  <div style="font-size:13px;color:rgba(255,255,255,0.5);margin-top:4px">${Ne>Z.mmr?"Promotion !":"Rétrogradation"}</div>`:`<div style="font-size:36px">${Ee>=0?"📈":"📉"}</div>
            ...
            </div>`}}catch(Z){console.error("[Ranked] MMR update error:",Z)}(ie=document.getElementById("pvp-end-overlay"))==null||ie.remove();const q=document.createElement("div");q.id="pvp-end-overlay",q.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;color:#fff;padding:24px;text-align:center;overflow-y:auto";const F=T?"🤝":j?"🏆":"😞",O=T?"MATCH NUL":j?"VICTOIRE !":"DÉFAITE",X=T?"#fff":j?"#FFD700":"#ff6b6b";q.innerHTML=`
      ...
      ${N}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${l?"🏆 Retour à la Mini League":a?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(q),(te=q.querySelector("#pvp-end-home"))==null||te.addEventListener("click",()=>{q.remove(),it(e),l?y("mini-league",{openLeagueId:l}):y(a?"ranked":"home")})}const ee=_.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},w=>{var A;const L=w.new;try{if(L.status==="finished"||L.forfeit){if(M)return;M=!0,k&&(clearInterval(k),k=null),Ue(),L.game_state&&(f=L.game_state),ne(L);return}if(L.game_state){const j=f;f=L.game_state;const T=f._lastGC;if(T&&T.seq&&!J.has(T.seq)&&(J.add(T.seq),T.by!==p)){E(T.type,T.by,()=>Y());return}const N=j[p+"Score"]||0,q=j[u+"Score"]||0,F=f[p+"Score"]||0,O=f[u+"Score"]||0,X=F>N,ie=O>q;if((X||ie)&&!P.has(f.round)){P.add(f.round);const te=[...f.log||[]].reverse().find(le=>le.isGoal),Z=((te==null?void 0:te.homePlayers)||[]).map(le=>({name:le.name,note:le.note,portrait:le.portrait,job:le.job}));!!((A=te==null?void 0:te.text)!=null&&A.includes("DERNIER CORNER"))?Te().then(()=>{Qe(Z,F,O,X,()=>Y())}):Qe(Z,F,O,X,()=>Y());return}Y()}}catch(j){console.error("[PvP] crash:",j)}}).subscribe();async function z(w){Object.assign(f,w),f.lastActionAt=new Date().toISOString();const{error:L}=await _.from("matches").update({game_state:f}).eq("id",i);L&&g("Erreur de synchronisation","error");try{Y()}catch(A){console.error("[PvP] renderPvpScreen crash:",A)}}async function I(){if(M)return;M=!0,Gt(),Ue(),k&&(clearInterval(k),k=null);const w=n?v.away_id:v.home_id,L=n?"p2":"p1",A=n?"p1":"p2",j={...f,[L+"Score"]:3,[A+"Score"]:0,phase:"finished"},T=j.p1Score||0,N=j.p2Score||0;try{await _.from("matches").update({game_state:j}).eq("id",i);const{data:q,error:F}=await _.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:w,p_home_score:T,p_away_score:N,p_forfeit:!0});if(F&&console.error("[PvP] finish_pvp_match (forfeit):",F),l)try{await _.rpc("check_and_finish_mini_league",{p_league_id:l})}catch(O){console.error("[PvP] check_and_finish_mini_league (forfeit):",O)}try{const{data:O}=await _.rpc("apply_match_rewards",{p_match_id:i});O!=null&&O.success&&!(O!=null&&O.skipped)&&typeof t.refreshProfile=="function"&&await t.refreshProfile()}catch(O){console.error("[PvP] apply_match_rewards (forfeit):",O)}}catch{}try{_.removeChannel(ee)}catch{}setTimeout(()=>{it(e),l?y("mini-league",{openLeagueId:l}):y("home")},800)}const K={BOOST_STAT:({value:w=1,count:L=1,roles:A=[]},j,T)=>{const N=j[p+"Team"],q=Object.entries(N).filter(([F])=>!A.length||A.includes(F)).flatMap(([F,O])=>O.filter(X=>!X.used).map(X=>({...X,_line:F})));if(!q.length){j.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),T(j);return}Q(q,L,`Choisir ${L} joueur(s) à booster (+${w})`,F=>{F.forEach(O=>{const X=(N[O._line]||[]).find(ie=>ie.cardId===O.cardId);X&&(X.boost=(X.boost||0)+w,j.log.push({text:`⚡ +${w} sur ${X.name}`,type:"info"}))}),j[p+"Team"]=N,T(j)})},DEBUFF_STAT:({value:w=1,count:L=1,roles:A=[],target:j="ai"},T,N)=>{const q=j==="home"?p:u,F=T[q+"Team"],O=j==="home"?"allié":"adverse",X=Object.entries(F).filter(([ie])=>!A.length||A.includes(ie)).flatMap(([ie,te])=>te.map(Z=>({...Z,_line:ie})));if(!X.length){T.log.push({text:`🎯 Aucun joueur ${O}`,type:"info"}),N(T);return}Q(X,L,`Choisir ${L} joueur(s) ${O}(s) (-${w})`,ie=>{ie.forEach(te=>{const Z=(F[te._line]||[]).find(ae=>ae.cardId===te.cardId);Z&&(Z.boost=(Z.boost||0)-w,T.log.push({text:`🎯 -${w} sur ${Z.name}`,type:"info"}))}),T[q+"Team"]=F,N(T)})},GRAY_PLAYER:({count:w=1,roles:L=[],target:A="ai"},j,T)=>{const N=A==="home"?p:u,q=j[N+"Team"],F=A==="home"?"allié":"adverse",O=Object.entries(q).filter(([X])=>!L.length||L.includes(X)).flatMap(([X,ie])=>ie.filter(te=>!te.used).map(te=>({...te,_line:X})));if(!O.length){j.log.push({text:`❌ Aucun joueur ${F}`,type:"info"}),T(j);return}Q(O,w,`Choisir ${w} joueur(s) ${F}(s) à exclure`,X=>{const ie="usedCardIds_"+N,te=new Set(j[ie]||[]);X.forEach(Z=>{const ae=(q[Z._line]||[]).find(le=>le.cardId===Z.cardId);ae&&(ae.used=!0,te.add(Z.cardId),j.log.push({text:`❌ ${ae.name} exclu !`,type:"info"}))}),j[ie]=[...te],j[N+"Team"]=q,T(j)})},REVIVE_PLAYER:({count:w=1,roles:L=[]},A,j)=>{const T=A[p+"Team"],N=Object.entries(T).filter(([q])=>!L.length||L.includes(q)).flatMap(([q,F])=>F.filter(O=>O.used).map(O=>({...O,_line:q})));if(!N.length){A.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),j(A);return}Q(N,w,`Choisir ${w} joueur(s) à ressusciter`,q=>{q.forEach(F=>{const O=(T[F._line]||[]).find(X=>X.cardId===F.cardId);O&&(O.used=!1,A.log.push({text:`💫 ${O.name} ressuscité !`,type:"info"}))}),A[p+"Team"]=T,j(A)})},REMOVE_GOAL:({},w,L)=>{const A=u+"Score";w[A]>0?(w[A]--,w.log.push({text:"🚫 Dernier but annulé !",type:"info"})):w.log.push({text:"🚫 Aucun but à annuler",type:"info"}),L(w)},ADD_GOAL_DRAW:({},w,L)=>{w[p+"Score"]===w[u+"Score"]?(w[p+"Score"]++,w.log.push({text:"🎯 But bonus !",type:"info"})):w.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),L(w)},ADD_SUB:({value:w=1},L,A)=>{L.maxSubs=(L.maxSubs||3)+w,L.log.push({text:`🔄 +${w} remplacement(s)`,type:"info"}),A(L)},CUSTOM:({},w,L)=>L(w)};function Q(w,L,A,j){const T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let N=[];function q(){var O,X;const F=w.map(ie=>{const te=ie._line||ie.job||"MIL",Z=N.find(le=>le.cardId===ie.cardId),ae=Ie({...ie,_evolution_bonus:0},{width:90,showStad:!0,role:te,extraNote:ie.boost||0});return`<div class="pp-item" data-cid="${ie.cardId}"
        ...
        </div>`,(O=T.querySelector("#pp-close"))==null||O.addEventListener("click",()=>T.remove()),T.querySelectorAll(".pp-item").forEach(ie=>{ie.addEventListener("click",()=>{const te=ie.dataset.cid,Z=w.find(le=>le.cardId===te),ae=N.findIndex(le=>le.cardId===te);Z&&(ae>-1?N.splice(ae,1):N.length<L&&N.push(Z),q())})}),(X=T.querySelector("#pp-confirm"))==null||X.addEventListener("click",()=>{T.remove(),j(N)})}q(),document.body.appendChild(T)}async function ue(w,L){const j=(f["gcCardsFull_"+p]||[]).find(F=>F.id===w),T=(j==null?void 0:j._gcDef)||(f.gcDefs||[]).find(F=>{var O;return F.name===L||((O=F.name)==null?void 0:O.toLowerCase().trim())===(L==null?void 0:L.toLowerCase().trim())}),N=[...f["usedGc_"+p]||[],w],q={type:L,by:p,seq:(f._gcAnimSeq||0)+1};J.add(q.seq),E(L,p,async()=>{if(T!=null&&T.effect_type&&T.effect_type!=="CUSTOM"){const O=K[T.effect_type];if(O){const X={...f};O(T.effect_params||{},X,async ie=>{ie["usedGc_"+p]=N,ie._lastGC=q,ie._gcAnimSeq=q.seq,await z(ie)});return}}const F={...f};switch(L){case"Remplacement+":F.maxSubs=(F.maxSubs||3)+1,F.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const O=u+"Score";F[O]>0&&(F[O]--,F.log.push({text:"🚫 But annulé",type:"info"}));break}}F["usedGc_"+p]=N,F._lastGC=q,F._gcAnimSeq=q.seq,await z(F)})}function fe(w,L){const A="usedCardIds_"+w,j=new Set(f[A]||[]);L.forEach(T=>j.add(T)),f[A]=[...j]}function C(){for(const w of["p1","p2"]){const L=new Set(f["usedCardIds_"+w]||[]),A=f[w+"Team"];if(A)for(const j of["GK","DEF","MIL","ATT"])(A[j]||[]).forEach(T=>{T.used=L.has(T.cardId)})}}function Y(){var nn,on,rn,an,sn,dn;if(f.phase==="reveal")return pe();if(f.phase==="midfield")return V();if(f.phase==="finished")return Ge();const w=f[p+"Team"],L=f[u+"Team"];C();const A=f[p+"Score"],j=f[u+"Score"],T=f[p+"Name"],N=f[u+"Name"],q=f.phase===p+"-attack",F=f.phase===p+"-defense",O=Array.isArray(f["selected_"+p])?f["selected_"+p]:[],X=O.map(W=>W.cardId),ie=window.innerWidth>=700,te=f[p+"Subs"]||[],Z=f["usedSubIds_"+p]||[],ae=te.filter(W=>!Z.includes(W.cardId)),le=f["gcCardsFull_"+p]||[],ge=f["usedGc_"+p]||[],ce=le.filter(W=>!ge.includes(W.id)),ye=f.boostOwner===p&&!f.boostUsed,be=[...w.MIL||[],...w.ATT||[]].filter(W=>!W.used),oe=[...L.MIL||[],...L.ATT||[]].filter(W=>!W.used),Ee=!ke(L),Ne=be.length===0&&oe.length===0&&!Ee,Le=(w.DEF||[]).filter(W=>!W.used),Ve=(w.GK||[]).filter(W=>!W.used);let Oe=[];q&&be.length===0&&(Ee?(Oe=Le.map(W=>W.cardId),Le.length===0&&(Oe=Oe.concat(Ve.map(W=>W.cardId)))):Ne&&(Oe=[...Le,...Ve].map(W=>W.cardId)));function nt(W,me,ze){var Me,ft;const _e=W._gcDef,Bt=(_e==null?void 0:_e.name)||W.gc_type,at=(_e==null?void 0:_e.effect)||((Me=je[W.gc_type])==null?void 0:Me.desc)||"",Fe=_e!=null&&_e.image_url?`/icons/${_e.image_url}`:null,Re=((ft=je[W.gc_type])==null?void 0:ft.icon)||"⚡";return`<div class="pvp-gc-mini" data-gc-id="${W.id}" data-gc-type="${W.gc_type}" style="flex-shrink:0;cursor:pointer">
      ...
      </div>`;function jt(){const W=e.querySelector(".match-screen");if(!W)return;const me=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);W.style.height=me+"px",W.style.minHeight=me+"px",W.style.maxHeight=me+"px",W.style.overflow="hidden";const ze=e.querySelector("#mobile-action-bar"),_e=e.querySelector("#mobile-play-area");ze&&_e&&(_e.style.paddingBottom=ze.offsetHeight+"px")}if(jt(),setTimeout(jt,120),setTimeout(jt,400),S||(S=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",jt),window.visualViewport.addEventListener("scroll",jt)),window.addEventListener("resize",jt)),function(){const me=e.querySelector("#match-field .terrain-wrapper svg")||e.querySelector(".terrain-wrapper svg");if(!me)return;const ze=me.closest("#match-terrain-wrap");ze&&(ze.style.cssText="position:relative;width:100%;height:100%;padding:0"),me.removeAttribute("width"),me.removeAttribute("height"),me.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",me.setAttribute("preserveAspectRatio","xMidYMid meet")}(),f._pvpResizeBound||(f._pvpResizeBound=!0,window.addEventListener("resize",()=>{const W=e.querySelector(".terrain-wrapper svg");if(W){const me=W.closest("#match-terrain-wrap");me&&(me.style.cssText="position:relative;width:100%;height:100%;padding:0"),W.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0"}})),e.querySelectorAll(".match-slot-hit").forEach(W=>{W.addEventListener("click",()=>{if(!q&&!F)return;const me=W.dataset.cardId,ze=W.dataset.role,_e=(w[ze]||[]).find(Re=>Re.cardId===me);if(!_e||_e.used)return;const Bt=Oe.includes(me);if(q&&!["MIL","ATT"].includes(ze)&&!Bt)return;Array.isArray(f["selected_"+p])||(f["selected_"+p]=[]);const at=f["selected_"+p],Fe=at.findIndex(Re=>Re.cardId===me);Fe>-1?at.splice(Fe,1):at.length<3&&at.push({..._e,_role:ze}),Y()})}),e.querySelectorAll(".match-used-hit").forEach(W=>{W.addEventListener("click",()=>R(W.dataset.cardId))}),e.querySelectorAll(".pvp-sub-btn").forEach(W=>{W.addEventListener("click",()=>R())}),(nn=e.querySelector("#pvp-sub-open"))==null||nn.addEventListener("click",()=>R()),e.querySelectorAll(".pvp-gc-mini").forEach(W=>{q?W.addEventListener("click",()=>D(W.dataset.gcId,W.dataset.gcType)):(W.style.opacity="0.35",W.style.cursor="default",W.addEventListener("click",()=>$t("⚡ Les Game Changers ne sont utilisables qu'en attaque","rgba(180,100,0,0.9)")))}),(on=e.querySelector("#pvp-boost-card"))==null||on.addEventListener("click",()=>H()),(rn=e.querySelector("#pvp-action"))==null||rn.addEventListener("click",W=>{q?W.currentTarget.dataset.pass==="1"||!ct(p)?se():Se():F&&Ae()}),(an=e.querySelector("#pvp-quit"))==null||an.addEventListener("click",()=>{confirm("Quitter ? Vous perdrez par forfait.")&&I()}),(sn=e.querySelector("#pvp-view-opp"))==null||sn.addEventListener("click",()=>B()),(dn=e.querySelector("#pvp-toggle-history"))==null||dn.addEventListener("click",()=>G()),k&&(clearInterval(k),k=null),Ue(),(q||F)&&!M){let W=30,me=!1;const ze=()=>document.getElementById("pvp-timer"),_e=()=>{ze()&&(ze().textContent=W+"s",ze().style.color=me?"#ff4444":"#fff")};_e(),k=setInterval(()=>{W--,W<0?me?(clearInterval(k),k=null,Ue(),q&&!ct(p)?se():I()):(me=!0,W=15,_e(),zn("/sounds/timer-urgent.mp3",.6)):_e()},1e3)}}function pe(){st(e),e.innerHTML=`
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="animation:gcFlipIn .7s cubic-bezier(.34,1.56,.64,1) both,gcGlow 1.8s ease infinite .7s">
        ${tt(N,F,O,q,{width:200})}
      </div>
      <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;animation:gcLabel .3s ease 1.2s both">Appuyer pour continuer</div>`,document.body.appendChild(te);let Z=!1;const ae=()=>{Z||(Z=!0,te.remove(),setTimeout(()=>A&&A(),50))};te.addEventListener("click",ae),setTimeout(ae,3e3)}function D(w,L){var ie,te,Z,ae;const j=(f["gcCardsFull_"+p]||[]).find(le=>le.id===w),T=j==null?void 0:j._gcDef,N=(T==null?void 0:T.name)||L,q=(T==null?void 0:T.effect)||((ie=je[L])==null?void 0:ie.desc)||"Carte spéciale.",F=T!=null&&T.image_url?`/icons/${T.image_url}`:null,O=((te=je[L])==null?void 0:te.icon)||"⚡",X=document.createElement("div");X.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",X.innerHTML=`
      ${tt(N,F,O,q,{width:190})}
      <div style="display:flex;gap:12px;width:190px">
        <button id="pvp-gc-back" style="flex:1;padding:13px;border-radius:12px;border:1px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Retour</button>
        <button id="pvp-gc-use" style="flex:1;padding:13px;border-radius:12px;border:none;background:#FFD700;color:#000;font-size:14px;font-weight:900;cursor:pointer">Utiliser ⚡</button>
      </div>`,document.body.appendChild(X),(Z=X.querySelector("#pvp-gc-back"))==null||Z.addEventListener("click",()=>X.remove()),(ae=X.querySelector("#pvp-gc-use"))==null||ae.addEventListener("click",()=>{X.remove(),ue(w,L)})}function H(){var j;const w=f[p+"Team"],L=Object.entries(w).flatMap(([T,N])=>(N||[]).filter(q=>!q.used).map(q=>({...q,_line:T})));if(!L.length)return;const A=document.createElement("div");A.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",A.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
      ...
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${L.map(T=>{const N={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[T._line]||"#555",q=Pe(T,T._line)+(T.boost||0);return`<div class="bp-item" data-cid="${T.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${N};overflow:hidden;cursor:pointer">
      ...
      </div>`,document.body.appendChild(A),(j=A.querySelector("#bp-close"))==null||j.addEventListener("click",()=>A.remove()),A.querySelectorAll(".bp-item").forEach(T=>{T.addEventListener("click",async()=>{const N=T.dataset.cid,q=L.find(O=>O.cardId===N);if(!q)return;const F=(w[q._line]||[]).find(O=>O.cardId===N);F&&(F.boost=(F.boost||0)+f.boostValue),A.remove(),await z({[p+"Team"]:w,boostUsed:!0})})})}function R(w=null){var ae,le;if(!(f.phase===p+"-attack")){g("Remplacement uniquement avant votre attaque","warning");return}const A=f[p+"Team"],j=f["usedSubIds_"+p]||[],T=f.maxSubs||3;if(j.length>=T){g(`Maximum ${T} remplacements atteint`,"warning");return}const N=Object.entries(A).flatMap(([ge,ce])=>(ce||[]).filter(ye=>ye.used).map(ye=>({...ye,_line:ge}))),q=(f[p+"Subs"]||[]).filter(ge=>!j.includes(ge.cardId));if(!N.length){g("Aucun joueur utilisé à remplacer","warning");return}if(!q.length){g("Aucun remplaçant disponible","warning");return}let F=Math.max(0,N.findIndex(ge=>ge.cardId===w));const O=((ae=N[F])==null?void 0:ae._line)||((le=N[F])==null?void 0:le.job);let X=Math.max(0,q.findIndex(ge=>ge.job===O)),ie=!1;const te=document.createElement("div");te.id="pvp-sub-overlay",te.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function Z(){var Ne,Le,Ve,Oe,nt,Kt;const ge=N[F],ce=q[X],ye=Math.min(130,Math.round((window.innerWidth-90)/2)),be=Math.round(ye*1.35),oe=De=>`background:rgba(255,255,255,0.12);border:none;color:${De?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${De?"default":"pointer"};flex-shrink:0`;te.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        <div id="pin-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
          ...
          <button id="pin-down" style="${oe(X>=q.length-1)}" ${X>=q.length-1?"disabled":""}>▼</button>
          <div style="font-size:10px;color:rgba(255,255,255,0.35)">${X+1}/${q.length}</div>
        </div>
        <div id="pout-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
          ...
          <button id="pout-down" style="${oe(F>=N.length-1)}" ${F>=N.length-1?"disabled":""}>▼</button>
          <div style="font-size:10px;color:rgba(255,255,255,0.35)">${F+1}/${N.length}</div>
        </div>
      </div>
      <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
        <button id="psub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
      </div>`,(Ne=te.querySelector("#psub-close"))==null||Ne.addEventListener("click",()=>te.remove()),(Le=te.querySelector("#pout-up"))==null||Le.addEventListener("click",()=>{F>0&&(F--,Z())}),(Ve=te.querySelector("#pout-down"))==null||Ve.addEventListener("click",()=>{F<N.length-1&&(F++,Z())}),(Oe=te.querySelector("#pin-up"))==null||Oe.addEventListener("click",()=>{X>0&&(X--,Z())}),(nt=te.querySelector("#pin-down"))==null||nt.addEventListener("click",()=>{X<q.length-1&&(X++,Z())});const Ee=(De,Ye,He,pt)=>{const ot=te.querySelector("#"+De);if(!ot)return;let Ct=0;ot.addEventListener("touchstart",ut=>{Ct=ut.touches[0].clientY},{passive:!0}),ot.addEventListener("touchend",ut=>{const kt=ut.changedTouches[0].clientY-Ct;if(Math.abs(kt)<30)return;const rt=Ye();kt<0&&rt<pt-1?(He(rt+1),Z()):kt>0&&rt>0&&(He(rt-1),Z())},{passive:!0})};Ee("pin-panel",()=>X,De=>X=De,q.length),Ee("pout-panel",()=>F,De=>F=De,N.length),(Kt=te.querySelector("#psub-confirm"))==null||Kt.addEventListener("click",async De=>{if(De.preventDefault(),De.stopPropagation(),ie)return;ie=!0;const Ye=N[F],He=q[X];if(!Ye||!He)return;const pt=Ye._line,ot=(A[pt]||[]).findIndex(kt=>kt.cardId===Ye.cardId);if(ot===-1){g("Erreur : joueur introuvable","error"),te.remove();return}const Ct={...He,_line:pt,position:Ye.position,used:!1,boost:0};A[pt].splice(ot,1,Ct);const ut=[...j,He.cardId];te.remove(),re(Ye,He,async()=>{await z({[p+"Team"]:A,[u+"Team"]:f[u+"Team"],["usedSubIds_"+p]:ut})})})}document.body.appendChild(te),Z()}function re(w,L,A){const j={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const N=(O,X,ie)=>`<div style="text-align:center">
      ...
      <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(T,1400)});await L("⚽ DERNIER CORNER","#FFD700"),await L("🧤 LE GARDIEN MONTE !","#4fc3f7"),w.remove()}async function ve(w,L){const{side:A,gk:j}=w;if(!j)return;const T=A+"Score",N=(L[T]||0)+1;j.used=!0;const q=[...L.log||[]];q.push({type:"duel",isGoal:!0,homeScored:A===p,homePlayers:A==="p1"?[qt(j)]:[],aiPlayers:A==="p2"?[qt(j)]:[],text:`⚽ DERNIER CORNER — Le gardien de ${L[A+"Name"]} marque !`});const F=(L.round||0)+1,O=A==="p1"?"p2":"p1",X={...L,[T]:N},ie=$e(X);P.add(F),await Te();const te=A===p?N:L[p+"Score"]||0,Z=A===p?L[u+"Score"]||0:N;Qe([qt(j)],te,Z,A===p,async()=>{await z({[T]:N,log:q,round:F,pendingAttack:null,phase:ie?"finished":O+"-attack",attacker:O,["selected_"+p]:[],modifiers:{...f.modifiers,[p]:{}}}),ie&&await qe({...f,[T]:N})})}async function Se(){const w=f[p+"Team"],L=!["GK","DEF","MIL","ATT"].some(N=>(f[u+"Team"][N]||[]).some(q=>!q.used)),A=(f["selected_"+p]||[]).map(N=>{const q=(w[N._role]||[]).find(le=>le.cardId===N.cardId)||N,F=["GK","DEF"].includes(N._role),O=w[N._role]||[],X=O.findIndex(le=>le.cardId===N.cardId),ie=yt(O.length),te=X>=0?ie[X]:q._col??1,Z=f.stadiumDef||f.homeStadiumDef||null,ae=q.stadiumBonus||Z&&(Z.club_id&&String(q.club_id)===String(Z.club_id)||Z.country_code&&q.country_code===Z.country_code)||!1;return{...q,_line:N._role,_col:te,stadiumBonus:ae,...F?{note_a:Math.max(1,Number(q.note_a)||0)}:{}}});if(!A.length)return;const j=oi(A,f.modifiers[p]||{},f[p+"Formation"]);fe(p,A.map(N=>N.cardId)),A.forEach(N=>{const q=(w[N._role]||[]).find(F=>F.cardId===N.cardId);q&&(q.used=!0)}),f["selected_"+p]=[],Y();const T=[...f.log||[]];if(L){if(A.length===1&&(A[0]._line==="GK"||A[0].job==="GK")&&(f.p1Score||0)===(f.p2Score||0)){await ve({side:p,gk:A[0]},{...f,[p+"Team"]:w,log:T});return}const q=(f[p+"Score"]||0)+1,F=A.map(Z=>qt(Z));T.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:F,homeTotal:j.total,aiTotal:0});const O=(f.round||0)+1,X=p==="p1"?"p2":"p1",ie={...f,[p+"Team"]:w,[p+"Score"]:q},te=$e(ie);P.add(O),Qe(F,q,f[u+"Score"]||0,!0,async()=>{await z({[p+"Team"]:w,[p+"Score"]:q,["selected_"+p]:[],modifiers:{...f.modifiers,[p]:{}},pendingAttack:null,phase:te?"finished":X+"-attack",attacker:X,round:O,log:T}),te&&await qe({...f,[p+"Score"]:q})});return}T.push({type:"pending",text:`⚔️ ${f[p+"Name"]} attaque (${j.total})`}),await z({[p+"Team"]:w,[u+"Team"]:f[u+"Team"],pendingAttack:{...j,players:A,side:p},["selected_"+p]:[],modifiers:{...f.modifiers,[p]:{}},phase:u+"-defense",log:T})}async function Ae(){const w=f[p+"Team"],L=(f["selected_"+p]||[]).map(ce=>{const ye=(w[ce._role]||[]).find(Le=>Le.cardId===ce.cardId)||ce,be=w[ce._role]||[],oe=be.findIndex(Le=>Le.cardId===ce.cardId),Ee=yt(be.length),Ne=oe>=0?Ee[oe]:ye._col??1;return{...ye,_line:ce._role,_col:Ne}}),A=ri(L,f.modifiers[p]||{},f[p+"Formation"]);fe(p,L.map(ce=>ce.cardId)),L.forEach(ce=>{const ye=(w[ce._role]||[]).find(be=>be.cardId===ce.cardId);ye&&(ye.used=!0)}),f["selected_"+p]=[],Y();const j=Xi(f.pendingAttack.total,A.total,f.modifiers[p]||{}),T=f.pendingAttack.side,N=j==="attack"||(j==null?void 0:j.goal),q=T==="p1"?"p2":"p1",F=(f.round||0)+1,O=(f.pendingAttack.players||[]).map(ce=>qt(ce)),X=[...f.log||[]];X.push({type:"duel",isGoal:N,homeScored:N&&T===p,text:N?`⚽ BUT de ${f[T+"Name"]} ! (${f.pendingAttack.total} vs ${A.total})`:`✋ Attaque stoppée (${f.pendingAttack.total} vs ${A.total})`,homePlayers:O,aiPlayers:L.map(ce=>qt(ce)),homeTotal:f.pendingAttack.total,aiTotal:A.total});const ie=N?(f[T+"Score"]||0)+1:f[T+"Score"]||0,te={...f,[p+"Team"]:w,[T+"Score"]:ie},Z=$e(te),ae=Z?"finished":q+"-attack",le=!Z&&!N?he(te):null,ge=async()=>{if(le){await ve(le,te);return}await z({[p+"Team"]:w,[u+"Team"]:f[u+"Team"],[T+"Score"]:ie,["selected_"+p]:[],modifiers:{...f.modifiers,[p]:{}},pendingAttack:null,phase:ae,attacker:q,round:F,log:X}),(ae==="finished"||Z)&&await qe({...f,[T+"Score"]:ie})};if(N){const ce=T===p,ye=ce?ie:f[p+"Score"]||0,be=ce?f[u+"Score"]||0:ie;P.add(F),Qe(O,ye,be,ce,ge)}else await ge()}function Be(w){return["MIL","ATT"].some(L=>(w[L]||[]).some(A=>!A.used))}function ke(w){return["GK","DEF","MIL","ATT"].some(L=>(w[L]||[]).some(A=>!A.used))}function wt(w){return ke(w)&&!Be(w)}function ct(w){const L=f[w+"Team"],A=f[(w==="p1"?"p2":"p1")+"Team"];return!!(Be(L)||wt(L)&&!ke(A)||wt(L)&&!Be(A)&&ke(A))}function $e(w){return!ke(w.p1Team)&&!ke(w.p2Team)}function Ce(w){const L=w.p1Score||0,A=w.p2Score||0;return L===A?null:L>A?v.home_id:v.away_id}async function qe(w){Gt(),Ue();try{const L=Ce(w),A=L?v.home_id===L?v.away_id:v.home_id:null,j=w.p1Score||0,T=w.p2Score||0,{data:N,error:q}=await _.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:L,p_home_score:j,p_away_score:T,p_forfeit:!1});if(q&&console.error("[PvP] finish_pvp_match:",q),l)try{await _.rpc("check_and_finish_mini_league",{p_league_id:l})}catch(F){console.error("[PvP] check_and_finish_mini_league:",F)}L&&A&&Lr(L,A).catch(()=>{});try{const{data:F}=await _.rpc("apply_match_rewards",{p_match_id:i});F!=null&&F.success&&!(F!=null&&F.skipped)&&typeof t.refreshProfile=="function"&&await t.refreshProfile()}catch(F){console.error("[PvP] apply_match_rewards:",F)}if(typeof c=="function")try{await c({homeScore:j,awayScore:T,winnerId:L,homeId:v.home_id,awayId:v.away_id,matchId:i})}catch(F){console.error("[PvP] onMatchEnd:",F)}}catch(L){console.error("[PvP] finishMatch:",L)}}function Ge(){var T;const w=f[p+"Score"],L=f[u+"Score"],A=w>L,j=w===L;st(e),e.innerHTML=`
    ...
    </div>`,(T=document.getElementById("pvp-home"))==null||T.addEventListener("click",()=>{try{_.removeChannel(ee)}catch{}it(e),l?y("mini-league",{openLeagueId:l}):y("home")})}Y()}async function Pi(e,t,i=!1){var s,d;const{state:n,navigate:r,toast:o}=t,a=i&&((d=(s=t==null?void 0:t.state)==null?void 0:s.params)==null?void 0:d.rankedData)||null;await wi(e,t,i?"ranked":"random",async({deckId:c,formation:l,starters:m,subsRaw:b,gcCardsEnriched:y,gcDefs:g,stadiumDef:p})=>{st(e);const u=async x=>{await Sr(e,t,c,l,m,b,x||[],g,p,i,a)};if(!(y!=null&&y.length)){await u([]);return}hi(e,y,u)})}async function Ir(e,t,i){return Tr(e,t,i)}async function Sr(e,t,i,n,r,o,a=[],s=[],d=null,c=!1,l=null){var ne,ee;const{state:m,navigate:b,toast:y}=t,g=(l==null?void 0:l.mmr)??((ne=m.profile)==null?void 0:ne.mmr)??1e3,p=c?Hi(g):null,u=c?"linear-gradient(135deg, #1a0a2e 0%, #2d1060 50%, #1a0a2e 100%)":"linear-gradient(135deg, #0a1a2e 0%, #0d3d1e 100%)",x=c?(p==null?void 0:p.color)||"#D4A017":"#FFD700";let h=!1,v=null,$=null;e.innerHTML=`
    ...
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:${Jt};border:1px solid ${Qt};border-radius:12px;padding:16px">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>${b.map((z,I)=>{const K=g>0&&a.status==="finished"?I===0?Math.floor(g*.7):I===1?Math.floor(g*.2):I===2?Math.floor(g*.1):0:0;return`<tr style="border-bottom:1px solid ${Lt};${z.userId===o?"background:rgba(74,222,128,0.08);":""}">
      ...
      <div style="background:${Jt};border:1px solid ${Qt};border-radius:12px;padding:16px">
        ...
        ${Array.from({length:Math.max(0,a.status==="active"?a.current_day-1:a.current_day)},(z,I)=>I+1).reverse().map(z=>{const I=(d||[]).filter(K=>K.matchday===z);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:${Ze};margin-bottom:6px">Journée ${z}</div>${I.map(K=>kn(K,m,o,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
  ...
  </div>`;let d="buy";const c=()=>{var p,u,x,h,v,$,f;return{name:(((p=document.getElementById("flt-name"))==null?void 0:p.value)||"").toLowerCase().trim(),club:(((u=document.getElementById("flt-club"))==null?void 0:u.value)||"").toLowerCase().trim(),country:(((x=document.getElementById("flt-country"))==null?void 0:x.value)||"").toLowerCase().trim(),job:((h=document.getElementById("flt-job"))==null?void 0:h.value)||"",rarity:((v=document.getElementById("flt-rarity"))==null?void 0:v.value)||"",note1:parseInt(($=document.getElementById("flt-note1"))==null?void 0:$.value)||0,note2:parseInt((f=document.getElementById("flt-note2"))==null?void 0:f.value)||0}};function l(p){const u=c();return p.filter(x=>{var P,J,ne;const h=(P=x.card)==null?void 0:P.player;if(!h)return!1;const v=`${h.firstname} ${h.surname_real}`.toLowerCase(),$=(((J=h.clubs)==null?void 0:J.encoded_name)||"").toLowerCase(),f=(h.country_code||"").toLowerCase(),M=((ne=x.card)==null?void 0:ne.evolution_bonus)||0,k=Dt(h,h.job,M),S=h.job2?Dt(h,h.job2,M):0;return!(u.name&&!v.includes(u.name)||u.club&&!$.includes(u.club)||u.country&&!f.includes(u.country)||u.job&&h.job!==u.job||u.rarity&&h.rarity!==u.rarity||u.note1&&k<u.note1||u.note2&&S<u.note2)})}function m(p){var S,P,J,ne;const u=(S=p.card)==null?void 0:S.player;if(!u)return"";const x=((P=p.card)==null?void 0:P.evolution_bonus)||0,h=Dt(u,u.job,x),v=u.job2?Dt(u,u.job2,x):0,$=(i.profile.credits||0)>=p.price,f=u.country_code?`https://flagsapi.com/${u.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,M=ui[u.job]||"#bbb",k=u.job2?ui[u.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden">
      <!-- Drapeau -->
    ...
    </div>`}function b(p){var S,P,J,ne;const u=(S=p.card)==null?void 0:S.player;if(!u)return"";const x=((P=p.card)==null?void 0:P.evolution_bonus)||0,h=Dt(u,u.job,x),v=u.job2?Dt(u,u.job2,x):0,$=p.status==="sold",f=u.country_code?`https://flagsapi.com/${u.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,M=ui[u.job]||"#bbb",k=u.job2?ui[u.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${$?"opacity:0.7":""}">
      ${f?`<img src="${f}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      ${(J=u.clubs)!=null&&J.logo_url?`<img src="${u.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${M};display:flex;align-items:center;justify-content:center">
    ...
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center">
      ...
      <div style="display:flex;gap:10px">
        ...
        <button id="buy-ok" style="flex:1;padding:12px;border-radius:10px;border:none;background:var(--green);color:#fff;font-size:14px;font-weight:900;cursor:pointer">Confirmer</button>
          ...
          <div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${z?"border:2px solid var(--yellow)":""}">
            ...
            <div style="text-align:right;flex-shrink:0">
              <div style="font-size:20px">${I?"❓":J.emoji}</div>
              <div style="font-size:11px;font-weight:700;color:${J.color}">${I?"Placement":J.label}</div>
              ${I?"":`<div style="font-size:10px;color:var(--tile-fg-dim)">${ee}% WR</div>`}
            </div>
          </div>`}).join(""):'<div style="text-align:center;color:var(--tile-fg-dim);padding:40px">Aucun joueur classé.</div>'}else if(d==="mini-league"){const k=a||[];v.innerHTML=k.length>0?k.map((S,P)=>`
        <div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${S.user_id===i.profile.id?"border:2px solid var(--yellow)":""}">
          <div style="width:32px;height:32px;border-radius:50%;background:${c(P)};color:${l(P)};display:flex;align-items:center;justify-content:center;font-weight:900;flex-shrink:0;font-size:${P<3?"16":"13"}px">${P<3?["🥇","🥈","🥉"][P]:P+1}</div>
          <div style="flex:1;min-width:0">
      ...
      `).join(""):`<div style="text-align:center;color:var(--tile-fg-dim);padding:40px">Aucun résultat de Mini League pour l'instant.</div>`}else{const k=s||[];v.innerHTML=k.length>0?k.map((S,P)=>`
        <div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${S.user_id===i.profile.id?"border:2px solid var(--yellow)":""}">
          <div style="width:32px;height:32px;border-radius:50%;background:${c(P)};color:${l(P)};display:flex;align-items:center;justify-content:center;font-weight:900;flex-shrink:0;font-size:${P<3?"16":"13"}px">${P<3?["🥇","🥈","🥉"][P]:P+1}</div>
          <div style="flex:1;min-width:0">
            <div style="font-weight:700;color:var(--tile-fg-on-page);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${S.pseudo}</div>
  ...
  <div class="page">
    ...
    </div>`,(M=document.getElementById("ranked-back"))==null||M.addEventListener("click",()=>n("home"));return}const d=s.mmr??1e3,c=s.mmr_deviation??350,l=s.mmr_volatility??.06,m=s.placement_matches??0,b=m<10,y=Math.max(0,10-m),g=Hi(d),p=ao(d),u=Li.findIndex(J=>J.id===g.id),x=Li[u+1]||null,h={bronze:"linear-gradient(160deg,#3d1c00,#7a3e00)",silver:"linear-gradient(160deg,#1a1a2e,#3a3a5e)",gold:"linear-gradient(160deg,#1a1200,#4a3500)",platinum:"linear-gradient(160deg,#001a20,#003040)",diamond:"linear-gradient(160deg,#001030,#1a2860)",master:"linear-gradient(160deg,#1a0030,#3d0070)"},v=(s.ranked_wins||0)+(s.ranked_losses||0)+(s.ranked_draws||0),$=v>0?Math.round((s.ranked_wins||0)/v*100):0,f=Li.map(J=>`
  ...
  <div class="page">
    ...
    <nav class="top-nav">
      <div class="logo" id="nav-logo" title="Manager Wars v2026.07.23-1159" style="cursor:pointer">
