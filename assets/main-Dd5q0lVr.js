import{s as $,F as Xe,b as Te,c as jt,l as ge,d as ri}from"./formation-links-CDBKB_z4.js";function vt(e,{navigate:t,toast:i}){e.innerHTML=`
  ...
  </div>`}function $t(e){const t=e.job||"ATT",i=ke(e,t),s=tt[e.country_code]||e.country_code||"";return`
  <div style="
    width:140px;border-radius:12px;padding:6px;
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),n=(d||[]).filter(_=>_.card_type==="player"&&_.player),r=(d||[]).filter(_=>_.card_type==="game_changer"),p=(d||[]).filter(_=>_.card_type==="formation"),{data:g}=await $.from("gc_definitions").select("name,gc_type,color,effect,image_url"),f={};(g||[]).forEach(_=>{f[_.name]=_});const y=Object.keys(Xe),u=Object.keys(de),h={};n.forEach(_=>{const b=_.player.id;h[b]=(h[b]||0)+1}),new Set(Object.keys(h).map(_=>String(_)));const T=new Set(p.map(_=>_.formation)),v=new Set(r.map(_=>_.gc_type));let x="player",k="Tous",F="",m=!1;function A(){return[...n].sort((_,b)=>{const C=ze.indexOf(_.player.job),E=ze.indexOf(b.player.job);return C!==E?C-E:(_.player.surname_encoded||"").localeCompare(b.player.surname_encoded||"")})}function M(){return[...l||[]].sort((_,b)=>{const C=ze.indexOf(_.job),E=ze.indexOf(b.job);return C!==E?C-E:(_.surname_encoded||"").localeCompare(b.surname_encoded||"")})}function R(){return A().filter(_=>{const b=_.player,C=k==="Tous"||b.job===k,E=!F||`${b.firstname} ${b.surname_encoded}`.toLowerCase().includes(F);return C&&E})}function G(){return M().filter(_=>{const b=k==="Tous"||_.job===k,C=!F||`${_.firstname} ${_.surname_encoded}`.toLowerCase().includes(F);return b&&C})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
      ...
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
        ...
        <div style="display:flex;justify-content:flex-end">
          ...
          <div style="height:150px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
            ${me?`<img src="${me}" style="max-width:120px;max-height:120px;object-fit:contain;border-radius:6px">`:`<span style="font-size:64px">${B.icon}</span>`}
          </div>
    ...
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      ...
      <div style="background:#f9f9f9;border-radius:10px;padding:12px;display:flex;justify-content:space-between;align-items:center">
        <div>
          ...
          <div style="font-size:18px;font-weight:900;color:var(--yellow)">${we.toLocaleString("fr")} crédits</div>
    ...
    <div style="margin-top:12px;border-top:1px solid var(--gray-200);padding-top:12px">
      ...
      <div style="display:flex;gap:8px">
        <input type="number" id="sell-price-form" min="1" placeholder="Prix en crédits" value="${we}"
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(u=document.getElementById("direct-sell-form-btn"))==null||u.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${n} pour ${we.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const v=g.find(k=>!k.is_for_sale)||g[0];if(!v){o("Aucune carte à vendre","error");return}const{error:x}=await $.from("cards").delete().eq("id",v.id);if(x){o(x.message,"error");return}await $.from("users").update({credits:(c.profile.credits||0)+we}).eq("id",c.profile.id),await l(),o(`+${we.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),a(),d("collection")}),(h=document.getElementById("market-sell-form-btn"))==null||h.addEventListener("click",async()=>{const v=parseInt(document.getElementById("sell-price-form").value);if(!v||v<1){o("Prix invalide","error");return}await $.from("cards").update({is_for_sale:!0,sale_price:v}).eq("id",e.id),await $.from("market_listings").insert({seller_id:c.profile.id,card_id:e.id,price:v}),o("Carte mise en vente sur le marché !","success"),a(),d("collection")}),(T=document.getElementById("cancel-sell-form-btn"))==null||T.addEventListener("click",async()=>{await $.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await $.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),o("Annonce retirée","success"),a(),d("collection")})}async function kt(e,t,i,s){var M,R,G,O,V,Z;const{state:c,toast:o,openModal:a,closeModal:d,navigate:l,refreshProfile:n}=s,r=e.player,p=t.filter(Y=>Y.player.id===r.id),g=p.length,f=ui[r.rarity]||1e3,y=r.rarity!=="legende",u=Gt(r),h=ke(r,r.job),T=r.job2?ke(r,r.job2):null,v=Fe[r.job]||"#1A6B3C",x=r.job2?Fe[r.job2]:null,k=Dt[r.rarity]||"#ccc",F=tt[r.country_code]||r.country_code||"",{data:m}=await $.from("transfer_history").select("club_name, manager_name, source, price, transferred_at").eq("player_id",r.id).order("transferred_at",{ascending:!0}),A=m&&m.length?`
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
    ...
    </div>`:""}`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(G=document.getElementById("close-detail"))==null||G.addEventListener("click",d),(O=document.getElementById("direct-sell-btn"))==null||O.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte ${r.surname_encoded} pour ${f.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const Y=p.find(L=>!L.is_for_sale)||p[0];if(!Y){o("Aucune carte à vendre","error");return}const{error:U}=await $.from("cards").delete().eq("id",Y.id);if(U){o(U.message,"error");return}await $.from("users").update({credits:(c.profile.credits||0)+f}).eq("id",c.profile.id),await n(),o(`+${f.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),d(),l("collection")}),(V=document.getElementById("market-sell-btn"))==null||V.addEventListener("click",async()=>{const Y=parseInt(document.getElementById("sell-price").value);if(!Y||Y<1){o("Prix invalide","error");return}await $.from("cards").update({is_for_sale:!0,sale_price:Y}).eq("id",e.id),await $.from("market_listings").insert({seller_id:c.profile.id,card_id:e.id,price:Y}),o("Carte mise en vente sur le marché !","success"),d(),l("collection")}),(Z=document.getElementById("cancel-sell-btn"))==null||Z.addEventListener("click",async()=>{await $.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await $.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),o("Annonce retirée","success"),d(),l("collection")})}const De={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},pe={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function Ne(e){const t=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function Rt(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function qt(e){var i;const t="https://fcnwclxlkytdfjotqkta.supabase.co";return(i=e==null?void 0:e.clubs)!=null&&i.logo_url?e.clubs.logo_url.startsWith("http")?e.clubs.logo_url:`${t}/storage/v1/object/public/assets/clubs/${e.clubs.logo_url}`:null}function mi(e,t=44,i=58){var f;const s=e?Ne(e):null,c=e?qt(e):null,o=Rt(e==null?void 0:e.country_code),a=(e==null?void 0:e.job)||"MIL",d=pe[a]||"#555",l={normal:"#aaa",pépite:"#D4A017",papyte:"#222",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",n=Number(a==="GK"?e==null?void 0:e.note_g:a==="DEF"?e==null?void 0:e.note_d:a==="MIL"?e==null?void 0:e.note_m:e==null?void 0:e.note_a)||0,r=Math.round(i*.19),p=Math.round(i*.25),g=i-r-p;return e?`<div style="width:${t}px;height:${i}px;border-radius:5px;border:2px solid ${l};background:${d};position:relative;overflow:hidden;flex-shrink:0">
  ...
  </div>`,yi(e,t,c,i),document.getElementById("builder-back").addEventListener("click",()=>s("decks")),document.getElementById("formation-select").addEventListener("change",n=>{t.formation=n.target.value;const r=At(t.formation),p={};r.forEach(g=>{t.slots[g]&&(p[g]=t.slots[g])}),t.slots=p,be(e,t,i)}),document.getElementById("save-deck").addEventListener("click",()=>bi(t,i)),e.querySelectorAll("[data-remove-sub]").forEach(n=>{n.addEventListener("click",()=>{t.subs=t.subs.filter(r=>r!==n.dataset.removeSub),be(e,t,i)})}),(l=document.getElementById("add-sub-btn"))==null||l.addEventListener("click",()=>{vi(t,e,i)})}function yi(e,t,i,s){const c=e.querySelector("#deck-field");if(!c)return;const o=Te[t.formation]||{},a=jt(t.formation)||[],d={};for(const T of i){const v=t.slots[T],x=v?t.playerCards.find(k=>k.id===v):null;d[T]=x?x.player:null}const l=300,n=300,r=48,p=64,g=13,f=16,y=38;function u(T){const v=o[T];return v?{x:v.x*l,y:v.y*n}:null}let h="";for(const[T,v]of a){const x=u(T),k=u(v);if(!x||!k)continue;const F=d[T]?{...d[T],club_id:d[T].club_id,country_code:d[T].country_code,rarity:d[T].rarity}:null,m=d[v]?{...d[v],club_id:d[v].club_id,country_code:d[v].country_code,rarity:d[v].rarity}:null,A=ge(F,m);A==="#ff3333"||A==="#cc2222"?h+=`<line x1="${x.x.toFixed(1)}" y1="${x.y.toFixed(1)}" x2="${k.x.toFixed(1)}" y2="${k.y.toFixed(1)}" stroke="${A}" stroke-width="2.5" stroke-linecap="round" opacity="0.4"/>`:(h+=`<line x1="${x.x.toFixed(1)}" y1="${x.y.toFixed(1)}" x2="${k.x.toFixed(1)}" y2="${k.y.toFixed(1)}" stroke="${A}" stroke-width="8" stroke-linecap="round" opacity="0.2"/>`,h+=`<line x1="${x.x.toFixed(1)}" y1="${x.y.toFixed(1)}" x2="${k.x.toFixed(1)}" y2="${k.y.toFixed(1)}" stroke="${A}" stroke-width="2.5" stroke-linecap="round" opacity="0.9"/>`)}for(const T of i){const v=u(T);if(!v)continue;const x=d[T],k=T.replace(/\d+/,""),F=pe[k]||"#555",m=(v.x-r/2).toFixed(1),A=(v.y-p/2).toFixed(1),M={normal:"#aaa",pépite:"#D4A017",papyte:"#222",légende:"#7a28b8"}[x==null?void 0:x.rarity]||"#aaa";if(x){const R=Ne(x),G=qt(x),O=Rt(x.country_code),V=Number(k==="GK"?x.note_g:k==="DEF"?x.note_d:k==="MIL"?x.note_m:x.note_a)||0,Z=p-g-f;h+=`<defs><clipPath id="dcp-${T}"><rect x="${m}" y="${(v.y-p/2+g).toFixed(1)}" width="${r}" height="${Z}"/></clipPath></defs>`,h+=`<rect x="${m}" y="${A}" width="${r}" height="${p}" rx="5" fill="${F}"/>`,R&&(h+=`<image href="${R}" x="${m}" y="${(v.y-p/2+g).toFixed(1)}" width="${r}" height="${Z}" clip-path="url(#dcp-${T})" preserveAspectRatio="xMidYMin slice"/>`),h+=`<rect x="${m}" y="${A}" width="${r}" height="${g}" fill="rgba(255,255,255,0.93)"/>`,h+=`<text x="${v.x.toFixed(1)}" y="${(v.y-p/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(x.surname_encoded||"").slice(0,9)}</text>`;const Y=(v.y+p/2-f).toFixed(1);h+=`<rect x="${m}" y="${Y}" width="${r}" height="${f}" fill="rgba(255,255,255,0.93)"/>`,O&&(h+=`<image href="${O}" x="${(v.x-21).toFixed(1)}" y="${(v.y+p/2-f+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`),h+=`<text x="${v.x.toFixed(1)}" y="${(v.y+p/2-f/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${V}</text>`,G&&(h+=`<image href="${G}" x="${(v.x+r/2-14).toFixed(1)}" y="${(v.y+p/2-f+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`),h+=`<rect x="${m}" y="${A}" width="${r}" height="${p}" rx="5" fill="none" stroke="${M}" stroke-width="2"/>`}else h+=`<rect x="${m}" y="${A}" width="${r}" height="${p}" rx="5" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="5,3"/>`,h+=`<text x="${v.x.toFixed(1)}" y="${v.y.toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="22" fill="rgba(255,255,255,0.35)">+</text>`,h+=`<text x="${v.x.toFixed(1)}" y="${(v.y+16).toFixed(1)}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.3)">${k}</text>`;h+=`<rect x="${m}" y="${A}" width="${r}" height="${p}" rx="5" fill="rgba(0,0,0,0.01)" class="deck-slot-hit" data-pos="${T}" style="cursor:pointer"/>`}c.innerHTML=`<svg viewBox="${-y} ${-y} ${l+y*2} ${n+y*2}" width="100%" style="display:block;max-width:440px;margin:0 auto">${h}</svg>`,c.querySelectorAll(".deck-slot-hit").forEach(T=>{T.addEventListener("click",()=>hi(T.dataset.pos,t,e,s))})}function hi(e,t,i,s){var g,f,y;const{openModal:c,closeModal:o}=s,a=e.replace(/\d+/,""),d=t.slots[e],l=d?t.playerCards.find(u=>u.id===d):null;(g=l==null?void 0:l.player)==null||g.id;const n=new Set;Object.entries(t.slots).forEach(([u,h])=>{var v;if(u===e||!h)return;const T=t.playerCards.find(x=>x.id===h);(v=T==null?void 0:T.player)!=null&&v.id&&n.add(T.player.id)}),t.subs.forEach(u=>{var T;const h=t.playerCards.find(v=>v.id===u);(T=h==null?void 0:h.player)!=null&&T.id&&n.add(h.player.id)});const r=new Set,p=t.playerCards.filter(u=>{const h=u.player;return!(h.job===a||h.job2===a)||n.has(h.id)||r.has(h.id)?!1:(r.add(h.id),!0)});p.sort((u,h)=>{const T=a==="GK"?u.player.note_g:a==="DEF"?u.player.note_d:a==="MIL"?u.player.note_m:u.player.note_a;return(a==="GK"?h.player.note_g:a==="DEF"?h.player.note_d:a==="MIL"?h.player.note_m:h.player.note_a)-T}),c(`Choisir ${a} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${p.length>0?p.map(u=>{var k,F;const h=u.player,T=a==="GK"?h.note_g:a==="DEF"?h.note_d:a==="MIL"?h.note_m:h.note_a,v=Ne(h),x={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[h.rarity];return`<div class="player-option" data-card-id="${u.id}"
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}const Oi="simulation",Pt="VOTRE_ZONE_ID";function Pi(){switch(Oi){case"propellerads":return Hi();case"adsense":return Ki();default:return it()}}function it(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:#111;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:14px";let i=5;t.innerHTML=`
    ...
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div id="deck-swipe-zone" style="flex:1;min-height:0;overflow:hidden;position:relative;touch-action:pan-y;display:flex;align-items:center;justify-content:center;padding:4px">
        ${u?`<div class="deck-preview-wrap" style="aspect-ratio:1/1;max-width:100%;max-height:100%;width:auto;height:100%;overflow:hidden">${Ae(u,y,null,[],285,285)}</div>`:`<div style="display:flex;align-items:center;justify-content:center;height:100%;opacity:.4;flex-direction:column;gap:8px">
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;height:${p}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ...
      ${c?`<img src="${c}" style="width:${p-4}px;height:${p-4}px;object-fit:contain">`:e!=null&&e.clubName?`<span style="font-size:${Math.max(4,p-8)}px;font-weight:700;color:#333">${(e.clubName||"").slice(0,3).toUpperCase()}</span>`:""}
  ...
  </div>`}function $e(e,t,i){if(!(e!=null&&e.length))return"";const s=e.slice(0,5);let c='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return s.forEach((o,a)=>{if(c+=fe(o,40,52),a<s.length-1){const d=ge(o,s[a+1]);c+=`<div style="width:7px;height:3px;background:${d==="#ff3333"||d==="#cc2222"?"rgba(255,255,255,0.12)":d};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(c+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),c+="</div>",c}function Ve(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function Wt(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function Ee(e,t,i,s,c=310,o=310){const a=Te[t]||{},d=jt(t)||Xe[t]||[],l={},n=["ATT","MIL","DEF","GK"];for(const v of n)(e[v]||[]).forEach((k,F)=>{l[`${v}${F+1}`]=k});function r(v){const x=a[v];return x?{x:x.x*c,y:x.y*o}:null}let p="";for(const[v,x]of d){const k=r(v),F=r(x);if(!k||!F)continue;const m=l[v],A=l[x],M=ge(m,A);M==="#00ff88"||M==="#FFD700"?(p+=`<line x1="${k.x.toFixed(1)}" y1="${k.y.toFixed(1)}" x2="${F.x.toFixed(1)}" y2="${F.y.toFixed(1)}"
        ...
        stroke="${M}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const g=48,f=64,y=13,u=16,h={normal:"#aaaaaa",pépite:"#D4A017",papyte:"#111111",légende:"#7a28b8"};for(const[v,x]of Object.entries(l)){const k=r(v);if(!k||!x)continue;const F=v.replace(/[0-9]/g,""),m=Ht[F]||"#555",A=i==="attack"&&["MIL","ATT"].includes(F)&&!x.used||i==="defense"&&["GK","DEF","MIL"].includes(F)&&!x.used,M=s.includes(x.cardId);let R;i==="attack"?R=F==="MIL"?Number(x.note_m)||0:Number(x.note_a)||0:i==="defense"?R=F==="GK"?Number(x.note_g)||0:F==="MIL"?Number(x.note_m)||0:Number(x.note_d)||0:R=Number(F==="GK"?x.note_g:F==="DEF"?x.note_d:F==="MIL"?x.note_m:x.note_a)||0,R=R+(x.boost||0);const G=(k.x-g/2).toFixed(1),O=(k.y-f/2).toFixed(1),V=x.used?.25:1,Z=h[x==null?void 0:x.rarity]||h.normal,Y=M?"#ff3030":Z,U=M?3:(x==null?void 0:x.rarity)==="légende"||(x==null?void 0:x.rarity)==="pépite"?2.5:2,L=f-y-u;p+=`<defs><clipPath id="cp-${v}"><rect x="${G}" y="${(k.y-f/2+y).toFixed(1)}" width="${g}" height="${L}"/></clipPath></defs>`,p+=`<rect x="${G}" y="${O}" width="${g}" height="${f}" rx="5" fill="${m}" opacity="${V}"/>`;const S=oe(x);S&&!x.used&&(p+=`<image href="${S}" x="${G}" y="${(k.y-f/2+y).toFixed(1)}" width="${g}" height="${L}" clip-path="url(#cp-${v})" preserveAspectRatio="xMidYMin slice"/>`),p+=`<rect x="${G}" y="${O}" width="${g}" height="${y}" rx="4" fill="rgba(255,255,255,0.92)"/>`,p+=`<text x="${k.x.toFixed(1)}" y="${(k.y-f/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(x.name||"").slice(0,9)}</text>`;const z=(k.y+f/2-u).toFixed(1);if(p+=`<rect x="${G}" y="${z}" width="${g}" height="${u}" fill="rgba(255,255,255,0.92)"/>`,x.used)p+=`<text x="${k.x.toFixed(1)}" y="${(k.y+f/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="13" font-weight="900" fill="rgba(0,0,0,0.4)" font-family="Arial Black">–</text>`;else{const N=Ve(x.country_code);N?p+=`<image href="${N}" x="${(k.x-20).toFixed(1)}" y="${(k.y+f/2-u+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:p+=`<text x="${(k.x-13).toFixed(1)}" y="${(k.y+f/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${Wt(x.country_code)}</text>`,p+=`<text x="${k.x.toFixed(1)}" y="${(k.y+f/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${R}</text>`;const D=Yt(x);D?p+=`<image href="${D}" x="${(k.x+g/2-14).toFixed(1)}" y="${(k.y+f/2-u+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:x.clubName&&(p+=`<text x="${(k.x+14).toFixed(1)}" y="${(k.y+f/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(x.clubName||"").slice(0,3).toUpperCase()}</text>`),x.boost&&(p+=`<rect x="${(k.x+g/2-12).toFixed(1)}" y="${(k.y-f/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,p+=`<text x="${(k.x+g/2-5).toFixed(1)}" y="${(k.y-f/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${x.boost}</text>`)}p+=`<rect x="${G}" y="${O}" width="${g}" height="${f}" rx="5" fill="${M?"rgba(255,255,255,0.12)":"none"}" stroke="${Y}" stroke-width="${U}" opacity="${V}"/>`,A&&(p+=`<rect x="${G}" y="${O}" width="${g}" height="${f}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${M?"selected":""}" data-card-id="${x.cardId}" data-role="${F}" style="cursor:pointer"/>`)}const T=38;return`<svg viewBox="${-T} ${-T} ${c+T*2} ${o+T*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${p}
  </svg>`}function Ae(e,t,i,s,c=300,o=300){return`<div id="match-terrain-wrap" style="position:relative;padding:0 4px">
    ${Ee(e,t,i,s,c,o)}
  ...
  <div style="text-align:center;flex-shrink:0;width:38px">
    ...
    <div style="font-size:7px;color:rgba(255,255,255,0.5);margin-top:1px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${(e.name||"").slice(0,7)}</div>
          ...
          card:cards(id, card_type, formation,
            player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
              clubs(encoded_name,logo_url)))`).eq("deck_id",l).order("slot_order")]);n=m[0].data,p=m[0].error,r=m[1].data,g=m[1].error}catch(m){console.error("[Match] Exception chargement deck:",m),ue(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>o("home"));return}if(p||g){console.error("[Match] Erreur Supabase:",p||g),ue(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>o("home"));return}const f=(r||[]).filter(m=>{var A;return m.is_starter&&((A=m.card)==null?void 0:A.player)}).map(m=>Ue(m.card,m.position)),y=(r||[]).filter(m=>{var A;return!m.is_starter&&((A=m.card)==null?void 0:A.player)}).map(m=>Ue(m.card,m.position));if(f.length<11){ue(e,"⚠️",`Deck incomplet (${f.length}/11).`,"Compléter",()=>o("decks"));return}const u=(r||[]).find(m=>{var A;return((A=m.card)==null?void 0:A.card_type)==="formation"}),h=(n==null?void 0:n.formation)||((F=u==null?void 0:u.card)==null?void 0:F.formation)||"4-4-2",{data:T,error:v}=await $.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",c.profile.id).eq("card_type","game_changer"),{data:x}=await $.from("gc_definitions").select("*").eq("is_active",!0),k=(T||[]).map(m=>({...m,_gcDef:(x==null?void 0:x.find(A=>A.name===m.gc_type||A.id===m.gc_definition_id))||null}));s({deckId:l,formation:h,starters:f,subsRaw:y,gcCardsEnriched:k,gcDefs:x||[]})}async function Vi(e,t){const{state:i}=t,c=(i.params||{}).matchMode||"vs_ai_easy",o=c.replace("vs_ai_",""),a=c;await Jt(e,t,c,async({deckId:d,formation:l,starters:n,subsRaw:r,gcCardsEnriched:p,gcDefs:g})=>{try{const f=Re(n,l),y=await Yi(l,o),u=async h=>{try{const{data:T,error:v}=await $.from("matches").insert({home_id:i.profile.id,away_id:null,mode:a,home_deck_id:d,status:"in_progress"}).select().single();if(v){console.error("[MatchIA] Erreur création match:",v),ue(e,"⚠️","Impossible de créer le match ("+v.message+").","Retour",()=>t.navigate("home"));return}const x={gcDefs:g||[],matchId:T==null?void 0:T.id,mode:a,difficulty:o,formation:l,homeTeam:f,aiTeam:y,homeSubs:r,subsUsed:0,maxSubs:Math.min(r.length,3),homeScore:0,aiScore:0,gcCards:h,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Ji(e,x,t)}catch(T){console.error("[MatchIA] Exception launchMatch:",T),ue(e,"⚠️","Erreur au lancement du match : "+T.message,"Retour",()=>t.navigate("home"))}};if(!p.length){u([]);return}Vt(e,p,u)}catch(f){console.error("[MatchIA] Exception setup:",f),ue(e,"⚠️","Erreur de préparation du match : "+f.message,"Retour",()=>t.navigate("home"))}})}async function Yi(e,t){var a,d;const{data:i}=await $.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(60);if(!i||i.length<11)return Wi(e);const s=FORMATIONS[e]||FORMATIONS["4-4-2"],c={GK:[],DEF:[],MIL:[],ATT:[]},o=[...i];for(const l of["GK","DEF","MIL","ATT"]){const n=o.filter(y=>y.job===l),r=o.filter(y=>y.job!==l),p=[...n,...r],g=[];for(let y=0;y<s[l];y++){const u=p[y]||o[y];u&&g.push({cardId:"ai-"+u.id+"-"+y,id:u.id,firstname:u.firstname,name:u.surname_encoded,country_code:u.country_code,club_id:u.club_id,job:u.job,job2:u.job2,note_g:Number(u.note_g)||0,note_d:Number(u.note_d)||0,note_m:Number(u.note_m)||0,note_a:Number(u.note_a)||0,rarity:u.rarity,skin:u.skin,hair:u.hair,hair_length:u.hair_length,clubName:((a=u.clubs)==null?void 0:a.encoded_name)||null,clubLogo:((d=u.clubs)==null?void 0:d.logo_url)||null,boost:0,used:!1,_line:l})}const f=Ge(g.length);g.forEach((y,u)=>{y._col=f[u]}),c[l]=g}return c}function Wi(e){const t=FORMATIONS[e]||FORMATIONS["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},s=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let c=0;for(const o of["GK","DEF","MIL","ATT"]){const a=[];for(let l=0;l<t[o];l++){const n=3+Math.floor(Math.random()*5);a.push({cardId:"fake-"+c,id:"fake-"+c,firstname:"IA",name:s[c%s.length],country_code:"XX",club_id:null,job:o,job2:null,note_g:o==="GK"?n:2,note_d:o==="DEF"?n:2,note_m:o==="MIL"?n:2,note_a:o==="ATT"?n:2,rarity:"normal",boost:0,used:!1,_line:o}),c++}const d=Ge(a.length);a.forEach((l,n)=>{l._col=d[n]}),i[o]=a}return i}function Ji(e,t,i){e.innerHTML=`
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;height:100%;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:100px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const A=t.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
            ...
            ${$e((A.players||[]).map(M=>({...M,used:!1})),"#ff6b6b",A.total)}
            ...
            ${$e((A.players||[]).map(M=>({...M,used:!1})),"#00ff88",A.total)}
    ...
    ${(()=>{const m=window.innerWidth>=700,A=(U,L,S)=>{var j,H;const z=(t.gcDefs||[]).find(K=>K.name===U.gc_type),N={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[z==null?void 0:z.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",D={purple:"#b06ce0",light_blue:"#00d4ef"}[z==null?void 0:z.color]||"#b06ce0",_=(z==null?void 0:z.name)||U.gc_type,b=(z==null?void 0:z.effect)||((j=de[U.gc_type])==null?void 0:j.desc)||"",C=z!=null&&z.image_url?`/manager-wars/icons/${z.image_url}`:null,E=((H=de[U.gc_type])==null?void 0:H.icon)||"⚡",w=Math.round(S*.22),B=Math.round(S*.22),I=S-w-B,q=_.length>12?7:9;return`<div class="gc-mini" data-gc-id="${U.id}" data-gc-type="${U.gc_type}"
          style="width:${L}px;height:${S}px;border-radius:10px;border:2px solid ${D};background:${N};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
          <div style="height:${w}px;background:rgba(255,255,255,0.14);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 3px">
            <span style="font-size:${q}px;font-weight:900;color:#fff;text-align:center;line-height:1.1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${L-6}px">${_}</span>
          ...
          <div style="height:${I}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
            ${C?`<img src="${C}" style="max-width:${L-10}px;max-height:${I-6}px;object-fit:contain">`:`<span style="font-size:${Math.round(I*.55)}px">${E}</span>`}
          </div>
        ...
        </div>`},M=(U,L)=>{var S;return L?`<div id="boost-card" style="width:110px;padding:8px 6px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:2px solid #87CEEB;border-radius:10px;cursor:pointer;display:flex;flex-direction:column;align-items:center;gap:4px;text-align:center">
            <div style="font-size:22px">⚡</div>
            <div style="font-size:10px;color:#000;font-weight:900">+${(S=t.boostCard)==null?void 0:S.value}</div>
          </div>`:A(U,110,150)},R=(U,L)=>{var S;return L?`<div id="boost-card" style="padding:4px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:2px solid #87CEEB;border-radius:7px;cursor:pointer;display:flex;flex-direction:column;align-items:center;gap:1px;text-align:center;min-width:42px">
        ...
        ${o.length===0?'<div style="font-size:7px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':o.map(U=>`<div class="sub-btn-col" data-sub-id="${U.cardId}" style="cursor:pointer;flex-shrink:0">${fe(U,44,58)}</div>`).join("")}
        ...
        <div class="terrain-wrapper" style="overflow:hidden;flex-shrink:0;display:flex;align-items:center;justify-content:center">
          ${Ae(t.homeTeam,t.formation,t.phase,s,300,300)}
        </div>
      </div>`;return m?`
  ...
  `,document.body.appendChild(s);let r=!1;const p=()=>{r||(r=!0,s.remove(),i())};s.addEventListener("click",p),setTimeout(p,2e3)}function _e(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function Mt(e,t,i,s=null){if(t.phase!=="attack"){_e("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){_e(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const c=Object.values(t.homeTeam).flat().filter(p=>p.used),o=t.homeSubs.filter(p=>!t.usedSubIds.includes(p.cardId));if(!c.length){_e("Aucun joueur utilisé à remplacer");return}if(!o.length){_e("Aucun remplaçant disponible");return}let a=0,d=Math.max(0,o.findIndex(p=>p.cardId===s)),l=!1;const n=document.createElement("div");n.id="sub-overlay",n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function r(){var T,v,x,k,F,m;const p=c[a],g=o[d],f=Math.min(130,Math.round((window.innerWidth-90)/2)),y=Math.round(f*1.35),u=A=>`background:rgba(255,255,255,0.12);border:none;color:${A?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${A?"default":"pointer"};flex-shrink:0`;n.innerHTML=`
    ...
    </div>`,(T=n.querySelector("#sub-close"))==null||T.addEventListener("click",()=>n.remove()),(v=n.querySelector("#out-up"))==null||v.addEventListener("click",()=>{a>0&&(a--,r())}),(x=n.querySelector("#out-down"))==null||x.addEventListener("click",()=>{a<c.length-1&&(a++,r())}),(k=n.querySelector("#in-up"))==null||k.addEventListener("click",()=>{d>0&&(d--,r())}),(F=n.querySelector("#in-down"))==null||F.addEventListener("click",()=>{d<o.length-1&&(d++,r())});const h=(A,M,R,G)=>{const O=n.querySelector("#"+A);if(!O)return;let V=0;O.addEventListener("touchstart",Z=>{V=Z.touches[0].clientY},{passive:!0}),O.addEventListener("touchend",Z=>{const Y=Z.changedTouches[0].clientY-V;if(Math.abs(Y)<30)return;const U=M();Y<0&&U<G-1?(R(U+1),r()):Y>0&&U>0&&(R(U-1),r())},{passive:!0})};h("in-panel",()=>d,A=>d=A,o.length),h("out-panel",()=>a,A=>a=A,c.length),(m=n.querySelector("#sub-confirm"))==null||m.addEventListener("click",A=>{if(A.preventDefault(),A.stopPropagation(),l)return;l=!0;const M=c[a],R=o[d];if(!M||!R)return;let G=null,O=-1;for(const[Z,Y]of Object.entries(t.homeTeam)){const U=(Y||[]).findIndex(L=>L.cardId===M.cardId);if(U!==-1){G=Z,O=U;break}}if(O===-1||!G){_e("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),n.remove();return}const V={...R,_line:G,_col:M._col||0,used:!1,boost:0};t.homeTeam[G].splice(O,1,V),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(R.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:M.name,firstname:M.firstname,note:te(M,G),portrait:oe(M),job:M.job,country_code:M.country_code,rarity:M.rarity,clubName:M.clubName,clubLogo:M.clubLogo},inPlayer:{name:R.name,firstname:R.firstname,note:te(R,G),portrait:oe(R),job:R.job,country_code:R.country_code,rarity:R.rarity,clubName:R.clubName,clubLogo:R.clubLogo},text:`🔄 ${R.firstname} ${R.name} remplace ${M.firstname} ${M.name}`}),n.remove(),oo(M,R,()=>ne(e,t,i))})}document.body.appendChild(n),r()}function no(e,t,i,s,c){var y,u;const o=(s.gcDefs||[]).find(h=>h.name===t),a=de[t]||{icon:"⚡",desc:"Carte spéciale."},d={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[o==null?void 0:o.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",l={purple:"#b06ce0",light_blue:"#00d4ef"}[o==null?void 0:o.color]||"#b06ce0",n=(o==null?void 0:o.name)||t,r=(o==null?void 0:o.effect)||a.desc,p=o!=null&&o.image_url?`/manager-wars/icons/${o.image_url}`:null,g=a.icon||"⚡",f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",f.innerHTML=`
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(o);let d=!1;const l=()=>{d||(d=!0,o.remove(),setTimeout(()=>c(),50))};o.addEventListener("click",l),setTimeout(l,3500)}async function ve(e,t,i){var r,p;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:s}=i,c=t.homeScore>t.aiScore,o=t.homeScore===t.aiScore,a=c?"victoire":o?"nul":"defaite",d=ci(t.mode,a);t.matchId&&await $.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:c?s.profile.id:null,home_credits_reward:d,played_at:new Date().toISOString()}).eq("id",t.matchId);const l={credits:(s.profile.credits||0)+d,matches_played:(s.profile.matches_played||0)+1};c?l.wins=(s.profile.wins||0)+1:o?l.draws=(s.profile.draws||0)+1:l.losses=(s.profile.losses||0)+1,await $.from("users").update(l).eq("id",s.profile.id),await i.refreshProfile();const n=document.createElement("div");n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",n.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const f=()=>document.getElementById("mm-status"),y=async(x=!0)=>{p=!0,g&&($.removeChannel(g),g=null),x&&await $.rpc("cancel_matchmaking",{p_user_id:l.profile.id}).catch(()=>{})};(v=document.getElementById("mm-cancel"))==null||v.addEventListener("click",async()=>{try{await y(!0)}catch(x){console.warn("[Matchmaking] cleanup error:",x)}le(),n("home")});const u=async(x,k,F)=>{p||(p=!0,g&&($.removeChannel(g),g=null),f()&&(f().textContent="Adversaire trouvé !"),await new Promise(m=>setTimeout(m,600)),e.isConnected!==!1&&po(e,t,x,F,a,d))},{data:h,error:T}=await $.rpc("try_matchmake",{p_user_id:l.profile.id,p_deck_id:i});if(T||!(h!=null&&h.success)){r("Erreur de matchmaking","error"),le(),n("home");return}if(h.matched){u(h.match_id,h.opponent_id,!1);return}f()&&(f().textContent="En attente d'un autre joueur..."),g=$.channel("matchmaking-"+l.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${l.profile.id}`},x=>{const k=x.new;k.status==="matched"&&k.match_id&&u(k.match_id,k.matched_with,!0)}).subscribe()}async function po(e,t,i,s,c=[],o=[]){const{state:a,navigate:d,toast:l}=t,n=s?"p1":"p2",r=s?"p2":"p1",p=(c||[]).map(L=>L.id),g=(c||[]).map(L=>({id:L.id,gc_type:L.gc_type,_gcDef:L._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation du match...</div>';const{data:f}=await $.from("matches").select("*").eq("id",i).single();if(!f){l("Match introuvable","error"),d("home");return}async function y(){const[{data:L,error:S},{data:z,error:N},{data:D},{data:_}]=await Promise.all([$.rpc("get_deck_for_match",{p_deck_id:f.home_deck_id}),$.rpc("get_deck_for_match",{p_deck_id:f.away_deck_id}),$.from("users").select("id,pseudo,club_name").eq("id",f.home_id).single(),$.from("users").select("id,pseudo,club_name").eq("id",f.away_id).single()]);S&&console.error("[PvP] get_deck_for_match p1 error:",S),N&&console.error("[PvP] get_deck_for_match p2 error:",N);function b(j,H){return{cardId:j.card_id,position:H||j.position,id:j.id,firstname:j.firstname,name:j.surname_encoded,country_code:j.country_code,club_id:j.club_id,job:j.job,job2:j.job2,note_g:Number(j.note_g)||0,note_d:Number(j.note_d)||0,note_m:Number(j.note_m)||0,note_a:Number(j.note_a)||0,rarity:j.rarity,skin:j.skin,hair:j.hair,hair_length:j.hair_length,clubName:j.club_encoded_name||null,clubLogo:j.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}}const C=((L==null?void 0:L.starters)||[]).map(j=>b(j,j.position)),E=((z==null?void 0:z.starters)||[]).map(j=>b(j,j.position)),w=((L==null?void 0:L.subs)||[]).map(j=>b(j,j.position)),B=((z==null?void 0:z.subs)||[]).map(j=>b(j,j.position)),I=(L==null?void 0:L.formation)||"4-4-2",q=(z==null?void 0:z.formation)||"4-4-2";return console.log("[PvP] p1Formation:",I,"p1 starters:",C.length,"positions:",C.map(j=>j.position)),console.log("[PvP] p2Formation:",q,"p2 starters:",E.length,"positions:",E.map(j=>j.position)),{p1Team:Re(C,I),p2Team:Re(E,q),p1Subs:w,p2Subs:B,p1Formation:I,p2Formation:q,p1Name:(D==null?void 0:D.club_name)||(D==null?void 0:D.pseudo)||"Joueur 1",p2Name:(_==null?void 0:_.club_name)||(_==null?void 0:_.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:s?p:[],gc_p2:s?[]:p,gcCardsFull_p1:s?g:[],gcCardsFull_p2:s?[]:g,gcReady_p1:!0,gcReady_p2:!0,usedGc_p1:[],usedGc_p2:[],lastActionAt:new Date().toISOString()}}let u=f.game_state&&Object.keys(f.game_state).length?f.game_state:null;if(!u)if(s){u=await y();const{data:L}=await $.from("matches").select("game_state").eq("id",i).single();!(L!=null&&L.game_state)||!Object.keys(L.game_state).length?await $.from("matches").update({game_state:u,turn_user_id:f.home_id}).eq("id",i):u=L.game_state}else{e.innerHTML=`<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation avec l'adversaire...</div>`;for(let L=0;L<30&&!u;L++){await new Promise(z=>setTimeout(z,400));const{data:S}=await $.from("matches").select("game_state").eq("id",i).single();S!=null&&S.game_state&&Object.keys(S.game_state).length&&(u=S.game_state)}if(!u){l("Erreur de synchronisation","error"),d("home");return}u.gc_p2=p,u.gcCardsFull_p2=g,await $.from("matches").update({game_state:u}).eq("id",i)}let h=!1;function T(L){var C;try{$.removeChannel(v)}catch{}const S=a.profile.id,z=L.winner_id===S,N=!!L.forfeit,D=u[n+"Score"]??0,_=u[r+"Score"]??0,b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center",b.innerHTML=`
      ...
      <button id="pvp-end-home" style="margin-top:10px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">Retour à l'accueil</button>`,document.body.appendChild(b),(C=b.querySelector("#pvp-end-home"))==null||C.addEventListener("click",()=>{b.remove(),le(),d("home")})}const v=$.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},L=>{const S=L.new;try{if(S.status==="finished"||S.forfeit){if(h)return;h=!0,u._timerInt&&(clearInterval(u._timerInt),u._timerInt=null),S.game_state&&(u=S.game_state),T(S);return}S.game_state&&(u=S.game_state,F())}catch(z){console.error("[PvP] Realtime render crash:",z,"gameState:",u)}}).subscribe();async function x(L){Object.assign(u,L),u.lastActionAt=new Date().toISOString();const{error:S}=await $.from("matches").update({game_state:u}).eq("id",i);S&&(console.error("[PvP] pushState DB error:",S.message,S),l("Erreur de synchronisation : "+S.message,"error"));try{F()}catch(z){console.error("[PvP] renderPvpScreen crash après pushState:",z),l("Erreur d'affichage : "+z.message,"error")}}async function k(){if(h)return;h=!0,u._timerInt&&(clearInterval(u._timerInt),u._timerInt=null);const L=s?f.away_id:f.home_id;try{await $.from("matches").update({status:"finished",forfeit:!0,winner_id:L}).eq("id",i)}catch(S){console.warn("[PvP] forfeit DB error:",S)}try{$.removeChannel(v)}catch{}le(),d("home")}function F(){var lt,ct,pt,ut,ft;const L=u[n+"Team"];u[r+"Team"];const S=u[n+"Score"],z=u[r+"Score"],N=u[n+"Name"],D=u[r+"Name"];if(u.phase==="gc-select")return m();if(u.phase==="reveal")return A();if(u.phase==="midfield")return M();if(u.phase==="finished")return Z();const _=u.phase===n+"-attack",b=u.phase===n+"-defense",C=u.phase===r+"-attack"||u.phase===r+"-defense",E=Array.isArray(u["selected_"+n])?u["selected_"+n]:[],w=E.map(P=>P.cardId);e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column";const B=window.innerWidth>=700,I=u[n+"Subs"]||[],q=u["usedSubIds_"+n]||[],j=I.filter(P=>!q.includes(P.cardId)),H=u["gcCardsFull_"+n]||[],K=u["usedGc_"+n]||[],Q=H.filter(P=>!K.includes(P.id)),ie=u.boostOwner===n&&!u.boostUsed,re=(P,J,ee)=>{var yt,ht;const W=P._gcDef,ce={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[W==null?void 0:W.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",Me={purple:"#b06ce0",light_blue:"#00d4ef"}[W==null?void 0:W.color]||"#b06ce0",ye=(W==null?void 0:W.name)||P.gc_type,ii=(W==null?void 0:W.effect)||((yt=de[P.gc_type])==null?void 0:yt.desc)||"",gt=W!=null&&W.image_url?`/manager-wars/icons/${W.image_url}`:null,oi=((ht=de[P.gc_type])==null?void 0:ht.icon)||"⚡",xt=Math.round(ee*.22),mt=Math.round(ee*.22),Oe=ee-xt-mt,ni=ye.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${P.id}" data-gc-type="${P.gc_type}"
        style="width:${J}px;height:${ee}px;border-radius:10px;border:2px solid ${Me};background:${ce};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
      ...
      ${j.length===0?'<div style="font-size:7px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':j.map(P=>`<div class="pvp-sub-btn" data-sub-id="${P.cardId}" style="cursor:pointer;flex-shrink:0">${fe(P,B?60:44,B?78:58)}</div>`).join("")}
    </div>`,xe=`<div style="overflow:hidden;min-width:0;flex:1;min-height:0" id="match-field">
      <div class="terrain-wrapper" style="width:100%;height:100%;overflow:hidden">
        ${Ae(L,u[n+"Formation"],_?"attack":b?"defense":"idle",w,300,300)}
      </div>
    </div>`,me=B?"padding:14px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:10px 8px;border-radius:12px;font-size:13px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",Le=_?`<button id="pvp-action" style="${me};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${E.length===0?"disabled":""}>⚔️ ATTAQUEZ <span id="pvp-timer"></span></button>`:b?`<button id="pvp-action" style="${me};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${E.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="pvp-timer"></span></button>`:`<div style="${me};background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);color:rgba(255,255,255,0.4)">⏳ Tour de ${D}</div>`,st=_||b?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${E.length}/3 sélectionné(s)</div>`:"",ei=(()=>{const P=u.pendingAttack;if(b&&P&&Array.isArray(P.players))return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
          ...
          ${$e(P.players.map(ce=>({...ce,used:!1})),"#ff6b6b",P.total)}
      ...
      </div>`;const P=e.querySelector(".terrain-wrapper");P&&(P.innerHTML=Ae(L,u[n+"Formation"],_?"attack":b?"defense":"idle",w,300,300))}if(e.querySelectorAll(".match-slot-hit").forEach(P=>{P.addEventListener("click",()=>{if(!_&&!b)return;const J=P.dataset.cardId,ee=P.dataset.role,W=(L[ee]||[]).find(ye=>ye.cardId===J);if(!W||W.used)return;Array.isArray(u["selected_"+n])||(u["selected_"+n]=[]);const ce=u["selected_"+n],Me=ce.findIndex(ye=>ye.cardId===J);Me>-1?ce.splice(Me,1):ce.length<3&&ce.push({...W,_role:ee}),F()})}),(lt=e.querySelector("#pvp-boost-card"))==null||lt.addEventListener("click",()=>{R()}),e.querySelectorAll(".pvp-gc-mini").forEach(P=>{P.addEventListener("click",()=>{V(P.dataset.gcId,P.dataset.gcType)})}),(ct=e.querySelector("#pvp-action"))==null||ct.addEventListener("click",()=>{_?Y():b&&U()}),(pt=e.querySelector("#pvp-quit"))==null||pt.addEventListener("click",()=>{confirm("Quitter le match ? Vous perdrez par forfait.")&&k()}),(ut=e.querySelector("#pvp-view-opp"))==null||ut.addEventListener("click",()=>{O()}),(ft=e.querySelector("#pvp-toggle-history"))==null||ft.addEventListener("click",()=>{G()}),function(){const J=e.querySelector(".terrain-wrapper svg");J&&(J.removeAttribute("width"),J.removeAttribute("height"),J.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",J.setAttribute("viewBox","-26 -26 352 352"),J.setAttribute("preserveAspectRatio","xMidYMid meet"));const ee=e.querySelector(".match-screen");if(ee){const W=e.clientHeight;W>50&&(ee.style.height=W+"px",ee.style.maxHeight=W+"px",ee.style.overflow="hidden")}}(),u._timerInt&&(clearInterval(u._timerInt),u._timerInt=null),(_||b)&&!h){let P=30,J=!1;const ee=()=>document.getElementById("pvp-timer"),W=()=>{ee()&&(ee().textContent=P+"s",ee().style.color=J?"#ff4444":"#fff")};W(),u._timerInt=setInterval(()=>{P--,P<0?J?(clearInterval(u._timerInt),u._timerInt=null,k()):(J=!0,P=15,W()):W()},1e3)}}function m(){var b;const L=u["gcReady_"+n],S=u["gcReady_"+r];Array.isArray(u["gc_"+n])||(u["gc_"+n]=[]);let z=u["gc_"+n];const N=3;function D(C,E){const w=C._gcDef,B={purple:"linear-gradient(135deg,#3d0a7a,#7a28b8)",light_blue:"linear-gradient(135deg,#006080,#00bcd4)"},I={purple:"#9b59b6",light_blue:"#00bcd4"},q=B[w==null?void 0:w.color]||B.purple,j=I[w==null?void 0:w.color]||I.purple;return`<div class="pvp-gc-card" data-id="${C.id}"
    ...
    </div>`,L)return;const _=n==="p1"?f.home_id:f.away_id;$.from("cards").select("id,gc_type").eq("owner_id",_).eq("card_type","game_changer").then(async({data:C})=>{const{data:E}=await $.from("gc_definitions").select("*").eq("is_active",!0),w=new Set,B=(C||[]).filter(q=>w.has(q.gc_type)?!1:(w.add(q.gc_type),!0)).map(q=>({...q,_gcDef:(E==null?void 0:E.find(j=>j.name===q.gc_type))||null})),I=document.getElementById("pvp-gc-grid");if(I){if(!B.length){I.innerHTML='<div style="color:rgba(255,255,255,0.4);font-size:13px;text-align:center;margin-top:30px">Aucune carte Game Changer disponible.</div>';return}I.innerHTML=B.map(q=>D(q,z.includes(q.id))).join(""),I.querySelectorAll(".pvp-gc-card").forEach(q=>{q.addEventListener("click",()=>{const j=q.dataset.id,H=z.indexOf(j);H>-1?z.splice(H,1):z.length<N&&z.push(j),F()})})}}),(b=e.querySelector("#pvp-gc-validate"))==null||b.addEventListener("click",async()=>{const C=n==="p1"?f.home_id:f.away_id,{data:E}=await $.from("cards").select("id,gc_type").eq("owner_id",C).eq("card_type","game_changer").in("id",z),{data:w}=await $.from("gc_definitions").select("*").eq("is_active",!0),B=(E||[]).map(I=>({...I,_gcDef:(w==null?void 0:w.find(q=>q.name===I.gc_type))||null}));await x({["gcReady_"+n]:!0,["gcCardsFull_"+n]:B}),u["gcReady_"+r]&&await x({phase:"reveal"})})}function A(){e.style.overflow="hidden",e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
      ...
      <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
    </div>`,n==="p1"&&setTimeout(async()=>{await x({phase:"midfield"})},5e3)}function M(){const L=u[n+"Team"].MIL||[],S=u[r+"Team"].MIL||[];function z(E){return E.reduce((w,B)=>w+te(B,"MIL"),0)}function N(E){let w=0;for(let B=0;B<E.length-1;B++){const I=ge(E[B],E[B+1]);I==="#00ff88"?w+=2:I==="#FFD700"&&(w+=1)}return w}const D=z(L)+N(L),_=z(S)+N(S),b=D>=_;function C(E,w,B){return`<div style="text-align:center">
        <div style="font-size:10px;color:rgba(255,255,255,0.5);letter-spacing:2px;margin-bottom:8px;text-transform:uppercase">${w}</div>
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${E.map((I,q)=>{const j=oe(I),H=q<E.length-1?ge(I,E[q+1]):null,K=H&&H!=="#ff3333"&&H!=="#cc2222";return`
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div id="pvp-midfield-result" style="opacity:0;text-align:center;transition:opacity 0.5s;color:#fff;max-width:320px"></div>
    </div>`,setTimeout(()=>{const E=document.getElementById("pvp-score-me"),w=document.getElementById("pvp-score-opp"),B=document.getElementById("pvp-midfield-result");E&&w&&(b?(E.style.fontSize="80px",E.style.color="#FFD700",w.style.opacity="0.25"):(w.style.fontSize="80px",w.style.color="#ff6b6b",E.style.opacity="0.25"));const I=u.p1Team.MIL||[],q=u.p2Team.MIL||[],j=z(I)+N(I),H=z(q)+N(q),K=j>=H?"p1":"p2";u[K+"Name"];let Q=u.boostValue;Q==null&&(Q=Kt(),u.boostValue=Q,u.boostOwner=K,u.boostUsed=!1),B&&(B.style.opacity="1",B.innerHTML=`<div style="font-size:20px;font-weight:900;margin-bottom:10px">
        ...
        <div style="background:rgba(135,206,235,0.15);border:2px solid #87CEEB;border-radius:14px;padding:14px 24px;display:inline-block;margin-top:4px" id="pvp-boost-display">
          ...
          <div style="font-size:10px;color:rgba(135,206,235,0.7)">Applicable sur n'importe quel joueur</div>
        </div>`:""}`),setTimeout(async()=>{if(n!=="p1")return;const ie=K;await x({phase:ie+"-attack",attacker:ie,round:1,boostValue:u.boostValue,boostUsed:!1,boostOwner:ie})},1800)},600)}function R(){var N;const L=u[n+"Team"],S=Object.entries(L).flatMap(([D,_])=>(_||[]).filter(b=>!b.used).map(b=>({...b,_line:D})));if(!S.length)return;const z=document.createElement("div");z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",z.innerHTML=`
      ...
      </div>`,document.body.appendChild(z),(N=z.querySelector("#boost-picker-close"))==null||N.addEventListener("click",()=>z.remove()),z.querySelectorAll(".boost-pick-item").forEach(D=>{D.addEventListener("click",async()=>{const _=D.dataset.cid,b=S.find(w=>w.cardId===_);if(!b)return;const C=u[n+"Team"],E=(C[b._line]||[]).find(w=>w.cardId===_);E&&(E.boost=(E.boost||0)+u.boostValue),z.remove(),await x({[n+"Team"]:C,boostUsed:!0})})})}function G(){var z;const L=Array.isArray(u.log)?u.log:[],S=document.createElement("div");S.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column",S.innerHTML=`
      ...
      </div>`,document.body.appendChild(q),(j=q.querySelector("#pvp-gc-back"))==null||j.addEventListener("click",()=>q.remove()),(H=q.querySelector("#pvp-gc-use"))==null||H.addEventListener("click",async()=>{q.remove();const K=u["usedGc_"+n]||[];K.push(L),await x({["usedGc_"+n]:K})})}function Z(){var D;const L=u[n+"Score"],S=u[r+"Score"],z=L>S,N=L===S;e.style.overflow="hidden",e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      ...
      <div style="font-size:32px;font-weight:900;color:#FFD700">${L} - ${S}</div>
  ...
  `;function l(n){const r=document.getElementById("mkt-content"),p=n==="buy"?a:d;if(p.length===0){r.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      ${g.map(f=>{var x,k,F,m;const y=(x=f.card)==null?void 0:x.player;if(!y)return"";const u=y.job==="GK"?y.note_g:y.job==="DEF"?y.note_d:y.job==="MIL"?y.note_m:y.note_a,h=uo[y.rarity],T=(i.profile.credits||0)>=f.price,v=f.status==="sold";return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${v?"opacity:0.65":""}">
