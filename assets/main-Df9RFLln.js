import{s as I,F as ot,b as Me,c as qt,l as ge,d as li}from"./formation-links-CDBKB_z4.js";function kt(e,{navigate:t,toast:i}){e.innerHTML=`
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
  ...
  </div>`}async function xi(e,t){const{state:i,navigate:n,toast:d,openModal:s,closeModal:a}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:l}=await I.from("cards").select(`id, card_type, current_note, gc_type, formation, is_for_sale, sale_price,
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),o=(l||[]).filter(y=>y.card_type==="player"&&y.player),r=(l||[]).filter(y=>y.card_type==="game_changer"),u=(l||[]).filter(y=>y.card_type==="formation"),{data:f}=await I.from("gc_definitions").select("name,gc_type,color,effect,image_url"),g={};(f||[]).forEach(y=>{g[y.name]=y});const w=Object.keys(ot),p=Object.keys(le),x={};o.forEach(y=>{const m=y.player.id;x[m]=(x[m]||0)+1}),new Set(Object.keys(x).map(y=>String(y)));const z=new Set(u.map(y=>y.formation)),v=new Set(r.map(y=>y.gc_type));let _="player",h="Tous",L="",E=!1;function D(){return[...o].sort((y,m)=>{const $=je.indexOf(y.player.job),T=je.indexOf(m.player.job);return $!==T?$-T:(y.player.surname_encoded||"").localeCompare(m.player.surname_encoded||"")})}function N(){return[...c||[]].sort((y,m)=>{const $=je.indexOf(y.job),T=je.indexOf(m.job);return $!==T?$-T:(y.surname_encoded||"").localeCompare(m.surname_encoded||"")})}function K(){return D().filter(y=>{const m=y.player,$=h==="Tous"||m.job===h,T=!L||`${m.firstname} ${m.surname_encoded}`.toLowerCase().includes(L);return $&&T})}function b(){return N().filter(y=>{const m=h==="Tous"||y.job===h,$=!L||`${y.firstname} ${y.surname_encoded}`.toLowerCase().includes(L);return m&&$})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
  ...
  </div>`;function R(){const y=document.getElementById("col-filters");y&&(_==="player"?(y.innerHTML=`
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",m=>{L=m.target.value.toLowerCase(),k()}),e.querySelectorAll(".filter-btn").forEach(m=>{m.addEventListener("click",()=>{h=m.dataset.job,R(),k()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{E=!E,R(),k()})):(y.innerHTML=`
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{E=!E,R(),k()})))}function k(){const y=document.getElementById("col-grid");y&&(_==="player"?C(y):_==="formation"?P(y):j(y))}function O(y,m,$,T,A){A=A||"#7a28b8";const G=document.getElementById("col-grid"),S=document.getElementById("col-big");if(!G||!S)return;var H=0;function q(){S.innerHTML='<div id="big-card-inner" style="display:inline-block;transform-origin:top center">'+m(y[H])+"</div>";var Y=S.querySelector("[data-card-id],[data-form-id],[data-gc-id]");Y&&Y.addEventListener("click",function(){T(y[H])}),requestAnimationFrame(function(){var W=document.getElementById("big-card-inner");if(document.getElementById("col-grid"),!(!W||!S)){var J=S.clientHeight-8,oe=S.clientWidth-24,ne=W.offsetHeight,ae=W.offsetWidth;if(ne>0&&ae>0&&J>40){var se=Math.min(J/ne,oe/ae,1);W.style.transform="scale("+se.toFixed(3)+")",W.style.transformOrigin="top center"}}}),G.innerHTML=y.map(function(W,J){return'<div class="col-mini-item" data-idx="'+J+'" style="flex-shrink:0;cursor:pointer;border-radius:8px;border:1.5px solid '+(J===H?A:"transparent")+';transition:border-color .15s;overflow:hidden">'+$(W,J===H)+"</div>"}).join(""),G.querySelectorAll(".col-mini-item").forEach(function(W){W.addEventListener("click",function(){H=Number(W.dataset.idx),q(),W.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})})})}q()}function F(y){var m=.54,$=Math.round(140*m),T=Math.round(310*m),A;if(!y||y._fake){var G=y?y.player:null;if(!G)return"";A=Tt(G)}else A=We(y,"");return'<div style="width:'+$+"px;height:"+T+'px;overflow:hidden;position:relative;flex-shrink:0"><div style="transform:scale('+m+');transform-origin:top left;position:absolute;top:0;left:0;pointer-events:none">'+A+"</div></div>"}function U(y,m,$){m=m||100,$=$||140;var T=Me[y]||{},A={GK:"#111111",DEF:"#cc2222",MIL:"#D4A017",ATT:"#22aa55"},G=Math.max(3,Math.round(m*.06)),S=Object.entries(T).map(function(q){var Y=q[0],W=q[1],J=Y.replace(/\d+$/,""),oe=A[J]||"#888",ne=Math.round(W.x*m),ae=Math.round(W.y*$);return'<circle cx="'+ne+'" cy="'+ae+'" r="'+G+'" fill="'+oe+'" stroke="rgba(255,255,255,0.8)" stroke-width="1"/>'}).join(""),H=Math.max(1,Math.round(m/50));return'<svg viewBox="0 0 '+m+" "+$+'" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:100%"><rect width="'+m+'" height="'+$+'" fill="#1A6B3C"/><rect x="'+Math.round(m*.2)+'" y="'+Math.round($*.02)+'" width="'+Math.round(m*.6)+'" height="'+Math.round($*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+H+'"/><line x1="0" y1="'+Math.round($*.5)+'" x2="'+m+'" y2="'+Math.round($*.5)+'" stroke="rgba(255,255,255,0.3)" stroke-width="'+H+'"/><ellipse cx="'+Math.round(m*.5)+'" cy="'+Math.round($*.5)+'" rx="'+Math.round(m*.18)+'" ry="'+Math.round($*.11)+'" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="'+H+'"/><rect x="'+Math.round(m*.2)+'" y="'+Math.round($*.82)+'" width="'+Math.round(m*.6)+'" height="'+Math.round($*.16)+'" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="'+H+'"/>'+S+"</svg>"}function M(y,m,$){var T=$>1?'<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×'+$+"</div>":"",A=m?'data-form-id="'+m.id+'"':"",G=y.length>7?14:y.length>5?16:19,S=!!m;return"<div "+A+' style="position:relative;width:140px;border-radius:12px;border:3px solid '+(S?"#2a7a40":"#bbb")+";background:#fff;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.12);"+(S?"":"filter:grayscale(1);opacity:0.5")+'">'+T+'<div style="padding:8px 6px 6px;background:#fff;text-align:center;border-bottom:3px solid '+(S?"#1A6B3C":"#aaa")+';flex-shrink:0"><div style="font-size:8px;color:#888;letter-spacing:1.5px;font-weight:700;margin-bottom:2px">FORMATION</div><div style="font-size:'+G+"px;font-weight:900;color:"+(S?"#1A6B3C":"#aaa")+';line-height:1">'+y+'</div></div><div style="flex:1;overflow:hidden;background:'+(S?"#1A6B3C":"#ccc")+'">'+U(y,140,220)+"</div></div>"}function B(y,m){var $=.54,T=Math.round(140*$),A=Math.round(305*$),G=Math.round(A*.22),S=A-G,H=y.length>7?5:7,q=U(y,T,S),Y=m?"1.5px solid #2a7a40":"1px solid #ddd",W=m?"":"filter:grayscale(1);opacity:0.45;",J=m?"#1A6B3C":"#bbb",oe="#fff";return'<div style="width:'+T+"px;height:"+A+"px;border-radius:6px;border:"+Y+";background:#fff;display:flex;flex-direction:column;overflow:hidden;"+W+'"><div style="height:'+G+"px;background:"+J+';display:flex;align-items:center;justify-content:center;padding:0 2px;flex-shrink:0"><span style="font-size:'+H+"px;font-weight:900;color:"+oe+";text-align:center;overflow:hidden;white-space:nowrap;max-width:"+(T-4)+'px">'+y+'</span></div><div style="height:'+S+'px;overflow:hidden;flex-shrink:0">'+q+"</div></div>"}function C(y){if(E){const m=b();if(!m.length){y.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}const $=m.map(T=>o.find(A=>A.player.id===T.id)||{_fake:!0,player:T,id:"fake-"+T.id});O($,T=>T._fake?Tt(T.player):We(T,""),T=>T._fake?F({player:T.player,id:"x",_fake:!0}):F(T),T=>{T._fake||Lt(T,o,x,t)},"#1A6B3C")}else{const m=K();if(!m.length){y.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const $={},T=[];m.forEach(A=>{$[A.player.id]||($[A.player.id]=!0,T.push(A))}),O(T,A=>{const G=x[A.player.id]||1,S=G>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${G}</div>`:"",q=o.filter(Y=>Y.player.id===A.player.id&&Y.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return We(A,S+q)},A=>F(A),A=>Lt(A,o,x,t),"#1A6B3C")}}function P(y){const m=E?w:[...z];if(!m.length){y.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}const $=m.map(T=>({formation:T,card:u.find(A=>A.formation===T)||null,owned:z.has(T)}));O($,({formation:T,card:A,owned:G})=>M(T,G?A:null,G?u.filter(S=>S.formation===T).length:0),({formation:T,owned:A})=>B(T,A),({card:T,owned:A})=>{A&&T&&hi(T,u,t,s)},"#1A6B3C")}function j(y){const m=Object.keys(g),$=E?m.length?m:p:[...v];if(!$.length){y.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}const T=$.map(A=>({type:A,gc:le[A]||{icon:"⚡",desc:""},def:g[A]||null,owned:v.has(A),card:r.find(G=>G.gc_type===A)||null}));O(T,({type:A,gc:G,def:S,owned:H,card:q})=>{const Y=H?r.filter(ze=>ze.gc_type===A).length:0,W=Y>1?`<div style="position:absolute;top:8px;right:8px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:2px 8px;z-index:3">×${Y}</div>`:"",J=(S==null?void 0:S.gc_type)==="ultra_game_changer",oe={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},ne={purple:"#b06ce0",light_blue:"#00d4ef"},ae=oe[S==null?void 0:S.color]||oe.purple,se=ne[S==null?void 0:S.color]||ne.purple,me=(S==null?void 0:S.effect)||G.desc||"",xe=S!=null&&S.image_url?`/manager-wars/icons/${S.image_url}`:null;return H&&q?`<div data-gc-id="${q.id}" data-gc-type="${A}" style="position:relative;width:140px;border-radius:12px;border:3px solid ${se};background:${ae};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${se}66;cursor:pointer">
          ${W}
        ...
        </div>`},({type:A,gc:G,def:S,owned:H})=>H?(()=>{const q=Math.round(75.60000000000001),Y=Math.round(310*.54),W=Math.round(Y*.65),J=Math.round(Y*.18),oe={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},ne={purple:"#9b59b6",light_blue:"#00bcd4"},ae=oe[S==null?void 0:S.color]||oe.purple,se=ne[S==null?void 0:S.color]||ne.purple,me=S!=null&&S.image_url?`/manager-wars/icons/${S.image_url}`:null;return`<div style="width:${q}px;height:${Y}px;border-radius:8px;background:${ae};border:1px solid ${se};display:flex;flex-direction:column;overflow:hidden"><div style="height:${J}px;background:rgba(255,255,255,0.15);display:flex;align-items:center;justify-content:center"><span style="font-size:7px;font-weight:900;color:#fff;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${q-6}px">${A}</span></div><div style="height:${W}px;display:flex;align-items:center;justify-content:center">${me?`<img src="${me}" style="max-width:${q-8}px;max-height:${W-4}px;object-fit:contain">`:`<span style="font-size:24px">${G.icon}</span>`}</div><div style="flex:1;display:flex;align-items:center;justify-content:center"><span style="font-size:7px;color:rgba(255,255,255,0.7);text-align:center;padding:0 2px">${((S==null?void 0:S.effect)||G.desc||"").slice(0,20)}</span></div></div>`})():(()=>{const q=Math.round(75.60000000000001),Y=Math.round(310*.54);return`<div style="width:${q}px;height:${Y}px;border-radius:8px;background:#eee;border:1px solid #ddd;display:flex;flex-direction:column;align-items:center;justify-content:center;filter:grayscale(1);opacity:0.45"><span style="font-size:22px">${G.icon}</span><span style="font-size:7px;color:#aaa;margin-top:4px;text-align:center;padding:0 4px">${A}</span></div>`})(),({type:A,owned:G,def:S})=>{G&&yi(A,S,s)},"#7a28b8")}e.querySelectorAll(".col-tab-btn").forEach(y=>{y.addEventListener("click",()=>{_=y.dataset.tab,h="Tous",L="",E=!1,e.querySelectorAll(".col-tab-btn").forEach(m=>{const $=m.dataset.tab===_;m.style.borderBottomColor=$?"var(--green)":"transparent",m.style.color=$?"var(--green)":"var(--gray-600)"}),R(),k()})}),R(),k()}function yi(e,t,i){const n=le[e]||{icon:"⚡",desc:"Effet spécial."},d=(t==null?void 0:t.gc_type)==="ultra_game_changer",s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},a={purple:"#b06ce0",light_blue:"#00d4ef"},l=s[t==null?void 0:t.color]||s.purple,c=a[t==null?void 0:t.color]||a.purple,o=(t==null?void 0:t.name)||e,r=(t==null?void 0:t.effect)||n.desc,u=t!=null&&t.image_url?`/manager-wars/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const $e=1e3;function hi(e,t,i,n){var p,x,z;const{state:d,toast:s,closeModal:a,navigate:l,refreshProfile:c}=i,o=e.formation,r={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function u(){const v=Me[o]||{},_=ot[o]||[],h=290,L=360,E=20;function D(K){const b=v[K];return b?{x:b.x*h,y:b.y*L}:null}let N=`<svg width="${h}" height="${L}" viewBox="0 0 ${h} ${L}" xmlns="http://www.w3.org/2000/svg">`;for(const[K,b]of _){const R=D(K),k=D(b);!R||!k||(N+=`<line x1="${R.x}" y1="${R.y}" x2="${k.x}" y2="${k.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const K of Object.keys(v)){const b=D(K);if(!b)continue;const R=K.replace(/\d+/,""),k=r[R]||"#555";N+=`<circle cx="${b.x}" cy="${b.y}" r="${E}" fill="${k}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,N+=`<text x="${b.x}" y="${b.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${R}</text>`}return N+="</svg>",N}const f=t.filter(v=>v.formation===o),g=f.length,w=!e.is_for_sale;n(`Formation ${o}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(p=document.getElementById("direct-sell-form-btn"))==null||p.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${o} pour ${$e.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const v=f.find(h=>!h.is_for_sale)||f[0];if(!v){s("Aucune carte à vendre","error");return}const{error:_}=await I.from("cards").delete().eq("id",v.id);if(_){s(_.message,"error");return}await I.from("users").update({credits:(d.profile.credits||0)+$e}).eq("id",d.profile.id),await c(),s(`+${$e.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),a(),l("collection")}),(x=document.getElementById("market-sell-form-btn"))==null||x.addEventListener("click",async()=>{const v=parseInt(document.getElementById("sell-price-form").value);if(!v||v<1){s("Prix invalide","error");return}await I.from("cards").update({is_for_sale:!0,sale_price:v}).eq("id",e.id),await I.from("market_listings").insert({seller_id:d.profile.id,card_id:e.id,price:v}),s("Carte mise en vente sur le marché !","success"),a(),l("collection")}),(z=document.getElementById("cancel-sell-form-btn"))==null||z.addEventListener("click",async()=>{await I.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await I.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),s("Annonce retirée","success"),a(),l("collection")})}async function Lt(e,t,i,n){var N,K,b,R,k,O;const{state:d,toast:s,openModal:a,closeModal:l,navigate:c,refreshProfile:o}=n,r=e.player,u=t.filter(F=>F.player.id===r.id),f=u.length,g=mi[r.rarity]||1e3,w=r.rarity!=="legende",p=Pt(r),x=Ie(r,r.job),z=r.job2?Ie(r,r.job2):null,v=Re[r.job]||"#1A6B3C",_=r.job2?Re[r.job2]:null,h=Ot[r.rarity]||"#ccc",L=st[r.country_code]||r.country_code||"",{data:E}=await I.from("transfer_history").select("club_name, manager_name, source, price, transferred_at").eq("player_id",r.id).order("transferred_at",{ascending:!0}),D=E&&E.length?`
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      ...
      <div style="width:140px;border-radius:12px;padding:6px;background:${h};flex-shrink:0">
        <div style="background:#f2e8d2;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
          ...
          <div style="background:#f2e8d2;padding:3px 6px;display:flex;align-items:center;justify-content:space-between;min-height:24px">
            ...
            ${(N=r.clubs)!=null&&N.logo_url?`<img src="${r.clubs.logo_url}" style="width:22px;height:16px;object-fit:contain">`:`<div style="background:#1a3a7a;color:#fff;border-radius:2px;padding:1px 3px;font-size:6px;font-weight:800">${(((K=r.clubs)==null?void 0:K.encoded_name)||"").slice(0,6)}</div>`}
          </div>
        </div>
      </div>
    ...
    </div>`:""}`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(b=document.getElementById("close-detail"))==null||b.addEventListener("click",l),(R=document.getElementById("direct-sell-btn"))==null||R.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte ${r.surname_encoded} pour ${g.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const F=u.find(M=>!M.is_for_sale)||u[0];if(!F){s("Aucune carte à vendre","error");return}const{error:U}=await I.from("cards").delete().eq("id",F.id);if(U){s(U.message,"error");return}await I.from("users").update({credits:(d.profile.credits||0)+g}).eq("id",d.profile.id),await o(),s(`+${g.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),l(),c("collection")}),(k=document.getElementById("market-sell-btn"))==null||k.addEventListener("click",async()=>{const F=parseInt(document.getElementById("sell-price").value);if(!F||F<1){s("Prix invalide","error");return}await I.from("cards").update({is_for_sale:!0,sale_price:F}).eq("id",e.id),await I.from("market_listings").insert({seller_id:d.profile.id,card_id:e.id,price:F}),s("Carte mise en vente sur le marché !","success"),l(),c("collection")}),(O=document.getElementById("cancel-sell-btn"))==null||O.addEventListener("click",async()=>{await I.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await I.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),s("Annonce retirée","success"),l(),c("collection")})}const Ne={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},ue={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function Ue(e){const t=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function Ht(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function Kt(e){var i;const t="https://fcnwclxlkytdfjotqkta.supabase.co";return(i=e==null?void 0:e.clubs)!=null&&i.logo_url?e.clubs.logo_url.startsWith("http")?e.clubs.logo_url:`${t}/storage/v1/object/public/assets/clubs/${e.clubs.logo_url}`:null}function vi(e,t=44,i=58){var g;const n=e?Ue(e):null,d=e?Kt(e):null,s=Ht(e==null?void 0:e.country_code),a=(e==null?void 0:e.job)||"MIL",l=ue[a]||"#555",c={normal:"#aaa",pépite:"#D4A017",papyte:"#222",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",o=Number(a==="GK"?e==null?void 0:e.note_g:a==="DEF"?e==null?void 0:e.note_d:a==="MIL"?e==null?void 0:e.note_m:e==null?void 0:e.note_a)||0,r=Math.round(i*.19),u=Math.round(i*.25),f=i-r-u;return e?`<div style="width:${t}px;height:${i}px;border-radius:5px;border:2px solid ${c};background:${l};position:relative;overflow:hidden;flex-shrink:0">
  ...
  <div class="page">
    ...
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(g=document.getElementById("close-selector"))==null||g.addEventListener("click",s),(w=document.getElementById("remove-player"))==null||w.addEventListener("click",()=>{delete t.slots[e],s(),be(i,t,n)}),document.querySelectorAll(".player-option").forEach(p=>{p.addEventListener("click",()=>{t.slots[e]=p.dataset.cardId,s(),be(i,t,n)})})}function _i(e,t,i){var c;const{openModal:n,closeModal:d}=i,s=new Set;Object.values(e.slots).filter(Boolean).forEach(o=>{var u;const r=e.playerCards.find(f=>f.id===o);(u=r==null?void 0:r.player)!=null&&u.id&&s.add(r.player.id)}),e.subs.forEach(o=>{var u;const r=e.playerCards.find(f=>f.id===o);(u=r==null?void 0:r.player)!=null&&u.id&&s.add(r.player.id)});const a=new Set,l=e.playerCards.filter(o=>{var r,u,f;return s.has((r=o.player)==null?void 0:r.id)||a.has((u=o.player)==null?void 0:u.id)?!1:(a.add((f=o.player)==null?void 0:f.id),!0)});n("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${l.length>0?l.map(o=>{var g;const r=o.player,u=Ue(r),f=r.job==="GK"?r.note_g:r.job==="DEF"?r.note_d:r.job==="MIL"?r.note_m:r.note_a;return`<div class="player-option" data-card-id="${o.id}"
          style="display:flex;align-items:center;gap:10px;padding:8px;border:1.5px solid var(--gray-200);border-radius:10px;cursor:pointer">
    ...
    </div>`,'<button class="btn btn-ghost" id="close-sub-selector">Fermer</button>'),(c=document.getElementById("close-sub-selector"))==null||c.addEventListener("click",d),document.querySelectorAll(".player-option").forEach(o=>{o.addEventListener("click",()=>{e.subs.push(o.dataset.cardId),d(),be(t,e,i)})})}async function $i(e,t){const{state:i,toast:n,navigate:d}=t,s=e.formationCards.find(c=>c.formation===e.formation),a=(s==null?void 0:s.id)||e.formationCardId;await I.from("decks").update({formation:e.formation,formation_card_id:a||null}).eq("id",e.deckId),await I.from("deck_cards").delete().eq("deck_id",e.deckId);const l=[];if(Object.entries(e.slots).forEach(([c,o],r)=>{l.push({deck_id:e.deckId,card_id:o,position:c,is_starter:!0,slot_order:r})}),e.subs.forEach((c,o)=>{l.push({deck_id:e.deckId,card_id:c,position:`SUB${o+1}`,is_starter:!1,slot_order:100+o})}),l.length>0){const{error:c}=await I.from("deck_cards").insert(l);if(c){n(c.message,"error");return}}n("Deck enregistré ✅","success"),d("decks")}function zt(e){const t=Ne[e]||Ne["4-4-2"],i=["GK1"];for(let n=1;n<=t.DEF;n++)i.push(`DEF${n}`);for(let n=1;n<=t.MIL;n++)i.push(`MIL${n}`);for(let n=1;n<=t.ATT;n++)i.push(`ATT${n}`);return i}async function ki(){const{data:e}=await I.from("booster_configs").select("*").eq("is_active",!0).order("sort_order");if(!(e!=null&&e.length))return[];const{data:t}=await I.from("booster_drop_rates").select("*").in("booster_id",e.map(i=>i.id)).order("sort_order");return e.map(i=>({...i,rates:(t||[]).filter(n=>n.booster_id===i.id)}))}function Ei(e){if(!(e!=null&&e.length))return null;const t=e.reduce((n,d)=>n+Number(d.percentage),0);let i=Math.random()*t;for(const n of e)if(i-=Number(n.percentage),i<=0)return n;return e[e.length-1]}const Ut=()=>Object.keys(Me),Ai=[{id:"players_std",img:"/manager-wars/icons/booster-players.png",name:"Players",sub:"5 cartes joueurs",cost:5e3,costLabel:"5 000 crédits",cardCount:5,type:"player"},{id:"players_pub",img:"/manager-wars/icons/booster-silver.png",name:"Players (pub)",sub:"3 cartes joueurs",cost:0,costLabel:"1 pub",cardCount:3,type:"player"},{id:"game_changer",img:"/manager-wars/icons/booster-gamechanger.png",name:"Game Changer",sub:"3 cartes spéciales",cost:1e4,costLabel:"10 000 crédits",cardCount:3,type:"game_changer"},{id:"formation",img:"/manager-wars/icons/booster-formation.png",name:"Formation",sub:"1 carte formation",cost:1e4,costLabel:"10 000 crédits",cardCount:1,type:"formation"}],Je={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé."},"Double attaque":{icon:"⚡",desc:"La prochaine attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la prochaine action IA."},Gel:{icon:"❄️",desc:"Bloque le meilleur attaquant IA."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function Ii(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";if(!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}const Ct={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},Ti={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"},Li={MA:"MAROC",FR:"FRANCE",AR:"ARGENTINE",PT:"PORTUGAL",BR:"BRESIL",ES:"ESPAGNE",DE:"ALLEMAGNE",GB:"ANGLETERRE",IT:"ITALIE",CM:"CAMEROUN",SN:"SENEGAL",NG:"NIGERIA",DK:"DANEMARK",NL:"PAYS-BAS",BE:"BELGIQUE",CI:"CÔTE D'IVOIRE",AL:"ALBANIE",HR:"CROATIE",RS:"SERBIE",TR:"TURQUIE"};function jt(e,t){return e&&Number(t==="GK"?e.note_g:t==="DEF"?e.note_d:t==="MIL"?e.note_m:e.note_a)||0}function Mi(e){var r,u;const t=e.player;if(!t)return"";const i=t.job||"ATT",n=Ct[i],d=Ti[t.rarity]||"#ccc",s=jt(t,i),a=t.job2?jt(t,t.job2):null,l=t.job2?Ct[t.job2]:null,c=Ii(t),o=Li[t.country_code]||t.country_code||"";return`
  ...
  `,document.body.appendChild(n);let d=!1;const s=document.getElementById("pack-cut-zone"),a=document.getElementById("pack-blade");let l=!1;const c=k=>k.touches&&k.touches[0]?k.touches[0].clientX:k.clientX;function o(k){d||(l=!0,a.style.opacity="1",r(k))}function r(k){if(!l||d)return;const O=s.getBoundingClientRect(),F=c(k)-O.left,U=Math.max(0,Math.min(1,F/O.width));a.style.width=U*O.width+"px",U>=.82&&f()}function u(){d||(l=!1,a.style.transition="width .2s ease, opacity .2s ease",a.style.width="0",a.style.opacity="0",setTimeout(()=>{d||(a.style.transition="")},220))}function f(){var O;if(d)return;d=!0,l=!1,a.style.width="100%",a.style.opacity="1",(O=document.getElementById("cut-flash"))==null||O.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const k=document.getElementById("cut-hint");k&&(k.style.opacity="0"),s.classList.add("pack-cut"),setTimeout(()=>{a.style.opacity="0",document.getElementById("pack-phase").style.display="none",x(0)},620)}s.addEventListener("pointerdown",o),window.addEventListener("pointermove",r),window.addEventListener("pointerup",u),s.addEventListener("touchstart",o,{passive:!0}),window.addEventListener("touchmove",r,{passive:!0}),window.addEventListener("touchend",u);let g=0,w=!1;const p=new Set;function x(k){g=k,document.getElementById("reveal-phase").style.display="flex",z(),v(k,0),D()}function z(){const k=document.getElementById("card-dots");k&&(k.innerHTML=e.map((O,F)=>`<div class="card-dot" data-i="${F}" style="width:8px;height:8px;border-radius:50%;background:${F===g?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),k.querySelectorAll(".card-dot").forEach(O=>O.addEventListener("click",()=>h(parseInt(O.dataset.i)))))}function v(k,O){var y;const F=e[k],U=document.getElementById("card-counter"),M=document.getElementById("card-track");U&&(U.textContent=`Carte ${k+1} / ${e.length}`);const B=F.card_type==="player"&&((y=F.player)==null?void 0:y.rarity)==="legende",C=!p.has(k);p.add(k);let P=0;if(F.card_type==="player"&&F.player){const m=F.player,$=m.job||"ATT";P=Number($==="GK"?m.note_g:$==="DEF"?m.note_d:$==="MIL"?m.note_m:m.note_a)||0}const j=m=>{M.innerHTML=`
        <div id="current-card-wrap" style="display:flex;flex-direction:column;align-items:center;gap:8px;${B?"filter:drop-shadow(0 0 20px #7a28b8)":""}">
          <div style="transform:scale(1.3);transform-origin:center">${Ri(F)}</div>
        ...
        </div>`;const $=document.getElementById("current-card-wrap");O!==0?($.style.transition="none",$.style.transform=`translateX(${O>0?100:-100}%)`,requestAnimationFrame(()=>{$.style.transition="transform .28s cubic-bezier(.25,1,.5,1)",$.style.transform="translateX(0)"})):$.animate([{opacity:0,transform:"scale(.7)"},{opacity:1,transform:"scale(1)"}],{duration:300,easing:"cubic-bezier(.34,1.56,.64,1)"}),m||B?K():b(),z()};C&&P>6&&F.card_type==="player"&&F.player?_(F,()=>j(!0)):j(!1)}function _(k,O){var j;w=!0;const F=k.player,U=`https://flagsapi.com/${F.country_code}/flat/64.png`,M=(j=F.clubs)==null?void 0:j.logo_url,B=document.getElementById("walkout-overlay"),C=document.getElementById("walkout-stage");if(!B||!C){w=!1,O();return}B.style.display="flex";const P=()=>{const y=C.firstElementChild;y&&(y.classList.remove("wo-in"),y.classList.add("wo-out"))};C.innerHTML=`<img class="wo-in" src="${U}" style="height:130px;border-radius:10px;box-shadow:0 10px 36px rgba(0,0,0,.6)" onerror="this.style.display='none'">`,navigator.vibrate&&navigator.vibrate(30),setTimeout(P,2e3),setTimeout(()=>{var y;C.innerHTML=M?`<img class="wo-in" src="${M}" style="max-height:160px;max-width:210px;object-fit:contain">`:`<div class="wo-in" style="font-size:34px;font-weight:900;color:#fff;text-align:center">${((y=F.clubs)==null?void 0:y.encoded_name)||"CLUB"}</div>`,navigator.vibrate&&navigator.vibrate(30)},2450),setTimeout(P,4450),setTimeout(()=>{B.style.display="none",C.innerHTML="",w=!1,navigator.vibrate&&navigator.vibrate([40,30,80]),O()},4900)}function h(k){if(w||k<0||k>=e.length||k===g)return;const O=k>g?1:-1;g=k,v(k,O)}function L(){h(g+1)}function E(){h(g-1)}function D(){const k=document.getElementById("card-viewport");if(!k||k._swipeBound)return;k._swipeBound=!0;let O=0,F=0,U=0,M=!1;const B=m=>m.touches?m.touches[0].clientX:m.clientX,C=m=>m.touches?m.touches[0].clientY:m.clientY,P=m=>{M=!0,O=B(m),F=C(m),U=0},j=m=>{if(!M)return;U=B(m)-O;const $=C(m)-F;if(Math.abs(U)<Math.abs($))return;const T=document.getElementById("current-card-wrap");T&&(T.style.transition="none",T.style.transform=`translateX(${U*.6}px) rotate(${U*.02}deg)`)},y=()=>{if(!M)return;M=!1;const m=document.getElementById("current-card-wrap"),$=55;U<=-$&&g<e.length-1?L():U>=$&&g>0?E():m&&(m.style.transition="transform .2s ease",m.style.transform="translateX(0)")};k.addEventListener("pointerdown",P),k.addEventListener("pointermove",j),k.addEventListener("pointerup",y),k.addEventListener("pointercancel",y),k.addEventListener("touchstart",P,{passive:!0}),k.addEventListener("touchmove",j,{passive:!0}),k.addEventListener("touchend",y),k.addEventListener("click",m=>{if(Math.abs(U)>8)return;const $=k.getBoundingClientRect();m.clientX-$.left>$.width/2?L():E()})}let N=null;function K(){const k=document.getElementById("fireworks-canvas");if(!k)return;k.width=window.innerWidth,k.height=window.innerHeight;const O=k.getContext("2d"),F=[];function U(){const B=Math.random()*k.width,C=Math.random()*k.height*.6,P=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],j=P[Math.floor(Math.random()*P.length)];for(let y=0;y<60;y++){const m=Math.PI*2/60*y,$=2+Math.random()*5;F.push({x:B,y:C,vx:Math.cos(m)*$,vy:Math.sin(m)*$,alpha:1,color:j,size:2+Math.random()*3})}}U(),N=setInterval(U,600);function M(){if(document.getElementById("fireworks-canvas")){O.clearRect(0,0,k.width,k.height);for(let B=F.length-1;B>=0;B--){const C=F[B];if(C.x+=C.vx,C.y+=C.vy+.08,C.vy*=.98,C.alpha-=.018,C.alpha<=0){F.splice(B,1);continue}O.globalAlpha=C.alpha,O.fillStyle=C.color,O.beginPath(),O.arc(C.x,C.y,C.size,0,Math.PI*2),O.fill()}O.globalAlpha=1,(N!==null||F.length>0)&&requestAnimationFrame(M)}}M()}function b(){N!==null&&(clearInterval(N),N=null);const k=document.getElementById("fireworks-canvas");k&&k.getContext("2d").clearRect(0,0,k.width,k.height)}document.getElementById("reveal-collection").addEventListener("click",()=>{b(),n.remove(),i("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{b(),n.remove(),i("boosters")})}function Ri(e){var t,i;if(e.card_type==="player"&&e.player)return Mi(e);if(e.card_type==="game_changer"){const n=e._gcDef,d=(n==null?void 0:n.gc_type)==="ultra_game_changer",s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},a={purple:"#b06ce0",light_blue:"#00d4ef"},l=s[n==null?void 0:n.color]||s.purple,c=a[n==null?void 0:n.color]||a.purple,o=(n==null?void 0:n.name)||e.gc_type||"Game Changer",r=(n==null?void 0:n.effect)||((t=Je[e.gc_type])==null?void 0:t.desc)||"",u=n!=null&&n.image_url?`/manager-wars/icons/${n.image_url}`:null,f=((i=Je[e.gc_type])==null?void 0:i.icon)||"⚡";return`<div style="width:170px;height:240px;background:${l};border-radius:14px;border:3px solid ${c};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${c}88;flex-shrink:0">
      ...
      </div>`,document.body.appendChild(n),n.addEventListener("click",a=>{a.target===n&&n.remove()}),(i=document.getElementById("odds-close"))==null||i.addEventListener("click",()=>n.remove());return}Oi()}function Oi(){const e=document.createElement("div");e.style.cssText=`position:fixed;inset:0;background:rgba(0,0,0,0.6);display:flex;
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}const Pi="simulation",Vt="VOTRE_ZONE_ID";function Hi(){switch(Pi){case"propellerads":return Ki();case"adsense":return Ui();default:return lt()}}function lt(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:#111;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:14px";let i=5;t.innerHTML=`
      ...
      <button id="ad-skip" disabled style="margin-top:8px;padding:10px 24px;border-radius:20px;border:none;background:rgba(255,255,255,0.15);color:rgba(255,255,255,0.4);font-size:13px;cursor:default">
        Passer (5)
      </button>`,document.body.appendChild(t);const n=setInterval(()=>{i--;const d=document.getElementById("ad-cd"),s=document.getElementById("ad-skip");d&&(d.textContent=i),s&&(s.textContent=i>0?`Passer (${i})`:"✓ Continuer"),i<=0&&(clearInterval(n),s&&(s.disabled=!1,s.style.cssText=s.style.cssText.replace("rgba(255,255,255,0.15)","#1A6B3C").replace("rgba(255,255,255,0.4)","#fff").replace("default","pointer")),s==null||s.addEventListener("click",()=>{t.remove(),e(!0)},{once:!0}))},1e3)})}function Ki(){return new Promise((e,t)=>{if(typeof window.propeller>"u")return console.warn("[Ad] PropellerAds non chargé → simulation"),e(lt());window.propeller.push({zone_id:Vt,onComplete:()=>e(!0),onSkip:()=>t(new Error("Publicité ignorée")),onError:()=>t(new Error("Erreur publicité"))})})}function Ui(){return new Promise((e,t)=>{if(typeof window.adsbygoogle>"u")return console.warn("[Ad] AdSense non chargé → simulation"),e(lt());try{(window.adsbygoogle=window.adsbygoogle||[]).push({google_ad_client:"ca-pub-VOTRE_PUBLISHER_ID",google_ad_slot:Vt,google_ad_format:"rewarded",on_reward:()=>e(!0),on_dismiss:()=>t(new Error("Publicité fermée"))})}catch(i){t(i)}})}const we={"4-3-3 (3)":{GK:1,DEF:4,MIL:3,ATT:3},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-3-3 (4)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-2-1":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (2)":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"4-3-3 (5)":{GK:1,DEF:4,MIL:3,ATT:3},"5-2-2-1":{GK:1,DEF:5,MIL:2,ATT:3},"4-3-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"5-2-1-2":{GK:1,DEF:5,MIL:3,ATT:2},"4-5-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"4-5-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2},"4-4-1-1":{GK:1,DEF:4,MIL:4,ATT:2},"4-1-2-1-2":{GK:1,DEF:4,MIL:4,ATT:2},"3-4-1-2":{GK:1,DEF:3,MIL:5,ATT:2},"3-4-2-1":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"4-1-4-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-2-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-2-3-1":{GK:1,DEF:4,MIL:5,ATT:1},"4-2-3-1 (2)":{GK:1,DEF:4,MIL:5,ATT:1},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"4-1-2-1-2 (2)":{GK:1,DEF:4,MIL:4,ATT:2}},_e={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function fe(e,t,i,n,d){var s;e.innerHTML=`<div class="match-screen" style="display:flex;align-items:center;justify-content:center;min-height:100vh">
    <div style="text-align:center;padding:40px;color:#fff">
      <div style="font-size:48px;margin-bottom:16px">${t}</div>
      <p style="margin-bottom:16px">${i}</p>
      <button class="btn btn-primary" id="msg-btn">${n}</button>
    </div>
  </div>`,(s=document.getElementById("msg-btn"))==null||s.addEventListener("click",d)}function re(e){const t=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!t||!(e!=null&&e.skin)||!(e!=null&&e.hair))return null;const i=e.hair==="chauve"?`${e.skin}-chauve-rase`:`${e.skin}-${e.hair}-${e.hair_length}`;return`${t}/storage/v1/object/public/assets/tetes/${i}.png`}function Qe(e,t){var n,d;const i=e.player;return{cardId:e.id,position:t||null,id:i.id,firstname:i.firstname,name:i.surname_encoded,country_code:i.country_code,club_id:i.club_id,job:i.job,job2:i.job2,note_g:Number(i.note_g)||0,note_d:Number(i.note_d)||0,note_m:Number(i.note_m)||0,note_a:Number(i.note_a)||0,rarity:i.rarity,skin:i.skin,hair:i.hair,hair_length:i.hair_length,clubName:((n=i.clubs)==null?void 0:n.encoded_name)||null,clubLogo:((d=i.clubs)==null?void 0:d.logo_url)||null,boost:0,used:!1,_line:null,_col:null}}function Oe(e){return e===1?[1]:e===2?[0,2]:e===3?[0,1,2]:e===4?[0,1,1,2]:e===5?[0,1,1,1,2]:[1]}function Yt(){const e=Math.random()*100;return e<10?3:e<30?2:1}function Pe(e,t){const i=we[t]||we["4-4-2"],n={GK:[],DEF:[],MIL:[],ATT:[]};if(e.length&&e.every(a=>a.position)){for(const a of["GK","DEF","MIL","ATT"]){const l=e.filter(o=>o.position&&o.position.replace(/\d+$/,"")===a).sort((o,r)=>parseInt(o.position.replace(/\D+/g,""),10)-parseInt(r.position.replace(/\D+/g,""),10)).map(o=>({...o,_line:a})),c=Oe(l.length);l.forEach((o,r)=>{o._col=c[r]}),n[a]=l}return n}const s=[...e];for(const a of["GK","DEF","MIL","ATT"]){const l=[];for(let o=0;o<i[a];o++){let r=s.findIndex(u=>u.job===a);if(r===-1&&(r=s.findIndex(u=>u.job2===a)),r===-1&&(r=0),s[r]){const u={...s[r],_line:a};l.push(u),s.splice(r,1)}}const c=Oe(l.length);l.forEach((o,r)=>{o._col=c[r]}),n[a]=l}return n}function Wt(e){document.querySelectorAll(".top-nav, .bottom-nav").forEach(t=>{t.style.setProperty("display","none","important"),t.dataset.matchHidden="1"}),e&&e.style.setProperty("padding-bottom","0","important")}function de(e){document.querySelectorAll(".top-nav, .bottom-nav").forEach(t=>{t.style.removeProperty("display"),delete t.dataset.matchHidden}),e&&e.style.removeProperty("padding-bottom")}function Xt(e,t,i){const d=new Set,s=t.filter(r=>{const u=r.gc_type||r.id;return d.has(u)?!1:(d.add(u),!0)});let a=[];function l(r,u){const f=r._gcDef,g={purple:"linear-gradient(135deg,#3d0a7a,#7a28b8)",light_blue:"linear-gradient(135deg,#006080,#00bcd4)"},w={purple:"#9b59b6",light_blue:"#00bcd4"},p=g[f==null?void 0:f.color]||g.purple,x=w[f==null?void 0:f.color]||w.purple;return`<div class="gc-select-card" data-id="${r.id}"
    ...
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div id="deck-swipe-zone" style="flex:1;min-height:0;overflow:hidden;position:relative;touch-action:pan-y;display:flex;align-items:center;justify-content:center;padding:4px">
        ${p?`<div class="deck-preview-wrap" style="aspect-ratio:1/1;max-width:100%;max-height:100%;width:auto;height:100%;overflow:hidden">${Le(p,w,null,[],285,285)}</div>`:`<div style="display:flex;align-items:center;justify-content:center;height:100%;opacity:.4;flex-direction:column;gap:8px">
    ...
    </div>`,function(){const N=e.querySelector(".deck-preview-wrap svg");N&&(N.removeAttribute("width"),N.removeAttribute("height"),N.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",N.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),(_=document.getElementById("prev-deck"))==null||_.addEventListener("click",()=>{c>0&&(c--,o())}),(h=document.getElementById("next-deck"))==null||h.addEventListener("click",()=>{c<s.length-1&&(c++,o())}),(L=document.getElementById("validate-deck"))==null||L.addEventListener("click",()=>{x&&t.navigate("match",{matchMode:i,deckId:r.id})}),(E=document.getElementById("cancel-deck-select"))==null||E.addEventListener("click",()=>{de(e),d("home")});const z=document.getElementById("deck-swipe-zone");if(z){let D=0,N=0;z.addEventListener("touchstart",K=>{D=K.touches[0].clientX,N=K.touches[0].clientY},{passive:!0}),z.addEventListener("touchend",K=>{const b=K.changedTouches[0].clientX-D,R=K.changedTouches[0].clientY-N;Math.abs(b)<40||Math.abs(b)<Math.abs(R)||(b<0&&c<s.length-1?(c++,o()):b>0&&c>0&&(c--,o()))},{passive:!0})}}o()}function Jt(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";return e!=null&&e.clubLogo?e.clubLogo.startsWith("http")?e.clubLogo:`${t}/storage/v1/object/public/assets/clubs/${e.clubLogo}`:null}function pe(e,t=44,i=58){if(!e)return`<div style="width:${t}px;height:${i}px;border:1.5px dashed rgba(255,255,255,0.2);border-radius:5px"></div>`;const n=typeof e.portrait=="string"&&e.portrait.startsWith("http")?e.portrait:re(e),d=Jt(e),s=e._line||e.job||"MIL",a=_e[s]||"#555",l={normal:"#aaa",pépite:"#D4A017",papyte:"#222",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",c=e.note!==void 0?Number(e.note)||0:(Number(ie(e,s))||0)+(e.boost||0),o=Qt(e==null?void 0:e.country_code),r=Math.round(i*.19),u=Math.round(i*.25),f=i-Math.round(i*.19)-Math.round(i*.25),g=e!=null&&e.used?.28:1;return`<div style="width:${t}px;height:${i}px;border-radius:5px;border:2px solid ${l};background:${a};position:relative;overflow:hidden;flex-shrink:0;opacity:${g}">
    <div style="position:absolute;top:0;left:0;right:0;height:${r}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:center;z-index:2">
  ...
  </div>`}function Ee(e,t,i){if(!(e!=null&&e.length))return"";const n=e.slice(0,5);let d='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return n.forEach((s,a)=>{if(d+=pe(s,40,52),a<n.length-1){const l=ge(s,n[a+1]);d+=`<div style="width:7px;height:3px;background:${l==="#ff3333"||l==="#cc2222"?"rgba(255,255,255,0.12)":l};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(d+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),d+="</div>",d}function Ze(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function Qt(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function Te(e,t,i,n,d=310,s=310,a=[]){const l=Me[t]||{},c=qt(t)||ot[t]||[],o={},r=["ATT","MIL","DEF","GK"];for(const _ of r)(e[_]||[]).forEach((L,E)=>{o[`${_}${E+1}`]=L});function u(_){const h=l[_];return h?{x:h.x*d,y:h.y*s}:null}let f="";for(const[_,h]of c){const L=u(_),E=u(h);if(!L||!E)continue;const D=o[_],N=o[h],K=ge(D,N);K==="#00ff88"||K==="#FFD700"?(f+=`<line x1="${L.x.toFixed(1)}" y1="${L.y.toFixed(1)}" x2="${E.x.toFixed(1)}" y2="${E.y.toFixed(1)}"
        stroke="${K}" stroke-width="10" stroke-linecap="round" opacity="0.22"/>`,f+=`<line x1="${L.x.toFixed(1)}" y1="${L.y.toFixed(1)}" x2="${E.x.toFixed(1)}" y2="${E.y.toFixed(1)}"
        stroke="${K}" stroke-width="3.5" stroke-linecap="round" opacity="0.95"/>`):f+=`<line x1="${L.x.toFixed(1)}" y1="${L.y.toFixed(1)}" x2="${E.x.toFixed(1)}" y2="${E.y.toFixed(1)}"
        stroke="${K}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const g=48,w=64,p=13,x=16,z={normal:"#aaaaaa",pépite:"#D4A017",papyte:"#111111",légende:"#7a28b8"};for(const[_,h]of Object.entries(o)){const L=u(_);if(!L||!h)continue;const E=_.replace(/[0-9]/g,""),D=_e[E]||"#555",N=a.includes(h.cardId),K=i==="attack"&&(["MIL","ATT"].includes(E)||N)&&!h.used||i==="defense"&&["GK","DEF","MIL"].includes(E)&&!h.used,b=n.includes(h.cardId);let R;i==="attack"?R=N?Math.max(1,Number(h.note_a)||0):E==="MIL"?Number(h.note_m)||0:Number(h.note_a)||0:i==="defense"?R=E==="GK"?Number(h.note_g)||0:E==="MIL"?Number(h.note_m)||0:Number(h.note_d)||0:R=Number(E==="GK"?h.note_g:E==="DEF"?h.note_d:E==="MIL"?h.note_m:h.note_a)||0,R=R+(h.boost||0);const k=(L.x-g/2).toFixed(1),O=(L.y-w/2).toFixed(1),F=h.used?.25:1,U=z[h==null?void 0:h.rarity]||z.normal,M=b?"#ff3030":U,B=b?3:(h==null?void 0:h.rarity)==="légende"||(h==null?void 0:h.rarity)==="pépite"?2.5:2,C=w-p-x;f+=`<defs><clipPath id="cp-${_}"><rect x="${k}" y="${(L.y-w/2+p).toFixed(1)}" width="${g}" height="${C}"/></clipPath></defs>`,f+=`<rect x="${k}" y="${O}" width="${g}" height="${w}" rx="5" fill="${D}" opacity="${F}"/>`;const P=re(h);P&&!h.used&&(f+=`<image href="${P}" x="${k}" y="${(L.y-w/2+p).toFixed(1)}" width="${g}" height="${C}" clip-path="url(#cp-${_})" preserveAspectRatio="xMidYMin slice"/>`),f+=`<rect x="${k}" y="${O}" width="${g}" height="${p}" rx="4" fill="rgba(255,255,255,0.92)"/>`,f+=`<text x="${L.x.toFixed(1)}" y="${(L.y-w/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(h.name||"").slice(0,9)}</text>`;const j=(L.y+w/2-x).toFixed(1);if(f+=`<rect x="${k}" y="${j}" width="${g}" height="${x}" fill="rgba(255,255,255,0.92)"/>`,h.used)f+=`<text x="${L.x.toFixed(1)}" y="${(L.y+w/2-x/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="13" font-weight="900" fill="rgba(0,0,0,0.4)" font-family="Arial Black">–</text>`;else{const y=Ze(h.country_code);y?f+=`<image href="${y}" x="${(L.x-20).toFixed(1)}" y="${(L.y+w/2-x+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:f+=`<text x="${(L.x-13).toFixed(1)}" y="${(L.y+w/2-x/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${Qt(h.country_code)}</text>`,f+=`<text x="${L.x.toFixed(1)}" y="${(L.y+w/2-x/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${R}</text>`;const m=Jt(h);m?f+=`<image href="${m}" x="${(L.x+g/2-14).toFixed(1)}" y="${(L.y+w/2-x+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:h.clubName&&(f+=`<text x="${(L.x+14).toFixed(1)}" y="${(L.y+w/2-x/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(h.clubName||"").slice(0,3).toUpperCase()}</text>`),h.boost&&(f+=`<rect x="${(L.x+g/2-12).toFixed(1)}" y="${(L.y-w/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,f+=`<text x="${(L.x+g/2-5).toFixed(1)}" y="${(L.y-w/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${h.boost}</text>`)}f+=`<rect x="${k}" y="${O}" width="${g}" height="${w}" rx="5" fill="${b?"rgba(255,255,255,0.12)":"none"}" stroke="${M}" stroke-width="${B}" opacity="${F}"/>`,K&&(f+=`<rect x="${k}" y="${O}" width="${g}" height="${w}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${b?"selected":""}" data-card-id="${h.cardId}" data-role="${E}" style="cursor:pointer"/>`)}const v=38;return`<svg viewBox="${-v} ${-v} ${d+v*2} ${s+v*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${f}
  </svg>`}function Le(e,t,i,n,d=300,s=300,a=[]){return`<div id="match-terrain-wrap" style="position:relative;padding:0 4px">
    ${Te(e,t,i,n,d,s,a)}
  ...
  </div>`}async function Zt(e,t,i,n){var L;const{state:d,navigate:s,toast:a}=t;Wt(e);const l=d.params||{};if(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!l.deckId)return Vi(e,t,i);const c=l.deckId;let o,r,u,f;try{const E=await Promise.all([I.from("decks").select("formation,name").eq("id",c).single(),I.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation,
            player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
              clubs(encoded_name,logo_url)))`).eq("deck_id",c).order("slot_order")]);o=E[0].data,u=E[0].error,r=E[1].data,f=E[1].error}catch(E){console.error("[Match] Exception chargement deck:",E),fe(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>s("home"));return}if(u||f){console.error("[Match] Erreur Supabase:",u||f),fe(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>s("home"));return}const g=(r||[]).filter(E=>{var D;return E.is_starter&&((D=E.card)==null?void 0:D.player)}).map(E=>Qe(E.card,E.position)),w=(r||[]).filter(E=>{var D;return!E.is_starter&&((D=E.card)==null?void 0:D.player)}).map(E=>Qe(E.card,E.position));if(g.length<11){fe(e,"⚠️",`Deck incomplet (${g.length}/11).`,"Compléter",()=>s("decks"));return}const p=(r||[]).find(E=>{var D;return((D=E.card)==null?void 0:D.card_type)==="formation"}),x=(o==null?void 0:o.formation)||((L=p==null?void 0:p.card)==null?void 0:L.formation)||"4-4-2",{data:z,error:v}=await I.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",d.profile.id).eq("card_type","game_changer"),{data:_}=await I.from("gc_definitions").select("*").eq("is_active",!0),h=(z||[]).map(E=>({...E,_gcDef:(_==null?void 0:_.find(D=>D.name===E.gc_type||D.id===E.gc_definition_id))||null}));n({deckId:c,formation:x,starters:g,subsRaw:w,gcCardsEnriched:h,gcDefs:_||[]})}async function Yi(e,t){const{state:i}=t,d=(i.params||{}).matchMode||"vs_ai_easy",s=d.replace("vs_ai_",""),a=d;await Zt(e,t,d,async({deckId:l,formation:c,starters:o,subsRaw:r,gcCardsEnriched:u,gcDefs:f})=>{try{const g=Pe(o,c),w=await Wi(c,s),p=async x=>{try{const{data:z,error:v}=await I.from("matches").insert({home_id:i.profile.id,away_id:null,mode:a,home_deck_id:l,status:"in_progress"}).select().single();if(v){console.error("[MatchIA] Erreur création match:",v),fe(e,"⚠️","Impossible de créer le match ("+v.message+").","Retour",()=>t.navigate("home"));return}const _={gcDefs:f||[],matchId:z==null?void 0:z.id,mode:a,difficulty:s,formation:c,homeTeam:g,aiTeam:w,homeSubs:r,subsUsed:0,maxSubs:Math.min(r.length,3),homeScore:0,aiScore:0,gcCards:x,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Ji(e,_,t)}catch(z){console.error("[MatchIA] Exception launchMatch:",z),fe(e,"⚠️","Erreur au lancement du match : "+z.message,"Retour",()=>t.navigate("home"))}};if(!u.length){p([]);return}Xt(e,u,p)}catch(g){console.error("[MatchIA] Exception setup:",g),fe(e,"⚠️","Erreur de préparation du match : "+g.message,"Retour",()=>t.navigate("home"))}})}async function Wi(e,t){var a,l;const{data:i}=await I.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(60);if(!i||i.length<11)return Xi(e);const n=we[e]||we["4-4-2"],d={GK:[],DEF:[],MIL:[],ATT:[]},s=[...i];for(const c of["GK","DEF","MIL","ATT"]){const o=s.filter(w=>w.job===c),r=s.filter(w=>w.job!==c),u=[...o,...r],f=[];for(let w=0;w<n[c];w++){const p=u[w]||s[w];p&&f.push({cardId:"ai-"+p.id+"-"+w,id:p.id,firstname:p.firstname,name:p.surname_encoded,country_code:p.country_code,club_id:p.club_id,job:p.job,job2:p.job2,note_g:Number(p.note_g)||0,note_d:Number(p.note_d)||0,note_m:Number(p.note_m)||0,note_a:Number(p.note_a)||0,rarity:p.rarity,skin:p.skin,hair:p.hair,hair_length:p.hair_length,clubName:((a=p.clubs)==null?void 0:a.encoded_name)||null,clubLogo:((l=p.clubs)==null?void 0:l.logo_url)||null,boost:0,used:!1,_line:c})}const g=Oe(f.length);f.forEach((w,p)=>{w._col=g[p]}),d[c]=f}return d}function Xi(e){const t=we[e]||we["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},n=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let d=0;for(const s of["GK","DEF","MIL","ATT"]){const a=[];for(let c=0;c<t[s];c++){const o=3+Math.floor(Math.random()*5);a.push({cardId:"fake-"+d,id:"fake-"+d,firstname:"IA",name:n[d%n.length],country_code:"XX",club_id:null,job:s,job2:null,note_g:s==="GK"?o:2,note_d:s==="DEF"?o:2,note_m:s==="MIL"?o:2,note_a:s==="ATT"?o:2,rarity:"normal",boost:0,used:!1,_line:s}),d++}const l=Oe(a.length);a.forEach((c,o)=>{c._col=l[o]}),i[s]=a}return i}function Ji(e,t,i){e.innerHTML=`
  ...
  <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
    ...
    <div id="duel-finale" style="position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;opacity:0;pointer-events:none;background:radial-gradient(circle at center, rgba(10,61,30,0.4), rgba(10,61,30,0.92))"></div>
          ...
          <button id="start-match-btn" style="margin-top:6px;padding:18px 46px;border-radius:14px;border:none;background:#1A6B3C;color:#fff;font-size:18px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,0.4);animation:fadeUp .4s ease both;animation-delay:.45s;opacity:0">
            ▶ Commencer le match
          </button>`,L.style.transition="opacity .45s ease",L.style.opacity="1",L.style.pointerEvents="auto",(E=document.getElementById("start-match-btn"))==null||E.addEventListener("click",w))},600)},700)},2800)}function Zi(e){if(e.type==="duel"){const t=e.homeTotal>=e.aiTotal;return`
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:100px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const R=t.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
            ...
            ${Ee((R.players||[]).map(k=>({...k,used:!1})),"#ff6b6b",R.total)}
            ...
            ${Ee((R.players||[]).map(k=>({...k,used:!1})),"#00ff88",R.total)}
          </div>`}const b=t.log[t.log.length-1];return b?'<div style="padding:2px 4px">'+Zi(b)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})()}
    </div>
    ...
    ${(()=>{const b=window.innerWidth>=700,R=(j,y,m)=>{var ne,ae;const $=(t.gcDefs||[]).find(se=>se.name===j.gc_type),T={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[$==null?void 0:$.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",A={purple:"#b06ce0",light_blue:"#00d4ef"}[$==null?void 0:$.color]||"#b06ce0",G=($==null?void 0:$.name)||j.gc_type,S=($==null?void 0:$.effect)||((ne=le[j.gc_type])==null?void 0:ne.desc)||"",H=$!=null&&$.image_url?`/manager-wars/icons/${$.image_url}`:null,q=((ae=le[j.gc_type])==null?void 0:ae.icon)||"⚡",Y=Math.round(m*.22),W=Math.round(m*.22),J=m-Y-W,oe=G.length>12?7:9;return`<div class="gc-mini" data-gc-id="${j.id}" data-gc-type="${j.gc_type}"
          ...
          <div style="height:${J}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
            ${H?`<img src="${H}" style="max-width:${y-10}px;max-height:${J-6}px;object-fit:contain">`:`<span style="font-size:${Math.round(J*.55)}px">${q}</span>`}
          </div>
          <div style="height:${W}px;background:rgba(0,0,0,0.38);display:flex;align-items:center;justify-content:center;padding:0 3px">
            <span style="font-size:6px;color:rgba(255,255,255,0.9);text-align:center;line-height:1.25">${S.slice(0,38)}</span>
          </div>
        </div>`},k=(j,y)=>{var m;return`<div id="boost-card"
          style="box-sizing:border-box;width:${j}px;height:${y}px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border:2px solid #87CEEB;border-radius:10px;cursor:pointer;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:${Math.round(y*.04)}px;text-align:center;flex-shrink:0">
          ...
          </div>`},O=(j,y)=>y?k(110,150):R(j,110,150),F=(j,y)=>y?k(68,95):R(j,68,95),U=b?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",M=g?`<button id="btn-results" style="${U};background:linear-gradient(135deg,#D4A017,#FFD700);border:none;color:#000">🏁 Résultats</button>`:r?`<div style="${U};background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);color:rgba(255,255,255,0.4)">⏳ Tour IA</div>`:u?`<button id="btn-action" style="${U};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${t.selected.length===0?"disabled":""}> ⚔️ ATTAQUEZ <span id="match-timer" style="font-weight:900"></span></button>`:f?`<button id="btn-action" style="${U};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${t.selected.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="match-timer" style="font-weight:900"></span></button>`:`<div style="${U};background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1)"></div>`,B=u||f?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${t.selected.length}/3 sélectionné(s)</div>`:"",C=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${b?56:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
        ${s.length===0?'<div style="font-size:7px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':s.map(j=>`<div class="sub-btn-col" data-sub-id="${j.cardId}" style="cursor:pointer;flex-shrink:0">${pe(j,44,58)}</div>`).join("")}
        ...
        <div class="terrain-wrapper" style="overflow:hidden;flex-shrink:0;display:flex;align-items:center;justify-content:center">
          ${Le(t.homeTeam,t.formation,t.phase,n,300,300,o)}
        </div>
      </div>`;return b?`
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(b=>{var R,k,O;if(b.type==="duel"){const F=b.isGoal,U=b.homeScored?"#FFD700":F?"#ff6b6b":"rgba(255,255,255,0.3)",M=b.homeScored?"⚽ BUT !":F?"⚽ BUT IA !":(R=b.homePlayers)!=null&&R.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${F?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${U};margin-bottom:4px">
                <div style="font-size:9px;color:${U};letter-spacing:1px;margin-bottom:5px;font-weight:700;text-transform:uppercase">${M}</div>
                ${(k=b.homePlayers)!=null&&k.length?`<div style="margin-bottom:3px">${Ee(b.homePlayers,"rgba(255,255,255,0.7)",b.homeTotal)}</div>`:""}
                ${(O=b.aiPlayers)!=null&&O.length?`<div style="opacity:0.7">${Ee(b.aiPlayers,"#ff6b6b",b.aiTotal)}</div>`:""}
  ...
  </div>`;function x(){const b=e.querySelector(".match-screen");if(!b)return;const R=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);b.style.bottom="auto",b.style.height=R+"px",b.style.minHeight=R+"px",b.style.maxHeight=R+"px",b.style.overflow="hidden";const k=e.querySelector("#mobile-action-bar"),O=e.querySelector("#mobile-play-area");k&&O&&(O.style.paddingBottom=k.offsetHeight+"px")}if(x(),setTimeout(x,120),setTimeout(x,400),setTimeout(x,1e3),t._vvBound||(t._vvBound=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",x),window.visualViewport.addEventListener("scroll",x)),window.addEventListener("resize",x)),function(){const R=e.querySelector(".terrain-wrapper svg");R&&(R.removeAttribute("width"),R.removeAttribute("height"),R.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",R.setAttribute("viewBox","-26 -26 352 352"),R.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),t._resizeBound||(t._resizeBound=!0,window.addEventListener("resize",()=>{const b=e.querySelector(".terrain-wrapper svg");b&&(b.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0")})),t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase==="attack"||t.phase==="defense"){let b=!1,R=30;const k=()=>document.getElementById("match-timer"),O=()=>{const F=k();if(!F)return;const U=String(Math.floor(R/60)).padStart(2,"0"),M=String(R%60).padStart(2,"0");F.textContent=` ${U}:${M}`,F.style.color=b?"#ff2222":"#ff9500",F.style.fontWeight="900"};O(),t._timerInt=setInterval(()=>{if(R--,R<0)if(!b)b=!0,R=15,O();else{clearInterval(t._timerInt),t._timerInt=null,t.homeScore=0,t.aiScore=3;const F=document.createElement("div");F.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;color:#fff;padding:24px;text-align:center",F.innerHTML='<div style="font-size:56px">⏱️</div><div style="font-size:24px;font-weight:900;color:#ff4444">MATCH PERDU PAR FORFAIT</div><div style="font-size:14px;color:rgba(255,255,255,0.6)">Temps écoulé</div>',document.body.appendChild(F),setTimeout(()=>{F.remove(),Ae(e,t,i)},2500)}else O()},1e3)}(v=document.getElementById("match-quit"))==null||v.addEventListener("click",()=>{de(e),confirm("Abandonner ? Résultat : défaite 3-0")&&(t.homeScore=0,t.aiScore=3,Ae(e,t,i))}),(_=document.getElementById("view-ai"))==null||_.addEventListener("click",()=>co(t,i)),(h=document.getElementById("toggle-history"))==null||h.addEventListener("click",()=>{var b;(b=document.getElementById("match-history-panel"))==null||b.classList.add("open")}),(L=document.getElementById("close-history"))==null||L.addEventListener("click",()=>{var b;(b=document.getElementById("match-history-panel"))==null||b.classList.remove("open")}),(E=document.getElementById("btn-action"))==null||E.addEventListener("click",()=>{t.selected.length!==0&&(u?to(e,t,i):f&&io(e,t,i))}),(D=document.getElementById("btn-results"))==null||D.addEventListener("click",()=>Ae(e,t,i)),e.querySelectorAll(".match-slot-hit").forEach(b=>{b.addEventListener("click",()=>eo(b,t,e,i))}),e.querySelectorAll(".gc-mini").forEach(b=>{b.addEventListener("click",()=>ro(b.dataset.gcId,b.dataset.gcType,e,t,i))}),(N=document.getElementById("boost-card"))==null||N.addEventListener("click",()=>lo(e,t,i)),e.querySelectorAll(".sub-btn-col").forEach(b=>{b.addEventListener("click",()=>Bt(e,t,i,b.dataset.subId))}),(K=document.getElementById("sub-btn-main"))==null||K.addEventListener("click",()=>Bt(e,t,i))}function eo(e,t,i,n){const d=e.dataset.cardId,s=e.dataset.role,a=t.selected.findIndex(l=>l.cardId===d);if(a!==-1)t.selected.splice(a,1);else{if(t.selected.length>=3){n.toast("Maximum 3 joueurs","error");return}const l=(t.homeTeam[s]||[]).find(c=>c.cardId===d);l&&t.selected.push({...l,_role:s,_line:s})}te(i,t,n)}function dt(e,t,i){e.matchId&&I.from("matches").update({last_player_id:i}).eq("id",e.matchId).then(()=>{})}function to(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),dt(t,i,i.state.profile.id);const n=!["GK","DEF","MIL","ATT"].some(a=>(t.aiTeam[a]||[]).some(l=>!l.used)),d=t.selected.map(a=>{const l=(t.homeTeam[a._role]||[]).find(o=>o.cardId===a.cardId)||a,c=n&&["GK","DEF"].includes(a._role);return{...l,_line:a._role,...c?{note_a:Math.max(1,Number(l.note_a)||0)}:{}}}),s=nt(d,t.modifiers.home);t.pendingAttack={...s,players:[...d],side:"home"},t.selected.forEach(a=>{const l=(t.homeTeam[a._role]||[]).find(c=>c.cardId===a.cardId);l&&(l.used=!0)}),t.log.push({text:`⚔️ Vous attaquez : ${s.total} (base ${s.base}${s.links?` +${s.links} liens`:""}) — ${t.selected.map(a=>a.name).join(", ")}`,type:"info"}),t.selected=[],t.modifiers.home={},t.phase="ai-defense",te(e,t,i),setTimeout(()=>oo(e,t,i),1200)}function io(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),dt(t,i,i.state.profile.id);const n=t.selected.map(l=>({...(t.homeTeam[l._role]||[]).find(o=>o.cardId===l.cardId)||l,_line:l._role})),d=rt(n,t.modifiers.home);t.selected.forEach(l=>{const c=(t.homeTeam[l._role]||[]).find(o=>o.cardId===l.cardId);c&&(c.used=!0)});const s=at(t.pendingAttack.total,d.total,t.modifiers.home),a={type:"duel",title:"Défense",aiPlayers:(t.pendingAttack.players||[]).map(l=>({name:l.name,note:l._line==="MIL"?l.note_m:l.note_a,portrait:re(l),job:l.job,country_code:l.country_code,rarity:l.rarity,clubName:l.clubName,clubLogo:l.clubLogo})),homePlayers:t.selected.map(l=>{const c=(t.homeTeam[l._role]||[]).find(o=>o.cardId===l.cardId)||l;return{name:c.name,note:(c._line==="GK"?Number(c.note_g)||0:c._line==="MIL"?Number(c.note_m)||0:Number(c.note_d)||0)+(c.boost||0),portrait:re(c),job:c.job,country_code:c.country_code,rarity:c.rarity,clubName:c.clubName,clubLogo:c.clubLogo}}),homeTotal:d.total,aiTotal:t.pendingAttack.total,isGoal:!1,homeScored:!1,text:""};if(s.shielded)a.text="🛡️ Bouclier ! But annulé.",t.log.push(a);else if(s.goal){t.aiScore++,a.isGoal=!0,a.homeScored=!1,a.text=`⚽ BUT IA ! (${t.pendingAttack.total} > ${d.total})`,t.log.push(a),t.selected=[],t.modifiers.home={},t.pendingAttack=null,te(e,t,i),He(a.aiPlayers,t.homeScore,t.aiScore,!1,()=>{ve(e,t,i,"home-attack")});return}else a.text=`🧤 Défense réussie ! (${d.total} ≥ ${t.pendingAttack.total})`,t.log.push(a);t.selected=[],t.modifiers.home={},t.pendingAttack=null,ve(e,t,i,"home-attack")}function et(e,t,i){dt(t,i,null);const n=[...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]],d=Nt(n,"attack",t.difficulty);if(!d.length){Fe(e,t,i);return}const s=nt(d,t.modifiers.ai);t.pendingAttack={...s,players:d,side:"ai"},d.forEach(o=>{o.used=!0}),t.log.push({text:`🤖 IA attaque : ${s.total} (${d.map(o=>o.name).join(", ")})`,type:"info"}),t.modifiers.ai={};const a=[...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(o=>!o.used),c=(t.homeSubs||[]).filter(o=>!(t.usedSubIds||[]).includes(o.cardId)).length>0&&t.subsUsed<t.maxSubs;if(a.length===0&&!c){t.aiScore++;const o={type:"duel",isGoal:!0,homeScored:!1,aiPlayers:d.map(r=>({name:r.name,note:r._line==="MIL"?r.note_m:r.note_a,portrait:re(r),job:r.job,country_code:r.country_code,rarity:r.rarity,clubName:r.clubName,clubLogo:r.clubLogo})),aiTotal:s.total,text:"⚽ BUT IA ! (aucun défenseur disponible)"};t.log.push(o),t.pendingAttack=null,te(e,t,i),He(o.aiPlayers,t.homeScore,t.aiScore,!1,()=>{ve(e,t,i,"home-attack")});return}t.phase="defense",te(e,t,i)}function oo(e,t,i){var o,r;const n=[...t.aiTeam.GK||[],...t.aiTeam.DEF||[],...t.aiTeam.MIL||[]],d=Nt(n,"defense",t.difficulty);if(!["GK","DEF","MIL","ATT"].flatMap(u=>(t.aiTeam[u]||[]).filter(f=>!f.used)).length){t.homeScore++;const f={type:"duel",isGoal:!0,homeScored:!0,homePlayers:(((o=t.pendingAttack)==null?void 0:o.players)||[]).map(g=>({name:g.name,note:ie(g,g._line||g.job),portrait:re(g),job:g.job,country_code:g.country_code,rarity:g.rarity,clubName:g.clubName,clubLogo:g.clubLogo})),homeTotal:((r=t.pendingAttack)==null?void 0:r.total)||0,aiTotal:0,text:"⚽ BUT ! L'IA n'a plus de joueurs — but automatique !"};t.log.push(f),t.modifiers.ai={},t.pendingAttack=null,te(e,t,i),He(f.homePlayers,t.homeScore,t.aiScore,!0,()=>{ve(e,t,i,"ai-attack")});return}const a=d.length>0?rt(d,t.modifiers.ai).total:0;d.forEach(u=>{u.used=!0});const l=at(t.pendingAttack.total,a,t.modifiers.ai),c={type:"duel",title:"Attaque",homePlayers:(t.pendingAttack.players||[]).map(u=>({name:u.name,note:u._line==="MIL"?u.note_m:u.note_a,portrait:re(u),job:u.job,country_code:u.country_code,rarity:u.rarity,clubName:u.clubName,clubLogo:u.clubLogo})),aiPlayers:d.map(u=>({name:u.name,note:u._line==="GK"?u.note_g:u._line==="MIL"?u.note_m:u.note_d,portrait:re(u),job:u.job,country_code:u.country_code,rarity:u.rarity,clubName:u.clubName,clubLogo:u.clubLogo})),homeTotal:t.pendingAttack.total,aiTotal:a,isGoal:!1,homeScored:!1,text:""};if(l.shielded)c.text="🛡️ Bouclier IA !",t.log.push(c);else if(l.goal){t.homeScore++,c.isGoal=!0,c.homeScored=!0,c.text=`⚽ BUT ! (${t.pendingAttack.total} > ${a})`,t.log.push(c),t.modifiers.ai={},t.pendingAttack=null,te(e,t,i),He(c.homePlayers,t.homeScore,t.aiScore,!0,()=>{ve(e,t,i,"ai-attack")});return}else c.text=`🧤 IA défend (${a} ≥ ${t.pendingAttack.total})`,t.log.push(c);t.modifiers.ai={},t.pendingAttack=null,ve(e,t,i,"ai-attack")}function ve(e,t,i,n){if(t.round++,ei(t)){Ae(e,t,i);return}if(n==="home-attack"){if(![...t.homeTeam.MIL||[],...t.homeTeam.ATT||[]].filter(s=>!s.used).length){if(![...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(l=>!l.used).length){Fe(e,t,i);return}if(![...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]].filter(l=>!l.used).length){Fe(e,t,i);return}setTimeout(()=>et(e,t,i),100);return}t.phase="attack",te(e,t,i)}else{if(![...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]].filter(s=>!s.used).length){Fe(e,t,i);return}t.phase="ai-attack",te(e,t,i),setTimeout(()=>et(e,t,i),800)}}function ei(e){const t=["MIL","ATT","GK","DEF"].some(n=>(e.homeTeam[n]||[]).some(d=>!d.used)),i=["MIL","ATT","GK","DEF"].some(n=>(e.aiTeam[n]||[]).some(d=>!d.used));return!t&&!i}function Fe(e,t,i){ei(t)?Ae(e,t,i):(t.phase="attack",te(e,t,i))}function no(e,t,i){const n=document.createElement("div");n.style.cssText=`
    position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:800;
    display:flex;flex-direction:column;align-items:center;justify-content:center;gap:20px;
    animation:subFadeIn 0.2s ease;
  `;const d=re(e),s=re(t),a=_e[e.job]||"#555",l=_e[t.job]||"#555",c=e.job==="GK"?e.note_g:e.job==="DEF"?e.note_d:e.job==="MIL"?e.note_m:e.note_a,o=t.job==="GK"?t.note_g:t.job==="DEF"?t.note_d:t.job==="MIL"?t.note_m:t.note_a;n.innerHTML=`
    <style>
  ...
  `,document.body.appendChild(n);let r=!1;const u=()=>{r||(r=!0,n.remove(),i())};n.addEventListener("click",u),setTimeout(u,2e3)}function ke(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function Bt(e,t,i,n=null){if(t.phase!=="attack"){ke("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){ke(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const d=Object.values(t.homeTeam).flat().filter(u=>u.used),s=t.homeSubs.filter(u=>!t.usedSubIds.includes(u.cardId));if(!d.length){ke("Aucun joueur utilisé à remplacer");return}if(!s.length){ke("Aucun remplaçant disponible");return}let a=0,l=Math.max(0,s.findIndex(u=>u.cardId===n)),c=!1;const o=document.createElement("div");o.id="sub-overlay",o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function r(){var z,v,_,h,L,E;const u=d[a],f=s[l],g=Math.min(130,Math.round((window.innerWidth-90)/2)),w=Math.round(g*1.35),p=D=>`background:rgba(255,255,255,0.12);border:none;color:${D?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${D?"default":"pointer"};flex-shrink:0`;o.innerHTML=`
    ...
    </div>`,(z=o.querySelector("#sub-close"))==null||z.addEventListener("click",()=>o.remove()),(v=o.querySelector("#out-up"))==null||v.addEventListener("click",()=>{a>0&&(a--,r())}),(_=o.querySelector("#out-down"))==null||_.addEventListener("click",()=>{a<d.length-1&&(a++,r())}),(h=o.querySelector("#in-up"))==null||h.addEventListener("click",()=>{l>0&&(l--,r())}),(L=o.querySelector("#in-down"))==null||L.addEventListener("click",()=>{l<s.length-1&&(l++,r())});const x=(D,N,K,b)=>{const R=o.querySelector("#"+D);if(!R)return;let k=0;R.addEventListener("touchstart",O=>{k=O.touches[0].clientY},{passive:!0}),R.addEventListener("touchend",O=>{const F=O.changedTouches[0].clientY-k;if(Math.abs(F)<30)return;const U=N();F<0&&U<b-1?(K(U+1),r()):F>0&&U>0&&(K(U-1),r())},{passive:!0})};x("in-panel",()=>l,D=>l=D,s.length),x("out-panel",()=>a,D=>a=D,d.length),(E=o.querySelector("#sub-confirm"))==null||E.addEventListener("click",D=>{if(D.preventDefault(),D.stopPropagation(),c)return;c=!0;const N=d[a],K=s[l];if(!N||!K)return;let b=null,R=-1;for(const[O,F]of Object.entries(t.homeTeam)){const U=(F||[]).findIndex(M=>M.cardId===N.cardId);if(U!==-1){b=O,R=U;break}}if(R===-1||!b){ke("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),o.remove();return}const k={...K,_line:b,_col:N._col||0,used:!1,boost:0};t.homeTeam[b].splice(R,1,k),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(K.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:N.name,firstname:N.firstname,note:ie(N,b),portrait:re(N),job:N.job,country_code:N.country_code,rarity:N.rarity,clubName:N.clubName,clubLogo:N.clubLogo},inPlayer:{name:K.name,firstname:K.firstname,note:ie(K,b),portrait:re(K),job:K.job,country_code:K.country_code,rarity:K.rarity,clubName:K.clubName,clubLogo:K.clubLogo},text:`🔄 ${K.firstname} ${K.name} remplace ${N.firstname} ${N.name}`}),o.remove(),no(N,K,()=>te(e,t,i))})}document.body.appendChild(o),r()}function ro(e,t,i,n,d){var w,p;const s=(n.gcDefs||[]).find(x=>x.name===t),a=le[t]||{icon:"⚡",desc:"Carte spéciale."},l={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[s==null?void 0:s.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",c={purple:"#b06ce0",light_blue:"#00d4ef"}[s==null?void 0:s.color]||"#b06ce0",o=(s==null?void 0:s.name)||t,r=(s==null?void 0:s.effect)||a.desc,u=s!=null&&s.image_url?`/manager-wars/icons/${s.image_url}`:null,f=a.icon||"⚡",g=document.createElement("div");g.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",g.innerHTML=`
    ...
    </div>`,document.body.appendChild(g),(w=g.querySelector("#gc-back"))==null||w.addEventListener("click",()=>g.remove()),(p=g.querySelector("#gc-use"))==null||p.addEventListener("click",()=>{g.remove(),so(e,t,i,n,d)})}function Be(e,t,i,n,d,s){const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let l=[];function c(){var o,r;a.innerHTML=`
    ...
    </div>`,(o=a.querySelector("#gc-picker-close"))==null||o.addEventListener("click",()=>a.remove()),a.querySelectorAll(".gc-pick-item").forEach(u=>{u.addEventListener("click",()=>{const f=u.dataset.cid,g=e.find(p=>p.cardId===f);if(!g)return;const w=l.findIndex(p=>p.cardId===f);w>-1?l.splice(w,1):l.length<t&&l.push(g),c()})}),(r=a.querySelector("#gc-picker-confirm"))==null||r.addEventListener("click",()=>{a.remove(),s(l)})}c(),document.body.appendChild(a)}const ao={BOOST_STAT:({value:e=1,count:t=1,roles:i=[]},n,d,s)=>{const a=Object.entries(n.homeTeam).filter(([l])=>!i.length||i.includes(l)).flatMap(([l,c])=>c.filter(o=>!o.used).map(o=>({...o,_line:l})));return a.length?(Be(a,t,`Choisir ${t} joueur(s) à booster (+${e})`,d,n,l=>{l.forEach(c=>{const o=(n.homeTeam[c._line]||[]).find(r=>r.cardId===c.cardId);o&&(o.boost=(o.boost||0)+e,n.log.push({text:`⚡ +${e} sur ${o.name}`,type:"info"}))}),te(d,n,s)}),!0):(n.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),te(d,n,s),!0)},DEBUFF_STAT:({value:e=1,count:t=1,roles:i=[],target:n="ai"},d,s,a)=>{const l=n==="home"?d.homeTeam:d.aiTeam,c=n==="ai"?"adverse":"allié",o=Object.entries(l).filter(([r])=>!i.length||i.includes(r)).flatMap(([r,u])=>u.filter(f=>!f.used).map(f=>({...f,_line:r})));return o.length?(Be(o,t,`Choisir ${t} joueur(s) ${c}(s) à débuffer (-${e})`,s,d,r=>{r.forEach(u=>{const g=((n==="home"?d.homeTeam:d.aiTeam)[u._line]||[]).find(w=>w.cardId===u.cardId);g&&(g.boost=(g.boost||0)-e,d.log.push({text:`🎯 -${e} sur ${g.name}${n==="ai"?" (IA)":""}`,type:"info"}))}),te(s,d,a)}),!0):(d.log.push({text:`🎯 Aucun joueur ${c} disponible`,type:"info"}),te(s,d,a),!0)},GRAY_PLAYER:({count:e=1,roles:t=[],target:i="ai"},n,d,s)=>{const a=i==="home"?n.homeTeam:n.aiTeam,l=i==="ai"?"adverse":"allié",c=Object.entries(a).filter(([o])=>!t.length||t.includes(o)).flatMap(([o,r])=>r.filter(u=>!u.used).map(u=>({...u,_line:o})));return c.length?(Be(c,e,`Choisir ${e} joueur(s) ${l}(s) à exclure`,d,n,o=>{o.forEach(r=>{const f=((i==="home"?n.homeTeam:n.aiTeam)[r._line]||[]).find(g=>g.cardId===r.cardId);f&&(f.used=!0,n.log.push({text:`❌ ${f.name}${i==="ai"?" (IA)":""} exclu !`,type:"info"}))}),te(d,n,s)}),!0):(n.log.push({text:`❌ Aucun joueur ${l} à exclure`,type:"info"}),te(d,n,s),!0)},REVIVE_PLAYER:({count:e=1,roles:t=[]},i,n,d)=>{const s=Object.entries(i.homeTeam).filter(([a])=>!t.length||t.includes(a)).flatMap(([a,l])=>l.filter(c=>c.used).map(c=>({...c,_line:a})));return s.length?(Be(s,e,`Choisir ${e} joueur(s) à ressusciter`,n,i,a=>{a.forEach(l=>{const c=(i.homeTeam[l._line]||[]).find(o=>o.cardId===l.cardId);c&&(c.used=!1,i.log.push({text:`💫 ${c.name} ressuscité !`,type:"info"}))}),te(n,i,d)}),!0):(i.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),te(n,i,d),!0)},REMOVE_GOAL:({},e)=>(e.aiScore>0?(e.aiScore--,e.log.push({text:"🚫 Dernier but IA annulé !",type:"info"})):e.log.push({text:"🚫 Aucun but à annuler",type:"info"}),!1),ADD_GOAL_DRAW:({},e)=>(e.homeScore===e.aiScore?(e.homeScore++,e.log.push({text:"🎯 But bonus (match nul) !",type:"info"})):e.log.push({text:"🎯 But bonus : non applicable (pas de match nul)",type:"info"}),!1),ADD_SUB:({value:e=1},t)=>(t.maxSubs=(t.maxSubs||3)+e,t.log.push({text:`🔄 +${e} remplacement(s) débloqué(s)`,type:"info"}),!1),CUSTOM:()=>!1};function so(e,t,i,n,d){n.usedGc.push(e);const s=n.gcDefs||[],a=s.find(c=>c.name===t)||s.find(c=>{var o;return((o=c.name)==null?void 0:o.toLowerCase().trim())===(t==null?void 0:t.toLowerCase().trim())});let l=!1;if(a!=null&&a.effect_type&&a.effect_type!=="CUSTOM"){const c=ao[a.effect_type];c?c(a.effect_params||{},n,i,d)||(l=!0):(d.toast(`Effet "${a.effect_type}" non implémenté`,"error"),l=!0)}else{switch(t){case"Double attaque":n.modifiers.home.doubleAttack=!0,n.log.push({text:"⚡ Double attaque activée !",type:"info"});break;case"Bouclier":n.modifiers.home.shield=!0,n.log.push({text:"🛡️ Bouclier activé !",type:"info"});break;case"Ressusciter":{const c=Object.entries(n.homeTeam).flatMap(([o,r])=>(r||[]).filter(u=>u.used).map(u=>({...u,_line:o})));c.length?(c[0].used=!1,n.log.push({text:`💫 ${c[0].name} ressuscité !`,type:"info"})):n.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"});break}case"Vol de note":n.modifiers.ai.stolenNote=(n.modifiers.ai.stolenNote||0)+1,n.log.push({text:"🎯 -1 à la prochaine attaque IA",type:"info"});break;case"Gel":{const c=[...n.aiTeam.ATT||[],...n.aiTeam.MIL||[]].filter(o=>!o.used);if(c.length){const o=c.sort((r,u)=>ie(u,"ATT")-ie(r,"ATT"))[0];o.used=!0,n.log.push({text:`❄️ ${o.name} (IA) gelé !`,type:"info"})}break}case"Remplacement+":n.maxSubs++,n.log.push({text:"🔄 +1 remplacement débloqué",type:"info"});break}l=!0}I.from("cards").delete().eq("id",e).then(()=>{}),l&&te(i,n,d)}function lo(e,t,i){const n=Object.values(t.homeTeam).flat().filter(d=>!d.used);if(!n.length){i.toast("Aucun joueur actif à booster","error");return}i.openModal("⚡ Utiliser le Boost",`<div style="margin-bottom:12px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border-radius:10px;padding:12px;text-align:center;color:#000">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(d=>{d.addEventListener("click",()=>{const s=d.dataset.cardId;for(const a of["GK","DEF","MIL","ATT"]){const l=(t.homeTeam[a]||[]).find(c=>c.cardId===s);if(l){l.boost=(l.boost||0)+t.boostCard.value,t.log.push({text:`⚡ Boost +${t.boostCard.value} appliqué à ${l.name}`,type:"info"});break}}t.boostUsed=!0,i.closeModal(),te(e,t,i)})})}function He(e,t,i,n,d){const s=document.createElement("div");s.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:900;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;overflow:hidden;cursor:pointer";const a=Array.from({length:10},(o,r)=>`
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(s);let l=!1;const c=()=>{l||(l=!0,s.remove(),setTimeout(()=>d(),50))};s.addEventListener("click",c),setTimeout(c,3500)}async function Ae(e,t,i){var r,u;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:n}=i,d=t.homeScore>t.aiScore,s=t.homeScore===t.aiScore,a=d?"victoire":s?"nul":"defaite",l=fi(t.mode,a);t.matchId&&await I.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:d?n.profile.id:null,home_credits_reward:l,played_at:new Date().toISOString()}).eq("id",t.matchId);const c={credits:(n.profile.credits||0)+l,matches_played:(n.profile.matches_played||0)+1};d?c.wins=(n.profile.wins||0)+1:s?c.draws=(n.profile.draws||0)+1:c.losses=(n.profile.losses||0)+1,await I.from("users").update(c).eq("id",n.profile.id),await i.refreshProfile();const o=document.createElement("div");o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",o.innerHTML=`
    ...
    </div>`,`<button class="btn btn-primary" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}async function po(e,t){const{state:i}=t;await Zt(e,t,"random",({deckId:d,formation:s,starters:a,subsRaw:l,gcCardsEnriched:c,gcDefs:o})=>{const r=u=>{uo(e,t,d,s,a,l,u,o||[])};if(!c.length){r([]);return}Xt(e,c,r)})}async function uo(e,t,i,n,d,s,a=[],l=[]){var v;const{state:c,navigate:o,toast:r}=t;let u=!1,f=null;e.style.overflow="hidden",e.innerHTML=`
    ...
    <style>@keyframes mmspin{to{transform:rotate(360deg)}}</style>`;const g=()=>document.getElementById("mm-status"),w=async(_=!0)=>{u=!0,f&&(I.removeChannel(f),f=null),_&&await I.rpc("cancel_matchmaking",{p_user_id:c.profile.id}).catch(()=>{})};(v=document.getElementById("mm-cancel"))==null||v.addEventListener("click",async()=>{try{await w(!0)}catch(_){console.warn("[Matchmaking] cleanup error:",_)}de(e),o("home")});const p=async(_,h,L)=>{u||(u=!0,f&&(I.removeChannel(f),f=null),g()&&(g().textContent="Adversaire trouvé !"),await new Promise(E=>setTimeout(E,600)),e.isConnected!==!1&&fo(e,t,_,L,a,l))},{data:x,error:z}=await I.rpc("try_matchmake",{p_user_id:c.profile.id,p_deck_id:i});if(z||!(x!=null&&x.success)){r("Erreur de matchmaking","error"),de(e),o("home");return}if(x.matched){p(x.match_id,x.opponent_id,!1);return}g()&&(g().textContent="En attente d'un autre joueur..."),f=I.channel("matchmaking-"+c.profile.id).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matchmaking_queue",filter:`user_id=eq.${c.profile.id}`},_=>{const h=_.new;h.status==="matched"&&h.match_id&&p(h.match_id,h.matched_with,!0)}).subscribe()}async function fo(e,t,i,n,d=[],s=[]){const{state:a,navigate:l,toast:c}=t,o=n?"p1":"p2",r=n?"p2":"p1",u=(d||[]).map(M=>M.id),f=(d||[]).map(M=>({id:M.id,gc_type:M.gc_type,_gcDef:M._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation du match...</div>';const{data:g}=await I.from("matches").select("*").eq("id",i).single();if(!g){c("Match introuvable","error"),l("home");return}async function w(){const[{data:M,error:B},{data:C,error:P},{data:j},{data:y}]=await Promise.all([I.rpc("get_deck_for_match",{p_deck_id:g.home_deck_id}),I.rpc("get_deck_for_match",{p_deck_id:g.away_deck_id}),I.from("users").select("id,pseudo,club_name").eq("id",g.home_id).single(),I.from("users").select("id,pseudo,club_name").eq("id",g.away_id).single()]);B&&console.error("[PvP] get_deck_for_match p1 error:",B),P&&console.error("[PvP] get_deck_for_match p2 error:",P);function m(q,Y){return{cardId:q.card_id,position:Y||q.position,id:q.id,firstname:q.firstname,name:q.surname_encoded,country_code:q.country_code,club_id:q.club_id,job:q.job,job2:q.job2,note_g:Number(q.note_g)||0,note_d:Number(q.note_d)||0,note_m:Number(q.note_m)||0,note_a:Number(q.note_a)||0,rarity:q.rarity,skin:q.skin,hair:q.hair,hair_length:q.hair_length,clubName:q.club_encoded_name||null,clubLogo:q.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}}const $=((M==null?void 0:M.starters)||[]).map(q=>m(q,q.position)),T=((C==null?void 0:C.starters)||[]).map(q=>m(q,q.position)),A=((M==null?void 0:M.subs)||[]).map(q=>m(q,q.position)),G=((C==null?void 0:C.subs)||[]).map(q=>m(q,q.position)),S=(M==null?void 0:M.formation)||"4-4-2",H=(C==null?void 0:C.formation)||"4-4-2";return console.log("[PvP] p1Formation:",S,"p1 starters:",$.length,"positions:",$.map(q=>q.position)),console.log("[PvP] p2Formation:",H,"p2 starters:",T.length,"positions:",T.map(q=>q.position)),{p1Team:Pe($,S),p2Team:Pe(T,H),p1Subs:A,p2Subs:G,p1Formation:S,p2Formation:H,p1Name:(j==null?void 0:j.club_name)||(j==null?void 0:j.pseudo)||"Joueur 1",p2Name:(y==null?void 0:y.club_name)||(y==null?void 0:y.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:n?u:[],gc_p2:n?[]:u,gcCardsFull_p1:n?f:[],gcCardsFull_p2:n?[]:f,gcReady_p1:!0,gcReady_p2:!0,usedGc_p1:[],usedGc_p2:[],lastActionAt:new Date().toISOString()}}let p=g.game_state&&Object.keys(g.game_state).length?g.game_state:null;if(!p)if(n){p=await w();const{data:M}=await I.from("matches").select("game_state").eq("id",i).single();!(M!=null&&M.game_state)||!Object.keys(M.game_state).length?await I.from("matches").update({game_state:p,turn_user_id:g.home_id}).eq("id",i):p=M.game_state}else{e.innerHTML=`<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation avec l'adversaire...</div>`;for(let M=0;M<30&&!p;M++){await new Promise(C=>setTimeout(C,400));const{data:B}=await I.from("matches").select("game_state").eq("id",i).single();B!=null&&B.game_state&&Object.keys(B.game_state).length&&(p=B.game_state)}if(!p){c("Erreur de synchronisation","error"),l("home");return}p.gc_p2=u,p.gcCardsFull_p2=f,await I.from("matches").update({game_state:p}).eq("id",i)}let x=!1;function z(M){var $;try{I.removeChannel(v)}catch{}const B=a.profile.id,C=M.winner_id===B,P=!!M.forfeit,j=p[o+"Score"]??0,y=p[r+"Score"]??0,m=document.createElement("div");m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;color:#fff;padding:24px;text-align:center",m.innerHTML=`
      ...
      ${q.length===0?'<div style="font-size:7px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':q.map(V=>`<div class="pvp-sub-btn" data-sub-id="${V.cardId}" style="cursor:pointer;flex-shrink:0">${pe(V,G?60:44,G?78:58)}</div>`).join("")}
    </div>`,me=`<div style="overflow:hidden;min-width:0;flex:1;min-height:0" id="match-field">
      <div class="terrain-wrapper" style="width:100%;height:100%;overflow:hidden">
        ${Le(M,p[o+"Formation"],y?"attack":m?"defense":"idle",A,300,300)}
      </div>
    </div>`,xe=G?"padding:14px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:10px 8px;border-radius:12px;font-size:13px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",ze=y?`<button id="pvp-action" style="${xe};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${T.length===0?"disabled":""}>⚔️ ATTAQUEZ <span id="pvp-timer"></span></button>`:m?`<button id="pvp-action" style="${xe};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${T.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="pvp-timer"></span></button>`:`<div style="${xe};background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);color:rgba(255,255,255,0.4)">⏳ Tour de ${j}</div>`,ut=y||m?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${T.length}/3 sélectionné(s)</div>`:"",oi=(()=>{const V=p.pendingAttack;if(m&&V&&Array.isArray(V.players))return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
          ...
          ${Ee(V.players.map(ce=>({...ce,used:!1})),"#ff6b6b",V.total)}
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 14px;background:rgba(0,0,0,0.25);flex-shrink:0">
        ...
        <div style="flex:1;text-align:center;color:#fff;font-size:14px;font-weight:700">
          ${P} <span style="color:#FFD700;font-size:18px">${B}</span> – <span style="color:#FFD700;font-size:18px">${C}</span> ${j}
        </div>
      ...
      <div style="text-align:center;padding:4px;background:rgba(0,0,0,0.15);font-size:11px;color:${$?"rgba(255,255,255,0.4)":"#FFD700"};font-weight:700;flex-shrink:0">
        ${$?`⏳ Tour de ${j}`:y?"⚔️ À vous d'attaquer !":m?"🛡️ À vous de défendre !":""}
      </div>
      <!-- Zone actions -->
      ...
      </div>`;const V=e.querySelector(".terrain-wrapper");V&&(V.innerHTML=Le(M,p[o+"Formation"],y?"attack":m?"defense":"idle",A,300,300))}if(e.querySelectorAll(".match-slot-hit").forEach(V=>{V.addEventListener("click",()=>{if(!y&&!m)return;const Q=V.dataset.cardId,ee=V.dataset.role,X=(M[ee]||[]).find(ye=>ye.cardId===Q);if(!X||X.used)return;Array.isArray(p["selected_"+o])||(p["selected_"+o]=[]);const ce=p["selected_"+o],Ce=ce.findIndex(ye=>ye.cardId===Q);Ce>-1?ce.splice(Ce,1):ce.length<3&&ce.push({...X,_role:ee}),L()})}),(gt=e.querySelector("#pvp-boost-card"))==null||gt.addEventListener("click",()=>{K()}),e.querySelectorAll(".pvp-gc-mini").forEach(V=>{V.addEventListener("click",()=>{k(V.dataset.gcId,V.dataset.gcType)})}),(mt=e.querySelector("#pvp-action"))==null||mt.addEventListener("click",()=>{y?F():m&&U()}),(xt=e.querySelector("#pvp-quit"))==null||xt.addEventListener("click",()=>{confirm("Quitter le match ? Vous perdrez par forfait.")&&h()}),(yt=e.querySelector("#pvp-view-opp"))==null||yt.addEventListener("click",()=>{R()}),(ht=e.querySelector("#pvp-toggle-history"))==null||ht.addEventListener("click",()=>{b()}),function(){const Q=e.querySelector(".terrain-wrapper svg");Q&&(Q.removeAttribute("width"),Q.removeAttribute("height"),Q.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",Q.setAttribute("viewBox","-26 -26 352 352"),Q.setAttribute("preserveAspectRatio","xMidYMid meet"));const ee=e.querySelector(".match-screen");if(ee){const X=e.clientHeight;X>50&&(ee.style.height=X+"px",ee.style.maxHeight=X+"px",ee.style.overflow="hidden")}}(),p._timerInt&&(clearInterval(p._timerInt),p._timerInt=null),(y||m)&&!x){let V=30,Q=!1;const ee=()=>document.getElementById("pvp-timer"),X=()=>{ee()&&(ee().textContent=V+"s",ee().style.color=Q?"#ff4444":"#fff")};X(),p._timerInt=setInterval(()=>{V--,V<0?Q?(clearInterval(p._timerInt),p._timerInt=null,h()):(Q=!0,V=15,X()):X()},1e3)}}function E(){var m;const M=p["gcReady_"+o],B=p["gcReady_"+r];Array.isArray(p["gc_"+o])||(p["gc_"+o]=[]);let C=p["gc_"+o];const P=3;function j($,T){const A=$._gcDef,G={purple:"linear-gradient(135deg,#3d0a7a,#7a28b8)",light_blue:"linear-gradient(135deg,#006080,#00bcd4)"},S={purple:"#9b59b6",light_blue:"#00bcd4"},H=G[A==null?void 0:A.color]||G.purple,q=S[A==null?void 0:A.color]||S.purple;return`<div class="pvp-gc-card" data-id="${$.id}"
        style="width:100px;border-radius:10px;border:3px solid ${T?"#FFD700":q};background:${H};
          display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0;position:relative;
    ...
    </div>`,M)return;const y=o==="p1"?g.home_id:g.away_id;I.from("cards").select("id,gc_type").eq("owner_id",y).eq("card_type","game_changer").then(async({data:$})=>{const{data:T}=await I.from("gc_definitions").select("*").eq("is_active",!0),A=new Set,G=($||[]).filter(H=>A.has(H.gc_type)?!1:(A.add(H.gc_type),!0)).map(H=>({...H,_gcDef:(T==null?void 0:T.find(q=>q.name===H.gc_type))||null})),S=document.getElementById("pvp-gc-grid");if(S){if(!G.length){S.innerHTML='<div style="color:rgba(255,255,255,0.4);font-size:13px;text-align:center;margin-top:30px">Aucune carte Game Changer disponible.</div>';return}S.innerHTML=G.map(H=>j(H,C.includes(H.id))).join(""),S.querySelectorAll(".pvp-gc-card").forEach(H=>{H.addEventListener("click",()=>{const q=H.dataset.id,Y=C.indexOf(q);Y>-1?C.splice(Y,1):C.length<P&&C.push(q),L()})})}}),(m=e.querySelector("#pvp-gc-validate"))==null||m.addEventListener("click",async()=>{const $=o==="p1"?g.home_id:g.away_id,{data:T}=await I.from("cards").select("id,gc_type").eq("owner_id",$).eq("card_type","game_changer").in("id",C),{data:A}=await I.from("gc_definitions").select("*").eq("is_active",!0),G=(T||[]).map(S=>({...S,_gcDef:(A==null?void 0:A.find(H=>H.name===S.gc_type))||null}));await _({["gcReady_"+o]:!0,["gcCardsFull_"+o]:G}),p["gcReady_"+r]&&await _({phase:"reveal"})})}function D(){e.style.overflow="hidden",e.innerHTML=`
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:12px;padding:12px 16px;background:#0a3d1e;overflow-y:auto">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:3px;text-transform:uppercase;margin-top:8px">Équipe adverse</div>
      <div style="font-size:20px;font-weight:900;color:#ff6b6b">${p[r+"Name"]}</div>
      <div style="width:min(90vw,420px)">${Te(p[r+"Team"],p[r+"Formation"],null,[],300,300)}</div>
      <div style="font-size:15px;color:rgba(255,255,255,0.7)">
        <span class="loading-dots">Chargement</span>
      </div>
      <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
    </div>`,o==="p1"&&setTimeout(async()=>{await _({phase:"midfield"})},5e3)}function N(){const M=p[o+"Team"].MIL||[],B=p[r+"Team"].MIL||[];function C(T){return T.reduce((A,G)=>A+ie(G,"MIL"),0)}function P(T){let A=0;for(let G=0;G<T.length-1;G++){const S=ge(T[G],T[G+1]);S==="#00ff88"?A+=2:S==="#FFD700"&&(A+=1)}return A}const j=C(M)+P(M),y=C(B)+P(B),m=j>=y;function $(T,A,G){return`<div style="text-align:center">
        <div style="font-size:10px;color:rgba(255,255,255,0.5);letter-spacing:2px;margin-bottom:8px;text-transform:uppercase">${A}</div>
        <div style="display:flex;align-items:center;justify-content:center;gap:0">
          ${T.map((S,H)=>{const q=re(S),Y=H<T.length-1?ge(S,T[H+1]):null,W=Y&&Y!=="#ff3333"&&Y!=="#cc2222";return`
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div id="pvp-midfield-result" style="opacity:0;text-align:center;transition:opacity 0.5s;color:#fff;max-width:320px"></div>
    </div>`,setTimeout(()=>{const T=document.getElementById("pvp-score-me"),A=document.getElementById("pvp-score-opp"),G=document.getElementById("pvp-midfield-result");T&&A&&(m?(T.style.fontSize="80px",T.style.color="#FFD700",A.style.opacity="0.25"):(A.style.fontSize="80px",A.style.color="#ff6b6b",T.style.opacity="0.25"));const S=p.p1Team.MIL||[],H=p.p2Team.MIL||[],q=C(S)+P(S),Y=C(H)+P(H),W=q>=Y?"p1":"p2";p[W+"Name"];let J=p.boostValue;J==null&&(J=Yt(),p.boostValue=J,p.boostOwner=W,p.boostUsed=!1),G&&(G.style.opacity="1",G.innerHTML=`<div style="font-size:20px;font-weight:900;margin-bottom:10px">
        ...
        </div>`:""}`),setTimeout(async()=>{if(o!=="p1")return;const oe=W;await _({phase:oe+"-attack",attacker:oe,round:1,boostValue:p.boostValue,boostUsed:!1,boostOwner:oe})},1800)},600)}function K(){var P;const M=p[o+"Team"],B=Object.entries(M).flatMap(([j,y])=>(y||[]).filter(m=>!m.used).map(m=>({...m,_line:j})));if(!B.length)return;const C=document.createElement("div");C.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden",C.innerHTML=`
      <div style="padding:12px 16px;background:rgba(255,255,255,0.08);display:flex;align-items:center;gap:10px;flex-shrink:0">
      ...
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${B.map(j=>{const y={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[j._line]||"#555",m=ie(j,j._line)+(j.boost||0);return`<div class="boost-pick-item" data-cid="${j.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${y};overflow:hidden;cursor:pointer">
      ...
      </div>`,document.body.appendChild(C),(P=C.querySelector("#boost-picker-close"))==null||P.addEventListener("click",()=>C.remove()),C.querySelectorAll(".boost-pick-item").forEach(j=>{j.addEventListener("click",async()=>{const y=j.dataset.cid,m=B.find(A=>A.cardId===y);if(!m)return;const $=p[o+"Team"],T=($[m._line]||[]).find(A=>A.cardId===y);T&&(T.boost=(T.boost||0)+p.boostValue),C.remove(),await _({[o+"Team"]:$,boostUsed:!0})})})}function b(){var C;const M=Array.isArray(p.log)?p.log:[],B=document.createElement("div");B.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column",B.innerHTML=`
      ...
      </div>`,document.body.appendChild(B),(C=B.querySelector("#pvp-hist-close"))==null||C.addEventListener("click",()=>B.remove())}function R(){var B;const M=document.createElement("div");M.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:800;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:20px;overflow-y:auto",M.innerHTML=`
      <div style="font-size:12px;color:rgba(255,255,255,0.5);letter-spacing:2px;text-transform:uppercase">Équipe adverse</div>
      <div style="font-size:18px;font-weight:900;color:#ff6b6b">${p[r+"Name"]}</div>
      <div style="width:min(90vw,420px)">${Te(p[r+"Team"],p[r+"Formation"],null,[],300,300)}</div>
      <button id="pvp-view-opp-close" style="margin-top:8px;padding:12px 28px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.3);background:transparent;color:#fff;font-size:14px;cursor:pointer">Fermer</button>`,document.body.appendChild(M),(B=M.querySelector("#pvp-view-opp-close"))==null||B.addEventListener("click",()=>M.remove())}function k(M,B){var q,Y;const P=(p["gcCardsFull_"+o]||[]).find(W=>W.id===M),j=P==null?void 0:P._gcDef,y=le[B]||{icon:"⚡",desc:"Carte spéciale."},m={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[j==null?void 0:j.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",$={purple:"#b06ce0",light_blue:"#00d4ef"}[j==null?void 0:j.color]||"#b06ce0",T=(j==null?void 0:j.name)||B,A=(j==null?void 0:j.effect)||y.desc,G=j!=null&&j.image_url?`/manager-wars/icons/${j.image_url}`:null,S=y.icon||"⚡",H=document.createElement("div");H.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",H.innerHTML=`
      ...
      </div>`,document.body.appendChild(H),(q=H.querySelector("#pvp-gc-back"))==null||q.addEventListener("click",()=>H.remove()),(Y=H.querySelector("#pvp-gc-use"))==null||Y.addEventListener("click",async()=>{H.remove();const W=p["usedGc_"+o]||[];W.push(M),await _({["usedGc_"+o]:W})})}function O(){var j;const M=p[o+"Score"],B=p[r+"Score"],C=M>B,P=M===B;e.style.overflow="hidden",e.innerHTML=`
    ...
    </div>`,(j=document.getElementById("pvp-home"))==null||j.addEventListener("click",()=>{I.removeChannel(v),de(e),l("home")})}async function F(){const M=(p["selected_"+o]||[]).map(P=>({...(p[o+"Team"][P._role]||[]).find(y=>y.cardId===P.cardId)||P,_line:P._role}));if(!M.length)return;const B=nt(M,p.modifiers[o]||{});M.forEach(P=>{const j=(p[o+"Team"][P._role]||[]).find(y=>y.cardId===P.cardId);j&&(j.used=!0)});const C=Array.isArray(p.log)?p.log:[];C.push({type:"attack",text:`⚔️ ${p[o+"Name"]} attaque (${B.total})`,players:M,total:B.total,side:o}),await _({pendingAttack:{...B,players:M,side:o},["selected_"+o]:[],modifiers:{...p.modifiers,[o]:{}},phase:r+"-defense",log:C})}async function U(){const M=(p["selected_"+o]||[]).map(G=>({...(p[o+"Team"][G._role]||[]).find(H=>H.cardId===G.cardId)||G,_line:G._role})),B=rt(M,p.modifiers[o]||{});M.forEach(G=>{const S=(p[o+"Team"][G._role]||[]).find(H=>H.cardId===G.cardId);S&&(S.used=!0)});const C=at(p.pendingAttack.total,B.total,p.modifiers[o]||{}),P=p.pendingAttack.side,j=C==="attack",y=p[P+"Score"]+(j?1:0),m=P==="p1"?"p2":"p1",$=(p.round||0)+1,T=$>10,A=Array.isArray(p.log)?p.log:[];A.push({type:"defense",text:`🛡️ ${p[o+"Name"]} défend (${B.total})`,players:M,total:B.total,side:o}),A.push({type:j?"goal":"stop",text:j?`⚽ BUT de ${p[P+"Name"]} ! (${p.pendingAttack.total} vs ${B.total})`:`✋ Attaque stoppée (${p.pendingAttack.total} vs ${B.total})`}),await _({[P+"Score"]:y,["selected_"+o]:[],modifiers:{...p.modifiers,[o]:{}},pendingAttack:null,phase:T?"finished":m+"-attack",attacker:m,round:$,log:A}),T&&await I.from("matches").update({status:"finished"}).eq("id",i)}L()}const go={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"};async function mo(e,t){const{state:i,toast:n}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement du marché...</div>',await ct(e,t)}async function ct(e,t){const{state:i,toast:n}=t,{data:d}=await I.from("market_listings").select(`id, price, status, listed_at, seller_id,
  ...
  `;function c(o){const r=document.getElementById("mkt-content"),u=o==="buy"?a:l;if(u.length===0){r.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      ${f.map(g=>{var _,h,L,E;const w=(_=g.card)==null?void 0:_.player;if(!w)return"";const p=w.job==="GK"?w.note_g:w.job==="DEF"?w.note_d:w.job==="MIL"?w.note_m:w.note_a,x=go[w.rarity],z=(i.profile.credits||0)>=g.price,v=g.status==="sold";return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${v?"opacity:0.65":""}">
          <div style="width:44px;height:44px;border-radius:8px;background:${ho(w.job)};color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:18px;border:2px solid ${x};flex-shrink:0">${p}</div>
          <div style="flex:1;min-width:0">
    ...
    </div>`,r.querySelectorAll("[data-buy]").forEach(g=>{g.addEventListener("click",()=>xo(g.dataset.buy,u,e,t))}),r.querySelectorAll("[data-cancel]").forEach(g=>{g.addEventListener("click",()=>yo(g.dataset.cancel,e,t))})}c("buy"),e.querySelectorAll(".mkt-tab").forEach(o=>{o.addEventListener("click",()=>{e.querySelectorAll(".mkt-tab").forEach(r=>{const u=r===o;r.style.background=u?"var(--green)":"#fff",r.style.color=u?"#fff":"var(--gray-600)",r.style.borderColor=u?"var(--green)":"var(--gray-200)"}),c(o.dataset.tab)})})}async function xo(e,t,i,n){var u;const{state:d,toast:s}=n,a=t.find(f=>f.id===e);if(!a)return;const l=a.price,c=d.profile.credits||0,o=(u=a.card)==null?void 0:u.player;if(c<l){s("Crédits insuffisants","error");return}if(!confirm(`Acheter ${o==null?void 0:o.firstname} ${o==null?void 0:o.surname_encoded} pour ${l.toLocaleString("fr")} crédits ?`))return;const r=document.querySelector(`[data-buy="${e}"]`);r&&(r.disabled=!0,r.textContent="⏳");try{const{data:f,error:g}=await I.rpc("buy_market_card",{p_listing_id:e,p_buyer_id:d.profile.id});if(g)throw new Error(g.message);if(!(f!=null&&f.success))throw new Error((f==null?void 0:f.error)||"Achat impossible");d.profile.credits=c-l;const w=document.querySelector("[data-credits]")||document.querySelector(".credits-display");w&&(w.textContent=(c-l).toLocaleString("fr")+" cr."),s(`✅ ${o==null?void 0:o.surname_encoded} ajouté à ta collection !`,"success"),ct(i,n)}catch(f){s("❌ "+f.message,"error"),r&&(r.disabled=!1,r.textContent="Acheter")}}async function yo(e,t,i){const{toast:n}=i,{data:d}=await I.from("market_listings").select("card_id").eq("id",e).single();await I.from("market_listings").update({status:"cancelled"}).eq("id",e),d&&await I.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",d.card_id),n("Annonce retirée","success"),ct(t,i)}function ho(e){return{GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[e]||"#888"}async function vo(e,{state:t,navigate:i,toast:n}){e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:d}=await I.from("users").select("id,pseudo,club_name,trophies_top1,trophies_top2,trophies_top3,wins,level").order("trophies_top1",{ascending:!1}).limit(100);e.innerHTML=`
  <div class="page">
    <div class="page-header">
  ...
  `}async function bo(e,{state:t,navigate:i,toast:n}){const d=t.profile;if(!d)return;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:s}=await I.from("matches").select(`id,home_id,away_id,home_score,away_score,status,mode,winner_id,created_at,played_at,
