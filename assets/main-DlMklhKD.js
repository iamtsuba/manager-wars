...
import{s as w,F as fi,r as Se,j as Bt,l as Yi,m as qt,h as gi,n as En,o as Ln,T as Qt,k as Tn,p as zn,b as Mn}from"./formation-links-EITwj297.js";const Sn="/";function ji(e,{navigate:t,toast:i}){let n="login";const a=()=>{var r,s,l,c,d,u;const o=n==="login";e.innerHTML=`
  ...
  `;function s(){var f;const c=document.getElementById("logo-preview"),d=document.getElementById("logo-initials"),u=((f=document.getElementById("setup-club"))==null?void 0:f.value)||"MW",g=u.trim().split(" ").filter(Boolean),m=g.length>=2?(g[0][0]+g[1][0]).toUpperCase():u.slice(0,2).toUpperCase();c&&(c.style.background=r,c.style.borderColor=o),d&&(d.textContent=m,d.style.color=o)}document.getElementById("color1").addEventListener("input",c=>{o=c.target.value,document.getElementById("swatch1").style.background=o,s()}),document.getElementById("color2").addEventListener("input",c=>{r=c.target.value,document.getElementById("swatch2").style.background=r,s()});function l(c){document.querySelectorAll(".setup-step").forEach(d=>d.classList.remove("active")),document.getElementById(`step-${c}`).classList.add("active"),document.getElementById("step-num").textContent=c,document.getElementById("progress-fill").style.width=`${Math.round(c/3*100)}%`,c===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),d=document.getElementById("step1-error");if(d.textContent="",c.length<3){d.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:u}=await w.from("users").select("id").eq("pseudo",c).maybeSingle();if(u){d.textContent="Ce pseudo est déjà pris.";return}l(2)}),document.getElementById("step2-back").addEventListener("click",()=>l(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const c=document.getElementById("setup-club").value.trim(),d=document.getElementById("step2-error");if(d.textContent="",c.length<2){d.textContent="Nom trop court (min. 2 caractères).";return}const{data:u}=await w.from("users").select("id").eq("club_name",c).maybeSingle();if(u){d.textContent="Ce nom de club est déjà pris.";return}l(3)}),document.getElementById("step3-back").addEventListener("click",()=>l(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),d=document.getElementById("setup-club").value.trim(),u=document.getElementById("step3-error"),g=document.getElementById("step3-finish");u.textContent="",g.disabled=!0,g.textContent="Création en cours…";try{const{error:m}=await w.from("users").insert({id:t.user.id,pseudo:c,club_name:d,club_color1:o,club_color2:r,credits:1e4});if(m)throw m;await An(t.user.id),await a(),n(`Bienvenue ${c} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(m){u.textContent=m.message,g.disabled=!1,g.textContent="🚀 Créer mon profil !"}})}async function An(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await w.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const jn="modulepreload",Cn=function(e){return"/"+e},Ci={},Wi=function(t,i,n){let a=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const r=document.querySelector("meta[property=csp-nonce]"),s=(r==null?void 0:r.nonce)||(r==null?void 0:r.getAttribute("nonce"));a=Promise.allSettled(i.map(l=>{if(l=Cn(l),l in Ci)return;Ci[l]=!0;const c=l.endsWith(".css"),d=c?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${l}"]${d}`))return;const u=document.createElement("link");if(u.rel=c?"stylesheet":jn,c||(u.as="script"),u.crossOrigin="",u.href=l,s&&u.setAttribute("nonce",s),document.head.appendChild(u),c)return new Promise((g,m)=>{u.addEventListener("load",g),u.addEventListener("error",()=>m(new Error(`Unable to preload CSS for ${l}`)))})}))}function o(r){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=r,window.dispatchEvent(s),!s.defaultPrevented)throw r}return a.then(r=>{for(const s of r||[])s.status==="rejected"&&o(s.reason);return t().catch(o)})},It="#1A6B3C",At="#cc2222",Bn="#D4A017",Bi="#888";async function qn(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    </style>`}const Pn="2026.07.17-1422";async function qi(e,{state:t,navigate:i,toast:n}){var o,r;const a=t.profile;a&&(e.innerHTML=`
  <style>
    .home-dark {
      min-height: 100%;
  ...
  <div class="home-dark" id="home-dark">
    ...
    ${Se(e,{width:140})}
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),c=(s||[]).filter(M=>M.card_type==="player"&&M.player),d=(s||[]).filter(M=>M.card_type==="game_changer"),u=(s||[]).filter(M=>M.card_type==="formation"),g=(s||[]).filter(M=>M.card_type==="stadium"),{data:m}=await w.from("gc_definitions").select("name,gc_type,color,effect,image_url"),f={};(m||[]).forEach(M=>{f[M.name]=M});const{data:p}=await w.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),x={};(p||[]).forEach(M=>{x[M.id]=M}),g.forEach(M=>{M.stadium_def&&(x[M.stadium_id]=M.stadium_def)});const b=Object.keys(fi),v=Object.keys(Ie),h={};c.forEach(M=>{const C=M.player.id;h[C]=(h[C]||0)+1}),new Set(Object.keys(h).map(M=>String(M)));const $=new Set(u.map(M=>M.formation)),y=new Set(d.map(M=>M.gc_type));let B="player",E="Tous",G="",N=!1;function ne(){return[...c].sort((M,C)=>{const F=Gt.indexOf(M.player.job),W=Gt.indexOf(C.player.job);return F!==W?F-W:(M.player.surname_real||"").localeCompare(C.player.surname_real||"")})}function ie(){return[...l||[]].sort((M,C)=>{const F=Gt.indexOf(M.job),W=Gt.indexOf(C.job);return F!==W?F-W:(M.surname_real||"").localeCompare(C.surname_real||"")})}function Q(){return ne().filter(M=>{const C=M.player,F=E==="Tous"||C.job===E,W=!G||`${C.firstname} ${C.surname_real}`.toLowerCase().includes(G);return F&&W})}function A(){return ie().filter(M=>{const C=E==="Tous"||M.job===E,F=!G||`${M.firstname} ${M.surname_real}`.toLowerCase().includes(G);return C&&F})}e.innerHTML=`
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(ae=document.getElementById("close-detail"))==null||ae.addEventListener("click",s);let j=new Set;const U=()=>{const Y=j.size,J=document.getElementById("sell-action-panel");if(!J)return;J.style.display=Y>0?"block":"none",document.getElementById("sell-selected-count").textContent=Y;const P=document.getElementById("evolve-btn");P&&(P.textContent=`⬆️ Évoluer ${Y>1?"(+"+Y+")":""}`)};document.querySelectorAll(".expl-check").forEach(Y=>{Y.addEventListener("change",()=>{const J=Y.dataset.id;Y.checked?j.add(J):j.delete(J);const P=Y.closest(".exemplaire-row");P&&(P.style.borderColor=Y.checked?"#1A6B3C":"#eee"),U()})}),document.querySelectorAll(".exemplaire-row").forEach(Y=>{Y.addEventListener("click",J=>{if(J.target.closest("button")||J.target.tagName==="INPUT")return;const P=Y.querySelector(".expl-check");P&&!P.disabled&&(P.checked=!P.checked,P.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(Y=>{Y.addEventListener("click",J=>{J.stopPropagation();const P=document.querySelector(`.expl-hist[data-hist="${Y.dataset.idx}"]`);P&&(P.style.display=P.style.display==="none"?"flex":"none")})}),(pe=document.getElementById("evolve-btn"))==null||pe.addEventListener("click",async()=>{if(g<=1)return;const Y=[...j];if(!Y.length)return;if(j.has(e.id)){const K=document.createElement("div");K.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",K.innerHTML=`
  ...
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
    ...
    </div>`;x("⚽ Choisir une formation",v),document.querySelectorAll("#modal-body [data-forma]").forEach(h=>{h.addEventListener("click",()=>{t.formation=h.dataset.forma;const $=Oi(t.formation),y={};$.forEach(B=>{t.slots[B]&&(y[B]=t.slots[B])}),t.slots=y,b(),it(e,t,i)})})};e.querySelectorAll("#formation-mobile-btn, #formation-pc-btn").forEach(x=>x.addEventListener("click",m)),e.querySelectorAll("#add-stad-btn-pc, #add-stad-btn").forEach(x=>x.addEventListener("click",()=>ao(t,e,i))),e.querySelectorAll("#save-deck").forEach(x=>x.addEventListener("click",()=>co(t,i))),e.querySelectorAll("[data-remove-sub]").forEach(x=>{x.addEventListener("click",()=>{t.subs=t.subs.filter(b=>b!==x.dataset.removeSub),it(e,t,i)})}),e.querySelectorAll("#add-sub-btn").forEach(x=>x.addEventListener("click",()=>lo(t,e,i)))}function ro(e,t,i,n){var h,$;const a=window.innerWidth>=900,o=e.querySelector(a?"#deck-field-pc":"#deck-field-mobile");if(!o)return;const r=(h=t.stadiumCards)==null?void 0:h.find(y=>y.id===t.stadiumCardId),s=r&&(($=t.stadDefMap)==null?void 0:$[r.stadium_id])||null,l=Bt[t.formation]||{},c=Yi(t.formation)||[],d={};for(const y of i){const B=t.slots[y],E=B?t.playerCards.find(G=>G.id===B):null;E!=null&&E.player?d[y]={...E.player,_evolution_bonus:E.evolution_bonus||0,face:E.player.face||null}:d[y]=null}const u=window.innerWidth>=900,g=u?window.innerWidth-280:window.innerWidth-20,m=u?Math.min(g,860):g,f=Math.round(u?m*.82:m*.85),p=u?84:44;let x="";for(const[y,B]of c){const E=l[y],G=l[B];if(!E||!G)continue;const N=E.x*m,ne=Math.round(.03*f+E.y*.85*f),ie=G.x*m,Q=Math.round(.03*f+G.y*.85*f),A=d[y],_=d[B],j=qt(A,_);j==="#ff3333"||j==="#cc2222"?x+=`<line x1="${N.toFixed(1)}" y1="${ne.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${Q.toFixed(1)}" stroke="${j}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(x+=`<line x1="${N.toFixed(1)}" y1="${ne.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${Q.toFixed(1)}" stroke="${j}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,x+=`<line x1="${N.toFixed(1)}" y1="${ne.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${Q.toFixed(1)}" stroke="${j}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let b="";const v=Math.round(p*657/507);for(const y of i){const B=l[y];if(!B)continue;const E=d[y],G=B.x*m;B.y*f;const N=Math.round(.03*f+B.y*(.85*f)),ne=Math.round(G-p/2),ie=Math.round(N-v/2);if(E){const Q=y.replace(/\d+/,""),A=s&&(s.club_id&&String(E.club_id)===String(s.club_id)||s.country_code&&E.country_code===s.country_code),_=Se({...E,_evolution_bonus:E._evolution_bonus||0},{width:p,showStad:!0,stadDef:s,role:Q});b+=`<div style="position:absolute;left:${ne}px;top:${ie}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${y}">
  ...
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff;position:relative">
      ...
      <!-- Pagination -->
      ${o.length>1?`<div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">${o.map((Q,A)=>`<div style="width:6px;height:6px;border-radius:50%;background:${A===d?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}</div>`:""}
    ...
    </div>`,requestAnimationFrame(()=>requestAnimationFrame(function Q(){const A=e.querySelector(".deck-preview-wrap"),_=e.querySelector("#deck-swipe-zone");if(!A||!_||!b)return;const j=_.clientWidth>=900,U=Math.max(200,_.clientHeight-(j?60:40)),ae=Math.max(200,_.clientWidth-16),pe=j?Math.min(117,Math.max(52,Math.round(ae*.22))):Math.max(44,Math.round(ae*.168));if(U<220||ae<220){requestAnimationFrame(Q);return}const se=j?null:Math.round(pe*.55);A.innerHTML=$t(b,x,null,[],ae,U,[],se),A.style.cssText=`width:${ae}px;height:${U}px;overflow:hidden;margin:${j?0:60}px auto 0`;const Ee=A.querySelector("svg");Ee&&(Ee.style.cssText="display:block;width:100%;height:100%",Ee.setAttribute("preserveAspectRatio",j?"xMidYMid meet":"none"))})),(G=document.getElementById("prev-deck"))==null||G.addEventListener("click",()=>{d>0&&(d--,u())}),(N=document.getElementById("next-deck"))==null||N.addEventListener("click",()=>{d<o.length-1&&(d++,u())}),(ne=document.getElementById("validate-deck"))==null||ne.addEventListener("click",()=>{if(!h)return;const Q=t.state.params||{};t.navigate("match",{...Q,matchMode:Q.matchMode||i,deckId:g.id})}),(ie=document.getElementById("cancel-deck-select"))==null||ie.addEventListener("click",()=>{Re(e),a("home")});const $=document.getElementById("deck-swipe-zone");if($){let Q=0,A=0;$.addEventListener("touchstart",_=>{Q=_.touches[0].clientX,A=_.touches[0].clientY},{passive:!0}),$.addEventListener("touchend",_=>{const j=_.changedTouches[0].clientX-Q,U=_.changedTouches[0].clientY-A;Math.abs(j)<40||Math.abs(j)<Math.abs(U)||(j<0&&d<o.length-1?(d++,u()):j>0&&d>0&&(d--,u()))},{passive:!0})}}u()}function tt(e,t=44,i=58,n=null){return Se(e,{width:t,showStad:!!n,stadDef:n,used:e==null?void 0:e.used})}function bt(e,t,i){if(!(e!=null&&e.length))return"";const n=e.slice(0,5);let a='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return n.forEach((o,r)=>{const s=o._line||o.job||"MIL";if(a+=Se(o,{width:40,role:s,extraNote:o.boost||0}),r<n.length-1){const l=qt(o,n[r+1]);a+=`<div style="width:7px;height:3px;background:${l==="#ff3333"||l==="#cc2222"?"rgba(255,255,255,0.12)":l};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(a+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),a+="</div>",a}function Wt(e,t,i,n,a=310,o=310,r=[],s=null){const l=Bt[t]||{},c=Yi(t)||fi[t]||[],d={},u=["ATT","MIL","DEF","GK"];for(const b of u)(e[b]||[]).forEach((h,$)=>{d[`${b}${$+1}`]=h});function g(b){const v=l[b];return v?{x:v.x*a,y:v.y*o}:null}let m="";for(const[b,v]of c){const h=g(b),$=g(v);if(!h||!$)continue;const y=d[b],B=d[v],E=qt(y,B);E==="#00ff88"||E==="#FFD700"?(m+=`<line x1="${h.x.toFixed(1)}" y1="${h.y.toFixed(1)}" x2="${$.x.toFixed(1)}" y2="${$.y.toFixed(1)}"
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:140px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const j=t.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b;text-align:center">
            ...
            <div style="display:flex;justify-content:center">${bt((j.players||[]).map(U=>({...U,used:!1})),"#ff6b6b",j.total)}</div>
            ...
            <div style="display:flex;justify-content:center">${bt((j.players||[]).map(U=>({...U,used:!1})),"#00ff88",j.total)}</div>
    ...
    ${(()=>{const _=window.innerWidth>=700,j=(P,M,C)=>{var ge,je;const F=(t.gcDefs||[]).find(Ae=>Ae.name===P.gc_type),W={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[F==null?void 0:F.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",K={purple:"#b06ce0",light_blue:"#00d4ef"}[F==null?void 0:F.color]||"#b06ce0",q=(F==null?void 0:F.name)||P.gc_type,O=(F==null?void 0:F.effect)||((ge=Ie[P.gc_type])==null?void 0:ge.desc)||"",we=F!=null&&F.image_url?`/icons/${F.image_url}`:null,_e=((je=Ie[P.gc_type])==null?void 0:je.icon)||"⚡",be=Math.round(C*.22),he=Math.round(C*.22),ke=C-be-he,Le=q.length>12?7:9;return`<div class="gc-mini" data-gc-id="${P.id}" data-gc-type="${P.gc_type}"
        ...
        <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
          ${$t(t.homeTeam,t.formation,t.phase,n,_?1300:lt(),_?600:ht(),c)}
        </div>
      </div>`;return _?`
  ...
  <div id="match-history-panel">
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:6px">
      ${t.log.length===0?`<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.3)">Aucune action pour l'instant</div>`:[...t.log].reverse().map(_=>{var j,U,le;if(_.type==="duel"){const ae=_.isGoal,pe=_.homeScored?"#FFD700":ae?"#ff6b6b":"rgba(255,255,255,0.3)",se=_.homeScored?"⚽ BUT !":ae?"⚽ BUT IA !":(j=_.homePlayers)!=null&&j.length?"⚔️ Attaque":"🛡️ Défense";return`<div style="padding:8px;border-radius:8px;background:${ae?"rgba(212,160,23,0.12)":"rgba(255,255,255,0.04)"};border-left:3px solid ${pe};margin-bottom:4px">
                ...
                ${(U=_.homePlayers)!=null&&U.length?`<div style="margin-bottom:3px">${bt(_.homePlayers,"rgba(255,255,255,0.7)",_.homeTotal)}</div>`:""}
                ${(le=_.aiPlayers)!=null&&le.length?`<div style="opacity:0.7">${bt(_.aiPlayers,"#ff6b6b",_.aiTotal)}</div>`:""}
    ...
    </div>`,(y=u.querySelector("#sub-close"))==null||y.addEventListener("click",()=>u.remove()),(B=u.querySelector("#out-up"))==null||B.addEventListener("click",()=>{s>0&&(s--,g())}),(E=u.querySelector("#out-down"))==null||E.addEventListener("click",()=>{s<o.length-1&&(s++,g())}),(G=u.querySelector("#in-up"))==null||G.addEventListener("click",()=>{c>0&&(c--,g())}),(N=u.querySelector("#in-down"))==null||N.addEventListener("click",()=>{c<r.length-1&&(c++,g())});const $=(ie,Q,A,_)=>{const j=u.querySelector("#"+ie);if(!j)return;let U=0;j.addEventListener("touchstart",le=>{U=le.touches[0].clientY},{passive:!0}),j.addEventListener("touchend",le=>{const ae=le.changedTouches[0].clientY-U;if(Math.abs(ae)<30)return;const pe=Q();ae<0&&pe<_-1?(A(pe+1),g()):ae>0&&pe>0&&(A(pe-1),g())},{passive:!0})};$("in-panel",()=>c,ie=>c=ie,r.length),$("out-panel",()=>s,ie=>s=ie,o.length),(ne=u.querySelector("#sub-confirm"))==null||ne.addEventListener("click",ie=>{if(ie.preventDefault(),ie.stopPropagation(),d)return;d=!0;const Q=o[s],A=r[c];if(!Q||!A)return;let _=null,j=-1;for(const[le,ae]of Object.entries(t.homeTeam)){const pe=(ae||[]).findIndex(se=>se.cardId===Q.cardId);if(pe!==-1){_=le,j=pe;break}}if(j===-1||!_){xt("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),u.remove();return}const U={...A,_line:_,_col:Q._col||0,used:!1,boost:0};t.homeTeam[_].splice(j,1,U),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(A.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:Q.name,firstname:Q.firstname,note:De(Q,_),portrait:jt(Q),job:Q.job,country_code:Q.country_code,rarity:Q.rarity,clubName:Q.clubName,clubLogo:Q.clubLogo},inPlayer:{name:A.name,firstname:A.firstname,note:De(A,_),portrait:jt(A),job:A.job,country_code:A.country_code,rarity:A.rarity,clubName:A.clubName,clubLogo:A.clubLogo},text:`🔄 ${A.firstname} ${A.name} remplace ${Q.firstname} ${Q.name}`}),u.remove(),Mo(Q,A,()=>$e(e,t,i))})}document.body.appendChild(u),g()}function Ro(e,t,i,n,a){var f,p;const o=(n.gcDefs||[]).find(x=>x.name===t),r=Ie[t]||{icon:"⚡",desc:"Carte spéciale."},s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[o==null?void 0:o.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",l={purple:"#b06ce0",light_blue:"#00d4ef"}[o==null?void 0:o.color]||"#b06ce0",c=(o==null?void 0:o.name)||t,d=(o==null?void 0:o.effect)||r.desc,u=o!=null&&o.image_url?`/icons/${o.image_url}`:null,g=r.icon||"⚡",m=document.createElement("div");m.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",m.innerHTML=`
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(a=>{a.addEventListener("click",()=>{const o=a.dataset.cardId;for(const r of["GK","DEF","MIL","ATT"]){const s=(t.homeTeam[r]||[]).find(l=>l.cardId===o);if(s){s.boost=(s.boost||0)+t.boostCard.value,t.log.push({text:`⚡ Boost +${t.boostCard.value} appliqué à ${s.name}`,type:"info"});break}}t.boostUsed=!0,i.closeModal(),$e(e,t,i)})})}async function vt(e,t,i){var d,u;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:n}=i,a=t.homeScore>t.aiScore,o=t.homeScore===t.aiScore,r=a?"victoire":o?"nul":"defaite",s=Un(t.mode,r);t.matchId&&await w.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:a?n.profile.id:null,home_credits_reward:s,played_at:new Date().toISOString()}).eq("id",t.matchId);const l={credits:(n.profile.credits||0)+s,matches_played:(n.profile.matches_played||0)+1};a?l.wins=(n.profile.wins||0)+1:o?l.draws=(n.profile.draws||0)+1:l.losses=(n.profile.losses||0)+1,await w.from("users").update(l).eq("id",n.profile.id),await i.refreshProfile();const c=document.createElement("div");c.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",c.innerHTML=`
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
    ...
    </div>`,`<button class="btn btn-primary" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const gt=e=>un(e);async function Et(e,t,i,n,a={}){return xn(e,t,i,n,a.myGC||[],a.gcDefs||[],a.isRanked||!1,a.rankedData||null,a.stadiumDef||null,a.onMatchEnd||null,a.mlLeagueId||null,a.mlMatchId||null)}async function Ko(e,t,i){const{data:n}=await w.from("matches").select("home_id,away_id,mode,is_ranked").eq("id",i).single();if(!n){t.toast("Match introuvable","error"),t.navigate("home");return}const a=n.home_id===t.state.user.id;return xn(e,t,i,a,[],[],n.is_ranked||!1,null,null,null,null,null)}async function xn(e,t,i,n,a=[],o=[],r=!1,s=null,l=null,c=null,d=null,u=null){const{state:g,navigate:m,toast:f}=t,p=n?"p1":"p2",x=n?"p2":"p1",b=(a||[]).map(k=>k.id),v=(a||[]).map(k=>({id:k.id,gc_type:k.gc_type,_gcDef:k._gcDef||null}));e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const{data:h}=await w.from("matches").select("*").eq("id",i).single();if(!h){f("Match introuvable","error"),m("home");return}async function $(){var me,xe;console.log("[PvP] buildGameState match:",{id:h.id,home_deck_id:h.home_deck_id,away_deck_id:h.away_deck_id,mode:h.mode,is_ranked:h.is_ranked});const[{data:k,error:L},{data:z,error:S},{data:T},{data:I}]=await Promise.all([w.rpc("get_deck_for_match",{p_deck_id:h.home_deck_id}),w.rpc("get_deck_for_match",{p_deck_id:h.away_deck_id}),w.from("users").select("id,pseudo,club_name").eq("id",h.home_id).single(),w.from("users").select("id,pseudo,club_name").eq("id",h.away_id).single()]);console.log("[PvP] get_deck_for_match p1:",L==null?void 0:L.message,"p2:",S==null?void 0:S.message,"p1D:",(me=k==null?void 0:k.starters)==null?void 0:me.length,"p2D:",(xe=z==null?void 0:z.starters)==null?void 0:xe.length);const D=te=>{const Te=Number(te.evolution_bonus)||0;return{cardId:te.card_id,position:te.position,id:te.id,firstname:te.firstname,name:te.surname_real,country_code:te.country_code,club_id:te.club_id,job:te.job,job2:te.job2,note_g:(Number(te.note_g)||0)+(te.job==="GK"||te.job2==="GK"&&Number(te.note_g)>0?Te:0),note_d:(Number(te.note_d)||0)+(te.job==="DEF"||te.job2==="DEF"&&Number(te.note_d)>0?Te:0),note_m:(Number(te.note_m)||0)+(te.job==="MIL"||te.job2==="MIL"&&Number(te.note_m)>0?Te:0),note_a:(Number(te.note_a)||0)+(te.job==="ATT"||te.job2==="ATT"&&Number(te.note_a)>0?Te:0),evolution_bonus:Te,rarity:te.rarity,skin:te.skin,hair:te.hair,hair_length:te.hair_length,face:te.face||null,clubName:te.club_encoded_name||null,clubLogo:te.club_logo_url||null,boost:0,used:!1,_line:null,_col:null}},R=((k==null?void 0:k.starters)||[]).map(te=>D(te)),X=((z==null?void 0:z.starters)||[]).map(te=>D(te)),V=(k==null?void 0:k.formation)||"4-4-2",oe=(z==null?void 0:z.formation)||"4-4-2";let Z=Ut(R,V),H=Ut(X,oe);const re=((k==null?void 0:k.subs)||[]).map(te=>D(te)),ce=((z==null?void 0:z.subs)||[]).map(te=>D(te)),ue=(k==null?void 0:k.stadium_def)||(n?l:null),de=(z==null?void 0:z.stadium_def)||(n?null:l);return ue&&(Z=Ct(Z,ue),Ht(re,ue)),de&&(H=Ct(H,de),Ht(ce,de)),{p1Team:Z,p2Team:H,p1Subs:re,p2Subs:ce,p1Formation:V,p2Formation:oe,p1Name:(T==null?void 0:T.club_name)||(T==null?void 0:T.pseudo)||"Joueur 1",p2Name:(I==null?void 0:I.club_name)||(I==null?void 0:I.pseudo)||"Joueur 2",p1Score:0,p2Score:0,p1Subs_used:0,p2Subs_used:0,maxSubs:3,phase:"reveal",attacker:null,round:0,selected_p1:[],selected_p2:[],pendingAttack:null,log:[],modifiers:{p1:{},p2:{}},gc_p1:n?b:[],gc_p2:n?[]:b,gcCardsFull_p1:n?v:[],gcCardsFull_p2:n?[]:v,usedGc_p1:[],usedGc_p2:[],boostValue:null,boostOwner:null,boostUsed:!1,gcDefs:o||[],lastActionAt:new Date().toISOString()}}let y=h.game_state&&Object.keys(h.game_state).length?h.game_state:null;if(console.log("[PvP] init - amIHome:",n,"gameState exists:",!!y,"match.status:",h.status,"home_id:",h.home_id,"away_id:",h.away_id,"myId:",g.profile.id),!y)if(n){y=await $(),console.log("[PvP] home - gameState built, p1Team keys:",Object.keys((y==null?void 0:y.p1Team)||{}));const{data:k}=await w.from("matches").select("game_state").eq("id",i).single();!(k!=null&&k.game_state)||!Object.keys(k.game_state).length?await w.from("matches").update({game_state:y,turn_user_id:h.home_id}).eq("id",i):y=k.game_state}else{e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Synchronisation...</div>',console.log("[PvP] away - waiting for home to write game_state...");for(let k=0;k<30&&!y;k++){await new Promise(z=>setTimeout(z,400));const{data:L}=await w.from("matches").select("game_state").eq("id",i).single();L!=null&&L.game_state&&Object.keys(L.game_state).length&&(y=L.game_state),k%5===0&&console.log("[PvP] away - poll",k,"game_state:",!!(L!=null&&L.game_state))}if(!y){f("Erreur de synchronisation","error"),m("home");return}y.gc_p2=b,y.gcCardsFull_p2=v,await w.from("matches").update({game_state:y}).eq("id",i)}let B=!1,E=null,G=!1;const N=new Set,ne=new Set;async function ie(k){var oe,Z;try{w.removeChannel(Q)}catch{}const L=y[p+"Score"]||0,z=y[x+"Score"]||0;let S,T;k.winner_id?(S=k.winner_id===g.profile.id,T=!1):k.forfeit?(S=L>z,T=!1):(T=L===z,S=L>z);let I="";if(r&&n)try{const{data:H}=await w.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",h.home_id).single(),{data:re}=await w.from("users").select("id,mmr,mmr_deviation,mmr_volatility,placement_matches").eq("id",h.away_id).single();if(H&&re){const ce=S?1:T?.5:0,ue=1-ce,de=H.placement_matches<10,me=re.placement_matches<10,xe=computeGlicko2(H.mmr,H.mmr_deviation,H.mmr_volatility,re.mmr,re.mmr_deviation,ce===1?1:ce===0?0:.5,de),te=computeGlicko2(re.mmr,re.mmr_deviation,re.mmr_volatility,H.mmr,H.mmr_deviation,ue===1?1:ue===0?0:.5,me);await w.rpc("update_mmr_after_ranked",{p_match_id:i,p_winner_id:T?null:S?h.home_id:h.away_id,p_home_id:h.home_id,p_away_id:h.away_id,p_home_delta:xe.delta,p_away_delta:te.delta,p_home_new_rd:xe.newRd,p_away_new_rd:te.newRd,p_home_new_vol:xe.newSigma,p_away_new_vol:te.newSigma});const Te=xe.delta,Be=xe.newMmr,Me=getTier(Be),Ge=Te>=0?"+":"",ut=Te>=0?"#4caf50":"#ff6b6b",Oe=getTier(H.mmr);I=`
      ...
      ${I}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${r?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(D),(Z=D.querySelector("#pvp-end-home"))==null||Z.addEventListener("click",()=>{D.remove(),Re(e),m(r?"ranked":"home")})}const Q=w.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},k=>{const L=k.new;try{if(L.status==="finished"||L.forfeit){if(B)return;B=!0,E&&(clearInterval(E),E=null),L.game_state&&(y=L.game_state),ie(L);return}if(L.game_state){const z=y;y=L.game_state;const S=y._lastGC;if(S&&S.seq&&!ne.has(S.seq)&&(ne.add(S.seq),S.by!==p)){P(S.type,S.by,()=>se());return}const T=z[p+"Score"]||0,I=z[x+"Score"]||0,D=y[p+"Score"]||0,R=y[x+"Score"]||0,X=D>T,V=R>I;if((X||V)&&!N.has(y.round)){N.add(y.round);const oe=[...y.log||[]].reverse().find(H=>H.isGoal),Z=((oe==null?void 0:oe.homePlayers)||[]).map(H=>({name:H.name,note:H.note,portrait:H.portrait,job:H.job}));Ve(Z,D,R,X,()=>se());return}se()}}catch(z){console.error("[PvP] crash:",z)}}).subscribe();async function A(k){Object.assign(y,k),y.lastActionAt=new Date().toISOString();const{error:L}=await w.from("matches").update({game_state:y}).eq("id",i);L&&f("Erreur de synchronisation","error");try{se()}catch(z){console.error("[PvP] renderPvpScreen crash:",z)}}async function _(){if(B)return;B=!0,E&&(clearInterval(E),E=null);const k=n?h.away_id:h.home_id,L=n?"p2":"p1",z=n?"p1":"p2",S={...y,[L+"Score"]:3,[z+"Score"]:0,phase:"finished"};try{await w.from("matches").update({status:"finished",forfeit:!0,winner_id:k,home_score:S.p1Score||0,away_score:S.p2Score||0,game_state:S}).eq("id",i)}catch{}try{w.removeChannel(Q)}catch{}setTimeout(()=>{Re(e),m("home")},800)}const j={BOOST_STAT:({value:k=1,count:L=1,roles:z=[]},S,T)=>{const I=S[p+"Team"],D=Object.entries(I).filter(([R])=>!z.length||z.includes(R)).flatMap(([R,X])=>X.filter(V=>!V.used).map(V=>({...V,_line:R})));if(!D.length){S.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),T(S);return}U(D,L,`Choisir ${L} joueur(s) à booster (+${k})`,R=>{R.forEach(X=>{const V=(I[X._line]||[]).find(oe=>oe.cardId===X.cardId);V&&(V.boost=(V.boost||0)+k,S.log.push({text:`⚡ +${k} sur ${V.name}`,type:"info"}))}),S[p+"Team"]=I,T(S)})},DEBUFF_STAT:({value:k=1,count:L=1,roles:z=[],target:S="ai"},T,I)=>{const D=S==="home"?p:x,R=T[D+"Team"],X=S==="home"?"allié":"adverse",V=Object.entries(R).filter(([oe])=>!z.length||z.includes(oe)).flatMap(([oe,Z])=>Z.map(H=>({...H,_line:oe})));if(!V.length){T.log.push({text:`🎯 Aucun joueur ${X}`,type:"info"}),I(T);return}U(V,L,`Choisir ${L} joueur(s) ${X}(s) (-${k})`,oe=>{oe.forEach(Z=>{const H=(R[Z._line]||[]).find(re=>re.cardId===Z.cardId);H&&(H.boost=(H.boost||0)-k,T.log.push({text:`🎯 -${k} sur ${H.name}`,type:"info"}))}),T[D+"Team"]=R,I(T)})},GRAY_PLAYER:({count:k=1,roles:L=[],target:z="ai"},S,T)=>{const I=z==="home"?p:x,D=S[I+"Team"],R=z==="home"?"allié":"adverse",X=Object.entries(D).filter(([V])=>!L.length||L.includes(V)).flatMap(([V,oe])=>oe.filter(Z=>!Z.used).map(Z=>({...Z,_line:V})));if(!X.length){S.log.push({text:`❌ Aucun joueur ${R}`,type:"info"}),T(S);return}U(X,k,`Choisir ${k} joueur(s) ${R}(s) à exclure`,V=>{const oe="usedCardIds_"+I,Z=new Set(S[oe]||[]);V.forEach(H=>{const re=(D[H._line]||[]).find(ce=>ce.cardId===H.cardId);re&&(re.used=!0,Z.add(H.cardId),S.log.push({text:`❌ ${re.name} exclu !`,type:"info"}))}),S[oe]=[...Z],S[I+"Team"]=D,T(S)})},REVIVE_PLAYER:({count:k=1,roles:L=[]},z,S)=>{const T=z[p+"Team"],I=Object.entries(T).filter(([D])=>!L.length||L.includes(D)).flatMap(([D,R])=>R.filter(X=>X.used).map(X=>({...X,_line:D})));if(!I.length){z.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),S(z);return}U(I,k,`Choisir ${k} joueur(s) à ressusciter`,D=>{D.forEach(R=>{const X=(T[R._line]||[]).find(V=>V.cardId===R.cardId);X&&(X.used=!1,z.log.push({text:`💫 ${X.name} ressuscité !`,type:"info"}))}),z[p+"Team"]=T,S(z)})},REMOVE_GOAL:({},k,L)=>{const z=x+"Score";k[z]>0?(k[z]--,k.log.push({text:"🚫 Dernier but annulé !",type:"info"})):k.log.push({text:"🚫 Aucun but à annuler",type:"info"}),L(k)},ADD_GOAL_DRAW:({},k,L)=>{k[p+"Score"]===k[x+"Score"]?(k[p+"Score"]++,k.log.push({text:"🎯 But bonus !",type:"info"})):k.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),L(k)},ADD_SUB:({value:k=1},L,z)=>{L.maxSubs=(L.maxSubs||3)+k,L.log.push({text:`🔄 +${k} remplacement(s)`,type:"info"}),z(L)},CUSTOM:({},k,L)=>L(k)};function U(k,L,z,S){const T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let I=[];function D(){var X,V;const R=k.map(oe=>{const Z=oe._line||oe.job||"MIL",H=I.find(ce=>ce.cardId===oe.cardId),re=Se({...oe,_evolution_bonus:0},{width:90,showStad:!0,role:Z,extraNote:oe.boost||0});return`<div class="pp-item" data-cid="${oe.cardId}"
        ...
        </div>`,(X=T.querySelector("#pp-close"))==null||X.addEventListener("click",()=>T.remove()),T.querySelectorAll(".pp-item").forEach(oe=>{oe.addEventListener("click",()=>{const Z=oe.dataset.cid,H=k.find(ce=>ce.cardId===Z),re=I.findIndex(ce=>ce.cardId===Z);H&&(re>-1?I.splice(re,1):I.length<L&&I.push(H),D())})}),(V=T.querySelector("#pp-confirm"))==null||V.addEventListener("click",()=>{T.remove(),S(I)})}D(),document.body.appendChild(T)}async function le(k,L){const S=(y["gcCardsFull_"+p]||[]).find(R=>R.id===k),T=(S==null?void 0:S._gcDef)||(y.gcDefs||[]).find(R=>{var X;return R.name===L||((X=R.name)==null?void 0:X.toLowerCase().trim())===(L==null?void 0:L.toLowerCase().trim())}),I=[...y["usedGc_"+p]||[],k],D={type:L,by:p,seq:(y._gcAnimSeq||0)+1};ne.add(D.seq),P(L,p,async()=>{if(T!=null&&T.effect_type&&T.effect_type!=="CUSTOM"){const X=j[T.effect_type];if(X){const V={...y};X(T.effect_params||{},V,async oe=>{oe["usedGc_"+p]=I,oe._lastGC=D,oe._gcAnimSeq=D.seq,await A(oe)});return}}const R={...y};switch(L){case"Remplacement+":R.maxSubs=(R.maxSubs||3)+1,R.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const X=x+"Score";R[X]>0&&(R[X]--,R.log.push({text:"🚫 But annulé",type:"info"}));break}}R["usedGc_"+p]=I,R._lastGC=D,R._gcAnimSeq=D.seq,await A(R)})}function ae(k,L){const z="usedCardIds_"+k,S=new Set(y[z]||[]);L.forEach(T=>S.add(T)),y[z]=[...S]}function pe(){for(const k of["p1","p2"]){const L=new Set(y["usedCardIds_"+k]||[]),z=y[k+"Team"];if(z)for(const S of["GK","DEF","MIL","ATT"])(z[S]||[]).forEach(T=>{T.used=L.has(T.cardId)})}}function se(){var rt,We,Xe,at,Li,Ti;if(y.phase==="reveal")return Ee();if(y.phase==="midfield")return J();if(y.phase==="finished")return Lt();const k=y[p+"Team"];y[x+"Team"],pe();const L=y[p+"Score"],z=y[x+"Score"],S=y[p+"Name"],T=y[x+"Name"],I=y.phase===p+"-attack",D=y.phase===p+"-defense",R=Array.isArray(y["selected_"+p])?y["selected_"+p]:[],X=R.map(ee=>ee.cardId),V=window.innerWidth>=700,oe=y[p+"Subs"]||[],Z=y["usedSubIds_"+p]||[],H=oe.filter(ee=>!Z.includes(ee.cardId)),re=y["gcCardsFull_"+p]||[],ce=y["usedGc_"+p]||[],ue=re.filter(ee=>!ce.includes(ee.id)),de=y.boostOwner===p&&!y.boostUsed,me=[...k.MIL||[],...k.ATT||[]].filter(ee=>!ee.used),xe=I&&me.length===0?[...k.GK||[],...k.DEF||[]].filter(ee=>!ee.used).map(ee=>ee.cardId):[];function te(ee,fe,ze){var Ii,Ai;const ye=ee._gcDef,st={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[ye==null?void 0:ye.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",ft={purple:"#b06ce0",light_blue:"#00d4ef"}[ye==null?void 0:ye.color]||"#b06ce0",Tt=(ye==null?void 0:ye.name)||ee.gc_type,zt=(ye==null?void 0:ye.effect)||((Ii=Ie[ee.gc_type])==null?void 0:Ii.desc)||"",zi=ye!=null&&ye.image_url?`/icons/${ye.image_url}`:null,kn=((Ai=Ie[ee.gc_type])==null?void 0:Ai.icon)||"⚡",Mi=Math.round(ze*.22),Si=Math.round(ze*.22),Jt=ze-Mi-Si,$n=Tt.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${ee.id}" data-gc-type="${ee.gc_type}"
      ...
      </div>`}const Be=(ee,fe)=>fe?Te(130,175):te(ee,130,175),Me=(ee,fe)=>fe?Te(68,95):te(ee,68,95),Ge=V?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",ut=I?Ae(p)?`<button id="pvp-action" style="${Ge};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${R.length===0?"disabled":""}>⚔️ ATTAQUEZ <span id="pvp-timer"></span></button>`:`<button id="pvp-action" data-pass="1" style="${Ge};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER <span id="pvp-timer"></span></button>`:D?`<button id="pvp-action" style="${Ge};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${R.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="pvp-timer"></span></button>`:`<div style="font-size:11px;color:rgba(255,255,255,0.3);text-align:center;padding:4px">⏳ Tour de ${T}</div>`,Oe=I&&!Ae(p)?'<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">Aucun attaquant — passez la main</div>':I||D?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${R.length}/3 sélectionné(s)</div>`:"",Ft=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${V?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
      ${H.length===0?'<div style="font-size:7px;color:rgba(255,255,255,0.25);text-align:center;margin-top:6px;line-height:1.4">Pas de<br>rempl.</div>':H.map(ee=>`<div class="pvp-sub-btn" data-sub-id="${ee.cardId}" style="cursor:pointer;flex-shrink:0">${tt(ee,V?76:44,V?100:58)}</div>`).join("")}
    </div>`,Ce=I?"attack":D?"defense":"idle",He=`<div style="overflow:hidden;min-width:0;flex:1;min-height:0;display:flex;flex-direction:column" id="match-field">
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${$t(k,y[p+"Formation"],Ce,X,V?1300:lt(),V?600:ht(),xe)}
      </div>
          ...
          <div style="display:flex;justify-content:center">${bt((ye.players||[]).map(st=>({...st,used:!1})),"#ff6b6b",ye.total)}</div>
        </div>`}if(I&&((ze=y.pendingAttack)!=null&&ze.players)){const ye=y.pendingAttack;return`<div style="padding:5px 8px;background:rgba(26,107,60,0.2);border-left:3px solid #00ff88;text-align:center">
          <div style="font-size:8px;color:#00ff88;letter-spacing:2px;margin-bottom:4px;text-transform:uppercase">⚔️ VOUS ATTAQUEZ</div>
          <div style="display:flex;justify-content:center">${bt((ye.players||[]).map(st=>({...st,used:!1})),"#00ff88",ye.total)}</div>
      ...
      <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
        ...
        <div id="mobile-play-area" style="flex:1;min-height:0;display:flex;overflow:hidden">
          <div id="match-field" style="flex:1;min-width:0;min-height:0;overflow:hidden">
            <div class="terrain-wrapper" style="width:100%;height:100%;overflow:hidden">
              ${$t(k,y[p+"Formation"],Ce,X,lt(),ht(),xe)}
            </div>
          </div>
        </div>
        <div id="mobile-action-bar" style="position:absolute;left:0;right:0;bottom:0;z-index:20;background:rgba(0,0,0,0.55);padding:6px 8px 8px;display:flex;flex-direction:column;gap:6px;box-shadow:0 -4px 16px rgba(0,0,0,0.5)">
          <div style="display:flex;gap:6px;overflow-x:auto;align-items:flex-end;min-height:96px;padding-bottom:2px">
            ${ue.map(ee=>Me(ee,!1)).join("")}
            ${de?Me(null,!0):""}
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="width:190px;border-radius:16px;border:3px solid ${D};background:${I};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 40px ${D}66">
        <div style="padding:10px;background:rgba(255,255,255,0.14);text-align:center">
      ...
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${L.map(T=>{const I={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[T._line]||"#555",D=De(T,T._line)+(T.boost||0);return`<div class="bp-item" data-cid="${T.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${I};overflow:hidden;cursor:pointer">
      ...
      </div>`,document.body.appendChild(z),(S=z.querySelector("#bp-close"))==null||S.addEventListener("click",()=>z.remove()),z.querySelectorAll(".bp-item").forEach(T=>{T.addEventListener("click",async()=>{const I=T.dataset.cid,D=L.find(X=>X.cardId===I);if(!D)return;const R=(k[D._line]||[]).find(X=>X.cardId===I);R&&(R.boost=(R.boost||0)+y.boostValue),z.remove(),await A({[p+"Team"]:k,boostUsed:!0})})})}function F(k=null){var re,ce;if(!(y.phase===p+"-attack")){f("Remplacement uniquement avant votre attaque","warning");return}const z=y[p+"Team"],S=y["usedSubIds_"+p]||[],T=y.maxSubs||3;if(S.length>=T){f(`Maximum ${T} remplacements atteint`,"warning");return}const I=Object.entries(z).flatMap(([ue,de])=>(de||[]).filter(me=>me.used).map(me=>({...me,_line:ue}))),D=(y[p+"Subs"]||[]).filter(ue=>!S.includes(ue.cardId));if(!I.length){f("Aucun joueur utilisé à remplacer","warning");return}if(!D.length){f("Aucun remplaçant disponible","warning");return}let R=Math.max(0,I.findIndex(ue=>ue.cardId===k));const X=((re=I[R])==null?void 0:re._line)||((ce=I[R])==null?void 0:ce.job);let V=Math.max(0,D.findIndex(ue=>ue.job===X)),oe=!1;const Z=document.createElement("div");Z.id="pvp-sub-overlay",Z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function H(){var Be,Me,Ge,ut,Oe,Ft;const ue=I[R],de=D[V],me=Math.min(130,Math.round((window.innerWidth-90)/2)),xe=Math.round(me*1.35),te=Ce=>`background:rgba(255,255,255,0.12);border:none;color:${Ce?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${Ce?"default":"pointer"};flex-shrink:0`;Z.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        ...
        <div id="pout-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
          ...
          <button id="pout-down" style="${te(R>=I.length-1)}" ${R>=I.length-1?"disabled":""}>▼</button>
          <div style="font-size:10px;color:rgba(255,255,255,0.35)">${R+1}/${I.length}</div>
        </div>
      </div>
      <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
        <button id="psub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
      </div>`,(Be=Z.querySelector("#psub-close"))==null||Be.addEventListener("click",()=>Z.remove()),(Me=Z.querySelector("#pout-up"))==null||Me.addEventListener("click",()=>{R>0&&(R--,H())}),(Ge=Z.querySelector("#pout-down"))==null||Ge.addEventListener("click",()=>{R<I.length-1&&(R++,H())}),(ut=Z.querySelector("#pin-up"))==null||ut.addEventListener("click",()=>{V>0&&(V--,H())}),(Oe=Z.querySelector("#pin-down"))==null||Oe.addEventListener("click",()=>{V<D.length-1&&(V++,H())});const Te=(Ce,He,Ue,Ye)=>{const qe=Z.querySelector("#"+Ce);if(!qe)return;let rt=0;qe.addEventListener("touchstart",We=>{rt=We.touches[0].clientY},{passive:!0}),qe.addEventListener("touchend",We=>{const Xe=We.changedTouches[0].clientY-rt;if(Math.abs(Xe)<30)return;const at=He();Xe<0&&at<Ye-1?(Ue(at+1),H()):Xe>0&&at>0&&(Ue(at-1),H())},{passive:!0})};Te("pin-panel",()=>V,Ce=>V=Ce,D.length),Te("pout-panel",()=>R,Ce=>R=Ce,I.length),(Ft=Z.querySelector("#psub-confirm"))==null||Ft.addEventListener("click",async Ce=>{if(Ce.preventDefault(),Ce.stopPropagation(),oe)return;oe=!0;const He=I[R],Ue=D[V];if(!He||!Ue)return;const Ye=He._line,qe=(z[Ye]||[]).findIndex(Xe=>Xe.cardId===He.cardId);if(qe===-1){f("Erreur : joueur introuvable","error"),Z.remove();return}const rt={...Ue,_line:Ye,position:He.position,used:!1,boost:0};z[Ye].splice(qe,1,rt);const We=[...S,Ue.cardId];Z.remove(),W(He,Ue,async()=>{await A({[p+"Team"]:z,[x+"Team"]:y[x+"Team"],["usedSubIds_"+p]:We})})})}document.body.appendChild(Z),H()}function W(k,L,z){const S={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const I=(X,V,oe)=>`<div style="text-align:center">
      ...
      <div style="width:min(90vw,420px)">${Wt(y[x+"Team"],y[x+"Formation"],null,[],lt(),ht())}</div>
      ...
      </div>`,document.body.appendChild(L),(z=L.querySelector("#pvp-hist-close"))==null||z.addEventListener("click",()=>L.remove())}async function O(){if(y.phase!==p+"-attack")return;const k=p==="p1"?"p2":"p1",L=(y.round||0)+1,z=[...y.log||[]];z.push({type:"info",text:`⏭️ ${y[p+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const S=Fe(y),T=Ae(k),I=S||!T?"finished":k+"-attack";await A({["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:I,attacker:k,round:L,log:z}),I==="finished"&&await Ne(y)}function we(k){const L=(k.GK||[]).some(S=>!S.used),z=["DEF","MIL","ATT"].some(S=>(k[S]||[]).some(T=>!T.used));return L&&!z}function _e(k){return(k.p1Score||0)!==(k.p2Score||0)?null:we(k.p1Team)&&!ge(k.p2Team)?{side:"p1",gk:(k.p1Team.GK||[]).find(L=>!L.used)}:we(k.p2Team)&&!ge(k.p1Team)?{side:"p2",gk:(k.p2Team.GK||[]).find(L=>!L.used)}:null}async function be(k,L){const{side:z,gk:S}=k;if(!S)return;const T=z+"Score",I=(L[T]||0)+1;S.used=!0;const D=[...L.log||[]];D.push({type:"duel",isGoal:!0,homeScored:z===p,homePlayers:z==="p1"?[gt(S)]:[],aiPlayers:z==="p2"?[gt(S)]:[],text:`⚽ DERNIER CORNER — Le gardien de ${L[z+"Name"]} marque !`});const R=(L.round||0)+1,X=z==="p1"?"p2":"p1",V={...L,[T]:I},oe=Fe(V);N.add(R);const Z=document.createElement("div");Z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:950;display:flex;align-items:center;justify-content:center;overflow:hidden;text-align:center;padding:24px",document.body.appendChild(Z);const H=(ue,de)=>new Promise(me=>{Z.innerHTML=`<div style="font-size:32px;font-weight:900;color:${de};letter-spacing:2px;animation:lcFade 1.4s ease both">${ue}</div>
      <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(me,1400)});await H("⚽ DERNIER CORNER","#FFD700"),await H("🧤 LE GARDIEN MONTE !","#4fc3f7"),Z.remove();const re=z===p?I:L[p+"Score"]||0,ce=z===p?L[x+"Score"]||0:I;Ve([gt(S)],re,ce,z===p,async()=>{await A({[T]:I,log:D,round:R,pendingAttack:null,phase:oe?"finished":X+"-attack",attacker:X,["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}}}),oe&&await Ne({...y,[T]:I})})}async function he(){const k=y[p+"Team"],L=!["GK","DEF","MIL","ATT"].some(I=>(y[x+"Team"][I]||[]).some(D=>!D.used)),z=(y["selected_"+p]||[]).map(I=>{const D=(k[I._role]||[]).find(ce=>ce.cardId===I.cardId)||I,R=["GK","DEF"].includes(I._role),X=k[I._role]||[],V=X.findIndex(ce=>ce.cardId===I.cardId),oe=kt(X.length),Z=V>=0?oe[V]:D._col??1,H=y.stadiumDef||y.homeStadiumDef||null,re=D.stadiumBonus||H&&(H.club_id&&String(D.club_id)===String(H.club_id)||H.country_code&&D.country_code===H.country_code)||!1;return{...D,_line:I._role,_col:Z,stadiumBonus:re,...R?{note_a:Math.max(1,Number(D.note_a)||0)}:{}}});if(!z.length)return;const S=yi(z,y.modifiers[p]||{});ae(p,z.map(I=>I.cardId)),z.forEach(I=>{const D=(k[I._role]||[]).find(R=>R.cardId===I.cardId);D&&(D.used=!0)}),y["selected_"+p]=[],se();const T=[...y.log||[]];if(L){if(z.length===1&&(z[0]._line==="GK"||z[0].job==="GK")&&(y.p1Score||0)===(y.p2Score||0)){await be({side:p,gk:z[0]},{...y,[p+"Team"]:k,log:T});return}const D=(y[p+"Score"]||0)+1,R=z.map(H=>gt(H));T.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:R,homeTotal:S.total,aiTotal:0});const X=(y.round||0)+1,V=p==="p1"?"p2":"p1",oe={...y,[p+"Team"]:k,[p+"Score"]:D},Z=Fe(oe);N.add(X),Ve(R,D,y[x+"Score"]||0,!0,async()=>{await A({[p+"Team"]:k,[p+"Score"]:D,["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:Z?"finished":V+"-attack",attacker:V,round:X,log:T}),Z&&await Ne({...y,[p+"Score"]:D})});return}T.push({type:"pending",text:`⚔️ ${y[p+"Name"]} attaque (${S.total})`}),await A({[p+"Team"]:k,[x+"Team"]:y[x+"Team"],pendingAttack:{...S,players:z,side:p},["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},phase:x+"-defense",log:T})}async function ke(){const k=y[p+"Team"],L=(y["selected_"+p]||[]).map(de=>{const me=(k[de._role]||[]).find(Me=>Me.cardId===de.cardId)||de,xe=k[de._role]||[],te=xe.findIndex(Me=>Me.cardId===de.cardId),Te=kt(xe.length),Be=te>=0?Te[te]:me._col??1;return{...me,_line:de._role,_col:Be}}),z=bi(L,y.modifiers[p]||{});ae(p,L.map(de=>de.cardId)),L.forEach(de=>{const me=(k[de._role]||[]).find(xe=>xe.cardId===de.cardId);me&&(me.used=!0)}),y["selected_"+p]=[],se();const S=hi(y.pendingAttack.total,z.total,y.modifiers[p]||{}),T=y.pendingAttack.side,I=S==="attack"||(S==null?void 0:S.goal),D=T==="p1"?"p2":"p1",R=(y.round||0)+1,X=(y.pendingAttack.players||[]).map(de=>gt(de)),V=[...y.log||[]];V.push({type:"duel",isGoal:I,homeScored:I&&T===p,text:I?`⚽ BUT de ${y[T+"Name"]} ! (${y.pendingAttack.total} vs ${z.total})`:`✋ Attaque stoppée (${y.pendingAttack.total} vs ${z.total})`,homePlayers:X,aiPlayers:L.map(de=>gt(de)),homeTotal:y.pendingAttack.total,aiTotal:z.total});const oe=I?(y[T+"Score"]||0)+1:y[T+"Score"]||0,Z={...y,[p+"Team"]:k,[T+"Score"]:oe},H=Fe(Z),re=H?"finished":D+"-attack",ce=!H&&!I?_e(Z):null,ue=async()=>{if(ce){await be(ce,Z);return}await A({[p+"Team"]:k,[x+"Team"]:y[x+"Team"],[T+"Score"]:oe,["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:re,attacker:D,round:R,log:V}),(re==="finished"||H)&&await Ne({...y,[T+"Score"]:oe})};if(I){const de=T===p,me=de?oe:y[p+"Score"]||0,xe=de?y[x+"Score"]||0:oe;N.add(R),Ve(X,me,xe,de,ue)}else await ue()}function Le(k){return["MIL","ATT"].some(L=>(k[L]||[]).some(z=>!z.used))}function ge(k){return["GK","DEF","MIL","ATT"].some(L=>(k[L]||[]).some(z=>!z.used))}function je(k){return ge(k)&&!Le(k)}function Ae(k){const L=y[k+"Team"];return!!(Le(L)||je(L))}function Fe(k){return!ge(k.p1Team)&&!ge(k.p2Team)}function ot(k){const L=k.p1Score||0,z=k.p2Score||0;return L===z?null:L>z?h.home_id:h.away_id}async function Ne(k){try{const L=ot(k),z=L?h.home_id===L?h.away_id:h.home_id:null;await w.from("matches").update({status:"finished",winner_id:L,home_score:k.p1Score||0,away_score:k.p2Score||0}).eq("id",i),L&&z&&Yn(L,z).catch(()=>{})}catch(L){console.error("[PvP] finishMatch:",L)}}function Lt(){var T;const k=y[p+"Score"],L=y[x+"Score"],z=k>L,S=k===L;Ke(e),e.innerHTML=`
    ...
    <div style="padding:12px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        ${u.map(A=>`
          <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid #f5f5f5">
            <div style="width:36px;height:36px;border-radius:50%;background:${A.club_color2||wt};display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:900;color:${A.club_color1||"#fff"}">${(A.pseudo||"?").slice(0,2).toUpperCase()}</div>
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>${g.map((A,_)=>{const j=f>0&&r.status==="finished"?_===0?Math.floor(f*.7):_===1?Math.floor(f*.2):_===2?Math.floor(f*.1):0:0;return`<tr style="border-bottom:1px solid #f5f5f5;${A.userId===o?"background:#f0fdf4;":""}">
              <td style="padding:7px 3px 7px 0;font-weight:700;color:${_===0?et:_<3?wt:"#555"}">${["🥇","🥈","🥉"][_]||_+1}</td>
              ...
              <td style="text-align:center;color:${A.goalDiff>=0?wt:"#cc2222"}">${A.goalDiff>=0?"+":""}${A.goalDiff}</td>
      ...
      <div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 1px 6px rgba(0,0,0,0.08)">
        ...
        ${Array.from({length:Math.max(0,r.status==="active"?r.current_day-1:r.current_day)},(A,_)=>_+1).reverse().map(A=>{const _=(l||[]).filter(j=>j.matchday===A);return`<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:#888;margin-bottom:6px">Journée ${A}</div>${_.map(j=>Ui(j,u,o,!1,!0)).join("")}</div>`}).join("")}
      </div>`:""}
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    </div>`}function g(p){var G,N,ne,ie;const x=(G=p.card)==null?void 0:G.player;if(!x)return"";const b=((N=p.card)==null?void 0:N.evolution_bonus)||0,v=mt(x,x.job,b),h=x.job2?mt(x,x.job2,b):0,$=p.status==="sold",y=x.country_code?`https://flagsapi.com/${x.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,B=Nt[x.job]||"#bbb",E=x.job2?Nt[x.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${$?"opacity:0.7":""}">
      ${y?`<img src="${y}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      ${(ne=x.clubs)!=null&&ne.logo_url?`<img src="${x.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${B};display:flex;align-items:center;justify-content:center">
