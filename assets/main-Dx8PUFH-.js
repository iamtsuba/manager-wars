import{s as $,F as et,b as Me,c as Ft,l as ge,d as ai}from"./formation-links-CDBKB_z4.js";function _t(e,{navigate:t,toast:i}){e.innerHTML=`
  ...
  `,e.querySelectorAll(".auth-tab").forEach(a=>{a.addEventListener("click",()=>{e.querySelectorAll(".auth-tab").forEach(c=>c.classList.remove("active")),a.classList.add("active"),document.getElementById("tab-login").style.display=a.dataset.tab==="login"?"block":"none",document.getElementById("tab-register").style.display=a.dataset.tab==="register"?"block":"none"})}),document.getElementById("login-btn").addEventListener("click",async()=>{const a=document.getElementById("login-email").value.trim(),c=document.getElementById("login-password").value,n=document.getElementById("login-error");if(n.textContent="",!a||!c){n.textContent="Remplissez tous les champs.";return}const s=document.getElementById("login-btn");s.textContent="Connexion…",s.disabled=!0;const{error:l}=await $.auth.signInWithPassword({email:a,password:c});if(s.textContent="Se connecter",s.disabled=!1,l){n.textContent=l.message.includes("Invalid")?"Email ou mot de passe incorrect.":l.message;return}window.location.reload()}),document.getElementById("login-password").addEventListener("keydown",a=>{a.key==="Enter"&&document.getElementById("login-btn").click()}),document.getElementById("reg-btn").addEventListener("click",async()=>{const a=document.getElementById("reg-email").value.trim(),c=document.getElementById("reg-password").value,n=document.getElementById("reg-confirm").value,s=document.getElementById("reg-error");if(s.textContent="",!a||!c||!n){s.textContent="Remplissez tous les champs.";return}if(c.length<6){s.textContent="Mot de passe trop court (min. 6 caractères).";return}if(c!==n){s.textContent="Les mots de passe ne correspondent pas.";return}const l=document.getElementById("reg-btn");l.textContent="Création…",l.disabled=!0;const{error:d}=await $.auth.signUp({email:a,password:c});if(l.textContent="Créer mon compte",l.disabled=!1,d){s.textContent=d.message;return}i("Compte créé ! Connectez-vous.","success",4e3),document.querySelector('[data-tab="login"]').click(),document.getElementById("login-email").value=a})}function si(e,{state:t,navigate:i,toast:a,refreshProfile:c}){let n="#1A6B3C",s="#D4A017";e.innerHTML=`
  ...
  `;function l(){var j;const o=document.getElementById("logo-preview"),r=document.getElementById("logo-initials"),u=((j=document.getElementById("setup-club"))==null?void 0:j.value)||"MW",g=u.trim().split(" ").filter(Boolean),f=g.length>=2?(g[0][0]+g[1][0]).toUpperCase():u.slice(0,2).toUpperCase();o&&(o.style.background=s,o.style.borderColor=n),r&&(r.textContent=f,r.style.color=n)}document.getElementById("color1").addEventListener("input",o=>{n=o.target.value,document.getElementById("swatch1").style.background=n,l()}),document.getElementById("color2").addEventListener("input",o=>{s=o.target.value,document.getElementById("swatch2").style.background=s,l()});function d(o){document.querySelectorAll(".setup-step").forEach(r=>r.classList.remove("active")),document.getElementById(`step-${o}`).classList.add("active"),document.getElementById("step-num").textContent=o,document.getElementById("progress-fill").style.width=`${Math.round(o/3*100)}%`,o===3&&l()}document.getElementById("step1-next").addEventListener("click",async()=>{const o=document.getElementById("setup-pseudo").value.trim(),r=document.getElementById("step1-error");if(r.textContent="",o.length<3){r.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:u}=await $.from("users").select("id").eq("pseudo",o).maybeSingle();if(u){r.textContent="Ce pseudo est déjà pris.";return}d(2)}),document.getElementById("step2-back").addEventListener("click",()=>d(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const o=document.getElementById("setup-club").value.trim(),r=document.getElementById("step2-error");if(r.textContent="",o.length<2){r.textContent="Nom trop court (min. 2 caractères).";return}const{data:u}=await $.from("users").select("id").eq("club_name",o).maybeSingle();if(u){r.textContent="Ce nom de club est déjà pris.";return}d(3)}),document.getElementById("step3-back").addEventListener("click",()=>d(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const o=document.getElementById("setup-pseudo").value.trim(),r=document.getElementById("setup-club").value.trim(),u=document.getElementById("step3-error"),g=document.getElementById("step3-finish");u.textContent="",g.disabled=!0,g.textContent="Création en cours…";try{const{error:f}=await $.from("users").insert({id:t.user.id,pseudo:o,club_name:r,club_color1:n,club_color2:s,credits:1e4});if(f)throw f;await li(t.user.id),await c(),a(`Bienvenue ${o} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(f){u.textContent=f.message,g.disabled=!1,g.textContent="🚀 Créer mon profil !"}})}async function li(e){const{data:t}=await $.from("players").select("id,job,firstname,surname_encoded,country_code,rarity,note_g,note_d,note_m,note_a,note_min,note_max,skin,hair,hair_length,sell_price").eq("is_active",!0);if(!t||t.length===0)return;const i=t,a=i.filter(d=>d.job==="GK"),c=i.filter(d=>d.job!=="GK"),n=[];for(let d=0;d<5;d++){let o=[];if(d===0&&a.length>0){const r=a[Math.floor(Math.random()*a.length)];o.push(r);const u=$t([...c]).slice(0,3);o.push(...u)}else o=$t([...i]).slice(0,4);o.forEach(r=>{n.push({owner_id:e,player_id:r.id,card_type:"player"})})}["Ressusciter","Double attaque","Bouclier"].forEach(d=>{n.push({owner_id:e,card_type:"game_changer",gc_type:d})});const l=["4-4-2","4-3-3","3-4-3","3-5-2","5-3-2"];n.push({owner_id:e,card_type:"formation",formation:l[Math.floor(Math.random()*l.length)]}),n.length>0&&await $.from("cards").insert(n),await $.from("users").update({first_booster_opened:!0}).eq("id",e)}function $t(e){for(let t=e.length-1;t>0;t--){const i=Math.floor(Math.random()*(t+1));[e[t],e[i]]=[e[i],e[t]]}return e}const di="2026.06.22-1159";async function kt(e,{state:t,navigate:i,toast:a}){var n,s;const c=t.profile;c&&(e.innerHTML=`
  <div class="page">
    <div class="page-body">
  ...
  </div>`}async function gi(e,t){const{state:i,navigate:a,toast:c,openModal:n,closeModal:s}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:l}=await $.from("cards").select(`id, card_type, current_note, gc_type, formation, is_for_sale, sale_price,
      player:players(id, firstname, surname_encoded, country_code, club_id, job, job2,
        note_g, note_d, note_m, note_a, rarity, note_min, note_max, skin, hair, hair_length, sell_price,
        clubs(encoded_name, logo_url))`).eq("owner_id",i.profile.id),{data:d}=await $.from("players").select(`id, firstname, surname_encoded, country_code, club_id, job, job2,
      note_g, note_d, note_m, note_a, rarity, note_min, note_max, skin, hair, hair_length,
      clubs(encoded_name, logo_url)`).eq("is_active",!0),o=(l||[]).filter(b=>b.card_type==="player"&&b.player),r=(l||[]).filter(b=>b.card_type==="game_changer"),u=(l||[]).filter(b=>b.card_type==="formation"),{data:g}=await $.from("gc_definitions").select("name,gc_type,color,effect,image_url"),f={};(g||[]).forEach(b=>{f[b.name]=b});const j=Object.keys(et),p=Object.keys(le),h={};o.forEach(b=>{const y=b.player.id;h[y]=(h[y]||0)+1}),new Set(Object.keys(h).map(b=>String(b)));const T=new Set(u.map(b=>b.formation)),v=new Set(r.map(b=>b.gc_type));let m="player",w="Tous",B="",C=!1;function x(){return[...o].sort((b,y)=>{const L=je.indexOf(b.player.job),k=je.indexOf(y.player.job);return L!==k?L-k:(b.player.surname_encoded||"").localeCompare(y.player.surname_encoded||"")})}function M(){return[...d||[]].sort((b,y)=>{const L=je.indexOf(b.job),k=je.indexOf(y.job);return L!==k?L-k:(b.surname_encoded||"").localeCompare(y.surname_encoded||"")})}function R(){return x().filter(b=>{const y=b.player,L=w==="Tous"||y.job===w,k=!B||`${y.firstname} ${y.surname_encoded}`.toLowerCase().includes(B);return L&&k})}function O(){return M().filter(b=>{const y=w==="Tous"||b.job===w,L=!B||`${b.firstname} ${b.surname_encoded}`.toLowerCase().includes(B);return y&&L})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(p=document.getElementById("direct-sell-form-btn"))==null||p.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${o} pour ${$e.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const v=g.find(w=>!w.is_for_sale)||g[0];if(!v){n("Aucune carte à vendre","error");return}const{error:m}=await $.from("cards").delete().eq("id",v.id);if(m){n(m.message,"error");return}await $.from("users").update({credits:(c.profile.credits||0)+$e}).eq("id",c.profile.id),await d(),n(`+${$e.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),s(),l("collection")}),(h=document.getElementById("market-sell-form-btn"))==null||h.addEventListener("click",async()=>{const v=parseInt(document.getElementById("sell-price-form").value);if(!v||v<1){n("Prix invalide","error");return}await $.from("cards").update({is_for_sale:!0,sale_price:v}).eq("id",e.id),await $.from("market_listings").insert({seller_id:c.profile.id,card_id:e.id,price:v}),n("Carte mise en vente sur le marché !","success"),s(),l("collection")}),(T=document.getElementById("cancel-sell-form-btn"))==null||T.addEventListener("click",async()=>{await $.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await $.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),n("Annonce retirée","success"),s(),l("collection")})}async function At(e,t,i,a){var M,R,O,K,I,H;const{state:c,toast:n,openModal:s,closeModal:l,navigate:d,refreshProfile:o}=a,r=e.player,u=t.filter(N=>N.player.id===r.id),g=u.length,f=fi[r.rarity]||1e3,j=r.rarity!=="legende",p=Rt(r),h=Ie(r,r.job),T=r.job2?Ie(r,r.job2):null,v=Ge[r.job]||"#1A6B3C",m=r.job2?Ge[r.job2]:null,w=Gt[r.rarity]||"#ccc",B=nt[r.country_code]||r.country_code||"",{data:C}=await $.from("transfer_history").select("club_name, manager_name, source, price, transferred_at").eq("player_id",r.id).order("transferred_at",{ascending:!0}),x=C&&C.length?`
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
  ...
  </div>`,document.getElementById("new-deck-btn").addEventListener("click",async()=>{const s=prompt("Nom du deck :",`Deck ${((n==null?void 0:n.length)||0)+1}`);if(!s)return;const{data:l,error:d}=await $.from("decks").insert({owner_id:i.profile.id,name:s}).select().single();if(d){c(d.message,"error");return}c("Deck créé !","success"),Tt(l.id,e,t)}),e.querySelectorAll("[data-open-deck]").forEach(s=>{s.addEventListener("click",()=>Tt(s.dataset.openDeck,e,t))}),e.querySelectorAll("[data-rename]").forEach(s=>{s.addEventListener("click",async()=>{const l=prompt("Nouveau nom :",s.dataset.name);if(!l||l===s.dataset.name)return;const{error:d}=await $.from("decks").update({name:l}).eq("id",s.dataset.rename);if(d){c(d.message,"error");return}c("Deck renommé !","success"),Ve(e,t)})}),e.querySelectorAll("[data-delete]").forEach(s=>{s.addEventListener("click",async()=>{if(!confirm(`Supprimer le deck "${s.dataset.name}" ? Cette action est irréversible.`))return;await $.from("deck_cards").delete().eq("deck_id",s.dataset.delete);const{error:l}=await $.from("decks").delete().eq("id",s.dataset.delete);if(l){c(l.message,"error");return}c("Deck supprimé.","success"),Ve(e,t)})})}async function Tt(e,t,i){const{state:a,toast:c}=i;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:n}=await $.from("decks").select("*").eq("id",e).single(),{data:s}=await $.from("cards").select(`id, card_type, formation,
      player:players(id, firstname, surname_encoded, country_code, club_id, job, job2,
        note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length,
        clubs(encoded_name, logo_url))`).eq("owner_id",a.profile.id),l=(s||[]).filter(f=>f.card_type==="player"&&f.player),d=(s||[]).filter(f=>f.card_type==="formation"),o=d.map(f=>f.formation).filter(Boolean),{data:r}=await $.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",e);let u=n.formation||"4-4-2";o.length>0&&!o.includes(u)&&(u=o[0]);const g={deckId:e,name:n.name,formation:u,formationCardId:n.formation_card_id,slots:{},subs:[],playerCards:l,formationCards:d,availableFormations:o};(r||[]).forEach(f=>{f.is_starter?g.slots[f.position]=f.card_id:g.subs.includes(f.card_id)||g.subs.push(f.card_id)}),be(t,g,i)}function be(e,t,i){var d;const{navigate:a}=i;Re[t.formation];const c=Lt(t.formation),n=c.filter(o=>t.slots[o]).length,s=t.availableFormations.length>0?t.availableFormations:Object.keys(Re),l=t.subs.map(o=>t.playerCards.find(r=>r.id===o)).filter(Boolean);[...Object.values(t.slots),...t.subs],e.innerHTML=`
  <div class="page">
    ...
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(f=document.getElementById("close-selector"))==null||f.addEventListener("click",n),(j=document.getElementById("remove-player"))==null||j.addEventListener("click",()=>{delete t.slots[e],n(),be(i,t,a)}),document.querySelectorAll(".player-option").forEach(p=>{p.addEventListener("click",()=>{t.slots[e]=p.dataset.cardId,n(),be(i,t,a)})})}function bi(e,t,i){var d;const{openModal:a,closeModal:c}=i,n=new Set;Object.values(e.slots).filter(Boolean).forEach(o=>{var u;const r=e.playerCards.find(g=>g.id===o);(u=r==null?void 0:r.player)!=null&&u.id&&n.add(r.player.id)}),e.subs.forEach(o=>{var u;const r=e.playerCards.find(g=>g.id===o);(u=r==null?void 0:r.player)!=null&&u.id&&n.add(r.player.id)});const s=new Set,l=e.playerCards.filter(o=>{var r,u,g;return n.has((r=o.player)==null?void 0:r.id)||s.has((u=o.player)==null?void 0:u.id)?!1:(s.add((g=o.player)==null?void 0:g.id),!0)});a("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${l.length>0?l.map(o=>{var f;const r=o.player,u=He(r),g=r.job==="GK"?r.note_g:r.job==="DEF"?r.note_d:r.job==="MIL"?r.note_m:r.note_a;return`<div class="player-option" data-card-id="${o.id}"
          style="display:flex;align-items:center;gap:10px;padding:8px;border:1.5px solid var(--gray-200);border-radius:10px;cursor:pointer">
    ...
    </div>`,'<button class="btn btn-ghost" id="close-sub-selector">Fermer</button>'),(d=document.getElementById("close-sub-selector"))==null||d.addEventListener("click",c),document.querySelectorAll(".player-option").forEach(o=>{o.addEventListener("click",()=>{e.subs.push(o.dataset.cardId),c(),be(t,e,i)})})}async function wi(e,t){const{state:i,toast:a,navigate:c}=t,n=e.formationCards.find(d=>d.formation===e.formation),s=(n==null?void 0:n.id)||e.formationCardId;await $.from("decks").update({formation:e.formation,formation_card_id:s||null}).eq("id",e.deckId),await $.from("deck_cards").delete().eq("deck_id",e.deckId);const l=[];if(Object.entries(e.slots).forEach(([d,o],r)=>{l.push({deck_id:e.deckId,card_id:o,position:d,is_starter:!0,slot_order:r})}),e.subs.forEach((d,o)=>{l.push({deck_id:e.deckId,card_id:d,position:`SUB${o+1}`,is_starter:!1,slot_order:100+o})}),l.length>0){const{error:d}=await $.from("deck_cards").insert(l);if(d){a(d.message,"error");return}}a("Deck enregistré ✅","success"),c("decks")}function Lt(e){const t=Re[e]||Re["4-4-2"],i=["GK1"];for(let a=1;a<=t.DEF;a++)i.push(`DEF${a}`);for(let a=1;a<=t.MIL;a++)i.push(`MIL${a}`);for(let a=1;a<=t.ATT;a++)i.push(`ATT${a}`);return i}const _i="modulepreload",$i=function(e){return"/manager-wars/"+e},Mt={},ki=function(t,i,a){let c=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const s=document.querySelector("meta[property=csp-nonce]"),l=(s==null?void 0:s.nonce)||(s==null?void 0:s.getAttribute("nonce"));c=Promise.allSettled(i.map(d=>{if(d=$i(d),d in Mt)return;Mt[d]=!0;const o=d.endsWith(".css"),r=o?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${r}`))return;const u=document.createElement("link");if(u.rel=o?"stylesheet":_i,o||(u.as="script"),u.crossOrigin="",u.href=d,l&&u.setAttribute("nonce",l),document.head.appendChild(u),o)return new Promise((g,f)=>{u.addEventListener("load",g),u.addEventListener("error",()=>f(new Error(`Unable to preload CSS for ${d}`)))})}))}function n(s){const l=new Event("vite:preloadError",{cancelable:!0});if(l.payload=s,window.dispatchEvent(l),!l.defaultPrevented)throw s}return c.then(s=>{for(const l of s||[])l.status==="rejected"&&n(l.reason);return t().catch(n)})};async function Ot(){const{data:e}=await $.from("booster_configs").select("*").eq("is_active",!0).order("sort_order");if(!(e!=null&&e.length))return[];const{data:t}=await $.from("booster_drop_rates").select("*").in("booster_id",e.map(i=>i.id)).order("sort_order");return e.map(i=>({...i,rates:(t||[]).filter(a=>a.booster_id===i.id)}))}function Ei(e){if(!(e!=null&&e.length))return null;const t=e.reduce((a,c)=>a+Number(c.percentage),0);let i=Math.random()*t;for(const a of e)if(i-=Number(a.percentage),i<=0)return a;return e[e.length-1]}const Ii=Object.freeze(Object.defineProperty({__proto__:null,loadActiveBoosters:Ot,rollDropRate:Ei},Symbol.toStringTag,{value:"Module"})),Ht=()=>Object.keys(Me),Ai=[{id:"players_std",img:"/manager-wars/icons/booster-players.png",name:"Players",sub:"5 cartes joueurs",cost:5e3,costLabel:"5 000 crédits",cardCount:5,type:"player"},{id:"players_pub",img:"/manager-wars/icons/booster-silver.png",name:"Players (pub)",sub:"3 cartes joueurs",cost:0,costLabel:"1 pub",cardCount:3,type:"player"},{id:"game_changer",img:"/manager-wars/icons/booster-gamechanger.png",name:"Game Changer",sub:"3 cartes spéciales",cost:1e4,costLabel:"10 000 crédits",cardCount:3,type:"game_changer"},{id:"formation",img:"/manager-wars/icons/booster-formation.png",name:"Formation",sub:"1 carte formation",cost:1e4,costLabel:"10 000 crédits",cardCount:1,type:"formation"}],Ye={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé."},"Double attaque":{icon:"⚡",desc:"La prochaine attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la prochaine action IA."},Gel:{icon:"❄️",desc:"Bloque le meilleur attaquant IA."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function Ti(e){var c;const t={};(e.rates||[]).forEach(n=>{t[n.card_type]=(t[n.card_type]||0)+Number(n.percentage||0)});const i=((c=Object.entries(t).sort((n,s)=>s[1]-n[1])[0])==null?void 0:c[0])||"player",a=e.image_url||"booster-players.png";return{id:e.id,img:"/manager-wars/icons/"+a,name:e.name,sub:`${e.card_count} carte(s)`,cost:e.price_type==="credits"&&e.price_credits||0,costLabel:e.price_type==="credits"?`${(e.price_credits||0).toLocaleString("fr")} crédits`:e.price_type==="pub"?"1 pub":"Gratuit",cardCount:e.card_count||5,type:i,isPub:e.price_type==="pub",rates:e.rates||[],_raw:e}}async function Li(e,{state:t,navigate:i,toast:a}){var s;const c=((s=t.profile)==null?void 0:s.credits)||0;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⏳ Chargement...</div>';let n=[];try{n=(await Ot()).map(Ti)}catch(l){console.warn("Erreur chargement boosters DB, fallback hardcodé",l)}n.length||(n=Ai.map(l=>({...l,rates:[],isPub:l.id==="players_pub"}))),e.innerHTML=`
  <div class="page">
    <div class="page-header">
      <h2>📦 Boosters</h2>
    ...
    <div class="page-body">
      <div class="booster-grid">
        ${n.map(l=>{const d=l.cost===0||c>=l.cost;return`<div class="booster-card ${d?"":"disabled"}" data-booster="${l.id}" style="position:relative">
  ...
  </div>`,e.querySelectorAll(".booster-card:not(.disabled)").forEach(l=>{l.addEventListener("click",async()=>{const d=n.find(o=>o.id===l.dataset.booster);if(d){l.style.opacity="0.5",l.style.pointerEvents="none";try{await Mi(d,{state:t,toast:a,navigate:i,container:e})}catch(o){a(o.message,"error"),l.style.opacity="",l.style.pointerEvents=""}}})}),e.querySelectorAll(".booster-info-btn").forEach(l=>{l.addEventListener("click",d=>{d.stopPropagation();const o=n.find(r=>r.id===l.dataset.boosterId);qi(o)})})}async function Mi(e,{state:t,toast:i,navigate:a,container:c}){var r;if(e.cost>0&&t.profile.credits<e.cost){i("Crédits insuffisants","error");return}if(e.isPub)try{await Ni()}catch(u){i(u.message||"Regardez la pub entièrement pour ouvrir le booster","error");return}const{data:n}=await $.from("cards").select("card_type, player_id, formation").eq("owner_id",t.profile.id),s=new Set((n||[]).filter(u=>u.card_type==="player").map(u=>u.player_id)),l=new Set((n||[]).filter(u=>u.card_type==="formation").map(u=>u.formation));let d=[];if((r=e.rates)!=null&&r.length)d=await zt(t.profile,e);else{const u=e.type||"player";u==="player"?d=await ji(t.profile,e.cardCount,e.cost):u==="game_changer"?d=await Si(t.profile,e.cardCount,e.cost):u==="formation"?d=await Bi(t.profile,e.cost):d=await zt(t.profile,e)}d.forEach(u=>{u.card_type==="player"&&u.player?u.isDuplicate=s.has(u.player.id):u.card_type==="formation"&&(u.isDuplicate=l.has(u.formation))});const{data:o}=await $.from("users").select("*").eq("id",t.profile.id).single();o&&(t.profile=o),Fi(d,e,a)}function zi(){const e=Math.random()*100;return e<.5?"legende":e<2?"special":e<10?"normal_high":"normal_low"}function he(e){return Math.max(Number(e.note_g)||0,Number(e.note_d)||0,Number(e.note_m)||0,Number(e.note_a)||0)}function Ci(e,t){let i;switch(t){case"legende":i=e.filter(a=>a.rarity==="legende"),i.length||(i=e.filter(a=>a.rarity==="pepite"||a.rarity==="papyte")),i.length||(i=e.filter(a=>he(a)>=6));break;case"special":i=e.filter(a=>a.rarity==="pepite"||a.rarity==="papyte"),i.length||(i=e.filter(a=>he(a)>=6));break;case"normal_high":i=e.filter(a=>a.rarity==="normal"&&he(a)>=6),i.length||(i=e.filter(a=>he(a)>=6));break;default:i=e.filter(a=>a.rarity==="normal"&&he(a)>=1&&he(a)<=5),i.length||(i=e.filter(a=>a.rarity==="normal"));break}return i.length||(i=e),i[Math.floor(Math.random()*i.length)]}async function zt(e,t){if(t.cost>0){const{error:c}=await $.from("users").update({credits:e.credits-t.cost}).eq("id",e.id);if(c)throw c}const{rollDropRate:i}=await ki(async()=>{const{rollDropRate:c}=await Promise.resolve().then(()=>Ii);return{rollDropRate:c}},void 0),a=[];for(let c=0;c<(t.cardCount||5);c++){const n=i(t.rates);if(n){if(n.card_type==="player"){let s=$.from("players").select("id,job,firstname,surname_encoded,country_code,club_id,rarity,note_g,note_d,note_m,note_a,skin,hair,hair_length,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);n.rarity&&(s=s.eq("rarity",n.rarity));const{data:l}=await s;let d=l||[];if((n.note_min||n.note_max)&&(d=d.filter(u=>{const g=Math.max(Number(u.note_g)||0,Number(u.note_d)||0,Number(u.note_m)||0,Number(u.note_a)||0);return(!n.note_min||g>=n.note_min)&&(!n.note_max||g<=n.note_max)})),d.length||(n.note_min||n.note_max?(d=l||[],console.warn("[Booster] Aucun joueur avec note",n.note_min,"-",n.note_max,"— fallback rareté uniquement")):d=l||[]),!d.length)continue;const o=d[Math.floor(Math.random()*d.length)],{data:r}=await $.from("cards").insert({owner_id:e.id,player_id:o.id,card_type:"player"}).select().single();r&&(a.push({...r,player:o}),$.rpc("record_transfer",{p_card_id:r.id,p_player_id:o.id,p_club_name:e.club_name||e.pseudo,p_manager_name:e.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{}))}else if(n.card_type==="game_changer"){const{data:s}=await $.from("gc_definitions").select("id,name").eq("is_active",!0).eq("gc_type","game_changer"),l=s!=null&&s.length?s:[{name:"Ressusciter"},{name:"Double attaque"},{name:"Bouclier"},{name:"Vol de note"},{name:"Gel"}],o=l[Math.floor(Math.random()*l.length)].name,{data:r}=await $.from("cards").insert({owner_id:e.id,card_type:"game_changer",gc_type:o}).select().single();r&&a.push(r)}else if(n.card_type==="formation"){const s=Ht(),l=s[Math.floor(Math.random()*s.length)],{data:d}=await $.from("cards").insert({owner_id:e.id,card_type:"formation",formation:l}).select();d!=null&&d[0]&&a.push(d[0])}}}return a}async function ji(e,t,i){if(i>0){const{error:o}=await $.from("users").update({credits:e.credits-i}).eq("id",e.id);if(o)throw o}const{data:a}=await $.from("players").select("id,job,firstname,surname_encoded,country_code,club_id,rarity,note_g,note_d,note_m,note_a,note_min,note_max,skin,hair,hair_length,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);if(!(a!=null&&a.length))throw new Error("Pas de joueurs en BDD — ajoutes-en via le panel admin !");const c=a.filter(o=>o.job==="GK"),n=a.filter(o=>o.job!=="GK"),s=!e.first_booster_opened&&c.length>0,l=[];for(let o=0;o<t;o++){const r=o===0&&s?c:o===0?n:a,u=zi(),g=Ci(r,u);g&&l.push(g)}s&&await $.from("users").update({first_booster_opened:!0}).eq("id",e.id);const{data:d}=await $.from("cards").insert(l.map(o=>({owner_id:e.id,player_id:o.id,card_type:"player"}))).select();return(d||[]).forEach((o,r)=>{$.rpc("record_transfer",{p_card_id:o.id,p_player_id:l[r].id,p_club_name:e.club_name||e.pseudo,p_manager_name:e.pseudo,p_source:"booster",p_price:null}).then(()=>{}).catch(()=>{})}),l.map((o,r)=>({...d[r],player:o}))}async function Si(e,t,i){const{error:a}=await $.from("users").update({credits:e.credits-i}).eq("id",e.id);if(a)throw a;const{data:c}=await $.from("gc_definitions").select("id,name,gc_type,color,effect,image_url").eq("is_active",!0),n=c!=null&&c.length?c:Object.keys(Ye).map(r=>({name:r,gc_type:"game_changer"})),s=Array.from({length:t},()=>{const r=n[Math.floor(Math.random()*n.length)];return{owner_id:e.id,card_type:"game_changer",gc_type:r.name,gc_definition_id:r.id||null}}),{data:l,error:d}=await $.from("cards").insert(s).select();return d&&console.error("[Booster GC] Erreur insert:",d.message,d),(l||[]).map(r=>{const u=c==null?void 0:c.find(g=>g.name===r.gc_type||g.id===r.gc_definition_id);return{...r,_gcDef:u||null}})}async function Bi(e,t){const{error:i}=await $.from("users").update({credits:e.credits-t}).eq("id",e.id);if(i)throw i;const a=Ht(),c=a[Math.floor(Math.random()*a.length)],{data:n,error:s}=await $.from("cards").insert({owner_id:e.id,card_type:"formation",formation:c}).select();return s&&console.error("[Booster Formation] Erreur insert:",s.message,s),n||[]}function Fi(e,t,i){var K;if(!e||e.length===0){const I=document.createElement("div");I.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:3000;gap:16px;color:#fff;padding:24px;text-align:center",I.innerHTML=`
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}const Ri="simulation",Kt="VOTRE_ZONE_ID";function Ni(){switch(Ri){case"propellerads":return Pi();case"adsense":return Oi();default:return rt()}}function rt(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:#111;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:14px";let i=5;t.innerHTML=`
    ...
    </div>`}const d=r=>{e.style.overflow="",e.style.height="",e.style.display="",e.style.flexDirection="",i(r)};function o(){var u,g,f;e.style.overflow="hidden",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column";const r=s.length>0;e.innerHTML=`
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      <div style="flex:1;overflow-y:auto;display:flex;flex-wrap:wrap;gap:10px;justify-content:center;align-content:flex-start;padding:8px 16px 16px">
        ${n.map(j=>{const p=s.find(h=>h.gc_type===j.gc_type);return l(j,!!p)}).join("")}
      </div>
    ...
    </div>`,e.querySelectorAll(".gc-select-card").forEach(j=>{j.addEventListener("click",()=>{const p=j.dataset.id,h=n.find(v=>v.id===p);if(!h)return;const T=s.findIndex(v=>v.gc_type===h.gc_type);T>-1?s.splice(T,1):s.length<3&&s.push(h),o()})}),(u=e.querySelector("#gc-launch"))==null||u.addEventListener("click",()=>{r&&d(s)}),(g=e.querySelector("#gc-no-gc"))==null||g.addEventListener("click",()=>d([])),(f=e.querySelector("#gc-reset"))==null||f.addEventListener("click",()=>{s.length&&(s=[],o())})}o()}async function Hi(e,t,i){const{state:a,navigate:c}=t;e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:n}=await $.from("decks").select("id,name,is_active,formation").eq("owner_id",a.profile.id).order("created_at",{ascending:!1});if(!n||n.length===0){fe(e,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>c("decks"));return}const s=n.map(r=>r.id),{data:l}=await $.from("deck_cards").select(`deck_id, position, is_starter, slot_order,
      card:cards(id,card_type,formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).in("deck_id",s).order("slot_order");let d=0;function o(){var v,m,w,B,C;const r=n[d],u=(l||[]).filter(x=>x.deck_id===r.id),g=u.filter(x=>{var M;return x.is_starter&&((M=x.card)==null?void 0:M.player)}).map(x=>We(x.card,x.position)),f=u.find(x=>{var M;return((M=x.card)==null?void 0:M.card_type)==="formation"}),j=r.formation||((v=f==null?void 0:f.card)==null?void 0:v.formation)||"4-4-2",p=g.length>=11?Pe(g,j):null,h=g.length>=11;Vt(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">
        ${n.map((x,M)=>`<div style="width:7px;height:7px;border-radius:50%;background:${M===d?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}
      </div>`:""}
    ...
    </div>`,function(){const M=e.querySelector(".deck-preview-wrap svg");M&&(M.removeAttribute("width"),M.removeAttribute("height"),M.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",M.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),(m=document.getElementById("prev-deck"))==null||m.addEventListener("click",()=>{d>0&&(d--,o())}),(w=document.getElementById("next-deck"))==null||w.addEventListener("click",()=>{d<n.length-1&&(d++,o())}),(B=document.getElementById("validate-deck"))==null||B.addEventListener("click",()=>{h&&t.navigate("match",{matchMode:i,deckId:r.id})}),(C=document.getElementById("cancel-deck-select"))==null||C.addEventListener("click",()=>{de(e),c("home")});const T=document.getElementById("deck-swipe-zone");if(T){let x=0,M=0;T.addEventListener("touchstart",R=>{x=R.touches[0].clientX,M=R.touches[0].clientY},{passive:!0}),T.addEventListener("touchend",R=>{const O=R.changedTouches[0].clientX-x,K=R.changedTouches[0].clientY-M;Math.abs(O)<40||Math.abs(O)<Math.abs(K)||(O<0&&d<n.length-1?(d++,o()):O>0&&d>0&&(d--,o()))},{passive:!0})}}o()}function Wt(e){const t="https://fcnwclxlkytdfjotqkta.supabase.co";return e!=null&&e.clubLogo?e.clubLogo.startsWith("http")?e.clubLogo:`${t}/storage/v1/object/public/assets/clubs/${e.clubLogo}`:null}function pe(e,t=44,i=58){if(!e)return`<div style="width:${t}px;height:${i}px;border:1.5px dashed rgba(255,255,255,0.2);border-radius:5px"></div>`;const a=typeof e.portrait=="string"&&e.portrait.startsWith("http")?e.portrait:ne(e),c=Wt(e),n=e._line||e.job||"MIL",s=_e[n]||"#555",l={normal:"#aaa",pépite:"#D4A017",papyte:"#222",légende:"#7a28b8"}[e==null?void 0:e.rarity]||"#aaa",d=e.note!==void 0?Number(e.note)||0:(Number(ie(e,n))||0)+(e.boost||0),o=Xt(e==null?void 0:e.country_code),r=Math.round(i*.19),u=Math.round(i*.25),g=i-Math.round(i*.19)-Math.round(i*.25),f=e!=null&&e.used?.28:1;return`<div style="width:${t}px;height:${i}px;border-radius:5px;border:2px solid ${l};background:${s};position:relative;overflow:hidden;flex-shrink:0;opacity:${f}">
    <div style="position:absolute;top:0;left:0;right:0;height:${r}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:center;z-index:2">
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;height:${u}px;background:rgba(255,255,255,0.93);display:flex;align-items:center;justify-content:space-between;padding:0 3px;z-index:2">
      ...
      <span style="font-size:${u-2}px;font-weight:900;color:#111;font-family:Arial Black,Arial">${e!=null&&e.used?"–":d}</span>
  ...
  </div>`}function Ee(e,t,i){if(!(e!=null&&e.length))return"";const a=e.slice(0,5);let c='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return a.forEach((n,s)=>{if(c+=pe(n,40,52),s<a.length-1){const l=ge(n,a[s+1]);c+=`<div style="width:7px;height:3px;background:${l==="#ff3333"||l==="#cc2222"?"rgba(255,255,255,0.12)":l};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(c+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),c+="</div>",c}function Xe(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function Xt(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function Ae(e,t,i,a,c=310,n=310){const s=Me[t]||{},l=Ft(t)||et[t]||[],d={},o=["ATT","MIL","DEF","GK"];for(const v of o)(e[v]||[]).forEach((w,B)=>{d[`${v}${B+1}`]=w});function r(v){const m=s[v];return m?{x:m.x*c,y:m.y*n}:null}let u="";for(const[v,m]of l){const w=r(v),B=r(m);if(!w||!B)continue;const C=d[v],x=d[m],M=ge(C,x);M==="#00ff88"||M==="#FFD700"?(u+=`<line x1="${w.x.toFixed(1)}" y1="${w.y.toFixed(1)}" x2="${B.x.toFixed(1)}" y2="${B.y.toFixed(1)}"
  ...
  </div>`}async function Jt(e,t,i,a){var B;const{state:c,navigate:n,toast:s}=t;Vt(e);const l=c.params||{};if(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>',!l.deckId)return Hi(e,t,i);const d=l.deckId;let o,r,u,g;try{const C=await Promise.all([$.from("decks").select("formation,name").eq("id",d).single(),$.from("deck_cards").select(`position, is_starter, slot_order,
          card:cards(id, card_type, formation,
            player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
              note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
              clubs(encoded_name,logo_url)))`).eq("deck_id",d).order("slot_order")]);o=C[0].data,u=C[0].error,r=C[1].data,g=C[1].error}catch(C){console.error("[Match] Exception chargement deck:",C),fe(e,"⚠️","Erreur réseau lors du chargement du deck. Réessaie.","Retour",()=>n("home"));return}if(u||g){console.error("[Match] Erreur Supabase:",u||g),fe(e,"⚠️","Erreur lors du chargement du deck.","Retour",()=>n("home"));return}const f=(r||[]).filter(C=>{var x;return C.is_starter&&((x=C.card)==null?void 0:x.player)}).map(C=>We(C.card,C.position)),j=(r||[]).filter(C=>{var x;return!C.is_starter&&((x=C.card)==null?void 0:x.player)}).map(C=>We(C.card,C.position));if(f.length<11){fe(e,"⚠️",`Deck incomplet (${f.length}/11).`,"Compléter",()=>n("decks"));return}const p=(r||[]).find(C=>{var x;return((x=C.card)==null?void 0:x.card_type)==="formation"}),h=(o==null?void 0:o.formation)||((B=p==null?void 0:p.card)==null?void 0:B.formation)||"4-4-2",{data:T,error:v}=await $.from("cards").select("id, gc_type, gc_definition_id").eq("owner_id",c.profile.id).eq("card_type","game_changer"),{data:m}=await $.from("gc_definitions").select("*").eq("is_active",!0),w=(T||[]).map(C=>({...C,_gcDef:(m==null?void 0:m.find(x=>x.name===C.gc_type||x.id===C.gc_definition_id))||null}));a({deckId:d,formation:h,starters:f,subsRaw:j,gcCardsEnriched:w,gcDefs:m||[]})}async function Ki(e,t){const{state:i}=t,c=(i.params||{}).matchMode||"vs_ai_easy",n=c.replace("vs_ai_",""),s=c;await Jt(e,t,c,async({deckId:l,formation:d,starters:o,subsRaw:r,gcCardsEnriched:u,gcDefs:g})=>{try{const f=Pe(o,d),j=await Ui(d,n),p=async h=>{try{const{data:T,error:v}=await $.from("matches").insert({home_id:i.profile.id,away_id:null,mode:s,home_deck_id:l,status:"in_progress"}).select().single();if(v){console.error("[MatchIA] Erreur création match:",v),fe(e,"⚠️","Impossible de créer le match ("+v.message+").","Retour",()=>t.navigate("home"));return}const m={gcDefs:g||[],matchId:T==null?void 0:T.id,mode:s,difficulty:n,formation:d,homeTeam:f,aiTeam:j,homeSubs:r,subsUsed:0,maxSubs:Math.min(r.length,3),homeScore:0,aiScore:0,gcCards:h,usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Yi(e,m,t)}catch(T){console.error("[MatchIA] Exception launchMatch:",T),fe(e,"⚠️","Erreur au lancement du match : "+T.message,"Retour",()=>t.navigate("home"))}};if(!u.length){p([]);return}Yt(e,u,p)}catch(f){console.error("[MatchIA] Exception setup:",f),fe(e,"⚠️","Erreur de préparation du match : "+f.message,"Retour",()=>t.navigate("home"))}})}async function Ui(e,t){var s,l;const{data:i}=await $.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,clubs(encoded_name,logo_url)").eq("is_active",!0).limit(60);if(!i||i.length<11)return Vi(e);const a=we[e]||we["4-4-2"],c={GK:[],DEF:[],MIL:[],ATT:[]},n=[...i];for(const d of["GK","DEF","MIL","ATT"]){const o=n.filter(j=>j.job===d),r=n.filter(j=>j.job!==d),u=[...o,...r],g=[];for(let j=0;j<a[d];j++){const p=u[j]||n[j];p&&g.push({cardId:"ai-"+p.id+"-"+j,id:p.id,firstname:p.firstname,name:p.surname_encoded,country_code:p.country_code,club_id:p.club_id,job:p.job,job2:p.job2,note_g:Number(p.note_g)||0,note_d:Number(p.note_d)||0,note_m:Number(p.note_m)||0,note_a:Number(p.note_a)||0,rarity:p.rarity,skin:p.skin,hair:p.hair,hair_length:p.hair_length,clubName:((s=p.clubs)==null?void 0:s.encoded_name)||null,clubLogo:((l=p.clubs)==null?void 0:l.logo_url)||null,boost:0,used:!1,_line:d})}const f=Ne(g.length);g.forEach((j,p)=>{j._col=f[p]}),c[d]=g}return c}function Vi(e){const t=we[e]||we["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},a=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let c=0;for(const n of["GK","DEF","MIL","ATT"]){const s=[];for(let d=0;d<t[n];d++){const o=3+Math.floor(Math.random()*5);s.push({cardId:"fake-"+c,id:"fake-"+c,firstname:"IA",name:a[c%a.length],country_code:"XX",club_id:null,job:n,job2:null,note_g:n==="GK"?o:2,note_d:n==="DEF"?o:2,note_m:n==="MIL"?o:2,note_a:n==="ATT"?o:2,rarity:"normal",boost:0,used:!1,_line:n}),c++}const l=Ne(s.length);s.forEach((d,o)=>{d._col=l[o]}),i[n]=s}return i}function Yi(e,t,i){e.innerHTML=`
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    ${(()=>{const x=window.innerWidth>=700,M=(E,A,G)=>{var W,J;const S=(t.gcDefs||[]).find(te=>te.name===E.gc_type),b={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[S==null?void 0:S.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",y={purple:"#b06ce0",light_blue:"#00d4ef"}[S==null?void 0:S.color]||"#b06ce0",L=(S==null?void 0:S.name)||E.gc_type,k=(S==null?void 0:S.effect)||((W=le[E.gc_type])==null?void 0:W.desc)||"",_=S!=null&&S.image_url?`/manager-wars/icons/${S.image_url}`:null,q=((J=le[E.gc_type])==null?void 0:J.icon)||"⚡",F=Math.round(G*.22),P=Math.round(G*.22),D=G-F-P,Y=L.length>12?7:9;return`<div class="gc-mini" data-gc-id="${E.id}" data-gc-type="${E.gc_type}"
          ...
          </div>`},O=(E,A)=>A?R(110,150):M(E,110,150),K=(E,A)=>A?R(68,95):M(E,68,95),I=x?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",H=r?`<button id="btn-results" style="${I};background:linear-gradient(135deg,#D4A017,#FFD700);border:none;color:#000">🏁 Résultats</button>`:l?`<div style="${I};background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);color:rgba(255,255,255,0.4)">⏳ Tour IA</div>`:d?`<button id="btn-action" style="${I};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${t.selected.length===0?"disabled":""}> ⚔️ ATTAQUEZ <span id="match-timer" style="font-weight:900"></span></button>`:o?`<button id="btn-action" style="${I};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${t.selected.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="match-timer" style="font-weight:900"></span></button>`:`<div style="${I};background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1)"></div>`,N=d||o?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${t.selected.length}/3 sélectionné(s)</div>`:"",V=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${x?56:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
  ...
  </div>`;function f(){const x=e.querySelector(".match-screen");if(!x)return;const M=Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight);x.style.bottom="auto",x.style.height=M+"px",x.style.minHeight=M+"px",x.style.maxHeight=M+"px",x.style.overflow="hidden";const R=e.querySelector("#mobile-action-bar"),O=e.querySelector("#mobile-play-area");R&&O&&(O.style.paddingBottom=R.offsetHeight+"px")}if(f(),setTimeout(f,120),setTimeout(f,400),setTimeout(f,1e3),t._vvBound||(t._vvBound=!0,window.visualViewport&&(window.visualViewport.addEventListener("resize",f),window.visualViewport.addEventListener("scroll",f)),window.addEventListener("resize",f)),function(){const M=e.querySelector(".terrain-wrapper svg");M&&(M.removeAttribute("width"),M.removeAttribute("height"),M.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0",M.setAttribute("viewBox","-26 -26 352 352"),M.setAttribute("preserveAspectRatio","xMidYMid meet"))}(),t._resizeBound||(t._resizeBound=!0,window.addEventListener("resize",()=>{const x=e.querySelector(".terrain-wrapper svg");x&&(x.style.cssText="width:100%;height:100%;display:block;max-width:none;margin:0")})),t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase==="attack"||t.phase==="defense"){let x=!1,M=30;const R=()=>document.getElementById("match-timer"),O=()=>{const K=R();if(!K)return;const I=String(Math.floor(M/60)).padStart(2,"0"),H=String(M%60).padStart(2,"0");K.textContent=` ${I}:${H}`,K.style.color=x?"#ff2222":"#ff9500",K.style.fontWeight="900"};O(),t._timerInt=setInterval(()=>{if(M--,M<0)if(!x)x=!0,M=15,O();else{clearInterval(t._timerInt),t._timerInt=null,t.homeScore=0,t.aiScore=3;const K=document.createElement("div");K.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;color:#fff;padding:24px;text-align:center",K.innerHTML='<div style="font-size:56px">⏱️</div><div style="font-size:24px;font-weight:900;color:#ff4444">MATCH PERDU PAR FORFAIT</div><div style="font-size:14px;color:rgba(255,255,255,0.6)">Temps écoulé</div>',document.body.appendChild(K),setTimeout(()=>{K.remove(),ve(e,t,i)},2500)}else O()},1e3)}(p=document.getElementById("match-quit"))==null||p.addEventListener("click",()=>{de(e),confirm("Abandonner ? Résultat : défaite 3-0")&&(t.homeScore=0,t.aiScore=3,ve(e,t,i))}),(h=document.getElementById("view-ai"))==null||h.addEventListener("click",()=>ro(t,i)),(T=document.getElementById("toggle-history"))==null||T.addEventListener("click",()=>{var x;(x=document.getElementById("match-history-panel"))==null||x.classList.add("open")}),(v=document.getElementById("close-history"))==null||v.addEventListener("click",()=>{var x;(x=document.getElementById("match-history-panel"))==null||x.classList.remove("open")}),(m=document.getElementById("btn-action"))==null||m.addEventListener("click",()=>{t.selected.length!==0&&(d?Zi(e,t,i):o&&Qi(e,t,i))}),(w=document.getElementById("btn-results"))==null||w.addEventListener("click",()=>ve(e,t,i)),e.querySelectorAll(".match-slot-hit").forEach(x=>{x.addEventListener("click",()=>Ji(x,t,e,i))}),e.querySelectorAll(".gc-mini").forEach(x=>{x.addEventListener("click",()=>io(x.dataset.gcId,x.dataset.gcType,e,t,i))}),(B=document.getElementById("boost-card"))==null||B.addEventListener("click",()=>no(e,t,i)),e.querySelectorAll(".sub-btn-col").forEach(x=>{x.addEventListener("click",()=>Ct(e,t,i,x.dataset.subId))}),(C=document.getElementById("sub-btn-main"))==null||C.addEventListener("click",()=>Ct(e,t,i))}function Ji(e,t,i,a){const c=e.dataset.cardId,n=e.dataset.role,s=t.selected.findIndex(l=>l.cardId===c);if(s!==-1)t.selected.splice(s,1);else{if(t.selected.length>=3){a.toast("Maximum 3 joueurs","error");return}const l=(t.homeTeam[n]||[]).find(d=>d.cardId===c);l&&t.selected.push({...l,_role:n,_line:n})}oe(i,t,a)}function at(e,t,i){e.matchId&&$.from("matches").update({last_player_id:i}).eq("id",e.matchId).then(()=>{})}function Zi(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),at(t,i,i.state.profile.id);const a=t.selected.map(n=>({...(t.homeTeam[n._role]||[]).find(l=>l.cardId===n.cardId)||n,_line:n._role})),c=tt(a,t.modifiers.home);t.pendingAttack={...c,players:[...a],side:"home"},t.selected.forEach(n=>{const s=(t.homeTeam[n._role]||[]).find(l=>l.cardId===n.cardId);s&&(s.used=!0)}),t.log.push({text:`⚔️ Vous attaquez : ${c.total} (base ${c.base}${c.links?` +${c.links} liens`:""}) — ${t.selected.map(n=>n.name).join(", ")}`,type:"info"}),t.selected=[],t.modifiers.home={},t.phase="ai-defense",oe(e,t,i),setTimeout(()=>eo(e,t,i),1200)}function Qi(e,t,i){t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),at(t,i,i.state.profile.id);const a=t.selected.map(l=>({...(t.homeTeam[l._role]||[]).find(o=>o.cardId===l.cardId)||l,_line:l._role})),c=it(a,t.modifiers.home);t.selected.forEach(l=>{const d=(t.homeTeam[l._role]||[]).find(o=>o.cardId===l.cardId);d&&(d.used=!0)});const n=ot(t.pendingAttack.total,c.total,t.modifiers.home),s={type:"duel",title:"Défense",aiPlayers:(t.pendingAttack.players||[]).map(l=>({name:l.name,note:l._line==="MIL"?l.note_m:l.note_a,portrait:ne(l),job:l.job,country_code:l.country_code,rarity:l.rarity,clubName:l.clubName,clubLogo:l.clubLogo})),homePlayers:t.selected.map(l=>{const d=(t.homeTeam[l._role]||[]).find(o=>o.cardId===l.cardId)||l;return{name:d.name,note:(d._line==="GK"?Number(d.note_g)||0:d._line==="MIL"?Number(d.note_m)||0:Number(d.note_d)||0)+(d.boost||0),portrait:ne(d),job:d.job,country_code:d.country_code,rarity:d.rarity,clubName:d.clubName,clubLogo:d.clubLogo}}),homeTotal:c.total,aiTotal:t.pendingAttack.total,isGoal:!1,homeScored:!1,text:""};if(n.shielded)s.text="🛡️ Bouclier ! But annulé.",t.log.push(s);else if(n.goal){t.aiScore++,s.isGoal=!0,s.homeScored=!1,s.text=`⚽ BUT IA ! (${t.pendingAttack.total} > ${c.total})`,t.log.push(s),t.selected=[],t.modifiers.home={},t.pendingAttack=null,oe(e,t,i),st(s.aiPlayers,t.homeScore,t.aiScore,!1,()=>{Le(e,t,i,"home-attack")});return}else s.text=`🧤 Défense réussie ! (${c.total} ≥ ${t.pendingAttack.total})`,t.log.push(s);t.selected=[],t.modifiers.home={},t.pendingAttack=null,Le(e,t,i,"home-attack")}function Je(e,t,i){at(t,i,null);const a=[...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]],c=qt(a,"attack",t.difficulty);if(!c.length){Qt(e,t,i);return}const n=tt(c,t.modifiers.ai);t.pendingAttack={...n,players:c,side:"ai"},c.forEach(o=>{o.used=!0}),t.log.push({text:`🤖 IA attaque : ${n.total} (${c.map(o=>o.name).join(", ")})`,type:"info"}),t.modifiers.ai={};const s=[...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(o=>!o.used),d=(t.homeSubs||[]).filter(o=>!(t.usedSubIds||[]).includes(o.cardId)).length>0&&t.subsUsed<t.maxSubs;if(s.length===0&&!d){t.aiScore++;const o={type:"duel",isGoal:!0,homeScored:!1,aiPlayers:c.map(r=>({name:r.name,note:r._line==="MIL"?r.note_m:r.note_a,portrait:ne(r),job:r.job,country_code:r.country_code,rarity:r.rarity,clubName:r.clubName,clubLogo:r.clubLogo})),aiTotal:n.total,text:"⚽ BUT IA ! (aucun défenseur disponible)"};t.log.push(o),t.pendingAttack=null,oe(e,t,i),st(o.aiPlayers,t.homeScore,t.aiScore,!1,()=>{Le(e,t,i,"home-attack")});return}t.phase="defense",oe(e,t,i)}function eo(e,t,i){const a=[...t.aiTeam.GK||[],...t.aiTeam.DEF||[],...t.aiTeam.MIL||[]],c=qt(a,"defense",t.difficulty),n=c.length>0?it(c,t.modifiers.ai).total:0;c.forEach(d=>{d.used=!0});const s=ot(t.pendingAttack.total,n,t.modifiers.ai),l={type:"duel",title:"Attaque",homePlayers:(t.pendingAttack.players||[]).map(d=>({name:d.name,note:d._line==="MIL"?d.note_m:d.note_a,portrait:ne(d),job:d.job,country_code:d.country_code,rarity:d.rarity,clubName:d.clubName,clubLogo:d.clubLogo})),aiPlayers:c.map(d=>({name:d.name,note:d._line==="GK"?d.note_g:d._line==="MIL"?d.note_m:d.note_d,portrait:ne(d),job:d.job,country_code:d.country_code,rarity:d.rarity,clubName:d.clubName,clubLogo:d.clubLogo})),homeTotal:t.pendingAttack.total,aiTotal:n,isGoal:!1,homeScored:!1,text:""};if(s.shielded)l.text="🛡️ Bouclier IA !",t.log.push(l);else if(s.goal){t.homeScore++,l.isGoal=!0,l.homeScored=!0,l.text=`⚽ BUT ! (${t.pendingAttack.total} > ${n})`,t.log.push(l),t.modifiers.ai={},t.pendingAttack=null,oe(e,t,i),st(l.homePlayers,t.homeScore,t.aiScore,!0,()=>{Le(e,t,i,"ai-attack")});return}else l.text=`🧤 IA défend (${n} ≥ ${t.pendingAttack.total})`,t.log.push(l);t.modifiers.ai={},t.pendingAttack=null,Le(e,t,i,"ai-attack")}function Le(e,t,i,a){if(t.round++,Zt(t)){ve(e,t,i);return}if(a==="home-attack"){if(![...t.homeTeam.MIL||[],...t.homeTeam.ATT||[]].filter(n=>!n.used).length){if(![...t.homeTeam.GK||[],...t.homeTeam.DEF||[],...t.homeTeam.MIL||[]].filter(s=>!s.used).length){ve(e,t,i);return}t.phase="ai-attack",oe(e,t,i),setTimeout(()=>Je(e,t,i),800);return}t.phase="attack",oe(e,t,i)}else{if(![...t.aiTeam.MIL||[],...t.aiTeam.ATT||[]].filter(n=>!n.used).length){Qt(e,t,i);return}t.phase="ai-attack",oe(e,t,i),setTimeout(()=>Je(e,t,i),800)}}function Zt(e){const t=["MIL","ATT","GK","DEF"].some(a=>(e.homeTeam[a]||[]).some(c=>!c.used)),i=["MIL","ATT","GK","DEF"].some(a=>(e.aiTeam[a]||[]).some(c=>!c.used));return!t&&!i}function Qt(e,t,i){Zt(t)?ve(e,t,i):(t.phase="attack",oe(e,t,i))}function to(e,t,i){const a=document.createElement("div");a.style.cssText=`
    position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:800;
    display:flex;flex-direction:column;align-items:center;justify-content:center;gap:20px;
    animation:subFadeIn 0.2s ease;
  `;const c=ne(e),n=ne(t),s=_e[e.job]||"#555",l=_e[t.job]||"#555",d=e.job==="GK"?e.note_g:e.job==="DEF"?e.note_d:e.job==="MIL"?e.note_m:e.note_a,o=t.job==="GK"?t.note_g:t.job==="DEF"?t.note_d:t.job==="MIL"?t.note_m:t.note_a;a.innerHTML=`
    <style>
  ...
  `,document.body.appendChild(a);let r=!1;const u=()=>{r||(r=!0,a.remove(),i())};a.addEventListener("click",u),setTimeout(u,2e3)}function ke(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function Ct(e,t,i,a=null){if(t.phase!=="attack"){ke("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){ke(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const c=Object.values(t.homeTeam).flat().filter(u=>u.used),n=t.homeSubs.filter(u=>!t.usedSubIds.includes(u.cardId));if(!c.length){ke("Aucun joueur utilisé à remplacer");return}if(!n.length){ke("Aucun remplaçant disponible");return}let s=0,l=Math.max(0,n.findIndex(u=>u.cardId===a)),d=!1;const o=document.createElement("div");o.id="sub-overlay",o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function r(){var T,v,m,w,B,C;const u=c[s],g=n[l],f=Math.min(130,Math.round((window.innerWidth-90)/2)),j=Math.round(f*1.35),p=x=>`background:rgba(255,255,255,0.12);border:none;color:${x?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${x?"default":"pointer"};flex-shrink:0`;o.innerHTML=`
    ...
    <div style="flex:1;display:flex;gap:0;overflow:hidden">
      ...
      <div id="in-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px;border-right:1px solid rgba(255,255,255,0.08)">
        ...
        <button id="in-down" style="${p(l>=n.length-1)}" ${l>=n.length-1?"disabled":""}>▼</button>
    ...
    </div>`,(T=o.querySelector("#sub-close"))==null||T.addEventListener("click",()=>o.remove()),(v=o.querySelector("#out-up"))==null||v.addEventListener("click",()=>{s>0&&(s--,r())}),(m=o.querySelector("#out-down"))==null||m.addEventListener("click",()=>{s<c.length-1&&(s++,r())}),(w=o.querySelector("#in-up"))==null||w.addEventListener("click",()=>{l>0&&(l--,r())}),(B=o.querySelector("#in-down"))==null||B.addEventListener("click",()=>{l<n.length-1&&(l++,r())});const h=(x,M,R,O)=>{const K=o.querySelector("#"+x);if(!K)return;let I=0;K.addEventListener("touchstart",H=>{I=H.touches[0].clientY},{passive:!0}),K.addEventListener("touchend",H=>{const N=H.changedTouches[0].clientY-I;if(Math.abs(N)<30)return;const V=M();N<0&&V<O-1?(R(V+1),r()):N>0&&V>0&&(R(V-1),r())},{passive:!0})};h("in-panel",()=>l,x=>l=x,n.length),h("out-panel",()=>s,x=>s=x,c.length),(C=o.querySelector("#sub-confirm"))==null||C.addEventListener("click",x=>{if(x.preventDefault(),x.stopPropagation(),d)return;d=!0;const M=c[s],R=n[l];if(!M||!R)return;let O=null,K=-1;for(const[H,N]of Object.entries(t.homeTeam)){const V=(N||[]).findIndex(z=>z.cardId===M.cardId);if(V!==-1){O=H,K=V;break}}if(K===-1||!O){ke("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),o.remove();return}const I={...R,_line:O,_col:M._col||0,used:!1,boost:0};t.homeTeam[O].splice(K,1,I),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(R.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:M.name,firstname:M.firstname,note:ie(M,O),portrait:ne(M),job:M.job,country_code:M.country_code,rarity:M.rarity,clubName:M.clubName,clubLogo:M.clubLogo},inPlayer:{name:R.name,firstname:R.firstname,note:ie(R,O),portrait:ne(R),job:R.job,country_code:R.country_code,rarity:R.rarity,clubName:R.clubName,clubLogo:R.clubLogo},text:`🔄 ${R.firstname} ${R.name} remplace ${M.firstname} ${M.name}`}),o.remove(),to(M,R,()=>oe(e,t,i))})}document.body.appendChild(o),r()}function io(e,t,i,a,c){var j,p;const n=(a.gcDefs||[]).find(h=>h.name===t),s=le[t]||{icon:"⚡",desc:"Carte spéciale."},l={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[n==null?void 0:n.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",d={purple:"#b06ce0",light_blue:"#00d4ef"}[n==null?void 0:n.color]||"#b06ce0",o=(n==null?void 0:n.name)||t,r=(n==null?void 0:n.effect)||s.desc,u=n!=null&&n.image_url?`/manager-wars/icons/${n.image_url}`:null,g=s.icon||"⚡",f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px",f.innerHTML=`
    ...
    </div>`,document.body.appendChild(f),(j=f.querySelector("#gc-back"))==null||j.addEventListener("click",()=>f.remove()),(p=f.querySelector("#gc-use"))==null||p.addEventListener("click",()=>{f.remove(),oo(e,t,i,a,c)})}function oo(e,t,i,a,c){if(a.usedGc.includes(e))return;a.usedGc.push(e);const n=(a.gcDefs||[]).find(l=>l.name===t);let s=!1;if(n!=null&&n.effect_type&&n.effect_type!=="CUSTOM"){const l=GC_ENGINE[n.effect_type];l?l(n.effect_params||{},a,i,c)||(s=!0):(c.toast(`Effet "${n.effect_type}" non implémenté`,"error"),s=!0)}else{switch(t){case"Double attaque":a.modifiers.home.doubleAttack=!0,a.log.push({text:"⚡ Double attaque activée !",type:"info"});break;case"Bouclier":a.modifiers.home.shield=!0,a.log.push({text:"🛡️ Bouclier activé !",type:"info"});break;case"Ressusciter":{const l=Object.entries(a.homeTeam).flatMap(([d,o])=>(o||[]).filter(r=>r.used).map(r=>({...r,_line:d})));l.length?(l[0].used=!1,a.log.push({text:`💫 ${l[0].name} ressuscité !`,type:"info"})):a.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"});break}case"Vol de note":a.modifiers.ai.stolenNote=(a.modifiers.ai.stolenNote||0)+1,a.log.push({text:"🎯 -1 à la prochaine attaque IA",type:"info"});break;case"Gel":{const l=[...a.aiTeam.ATT||[],...a.aiTeam.MIL||[]].filter(d=>!d.used);if(l.length){const d=l.sort((o,r)=>ie(r,"ATT")-ie(o,"ATT"))[0];d.used=!0,a.log.push({text:`❄️ ${d.name} (IA) gelé !`,type:"info"})}break}case"Remplacement+":a.maxSubs++,a.log.push({text:"🔄 +1 remplacement débloqué",type:"info"});break}s=!0}$.from("cards").delete().eq("id",e).then(()=>{}),s&&oe(i,a,c)}function no(e,t,i){const a=Object.values(t.homeTeam).flat().filter(c=>!c.used);if(!a.length){i.toast("Aucun joueur actif à booster","error");return}i.openModal("⚡ Utiliser le Boost",`<div style="margin-bottom:12px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border-radius:10px;padding:12px;text-align:center;color:#000">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(c=>{c.addEventListener("click",()=>{const n=c.dataset.cardId;for(const s of["GK","DEF","MIL","ATT"]){const l=(t.homeTeam[s]||[]).find(d=>d.cardId===n);if(l){l.boost=(l.boost||0)+t.boostCard.value,t.log.push({text:`⚡ Boost +${t.boostCard.value} appliqué à ${l.name}`,type:"info"});break}}t.boostUsed=!0,i.closeModal(),oe(e,t,i)})})}function st(e,t,i,a,c){const n=document.createElement("div");n.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:900;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;overflow:hidden;cursor:pointer";const s=Array.from({length:10},(o,r)=>`
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(n);let l=!1;const d=()=>{l||(l=!0,n.remove(),setTimeout(()=>c(),50))};n.addEventListener("click",d),setTimeout(d,3500)}async function ve(e,t,i){var r,u;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:a}=i,c=t.homeScore>t.aiScore,n=t.homeScore===t.aiScore,s=c?"victoire":n?"nul":"defaite",l=pi(t.mode,s);t.matchId&&await $.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:c?a.profile.id:null,home_credits_reward:l,played_at:new Date().toISOString()}).eq("id",t.matchId);const d={credits:(a.profile.credits||0)+l,matches_played:(a.profile.matches_played||0)+1};c?d.wins=(a.profile.wins||0)+1:n?d.draws=(a.profile.draws||0)+1:d.losses=(a.profile.losses||0)+1,await $.from("users").update(d).eq("id",a.profile.id),await i.refreshProfile();const o=document.createElement("div");o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",o.innerHTML=`
    ...
    </div>`,`<button class="btn btn-primary" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}async function ao(e,t){const{state:i}=t;await Jt(e,t,"random",({deckId:c,formation:n,starters:s,subsRaw:l,gcCardsEnriched:d,gcDefs:o})=>{const r=u=>{so(e,t,c,n,s,l,u,o||[])};if(!d.length){r([]);return}Yt(e,d,r)})}async function so(e,t,i,a,c,n,s=[],l=[]){var v;const{state:d,navigate:o,toast:r}=t;let u=!1,g=null;e.style.overflow="hidden",e.innerHTML=`
    ...
    <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:18px;padding:24px;background:#0a3d1e;text-align:center">
      ...
      card:cards(id, card_type,
        player:players(id, firstname, surname_encoded, country_code, job, job2,
          note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length, clubs(encoded_name, logo_url)))`).eq("seller_id",i.profile.id).in("status",["active","sold"]).order("listed_at",{ascending:!1}).limit(100),s=(c||[]).filter(o=>o.seller_id!==i.profile.id),l=n||[];l.filter(o=>o.status==="active").length,e.innerHTML=`
  <div class="page">
    <div class="page-header">
  ...
  `;function d(o){const r=document.getElementById("mkt-content"),u=o==="buy"?s:l;if(u.length===0){r.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
    ...
    </div>`,r.querySelectorAll("[data-buy]").forEach(f=>{f.addEventListener("click",()=>uo(f.dataset.buy,u,e,t))}),r.querySelectorAll("[data-cancel]").forEach(f=>{f.addEventListener("click",()=>fo(f.dataset.cancel,e,t))})}d("buy"),e.querySelectorAll(".mkt-tab").forEach(o=>{o.addEventListener("click",()=>{e.querySelectorAll(".mkt-tab").forEach(r=>{const u=r===o;r.style.background=u?"var(--green)":"#fff",r.style.color=u?"#fff":"var(--gray-600)",r.style.borderColor=u?"var(--green)":"var(--gray-200)"}),d(o.dataset.tab)})})}async function uo(e,t,i,a){var u;const{state:c,toast:n}=a,s=t.find(g=>g.id===e);if(!s)return;const l=s.price,d=c.profile.credits||0,o=(u=s.card)==null?void 0:u.player;if(d<l){n("Crédits insuffisants","error");return}if(!confirm(`Acheter ${o==null?void 0:o.firstname} ${o==null?void 0:o.surname_encoded} pour ${l.toLocaleString("fr")} crédits ?`))return;const r=document.querySelector(`[data-buy="${e}"]`);r&&(r.disabled=!0,r.textContent="⏳");try{const{data:g,error:f}=await $.rpc("buy_market_card",{p_listing_id:e,p_buyer_id:c.profile.id});if(f)throw new Error(f.message);if(!(g!=null&&g.success))throw new Error((g==null?void 0:g.error)||"Achat impossible");c.profile.credits=d-l;const j=document.querySelector("[data-credits]")||document.querySelector(".credits-display");j&&(j.textContent=(d-l).toLocaleString("fr")+" cr."),n(`✅ ${o==null?void 0:o.surname_encoded} ajouté à ta collection !`,"success"),lt(i,a)}catch(g){n("❌ "+g.message,"error"),r&&(r.disabled=!1,r.textContent="Acheter")}}async function fo(e,t,i){const{toast:a}=i,{data:c}=await $.from("market_listings").select("card_id").eq("id",e).single();await $.from("market_listings").update({status:"cancelled"}).eq("id",e),c&&await $.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",c.card_id),a("Annonce retirée","success"),lt(t,i)}function go(e){return{GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[e]||"#888"}async function mo(e,{state:t,navigate:i,toast:a}){e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:c}=await $.from("users").select("id,pseudo,club_name,trophies_top1,trophies_top2,trophies_top3,wins,level").order("trophies_top1",{ascending:!1}).limit(100);e.innerHTML=`
  <div class="page">
    <div class="page-header">
