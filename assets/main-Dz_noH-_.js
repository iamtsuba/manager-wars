...
import{s as w,l as Gt,m as Ii,F as Si,r as Se,j as Jt,h as Ai,n as jn,o as Cn,T as gi,k as Bn,p as qn,b as Dn}from"./formation-links-yJiMLlPw.js";const Fn="/";function Ni(e,{navigate:t,toast:i}){let n="login";const r=()=>{var a,s,l,c,d,u;const o=n==="login";e.innerHTML=`
    <div style="
  ...
  `;function s(){var g;const c=document.getElementById("logo-preview"),d=document.getElementById("logo-initials"),u=((g=document.getElementById("setup-club"))==null?void 0:g.value)||"MW",x=u.trim().split(" ").filter(Boolean),m=x.length>=2?(x[0][0]+x[1][0]).toUpperCase():u.slice(0,2).toUpperCase();c&&(c.style.background=a,c.style.borderColor=o),d&&(d.textContent=m,d.style.color=o)}document.getElementById("color1").addEventListener("input",c=>{o=c.target.value,document.getElementById("swatch1").style.background=o,s()}),document.getElementById("color2").addEventListener("input",c=>{a=c.target.value,document.getElementById("swatch2").style.background=a,s()});function l(c){document.querySelectorAll(".setup-step").forEach(d=>d.classList.remove("active")),document.getElementById(`step-${c}`).classList.add("active"),document.getElementById("step-num").textContent=c,document.getElementById("progress-fill").style.width=`${Math.round(c/3*100)}%`,c===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),d=document.getElementById("step1-error");if(d.textContent="",c.length<3){d.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:u}=await w.from("users").select("id").eq("pseudo",c).maybeSingle();if(u){d.textContent="Ce pseudo est déjà pris.";return}l(2)}),document.getElementById("step2-back").addEventListener("click",()=>l(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const c=document.getElementById("setup-club").value.trim(),d=document.getElementById("step2-error");if(d.textContent="",c.length<2){d.textContent="Nom trop court (min. 2 caractères).";return}const{data:u}=await w.from("users").select("id").eq("club_name",c).maybeSingle();if(u){d.textContent="Ce nom de club est déjà pris.";return}l(3)}),document.getElementById("step3-back").addEventListener("click",()=>l(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const c=document.getElementById("setup-pseudo").value.trim(),d=document.getElementById("setup-club").value.trim(),u=document.getElementById("step3-error"),x=document.getElementById("step3-finish");u.textContent="",x.disabled=!0,x.textContent="Création en cours…";try{const{error:m}=await w.from("users").insert({id:t.user.id,pseudo:c,club_name:d,club_color1:o,club_color2:a,credits:1e4});if(m)throw m;await Gn(t.user.id),await r(),n(`Bienvenue ${c} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(m){u.textContent=m.message,x.disabled=!1,x.textContent="🚀 Créer mon profil !"}})}async function Gn(e){const t=[{type:"player",count:5,guaranteeGK:!0},{type:"player",count:5},{type:"player",count:5},{type:"player",count:5},{type:"game_changer",count:3},{type:"formation",count:1}];try{await w.from("users").update({pending_boosters:t,onboarding_done:!1,first_booster_opened:!1}).eq("id",e)}catch(i){console.warn("[Setup] Colonnes pending_boosters/onboarding_done absentes — migration requise",i)}}const Rn="modulepreload",Nn=function(e){return"/"+e},Oi={},rn=function(t,i,n){let r=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),s=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));r=Promise.allSettled(i.map(l=>{if(l=Nn(l),l in Oi)return;Oi[l]=!0;const c=l.endsWith(".css"),d=c?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${l}"]${d}`))return;const u=document.createElement("link");if(u.rel=c?"stylesheet":Rn,c||(u.as="script"),u.crossOrigin="",u.href=l,s&&u.setAttribute("nonce",s),document.head.appendChild(u),c)return new Promise((x,m)=>{u.addEventListener("load",x),u.addEventListener("error",()=>m(new Error(`Unable to preload CSS for ${l}`)))})}))}function o(a){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=a,window.dispatchEvent(s),!s.defaultPrevented)throw a}return r.then(a=>{for(const s of a||[])s.status==="rejected"&&o(s.reason);return t().catch(o)})},Ut="#1A6B3C",Kt="#cc2222",On="#D4A017",Hi="#888";async function Hn(e,t){const{state:i,toast:n}=t;e.innerHTML=`
    ...
    </div>`}function Kn(e,t){const i=Mi();i.innerHTML=`
    <div class="popup-box">
    ...
    </style>`}const Yn="2026.07.19-0019";async function Ui(e,{state:t,navigate:i,toast:n}){var o,a;const r=t.profile;r&&(e.innerHTML=`
  <style>
    .home-dark {
      min-height: 100%;
  ...
  <div class="home-dark" id="home-dark">
    ...
    ${Se(e,{width:140})}
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),c=(s||[]).filter(A=>A.card_type==="player"&&A.player),d=(s||[]).filter(A=>A.card_type==="game_changer"),u=(s||[]).filter(A=>A.card_type==="formation"),x=(s||[]).filter(A=>A.card_type==="stadium"),{data:m}=await w.from("gc_definitions").select("name,gc_type,color,effect,image_url"),g={};(m||[]).forEach(A=>{g[A.name]=A});const{data:p}=await w.from("stadium_definitions").select("id,name,club_id,country_code,image_url, club:clubs(encoded_name,logo_url)"),f={};(p||[]).forEach(A=>{f[A.id]=A}),x.forEach(A=>{A.stadium_def&&(f[A.stadium_id]=A.stadium_def)});const h=Object.keys(Si),_=Object.keys(Me),b={};c.forEach(A=>{const C=A.player.id;b[C]=(b[C]||0)+1}),new Set(Object.keys(b).map(A=>String(A)));const $=new Set(u.map(A=>A.formation)),y=new Set(d.map(A=>A.gc_type));let j="player",E="Tous",P="",N=!1;function ne(){return[...c].sort((A,C)=>{const F=Zt.indexOf(A.player.job),X=Zt.indexOf(C.player.job);return F!==X?F-X:(A.player.surname_real||"").localeCompare(C.player.surname_real||"")})}function ie(){return[...l||[]].sort((A,C)=>{const F=Zt.indexOf(A.job),X=Zt.indexOf(C.job);return F!==X?F-X:(A.surname_real||"").localeCompare(C.surname_real||"")})}function Q(){return ne().filter(A=>{const C=A.player,F=E==="Tous"||C.job===E,X=!P||`${C.firstname} ${C.surname_real}`.toLowerCase().includes(P);return F&&X})}function K(){return ie().filter(A=>{const C=E==="Tous"||A.job===E,F=!P||`${A.firstname} ${A.surname_real}`.toLowerCase().includes(P);return C&&F})}e.innerHTML=`
  <div class="page" style="display:flex;flex-direction:column;height:100%;overflow:hidden">
    ...
    <div style="flex-shrink:0;padding:0">
      <div id="col-grid" style="display:flex;overflow-x:auto;gap:8px;padding:8px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none"></div>
  ...
  </div>`;function v(){const A=document.getElementById("col-filters");A&&(j==="player"?(A.innerHTML=`
        ...
        </div>`,document.getElementById("col-search").addEventListener("input",C=>{P=C.target.value.toLowerCase(),I()}),e.querySelectorAll(".filter-btn").forEach(C=>{C.addEventListener("click",()=>{E=C.dataset.job,v(),I()})}),document.getElementById("show-all-btn").addEventListener("click",()=>{N=!N,v(),I()})):(A.innerHTML=`
        ...
        </div>`},null)}e.querySelectorAll(".col-tab-btn").forEach(A=>{A.addEventListener("click",()=>{j=A.dataset.tab,E="Tous",P="",N=!1,e.querySelectorAll(".col-tab-btn").forEach(C=>{const F=C.dataset.tab===j;C.style.borderBottomColor=F?"var(--green)":"transparent",C.style.color=F?"var(--green)":"var(--gray-600)"}),v(),I()})}),v(),I()}function uo(e,t,i){const n=Me[e]||{icon:"⚡",desc:"Effet spécial."},r=(t==null?void 0:t.gc_type)==="ultra_game_changer",o={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},a={purple:"#b06ce0",light_blue:"#00d4ef"},s=o[t==null?void 0:t.color]||o.purple,l=a[t==null?void 0:t.color]||a.purple,c=(t==null?void 0:t.name)||e,d=(t==null?void 0:t.effect)||n.desc,u=t!=null&&t.image_url?`/icons/${t.image_url}`:null;i("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    <div style="margin-top:16px;border-top:1px solid var(--gray-200);padding-top:14px">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${u.map((W,J)=>{const R=Q[W.id]||[],A=W.is_for_sale,C=R.length?R[R.length-1]:null,X=(d.rarity==="pepite"||d.rarity==="papyte")&&W.current_note!=null?` (☆${W.current_note})`:"";return`
            <div data-card-id="${W.id}" data-card-idx="${J}" class="exemplaire-row"
              ...
              <div style="display:flex;align-items:center;gap:10px">
                <input type="checkbox" class="expl-check" data-id="${W.id}" ${A?"disabled":""} style="width:18px;height:18px;cursor:pointer;accent-color:#1A6B3C;flex-shrink:0">
                <div style="flex:1;min-width:0">
                  <div style="font-size:12px;font-weight:700">Exemplaire ${J+1}${X}${A?" 🏷️ En vente":""}</div>
      ...
      <div style="flex:1;min-width:160px;display:flex;flex-direction:column;gap:10px">
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ${[["GK",d.note_g],["DEF",d.note_d],["MIL",d.note_m],["ATT",d.note_a]].map(([W,J])=>{const R=mi[W],A=W==="GK"?"#fff":R,C=W===d.job||W===d.job2,F=(Number(J)||0)+(C&&j>0?j:0);return`<div style="display:flex;flex-direction:column;align-items:center;gap:4px">
                <div style="width:48px;height:48px;border-radius:8px;background:#111;border:2.5px solid ${R};
                  display:flex;align-items:center;justify-content:center">
                  <span style="font-size:20px;font-weight:900;color:${A};font-family:Arial Black,Arial;line-height:1">${F}</span>
                </div>
                <span style="font-size:10px;font-weight:700;color:${A}">${W}</span>
              </div>`}).join("")}
          </div>
        </div>
        <div>
...
`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(ae=document.getElementById("close-detail"))==null||ae.addEventListener("click",s);let I=new Set;const G=()=>{const W=I.size,J=document.getElementById("sell-action-panel");if(!J)return;J.style.display=W>0?"block":"none",document.getElementById("sell-selected-count").textContent=W;const R=document.getElementById("evolve-btn");R&&(R.textContent=`⬆️ Évoluer ${W>1?"(+"+W+")":""}`)};document.querySelectorAll(".expl-check").forEach(W=>{W.addEventListener("change",()=>{const J=W.dataset.id;W.checked?I.add(J):I.delete(J);const R=W.closest(".exemplaire-row");R&&(R.style.borderColor=W.checked?"#1A6B3C":"#eee"),G()})}),document.querySelectorAll(".exemplaire-row").forEach(W=>{W.addEventListener("click",J=>{if(J.target.closest("button")||J.target.tagName==="INPUT")return;const R=W.querySelector(".expl-check");R&&!R.disabled&&(R.checked=!R.checked,R.dispatchEvent(new Event("change")))})}),document.querySelectorAll(".expl-hist-toggle").forEach(W=>{W.addEventListener("click",J=>{J.stopPropagation();const R=document.querySelector(`.expl-hist[data-hist="${W.dataset.idx}"]`);R&&(R.style.display=R.style.display==="none"?"flex":"none")})}),(se=document.getElementById("evolve-btn"))==null||se.addEventListener("click",async()=>{if(x<=1)return;const W=[...I];if(!W.length)return;if(I.has(e.id)){const V=document.createElement("div");V.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px",V.innerHTML=`
  ...
  <div style="height:100%;overflow:hidden;background:var(--page-bg)">
  ...
  </div>`;const d=window.innerWidth>=900,u=e.querySelector(".deck-pc-layout"),x=e.querySelector(".deck-mobile-layout");u&&(u.style.display=d?"block":"none"),x&&(x.style.display=d?"none":"block"),mo(e,t,r,i),e.querySelectorAll("#builder-back").forEach(f=>f.addEventListener("click",()=>n("decks")));const m=()=>{const{openModal:f,closeModal:h}=i,_=`<div style="display:flex;flex-wrap:wrap;gap:8px;padding:8px">
    ...
    </div>`;f("⚽ Choisir une formation",_),document.querySelectorAll("#modal-body [data-forma]").forEach(b=>{b.addEventListener("click",()=>{t.formation=b.dataset.forma;const $=Zi(t.formation),y={};$.forEach(j=>{t.slots[j]&&(y[j]=t.slots[j])}),t.slots=y,h(),xt(e,t,i)})})};e.querySelectorAll("#formation-mobile-btn, #formation-pc-btn").forEach(f=>f.addEventListener("click",m)),e.querySelectorAll("#add-stad-btn-pc, #add-stad-btn").forEach(f=>f.addEventListener("click",()=>xo(t,e,i))),e.querySelectorAll("#save-deck").forEach(f=>f.addEventListener("click",()=>ho(t,i))),e.querySelectorAll("[data-remove-sub]").forEach(f=>{f.addEventListener("click",()=>{t.subs=t.subs.filter(h=>h!==f.dataset.removeSub),xt(e,t,i)})}),e.querySelectorAll("#add-sub-btn").forEach(f=>f.addEventListener("click",()=>bo(t,e,i)))}function mo(e,t,i,n){var b,$;const r=window.innerWidth>=900,o=e.querySelector(r?"#deck-field-pc":"#deck-field-mobile");if(!o)return;const a=(b=t.stadiumCards)==null?void 0:b.find(y=>y.id===t.stadiumCardId),s=a&&(($=t.stadDefMap)==null?void 0:$[a.stadium_id])||null,l=Jt[t.formation]||{},c=Ii(t.formation)||[],d={};for(const y of i){const j=t.slots[y],E=j?t.playerCards.find(P=>P.id===j):null;E!=null&&E.player?d[y]={...E.player,_evolution_bonus:E.evolution_bonus||0,face:E.player.face||null}:d[y]=null}const u=window.innerWidth>=900,x=u?window.innerWidth-280:window.innerWidth-20,m=u?Math.min(x,860):x,g=Math.round(u?m*.82:m*.85),p=u?84:44;let f="";for(const[y,j]of c){const E=l[y],P=l[j];if(!E||!P)continue;const N=E.x*m,ne=Math.round(.03*g+E.y*.85*g),ie=P.x*m,Q=Math.round(.03*g+P.y*.85*g),K=d[y],v=d[j],I=Gt(K,v);I==="#ff3333"||I==="#cc2222"?f+=`<line x1="${N.toFixed(1)}" y1="${ne.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${Q.toFixed(1)}" stroke="${I}" stroke-width="2" stroke-linecap="round" opacity="0.35"/>`:(f+=`<line x1="${N.toFixed(1)}" y1="${ne.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${Q.toFixed(1)}" stroke="${I}" stroke-width="8" stroke-linecap="round" opacity="0.15"/>`,f+=`<line x1="${N.toFixed(1)}" y1="${ne.toFixed(1)}" x2="${ie.toFixed(1)}" y2="${Q.toFixed(1)}" stroke="${I}" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/>`)}let h="";const _=Math.round(p*657/507);for(const y of i){const j=l[y];if(!j)continue;const E=d[y],P=j.x*m;j.y*g;const N=Math.round(.03*g+j.y*(.85*g)),ne=Math.round(P-p/2),ie=Math.round(N-_/2);if(E){const Q=y.replace(/\d+/,""),K=s&&(s.club_id&&String(E.club_id)===String(s.club_id)||s.country_code&&E.country_code===s.country_code),v=Se({...E,_evolution_bonus:E._evolution_bonus||0},{width:p,showStad:!0,stadDef:s,role:Q});h+=`<div style="position:absolute;left:${ne}px;top:${ie}px;cursor:pointer;z-index:2;position:absolute" class="deck-slot-hit" data-pos="${y}">
    ...
    <div style="position:relative;width:${m}px;height:${g}px;margin:0 auto">
      ...
      ${o.map(s=>{var h,_;const l=e.stadDefMap[s.stadium_id],c=((h=l==null?void 0:l.club)==null?void 0:h.logo_url)||null,d=l!=null&&l.image_url?"/icons/"+l.image_url:null,u=e.stadiumCardId===s.id,x=l!=null&&l.country_code&&!c?`https://flagsapi.com/${l.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,m=(l==null?void 0:l.name)||"Stade",g=m.match(/^(Stade\s+(?:de\s+)?(?:\w+)?)(.*)?$/i),p=g?g[1].trim():m.slice(0,8),f=((_=g==null?void 0:g[2])==null?void 0:_.trim())||"";return`<div class="stad-choice" data-stad-id="${s.id}" style="cursor:pointer;width:80px;text-align:center">
  ...
  <div class="page">
    ...
    <div class="page-body">
      <div class="booster-grid">
        ${o.map(g=>{const p=g.cost===0||r>=g.cost,f=d[g.id]===!0;return`<div class="booster-card ${!p||f?"disabled":""}" data-booster="${g.id}" style="position:relative">
  ...
  `,document.body.appendChild(r);let o=!1;const a=document.getElementById("pack-cut-zone"),s=document.getElementById("pack-blade");let l=!1;const c=v=>v.touches&&v.touches[0]?v.touches[0].clientX:v.clientX;function d(v){o||(l=!0,s.style.opacity="1",u(v))}function u(v){if(!l||o)return;const I=a.getBoundingClientRect(),G=c(v)-I.left,re=Math.max(0,Math.min(1,G/I.width));s.style.width=re*I.width+"px",re>=.82&&m()}function x(){o||(l=!1,s.style.transition="width .2s ease, opacity .2s ease",s.style.width="0",s.style.opacity="0",setTimeout(()=>{o||(s.style.transition="")},220))}function m(){var I;if(o)return;o=!0,l=!1,s.style.width="100%",s.style.opacity="1",(I=document.getElementById("cut-flash"))==null||I.classList.add("cut-flash-go"),navigator.vibrate&&navigator.vibrate([30,20,50]);const v=document.getElementById("cut-hint");v&&(v.style.opacity="0"),a.classList.add("pack-cut"),setTimeout(()=>{s.style.opacity="0",document.getElementById("pack-phase").style.display="none",h(0)},620)}a.addEventListener("pointerdown",d),window.addEventListener("pointermove",u),window.addEventListener("pointerup",x),a.addEventListener("touchstart",d,{passive:!0}),window.addEventListener("touchmove",u,{passive:!0}),window.addEventListener("touchend",x);let g=0,p=!1;const f=new Set;function h(v){g=v,document.getElementById("reveal-phase").style.display="flex",_(),b(v,0),P()}function _(){const v=document.getElementById("card-dots");v&&(v.innerHTML=e.map((I,G)=>`<div class="card-dot" data-i="${G}" style="width:8px;height:8px;border-radius:50%;background:${G===g?"#FFD700":"rgba(255,255,255,0.3)"};transition:background .2s;cursor:pointer"></div>`).join(""),v.querySelectorAll(".card-dot").forEach(I=>I.addEventListener("click",()=>y(parseInt(I.dataset.i)))))}function b(v,I){var R,A;const G=e[v],re=document.getElementById("card-counter"),ae=document.getElementById("card-track");re&&(re.textContent=`Carte ${v+1} / ${e.length}`);const se=document.getElementById("reveal-btns");se&&(se.style.display=v===e.length-1?"flex":"none");const de=G.card_type==="player"&&((R=G.player)==null?void 0:R.rarity)==="legende",ke=!f.has(v);f.add(v);let W=0;if(G.card_type==="player"&&G.player){const C=G.player,F=C.job||"ATT";W=Number(F==="GK"?C.note_g:F==="DEF"?C.note_d:F==="MIL"?C.note_m:C.note_a)||0}const J=C=>{ae.innerHTML=`
        ...
        ${J}
      </div>`,A&&navigator.vibrate&&navigator.vibrate([50,30,100,30,200])},(q+O)*3),setTimeout(V,(q+O)*3+q),setTimeout(()=>{C.style.display="none",F.innerHTML="",p=!1,X&&!R&&X.pause(),navigator.vibrate&&navigator.vibrate([40,30,80]),I()},(q+O)*4)}function y(v){if(p||v<0||v>=e.length||v===g)return;const I=v>g?1:-1;g=v,b(v,I)}function j(){y(g+1)}function E(){y(g-1)}function P(){const v=document.getElementById("card-viewport");if(!v||v._swipeBound)return;v._swipeBound=!0;let I=0,G=0,re=0,ae=!1;const se=R=>R.touches?R.touches[0].clientX:R.clientX,de=R=>R.touches?R.touches[0].clientY:R.clientY,ke=R=>{ae=!0,I=se(R),G=de(R),re=0},W=R=>{if(!ae)return;re=se(R)-I;const A=de(R)-G;if(Math.abs(re)<Math.abs(A))return;const C=document.getElementById("current-card-wrap");C&&(C.style.transition="none",C.style.transform=`translateX(${re*.6}px) rotate(${re*.02}deg)`)},J=()=>{if(!ae)return;ae=!1;const R=document.getElementById("current-card-wrap"),A=55;re<=-A&&g<e.length-1?j():re>=A&&g>0?E():R&&(R.style.transition="transform .2s ease",R.style.transform="translateX(0)")};v.addEventListener("pointerdown",ke),v.addEventListener("pointermove",W),v.addEventListener("pointerup",J),v.addEventListener("pointercancel",J),v.addEventListener("touchstart",ke,{passive:!0}),v.addEventListener("touchmove",W,{passive:!0}),v.addEventListener("touchend",J),v.addEventListener("click",R=>{if(Math.abs(re)>8)return;const A=v.getBoundingClientRect();R.clientX-A.left>A.width/2?j():E()})}let N=null;function ne(){const v=document.getElementById("fireworks-canvas");if(!v)return;v.width=window.innerWidth,v.height=window.innerHeight;const I=v.getContext("2d"),G=[];function re(){const se=Math.random()*v.width,de=Math.random()*v.height*.6,ke=["#7a28b8","#ff4081","#D4A017","#00e676","#fff","#e040fb","#40c4ff"],W=ke[Math.floor(Math.random()*ke.length)];for(let J=0;J<60;J++){const R=Math.PI*2/60*J,A=2+Math.random()*5;G.push({x:se,y:de,vx:Math.cos(R)*A,vy:Math.sin(R)*A,alpha:1,color:W,size:2+Math.random()*3})}}re(),N=setInterval(re,600);function ae(){if(document.getElementById("fireworks-canvas")){I.clearRect(0,0,v.width,v.height);for(let se=G.length-1;se>=0;se--){const de=G[se];if(de.x+=de.vx,de.y+=de.vy+.08,de.vy*=.98,de.alpha-=.018,de.alpha<=0){G.splice(se,1);continue}I.globalAlpha=de.alpha,I.fillStyle=de.color,I.beginPath(),I.arc(de.x,de.y,de.size,0,Math.PI*2),I.fill()}I.globalAlpha=1,(N!==null||G.length>0)&&requestAnimationFrame(ae)}}ae()}function ie(){N!==null&&(clearInterval(N),N=null);const v=document.getElementById("fireworks-canvas");v&&v.getContext("2d").clearRect(0,0,v.width,v.height)}if(n){const v=document.getElementById("reveal-btns");v&&(v.innerHTML='<button class="btn btn-primary" id="reveal-next" style="flex:1">Continuer →</button>'),(K=document.getElementById("reveal-next"))==null||K.addEventListener("click",()=>{ie(),r.remove(),n()})}else document.getElementById("reveal-collection").addEventListener("click",()=>{ie(),r.remove(),i("collection")}),document.getElementById("reveal-more").addEventListener("click",()=>{ie(),r.remove(),i("boosters")})}function zo(e){var t,i,n,r;if(e.card_type==="player"&&e.player)return ko(e);if(e.card_type==="game_changer"){const o=e._gcDef,a=(o==null?void 0:o.gc_type)==="ultra_game_changer",s={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"},l={purple:"#b06ce0",light_blue:"#00d4ef"},c=s[o==null?void 0:o.color]||s.purple,d=l[o==null?void 0:o.color]||l.purple,u=(o==null?void 0:o.name)||e.gc_type||"Game Changer",x=(o==null?void 0:o.effect)||((t=wi[e.gc_type])==null?void 0:t.desc)||"",m=o!=null&&o.image_url?`/icons/${o.image_url}`:null,g=((i=wi[e.gc_type])==null?void 0:i.icon)||"⚡";return`<div style="width:170px;height:240px;background:${c};border-radius:14px;border:3px solid ${d};display:flex;flex-direction:column;overflow:hidden;box-shadow:0 0 24px ${d}88;flex-shrink:0">
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}function Ao(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText=`
    ...
    `,document.body.appendChild(t);let i=5;const n=setInterval(()=>{i--;const r=document.getElementById("mw-ad-cd");r&&(r.textContent=i),i<=0&&(clearInterval(n),t.remove(),e(!0))},1e3)})}async function Mo(e,{state:t,navigate:i,toast:n,refreshProfile:r}){var x,m;const{data:o}=await w.from("users").select("*").eq("id",t.user.id).single();o&&(t.profile=o);let a=Array.isArray((x=t.profile)==null?void 0:x.pending_boosters)?[...t.profile.pending_boosters]:[];if(!a.length){await w.from("users").update({onboarding_done:!0}).eq("id",t.user.id),i("home");return}let s=null;try{const p=(await gn()).find(f=>(f.name||"").toLowerCase().includes("new player"));p&&(s=xn(p))}catch(g){console.warn('[Onboarding] Config "Booster (new player)" introuvable, fallback taux par défaut',g)}const l=a.length;let c=0;e.innerHTML=`
  <div class="page" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(160deg,#0a3d1e,#063015);padding:24px">
    ...
    <div id="gc-screen-wrap" style="position:relative;display:flex;flex-direction:column;height:100%;overflow:hidden;background:linear-gradient(180deg,#0a1628,#1a0a2e)">
      ...
      card:cards(id,card_type,formation,stadium_id,evolution_bonus,
        player:players(id,firstname,surname_real,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,face,
          clubs(encoded_name,logo_url)))`).in("deck_id",a).order("slot_order"),l=[...new Set((s||[]).filter(x=>{var m,g;return((m=x.card)==null?void 0:m.card_type)==="stadium"&&((g=x.card)==null?void 0:g.stadium_id)}).map(x=>x.card.stadium_id))],c={};if(l.length){const{data:x}=await w.from("stadium_definitions").select("id,name,club_id,country_code,image_url,club:clubs(encoded_name,logo_url)").in("id",l);(x||[]).forEach(m=>{c[m.id]=m}),(s||[]).forEach(m=>{var g,p;((g=m.card)==null?void 0:g.card_type)==="stadium"&&((p=m.card)!=null&&p.stadium_id)&&(m.card._stadiumDef=c[m.card.stadium_id]||null)})}let d=0;function u(){var y,j,E,P,N,ne,ie;const x=o[d],m=(s||[]).filter(Q=>Q.deck_id===x.id),g=m.filter(Q=>{var K;return Q.is_starter&&((K=Q.card)==null?void 0:K.player)}).map(Q=>ki(Q.card,Q.position)),p=m.find(Q=>{var K;return((K=Q.card)==null?void 0:K.card_type)==="formation"}),f=x.formation||((y=p==null?void 0:p.card)==null?void 0:y.formation)||"4-4-2";let h=g.length>=11?ri(g,f):null,_=((j=x.stadium_card)==null?void 0:j.stadium_def)||null;_&&h&&(h=Xt(h,_));const b=g.length>=11;nt(e),e.style.height="100%",e.style.overflow="hidden",e.innerHTML=`
  ...
  <div class="match-screen" style="position:fixed;top:0;left:0;right:0;bottom:auto;z-index:100;display:flex;flex-direction:column;overflow:hidden;background:#0a3d1e;width:100%">
    ...
    ${(()=>{const v=window.innerWidth>=700,I=(R,A,C)=>{var ge,Be;const F=(t.gcDefs||[]).find(je=>je.name===R.gc_type),X={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[F==null?void 0:F.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",V={purple:"#b06ce0",light_blue:"#00d4ef"}[F==null?void 0:F.color]||"#b06ce0",q=(F==null?void 0:F.name)||R.gc_type,O=(F==null?void 0:F.effect)||((ge=Me[R.gc_type])==null?void 0:ge.desc)||"",we=F!=null&&F.image_url?`/icons/${F.image_url}`:null,_e=((Be=Me[R.gc_type])==null?void 0:Be.icon)||"⚡",ye=Math.round(C*.22),be=Math.round(C*.22),$e=C-ye-be,Le=q.length>12?7:9;return`<div class="gc-mini" data-gc-id="${R.id}" data-gc-type="${R.gc_type}"
          style="box-sizing:border-box;width:${A}px;height:${C}px;border-radius:10px;border:2px solid ${V};background:${X};display:flex;flex-direction:column;overflow:hidden;cursor:pointer;flex-shrink:0">
          <div style="height:${ye}px;background:rgba(255,255,255,0.14);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 3px">
            <span style="font-size:${Le}px;font-weight:900;color:#fff;text-align:center;line-height:1.1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:${A-6}px">${q}</span>
          ...
          <div style="height:${$e}px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.06)">
            ${we?`<img src="${we}" style="max-width:${A-10}px;max-height:${$e-6}px;object-fit:contain">`:`<span style="font-size:${Math.round($e*.55)}px">${_e}</span>`}
          </div>
        ...
        </div>`},G=(R,A)=>{var C;return`<div id="boost-card"
  ...
  <div id="match-history-panel">
    ...
    </div>`,document.body.appendChild(m),(g=m.querySelector("#gc-back"))==null||g.addEventListener("click",()=>m.remove()),(p=m.querySelector("#gc-use"))==null||p.addEventListener("click",()=>{m.remove(),Qo(e,t,i,n,r)})}function ti(e,t,i,n,r,o){const a=document.createElement("div");a.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let s=[];function l(){var c,d;a.innerHTML=`
    ...
    <div style="text-align:center;padding:40px;color:#fff;max-width:360px">
      ...
      <div style="font-size:18px">${y[p+"Name"]} ${L} – ${z} ${y[f+"Name"]}</div>
      ${k.forfeit?`<div style="font-size:13px;color:rgba(255,255,255,0.5)">${S?"L'adversaire a quitté":"Perdu par forfait"}</div>`:""}
      ${M}
      <button id="pvp-end-home" style="margin-top:8px;padding:14px 32px;border-radius:12px;border:none;background:#1A6B3C;color:#fff;font-size:16px;font-weight:900;cursor:pointer">${a?"⚔️ Retour au Ranked":"Retour à l'accueil"}</button>`,document.body.appendChild(D),(Z=D.querySelector("#pvp-end-home"))==null||Z.addEventListener("click",()=>{D.remove(),We(e),m(a?"ranked":"home")})}const Q=w.channel("pvp-match-"+i).on("postgres_changes",{event:"UPDATE",schema:"public",table:"matches",filter:`id=eq.${i}`},k=>{const L=k.new;try{if(L.status==="finished"||L.forfeit){if(j)return;j=!0,E&&(clearInterval(E),E=null),L.game_state&&(y=L.game_state),ie(L);return}if(L.game_state){const z=y;y=L.game_state;const S=y._lastGC;if(S&&S.seq&&!ne.has(S.seq)&&(ne.add(S.seq),S.by!==p)){R(S.type,S.by,()=>de());return}const T=z[p+"Score"]||0,M=z[f+"Score"]||0,D=y[p+"Score"]||0,B=y[f+"Score"]||0,U=D>T,Y=B>M;if((U||Y)&&!N.has(y.round)){N.add(y.round);const oe=[...y.log||[]].reverse().find(H=>H.isGoal),Z=((oe==null?void 0:oe.homePlayers)||[]).map(H=>({name:H.name,note:H.note,portrait:H.portrait,job:H.job}));ot(Z,D,B,U,()=>de());return}de()}}catch(z){console.error("[PvP] crash:",z)}}).subscribe();async function K(k){Object.assign(y,k),y.lastActionAt=new Date().toISOString();const{error:L}=await w.from("matches").update({game_state:y}).eq("id",i);L&&g("Erreur de synchronisation","error");try{de()}catch(z){console.error("[PvP] renderPvpScreen crash:",z)}}async function v(){if(j)return;j=!0,E&&(clearInterval(E),E=null);const k=n?b.away_id:b.home_id,L=n?"p2":"p1",z=n?"p1":"p2",S={...y,[L+"Score"]:3,[z+"Score"]:0,phase:"finished"},T=S.p1Score||0,M=S.p2Score||0;try{await w.from("matches").update({game_state:S}).eq("id",i);const{data:D,error:B}=await w.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:k,p_home_score:T,p_away_score:M,p_forfeit:!0});B&&console.error("[PvP] finish_pvp_match (forfeit):",B);try{const{data:U}=await w.rpc("apply_match_rewards",{p_match_id:i});U!=null&&U.success&&!(U!=null&&U.skipped)&&typeof t.refreshProfile=="function"&&await t.refreshProfile()}catch(U){console.error("[PvP] apply_match_rewards (forfeit):",U)}}catch{}try{w.removeChannel(Q)}catch{}setTimeout(()=>{We(e),d?m("mini-league",{openLeagueId:d}):m("home")},800)}const I={BOOST_STAT:({value:k=1,count:L=1,roles:z=[]},S,T)=>{const M=S[p+"Team"],D=Object.entries(M).filter(([B])=>!z.length||z.includes(B)).flatMap(([B,U])=>U.filter(Y=>!Y.used).map(Y=>({...Y,_line:B})));if(!D.length){S.log.push({text:"⚡ Aucun joueur disponible",type:"info"}),T(S);return}G(D,L,`Choisir ${L} joueur(s) à booster (+${k})`,B=>{B.forEach(U=>{const Y=(M[U._line]||[]).find(oe=>oe.cardId===U.cardId);Y&&(Y.boost=(Y.boost||0)+k,S.log.push({text:`⚡ +${k} sur ${Y.name}`,type:"info"}))}),S[p+"Team"]=M,T(S)})},DEBUFF_STAT:({value:k=1,count:L=1,roles:z=[],target:S="ai"},T,M)=>{const D=S==="home"?p:f,B=T[D+"Team"],U=S==="home"?"allié":"adverse",Y=Object.entries(B).filter(([oe])=>!z.length||z.includes(oe)).flatMap(([oe,Z])=>Z.map(H=>({...H,_line:oe})));if(!Y.length){T.log.push({text:`🎯 Aucun joueur ${U}`,type:"info"}),M(T);return}G(Y,L,`Choisir ${L} joueur(s) ${U}(s) (-${k})`,oe=>{oe.forEach(Z=>{const H=(B[Z._line]||[]).find(le=>le.cardId===Z.cardId);H&&(H.boost=(H.boost||0)-k,T.log.push({text:`🎯 -${k} sur ${H.name}`,type:"info"}))}),T[D+"Team"]=B,M(T)})},GRAY_PLAYER:({count:k=1,roles:L=[],target:z="ai"},S,T)=>{const M=z==="home"?p:f,D=S[M+"Team"],B=z==="home"?"allié":"adverse",U=Object.entries(D).filter(([Y])=>!L.length||L.includes(Y)).flatMap(([Y,oe])=>oe.filter(Z=>!Z.used).map(Z=>({...Z,_line:Y})));if(!U.length){S.log.push({text:`❌ Aucun joueur ${B}`,type:"info"}),T(S);return}G(U,k,`Choisir ${k} joueur(s) ${B}(s) à exclure`,Y=>{const oe="usedCardIds_"+M,Z=new Set(S[oe]||[]);Y.forEach(H=>{const le=(D[H._line]||[]).find(pe=>pe.cardId===H.cardId);le&&(le.used=!0,Z.add(H.cardId),S.log.push({text:`❌ ${le.name} exclu !`,type:"info"}))}),S[oe]=[...Z],S[M+"Team"]=D,T(S)})},REVIVE_PLAYER:({count:k=1,roles:L=[]},z,S)=>{const T=z[p+"Team"],M=Object.entries(T).filter(([D])=>!L.length||L.includes(D)).flatMap(([D,B])=>B.filter(U=>U.used).map(U=>({...U,_line:D})));if(!M.length){z.log.push({text:"💫 Aucun joueur à ressusciter",type:"info"}),S(z);return}G(M,k,`Choisir ${k} joueur(s) à ressusciter`,D=>{D.forEach(B=>{const U=(T[B._line]||[]).find(Y=>Y.cardId===B.cardId);U&&(U.used=!1,z.log.push({text:`💫 ${U.name} ressuscité !`,type:"info"}))}),z[p+"Team"]=T,S(z)})},REMOVE_GOAL:({},k,L)=>{const z=f+"Score";k[z]>0?(k[z]--,k.log.push({text:"🚫 Dernier but annulé !",type:"info"})):k.log.push({text:"🚫 Aucun but à annuler",type:"info"}),L(k)},ADD_GOAL_DRAW:({},k,L)=>{k[p+"Score"]===k[f+"Score"]?(k[p+"Score"]++,k.log.push({text:"🎯 But bonus !",type:"info"})):k.log.push({text:"🎯 Non applicable (pas de nul)",type:"info"}),L(k)},ADD_SUB:({value:k=1},L,z)=>{L.maxSubs=(L.maxSubs||3)+k,L.log.push({text:`🔄 +${k} remplacement(s)`,type:"info"}),z(L)},CUSTOM:({},k,L)=>L(k)};function G(k,L,z,S){const T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:900;display:flex;flex-direction:column;overflow:hidden";let M=[];function D(){var U,Y;const B=k.map(oe=>{const Z=oe._line||oe.job||"MIL",H=M.find(pe=>pe.cardId===oe.cardId),le=Se({...oe,_evolution_bonus:0},{width:90,showStad:!0,role:Z,extraNote:oe.boost||0});return`<div class="pp-item" data-cid="${oe.cardId}"
        ...
        </div>`,(U=T.querySelector("#pp-close"))==null||U.addEventListener("click",()=>T.remove()),T.querySelectorAll(".pp-item").forEach(oe=>{oe.addEventListener("click",()=>{const Z=oe.dataset.cid,H=k.find(pe=>pe.cardId===Z),le=M.findIndex(pe=>pe.cardId===Z);H&&(le>-1?M.splice(le,1):M.length<L&&M.push(H),D())})}),(Y=T.querySelector("#pp-confirm"))==null||Y.addEventListener("click",()=>{T.remove(),S(M)})}D(),document.body.appendChild(T)}async function re(k,L){const S=(y["gcCardsFull_"+p]||[]).find(B=>B.id===k),T=(S==null?void 0:S._gcDef)||(y.gcDefs||[]).find(B=>{var U;return B.name===L||((U=B.name)==null?void 0:U.toLowerCase().trim())===(L==null?void 0:L.toLowerCase().trim())}),M=[...y["usedGc_"+p]||[],k],D={type:L,by:p,seq:(y._gcAnimSeq||0)+1};ne.add(D.seq),R(L,p,async()=>{if(T!=null&&T.effect_type&&T.effect_type!=="CUSTOM"){const U=I[T.effect_type];if(U){const Y={...y};U(T.effect_params||{},Y,async oe=>{oe["usedGc_"+p]=M,oe._lastGC=D,oe._gcAnimSeq=D.seq,await K(oe)});return}}const B={...y};switch(L){case"Remplacement+":B.maxSubs=(B.maxSubs||3)+1,B.log.push({text:"🔄 +1 remplacement",type:"info"});break;case"VAR":{const U=f+"Score";B[U]>0&&(B[U]--,B.log.push({text:"🚫 But annulé",type:"info"}));break}}B["usedGc_"+p]=M,B._lastGC=D,B._gcAnimSeq=D.seq,await K(B)})}function ae(k,L){const z="usedCardIds_"+k,S=new Set(y[z]||[]);L.forEach(T=>S.add(T)),y[z]=[...S]}function se(){for(const k of["p1","p2"]){const L=new Set(y["usedCardIds_"+k]||[]),z=y[k+"Team"];if(z)for(const S of["GK","DEF","MIL","ATT"])(z[S]||[]).forEach(T=>{T.used=L.has(T.cardId)})}}function de(){var at,st,vt,Pi,Gi,Ri;if(y.phase==="reveal")return ke();if(y.phase==="midfield")return J();if(y.phase==="finished")return Rt();const k=y[p+"Team"];y[f+"Team"],se();const L=y[p+"Score"],z=y[f+"Score"],S=y[p+"Name"],T=y[f+"Name"],M=y.phase===p+"-attack",D=y.phase===p+"-defense",B=Array.isArray(y["selected_"+p])?y["selected_"+p]:[],U=B.map(ee=>ee.cardId),Y=window.innerWidth>=700,oe=y[p+"Subs"]||[],Z=y["usedSubIds_"+p]||[],H=oe.filter(ee=>!Z.includes(ee.cardId)),le=y["gcCardsFull_"+p]||[],pe=y["usedGc_"+p]||[],ue=le.filter(ee=>!pe.includes(ee.id)),ce=y.boostOwner===p&&!y.boostUsed,me=[...k.MIL||[],...k.ATT||[]].filter(ee=>!ee.used),xe=M&&me.length===0?[...k.GK||[],...k.DEF||[]].filter(ee=>!ee.used).map(ee=>ee.cardId):[];function te(ee,fe,ze){var Mt,lt;const he=ee._gcDef,St={purple:"linear-gradient(160deg,#4a0a8a,#7a28b8)",light_blue:"linear-gradient(160deg,#006080,#00bcd4)"}[he==null?void 0:he.color]||"linear-gradient(160deg,#4a0a8a,#7a28b8)",et={purple:"#b06ce0",light_blue:"#00d4ef"}[he==null?void 0:he.color]||"#b06ce0",De=(he==null?void 0:he.name)||ee.gc_type,Ne=(he==null?void 0:he.effect)||((Mt=Me[ee.gc_type])==null?void 0:Mt.desc)||"",Ae=he!=null&&he.image_url?`/icons/${he.image_url}`:null,wt=((lt=Me[ee.gc_type])==null?void 0:lt.icon)||"⚡",Pe=Math.round(ze*.22),dt=Math.round(ze*.22),tt=ze-Pe-dt,At=De.length>12?7:9;return`<div class="pvp-gc-mini" data-gc-id="${ee.id}" data-gc-type="${ee.gc_type}"
      ...
      </div>`}const Fe=(ee,fe)=>fe?Te(130,175):te(ee,130,175),Ie=(ee,fe)=>fe?Te(68,95):te(ee,68,95),He=Y?"padding:28px 20px;border-radius:14px;font-size:16px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;width:100%":"padding:22px 8px;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;width:100%",It=M?je(p)?`<button id="pvp-action" style="${He};background:linear-gradient(135deg,#c47a00,#FFD700);border:none;color:#fff;box-shadow:0 0 18px rgba(255,215,0,0.4)" ${B.length===0?"disabled":""}>⚔️ ATTAQUEZ <span id="pvp-timer"></span></button>`:`<button id="pvp-action" data-pass="1" style="${He};background:linear-gradient(135deg,#555,#888);border:none;color:#fff">⏭️ PASSER <span id="pvp-timer"></span></button>`:D?`<button id="pvp-action" style="${He};background:linear-gradient(135deg,#1a4a8a,#3a7bd5);border:none;color:#fff;box-shadow:0 0 18px rgba(135,206,235,0.4)" ${B.length===0?"disabled":""}>🛡️ DÉFENDEZ <span id="pvp-timer"></span></button>`:`<div style="font-size:11px;color:rgba(255,255,255,0.3);text-align:center;padding:4px">⏳ Tour de ${T}</div>`,Je=M&&!je(p)?'<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">Aucun attaquant — passez la main</div>':M||D?`<div style="font-size:9px;color:rgba(255,255,255,0.4);text-align:center;margin-top:2px">${B.length}/3 sélectionné(s)</div>`:"",Qt=`<div style="display:flex;flex-direction:column;gap:4px;padding:4px 2px;width:${Y?90:50}px;align-items:center;overflow-y:auto;flex-shrink:0;background:rgba(0,0,0,0.15)">
    ...
    </div>`,qe=M?"attack":D?"defense":"idle",Qe=`<div style="overflow:hidden;min-width:0;flex:1;min-height:0;display:flex;flex-direction:column" id="match-field">
      <div class="terrain-wrapper" style="overflow:hidden;width:100%;flex:1;min-height:0;display:flex;align-items:center;justify-content:center">
        ${Pt(k,y[p+"Formation"],qe,U,Y?1300:Lt(),Y?600:qt(),xe)}
      </div>
    </div>`,Re=y[p+"Team"],ht=(()=>{var fe,ze,he,St,et;if(D&&((fe=y.pendingAttack)!=null&&fe.players)){const De=y.pendingAttack;let Ne="";if(B.length>0){const Ae=B.map(Pe=>{const dt=(Re[Pe._role]||[]).find(fi=>fi.cardId===Pe.cardId)||Pe,tt=Re[Pe._role]||[],At=tt.findIndex(fi=>fi.cardId===Pe.cardId),Mt=pt(tt.length),lt=At>=0?Mt[At]:dt._col??1;return{...dt,_line:Pe._role,_col:lt}}),wt=Yt(Ae,((ze=y.modifiers)==null?void 0:ze[p])||{},y[p+"Formation"]);Ne=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15)">
          ...
          <div style="display:flex;justify-content:center">${Ye((De.players||[]).map(Ae=>({...Ae,used:!1})),"#ff6b6b",De.total,"attack",y[f+"Formation"])}</div>
          ${Ne}
    ...
    <div class="match-screen" style="position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;overflow:hidden;gap:14px;padding:16px;background:#0a3d1e;overflow-y:auto">
      ...
      <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
        ${L.map(T=>{const M={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[T._line]||"#555",D=Ge(T,T._line)+(T.boost||0);return`<div class="bp-item" data-cid="${T.cardId}" style="width:80px;border-radius:8px;border:2px solid rgba(255,255,255,0.25);background:${M};overflow:hidden;cursor:pointer">
      ...
      </div>`,document.body.appendChild(z),(S=z.querySelector("#bp-close"))==null||S.addEventListener("click",()=>z.remove()),z.querySelectorAll(".bp-item").forEach(T=>{T.addEventListener("click",async()=>{const M=T.dataset.cid,D=L.find(U=>U.cardId===M);if(!D)return;const B=(k[D._line]||[]).find(U=>U.cardId===M);B&&(B.boost=(B.boost||0)+y.boostValue),z.remove(),await K({[p+"Team"]:k,boostUsed:!0})})})}function F(k=null){var le,pe;if(!(y.phase===p+"-attack")){g("Remplacement uniquement avant votre attaque","warning");return}const z=y[p+"Team"],S=y["usedSubIds_"+p]||[],T=y.maxSubs||3;if(S.length>=T){g(`Maximum ${T} remplacements atteint`,"warning");return}const M=Object.entries(z).flatMap(([ue,ce])=>(ce||[]).filter(me=>me.used).map(me=>({...me,_line:ue}))),D=(y[p+"Subs"]||[]).filter(ue=>!S.includes(ue.cardId));if(!M.length){g("Aucun joueur utilisé à remplacer","warning");return}if(!D.length){g("Aucun remplaçant disponible","warning");return}let B=Math.max(0,M.findIndex(ue=>ue.cardId===k));const U=((le=M[B])==null?void 0:le._line)||((pe=M[B])==null?void 0:pe.job);let Y=Math.max(0,D.findIndex(ue=>ue.job===U)),oe=!1;const Z=document.createElement("div");Z.id="pvp-sub-overlay",Z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function H(){var Fe,Ie,He,It,Je,Qt;const ue=M[B],ce=D[Y],me=Math.min(130,Math.round((window.innerWidth-90)/2)),xe=Math.round(me*1.35),te=qe=>`background:rgba(255,255,255,0.12);border:none;color:${qe?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${qe?"default":"pointer"};flex-shrink:0`;Z.innerHTML=`
      <div style="display:flex;align-items:center;padding:12px 16px;background:rgba(0,0,0,0.5);flex-shrink:0">
      ...
      <div style="flex:1;display:flex;gap:0;overflow:hidden">
        ...
        <div id="pout-panel" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:12px 6px">
          ...
          <button id="pout-down" style="${te(B>=M.length-1)}" ${B>=M.length-1?"disabled":""}>▼</button>
          <div style="font-size:10px;color:rgba(255,255,255,0.35)">${B+1}/${M.length}</div>
        </div>
      </div>
      <div style="padding:12px 16px;background:rgba(0,0,0,0.4);flex-shrink:0">
        <button id="psub-confirm" style="width:100%;padding:14px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:900;cursor:pointer">✅ Confirmer</button>
      </div>`,(Fe=Z.querySelector("#psub-close"))==null||Fe.addEventListener("click",()=>Z.remove()),(Ie=Z.querySelector("#pout-up"))==null||Ie.addEventListener("click",()=>{B>0&&(B--,H())}),(He=Z.querySelector("#pout-down"))==null||He.addEventListener("click",()=>{B<M.length-1&&(B++,H())}),(It=Z.querySelector("#pin-up"))==null||It.addEventListener("click",()=>{Y>0&&(Y--,H())}),(Je=Z.querySelector("#pin-down"))==null||Je.addEventListener("click",()=>{Y<D.length-1&&(Y++,H())});const Te=(qe,Qe,Re,ht)=>{const Ze=Z.querySelector("#"+qe);if(!Ze)return;let Ue=0;Ze.addEventListener("touchstart",at=>{Ue=at.touches[0].clientY},{passive:!0}),Ze.addEventListener("touchend",at=>{const st=at.changedTouches[0].clientY-Ue;if(Math.abs(st)<30)return;const vt=Qe();st<0&&vt<ht-1?(Re(vt+1),H()):st>0&&vt>0&&(Re(vt-1),H())},{passive:!0})};Te("pin-panel",()=>Y,qe=>Y=qe,D.length),Te("pout-panel",()=>B,qe=>B=qe,M.length),(Qt=Z.querySelector("#psub-confirm"))==null||Qt.addEventListener("click",async qe=>{if(qe.preventDefault(),qe.stopPropagation(),oe)return;oe=!0;const Qe=M[B],Re=D[Y];if(!Qe||!Re)return;const ht=Qe._line,Ze=(z[ht]||[]).findIndex(st=>st.cardId===Qe.cardId);if(Ze===-1){g("Erreur : joueur introuvable","error"),Z.remove();return}const Ue={...Re,_line:ht,position:Qe.position,used:!1,boost:0};z[ht].splice(Ze,1,Ue);const at=[...S,Re.cardId];Z.remove(),X(Qe,Re,async()=>{await K({[p+"Team"]:z,[f+"Team"]:y[f+"Team"],["usedSubIds_"+p]:at})})})}document.body.appendChild(Z),H()}function X(k,L,z){const S={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},T=document.createElement("div");T.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:850;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;overflow:hidden;cursor:pointer";const M=(U,Y,oe)=>`<div style="text-align:center">
      ...
      </div>`,document.body.appendChild(L),(z=L.querySelector("#pvp-hist-close"))==null||z.addEventListener("click",()=>L.remove())}async function O(){if(y.phase!==p+"-attack")return;const k=p==="p1"?"p2":"p1",L=(y.round||0)+1,z=[...y.log||[]];z.push({type:"info",text:`⏭️ ${y[p+"Name"]||"Vous"} passe (aucun attaquant disponible)`});const S=Oe(y),T=je(k),M=S||!T?"finished":k+"-attack";await K({["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:M,attacker:k,round:L,log:z}),M==="finished"&&await Xe(y)}function we(k){const L=(k.GK||[]).some(S=>!S.used),z=["DEF","MIL","ATT"].some(S=>(k[S]||[]).some(T=>!T.used));return L&&!z}function _e(k){return(k.p1Score||0)!==(k.p2Score||0)?null:we(k.p1Team)&&!ge(k.p2Team)?{side:"p1",gk:(k.p1Team.GK||[]).find(L=>!L.used)}:we(k.p2Team)&&!ge(k.p1Team)?{side:"p2",gk:(k.p2Team.GK||[]).find(L=>!L.used)}:null}async function ye(k,L){const{side:z,gk:S}=k;if(!S)return;const T=z+"Score",M=(L[T]||0)+1;S.used=!0;const D=[...L.log||[]];D.push({type:"duel",isGoal:!0,homeScored:z===p,homePlayers:z==="p1"?[jt(S)]:[],aiPlayers:z==="p2"?[jt(S)]:[],text:`⚽ DERNIER CORNER — Le gardien de ${L[z+"Name"]} marque !`});const B=(L.round||0)+1,U=z==="p1"?"p2":"p1",Y={...L,[T]:M},oe=Oe(Y);N.add(B);const Z=document.createElement("div");Z.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:950;display:flex;align-items:center;justify-content:center;overflow:hidden;text-align:center;padding:24px",document.body.appendChild(Z);const H=(ue,ce)=>new Promise(me=>{Z.innerHTML=`<div style="font-size:32px;font-weight:900;color:${ce};letter-spacing:2px;animation:lcFade 1.4s ease both">${ue}</div>
      <style>@keyframes lcFade{0%{opacity:0;transform:scale(0.8)}18%{opacity:1;transform:scale(1)}82%{opacity:1}100%{opacity:0;transform:scale(1.05)}}</style>`,setTimeout(me,1400)});await H("⚽ DERNIER CORNER","#FFD700"),await H("🧤 LE GARDIEN MONTE !","#4fc3f7"),Z.remove();const le=z===p?M:L[p+"Score"]||0,pe=z===p?L[f+"Score"]||0:M;ot([jt(S)],le,pe,z===p,async()=>{await K({[T]:M,log:D,round:B,pendingAttack:null,phase:oe?"finished":U+"-attack",attacker:U,["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}}}),oe&&await Xe({...y,[T]:M})})}async function be(){const k=y[p+"Team"],L=!["GK","DEF","MIL","ATT"].some(M=>(y[f+"Team"][M]||[]).some(D=>!D.used)),z=(y["selected_"+p]||[]).map(M=>{const D=(k[M._role]||[]).find(pe=>pe.cardId===M.cardId)||M,B=["GK","DEF"].includes(M._role),U=k[M._role]||[],Y=U.findIndex(pe=>pe.cardId===M.cardId),oe=pt(U.length),Z=Y>=0?oe[Y]:D._col??1,H=y.stadiumDef||y.homeStadiumDef||null,le=D.stadiumBonus||H&&(H.club_id&&String(D.club_id)===String(H.club_id)||H.country_code&&D.country_code===H.country_code)||!1;return{...D,_line:M._role,_col:Z,stadiumBonus:le,...B?{note_a:Math.max(1,Number(D.note_a)||0)}:{}}});if(!z.length)return;const S=Vt(z,y.modifiers[p]||{},y[p+"Formation"]);ae(p,z.map(M=>M.cardId)),z.forEach(M=>{const D=(k[M._role]||[]).find(B=>B.cardId===M.cardId);D&&(D.used=!0)}),y["selected_"+p]=[],de();const T=[...y.log||[]];if(L){if(z.length===1&&(z[0]._line==="GK"||z[0].job==="GK")&&(y.p1Score||0)===(y.p2Score||0)){await ye({side:p,gk:z[0]},{...y,[p+"Team"]:k,log:T});return}const D=(y[p+"Score"]||0)+1,B=z.map(H=>jt(H));T.push({type:"duel",isGoal:!0,homeScored:!0,text:"⚽ BUT ! L'adversaire n'a plus de joueurs.",homePlayers:B,homeTotal:S.total,aiTotal:0});const U=(y.round||0)+1,Y=p==="p1"?"p2":"p1",oe={...y,[p+"Team"]:k,[p+"Score"]:D},Z=Oe(oe);N.add(U),ot(B,D,y[f+"Score"]||0,!0,async()=>{await K({[p+"Team"]:k,[p+"Score"]:D,["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:Z?"finished":Y+"-attack",attacker:Y,round:U,log:T}),Z&&await Xe({...y,[p+"Score"]:D})});return}T.push({type:"pending",text:`⚔️ ${y[p+"Name"]} attaque (${S.total})`}),await K({[p+"Team"]:k,[f+"Team"]:y[f+"Team"],pendingAttack:{...S,players:z,side:p},["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},phase:f+"-defense",log:T})}async function $e(){const k=y[p+"Team"],L=(y["selected_"+p]||[]).map(ce=>{const me=(k[ce._role]||[]).find(Ie=>Ie.cardId===ce.cardId)||ce,xe=k[ce._role]||[],te=xe.findIndex(Ie=>Ie.cardId===ce.cardId),Te=pt(xe.length),Fe=te>=0?Te[te]:me._col??1;return{...me,_line:ce._role,_col:Fe}}),z=Yt(L,y.modifiers[p]||{},y[p+"Formation"]);ae(p,L.map(ce=>ce.cardId)),L.forEach(ce=>{const me=(k[ce._role]||[]).find(xe=>xe.cardId===ce.cardId);me&&(me.used=!0)}),y["selected_"+p]=[],de();const S=Ci(y.pendingAttack.total,z.total,y.modifiers[p]||{}),T=y.pendingAttack.side,M=S==="attack"||(S==null?void 0:S.goal),D=T==="p1"?"p2":"p1",B=(y.round||0)+1,U=(y.pendingAttack.players||[]).map(ce=>jt(ce)),Y=[...y.log||[]];Y.push({type:"duel",isGoal:M,homeScored:M&&T===p,text:M?`⚽ BUT de ${y[T+"Name"]} ! (${y.pendingAttack.total} vs ${z.total})`:`✋ Attaque stoppée (${y.pendingAttack.total} vs ${z.total})`,homePlayers:U,aiPlayers:L.map(ce=>jt(ce)),homeTotal:y.pendingAttack.total,aiTotal:z.total});const oe=M?(y[T+"Score"]||0)+1:y[T+"Score"]||0,Z={...y,[p+"Team"]:k,[T+"Score"]:oe},H=Oe(Z),le=H?"finished":D+"-attack",pe=!H&&!M?_e(Z):null,ue=async()=>{if(pe){await ye(pe,Z);return}await K({[p+"Team"]:k,[f+"Team"]:y[f+"Team"],[T+"Score"]:oe,["selected_"+p]:[],modifiers:{...y.modifiers,[p]:{}},pendingAttack:null,phase:le,attacker:D,round:B,log:Y}),(le==="finished"||H)&&await Xe({...y,[T+"Score"]:oe})};if(M){const ce=T===p,me=ce?oe:y[p+"Score"]||0,xe=ce?y[f+"Score"]||0:oe;N.add(B),ot(U,me,xe,ce,ue)}else await ue()}function Le(k){return["MIL","ATT"].some(L=>(k[L]||[]).some(z=>!z.used))}function ge(k){return["GK","DEF","MIL","ATT"].some(L=>(k[L]||[]).some(z=>!z.used))}function Be(k){return ge(k)&&!Le(k)}function je(k){const L=y[k+"Team"];return!!(Le(L)||Be(L))}function Oe(k){return!ge(k.p1Team)&&!ge(k.p2Team)}function bt(k){const L=k.p1Score||0,z=k.p2Score||0;return L===z?null:L>z?b.home_id:b.away_id}async function Xe(k){try{const L=bt(k),z=L?b.home_id===L?b.away_id:b.home_id:null,S=k.p1Score||0,T=k.p2Score||0,{data:M,error:D}=await w.rpc("finish_pvp_match",{p_match_id:i,p_winner_id:L,p_home_score:S,p_away_score:T,p_forfeit:!1});D&&console.error("[PvP] finish_pvp_match:",D),L&&z&&io(L,z).catch(()=>{});try{const{data:B}=await w.rpc("apply_match_rewards",{p_match_id:i});B!=null&&B.success&&!(B!=null&&B.skipped)&&typeof t.refreshProfile=="function"&&await t.refreshProfile()}catch(B){console.error("[PvP] apply_match_rewards:",B)}if(typeof c=="function")try{await c({homeScore:S,awayScore:T,winnerId:L,homeId:b.home_id,awayId:b.away_id,matchId:i})}catch(B){console.error("[PvP] onMatchEnd:",B)}}catch(L){console.error("[PvP] finishMatch:",L)}}function Rt(){var T;const k=y[p+"Score"],L=y[f+"Score"],z=k>L,S=k===L;nt(e),e.innerHTML=`
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="display:flex;background:var(--nav-bg,#0d1a0f);border-bottom:1px solid ${$t}">
      ${f.map(b=>`<button class="ml-tab" data-tab="${b.key}" style="flex:1;padding:11px 4px;border:none;border-bottom:2px solid ${i===b.key?Tn:"transparent"};background:none;font-size:12px;font-weight:${i===b.key?"900":"600"};color:${i===b.key?"#4ade80":Ke};cursor:pointer">${b.label}${b.count?` (${b.count})`:""}</button>`).join("")}
    </div>
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:10px">
      ${i==="waiting"?cr(x,p,r):i==="active"?pr(m,r):ur(g,r)}
    </div>
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    <div style="padding:14px 16px;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:${Nt};border:1px solid ${kt};border-radius:12px;padding:16px">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>${x.map((v,I)=>{const G=g>0&&a.status==="finished"?I===0?Math.floor(g*.7):I===1?Math.floor(g*.2):I===2?Math.floor(g*.1):0:0;return`<tr style="border-bottom:1px solid ${$t};${v.userId===o?"background:rgba(74,222,128,0.08);":""}">
    ...
    </div>`;let d=!1;const u=async f=>{if(!d){if(d=!0,clearInterval(g),m)try{m.unsubscribe()}catch{}f&&await zt(e,t,f,r,{mlLeagueId:n,mlMatchId:i,myGC:o||[],gcDefs:a,stadiumDef:s}),l()}};(p=document.getElementById("wait-back"))==null||p.addEventListener("click",()=>{if(d=!0,clearInterval(g),m)try{m.unsubscribe()}catch{}c("mini-league",{openLeagueId:n}),l()});const x=async()=>{if(d)return;const{data:f}=await w.from("mini_league_matches").select("match_id").eq("id",i).single();f!=null&&f.match_id&&u(f.match_id)},m=w.channel(`ml_match_${i}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"mini_league_matches",filter:`id=eq.${i}`},f=>{var h;(h=f.new)!=null&&h.match_id&&u(f.new.match_id)}).subscribe(),g=setInterval(x,3e3);x()})}const _r="/",kr=[{emoji:"⚽",title:"Bienvenue dans Manager Wars !",color:"#1A6B3C",content:`<p>Tu es désormais un <strong>manager de football</strong> virtuel.</p>
  ...
  <div style="height:100%;overflow-y:auto;background:var(--page-bg)">
    ...
    </div>`}function x(p){var P,N,ne,ie;const f=(P=p.card)==null?void 0:P.player;if(!f)return"";const h=((N=p.card)==null?void 0:N.evolution_bonus)||0,_=Ct(f,f.job,h),b=f.job2?Ct(f,f.job2,h):0,$=p.status==="sold",y=f.country_code?`https://flagsapi.com/${f.country_code.slice(0,2).toUpperCase()}/flat/64.png`:null,j=ii[f.job]||"#bbb",E=f.job2?ii[f.job2]||"#bbb":null;return`<div class="card-panel" style="display:flex;align-items:center;gap:10px;padding:10px 12px;overflow:hidden;${$?"opacity:0.7":""}">
      ${y?`<img src="${y}" style="width:32px;height:24px;object-fit:cover;border-radius:3px;flex-shrink:0">`:'<span style="font-size:20px">🌍</span>'}
      ${(ne=f.clubs)!=null&&ne.logo_url?`<img src="${f.clubs.logo_url}" style="width:28px;height:28px;object-fit:contain;flex-shrink:0">`:""}
      <div style="display:flex;gap:4px;flex-shrink:0">
        <div style="width:36px;height:36px;border-radius:6px;background:#111;border:2px solid ${j};display:flex;align-items:center;justify-content:center">
    ...
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:320px;width:100%;text-align:center">
      ...
      `).join(""):'<div style="text-align:center;color:var(--gray-600);padding:40px">Aucun manager.</div>'}else{const h=a||[];g.innerHTML=h.length>0?h.map((_,b)=>{const $=Ai(_.mmr??1e3),y=(_.ranked_wins||0)+(_.ranked_losses||0)+(_.ranked_draws||0),j=y>0?Math.round((_.ranked_wins||0)/y*100):0,E=_.id===i.profile.id,P=(_.placement_matches||0)<10;return`
  ...
  </div>`}Bn(Dn);const ve={user:null,profile:null,page:"home",params:{}};function Bt(e,t="info",i=3e3){const n=document.getElementById("toast");n&&(n.textContent=e,n.className=`show ${t}`,clearTimeout(n._t),n._t=setTimeout(()=>{n.className=""},i))}function jr(e,t,i=""){document.getElementById("modal-title").textContent=e,document.getElementById("modal-body").innerHTML=t,document.getElementById("modal-footer").innerHTML=i,document.getElementById("modal-overlay").classList.remove("hidden")}function zi(){document.getElementById("modal-overlay").classList.add("hidden")}async function Ot(){if(!ve.user)return;const{data:e}=await w.from("users").select("*").eq("id",ve.user.id).single();e&&(ve.profile=e)}function Ht(e,t={}){ve.page=e,ve.params=t,An()}async function An(){var n,r,o,a;const e=document.getElementById("page-content");if(!e)return;document.querySelectorAll(".bottom-nav a").forEach(s=>{s.classList.toggle("active",s.dataset.page===ve.page)});const t=document.getElementById("nav-credits");t&&ve.profile&&(t.textContent=`💰 ${(ve.profile.credits||0).toLocaleString("fr")}`);const i={state:ve,navigate:Ht,toast:Bt,openModal:jr,closeModal:zi,refreshProfile:Ot};switch(e.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽</div>',ve.page){case"home":await Ui(e,i);break;case"collection":await po(e,i);break;case"decks":await vi(e,i);break;case"boosters":await $o(e,i);break;case"ranked":await Ar(e,i);break;case"match":{const s=ve.params&&ve.params.matchMode||"vs_ai_easy";s==="random"?await Li(e,i,!1):s==="ranked"?await Li(e,i,!0):s==="friend"?await ar(e,i,(n=ve.params)==null?void 0:n.friendId,(r=ve.params)==null?void 0:r.friendName):s==="mini_league"||s==="mini-league"?await nn(e,i,(o=ve.params)==null?void 0:o.mlMatchId,(a=ve.params)==null?void 0:a.leagueId):await Po(e,i);break}case"market":await Lr(e,i);break;case"rankings":await Sr(e,i);break;case"matches":await Mr(e,i);break;case"friends":await Hn(e,i);break;case"mini-league":await lr(e,i);break;case"match-mini-league":{const s=ve.params||{};await nn(e,i,s.mlMatchId,s.leagueId);break}default:await Ui(e,i)}}function Cr(){var n;const e=document.getElementById("app"),t=ve.profile;if(!t)return;const i="/icons/";e.innerHTML=`
    <nav class="top-nav">
  ...
  </div>`,(n=e.querySelector("#pl-play-btn"))==null||n.addEventListener("click",t)}async function Dr(){document.documentElement.setAttribute("data-theme","light"),document.getElementById("modal-overlay").addEventListener("click",i=>{i.target===i.currentTarget&&zi()}),document.getElementById("modal-close").addEventListener("click",zi);const{data:{session:e}}=await w.auth.getSession();if(!e){on(),qr(document.getElementById("app"),{onPlay:()=>Ni(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:Bt})});return}ve.user=e.user,await Ot(),on();try{const{data:i}=await w.from("formation_links_overrides").select("formation, links"),n={};(i||[]).forEach(r=>{n[r.formation]=r.links}),qn(n)}catch(i){console.warn("Impossible de charger les overrides de formation:",i)}if(!ve.profile){Pn(document.getElementById("app"),{state:ve,navigate:async()=>{await Ot(),hi()},toast:Bt,refreshProfile:Ot});return}const t=Array.isArray(ve.profile.pending_boosters)?ve.profile.pending_boosters:[];if(!ve.profile.onboarding_done&&t.length>0){Mo(document.getElementById("app"),{state:ve,navigate:()=>hi(),toast:Bt,refreshProfile:Ot});return}hi(),setTimeout(()=>Er(ve.profile,Ht,Bt),800),w.auth.onAuthStateChange(async(i,n)=>{i==="SIGNED_OUT"&&(ve.user=null,ve.profile=null,document.getElementById("app").innerHTML="",Ni(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:Bt}))})}function Fr(){return Math.round(window.visualViewport&&window.visualViewport.height||window.innerHeight)}function ui(){const e=document.getElementById("app");e&&(e.style.height=Fr()+"px")}window.addEventListener("resize",ui);window.addEventListener("orientationchange",()=>setTimeout(ui,150));window.visualViewport&&window.visualViewport.addEventListener("resize",ui);function hi(){const e=()=>{var i;(i=ve.user)!=null&&i.id&&w.from("users").update({last_seen_at:new Date().toISOString()}).eq("id",ve.user.id).then(()=>{})};e(),window._presencePingInterval&&clearInterval(window._presencePingInterval),window._presencePingInterval=setInterval(e,6e4);const t=document.getElementById("app");t.style.display="flex",t.style.flexDirection="column",ui(),Cr(),An()}function on(){const e=document.getElementById("app-loader"),t=document.getElementById("app");t&&(t.style.display=""),e&&(e.classList.add("zoom-out"),setTimeout(()=>e.style.display="none",500))}function Mn(e){var n;const t=document.getElementById("app-loader");if(t&&(t.style.display="none"),document.getElementById("boot-error"))return;const i=document.createElement("div");i.id="boot-error",i.style.cssText="position:fixed;inset:0;background:#0a1628;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:99999;gap:16px;color:#fff;padding:24px;text-align:center",i.innerHTML=`
    <div style="font-size:42px">📡</div>
    <div style="font-size:18px;font-weight:900">Connexion impossible</div>
    <div style="font-size:13px;color:rgba(255,255,255,0.6);max-width:280px">${e||"Le chargement a échoué. Vérifie ta connexion et réessaie."}</div>
    <button id="boot-retry" style="margin-top:8px;padding:12px 30px;border-radius:10px;border:none;background:#1A6B3C;color:#fff;font-size:15px;font-weight:700;cursor:pointer">Réessayer</button>`,document.body.appendChild(i),(n=document.getElementById("boot-retry"))==null||n.addEventListener("click",()=>window.location.reload())}Dr().catch(e=>{console.error("Échec du démarrage:",e),Mn()});setTimeout(()=>{const e=document.getElementById("app-loader");e&&e.style.display!=="none"&&!e.classList.contains("zoom-out")&&!document.getElementById("boot-error")&&Mn("Le serveur met trop de temps à répondre.")},12e3);
