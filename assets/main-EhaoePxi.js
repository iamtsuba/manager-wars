import{s as $,l as J,F as ke,b as ge,c as He,d as ut}from"./formation-links-CDBKB_z4.js";function je(e,{navigate:t,toast:i}){e.innerHTML=`
  ...
  </div>`}async function wt(e,t){const{state:i,navigate:s,toast:d,openModal:o,closeModal:l}=t;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:a}=await $.from("cards").select(`id, card_type, current_note, gc_type, formation, is_for_sale, sale_price,
      player:players(id, firstname, surname_encoded, country_code, club_id, job, job2,
        note_g, note_d, note_m, note_a, rarity, note_min, note_max, skin, hair, hair_length, sell_price,
        clubs(encoded_name, logo_url))`).eq("owner_id",i.profile.id),{data:n}=await $.from("players").select(`id, firstname, surname_encoded, country_code, club_id, job, job2,
      note_g, note_d, note_m, note_a, rarity, note_min, note_max, skin, hair, hair_length,
      clubs(encoded_name, logo_url)`).eq("is_active",!0),r=(a||[]).filter(I=>I.card_type==="player"&&I.player),c=(a||[]).filter(I=>I.card_type==="game_changer"),p=(a||[]).filter(I=>I.card_type==="formation"),f=Object.keys(ke),u=Object.keys(ie),x={};r.forEach(I=>{const L=I.player.id;x[L]=(x[L]||0)+1});const y=new Set(Object.keys(x).map(I=>String(I))),h=new Set(p.map(I=>I.formation)),k=new Set(c.map(I=>I.gc_type));let m="player",g="Tous",v="",E=!1;function b(){return[...r].sort((I,L)=>{const M=re.indexOf(I.player.job),B=re.indexOf(L.player.job);return M!==B?M-B:(I.player.surname_encoded||"").localeCompare(L.player.surname_encoded||"")})}function w(){return[...n||[]].sort((I,L)=>{const M=re.indexOf(I.job),B=re.indexOf(L.job);return M!==B?M-B:(I.surname_encoded||"").localeCompare(L.surname_encoded||"")})}function _(){return b().filter(I=>{const L=I.player,M=g==="Tous"||L.job===g,B=!v||`${L.firstname} ${L.surname_encoded}`.toLowerCase().includes(v);return M&&B})}function T(){return w().filter(I=>{const L=g==="Tous"||I.job===g,M=!v||`${I.firstname} ${I.surname_encoded}`.toLowerCase().includes(v);return L&&M})}e.innerHTML=`
  <div class="page">
    ...
    <div class="page-body">
      <div style="display:flex;flex-wrap:wrap;gap:10px;justify-content:flex-start" id="col-grid"></div>
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{E=!E,A(),z()})))}function z(){const I=document.getElementById("col-grid");I&&(m==="player"?C(I):m==="formation"?N(I):q(I))}function C(I){if(E){const L=T();if(!L.length){I.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}I.innerHTML=L.map(M=>{if(y.has(String(M.id))){const S=r.find(H=>H.player.id===M.id),G=x[M.id]||1,O=G>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${G}</div>`:"",K=r.filter(H=>H.player.id===M.id&&H.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Fe(S,O+K)}return vt(M)}).join("")}else{const L=_();if(!L.length){I.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const M={},B=[];L.forEach(S=>{const G=S.player.id;M[G]||(M[G]=!0,B.push(S))}),I.innerHTML=B.map(S=>{const G=x[S.player.id]||1,O=G>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${G}</div>`:"",K=r.filter(H=>H.player.id===S.player.id&&H.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Fe(S,O+K)}).join("")}I.querySelectorAll("[data-card-id]").forEach(L=>{L.addEventListener("click",()=>{const M=r.find(B=>B.id===L.dataset.cardId);M&&kt(M,r,x,t)})})}function N(I){const L=E?f:[...h];if(!L.length){I.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}I.innerHTML=L.map(M=>{if(h.has(M)){const S=p.find(P=>P.formation===M),G=p.filter(P=>P.formation===M).length,O=G>1?`<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${G}</div>`:"";return`
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const ee=1e3;function _t(e,t,i,s){var y,h,k;const{state:d,toast:o,closeModal:l,navigate:a,refreshProfile:n}=i,r=e.formation,c={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function p(){const m=ge[r]||{},g=ke[r]||[],v=290,E=360,b=20;function w(T){const A=m[T];return A?{x:A.x*v,y:A.y*E}:null}let _=`<svg width="${v}" height="${E}" viewBox="0 0 ${v} ${E}" xmlns="http://www.w3.org/2000/svg">`;for(const[T,A]of g){const z=w(T),C=w(A);!z||!C||(_+=`<line x1="${z.x}" y1="${z.y}" x2="${C.x}" y2="${C.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const T of Object.keys(m)){const A=w(T);if(!A)continue;const z=T.replace(/\d+/,""),C=c[z]||"#555";_+=`<circle cx="${A.x}" cy="${A.y}" r="${b}" fill="${C}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,_+=`<text x="${A.x}" y="${A.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${z}</text>`}return _+="</svg>",_}const f=t.filter(m=>m.formation===r),u=f.length,x=!e.is_for_sale;s(`Formation ${r}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`:""}`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(y=document.getElementById("direct-sell-form-btn"))==null||y.addEventListener("click",async()=>{if(!confirm(`Vendre 1 carte Formation ${r} pour ${ee.toLocaleString("fr")} crédits ? Cette action est irréversible.`))return;const m=f.find(v=>!v.is_for_sale)||f[0];if(!m){o("Aucune carte à vendre","error");return}const{error:g}=await $.from("cards").delete().eq("id",m.id);if(g){o(g.message,"error");return}await $.from("users").update({credits:(d.profile.credits||0)+ee}).eq("id",d.profile.id),await n(),o(`+${ee.toLocaleString("fr")} crédits ! Carte vendue.`,"success"),l(),a("collection")}),(h=document.getElementById("market-sell-form-btn"))==null||h.addEventListener("click",async()=>{const m=parseInt(document.getElementById("sell-price-form").value);if(!m||m<1){o("Prix invalide","error");return}await $.from("cards").update({is_for_sale:!0,sale_price:m}).eq("id",e.id),await $.from("market_listings").insert({seller_id:d.profile.id,card_id:e.id,price:m}),o("Carte mise en vente sur le marché !","success"),l(),a("collection")}),(k=document.getElementById("cancel-sell-form-btn"))==null||k.addEventListener("click",async()=>{await $.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",e.id),await $.from("market_listings").update({status:"cancelled"}).eq("card_id",e.id).eq("status","active"),o("Annonce retirée","success"),l(),a("collection")})}function kt(e,t,i,s){var b,w,_,T,A,z;const{state:d,toast:o,openModal:l,closeModal:a,navigate:n,refreshProfile:r}=s,c=e.player,p=t.filter(C=>C.player.id===c.id),f=p.length,u=bt[c.rarity]||1e3,x=c.rarity!=="legende",y=Qe(c),h=oe(c,c.job),k=c.job2?oe(c,c.job2):null,m=pe[c.job]||"#1A6B3C",g=c.job2?pe[c.job2]:null,v=Xe[c.rarity]||"#ccc",E=Ee[c.country_code]||c.country_code||"";l(`${c.firstname} ${c.surname_encoded}`,`<div style="display:flex;gap:16px;flex-wrap:wrap;justify-content:center">
  ...
  </div>`,document.getElementById("new-deck-btn").addEventListener("click",async()=>{const l=prompt("Nom du deck :",`Deck ${((o==null?void 0:o.length)||0)+1}`);if(!l)return;const{data:a,error:n}=await $.from("decks").insert({owner_id:i.profile.id,name:l}).select().single();if(n){d(n.message,"error");return}d("Deck créé !","success"),De(a.id,e,t)}),e.querySelectorAll("[data-open-deck]").forEach(l=>{l.addEventListener("click",()=>De(l.dataset.openDeck,e,t))}),e.querySelectorAll("[data-rename]").forEach(l=>{l.addEventListener("click",async()=>{const a=prompt("Nouveau nom :",l.dataset.name);if(!a||a===l.dataset.name)return;const{error:n}=await $.from("decks").update({name:a}).eq("id",l.dataset.rename);if(n){d(n.message,"error");return}d("Deck renommé !","success"),ye(e,t)})}),e.querySelectorAll("[data-delete]").forEach(l=>{l.addEventListener("click",async()=>{if(!confirm(`Supprimer le deck "${l.dataset.name}" ? Cette action est irréversible.`))return;await $.from("deck_cards").delete().eq("deck_id",l.dataset.delete);const{error:a}=await $.from("decks").delete().eq("id",l.dataset.delete);if(a){d(a.message,"error");return}d("Deck supprimé.","success"),ye(e,t)})})}async function De(e,t,i){const{state:s,toast:d}=i;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:o}=await $.from("decks").select("*").eq("id",e).single(),{data:l}=await $.from("cards").select(`id, card_type, formation,
      player:players(id, firstname, surname_encoded, country_code, club_id, job, job2,
        note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length,
        clubs(encoded_name, logo_url))`).eq("owner_id",s.profile.id),a=(l||[]).filter(u=>u.card_type==="player"&&u.player),n=(l||[]).filter(u=>u.card_type==="formation"),r=n.map(u=>u.formation).filter(Boolean),{data:c}=await $.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",e);let p=o.formation||"4-4-2";r.length>0&&!r.includes(p)&&(p=r[0]);const f={deckId:e,name:o.name,formation:p,formationCardId:o.formation_card_id,slots:{},subs:[],playerCards:a,formationCards:n,availableFormations:r};(c||[]).forEach(u=>{u.is_starter?f.slots[u.position]=u.card_id:f.subs.includes(u.card_id)||f.subs.push(u.card_id)}),X(t,f,i)}function X(e,t,i){var n;const{navigate:s}=i;fe[t.formation];const d=Ge(t.formation),o=d.filter(r=>t.slots[r]).length,l=t.availableFormations.length>0?t.availableFormations:Object.keys(fe),a=t.subs.map(r=>t.playerCards.find(c=>c.id===r)).filter(Boolean);[...Object.values(t.slots),...t.subs],e.innerHTML=`
  <div class="page">
  ...
  </div>`,It(e,t,d,i),document.getElementById("builder-back").addEventListener("click",()=>s("decks")),document.getElementById("formation-select").addEventListener("change",r=>{t.formation=r.target.value;const c=Ge(t.formation),p={};c.forEach(f=>{t.slots[f]&&(p[f]=t.slots[f])}),t.slots=p,X(e,t,i)}),document.getElementById("save-deck").addEventListener("click",()=>Lt(t,i)),e.querySelectorAll("[data-remove-sub]").forEach(r=>{r.addEventListener("click",()=>{t.subs=t.subs.filter(c=>c!==r.dataset.removeSub),X(e,t,i)})}),(n=document.getElementById("add-sub-btn"))==null||n.addEventListener("click",()=>{Tt(t,e,i)})}function It(e,t,i,s){const d=e.querySelector("#deck-field");if(!d)return;const o=ge[t.formation]||{},l=He(t.formation)||[],a={};for(const k of i){const m=t.slots[k],g=m?t.playerCards.find(v=>v.id===m):null;a[k]=g?g.player:null}const n=300,r=300,c=48,p=64,f=13,u=16,x=38;function y(k){const m=o[k];return m?{x:m.x*n,y:m.y*r}:null}let h="";for(const[k,m]of l){const g=y(k),v=y(m);if(!g||!v)continue;const E=a[k]?{...a[k],club_id:a[k].club_id,country_code:a[k].country_code,rarity:a[k].rarity}:null,b=a[m]?{...a[m],club_id:a[m].club_id,country_code:a[m].country_code,rarity:a[m].rarity}:null,w=J(E,b);w==="#ff3333"||w==="#cc2222"?h+=`<line x1="${g.x.toFixed(1)}" y1="${g.y.toFixed(1)}" x2="${v.x.toFixed(1)}" y2="${v.y.toFixed(1)}" stroke="${w}" stroke-width="2.5" stroke-linecap="round" opacity="0.4"/>`:(h+=`<line x1="${g.x.toFixed(1)}" y1="${g.y.toFixed(1)}" x2="${v.x.toFixed(1)}" y2="${v.y.toFixed(1)}" stroke="${w}" stroke-width="8" stroke-linecap="round" opacity="0.2"/>`,h+=`<line x1="${g.x.toFixed(1)}" y1="${g.y.toFixed(1)}" x2="${v.x.toFixed(1)}" y2="${v.y.toFixed(1)}" stroke="${w}" stroke-width="2.5" stroke-linecap="round" opacity="0.9"/>`)}for(const k of i){const m=y(k);if(!m)continue;const g=a[k],v=k.replace(/\d+/,""),E=U[v]||"#555",b=(m.x-c/2).toFixed(1),w=(m.y-p/2).toFixed(1),_={normal:"#aaa",pépite:"#D4A017",papyte:"#222",légende:"#7a28b8"}[g==null?void 0:g.rarity]||"#aaa";if(g){const T=xe(g),A=et(g),z=Ze(g.country_code),C=Number(v==="GK"?g.note_g:v==="DEF"?g.note_d:v==="MIL"?g.note_m:g.note_a)||0,N=p-f-u;h+=`<defs><clipPath id="dcp-${k}"><rect x="${b}" y="${(m.y-p/2+f).toFixed(1)}" width="${c}" height="${N}"/></clipPath></defs>`,h+=`<rect x="${b}" y="${w}" width="${c}" height="${p}" rx="5" fill="${E}"/>`,T&&(h+=`<image href="${T}" x="${b}" y="${(m.y-p/2+f).toFixed(1)}" width="${c}" height="${N}" clip-path="url(#dcp-${k})" preserveAspectRatio="xMidYMin slice"/>`),h+=`<rect x="${b}" y="${w}" width="${c}" height="${f}" fill="rgba(255,255,255,0.93)"/>`,h+=`<text x="${m.x.toFixed(1)}" y="${(m.y-p/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(g.surname_encoded||"").slice(0,9)}</text>`;const q=(m.y+p/2-u).toFixed(1);h+=`<rect x="${b}" y="${q}" width="${c}" height="${u}" fill="rgba(255,255,255,0.93)"/>`,z&&(h+=`<image href="${z}" x="${(m.x-21).toFixed(1)}" y="${(m.y+p/2-u+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`),h+=`<text x="${m.x.toFixed(1)}" y="${(m.y+p/2-u/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${C}</text>`,A&&(h+=`<image href="${A}" x="${(m.x+c/2-14).toFixed(1)}" y="${(m.y+p/2-u+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`),h+=`<rect x="${b}" y="${w}" width="${c}" height="${p}" rx="5" fill="none" stroke="${_}" stroke-width="2"/>`}else h+=`<rect x="${b}" y="${w}" width="${c}" height="${p}" rx="5" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="5,3"/>`,h+=`<text x="${m.x.toFixed(1)}" y="${m.y.toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="22" fill="rgba(255,255,255,0.35)">+</text>`,h+=`<text x="${m.x.toFixed(1)}" y="${(m.y+16).toFixed(1)}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.3)">${v}</text>`;h+=`<rect x="${b}" y="${w}" width="${c}" height="${p}" rx="5" fill="rgba(0,0,0,0.01)" class="deck-slot-hit" data-pos="${k}" style="cursor:pointer"/>`}d.innerHTML=`<svg viewBox="${-x} ${-x} ${n+x*2} ${r+x*2}" width="100%" style="display:block;max-width:440px;margin:0 auto">${h}</svg>`,d.querySelectorAll(".deck-slot-hit").forEach(k=>{k.addEventListener("click",()=>At(k.dataset.pos,t,e,s))})}function At(e,t,i,s){var f,u,x;const{openModal:d,closeModal:o}=s,l=e.replace(/\d+/,""),a=t.slots[e],n=a?t.playerCards.find(y=>y.id===a):null;(f=n==null?void 0:n.player)==null||f.id;const r=new Set;Object.entries(t.slots).forEach(([y,h])=>{var m;if(y===e||!h)return;const k=t.playerCards.find(g=>g.id===h);(m=k==null?void 0:k.player)!=null&&m.id&&r.add(k.player.id)}),t.subs.forEach(y=>{var k;const h=t.playerCards.find(m=>m.id===y);(k=h==null?void 0:h.player)!=null&&k.id&&r.add(h.player.id)});const c=new Set,p=t.playerCards.filter(y=>{const h=y.player;return!(h.job===l||h.job2===l)||r.has(h.id)||c.has(h.id)?!1:(c.add(h.id),!0)});p.sort((y,h)=>{const k=l==="GK"?y.player.note_g:l==="DEF"?y.player.note_d:l==="MIL"?y.player.note_m:y.player.note_a;return(l==="GK"?h.player.note_g:l==="DEF"?h.player.note_d:l==="MIL"?h.player.note_m:h.player.note_a)-k}),d(`Choisir ${l} — ${e}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${t.slots[e]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${p.length>0?p.map(y=>{var v,E;const h=y.player,k=l==="GK"?h.note_g:l==="DEF"?h.note_d:l==="MIL"?h.note_m:h.note_a,m=xe(h),g={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[h.rarity];return`<div class="player-option" data-card-id="${y.id}"
  ...
  `,document.body.appendChild(s);let d=!1;document.getElementById("pack-visual").addEventListener("click",()=>{if(d)return;d=!0;const u=document.getElementById("pack-visual");u.classList.add("shaking"),setTimeout(()=>{u.classList.add("pack-open"),setTimeout(()=>{document.getElementById("pack-phase").style.display="none",l(0)},600)},500)});let o=0;function l(u){o=u;const x=document.getElementById("reveal-phase");x.style.display="flex",a(u)}function a(u){var b;const x=e[u],y=document.getElementById("card-counter"),h=document.getElementById("single-card-slot"),k=document.getElementById("card-tap-hint");y&&(y.textContent=`Carte ${u+1} / ${e.length}`),k&&(k.textContent=u<e.length-1?"Appuie pour continuer →":"Appuie pour voir toutes tes cartes");const m=Re(x),g=x.card_type==="player"&&((b=x.player)==null?void 0:b.rarity)==="legende";h.innerHTML=`
      ...
      </div>`,g&&c();const v=document.getElementById("current-card-wrap");let E=!1;v.addEventListener("click",()=>{if(E)return;E=!0;const w=u+1;w<e.length?(v.style.transition="all 0.25s ease",v.style.transform="translateX(-120%) rotate(-15deg)",v.style.opacity="0",setTimeout(()=>a(w),250)):n()})}function n(){p(),document.getElementById("reveal-phase").style.display="none";const u=document.getElementById("recap-phase");u.style.display="flex";const x=document.getElementById("recap-grid");x.innerHTML=e.map((y,h)=>`
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("odds-close").addEventListener("click",()=>e.remove())}const Xt="simulation",ot="VOTRE_ZONE_ID";function Qt(){switch(Xt){case"propellerads":return Zt();case"adsense":return ei();default:return Ie()}}function Ie(){return new Promise(e=>{const t=document.createElement("div");t.style.cssText="position:fixed;inset:0;background:#111;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:14px";let i=5;t.innerHTML=`
    ...
    <div id="deck-select-screen" style="display:flex;flex-direction:column;height:calc(100dvh - 130px);overflow:hidden;background:#0a3d1e;color:#fff">
      ...
      <div style="display:flex;justify-content:center;gap:6px;padding:4px;flex-shrink:0">
        ${o.map((w,_)=>`<div style="width:7px;height:7px;border-radius:50%;background:${_===n?"#FFD700":"rgba(255,255,255,0.25)"}"></div>`).join("")}
      </div>`:""}
    ...
    </div>`,(g=document.getElementById("prev-deck"))==null||g.addEventListener("click",()=>{n>0&&(n--,r())}),(v=document.getElementById("next-deck"))==null||v.addEventListener("click",()=>{n<o.length-1&&(n++,r())}),(E=document.getElementById("validate-deck"))==null||E.addEventListener("click",()=>{h&&t.navigate("match",{matchMode:i,deckId:c.id})}),(b=document.getElementById("cancel-deck-select"))==null||b.addEventListener("click",()=>d("home"));const k=document.getElementById("deck-swipe-zone");if(k){let w=0,_=0;k.addEventListener("touchstart",T=>{w=T.touches[0].clientX,_=T.touches[0].clientY},{passive:!0}),k.addEventListener("touchend",T=>{const A=T.changedTouches[0].clientX-w,z=T.changedTouches[0].clientY-_;Math.abs(A)<40||Math.abs(A)<Math.abs(z)||(A<0&&n<o.length-1?(n++,r()):A>0&&n>0&&(n--,r()))},{passive:!0})}}r()}function si(e,t,i){e.innerHTML=`
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;min-height:100vh;gap:12px;padding:12px 16px;background:#0a3d1e">
    ...
    <style>@keyframes ld{0%,20%{opacity:0.3}50%{opacity:1}80%,100%{opacity:0.3}}.loading-dots::after{content:'...';animation:ld 1.4s infinite}</style>
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${p.map((x,y)=>{const h=R(x),k=y<p.length-1?J(x,p[y+1]):null,m=k&&k!=="#ff3333"&&k!=="#cc2222";return`
  ...
  </div>`}function ae(e,t,i){if(!(e!=null&&e.length))return"";const s=e.slice(0,5);let d='<div style="display:flex;align-items:center;gap:0;flex-wrap:nowrap;overflow:hidden">';return s.forEach((o,l)=>{if(d+=Y(o,40,52),l<s.length-1){const a=J(o,s[l+1]);d+=`<div style="width:7px;height:3px;background:${a==="#ff3333"||a==="#cc2222"?"rgba(255,255,255,0.12)":a};border-radius:2px;flex-shrink:0;margin:0 1px"></div>`}}),i!==void 0&&(d+=`<div style="margin-left:6px;background:${t};color:${t==="#00ff88"?"#000":"#fff"};border-radius:6px;padding:3px 8px;font-size:15px;font-weight:900;flex-shrink:0">${i}</div>`),d+="</div>",d}function ve(e){return!e||e.length<2?null:`https://flagcdn.com/24x18/${e.slice(0,2).toLowerCase()}.png`}function st(e){if(!e||e.length<2)return"🌍";try{return String.fromCodePoint(127462+e.charCodeAt(0)-65)+String.fromCodePoint(127462+e.charCodeAt(1)-65)}catch{return"🌍"}}function Te(e,t,i,s,d=310,o=310){const l=ge[t]||{},a=He(t)||ke[t]||[],n={},r=["ATT","MIL","DEF","GK"];for(const m of r)(e[m]||[]).forEach((v,E)=>{n[`${m}${E+1}`]=v});function c(m){const g=l[m];return g?{x:g.x*d,y:g.y*o}:null}let p="";for(const[m,g]of a){const v=c(m),E=c(g);if(!v||!E)continue;const b=n[m],w=n[g],_=J(b,w);_==="#00ff88"||_==="#FFD700"?(p+=`<line x1="${v.x.toFixed(1)}" y1="${v.y.toFixed(1)}" x2="${E.x.toFixed(1)}" y2="${E.y.toFixed(1)}"
        stroke="${_}" stroke-width="10" stroke-linecap="round" opacity="0.22"/>`,p+=`<line x1="${v.x.toFixed(1)}" y1="${v.y.toFixed(1)}" x2="${E.x.toFixed(1)}" y2="${E.y.toFixed(1)}"
        stroke="${_}" stroke-width="3.5" stroke-linecap="round" opacity="0.95"/>`):p+=`<line x1="${v.x.toFixed(1)}" y1="${v.y.toFixed(1)}" x2="${E.x.toFixed(1)}" y2="${E.y.toFixed(1)}"
        stroke="${_}" stroke-width="3.5" stroke-linecap="round" opacity="0.7"/>`}const f=48,u=64,x=13,y=16,h={normal:"#aaaaaa",pépite:"#D4A017",papyte:"#111111",légende:"#7a28b8"};for(const[m,g]of Object.entries(n)){const v=c(m);if(!v||!g)continue;const E=m.replace(/[0-9]/g,""),b=Z[E]||"#555",w=i==="attack"&&["MIL","ATT"].includes(E)&&!g.used||i==="defense"&&["GK","DEF","MIL"].includes(E)&&!g.used,_=s.includes(g.cardId);let T;i==="attack"?T=E==="MIL"?Number(g.note_m)||0:Number(g.note_a)||0:i==="defense"?T=E==="GK"?Number(g.note_g)||0:E==="MIL"?Number(g.note_m)||0:Number(g.note_d)||0:T=Number(E==="GK"?g.note_g:E==="DEF"?g.note_d:E==="MIL"?g.note_m:g.note_a)||0,T=T+(g.boost||0);const A=(v.x-f/2).toFixed(1),z=(v.y-u/2).toFixed(1),C=g.used?.25:1,N=h[g==null?void 0:g.rarity]||h.normal,q=_?"#ff3030":N,I=_?3:(g==null?void 0:g.rarity)==="légende"||(g==null?void 0:g.rarity)==="pépite"?2.5:2,L=u-x-y;p+=`<defs><clipPath id="cp-${m}"><rect x="${A}" y="${(v.y-u/2+x).toFixed(1)}" width="${f}" height="${L}"/></clipPath></defs>`,p+=`<rect x="${A}" y="${z}" width="${f}" height="${u}" rx="5" fill="${b}" opacity="${C}"/>`;const M=R(g);M&&!g.used&&(p+=`<image href="${M}" x="${A}" y="${(v.y-u/2+x).toFixed(1)}" width="${f}" height="${L}" clip-path="url(#cp-${m})" preserveAspectRatio="xMidYMin slice"/>`),p+=`<rect x="${A}" y="${z}" width="${f}" height="${x}" rx="4" fill="rgba(255,255,255,0.92)"/>`,p+=`<text x="${v.x.toFixed(1)}" y="${(v.y-u/2+8.5).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="6.5" font-weight="900" fill="#111" font-family="Arial Black,Arial">${(g.name||"").slice(0,9)}</text>`;const B=(v.y+u/2-y).toFixed(1);if(p+=`<rect x="${A}" y="${B}" width="${f}" height="${y}" fill="rgba(255,255,255,0.92)"/>`,g.used)p+=`<text x="${v.x.toFixed(1)}" y="${(v.y+u/2-y/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="13" font-weight="900" fill="rgba(0,0,0,0.4)" font-family="Arial Black">–</text>`;else{const S=ve(g.country_code);S?p+=`<image href="${S}" x="${(v.x-20).toFixed(1)}" y="${(v.y+u/2-y+3).toFixed(1)}" width="13" height="10" preserveAspectRatio="xMidYMid slice"/>`:p+=`<text x="${(v.x-13).toFixed(1)}" y="${(v.y+u/2-y/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="10">${st(g.country_code)}</text>`,p+=`<text x="${v.x.toFixed(1)}" y="${(v.y+u/2-y/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="12" font-weight="900" fill="#111" font-family="Arial Black">${T}</text>`;const G=at(g);G?p+=`<image href="${G}" x="${(v.x+f/2-14).toFixed(1)}" y="${(v.y+u/2-y+2).toFixed(1)}" width="12" height="12" preserveAspectRatio="xMidYMid meet"/>`:g.clubName&&(p+=`<text x="${(v.x+14).toFixed(1)}" y="${(v.y+u/2-y/2).toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="5.5" font-weight="700" fill="#333">${(g.clubName||"").slice(0,3).toUpperCase()}</text>`),g.boost&&(p+=`<rect x="${(v.x+f/2-12).toFixed(1)}" y="${(v.y-u/2).toFixed(1)}" width="14" height="10" rx="3" fill="#87CEEB"/>`,p+=`<text x="${(v.x+f/2-5).toFixed(1)}" y="${(v.y-u/2+6).toFixed(1)}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${g.boost}</text>`)}p+=`<rect x="${A}" y="${z}" width="${f}" height="${u}" rx="5" fill="${_?"rgba(255,255,255,0.12)":"none"}" stroke="${q}" stroke-width="${I}" opacity="${C}"/>`,w&&(p+=`<rect x="${A}" y="${z}" width="${f}" height="${u}" rx="5" fill="rgba(0,0,0,0.01)" class="match-slot-hit ${_?"selected":""}" data-card-id="${g.cardId}" data-role="${E}" style="cursor:pointer"/>`)}const k=38;return`<svg viewBox="${-k} ${-k} ${d+k*2} ${o+k*2}" width="100%" style="display:block;width:100%;max-width:440px;margin:0 auto">
    ${p}
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;height:calc(100dvh - 130px);max-height:calc(100dvh - 130px);overflow:hidden;background:#0a3d1e;position:relative">
    ...
    <div id="last-action-zone" style="background:rgba(0,0,0,0.3);flex-shrink:0;overflow:hidden;max-height:100px">
      ${(()=>{if(t.phase==="defense"&&t.pendingAttack){const w=t.pendingAttack;return`<div style="padding:5px 8px;background:rgba(180,30,30,0.2);border-left:3px solid #ff6b6b">
            ...
            ${ae((w.players||[]).map(_=>({..._,used:!1})),"#ff6b6b",w.total)}
            ...
            ${ae((w.players||[]).map(_=>({..._,used:!1})),"#00ff88",w.total)}
          </div>`}const b=t.log[t.log.length-1];return b?'<div style="padding:2px 4px">'+li(b)+"</div>":'<div style="padding:6px 8px;font-size:11px;color:rgba(255,255,255,0.3)">⏳ Match en cours...</div>'})()}
    </div>
  ...
  `,document.body.appendChild(s);let c=!1;const p=()=>{c||(c=!0,s.remove(),i())};s.addEventListener("click",p),setTimeout(p,2e3)}function te(e,t="rgba(0,0,0,0.8)"){const i=document.createElement("div");i.style.cssText=`position:fixed;bottom:110px;left:50%;transform:translateX(-50%);background:${t};color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:1200;pointer-events:none;text-align:center;max-width:80vw;white-space:nowrap`,i.textContent=e,document.body.appendChild(i),setTimeout(()=>i.remove(),2200)}function qe(e,t,i,s=null){if(t.phase!=="attack"){te("⏰ Remplacement uniquement avant une attaque","rgba(180,100,0,0.9)");return}if(t.usedSubIds||(t.usedSubIds=[]),t.subsUsed>=t.maxSubs){te(`Maximum ${t.maxSubs} remplacements atteint`,"rgba(180,30,30,0.9)");return}const d=Object.values(t.homeTeam).flat().filter(p=>p.used),o=t.homeSubs.filter(p=>!t.usedSubIds.includes(p.cardId));if(!d.length){te("Aucun joueur utilisé à remplacer");return}if(!o.length){te("Aucun remplaçant disponible");return}let l=0,a=Math.max(0,o.findIndex(p=>p.cardId===s)),n=!1;const r=document.createElement("div");r.id="sub-overlay",r.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:700;display:flex;flex-direction:column;overflow:hidden";function c(){var k,m,g,v,E,b;const p=d[l],f=o[a],u=Math.min(130,Math.round((window.innerWidth-90)/2)),x=Math.round(u*1.35),y=w=>`background:rgba(255,255,255,0.12);border:none;color:${w?"rgba(255,255,255,0.2)":"#fff"};width:40px;height:40px;border-radius:50%;font-size:20px;cursor:${w?"default":"pointer"};flex-shrink:0`;r.innerHTML=`
    ...
    </div>`,(k=r.querySelector("#sub-close"))==null||k.addEventListener("click",()=>r.remove()),(m=r.querySelector("#out-up"))==null||m.addEventListener("click",()=>{l>0&&(l--,c())}),(g=r.querySelector("#out-down"))==null||g.addEventListener("click",()=>{l<d.length-1&&(l++,c())}),(v=r.querySelector("#in-up"))==null||v.addEventListener("click",()=>{a>0&&(a--,c())}),(E=r.querySelector("#in-down"))==null||E.addEventListener("click",()=>{a<o.length-1&&(a++,c())});const h=(w,_,T,A)=>{const z=r.querySelector("#"+w);if(!z)return;let C=0;z.addEventListener("touchstart",N=>{C=N.touches[0].clientY},{passive:!0}),z.addEventListener("touchend",N=>{const q=N.changedTouches[0].clientY-C;if(Math.abs(q)<30)return;const I=_();q<0&&I<A-1?(T(I+1),c()):q>0&&I>0&&(T(I-1),c())},{passive:!0})};h("in-panel",()=>a,w=>a=w,o.length),h("out-panel",()=>l,w=>l=w,d.length),(b=r.querySelector("#sub-confirm"))==null||b.addEventListener("click",w=>{if(w.preventDefault(),w.stopPropagation(),n)return;n=!0;const _=d[l],T=o[a];if(!_||!T)return;let A=null,z=-1;for(const[N,q]of Object.entries(t.homeTeam)){const I=(q||[]).findIndex(L=>L.cardId===_.cardId);if(I!==-1){A=N,z=I;break}}if(z===-1||!A){te("Erreur : joueur introuvable","rgba(180,0,0,0.9)"),r.remove();return}const C={...T,_line:A,_col:_._col||0,used:!1,boost:0};t.homeTeam[A].splice(z,1,C),t.usedSubIds||(t.usedSubIds=[]),t.usedSubIds.push(T.cardId),t.subsUsed++,t.selected=[],t.log.push({type:"sub",subSide:"home",clubName:t.clubName,outPlayer:{name:_.name,firstname:_.firstname,note:F(_,A),portrait:R(_),job:_.job,country_code:_.country_code,rarity:_.rarity,clubName:_.clubName,clubLogo:_.clubLogo},inPlayer:{name:T.name,firstname:T.firstname,note:F(T,A),portrait:R(T),job:T.job,country_code:T.country_code,rarity:T.rarity,clubName:T.clubName,clubLogo:T.clubLogo},text:`🔄 ${T.firstname} ${T.name} remplace ${_.firstname} ${_.name}`}),r.remove(),gi(_,T,()=>D(e,t,i))})}document.body.appendChild(r),c()}function xi(e,t,i,s,d){var a,n;const o=ie[t]||{icon:"⚡",desc:"Carte spéciale."},l=document.createElement("div");l.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:750;display:flex;align-items:center;justify-content:center;padding:24px",l.innerHTML=`
    ...
    <div style="flex:1;overflow-y:auto;padding:12px;display:flex;flex-wrap:wrap;gap:8px;align-content:flex-start">
      ${e.map(p=>{const f=p._line||p.job||"MIL",u={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[f]||"#555",x=F(p,f)+(p.boost||0),y=a.find(h=>h.cardId===p.cardId);return`<div class="gc-pick-item" data-cid="${p.cardId}"
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:8px;animation:scoreIn 0.3s ease 1.4s both;position:relative;z-index:1">Appuyer pour continuer</div>`,document.body.appendChild(o);let a=!1;const n=()=>{a||(a=!0,o.remove(),setTimeout(()=>d(),50))};o.addEventListener("click",n),setTimeout(n,3500)}async function W(e,t,i){var c,p;t._timerInt&&(clearInterval(t._timerInt),t._timerInt=null),t.phase="finished";const{state:s}=i,d=t.homeScore>t.aiScore,o=t.homeScore===t.aiScore,l=d?"victoire":o?"nul":"defaite",a=yt(t.mode,l);t.matchId&&await $.from("matches").update({status:"finished",home_score:t.homeScore,away_score:t.aiScore,winner_id:d?s.profile.id:null,home_credits_reward:a,played_at:new Date().toISOString()}).eq("id",t.matchId);const n={credits:(s.profile.credits||0)+a,matches_played:(s.profile.matches_played||0)+1};d?n.wins=(s.profile.wins||0)+1:o?n.draws=(s.profile.draws||0)+1:n.losses=(s.profile.losses||0)+1,await $.from("users").update(n).eq("id",s.profile.id),await i.refreshProfile();const r=document.createElement("div");r.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:2000",r.innerHTML=`
  ...
  `;function a(n){const r=document.getElementById("mkt-content"),c=n==="buy"?l:o;if(c.length===0){r.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      ${c.map(p=>{var h,k,m;const f=(h=p.card)==null?void 0:h.player;if(!f)return"";const u=f.job==="GK"?f.note_g:f.job==="DEF"?f.note_d:f.job==="MIL"?f.note_m:f.note_a,x=bi[f.rarity],y=(i.profile.credits||0)>=p.price;return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px">
